...
{
    for (ContainerNode* node = starting_node; node; node = node->ParentOrShadowHostNode()) {
...
Node::InsertionNotificationRequest HTMLFormElement::InsertedInto(ContainerNode& insertion_point)
{
    HTMLElement::InsertedInto(insertion_point);
    LogAddElementIfIsolatedWorldAndInDocument("form", html_names::kMethodAttr, html_names::kActionAttr);
    if (insertion_point.isConnected()) {
        InvalidateShadowIncludingAncestorForms(ParentElementOrShadowRoot());
        GetDocument().MarkTopLevelFormsDirty();
        GetDocument().DidChangeFormRelatedElementDynamically(this, WebFormRelatedChangeType::kAdd);
    }
    return kInsertionDone;
}

template <class T> void NotifyFormRemovedFromTree(const T& elements, Node& root)
{
...
void HTMLFormElement::RemovedFrom(ContainerNode& insertion_point)
{
    ...
    if (has_elements_associated_by_parser_) {
        ...
        if (!listed_elements_are_dirty_) {
            ...
            CollectListedElements(&NodeTraversal::HighestAncestorOrSelf(insertion_point), elements);
            NotifyFormRemovedFromTree(elements, root);
        ...
        if (!image_elements_are_dirty_) {
            ...
            CollectImageElements(NodeTraversal::HighestAncestorOrSelf(insertion_point), images);
            NotifyFormRemovedFromTree(images, root);
    ...
    if (insertion_point.isConnected()) {
        ...
        GetDocument().MarkTopLevelFormsDirty();
        GetDocument().DidChangeFormRelatedElementDynamically(this, WebFormRelatedChangeType::kRemove);
    }
}
...
void HTMLFormElement::HandleLocalEvents(Event& event)
{
    ...
    if (event.eventPhase() != Event::PhaseType::kCapturingPhase && target_node && target_node != this
        && (event.type() == event_type_names::kSubmit || event.type() == event_type_names::kReset)) {
...
void HTMLFormElement::SubmitImplicitly(const Event& event, bool from_implicit_submission_trigger)
{
...
bool HTMLFormElement::ValidateInteractively()
{
    ...
    if (!CheckInvalidControlsAndCollectUnhandled(&unhandled_invalid_controls))
        return true;
    UseCounter::Count(GetDocument(), WebFeature::kFormValidationAbortedSubmission);
    ...
    for (const auto& unhandled : unhandled_invalid_controls) {
        if (unhandled->ValidationAnchorOrHostIsFocusable()) {
            ...
            UseCounter::Count(GetDocument(), WebFeature::kFormValidationShowedMessage);
            break;
        }
    }
    ...
    if (GetDocument().GetFrame()) {
        for (const auto& unhandled : unhandled_invalid_controls) {
            ...
            unhandled->ToHTMLElement().AddConsoleMessage(mojom::blink::ConsoleMessageSource::kRendering, mojom::blink::ConsoleMessageLevel::kError, message);
        }
    }
    return false;
}

void HTMLFormElement::PrepareForSubmission(const Event* event, HTMLFormControlElement* submit_button)
{
    ...
    if (!isConnected()) {
        GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(
            mojom::ConsoleMessageSource::kJavaScript, mojom::ConsoleMessageLevel::kWarning, "Form submission canceled because the form is not connected"));
        return;
    }

    if (GetExecutionContext()->IsSandboxed(network::mojom::blink::WebSandboxFlags::kForms)) {
    ...
    for (ListedElement* element : ListedElements()) {
        ...
        if (form_control_element && form_control_element->BlocksFormSubmission()) {
            UseCounter::Count(GetDocument(), WebFeature::kFormSubmittedWithUnclosedFormControl);
            if (RuntimeEnabledFeatures::UnclosedFormControlIsInvalidEnabled()) {
                String tag_name = To<HTMLFormControlElement>(element)->tagName();
                GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kSecurity, mojom::ConsoleMessageLevel::kError,
    ...
    bool should_submit;
    {
        base::AutoReset<bool> submit_event_handler_scope(&in_user_js_submit_event_, true);
        ...
        if (!skip_validation && !ValidateInteractively()) {
            ...
            submit_event_init->setSubmitter(submit_button ? &submit_button->ToHTMLElement() : nullptr);
            should_submit
                = DispatchEvent(*MakeGarbageCollected<SubmitEvent>(event_type_names::kSubmit, submit_event_init)) == DispatchEventResult::kNotCanceled;
        }
    }
    if (should_submit) {
...
void HTMLFormElement::requestSubmit(HTMLElement* submitter, ExceptionState& exception_state)
{
    ...
    if (submitter) {
        ...
        if (!control || !control->CanBeSuccessfulSubmitButton()) {
            exception_state.ThrowTypeError("The specified element is not a submit button.");
            return;
        }
        // 1.2. If submitter's form owner is not this form element, then throw a
        // "NotFoundError" DOMException.
        if (control->formOwner() != this) {
            exception_state.ThrowDOMException(DOMExceptionCode::kNotFoundError, "The specified element is not owned by this form element.");
            return;
        }
    }
...
void HTMLFormElement::ScheduleFormSubmission(const Event* event, HTMLFormControlElement* submit_button)
{
    ...
    FormSubmission* form_submission = FormSubmission::Create(this, attributes_, event, submit_button);
    if (!form_submission) {
    ...
    DCHECK(form_submission->Method() == FormSubmission::kPostMethod || form_submission->Method() == FormSubmission::kGetMethod);
    ...
    if (GetExecutionContext()->IsSandboxed(network::mojom::blink::WebSandboxFlags::kForms)) {
        ...
        GetExecutionContext()->AddConsoleMessage(
            MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kSecurity, mojom::blink::ConsoleMessageLevel::kError,
                "Blocked form submission to '" + form_submission->Action().ElidedString()
                    + "' because the form's frame is sandboxed and the 'allow-forms' "
                      "permission is not set."));
        return;
    }
    ...
    if (form_submission->Action().ProtocolIsJavaScript()) {
        ...
        // to javascript: URLs.
        if (!GetExecutionContext()->GetContentSecurityPolicy()->AllowFormAction(form_submission->Action())) {
            return;
        }
    }

    UseCounter::Count(GetDocument(), WebFeature::kFormsSubmitted);
    if (MixedContentChecker::IsMixedFormAction(GetDocument().GetFrame(), form_submission->Action())) {
        UseCounter::Count(GetDocument(), WebFeature::kMixedContentFormsSubmitted);
    }
    if (FastHasAttribute(html_names::kDisabledAttr)) {
        UseCounter::Count(GetDocument(), WebFeature::kFormDisabledAttributePresentAndSubmit);
    }
    ...
    if (auto* target_local_frame = DynamicTo<LocalFrame>(target_frame)) {
        ...
        if (target_local_frame == GetDocument().GetFrame() && !form_submission->Action().ProtocolIsJavaScript()) {
...
FormData* HTMLFormElement::ConstructEntryList(HTMLFormControlElement* submit_button, const WTF::TextEncoding& encoding)
{
    if (is_constructing_entry_list_) {
        return nullptr;
    }
    ...
    for (ListedElement* control : ListedElements()) {
        ...
        if (auto* input = DynamicTo<HTMLInputElement>(element)) {
            if (input->FormControlType() == FormControlType::kInputPassword && !input->Value().empty()) {
...
void HTMLFormElement::reset()
{
    ...
    if (DispatchEvent(*Event::CreateCancelableBubble(event_type_names::kReset)) != DispatchEventResult::kNotCanceled) {
        is_in_reset_function_ = false;
        return;
    }
...
void HTMLFormElement::ParseAttribute(const AttributeModificationParams& params)
{
    ...
    if (name == html_names::kActionAttr) {
        ...
        if (GetExecutionContext()
            && (GetExecutionContext()->GetSecurityContext().GetInsecureRequestPolicy() & mojom::blink::InsecureRequestPolicy::kUpgradeInsecureRequests)
                != mojom::blink::InsecureRequestPolicy::kLeaveInsecureRequestsAlone)
            return;
        KURL action_url = GetDocument().CompleteURL(attributes_.Action().empty() ? GetDocument().Url().GetString() : attributes_.Action());
        if (MixedContentChecker::IsMixedFormAction(GetDocument().GetFrame(), action_url)) {
...
{
    return attribute.GetName() == html_names::kActionAttr || HTMLElement::IsURLAttribute(attribute);
}

bool HTMLFormElement::HasLegalLinkAttribute(const QualifiedName& name) const
{
    return name == html_names::kActionAttr || HTMLElement::HasLegalLinkAttribute(name);
}
...
void HTMLFormElement::CollectListedElements(
    const Node* root, ListedElement::List& elements, ListedElement::List* elements_including_shadow_trees, bool in_shadow_tree) const
{
    CHECK(root);
    ...
    if (!in_shadow_tree) {
        ...
        if (elements_including_shadow_trees) {
            for (HTMLFormElement& nested_form : Traversal<HTMLFormElement>::DescendantsOf(*this)) {
    ...
    // root node.
    const bool nested_forms_have_form_associated_elements = base::ranges::any_of(nested_forms, [](const auto& form) {
        return form->has_elements_associated_by_form_attribute_
            || (form->has_elements_associated_by_parser_ && base::FeatureList::IsEnabled(features::kAutofillFixFieldsAssociatedWithNestedFormsByParser));
    });
    ...
    for (HTMLElement& element : Traversal<HTMLElement>::DescendantsOf(*root)) {
        ...
        if (elements_including_shadow_trees && element.AuthorShadowRoot() && (root_is_descendant || element.IsDescendantOf(this))) {
            CollectListedElements(element.AuthorShadowRoot(), elements, elements_including_shadow_trees,
                /*in_shadow_tree=*/true);
        }
    }
}
...
const ListedElement::List& HTMLFormElement::ListedElements(bool include_shadow_trees) const
{
    bool collect_shadow_inputs = include_shadow_trees && listed_elements_including_shadow_trees_are_dirty_;
    ...
    if (listed_elements_are_dirty_ || collect_shadow_inputs) {
        ...
        CollectListedElements(scope, mutable_this->listed_elements_, collect_shadow_inputs ? &mutable_this->listed_elements_including_shadow_trees_ : nullptr);
        mutable_this->listed_elements_are_dirty_ = false;
        mutable_this->listed_elements_including_shadow_trees_are_dirty_ = !collect_shadow_inputs;
    }
    return include_shadow_trees ? listed_elements_including_shadow_trees_ : listed_elements_;
}

void HTMLFormElement::CollectImageElements(Node& root, HeapVector<Member<HTMLImageElement>>& elements)
{
    elements.clear();
    for (HTMLImageElement& image : Traversal<HTMLImageElement>::DescendantsOf(root)) {
...
const HeapVector<Member<HTMLImageElement>>& HTMLFormElement::ImageElements()
{
    ...
    CollectImageElements(has_elements_associated_by_parser_ ? NodeTraversal::HighestAncestorOrSelf(*this) : *this, image_elements_);
    image_elements_are_dirty_ = false;
    return image_elements_;
}
...
String HTMLFormElement::action() const
{
    ...
    KURL action_url = document.CompleteURL(attributes_.Action().empty() ? document.Url().GetString() : attributes_.Action());
...
bool HTMLFormElement::CheckInvalidControlsAndCollectUnhandled(ListedElement::List* unhandled_invalid_controls)
{
    ...
    for (ListedElement* element : elements) {
        ...
        if (should_check_validity && !element->checkValidity(unhandled_invalid_controls) && element->Form() == this) {
...
Element* HTMLFormElement::ElementFromPastNamesMap(const AtomicString& past_name)
{
    ...
    if (IsA<HTMLImageElement>(*element)) {
        ...
        SECURITY_DCHECK(listed_element && ListedElements().Find(listed_element) != kNotFound);
    }
#endif
    return element;
}

void HTMLFormElement::AddToPastNamesMap(Element* element, const AtomicString& past_name)
{
    if (past_name.empty())
        return;
    if (!past_names_map_)
...
void HTMLFormElement::GetNamedElements(const AtomicString& name, HeapVector<Member<Element>>& named_items)
{
    ...
    if (named_items.size() && named_items.front() != element_from_past) {
        ...
        UseCounter::Count(GetDocument(), WebFeature::kFormNameAccessForPastNamesMap);
    }
}
...
{
    return !EqualIgnoringASCIICase(FastGetAttribute(html_names::kAutocompleteAttr), "off");
}
...
V8UnionElementOrRadioNodeList* HTMLFormElement::AnonymousNamedGetter(const AtomicString& name)
{
    ...
    bool only_match_img = !elements.empty() && IsA<HTMLImageElement>(*elements.front());
    if (only_match_img) {
        UseCounter::Count(GetDocument(), WebFeature::kFormNameAccessForImageElement);
        ...
        for (auto& element : elements) {
            if (IsA<HTMLImageElement>(*element) && !element->IsDescendantOf(this)) {
                UseCounter::Count(GetDocument(), WebFeature::kFormNameAccessForNonDescendantImageElement);
                break;
            }
        }
    }
    ...
    return MakeGarbageCollected<V8UnionElementOrRadioNodeList>(GetRadioNodeList(name, only_match_img));
}

bool HTMLFormElement::NamedPropertyQuery(const AtomicString& name, ExceptionState&)
{
...
void HTMLFormElement::UseCountPropertyAccess(v8::Local<v8::Name>& v8_property_name, const v8::PropertyCallbackInfo<v8::Value>& info)
{
    bool hasPropertyInPrototypeChain = !info.Holder()->GetRealNamedPropertyInPrototypeChain(info.GetIsolate()->GetCurrentContext(), v8_property_name).IsEmpty();

    UseCounter::Count(GetDocument(),
        hasPropertyInPrototypeChain ? WebFeature::kDOMClobberedShadowedFormPropertyAccessed : WebFeature::kDOMClobberedNotShadowedFormPropertyAccessed);
}
