...
public:
    ...
    {
        return attributes_.EncodingType();
    }
    ...
    FormSubmission::SubmitMethod Method() const
    {
        return attributes_.Method();
    }
    ...
    // descendant of) `this`.
    const ListedElement::List& ListedElements(bool include_shadow_trees = false) const;
    ...
    FormData* ConstructEntryList(HTMLFormControlElement* submit_button, const WTF::TextEncoding& encoding);

    void InvalidateListedElementsIncludingShadowTrees();
    void UseCountPropertyAccess(v8::Local<v8::Name>&, const v8::PropertyCallbackInfo<v8::Value>&);
...
private:
    ...
    void ScheduleFormSubmission(const Event*, HTMLFormControlElement* submit_button);

    void CollectListedElements(
        const Node* root, ListedElement::List& elements, ListedElement::List* elements_including_shadow_trees = nullptr, bool in_shadow_tree = false) const;
