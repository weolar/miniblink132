...
protected:
    void SetUp() override
    {
        ...
        element->setInnerHTML("<option><option><option><option style='display:none;'><option "
...
TEST_F(ExternalPopupMenuDisplayNoneItemsTest, PopupMenuInfoSizeTest)
{
    ...
    ExternalPopupMenu::GetPopupMenuInfo(*owner_element_, &item_height, &font_size, &selected_item, &menu_items, &right_aligned, &allow_multiple_selection);
...
TEST_F(ExternalPopupMenuDisplayNoneItemsTest, IndexMappingTest)
{
    ...
    // and vice-versa.
    EXPECT_EQ(4, ExternalPopupMenu::ToExternalPopupMenuItemIndex(6, *owner_element_));
    EXPECT_EQ(6, ExternalPopupMenu::ToPopupMenuItemIndex(4, *owner_element_));

    // Invalid index, methods should return -1.
    EXPECT_EQ(-1, ExternalPopupMenu::ToExternalPopupMenuItemIndex(8, *owner_element_));
...
{
    EXPECT_EQ(0, ExternalPopupMenu::ToExternalPopupMenuItemIndex(0, *owner_element_));
    EXPECT_EQ(1, ExternalPopupMenu::ToExternalPopupMenuItemIndex(1, *owner_element_));
#if BUILDFLAG(IS_ANDROID)
    EXPECT_EQ(-1, ExternalPopupMenu::ToExternalPopupMenuItemIndex(2, *owner_element_));
    EXPECT_EQ(2, ExternalPopupMenu::ToExternalPopupMenuItemIndex(3, *owner_element_));
#else
    EXPECT_EQ(2, ExternalPopupMenu::ToExternalPopupMenuItemIndex(2, *owner_element_));
    EXPECT_EQ(3, ExternalPopupMenu::ToExternalPopupMenuItemIndex(3, *owner_element_));
#endif
...
public:
        ...
        int32_t selected_item, Vector<mojom::blink::MenuItemPtr> menu_items, bool right_aligned, bool allow_multiple_selection) override
    {
        ...
        popup_client_.set_disconnect_handler(WTF::BindOnce(&TestLocalFrameExternalPopupClient::Reset, WTF::Unretained(this)));
    ...
    const gfx::Rect& ShownBounds() const
    {
        return bounds_;
    }
...
protected:
    void SetUp() override
    {
        frame_host_.Init(web_frame_client_.GetRemoteNavigationAssociatedInterfaces());
    ...
    void RegisterMockedURLLoad(const std::string& file_name)
    {
        ...
        url_test_helpers::RegisterMockedURLLoadFromBase(
            WebString::FromUTF8(base_url_), test::CoreTestDataPath("popup"), WebString::FromUTF8(file_name), WebString::FromUTF8("text/html"));
    }
    ...
    bool IsBound() const
    {
        return frame_host_.IsBound();
    }

    int32_t SelectedItem() const
    {
        return frame_host_.SelectedItem();
    }

    const gfx::Rect& ShownBounds() const
    {
        return frame_host_.ShownBounds();
    }
    ...
    void WaitUntilShowedPopup()
    {
        frame_host_.WaitUntilShowedPopup();
    }
...
TEST_F(ExternalPopupMenuTest, PopupAccountsForVisualViewportTransform)
{
    ...
    auto* select = To<HTMLSelectElement>(MainFrame()->GetFrame()->GetDocument()->getElementById(AtomicString("select")));
    ...
    const int expected_x = (rect_in_document.x() - scroll_delta.x()) * kScaleFactor;
    const int expected_y = (rect_in_document.y() - scroll_delta.y()) * kScaleFactor;
...
#ifdef OS_ANDROID
#define MAYBE_PopupAccountsForDeviceScaleFactor DISABLED_PopupAccountsForDeviceScaleFactor
#else
#define MAYBE_PopupAccountsForDeviceScaleFactor PopupAccountsForDeviceScaleFactor
#endif
...
TEST_F(ExternalPopupMenuTest, ShowPopupThenNavigate)
{
    ...
    select = To<HTMLSelectElement>(document->getElementById(AtomicString("select")));
    EXPECT_FALSE(select);
...
TEST_F(ExternalPopupMenuTest, EmptySelect)
{
    ...
    auto* select = To<HTMLSelectElement>(MainFrame()->GetFrame()->GetDocument()->getElementById(AtomicString("emptySelect")));
    EXPECT_TRUE(select);
    select->click();
}
...
TEST_F(ExternalPopupMenuTest, RemoveOnChange)
{
    ...
    // Show the popup-menu.
    auto* document = MainFrame()->GetFrame()->GetDocument();
    auto* select = To<HTMLSelectElement>(document->getElementById(AtomicString("s")));
    auto* layout_object = select->GetLayoutObject();
    ASSERT_TRUE(layout_object);
...
TEST_F(ExternalPopupMenuTest, RemoveFrameOnChange)
{
    ...
    // Open a popup.
    auto* iframe = To<HTMLIFrameElement>(MainFrame()->GetFrame()->GetDocument()->QuerySelector(AtomicString("iframe")));
    auto* select = To<HTMLSelectElement>(iframe->contentDocument()->QuerySelector(AtomicString("select")));
    auto* layout_object = select->GetLayoutObject();
    ASSERT_TRUE(layout_object);
