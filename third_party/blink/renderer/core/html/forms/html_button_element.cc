...
LayoutObject* HTMLButtonElement::CreateLayoutObject(const ComputedStyle& style)
{
    ...
    if (display == EDisplay::kInlineGrid || display == EDisplay::kGrid || display == EDisplay::kInlineFlex || display == EDisplay::kFlex
        || display == EDisplay::kInlineLayoutCustom || display == EDisplay::kLayoutCustom)
...
void HTMLButtonElement::ParseAttribute(const AttributeModificationParams& params)
{
        ...
        if (EqualIgnoringASCIICase(params.new_value, "reset")) {
            ...
            if (!params.new_value.IsNull()) {
                if (params.new_value.empty()) {
                    UseCounter::Count(GetDocument(), WebFeature::kButtonTypeAttrEmptyString);
...
Element* HTMLButtonElement::commandForElement()
{
    ...
    if (!IsInTreeScope() || IsDisabledFormControl() || (Form() && CanBeSuccessfulSubmitButton())) {
        return nullptr;
    }
...
AtomicString HTMLButtonElement::command() const
{
    ...
    const AtomicString& attribute_value = FastGetAttribute(html_names::kCommandAttr);
...
void HTMLButtonElement::DefaultEventHandler(Event& event)
{
    if (event.type() == event_type_names::kDOMActivate) {
        ...
        if (auto* command_target = commandForElement()) {
            ...
            bool is_valid_builtin = command_target->IsValidBuiltinCommand(*this, action);
            bool should_dispatch = is_valid_builtin || action == CommandEventType::kCustom;
            if (should_dispatch) {
                Event* commandEvent = CommandEvent::Create(event_type_names::kCommand, command(), this);
...
{
    if (!IsDisabledFormControl() && Form() && (type_ == kSubmit || type_ == kReset))
        return true;
...
void HTMLButtonElement::AccessKeyAction(SimulatedClickCreationScope creation_scope)
{
...
{
    return attribute.GetName() == html_names::kFormactionAttr || HTMLFormControlElement::IsURLAttribute(attribute);
}
...
Node::InsertionNotificationRequest HTMLButtonElement::InsertedInto(ContainerNode& insertion_point)
{
    InsertionNotificationRequest request = HTMLFormControlElement::InsertedInto(insertion_point);
    LogAddElementIfIsolatedWorldAndInDocument("button", html_names::kTypeAttr, html_names::kFormmethodAttr, html_names::kFormactionAttr);
    return request;
}

void HTMLButtonElement::DispatchBlurEvent(Element* new_focused_element, mojom::blink::FocusType type, InputDeviceCapabilities* source_capabilities)
{
    ...
    HTMLFormControlElement::DispatchBlurEvent(new_focused_element, type, source_capabilities);
}
