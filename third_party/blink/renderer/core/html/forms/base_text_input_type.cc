...
bool BaseTextInputType::TooLong(const String& value, TextControlElement::NeedsToCheckDirtyFlag check) const
{
...
bool BaseTextInputType::TooShort(const String& value, TextControlElement::NeedsToCheckDirtyFlag check) const
{
...
{
    const AtomicString& raw_pattern = GetElement().FastGetAttribute(html_names::kPatternAttr);
    ...
    if (!regexp_ || pattern_for_regexp_ != raw_pattern) {
        ...
        ScriptRegexp* raw_regexp
            = MakeGarbageCollected<ScriptRegexp>(isolate, raw_pattern, kTextCaseSensitive, MultilineMode::kMultilineDisabled, unicode_mode);
        if (!raw_regexp->IsValid()) {
            GetElement().GetDocument().AddConsoleMessage(
                MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kRendering, mojom::blink::ConsoleMessageLevel::kError,
                    "Pattern attribute value " + raw_pattern + " is not a valid regular expression: " + raw_regexp->ExceptionMessage()));
            regexp_ = raw_regexp;
            pattern_for_regexp_ = raw_pattern;
            return false;
        }
        String pattern = "^(?:" + raw_pattern + ")$";
        regexp_ = MakeGarbageCollected<ScriptRegexp>(isolate, pattern, kTextCaseSensitive, MultilineMode::kMultilineDisabled, unicode_mode);
        pattern_for_regexp_ = raw_pattern;
