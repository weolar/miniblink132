...
{
    WebKeyboardEvent web_event(WebInputEvent::Type::kChar, 0, base::TimeTicks() + base::Milliseconds(500));
    web_event.text[0] = ' ';
...
{
    {
        ...
        web_event.text[0] = ' ';
        auto& event = *KeyboardEvent::Create(web_event, nullptr);
        type_ahead_.HandleEvent(event, event.charCode(), TypeAhead::kMatchPrefix | TypeAhead::kCycleFirstChar);
...

    {
        ...
        WebKeyboardEvent web_event(WebInputEvent::Type::kChar, 0, base::TimeTicks() + base::Milliseconds(1500));
        web_event.text[0] = ' ';
        auto& event = *KeyboardEvent::Create(web_event, nullptr);
        EXPECT_TRUE(type_ahead_.HasActiveSession(event));
    }

    {
        ...
        WebKeyboardEvent web_event(WebInputEvent::Type::kChar, 0, base::TimeTicks() + base::Milliseconds(1501));
        web_event.text[0] = ' ';
        auto& event = *KeyboardEvent::Create(web_event, nullptr);
        EXPECT_FALSE(type_ahead_.HasActiveSession(event));
    }
