...
public:
    ...
    public:
        ...
        StepDescription(int default_step, int default_step_base, int step_scale_factor, StepValueShouldBe step_value_should_be = kStepValueShouldBeReal)
    ...
    StepRange(const Decimal& step_base, const Decimal& minimum, const Decimal& maximum, bool has_range_limitations, bool supports_reversed_range,
        const Decimal& step, const StepDescription&);

    Decimal AlignValueForStep(const Decimal& current_value, const Decimal& new_value) const;
    ...
    Decimal DefaultValue() const
    {
        return ClampValue((minimum_ + maximum_) / 2);
    }
