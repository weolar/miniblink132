...
AutoCompleteCategory GetAutoCompleteCategory(const AtomicString& token)
{
    ...
    DEFINE_STATIC_LOCAL(Map, category_map,
        ({
...
HTMLFormControlElementWithState::HTMLFormControlElementWithState(const QualifiedName& tag_name, Document& doc)
...
String HTMLFormControlElementWithState::IDLExposedAutofillValue() const
{
    ...
    // default.
    if ((category == AutoCompleteCategory::kOff || category == AutoCompleteCategory::kAutomatic) && IsWearingAutofillAnchorMantle()) {
        return g_empty_string;
    }
    ...
    if (category == AutoCompleteCategory::kCredential) {
        ...
        if (index != 0) {
            ...
            if (category != AutoCompleteCategory::kNormal && category != AutoCompleteCategory::kContact) {
                return g_empty_string;
            }
    ...
    if (index != 0) {
        ...
        if (category == AutoCompleteCategory::kContact) {
            ...
            if (contact == "home" || contact == "work" || contact == "mobile" || contact == "fax" || contact == "pager") {
...
void HTMLFormControlElementWithState::setIDLExposedAutofillValue(const String& autocomplete_value)
{
...
bool HTMLFormControlElementWithState::ShouldSaveAndRestoreFormControlState() const
{
    ...
    if (RuntimeEnabledFeatures::FormControlRestoreStateIfAutocompleteOffEnabled()) {
        return ShouldAutocomplete();
    }
    if (Form() && !Form()->ShouldAutocomplete()) {
        return false;
    }
    if (EqualIgnoringASCIICase(FastGetAttribute(html_names::kAutocompleteAttr), "off")) {
        return false;
    }
    return true;
}
...
{
    return (UserHasEditedTheFieldAndBlurred() || force_user_valid_) && MatchesValidityPseudoClasses() && !ListedElement::IsValidElement();
}
