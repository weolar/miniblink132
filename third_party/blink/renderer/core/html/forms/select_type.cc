...
bool CanAssignToSelectSlot(const Node& node)
{
    ...
    return node.HasTagName(html_names::kOptionTag) || node.HasTagName(html_names::kOptgroupTag) || node.HasTagName(html_names::kHrTag);
}
...
bool CanAssignToCustomizableSelectSlot(const Node& node)
{
    ...
    // customizable select.
    return IsA<HTMLOptionElement>(node) || IsA<HTMLOptGroupElement>(node) || IsA<HTMLHRElement>(node) || IsA<HTMLSpanElement>(node)
...
public:
    ...
    void ShowPopoverInternal(Element* invoker, ExceptionState* exception_state) override
    {
        ...
        if (auto* select = ParentSelect()) {
            ...
            if (AXObjectCache* cache = select->GetDocument().ExistingAXObjectCache()) {
                cache->DidShowMenuListPopup(select);
            }
        }
    }

    void HidePopoverInternal(HidePopoverFocusBehavior focus_behavior, HidePopoverTransitionBehavior event_firing, ExceptionState* exception_state) override
    {
        HTMLDivElement::HidePopoverInternal(focus_behavior, event_firing, exception_state);
        if (auto* select = ParentSelect()) {
            // MenuListSelectType::ManuallyAssignSlots changes behavior based on
            // whether the popover is opened or closed.
            select->GetShadowRoot()->SetNeedsAssignmentRecalc();
    ...
    {
        InsertionNotificationRequest return_value = HTMLDivElement::InsertedInto(container);
    ...
    void DidRecalcStyle(const StyleRecalcChange change) override
    {
        ...
        if (auto* style = GetComputedStyle()) {
            if (style->EffectiveAppearance() == ControlPart::kBaseSelectPart) {
                UseCounter::Count(GetDocument(), WebFeature::kSelectElementPickerAppearanceBaseSelect);
            }
        }
    }
...
private:
    HTMLSelectElement* ParentSelect()
    {
        if (auto* shadowroot = DynamicTo<ShadowRoot>(parentNode())) {
            HTMLSelectElement* select = DynamicTo<HTMLSelectElement>(shadowroot->host());
            CHECK(select);
            return select;
        }
        return nullptr;
...
public:
    ...
    void DidSelectOption(HTMLOptionElement* element, HTMLSelectElement::SelectOptionFlags flags, bool should_update_popup) override;
    ...
    void UpdateTextStyle() override
    {
        UpdateTextStyleInternal();
    }
    ...
    HTMLSelectElement::SelectAutofillPreviewElement* GetAutofillPreviewElement() const override;
...
bool MenuListSelectType::DefaultEventHandler(const Event& event)
{
    ...
    if (IsAppearanceBasePicker() && event.HasEventPath()) {
        ...
        if (auto* button = SlottedButton()) {
            ...
            target_is_button = event.target() == select_ || event.target() == &InnerElement();
        }
        if (!target_is_button) {
            return false;
        }
    ...
    const int ignore_modifiers = WebInputEvent::kShiftKey | WebInputEvent::kControlKey | WebInputEvent::kAltKey | WebInputEvent::kMetaKey;
    ...
    if (event.type() == event_type_names::kKeydown) {
        ...
        // mode Mac
        if (LayoutTheme::GetTheme().PopsMenuByArrowKeys() && !IsSpatialNavigationEnabled(select_->GetDocument().GetFrame()))
            return false;
        ...
        if (handled && option) {
            select_->SelectOption(option,
                HTMLSelectElement::kDeselectOtherOptionsFlag | HTMLSelectElement::kMakeOptionDirtyFlag | HTMLSelectElement::kDispatchInputAndChangeEventFlag);
        }
        return handled;
    }
    ...
    if (event.type() == event_type_names::kKeypress) {
        ...
        if (key_code == ' ' && IsSpatialNavigationEnabled(select_->GetDocument().GetFrame())) {
    ...
    if (event.type() == event_type_names::kMousedown && mouse_event && mouse_event->button() == static_cast<int16_t>(WebPointerProperties::Button::kLeft)) {
        InputDeviceCapabilities* source_capabilities
            = select_->GetDocument().domWindow()->GetInputDeviceCapabilities()->FiresTouchEvents(mouse_event->FromTouch());
        select_->Focus(FocusParams(
            SelectionBehaviorOnFocus::kRestore, mojom::blink::FocusType::kMouse, source_capabilities, FocusOptions::Create(), FocusTrigger::kUserGesture));
        if (select_->GetLayoutObject() && !will_be_destroyed_ && !select_->IsDisabledFormControl()) {
            if (PopupIsVisible()) {
                HidePopup();
            } else {
                ...
                ShowPopup(mouse_event->FromTouch() ? PopupMenu::kTouch : PopupMenu::kOther);
            }
        }
        return true;
    }
...
bool MenuListSelectType::ShouldOpenPopupForKeyDownEvent(const KeyboardEvent& event)
{
    ...
    // keyboard behavior after a resolution here:
    // https://github.com/openui/open-ui/issues/1087
    if (IsAppearanceBaseButton() && (key == keywords::kArrowDown || key == keywords::kArrowUp || key == keywords::kArrowLeft || key == keywords::kArrowRight)) {
        return true;
    }

    return ((layout_theme.PopsMenuByArrowKeys() && (key == keywords::kArrowDown || key == keywords::kArrowUp))
        || ((key == keywords::kArrowDown || key == keywords::kArrowUp) && event.altKey()) || ((!event.altKey() && !event.ctrlKey() && key == "F4")));
}

bool MenuListSelectType::ShouldOpenPopupForKeyPressEvent(const KeyboardEvent& event)
{
    LayoutTheme& layout_theme = LayoutTheme::GetTheme();
    ...
    return ((key_code == ' ' && !select_->type_ahead_.HasActiveSession(event)) || (layout_theme.PopsMenuByReturnKey() && key_code == '\r'));
}
...
void MenuListSelectType::CreateShadowSubtree(ShadowRoot& root)
{
    ...
    if (RuntimeEnabledFeatures::CustomizableSelectEnabled()) {
        ...
        popover_options_slot_->SetIdAttribute(shadow_element_names::kSelectPopoverOptions);
        popover_->AppendChild(popover_options_slot_);

        autofill_popover_ = MakeGarbageCollected<HTMLSelectElement::SelectAutofillPreviewElement>(doc, select_);
        autofill_popover_->setAttribute(html_names::kPopoverAttr, keywords::kManual);
        if (!RuntimeEnabledFeatures::PopoverAnchorRelationshipsEnabled()) {
            autofill_popover_->SetImplicitAnchor(select_);
        }
        autofill_popover_->SetShadowPseudoId(shadow_element_names::kSelectAutofillPreview);
        root.appendChild(autofill_popover_);

        autofill_popover_text_ = MakeGarbageCollected<HTMLDivElement>(doc);
        autofill_popover_text_->SetShadowPseudoId(shadow_element_names::kSelectAutofillPreviewText);
...
HTMLButtonElement* MenuListSelectType::SlottedButton() const
{
    ...
    for (auto* child = select_->firstChild(); child; child = child->nextSibling()) {
...
HTMLSelectElement::SelectAutofillPreviewElement* MenuListSelectType::GetAutofillPreviewElement() const
{
...
void MenuListSelectType::ShowPopup(PopupMenu::ShowEventType type)
{
    ...
    if (document.GetFrame()->LocalFrameRoot().IsOutermostMainFrame()) {
        gfx::Rect visual_viewport_rect = document.GetPage()->GetVisualViewport().RootFrameToViewport(local_root_rect);
        visual_viewport_rect.Intersect(gfx::Rect(document.GetPage()->GetVisualViewport().Size()));
    ...
    if (RuntimeEnabledFeatures::SelectPopupLessUpdatesEnabled()) {
        ...
        if (RuntimeEnabledFeatures::CSSPseudoOpenClosedEnabled()) {
            select_->GetDocument().UpdateStyleAndLayoutForNode(select_, DocumentUpdateReason::kPagePopup);
        }
    }

    if (!popup_) {
        popup_ = document.GetPage()->GetChromeClient().OpenPopupMenu(*document.GetFrame(), *select_);
    }
    if (!popup_) {
        if (RuntimeEnabledFeatures::SelectPopupLessUpdatesEnabled()) {
...
void MenuListSelectType::DidSelectOption(HTMLOptionElement* element, HTMLSelectElement::SelectOptionFlags flags, bool should_update_popup)
{
    // Need to update last_on_change_option_ before UpdateFromElement().
    const bool should_dispatch_events = (flags & HTMLSelectElement::kDispatchInputAndChangeEventFlag) && select_->last_on_change_option_ != element;
...
void MenuListSelectType::DidSetSuggestedOption(HTMLOptionElement* option)
{
    ...
    if (IsAppearanceBaseButton()) {
        if (option) {
            ...
            autofill_popover_->HidePopoverInternal(HidePopoverFocusBehavior::kNone, HidePopoverTransitionBehavior::kNoEventsNoWaiting,
                /*exception_state=*/nullptr);
        }
    }
}
...
{
    if (auto* style = select_->GetComputedStyle()) {
        bool is_appearance_base_select = style->EffectiveAppearance() == ControlPart::kBaseSelectPart;
        if (is_appearance_base_select_ != is_appearance_base_select) {
            ...
            select_->GetShadowRoot()->SetDelegatesFocus(is_appearance_base_select && SlottedButton());
        }
        if (is_appearance_base_select) {
            UseCounter::Count(select_->GetDocument(), WebFeature::kSelectElementAppearanceBaseSelect);
        }
    }
...
String MenuListSelectType::UpdateTextStyleInternal()
{
    ...
    if (select_->IsMultiple()) {
        ...
        if (selected_count == 1) {
            ...
            String localized_number_string = locale.ConvertToLocalizedNumber(String::Number(selected_count));
            text = locale.QueryString(IDS_FORM_SELECT_MENU_LIST_TEXT, localized_number_string);
            DCHECK(!option_style);
        }
    } else {
    ...
    if (!IsAppearanceBaseButton()) {
        ...
        if (inner_style && option_style
            && ((option_style->Direction() != inner_style->Direction() || option_style->GetUnicodeBidi() != inner_style->GetUnicodeBidi()
            ...
            if (auto* inner_layout = inner_element.GetLayoutObject()) {
                inner_layout->SetModifiedStyleOutsideStyleRecalc(new_style, LayoutObject::ApplyStyleChanges::kYes);
            } else {
...
void MenuListSelectType::DidUpdateActiveOption(HTMLOptionElement* option)
{
    ...
    HTMLOptionElement* prev_selected_option = select_->OptionAtListIndex(ax_menulist_last_active_index_);
...
{
    if (auto* option = select_->OptionAtListIndex(select_->index_to_select_on_cancel_))
        return option;
...
void MenuListSelectType::MaximumOptionWidthMightBeChanged() const
{
    if (LayoutObject* layout_object = select_->GetLayoutObject()) {
        layout_object->SetNeedsLayoutAndIntrinsicWidthsRecalc(layout_invalidation_reason::kMenuOptionsChanged);
    }
}
...
public:
    explicit PopupUpdater(MenuListSelectType& select_type, HTMLSelectElement& select)
...
private:
    ...
    void UpdateSelectedState(HTMLOptionElement* clicked_option, SelectionMode mode);
...
bool ListBoxSelectType::DefaultEventHandler(const Event& event)
{
    ...
    if (event.type() == event_type_names::kGesturetap && gesture_event) {
        ...
        if (HTMLOptionElement* option = EventTargetOption(*gesture_event)) {
            if (!select_->IsDisabledFormControl()) {
                UpdateSelectedState(option, gesture_event->shiftKey() ? SelectionMode::kRange : SelectionMode::kNotChangeOthers);
                ListBoxOnChange();
            }
            return true;
        }
        ...
        if (HTMLOptionElement* option = EventTargetOption(*mouse_event)) {
            if (!option->IsDisabledFormControl()) {
                ...
                UpdateSelectedState(option,
                    mouse_event->shiftKey() ? SelectionMode::kRange
                        : meta_or_ctrl      ? SelectionMode::kNotChangeOthers
    ...
    if (event.type() == event_type_names::kMousemove && mouse_event) {
        if (mouse_event->button() != static_cast<int16_t>(WebPointerProperties::Button::kLeft) || !mouse_event->ButtonDown())
            return false;
        ...
        if (auto* layout_object = select_->GetLayoutObject()) {
            if (Page* page = select_->GetDocument().GetPage()) {
                page->GetAutoscrollController().StartAutoscrollForSelection(layout_object);
            }
        }
    ...
    if (event.type() == event_type_names::kMouseup && mouse_event && mouse_event->button() == static_cast<int16_t>(WebPointerProperties::Button::kLeft)
        && select_->GetLayoutObject()) {
        auto* page = select_->GetDocument().GetPage();
        if (page && page->GetAutoscrollController().AutoscrollInProgressFor(select_->GetLayoutBox()))
    ...
    if (event.type() == event_type_names::kKeydown) {
        ...
        const PhysicalToLogical<const AtomicString*> key_mapper(
            select_->GetComputedStyle()->GetWritingDirection(), &keywords::kArrowUp, &keywords::kArrowRight, &keywords::kArrowDown, &keywords::kArrowLeft);
        ...
        if (!active_selection_end_) {
            ...
            if (key == *key_next || key == keywords::kPageDown) {
                ...
                if (key == *key_next) {
                    ...
                    end_option = NextSelectableOptionPageAway(start_option, kSkipForwards);
                }
                ...
                if (key == *key_previous) {
                    ...
                    end_option = NextSelectableOptionPageAway(start_option, kSkipBackwards);
                }
            }
        } else {
            ...
            if (key == *key_next) {
                ...
                end_option = NextSelectableOptionPageAway(active_selection_end_, kSkipForwards);
                handled = true;
            } else if (key == keywords::kPageUp) {
                end_option = NextSelectableOptionPageAway(active_selection_end_, kSkipBackwards);
                handled = true;
            }
        }
        ...
        if (IsSpatialNavigationEnabled(select_->GetDocument().GetFrame())) {
            ...
            if (key == keywords::kArrowLeft || key == keywords::kArrowRight
                || ((key == keywords::kArrowDown || key == keywords::kArrowUp) && end_option == active_selection_end_)) {
                return false;
            }
        }
        ...
        if (select_->is_multiple_ && keyboard_event->keyCode() == ' ' && is_control_key && active_selection_end_) {
        ...
        if (end_option && handled) {
            ...
            bool select_new_item = !select_->is_multiple_ || keyboard_event->shiftKey()
                || (!IsSpatialNavigationEnabled(select_->GetDocument().GetFrame()) && !is_in_non_contiguous_selection_);
            if (select_new_item)
                active_selection_state_ = true;
            // If the anchor is uninitialized, or if we're going to deselect all
            // other options, then set the anchor index equal to the end index.
            bool deselect_others = !select_->is_multiple_ || (!keyboard_event->shiftKey() && select_new_item);
...
void ListBoxSelectType::DidSelectOption(HTMLOptionElement* element, HTMLSelectElement::SelectOptionFlags flags, bool should_update_popup)
{
    ...
    if (element) {
        ...
        const bool deselect_other_options = flags & HTMLSelectElement::kDeselectOtherOptionsFlag;
...
void ListBoxSelectType::SaveLastSelection()
{
    ...
    for (auto& element : select_->GetListItems()) {
        ...
        last_on_change_selection_.push_back(option_element && option_element->Selected());
    }
}
...
void ListBoxSelectType::UpdateMultiSelectFocus()
{
    ...
    for (auto* const option : select_->GetOptionList()) {
        if (option->IsDisabledFormControl() || !option->GetLayoutObject())
            continue;
        bool is_focused = (option == active_selection_end_) && is_in_non_contiguous_selection_;
...
void ListBoxSelectType::ScrollToOption(HTMLOptionElement* option)
{
    ...
    if (!has_pending_task) {
        select_->GetDocument()
            ...
            ->PostTask(FROM_HERE, WTF::BindOnce(&ListBoxSelectType::ScrollToOptionTask, WrapPersistent(this)));
    }
}
...
void ListBoxSelectType::ScrollToOptionTask()
{
    ...
    select_->GetDocument().UpdateStyleAndLayoutForNode(select_, DocumentUpdateReason::kScroll);
    if (!select_->GetLayoutObject())
        return;
    ...
    box->Layer()->GetScrollableArea()->ScrollIntoView(bounds, PhysicalBoxStrut(),
        scroll_into_view_util::CreateScrollIntoViewParams(ScrollAlignment::ToEdgeIfNeeded(), ScrollAlignment::ToEdgeIfNeeded(),
            mojom::blink::ScrollType::kProgrammatic, false, mojom::blink::ScrollBehavior::kInstant));
}
...
HTMLOptionElement* ListBoxSelectType::NextSelectableOptionPageAway(HTMLOptionElement* start_option, SkipDirection direction) const
{
    ...
    int skip_amount = page_size + ((direction == kSkipForwards) ? start_index : (edge_index - start_index));
...
void ListBoxSelectType::UpdateSelectedState(HTMLOptionElement* clicked_option, SelectionMode mode)
{
    ...
    active_selection_state_ = !(clicked_option->Selected() && mode == SelectionMode::kNotChangeOthers);
    ...
    if (mode == SelectionMode::kDeselectOthers) {
        bool did_deselect_others = select_->DeselectItemsWithoutValidation(clicked_option);
        // In a multi-select, if nothing else could be deselected,
        // deselect the (already selected) clicked option instead.
        if (select_->is_multiple_ && !did_deselect_others && clicked_option->Selected() && RuntimeEnabledFeatures::MultiSelectDeselectWhenOnlyOptionEnabled()) {
...
void ListBoxSelectType::UpdateListBoxSelection(bool deselect_other_options, bool scroll)
{
    ...
    for (auto* const option : select_->GetOptionList()) {
        ...
        } else if (deselect_other_options || i >= static_cast<int>(cached_state_for_active_selection_.size())) {
...
void ListBoxSelectType::ListBoxOnChange()
{
    ...
    if (last_on_change_selection_.empty() || last_on_change_selection_.size() != items.size()) {
        select_->DispatchChangeEvent();
        return;
    }
...
void ListBoxSelectType::ManuallyAssignSlots()
{
    ...
    for (Node& child : NodeTraversal::ChildrenOf(*select_)) {
        if (child.IsSlotable()
            && (CanAssignToSelectSlot(child) || (RuntimeEnabledFeatures::CustomizableSelectEnabled() && CanAssignToCustomizableSelectSlot(child)))) {
            option_nodes.push_back(child);
        }
    }
    CHECK(option_slot_);
...
HTMLSelectElement::SelectAutofillPreviewElement* ListBoxSelectType::GetAutofillPreviewElement() const
{
...
HTMLOptionElement* SelectType::NextValidOption(int list_index, SkipDirection direction, int skip) const
{
    ...
    for (list_index += direction; list_index >= 0 && list_index < size; list_index += direction) {
        --skip;
...
HTMLOptionElement* SelectType::NextSelectableOption(HTMLOptionElement* start_option) const
{
    return NextValidOption(start_option ? start_option->ListIndex() : -1, kSkipForwards, 1);
}

HTMLOptionElement* SelectType::PreviousSelectableOption(HTMLOptionElement* start_option) const
{
    return NextValidOption(start_option ? start_option->ListIndex() : select_->GetListItems().size(), kSkipBackwards, 1);
}
