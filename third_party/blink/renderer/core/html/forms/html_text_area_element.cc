    ...
    , is_placeholder_visible_(false)
{
    ...
    if (!is_default_font_prewarmed_) {
        if (Settings* settings = document.GetSettings()) {
            ...
            FontCache::PrewarmFamily(settings->GetGenericFontFamilySettings().Fixed(LayoutLocale::GetDefault().GetScript()));
...
int HTMLTextAreaElement::scrollWidth()
{
    ...
    if (!box || !editor_box)
        return TextControlElement::scrollWidth();
    LayoutUnit width = editor_box->ClientWidth() + box->PaddingLeft() + box->PaddingRight();
    return AdjustForAbsoluteZoom::AdjustLayoutUnit(width, box->StyleRef()).Round();
}
...
int HTMLTextAreaElement::scrollHeight()
{
    ...
    if (!box || !editor_box)
        return TextControlElement::scrollHeight();
    LayoutUnit height = editor_box->ClientHeight() + box->PaddingTop() + box->PaddingBottom();
    return AdjustForAbsoluteZoom::AdjustLayoutUnit(height, box->StyleRef()).Round();
}
...
void HTMLTextAreaElement::CollectStyleForPresentationAttribute(const QualifiedName& name, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
    if (name == html_names::kWrapAttr) {
        if (ShouldWrapText()) {
            // Longhands of `white-space: pre-wrap`.
            AddPropertyToPresentationAttributeStyle(style, CSSPropertyID::kWhiteSpaceCollapse, CSSValueID::kPreserve);
            AddPropertyToPresentationAttributeStyle(style, CSSPropertyID::kTextWrapMode, CSSValueID::kWrap);
            ...
            AddPropertyToPresentationAttributeStyle(style, CSSPropertyID::kWhiteSpaceCollapse, CSSValueID::kPreserve);
            AddPropertyToPresentationAttributeStyle(style, CSSPropertyID::kTextWrapMode, CSSValueID::kNowrap);
            AddPropertyToPresentationAttributeStyle(style, CSSPropertyID::kOverflowWrap, CSSValueID::kNormal);
        }
    } else {
...
void HTMLTextAreaElement::ParseAttribute(const AttributeModificationParams& params)
{
    const QualifiedName& name = params.name;
    const AtomicString& value = params.new_value;
    if (name == html_names::kRowsAttr) {
        unsigned rows = 0;
        if (value.empty() || !ParseHTMLNonNegativeInteger(value, rows) || rows <= 0 || rows > 0x7fffffffu)
            rows = kDefaultRows;
        if (rows_ != rows) {
            rows_ = rows;
            if (GetLayoutObject()) {
        ...
        unsigned cols = 0;
        if (value.empty() || !ParseHTMLNonNegativeInteger(value, cols) || cols <= 0 || cols > 0x7fffffffu)
            cols = kDefaultCols;
        if (cols_ != cols) {
            cols_ = cols;
            if (LayoutObject* layout_object = GetLayoutObject()) {
                layout_object->SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kAttributeChanged);
            }
        }
        ...
        WrapMethod wrap;
        if (EqualIgnoringASCIICase(value, "physical") || EqualIgnoringASCIICase(value, "hard") || EqualIgnoringASCIICase(value, "on"))
            wrap = kHardWrap;
...
void HTMLTextAreaElement::AppendToFormData(FormData& form_data)
{
    ...
    const String& text = (wrap_ == kHardWrap) ? ValueWithHardLineBreaks() : Value();
    form_data.AppendFromElement(GetName(), text);

    const AtomicString& dirname_attr_value = FastGetAttribute(html_names::kDirnameAttr);
...
{
    SetNonDirtyValue(defaultValue(), TextControlSetValueSelection::kSetSelectionToEnd);
...
bool HTMLTextAreaElement::IsKeyboardFocusable(UpdateBehavior update_behavior) const
{
...
void HTMLTextAreaElement::UpdateSelectionOnFocus(SelectionBehaviorOnFocus selection_behavior, const FocusOptions* options)
{
...
{
    if (GetLayoutObject()
        && (IsA<MouseEvent>(event) || IsA<DragEvent>(event) || event.HasInterface(event_interface_names::kWheelEvent)
...
void HTMLTextAreaElement::HandleBeforeTextInsertedEvent(BeforeTextInsertedEvent* event)
{
    DCHECK(event);
    ...
    if (current_length + ComputeLengthForAPIValue(event->GetText()) < unsigned_max_length)
        return;
    ...
    if (IsFocused()) {
        ...
        selection_length = ComputeLengthForAPIValue(GetDocument().GetFrame()->Selection().SelectedText());
    }
    DCHECK_GE(current_length, selection_length);
    unsigned base_length = current_length - selection_length;
    unsigned appendable_length = unsigned_max_length > base_length ? unsigned_max_length - base_length : 0;
    event->SetText(SanitizeUserInputValue(event->GetText(), appendable_length));

    if (selection_length == current_length && selection_length != 0 && !event->GetText().empty()) {
        GetDocument().GetPage()->GetChromeClient().DidClearValueInTextField(*this);
    }
}

String HTMLTextAreaElement::SanitizeUserInputValue(const String& proposed_value, unsigned max_length)
{
    unsigned submission_length = 0;
    unsigned i = 0;
    for (; i < proposed_value.length(); ++i) {
        if (proposed_value[i] == '\r' && i + 1 < proposed_value.length() && proposed_value[i + 1] == '\n')
            continue;
        ++submission_length;
...
void HTMLTextAreaElement::setValueForBinding(const String& value)
{
    ...
    SetValue(value, TextFieldEventBehavior::kDispatchNoEvent, TextControlSetValueSelection::kSetSelectionToEnd,
        was_autofilled && !value_changed ? WebAutofillState::kAutofilled : WebAutofillState::kNotFilled);
...
void HTMLTextAreaElement::SetValue(
...
{
    SetValueCommon(value, TextFieldEventBehavior::kDispatchNoEvent, selection, WebAutofillState::kNotFilled);
    is_dirty_ = false;
}

void HTMLTextAreaElement::SetValueCommon(
    const String& new_value, TextFieldEventBehavior event_behavior, TextControlSetValueSelection selection, WebAutofillState autofill_state)
{
    ...
    if (selection == TextControlSetValueSelection::kSetSelectionToEnd) {
        ...
        SetSelectionRange(std::min(end_of_string, selection_start), std::min(end_of_string, selection_end));
    }
...
{
    SetAutofillState(!value.empty() ? WebAutofillState::kPreviewed : WebAutofillState::kNotFilled);
...
String HTMLTextAreaElement::validationMessage() const
{
    ...
    if (TooShort()) {
        return GetLocale().ValidationMessageTooShortText(Value().length(), minLength());
    }
...
bool HTMLTextAreaElement::ValueMissing(const String* value) const
{
    ...
    return IsRequiredFormControl() && !IsDisabledOrReadOnly() && (value ? *value : this->Value()).empty();
}
...
{
    return !ValueMissing(&candidate) && !TooLong(&candidate, kIgnoreDirtyFlag) && !TooShort(&candidate, kIgnoreDirtyFlag);
}
...
{
    SetUnsignedIntegralAttribute(html_names::kColsAttr, cols ? cols : kDefaultCols, kDefaultCols);
}

void HTMLTextAreaElement::setRows(unsigned rows)
{
    SetUnsignedIntegralAttribute(html_names::kRowsAttr, rows ? rows : kDefaultRows, kDefaultRows);
}
...
HTMLElement* HTMLTextAreaElement::UpdatePlaceholderText()
{
    ...
    if (!placeholder) {
        ...
        placeholder->setAttribute(html_names::kIdAttr, shadow_element_names::kIdPlaceholder);
        placeholder->SetInlineStyleProperty(CSSPropertyID::kDisplay, IsPlaceholderVisible() ? CSSValueID::kBlock : CSSValueID::kNone, true);
        UserAgentShadowRoot()->InsertBefore(placeholder, InnerEditorElement());
    }
    if (is_suggested_value) {
        placeholder->SetInlineStyleProperty(CSSPropertyID::kUserSelect, CSSValueID::kNone, true);
    } else {
...
{
    return !SuggestedValue().empty() ? SuggestedValue() : FastGetAttribute(html_names::kPlaceholderAttr);
}
...
void HTMLTextAreaElement::CloneNonAttributePropertiesFrom(const Element& source, NodeCloningData& data)
{
    const auto& source_element = To<HTMLTextAreaElement>(source);
    SetValueCommon(source_element.Value(), TextFieldEventBehavior::kDispatchNoEvent, TextControlSetValueSelection::kSetSelectionToStart,
...
void HTMLTextAreaElement::SetFocused(bool is_focused, mojom::blink::FocusType focus_type)
{
