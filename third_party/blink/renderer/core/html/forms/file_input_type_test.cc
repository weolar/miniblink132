...
TEST(FileInputTypeTest, createFileList)
{
    ...
    // Native file.
    files.push_back(CreateFileChooserFileInfoNative("/native/path/native-file", "display-name"));

    // Non-native file.
    KURL url("filesystem:http://example.com/isolated/hash/non-native-file");
    files.push_back(CreateFileChooserFileInfoFileSystem(url, base::Time::FromMillisecondsSinceUnixEpoch(1.0 * kMsPerDay + 3), 64));

    ScopedNullExecutionContext execution_context;
    FileList* list = FileInputType::CreateFileList(execution_context.GetExecutionContext(), files, base::FilePath());
    ASSERT_TRUE(list);
...
TEST(FileInputTypeTest, createFileListContentUri)
{
    ...
    files.push_back(CreateFileChooserFileInfoNative("content://authority/id-123", "display-name", Vector<String>({ "base", "subdir" })));

    ScopedNullExecutionContext execution_context;
    FileList* list = FileInputType::CreateFileList(execution_context.GetExecutionContext(), files, base::FilePath("content://authority/id-base"));
    ASSERT_TRUE(list);
...
TEST(FileInputTypeTest, ignoreDroppedNonNativeFiles)
{
    ...
    const DragData native_file_drag_data(native_file_raw_drag_data, gfx::PointF(), gfx::PointF(), kDragOperationCopy, false);
    native_file_drag_data.PlatformData()->Add(MakeGarbageCollected<File>(&execution_context.GetExecutionContext(), "/native/path"));
    ...
    const DragData non_native_file_drag_data(non_native_file_raw_drag_data, gfx::PointF(), gfx::PointF(), kDragOperationCopy, false);
    FileMetadata metadata;
    metadata.length = 1234;
    const KURL url("filesystem:http://example.com/isolated/hash/non-native-file");
    non_native_file_drag_data.PlatformData()->Add(File::CreateForFileSystemFile(url, metadata, File::kIsUserVisible, BlobDataHandle::Create()));
...
TEST(FileInputTypeTest, setFilesFromPaths)
{
    ...
    EXPECT_EQ(String("/native/real/path1"), file_input->Files()->item(0)->GetPath());
    EXPECT_EQ(String("/native/real/path2"), file_input->Files()->item(1)->GetPath());
}
...
TEST(FileInputTypeTest, DropTouchesNoPopupOpeningObserver)
{
    ...
    auto page_holder = std::make_unique<DummyPageHolder>(gfx::Size(), chrome_client);
    ...
    MockFileChooser chooser(doc.GetFrame()->GetBrowserInterfaceBroker(), run_loop.QuitClosure());
    DragData drag_data(DataObject::Create(), gfx::PointF(), gfx::PointF(), kDragOperationCopy, false);
    drag_data.PlatformData()->Add(MakeGarbageCollected<File>(&execution_context.GetExecutionContext(), "/foo/bar"));
...
{
    test::TaskEnvironment task_environment;
    std::unique_ptr<DummyPageHolder> page_holder = std::make_unique<DummyPageHolder>(gfx::Size(800, 600));
    Document& doc = page_holder->GetDocument();
...
TEST(FileInputTypeTest, ChangeTypeDuringOpeningFileChooser)
{
    ...
    base::RunLoop run_loop;
    MockFileChooser chooser(frame->GetBrowserInterfaceBroker(), run_loop.QuitClosure());
    ...
    // for another type.
    EXPECT_TRUE(IsA<HTMLElement>(input.EnsureShadowSubtree()->firstChild()->firstChild()));
}
...
TEST(FileInputTypeTest, SetFilesFireCorrectEventsForSameFile)
{
    ...
    selection_1->Append(MakeGarbageCollected<File>(&execution_context.GetExecutionContext(), "/path/to/A.txt"));
    ...
    selection_2->Append(MakeGarbageCollected<File>(&execution_context.GetExecutionContext(), "/path/to/A.txt"));
...
TEST(FileInputTypeTest, SetFilesFireCorrectEventsForSameFiles)
{
    ...
    auto* const selection_1 = MakeGarbageCollected<FileList>();
    selection_1->Append(MakeGarbageCollected<File>(&execution_context.GetExecutionContext(), "/path/to/A.txt"));
    selection_1->Append(MakeGarbageCollected<File>(&execution_context.GetExecutionContext(), "/path/to/B.txt"));
    file_input->SetFilesAndDispatchEvents(selection_1);
    EXPECT_TRUE(listener_change->invoked);
    EXPECT_FALSE(listener_cancel->invoked);
    ...
    auto* const selection_2 = MakeGarbageCollected<FileList>();
    selection_2->Append(MakeGarbageCollected<File>(&execution_context.GetExecutionContext(), "/path/to/A.txt"));
    selection_2->Append(MakeGarbageCollected<File>(&execution_context.GetExecutionContext(), "/path/to/B.txt"));
    file_input->SetFilesAndDispatchEvents(selection_2);
    EXPECT_FALSE(listener_change->invoked);
    EXPECT_TRUE(listener_cancel->invoked);
}
