...
class CORE_EXPORT HTMLInputElement : public TextControlElement, public LazyActiveScriptWrappable<HTMLInputElement> {
    DEFINE_WRAPPERTYPEINFO();

public:
    explicit HTMLInputElement(Document&, const CreateElementFlags = CreateElementFlags());
    ...
    bool checkedForBinding() const
    {
        return Checked();
    }
    void setCheckedForBinding(bool);
    bool Checked() const;
    void SetChecked(bool, TextFieldEventBehavior = TextFieldEventBehavior::kDispatchNoEvent, WebAutofillState = WebAutofillState::kNotFilled);
    ...
    void SetValue(const String&, TextFieldEventBehavior = TextFieldEventBehavior::kDispatchNoEvent,
        TextControlSetValueSelection = TextControlSetValueSelection::kSetSelectionToEnd, WebAutofillState = WebAutofillState::kNotFilled) override;
    String valueForBinding() const
    {
        return Value();
    }
    ...
    void setValueAsDate(ScriptState* script_state, const ScriptValue& value, ExceptionState& exception_state);

    double valueAsNumber() const;
    void setValueAsNumber(double, ExceptionState&, TextFieldEventBehavior = TextFieldEventBehavior::kDispatchNoEvent);
    ...
    void setSelectionRangeForBinding(unsigned start, unsigned end, ExceptionState&);
    void setSelectionRangeForBinding(unsigned start, unsigned end, const String& direction, ExceptionState&);
    // This function can be used to allow tests to set the selection
    // range for Number inputs, which do not support the ordinary
    // selection API.
    void SetSelectionRangeForTesting(unsigned start, unsigned end, ExceptionState&);

    bool LayoutObjectIsNeeded(const DisplayStyle&) const final;
    LayoutObject* CreateLayoutObject(const ComputedStyle&) override;
    void DetachLayoutTree(bool performing_reattach) final;
    void UpdateSelectionOnFocus(SelectionBehaviorOnFocus, const FocusOptions*) final;
    ...
    void UpdateView();
    bool NeedsToUpdateViewValue() const
    {
        return needs_to_update_view_value_;
    }
    ...
    static Vector<String> FilesFromFileInputFormControlState(const FormControlState&);
    ...
    void setRangeText(const String& replacement, unsigned start, unsigned end, const V8SelectionMode& selection_mode, ExceptionState&) final;
    ...
    {
        return should_reveal_password_;
    }
    ...
    bool IsPlaceholderVisible() const override
    {
        return is_placeholder_visible_;
    }
...
private:
    ...
    bool IsKeyboardFocusable(UpdateBehavior update_behavior = UpdateBehavior::kStyleAndLayout) const final;
    ...
    bool IsTextControl() const final
    {
        return IsTextField();
    }
    ...
    bool CanTriggerImplicitSubmission() const final
    {
        return IsTextField();
    }
    ...
    void DispatchFocusInEvent(
        const AtomicString& event_type, Element* old_focused_element, mojom::blink::FocusType, InputDeviceCapabilities* source_capabilities) final;

    bool IsOptionalFormControl() const final
    {
