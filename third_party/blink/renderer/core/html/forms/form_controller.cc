...
inline HTMLFormElement* OwnerFormForState(const ListedElement& control)
{
    ...
    // indeterminate.
    return control.ToHTMLElement().FastHasAttribute(html_names::kFormAttr) ? nullptr : control.Form();
}
...
{
    if (auto* form_control_element = DynamicTo<HTMLFormControlElementWithState>(control))
...
FormControlState FormControlState::Deserialize(const Vector<String>& state_vector, wtf_size_t& index)
{
...
public:
    ...
    StringImpl* GetName() const
    {
        return name_;
    }
    StringImpl* GetType() const
    {
        return type_;
    }
    ...
    // destroyed.
    ControlKey(WTF::HashTableDeletedValueType)
...
public:
    SavedFormState()
        : control_state_count_(0)
    {
    }
    SavedFormState(const SavedFormState&) = delete;
    SavedFormState& operator=(const SavedFormState&) = delete;

    static std::unique_ptr<SavedFormState> Deserialize(const Vector<String>&, wtf_size_t& index);
    void SerializeTo(Vector<String>&) const;
    bool IsEmpty() const
    {
        return state_for_new_controls_.empty();
    }
...
private:
    using ControlStateMap = HashMap<ControlKey, Deque<FormControlState>, ControlKeyHashTraits>;
...
std::unique_ptr<SavedFormState> SavedFormState::Deserialize(const Vector<String>& state_vector, wtf_size_t& index)
{
    if (index >= state_vector.size())
        return nullptr;
    ...
    if (!item_count)
        return nullptr;
    std::unique_ptr<SavedFormState> saved_form_state = base::WrapUnique(new SavedFormState);
    while (item_count--) {
        ...
        if (type.empty() || (type.Find(IsNotFormControlTypeCharacter) != kNotFound && !CustomElement::IsValidName(AtomicString(type))) || state.IsFailure())
            return nullptr;
        saved_form_state->AppendControlState(AtomicString(name), AtomicString(type), state);
    }
...
FormControlState SavedFormState::TakeControlState(const AtomicString& name, const AtomicString& type)
{
    if (state_for_new_controls_.empty())
        return FormControlState();
    ControlStateMap::iterator it = state_for_new_controls_.find(ControlKey(name.Impl(), type.Impl()));
    if (it == state_for_new_controls_.end())
        return FormControlState();
...
Vector<String> SavedFormState::GetReferencedFilePaths() const
{
    ...
    for (const auto& form_control : state_for_new_controls_) {
        ...
        const Deque<FormControlState>& queue = form_control.value;
        for (const FormControlState& form_control_state : queue) {
            to_return.AppendVector(HTMLInputElement::FilesFromFileInputFormControlState(form_control_state));
        }
    }
    return to_return;
}
...
public:
    ...
    {
        visitor->Trace(form_to_key_map_);
    }
...
static inline void RecordFormStructure(const HTMLFormElement& form, StringBuilder& builder)
{
    ...
    builder.Append(" [");
    for (wtf_size_t i = 0, named_controls = 0; i < controls.size() && named_controls < kNamedControlsToBeRecorded; ++i) {
...
const AtomicString& FormKeyGenerator::FormKey(const ListedElement& control)
{
    ...
    if (!form) {
        DEFINE_STATIC_LOCAL(const AtomicString, form_key_for_no_owner, ("No owner"));
    ...
    FormSignatureToNextIndexMap::AddResult result = form_signature_to_next_index_map_.insert(signature, 0);
    ...
    FormToKeyMap::AddResult add_form_keyresult = form_to_key_map_.insert(form, form_key_builder.ToAtomicString());
...
Vector<String> DocumentState::ToStateVector()
{
    ...
    std::unique_ptr<SavedFormStateMap> state_map = base::WrapUnique(new SavedFormStateMap);
    for (auto& control : GetControlList()) {
        DCHECK(control->ToHTMLElement().isConnected());
        if (!control->ShouldSaveAndRestoreFormControlState())
            continue;
        SavedFormStateMap::AddResult result = state_map->insert(key_generator->FormKey(*control), nullptr);
        if (result.is_new_entry)
            result.stored_value->value = std::make_unique<SavedFormState>();
        result.stored_value->value->AppendControlState(control->GetName(), ControlType(*control), control->SaveFormControlState());
    }
...
void FormController::SetStateForNewControls(const Vector<String>& state_vector)
{
...
FormControlState FormController::TakeStateForControl(const ListedElement& control)
{
    if (saved_form_state_map_.empty())
        return FormControlState();
    if (!form_key_generator_)
        form_key_generator_ = MakeGarbageCollected<FormKeyGenerator>();
    SavedFormStateMap::iterator it = saved_form_state_map_.find(form_key_generator_->FormKey(control));
    if (it == saved_form_state_map_.end())
        return FormControlState();
    FormControlState state = it->value->TakeControlState(control.GetName(), ControlType(control));
    if (it->value->IsEmpty())
        saved_form_state_map_.erase(it);
    return state;
}

void FormController::ControlStatesFromStateVector(const Vector<String>& state_vector, SavedFormStateMap& map)
{
    map.clear();
    ...
    while (i + 1 < state_vector.size()) {
        ...
        std::unique_ptr<SavedFormState> state = SavedFormState::Deserialize(state_vector, i);
        if (!state) {
            i = 0;
            break;
        }
...
void FormController::RestoreControlStateInternal(ListedElement& control)
{
    ...
    if (element.IsDisabledFormControl() || element.FastHasAttribute(html_names::kReadonlyAttr))
        return;
...
void FormController::ScheduleRestore()
{
    document_->GetTaskRunner(TaskType::kInternalLoading)
        ->PostTask(FROM_HERE, WTF::BindOnce(&FormController::RestoreAllControlsInDocumentOrder, WrapPersistent(this)));
}
...
Vector<String> FormController::GetReferencedFilePaths(const Vector<String>& state_vector)
{
    Vector<String> to_return;
