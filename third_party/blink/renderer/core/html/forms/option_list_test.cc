...
protected:
    void SetUp() override
    {
        auto* document = HTMLDocument::CreateForTest(execution_context_.GetExecutionContext());
...
{
    Select().setInnerHTML("text<input><option id=o1></option><input><option "
                          "id=o2></option><input>");
    auto* div = To<HTMLElement>(Select().GetDocument().CreateRawElement(html_names::kDivTag));
...
TEST_F(OptionListTest, Optgroup)
{
    ...
    To<HTMLElement>(Select().firstChild())
        ->setInnerHTML("<optgroup><option id=gg11></option></optgroup>"
