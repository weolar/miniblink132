...
enum TextFieldSelectionDirection { kSelectionHasNoDirection, kSelectionHasForwardDirection, kSelectionHasBackwardDirection };
enum class TextFieldEventBehavior { kDispatchNoEvent, kDispatchChangeEvent, kDispatchInputEvent, kDispatchInputAndChangeEvent };
...
public:
    ...
    bool IsRichlyEditableForAccessibility() const override
    {
        return false;
    }
    ...
    virtual void setRangeText(const String& replacement, unsigned start, unsigned end, const V8SelectionMode& selection_mode, ExceptionState&);
    // Web-exposed setSelectionRange() function. This schedule to dispatch
    // 'select' event.
    void setSelectionRangeForBinding(unsigned start, unsigned end, const String& direction = "none");
    ...
    bool SetSelectionRange(unsigned start, unsigned end, TextFieldSelectionDirection = kSelectionHasNoDirection);
    ...
    virtual void SetValue(const String&, TextFieldEventBehavior = TextFieldEventBehavior::kDispatchNoEvent,
        TextControlSetValueSelection = TextControlSetValueSelection::kSetSelectionToEnd, WebAutofillState = WebAutofillState::kNotFilled)
        = 0;
    ...
    void DropInnerEditorElement()
    {
        inner_editor_ = nullptr;
    }
    ...
    void SetAutofillValue(const String& value, WebAutofillState = WebAutofillState::kAutofilled);
...
protected:
    ...
    void SetLastChangeWasNotUserEdit()
    {
...
private:
    ...
    void ComputeSelection(uint32_t flags, ComputedSelection& computed_selection) const;
    ...
    bool DispatchFocusEvent(Element* old_focused_element, mojom::blink::FocusType, InputDeviceCapabilities* source_capabilities) final;
    void DispatchBlurEvent(Element* new_focused_element, mojom::blink::FocusType, InputDeviceCapabilities* source_capabilities) final;
    ...
    virtual void HandleFocusEvent(Element* /* oldFocusedNode */, mojom::blink::FocusType)
    {
    }
...
template <> struct DowncastTraits<TextControlElement> {
    static bool AllowFrom(const Node& node)
    {
        return node.HasTagName(html_names::kInputTag) || node.HasTagName(html_names::kTextareaTag);
    }
};
