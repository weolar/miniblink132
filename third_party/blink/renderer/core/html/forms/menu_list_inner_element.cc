...
const ComputedStyle* MenuListInnerElement::CustomStyleForLayoutObject(const StyleRecalcContext& style_recalc_context)
{
    ...
    ComputedStyleBuilder style_builder = GetDocument().GetStyleResolver().CreateAnonymousStyleBuilderWithDisplay(parent_style, EDisplay::kBlock);
    ...
    if (style_builder.HasInitialLineHeight()) {
        ...
        if (font_data) {
            style_builder.SetLineHeight(Length::Fixed(font_data->GetFontMetrics().Height()));
        } else {
    ...
    // center.
    if (parent_style.AlignItems().GetPosition() == ItemPosition::kCenter || parent_style.AlignItems().GetPosition() == ItemPosition::kAnchorCenter) {
        style_builder.SetMarginTop(Length());
        style_builder.SetMarginBottom(Length());
        style_builder.SetAlignSelf(StyleSelfAlignmentData(ItemPosition::kStart, OverflowAlignment::kDefault));
    }

    // We set margin-* instead of padding-* to clip text by 'overflow: hidden'.
    LogicalToPhysicalSetter margin_setter(style_builder.GetWritingDirection(), style_builder, &ComputedStyleBuilder::SetMarginTop,
        &ComputedStyleBuilder::SetMarginRight, &ComputedStyleBuilder::SetMarginBottom, &ComputedStyleBuilder::SetMarginLeft);
    LayoutTheme& theme = LayoutTheme::GetTheme();
    Length margin_start = Length::Fixed(theme.PopupInternalPaddingStart(parent_style));
    Length margin_end = Length::Fixed(theme.PopupInternalPaddingEnd(GetDocument().GetFrame(), parent_style));
    ...
    LogicalToPhysicalSetter padding_setter(style_builder.GetWritingDirection(), style_builder, &ComputedStyleBuilder::SetPaddingTop,
        &ComputedStyleBuilder::SetPaddingRight, &ComputedStyleBuilder::SetPaddingBottom, &ComputedStyleBuilder::SetPaddingLeft);
    padding_setter.SetBlockStart(Length::Fixed(theme.PopupInternalPaddingTop(parent_style)));
    padding_setter.SetBlockEnd(Length::Fixed(theme.PopupInternalPaddingBottom(parent_style)));

    if (const ComputedStyle* option_style = To<HTMLSelectElement>(OwnerShadowHost())->OptionStyle()) {
