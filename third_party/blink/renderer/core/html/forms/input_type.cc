...
InputType* InputType::Create(HTMLInputElement& element, const AtomicString& type_name)
{
    if (type_name.empty())
...
const AtomicString& InputType::NormalizeTypeName(const AtomicString& type_name)
{
    if (type_name.empty())
        return input_type_names::kText;
...
template <typename T> bool ValidateInputType(const T& input_type, const String& value)
{
    if (!input_type.CanSetStringValue()) {
        NOTREACHED();
    }
    return !input_type.TypeMismatchFor(value) && !input_type.StepMismatch(value) && !input_type.RangeUnderflow(value) && !input_type.RangeOverflow(value)
...
void InputType::AppendToFormData(FormData& form_data) const
{
    ...
    if (IsAutoDirectionalityFormAssociated()) {
        const AtomicString& dirname_attr_value = GetElement().FastGetAttribute(html_names::kDirnameAttr);
        if (!dirname_attr_value.IsNull()) {
            form_data.AppendFromElement(dirname_attr_value, GetElement().DirectionForFormData());
        }
    }
}
...
void InputType::SetValueAsDate(const std::optional<base::Time>&, ExceptionState& exception_state) const
{
    exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "This input element does not support Date values.");
}
...
void InputType::SetValueAsDouble(double double_value, TextFieldEventBehavior event_behavior, ExceptionState& exception_state) const
{
    exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "This input element does not support Number values.");
}

void InputType::SetValueAsDecimal(const Decimal& new_value, TextFieldEventBehavior event_behavior, ExceptionState&) const
{
...
bool InputType::TooLong(const String&, TextControlElement::NeedsToCheckDirtyFlag) const
{
    return false;
}

bool InputType::TooShort(const String&, TextControlElement::NeedsToCheckDirtyFlag) const
{
    return false;
}
...
bool InputType::RangeUnderflow(const String& value) const
{
    ...
    if (step_range.HasReversedRange()) {
        ...
        return numeric_value > step_range.Maximum() && numeric_value < step_range.Minimum();
    } else {
...
bool InputType::IsInRange(const String& value) const
{
    ...
    StepRange step_range(CreateStepRange(kRejectAny));
    return step_range.HasRangeLimitations() && numeric_value >= step_range.Minimum() && numeric_value <= step_range.Maximum();
}
...
{
    static auto* input_type = base::debug::AllocateCrashKeyString("input-type", base::debug::CrashKeySize::Size32);
    base::debug::SetCrashKeyString(input_type, FormControlTypeAsString().GetString().Utf8().c_str());
    NOTREACHED() << "This should not get called. Check if input type '" << FormControlTypeAsString() << "' should have a RangeOverflowText implementation."
...
String InputType::RangeUnderflowText(const Decimal&) const
{
    static auto* input_type = base::debug::AllocateCrashKeyString("input-type", base::debug::CrashKeySize::Size32);
    base::debug::SetCrashKeyString(input_type, FormControlTypeAsString().GetString().Utf8().c_str());
    NOTREACHED() << "This should not get called. Check if input type '" << FormControlTypeAsString() << "' should have a RangeUnderflowText implementation."
                 << "See crbug.com/1423280";
}

String InputType::ReversedRangeOutOfRangeText(const Decimal&, const Decimal&) const
{
    NOTREACHED();
}
...
String InputType::RangeInvalidText(const Decimal&, const Decimal&) const
{
    static auto* input_type = base::debug::AllocateCrashKeyString("input-type", base::debug::CrashKeySize::Size32);
    base::debug::SetCrashKeyString(input_type, FormControlTypeAsString().GetString().Utf8().c_str());
    NOTREACHED() << "This should not get called. Check if input type '" << FormControlTypeAsString() << "' should have a RangeInvalidText implementation."
...
std::pair<String, String> InputType::ValidationMessage(const InputTypeView& input_type_view) const
{
    ...
    if (PatternMismatch(value)) {
        ...
        return std::make_pair(GetLocale().QueryString(IDS_FORM_VALIDATION_PATTERN_MISMATCH), GetElement().FastGetAttribute(html_names::kTitleAttr).GetString());
    }

    if (GetElement().TooLong()) {
        return std::make_pair(GetLocale().ValidationMessageTooLongText(value.length(), GetElement().maxLength()), g_empty_string);
    }

    if (GetElement().TooShort()) {
        return std::make_pair(GetLocale().ValidationMessageTooShortText(value.length(), GetElement().minLength()), g_empty_string);
    }
    ...
    if (step_range.Minimum() > step_range.Maximum() && !step_range.HasReversedRange()) {
        return std::make_pair(RangeInvalidText(step_range.Minimum(), step_range.Maximum()), g_empty_string);
    }

    if (step_range.HasReversedRange() && numeric_value < step_range.Minimum() && numeric_value > step_range.Maximum()) {
        return std::make_pair(ReversedRangeOutOfRangeText(step_range.Minimum(), step_range.Maximum()), g_empty_string);
    }

    if (numeric_value != step_range.Minimum() && step_range.Minimum() == step_range.Maximum()) {
        return std::make_pair(ValueNotEqualText(step_range.Minimum()), g_empty_string);
    }

    if (numeric_value < step_range.Minimum())
        return std::make_pair(RangeUnderflowText(step_range.Minimum()), g_empty_string);

    if (numeric_value > step_range.Maximum())
        return std::make_pair(RangeOverflowText(step_range.Maximum()), g_empty_string);
    ...
    if (step_range.StepMismatch(numeric_value)) {
        ...
        Decimal candidate2 = candidate1 < numeric_value ? candidate1 + step_range.Step() : candidate1 - step_range.Step();
        if (!candidate2.IsFinite() || candidate2 < step_range.Minimum() || candidate2 > step_range.Maximum()) {
            return std::make_pair(GetLocale().QueryString(IDS_FORM_VALIDATION_STEP_MISMATCH_CLOSE_TO_LIMIT, localized_candidate1), g_empty_string);
        }
        String localized_candidate2 = LocalizeValue(Serialize(candidate2));
        if (candidate1 < candidate2) {
            return std::make_pair(GetLocale().QueryString(IDS_FORM_VALIDATION_STEP_MISMATCH, localized_candidate1, localized_candidate2), g_empty_string);
        }
        return std::make_pair(GetLocale().QueryString(IDS_FORM_VALIDATION_STEP_MISMATCH, localized_candidate2, localized_candidate1), g_empty_string);
    }

    return std::make_pair(g_empty_string, g_empty_string);
}

Decimal InputType::ParseToNumber(const String&, const Decimal& default_value) const
{
    NOTREACHED();
}
...
bool InputType::IsKeyboardFocusable(Element::UpdateBehavior update_behavior) const
{
    // Inputs are always keyboard focusable if they are focusable at all,
    // and don't have a negative tabindex set.
    return GetElement().IsFocusable(update_behavior) && GetElement().tabIndex() >= 0;
}
...
void InputType::SetValue(const String& sanitized_value, bool value_changed, TextFieldEventBehavior event_behavior, TextControlSetValueSelection)
{
...
HTMLFormControlElement::PopoverTriggerSupport InputType::SupportsPopoverTriggering() const
{
...
void InputType::ApplyStep(const Decimal& current, const bool current_was_invalid, double count, AnyStepHandling any_step_handling,
    TextFieldEventBehavior event_behavior, ExceptionState& exception_state)
{
    ...
    if (!step_range.HasStep()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "This form element does not have an allowed value step.");
        return;
    }
    ...
    const AtomicString& step_string = GetElement().FastGetAttribute(html_names::kStepAttr);
    if (!EqualIgnoringASCIICase(step_string, "any") && step_range.StepMismatch(current)) {
    ...
    if (new_value < step_range.Minimum()) {
        const Decimal aligned_minimum = base + ((step_range.Minimum() - base) / step).Ceil() * step;
    ...
    if (!current_was_invalid && ((count < 0 && current < new_value) || (count > 0 && current > new_value))) {
        return;
    }
    ...
    if (RuntimeEnabledFeatures::DispatchBeforeInputForSpinButtonInteractionsEnabled() && event_behavior == TextFieldEventBehavior::kDispatchChangeEvent
        && DispatchBeforeInputInsertText(EventTargetNodeForDocument(&GetElement().GetDocument()), new_value.ToString()) != DispatchEventResult::kNotCanceled) {
        return;
    }
...
void InputType::StepUp(double n, ExceptionState& exception_state)
{
    ...
    if (!IsSteppable()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "This form element is not steppable.");
        return;
    }
    ...
    ApplyStep(current, current_was_invalid, n, kRejectAny, TextFieldEventBehavior::kDispatchNoEvent, exception_state);
}
...
void InputType::StepUpFromLayoutObject(int n)
{
    ...
    if (!current.IsFinite()) {
        ...
        SetValueAsDecimal(current, TextFieldEventBehavior::kDispatchNoEvent, IGNORE_EXCEPTION_FOR_TESTING);
    }
    if ((sign > 0 && current < step_range.Minimum()) || (sign < 0 && current > step_range.Maximum())) {
        SetValueAsDecimal(sign > 0 ? step_range.Minimum() : step_range.Maximum(), TextFieldEventBehavior::kDispatchChangeEvent, IGNORE_EXCEPTION_FOR_TESTING);
        return;
    }
    if ((sign > 0 && current >= step_range.Maximum()) || (sign < 0 && current <= step_range.Minimum()))
        return;
    ...
    ApplyStep(current, current_was_invalid, n, kAnyIsDefaultStep, TextFieldEventBehavior::kDispatchChangeEvent, IGNORE_EXCEPTION_FOR_TESTING);
}
...
{
    Decimal step_base = ParseToNumber(GetElement().FastGetAttribute(html_names::kMinAttr), Decimal::Nan());
    if (!step_base.IsFinite()) {
        step_base = ParseToNumber(GetElement().FastGetAttribute(html_names::kValueAttr), default_value);
    }
    return step_base;
}

StepRange InputType::CreateReversibleStepRange(AnyStepHandling any_step_handling, const Decimal& step_base_default, const Decimal& minimum_default,
    const Decimal& maximum_default, const StepRange::StepDescription& step_description) const
{
    return CreateStepRange(any_step_handling, step_base_default, minimum_default, maximum_default, step_description,
        /*supports_reversed_range=*/true);
}

StepRange InputType::CreateStepRange(AnyStepHandling any_step_handling, const Decimal& step_base_default, const Decimal& minimum_default,
    const Decimal& maximum_default, const StepRange::StepDescription& step_description) const
{
    return CreateStepRange(any_step_handling, step_base_default, minimum_default, maximum_default, step_description,
...
StepRange InputType::CreateStepRange(AnyStepHandling any_step_handling, const Decimal& step_base_default, const Decimal& minimum_default,
    const Decimal& maximum_default, const StepRange::StepDescription& step_description, bool supports_reversed_range) const
{
    bool has_range_limitations = false;
    const Decimal step_base = FindStepBase(step_base_default);
    Decimal minimum = ParseToNumberOrNaN(GetElement().FastGetAttribute(html_names::kMinAttr));
    if (minimum.IsFinite())
        has_range_limitations = true;
    else
        minimum = minimum_default;
    Decimal maximum = ParseToNumberOrNaN(GetElement().FastGetAttribute(html_names::kMaxAttr));
    if (maximum.IsFinite())
        has_range_limitations = true;
    else
        maximum = maximum_default;
    const Decimal step = StepRange::ParseStep(any_step_handling, step_description, GetElement().FastGetAttribute(html_names::kStepAttr));
    bool has_reversed_range = has_range_limitations && supports_reversed_range && maximum < minimum;
    return StepRange(step_base, minimum, maximum, has_range_limitations, has_reversed_range, step, step_description);
}

void InputType::AddWarningToConsole(const char* message_format, const String& value) const
{
    GetElement().GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kRendering,
        mojom::ConsoleMessageLevel::kWarning, String::Format(message_format, JSONValue::QuoteString(value).Utf8().c_str())));
}
