...
public:
    ...
    HTMLInputElement* CheckedButton() const
    {
        return checked_button_.Get();
    }
...
void RadioButtonGroup::UpdateRequiredButton(MemberKeyValue& it, bool is_required)
{
...
void RadioButtonGroupScope::AddButton(HTMLInputElement* element)
{
    ...
    auto* key_value = name_to_group_map_->insert(element->GetName(), nullptr).stored_value;
    if (!key_value->value)
...
RadioButtonGroup* RadioButtonGroupScope::FindGroupByName(const AtomicString& name) const
{
    if (!name_to_group_map_)
        return nullptr;
