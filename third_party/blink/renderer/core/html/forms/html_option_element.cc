...
HTMLOptionElement::HTMLOptionElement(Document& document)
    : HTMLElement(html_names::kOptionTag, document)
    , is_selected_(false)
{
...
HTMLOptionElement* HTMLOptionElement::CreateForJSConstructor(
    Document& document, const String& data, const AtomicString& value, bool default_selected, bool selected, ExceptionState& exception_state)
{
    HTMLOptionElement* element = MakeGarbageCollected<HTMLOptionElement>(document);
...
FocusableState HTMLOptionElement::SupportsFocus(UpdateBehavior update_behavior) const
{
...
{
    String label_attr = String(FastGetAttribute(html_names::kLabelAttr)).StripWhiteSpace(IsHTMLSpace<UChar>).SimplifyWhiteSpace(IsHTMLSpace<UChar>);
    String inner_text = CollectOptionInnerText().StripWhiteSpace(IsHTMLSpace<UChar>).SimplifyWhiteSpace(IsHTMLSpace<UChar>);
...
void HTMLOptionElement::ParseAttribute(const AttributeModificationParams& params)
{
...
Node::InsertionNotificationRequest HTMLOptionElement::InsertedInto(ContainerNode& insertion_point)
{
...
void HTMLOptionElement::RemovedFrom(ContainerNode& insertion_point)
{
    ...
    bool was_removed_from_select_parent = insertion_point_passed && is_parent_select_or_optgroup;
...
void HTMLOptionElement::SetTextOnlyRendering(bool text_only)
{
    ...
    if (auto* first_child = GetShadowRoot()->firstChild()) {
        ...
        CHECK_NE(currently_text_only, IsA<HTMLSlotElement>(first_child)) << " <option>'s UA ShadowRoot should either be text or a <slot>.";
    ...
    if (!text_only) {
        ...
        GetShadowRoot()->AppendChild(MakeGarbageCollected<HTMLSlotElement>(GetDocument()));
    } else {
...
void HTMLOptionElement::DefaultEventHandlerInternal(Event& event)
{
    ...
    if (select) {
        ...
        if ((event.type() == event_type_names::kGesturetap && gesture_event)
            || (event.type() == event_type_names::kMousedown && mouse_event
                && mouse_event->button() == static_cast<int16_t>(WebPointerProperties::Button::kLeft))) {
    ...
    int tab_ignore_modifiers = WebInputEvent::kControlKey | WebInputEvent::kAltKey | WebInputEvent::kMetaKey;
    ...
    if (keyboard_event && event.type() == event_type_names::kKeydown) {
        ...
        if (key == keywords::kTab && !(keyboard_event->GetModifiers() & tab_ignore_modifiers)) {
            if (select) {
