...
void InvalidateShadowIncludingAncestorForms(ContainerNode& insertion_point)
{
    ...
    for (ContainerNode* parent = starting_node; parent; parent = parent->ParentOrShadowHostNode()) {
...
void ListedElement::InsertedInto(ContainerNode& insertion_point)
{
    ...
    if (!form_was_set_by_parser_ || !form_ || NodeTraversal::HighestAncestorOrSelf(insertion_point) != NodeTraversal::HighestAncestorOrSelf(*form_.Get()))
        ResetFormOwner();
    ...
    FieldSetAncestorsSetNeedsValidityCheck(&insertion_point, StartingNodeType::IS_INSERTION_POINT);
    DisabledStateMightBeChanged();

    if (ClassSupportsStateRestore() && insertion_point.isConnected() && !element.ContainingShadowRoot()) {
        element.GetDocument().GetFormController().InvalidateStatefulFormControlList();
    }

    // Trigger for elements outside of forms.
    if (!form_ && insertion_point.isConnected()) {
        element.GetDocument().DidChangeFormRelatedElementDynamically(&element, WebFormRelatedChangeType::kAdd);
    }
...
void ListedElement::RemovedFrom(ContainerNode& insertion_point)
{
    ...
    // updated.
    if (ancestor_disabled_state_ == AncestorDisabledState::kEnabled && data_list_ancestor_state_ == DataListAncestorState::kNotInsideDataList) {
    ...
    HTMLElement& element = ToHTMLElement();
    if (insertion_point.isConnected() && element.FastHasAttribute(html_names::kFormAttr)) {
        SetFormAttributeTargetObserver(nullptr);
        ResetFormOwner();
        ...
        form_was_set_by_parser_ = false;
        SetForm(nullptr);
    } else if (form_ && NodeTraversal::HighestAncestorOrSelf(element) != NodeTraversal::HighestAncestorOrSelf(*form_.Get())) {
    ...
    if (ClassSupportsStateRestore() && insertion_point.isConnected() && !element.ContainingShadowRoot() && !insertion_point.ContainingShadowRoot()) {
        element.GetDocument().GetFormController().InvalidateStatefulFormControlList();
    }
    ...
    if (insertion_point.isConnected()) {
        ...
        element.GetDocument().DidChangeFormRelatedElementDynamically(&element, WebFormRelatedChangeType::kRemove);
    }
}
...
void ListedElement::DidChangeForm()
{
    if (!form_was_set_by_parser_ && form_ && form_->isConnected()) {
        ...
        element.GetDocument().DidChangeFormRelatedElementDynamically(&element, WebFormRelatedChangeType::kReassociate);
    }
    FormOwnerSetNeedsValidityCheck();
}
...
void ListedElement::FieldSetAncestorsSetNeedsValidityCheck(Node* node, StartingNodeType starting_type)
{
    if (!node)
        return;
    ...
    } while ((field_set = Traversal<HTMLFieldSetElement>::FirstAncestor(*field_set)));
}
...
void ListedElement::ResetFormOwner()
{
    ...
    if (!form_id.IsNull() && element.isConnected()) {
        Element* new_form_candidate = element.GetTreeScope().getElementById(form_id);
...
bool ListedElement::RecalcWillValidate() const
{
    ...
    if (data_list_ancestor_state_ == DataListAncestorState::kUnknown) {
        if (element.GetDocument().HasAtLeastOneDataList() && Traversal<HTMLDataListElement>::FirstAncestor(element)) {
    ...
    return data_list_ancestor_state_ == DataListAncestorState::kNotInsideDataList && !element.IsDisabledFormControl() && !is_readonly_;
}

bool ListedElement::WillValidate() const
{
    if (!will_validate_initialized_ || data_list_ancestor_state_ == DataListAncestorState::kUnknown) {
...
{
    bool some_error = TypeMismatch() || StepMismatch() || RangeUnderflow() || RangeOverflow() || TooLong() || TooShort() || PatternMismatch() || ValueMissing()
        || HasBadInput() || CustomError();
    return !some_error;
}
...
{
    return ToHTMLElement().willValidate() && CustomError() ? custom_validation_message_ : String();
}
...
void ListedElement::FindCustomValidationMessageTextDirection(
    const String& message, TextDirection& message_dir, String& sub_message, TextDirection& sub_message_dir)
{
...
void ListedElement::UpdateVisibleValidationMessage()
{
    ...
    if (page->Paused())
        return;
    String message;
    if (element.GetLayoutObject() && WillValidate() && ToHTMLElement().IsShadowIncludingInclusiveAncestorOf(element))
    ...
    if (message.empty()) {
        ...
        FindCustomValidationMessageTextDirection(message, message_dir, sub_message, sub_message_dir);
    }
    client->ShowValidationMessage(element, message, message_dir, sub_message, sub_message_dir);
}
...
bool ListedElement::checkValidity(List* unhandled_invalid_controls)
{
    ...
    DispatchEventResult dispatch_result = element.DispatchEvent(*Event::CreateCancelable(event_type_names::kInvalid));
    if (dispatch_result == DispatchEventResult::kNotCanceled && unhandled_invalid_controls && element.isConnected()
...
void ListedElement::SetNeedsValidityCheck()
{
    ...
    if (!validity_is_dirty_) {
        validity_is_dirty_ = true;
        FormOwnerSetNeedsValidityCheck();
        FieldSetAncestorsSetNeedsValidityCheck(element.parentNode(), StartingNodeType::IS_PARENT);
    ...
    if (IsValidationMessageVisible()) {
        ...
        element.GetDocument()
            ...
            ->PostTask(FROM_HERE, WTF::BindOnce(&ListedElement::UpdateVisibleValidationMessage, WrapPersistent(this)));
    }
}
...
void ListedElement::UpdateAncestorDisabledState() const
{
    ...
    const HTMLElement& element = ToHTMLElement();
    if (may_have_fieldset_ancestor_ && element.GetDocument().HasAtLeastOneDisabledFieldset()) {
        may_have_fieldset_ancestor_ = false;
        ContainerNode* last_legend_ancestor = nullptr;
        for (auto* ancestor = Traversal<HTMLElement>::FirstAncestor(element); ancestor; ancestor = Traversal<HTMLElement>::FirstAncestor(*ancestor)) {
            if (IsA<HTMLLegendElement>(*ancestor)) {
                last_legend_ancestor = ancestor;
                continue;
            }
            if (HTMLFieldSetElement* fieldset_ancestor = DynamicTo<HTMLFieldSetElement>(ancestor)) {
                may_have_fieldset_ancestor_ = true;
                if (fieldset_ancestor->is_element_disabled_) {
                    if (last_legend_ancestor && last_legend_ancestor == fieldset_ancestor->Legend()) {
                        continue;
                    }
    ...
    if (ancestor_disabled_state_ == AncestorDisabledState::kEnabled && RuntimeEnabledFeatures::CustomizableSelectEnabled()) {
...
void ListedElement::TakeStateAndRestore()
{
    if (ClassSupportsStateRestore()) {
        ToHTMLElement().GetDocument().GetFormController().RestoreControlStateFor(*this);
    }
}

void ListedElement::SetFormAttributeTargetObserver(FormAttributeTargetObserver* new_observer)
{
...
void ListedElement::ResetFormAttributeTargetObserver()
{
    ...
    const AtomicString& form_id(element.FastGetAttribute(html_names::kFormAttr));
    if (!form_id.IsNull() && element.isConnected()) {
        SetFormAttributeTargetObserver(MakeGarbageCollected<FormAttributeTargetObserver>(form_id, this));
    } else {
        SetFormAttributeTargetObserver(nullptr);
    }
}
...
{
    return const_cast<HTMLElement&>(static_cast<const ListedElement&>(*this).ToHTMLElement());
}

FormAttributeTargetObserver::FormAttributeTargetObserver(const AtomicString& id, ListedElement* element)
    : IdTargetObserver(element->ToHTMLElement().GetTreeScope().EnsureIdTargetObserverRegistry(), id)
    , element_(element)
{
}
