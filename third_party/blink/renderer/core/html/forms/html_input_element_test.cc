...
public:
    MOCK_METHOD(void, PasswordFieldReset, (HTMLInputElement & element), (override));
};

class HTMLInputElementTestChromeClient : public EmptyChromeClient {
public:
    gfx::Rect LocalRootToScreenDIPs(const gfx::Rect& local_root_rect, const LocalFrameView* view) const override
    {
        return view->GetPage()->GetVisualViewport().RootFrameToViewport(local_root_rect);
    }
};
...
protected:
    void SetUp() override
    {
        auto* chrome_client = MakeGarbageCollected<HTMLInputElementTestChromeClient>();
...
TEST_F(HTMLInputElementTest, FilteredDataListOptionsNoList)
{
    ...
    GetDocument().documentElement()->setInnerHTML("<input id=test list=dl1><datalist id=dl1></datalist>");
    EXPECT_TRUE(TestElement().FilteredDataListOptions().empty());
}

TEST_F(HTMLInputElementTest, FilteredDataListOptionsContain)
{
    GetDocument().documentElement()->setInnerHTML("<input id=test value=BC list=dl2>"
                                                  "<datalist id=dl2>"
    ...
    GetDocument().documentElement()->setInnerHTML("<input id=test value=i list=dl2>"
                                                  "<datalist id=dl2>"
                                                  "<option>I</option>"
...
{
    auto* input = MakeGarbageCollected<HTMLInputElement>(GetDocument(), CreateElementFlags::ByCreateElement());
    EXPECT_EQ(nullptr, input->UserAgentShadowRoot());

    input = MakeGarbageCollected<HTMLInputElement>(GetDocument(), CreateElementFlags::ByParser(&GetDocument()));
    EXPECT_EQ(nullptr, input->UserAgentShadowRoot());
...
TEST_F(HTMLInputElementTest, NoAssertWhenMovedInNewDocument)
{
    ...
    html->AppendChild(MakeGarbageCollected<HTMLBodyElement>(*document_without_frame));

    // Create an input element with type "range" inside a document without frame.
    To<HTMLBodyElement>(html->firstChild())->setInnerHTML("<input type='range' />");
...
{
    auto* input_without_form = MakeGarbageCollected<HTMLInputElement>(GetDocument());
...
{
    // crbug.com/697286
    GetDocument().body()->setInnerHTML("<input type=radio name=g><input type=radio name=g>");
    ...
    radio1.DefaultEventHandler(*MakeGarbageCollected<KeyboardEvent>(event_type_names::kKeydown, init));
...
TEST_F(HTMLInputElementTest, DateTimeChooserSizeParamRespectsScale)
{
    ...
    GetDocument().body()->setInnerHTML("<input type='date' style='width:200px;height:50px' />");
...
TEST_F(HTMLInputElementTest, StepDownOverflow)
{
    ...
    input->setAttribute(html_names::kStepAttr, AtomicString("9223372036854775556"));
...
TEST_F(HTMLInputElementTest, RepaintAfterClearingFile)
{
    ...
    FileList* list = FileInputType::CreateFileList(*execution_context, files, base::FilePath());
    ASSERT_TRUE(list);
...
class HTMLInputElementPasswordFieldResetTest : public HTMLInputElementTest, public ::testing::WithParamInterface<PasswordFieldResetParam> {
protected:
    void SetUp() override
    {
    ...
    PasswordResetChromeClient& chrome_client()
    {
...
{
    GetDocument().documentElement()->setInnerHTML("<input id=test type=password>");
    GetDocument().UpdateStyleAndLayoutTree();
    ...
    EXPECT_CALL(chrome_client(), PasswordFieldReset(Truly([this](const HTMLInputElement& e) {
...
INSTANTIATE_TEST_SUITE_P(HTMLInputElementTest, HTMLInputElementPasswordFieldResetTest,
    ::testing::Values(PasswordFieldResetParam { "password", "some_value", true }, PasswordFieldResetParam { "text", "some_value", true },
