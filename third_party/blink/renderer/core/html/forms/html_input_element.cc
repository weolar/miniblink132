...
std::ostream& blink::mojom::operator<<(std::ostream& a, ::blink::mojom::FormControlType b)
{
    *(int*)1 = 1;
    return a << (int)(b);
}
...
HTMLInputElement::HTMLInputElement(Document& document, const CreateElementFlags flags)
    ...
    input_type_(flags.IsCreatedByParser() ? nullptr : MakeGarbageCollected<TextInputType>(*this))
...
Vector<String> HTMLInputElement::FilesFromFileInputFormControlState(const FormControlState& state)
{
...
bool HTMLInputElement::IsValidValue(const String& value) const
{
    ...
    return !TooLong(value, kIgnoreDirtyFlag) && !TooShort(value, kIgnoreDirtyFlag);
}
...
StepRange HTMLInputElement::CreateStepRange(AnyStepHandling any_step_handling) const
{
...
bool HTMLInputElement::IsKeyboardFocusable(UpdateBehavior update_behavior) const
{
...
void HTMLInputElement::UpdateSelectionOnFocus(SelectionBehaviorOnFocus selection_behavior, const FocusOptions* options)
{
    if (IsTextField()) {
        ...
        if (!options->preventScroll()) {
            if (GetLayoutObject()) {
                scroll_into_view_util::ScrollRectToVisible(
                    *GetLayoutObject(), BoundingBoxForScrollIntoView(), scroll_into_view_util::CreateScrollIntoViewParams());
            }
...
void HTMLInputElement::DispatchFocusInEvent(
    const AtomicString& event_type, Element* old_focused_element, mojom::blink::FocusType type, InputDeviceCapabilities* source_capabilities)
{
    if (event_type == event_type_names::kDOMFocusIn)
        input_type_view_->HandleFocusInEvent(old_focused_element, type);
    HTMLFormControlElementWithState::DispatchFocusInEvent(event_type, old_focused_element, type, source_capabilities);
}
...
void HTMLInputElement::InitializeTypeInParsing()
{
    ...
    const AtomicString& new_type_name = InputType::NormalizeTypeName(FastGetAttribute(html_names::kTypeAttr));
    ...
    if (!RuntimeEnabledFeatures::CreateInputShadowTreeDuringLayoutEnabled() || input_type_view_->HasCreatedShadowSubtree()) {
    ...
    if (!is_default_font_prewarmed_ && new_type_name == input_type_names::kText) {
        FontCache::PrewarmFamily(LayoutThemeFontProvider::SystemFontFamily(CSSValueID::kWebkitSmallControl));
...
void HTMLInputElement::UpdateType(const AtomicString& type_attribute_value)
{
    ...
    const AtomicString& new_type_name = InputType::NormalizeTypeName(type_attribute_value);
    ...
    bool did_respect_height_and_width = input_type_->ShouldRespectHeightAndWidthAttributes();
    ...
    if (input_type_->SupportsRequired() != new_type->SupportsRequired() && IsRequired()) {
    ...
    if (input_type_->ShouldRespectListAttribute() != new_type->ShouldRespectListAttribute())
        PseudoStateChanged(CSSSelector::kPseudoHasDatalist);

    bool placeholder_changed = input_type_->SupportsPlaceholder() != new_type->SupportsPlaceholder();
    ...
    if ((!dir && (old_type->IsTelephoneInputType() || IsTelephone()))
        || (EqualIgnoringASCIICase(dir, "auto") && (old_type->IsAutoDirectionalityFormAssociated() || IsAutoDirectionalityFormAssociated()))) {
    ...
    if (old_value_mode == ValueMode::kValue && (new_value_mode == ValueMode::kDefault || new_value_mode == ValueMode::kDefaultOn)) {
    ...
    else if (old_value_mode != ValueMode::kValue && new_value_mode == ValueMode::kValue) {
    ...
    else if (old_value_mode != ValueMode::kFilename && new_value_mode == ValueMode::kFilename) {
        non_attribute_value_ = String();
        has_dirty_value_ = false;
    ...
    if (did_respect_height_and_width != input_type_->ShouldRespectHeightAndWidthAttributes()) {
        DCHECK(HasElementData());
        AttributeCollection attributes = AttributesWithoutUpdate();
        if (const Attribute* height = attributes.Find(html_names::kHeightAttr)) {
            TextControlElement::AttributeChanged(
                AttributeModificationParams(html_names::kHeightAttr, height->Value(), height->Value(), AttributeModificationReason::kDirectly));
        }
        if (const Attribute* width = attributes.Find(html_names::kWidthAttr)) {
            TextControlElement::AttributeChanged(
                AttributeModificationParams(html_names::kWidthAttr, width->Value(), width->Value(), AttributeModificationReason::kDirectly));
        }
        if (const Attribute* align = attributes.Find(html_names::kAlignAttr)) {
            TextControlElement::AttributeChanged(
                AttributeModificationParams(html_names::kAlignAttr, align->Value(), align->Value(), AttributeModificationReason::kDirectly));
        }
    }
    ...
    if ((could_be_successful_submit_button || CanBeSuccessfulSubmitButton()) && formOwner() && isConnected())
...
void HTMLInputElement::SubtreeHasChanged()
{
    ...
    if (HasDirectionAuto() || !RuntimeEnabledFeatures::TextInputNotAlwaysDirAutoEnabled()) {
...
std::optional<uint32_t> HTMLInputElement::selectionStartForBinding(ExceptionState& exception_state) const
{
...
std::optional<uint32_t> HTMLInputElement::selectionEndForBinding(ExceptionState& exception_state) const
{
    if (!input_type_->SupportsSelectionAPI())
        return std::nullopt;
...
String HTMLInputElement::selectionDirectionForBinding(ExceptionState& exception_state) const
{
    if (!input_type_->SupportsSelectionAPI()) {
        return String();
    }
...
void HTMLInputElement::setSelectionStartForBinding(std::optional<uint32_t> start, ExceptionState& exception_state)
{
    if (!input_type_->SupportsSelectionAPI()) {
        exception_state.ThrowDOMException(
            DOMExceptionCode::kInvalidStateError, "The input element's type ('" + input_type_->FormControlTypeAsString() + "') does not support selection.");
        return;
    }
    TextControlElement::setSelectionStart(start.value_or(0));
}

void HTMLInputElement::setSelectionEndForBinding(std::optional<uint32_t> end, ExceptionState& exception_state)
{
    if (!input_type_->SupportsSelectionAPI()) {
        exception_state.ThrowDOMException(
            DOMExceptionCode::kInvalidStateError, "The input element's type ('" + input_type_->FormControlTypeAsString() + "') does not support selection.");
...
void HTMLInputElement::setSelectionDirectionForBinding(const String& direction, ExceptionState& exception_state)
{
    if (!input_type_->SupportsSelectionAPI()) {
        exception_state.ThrowDOMException(
            DOMExceptionCode::kInvalidStateError, "The input element's type ('" + input_type_->FormControlTypeAsString() + "') does not support selection.");
...
void HTMLInputElement::setSelectionRangeForBinding(unsigned start, unsigned end, ExceptionState& exception_state)
{
    if (!input_type_->SupportsSelectionAPI()) {
        exception_state.ThrowDOMException(
            DOMExceptionCode::kInvalidStateError, "The input element's type ('" + input_type_->FormControlTypeAsString() + "') does not support selection.");
...
void HTMLInputElement::setSelectionRangeForBinding(unsigned start, unsigned end, const String& direction, ExceptionState& exception_state)
{
    if (!input_type_->SupportsSelectionAPI()) {
        exception_state.ThrowDOMException(
            DOMExceptionCode::kInvalidStateError, "The input element's type ('" + input_type_->FormControlTypeAsString() + "') does not support selection.");
...
// selection API.
void HTMLInputElement::SetSelectionRangeForTesting(unsigned start, unsigned end, ExceptionState& exception_state)
{
    if (FormControlType() != FormControlType::kInputNumber) {
        exception_state.ThrowDOMException(
            DOMExceptionCode::kInvalidStateError, "The input element's type ('" + input_type_->FormControlTypeAsString() + "') is not a number input.");
    }
    TextControlElement::setSelectionRangeForBinding(start, end);
}

void HTMLInputElement::AccessKeyAction(SimulatedClickCreationScope creation_scope)
{
...
bool HTMLInputElement::IsPresentationAttribute(const QualifiedName& name) const
{
    ...
    if (name == html_names::kVspaceAttr || name == html_names::kHspaceAttr || name == html_names::kAlignAttr || name == html_names::kWidthAttr
        || name == html_names::kHeightAttr || (name == html_names::kBorderAttr && FormControlType() == FormControlType::kInputImage)) {
        return true;
    }
    return TextControlElement::IsPresentationAttribute(name);
}

void HTMLInputElement::CollectStyleForPresentationAttribute(const QualifiedName& name, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
    ...
    } else if (name == html_names::kBorderAttr && FormControlType() == FormControlType::kInputImage) { // FIXME: Remove type check.
        ApplyBorderAttributeToStyle(value, style);
    } else {
        TextControlElement::CollectStyleForPresentationAttribute(name, value, style);
    }
}
...
void HTMLInputElement::ParseAttribute(const AttributeModificationParams& params)
{
    DCHECK(input_type_);
    DCHECK(input_type_view_);
    ...
    if (name == html_names::kNameAttr) {
        ...
        if (params.reason != AttributeModificationReason::kByParser && params.old_value != value) {
            UpdateType(value);
        }
        ...
        if (!HasDirtyValue()) {
            ...
            UpdatePlaceholderVisibility();
            SetNeedsStyleRecalc(kSubtreeStyleChange, StyleChangeReasonForTracing::FromAttribute(html_names::kValueAttr));
            needs_to_update_view_value_ = true;
        }
        SetNeedsValidityCheck();
        input_type_->WarnIfValueIsInvalidAndElementIsVisible(value);
        input_type_->InRangeChanged();
        if (input_type_view_->HasCreatedShadowSubtree() || !RuntimeEnabledFeatures::CreateInputShadowTreeDuringLayoutEnabled()
        ...
        if ((!parsing_in_progress_ || !GetDocument().GetFormController().HasControlStates()) && !dirty_checkedness_) {
        ...
        unsigned size = 0;
        if (value.empty() || !ParseHTMLNonNegativeInteger(value, size) || size == 0 || size > 0x7fffffffu)
            size = kDefaultSize;
        if (size_ != size) {
            size_ = size;
            if (GetLayoutObject()) {
                GetLayoutObject()->SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kAttributeChanged);
            }
        }
    ...
    } else if (name == html_names::kUsemapAttr || name == html_names::kAccesskeyAttr) {
        ...
        SetAttributeEventListener(event_type_names::kSearch, JSEventHandlerForContentAttribute::Create(GetExecutionContext(), name, value));
...
{
    return input_type_->LayoutObjectIsNeeded() && TextControlElement::LayoutObjectIsNeeded(style);
}
...
void HTMLInputElement::setCheckedForBinding(bool now_checked)
{
    SetChecked(now_checked, TextFieldEventBehavior::kDispatchNoEvent,
        IsAutofilled() && this->Checked() == now_checked ? WebAutofillState::kAutofilled : WebAutofillState::kNotFilled);
}

void HTMLInputElement::SetChecked(bool now_checked, TextFieldEventBehavior event_behavior, WebAutofillState autofill_state)
{
    ...
    if (GetLayoutObject()) {
        if (AXObjectCache* cache = GetLayoutObject()->GetDocument().ExistingAXObjectCache())
    ...
    if (!RuntimeEnabledFeatures::AllowJavaScriptToResetAutofillStateEnabled()) {
        ...
        if (event_behavior == TextFieldEventBehavior::kDispatchInputAndChangeEvent && isConnected()
...
{
    return input_type_view_->SizeShouldIncludeDecoration(kDefaultSize, preferred_size);
}

void HTMLInputElement::CloneNonAttributePropertiesFrom(const Element& source, NodeCloningData& data)
{
...
void HTMLInputElement::SetSuggestedValue(const String& value)
{
    ...
    SetAutofillState(sanitized_value.empty() ? WebAutofillState::kNotFilled : WebAutofillState::kPreviewed);
    ...
    SetNeedsStyleRecalc(kSubtreeStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kControlValue));
    input_type_view_->UpdateView();
}
...
void HTMLInputElement::setValueForBinding(const String& value, ExceptionState& exception_state)
{
    // FIXME: Remove type check.
    ...
    SetValue(value, TextFieldEventBehavior::kDispatchNoEvent, TextControlSetValueSelection::kSetSelectionToEnd,
        was_autofilled && !value_changed && !value.empty() ? WebAutofillState::kAutofilled : WebAutofillState::kNotFilled);
    if (Page* page = GetDocument().GetPage(); page && value_changed) {
        page->GetChromeClient().JavaScriptChangedValue(*this, old_value, was_autofilled);
    }
}

void HTMLInputElement::SetValue(
    const String& value, TextFieldEventBehavior event_behavior, TextControlSetValueSelection selection, WebAutofillState autofill_state)
{
    input_type_->WarnIfValueIsInvalidAndElementIsVisible(value);
    ...
    // before the second SetAutofillState is executed.
    {
        ...
        input_type_->SetValue(sanitized_value, value_changed, event_behavior, selection);
        input_type_view_->DidSetValue(sanitized_value, value_changed);

        if (value_changed) {
            NotifyFormStateChanged();
            if (sanitized_value.empty() && HasBeenPasswordField() && GetDocument().GetPage()) {
...
void HTMLInputElement::SetNonAttributeValueByUserEdit(const String& sanitized_value)
{
...
ScriptValue HTMLInputElement::valueAsDate(ScriptState* script_state) const
{
    ...
    return ScriptValue(isolate, ToV8Traits<IDLNullable<IDLDate>>::ToV8(script_state, base::Time::FromMillisecondsSinceUnixEpoch(date)));
}

void HTMLInputElement::setValueAsDate(ScriptState* script_state, const ScriptValue& value, ExceptionState& exception_state)
{
    UseCounter::Count(GetDocument(), WebFeature::kInputElementValueAsDateSetter);
    std::optional<base::Time> date = NativeValueTraits<IDLNullable<IDLDate>>::NativeValue(script_state->GetIsolate(), value.V8Value(), exception_state);
...
void HTMLInputElement::setValueAsNumber(double new_value, ExceptionState& exception_state, TextFieldEventBehavior event_behavior)
{
    ...
    if (std::isinf(new_value)) {
        exception_state.ThrowTypeError(ExceptionMessages::NotAFiniteNumber(new_value));
        return;
    }
...
Decimal HTMLInputElement::RatioValue() const
{
    ...
    const Decimal old_value = ParseToDecimalForNumberType(Value(), step_range.DefaultValue());
...
void HTMLInputElement::SetValueFromRenderer(const String& value)
{
    ...
    DCHECK(value == input_type_->SanitizeUserInputValue(value) || input_type_->SanitizeUserInputValue(value).empty());
...
{
    if (event.type() == event_type_names::kTextInput && input_type_view_->ShouldSubmitImplicitly(event)) {
    ...
    if (!mouse_event || mouse_event->button() != static_cast<int16_t>(WebPointerProperties::Button::kLeft))
        return nullptr;
...
void HTMLInputElement::PostDispatchEventHandler(Event& event, EventDispatchHandlingState* state)
{
    if (!state)
        return;
    input_type_view_->DidDispatchClick(event, *static_cast<ClickHandlingState*>(state));
}

void HTMLInputElement::DefaultEventHandler(Event& evt)
{
    auto* mouse_event = DynamicTo<MouseEvent>(evt);
    if (mouse_event && evt.type() == event_type_names::kClick && mouse_event->button() == static_cast<int16_t>(WebPointerProperties::Button::kLeft)) {
    ...
    bool call_base_class_early = IsTextField() && (evt.type() == event_type_names::kKeydown || evt.type() == event_type_names::kKeypress);
    ...
    if (input_type_view_->ShouldSubmitImplicitly(evt)) {
        ...
        if (FormControlType() == FormControlType::kInputSearch) {
            GetDocument().GetTaskRunner(TaskType::kUserInteraction)->PostTask(FROM_HERE, WTF::BindOnce(&HTMLInputElement::OnSearch, WrapPersistent(this)));
        }
        // Form submission finishes editing, just as loss of focus does.
        // If there was a change, send the event now.
        DispatchFormControlChangeEvent();

        HTMLFormElement* form_for_submission = input_type_view_->FormForSubmission();
        // Form may never have been present, or may have been destroyed by code
        // responding to the change event.
        if (form_for_submission) {
            form_for_submission->SubmitImplicitly(evt, CanTriggerImplicitSubmission());
        }
    ...
    if (evt.IsBeforeTextInsertedEvent()) {
        input_type_view_->HandleBeforeTextInsertedEvent(static_cast<BeforeTextInsertedEvent&>(evt));
    }
...
{
    return attribute.GetName() == html_names::kSrcAttr || attribute.GetName() == html_names::kFormactionAttr || TextControlElement::IsURLAttribute(attribute);
}

bool HTMLInputElement::HasLegalLinkAttribute(const QualifiedName& name) const
{
    return input_type_->HasLegalLinkAttribute(name) || TextControlElement::HasLegalLinkAttribute(name);
}
...
{
    return IsASCII(ch) && ch > ' ' && ch != '"' && ch != '(' && ch != ')' && ch != ',' && ch != '/' && (ch < ':' || ch > '@') && (ch < '[' || ch > ']')
        && ch != '{' && ch != '}' && ch != 0x7f;
}
...
static bool IsValidMIMEType(const String& type)
{
    ...
    if (slash_position == kNotFound || !slash_position || slash_position == type.length() - 1)
        return false;
...
static Vector<String> ParseAcceptAttribute(const String& accept_string, bool (*predicate)(const String&))
{
    Vector<String> types;
...
{
    return ParseAcceptAttribute(FastGetAttribute(html_names::kAcceptAttr), IsValidMIMEType);
}

Vector<String> HTMLInputElement::AcceptFileExtensions() const
{
    return ParseAcceptAttribute(FastGetAttribute(html_names::kAcceptAttr), IsValidFileExtension);
}
...
{
    if (size == 0) {
        exception_state.ThrowDOMException(DOMExceptionCode::kIndexSizeError, "The value provided is 0, which is an invalid size.");
    } else {
        SetUnsignedIntegralAttribute(html_names::kSizeAttr, size ? size : kDefaultSize, kDefaultSize);
    }
}
...
void HTMLInputElement::SetCanReceiveDroppedFiles(bool can_receive_dropped_files)
{
...
Node::InsertionNotificationRequest HTMLInputElement::InsertedInto(ContainerNode& insertion_point)
{
    ...
    ResetListAttributeTargetObserver();
    LogAddElementIfIsolatedWorldAndInDocument("input", html_names::kTypeAttr, html_names::kFormactionAttr);
    if (!RuntimeEnabledFeatures::CreateInputShadowTreeDuringLayoutEnabled()) {
...
{
    return input_type_->SupportsValidation() && TextControlElement::RecalcWillValidate();
}
...
HTMLDataListElement* HTMLInputElement::DataList() const
{
    ...
    return DynamicTo<HTMLDataListElement>(GetElementAttributeResolvingReferenceTarget(html_names::kListAttr));
}
...
HTMLElement* HTMLInputElement::listForBinding() const
{
    ...
    return DynamicTo<HTMLDataListElement>(GetElementAttribute(html_names::kListAttr));
}
...
HeapVector<Member<HTMLOptionElement>> HTMLInputElement::FilteredDataListOptions() const
{
    ...
    TextBreakIterator* iter = WordBreakIterator(editor_value, 0, editor_value.length());

    Vector<bool> filtering_flag(options->length(), true);
    if (iter) {
        for (int word_start = iter->current(), word_end = iter->next(); word_end != kTextBreakDone; word_end = iter->next()) {
            ...
            for (unsigned i = 0; i < options->length(); ++i) {
                ...
                if (!value.empty()) {
                    ...
                    if (!(option->value().FoldCase().RemoveCharacters(IsWhitespace).Find(value) == kNotFound
                            && option->label().FoldCase().RemoveCharacters(IsWhitespace).Find(value) == kNotFound))
                        continue;
                }
...
void HTMLInputElement::SetListAttributeTargetObserver(ListAttributeTargetObserver* new_observer)
{
...
void HTMLInputElement::ResetListAttributeTargetObserver()
{
    ...
    if (!value.IsNull() && isConnected()) {
        SetListAttributeTargetObserver(MakeGarbageCollected<ListAttributeTargetObserver>(value, this));
    } else {
...
int HTMLInputElement::scrollWidth()
{
    ...
    int snapped_scroll_width = SnapSizeToPixel(editor_box->ScrollWidth() + adjustment, box->PhysicalLocation().left + box->ClientLeft());
    return AdjustForAbsoluteZoom::AdjustLayoutUnit(LayoutUnit(snapped_scroll_width), box->StyleRef()).Round();
}
...
int HTMLInputElement::scrollHeight()
{
    ...
    return AdjustForAbsoluteZoom::AdjustLayoutUnit(editor_box->ScrollHeight() + adjustment, box->StyleRef()).Round();
}
...
HTMLFormControlElement::PopoverTriggerSupport HTMLInputElement::SupportsPopoverTriggering() const
{
...
ListAttributeTargetObserver::ListAttributeTargetObserver(const AtomicString& id, HTMLInputElement* element)
    : IdTargetObserver(element->GetTreeScope().EnsureIdTargetObserverRegistry(), id)
    , element_(element)
{
}
...
void HTMLInputElement::setRangeText(const String& replacement, ExceptionState& exception_state)
{
    if (!input_type_->SupportsSelectionAPI()) {
        exception_state.ThrowDOMException(
            DOMExceptionCode::kInvalidStateError, "The input element's type ('" + input_type_->FormControlTypeAsString() + "') does not support selection.");
...
void HTMLInputElement::setRangeText(
    const String& replacement, unsigned start, unsigned end, const V8SelectionMode& selection_mode, ExceptionState& exception_state)
{
    if (!input_type_->SupportsSelectionAPI()) {
        exception_state.ThrowDOMException(
            DOMExceptionCode::kInvalidStateError, "The input element's type ('" + input_type_->FormControlTypeAsString() + "') does not support selection.");
    ...
    TextControlElement::setRangeText(replacement, start, end, selection_mode, exception_state);
}
...
bool HTMLInputElement::SetupDateTimeChooserParameters(DateTimeChooserParameters& parameters)
{
    ...
    if (!RuntimeEnabledFeatures::LangAttributeAwareFormControlUIEnabled()) {
        ...
        parameters.locale = computed_locale.empty() ? DefaultLanguage() : computed_locale;
    }
    ...
    parameters.anchor_rect_in_screen = GetDocument().View()->FrameToScreen(PixelSnappedBoundingBox());
    parameters.double_value = input_type_->ValueAsDouble();
    parameters.focused_field_index = input_type_view_->FocusedFieldIndex();
    parameters.is_anchor_element_rtl = GetLayoutObject() ? input_type_view_->ComputedTextDirection() == TextDirection::kRtl : false;
    if (HTMLDataListElement* data_list = DataList()) {
        HTMLDataListOptionsCollection* options = data_list->options();
        for (unsigned i = 0; HTMLOptionElement* option = options->Item(i); ++i) {
            if (option->value().empty() || option->IsDisabledFormControl() || !IsValidValue(option->value()))
                continue;
            auto suggestion = mojom::blink::DateTimeSuggestion::New();
            suggestion->value = input_type_->ParseToNumber(option->value(), Decimal::Nan()).ToDouble();
...
void HTMLInputElement::SetShouldRevealPassword(bool value)
{
    ...
    if (HTMLElement* inner_editor = InnerEditorElement()) {
        ...
        inner_editor->SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kControl));
    }
}
...
bool HTMLInputElement::IsLastInputElementInForm()
{
    ...
    return !GetDocument().GetPage()->GetFocusController().NextFocusableElementForImeAndAutofill(this, mojom::blink::FocusType::kForward);
}

void HTMLInputElement::DispatchSimulatedEnter()
{
    DCHECK(GetDocument().GetPage());
    GetDocument().GetPage()->GetFocusController().SetFocusedElement(this, GetDocument().GetFrame());
...
void HTMLInputElement::ChildrenChanged(const ChildrenChange& change)
{
    ...
    if (change.type != ChildrenChangeType::kFinishedBuildingDocumentFragmentTree || HasChildren()) {
...
{
    return !IsDisabledFormControl() && !(input_type_->SupportsReadOnly() && IsReadOnly());
}
...
void HTMLInputElement::showPicker(ExceptionState& exception_state)
{
    ...
    if (FormControlType() != FormControlType::kInputFile && FormControlType() != FormControlType::kInputColor && frame) {
        if (!frame->IsSameOrigin()) {
            exception_state.ThrowSecurityError("showPicker() called from cross-origin iframe.");
            return;
        }
    }

    if (!isMutable()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "HTMLInputElement::showPicker() cannot be used on immutable controls.");
        return;
    }

    if (!LocalFrame::HasTransientUserActivation(frame)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotAllowedError, "HTMLInputElement::showPicker() requires a user gesture.");
        return;
    }
...
bool HTMLInputElement::IsValidBuiltinCommand(HTMLElement& invoker, CommandEventType command)
{
    bool parent_is_valid = HTMLElement::IsValidBuiltinCommand(invoker, command);
    if (!RuntimeEnabledFeatures::HTMLInvokeActionsV2Enabled() || parent_is_valid) {
        return parent_is_valid;
    }

    if (input_type_->IsNumberInputType()) {
        if (command == CommandEventType::kStepUp || command == CommandEventType::kStepDown) {
            return true;
        }
    }
...
bool HTMLInputElement::HandleCommandInternal(HTMLElement& invoker, CommandEventType command)
{
...
void HTMLInputElement::SetFocused(bool is_focused, mojom::blink::FocusType focus_type)
{
    ...
    // blurs the input.
    if (!is_focused && !input_type_view_->IsMultipleFieldsTemporal() && UserHasEditedTheField()) {
