...
{
    return node.HasTagName(html_names::kOptionTag) || node.HasTagName(html_names::kHrTag);
}
...
void HTMLOptGroupElement::ParseAttribute(const AttributeModificationParams& params)
{
...
FocusableState HTMLOptGroupElement::SupportsFocus(UpdateBehavior update_behavior) const
{
...
void HTMLOptGroupElement::ChildrenChanged(const ChildrenChange& change)
{
    ...
    DCHECK_NE(change.type, ChildrenChangeType::kFinishedBuildingDocumentFragmentTree);
...
Node::InsertionNotificationRequest HTMLOptGroupElement::InsertedInto(ContainerNode& insertion_point)
{
    ...
    if (HTMLSelectElement* select = OwnerSelectElement()) {
        ...
        customizable_select_rendering_ = RuntimeEnabledFeatures::CustomizableSelectEnabled() && select->UsesMenuList();
    }
...
String HTMLOptGroupElement::GroupLabelText() const
{
    ...
    if (RuntimeEnabledFeatures::CustomizableSelectEnabled() && label_attribute_text.ContainsOnlyWhitespaceOrEmpty()) {
...
HTMLSelectElement* HTMLOptGroupElement::OwnerSelectElement() const
{
    if (RuntimeEnabledFeatures::SelectParserRelaxationEnabled()) {
        ...
        for (Node& ancestor : NodeTraversal::AncestorsOf(*this)) {
            if (IsA<HTMLOptGroupElement>(ancestor) || IsA<HTMLOptionElement>(ancestor)) {
                return nullptr;
            }
...
void HTMLOptGroupElement::AccessKeyAction(SimulatedClickCreationScope creation_scope)
{
    HTMLSelectElement* select = OwnerSelectElement();
...
{
    auto* element = UserAgentShadowRoot()->getElementById(shadow_element_names::kIdOptGroupLabel);
