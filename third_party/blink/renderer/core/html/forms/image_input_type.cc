...
void ImageInputType::AltAttributeChanged()
{
    if (GetElement().UserAgentShadowRoot()) {
        Element* text = GetElement().UserAgentShadowRoot()->getElementById(AtomicString("alttext"));
...
void ImageInputType::SrcAttributeChanged()
{
    ...
    GetElement().EnsureImageLoader().UpdateFromElement(ImageLoader::kUpdateIgnorePreviousError);
}
...
unsigned ImageInputType::Height() const
{
    if (!GetElement().GetLayoutObject()) {
        ...
        unsigned height;
        if (ParseHTMLNonNegativeInteger(GetElement().FastGetAttribute(html_names::kHeightAttr), height))
            return height;

        // If the image is available, use its height.
        HTMLImageLoader* image_loader = GetElement().ImageLoader();
        if (image_loader && image_loader->GetContent()) {
            return image_loader->GetContent()->IntrinsicSize(kRespectImageOrientation).height();
        }
    }

    GetElement().GetDocument().UpdateStyleAndLayoutForNode(&GetElement(), DocumentUpdateReason::kJavaScript);

    LayoutBox* box = GetElement().GetLayoutBox();
    return box ? AdjustForAbsoluteZoom::AdjustInt(box->ContentHeight().ToInt(), box) : 0;
}
...
unsigned ImageInputType::Width() const
{
    if (!GetElement().GetLayoutObject()) {
        // Check the attribute first for an explicit pixel value.
        unsigned width;
        if (ParseHTMLNonNegativeInteger(GetElement().FastGetAttribute(html_names::kWidthAttr), width))
            return width;

        // If the image is available, use its width.
        HTMLImageLoader* image_loader = GetElement().ImageLoader();
        if (image_loader && image_loader->GetContent()) {
            return image_loader->GetContent()->IntrinsicSize(kRespectImageOrientation).width();
        }
    }
    ...
    LayoutBox* box = GetElement().GetLayoutBox();
    return box ? AdjustForAbsoluteZoom::AdjustInt(box->ContentWidth().ToInt(), box) : 0;
}

bool ImageInputType::HasLegalLinkAttribute(const QualifiedName& name) const
{
    return name == html_names::kSrcAttr || BaseButtonInputType::HasLegalLinkAttribute(name);
}
...
void ImageInputType::ReattachFallbackContent()
{
    if (!GetElement().GetDocument().InStyleRecalc()) {
        ...
        GetElement().SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kUseFallback));
