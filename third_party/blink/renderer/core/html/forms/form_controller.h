...
public:
    ...
    CORE_EXPORT static FormControlState Deserialize(const Vector<String>& state_vector, wtf_size_t& index);
    FormControlState(const FormControlState& another) = default;
    FormControlState& operator=(const FormControlState&);

    bool IsFailure() const
    {
        return type_ == kTypeFailure;
    }
    wtf_size_t ValueSize() const
    {
        return values_.size();
    }
    const String& operator[](wtf_size_t i) const
    {
        return values_[i];
    }
...
inline FormControlState& FormControlState::operator=(const FormControlState& another) = default;
...
class CORE_EXPORT FormController final : public GarbageCollected<FormController> {
public:
    ...
    bool DropReferencedFilePaths() const
    {
        return drop_referenced_file_paths_;
    }
    void SetDropReferencedFilePathsForTesting()
    {
        drop_referenced_file_paths_ = true;
    }
    static Vector<String> GetReferencedFilePaths(const Vector<String>& state_vector);

private:
    FormControlState TakeStateForControl(const ListedElement&);
    static void ControlStatesFromStateVector(const Vector<String>&, SavedFormStateMap&);
