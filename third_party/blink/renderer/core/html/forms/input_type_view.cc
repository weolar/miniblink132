...
bool InputTypeView::SizeShouldIncludeDecoration(int, int& preferred_size) const
{
...
{
    GetElement().Focus(
        FocusParams(SelectionBehaviorOnFocus::kReset, mojom::blink::FocusType::kNone, nullptr, FocusOptions::Create(), FocusTrigger::kUserGesture));
}

bool InputTypeView::ShouldSubmitImplicitly(const Event& event)
{
    auto* keyboard_event = DynamicTo<KeyboardEvent>(event);
    return keyboard_event && event.type() == event_type_names::kKeypress && keyboard_event->charCode() == '\r';
}
...
void InputTypeView::CreateShadowSubtreeIfNeeded(bool is_type_changing)
{
    ...
    if (RuntimeEnabledFeatures::CreateInputShadowTreeDuringLayoutEnabled() && !is_type_changing) {
        ...
        if (!GetElement().SuggestedValue().empty() || GetElement().FastHasAttribute(html_names::kPlaceholderAttr)) {
