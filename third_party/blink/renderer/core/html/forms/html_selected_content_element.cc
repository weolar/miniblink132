...
void HTMLSelectedContentElement::CloneContentsFromOptionElement(const HTMLOptionElement* option)
{
    if (disabled_) {
        return;
    }
...
Node::InsertionNotificationRequest HTMLSelectedContentElement::InsertedInto(ContainerNode& insertion_point)
{
    ...
    for (auto* ancestor = parentNode(); ancestor; ancestor = ancestor->parentNode()) {
        ...
        if (IsA<HTMLOptionElement>(ancestor) || IsA<HTMLSelectedContentElement>(ancestor)) {
