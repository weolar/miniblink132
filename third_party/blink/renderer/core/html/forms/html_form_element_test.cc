...
protected:
    ...
    HTMLFormElement* GetFormElement(const char* id, ShadowRoot* shadow_root = nullptr)
    {
        return DynamicTo<HTMLFormElement>(shadow_root ? shadow_root->getElementById(AtomicString(id)) : GetElementById(id));
    }

    ListedElement* GetListedElement(const char* id, ShadowRoot* shadow_root = nullptr)
    {
        if (Element* element = shadow_root ? shadow_root->getElementById(AtomicString(id)) : GetElementById(id)) {
...
TEST_F(HTMLFormElementTest, ListedElementsIncludeShadowTrees)
{
    ...
    HTMLDivElement* form1div = MakeGarbageCollected<HTMLDivElement>(GetDocument());
    form1->AppendChild(form1div);
    ShadowRoot& form1root = form1div->AttachShadowRootForTesting(ShadowRootMode::kOpen);
    ...
    HTMLDivElement* form3div = MakeGarbageCollected<HTMLDivElement>(GetDocument());
    form3->AppendChild(form3div);
    ShadowRoot& form3root = form3div->AttachShadowRootForTesting(ShadowRootMode::kOpen);
...
TEST_F(HTMLFormElementTest, ListedElementsAfterIncludeShadowTrees)
{
    ...
    EXPECT_THAT(form1->ListedElements(/*include_shadow_trees=*/true),
        ElementsAre(GetListedElement("input1"), GetListedElement("input2", GetElementById("div1")->GetShadowRoot())));
...
TEST_F(HTMLFormElementTest, ListedElementsIncludesOnlyDescendants)
{
    ...
    HTMLFormElement* form1 = GetFormElement("form1");
    ASSERT_NE(form1, nullptr);
    EXPECT_THAT(form1->ListedElements(/*include_shadow_trees=*/true), ElementsAre(GetListedElement("input1", GetElementById("div1")->GetShadowRoot())));
}
...
TEST_F(HTMLFormElementTest, ListedElementsInNestedForms)
{
    ...
    EXPECT_THAT(f1->ListedElements(/*include_shadow_trees=*/true),
        ElementsAre(GetListedElement("i1", shadow_root), GetListedElement("i2", shadow_root), GetListedElement("i3", shadow_root)));
}
...
TEST_F(HTMLFormElementTest, ListedElementsInDeepNestedForms)
{
    ...
    ShadowRoot* shadow_root2 = shadow_root1->getElementById(AtomicString("shadowhost2"))->GetShadowRoot();
    ...
    EXPECT_THAT(f1->ListedElements(), IsEmpty());
    EXPECT_THAT(f1->ListedElements(/*include_shadow_trees=*/true), ElementsAre(i1, i2));

    // Test that cache invalidation happens for all ancestor forms when an input
    // field is added and removed.
    HTMLInputElement* input = MakeGarbageCollected<HTMLInputElement>(GetDocument());
    d1->AppendChild(input);
    EXPECT_THAT(f1->ListedElements(/*include_shadow_trees=*/true), ElementsAre(i1, i2, input));
    input->remove();
    EXPECT_THAT(f1->ListedElements(/*include_shadow_trees=*/true), ElementsAre(i1, i2));
...
TEST_F(HTMLFormElementTest, ListedElementsInDeepNestedFormsLightDom)
{
    ...
    EXPECT_THAT(f1->ListedElements(/*include_shadow_trees=*/true), ElementsAre(input));
    EXPECT_THAT(f2->ListedElements(/*include_shadow_trees=*/true), IsEmpty());
    EXPECT_THAT(f3->ListedElements(/*include_shadow_trees=*/true), IsEmpty());
    ...
    EXPECT_THAT(f1->ListedElements(/*include_shadow_trees=*/true), ElementsAre(input));
    EXPECT_THAT(f2->ListedElements(/*include_shadow_trees=*/true), ElementsAre(input));
    EXPECT_THAT(f3->ListedElements(/*include_shadow_trees=*/true), IsEmpty());
    ...
    EXPECT_THAT(f1->ListedElements(/*include_shadow_trees=*/true), ElementsAre(input));
    EXPECT_THAT(f2->ListedElements(/*include_shadow_trees=*/true), ElementsAre(input));
    EXPECT_THAT(f3->ListedElements(/*include_shadow_trees=*/true), ElementsAre(input));
}
...
TEST_F(HTMLFormElementTest, ShadowDomTreesMustBeDescendantsOfForm)
{
    ...
    EXPECT_THAT(f1->ListedElements(), ElementsAre(GetListedElement("i1"), GetListedElement("i2")));
    EXPECT_THAT(f1->ListedElements(/*include_shadow_trees=*/true), ElementsAre(GetListedElement("i1"), GetListedElement("i2")));
}
...
TEST_F(HTMLFormElementTest, FormInsertionsInvalidateFormCaches)
{
    ...
    // true.
    EXPECT_THAT(f1->ListedElements(), IsEmpty());
    EXPECT_THAT(f1->ListedElements(/*include_shadow_trees=*/true), ElementsAre(t));
    EXPECT_THAT(f2->ListedElements(), IsEmpty());
    EXPECT_THAT(f2->ListedElements(/*include_shadow_trees=*/true), ElementsAre(t));
    EXPECT_THAT(f3->ListedElements(), ElementsAre(t));
    EXPECT_THAT(f3->ListedElements(/*include_shadow_trees=*/true), ElementsAre(t));
}
...
TEST_F(HTMLFormElementTest, ElementsAssociateWithNestedForms)
{
    ...
    EXPECT_THAT(f1->ListedElements(/*include_shadow_trees=*/true), ElementsAre(t1, t2));
}
...
TEST_F(HTMLFormElementSimTest, NestedFormsAssociatedByParserMalformedHtml_Large)
{
    LoadHTML(R"HTML(
    <!DOCTYPE html>
    <div>
    ...
    EXPECT_THAT(f1->ListedElements(/*include_shadow_trees=*/true), ElementsAre(t2, t3, t4));
}
