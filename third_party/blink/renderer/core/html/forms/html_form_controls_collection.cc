...
HTMLFormControlsCollection::HTMLFormControlsCollection(ContainerNode& owner_node, CollectionType type)
...
static unsigned FindListedElement(const ListedElement::List& listed_elements, Element* element)
{
    unsigned i = 0;
    for (; i < listed_elements.size(); ++i) {
        ListedElement* listed_element = listed_elements[i];
        if (listed_element->IsEnumeratable() && &listed_element->ToHTMLElement() == element)
            break;
    }
    return i;
}
...
static HTMLElement* FirstNamedItem(const ListedElement::List& elements_array, const QualifiedName& attr_name, const String& name)
{
    DCHECK(attr_name == html_names::kIdAttr || attr_name == html_names::kNameAttr);

    for (const auto& listed_element : elements_array) {
        HTMLElement& element = listed_element->ToHTMLElement();
        if (listed_element->IsEnumeratable() && element.FastGetAttribute(attr_name) == name)
            return &element;
    }
    return nullptr;
}
...
HTMLElement* HTMLFormControlsCollection::namedItem(const AtomicString& name) const
{
    ...
    if (HTMLElement* item = FirstNamedItem(ListedElements(), html_names::kIdAttr, name))
        return item;
...
void HTMLFormControlsCollection::UpdateIdNameCache() const
{
    ...
    for (const auto& element : FormImageElements()) {
        const AtomicString& id_attr_val = element->GetIdAttribute();
        const AtomicString& name_attr_val = element->GetNameAttribute();
        if (!id_attr_val.empty() && !found_input_elements.Contains(id_attr_val.Impl()))
            cache->AddElementWithId(id_attr_val, element);
        if (!name_attr_val.empty() && id_attr_val != name_attr_val && !found_input_elements.Contains(name_attr_val.Impl()))
            cache->AddElementWithName(name_attr_val, element);
    }
...
V8UnionElementOrRadioNodeList* HTMLFormControlsCollection::namedGetter(const AtomicString& name)
{
    ...
    if (named_items.size() == 1) {
        if (!IsA<HTMLImageElement>(*named_items[0])) {
            return MakeGarbageCollected<V8UnionElementOrRadioNodeList>(named_items[0]);
        }
        return nullptr;
    }
    ...
    return MakeGarbageCollected<V8UnionElementOrRadioNodeList>(ownerNode().GetRadioNodeList(name));
}
