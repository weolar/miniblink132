...
LabelsNodeList::LabelsNodeList(ContainerNode& owner_node)
    : LiveNodeList(owner_node, kLabelsNodeListType, kInvalidateForFormControls, NodeListSearchRoot::kTreeScope)
{
}
...
ContainerNode& LabelsNodeList::RootNode() const
{
    ...
    while (host && host->GetShadowReferenceTarget(html_names::kForAttr) == &ownerNode()) {
...
Element* LabelsNodeList::Next(Element& current) const
{
    if (current.GetShadowReferenceTarget(html_names::kForAttr) == &ownerNode()) {
        ...
        if (Element* first = ElementTraversal::FirstWithin(current.GetShadowRoot()->RootNode())) {
            return first;
        }
    }
...
Element* LabelsNodeList::Previous(Element& current) const
{
    ...
    if (!prev) {
        ...
        Element* host = current.OwnerShadowHost();
        if (host && host->GetShadowReferenceTarget(html_names::kForAttr) == &ownerNode()) {
            DCHECK(host->IsShadowIncludingAncestorOf(ownerNode()));
            return host;
        }
        return nullptr;
    } else if (prev->GetShadowReferenceTarget(html_names::kForAttr) == &ownerNode()) {
        DCHECK(prev->IsShadowIncludingAncestorOf(ownerNode()));
        // If the owner node is the reference target of the previous element,
        // drill into its shadow tree to continue iterating.
        if (Element* last = ElementTraversal::LastWithin(prev->GetShadowRoot()->RootNode())) {
            return last;
        }
    }
...
Element* LabelsNodeList::TraverseToFirst() const
{
    ...
    for (Element* ele = ElementTraversal::FirstWithin(RootNode()); ele; ele = Next(*ele)) {
...
Element* LabelsNodeList::TraverseToLast() const
{
    ...
    for (Element* ele = ElementTraversal::LastWithin(RootNode()); ele; ele = Previous(*ele)) {
        if (ElementMatches(*ele)) {
            return ele;
        }
    }
...
Element* LabelsNodeList::TraverseForwardToOffset(unsigned offset, Element& current_node, unsigned& current_offset) const
{
    if (!RuntimeEnabledFeatures::ShadowRootReferenceTargetEnabled()) {
        return LiveNodeList::TraverseForwardToOffset(offset, current_node, current_offset);
    }
...
Element* LabelsNodeList::TraverseBackwardToOffset(unsigned offset, Element& current_node, unsigned& current_offset) const
{
    if (!RuntimeEnabledFeatures::ShadowRootReferenceTargetEnabled()) {
        return LiveNodeList::TraverseBackwardToOffset(offset, current_node, current_offset);
    }
