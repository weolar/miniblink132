...
HTMLInputElement* NextInputElement(const HTMLInputElement& element, const HTMLFormElement* stay_within, bool forward)
{
    return forward ? Traversal<HTMLInputElement>::Next(element, stay_within) : Traversal<HTMLInputElement>::Previous(element, stay_within);
}
...
bool RadioInputType::ValueMissing(const String&) const
{
    ...
    for (auto* another = Traversal<HTMLInputElement>::InclusiveFirstWithin(root); another; another = Traversal<HTMLInputElement>::Next(*another, &root)) {
        if (another->FormControlType() != FormControlType::kInputRadio || another->GetName() != name || another->formOwner()) {
            continue;
        }
        if (another->Checked())
            is_checked = true;
...
HTMLInputElement* RadioInputType::FindNextFocusableRadioButtonInGroup(HTMLInputElement* current_element, bool forward)
{
    for (HTMLInputElement* input_element = NextRadioButtonInGroup(current_element, forward); input_element;
...
void RadioInputType::HandleKeydownEvent(KeyboardEvent& event)
{
    ...
    if (key != keywords::kArrowUp && key != keywords::kArrowDown && key != keywords::kArrowLeft && key != keywords::kArrowRight) {
        return;
    }
    ...
    if (IsSpatialNavigationEnabled(document.GetFrame()))
        return;
    bool forward = ComputedTextDirection() == TextDirection::kRtl ? (key == keywords::kArrowDown || key == keywords::kArrowLeft)
    ...
    HTMLInputElement* input_element = FindNextFocusableRadioButtonInGroup(&GetElement(), forward);
    if (!input_element) {
        // Traverse in reverse direction till last or first radio button
        forward = !(forward);
        HTMLInputElement* next_input_element = FindNextFocusableRadioButtonInGroup(&GetElement(), forward);
        while (next_input_element) {
            input_element = next_input_element;
            next_input_element = FindNextFocusableRadioButtonInGroup(next_input_element, forward);
        }
    }
    if (input_element) {
        document.SetFocusedElement(input_element, FocusParams(SelectionBehaviorOnFocus::kRestore, mojom::blink::FocusType::kNone, nullptr));
...
void RadioInputType::HandleKeyupEvent(KeyboardEvent& event)
{
    ...
    if (event.key() == " " || (IsSpatialNavigationEnabled(GetElement().GetDocument().GetFrame()) && event.key() == keywords::kCapitalEnter)) {
...
bool RadioInputType::IsKeyboardFocusable(Element::UpdateBehavior update_behavior) const
{
    ...
    Element* current_focused_element = GetElement().GetDocument().FocusedElement();
    if (auto* focused_input = DynamicTo<HTMLInputElement>(current_focused_element)) {
        if (focused_input->FormControlType() == FormControlType::kInputRadio && focused_input->GetTreeScope() == GetElement().GetTreeScope()
            && focused_input->Form() == GetElement().Form() && focused_input->GetName() == GetElement().GetName()) {
            return false;
        }
    }
...
void RadioInputType::DidDispatchClick(Event& event, const ClickHandlingState& state)
{
    if (event.defaultPrevented() || event.DefaultHandled()) {
        ...
        } else if (checked_radio_button->FormControlType() == FormControlType::kInputRadio && checked_radio_button->Form() == GetElement().Form()
...
HTMLInputElement* RadioInputType::NextRadioButtonInGroup(HTMLInputElement* current, bool forward)
{
    ...
    // case-insensitive.
    for (HTMLInputElement* input_element = NextInputElement(*current, current->Form(), forward); input_element;
         input_element = NextInputElement(*input_element, current->Form(), forward)) {
        if (current->Form() == input_element->Form() && input_element->GetTreeScope() == current->GetTreeScope()
            && input_element->FormControlType() == FormControlType::kInputRadio && input_element->GetName() == current->GetName()) {
            return input_element;
        }
    }
    return nullptr;
