...
public:
    ...
    MockFileChooser(const blink::BrowserInterfaceBrokerProxy& broker, base::OnceClosure reached_callback)
        : broker_(broker)
        , reached_callback_(std::move(reached_callback))
    {
        broker.SetBinderForTesting(FileChooser::Name_, WTF::BindRepeating(&MockFileChooser::BindFileChooserReceiver, WTF::Unretained(this)));
    }
    ...
    {
        DCHECK(callback_) << "OpenFileChooser() or EnumerateChosenDirectory() should "
                             "be called beforehand.";
        std::move(callback_).Run(mojom::blink::FileChooserResult::New(std::move(files), base::FilePath()));
