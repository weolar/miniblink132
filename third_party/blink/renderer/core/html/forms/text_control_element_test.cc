...
protected:
    ...
    {
        return *dummy_page_holder_;
    }
    Document& GetDocument() const
    {
        return *document_;
    }
    TextControlElement& TextControl() const
    {
        return *text_control_;
    }
    HTMLInputElement& Input() const
    {
        return *input_;
    }
...
{
    dummy_page_holder_ = std::make_unique<DummyPageHolder>(gfx::Size(800, 600), nullptr);

    document_ = &dummy_page_holder_->GetDocument();
    document_->documentElement()->setInnerHTML("<body><textarea id=textarea></textarea><input id=input /></body>");
    UpdateAllLifecyclePhases();
    text_control_ = ToTextControl(document_->getElementById(AtomicString("textarea")));
    text_control_->Focus();
    input_ = To<HTMLInputElement>(document_->getElementById(AtomicString("input")));
}
...
TEST_F(TextControlElementTest, IndexForPosition)
{
    ...
    EXPECT_EQ(5u, TextControlElement::IndexForPosition(inner_editor, Position(inner_editor, PositionAnchorType::kAfterAnchor)));
}
...
TEST_F(TextControlElementTest, ReadOnlyAttributeChangeEditability)
{
    ...
    UpdateAllLifecyclePhases();
    EXPECT_EQ(EUserModify::kReadOnly, Input().InnerEditorElement()->GetComputedStyle()->UsedUserModify());

    Input().removeAttribute(html_names::kReadonlyAttr);
    UpdateAllLifecyclePhases();
    EXPECT_EQ(EUserModify::kReadWritePlaintextOnly, Input().InnerEditorElement()->GetComputedStyle()->UsedUserModify());
}
...
{
    GetDocument().body()->setInnerHTML("<input id='p0' placeholder='first line &#13;&#10;second line'>"
...
{
    GetDocument().body()->setInnerHTML("<textarea id='p0' placeholder='first line &#13;&#10;second line'>"
