...
class TestDateTimeChooserClient final : public GarbageCollected<TestDateTimeChooserClient>, public DateTimeChooserClient {
public:
    explicit TestDateTimeChooserClient(Element* element)
        : element_(element)
    {
    }
...
TEST_F(ExternalDateTimeChooserTest, EndChooserShouldNotCrash)
{
    ...
    auto* external_date_time_chooser = MakeGarbageCollected<ExternalDateTimeChooser>(client);
...
// receiving side.
TEST_F(ExternalDateTimeChooserTest, OpenDateTimeChooserShouldNotCrashWhenLabelAndValueIsTheSame)
{
    ScopedInputMultipleFieldsUIForTest input_multiple_fields_ui(false);
    ...
    auto* input = To<HTMLInputElement>(GetDocument().getElementById(AtomicString("test")));
    ASSERT_TRUE(input);
    ...
    auto* client = MakeGarbageCollected<TestDateTimeChooserClient>(GetDocument().documentElement());
    auto* external_date_time_chooser = MakeGarbageCollected<ExternalDateTimeChooser>(client);
    client->SetDateTimeChooser(external_date_time_chooser);
    external_date_time_chooser->OpenDateTimeChooser(GetDocument().GetFrame(), params);
