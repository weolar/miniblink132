...
FormData* Deserialize(ExecutionContext& context, const Vector<String>& strings)
{
    wtf_size_t i = 0;
...
TEST(FormDataTest, AppendToControlState)
{
...

    {
        auto* fd = MakeGarbageCollected<FormData>();
        fd->append("n1", "string");
        fd->AppendFromElement("n1", MakeGarbageCollected<File>(&context.GetExecutionContext(), "/etc/hosts"));
        FormControlState state;
        fd->AppendToControlState(state);
...
TEST(FormDataTest, CreateFromControlState)
{
    test::TaskEnvironment task_environment;
    ScopedNullExecutionContext context;
    EXPECT_EQ(nullptr, Deserialize(context.GetExecutionContext(), { "1", "not-a-number" })) << "Should fail on size parsing";

    auto* fd0 = Deserialize(context.GetExecutionContext(), { "1", "0" });
    ASSERT_NE(nullptr, fd0);
    EXPECT_EQ(0u, fd0->size());

    EXPECT_EQ(nullptr, Deserialize(context.GetExecutionContext(), { "1", "1" })) << "Missing name value";

    EXPECT_EQ(nullptr, Deserialize(context.GetExecutionContext(), { "2", "1", "n0" })) << "Missing entry type";

    EXPECT_EQ(nullptr, Deserialize(context.GetExecutionContext(), { "3", "1", "n0", "DOMString" })) << "Unknown entry type";

    EXPECT_EQ(nullptr, Deserialize(context.GetExecutionContext(), { "3", "1", "n0", "USVString" })) << "Missing USVString value";

    EXPECT_EQ(nullptr, Deserialize(context.GetExecutionContext(), { "3", "1", "n1", "File" })) << "Missing File value 1";

    EXPECT_EQ(nullptr, Deserialize(context.GetExecutionContext(), { "4", "1", "n1", "File", "/etc/hosts" })) << "Missing File value 2";

    EXPECT_EQ(nullptr, Deserialize(context.GetExecutionContext(), { "5", "1", "n1", "File", "/etc/password", "pasword" })) << "Missing File value 3";

    auto* fd = Deserialize(context.GetExecutionContext(), { "9", "2", "n1", "USVString", "string-value", "n2", "File", "/etc/password", "pasword", "" });
...
TEST(FormDataTest, FilenameWithLoneSurrogates)
{
    ...
    auto* file = MakeGarbageCollected<File>(filename, std::nullopt, BlobDataHandle::Create());
