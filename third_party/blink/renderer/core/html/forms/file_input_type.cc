...
VectorType CreateFilesFrom(const FormControlState& state, ItemType (*factory)(const FormControlState&, wtf_size_t&))
{
    VectorType files;
...
template <typename ItemType, typename VectorType>
VectorType CreateFilesFrom(
    const FormControlState& state, ExecutionContext* execution_context, ItemType (*factory)(ExecutionContext*, const FormControlState&, wtf_size_t&))
{
    VectorType files;
    files.ReserveInitialCapacity(state.ValueSize() / 3);
    for (wtf_size_t i = 0; i < state.ValueSize();) {
...
Vector<String> FileInputType::FilesFromFormControlState(const FormControlState& state)
{
    return CreateFilesFrom<String, Vector<String>>(state, &File::PathFromControlState);
}

FormControlState FileInputType::SaveFormControlState() const
{
    if (file_list_->IsEmpty() || GetElement().GetDocument().GetFormController().DropReferencedFilePaths())
...
void FileInputType::RestoreFormControlState(const FormControlState& state)
{
    ...
    HeapVector<Member<File>> file_vector = CreateFilesFrom<File*, HeapVector<Member<File>>>(state, execution_context, &File::CreateFromControlState);
...
void FileInputType::AppendToFormData(FormData& form_data) const
{
    ...
    if (num_files == 0) {
        form_data.AppendFromElement(GetElement().GetName(), MakeGarbageCollected<File>(context, ""));
        return;
    }
...
{
    return GetLocale().QueryString(GetElement().Multiple() ? IDS_FORM_VALIDATION_VALUE_MISSING_MULTIPLE_FILE : IDS_FORM_VALIDATION_VALUE_MISSING_FILE);
}
...
void FileInputType::HandleDOMActivateEvent(Event& event)
{
    ...
    if (!LocalFrame::HasTransientUserActivation(document.GetFrame())) {
        String message = "File chooser dialog can only be shown with a user activation.";
...
void FileInputType::OpenPopupView()
{
    ...
    probe::FileChooserOpened(document.GetFrame(), &input, input.Multiple(), &intercepted);
    if (intercepted) {
        return;
    }
    ...
    if (ChromeClient* chrome_client = GetChromeClient()) {
        ...
        bool is_directory = input.FastHasAttribute(html_names::kWebkitdirectoryAttr);
        if (is_directory)
        ...
        params.use_media_capture = RuntimeEnabledFeatures::MediaCaptureEnabled() && input.FastHasAttribute(html_names::kCaptureAttr);
        params.requestor = document.Url();

        UseCounter::Count(document,
            GetElement().GetExecutionContext()->IsSecureContext() ? WebFeature::kInputTypeFileSecureOriginOpenChooser
...
void FileInputType::SetValue(const String&, bool value_changed, TextFieldEventBehavior, TextControlSetValueSelection)
{
    if (!value_changed)
        return;
...
FileList* FileInputType::CreateFileList(ExecutionContext& context, const FileChooserFileInfoList& files, const base::FilePath& base_dir)
{
    auto* file_list(MakeGarbageCollected<FileList>());
    ...
    if (size && !base_dir.empty()) {
        ...
        for (const auto& file : files) {
            ...
            if (display_name.empty()) {
                display_name = FilePathToString(file->get_native_file()->file_path.BaseName());
            }
            ...
            if (relative_path.empty()) {
                DCHECK(string_path.StartsWithIgnoringASCIICase(FilePathToString(base_dir)))
                    << "A path in a FileChooserFileInfo " << string_path << " should start with " << FilePathToString(base_dir);
                relative_path = string_path.Substring(root_length).Replace('\\', '/');
            }
            file_list->Append(File::CreateWithRelativePath(&context, string_path, display_name, relative_path));
        }
        return file_list;
    }
    ...
    for (const auto& file : files) {
        if (file->is_native_file()) {
            file_list->Append(
                File::CreateForUserProvidedFile(&context, FilePathToString(file->get_native_file()->file_path), file->get_native_file()->display_name));
        } else {
...
void FileInputType::CreateShadowSubtree()
{
    ...
    button->setAttribute(html_names::kValueAttr,
        AtomicString(GetLocale().QueryString(GetElement().Multiple() ? IDS_FORM_MULTIPLE_FILES_BUTTON_LABEL : IDS_FORM_FILE_BUTTON_LABEL)));
    button->SetShadowPseudoId(shadow_element_names::kPseudoFileUploadButton);
    button->setAttribute(html_names::kIdAttr, shadow_element_names::kIdFileUploadButton);
...
{
    Element* element = GetElement().EnsureShadowSubtree()->getElementById(shadow_element_names::kIdFileUploadButton);
...
{
    DCHECK(RuntimeEnabledFeatures::CreateInputShadowTreeDuringLayoutEnabled() || IsShadowHost(GetElement()));
    if (Element* button = UploadButton()) {
        button->SetBooleanAttribute(html_names::kDisabledAttr, GetElement().IsDisabledFormControl());
    }
}
...
void FileInputType::FilesChosen(FileChooserFileInfoList files, const base::FilePath& base_dir)
{
    for (wtf_size_t i = 0; i < files.size();) {
        ...
        if (files[i]->is_native_file() && FilePathToString(files[i]->get_native_file()->file_path).empty()) {
            files.EraseAt(i);
            // Do not increment |i|.
            continue;
        }
    ...
    if (!will_be_destroyed_) {
        SetFilesAndDispatchEvents(CreateFileList(*GetElement().GetExecutionContext(), files, base_dir));
    }
...
void FileInputType::WillOpenPopup()
{
    ...
    if (HasConnectedFileChooser()) {
        UseCounter::Count(GetElement().GetDocument(), WebFeature::kPopupOpenWhileFileChooserOpened);
    }
}
...
String FileInputType::FileStatusText() const
{
    ...
    return locale.QueryString(IDS_FORM_FILE_MULTIPLE_UPLOAD, locale.ConvertToLocalizedNumber(String::Number(file_list_->length())));
}
