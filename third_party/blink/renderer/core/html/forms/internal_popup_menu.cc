...
const String SerializeComputedStyleForProperty(const ComputedStyle& style, CSSPropertyID id)
{
    const CSSProperty& property = CSSProperty::Get(id);
    const CSSValue* value = property.CSSValueFromComputedStyle(style, nullptr, false, CSSValuePhase::kResolvedValue);
    return String::Format("%s : %s;\n", property.GetPropertyName(), value->CssText().Utf8().c_str());
}
...
const String SerializeColorScheme(const ComputedStyle& style)
{
    ...
    for (const AtomicString& ident : style.ColorScheme()) {
        if (ident == AtomicString("only") || ident == AtomicString("light") || ident == AtomicString("dark")) {
            if (first_added)
                buffer.Append(" ");
            else
                first_added = true;
...
const ComputedStyle* StyleForHoveredScrollbarPart(HTMLSelectElement& element, const ComputedStyle* style, Scrollbar* scrollbar, PseudoId target_id)
{
    ScrollbarPart part = ScrollbarPartFromPseudoId(target_id);
    if (part == kNoPart)
        return nullptr;
    scrollbar->SetHoveredPart(part);
    const ComputedStyle* part_style = element.UncachedStyleForPseudoElement(StyleRequest(target_id, To<CustomScrollbar>(scrollbar), part, style));
    return part_style;
}

} // anonymous namespace

class PopupMenuCSSFontSelector : public CSSFontSelector, private FontSelectorClient {
public:
    ...
    const FontData* GetFontData(const FontDescription&, const FontFamily&) override;
...
PopupMenuCSSFontSelector::PopupMenuCSSFontSelector(Document& document, CSSFontSelector* owner_font_selector)
    : CSSFontSelector(document)
    , owner_font_selector_(owner_font_selector)
{
...
const FontData* PopupMenuCSSFontSelector::GetFontData(const FontDescription& description, const FontFamily& font_family)
{
    return owner_font_selector_->GetFontData(description, font_family);
}

void PopupMenuCSSFontSelector::FontsNeedUpdate(FontSelector* font_selector, FontInvalidationReason reason)
{
...
public:
    ItemIterationContext(const ComputedStyle& style, SegmentedBuffer& buffer)
        : base_style_(style)
        , background_color_(style.VisitedDependentColor(GetCSSPropertyBackgroundColor()))
        , buffer_(buffer)
    {
    }
    ...
    void SerializeBaseStyle()
    {
        ...
        if (!BaseStyle().ColorSchemeForced()) {
            AddProperty("backgroundColor", background_color_.SerializeAsCSSColor(), buffer_);
            AddProperty("color", BaseStyle().VisitedDependentColor(GetCSSPropertyColor()).SerializeAsCSSColor(), buffer_);
        }
        AddProperty("textTransform", TextTransformToString(BaseStyle().TextTransform()), buffer_);
        AddProperty("textAlign", TextAlignToString(BaseStyle().GetTextAlign(false)), buffer_);
        AddProperty("fontSize", BaseFont().ComputedPixelSize(), buffer_);
        AddProperty("fontStyle", String(FontStyleToString(BaseFont().Style())), buffer_);
        AddProperty("fontVariant", BaseFont().VariantCaps() == FontDescription::kSmallCaps ? String("small-caps") : String(), buffer_);

        AddProperty("fontFamily", ComputedStyleUtils::ValueForFontFamily(BaseFont().Family())->CssText(), buffer_);
    ...
    {
        return is_in_group_ ? group_style_->VisitedDependentColor(GetCSSPropertyBackgroundColor()) : background_color_;
    }
    ...
    {
        return is_in_group_ ? group_style_->GetFontDescription() : base_style_.GetFontDescription();
    }
...
InternalPopupMenu::InternalPopupMenu(ChromeClient* chrome_client, HTMLSelectElement& owner_element)
...
void InternalPopupMenu::WriteDocument(SegmentedBuffer& data)
{
    ...
    gfx::Rect anchor_rect_in_screen = chrome_client_->LocalRootToScreenDIPs(owner_element.VisibleBoundsInLocalRoot(), owner_element.GetDocument().View());

    float scale_factor = chrome_client_->WindowToViewportScalar(owner_element.GetDocument().GetFrame(), 1.f);
    PagePopupClient::AddString("<!DOCTYPE html><head><meta charset='UTF-8'>", data);
    ...
    PagePopupClient::AddString(owner_style.DarkColorScheme() ? "dark" : "light", data);
    PagePopupClient::AddString("'><style>\n", data);

    LayoutObject* owner_layout = owner_element.GetLayoutObject();

    std::pair<PseudoId, const String> targets[] = { { kPseudoIdScrollbar, "select::-webkit-scrollbar" },
        { kPseudoIdScrollbarThumb, "select::-webkit-scrollbar-thumb" }, { kPseudoIdScrollbarTrack, "select::-webkit-scrollbar-track" },
        { kPseudoIdScrollbarTrackPiece, "select::-webkit-scrollbar-track-piece" }, { kPseudoIdScrollbarCorner, "select::-webkit-scrollbar-corner" } };
    ...
    if (box && box->GetScrollableArea()) {
        if (ScrollableArea* scrollable = box->GetScrollableArea()) {
            temp_scrollbar = MakeGarbageCollected<CustomScrollbar>(scrollable, kVerticalScrollbar, box);
        }
    }
    for (auto target : targets) {
        if (const ComputedStyle* style = owner_layout->GetCachedPseudoElementStyle(target.first)) {
        ...
        if (temp_scrollbar) {
            const ComputedStyle* part_style = StyleForHoveredScrollbarPart(owner_element, owner_element.GetComputedStyle(), temp_scrollbar, target.first);
            if (part_style) {
                AppendOwnerElementPseudoStyles(target.second + ":hover", data, *part_style);
            }
        }
    }
    if (temp_scrollbar)
    ...
    if (taller_options_) {
        ...
        if (RuntimeEnabledFeatures::SelectOptionAccessibilityTargetSizeEnabled()) {
            PagePopupClient::AddString(String::Format("option {"
                                                      "display: block;"
    ...
    AddProperty("paddingStart", is_rtl ? owner_element.ClientPaddingRight().ToDouble() : owner_element.ClientPaddingLeft().ToDouble(), data);
    PagePopupClient::AddString("};\n", data);
...
void InternalPopupMenu::AddElementStyle(ItemIterationContext& context, HTMLElement& element)
{
    ...
    if (base_style.Direction() != style->Direction()) {
        AddProperty("direction", String(style->Direction() == TextDirection::kRtl ? "rtl" : "ltr"), data);
    }
    ...
    if (!base_style.ColorSchemeForced()) {
        ...
        if (base_style.VisitedDependentColor(GetCSSPropertyColor()) != foreground_color) {
        ...
        if (background_color != Color::kTransparent && (context.BackgroundColor() != background_color)) {
            AddProperty("backgroundColor", background_color.SerializeAsCSSColor(), data);
            color_applied = true;
        }
        if (color_applied)
    ...
    if (base_font.Family() != font_description.Family()) {
        AddProperty("fontFamily", ComputedStyleUtils::ValueForFontFamily(font_description.Family())->CssText(), data);
    }
    if (base_font.Style() != font_description.Style()) {
        AddProperty("fontStyle", String(FontStyleToString(font_description.Style())), data);
    }

    if (base_font.VariantCaps() != font_description.VariantCaps() && font_description.VariantCaps() == FontDescription::kSmallCaps)
        AddProperty("fontVariant", String("small-caps"), data);

    if (base_style.TextTransform() != style->TextTransform()) {
        AddProperty("textTransform", TextTransformToString(style->TextTransform()), data);
    }
    if (base_style.GetTextAlign(false) != style->GetTextAlign(false)) {
        AddProperty("textAlign", TextAlignToString(style->GetTextAlign(false)), data);
    }

    PagePopupClient::AddString("},\n", data);
}

void InternalPopupMenu::AddOption(ItemIterationContext& context, HTMLOptionElement& element)
{
    SegmentedBuffer& data = context.buffer_;
    ...
    const AtomicString& aria_label = element.FastGetAttribute(html_names::kAriaLabelAttr);
    if (!aria_label.empty())
...
void InternalPopupMenu::AddOptGroup(ItemIterationContext& context, HTMLOptGroupElement& element)
{
    SegmentedBuffer& data = context.buffer_;
    PagePopupClient::AddString("{\n", data);
    ...
    AddProperty("title", element.title(), data);
    AddProperty("ariaLabel", element.FastGetAttribute(html_names::kAriaLabelAttr), data);
...
void InternalPopupMenu::AddSeparator(ItemIterationContext& context, HTMLHRElement& element)
{
    SegmentedBuffer& data = context.buffer_;
    PagePopupClient::AddString("{\n", data);
...
void InternalPopupMenu::AppendOwnerElementPseudoStyles(const String& target, SegmentedBuffer& data, const ComputedStyle& style)
{
    PagePopupClient::AddString(target + "{ \n", data);

    const CSSPropertyID serialize_targets[]
        = { CSSPropertyID::kDisplay, CSSPropertyID::kBackgroundColor, CSSPropertyID::kWidth, CSSPropertyID::kBorderBottom, CSSPropertyID::kBorderLeft,
              CSSPropertyID::kBorderRight, CSSPropertyID::kBorderTop, CSSPropertyID::kBorderRadius, CSSPropertyID::kBackgroundClip, CSSPropertyID::kBoxShadow };

    for (CSSPropertyID id : serialize_targets) {
        PagePopupClient::AddString(SerializeComputedStyleForProperty(style, id), data);
    }

    PagePopupClient::AddString("}\n", data);
}

CSSFontSelector* InternalPopupMenu::CreateCSSFontSelector(Document& popup_document)
{
    Document& owner_document = OwnerElement().GetDocument();
    return MakeGarbageCollected<PopupMenuCSSFontSelector>(popup_document, owner_document.GetStyleEngine().GetFontSelector());
}

void InternalPopupMenu::SetValueAndClosePopup(int num_value, const String& string_value)
{
    DCHECK(popup_);
    ...
    if (owner_element_) {
        ...
        if (LocalFrame* frame = owner->GetDocument().GetFrame()) {
            frame->GetEventHandler().HandleTargetedMouseEvent(owner, event, event_type_names::kMouseup, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
            frame->GetEventHandler().HandleTargetedMouseEvent(owner, event, event_type_names::kClick, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
        }
    }
}
...
{
    DCHECK(!popup_);
    taller_options_ = type == PopupMenu::kTouch || RuntimeEnabledFeatures::ForceTallerSelectPopupEnabled()
...
void InternalPopupMenu::Update(bool force_update)
{
    ...
    if (!gfx::Rect(gfx::Point(), OwnerElement().GetDocument().View()->Size()).Intersects(OwnerElement().PixelSnappedBoundingBox())) {
        Hide();
        return;
    }
    ...
    context.FinishGroupIfNecessary();
    PagePopupClient::AddString("],\n", data);
    gfx::Rect anchor_rect_in_screen = chrome_client_->LocalRootToScreenDIPs(owner_element_->VisibleBoundsInLocalRoot(), OwnerElement().GetDocument().View());
    AddProperty("anchorRectInScreen", anchor_rect_in_screen, data);
    PagePopupClient::AddString("}\n", data);
    Vector<char> flatten_data = std::move(data).CopyAs<Vector<char>>();
    popup_->PostMessageToPopup(String::FromUTF8(base::span<const uint8_t>((const uint8_t*)flatten_data.data(), flatten_data.size())));
}
...
void InternalPopupMenu::SetMenuListOptionsBoundsInAXTree(WTF::Vector<gfx::Rect>& options_bounds, gfx::Point popup_origin)
{
    WebFrameWidgetImpl* widget = WebLocalFrameImpl::FromFrame(owner_element_->GetDocument().GetFrame())->LocalRootFrameWidget();
    if (!widget) {
        return;
    }
    ...
    const float page_zoom_factor = owner_element_->GetDocument().GetFrame()->LayoutZoomFactor();
    popup_origin.Offset(window->scrollX() * page_zoom_factor, window->scrollY() * page_zoom_factor);
