...
public:
    scoped_refptr<const SecurityOrigin> GetOriginForPermissionsPolicy(HTMLIFrameElement* element)
    {
    ...
    void SetUp() final
    {
        ...
        window_->GetSecurityContext().SetSecurityOriginForTesting(SecurityOrigin::Create(document_url));
        frame_element_ = MakeGarbageCollected<HTMLIFrameElement>(*window_->document());
    }
...
{
    frame_element_->setAttribute(html_names::kSrcAttr, AtomicString("about:blank"));
    scoped_refptr<const SecurityOrigin> effective_origin = GetOriginForPermissionsPolicy(frame_element_);
    EXPECT_TRUE(effective_origin->IsSameOriginWith(window_->GetSecurityOrigin()));

    frame_element_->setAttribute(html_names::kSrcAttr, AtomicString("data:text/html;base64,PHRpdGxlPkFCQzwvdGl0bGU+"));
    effective_origin = GetOriginForPermissionsPolicy(frame_element_);
    EXPECT_FALSE(effective_origin->IsSameOriginWith(window_->GetSecurityOrigin()));
    EXPECT_TRUE(effective_origin->IsOpaque());

    frame_element_->setAttribute(html_names::kSrcAttr, AtomicString("http://example.net/"));
    effective_origin = GetOriginForPermissionsPolicy(frame_element_);
    EXPECT_FALSE(effective_origin->IsSameOriginWith(window_->GetSecurityOrigin()));
...
TEST_F(HTMLIFrameElementTest, SandboxFramesUseCorrectOrigin)
{
    ...
    frame_element_->setAttribute(html_names::kSrcAttr, AtomicString("http://example.com/"));
    scoped_refptr<const SecurityOrigin> effective_origin = GetOriginForPermissionsPolicy(frame_element_);
    EXPECT_FALSE(effective_origin->IsSameOriginWith(window_->GetSecurityOrigin()));
    EXPECT_TRUE(effective_origin->IsOpaque());
...
{
    frame_element_->setAttribute(html_names::kSandboxAttr, AtomicString("allow-same-origin"));
    frame_element_->setAttribute(html_names::kSrcAttr, AtomicString("http://example.com/"));
    scoped_refptr<const SecurityOrigin> effective_origin = GetOriginForPermissionsPolicy(frame_element_);
    EXPECT_TRUE(effective_origin->IsSameOriginWith(window_->GetSecurityOrigin()));
    EXPECT_FALSE(effective_origin->IsOpaque());
...
{
    frame_element_->setAttribute(html_names::kSrcdocAttr, AtomicString("<title>title</title>"));
    scoped_refptr<const SecurityOrigin> effective_origin = GetOriginForPermissionsPolicy(frame_element_);
    EXPECT_TRUE(effective_origin->IsSameOriginWith(window_->GetSecurityOrigin()));
}
...
TEST_F(HTMLIFrameElementTest, RelativeURLsUseCorrectOrigin)
{
    ...
    frame_element_->setAttribute(html_names::kSrcAttr, AtomicString("index2.html"));
    scoped_refptr<const SecurityOrigin> effective_origin = GetOriginForPermissionsPolicy(frame_element_);
    EXPECT_TRUE(effective_origin->IsSameOriginWith(window_->GetSecurityOrigin()));

    // Scheme-relative URLs should not resolve to the same domain as the parent.
    frame_element_->setAttribute(html_names::kSrcAttr, AtomicString("//example.net/index2.html"));
    effective_origin = GetOriginForPermissionsPolicy(frame_element_);
    EXPECT_FALSE(effective_origin->IsSameOriginWith(window_->GetSecurityOrigin()));
}
...
TEST_F(HTMLIFrameElementTest, DefaultContainerPolicy)
{
    ...
    const ParsedPermissionsPolicy& container_policy = frame_element_->GetFramePolicy().container_policy;
...
{
    frame_element_->setAttribute(html_names::kSrcAttr, AtomicString("http://example.net/"));
    frame_element_->setAttribute(html_names::kAllowAttr, AtomicString("fullscreen"));
    frame_element_->UpdateContainerPolicyForTests();

    const ParsedPermissionsPolicy& container_policy1 = frame_element_->GetFramePolicy().container_policy;

    EXPECT_EQ(1UL, container_policy1.size());
    EXPECT_EQ(mojom::blink::PermissionsPolicyFeature::kFullscreen, container_policy1[0].feature);
    EXPECT_FALSE(container_policy1[0].matches_all_origins);
    EXPECT_EQ(1UL, container_policy1[0].allowed_origins.size());
    EXPECT_EQ("http://example.net", container_policy1[0].allowed_origins.begin()->Serialize());

    frame_element_->setAttribute(html_names::kAllowAttr, AtomicString("payment; fullscreen"));
    frame_element_->UpdateContainerPolicyForTests();

    const ParsedPermissionsPolicy& container_policy2 = frame_element_->GetFramePolicy().container_policy;
    EXPECT_EQ(2UL, container_policy2.size());
    EXPECT_TRUE(container_policy2[0].feature == mojom::blink::PermissionsPolicyFeature::kFullscreen
        || container_policy2[1].feature == mojom::blink::PermissionsPolicyFeature::kFullscreen);
    EXPECT_TRUE(container_policy2[0].feature == mojom::blink::PermissionsPolicyFeature::kPayment
        || container_policy2[1].feature == mojom::blink::PermissionsPolicyFeature::kPayment);
    EXPECT_EQ(1UL, container_policy2[0].allowed_origins.size());
    EXPECT_EQ("http://example.net", container_policy2[0].allowed_origins.begin()->Serialize());
    EXPECT_FALSE(container_policy2[1].matches_all_origins);
    EXPECT_EQ(1UL, container_policy2[1].allowed_origins.size());
    EXPECT_EQ("http://example.net", container_policy2[1].allowed_origins.begin()->Serialize());
}
...
{
    ParsedPermissionsPolicy container_policy = frame_element_->ConstructContainerPolicy();
    EXPECT_EQ(0UL, container_policy.size());
}
...
{
    frame_element_->setAttribute(html_names::kAllowAttr, AtomicString("payment; usb"));
    ParsedPermissionsPolicy container_policy = frame_element_->ConstructContainerPolicy();
    EXPECT_EQ(2UL, container_policy.size());
    EXPECT_EQ(mojom::blink::PermissionsPolicyFeature::kPayment, container_policy[0].feature);
    EXPECT_FALSE(container_policy[0].matches_all_origins);
    EXPECT_EQ(1UL, container_policy[0].allowed_origins.size());
    EXPECT_TRUE(container_policy[0].allowed_origins.begin()->DoesMatchOrigin(GetOriginForPermissionsPolicy(frame_element_)->ToUrlOrigin()));
    EXPECT_EQ(mojom::blink::PermissionsPolicyFeature::kUsb, container_policy[1].feature);
    EXPECT_EQ(1UL, container_policy[1].allowed_origins.size());
    EXPECT_TRUE(container_policy[1].allowed_origins.begin()->DoesMatchOrigin(GetOriginForPermissionsPolicy(frame_element_)->ToUrlOrigin()));
}
...
TEST_F(HTMLIFrameElementTest, ConstructContainerPolicyWithAllowFullscreen)
{
    ...
    EXPECT_EQ(mojom::blink::PermissionsPolicyFeature::kFullscreen, container_policy[0].feature);
...
TEST_F(HTMLIFrameElementTest, ConstructContainerPolicyWithAllowPaymentRequest)
{
    ...
    frame_element_->SetBooleanAttribute(html_names::kAllowpaymentrequestAttr, true);

    ParsedPermissionsPolicy container_policy = frame_element_->ConstructContainerPolicy();
    EXPECT_EQ(2UL, container_policy.size());
    EXPECT_EQ(mojom::blink::PermissionsPolicyFeature::kUsb, container_policy[0].feature);
    EXPECT_FALSE(container_policy[0].matches_all_origins);
    EXPECT_EQ(1UL, container_policy[0].allowed_origins.size());
    EXPECT_TRUE(container_policy[0].allowed_origins.begin()->DoesMatchOrigin(GetOriginForPermissionsPolicy(frame_element_)->ToUrlOrigin()));
    EXPECT_EQ(mojom::blink::PermissionsPolicyFeature::kPayment, container_policy[1].feature);
}
...
TEST_F(HTMLIFrameElementTest, ConstructContainerPolicyWithAllowAttributes)
{
    ...
    EXPECT_EQ(1UL, container_policy[1].allowed_origins.size());
    EXPECT_TRUE(container_policy[1].allowed_origins.begin()->DoesMatchOrigin(GetOriginForPermissionsPolicy(frame_element_)->ToUrlOrigin()));
    EXPECT_EQ(mojom::blink::PermissionsPolicyFeature::kFullscreen, container_policy[2].feature);
}
...
TEST_F(HTMLIFrameElementSimTest, PolicyAttributeParsingError)
{
    ...
    for (const auto& message : ConsoleMessages()) {
        EXPECT_TRUE(message.StartsWith("Unrecognized document policy feature name"));
    }
}
...
TEST_F(HTMLIFrameElementSimTest, AllowAttributeParsingError)
{
    ...
    EXPECT_EQ(ConsoleMessages().size(), 1u) << "Allow attribute parsing should only generate console message once, "
                                               "even though there might be multiple call to "
                                               "PermissionsPolicyParser::ParseAttribute.";
    EXPECT_TRUE(ConsoleMessages().front().StartsWith("Unrecognized feature")) << "Expect permissions policy parser raising error for unrecognized "
                                                                                 "feature but got: "
...
TEST_F(HTMLIFrameElementSimTest, Adauctionheaders_InsecureContext_NotAllowed)
{
    ...
    EXPECT_TRUE(ConsoleMessages().front().StartsWith("adAuctionHeaders: Protected Audience APIs "
...
TEST_F(HTMLIFrameElementSimTest, Sharedstoragewritable_InsecureContext_NotAllowed)
{
    WebRuntimeFeaturesBase::EnableSharedStorageAPI(true);
    WebRuntimeFeaturesBase::EnableSharedStorageAPIM118(true);
    SimRequest main_resource("http://example.com", "text/html");
    ...
    EXPECT_EQ(ConsoleMessages().size(), 1u);
    EXPECT_TRUE(ConsoleMessages().front().StartsWith("sharedStorageWritable: sharedStorage operations are only available in "
                                                     "secure contexts."))
