...
{
    for (Node* node = FlatTreeTraversal::FirstChild(anchor_element); node; node = FlatTreeTraversal::Next(*node, &anchor_element)) {
...
bool IsSameHost(const HTMLAnchorElementBase& anchor_element, const KURL& anchor_href)
{
...
bool IsStringIncrementedByOne(const String& source, const String& target)
{
    ...
    unsigned int left = 0;
    while (left < source.length() && left < target.length() && source[left] == target[left]) {
        left++;
    }

    // There is no difference, or the difference is not a digit.
    if (left == source.length() || left == target.length() || !IsASCIIDigit(source[left]) || !IsASCIIDigit(target[left])) {
        return false;
    }
    ...
    int source_number = CharactersToInt(StringView(source, left, source_right - left), WTF::NumberParsingOptions(), /*ok=*/nullptr);
    int target_number = CharactersToInt(StringView(target, left, target_right - left), WTF::NumberParsingOptions(), /*ok=*/nullptr);

    // The second number should increment by one and the rest of the strings
    // should be the same.
    return source_number + 1 == target_number && StringView(source, source_right) == StringView(target, target_right);
}

// Extract source and target link url, and return IsStringIncrementedByOne().
bool IsUrlIncrementedByOne(const HTMLAnchorElementBase& anchor_element, const KURL& anchor_href)
{
...
{
    Vector<PhysicalRect> rects = layout_object.OutlineRects(nullptr, PhysicalOffset(), OutlineType::kIncludeBlockInkOverflow);
...
{
    for (auto* text = DynamicTo<Text>(anchor_element.previousSibling()); text; text = DynamicTo<Text>(text->previousSibling())) {
        if (!text->ContainsOnlyWhitespaceOrEmpty()) {
            return true;
        }
    }

    for (auto* text = DynamicTo<Text>(anchor_element.nextSibling()); text; text = DynamicTo<Text>(text->nextSibling())) {
        if (!text->ContainsOnlyWhitespaceOrEmpty()) {
            return true;
        }
    }
...
mojom::blink::AnchorElementMetricsPtr CreateAnchorElementMetrics(const HTMLAnchorElementBase& anchor_element)
{
    ...
    gfx::Rect bounding_box = is_in_iframe ? layout_object->AbsoluteBoundingBoxRect() : AbsoluteElementBoundingBoxRect(*layout_object);
    if (bounding_box.IsEmpty()) {
        return nullptr;
    }

    mojom::blink::AnchorElementMetricsPtr metrics = mojom::blink::AnchorElementMetrics::New();
    ...
    metrics->is_url_incremented_by_one = IsUrlIncrementedByOne(anchor_element, anchor_href);
    ...
    metrics->font_weight = static_cast<uint32_t>(computed_style.GetFontWeight() + 0.5f);
    ...
    float ratio_area = std::min(1.0f, target.height() / base_height) * std::min(1.0f, target.width() / base_width);
    DCHECK_GE(1.0, ratio_area);
    metrics->ratio_area = ratio_area;

    float ratio_distance_top_to_visible_top = target.y() / base_height;
    metrics->ratio_distance_top_to_visible_top = ratio_distance_top_to_visible_top;

    float ratio_distance_root_top = (target.y() + root_frame_view->LayoutViewport()->ScrollOffsetInt().y()) / base_height;
