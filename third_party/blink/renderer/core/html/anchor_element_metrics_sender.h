...
class CORE_EXPORT AnchorElementMetricsSender final : public GarbageCollected<AnchorElementMetricsSender>,
...
public:
    ...
    AnchorElementMetricsSender& operator=(const AnchorElementMetricsSender&) = delete;
    ...
    // is an HTTP(S) link.
    void MaybeReportClickedMetricsOnClick(const HTMLAnchorElementBase& anchor_element);

    // Report the on-hover event and anchor element pointer data to the browser
    // process.
    void MaybeReportAnchorElementPointerDataOnHoverTimerFired(AnchorId anchor_id, mojom::blink::AnchorElementPointerDataPtr mouse_data);
    ...
    void MaybeReportAnchorElementPointerEvent(HTMLAnchorElementBase& element, const PointerEvent& pointer_event);
...
private:
    ...
    void ViewportIntersectionUpdate(
        const HeapVector<Member<const HTMLAnchorElementBase>>& entered_viewport, const HeapVector<Member<const HTMLAnchorElementBase>>& left_viewport) override;
    void AnchorPositionsUpdated(HeapVector<Member<AnchorPositionUpdate>>& position_updates) override;
    ...
    WTF::Vector<mojom::blink::AnchorElementEnteredViewportPtr> entered_viewport_messages_;
    ...
    WTF::HashMap<AnchorId, AnchorElementTimingStats> anchor_elements_timing_stats_;

    WTF::Vector<mojom::blink::AnchorElementLeftViewportPtr> left_viewport_messages_;

    WTF::Vector<mojom::blink::AnchorElementPositionUpdatePtr> position_update_messages_;
