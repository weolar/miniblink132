...
static bool StyleSheetTypeIsSupported(const String& type)
{
    ...
    return trimmed_type.empty() || MIMETypeRegistry::IsSupportedStyleSheetMIMEType(trimmed_type);
}
...
void LinkStyle::NotifyFinished(Resource* resource)
{
    ...
    if (resource->LoadFailedOrCanceled()) {
        AuditsIssue::ReportStylesheetLoadingRequestFailedIssue(&GetDocument(), resource->Url(), resource->LastResourceRequest().GetDevToolsId(),
            GetDocument().Url(), resource->Options().initiator_info.position.line_, resource->Options().initiator_info.position.column_,
            resource->GetResourceError().LocalizedDescription());
    }

    auto* cached_style_sheet = To<CSSStyleSheetResource>(resource);
    if ((!cached_style_sheet->ErrorOccurred() && !owner_->FastGetAttribute(html_names::kIntegrityAttr).empty()
            && !cached_style_sheet->IntegrityMetadata().empty())
        || resource->ForceIntegrityChecks()) {
        SubresourceIntegrityHelper::DoReport(*GetExecutionContext(), cached_style_sheet->IntegrityReportInfo());

        if (!cached_style_sheet->PassedIntegrityChecks()) {
            loading_ = false;
            RemovePendingSheet();
            NotifyLoadedSheetAndAllCriticalSubresources(Node::kErrorOccurredLoadingSubresource);
            return;
        }
    }

    auto* parser_context = MakeGarbageCollected<CSSParserContext>(GetDocument(), cached_style_sheet->GetResponse().ResponseUrl(),
        cached_style_sheet->GetResponse().IsCorsSameOrigin(),
        Referrer(cached_style_sheet->GetResponse().ResponseUrl(), cached_style_sheet->GetReferrerPolicy()), cached_style_sheet->Encoding());
    if (cached_style_sheet->GetResourceRequest().IsAdResource()) {
        parser_context->SetIsAdRelated();
    }

    if (StyleSheetContents* parsed_sheet = cached_style_sheet->CreateParsedStyleSheetFromCache(parser_context)) {
        if (sheet_)
            ClearSheet();
        sheet_ = MakeGarbageCollected<CSSStyleSheet>(parsed_sheet, *owner_);
        sheet_->SetMediaQueries(MediaQuerySet::Create(owner_->Media(), GetExecutionContext()));
    ...
    auto* style_sheet = MakeGarbageCollected<StyleSheetContents>(parser_context, cached_style_sheet->Url());
    ...
    if (style_sheet->IsCacheableForResource()) {
        const_cast<CSSStyleSheetResource*>(cached_style_sheet)->SaveParsedStyleSheet(style_sheet);
    }
    base::UmaHistogramMicrosecondsTimes("Blink.CSSStyleSheetResource.ParseTime", base::TimeTicks::Now() - parser_start_time);
    ClearResource();
}
...
void LinkStyle::NotifyLoadedSheetAndAllCriticalSubresources(Node::LoadedSheetErrorStatus error_status)
{
    if (fired_load_)
        return;
...
void LinkStyle::AddPendingSheet(PendingSheetType type)
{
    ...
    if (pending_sheet_type_ == PendingSheetType::kNonBlocking)
        return;
    GetDocument().GetStyleEngine().AddPendingBlockingSheet(*owner_, pending_sheet_type_);
}
...
void LinkStyle::SetDisabledState(bool disabled)
{
    ...
    if (StyleSheetIsLoading()) {
        ...
        if (owner_->RelAttribute().IsAlternate() && disabled_state_ == kEnabledViaScript)
            AddPendingSheet(PendingSheetType::kBlocking);
        ...
        // only 3 sheets. :)
        if (!owner_->RelAttribute().IsAlternate() && disabled_state_ == kEnabledViaScript && old_disabled_state == kDisabled)
            AddPendingSheet(PendingSheetType::kBlocking);
    ...
    if (sheet_) {
        ...
        ClearSheet();
        GetDocument().GetStyleEngine().SetNeedsActiveStyleUpdate(owner_->GetTreeScope());
        return;
    }
...
LinkStyle::LoadReturnValue LinkStyle::LoadStylesheetIfNeeded(const LinkLoadParameters& params, const WTF::TextEncoding& charset)
{
    if (GetDocument().StatePreservingAtomicMoveInProgress()) {
        return kNotNeeded;
    }

    if (disabled_state_ == kDisabled || !owner_->RelAttribute().IsStyleSheet() || !StyleSheetTypeIsSupported(params.type) || !ShouldLoadResource()
        || !params.href.IsValid())
        return kNotNeeded;
    ...
    if (!title.empty() && !owner_->IsAlternate() && disabled_state_ != kEnabledViaScript && owner_->IsInDocumentTree()) {
    ...
    if (!owner_->Media().empty() && frame) {
        MediaQuerySet* media = MediaQuerySet::Create(owner_->Media(), GetExecutionContext());
        MediaQueryEvaluator* evaluator = MakeGarbageCollected<MediaQueryEvaluator>(frame);
    ...
    auto type_and_behavior = ComputePendingSheetTypeAndRenderBlockingBehavior(*owner_, critical_style, owner_->IsCreatedByParser());
    ...
    FetchParameters::DeferOption defer_option = !critical_style ? FetchParameters::kLazyLoad : FetchParameters::kNoDefer;

    render_blocking_behavior_ = type_and_behavior.second;
    owner_->LoadStylesheet(params, charset, defer_option, this, render_blocking_behavior_);
...
void LinkStyle::Process(LinkLoadParameters::Reason reason)
{
    ...
    const LinkLoadParameters params(owner_->RelAttribute(), GetCrossOriginAttributeValue(owner_->FastGetAttribute(html_names::kCrossoriginAttr)),
        owner_->TypeValue().DeprecatedLower(), owner_->AsValue().DeprecatedLower(), owner_->Media().DeprecatedLower(), owner_->nonce(),
        owner_->IntegrityValue(), owner_->FetchPriorityHintValue().LowerASCII(), owner_->GetReferrerPolicy(),
        owner_->GetNonEmptyURLAttribute(html_names::kHrefAttr), owner_->FastGetAttribute(html_names::kImagesrcsetAttr),
        owner_->FastGetAttribute(html_names::kImagesizesAttr), owner_->FastGetAttribute(html_names::kBlockingAttr), reason);

    WTF::TextEncoding charset = GetCharset();

    if (owner_->RelAttribute().GetIconType() != mojom::blink::FaviconIconType::kInvalid && params.href.IsValid() && !params.href.IsEmpty()) {
        if (!owner_->ShouldLoadLink())
            return;
        ...
        if (!GetExecutionContext()->GetSecurityOrigin()->CanDisplay(params.href))
            return;
        if (!GetExecutionContext()->GetContentSecurityPolicy()->AllowImageFromSource(params.href, params.href, RedirectStatus::kNoRedirect)) {
            return;
        }
...
void LinkStyle::UnblockRenderingForPendingSheet()
{
    ...
    if (pending_sheet_type_ == PendingSheetType::kDynamicRenderBlocking) {
        GetDocument().GetStyleEngine().RemovePendingBlockingSheet(*owner_, pending_sheet_type_);
