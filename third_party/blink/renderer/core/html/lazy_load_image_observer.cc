...
bool IsElementInInvisibleSubTree(const Element& element)
{
    ...
    for (Node& ancestor : FlatTreeTraversal::InclusiveAncestorsOf(element)) {
        ...
        if (style && (style->Visibility() != EVisibility::kVisible || style->Display() == EDisplay::kNone)) {
            return true;
        }
    }
    return false;
...
void LazyLoadImageObserver::StartMonitoringNearViewport(Document* root_document, Element* element)
{
    if (!lazy_load_intersection_observer_) {
        ...
        lazy_load_intersection_observer_
            = IntersectionObserver::Create(*root_document, WTF::BindRepeating(&LazyLoadImageObserver::LoadIfNearViewport, WrapWeakPersistent(this)),
                LocalFrameUkmAggregator::kLazyLoadIntersectionObserver, std::move(params));
    }
...
bool LazyLoadImageObserver::LoadAllImagesAndBlockLoadEvent(Document& for_document)
{
    if (!lazy_load_intersection_observer_) {
        return false;
    }
    ...
    for (const IntersectionObservation* observation : lazy_load_intersection_observer_->Observations()) {
        ...
        if (auto* image_element = DynamicTo<HTMLImageElement>(element)) {
            const_cast<HTMLImageElement*>(image_element)->LoadDeferredImageBlockingLoad();
...
void LazyLoadImageObserver::LoadIfNearViewport(const HeapVector<Member<IntersectionObserverEntry>>& entries)
{
    ...
    for (auto entry : entries) {
        ...
        // intersection.
        if (!entry->isIntersecting() && image_element && !image_element->HasLazyLoadingAttribute()) {
...
int LazyLoadImageObserver::GetLazyLoadingImageMarginPx(const Document& document)
{
