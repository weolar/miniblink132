...
LayoutObject* HTMLMeterElement::CreateLayoutObject(const ComputedStyle& style)
{
    ...
    case kMeterPart:
        UseCounter::Count(GetDocument(), WebFeature::kMeterElementWithMeterAppearance);
        break;
    case kNoControlPart:
        UseCounter::Count(GetDocument(), WebFeature::kMeterElementWithNoneAppearance);
        break;
    default:
        break;
    }
...
void HTMLMeterElement::ParseAttribute(const AttributeModificationParams& params)
{
    const QualifiedName& name = params.name;
    if (name == html_names::kValueAttr || name == html_names::kMinAttr || name == html_names::kMaxAttr || name == html_names::kLowAttr
...
{
    return std::max(GetFloatingPointAttribute(html_names::kMaxAttr, std::max(1.0, min())), min());
}
...
{
    double optimum = GetFloatingPointAttribute(html_names::kOptimumAttr, (max() + min()) / 2);
...
{
    DEFINE_STATIC_LOCAL(AtomicString, optimum_pseudo_id, ("-webkit-meter-optimum-value"));
    DEFINE_STATIC_LOCAL(AtomicString, suboptimum_pseudo_id, ("-webkit-meter-suboptimum-value"));
    DEFINE_STATIC_LOCAL(AtomicString, even_less_good_pseudo_id, ("-webkit-meter-even-less-good-value"));

    value_->SetInlineStyleProperty(CSSPropertyID::kInlineSize, percentage, CSSPrimitiveValue::UnitType::kPercentage);
    value_->SetInlineStyleProperty(CSSPropertyID::kBlockSize, 100, CSSPrimitiveValue::UnitType::kPercentage);
...
void HTMLMeterElement::AdjustStyle(ComputedStyleBuilder& builder)
{
    ...
    builder.SetUAShadowHostData(std::make_unique<StyleUAShadowHostData>(
        /* width */ Length(),
        /* height */ Length(), StyleAspectRatio(EAspectRatioType::kAuto, gfx::SizeF()),
