...
HTMLFrameElementBase::HTMLFrameElementBase(const QualifiedName& tag_name, Document& document)
...
void HTMLFrameElementBase::OpenURL(bool replace_current_item)
{
    ...
    if (!url.IsValid() && GetDocument().BaseURL().ProtocolIsData()) {
        GetExecutionContext()->AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kRendering,
            mojom::ConsoleMessageLevel::kWarning, "Invalid relative frame source URL (" + url_ + ") within data URL."));
    }
    LoadOrRedirectSubframe(url, frame_name_, replace_current_item);
}

void HTMLFrameElementBase::ParseAttribute(const AttributeModificationParams& params)
{
    ...
    if (name == html_names::kSrcdocAttr) {
    ...
    } else if (name == html_names::kSrcAttr && !FastHasAttribute(html_names::kSrcdocAttr)) {
        ...
        if (EqualIgnoringASCIICase(value, "off") || EqualIgnoringASCIICase(value, "noscroll") || EqualIgnoringASCIICase(value, "no"))
        ...
        SetAttributeEventListener(event_type_names::kBeforeunload,
...
scoped_refptr<const SecurityOrigin> HTMLFrameElementBase::GetOriginForPermissionsPolicy() const
{
    // Sandboxed frames have a unique origin.
    if ((GetFramePolicy().sandbox_flags & network::mojom::blink::WebSandboxFlags::kOrigin) != network::mojom::blink::WebSandboxFlags::kNone) {
...
Node::InsertionNotificationRequest HTMLFrameElementBase::InsertedInto(ContainerNode& insertion_point)
{
    ...
    SECURITY_CHECK(!ContentFrame() || GetDocument().StatePreservingAtomicMoveInProgress());
...
void HTMLFrameElementBase::SetFocused(bool received, mojom::blink::FocusType focus_type)
{
...
{
    return attribute.GetName() == html_names::kLongdescAttr || attribute.GetName() == html_names::kSrcAttr || HTMLFrameOwnerElement::IsURLAttribute(attribute);
}

bool HTMLFrameElementBase::HasLegalLinkAttribute(const QualifiedName& name) const
{
    return name == html_names::kSrcAttr || HTMLFrameOwnerElement::HasLegalLinkAttribute(name);
}

bool HTMLFrameElementBase::IsHTMLContentAttribute(const Attribute& attribute) const
{
    return attribute.GetName() == html_names::kSrcdocAttr || HTMLFrameOwnerElement::IsHTMLContentAttribute(attribute);
}

void HTMLFrameElementBase::SetScrollbarMode(mojom::blink::ScrollbarMode scrollbar_mode)
{
    ...
    if (contentDocument()) {
        contentDocument()->WillChangeFrameOwnerProperties(
            margin_width_, margin_height_, scrollbar_mode, IsDisplayNone(), GetColorScheme(), GetPreferredColorScheme());
    }
...
void HTMLFrameElementBase::SetMarginWidth(int margin_width)
{
    ...
    if (contentDocument()) {
        contentDocument()->WillChangeFrameOwnerProperties(
            margin_width, margin_height_, scrollbar_mode_, IsDisplayNone(), GetColorScheme(), GetPreferredColorScheme());
    }
...
void HTMLFrameElementBase::SetMarginHeight(int margin_height)
{
    ...
    if (contentDocument()) {
        contentDocument()->WillChangeFrameOwnerProperties(
            margin_width_, margin_height, scrollbar_mode_, IsDisplayNone(), GetColorScheme(), GetPreferredColorScheme());
    }
