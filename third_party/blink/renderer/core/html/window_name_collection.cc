...
WindowNameCollection::WindowNameCollection(ContainerNode& document, CollectionType type, const AtomicString& name)
...
bool WindowNameCollection::ElementMatches(const Element& element) const
{
    ...
    // but anything by id
    if (IsA<HTMLImageElement>(element) || IsA<HTMLFormElement>(element) || IsA<HTMLEmbedElement>(element) || IsA<HTMLObjectElement>(element)) {
