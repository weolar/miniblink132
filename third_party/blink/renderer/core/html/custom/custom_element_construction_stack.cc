...
using ConstructorToStackMap = HeapHashMap<Member<V8CustomElementConstructor>, Member<CustomElementConstructionStack>, V8CustomElementConstructorHashTraits>;

using WindowMap = HeapHashMap<Member<const LocalDOMWindow>, Member<ConstructorToStackMap>>;
...
ConstructorToStackMap& EnsureConstructorToStackMap(const LocalDOMWindow* window)
{
    WindowMap& window_map = EnsureWindowMap();
    auto add_result = window_map.insert(window, nullptr);
    if (add_result.is_new_entry) {
        add_result.stored_value->value = MakeGarbageCollected<ConstructorToStackMap>();
    }
    return *add_result.stored_value->value;
}

CustomElementConstructionStack& EnsureConstructionStack(CustomElementDefinition& definition)
{
    ...
    V8CustomElementConstructor* constructor = definition.GetV8CustomElementConstructor();
    v8::HandleScope handle_scope(constructor->GetIsolate());
    auto add_result = stack_map.insert(constructor, nullptr);
    if (add_result.is_new_entry) {
        add_result.stored_value->value = MakeGarbageCollected<CustomElementConstructionStack>();
    }
    return *add_result.stored_value->value;
}
...
CustomElementConstructionStack* GetCustomElementConstructionStack(const LocalDOMWindow* window, v8::Local<v8::Object> constructor)
{
    ...
    ConstructorToStackMap* constructor_stack_map = constructor_stack_map_iter->value;
    auto construction_stack_iter = constructor_stack_map->Find<V8CustomElementConstructorHashTranslator>(constructor);
...
CustomElementConstructionStackScope::CustomElementConstructionStackScope(CustomElementDefinition& definition, Element& element)
    : construction_stack_(EnsureConstructionStack(definition))
{
    // Push the construction stack.
    construction_stack_.push_back(CustomElementConstructionStackEntry(element, definition));
    ++nesting_level_;
#if DCHECK_IS_ON()
    element_ = &element;
...
{
#if DCHECK_IS_ON()
    DCHECK(!construction_stack_.back().element || construction_stack_.back().element == element_);
