...
public:
    ...
    CustomElementDescriptor(const AtomicString& name, const AtomicString& local_name)
        : name_(name)
    ...
    {
        return LocalName() == element.localName() && (IsAutonomous() || GetName() == element.IsValue())
            && element.namespaceURI() == html_names::xhtmlNamespaceURI;
    }

    bool IsAutonomous() const
    {
        return name_ == local_name_;
    }
