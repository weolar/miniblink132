...
CustomElementUpgradeSorter::AddResult CustomElementUpgradeSorter::AddToParentChildMap(Node* parent, Node* child)
{
...
void CustomElementUpgradeSorter::Add(Element* element)
{
    ...
    for (Node *n = element, *parent = n->ParentOrShadowHostNode(); parent; n = parent, parent = parent->ParentOrShadowHostNode()) {
        if (AddToParentChildMap(parent, n) == kParentAlreadyExistsInMap)
            break;
    }
}

void CustomElementUpgradeSorter::Visit(HeapVector<Member<Element>>* result, ChildSet& children, const ChildSet::iterator& it)
{
    if (it == children.end())
        return;
...
void CustomElementUpgradeSorter::Sorted(HeapVector<Member<Element>>* result, Node* parent)
{
    ...
    for (Element* e = ElementTraversal::FirstChild(*parent); e && children->size() > 1; e = ElementTraversal::NextSibling(*e)) {
