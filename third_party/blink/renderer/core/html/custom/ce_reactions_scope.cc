...
CEReactionsScope::~CEReactionsScope()
{
    if (stack_) {
        ...
        if (!original_exception.IsEmpty()) [[unlikely]] {
            V8ThrowException::ThrowException(stack_->GetSupplementable()->isolate(), original_exception);
            try_catch_->ReThrow();
        }
    }
...
void CEReactionsScope::EnqueueToCurrentQueue(CustomElementReactionStack& stack, Element& element, CustomElementReaction& reaction)
{
    if (!stack_)
        stack.Push();
    stack_ = &stack;
