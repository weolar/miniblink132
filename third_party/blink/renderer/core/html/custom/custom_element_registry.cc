...
void CollectUpgradeCandidateInNode(Node& root, HeapVector<Member<Element>>& candidates)
{
...
bool ThrowIfInvalidName(const AtomicString& name, bool allow_embedder_names, ExceptionState& exception_state)
{
    if (CustomElement::IsValidName(name, allow_embedder_names))
        return false;
    exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "\"" + name + "\" is not a valid custom element name");
    return true;
}

// Returns true if |name| is valid.
bool ThrowIfValidName(const AtomicString& name, ExceptionState& exception_state)
{
    if (!CustomElement::IsValidName(name, false))
        return false;
    exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "\"" + name + "\" is a valid custom element name");
    return true;
}
...
// static
CustomElementRegistry* CustomElementRegistry::Create(ScriptState* script_state)
{
    DCHECK(RuntimeEnabledFeatures::ScopedCustomElementRegistryEnabled());
    return MakeGarbageCollected<CustomElementRegistry>(LocalDOMWindow::From(script_state));
}
...
CustomElementDefinition* CustomElementRegistry::define(ScriptState* script_state, const AtomicString& name, V8CustomElementConstructor* constructor,
    const ElementDefinitionOptions* options, ExceptionState& exception_state)
{
    ScriptCustomElementDefinitionBuilder builder(script_state, this, constructor, exception_state);
    return DefineInternal(script_state, name, builder, options, exception_state);
}

// https://html.spec.whatwg.org/C/#element-definition
CustomElementDefinition* CustomElementRegistry::DefineInternal(ScriptState* script_state, const AtomicString& name, CustomElementDefinitionBuilder& builder,
    const ElementDefinitionOptions* options, ExceptionState& exception_state)
{
    ...
    const bool allow_embedder_names = WebCustomElement::EmbedderNamesAllowedScope::IsAllowed();
    if (ThrowIfInvalidName(name, allow_embedder_names, exception_state))
        return nullptr;

    if (NameIsDefined(name)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "the name \"" + name + "\" has already been used with this registry");
        return nullptr;
    }
    ...
    if (!options->extends().IsNull()) {
        ...
        if (HtmlElementTypeForTag(extends, owner_->document()) == HTMLElementType::kHTMLUnknownElement) {
            exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "\"" + extends + "\" is an HTMLUnknownElement");
            return nullptr;
        }
    ...
    if (element_definition_is_running_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "an element definition is already being processed");
        return nullptr;
    }
    ...
    auto constructor_add_result = constructor_map_.insert(builder.Constructor(), definition);
...
const AtomicString& CustomElementRegistry::getName(V8CustomElementConstructor* constructor)
{
    if (!constructor) {
        return g_null_atom;
    }
...
CustomElementDefinition* CustomElementRegistry::DefinitionForName(const AtomicString& name) const
{
...
CustomElementDefinition* CustomElementRegistry::DefinitionForConstructor(V8CustomElementConstructor* constructor) const
{
...
CustomElementDefinition* CustomElementRegistry::DefinitionForConstructor(v8::Local<v8::Object> constructor) const
{
    const auto it = constructor_map_.Find<V8CustomElementConstructorHashTranslator>(constructor);
    if (it == constructor_map_.end())
        return nullptr;
    return it->value.Get();
}
...
void CustomElementRegistry::AddCandidate(Element& candidate)
{
    ...
    if (it != upgrade_candidates_->end()) {
        ...
        set = upgrade_candidates_->insert(name, MakeGarbageCollected<UpgradeCandidateSet>()).stored_value->value;
    }
...
ScriptPromise<V8CustomElementConstructor> CustomElementRegistry::whenDefined(
    ScriptState* script_state, const AtomicString& name, ExceptionState& exception_state)
{
    if (ThrowIfInvalidName(name, false, exception_state))
        return EmptyPromise();
    if (CustomElementDefinition* definition = DefinitionForName(name)) {
        return ToResolvedPromise<V8CustomElementConstructor>(script_state, definition->GetV8CustomElementConstructor());
    }
    const auto it = when_defined_promise_map_.find(name);
    if (it != when_defined_promise_map_.end())
        return it->value->Promise();
    auto* new_resolver = MakeGarbageCollected<ScriptPromiseResolver<V8CustomElementConstructor>>(script_state, exception_state.GetContext());
    when_defined_promise_map_.insert(name, new_resolver);
    return new_resolver->Promise();
}

void CustomElementRegistry::CollectCandidates(const CustomElementDescriptor& desc, HeapVector<Member<Element>>* elements)
{
