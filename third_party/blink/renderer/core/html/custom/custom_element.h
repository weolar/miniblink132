...
public:
    ...
    static bool IsValidName(const AtomicString& name, bool including_embedder_names)
    {
    ...
    static bool ShouldCreateCustomizedBuiltinElement(const AtomicString& local_name, const Document&);
    static bool ShouldCreateCustomizedBuiltinElement(const QualifiedName&, const Document&);

    // Look up a definition, and create an autonomous custom element if
    // it's found.
    static HTMLElement* CreateCustomElement(TreeScope&, const QualifiedName&, const CreateElementFlags);

    // Creates "uncustomized" or "undefined" state element. This should be
    // used when CustomElementDefinition is not found.
    static Element* CreateUncustomizedOrUndefinedElement(Document&, const QualifiedName&, const CreateElementFlags, const AtomicString& is_value);
    ...
    static void EnqueueAdoptedCallback(Element&, Document& old_owner, Document& new_owner);
    static void EnqueueAttributeChangedCallback(Element&, const QualifiedName&, const AtomicString& old_value, const AtomicString& new_value);
    static void EnqueueFormAssociatedCallback(Element& element, HTMLFormElement* nullable_form);
    ...
    static void EnqueueFormStateRestoreCallback(Element& element, const V8ControlValue* value, const String& mode);

    static void TryToUpgrade(Element&);

    static void AddEmbedderCustomElementNameForTesting(const AtomicString& name, ExceptionState&);
...
private:
    ...
    static Element* CreateUncustomizedOrUndefinedElementTemplate(Document&, const QualifiedName&, const CreateElementFlags, const AtomicString& is_value);
};
