...
CustomElementDefinition::CustomElementDefinition(CustomElementRegistry& registry, const CustomElementDescriptor& descriptor)
    : registry_(registry)
...
CustomElementDefinition::CustomElementDefinition(CustomElementRegistry& registry, const CustomElementDescriptor& descriptor,
    const HashSet<AtomicString>& observed_attributes, const Vector<String>& disabled_features, FormAssociationFlag form_association_flag)
    : registry_(registry)
    , descriptor_(descriptor)
    , observed_attributes_(observed_attributes)
    , has_style_attribute_changed_callback_(observed_attributes.Contains(html_names::kStyleAttr.LocalName()))
...
static String ErrorMessageForConstructorResult(Element& element, Document& document, const QualifiedName& tag_name)
{
...
void CustomElementDefinition::CheckConstructorResult(Element* element, Document& document, const QualifiedName& tag_name, ExceptionState& exception_state)
{
    // https://dom.spec.whatwg.org/#concept-create-element
    ...
    if (!element || !element->IsHTMLElement()) {
        exception_state.ThrowTypeError("The result must implement HTMLElement interface");
        return;
    }

    // 6.1.4. through 6.1.9.
    const String message = ErrorMessageForConstructorResult(*element, document, tag_name);
    if (!message.empty()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, message);
    }
}

HTMLElement* CustomElementDefinition::CreateElementForConstructor(Document& document)
{
    HTMLElement* element = HTMLElementFactory::Create(Descriptor().LocalName(), document, CreateElementFlags::ByCreateElement());
    if (element) {
        element->SetIsValue(Descriptor().GetName());
    } else {
        element = MakeGarbageCollected<HTMLElement>(QualifiedName(g_null_atom, Descriptor().LocalName(), html_names::xhtmlNamespaceURI), document);
    }
...
HTMLElement* CustomElementDefinition::CreateElement(Document& document, const QualifiedName& tag_name, CreateElementFlags flags)
{
    DCHECK(CustomElement::ShouldCreateCustomElement(tag_name) || CustomElement::ShouldCreateCustomizedBuiltinElement(tag_name, document)) << tag_name;
    ...
    if (!flags.IsAsyncCustomElements()) {
        ...
        CHECK(!RuntimeEnabledFeatures::ScopedCustomElementRegistryEnabled() || registry_->IsGlobalRegistry());
...
void CustomElementDefinition::Upgrade(Element& element)
{
    ...
    if (element.GetCustomElementState() != CustomElementState::kUndefined && element.GetCustomElementState() != CustomElementState::kUncustomized) {
        return;
    }
    ...
    bool succeeded = false;
    {
        ...
        CustomElementConstructionStackScope construction_stack_scope(*this, element);
    ...
    if (!succeeded) {
        ...
        CustomElementReactionStack::From(element.GetDocument().GetAgent()).ClearQueue(element);
        return;
    }
...
bool CustomElementDefinition::HasAttributeChangedCallback(const QualifiedName& name) const
{
...
{
    CustomElement::Enqueue(element, CustomElementReactionFactory::CreateUpgrade(*this));
}

void CustomElementDefinition::EnqueueConnectedCallback(Element& element)
{
    CustomElement::Enqueue(element, CustomElementReactionFactory::CreateConnected(*this));
}

void CustomElementDefinition::EnqueueDisconnectedCallback(Element& element)
{
    CustomElement::Enqueue(element, CustomElementReactionFactory::CreateDisconnected(*this));
}

void CustomElementDefinition::EnqueueConnectedMoveCallback(Element& element)
{
    CustomElement::Enqueue(element, CustomElementReactionFactory::CreateConnectedMove(*this));
}

void CustomElementDefinition::EnqueueAdoptedCallback(Element& element, Document& old_document, Document& new_document)
{
    CustomElement::Enqueue(element, CustomElementReactionFactory::CreateAdopted(*this, old_document, new_document));
}

void CustomElementDefinition::EnqueueAttributeChangedCallback(
    Element& element, const QualifiedName& name, const AtomicString& old_value, const AtomicString& new_value)
{
    CustomElement::Enqueue(element, CustomElementReactionFactory::CreateAttributeChanged(*this, name, old_value, new_value));
}

void CustomElementDefinition::EnqueueAttributeChangedCallbackForAllAttributes(Element& element)
{
    ...
    for (const auto& attribute : element.AttributesWithoutUpdate()) {
        if (HasAttributeChangedCallback(attribute.GetName())) {
            EnqueueAttributeChangedCallback(element, attribute.GetName(), g_null_atom, attribute.Value());
        }
    }
}
