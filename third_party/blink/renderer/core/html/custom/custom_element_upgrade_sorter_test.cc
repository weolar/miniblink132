...
protected:
    ...
    Element* CreateElementWithId(const char* local_name, const char* id)
    {
        ...
        Element* element = GetDocument().CreateElementForBinding(AtomicString(local_name), nullptr, no_exceptions);
...
{
    NonThrowableExceptionState no_exceptions;
    Element* element = GetDocument().CreateElementForBinding(AtomicString("a-a"), nullptr, no_exceptions);
    ...
    EXPECT_EQ(0u, elements.size()) << "the adopted-away candidate should not have been included";
}
...
TEST_F(CustomElementUpgradeSorterTest, oneCandidate)
{
    ...
    HeapVector<Member<Element>> elements;
    sorter.Sorted(&elements, &GetDocument());
    EXPECT_EQ(1u, elements.size()) << "exactly one candidate should be in the result set";
