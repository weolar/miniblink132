...
TEST(CustomElementReactionQueueTest, invokeReactions_one)
{
    ...
    Vector<char> log;
    CustomElementReactionQueue* queue = MakeGarbageCollected<CustomElementReactionQueue>();
    ...
    EXPECT_EQ(log, Vector<char>({ 'a' })) << "the reaction should have been invoked";
}
...
TEST(CustomElementReactionQueueTest, invokeReactions_many)
{
    ...
    Element* test_element = CreateElement(AtomicString("my-element"));
    queue->InvokeReactions(*test_element);
    EXPECT_EQ(log, Vector<char>({ 'a', 'b', 'c' })) << "the reaction should have been invoked";
}
...
TEST(CustomElementReactionQueueTest, invokeReactions_recursive)
{
    ...
    CustomElementReaction* third = MakeGarbageCollected<TestReaction>(std::move(third_commands)); // "Empty" recursion
    ...
    CustomElementReaction* second = MakeGarbageCollected<TestReaction>(std::move(second_commands)); // Unwinds one level of recursion
    ...
    CustomElementReaction* first = MakeGarbageCollected<TestReaction>(std::move(first_commands)); // Non-empty recursion

    queue->Add(*first);
    Element* test_element = CreateElement(AtomicString("my-element"));
    queue->InvokeReactions(*test_element);
    EXPECT_EQ(log, Vector<char>({ 'a', 'b', 'c' })) << "the reactions should have been invoked";
}
...
TEST(CustomElementReactionQueueTest, clear_duringInvoke)
{
    ...
    }
    {
        HeapVector<Member<Command>> commands;
        commands.push_back(
            MakeGarbageCollected<Call>(WTF::BindOnce([](CustomElementReactionQueue* queue, Element&) { queue->Clear(); }, WrapPersistent(queue))));
        queue->Add(*MakeGarbageCollected<TestReaction>(std::move(commands)));
    }
    {
        HeapVector<Member<Command>> commands;
    ...
    Element* test_element = CreateElement(AtomicString("my-element"));
    queue->InvokeReactions(*test_element);
    EXPECT_EQ(log, Vector<char>({ 'a' })) << "only 'a' should be logged; the second log should have been cleared";
}
