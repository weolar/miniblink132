...
public:
    ...
    {
        DCHECK(instance_) << "Custom element unit tests require CustomElementTestingScope";
        return *instance_;
    }
...
class TestCustomElementDefinitionBuilder : public CustomElementDefinitionBuilder {
    STACK_ALLOCATED();

public:
    TestCustomElementDefinitionBuilder();
    TestCustomElementDefinitionBuilder(const TestCustomElementDefinitionBuilder&) = delete;
    TestCustomElementDefinitionBuilder& operator=(const TestCustomElementDefinitionBuilder&) = delete;
...
public:
    explicit TestCustomElementDefinition(const CustomElementDescriptor& descriptor);

    TestCustomElementDefinition(const CustomElementDescriptor& descriptor, V8CustomElementConstructor* constructor);

    TestCustomElementDefinition(const CustomElementDescriptor& descriptor, V8CustomElementConstructor* constructor, HashSet<AtomicString>&& observed_attributes,
        const Vector<String>& disabled_features);

    TestCustomElementDefinition(const TestCustomElementDefinition&) = delete;
    TestCustomElementDefinition& operator=(const TestCustomElementDefinition&) = delete;
    ...
    HTMLElement* CreateAutonomousCustomElementSync(Document& document, const QualifiedName&) override
    {
    ...
    void RunAdoptedCallback(Element&, Document& old_owner, Document& new_owner) override
    {
        NOTREACHED() << "definition does not have adopted callback";
    }

    void RunAttributeChangedCallback(Element&, const QualifiedName&, const AtomicString& old_value, const AtomicString& new_value) override
    {
        NOTREACHED() << "definition does not have attribute changed callback";
    }
    void RunFormAssociatedCallback(Element& element, HTMLFormElement* nullable_form) override
    {
    ...
    void RunFormStateRestoreCallback(Element& element, const V8ControlValue* value, const String& mode) override
    {
...
public:
    ...
    operator Element*() const
    {
        ...
        Element* element = document->CreateElement(QualifiedName(g_null_atom, local_name_, namespace_uri_), CreateElementFlags::ByCreateElement(), is_value_);
