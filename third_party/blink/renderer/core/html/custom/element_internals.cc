...
bool IsValidityStateFlagsValid(const ValidityStateFlags* flags)
{
    if (!flags)
        return true;
    if (flags->badInput() || flags->customError() || flags->patternMismatch() || flags->rangeOverflow() || flags->rangeUnderflow() || flags->stepMismatch()
        || flags->tooLong() || flags->tooShort() || flags->typeMismatch() || flags->valueMissing())
        return false;
    return true;
}

void AppendToFormControlState(const V8ControlValue& value, FormControlState& state)
{
...
const V8ControlValue* RestoreFromFormControlState(
    ExecutionContext& execution_context, const FormControlState& state, const StringView& section_title, wtf_size_t& index)
{
    ...
    if (entry_type == "USVString") {
        ...
        if (auto* file = File::CreateFromControlState(&execution_context, state, index)) {
            restored_value = MakeGarbageCollected<V8ControlValue>(file);
        }
    } else if (entry_type == "FormData") {
        if (auto* form_data = FormData::CreateFromControlState(execution_context, state, index)) {
...
void ElementInternals::setFormValue(const V8ControlValue* value, ExceptionState& exception_state)
{
    setFormValue(value, value, exception_state);
}

void ElementInternals::setFormValue(const V8ControlValue* value, const V8ControlValue* state, ExceptionState& exception_state)
{
    if (!IsTargetFormAssociated()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "The target element is not a form-associated custom element.");
        return;
    }

    if (value && value->IsFormData()) {
        value_ = MakeGarbageCollected<V8ControlValue>(MakeGarbageCollected<FormData>(*value->GetAsFormData()));
    } else {
        value_ = value;
    }
    ...
    if (value == state) {
        ...
        state_ = MakeGarbageCollected<V8ControlValue>(MakeGarbageCollected<FormData>(*state->GetAsFormData()));
    } else {
        state_ = state;
    }
...
void ElementInternals::setValidity(ValidityStateFlags* flags, ExceptionState& exception_state)
{
    setValidity(flags, String(), nullptr, exception_state);
}

void ElementInternals::setValidity(ValidityStateFlags* flags, const String& message, ExceptionState& exception_state)
{
    setValidity(flags, message, nullptr, exception_state);
}

void ElementInternals::setValidity(ValidityStateFlags* flags, const String& message, HTMLElement* anchor, ExceptionState& exception_state)
{
    if (!IsTargetFormAssociated()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "The target element is not a form-associated custom element.");
        return;
    ...
    if (!IsValidityStateFlagsValid(flags) && message.empty()) {
        exception_state.ThrowTypeError("The second argument should not be empty if one or more flags in the "
...
String ElementInternals::ValidationMessageForBinding(ExceptionState& exception_state)
{
    if (!IsTargetFormAssociated()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "The target element is not a form-associated custom element.");
        return String();
...
const AtomicString& ElementInternals::FastGetAttribute(const QualifiedName& attribute) const
{
...
const HashMap<QualifiedName, AtomicString>& ElementInternals::GetAttributes() const
{
    return accessibility_semantics_map_;
}

void ElementInternals::setAttribute(const QualifiedName& attribute, const AtomicString& value)
{
...
void ElementInternals::DidUpgrade()
{
    ...
    Target().GetDocument().GetFormController().RestoreControlStateOnUpgrade(*this);
}

void ElementInternals::SetElementAttribute(const QualifiedName& attribute, Element* element)
{
    if (!element) {
    ...
    FrozenArray<Element>* array = MakeGarbageCollected<FrozenArray<Element>>(std::move(vector));
...
Element* ElementInternals::GetElementAttribute(const QualifiedName& attribute) const
{
...
void ElementInternals::SetElementArrayAttribute(const QualifiedName& attribute, const HeapVector<Member<Element>>* given_elements)
{
    if (!given_elements) {
        explicitly_set_attr_elements_map_.erase(attribute);
        setAttribute(attribute, g_empty_atom);
    ...
    FrozenArray<Element>* frozen_elements = MakeGarbageCollected<FrozenArray<Element>>((std::move(*given_elements)));
...
const FrozenArray<Element>* ElementInternals::GetElementArrayAttribute(const QualifiedName& attribute) const
{
    auto it = explicitly_set_attr_elements_map_.find(attribute);
    if (it == explicitly_set_attr_elements_map_.end()) {
        return nullptr;
...
void ElementInternals::setAriaControlsElements(HeapVector<Member<Element>>* given_elements)
{
...
void ElementInternals::setAriaDescribedByElements(HeapVector<Member<Element>>* given_elements)
{
...
void ElementInternals::setAriaDetailsElements(HeapVector<Member<Element>>* given_elements)
{
...
void ElementInternals::setAriaErrorMessageElements(HeapVector<Member<Element>>* given_elements)
{
...
void ElementInternals::setAriaFlowToElements(HeapVector<Member<Element>>* given_elements)
{
...
void ElementInternals::setAriaLabelledByElements(HeapVector<Member<Element>>* given_elements)
{
...
void ElementInternals::setAriaOwnsElements(HeapVector<Member<Element>>* given_elements)
{
...
bool ElementInternals::IsTargetFormAssociated() const
{
    ...
    if (Target().GetCustomElementState() != CustomElementState::kUndefined && Target().GetCustomElementState() != CustomElementState::kFailed
...
void ElementInternals::RestoreFormControlState(const FormControlState& state)
{
    ...
    if (const V8ControlValue* restored_value = RestoreFromFormControlState(*execution_context, state, "Value", index)) {
        value_ = restored_value;
    }

    const V8ControlValue* restored_state = RestoreFromFormControlState(*execution_context, state, "State", index);
    if (restored_state) {
        CustomElement::EnqueueFormStateRestoreCallback(Target(), restored_state, "restore");
    }
}
