...
public:
    ...
    CustomElementDefinition* define(
        ScriptState*, const AtomicString& name, V8CustomElementConstructor* constructor, const ElementDefinitionOptions*, ExceptionState&);
    ...
    CustomElementDefinition* DefinitionForConstructor(V8CustomElementConstructor*) const;
    CustomElementDefinition* DefinitionForConstructor(v8::Local<v8::Object> constructor) const;
    ...
    ScriptPromise<V8CustomElementConstructor> whenDefined(ScriptState*, const AtomicString& name, ExceptionState&);
    void upgrade(Node* root);

    const LocalDOMWindow* GetOwnerWindow() const
    {
        return owner_.Get();
    }
...
private:
    CustomElementDefinition* DefineInternal(
        ScriptState*, const AtomicString& name, CustomElementDefinitionBuilder&, const ElementDefinitionOptions*, ExceptionState&);

    void CollectCandidates(const CustomElementDescriptor&, HeapVector<Member<Element>>*);

    bool element_definition_is_running_;

    using ConstructorMap = HeapHashMap<Member<V8CustomElementConstructor>, Member<CustomElementDefinition>, V8CustomElementConstructorHashTraits>;
    ...
    using WhenDefinedPromiseMap = HeapHashMap<AtomicString, Member<ScriptPromiseResolver<V8CustomElementConstructor>>>;
    ...
    FRIEND_TEST_ALL_PREFIXES(CustomElementTest, CreateElement_TagNameCaseHandlingCreatingCustomElement);
