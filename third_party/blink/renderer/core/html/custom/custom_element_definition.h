...
class CORE_EXPORT CustomElementDefinition : public GarbageCollected<CustomElementDefinition>, public NameClient, public ElementRareDataField {
public:
    ...
    CustomElementRegistry& GetRegistry()
    {
        return *registry_;
    }
    ...
    virtual HTMLElement* CreateAutonomousCustomElementSync(Document&, const QualifiedName&) = 0;
    HTMLElement* CreateElement(Document&, const QualifiedName&, const CreateElementFlags);
    ...
    virtual void RunAdoptedCallback(Element&, Document& old_owner, Document& new_owner) = 0;
    virtual void RunAttributeChangedCallback(Element&, const QualifiedName&, const AtomicString& old_value, const AtomicString& new_value) = 0;
    virtual void RunFormAssociatedCallback(Element& element, HTMLFormElement* nullable_form) = 0;
    ...
    virtual void RunFormStateRestoreCallback(Element& element, const V8ControlValue* value, const String& mode) = 0;
    ...
    void EnqueueAdoptedCallback(Element&, Document& old_owner, Document& new_owner);
    void EnqueueAttributeChangedCallback(Element&, const QualifiedName&, const AtomicString& old_value, const AtomicString& new_value);

    bool DisableShadow() const
    {
        return disable_shadow_;
    }
    bool DisableInternals() const
    {
        return disable_internals_;
    }
    bool IsFormAssociated() const
    {
        return is_form_associated_;
    }

protected:
    CustomElementDefinition(CustomElementRegistry&, const CustomElementDescriptor&);

    CustomElementDefinition(CustomElementRegistry&, const CustomElementDescriptor&, const HashSet<AtomicString>& observed_attributes,
        const Vector<String>& disabled_features, FormAssociationFlag form_association_flag);
