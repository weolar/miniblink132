...
// static
const char CustomElementReactionStack::kSupplementName[] = "CustomElementReactionStackAgentData";
...
void CustomElementReactionStack::EnqueueToCurrentQueue(Element& element, CustomElementReaction& reaction)
{
    Enqueue(stack_.back(), element, reaction);
}

void CustomElementReactionStack::Enqueue(Member<ElementQueue>& queue, Element& element, CustomElementReaction& reaction)
{
    if (!queue)
    ...
    if (it != map_.end()) {
        ...
        CustomElementReactionQueue* reactions = MakeGarbageCollected<CustomElementReactionQueue>();
        map_.insert(&element, reactions);
        reactions->Add(reaction);
    }
}

void CustomElementReactionStack::EnqueueToBackupQueue(Element& element, CustomElementReaction& reaction)
{
    ...
    if (!backup_queue_ || backup_queue_->empty()) {
        element.GetDocument().GetAgent().event_loop()->EnqueueMicrotask(
            WTF::BindOnce(&CustomElementReactionStack::InvokeBackupQueue, Persistent<CustomElementReactionStack>(this)));
    }
...
{
    CustomElementReactionStack* supplement = Supplement<Agent>::From<CustomElementReactionStack>(agent);
    if (!supplement) {
...
CustomElementReactionStack* CustomElementReactionStack::Swap(Agent& agent, CustomElementReactionStack* new_stack)
{
    CustomElementReactionStack* old_stack = &CustomElementReactionStack::From(agent);
