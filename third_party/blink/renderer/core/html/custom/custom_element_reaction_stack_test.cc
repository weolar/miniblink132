...
TEST(CustomElementReactionStackTest, one)
{
    ...
    CustomElementReactionStack* stack = MakeGarbageCollected<CustomElementReactionStack>(*execution_context.GetExecutionContext().GetAgent());
    stack->Push();
    HeapVector<Member<Command>> commands;
    commands.push_back(MakeGarbageCollected<Log>('a', log));
    stack->EnqueueToCurrentQueue(*CreateElement(AtomicString("a")), *MakeGarbageCollected<TestReaction>(std::move(commands)));
    stack->PopInvokingReactions();

    EXPECT_EQ(log, Vector<char>({ 'a' })) << "popping the reaction stack should run reactions";
}
...
TEST(CustomElementReactionStackTest, multipleElements)
{
    ...
    EXPECT_EQ(log, Vector<char>({ 'a', 'b' })) << "reactions should run in the order the elements queued";
}
...
TEST(CustomElementReactionStackTest, popTopEmpty)
{
    ...
    EXPECT_EQ(log, Vector<char>()) << "popping the empty top-of-stack should not run any reactions";
}
...
TEST(CustomElementReactionStackTest, popTop)
{
    ...
    EXPECT_EQ(log, Vector<char>({ 'b' })) << "popping the top-of-stack should only run top-of-stack reactions";
}
...
TEST(CustomElementReactionStackTest, requeueingDoesNotReorderElements)
{
    ...
    stack->Push();
    {
        HeapVector<Member<Command>> commands;
        commands.push_back(MakeGarbageCollected<Log>('a', log));
        stack->EnqueueToCurrentQueue(element, *MakeGarbageCollected<TestReaction>(std::move(commands)));
    }
    {
        HeapVector<Member<Command>> commands;
    ...
    EXPECT_EQ(log, Vector<char>({ 'a', 'b', 'z' })) << "reactions should run together in the order elements were queued";
}
...
TEST(CustomElementReactionStackTest, oneReactionQueuePerElement)
{
    ...
    EXPECT_EQ(log, Vector<char>({ 'y', 'a', 'b' })) << "reactions should run together in the order elements were queued";
...
TEST(CustomElementReactionStackTest, enqueueFromReaction)
{
    ...
    stack->Push();
    {
        ...
        HeapVector<Member<Command>> commands;
        commands.push_back(MakeGarbageCollected<EnqueueToStack>(stack, element, MakeGarbageCollected<TestReaction>(std::move(subcommands))));
        stack->EnqueueToCurrentQueue(element, *MakeGarbageCollected<TestReaction>(std::move(commands)));
    }
    stack->PopInvokingReactions();
