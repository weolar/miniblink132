...
class FencedFrameConfigTest : private ScopedFencedFramesForTest, public testing::Test {
public:
    FencedFrameConfigTest()
        : ScopedFencedFramesForTest(true)
    {
        enabled_feature_list_.InitAndEnableFeatureWithParameters(blink::features::kFencedFrames, {});
    }
...
TEST_F(FencedFrameConfigTest, FencedFrameConfigConstructionWithURL)
{
    ...
    EXPECT_EQ(config->GetValueIgnoringVisibility<FencedFrameConfig::Attribute::kURL>(), url);
    ...
    // truncation.
    String long_context(std::string(kFencedFrameConfigSharedStorageContextMaxLength, 'x'));
