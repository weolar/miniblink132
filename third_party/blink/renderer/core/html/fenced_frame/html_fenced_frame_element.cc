...
{
    return PhysicalRect(LayoutUnit::FromDoubleRound(rect.x()), LayoutUnit::FromDoubleRound(rect.y()), LayoutUnit::FromDoubleRound(rect.width()),
        LayoutUnit::FromDoubleRound(rect.height()));
}

String DeprecatedFencedFrameModeToString(blink::FencedFrame::DeprecatedFencedFrameMode mode)
{
    switch (mode) {
...
// tree).
bool ParentModeIsDifferent(blink::FencedFrame::DeprecatedFencedFrameMode current_mode, LocalFrame& frame)
{
    Page* ancestor_page = frame.GetPage();
    return ancestor_page->IsMainFrameFencedFrameRoot() && ancestor_page->DeprecatedFencedFrameMode() != current_mode;
}

bool HasDifferentModeThanParent(HTMLFencedFrameElement& outer_element)
{
    return ParentModeIsDifferent(outer_element.GetDeprecatedMode(), *(outer_element.GetDocument().GetFrame()));
}
...
bool SizeMatchesExactly(const PhysicalSize& requested_size, const gfx::Size& allowed_size)
{
...
double ComputeSizeLossFunction(const PhysicalSize& requested_size, const gfx::Size& allowed_size)
{
    ...
    const double wasted_area
        = scale_x < scale_y ? allowed_width * (allowed_height - (scale_x * requested_height)) : allowed_height * (allowed_width - (scale_y * requested_width));
    ...
    const double resolution_penalty = std::abs(1 - std::min(requested_area, allowed_area) / std::max(requested_area, allowed_area));

    return wasted_area_fraction + resolution_penalty;
}

std::optional<WTF::AtomicString> ConvertEventTypeToFencedEventType(const WTF::String& event_type)
{
...
ParsedPermissionsPolicy HTMLFencedFrameElement::ConstructContainerPolicy() const
{
    if (!GetExecutionContext()) {
        return ParsedPermissionsPolicy();
    }

    scoped_refptr<const SecurityOrigin> src_origin = GetOriginForPermissionsPolicy();
    scoped_refptr<const SecurityOrigin> self_origin = GetExecutionContext()->GetSecurityOrigin();

    PolicyParserMessageBuffer logger;

    ParsedPermissionsPolicy container_policy = PermissionsPolicyParser::ParseAttribute(allow_, self_origin, src_origin, logger, GetExecutionContext());

    for (const auto& message : logger.GetMessages()) {
        GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kOther, message.level, message.content),
...
void HTMLFencedFrameElement::SetCollapsed(bool collapse)
{
    ...
    SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kFrame));
}
...
bool HTMLFencedFrameElement::canLoadOpaqueURL(ScriptState* script_state)
{
    ...
    UseCounter::Count(LocalDOMWindow::From(script_state)->document(), WebFeature::kFencedFrameCanLoadOpaqueURL);
    ...
    if (ParentModeIsDifferent(blink::FencedFrame::DeprecatedFencedFrameMode::kOpaqueAds, *frame_to_check)) {
        return false;
    }
...
Node::InsertionNotificationRequest HTMLFencedFrameElement::InsertedInto(ContainerNode& insertion_point)
{
...
void HTMLFencedFrameElement::ParseAttribute(const AttributeModificationParams& params)
{
    if (params.name == html_names::kSandboxAttr) {
        sandbox_->DidUpdateAttributeValue(params.old_value, params.new_value);

        network::mojom::blink::WebSandboxFlags current_flags = network::mojom::blink::WebSandboxFlags::kNone;
        if (!params.new_value.IsNull()) {
            using network::mojom::blink::WebSandboxFlags;
            auto parsed = network::ParseWebSandboxPolicy(sandbox_->value().Utf8(), WebSandboxFlags::kNone);
            current_flags = parsed.flags;
            if (!parsed.error_message.empty()) {
                GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kOther,
                    mojom::blink::ConsoleMessageLevel::kError, "Error while parsing the 'sandbox' attribute: " + String::FromUTF8(parsed.error_message)));
            }
        }
...
bool HTMLFencedFrameElement::IsPresentationAttribute(const QualifiedName& name) const
{
...
void HTMLFencedFrameElement::CollectStyleForPresentationAttribute(const QualifiedName& name, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
...
void HTMLFencedFrameElement::Navigate(const KURL& url, std::optional<bool> deprecated_should_freeze_initial_size, std::optional<gfx::Size> container_size,
    std::optional<gfx::Size> content_size, String embedder_shared_storage_context)
{
    ...
    if (!GetExecutionContext()->IsSecureContext()) {
        GetDocument().AddConsoleMessage(
            MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kRendering, mojom::blink::ConsoleMessageLevel::kWarning,
                "A fenced frame was not loaded because the page is not in a secure "
                "context."));
        RecordFencedFrameCreationOutcome(FencedFrameCreationOutcome::kInsecureContext);
        return;
    }
    ...
    if (IsValidUrnUuidURL(GURL(url))) {
        ...
        RecordFencedFrameCreationOutcome(FencedFrameCreationOutcome::kIncompatibleURLDefault);
        return;
    }

    if (HasDifferentModeThanParent(*this)) {
        blink::FencedFrame::DeprecatedFencedFrameMode parent_mode = GetDocument().GetPage()->DeprecatedFencedFrameMode();

        GetDocument().AddConsoleMessage(
            MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kRendering, mojom::blink::ConsoleMessageLevel::kWarning,
                "Cannot create a fenced frame with mode '" + DeprecatedFencedFrameModeToString(GetDeprecatedMode()) + "' nested in a fenced frame with mode '"
                    + DeprecatedFencedFrameModeToString(parent_mode) + "'."));
        RecordFencedFrameCreationOutcome(FencedFrameCreationOutcome::kIncompatibleMode);
        return;
    }
    ...
    if (static_cast<int>(GetFramePolicy().sandbox_flags) & static_cast<int>(blink::kFencedFrameMandatoryUnsandboxedFlags)) {
        GetDocument().AddConsoleMessage(
        ...
        RecordFencedFrameCreationOutcome(FencedFrameCreationOutcome::kSandboxFlagsNotSet);
    ...
    RecordFencedFrameCreationOutcome(mode_ == blink::FencedFrame::DeprecatedFencedFrameMode::kDefault ? FencedFrameCreationOutcome::kSuccessDefault
    ...
    if (content_size.has_value()) {
        ...
        CHECK(deprecated_should_freeze_initial_size.has_value() && !deprecated_should_freeze_initial_size.value());
        PhysicalSize converted_size(LayoutUnit(content_size->width()), LayoutUnit(content_size->height()));
        FreezeFrameSize(converted_size, /*should_coerce_size=*/false);
    } else {
        if ((!deprecated_should_freeze_initial_size.has_value() && IsValidUrnUuidURL(GURL(url)))
            || (deprecated_should_freeze_initial_size.has_value() && *deprecated_should_freeze_initial_size)) {
...
void HTMLFencedFrameElement::NavigateToConfig()
{
    ...
    if (config_->urn_uuid(PassKey())) {
        ...
        CHECK(config_->url());
        url = config_->GetValueIgnoringVisibility<FencedFrameConfig::Attribute::kURL>();
    }
    Navigate(url, config_->deprecated_should_freeze_initial_size(PassKey()), config_->container_size(PassKey()), config_->content_size(PassKey()),
        config_->GetSharedStorageContext());
}

void HTMLFencedFrameElement::CreateDelegateAndNavigate()
{
    TRACE_EVENT0("navigation", "HTMLFencedFrameElement::CreateDelegateAndNavigate");
    ...
    if (GetDocument().IsPrerendering()) {
        GetDocument().AddPostPrerenderingActivationStep(WTF::BindOnce(&HTMLFencedFrameElement::CreateDelegateAndNavigate, WrapWeakPersistent(this)));
        return;
    }
...
bool HTMLFencedFrameElement::LayoutObjectIsNeeded(const DisplayStyle& style) const
{
    return !collapsed_by_client_ && HTMLFrameOwnerElement::LayoutObjectIsNeeded(style);
}
...
{
    return (frame_delegate_ && frame_delegate_->SupportsFocus()) ? FocusableState::kFocusable : FocusableState::kNotFocusable;
}

PhysicalSize HTMLFencedFrameElement::CoerceFrameSize(const PhysicalSize& requested_size)
{
    ...
    // #include "third_party/blink/renderer/core/frame/local_dom_window.h"
    // #include "third_party/blink/renderer/core/frame/screen.h"
    if (GetDeprecatedMode() != blink::FencedFrame::DeprecatedFencedFrameMode::kOpaqueAds || GetDocument().GetFrame()->IsInFencedFrameTree()) {
        return requested_size;
    }

    // If the requested size is degenerate, return the first allowed ad size.
    if (requested_size.width.ToDouble() < std::numeric_limits<double>::epsilon() || requested_size.height.ToDouble() < std::numeric_limits<double>::epsilon()) {
    ...
    int width_for_scaling = document.domWindow() && document.domWindow()->screen() ? document.domWindow()->screen()->availWidth() : 0;
    ...
    if (width_for_scaling > 0) {
        ...
        for (const int allowed_height : kAllowedAdHeights) {
            if (SizeMatchesExactly(requested_size, { width_for_scaling, allowed_height })) {
                return requested_size;
            }
        }
        ...
        for (const gfx::Size& allowed_aspect_ratio : kAllowedAdAspectRatios) {
            if (SizeMatchesExactly(requested_size, { width_for_scaling, (width_for_scaling * allowed_aspect_ratio.height()) / allowed_aspect_ratio.width() })) {
                return requested_size;
            }
        }
    }
    ...
    if (width_for_scaling > 0) {
        ...
        for (const gfx::Size& allowed_aspect_ratio : kAllowedAdAspectRatios) {
            const gfx::Size allowed_size = { width_for_scaling, (width_for_scaling * allowed_aspect_ratio.height()) / allowed_aspect_ratio.width() };
            double size_loss = ComputeSizeLossFunction(requested_size, allowed_size);
            if (size_loss < best_size_loss) {
                best_size_loss = size_loss;
                best_size = allowed_size;
...
const std::optional<PhysicalSize> HTMLFencedFrameElement::FrozenFrameSize() const
{
    if (!frozen_frame_size_)
        return std::nullopt;
    const float ratio = GetDocument().DevicePixelRatio();
    return PhysicalSize(LayoutUnit::FromFloatRound(frozen_frame_size_->width * ratio), LayoutUnit::FromFloatRound(frozen_frame_size_->height * ratio));
}
...
void HTMLFencedFrameElement::FreezeCurrentFrameSize()
{
    ...
    if (frozen_frame_size_.has_value() && content_rect_.has_value() && content_rect_->size == *frozen_frame_size_) {
        return;
    }
...
{
    setAttribute(html_names::kWidthAttr, AtomicString(String::Format("%dpx", size.width())));
    setAttribute(html_names::kHeightAttr, AtomicString(String::Format("%dpx", size.height())));

    frame_delegate_->MarkContainerSizeStale();
}

void HTMLFencedFrameElement::FreezeFrameSize(const PhysicalSize& size, bool should_coerce_size)
{
...
void HTMLFencedFrameElement::StartResizeObserver()
{
    ...
    resize_observer_ = ResizeObserver::Create(GetDocument().domWindow(), MakeGarbageCollected<ResizeObserverDelegate>());
    resize_observer_->observe(this);
}

void HTMLFencedFrameElement::ResizeObserverDelegate::OnResize(const HeapVector<Member<ResizeObserverEntry>>& entries)
{
    if (entries.empty())
        return;
...
void HTMLFencedFrameElement::DispatchFencedEvent(const WTF::String& event_type)
{
    std::optional<WTF::AtomicString> fenced_event_type = ConvertEventTypeToFencedEventType(event_type);
...
// static
HTMLFencedFrameElement::FencedFrameDelegate* HTMLFencedFrameElement::FencedFrameDelegate::Create(HTMLFencedFrameElement* outer_element)
{
    DCHECK(RuntimeEnabledFeatures::FencedFramesEnabled(outer_element->GetExecutionContext()));
    ...
    if (!outer_element->GetExecutionContext()) {
        outer_element->GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(
            mojom::blink::ConsoleMessageSource::kJavaScript, mojom::blink::ConsoleMessageLevel::kWarning, "Can't create a fenced frame in a detached frame."));
        return nullptr;
    }
    ...
    if (!outer_element->isConnected()) {
        outer_element->GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(
            mojom::blink::ConsoleMessageSource::kJavaScript, mojom::blink::ConsoleMessageLevel::kWarning, "Can't create a fenced frame when disconnected."));
        return nullptr;
    }

    if (outer_element->GetExecutionContext()->IsSandboxed(kFencedFrameMandatoryUnsandboxedFlags)) {
        outer_element->GetDocument().AddConsoleMessage(
            MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kJavaScript, mojom::blink::ConsoleMessageLevel::kWarning,
                ...
                "allow-top-navigation-by-user-activation."));
        RecordFencedFrameCreationOutcome(FencedFrameCreationOutcome::kSandboxFlagsNotSet);
        RecordFencedFrameUnsandboxedFlags(outer_element->GetExecutionContext()->GetSandboxFlags());
        RecordFencedFrameFailedSandboxLoadInTopLevelFrame(outer_element->GetDocument().IsInMainFrame());
        return nullptr;
    }

    if (!SubframeLoadingDisabler::CanLoadFrame(*outer_element)) {
        outer_element->GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kJavaScript,
            mojom::blink::ConsoleMessageLevel::kWarning, "Can't create a fenced frame. Subframe loading disabled."));
        return nullptr;
    }
    ...
    if (!outer_element->IsCurrentlyWithinFrameLimit()) {
        outer_element->GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kJavaScript,
            mojom::blink::ConsoleMessageLevel::kWarning, "Can't create a fenced frame. Frame limit exceeded."));
        return nullptr;
    }
...
HTMLFencedFrameElement::FencedFrameDelegate::FencedFrameDelegate(HTMLFencedFrameElement* outer_element)
    : outer_element_(outer_element)
    , remote_(GetElement().GetDocument().GetExecutionContext())
{
    DocumentFencedFrames::GetOrCreate(GetElement().GetDocument()).RegisterFencedFrame(&GetElement());
    mojo::PendingAssociatedRemote<mojom::blink::FencedFrameOwnerHost> remote;
    mojo::PendingAssociatedReceiver<mojom::blink::FencedFrameOwnerHost> receiver = remote.InitWithNewEndpointAndPassReceiver();
    auto task_runner = GetElement().GetDocument().GetTaskRunner(TaskType::kInternalDefault);
    remote_.Bind(std::move(remote), task_runner);

    RemoteFrame* remote_frame = GetElement().GetDocument().GetFrame()->Client()->CreateFencedFrame(&GetElement(), std::move(receiver));
    DCHECK_EQ(remote_frame, GetElement().ContentFrame());
}

void HTMLFencedFrameElement::FencedFrameDelegate::Navigate(const KURL& url, const String& embedder_shared_storage_context)
{
    DCHECK(remote_.get());
    const auto navigation_start_time = base::TimeTicks::Now();
    remote_->Navigate(url, navigation_start_time, embedder_shared_storage_context);
}
...
{
    RemoteFrameView* view = DynamicTo<RemoteFrameView>(GetElement().OwnedEmbeddedContentView());
    if (view) {
        view->ResetFrozenSize();
    }
    if (auto* layout_object = GetElement().GetLayoutObject()) {
        layout_object->SetNeedsLayoutAndFullPaintInvalidation("Froze fenced frame content size");
    }
}
...
{
    if (auto* layout_object = GetElement().GetLayoutObject()) {
        layout_object->SetNeedsLayoutAndFullPaintInvalidation("Stored fenced frame container size");
    }
}

void HTMLFencedFrameElement::FencedFrameDelegate::DidChangeFramePolicy(const FramePolicy& frame_policy)
{
    DCHECK(remote_.get());
    remote_->DidChangeFramePolicy(frame_policy);
}

void HTMLFencedFrameElement::FencedFrameDelegate::Trace(Visitor* visitor) const
{
