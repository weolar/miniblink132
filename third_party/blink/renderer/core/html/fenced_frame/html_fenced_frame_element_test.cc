...
class HTMLFencedFrameElementTest : private ScopedFencedFramesForTest, public RenderingTest {
public:
    HTMLFencedFrameElementTest()
        : ScopedFencedFramesForTest(true)
        , RenderingTest(MakeGarbageCollected<SingleChildLocalFrameClient>())
    {
        enabled_feature_list_.InitWithFeaturesAndParameters({ { blink::features::kFencedFrames, {} } }, { /* disabled_features */ });
    }
...
protected:
    ...
    {
        RenderingTest::SetUp();
        SecurityContext& security_context = GetDocument().GetFrame()->DomWindow()->GetSecurityContext();
        security_context.SetSecurityOriginForTesting(nullptr);
        security_context.SetSecurityOrigin(SecurityOrigin::CreateFromString("https://fencedframedelegate.test"));
        EXPECT_EQ(security_context.GetSecureContextMode(), SecureContextMode::kSecureContext);
    }
...
TEST_F(HTMLFencedFrameElementTest, FreezeSizeLayoutZoomFactor)
{
    ...
    EXPECT_EQ(*fenced_frame->FrozenFrameSize(), PhysicalSize(size.width * 2, size.height * 2));
...
TEST_F(HTMLFencedFrameElementTest, CoerceFrameSizeTest)
{
    Document& doc = GetDocument();
    auto* fenced_frame = MakeGarbageCollected<HTMLFencedFrameElement>(doc);
    fenced_frame->mode_ = blink::FencedFrame::DeprecatedFencedFrameMode::kOpaqueAds;
    doc.body()->AppendChild(fenced_frame);
    ...
    for (const gfx::Size& allowed_size : kAllowedAdSizes) {
        ...
        const PhysicalSize coerced_size = fenced_frame->CoerceFrameSize(requested_size);
        EXPECT_EQ(requested_size, coerced_size);
    }

    // Check that all of the coercion calls were logged properly.
    histogram_tester_.ExpectBucketCount(kIsOpaqueFencedFrameSizeCoercedHistogram, 0, kAllowedAdSizes.size());
    ...
    auto IsAllowedSize = [](const PhysicalSize coerced_size, int screen_width) {
        ...
        for (const gfx::Size& allowed_aspect_ratio : kAllowedAdAspectRatios) {
            if (coerced_size == PhysicalSize(screen_width, (screen_width * allowed_aspect_ratio.height()) / allowed_aspect_ratio.width())) {
                return true;
            }
        }
#endif
    ...
    std::vector<PhysicalSize> test_cases = {
        ...
        PhysicalSize(LayoutUnit(std::numeric_limits<double>::infinity()), LayoutUnit(std::numeric_limits<double>::infinity())),
        PhysicalSize(LayoutUnit(std::numeric_limits<double>::quiet_NaN()), LayoutUnit(std::numeric_limits<double>::quiet_NaN())),
        PhysicalSize(LayoutUnit(std::numeric_limits<double>::signaling_NaN()), LayoutUnit(std::numeric_limits<double>::signaling_NaN())),
        PhysicalSize(LayoutUnit(std::numeric_limits<double>::denorm_min()), LayoutUnit(std::numeric_limits<double>::denorm_min())),
    };
    ...
    for (const PhysicalSize& requested_size : test_cases) {
        ...
        if (!(coerced_size == requested_size) && requested_size.width.ToDouble() > 0 && requested_size.height.ToDouble() > 0) {
    ...
    // to be logged.
    histogram_tester_.ExpectBucketCount(kIsOpaqueFencedFrameSizeCoercedHistogram, 1, expected_coercion_count);
}
...
TEST_F(HTMLFencedFrameElementTest, HistogramTestInsecureContext)
{
    ...
    SecurityContext& security_context = doc.GetFrame()->DomWindow()->GetSecurityContext();
    security_context.SetSecurityOriginForTesting(nullptr);
    security_context.SetSecurityOrigin(SecurityOrigin::CreateFromString("http://insecure_top_level.test"));

    auto* fenced_frame = MakeGarbageCollected<HTMLFencedFrameElement>(doc);
    fenced_frame->setConfig(FencedFrameConfig::Create(String("https://example.com/")));
    doc.body()->AppendChild(fenced_frame);

    histogram_tester_.ExpectUniqueSample(kFencedFrameCreationOrNavigationOutcomeHistogram, FencedFrameCreationOutcome::kInsecureContext, 1);
}
...
TEST_F(HTMLFencedFrameElementTest, HistogramTestIncompatibleUrlHTTPDefault)
{
    ...
    histogram_tester_.ExpectUniqueSample(
        kFencedFrameCreationOrNavigationOutcomeHistogram, FencedFrameCreationOutcome::kIncompatibleURLDefault, test_cases.size());
}
...
TEST_F(HTMLFencedFrameElementTest, HistogramTestSandboxFlags)
{
    ...
    doc.GetFrame()->DomWindow()->GetSecurityContext().SetSandboxFlags(WebSandboxFlags::kAll);

    auto* fenced_frame = MakeGarbageCollected<HTMLFencedFrameElement>(doc);
    fenced_frame->SetAttributeWithValidation(html_names::kSrcAttr, AtomicString("https://test.com/"), ASSERT_NO_EXCEPTION);
    doc.body()->AppendChild(fenced_frame);
    histogram_tester_.ExpectUniqueSample(kFencedFrameCreationOrNavigationOutcomeHistogram, FencedFrameCreationOutcome::kSandboxFlagsNotSet, 1);

    // Test that only the offending sandbox flags are being logged.
    for (int32_t i = 1; i <= static_cast<int32_t>(WebSandboxFlags::kMaxValue); i = i << 1) {
        WebSandboxFlags current_mask = static_cast<WebSandboxFlags>(i);
        histogram_tester_.ExpectBucketCount(
            kFencedFrameMandatoryUnsandboxedFlagsSandboxed, i, (kFencedFrameMandatoryUnsandboxedFlags & current_mask) != WebSandboxFlags::kNone ? 1 : 0);
    }

    // Test that it logged that the fenced frame creation attempt was in the
    // outermost main frame.
    histogram_tester_.ExpectUniqueSample(kFencedFrameFailedSandboxLoadInTopLevelFrame, true, 1);
}
...
TEST_F(HTMLFencedFrameElementTest, HistogramTestSandboxFlagsInIframe)
{
    ...
    iframe->SetAttributeWithValidation(html_names::kSrcAttr, AtomicString("https://test.com/"), ASSERT_NO_EXCEPTION);
    doc.body()->AppendChild(iframe);
    Document* iframe_doc = iframe->contentDocument();
    iframe_doc->GetFrame()->DomWindow()->GetSecurityContext().SetSandboxFlags(network::mojom::blink::WebSandboxFlags::kAll);

    // Create fenced frame and embed it in the main frame
    auto* fenced_frame = MakeGarbageCollected<HTMLFencedFrameElement>(*iframe_doc);
    fenced_frame->SetAttributeWithValidation(html_names::kSrcAttr, AtomicString("https://test.com/"), ASSERT_NO_EXCEPTION);
    iframe_doc->body()->AppendChild(fenced_frame);

    // Test that it logged that the fenced frame creation attempt was NOT in the
    // outermost main frame.
    histogram_tester_.ExpectUniqueSample(kFencedFrameFailedSandboxLoadInTopLevelFrame, false, 1);
}

TEST_F(HTMLFencedFrameElementTest, HistogramTestCanLoadOpaqueURL)
{
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kFencedFrameCanLoadOpaqueURL));
    ...
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kFencedFrameCanLoadOpaqueURL));
}
