...
public:
    ...
    class CORE_EXPORT FencedFrameDelegate : public GarbageCollected<FencedFrameDelegate> {
    public:
    ...
    protected:
        HTMLFencedFrameElement& GetElement() const
        {
            return *outer_element_;
        }
    ...
    bool IsHTMLFencedFrameElement() const final
    {
        return true;
    }
...
private:
    ...
    void Navigate(const KURL& url, std::optional<bool> deprecated_should_freeze_initial_size = std::nullopt,
        std::optional<gfx::Size> container_size = std::nullopt, std::optional<gfx::Size> content_size = std::nullopt,
    ...
    blink::FencedFrame::DeprecatedFencedFrameMode mode_ = blink::FencedFrame::DeprecatedFencedFrameMode::kDefault;
    ...
    FRIEND_TEST_ALL_PREFIXES(HTMLFencedFrameElementTest, FreezeSizeLayoutZoomFactor);
    FRIEND_TEST_ALL_PREFIXES(HTMLFencedFrameElementTest, CoerceFrameSizeTest);
    FRIEND_TEST_ALL_PREFIXES(HTMLFencedFrameElementTest, HistogramTestResizeAfterFreeze);
};
...
// over.
template <> struct DowncastTraits<HTMLFencedFrameElement> {
