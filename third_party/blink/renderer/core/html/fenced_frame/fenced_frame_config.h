...
public:
    ...
    static FencedFrameConfig* Create(const KURL url, const String& shared_storage_context, std::optional<KURL> urn_uuid,
        std::optional<gfx::Size> container_size, std::optional<gfx::Size> content_size, AttributeVisibility url_visibility, bool freeze_initial_size);

    static FencedFrameConfig* From(const FencedFrame::RedactedFencedFrameConfig& config);

    // Construct an inner config with a given url, the url will be transparent.
    explicit FencedFrameConfig(const String& url);

    explicit FencedFrameConfig(const KURL url, const String& shared_storage_context, std::optional<KURL> urn_uuid, std::optional<gfx::Size> container_size,
        std::optional<gfx::Size> content_size, AttributeVisibility url_visibility, bool freeze_initial_size);

    // Construct an inner config given a redacted fenced frame config
    explicit FencedFrameConfig(const FencedFrame::RedactedFencedFrameConfig& config);
    ...
    template <Attribute attr> auto GetValueIgnoringVisibility() const
    {
    ...
    std::optional<gfx::Size> container_size(base::PassKey<HTMLFencedFrameElement>)
    {
        return container_size_;
    }

    std::optional<gfx::Size> container_size(base::PassKey<V8ScriptValueSerializer>)
    {
        return container_size_;
    }
    ...
    std::optional<gfx::Size> content_size(base::PassKey<V8ScriptValueSerializer>)
    {
        return content_size_;
    }

    bool deprecated_should_freeze_initial_size(base::PassKey<HTMLFencedFrameElement>)
    {
        return deprecated_should_freeze_initial_size_;
    }

    bool deprecated_should_freeze_initial_size(base::PassKey<V8ScriptValueSerializer>)
    {
        return deprecated_should_freeze_initial_size_;
    }

    // Get attribute's visibility.
    template <Attribute attr> AttributeVisibility GetAttributeVisibility(base::PassKey<V8ScriptValueSerializer>) const
    {
...
private:
    ...
    template <typename T> struct AttributeUnion;

    // Get attribute's visibility.
    template <Attribute attr> AttributeVisibility GetAttributeVisibility() const
    {
        switch (attr) {
        case Attribute::kURL:
    ...
    template <Attribute attr> auto GetValue() const
    {
        if constexpr (attr == Attribute::kURL) {
    ...
    template <Attribute attr> auto Get() const -> typename AttributeUnion<decltype(GetValue<attr>())>::Type*
    {
        ...
        case AttributeVisibility::kOpaque:
            return MakeGarbageCollected<UnionType>(V8OpaqueProperty(V8OpaqueProperty::Enum::kOpaque));
...
- Modify the V8ScriptValueSerializerTest.RoundTripFencedFrameConfig test to
  check that your new field is deserialized/serialized properly.
    ...
    FRIEND_TEST_ALL_PREFIXES(V8ScriptValueSerializerTest, RoundTripFencedFrameConfig);
    FRIEND_TEST_ALL_PREFIXES(V8ScriptValueSerializerTest, RoundTripFencedFrameConfigNullValues);
};

template <> struct FencedFrameConfig::AttributeUnion<AtomicString> {
    using Type = V8UnionOpaquePropertyOrUSVString;
};
template <> struct FencedFrameConfig::AttributeUnion<uint32_t> {
