...
public:
    ...
    {
        scoped_feature_list_.InitWithFeatures({ { blink::features::kFencedFrames }, { blink::features::kPrivateAggregationApi } },
            /*disabled_features=*/ {});
    }
...
TEST_F(FenceTest, ReportPrivateAggregationEvent)
{
    ...
    Fence* fence = MakeGarbageCollected<Fence>(*(GetDocument().GetFrame()->DomWindow()));
    ...
    EXPECT_EQ(ConsoleMessages().front(), "This frame did not register reporting metadata.");
}
...
TEST_F(FenceTest, ReportPrivateAggregationReservedEvent)
{
    ...
    V8TestingScope scope(base_url);
    Fence* fence = MakeGarbageCollected<Fence>(*(GetDocument().GetFrame()->DomWindow()));
    fence->reportPrivateAggregationEvent("reserved.event", scope.GetExceptionState());

    // There should be a "Reserved events cannot be triggered manually." console
    // warning.
    EXPECT_EQ(ConsoleMessages().size(), 1u);
    EXPECT_EQ(ConsoleMessages().front(), "Reserved events cannot be triggered manually.");
}
...
TEST_F(FenceTest, ReportReservedEvent)
{
    ...
    V8UnionFenceEventOrString* event_union = MakeGarbageCollected<V8UnionFenceEventOrString>(event);
