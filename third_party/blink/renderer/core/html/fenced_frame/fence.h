...
class CORE_EXPORT Fence final : public ScriptWrappable, public ExecutionContextClient {
...
public:
    ...
    void reportEvent(const V8UnionFenceEventOrString* event, ExceptionState& exception_state);
    ...
    void setReportEventDataForAutomaticBeacons(const FenceEvent* event, ExceptionState& exception_state);
    ...
    HeapVector<Member<FencedFrameConfig>> getNestedConfigs(ExceptionState& exception_state);

    ScriptPromise<IDLUndefined> disableUntrustedNetwork(ScriptState* script_state, ExceptionState& exception_state);
    ...
    void notifyEvent(const Event* triggering_event, ExceptionState& exception_state);
...
private:
    ...
    // `destination`.
    void reportEventToDestinationEnum(const FenceEvent* event, ExceptionState& exception_state);

    // Sends a report to `destinationURL`, with substitution of buyer macros.
    void reportEventToDestinationURL(const FenceEvent* event, ExceptionState& exception_state);
    ...
    void reportPrivateAggregationEvent(const String& event, ExceptionState& exception_state);

    void AddConsoleMessage(const String& message, mojom::blink::ConsoleMessageLevel level = mojom::blink::ConsoleMessageLevel::kError);
