...
const char kWarningUnmuteFailed[] = "Unmuting failed and the element was paused instead because the user "
                                    "didn't interact with the document before. https://goo.gl/xX8pDD";
const char kErrorAutoplayFuncUnified[] = "play() failed because the user didn't interact with the document first. "
                                         "https://goo.gl/xX8pDD";
const char kErrorAutoplayFuncMobile[] = "play() can only be initiated by a user gesture.";
...
// static
AutoplayPolicy::Type AutoplayPolicy::GetAutoplayPolicyForDocument(const Document& document)
{
...
bool AutoplayPolicy::IsDocumentAllowedToPlay(const Document& document)
{
    ...
    bool permissions_policy_enabled = document.GetExecutionContext()->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kAutoplay);

    for (Frame* frame = document.GetFrame(); frame; frame = frame->Tree().Parent()) {
        if (frame->HasStickyUserActivation() || frame->HadStickyUserActivationBeforeNavigation()) {
            return true;
        }

        if (RuntimeEnabledFeatures::MediaEngagementBypassAutoplayPoliciesEnabled() && frame->IsOutermostMainFrame()
...
bool AutoplayPolicy::DocumentHasHighMediaEngagement(const Document& document)
{
    if (!document.GetPage())
        return false;
    return document.GetPage()->AutoplayFlags() & mojom::blink::kAutoplayFlagHighMediaEngagement;
}
...
bool AutoplayPolicy::DocumentHasForceAllowFlag(const Document& document)
{
    if (!document.GetPage())
        return false;
    return document.GetPage()->AutoplayFlags() & mojom::blink::kAutoplayFlagForceAllow;
}
...
bool AutoplayPolicy::DocumentHasUserExceptionFlag(const Document& document)
{
    if (!document.GetPage())
        return false;
    return document.GetPage()->AutoplayFlags() & mojom::blink::kAutoplayFlagUserException;
}

// static
bool AutoplayPolicy::DocumentShouldAutoplayMutedVideos(const Document& document)
{
    return GetAutoplayPolicyForDocument(document) != AutoplayPolicy::Type::kNoUserGestureRequired;
}
...
{
    locked_pending_user_gesture_ = ComputeLockPendingUserGestureRequired(element->GetDocument());
}
...
void AutoplayPolicy::DidMoveToNewDocument(Document& old_document)
{
    ...
    bool old_document_requires_user_gesture = ComputeLockPendingUserGestureRequired(old_document);
    bool new_document_requires_user_gesture = ComputeLockPendingUserGestureRequired(element_->GetDocument());
...
bool AutoplayPolicy::IsEligibleForAutoplayMuted() const
{
    ...
    if (RuntimeEnabledFeatures::VideoAutoFullscreenEnabled() && !element_->FastHasAttribute(html_names::kPlaysinlineAttr)) {
        return false;
    }

    return !element_->EffectiveMediaVolume() && DocumentShouldAutoplayMutedVideos(element_->GetDocument());
}
...
void AutoplayPolicy::StartAutoplayMutedWhenVisible()
{
    ...
    autoplay_intersection_observer_
        = IntersectionObserver::Create(element_->GetDocument(), WTF::BindRepeating(&AutoplayPolicy::OnIntersectionChangedForAutoplay, WrapWeakPersistent(this)),
        ...
        if (IsGestureNeededForPlayback()) {
            if (IsUsingDocumentUserActivationRequiredPolicy()) {
                element_->GetDocument().AddConsoleMessage(
                    MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kJavaScript, mojom::ConsoleMessageLevel::kWarning, kWarningUnmuteFailed));
            }

            autoplay_uma_helper_->RecordAutoplayUnmuteStatus(AutoplayUnmuteActionStatus::kFailure);
            return false;
        }
        autoplay_uma_helper_->RecordAutoplayUnmuteStatus(AutoplayUnmuteActionStatus::kSuccess);
    }
    return true;
}
...
bool AutoplayPolicy::HasTransientUserActivation() const
{
    ...
    if (opener && opener->IsLocalFrame() && LocalFrame::HasTransientUserActivation(To<LocalFrame>(opener))) {
        return true;
    }
...
{
    if (RuntimeEnabledFeatures::MediaPlaybackWhileNotVisiblePermissionPolicyEnabled() && !CanPlayWhileHidden() && IsFrameHidden()) {
...
{
    if (!IsA<HTMLVideoElement>(element_.Get()) || !DocumentShouldAutoplayMutedVideos(element_->GetDocument())) {
        return false;
    }
...
{
    if (IsLockedPendingUserGesture() && LocalFrame::HasTransientUserActivation(element_->GetDocument().GetFrame())) {
        locked_pending_user_gesture_ = false;
    }
}
...
bool AutoplayPolicy::CanPlayWhileHidden() const
{
    return element_->GetExecutionContext()
        && element_->GetExecutionContext()->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kMediaPlaybackWhileNotVisible);
}
...
bool AutoplayPolicy::IsFrameHidden() const
{
    ...
    return frame
        && (frame->View()->GetFrameVisibility().value_or(mojom::blink::FrameVisibility::kRenderedInViewport) == mojom::blink::FrameVisibility::kNotRendered);
}

String AutoplayPolicy::GetPlayErrorMessage() const
{
    return IsUsingDocumentUserActivationRequiredPolicy() ? kErrorAutoplayFuncUnified : kErrorAutoplayFuncMobile;
}
...
void AutoplayPolicy::OnIntersectionChangedForAutoplay(const HeapVector<Member<IntersectionObserverEntry>>& entries)
{
    ...
    if (!is_visible) {
        auto pause_and_preserve_autoplay = [](AutoplayPolicy* self) {
            ...
            if (self->element_->can_autoplay_ && self->element_->Autoplay()) {
                self->element_->PauseInternal(HTMLMediaElement::PlayPromiseError::kPaused_AutoplayAutoPause);
        ...
        element_->GetDocument()
            ...
            ->PostTask(FROM_HERE, WTF::BindOnce(pause_and_preserve_autoplay, WrapWeakPersistent(this)));
        return;
    }
    ...
    element_->GetDocument().GetTaskRunner(TaskType::kInternalMedia)->PostTask(FROM_HERE, WTF::BindOnce(maybe_autoplay, WrapWeakPersistent(this)));
}

bool AutoplayPolicy::IsUsingDocumentUserActivationRequiredPolicy() const
{
    return GetAutoplayPolicyForDocument(element_->GetDocument()) == AutoplayPolicy::Type::kDocumentUserActivationRequired;
}
...
void AutoplayPolicy::MaybeSetAutoplayInitiated()
{
    ...
    bool permissions_policy_enabled
        = element_->GetExecutionContext() && element_->GetExecutionContext()->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kAutoplay);

    for (Frame* frame = element_->GetDocument().GetFrame(); frame; frame = frame->Tree().Parent()) {
        if (frame->HasStickyUserActivation() || frame->HadStickyUserActivationBeforeNavigation()) {
            autoplay_initiated_ = false;
            break;
        }
...
{
    if (!element_->GetExecutionContext() || element_->GetExecutionContext()->IsSandboxed(network::mojom::blink::WebSandboxFlags::kAutomaticFeatures)) {
        return false;
    }
...
STATIC_ASSERT_ENUM(mojom::blink::AutoplayPolicy::kNoUserGestureRequired, AutoplayPolicy::Type::kNoUserGestureRequired);
STATIC_ASSERT_ENUM(mojom::blink::AutoplayPolicy::kUserGestureRequired, AutoplayPolicy::Type::kUserGestureRequired);
STATIC_ASSERT_ENUM(mojom::blink::AutoplayPolicy::kDocumentUserActivationRequired, AutoplayPolicy::Type::kDocumentUserActivationRequired);
