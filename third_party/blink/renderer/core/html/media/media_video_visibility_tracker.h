...
// MediaControls.
class CORE_EXPORT MediaVideoVisibilityTracker final : public NativeEventListener, public LocalFrameView::LifecycleNotificationObserver {
public:
    ...
    static constexpr base::TimeDelta kMinimumAllowedHitTestInterval = base::Milliseconds(500);
    ...
    MediaVideoVisibilityTracker(HTMLVideoElement& video, int visibility_threshold, ReportVisibilityCb report_visibility_cb,
...
private:
    ...
    // the screen.
    const ClientIdsSet GetClientIdsSet(DisplayItemClientId start_after_display_item_client_id) const;

    ListBasedHitTestBehavior ComputeOcclusion(const ClientIdsSet& client_ids_set, Metrics&, const Node& node);
