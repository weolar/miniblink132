...
AtomicString SrcSchemeToURL(TestURLScheme scheme)
{
    ...
    case TestURLScheme::kBlob:
        return AtomicString("blob:http://example.com/00000000-0000-0000-0000-000000000000");
    default:
        NOTREACHED();
    }
}
...
public:
    ...
    MOCK_METHOD1(SetWasPlayedWithUserActivationAndHighMediaEngagement, void(bool));
    ...
    MOCK_METHOD4(Load, WebMediaPlayer::LoadTiming(LoadType load_type, const blink::WebMediaPlayerSource& source, CorsMode cors_mode, bool is_cache_disabled));
...
public:
    ...
    void EnterFullscreen(LocalFrame& frame, const FullscreenOptions*, FullscreenRequestType) override
    {
        Fullscreen::DidResolveEnterFullscreenRequest(*frame.GetDocument(), true /* granted */);
    }
...
public:
    ...
    HTMLMediaElement::RequestVisibilityCallback VisibilityCallback()
    {
        ...
        // RunLoop::Run() returns.
        return base::BindOnce(&RequestVisibilityWaiter::RequestVisibility, base::Unretained(this));
    }
...
class TestMediaPlayerObserver final : public media::mojom::blink::MediaPlayerObserver {
public:
    ...
    void OnMediaMetadataChanged(bool has_audio, bool has_video, media::MediaContentType content_type) override
    {
        ...
        // content_type};
        received_metadata_changed_result_ = OnMetadataChangedResult { has_audio, has_video, content_type };
        run_loop_->Quit();
    }

    void OnMediaPositionStateChanged(::media_session::mojom::blink::MediaPositionPtr) override
    {
    }
    ...
    bool received_media_playing() const
    {
        return received_media_playing_;
    }
    ...
    const std::optional<OnMetadataChangedResult>& received_metadata_changed_result() const
    {
        return received_metadata_changed_result_;
    }

    gfx::Size received_media_size() const
    {
        return received_media_size_;
    }
    ...
    bool received_remote_playback_metadata(media_session::mojom::blink::RemotePlaybackMetadataPtr remote_playback_metadata) const
    {
...
private:
    ...
    media_session::mojom::blink::RemotePlaybackMetadataPtr received_remote_playback_metadata_;
};
...
public:
    ...
    void OnMediaPlayerAdded(mojo::PendingAssociatedRemote<media::mojom::blink::MediaPlayer>
        /*media_player*/,
        mojo::PendingAssociatedReceiver<media::mojom::blink::MediaPlayerObserver> media_player_observer, int32_t /*player_id*/) override
    {
        receiver_.Bind(std::move(media_player_observer));
        run_loop_.Quit();
    }

    TestMediaPlayerObserver& observer()
    {
        return observer_;
    }
...
private:
    ...
    mojo::AssociatedReceiver<media::mojom::blink::MediaPlayerObserver> receiver_ { &observer_ };
...
protected:
    void SetUp() override
    {
        ...
        EXPECT_CALL(*mock_media_player, Seekable()).WillRepeatedly(Return(WebTimeRanges()));
        ...
        EXPECT_CALL(*mock_media_player, Load(_, _, _, _)).Times(AnyNumber()).WillRepeatedly(Return(WebMediaPlayer::LoadTiming::kImmediate));
        EXPECT_CALL(*mock_media_player, DidLazyLoad).WillRepeatedly(Return(false));
        EXPECT_CALL(*mock_media_player, WouldTaintOrigin).WillRepeatedly(Return(true));
        EXPECT_CALL(*mock_media_player, GetNetworkState).WillRepeatedly(Return(WebMediaPlayer::kNetworkStateIdle));
        ...
        dummy_page_holder_
            = std::make_unique<DummyPageHolder>(gfx::Size(), chrome_client_, MakeGarbageCollected<WebMediaStubLocalFrameClient>(std::move(mock_media_player)));

        if (GetParam() == MediaTestParam::kAudio) {
            media_ = MakeGarbageCollected<HTMLAudioElement>(dummy_page_holder_->GetDocument());
        } else {
            media_ = MakeGarbageCollected<HTMLVideoElement>(dummy_page_holder_->GetDocument());
        }

        media_->SetMediaPlayerHostForTesting(media_player_host_receiver_.BindNewEndpointAndPassDedicatedRemote());
    ...
    {
        KURL url(src);
        Media()->current_src_.SetSource(url, HTMLMediaElement::SourceMetadata::SourceVisibility::kVisibleToApp);
    }

    MockWebMediaPlayer* MockMediaPlayer()
    {
        return media_player_;
    }

    bool WasAutoplayInitiated()
    {
        return Media()->WasAutoplayInitiated();
    }

    bool CouldPlayIfEnoughData()
    {
        return Media()->CouldPlayIfEnoughData();
    }

    bool PotentiallyPlaying()
    {
        return Media()->PotentiallyPlaying();
    }

    bool ShouldDelayLoadEvent()
    {
    ...
    {
        return Media()->playing_;
    }

    void ResetWebMediaPlayer() const
    {
        Media()->web_media_player_.reset();
    }
    ...
    {
        return Media()->MediaShouldBeOpaque();
    }

    void SetError(MediaError* err)
    {
        Media()->MediaEngineError(err);
    }

    void SimulateHighMediaEngagement()
    {
        Media()->GetDocument().GetPage()->AddAutoplayFlags(mojom::blink::kAutoplayFlagHighMediaEngagement);
    }
    ...
    {
        return Media()->ShouldShowControls();
    }
    ...
    MediaVideoVisibilityTracker::TrackerAttachedToDocument VideoVisibilityTrackerAttachedToDocument(HTMLVideoElement* video) const
    {
        DCHECK(video->visibility_tracker_for_tests());
        return video->visibility_tracker_for_tests()->tracker_attached_to_document_;
    }

    void RequestVisibility(HTMLMediaElement::RequestVisibilityCallback request_visibility_callback) const
    {
...
protected:
    ...
    {
        return media_player_observer().received_media_paused_stream_ended() == stream_ended;
    }
    ...
    void NotifyMediaMetadataChanged(bool has_audio, bool has_video, media::AudioCodec audio_codec, media::VideoCodec video_codec,
        media::MediaContentType media_content_type, bool is_encrypted_media)
    {
        media_->DidMediaMetadataChange(has_audio, has_video, audio_codec, video_codec, media_content_type, is_encrypted_media);
        media_player_observer().WaitUntilReceivedMessage();
        // wait for OnRemotePlaybackMetadataChange() to be called.
        media_player_observer().WaitUntilReceivedMessage();
    }

    bool ReceivedMessageMediaMetadataChanged(bool has_audio, bool has_video, media::MediaContentType media_content_type)
    {
        const auto& result = media_player_observer().received_metadata_changed_result();
        return result->has_audio == has_audio && result->has_video == has_video && result->media_content_type == media_content_type;
    }
    ...
    {
        return media_player_observer().received_use_audio_service_changed(uses_audio_service);
    }
    ...
    bool ReceivedRemotePlaybackMetadataChange(media_session::mojom::blink::RemotePlaybackMetadataPtr remote_playback_metadata)
    {
        return media_player_observer().received_remote_playback_metadata(std::move(remote_playback_metadata));
    }

    bool WasPlayerDestroyed() const
    {
        return !media_player_weak_;
    }

    // Create a dummy page holder with the given security origin.
    std::unique_ptr<DummyPageHolder> CreatePageWithSecurityOrigin(const char* origin, bool is_picture_in_picture)
    {
        // Make another document with the same security origin.

        auto dummy_page_holder = std::make_unique<DummyPageHolder>(gfx::Size(), nullptr,
            MakeGarbageCollected<WebMediaStubLocalFrameClient>(
                /*player=*/nullptr));
        Document& document = dummy_page_holder->GetDocument();
        document.domWindow()->GetSecurityContext().SetSecurityOriginForTesting(SecurityOrigin::CreateFromString(origin));
        document.domWindow()->set_is_picture_in_picture_window_for_testing(is_picture_in_picture);
    ...
    {
        Media()->GetDocument().domWindow()->GetSecurityContext().SetSecurityOriginForTesting(SecurityOrigin::CreateFromString(origin));
    }
    ...
    void MoveElementAndTestPlayerDestruction(const char* old_origin, const char* new_origin, bool should_destroy, bool is_new_document_picture_in_picture,
        bool is_old_document_picture_in_picture, bool is_new_document_opener, bool is_old_document_opener)
    {
        ...
        if (is_old_document_picture_in_picture) {
            original_document.domWindow()->set_is_picture_in_picture_window_for_testing(is_old_document_picture_in_picture);
        }

        // Make another document with the correct security origin.
        auto new_dummy_page_holder = CreatePageWithSecurityOrigin(new_origin, is_new_document_picture_in_picture);
    ...
    bool HasEventListenerRegistered(EventTarget& target, const AtomicString& event_type, const EventListener* listener) const
    {
    ...
    {
        {
            LocalFrame::NotifyUserActivation(element->GetDocument().GetFrame(), mojom::UserActivationNotificationType::kTest);
        ...
        element->GetDocument().DispatchEvent(*Event::Create(event_type_names::kFullscreenchange));
        EXPECT_EQ(element, Fullscreen::FullscreenElementFrom(element->GetDocument()));
    }
    ...
    void SimulateExitFullscreen(Element* element)
    {
        ...
        element->GetDocument().DispatchEvent(*Event::Create(event_type_names::kFullscreenchange));
        EXPECT_EQ(nullptr, Fullscreen::FullscreenElementFrom(element->GetDocument()));
    }
...
private:
    ...
    mojo::AssociatedReceiver<media::mojom::blink::MediaPlayerHost> media_player_host_receiver_ { &media_player_host_ };
};

INSTANTIATE_TEST_SUITE_P(Audio, HTMLMediaElementTest, testing::Values(MediaTestParam::kAudio));
INSTANTIATE_TEST_SUITE_P(Video, HTMLMediaElementTest, testing::Values(MediaTestParam::kVideo));
...
TEST_P(HTMLMediaElementTest, preloadType)
{
    ...
    struct TestData {
        ...
        { false, true, TestURLScheme::kHttp, AtomicString("scheme"), metadata_string },
        { false, true, TestURLScheme::kHttp, none_string, none_string },
        // Tests that the preload is overridden to metadata_string.
        { false, false, TestURLScheme::kHttp, AtomicString("foo"), metadata_string },
    };

    int index = 0;
    for (const auto& data : test_data) {
        GetNetworkStateNotifier().SetSaveDataEnabledOverride(data.data_saver_enabled);
        if (data.is_cellular) {
            GetNetworkStateNotifier().SetNetworkConnectionInfoOverride(
                true, WebConnectionType::kWebConnectionTypeCellular3G, WebEffectiveConnectionType::kTypeUnknown, 1.0, 2.0);
        } else {
        ...
        EXPECT_EQ(data.preload_expected, Media()->preload()) << "preload type differs at index" << index;
        ++index;
    }
}
...
TEST_P(HTMLMediaElementTest, CouldPlayIfEnoughDataRespondsToEnded)
{
    ...
    MockWebMediaPlayer* mock_wmpi = reinterpret_cast<MockWebMediaPlayer*>(Media()->GetWebMediaPlayer());
    ...
    EXPECT_CALL(*mock_wmpi, CurrentTime()).WillRepeatedly(Return(Media()->duration()));
...
TEST_P(HTMLMediaElementTest, CouldPlayIfEnoughDataInfiniteStreamNeverEnds)
{
    ...
    EXPECT_CALL(*MockMediaPlayer(), Duration()).WillRepeatedly(Return(std::numeric_limits<double>::infinity()));
    EXPECT_CALL(*MockMediaPlayer(), CurrentTime()).WillRepeatedly(Return(std::numeric_limits<double>::infinity()));
...
TEST_P(HTMLMediaElementTest, AutoplayInitiated_DocumentActivation_Low_Gesture)
{
    ...
    Media()->GetDocument().GetSettings()->SetAutoplayPolicy(AutoplayPolicy::Type::kDocumentUserActivationRequired);
    LocalFrame::NotifyUserActivation(Media()->GetDocument().GetFrame(), mojom::UserActivationNotificationType::kTest);
...
TEST_P(HTMLMediaElementTest, AutoplayInitiated_DocumentActivation_High_Gesture)
{
    // Setup is the following:
    // - Policy: DocumentUserActivation (aka. unified autoplay)
    // - MEI: high;
...
TEST_P(HTMLMediaElementTest, AutoplayInitiated_DocumentActivation_High_NoGesture)
{
    // Setup is the following:
    // - Policy: DocumentUserActivation (aka. unified autoplay)
    // - MEI: high;
...
TEST_P(HTMLMediaElementTest, AutoplayInitiated_GestureRequired_Gesture)
{
    ...
    Media()->GetDocument().GetSettings()->SetAutoplayPolicy(AutoplayPolicy::Type::kUserGestureRequired);
    LocalFrame::NotifyUserActivation(Media()->GetDocument().GetFrame(), mojom::UserActivationNotificationType::kTest);
...
TEST_P(HTMLMediaElementTest, AutoplayInitiated_NoGestureRequired_Gesture)
{
    ...
    // - Frame received a user gesture.
    // - MEI doesn't matter as it's not used by the policy.
    Media()->GetDocument().GetSettings()->SetAutoplayPolicy(AutoplayPolicy::Type::kNoUserGestureRequired);
    LocalFrame::NotifyUserActivation(Media()->GetDocument().GetFrame(), mojom::UserActivationNotificationType::kTest);
...
TEST_P(HTMLMediaElementTest, DeferredMediaPlayerLoadDoesNotDelayWindowLoadEvent)
{
    ...
    testing::Mock::VerifyAndClearExpectations(MockMediaPlayer());
    EXPECT_CALL(*MockMediaPlayer(), Load(_, _, _, _)).WillOnce(Return(WebMediaPlayer::LoadTiming::kDeferred));
...
TEST_P(HTMLMediaElementTest, ImmediateMediaPlayerLoadDoesDelayWindowLoadEvent)
{
    ...
    EXPECT_CALL(*MockMediaPlayer(), Load(_, _, _, _)).WillOnce(Return(WebMediaPlayer::LoadTiming::kImmediate));
...
TEST_P(HTMLMediaElementTest, ContextFrozen)
{
    ...
    EXPECT_FALSE(Media()->paused());
    GetExecutionContext()->SetLifecycleState(mojom::FrameLifecycleState::kFrozenAutoResumeMedia);
    EXPECT_TRUE(Media()->paused());
    testing::Mock::VerifyAndClearExpectations(MockMediaPlayer());

    // Now, if we set back to running the media should auto resume.
    GetExecutionContext()->SetLifecycleState(mojom::FrameLifecycleState::kRunning);
    EXPECT_FALSE(Media()->paused());
...
TEST_P(HTMLMediaElementTest, CapturesRedirectedSrc)
{
    ...
    EXPECT_CALL(*MockMediaPlayer(), GetSrcAfterRedirects()).WillRepeatedly(Return(GURL(redirected_url)));
    SetReadyState(HTMLMediaElement::kHaveFutureData);
...
TEST_P(HTMLMediaElementTest, NoPendingActivityAfterCurrentData)
{
    ...
    EXPECT_CALL(*MockMediaPlayer(), GetSrcAfterRedirects).WillRepeatedly(Return(GURL()));
...
TEST_P(HTMLMediaElementTest, SendMediaMetadataChangedToObserver)
{
    ...
    media::MediaContentType media_content_type = media::MediaContentType::kTransient;

    NotifyMediaMetadataChanged(has_audio, has_video, audio_codec, video_codec, media_content_type, is_encrypted_media);
    EXPECT_TRUE(ReceivedMessageMediaMetadataChanged(has_audio, has_video, media_content_type));
    ...
    NotifyMediaMetadataChanged(has_audio, has_video, audio_codec, video_codec, media_content_type, is_encrypted_media);
    EXPECT_TRUE(ReceivedRemotePlaybackMetadataChange(media_session::mojom::blink::RemotePlaybackMetadata::New(
        "unknown", WTF::String(media::GetCodecName(audio_codec)), false, false, WTF::String(), is_encrypted_media)));

    has_video = true;
    NotifyMediaMetadataChanged(has_audio, has_video, audio_codec, video_codec, media_content_type, is_encrypted_media);
    EXPECT_TRUE(ReceivedRemotePlaybackMetadataChange(media_session::mojom::blink::RemotePlaybackMetadata::New(
        WTF::String(media::GetCodecName(video_codec)), WTF::String(media::GetCodecName(audio_codec)), false, false, WTF::String(), is_encrypted_media)));
}
...
TEST_P(HTMLMediaElementTest, SendRemotePlaybackMetadataChangeToObserver)
{
    ...
    bool is_encrypted_media = false;
    NotifyMediaMetadataChanged(true, true, audio_codec, video_codec, media::MediaContentType::kTransient, is_encrypted_media);
    NotifyRemotePlaybackDisabled(is_remote_playback_disabled);
    EXPECT_TRUE(ReceivedRemotePlaybackMetadataChange(media_session::mojom::blink::RemotePlaybackMetadata::New(WTF::String(media::GetCodecName(video_codec)),
        WTF::String(media::GetCodecName(audio_codec)), is_remote_playback_disabled, is_remote_playback_started, WTF::String(), is_encrypted_media)));
}
...
TEST_P(HTMLMediaElementTest, MediaShouldShowAllControlsDependsOnControlslistAttr)
{
    // Enable scripts to prevent controls being shown due to no scripts.
    Media()->GetDocument().GetSettings()->SetScriptEnabled(true);
    ...
    // controls.
    Media()->setAttribute(blink::html_names::kControlslistAttr, AtomicString("nofullscreen"));
    ...
    // the controls.
    Media()->setAttribute(blink::html_names::kControlslistAttr, AtomicString("foo"));
    EXPECT_TRUE(MediaShouldShowAllControls());

    // Setting the controlsList attribute to another valid value should not show
    // the controls.
    Media()->setAttribute(blink::html_names::kControlslistAttr, AtomicString("noplaybackrate"));
    EXPECT_FALSE(MediaShouldShowAllControls());
...
TEST_P(HTMLMediaElementTest, DestroyMediaPlayerWhenSwitchingSameOriginDocumentsIfReuseIsNotEnabled)
{
...
TEST_P(HTMLMediaElementTest, DestroyMediaPlayerWhenSwitchingDifferentOriginDocumentsIfReuseIsNotEnabled)
{
...
TEST_P(HTMLMediaElementTest, DoNotDestroyMediaPlayerWhenSwitchingSameOriginDocumentsIfReuseIsEnabled)
{
...
TEST_P(HTMLMediaElementTest, DestroyMediaPlayerWhenSwitchingSameOriginDocumentsIfNewDocumentIsNotInPictureInPicture)
{
    // Ensure that the WebMediaPlayer is destroyed when moving to a same-origin
...
TEST_P(HTMLMediaElementTest, DestroyMediaPlayerWhenSwitchingSameOriginDocumentsIfFirstDocumentIsInPictureInPicture)
{
    // Ensure that the WebMediaPlayer is destroyed when moving to a same-origin
...
TEST_P(HTMLMediaElementTest, DestroyMediaPlayerWhenSwitchingSameOriginDocumentsIfNotOpenerPipRelation)
{
...
TEST_P(HTMLMediaElementTest, DestroyMediaPlayerWhenSwitchingDifferentOriginDocumentsIfReuseIsEnabled)
{
    // Ensure that the WebMediaPlayer is destroyed when moving to a new origin
...
TEST_P(HTMLMediaElementTest, DestroyMediaPlayerWhenUnloadingOpenerIfReuseIsEnabled)
{
    ...
    WaitForPlayer();
    auto new_dummy_page_holder = CreatePageWithSecurityOrigin(origin, /*is_picture_in_picture=*/true);
    new_dummy_page_holder->GetDocument().GetFrame()->SetOpener(Media()->GetDocument().GetFrame());
...
TEST_P(HTMLMediaElementTest, CreateMediaPlayerAfterMovingElementUsesOpenerFrameIfReuseIsEnabled)
{
    ScopedDocumentPictureInPictureAPIForTest scoped_feature(true);
...
TEST_P(HTMLMediaElementTest, CreateMediaPlayerAfterMovingElementUsesNewFrameIfReuseIsNotEnabled)
{
    ScopedDocumentPictureInPictureAPIForTest scoped_feature(false);
    // Move the element before creating the player.
    const char* origin = "https://a.com";
    SetSecurityOrigin(origin);
    auto new_dummy_page_holder = CreatePageWithSecurityOrigin(origin, /*is_picture_in_picture=*/false);
    Document& new_document = new_dummy_page_holder->GetDocument();
    LocalFrame* old_frame = Media()->GetDocument().GetFrame();
    EXPECT_EQ(old_frame, Media()->LocalFrameForPlayer());
    new_document.adoptNode(Media(), ASSERT_NO_EXCEPTION);
...
TEST_P(HTMLMediaElementTest, PlayedWithoutUserActivation)
{
    ...
    EXPECT_CALL(*MockMediaPlayer(), SetWasPlayedWithUserActivationAndHighMediaEngagement(false));
    Media()->Play();
}
...
TEST_P(HTMLMediaElementTest, PlayedWithUserActivationAndHighMediaEngagement)
{
    ...
    EXPECT_CALL(*MockMediaPlayer(), SetWasPlayedWithUserActivationAndHighMediaEngagement(true));
    Media()->Play();
}
...
TEST_P(HTMLMediaElementTest, PlayedWithUserActivationBeforeLoad)
{
    ...
    EXPECT_CALL(*MockMediaPlayer(), SetWasPlayedWithUserActivationAndHighMediaEngagement(_)).Times(0);
    Media()->Play();
}
...
TEST_P(HTMLMediaElementTest, CanFreezeWithoutMediaPlayerAttached)
{
    ...
    MediaContextLifecycleStateChanged(mojom::FrameLifecycleState::kFrozenAutoResumeMedia);
...
{
    auto* second_document = dummy_page_holder_->GetDocument().implementation().createHTMLDocument();
...
TEST_P(HTMLMediaElementTest, LoadingFailsAfterContextDestruction)
{
    ...
    WaitForPlayer();
    auto new_dummy_page_holder = CreatePageWithSecurityOrigin(kOrigin, /*is_picture_in_picture=*/false);
    EXPECT_FALSE(WasPlayerDestroyed());
...
TEST_P(HTMLMediaElementTest, VideoVisibilityTrackerDetachedOnEnded)
{
    ...
    MockWebMediaPlayer* mock_wmpi = reinterpret_cast<MockWebMediaPlayer*>(video->GetWebMediaPlayer());
    ASSERT_NE(mock_wmpi, nullptr);

    // Advance current time to duration, and verify that the visibility tracker
    // has been detached.
    testing::Mock::VerifyAndClearExpectations(mock_wmpi);
    EXPECT_CALL(*mock_wmpi, CurrentTime()).WillRepeatedly(Return(video->duration()));
    EXPECT_CALL(*mock_wmpi, IsEnded()).WillRepeatedly(Return(true));
...
TEST_P(HTMLMediaElementTest, VideoVisibilityTrackerDetachedOnWebMediaPlayerCleared)
{
    if (GetParam() != MediaTestParam::kVideo) {
        return;
    }
...
TEST_P(HTMLMediaElementTest, VideoVisibilityTrackerInsertingPlayingVideoReusesTracker)
{
    if (GetParam() != MediaTestParam::kVideo) {
        return;
    }
    ...
    MockWebMediaPlayer* mock_wmpi = reinterpret_cast<MockWebMediaPlayer*>(video->GetWebMediaPlayer());
    ASSERT_NE(mock_wmpi, nullptr);
    EXPECT_CALL(*mock_wmpi, CurrentTime()).WillRepeatedly(Return(video->duration() / 2));
    EXPECT_CALL(*mock_wmpi, IsEnded()).WillRepeatedly(Return(false));
    ...
    video->GetDocument().body()->setInnerHTML("<div id='container' style='width:200px; height:200px;'></div>");
    video->GetDocument().body()->getElementById(AtomicString("container"))->AppendChild(video);
...
TEST_P(HTMLMediaElementTest, VideoVisibilityTrackerStatusUpdatedOnVideoEnterAndExitFullscreen)
{
    if (GetParam() != MediaTestParam::kVideo) {
        return;
    }
    ...
    EXPECT_NE(VideoVisibilityTrackerAttachedToDocument(video), nullptr);
    EXPECT_FALSE(video->IsFullscreen());
    EXPECT_TRUE(HasEventListenerRegistered(video->GetDocument(), event_type_names::kFullscreenchange, VideoVisibilityTracker()));
...
TEST_P(HTMLMediaElementTest, VideoVisibilityTrackerStatusUpdatedOnAnyElementEnterAndExitFullscreen)
{
    if (GetParam() != MediaTestParam::kVideo) {
        return;
    }
    ...
    video->GetDocument().body()->setInnerHTML("<div id='fullscreen-div' style='width:200px; height:200px;'></div>");
    Element* fullscreen_div = video->GetDocument().body()->getElementById(AtomicString("fullscreen-div"));
    video->GetDocument().body()->AppendChild(video);
    video->SetSrc(SrcSchemeToURL(TestURLScheme::kHttp));
    test::RunPendingTasks();
    ASSERT_EQ(VideoVisibilityTracker(), nullptr);
...
TEST_P(HTMLMediaElementTest, VideoVisibilityTrackerDetachedOnEnterFullscreenAfterVideoPauseThenPlay)
{
    if (GetParam() != MediaTestParam::kVideo) {
        return;
    }
...
TEST_P(HTMLMediaElementTest, VideoVisibilityTrackerDetachedOnEntereFullscreenAfterInsertingPlayingVideo)
{
    if (GetParam() != MediaTestParam::kVideo) {
        return;
    }
    ...
    ASSERT_NE(VideoVisibilityTrackerAttachedToDocument(video), nullptr);
    const auto* tracker_before_append = video->visibility_tracker_for_tests();
    EXPECT_TRUE(HasEventListenerRegistered(video->GetDocument(), event_type_names::kFullscreenchange, tracker_before_append));
    ...
    ASSERT_NE(VideoVisibilityTrackerAttachedToDocument(video), nullptr);
    const auto* tracker_after_append = video->visibility_tracker_for_tests();
    EXPECT_TRUE(HasEventListenerRegistered(video->GetDocument(), event_type_names::kFullscreenchange, tracker_after_append));
...
TEST_P(HTMLMediaElementTest, VideoVisibilityTrackerFullScreenEventListenerRemovedOnDidMoveToNewDocument)
{
    if (GetParam() != MediaTestParam::kVideo) {
        return;
    }
    ...
    EXPECT_FALSE(HasEventListenerRegistered(video->GetDocument(), event_type_names::kFullscreenchange, VideoVisibilityTracker()));

    // Verify that, for the old document, the fullscreen change event listener has
    // been removed.
    EXPECT_FALSE(HasEventListenerRegistered(old_document, event_type_names::kFullscreenchange, VideoVisibilityTracker()));
}

TEST_P(HTMLMediaElementTest, VideoVisibilityTrackerFullScreenEventListenerRemovedOnRemovedFrom)
{
    if (GetParam() != MediaTestParam::kVideo) {
        return;
    }
    ...
    test::RunPendingTasks();
    EXPECT_EQ(VideoVisibilityTrackerAttachedToDocument(video), nullptr);
    EXPECT_FALSE(HasEventListenerRegistered(video->GetDocument(), event_type_names::kFullscreenchange, video->visibility_tracker_for_tests()));
}

TEST_P(HTMLMediaElementTest, RequestVisibilityReturnsFalseWhenTrackerDoesNotExist)
{
    if (GetParam() != MediaTestParam::kVideo) {
        return;
    }
...
TEST_P(HTMLMediaElementTest, RequestVisibilityReturnsFalseWhenTrackerIsNotAttached)
{
    if (GetParam() != MediaTestParam::kVideo) {
        return;
    }
...
TEST_P(HTMLMediaElementTest, StartVideoWithTrackSelectionFragment)
{
    ...
    EXPECT_CALL(*MockMediaPlayer(), GetSrcAfterRedirects()).WillRepeatedly(Return(GURL(frag_url)));
    bool audio_only = GetParam() == MediaTestParam::kAudio;

    EXPECT_CALL(*MockMediaPlayer(), Load(_, _, _, _))
        .Times(1)
        .WillOnce([element = Media(), audio_only](
                      EmptyWebMediaPlayer::LoadType, const blink::WebMediaPlayerSource&, EmptyWebMediaPlayer::CorsMode, bool) -> WebMediaPlayer::LoadTiming {
            element->AddMediaTrackForTesting(media::MediaTrack::CreateAudioTrack("audio1", media::MediaTrack::AudioKind::kMain, "audio1", "", true, 0, true));
            element->AddMediaTrackForTesting(media::MediaTrack::CreateAudioTrack("audio2", media::MediaTrack::AudioKind::kMain, "audio2", "", false, 0, true));
            element->AddMediaTrackForTesting(media::MediaTrack::CreateAudioTrack("audio3", media::MediaTrack::AudioKind::kMain, "audio3", "", false, 0, true));
            if (!audio_only) {
                element->AddMediaTrackForTesting(media::MediaTrack::CreateVideoTrack("video1", media::MediaTrack::VideoKind::kMain, "video1", "", true, 0));
                element->AddMediaTrackForTesting(media::MediaTrack::CreateVideoTrack("video2", media::MediaTrack::VideoKind::kMain, "video2", "", false, 0));
                element->AddMediaTrackForTesting(media::MediaTrack::CreateVideoTrack("video3", media::MediaTrack::VideoKind::kMain, "video3", "", false, 0));
            }
    ...
    EXPECT_CALL(*MockMediaPlayer(), EnabledAudioTracksChanged(_)).WillOnce([](const WebVector<WebMediaPlayer::TrackId>& tracks) {
        ASSERT_EQ(tracks.size(), 1u);
        ASSERT_EQ(tracks[0], "audio2");
    });

    if (!audio_only) {
        EXPECT_CALL(*MockMediaPlayer(), SelectedVideoTrackChanged(_)).WillOnce([](std::optional<WebMediaPlayer::TrackId> track) {
