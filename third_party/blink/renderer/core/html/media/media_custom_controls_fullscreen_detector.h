...
class CORE_EXPORT MediaCustomControlsFullscreenDetector final : public NativeEventListener {
public:
...
private:
    ...
    void OnIntersectionChanged(const HeapVector<Member<IntersectionObserverEntry>>&);
    bool IsVideoOrParentFullscreen();
    void ReportEffectivelyFullscreen(bool);
    static bool IsFullscreenVideoOfDifferentRatioForTesting(const gfx::Size& video_size, const gfx::Size& viewport_size, const gfx::Size& intersection_size);

    void UpdateDominantAndFullscreenStatus(bool is_dominant_visible_content, bool is_effectively_fullscreen);
