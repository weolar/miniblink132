...
class CORE_EXPORT AutoplayUmaHelper : public NativeEventListener, public ExecutionContextLifecycleObserver {
public:
...
private:
    ...
    void OnIntersectionChangedForMutedVideoOffscreenDuration(const HeapVector<Member<IntersectionObserverEntry>>& entries);
    void OnIntersectionChangedForMutedVideoPlayMethodBecomeVisible(const HeapVector<Member<IntersectionObserverEntry>>& entries);
    ...
    Member<IntersectionObserver> muted_video_offscreen_duration_intersection_observer_;
};
