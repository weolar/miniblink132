...
constexpr base::TimeDelta kPictureInPictureStyleChangeTransitionDuration = base::Milliseconds(200);
constexpr base::TimeDelta kPictureInPictureHiddenAnimationSeconds = base::Milliseconds(300);

} // namespace

namespace blink {

class PictureInPictureInterstitial::VideoElementResizeObserverDelegate final : public ResizeObserver::Delegate {
public:
    explicit VideoElementResizeObserverDelegate(PictureInPictureInterstitial* interstitial)
...
PictureInPictureInterstitial::PictureInPictureInterstitial(HTMLVideoElement& videoElement)
    : HTMLDivElement(videoElement.GetDocument())
    , resize_observer_(ResizeObserver::Create(videoElement.GetDocument().domWindow(), MakeGarbageCollected<VideoElementResizeObserverDelegate>(this)))
    , interstitial_timer_(videoElement.GetDocument().GetTaskRunner(TaskType::kInternalMedia), this, &PictureInPictureInterstitial::ToggleInterstitialTimerFired)
    , video_element_(&videoElement)
{
    ...
    message_element_->SetShadowPseudoId(AtomicString("-internal-picture-in-picture-interstitial-message"));
    message_element_->setInnerText(GetVideoElement().GetLocale().QueryString(IDS_MEDIA_PICTURE_IN_PICTURE_INTERSTITIAL_TEXT));
...
void PictureInPictureInterstitial::Show()
{
    ...
    interstitial_timer_.StartOneShot(kPictureInPictureStyleChangeTransitionDuration, FROM_HERE);
...
void PictureInPictureInterstitial::Hide()
{
    ...
    interstitial_timer_.StartOneShot(kPictureInPictureHiddenAnimationSeconds, FROM_HERE);
...
Node::InsertionNotificationRequest PictureInPictureInterstitial::InsertedInto(ContainerNode& root)
{
    if (GetVideoElement().isConnected() && !resize_observer_) {
        resize_observer_ = ResizeObserver::Create(GetVideoElement().GetDocument().domWindow(), MakeGarbageCollected<VideoElementResizeObserverDelegate>(this));
...
void PictureInPictureInterstitial::NotifyElementSizeChanged(const DOMRectReadOnly& new_size)
{
    message_element_->setAttribute(html_names::kClassAttr, MediaControls::GetSizingCSSClass(MediaControls::GetSizingClass(new_size.width())));
