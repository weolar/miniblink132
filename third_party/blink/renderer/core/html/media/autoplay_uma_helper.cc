...
static void RecordAutoplaySourceMetrics(HTMLMediaElement* element, AutoplaySource source)
{
...
void AutoplayUmaHelper::OnAutoplayInitiated(AutoplaySource source)
{
    ...
    ukm::builders::Media_Autoplay_Attempt(element_->GetDocument().UkmSourceID())
        ...
        .SetUserGestureRequired(element_->GetAutoplayPolicy().IsGestureNeededForPlayback())
        .SetMuted(element_->muted())
        .SetHighMediaEngagement(AutoplayPolicy::DocumentHasHighMediaEngagement(element_->GetDocument()))
...
{
    base::UmaHistogramEnumeration("Media.Video.Autoplay.Muted.UnmuteAction", status);
    ...
    if (element_->GetDocument().IsInOutermostMainFrame()) {
        ...
        ukm::UkmRecorder* ukm_recorder = element_->GetDocument().UkmRecorder();
        DCHECK(ukm_recorder);
        ukm::builders::Media_Autoplay_Muted_UnmuteAction(element_->GetDocument().UkmSourceID())
            .SetSource(source)
...
void AutoplayUmaHelper::VideoWillBeDrawnToCanvas()
{
    if (HasSource() && !IsVisible()) {
        UseCounter::Count(element_->GetDocument(), WebFeature::kHiddenAutoplayedVideoInCanvas);
    }
}
...
void AutoplayUmaHelper::OnIntersectionChangedForMutedVideoPlayMethodBecomeVisible(const HeapVector<Member<IntersectionObserverEntry>>& entries)
{
...
void AutoplayUmaHelper::OnIntersectionChangedForMutedVideoOffscreenDuration(const HeapVector<Member<IntersectionObserverEntry>>& entries)
{
    bool is_visible = (entries.back()->intersectionRatio() > 0);
    ...
    if (is_visible) {
        muted_video_autoplay_offscreen_duration_ += base::TimeTicks::Now() - muted_video_autoplay_offscreen_start_time_;
    } else {
...
void AutoplayUmaHelper::Invoke(ExecutionContext* execution_context, Event* event)
{
...
{
    if (!sources_.Contains(AutoplaySource::kMethod) || !IsA<HTMLVideoElement>(element_.Get()) || !element_->muted())
        return;

    muted_video_play_method_intersection_observer_ = IntersectionObserver::Create(element_->GetDocument(),
        WTF::BindRepeating(&AutoplayUmaHelper::OnIntersectionChangedForMutedVideoPlayMethodBecomeVisible, WrapWeakPersistent(this)),
...
void AutoplayUmaHelper::MaybeStopRecordingMutedVideoPlayMethodBecomeVisible(bool visible)
{
    ...
    base::UmaHistogramBoolean("Media.Video.Autoplay.Muted.PlayMethod.BecomesVisible", visible);
...
{
    if (!IsA<HTMLVideoElement>(element_.Get()) || !element_->muted() || !sources_.Contains(AutoplaySource::kMethod))
        return;

    // Start recording muted video playing offscreen duration.
    muted_video_autoplay_offscreen_start_time_ = base::TimeTicks::Now();
    is_visible_ = false;
    muted_video_offscreen_duration_intersection_observer_ = IntersectionObserver::Create(element_->GetDocument(),
        WTF::BindRepeating(&AutoplayUmaHelper::OnIntersectionChangedForMutedVideoOffscreenDuration, WrapWeakPersistent(this)),
        LocalFrameUkmAggregator::kMediaIntersectionObserver, IntersectionObserver::Params { .thresholds = { IntersectionObserver::kMinimumThreshold } });
...
void AutoplayUmaHelper::MaybeStopRecordingMutedVideoOffscreenDuration()
{
    ...
    UMA_HISTOGRAM_CUSTOM_TIMES("Media.Video.Autoplay.Muted.PlayMethod.OffscreenDuration", muted_video_autoplay_offscreen_duration_, base::Milliseconds(1),
...
void AutoplayUmaHelper::MaybeUnregisterContextDestroyedObserver()
{
    ...
    if (!ShouldListenToContextDestroyed() && !GetExecutionContext()->ContextLifecycleObserverSet().IsIteratingOverObservers()) {
...
{
    return muted_video_play_method_intersection_observer_ || muted_video_offscreen_duration_intersection_observer_;
}
