...
TEST_P(ParseNPTTimeTest, TestParseNPTTime)
{
    ...
    ASSERT_EQ(parser.ParseNPTTime(test_case.time_string, offset, time), test_case.expected_result);
    ASSERT_EQ(time, test_case.expected_time);
}

INSTANTIATE_TEST_SUITE_P(ParseNPTTimeTests, ParseNPTTimeTest,
    ...
    [](const testing::TestParamInfo<ParseNPTTimeTest::ParamType>& info) { return info.param.test_name; });
} // namespace blink
