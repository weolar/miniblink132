...
public:
    MOCK_METHOD3(EnterFullscreen, void(LocalFrame&, const FullscreenOptions*, FullscreenRequestType));
...
protected:
    ...
    FullscreenMockChromeClient& GetMockChromeClient()
    {
...
TEST_F(HTMLVideoElementPersistentTest, internalPseudoClassOnlyUAStyleSheet)
{
...

    {
        ...
        EXPECT_FALSE(DivElement()->matches(AtomicString(":-internal-video-persistent-ancestor"), exception_state));
        EXPECT_TRUE(exception_state.HadException());
    }
    {
        DummyExceptionStateForTesting exception_state;
        EXPECT_FALSE(VideoElement()->matches(AtomicString(":-internal-video-persistent"), exception_state));
        EXPECT_TRUE(exception_state.HadException());
    }
    {
        DummyExceptionStateForTesting exception_state;
        EXPECT_FALSE(VideoElement()->matches(AtomicString(":-internal-video-persistent-ancestor"), exception_state));
        EXPECT_TRUE(exception_state.HadException());
    }
...
TEST_F(HTMLVideoElementPersistentTest, removeVideoWithLayerWhilePersisting)
{
    ...
    Persistent<Element> span = GetDocument().CreateRawElement(html_names::kSpanTag);
