...
constexpr base::TimeDelta kStyleChangeTransitionDuration = base::Milliseconds(200);
...
MediaRemotingInterstitial::MediaRemotingInterstitial(HTMLVideoElement& videoElement)
    : HTMLDivElement(videoElement.GetDocument())
    , toggle_interstitial_timer_(
          videoElement.GetDocument().GetTaskRunner(TaskType::kInternalMedia), this, &MediaRemotingInterstitial::ToggleInterstitialTimerFired)
    , video_element_(&videoElement)
{
    ...
    background_image_->SetShadowPseudoId(AtomicString("-internal-media-interstitial-background-image"));
    background_image_->setAttribute(html_names::kSrcAttr, videoElement.FastGetAttribute(html_names::kPosterAttr));
    AppendChild(background_image_);

    cast_icon_ = MakeGarbageCollected<HTMLDivElement>(GetDocument());
    cast_icon_->SetShadowPseudoId(AtomicString("-internal-media-remoting-cast-icon"));
    AppendChild(cast_icon_);

    cast_text_message_ = MakeGarbageCollected<HTMLDivElement>(GetDocument());
    cast_text_message_->SetShadowPseudoId(AtomicString("-internal-media-interstitial-message"));
    AppendChild(cast_text_message_);

    toast_message_ = MakeGarbageCollected<HTMLDivElement>(GetDocument());
    toast_message_->SetShadowPseudoId(AtomicString("-internal-media-remoting-toast-message"));
    AppendChild(toast_message_);
}

void MediaRemotingInterstitial::Show(const WebString& remote_device_friendly_name)
{
    if (IsVisible())
        return;
    if (remote_device_friendly_name.IsEmpty()) {
        cast_text_message_->setInnerText(GetVideoElement().GetLocale().QueryString(IDS_MEDIA_REMOTING_CAST_TO_UNKNOWN_DEVICE_TEXT));
    } else {
        cast_text_message_->setInnerText(GetVideoElement().GetLocale().QueryString(IDS_MEDIA_REMOTING_CAST_TEXT, remote_device_friendly_name));
    }
    if (toggle_interstitial_timer_.IsActive())
        toggle_interstitial_timer_.Stop();
    state_ = kVisible;
    RemoveInlineStyleProperty(CSSPropertyID::kDisplay);
    SetInlineStyleProperty(CSSPropertyID::kOpacity, 0, CSSPrimitiveValue::UnitType::kNumber);
    toggle_interstitial_timer_.StartOneShot(kStyleChangeTransitionDuration, FROM_HERE);
}
...
void MediaRemotingInterstitial::Hide(int error_code)
{
    ...
    if (error_code == MediaPlayerClient::kMediaRemotingStopNoText) {
        ...
        String stop_text = GetVideoElement().GetLocale().QueryString(IDS_MEDIA_REMOTING_STOP_TEXT);
...
void MediaRemotingInterstitial::ToggleInterstitialTimerFired(TimerBase*)
{
    ...
    if (IsVisible()) {
        ...
        toast_message_->SetInlineStyleProperty(CSSPropertyID::kDisplay, CSSValueID::kNone);
        SetInlineStyleProperty(CSSPropertyID::kBackgroundColor, CSSValueID::kBlack);
        SetInlineStyleProperty(CSSPropertyID::kOpacity, 1, CSSPrimitiveValue::UnitType::kNumber);
        ...
        SetInlineStyleProperty(CSSPropertyID::kBackgroundColor, CSSValueID::kTransparent);
        SetInlineStyleProperty(CSSPropertyID::kOpacity, 1, CSSPrimitiveValue::UnitType::kNumber);
        background_image_->SetInlineStyleProperty(CSSPropertyID::kDisplay, CSSValueID::kNone);
        cast_icon_->SetInlineStyleProperty(CSSPropertyID::kDisplay, CSSValueID::kNone);
        cast_text_message_->SetInlineStyleProperty(CSSPropertyID::kDisplay, CSSValueID::kNone);
        toast_message_->SetInlineStyleProperty(CSSPropertyID::kOpacity, 1, CSSPrimitiveValue::UnitType::kNumber);
        state_ = kHidden;
...
{
    toggle_interstitial_timer_.MoveToNewTaskRunner(GetDocument().GetTaskRunner(TaskType::kInternalMedia));
...
{
    background_image_->setAttribute(html_names::kSrcAttr, GetVideoElement().FastGetAttribute(html_names::kPosterAttr));
}
