...
protected:
    ...
    {
        std::unique_ptr<SimRequest> main_resource = std::make_unique<SimRequest>("https://example.com/", "text/html");
...
TEST_F(VideoFillingViewportTest, MostlyFillingViewport)
{
    ...
    auto* element = To<HTMLVideoElement>(GetDocument().getElementById(AtomicString("video")));
...
TEST_F(VideoFillingViewportTest, FillingViewportChanged)
{
    ...
    element->setAttribute(html_names::kStyleAttr, AtomicString("position:fixed; left:0; top:0; width:80%; height:80%;"));
    DoCompositeAndPropagate();
    EXPECT_FALSE(IsMostlyFillingViewport(element));
}
...
TEST_F(VideoFillingViewportTest, VideoScrollOutHalf)
{
    ...
    element->setAttribute(html_names::kStyleAttr, AtomicString("position:fixed; left:0; top:240px; width:100%; height:100%;"));
    DoCompositeAndPropagate();
    EXPECT_FALSE(IsMostlyFillingViewport(element));
}
