...
HitTestResult HitTestForOcclusionRatio(
    const HTMLVideoElement& video_element, const PhysicalRect& hit_rect, std::optional<HitTestRequest::HitNodeCb> hit_node_cb)
{
    ...
    HitTestRequest::HitTestRequestType hit_type = HitTestRequest::kIgnorePointerEventsNone | HitTestRequest::kReadOnly | HitTestRequest::kIgnoreClipping
        | HitTestRequest::kIgnoreZeroOpacityObjects | HitTestRequest::kHitTestVisualOverflow | HitTestRequest::kListBased | HitTestRequest::kPenetratingList
        | HitTestRequest::kAvoidCache;
    HitTestLocation location(hit_rect);
    return frame->GetEventHandler().HitTestResultAtLocation(location, hit_type, video_element.GetLayoutObject(), true, hit_node_cb);
}
...
bool HasEnoughVisibleAreaRemaining(float occluded_area, const PhysicalRect& video_element_rect, const int visibility_threshold)
{
    return ComputeArea(video_element_rect) - occluded_area >= visibility_threshold;
}

float ComputeOccludingArea(const Vector<SkIRect>& occluding_rects, float video_element_area)
{
    // Record the total time spent computing the occluding area.
    SCOPED_UMA_HISTOGRAM_TIMER_MICROS("Media.MediaVideoVisibilityTracker.ComputeOcclusion.ComputeOccludingArea."
                                      "TotalDuration");
    ...
    for (SkRegion::Iterator it(region); !it.done(); it.next()) {
        ...
        occluding_area += ComputeArea(PhysicalRect(gfx::SkIRectToRect(occluding_rect)));
...
void RecordTotalCounts(const MediaVideoVisibilityTracker::Metrics& counts)
{
    ...
    UMA_HISTOGRAM_COUNTS_1000("Media.MediaVideoVisibilityTracker."
                              "HitTestedNodesContributingToOcclusionCount.ExponentialHistogram."
                              "TotalCount",
        counts.total_hit_tested_nodes_contributing_to_occlusion);

    if (counts.total_hit_tested_nodes_contributing_to_occlusion <= kRecordLinearHistogramLimit) {
        UMA_HISTOGRAM_EXACT_LINEAR("Media.MediaVideoVisibilityTracker."
                                   "HitTestedNodesContributingToOcclusionCount.LinearHistogram.TotalCount",
            counts.total_hit_tested_nodes_contributing_to_occlusion, kRecordLinearHistogramLimit);
    }

    // Record the total number of hit tested nodes.
    UMA_HISTOGRAM_COUNTS_1000("Media.MediaVideoVisibilityTracker.HitTestedNodesCount."
                              "ExponentialHistogram.TotalCount",
        counts.total_hit_tested_nodes);

    if (counts.total_hit_tested_nodes <= kRecordLinearHistogramLimit) {
        UMA_HISTOGRAM_EXACT_LINEAR("Media.MediaVideoVisibilityTracker.HitTestedNodesCount.LinearHistogram."
                                   "TotalCount",
    ...
    // being opaque.
    UMA_HISTOGRAM_COUNTS_1000("Media.MediaVideoVisibilityTracker.IgnoredNodesNotOpaqueCount."
                              "ExponentialHistogram.TotalCount",
        counts.total_ignored_nodes_not_opaque);

    if (counts.total_ignored_nodes_not_opaque <= kRecordLinearHistogramLimit) {
        UMA_HISTOGRAM_EXACT_LINEAR("Media.MediaVideoVisibilityTracker.IgnoredNodesNotOpaqueCount."
    ...
    UMA_HISTOGRAM_COUNTS_1000("Media.MediaVideoVisibilityTracker.IgnoredNodesUserAgentShadowRootCount."
                              "ExponentialHistogram."
                              "TotalCount",
        counts.total_ignored_nodes_user_agent_shadow_root);

    if (counts.total_ignored_nodes_user_agent_shadow_root <= kRecordLinearHistogramLimit) {
        UMA_HISTOGRAM_EXACT_LINEAR("Media.MediaVideoVisibilityTracker."
                                   "IgnoredNodesUserAgentShadowRootCount.LinearHistogram.TotalCount",
            counts.total_ignored_nodes_user_agent_shadow_root, kRecordLinearHistogramLimit);
    }

    // Record the total number of occluding rects.
    UMA_HISTOGRAM_COUNTS_1000("Media.MediaVideoVisibilityTracker.OccludingRectsCount."
                              "ExponentialHistogram.TotalCount",
        counts.total_occluding_rects);

    if (counts.total_occluding_rects <= kRecordLinearHistogramLimit) {
        UMA_HISTOGRAM_EXACT_LINEAR("Media.MediaVideoVisibilityTracker.OccludingRectsCount.LinearHistogram."
                                   "TotalCount",
    ...
    if (counts.total_hit_tested_nodes != 0) {
        ignored_nodes_not_opaque_percentage = 100 * counts.total_ignored_nodes_not_opaque / counts.total_hit_tested_nodes;
        ignored_nodes_user_agent_shadow_root_percentage = 100 * counts.total_ignored_nodes_user_agent_shadow_root / counts.total_hit_tested_nodes;
        total_hit_tested_nodes_contributing_to_occlusion_percentage
            = 100 * counts.total_hit_tested_nodes_contributing_to_occlusion / counts.total_hit_tested_nodes;
    }

    // Record the percentage of the total hit tested nodes that are ignored due to
    // not being opaque.
    UMA_HISTOGRAM_PERCENTAGE("Media.MediaVideoVisibilityTracker.IgnoredNodesNotOpaque.Percentage", ignored_nodes_not_opaque_percentage);

    // Record the percentage of the total hit tested nodes that are ignored due to
    // being in the shadow root and of user agent type.
    UMA_HISTOGRAM_PERCENTAGE("Media.MediaVideoVisibilityTracker.IgnoredNodesUserAgentShadowRoot."
                             "Percentage",
    ...
    UMA_HISTOGRAM_PERCENTAGE("Media.MediaVideoVisibilityTracker."
...
{
    DEFINE_STATIC_LOCAL(
        const Vector<AtomicString>, fullscreen_change_event_types, ({ event_type_names::kWebkitfullscreenchange, event_type_names::kFullscreenchange }));
    return fullscreen_change_event_types;
}

// Returns true if `target` has `listener` event listener registered.
bool HasEventListenerRegistered(EventTarget& target, const AtomicString& event_type, const EventListener* listener)
{
...
{
    return !(type == DisplayItem::kFrameOverlay || type == DisplayItem::kForeignLayerLinkHighlight || type == DisplayItem::kForeignLayerViewportScroll
        || type == DisplayItem::kForeignLayerViewportScrollbar);
}

void RecordVideoOcclusionState(const HTMLVideoElement& video_element, const MediaVideoVisibilityTracker::OcclusionState& occlusion_state,
    bool has_sufficiently_visible_video, const int visibility_threshold)
{
    ...
    for (wtf_size_t i = 0; i < occluding_rects.size(); ++i) {
        ...
        occluding_rects_stream << String::Format("[x: %d, y: %d, width: %d, height: %d]", rect.x(), rect.y(), rect.width(), rect.height());
    ...
    const String intersection_rect_string = intersection_rect.IsEmpty()
        ? "None"
        : String::Format("x: %d, y: %d, width: %d, height: %d", intersection_rect.X().ToInt(), intersection_rect.Y().ToInt(), intersection_rect.Width().ToInt(),
            intersection_rect.Height().ToInt());

    const auto& video_element_rect = occlusion_state.video_element_rect;
    const String video_element_rect_string = video_element_rect.IsEmpty()
        ? "None"
        : String::Format("x: %d, y: %d, width: %d, height: %d", video_element_rect.X().ToInt(), video_element_rect.Y().ToInt(),
            video_element_rect.Width().ToInt(), video_element_rect.Height().ToInt());

    const String occlusion_state_string = String::Format("has sufficiently visible video: {%s}, occluded area: {%.2f}, occluding "
                                                         "rects: {%s}, intersection rect: {%s}, video element rect: {%s}, "
                                                         "visibility threshold: {%d}",
        has_sufficiently_visible_video ? "True" : "False", occlusion_state.occluded_area, occluding_rects_stream.str().c_str(),
        intersection_rect_string.Ascii().c_str(), video_element_rect_string.Ascii().c_str(), visibility_threshold);

    video_element.RecordVideoOcclusionState(occlusion_state_string.Ascii().c_str());
}

} // anonymous namespace

MediaVideoVisibilityTracker::MediaVideoVisibilityTracker(
    HTMLVideoElement& video, const int visibility_threshold, ReportVisibilityCb report_visibility_cb, base::TimeDelta hit_test_interval)
    ...
    , hit_test_interval_(hit_test_interval)
{
    ...
    DCHECK_GT(visibility_threshold_, 0) << "Invalid threshold: " << visibility_threshold_;
...
void MediaVideoVisibilityTracker::UpdateVisibilityTrackerState()
{
    ...
    // other element.
    Element* fullscreen_element = Fullscreen::FullscreenElementFrom(video_element.GetDocument());

    if (video_element.GetWebMediaPlayer() && video_element.GetExecutionContext() && !video_element.paused() && !fullscreen_element) {
        Attach();
    } else {
        Detach();
    }
...
void MediaVideoVisibilityTracker::Invoke(ExecutionContext* context, Event* event)
{
...
void MediaVideoVisibilityTracker::MaybeRemoveFullscreenEventListeners()
{
    ...
    if (video_element.isConnected() && document == tracker_attached_to_document_) {
        return;
    }
    ...
    if (document != tracker_attached_to_document_) {
        // Ignore event listeners that have already been removed.
        for (const auto& event_type : FullscreenEventTypes()) {
            if (!HasEventListenerRegistered(*tracker_attached_to_document_.Get(), event_type, this)) {
                continue;
            }
            tracker_attached_to_document_->removeEventListener(event_type, this, true);
        }
    }
}

void MediaVideoVisibilityTracker::RequestVisibility(RequestVisibilityCallback request_visibility_callback)
{
...
const MediaVideoVisibilityTracker::ClientIdsSet MediaVideoVisibilityTracker::GetClientIdsSet(DisplayItemClientId start_after_display_item_client_id) const
{
    SCOPED_UMA_HISTOGRAM_TIMER("Media.MediaVideoVisibilityTracker.GetClientIdsSet.SetConstruction."
                               "TotalDuration");
    ...
    LocalFrameView::InvalidationDisallowedScope invalidation_disallowed(*document_view);

    const auto& paint_artifact = document_view->GetPaintArtifact();
    const DisplayItemList& display_item_list = paint_artifact.GetDisplayItemList();
    ...
    while (begin_index < end_index && display_item_list[begin_index].ClientId() != start_after_display_item_client_id) {
        begin_index++;
    }
    ...
    while (end_index > begin_index && !IsContentType(display_item_list[end_index - 1].GetType())) {
        not_content_type_count++;
        end_index--;
    }
    UMA_HISTOGRAM_COUNTS_10000("Media.MediaVideoVisibilityTracker.GetClientIdsSet.NotContentTypeCount."
                               "TotalCount",
        not_content_type_count);
    ...
    for (const auto& display_item : display_item_list.ItemsInRange(begin_index, end_index)) {
    ...
    UMA_HISTOGRAM_COUNTS_10000("Media.MediaVideoVisibilityTracker.GetClientIdsSet.ItemsInSetCount."
                               "TotalCount",
        set_size);
    ...
    UMA_HISTOGRAM_PERCENTAGE("Media.MediaVideoVisibilityTracker.GetClientIdsSet.NotContentType."
                             "Percentage",
...
ListBasedHitTestBehavior MediaVideoVisibilityTracker::ComputeOcclusion(const ClientIdsSet& client_ids_set, Metrics& counts, const Node& node)
{
    counts.total_hit_tested_nodes++;
    ...
    if (node.IsInShadowTree() && node.ContainingShadowRoot() && node.ContainingShadowRoot()->IsUserAgent()) {
        counts.total_ignored_nodes_user_agent_shadow_root++;
    ...
    if (!client_ids_set.empty() && !client_ids_set.Contains(node.GetLayoutObject()->Id())) {
        return kContinueHitTesting;
    }
    ...
    occlusion_state_.occluding_rects.push_back(gfx::RectToSkIRect(ToPixelSnappedRect(node_rect)));
    occlusion_state_.occluded_area = ComputeOccludingArea(occlusion_state_.occluding_rects, ComputeArea(occlusion_state_.video_element_rect));

    counts.total_hit_tested_nodes_contributing_to_occlusion++;

    if (HasEnoughVisibleAreaRemaining(occlusion_state_.occluded_area, occlusion_state_.video_element_rect, visibility_threshold_)) {
        return kContinueHitTesting;
    }
...
bool MediaVideoVisibilityTracker::MeetsVisibilityThreshold(Metrics& counts, const PhysicalRect& rect)
{
    const ClientIdsSet client_ids_set = GetClientIdsSet(VideoElement().GetLayoutObject()->Id());

    {
        // Record the total time spent computing occlusion.
        SCOPED_UMA_HISTOGRAM_TIMER("Media.MediaVideoVisibilityTracker.ComputeOcclusion.TotalDuration");

        HitTestResult result(HitTestForOcclusionRatio(
            VideoElement(), rect, WTF::BindRepeating(&MediaVideoVisibilityTracker::ComputeOcclusion, WrapPersistent(this), client_ids_set, std::ref(counts))));
    }

    return HasEnoughVisibleAreaRemaining(occlusion_state_.occluded_area, occlusion_state_.video_element_rect, visibility_threshold_) ? true : false;
}
...
bool MediaVideoVisibilityTracker::ComputeVisibility()
{
    ...
    Metrics counts;
    bool meets_visibility_threshold = MeetsVisibilityThreshold(counts, occlusion_state_.intersection_rect);

    counts.total_occluding_rects = base::saturated_cast<int>(occlusion_state_.occluding_rects.size());
...
void MediaVideoVisibilityTracker::ComputeAreaOccludedByViewport(const LocalFrameView& local_frame_view)
{
    DCHECK(VideoElement().GetLayoutObject());
    ...
    if (auto* layout_video = DynamicTo<LayoutVideo>(VideoElement().GetLayoutObject())) {
        PhysicalRect content_rect = layout_video->ReplacedContentRect();
        content_bounds = VideoElement().GetDocument().View()->FrameToViewport(ToEnclosingRect(layout_video->LocalToAbsoluteRect(content_rect)));
    ...
    gfx::Rect viewport_in_root_frame = ToEnclosingRect(local_frame_view.GetFrame().GetPage()->GetVisualViewport().VisibleRect());
    gfx::Rect absolute_viewport(local_frame_view.ConvertFromRootFrame(viewport_in_root_frame));
    occlusion_state_.intersection_rect = PhysicalRect(IntersectRects(absolute_viewport, content_bounds));
    ...
    if (region.op(gfx::RectToSkIRect(absolute_viewport), SkRegion::kDifference_Op)) {
        for (SkRegion::Iterator it(region); !it.done(); it.next()) {
            auto occluding_rect = it.rect();
...
void MediaVideoVisibilityTracker::MaybeComputeVisibility(ShouldReportVisibility should_report_visibility)
{
    if (!tracker_attached_to_document_ || !tracker_attached_to_document_->GetFrame()->View()
        || !tracker_attached_to_document_->GetFrame()->IsOutermostMainFrame() || !VideoElement().GetLayoutObject()) {
        if (request_visibility_callback_) {
            RecordVideoOcclusionState(VideoElement(), occlusion_state_, false, visibility_threshold_);
            std::move(request_visibility_callback_).Run(false);
        }
        return;
    }

    if (VideoElement().GetDocument().Lifecycle().GetState() != DocumentLifecycle::kPaintClean) {
        // If we have a pending visibility request, run it now with the cached
        // `meets_visibility_threshold_` value.
        if (request_visibility_callback_) {
            RecordVideoOcclusionState(VideoElement(), occlusion_state_, meets_visibility_threshold_, visibility_threshold_);
            std::move(request_visibility_callback_).Run(meets_visibility_threshold_);
        }
        return;
    }

    SCOPED_UMA_HISTOGRAM_TIMER("Media.MediaVideoVisibilityTracker.UpdateTime.TotalDuration");

    occlusion_state_ = {};
    ComputeAreaOccludedByViewport(*tracker_attached_to_document_->GetFrame()->View());
...
void MediaVideoVisibilityTracker::DidFinishLifecycleUpdate(const LocalFrameView& local_frame_view)
{
    if ((base::TimeTicks::Now() - last_hit_test_timestamp_ < hit_test_interval_) && !request_visibility_callback_) {
        return;
    }
