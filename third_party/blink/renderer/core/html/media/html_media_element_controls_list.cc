...
HTMLMediaElementControlsList::HTMLMediaElementControlsList(HTMLMediaElement* element)
    : DOMTokenList(*element, html_names::kControlslistAttr)
{
}

bool HTMLMediaElementControlsList::ValidateTokenValue(const AtomicString& token_value, ExceptionState&) const
{
    return token_value == keywords::kNodownload || token_value == keywords::kNofullscreen || token_value == keywords::kNoplaybackrate
...
{
    return ShouldHideDownload() || ShouldHideFullscreen() || ShouldHidePlaybackRate() || ShouldHideRemotePlayback();
}
