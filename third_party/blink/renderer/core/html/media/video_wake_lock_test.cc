...
class VideoWakeLockPictureInPictureSession final : public mojom::blink::PictureInPictureSession {
public:
    explicit VideoWakeLockPictureInPictureSession(mojo::PendingReceiver<mojom::blink::PictureInPictureSession> receiver)
    ...
    void Update(uint32_t player_id, mojo::PendingAssociatedRemote<media::mojom::blink::MediaPlayer> player_remote, const viz::SurfaceId&, const gfx::Size&,
...
// called in order for the events to be fired.
class VideoWakeLockPictureInPictureService final : public mojom::blink::PictureInPictureService {
public:
    VideoWakeLockPictureInPictureService()
        : receiver_(this)
    {
    }
    ...
    void StartSession(uint32_t, mojo::PendingAssociatedRemote<media::mojom::blink::MediaPlayer>, const viz::SurfaceId&, const gfx::Size&, bool,
        mojo::PendingRemote<mojom::blink::PictureInPictureSessionObserver>, const gfx::Rect&, StartSessionCallback callback) override
    {
        mojo::PendingRemote<mojom::blink::PictureInPictureSession> session_remote;
        session_ = std::make_unique<VideoWakeLockPictureInPictureSession>(session_remote.InitWithNewPipeAndPassReceiver());
...
public:
    ...
    void OnRequestPictureInPicture() override
    {
        ...
        surface_id_ = viz::SurfaceId(viz::FrameSinkId(1, 1), viz::LocalSurfaceId(11, base::UnguessableToken::CreateForTesting(0x111111, 0)));
    }
...
class VideoWakeLockTestWebFrameClient : public frame_test_helpers::TestWebFrameClient {
public:
    explicit VideoWakeLockTestWebFrameClient(std::unique_ptr<WebMediaPlayer> web_media_player)
        : web_media_player_(std::move(web_media_player))
    {
    }

    std::unique_ptr<WebMediaPlayer> CreateMediaPlayer(const WebMediaPlayerSource&, WebMediaPlayerClient* client, blink::MediaInspectorContext*,
...
public:
    void SetUp() override
    {
        ...
        client_ = std::make_unique<VideoWakeLockTestWebFrameClient>(std::move(media_player));

        helper_.Initialize(client_.get());
        helper_.Resize(kWindowSize);

        GetFrame().GetBrowserInterfaceBroker().SetBinderForTesting(
            mojom::blink::PictureInPictureService::Name_, WTF::BindRepeating(&VideoWakeLockPictureInPictureService::Bind, WTF::Unretained(&pip_service_)));

        fake_layer_ = cc::Layer::Create();

        GetDocument().body()->setInnerHTML("<body><div></div><video></video></body>");
        ...
        GetPage().SetVisibilityState(mojom::blink::PageVisibilityState::kVisible, true);
    }
    ...
    VideoWakeLock* GetVideoWakeLock() const
    {
        return video_wake_lock_.Get();
    }
    VideoWakeLockMediaPlayer* GetMediaPlayer() const
    {
    ...
    void SetFakeCcLayer(cc::Layer* layer)
    {
        video_->SetCcLayer(layer);
    }

    void SimulatePlaying()
    {
        video_wake_lock_->Invoke(GetFrame().DomWindow(), Event::Create(event_type_names::kPlaying));
    }

    void SimulatePause()
    {
        video_wake_lock_->Invoke(GetFrame().DomWindow(), Event::Create(event_type_names::kPause));
    }

    void SimulateEnterPictureInPicture()
    {
        PictureInPictureController::From(GetDocument()).EnterPictureInPicture(Video(), /*promise=*/nullptr);

        MakeGarbageCollected<WaitForEvent>(video_.Get(), event_type_names::kEnterpictureinpicture);
    }

    void SimulateLeavePictureInPicture()
    {
        PictureInPictureController::From(GetDocument()).ExitPictureInPicture(Video(), nullptr);

        MakeGarbageCollected<WaitForEvent>(video_.Get(), event_type_names::kLeavepictureinpicture);
    }

    void SimulateContextPause()
    {
        GetFrame().DomWindow()->SetLifecycleState(mojom::FrameLifecycleState::kPaused);
    }
    ...
    bool HasWakeLock()
    {
        return GetVideoWakeLock()->active_for_tests();
    }

    void SetDivHeight(double height_in_pixels)
    {
        div_->SetInlineStyleProperty(CSSPropertyID::kHeight, height_in_pixels, CSSPrimitiveValue::UnitType::kPixels);
        div_->SetInlineStyleProperty(CSSPropertyID::kWidth, kWindowSize.width(), CSSPrimitiveValue::UnitType::kPixels);
    }
...
TEST_F(VideoWakeLockTest, HiddingPageCancelsLock)
{
    ...
    GetPage().SetVisibilityState(mojom::blink::PageVisibilityState::kHidden, false);
    EXPECT_FALSE(HasWakeLock());
}
...
TEST_F(VideoWakeLockTest, ShowingPageRequestsLock)
{
    ...
    GetPage().SetVisibilityState(mojom::blink::PageVisibilityState::kVisible, false);
    EXPECT_TRUE(HasWakeLock());
}
...
{
    SimulatePlaying();
    GetVideoWakeLock()->OnRemotePlaybackStateChanged(mojom::blink::PresentationConnectionState::CLOSED);
    EXPECT_TRUE(HasWakeLock());
}
...
{
    SimulatePlaying();
    GetVideoWakeLock()->OnRemotePlaybackStateChanged(mojom::blink::PresentationConnectionState::CONNECTING);
    EXPECT_TRUE(HasWakeLock());
}
...
TEST_F(VideoWakeLockTest, ActiveRemotePlaybackCancelsLock)
{
    ...
    GetVideoWakeLock()->OnRemotePlaybackStateChanged(mojom::blink::PresentationConnectionState::CONNECTED);
    EXPECT_FALSE(HasWakeLock());
}
...
{
    ASSERT_LT(kSmallVideoSize.Area64() / static_cast<double>(kWindowSize.Area64()), GetVideoWakeLock()->GetSizeThresholdForTests());
    ASSERT_GT(kNormalVideoSize.Area64() / static_cast<double>(kWindowSize.Area64()), GetVideoWakeLock()->GetSizeThresholdForTests());
...
TEST_F(VideoWakeLockTest, MutedVideoTooFarOffscreenDoesNotTakeLock)
{
    ...
    SetDivHeight(kWindowSize.height() - kNormalVideoSize.height() + (1 - kThreshold) * kNormalVideoSize.height());
    ...
    SetDivHeight(kWindowSize.height() - kNormalVideoSize.height() + (1 - kThreshold * 1.10) * kNormalVideoSize.height());
    UpdateObservers();
    EXPECT_TRUE(HasWakeLock());
}
...
TEST_F(VideoWakeLockTest, VideoOnlyMediaStreamAlwaysTakesLock)
{
    ...
    auto* descriptor = MakeGarbageCollected<MediaStreamDescriptor>(dummy_components, dummy_components);
