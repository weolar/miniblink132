...
public:
    ...
    MOCK_METHOD(void, RecordVideoOcclusionState, (std::string_view occlusion_state));
    MOCK_METHOD(gfx::Size, NaturalSize, (), (const));
};
} // namespace

class HTMLVideoElementTest : public PaintTestConfigurations, public RenderingTest {
public:
    void SetUp() override
    {
        auto mock_media_player = std::make_unique<HTMLVideoElementMockMediaPlayer>();
    ...
    HTMLVideoElementMockMediaPlayer* MockMediaPlayer()
    {
        return media_player_;
    }
    ...
    MediaVideoVisibilityTracker::TrackerAttachedToDocument VideoVisibilityTrackerAttachedToDocument() const
    {
        DCHECK(video_);
        DCHECK(video_->visibility_tracker_for_tests());
        return video_->visibility_tracker_for_tests()->tracker_attached_to_document_;
    }
...
TEST_P(HTMLVideoElementTest, PictureInPictureInterstitial_Reattach)
{
    ...
    EXPECT_CALL(*MockWebMediaPlayer(), OnDisplayTypeChanged(DisplayType::kInline));
    EXPECT_CALL(*MockWebMediaPlayer(), HasAvailableVideoFrame()).WillRepeatedly(testing::Return(true));

    // Simulate entering Picture-in-Picture.
    video()->OnEnteredPictureInPicture();

    EXPECT_CALL(*MockWebMediaPlayer(), OnDisplayTypeChanged(DisplayType::kInline)).Times(3);
...
TEST_P(HTMLVideoElementTest, EffectivelyFullscreen_DisplayType)
{
    ...
    Vector<std::pair<WebFullscreenVideoStatus, DisplayType>> tests = {
        { WebFullscreenVideoStatus::kNotEffectivelyFullscreen, DisplayType::kInline },
        { WebFullscreenVideoStatus::kFullscreenAndPictureInPictureEnabled, DisplayType::kFullscreen },
        { WebFullscreenVideoStatus::kFullscreenAndPictureInPictureDisabled, DisplayType::kFullscreen },
        { WebFullscreenVideoStatus::kNotEffectivelyFullscreen, DisplayType::kInline },
    };
...
TEST_P(HTMLVideoElementTest, ChangeLayerNeedsCompositingUpdate)
{
    ...
    auto* painting_layer = To<LayoutBoxModelObject>(video()->GetLayoutObject())->PaintingLayer();
...
TEST_P(HTMLVideoElementTest, HasAvailableVideoFrameChecksWMP)
{
    ...
    EXPECT_CALL(*MockWebMediaPlayer(), HasAvailableVideoFrame()).WillOnce(testing::Return(false)).WillOnce(testing::Return(true));
...
TEST_P(HTMLVideoElementTest, DefaultPosterImage)
{
    ...
    video->setAttribute(html_names::kPosterAttr, AtomicString("http://www.example.com/bar.jpg"));
    EXPECT_FALSE(video->IsDefaultPosterImageURL());
    EXPECT_NE(kDefaultPosterImage, video->PosterImageURL());
}

TEST_P(HTMLVideoElementTest, RecordVideoOcclusionStateCalledWhenVisibilityIsRequested)
{
    video()->SetSrc(AtomicString("http://example.com/foo.mp4"));
    test::RunPendingTasks();
    UpdateAllLifecyclePhasesForTest();
    ...
    const std::string expected_occlusion_state = "has sufficiently visible video: {True}, occluded area: {0.00}, "
    ...
    EXPECT_CALL((*MockMediaPlayer()), RecordVideoOcclusionState(expected_occlusion_state));
...
TEST_P(HTMLVideoElementTest, RecordVideoOcclusionStateNotCalledIfVisibilityIsNotRequested)
{
    video()->SetSrc(AtomicString("http://example.com/foo.mp4"));
    test::RunPendingTasks();
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(HTMLVideoElementTest, RecordVideoOcclusionStateCalledWhenTrackerNotAttached)
{
    video()->SetSrc(AtomicString("http://example.com/foo.mp4"));
    test::RunPendingTasks();
    UpdateAllLifecyclePhasesForTest();
    ...
    const std::string expected_occlusion_state = "has sufficiently visible video: {False}, occluded area: {0.00}, "
...
TEST_P(HTMLVideoElementTest, VideoVisibilityTrackerVideoElementRectDimensions)
{
    ...
    EXPECT_CALL(*MockMediaPlayer(), NaturalSize()).WillRepeatedly(Return(expected_natural_size));

    video()->SetSrc(AtomicString("http://example.com/foo.mp4"));
    video()->setAttribute(html_names::kStyleAttr, AtomicString("width: 300px; height: 300px"));
    test::RunPendingTasks();
    UpdateAllLifecyclePhasesForTest();
    ASSERT_EQ(VideoVisibilityTracker(), nullptr);
    ...
    video()->Play();
    EXPECT_TRUE(video()->GetWebMediaPlayer());
    ASSERT_EQ(expected_natural_size, gfx::Size(video()->videoWidth(), video()->videoHeight()));
    ASSERT_NE(VideoVisibilityTracker(), nullptr);
    EXPECT_NE(VideoVisibilityTrackerAttachedToDocument(), nullptr);
    ...
    EXPECT_EQ(box->AbsoluteBoundingBoxRect().width(), occlusion_state.video_element_rect.Width());
    EXPECT_GT(box->AbsoluteBoundingBoxRect().height(), occlusion_state.video_element_rect.Height());

    // Verify that the `video_element_rect` intersects the `HTMLVideoElement`
    // bounds.
    const auto intersection = Intersection(VisualRectInDocument(*box), occlusion_state.video_element_rect);
