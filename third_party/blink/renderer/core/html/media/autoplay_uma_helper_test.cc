...
public:
    ...
    {
        ON_CALL(*this, HandleContextDestroyed()).WillByDefault(testing::Invoke(this, &MockAutoplayUmaHelper::ReallyHandleContextDestroyed));
    }

    void HandlePlayingEvent()
    {
        AutoplayUmaHelper::HandlePlayingEvent();
    }
...
private:
    ...
    {
        PageTestBase::SetUp();
        GetDocument().documentElement()->setInnerHTML("<video id=video></video>", ASSERT_NO_EXCEPTION);
