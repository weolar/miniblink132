...
public:
    FakeWebMediaPlayer(WebMediaPlayerClient* client, ExecutionContext* context, double duration)
    ...
    MOCK_METHOD1(SetIsEffectivelyFullscreen, void(blink::WebFullscreenVideoStatus));
...
private:
    void ScheduleTimeIncrement()
    {
        ...
        context_->GetTaskRunner(TaskType::kInternalMediaRealTime)
            ->PostDelayedTask(FROM_HERE, WTF::BindOnce(&FakeWebMediaPlayer::AutoTimeIncrement, WTF::Unretained(this), auto_time_increment_delta_.value()),
    ...
    bool playing_ = false;
    std::optional<base::TimeDelta> auto_time_increment_delta_ = base::Milliseconds(33);
...
public:
    std::unique_ptr<WebMediaPlayer> CreateWebMediaPlayer(HTMLMediaElement& element, const WebMediaPlayerSource&, WebMediaPlayerClient* client) override
    {
        return std::make_unique<FakeWebMediaPlayer>(client, element.GetExecutionContext(), media_duration_);
    }
...
protected:
    ...
    HTMLMediaElementEventListenersTest(base::test::TaskEnvironment::TimeSource time_source)
        : PageTestBase(time_source)
    {
    }
    void SetUp() override
    {
        SetupPageWithClients(nullptr, MakeGarbageCollected<MediaStubLocalFrameClient>());
    }
...
TEST_F(HTMLMediaElementEventListenersTest, ReInsertingInDocumentCollectsControls)
{
    EXPECT_EQ(Video(), nullptr);
    GetDocument().body()->setInnerHTML("<video controls></video>");
    EXPECT_NE(Video(), nullptr);
...
TEST_F(HTMLMediaElementEventListenersTest, FullscreenDetectorTimerCancelledOnContextDestroy)
{
    EXPECT_EQ(Video(), nullptr);
    ...
    Fullscreen::DidResolveEnterFullscreenRequest(GetDocument(), true /* granted */);

    test::RunPendingTasks();

    Persistent<Document> persistent_document = &GetDocument();
    Persistent<MediaCustomControlsFullscreenDetector> detector = FullscreenDetector();

    Vector<blink::WebFullscreenVideoStatus> observed_results;

    ON_CALL(*WebMediaPlayer(), SetIsEffectivelyFullscreen(_)).WillByDefault(testing::Invoke([&](blink::WebFullscreenVideoStatus fullscreen_video_status) {
    ...
    EXPECT_EQ(blink::WebFullscreenVideoStatus::kNotEffectivelyFullscreen, observed_results[0]);
}
...
class HTMLMediaElementWithMockSchedulerTest : public HTMLMediaElementEventListenersTest {
protected:
    ...
    HTMLMediaElementWithMockSchedulerTest()
        : HTMLMediaElementEventListenersTest(base::test::TaskEnvironment::TimeSource::MOCK_TIME)
    {
    }
    ...
    void SetUp() override
    {
        ...
        auto start = base::TimeTicks::Now().SnappedToNextTick(base::TimeTicks(), base::Seconds(1));
    ...
    {
        return s_platform_clock_->NowTicks();
    }
...
TEST_F(HTMLMediaElementWithMockSchedulerTest, ShowPosterFlag_FalseAfterLoop)
{
    ...
    GetDocument().body()->setInnerHTML("<video loop src=\"http://example.com\"></video>");
    platform()->RunUntilIdle();
    EXPECT_NE(WebMediaPlayer(), nullptr);
...
TEST_F(HTMLMediaElementWithMockSchedulerTest, ShowPosterFlag_FalseAfterEnded)
{
    ...
    GetDocument().body()->setInnerHTML("<video src=\"http://example.com\"></video>");
    platform()->RunUntilIdle();
    EXPECT_NE(WebMediaPlayer(), nullptr);
    EXPECT_EQ(WebMediaPlayer()->Duration(), 10.0);
...
public:
    void Invoke(ExecutionContext* ctx, Event* event) override
    {
        if (event->type() == event_type_names::kEnter) {
            EXPECT_TRUE(event->target()->GetWrapperTypeInfo()->Equals(VTTCue::GetStaticWrapperTypeInfo()));
...
private:
    ...
    void OnCueExit(HTMLMediaElement* media_element, VTTCue* cue)
    {
        ...
        double const diff_seconds = std::fabs(media_element->currentTime() - cue->endTime());
        cue_event_deltas_[cue_index].exit_time_delta = base::Seconds(diff_seconds);
    }

    std::array<CueChangeEventTimeDelta, std::size(kTestCueData)> cue_event_deltas_;
};
...
TEST_F(HTMLMediaElementWithMockSchedulerTest, CueEnterExitEventLatency)
{
    ...
    auto* text_track = Video()->addTextTrack(V8TextTrackKind(V8TextTrackKind::Enum::kSubtitles), g_empty_atom, g_empty_atom, ASSERT_NO_EXCEPTION);

    auto* listener = MakeGarbageCollected<CueEventListener>();
    for (auto cue_data : kTestCueData) {
        VTTCue* cue = MakeGarbageCollected<VTTCue>(GetDocument(), cue_data.start_time, cue_data.end_time, cue_data.text);
