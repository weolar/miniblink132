    ...
    , visibility_threshold_(kStrictVisibilityThreshold)
{
    ...
    VideoElement().addEventListener(event_type_names::kEnterpictureinpicture, this, true);
    VideoElement().addEventListener(event_type_names::kLeavepictureinpicture, this, true);
    VideoElement().addEventListener(event_type_names::kVolumechange, this, true);
    StartIntersectionObserver();

    RemotePlaybackController* remote_playback_controller = RemotePlaybackController::From(VideoElement());
...
void VideoWakeLock::OnVisibilityChanged(const HeapVector<Member<IntersectionObserverEntry>>& entries)
{
    is_visible_ = entries.back()->intersectionRatio() > visibility_threshold_;
    Update();
}

void VideoWakeLock::OnSizeChanged(const HeapVector<Member<IntersectionObserverEntry>>& entries)
{
...
void VideoWakeLock::Invoke(ExecutionContext*, Event* event)
{
    if (event->type() == event_type_names::kPlaying) {
        playing_ = true;
    } else if (event->type() == event_type_names::kPause || event->type() == event_type_names::kEmptied) {
        ...
        DCHECK(event->type() == event_type_names::kEnterpictureinpicture || event->type() == event_type_names::kLeavepictureinpicture
...
void VideoWakeLock::OnRemotePlaybackStateChanged(mojom::blink::PresentationConnectionState state)
{
...
bool VideoWakeLock::ShouldBeActive() const
{
    ...
    bool in_picture_in_picture = PictureInPictureController::IsElementInPictureInPicture(&VideoElement());
    bool context_is_running = VideoElement().GetExecutionContext() && !VideoElement().GetExecutionContext()->IsContextPaused();
    ...
    bool is_size_exempt = VideoElement().GetLoadType() == WebMediaPlayer::kLoadTypeMediaStream;
    ...
    bool visibility_requirements_met = VideoElement().HasVideo() && (in_picture_in_picture || (page_visible && ((is_visible_ && is_big_enough) || has_audio)));
    ...
    return playing_ && visibility_requirements_met && remote_playback_state_ != mojom::blink::PresentationConnectionState::CONNECTED && context_is_running;
}
...
void VideoWakeLock::EnsureWakeLockService()
{
    ...
    scoped_refptr<base::SingleThreadTaskRunner> task_runner = frame->GetTaskRunner(TaskType::kMediaElementEvent);

    mojo::Remote<blink::mojom::blink::WakeLockService> service;
    frame->GetBrowserInterfaceBroker().GetInterface(service.BindNewPipeAndPassReceiver(task_runner));
    service->GetWakeLock(device::mojom::WakeLockType::kPreventDisplaySleep, device::mojom::blink::WakeLockReason::kVideoPlayback, "Video Wake Lock",
        wake_lock_service_.BindNewPipeAndPassReceiver(task_runner));
    wake_lock_service_.set_disconnect_handler(WTF::BindOnce(&VideoWakeLock::OnConnectionError, WrapWeakPersistent(this)));
}
...
void VideoWakeLock::StartIntersectionObserver()
{
    ...
    visibility_observer_ = IntersectionObserver::Create(VideoElement().GetDocument(),
        WTF::BindRepeating(&VideoWakeLock::OnVisibilityChanged, WrapWeakPersistent(this)), LocalFrameUkmAggregator::kMediaIntersectionObserver,
    ...
    size_observer_ = IntersectionObserver::Create(VideoElement().GetDocument().TopDocument(),
        WTF::BindRepeating(&VideoWakeLock::OnSizeChanged, WrapWeakPersistent(this)), LocalFrameUkmAggregator::kMediaIntersectionObserver,
        IntersectionObserver::Params {
