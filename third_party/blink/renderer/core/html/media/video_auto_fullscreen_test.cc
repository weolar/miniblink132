...
public:
    ...
    void EnterFullscreen(mojom::blink::FullscreenOptionsPtr options, EnterFullscreenCallback callback) override
    {
        ...
        web_view_->MainFrameImpl()
            ...
            ->PostTask(FROM_HERE, WTF::BindOnce([](WebViewImpl* web_view) { web_view->DidEnterFullscreen(); }, WTF::Unretained(web_view_)));
    }
    ...
    void ExitFullscreen() override
    {
        web_view_->MainFrameImpl()
            ->GetTaskRunner(TaskType::kInternalNavigationAssociated)
            ->PostTask(FROM_HERE, WTF::BindOnce([](WebViewImpl* web_view) { web_view->DidExitFullscreen(); }, WTF::Unretained(web_view_)));
    }

    void set_web_view(WebViewImpl* web_view)
    {
        web_view_ = web_view;
    }
...
class VideoAutoFullscreenFrameClient : public frame_test_helpers::TestWebFrameClient {
public:
...
class VideoAutoFullscreen : public testing::Test, private ScopedVideoAutoFullscreenForTest {
public:
    VideoAutoFullscreen()
        : ScopedVideoAutoFullscreenForTest(true)
    {
    }
    void SetUp() override
    {
        ...
        GetWebView()->GetSettings()->SetAutoplayPolicy(mojom::AutoplayPolicy::kUserGestureRequired);
