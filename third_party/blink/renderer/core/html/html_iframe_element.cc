...
const AttrNameToTrustedType& HTMLIFrameElement::GetCheckedAttributeTypes() const
{
    DEFINE_STATIC_LOCAL(AttrNameToTrustedType, attribute_map, ({ { "srcdoc", SpecificTrustedType::kHTML } }));
...
DOMFeaturePolicy* HTMLIFrameElement::featurePolicy()
{
    if (!policy_ && GetExecutionContext()) {
        policy_ = MakeGarbageCollected<IFramePolicy>(GetExecutionContext(), GetFramePolicy().container_policy, GetOriginForPermissionsPolicy());
    }
    return policy_.Get();
}
...
{
    if (name == html_names::kWidthAttr || name == html_names::kHeightAttr || name == html_names::kAlignAttr || name == html_names::kFrameborderAttr) {
        return true;
    }
    return HTMLFrameElementBase::IsPresentationAttribute(name);
}

void HTMLIFrameElement::CollectStyleForPresentationAttribute(const QualifiedName& name, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
    if (name == html_names::kWidthAttr) {
        ...
        if (!value.ToInt()) {
            ...
            for (CSSPropertyID property_id :
                ...
                AddPropertyToPresentationAttributeStyle(style, property_id, 0, CSSPrimitiveValue::UnitType::kPixels);
            }
        }
    } else {
...
void HTMLIFrameElement::ParseAttribute(const AttributeModificationParams& params)
{
    ...
    if (name == html_names::kNameAttr) {
        ...
        if (name_.Contains('\n') && name_.Contains('<')) {
            ...
            if (!name_.EndsWith('>')) {
                UseCounter::Count(GetDocument(), WebFeature::kDanglingMarkupInWindowNameNotEndsWithGT);
                if (!name_.EndsWith('\n')) {
                    UseCounter::Count(GetDocument(), WebFeature::kDanglingMarkupInWindowNameNotEndsWithNewLineOrGT);
                }
            }
        }
        ...
        referrer_policy_ = network::mojom::ReferrerPolicy::kDefault;
        if (!value.IsNull()) {
            SecurityPolicy::ReferrerPolicyFromString(value, kSupportReferrerPolicyLegacyKeywords, &referrer_policy_);
            UseCounter::Count(GetDocument(), WebFeature::kHTMLIFrameElementReferrerPolicyAttribute);
        }
        ...
        if (allow_fullscreen_ != old_allow_fullscreen) {
            ...
            if (allow_fullscreen_ && ContentFrame()) {
                UseCounter::Count(GetDocument(), WebFeature::kHTMLIFrameElementAllowfullscreenAttributeSetAfterContentLoad);
            }
            FrameOwnerPropertiesChanged();
        ...
        if (value && (value.Contains('\n') || value.Contains('\r') || !MatchesTheSerializedCSPGrammar(value.GetString()))) {
            ...
            GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(
                mojom::blink::ConsoleMessageSource::kOther, mojom::blink::ConsoleMessageLevel::kError, "'csp' attribute is invalid: " + value));
        ...
        if (GetExecutionContext() && RuntimeEnabledFeatures::TopicsAPIEnabled(GetExecutionContext()) && GetExecutionContext()->IsSecureContext()) {
            bool old_browsing_topics = !params.old_value.IsNull();
            bool new_browsing_topics = !params.new_value.IsNull();

            if (new_browsing_topics) {
                UseCounter::Count(GetDocument(), WebFeature::kIframeBrowsingTopicsAttribute);
    ...
    } else if (name == html_names::kAdauctionheadersAttr && GetExecutionContext()) {
        if (!GetExecutionContext()->IsSecureContext()) {
            ...
            if (!params.new_value.IsNull()) {
                UseCounter::Count(GetDocument(), WebFeature::kSharedStorageAPI_Iframe_Attribute);
            }
        }
    } else if (name == html_names::kSharedstoragewritableAttr && GetExecutionContext()
        && RuntimeEnabledFeatures::SharedStorageAPIM118Enabled(GetExecutionContext())) {
        if (!GetExecutionContext()->IsSecureContext()) {
            GetDocument().AddConsoleMessage(
                MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kOther, mojom::blink::ConsoleMessageLevel::kError,
    ...
    } else if (name == html_names::kCredentiallessAttr && RuntimeEnabledFeatures::AnonymousIframeEnabled()) {
        ...
        // page.
        if (name == AtomicString("gesture") && value == AtomicString("media") && GetDocument().Loader()
            && !GetDocument().Loader()->GetUseCounter().IsCounted(WebFeature::kHTMLIFrameElementGestureMedia)) {
            UseCounter::Count(GetDocument(), WebFeature::kHTMLIFrameElementGestureMedia);
            GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kOther, mojom::ConsoleMessageLevel::kWarning,
...
{
    if (!RuntimeEnabledFeatures::DocumentPolicyNegotiationEnabled(GetExecutionContext())) {
        return {};
    }

    if (!required_policy_.empty()) {
        UseCounter::Count(GetDocument(), mojom::blink::WebFeature::kDocumentPolicyIframePolicyAttribute);
    }

    PolicyParserMessageBuffer logger;
    DocumentPolicy::ParsedDocumentPolicy new_required_policy
        = DocumentPolicyParser::Parse(required_policy_, logger).value_or(DocumentPolicy::ParsedDocumentPolicy {});
    ...
    if (!new_required_policy.endpoint_map.empty()) {
        GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kOther,
            mojom::blink::ConsoleMessageLevel::kWarning, "Iframe policy attribute cannot specify reporting endpoint."));
    }
    ...
    for (const auto& policy_entry : new_required_policy.feature_state) {
        ...
        if (!GetDocument().DocumentPolicyFeatureObserved(feature)) {
            UMA_HISTOGRAM_ENUMERATION("Blink.UseCounter.DocumentPolicy.PolicyAttribute", feature);
        }
    }
...
ParsedPermissionsPolicy HTMLIFrameElement::ConstructContainerPolicy() const
{
    ...
    if (AllowFullscreen()) {
        bool policy_changed = AllowFeatureEverywhereIfNotPresent(mojom::blink::PermissionsPolicyFeature::kFullscreen, container_policy);
        if (!policy_changed) {
            logger.Warn("Allow attribute will take precedence over 'allowfullscreen'.");
        }
    }
    // If the allowpaymentrequest attribute is present and no 'payment' policy is
    // set, enable the feature for all origins.
    if (AllowPaymentRequest()) {
        bool policy_changed = AllowFeatureEverywhereIfNotPresent(mojom::blink::PermissionsPolicyFeature::kPayment, container_policy);
        // Measure cases where allowpaymentrequest had an actual effect, to see if
        // we can deprecate it. See https://crbug.com/1127988
        if (policy_changed) {
            UseCounter::Count(GetDocument(), WebFeature::kAllowPaymentRequestAttributeHasEffect);
        } else {
            logger.Warn("Allow attribute will take precedence over 'allowpaymentrequest'.");
        }
    }

    // Factor in changes in client hint permissions.
    UpdateIFrameContainerPolicyWithDelegationSupportForClientHints(container_policy, GetDocument().domWindow());
...
{
    return ContentFrame() && !collapsed_by_client_ && HTMLElement::LayoutObjectIsNeeded(style);
}
...
Node::InsertionNotificationRequest HTMLIFrameElement::InsertedInto(ContainerNode& insertion_point)
{
    InsertionNotificationRequest result = HTMLFrameElementBase::InsertedInto(insertion_point);
...
network::mojom::blink::TrustTokenParamsPtr HTMLIFrameElement::ConstructTrustTokenParams() const
{
    if (!trust_token_) {
        return nullptr;
    }
    ...
    std::unique_ptr<JSONValue> parsed_attribute = ParseJSON(trust_token_, &parse_error);
    if (!parsed_attribute) {
        GetDocument().AddConsoleMessage(
            MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kOther, mojom::blink::ConsoleMessageLevel::kError,
                "iframe trusttoken attribute was invalid JSON: " + parse_error.message
                    + String::Format(" (line %d, col %d)", parse_error.line, parse_error.column)));
        return nullptr;
    }

    network::mojom::blink::TrustTokenParamsPtr parsed_params = internal::TrustTokenParamsFromJson(std::move(parsed_attribute));
    if (!parsed_params) {
        GetDocument().AddConsoleMessage(
            MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kOther, mojom::blink::ConsoleMessageLevel::kError,
    ...
    if (parsed_params->operation != network::mojom::blink::TrustTokenOperationType::kSigning) {
        GetDocument().AddConsoleMessage(
            MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kOther, mojom::blink::ConsoleMessageLevel::kError,
    ...
    if (!GetExecutionContext()->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kTrustTokenRedemption)) {
...
void HTMLIFrameElement::DidChangeAttributes()
{
    ...
    Vector<network::mojom::blink::ContentSecurityPolicyPtr> csp = ParseContentSecurityPolicies(
        required_csp_, network::mojom::blink::ContentSecurityPolicyType::kEnforce, network::mojom::blink::ContentSecurityPolicySource::kHTTP, KURL());
    ...
    if (RuntimeEnabledFeatures::TopicsAPIEnabled(GetExecutionContext()) && GetExecutionContext()->IsSecureContext()) {
        attributes->browsing_topics = !FastGetAttribute(html_names::kBrowsingtopicsAttr).IsNull();
    }

    if (GetExecutionContext()->IsSecureContext()) {
        attributes->ad_auction_headers = !FastGetAttribute(html_names::kAdauctionheadersAttr).IsNull();
    }

    if (RuntimeEnabledFeatures::SharedStorageAPIM118Enabled(GetExecutionContext()) && GetExecutionContext()->IsSecureContext()) {
        attributes->shared_storage_writable_opted_in = !FastGetAttribute(html_names::kSharedstoragewritableAttr).IsNull();
    }

    attributes->id = ConvertToReportValue(id_);
    attributes->name = ConvertToReportValue(name_);
    attributes->src = ConvertToReportValue(src_);
    GetDocument().GetFrame()->GetLocalFrameHostRemote().DidChangeIframeAttributes(ContentFrame()->GetFrameToken(), std::move(attributes));
    ...
    GetDocument().GetFrame()->GetLocalFrameHostRemote().DidChangeSrcDoc(ContentFrame()->GetFrameToken(), srcdoc_value);
}
