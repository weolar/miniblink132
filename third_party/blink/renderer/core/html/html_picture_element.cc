...
void HTMLPictureElement::SourceChanged(ImageSourceChangeType change_type)
{
    ImageLoader::UpdateFromElementBehavior update_behavior
        = change_type == ImageSourceChangeType::kMedia ? ImageLoader::kUpdateSizeChanged : ImageLoader::kUpdateNormal;
    for (HTMLImageElement& image_element : Traversal<HTMLImageElement>::ChildrenOf(*this)) {
...
{
    for (HTMLSourceElement& source_element : Traversal<HTMLSourceElement>::ChildrenOf(*this)) {
...
Node::InsertionNotificationRequest HTMLPictureElement::InsertedInto(ContainerNode& insertion_point)
{
