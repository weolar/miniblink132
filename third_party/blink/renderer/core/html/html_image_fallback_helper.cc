...
static bool TreatImageAsReplaced(const Document& document, const StyleUAShadowHostData& host_data)
{
    // TODO(https://crbug.com/313072): Is this still correct in the presence of
    // intrinsic sizing keywords or calc-size?
    bool has_intrinsic_dimensions = !host_data.Width().IsAuto() && !host_data.Height().IsAuto();
    bool has_dimensions_from_ar = !host_data.AspectRatio().IsAuto() && (!host_data.Width().IsAuto() || !host_data.Height().IsAuto());
    bool has_no_alt_attribute = host_data.AltAttribute().empty();
    return (has_intrinsic_dimensions || has_dimensions_from_ar) && (document.InQuirksMode() || has_no_alt_attribute);
}
...
public:
    ...
    void AdjustStyle(ComputedStyleBuilder& builder) override
    {
        ...
        if (GetDocument().InQuirksMode() && !host_data.Width().IsAuto() && !host_data.Height().IsAuto()) {
...
private:
    void ShowAsReplaced(ComputedStyleBuilder& builder, const Length& width, const Length& height)
    {
...
void HTMLImageFallbackHelper::CreateAltTextShadowTree(Element& element)
{
    ...
    container->setAttribute(html_names::kIdAttr, AtomicString("alttext-container"));

    auto* broken_image = MakeGarbageCollected<HTMLAltTextImageElement>(document);
    broken_image->SetIsFallbackImage();
    broken_image->setAttribute(html_names::kIdAttr, AtomicString("alttext-image"));
    ...
    broken_image->SetInlineStyleProperty(CSSPropertyID::kMargin, 0, CSSPrimitiveValue::UnitType::kPixels);
...
void HTMLImageFallbackHelper::AdjustHostStyle(HTMLElement& element, ComputedStyleBuilder& builder)
{
    ...
    builder.SetUAShadowHostData(std::make_unique<StyleUAShadowHostData>(builder.Width(), builder.Height(), builder.AspectRatio(), element.AltText(),
        element.getAttribute(html_names::kAltAttr), element.getAttribute(html_names::kSrcAttr), /* has_appearance */ false));

    if (!TreatImageAsReplaced(element.GetDocument(), *builder.UAShadowHostData())) {
