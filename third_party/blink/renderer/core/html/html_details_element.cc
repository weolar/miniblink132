...
void HTMLDetailsElement::DispatchPendingEvent(const AttributeModificationReason reason)
{
...
{
    auto* default_summary = MakeGarbageCollected<HTMLSummaryElement>(GetDocument());
    default_summary->AppendChild(Text::Create(GetDocument(), GetLocale().QueryString(IDS_DETAILS_WITHOUT_SUMMARY_LABEL)));
    ...
    content_slot_->SetInlineStyleProperty(CSSPropertyID::kContentVisibility, CSSValueID::kHidden);
    content_slot_->EnsureDisplayLockContext().SetIsDetailsSlotElement(true);
    content_slot_->SetInlineStyleProperty(CSSPropertyID::kDisplay, CSSValueID::kBlock);
    root.AppendChild(content_slot_);

    auto* default_summary_style = MakeGarbageCollected<HTMLStyleElement>(GetDocument());
...
Element* HTMLDetailsElement::FindMainSummary() const
{
    if (HTMLSummaryElement* summary = Traversal<HTMLSummaryElement>::FirstChild(*this))
        return summary;
    HTMLSlotElement& slot = To<HTMLSlotElement>(*UserAgentShadowRoot()->firstChild());
...
void HTMLDetailsElement::ManuallyAssignSlots()
{
    ...
    for (Node& child : NodeTraversal::ChildrenOf(*this)) {
        ...
        bool is_first_summary = summary_nodes.empty() && IsA<HTMLSummaryElement>(child);
        if (is_first_summary) {
...
void HTMLDetailsElement::ParseAttribute(const AttributeModificationParams& params)
{
    CHECK(params.reason != AttributeModificationReason::kByParser || !parentNode()) << "This code depends on the parser setting attributes before inserting "
    ...
    if (params.name == html_names::kOpenAttr) {
        ...
        pending_event_task_ = PostCancellableTask(*GetDocument().GetTaskRunner(TaskType::kDOMManipulation), FROM_HERE,
            WTF::BindOnce(&HTMLDetailsElement::DispatchPendingEvent, WrapPersistent(this), params.reason));

        Element* content = EnsureUserAgentShadowRoot(SlotAssignmentMode::kManual).getElementById(shadow_element_names::kIdDetailsContent);
        DCHECK(content);
        ...
        if (is_open_) {
            ...
            CHECK_NE(params.reason, AttributeModificationReason::kBySynchronizationOfLazyAttribute);
            // TODO(https://crbug.com/1444057): Should this be in
            // AttributeChanged instead?
            if (!GetName().empty() && params.reason == AttributeModificationReason::kDirectly) {
                // Don't fire mutation events for any changes to the open attribute
                // that this causes.
                MutationEventSuppressionScope scope(GetDocument());

                HeapVector<Member<HTMLDetailsElement>> details_with_name(OtherElementsInNameGroup());
                for (HTMLDetailsElement* other_details : details_with_name) {
                    CHECK_NE(other_details, this);
                    UseCounter::Count(GetDocument(), WebFeature::kHTMLDetailsElementNameAttributeClosesOther);
                    other_details->setAttribute(html_names::kOpenAttr, g_null_atom);
                }
            }
        } else {
            if (!RuntimeEnabledFeatures::DetailsStylingEnabled()) {
                content->SetInlineStyleProperty(CSSPropertyID::kDisplay, CSSValueID::kBlock);
            }
            content->SetInlineStyleProperty(CSSPropertyID::kContentVisibility, CSSValueID::kHidden);
...
void HTMLDetailsElement::AttributeChanged(const AttributeModificationParams& params)
{
    const QualifiedName& name = params.name;
    if (name == html_names::kNameAttr) {
        if (!params.new_value.empty()) {
            UseCounter::Count(GetDocument(), WebFeature::kHTMLDetailsElementNameAttribute);
        }
...
Node::InsertionNotificationRequest HTMLDetailsElement::InsertedInto(ContainerNode& insertion_point)
{
...
void HTMLDetailsElement::MaybeCloseForExclusivity()
{
    ...
    for (HTMLDetailsElement* other_details : details_with_name) {
        ...
        if (other_details->is_open_) {
            ...
            UseCounter::Count(GetDocument(), WebFeature::kHTMLDetailsElementNameAttributeClosesSelf);
            ToggleOpen();
            break;
        }
    }
...
HeapVector<Member<HTMLDetailsElement>> HTMLDetailsElement::OtherElementsInNameGroup()
{
...
bool HTMLDetailsElement::ExpandDetailsAncestors(const Node& node)
{
    ...
    for (Node& parent : FlatTreeTraversal::AncestorsOf(node)) {
        if (HTMLDetailsElement* details = DynamicTo<HTMLDetailsElement>(parent)) {
            ...
            if (!inside_summary && !details->FastHasAttribute(html_names::kOpenAttr)) {
...
bool HTMLDetailsElement::IsValidBuiltinCommand(HTMLElement& invoker, CommandEventType command)
{
    bool parent_is_valid = HTMLElement::IsValidBuiltinCommand(invoker, command);
    if (!RuntimeEnabledFeatures::HTMLInvokeActionsV2Enabled()) {
        return parent_is_valid;
    }
    return parent_is_valid || command == CommandEventType::kToggle || command == CommandEventType::kOpen || command == CommandEventType::kClose;
}

bool HTMLDetailsElement::HandleCommandInternal(HTMLElement& invoker, CommandEventType command)
{
