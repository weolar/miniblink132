...
public:
    ...
    void Invoke(ExecutionContext*, Event* event) override
    {
        ...
        if (!parsed_json_value_ && opt_error_.type == JSONParseErrorType::kNoError) {
...
public:
    explicit JSONDocumentParser(JSONDocument& document, ParserSynchronizationPolicy sync_policy)
...
private:
    ...
    void CreateDocumentStructure()
    {
        ...
        auto* meta_charset = MakeGarbageCollected<HTMLMetaElement>(*GetDocument(), CreateElementFlags());
        ...
        pre_ = MakeGarbageCollected<HTMLPreElement>(html_names::kPreTag, *GetDocument());

        auto* label = MakeGarbageCollected<HTMLLabelElement>(*GetDocument());
        label->ParserAppendChild(Text::Create(*GetDocument(), WTF::AtomicString(Locale::DefaultLocale().QueryString(IDS_PRETTY_PRINT_JSON))));
        ...
        checkbox->addEventListener(event_type_names::kChange, MakeGarbageCollected<PrettyPrintJSONListener>(pre_, checkbox),
            /*use_capture=*/false);
        checkbox->setAttribute(html_names::kAriaLabelAttr, WTF::AtomicString(Locale::DefaultLocale().QueryString(IDS_PRETTY_PRINT_JSON)));
        ...
        div->setAttribute(html_names::kClassAttr, AtomicString("json-formatter-container"));
...
{
    return MakeGarbageCollected<JSONDocumentParser>(*this, GetParserSynchronizationPolicy());
}
} // namespace blink
