...
HTMLQuoteElement::HTMLQuoteElement(const QualifiedName& tag_name, Document& document)
    : HTMLElement(tag_name, document)
{
    DCHECK(HasTagName(html_names::kQTag) || HasTagName(html_names::kBlockquoteTag));
}

void HTMLQuoteElement::AdjustPseudoStyleLocale(ComputedStyleBuilder& pseudo_style_builder)
{
    ...
    if (parent) {
        font_description.SetLocale(LayoutLocale::Get(parent->ComputeInheritedLanguage()));
    } else {
...
{
    return attribute.GetName() == html_names::kCiteAttr || HTMLElement::IsURLAttribute(attribute);
}

bool HTMLQuoteElement::HasLegalLinkAttribute(const QualifiedName& name) const
{
    return name == html_names::kCiteAttr || HTMLElement::HasLegalLinkAttribute(name);
}
