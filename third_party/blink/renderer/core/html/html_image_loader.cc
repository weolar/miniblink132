...
void HTMLImageLoader::DispatchLoadEvent()
{
    ...
    if (IsA<HTMLObjectElement>(*GetElement()) && GetContent()->GetResponse().HttpStatusCode() >= 400) {
        DispatchErrorEvent();
        return;
    }
...
void HTMLImageLoader::ImageNotifyFinished(ImageResourceContent*)
{
    ...
    if ((load_error || cached_image->GetResponse().HttpStatusCode() >= 400) && html_object_element) {
        ...
        html_object_element->RenderFallbackContent(HTMLObjectElement::ErrorEventPolicy::kDoNotDispatch);
    }
}
