...
TEST(HTMLTableRowElementTest, rowIndex_notInTable)
{
    ...
    EXPECT_EQ(-1, row->rowIndex()) << "rows not in tables should have row index -1";
}
...
TEST(HTMLTableRowElementTest, rowIndex_directChildOfTable)
{
    ...
    EXPECT_EQ(0, row->rowIndex()) << "rows that are direct children of a table should have a row index";
}
...
TEST(HTMLTableRowElementTest, rowIndex_inUnrelatedElementInTable)
{
    ...
    EXPECT_EQ(-1, row->rowIndex()) << "rows in a table, but within an unrelated element, should have "
                                   << "row index -1";
}
