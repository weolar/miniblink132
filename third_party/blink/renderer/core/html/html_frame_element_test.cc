...
TEST_F(HTMLFrameElementTest, DefaultContainerPolicy)
{
    ...
    auto* document = MakeGarbageCollected<Document>(DocumentInit::Create().ForTest(execution_context.GetExecutionContext()).WithURL(document_url));

    auto* frame_element = MakeGarbageCollected<HTMLFrameElement>(*document);

    frame_element->setAttribute(html_names::kSrcAttr, AtomicString("http://example.net/"));
    frame_element->UpdateContainerPolicyForTests();

    const ParsedPermissionsPolicy& container_policy = frame_element->GetFramePolicy().container_policy;
