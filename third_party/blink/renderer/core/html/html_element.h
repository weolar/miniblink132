...
public:
    ...
    virtual void setInnerTextForBinding(const V8UnionStringLegacyNullToEmptyStringOrTrustedScript* string_or_trusted_script, ExceptionState& exception_state);
    ...
    static const TextControlElement* ElementIfAutoDirectionalityFormAssociatedOrNull(const Element* element);
    static TextControlElement* ElementIfAutoDirectionalityFormAssociatedOrNull(Element* element)
    {
        return const_cast<TextControlElement*>(ElementIfAutoDirectionalityFormAssociatedOrNull(const_cast<const Element*>(element)));
    }
    ...
    virtual bool IsHTMLFencedFrameElement() const
    {
        return false;
    }
    virtual bool IsHTMLFrameSetElement() const
    {
        return false;
    }
    virtual bool IsHTMLPermissionElement() const
    {
        return false;
    }
    ...
    static const AtomicString& EventNameForAttributeName(const QualifiedName& attr_name);
    ...
    virtual FormAssociated* ToFormAssociatedOrNull()
    {
        return nullptr;
    }
    ...
    void UpdateDirectionalityAfterInputTypeChange(const AtomicString& old_value, const AtomicString& new_value);
    ...
    virtual bool IsPotentiallyRenderBlocking() const
    {
        return false;
    }
    ...
    // do not match.
    bool IsPopoverReady(PopoverTriggerAction action, ExceptionState* exception_state, bool include_event_handler_text, Document* expected_document) const;
    bool togglePopover(ExceptionState& exception_state);
    bool togglePopover(V8UnionBooleanOrTogglePopoverOptions* options_or_force, ExceptionState& exception_state);
    void showPopover(ExceptionState& exception_state);
    void showPopover(ShowPopoverOptions* options, ExceptionState& exception_state);
    ...
    virtual void ShowPopoverInternal(Element* invoker, ExceptionState* exception_state);
    virtual void HidePopoverInternal(HidePopoverFocusBehavior focus_behavior, HidePopoverTransitionBehavior event_firing, ExceptionState* exception_state);
    void PopoverHideFinishIfNeeded(bool immediate);
    static const HTMLElement* FindTopmostPopoverAncestor(Element& new_popover_or_top_layer_element, HeapVector<Member<HTMLElement>>& stack_to_check,
        Element* new_popovers_invoker, TopLayerElementType top_layer_element_type = TopLayerElementType::kPopover);
    static const HTMLElement* TopLayerElementPopoverAncestor(Element& top_layer_element, TopLayerElementType top_layer_element_type);
    ...
    static void HideAllPopoversUntil(const HTMLElement*, Document&, HidePopoverFocusBehavior, HidePopoverTransitionBehavior);
    ...
    bool DispatchFocusEvent(Element* old_focused_element, mojom::blink::FocusType, InputDeviceCapabilities* source_capabilities) override;
    ...
    bool IsValidBuiltinCommand(HTMLElement& invoker, CommandEventType command) override;
    bool HandleCommandInternal(HTMLElement& invoker, CommandEventType command) override;
...
protected:
    ...
    void AddHTMLLengthToStyle(
        MutableCSSPropertyValueSet*, CSSPropertyID, const String& value, AllowPercentage = kAllowPercentageValues, AllowZero = kAllowZeroValues);
    void AddHTMLColorToStyle(MutableCSSPropertyValueSet*, CSSPropertyID, const String& color);
    void AddHTMLBackgroundImageToStyle(MutableCSSPropertyValueSet*, const String& url_value, const AtomicString& initiator_name = g_null_atom);
    ...
    void ApplyAspectRatioToStyle(const AtomicString& width, const AtomicString& height, MutableCSSPropertyValueSet*);
    // This corresponds to:
    //  'map to the aspect-ratio property'
    // described by:
    // https://html.spec.whatwg.org/multipage/rendering.html#map-to-the-aspect-ratio-property
    void ApplyIntegerAspectRatioToStyle(const AtomicString& width, const AtomicString& height, MutableCSSPropertyValueSet*);
    void ApplyAlignmentAttributeToStyle(const AtomicString&, MutableCSSPropertyValueSet*);
    void ApplyBorderAttributeToStyle(const AtomicString&, MutableCSSPropertyValueSet*);

    void AttributeChanged(const AttributeModificationParams&) override;
    void ParseAttribute(const AttributeModificationParams&) override;
    static bool ParseColorWithLegacyRules(const String& attribute_value, Color& parsed_color);
    bool IsPresentationAttribute(const QualifiedName&) const override;
    void CollectStyleForPresentationAttribute(const QualifiedName&, const AtomicString&, MutableCSSPropertyValueSet*) override;
...
private:
    ...
    void ApplyAspectRatioToStyle(double width, double height, MutableCSSPropertyValueSet* style);
    ...
    static void CloseEntirePopoverStack(
        HeapVector<Member<HTMLElement>>& stack, HidePopoverFocusBehavior focus_behavior, HidePopoverTransitionBehavior transition_behavior);

    static AttributeTriggers* TriggersForAttributeName(const QualifiedName& attr_name);
...
template <> struct DowncastTraits<HTMLElement> {
...
inline HTMLElement::HTMLElement(const QualifiedName& tag_name, Document& document, ConstructionType type = kCreateHTMLElement)
