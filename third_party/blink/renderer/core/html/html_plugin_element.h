...
class CORE_EXPORT HTMLPlugInElement : public HTMLFrameOwnerElement, public ActiveScriptWrappable<HTMLPlugInElement> {
public:
    ~HTMLPlugInElement() override;
    void Trace(Visitor*) const override;

    bool IsPlugin() const final
    {
        return true;
    }
    ...
    bool NeedsPluginUpdate() const
    {
        return needs_plugin_update_;
    }
    ...
    NamedPropertySetterResult AnonymousNamedSetter(const AtomicString&, const ScriptValue&);

protected:
    HTMLPlugInElement(const QualifiedName& tag_name, Document&, const CreateElementFlags);
...
private:
    ...
    bool IsFocusableStyle(UpdateBehavior update_behavior = UpdateBehavior::kStyleAndLayout) const final;
    ...
    bool LoadPlugin(const KURL&, const String& mime_type, const PluginParameters& plugin_params, bool use_fallback);
...
template <> struct DowncastTraits<HTMLPlugInElement> {
