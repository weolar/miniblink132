...
HTMLAreaElement::HTMLAreaElement(Document& document)
    : HTMLAnchorElementBase(html_names::kAreaTag, document)
    , shape_(kRect)
{
}
...
void HTMLAreaElement::ParseAttribute(const AttributeModificationParams& params)
{
    ...
    if (params.name == html_names::kShapeAttr) {
        ...
        InvalidateCachedPath();
    } else if (params.name == html_names::kAltAttr || params.name == html_names::kAccesskeyAttr) {
        // Do nothing.
    } else {
...
bool HTMLAreaElement::PointInArea(const PhysicalOffset& location, const LayoutObject* container_object) const
{
    return GetPath(container_object).Contains(gfx::PointF(location));
}

PhysicalRect HTMLAreaElement::ComputeAbsoluteRect(const LayoutObject* container_object) const
{
    if (!container_object)
        return PhysicalRect();

    // FIXME: This doesn't work correctly with transforms.
    PhysicalOffset abs_pos = container_object->LocalToAbsolutePoint(PhysicalOffset(), kIgnoreTransforms);
    ...
    if (path_) {
        ...
        case kPoly:
            if (coords_.size() >= 6) {
                ...
                path.MoveTo(gfx::PointF(ClampCoordinate(coords_[0]), ClampCoordinate(coords_[1])));
                for (int i = 1; i < num_points; ++i) {
                    path.AddLineTo(gfx::PointF(ClampCoordinate(coords_[i * 2]), ClampCoordinate(coords_[i * 2 + 1])));
                }
                path.CloseSubpath();
        ...
        case kCircle:
            if (coords_.size() >= 3 && coords_[2] > 0) {
                ...
                path.AddEllipse(gfx::PointF(ClampCoordinate(coords_[0]), ClampCoordinate(coords_[1])), r, r);
            }
            break;
        case kRect:
            if (coords_.size() >= 4) {
...
{
    if (HTMLMapElement* map_element = Traversal<HTMLMapElement>::FirstAncestor(*this))
        return map_element->ImageElement();
    return nullptr;
}

bool HTMLAreaElement::IsKeyboardFocusable(UpdateBehavior update_behavior) const
{
...
FocusableState HTMLAreaElement::IsFocusableState(UpdateBehavior update_behavior) const
{
...
bool HTMLAreaElement::IsFocusableStyle(UpdateBehavior update_behavior) const
{
    ...
    return !style.IsInert() && style.Visibility() == EVisibility::kVisible && Element::tabIndex() >= 0
        && SupportsFocus(update_behavior) != FocusableState::kNotFocusable;
}

void HTMLAreaElement::SetFocused(bool should_be_focused, mojom::blink::FocusType focus_type)
{
...
void HTMLAreaElement::UpdateSelectionOnFocus(SelectionBehaviorOnFocus selection_behavior, const FocusOptions* options)
{
