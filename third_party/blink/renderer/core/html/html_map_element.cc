...
HTMLAreaElement* HTMLMapElement::AreaForPoint(const PhysicalOffset& location, const LayoutObject* container_object)
{
...
HTMLImageElement* HTMLMapElement::ImageElement()
{
    ...
    for (unsigned i = 0; Element* curr = images->item(i); ++i) {
        ...
        String use_map_name = image_element.FastGetAttribute(html_names::kUsemapAttr).GetString().Substring(1);
        if (!use_map_name.empty() && (use_map_name == name_ || use_map_name == GetIdAttribute())) {
...
void HTMLMapElement::ParseAttribute(const AttributeModificationParams& params)
{
    ...
    if (params.name == html_names::kIdAttr || params.name == html_names::kNameAttr) {
...
Node::InsertionNotificationRequest HTMLMapElement::InsertedInto(ContainerNode& insertion_point)
{
