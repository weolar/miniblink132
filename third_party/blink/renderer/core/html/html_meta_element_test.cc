...
class HTMLMetaElementTest : public PageTestBase, private ScopedDisplayCutoutAPIForTest {
public:
    HTMLMetaElementTest()
    ...
    mojom::ViewportFit LoadTestPageAndReturnViewportFit(const String& value)
    {
        ...
        return GetDocument().GetViewportData().GetViewportDescription().GetViewportFit();
    }

protected:
    HTMLMetaElement* CreateColorSchemeMeta(const char* content)
    {
        auto* meta = MakeGarbageCollected<HTMLMetaElement>(GetDocument(), CreateElementFlags());
...
private:
    ...
    {
        GetDocument().documentElement()->setInnerHTML("<head>"
...
{
    EXPECT_EQ(mojom::ViewportFit::kAuto, LoadTestPageAndReturnViewportFit("auto"));
}

TEST_F(HTMLMetaElementTest, ViewportFit_Contain)
{
    EXPECT_EQ(mojom::ViewportFit::kContain, LoadTestPageAndReturnViewportFit("contain"));
}

TEST_F(HTMLMetaElementTest, ViewportFit_Cover)
{
    EXPECT_EQ(mojom::ViewportFit::kCover, LoadTestPageAndReturnViewportFit("cover"));
}

TEST_F(HTMLMetaElementTest, ViewportFit_Invalid)
{
    EXPECT_EQ(mojom::ViewportFit::kAuto, LoadTestPageAndReturnViewportFit("invalid"));
}
...
TEST_F(HTMLMetaElementTest, ColorSchemeProcessing_Remove)
{
    ...
    ExpectPageColorSchemes(static_cast<ColorSchemeFlags>(ColorSchemeFlag::kLight));
}
...
TEST_F(HTMLMetaElementTest, ColorSchemeProcessing_SetAttribute)
{
    ...
    GetDocument().getElementById(AtomicString("meta"))->setAttribute(html_names::kContentAttr, AtomicString("light"));
...
TEST_F(HTMLMetaElementTest, ColorSchemeProcessing_RemoveContentAttribute)
{
    ...
    GetDocument().getElementById(AtomicString("meta"))->removeAttribute(html_names::kContentAttr);

    ExpectPageColorSchemes(static_cast<ColorSchemeFlags>(ColorSchemeFlag::kNormal));
}
...
TEST_F(HTMLMetaElementTest, ColorSchemeProcessing_RemoveNameAttribute)
{
    ...
    GetDocument().getElementById(AtomicString("meta"))->removeAttribute(html_names::kNameAttr);
...
TEST_F(HTMLMetaElementTest, ColorSchemeParsing)
{
    ...
    ExpectPageColorSchemes(static_cast<ColorSchemeFlags>(ColorSchemeFlag::kLight) | static_cast<ColorSchemeFlags>(ColorSchemeFlag::kDark));
    ...
    ExpectPageColorSchemes(static_cast<ColorSchemeFlags>(ColorSchemeFlag::kOnly) | static_cast<ColorSchemeFlags>(ColorSchemeFlag::kLight));
}
...
TEST_F(HTMLMetaElementTest, ColorSchemeForcedDarkeningAndMQ)
{
    ...
    auto* media_query = GetDocument().GetMediaQueryMatcher().MatchMedia("(prefers-color-scheme: dark)");
...
TEST_F(HTMLMetaElementTest, ReferrerPolicyWithoutContent)
{
    ...
    EXPECT_EQ(network::mojom::ReferrerPolicy::kStrictOrigin, GetFrame().DomWindow()->GetReferrerPolicy());
    EXPECT_EQ(network::mojom::ReferrerPolicy::kStrictOrigin, GetFrame().DomWindow()->GetPolicyContainer()->GetReferrerPolicy());
}
...
TEST_F(HTMLMetaElementTest, WebMonetizationCounter)
{
    ...
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kHTMLMetaElementMonetization));

    // A <link rel="monetization"> with a content attribute is counted.
    GetDocument().head()->setInnerHTML(R"HTML(
    <meta name="monetization" content="$payment.pointer.url">
  )HTML");
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kHTMLMetaElementMonetization));

    // However, it does not affect the counter for <link rel="monetization">.
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kHTMLLinkElementMonetization));
}
