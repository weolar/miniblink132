...
public:
    explicit DestroyOnAbortAlgorithm(CloseWatcher* watcher)
        : watcher_(watcher)
    {
    }
...
void CloseWatcher::WatcherStack::Add(CloseWatcher* watcher)
{
    if (watcher_groups_.empty()) {
        ...
        host.SetCloseListener(receiver_.BindNewPipeAndPassRemote(window_->GetTaskRunner(TaskType::kMiscPlatformAPI)));
    }
...
void CloseWatcher::WatcherStack::Remove(CloseWatcher* watcher)
{
    for (auto& group : watcher_groups_) {
        ...
        if (watcher_it != group.end()) {
            ...
            if (group.empty()) {
                auto group_it = std::find(watcher_groups_.begin(), watcher_groups_.end(), group);
...
void CloseWatcher::WatcherStack::SetHadUserInteraction(bool had_user_interaction)
{
...
{
    return watcher_groups_.size() < allowed_groups_ && window_->GetFrame()->IsHistoryUserActivationActive();
}

void CloseWatcher::WatcherStack::EscapeKeyHandler(KeyboardEvent* event)
{
    if (!watcher_groups_.empty() && !event->DefaultHandled() && event->isTrusted() && event->keyCode() == VKEY_ESCAPE) {
        Signal();
    }
}
...
// static
CloseWatcher* CloseWatcher::Create(ScriptState* script_state, CloseWatcherOptions* options, ExceptionState& exception_state)
{
    LocalDOMWindow* window = LocalDOMWindow::From(script_state);
    if (!window || !window->GetFrame()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "CloseWatchers cannot be created in detached Windows.");
        return nullptr;
    }
...
// static
CloseWatcher* CloseWatcher::CreateInternal(LocalDOMWindow& window, WatcherStack& stack, CloseWatcherOptions* options)
{
    ...
    if (options && options->hasSignal()) {
        ...
        watcher->abort_handle_ = signal->AddAlgorithm(MakeGarbageCollected<DestroyOnAbortAlgorithm>(watcher));
    }
...
bool CloseWatcher::requestClose()
{
    ...
    Event& cancel_event = stack.CancelEventCanBeCancelable() ? *Event::CreateCancelable(event_type_names::kCancel) : *Event::Create(event_type_names::kCancel);
