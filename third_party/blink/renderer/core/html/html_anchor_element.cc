...
bool ShouldInterveneDownloadByFramePolicy(LocalFrame* frame)
{
    ...
    if (frame->IsAdFrame()) {
        ...
        if (!has_gesture) {
            UseCounter::Count(document, WebFeature::kDownloadInAdFrameWithoutUserGesture);
            should_intervene_download = true;
        }
    }
    if (frame->DomWindow()->IsSandboxed(network::mojom::blink::WebSandboxFlags::kDownloads)) {
...
void EmitDidAnchorElementReceiveMouseEvent(HTMLAnchorElementBase& anchor_element, Event& event)
{
    ...
    if (event.type() == event_type_names::kMousedown) {
        triggerer->DidAnchorElementReceiveMouseDownEvent(WebElement(&anchor_element), button, mev->ClickCount());
    } else if (event.type() == event_type_names::kMouseup) {
        triggerer->DidAnchorElementReceiveMouseUpEvent(WebElement(&anchor_element), button, mev->ClickCount());
    }
}

} // namespace

HTMLAnchorElementBase::HTMLAnchorElementBase(const QualifiedName& tag_name, Document& document)
...
FocusableState HTMLAnchorElementBase::SupportsFocus(UpdateBehavior update_behavior) const
{
...
bool HTMLAnchorElementBase::ShouldHaveFocusAppearance() const
{
    ...
    return (GetDocument().LastFocusType() != mojom::blink::FocusType::kMouse)
        || HTMLElement::SupportsFocus(UpdateBehavior::kNoneForFocusManagement) != FocusableState::kNotFocusable;
}

FocusableState HTMLAnchorElementBase::IsFocusableState(UpdateBehavior update_behavior) const
{
...
bool HTMLAnchorElementBase::IsKeyboardFocusable(UpdateBehavior update_behavior) const
{
    if (!IsFocusableStyle(update_behavior)) {
        return false;
    }
    ...
    if (Element::SupportsFocus(update_behavior) != FocusableState::kNotFocusable && IsFocusable(update_behavior)) {
...
static void AppendServerMapMousePosition(StringBuilder& url, Event* event)
{
    ...
    gfx::PointF map_point = layout_object->AbsoluteToLocalPoint(mouse_event->AbsoluteLocation());

    // The origin (0,0) is at the upper left of the content area, inside the
    // padding and border.
    map_point -= gfx::Vector2dF(To<LayoutBox>(layout_object)->PhysicalContentBoxOffset());
...
void HTMLAnchorElementBase::AttributeChanged(const AttributeModificationParams& params)
{
...
void HTMLAnchorElementBase::ParseAttribute(const AttributeModificationParams& params)
{
    if (params.name == html_names::kHrefAttr) {
        ...
        if (isConnected() && params.old_value != params.new_value) {
            if (auto* document_rules = DocumentSpeculationRules::FromIfExists(GetDocument())) {
                document_rules->HrefAttributeChanged(this, params.old_value, params.new_value);
            }
        }
        InvalidateCachedVisitedLinkHash();
        LogUpdateAttributeIfIsolatedWorldAndInDocument("a", params);
    } else if (params.name == html_names::kNameAttr) {
        if (GetDocument().HasRenderBlockingExpectLinkElements() && isConnected() && IsFinishedParsingChildren() && !params.new_value.empty()) {
            DCHECK(GetDocument().GetRenderBlockingResourceManager());
            GetDocument().GetRenderBlockingResourceManager()->RemovePendingParsingElement(params.new_value, this);
        }
...
{
    return attribute.GetName().LocalName() == html_names::kHrefAttr || HTMLElement::IsURLAttribute(attribute);
}

bool HTMLAnchorElementBase::HasLegalLinkAttribute(const QualifiedName& name) const
{
    return name == html_names::kHrefAttr || HTMLElement::HasLegalLinkAttribute(name);
}
...
void HTMLAnchorElementBase::FinishParsingChildren()
{
    ...
    if (GetDocument().HasRenderBlockingExpectLinkElements()) {
        DCHECK(GetDocument().GetRenderBlockingResourceManager());
        GetDocument().GetRenderBlockingResourceManager()->RemovePendingParsingElement(GetNameAttribute(), this);
    }
}
...
{
    return GetDocument().CompleteURL(StripLeadingAndTrailingHTMLSpaces(FastGetAttribute(html_names::kHrefAttr)));
}
...
void HTMLAnchorElementBase::SendPings(const KURL& destination_url) const
{
    ...
    if (ping_value.IsNull() || !GetDocument().GetSettings() || !GetDocument().GetSettings()->GetHyperlinkAuditingEnabled()) {
        return;
    }
    ...
    if ((ping_value.Contains('\n') || ping_value.Contains('\r') || ping_value.Contains('\t')) && ping_value.Contains('<')) {
        Deprecation::CountDeprecation(GetExecutionContext(), WebFeature::kCanRequestURLHTTPContainingNewline);
        return;
    }
    ...
    for (unsigned i = 0; i < ping_urls.size(); i++) {
        PingLoader::SendLinkAuditPing(GetDocument().GetFrame(), GetDocument().CompleteURL(ping_urls[i]), destination_url);
    }
}

void HTMLAnchorElementBase::NavigateToHyperlink(
    ResourceRequest request, NavigationPolicy navigation_policy, bool is_trusted, base::TimeTicks platform_time_stamp, KURL completed_url)
{
    ...
    const AtomicString& target = frame_request.CleanNavigationTarget(GetEffectiveTarget());
    ...
    if (RuntimeEnabledFeatures::RelOpenerBcgDependencyHintEnabled(GetExecutionContext()) && HasRel(kRelationOpener)
        && !frame_request.GetWindowFeatures().noopener) {
        frame_request.SetExplicitOpener();
    }

    frame_request.SetTriggeringEventInfo(
        is_trusted ? mojom::blink::TriggeringEventInfo::kFromTrustedEvent : mojom::blink::TriggeringEventInfo::kFromUntrustedEvent);
    frame_request.SetInputStartTime(platform_time_stamp);

    if (const AtomicString& attribution_src = FastGetAttribute(html_names::kAttributionsrcAttr); !attribution_src.IsNull()) {
        ...
        frame_request.SetImpression(frame->GetAttributionSrcLoader()->RegisterNavigation(
            /*navigation_url=*/completed_url, attribution_src,
            /*element=*/this, request.HasUserGesture(), request.GetReferrerPolicy()));
    }

    Frame* target_frame = frame->Tree().FindOrCreateFrameForNavigation(frame_request, target).frame;

    // If hrefTranslate is enabled and set restrict processing it
    // to same frame or navigations with noopener set.
    if (RuntimeEnabledFeatures::HrefTranslateEnabled(GetExecutionContext()) && FastHasAttribute(html_names::kHreftranslateAttr)
        && (target_frame == frame || frame_request.GetWindowFeatures().noopener)) {
        frame_request.SetHrefTranslate(FastGetAttribute(html_names::kHreftranslateAttr));
        UseCounter::Count(GetDocument(), WebFeature::kHTMLAnchorElementHrefTranslateAttribute);
    }
    ...
    if (target_frame == frame && HasRel(kRelationOpener)) {
        ...
        // inadvertently?
        UseCounter::Count(GetDocument(), WebFeature::kLinkRelOpenerTargetingSameFrame);
    }
...
Element* HTMLAnchorElementBase::interestTargetElement()
{
    ...
    return GetElementAttributeResolvingReferenceTarget(html_names::kInteresttargetAttr);
}

AtomicString HTMLAnchorElementBase::interestAction() const
{
    CHECK(RuntimeEnabledFeatures::HTMLInterestTargetAttributeEnabled());
    const AtomicString& attribute_value = FastGetAttribute(html_names::kInterestactionAttr);
    if (attribute_value && !attribute_value.IsNull() && !attribute_value.empty()) {
...
void HTMLAnchorElementBase::HandleClick(MouseEvent& event)
{
    ...
    if (!isConnected()) {
        UseCounter::Count(GetDocument(), WebFeature::kAnchorClickDispatchForNonConnectedNode);
    }
    ...
    StringBuilder url;
    url.Append(StripLeadingAndTrailingHTMLSpaces(FastGetAttribute(html_names::kHrefAttr)));
    ...
    if (FastHasAttribute(html_names::kReferrerpolicyAttr)
        && SecurityPolicy::ReferrerPolicyFromString(FastGetAttribute(html_names::kReferrerpolicyAttr), kSupportReferrerPolicyLegacyKeywords, &policy)
        && !HasRel(kRelationNoReferrer)) {
        UseCounter::Count(GetDocument(), WebFeature::kHTMLAnchorElementReferrerPolicyAttribute);
    ...
    if (FastHasAttribute(html_names::kDownloadAttr) && navigation_policy != kNavigationPolicyDownload
        && window->GetSecurityOrigin()->CanReadContent(completed_url)) {
        if (ShouldInterveneDownloadByFramePolicy(frame))
            return;

        String download_attr = static_cast<String>(FastGetAttribute(html_names::kDownloadAttr));
        if (download_attr.length() > kMaxDownloadAttrLength) {
            AddConsoleMessage(mojom::blink::ConsoleMessageSource::kRendering, mojom::blink::ConsoleMessageLevel::kError,
        ...
        auto* params = MakeGarbageCollected<NavigateEventDispatchParams>(completed_url, NavigateEventType::kCrossDocument, WebFrameLoadType::kStandard);
        if (event.isTrusted())
            params->involvement = UserNavigationInvolvement::kActivation;
        params->download_filename = download_attr;
        params->source_element = this;
        if (window->navigation()->DispatchNavigateEvent(params) != NavigationApi::DispatchResult::kContinue) {
            return;
        }
        ...
        window->navigation()->InformAboutCanceledNavigation(CancelNavigationReason::kDropped);
        ...
        Referrer referrer = SecurityPolicy::GenerateReferrer(referrer_policy, completed_url, window->OutgoingReferrer());
    ...
    base::OnceClosure navigate_closure = WTF::BindOnce(&HTMLAnchorElementBase::NavigateToHyperlink, WrapWeakPersistent(this), std::move(request),
        navigation_policy, event.isTrusted(), event.PlatformTimeStamp(), std::move(completed_url));

    if (navigation_policy == kNavigationPolicyDownload || navigation_policy == kNavigationPolicyLinkPreview) {
        ...
        auto task_handle = PostDelayedCancellableTask(
            *base::SingleThreadTaskRunner::GetCurrentDefault(), FROM_HERE, std::move(navigate_closure), base::Milliseconds(ui::kDoubleClickTimeMs));
        frame->GetEventHandler().SetDelayedNavigationTaskHandle(std::move(task_handle));
    } else {
...
bool IsEnterKeyKeydownEvent(Event& event)
{
    ...
    return event.type() == event_type_names::kKeydown && keyboard_event && keyboard_event->key() == keywords::kCapitalEnter && !keyboard_event->repeat();
}

bool IsLinkClick(Event& event)
{
    auto* mouse_event = DynamicTo<MouseEvent>(event);
    if ((event.type() != event_type_names::kClick && event.type() != event_type_names::kAuxclick) || !mouse_event) {
        return false;
    }
    int16_t button = mouse_event->button();
    return (button == static_cast<int16_t>(WebPointerProperties::Button::kLeft) || button == static_cast<int16_t>(WebPointerProperties::Button::kMiddle));
}
...
Node::InsertionNotificationRequest HTMLAnchorElementBase::InsertedInto(ContainerNode& insertion_point)
{
    InsertionNotificationRequest request = HTMLElement::InsertedInto(insertion_point);
    LogAddElementIfIsolatedWorldAndInDocument("a", html_names::kHrefAttr);

    if (isConnected()) {
        if (auto* sender = AnchorElementMetricsSender::GetForFrame(GetDocument().GetFrame())) {
