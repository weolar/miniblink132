...
HTMLLinkElement::HTMLLinkElement(Document& document, const CreateElementFlags flags)
...
void HTMLLinkElement::ParseAttribute(const AttributeModificationParams& params)
{
    ...
    if (name == html_names::kRelAttr) {
        ...
        if (rel_attribute_.IsMonetization() && GetDocument().IsInOutermostMainFrame()) {
            ...
            UseCounter::Count(&GetDocument(), WebFeature::kHTMLLinkElementMonetization);
        }
        if (rel_attribute_.IsCanonical() && GetDocument().IsInOutermostMainFrame()) {
        ...
        if (rel_attribute_.IsPayment() && GetDocument().IsInOutermostMainFrame()) {
            ...
            if (RuntimeEnabledFeatures::PaymentLinkDetectionEnabled()) {
                GetDocument().HandlePaymentLink(GetNonEmptyURLAttribute(html_names::kHrefAttr));
            }
#endif
        }
        ...
        if (!value.IsNull()) {
            SecurityPolicy::ReferrerPolicyFromString(value, kDoNotSupportReferrerPolicyLegacyKeywords, &referrer_policy_);
            UseCounter::Count(GetDocument(), WebFeature::kHTMLLinkElementReferrerPolicyAttribute);
        }
    } else if (name == html_names::kSizesAttr) {
        sizes_->DidUpdateAttributeValue(params.old_value, value);
        WebVector<gfx::Size> web_icon_sizes = WebIconSizesParser::ParseIconSizes(value);
...
void HTMLLinkElement::LoadStylesheet(const LinkLoadParameters& params, const WTF::TextEncoding& charset, FetchParameters::DeferOption defer_option,
    ResourceClient* link_client, RenderBlockingBehavior render_blocking)
{
    return link_loader_->LoadStylesheet(params, localName(), charset, defer_option, GetDocument(), link_client, render_blocking);
}
...
Node::InsertionNotificationRequest HTMLLinkElement::InsertedInto(ContainerNode& insertion_point)
{
    HTMLElement::InsertedInto(insertion_point);
    LogAddElementIfIsolatedWorldAndInDocument("link", html_names::kRelAttr, html_names::kHrefAttr);
...
void HTMLLinkElement::RemovedFrom(ContainerNode& insertion_point)
{
    ...
    if (!insertion_point.isConnected() || GetDocument().StatePreservingAtomicMoveInProgress()) {
        return;
    }
    ...
    GetDocument().GetStyleEngine().RemoveStyleSheetCandidateNode(*this, insertion_point);
    if (link_)
        link_->OwnerRemoved();
...
void HTMLLinkElement::NotifyLoadedSheetAndAllCriticalSubresources(LoadedSheetErrorStatus error_status)
{
    DCHECK(GetLinkStyle());
    GetLinkStyle()->NotifyLoadedSheetAndAllCriticalSubresources(error_status);
}

void HTMLLinkElement::DispatchPendingEvent(std::unique_ptr<IncrementLoadEventDelayCount> count)
{
    DCHECK(link_);
    if (link_->HasLoaded())
        LinkLoaded();
...
void HTMLLinkElement::ScheduleEvent()
{
    GetDocument()
        ...
        ->PostTask(FROM_HERE,
            WTF::BindOnce(&HTMLLinkElement::DispatchPendingEvent, WrapPersistent(this), std::make_unique<IncrementLoadEventDelayCount>(GetDocument())));
}
...
{
    return blocking_attribute_->HasRenderToken() || (IsCreatedByParser() && rel_attribute_.IsStyleSheet());
}
...
void HTMLLinkElement::HandleExpectHrefChanges(const String& old_value, const String& new_value)
{
    if (!rel_attribute_.IsExpect()) {
        return;
    }
...
bool HTMLLinkElement::MediaQueryMatches() const
{
    if (LocalFrame* frame = GetDocument().GetFrame(); frame && !media_.empty()) {
        auto* media_queries = MediaQuerySet::Create(media_, GetDocument().GetExecutionContext());
...
void HTMLLinkElement::RemoveExpectRenderBlockingLink(const String& href)
{
    ...
    if (auto* render_blocking_resource_manager = GetDocument().GetRenderBlockingResourceManager()) {
        render_blocking_resource_manager->RemovePendingParsingElementLink(ParseSameDocumentIdFromHref(href), this);
    }
}

AtomicString HTMLLinkElement::ParseSameDocumentIdFromHref(const String& href)
{
    String actual_href = href.IsNull() ? FastGetAttribute(html_names::kHrefAttr) : href;
    ...
    return EqualIgnoringFragmentIdentifier(url, GetDocument().Url()) ? AtomicString(url.FragmentIdentifier()) : g_null_atom;
}

void HTMLLinkElement::AddExpectRenderBlockingLinkIfNeeded(const String& href, bool media_known_to_match)
{
    if (!rel_attribute_.IsExpect()) {
        return;
    }
    ...
    if (auto* render_blocking_resource_manager = GetDocument().GetRenderBlockingResourceManager()) {
        render_blocking_resource_manager->AddPendingParsingElementLink(ParseSameDocumentIdFromHref(href), this);
    }
}
