...
{
    return name == html_names::kBackgroundAttr || HTMLTablePartElement::HasLegalLinkAttribute(name);
}

static int FindIndexInRowCollection(const HTMLCollection& rows, const HTMLTableRowElement& target)
{
...
HTMLElement* HTMLTableRowElement::insertCell(int index, ExceptionState& exception_state)
{
    HTMLCollection* children = cells();
    int num_cells = children ? children->length() : 0;
    if (index < -1 || index > num_cells) {
        exception_state.ThrowDOMException(DOMExceptionCode::kIndexSizeError,
            "The value provided (" + String::Number(index) + ") is outside the range [-1, " + String::Number(num_cells) + "].");
        return nullptr;
    }

    auto* cell = MakeGarbageCollected<HTMLTableCellElement>(html_names::kTdTag, GetDocument());
...
void HTMLTableRowElement::deleteCell(int index, ExceptionState& exception_state)
{
    HTMLCollection* children = cells();
    int num_cells = children ? children->length() : 0;
    ...
    if (index < -1 || index >= num_cells) {
        exception_state.ThrowDOMException(DOMExceptionCode::kIndexSizeError,
            "The value provided (" + String::Number(index) + ") is outside the range [0, " + String::Number(num_cells) + ").");
        return;
    }
