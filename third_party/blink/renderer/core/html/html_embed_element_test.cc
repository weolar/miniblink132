...
TEST_F(HTMLEmbedElementTest, FallbackState)
{
    ...
    const ComputedStyle* initial_style = GetDocument().GetStyleResolver().InitialStyleForElement();

    // We should get |true| as a result and don't trigger a DCHECK.
    EXPECT_TRUE(static_cast<Element*>(embed)->LayoutObjectIsNeeded(*initial_style));
...
TEST_F(HTMLEmbedElementTest, NotEnforceLayoutImageType)
{
    ...
    EXPECT_FALSE(static_cast<Element*>(embed)->LayoutObjectIsNeeded(*initial_style));
