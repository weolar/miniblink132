...
public:
    ...
    const std::vector<std::pair<DOMNodeId, gfx::Rect>>& observed_image_ad_rects() const
    {
...
protected:
    ...
    void SetUp() override
    {
        ...
        PageTestBase::SetupPageWithClients(nullptr, test_frame_client_.Get(), nullptr, gfx::Size(kViewportWidth, kViewportHeight));
    }
...
TEST_F(HTMLImageElementTest, ImageAdRectangleUpdate)
{
    ...
    EXPECT_EQ(test_frame_client_->observed_image_ad_rects()[0].second, gfx::Rect(5, 5, 10, 10));
    ...
    EXPECT_EQ(test_frame_client_->observed_image_ad_rects()[1].second, gfx::Rect());
    ...
    EXPECT_EQ(test_frame_client_->observed_image_ad_rects()[2].second, gfx::Rect(5, 5, 30, 30));
    ...
    EXPECT_EQ(test_frame_client_->observed_image_ad_rects()[3].second, gfx::Rect());
}
...
TEST_F(HTMLImageElementSimTest, Sharedstoragewritable_SecureContext_Allowed)
{
    ...
    SimSubresourceRequest image_resource("https://example.com/foo.png", "image/png");
...
TEST_F(HTMLImageElementSimTest, Sharedstoragewritable_InsecureContext_NotAllowed)
{
    WebRuntimeFeaturesBase::EnableSharedStorageAPI(true);
    WebRuntimeFeaturesBase::EnableSharedStorageAPIM118(true);
    SimRequest main_resource("http://example.com/index.html", "text/html");
    SimSubresourceRequest image_resource("http://example.com/foo.png", "image/png");
...
class TransparentPlaceholderImageSimTest : public SimTest, public ::testing::WithParamInterface<bool> {
protected:
    void SetUp() override
    {
        SimTest::SetUp();
        if (GetParam()) {
            feature_list_.InitAndEnableFeature(features::kSimplifyLoadingTransparentPlaceholderImage);
        } else {
            feature_list_.InitAndDisableFeature(features::kSimplifyLoadingTransparentPlaceholderImage);
        }
    }
...
INSTANTIATE_TEST_SUITE_P(TransparentPlaceholderImageSimTest, TransparentPlaceholderImageSimTest, testing::Bool());
...
TEST_P(TransparentPlaceholderImageSimTest, CurrentSrcForTransparentPlaceholderImage)
{
    const String image_source = "data:image/gif;base64,R0lGODlhAQABAIAAAP///////"
