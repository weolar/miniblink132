...
{
    const base::FeatureParam<int> max_number_of_observations { &features::kNavigationPredictor, "max_intersection_observations", -1 };
...
{
    const base::FeatureParam<base::TimeDelta> param { &features::kNavigationPredictor, "intersection_observer_delay", base::Milliseconds(100) };
    return param.Get();
}
...
void AnchorElementViewportPositionTracker::Observer::AnchorPositionUpdate::Trace(Visitor* visitor) const
{
    visitor->Trace(anchor_element);
}

// static
const char AnchorElementViewportPositionTracker::kSupplementName[] = "DocumentAnchorElementViewportPositionTracker";

AnchorElementViewportPositionTracker::AnchorElementViewportPositionTracker(Document& document)
    ...
    , position_update_timer_(
          document.GetExecutionContext()->GetTaskRunner(TaskType::kInternalDefault), this, &AnchorElementViewportPositionTracker::PositionUpdateTimerFired)
{
    intersection_observer_
        = IntersectionObserver::Create(document, WTF::BindRepeating(&AnchorElementViewportPositionTracker::UpdateVisibleAnchors, WrapWeakPersistent(this)),
            LocalFrameUkmAggregator::kAnchorElementMetricsIntersectionObserver,
            { .thresholds = { kIntersectionRatioThreshold }, .delay = intersection_observer_delay_ });
}

AnchorElementViewportPositionTracker::~AnchorElementViewportPositionTracker() = default;
...
// static
AnchorElementViewportPositionTracker* AnchorElementViewportPositionTracker::MaybeGetOrCreateFor(Document& document)
{
    if (!document.GetFrame() || !document.GetFrame()->IsOutermostMainFrame()) {
        return nullptr;
    }

    AnchorElementViewportPositionTracker* tracker = Supplement<Document>::From<AnchorElementViewportPositionTracker>(document);
    if (tracker) {
        return tracker;
    }

    tracker = MakeGarbageCollected<AnchorElementViewportPositionTracker>(document);
    ProvideTo(document, tracker);
    return tracker;
}

void AnchorElementViewportPositionTracker::AddObserver(AnchorElementViewportPositionTracker::Observer* observer)
{
    observers_.insert(observer);
}

HTMLAnchorElementBase* AnchorElementViewportPositionTracker::MaybeObserveAnchor(
    HTMLAnchorElementBase& anchor_element, const mojom::blink::AnchorElementMetrics& metrics)
{
    if (!max_number_of_observations_) {
        return nullptr;
    }

    int percent_area = std::clamp(static_cast<int>(metrics.ratio_area * 100.0f), 0, 100);
    ...
    if (observed_anchors_.size() < max_number_of_observations_) {
        should_observe = true;
    } else if (auto smallest_observed_anchor_it = observed_anchors_.begin(); smallest_observed_anchor_it->percent_area < percent_area) {
        should_observe = true;
        Node* node = DOMNodeIds::NodeForId(smallest_observed_anchor_it->dom_node_id);
        CHECK(node);
        anchor_unobserved = To<HTMLAnchorElementBase>(node);
        intersection_observer_->unobserve(anchor_unobserved);
        not_observed_anchors_.insert(observed_anchors_.extract(smallest_observed_anchor_it));
    }

    if (should_observe) {
        // Observe the element to collect time_in_viewport stats.
        intersection_observer_->observe(&anchor_element);
        observed_anchors_.insert({ .percent_area = percent_area, .dom_node_id = DOMNodeIds::IdForNode(&anchor_element) });
    } else {
        not_observed_anchors_.insert({ .percent_area = percent_area, .dom_node_id = DOMNodeIds::IdForNode(&anchor_element) });
    }

    return anchor_unobserved;
}

void AnchorElementViewportPositionTracker::RemoveAnchor(HTMLAnchorElementBase& anchor)
{
    if (DOMNodeId node_id = DOMNodeIds::ExistingIdForNode(&anchor); node_id && max_number_of_observations_) {
        ...
        // expensive.
        if (auto observed_anchors_it = base::ranges::find(observed_anchors_, node_id, &AnchorObservation::dom_node_id);
            ...
            if (!not_observed_anchors_.empty()) {
                auto largest_non_observed_anchor_it = std::prev(not_observed_anchors_.end());
                intersection_observer_->observe(To<Element>(DOMNodeIds::NodeForId(largest_non_observed_anchor_it->dom_node_id)));
                observed_anchors_.insert(not_observed_anchors_.extract(largest_non_observed_anchor_it));
            }
        } else if (auto not_observed_anchors_it = base::ranges::find(not_observed_anchors_, node_id, &AnchorObservation::dom_node_id);
...
void AnchorElementViewportPositionTracker::RecordPointerDown(const PointerEvent& pointer_event)
{
    ...
    pointer_down_location = document->GetFrame()->View()->FrameToViewport(pointer_down_location);
    pointer_down_location.Offset(0, GetBrowserControlsHeight(*GetSupplementable()));
...
void AnchorElementViewportPositionTracker::OnScrollEnd()
{
    ...
    if (!position_update_timer_.IsActive()) {
        position_update_timer_.StartOneShot(intersection_observer_delay_, FROM_HERE);
    }
}

IntersectionObserver* AnchorElementViewportPositionTracker::GetIntersectionObserverForTesting()
{
    return intersection_observer_;
}

void AnchorElementViewportPositionTracker::UpdateVisibleAnchors(const HeapVector<Member<IntersectionObserverEntry>>& entries)
{
    ...
    for (const auto& entry : entries) {
        ...
        const HTMLAnchorElementBase& anchor_element = To<HTMLAnchorElementBase>(*element);
...
void AnchorElementViewportPositionTracker::PositionUpdateTimerFired(TimerBase*)
{
    CHECK(ShouldReportViewportPositions());
...
void AnchorElementViewportPositionTracker::DidFinishLifecycleUpdate(const LocalFrameView& local_frame_view)
{
    CHECK(ShouldReportViewportPositions());
...
void AnchorElementViewportPositionTracker::DispatchAnchorElementsPositionUpdates()
{
    CHECK(ShouldReportViewportPositions());

    Screen* screen = GetSupplementable()->domWindow()->screen();
    FrameWidget* widget = GetSupplementable()->GetFrame()->GetWidgetForLocalRoot();
    ...
    const float browser_controls_height = GetBrowserControlsHeight(*GetSupplementable());
    ...
    for (const HTMLAnchorElementBase* anchor : anchors_in_viewport_) {
        ...
        float vertical_position = center_point_y / (viewport_height + browser_controls_height);
        ...
        if (last_pointer_down_.has_value()) {
            ...
            float distance_from_pointer_down = center_point_y - last_pointer_down_.value();
            distance_from_pointer_down_ratio = distance_from_pointer_down / screen_height;
        }
        auto* position_update = MakeGarbageCollected<Observer::AnchorPositionUpdate>();
        position_update->anchor_element = anchor;
        position_update->vertical_position = vertical_position;
        position_update->distance_from_pointer_down = distance_from_pointer_down_ratio;
        position_update->size_in_viewport = rect.size().GetCheckedArea().ValueOrDefault(std::numeric_limits<int>::max());
