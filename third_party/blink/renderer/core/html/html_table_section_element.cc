...
HTMLTableSectionElement::HTMLTableSectionElement(const QualifiedName& tag_name, Document& document)
    : HTMLTablePartElement(tag_name, document)
{
}

const CSSPropertyValueSet* HTMLTableSectionElement::AdditionalPresentationAttributeStyle()
{
...
HTMLElement* HTMLTableSectionElement::insertRow(int index, ExceptionState& exception_state)
{
    ...
    if (index < -1 || index > num_rows) {
        exception_state.ThrowDOMException(DOMExceptionCode::kIndexSizeError,
            "The provided index (" + String::Number(index) + " is outside the range [-1, " + String::Number(num_rows) + "].");
        return nullptr;
    }
...
void HTMLTableSectionElement::deleteRow(int index, ExceptionState& exception_state)
{
    HTMLCollection* children = rows();
