...
public:
    AnchorIdTargetObserver(const AtomicString& id, AnchorElementObserver* anchor_element_observer)
        : IdTargetObserver(anchor_element_observer->GetSourceElement().GetTreeScope().EnsureIdTargetObserverRegistry(), id)
    ...
    {
        anchor_element_observer_->Notify();
    }

    bool SameId(const AtomicString& id) const
    {
        return id == Id();
    }
...
{
    return element.IsInTreeScope() && !element.HasExplicitlySetAttrAssociatedElements(html_names::kAnchorAttr)
...
void AnchorElementObserver::Notify()
{
    ...
    if (current_anchor_ != new_anchor) {
        ...
        if (source_element_->GetLayoutObject()) {
            source_element_->GetLayoutObject()->SetNeedsLayoutAndFullPaintInvalidation(layout_invalidation_reason::kAnchorPositioning);
        }
    }
...
void AnchorElementObserver::ResetIdTargetObserverIfNeeded()
{
    ...
    const AtomicString& anchor_id = source_element_->FastGetAttribute(html_names::kAnchorAttr);
    if (id_target_observer_) {
        if (static_cast<AnchorIdTargetObserver*>(id_target_observer_.Get())->SameId(anchor_id)) {
            // Already observing the same id target. Nothing more to do.
            return;
        }
        id_target_observer_->Unregister();
    }
    id_target_observer_ = MakeGarbageCollected<AnchorIdTargetObserver>(anchor_id, this);
}
