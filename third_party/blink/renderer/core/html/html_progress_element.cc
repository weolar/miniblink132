...
HTMLProgressElement::HTMLProgressElement(Document& document)
    : HTMLElement(html_names::kProgressTag, document)
    , value_(nullptr)
{
...
LayoutObject* HTMLProgressElement::CreateLayoutObject(const ComputedStyle& style)
{
    if (!style.HasEffectiveAppearance()) {
        UseCounter::Count(GetDocument(), WebFeature::kProgressElementWithNoneAppearance);
        return LayoutObject::CreateObject(this, style);
    }
    UseCounter::Count(GetDocument(), WebFeature::kProgressElementWithProgressBarAppearance);
...
void HTMLProgressElement::DidRecalcStyle(const StyleRecalcChange change)
{
    ...
    if (style) {
        ...
        if (is_horizontal && is_ltr) {
            UseCounter::Count(GetDocument(), WebFeature::kProgressElementHorizontalLtr);
        } else if (is_horizontal && !is_ltr) {
            UseCounter::Count(GetDocument(), WebFeature::kProgressElementHorizontalRtl);
        } else if (is_ltr) {
...
void HTMLProgressElement::ParseAttribute(const AttributeModificationParams& params)
{
...
{
    value_->SetInlineStyleProperty(CSSPropertyID::kInlineSize, position, CSSPrimitiveValue::UnitType::kPercentage);
