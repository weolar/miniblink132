...
LayoutObject* HTMLSummaryElement::CreateLayoutObject(const ComputedStyle& style)
{
    ...
    if (display == EDisplay::kFlex || display == EDisplay::kInlineFlex || display == EDisplay::kGrid || display == EDisplay::kInlineGrid
...
FocusableState HTMLSummaryElement::SupportsFocus(UpdateBehavior update_behavior) const
{
    if (IsMainSummary()) {
...
{
    if (IsMainSummary()) {
        if (event.type() == event_type_names::kDOMActivate && !IsClickableControl(event.target()->ToNode())) {
            if (HTMLDetailsElement* details = DetailsElement())
                details->ToggleOpen();
