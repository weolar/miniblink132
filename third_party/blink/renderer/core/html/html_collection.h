...
template <class CollectionType, class NodeType> class HTMLCollectionIterator {
    STACK_ALLOCATED();
...
public:
    ...
    NodeType* operator*()
    {
        return collection_->item(index_);
    }
...
class CORE_EXPORT HTMLCollection : public ScriptWrappable, public LiveNodeListBase {
...
public:
    ...
    bool IsEmpty() const
    {
        return collection_items_cache_.IsEmpty(*this);
    }
    ...
    bool CanTraverseBackward() const
    {
        return !OverridesItemAfter();
    }
...
protected:
    ...
    public:
        ...
        const HeapVector<Member<Element>>* GetElementsById(const AtomicString& id) const
        {
        ...
        const HeapVector<Member<Element>>* GetElementsByName(const AtomicString& name) const
        {
    ...
    private:
        typedef HeapHashMap<AtomicString, Member<HeapVector<Member<Element>>>> StringToElementsMap;
        static void AddElementToMap(StringToElementsMap& map, const AtomicString& key, Element* element)
        {
            HeapVector<Member<Element>>* vector = map.insert(key, MakeGarbageCollected<HeapVector<Member<Element>>>()).stored_value->value.Get();
    ...
    bool OverridesItemAfter() const
    {
        return overrides_item_after_;
    }
    ...
    bool HasValidIdNameCache() const
    {
        return named_item_cache_ != nullptr;
    }
...
private:
    void InvalidateIdNameCacheMaps(Document* old_document = nullptr) const
    {
        ...
        UnregisterIdNameCacheFromDocument(old_document ? *old_document : GetDocument());
...
template <> struct DowncastTraits<HTMLCollection> {
...
DISABLE_CFI_PERF
inline void HTMLCollection::InvalidateCacheForAttribute(const QualifiedName* attr_name) const
{
    if (!attr_name || ShouldInvalidateTypeOnAttributeChange(InvalidationType(), *attr_name))
        InvalidateCache();
    else if (*attr_name == html_names::kIdAttr || *attr_name == html_names::kNameAttr)
