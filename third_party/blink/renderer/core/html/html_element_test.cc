...
TEST_F(HTMLElementTest, DirectStyleMutationTriggered)
{
    ...
    EXPECT_FALSE(GetDocument().GetPage()->Animator().has_inline_style_mutation_for_test());
    ...
    EXPECT_TRUE(GetDocument().GetPage()->Animator().has_inline_style_mutation_for_test());
...
TEST_F(HTMLElementTest, DirectStyleMutationInFrame)
{
    ...
    EXPECT_FALSE(ChildDocument().GetPage()->Animator().has_inline_style_mutation_for_test());
    RunDocumentLifecycle();
...
TEST_F(HTMLElementTest, DirectStyleMutationNotTriggeredByPseudoClassStyleChange)
{
    GetDocument().GetSettings()->SetScriptEnabled(true);
    SetBodyInnerHTML(R"HTML(
    .button {
    ...
    GetDocument().body()->appendChild(script);
    EXPECT_EQ(GetDocument().FocusedElement(), GetDocument().getElementById(AtomicString("box")));
    EXPECT_FALSE(GetDocument().GetPage()->Animator().has_inline_style_mutation_for_test());
}
...
TEST_F(HTMLElementTest, HasImplicitlyAnchoredElement)
{
    ...
    HTMLElement* target = To<HTMLElement>(GetDocument().getElementById(AtomicString("target")));
...
TEST_F(HTMLElementTest, ImplicitlyAnchoredElementRemoved)
{
    ...
    HTMLElement* target1 = To<HTMLElement>(GetDocument().getElementById(AtomicString("target1")));
    HTMLElement* target2 = To<HTMLElement>(GetDocument().getElementById(AtomicString("target2")));
...
TEST_F(HTMLElementTest, ImplicitlyAnchorElementConnected)
{
    ...
    HTMLElement* target1 = To<HTMLElement>(GetDocument().CreateElementForBinding(AtomicString("div")));
    target1->setAttribute(html_names::kAnchorAttr, AtomicString("anchor"));

    HTMLElement* target2 = To<HTMLElement>(GetDocument().CreateElementForBinding(AtomicString("div")));
    target2->setAnchorElementForBinding(anchor);
...
TEST_F(HTMLElementTest, PopoverTopLayerRemovalTiming)
{
    ...
    target->HidePopoverInternal(HidePopoverFocusBehavior::kFocusPreviousElement, HidePopoverTransitionBehavior::kFireEventsAndWaitForTransitions, nullptr);
    EXPECT_FALSE(target->popoverOpen());
    EXPECT_TRUE(target->IsInTopLayer());
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(target->IsInTopLayer());
...
TEST_F(HTMLElementTest, DialogTopLayerRemovalTiming)
{
    ...
    auto* target = To<HTMLDialogElement>(GetDocument().getElementById(AtomicString("target")));
