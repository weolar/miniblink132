...
HTMLObjectElement::HTMLObjectElement(Document& document, const CreateElementFlags flags)
...
const AttrNameToTrustedType& HTMLObjectElement::GetCheckedAttributeTypes() const
{
    DEFINE_STATIC_LOCAL(
        AttrNameToTrustedType, attribute_map, ({ { "data", SpecificTrustedType::kScriptURL }, { "codebase", SpecificTrustedType::kScriptURL } }));
    return attribute_map;
}

LayoutEmbeddedContent* HTMLObjectElement::ExistingLayoutEmbeddedContent() const
{
...
void HTMLObjectElement::CollectStyleForPresentationAttribute(const QualifiedName& name, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
    if (name == html_names::kBorderAttr)
...
void HTMLObjectElement::ParseAttribute(const AttributeModificationParams& params)
{
...
{
    if (MIMETypeRegistry::IsJavaAppletMIMEType(service_type_) && ClassId().StartsWithIgnoringASCIICase("java:"))
        return true;
...
void HTMLObjectElement::ReloadPluginOnAttributeChange(const QualifiedName& name)
{
    ...
    if (name == html_names::kTypeAttr) {
        needs_invalidation = !FastHasAttribute(html_names::kClassidAttr) && !FastHasAttribute(html_names::kDataAttr);
    } else if (name == html_names::kDataAttr) {
...
void HTMLObjectElement::UpdatePluginInternal()
{
    ...
    KURL overriden_url = GetDocument().GetFrame()->Client()->OverrideFlashEmbedWithHTML(GetDocument().CompleteURL(url_));
...
Node::InsertionNotificationRequest HTMLObjectElement::InsertedInto(ContainerNode& insertion_point)
{
...
{
    return attribute.GetName() == html_names::kCodebaseAttr || attribute.GetName() == html_names::kDataAttr || HTMLPlugInElement::IsURLAttribute(attribute);
}

bool HTMLObjectElement::HasLegalLinkAttribute(const QualifiedName& name) const
{
    return name == html_names::kClassidAttr || name == html_names::kDataAttr || name == html_names::kCodebaseAttr
...
void HTMLObjectElement::ReattachFallbackContent()
{
    if (!GetDocument().InStyleRecalc()) {
        ...
        SetNeedsStyleRecalc(kSubtreeStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kUseFallback));
        SetForceReattachLayoutTree();
    }
}

void HTMLObjectElement::RenderFallbackContent(ErrorEventPolicy should_dispatch_error_event)
{
    ...
    // type issue.
    if (image_loader_ && image_loader_->GetContent() && image_loader_->GetContent()->GetContentStatus() != ResourceStatus::kLoadError) {
    ...
    if (RuntimeEnabledFeatures::HTMLObjectElementFallbackDetachContentFrameEnabled()) {
...
bool HTMLObjectElement::IsExposed() const
{
    ...
    for (HTMLObjectElement* ancestor = Traversal<HTMLObjectElement>::FirstAncestor(*this); ancestor;
...
{
    if (MIMETypeRegistry::IsJavaAppletMIMEType(FastGetAttribute(html_names::kTypeAttr)))
        return true;

    for (HTMLElement& child : Traversal<HTMLElement>::ChildrenOf(*this)) {
        if (IsA<HTMLParamElement>(child) && EqualIgnoringASCIICase(child.GetNameAttribute(), "type")
            && MIMETypeRegistry::IsJavaAppletMIMEType(child.FastGetAttribute(html_names::kValueAttr).GetString()))
            return true;
