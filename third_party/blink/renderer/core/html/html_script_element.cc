...
HTMLScriptElement::HTMLScriptElement(Document& document, const CreateElementFlags flags)
...
const AttrNameToTrustedType& HTMLScriptElement::GetCheckedAttributeTypes() const
{
    DEFINE_STATIC_LOCAL(AttrNameToTrustedType, attribute_map, ({ { "src", SpecificTrustedType::kScriptURL } }));
...
void HTMLScriptElement::ParseAttribute(const AttributeModificationParams& params)
{
    if (params.name == html_names::kSrcAttr) {
        ...
        blocking_attribute_->OnAttributeValueChanged(params.old_value, params.new_value);
        if (GetDocument().GetRenderBlockingResourceManager() && !IsPotentiallyRenderBlocking()) {
            GetDocument().GetRenderBlockingResourceManager()->RemovePendingScript(*this);
        }
        ...
        if (GetDocument().GetFrame()) {
            ...
            GetDocument().GetFrame()->GetAttributionSrcLoader()->Register(params.new_value, /*element=*/this, referrer_policy);
        }
    } else {
...
Node::InsertionNotificationRequest HTMLScriptElement::InsertedInto(ContainerNode& insertion_point)
{
    if (insertion_point.isConnected() && HasSourceAttribute()
        && ScriptLoader::GetScriptTypeAtPrepare(TypeAttributeValue(), LanguageAttributeValue()) == ScriptLoader::ScriptTypeAtPrepare::kInvalid) {
        UseCounter::Count(GetDocument(), WebFeature::kScriptElementWithInvalidTypeHasSrc);
    }
...
void HTMLScriptElement::RemovedFrom(ContainerNode& insertion_point)
{
    ...
    loader_->Removed();
    if (GetDocument().GetRenderBlockingResourceManager() && !GetDocument().StatePreservingAtomicMoveInProgress()) {
        GetDocument().GetRenderBlockingResourceManager()->RemovePendingScript(*this);
    }
}
...
void HTMLScriptElement::setInnerTextForBinding(
    const V8UnionStringLegacyNullToEmptyStringOrTrustedScript* string_or_trusted_script, ExceptionState& exception_state)
{
    const String& value = TrustedTypesCheckForScript(string_or_trusted_script, GetExecutionContext(), "HTMLScriptElement", "innerText", exception_state);
...
void HTMLScriptElement::setTextContentForBinding(const V8UnionStringOrTrustedScript* value, ExceptionState& exception_state)
{
    const String& string = TrustedTypesCheckForScript(value, GetExecutionContext(), "HTMLScriptElement", "textContent", exception_state);
    if (exception_state.HadException())
        return;
    setTextContent(string);
}
...
{
    return ContentSecurityPolicy::IsNonceableElement(this) ? nonce() : g_null_atom;
}

bool HTMLScriptElement::AllowInlineScriptForCSP(const AtomicString& nonce, const WTF::OrdinalNumber& context_line, const String& script_content)
{
    ...
    DCHECK(loader_);
    ContentSecurityPolicy::InlineType inline_type = loader_->GetScriptType() == ScriptLoader::ScriptTypeAtPrepare::kSpeculationRules
        ? ContentSecurityPolicy::InlineType::kScriptSpeculationRules
        : ContentSecurityPolicy::InlineType::kScript;
    return GetExecutionContext()->GetContentSecurityPolicyForCurrentWorld()->AllowInline(
        inline_type, this, script_content, nonce, GetDocument().Url(), context_line);
}
...
Element& HTMLScriptElement::CloneWithoutAttributesAndChildren(Document& factory) const
{
    CreateElementFlags flags = CreateElementFlags::ByCloneNode().SetAlreadyStarted(loader_->AlreadyStarted());
...
bool HTMLScriptElement::IsPotentiallyRenderBlocking() const
{
    ...
    if (loader_->IsParserInserted() && loader_->GetScriptType() == ScriptLoader::ScriptTypeAtPrepare::kClassic) {
        ...
        if (base::FeatureList::IsEnabled(features::kForceInOrderScript) && HasSourceAttribute())
            return false;
