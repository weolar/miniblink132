...
TEST(HTMLParsePerfTest, Speedometer)
{
    ...
    const std::string html_parse_iterations_str = base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII("html-parse-iterations");
    int html_parse_iterations = html_parse_iterations_str.empty() ? 1 : stoi(html_parse_iterations_str);

    auto reporter = perf_test::PerfResultReporter("BlinkHTML", label);

    std::optional<Vector<char>> serialized = test::ReadFromFile(test::CoreTestDataPath(filename));
    CHECK(serialized);
