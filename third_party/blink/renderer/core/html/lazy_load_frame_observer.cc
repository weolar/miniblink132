...
// properly.
bool IsFrameProbablyHidden(const gfx::RectF& bounding_client_rect, const Element& element)
{
    ...
    if (bounding_client_rect.width() <= 4.0f || bounding_client_rect.height() <= 4.0f) {
        return true;
    }
    ...
    if (bounding_client_rect.right() < 0.0f || bounding_client_rect.bottom() < 0.0f) {
        return true;
    }
...
struct LazyLoadFrameObserver::LazyLoadRequestInfo {
    LazyLoadRequestInfo(const ResourceRequestHead& passed_resource_request, WebFrameLoadType frame_load_type)
...
LazyLoadFrameObserver::LazyLoadFrameObserver(HTMLFrameOwnerElement& element, LoadType load_type)
    : element_(&element)
...
void LazyLoadFrameObserver::DeferLoadUntilNearViewport(const ResourceRequestHead& resource_request, WebFrameLoadType frame_load_type)
{
    DCHECK(!lazy_load_intersection_observer_);
    DCHECK(!lazy_load_request_info_);
    lazy_load_request_info_ = std::make_unique<LazyLoadRequestInfo>(resource_request, frame_load_type);

    if (RuntimeEnabledFeatures::LazyLoadScrollMarginIframeEnabled()) {
        lazy_load_intersection_observer_ = IntersectionObserver::Create(element_->GetDocument(),
            WTF::BindRepeating(&LazyLoadFrameObserver::LoadIfHiddenOrNearViewport, WrapWeakPersistent(this)),
            LocalFrameUkmAggregator::kLazyLoadIntersectionObserver,
            IntersectionObserver::Params {
                .scroll_margin = { Length::Fixed(GetLazyLoadingFrameMarginPx(element_->GetDocument())) },
        ...
        lazy_load_intersection_observer_ = IntersectionObserver::Create(element_->GetDocument(),
            ...
            LocalFrameUkmAggregator::kLazyLoadIntersectionObserver,
            IntersectionObserver::Params {
                .margin = { Length::Fixed(GetLazyLoadingFrameMarginPx(element_->GetDocument())) },
                .thresholds = { std::numeric_limits<float>::min() },
            });
    }
...
void LazyLoadFrameObserver::LoadIfHiddenOrNearViewport(const HeapVector<Member<IntersectionObserverEntry>>& entries)
{
    ...
    LoadingAttributeValue loading_attr = GetLoadingAttributeValue(element_->FastGetAttribute(html_names::kLoadingAttr));
    if (loading_attr != LoadingAttributeValue::kLazy && IsFrameProbablyHidden(entries.back()->GetGeometry().TargetRect(), *element_)) {
        LoadImmediately();
        return;
    }
}
...
void LazyLoadFrameObserver::LoadImmediately()
{
    ...
    std::unique_ptr<LazyLoadRequestInfo> scoped_request_info = std::move(lazy_load_request_info_);
    ...
    FrameLoadRequest request(element_->GetDocument().domWindow(), scoped_request_info->resource_request);

    if (load_type_ == LoadType::kFirst) {
        To<LocalFrame>(element_->ContentFrame())->Loader().StartNavigation(request, scoped_request_info->frame_load_type);
    } else if (load_type_ == LoadType::kSubsequent) {
        element_->ContentFrame()->Navigate(request, scoped_request_info->frame_load_type);
    }
