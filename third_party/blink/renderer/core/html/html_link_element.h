...
public:
    ...
    String TypeValue() const
    {
        return type_;
    }
    String AsValue() const
    {
        return as_;
    }
    String IntegrityValue() const
    {
        return integrity_;
    }
    String FetchPriorityHintValue() const
    {
    ...
    {
        return rel_attribute_;
    }
    DOMTokenList& relList() const
    {
        return static_cast<DOMTokenList&>(*rel_list_);
    }
    BlockingAttribute& blocking() const
    {
        return *blocking_attribute_;
    }
    ...
    void LoadStylesheet(
        const LinkLoadParameters&, const WTF::TextEncoding&, FetchParameters::DeferOption, ResourceClient*, RenderBlockingBehavior render_blocking);
    bool IsAlternate() const
    {
        return GetLinkStyle()->IsUnset() && rel_attribute_.IsAlternate() && !GetLinkStyle()->IsExplicitlyEnabled();
    }
    bool ShouldProcessStyle()
    {
        return LinkResourceToProcess() && GetLinkStyle();
    }
    bool IsCreatedByParser() const
    {
        return created_by_parser_;
    }
...
private:
    ...
    void Process(LinkLoadParameters::Reason reason = LinkLoadParameters::Reason::kDefault);
    ...
    void HandleExpectHrefChanges(const String& old_value, const String& new_value);
    void HandleExpectMediaChanges();

    void RemoveExpectRenderBlockingLink(const String& href = String());
    void AddExpectRenderBlockingLinkIfNeeded(const String& href = String(), bool media_known_to_match = false);
