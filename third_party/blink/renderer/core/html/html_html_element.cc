...
{
    return attribute.GetName() == html_names::kManifestAttr || HTMLElement::IsURLAttribute(attribute);
}
...
bool NeedsLayoutStylePropagation(const ComputedStyle& layout_style, const ComputedStyle& propagated_style)
{
    return layout_style.GetWritingMode() != propagated_style.GetWritingMode() || layout_style.Direction() != propagated_style.Direction();
}

const ComputedStyle* CreateLayoutStyle(const ComputedStyle& style, const ComputedStyle& propagated_style)
{
...
const ComputedStyle* HTMLHtmlElement::LayoutStyleForElement(const ComputedStyle* style)
{
    DCHECK(style);
...
void HTMLHtmlElement::PropagateWritingModeAndDirectionFromBody()
{
    ...
    const ComputedStyle* new_style = LayoutStyleForElement(layout_object->Style());

    if (old_style == new_style)
        return;

    const bool is_orthogonal = old_style->IsHorizontalWritingMode() != new_style->IsHorizontalWritingMode();
    ...
    for (Node* node = firstChild(); node; node = node->nextSibling()) {
        ...
        auto* const text_combine = DynamicTo<LayoutTextCombine>(layout_text->Parent());
