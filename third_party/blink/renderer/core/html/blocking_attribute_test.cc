...
TEST_F(BlockingAttributeTest, CountRenderTokenUsageInLink)
{
    ...
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kBlockingAttributeRenderToken));
}
...
TEST_F(BlockingAttributeTest, NoCountIfElementDoesNotSupportTheAttribute)
{
    ...
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kBlockingAttributeRenderToken));
}
