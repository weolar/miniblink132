...
public:
    FrameEdgeInfo(bool prevent_resize, bool allow_border)
        : prevent_resize_(kFrameEdgeCount)
        , allow_border_(kFrameEdgeCount)
    {
        prevent_resize_.Fill(prevent_resize);
        allow_border_.Fill(allow_border);
    }

    bool PreventResize(FrameEdge edge) const
    {
        return prevent_resize_[edge];
    }
    bool AllowBorder(FrameEdge edge) const
    {
        return allow_border_[edge];
    }
