...
Node::InsertionNotificationRequest HTMLTitleElement::InsertedInto(ContainerNode& insertion_point)
{
...
void HTMLTitleElement::setText(const String& value)
{
...

    {
        ...
        base::AutoReset<bool> inhibit_title_update_scope(&ignore_title_updates_when_children_change_, !value.empty());
        RemoveChildren(kOmitSubtreeModifiedEvent);
    }

    if (!value.empty()) {
        AppendChild(GetDocument().createTextNode(value.Impl()), IGNORE_EXCEPTION_FOR_TESTING);
    }
}
