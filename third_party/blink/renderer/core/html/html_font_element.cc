...
template <typename CharacterType> static bool ParseFontSize(const CharacterType* characters, unsigned length, int& size)
{
    // Step 1
    // Step 2
    ...
    // Step 8
    int value
        = CharactersToInt(base::span<const CharacterType>(digits_start, static_cast<size_t>(position - digits_start)), WTF::NumberParsingOptions(), nullptr);
...
static const CSSValueList* CreateFontFaceValueWithPool(const AtomicString& string, SecureContextMode secure_context_mode)
{
    CSSValuePool::FontFaceValueCache::AddResult entry = CssValuePool().GetFontFaceCacheEntry(string);
    if (!entry.stored_value->value) {
        const CSSValue* parsed_value = CSSParser::ParseSingleValue(CSSPropertyID::kFontFamily, string, StrictCSSParserContext(secure_context_mode));
...
{
    if (name == html_names::kSizeAttr || name == html_names::kColorAttr || name == html_names::kFaceAttr)
        return true;
    return HTMLElement::IsPresentationAttribute(name);
}

void HTMLFontElement::CollectStyleForPresentationAttribute(const QualifiedName& name, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
    if (name == html_names::kSizeAttr) {
        CSSValueID size = CSSValueID::kInvalid;
        if (CssValueFromFontSizeNumber(value, size)) {
            AddPropertyToPresentationAttributeStyle(style, CSSPropertyID::kFontSize, size);
        }
        ...
        if (const CSSValueList* font_face_value = CreateFontFaceValueWithPool(value, GetExecutionContext()->GetSecureContextMode())) {
            style->SetLonghandProperty(CSSPropertyValue(CSSPropertyName(CSSPropertyID::kFontFamily), *font_face_value));
        }
    } else {
