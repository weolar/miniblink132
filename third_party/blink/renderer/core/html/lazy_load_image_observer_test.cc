...
{
    DEFINE_THREAD_SAFE_STATIC_LOCAL(const Vector<char>, test_image, (*test::ReadFromFile(test::CoreTestDataPath("notifications/500x500.png"))));
    return test_image;
}
...
TEST_F(LazyLoadImagesSimTest, ImgSrcset)
{
    ...
    GetDocument().View()->LayoutViewport()->SetScrollOffset(ScrollOffset(0, 10000), mojom::blink::ScrollType::kProgrammatic);
    ...
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kLazyLoadImageLoadingAttributeLazy));
}

class LazyLoadImagesParamsTest : public SimTest, public ::testing::WithParamInterface<WebEffectiveConnectionType> {
public:
    ...
    void SetUp() override
    {
        GetNetworkStateNotifier().SetNetworkConnectionInfoOverride(
            true /*on_line*/, kWebConnectionTypeWifi, GetParam(), 1000 /*http_rtt_msec*/, 100 /*max_bandwidth_mbps*/);

        SimTest::SetUp();
        WebView().MainFrameViewWidget()->Resize(gfx::Size(kViewportWidth, kViewportHeight));
    ...
    int GetMargin() const
    {
        ...
        return kDistanceThresholdByEffectiveConnectionType[static_cast<int>(GetParam())];
    }
};
...
{
    SimRequest main_resource("https://example.com/", "text/html");
    SimSubresourceRequest css_resource("https://example.com/style.css", "text/css");

    SimSubresourceRequest eager_resource("https://example.com/eager.png", "image/png");
    std::optional<SimSubresourceRequest> lazy_resource, auto_resource, unset_resource;
...
INSTANTIATE_TEST_SUITE_P(LazyImageLoading, LazyLoadImagesParamsTest,
...
public:
    ...
    void LoadMainResourceWithImageFarFromViewport(const String& main_resource_string)
    {
        SimRequest main_resource("https://example.com/", "text/html");
        LoadURL("https://example.com/");
    ...
    {
        LoadMainResourceWithImageFarFromViewport(MakeMainResourceString(image_attributes));
    }
    ...
    {
        SimSubresourceRequest full_resource("https://example.com/image.png", "image/png");
        ...
        EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kLazyLoadImageLoadingAttributeLazy));
        EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kLazyLoadImageLoadingAttributeEager));
    }
};
...
TEST_F(LazyLoadImagesTest, LoadAllImagesIfPrinting)
{
    ...
    SimSubresourceRequest img_resource("https://example.com/image.png", "image/png");
...
TEST_F(LazyLoadImagesTest, LoadAllImagesIfPrintingIFrame)
{
    ...
    const String main_resource = String::Format(R"HTML(
    <body onload='console.log("main body onload");'>
    <div style='height: %dpx;'></div>
...
TEST_F(LazyLoadImagesTest, AttributeChangedFromLazyToEager)
{
    ...
    SimSubresourceRequest full_resource("https://example.com/image.png", "image/png");
    GetDocument().getElementById(AtomicString("my_image"))->setAttribute(html_names::kLoadingAttr, AtomicString("eager"));
...
TEST_F(LazyLoadImagesTest, ImageInsideLazyLoadedFrame)
{
    ...
    SimSubresourceRequest css_resource("https://example.com/style.css", "text/css");
    SimSubresourceRequest eager_resource("https://example.com/eager.png", "image/png");
    SimSubresourceRequest auto_resource("https://example.com/auto.png", "image/png");
    SimSubresourceRequest unset_resource("https://example.com/unset.png", "image/png");
    ...
    SimSubresourceRequest lazy_resource("https://example.com/lazy.png", "image/png");

    GetDocument().View()->LayoutViewport()->SetScrollOffset(ScrollOffset(0, 250), mojom::blink::ScrollType::kProgrammatic);
...
TEST_F(LazyLoadImagesTest, LazyLoadFileUrls)
{
    ...
    auto* lazy = To<HTMLImageElement>(GetDocument().getElementById(AtomicString("lazy")));
    EXPECT_FALSE(lazy->CachedImage()->IsLoading());

    // Scroll down such that the image is visible.
    GetDocument().View()->LayoutViewport()->SetScrollOffset(
        ScrollOffset(0, kViewportHeight + kLoadingDistanceThreshold), mojom::blink::ScrollType::kProgrammatic);
...
TEST_F(LazyLoadImagesTest, GarbageCollectDeferredLazyLoadImages)
{
    ...
    WeakPersistent<HTMLImageElement> image = To<HTMLImageElement>(GetDocument().QuerySelector(AtomicString("img")));
