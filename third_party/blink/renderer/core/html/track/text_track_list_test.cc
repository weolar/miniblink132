...
TEST(TextTrackListTest, InvalidateTrackIndexes)
{
    ...
    auto* list = MakeGarbageCollected<TextTrackList>(MakeGarbageCollected<HTMLVideoElement>(std::make_unique<DummyPageHolder>()->GetDocument()));
    const size_t kNumTextTracks = 4;
    std::array<TextTrack*, kNumTextTracks> text_tracks;
    for (size_t i = 0; i < kNumTextTracks; ++i) {
        text_tracks[i] = MakeGarbageCollected<TextTrack>(V8TextTrackKind(V8TextTrackKind::Enum::kSubtitles), g_empty_atom, g_empty_atom, *list->Owner());
