...
// incorrect results.
gfx::Rect BorderBoxRelativeToAncestor(const LayoutBox& box, const LayoutBoxModelObject& ancestor)
{
...
gfx::Rect ComputeControlsRect(const LayoutObject& container)
{
    ...
    if (!button_panel_layout_object || !button_panel_layout_object->IsBox() || !timeline_layout_object || !timeline_layout_object->IsBox()) {
        return gfx::Rect();
    }

    const auto& container_box = *To<LayoutBox>(controls->ContainerLayoutObject());
    gfx::Rect button_panel_box = BorderBoxRelativeToAncestor(To<LayoutBox>(*button_panel_layout_object), container_box);
    gfx::Rect timeline_box = BorderBoxRelativeToAncestor(To<LayoutBox>(*timeline_layout_object), container_box);
...
VttCueLayoutAlgorithm::VttCueLayoutAlgorithm(VTTCueBox& cue)
    : cue_(cue)
    , snap_to_lines_position_(cue.SnapToLinesPosition())
{
}
...
// static
PhysicalSize VttCueLayoutAlgorithm::FirstInlineBoxSize(const LayoutBox& cue_box)
{
...
LayoutUnit VttCueLayoutAlgorithm::ComputeInitialPositionAdjustment(LayoutUnit max_dimension, const gfx::Rect& controls_rect)
{
...
gfx::Rect VttCueLayoutAlgorithm::CueBoundingBox(const LayoutBox& cue_box)
{
    ...
    PhysicalRect border_box = cue_box.LocalToAncestorRect(cue_box.PhysicalBorderBoxRect(), container);
...
bool VttCueLayoutAlgorithm::IsOverlapping(const gfx::Rect& controls_rect) const
{
    gfx::Rect cue_box_rect = CueBoundingBox(*cue_.GetLayoutBox());
    for (LayoutBox* box = cue_.GetLayoutBox()->PreviousSiblingBox(); box; box = box->PreviousSiblingBox()) {
        if (IsA<VTTCueBox>(box->GetNode()) && cue_box_rect.Intersects(CueBoundingBox(*box)))
            return true;
    }
    return cue_box_rect.Intersects(controls_rect);
}

bool VttCueLayoutAlgorithm::ShouldSwitchDirection(LayoutUnit cue_block_position, LayoutUnit cue_block_size, LayoutUnit full_dimension) const
{
    ...
    if (step_ < 0 && cue_block_position < margin_)
        return true;
    if (step_ > 0 && cue_block_position + cue_block_size + margin_ > full_dimension)
        return true;
    return false;
}
...
void VttCueLayoutAlgorithm::AdjustPositionWithSnapToLines()
{
    ...
    const LayoutUnit full_dimension = is_horizontal ? container.Size().height : container.Size().width;
    ...
    const double margin_ratio = settings ? settings->GetTextTrackMarginPercentage() / 100.0 : 0;
    ...
    LayoutUnit position = ComputeInitialPositionAdjustment(max_dimension, controls_rect);

    // 10. Horizontal: Move all the boxes in boxes down by the distance given
    // by position.
    //     Vertical: Move all the boxes in boxes right ...
    LayoutUnit& adjusted_position = cue_.StartAdjustment(cue_.AdjustedPosition(full_dimension, PassKey()) + position, PassKey());
    ...
    while (IsOutside(title_area) || IsOverlapping(controls_rect)) {
        // Step 14
        if (!ShouldSwitchDirection(adjusted_position, cue_box.LogicalHeight(), full_dimension)) {
    ...
    double percentage_position = full_dimension ? (adjusted_position * 100 / full_dimension).ToDouble() : 100.0;
    cue_.SetInlineStyleProperty(is_horizontal ? CSSPropertyID::kTop : CSSPropertyID::kLeft, percentage_position, CSSPrimitiveValue::UnitType::kPercentage);
}
