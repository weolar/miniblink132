...
CueInterval CreateCueInterval(TextTrackCue* cue)
{
    ...
    return CueIntervalTree::CreateInterval(cue->startTime(), interval_end_time, cue);
}

base::TimeDelta CalculateEventTimeout(double event_time, HTMLMediaElement const& media_element)
{
    static_assert(HTMLMediaElement::kMinPlaybackRate >= 0, "The following code assumes playback rates are never negative");
    DCHECK_NE(media_element.playbackRate(), 0);

    auto const timeout = base::Seconds((event_time - media_element.currentTime()) / media_element.playbackRate());
    ...
    return std::max(timeout.CeilToMultiple(kMinTimeoutInterval), kMinTimeoutInterval);
}
...
CueTimeline::CueTimeline(HTMLMediaElement& media_element)
    ...
    , last_update_time_(-1)
    , cue_event_timer_(media_element.GetDocument().GetTaskRunner(TaskType::kInternalMedia), this, &CueTimeline::CueEventTimerFired)
    , cue_timestamp_event_timer_(media_element.GetDocument().GetTaskRunner(TaskType::kInternalMedia), this, &CueTimeline::CueTimestampEventTimerFired)
    , ignore_update_(0)
...
static Event* CreateEventWithTarget(const AtomicString& event_name, EventTarget* event_target)
{
...
void CueTimeline::TimeMarchesOn()
{
    ...
    // kHaveNothing.
    if (media_element.getReadyState() != HTMLMediaElement::kHaveNothing && media_element.GetWebMediaPlayer()) {
        current_cues = cue_tree_.AllOverlaps(cue_tree_.CreateInterval(movie_time, movie_time));
    }
    ...
    if (last_time >= 0 && last_seek_time < movie_time) {
        CueList potentially_skipped_cues = cue_tree_.AllOverlaps(cue_tree_.CreateInterval(last_time, movie_time));
        missed_cues.ReserveInitialCapacity(potentially_skipped_cues.size());

        for (CueInterval cue : potentially_skipped_cues) {
            // Consider cues that may have been missed since the last seek time.
            if (cue.Low() > std::max(last_seek_time, last_time) && cue.High() < movie_time)
    ...
    for (wtf_size_t i = 0; !active_set_changed && i < previous_cues_size; ++i) {
        if (!current_cues.Contains(previous_cues[i]) && previous_cues[i].Data()->IsActive())
    ...
    for (wtf_size_t i = 0; !media_element.paused() && i < previous_cues_size; ++i) {
        if (previous_cues[i].Data()->pauseOnExit() && previous_cues[i].Data()->IsActive() && !current_cues.Contains(previous_cues[i]))
            media_element.pause();
    }
    ...
    for (const auto& missed_cue : missed_cues) {
        ...
        event_tasks.push_back(std::make_pair(missed_cue.Data()->startTime(), missed_cue.Data()));
        ...
        if (missed_cue.Data()->startTime() < missed_cue.Data()->endTime()) {
            event_tasks.push_back(std::make_pair(missed_cue.Data()->endTime(), missed_cue.Data()));
        }
    }
    ...
    for (const auto& previous_cue : previous_cues) {
        ...
        if (!current_cues.Contains(previous_cue)) {
            event_tasks.push_back(std::make_pair(previous_cue.Data()->endTime(), previous_cue.Data()));
        }
    }
    ...
    for (const auto& current_cue : current_cues) {
        ...
        if (!previous_cues.Contains(current_cue)) {
            event_tasks.push_back(std::make_pair(current_cue.Data()->startTime(), current_cue.Data()));
        }
    }
    ...
    for (const auto& task : event_tasks) {
        ...
        if (task.second->startTime() >= task.second->endTime()) {
            media_element.ScheduleEvent(CreateEventWithTarget(event_type_names::kEnter, task.second.Get()));
            media_element.ScheduleEvent(CreateEventWithTarget(event_type_names::kExit, task.second.Get()));
        } else {
            TextTrackCue* cue = task.second.Get();
            bool is_enter_event = task.first == task.second->startTime();
            AtomicString event_name = is_enter_event ? event_type_names::kEnter : event_type_names::kExit;
            media_element.ScheduleEvent(CreateEventWithTarget(event_name, task.second.Get()));
    ...
    for (const auto& track : affected_tracks) {
        media_element.ScheduleEvent(CreateEventWithTarget(event_type_names::kCuechange, track.Get()));
        ...
        if (auto* loadable_text_track = DynamicTo<LoadableTextTrack>(track.Get())) {
            ...
            media_element.ScheduleEvent(CreateEventWithTarget(event_type_names::kCuechange, track_element));
        }
    }
...
void CueTimeline::EndIgnoreUpdateScope(base::PassKey<IgnoreUpdateScope>, IgnoreUpdateScope const& scope)
{
...
void CueTimeline::SetCueEventTimer()
{
    ...
    if (!next_cue_event_.has_value() || media_element.paused() || media_element.playbackRate() == 0) {
        CancelCueEventTimer();
        return;
    }

    auto const timeout = CalculateEventTimeout(next_cue_event_.value(), media_element);
...
void CueTimeline::SetCueTimestampEventTimer()
{
    ...
    for (auto cue : currently_active_cues_) {
        ...
        next_cue_timestamp_event = std::min(next_cue_timestamp_event, timestamp.value_or(kInfinity));
    }
    ...
    auto const timeout = CalculateEventTimeout(next_cue_timestamp_event, media_element);
...
{
    cue_event_timer_.MoveToNewTaskRunner(MediaElement().GetDocument().GetTaskRunner(TaskType::kInternalMedia));
    cue_timestamp_event_timer_.MoveToNewTaskRunner(MediaElement().GetDocument().GetTaskRunner(TaskType::kInternalMedia));
}
