...
static int TextTrackLanguageSelectionScore(const TextTrack& track)
{
    ...
    wtf_size_t language_match_index = IndexOfBestMatchingLanguageInList(track.language(), languages);
...
{
    if (configuration_.text_track_kind_user_preference == TextTrackKindUserPreference::kSubtitles)
        return TextTrack::SubtitlesKeyword();
    if (configuration_.text_track_kind_user_preference == TextTrackKindUserPreference::kCaptions)
...
void AutomaticTrackSelection::PerformAutomaticTextTrackSelection(const TrackGroup& group)
{
    ...
    for (const auto& text_track : group.tracks) {
        if (configuration_.disable_currently_enabled_tracks && text_track->mode() == TextTrackMode::kShowing)
        ...
        } else if (!group.visible_track && !default_track && text_track->IsDefault()) {
    ...
    if (configuration_.text_track_kind_user_preference != TextTrackKindUserPreference::kDefault)
        track_to_enable = preferred_track;

    if (!track_to_enable && default_track)
        track_to_enable = default_track;

    if (!track_to_enable && configuration_.force_enable_subtitle_or_caption_track && group.kind == TrackGroup::kCaptionsAndSubtitles) {
        if (fallback_track)
...
void AutomaticTrackSelection::Perform(TextTrackList& text_tracks)
{
    ...
    for (wtf_size_t i = 0; i < text_tracks.length(); ++i) {
        ...
        if (!current_group->visible_track && text_track->mode() == TextTrackMode::kShowing)
