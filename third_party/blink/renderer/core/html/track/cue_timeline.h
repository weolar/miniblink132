...
public:
    ...
    public:
        ~IgnoreUpdateScope()
        {
            ...
            cue_timeline_->EndIgnoreUpdateScope(base::PassKey<IgnoreUpdateScope>(), *this);
        }
    ...
    const CueList& CurrentlyActiveCues() const
    {
        return currently_active_cues_;
    }
    ...
    bool InsideIgnoreUpdateScope() const
    {
        return ignore_update_ > 0;
    }
    IgnoreUpdateScope BeginIgnoreUpdateScope();
    void EndIgnoreUpdateScope(base::PassKey<IgnoreUpdateScope>, IgnoreUpdateScope const& scope);
...
template <> struct ValueToString<blink::TextTrackCue*> {
