...
std::ostream& blink::mojom::operator<<(std::ostream& a, ::blink::mojom::PermissionName b)
{
    *(int*)1 = 1;
    return a << (int)(b);
}
...
// list.
Vector<PermissionDescriptorPtr> ParsePermissionDescriptorsFromString(const AtomicString& type)
{
    ...
    for (unsigned i = 0; i < permissions.size(); i++) {
        if (permissions[i] == "geolocation") {
            permission_descriptors.push_back(CreatePermissionDescriptor(PermissionName::GEOLOCATION));
        } else if (permissions[i] == "camera") {
            permission_descriptors.push_back(CreatePermissionDescriptor(PermissionName::VIDEO_CAPTURE));
        } else if (permissions[i] == "microphone") {
            permission_descriptors.push_back(CreatePermissionDescriptor(PermissionName::AUDIO_CAPTURE));
        } else {
    ...
    if ((permission_descriptors[0]->name == PermissionName::VIDEO_CAPTURE && permission_descriptors[1]->name == PermissionName::AUDIO_CAPTURE)
        || (permission_descriptors[0]->name == PermissionName::AUDIO_CAPTURE && permission_descriptors[1]->name == PermissionName::VIDEO_CAPTURE)) {
        return permission_descriptors;
    }
...
int GetTranslatedMessageID(int message_id, const AtomicString& language_string)
{
...
// one element.
int GetUntranslatedMessageIDSinglePermission(PermissionName name, bool granted, bool is_precise_location)
{
    if (name == PermissionName::VIDEO_CAPTURE) {
        return granted ? IDS_PERMISSION_REQUEST_CAMERA_ALLOWED : IDS_PERMISSION_REQUEST_CAMERA;
    }

    if (name == PermissionName::AUDIO_CAPTURE) {
        return granted ? IDS_PERMISSION_REQUEST_MICROPHONE_ALLOWED : IDS_PERMISSION_REQUEST_MICROPHONE;
    }

    if (name == PermissionName::GEOLOCATION) {
        if (is_precise_location) {
            // This element uses precise location.
            return granted ? IDS_PERMISSION_REQUEST_PRECISE_GEOLOCATION_ALLOWED : IDS_PERMISSION_REQUEST_PRECISE_GEOLOCATION;
        }
        return granted ? IDS_PERMISSION_REQUEST_GEOLOCATION_ALLOWED : IDS_PERMISSION_REQUEST_GEOLOCATION;
    }
...
{
    return granted ? IDS_PERMISSION_REQUEST_CAMERA_MICROPHONE_ALLOWED : IDS_PERMISSION_REQUEST_CAMERA_MICROPHONE;
}

// Helper to get `PermissionsPolicyFeature` from permission name
mojom::blink::PermissionsPolicyFeature PermissionNameToPermissionsPolicyFeature(PermissionName permission_name)
{
...
float ContrastBetweenColorAndBackgroundColor(const ComputedStyle* style)
{
    return color_utils::GetContrastRatio(
        style->VisitedDependentColor(GetCSSPropertyColor()).toSkColor4f(), style->VisitedDependentColor(GetCSSPropertyBackgroundColor()).toSkColor4f());
}
...
{
    auto background_color = style->VisitedDependentColor(GetCSSPropertyBackgroundColor()).toSkColor4f();
    SkColor4f border_colors[] = { style->VisitedDependentColor(GetCSSPropertyBorderBottomColor()).toSkColor4f(),
        style->VisitedDependentColor(GetCSSPropertyBorderTopColor()).toSkColor4f(), style->VisitedDependentColor(GetCSSPropertyBorderLeftColor()).toSkColor4f(),
        style->VisitedDependentColor(GetCSSPropertyBorderRightColor()).toSkColor4f() };
...
{
    return style->VisitedDependentColor(GetCSSPropertyColor()).Alpha() != 1. || style->VisitedDependentColor(GetCSSPropertyBackgroundColor()).Alpha() != 1;
}
...
{
    return style->VisitedDependentColor(GetCSSPropertyBorderBottomColor()).Alpha() != 1.
        || style->VisitedDependentColor(GetCSSPropertyBorderTopColor()).Alpha() != 1.
        || style->VisitedDependentColor(GetCSSPropertyBorderLeftColor()).Alpha() != 1.
        || style->VisitedDependentColor(GetCSSPropertyBorderRightColor()).Alpha() != 1.;
}

bool IsBorderSufficientlyDistinctFromBackgroundColor(const ComputedStyle* style)
{
...
scoped_refptr<const CalculationExpressionNode> BuildFitContentExpr(float factor)
{
    auto constant_expr = base::MakeRefCounted<CalculationExpressionNumberNode>(factor);
    auto size_expr = base::MakeRefCounted<CalculationExpressionSizingKeywordNode>(CalculationExpressionSizingKeywordNode::Keyword::kSize);
    return CalculationExpressionOperationNode::CreateSimplified(
        CalculationExpressionOperationNode::Children({ constant_expr, size_expr }), CalculationOperator::kMultiply);
}

// Builds an expression that takes a |length| and bounds it lower, higher, or on
// both sides with the provided expressions.
scoped_refptr<const CalculationExpressionNode> BuildLengthBoundExpr(const Length& length,
    std::optional<scoped_refptr<const CalculationExpressionNode>> lower_bound_expr,
    std::optional<scoped_refptr<const CalculationExpressionNode>> upper_bound_expr)
{
    if (lower_bound_expr.has_value() && upper_bound_expr.has_value()) {
        return CalculationExpressionOperationNode::CreateSimplified(CalculationExpressionOperationNode::Children({ lower_bound_expr.value(),
                                                                        length.AsCalculationValue()->GetOrCreateExpression(), upper_bound_expr.value() }),
            CalculationOperator::kClamp);
    }

    if (lower_bound_expr.has_value()) {
        return CalculationExpressionOperationNode::CreateSimplified(
            CalculationExpressionOperationNode::Children({ lower_bound_expr.value(), length.AsCalculationValue()->GetOrCreateExpression() }),
            CalculationOperator::kMax);
    }

    if (upper_bound_expr.has_value()) {
        return CalculationExpressionOperationNode::CreateSimplified(
            CalculationExpressionOperationNode::Children({ upper_bound_expr.value(), length.AsCalculationValue()->GetOrCreateExpression() }),
...
HTMLPermissionElement::HTMLPermissionElement(Document& document)
    ...
    , embedded_permission_control_receiver_(this, document.GetExecutionContext())
    , disable_reason_expire_timer_(this, &HTMLPermissionElement::DisableReasonExpireTimerFired)
{
    DCHECK(RuntimeEnabledFeatures::PermissionElementEnabled(document.GetExecutionContext()));
    SetHasCustomStyleCallbacks();
    intersection_observer_
        = IntersectionObserver::Create(GetDocument(), WTF::BindRepeating(&HTMLPermissionElement::OnIntersectionChanged, WrapWeakPersistent(this)),
...
{
    return V8PermissionState(PermissionStatusToV8Enum(initial_aggregated_permission_status_.value_or(MojoPermissionStatus::ASK)));
}

V8PermissionState HTMLPermissionElement::permissionStatus() const
{
    return V8PermissionState(PermissionStatusToV8Enum(aggregated_permission_status_.value_or(MojoPermissionStatus::ASK)));
}
...
void HTMLPermissionElement::OnPermissionStatusInitialized(PermissionStatusMap initilized_map)
{
    permission_status_map_ = std::move(initilized_map);
    UpdatePermissionStatusAndAppearance();
}

Node::InsertionNotificationRequest HTMLPermissionElement::InsertedInto(ContainerNode& insertion_point)
{
...
void HTMLPermissionElement::AttachLayoutTree(AttachContext& context)
{
    ...
    DisableClickingTemporarily(DisableReason::kRecentlyAttachedToLayoutTree, kDefaultDisableTimeout);
...
void HTMLPermissionElement::RemovedFrom(ContainerNode& insertion_point)
{
    ...
    if (LocalDOMWindow* window = GetDocument().domWindow()) {
        CachedPermissionStatus::From(window)->UnregisterClient(this, permission_descriptors_);
    }
}
...
FocusableState HTMLPermissionElement::SupportsFocus(UpdateBehavior) const
{
    ...
    return permission_descriptors_.empty() ? FocusableState::kNotFocusable : FocusableState::kFocusable;
}
...
{
    return !GetRecentlyAttachedTimeoutRemaining() && IsClickingDisabledIndefinitely(DisableReason::kIntersectionVisibilityOccludedOrDistorted);
}

// static
Vector<PermissionDescriptorPtr> HTMLPermissionElement::ParsePermissionDescriptorsForTesting(const AtomicString& type)
{
...
// static
HTMLPermissionElement::UserInteractionDeniedReason HTMLPermissionElement::DisableReasonToUserInteractionDeniedReason(DisableReason reason)
{
    switch (reason) {
    case DisableReason::kRecentlyAttachedToLayoutTree:
    ...
    case DisableReason::kIntersectionVisibilityOutOfViewPortOrClipped:
        return UserInteractionDeniedReason::kIntersectionVisibilityOutOfViewPortOrClipped;
    case DisableReason::kIntersectionVisibilityOccludedOrDistorted:
        return UserInteractionDeniedReason::kIntersectionVisibilityOccludedOrDistorted;
    case DisableReason::kInvalidStyle:
...
// static
AtomicString HTMLPermissionElement::DisableReasonToInvalidReasonString(DisableReason reason)
{
    switch (reason) {
    case DisableReason::kRecentlyAttachedToLayoutTree:
...
PermissionService* HTMLPermissionElement::GetPermissionService()
{
    if (!permission_service_.is_bound()) {
        GetExecutionContext()->GetBrowserInterfaceBroker().GetInterface(permission_service_.BindNewPipeAndPassReceiver(GetTaskRunner()));
        permission_service_.set_disconnect_handler(WTF::BindOnce(&HTMLPermissionElement::OnPermissionServiceConnectionFailed, WrapWeakPersistent(this)));
    }
...
bool HTMLPermissionElement::MaybeRegisterPageEmbeddedPermissionControl()
{
    ...
    if (frame->IsInFencedFrameTree()) {
        AddConsoleError(String::Format("The permission '%s' is not allowed in fenced frame", GetType().Utf8().c_str()));
        return false;
    }

    if (frame->IsCrossOriginToOutermostMainFrame() && !GetExecutionContext()->GetContentSecurityPolicy()->HasEnforceFrameAncestorsDirectives()) {
        AddConsoleError(String::Format("The permission '%s' is not allowed without the CSP "
    ...
    for (const PermissionDescriptorPtr& descriptor : permission_descriptors_) {
        if (!GetExecutionContext()->IsFeatureEnabled(PermissionNameToPermissionsPolicyFeature(descriptor->name))) {
            AddConsoleError(String::Format("The permission '%s' is not allowed in the current context due to "
                                           "PermissionsPolicy",
    ...
    CachedPermissionStatus::From(GetDocument().domWindow())->RegisterClient(this, permission_descriptors_);
    mojo::PendingRemote<EmbeddedPermissionControlClient> client;
    embedded_permission_control_receiver_.Bind(client.InitWithNewPipeAndPassReceiver(), GetTaskRunner());
    GetPermissionService()->RegisterPageEmbeddedPermissionControl(mojo::Clone(permission_descriptors_), std::move(client));
    return true;
}

void HTMLPermissionElement::AttributeChanged(const AttributeModificationParams& params)
{
    if (params.name == html_names::kTypeAttr) {
        ...
        CHECK_LE(permission_descriptors_.size(), 2U) << "Unexpected permissions size " << permission_descriptors_.size();
    }
...
void HTMLPermissionElement::DidAddUserAgentShadowRoot(ShadowRoot& root)
{
    ...
    permission_text_span_->SetShadowPseudoId(shadow_element_names::kPseudoInternalPermissionTextSpan);
...
void HTMLPermissionElement::AdjustStyle(ComputedStyleBuilder& builder)
{
    ...
    auto device_pixel_ratio = GetDocument().GetFrame()->LocalFrameRoot().DevicePixelRatio();

    builder.SetMarginLeft(AdjustedBoundedLength(builder.MarginLeft(), /*lower_bound=*/kMinMargin * device_pixel_ratio,
        /*upper_bound=*/std::nullopt,
        /*should_multiply_by_content_size=*/false));
    builder.SetMarginRight(AdjustedBoundedLength(builder.MarginRight(), /*lower_bound=*/kMinMargin * device_pixel_ratio,
        /*upper_bound=*/std::nullopt,
        /*should_multiply_by_content_size=*/false));
    builder.SetMarginTop(AdjustedBoundedLength(builder.MarginTop(), /*lower_bound=*/kMinMargin * device_pixel_ratio,
        /*upper_bound=*/std::nullopt,
        /*should_multiply_by_content_size=*/false));
    builder.SetMarginBottom(AdjustedBoundedLength(builder.MarginBottom(), /*lower_bound=*/kMinMargin * device_pixel_ratio,
        /*upper_bound=*/std::nullopt,
        /*should_multiply_by_content_size=*/false));
    ...
    if (builder.GetFontDescription().Style() != kItalicSlopeValue && builder.GetFontDescription().Style() != kNormalSlopeValue) {
        if (!new_font_description) {
            new_font_description = builder.GetFontDescription();
        }
    ...
    if (builder.GetFontDescription().WordSpacing() > kMaximumWordSpacingToFontSizeRatio * builder.FontSize()) {
        builder.SetWordSpacing(builder.FontSize() * kMaximumWordSpacingToFontSizeRatio);
    } else if (builder.GetFontDescription().WordSpacing() < 0) {
        builder.SetWordSpacing(0);
    }

    if (builder.GetDisplayStyle().Display() != EDisplay::kNone && builder.GetDisplayStyle().Display() != EDisplay::kInlineBlock) {
        builder.SetDisplay(EDisplay::kInlineBlock);
    }

    if (builder.GetFontDescription().LetterSpacing() > kMaximumLetterSpacingToFontSizeRatio * builder.FontSize()) {
        builder.SetLetterSpacing(builder.FontSize() * kMaximumLetterSpacingToFontSizeRatio);
    } else if (builder.GetFontDescription().LetterSpacing() < kMinimumLetterSpacingToFontSizeRatio * builder.FontSize()) {
        builder.SetLetterSpacing(builder.FontSize() * kMinimumLetterSpacingToFontSizeRatio);
    }

    builder.SetMinHeight(AdjustedBoundedLength(builder.MinHeight(),
        /*lower_bound=*/builder.FontSize() * kMinLengthToFontSizeRatio,
        /*upper_bound=*/builder.FontSize() * kMaxLengthToFontSizeRatio,
        /*should_multiply_by_content_size=*/false));
    builder.SetMaxHeight(AdjustedBoundedLength(builder.MaxHeight(),
        /*lower_bound=*/std::nullopt,
        /*upper_bound=*/builder.FontSize() * kMaxLengthToFontSizeRatio,
        /*should_multiply_by_content_size=*/false));

    builder.SetMinWidth(AdjustedBoundedLength(builder.MinWidth(),
        /*lower_bound=*/kMinLengthToFontSizeRatio,
        /*upper_bound=*/kMaxLengthToFontSizeRatio,
        /*should_multiply_by_content_size=*/true));

    bool unlimited_width_allowed = IsBorderSufficientlyDistinctFromBackgroundColor(builder.CloneStyle());

    if (unlimited_width_allowed) {
        if (builder.PaddingRight().IsSpecified() && !builder.PaddingRight().IsZero() && builder.PaddingLeft() != builder.PaddingRight()) {
            AddConsoleError("The permission element does not support 'padding-right'. "
                            "'padding-right' is always set to be identical to 'padding-left'.");
        }
        builder.SetPaddingRight(builder.PaddingLeft());
    } else {
        builder.SetMaxWidth(AdjustedBoundedLength(builder.MaxWidth(),
        ...
        if (builder.Width().IsAuto() && builder.PaddingLeft().IsSpecified() && !builder.PaddingLeft().IsZero()) {
            if (builder.PaddingRight().IsSpecified() && !builder.PaddingRight().IsZero() && builder.PaddingLeft() != builder.PaddingRight()) {
                AddConsoleError("The permission element does not support 'padding-right'. "
                                "'padding-right' is always set to be identical to 'padding-left'.");
            }

            builder.SetPaddingLeft(AdjustedBoundedLength(builder.PaddingLeft(),
                /*lower_bound=*/std::nullopt,
                /*upper_bound=*/builder.FontSize() * kMaxHorizontalPaddingToFontSizeRatio,
                /*should_multiply_by_content_size=*/false));
            builder.SetPaddingRight(builder.PaddingLeft());
        } else {
    ...
    if (builder.Height().IsAuto() && builder.PaddingTop().IsSpecified() && !builder.PaddingTop().IsZero()) {
        if (builder.PaddingBottom().IsSpecified() && !builder.PaddingBottom().IsZero() && builder.PaddingTop() != builder.PaddingBottom()) {
            AddConsoleError("The permission element does not support 'padding-bottom'. "
                            "'padding-bottom' is always set to be identical to 'padding-top'.");
        }
        builder.SetPaddingTop(AdjustedBoundedLength(builder.PaddingTop(),
            /*lower_bound=*/std::nullopt,
...
void HTMLPermissionElement::DidRecalcStyle(const StyleRecalcChange change)
{
    ...
    EnableClickingAfterDelay(DisableReason::kInvalidStyle, kDefaultDisableTimeout);
    gfx::Rect intersection_rect = ComputeIntersectionRectWithViewport(GetDocument().GetPage());
    if (intersection_rect_.has_value() && intersection_rect_.value() != intersection_rect) {
        DisableClickingTemporarily(DisableReason::kIntersectionWithViewportChanged, kDefaultDisableTimeout);
    }
...
void HTMLPermissionElement::DefaultEventHandler(Event& event)
{
    if (event.type() == event_type_names::kDOMActivate) {
        ...
        if (event.IsFullyTrusted() || RuntimeEnabledFeatures::BypassPepcSecurityForTestingEnabled()) {
            ...
            // disallowing them.
            if (pending_request_created_ && base::TimeTicks::Now() - *pending_request_created_ < kDefaultDisableTimeout) {
                AddConsoleError("The permission element already has a request in progress.");
                return;
            }
            ...
            AddConsoleError("The permission element can only be activated by actual user "
                            "clicks.");
            base::UmaHistogramEnumeration("Blink.PermissionElement.UserInteractionDeniedReason", UserInteractionDeniedReason::kUntrustedEvent);
        }
        return;
    }
...
void HTMLPermissionElement::RequestPageEmbededPermissions()
{
    ...
    GetPermissionService()->RequestPageEmbeddedPermission(
        std::move(descriptor), WTF::BindOnce(&HTMLPermissionElement::OnEmbeddedPermissionsDecided, WrapWeakPersistent(this)));
}

void HTMLPermissionElement::RegisterPermissionObserver(const PermissionDescriptorPtr& descriptor, MojoPermissionStatus current_status)
{
    mojo::PendingRemote<PermissionObserver> observer;
    permission_observer_receivers_.Add(observer.InitWithNewPipeAndPassReceiver(), descriptor->name, GetTaskRunner());
    GetPermissionService()->AddPageEmbeddedPermissionObserver(descriptor.Clone(), current_status, std::move(observer));
}

void HTMLPermissionElement::OnPermissionStatusChange(MojoPermissionStatus status)
{
...
void HTMLPermissionElement::OnEmbeddedPermissionControlRegistered(bool allowed, const std::optional<Vector<MojoPermissionStatus>>& statuses)
{
    if (!allowed) {
        AddConsoleError(String::Format("The permission '%s' has not passed security checks or has surpassed "
    ...
    bool needs_permission_observer_registration = permission_observer_receivers_.empty();
...
void HTMLPermissionElement::OnEmbeddedPermissionsDecided(EmbeddedPermissionControlResult result)
{
    ...
    case EmbeddedPermissionControlResult::kNotSupported:
        AddConsoleError(String::Format("The permission request type '%s' is not supported and "
...
void HTMLPermissionElement::MaybeDispatchValidationChangeEvent()
{
    ...
    DispatchEvent(*Event::CreateCancelableBubble(event_type_names::kValidationstatuschange));
}
...
scoped_refptr<base::SingleThreadTaskRunner> HTMLPermissionElement::GetTaskRunner()
{
...
{
    if (permission_descriptors_.empty()) {
        AddConsoleError(String::Format("The permission element '%s' cannot be activated due to invalid type.", GetType().Utf8().c_str()));
        base::UmaHistogramEnumeration("Blink.PermissionElement.UserInteractionDeniedReason", UserInteractionDeniedReason::kInvalidType);
        return false;
    }

    if (!IsRegisteredInBrowserProcess()) {
        AddConsoleError(String::Format("The permission element '%s' cannot be activated because of security "
                                       "checks or because the page's quota has been exceeded.",
            GetType().Utf8().c_str()));
        base::UmaHistogramEnumeration("Blink.PermissionElement.UserInteractionDeniedReason", UserInteractionDeniedReason::kFailedOrHasNotBeenRegistered);
        return false;
    }
    ...
    while (!clicking_disabled_reasons_.empty()) {
        ...
        if (it->value >= now) {
            AddConsoleError(String::Format(
                "The permission element '%s' cannot be activated due to %s.", GetType().Utf8().c_str(), DisableReasonToString(it->key).Utf8().c_str()));
            if (it->key == DisableReason::kIntersectionVisibilityOccludedOrDistorted && occluder_node_id_ != kInvalidDOMNodeId) {
                if (Node* node = DOMNodeIds::NodeForId(occluder_node_id_)) {
                    AddConsoleError(String::Format("The permission element is occluded by node %s", node->ToString().Utf8().c_str()));
                }
            }
            base::UmaHistogramEnumeration("Blink.PermissionElement.UserInteractionDeniedReason", DisableReasonToUserInteractionDeniedReason(it->key));
            return false;
        }
...
void HTMLPermissionElement::DisableClickingTemporarily(DisableReason reason, const base::TimeDelta& duration)
{
    ...
    if (clicking_disabled_reasons_.Contains(reason) && clicking_disabled_reasons_.at(reason) > timeout_time) {
        return;
    }
    ...
    if (!disable_reason_expire_timer_.IsActive() || disable_reason_expire_timer_.NextFireInterval() < duration) {
...
void HTMLPermissionElement::EnableClickingAfterDelay(DisableReason reason, const base::TimeDelta& delay)
{
...
HTMLPermissionElement::ClickingEnabledState HTMLPermissionElement::GetClickingEnabledState() const
{
    if (permission_descriptors_.empty()) {
    ...
    if (disable_reason_expire_timer_.IsActive()) {
        return { false, DisableReasonToInvalidReasonString(disable_reason_expire_timer_.reason()) };
    }
...
void HTMLPermissionElement::RefreshDisableReasonsAndUpdateTimer()
{
    ...
    for (auto it = clicking_disabled_reasons_.begin(); it != clicking_disabled_reasons_.end(); ++it) {
        if (it->value == base::TimeTicks::Max()) {
    ...
    if (max_time_ticks != base::TimeTicks::Min()) {
        disable_reason_expire_timer_.StartOrRestartWithReason(reason, max_time_ticks - now);
    }
...
{
    if (base::ranges::any_of(permission_status_map_, [](const auto& status) { return status.value == MojoPermissionStatus::DENIED; })) {
        aggregated_permission_status_ = MojoPermissionStatus::DENIED;
    } else if (base::ranges::any_of(permission_status_map_, [](const auto& status) { return status.value == MojoPermissionStatus::ASK; })) {
...
void HTMLPermissionElement::UpdateText()
{
    ...
    int untranslated_message_id = permission_count == 1 ? GetUntranslatedMessageIDSinglePermission(permission_name, permission_granted, is_precise_location_)
                                                        : GetUntranslatedMessageIDMultiplePermissions(permission_granted);
    int translated_message_id = GetTranslatedMessageID(untranslated_message_id, language_string);
    CHECK(translated_message_id);
    permission_text_span_->setInnerText(GetLocale().QueryString(translated_message_id));
}

void HTMLPermissionElement::AddConsoleError(String error)
{
    AddConsoleMessage(mojom::blink::ConsoleMessageSource::kRendering, mojom::blink::ConsoleMessageLevel::kError, error);
}

void HTMLPermissionElement::AddConsoleWarning(String warning)
{
    AddConsoleMessage(mojom::blink::ConsoleMessageSource::kRendering, mojom::blink::ConsoleMessageLevel::kWarning, warning);
}

void HTMLPermissionElement::OnIntersectionChanged(const HeapVector<Member<IntersectionObserverEntry>>& entries)
{
    CHECK(!entries.empty());
    Member<IntersectionObserverEntry> latest_observation = entries.back();
    CHECK_EQ(this, latest_observation->target());
    IntersectionVisibility intersection_visibility = IntersectionVisibility::kFullyVisible;
    ...
    if (!latest_observation->isVisible()) {
        intersection_visibility = latest_observation->intersectionRatio() >= kIntersectionThreshold ? IntersectionVisibility::kOccludedOrDistorted
    ...
    case IntersectionVisibility::kFullyVisible: {
        std::optional<base::TimeDelta> interval = GetRecentlyAttachedTimeoutRemaining();
        DisableClickingTemporarily(DisableReason::kIntersectionRecentlyFullyVisible, interval ? interval.value() : kDefaultDisableTimeout);
        EnableClicking(DisableReason::kIntersectionVisibilityOccludedOrDistorted);
        EnableClicking(DisableReason::kIntersectionVisibilityOutOfViewPortOrClipped);
        break;
    }
    case IntersectionVisibility::kOccludedOrDistorted:
        occluder_node_id_ = latest_observation->GetGeometry().occluder_node_id();
        DisableClickingIndefinitely(DisableReason::kIntersectionVisibilityOccludedOrDistorted);
        break;
    case IntersectionVisibility::kOutOfViewportOrClipped:
        DisableClickingIndefinitely(DisableReason::kIntersectionVisibilityOutOfViewPortOrClipped);
        break;
    }

    // TODO(crbug.com/342330035): revisit it when we write spec for <permission>
    // element.
    GetTaskRunner()->PostTask(FROM_HERE, WTF::BindOnce(&HTMLPermissionElement::UpdateSnapshot, WrapWeakPersistent(this)));
}
...
bool HTMLPermissionElement::IsStyleValid()
{
    ...
    if (!GetComputedStyle()) {
        AddConsoleWarning(String::Format("Cannot compute style for the permission element '%s'", GetType().Utf8().c_str()));
        base::UmaHistogramEnumeration("Blink.PermissionElement.InvalidStyleReason", InvalidStyleReason::kNoComputedStyle);
        return false;
    }

    if (AreColorsNonOpaque(GetComputedStyle())) {
        AddConsoleWarning(String::Format("Color or background color of the permission element "
                                         "'%s' is non-opaque",
            GetType().Utf8().c_str()));
        base::UmaHistogramEnumeration("Blink.PermissionElement.InvalidStyleReason", InvalidStyleReason::kNonOpaqueColorOrBackgroundColor);
        return false;
    }

    if (ContrastBetweenColorAndBackgroundColor(GetComputedStyle()) < kMinimumAllowedContrast) {
        AddConsoleWarning(String::Format("Contrast between color and background color of the "
                                         "permission element '%s' is too low",
            GetType().Utf8().c_str()));
        base::UmaHistogramEnumeration("Blink.PermissionElement.InvalidStyleReason", InvalidStyleReason::kLowConstrastColorAndBackgroundColor);
        return false;
    }
    ...
    float font_size_dip = GetComputedStyle()->ComputedFontSize() / GetComputedStyle()->EffectiveZoom();

    float css_zoom_factor = GetComputedStyle()->EffectiveZoom() / GetDocument().GetFrame()->LocalFrameRoot().LayoutZoomFactor();
    ...
    float min_font_size_dip = FontSizeFunctions::FontSizeForKeyword(
        &GetDocument(), FontSizeFunctions::KeywordSize(CSSValueID::kSmall), GetComputedStyle()->GetFontDescription().IsMonospace());
    if (font_size_dip < min_font_size_dip / css_zoom_factor) {
        AddConsoleWarning(String::Format("Font size of the permission element '%s' is too small", GetType().Utf8().c_str()));
        base::UmaHistogramEnumeration("Blink.PermissionElement.InvalidStyleReason", InvalidStyleReason::kTooSmallFontSize);
        return false;
    }
    ...
    float max_font_size_dip = FontSizeFunctions::FontSizeForKeyword(
        &GetDocument(), FontSizeFunctions::KeywordSize(CSSValueID::kXxxLarge), GetComputedStyle()->GetFontDescription().IsMonospace());
    if (font_size_dip > max_font_size_dip / css_zoom_factor) {
        AddConsoleWarning(String::Format("Font size of the permission element '%s' is too large", GetType().Utf8().c_str()));
        base::UmaHistogramEnumeration("Blink.PermissionElement.InvalidStyleReason", InvalidStyleReason::kTooLargeFontSize);
        return false;
    }
...
Length HTMLPermissionElement::AdjustedBoundedLength(
    const Length& length, std::optional<float> lower_bound, std::optional<float> upper_bound, bool should_multiply_by_content_size)
{
    CHECK(lower_bound.has_value() || upper_bound.has_value());
    bool is_content_or_stretch = length.HasContentOrIntrinsic() || length.HasStretch();
    if (is_content_or_stretch && !length_console_error_sent_) {
        length_console_error_sent_ = true;
        AddConsoleWarning("content, intrinsic, or stretch sizes are not supported as values for "
                          "the min/max width and height of the permission element");
    }

    const Length& length_to_use = is_content_or_stretch || length.IsNone() ? Length::Auto() : length;
    ...
    if (length_to_use.IsAuto() && !should_multiply_by_content_size) {
        return Length(lower_bound.has_value() ? lower_bound.value() : upper_bound.value(), Length::Type::kFixed);
    }
    ...
    if (!should_multiply_by_content_size) {
        auto lower_bound_expr = lower_bound.has_value()
            ? std::optional(base::MakeRefCounted<blink::CalculationExpressionPixelsAndPercentNode>(PixelsAndPercent(lower_bound.value())))
            : std::nullopt;

        auto upper_bound_expr = upper_bound.has_value()
            ? std::optional(base::MakeRefCounted<blink::CalculationExpressionPixelsAndPercentNode>(PixelsAndPercent(upper_bound.value())))
            : std::nullopt;

        // expr = min|max|clamp(bound, length, [bound2])
        auto expr = BuildLengthBoundExpr(length_to_use, lower_bound_expr, upper_bound_expr);
        return Length(CalculationValue::CreateSimplified(std::move(expr), Length::ValueRange::kNonNegative));
    }

    // bound_expr = size * bound.
    auto lower_bound_expr = lower_bound.has_value() ? std::optional(BuildFitContentExpr(lower_bound.value())) : std::nullopt;
    auto upper_bound_expr = upper_bound.has_value() ? std::optional(BuildFitContentExpr(upper_bound.value())) : std::nullopt;
    ...
    if (!length_to_use.IsAuto()) {
        ...
        bound_expr = BuildLengthBoundExpr(length_to_use, lower_bound_expr, upper_bound_expr);
    } else {
        bound_expr = lower_bound_expr.has_value() ? std::move(lower_bound_expr.value()) : std::move(upper_bound_expr.value());
    }
    ...
    auto fit_content_expr = base::MakeRefCounted<CalculationExpressionSizingKeywordNode>(CalculationExpressionSizingKeywordNode::Keyword::kFitContent);

    // expr = calc-size(fit-content, bound_expr)
    auto expr = CalculationExpressionOperationNode::CreateSimplified(
        CalculationExpressionOperationNode::Children({ fit_content_expr, bound_expr }), CalculationOperator::kCalcSize);

    return Length(CalculationValue::CreateSimplified(std::move(expr), Length::ValueRange::kNonNegative));
}

void HTMLPermissionElement::DidFinishLifecycleUpdate(const LocalFrameView& local_frame_view)
{
    ...
    gfx::Rect intersection_rect = ComputeIntersectionRectWithViewport(local_frame_view.GetFrame().GetPage());
    if (intersection_rect_.has_value() && intersection_rect_.value() != intersection_rect) {
        DisableClickingTemporarily(DisableReason::kIntersectionWithViewportChanged, kDefaultDisableTimeout);
    }
    intersection_rect_ = intersection_rect;
...
gfx::Rect HTMLPermissionElement::ComputeIntersectionRectWithViewport(const Page* page)
{
    ...
    gfx::Rect viewport_in_root_frame = ToEnclosingRect(page->GetVisualViewport().VisibleRect());
...
std::optional<base::TimeDelta> HTMLPermissionElement::GetRecentlyAttachedTimeoutRemaining() const
{
    base::TimeTicks now = base::TimeTicks::Now();
    auto it = clicking_disabled_reasons_.find(DisableReason::kRecentlyAttachedToLayoutTree);
