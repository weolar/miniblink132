...
{
    DEFINE_STATIC_LOCAL(Persistent<PluginSet>, set, (MakeGarbageCollected<PluginSet>()));
    return *set;
}
...
bool IsFrameLazyLoadable(ExecutionContext* context, const KURL& url, bool is_loading_attr_lazy, bool should_lazy_load_children)
{
    ...
    if (!should_lazy_load_children ||
        ...
        context->GetSecurityOrigin()->CanAccess(SecurityOrigin::Create(url).get())) {
        return false;
    }
...
bool CheckAndRecordIfShouldLazilyLoadFrame(const Document& document, bool is_loading_attr_lazy)
{
...
void HTMLFrameOwnerElement::PluginDisposeSuspendScope::PerformDeferredPluginDispose()
{
...
HTMLFrameOwnerElement::HTMLFrameOwnerElement(const QualifiedName& tag_name, Document& document)
...
Node::InsertionNotificationRequest HTMLFrameOwnerElement::InsertedInto(ContainerNode& insertion_point)
{
    InsertionNotificationRequest result = HTMLElement::InsertedInto(insertion_point);
...
void HTMLFrameOwnerElement::RemovedFrom(ContainerNode& insertion_point)
{
    ...
    if (GetDocument().StatePreservingAtomicMoveInProgress()) {
        ...
        for (ContainerNode* node = &insertion_point; node; node = node->ParentOrShadowHostNode()) {
...
void HTMLFrameOwnerElement::SetContentFrame(Frame& frame)
{
    ...
    DCHECK(!lazy_load_frame_observer_ || !lazy_load_frame_observer_->IsLazyLoadPending());
...
void HTMLFrameOwnerElement::ClearContentFrame()
{
    ...
    RendererResourceCoordinator::Get()->OnBeforeContentFrameDetached(*content_frame_, *this);
...
void HTMLFrameOwnerElement::DisconnectContentFrame()
{
    ...
    bool have_to_check_if_parent_is_completed = ContentFrame()->IsRemoteFrame() && ContentFrame()->IsLoading();
...
void HTMLFrameOwnerElement::SetSandboxFlags(network::mojom::blink::WebSandboxFlags flags)
{
    ...
    if (ContentFrame()) {
        GetDocument().GetFrame()->GetLocalFrameHostRemote().DidChangeFramePolicy(ContentFrame()->GetFrameToken(), frame_policy_);
    }
}

bool HTMLFrameOwnerElement::IsKeyboardFocusable(UpdateBehavior update_behavior) const
{
...
void HTMLFrameOwnerElement::UpdateRequiredPolicy()
{
    ...
    DocumentPolicyFeatureState new_required_policy = frame ? DocumentPolicy::MergeFeatureState(ConstructRequiredPolicy(), /* self_required_policy */
    ...
    for (auto i = new_required_policy.begin(), last = new_required_policy.end(); i != last;) {
    ...
    if (ContentFrame()) {
        frame->GetLocalFrameHostRemote().DidChangeFramePolicy(ContentFrame()->GetFrameToken(), frame_policy_);
    }
}

network::mojom::blink::TrustTokenParamsPtr HTMLFrameOwnerElement::ConstructTrustTokenParams() const
{
    return nullptr;
}
...
void HTMLFrameOwnerElement::FrameOwnerPropertiesChanged()
{
    ...
    mojom::blink::FrameOwnerPropertiesPtr properties = mojom::blink::FrameOwnerProperties::New();
    properties->name = BrowsingContextContainerName().IsNull() ? WTF::g_empty_string : BrowsingContextContainerName(),
    ...
    GetDocument().GetFrame()->GetLocalFrameHostRemote().DidChangeFrameOwnerProperties(ContentFrame()->GetFrameToken(), std::move(properties));
}

void HTMLFrameOwnerElement::AddResourceTiming(mojom::blink::ResourceTimingInfoPtr info)
{
    ...
    DOMWindowPerformance::performance(*GetDocument().domWindow())->AddResourceTiming(std::move(info), localName());
...
void HTMLFrameOwnerElement::ReportFallbackResourceTimingIfNeeded()
{
    ...
    DOMWindowPerformance::performance(*GetDocument().domWindow())->AddResourceTiming(std::move(resource_timing_info), localName());
}
...
Document* HTMLFrameOwnerElement::getSVGDocument(ExceptionState& exception_state) const
{
...
void HTMLFrameOwnerElement::SetEmbeddedContentView(EmbeddedContentView* embedded_content_view)
{
    ...
    if (doc && doc->GetFrame()) {
        ...
        if (IsDisplayNone() != will_be_display_none) {
            doc->WillChangeFrameOwnerProperties(
                MarginWidth(), MarginHeight(), ScrollbarMode(), will_be_display_none, GetColorScheme(), GetPreferredColorScheme());
        }
    }
...
bool HTMLFrameOwnerElement::LazyLoadIfPossible(const KURL& url, const ResourceRequestHead& request, WebFrameLoadType frame_load_type)
{
    ...
    if (!IsFrameLazyLoadable(GetExecutionContext(), url, loading_lazy_set, should_lazy_load_children_)) {
        return false;
    }
    ...
    lazy_load_frame_observer_ = MakeGarbageCollected<LazyLoadFrameObserver>(*this, LazyLoadFrameObserver::LoadType::kSubsequent);

    if (CheckAndRecordIfShouldLazilyLoadFrame(GetDocument(), loading_lazy_set)) {
        lazy_load_frame_observer_->DeferLoadUntilNearViewport(request, frame_load_type);
        return true;
    }
    return false;
}
...
bool HTMLFrameOwnerElement::LoadOrRedirectSubframe(const KURL& url, const AtomicString& frame_name, bool replace_current_item)
{
    ...
    // does.
    if (should_lazy_load_children_ && EqualIgnoringASCIICase(FastGetAttribute(html_names::kLoadingAttr), "eager")) {
        should_lazy_load_children_ = false;
    }
    ...
    request.SetHasUserGesture(LocalFrame::HasTransientUserActivation(GetDocument().GetFrame()));

    network::mojom::blink::TrustTokenParamsPtr trust_token_params = ConstructTrustTokenParams();
    ...
    if (ContentFrame()) {
        ...
        frame_load_request.SetClientNavigationReason(ClientNavigationReason::kFrameNavigation);
    ...
    if (GetDocument().GetFrame()->GetPage()->SubframeCount() >= Page::MaxNumberOfFrames()) {
        return false;
    }

    LocalFrame* child_frame = GetDocument().GetFrame()->Client()->CreateFrame(frame_name, this);
    ...
    if (url != BlankURL() && !GetDocument().LoadEventFinished() && GetDocument().Loader()->LoadType() == WebFrameLoadType::kReloadBypassingCache) {
    ...
    if (!lazy_load_frame_observer_ && LazyLoadIfPossible(url, request, child_load_type)) {
        return true;
    }

    FrameLoadRequest frame_load_request(GetDocument().domWindow(), request);
    frame_load_request.SetIsContainerInitiated(true);
    frame_load_request.SetClientNavigationReason(ClientNavigationReason::kInitialFrameNavigation);
...
void HTMLFrameOwnerElement::ParseAttribute(const AttributeModificationParams& params)
{
    if (params.name == html_names::kLoadingAttr) {
        ...
        if (loading == LoadingAttributeValue::kEager) {
            UseCounter::Count(GetDocument(), WebFeature::kLazyLoadFrameLoadingAttributeEager);
        } else if (loading == LoadingAttributeValue::kLazy) {
            UseCounter::Count(GetDocument(), WebFeature::kLazyLoadFrameLoadingAttributeLazy);
        }
        ...
        // loading is disabled.
        if (loading == LoadingAttributeValue::kEager
            || (GetDocument().GetSettings() && !CheckAndRecordIfShouldLazilyLoadFrame(GetDocument(), loading == LoadingAttributeValue::kLazy))) {
            should_lazy_load_children_ = false;
            if (lazy_load_frame_observer_ && lazy_load_frame_observer_->IsLazyLoadPending()) {
                lazy_load_frame_observer_->LoadImmediately();
            }
        }
    } else {
...
void HTMLFrameOwnerElement::SetColorScheme(mojom::blink::ColorScheme color_scheme)
{
    Document* doc = contentDocument();
    if (doc && doc->GetFrame()) {
        doc->WillChangeFrameOwnerProperties(MarginWidth(), MarginHeight(), ScrollbarMode(), IsDisplayNone(), color_scheme, GetPreferredColorScheme());
    }
    FrameOwnerPropertiesChanged();
}

mojom::blink::PreferredColorScheme HTMLFrameOwnerElement::GetPreferredColorScheme() const
{
    return preferred_color_scheme_;
}

void HTMLFrameOwnerElement::SetPreferredColorScheme(mojom::blink::PreferredColorScheme preferred_color_scheme)
{
    ...
    Document* doc = contentDocument();
    if (doc && doc->GetFrame()) {
        doc->WillChangeFrameOwnerProperties(MarginWidth(), MarginHeight(), ScrollbarMode(), IsDisplayNone(), GetColorScheme(), preferred_color_scheme);
    }
    FrameOwnerPropertiesChanged();
}
...
ParsedPermissionsPolicy HTMLFrameOwnerElement::GetLegacyFramePolicies()
{
    ParsedPermissionsPolicy container_policy;
    {
        ...
        ParsedPermissionsPolicyDeclaration allowlist(mojom::blink::PermissionsPolicyFeature::kFullscreen);
    ...
    }
    {
        ...
        ParsedPermissionsPolicyDeclaration allowlist(mojom::blink::PermissionsPolicyFeature::kUnload, {}, std::nullopt,
...
void HTMLFrameOwnerElement::DidRecalcStyle(const StyleRecalcChange style_recalc_change)
{
    HTMLElement::DidRecalcStyle(style_recalc_change);
    SetPreferredColorScheme(GetDocument().GetStyleEngine().ResolveColorSchemeForEmbedding(GetComputedStyle()));
}
