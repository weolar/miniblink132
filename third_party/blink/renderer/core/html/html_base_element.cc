...
void HTMLBaseElement::ParseAttribute(const AttributeModificationParams& params)
{
    if (params.name == html_names::kHrefAttr || params.name == html_names::kTargetAttr)
        GetDocument().ProcessBaseElement();
    else
        HTMLElement::ParseAttribute(params);
}

Node::InsertionNotificationRequest HTMLBaseElement::InsertedInto(ContainerNode& insertion_point)
{
...
String HTMLBaseElement::href() const
{
    ...
    auto stripped_attribute_value = StripLeadingAndTrailingHTMLSpaces(attribute_value);

    KURL url = GetDocument().Encoding().IsValid() ? KURL(GetDocument().FallbackBaseURL(), stripped_attribute_value)
                                                  : KURL(GetDocument().FallbackBaseURL(), stripped_attribute_value, GetDocument().Encoding());
