...
HTMLMetaElement::HTMLMetaElement(Document& document, const CreateElementFlags flags)
    : HTMLElement(html_names::kMetaTag, document)
    , is_sync_parser_(flags.IsCreatedByParser() && !flags.IsAsyncCustomElements() && !document.IsInDocumentWrite())
{
}
...
void HTMLMetaElement::ParseViewportContentAttribute(
    const String& content, ViewportDescription& viewport_description, Document* document, bool viewport_meta_zero_values_quirk)
{
    ...
    for (unsigned i = 0; i < length; /* no increment here */) {
        ...
        ProcessViewportKeyValuePair(document, !has_invalid_separator, key_string, value_string, viewport_meta_zero_values_quirk, viewport_description);
    }
    if (has_invalid_separator && document) {
        ...
        document->AddConsoleMessage(
            MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kRendering, mojom::ConsoleMessageLevel::kWarning, message));
    }
}
...
float HTMLMetaElement::ParsePositiveNumber(Document* document, bool report_warnings, const String& key_string, const String& value_string, bool* ok)
{
    size_t parsed_length;
    float value = WTF::VisitCharacters(value_string, [&](auto chars) { return CharactersToFloat(chars, parsed_length); });
    if (!parsed_length) {
        if (report_warnings)
            ReportViewportWarning(document, kUnrecognizedViewportArgumentValueError, value_string, key_string);
        if (ok)
            *ok = false;
        return 0;
    }
    if (parsed_length < value_string.length() && report_warnings)
        ReportViewportWarning(document, kTruncatedViewportArgumentValueError, value_string, key_string);
    if (ok)
        *ok = true;
    return value;
}

Length HTMLMetaElement::ParseViewportValueAsLength(Document* document, bool report_warnings, const String& key_string, const String& value_string)
{
    ...
    float value = ParsePositiveNumber(document, report_warnings, key_string, value_string, &ok);
    ...
    if (document && document->GetPage()) {
        value = document->GetPage()->GetChromeClient().WindowToViewportScalar(document->GetFrame(), value);
    }
    return Length::Fixed(value);
}

float HTMLMetaElement::ParseViewportValueAsZoom(Document* document, bool report_warnings, const String& key_string, const String& value_string,
    bool& computed_value_matches_parsed_value, bool viewport_meta_zero_values_quirk)
{
    ...
    float value = ParsePositiveNumber(document, report_warnings, key_string, value_string);

    if (value < 0)
        return ViewportDescription::kValueAuto;

    if (value > 10.0 && report_warnings)
        ReportViewportWarning(document, kMaximumScaleTooLargeError, String(), String());
...
bool HTMLMetaElement::ParseViewportValueAsUserZoom(
    Document* document, bool report_warnings, const String& key_string, const String& value_string, bool& computed_value_matches_parsed_value)
{
...
float HTMLMetaElement::ParseViewportValueAsDPI(Document* document, bool report_warnings, const String& key_string, const String& value_string)
{
...
blink::mojom::ViewportFit HTMLMetaElement::ParseViewportFitValueAsEnum(bool& unknown_value, const String& value_string)
{
...
// static
std::optional<ui::mojom::blink::VirtualKeyboardMode> HTMLMetaElement::ParseVirtualKeyboardValueAsEnum(const String& value)
{
...
void HTMLMetaElement::ProcessViewportKeyValuePair(Document* document, bool report_warnings, const String& key_string, const String& value_string,
    bool viewport_meta_zero_values_quirk, ViewportDescription& description)
{
    if (key_string == "width") {
        const Length& width = ParseViewportValueAsLength(document, report_warnings, key_string, value_string);
        if (!width.IsAuto()) {
            description.min_width = Length::ExtendToZoom();
            description.max_width = width;
        }
    } else if (key_string == "height") {
        const Length& height = ParseViewportValueAsLength(document, report_warnings, key_string, value_string);
        if (!height.IsAuto()) {
        ...
        description.zoom
            = ParseViewportValueAsZoom(document, report_warnings, key_string, value_string, description.zoom_is_explicit, viewport_meta_zero_values_quirk);
    } else if (key_string == "minimum-scale") {
        description.min_zoom
            = ParseViewportValueAsZoom(document, report_warnings, key_string, value_string, description.min_zoom_is_explicit, viewport_meta_zero_values_quirk);
    } else if (key_string == "maximum-scale") {
        description.max_zoom
            = ParseViewportValueAsZoom(document, report_warnings, key_string, value_string, description.max_zoom_is_explicit, viewport_meta_zero_values_quirk);
    } else if (key_string == "user-scalable") {
        description.user_zoom = ParseViewportValueAsUserZoom(document, report_warnings, key_string, value_string, description.user_zoom_is_explicit);
    } else if (key_string == "target-densitydpi") {
        description.deprecated_target_density_dpi = ParseViewportValueAsDPI(document, report_warnings, key_string, value_string);
        if (report_warnings)
            ReportViewportWarning(document, kTargetDensityDpiUnsupported, String(), String());
        ...
        if (RuntimeEnabledFeatures::DisplayCutoutAPIEnabled()) {
            ...
            description.SetViewportFit(ParseViewportFitValueAsEnum(unknown_value, value_string));

            // If we got an unknown value then report a warning.
            if (unknown_value) {
                ReportViewportWarning(document, kViewportFitUnsupported, value_string, String());
            }
        }
    } else if (key_string == "shrink-to-fit") {
        // Ignore vendor-specific argument.
    } else if (key_string == "interactive-widget") {
        std::optional<ui::mojom::blink::VirtualKeyboardMode> resize_type = ParseVirtualKeyboardValueAsEnum(value_string);
        ...
        if (resize_type) {
            ...
            case ui::mojom::blink::VirtualKeyboardMode::kOverlaysContent: {
                UseCounter::Count(document, WebFeature::kInteractiveWidgetOverlaysContent);
            } break;
            case ui::mojom::blink::VirtualKeyboardMode::kResizesContent: {
                UseCounter::Count(document, WebFeature::kInteractiveWidgetResizesContent);
            } break;
            case ui::mojom::blink::VirtualKeyboardMode::kResizesVisual: {
                UseCounter::Count(document, WebFeature::kInteractiveWidgetResizesVisual);
            } break;
            ...
            description.virtual_keyboard_mode = ui::mojom::blink::VirtualKeyboardMode::kUnset;
            ReportViewportWarning(document, kUnrecognizedViewportArgumentValueError, value_string, key_string);
        }
    } else if (report_warnings) {
        ReportViewportWarning(document, kUnrecognizedViewportArgumentKeyError, key_string, String());
    }
}
...
static mojom::ConsoleMessageLevel ViewportErrorMessageLevel(ViewportErrorCode error_code)
{
    switch (error_code) {
...
void HTMLMetaElement::ReportViewportWarning(Document* document, ViewportErrorCode error_code, const String& replacement1, const String& replacement2)
{
    ...
    document->AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kRendering, ViewportErrorMessageLevel(error_code), message));
}

void HTMLMetaElement::GetViewportDescriptionFromContentAttribute(
    const String& content, ViewportDescription& description, Document* document, bool viewport_meta_zero_values_quirk)
{
    ParseViewportContentAttribute(content, description, document, viewport_meta_zero_values_quirk);
...
void HTMLMetaElement::ProcessViewportContentAttribute(const String& content, ViewportDescription::Type origin)
{
    ...
    GetViewportDescriptionFromContentAttribute(
        content, description_from_legacy_tag, &GetDocument(), GetDocument().GetSettings() && GetDocument().GetSettings()->GetViewportMetaZeroValuesQuirk());

    viewport_data.SetViewportDescription(description_from_legacy_tag);

    TRACE_EVENT_INSTANT(TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "ParseMetaViewport", "data", [&](perfetto::TracedValue context) {
...
{
    const AtomicString& content_value = FastGetAttribute(html_names::kContentAttr);
    if (content_value.IsNull())
        return;
    if (EqualIgnoringASCIICase(name_value, "theme-color") && GetDocument().GetFrame()) {
    ...
    } else if (RuntimeEnabledFeatures::AppTitleEnabled(GetExecutionContext()) && EqualIgnoringASCIICase(name_value, "app-title")) {
        GetDocument().UpdateAppTitle();
    }
}

void HTMLMetaElement::ParseAttribute(const AttributeModificationParams& params)
{
...
void HTMLMetaElement::ProcessHttpEquiv()
{
    ...
    if (content_value.IsNull())
        return;
    const AtomicString& http_equiv_value = FastGetAttribute(html_names::kHttpEquivAttr);
    if (http_equiv_value.empty())
        return;
    HttpEquiv::Process(GetDocument(), http_equiv_value, content_value, InDocumentHead(this), is_sync_parser_, this);
}
...
ContentClassificationOpenGraph GetContentClassification(const AtomicString& open_graph_type)
{
...
void HTMLMetaElement::ProcessContent()
{
    ...
    const AtomicString& property_value = FastGetAttribute(html_names::kPropertyAttr);
    const AtomicString& content_value = FastGetAttribute(html_names::kContentAttr);

    if (EqualIgnoringASCIICase(property_value, "og:type")) {
        UMA_HISTOGRAM_ENUMERATION("Content.Classification.OpenGraph", GetContentClassification(content_value));
    }
    ...
    if (EqualIgnoringASCIICase(name_value, "viewport")) {
        ProcessViewportContentAttribute(content_value, ViewportDescription::kViewportMeta);
    } else if (EqualIgnoringASCIICase(name_value, "referrer") && GetExecutionContext()) {
        UseCounter::Count(&GetDocument(), WebFeature::kHTMLMetaElementReferrerPolicy);
        if (!IsDescendantOf(GetDocument().head())) {
            UseCounter::Count(&GetDocument(), WebFeature::kHTMLMetaElementReferrerPolicyOutsideHead);
        }
        network::mojom::ReferrerPolicy old_referrer_policy = GetExecutionContext()->GetReferrerPolicy();
        GetExecutionContext()->ParseAndSetReferrerPolicy(content_value, kPolicySourceMetaTag);
        network::mojom::ReferrerPolicy new_referrer_policy = GetExecutionContext()->GetReferrerPolicy();
        if (old_referrer_policy != new_referrer_policy) {
            if (auto* document_rules = DocumentSpeculationRules::FromIfExists(GetDocument())) {
                document_rules->DocumentReferrerPolicyChanged();
            }
        }
    } else if (EqualIgnoringASCIICase(name_value, "handheldfriendly") && EqualIgnoringASCIICase(content_value, "true")) {
        ProcessViewportContentAttribute("width=device-width", ViewportDescription::kHandheldFriendlyMeta);
    } else if (EqualIgnoringASCIICase(name_value, "mobileoptimized")) {
        ProcessViewportContentAttribute("width=device-width, initial-scale=1", ViewportDescription::kMobileOptimizedMeta);
        ...
        if (GetDocument().IsInOutermostMainFrame()) {
            UseCounter::Count(&GetDocument(), WebFeature::kHTMLMetaElementMonetization);
        }
    } else if (RuntimeEnabledFeatures::AppTitleEnabled(GetExecutionContext()) && EqualIgnoringASCIICase(name_value, "app-title")) {
...
WTF::TextEncoding HTMLMetaElement::ComputeEncoding() const
{
    ...
    for (const Attribute& attr : Attributes())
        attribute_list.push_back(std::make_pair(attr.GetName().LocalName(), attr.Value().GetString()));
...
// static
void HTMLMetaElement::ProcessMetaCH(Document& document, const AtomicString& content, network::MetaCHType type, bool is_doc_preloader, bool is_sync_parser)
{
    ...
    case network::MetaCHType::HttpEquivAcceptCH:
        UseCounter::Count(document, WebFeature::kClientHintsMetaHTTPEquivAcceptCH);
        break;
    ...
    UpdateWindowPermissionsPolicyWithDelegationSupportForClientHints(
        frame->GetClientHintsPreferences(), document.domWindow(), content, document.Url(), &hints_context, type, is_doc_preloader, is_sync_parser);
}
