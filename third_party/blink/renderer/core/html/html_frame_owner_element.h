...
public:
    ...
    // Node overrides:
    Node::InsertionNotificationRequest InsertedInto(ContainerNode& insertion_point) override;
    void RemovedFrom(ContainerNode& insertion_point) override;
    // Element overrides:
    void DidRecalcStyle(const StyleRecalcChange) override;

    // FrameOwner overrides:
    Frame* ContentFrame() const final
    {
        return content_frame_.Get();
    }
    ...
    const FramePolicy& GetFramePolicy() const final
    {
    ...
    void UpdateContainerPolicyForTests()
    {
...
protected:
    ...
    bool LoadOrRedirectSubframe(const KURL&, const AtomicString& frame_name, bool replace_current_item);
    bool IsKeyboardFocusable(UpdateBehavior update_behavior = UpdateBehavior::kStyleAndLayout) const override;
    ...
    virtual scoped_refptr<const SecurityOrigin> GetOriginForPermissionsPolicy() const
    {
    ...
    virtual network::mojom::blink::TrustTokenParamsPtr ConstructTrustTokenParams() const;
    void ReportFallbackResourceTimingIfNeeded();

protected:
    bool is_swapping_frames() const
    {
        return is_swapping_frames_;
    }
...
private:
    ...
    bool IsFrameOwnerElement() const final
    {
        return true;
    }
    ...
    bool LazyLoadIfPossible(const KURL&, const ResourceRequestHead&, WebFrameLoadType frame_load_type);
...
template <> struct DowncastTraits<HTMLFrameOwnerElement> {
