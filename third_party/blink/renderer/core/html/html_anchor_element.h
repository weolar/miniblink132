...
class CORE_EXPORT HTMLAnchorElementBase : public HTMLElement, public DOMURLUtils {
public:
    ...
    void InvalidateCachedVisitedLinkHash()
    {
...
private:
    ...
    FocusableState IsFocusableState(UpdateBehavior update_behavior) const override;
    ...
    void NavigateToHyperlink(ResourceRequest, NavigationPolicy, bool is_trusted, base::TimeTicks platform_time_stamp, KURL);
...
template <> struct DowncastTraits<HTMLAnchorElementBase> {
    static bool AllowFrom(const Element& element)
    {
        return element.HasTagName(html_names::kATag) || element.HasTagName(html_names::kAreaTag);
    }

    static bool AllowFrom(const Node& node)
    {
        return node.IsHTMLElement() && IsA<HTMLAnchorElementBase>(UnsafeTo<HTMLElement>(node));
    }
};
...
inline LinkHash HTMLAnchorElementBase::VisitedLinkHash() const
{
    if (!cached_visited_link_hash_) {
        cached_visited_link_hash_ = blink::VisitedLinkHash(GetDocument().BaseURL(), FastGetAttribute(html_names::kHrefAttr));
    }
    return cached_visited_link_hash_;
}

inline LinkHash HTMLAnchorElementBase::PartitionedVisitedLinkFingerprint() const
{
    if (!cached_visited_link_hash_) {
