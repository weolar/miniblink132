...
class CORE_EXPORT CanvasRenderingContextHost : public CanvasResourceHost, public CanvasImageSource, public GarbageCollectedMixin {
public:
    enum class HostType {
        kNone,
        kCanvasHost,
    ...
    void DidDraw()
    {
        DidDraw(SkIRect::MakeWH(width(), height()));
    }

    virtual void PreFinalizeFrame() = 0;
    virtual void PostFinalizeFrame(FlushReason) = 0;
    virtual bool PushFrame(scoped_refptr<CanvasResource>&& frame, const SkIRect& damage_rect) = 0;
    ...
    virtual void Commit(scoped_refptr<CanvasResource>&& canvas_resource, const SkIRect& damage_rect);
    ...
    CanvasResourceProvider* GetOrCreateCanvasResourceProviderImpl(RasterModeHint hint) final;
...
protected:
    ...
    scoped_refptr<StaticBitmapImage> CreateTransparentImage(const gfx::Size&) const;
    ...
    // bits.
    IdentifiableToken IdentifiabilityInputDigest(const CanvasRenderingContext* const context) const;
