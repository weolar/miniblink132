...
const ComputedStyle* CreateDefaultFontStyle(const Document& document)
{
    ...
    default_font_description.SetFamily(FontFamily(default_font_family, FontFamily::InferredTypeFor(default_font_family)));
    default_font_description.SetSpecifiedSize(defaultFontSize);
    default_font_description.SetComputedSize(defaultFontSize);
    ComputedStyleBuilder builder
        = document.IsActive() ? document.GetStyleResolver().CreateComputedStyleBuilder() : ComputedStyleBuilder(*ComputedStyle::GetInitialStyleSingleton());
...
{
    return MemoryPressureListenerRegistry::IsLowEndDeviceOrPartialLowEndModeEnabledIncludingCanvasFontCache() ? CanvasFontCacheMaxFontsLowEnd
...
unsigned CanvasFontCache::HardMaxFonts()
{
    return document_->hidden()
        ...
        : (MemoryPressureListenerRegistry::IsLowEndDeviceOrPartialLowEndModeEnabledIncludingCanvasFontCache() ? CanvasFontCacheHardMaxFontsLowEnd
                                                                                                              : CanvasFontCacheHardMaxFonts);
}

bool CanvasFontCache::GetFontUsingDefaultStyle(HTMLCanvasElement& element, const String& font_string, Font& resolved_font)
{
    ...
    auto add_result = fonts_resolved_using_default_style_.insert(
        font_string, MakeGarbageCollected<FontWrapper>(document_->GetStyleEngine().ComputeFont(element, *default_font_style_, *parsed_style)));
    resolved_font = add_result.stored_value->value->font;
    return true;
}

MutableCSSPropertyValueSet* CanvasFontCache::ParseFont(const String& font_string)
{
    ...
    if (i != fetched_fonts_.end()) {
        ...
        parsed_style = CSSParser::ParseFont(font_string, document_->GetExecutionContext());
        if (!parsed_style)
            return nullptr;
