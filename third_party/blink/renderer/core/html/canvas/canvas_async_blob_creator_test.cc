...
public:
    MockCanvasAsyncBlobCreator(
        scoped_refptr<StaticBitmapImage> image, ImageEncodingMimeType mime_type, Document* document, bool fail_encoder_initialization = false)
        : CanvasAsyncBlobCreator(image, CanvasAsyncBlobCreator::GetImageEncodeOptionsForMimeType(mime_type), kHTMLCanvasToBlobCallback, nullptr,
            base::TimeTicks(), document->GetExecutionContext(), 0, nullptr)
    {
...
protected:
    ...
    void PostDelayedTaskToCurrentThread(const base::Location&, base::OnceClosure, double delay_ms) override;
};
...
void MockCanvasAsyncBlobCreator::PostDelayedTaskToCurrentThread(const base::Location& location, base::OnceClosure task, double delay_ms)
{
    // override delay to 0.
    CanvasAsyncBlobCreator::PostDelayedTaskToCurrentThread(location, std::move(task),
        /*delay_ms=*/0);
}
...
public:
    MockCanvasAsyncBlobCreatorWithoutStart(scoped_refptr<StaticBitmapImage> image, Document* document)
...
public:
    MockCanvasAsyncBlobCreatorWithoutComplete(scoped_refptr<StaticBitmapImage> image, Document* document, bool fail_encoder_initialization = false)
        : MockCanvasAsyncBlobCreator(image, kMimeTypePng, document, fail_encoder_initialization)
    {
    }
...
protected:
    ...
    {
        PostDelayedTaskToCurrentThread(FROM_HERE,
            WTF::BindOnce(&MockCanvasAsyncBlobCreatorWithoutComplete::InitiateEncoding, WrapPersistent(this), quality, base::TimeTicks::Max()),
            /*delay_ms=*/0);
    }
...
{
    async_blob_creator_ = MakeGarbageCollected<MockCanvasAsyncBlobCreatorWithoutStart>(CreateTransparentImage(20, 20), &GetDocument());
}

void CanvasAsyncBlobCreatorTest::PrepareMockCanvasAsyncBlobCreatorWithoutComplete()
{
    async_blob_creator_ = MakeGarbageCollected<MockCanvasAsyncBlobCreatorWithoutComplete>(CreateTransparentImage(20, 20), &GetDocument());
}
...
void CanvasAsyncBlobCreatorTest::PrepareMockCanvasAsyncBlobCreatorFail()
{
    ...
    // completion.
    async_blob_creator_ = MakeGarbageCollected<MockCanvasAsyncBlobCreatorWithoutComplete>(CreateTransparentImage(20, 20), &GetDocument(), true);
}
...
TEST_F(CanvasAsyncBlobCreatorTest, IdleTaskNotStartedWhenStartTimeoutEventHappens)
{
    ...
    EXPECT_EQ(IdleTaskStatus::kIdleTaskSwitchedToImmediateTask, AsyncBlobCreator()->GetIdleTaskStatus());
}
...
TEST_F(CanvasAsyncBlobCreatorTest, IdleTaskFailedWhenStartTimeoutEventHappens)
{
    ...
    EXPECT_EQ(IdleTaskStatus::kIdleTaskFailed, AsyncBlobCreator()->GetIdleTaskStatus());
}
