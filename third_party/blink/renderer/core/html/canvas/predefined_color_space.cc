...
bool ValidateAndConvertColorSpace(const V8PredefinedColorSpace& v8_color_space, PredefinedColorSpace& color_space, ExceptionState& exception_state)
{
    ...
    if (needs_hdr && !RuntimeEnabledFeatures::CanvasHDREnabled()) {
        exception_state.ThrowTypeError("The provided value '" + v8_color_space.AsString() + "' is not a valid enum value of the type PredefinedColorSpace.");
        return false;
    }
    return true;
}

V8PredefinedColorSpace PredefinedColorSpaceToV8(PredefinedColorSpace color_space)
{
    switch (color_space) {
...
void ParseCanvasHighDynamicRangeOptions(const CanvasHighDynamicRangeOptions* options, gfx::HDRMetadata& hdr_metadata)
{
