...
public:
    ...
    {
        return key_.get<IsOffscreenField>();
    }
    bool IsAccelerated() const
    {
        return key_.get<IsAcceleratedField>();
    }
...
RenderingContextDescriptionCodec::RenderingContextDescriptionCodec(const CanvasRenderingContext* context)
{
    ...
    key_.set<IsAcceleratedField>(context->Host()->GetRasterMode() == blink::RasterMode::kGPU);
    key_.set<RenderingAPIField>(static_cast<uint32_t>(context->GetRenderingAPI()));
...
RenderingContextDescriptionCodec::RenderingContextDescriptionCodec(const uint32_t& key)
    : key_(key)
    , is_valid_(true)
{
}

CanvasRenderingContext::CanvasRenderingAPI RenderingContextDescriptionCodec::GetRenderingAPI() const
{
    return static_cast<CanvasRenderingContext::CanvasRenderingAPI>(key_.get<RenderingAPIField>());
}
...
const char* RenderingContextDescriptionCodec::GetRenderingAPIName() const
{
    ...
    case CanvasRenderingContext::CanvasRenderingAPI::k2D:
        return IsAccelerated() ? kRenderingAPIName_2D_Accelerated : kRenderingAPIName_2D_Unaccelerated;
...
void CanvasPerformanceMonitor::CurrentTaskDrawsToContext(CanvasRenderingContext* context)
{
    if (!is_render_task_) {
...
void CanvasPerformanceMonitor::RecordMetrics(TimeTicks start_time, TimeTicks end_time)
{
    ...
    size_t blink_gc_alloc_kb = ProcessHeap::TotalAllocatedObjectSize() / kKiloByte;

    while (!rendering_context_descriptions_.empty()) {
        RenderingContextDescriptionCodec desc(rendering_context_descriptions_.TakeAny());
        ...
        // info.
        WTF::String histogram_name_prefix = WTF::String("Blink") + desc.GetHostTypeName();
        WTF::String histogram_name_radical = WTF::String(desc.GetRenderingAPIName());

        // Render task duration metric for all render tasks.
        {
            WTF::String histogram_name = histogram_name_prefix + kMeasurementName_RenderTaskDuration + histogram_name_radical + kFilterName_All;
            base::UmaHistogramMicrosecondsTimes(histogram_name.Latin1(), elapsed_time);
        }

        // Render task duration metric for rAF callbacks only.
        if (call_type_ == CallType::kAnimation) {
            WTF::String histogram_name = histogram_name_prefix + kMeasurementName_RenderTaskDuration + histogram_name_radical + kFilterName_Animation;
            base::UmaHistogramMicrosecondsTimes(histogram_name.Latin1(), elapsed_time);
        }

        // Filtered histograms that apply to 2D canvases
        if (desc.GetRenderingAPI() == CanvasRenderingContext::CanvasRenderingAPI::k2D) {
            if (draw_types_ & static_cast<uint32_t>(DrawType::kPath)) {
                WTF::String histogram_name = histogram_name_prefix + kMeasurementName_RenderTaskDuration + histogram_name_radical + kFilterName_Path;
                base::UmaHistogramMicrosecondsTimes(histogram_name.Latin1(), elapsed_time);
            }
            if (draw_types_ & static_cast<uint32_t>(DrawType::kImage)) {
                WTF::String histogram_name = histogram_name_prefix + kMeasurementName_RenderTaskDuration + histogram_name_radical + kFilterName_Image;
                base::UmaHistogramMicrosecondsTimes(histogram_name.Latin1(), elapsed_time);
            }
            if (draw_types_ & static_cast<uint32_t>(DrawType::kImageData)) {
                WTF::String histogram_name = histogram_name_prefix + kMeasurementName_RenderTaskDuration + histogram_name_radical + kFilterName_ImageData;
                base::UmaHistogramMicrosecondsTimes(histogram_name.Latin1(), elapsed_time);
            }
            if (draw_types_ & static_cast<uint32_t>(DrawType::kText)) {
                WTF::String histogram_name = histogram_name_prefix + kMeasurementName_RenderTaskDuration + histogram_name_radical + kFilterName_Text;
                base::UmaHistogramMicrosecondsTimes(histogram_name.Latin1(), elapsed_time);
            }
            if (draw_types_ & static_cast<uint32_t>(DrawType::kRectangle)) {
                WTF::String histogram_name = histogram_name_prefix + kMeasurementName_RenderTaskDuration + histogram_name_radical + kFilterName_Rectangle;
                base::UmaHistogramMicrosecondsTimes(histogram_name.Latin1(), elapsed_time);
            }
        } else if (desc.GetRenderingAPI() == CanvasRenderingContext::CanvasRenderingAPI::kWebgl
            || desc.GetRenderingAPI() == CanvasRenderingContext::CanvasRenderingAPI::kWebgl2) {
            // Filtered histograms that apply to WebGL canvases
            if (draw_types_ & static_cast<uint32_t>(DrawType::kDrawArrays)) {
                WTF::String histogram_name = histogram_name_prefix + kMeasurementName_RenderTaskDuration + histogram_name_radical + kFilterName_DrawArrays;
                base::UmaHistogramMicrosecondsTimes(histogram_name.Latin1(), elapsed_time);
            }
            if (draw_types_ & static_cast<uint32_t>(DrawType::kDrawElements)) {
                WTF::String histogram_name = histogram_name_prefix + kMeasurementName_RenderTaskDuration + histogram_name_radical + kFilterName_DrawElements;
                base::UmaHistogramMicrosecondsTimes(histogram_name.Latin1(), elapsed_time);
            }
        }
        ...
        {
            WTF::String histogram_name = histogram_name_prefix + kMeasurementName_PartitionAlloc + histogram_name_radical;
            base::UmaHistogramMemoryKB(histogram_name.Latin1(), static_cast<int>(partition_alloc_kb));
        }

        // Blink garbage collected heap size metric
        {
            WTF::String histogram_name = histogram_name_prefix + kMeasurementName_BlinkGC + histogram_name_radical;
            base::UmaHistogramMemoryKB(histogram_name.Latin1(), static_cast<int>(blink_gc_alloc_kb));
        }
    }
}

void CanvasPerformanceMonitor::DidProcessTask(TimeTicks start_time, TimeTicks end_time)
{
