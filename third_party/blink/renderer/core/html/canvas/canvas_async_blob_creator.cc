...
constexpr base::TimeDelta kCreateBlobSlackBeforeDeadline = base::Milliseconds(1);
constexpr base::TimeDelta kEncodeRowSlackBeforeDeadline = base::Microseconds(100);
...
bool IsEncodeRowDeadlineNearOrPassed(base::TimeTicks deadline, size_t image_width)
{
    ...
    int row_encode_time_us = 1000 * (kIdleTaskCompleteTimeoutDelayMs / 4000.0) * (image_width / 4000.0);
    base::TimeDelta row_encode_time_delta = base::Microseconds(row_encode_time_us);
    return base::TimeTicks::Now() >= deadline - row_encode_time_delta - kEncodeRowSlackBeforeDeadline;
}

void RecordIdleTaskStatusHistogram(CanvasAsyncBlobCreator::IdleTaskStatus status)
{
    UMA_HISTOGRAM_ENUMERATION("Blink.Canvas.ToBlob.IdleTaskStatus", status);
}

void RecordInitiateEncodingTimeHistogram(ImageEncodingMimeType mime_type, base::TimeDelta elapsed_time)
{
    if (mime_type == kMimeTypePng) {
        UmaHistogramMicrosecondsTimes("Blink.Canvas.ToBlob.InitialEncodingDelay.PNG", elapsed_time);
    } else if (mime_type == kMimeTypeJpeg) {
        UmaHistogramMicrosecondsTimes("Blink.Canvas.ToBlob.InitialEncodingDelay.JPEG", elapsed_time);
    }
}

void RecordCompleteEncodingTimeHistogram(ImageEncodingMimeType mime_type, base::TimeDelta elapsed_time)
{
    if (mime_type == kMimeTypePng) {
        UmaHistogramMicrosecondsTimes("Blink.Canvas.ToBlob.TotalEncodingDelay.PNG", elapsed_time);
    } else if (mime_type == kMimeTypeJpeg) {
        UmaHistogramMicrosecondsTimes("Blink.Canvas.ToBlob.TotalEncodingDelay.JPEG", elapsed_time);
    }
}

void RecordScaledDurationHistogram(ImageEncodingMimeType mime_type, base::TimeDelta elapsed_time, float width, float height)
{
    ...
    if (mime_type == kMimeTypePng) {
        UMA_HISTOGRAM_COUNTS_100000("Blink.Canvas.ToBlob.ScaledDuration.PNG", scaled_time_int);
    } else if (mime_type == kMimeTypeJpeg) {
        UMA_HISTOGRAM_COUNTS_100000("Blink.Canvas.ToBlob.ScaledDuration.JPEG", scaled_time_int);
    } else if (mime_type == kMimeTypeWebp) {
        UMA_HISTOGRAM_COUNTS_100000("Blink.Canvas.ToBlob.ScaledDuration.WEBP", scaled_time_int);
    }
}

} // anonymous namespace

CanvasAsyncBlobCreator::CanvasAsyncBlobCreator(scoped_refptr<StaticBitmapImage> image, const ImageEncodeOptions* options, ToBlobFunctionType function_type,
    base::TimeTicks start_time, ExecutionContext* context, const IdentifiableToken& input_digest, ScriptPromiseResolver<Blob>* resolver)
    : CanvasAsyncBlobCreator(image, options, function_type, nullptr, start_time, context, input_digest, resolver)
{
}

CanvasAsyncBlobCreator::CanvasAsyncBlobCreator(scoped_refptr<StaticBitmapImage> image, const ImageEncodeOptions* options, ToBlobFunctionType function_type,
    V8BlobCallback* callback, base::TimeTicks start_time, ExecutionContext* context, const IdentifiableToken& input_digest,
    ...
    , script_promise_resolver_(resolver)
{
    ...
    mime_type_ = ImageEncoderUtils::ToEncodingMimeType(options->type(), ImageEncoderUtils::kEncodeReasonConvertToBlobPromise);
    ...
    if (image_) {
        ...
        if (skia_image_->peekPixels(&src_data_)) {
            ...
            if (std::max(src_data_.width(), src_data_.height()) > max_dimension) {
                ...
                info = info.makeWH(std::min(info.width(), max_dimension), std::min(info.height(), max_dimension));
    ...
    if (context->IsWindow()) {
        parent_frame_task_runner_ = context->GetTaskRunner(TaskType::kCanvasBlobSerialization);
    }
}
...
{
    TRACE_EVENT_INSTANT(TRACE_DISABLED_BY_DEFAULT("identifiability.high_entropy_api"), "CanvasReadback", perfetto::TerminatingFlow::FromPointer(this));
...
ImageEncodeOptions* CanvasAsyncBlobCreator::GetImageEncodeOptionsForMimeType(ImageEncodingMimeType mime_type)
{
...
bool CanvasAsyncBlobCreator::EncodeImage(
    std::unique_ptr<ImageDataBuffer> buffer, ImageEncodingMimeType mime_type, const double& quality, Vector<unsigned char>* encoded_image)
{
    CHECK(encoded_image);
    if (!buffer) {
        return false;
...
{
    TRACE_EVENT_INSTANT(TRACE_DISABLED_BY_DEFAULT("identifiability.high_entropy_api"), "CanvasReadback", perfetto::Flow::FromPointer(this));

    if (!static_bitmap_image_loaded_) {
        context_->GetTaskRunner(TaskType::kCanvasBlobSerialization)
            ->PostTask(FROM_HERE, WTF::BindOnce(&CanvasAsyncBlobCreator::CreateNullAndReturnResult, WrapPersistent(this)));
        return;
    }
    ...
    // is not busy.
    bool use_idle_encoding = WTF::IsMainThread() && (mime_type_ != kMimeTypeWebp)
        && (enforce_idle_encoding_for_test_ || !RuntimeEnabledFeatures::NoIdleEncodingForWebTestsEnabled());
    ...
    if (!use_idle_encoding) {
        if (!IsMainThread()) {
            ...
            if (!EncodeImage(ImageDataBuffer::Create(src_data_), mime_type_, quality, &encoded_image)) {
                context_->GetTaskRunner(TaskType::kCanvasBlobSerialization)
                    ->PostTask(FROM_HERE, WTF::BindOnce(&CanvasAsyncBlobCreator::CreateNullAndReturnResult, WrapPersistent(this)));
            ...
            context_->GetTaskRunner(TaskType::kCanvasBlobSerialization)
                ->PostTask(FROM_HERE, WTF::BindOnce(&CanvasAsyncBlobCreator::CreateBlobAndReturnResult, WrapPersistent(this), std::move(encoded_image)));

        } else {
            // Off-thread case, see (2a) in function comment.

            worker_pool::PostTask(FROM_HERE,
                CrossThreadBindOnce(&CanvasAsyncBlobCreator::EncodeImageOnEncoderThread, MakeCrossThreadHandle(this), parent_frame_task_runner_, skia_image_,
                    ImageDataBuffer::Create(src_data_), mime_type_, quality));
        }
    } else {
        ...
        PostDelayedTaskToCurrentThread(
            FROM_HERE, WTF::BindOnce(&CanvasAsyncBlobCreator::IdleTaskStartTimeoutEvent, WrapPersistent(this), quality), kIdleTaskStartTimeoutDelayMs);
    }
}
...
void CanvasAsyncBlobCreator::ScheduleInitiateEncoding(double quality)
{
    ...
    ThreadScheduler::Current()->PostIdleTask(FROM_HERE, WTF::BindOnce(&CanvasAsyncBlobCreator::InitiateEncoding, WrapPersistent(this), quality));
}

void CanvasAsyncBlobCreator::InitiateEncoding(double quality, base::TimeTicks deadline)
{
    if (idle_task_status_ == kIdleTaskSwitchedToImmediateTask) {
        return;
    }
    RecordInitiateEncodingTimeHistogram(mime_type_, base::TimeTicks::Now() - schedule_idle_task_start_time_);
...
void CanvasAsyncBlobCreator::IdleEncodeRows(base::TimeTicks deadline)
{
    ...
    for (int y = num_rows_completed_; y < src_data_.height(); ++y) {
        if (IsEncodeRowDeadlineNearOrPassed(deadline, src_data_.width())) {
            ...
            ThreadScheduler::Current()->PostIdleTask(FROM_HERE, WTF::BindOnce(&CanvasAsyncBlobCreator::IdleEncodeRows, WrapPersistent(this)));
            return;
        }
    ...
    base::TimeDelta elapsed_time = base::TimeTicks::Now() - schedule_idle_task_start_time_;
    RecordCompleteEncodingTimeHistogram(mime_type_, elapsed_time);
    if (IsCreateBlobDeadlineNearOrPassed(deadline)) {
        context_->GetTaskRunner(TaskType::kCanvasBlobSerialization)
            ->PostTask(FROM_HERE,
                WTF::BindOnce(
                    &CanvasAsyncBlobCreator::CreateBlobAndReturnResult, WrapPersistent(this), std::exchange(encoded_image_, Vector<unsigned char>())));
    } else {
        CreateBlobAndReturnResult(std::exchange(encoded_image_, Vector<unsigned char>()));
    }
}
...
void CanvasAsyncBlobCreator::CreateBlobAndReturnResult(Vector<unsigned char> encoded_image)
{
    RecordIdleTaskStatusHistogram(idle_task_status_);

    Blob* result_blob = Blob::Create(base::span<const uint8_t>(encoded_image.data(), encoded_image.size()), ImageEncodingMimeTypeName(mime_type_));
    if (function_type_ == kHTMLCanvasToBlobCallback) {
        context_->GetTaskRunner(TaskType::kCanvasBlobSerialization)
            ->PostTask(
                FROM_HERE, WTF::BindOnce(&V8BlobCallback::InvokeAndReportException, WrapPersistent(callback_.Get()), nullptr, WrapPersistent(result_blob)));
    } else {
        context_->GetTaskRunner(TaskType::kCanvasBlobSerialization)
            ->PostTask(FROM_HERE,
                WTF::BindOnce(&ScriptPromiseResolver<Blob>::Resolve<Blob*>, WrapPersistent(script_promise_resolver_.Get()), WrapPersistent(result_blob)));
    }

    RecordScaledDurationHistogram(mime_type_, base::TimeTicks::Now() - start_time_, image_->width(), image_->height());
    ...
    if (IdentifiabilityStudySettings::Get()->ShouldSampleType(blink::IdentifiableSurface::Type::kCanvasReadback)) {
        ...
        context_->GetTaskRunner(TaskType::kCanvasBlobSerialization)
            ->PostTask(FROM_HERE, WTF::BindOnce(&CanvasAsyncBlobCreator::RecordIdentifiabilityMetric, WrapPersistent(this)));
    } else {
...
{
    std::unique_ptr<ImageDataBuffer> data_buffer = ImageDataBuffer::Create(image_);

    if (data_buffer) {
        blink::IdentifiabilityMetricBuilder(context_->UkmSourceID())
            .Add(blink::IdentifiableSurface::FromTypeAndToken(blink::IdentifiableSurface::Type::kCanvasReadback, input_digest_),
                blink::IdentifiabilityDigestOfBytes(base::make_span(data_buffer->Pixels(), data_buffer->ComputeByteSize())))
...
void CanvasAsyncBlobCreator::TraceCanvasContent(Vector<unsigned char>* encoded_image)
{
    TRACE_EVENT_INSTANT(
        TRACE_DISABLED_BY_DEFAULT("identifiability.high_entropy_api"), "CanvasReadback", perfetto::Flow::FromPointer(this), [&](perfetto::EventContext ctx) {
            String data = "data:";
            if (encoded_image) {
                data = data + ImageEncodingMimeTypeName(mime_type_) + ";base64," + Base64Encode(*encoded_image);
            }
...
void CanvasAsyncBlobCreator::CreateNullAndReturnResult()
{
    ...
    if (function_type_ == kHTMLCanvasToBlobCallback) {
        ...
        RecordIdleTaskStatusHistogram(idle_task_status_);
        context_->GetTaskRunner(TaskType::kCanvasBlobSerialization)
            ->PostTask(FROM_HERE, WTF::BindOnce(&V8BlobCallback::InvokeAndReportException, WrapPersistent(callback_.Get()), nullptr, nullptr));
    } else {
        context_->GetTaskRunner(TaskType::kCanvasBlobSerialization)
            ->PostTask(FROM_HERE,
                WTF::BindOnce(&ScriptPromiseResolverBase::Reject<DOMException, DOMException*>, WrapPersistent(script_promise_resolver_.Get()),
                    WrapPersistent(MakeGarbageCollected<DOMException>(DOMExceptionCode::kEncodingError, "Encoding of the source image has failed."))));
    }
...
void CanvasAsyncBlobCreator::EncodeImageOnEncoderThread(CrossThreadHandle<CanvasAsyncBlobCreator> cross_thread_handle,
    scoped_refptr<base::SingleThreadTaskRunner> task_runner, sk_sp<SkImage> skia_image, std::unique_ptr<ImageDataBuffer> data_buffer,
    ImageEncodingMimeType mime_type, double quality)
{
    DCHECK(!IsMainThread());
    Vector<unsigned char> encoded_image;
    if (!EncodeImage(std::move(data_buffer), mime_type, quality, &encoded_image)) {
        PostCrossThreadTask(*task_runner, FROM_HERE,
            CrossThreadBindOnce(&CanvasAsyncBlobCreator::CreateNullAndReturnResult, MakeUnwrappingCrossThreadHandle(cross_thread_handle)));
        return;
    }

    PostCrossThreadTask(*task_runner, FROM_HERE,
        CrossThreadBindOnce(
            &CanvasAsyncBlobCreator::CreateBlobAndReturnResult, MakeUnwrappingCrossThreadHandle(cross_thread_handle), std::move(encoded_image)));
}
...
void CanvasAsyncBlobCreator::IdleTaskStartTimeoutEvent(double quality)
{
    if (idle_task_status_ == kIdleTaskStarted) {
        ...
        PostDelayedTaskToCurrentThread(
            FROM_HERE, WTF::BindOnce(&CanvasAsyncBlobCreator::IdleTaskCompleteTimeoutEvent, WrapPersistent(this)), kIdleTaskCompleteTimeoutDelayMs);
        ...
        if (InitializeEncoder(quality)) {
            context_->GetTaskRunner(TaskType::kCanvasBlobSerialization)
                ->PostTask(FROM_HERE, WTF::BindOnce(&CanvasAsyncBlobCreator::ForceEncodeRows, WrapPersistent(this)));
        } else {
            // Failing in initialization of encoder
            SignalAlternativeCodePathFinishedForTesting();
        }
    } else {
        DCHECK(idle_task_status_ == kIdleTaskFailed || idle_task_status_ == kIdleTaskCompleted);
        SignalAlternativeCodePathFinishedForTesting();
    }
}
...
void CanvasAsyncBlobCreator::PostDelayedTaskToCurrentThread(const base::Location& location, base::OnceClosure task, double delay_ms)
{
    context_->GetTaskRunner(TaskType::kCanvasBlobSerialization)->PostDelayedTask(location, std::move(task), base::Milliseconds(delay_ms));
}
