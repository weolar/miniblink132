...
// static
Image::ImageDecodingMode ImageElementBase::ParseImageDecodingMode(const AtomicString& async_attr_value)
{
...
mojom::blink::PreferredColorScheme ImageElementBase::PreferredColorScheme() const
{
    const Element& element = GetElement();
    const ComputedStyle* style = element.GetComputedStyle();
    return element.GetDocument().GetStyleEngine().ResolveColorSchemeForEmbedding(style);
}
...
scoped_refptr<Image> ImageElementBase::GetSourceImageForCanvas(
    FlushReason, SourceImageStatus* status, const gfx::SizeF& default_object_size, const AlphaDisposition alpha_disposition)
{
    ...
    if (auto* svg_image = DynamicTo<SVGImage>(source_image.get())) {
        ...
        const SVGImageViewInfo* view_info = SVGImageForContainer::CreateViewInfo(*svg_image, GetElement());
        const gfx::SizeF image_size = SVGImageForContainer::ConcreteObjectSize(*svg_image, view_info, default_object_size);
        if (image_size.IsEmpty()) {
            *status = kZeroSizeImageSourceStatus;
            return nullptr;
        }
        source_image = SVGImageForContainer::Create(*svg_image, image_size, 1, view_info, PreferredColorScheme());
    }
...
gfx::SizeF ImageElementBase::ElementSize(const gfx::SizeF& default_object_size, const RespectImageOrientationEnum respect_orientation) const
{
    ImageResourceContent* image_content = CachedImage();
...
gfx::SizeF ImageElementBase::DefaultDestinationSize(const gfx::SizeF& default_object_size, const RespectImageOrientationEnum respect_orientation) const
{
...
static bool HasDimensionsForImage(SVGImage& svg_image, std::optional<gfx::Rect> crop_rect, const ImageBitmapOptions* options)
{
    if (crop_rect) {
        return true;
    }
    ...
    if (!SVGImageForContainer::ConcreteObjectSize(svg_image, nullptr, gfx::SizeF()).IsEmpty()) {
        return true;
    }
    return false;
}
    ...
    ScriptState* script_state, std::optional<gfx::Rect> crop_rect, const ImageBitmapOptions* options, ExceptionState& exception_state)
{
    ImageResourceContent* image_content = CachedImage();
    if (!image_content) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "No image can be retrieved from the provided element.");
        return EmptyPromise();
    }
    if (options->hasResizeWidth() && options->resizeWidth() == 0) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The resize width dimension is equal to 0.");
        return EmptyPromise();
    }
    ...
    if (auto* svg_image = DynamicTo<SVGImage>(image_content->GetImage())) {
        ...
        return ImageBitmap::CreateAsync(this, crop_rect, script_state, GetElement().GetDocument().GetTaskRunner(TaskType::kInternalDefault),
...
Image::ImageDecodingMode ImageElementBase::GetDecodingModeForPainting(PaintImage::Id new_id)
{
    const bool content_transitioned = last_painted_image_id_ != PaintImage::kInvalidId && new_id != PaintImage::kInvalidId && last_painted_image_id_ != new_id;
