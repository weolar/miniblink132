...
class HTMLCanvasElementTest : public RenderingTest, public PaintTestConfigurations {
public:
...
TEST_P(HTMLCanvasElementTest, NoResourceProviderAfterCanvas2DLayerBridgeCreation)
{
    ...
    auto* canvas = To<HTMLCanvasElement>(GetDocument().getElementById(AtomicString("c")));
...
TEST_P(HTMLCanvasElementTest, CleanCanvasResizeDoesntClearFrameBuffer)
{
    ...
    auto* canvas = To<HTMLCanvasElement>(GetDocument().getElementById(AtomicString("c")));
    CanvasResourceProvider* provider = canvas->GetOrCreateCanvasResourceProvider(RasterModeHint::kPreferGPU);

    cc::PaintFlags fill_flags = FillFlags();
    fill_flags.setColor(SkColors::kBlue);
    EXPECT_THAT(provider->LastRecording(), Optional(RecordedOpsAre(PaintOpEq<DrawRectOp>(SkRect::MakeXYWH(0, 0, 5, 5), fill_flags))));
}
...
TEST_P(HTMLCanvasElementTest, CanvasResizeClearsFrameBuffer)
{
    ...
    EXPECT_THAT(provider->LastRecording(),
        Optional(RecordedOpsAre(
            PaintOpEq<DrawRectOp>(SkRect::MakeXYWH(0, 0, 10, 20), ClearRectFlags()), PaintOpEq<DrawRectOp>(SkRect::MakeXYWH(0, 0, 5, 5), fill_flags))));
}
...
TEST_P(HTMLCanvasElementTest, CreateLayerUpdatesCompositing)
{
    ...
    auto* canvas = To<HTMLCanvasElement>(GetDocument().getElementById(AtomicString("canvas")));
    EXPECT_FALSE(canvas->GetLayoutObject()->FirstFragment().PaintProperties()->PaintOffsetTranslation());
    ...
    ASSERT_EQ(painting_layer, To<LayoutBoxModelObject>(canvas->GetLayoutObject())->PaintingLayer());
    EXPECT_FALSE(canvas->GetLayoutObject()->FirstFragment().PaintProperties()->PaintOffsetTranslation());
}
...
TEST_P(HTMLCanvasElementTest, CanvasInvalidationInFrame)
{
    ...
    EXPECT_FALSE(ChildDocument().GetPage()->Animator().has_canvas_invalidation_for_test());
    RunDocumentLifecycle();
...
{
    EXPECT_NE(HTMLCanvasElement::BrokenCanvas(2.0).first, HTMLCanvasElement::BrokenCanvas(1.0).first);
...
class HTMLCanvasElementWithTracingSyncTest : public HTMLCanvasElementWithTracingTest, public testing::WithParamInterface<const char*> { };

INSTANTIATE_TEST_SUITE_P(All, HTMLCanvasElementWithTracingSyncTest,
...
TEST_P(HTMLCanvasElementWithTracingSyncTest, CanvasReadbackEmitsIdentifiabilityTraces)
{
    // Enable script so that the canvas will create a LayoutHTMLCanvas.
    GetDocument().GetSettings()->SetScriptEnabled(true);
    ...
    test_trace_processor.StartTrace(base::test::DefaultTraceConfig("disabled-by-default-identifiability.high_entropy_api", false), perfetto::kInProcessBackend);
    auto* script = GetDocument().CreateRawElement(html_names::kScriptTag);
    ...
    EXPECT_THAT(result.value(), Contains(ElementsAre(Eq("CanvasReadback"), StartsWith("data:"))));
}

class HTMLCanvasElementWithTracingAsyncTest : public HTMLCanvasElementWithTracingTest,
                                              public testing::WithParamInterface<std::pair<const char*, const char*>> { };

INSTANTIATE_TEST_SUITE_P(All, HTMLCanvasElementWithTracingAsyncTest,
...
TEST_P(HTMLCanvasElementWithTracingAsyncTest, CanvasReadbackEmitsIdentifiabilityTraces)
{
    // Enable script so that the canvas will create a LayoutHTMLCanvas.
    GetDocument().GetSettings()->SetScriptEnabled(true);
    ...
    ClassicScript* script = ClassicScript::CreateUnspecifiedScript(GetParam().first, ScriptSourceLocationType::kUnknown, SanitizeScriptErrors::kSanitize);

    ScriptEvaluationResult script_result = script->RunScriptOnScriptStateAndReturnValue(script_state);

    auto promise = ToResolvedPromise<IDLAny>(script_state, script_result.GetSuccessValue());
...

    {
      ...
      SELECT s_in.name, s_out.name
        ...
        auto result = test_trace_processor.RunQuery(query);
        ASSERT_TRUE(result.has_value()) << result.error();
        EXPECT_THAT(result.value(), Contains(ElementsAre(Eq("CanvasReadback"), Eq("CanvasReadback"))).Times(2));
    }

    {
      ...
      WITH RECURSIVE canvas_readback AS (
        ...
        auto result = test_trace_processor.RunQuery(query);
        ASSERT_TRUE(result.has_value()) << result.error();
        EXPECT_THAT(result.value(), Contains(ElementsAre(Eq(GetParam().second), StartsWith("data:"))));
    }
}
