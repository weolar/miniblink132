...
class CORE_EXPORT CanvasFontCache final : public GarbageCollected<CanvasFontCache>, public Thread::TaskObserver {
...
public:
    ...
    void WillUseCurrentFont()
    {
        SchedulePruningIfNeeded();
    }
    bool GetFontUsingDefaultStyle(HTMLCanvasElement& canvas, const String&, Font&);
...
private:
    ...
    typedef HeapHashMap<String, Member<MutableCSSPropertyValueSet>> MutableStylePropertyMap;

    struct FontWrapper : public GarbageCollected<FontWrapper> {
        explicit FontWrapper(Font&& font)
            : font(font)
        {
        }
