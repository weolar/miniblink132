...
CanvasResourceTracker* CanvasResourceTracker::For(v8::Isolate* isolate)
{
    ...
    auto* canvas_resource_tracker = static_cast<CanvasResourceTracker*>(isolate_data->GetUserData(UserData::Key::kCanvasResourceTracker));
    if (!canvas_resource_tracker) {
        canvas_resource_tracker = MakeGarbageCollected<CanvasResourceTracker>();
        isolate_data->SetUserData(UserData::Key::kCanvasResourceTracker, canvas_resource_tracker);
    }
    return canvas_resource_tracker;
}

void CanvasResourceTracker::Add(CanvasRenderingContextHost* resource, ExecutionContext* execution_context)
{
    resource_map_.insert(resource, execution_context);
}

const CanvasResourceTracker::ResourceMap& CanvasResourceTracker::GetResourceMap() const
{
