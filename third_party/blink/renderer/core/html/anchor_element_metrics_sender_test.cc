...
class MockAnchorElementMetricsHost : public mojom::blink::AnchorElementMetricsHost {
public:
    explicit MockAnchorElementMetricsHost(mojo::PendingReceiver<mojom::blink::AnchorElementMetricsHost> pending_receiver)
    {
...
private:
    ...
    void ReportAnchorElementClick(mojom::blink::AnchorElementClickPtr click) override
    {
        clicks_.emplace_back(std::move(click));
    }

    void ReportAnchorElementsEnteredViewport(WTF::Vector<mojom::blink::AnchorElementEnteredViewportPtr> elements) override
    {
    ...
    void ReportAnchorElementsLeftViewport(WTF::Vector<mojom::blink::AnchorElementLeftViewportPtr> elements) override
    {
        for (auto& element : elements) {
    ...
    void ReportAnchorElementPointerDataOnHoverTimerFired(mojom::blink::AnchorElementPointerDataOnHoverTimerFiredPtr pointer_data) override
    {
        pointer_data_on_hover_.emplace_back(std::move(pointer_data));
    }

    void ReportAnchorElementPointerOver(mojom::blink::AnchorElementPointerOverPtr pointer_over_event) override
    {
        pointer_over_.emplace_back(std::move(pointer_over_event));
    }

    void ReportAnchorElementPointerOut(mojom::blink::AnchorElementPointerOutPtr hover_event) override
    {
        pointer_hover_dwell_time_.emplace_back(std::move(hover_event));
    }

    void ReportAnchorElementPointerDown(mojom::blink::AnchorElementPointerDownPtr pointer_down_event) override
    {
        pointer_down_.emplace_back(std::move(pointer_down_event));
    }

    void ReportAnchorElementsPositionUpdate(WTF::Vector<mojom::blink::AnchorElementPositionUpdatePtr> position_updates) override
    {
    ...
    void ReportNewAnchorElements(WTF::Vector<mojom::blink::AnchorElementMetricsPtr> elements, const WTF::Vector<uint32_t>& removed_elements) override
    {
        for (auto& element : elements) {
        ...
        removed_anchor_ids_.insert(removed_elements.begin(), removed_elements.end());
    }

    void ProcessPointerEventUsingMLModel(mojom::blink::AnchorElementPointerEventForMLModelPtr pointer_event) override
    {
    }
...
public:
    ...
    std::vector<mojom::blink::AnchorElementPointerOutPtr> pointer_hover_dwell_time_;
    std::vector<mojom::blink::AnchorElementPointerDownPtr> pointer_down_;
    std::vector<mojom::blink::AnchorElementPointerDataOnHoverTimerFiredPtr> pointer_data_on_hover_;
...
protected:
    ...
    void SetUp() override
    {
        ...
        feature_list_.InitAndEnableFeatureWithParameters(features::kNavigationPredictor, params);
        ...
        MainFrame().GetFrame()->GetBrowserInterfaceBroker().SetBinderForTesting(
            mojom::blink::AnchorElementMetricsHost::Name_, WTF::BindRepeating(&AnchorElementMetricsSenderTest::Bind, WTF::Unretained(this)));
    }

    void TearDown() override
    {
        MainFrame().GetFrame()->GetBrowserInterfaceBroker().SetBinderForTesting(mojom::blink::AnchorElementMetricsHost::Name_, {});
        hosts_.clear();
    ...
    {
        auto* test_web_frame_widget = MakeGarbageCollected<frame_test_helpers::TestWebFrameWidget>(std::move(pass_key), std::move(frame_widget_host),
            std::move(frame_widget), std::move(widget_host), std::move(widget), std::move(task_runner), frame_sink_id, hidden, never_composited,
    ...
    {
        auto host
            = std::make_unique<MockAnchorElementMetricsHost>(mojo::PendingReceiver<mojom::blink::AnchorElementMetricsHost>(std::move(message_pipe_handle)));
    ...
    void ProcessEvents(size_t expected_anchors)
    {
        ...
        if (auto* metrics_sender = AnchorElementMetricsSender::From(GetDocument())) {
        ...
        while (expected_anchors > 0 && (hosts_.empty() || expected_anchors > hosts_[0]->elements_.size())) {
            // Wait 50ms.
    ...
    void SetMockClock()
    {
        AnchorElementMetricsSender::From(GetDocument())->SetTickClockForTesting(&clock_);
    }

    void VerticalScroll(float dy)
    {
        GetWebFrameWidget().DispatchThroughCcInputHandler(SyntheticWebGestureEventBuilder::BuildScrollBegin(
            /*dx_hint=*/0.0f, /*dy_hint=*/0.0f, WebGestureDevice::kTouchscreen));
        GetWebFrameWidget().DispatchThroughCcInputHandler(SyntheticWebGestureEventBuilder::BuildScrollUpdate(
            /*dx=*/0.0f, dy, WebInputEvent::kNoModifiers, WebGestureDevice::kTouchscreen));
        GetWebFrameWidget().DispatchThroughCcInputHandler(
            SyntheticWebGestureEventBuilder::Build(WebInputEvent::Type::kGestureScrollEnd, WebGestureDevice::kTouchscreen));
    ...
    void ProcessPositionUpdates()
    {
        platform_->RunForPeriodSeconds(ConvertDOMHighResTimeStampToSeconds(
            AnchorElementViewportPositionTracker::MaybeGetOrCreateFor(GetDocument())->GetIntersectionObserverForTesting()->delay()));
        GetDocument().View()->UpdateAllLifecyclePhasesForTest();
        platform_->RunForPeriod(AnchorElementMetricsSender::kUpdateMetricsTimeGap);
        base::RunLoop().RunUntilIdle();
    }

    HTMLAnchorElement* AddAnchor(Document& document, String inner_text, int height)
    {
        auto* anchor = MakeGarbageCollected<HTMLAnchorElement>(document);
        anchor->setInnerText(inner_text);
        anchor->setHref("https://foo.com");
        anchor->SetInlineStyleProperty(CSSPropertyID::kHeight, String::Format("%dpx", height));
...
TEST_F(AnchorElementMetricsSenderTest, AddAnchorElementHTTP)
{
    ...
    LoadURL(source);
    main_resource.Complete(R"HTML(<a id="anchor1" href="">example</a><a id="anchor2" href="">example</a>)HTML");
...
TEST_F(AnchorElementMetricsSenderTest, AddAnchorElementAfterLoad)
{
    ...
    EXPECT_EQ(mock_host->entered_viewport_[0]->anchor_id, mock_host->elements_[0]->anchor_id);
}
...
TEST_F(AnchorElementMetricsSenderTest, RemoveAnchorElementWithoutMoreInsertions)
{
    String source("https://example.com/p1");
...
TEST_F(AnchorElementMetricsSenderTest, AnchorElementLeftViewport)
{
    ...
    SetMockClock();
    AnchorElementMetricsSender::From(GetDocument())->SetNowAsNavigationStartForTesting();
    ...
    GetDocument().View()->LayoutViewport()->SetScrollOffset(ScrollOffset(0, 2 * kViewportHeight), mojom::blink::ScrollType::kProgrammatic);
    ProcessEvents(1);
    EXPECT_EQ(1u, mock_host->entered_viewport_.size());
    EXPECT_EQ(wait_time1, mock_host->entered_viewport_[0]->navigation_start_to_entered_viewport);
    EXPECT_EQ(0u, mock_host->left_viewport_.size());
    ...
    ProcessEvents(1);
    EXPECT_EQ(2u, mock_host->entered_viewport_.size());
    EXPECT_EQ(wait_time1 + time_in_viewport_1 + wait_time2, mock_host->entered_viewport_[1]->navigation_start_to_entered_viewport);
    EXPECT_EQ(1u, mock_host->left_viewport_.size());
...
TEST_F(AnchorElementMetricsSenderTest, AnchorElementInteractionTrackerSendsPointerEvents)
{
    String source("https://example.com/p1");
    ...
    auto move_to = [this](const auto x, const auto y) {
        ...
        WebMouseEvent event(WebInputEvent::Type::kMouseMove, coordinates, coordinates, WebPointerProperties::Button::kNoButton, 0, WebInputEvent::kNoModifiers,
            WebInputEvent::GetStaticTimeStampForTests());
        GetDocument().GetFrame()->GetEventHandler().HandleMouseMoveEvent(event, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
    };
    ...
    auto mouse_press = [this](const auto x, const auto y, const auto button) {
        ...
        WebMouseEvent event(WebInputEvent::Type::kMouseDown, coordinates, coordinates, button, 0, modifier, WebInputEvent::GetStaticTimeStampForTests());
    ...
    EXPECT_EQ(mock_host->elements_[0]->anchor_id, mock_host->pointer_over_[0]->anchor_id);
    EXPECT_EQ(wait_time_1, mock_host->pointer_over_[0]->navigation_start_to_pointer_over);
    EXPECT_EQ(0u, mock_host->pointer_hover_dwell_time_.size());
    ...
    EXPECT_EQ(mock_host->elements_[0]->anchor_id, mock_host->pointer_hover_dwell_time_[0]->anchor_id);
    EXPECT_EQ(hover_dwell_time_1, mock_host->pointer_hover_dwell_time_[0]->hover_dwell_time);
    ...
    EXPECT_EQ(mock_host->elements_[0]->anchor_id, mock_host->pointer_over_[1]->anchor_id);
    EXPECT_EQ(wait_time_1 + hover_dwell_time_1 + wait_time_2, mock_host->pointer_over_[1]->navigation_start_to_pointer_over);
    EXPECT_EQ(1u, mock_host->pointer_hover_dwell_time_.size());
    ...
    EXPECT_EQ(mock_host->elements_[0]->anchor_id, mock_host->pointer_hover_dwell_time_[1]->anchor_id);
    EXPECT_EQ(hover_dwell_time_2, mock_host->pointer_hover_dwell_time_[1]->hover_dwell_time);
    ...
    EXPECT_EQ(wait_time_1 + hover_dwell_time_1 + wait_time_2 + hover_dwell_time_2, mock_host->pointer_down_[0]->navigation_start_to_pointer_down);
    ...
    EXPECT_EQ(wait_time_1 + hover_dwell_time_1 + wait_time_2 + hover_dwell_time_2, mock_host->pointer_down_[1]->navigation_start_to_pointer_down);
}
...
TEST_F(AnchorElementMetricsSenderTest, AnchorElementClicked)
{
    ...
    EXPECT_LE(base::TimeDelta(), mock_host->clicks_[0]->navigation_start_to_click);
    // Wait until the script has had time to run.
    platform_->RunForPeriodSeconds(5.);
...
TEST_F(AnchorElementMetricsSenderTest, ReportAnchorElementPointerDataOnHoverTimerFired)
{
    String source("https://example.com/p1");
    SimRequest main_resource(source, "text/html");
    LoadURL(source);
    ...
    GetDocument().GetAnchorElementInteractionTracker()->SetTaskRunnerForTesting(task_runner, task_runner->GetMockTickClock());
    ...
    for (base::TimeDelta t; t <= 2 * AnchorElementInteractionTracker::GetHoverDwellTime(); t += timestep) {
        gfx::PointF coordinates = origin + gfx::ScaleVector2d(velocity, t.InSecondsF());
        WebMouseEvent event(WebInputEvent::Type::kMouseMove, coordinates, coordinates, WebPointerProperties::Button::kNoButton, 0, WebInputEvent::kNoModifiers,
            WebInputEvent::GetStaticTimeStampForTests());
        GetDocument().GetFrame()->GetEventHandler().HandleMouseMoveEvent(event, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
    ...
    EXPECT_TRUE(mock_host->pointer_data_on_hover_[0]->pointer_data->is_mouse_pointer);
    EXPECT_NEAR(20.0 * std::sqrt(2.0), mock_host->pointer_data_on_hover_[0]->pointer_data->mouse_velocity, 0.5);
}
...
TEST_F(AnchorElementMetricsSenderTest, MaxIntersectionObservations)
{
    ...
    scoped_feature_list.InitAndEnableFeatureWithParameters(
        features::kNavigationPredictor, { { "max_intersection_observations", "3" }, { "random_anchor_sampling_period", "1" } });
    ...
    ProcessEvents(3);
    ASSERT_EQ(1u, hosts_.size());
    auto* intersection_observer = AnchorElementViewportPositionTracker::MaybeGetOrCreateFor(GetDocument())->GetIntersectionObserverForTesting();
    ...
    auto observations = [&]() -> HeapVector<Member<HTMLAnchorElement>> {
        ...
        base::ranges::transform(intersection_observer->Observations(), std::back_inserter(observed_anchors),
            [](IntersectionObservation* observation) { return To<HTMLAnchorElement>(observation->Target()); });
        return observed_anchors;
    };
    EXPECT_THAT(observations(), ::testing::UnorderedElementsAre(anchor_1, anchor_2, anchor_3));

    // Remove anchor 1.
    anchor_1->remove();
    EXPECT_THAT(observations(), ::testing::UnorderedElementsAre(anchor_2, anchor_3));
    ...
    ProcessEvents(4);
    EXPECT_THAT(observations(), ::testing::UnorderedElementsAre(anchor_2, anchor_3, anchor_4));
    ...
    anchor_2->remove();
    EXPECT_THAT(observations(), ::testing::UnorderedElementsAre(anchor_1, anchor_3, anchor_4));
...
TEST_F(AnchorElementMetricsSenderTest, AnchorUnobservedByIntersectionObserver)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeatureWithParameters(
        features::kNavigationPredictor, { { "max_intersection_observations", "1" }, { "random_anchor_sampling_period", "1" } });
...
TEST_F(AnchorElementMetricsSenderTest, AnchorNotInViewportUnobservedByIntersectionObserver)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeatureWithParameters(
        features::kNavigationPredictor, { { "max_intersection_observations", "1" }, { "random_anchor_sampling_period", "1" } });
...
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeatureWithParameters(features::kNavigationPredictor, { { "intersection_observer_delay", "252ms" } });
    ...
    IntersectionObserver* intersection_observer = AnchorElementViewportPositionTracker::MaybeGetOrCreateFor(GetDocument())->GetIntersectionObserverForTesting();
...
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeature(features::kNavigationPredictorNewViewportFeatures);
    ...
    <body style="margin: 0px">
      ...
      <a href="https://bar.com/3"
        ...
        div_1_height, anchor_1_height, div_2_height, anchor_2_height, anchor_3_height));
    Compositor().BeginFrame();
    ...
    uint32_t anchor_1_id = AnchorElementId(To<HTMLAnchorElement>(*anchors->item(0)));
    uint32_t anchor_2_id = AnchorElementId(To<HTMLAnchorElement>(*anchors->item(1)));
    uint32_t anchor_3_id = AnchorElementId(To<HTMLAnchorElement>(*anchors->item(2)));
    ...
    WebInputEvent::Modifiers modifier = WebInputEvent::kLeftButtonDown;
    WebMouseEvent event(WebInputEvent::Type::kMouseDown, coordinates, screen_coordinates, WebPointerProperties::Button::kLeft, 0, modifier,
        WebInputEvent::GetStaticTimeStampForTests());
    GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(event);
    VerticalScroll(-unit);
    ...
    EXPECT_FLOAT_EQ(-2.5f * unit / kViewportHeight, get_distance_ratio(anchor_1_id));
    EXPECT_FLOAT_EQ(2.5f * unit / kViewportHeight, get_position_ratio(anchor_1_id));
    EXPECT_FLOAT_EQ(2.5f * unit / kViewportHeight, get_distance_ratio(anchor_2_id));
    EXPECT_FLOAT_EQ(7.5f * unit / kViewportHeight, get_position_ratio((anchor_2_id)));
    ...
    EXPECT_EQ(2u, positions.size());
    EXPECT_FLOAT_EQ(-2.0f * unit / kViewportHeight, get_distance_ratio(anchor_1_id));
    EXPECT_FLOAT_EQ(3.0f * unit / kViewportHeight, get_position_ratio(anchor_1_id));
    ...
    EXPECT_FLOAT_EQ(8.0f * unit / kViewportHeight, get_distance_ratio(anchor_2_id));
    EXPECT_FLOAT_EQ(13.0f * unit / kViewportHeight, get_position_ratio(anchor_2_id));
    EXPECT_TRUE(!base::Contains(positions, anchor_3_id));
}
...
// main frames.
TEST_F(AnchorElementMetricsSenderTest, PositionUpdate_IgnorePointerDownInsideLocalRootSubframe)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeature(features::kNavigationPredictorNewViewportFeatures);
    ...
    <body style="margin: 0px">
      ...
      <div style="height: %dpx;"></div>
        ...
        div_1_height, anchor_height, iframe_height, div_2_height));
    ...
    WebLocalFrameImpl* local_child = WebViewHelper().CreateLocalChild(*remote_child);
    ...
    auto create_mouse_press = [](gfx::PointF coordinates) -> WebMouseEvent {
        return WebMouseEvent(WebInputEvent::Type::kMouseDown, coordinates, coordinates, WebPointerProperties::Button::kLeft, 0, WebInputEvent::kLeftButtonDown,
            WebInputEvent::GetStaticTimeStampForTests());
    };
    ...
    EXPECT_FLOAT_EQ(-5.5f * unit / kViewportHeight, positions.begin()->second->distance_from_pointer_down_ratio.value());
}

// TODO(crbug.com/347719430): This test can be removed if
// LocalFrameView::FrameToViewport supports local root subframes with local
// main frames.
TEST_F(AnchorElementMetricsSenderTest, PositionUpdate_NotComputedForAnchorInsideLocalRootSubframe)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeature(features::kNavigationPredictorNewViewportFeatures);
    ...
    auto viewport_intersection_state = mojom::blink::ViewportIntersectionState::New();
    ...
    static_cast<WebFrameWidgetImpl*>(widget)->ApplyViewportIntersectionForTesting(std::move(viewport_intersection_state));
    ...
    WebInputEvent::Modifiers modifier = WebInputEvent::kLeftButtonDown;
    WebMouseEvent event(WebInputEvent::Type::kMouseDown, coordinates, coordinates, WebPointerProperties::Button::kLeft, 0, modifier,
        WebInputEvent::GetStaticTimeStampForTests());
    GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(event);
...
TEST_F(AnchorElementMetricsSenderTest, PositionUpdate_BrowserTopControlsHeight)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeature(features::kNavigationPredictorNewViewportFeatures);
    ...
    WebView().ResizeWithBrowserControls(gfx::Size(kViewportWidth, kViewportHeight - top_controls_height), top_controls_height, /*bottom_controls_height=*/0,
    ...
    EXPECT_EQ(kViewportHeight - top_controls_height, visual_viewport.Size().height());
    ...
    GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(WebMouseEvent(WebInputEvent::Type::kMouseDown, coordinates, coordinates,
        WebPointerProperties::Button::kLeft, 0, WebInputEvent::kLeftButtonDown, WebInputEvent::GetStaticTimeStampForTests()));
    ...
    WebView().ResizeWithBrowserControls(gfx::Size(kViewportWidth, kViewportHeight), top_controls_height,
    ...
    EXPECT_FLOAT_EQ(-4.5f * unit / kViewportHeight, positions.begin()->second->distance_from_pointer_down_ratio.value());
    EXPECT_FLOAT_EQ(1.5f * unit / kViewportHeight, positions.begin()->second->vertical_position_ratio);
    ...
    EXPECT_FLOAT_EQ(-2.5f * unit / kViewportHeight, positions.begin()->second->distance_from_pointer_down_ratio.value());
    EXPECT_FLOAT_EQ(3.5f * unit / kViewportHeight, positions.begin()->second->vertical_position_ratio);
}
...
TEST_F(AnchorElementMetricsSenderTest, SubframeWithObservedAnchorsDetached)
{
    ...
    frame_test_helpers::LoadFrameDontWait(MainFrame().FirstChild()->ToWebLocalFrame(), KURL(subframe_source));
    ...
    WebLocalFrameImpl* subframe = To<WebLocalFrameImpl>(MainFrame().FirstChild()->ToWebLocalFrame());
    Persistent<Document> subframe_doc = subframe->GetFrame()->GetDocument();
    uint32_t subframe_anchor_id = AnchorElementId(To<HTMLAnchorElement>(*subframe_doc->links()->item(0)));
    ...
    EXPECT_EQ(1u, mock_host->removed_anchor_ids_.size());
    EXPECT_TRUE(base::Contains(mock_host->removed_anchor_ids_, subframe_anchor_id));
}

TEST_F(AnchorElementMetricsSenderTest, AnchorsNotReportedAsRemovedWhenMainFrameIsDetached)
{
    // Navigate the main frame.
    String source("https://foo.com");
    SimRequest main_resource(source, "text/html");
    ...
    AnchorElementMetricsSender* sender = AnchorElementMetricsSender::From(GetDocument());
    ...
    static_cast<frame_test_helpers::TestWebFrameClient*>(MainFrame().FirstChild()->ToWebLocalFrame()->Client())
        ->SetFrameDetachedCallback(
            base::BindLambdaForTesting([&document, &microtask]() { document->GetAgent().event_loop()->EnqueueMicrotask(std::move(microtask)); }));
...
TEST_F(AnchorElementMetricsSenderTest, ObservedAnchorInIframeHasHrefUnsetAndIsRemoved)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeatureWithParameters(
        features::kNavigationPredictor, { { "max_intersection_observations", "1" }, { "random_anchor_sampling_period", "1" } });
    ...
    WebLocalFrameImpl* subframe = static_cast<WebLocalFrameImpl*>(MainFrame().FirstChild()->ToWebLocalFrame());
    WeakPersistent<Document> subframe_document = static_cast<Document*>(subframe->GetDocument());

    // Create a shadow root in the subframe and add an anchor to it.
    ShadowRoot& shadow_root = subframe_document->body()->AttachShadowRootForTesting(ShadowRootMode::kOpen);
    WeakPersistent<HTMLAnchorElement> anchor_1 = MakeGarbageCollected<HTMLAnchorElement>(*subframe_document);
    ...
    GetDocument().QuerySelector(AtomicString("iframe"), ASSERT_NO_EXCEPTION)->remove();
