...
void HTMLBRElement::CollectStyleForPresentationAttribute(const QualifiedName& name, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
    if (name == html_names::kClearAttr) {
        ...
        if (!value.empty()) {
            if (EqualIgnoringASCIICase(value, "all")) {
                AddPropertyToPresentationAttributeStyle(style, CSSPropertyID::kClear, CSSValueID::kBoth);
            } else {
                AddPropertyToPresentationAttributeStyle(style, CSSPropertyID::kClear, value);
            }
        }
    } else {
