...
public:
    ...
    DEFINE_ATTRIBUTE_EVENT_LISTENER(validationstatuschange, kValidationstatuschange)

    void Trace(Visitor*) const override;

    using PermissionStatusMap = HashMap<mojom::blink::PermissionName, mojom::blink::PermissionStatus>;
    // CachedPermissionStatus::Client overrides.
    void OnPermissionStatusInitialized(PermissionStatusMap initilized_map) override;
    ...
    // only.
    static Vector<mojom::blink::PermissionDescriptorPtr> ParsePermissionDescriptorsForTesting(const AtomicString& type);
    ...
    bool IsHTMLPermissionElement() const final
    {
        return true;
    }
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(HTMLPemissionElementClickingEnabledTest, UnclickableBeforeRegistered);
    FRIEND_TEST_ALL_PREFIXES(HTMLPemissionElementIntersectionTest, IntersectionChanged);
    FRIEND_TEST_ALL_PREFIXES(HTMLPemissionElementIntersectionTest, IntersectionChangedDisableEnableDisable);
    FRIEND_TEST_ALL_PREFIXES(HTMLPemissionElementIntersectionTest, ClickingDisablePseudoClass);
    FRIEND_TEST_ALL_PREFIXES(HTMLPemissionElementIntersectionTest, ContainerDivRotates);
    FRIEND_TEST_ALL_PREFIXES(HTMLPemissionElementIntersectionTest, ContainerDivOpacity);
    FRIEND_TEST_ALL_PREFIXES(HTMLPemissionElementIntersectionTest, ContainerDivClipPath);
    FRIEND_TEST_ALL_PREFIXES(HTMLPemissionElementIntersectionTest, IntersectionVisibleOverlapsRecentAttachedInterval);
    FRIEND_TEST_ALL_PREFIXES(HTMLPemissionElementFencedFrameTest, NotAllowedInFencedFrame);
    FRIEND_TEST_ALL_PREFIXES(HTMLPemissionElementSimTest, BlockedByMissingFrameAncestorsCSP);
    FRIEND_TEST_ALL_PREFIXES(HTMLPemissionElementSimTest, EnableClickingAfterDelay);
    FRIEND_TEST_ALL_PREFIXES(HTMLPemissionElementSimTest, FontSizeCanDisableElement);
    FRIEND_TEST_ALL_PREFIXES(HTMLPemissionElementSimTest, MovePEPCToAnotherDocument);
    FRIEND_TEST_ALL_PREFIXES(HTMLPemissionElementLayoutChangeTest, InvalidatePEPCAfterMove);
    FRIEND_TEST_ALL_PREFIXES(HTMLPemissionElementLayoutChangeTest, InvalidatePEPCAfterResize);
    FRIEND_TEST_ALL_PREFIXES(HTMLPemissionElementLayoutChangeTest, InvalidatePEPCAfterMoveContainer);
    FRIEND_TEST_ALL_PREFIXES(HTMLPemissionElementLayoutChangeTest, InvalidatePEPCAfterTransformContainer);
    FRIEND_TEST_ALL_PREFIXES(HTMLPemissionElementLayoutChangeTest, InvalidatePEPCLayoutInAnimationFrameCallback);
    FRIEND_TEST_ALL_PREFIXES(HTMLPemissionElementDispatchValidationEventTest, ChangeReasonRestartTimer);
    FRIEND_TEST_ALL_PREFIXES(HTMLPemissionElementDispatchValidationEventTest, DisableEnableClicking);
    FRIEND_TEST_ALL_PREFIXES(HTMLPemissionElementDispatchValidationEventTest, DisableEnableClickingDifferentReasons);
    FRIEND_TEST_ALL_PREFIXES(HTMLPemissionElementTestBase, SetPreciseLocationAttribute);
    ...
    public:
        ...
        DisableReasonExpireTimer(HTMLPermissionElement* element, TimerFiredFunction function)
        ...
        void StartOrRestartWithReason(DisableReason reason, base::TimeDelta interval)
        {
            if (IsActive()) {
                Stop();
            }
        ...
        void set_reason(DisableReason reason)
        {
            reason_ = reason;
        }
        DisableReason reason() const
        {
            return reason_;
        }
    ...
    protected:
        void Fired() final
        {
            (element_->*function_)(this);
        }

        base::OnceClosure BindTimerClosure() final
        {
            return WTF::BindOnce(&DisableReasonExpireTimer::RunInternalTrampoline, WTF::Unretained(this), WrapWeakPersistent(element_.Get()));
        }
    ...
    private:
        ...
        static void RunInternalTrampoline(DisableReasonExpireTimer* timer, HTMLPermissionElement* element)
        {
    ...
    // used for metrics.
    static UserInteractionDeniedReason DisableReasonToUserInteractionDeniedReason(DisableReason reason);
    ...
    void RegisterPermissionObserver(const mojom::blink::PermissionDescriptorPtr& descriptor, mojom::blink::PermissionStatus current_status);
    ...
    void OnEmbeddedPermissionControlRegistered(bool allowed, const std::optional<Vector<mojom::blink::PermissionStatus>>& statuses) override;

    // Callback triggered when permission is decided from browser side.
    void OnEmbeddedPermissionsDecided(mojom::blink::EmbeddedPermissionControlResult result);
    ...
    {
        if (disable_reason_expire_timer_.IsActive() && disable_reason_expire_timer_.reason() == reason) {
    ...
    void DisableClickingTemporarily(DisableReason reason, const base::TimeDelta& duration);
    ...
    void EnableClickingAfterDelay(DisableReason reason, const base::TimeDelta& delay);
    ...
    struct ClickingEnabledState {
        ...
        {
            return is_valid == other.is_valid && invalid_reason == other.invalid_reason;
        }
    };
    ...
    void OnIntersectionChanged(const HeapVector<Member<IntersectionObserverEntry>>& entries);
    ...
    bool IsClickingDisabledIndefinitely(DisableReason reason) const
    {
        ...
        return it != clicking_disabled_reasons_.end() && it->value == base::TimeTicks::Max();
    }

    bool PermissionsGranted() const
    {
        return aggregated_permission_status_.has_value() && aggregated_permission_status_ == mojom::blink::PermissionStatus::GRANTED;
    }
    ...
    HeapMojoReceiverSet<mojom::blink::PermissionObserver, HTMLPermissionElement, HeapMojoWrapperMode::kWithContextObserver, mojom::blink::PermissionName>
    ...
    // browser process.
    HeapMojoReceiver<mojom::blink::EmbeddedPermissionControlClient, HTMLPermissionElement> embedded_permission_control_receiver_;
    ...
    std::optional<mojom::blink::PermissionStatus> initial_aggregated_permission_status_;
    ...
    struct ClickingDisablePseudoState {
        ...
        {
            return has_invalid_style == other.has_invalid_style && is_occluded == other.is_occluded;
        }
    };
    ...
    IntersectionVisibility intersection_visibility_ = IntersectionVisibility::kFullyVisible;
...
template <> struct DowncastTraits<HTMLPermissionElement> {
