...
HTMLImageElement::HTMLImageElement(Document& document, const CreateElementFlags flags)
...
HTMLImageElement* HTMLImageElement::CreateForJSConstructor(Document& document, unsigned width)
{
    HTMLImageElement* image = MakeGarbageCollected<HTMLImageElement>(document);
...
HTMLImageElement* HTMLImageElement::CreateForJSConstructor(Document& document, unsigned width, unsigned height)
{
    HTMLImageElement* image = MakeGarbageCollected<HTMLImageElement>(document);
    image->setWidth(width);
...
{
    if (name == html_names::kWidthAttr || name == html_names::kHeightAttr || name == html_names::kBorderAttr || name == html_names::kVspaceAttr
        || name == html_names::kHspaceAttr || name == html_names::kAlignAttr || name == html_names::kValignAttr)
        return true;
    return HTMLElement::IsPresentationAttribute(name);
}

void HTMLImageElement::CollectStyleForPresentationAttribute(const QualifiedName& name, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
    if (name == html_names::kWidthAttr) {
        ...
        AddPropertyToPresentationAttributeStyle(style, CSSPropertyID::kVerticalAlign, value);
    } else {
        HTMLElement::CollectStyleForPresentationAttribute(name, value, style);
    }
}

void HTMLImageElement::CollectExtraStyleForPresentationAttribute(MutableCSSPropertyValueSet* style)
{
    if (!source_)
        return;

    const AtomicString& width = source_->FastGetAttribute(html_names::kWidthAttr);
    const AtomicString& height = source_->FastGetAttribute(html_names::kHeightAttr);
    if (!width && !height)
        return;
    ...
    if (width) {
        ...
        AddPropertyToPresentationAttributeStyle(style, CSSPropertyID::kWidth, CSSValueID::kAuto);
    }

    if (height) {
        AddHTMLLengthToStyle(style, CSSPropertyID::kHeight, height);
    } else {
        AddPropertyToPresentationAttributeStyle(style, CSSPropertyID::kHeight, CSSValueID::kAuto);
    }

    if (width && height) {
        ApplyAspectRatioToStyle(width, height, style);
    } else {
        AddPropertyToPresentationAttributeStyle(style, CSSPropertyID::kAspectRatio, CSSValueID::kAuto);
    }
}

const AtomicString HTMLImageElement::ImageSourceURL() const
{
    return best_fit_image_url_.IsNull() ? FastGetAttribute(html_names::kSrcAttr) : best_fit_image_url_;
}
...
void HTMLImageElement::ParseAttribute(const AttributeModificationParams& params)
{
    const QualifiedName& name = params.name;
    if (name == html_names::kAltAttr || name == html_names::kTitleAttr) {
        if (UserAgentShadowRoot()) {
            Element* text = UserAgentShadowRoot()->getElementById(AtomicString("alttext"));
            String alt_text_content = AltText();
            if (text && text->textContent() != alt_text_content)
                text->setTextContent(alt_text_content);
        }
    } else if (name == html_names::kSrcAttr || name == html_names::kSrcsetAttr || name == html_names::kSizesAttr) {
        ...
        network::mojom::ReferrerPolicy new_referrer_policy = network::mojom::ReferrerPolicy::kDefault;
        if (!params.new_value.IsNull()) {
            UseCounter::Count(GetDocument(), WebFeature::kHTMLImageElementReferrerPolicyAttribute);

            SecurityPolicy::ReferrerPolicyFromString(params.new_value, kSupportReferrerPolicyLegacyKeywords, &new_referrer_policy);
        }

        network::mojom::ReferrerPolicy old_referrer_policy = network::mojom::ReferrerPolicy::kDefault;
        if (!params.old_value.IsNull()) {
            SecurityPolicy::ReferrerPolicyFromString(params.old_value, kSupportReferrerPolicyLegacyKeywords, &old_referrer_policy);
        }
        ...
        if (loading == LoadingAttributeValue::kEager || (loading == LoadingAttributeValue::kAuto)) {
            GetDocument().UnobserveForLazyLoadedAutoSizedImg(this);
        ...
        CrossOriginAttributeValue new_crossorigin_state = GetCrossOriginAttributeValue(params.new_value);
        CrossOriginAttributeValue old_crossorigin_state = GetCrossOriginAttributeValue(params.old_value);
        ...
        if (window && window->GetFrame()) {
            ...
            AtomicString referrer_policy_attribute = FastGetAttribute(html_names::kReferrerpolicyAttr);
            ...
            window->GetFrame()->GetAttributionSrcLoader()->Register(params.new_value,
                /*element=*/this, referrer_policy);
        }
    } else if (name == html_names::kSharedstoragewritableAttr && RuntimeEnabledFeatures::SharedStorageAPIM118Enabled(GetExecutionContext())) {
        if (!GetExecutionContext()->IsSecureContext()) {
            GetDocument().AddConsoleMessage(
                MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kOther, mojom::blink::ConsoleMessageLevel::kError,
                    "sharedStorageWritable: sharedStorage operations are only available "
                    "in secure contexts."));
        } else if (!params.new_value.IsNull()) {
            UseCounter::Count(GetDocument(), WebFeature::kSharedStorageAPI_Image_Attribute);
        }
    } else {
...
void HTMLImageElement::InvalidateAttributeMapping()
{
    ...
    SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kPictureSourceChanged));
}

bool HTMLImageElement::SupportedImageType(const String& type, const HashSet<String>* disabled_image_types)
{
...
{
    return GetLoadingAttributeValue(FastGetAttribute(html_names::kLoadingAttr)) == LoadingAttributeValue::kLazy;
}
...
ImageCandidate HTMLImageElement::FindBestFitImageFromPictureParent()
{
    ...
    for (Node* child = picture_parent->firstChild(); child; child = child->nextSibling()) {
        if (child == this)
            return ImageCandidate();
        ...
        if (!source->FastGetAttribute(html_names::kSrcAttr).IsNull()) {
            Deprecation::CountDeprecation(GetExecutionContext(), WebFeature::kPictureSourceSrc);
        }
        ...
        ImageCandidate candidate = BestFitSourceForSrcsetAttribute(
            GetDocument().DevicePixelRatio(), SourceSize(*source), source->FastGetAttribute(html_names::kSrcsetAttr), &GetDocument());
...
{
    if (auto* content_image = DynamicTo<ImageContentData>(style.GetContentData())) {
...
Node::InsertionNotificationRequest HTMLImageElement::InsertedInto(ContainerNode& insertion_point)
{
    if (!form_was_set_by_parser_ || NodeTraversal::HighestAncestorOrSelf(insertion_point) != NodeTraversal::HighestAncestorOrSelf(*form_.Get()))
        ResetFormOwner();
    if (listener_)
        GetDocument().GetMediaQueryMatcher().AddViewportListener(listener_);
    ...
    static const bool is_lcp_script_observer_enabled = blink::LcppScriptObserverEnabled();
...
void HTMLImageElement::RemovedFrom(ContainerNode& insertion_point)
{
    if (InActiveDocument() && !last_reported_ad_rect_.IsEmpty()) {
        gfx::Rect empty_rect;
        GetDocument().GetFrame()->Client()->OnMainFrameImageAdRectangleChanged(this->GetDomNodeId(), empty_rect);
        last_reported_ad_rect_ = empty_rect;
    }

    if (!form_ || NodeTraversal::HighestAncestorOrSelf(*form_.Get()) != NodeTraversal::HighestAncestorOrSelf(*this))
        ResetFormOwner();
    if (listener_)
        GetDocument().GetMediaQueryMatcher().RemoveViewportListener(listener_);
    bool was_removed_from_parent = !parentNode();
    auto* picture_parent = DynamicTo<HTMLPictureElement>(was_removed_from_parent ? &insertion_point : parentNode());
    if (picture_parent) {
...
unsigned HTMLImageElement::width()
{
    ...
    if (!GetLayoutObject()) {
        ...
        unsigned width = 0;
        if (ParseHTMLNonNegativeInteger(FastGetAttribute(html_names::kWidthAttr), width))
            return width;
...
unsigned HTMLImageElement::height()
{
    ...
    if (!GetLayoutObject()) {
        ...
        // TODO(cbiesinger): The attribute could be a float or percentage value...
        unsigned height = 0;
        if (ParseHTMLNonNegativeInteger(FastGetAttribute(html_names::kHeightAttr), height))
            return height;
...
PhysicalSize HTMLImageElement::DensityCorrectedIntrinsicDimensions() const
{
    ...
    if (image_content->HasDevicePixelRatioHeaderValue() && image_content->DevicePixelRatioHeaderValue() > 0)
        pixel_density = 1 / image_content->DevicePixelRatioHeaderValue();

    PhysicalSize natural_size(image_content->GetImage()->Size(kRespectImageOrientation));
...
unsigned HTMLImageElement::LayoutBoxWidth() const
{
    ...
    return box ? AdjustForAbsoluteZoom::AdjustLayoutUnit(box->ContentWidth(), *box).Round() : 0;
}

unsigned HTMLImageElement::LayoutBoxHeight() const
{
    LayoutBox* box = GetLayoutBox();
    return box ? AdjustForAbsoluteZoom::AdjustLayoutUnit(box->ContentHeight(), *box).Round() : 0;
}
...
const String& HTMLImageElement::currentSrc() const
{
    ...
    ImageResourceContent* image_content = GetImageLoader().GetContent();
    if (!image_content || (!image_content->ErrorOccurred() && !image_content->HasImage()))
        return g_empty_atom;
...
{
    return attribute.GetName() == html_names::kSrcAttr || attribute.GetName() == html_names::kLowsrcAttr || attribute.GetName() == html_names::kLongdescAttr
        || (attribute.GetName() == html_names::kUsemapAttr && attribute.Value()[0] != '#') || HTMLElement::IsURLAttribute(attribute);
}

bool HTMLImageElement::HasLegalLinkAttribute(const QualifiedName& name) const
{
    return name == html_names::kSrcAttr || HTMLElement::HasLegalLinkAttribute(name);
}
...
void HTMLImageElement::DidFinishLifecycleUpdate(const LocalFrameView& local_frame_view)
{
    ...
    if (LayoutObject* r = GetLayoutObject()) {
        ...
        if (rect_in_viewport.width() > 1 && rect_in_viewport.height() > 1) {
            ...
            rect_to_report = rect_in_viewport + frame->View()->LayoutViewport()->ScrollOffsetInt();
        }
    }

    if (last_reported_ad_rect_ != rect_to_report) {
        frame->Client()->OnMainFrameImageAdRectangleChanged(this->GetDomNodeId(), rect_to_report);
...
bool HTMLImageElement::draggable() const
{
    ...
    return !EqualIgnoringASCIICase(FastGetAttribute(html_names::kDraggableAttr), "false");
}
...
int HTMLImageElement::x() const
{
    ...
    PhysicalOffset abs_pos = r->LocalToAbsolutePoint(PhysicalOffset(), kIgnoreTransforms);
...
ScriptPromise<IDLUndefined> HTMLImageElement::decode(ScriptState* script_state, ExceptionState& exception_state)
{
...
bool HTMLImageElement::IsServerMap() const
{
    ...
    return GetDocument().CompleteURL(StripLeadingAndTrailingHTMLSpaces(usemap)).IsEmpty();
}
...
gfx::SizeF HTMLImageElement::DefaultDestinationSize(const gfx::SizeF& default_object_size, const RespectImageOrientationEnum respect_orientation) const
{
    ...
    if (auto* svg_image = DynamicTo<SVGImage>(image)) {
        const SVGImageViewInfo* view_info = SVGImageForContainer::CreateViewInfo(*svg_image, *this);
        return SVGImageForContainer::ConcreteObjectSize(*svg_image, view_info, default_object_size);
    }

    PhysicalSize size(image->Size(respect_orientation));
    if (GetLayoutObject() && GetLayoutObject()->IsLayoutImage() && image->HasIntrinsicSize())
...
static SourceSizeValueResult SourceSizeValue(const Element* element, Document& current_document)
{
    ...
    if (sizes.IsNull() && img != element && img && img->AllowAutoSizes() && img->FastGetAttribute(html_names::kSizesAttr).StartsWithIgnoringASCIICase("auto")) {
        // Spec:
    ...
    SizesAttributeParser sizes_attribute_parser { MediaValuesDynamic::Create(current_document), sizes, current_document.GetExecutionContext(), img };
...
std::optional<float> HTMLImageElement::GetResourceWidth() const
{
    ...
    const SourceSizeValueResult source_size_val_res = SourceSizeValue(element ? element : this, GetDocument());
...
{
    const SourceSizeValueResult source_size_val_res = SourceSizeValue(&element, GetDocument());
...
void HTMLImageElement::SelectSourceURL(ImageLoader::UpdateFromElementBehavior behavior)
{
    ...
    if (candidate.IsEmpty()) {
        ...
        candidate = BestFitSourceForImageAttributes(
            GetDocument().DevicePixelRatio(), source_size, FastGetAttribute(html_names::kSrcAttr), FastGetAttribute(html_names::kSrcsetAttr), &GetDocument());
    }
    ...
    if (behavior != HTMLImageLoader::kUpdateSizeChanged || best_fit_image_url_ != old_url) {
...
void HTMLImageElement::StartLoadingImageDocument(ImageResourceContent* image_content)
{
...
void HTMLImageElement::EnsureFallbackForGeneratedContent()
{
    ...
    SetLayoutDisposition(LayoutDisposition::kFallbackContent, true /* force_reattach */);
}
...
void HTMLImageElement::EnsureCollapsedOrFallbackContent()
{
    ...
    ImageResourceContent* image_content = GetImageLoader().GetContent();
    std::optional<ResourceError> error = image_content ? image_content->GetResourceError() : std::nullopt;
    SetLayoutDisposition(error && error->ShouldCollapseInitiator() ? LayoutDisposition::kCollapsed : LayoutDisposition::kFallbackContent);
}
...
void HTMLImageElement::SetAutoSizesUsecounter()
{
    if (listener_ && HasLazyLoadingAttribute()) {
        UseCounter::Count(GetDocument(),
            HasSizesAttribute() ? WebFeature::kViewportDependentLazyLoadedImageWithSizesAttribute
                                : WebFeature::kViewportDependentLazyLoadedImageWithoutSizesAttribute);
    }
}

void HTMLImageElement::SetLayoutDisposition(LayoutDisposition layout_disposition, bool force_reattach)
{
    ...
    SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kUseFallback));
