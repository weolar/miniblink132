...
DocumentNameCollection::DocumentNameCollection(ContainerNode& document, CollectionType type, const AtomicString& name)
...
bool DocumentNameCollection::ElementMatches(const HTMLElement& element) const
{
    ...
    if (IsA<HTMLFormElement>(element) || IsA<HTMLIFrameElement>(element) || (html_embed_element && html_embed_element->IsExposed()))
    ...
    if (html_image_element && html_image_element->IsExposed())
        return element.GetNameAttribute() == name_ || element.GetIdAttribute() == name_;
    if (IsA<HTMLImageElement>(element)) {
        const AtomicString& name_value = element.GetNameAttribute();
        return name_value == name_ || (element.GetIdAttribute() == name_ && !name_value.empty());
    }
    return false;
}
