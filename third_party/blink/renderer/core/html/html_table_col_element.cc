...
HTMLTableColElement::HTMLTableColElement(const QualifiedName& tag_name, Document& document)
    : HTMLTablePartElement(tag_name, document)
    , span_(kDefaultColSpan)
{
}
...
void HTMLTableColElement::CollectStyleForPresentationAttribute(const QualifiedName& name, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
    if (name == html_names::kWidthAttr)
...
void HTMLTableColElement::ParseAttribute(const AttributeModificationParams& params)
{
    if (params.name == html_names::kSpanAttr) {
        unsigned new_span = 0;
        if (!ParseHTMLClampedNonNegativeInteger(params.new_value, kMinColSpan, kMaxColSpan, new_span)) {
        ...
        if (!params.new_value.empty()) {
            if (GetLayoutObject() && GetLayoutObject()->IsLayoutTableCol()) {
                ...
                if (new_width != col->Size().width) {
                    col->SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kAttributeChanged);
                }
            }
        }
    } else {
...
const CSSPropertyValueSet* HTMLTableColElement::AdditionalPresentationAttributeStyle()
{
