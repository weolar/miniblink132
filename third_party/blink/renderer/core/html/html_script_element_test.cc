...
public:
    ...
    HTMLScriptElement* MakeScript()
    {
        HTMLScriptElement* script = To<HTMLScriptElement>(
            document().body()->AppendChild(MakeGarbageCollected<HTMLScriptElement>(document(), CreateElementFlags::ByParser(&document()))));
        EXPECT_TRUE(script);
        return script;
    }
...
TEST_F(HTMLScriptElementTest, ScriptTextInternalSlotScriptParsingInterruptedByApiCall)
{
    HTMLScriptElement* script = MakeScript();
    script->ParserAppendChild(Text::Create(document(), "abc"));
