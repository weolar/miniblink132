...
HTMLEmbedElement::HTMLEmbedElement(Document& document, const CreateElementFlags flags)
    : HTMLPlugInElement(html_names::kEmbedTag, document, flags)
{
    EnsureUserAgentShadowRoot();
}

const AttrNameToTrustedType& HTMLEmbedElement::GetCheckedAttributeTypes() const
{
...
void HTMLEmbedElement::CollectStyleForPresentationAttribute(const QualifiedName& name, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
    if (name == html_names::kHiddenAttr) {
        AddPropertyToPresentationAttributeStyle(style, CSSPropertyID::kWidth, 0, CSSPrimitiveValue::UnitType::kPixels);
        AddPropertyToPresentationAttributeStyle(style, CSSPropertyID::kHeight, 0, CSSPrimitiveValue::UnitType::kPixels);
    } else {
        HTMLPlugInElement::CollectStyleForPresentationAttribute(name, value, style);
    }
}

void HTMLEmbedElement::ParseAttribute(const AttributeModificationParams& params)
{
    if (params.name == html_names::kTypeAttr) {
        ...
        if (GetLayoutObject()) {
            ...
            GetLayoutObject()->SetNeedsLayoutAndFullPaintInvalidation("Embed type changed");
        }
        ...
        if (GetLayoutObject() && IsImageType()) {
            ...
            if (!FastHasAttribute(html_names::kTypeAttr)) {
                UseCounter::Count(GetDocument(), WebFeature::kEmbedElementWithoutTypeSrcChanged);
            }
            SetNeedsPluginUpdate(true);
...
bool HTMLEmbedElement::LayoutObjectIsNeeded(const DisplayStyle& style) const
{
    ...
    // enforce layout.
    if (!RuntimeEnabledFeatures::HTMLEmbedElementNotForceLayoutEnabled() && IsImageType()) {
    ...
    if (!FastHasAttribute(html_names::kSrcAttr) && !FastHasAttribute(html_names::kTypeAttr))
        return false;
    ...
    if (auto* object = DynamicTo<HTMLObjectElement>(p)) {
        if (!object->WillUseFallbackContentAtLayout() && !object->UseFallbackContent()) {
            return false;
        }
    }
    return HTMLPlugInElement::LayoutObjectIsNeeded(style);
...
{
    return attribute.GetName() == html_names::kSrcAttr || HTMLPlugInElement::IsURLAttribute(attribute);
}
...
bool HTMLEmbedElement::IsExposed() const
{
    ...
    for (HTMLObjectElement* object = Traversal<HTMLObjectElement>::FirstAncestor(*this); object;
         object = Traversal<HTMLObjectElement>::FirstAncestor(*object)) {
        if (object->IsExposed())
            return false;
    }
    return true;
