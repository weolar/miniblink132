...
void HTMLMarqueeElement::DidAddUserAgentShadowRoot(ShadowRoot& shadow_root)
{
    ...
    style->setTextContent(":host { display: inline-block; overflow: hidden;"
...
class HTMLMarqueeElement::RequestAnimationFrameCallback final : public FrameCallback {
public:
    ...
    RequestAnimationFrameCallback& operator=(const RequestAnimationFrameCallback&) = delete;
...
Node::InsertionNotificationRequest HTMLMarqueeElement::InsertedInto(ContainerNode& insertion_point)
{
...
unsigned HTMLMarqueeElement::scrollAmount() const
{
    ...
    if (value.empty() || !ParseHTMLNonNegativeInteger(value, scroll_amount) || scroll_amount > 0x7fffffffu)
...
{
    SetUnsignedIntegralAttribute(html_names::kScrollamountAttr, value, kDefaultScrollAmount);
}
...
unsigned HTMLMarqueeElement::scrollDelay() const
{
    ...
    if (value.empty() || !ParseHTMLNonNegativeInteger(value, scroll_delay) || scroll_delay > 0x7fffffffu)
...
{
    SetUnsignedIntegralAttribute(html_names::kScrolldelayAttr, value, kDefaultScrollDelayMS);
}
...
{
    if (value <= 0 && value != -1) {
        exception_state.ThrowDOMException(DOMExceptionCode::kIndexSizeError, "The provided value (" + String::Number(value) + ") is neither positive nor -1.");
        return;
    }
...
void HTMLMarqueeElement::start()
{
    ...
    RequestAnimationFrameCallback* callback = MakeGarbageCollected<RequestAnimationFrameCallback>(this);
...
bool HTMLMarqueeElement::IsPresentationAttribute(const QualifiedName& attr) const
{
    if (attr == html_names::kBgcolorAttr || attr == html_names::kHeightAttr || attr == html_names::kHspaceAttr || attr == html_names::kVspaceAttr
...
void HTMLMarqueeElement::CollectStyleForPresentationAttribute(const QualifiedName& attr, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
...
StringKeyframeEffectModel* HTMLMarqueeElement::CreateEffectModel(const AnimationParameters& parameters)
{
    StyleSheetContents* style_sheet_contents = mover_->GetDocument().ElementSheet().Contents();
    MutableCSSPropertyValueSet::SetResult set_result;

    SecureContextMode secure_context_mode = mover_->GetExecutionContext()->GetSecureContextMode();

    StringKeyframeVector keyframes;
    auto* keyframe1 = MakeGarbageCollected<StringKeyframe>();
    set_result = keyframe1->SetCSSPropertyValue(CSSPropertyID::kTransform, parameters.transform_begin, secure_context_mode, style_sheet_contents);
    ...
    set_result = keyframe2->SetCSSPropertyValue(CSSPropertyID::kTransform, parameters.transform_end, secure_context_mode, style_sheet_contents);
    DCHECK(set_result != MutableCSSPropertyValueSet::kParseError);
    keyframes.push_back(keyframe2);

    return MakeGarbageCollected<StringKeyframeEffectModel>(keyframes, EffectModel::kCompositeReplace, LinearTimingFunction::Shared());
}
...
void HTMLMarqueeElement::ContinueAnimation()
{
    ...
    if (player_ && player_->CalculateAnimationPlayState() == V8AnimationPlayState::Enum::kPaused) {
        player_->play();
        return;
    }
    ...
    if (scroll_delay < kMinimumScrollDelayMS && !FastHasAttribute(html_names::kTruespeedAttr))
    ...
    effect_timing->setDuration(MakeGarbageCollected<V8UnionCSSNumericValueOrStringOrUnrestrictedDouble>(duration));
    TimingInput::Update(timing, effect_timing, nullptr, ASSERT_NO_EXCEPTION);

    auto* keyframe_effect = MakeGarbageCollected<KeyframeEffect>(mover_, effect_model, timing);
...
{
    Metrics metrics;
    CSSStyleDeclaration* marquee_style = GetDocument().domWindow()->getComputedStyle(this);
    ...
    // inline marquees.
    if (marquee_style->getPropertyValue("width") == "auto" && marquee_style->getPropertyValue("height") == "auto") {
    ...
    if (IsHorizontal()) {
        mover_->style()->setProperty(GetExecutionContext(), "width", "-webkit-max-content", "important", ASSERT_NO_EXCEPTION);
    } else {
        mover_->style()->setProperty(GetExecutionContext(), "height", "-webkit-max-content", "important", ASSERT_NO_EXCEPTION);
    }
    CSSStyleDeclaration* mover_style = GetDocument().domWindow()->getComputedStyle(mover_);
...
HTMLMarqueeElement::AnimationParameters HTMLMarqueeElement::GetAnimationParameters()
{
    ...
    case kAlternate:
        switch (GetDirection()) {
        case kRight:
            parameters.transform_begin = CreateTransform(inner_width >= 0 ? 0 : inner_width);
            parameters.transform_end = CreateTransform(inner_width >= 0 ? inner_width : 0);
            parameters.distance = std::abs(inner_width);
            break;
        case kUp:
            parameters.transform_begin = CreateTransform(inner_height >= 0 ? inner_height : 0);
            parameters.transform_end = CreateTransform(inner_height >= 0 ? 0 : inner_height);
            parameters.distance = std::abs(inner_height);
            break;
        case kDown:
            parameters.transform_begin = CreateTransform(inner_height >= 0 ? 0 : inner_height);
            parameters.transform_end = CreateTransform(inner_height >= 0 ? inner_height : 0);
            parameters.distance = std::abs(inner_height);
            break;
        case kLeft:
        default:
            parameters.transform_begin = CreateTransform(inner_width >= 0 ? inner_width : 0);
            parameters.transform_end = CreateTransform(inner_width >= 0 ? 0 : inner_width);
            parameters.distance = std::abs(inner_width);
        }
