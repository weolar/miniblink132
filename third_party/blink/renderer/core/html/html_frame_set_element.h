...
class HTMLFrameSetElement final : public HTMLElement, public WindowEventHandlers {
...
public:
    ...
    bool IsHTMLFrameSetElement() const override
    {
        return true;
    }
    ...
    const Vector<HTMLDimension>& RowLengths() const
    {
        return row_lengths_;
    }
    const Vector<HTMLDimension>& ColLengths() const
    {
        return col_lengths_;
    }
    const Vector<int>& RowDeltas() const
    {
        return resize_rows_.deltas_;
    }
    const Vector<int>& ColDeltas() const
    {
        return resize_cols_.deltas_;
    }
...
private:
    ...
    public:
        ...
        bool IsResizingSplit() const
        {
            return split_being_resized_ != kNoSplit;
        }
        // Returns true if a split is being resized now.
    ...
    void StartResizing(const Vector<LayoutUnit>& sizes, int position, ResizeAxis& resize_axis);
    void ContinueResizing(const Vector<LayoutUnit>& sizes, int position, ResizeAxis& resize_axis);
    ...
    void FillFromEdgeInfo(const FrameEdgeInfo& edge_info, wtf_size_t r, wtf_size_t c);
