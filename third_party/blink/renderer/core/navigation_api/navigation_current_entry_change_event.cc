...
NavigationCurrentEntryChangeEvent::NavigationCurrentEntryChangeEvent(const AtomicString& type, NavigationCurrentEntryChangeEventInit* init)
    : Event(type, init)
    , from_(init->from())
{
...
std::optional<V8NavigationType> NavigationCurrentEntryChangeEvent::navigationType()
{
    if (!navigation_type_) {
        return std::nullopt;
    }
