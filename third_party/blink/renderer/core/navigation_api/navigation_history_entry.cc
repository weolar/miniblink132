...
NavigationHistoryEntry::NavigationHistoryEntry(
    LocalDOMWindow* window, const String& key, const String& id, const KURL& url, int64_t document_sequence_number, scoped_refptr<SerializedScriptValue> state)
...
{
    return MakeGarbageCollected<NavigationHistoryEntry>(window, key_, id_, url_, document_sequence_number_, state_.get());
}
...
bool NavigationHistoryEntry::sameDocument() const
{
    if (!DomWindow())
        return false;
    HistoryItem* current_item = DomWindow()->document()->Loader()->GetHistoryItem();
...
void NavigationHistoryEntry::SetAndSaveState(scoped_refptr<SerializedScriptValue> state)
{
    CHECK_EQ(this, DomWindow()->navigation()->currentEntry());
    state_ = state;
    DomWindow()->document()->Loader()->GetHistoryItem()->SetNavigationApiState(state_.get());
