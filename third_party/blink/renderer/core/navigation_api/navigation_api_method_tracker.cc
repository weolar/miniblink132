...
NavigationApiMethodTracker::NavigationApiMethodTracker(
    ScriptState* script_state, NavigationOptions* options, const String& key, scoped_refptr<SerializedScriptValue> state)
    : serialized_state_(std::move(state))
    , info_(options->getInfoOr(ScriptValue(script_state->GetIsolate(), v8::Undefined(script_state->GetIsolate()))))
    , key_(key)
    , committed_resolver_(MakeGarbageCollected<ScriptPromiseResolver<NavigationHistoryEntry>>(script_state))
    , finished_resolver_(MakeGarbageCollected<ScriptPromiseResolver<NavigationHistoryEntry>>(script_state))
...
void NavigationApiMethodTracker::NotifyAboutTheCommittedToEntry(NavigationHistoryEntry* entry, WebFrameLoadType type)
{
...
void NavigationApiMethodTracker::RejectFinishedPromise(const ScriptValue& value)
{
