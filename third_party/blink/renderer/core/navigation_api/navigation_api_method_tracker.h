...
class NavigationApiMethodTracker final : public GarbageCollected<NavigationApiMethodTracker> {
public:
    NavigationApiMethodTracker(ScriptState*, NavigationOptions*, const String& key, scoped_refptr<SerializedScriptValue> state = nullptr);

    void NotifyAboutTheCommittedToEntry(NavigationHistoryEntry*, WebFrameLoadType);
    ...
    NavigationResult* GetNavigationResult() const
    {
        return result_.Get();
    }

    const ScriptValue& GetInfo() const
    {
        return info_;
    }
