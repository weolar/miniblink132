...
public:
    ...
    void InitializeForNewWindow(HistoryItem& current, WebFrameLoadType, CommitReason, NavigationApi* previous, const WebVector<WebHistoryItem>& back_entries,
        const WebVector<WebHistoryItem>& forward_entries, HistoryItem* previous_entry);
    void UpdateForNavigation(HistoryItem&, WebFrameLoadType);
    void SetEntriesForRestore(const mojom::blink::NavigationApiHistoryEntryArraysPtr&, mojom::blink::NavigationApiEntryRestoreReason);
    ...
    void updateCurrentEntry(NavigationUpdateCurrentEntryOptions*, ExceptionState&);
    ...
    NavigationResult* navigate(ScriptState*, const String& url, NavigationNavigateOptions*);
    NavigationResult* reload(ScriptState*, NavigationReloadOptions*);

    NavigationResult* traverseTo(ScriptState*, const String& key, NavigationOptions*);
    ...
    //   argument.
    void InformAboutCanceledNavigation(CancelNavigationReason reason = CancelNavigationReason::kOther);

    // Called when a traverse is cancelled before its navigate event is fired.
    void TraverseCancelled(const String& key, mojom::blink::TraverseCancelledReason reason);

    int GetIndexFor(NavigationHistoryEntry*);
    NavigationHistoryEntry* GetExistingEntryFor(const String& key, const String& id);
    ...
    void RemovedEventListener(const AtomicString&, const RegisteredEventListener&) final;

    void Trace(Visitor*) const final;

private:
    friend class NavigateEvent;
    NavigationHistoryEntry* GetEntryForRestore(const mojom::blink::NavigationApiHistoryEntryPtr&);
    void PopulateKeySet();
    ...
    NavigationResult* PerformNonTraverseNavigation(ScriptState*, FrameLoadRequest&, scoped_refptr<SerializedScriptValue>, NavigationOptions*, WebFrameLoadType);

    DOMException* PerformSharedNavigationChecks(const String& method_name_for_error_message);

    void PromoteUpcomingNavigationToOngoing(const String& key);

    scoped_refptr<SerializedScriptValue> SerializeState(const ScriptValue&, ExceptionState&);
    ...
    HeapHashMap<String, Member<NavigationApiMethodTracker>> upcoming_traverse_api_method_trackers_;
