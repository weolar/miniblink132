...
class NavigateEvent::FulfillReaction final : public ThenCallable<IDLUndefined, FulfillReaction> {
public:
...
class NavigateEvent::RejectReaction final : public ThenCallable<IDLAny, RejectReaction> {
public:
...
NavigateEvent::NavigateEvent(ExecutionContext* context, const AtomicString& type, NavigateEventInit* init, AbortController* controller)
    : Event(type, init)
    ...
    , info_(init->hasInfo() ? init->info() : ScriptValue(context->GetIsolate(), v8::Undefined(context->GetIsolate())))
...
bool NavigateEvent::PerformSharedChecks(const String& function_name, ExceptionState& exception_state)
{
    if (!DomWindow()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, function_name + "() may not be called in a detached window.");
        return false;
    }
    if (!isTrusted()) {
        exception_state.ThrowSecurityError(function_name + "() may only be called on a trusted event.");
        return false;
    }
    if (defaultPrevented()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, function_name + "() may not be called if the event has been canceled.");
        return false;
    }
    return true;
}

void NavigateEvent::intercept(NavigationInterceptOptions* options, ExceptionState& exception_state)
{
    ...
    if (!can_intercept_) {
        exception_state.ThrowSecurityError("A navigation with URL '" + dispatch_params_->url.ElidedString()
            + "' cannot be intercepted by in a window with origin '" + DomWindow()->GetSecurityOrigin()->ToString() + "' and URL '"
    ...
    if (RuntimeEnabledFeatures::NavigateEventCommitBehaviorEnabled() && !cancelable() && options->hasCommit()
        && options->commit().AsEnum() == V8NavigationCommitBehavior::Enum::kAfterTransition) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError,
    ...
    if (options->hasFocusReset()) {
        if (focus_reset_behavior_ && focus_reset_behavior_->AsEnum() != options->focusReset().AsEnum()) {
    ...
    if (options->hasScroll()) {
        if (scroll_behavior_ && scroll_behavior_->AsEnum() != options->scroll().AsEnum()) {
            GetExecutionContext()->AddConsoleMessage(
                MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kJavaScript, mojom::blink::ConsoleMessageLevel::kWarning,
    ...
    if (RuntimeEnabledFeatures::NavigateEventCommitBehaviorEnabled()) {
        if (options->hasCommit()) {
            if (commit_behavior_ && commit_behavior_->AsEnum() != options->commit().AsEnum()) {
                GetExecutionContext()->AddConsoleMessage(
                    MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kJavaScript, mojom::blink::ConsoleMessageLevel::kWarning,
    ...
    CHECK(intercept_state_ == InterceptState::kNone || intercept_state_ == InterceptState::kIntercepted);
...
void NavigateEvent::commit(ExceptionState& exception_state)
{
    ...
    if (intercept_state_ == InterceptState::kNone) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "intercept() must be called before commit().");
        return;
    }
    ...
    if (IsBeingDispatched()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "commit() may not be called during event dispatch");
        return;
    }
    if (intercept_state_ == InterceptState::kFinished) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "commit() may not be called after transition completes.");
        return;
    }
    if (intercept_state_ == InterceptState::kCommitted || intercept_state_ == InterceptState::kScrolled) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "commit() already called.");
        return;
    }
    CommitNow();
}
...
{
    delayed_load_start_task_handle_ = PostDelayedCancellableTask(*DomWindow()->GetTaskRunner(TaskType::kInternalLoading), FROM_HERE,
        WTF::BindOnce(&NavigateEvent::DelayedLoadStartTimerFired, WrapWeakPersistent(this)), kDelayLoadStart);
...
{
    return !commit_behavior_ || commit_behavior_->AsEnum() == V8NavigationCommitBehavior::Enum::kImmediate;
}
...
void NavigateEvent::CommitNow()
{
    ...
    auto* state_object = dispatch_params_->destination_item ? dispatch_params_->destination_item->StateObject() : dispatch_params_->state_object.get();
    ...
    DomWindow()->document()->Loader()->RunURLAndHistoryUpdateSteps(dispatch_params_->url, dispatch_params_->destination_item,
        mojom::blink::SameDocumentNavigationType::kNavigationApiIntercept, state_object, dispatch_params_->frame_load_type,
        dispatch_params_->event_type == NavigateEventType::kHistoryApi ? FirePopstate::kNo : FirePopstate::kYes, dispatch_params_->is_browser_initiated,
        dispatch_params_->is_synchronously_committed_same_document, dispatch_params_->soft_navigation_heuristics_task_id);
}
...
void NavigateEvent::React(ScriptState* script_state)
{
    ...
    if (navigation_action_promises_list_.empty()) {
        ...
        navigation_action_promises_list_.push_back(ToResolvedUndefinedPromise(script_state));
    }

    auto promise = PromiseAll<IDLUndefined>::Create(script_state, navigation_action_promises_list_);
    promise.React(script_state, MakeGarbageCollected<FulfillReaction>(this), MakeGarbageCollected<RejectReaction>(this));

    if (HasNavigationActions() && DomWindow()) {
        if (AXObjectCache* cache = DomWindow()->document()->ExistingAXObjectCache()) {
...
void NavigateEvent::ReactDone(ScriptValue value, bool did_fulfill)
{
    ...
    if (HasNavigationActions()) {
        if (LocalFrame* frame = window->GetFrame()) {
            frame->Loader().DidFinishNavigation(did_fulfill ? FrameLoader::NavigationFinishState::kSuccess : FrameLoader::NavigationFinishState::kFailure);
        }
...
{
    CHECK(intercept_state_ == InterceptState::kCommitted || intercept_state_ == InterceptState::kScrolled);
    ...
    if (focus_reset_behavior_ && focus_reset_behavior_->AsEnum() != V8NavigationFocusReset::Enum::kAfterTransition) {
        return;
    }
...
void NavigateEvent::scroll(ExceptionState& exception_state)
{
    ...
    if (intercept_state_ == InterceptState::kFinished) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "scroll() may not be called after transition completes");
        return;
    }
    if (intercept_state_ == InterceptState::kScrolled) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "scroll() already called");
        return;
    }
    if (intercept_state_ == InterceptState::kNone) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "intercept() must be called before scroll()");
        return;
    }
    if (intercept_state_ == InterceptState::kIntercepted) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "scroll() may not be called before commit.");
        return;
    }
...
void NavigateEvent::PotentiallyProcessScrollBehavior()
{
    ...
    if (scroll_behavior_ && scroll_behavior_->AsEnum() == V8NavigationScrollBehavior::Enum::kManual) {
        return;
    }
    ProcessScrollBehavior();
}

WebFrameLoadType LoadTypeFromNavigation(V8NavigationType::Enum navigation_type)
{
...
void NavigateEvent::ProcessScrollBehavior()
{
    ...
    std::optional<HistoryItem::ViewState> view_state = dispatch_params_->destination_item ? dispatch_params_->destination_item->GetViewState() : std::nullopt;
    auto scroll_behavior = has_ua_visual_transition_ ? mojom::blink::ScrollBehavior::kInstant : mojom::blink::ScrollBehavior::kAuto;
    ...
    DomWindow()->GetFrame()->Loader().ProcessScrollForSameDocumentNavigation(
        dispatch_params_->url, LoadTypeFromNavigation(navigation_type_), view_state, mojom::blink::ScrollRestorationType::kAuto, scroll_behavior);
}
