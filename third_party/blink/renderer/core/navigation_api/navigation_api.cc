...
template <typename... DOMExceptionArgs> NavigationResult* EarlyErrorResult(ScriptState* script_state, DOMExceptionArgs&&... args)
{
    auto* ex = MakeGarbageCollected<DOMException>(std::forward<DOMExceptionArgs>(args)...);
    return EarlyErrorResult(script_state, ex);
}

NavigationResult* EarlyErrorResult(ScriptState* script_state, DOMException* ex)
{
    auto* result = NavigationResult::Create();
    result->setCommitted(ScriptPromise<NavigationHistoryEntry>::RejectWithDOMException(script_state, ex));
    result->setFinished(ScriptPromise<NavigationHistoryEntry>::RejectWithDOMException(script_state, ex));
    return result;
}

NavigationResult* EarlyErrorResult(ScriptState* script_state, v8::Local<v8::Value> ex)
{
    auto* result = NavigationResult::Create();
    result->setCommitted(ScriptPromise<NavigationHistoryEntry>::Reject(script_state, ex));
    result->setFinished(ScriptPromise<NavigationHistoryEntry>::Reject(script_state, ex));
    return result;
}

NavigationResult* EarlySuccessResult(ScriptState* script_state, NavigationHistoryEntry* entry)
{
    auto* result = NavigationResult::Create();
    result->setCommitted(ToResolvedPromise<NavigationHistoryEntry>(script_state, entry));
    result->setFinished(ToResolvedPromise<NavigationHistoryEntry>(script_state, entry));
    return result;
}
...
void NavigationApi::UpdateActivation(HistoryItem* previous_item, WebFrameLoadType load_type)
{
    NavigationHistoryEntry* previous_history_entry = nullptr;
    if (previous_item) {
        if (auto* entry = GetExistingEntryFor(previous_item->GetNavigationApiKey(), previous_item->GetNavigationApiId())) {
    ...
    V8NavigationType::Enum navigation_type
        = window_->GetFrame()->GetPage()->IsPrerendering() ? V8NavigationType::Enum::kPush : DetermineNavigationType(load_type);
    activation_->Update(currentEntry(), previous_history_entry, navigation_type);
}

NavigationHistoryEntry* NavigationApi::GetExistingEntryFor(const String& key, const String& id)
{
...
void NavigationApi::InitializeForNewWindow(HistoryItem& current, WebFrameLoadType load_type, CommitReason commit_reason, NavigationApi* previous,
    const WebVector<WebHistoryItem>& back_entries, const WebVector<WebHistoryItem>& forward_entries, HistoryItem* previous_entry)
{
    ...
    // navigations.
    if (commit_reason != CommitReason::kRegular || (current.Url() == BlankURL() && !IsBackForwardLoadType(load_type))
        || (current.Url().IsAboutSrcdocURL() && !IsBackForwardLoadType(load_type))) {
        if (previous && !previous->entries_.empty() && window_->GetSecurityOrigin()->IsSameOriginWith(previous->window_->GetSecurityOrigin())) {
            CHECK(entries_.empty());
    ...
    entries_.reserve(base::checked_cast<wtf_size_t>(back_entries.size() + forward_entries.size() + 1));
...
void NavigationApi::UpdateCurrentEntryForTesting(HistoryItem& item)
{
    ...
    keys_to_indices_.insert(entries_[current_entry_index_]->key(), current_entry_index_);
}

void NavigationApi::UpdateForNavigation(HistoryItem& item, WebFrameLoadType type)
{
    ...
    if (type == WebFrameLoadType::kStandard || type == WebFrameLoadType::kReplaceCurrentItem) {
    ...
    if (ongoing_api_method_tracker_) {
        ongoing_api_method_tracker_->NotifyAboutTheCommittedToEntry(entries_[current_entry_index_], type);
    }
    ...
    v8::MicrotasksScope scope(window_->GetIsolate(), ToMicrotaskQueue(window_), v8::MicrotasksScope::kRunMicrotasks);
    ...
    DispatchEvent(*NavigationCurrentEntryChangeEvent::Create(event_type_names::kCurrententrychange, init));
...
NavigationHistoryEntry* NavigationApi::GetEntryForRestore(const mojom::blink::NavigationApiHistoryEntryPtr& entry)
{
    if (!entry) {
        return nullptr;
    }
    ...
    return MakeGarbageCollected<NavigationHistoryEntry>(window_, entry->key, entry->id, KURL(entry->url), entry->document_sequence_number,
        entry->state ? SerializedScriptValue::Create(entry->state) : nullptr);
}

// static
void FireDisposeEventsAsync(HeapVector<Member<NavigationHistoryEntry>>* disposed_entries)
{
...
void NavigationApi::SetEntriesForRestore(
    const mojom::blink::NavigationApiHistoryEntryArraysPtr& entry_arrays, mojom::blink::NavigationApiEntryRestoreReason restore_reason)
{
    ...
    new_entries.reserve(base::checked_cast<wtf_size_t>(entry_arrays->back_entries.size() + entry_arrays->forward_entries.size() + 1));
    ...
    current_entry_index_ = base::checked_cast<wtf_size_t>(entry_arrays->back_entries.size());
    ...
    case mojom::blink::NavigationApiEntryRestoreReason::kBFCache:
        ...
        break;
    case mojom::blink::NavigationApiEntryRestoreReason::kPrerenderActivationPush:
        navigation_type = V8NavigationType::Enum::kPush;
        break;
    case mojom::blink::NavigationApiEntryRestoreReason::kPrerenderActivationReplace:
        navigation_type = V8NavigationType::Enum::kReplace;
        break;
    default:
        NOTREACHED();
    }
    activation_->Update(currentEntry(), GetEntryForRestore(entry_arrays->previous_entry), navigation_type);

    // |new_entries| now contains the previous entries_. Find the ones that are no
    // longer in entries_ so they can be disposed.
    HeapVector<Member<NavigationHistoryEntry>>* disposed_entries = MakeGarbageCollected<HeapVector<Member<NavigationHistoryEntry>>>();
    ...
    window_->GetTaskRunner(TaskType::kInternalDefault)->PostTask(FROM_HERE, WTF::BindOnce(&FireDisposeEventsAsync, WrapPersistent(disposed_entries)));
}

void NavigationApi::DisposeEntriesForSessionHistoryRemoval(const Vector<String>& keys)
{
    if (HasEntriesAndEventsDisabled())
        return;
...
NavigationHistoryEntry* NavigationApi::currentEntry() const
{
    ...
    return !HasEntriesAndEventsDisabled() && current_entry_index_ >= 0 ? entries_[current_entry_index_] : nullptr;
}

HeapVector<Member<NavigationHistoryEntry>> NavigationApi::entries()
{
    return HasEntriesAndEventsDisabled() ? HeapVector<Member<NavigationHistoryEntry>>() : entries_;
}

void NavigationApi::updateCurrentEntry(NavigationUpdateCurrentEntryOptions* options, ExceptionState& exception_state)
{
    NavigationHistoryEntry* current_entry = currentEntry();

    if (!current_entry) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "updateCurrent() cannot be called when navigation.current is null.");
        return;
    }

    scoped_refptr<SerializedScriptValue> serialized_state = SerializeState(options->state(), exception_state);
...
NavigationResult* NavigationApi::navigate(ScriptState* script_state, const String& url, NavigationNavigateOptions* options)
{
    KURL completed_url = KURL(window_->BaseURL(), url);
    if (!completed_url.IsValid()) {
        return EarlyErrorResult(script_state, DOMExceptionCode::kSyntaxError, "Invalid URL '" + completed_url.GetString() + "'.");
    }
    ...
    scoped_refptr<SerializedScriptValue> serialized_state = nullptr;
    {
        if (options->hasState()) {
            ...
            serialized_state = SerializeState(options->state(), PassThroughException(script_state->GetIsolate()));
    ...
    WebFrameLoadType frame_load_type
        = options->history() == V8NavigationHistoryBehavior::Enum::kReplace ? WebFrameLoadType::kReplaceCurrentItem : WebFrameLoadType::kStandard;

    return PerformNonTraverseNavigation(script_state, request, std::move(serialized_state), options, frame_load_type);
}

NavigationResult* NavigationApi::reload(ScriptState* script_state, NavigationReloadOptions* options)
{
    scoped_refptr<SerializedScriptValue> serialized_state = nullptr;
    {
        if (options->hasState()) {
    ...
    return PerformNonTraverseNavigation(script_state, request, std::move(serialized_state), options, WebFrameLoadType::kReload);
}

NavigationResult* NavigationApi::PerformNonTraverseNavigation(ScriptState* script_state, FrameLoadRequest& request,
    scoped_refptr<SerializedScriptValue> serialized_state, NavigationOptions* options, WebFrameLoadType frame_load_type)
{
    CHECK(frame_load_type == WebFrameLoadType::kReplaceCurrentItem || frame_load_type == WebFrameLoadType::kReload
        || frame_load_type == WebFrameLoadType::kStandard);

    String method_name_for_error_message(frame_load_type == WebFrameLoadType::kReload ? "reload()" : "navigate()");
    if (DOMException* maybe_ex = PerformSharedNavigationChecks(method_name_for_error_message))
        return EarlyErrorResult(script_state, maybe_ex);

    NavigationApiMethodTracker* api_method_tracker
        = MakeGarbageCollected<NavigationApiMethodTracker>(script_state, options, String(), std::move(serialized_state));
    if (HasEntriesAndEventsDisabled()) {
    ...
    if (upcoming_non_traverse_api_method_tracker_ == api_method_tracker) {
        ...
        return EarlyErrorResult(script_state, DOMExceptionCode::kAbortError, "Navigation was aborted");
    }
    return api_method_tracker->GetNavigationResult();
}

NavigationResult* NavigationApi::traverseTo(ScriptState* script_state, const String& key, NavigationOptions* options)
{
    if (DOMException* maybe_ex = PerformSharedNavigationChecks("traverseTo()/back()/forward()")) {
        return EarlyErrorResult(script_state, maybe_ex);
    }

    if (!keys_to_indices_.Contains(key)) {
        return EarlyErrorResult(script_state, DOMExceptionCode::kInvalidStateError, "Invalid key");
    }
    if (key == currentEntry()->key()) {
        return EarlySuccessResult(script_state, currentEntry());
    }

    auto previous_api_method_tracker = upcoming_traverse_api_method_trackers_.find(key);
    if (previous_api_method_tracker != upcoming_traverse_api_method_trackers_.end()) {
        return previous_api_method_tracker->value->GetNavigationResult();
    }

    NavigationApiMethodTracker* api_method_tracker = MakeGarbageCollected<NavigationApiMethodTracker>(script_state, options, key);
    ...
    if (script_state->World().IsMainWorld() && frame->IsOutermostMainFrame()) {
        if (SoftNavigationHeuristics* heuristics = SoftNavigationHeuristics::From(*window_)) {
            soft_navigation_task_id = heuristics->AsyncSameDocumentNavigationStarted();
        }
    }
    frame->GetLocalFrameHostRemote().NavigateToNavigationApiKey(key, LocalFrame::HasTransientUserActivation(frame), soft_navigation_task_id);
    return api_method_tracker->GetNavigationResult();
}
...
{
    return !HasEntriesAndEventsDisabled() && current_entry_index_ != -1 && static_cast<size_t>(current_entry_index_) < entries_.size() - 1;
}

NavigationResult* NavigationApi::back(ScriptState* script_state, NavigationOptions* options)
{
    if (!canGoBack()) {
        return EarlyErrorResult(script_state, DOMExceptionCode::kInvalidStateError, "Cannot go back");
    }
    return traverseTo(script_state, entries_[current_entry_index_ - 1]->key(), options);
}

NavigationResult* NavigationApi::forward(ScriptState* script_state, NavigationOptions* options)
{
    if (!canGoForward()) {
        return EarlyErrorResult(script_state, DOMExceptionCode::kInvalidStateError, "Cannot go forward");
    }
    return traverseTo(script_state, entries_[current_entry_index_ + 1]->key(), options);
}

DOMException* NavigationApi::PerformSharedNavigationChecks(const String& method_name_for_error_message)
{
    if (!window_->GetFrame()) {
        return MakeGarbageCollected<DOMException>(
            DOMExceptionCode::kInvalidStateError, method_name_for_error_message + " cannot be called when the Window is detached.");
    }
    if (window_->document()->PageDismissalEventBeingDispatched()) {
        return MakeGarbageCollected<DOMException>(
            DOMExceptionCode::kInvalidStateError, method_name_for_error_message + " cannot be called during unload or beforeunload.");
    }
    return nullptr;
}

scoped_refptr<SerializedScriptValue> NavigationApi::SerializeState(const ScriptValue& value, ExceptionState& exception_state)
{
    return SerializedScriptValue::Serialize(
        window_->GetIsolate(), value.V8Value(), SerializedScriptValue::SerializeOptions(SerializedScriptValue::kForStorage), exception_state);
}
...
void NavigationApi::PromoteUpcomingNavigationToOngoing(const String& key)
{
    ...
    if (!key.IsNull()) {
        ...
        ongoing_api_method_tracker_ = upcoming_non_traverse_api_method_tracker_.Release();
    }
}
...
bool NavigationApi::HasEntriesAndEventsDisabled() const
{
    ...
    return !window_->GetFrame() || window_->IsContextDestroyed() || !window_->GetFrame()->Loader().HasLoadedNonInitialEmptyDocument()
...
NavigationHistoryEntry* NavigationApi::MakeEntryFromItem(HistoryItem& item)
{
    return MakeGarbageCollected<NavigationHistoryEntry>(
        window_, item.GetNavigationApiKey(), item.GetNavigationApiId(), item.Url(), item.DocumentSequenceNumber(), item.GetNavigationApiState());
}

NavigationApi::DispatchResult NavigationApi::DispatchNavigateEvent(NavigateEventDispatchParams* params)
{
    ...
    const String& key = params->destination_item ? params->destination_item->GetNavigationApiKey() : String();
    if (IsBackForwardOrRestore(params->frame_load_type) && params->event_type == NavigateEventType::kFragment && !keys_to_indices_.Contains(key)) {
    ...
    V8NavigationType::Enum navigation_type = DetermineNavigationType(params->frame_load_type);
    ...
    NavigationDestination* destination
        = MakeGarbageCollected<NavigationDestination>(params->url, params->event_type != NavigateEventType::kCrossDocument, destination_state);
    ...
    bool should_allow_traversal_cancellation = IsBackForwardOrRestore(params->frame_load_type) && params->event_type != NavigateEventType::kCrossDocument
        && frame->IsMainFrame() && (!params->is_browser_initiated || frame->IsHistoryUserActivationActive());
    init->setCancelable(!IsBackForwardOrRestore(params->frame_load_type) || should_allow_traversal_cancellation);
    init->setCanIntercept(CanChangeToUrlForHistoryApi(params->url, window_->GetSecurityOrigin(), window_->Url())
        && (params->event_type != NavigateEventType::kCrossDocument || !IsBackForwardOrRestore(params->frame_load_type)));
    init->setHashChange(
        params->event_type == NavigateEventType::kFragment && params->url != window_->Url() && EqualIgnoringFragmentIdentifier(params->url, window_->Url()));

    init->setUserInitiated(params->involvement != UserNavigationInvolvement::kNone);
    if (params->source_element) {
        HTMLFormElement* form = DynamicTo<HTMLFormElement>(params->source_element.Get());
        if (!form) {
            if (auto* control = DynamicTo<HTMLFormControlElement>(params->source_element.Get())) {
    ...
    if (params->source_element && params->source_element->GetExecutionContext() == window_) {
        init->setSourceElement(params->source_element);
    }
    init->setHasUAVisualTransition(params->has_ua_visual_transition);

    auto* navigate_event = NavigateEvent::Create(window_, event_type_names::kNavigate, init, controller);
    navigate_event->SetDispatchParams(params);

    std::optional<SoftNavigationHeuristics::EventScope> soft_navigation_scope;
    if (params->frame_load_type != WebFrameLoadType::kReplaceCurrentItem && init->userInitiated() && !init->downloadRequest() && init->canIntercept()) {
        if (auto* heuristics = SoftNavigationHeuristics::From(*window_)) {
            ...
            soft_navigation_scope = heuristics->MaybeCreateEventScopeForEvent(*navigate_event);
        }
    }
    ...
    if (navigate_event->defaultPrevented()) {
        if (IsBackForwardOrRestore(params->frame_load_type) && window_->GetFrame()) {
    ...
    if (navigate_event->HasNavigationActions()) {
        transition_ = MakeGarbageCollected<NavigationTransition>(window_, navigation_type, currentEntry());
        navigate_event->MaybeCommitImmediately(script_state);
    }

    if (navigate_event->HasNavigationActions() || params->event_type != NavigateEventType::kCrossDocument) {
    ...
    return navigate_event->HasNavigationActions() ? DispatchResult::kIntercept : DispatchResult::kContinue;
}

void NavigationApi::InformAboutCanceledNavigation(CancelNavigationReason reason)
{
    if (auto* tracker = scheduler::TaskAttributionTracker::From(window_->GetIsolate()); tracker && reason != CancelNavigationReason::kNavigateEvent) {
    ...
    if (!upcoming_traverse_api_method_trackers_.empty() && window_->GetFrame() && !window_->GetFrame()->IsAttached()) {
        ...
        for (auto& traversal : traversals) {
            TraverseCancelled(traversal->GetKey(), mojom::blink::TraverseCancelledReason::kAbortedBeforeCommit);
        }
        CHECK(upcoming_traverse_api_method_trackers_.empty());
    }
}

void NavigationApi::TraverseCancelled(const String& key, mojom::blink::TraverseCancelledReason reason)
{
    ...
    if (reason == mojom::blink::TraverseCancelledReason::kNotFound) {
        exception = MakeGarbageCollected<DOMException>(DOMExceptionCode::kInvalidStateError, "Invalid key");
    } else if (reason == mojom::blink::TraverseCancelledReason::kSandboxViolation) {
        exception = MakeGarbageCollected<DOMException>(DOMExceptionCode::kSecurityError,
            "Navigating to key " + key
                + " would require a navigation that "
                  "violates this frame's sandbox policy");
    } else if (reason == mojom::blink::TraverseCancelledReason::kAbortedBeforeCommit) {
        exception = MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, "Navigation was aborted");
    }
    CHECK(exception);
    traversal->value->RejectFinishedPromise(ScriptValue::From(script_state, exception));
    upcoming_traverse_api_method_trackers_.erase(traversal);
}

bool NavigationApi::HasNonDroppedOngoingNavigation() const
{
    bool has_ongoing_intercept = ongoing_navigate_event_ && ongoing_navigate_event_->HasNavigationActions();
...
void NavigationApi::DidFailOngoingNavigation(ScriptValue value)
{
    ...
    v8::Local<v8::Message> message = v8::Exception::CreateMessage(isolate, value.V8Value());
    std::unique_ptr<SourceLocation> location = blink::CaptureSourceLocation(isolate, message, window_);
    ErrorEvent* event
        = ErrorEvent::Create(ToCoreStringWithNullCheck(isolate, message->Get()), std::move(location), value, &DOMWrapperWorld::MainWorld(isolate));
...
{
    CHECK(ongoing_navigate_event_);
    ScriptValue error = ScriptValue::From(script_state, MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, "Navigation was aborted"));
...
void NavigationApi::AddedEventListener(const AtomicString& event_type, RegisteredEventListener& registered_listener)
{
...
void NavigationApi::RemovedEventListener(const AtomicString& event_type, const RegisteredEventListener& registered_listener)
{
