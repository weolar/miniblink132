...
public:
    NavigationDestination(const KURL& url, bool same_document, SerializedScriptValue* state)
        : url_(url)
    ...
    void SetDestinationEntry(NavigationHistoryEntry* entry)
    {
        entry_ = entry;
    }
    ...
    String key() const
    {
        return entry_ ? entry_->key() : String();
    }
    String id() const
    {
        return entry_ ? entry_->id() : String();
    }
    ...
    ScriptValue getState(ScriptState* script_state)
    {
        ...
        return state_ ? ScriptValue(isolate, state_->Deserialize(isolate)) : ScriptValue();
    }
