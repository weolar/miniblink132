...
ThreadedMessagingProxyBase::ThreadedMessagingProxyBase(
    ...
    , parent_execution_context_task_runners_(execution_context ? ParentExecutionContextTaskRunners::Create(*execution_context) : nullptr)
    , parent_agent_group_task_runner_(parent_agent_group_task_runner)
    , terminate_sync_load_event_(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED)
{
...
void ThreadedMessagingProxyBase::InitializeWorkerThread(std::unique_ptr<GlobalScopeCreationParams> global_scope_creation_params,
    const std::optional<WorkerBackingThreadStartupData>& thread_startup_data, const std::optional<const blink::DedicatedWorkerToken>& token,
    std::unique_ptr<WorkerDevToolsParams> client_provided_devtools_params)
{
    ...
    if (global_scope_creation_params->web_worker_fetch_context) {
        global_scope_creation_params->web_worker_fetch_context->SetTerminateSyncLoadEvent(&terminate_sync_load_event_);
    }
...
void ThreadedMessagingProxyBase::CountWebDXFeature(mojom::blink::WebDXFeature feature)
{
    DCHECK(IsParentContextThread());
...
void ThreadedMessagingProxyBase::ReportConsoleMessage(
    mojom::ConsoleMessageSource source, mojom::ConsoleMessageLevel level, const String& message, std::unique_ptr<SourceLocation> location)
{
    DCHECK(IsParentContextThread());
    if (asked_to_terminate_)
        return;
    execution_context_->AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(level, message, std::move(location), worker_thread_.get()));
}
...
ParentExecutionContextTaskRunners* ThreadedMessagingProxyBase::GetParentExecutionContextTaskRunners() const
{
    DCHECK(IsParentContextThread());
    return parent_execution_context_task_runners_;
}

scoped_refptr<base::SingleThreadTaskRunner> ThreadedMessagingProxyBase::GetParentAgentGroupTaskRunner() const
{
    DCHECK(IsParentContextThread());
