...
class CORE_EXPORT ThreadedWorkletMessagingProxy : public ThreadedMessagingProxyBase, public WorkletGlobalScopeProxy {
public:
    ...
    void Initialize(WorkerClients*, WorkletModuleResponsesMap*, const std::optional<WorkerBackingThreadStartupData>& = std::nullopt,
        mojom::blink::WorkletGlobalScopeCreationParamsPtr client_provided_global_scope_creation_params = {});

    void Trace(Visitor*) const override;

protected:
    explicit ThreadedWorkletMessagingProxy(ExecutionContext*, scoped_refptr<base::SingleThreadTaskRunner> parent_agent_group_task_runner = nullptr);
...
private:
    ...
    virtual std::unique_ptr<ThreadedWorkletObjectProxy> CreateObjectProxy(
        ThreadedWorkletMessagingProxy*, ParentExecutionContextTaskRunners*, scoped_refptr<base::SingleThreadTaskRunner> parent_agent_group_task_runner);
