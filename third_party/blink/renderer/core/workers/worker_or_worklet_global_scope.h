...
class CORE_EXPORT WorkerOrWorkletGlobalScope : public EventTarget,
                                               public ExecutionContext,
                                               public scheduler::WorkerScheduler::Delegate,
                                               public BackForwardCacheLoaderHelperImpl::Delegate {
public:
    WorkerOrWorkletGlobalScope(v8::Isolate*, scoped_refptr<SecurityOrigin> origin, bool is_creator_secure_context, Agent* agent, const String& name,
        const base::UnguessableToken& parent_devtools_token, mojom::blink::V8CacheOptions, WorkerClients*, std::unique_ptr<WebContentSettingsClient>,
        scoped_refptr<WebWorkerFetchContext>, WorkerReportingProxy&, bool is_worker_loaded_from_data_url, bool is_default_world_of_isolate);
    ...
    // ExecutionContext
    bool IsWorkerOrWorkletGlobalScope() const final
    {
        return true;
    }
    ...
    virtual WorkerNavigator* navigator() const
    {
        return nullptr;
    }
    ...
    virtual RejectCoepUnsafeNone ShouldRejectCoepUnsafeNoneTopModuleScript() const
    {
    ...
    // dependencies.
    ResourceFetcher* CreateOutsideSettingsFetcher(const FetchClientSettingsObject&, WorkerResourceTimingNotifier&);
    ...
    WorkerReportingProxy& ReportingProxy()
    {
        return reporting_proxy_;
    }
    ...
    virtual CodeCacheHost* GetCodeCacheHost()
    {
        return nullptr;
    }
    ...
    virtual void OnConsoleApiMessage(mojom::ConsoleMessageLevel level, const String& message, SourceLocation* location);

    // Called when BestEffortServiceWorker(crbug.com/1420517) is enabled.
    virtual std::optional<mojo::PendingRemote<network::mojom::blink::URLLoaderFactory>> FindRaceNetworkRequestURLLoaderFactory(
...
protected:
    ...
    // fetch.
    void SetOutsideContentSecurityPolicies(Vector<network::mojom::blink::ContentSecurityPolicyPtr>);

    // Initializes inside's CSP used for subresource fetch etc.
    void InitContentSecurityPolicyFromVector(Vector<network::mojom::blink::ContentSecurityPolicyPtr> policies);
    virtual void BindContentSecurityPolicyToExecutionContext();

    void FetchModuleScript(const KURL& module_url_record, const FetchClientSettingsObjectSnapshot&, WorkerResourceTimingNotifier&,
        mojom::blink::RequestContextType context_type, network::mojom::RequestDestination destination, network::mojom::CredentialsMode,
        ModuleScriptCustomFetchType, ModuleTreeClient*);

    const Vector<network::mojom::blink::ContentSecurityPolicyPtr>& OutsideContentSecurityPolicies() const
    {
    ...
    virtual ResourceLoadScheduler::ThrottleOptionOverride GetThrottleOptionOverride() const;
    ...
    bool IsCreatorSecureContext() const
    {
...
private:
    ...
    ResourceFetcher* CreateFetcherInternal(const FetchClientSettingsObject&, ContentSecurityPolicy&, WorkerResourceTimingNotifier&);
    ...
    Vector<network::mojom::blink::ContentSecurityPolicyPtr> outside_content_security_policies_;
    ...
    std::bitset<static_cast<size_t>(WebDXFeature::kMaxValue) + 1> used_webdx_features_;

    // This tracks deprecation features that have been used.
    Deprecation deprecation_;
};

template <> struct DowncastTraits<WorkerOrWorkletGlobalScope> {
