    ...
    ScriptState* script_state, const String& module_url, const WorkletOptions* options, ExceptionState& exception_state)
{
    ...
    if (!GetExecutionContext()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "This frame is already detached");
        return EmptyPromise();
    }
    UseCounter::Count(GetExecutionContext(), mojom::WebFeature::kWorkletAddModule);
    ...
    if (!module_url_record.IsValid()) {
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kSyntaxError, "'" + module_url + "' is not a valid URL."));
        return promise;
    }

    WorkletPendingTasks* pending_tasks = MakeGarbageCollected<WorkletPendingTasks>(this, resolver);
    ...
    GetExecutionContext()
        ...
        ->PostTask(FROM_HERE,
            WTF::BindOnce(
                &Worklet::FetchAndInvokeScript, WrapPersistent(this), module_url_record, options->credentials().AsEnum(), WrapPersistent(pending_tasks)));
    return promise;
}
...
// algorithm:
// https://drafts.css-houdini.org/worklets/#dom-worklet-addmodule
void Worklet::FetchAndInvokeScript(const KURL& module_url_record, V8RequestCredentials::Enum credentials, WorkletPendingTasks* pending_tasks)
{
    DCHECK(IsMainThread());
    if (!GetExecutionContext())
        return;
    ...
    network::mojom::CredentialsMode credentials_mode = Request::V8RequestCredentialsToCredentialsMode(credentials);

    // Step 7: "Let outsideSettings be the relevant settings object of this."
    auto* outside_settings_object
        = MakeGarbageCollected<FetchClientSettingsObjectSnapshot>(GetExecutionContext()->Fetcher()->GetProperties().GetFetchClientSettingsObject());

    auto* outside_resource_timing_notifier = WorkerResourceTimingNotifierImpl::CreateForInsideResourceFetcher(*GetExecutionContext());

    // Specify TaskType::kInternalLoading because it's commonly used for module
    // loading.
    scoped_refptr<base::SingleThreadTaskRunner> outside_settings_task_runner = GetExecutionContext()->GetTaskRunner(TaskType::kInternalLoading);
    ...
    for (const auto& proxy : proxies_) {
        proxy->FetchAndInvokeScript(
            module_url_record, credentials_mode, *outside_settings_object, *outside_resource_timing_notifier, outside_settings_task_runner, pending_tasks);
    }
}
