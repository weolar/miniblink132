...
class CORE_EXPORT ParentExecutionContextTaskRunners final : public GarbageCollected<ParentExecutionContextTaskRunners>,
...
public:
    ...
    ParentExecutionContextTaskRunners(const ParentExecutionContextTaskRunners&) = delete;
    ParentExecutionContextTaskRunners& operator=(const ParentExecutionContextTaskRunners&) = delete;

    // Might return nullptr for unsupported task types. This can be called from
    // any threads.
    scoped_refptr<base::SingleThreadTaskRunner> Get(TaskType) LOCKS_EXCLUDED(lock_);
