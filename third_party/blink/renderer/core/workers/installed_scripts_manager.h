...
public:
    ...
    public:
        ScriptData() = default;
        ScriptData(const KURL& script_url, String source_text, std::unique_ptr<Vector<uint8_t>> meta_data, std::unique_ptr<CrossThreadHTTPHeaderMapData>);
        ...
        String TakeSourceText()
        {
            return std::move(source_text_);
        }
        ...
        ContentSecurityPolicyResponseHeaders GetContentSecurityPolicyResponseHeaders();
