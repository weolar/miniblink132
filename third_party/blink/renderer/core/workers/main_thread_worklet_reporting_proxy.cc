...
MainThreadWorkletReportingProxy::MainThreadWorkletReportingProxy(ExecutionContext* context)
    : context_(context)
{
}
...
void MainThreadWorkletReportingProxy::CountWebDXFeature(mojom::blink::WebDXFeature feature)
{
    DCHECK(IsMainThread());
    // A parent context is on the same thread, so just record API use in the
    // context's UseCounter.
