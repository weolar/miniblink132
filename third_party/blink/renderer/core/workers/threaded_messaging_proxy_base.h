...
class CORE_EXPORT ThreadedMessagingProxyBase : public GarbageCollected<ThreadedMessagingProxyBase> {
public:
    ...
    void ReportConsoleMessage(mojom::ConsoleMessageSource, mojom::ConsoleMessageLevel, const String& message, std::unique_ptr<SourceLocation>);
...
protected:
    explicit ThreadedMessagingProxyBase(ExecutionContext*, scoped_refptr<base::SingleThreadTaskRunner> parent_agent_group_task_runner = nullptr);
    ...
    void InitializeWorkerThread(std::unique_ptr<GlobalScopeCreationParams>, const std::optional<WorkerBackingThreadStartupData>&,
        const std::optional<const blink::DedicatedWorkerToken>&, std::unique_ptr<WorkerDevToolsParams> client_provided_devtools_params = nullptr);

    ExecutionContext* GetExecutionContext() const;
    ParentExecutionContextTaskRunners* GetParentExecutionContextTaskRunners() const;

    scoped_refptr<base::SingleThreadTaskRunner> GetParentAgentGroupTaskRunner() const;
...
private:
    ...
    CrossThreadPersistent<ParentExecutionContextTaskRunners> parent_execution_context_task_runners_;
