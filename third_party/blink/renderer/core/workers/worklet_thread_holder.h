...
// AudioWorkletThread).
template <class DerivedWorkletThread> class WorkletThreadHolder {
public:
    ...
    static void EnsureInstance(const ThreadCreationParams& params)
    {
        ...
        thread_holder_instance_->Initialize(std::make_unique<WorkerBackingThread>(params));
    }
...
private:
    ...
    void Initialize(std::unique_ptr<WorkerBackingThread> backing_thread)
    {
        ...
        PostCrossThreadTask(*thread_->BackingThread().GetTaskRunner(), FROM_HERE,
            CrossThreadBindOnce(&WorkletThreadHolder::InitializeOnWorkletThread, CrossThreadUnretained(this)));
    }

    void InitializeOnWorkletThread()
    {
        base::AutoLock locker(HolderInstanceLock());
        thread_->InitializeOnBackingThread(WorkerBackingThreadStartupData::CreateDefault());
    }
    ...
    void ShutdownAndWait()
    {
        ...
        PostCrossThreadTask(*thread_->BackingThread().GetTaskRunner(), FROM_HERE,
            CrossThreadBindOnce(&WorkletThreadHolder::ShutdownOnWorkletThread, CrossThreadUnretained(this), CrossThreadUnretained(&waitable_event)));
        waitable_event.Wait();
    }
...
WorkletThreadHolder<DerivedWorkletThread>* WorkletThreadHolder<DerivedWorkletThread>::thread_holder_instance_ GUARDED_BY(
