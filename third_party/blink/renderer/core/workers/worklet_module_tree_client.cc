...
WorkletModuleTreeClient::WorkletModuleTreeClient(
    ScriptState* script_state, scoped_refptr<base::SingleThreadTaskRunner> outside_settings_task_runner, WorkletPendingTasks* pending_tasks)
...
void WorkletModuleTreeClient::NotifyModuleTreeLoadFinished(ModuleScript* module_script)
{
    ...
    if (!module_script) {
        ...
        PostCrossThreadTask(*outside_settings_task_runner_, FROM_HERE,
            CrossThreadBindOnce(&WorkletPendingTasks::Abort, WrapCrossThreadPersistent(pending_tasks_.Get()),
    ...
    if (module_script->HasErrorToRethrow()) {
        ...
        PostCrossThreadTask(*outside_settings_task_runner_, FROM_HERE,
            CrossThreadBindOnce(&WorkletPendingTasks::Abort, WrapCrossThreadPersistent(pending_tasks_.Get()),
                SerializedScriptValue::SerializeAndSwallowExceptions(script_state_->GetIsolate(), module_script->CreateErrorToRethrow().V8Value())));
        return;
    }

    // Step 5: "Run a module script given script."
    ScriptEvaluationResult result = module_script->RunScriptOnScriptStateAndReturnValue(script_state_);

    auto* global_scope = To<WorkletGlobalScope>(ExecutionContext::From(script_state_));

    global_scope->ReportingProxy().DidEvaluateTopLevelScript(result.GetResultType() == ScriptEvaluationResult::ResultType::kSuccess);
    ...
    PostCrossThreadTask(*outside_settings_task_runner_, FROM_HERE,
        CrossThreadBindOnce(&WorkletPendingTasks::DecrementCounter, WrapCrossThreadPersistent(pending_tasks_.Get())));
}
