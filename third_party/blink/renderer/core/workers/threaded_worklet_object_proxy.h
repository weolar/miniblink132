...
public:
    ...
    ThreadedWorkletObjectProxy& operator=(const ThreadedWorkletObjectProxy&) = delete;
    ~ThreadedWorkletObjectProxy() override;

    void FetchAndInvokeScript(const KURL& module_url_record, network::mojom::CredentialsMode,
        ...
        scoped_refptr<base::SingleThreadTaskRunner> outside_settings_task_runner, WorkletPendingTasks*, WorkerThread*);

protected:
    ThreadedWorkletObjectProxy(
        ThreadedWorkletMessagingProxy*, ParentExecutionContextTaskRunners*, scoped_refptr<base::SingleThreadTaskRunner> parent_agent_group_task_runner);

    CrossThreadWeakPersistent<ThreadedMessagingProxyBase> MessagingProxyWeakPtr() final;
...
private:
    ...
    // the tasks.
    CrossThreadWeakPersistent<ThreadedWorkletMessagingProxy> messaging_proxy_weak_ptr_;
};
