...
void WorkletModuleResponsesMap::Entry::AddClient(ModuleScriptFetcher::Client* client, scoped_refptr<base::SingleThreadTaskRunner> client_task_runner)
{
...
void WorkletModuleResponsesMap::Entry::SetParams(const std::optional<ModuleScriptCreationParams>& params)
{
    DCHECK_EQ(state_, State::kFetching);
    ...
    if (params) {
        ...
        for (auto& it : clients_) {
            PostCrossThreadTask(*it.value, FROM_HERE, CrossThreadBindOnce(&ModuleScriptFetcher::Client::OnFetched, it.key, *params));
        }
    } else {
        state_ = State::kFailed;
        // TODO(nhiroki): Add |error_messages| to the context's message storage.
        for (auto& it : clients_) {
            PostCrossThreadTask(*it.value, FROM_HERE, CrossThreadBindOnce(&ModuleScriptFetcher::Client::OnFailed, it.key));
        }
    }
...
bool WorkletModuleResponsesMap::GetEntry(
    const KURL& url, ModuleType module_type, ModuleScriptFetcher::Client* client, scoped_refptr<base::SingleThreadTaskRunner> client_task_runner)
{
    base::AutoLock locker(lock_);
    DCHECK_NE(module_type, ModuleType::kInvalid);
    if (!is_available_ || !IsValidURL(url)) {
        client_task_runner->PostTask(FROM_HERE, WTF::BindOnce(&ModuleScriptFetcher::Client::OnFailed, WrapPersistent(client)));
        return true;
    }
    ...
    if (it != entries_.end()) {
        ...
        case Entry::State::kFetched:
            ...
            // steps."
            client_task_runner->PostTask(FROM_HERE, WTF::BindOnce(&ModuleScriptFetcher::Client::OnFetched, WrapPersistent(client), entry->GetParams()));
            return true;
...
void WorkletModuleResponsesMap::SetEntryParams(const KURL& url, ModuleType module_type, const std::optional<ModuleScriptCreationParams>& params)
{
    base::AutoLock locker(lock_);
    if (!is_available_)
        return;
