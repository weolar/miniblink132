...
class MainThreadWorkletReportingProxyForTest final : public MainThreadWorkletReportingProxy {
public:
...
public:
    ...
    void SetUpScope(const String& csp_header)
    {
        ...
        csp->AddPolicies(ParseContentSecurityPolicies(
            csp_header, network::mojom::ContentSecurityPolicyType::kEnforce, network::mojom::ContentSecurityPolicySource::kHTTP, *(self_origin)));
        window->SetContentSecurityPolicy(csp);

        reporting_proxy_ = std::make_unique<MainThreadWorkletReportingProxyForTest>(window);
        auto creation_params = std::make_unique<GlobalScopeCreationParams>(window->Url(), mojom::blink::ScriptType::kModule, "MainThreadWorklet",
            window->UserAgent(), window->GetFrame()->Loader().UserAgentMetadata(), nullptr /* web_worker_fetch_context */,
            mojo::Clone(window->GetContentSecurityPolicy()->GetParsedPolicies()), Vector<network::mojom::blink::ContentSecurityPolicyPtr>(),
            ...
            mojo::NullRemote() /* blob_url_store */, BeginFrameProviderParams(), nullptr /* parent_permissions_policy */, window->GetAgentClusterID(),
...
TEST_F(MainThreadWorkletTest, AgentCluster)
{
    ...
    EXPECT_EQ(global_scope_->GetAgentClusterID(), GetFrame().DomWindow()->GetAgentClusterID());
}
...
TEST_F(MainThreadWorkletTest, ContentSecurityPolicy)
{
    ...
    EXPECT_TRUE(
        csp->AllowScriptFromSource(global_scope_->Url(), String(), IntegrityMetadataSet(), kParserInserted, global_scope_->Url(), RedirectStatus::kNoRedirect));
...
{
    scoped_refptr<base::SingleThreadTaskRunner> task_runner = global_scope_->GetTaskRunner(TaskType::kInternalTest);
...
{
    const Vector<network::mojom::blink::ContentSecurityPolicyPtr>& csp = global_scope_->GetContentSecurityPolicy()->GetParsedPolicies();
