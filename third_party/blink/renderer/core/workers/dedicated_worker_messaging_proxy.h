...
class CORE_EXPORT DedicatedWorkerMessagingProxy : public ThreadedMessagingProxyBase {
public:
    ...
    DedicatedWorkerMessagingProxy& operator=(const DedicatedWorkerMessagingProxy&) = delete;
    ~DedicatedWorkerMessagingProxy() override;

    // These methods should only be used on the parent context thread.
    void StartWorkerGlobalScope(std::unique_ptr<GlobalScopeCreationParams>, std::unique_ptr<WorkerMainScriptLoadParameters> worker_main_script_load_params,
        const WorkerOptions*, const KURL& script_url, const FetchClientSettingsObjectSnapshot& outside_settings_object, const v8_inspector::V8StackTraceId&,
        const String& source_code, RejectCoepUnsafeNone reject_coep_unsafe_none, const blink::DedicatedWorkerToken& token,
    ...
    void PostCustomEventToWorkerGlobalScope(TaskType task_type, CrossThreadFunction<Event*(ScriptState*, CustomEventMessage)> event_factory_callback,
    ...
    void DispatchErrorEvent(const String& error_message, std::unique_ptr<SourceLocation>, int exception_id);
...
private:
    ...
    std::optional<WorkerBackingThreadStartupData> CreateBackingThreadStartupData(v8::Isolate*);
    ...
    mojo::PendingRemote<mojom::blink::BackForwardCacheControllerHost> pending_back_forward_cache_controller_host_;
