...
WorkletGlobalScope::WorkletGlobalScope(std::unique_ptr<GlobalScopeCreationParams> creation_params, WorkerReportingProxy& reporting_proxy, LocalFrame* frame)
    : WorkletGlobalScope(std::move(creation_params), reporting_proxy, ToIsolate(frame), ThreadType::kMainThread, frame, nullptr /* worker_thread */)
{
}

WorkletGlobalScope::WorkletGlobalScope(
    std::unique_ptr<GlobalScopeCreationParams> creation_params, WorkerReportingProxy& reporting_proxy, WorkerThread* worker_thread)
    : WorkletGlobalScope(
        std::move(creation_params), reporting_proxy, worker_thread->GetIsolate(), ThreadType::kOffMainThread, nullptr /* frame */, worker_thread)
{
}
...
WorkletGlobalScope::WorkletGlobalScope(std::unique_ptr<GlobalScopeCreationParams> creation_params, WorkerReportingProxy& reporting_proxy, v8::Isolate* isolate,
    ThreadType thread_type, LocalFrame* frame, WorkerThread* worker_thread)
    : WorkerOrWorkletGlobalScope(isolate, SecurityOrigin::CreateUniqueOpaque(), creation_params->starter_secure_context,
        MakeGarbageCollected<Agent>(isolate, creation_params->agent_cluster_id, v8::MicrotaskQueue::New(isolate, v8::MicrotasksPolicy::kScoped)),
        ...
        std::move(creation_params->content_settings_client), std::move(creation_params->web_worker_fetch_context), reporting_proxy,
    ...
    frame_token_(creation_params->parent_context_token ? creation_params->parent_context_token->GetAs<LocalFrameToken>() : blink::LocalFrameToken())
    , parent_cross_origin_isolated_capability_(creation_params->parent_cross_origin_isolated_capability)
    , parent_is_isolated_context_(creation_params->parent_is_isolated_context)
    , browser_interface_broker_proxy_(this)
{
    DCHECK((thread_type_ == ThreadType::kMainThread && frame_) || (thread_type_ == ThreadType::kOffMainThread && worker_thread_));
    ...
    CHECK(!creation_params->is_default_world_of_isolate || thread_type == ThreadType::kOffMainThread);

    // Worklet should be in the owner's agent cluster.
    // https://html.spec.whatwg.org/C/#obtain-a-worklet-agent
    DCHECK(creation_params->agent_cluster_id || !creation_params->parent_context_token);
    ...
    SetOutsideContentSecurityPolicies(mojo::Clone(creation_params->outside_content_security_policies));
    ...
    // workletGlobalScope."
    InitContentSecurityPolicyFromVector(std::move(creation_params->outside_content_security_policies));
    BindContentSecurityPolicyToExecutionContext();

    OriginTrialContext::ActivateWorkerInheritedFeatures(this, creation_params->inherited_trial_features.get());
    ...
    if (creation_params->code_cache_host_interface.is_valid()) {
        code_cache_host_ = std::make_unique<CodeCacheHost>(mojo::Remote<mojom::blink::CodeCacheHost>(std::move(creation_params->code_cache_host_interface)));
    }
...
const BrowserInterfaceBrokerProxy& WorkletGlobalScope::GetBrowserInterfaceBroker() const
{
...
void WorkletGlobalScope::AddConsoleMessageImpl(ConsoleMessage* console_message, bool discard_duplicates)
{
    if (IsMainThreadWorkletGlobalScope()) {
    ...
    worker_thread_->GetWorkerReportingProxy().ReportConsoleMessage(
        console_message->GetSource(), console_message->GetLevel(), console_message->Message(), console_message->Location());
    worker_thread_->GetConsoleMessageStorage()->AddConsoleMessage(worker_thread_->GlobalScope(), console_message, discard_duplicates);
}
...
void WorkletGlobalScope::AddInspectorIssue(AuditsIssue issue)
{
    if (IsMainThreadWorkletGlobalScope()) {
        ...
        worker_thread_->GetInspectorIssueStorage()->AddInspectorIssue(this, std::move(issue));
    }
}
...
{
    if (IsMainThreadWorkletGlobalScope()) {
        MainThreadDebugger::Instance(GetIsolate())->ExceptionThrown(this, error_event);
        return;
    }
...
scoped_refptr<base::SingleThreadTaskRunner> WorkletGlobalScope::GetTaskRunner(TaskType task_type)
{
    if (IsMainThreadWorkletGlobalScope())
...
void WorkletGlobalScope::FetchAndInvokeScript(const KURL& module_url_record, network::mojom::CredentialsMode credentials_mode,
    ...
    scoped_refptr<base::SingleThreadTaskRunner> outside_settings_task_runner, WorkletPendingTasks* pending_tasks)
{
    ...
    auto* client = MakeGarbageCollected<WorkletModuleTreeClient>(ScriptController()->GetScriptState(), std::move(outside_settings_task_runner), pending_tasks);

    auto request_context_type = mojom::blink::RequestContextType::SCRIPT;
    FetchModuleScript(module_url_record, outside_settings_object, outside_resource_timing_notifier, request_context_type, GetDestination(), credentials_mode,
...
mojo::PendingRemote<mojom::blink::BlobURLStore> WorkletGlobalScope::TakeBlobUrlStorePendingRemote()
{
