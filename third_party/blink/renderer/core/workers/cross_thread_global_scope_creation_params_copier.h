...
template <> struct CrossThreadCopier<Vector<network::mojom::blink::ContentSecurityPolicyPtr>> {
    STATIC_ONLY(CrossThreadCopier);
    using Type = Vector<network::mojom::blink::ContentSecurityPolicyPtr>;
    static Type Copy(const Type&);
};

template <> struct CrossThreadCopier<std::unique_ptr<blink::GlobalScopeCreationParams>> {
    STATIC_ONLY(CrossThreadCopier);
