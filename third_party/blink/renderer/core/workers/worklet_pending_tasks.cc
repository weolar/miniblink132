...
WorkletPendingTasks::WorkletPendingTasks(Worklet* worklet, ScriptPromiseResolver<IDLUndefined>* resolver)
    : resolver_(resolver)
    , worklet_(worklet)
{
    DCHECK(IsMainThread());
...
void WorkletPendingTasks::Abort(scoped_refptr<SerializedScriptValue> error_to_rethrow)
{
    DCHECK(IsMainThread());
    ...
    if (counter_ != -1) {
        ...
        if (error_to_rethrow) {
            ...
            resolver_->Reject(error_to_rethrow->Deserialize(resolver_->GetScriptState()->GetIsolate()));
        } else {
            resolver_->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError, "Unable to load a worklet's module."));
        }
    }
}
