...
class CORE_EXPORT WorkletModuleResponsesMap final : public GarbageCollected<WorkletModuleResponsesMap> {
public:
    ...
    bool GetEntry(const KURL&, ModuleType, ModuleScriptFetcher::Client*, scoped_refptr<base::SingleThreadTaskRunner> client_task_runner) LOCKS_EXCLUDED(lock_);

    // Called on worklet threads.
    void SetEntryParams(const KURL&, ModuleType, const std::optional<ModuleScriptCreationParams>&) LOCKS_EXCLUDED(lock_);
...
private:
    ...
    public:
        ...
        void AddClient(ModuleScriptFetcher::Client* client, scoped_refptr<base::SingleThreadTaskRunner> client_task_runner);
    ...
    private:
        ...
        HashMap<CrossThreadPersistent<ModuleScriptFetcher::Client>, scoped_refptr<base::SingleThreadTaskRunner>> clients_;
    };
