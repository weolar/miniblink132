...
class CORE_EXPORT WorkletGlobalScope : public WorkerOrWorkletGlobalScope, public ActiveScriptWrappable<WorkletGlobalScope> {
...
public:
    ...
    bool IsWorkletGlobalScope() const final
    {
        return true;
    }
    ...
    mojo::PendingRemote<mojom::blink::BlobURLStore> TakeBlobUrlStorePendingRemote();
    ...
    void FetchAndInvokeScript(const KURL& module_url_record, network::mojom::CredentialsMode, const FetchClientSettingsObjectSnapshot& outside_settings_object,
        WorkerResourceTimingNotifier& outside_resource_timing_notifier, scoped_refptr<base::SingleThreadTaskRunner> outside_settings_task_runner,
        WorkletPendingTasks*);
    ...
    // document.
    bool DocumentSecureContext() const
    {
        return IsCreatorSecureContext();
    }
    ...
    // thread.
    WorkletGlobalScope(std::unique_ptr<GlobalScopeCreationParams>, WorkerReportingProxy&, LocalFrame*);

    // Constructs an instance as a threaded worklet. Must be called on a worker
    // thread.
    WorkletGlobalScope(std::unique_ptr<GlobalScopeCreationParams>, WorkerReportingProxy&, WorkerThread*);
...
private:
    ...
    // https://drafts.css-houdini.org/worklets/#script-settings-for-worklets
    WorkletGlobalScope(std::unique_ptr<GlobalScopeCreationParams>, WorkerReportingProxy&, v8::Isolate*, ThreadType, LocalFrame*, WorkerThread*);
    ...
    mojo::PendingRemote<mojom::blink::BlobURLStore> blob_url_store_pending_remote_;
};

template <> struct DowncastTraits<WorkletGlobalScope> {
