...
public:
    GlobalScopeCreationParams(const KURL& script_url, mojom::blink::ScriptType script_type, const String& global_scope_name, const String& user_agent,
        const std::optional<UserAgentMetadata>& ua_metadata, scoped_refptr<WebWorkerFetchContext>,
        ...
        const SecurityOrigin*, bool starter_secure_context, HttpsState starter_https_state, WorkerClients*, std::unique_ptr<WebContentSettingsClient>,
        const Vector<mojom::blink::OriginTrialFeature>* inherited_trial_features, const base::UnguessableToken& parent_devtools_token,
        std::unique_ptr<WorkerSettings>, mojom::blink::V8CacheOptions, WorkletModuleResponsesMap*,
        mojo::PendingRemote<mojom::blink::BrowserInterfaceBroker> browser_interface_broker = mojo::NullRemote(),
        mojo::PendingRemote<mojom::blink::CodeCacheHost> code_cahe_host = mojo::NullRemote(),
        mojo::PendingRemote<mojom::blink::BlobURLStore> blob_url_store = mojo::NullRemote(), BeginFrameProviderParams begin_frame_provider_params = {},
        const PermissionsPolicy* parent_permissions_policy = nullptr, base::UnguessableToken agent_cluster_id = {},
        ukm::SourceId ukm_source_id = ukm::kInvalidSourceId, const std::optional<ExecutionContextToken>& parent_context_token = std::nullopt,
        bool parent_cross_origin_isolated_capability = false, bool parent_is_isolated_context = false, InterfaceRegistry* interface_registry = nullptr,
        scoped_refptr<base::SingleThreadTaskRunner> agent_group_scheduler_compositor_task_runner = nullptr,
        const SecurityOrigin* top_level_frame_security_origin = nullptr,
        net::StorageAccessApiStatus parent_storage_access_api_status = net::StorageAccessApiStatus::kNone, bool require_cross_site_request_for_cookies = false,
        scoped_refptr<SecurityOrigin> origin_to_use = nullptr);
    GlobalScopeCreationParams(const GlobalScopeCreationParams&) = delete;
    GlobalScopeCreationParams& operator=(const GlobalScopeCreationParams&) = delete;
    ...
    Vector<network::mojom::blink::ContentSecurityPolicyPtr> response_content_security_policies;

    network::mojom::ReferrerPolicy referrer_policy;

    // Origin trial features to be inherited by worker/worklet from the document
    // loading it.
    std::unique_ptr<Vector<mojom::blink::OriginTrialFeature>> inherited_trial_features;
    ...
    mojo::PendingRemote<mojom::blink::BrowserInterfaceBroker> browser_interface_broker;
