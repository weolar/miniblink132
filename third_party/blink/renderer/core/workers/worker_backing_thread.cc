...
HashSet<v8::Isolate*>& ForegroundedIsolates() EXCLUSIVE_LOCKS_REQUIRED(IsolatesLock())
{
    DEFINE_THREAD_SAFE_STATIC_LOCAL(HashSet<v8::Isolate*>, foregrounded_isolates, ());
    return foregrounded_isolates;
}

v8::Isolate::Priority& IsolateCurrentPriority() EXCLUSIVE_LOCKS_REQUIRED(IsolatesLock())
{
    DEFINE_THREAD_SAFE_STATIC_LOCAL(v8::Isolate::Priority, isolate_current_priority, (v8::Isolate::Priority::kUserBlocking));
...
{
    Thread::MainThread()->Scheduler()->ToMainThreadScheduler()->ForEachMainThreadIsolate(
        WTF::BindRepeating([](v8::MemoryPressureLevel level, v8::Isolate* isolate) { isolate->MemoryPressureNotification(level); }, level));
...
{
    Thread::MainThread()->Scheduler()->ToMainThreadScheduler()->ForEachMainThreadIsolate(WTF::BindRepeating(
        [](bool battery_saver_mode_enabled, v8::Isolate* isolate) { isolate->SetBatterySaverMode(battery_saver_mode_enabled); }, battery_saver_mode_enabled));
    WorkerBackingThread::SetBatterySaverModeForWorkerThreadIsolates(battery_saver_mode_enabled);
}

WorkerBackingThread::WorkerBackingThread(const ThreadCreationParams& params)
    : backing_thread_(blink::NonMainThread::CreateThread(ThreadCreationParams(params).SetSupportsGC(true)))
{
}

WorkerBackingThread::~WorkerBackingThread() = default;

void WorkerBackingThread::InitializeOnBackingThread(const WorkerBackingThreadStartupData& startup_data)
{
    DCHECK(backing_thread_->IsCurrentThread());

    DCHECK(!isolate_);
    ThreadScheduler* scheduler = BackingThread().Scheduler();
    isolate_ = V8PerIsolateData::Initialize(scheduler->V8TaskRunner(), scheduler->V8UserVisibleTaskRunner(), scheduler->V8BestEffortTaskRunner(),
        V8PerIsolateData::V8ContextSnapshotMode::kDontUseSnapshot, nullptr, nullptr);
    scheduler->SetV8Isolate(isolate_);
    AddWorkerIsolate(isolate_);
    V8Initializer::InitializeWorker(isolate_);

    if (RuntimeEnabledFeatures::V8IdleTasksEnabled()) {
        V8PerIsolateData::EnableIdleTasks(isolate_, std::make_unique<V8IdleTaskRunner>(scheduler));
    }
    Platform::Current()->DidStartWorkerThread();

    V8PerIsolateData::From(isolate_)->SetThreadDebugger(std::make_unique<WorkerThreadDebugger>(isolate_));

    if (startup_data.heap_limit_mode == WorkerBackingThreadStartupData::HeapLimitMode::kIncreasedForDebugging) {
        isolate_->IncreaseHeapLimitForDebugging();
    }
    isolate_->SetAllowAtomicsWait(startup_data.atomics_wait_mode == WorkerBackingThreadStartupData::AtomicsWaitMode::kAllow);
}
...
// static
void WorkerBackingThread::MemoryPressureNotificationToWorkerThreadIsolates(v8::MemoryPressureLevel level)
{
    base::AutoLock locker(IsolatesLock());
...
// static
void WorkerBackingThread::SetWorkerThreadIsolatesPriority(v8::Isolate::Priority priority)
{
    base::AutoLock locker(IsolatesLock());
...
// static
void WorkerBackingThread::SetBatterySaverModeForWorkerThreadIsolates(bool battery_saver_mode_enabled)
{
    base::AutoLock locker(IsolatesLock());
