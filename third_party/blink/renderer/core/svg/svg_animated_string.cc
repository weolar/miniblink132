...
{
    return MakeGarbageCollected<V8UnionStringOrTrustedScriptURL>(SVGAnimatedProperty<SVGString>::baseVal());
}

void SVGAnimatedString::setBaseVal(const V8UnionStringOrTrustedScriptURL* value, ExceptionState& exception_state)
{
    DCHECK(value);
    ...
    case V8UnionStringOrTrustedScriptURL::ContentType::kString:
        ...
        if (ContextElement()->IsScriptElement()) {
            string = TrustedTypesCheckForScriptURL(string, ContextElement()->GetExecutionContext(), "SVGAnimatedString", "baseVal", exception_state);
