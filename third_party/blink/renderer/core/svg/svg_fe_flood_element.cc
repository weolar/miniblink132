...
bool SVGFEFloodElement::SetFilterEffectAttribute(FilterEffect* effect, const QualifiedName& attr_name)
{
    ...
    if (attr_name == svg_names::kFloodColorAttr) {
        ...
        // SkColor4f
        return flood->SetFloodColor(style.VisitedDependentColor(GetCSSPropertyFloodColor()).toSkColor4f());
    }
...
FilterEffect* SVGFEFloodElement::Build(SVGFilterBuilder*, Filter* filter)
{
    ...
    SkColor4f color = style->VisitedDependentColor(GetCSSPropertyFloodColor()).toSkColor4f();
