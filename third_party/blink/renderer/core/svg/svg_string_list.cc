...
template <typename CharType> void SVGStringListBase::ParseInternal(const CharType* ptr, const CharType* end, char list_delimiter)
{
    while (ptr < end) {
...
SVGParsingError SVGStringListBase::SetValueAsStringWithDelimiter(const String& data, char list_delimiter)
{
    ...
    WTF::VisitCharacters(data, [&](auto chars) { ParseInternal(chars.data(), chars.data() + chars.size(), list_delimiter); });
    return SVGParseStatus::kNoError;
}

String SVGStringListBase::ValueAsStringWithDelimiter(char list_delimiter) const
{
    if (values_.empty())
        return String();
...
void SVGStringListBase::Add(const SVGPropertyBase* other, const SVGElement* context_element)
{
...
void SVGStringListBase::CalculateAnimatedValue(
    const SMILAnimationEffectParameters&, float, unsigned, const SVGPropertyBase*, const SVGPropertyBase*, const SVGPropertyBase*, const SVGElement*)
{
    // SVGStringList is never animated.
    NOTREACHED();
}

float SVGStringListBase::CalculateDistance(const SVGPropertyBase*, const SVGElement*) const
{
    // SVGStringList is never animated.
    NOTREACHED();
}
