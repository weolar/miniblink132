...
SVGPathByteStream BlendPathByteStreams(const SVGPathByteStream& from_stream, const SVGPathByteStream& to_stream, float progress)
{
...
SVGPathByteStream AddPathByteStreams(const SVGPathByteStream& from_stream, const SVGPathByteStream& by_stream, unsigned repeat_count = 1)
{
    SVGPathByteStreamBuilder builder;
    SVGPathByteStreamSource from_source(from_stream);
...
SVGPathByteStream ConditionallyAddPathByteStreams(SVGPathByteStream from_stream, const SVGPathByteStream& by_stream, unsigned repeat_count = 1)
{
...
SVGPropertyBase* SVGPath::CloneForAnimation(const String& value) const
{
    ...
    return MakeGarbageCollected<SVGPath>(*MakeGarbageCollected<CSSPathValue>(builder.CopyByteStream()));
}
...
void SVGPath::Add(const SVGPropertyBase* other, const SVGElement*)
{
    ...
    if (ByteStream().size() != other_path_byte_stream.size() || ByteStream().IsEmpty() || other_path_byte_stream.IsEmpty())
        return;

    path_value_ = MakeGarbageCollected<CSSPathValue>(AddPathByteStreams(ByteStream(), other_path_byte_stream));
}

void SVGPath::CalculateAnimatedValue(const SMILAnimationEffectParameters& parameters, float percentage, unsigned repeat_count,
    const SVGPropertyBase* from_value, const SVGPropertyBase* to_value, const SVGPropertyBase* to_at_end_of_duration_value, const SVGElement*)
{
    ...
    SVGPathByteStream new_stream = BlendPathByteStreams(from_stream, to_stream, percentage);

    // Handle accumulate='sum'.
    if (repeat_count && parameters.is_cumulative) {
        new_stream = ConditionallyAddPathByteStreams(std::move(new_stream), To<SVGPath>(to_at_end_of_duration_value)->ByteStream(), repeat_count);
    }

    // Handle additive='sum'.
    if (parameters.is_additive) {
        new_stream = ConditionallyAddPathByteStreams(std::move(new_stream), ByteStream());
    }

    path_value_ = MakeGarbageCollected<CSSPathValue>(std::move(new_stream));
}

float SVGPath::CalculateDistance(const SVGPropertyBase* to, const SVGElement*) const
{
