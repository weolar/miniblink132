...
SVGStyleElement::SVGStyleElement(Document& document, const CreateElementFlags flags)
...
Node::InsertionNotificationRequest SVGStyleElement::InsertedInto(ContainerNode& insertion_point)
{
...
void SVGStyleElement::NotifyLoadedSheetAndAllCriticalSubresources(LoadedSheetErrorStatus error_status)
{
    if (error_status != kNoErrorLoadingSubresource) {
        GetDocument()
            .GetTaskRunner(TaskType::kDOMManipulation)
            ->PostTask(FROM_HERE, WTF::BindOnce(&SVGStyleElement::DispatchPendingEvent, WrapPersistent(this)));
    }
}
