...
public:
    ...
    {
        auto params = WebNavigationParams::CreateWithHTMLStringForTesting(data, KURL("http://example.com"));
        GetFrame().Loader().CommitNavigation(std::move(params), nullptr /* extra_data */);
        GetAnimationClock().OverrideDynamicClockForTesting(platform()->GetTickClock());
...
TEST_F(SMILTimeContainerTest, ServiceAnimationsFlushesPendingSynchronizations)
{
    ...
    animation->setAttribute(svg_names::kAttributeNameAttr, AtomicString("height"));
...
TEST_F(SMILTimeContainerTest, ServiceAnimationsNoSuspendOnAnimationSync)
{
    ...
    animation->setAttribute(svg_names::kRepeatCountAttr, AtomicString("indefinite"));
...
public:
    ...
    {
        std::move(callback_).Run(*To<LocalDOMWindow>(execution_context)->document());
    }
...
public:
    ...
    {
        GetFrame().DomWindow()->addEventListener(event_type_names::kDOMContentLoaded, MakeGarbageCollected<ContentLoadedEventListener>(std::move(callback)));
    }

private:
    static void OverrideSettings(Settings& settings)
    {
        settings.SetImageAnimationPolicy(mojom::blink::ImageAnimationPolicy::kImageAnimationPolicyAnimateOnce);
    }
...
TEST_F(SMILTimeContainerAnimationPolicyOnceTest, SetElapsedBeforeStart)
{
    ...
    OnContentLoaded(WTF::BindOnce([](Document& document) {
        auto* svg_root = To<SVGSVGElement>(document.getElementById(AtomicString("container")));
        ASSERT_TRUE(svg_root);
        auto* rect = Traversal<SVGRectElement>::FirstChild(*svg_root);
        ASSERT_TRUE(rect);
        SVGLengthContext length_context(rect);
...
TEST_F(SMILTimeContainerAnimationPolicyOnceTest, PauseAndSetElapsedBeforeStart)
{
    Load(R"HTML(
    <svg id="container">
      <rect width="100" height="0" fill="green">
