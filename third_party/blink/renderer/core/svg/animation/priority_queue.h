...
template <typename PriorityType, typename ElementType> class PriorityQueue {
    DISALLOW_NEW();
...
public:
    ...
    const PriorityType& Min() const
    {
        return heap_.front().first;
    }
    ElementType* MinElement() const
    {
        return heap_.front().second.Get();
    }
    ...
    const EntryType& operator[](wtf_size_t index) const
    {
        return heap_[index];
    }
...
private:
    ...
    static bool IsRoot(wtf_size_t index)
    {
        return index == 0;
    }
    ...
    static wtf_size_t LeftChildIndex(wtf_size_t index)
    {
        return 2 * index + 1;
    }
...
template <typename PriorityType, typename ElementType> inline wtf_size_t PriorityQueue<PriorityType, ElementType>::PercolateUp(wtf_size_t index)
{
    while (!IsRoot(index)) {
...
template <typename PriorityType, typename ElementType> inline wtf_size_t PriorityQueue<PriorityType, ElementType>::SmallestChildIndex(wtf_size_t index) const
{
    ...
    if (right_child_index < heap_.size() && CompareLess(heap_[right_child_index], heap_[left_child_index]))
        return right_child_index;
    return left_child_index;
}

template <typename PriorityType, typename ElementType> inline void PriorityQueue<PriorityType, ElementType>::PercolateDown(wtf_size_t index)
{
    while (true) {
...
template <typename PriorityType, typename ElementType> inline void PriorityQueue<PriorityType, ElementType>::Insert(PriorityType priority, ElementType* element)
{
...
template <typename PriorityType, typename ElementType> inline void PriorityQueue<PriorityType, ElementType>::Remove(ElementType* element)
{
    DCHECK(Contains(element));
...
template <typename PriorityType, typename ElementType> inline void PriorityQueue<PriorityType, ElementType>::Update(PriorityType priority, ElementType* element)
{
    DCHECK(Contains(element));
    wtf_size_t index = element->PriorityQueueHandle();
...
template <typename PriorityType, typename ElementType> inline void PriorityQueue<PriorityType, ElementType>::ResetAllPriorities(PriorityType priority)
{
