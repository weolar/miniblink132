...
bool TargetCanHaveMotionTransform(const SVGElement& target)
{
    ...
    // cause problems.
    return IsA<SVGGElement>(target) || IsA<SVGDefsElement>(target) || IsA<SVGUseElement>(target) || IsA<SVGImageElement>(target)
        || IsA<SVGSwitchElement>(target) || IsA<SVGPathElement>(target) || IsA<SVGRectElement>(target) || IsA<SVGCircleElement>(target)
        || IsA<SVGEllipseElement>(target) || IsA<SVGLineElement>(target) || IsA<SVGPolylineElement>(target) || IsA<SVGPolygonElement>(target)
        || IsA<SVGTextElement>(target) || IsA<SVGAElement>(target) || IsA<SVGForeignObjectElement>(target);
}
...
void SVGAnimateMotionElement::ParseAttribute(const AttributeModificationParams& params)
{
...
template <typename CharType> static bool ParsePointInternal(const CharType* ptr, const CharType* end, gfx::PointF& point)
{
...
static bool ParsePoint(const String& string, gfx::PointF& point)
{
    if (string.empty())
        return false;
    return WTF::VisitCharacters(string, [&](auto chars) { return ParsePointInternal(chars.data(), chars.data() + chars.size(), point); });
}
...
bool SVGAnimateMotionElement::CalculateToAtEndOfDurationValue(const String& to_at_end_of_duration_string)
{
    ParsePoint(to_at_end_of_duration_string, to_point_at_end_of_duration_);
    return true;
}

void SVGAnimateMotionElement::CalculateFromAndToValues(const String& from_string, const String& to_string)
{
...
void SVGAnimateMotionElement::CalculateFromAndByValues(const String& from_string, const String& by_string)
{
...
void SVGAnimateMotionElement::CalculateAnimationValue(SMILAnimationValue& animation_value, float percentage, unsigned repeat_count) const
{
    ...
    if (GetAnimationMode() != kPathAnimation) {
        position.point
            = gfx::PointF(ComputeAnimatedNumber(parameters, percentage, repeat_count, from_point_.x(), to_point_.x(), to_point_at_end_of_duration_.x()),
                ComputeAnimatedNumber(parameters, percentage, repeat_count, from_point_.y(), to_point_.y(), to_point_at_end_of_duration_.y()));
        position.tangent_in_degrees = Rad2deg((to_point_ - from_point_).SlopeAngleRadians());
    } else {
        ...
        if (repeat_count && parameters.is_cumulative) {
            const gfx::PointF position_at_end_of_duration = animation_path_.PointAtLength(path_length);
            position.point += gfx::ScalePoint(position_at_end_of_duration, repeat_count).OffsetFromOrigin();
        }
    }
...
void SVGAnimateMotionElement::ApplyResultsToTarget(const SMILAnimationValue& animation_value)
{
    SVGElement* target_element = targetElement();
    DCHECK(target_element);
...
float SVGAnimateMotionElement::CalculateDistance(const String& from_string, const String& to_string)
{
    gfx::PointF from;
    gfx::PointF to;
