...
template <> const SVGEnumerationMap& GetEnumerationMap<SVGMarkerOrientType>()
{
    ...
    static const SVGEnumerationMap entries(base::span<const char* const>(enum_items.data(), enum_items.size()), kSVGMarkerOrientAngle);
    return entries;
}
...
public:
    explicit SVGMarkerOrientEnumeration(SVGAngle* angle)
        : SVGEnumeration(kSVGMarkerOrientAngle)
        , angle_(angle)
    {
    }
...
SVGAngle::SVGAngle(SVGAngleType unit_type, float value_in_specified_units, SVGMarkerOrientType orient_type)
...
{
    return MakeGarbageCollected<SVGAngle>(unit_type_, value_in_specified_units_, OrientTypeValue());
}
...
template <typename CharType> static SVGAngle::SVGAngleType StringToAngleType(const CharType*& ptr, const CharType* end)
{
...
static SVGParsingError ParseValue(const CharType* start, const CharType* end, float& value_in_specified_units, SVGAngle::SVGAngleType& unit_type)
{
...
SVGParsingError SVGAngle::SetValueAsString(const String& value)
{
    ...
    SVGParsingError error
        = WTF::VisitCharacters(value, [&](auto chars) { return ParseValue(chars.data(), chars.data() + chars.size(), value_in_specified_units, unit_type); });
...
void SVGAngle::NewValueSpecifiedUnits(SVGAngleType unit_type, float value_in_specified_units)
{
    orient_type_->SetEnumValue(kSVGMarkerOrientAngle);
    unit_type_ = unit_type;
    value_in_specified_units_ = value_in_specified_units;
...
void SVGAngle::ConvertToSpecifiedUnits(SVGAngleType unit_type)
{
    ...
    case kSvgAngletypeTurn:
        ...
        case kSvgAngletypeDeg:
            ...
            break;
        case kSvgAngletypeRad:
            value_in_specified_units_ = Deg2rad(Turn2deg(value_in_specified_units_));
            break;
        case kSvgAngletypeTurn:
        case kSvgAngletypeUnknown:
            NOTREACHED();
    ...
    case kSvgAngletypeRad:
        ...
        case kSvgAngletypeDeg:
            ...
            break;
        case kSvgAngletypeTurn:
            value_in_specified_units_ = Deg2turn(Rad2deg(value_in_specified_units_));
            break;
        case kSvgAngletypeRad:
        case kSvgAngletypeUnknown:
            NOTREACHED();
...
void SVGAngle::CalculateAnimatedValue(const SMILAnimationEffectParameters& parameters, float percentage, unsigned repeat_count, const SVGPropertyBase* from,
    const SVGPropertyBase* to, const SVGPropertyBase* to_at_end_of_duration, const SVGElement*)
{
    ...
    float result
        = ComputeAnimatedNumber(parameters, percentage, repeat_count, from_angle->Value(), to_angle->Value(), To<SVGAngle>(to_at_end_of_duration)->Value());
...
float SVGAngle::CalculateDistance(const SVGPropertyBase* other, const SVGElement*) const
{
