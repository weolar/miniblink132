...
template <> const SVGEnumerationMap& GetEnumerationMap<SVGSpreadMethodType>()
{
...
SVGGradientElement::SVGGradientElement(const QualifiedName& tag_name, Document& document)
    : SVGElement(tag_name, document)
    , SVGURIReference(this)
    , gradient_transform_(MakeGarbageCollected<SVGAnimatedTransformList>(this, svg_names::kGradientTransformAttr, CSSPropertyID::kTransform))
    , spread_method_(MakeGarbageCollected<SVGAnimatedEnumeration<SVGSpreadMethodType>>(this, svg_names::kSpreadMethodAttr, kSVGSpreadMethodPad))
    , gradient_units_(MakeGarbageCollected<SVGAnimatedEnumeration<SVGUnitTypes::SVGUnitType>>(
          this, svg_names::kGradientUnitsAttr, SVGUnitTypes::kSvgUnitTypeObjectboundingbox))
{
}
...
void SVGGradientElement::SvgAttributeChanged(const SvgAttributeChangedParams& params)
{
    ...
    if (attr_name == svg_names::kGradientUnitsAttr || attr_name == svg_names::kGradientTransformAttr || attr_name == svg_names::kSpreadMethodAttr) {
        InvalidateGradient();
        return;
    }
...
Node::InsertionNotificationRequest SVGGradientElement::InsertedInto(ContainerNode& root_parent)
{
...
void SVGGradientElement::CollectCommonAttributes(GradientAttributes& attributes) const
{
    ...
    if (!attributes.HasGradientTransform() && HasTransform(SVGElement::kExcludeMotionTransform)) {
        attributes.SetGradientTransform(CalculateTransform(SVGElement::kExcludeMotionTransform));
    }
...
const SVGGradientElement* SVGGradientElement::ReferencedElement() const
{
    ...
    return DynamicTo<SVGGradientElement>(TargetElementFromIRIString(HrefString(), GetTreeScope()));
}
...
Vector<Gradient::ColorStop> SVGGradientElement::BuildStops() const
{
    ...
    for (const SVGStopElement& stop : Traversal<SVGStopElement>::ChildrenOf(*this)) {
        ...
        stops.push_back(Gradient::ColorStop(offset, stop.StopColorIncludingOpacity()));
    }
    return stops;
}

SVGAnimatedPropertyBase* SVGGradientElement::PropertyFromAttribute(const QualifiedName& attribute_name) const
{
...
{
    SVGAnimatedPropertyBase* attrs[] { gradient_transform_.Get(), spread_method_.Get(), gradient_units_.Get() };
    SynchronizeListOfSVGAttributes(attrs);
    SVGURIReference::SynchronizeAllSVGAttributes();
    SVGElement::SynchronizeAllSVGAttributes();
}

void SVGGradientElement::CollectExtraStyleForPresentationAttribute(MutableCSSPropertyValueSet* style)
{
