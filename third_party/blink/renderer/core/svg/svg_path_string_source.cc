...
template <typename CharType> bool ParseArcFlag(const CharType*& ptr, const CharType* end, bool& flag)
{
    if (ptr >= end) {
        return false;
    }
...
bool MaybeImplicitCommand(unsigned lookahead, SVGPathSegType previous_command, SVGPathSegType& next_command)
{
...
void SVGPathStringSource::SetErrorMark(SVGParseStatus status)
{
    if (error_.Status() != SVGParseStatus::kNoError)
        return;
    size_t locus = is_8bit_source_ ? current_.character8_ - source_.Characters8() : current_.character16_ - source_.Characters16();
...
float SVGPathStringSource::ParseNumberWithError()
{
    ...
    if (is_8bit_source_)
        ...
        error = !ParseNumber(current_.character16_, end_.character16_, number_value);
    if (error) [[unlikely]] {
...
PathSegmentData SVGPathStringSource::ParseSegment()
{
    ...
    unsigned lookahead = is_8bit_source_ ? *current_.character8_ : *current_.character16_;
