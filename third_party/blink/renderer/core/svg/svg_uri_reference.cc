...
public:
    SVGElementReferenceObserver(TreeScope& tree_scope, const AtomicString& id, base::RepeatingClosure closure)
...
SVGAnimatedPropertyBase* SVGURIReference::PropertyFromAttribute(const QualifiedName& attribute_name) const
{
...
SVGURLReferenceResolver::SVGURLReferenceResolver(const String& url_string, const Document& document)
...
{
    return is_local_ || EqualIgnoringFragmentIdentifier(AbsoluteUrl(), document_->Url());
}
...
AtomicString SVGURLReferenceResolver::FragmentIdentifier() const
{
    ...
    return AtomicString(DecodeURLEscapeSequences(AbsoluteUrl().FragmentIdentifier(), DecodeURLMode::kUTF8OrIsomorphic));
}

AtomicString SVGURIReference::FragmentIdentifierFromIRIString(const String& url_string, const TreeScope& tree_scope)
{
...
Element* SVGURIReference::TargetElementFromIRIString(const String& url_string, const TreeScope& tree_scope, AtomicString* fragment_identifier)
{
...
Element* SVGURIReference::ObserveTarget(Member<IdTargetObserver>& observer, SVGElement& context_element)
{
    return ObserveTarget(observer, context_element, HrefString());
}

Element* SVGURIReference::ObserveTarget(Member<IdTargetObserver>& observer, SVGElement& context_element, const String& href_string)
{
    ...
    return ObserveTarget(observer, tree_scope, id, WTF::BindRepeating(&SVGElement::BuildPendingResource, WrapWeakPersistent(&context_element)));
}

Element* SVGURIReference::ObserveTarget(Member<IdTargetObserver>& observer, TreeScope& tree_scope, const AtomicString& id, base::RepeatingClosure closure)
{
    DCHECK(!observer);
    if (id.empty())
        return nullptr;
    observer = MakeGarbageCollected<SVGElementReferenceObserver>(tree_scope, id, std::move(closure));
    return tree_scope.getElementById(id);
}
