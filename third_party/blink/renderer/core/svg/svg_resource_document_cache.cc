...
SVGResourceDocumentCache::SVGResourceDocumentCache(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    : dispose_task_runner_(std::move(task_runner))
{
}

SVGResourceDocumentCache::CacheKey SVGResourceDocumentCache::MakeCacheKey(const FetchParameters& params)
{
    const KURL url_without_fragment = MemoryCache::RemoveFragmentIdentifierIfNeeded(params.Url());
    return { url_without_fragment.GetString(), params.GetResourceRequest().GetMode() };
}
...
void SVGResourceDocumentCache::Put(const CacheKey& key, SVGResourceDocumentContent* content)
{
    ...
    SVGResourceDocumentContent* old_content = std::exchange(result.stored_value->value, content);
    if (old_content) {
        old_content->Dispose();
    }
}
...
void SVGResourceDocumentCache::ProcessCustomWeakness(const LivenessBroker& info)
{
    ...
    const bool all_entries_are_observed = base::ranges::all_of(entries_.Values(), [](SVGResourceDocumentContent* content) { return content->HasObservers(); });
    if (all_entries_are_observed) {
        return;
    }
    dispose_task_pending_ = dispose_task_runner_->PostTask(FROM_HERE, WTF::BindOnce(&SVGResourceDocumentCache::DisposeUnobserved, WrapWeakPersistent(this)));
}

void SVGResourceDocumentCache::Trace(Visitor* visitor) const
{
    visitor->template RegisterWeakCallbackMethod<SVGResourceDocumentCache, &SVGResourceDocumentCache::ProcessCustomWeakness>(this);
