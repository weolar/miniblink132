...
void SVGFEImageElement::FetchImageResource()
{
    ...
    FetchParameters params(ResourceRequest(GetDocument().CompleteURL(HrefString())), options);
...
void SVGFEImageElement::SvgAttributeChanged(const SvgAttributeChangedParams& params)
{
...
Node::InsertionNotificationRequest SVGFEImageElement::InsertedInto(ContainerNode& root_parent)
{
...
const SVGElement* SVGFEImageElement::TargetElement() const
{
    if (cached_image_)
        return nullptr;
    return DynamicTo<SVGElement>(TargetElementFromIRIString(HrefString(), GetTreeScope()));
}
...
FilterEffect* SVGFEImageElement::Build(SVGFilterBuilder*, Filter* filter)
{
    if (cached_image_) {
        ...
        scoped_refptr<Image> image = cached_image_->ErrorOccurred() ? nullptr : cached_image_->GetImage();
        return MakeGarbageCollected<FEImage>(filter, image, preserve_aspect_ratio_->CurrentValue());
    }
    return MakeGarbageCollected<FEImage>(filter, TargetElement(), preserve_aspect_ratio_->CurrentValue());
}
...
SVGAnimatedPropertyBase* SVGFEImageElement::PropertyFromAttribute(const QualifiedName& attribute_name) const
{
