...
public:
    SVGPathTraversalState(PathTraversalState::PathTraversalAction traversal_action, float desired_length = 0)
    ...
    {
        return traversal_state_.total_length_;
    }
    gfx::PointF ComputedPoint() const
    {
        return traversal_state_.current_;
    }

    bool IsDone() const
    {
        return traversal_state_.success_;
    }
...
void SVGPathTraversalState::EmitSegment(const PathSegmentData& segment)
{
    ...
    case kPathSegMoveToAbs:
        traversal_state_.total_length_ += traversal_state_.MoveTo(segment.target_point);
        break;
    case kPathSegLineToAbs:
        traversal_state_.total_length_ += traversal_state_.LineTo(segment.target_point);
        break;
    case kPathSegClosePath:
        traversal_state_.total_length_ += traversal_state_.CloseSubpath();
        break;
    case kPathSegCurveToCubicAbs:
        traversal_state_.total_length_ += traversal_state_.CubicBezierTo(segment.point1, segment.point2, segment.target_point);
        break;
    default:
        NOTREACHED();
    }
...
void ExecuteQuery(const SVGPathByteStream& path_byte_stream, SVGPathTraversalState& traversal_state)
{
...
{
    SVGPathTraversalState traversal_state(PathTraversalState::kTraversalTotalLength);
...
{
    SVGPathTraversalState traversal_state(PathTraversalState::kTraversalPointAtLength, length);
    ExecuteQuery(path_byte_stream_, traversal_state);
