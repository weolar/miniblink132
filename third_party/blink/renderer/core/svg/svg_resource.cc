...
class SVGResource::ImageResourceObserverWrapper : public GarbageCollected<SVGResource::ImageResourceObserverWrapper>, public SVGResourceClient {
public:
...
private:
    ...
    {
        observer_->ImageChanged(static_cast<WrappedImagePtr>(resource), ImageResourceObserver::CanDeferInvalidation::kNo);
    }
...
{
    auto& wrapper = observer_wrappers_.insert(&observer, nullptr).stored_value->value;
    if (!wrapper) {
...
SVGResourceClient* SVGResource::GetObserverResourceClient(ImageResourceObserver& observer)
{
    auto it = observer_wrappers_.find(&observer);
...
LayoutSVGResourceContainer* SVGResource::ResourceContainer(SVGResourceClient& client) const
{
    auto it = clients_.find(&client);
    if (it == clients_.end())
        return nullptr;
...
{
    target_ = SVGURIReference::ObserveTarget(id_observer_, tree_scope, id, WTF::BindRepeating(&LocalSVGResource::TargetChanged, WrapWeakPersistent(this), id));
}
...
void LocalSVGResource::NotifyFilterPrimitiveChanged(SVGFilterPrimitiveStandardAttributes& primitive, const QualifiedName& attribute)
{
    HeapVector<Member<SVGResourceClient>> clients;
    CopyKeysToVector(clients_, clients);
...
ExternalSVGResourceDocumentContent::ExternalSVGResourceDocumentContent(const KURL& url)
    : url_(url)
{
}

void ExternalSVGResourceDocumentContent::Load(Document& document, CrossOriginAttributeValue cross_origin)
{
    ...
    if (cross_origin == kCrossOriginAttributeNotSet) {
        ...
        params.SetCrossOriginAccessControl(execution_context->GetSecurityOrigin(), cross_origin);
    }
...
void ExternalSVGResourceDocumentContent::LoadWithoutCSP(Document& document)
{
    ...
    options.initiator_info.name = fetch_initiator_type_names::kCSS;
    FetchParameters params(ResourceRequest(url_), options);
    params.SetContentSecurityCheck(network::mojom::blink::CSPDisposition::DO_NOT_CHECK);
    params.MutableResourceRequest().SetMode(network::mojom::blink::RequestMode::kSameOrigin);
    document_content_ = SVGResourceDocumentContent::Fetch(params, document);
    if (!document_content_) {
        return;
...
void ExternalSVGResourceDocumentContent::ResourceNotifyFinished(SVGResourceDocumentContent* document_content)
{
...
void ExternalSVGResourceDocumentContent::ResourceContentChanged(SVGResourceDocumentContent* document_content)
{
    DCHECK_EQ(document_content_, document_content);
    if (!target_) {
        return;
...
Element* ExternalSVGResourceDocumentContent::ResolveTarget()
{
    ...
    if (!external_document)
        return nullptr;
    AtomicString decoded_fragment(DecodeURLEscapeSequences(url_.FragmentIdentifier(), DecodeURLMode::kUTF8OrIsomorphic));
...
ExternalSVGResourceImageContent::ExternalSVGResourceImageContent(ImageResourceContent* image_content, const AtomicString& fragment)
...
Element* ExternalSVGResourceImageContent::ResolveTarget()
{
    ...
    AtomicString decoded_fragment(DecodeURLEscapeSequences(fragment_, DecodeURLMode::kUTF8OrIsomorphic));
