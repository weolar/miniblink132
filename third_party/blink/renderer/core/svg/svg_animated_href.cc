...
SVGAnimatedHref::SVGAnimatedHref(SVGElement* context_element)
    ...
    , xlink_href_(MakeGarbageCollected<SVGAnimatedString>(context_element, xlink_names::kHrefAttr))
{
}

SVGAnimatedPropertyBase* SVGAnimatedHref::PropertyFromAttribute(const QualifiedName& attribute_name)
{
...
{
    return attr_name.Matches(svg_names::kHrefAttr) || attr_name.Matches(xlink_names::kHrefAttr);
}
...
{
    UseCounter::Count(ContextElement()->GetDocument(), WebFeature::kSVGHrefBaseVal);
    return BackingString()->SVGAnimatedString::baseVal();
}

void SVGAnimatedHref::setBaseVal(const V8UnionStringOrTrustedScriptURL* value, ExceptionState& exception_state)
{
    UseCounter::Count(ContextElement()->GetDocument(), WebFeature::kSVGHrefBaseVal);
...
{
    UseCounter::Count(ContextElement()->GetDocument(), WebFeature::kSVGHrefAnimVal);
