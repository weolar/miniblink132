...
SVGScriptElement::SVGScriptElement(Document& document, const CreateElementFlags flags)
...
void SVGScriptElement::ParseAttribute(const AttributeModificationParams& params)
{
    if (params.name == html_names::kOnerrorAttr) {
        SetAttributeEventListener(event_type_names::kError,
            JSEventHandlerForContentAttribute::Create(GetExecutionContext(), params.name, params.new_value, JSEventHandler::HandlerType::kOnErrorEventHandler));
    } else {
        SVGElement::ParseAttribute(params);
    }
}

void SVGScriptElement::SvgAttributeChanged(const SvgAttributeChangedParams& params)
{
...
Node::InsertionNotificationRequest SVGScriptElement::InsertedInto(ContainerNode& root_parent)
{
...
bool SVGScriptElement::AllowInlineScriptForCSP(const AtomicString& nonce, const WTF::OrdinalNumber& context_line, const String& script_content)
{
    return GetExecutionContext()->GetContentSecurityPolicyForCurrentWorld()->AllowInline(
        ContentSecurityPolicy::InlineType::kScript, this, script_content, nonce, GetDocument().Url(), context_line);
}
...
Element& SVGScriptElement::CloneWithoutAttributesAndChildren(Document& factory) const
{
...
{
    if (name == svg_names::kTypeAttr || name == svg_names::kHrefAttr || name == xlink_names::kHrefAttr)
        return false;
    return SVGElement::IsAnimatableAttribute(name);
}
#endif

const AttrNameToTrustedType& SVGScriptElement::GetCheckedAttributeTypes() const
{
...
SVGAnimatedPropertyBase* SVGScriptElement::PropertyFromAttribute(const QualifiedName& attribute_name) const
{
    if (SVGAnimatedPropertyBase* ret = SVGURIReference::PropertyFromAttribute(attribute_name); ret) {
        return ret;
    }
