...
SVGRadialGradientElement::SVGRadialGradientElement(Document& document)
    ...
    cx_(MakeGarbageCollected<SVGAnimatedLength>(this, svg_names::kCxAttr, SVGLengthMode::kWidth, SVGLength::Initial::kPercent50))
    , cy_(MakeGarbageCollected<SVGAnimatedLength>(this, svg_names::kCyAttr, SVGLengthMode::kHeight, SVGLength::Initial::kPercent50))
    , r_(MakeGarbageCollected<SVGAnimatedLength>(this, svg_names::kRAttr, SVGLengthMode::kOther, SVGLength::Initial::kPercent50))
    , fx_(MakeGarbageCollected<SVGAnimatedLength>(this, svg_names::kFxAttr, SVGLengthMode::kWidth, SVGLength::Initial::kPercent50))
    , fy_(MakeGarbageCollected<SVGAnimatedLength>(this, svg_names::kFyAttr, SVGLengthMode::kHeight, SVGLength::Initial::kPercent50))
    ,
    // SVG2-Draft Spec: If the fr attribute is not specified, the effect is as
    // if a value of "0%" were specified.
    fr_(MakeGarbageCollected<SVGAnimatedLength>(this, svg_names::kFrAttr, SVGLengthMode::kOther, SVGLength::Initial::kPercent0))
{
}
...
void SVGRadialGradientElement::SvgAttributeChanged(const SvgAttributeChangedParams& params)
{
    const QualifiedName& attr_name = params.name;
    if (attr_name == svg_names::kCxAttr || attr_name == svg_names::kCyAttr || attr_name == svg_names::kFxAttr || attr_name == svg_names::kFyAttr
...
static void SetGradientAttributes(const SVGGradientElement& element, RadialGradientAttributes& attributes, bool is_radial)
{
...
RadialGradientAttributes SVGRadialGradientElement::CollectGradientAttributes() const
{
    ...
    while (true) {
        SetGradientAttributes(*current, attributes, IsA<SVGRadialGradientElement>(*current));
...
{
    return cx_->CurrentValue()->IsRelative() || cy_->CurrentValue()->IsRelative() || r_->CurrentValue()->IsRelative() || fx_->CurrentValue()->IsRelative()
        || fy_->CurrentValue()->IsRelative() || fr_->CurrentValue()->IsRelative();
}

SVGAnimatedPropertyBase* SVGRadialGradientElement::PropertyFromAttribute(const QualifiedName& attribute_name) const
{
...
{
    SVGAnimatedPropertyBase* attrs[] { cx_.Get(), cy_.Get(), r_.Get(), fx_.Get(), fy_.Get(), fr_.Get() };
