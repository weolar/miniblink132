...
SVGFELightElement::SVGFELightElement(const QualifiedName& tag_name, Document& document)
    : SVGElement(tag_name, document)
    , azimuth_(MakeGarbageCollected<SVGAnimatedNumber>(this, svg_names::kAzimuthAttr, 0.0f))
    , elevation_(MakeGarbageCollected<SVGAnimatedNumber>(this, svg_names::kElevationAttr, 0.0f))
    , x_(MakeGarbageCollected<SVGAnimatedNumber>(this, svg_names::kXAttr, 0.0f))
    , y_(MakeGarbageCollected<SVGAnimatedNumber>(this, svg_names::kYAttr, 0.0f))
    , z_(MakeGarbageCollected<SVGAnimatedNumber>(this, svg_names::kZAttr, 0.0f))
    , points_at_x_(MakeGarbageCollected<SVGAnimatedNumber>(this, svg_names::kPointsAtXAttr, 0.0f))
    , points_at_y_(MakeGarbageCollected<SVGAnimatedNumber>(this, svg_names::kPointsAtYAttr, 0.0f))
    , points_at_z_(MakeGarbageCollected<SVGAnimatedNumber>(this, svg_names::kPointsAtZAttr, 0.0f))
    , specular_exponent_(MakeGarbageCollected<SVGAnimatedNumber>(this, svg_names::kSpecularExponentAttr, 1))
    , limiting_cone_angle_(MakeGarbageCollected<SVGAnimatedNumber>(this, svg_names::kLimitingConeAngleAttr, 0.0f))
{
}
...
SVGFELightElement* SVGFELightElement::FindLightElement(const SVGElement& svg_element)
{
...
{
    return gfx::Point3F(x()->CurrentValue()->Value(), y()->CurrentValue()->Value(), z()->CurrentValue()->Value());
}

gfx::Point3F SVGFELightElement::PointsAt() const
{
    return gfx::Point3F(pointsAtX()->CurrentValue()->Value(), pointsAtY()->CurrentValue()->Value(), pointsAtZ()->CurrentValue()->Value());
}

std::optional<bool> SVGFELightElement::SetLightSourceAttribute(FELighting* lighting_effect, const QualifiedName& attr_name) const
{
    ...
    if (attr_name == svg_names::kXAttr || attr_name == svg_names::kYAttr || attr_name == svg_names::kZAttr)
        return light_source->SetPosition(filter->Resolve3dPoint(GetPosition()));
    if (attr_name == svg_names::kPointsAtXAttr || attr_name == svg_names::kPointsAtYAttr || attr_name == svg_names::kPointsAtZAttr)
        return light_source->SetPointsAt(filter->Resolve3dPoint(PointsAt()));
    if (attr_name == svg_names::kSpecularExponentAttr) {
        return light_source->SetSpecularExponent(specularExponent()->CurrentValue()->Value());
    }
    if (attr_name == svg_names::kLimitingConeAngleAttr) {
        return light_source->SetLimitingConeAngle(limitingConeAngle()->CurrentValue()->Value());
    }
    return std::nullopt;
}

void SVGFELightElement::SvgAttributeChanged(const SvgAttributeChangedParams& params)
{
    const QualifiedName& attr_name = params.name;
    if (attr_name == svg_names::kAzimuthAttr || attr_name == svg_names::kElevationAttr || attr_name == svg_names::kXAttr || attr_name == svg_names::kYAttr
        || attr_name == svg_names::kZAttr || attr_name == svg_names::kPointsAtXAttr || attr_name == svg_names::kPointsAtYAttr
        || attr_name == svg_names::kPointsAtZAttr || attr_name == svg_names::kSpecularExponentAttr || attr_name == svg_names::kLimitingConeAngleAttr) {
...
SVGAnimatedPropertyBase* SVGFELightElement::PropertyFromAttribute(const QualifiedName& attribute_name) const
{
...
{
    SVGAnimatedPropertyBase* attrs[] { azimuth_.Get(), elevation_.Get(), x_.Get(), y_.Get(), z_.Get(), points_at_x_.Get(), points_at_y_.Get(),
        points_at_z_.Get(), specular_exponent_.Get(), limiting_cone_angle_.Get() };
