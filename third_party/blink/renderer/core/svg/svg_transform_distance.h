...
public:
    ...
    SVGTransformDistance(const SVGTransform* from_transform, const SVGTransform* to_transform);

    SVGTransformDistance ScaledDistance(float scale_factor) const;
    SVGTransform* AddToSVGTransform(const SVGTransform*) const;

    static SVGTransform* AddSVGTransforms(const SVGTransform*, const SVGTransform*, unsigned repeat_count = 1);
...
private:
    ...
    float cx_;
    float cy_;
    AffineTransform transform_; // for storing scale, translation or matrix transforms
};
} // namespace blink
