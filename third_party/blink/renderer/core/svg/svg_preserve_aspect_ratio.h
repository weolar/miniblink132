...
public:
    ...
    enum SVGMeetOrSliceType { kSvgMeetorsliceUnknown = 0, kSvgMeetorsliceMeet = 1, kSvgMeetorsliceSlice = 2 };
    ...
    void SetAlign(SVGPreserveAspectRatioType align)
    {
        align_ = align;
    }
    SVGPreserveAspectRatioType Align() const
    {
        return align_;
    }
    ...
    SVGMeetOrSliceType MeetOrSlice() const
    {
        return meet_or_slice_;
    }

    void TransformRect(gfx::RectF& dest_rect, gfx::RectF& src_rect) const;

    AffineTransform ComputeTransform(const gfx::RectF& view_box, const gfx::SizeF& viewport_size) const;
...
private:
    template <typename CharType> SVGParsingError ParseInternal(const CharType*& ptr, const CharType* end, bool validate);

    SVGPreserveAspectRatioType align_;
    SVGMeetOrSliceType meet_or_slice_;
};

template <> struct DowncastTraits<SVGPreserveAspectRatio> {
