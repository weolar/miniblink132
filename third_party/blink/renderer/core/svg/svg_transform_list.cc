...
static_assert(static_cast<int>(SVGTransformType::kUnknown) == 0, "index of SVGTransformType::kUnknown has changed");
static_assert(static_cast<int>(SVGTransformType::kMatrix) == 1, "index of SVGTransformType::kMatrix has changed");
static_assert(static_cast<int>(SVGTransformType::kTranslate) == 2, "index of SVGTransformType::kTranslate has changed");
static_assert(static_cast<int>(SVGTransformType::kScale) == 3, "index of SVGTransformType::kScale has changed");
static_assert(static_cast<int>(SVGTransformType::kRotate) == 4, "index of SVGTransformType::kRotate has changed");
static_assert(static_cast<int>(SVGTransformType::kSkewx) == 5, "index of SVGTransformType::kSkewx has changed");
static_assert(static_cast<int>(SVGTransformType::kSkewy) == 6, "index of SVGTransformType::kSkewy has changed");
static_assert(std::size(kRequiredValuesForType) - 1 == static_cast<int>(SVGTransformType::kSkewy), "the number of transform types have changed");
static_assert(std::size(kRequiredValuesForType) == std::size(kOptionalValuesForType), "the arrays should have the same number of elements");
...
{
    return { angle, gfx::PointF(cx, cy), AffineTransform::MakeRotationAroundPoint(angle, cx, cy) };
}
SVGTransformData MatrixTransformValue(const TransformArguments& arguments)
{
    return { 0, gfx::PointF(), AffineTransform(arguments[0], arguments[1], arguments[2], arguments[3], arguments[4], arguments[5]) };
}

template <typename CharType>
SVGParseStatus ParseTransformArgumentsForType(SVGTransformType type, const CharType*& ptr, const CharType* end, TransformArguments& arguments)
{
    ...
    if (arguments.size() != required && arguments.size() != required_with_optional)
...
SVGTransformData TransformDataFromValues(SVGTransformType type, const TransformArguments& arguments)
{
    switch (type) {
...
SVGTransformList::SVGTransformList(SVGTransformType transform_type, const String& value)
{
    if (value.empty())
        return;
    TransformArguments arguments;
    bool success = WTF::VisitCharacters(value, [&](auto chars) {
        const auto* ptr = chars.data();
        const auto* end = ptr + chars.size();
        SVGParseStatus status = ParseTransformArgumentsForType(transform_type, ptr, end, arguments);
        return status == SVGParseStatus::kNoError && !SkipOptionalSVGSpaces(ptr, end);
    });
    if (success)
...
CSSValue* CreateTransformCSSValue(const SVGTransform& transform)
{
    ...
    CSSFunctionValue* transform_value = MakeGarbageCollected<CSSFunctionValue>(function_id);
    switch (function_id) {
    case CSSValueID::kRotate: {
        transform_value->Append(*CSSNumericLiteralValue::Create(transform.Angle(), CSSPrimitiveValue::UnitType::kDegrees));
        gfx::PointF rotation_origin = transform.RotationCenter();
        if (!rotation_origin.IsOrigin()) {
            transform_value->Append(*CSSNumericLiteralValue::Create(rotation_origin.x(), CSSPrimitiveValue::UnitType::kUserUnits));
            transform_value->Append(*CSSNumericLiteralValue::Create(rotation_origin.y(), CSSPrimitiveValue::UnitType::kUserUnits));
        }
        break;
    }
    ...
    case CSSValueID::kMatrix:
        transform_value->Append(*CSSNumericLiteralValue::Create(transform.Matrix().A(), CSSPrimitiveValue::UnitType::kNumber));
        transform_value->Append(*CSSNumericLiteralValue::Create(transform.Matrix().B(), CSSPrimitiveValue::UnitType::kNumber));
        transform_value->Append(*CSSNumericLiteralValue::Create(transform.Matrix().C(), CSSPrimitiveValue::UnitType::kNumber));
        transform_value->Append(*CSSNumericLiteralValue::Create(transform.Matrix().D(), CSSPrimitiveValue::UnitType::kNumber));
        transform_value->Append(*CSSNumericLiteralValue::Create(transform.Matrix().E(), CSSPrimitiveValue::UnitType::kNumber));
        transform_value->Append(*CSSNumericLiteralValue::Create(transform.Matrix().F(), CSSPrimitiveValue::UnitType::kNumber));
        break;
    ...
    case CSSValueID::kTranslate:
        transform_value->Append(*CSSNumericLiteralValue::Create(transform.Matrix().E(), CSSPrimitiveValue::UnitType::kUserUnits));
        transform_value->Append(*CSSNumericLiteralValue::Create(transform.Matrix().F(), CSSPrimitiveValue::UnitType::kUserUnits));
        break;
    default:
        NOTREACHED();
    }
...
template <typename CharType> SVGTransformType ParseAndSkipTransformType(const CharType*& ptr, const CharType* end)
{
    if (ptr >= end)
...
template <typename CharType> SVGParsingError SVGTransformList::ParseInternal(const CharType*& ptr, const CharType* end)
{
    Clear();
    ...
    while (SkipOptionalSVGSpaces(ptr, end)) {
        ...
        if (transform_type == SVGTransformType::kUnknown)
            return SVGParsingError(SVGParseStatus::kExpectedTransformFunction, ptr - start);

        if (!SkipOptionalSVGSpaces(ptr, end) || *ptr != '(')
            return SVGParsingError(SVGParseStatus::kExpectedStartOfArguments, ptr - start);
        ptr++;
        ...
        DCHECK_GE(arguments.size(), kRequiredValuesForType[static_cast<int>(transform_type)]);

        if (!SkipOptionalSVGSpaces(ptr, end) || *ptr != ')')
            return SVGParsingError(SVGParseStatus::kExpectedEndOfArguments, ptr - start);
        ptr++;
...
void SVGTransformList::Add(const SVGPropertyBase* other, const SVGElement* context_element)
{
    if (IsEmpty())
        return;
...
void SVGTransformList::CalculateAnimatedValue(const SMILAnimationEffectParameters& parameters, float percentage, unsigned repeat_count,
    const SVGPropertyBase* from_value, const SVGPropertyBase* to_value, const SVGPropertyBase* to_at_end_of_duration_value, const SVGElement* context_element)
{
    ...
    auto* to_at_end_of_duration_list = To<SVGTransformList>(to_at_end_of_duration_value);
    ...
    if (from_list->length() && from_list->at(0)->TransformType() == to_transform->TransformType())
        effective_from = from_list->at(0);
    else
        effective_from = MakeGarbageCollected<SVGTransform>(to_transform->TransformType(), SVGTransform::kConstructZeroTransform);

    SVGTransform* current_transform = SVGTransformDistance(effective_from, to_transform).ScaledDistance(percentage).AddToSVGTransform(effective_from);

    // Handle accumulation.
    if (repeat_count && parameters.is_cumulative) {
        const SVGTransform* effective_to_at_end = !to_at_end_of_duration_list->IsEmpty()
            ? to_at_end_of_duration_list->at(0)
            : MakeGarbageCollected<SVGTransform>(to_transform->TransformType(), SVGTransform::kConstructZeroTransform);
        current_transform = SVGTransformDistance::AddSVGTransforms(current_transform, effective_to_at_end, repeat_count);
    }
...
float SVGTransformList::CalculateDistance(const SVGPropertyBase* to_value, const SVGElement*) const
{
