...
SVGAElement::SVGAElement(Document& document)
    ...
    , svg_target_(MakeGarbageCollected<SVGAnimatedString>(this, svg_names::kTargetAttr))
{
}
...
void SVGAElement::DefaultEventHandler(Event& event)
{
    if (IsLink()) {
        ...
        if (IsLinkClick(event)) {
            ...
            if (url[0] == '#') {
                Element* target_element = GetTreeScope().getElementById(AtomicString(url.Substring(1)));
                if (auto* svg_smil_element = DynamicTo<SVGSMILElement>(target_element)) {
            ...
            FrameLoadRequest frame_request(GetDocument().domWindow(), ResourceRequest(GetDocument().CompleteURL(url)));

            AtomicString target = frame_request.CleanNavigationTarget(AtomicString(svg_target_->CurrentValue()->Value()));
            ...
            frame_request.GetResourceRequest().SetHasUserGesture(LocalFrame::HasTransientUserActivation(GetDocument().GetFrame()));

            Frame* frame = GetDocument().GetFrame()->Tree().FindOrCreateFrameForNavigation(frame_request, target).frame;
            if (!frame)
                return;
...
Element* SVGAElement::interestTargetElement()
{
    ...
    return GetElementAttributeResolvingReferenceTarget(svg_names::kInteresttargetAttr);
}
...
FocusableState SVGAElement::SupportsFocus(UpdateBehavior update_behavior) const
{
    if (IsEditable(*this)) {
...
bool SVGAElement::ShouldHaveFocusAppearance() const
{
    return (GetDocument().LastFocusType() != mojom::blink::FocusType::kMouse)
        || SVGGraphicsElement::SupportsFocus(UpdateBehavior::kNoneForFocusManagement) != FocusableState::kNotFocusable;
}

bool SVGAElement::IsURLAttribute(const Attribute& attribute) const
{
    return attribute.GetName().LocalName() == html_names::kHrefAttr || SVGGraphicsElement::IsURLAttribute(attribute);
}
...
SVGAnimatedPropertyBase* SVGAElement::PropertyFromAttribute(const QualifiedName& attribute_name) const
{
