...
public:
    ...
    bool HasMotionTransform() const
    {
        return HasSVGRareData();
    }
    // Apply any "motion transform" contribution (if existing.)
    void ApplyMotionTransform(AffineTransform&) const;

    enum ApplyMotionTransformTag { kExcludeMotionTransform, kIncludeMotionTransform };
    ...
    virtual bool IsSVGGeometryElement() const
    {
        return false;
    }
    ...
    virtual bool IsTextPositioning() const
    {
        return false;
    }
    virtual bool IsStructurallyExternal() const
    {
        return false;
    }
    ...
    virtual SVGAnimatedPropertyBase* PropertyFromAttribute(const QualifiedName& attribute_name) const;
    static AnimatedPropertyType AnimatedPropertyTypeForCSSAttribute(const QualifiedName& attribute_name);

    void SendSVGLoadEventToSelfAndAncestorChainIfPossible();
    bool SendSVGLoadEventIfPossible();

    virtual AffineTransform* AnimateMotionTransform()
    {
        return nullptr;
    }
    ...
    const ComputedStyle* CustomStyleForLayoutObject(const StyleRecalcContext&) final;
    ...
    template <typename InvalidationFunction> void NotifyIncomingReferences(InvalidationFunction&&);
    ...
    static void SynchronizeListOfSVGAttributes(const base::span<SVGAnimatedPropertyBase*> attributes);

    bool HasFocusEventListeners() const;

protected:
    SVGElement(const QualifiedName&, Document&, ConstructionType = kCreateSVGElement);
    ...
    void UpdatePresentationAttributeStyle(CSSPropertyID, const QualifiedName&, const AtomicString& value);
    ...
    void AddPropertyToPresentationAttributeStyleWithCache(MutableCSSPropertyValueSet*, CSSPropertyID, const AtomicString& value);
    void AddAnimatedPropertyToPresentationAttributeStyle(const SVGAnimatedPropertyBase& property, MutableCSSPropertyValueSet* style);
    void AddAnimatedPropertiesToPresentationAttributeStyle(const base::span<const SVGAnimatedPropertyBase*> properties, MutableCSSPropertyValueSet* style);
    ...
    static CSSPropertyID CssPropertyIdForSVGAttributeName(const ExecutionContext*, const QualifiedName&);
    ...
    inline bool HasSVGRareData() const
    {
        return svg_rare_data_ != nullptr;
    }
...
private:
    FRIEND_TEST_ALL_PREFIXES(SVGElementTest, BaseComputedStyleForSMILWithContainerQueries);
    ...
    void UpdateWebAnimatedAttributeOnBaseValChange(const SVGAnimatedPropertyBase&);
...
template <typename InvalidationFunction> void SVGElement::NotifyIncomingReferences(InvalidationFunction&& invalidation_function)
{
...
struct SVGAttributeHashTranslator {
    ...
    {
        if (key.HasPrefix()) {
            QualifiedNameComponents components = { g_null_atom.Impl(), key.LocalName().Impl(), key.NamespaceURI().Impl() };
...
template <> struct DowncastTraits<SVGElement> {
