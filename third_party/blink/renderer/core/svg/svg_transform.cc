...
SVGTransform::SVGTransform(SVGTransformType transform_type, ConstructionMode mode)
...
{
    return MakeGarbageCollected<SVGTransform>(transform_type_, angle_, center_, matrix_);
}
...
{
    return gfx::Vector2dF(ClampTo<float>(matrix_.E()), ClampTo<float>(matrix_.F()));
}
...
{
    return gfx::Vector2dF(ClampTo<float>(matrix_.A()), ClampTo<float>(matrix_.D()));
}
...
gfx::PointF DecomposeRotationCenter(const AffineTransform& matrix, float angle)
{
...
void SVGTransform::CalculateAnimatedValue(
...
float SVGTransform::CalculateDistance(const SVGPropertyBase*, const SVGElement*) const
{
    // SVGTransform is not animated by itself.
    NOTREACHED();
}
