...
SVGFilterPrimitiveStandardAttributes::SVGFilterPrimitiveStandardAttributes(const QualifiedName& tag_name, Document& document)
    : SVGElement(tag_name, document)
    ,
    // Spec: If the x/y attribute is not specified, the effect is as if a
    // value of "0%" were specified.
    x_(MakeGarbageCollected<SVGAnimatedLength>(this, svg_names::kXAttr, SVGLengthMode::kWidth, SVGLength::Initial::kPercent0))
    , y_(MakeGarbageCollected<SVGAnimatedLength>(this, svg_names::kYAttr, SVGLengthMode::kHeight, SVGLength::Initial::kPercent0))
    ,
    // Spec: If the width/height attribute is not specified, the effect is as
    // if a value of "100%" were specified.
    width_(MakeGarbageCollected<SVGAnimatedLength>(this, svg_names::kWidthAttr, SVGLengthMode::kWidth, SVGLength::Initial::kPercent100))
    , height_(MakeGarbageCollected<SVGAnimatedLength>(this, svg_names::kHeightAttr, SVGLengthMode::kHeight, SVGLength::Initial::kPercent100))
    , result_(MakeGarbageCollected<SVGAnimatedString>(this, svg_names::kResultAttr))
{
}
...
bool SVGFilterPrimitiveStandardAttributes::SetFilterEffectAttribute(FilterEffect* effect, const QualifiedName& attr_name)
{
    DCHECK(attr_name == svg_names::kColorInterpolationFiltersAttr);
    DCHECK(GetLayoutObject());
    EColorInterpolation color_interpolation = GetLayoutObject()->StyleRef().ColorInterpolationFilters();
    InterpolationSpace resolved_interpolation_space = SVGFilterBuilder::ResolveInterpolationSpace(color_interpolation);
...
void SVGFilterPrimitiveStandardAttributes::SvgAttributeChanged(const SvgAttributeChangedParams& params)
{
...
static gfx::RectF DefaultFilterPrimitiveSubregion(FilterEffect* filter_effect)
{
    ...
    if (filter_effect->GetFilterEffectType() == kFilterEffectTypeTile || !filter_effect->NumberOfEffectInputs())
...
void SVGFilterPrimitiveStandardAttributes::SetStandardAttributes(
    FilterEffect* filter_effect, SVGUnitTypes::SVGUnitType primitive_units, const gfx::RectF& reference_box) const
{
    DCHECK(filter_effect);

    gfx::RectF subregion = DefaultFilterPrimitiveSubregion(filter_effect);
    gfx::RectF primitive_boundaries = LayoutSVGResourceContainer::ResolveRectangle(*this, primitive_units, reference_box);
...
bool SVGFilterPrimitiveStandardAttributes::LayoutObjectIsNeeded(const DisplayStyle& style) const
{
...
{
    auto* svg_parent = DynamicTo<SVGFilterPrimitiveStandardAttributes>(element.parentElement());
    if (!svg_parent)
        return;
    svg_parent->Invalidate();
}

SVGAnimatedPropertyBase* SVGFilterPrimitiveStandardAttributes::PropertyFromAttribute(const QualifiedName& attribute_name) const
{
...
{
    SVGAnimatedPropertyBase* attrs[] { x_.Get(), y_.Get(), width_.Get(), height_.Get(), result_.Get() };
