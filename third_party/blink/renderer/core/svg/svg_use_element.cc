...
SVGUseElement::SVGUseElement(Document& document)
    ...
    , width_(MakeGarbageCollected<SVGAnimatedLength>(this, svg_names::kWidthAttr, SVGLengthMode::kWidth, SVGLength::Initial::kUnitlessZero))
    , height_(MakeGarbageCollected<SVGAnimatedLength>(this, svg_names::kHeightAttr, SVGLengthMode::kHeight, SVGLength::Initial::kUnitlessZero))
...
Node::InsertionNotificationRequest SVGUseElement::InsertedInto(ContainerNode& root_parent)
{
...
static void TransferUseWidthAndHeightIfNeeded(const SVGUseElement& use, SVGElement& shadow_element, const SVGElement& original_element)
{
    ...
    if (!IsA<SVGSymbolElement>(original_element) && !IsA<SVGSVGElement>(original_element))
        return;
    ...
    AtomicString width_value(use.width()->IsSpecified() ? use.width()->CurrentValue()->ValueAsString() : original_element.getAttribute(svg_names::kWidthAttr));
    shadow_element.setAttribute(svg_names::kWidthAttr, width_value);
    AtomicString height_value(
        use.height()->IsSpecified() ? use.height()->CurrentValue()->ValueAsString() : original_element.getAttribute(svg_names::kHeightAttr));
    shadow_element.setAttribute(svg_names::kHeightAttr, height_value);
}

bool SVGUseElement::IsStructurallyExternal() const
{
    return !element_url_is_local_ && !EqualIgnoringFragmentIdentifier(element_url_, GetDocument().Url());
}
...
void SVGUseElement::UpdateDocumentContent(SVGResourceDocumentContent* document_content)
{
    ...
    if (document_content_) {
        load_event_delayer_ = std::make_unique<IncrementLoadEventDelayCount>(GetDocument());
...
void SVGUseElement::UpdateTargetReference()
{
    ...
    if (!element_url_.HasFragmentIdentifier() || (document_content_ && EqualIgnoringFragmentIdentifier(element_url_, document_content_->Url()))) {
        return;
    }
    ...
    if (element_url_.ProtocolIsData()) {
        Deprecation::CountDeprecation(GetDocument().domWindow(), WebFeature::kDataUrlInSvgUse);
    }
    ...
    params.MutableResourceRequest().SetMode(network::mojom::blink::RequestMode::kSameOrigin);
    auto* document_content = SVGResourceDocumentContent::Fetch(params, *context_document);
    UpdateDocumentContent(document_content);
}

void SVGUseElement::SvgAttributeChanged(const SvgAttributeChangedParams& params)
{
    const QualifiedName& attr_name = params.name;
    if (attr_name == svg_names::kXAttr || attr_name == svg_names::kYAttr || attr_name == svg_names::kWidthAttr || attr_name == svg_names::kHeightAttr) {
        ...
        if (SVGElement* instance_root = InstanceRoot()) {
            ...
            TransferUseWidthAndHeightIfNeeded(*this, *instance_root, *instance_root->CorrespondingElement());
        }
...
static bool IsDisallowedElement(const Element& element)
{
    ...
    return !allowed_element_tags.Contains<SVGAttributeHashTranslator>(element.TagQName());
}
...
Element* SVGUseElement::ResolveTargetElement()
{
    if (!element_url_.HasFragmentIdentifier())
        return nullptr;
    AtomicString element_identifier(DecodeURLEscapeSequences(element_url_.FragmentIdentifier(), DecodeURLMode::kUTF8OrIsomorphic));
    ...
    if (!IsStructurallyExternal()) {
        ...
        if (InUseShadowTree()) {
            ...
            return ObserveTarget(target_id_observer_, OriginatingTreeScope(), element_identifier,
                WTF::BindRepeating(&SVGUseElement::InvalidateTargetReference, WrapWeakPersistent(this)));
        }
    }
    if (!document_content_) {
        return nullptr;
    }
    external_resource_target_ = document_content_->GetResourceTarget(element_identifier);
...
String SVGUseElement::title() const
{
    ...
    // shadow tree.
    if (SVGElement* instance_root = InstanceRoot()) {
        if (Element* title_element = Traversal<SVGTitleElement>::FirstChild(*instance_root))
            return title_element->innerText();
    }
...
static void PostProcessInstanceTree(SVGElement& target_root, SVGElement& instance_root)
{
    ...
    while (target_element) {
        ...
        DCHECK(!IsA<SVGElement>(*instance_element) || !To<SVGElement>(*instance_element).CorrespondingElement());
        if (IsDisallowedElement(*target_element)) {
            Element* instance_next = ElementTraversal::NextSkippingChildren(*instance_element, &instance_root);
            ...
            target_element = ElementTraversal::NextSkippingChildren(*target_element, &target_root);
            instance_element = instance_next;
        } else {
            // Set up the corresponding element association.
            if (auto* svg_instance_element = DynamicTo<SVGElement>(instance_element)) {
                svg_instance_element->SetCorrespondingElement(To<SVGElement>(target_element));
            }
            target_element = ElementTraversal::Next(*target_element, &target_root);
            instance_element = ElementTraversal::Next(*instance_element, &instance_root);
        }
    }
    DCHECK(!instance_element);
}

static void MoveChildrenToReplacementElement(ContainerNode& source_root, ContainerNode& destination_root)
{
...
SVGElement* SVGUseElement::CreateInstanceTree(SVGElement& target_root) const
{
    ...
    SVGElement* instance_root = &To<SVGElement>(target_root.CloneWithChildren(data, /*document*/ nullptr, /*append_to*/ nullptr));
    if (IsA<SVGSymbolElement>(target_root)) {
        ...
        auto* svg_element = MakeGarbageCollected<SVGSVGElement>(target_root.GetDocument());
...
void SVGUseElement::AttachShadowTree(SVGElement& target)
{
    ...
    for (SVGElement& instance : Traversal<SVGElement>::DescendantsOf(UseShadowRoot())) {
        ...
        if (EventTargetData* data = corresponding_element->GetEventTargetData()) {
            data->event_listener_map.CopyEventListenersNotCreatedFromMarkupToTarget(&instance);
        }
...
{
    return IsA<SVGPathElement>(element) || IsA<SVGRectElement>(element) || IsA<SVGCircleElement>(element) || IsA<SVGEllipseElement>(element)
        || IsA<SVGPolygonElement>(element) || IsA<SVGPolylineElement>(element) || IsA<SVGTextElement>(element);
}
...
bool SVGUseElement::HasCycleUseReferencing(const ContainerNode& target_instance, const SVGElement& target) const
{
    ...
    auto* element = DynamicTo<SVGElement>(target_instance.ParentOrShadowHostElement());
    while (element) {
        if (element->HasID() && element->GetIdAttribute() == target_id && element->GetDocument() == target.GetDocument())
            return true;
...
{
    DCHECK(GetLayoutObject());
    auto& transformable_container = To<LayoutSVGTransformableContainer>(*GetLayoutObject());
...
void SVGUseElement::QueueOrDispatchPendingEvent(const AtomicString& event_name)
{
    if (GetDocument().GetExecutionContext() && GetDocument().GetExecutionContext()->is_in_back_forward_cache()) {
...
void SVGUseElement::ResourceNotifyFinished(SVGResourceDocumentContent* document_content)
{
    ...
    const AtomicString& event_name = is_error ? event_type_names::kError : event_type_names::kLoad;
    DCHECK(!pending_event_.IsActive());
    pending_event_ = PostCancellableTask(*GetDocument().GetTaskRunner(TaskType::kDOMManipulation), FROM_HERE,
        WTF::BindOnce(&SVGUseElement::QueueOrDispatchPendingEvent, WrapPersistent(this), event_name));
}

SVGAnimatedPropertyBase* SVGUseElement::PropertyFromAttribute(const QualifiedName& attribute_name) const
{
    if (attribute_name == svg_names::kXAttr) {
        ...
        SVGAnimatedPropertyBase* ret = SVGURIReference::PropertyFromAttribute(attribute_name);
        if (ret) {
            return ret;
        } else {
...
void SVGUseElement::CollectExtraStyleForPresentationAttribute(MutableCSSPropertyValueSet* style)
{
    auto pres_attrs = std::to_array<const SVGAnimatedPropertyBase*>({ x_.Get(), y_.Get() });
