...
template <typename CharType> SVGParsingError SVGNumberList::Parse(const CharType*& ptr, const CharType* end)
{
...
void SVGNumberList::Add(const SVGPropertyBase* other, const SVGElement* context_element)
{
...
void SVGNumberList::CalculateAnimatedValue(const SMILAnimationEffectParameters& parameters, float percentage, unsigned repeat_count,
    const SVGPropertyBase* from_value, const SVGPropertyBase* to_value, const SVGPropertyBase* to_at_end_of_duration_value, const SVGElement* context_element)
{
    ...
    auto* to_at_end_of_duration_list = To<SVGNumberList>(to_at_end_of_duration_value);
    ...
    const SVGNumber* neutral = needs_neutral_element ? CreatePaddingItem() : nullptr;
    for (uint32_t i = 0; i < to_list_size; ++i) {
        const SVGNumber* from = from_list_size ? from_list->at(i) : neutral;
        const SVGNumber* to_at_end = i < to_at_end_of_duration_list_size ? to_at_end_of_duration_list->at(i) : neutral;
        at(i)->CalculateAnimatedValue(parameters, percentage, repeat_count, from, to_list->at(i), to_at_end, context_element);
    }
}

float SVGNumberList::CalculateDistance(const SVGPropertyBase* to, const SVGElement*) const
{
