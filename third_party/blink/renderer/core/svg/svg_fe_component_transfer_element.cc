...
SVGFEComponentTransferElement::SVGFEComponentTransferElement(Document& document)
    : SVGFilterPrimitiveStandardAttributes(svg_names::kFEComponentTransferTag, document)
...
void SVGFEComponentTransferElement::SvgAttributeChanged(const SvgAttributeChangedParams& params)
{
...
FilterEffect* SVGFEComponentTransferElement::Build(SVGFilterBuilder* filter_builder, Filter* filter)
{
    ...
    auto* effect = MakeGarbageCollected<FEComponentTransfer>(filter, red, green, blue, alpha);
    effect->InputEffects().push_back(input1);
    return effect;
}

SVGAnimatedPropertyBase* SVGFEComponentTransferElement::PropertyFromAttribute(const QualifiedName& attribute_name) const
{
