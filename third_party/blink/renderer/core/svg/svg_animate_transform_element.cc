...
SVGPropertyBase* SVGAnimateTransformElement::CreateUnderlyingValueForAnimation() const
{
    DCHECK(IsAnimatingSVGDom());
    return To<SVGTransformList>(target_property_->BaseValueBase()).Clone();
}

SVGPropertyBase* SVGAnimateTransformElement::ParseValue(const String& value) const
{
    DCHECK(IsAnimatingSVGDom());
...
void SVGAnimateTransformElement::ParseAttribute(const AttributeModificationParams& params)
{
