...
String BuildStringFromByteStream(const SVGPathByteStream& stream, PathSerializationFormat format)
{
    if (stream.IsEmpty())
        return String();
...
SVGParsingError BuildByteStreamFromString(const StringView& path_string, SVGPathByteStreamBuilder& builder)
{
    if (path_string.empty())
