...
static gfx::PointF ReflectedPoint(const gfx::PointF& reflect_in, const gfx::PointF& point_to_reflect)
{
    return gfx::PointF(2 * reflect_in.x() - point_to_reflect.x(), 2 * reflect_in.y() - point_to_reflect.y());
}
...
static gfx::PointF BlendPoints(const gfx::PointF& p1, const gfx::PointF& p2)
{
    ...
    return gfx::PointF((p1.x() + 2 * p2.x()) * kOneOverThree, (p1.y() + 2 * p2.y()) * kOneOverThree);
}

static inline bool IsCubicCommand(SVGPathSegType command)
{
    return command == kPathSegCurveToCubicAbs || command == kPathSegCurveToCubicRel || command == kPathSegCurveToCubicSmoothAbs
        || command == kPathSegCurveToCubicSmoothRel;
}

static inline bool IsQuadraticCommand(SVGPathSegType command)
{
    return command == kPathSegCurveToQuadraticAbs || command == kPathSegCurveToQuadraticRel || command == kPathSegCurveToQuadraticSmoothAbs
...
bool SVGPathNormalizer::DecomposeArcToCubic(const gfx::PointF& current_point, const PathSegmentData& arc_segment)
{
    ...
    gfx::PointF transformed_mid_point = point_transform.MapPoint(gfx::PointF(mid_point_distance.x(), mid_point_distance.y()));
    ...
    for (int i = 0; i < segments; ++i) {
        ...
        point1 = gfx::PointF(cos_start_theta - t * sin_start_theta, sin_start_theta + t * cos_start_theta);
