...
gfx::SizeF SVGViewportResolver::ResolveViewport() const
{
    ...
    for (const LayoutObject* object = context_object_->Parent(); object; object = object->Parent()) {
...
float SVGViewportResolver::ViewportDimension(SVGLengthMode mode) const
{
    ...
    case SVGLengthMode::kOther:
        ...
        return ClampTo<float>(std::sqrt(gfx::Vector2dF(viewport_size.width(), viewport_size.height()).LengthSquared() / 2));
    }
    NOTREACHED();
}
...
float ValueForLength(const Length& length, const ComputedStyle& style, float dimension)
{
    return ValueForLength(length, style.EffectiveZoom(), dimension);
}

float ValueForLength(const Length& length, const SVGViewportResolver& viewport_resolver, float zoom, SVGLengthMode mode)
{
    // The viewport will be unaffected by zoom.
    const float dimension = length.MayHavePercentDependence() ? viewport_resolver.ViewportDimension(mode) : 0;
    return ValueForLength(length, zoom, dimension);
}

float ValueForLength(const Length& length, const SVGViewportResolver& viewport_resolver, const ComputedStyle& style, SVGLengthMode mode)
{
    return ValueForLength(length, viewport_resolver, style.EffectiveZoom(), mode);
}

float ValueForLength(const UnzoomedLength& unzoomed_length, const SVGViewportResolver& viewport_resolver, SVGLengthMode mode)
{
    return ValueForLength(unzoomed_length.length(), viewport_resolver, 1, mode);
}

gfx::Vector2dF VectorForLengthPair(const Length& x_length, const Length& y_length, float zoom, const gfx::SizeF& viewport_size)
{
    ...
    return gfx::Vector2dF(
        ValueForLength(x_length, zoom, viewport_size_considering_auto.width()), ValueForLength(y_length, zoom, viewport_size_considering_auto.height()));
}

gfx::Vector2dF VectorForLengthPair(const Length& x_length, const Length& y_length, const SVGViewportResolver& viewport_resolver, const ComputedStyle& style)
{
    gfx::SizeF viewport_size;
    if (x_length.MayHavePercentDependence() || y_length.MayHavePercentDependence()) {
        viewport_size = viewport_resolver.ResolveViewport();
    }
    return VectorForLengthPair(x_length, y_length, style.EffectiveZoom(), viewport_size);
}
