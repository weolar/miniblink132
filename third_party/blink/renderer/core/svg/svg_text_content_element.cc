...
{
    return object && (object->IsSVGText() || object->IsInLayoutNGInlineFormattingContext());
}

} // namespace

template <> const SVGEnumerationMap& GetEnumerationMap<SVGLengthAdjustType>()
{
...
public:
    SVGAnimatedTextLength(SVGTextContentElement* context_element)
        : SVGAnimatedLength(context_element, svg_names::kTextLengthAttr, SVGLengthMode::kWidth, SVGLength::Initial::kUnitlessZero)
    {
    }
    ...
    SVGLengthTearOff* baseVal() override
    {
        ...
        if (!text_content_element->TextLengthIsSpecifiedByUser())
            BaseValue()->NewValueSpecifiedUnits(CSSPrimitiveValue::UnitType::kNumber, text_content_element->getComputedTextLength());

        return SVGAnimatedLength::baseVal();
    }
};

SVGTextContentElement::SVGTextContentElement(const QualifiedName& tag_name, Document& document)
    ...
    , length_adjust_(MakeGarbageCollected<SVGAnimatedEnumeration<SVGLengthAdjustType>>(this, svg_names::kLengthAdjustAttr, kSVGLengthAdjustSpacing))
{
}
...
float SVGTextContentElement::getSubStringLength(unsigned charnum, unsigned nchars, ExceptionState& exception_state)
{
    GetDocument().UpdateStyleAndLayoutForNode(this, DocumentUpdateReason::kJavaScript);
    ...
    if (charnum >= number_of_chars) {
        exception_state.ThrowDOMException(
            DOMExceptionCode::kIndexSizeError, ExceptionMessages::IndexExceedsMaximumBound("charnum", charnum, getNumberOfChars()));
        return 0.0f;
    }
...
SVGPointTearOff* SVGTextContentElement::getStartPositionOfChar(unsigned charnum, ExceptionState& exception_state)
{
    GetDocument().UpdateStyleAndLayoutForNode(this, DocumentUpdateReason::kJavaScript);
...
SVGPointTearOff* SVGTextContentElement::getEndPositionOfChar(unsigned charnum, ExceptionState& exception_state)
{
    GetDocument().UpdateStyleAndLayoutForNode(this, DocumentUpdateReason::kJavaScript);
...
SVGRectTearOff* SVGTextContentElement::getExtentOfChar(unsigned charnum, ExceptionState& exception_state)
{
    GetDocument().UpdateStyleAndLayoutForNode(this, DocumentUpdateReason::kJavaScript);
...
float SVGTextContentElement::getRotationOfChar(unsigned charnum, ExceptionState& exception_state)
{
    GetDocument().UpdateStyleAndLayoutForNode(this, DocumentUpdateReason::kJavaScript);
...
int SVGTextContentElement::getCharNumAtPosition(SVGPointTearOff* point, ExceptionState& exception_state)
{
    GetDocument().UpdateStyleAndLayoutForNode(this, DocumentUpdateReason::kJavaScript);
    auto* layout_object = GetLayoutObject();
    if (IsNGTextOrInline(layout_object)) {
        return SvgTextQuery(*layout_object).CharacterNumberAtPosition(point->Target()->Value());
    }
    return -1;
}

void SVGTextContentElement::selectSubString(unsigned charnum, unsigned nchars, ExceptionState& exception_state)
{
    unsigned number_of_chars = getNumberOfChars();
    if (charnum >= number_of_chars) {
        exception_state.ThrowDOMException(
...
void SVGTextContentElement::CollectStyleForPresentationAttribute(const QualifiedName& name, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
    if (name.Matches(xml_names::kSpaceAttr)) {
        ...
        if (value == preserve_string) {
            ...
            UseCounter::Count(GetDocument(), WebFeature::kWhiteSpaceNowrapFromXMLSpace);
            // Longhands of `white-space: nowrap`.
            AddPropertyToPresentationAttributeStyle(style, CSSPropertyID::kWhiteSpaceCollapse, CSSValueID::kCollapse);
            AddPropertyToPresentationAttributeStyle(style, CSSPropertyID::kTextWrapMode, CSSValueID::kNowrap);
        }
    } else {
...
void SVGTextContentElement::SvgAttributeChanged(const SvgAttributeChangedParams& params)
{
    const QualifiedName& attr_name = params.name;
    if (attr_name == svg_names::kTextLengthAttr)
        text_length_is_specified_by_user_ = true;

    if (attr_name == svg_names::kTextLengthAttr || attr_name == svg_names::kLengthAdjustAttr || attr_name == xml_names::kSpaceAttr) {
        if (LayoutObject* layout_object = GetLayoutObject()) {
            if (auto* ng_text = LayoutSVGText::LocateLayoutSVGTextAncestor(layout_object)) {
...
SVGTextContentElement* SVGTextContentElement::ElementFromLineLayoutItem(const LineLayoutItem& line_layout_item)
{
    return nullptr;
}

SVGAnimatedPropertyBase* SVGTextContentElement::PropertyFromAttribute(const QualifiedName& attribute_name) const
{
