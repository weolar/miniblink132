...
SVGAnimationElement::SVGAnimationElement(const QualifiedName& tag_name, Document& document)
...
static bool ParseKeyTimes(const String& string, HeapVector<float>& result, bool verify_order)
{
    result.clear();
    Vector<String> parse_list;
...
template <typename CharType> static bool ParseKeySplinesInternal(const CharType* ptr, const CharType* end, Vector<gfx::CubicBezier>& result)
{
    ...
    while (ptr < end) {
        ...
        if (!IsInZeroToOneRange(cp1x) || !IsInZeroToOneRange(cp1y) || !IsInZeroToOneRange(cp2x) || !IsInZeroToOneRange(cp2y))
            return false;
...
static bool ParseKeySplines(const String& string, Vector<gfx::CubicBezier>& result)
{
    ...
    if (string.empty())
        return true;
    bool parsed = WTF::VisitCharacters(string, [&](auto chars) { return ParseKeySplinesInternal(chars.data(), chars.data() + chars.size(), result); });
    if (!parsed) {
        result.clear();
        return false;
    }
...
void SVGAnimationElement::ParseAttribute(const AttributeModificationParams& params)
{
    const QualifiedName& name = params.name;
    if (name == svg_names::kValuesAttr) {
        if (!ParseValues(params.new_value, values_)) {
            ReportAttributeParsingError(SVGParseStatus::kParsingFailed, name, params.new_value);
        }
...
void SVGAnimationElement::UnregisterAnimation(const QualifiedName& attribute_name)
{
...
void SVGAnimationElement::RegisterAnimation(const QualifiedName& attribute_name)
{
...
float SVGAnimationElement::getStartTime(ExceptionState& exception_state) const
{
    ...
    if (!start_time.IsFinite()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "No current interval.");
        return 0;
    }
...
float SVGAnimationElement::getSimpleDuration(ExceptionState& exception_state) const
{
    SMILTime duration = SimpleDuration();
    if (!duration.IsFinite()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "No simple duration defined.");
        return 0;
    }
...
void SVGAnimationElement::beginElementAt(float offset)
{
    ...
    AddInstanceTimeAndUpdate(kBegin, Elapsed() + SMILTime::FromSecondsD(offset), SMILTimeOrigin::kScript);
}

void SVGAnimationElement::endElementAt(float offset)
{
    DCHECK(std::isfinite(offset));
    AddInstanceTimeAndUpdate(kEnd, Elapsed() + SMILTime::FromSecondsD(offset), SMILTimeOrigin::kScript);
}
...
void SVGAnimationElement::SetCalcMode(const AtomicString& calc_mode)
{
    ...
    if (calc_mode == discrete) {
        ...
        SetCalcMode(IsA<SVGAnimateMotionElement>(*this) ? kCalcModePaced : kCalcModeLinear);
    }
}
...
void SVGAnimationElement::CalculateKeyTimesForCalcModePaced()
{
    ...
    for (unsigned n = 1; n < calculated_key_times.size() - 1; ++n) {
        calculated_key_times[n] = calculated_key_times[n - 1] + calculated_key_times[n] / total_distance;
    }
...
float SVGAnimationElement::CalculatePercentForSpline(float percent, unsigned spline_index) const
{
    ...
    return ClampTo<float>(bezier.SolveWithEpsilon(percent, SolveEpsilon(duration.InSecondsF())));
}

float SVGAnimationElement::CalculatePercentFromKeyPoints(float percent) const
{
    DCHECK(GetCalcMode() != kCalcModePaced || GetAnimationMode() == kPathAnimation);
    ...
    float key_point_percent = (percent - from_percent) / (to_percent - from_percent);
...
float SVGAnimationElement::CurrentValuesFromKeyPoints(float percent, String& from, String& to) const
{
    DCHECK_NE(GetCalcMode(), kCalcModePaced);
    DCHECK(!key_points_.empty());
    DCHECK_EQ(key_points_.size(), KeyTimes().size());
    float effective_percent = CalculatePercentFromKeyPoints(percent);
    unsigned index = effective_percent == 1 ? values_.size() - 2 : static_cast<unsigned>(effective_percent * (values_.size() - 1));
    from = values_[index];
    to = values_[index + 1];
    return effective_percent;
}

float SVGAnimationElement::CurrentValuesForValuesAnimation(float percent, String& from, String& to) const
{
    unsigned values_count = values_.size();
    ...
    float effective_percent = (percent - from_percent) / (to_percent - from_percent);
...
bool SVGAnimationElement::CheckAnimationParameters() const
{
    ...
    if (GetCalcMode() == kCalcModeSpline) {
        ...
        if (key_splines_.empty() || (has_key_points && key_splines_.size() != key_points_.size() - 1)
    ...
    if (animation_mode_ == kValuesAnimation) {
        ...
        // 'values'.
        if (calc_mode != kCalcModePaced && !has_key_points && has_key_times && values_.size() != KeyTimes().size()) {
            return false;
        }
        // If 'keyTimes' is specified its last value should be 1 (and the first 0)
        // unless 'calcMode' is 'discrete'.
        if (calc_mode != kCalcModeDiscrete && !KeyTimes().empty() && KeyTimes().back() != 1) {
            return false;
        }
        // If 'calcMode' is 'spline', there should be one less spline than there
        // are 'values'.
        if (calc_mode == kCalcModeSpline && key_splines_.size() != values_.size() - 1) {
            return false;
        }
    }
    return true;
...
SMILAnimationEffectParameters SVGAnimationElement::ComputeEffectParameters() const
{
...
void SVGAnimationElement::ApplyAnimation(SMILAnimationValue& animation_value)
{
    if (animation_valid_ == AnimationValidity::kUnknown) {
        if (UpdateAnimationParameters() && UpdateAnimationValues()) {
            ...
            if (IsAdditive() || GetAnimationMode() == kByAnimation || (IsAccumulated() && GetAnimationMode() != kToAnimation)) {
    ...
    if (animation_mode == kValuesAnimation) {
        ...
        if (from != last_values_animation_from_ || to != last_values_animation_to_) {
            CalculateFromAndToValues(from, to);
            last_values_animation_from_ = from;
            last_values_animation_to_ = to;
        }
    } else if (!key_points_.empty() && (animation_mode == kPathAnimation || calc_mode != kCalcModePaced)) {
        effective_percent = CalculatePercentFromKeyPoints(percent);
    } else if (calc_mode == kCalcModeSpline && key_points_.empty() && KeyTimes().size() > 1) {
        effective_percent = CalculatePercentForSpline(percent, CalculateKeyTimesIndex(percent));
    ...
    CalculateAnimationValue(animation_value, effective_percent, progress_state.repeat);
}
