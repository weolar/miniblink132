...
private:
    class PauseControlImageObserver : public GarbageCollected<PauseControlImageObserver>, public ImageObserver {
    public:
...
TEST_F(SVGImageTest, TimelineSuspendAndResume)
{
    ...
    DisallowNewWrapper<HeapTaskRunnerTimer<SVGImageChromeClient>>* timer = MakeGarbageCollected<DisallowNewWrapper<HeapTaskRunnerTimer<SVGImageChromeClient>>>(
        scheduler::GetSingleThreadTaskRunnerForTesting(), &chrome_client, &SVGImageChromeClient::AnimationTimerFired);
    ...
    // suspended.
    test::RunDelayedTasks(base::Milliseconds(1) + timer->Value().NextFireInterval());
...
TEST_F(SVGImageTest, SupportsSubsequenceCaching)
{
    ...
    Load(kAnimatedDocument, kShouldPause);
    PumpFrame();
    LocalFrame* local_frame = To<LocalFrame>(GetImage().GetPageForTesting()->MainFrame());
    ...
    EXPECT_TRUE(To<LayoutBoxModelObject>(svg_root)->Layer()->SupportsSubsequenceCaching());
}
...
{
    const bool kDontPause = false;
    Load("<svg xmlns='http://www.w3.org/2000/svg'>"
    ...
    EXPECT_EQ(local_frame->View()->Size(), GetImage().GetPageForTesting()->GetVisualViewport().Size());
}
...
TEST_F(SVGImageTest, DisablesSMILEvents)
{
    ...
    LocalFrame* local_frame = To<LocalFrame>(GetImage().GetPageForTesting()->MainFrame());
    EXPECT_TRUE(local_frame->GetDocument()->IsSVGDocument());
    SMILTimeContainer* time_container = To<SVGSVGElement>(local_frame->GetDocument()->documentElement())->TimeContainer();
...
TEST_F(SVGImageTest, PaintFrameForCurrentFrameWithMQAndZoom)
{
    ...
    auto container = SVGImageForContainer::Create(GetImage(), gfx::SizeF(100, 100), 2, nullptr, mojom::blink::PreferredColorScheme::kLight);
    SkBitmap bitmap = container->AsSkBitmapForCurrentFrame(kDoNotRespectImageOrientation);
...
TEST_F(SVGImageSimTest, PageVisibilityHiddenToVisible)
{
    ...
    SimSubresourceRequest image_resource("https://example.com/image.svg", "image/svg+xml");
    ...
    ImageResourceContent* image_content = To<HTMLImageElement>(*element).CachedImage();
    ...
    SVGImageChromeClient& svg_image_chrome_client = To<SVGImage>(*image).ChromeClientForTesting();
...
<svg>
    ...
    histograms.ExpectBucketCount("Blink.UseCounter.Extensions.Features", WebFeature::kPageVisits, 1);
}
...
size_t CountPaintOpType(const cc::PaintRecord& record, cc::PaintOpType type)
{
    ...
    for (const cc::PaintOp& op : record) {
        if (op.IsPaintOpWithFlags()) {
            const cc::PaintFlags& flags = static_cast<const cc::PaintOpWithFlags&>(op).flags;
...
TEST_F(SVGImageSimTest, SpriteSheetCulling)
{
    ...
    div->setAttribute(html_names::kStyleAttr, AtomicString("height: 200px; background-position-y: -200px;"));
    Compositor().BeginFrame();
    record = GetDocument().View()->GetPaintRecord();
...
TEST_F(SVGImageSimTest, ClippedAbsoluteImageSpriteSheetCulling)
{
    ...
    div_element->setAttribute(html_names::kStyleAttr, AtomicString("height: 200px;"));
    Compositor().BeginFrame();
    record = GetDocument().View()->GetPaintRecord();
    EXPECT_EQ(4U, CountPaintOpType(record, cc::PaintOpType::kDrawOval));
    ...
    image_element->setAttribute(html_names::kStyleAttr, AtomicString("top: -200px;"));
    Compositor().BeginFrame();
    record = GetDocument().View()->GetPaintRecord();
    EXPECT_EQ(3U, CountPaintOpType(record, cc::PaintOpType::kDrawOval));
}
...
TEST_F(SVGImageSimTest, ClippedStaticImageSpriteSheetCulling)
{
    ...
    // and ensure three circles are recorded.
    Element* image_element = GetDocument().getElementById(AtomicString("image"));
    image_element->setAttribute(html_names::kStyleAttr, AtomicString("margin-top: -200px;"));
    Compositor().BeginFrame();
    record = GetDocument().View()->GetPaintRecord();
    EXPECT_EQ(3U, CountPaintOpType(record, cc::PaintOpType::kDrawOval));
    ...
    div_element->setAttribute(html_names::kStyleAttr, AtomicString("margin-left: 0.5px; height: 200px;"));
    Compositor().BeginFrame();
    record = GetDocument().View()->GetPaintRecord();
    EXPECT_EQ(3U, CountPaintOpType(record, cc::PaintOpType::kDrawOval));
}
...
TEST_F(SVGImageSimTest, InterestRectDoesNotCullImageSpriteSheet)
{
    ...
    Element* div_element = GetDocument().getElementById(AtomicString("div"));
    div_element->setAttribute(html_names::kStyleAttr, AtomicString("width: 100px; height: 200px;"));
    Compositor().BeginFrame();
    record = GetDocument().View()->GetPaintRecord();
    EXPECT_EQ(1U, CountPaintOpType(record, cc::PaintOpType::kDrawOval));
