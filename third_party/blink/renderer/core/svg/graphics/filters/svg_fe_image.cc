...
FEImage::FEImage(Filter* filter, scoped_refptr<Image> image, const SVGPreserveAspectRatio* preserve_aspect_ratio)
    : FilterEffect(filter)
...
FEImage::FEImage(Filter* filter, const SVGElement* element, const SVGPreserveAspectRatio* preserve_aspect_ratio)
    : FilterEffect(filter)
    , element_(element)
    , preserve_aspect_ratio_(preserve_aspect_ratio)
{
...
static gfx::RectF GetLayoutObjectRepaintRect(const LayoutObject& layout_object)
{
    return layout_object.LocalToSVGParentTransform().MapRect(layout_object.VisualRectInLocalSVGCoordinates());
}

static gfx::SizeF ComputeViewportAdjustmentScale(const LayoutObject& layout_object, const gfx::SizeF& target_size)
{
    ...
    // ones.
    const gfx::SizeF viewport_size = SVGViewportResolver(layout_object).ResolveViewport();
    if (viewport_size.IsEmpty()) {
        return gfx::SizeF(1, 1);
    }
    return gfx::SizeF(target_size.width() / viewport_size.width(), target_size.height() / viewport_size.height());
}

AffineTransform FEImage::SourceToDestinationTransform(const LayoutObject& layout_object, const gfx::RectF& dest_rect) const
{
    gfx::SizeF viewport_scale(GetFilter()->Scale(), GetFilter()->Scale());
    if (element_->HasRelativeLengths()) {
        viewport_scale = ComputeViewportAdjustmentScale(layout_object, dest_rect.size());
    }
...
{
    gfx::RectF dest_rect = GetFilter()->MapLocalRectToAbsoluteRect(FilterPrimitiveSubregion());
    if (const LayoutObject* layout_object = ReferencedLayoutObject()) {
        const AffineTransform transform = SourceToDestinationTransform(*layout_object, dest_rect);
        const gfx::RectF src_rect = transform.MapRect(GetLayoutObjectRepaintRect(*layout_object));
...
StringBuilder& FEImage::ExternalRepresentation(StringBuilder& ts, wtf_size_t indent) const
{
    gfx::Size image_size;
    if (image_) {
        image_size = image_->Size();
    } else if (const LayoutObject* layout_object = ReferencedLayoutObject()) {
        image_size = gfx::ToEnclosingRect(GetLayoutObjectRepaintRect(*layout_object)).size();
    }
...
sk_sp<PaintFilter> FEImage::CreateImageFilterForLayoutObject(const LayoutObject& layout_object, const gfx::RectF& dst_rect, const gfx::RectF& crop_rect)
{
    const AffineTransform transform = SourceToDestinationTransform(layout_object, dst_rect);
    const gfx::RectF src_rect = transform.MapRect(GetLayoutObjectRepaintRect(layout_object));
    ...
    canvas->concat(AffineTransformToSkM44(transform));
    {
        ...
        SVGObjectPainter(layout_object, nullptr).PaintResourceSubtree(builder.Context());
        builder.EndRecording(*canvas);
    }
    return sk_make_sp<RecordPaintFilter>(paint_recorder.finishRecordingAsPicture(), gfx::RectFToSkRect(cull_rect));
}
...
sk_sp<PaintFilter> FEImage::CreateImageFilter()
{
    ...
    gfx::RectF crop_rect = gfx::SkRectToRectF(GetCropRect().value_or(PaintFilter::CropRect()));
    gfx::RectF dst_rect = GetFilter()->MapLocalRectToAbsoluteRect(FilterPrimitiveSubregion());
    ...
    if (PaintImage image = image_ ? image_->PaintImageForCurrentFrame() : PaintImage()) {
        gfx::RectF src_rect(gfx::SizeF(image_->Size()));
        ...
        return sk_make_sp<ImagePaintFilter>(
            std::move(image), gfx::RectFToSkRect(src_rect), gfx::RectFToSkRect(crop_rect), cc::PaintFlags::FilterQuality::kHigh);
    }
