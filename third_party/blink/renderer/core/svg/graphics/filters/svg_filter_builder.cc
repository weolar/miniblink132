...
public:
    ...
    {
        DEFINE_STATIC_LOCAL(const AtomicString, source_graphic_name, ("SourceGraphic"));
...
void SVGFilterGraphNodeMap::AddPrimitive(SVGFilterPrimitiveStandardAttributes& primitive, FilterEffect* effect)
{
...
SVGFilterBuilder::SVGFilterBuilder(
    FilterEffect* source_graphic, SVGFilterGraphNodeMap* node_map, const cc::PaintFlags* fill_flags, const cc::PaintFlags* stroke_flags)
    : node_map_(node_map)
{
    builtin_effects_.insert(FilterInputKeywords::GetSourceGraphic(), source_graphic);
    builtin_effects_.insert(FilterInputKeywords::SourceAlpha(), MakeGarbageCollected<SourceAlpha>(source_graphic));
    if (fill_flags) {
        builtin_effects_.insert(FilterInputKeywords::FillPaint(), MakeGarbageCollected<PaintFilterEffect>(source_graphic->GetFilter(), *fill_flags));
    }
    if (stroke_flags) {
        builtin_effects_.insert(FilterInputKeywords::StrokePaint(), MakeGarbageCollected<PaintFilterEffect>(source_graphic->GetFilter(), *stroke_flags));
    }
    AddBuiltinEffects();
}
...
static EColorInterpolation ColorInterpolationForElement(SVGElement& element, EColorInterpolation parent_color_interpolation)
{
    ...
    if (const CSSPropertyValueSet* property_set = element.PresentationAttributeStyle()) {
        const CSSValue* css_value = property_set->GetPropertyCSSValue(CSSPropertyID::kColorInterpolationFilters);
...
InterpolationSpace SVGFilterBuilder::ResolveInterpolationSpace(EColorInterpolation color_interpolation)
{
    return color_interpolation == EColorInterpolation::kLinearrgb ? kInterpolationSpaceLinear : kInterpolationSpaceSRGB;
}

void SVGFilterBuilder::BuildGraph(Filter* filter, SVGFilterElement& filter_element, const gfx::RectF& reference_box)
{
    EColorInterpolation filter_color_interpolation = ColorInterpolationForElement(filter_element, EColorInterpolation::kAuto);
    SVGUnitTypes::SVGUnitType primitive_units = filter_element.primitiveUnits()->CurrentEnumValue();

    for (SVGElement* element = Traversal<SVGElement>::FirstChild(filter_element); element; element = Traversal<SVGElement>::NextSibling(*element)) {
        ...
        EColorInterpolation color_interpolation = ColorInterpolationForElement(effect_element, filter_color_interpolation);
        effect->SetOperatingInterpolationSpace(ResolveInterpolationSpace(color_interpolation));
