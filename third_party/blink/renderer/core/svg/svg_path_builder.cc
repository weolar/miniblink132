...
{
    bool last_was_quadratic = last_command_ == kPathSegCurveToQuadraticAbs || last_command_ == kPathSegCurveToQuadraticRel
        || last_command_ == kPathSegCurveToQuadraticSmoothAbs || last_command_ == kPathSegCurveToQuadraticSmoothRel;

    EmitQuadTo(SmoothControl(last_was_quadratic), p);
}

void SVGPathBuilder::EmitCubicTo(const gfx::PointF& c0, const gfx::PointF& c1, const gfx::PointF& p)
{
...
void SVGPathBuilder::EmitSmoothCubicTo(const gfx::PointF& c1, const gfx::PointF& p)
{
    bool last_was_cubic = last_command_ == kPathSegCurveToCubicAbs || last_command_ == kPathSegCurveToCubicRel || last_command_ == kPathSegCurveToCubicSmoothAbs
        || last_command_ == kPathSegCurveToCubicSmoothRel;

    EmitCubicTo(SmoothControl(last_was_cubic), c1, p);
}

void SVGPathBuilder::EmitArcTo(const gfx::PointF& p, float radius_x, float radius_y, float rotate, bool large_arc, bool sweep)
{
...
void SVGPathBuilder::EmitSegment(const PathSegmentData& segment)
{
    ...
    case kPathSegCurveToQuadraticRel:
        EmitQuadTo(current_point_ + segment.point1.OffsetFromOrigin(), current_point_ + segment.target_point.OffsetFromOrigin());
        break;
    case kPathSegCurveToQuadraticSmoothAbs:
        EmitSmoothQuadTo(segment.target_point);
        break;
    case kPathSegCurveToQuadraticSmoothRel:
        EmitSmoothQuadTo(current_point_ + segment.target_point.OffsetFromOrigin());
        break;
    case kPathSegCurveToCubicAbs:
        EmitCubicTo(segment.point1, segment.point2, segment.target_point);
        break;
    case kPathSegCurveToCubicRel:
        EmitCubicTo(current_point_ + segment.point1.OffsetFromOrigin(), current_point_ + segment.point2.OffsetFromOrigin(),
    ...
    case kPathSegCurveToCubicSmoothRel:
        EmitSmoothCubicTo(current_point_ + segment.point2.OffsetFromOrigin(), current_point_ + segment.target_point.OffsetFromOrigin());
        break;
    case kPathSegArcAbs:
        EmitArcTo(segment.target_point, segment.ArcRadiusX(), segment.ArcRadiusY(), segment.ArcAngle(), segment.LargeArcFlag(), segment.SweepFlag());
        break;
    case kPathSegArcRel:
        EmitArcTo(current_point_ + segment.target_point.OffsetFromOrigin(), segment.ArcRadiusX(), segment.ArcRadiusY(), segment.ArcAngle(),
