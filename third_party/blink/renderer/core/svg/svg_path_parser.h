...
template <typename SourceType, typename ConsumerType> inline bool ParsePath(SourceType& source, ConsumerType& consumer)
{
...
protected:
    bool DecomposeArcToCubic(const gfx::PointF& current_point, const PathSegmentData&);
...
public:
    SVGPathAbsolutizer(SVGPathConsumer* consumer)
        : consumer_(consumer)
    {
        DCHECK(consumer_);
    }
