...
bool AllowedRequestMode(const ResourceRequest& request)
{
    ...
    if (request.GetMode() == network::mojom::blink::RequestMode::kCors) {
        return request.GetCredentialsMode() == network::mojom::CredentialsMode::kSameOrigin;
    }
    return false;
}
...
class SVGResourceDocumentContent::ChromeClient final : public IsolatedSVGChromeClient {
public:
...
SVGResourceDocumentContent::SVGResourceDocumentContent(AgentGroupScheduler& agent_group_scheduler, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
...
SVGResourceDocumentContent::UpdateResult SVGResourceDocumentContent::UpdateDocument(scoped_refptr<SharedBuffer> data, const KURL& request_url)
{
    ...
    document_host_ = MakeGarbageCollected<IsolatedSVGDocumentHost>(*chrome_client, *agent_group_scheduler_, std::move(data),
        WTF::BindOnce(&SVGResourceDocumentContent::AsyncLoadingFinished, WrapWeakPersistent(this)), nullptr, IsolatedSVGDocumentHost::ProcessingMode::kStatic);
...
void SVGResourceDocumentContent::LoadingFinished()
{
    ...
    frame->View()->UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason::kSVGImage);
...
void SVGResourceDocumentContent::AddObserver(SVGResourceDocumentObserver* observer)
{
    ...
    if (IsLoaded()) {
        task_runner_->PostTask(FROM_HERE, WTF::BindOnce(&SVGResourceDocumentContent::NotifyObserver, WrapPersistent(this), WrapWeakPersistent(observer)));
    }
}

void SVGResourceDocumentContent::RemoveObserver(SVGResourceDocumentObserver* observer)
{
    observers_.erase(observer);
}

void SVGResourceDocumentContent::NotifyObserver(SVGResourceDocumentObserver* observer)
{
...
SVGResourceTarget* SVGResourceDocumentContent::GetResourceTarget(const AtomicString& element_id)
{
    Document* document = GetDocument();
    if (!document) {
        return nullptr;
    }
    auto* svg_target = DynamicTo<SVGElement>(document->getElementById(element_id));
    if (!svg_target) {
        return nullptr;
    }
...
SVGResourceDocumentContent* SVGResourceDocumentContent::Fetch(FetchParameters& params, Document& document)
{
    ...
    DCHECK_EQ(params.GetResourceRequest().GetRequestContext(), mojom::blink::RequestContextType::UNSPECIFIED);
    ...
    const SVGResourceDocumentCache::CacheKey key = SVGResourceDocumentCache::MakeCacheKey(params);
    auto* cached_content = cache.Get(key);
    if (cached_content && CanReuseContent(*cached_content)) {
        return cached_content;
    }

    SVGDocumentResource* resource = SVGDocumentResource::Fetch(params, document.Fetcher(), page->GetAgentGroupScheduler());
    if (!resource) {
        return nullptr;
    }
