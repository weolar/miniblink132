...
SVGTests::SVGTests(SVGElement* context_element)
    : required_extensions_(SVGStaticStringList::Create<' '>(context_element, svg_names::kRequiredExtensionsAttr))
    , system_language_(SVGStaticStringList::Create<','>(context_element, svg_names::kSystemLanguageAttr))
{
...
SVGAnimatedPropertyBase* SVGTests::PropertyFromAttribute(const QualifiedName& attribute_name) const
{
...
{
    SVGAnimatedPropertyBase* attrs[] { required_extensions_.Get(), system_language_.Get() };
...
static bool IsLangTagPrefix(const String& lang_tag, const String& language)
{
    if (!lang_tag.StartsWithIgnoringASCIICase(language))
        return false;
    return lang_tag.length() == language.length() || lang_tag[language.length()] == '-';
}

static bool MatchLanguageList(const String& lang_tag, const Vector<String>& languages)
{
...
bool SVGTests::IsValid() const
{
    if (system_language_->IsSpecified()) {
        ...
        system_language_->ContextElement()->GetDocument().GetPage()->GetChromeClient().AcceptLanguages().Split(',', languages);
    ...
    if (required_extensions_->IsSpecified()) {
        ...
        for (const auto& extension : extensions) {
            if (extension != html_names::xhtmlNamespaceURI && extension != mathml_names::kNamespaceURI) {
                return false;
            }
        }
    }
...
{
    return attr_name == svg_names::kRequiredExtensionsAttr || attr_name == svg_names::kSystemLanguageAttr;
}
