...
public:
    ...
    bool BlendSegments(const PathSegmentData& from_seg, const PathSegmentData& to_seg, PathSegmentData&);

private:
    float BlendAnimatedDimensonalFloat(float, float, FloatBlendMode);
    gfx::PointF BlendAnimatedPointSameCoordinates(const gfx::PointF& from, const gfx::PointF& to);
    gfx::PointF BlendAnimatedPoint(const gfx::PointF& from, const gfx::PointF& to);
...
float SVGPathBlender::BlendState::BlendAnimatedDimensonalFloat(float from, float to, FloatBlendMode blend_mode)
{
    if (add_types_count_) {
    ...
    float from_value = blend_mode == kBlendHorizontal ? from_current_point_.x() : from_current_point_.y();
    float to_value = blend_mode == kBlendHorizontal ? to_current_point_.x() : to_current_point_.y();

    // Transform toY to the coordinate mode of fromY
    float anim_value = Blend(from, from_is_absolute_ ? to + to_value : to - to_value, progress_);
    ...
    return !from_is_absolute_ ? anim_value + current_value : anim_value - current_value;
}

gfx::PointF SVGPathBlender::BlendState::BlendAnimatedPointSameCoordinates(const gfx::PointF& from_point, const gfx::PointF& to_point)
{
    if (add_types_count_) {
        gfx::PointF repeated_to_point = gfx::ScalePoint(to_point, add_types_count_, add_types_count_);
        return from_point + repeated_to_point.OffsetFromOrigin();
    }
    return Blend(from_point, to_point, progress_);
}

gfx::PointF SVGPathBlender::BlendState::BlendAnimatedPoint(const gfx::PointF& from_point, const gfx::PointF& to_point)
{
    if (types_are_equal_)
    ...
    // Transform the animated point to the coordinate mode, needed for the current
    // progress.
    gfx::PointF current_point = Blend(from_current_point_, to_current_point_, progress_);
    if (!from_is_absolute_)
...
bool SVGPathBlender::BlendState::CanBlend(const PathSegmentData& from_seg, const PathSegmentData& to_seg)
{
    ...
    return ToAbsolutePathSegType(from_seg.command) == ToAbsolutePathSegType(to_seg.command);
}

static void UpdateCurrentPoint(gfx::PointF& sub_path_point, gfx::PointF& current_point, const PathSegmentData& segment)
{
...
bool SVGPathBlender::BlendState::BlendSegments(const PathSegmentData& from_seg, const PathSegmentData& to_seg, PathSegmentData& blended_segment)
{
    if (!CanBlend(from_seg, to_seg))
        return false;

    blended_segment.command = is_in_first_half_of_animation_ ? from_seg.command : to_seg.command;

    switch (to_seg.command) {
    case kPathSegCurveToCubicRel:
    case kPathSegCurveToCubicAbs:
        blended_segment.point1 = BlendAnimatedPoint(from_seg.point1, to_seg.point1);
        [[fallthrough]];
    case kPathSegCurveToCubicSmoothRel:
    case kPathSegCurveToCubicSmoothAbs:
        blended_segment.point2 = BlendAnimatedPoint(from_seg.point2, to_seg.point2);
        [[fallthrough]];
    case kPathSegMoveToRel:
    case kPathSegMoveToAbs:
    case kPathSegLineToRel:
    ...
    case kPathSegCurveToQuadraticSmoothRel:
    case kPathSegCurveToQuadraticSmoothAbs:
        blended_segment.target_point = BlendAnimatedPoint(from_seg.target_point, to_seg.target_point);
        break;
    case kPathSegLineToHorizontalRel:
    case kPathSegLineToHorizontalAbs:
        blended_segment.target_point.set_x(BlendAnimatedDimensonalFloat(from_seg.target_point.x(), to_seg.target_point.x(), kBlendHorizontal));
        break;
    case kPathSegLineToVerticalRel:
    case kPathSegLineToVerticalAbs:
        blended_segment.target_point.set_y(BlendAnimatedDimensonalFloat(from_seg.target_point.y(), to_seg.target_point.y(), kBlendVertical));
        break;
    case kPathSegClosePath:
        break;
    case kPathSegCurveToQuadraticRel:
    ...
    case kPathSegArcAbs:
        blended_segment.target_point = BlendAnimatedPoint(from_seg.target_point, to_seg.target_point);
        blended_segment.point1 = BlendAnimatedPointSameCoordinates(from_seg.point1, to_seg.point1);
        blended_segment.point2 = BlendAnimatedPointSameCoordinates(from_seg.point2, to_seg.point2);
        if (add_types_count_) {
            ...
            blended_segment.arc_large = is_in_first_half_of_animation_ ? from_seg.arc_large : to_seg.arc_large;
            blended_segment.arc_sweep = is_in_first_half_of_animation_ ? from_seg.arc_sweep : to_seg.arc_sweep;
        }
        break;
    default:
        NOTREACHED();
...
SVGPathBlender::SVGPathBlender(SVGPathByteStreamSource* from_source, SVGPathByteStreamSource* to_source, SVGPathConsumer* consumer)
