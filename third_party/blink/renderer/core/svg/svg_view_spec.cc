...
const SVGViewSpec* SVGViewSpec::CreateForViewElement(const SVGViewElement& view)
{
    SVGViewSpec* view_spec = MakeGarbageCollected<SVGViewSpec>();
    if (view.HasValidViewBox())
        view_spec->view_box_ = view.viewBox()->CurrentValue()->Clone();
    if (view.preserveAspectRatio()->IsSpecified()) {
        view_spec->preserve_aspect_ratio_ = view.preserveAspectRatio()->CurrentValue()->Clone();
    }
...
bool SVGViewSpec::ParseViewSpec(const String& spec)
{
    if (spec.empty())
        return false;
    return WTF::VisitCharacters(spec, [&](auto chars) { return ParseViewSpecInternal(chars.data(), chars.data() + chars.size()); });
}
...
template <typename CharType> static ViewSpecFunctionType ScanViewSpecFunction(const CharType*& ptr, const CharType* end)
{
    DCHECK_LT(ptr, end);
    switch (*ptr) {
    case 'v':
...
template <typename CharType> bool SVGViewSpec::ParseViewSpecInternal(const CharType* ptr, const CharType* end)
{
