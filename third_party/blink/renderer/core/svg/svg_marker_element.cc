...
template <> const SVGEnumerationMap& GetEnumerationMap<SVGMarkerUnitsType>()
{
...
SVGMarkerElement::SVGMarkerElement(Document& document)
    ...
    , ref_x_(MakeGarbageCollected<SVGAnimatedLength>(this, svg_names::kRefXAttr, SVGLengthMode::kWidth, SVGLength::Initial::kUnitlessZero))
    , ref_y_(MakeGarbageCollected<SVGAnimatedLength>(this, svg_names::kRefYAttr, SVGLengthMode::kHeight, SVGLength::Initial::kUnitlessZero))
    ,
    // Spec: If the markerWidth/markerHeight attribute is not specified, the
    // effect is as if a value of "3" were specified.
    marker_width_(MakeGarbageCollected<SVGAnimatedLength>(this, svg_names::kMarkerWidthAttr, SVGLengthMode::kWidth, SVGLength::Initial::kNumber3))
    , marker_height_(MakeGarbageCollected<SVGAnimatedLength>(this, svg_names::kMarkerHeightAttr, SVGLengthMode::kHeight, SVGLength::Initial::kNumber3))
    , orient_angle_(MakeGarbageCollected<SVGAnimatedAngle>(this))
    , marker_units_(MakeGarbageCollected<SVGAnimatedEnumeration<SVGMarkerUnitsType>>(this, svg_names::kMarkerUnitsAttr, kSVGMarkerUnitsStrokeWidth))
{
}
...
AffineTransform SVGMarkerElement::ViewBoxToViewTransform(const gfx::SizeF& viewport_size) const
{
    return SVGFitToViewBox::ViewBoxToViewTransform(viewBox()->CurrentValue()->Rect(), preserveAspectRatio()->CurrentValue(), viewport_size);
}

void SVGMarkerElement::SvgAttributeChanged(const SvgAttributeChangedParams& params)
{
    const QualifiedName& attr_name = params.name;
    bool viewbox_attribute_changed = SVGFitToViewBox::IsKnownAttribute(attr_name);
    bool length_attribute_changed = attr_name == svg_names::kRefXAttr || attr_name == svg_names::kRefYAttr || attr_name == svg_names::kMarkerWidthAttr
        || attr_name == svg_names::kMarkerHeightAttr;
    if (length_attribute_changed)
        UpdateRelativeLengthsInformation();

    if (viewbox_attribute_changed || length_attribute_changed || attr_name == svg_names::kMarkerUnitsAttr || attr_name == svg_names::kOrientAttr) {
        ...
        if (resource_container) {
            ...
            if (viewbox_attribute_changed || length_attribute_changed) {
                ...
                resource_container->SetNeedsLayoutAndFullPaintInvalidation(layout_invalidation_reason::kSvgResourceInvalidated);
            }
        }
        return;
    }
...
{
    return ref_x_->CurrentValue()->IsRelative() || ref_y_->CurrentValue()->IsRelative() || marker_width_->CurrentValue()->IsRelative()
...
SVGAnimatedPropertyBase* SVGMarkerElement::PropertyFromAttribute(const QualifiedName& attribute_name) const
{
...
{
    SVGAnimatedPropertyBase* attrs[] { ref_x_.Get(), ref_y_.Get(), marker_width_.Get(), marker_height_.Get(), orient_angle_.Get(), marker_units_.Get() };
