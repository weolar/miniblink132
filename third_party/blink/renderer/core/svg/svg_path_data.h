...
static inline SVGPathSegType ToAbsolutePathSegType(const SVGPathSegType type)
{
    ...
    return type >= kPathSegMoveToAbs ? static_cast<SVGPathSegType>(type & ~1u) : type;
}
...
public:
    ...
    float ArcRadiusX() const
    {
        return point1.x();
    }
    void SetArcRadiusX(float x)
    {
        point1.set_x(x);
    }
    float ArcRadiusY() const
    {
        return point1.y();
    }
    void SetArcRadiusY(float y)
    {
        point1.set_y(y);
    }
