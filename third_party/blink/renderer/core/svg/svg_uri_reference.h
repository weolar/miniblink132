...
public:
    ...
    static Element* ObserveTarget(Member<IdTargetObserver>&, SVGElement&, const String& href_string);
    // Create an 'id' observer for |id| in the specified TreeScope. On changes,
    // the passed Closure will be called.
    static Element* ObserveTarget(Member<IdTargetObserver>&, TreeScope&, const AtomicString& id, base::RepeatingClosure);
...
protected:
    ...
    SVGAnimatedPropertyBase* PropertyFromAttribute(const QualifiedName& attribute_name) const;
