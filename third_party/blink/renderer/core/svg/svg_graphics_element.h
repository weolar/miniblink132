...
class CORE_EXPORT SVGGraphicsElement : public SVGTransformableElement, public SVGTests {
...
public:
    ...
    AffineTransform ComputeCTM(CTMScope mode, const SVGGraphicsElement* ancestor = nullptr) const;

    void Trace(Visitor*) const override;

protected:
    SVGGraphicsElement(const QualifiedName&, Document&, ConstructionType = kCreateSVGElement);
...
private:
    bool IsSVGGraphicsElement() const final
    {
        return true;
    }
};

template <> struct DowncastTraits<SVGGraphicsElement> {
