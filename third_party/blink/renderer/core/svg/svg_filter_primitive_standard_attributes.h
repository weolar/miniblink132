...
public:
    void SetStandardAttributes(FilterEffect*, SVGUnitTypes::SVGUnitType, const gfx::RectF& reference_box) const;

    virtual FilterEffect* Build(SVGFilterBuilder*, Filter*) = 0;
    // Returns true, if the new value is different from the old one.
    virtual bool SetFilterEffectAttribute(FilterEffect*, const QualifiedName&);

    virtual bool TaintsOrigin() const
    {
        return true;
    }
...
private:
    bool IsFilterEffect() const final
    {
        return true;
    }
...
template <> struct DowncastTraits<SVGFilterPrimitiveStandardAttributes> {
