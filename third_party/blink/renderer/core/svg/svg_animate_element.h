...
protected:
    ...
    bool CalculateToAtEndOfDurationValue(const String& to_at_end_of_duration_string) final;
    void CalculateFromAndToValues(const String& from_string, const String& to_string) final;
    void CalculateFromAndByValues(const String& from_string, const String& by_string) final;
    void CalculateAnimationValue(SMILAnimationValue&, float percentage, unsigned repeat_count) const final;
    void ApplyResultsToTarget(const SMILAnimationValue&) final;
    float CalculateDistance(const String& from_string, const String& to_string) final;
...
private:
    ...
    SVGPropertyBase* AdjustForInheritance(SVGPropertyBase*, AnimatedPropertyValueType) const;
...
protected:
    ...
    bool IsAnimatingSVGDom() const
    {
        return target_property_ != nullptr;
    }
...
{
    return element.HasTagName(svg_names::kAnimateTag) || element.HasTagName(svg_names::kAnimateTransformTag) || element.HasTagName(svg_names::kSetTag);
}

template <> struct DowncastTraits<SVGAnimateElement> {
