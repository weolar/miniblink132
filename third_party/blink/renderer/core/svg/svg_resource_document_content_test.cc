...
class FakeSVGResourceDocumentObserver final : public GarbageCollected<FakeSVGResourceDocumentObserver>, public SVGResourceDocumentObserver {
public:
...
TEST_F(SVGResourceDocumentContentSimTest, LoadCompleteAfterDispose)
{
    ...
    EXPECT_EQ(GetDocument().GetPage()->GetSVGResourceDocumentCache().Get(SVGResourceDocumentCache::MakeCacheKey(params)), nullptr);
...
TEST_F(SVGResourceDocumentContentTest, CacheCleanup)
{
    ...
    const char kImageDataUrl1[] = "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg'/>";
    FetchParameters params1(ResourceRequest(kImageDataUrl1), options);
    params1.MutableResourceRequest().SetMode(network::mojom::blink::RequestMode::kSameOrigin);
    auto* content1 = SVGResourceDocumentContent::Fetch(params1, GetDocument());
    EXPECT_TRUE(content1->IsLoaded());

    const char kImageDataUrl2[] = "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' id='two'/>";
    FetchParameters params2(ResourceRequest(kImageDataUrl2), options);
    params2.MutableResourceRequest().SetMode(network::mojom::blink::RequestMode::kSameOrigin);
    auto* content2 = SVGResourceDocumentContent::Fetch(params2, GetDocument());
    EXPECT_TRUE(content2->IsLoaded());

    Persistent<FakeSVGResourceDocumentObserver> observer = MakeGarbageCollected<FakeSVGResourceDocumentObserver>();
    content2->AddObserver(observer);

    auto& cache = GetPage().GetSVGResourceDocumentCache();

    // Both document contents should be in the cache.
    EXPECT_NE(cache.Get(SVGResourceDocumentCache::MakeCacheKey(params1)), nullptr);
    EXPECT_NE(cache.Get(SVGResourceDocumentCache::MakeCacheKey(params2)), nullptr);
    ...
    EXPECT_EQ(cache.Get(SVGResourceDocumentCache::MakeCacheKey(params1)), nullptr);
    EXPECT_NE(cache.Get(SVGResourceDocumentCache::MakeCacheKey(params2)), nullptr);
    ...
    EXPECT_EQ(cache.Get(SVGResourceDocumentCache::MakeCacheKey(params1)), nullptr);
    EXPECT_EQ(cache.Get(SVGResourceDocumentCache::MakeCacheKey(params2)), nullptr);
}
