...
void SVGDocumentExtensions::ServiceWebAnimationsOnAnimationFrame(Document& document)
{
    if (!document.SvgExtensions())
        return;
...
bool SVGDocumentExtensions::ServiceSmilAnimations()
{
    ...
    for (const auto& container : time_containers) {
        did_schedule_animation_frame |= container->TimeContainer()->ServiceAnimations();
    }
...
void SVGDocumentExtensions::DispatchSVGLoadEventToOutermostSVGElements()
{
    ...
    for (const auto& container : time_containers) {
        ...
        // XML/standalone svg).
        if (outer_svg->GetDocument().WellFormed() || !outer_svg->GetDocument().IsSVGDocument())
            outer_svg->SendSVGLoadEventIfPossible();
    }
}

void SVGDocumentExtensions::AddSVGRootWithRelativeLengthDescendents(SVGSVGElement* svg_root)
{
#if DCHECK_IS_ON()
...
void SVGDocumentExtensions::RemoveSVGRootWithRelativeLengthDescendents(SVGSVGElement* svg_root)
{
#if DCHECK_IS_ON()
    DCHECK(!in_relative_length_svg_roots_invalidation_);
#endif
...
#if DCHECK_IS_ON()
    DCHECK(!in_relative_length_svg_roots_invalidation_);
    base::AutoReset<bool> in_relative_length_svg_roots_change(&in_relative_length_svg_roots_invalidation_, true);
#endif
...
void SVGDocumentExtensions::StartPan(const gfx::PointF& start)
{
    if (SVGSVGElement* svg = rootElement(*document_)) {
        translate_ = gfx::Vector2dF(start.x() - svg->CurrentTranslate().x(), start.y() - svg->CurrentTranslate().y());
    }
}
...
{
    if (SVGSVGElement* svg = rootElement(*document_)) {
        svg->SetCurrentTranslate(gfx::Vector2dF(pos.x() - translate_.x(), pos.y() - translate_.y()));
    }
}
