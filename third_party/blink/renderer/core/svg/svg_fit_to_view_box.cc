...
SVGParsingError SVGAnimatedViewBoxRect::AttributeChanged(const String& value)
{
    ...
    if (parse_status == SVGParseStatus::kNoError && (BaseValue()->Width() < 0 || BaseValue()->Height() < 0)) {
...
SVGFitToViewBox::SVGFitToViewBox(SVGElement* element)
    ...
    , preserve_aspect_ratio_(MakeGarbageCollected<SVGAnimatedPreserveAspectRatio>(element, svg_names::kPreserveAspectRatioAttr))
{
    DCHECK(element);
}
...
AffineTransform SVGFitToViewBox::ViewBoxToViewTransform(
    const gfx::RectF& view_box_rect, const SVGPreserveAspectRatio* preserve_aspect_ratio, const gfx::SizeF& viewport_size)
{
...
SVGAnimatedPropertyBase* SVGFitToViewBox::PropertyFromAttribute(const QualifiedName& attribute_name) const
{
...
{
    SVGAnimatedPropertyBase* attrs[] { view_box_.Get(), preserve_aspect_ratio_.Get() };
