...
template <typename CharType> SVGParsingError SVGPreserveAspectRatio::ParseInternal(const CharType*& ptr, const CharType* end, bool validate)
{
...
void SVGPreserveAspectRatio::TransformRect(gfx::RectF& dest_rect, gfx::RectF& src_rect) const
{
    ...
    case SVGPreserveAspectRatio::kSvgMeetorsliceMeet: {
        ...
        if (orig_dest_height > orig_dest_width * width_to_height_multiplier) {
            ...
            case SVGPreserveAspectRatio::kSvgPreserveaspectratioXmaxymid:
                dest_rect.set_y(dest_rect.y() + orig_dest_height / 2 - dest_rect.height() / 2);
                break;
            ...
            case SVGPreserveAspectRatio::kSvgPreserveaspectratioXmaxymax:
                dest_rect.set_y(dest_rect.y() + orig_dest_height - dest_rect.height());
                break;
            default:
                break;
            }
        ...
        if (orig_dest_width > orig_dest_height / width_to_height_multiplier) {
            ...
            case SVGPreserveAspectRatio::kSvgPreserveaspectratioXmidymid:
            case SVGPreserveAspectRatio::kSvgPreserveaspectratioXmidymax:
                dest_rect.set_x(dest_rect.x() + orig_dest_width / 2 - dest_rect.width() / 2);
                break;
            case SVGPreserveAspectRatio::kSvgPreserveaspectratioXmaxymin:
            case SVGPreserveAspectRatio::kSvgPreserveaspectratioXmaxymid:
            case SVGPreserveAspectRatio::kSvgPreserveaspectratioXmaxymax:
                dest_rect.set_x(dest_rect.x() + orig_dest_width - dest_rect.width());
                break;
            default:
                break;
            }
    ...
    case SVGPreserveAspectRatio::kSvgMeetorsliceSlice: {
        ...
        if (orig_dest_height < orig_dest_width * width_to_height_multiplier) {
            ...
            case SVGPreserveAspectRatio::kSvgPreserveaspectratioXmidymid:
            case SVGPreserveAspectRatio::kSvgPreserveaspectratioXmaxymid:
                src_rect.set_y(src_rect.y() + image_size.height() / 2 - src_rect.height() / 2);
                break;
            case SVGPreserveAspectRatio::kSvgPreserveaspectratioXminymax:
            case SVGPreserveAspectRatio::kSvgPreserveaspectratioXmidymax:
            case SVGPreserveAspectRatio::kSvgPreserveaspectratioXmaxymax:
                src_rect.set_y(src_rect.y() + image_size.height() - src_rect.height());
                break;
            default:
                break;
            }
        ...
        if (orig_dest_width < orig_dest_height / width_to_height_multiplier) {
            ...
            case SVGPreserveAspectRatio::kSvgPreserveaspectratioXmidymid:
            case SVGPreserveAspectRatio::kSvgPreserveaspectratioXmidymax:
                src_rect.set_x(src_rect.x() + image_size.width() / 2 - src_rect.width() / 2);
                break;
            case SVGPreserveAspectRatio::kSvgPreserveaspectratioXmaxymin:
            case SVGPreserveAspectRatio::kSvgPreserveaspectratioXmaxymid:
            case SVGPreserveAspectRatio::kSvgPreserveaspectratioXmaxymax:
                src_rect.set_x(src_rect.x() + image_size.width() - src_rect.width());
                break;
            default:
                break;
            }
...
AffineTransform SVGPreserveAspectRatio::ComputeTransform(const gfx::RectF& view_box, const gfx::SizeF& viewport_size) const
{
    ...
    if (align_ == kSvgPreserveaspectratioNone) {
        transform.ScaleNonUniform(extended_physical_width / extended_logical_width, extended_physical_height / extended_logical_height);
    ...
    if ((logical_ratio < physical_ratio && (meet_or_slice_ == kSvgMeetorsliceMeet))
        || (logical_ratio >= physical_ratio && (meet_or_slice_ == kSvgMeetorsliceSlice))) {
        transform.ScaleNonUniform(extended_physical_height / extended_logical_height, extended_physical_height / extended_logical_height);

        if (align_ == kSvgPreserveaspectratioXminymin || align_ == kSvgPreserveaspectratioXminymid || align_ == kSvgPreserveaspectratioXminymax)
            transform.Translate(-extended_logical_x, -extended_logical_y);
        else if (align_ == kSvgPreserveaspectratioXmidymin || align_ == kSvgPreserveaspectratioXmidymid || align_ == kSvgPreserveaspectratioXmidymax)
            transform.Translate(
                -extended_logical_x - (extended_logical_width - extended_physical_width * extended_logical_height / extended_physical_height) / 2,
                -extended_logical_y);
        else
            transform.Translate(-extended_logical_x - (extended_logical_width - extended_physical_width * extended_logical_height / extended_physical_height),
                -extended_logical_y);

        return transform;
    }

    transform.ScaleNonUniform(extended_physical_width / extended_logical_width, extended_physical_width / extended_logical_width);

    if (align_ == kSvgPreserveaspectratioXminymin || align_ == kSvgPreserveaspectratioXmidymin || align_ == kSvgPreserveaspectratioXmaxymin)
        transform.Translate(-extended_logical_x, -extended_logical_y);
    else if (align_ == kSvgPreserveaspectratioXminymid || align_ == kSvgPreserveaspectratioXmidymid || align_ == kSvgPreserveaspectratioXmaxymid)
        transform.Translate(-extended_logical_x,
            -extended_logical_y - (extended_logical_height - extended_physical_height * extended_logical_width / extended_physical_width) / 2);
    else
        transform.Translate(
            -extended_logical_x, -extended_logical_y - (extended_logical_height - extended_physical_height * extended_logical_width / extended_physical_width));
...
void SVGPreserveAspectRatio::Add(const SVGPropertyBase* other, const SVGElement*)
{
    NOTREACHED();
}

void SVGPreserveAspectRatio::CalculateAnimatedValue(const SMILAnimationEffectParameters&, float percentage, unsigned repeat_count,
    const SVGPropertyBase* from_value, const SVGPropertyBase* to_value, const SVGPropertyBase*, const SVGElement*)
{
    NOTREACHED();
}

float SVGPreserveAspectRatio::CalculateDistance(const SVGPropertyBase* to_value, const SVGElement* context_element) const
{
