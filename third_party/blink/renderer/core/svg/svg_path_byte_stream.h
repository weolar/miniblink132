...
template <typename DataType> union ByteType {
    DataType value;
...
public:
    ...
    unsigned Hash() const
    {
        return StringHasher::HashMemory(base::span<const uint8_t>(data_.data(), data_.size()));
    }

private:
    explicit SVGPathByteStream(const Data& data)
        : data_(data)
    {
    }
