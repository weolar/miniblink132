...
SVGSVGElement::SVGSVGElement(Document& doc)
    ...
    , x_(MakeGarbageCollected<SVGAnimatedLength>(this, svg_names::kXAttr, SVGLengthMode::kWidth, SVGLength::Initial::kUnitlessZero, CSSPropertyID::kX))
    , y_(MakeGarbageCollected<SVGAnimatedLength>(this, svg_names::kYAttr, SVGLengthMode::kHeight, SVGLength::Initial::kUnitlessZero, CSSPropertyID::kY))
    , width_(
          MakeGarbageCollected<SVGAnimatedLength>(this, svg_names::kWidthAttr, SVGLengthMode::kWidth, SVGLength::Initial::kPercent100, CSSPropertyID::kWidth))
    , height_(MakeGarbageCollected<SVGAnimatedLength>(
          this, svg_names::kHeightAttr, SVGLengthMode::kHeight, SVGLength::Initial::kPercent100, CSSPropertyID::kHeight))
...
void SVGSVGElement::UpdateUserTransform()
{
    if (LayoutObject* object = GetLayoutObject()) {
        object->SetNeedsLayoutAndFullPaintInvalidation(layout_invalidation_reason::kUnknown);
    }
}
...
void SVGSVGElement::ParseAttribute(const AttributeModificationParams& params)
{
    ...
    if (!nearestViewportElement()) {
        ...
        if (name == html_names::kOnunloadAttr) {
            GetDocument().SetWindowAttributeEventListener(
                event_type_names::kUnload, JSEventHandlerForContentAttribute::Create(GetExecutionContext(), name, value));
        } else if (name == html_names::kOnresizeAttr) {
            GetDocument().SetWindowAttributeEventListener(
                event_type_names::kResize, JSEventHandlerForContentAttribute::Create(GetExecutionContext(), name, value));
        } else if (name == html_names::kOnscrollAttr) {
            GetDocument().SetWindowAttributeEventListener(
                event_type_names::kScroll, JSEventHandlerForContentAttribute::Create(GetExecutionContext(), name, value));
        } else {
            set_listener = false;
        }
    ...
    if (name == html_names::kOnabortAttr) {
        GetDocument().SetWindowAttributeEventListener(event_type_names::kAbort, JSEventHandlerForContentAttribute::Create(GetExecutionContext(), name, value));
    } else if (name == html_names::kOnerrorAttr) {
        GetDocument().SetWindowAttributeEventListener(event_type_names::kError,
            JSEventHandlerForContentAttribute::Create(GetExecutionContext(), name, value, JSEventHandler::HandlerType::kOnErrorEventHandler));
...
{
    if ((name == svg_names::kWidthAttr || name == svg_names::kHeightAttr) && !IsOutermostSVGSVGElement())
        return false;
    return SVGGraphicsElement::IsPresentationAttribute(name);
}

void SVGSVGElement::CollectStyleForPresentationAttribute(const QualifiedName& name, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
...
void SVGSVGElement::SvgAttributeChanged(const SvgAttributeChangedParams& params)
{
    const QualifiedName& attr_name = params.name;
    bool update_relative_lengths_or_view_box = false;
    bool width_or_height_changed = attr_name == svg_names::kWidthAttr || attr_name == svg_names::kHeightAttr;
    ...
    if (update_relative_lengths_or_view_box || SVGZoomAndPan::IsKnownAttribute(attr_name)) {
...
const SVGElement* InnermostCommonSubtreeRoot(const SVGSVGElement& svg_root, const SVGElement* reference_element)
{
...
HeapVector<Member<Element>> ComputeIntersectionList(
    const SVGSVGElement& root, const SVGElement* reference_element, const gfx::RectF& rect, ElementResultFilter filter)
{
    HeapVector<Member<Element>> elements;
    LocalFrameView* frame_view = root.GetDocument().View();
    if (!frame_view || !frame_view->UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason::kJavaScript)) {
        return elements;
    }
    ...
    const SVGElement* common_subtree_root = InnermostCommonSubtreeRoot(root, reference_element);
    if (!common_subtree_root) {
        return elements;
    }

    HitTestRequest request(HitTestRequest::kReadOnly | HitTestRequest::kActive | HitTestRequest::kListBased | HitTestRequest::kPenetratingList);
    ...
    TransformedHitTestLocation local_location(location, root.ComputeCTM(SVGElement::kAncestorScope, &root));
    if (local_location) {
        if (const auto* layout_root = DynamicTo<LayoutSVGRoot>(layout_object)) {
            layout_root->IntersectChildren(result, *local_location);
        } else {
            To<LayoutSVGViewportContainer>(layout_object)->IntersectChildren(result, *local_location);
        }
    }
    ...
    // common subtree root.
    auto partition_condition = [common_subtree_root, filter](const Member<Element>& item) {
        if (!IsA<SVGGraphicsElement>(*item)) {
            return false;
        }
        return filter == ElementResultFilter::kDescendantsOrReference ? common_subtree_root->contains(item) : item->IsDescendantOf(common_subtree_root);
    };
    auto to_remove = std::stable_partition(elements.begin(), elements.end(), partition_condition);
...
StaticNodeTypeList<Element>* SVGSVGElement::getIntersectionList(SVGRectTearOff* rect, SVGElement* reference_element) const
{
    ...
    HeapVector<Member<Element>> intersecting_elements
        = ComputeIntersectionList(*this, reference_element, rect->Target()->Rect(), ElementResultFilter::kOnlyDescendants);
...
bool SVGSVGElement::checkIntersection(SVGElement* element, SVGRectTearOff* rect) const
{
    ...
    HeapVector<Member<Element>> intersecting_elements
        = ComputeIntersectionList(*this, element, rect->Target()->Rect(), ElementResultFilter::kDescendantsOrReference);
    ...
    for (SVGGraphicsElement& descendant : Traversal<SVGGraphicsElement>::InclusiveDescendantsOf(*graphics_element)) {
...
{
    if (!layout_object || layout_object->StyleRef().UsedPointerEvents() == EPointerEvents::kNone) {
        return false;
    }
    return layout_object->IsSVGShape() || layout_object->IsSVGText() || layout_object->IsSVGImage() || IsA<SVGUseElement>(*layout_object->GetNode());
}

bool SVGSVGElement::CheckEnclosure(const SVGElement& element, const gfx::RectF& rect) const
{
    ...
    AffineTransform ctm = To<SVGGraphicsElement>(element).ComputeCTM(kAncestorScope, this);
    gfx::RectF visual_rect = layout_object->VisualRectInLocalSVGCoordinates();
    SVGLayoutSupport::AdjustWithClipPathAndMask(*layout_object, layout_object->ObjectBoundingBox(), visual_rect);
    gfx::RectF mapped_repaint_rect = ctm.MapRect(visual_rect);
    return rect.Contains(mapped_repaint_rect);
}

StaticNodeList* SVGSVGElement::getEnclosureList(SVGRectTearOff* query_rect, SVGElement* reference_element) const
{
    ...
    if (const SVGElement* root = InnermostCommonSubtreeRoot(*this, reference_element)) {
...
AffineTransform SVGSVGElement::LocalCoordinateSpaceTransform(CTMScope mode) const
{
    ...
    if (!IsOutermostSVGSVGElement()) {
        ...
        transform.Translate(x_->CurrentValue()->Value(length_context), y_->CurrentValue()->Value(length_context));
        if (layout_object) {
            viewport_size = To<LayoutSVGViewportContainer>(*layout_object).Viewport().size();
        }
        ...
        if (mode == kScreenScope) {
            ...
            matrix.PreConcat(To<LayoutSVGRoot>(layout_object)->LocalToBorderBoxTransform().ToTransform());
...
Node::InsertionNotificationRequest SVGSVGElement::InsertedInto(ContainerNode& root_parent)
{
...
{
    return x_->CurrentValue()->IsRelative() || y_->CurrentValue()->IsRelative() || width_->CurrentValue()->IsRelative()
...
gfx::RectF SVGSVGElement::CurrentViewBoxRect() const
{
    ...
    SVGLengthContext length_context(this);
    gfx::SizeF synthesized_view_box_size(width()->CurrentValue()->Value(length_context), height()->CurrentValue()->Value(length_context));
    return gfx::RectF(synthesized_view_box_size);
}

const SVGPreserveAspectRatio* SVGSVGElement::CurrentPreserveAspectRatio() const
{
    ...
    if (!HasValidViewBox(CurrentViewBox()) && ShouldSynthesizeViewBox()) {
        ...
        synthesized_par->SetAlign(SVGPreserveAspectRatio::kSvgPreserveaspectratioNone);
...
AffineTransform SVGSVGElement::ViewBoxToViewTransform(const gfx::SizeF& viewport_size) const
{
    AffineTransform ctm = SVGFitToViewBox::ViewBoxToViewTransform(CurrentViewBoxRect(), CurrentPreserveAspectRatio(), viewport_size);
...
const SVGViewSpec* SVGSVGElement::ParseViewSpec(const String& fragment_identifier, Element* anchor_node) const
{
    if (fragment_identifier.StartsWith("svgView(")) {
        const SVGViewSpec* view_spec = SVGViewSpec::CreateFromFragment(fragment_identifier);
        if (view_spec) {
            UseCounter::Count(GetDocument(), WebFeature::kSVGSVGElementFragmentSVGView);
            return view_spec;
        }
    }
    if (auto* svg_view_element = DynamicTo<SVGViewElement>(anchor_node)) {
        ...
        const SVGViewSpec* view_spec = SVGViewSpec::CreateForViewElement(*svg_view_element);
        UseCounter::Count(GetDocument(), WebFeature::kSVGSVGElementFragmentSVGViewElement);
        return view_spec;
    }
    return nullptr;
}
...
SVGAnimatedPropertyBase* SVGSVGElement::PropertyFromAttribute(const QualifiedName& attribute_name) const
{
    if (attribute_name == svg_names::kXAttr) {
        ...
        SVGAnimatedPropertyBase* ret = SVGFitToViewBox::PropertyFromAttribute(attribute_name);
        if (ret) {
            return ret;
        } else {
...
{
    SVGAnimatedPropertyBase* attrs[] { x_.Get(), y_.Get(), width_.Get(), height_.Get() };
    SynchronizeListOfSVGAttributes(attrs);
    SVGFitToViewBox::SynchronizeAllSVGAttributes();
    SVGGraphicsElement::SynchronizeAllSVGAttributes();
}

void SVGSVGElement::CollectExtraStyleForPresentationAttribute(MutableCSSPropertyValueSet* style)
{
    auto pres_attrs = std::to_array<const SVGAnimatedPropertyBase*>({ x_.Get(), y_.Get(), width_.Get(), height_.Get() });
    AddAnimatedPropertiesToPresentationAttributeStyle(base::span<const SVGAnimatedPropertyBase*>(pres_attrs.data(), pres_attrs.size()), style);
