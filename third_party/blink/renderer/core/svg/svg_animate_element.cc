...
String ComputeCSSPropertyValue(SVGElement* element, CSSPropertyID id)
{
    ...
    element->GetDocument().UpdateStyleAndLayoutTreeForElement(element, DocumentUpdateReason::kSMILAnimation);
    ...
    if (!style)
        return "";
    const CSSValue* value = CSSProperty::Get(id).CSSValueFromComputedStyle(*style, element->GetLayoutObject(), false, CSSValuePhase::kResolvedValue);
    return value ? value->CssText() : "";
}

AnimatedPropertyValueType PropertyValueType(const QualifiedName& attribute_name, const String& value)
{
    DEFINE_STATIC_LOCAL(const AtomicString, inherit, ("inherit"));
    if (value.empty() || value != inherit || !SVGElement::IsAnimatableCSSProperty(attribute_name))
        return kRegularPropertyValue;
    return kInheritValue;
}

QualifiedName ConstructQualifiedName(const SVGElement& svg_element, const AtomicString& attribute_name)
{
    ...
    if (!Document::ParseQualifiedName(attribute_name, prefix, local_name, IGNORE_EXCEPTION_FOR_TESTING))
        return AnyQName();
...
SVGAnimateElement::SVGAnimateElement(const QualifiedName& tag_name, Document& document)
...
{
    if ((attribute.GetName() == svg_names::kFromAttr || attribute.GetName() == svg_names::kToAttr) && AttributeValueIsJavaScriptURL(attribute))
        return true;
...
Node::InsertionNotificationRequest SVGAnimateElement::InsertedInto(ContainerNode& root_parent)
{
    SVGAnimationElement::InsertedInto(root_parent);
    if (root_parent.isConnected()) {
        SetAttributeName(ConstructQualifiedName(*this, FastGetAttribute(svg_names::kAttributeNameAttr)));
    }
...
void SVGAnimateElement::ParseAttribute(const AttributeModificationParams& params)
{
...
void SVGAnimateElement::ResolveTargetProperty()
{
    ...
    if (target_property_) {
        ...
        css_property_id_
            = type_ != kAnimatedUnknown ? CssPropertyID(targetElement()->GetExecutionContext(), AttributeName().LocalName()) : CSSPropertyID::kInvalid;
    }
    ...
    DCHECK(type_ != kAnimatedPoint && type_ != kAnimatedStringList && type_ != kAnimatedTransform && type_ != kAnimatedTransformList);
}
...
SVGPropertyBase* SVGAnimateElement::CreatePropertyForAttributeAnimation(const String& value) const
{
...
SVGPropertyBase* SVGAnimateElement::CreateUnderlyingValueForAttributeAnimation() const
{
    // SVG DOM animVal animation code-path.
    DCHECK_NE(type_, kAnimatedTransformList);
    DCHECK(target_property_);
...
SVGPropertyBase* SVGAnimateElement::CreatePropertyForCSSAnimation(const String& value) const
{
...
SVGPropertyBase* SVGAnimateElement::AdjustForInheritance(SVGPropertyBase* property_value, AnimatedPropertyValueType value_type) const
{
...
static SVGPropertyBase* DiscreteSelectValue(AnimationMode animation_mode, float percentage, SVGPropertyBase* from, SVGPropertyBase* to)
{
    if (((animation_mode == kFromToAnimation || animation_mode == kToAnimation) && percentage > 0.5) || percentage == 1) {
        return to;
    }
    return from;
}

void SVGAnimateElement::CalculateAnimationValue(SMILAnimationValue& animation_value, float percentage, unsigned repeat_count) const
{
    DCHECK(targetElement());
    ...
    SVGPropertyBase* to_at_end_of_duration_value = to_at_end_of_duration_property_ ? to_at_end_of_duration_property_ : to_property_;
    SVGPropertyBase* from_value = GetAnimationMode() == kToAnimation ? animated_value : from_property_.Get();
    ...
    if (!AnimatedPropertyTypeSupportsAddition()) {
        animation_value.property_value = DiscreteSelectValue(GetAnimationMode(), percentage, from_value, to_value);
        return;
    }

    SMILAnimationEffectParameters parameters = ComputeEffectParameters();
    animated_value->CalculateAnimatedValue(parameters, percentage, repeat_count, from_value, to_value, to_at_end_of_duration_value, targetElement());
}
...
bool SVGAnimateElement::CalculateToAtEndOfDurationValue(const String& to_at_end_of_duration_string)
{
...
void SVGAnimateElement::CalculateFromAndToValues(const String& from_string, const String& to_string)
{
    DCHECK(targetElement());
...
void SVGAnimateElement::CalculateFromAndByValues(const String& from_string, const String& by_string)
{
    DCHECK(targetElement());
    DCHECK(GetAnimationMode() == kByAnimation || GetAnimationMode() == kFromByAnimation);
...
SVGPropertyBase* SVGAnimateElement::CreateUnderlyingValueForAnimation() const
{
    ...
    // CSS properties animation code-path.
    String base_value = ComputeCSSPropertyValue(targetElement(), css_property_id_);
...
void SVGAnimateElement::ClearAnimationValue()
{
    ...
    if (IsAnimatingCSSProperty()) {
        MutableCSSPropertyValueSet* property_set = target_element->EnsureAnimatedSMILStyleProperties();
        if (property_set->RemoveProperty(css_property_id_)) {
            target_element->SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kAnimation));
        }
    }
    // SVG DOM animVal animation code-path.
    if (IsAnimatingSVGDom())
...
void SVGAnimateElement::ApplyResultsToTarget(const SMILAnimationValue& animation_value)
{
    DCHECK(animation_value.property_value);
    DCHECK(targetElement());
    DCHECK_NE(type_, kAnimatedUnknown);
    ...
    if (IsAnimatingCSSProperty()) {
        ...
        MutableCSSPropertyValueSet* properties = target_element->EnsureAnimatedSMILStyleProperties();
        ...
        auto set_result = properties->ParseAndSetProperty(
            css_property_id_, animated_value_string, false, document.GetExecutionContext()->GetSecureContextMode(), document.ElementSheet().Contents());
...
float SVGAnimateElement::CalculateDistance(const String& from_string, const String& to_string)
{
    DCHECK(targetElement());
...
void SVGAnimateElement::SetAttributeType(const AtomicString& attribute_type_string)
{
