...
SVGElement::SVGElement(const QualifiedName& tag_name, Document& document, ConstructionType construction_type)
    : Element(tag_name, &document, construction_type)
    , svg_rare_data_(nullptr)
    , class_name_(MakeGarbageCollected<SVGAnimatedString>(this, html_names::kClassAttr))
{
...
void SVGElement::ReportAttributeParsingError(SVGParsingError error, const QualifiedName& name, const AtomicString& value)
{
    ...
    GetDocument().AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(
        mojom::ConsoleMessageSource::kRendering, mojom::ConsoleMessageLevel::kError, "Error: " + error.Format(tagName(), name, value)));
}
...
{
    ActiveInterpolationsMap active_interpolations_map = EffectStack::ActiveInterpolations(
        &GetElementAnimations()->GetEffectStack(), nullptr, nullptr, KeyframeEffect::kDefaultPriority, IsSVGAttributeHandle);
    for (auto& entry : active_interpolations_map) {
        const QualifiedName& attribute = entry.key.SvgAttribute();
        SVGInterpolationTypesMap map;
        SVGInterpolationEnvironment environment(map, *this, PropertyFromAttribute(attribute)->BaseValueBase());
...
template <typename T> static void ForSelfAndInstances(SVGElement* element, T callback)
{
    callback(element);
...
void SVGElement::ClearWebAnimatedAttributes()
{
    if (!HasSVGRareData())
        return;
    HashSet<QualifiedName>& animated_attributes = SvgRareData()->WebAnimatedAttributes();
...
void SVGElement::SetAnimatedAttribute(const QualifiedName& attribute, SVGPropertyBase* value)
{
    ...
    ForSelfAndInstances(this, [&attribute, &value](SVGElement* element) {
        if (SVGAnimatedPropertyBase* animated_property = element->PropertyFromAttribute(attribute)) {
            animated_property->SetAnimatedValue(value);
            element->SvgAttributeChanged({ *animated_property, attribute, AttributeModificationReason::kDirectly });
        }
    });
}
...
void SVGElement::SetAnimatedMotionTransform(const AffineTransform& motion_transform)
{
...
bool SVGElement::HasTransform(ApplyMotionTransformTag apply_motion_transform) const
{
    return (GetLayoutObject() && GetLayoutObject()->HasTransform()) || (apply_motion_transform == kIncludeMotionTransform && HasMotionTransform());
}

AffineTransform SVGElement::CalculateTransform(ApplyMotionTransformTag apply_motion_transform) const
{
    const LayoutObject* layout_object = GetLayoutObject();

    AffineTransform matrix;
    if (layout_object && layout_object->HasTransform()) {
        const gfx::RectF reference_box = TransformHelper::ComputeReferenceBox(*layout_object);
        matrix = TransformHelper::ComputeTransform(GetDocument(), layout_object->StyleRef(), reference_box, ComputedStyle::kIncludeTransformOrigin);
    }
...
Node::InsertionNotificationRequest SVGElement::InsertedInto(ContainerNode& root_parent)
{
...
void SVGElement::RemovedFrom(ContainerNode& root_parent)
{
    ...
    auto* root_parent_svg_element = DynamicTo<SVGElement>(root_parent.IsShadowRoot() ? root_parent.ParentOrShadowHostElement() : &root_parent);
    ...
    if (was_in_document && HasRelativeLengths()) {
        ...
        if (root_parent_svg_element && !ParentOrShadowHostElement()) {
            DCHECK(root_parent_svg_element->elements_with_relative_lengths_.Contains(this));
    ...
    DCHECK(!root_parent_svg_element || !root_parent_svg_element->elements_with_relative_lengths_.Contains(this));

    Element::RemovedFrom(root_parent);

    if (was_in_document) {
        if (SVGElement* corresponding_element = HasSVGRareData() ? SvgRareData()->CorrespondingElement() : nullptr) {
...
CSSPropertyID SVGElement::CssPropertyIdForSVGAttributeName(const ExecutionContext* execution_context, const QualifiedName& attr_name)
{
    ...
    if (!property_name_to_id_map) {
        ...
        for (const auto* qname : attr_names) {
            CSSPropertyID property_id = CssPropertyID(execution_context, qname->LocalName());
...
void SVGElement::UpdateRelativeLengthsInformation(bool client_has_relative_lengths, SVGElement* client_element)
{
    ...
    for (Node* current_node = this; current_node; current_node = current_node->ParentOrShadowHostNode()) {
    ...
    for (Element* current_node = this; current_node; current_node = current_node->ParentOrShadowHostElement()) {
...
void SVGElement::InvalidateRelativeLengthClients()
{
    ...
    base::AutoReset<bool> in_relative_length_clients_invalidation_change(&in_relative_length_clients_invalidation_, true);
#endif
    ...
    if (LayoutObject* layout_object = GetLayoutObject()) {
        if (HasRelativeLengths() && layout_object->IsSVGResourceContainer()) {
            ...
            resource_container->SetNeedsLayoutAndFullPaintInvalidation(layout_invalidation_reason::kSizeChanged);
            resource_container->InvalidateCache();
        } else if (SelfHasRelativeLengths()) {
            layout_object->SetNeedsLayoutAndFullPaintInvalidation(layout_invalidation_reason::kUnknown, kMarkContainerChain);
        }
    }
...
SVGElement* SVGElement::viewportElement() const
{
    ...
    ContainerNode* n = ParentOrShadowHostNode();
    while (n) {
        if (IsA<SVGSVGElement>(*n) || IsA<SVGImageElement>(*n) || IsA<SVGSymbolElement>(*n))
...
{
    DEFINE_STATIC_LOCAL(Persistent<HeapHashSet<WeakMember<SVGElement>>>, empty_instances, (MakeGarbageCollected<HeapHashSet<WeakMember<SVGElement>>>()));
...
{
    DCHECK(!HasSVGRareData() || !SvgRareData()->CorrespondingElement() || ContainingShadowRoot());
...
void SVGElement::ParseAttribute(const AttributeModificationParams& params)
{
    ...
    const AtomicString& event_name = HTMLElement::EventNameForAttributeName(params.name);
    if (!event_name.IsNull()) {
        SetAttributeEventListener(event_name, JSEventHandlerForContentAttribute::Create(GetExecutionContext(), params.name, params.new_value));
        return;
    }
...
AnimatedPropertyType SVGElement::AnimatedPropertyTypeForCSSAttribute(const QualifiedName& attribute_name)
{
...
SVGAnimatedPropertyBase* SVGElement::PropertyFromAttribute(const QualifiedName& attribute_name) const
{
...
{
    return value.length() > 5 && value.Is8Bit() && memcmp(value.Characters8(), "url(", 4) == 0;
}

bool UseCSSURIValueCacheForProperty(CSSPropertyID property_id)
{
    return property_id == CSSPropertyID::kFill || property_id == CSSPropertyID::kClipPath;
}

} // namespace

void SVGElement::AddPropertyToPresentationAttributeStyleWithCache(MutableCSSPropertyValueSet* style, CSSPropertyID property_id, const AtomicString& value)
{
    if (UseCSSURIValueCacheForProperty(property_id) && ProbablyUrlFunction(value)) {
        ...
        if (const CSSValue* cached_value = engine.GetCachedFillOrClipPathURIValue(value)) {
            AddPropertyToPresentationAttributeStyle(style, property_id, *cached_value);
        } else {
            ...
            if (unsigned count = style->PropertyCount()) {
                ...
                CSSPropertyValueSet::PropertyReference last_decl = style->PropertyAt(--count);
...
void SVGElement::CollectStyleForPresentationAttribute(const QualifiedName& name, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
    CSSPropertyID property_id = CssPropertyIdForSVGAttributeName(GetExecutionContext(), name);
    ...
    if (property && property->HasPresentationAttributeMapping()) {
        if (const CSSValue* css_value = property->CssValue()) {
            AddPropertyToPresentationAttributeStyle(style, property->CssPropertyId(), *css_value);
        }
        return;
    }
...
void SVGElement::AddedEventListener(const AtomicString& event_type, RegisteredEventListener& registered_listener)
{
    ...
    for (SVGElement* element : instances) {
        bool result = element->Node::AddEventListenerInternal(event_type, listener, options);
        DCHECK(result);
    }
}

void SVGElement::RemovedEventListener(const AtomicString& event_type, const RegisteredEventListener& registered_listener)
{
    ...
    for (SVGElement* shadow_tree_element : instances) {
        ...
        shadow_tree_element->Node::RemoveEventListenerInternal(event_type, listener, options);
    }
}
...
static bool HasLoadListener(Element* element)
{
    ...
    for (element = element->ParentOrShadowHostElement(); element; element = element->ParentOrShadowHostElement()) {
        EventListenerVector* entry = element->GetEventListeners(event_type_names::kLoad);
        if (!entry)
            continue;
...
bool SVGElement::SendSVGLoadEventIfPossible()
{
    if (!HaveLoadedRequiredResources())
        return false;
    if ((IsStructurallyExternal() || IsA<SVGSVGElement>(*this)) && HasLoadListener(this))
...
void SVGElement::AttributeChanged(const AttributeModificationParams& params)
{
    ...
    CSSPropertyID prop_id = CssPropertyIdForSVGAttributeName(GetExecutionContext(), params.name);
...
void SVGElement::BaseValueChanged(const SVGAnimatedPropertyBase& property)
{
    ...
    SvgAttributeChanged({ property, property.AttributeName(), AttributeModificationReason::kDirectly });
    if (class_name_ == &property) {
        UpdateClassList(g_null_atom, AtomicString(class_name_->BaseValue()->Value()));
    }
    UpdateWebAnimatedAttributeOnBaseValChange(property);
    InvalidateInstances();
}

void SVGElement::UpdateWebAnimatedAttributeOnBaseValChange(const SVGAnimatedPropertyBase& property)
{
    if (!HasSVGRareData())
        return;
    const auto& animated_attributes = SvgRareData()->WebAnimatedAttributes();
    if (animated_attributes.empty() || !animated_attributes.Contains(property.AttributeName())) {
        return;
    }
...
void SVGElement::EnsureAttributeAnimValUpdated()
{
    ...
    if ((HasSVGRareData() && SvgRareData()->WebAnimatedAttributesDirty())
        || (GetElementAnimations() && GetDocument().GetDocumentAnimations().NeedsAnimationTimingUpdate())) {
...
MutableCSSPropertyValueSet* SVGElement::GetPresentationAttributeStyleForDirectUpdate()
{
    ...
    return To<MutableCSSPropertyValueSet>(element_data.presentation_attribute_style_.Get());
}

void SVGElement::UpdatePresentationAttributeStyle(const SVGAnimatedPropertyBase& property)
{
    ...
    SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::FromAttribute(property.AttributeName()));
}

void SVGElement::UpdatePresentationAttributeStyle(CSSPropertyID property_id, const QualifiedName& attr_name, const AtomicString& value)
{
    auto set_result = MutableCSSPropertyValueSet::kModifiedExisting;
    if (auto* mutable_style = GetPresentationAttributeStyleForDirectUpdate()) {
        auto* execution_context = GetExecutionContext();
        set_result = mutable_style->ParseAndSetProperty(property_id, value, false,
            execution_context ? execution_context->GetSecureContextMode() : SecureContextMode::kInsecureContext, GetDocument().ElementSheet().Contents());
    ...
    if (set_result >= MutableCSSPropertyValueSet::kModifiedExisting) {
        SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::FromAttribute(attr_name));
    }
}

void SVGElement::AddAnimatedPropertyToPresentationAttributeStyle(const SVGAnimatedPropertyBase& property, MutableCSSPropertyValueSet* style)
{
    DCHECK(property.HasPresentationAttributeMapping());
    ...
    AddPropertyToPresentationAttributeStyle(style, property.CssPropertyId(), *value);
}

const ComputedStyle* SVGElement::CustomStyleForLayoutObject(const StyleRecalcContext& style_recalc_context)
{
    ...
    CSSParserContext::ParserModeOverridingScope scope(*GetDocument().ElementSheet().Contents()->ParserContext(), kSVGAttributeMode);
    ...
    corresponding_recalc_context.old_style = PostStyleUpdateScope::GetOldStyle(*this);
    return GetDocument().GetStyleResolver().ResolveStyle(corresponding_element, corresponding_recalc_context, style_request);
}
...
{
    return HasEventListeners(event_type_names::kFocusin) || HasEventListeners(event_type_names::kFocusout) || HasEventListeners(event_type_names::kFocus)
        || HasEventListeners(event_type_names::kBlur);
}

void SVGElement::MarkForLayoutAndParentResourceInvalidation(LayoutObject& layout_object)
{
    LayoutSVGResourceContainer::MarkForLayoutAndParentResourceInvalidation(layout_object, true);
}

void SVGElement::NotifyResourceClients() const
{
    LocalSVGResource* resource = GetTreeScope().EnsureSVGTreeScopedResources().ExistingResourceForId(GetIdAttribute());
...
void SVGElement::SetNeedsStyleRecalcForInstances(StyleChangeType change_type, const StyleChangeReasonForTracing& reason)
{
    const HeapHashSet<WeakMember<SVGElement>>& set = InstancesForElement();
    if (set.empty())
        return;
...
SVGElementSet& SVGElement::GetDependencyTraversalVisitedSet()
{
    ...
    DEFINE_STATIC_LOCAL(Persistent<SVGElementSet>, invalidating_dependencies, (MakeGarbageCollected<SVGElementSet>()));
...
void SVGElement::SynchronizeListOfSVGAttributes(const base::span<SVGAnimatedPropertyBase*> attributes)
{
...
void SVGElement::AddAnimatedPropertiesToPresentationAttributeStyle(
    const base::span<const SVGAnimatedPropertyBase*> properties, MutableCSSPropertyValueSet* style)
{
...
void SVGElement::AttachLayoutTree(AttachContext& context)
{
    ...
    if (!context.performing_reattach && GetLayoutObject() && GetSMILAnimations()) {
