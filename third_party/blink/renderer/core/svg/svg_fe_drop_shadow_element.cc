...
SVGFEDropShadowElement::SVGFEDropShadowElement(Document& document)
    : SVGFilterPrimitiveStandardAttributes(svg_names::kFEDropShadowTag, document)
    ...
    , std_deviation_(MakeGarbageCollected<SVGAnimatedNumberOptionalNumber>(this, svg_names::kStdDeviationAttr, 2))
...
bool SVGFEDropShadowElement::SetFilterEffectAttribute(FilterEffect* effect, const QualifiedName& attr_name)
{
    const ComputedStyle& style = ComputedStyleRef();

    FEDropShadow* drop_shadow = static_cast<FEDropShadow*>(effect);
    if (attr_name == svg_names::kFloodColorAttr) {
        drop_shadow->SetShadowColor(style.VisitedDependentColor(GetCSSPropertyFloodColor()));
        return true;
    }
...
void SVGFEDropShadowElement::SvgAttributeChanged(const SvgAttributeChangedParams& params)
{
    const QualifiedName& attr_name = params.name;
    if (attr_name == svg_names::kInAttr || attr_name == svg_names::kStdDeviationAttr || attr_name == svg_names::kDxAttr || attr_name == svg_names::kDyAttr) {
        Invalidate();
        return;
    }
...
FilterEffect* SVGFEDropShadowElement::Build(SVGFilterBuilder* filter_builder, Filter* filter)
{
    ...
    auto* effect = MakeGarbageCollected<FEDropShadow>(filter, std_dev_x, std_dev_y, dx_->CurrentValue()->Value(), dy_->CurrentValue()->Value(), color, opacity);
...
SVGAnimatedPropertyBase* SVGFEDropShadowElement::PropertyFromAttribute(const QualifiedName& attribute_name) const
{
...
{
    SVGAnimatedPropertyBase* attrs[] { dx_.Get(), dy_.Get(), std_deviation_.Get(), in1_.Get() };
