...
enum class SVGTransformType { kUnknown = 0, kMatrix = 1, kTranslate = 2, kScale = 3, kRotate = 4, kSkewx = 5, kSkewy = 6 };

class SVGTransform final : public SVGListablePropertyBase {
public:
    typedef SVGTransformTearOff TearOffType;

    enum ConstructionMode { kConstructIdentityTransform, kConstructZeroTransform };

    SVGTransform();
    explicit SVGTransform(SVGTransformType, ConstructionMode = kConstructIdentityTransform);
    explicit SVGTransform(const AffineTransform&);
    SVGTransform(SVGTransformType transform_type, float angle, const gfx::PointF& center, const AffineTransform& matrix)
    ...
    SVGTransformType TransformType() const
    {
    ...
    gfx::PointF RotationCenter() const
    {
        return center_;
    }
    ...
    void CalculateAnimatedValue(const SMILAnimationEffectParameters&, float percentage, unsigned repeat_count, const SVGPropertyBase* from,
        const SVGPropertyBase* to, const SVGPropertyBase* to_at_end_of_duration_value, const SVGElement* context_element) override;
    float CalculateDistance(const SVGPropertyBase* to, const SVGElement* context_element) const override;

    static AnimatedPropertyType ClassType()
    {
        return kAnimatedTransform;
    }
...
template <> struct DowncastTraits<SVGTransform> {
