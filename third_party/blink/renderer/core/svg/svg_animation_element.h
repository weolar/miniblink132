...
enum CalcMode { kCalcModeDiscrete, kCalcModeLinear, kCalcModePaced, kCalcModeSpline };
...
private:
    ...
    virtual bool CalculateToAtEndOfDurationValue(const String& to_at_end_of_duration_string) = 0;
    virtual void CalculateFromAndToValues(const String& from_string, const String& to_string) = 0;
    virtual void CalculateFromAndByValues(const String& from_string, const String& by_string) = 0;
    virtual void CalculateAnimationValue(SMILAnimationValue&, float percent, unsigned repeat_count) const = 0;
    virtual float CalculateDistance(const String& /*fromString*/, const String& /*toString*/)
    {
        return -1.f;
    }

    float CurrentValuesForValuesAnimation(float percent, String& from, String& to) const;
    ...
    {
        return use_paced_key_times_ ? key_times_for_paced_ : key_times_from_attribute_;
    }

    float CalculatePercentFromKeyPoints(float percent) const;
    float CurrentValuesFromKeyPoints(float percent, String& from, String& to) const;
