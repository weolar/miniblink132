...
SVGPatternElement::SVGPatternElement(Document& document)
    ...
    , x_(MakeGarbageCollected<SVGAnimatedLength>(this, svg_names::kXAttr, SVGLengthMode::kWidth, SVGLength::Initial::kUnitlessZero))
    , y_(MakeGarbageCollected<SVGAnimatedLength>(this, svg_names::kYAttr, SVGLengthMode::kHeight, SVGLength::Initial::kUnitlessZero))
    ...
    , pattern_transform_(MakeGarbageCollected<SVGAnimatedTransformList>(this, svg_names::kPatternTransformAttr, CSSPropertyID::kTransform))
    , pattern_units_(MakeGarbageCollected<SVGAnimatedEnumeration<SVGUnitTypes::SVGUnitType>>(
          this, svg_names::kPatternUnitsAttr, SVGUnitTypes::kSvgUnitTypeObjectboundingbox))
    , pattern_content_units_(MakeGarbageCollected<SVGAnimatedEnumeration<SVGUnitTypes::SVGUnitType>>(
          this, svg_names::kPatternContentUnitsAttr, SVGUnitTypes::kSvgUnitTypeUserspaceonuse))
{
}
...
void SVGPatternElement::SvgAttributeChanged(const SvgAttributeChangedParams& params)
{
    const QualifiedName& attr_name = params.name;
    bool is_length_attr
        = attr_name == svg_names::kXAttr || attr_name == svg_names::kYAttr || attr_name == svg_names::kWidthAttr || attr_name == svg_names::kHeightAttr;

    if (attr_name == svg_names::kPatternTransformAttr) {
        UpdatePresentationAttributeStyle(*pattern_transform_);
    }

    if (is_length_attr || attr_name == svg_names::kPatternUnitsAttr || attr_name == svg_names::kPatternContentUnitsAttr
        || attr_name == svg_names::kPatternTransformAttr || SVGFitToViewBox::IsKnownAttribute(attr_name) || SVGTests::IsKnownAttribute(attr_name)) {
        if (is_length_attr)
...
Node::InsertionNotificationRequest SVGPatternElement::InsertedInto(ContainerNode& root_parent)
{
...
static void SetPatternAttributes(const SVGPatternElement& element, PatternAttributes& attributes)
{
    ...
    if (!attributes.HasPatternContentUnits() && element.patternContentUnits()->IsSpecified()) {
        attributes.SetPatternContentUnits(element.patternContentUnits()->CurrentEnumValue());
    }

    if (!attributes.HasPatternTransform() && element.HasTransform(SVGElement::kExcludeMotionTransform)) {
        attributes.SetPatternTransform(element.CalculateTransform(SVGElement::kExcludeMotionTransform));
    }
...
{
    return DynamicTo<SVGPatternElement>(TargetElementFromIRIString(HrefString(), GetTreeScope()));
}
...
SVGAnimatedPropertyBase* SVGPatternElement::PropertyFromAttribute(const QualifiedName& attribute_name) const
{
...
{
    SVGAnimatedPropertyBase* attrs[] { x_.Get(), y_.Get(), width_.Get(), height_.Get(), pattern_transform_.Get(), pattern_units_.Get(),
...
void SVGPatternElement::CollectExtraStyleForPresentationAttribute(MutableCSSPropertyValueSet* style)
{
