...
TEST_F(SVGUseElementTest, InstanceInvalidatedWhenNonAttachedTargetRemoved)
{
    ...
    // removed.
    auto* use = To<SVGUseElement>(GetDocument().getElementById(AtomicString("use")));
    ASSERT_TRUE(use);
    ASSERT_TRUE(use->GetShadowRoot());
    ASSERT_FALSE(use->GetShadowRoot()->getElementById(AtomicString("target")));
}

TEST_F(SVGUseElementTest, InstanceInvalidatedWhenNonAttachedTargetMovedInDocument)
{
    GetDocument().body()->setInnerHTML(R"HTML(
    <svg>
...
TEST_F(SVGUseElementTest, NullInstanceRootWhenNotConnectedToDocument)
{
    ...
    auto* target = To<SVGUseElement>(GetDocument().getElementById(AtomicString("target")));
    ASSERT_TRUE(target);
...
TEST_F(SVGUseElementTest, NullInstanceRootWhenConnectedToInactiveDocument)
{
    ...
    Document* other_document = GetDocument().implementation().createHTMLDocument();
...
TEST_F(SVGUseElementTest, NullInstanceRootWhenShadowTreePendingRebuild)
{
    ...
    GetDocument().getElementById(AtomicString("r"))->setAttribute(html_names::kWidthAttr, AtomicString("50"));
