...
template <typename CharType> SVGParsingError SVGLengthList::ParseInternal(const CharType* ptr, const CharType* end)
{
...
SVGParsingError SVGLengthList::SetValueAsString(const String& value)
{
    ...
    return WTF::VisitCharacters(value, [&](auto chars) { return ParseInternal(chars.data(), chars.data() + chars.size()); });
}

void SVGLengthList::Add(const SVGPropertyBase* other, const SVGElement* context_element)
{
...
void SVGLengthList::CalculateAnimatedValue(const SMILAnimationEffectParameters& parameters, float percentage, unsigned repeat_count,
    const SVGPropertyBase* from_value, const SVGPropertyBase* to_value, const SVGPropertyBase* to_at_end_of_duration_value, const SVGElement* context_element)
{
    ...
    auto* to_at_end_of_duration_list = To<SVGLengthList>(to_at_end_of_duration_value);

    uint32_t from_list_size = from_list->length();
    uint32_t to_list_size = to_list->length();
    uint32_t to_at_end_of_duration_list_size = to_at_end_of_duration_list->length();

    const bool needs_neutral_element = !from_list_size || to_list_size != to_at_end_of_duration_list_size;
    const SVGLength* neutral = needs_neutral_element ? CreatePaddingItem() : nullptr;
    for (uint32_t i = 0; i < to_list_size; ++i) {
        const SVGLength* from = from_list_size ? from_list->at(i) : neutral;
        const SVGLength* to_at_end = i < to_at_end_of_duration_list_size ? to_at_end_of_duration_list->at(i) : neutral;
        at(i)->CalculateAnimatedValue(parameters, percentage, repeat_count, from, to_list->at(i), to_at_end, context_element);
    }
}

float SVGLengthList::CalculateDistance(const SVGPropertyBase* to, const SVGElement*) const
{
