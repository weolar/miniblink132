...
void AppendErrorContextInfo(StringBuilder& builder, const String& tag_name, const QualifiedName& name)
{
    builder.Append('<');
...
bool DisableLocus(SVGParseStatus status)
{
    ...
    return status == SVGParseStatus::kNegativeValue || status == SVGParseStatus::kZeroValue || status == SVGParseStatus::kParsingFailed;
}
...
void AppendValue(StringBuilder& builder, SVGParsingError error, const AtomicString& value)
{
    ...
    if (!error.HasLocus() || DisableLocus(error.Status())) {
        ...
        EscapeStringForJSON(value.GetString().Substring(context_start, context_end - context_start), &builder);
...
String SVGParsingError::Format(const String& tag_name, const QualifiedName& name, const AtomicString& value) const
{
