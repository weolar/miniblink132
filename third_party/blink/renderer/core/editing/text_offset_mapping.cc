...
bool HasNonPsuedoNode(const LayoutObject& parent)
{
    if (parent.NonPseudoNode())
        return true;
    for (const LayoutObject* runner = &parent; runner; runner = runner->NextInPreOrder(&parent)) {
...
Node* PreviousNodeSkippingAncestors(const Node& node)
{
    ...
    for (Node* runner = FlatTreeTraversal::Previous(node); runner; runner = FlatTreeTraversal::Previous(*runner)) {
        if (runner != parent)
            return runner;
...
const LayoutBlockFlow& RootInlineContentsContainerOf(const LayoutBlockFlow& block_flow)
{
    DCHECK(block_flow.ChildrenInline()) << block_flow;
    const LayoutBlockFlow* root_block_flow = &block_flow;
    for (const LayoutBlock* runner = block_flow.ContainingBlock(); runner; runner = runner->ContainingBlock()) {
    ...
    DCHECK(!root_block_flow->IsAtomicInlineLevel()) << block_flow << ' ' << root_block_flow;
...
LayoutObject* NextForInlineContents(const LayoutObject& layout_object, const LayoutObject& container)
{
...
{
    for (const LayoutObject* layout_object = container.SlowFirstChild(); layout_object; layout_object = NextForInlineContents(*layout_object, container)) {
...
// |LayoutBlockFlow|.
const LayoutBlockFlow* ComputeInlineContentsAsBlockFlow(const LayoutObject& layout_object)
{
    ...
    if (block_flow->IsAtomicInlineLevel()
        || (!RuntimeEnabledFeatures::TextSegmentBoundaryForElementWithFloatStyleEnabled() && block_flow->IsFloatingOrOutOfFlowPositioned())) {
        const LayoutBlockFlow& root_block_flow = RootInlineContentsContainerOf(*block_flow);
...
TextOffsetMapping::InlineContents CreateInlineContentsFromBlockFlow(const LayoutBlockFlow& block_flow, const LayoutObject& target)
{
    DCHECK(block_flow.ChildrenInline()) << block_flow;
    ...
    for (layout_object = block_flow.FirstChild(); layout_object; layout_object = NextForInlineContents(*layout_object, block_flow)) {
    ...
    for (; layout_object; layout_object = NextForInlineContents(*layout_object, block_flow)) {
        if (layout_object->IsBlockInInline()) {
    ...
    DCHECK(last);
    return TextOffsetMapping::InlineContents(block_flow, block_in_inline_before, *first, *last, block_in_inline_after);
}

TextOffsetMapping::InlineContents ComputeInlineContentsFromNode(const Node& node)
{
    const LayoutObject* const layout_object = node.GetLayoutObject();
    if (!layout_object)
        return TextOffsetMapping::InlineContents();
    const LayoutBlockFlow* const block_flow = ComputeInlineContentsAsBlockFlow(*layout_object);
    if (!block_flow)
        return TextOffsetMapping::InlineContents();
...
TextOffsetMapping::TextOffsetMapping(const InlineContents& inline_contents, const TextIteratorBehavior& behavior)
    : behavior_(behavior)
...
TextOffsetMapping::TextOffsetMapping(const InlineContents& inline_contents)
    : TextOffsetMapping(
        inline_contents, TextIteratorBehavior::Builder().SetEmitsCharactersBetweenAllVisiblePositions(true).SetEmitsSmallXForTextSecurity(true).Build())
{
}

int TextOffsetMapping::ComputeTextOffset(const PositionInFlatTree& position) const
{
    if (position <= range_.StartPosition())
        return 0;
    if (position >= range_.EndPosition())
        return text16_.length();
    return TextIteratorInFlatTree::RangeLength(range_.StartPosition(), position, behavior_);
}
...
EphemeralRangeInFlatTree TextOffsetMapping::ComputeRange(unsigned start, unsigned end) const
{
    DCHECK_LE(end, text16_.length());
    DCHECK_LE(start, end);
    if (start == end)
        return EphemeralRangeInFlatTree();
    return EphemeralRangeInFlatTree(GetPositionBefore(start), GetPositionAfter(end));
}

unsigned TextOffsetMapping::FindNonWhitespaceCharacterFrom(unsigned offset) const
{
...
// static
TextOffsetMapping::BackwardRange TextOffsetMapping::BackwardRangeOf(const PositionInFlatTree& position)
{
    return BackwardRange(FindBackwardInlineContents(position));
}

// static
TextOffsetMapping::ForwardRange TextOffsetMapping::ForwardRangeOf(const PositionInFlatTree& position)
{
    return ForwardRange(FindForwardInlineContents(position));
}

// static
template <typename Traverser> TextOffsetMapping::InlineContents TextOffsetMapping::FindInlineContentsInternal(const Node* start_node, Traverser traverser)
{
...
// static
TextOffsetMapping::InlineContents TextOffsetMapping::FindBackwardInlineContents(const PositionInFlatTree& position)
{
    const Node* previous_node = position.NodeAsRangeLastNode();
    if (!previous_node)
        return InlineContents();

    if (const TextControlElement* enclosing_text_control = EnclosingTextControl(position)) {
        if (!FlatTreeTraversal::IsDescendantOf(*previous_node, *enclosing_text_control)) {
            // The first position in a text control reaches here.
            return InlineContents();
        }

        return TextOffsetMapping::FindInlineContentsInternal(
            previous_node, [enclosing_text_control](const Node& node) { return FlatTreeTraversal::Previous(node, enclosing_text_control); });
    }
    ...
    auto previous_skipping_text_control = [](const Node& node) -> const Node* {
        ...
        if (!previous)
            return previous;
        const TextControlElement* previous_text_control = EnclosingTextControl(previous);
    ...
    if (const TextControlElement* last_enclosing_text_control = EnclosingTextControl(previous_node)) {
        // Example, <input value=foo><span>bar</span>, span@beforeAnchor
        return TextOffsetMapping::FindInlineContentsInternal(last_enclosing_text_control, previous_skipping_text_control);
    }
    return TextOffsetMapping::FindInlineContentsInternal(previous_node, previous_skipping_text_control);
}

// static
// Note: "doubleclick-whitespace-img-crash.html" call |NextWordPosition())
// with AfterNode(IMG) for <body><img></body>
TextOffsetMapping::InlineContents TextOffsetMapping::FindForwardInlineContents(const PositionInFlatTree& position)
{
    ...
    if (const TextControlElement* enclosing_text_control = EnclosingTextControl(position)) {
        if (!FlatTreeTraversal::IsDescendantOf(*next_node, *enclosing_text_control)) {
            // The last position in a text control reaches here.
            return InlineContents();
        }

        return TextOffsetMapping::FindInlineContentsInternal(
            next_node, [enclosing_text_control](const Node& node) { return FlatTreeTraversal::Next(node, enclosing_text_control); });
    }
    ...
    return TextOffsetMapping::FindInlineContentsInternal(next_node, next_skipping_text_control);
}

// ----

TextOffsetMapping::InlineContents::InlineContents(const LayoutBlockFlow& block_flow)
...
TextOffsetMapping::InlineContents::InlineContents(const LayoutBlockFlow& block_flow, const LayoutObject* block_in_inline_before, const LayoutObject& first,
    const LayoutObject& last, const LayoutObject* block_in_inline_after)
    : block_flow_(&block_flow)
...
{
    DCHECK(!block_in_inline_before_ || block_in_inline_before_->IsBlockInInline()) << block_in_inline_before_;
    DCHECK(!block_in_inline_after_ || block_in_inline_after_->IsBlockInInline()) << block_in_inline_after_;
...
bool TextOffsetMapping::InlineContents::operator==(const InlineContents& other) const
{
    return block_flow_ == other.block_flow_;
}

const LayoutBlockFlow* TextOffsetMapping::InlineContents::GetEmptyBlock() const
{
    DCHECK(block_flow_ && !first_ && !last_);
    return block_flow_;
}

const LayoutObject& TextOffsetMapping::InlineContents::FirstLayoutObject() const
{
    DCHECK(first_);
    return *first_;
}

const LayoutObject& TextOffsetMapping::InlineContents::LastLayoutObject() const
{
    DCHECK(last_);
    return *last_;
}
...
EphemeralRangeInFlatTree TextOffsetMapping::InlineContents::GetRange() const
{
    ...
    if (!first_) {
        ...
        return EphemeralRangeInFlatTree(PositionInFlatTree::FirstPositionInNode(node), PositionInFlatTree::LastPositionInNode(node));
    }
    ...
    return EphemeralRangeInFlatTree(first_text_node ? PositionInFlatTree(first_node, 0) : PositionInFlatTree::BeforeNode(first_node),
        last_text_node ? PositionInFlatTree(last_node, last_text_node->length()) : PositionInFlatTree::AfterNode(last_node));
}

PositionInFlatTree TextOffsetMapping::InlineContents::LastPositionBeforeBlockFlow() const
{
    DCHECK(block_flow_);
    if (block_in_inline_before_) {
        for (const LayoutObject* block = block_in_inline_before_->SlowLastChild(); block; block = block->PreviousSibling()) {
...
PositionInFlatTree TextOffsetMapping::InlineContents::FirstPositionAfterBlockFlow() const
{
    DCHECK(block_flow_);
    if (block_in_inline_after_) {
        for (const LayoutObject* block = block_in_inline_after_->SlowFirstChild(); block; block = block->NextSibling()) {
            if (auto* block_node = block->NonPseudoNode())
...
// static
TextOffsetMapping::InlineContents TextOffsetMapping::InlineContents::NextOf(const InlineContents& inline_contents)
{
    const PositionInFlatTree position_after = inline_contents.FirstPositionAfterBlockFlow();
...
// static
TextOffsetMapping::InlineContents TextOffsetMapping::InlineContents::PreviousOf(const InlineContents& inline_contents)
{
    const PositionInFlatTree position_before = inline_contents.LastPositionBeforeBlockFlow();
    if (position_before.IsNull())
        return InlineContents();
    return TextOffsetMapping::FindBackwardInlineContents(position_before);
}

std::ostream& operator<<(std::ostream& ostream, const TextOffsetMapping::InlineContents& inline_contents)
{
    return ostream << '[' << inline_contents.FirstLayoutObject() << ", " << inline_contents.LastLayoutObject() << ']';
}

// ----

TextOffsetMapping::InlineContents TextOffsetMapping::BackwardRange::Iterator::operator*() const
{
...
TextOffsetMapping::InlineContents TextOffsetMapping::ForwardRange::Iterator::operator*() const
{
    DCHECK(current_.IsNotNull());
    return current_;
}
