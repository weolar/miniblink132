...
public:
    ...
    const Node* ConstrainingAncestor() const
    {
        return constraining_ancestor_;
    }
    AbsoluteURLs ShouldResolveURLs() const
    {
        return should_resolve_urls_;
    }
...
public:
    Builder() = default;
    explicit Builder(const CreateMarkupOptions& options)
        : data_(options)
    {
    }

    CreateMarkupOptions Build() const
    {
        return data_;
    }
    ...
    Builder& SetIgnoresCSSTextTransformsForRenderedText(bool text_without_transforms);
