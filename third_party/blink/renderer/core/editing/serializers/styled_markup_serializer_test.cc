...
protected:
    ...
    {
        return CreateMarkupOptions::Builder().SetShouldAnnotateForInterchange(true).Build();
    }

    template <typename Strategy> std::string Serialize(const CreateMarkupOptions& options = CreateMarkupOptions());

    template <typename Strategy>
    std::string SerializePart(
        const PositionTemplate<Strategy>& start, const PositionTemplate<Strategy>& end, const CreateMarkupOptions& options = CreateMarkupOptions());
};

template <typename Strategy> std::string StyledMarkupSerializerTest::Serialize(const CreateMarkupOptions& options)
{
    PositionTemplate<Strategy> start = PositionTemplate<Strategy>::FirstPositionInNode(*GetDocument().body());
    PositionTemplate<Strategy> end = PositionTemplate<Strategy>::LastPositionInNode(*GetDocument().body());
    return CreateMarkup(start, end, options).Utf8();
}

template <typename Strategy>
std::string StyledMarkupSerializerTest::SerializePart(
    const PositionTemplate<Strategy>& start, const PositionTemplate<Strategy>& end, const CreateMarkupOptions& options)
{
    return CreateMarkup(start, end, options).Utf8();
}
...
TEST_F(StyledMarkupSerializerTest, TextOnly)
{
    ...
    const char* expected_result = "<span style=\"display: inline !important; float: none;\">Hello "
                                  "world!</span>";
...
{
    const char* body_content = "<select><option value=\"1\">one</option><option "
...
TEST_F(StyledMarkupSerializerTest, FormControlTextArea)
{
    ...
    EXPECT_EQ(expected_result, Serialize<EditingStrategy>()) << "contents of TEXTAREA element should not be appeared.";
    EXPECT_EQ(expected_result, Serialize<EditingInFlatTreeStrategy>());
}
...
TEST_F(StyledMarkupSerializerTest, ShadowTreeDistributeOrder)
{
    const char* body_content = "<p id=\"host\">00<b slot='#one' id=\"one\">11</b><b slot='#two' "
                               "id=\"two\">22</b>33</p>";
    const char* shadow_content = "<a><slot name='#two'></slot><slot name='#one'></slot></a>";
    SetBodyContent(body_content);
    SetShadowContent(shadow_content, "host");
    EXPECT_EQ("<p id=\"host\"><b slot=\"#one\" id=\"one\">11</b><b slot=\"#two\" "
              "id=\"two\">22</b></p>",
        Serialize<EditingStrategy>())
        << "00 and 33 aren't appeared since they aren't distributed.";
    EXPECT_EQ("<p id=\"host\"><a><slot name=\"#two\"><b slot=\"#two\" "
...
TEST_F(StyledMarkupSerializerTest, ShadowTreeNested)
{
    const char* body_content = "<p id='host'>00<b slot='#one' id='one'>11</b><b slot='#two' "
                               "id='two'>22</b>33</p>";
    const char* shadow_content1 = "<a><slot name='#two'></slot><span id=host2></span><slot "
                                  "name='#one'></slot></a>";
    const char* shadow_content2 = "NESTED";
    SetBodyContent(body_content);
    ShadowRoot* shadow_root1 = SetShadowContent(shadow_content1, "host");
    CreateShadowRootForElementWithIDAndSetInnerHTML(*shadow_root1, "host2", shadow_content2);
...
TEST_F(StyledMarkupSerializerTest, ShadowTreeInterchangedNewline)
{
    ...
    std::string result_from_dom_tree = Serialize<EditingStrategy>(ShouldAnnotateOptions());
    std::string result_from_flat_tree = Serialize<EditingInFlatTreeStrategy>(ShouldAnnotateOptions());
    size_t mismatched_index = Mismatch(result_from_dom_tree, result_from_flat_tree);
    ...
    EXPECT_EQ("b slot=\"#one\" id=\"one\">1</b></span>", result_from_dom_tree.substr(mismatched_index));
    EXPECT_EQ("slot name=\"#one\"><b slot=\"#one\" "
...
{
    const char* body_content = "<p id='host' style='color: red'><span style='font-weight: bold;'><span "
    ...
    const std::string& serialized_dom = SerializePart<EditingStrategy>(start_dom, end_dom, ShouldAnnotateOptions());

    body_content = "<p id='host' style='color: red'>00<span slot='#one' "
                   "id='one'>11</span>22</p>\n";
    const char* shadow_content = "<span style='font-weight: bold'><slot name='#one'></slot></span>";
    SetBodyContent(body_content);
    SetShadowContent(shadow_content, "host");
    ...
    const std::string& serialized_ict = SerializePart<EditingInFlatTreeStrategy>(start_ict, end_ict, ShouldAnnotateOptions());
...
{
    const char* body_content = "<p id='host1'>[<span id='one'>11</span>]</p><p id='host2'>[<span "
                               "id='two'>22</span>]</p>";
    SetBodyContent(body_content);
    Element* one = GetDocument().getElementById(AtomicString("one"));
    ...
    body_content = "<p id='host1'><span slot='#one' id='one'>11</span></p><p "
                   "id='host2'><span "
...
{
    const char* body_content = "<span id='span1' style='display: none'>11</span><span id='span2' "
...
TEST_F(StyledMarkupSerializerTest, DisplayContentsStyle)
{
    ...
    const char* expected_result = "<span style=\"display: inline !important; float: none;\">1</span><span "
