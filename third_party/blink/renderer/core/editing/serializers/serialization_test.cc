...
TEST_F(SerializationTest, CantCreateFragmentCrash)
{
    ...
    // instead of crashing.
    const String html = "<article><dcell></dcell>A<td><dcol></"
    ...
    DocumentFragment* strictly_processed_fragment = CreateStrictlyProcessedFragmentFromMarkupWithContext(GetDocument(), html, 0, html.length(), KURL());
...
TEST_F(SerializationTest, CreateFragmentWithDataUrlCrash)
{
    ...
    // data: URL.
    const String html = "<div style=\"filter: url(data:image/gif;base64,xx);\">"
...
{
    InsertStyleElement("a { color: #010101; }"
                       "a:link { color: #020202; }"
                       "a:visited { color: #030303; }");
    SetBodyContent("<a id=a1>text</a>"
    ...
    ASSERT_THAT(style1.VisitedDependentColor(GetCSSPropertyColor()), Color::FromRGB(1, 1, 1)) << "should not be :visited/:link color";
    EXPECT_THAT(SerailizeToHTMLText(a1), MatchesRegex(R"re(<a id="a1" style=".*;? ?color: rgb\(1, 1, 1\);.*">text</a>)re"));
    ...
    ASSERT_THAT(style2.VisitedDependentColor(GetCSSPropertyColor()), Color::FromRGB(3, 3, 3)) << "should be :visited color";
    EXPECT_THAT(SerailizeToHTMLText(a2), MatchesRegex(R"re(<a id="a2" href="" style=".*;? ?color: rgb\(2, 2, 2\);.*">visited</a>)re"));

    // a3
    ASSERT_THAT(style3.InsideLink(), EInsideLink::kInsideUnvisitedLink);
    ASSERT_THAT(style3.VisitedDependentColor(GetCSSPropertyColor()), Color::FromRGB(2, 2, 2)) << "should be :link color";
    EXPECT_THAT(SerailizeToHTMLText(a3), MatchesRegex(R"re(<a id="a3" href="https://1.1.1.1/" style=".*;? ?color: rgb\(2, 2, 2\);.*">unvisited</a>)re"));
}

// Regression test for https://crbug.com/1032389
TEST_F(SerializationTest, SVGForeignObjectCrash)
{
    const String markup = "<svg>"
                          "  <foreignObject>"
                          "    <br>"
                          ...
                          "</svg>"
                          "<span>\u00A0</span>";
    DocumentFragment* strictly_processed_fragment = CreateStrictlyProcessedFragmentFromMarkupWithContext(GetDocument(), markup, 0, markup.length(), KURL());
