...
private:
    ...
    void ProcessChildrenWithRequiredLineBreaks(const Node& node, int required_line_break_count);
...
// static
bool ElementInnerTextCollector::ShouldEmitNewlineForTableRow(const LayoutTableRow& table_row)
{
    ...
    for (const LayoutObject* runner = table_section->NextSibling(); runner; runner = runner->NextSibling()) {
...
const OffsetMapping* ElementInnerTextCollector::GetOffsetMapping(const LayoutText& layout_text)
{
    ...
    LayoutBlockFlow* const block_flow = OffsetMapping::GetInlineFormattingContextOf(layout_text);
...
void ElementInnerTextCollector::ProcessChildrenWithRequiredLineBreaks(const Node& node, int required_line_break_count)
{
...
void ElementInnerTextCollector::ProcessLayoutText(const LayoutText& layout_text, const Text& text_node)
{
    ...
    for (const OffsetMappingUnit& unit : mapping->GetMappingUnitsForNode(text_node)) {
        result_.EmitText(StringView(mapping->GetText(), unit.TextContentStart(), unit.TextContentEnd() - unit.TextContentStart()));
    }
}
...
void ElementInnerTextCollector::ProcessNode(const Node& node)
{
    ...
    if (style->Display() == EDisplay::kTableRow) {
        ProcessChildren(node);
        if (layout_object.IsTableRow() && ShouldEmitNewlineForTableRow(To<LayoutTableRow>(layout_object))) {
            result_.EmitNewline();
        }
        return;
    }
...
void ElementInnerTextCollector::ProcessOptionElement(const HTMLOptionElement& option_element)
{
...
void ElementInnerTextCollector::ProcessSelectElement(const HTMLSelectElement& select_element)
{
...
void ElementInnerTextCollector::ProcessTextNode(const Text& node)
{
    ...
    if (LayoutText* first_letter_part = layout_text.GetFirstLetterPart()) {
        if (layout_text.HasEmptyText()
            || OffsetMapping::GetInlineFormattingContextOf(layout_text) != OffsetMapping::GetInlineFormattingContextOf(*first_letter_part)) {
