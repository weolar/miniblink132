...
inline bool FullyClipsContents(const Node* node)
{
    ...
    if (!layout_object || !layout_object->IsBox() || !layout_object->IsScrollContainer() || IsA<LayoutView>(layout_object))
        return false;
...
template <typename Strategy> unsigned DepthCrossingShadowBoundaries(const Node& node)
{
    unsigned depth = 0;
...
template <typename Strategy> FullyClippedStateStackAlgorithm<Strategy>::FullyClippedStateStackAlgorithm() = default;

template <typename Strategy> FullyClippedStateStackAlgorithm<Strategy>::~FullyClippedStateStackAlgorithm() = default;

template <typename Strategy> void FullyClippedStateStackAlgorithm<Strategy>::PushFullyClippedState(const Node* node)
{
...
template <typename Strategy> void FullyClippedStateStackAlgorithm<Strategy>::SetUpFullyClippedStack(const Node* node)
{
