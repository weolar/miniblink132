...
public:
    TextIteratorTextNodeHandler(const TextIteratorBehavior&, TextIteratorTextState*);
    TextIteratorTextNodeHandler(const TextIteratorTextNodeHandler&) = delete;
    ~TextIteratorTextNodeHandler()
    {
        mapping_units_.clear();
    }
    TextIteratorTextNodeHandler& operator=(const TextIteratorTextNodeHandler&) = delete;

    const Text* GetNode() const
    {
        return text_node_;
    }
    ...
    void HandleTextNodeInRange(const Text*, unsigned start_offset, unsigned end_offset);
