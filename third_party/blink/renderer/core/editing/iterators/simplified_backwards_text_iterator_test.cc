...
protected:
    std::string ExtractStringInRange(const std::string selection_text, const TextIteratorBehavior& behavior = TextIteratorBehavior())
    {
        const SelectionInDOMTree selection = SetSelectionTextToBody(selection_text);
        StringBuilder builder;
        bool is_first = true;
        for (SimplifiedBackwardsTextIterator iterator(selection.ComputeRange(), behavior); !iterator.AtEnd(); iterator.Advance()) {
            if (!is_first)
                builder.Append(", ", 2);
            is_first = false;
...
template <typename Strategy> static String ExtractString(const Element& element)
{
    const EphemeralRangeTemplate<Strategy> range = EphemeralRangeTemplate<Strategy>::RangeOfContents(element);
    String result;
    for (SimplifiedBackwardsTextIteratorAlgorithm<Strategy> it(range); !it.AtEnd(); it.Advance()) {
...
TEST_F(SimplifiedBackwardsTextIteratorTest, IterateWithFirstLetterPart)
{
    ...
    EXPECT_EQ("6,  [(3)], \n, ab", ExtractStringInRange("^ab<p> [(3)]6|78</p>")) << "From remaining part to outside";

    EXPECT_EQ("(3)", ExtractStringInRange("<p> [^(3)|]678</p>")) << "Iterate in first-letter part";

    EXPECT_EQ("67, (3)]", ExtractStringInRange("<p> [^(3)]67|8</p>")) << "From remaining part to first-letter part";

    EXPECT_EQ("789", ExtractStringInRange("<p> [(3)]6^789|a</p>")) << "Iterate in remaining part";

    EXPECT_EQ("9, \n, 78", ExtractStringInRange("<p> [(3)]6^78</p>9|a")) << "Enter into remaining part and stop in remaining part";

    EXPECT_EQ("9, \n, 678, (3)]", ExtractStringInRange("<p> [^(3)]678</p>9|a")) << "Enter into remaining part and stop in first-letter part";
}
...
TEST_F(SimplifiedBackwardsTextIteratorTest, Basic)
{
    ...
    SimplifiedBackwardsTextIterator iterator(EphemeralRange(Position(sample->firstChild(), 0), Position(sample->firstChild(), 9)));
    // TODO(editing-dev): |SimplifiedBackwardsTextIterator| should not account
    // collapsed whitespace (http://crbug.com/760428)
    EXPECT_EQ(9, iterator.length()) << "We should have 8 as ignoring collapsed whitespace.";
...
TEST_F(SimplifiedBackwardsTextIteratorTest, NbspCharacter)
{
    ...
    SimplifiedBackwardsTextIteratorInFlatTree iterator(
        EphemeralRangeInFlatTree(PositionInFlatTree(p->firstChild(), 0), PositionInFlatTree(p->firstChild(), 11)));
    ...
    TextIteratorBehavior behavior = TextIteratorBehavior::Builder().SetEmitsSpaceForNbsp(true).Build();
    SimplifiedBackwardsTextIteratorInFlatTree emits_space_iterator(
        EphemeralRangeInFlatTree(PositionInFlatTree(p->firstChild(), 0), PositionInFlatTree(p->firstChild(), 11)), behavior);
...
TEST_F(SimplifiedBackwardsTextIteratorTest, EmitsPunctuationForImage)
{
    ...
    SimplifiedBackwardsTextIteratorInFlatTree iterator(EphemeralRangeInFlatTree(PositionInFlatTree(img, 0), PositionInFlatTree(p->firstChild(), 1)));
    ...
    TextIteratorBehavior behavior = TextIteratorBehavior::Builder().SetEmitsPunctuationForReplacedElements(true).Build();
    SimplifiedBackwardsTextIteratorInFlatTree with_punctuation_iterator(
        EphemeralRangeInFlatTree(PositionInFlatTree(img, 0), PositionInFlatTree(p->firstChild(), 1)), behavior);
...
{
    SetBodyContent("<style>p::first-letter {font-size: 200%}</style>"
                   "<p> [(3)]678</p>");
    const Element* const sample = GetDocument().QuerySelector(AtomicString("p"));
    SimplifiedBackwardsTextIterator iterator(EphemeralRange(Position(sample->firstChild(), 0), Position(sample->firstChild(), 9)));
    ...
    // TODO(editing-dev): |SimplifiedBackwardsTextIterator| should not account
    // collapsed whitespace (http://crbug.com/760428)
    EXPECT_EQ(6, iterator.length()) << "We should have 5 as ignoring collapsed whitespace.";
    EXPECT_EQ(Position(sample->firstChild(), 0), iterator.StartPosition());
...
{
    static const char* body_content = "<span id=host><b slot='#one' id=one>one</b> not appeared <b slot='#two' "
                                      "id=two>two</b></span>";
...
TEST_F(SimplifiedBackwardsTextIteratorTest, characterAt)
{
    ...
    SimplifiedBackwardsTextIteratorAlgorithm<EditingInFlatTreeStrategy> back_iter2(range2);
...
TEST_F(SimplifiedBackwardsTextIteratorTest, TextSecurity)
{
    ...
    EXPECT_EQ("baz, xxx, abc", ExtractStringInRange("^abc<s>foo</s>baz|", EmitsSmallXForTextSecurityBehavior()));
    // E2 80 A2 is U+2022 BULLET
    EXPECT_EQ("baz, \xE2\x80\xA2\xE2\x80\xA2\xE2\x80\xA2, abc", ExtractStringInRange("^abc<s>foo</s>baz|", TextIteratorBehavior()));
}
