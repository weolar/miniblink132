...
bool ShouldSkipInvisibleTextAt(const Text& text, unsigned offset, bool ignores_visibility)
{
...
String TextIgnoringCSSTextTransforms(const LayoutText& layout_text, const OffsetMappingUnit& unit)
{
    ...
    // associated DOM node.
    String text = layout_text.IsTextFragment() ? To<LayoutTextFragment>(layout_text).CompleteText() : layout_text.OriginalText();
...
StringAndOffsetRange ComputeTextAndOffsetsForEmission(const OffsetMapping& mapping, const OffsetMappingUnit& unit, const TextIteratorBehavior& behavior)
{
    StringAndOffsetRange result { mapping.GetText(), unit.TextContentStart(), unit.TextContentEnd() };
    ...
    if (behavior.IgnoresCSSTextTransforms() && layout_text.HasTextTransform() && !layout_text.IsSecure()) {
    ...
    if (behavior.EmitsOriginalText()) {
        result.string = layout_text.OriginalText().Substring(unit.DOMStart(), unit.DOMEnd() - unit.DOMStart());
        result.start = 0;
        result.end = result.string.length();
    }

    if (behavior.EmitsSpaceForNbsp()) {
        result.string = result.string.Substring(result.start, result.end - result.start);
...
TextIteratorTextNodeHandler::TextIteratorTextNodeHandler(const TextIteratorBehavior& behavior, TextIteratorTextState* text_state)
    : behavior_(behavior)
...
void TextIteratorTextNodeHandler::HandleTextNodeWithLayoutNG()
{
    ...
    while (offset_ < end_offset_ && !text_state_.PositionNode()) {
        const EphemeralRange range_to_emit(Position(text_node_, offset_), Position(text_node_, end_offset_));
        ...
        for (; mapping_units_index_ < mapping_units_.size(); ++mapping_units_index_) {
            const auto& unit = mapping_units_[mapping_units_index_];
            if (unit.TextContentEnd() == unit.TextContentStart() || ShouldSkipInvisibleTextAt(*text_node_, unit.DOMStart(), IgnoresStyleVisibility())) {
                offset_ = unit.DOMEnd();
                continue;
            }

            auto string_and_offsets = ComputeTextAndOffsetsForEmission(*mapping, unit, behavior_);
            ...
            text_state_.EmitText(*text_node_, unit.DOMStart(), unit.DOMEnd(), string, text_content_start, text_content_end);
            offset_ = unit.DOMEnd();
...
void TextIteratorTextNodeHandler::HandleTextNodeInRange(const Text* node, unsigned start_offset, unsigned end_offset)
{
    DCHECK(node);
    ...
    const OffsetMapping* const mapping = OffsetMapping::ForceGetFor(Position(node, offset_));
    if (!mapping) [[unlikely]] {
        DUMP_WILL_BE_NOTREACHED() << "We have LayoutText outside LayoutBlockFlow " << text_node_;
        return;
    }

    mapping_units_ = mapping->GetMappingUnitsForDOMRange(EphemeralRange(Position(node, offset_), Position(node, end_offset_)));
    mapping_units_index_ = 0;
    HandleTextNodeWithLayoutNG();
}

void TextIteratorTextNodeHandler::HandleTextNodeStartFrom(const Text* node, unsigned start_offset)
{
    HandleTextNodeInRange(node, start_offset, node->data().length());
}

void TextIteratorTextNodeHandler::HandleTextNodeEndAt(const Text* node, unsigned end_offset)
{
