...
{
    SetBodyContent("<style>div::first-letter { color: red }</style><div "
                   "id=sample>1ab\nde</div>");
...
{
    SetBodyContent("<style>pre::first-letter { color: red }</style><pre "
                   "id=sample>1ab\nde</pre>");
...
{
    const char* body_content = "<b id=zero>0</b><span id=host><b slot='#one' id=one>1</b><b slot='#two' "
                               "id=two>22</b></span><b id=three>333</b>";
    const char* shadow_content = "<p><slot name=#two></slot></p><p><slot name=#one></slot></p>";
    SetBodyContent(body_content);
    SetShadowContent(shadow_content, "host");
...
{
    SetBodyContent("<input style='display: table-cell' type='file' "
                   "maxlength='100'><select>");

    const Position& input = Position::BeforeNode(*GetDocument().QuerySelector(AtomicString("input")));
    const Position& select = Position::BeforeNode(*GetDocument().QuerySelector(AtomicString("select")));

    const VisiblePosition& input_position = CreateVisiblePosition(input);
    const VisiblePosition& after_input = VisiblePosition::AfterNode(*input.AnchorNode());
...
TEST_F(VisibleUnitsParagraphTest, endOfParagraphWithDifferentUpAndDownVisiblePositions)
{
    InsertStyleElement("span, div { display: inline-block; width: 50vw; }");
    SetBodyContent("x<span></span><div></div>");

    const Position& text_end = Position::LastPositionInNode(*GetDocument().body()->firstChild());
    const Position& before_div = Position::BeforeNode(*GetDocument().QuerySelector(AtomicString("div")));
    const VisiblePosition& upstream = CreateVisiblePosition(before_div, TextAffinity::kUpstream);
    const VisiblePosition& downstream = CreateVisiblePosition(before_div, TextAffinity::kDownstream);
...
{
    SetBodyContent("<span contenteditable>x<br contenteditable=false>"
...
{
    SetBodyContent("<div><span contenteditable id=span>this </span>"
    ...
    const VisiblePosition& end1 = EndOfParagraph(vp1);
    const VisiblePosition& end2 = EndOfParagraph(vp1, EditingBoundaryCrossingRule::kCanSkipOverEditingBoundary);
...
{
    SetBodyContent("<span contenteditable><br contenteditable=false>"
