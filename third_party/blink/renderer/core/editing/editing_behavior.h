...
public:
    explicit EditingBehavior(mojom::blink::EditingBehavior type)
        : type_(type)
    {
    }
    ...
    {
        return type_ != mojom::blink::EditingBehavior::kEditingWindowsBehavior && type_ != mojom::blink::EditingBehavior::kEditingAndroidBehavior
    ...
    {
        return type_ == mojom::blink::EditingBehavior::kEditingMacBehavior || type_ == mojom::blink::EditingBehavior::kEditingChromeOSBehavior;
    }
    ...
    {
        return type_ != mojom::blink::EditingBehavior::kEditingWindowsBehavior && type_ != mojom::blink::EditingBehavior::kEditingMacBehavior;
    }

    // Convert a KeyboardEvent to a command name like "Copy", "Undo" and so on.
    // If nothing, return empty string.
    const char* InterpretKeyEvent(const KeyboardEvent&, WritingMode writing_mode) const;
