...
protected:
    template <typename Traversal = NodeTraversal> std::string TraverseRange(Range*) const;

    template <typename Strategy> std::string TraverseRange(const EphemeralRangeTemplate<Strategy>&) const;

    Range* GetBodyRange() const;
};

template <typename Traversal> std::string EphemeralRangeTest::TraverseRange(Range* range) const
{
...
template <typename Strategy> std::string EphemeralRangeTest::TraverseRange(const EphemeralRangeTemplate<Strategy>& range) const
{
    std::stringstream nodes_content;
...
{
    const char* body_content = "<p id='host'>"
                               "<b id='zero'>0</b>"
                               "<b id='one'>1</b>"
                               "<b id='two'>22</b>"
    ...
    const std::string expected_nodes("[BODY][P id=\"host\"][B id=\"zero\"][#text \"0\"][B id=\"one\"][#text "
    ...
    EXPECT_EQ(TraverseRange<FlatTreeTraversal>(GetBodyRange()), TraverseRange(EphemeralRangeInFlatTree(GetBodyRange())));
}

// Tests that |inRange| helper will traverse the whole range with shadow DOM.
TEST_F(EphemeralRangeTest, rangeShadowTraversal)
{
    const char* body_content = "<b id='zero'>0</b>"
                               "<p id='host'>"
                               ...
                               "</p>"
                               "<b id='four'>4444</b>";
    const char* shadow_content = "<p id='five'>55555</p>"
                                 "<slot name=#two></slot>"
                                 "<slot name=#one></slot>"
    ...
    const std::string expected_nodes("[BODY][B id=\"zero\"][#text \"0\"][P id=\"host\"][P id=\"five\"][#text "
...
TEST_F(EphemeralRangeTest, rangeTraversalLimitedDOM)
{
    ...
    until_b->setEnd(GetDocument().getElementById(AtomicString("one")), 0, IGNORE_EXCEPTION_FOR_TESTING);
    EXPECT_EQ("[BODY][P id=\"host\"][B id=\"zero\"][#text \"0\"][B id=\"one\"]", TraverseRange<>(until_b));
    EXPECT_EQ(TraverseRange<>(until_b), TraverseRange(EphemeralRange(until_b)));

    Range* from_b_to_span = GetBodyRange();
    from_b_to_span->setStart(GetDocument().getElementById(AtomicString("one")), 0, IGNORE_EXCEPTION_FOR_TESTING);
    from_b_to_span->setEnd(GetDocument().getElementById(AtomicString("three")), 0, IGNORE_EXCEPTION_FOR_TESTING);
    EXPECT_EQ("[#text \"1\"][B id=\"two\"][#text \"22\"][SPAN id=\"three\"]", TraverseRange<>(from_b_to_span));
    EXPECT_EQ(TraverseRange<>(from_b_to_span), TraverseRange(EphemeralRange(from_b_to_span)));
}
...
TEST_F(EphemeralRangeTest, rangeTraversalLimitedFlatTree)
{
    const char* body_content = "<b id='zero'>0</b>"
                               ...
                               "</p>"
                               "<b id='three'>333</b>";
    const char* shadow_content = "<p id='four'>4444</p>"
                                 "<slot name=#two></slot>"
                                 "<slot name=#one></slot>"
    ...
    const PositionInFlatTree start_position(GetDocument().getElementById(AtomicString("one")), 0);
    const PositionInFlatTree limit_position(shadow_root->getElementById(AtomicString("five")), 0);
    const PositionInFlatTree end_position(shadow_root->getElementById(AtomicString("six")), 0);
...
TEST_F(EphemeralRangeTest, traversalEmptyRanges)
{
    ...
    EXPECT_EQ(single_position_range.StartPosition().NodeAsRangeFirstNode(), single_position_range.EndPosition().NodeAsRangePastLastNode());
}

TEST_F(EphemeralRangeTest, commonAncesstorDOM)
{
    const char* body_content = "<p id='host'>00"
                               "<b id='one'>11</b>"
                               "<b id='two'>22</b>"
                               "<b id='three'>33</b>"
                               "</p>";
    ...
    const Position start_position(GetDocument().getElementById(AtomicString("one")), 0);
    const Position end_position(GetDocument().getElementById(AtomicString("two")), 0);
    const EphemeralRange range(start_position, end_position);
    EXPECT_EQ(GetDocument().getElementById(AtomicString("host")), range.CommonAncestorContainer());
}
...
TEST_F(EphemeralRangeTest, commonAncesstorFlatTree)
{
    ...
    const PositionInFlatTree start_position(GetDocument().getElementById(AtomicString("one")), 0);
    const PositionInFlatTree end_position(shadow_root->getElementById(AtomicString("five")), 0);
...
{
    SetBodyContent("<div id='first'></div>"
                   "<div id='last'></div>");
