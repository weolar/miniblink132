...
public:
    ...
    SelectionPaintRange(
        const Node& passed_start_node, std::optional<unsigned> passed_start_offset, const Node& passed_end_node, std::optional<unsigned> passed_end_offset)
    ...
    void AssertSanity() const
    {
        ...
        if (start_node) {
            DCHECK(end_node);
            DCHECK(start_node->GetLayoutObject()->GetSelectionState() == SelectionState::kStart
                || start_node->GetLayoutObject()->GetSelectionState() == SelectionState::kStartAndEnd);
            DCHECK(end_node->GetLayoutObject()->GetSelectionState() == SelectionState::kEnd
                || end_node->GetLayoutObject()->GetSelectionState() == SelectionState::kStartAndEnd);
            return;
        }
        DCHECK(!end_node);
...
static SelectionMode ComputeSelectionMode(const FrameSelection& frame_selection)
{
    const SelectionInDOMTree& selection_in_dom = frame_selection.GetSelectionInDOMTree();
...
static EphemeralRangeInFlatTree CalcSelectionInFlatTree(const FrameSelection& frame_selection)
{
    const SelectionInDOMTree& selection_in_dom = frame_selection.GetSelectionInDOMTree();
    ...
    case SelectionMode::kRange: {
        const PositionInFlatTree& anchor = ToPositionInFlatTree(selection_in_dom.Anchor());
        const PositionInFlatTree& focus = ToPositionInFlatTree(selection_in_dom.Focus());
        if (anchor.IsNull() || focus.IsNull() || anchor == focus || !anchor.IsValidFor(frame_selection.GetDocument())
            || !focus.IsValidFor(frame_selection.GetDocument())) {
            return {};
        }
        return anchor <= focus ? EphemeralRangeInFlatTree(anchor, focus) : EphemeralRangeInFlatTree(focus, anchor);
    }
    }
    NOTREACHED();
}
...
public:
    ...
    NewPaintRangeAndSelectedNodes(SelectionPaintRange* passed_paint_range, HeapHashSet<Member<const Node>>&& passed_selected_objects)
    ...
    NewPaintRangeAndSelectedNodes& operator=(const NewPaintRangeAndSelectedNodes&) = delete;
...
static void SetShouldInvalidateIfNeeded(LayoutObject* layout_object)
{
    ...
    if (!layout_object->IsSVG())
        return;
    for (LayoutObject* parent = layout_object->Parent(); parent; parent = parent->Parent()) {
        if (parent->IsSVGRoot())
            return;
        if (parent->IsSVGText()) {
...
static void SetShouldInvalidateSelection(const NewPaintRangeAndSelectedNodes& new_range, const OldSelectedNodes& old_selected_objects)
{
    ...
    for (const Node* node : new_range.selected_objects) {
        if (old_selected_objects.selected_map.Contains(node))
            continue;
        const SelectionState new_state = node->GetLayoutObject()->GetSelectionState();
    ...
    for (const auto& key_value : old_selected_objects.selected_map) {
        ...
        if (new_state == old_state)
            continue;
        DCHECK(new_state != SelectionState::kNone || old_state != SelectionState::kNone) << node;
        DCHECK_NE(new_state, SelectionState::kContain) << node;
    ...
    if (new_paint_range.IsNull())
        return;
    if (new_paint_range.start_node->IsTextNode() && new_paint_range.start_node == old_paint_range.start_node
        && new_paint_range.start_offset != old_paint_range.start_offset)
        SetShouldInvalidateIfNeeded(*new_paint_range.start_node);
    if (new_paint_range.end_node->IsTextNode() && new_paint_range.end_node == old_paint_range.end_node
...
template <typename Visitor> static void VisitSelectedInclusiveDescendantsOfInternal(Node& node, Visitor* visitor)
{
...
template <typename Visitor> static void VisitSelectedInclusiveDescendantsOf(Node& node, Visitor* visitor)
{
    DCHECK(IsFlatTreeClean(node));
    return VisitSelectedInclusiveDescendantsOfInternal(node, visitor);
}

static OldSelectedNodes ResetOldSelectedNodes(Node& root, std::optional<unsigned> old_start_offset, std::optional<unsigned> old_end_offset)
{
    class OldSelectedVisitor {
        STACK_ALLOCATED();

    public:
        OldSelectedVisitor(std::optional<unsigned> passed_old_start_offset, std::optional<unsigned> passed_old_end_offset)
...
static std::optional<unsigned> ComputeStartOffset(const Node& node, const PositionInFlatTree& selection_start)
{
    if (!node.IsTextNode())
        return std::nullopt;
...
static std::optional<unsigned> ComputeEndOffset(const Node& node, const PositionInFlatTree& selection_end)
{
...
static bool IsPositionValidText(const Position& position)
{
    if (position.AnchorNode()->IsTextNode() && position.IsOffsetInAnchor())
        return true;
    if ((IsA<HTMLBRElement>(position.AnchorNode()) || IsA<HTMLWBRElement>(position.AnchorNode())) && (position.IsBeforeAnchor() || position.IsAfterAnchor()))
        return true;
    return false;
}
#endif
...
static std::optional<unsigned> GetTextContentOffset(const Position& position)
{
    ...
    if (offset_mapping == nullptr)
        return std::nullopt;
    const std::optional<unsigned>& ng_offset = offset_mapping->GetTextContentOffset(position);
    return ng_offset;
}

// Computes text content offset of selection start if |layout_object| is
// LayoutText.
static std::optional<unsigned> GetTextContentOffsetStart(const Node& node, std::optional<unsigned> node_offset)
{
...
// LayoutText.
static std::optional<unsigned> GetTextContentOffsetEnd(const Node& node, std::optional<unsigned> node_offset)
{
    if (!node.GetLayoutObject()->IsText())
        return {};
    if (node.IsTextNode()) {
...
static SelectionPaintRange* ComputeNewPaintRange(const SelectionPaintRange& paint_range)
{
    ...
    const std::optional<unsigned> start_offset
        = ShouldUseLayoutNGTextContent(start_node) ? GetTextContentOffsetStart(start_node, paint_range.start_offset) : paint_range.start_offset;

    const Node& end_node = *paint_range.end_node;
    const std::optional<unsigned> end_offset
        = ShouldUseLayoutNGTextContent(end_node) ? GetTextContentOffsetEnd(end_node, paint_range.end_offset) : paint_range.end_offset;

    return MakeGarbageCollected<SelectionPaintRange>(*paint_range.start_node, start_offset, *paint_range.end_node, end_offset);
}

static unsigned ClampOffset(unsigned offset, unsigned start_offset, unsigned end_offset)
{
...
static SelectionState GetSelectionStateFor(const InlineCursorPosition& position)
{
    DCHECK(position.GetLayoutObject());
    return GetSelectionStateFor(To<LayoutText>(*position.GetLayoutObject()));
}

static SelectionState GetPaintingSelectionStateFor(const LayoutText& layout_text)
{
    if (const auto* text_fragment = DynamicTo<LayoutTextFragment>(layout_text)) {
        Node* node = text_fragment->AssociatedTextNode();
        if (!node)
...
static inline unsigned ClampOffset(unsigned node_offset, const LayoutTextFragment& fragment)
{
...
static LayoutTextSelectionStatus ComputeSelectionStatusForNode(
    const Text& text, SelectionState selection_state, std::optional<unsigned> start_offset, std::optional<unsigned> end_offset)
{
    switch (selection_state) {
    case SelectionState::kInside:
        return { 0, text.length(), SelectionIncludeEnd::kInclude };
    case SelectionState::kStart:
        return { start_offset.value(), text.length(), SelectionIncludeEnd::kInclude };
    case SelectionState::kEnd:
        return { 0, end_offset.value(), SelectionIncludeEnd::kNotInclude };
    case SelectionState::kStartAndEnd:
        return { start_offset.value(), end_offset.value(), SelectionIncludeEnd::kNotInclude };
    default:
        NOTREACHED();
    }
}

LayoutTextSelectionStatus LayoutSelection::ComputeSelectionStatus(const LayoutText& layout_text) const
{
    AssertIsValid();
    ...
    if (Text* text = AssociatedTextNode(layout_text)) {
        const LayoutTextSelectionStatus text_status
            = ComputeSelectionStatusForNode(*text, selection_state, paint_range_->start_offset, paint_range_->end_offset);
        if (const auto* text_fragment = DynamicTo<LayoutTextFragment>(layout_text)) {
            return { ClampOffset(text_status.start, *text_fragment), ClampOffset(text_status.end, *text_fragment), text_status.include_end };
        }
        return text_status;
    }
    ...
    if (layout_text.IsBR() && selection_state == SelectionState::kEnd)
        return { 0, 0, SelectionIncludeEnd::kNotInclude };
    return { 0, layout_text.TransformedTextLength(), SelectionIncludeEnd::kInclude };
}

LayoutTextSelectionStatus FrameSelection::ComputeLayoutSelectionStatus(const LayoutText& text) const
{
...
LayoutSelectionStatus LayoutSelection::ComputeSelectionStatus(const InlineCursor& cursor) const
{
    ...
    DCHECK_GT(offset, 0u);
    LayoutSelectionStatus status = ComputeSelectionStatus(cursor, { offset - 1, offset });
...
LayoutSelectionStatus LayoutSelection::ComputeSelectionStatus(const InlineCursor& cursor, const TextOffsetRange& offset) const
{
    ...
    case SelectionState::kStart: {
        ...
        return { ClampOffset(start_in_block, start_offset, end_offset), end_offset,
            (is_continuous && cursor.IsBeforeSoftLineBreak()) ? SelectSoftLineBreak::kSelected : SelectSoftLineBreak::kNotSelected };
    }
    case SelectionState::kEnd: {
        const unsigned end_in_block = paint_range_->end_offset.value();
        const unsigned end_in_fragment = ClampOffset(end_in_block, start_offset, end_offset);
    ...
    case SelectionState::kStartAndEnd: {
        ...
        const unsigned end_in_block = paint_range_->end_offset.value();
        const unsigned end_in_fragment = ClampOffset(end_in_block, start_offset, end_offset);
        const bool is_continuous = start_in_block <= end_offset && end_offset < end_in_block;
        return { ClampOffset(start_in_block, start_offset, end_offset), end_in_fragment,
            (is_continuous && cursor.IsBeforeSoftLineBreak()) ? SelectSoftLineBreak::kSelected : SelectSoftLineBreak::kNotSelected };
    }
    case SelectionState::kInside: {
        return { start_offset, end_offset, cursor.IsBeforeSoftLineBreak() ? SelectSoftLineBreak::kSelected : SelectSoftLineBreak::kNotSelected };
    }
    default:
...
SelectionState LayoutSelection::ComputeSelectionStateFromOffsets(SelectionState state, unsigned start_offset, unsigned end_offset) const
{
    switch (state) {
    case SelectionState::kStart: {
        const unsigned start_in_block = paint_range_->start_offset.value_or(start_offset);
        return start_offset <= start_in_block && start_in_block <= end_offset ? SelectionState::kStart : SelectionState::kNone;
    }
    case SelectionState::kEnd: {
        const unsigned end_in_block = paint_range_->end_offset.value_or(end_offset);
        return start_offset <= end_in_block && end_in_block <= end_offset ? SelectionState::kEnd : SelectionState::kNone;
    }
    case SelectionState::kStartAndEnd: {
        const unsigned start_in_block = paint_range_->start_offset.value_or(start_offset);
        const unsigned end_in_block = paint_range_->end_offset.value_or(end_offset);
        const bool is_start_in_current_cursor = start_offset <= start_in_block && start_in_block <= end_offset;
        const bool is_end_in_current_cursor = start_offset <= end_in_block && end_in_block <= end_offset;
...
SelectionState LayoutSelection::ComputePaintingSelectionStateForCursor(const InlineCursorPosition& position) const
{
    if (!position)
        return SelectionState::kNone;
    ...
    SelectionState state = GetPaintingSelectionStateFor(To<LayoutText>(*position.GetLayoutObject()));
    return ComputeSelectionStateFromOffsets(state, start_offset, end_offset);
}

static void SetSelectionStateForPaint(const EphemeralRangeInFlatTree& selection)
{
    ...
    for (Node& node : selection.Nodes()) {
        ...
        if (end_node != start_node) {
            end_node->GetLayoutObject()->SetSelectionStateForPaint(SelectionState::kInside);
        }
        end_node = &node;
    }
    ...
    if (start_node == end_node) {
        start_node->GetLayoutObject()->SetSelectionStateForPaint(SelectionState::kStartAndEnd);
    } else {
        start_node->GetLayoutObject()->SetSelectionStateForPaint(SelectionState::kStart);
        end_node->GetLayoutObject()->SetSelectionStateForPaint(SelectionState::kEnd);
    }
}

static NewPaintRangeAndSelectedNodes CalcSelectionRangeAndSetSelectionState(const FrameSelection& frame_selection)
{
    const SelectionInDOMTree& selection_in_dom = frame_selection.GetSelectionInDOMTree();
    if (selection_in_dom.IsNone())
        return {};

    const EphemeralRangeInFlatTree& selection = CalcSelectionInFlatTree(frame_selection);
    ...
    const std::optional<unsigned> start_offset = ComputeStartOffset(*start_node, selection.StartPosition().ToOffsetInAnchor());
    const std::optional<unsigned> end_offset = ComputeEndOffset(*end_node, selection.EndPosition().ToOffsetInAnchor());
    if (start_node == end_node) {
    ...
    SelectionPaintRange* new_range = MakeGarbageCollected<SelectionPaintRange>(*start_node, start_offset, *end_node, end_offset);
...
void LayoutSelection::Commit()
{
    ...
    DCHECK_GE(frame_selection_->GetDocument().Lifecycle().GetState(), DocumentLifecycle::kLayoutClean);
    DocumentLifecycle::DisallowTransitionScope disallow_transition(frame_selection_->GetDocument().Lifecycle());

    const OldSelectedNodes& old_selected_objects = ResetOldSelectedNodes(frame_selection_->GetDocument(), paint_range_->start_offset, paint_range_->end_offset);
    const NewPaintRangeAndSelectedNodes& new_range = CalcSelectionRangeAndSetSelectionState(*frame_selection_);
...
template <typename Visitor> static void VisitLayoutObjectsOf(const Node& node, Visitor* visitor)
{
    LayoutObject* layout_object = node.GetLayoutObject();
    if (!layout_object)
        return;
...
gfx::Rect LayoutSelection::AbsoluteSelectionBounds()
{
    ...
    VisitSelectedInclusiveDescendantsOf(frame_selection_->GetDocument(), &visitor);
    ...
    public:
        void Visit(Node& node)
        {
            ...
            if (auto* this_element = DynamicTo<Element>(node)) {
                ...
                if (element_style && element_style->HasPseudoElementStyle(kPseudoIdSelection)) {
                    node.SetNeedsStyleRecalc(kLocalStyleChange,
                        StyleChangeReasonForTracing::CreateWithExtraData(style_change_reason::kPseudoClass, style_change_extra_data::g_active));
...
#if DCHECK_IS_ON()
std::ostream& operator<<(std::ostream& ostream, const std::optional<unsigned>& offset)
{
    if (offset.has_value())
...
std::ostream& operator<<(std::ostream& ostream, const SelectionPaintRange& range)
{
    ostream << range.start_node << ": " << range.start_offset << ", " << range.end_node << ": " << range.end_offset;
    return ostream;
}

std::ostream& operator<<(std::ostream& ostream, const HeapHashMap<Member<const Node>, SelectionState>& map)
{
    ostream << "[";
    const char* comma = "";
...
std::ostream& operator<<(std::ostream& ostream, const OldSelectedNodes& old_node)
{
...
std::ostream& operator<<(std::ostream& ostream, const HeapHashSet<Member<const Node>>& selected_objects)
{
    ostream << "[";
    const char* comma = "";
...
std::ostream& operator<<(std::ostream& ostream, const NewPaintRangeAndSelectedNodes& new_range)
{
...
void PrintSelectionStateInDocument(const FrameSelection& selection)
{
    ...
    public:
        void Visit(const Node& node)
        {
            PrintSelectionStatus(stream, node);
        }
        std::stringstream stream;
    } visitor;
