...
// static
TextSegments::Finder::Position TextSegments::Finder::Position::Before(unsigned offset)
{
    return Position(offset, kBefore);
}

// static
TextSegments::Finder::Position TextSegments::Finder::Position::After(unsigned offset)
{
...
// static
PositionInFlatTree TextSegments::FindBoundaryForward(const PositionInFlatTree& position, Finder* finder)
{
    ...
    for (auto inline_contents : TextOffsetMapping::ForwardRangeOf(position)) {
        ...
        const unsigned offset = is_start_contents ? mapping.ComputeTextOffset(position) : 0;
...
// static
PositionInFlatTree TextSegments::FindBoundaryBackward(const PositionInFlatTree& position, Finder* finder)
{
    DCHECK(position.IsNotNull());
    PositionInFlatTree last_position = position;
    bool is_start_contents = true;
    for (auto inline_contents : TextOffsetMapping::BackwardRangeOf(position)) {
        const TextOffsetMapping mapping(inline_contents);
        const String text = mapping.GetText();
        const unsigned offset = is_start_contents ? mapping.ComputeTextOffset(position) : mapping.GetText().length();
        is_start_contents = false;
        const TextSegments::Finder::Position result = finder->Find(text, offset);
        if (result.IsBefore())
            return mapping.GetPositionBefore(result.Offset());
