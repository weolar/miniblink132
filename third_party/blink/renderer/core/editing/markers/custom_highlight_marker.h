...
public:
    CustomHighlightMarker(unsigned start_offset, unsigned end_offset, const String& highlight_name, const Member<Highlight> highlight);
    ...
    {
        return highlight_.Get();
    }
    const AtomicString& GetHighlightName() const
    {
        return highlight_name_;
    }
...
template <> struct DowncastTraits<CustomHighlightMarker> {
