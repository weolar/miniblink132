...
void InvalidateVisualOverflowForNode(const Node& node, DocumentMarker::MarkerType type)
{
    LayoutObject* layout_object = node.GetLayoutObject();
    if (!layout_object || !DocumentMarker::MarkerTypes::HighlightPseudos().Intersects(DocumentMarker::MarkerTypes(type))) {
        return;
    }
    if (HighlightStyleUtils::ShouldInvalidateVisualOverflow(*layout_object, type)) {
...
void InvalidatePaintForNode(const Node& node)
{
    ...
    layout_object->SetShouldDoFullPaintInvalidation(PaintInvalidationReason::kDocumentMarker);
    ...
    if (RuntimeEnabledFeatures::PaintHighlightsForFirstLetterEnabled()) {
        ...
        if (text_layout && text_layout->GetFirstLetterPseudoElement()) {
            ...
            first_letter_layout->SetShouldDoFullPaintInvalidation(PaintInvalidationReason::kDocumentMarker);
        }
    }
...
PositionInFlatTree SearchAroundPositionStart(const PositionInFlatTree& position)
{
    const PositionInFlatTree start_of_word_or_null = StartOfWordPosition(position, kPreviousWordIfOnBoundary);
...
{
    const PositionInFlatTree end_of_word_or_null = EndOfWordPosition(position, kNextWordIfOnBoundary);
    return end_of_word_or_null.IsNotNull() ? end_of_word_or_null : position;
}

} // namespace

bool DocumentMarkerController::PossiblyHasMarkers(DocumentMarker::MarkerType type) const
{
    return PossiblyHasMarkers(DocumentMarker::MarkerTypes(type));
}

inline bool DocumentMarkerController::PossiblyHasMarkers(DocumentMarker::MarkerTypes types) const
{
    DCHECK(!markers_.empty() || possibly_existing_marker_types_ == DocumentMarker::MarkerTypes(0));
...
void DocumentMarkerController::AddSpellingMarker(const EphemeralRange& range, const String& description)
{
    AddMarkerInternal(
        range, [&description](int start_offset, int end_offset) { return MakeGarbageCollected<SpellingMarker>(start_offset, end_offset, description); });
}

void DocumentMarkerController::AddGrammarMarker(const EphemeralRange& range, const String& description)
{
    AddMarkerInternal(
        range, [&description](int start_offset, int end_offset) { return MakeGarbageCollected<GrammarMarker>(start_offset, end_offset, description); });
}

void DocumentMarkerController::AddTextMatchMarker(const EphemeralRange& range, TextMatchMarker::MatchStatus match_status)
{
    DCHECK(!document_->NeedsLayoutTreeUpdate());
    AddMarkerInternal(
        range, [match_status](int start_offset, int end_offset) { return MakeGarbageCollected<TextMatchMarker>(start_offset, end_offset, match_status); },
...
void DocumentMarkerController::AddCompositionMarker(const EphemeralRange& range, Color underline_color, ui::mojom::ImeTextSpanThickness thickness,
    ui::mojom::ImeTextSpanUnderlineStyle underline_style, Color text_color, Color background_color)
{
    DCHECK(!document_->NeedsLayoutTreeUpdate());
    AddMarkerInternal(range, [underline_color, thickness, underline_style, text_color, background_color](int start_offset, int end_offset) {
        return MakeGarbageCollected<CompositionMarker>(start_offset, end_offset, underline_color, thickness, underline_style, text_color, background_color);
    });
}

void DocumentMarkerController::AddActiveSuggestionMarker(const EphemeralRange& range, Color underline_color, ui::mojom::ImeTextSpanThickness thickness,
    ui::mojom::ImeTextSpanUnderlineStyle underline_style, Color text_color, Color background_color)
{
    DCHECK(!document_->NeedsLayoutTreeUpdate());
    AddMarkerInternal(range, [underline_color, thickness, underline_style, text_color, background_color](int start_offset, int end_offset) {
        return MakeGarbageCollected<ActiveSuggestionMarker>(
            start_offset, end_offset, underline_color, thickness, underline_style, text_color, background_color);
    });
}

void DocumentMarkerController::AddSuggestionMarker(const EphemeralRange& range, const SuggestionMarkerProperties& properties)
{
    DCHECK(!document_->NeedsLayoutTreeUpdate());
    AddMarkerInternal(
        range, [&properties](int start_offset, int end_offset) { return MakeGarbageCollected<SuggestionMarker>(start_offset, end_offset, properties); });
}

void DocumentMarkerController::AddTextFragmentMarker(const EphemeralRange& range)
{
    DCHECK(!document_->NeedsLayoutTreeUpdate());
    AddMarkerInternal(range, [](int start_offset, int end_offset) { return MakeGarbageCollected<TextFragmentMarker>(start_offset, end_offset); });
}

void DocumentMarkerController::AddCustomHighlightMarker(const EphemeralRange& range, const String& highlight_name, const Member<Highlight> highlight)
{
    DCHECK(!document_->NeedsLayoutTreeUpdate());
    AddMarkerInternal(range, [highlight_name, highlight](int start_offset, int end_offset) {
        return MakeGarbageCollected<CustomHighlightMarker>(start_offset, end_offset, highlight_name, highlight);
    });
}
...
void DocumentMarkerController::RemoveMarkers(TextIterator& marked_text, DocumentMarker::MarkerTypes marker_types)
{
    for (; !marked_text.AtEnd(); marked_text.Advance()) {
        ...
        for (DocumentMarker::MarkerType type : marker_types) {
            RemoveMarkersInternal(*text_node, start_offset, end_offset - start_offset, type);
        }
    }
}

void DocumentMarkerController::RemoveMarkersInRange(const EphemeralRange& range, DocumentMarker::MarkerTypes marker_types)
{
    DCHECK(!document_->NeedsLayoutTreeUpdate());
...
void DocumentMarkerController::AddMarkerInternal(
    const EphemeralRange& range, base::FunctionRef<DocumentMarker*(int, int)> create_marker_from_offsets, const TextIteratorBehavior& iterator_behavior)
{
    DocumentMarkerGroup* new_marker_group = MakeGarbageCollected<DocumentMarkerGroup>();
    for (TextIterator marked_text(range.StartPosition(), range.EndPosition(), iterator_behavior); !marked_text.AtEnd(); marked_text.Advance()) {
        const int start_offset_in_current_container = marked_text.StartOffsetInCurrentContainer();
        const int end_offset_in_current_container = marked_text.EndOffsetInCurrentContainer();

        DCHECK_GE(end_offset_in_current_container, start_offset_in_current_container);
        ...
        DocumentMarker* const new_marker = create_marker_from_offsets(start_offset_in_current_container, end_offset_in_current_container);
...
void DocumentMarkerController::AddMarkerToNode(const Text& text, DocumentMarker* new_marker)
{
    DCHECK_GE(text.length(), new_marker->EndOffset());
    possibly_existing_marker_types_ = possibly_existing_marker_types_.Add(DocumentMarker::MarkerTypes(new_marker->GetType()));
    SetDocument(document_);

    DocumentMarker::MarkerType new_marker_type = new_marker->GetType();
    const DocumentMarker::MarkerTypeIndex type_index = MarkerTypeToMarkerIndex(new_marker_type);
...
void DocumentMarkerController::MoveMarkers(const Text& src_node, int length, const Text& dst_node)
{
    if (length <= 0) {
        return;
    }
    ...
    for (auto& marker_map : markers_) {
        ...
        auto& dst_marker_entry = marker_map->insert(&dst_node, nullptr).stored_value->value;
        if (!dst_marker_entry) {
...
void DocumentMarkerController::DidRemoveNodeFromMap(DocumentMarker::MarkerType type, bool clear_document_allowed)
{
    ...
    if (markers_[type_index]->empty()) {
        ...
        possibly_existing_marker_types_ = possibly_existing_marker_types_.Subtract(DocumentMarker::MarkerTypes(type));
    }
    if (clear_document_allowed && possibly_existing_marker_types_ == DocumentMarker::MarkerTypes()) {
        SetDocument(nullptr);
    }
}

void DocumentMarkerController::RemoveMarkersInternal(const Text& text, unsigned start_offset, int length, DocumentMarker::MarkerType marker_type)
{
    if (length <= 0) {
        return;
    }
    ...
    for (const Member<DocumentMarker>& marker : list->GetMarkers()) {
        if (marker->EndOffset() > start_offset && marker->StartOffset() < end_offset) {
            auto it = marker_groups_.find(marker);
            if (it != marker_groups_.end()) {
...
DocumentMarkerList* DocumentMarkerController::FindMarkers(const MarkerMap* marker_map, const Text* key) const
{
...
DocumentMarkerList* DocumentMarkerController::FindMarkersForType(DocumentMarker::MarkerType type, const Text* key) const
{
...
DocumentMarker* DocumentMarkerController::FirstMarkerAroundPosition(const PositionInFlatTree& position, DocumentMarker::MarkerTypes types)
{
    if (position.IsNull())
        return nullptr;
    ...
    for (const Node& node : EphemeralRangeInFlatTree(start, end).Nodes()) {
        ...
        const unsigned end_range_offset = node == end_node ? end_offset : text_node->length();

        DocumentMarker* const found_marker = FirstMarkerIntersectingOffsetRange(*text_node, start_range_offset, end_range_offset, types);
        if (found_marker) {
            return found_marker;
        }
    }
...
DocumentMarker* DocumentMarkerController::FirstMarkerIntersectingEphemeralRange(const EphemeralRange& range, DocumentMarker::MarkerTypes types)
{
    if (range.IsNull()) {
        return nullptr;
    }
    ...
    if (range.IsCollapsed()) {
        return FirstMarkerAroundPosition(ToPositionInFlatTree(range.StartPosition()), types);
    }

    const Node* const start_container = range.StartPosition().ComputeContainerNode();
    ...
    const unsigned start_offset = range.StartPosition().ComputeOffsetInContainerNode();
    const unsigned end_offset = start_container == end_container ? range.EndPosition().ComputeOffsetInContainerNode() : text_node->length();

    return FirstMarkerIntersectingOffsetRange(*text_node, start_offset, end_offset, types);
}

DocumentMarker* DocumentMarkerController::FirstMarkerIntersectingOffsetRange(
    const Text& node, unsigned start_offset, unsigned end_offset, DocumentMarker::MarkerTypes types)
{
    ...
    for (DocumentMarker::MarkerType type : types) {
        ...
        DocumentMarker* found_marker = list->FirstMarkerIntersectingRange(start_offset, end_offset);
        if (found_marker) {
            return found_marker;
        }
    }
...
DocumentMarkerGroup* DocumentMarkerController::FirstMarkerGroupAroundPosition(const PositionInFlatTree& position, DocumentMarker::MarkerTypes types)
{
    return GetMarkerGroupForMarker(FirstMarkerAroundPosition(position, types));
}

DocumentMarkerGroup* DocumentMarkerController::FirstMarkerGroupIntersectingEphemeralRange(const EphemeralRange& range, DocumentMarker::MarkerTypes types)
{
    return GetMarkerGroupForMarker(FirstMarkerIntersectingEphemeralRange(range, types));
}

DocumentMarkerGroup* DocumentMarkerController::GetMarkerGroupForMarker(const DocumentMarker* marker)
{
    if (marker) {
        auto it = marker_groups_.find(marker);
        if (it != marker_groups_.end()) {
...
HeapVector<std::pair<Member<const Text>, Member<DocumentMarker>>> DocumentMarkerController::MarkersAroundPosition(
    const PositionInFlatTree& position, DocumentMarker::MarkerTypes types)
{
    HeapVector<std::pair<Member<const Text>, Member<DocumentMarker>>> node_marker_pairs;
    ...
    for (const Node& node : EphemeralRangeInFlatTree(start, end).Nodes()) {
        ...
        for (DocumentMarker::MarkerType type : types) {
            const DocumentMarkerList* const list = FindMarkersForType(type, text_node);
            if (!list) {
                continue;
            }

            const DocumentMarkerVector& marker_vector = list->MarkersIntersectingRange(start_range_offset, end_range_offset);
...
HeapVector<std::pair<Member<const Text>, Member<DocumentMarker>>> DocumentMarkerController::MarkersIntersectingRange(
    const EphemeralRangeInFlatTree& range, DocumentMarker::MarkerTypes types)
{
    HeapVector<std::pair<Member<const Text>, Member<DocumentMarker>>> node_marker_pairs;
    if (!PossiblyHasMarkers(types))
        return node_marker_pairs;

    const Node* const range_start_container = range.StartPosition().ComputeContainerNode();
    const unsigned range_start_offset = range.StartPosition().ComputeOffsetInContainerNode();
    const Node* const range_end_container = range.EndPosition().ComputeContainerNode();
    const unsigned range_end_offset = range.EndPosition().ComputeOffsetInContainerNode();
    ...
    for (Node& node : range.Nodes()) {
        ...
        const unsigned start_offset = node == range_start_container ? range_start_offset : 0;
        const unsigned max_character_offset = To<CharacterData>(node).length();
        const unsigned end_offset = node == range_end_container ? range_end_offset : max_character_offset;
        ...
        for (DocumentMarker::MarkerType type : types) {
            ...
            const DocumentMarkerVector& markers_from_this_list = list->MarkersIntersectingRange(start_offset, end_offset);
...
DocumentMarkerVector DocumentMarkerController::MarkersFor(const Text& text, DocumentMarker::MarkerTypes marker_types) const
{
    ...
    std::optional<DocumentMarker::MarkerType> lone_marker = marker_types.IsOneMarkerType();
    if (lone_marker) {
    ...
    std::sort(result.begin(), result.end(),
        [](const Member<DocumentMarker>& marker1, const Member<DocumentMarker>& marker2) { return marker1->StartOffset() < marker2->StartOffset(); });
    return result;
}

DocumentMarkerVector DocumentMarkerController::MarkersFor(
    const Text& text, DocumentMarker::MarkerType marker_type, unsigned start_offset, unsigned end_offset) const
{
    DocumentMarkerVector result;
    DocumentMarkerList* const list = FindMarkersForType(marker_type, &text);
    return list ? list->MarkersIntersectingRange(start_offset, end_offset) : result;
}
...
void DocumentMarkerController::ApplyToMarkersOfType(base::FunctionRef<void(const Text&, DocumentMarker*)> func, DocumentMarker::MarkerType type)
{
...
void DocumentMarkerController::MergeOverlappingMarkers(DocumentMarker::MarkerType type)
{
    MarkerMap* marker_map = markers_[MarkerTypeToMarkerIndex(type)];
    if (!marker_map) {
        return;
...
DocumentMarkerVector DocumentMarkerController::ComputeMarkersToPaint(const Text& text) const
{
    DocumentMarker::MarkerTypes excluded_highlight_pseudos = DocumentMarker::MarkerTypes::HighlightPseudos();
    ...
    // start offset.
    const DocumentMarkerVector& suggestion_markers = MarkersFor(text, DocumentMarker::MarkerTypes::Suggestion());
    if (suggestion_markers.empty()) {
        // If there are no suggestion markers, we can return early as a minor
        // performance optimization.
        markers_to_paint.AppendVector(MarkersFor(text,
            DocumentMarker::MarkerTypes::AllBut(DocumentMarker::MarkerTypes(DocumentMarker::kSuggestion | DocumentMarker::kCustomHighlight))
    ...
    const DocumentMarkerVector& markers_overridden_by_suggestion_markers
        = MarkersFor(text, DocumentMarker::MarkerTypes(DocumentMarker::kComposition | DocumentMarker::kSpelling).Subtract(excluded_highlight_pseudos));
    ...
    for (DocumentMarker* marker : markers_overridden_by_suggestion_markers) {
        while (suggestion_starts_index < suggestion_starts.size() && suggestion_starts[suggestion_starts_index] <= marker->StartOffset()) {
            ++suggestion_starts_index;
            ++number_suggestions_currently_inside;
        }
        while (suggestion_ends_index < suggestion_ends.size() && suggestion_ends[suggestion_ends_index] <= marker->StartOffset()) {
        ...
        if (suggestion_starts_index < suggestion_starts.size() && suggestion_starts[suggestion_starts_index] < marker->EndOffset()) {
            continue;
        }
    ...
    markers_to_paint.AppendVector(MarkersFor(text,
        DocumentMarker::MarkerTypes::AllBut(DocumentMarker::MarkerTypes(DocumentMarker::kComposition | DocumentMarker::kSpelling | DocumentMarker::kSuggestion
...
Vector<gfx::Rect> DocumentMarkerController::LayoutRectsForTextMatchMarkers()
{
    ...
    MarkerMap* marker_map = markers_[MarkerTypeToMarkerIndex(DocumentMarker::kTextMatch)];
    DCHECK(marker_map);
    MarkerMap::iterator end = marker_map->end();
    for (MarkerMap::iterator node_iterator = marker_map->begin(); node_iterator != end; ++node_iterator) {
...
void DocumentMarkerController::InvalidateRectsForTextMatchMarkersInNode(const Text& node)
{
    if (!PossiblyHasMarkers(DocumentMarker::kTextMatch)) {
        return;
    }

    const DocumentMarkerList* const marker_list = FindMarkersForType(DocumentMarker::kTextMatch, &node);
    if (!marker_list) {
        return;
    }

    const HeapVector<Member<DocumentMarker>>& markers_in_list = marker_list->GetMarkers();
...
void DocumentMarkerController::InvalidateRectsForAllTextMatchMarkers()
{
    ...
    const MarkerMap* marker_map = markers_[MarkerTypeToMarkerIndex(DocumentMarker::kTextMatch)];
    DCHECK(marker_map);
...
void DocumentMarkerController::RemoveMarkersForNode(const Text& text, DocumentMarker::MarkerTypes marker_types)
{
    if (!PossiblyHasMarkers(marker_types)) {
        return;
    }
...
void DocumentMarkerController::RemoveSpellingMarkersUnderWords(const Vector<String>& words)
{
    for (DocumentMarker::MarkerType type : DocumentMarker::MarkerTypes::Misspelling()) {
        MarkerMap* marker_map = markers_[MarkerTypeToMarkerIndex(type)];
        if (!marker_map) {
            continue;
        }
        ...
        for (auto& node_markers : *marker_map) {
            ...
            if (To<SpellCheckMarkerListImpl>(list)->RemoveMarkersUnderWords(text.data(), words)) {
...
void DocumentMarkerController::RemoveSuggestionMarkerInRangeOnFinish(const EphemeralRangeInFlatTree& range)
{
    ...
    const HeapVector<std::pair<Member<const Text>, Member<DocumentMarker>>>& node_marker_pairs
        = MarkersIntersectingRange(range, DocumentMarker::MarkerTypes::Suggestion());
    MarkerMap* marker_map = markers_[MarkerTypeToMarkerIndex(DocumentMarker::kSuggestion)];
    for (const auto& node_marker_pair : node_marker_pairs) {
        ...
        if (suggestion_marker->NeedsRemovalOnFinishComposing()) {
            ...
            To<SuggestionMarkerListImpl>(list)->RemoveMarkerByTag(suggestion_marker->Tag());
            InvalidatePaintForNode(text);
            if (list->IsEmpty()) {
                marker_map->erase(&text);
...
void DocumentMarkerController::RemoveSuggestionMarkerByType(const EphemeralRangeInFlatTree& range, const SuggestionMarker::SuggestionType& type)
{
    // MarkersIntersectingRange() might be expensive. In practice, we hope we will
...
void DocumentMarkerController::RemoveSuggestionMarkerByType(const SuggestionMarker::SuggestionType& type)
{
...
void DocumentMarkerController::RemoveSuggestionMarkerByTag(const Text& text, int32_t marker_tag)
{
    if (!PossiblyHasMarkers(DocumentMarker::kSuggestion)) {
        return;
    }
...
void DocumentMarkerController::RemoveMarkersOfTypes(DocumentMarker::MarkerTypes marker_types)
{
    if (!PossiblyHasMarkers(marker_types)) {
        return;
    }
...
void DocumentMarkerController::RemoveMarkersFromList(MarkerMap::iterator iterator, DocumentMarker::MarkerType marker_type)
{
...
bool DocumentMarkerController::SetTextMatchMarkersActive(const EphemeralRange& range, bool active)
{
    if (!PossiblyHasMarkers(DocumentMarker::kTextMatch)) {
        return false;
    }
    ...
    const unsigned container_start_offset = range.StartPosition().ComputeOffsetInContainerNode();
    const unsigned container_end_offset = range.EndPosition().ComputeOffsetInContainerNode();
    ...
    for (Node& node : range.Nodes()) {
        ...
        marker_found |= SetTextMatchMarkersActive(*text_node, start_offset, end_offset, active);
    }
    return marker_found;
}

bool DocumentMarkerController::SetTextMatchMarkersActive(const Text& text, unsigned start_offset, unsigned end_offset, bool active)
{
    DocumentMarkerList* const list = FindMarkersForType(DocumentMarker::kTextMatch, &text);
    if (!list) {
        return false;
    }

    bool doc_dirty = To<TextMatchMarkerListImpl>(list)->SetTextMatchMarkersActive(start_offset, end_offset, active);
...
void DocumentMarkerController::ShowMarkers() const
{
    ...
    for (DocumentMarker::MarkerType type : DocumentMarker::MarkerTypes::All()) {
        ...
        for (auto& node_iterator : *marker_map) {
            ...
            const HeapVector<Member<DocumentMarker>>& markers_in_list = list->GetMarkers();
            for (const DocumentMarker* marker : markers_in_list) {
                ...
                builder.AppendFormat(" %u:[%u:%u](%d)", static_cast<uint32_t>(marker->GetType()), marker->StartOffset(), marker->EndOffset(), is_active_match);
            }
        }
        builder.Append("\n");
    }
...
void DocumentMarkerController::DidUpdateCharacterData(CharacterData* node, unsigned offset, unsigned old_length, unsigned new_length)
{
    if (!PossiblyHasMarkers(DocumentMarker::MarkerTypes::All()))
        return;
