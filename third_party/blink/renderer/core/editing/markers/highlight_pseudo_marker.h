...
template <> struct DowncastTraits<HighlightPseudoMarker> {
    static bool AllowFrom(const DocumentMarker& document_marker)
    {
        return document_marker.GetType() == DocumentMarker::kCustomHighlight || document_marker.GetType() == DocumentMarker::kTextFragment;
    }
};
