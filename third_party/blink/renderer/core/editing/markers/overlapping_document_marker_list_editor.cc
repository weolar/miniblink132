...
void OverlappingDocumentMarkerListEditor::AddMarker(MarkerList* list, DocumentMarker* marker)
{
...
bool OverlappingDocumentMarkerListEditor::MoveMarkers(MarkerList* src_list, int length, DocumentMarkerList* dst_list)
{
    DCHECK_GT(length, 0);
...
bool OverlappingDocumentMarkerListEditor::RemoveMarkers(MarkerList* list, unsigned start_offset, int length)
{
...
bool OverlappingDocumentMarkerListEditor::ShiftMarkers(MarkerList* list, unsigned offset, unsigned old_length, unsigned new_length)
{
...
HeapVector<Member<DocumentMarker>> OverlappingDocumentMarkerListEditor::MarkersIntersectingRange(
    const MarkerList& list, unsigned start_offset, unsigned end_offset)
{
    ...
    base::ranges::copy_if(list.begin(), end_it, std::back_inserter(results),
        [start_offset, end_offset](const DocumentMarker* marker) { return marker->StartOffset() < end_offset && marker->EndOffset() > start_offset; });
    return results;
}
