...
void TextMatchMarkerListImpl::Add(DocumentMarker* marker)
{
    ...
    SortedDocumentMarkerListEditor::AddMarkerWithoutMergingOverlapping(&markers_, marker);
}
...
const HeapVector<Member<DocumentMarker>>& TextMatchMarkerListImpl::GetMarkers() const
{
    return markers_;
}

DocumentMarker* TextMatchMarkerListImpl::FirstMarkerIntersectingRange(unsigned start_offset, unsigned end_offset) const
{
    return SortedDocumentMarkerListEditor::FirstMarkerIntersectingRange(markers_, start_offset, end_offset);
}

HeapVector<Member<DocumentMarker>> TextMatchMarkerListImpl::MarkersIntersectingRange(unsigned start_offset, unsigned end_offset) const
{
    return SortedDocumentMarkerListEditor::MarkersIntersectingRange(markers_, start_offset, end_offset);
}

bool TextMatchMarkerListImpl::MoveMarkers(int length, DocumentMarkerList* dst_list)
{
...
bool TextMatchMarkerListImpl::ShiftMarkers(const String&, unsigned offset, unsigned old_length, unsigned new_length)
{
...
static void UpdateMarkerLayoutRect(const Node& node, TextMatchMarker& marker)
{
    ...
    if (auto* locked_root = DisplayLockUtilities::HighestLockedInclusiveAncestor(node)) {
        if (auto* locked_root_layout_object = locked_root->GetLayoutObject()) {
            marker.SetRect(frame_view->FrameToDocument(PhysicalRect(locked_root_layout_object->AbsoluteBoundingBoxRect())));
        } else {
    ...
    marker.SetRect(frame_view->FrameToDocument(PhysicalRect(ComputeTextRect(range))));
}
...
bool TextMatchMarkerListImpl::SetTextMatchMarkersActive(unsigned start_offset, unsigned end_offset, bool active)
{
    bool doc_dirty = false;
    auto const start = std::upper_bound(markers_.begin(), markers_.end(), start_offset,
        [](size_t start_offset, const Member<DocumentMarker>& marker) { return start_offset < marker->EndOffset(); });
    auto start_position = base::checked_cast<wtf_size_t>(start - markers_.begin());
