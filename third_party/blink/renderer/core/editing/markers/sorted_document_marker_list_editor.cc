...
void SortedDocumentMarkerListEditor::AddMarkerWithoutMergingOverlapping(MarkerList* list, DocumentMarker* marker)
{
    ...
    auto const pos
        = std::lower_bound(list->begin(), list->end(), marker, [](const Member<DocumentMarker>& marker_in_list, const DocumentMarker* marker_to_insert) {
...
bool SortedDocumentMarkerListEditor::MoveMarkers(MarkerList* src_list, int length, DocumentMarkerList* dst_list)
{
    DCHECK_GT(length, 0);
...
bool SortedDocumentMarkerListEditor::RemoveMarkers(MarkerList* list, unsigned start_offset, int length)
{
    const unsigned end_offset = start_offset + length;
    MarkerList::iterator start_pos = std::upper_bound(
        list->begin(), list->end(), start_offset, [](size_t start_offset, const Member<DocumentMarker>& marker) { return start_offset < marker->EndOffset(); });

    MarkerList::iterator end_pos = std::lower_bound(
        list->begin(), list->end(), end_offset, [](const Member<DocumentMarker>& marker, size_t end_offset) { return marker->StartOffset() < end_offset; });

    list->EraseAt(base::checked_cast<wtf_size_t>(start_pos - list->begin()), base::checked_cast<wtf_size_t>(end_pos - start_pos));
    return start_pos != end_pos;
}

bool SortedDocumentMarkerListEditor::ShiftMarkersContentDependent(MarkerList* list, unsigned offset, unsigned old_length, unsigned new_length)
{
    ...
    // text node.
    const MarkerList::iterator& shift_range_begin = std::upper_bound(
        list->begin(), list->end(), offset, [](size_t offset, const Member<DocumentMarker>& marker) { return offset < marker->EndOffset(); });

    wtf_size_t num_removed = 0;
    bool did_shift_marker = false;

    auto begin_offset = base::checked_cast<wtf_size_t>(shift_range_begin - list->begin());
...
bool SortedDocumentMarkerListEditor::ShiftMarkersContentIndependent(MarkerList* list, unsigned offset, unsigned old_length, unsigned new_length)
{
    // Find first marker that ends after the start of the region being edited.
    // Markers before this one can be left untouched. This saves us some time over
    // scanning the entire list linearly if the edit region is near the end of the
    ...
    auto position = base::checked_cast<wtf_size_t>(shift_range_begin - list->begin());
    ...
    for (auto marker : sub_span) {
        std::optional<DocumentMarker::MarkerOffsets> result = marker->ComputeOffsetsAfterShift(offset, old_length, new_length);
        ...
        if (marker->StartOffset() != result.value().start_offset || marker->EndOffset() != result.value().end_offset) {
            did_shift_marker = true;
...
DocumentMarker* SortedDocumentMarkerListEditor::FirstMarkerIntersectingRange(const MarkerList& list, unsigned start_offset, unsigned end_offset)
{
    DCHECK_LE(start_offset, end_offset);

    auto const marker_it = std::lower_bound(
        list.begin(), list.end(), start_offset, [](const DocumentMarker* marker, unsigned start_offset) { return marker->EndOffset() <= start_offset; });
...
HeapVector<Member<DocumentMarker>> SortedDocumentMarkerListEditor::MarkersIntersectingRange(const MarkerList& list, unsigned start_offset, unsigned end_offset)
{
    DCHECK_LE(start_offset, end_offset);

    auto const start_it = std::lower_bound(
        list.begin(), list.end(), start_offset, [](const DocumentMarker* marker, unsigned start_offset) { return marker->EndOffset() <= start_offset; });
    auto const end_it = std::upper_bound(
        list.begin(), list.end(), end_offset, [](unsigned end_offset, const DocumentMarker* marker) { return end_offset <= marker->StartOffset(); });
