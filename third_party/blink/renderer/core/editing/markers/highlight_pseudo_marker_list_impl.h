...
public:
    ...
    HighlightPseudoMarkerListImpl& operator=(const HighlightPseudoMarkerListImpl&) = delete;
...
template <> struct DowncastTraits<HighlightPseudoMarkerListImpl> {
    static bool AllowFrom(const DocumentMarkerList& list)
    {
        return list.MarkerType() == DocumentMarker::kCustomHighlight || list.MarkerType() == DocumentMarker::kTextFragment;
    }
};
