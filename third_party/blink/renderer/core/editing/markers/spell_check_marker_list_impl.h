...
public:
    ...
    bool RemoveMarkersUnderWords(const String& node_text, const Vector<String>& words);
...
template <> struct DowncastTraits<SpellCheckMarkerListImpl> {
    static bool AllowFrom(const DocumentMarkerList& list)
    {
        return list.MarkerType() == DocumentMarker::kSpelling || list.MarkerType() == DocumentMarker::kGrammar;
    }
};
