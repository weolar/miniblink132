...
protected:
    ...
    CustomHighlightMarker* CreateMarker(unsigned start_offset, unsigned end_offset, const AtomicString& name)
    {
        HeapVector<Member<AbstractRange>> dummy_ranges;
        return MakeGarbageCollected<CustomHighlightMarker>(start_offset, end_offset, name, MakeGarbageCollected<Highlight>(dummy_ranges));
    }
...
TEST_F(CustomHighlightMarkerListImplTest, MergeOverlappingMarkersSingleton)
{
    ...
    marker_list_->MergeOverlappingMarkers();
    const HeapVector<Member<DocumentMarker>>& markers = marker_list_->GetMarkers();
...
TEST_F(CustomHighlightMarkerListImplTest, MergeOverlappingMarkersMultiNames)
{
    ...
    EXPECT_EQ("A", To<CustomHighlightMarker>(markers[0].Get())->GetHighlightName().GetString());
    EXPECT_EQ(0u, markers[1]->StartOffset());
    EXPECT_EQ(15u, markers[1]->EndOffset());
    EXPECT_EQ("B", To<CustomHighlightMarker>(markers[1].Get())->GetHighlightName().GetString());
    EXPECT_EQ(10u, markers[2]->StartOffset());
    EXPECT_EQ(20u, markers[2]->EndOffset());
    EXPECT_EQ("A", To<CustomHighlightMarker>(markers[2].Get())->GetHighlightName().GetString());
    EXPECT_EQ(15u, markers[3]->StartOffset());
    EXPECT_EQ(30u, markers[3]->EndOffset());
    EXPECT_EQ("B", To<CustomHighlightMarker>(markers[3].Get())->GetHighlightName().GetString());
    EXPECT_EQ(25u, markers[4]->StartOffset());
    EXPECT_EQ(30u, markers[4]->EndOffset());
    EXPECT_EQ("A", To<CustomHighlightMarker>(markers[4].Get())->GetHighlightName().GetString());
}
