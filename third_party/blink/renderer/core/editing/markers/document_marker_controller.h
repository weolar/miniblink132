...
class CORE_EXPORT DocumentMarkerController final : public GarbageCollected<DocumentMarkerController>, public SynchronousMutationObserver {
public:
    ...
    void AddSpellingMarker(const EphemeralRange&, const String& description = g_empty_string);
    void AddGrammarMarker(const EphemeralRange&, const String& description = g_empty_string);
    void AddTextMatchMarker(const EphemeralRange&, TextMatchMarker::MatchStatus);
    void AddCompositionMarker(const EphemeralRange&, Color underline_color, ui::mojom::ImeTextSpanThickness, ui::mojom::ImeTextSpanUnderlineStyle,
        Color text_color, Color background_color);
    void AddActiveSuggestionMarker(const EphemeralRange&, Color underline_color, ui::mojom::ImeTextSpanThickness, ui::mojom::ImeTextSpanUnderlineStyle,
        Color text_color, Color background_color);
    void AddSuggestionMarker(const EphemeralRange&, const SuggestionMarkerProperties&);
    void AddTextFragmentMarker(const EphemeralRange&);
    void AddCustomHighlightMarker(const EphemeralRange&, const String& highlight_name, const Member<Highlight> highlight);
    ...
    void RemoveMarkersForNode(const Text&, DocumentMarker::MarkerTypes = DocumentMarker::MarkerTypes::All());
    void RemoveSpellingMarkersUnderWords(const Vector<String>& words);
    void RemoveSuggestionMarkerByTag(const Text&, int32_t marker_tag);
    void RemoveSuggestionMarkerByType(const EphemeralRangeInFlatTree& range, const SuggestionMarker::SuggestionType& type);
    void RemoveSuggestionMarkerByType(const SuggestionMarker::SuggestionType& type);
    ...
    bool SetTextMatchMarkersActive(const Text&, unsigned start_offset, unsigned end_offset, bool);
    ...
    DocumentMarker* FirstMarkerAroundPosition(const PositionInFlatTree&, DocumentMarker::MarkerTypes);
    ...
    DocumentMarker* FirstMarkerIntersectingEphemeralRange(const EphemeralRange&, DocumentMarker::MarkerTypes);
    ...
    // If such a marker exists, this method will return one of them (no guarantees
    // are provided as to which one). Otherwise, this method will return null.
    DocumentMarker* FirstMarkerIntersectingOffsetRange(const Text&, unsigned start_offset, unsigned end_offset, DocumentMarker::MarkerTypes);
    // Wrappers for FirstMarker functions that return the DocumentMarkerGroup for
    // the found DocumentMarker.
    DocumentMarkerGroup* FirstMarkerGroupAroundPosition(const PositionInFlatTree&, DocumentMarker::MarkerTypes);
    DocumentMarkerGroup* FirstMarkerGroupIntersectingEphemeralRange(const EphemeralRange&, DocumentMarker::MarkerTypes);
    // If the given position is either at the boundary or inside a word, expands
    ...
    HeapVector<std::pair<Member<const Text>, Member<DocumentMarker>>> MarkersAroundPosition(
    ...
    // returned.
    HeapVector<std::pair<Member<const Text>, Member<DocumentMarker>>> MarkersIntersectingRange(const EphemeralRangeInFlatTree&, DocumentMarker::MarkerTypes);
    DocumentMarkerVector MarkersFor(const Text&, DocumentMarker::MarkerTypes = DocumentMarker::MarkerTypes::All()) const;
    DocumentMarkerVector MarkersFor(const Text&, DocumentMarker::MarkerType, unsigned start_offset, unsigned end_offset) const;
    ...
    void ApplyToMarkersOfType(base::FunctionRef<void(const Text&, DocumentMarker*)>, DocumentMarker::MarkerType);
...
private:
    void AddMarkerInternal(
        const EphemeralRange&, base::FunctionRef<DocumentMarker*(int, int)> create_marker_from_offsets, const TextIteratorBehavior& iterator_behavior = {});
    ...
    void RemoveMarkersInternal(const Text&, unsigned start_offset, int length, DocumentMarker::MarkerType);
    // Searches marker_map for key. Returns the mapped value if it is present,
    // otherwise nullptr.
    DocumentMarkerList* FindMarkers(const MarkerMap* marker_map, const Text* key) const;
    // Find the marker list of the given type for the given text node,
    // or nullptr if the node has no markers of that type.
    DocumentMarkerList* FindMarkersForType(DocumentMarker::MarkerType, const Text* key) const;
    ...
    void DidRemoveNodeFromMap(DocumentMarker::MarkerType, bool clear_document_allowed = true);

    MarkerMaps markers_;

    using MarkerGroup = HeapHashMap<WeakMember<const DocumentMarker>, Member<DocumentMarkerGroup>>;
