...
class CORE_EXPORT DocumentMarkerList : public GarbageCollected<DocumentMarkerList> {
public:
    ...
    virtual DocumentMarker* FirstMarkerIntersectingRange(unsigned start_offset, unsigned end_offset) const = 0;
    ...
    virtual HeapVector<Member<DocumentMarker>> MarkersIntersectingRange(unsigned start_offset, unsigned end_offset) const = 0;
    ...
    // applied.
    virtual bool ShiftMarkers(const String& node_text, unsigned offset, unsigned old_length, unsigned new_length) = 0;
