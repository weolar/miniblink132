...
public:
    ...
    {
        return GetDocument().GetFrame()->GetEditor();
    }
    ...
    {
        return std::distance(GetEditor().GetUndoStack().RedoSteps().begin(), GetEditor().GetUndoStack().RedoSteps().end());
    }

    ptrdiff_t SizeOfUndoStack() const
    {
        return std::distance(GetEditor().GetUndoStack().UndoSteps().begin(), GetEditor().GetUndoStack().UndoSteps().end());
    }
};

TEST_F(EditorTest, CanCopyCrossingShadowBoundary)
{
    const SelectionInDOMTree selection = SetSelectionTextToBody("<p><template data-mode=open>^abc</template></p><b>|</b>");
...
TEST_F(EditorTest, copyGeneratedPassword)
{
    ...
    auto& element = To<HTMLInputElement>(*GetDocument().getElementById(AtomicString("password")));
...
TEST_F(EditorTest, CopyVisibleSelection)
{
    ...
    auto& text_control = To<HTMLInputElement>(*GetDocument().getElementById(AtomicString("hiding")));
    text_control.select();

    ExecuteCopy();

    const String copied = GetDocument().GetFrame()->GetSystemClipboard()->ReadPlainText();
    EXPECT_EQ("HEY", copied);
}

TEST_F(EditorTest, DontCopyHiddenSelections)
{
    const char* body_content = "<input type=checkbox id=checkbox>"
    ...
    auto& checkbox = To<HTMLInputElement>(*GetDocument().getElementById(AtomicString("checkbox")));
    checkbox.Focus();
...
TEST_F(EditorTest, ReplaceSelection)
{
    ...
    auto& text_control = To<HTMLInputElement>(*GetDocument().getElementById(AtomicString("text")));
    text_control.select();
...
TEST_F(EditorTest, RedoWithDisconnectedEditable)
{
    ...
    target.Focus();
    GetDocument().execCommand("insertHtml", false, "<b>xyz</b>", ASSERT_NO_EXCEPTION);
    ...
    target.remove();
    EXPECT_EQ(0, SizeOfRedoStack()) << "We don't need to have redo steps for removed <input>";
...
TEST_F(EditorTest, UndoWithDisconnectedEditable)
{
    ...
    EXPECT_EQ(0, SizeOfUndoStack()) << "We don't need to have undo steps for removed editable";
}
...
TEST_F(EditorTest, UndoWithDisconnectedInput)
{
    ...
    input.remove();
    EXPECT_EQ(0, SizeOfRedoStack());
    EXPECT_EQ(0, SizeOfUndoStack()) << "We don't need to have undo steps for removed <input>";
}

// http://crbug.com/873037
TEST_F(EditorTest, UndoWithInvalidSelection)
{
    const SelectionInDOMTree selection = SetSelectionTextToBody("<div contenteditable><div></div><b>^abc|</b></div>");
    Selection().SetSelection(selection, SetSelectionOptions());
    ...
    EXPECT_EQ("<div contenteditable><div></div><b>|</b></div>", GetSelectionTextFromBody());
}
