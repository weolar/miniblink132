...
{
    GetFrame().Selection().SetSelection(SetSelectionTextToBody("<div contenteditable id=\"sample\">a|b</div>"), SetSelectionOptions());
    const Element& sample = *GetElementById("sample");

    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kSelection);
    EXPECT_EQ(PlainTextRange(1, 1), GetInputMethodController().GetSelectionOffsets());
...
{
    GetFrame().Selection().SetSelection(SetSelectionTextToBody("<span contenteditable><img>|a</img></span>"), SetSelectionOptions());
    ...
    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kSelection);
    EXPECT_EQ(PlainTextRange(5, 5), GetInputMethodController().GetSelectionOffsets());
...
TEST_F(CachedTextInputInfoTest, PlaceholderBRInTextArea)
{
    ...
    GetFrame().Selection().SetSelection(
        SelectionInDOMTree::Builder().Collapse(Position::LastPositionInNode(*target.InnerEditorElement())).Build(), SetSelectionOptions());

    EXPECT_EQ(PlainTextRange(4, 4), GetInputMethodController().GetSelectionOffsets());
    EXPECT_EQ("abc\n", GetCachedTextInputInfo().GetText()) << "We should not emit a newline for placeholder <br>";
}
...
TEST_F(CachedTextInputInfoTest, PlaceholderBROnlyInTextArea)
{
    ...
    EXPECT_EQ(PlainTextRange(0, 0), GetInputMethodController().GetSelectionOffsets());
    EXPECT_EQ("", GetCachedTextInputInfo().GetText());
}

TEST_F(CachedTextInputInfoTest, RelayoutBoundary)
{
    InsertStyleElement("#sample { contain: strict; width: 100px; height: 100px; }");
    GetFrame().Selection().SetSelection(SetSelectionTextToBody("<div contenteditable><div id=\"sample\">^a|b</div>"), SetSelectionOptions());
    const Element& sample = *GetElementById("sample");
    ASSERT_TRUE(sample.GetLayoutObject()->IsRelayoutBoundary());

    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kSelection);
    EXPECT_EQ(PlainTextRange(0, 1), GetInputMethodController().GetSelectionOffsets());
    EXPECT_EQ("ab", GetCachedTextInputInfo().GetText());
...
{
    GetFrame().Selection().SetSelection(SetSelectionTextToBody("<div contenteditable>"
    ...
    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kSelection);
    EXPECT_EQ(PlainTextRange(2, 2), GetInputMethodController().GetSelectionOffsets());
    ...
    // Move caret after "cd"
    GetFrame().Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position(text_cd, 2)).Build(), SetSelectionOptions());

    // Insert "CD" after "cd"
    GetDocument().execCommand("insertText", false, "CD", ASSERT_NO_EXCEPTION);

    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kSelection);
    EXPECT_EQ(PlainTextRange(8, 8), GetInputMethodController().GetSelectionOffsets());
...
{
    GetFrame().Selection().SetSelection(SetSelectionTextToBody("<div id=host><template data-mode=open>"
                                                               "<a>012</a><b>3^45</b>67|8"
                                                               "</template></div>"),
        SetSelectionOptions());

    EXPECT_EQ(PlainTextRange(4, 8), GetInputMethodController().GetSelectionOffsets());

    // Change shadow tree to "XYZ<a>012</a><b>345</b>678"
    auto& shadow_root = *GetElementById("host")->GetShadowRoot();
    shadow_root.insertBefore(Text::Create(GetDocument(), "XYZ"), shadow_root.firstChild());

    // Ask |CachedTextInputInfo| to compute |PlainTextRange| for selection.
    GetFrame().Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position(*To<Text>(shadow_root.lastChild()), 0)).Build(), SetSelectionOptions());

    EXPECT_EQ(PlainTextRange(9, 9), GetInputMethodController().GetSelectionOffsets());
}

// http://crbug.com/1228635
TEST_F(CachedTextInputInfoTest, VisibilityHiddenToVisible)
{
    GetFrame().Selection().SetSelection(SetSelectionTextToBody("<div contenteditable id=sample>"
                                                               "<b id=target style='visibility: hidden'>A</b><b>^Z|</b></div>"),
        SetSelectionOptions());

    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kSelection);
    EXPECT_EQ(PlainTextRange(0, 1), GetInputMethodController().GetSelectionOffsets());
    EXPECT_EQ("Z", GetCachedTextInputInfo().GetText()) << "Texts within visibility:hidden are excluded";

    Element& target = *GetElementById("target");
    target.style()->setProperty(GetDocument().GetExecutionContext(), "visibility", "visible", "", ASSERT_NO_EXCEPTION);

    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kSelection);
    EXPECT_EQ(PlainTextRange(1, 2), GetInputMethodController().GetSelectionOffsets());
...
TEST_F(CachedTextInputInfoTest, VisibilityVisibleToHidden)
{
    ...
    Element& target = *GetElementById("target");
    target.style()->setProperty(GetDocument().GetExecutionContext(), "visibility", "hidden", "", ASSERT_NO_EXCEPTION);
