...
EphemeralRange ComputeWholeContentRange(const ContainerNode& container)
{
    ...
    if (!before_placeholder) {
        ...
        return EphemeralRange(Position::FirstPositionInNode(container), Position::FirstPositionInNode(container));
    }
    ...
    return EphemeralRange(Position::FirstPositionInNode(container), Position::AfterNode(*before_placeholder));
}
...
{
    return TextIteratorBehavior::Builder().SetEmitsObjectReplacementCharacter(true).SetEmitsSpaceForNbsp(true).Build();
}
...
void CachedTextInputInfo::DidChangeVisibility(const LayoutObject& layout_object)
{
...
void CachedTextInputInfo::EnsureCached(const ContainerNode& container) const
{
    ...
    TextIteratorAlgorithm<EditingStrategy> it(ComputeWholeContentRange(container), Behavior());
    if (it.AtEnd())
        return;
...
PlainTextRange CachedTextInputInfo::GetComposition(const EphemeralRange& range) const
{
    DCHECK(container_);
    return GetPlainTextRangeWithCache(range, &composition_);
}

PlainTextRange CachedTextInputInfo::GetPlainTextRangeWithCache(const EphemeralRange& range, CachedPlainTextRange* text_range) const
{
...
PlainTextRange CachedTextInputInfo::GetPlainTextRange(const EphemeralRange& range) const
{
    if (range.IsNull())
    ...
    if (container_start > range.StartPosition())
        return PlainTextRange();
    const unsigned start_offset = RangeLength(EphemeralRange(container_start, range.StartPosition()));
    const unsigned end_offset = range.IsCollapsed() ? start_offset : RangeLength(EphemeralRange(container_start, range.EndPosition()));
// TODO(crbug.com/1256635): This DCHECK is triggered by Crostini on CrOS.
#if !BUILDFLAG(IS_CHROMEOS_ASH)
    DCHECK_EQ(static_cast<unsigned>(TextIterator::RangeLength(EphemeralRange(container_start, range.EndPosition()), Behavior())), end_offset);
#endif
    return PlainTextRange(start_offset, end_offset);
}

PlainTextRange CachedTextInputInfo::GetSelection(const EphemeralRange& range) const
{
    DCHECK(container_);
    if (range.IsNull())
        return PlainTextRange();
...
{
    return container_ == container && layout_object_ == container.GetLayoutObject();
}

void CachedTextInputInfo::LayoutObjectWillBeDestroyed(const LayoutObject& layout_object)
{
    ClearIfNeeded(layout_object);
}
...
unsigned CachedTextInputInfo::RangeLength(const EphemeralRange& range) const
{
    ...
    if (range.StartPosition() == Position(*container_, 0) && IsA<Text>(node)) {
        ...
        if (it != offset_map_.end()) {
            const unsigned length = it->value + TextIterator::RangeLength(EphemeralRange(Position(node, 0), range.EndPosition()), Behavior());
// TODO(crbug.com/1256635): Revert https://crrev.com/c/3221041 to re-enable this
// DCHECK on CrOS.
#if !BUILDFLAG(IS_CHROMEOS_ASH)
            DCHECK_EQ(static_cast<unsigned>(TextIterator::RangeLength(range, Behavior())), length) << it->value << " " << range;
#endif
            return length;
        }
    }
...
bool CachedTextInputInfo::CachedPlainTextRange::IsValidFor(const EphemeralRange& range) const
{
    return range.StartPosition() == start_ && range.EndPosition() == end_;
}

void CachedTextInputInfo::CachedPlainTextRange::Set(const EphemeralRange& range, const PlainTextRange& text_range)
{
