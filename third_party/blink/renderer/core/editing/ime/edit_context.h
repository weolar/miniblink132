...
public:
    ...
    static EditContext* Create(ScriptState* script_state, const EditContextInit* dict);
    ...
    void updateSelection(uint32_t start, uint32_t end, ExceptionState& exception_state);
    ...
    void updateCharacterBounds(uint32_t range_start, const HeapVector<Member<DOMRect>>& character_bounds);
    ...
    // updateSelection.
    void updateText(uint32_t start, uint32_t end, const String& new_text, ExceptionState& exception_state);
    ...
    bool SetComposition(const WebString& text, const WebVector<ui::ImeTextSpan>& ime_text_spans, const WebRange& replacement_range, int selection_start,
        int selection_end) override;
    bool CommitText(
        const WebString& text, const WebVector<ui::ImeTextSpan>& ime_text_spans, const WebRange& replacement_range, int relative_caret_position) override;
    bool FinishComposingText(ConfirmCompositionBehavior selection_behavior) override;
    ...
    void GetLayoutBounds(gfx::Rect* control_bounds, gfx::Rect* selection_bounds) override;

    // Sets the composition range from the already existing text
    // This is used for reconversion scenarios in JPN IME.
    bool SetCompositionFromExistingText(int composition_start, int composition_end, const WebVector<ui::ImeTextSpan>& ime_text_spans);
    ...
    ui::mojom::VirtualKeyboardVisibilityRequest GetLastVirtualKeyboardVisibilityRequest() const override;
    // Sets the VirtualKeyboard visibility request(show/hide/none).
    void SetVirtualKeyboardVisibilityRequest(ui::mojom::VirtualKeyboardVisibilityRequest vk_visibility_request) override;
    ...
    void SetSelection(int start, int end, bool dispatch_text_update_event = false);
    ...
    bool FirstRectForCharacterRange(uint32_t location, uint32_t length, gfx::Rect& rect_in_viewport);
...
private:
    ...
    void DispatchTextFormatEvent(const WebVector<ui::ImeTextSpan>& ime_text_spans);
    ...
    void DispatchCharacterBoundsUpdateEvent(uint32_t range_start, uint32_t range_end);
