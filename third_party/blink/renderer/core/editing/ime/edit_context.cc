...
EditContext* EditContext::Create(ScriptState* script_state, const EditContextInit* dict)
{
...
ui::mojom::VirtualKeyboardVisibilityRequest EditContext::GetLastVirtualKeyboardVisibilityRequest() const
{
    return GetInputMethodController().GetLastVirtualKeyboardVisibilityRequest();
}

void EditContext::SetVirtualKeyboardVisibilityRequest(ui::mojom::VirtualKeyboardVisibilityRequest vk_visibility_request)
{
    GetInputMethodController().SetVirtualKeyboardVisibilityRequest(vk_visibility_request);
}

void EditContext::DispatchCompositionEndEvent(const String& text)
{
    auto* event = MakeGarbageCollected<CompositionEvent>(event_type_names::kCompositionend, DomWindow(), text);
    DispatchEvent(*event);
}

bool EditContext::DispatchCompositionStartEvent(const String& text)
{
    auto* event = MakeGarbageCollected<CompositionEvent>(event_type_names::kCompositionstart, DomWindow(), text);
    DispatchEvent(*event);
    return DomWindow();
}

void EditContext::DispatchCharacterBoundsUpdateEvent(uint32_t range_start, uint32_t range_end)
{
    auto* event = MakeGarbageCollected<CharacterBoundsUpdateEvent>(event_type_names::kCharacterboundsupdate, range_start, range_end);
    DispatchEvent(*event);
}

void EditContext::DispatchTextUpdateEvent(
    const String& text, uint32_t update_range_start, uint32_t update_range_end, uint32_t new_selection_start, uint32_t new_selection_end)
{
    TextUpdateEvent* event = MakeGarbageCollected<TextUpdateEvent>(
        event_type_names::kTextupdate, text, update_range_start, update_range_end, new_selection_start, new_selection_end);
    DispatchEvent(*event);
}

void EditContext::DispatchTextFormatEvent(const WebVector<ui::ImeTextSpan>& ime_text_spans)
{
    ...
    for (const auto& ime_text_span : ime_text_spans) {
        const auto range_start = base::checked_cast<wtf_size_t>(ime_text_span.start_offset + composition_range_start_);
        const auto range_end = base::checked_cast<wtf_size_t>(ime_text_span.end_offset + composition_range_start_);
        ...
        text_formats.push_back(TextFormat::Create(range_start, range_end, underline_style, underline_thickness));
    }

    TextFormatUpdateEvent* event = MakeGarbageCollected<TextFormatUpdateEvent>(event_type_names::kTextformatupdate, text_formats);
    DispatchEvent(*event);
}
...
void EditContext::Focus()
{
    ...
    EditContext* current_active_edit_context = GetInputMethodController().GetActiveEditContext();
    if (current_active_edit_context && current_active_edit_context != this) {
        ...
        current_active_edit_context->FinishComposingText(ConfirmCompositionBehavior::kKeepSelection);
    }
...
void EditContext::updateSelection(uint32_t start, uint32_t end, ExceptionState& exception_state)
{
    TRACE_EVENT2("ime", "EditContext::updateSelection", "start", std::to_string(start), "end", std::to_string(end));
...
void EditContext::updateCharacterBounds(uint32_t range_start, const HeapVector<Member<DOMRect>>& character_bounds)
{
    character_bounds_range_start_ = range_start;

    TRACE_EVENT1(
        "ime", "EditContext::updateCharacterBounds", "range_start, size", std::to_string(range_start) + ", " + std::to_string(character_bounds.size()));

    character_bounds_.clear();
    base::ranges::for_each(character_bounds, [this](const auto& bounds) {
        auto result_bounds = gfx::ToEnclosingRect(gfx::RectF(ClampToWithNaNTo0<float>(bounds->x()), ClampToWithNaNTo0<float>(bounds->y()),
            ClampToWithNaNTo0<float>(bounds->width()), ClampToWithNaNTo0<float>(bounds->height())));
        TRACE_EVENT1("ime", "EditContext::updateCharacterBounds", "charBounds", result_bounds.ToString());
...
{
    control_bounds_ = gfx::ToEnclosingRect(gfx::RectF(ClampToWithNaNTo0<float>(control_bounds->x()), ClampToWithNaNTo0<float>(control_bounds->y()),
        ClampToWithNaNTo0<float>(control_bounds->width()), ClampToWithNaNTo0<float>(control_bounds->height())));
    TRACE_EVENT1("ime", "EditContext::updateControlBounds", "control_bounds", control_bounds_.ToString());
}

void EditContext::updateSelectionBounds(DOMRect* selection_bounds)
{
    selection_bounds_ = gfx::ToEnclosingRect(gfx::RectF(ClampToWithNaNTo0<float>(selection_bounds->x()), ClampToWithNaNTo0<float>(selection_bounds->y()),
        ClampToWithNaNTo0<float>(selection_bounds->width()), ClampToWithNaNTo0<float>(selection_bounds->height())));
    TRACE_EVENT1("ime", "EditContext::updateSelectionBounds", "selection_bounds", selection_bounds_.ToString());
}

void EditContext::updateText(uint32_t start, uint32_t end, const String& new_text, ExceptionState& exception_state)
{
    TRACE_EVENT2("ime", "EditContext::updateText", "start, end", std::to_string(start) + ", " + std::to_string(end), "new_text", new_text);
    if (start > end) {
        std::swap(start, end);
    }
...
const HeapVector<Member<DOMRect>> EditContext::characterBounds()
{
    ...
    base::ranges::transform(character_bounds_, std::back_inserter(dom_rects),
        [](const auto& bound) { return DOMRect::Create(bound.x(), bound.y(), bound.width(), bound.height()); });
    return dom_rects;
}

void EditContext::GetLayoutBounds(gfx::Rect* control_bounds, gfx::Rect* selection_bounds)
{
    // EditContext's coordinates are in CSS pixels, which need to be converted to
    // physical pixels before return.
    *control_bounds = gfx::ScaleToEnclosingRect(control_bounds_, DomWindow()->GetFrame()->DevicePixelRatio());
    *selection_bounds = gfx::ScaleToEnclosingRect(selection_bounds_, DomWindow()->GetFrame()->DevicePixelRatio());

    TRACE_EVENT2("ime", "EditContext::GetLayoutBounds", "control", control_bounds->ToString(), "selection", selection_bounds->ToString());
}

bool EditContext::SetComposition(
    const WebString& text, const WebVector<ui::ImeTextSpan>& ime_text_spans, const WebRange& replacement_range, int selection_start, int selection_end)
{
    TRACE_EVENT2(
        "ime", "EditContext::SetComposition", "start, end", std::to_string(selection_start) + ", " + std::to_string(selection_end), "text", text.Utf8());
    ...
    if (actual_replacement_range.IsEmpty()) {
        ...
        // Otherwise, the current composition range will be replaced.
            ...
            actual_replacement_range = WebRange(composition_range_start_, composition_range_end_ - composition_range_start_);
        }
    }

    // Update the selection and buffer if the composition range has changed.
    String update_text(text);
    text_ = text_.Substring(0, actual_replacement_range.StartOffset()) + update_text + text_.Substring(actual_replacement_range.EndOffset());
    ...
    // inserted string.
    SetSelection(actual_replacement_range.StartOffset() + selection_start, actual_replacement_range.StartOffset() + selection_end);
    DispatchTextUpdateEvent(update_text, actual_replacement_range.StartOffset(), actual_replacement_range.EndOffset(), selection_start_, selection_end_);

    composition_range_start_ = actual_replacement_range.StartOffset();
    composition_range_end_ = actual_replacement_range.StartOffset() + update_text.length();
    DispatchTextFormatEvent(ime_text_spans);
    DispatchCharacterBoundsUpdateEvent(composition_range_start_, composition_range_end_);
    return true;
}
...
bool EditContext::SetCompositionFromExistingText(int composition_start, int composition_end, const WebVector<ui::ImeTextSpan>& ime_text_spans)
{
    TRACE_EVENT1(
        "ime", "EditContext::SetCompositionFromExistingText", "start, end", std::to_string(composition_start) + ", " + std::to_string(composition_end));
    ...
    composition_start = std::min(composition_start, static_cast<int>(text_.length()));
    composition_end = std::min(composition_end, static_cast<int>(text_.length()));
    String update_text(text_.Substring(composition_start, composition_end - composition_start));
    if (composition_range_start_ == 0 && composition_range_end_ == 0) {
        composition_range_start_ = composition_start;
        composition_range_end_ = composition_end;
    }

    DispatchTextUpdateEvent(update_text, composition_range_start_, composition_range_end_, selection_start_, selection_end_);
    DispatchTextFormatEvent(ime_text_spans);
    DispatchCharacterBoundsUpdateEvent(composition_range_start_, composition_range_end_);
    return true;
}
...
void EditContext::CancelComposition()
{
    ...
    text_ = text_.Substring(0, composition_range_start_) + text_.Substring(composition_range_end_);

    // Place the selection where the deleted composition had been
    SetSelection(composition_range_start_, composition_range_start_);
    DispatchTextUpdateEvent(g_empty_string, composition_range_start_, composition_range_end_, selection_start_, selection_end_);
...
bool EditContext::InsertText(const WebString& text)
{
    ...
    String update_text(text);
    text_ = text_.Substring(0, OrderedSelectionStart()) + update_text + text_.Substring(OrderedSelectionEnd());
    uint32_t update_range_start = OrderedSelectionStart();
    uint32_t update_range_end = OrderedSelectionEnd();
    SetSelection(OrderedSelectionStart() + update_text.length(), OrderedSelectionStart() + update_text.length());
    DispatchTextUpdateEvent(update_text, update_range_start, update_range_end, selection_start_, selection_end_);
    return true;
}
...
void EditContext::DeleteCurrentSelection()
{
    ...
    DispatchTextUpdateEvent(g_empty_string, OrderedSelectionStart(), OrderedSelectionEnd(), OrderedSelectionStart(), OrderedSelectionStart());
...
void EditContext::DeleteBackward()
{
    ...
    if (selection_start_ == selection_end_) {
        SetSelection(FindNextBoundaryOffset<BackwardGraphemeBoundaryStateMachine>(text_, selection_start_), selection_end_);
    }
...
{
    if (selection_start_ == selection_end_) {
        SetSelection(selection_start_, FindNextBoundaryOffset<ForwardGraphemeBoundaryStateMachine>(text_, selection_start_));
    }
...
void EditContext::DeleteWordBackward()
{
    if (selection_start_ == selection_end_) {
        ...
        SetSelection(FindNextWordBackward(text16bit.Span16(), selection_end_), selection_end_);
    }
...
void EditContext::DeleteWordForward()
{
    if (selection_start_ == selection_end_) {
        ...
        text16bit.Ensure16Bit();
        // TODO(shihken): implement platform behaviors when the spec is finalized.
        SetSelection(selection_start_, FindNextWordForward(text16bit.Span16(), selection_start_));
    }

    DeleteCurrentSelection();
}

bool EditContext::CommitText(
    const WebString& text, const WebVector<ui::ImeTextSpan>& ime_text_spans, const WebRange& replacement_range, int relative_caret_position)
{
    TRACE_EVENT2("ime", "EditContext::CommitText", "range, ralative_caret",
        "(" + std::to_string(replacement_range.StartOffset()) + "," + std::to_string(replacement_range.EndOffset()) + ")" + ", "
    ...
    text_ = text_.Substring(0, actual_replacement_range.StartOffset()) + update_text + text_.Substring(actual_replacement_range.EndOffset());
    SetSelection(actual_replacement_range.StartOffset() + update_text.length(), actual_replacement_range.StartOffset() + update_text.length());
...
bool EditContext::FinishComposingText(ConfirmCompositionBehavior selection_behavior)
{
    TRACE_EVENT0("ime", "EditContext::FinishComposingText");
    int text_length = 0;
    if (has_composition_) {
        String text = text_.Substring(composition_range_start_, composition_range_end_ - composition_range_start_);
...
{
    TRACE_EVENT1("ime", "EditContext::ExtendSelectionAndDelete", "before, after", std::to_string(before) + ", " + std::to_string(after));
    before = std::min(before, static_cast<int>(OrderedSelectionStart()));
    after = std::min(after, static_cast<int>(text_.length()));
    text_ = text_.Substring(0, OrderedSelectionStart() - before) + text_.Substring(OrderedSelectionEnd() + after);
    const uint32_t update_range_start = OrderedSelectionStart() - before;
    const uint32_t update_range_end = OrderedSelectionEnd() + after;
    SetSelection(OrderedSelectionStart() - before, OrderedSelectionStart() - before);
    DispatchTextUpdateEvent(g_empty_string, update_range_start, update_range_end, selection_start_, selection_end_);
}

void EditContext::DeleteSurroundingText(int before, int after)
{
    TRACE_EVENT1("ime", "EditContext::DeleteSurroundingText", "before, after", std::to_string(before) + ", " + std::to_string(after));
    const bool is_backwards_selection = selection_start_ > selection_end_;
    const uint32_t update_range_start = std::max(OrderedSelectionStart() - before, 0U);
    const uint32_t update_range_end = std::min(OrderedSelectionEnd() + after, text_.length());
    SetSelection(update_range_start, OrderedSelectionEnd() - (OrderedSelectionStart() - update_range_start));
    CHECK_GE(selection_end_, selection_start_);
    text_ = text_.Substring(0, update_range_start) + text_.Substring(selection_start_, selection_end_ - selection_start_) + text_.Substring(update_range_end);
    String update_event_text(text_.Substring(selection_start_, selection_end_ - selection_start_));

    if (is_backwards_selection) {
        SetSelection(selection_end_, selection_start_);
    }

    DispatchTextUpdateEvent(update_event_text, update_range_start, update_range_end, selection_start_, selection_end_);
}

void EditContext::SetSelection(int start, int end, bool dispatch_text_update_event)
{
    TRACE_EVENT1("ime", "EditContext::SetSelection", "start, end", std::to_string(start) + ", " + std::to_string(end));
    ...
    if (DomWindow() && DomWindow()->GetFrame()) {
        DomWindow()->GetFrame()->Client()->DidChangeSelection(
            /*is_selection_empty=*/selection_start_ == selection_end_, blink::SyncCondition::kNotForced);
    }

    if (dispatch_text_update_event) {
        DispatchTextUpdateEvent(g_empty_string, /*update_range_start=*/0,
            /*update_range_end=*/0, selection_start_, selection_end_);
    }
}
...
void EditContext::AttachElement(HTMLElement* element_to_attach)
{
    ...
    CHECK(attached_elements_.empty()) << "An EditContext can be only be associated with a single element";
...
{
    auto it = base::ranges::find(attached_elements_, element_to_detach, &Member<HTMLElement>::Get);
...
WebTextInputInfo EditContext::TextInputInfo()
{
    ...
    info.virtual_keyboard_policy = GetInputMethodController().VirtualKeyboardPolicyOfFocusedElement();
    info.value = text();
...
{
    return WebRange(composition_range_start_, composition_range_end_ - composition_range_start_);
}
...
bool EditContext::GetCompositionCharacterBounds(WebVector<gfx::Rect>& bounds)
{
    ...
    TRACE_EVENT1("ime", "EditContext::GetCompositionCharacterBounds", "size", std::to_string(character_bounds_.size()));

    bounds.clear();
    base::ranges::for_each(character_bounds_, [&bounds, this](auto& bound_in_css_pixels) {
        // EditContext's coordinates are in CSS pixels, which need to be
        // converted to physical pixels before return.
        auto result_bounds = gfx::ScaleToEnclosingRect(bound_in_css_pixels, DomWindow()->GetFrame()->DevicePixelRatio());
        bounds.push_back(result_bounds);
        TRACE_EVENT1("ime", "EditContext::GetCompositionCharacterBounds", "charBounds", result_bounds.ToString());
    });

    return true;
}

bool EditContext::FirstRectForCharacterRange(uint32_t location, uint32_t length, gfx::Rect& rect_in_viewport)
{
    ...
    if (HasValidCompositionBounds()) {
        ...
        if (base::saturated_cast<int>(location) >= range.StartOffset() && base::saturated_cast<int>(location + length) <= range.EndOffset()) {
            ...
            if (length == 0) {
                if (start_in_composition == character_bounds_.size()) {
                    ...
                    rect_in_css_pixels = gfx::Rect(character_bounds_[start_in_composition - 1].right(), character_bounds_[start_in_composition - 1].y(), 0,
                        character_bounds_[start_in_composition - 1].height());
                } else {
                    // Zero-width rect before the next character in the composition range
                    rect_in_css_pixels = gfx::Rect(character_bounds_[start_in_composition].x(), character_bounds_[start_in_composition].y(), 0,
    ...
    if (found_rect) {
        ...
        rect_in_viewport = gfx::ScaleToEnclosingRect(rect_in_css_pixels, DomWindow()->GetFrame()->DevicePixelRatio());
    }
...
bool EditContext::HasValidCompositionBounds() const
{
    ...
    return (base::saturated_cast<int>(character_bounds_.size()) == composition_range.length());
}

WebRange EditContext::GetSelectionOffsets() const
{
    return WebRange(OrderedSelectionStart(), OrderedSelectionEnd() - OrderedSelectionStart());
}
