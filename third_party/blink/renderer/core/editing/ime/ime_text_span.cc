...
ImeTextSpan::ImeTextSpan(Type type, wtf_size_t start_offset, wtf_size_t end_offset, const Color& underline_color, ui::mojom::ImeTextSpanThickness thickness,
    ui::mojom::ImeTextSpanUnderlineStyle underline_style, const Color& text_color, const Color& background_color, const Color& suggestion_highlight_color,
    bool remove_on_finish_composing, bool interim_char_selection, const Vector<String>& suggestions)
    : type_(type)
    ...
    , suggestions_(suggestions)
{
    ...
    start_offset_ = std::min(start_offset, std::numeric_limits<wtf_size_t>::max() - 1u);
    end_offset_ = std::max(start_offset_ + 1u, end_offset);
}

namespace {

Vector<String> ConvertStdVectorOfStdStringsToVectorOfStrings(const std::vector<std::string>& input)
{
...
std::vector<std::string> ConvertVectorOfStringsToStdVectorOfStdStrings(const Vector<String>& input)
{
...
ui::mojom::ImeTextSpanThickness ConvertUiThicknessToThickness(ui::ImeTextSpan::Thickness thickness)
{
    switch (thickness) {
...
ui::mojom::ImeTextSpanUnderlineStyle ConvertUiUnderlineToUnderline(ui::ImeTextSpan::UnderlineStyle underline)
{
    switch (underline) {
...
ImeTextSpan::ImeTextSpan(const ui::ImeTextSpan& ime_text_span)
    : ImeTextSpan(ConvertUiTypeToType(ime_text_span.type), base::checked_cast<wtf_size_t>(ime_text_span.start_offset),
        base::checked_cast<wtf_size_t>(ime_text_span.end_offset), Color::FromSkColor(ime_text_span.underline_color),
        ConvertUiThicknessToThickness(ime_text_span.thickness), ConvertUiUnderlineToUnderline(ime_text_span.underline_style),
        Color::FromSkColor(ime_text_span.text_color), Color::FromSkColor(ime_text_span.background_color),
        Color::FromSkColor(ime_text_span.suggestion_highlight_color), ime_text_span.remove_on_finish_composing, ime_text_span.interim_char_selection,
        ConvertStdVectorOfStdStringsToVectorOfStrings(ime_text_span.suggestions))
{
}

ui::ImeTextSpan ImeTextSpan::ToUiImeTextSpan()
{
    auto span = ui::ImeTextSpan(ConvertImeTextSpanTypeToUiType(GetType()), StartOffset(), EndOffset());
    span.suggestions = ConvertVectorOfStringsToStdVectorOfStdStrings(Suggestions());
    return span;
}
