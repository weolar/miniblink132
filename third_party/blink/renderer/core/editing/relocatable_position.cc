...
RelocatablePosition::RelocatablePosition(const Position& position)
    : range_(position.IsNotNull() ? MakeGarbageCollected<Range>(*position.GetDocument(), position, position) : nullptr)
...
Position RelocatablePosition::GetPosition() const
{
    ...
    if (original_position_.IsConnected() && original_position_.IsEquivalent(position))
