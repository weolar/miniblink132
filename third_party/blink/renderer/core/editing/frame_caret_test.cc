...
TEST_F(FrameCaretTest, MAYBE_BlinkAfterTyping)
{
    ...
    scoped_refptr<scheduler::FakeTaskRunner> task_runner = base::MakeRefCounted<scheduler::FakeTaskRunner>();
    task_runner->SetTime(0);
    caret.RecreateCaretBlinkTimerForTesting(task_runner.get(), task_runner->GetMockTickClock());
    ...
    EXPECT_TRUE(IsVisibleIfActive(caret)) << "Initially a caret should be in visible cycle.";
    ...
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(IsVisibleIfActive(caret)) << "The caret should be in visible cycle just after a typing command.";

    task_runner->AdvanceTimeAndRun(kInterval - 1);
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(IsVisibleIfActive(caret)) << "The typing command reset the timer. The caret is still visible.";

    task_runner->AdvanceTimeAndRun(1);
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(IsVisibleIfActive(caret)) << "The caret should blink after the typing command.";
}
...
TEST_F(FrameCaretTest, ShouldNotBlinkWhenSelectionLooseFocus)
{
    ...
    GetDocument().GetPage()->GetFocusController().SetActive(true);
    GetDocument().GetPage()->GetFocusController().SetFocused(true);
    GetDocument().body()->setInnerHTML("<div id='outer' tabindex='-1'>"
    ...
    EXPECT_EQ(selection.Anchor(), Position::FirstPositionInNode(*(input->firstChild())));
...
{
    FrameCaret& caret = Selection().FrameCaretForTesting();
    Selection().SetSelection(SetSelectionTextToBody("<div>a|b</div>"), SetSelectionOptions());
...
TEST_F(FrameCaretTest, CaretAnimationManual)
{
    ...
    editor->SetInlineStyleProperty(CSSPropertyID::kCaretAnimation, CSSValueID::kAuto);
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(FrameCaretTest, CaretAnimationInvalidation)
{
    ...
    editor->SetInlineStyleProperty(CSSPropertyID::kCaretAnimation, CSSValueID::kManual);
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(IsVisibleIfActive(caret)) << "The caret turns on when manual again.";
    ...
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(IsVisibleIfActive(caret)) << "The caret initially on when auto again.";
