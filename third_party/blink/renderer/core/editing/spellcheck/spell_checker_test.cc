...
TEST_F(SpellCheckerTest, AdvancedToNextMisspellingWrapSearchNoCrash)
{
    ...
    div->Focus();
    Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position::LastPositionInNode(*div)).Build(), SetSelectionOptions());
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(SpellCheckerTest, SpellCheckDoesNotCauseUpdateLayout)
{
    ...
    auto* input = To<HTMLInputElement>(GetDocument().QuerySelector(AtomicString("input")));
    input->Focus();
    input->SetValue("Hello, input field");
    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kTest);

    Position new_position(input->InnerEditorElement()->firstChild(), 3);
    GetDocument().GetFrame()->Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(new_position).Build(), SetSelectionOptions());
...
TEST_F(SpellCheckerTest, MarkAndReplaceForHandlesMultipleReplacements)
{
    ...
    EphemeralRange range_to_check = EphemeralRange(Position(text, 0), Position(text, 8));
    ...
    GetDocument().GetFrame()->GetSpellChecker().MarkAndReplaceFor(request, Vector<TextCheckingResult>({ result }));
    ...
    EXPECT_EQ("spellcheck\nspillchuck", To<SpellCheckMarker>(GetDocument().Markers().Markers()[0].Get())->Description());
}
...
TEST_F(SpellCheckerTest, GetSpellCheckMarkerUnderSelection_FirstCharSelected)
{
    ...
    GetDocument().GetFrame()->Selection().SetSelection(
        SelectionInDOMTree::Builder().SetBaseAndExtent(Position(text, 0), Position(text, 1)).Build(), SetSelectionOptions());

    DocumentMarkerGroup* result = GetDocument().GetFrame()->GetSpellChecker().GetSpellCheckMarkerGroupUnderSelection();
...
TEST_F(SpellCheckerTest, GetSpellCheckMarkerUnderSelection_LastCharSelected)
{
    ...
    GetDocument().GetFrame()->Selection().SetSelection(
        SelectionInDOMTree::Builder().SetBaseAndExtent(Position(text, 7), Position(text, 8)).Build(), SetSelectionOptions());
...
TEST_F(SpellCheckerTest, GetSpellCheckMarkerUnderSelection_SingleCharWordSelected)
{
    SetBodyContent("<div contenteditable>"
                   "s"
                   "</div>");
    ...
    GetDocument().Markers().AddSpellingMarker(EphemeralRange(Position(text, 0), Position(text, 1)));
...
TEST_F(SpellCheckerTest, GetSpellCheckMarkerUnderSelection_CaretLeftOfSingleCharWord)
{
    SetBodyContent("<div contenteditable>"
                   "s"
                   "</div>");
...
TEST_F(SpellCheckerTest, GetSpellCheckMarkerUnderSelection_CaretRightOfSingleCharWord)
{
    SetBodyContent("<div contenteditable>"
                   "s"
                   "</div>");
...
TEST_F(SpellCheckerTest, GetSpellCheckMarkerUnderSelection_CaretLeftOfMultiCharWord)
{
    SetBodyContent("<div contenteditable>"
                   "spllchck"
                   "</div>");
...
TEST_F(SpellCheckerTest, GetSpellCheckMarkerUnderSelection_CaretRightOfMultiCharWord)
{
    SetBodyContent("<div contenteditable>"
                   "spllchck"
                   "</div>");
    ...
    GetDocument().GetFrame()->Selection().SetSelection(
        SelectionInDOMTree::Builder().SetBaseAndExtent(Position(text, 8), Position(text, 8)).Build(), SetSelectionOptions());
...
TEST_F(SpellCheckerTest, GetSpellCheckMarkerUnderSelection_CaretMiddleOfWord)
{
    ...
    GetDocument().GetFrame()->Selection().SetSelection(
        SelectionInDOMTree::Builder().SetBaseAndExtent(Position(text, 4), Position(text, 4)).Build(), SetSelectionOptions());
...
TEST_F(SpellCheckerTest, GetSpellCheckMarkerUnderSelection_CaretOneCharLeftOfMisspelling)
{
    SetBodyContent("<div contenteditable>"
                   "a spllchck"
                   "</div>");
    ...
    GetDocument().Markers().AddSpellingMarker(EphemeralRange(Position(text, 2), Position(text, 10)));
...
TEST_F(SpellCheckerTest, GetSpellCheckMarkerUnderSelection_CaretOneCharRightOfMisspelling)
{
    SetBodyContent("<div contenteditable>"
                   "spllchck a"
                   "</div>");
    ...
    GetDocument().GetFrame()->Selection().SetSelection(
        SelectionInDOMTree::Builder().SetBaseAndExtent(Position(text, 9), Position(text, 9)).Build(), SetSelectionOptions());
...
TEST_F(SpellCheckerTest, GetSpellCheckMarkerUnderSelection_MultiNodeMisspell)
{
    ...
    GetDocument().Markers().AddSpellingMarker(EphemeralRange(Position(first_text, 0), Position(third_text, 3)));

    GetDocument().GetFrame()->Selection().SetSelection(
        SelectionInDOMTree::Builder().SetBaseAndExtent(Position(second_text, 1), Position(second_text, 1)).Build(), SetSelectionOptions());

    DocumentMarkerGroup* result = GetDocument().GetFrame()->GetSpellChecker().GetSpellCheckMarkerGroupUnderSelection();
    ASSERT_NE(nullptr, result);
    const DocumentMarker* first_marker = result->GetMarkerForText(To<Text>(first_text));
    const DocumentMarker* second_marker = result->GetMarkerForText(To<Text>(second_text));
    const DocumentMarker* third_marker = result->GetMarkerForText(To<Text>(third_text));
...
TEST_F(SpellCheckerTest, PasswordFieldsAreIgnored)
{
    ...
    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kTest);
    EXPECT_TRUE(SpellChecker::IsSpellCheckingEnabledAt(Position(input->InnerEditorElement()->firstChild(), 0)));

    // But if this turns into a password field, this disables spellchecking.
    // input->setType(input_type_names::kPassword);
    input->setType(input_type_names::kPassword);
    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kTest);
    EXPECT_FALSE(SpellChecker::IsSpellCheckingEnabledAt(Position(input->InnerEditorElement()->firstChild(), 0)));
