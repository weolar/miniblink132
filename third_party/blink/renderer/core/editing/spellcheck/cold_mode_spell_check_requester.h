...
class ColdModeSpellCheckRequester : public GarbageCollected<ColdModeSpellCheckRequester> {
public:
    explicit ColdModeSpellCheckRequester(LocalDOMWindow&);
    ColdModeSpellCheckRequester(const ColdModeSpellCheckRequester&) = delete;
    ColdModeSpellCheckRequester& operator=(const ColdModeSpellCheckRequester&) = delete;
...
private:
    ...
    CheckingType AccumulateTextDeltaAndComputeCheckingType(const Element& element_to_check);
    ...
    HeapHashMap<WeakMember<const Element>, FullyCheckedEditableEntry> fully_checked_root_editables_;
