...
EphemeralRange CalculateHotModeCheckingRange(const Element& editable, const Position& position)
{
    ...
    const EphemeralRange& paragraph_range = ExpandToParagraphBoundary(EphemeralRange(position));
...
HotModeSpellCheckRequester::HotModeSpellCheckRequester(SpellCheckRequester& requester)
...
void HotModeSpellCheckRequester::CheckSpellingAt(const Position& position)
{
    ...
    if (!root_editable->IsSpellCheckingEnabled() && !SpellChecker::IsSpellCheckingEnabledAt(position)) {
        return;
    }

    const EphemeralRange& current_word = CurrentWordIfTypingInPartialWord(*root_editable);
    if (current_word.IsNotNull()) {
        root_editable->GetDocument().Markers().RemoveMarkersInRange(current_word, DocumentMarker::MarkerTypes::Misspelling());
        return;
    }

    const EphemeralRange& checking_range = CalculateHotModeCheckingRange(*root_editable, position);
