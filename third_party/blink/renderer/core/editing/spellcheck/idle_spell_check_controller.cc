...
constexpr base::TimeDelta kConsecutiveColdModeTimerInterval = base::Milliseconds(200);
...
private:
    void invoke(IdleDeadline* deadline) final
    {
        controller_->Invoke(deadline);
    }
...
IdleSpellCheckController::IdleSpellCheckController(LocalDOMWindow& window, SpellCheckRequester& requester)
    : ExecutionContextLifecycleObserver(&window)
    , idle_callback_handle_(kInvalidHandle)
    , cold_mode_requester_(MakeGarbageCollected<ColdModeSpellCheckRequester>(window))
...
void IdleSpellCheckController::DisposeIdleCallback()
{
    if (idle_callback_handle_ != kInvalidHandle && GetExecutionContext()) {
        ScriptedIdleTaskController::From(*GetExecutionContext()).CancelCallback(idle_callback_handle_);
    }
    idle_callback_handle_ = kInvalidHandle;
}
...
void IdleSpellCheckController::SetNeedsInvocation()
{
    ...
    idle_callback_handle_
        = ScriptedIdleTaskController::From(CHECK_DEREF(GetExecutionContext())).RegisterCallback(MakeGarbageCollected<IdleCallback>(this), options);
...
{
    DCHECK(IsSpellCheckingEnabled());
    if (state_ != State::kInactive && state_ != State::kInHotModeInvocation && state_ != State::kInColdModeInvocation)
        return;

    DCHECK(!cold_mode_timer_.IsActive());
    base::TimeDelta interval = state_ == State::kInColdModeInvocation ? kConsecutiveColdModeTimerInterval : kColdModeTimerInterval;
    cold_mode_timer_ = PostDelayedCancellableTask(*GetWindow().GetTaskRunner(TaskType::kInternalDefault), FROM_HERE,
        WTF::BindOnce(&IdleSpellCheckController::ColdModeTimerFired, WrapPersistent(this)), interval);
...
void IdleSpellCheckController::ColdModeTimerFired()
{
    ...
    idle_callback_handle_ = ScriptedIdleTaskController::From(CHECK_DEREF(GetExecutionContext()))
                                .RegisterCallback(MakeGarbageCollected<IdleCallback>(this), IdleRequestOptions::Create());
    state_ = State::kColdModeRequested;
}

bool IdleSpellCheckController::NeedsHotModeCheckingUnderCurrentSelection() const
{
    if (needs_invocation_for_changed_contents_ || needs_invocation_for_changed_enablement_) {
        return true;
    }
    ...
    const Position& position = GetWindow().GetFrame()->Selection().GetSelectionInDOMTree().Focus();
...
void IdleSpellCheckController::HotModeInvocation(IdleDeadline* deadline)
{
    ...
    if (NeedsHotModeCheckingUnderCurrentSelection()) {
        requester.CheckSpellingAt(GetWindow().GetFrame()->Selection().GetSelectionInDOMTree().Focus());
    }

    const uint64_t watermark = last_processed_undo_step_sequence_;
    for (const UndoStep* step : GetWindow().GetFrame()->GetEditor().GetUndoStack().UndoSteps()) {
        if (step->SequenceNumber() <= watermark)
            break;
        last_processed_undo_step_sequence_ = std::max(step->SequenceNumber(), last_processed_undo_step_sequence_);
...
void IdleSpellCheckController::Invoke(IdleDeadline* deadline)
{
    ...
    if (state_ == State::kHotModeRequested) {
        ...
        static auto* state_data = base::debug::AllocateCrashKeyString("spellchecker-state-on-invocation", base::debug::CrashKeySize::Size32);
...
void IdleSpellCheckController::ForceInvocationForTesting()
{
    ...
    auto* deadline = MakeGarbageCollected<IdleDeadline>(
        base::TimeTicks::Now() + kIdleSpellcheckTestTimeout, cross_origin_isolated_capability, IdleDeadline::CallbackType::kCalledWhenIdle);
...
void IdleSpellCheckController::SetSpellCheckingDisabled(const Element& element)
{
