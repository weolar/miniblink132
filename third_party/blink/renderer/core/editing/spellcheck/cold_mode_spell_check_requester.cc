...
{
    const EphemeralRange& full_range = EphemeralRange::RangeOfContents(root_editable);
...
bool ColdModeSpellCheckRequester::FullyCheckedCurrentRootEditable() const
{
    ...
    return !root_editable_ || !remaining_check_range_ || remaining_check_range_->collapsed() || !remaining_check_range_->IsConnected()
        || !root_editable_->contains(remaining_check_range_->commonAncestorContainer());
}

SpellCheckRequester& ColdModeSpellCheckRequester::GetSpellCheckRequester() const
{
...
{
    const Position position = window_->GetFrame()->Selection().GetSelectionInDOMTree().Focus();
    if (position.IsNull())
        return nullptr;
    ...
    if (!element->IsSpellCheckingEnabled() || !SpellChecker::IsSpellCheckingEnabledAt(position))
        return nullptr;
...
void ColdModeSpellCheckRequester::RequestFullChecking(const Element& element_to_check, IdleDeadline* deadline)
{
    ...
    if (root_editable_ != &element_to_check) {
        ...
        remaining_check_range_->selectNodeContents(const_cast<Element*>(root_editable_.Get()), ASSERT_NO_EXCEPTION);
    }
...
void ColdModeSpellCheckRequester::SetHasFullyCheckedCurrentRootEditable()
{
    ...
    fully_checked_root_editables_.Set(
        root_editable_, FullyCheckedEditableEntry { TotalTextLength(*root_editable_), 0, root_editable_->GetDocument().DomTreeVersion() });
    last_chunk_index_ = kInvalidChunkIndex;
    if (!remaining_check_range_)
        return;
    remaining_check_range_->Dispose();
...
bool ColdModeSpellCheckRequester::RequestCheckingForNextChunk()
{
    ...
    const int remaining_length = TextIterator::RangeLength(remaining_range,
    ...
    const Position chunk_end = CalculateCharacterSubrange(remaining_range, 0, std::min(remaining_length, kColdModeFullCheckingChunkSize)).EndPosition();
    ...
    const Position check_end = extended_end.IsNull() || extended_end < chunk_end ? chunk_end : std::min(extended_end, remaining_range.EndPosition());
...
ColdModeSpellCheckRequester::CheckingType ColdModeSpellCheckRequester::AccumulateTextDeltaAndComputeCheckingType(const Element& element_to_check)
{
    ...
    iter->value.previous_checked_dom_tree_version = element_to_check.GetDocument().DomTreeVersion();
    ...
    int delta = std::abs(current_text_length - iter->value.previous_checked_length);
...
void ColdModeSpellCheckRequester::RequestLocalChecking(const Element& element_to_check)
{
    TRACE_EVENT0("blink", "ColdModeSpellCheckRequester::RequestLocalChecking");

    const EphemeralRange& full_range = EphemeralRange::RangeOfContents(element_to_check);
    const Position position = window_->GetFrame()->Selection().GetSelectionInDOMTree().Focus();
    DCHECK(position.IsNotNull());

    TextIteratorBehavior behavior
        = TextIteratorBehavior::Builder().SetEmitsObjectReplacementCharacter(true).SetEmitsPunctuationForReplacedElements(true).Build();
    BackwardsCharacterIterator backward_iterator(EphemeralRange(full_range.StartPosition(), position), behavior);
    ...
    CharacterIterator forward_iterator(position, full_range.EndPosition(), behavior);
    ...
    EphemeralRange checking_range = ExpandRangeToSentenceBoundary(EphemeralRange(chunk_start, chunk_end));
