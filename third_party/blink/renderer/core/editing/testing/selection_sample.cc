...
void ConvertTemplatesToShadowRoots(HTMLElement& element)
{
    ...
    // TEMPLATE element.
    HTMLCollection* const templates = element.getElementsByTagName(AtomicString("template"));
    ...
    for (Element* template_element : template_vector) {
        const AtomicString& data_mode = template_element->getAttribute(AtomicString("data-mode"));
        ...
        ShadowRoot& shadow_root = parent->AttachShadowRootForTesting(ShadowRootMode::kOpen);
        Node* const fragment = document->importNode(To<HTMLTemplateElement>(template_element)->content(), true, ASSERT_NO_EXCEPTION);
...
public:
    ...
    SelectionInDOMTree SetSelectionText(HTMLElement* element, const std::string& selection_text)
    {
        ...
        if (anchor_node_ && focus_node_) {
            return typename SelectionInDOMTree::Builder().Collapse(Position(anchor_node_, anchor_offset_)).Extend(Position(focus_node_, focus_offset_)).Build();
        }
        DCHECK(focus_node_) << "Need just '|', or '^' and '|'";
        return typename SelectionInDOMTree::Builder().Collapse(Position(focus_node_, focus_offset_)).Build();
    }
...
private:
    ...
    {
        DCHECK(!anchor_node_) << "Found more than one '^' in " << *anchor_node_ << " and " << *node;
        anchor_node_ = node;
    ...
    {
        DCHECK(!focus_node_) << "Found more than one '|' in " << *focus_node_ << " and " << *node;
        focus_node_ = node;
...
template <typename Strategy> class Serializer final {
    STACK_ALLOCATED();
...
private:
    void HandleCharacterData(const CharacterData& node)
    {
        ...
        const int anchor_offset = selection_.Anchor().ComputeOffsetInContainerNode();
        ...
        if (anchor_node == node && focus_node == node) {
            ...
            if (anchor_offset < focus_offset) {
                builder_.Append(text.Left(anchor_offset));
                builder_.Append('^');
                builder_.Append(text.Substring(anchor_offset, focus_offset - anchor_offset));
                builder_.Append('|');
            ...
            builder_.Append('|');
            builder_.Append(text.Substring(focus_offset, anchor_offset - focus_offset));
            builder_.Append('^');
            builder_.Append(text.Substring(anchor_offset));
            return;
        }
    ...
    void HandleAttributes(const Element& element)
    {
        ...
        std::sort(attributes.begin(), attributes.end(), [](const Attribute* attribute1, const Attribute* attribute2) {
            return CodeUnitCompareLessThan(attribute1->GetName().ToString(), attribute2->GetName().ToString());
        });
    ...
    void HandleNode(const Node& node)
    {
        ...
        if (auto* processing_instruction_node = DynamicTo<ProcessingInstruction>(node)) {
            builder_.Append("<?");
...
void SelectionSample::ConvertTemplatesToShadowRootsForTesring(HTMLElement& element)
{
    ConvertTemplatesToShadowRoots(element);
}

SelectionInDOMTree SelectionSample::SetSelectionText(HTMLElement* element, const std::string& selection_text)
{
    SelectionInDOMTree selection = Parser().SetSelectionText(element, selection_text);
...
std::string SelectionSample::GetSelectionText(const ContainerNode& root, const SelectionInDOMTree& selection)
{
    return Serializer<EditingStrategy>(selection).Serialize(root);
}

std::string SelectionSample::GetSelectionTextInFlatTree(const ContainerNode& root, const SelectionInFlatTree& selection)
{
