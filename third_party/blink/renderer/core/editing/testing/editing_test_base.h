...
public:
    static ShadowRoot* CreateShadowRootForElementWithIDAndSetInnerHTML(TreeScope&, const char* host_element_id, const char* shadow_root_content);
...
protected:
    ...
    // |FrameSelection|.
    SelectionInDOMTree SetSelectionText(HTMLElement*, const std::string& selection_text);
    ...
    // Returns selection text for child nodes of BODY with specified
    // |SelectionInFlatTree|.
    std::string GetSelectionTextInFlatTreeFromBody(const SelectionInFlatTree&) const;

    ShadowRoot* SetShadowContent(const char* shadow_content, const char* shadow_host_id);
};
