...
TEST_F(ApplyStyleCommandTest, RemoveRedundantBlocksWithStarEditableStyle)
{
    ...
    frame->Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position(li, PositionAnchorType::kBeforeAnchor)).Build(), SetSelectionOptions());
    ...
    MakeGarbageCollected<ApplyStyleCommand>(
        GetDocument(), MakeGarbageCollected<EditingStyle>(style), InputEvent::InputType::kFormatJustifyCenter, ApplyStyleCommand::kForceBlockProperties)
        ->Apply();
    // Shouldn't crash.
}
...
{
    Selection().SetSelection(SetSelectionTextToBody("<div contenteditable>^<div></div>a<span></span>|</div>"), SetSelectionOptions());
    ...
    MakeGarbageCollected<ApplyStyleCommand>(GetDocument(), MakeGarbageCollected<EditingStyle>(style), InputEvent::InputType::kNone)->Apply();
    EXPECT_EQ("<div contenteditable><div></div><span>^a|</span></div>", GetSelectionTextFromBody());
}
...
TEST_F(ApplyStyleCommandTest, JustifyRightWithSVGForeignObject)
{
    ...
    Selection().SetSelection(SetSelectionTextToBody("<svg>"
    ...
    style->ParseAndSetProperty(CSSPropertyID::kTextAlign, "right",
        /* important */ false, GetFrame().DomWindow()->GetSecureContextMode());
    MakeGarbageCollected<ApplyStyleCommand>(
        GetDocument(), MakeGarbageCollected<EditingStyle>(style), InputEvent::InputType::kFormatJustifyRight, ApplyStyleCommand::kForceBlockProperties)
        ->Apply();
    EXPECT_EQ("<svg>"
              "<foreignObject>|1"
              "</foreignObject>"
...
{
    GetDocument().setDesignMode("on");
    Selection().SetSelection(SetSelectionTextToBody("|x<div contenteditable=false></div>"), SetSelectionOptions());
    ...
    EXPECT_EQ("<div style=\"text-align: center;\">|x</div><div "
...
TEST_F(ApplyStyleCommandTest, StyledInlineElementIsActuallyABlock)
{
    ...
    Selection().SetSelection(SetSelectionTextToBody("^<sub>a</sub>|"), SetSelectionOptions());
    GetDocument().setDesignMode("on");
    Element* styled_inline_element = GetDocument().QuerySelector(AtomicString("sub"));
    bool remove_only = true;
    // Shouldn't crash.
    MakeGarbageCollected<ApplyStyleCommand>(styled_inline_element, remove_only)->Apply();
...
TEST_F(ApplyStyleCommandTest, ItalicCrossingIgnoredContentBoundary)
{
    ...
    Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position(body, 0)).Extend(Position(option, 0)).Build(), SetSelectionOptions());

    auto* style = MakeGarbageCollected<MutableCSSPropertyValueSet>(kUASheetMode);
    style->ParseAndSetProperty(CSSPropertyID::kFontStyle, "italic",
        /* important */ false, GetFrame().DomWindow()->GetSecureContextMode());
    MakeGarbageCollected<ApplyStyleCommand>(GetDocument(), MakeGarbageCollected<EditingStyle>(style), InputEvent::InputType::kFormatItalic)->Apply();

#if BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_IOS)
    EXPECT_EQ("|a<select multiple><option></option></select>b", GetSelectionTextFromBody());
#else
    EXPECT_EQ("<i>^a<select multiple><option>|</option></select></i>b", GetSelectionTextFromBody());
...
TEST_F(ApplyStyleCommandTest, RemoveEmptyItalic)
{
    ...
    Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position(i, 0)).Build(), SetSelectionOptions());
    auto* command = MakeGarbageCollected<ApplyStyleCommand>(GetDocument(), MakeGarbageCollected<EditingStyle>(div), InputEvent::InputType::kFormatRemove);
