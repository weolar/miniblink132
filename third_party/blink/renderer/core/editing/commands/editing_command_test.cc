...
static_assert(std::size(kCommandNameEntries) + 1 == static_cast<size_t>(EditingCommandType::kNumberOfCommandTypes), "must test all valid EditingCommandType");
...
TEST_F(EditingCommandTest, EditorCommandOrder)
{
    for (size_t i = 1; i < std::size(kCommandNameEntries); ++i) {
        EXPECT_GT(0, WTF::CodeUnitCompareIgnoringASCIICase(kCommandNameEntries[i - 1].name, kCommandNameEntries[i].name))
            << "EDITOR_COMMAND_MAP must be case-folding ordered. Incorrect index:" << i;
    }
}
...
TEST_F(EditingCommandTest, CreateCommandFromStringCaseFolding)
{
    Editor& dummy_editor = GetDocument().GetFrame()->GetEditor();
    for (const auto& entry : kCommandNameEntries) {
        const EditorCommand lower_name_command = dummy_editor.CreateCommand(String(entry.name).LowerASCII());
        EXPECT_EQ(static_cast<int>(entry.type), lower_name_command.IdForHistogram()) << entry.name;
        const EditorCommand upper_name_command = dummy_editor.CreateCommand(String(entry.name).UpperASCII());
        EXPECT_EQ(static_cast<int>(entry.type), upper_name_command.IdForHistogram()) << entry.name;
    }
}
...
TEST_F(EditingCommandTest, EnabledVisibleSelection)
{
    ...
    const EditorCommand command = editor.CreateCommand("MoveRightAndModifySelection");
    Selection().SetSelection(SetSelectionTextToBody("<div contenteditable>a|b<div>"), SetSelectionOptions());
    Element* div = GetDocument().QuerySelector(AtomicString("div"));
    GetDocument().SetFocusedElement(div, FocusParams(SelectionBehaviorOnFocus::kNone, mojom::blink::FocusType::kNone, nullptr));
...
TEST_F(EditingCommandTest, DeleteSoftLineBackwardTargetRanges)
{
    ...
    Selection().SetSelection(SetSelectionTextToBody("<div contenteditable>abcdef<br>123|<div>"), SetSelectionOptions());
    Element* div = GetDocument().QuerySelector(AtomicString("div"));
    GetDocument().SetFocusedElement(div, FocusParams(SelectionBehaviorOnFocus::kNone, mojom::blink::FocusType::kNone, nullptr));
    EXPECT_TRUE(command.IsEnabled());
