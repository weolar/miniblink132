...
public:
    ...
    const SelectionForUndoStep& Build() const
    {
        return selection_;
    }

    // |anchor| and |focus| can be disconnected.
    Builder& SetAnchorAndFocusAsBackwardSelection(const Position& anchor, const Position& focus);

    // |anchor| and |focus| can be disconnected.
    Builder& SetAnchorAndFocusAsForwardSelection(const Position& anchor, const Position& focus);
