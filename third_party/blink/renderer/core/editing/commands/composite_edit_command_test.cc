...
public:
    ...
    void InsertNodeBefore(Node*, Node* ref_child, EditingState*, ShouldAssumeContentIsAlwaysEditable = kDoNotAssumeContentIsAlwaysEditable);
    void InsertNodeAfter(Node*, Node*, EditingState*);

    void MoveParagraphContentsToNewBlockIfNecessary(const Position&, EditingState*);
    void MoveParagraphs(const VisiblePosition& start_of_paragraph_to_move, const VisiblePosition& end_of_paragraph_to_move, const VisiblePosition& destination,
...
{
    CompositeEditCommand::InsertNodeBefore(insert_child, ref_child, editing_state, should_assume_content_is_always_editable);
}

void SampleCommand::InsertNodeAfter(Node* insert_child, Node* ref_child, EditingState* editing_state)
{
    CompositeEditCommand::InsertNodeAfter(insert_child, ref_child, editing_state);
}

void SampleCommand::MoveParagraphContentsToNewBlockIfNecessary(const Position& position, EditingState* editing_state)
{
    CompositeEditCommand::MoveParagraphContentsToNewBlockIfNecessary(position, editing_state);
}

void SampleCommand::MoveParagraphs(const VisiblePosition& start_of_paragraph_to_move, const VisiblePosition& end_of_paragraph_to_move,
    const VisiblePosition& destination, EditingState* editing_state)
{
    CompositeEditCommand::MoveParagraphs(start_of_paragraph_to_move, end_of_paragraph_to_move, destination, editing_state);
}
...
TEST_F(CompositeEditCommandTest, insertNodeBeforeDisconnectedNode)
{
    ...
    sample.InsertNodeBefore(insert_child, ref_child, &editing_state);
    EXPECT_FALSE(editing_state.IsAborted());
    EXPECT_EQ("<b></b>", div->innerHTML()) << "InsertNodeBeforeCommand does nothing for disconnected node";
}
...
TEST_F(CompositeEditCommandTest, MoveParagraphContentsToNewBlockWithNonEditableStyle)
{
    SetBodyContent("<style>div{-webkit-user-modify:read-only;user-select:none;}</style>"
                   "foo");
    SampleCommand& sample = *MakeGarbageCollected<SampleCommand>(GetDocument());
    ...
    EditingState editing_state;
    sample.MoveParagraphContentsToNewBlockIfNecessary(Position(text, 0), &editing_state);
    EXPECT_TRUE(editing_state.IsAborted());
    EXPECT_EQ("<div><br></div>"
              "<style>div{-webkit-user-modify:read-only;user-select:none;}</style>"
              "foo",
        body->innerHTML());
}

TEST_F(CompositeEditCommandTest, MoveParagraphContentsToNewBlockWithUAShadowDOM1)
{
    ...
    EXPECT_FALSE(editing_state.IsAborted());
    EXPECT_EQ("<object contenteditable=\"\"><div><input></div></object>", GetDocument().body()->innerHTML());
}

TEST_F(CompositeEditCommandTest, MoveParagraphContentsToNewBlockWithUAShadowDOM2)
{
    ...
    sample.MoveParagraphContentsToNewBlockIfNecessary(pos, &editing_state);
    EXPECT_FALSE(editing_state.IsAborted());
    EXPECT_EQ("<div></div><span></span><button><meter></meter></button>", GetDocument().body()->innerHTML());
}

TEST_F(CompositeEditCommandTest, MoveParagraphContentsToNewBlockWithButtonAndBr)
{
    GetDocument().setDesignMode("on");
    ...
    sample.MoveParagraphContentsToNewBlockIfNecessary(pos, &editing_state);
    EXPECT_FALSE(editing_state.IsAborted());
    EXPECT_EQ("<button><div><br></div><br></button>", GetDocument().body()->innerHTML());
}
...
TEST_F(CompositeEditCommandTest, MoveParagraphsWithBr)
{
    ...
    EXPECT_GT(MostForwardCaretPosition(start.DeepEquivalent()), MostBackwardCaretPosition(end.DeepEquivalent()));

    // Should not crash
    sample.MoveParagraphs(start, end, destination, &editing_state);
    EXPECT_FALSE(editing_state.IsAborted());
    EXPECT_EQ("<ol><li><span><br></span></li></ol><br>", GetDocument().body()->innerHTML());
}
...
TEST_F(CompositeEditCommandTest, MoveParagraphsWithInlineBlocks)
{
    ...
    sample.MoveParagraphs(start, end, destination, &editing_state);
    EXPECT_FALSE(editing_state.IsAborted());
    EXPECT_EQ("<div><span></span><span></span> </div><br>", GetDocument().body()->innerHTML());
}
...
TEST_F(CompositeEditCommandTest, MoveParagraphsWithTableAndCaption)
{
    ...
    InsertStyleElement("table { writing-mode: vertical-lr; }"
    ...
    sample.MoveParagraphs(start, end, destination, &editing_state);
    EXPECT_FALSE(editing_state.IsAborted());
    EXPECT_EQ("<table><caption><div><input></div></caption></table>", GetDocument().body()->innerHTML());
}

TEST_F(CompositeEditCommandTest, MoveParagraphContentsToNewBlockWithNullVisiblePosition1)
{
    EditingState editing_state;
    Document& document = GetDocument();
    ...
    sample.MoveParagraphContentsToNewBlockIfNecessary(Position(body, 0), &editing_state);
    EXPECT_TRUE(editing_state.IsAborted());
    EXPECT_EQ("<div contenteditable=\"false\"><br></div>", body->innerHTML());
}

TEST_F(CompositeEditCommandTest, MoveParagraphContentsToNewBlockWithNullVisiblePosition2)
{
    EditingState editing_state;
    Document& document = GetDocument();
    Element* body = document.body();
