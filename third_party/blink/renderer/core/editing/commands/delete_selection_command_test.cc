...
TEST_F(DeleteSelectionCommandTest, deleteListFromTable)
{
    ...
    DeleteSelectionCommand* command = MakeGarbageCollected<DeleteSelectionCommand>(
        GetDocument(), DeleteSelectionOptions::Builder().SetMergeBlocksAfterDelete(true).SetSanitizeMarkup(true).Build(), InputEvent::InputType::kDeleteByCut);

    EXPECT_TRUE(command->Apply()) << "the delete command should have succeeded";
    EXPECT_EQ("<div contenteditable=\"true\"><br></div>", GetDocument().body()->innerHTML());
    EXPECT_TRUE(frame->Selection().GetSelectionInDOMTree().IsCaret());
    EXPECT_EQ(Position(div, 0), frame->Selection().ComputeVisibleSelectionInDOMTree().Anchor().ToOffsetInAnchor());
}
...
TEST_F(DeleteSelectionCommandTest, FixupWhitespace)
{
    ...
    Selection().SetSelection(SetSelectionTextToBody("<p contenteditable>a<b>&#32;^X|</b>&#32;Y</p>"), SetSelectionOptions());

    DeleteSelectionCommand& command = *MakeGarbageCollected<DeleteSelectionCommand>(
        GetDocument(), DeleteSelectionOptions::Builder().SetMergeBlocksAfterDelete(true).SetSanitizeMarkup(true).Build());
    EXPECT_TRUE(command.Apply()) << "the delete command should have succeeded";
    EXPECT_EQ("<p contenteditable>a<b> |</b>\u00A0Y</p>", GetSelectionTextFromBody());
}

TEST_F(DeleteSelectionCommandTest, ForwardDeleteWithFirstLetter)
{
    InsertStyleElement("p::first-letter {font-size:200%;}");
    Selection().SetSelection(SetSelectionTextToBody("<p contenteditable>a^b|c</p>"), SetSelectionOptions());
...
{
    Selection().SetSelection(SetSelectionTextToBody("<p contenteditable>"
                                                    "^<option></option>|"
...
{
    Selection().SetSelection(SetSelectionTextToBody("^<style>body{-webkit-user-modify:read-write}</style>x|"), SetSelectionOptions());
...
TEST_F(DeleteSelectionCommandTest, FloatingInputsWithTrailingSpace)
{
    ...
    Selection().SetSelection(SetSelectionTextToBody("<input>^<input><input>| "), SetSelectionOptions());
