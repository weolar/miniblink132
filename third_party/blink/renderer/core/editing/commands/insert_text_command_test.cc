...
TEST_F(InsertTextCommandTest, WithTypingStyle)
{
    ...
    Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position(sample, 0)).Build(), SetSelectionOptions());
    ...
    auto* const command = MakeGarbageCollected<InsertTextCommand>(GetDocument(), "x");
    command->Apply();

    EXPECT_EQ("<div contenteditable=\"true\"><option id=\"sample\">x</option></div>", GetDocument().body()->innerHTML())
...
{
    Selection().SetSelection(SetSelectionTextToBody("<p contenteditable><span>\ta|c</span></p>"), SetSelectionOptions());
    GetDocument().execCommand("insertText", false, "B", ASSERT_NO_EXCEPTION);
    EXPECT_EQ("<p contenteditable><span>\taB|c</span></p>", GetSelectionTextFromBody()) << "We should not split Text node";
}

// http://crbug.com/741826
TEST_F(InsertTextCommandTest, InsertCharToWhiteSpacePre)
{
    Selection().SetSelection(SetSelectionTextToBody("<p contenteditable><span style='white-space:pre'>\ta|c</span></p>"), SetSelectionOptions());
    GetDocument().execCommand("insertText", false, "B", ASSERT_NO_EXCEPTION);
...
TEST_F(InsertTextCommandTest, InsertSpace)
{
    ...
    EXPECT_EQ("<p contenteditable><span>\ta\xC2\xA0 |c</span></p>", GetSelectionTextFromBody()) << "We should insert U+0020 without splitting SPAN";
}
...
TEST_F(InsertTextCommandTest, InsertTabToWhiteSpacePre)
{
    Selection().SetSelection(SetSelectionTextToBody("<p contenteditable><span style='white-space:pre'>\ta|c</span></p>"), SetSelectionOptions());
    GetDocument().execCommand("insertText", false, "\t", ASSERT_NO_EXCEPTION);
    EXPECT_EQ("<p contenteditable><span style=\"white-space:pre\">\ta\t|c</span></p>", GetSelectionTextFromBody());
}

// http://crbug.com/752860
TEST_F(InsertTextCommandTest, WhitespaceFixupBeforeParagraph)
{
    Selection().SetSelection(SetSelectionTextToBody("<div contenteditable>qux ^bar|<p>baz</p>"), SetSelectionOptions());
    GetDocument().execCommand("insertText", false, "", ASSERT_NO_EXCEPTION);
    // The space after "qux" should have been converted to a no-break space
    // (U+00A0) to prevent it from being collapsed.
    EXPECT_EQ("<div contenteditable>qux\xC2\xA0|<p>baz</p></div>", GetSelectionTextFromBody());
    ...
    EXPECT_EQ("<div contenteditable>qux|<p>baz</p></div>", GetSelectionTextFromBody());
...
{
    Selection().SetSelection(SetSelectionTextToBody("<div contenteditable><p>baz</p>^bar| qux"), SetSelectionOptions());
    GetDocument().execCommand("insertText", false, "", ASSERT_NO_EXCEPTION);
    // The space before "qux" should have been converted to a no-break space
    // (U+00A0) to prevent it from being collapsed.
    EXPECT_EQ("<div contenteditable><p>baz</p>|\xC2\xA0qux</div>", GetSelectionTextFromBody());

    Selection().SetSelection(SetSelectionTextToBody("<div contenteditable><p>baz</p>^bar |qux"), SetSelectionOptions());
    GetDocument().execCommand("insertText", false, " ", ASSERT_NO_EXCEPTION);
    // The newly-inserted space should have been converted to a no-break space
    // (U+00A0) to prevent it from being collapsed.
    EXPECT_EQ("<div contenteditable><p>baz</p>\xC2\xA0|qux</div>", GetSelectionTextFromBody());

    Selection().SetSelection(SetSelectionTextToBody("<div contenteditable><p>baz</p> ^bar|qux"), SetSelectionOptions());
    GetDocument().execCommand("insertText", false, "", ASSERT_NO_EXCEPTION);
    // The space before "bar" was already being collapsed before the edit. It
    // should not have been converted to a no-break space.
    EXPECT_EQ("<div contenteditable><p>baz</p>|qux</div>", GetSelectionTextFromBody());
...
TEST_F(InsertTextCommandTest, NoVisibleSelectionAfterDeletingSelection)
{
    ...
    InsertStyleElement(":root { font-size: 10px; }"
...
{
    InsertStyleElement("head {-webkit-text-stroke-color: black; display: list-item;}");
    ...
    Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position(head, 0)).Extend(Position(body, 0)).Build(), SetSelectionOptions());

    // Shouldn't crash inside
    GetDocument().execCommand("insertText", false, "\t", ASSERT_NO_EXCEPTION);

    // This only records the current behavior, which is not necessarily correct.
    EXPECT_EQ("<body><span style=\"white-space:pre\">\t|</span></body>"
              "<style>"
              "head {-webkit-text-stroke-color: black; display: list-item;}"
              "</style>",
        SelectionSample::GetSelectionText(*GetDocument().documentElement(), Selection().GetSelectionInDOMTree()));
}
...
TEST_F(InsertTextCommandTest, AnchorElementWithBlockCrash)
{
    ...
    Selection().SetSelection(
        SelectionInDOMTree::Builder().SetBaseAndExtent(Position(iElement_text_node, 0), Position(iElement_text_node, 4)).Build(), SetSelectionOptions());
    // Crash happens here with when '\n' is inserted.
    GetDocument().execCommand("inserttext", false, "a\n", ASSERT_NO_EXCEPTION);
    EXPECT_EQ("<a href=\"www\" style=\"display:block\"><i>a</i></a><a href=\"www\" "
...
TEST_F(InsertTextCommandTest, MultilineSelectionCrash)
{
    ...
    Selection().SetSelection(SetSelectionTextToBody("A^<span> B|</span>"), SetSelectionOptions());
    GetDocument().setDesignMode("on");
