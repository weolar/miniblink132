...
SetCharacterDataCommand::SetCharacterDataCommand(Text* node, unsigned offset, unsigned count, const String& text)
...
void SetCharacterDataCommand::DoApply(EditingState*)
{
    ...
    previous_text_for_undo_ = node_->substringData(offset_, count_, exception_state);
    if (exception_state.HadException())
        return;

    const bool password_echo_enabled = GetDocument().GetSettings() && GetDocument().GetSettings()->GetPasswordEchoEnabled();

    if (password_echo_enabled) {
        LayoutText* layout_text = node_->GetLayoutObject();
        if (layout_text && layout_text->IsSecure()) {
            layout_text->MomentarilyRevealLastTypedCharacter(offset_ + new_text_.length() - 1);
        }
    }
...
void SetCharacterDataCommand::DoUnapply()
{
    ...
    node_->replaceData(offset_, new_text_.length(), previous_text_for_undo_, IGNORE_EXCEPTION_FOR_TESTING);
}
