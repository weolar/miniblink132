...
TEST_F(SplitTextNodeCommandTest, splitInMarkerInterior)
{
    ...
    GetDocument().Markers().AddTextMatchMarker(range, TextMatchMarker::MatchStatus::kInactive);
    ...
    SimpleEditCommand* command = MakeGarbageCollected<SplitTextNodeCommand>(To<Text>(GetDocument().body()->firstChild()->firstChild()), 8);
