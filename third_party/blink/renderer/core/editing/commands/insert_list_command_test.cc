...
TEST_F(InsertListCommandTest, ShouldCleanlyRemoveSpuriousTextNode)
{
    ...
    GetDocument().body()->InsertBefore(empty_text, GetDocument().body()->firstChild());
    UpdateAllLifecyclePhasesForTest();
    GetDocument().GetFrame()->Selection().SetSelection(
        SelectionInDOMTree::Builder().Collapse(Position(GetDocument().body(), 0)).Extend(Position(GetDocument().body(), 2)).Build(), SetSelectionOptions());

    auto* command = MakeGarbageCollected<InsertListCommand>(GetDocument(), InsertListCommand::kOrderedList);
...
{
    GetDocument().setDesignMode("on");
    Selection().SetSelection(SetSelectionTextToBody("^<dl>"
                                                    "<textarea style='float:left;'></textarea>"
                                                    "</dl>|"),
        SetSelectionOptions());
    auto* command = MakeGarbageCollected<InsertListCommand>(GetDocument(), InsertListCommand::kUnorderedList);
    // Crash happens here.
...
{
    GetDocument().setDesignMode("on");
    InsertStyleElement("* { -webkit-appearance:checkbox; }"
    ...
    // Crash happens here.
    EXPECT_TRUE(command->Apply());
    EXPECT_EQ("<ul><li><table><colgroup><col>"
              "</colgroup></table></li>"
...
TEST_F(InsertListCommandTest, InsertListOnEmptyHiddenElements)
{
    ...
    Selection().SetSelection(SetSelectionTextToBody("^<button></button>|"), SetSelectionOptions());
    auto* command = MakeGarbageCollected<InsertListCommand>(GetDocument(), InsertListCommand::kUnorderedList);

    // Crash happens here.
    EXPECT_FALSE(command->Apply());
    EXPECT_EQ("^<button><ul><li><br></li></ul></button>|", GetSelectionTextFromBody());
}
...
{
    GetDocument().setDesignMode("on");
    InsertStyleElement("ul { visibility:collapse; }"
                       "dl { visibility:visible; }");

    Selection().SetSelection(SetSelectionTextToBody("^<dl>a</dl>|"), SetSelectionOptions());
    auto* command = MakeGarbageCollected<InsertListCommand>(GetDocument(), InsertListCommand::kOrderedList);
...
{
    GetDocument().setDesignMode("on");
    Selection().SetSelection(SetSelectionTextToBody("<li>a|<div contenteditable=false>b</div></li>"), SetSelectionOptions());
    auto* command = MakeGarbageCollected<InsertListCommand>(GetDocument(), InsertListCommand::kUnorderedList);

    // Crash happens here.
    EXPECT_FALSE(command->Apply());
    EXPECT_EQ("<ul><li>a|<div contenteditable=\"false\">b</div></li></ul><br>", GetSelectionTextFromBody());
}
...
TEST_F(InsertListCommandTest, NestedSpansJustInsideBody)
{
    ...
    GetDocument().setDesignMode("on");
    Selection().SetSelection(SetSelectionTextToBody("<span><span><span>a</span></span></span>|b"), SetSelectionOptions());
    auto* command = MakeGarbageCollected<InsertListCommand>(GetDocument(), InsertListCommand::kUnorderedList);

    // Crash happens here.
    EXPECT_FALSE(command->Apply());
    EXPECT_EQ("<ul><li><br>a</li></ul><span><span><span>^a</span></span></span>b|", GetSelectionTextFromBody());
}

TEST_F(InsertListCommandTest, ListifyInputInTableCell)
{
    GetDocument().setDesignMode("on");
    Selection().SetSelection(SetSelectionTextToBody("^<ruby><div style='display: table-cell'><input style='display: "
    ...
    // Crash happens here.
    EXPECT_TRUE(command->Apply());
    EXPECT_EQ("<ruby><div style=\"display: "
...
{
    GetDocument().setDesignMode("on");
    InsertStyleElement("rb { display: table-cell; }"
                       "input { float: left; }");
    Selection().SetSelection(SetSelectionTextToBody("<div contenteditable='true'><ol><li>^<br></li>"
    ...
    // Crash happens here.
    EXPECT_TRUE(command->Apply());
    EXPECT_EQ("<div contenteditable=\"true\">^<br><ol><li><ruby><rb><ol><li><br></li>"
...
TEST_F(InsertListCommandTest, NonCanonicalVisiblePosition)
{
    ...
    SetBodyInnerHTML("<textarea></textarea><svg></svg><select></select><div><input></div>");
    const Position& base = Position::BeforeNode(*document.QuerySelector(AtomicString("select")));
    const Position& extent = Position::AfterNode(*document.QuerySelector(AtomicString("input")));
    Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(base).Extend(extent).Build(), SetSelectionOptions());
    ...
    auto* command = MakeGarbageCollected<InsertListCommand>(document, InsertListCommand::kUnorderedList);

    // Crash happens here.
    EXPECT_TRUE(command->Apply());
    EXPECT_EQ("<ul><li><textarea></textarea><svg></svg>^<select></select></li>"
              "<li><input>|</li></ul>",
        GetSelectionTextFromBody());
}
...
TEST_F(InsertListCommandTest, TimeAndMeterInRoot)
{
    ...
    Selection().SetSelection(
        SelectionInDOMTree::Builder().Collapse(Position(time, 0)).Extend(Position::LastPositionInNode(*time)).Build(), SetSelectionOptions());

    auto* command = MakeGarbageCollected<InsertListCommand>(document, InsertListCommand::kUnorderedList);

    // Crash happens here.
    EXPECT_TRUE(command->Apply());
    EXPECT_EQ("<ul><li>|<time></time></li></ul><head></head><body></body>", SelectionSample::GetSelectionText(*root, Selection().GetSelectionInDOMTree()));
}
...
TEST_F(InsertListCommandTest, PreservedNewline)
{
    Document& document = GetDocument();
    document.setDesignMode("on");
    Selection().SetSelection(SetSelectionTextToBody("<pre><span></span>\nX^<div></div>|</pre>"), SetSelectionOptions());

    auto* command = MakeGarbageCollected<InsertListCommand>(document, InsertListCommand::kOrderedList);

    // Crash happens here.
    EXPECT_TRUE(command->Apply());
    EXPECT_EQ("<pre><span></span>\n<ol><li>|X</li></ol><div></div></pre>", GetSelectionTextFromBody());
}
...
TEST_F(InsertListCommandTest, EmptyInlineBlock)
{
    ...
    Selection().SetSelection(SetSelectionTextToBody("<ul><li><span>|</span></li></ul>"), SetSelectionOptions());
...
TEST_F(InsertListCommandTest, SelectionFromEndOfTableToAfterTable)
{
    Document& document = GetDocument();
    document.setDesignMode("on");
    Selection().SetSelection(SetSelectionTextToBody("<table><td>^</td></table>|"), SetSelectionOptions());

    auto* command = MakeGarbageCollected<InsertListCommand>(document, InsertListCommand::kOrderedList);

    // Crash happens here.
    EXPECT_TRUE(command->Apply());
    EXPECT_EQ("<table><tbody><tr><td><ol><li>|<br></li></ol></td></tr></tbody></table>", GetSelectionTextFromBody());
}
...
TEST_F(InsertListCommandTest, ListItemWithSpace)
{
    Document& document = GetDocument();
    document.setDesignMode("on");
    Selection().SetSelection(SetSelectionTextToBody("<li>^ <div contenteditable='false'>A</div>B|</li>"), SetSelectionOptions());

    auto* command = MakeGarbageCollected<InsertListCommand>(document, InsertListCommand::kOrderedList);

    // Crash happens here.
    EXPECT_FALSE(command->Apply());
    EXPECT_EQ("<ul><li> <div contenteditable=\"false\">A</div>B|</li></ul><br>", GetSelectionTextFromBody());
}
}
