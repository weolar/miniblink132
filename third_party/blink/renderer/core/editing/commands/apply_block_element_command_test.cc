...
{
    GetDocument().head()->insertAdjacentHTML("afterbegin", "<style> .CLASS13 { -webkit-user-modify: read-write; }</style></head>", ASSERT_NO_EXCEPTION);
    GetDocument().body()->insertAdjacentHTML("afterbegin", "\n<pre><var id='va' class='CLASS13'>\nC\n</var></pre><input />", ASSERT_NO_EXCEPTION);
    GetDocument().body()->insertAdjacentText("beforebegin", "foo", ASSERT_NO_EXCEPTION);
    ...
    Selection().SetSelection(
        SelectionInDOMTree::Builder()
            .SetBaseAndExtent(Position(GetDocument().documentElement(), 1), Position(GetDocument().getElementById(AtomicString("va"))->firstChild(), 2))
            .Build(),
        SetSelectionOptions());

    auto* command = MakeGarbageCollected<FormatBlockCommand>(GetDocument(), html_names::kFooterTag);
    command->Apply();
...
{
    GetDocument().head()->insertAdjacentHTML("afterbegin", "<style>li:first-child { visibility:visible; }</style>", ASSERT_NO_EXCEPTION);
    SetBodyContent("<ul style='visibility:hidden'><li>xyz</li></ul>");
    GetDocument().setDesignMode("on");

    UpdateAllLifecyclePhasesForTest();
    Selection().SetSelection(
        SelectionInDOMTree::Builder().Collapse(Position(GetDocument().QuerySelector(AtomicString("li")), 0)).Build(), SetSelectionOptions());

    auto* command = MakeGarbageCollected<IndentOutdentCommand>(GetDocument(), IndentOutdentCommand::kIndent);
    command->Apply();

    EXPECT_EQ("<head><style>li:first-child { visibility:visible; }</style></head>"
...
{
    SetBodyContent("<div contenteditable=\"true\">"
    ...
    Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position(button, 0)).Extend(Position(object, 0)).Build(), SetSelectionOptions());

    auto* command = MakeGarbageCollected<IndentOutdentCommand>(GetDocument(), IndentOutdentCommand::kIndent);
    command->Apply();

    // This only records the current behavior, which can be wrong.
    EXPECT_EQ("<div contenteditable=\"true\">"
...
TEST_F(ApplyBlockElementCommandTest, InsertPlaceHolderAtDisconnectedPosition)
{
    ...
    Selection().SetSelection(SetSelectionTextToBody("^<input><input class=\"input\" style=\"position:absolute\">|"), SetSelectionOptions());
    auto* command = MakeGarbageCollected<FormatBlockCommand>(GetDocument(), html_names::kPreTag);
    // Crash happens here.
    EXPECT_TRUE(command->Apply());
    EXPECT_EQ("<pre>^<input>|</pre><input class=\"input\" style=\"position:absolute\">", GetSelectionTextFromBody());
}
...
TEST_F(ApplyBlockElementCommandTest, FormatBlockCrossingUserModifyBoundary)
{
    ...
    Selection().SetSelection(SetSelectionTextToBody("^<b style=\"-webkit-user-modify:read-only\"><button></button></b>|"), SetSelectionOptions());
    auto* command = MakeGarbageCollected<FormatBlockCommand>(GetDocument(), html_names::kPreTag);
    // Shouldn't crash here.
    EXPECT_TRUE(command->Apply());
    EXPECT_EQ("<pre>|<br></pre>"
              "<b style=\"-webkit-user-modify:read-only\"><button></button></b>",
        GetSelectionTextFromBody());
}

// https://crbug.com/873084
TEST_F(ApplyBlockElementCommandTest, FormatBlockWithTableCrossingUserModifyBoundary)
{
    InsertStyleElement("*{-webkit-user-modify:read-write}");
    Selection().SetSelection(SetSelectionTextToBody("^<table></table>"
                                                    "<kbd "
...
TEST_F(ApplyBlockElementCommandTest, FormatBlockWithDirectChildrenOfRoot)
{
    ...
    Selection().SetSelection(SelectionInDOMTree::Builder().SelectAllChildren(*root).Build(), SetSelectionOptions());
    auto* command = MakeGarbageCollected<FormatBlockCommand>(GetDocument(), html_names::kPreTag);
    // Shouldn't crash here.
    EXPECT_FALSE(command->Apply());
    const SelectionInDOMTree& selection = Selection().GetSelectionInDOMTree();
    EXPECT_EQ("^a<div>b</div>c|", SelectionSample::GetSelectionText(*root, selection));
}

// This is a regression test for https://crbug.com/1180699
TEST_F(ApplyBlockElementCommandTest, OutdentEmptyBlockquote)
{
    Vector<std::string> selection_texts = { "<blockquote style='padding:5px'>|</blockquote>", "a<blockquote style='padding:5px'>|</blockquote>",
        "<blockquote style='padding:5px'>|</blockquote>b", "a<blockquote style='padding:5px'>|</blockquote>b" };
    Vector<std::string> expectations = { "|", "a|<br>", "|<br>b", "a<br>|b" };

    GetDocument().setDesignMode("on");
    for (unsigned i = 0; i < selection_texts.size(); ++i) {
        Selection().SetSelection(SetSelectionTextToBody(selection_texts[i]), SetSelectionOptions());
        auto* command = MakeGarbageCollected<IndentOutdentCommand>(GetDocument(), IndentOutdentCommand::kOutdent);
...
TEST_F(ApplyBlockElementCommandTest, IndentSVGWithTable)
{
    ...
    // Shouldn't crash here.
    EXPECT_TRUE(command->Apply());
    EXPECT_EQ("<blockquote style=\"margin: 0 0 0 40px; border: none; padding: 0px;\">"
...
TEST_F(ApplyBlockElementCommandTest, IndentOutdentLinesDoubleBr)
{
    ...
    auto* indent = MakeGarbageCollected<IndentOutdentCommand>(GetDocument(), IndentOutdentCommand::kIndent);
    ...
    auto* outdent = MakeGarbageCollected<IndentOutdentCommand>(GetDocument(), IndentOutdentCommand::kOutdent);
...
TEST_F(ApplyBlockElementCommandTest, SplitTextNodeWithJustNewline)
{
    ...
    auto* const format_block = MakeGarbageCollected<FormatBlockCommand>(GetDocument(), html_names::kDivTag);

    ASSERT_TRUE(format_block->Apply());
    EXPECT_EQ("<pre contenteditable><b><div>|X</div>\n</b></pre>", GetSelectionTextFromBody());
}
