...
{
    InsertStyleElement("b { display: inline-block; width: 100px; }"
                       "div { width: 100px; }");
...
TEST_F(TypingCommandTest, insertLineBreakWithIllFormedHTML)
{
    ...
    frame->Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position(form, 0)).Extend(Position(header, 0)).Build(), SetSelectionOptions());
...
TEST_F(TypingCommandTest, DontCrashWhenReplaceSelectionCommandLeavesBadSelection)
{
    Selection().SetSelection(SetSelectionTextToBody("<div contenteditable>^<h1>H1</h1>ello|</div>"), SetSelectionOptions());

    // This call shouldn't crash.
    TypingCommand::InsertText(GetDocument(), " ", 0, TypingCommand::TextCompositionType::kTextCompositionUpdate, true);
    EXPECT_EQ("<div contenteditable><h1>\xC2\xA0|</h1></div>", GetSelectionTextFromBody());
}
...
TEST_F(TypingCommandTest, ForwardDeleteInvalidatesSelection)
{
    ...
    Selection().SetSelection(SetSelectionTextToBody("<blockquote>^"
                                                    "<q>"
...
TEST_F(TypingCommandTest, ForwardDeleteAtTableEnd)
{
    ...
    Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position(table->firstChild(), 1)).Build(), SetSelectionOptions());
...
TEST_F(TypingCommandTest, TypedCharactersInContentEditable)
{
    ...
    UpdateAllLifecyclePhasesForTest();
    Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position(table->firstChild(), 1)).Build(), SetSelectionOptions());
    TypingCommand::InsertText(GetDocument(), "b", 0, TypingCommand::TextCompositionType::kTextCompositionUpdate, true);
    TypingCommand::InsertText(GetDocument(), "c", 0, TypingCommand::TextCompositionType::kTextCompositionUpdate, true);
