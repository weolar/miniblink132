...
TEST_F(ReplaceSelectionCommandTest, pastingEmptySpan)
{
    ...
    frame->Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position(GetDocument().body(), 0)).Build(), SetSelectionOptions());
    ...
    ReplaceSelectionCommand::CommandOptions options = ReplaceSelectionCommand::kPreventNesting | ReplaceSelectionCommand::kSanitizeFragment
        | ReplaceSelectionCommand::kSelectReplacement | ReplaceSelectionCommand::kSmartReplace;
    auto* command = MakeGarbageCollected<ReplaceSelectionCommand>(GetDocument(), fragment, options);
...
TEST_F(ReplaceSelectionCommandTest, pasteSpanInText)
{
    ...
    LocalFrame* frame = GetDocument().GetFrame();
    frame->Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position(b_element->firstChild(), 1)).Build(), SetSelectionOptions());
    ...
    EXPECT_TRUE(command->Apply()) << "the replace command should have succeeded";
    EXPECT_EQ("<b>t</b>bar<b>ext</b>", GetDocument().body()->innerHTML()) << "'bar' should have been inserted";
}
...
TEST_F(ReplaceSelectionCommandTest, TextAutosizingDoesntInflateText)
{
    ...
    GetDocument().GetFrame()->Selection().SetSelection(
        SelectionInDOMTree::Builder().Collapse(Position(span->firstChild(), 4)).Extend(Position(span->firstChild(), 7)).Build(), SetSelectionOptions());

    DocumentFragment* fragment = GetDocument().createDocumentFragment();
    fragment->ParseHTML("baz", span);

    ReplaceSelectionCommand::CommandOptions options = ReplaceSelectionCommand::kMatchStyle;
...
{
    GetDocument().setDesignMode("on");
    Selection().SetSelection(SetSelectionTextToBody("<div>^foo|</div>"), SetSelectionOptions());

    DocumentFragment* fragment = GetDocument().createDocumentFragment();
    fragment->ParseHTML("<div>bar</div> ", GetDocument().QuerySelector(AtomicString("div")));
    ReplaceSelectionCommand::CommandOptions options = 0;
    auto* command = MakeGarbageCollected<ReplaceSelectionCommand>(GetDocument(), fragment, options);
...
TEST_F(ReplaceSelectionCommandTest, CrashWithNoSelection)
{
    ...
    ReplaceSelectionCommand::CommandOptions options = 0;
    auto* command = MakeGarbageCollected<ReplaceSelectionCommand>(GetDocument(), nullptr, options);
...
TEST_F(ReplaceSelectionCommandTest, SmartPlainTextPaste)
{
    ...
    Selection().SetSelection(SetSelectionTextToBody("<div contenteditable>abc<div>def</div>|</div>"), SetSelectionOptions());
    DocumentFragment& fragment = *GetDocument().createDocumentFragment();
    fragment.appendChild(Text::Create(GetDocument(), "XYZ"));
    const ReplaceSelectionCommand::CommandOptions options = ReplaceSelectionCommand::kPreventNesting | ReplaceSelectionCommand::kSanitizeFragment
        | ReplaceSelectionCommand::kMatchStyle | ReplaceSelectionCommand::kSmartReplace;
    auto& command = *MakeGarbageCollected<ReplaceSelectionCommand>(GetDocument(), &fragment, options, InputEvent::InputType::kInsertFromPaste);

    EXPECT_TRUE(command.Apply());
    // Smart paste inserts a space before pasted text.
    EXPECT_EQ("<div contenteditable>abc<div>def XYZ|</div></div>", GetSelectionTextFromBody());
}
...
TEST_F(ReplaceSelectionCommandTest, TableAndImages)
{
    ...
    Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position(tbody, 1)).Build(), SetSelectionOptions());

    DocumentFragment* fragment = GetDocument().createDocumentFragment();
    fragment->AppendChild(GetDocument().CreateRawElement(html_names::kImgTag));
    auto& command
        = *MakeGarbageCollected<ReplaceSelectionCommand>(GetDocument(), fragment, ReplaceSelectionCommand::kPreventNesting, InputEvent::InputType::kNone);

    // Should not crash
    EXPECT_TRUE(command.Apply());
    EXPECT_EQ("<table> <tbody><img><img>|</tbody> </table>", GetSelectionTextFromBody());
}
...
{
    GetDocument().setDesignMode("on");
    Selection().SetSelection(SetSelectionTextToBody("<span><div></div>|a</span>"), SetSelectionOptions());
    ...
    // Should not crash
    EXPECT_TRUE(command.Apply());
    EXPECT_EQ("<span><div></div></span><img>|<span>a</span>", GetSelectionTextFromBody());
}
...
{
    GetDocument().setDesignMode("on");
    Selection().SetSelection(SetSelectionTextToBody("<button><div></div><svg></svg>&#x20;|</button>x<input>"), SetSelectionOptions());
    ...
    // Should not crash
    EXPECT_TRUE(command.Apply());
    EXPECT_EQ("<button><div></div><svg></svg></button><img>|x<input>", GetSelectionTextFromBody());
}
...
{
    GetDocument().setDesignMode("on");
    Selection().SetSelection(SetSelectionTextToBody("<div contenteditable=\"false\"><span contenteditable>"
                                                    "a|b</span></div>"),
        SetSelectionOptions());
    ...
    // Should not crash
    EXPECT_TRUE(command.Apply());
    EXPECT_EQ("<div contenteditable=\"false\"><span contenteditable>"
              "a<img>|<br>b</span></div>",
...
{
    GetDocument().setDesignMode("on");
    Selection().SetSelection(SetSelectionTextToBody("<strong xml:space><div contenteditable=\"false\">"
    ...
    // Should not crash
    EXPECT_TRUE(command.Apply());
    EXPECT_EQ("<strong xml:space><div contenteditable=\"false\">"
