...
TEST_F(InsertParagraphSeparatorCommandTest, CrashWithAppearanceStyleOnEmptyColgroup)
{
    Selection().SetSelection(SetSelectionTextToBody("<table contenteditable>"
                                                    "    <colgroup style='-webkit-appearance:radio;'><!--|--></colgroup>"
                                                    "</table>"),
        SetSelectionOptions());

    auto* command = MakeGarbageCollected<InsertParagraphSeparatorCommand>(GetDocument());
    // Crash should not be observed here.
    command->Apply();

    EXPECT_EQ("<table contenteditable>"
              "|    <colgroup style=\"-webkit-appearance:radio;\"></colgroup>"
              "</table>",
...
// http://crbug.com/777378
TEST_F(InsertParagraphSeparatorCommandTest, CrashWithAppearanceStyleOnEmptyColumn)
{
    Selection().SetSelection(SetSelectionTextToBody("<table contenteditable>"
...
TEST_F(InsertParagraphSeparatorCommandTest, CrashWithCaptionBeforeBody)
{
    ...
    Element* caption = GetDocument().CreateElementForBinding(AtomicString("caption"));
    caption->setInnerHTML("AxBxC");
    GetDocument().documentElement()->insertBefore(caption, GetDocument().body());

    Selection().SetSelection(SelectionInDOMTree::Builder().SetBaseAndExtent(EphemeralRange::RangeOfContents(*caption)).Build(), SetSelectionOptions());

    auto* command = MakeGarbageCollected<InsertParagraphSeparatorCommand>(GetDocument());
    // Shouldn't crash inside.
    EXPECT_FALSE(command->Apply());
    EXPECT_EQ("<body><style><br>|*{max-width:inherit;display:initial;}</style></body>",
...
{
    GetDocument().setDesignMode("on");
    Selection().SetSelection(SetSelectionTextToBody("<object><b>|ABC</b></object>"), SetSelectionOptions());
    base::RunLoop().RunUntilIdle(); // prepare <object> fallback content

    auto* command = MakeGarbageCollected<InsertParagraphSeparatorCommand>(GetDocument());

    EXPECT_TRUE(command->Apply());
    EXPECT_EQ("<div><object><b><br></b></object></div>"
...
TEST_F(InsertParagraphSeparatorCommandTest, CrashWithObjectWithFloat)
{
    ...
    Element& object_element = *GetDocument().QuerySelector(AtomicString("object"));
    object_element.appendChild(Text::Create(GetDocument(), "XYZ"));

    auto* command = MakeGarbageCollected<InsertParagraphSeparatorCommand>(GetDocument());

    EXPECT_TRUE(command->Apply());
    EXPECT_EQ("<object><b><br></b></object>"
