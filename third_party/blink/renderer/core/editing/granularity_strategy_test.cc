...
protected:
    ...
    void SetupTextSpan(String str1, String str2, String str3, wtf_size_t sel_begin, wtf_size_t sel_end);
    void SetupVerticalAlign(String str1, String str2, String str3, wtf_size_t sel_begin, wtf_size_t sel_end);
    void SetupFontSize(String str1, String str2, String str3, wtf_size_t sel_begin, wtf_size_t sel_end);
...
void GranularityStrategyTest::ParseText(const TextNodeVector& text_nodes)
{
    ...
    for (auto& text : text_nodes) {
        ...
        for (wtf_size_t i = 0; i < str.length(); i++) {
            letter_pos_.push_back(VisiblePositionToContentsPoint(CreateVisiblePosition(Position(text, i))));
            char c = str[i];
            if (IsASCIIAlphanumeric(c) && !word_started) {
                ...
                gfx::Point word_middle(
                    (letter_pos_[word_start_index].x() + letter_pos_[i + word_start_index_offset].x()) / 2, letter_pos_[word_start_index].y());
    ...
    if (word_started) {
        ...
        int x_end = VisiblePositionToContentsPoint(CreateVisiblePosition(Position(last_node, last_node->wholeText().length()))).x();
        gfx::Point word_middle((letter_pos_[word_start_index].x() + x_end) / 2, letter_pos_[word_start_index].y());
        word_middles_.push_back(word_middle);
    }
}
...
void GranularityStrategyTest::SetupTextSpan(String str1, String str2, String str3, wtf_size_t sel_begin, wtf_size_t sel_end)
{
    ...
    Selection().SetSelection(SelectionInDOMTree::Builder().SetBaseAndExtent(p1, p2).Build(), SetSelectionOptions());
}

void GranularityStrategyTest::SetupVerticalAlign(String str1, String str2, String str3, wtf_size_t sel_begin, wtf_size_t sel_end)
{
    SetInnerHTML("<html>"
                 "<head>"
                 "<style>"
...
void GranularityStrategyTest::SetupFontSize(String str1, String str2, String str3, wtf_size_t sel_begin, wtf_size_t sel_end)
{
    SetInnerHTML("<html>"
                 "<head>"
                 "<style>"
...
{
    GetDummyPageHolder().GetFrame().GetSettings()->SetSelectionStrategy(SelectionStrategy::kCharacter);
...
TEST_F(GranularityStrategyTest, DirectionExpandTranslateZ)
{
    ...
    Selection().SetSelection(SelectionInDOMTree::Builder().SetBaseAndExtent(Position(text, 18), Position(text, 19)).Build(), SetSelectionOptions());
    EXPECT_EQ_SELECTED_TEXT("p");
    TestDirectionExpand();
}
...
TEST_F(GranularityStrategyTest, DirectionShrinkTranslateZ)
{
    ...
    Selection().SetSelection(SelectionInDOMTree::Builder().SetBaseAndExtent(Position(text, 18), Position(text, 21)).Build(), SetSelectionOptions());
    EXPECT_EQ_SELECTED_TEXT("pqr");
    TestDirectionShrink();
}
...
TEST_F(GranularityStrategyTest, DirectionSwitchSideWordGranularityThenShrink)
{
    ...
    GetDocument().body()->AppendChild(text);
    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kTest);
    GetDummyPageHolder().GetFrame().GetSettings()->SetSelectionStrategy(SelectionStrategy::kDirection);
    ...
    gfx::Point p = gfx::Point(word_middles_[2].x() - offset - 1, word_middles_[2].y());
    Selection().MoveRangeSelectionExtent(p);
...
TEST_F(GranularityStrategyTest, DirectionSwitchStartOnBoundary)
{
    ...
    // > means end).
    Selection().SetSelection(SelectionInDOMTree::Builder().SetBaseAndExtent(Position(text, 15), Position(text, 22)).Build(), SetSelectionOptions());
...
{
    GetDummyPageHolder().GetFrame().GetSettings()->SetSelectionStrategy(SelectionStrategy::kCharacter);
    GetDocument().body()->setInnerHTML("<div id=host></div><div id=sample>ab</div>");
    ...
    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kTest);
    const SelectionInDOMTree& selection_in_dom_tree = SelectionInDOMTree::Builder().Collapse(Position(sample->firstChild(), 2)).Build();
    Selection().SetSelection(selection_in_dom_tree,
        SetSelectionOptions::Builder().SetShouldCloseTyping(true).SetShouldClearTypingStyle(true).SetShouldShowHandle(true).SetIsDirectional(true).Build());

    // Since, it is not obvious that
    // |PositionForContentsPointRespectingEditingBoundary()| returns null
    // position, we verify here.
    ASSERT_EQ(Position(), CreateVisiblePosition(PositionForContentsPointRespectingEditingBoundary(gfx::Point(0, 0), &GetFrame())).DeepEquivalent())
...
{
    const SelectionInDOMTree selection = SetSelectionTextToBody("<style>body { margin: 0; padding: 0; font: 10px monospace; }</style>"
                                                                "<div contenteditable id=target></div>|def^");
    Selection().SetSelection(selection, SetSelectionOptions());

    // Move inside content editable
    ASSERT_EQ(Position(*GetDocument().getElementById(AtomicString("target")), 0),
        CreateVisiblePosition(PositionForContentsPointRespectingEditingBoundary(gfx::Point(0, 0), &GetFrame())).DeepEquivalent())
