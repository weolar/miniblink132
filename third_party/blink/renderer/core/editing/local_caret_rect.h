...
public:
    ...
    LocalCaretRect(const LayoutObject* layout_object, const PhysicalRect& rect, const PhysicalBoxFragment* root_box_fragment = nullptr)
    ...
    bool IsEmpty() const
    {
        return !layout_object || rect.IsEmpty();
    }
};

// Rect is local to the returned layoutObject
CORE_EXPORT LocalCaretRect LocalCaretRectOfPosition(const PositionWithAffinity&, EditingBoundaryCrossingRule = kCanCrossEditingBoundary);
CORE_EXPORT LocalCaretRect LocalCaretRectOfPosition(const PositionInFlatTreeWithAffinity&, EditingBoundaryCrossingRule = kCanCrossEditingBoundary);

LocalCaretRect LocalSelectionRectOfPosition(const PositionWithAffinity&);

// Bounds of (possibly transformed) caret in absolute coords
CORE_EXPORT gfx::Rect AbsoluteCaretBoundsOf(const PositionWithAffinity&, EditingBoundaryCrossingRule rule = kCanCrossEditingBoundary);
CORE_EXPORT gfx::Rect AbsoluteCaretBoundsOf(const PositionInFlatTreeWithAffinity&);
