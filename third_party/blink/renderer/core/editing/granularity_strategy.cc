...
static bool ArePositionsInSpecifiedOrder(const VisiblePosition& vp1, const VisiblePosition& vp2, int specified_order)
{
...
static Position NextWordBound(const Position& pos, SearchDirection direction, BoundAdjust word_bound_adjust)
{
    bool next_bound_if_on_bound = word_bound_adjust == BoundAdjust::kNextBoundIfOnBound;
    if (direction == SearchDirection::kSearchForward) {
        WordSide word_side = next_bound_if_on_bound ? kNextWordIfOnBoundary : kPreviousWordIfOnBoundary;
        return EndOfWordPosition(pos, word_side);
    }
    WordSide word_side = next_bound_if_on_bound ? kPreviousWordIfOnBoundary : kNextWordIfOnBoundary;
...
SelectionInDOMTree CharacterGranularityStrategy::UpdateExtent(const gfx::Point& extent_point, LocalFrame* frame)
{
    const VisiblePosition& extent_position = CreateVisiblePosition(PositionForContentsPointRespectingEditingBoundary(extent_point, frame));
    const VisibleSelection& selection = frame->Selection().ComputeVisibleSelectionInDOMTree();
    if (extent_position.IsNull() || selection.VisibleAnchor().DeepEquivalent() == extent_position.DeepEquivalent()) {
        return selection.AsSelection();
    }
    return SelectionInDOMTree::Builder().Collapse(selection.Anchor()).Extend(extent_position.DeepEquivalent()).SetAffinity(selection.Affinity()).Build();
}
...
SelectionInDOMTree DirectionGranularityStrategy::UpdateExtent(const gfx::Point& extent_point, LocalFrame* frame)
{
    const VisibleSelection& selection = frame->Selection().ComputeVisibleSelectionInDOMTree();
    ...
    gfx::Point old_offset_focus_point = old_focus_location + diff_extent_point_from_extent_position_;
    gfx::Point old_focus_point = gfx::Point(old_offset_focus_point.x() - offset_, old_offset_focus_point.y());
    ...
    VisiblePosition new_offset_focus_position = CreateVisiblePosition(PositionForContentsPointRespectingEditingBoundary(new_offset_focus_point, frame));
    ...
    if (vertical_change) {
        ...
        new_offset_focus_position = CreateVisiblePosition(PositionForContentsPointRespectingEditingBoundary(extent_point, frame));
        if (new_offset_focus_position.IsNull()) {
            return selection.AsSelection();
        }
    }
    ...
    if (vertical_change && InSameLine(new_offset_focus_position, old_offset_focus_position)) {
    ...
    if (new_offset_focus_position.DeepEquivalent() == old_offset_focus_position.DeepEquivalent()) {
        ...
        bool selection_expanded = ArePositionsInSpecifiedOrder(new_offset_focus_position, old_offset_focus_position, old_focus_anchor_order);
        bool extent_base_order_switched = selection_expanded ? false : !ArePositionsInSpecifiedOrder(new_offset_focus_position, anchor, old_focus_anchor_order);
        new_focus_anchor_order = extent_base_order_switched ? -old_focus_anchor_order : old_focus_anchor_order;
        ...
        if (extent_base_order_switched) {
            ...
            // the |base| position.
            word_boundary_position = NextWordBound(anchor.DeepEquivalent(),
                new_focus_anchor_order > 0 ? SearchDirection::kSearchForward : SearchDirection::kSearchBackwards, BoundAdjust::kNextBoundIfOnBound);
            granularity_ = TextGranularity::kCharacter;
        } else {
            ...
            word_boundary_position = NextWordBound(old_offset_focus_position.DeepEquivalent(),
                old_focus_anchor_order > 0 ? SearchDirection::kSearchForward : SearchDirection::kSearchBackwards,
                state_ == StrategyState::kShrinking ? BoundAdjust::kNextBoundIfOnBound : BoundAdjust::kCurrentPosIfOnBound);
        }
        VisiblePosition word_boundary = CreateVisiblePosition(word_boundary_position);

        bool expanded_beyond_word_boundary;
        if (selection_expanded) {
            expanded_beyond_word_boundary = ArePositionsInSpecifiedOrder(new_offset_focus_position, word_boundary, new_focus_anchor_order);
        ...
        this_move_shrunk_selection = !extent_base_order_switched && !selection_expanded;
    ...
    if (granularity_ == TextGranularity::kWord) {
        ...
        VisiblePosition bound_before_extent = CreateVisiblePosition(
            NextWordBound(new_offset_focus_position.DeepEquivalent(), SearchDirection::kSearchBackwards, BoundAdjust::kCurrentPosIfOnBound));
        if (bound_before_extent.IsNull())
            return selection.AsSelection();
        VisiblePosition bound_after_extent = CreateVisiblePosition(
            NextWordBound(new_offset_focus_position.DeepEquivalent(), SearchDirection::kSearchForward, BoundAdjust::kCurrentPosIfOnBound));
        if (bound_after_extent.IsNull())
            return selection.AsSelection();
        int x_middle_between_bounds = (PositionLocation(bound_after_extent).x() + PositionLocation(bound_before_extent).x()) / 2;
        bool offset_extent_before_middle = new_offset_focus_point.x() < x_middle_between_bounds;
        new_selection_extent = offset_extent_before_middle ? bound_before_extent : bound_after_extent;
        // Update the offset if selection expanded in word granularity.
        if (new_selection_extent.DeepEquivalent() != old_offset_focus_position.DeepEquivalent()
            && ((new_focus_anchor_order > 0 && !offset_extent_before_middle) || (new_focus_anchor_order < 0 && offset_extent_before_middle))) {
    ...
    // this move.
    if (new_selection_extent.DeepEquivalent() != old_offset_focus_position.DeepEquivalent()) {
        state_ = this_move_shrunk_selection ? StrategyState::kShrinking : StrategyState::kExpanding;
    }

    diff_extent_point_from_extent_position_ = extent_point + gfx::Vector2d(offset_, 0) - PositionLocation(new_selection_extent);
    return SelectionInDOMTree::Builder(selection.AsSelection()).Collapse(selection.Anchor()).Extend(new_selection_extent.DeepEquivalent()).Build();
}
