...
{
    SetBodyContent("<div id=target>abc"
...
{
    InsertStyleElement("div { display: list-item; overflow: hidden; }"
...
public:
    explicit TextVisitorImpl(VisitedNodes& nodes)
        : nodes_(nodes)
    {
    }
...
{
    SetBodyContent("<select id='0'><optgroup id='1'><option "
    ...
    // id from 0-4.
    for (int i = 0; i < 4; ++i) {
        Element* element = GetDocument().getElementById(AtomicString(String::Number(i)));
