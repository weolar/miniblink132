...
template <typename Traversal> int EditingAlgorithm<Traversal>::CaretMaxOffset(const Node& node)
{
    ...
    if (auto* text = DynamicTo<Text>(node); text && text->GetLayoutObject()) {
        ...
        // ::first-letter
        if (auto* first_letter_remaining_part = DynamicTo<LayoutTextFragment>(layout_object)) {
            return first_letter_remaining_part->Start() + first_letter_remaining_part->CaretMaxOffset();
        }
...
template <typename Traversal> int EditingAlgorithm<Traversal>::LastOffsetForEditing(const Node* node)
{
    DCHECK(node);
    if (!node)
        return 0;
...
template <typename Strategy> Node* EditingAlgorithm<Strategy>::RootUserSelectAllForNode(Node* node)
{
