...
protected:
    ...
    std::string GetRange(const TextOffsetMapping::InlineContents& contents)
    {
        ...
        return GetSelectionTextInFlatTreeFromBody(SelectionInFlatTree::Builder().SetBaseAndExtent(mapping.GetRange()).Build());
    }

    std::string GetPositionBefore(const std::string& html_text, int offset)
    {
        SetBodyContent(html_text);
        TextOffsetMapping mapping(GetInlineContents(PositionInFlatTree(*GetDocument().body()->firstChild(), 0)));
        return GetSelectionTextInFlatTreeFromBody(SelectionInFlatTree::Builder().Collapse(mapping.GetPositionBefore(offset)).Build());
    }
    ...
    std::string GetPositionAfter(const std::string& html_text, int offset)
    {
        SetBodyContent(html_text);
        TextOffsetMapping mapping(GetInlineContents(PositionInFlatTree(*GetDocument().body()->firstChild(), 0)));
        return GetSelectionTextInFlatTreeFromBody(SelectionInFlatTree::Builder().Collapse(mapping.GetPositionAfter(offset)).Build());
    }

private:
    static TextOffsetMapping::InlineContents GetInlineContents(const PositionInFlatTree& position)
    {
        const TextOffsetMapping::InlineContents inline_contents = TextOffsetMapping::FindForwardInlineContents(position);
...
{
    EXPECT_EQ("<div><p>abc</p>^def|<p>ghi</p></div>", GetRange("<div><p>abc</p>d|ef<p>ghi</p></div>"));
}

TEST_F(TextOffsetMappingTest, RangeOfBlockOnInlineBlock)
{
    // display:inline-block doesn't introduce block.
    EXPECT_EQ("^abc<p style=\"display:inline\">def<br>ghi</p>xyz|", GetRange("|abc<p style=display:inline>def<br>ghi</p>xyz"));
    EXPECT_EQ("^abc<p style=\"display:inline\">def<br>ghi</p>xyz|", GetRange("abc<p style=display:inline>|def<br>ghi</p>xyz"));
}
...
{
    EXPECT_EQ("^abc<br>xyz|", GetRange("abc|<br>xyz")) << "BR doesn't affect block";
}

TEST_F(TextOffsetMappingTest, RangeOfBlockWithPRE)
{
    // "\n" doesn't affect block.
    EXPECT_EQ("<pre>^abc\ndef\nghi\n|</pre>", GetRange("<pre>|abc\ndef\nghi\n</pre>"));
    EXPECT_EQ("<pre>^abc\ndef\nghi\n|</pre>", GetRange("<pre>abc\n|def\nghi\n</pre>"));
    EXPECT_EQ("<pre>^abc\ndef\nghi\n|</pre>", GetRange("<pre>abc\ndef\n|ghi\n</pre>"));
    EXPECT_EQ("<pre>^abc\ndef\nghi\n|</pre>", GetRange("<pre>abc\ndef\nghi\n|</pre>"));
}
...
{
    const char* whole_text_selected = "^<ruby>abc<br>def<rt>123<br>456|</rt></ruby>";
    EXPECT_EQ(whole_text_selected, GetRange("<ruby>|abc<br>def<rt>123<br>456</rt></ruby>")) << "RT(LayoutRubyColumn) is a block";
    EXPECT_EQ(whole_text_selected, GetRange("<ruby>abc<br>def<rt>123|<br>456</rt></ruby>")) << "RUBY introduce LayoutRuleBase for 'abc'";
}

TEST_F(TextOffsetMappingTest, RangeOfBlockWithTABLE)
{
    EXPECT_EQ("^abc|<table><tbody><tr><td>one</td></tr></tbody></table>xyz", GetRange("|abc<table><tr><td>one</td></tr></table>xyz")) << "Before TABLE";
    EXPECT_EQ("abc<table><tbody><tr><td>^one|</td></tr></tbody></table>xyz", GetRange("abc<table><tr><td>o|ne</td></tr></table>xyz")) << "In TD";
    EXPECT_EQ("abc<table><tbody><tr><td>one</td></tr></tbody></table>^xyz|", GetRange("abc<table><tr><td>one</td></tr></table>x|yz")) << "After TABLE";
}
...
{
    const PositionInFlatTree position = ToPositionInFlatTree(SetSelectionTextToBody("<div><p>abc</p><p id='target'>|</p><p>ghi</p></div>").Anchor());
    const LayoutObject* const target_layout_object = GetDocument().getElementById(AtomicString("target"))->GetLayoutObject();
    const TextOffsetMapping::InlineContents inline_contents = TextOffsetMapping::FindForwardInlineContents(position);
    ASSERT_TRUE(inline_contents.IsNotNull());
    EXPECT_EQ(target_layout_object, inline_contents.GetEmptyBlock());
    EXPECT_EQ(inline_contents, TextOffsetMapping::FindBackwardInlineContents(position));
}
...
TEST_F(TextOffsetMappingTest, AnonymousBlockFlowWrapperForFloatPseudo)
{
    ...
    const PositionInFlatTree position = ToPositionInFlatTree(SetCaretTextToBody("<table></table>|foo"));
    const TextOffsetMapping::InlineContents inline_contents = TextOffsetMapping::FindBackwardInlineContents(position);
    ASSERT_TRUE(inline_contents.IsNotNull());
    const TextOffsetMapping::InlineContents previous_contents = TextOffsetMapping::InlineContents::PreviousOf(inline_contents);
...
TEST_F(TextOffsetMappingTest, BlockInInlineWithAbsolute)
{
    ...
    const PositionInFlatTree position = ToPositionInFlatTree(SetCaretTextToBody("<div id=t><i><p><a></a></p></i> </div><p>|ab</p>"));

    Vector<String> results;
    for (const auto contents : TextOffsetMapping::BackwardRangeOf(position))
        results.push_back(GetRange(contents));

    ElementsAre("<div id=\"t\"><i><p><a></a></p></i> </div><p>^ab|</p>", "<div id=\"t\"><i><p><a></a></p></i>^ |</div><p>ab</p>",
...
TEST_F(TextOffsetMappingTest, ForwardRangesWithTextControl)
{
    ...
    // containing block.
    const PositionInFlatTree outside_position = ToPositionInFlatTree(SetCaretTextToBody("foo<!--|--><input value=\"bla\">bar"));
    const TextOffsetMapping::InlineContents outside_contents = TextOffsetMapping::FindForwardInlineContents(outside_position);
    EXPECT_EQ("^foo<input value=\"bla\"><div>bla</div></input>bar|", GetRange(outside_contents));
    ...
    const PositionInFlatTree inside_first = PositionInFlatTree::FirstPositionInNode(*input);
    const TextOffsetMapping::InlineContents inside_contents = TextOffsetMapping::FindForwardInlineContents(inside_first);
    EXPECT_EQ("foo<input value=\"bla\"><div>^bla|</div></input>bar", GetRange(inside_contents));
    EXPECT_TRUE(TextOffsetMapping::InlineContents::NextOf(inside_contents).IsNull());

    const PositionInFlatTree inside_last = PositionInFlatTree::LastPositionInNode(*input);
    EXPECT_TRUE(TextOffsetMapping::FindForwardInlineContents(inside_last).IsNull());
}
...
TEST_F(TextOffsetMappingTest, BackwardRangesWithTextControl)
{
    ...
    // containing block.
    const PositionInFlatTree outside_position = ToPositionInFlatTree(SetCaretTextToBody("foo<input value=\"bla\"><!--|-->bar"));
    const TextOffsetMapping::InlineContents outside_contents = TextOffsetMapping::FindBackwardInlineContents(outside_position);
    EXPECT_EQ("^foo<input value=\"bla\"><div>bla</div></input>bar|", GetRange(outside_contents));
    ...
    const Element* input = GetDocument().QuerySelector(AtomicString("input"));
    const PositionInFlatTree inside_last = PositionInFlatTree::LastPositionInNode(*input);
    const TextOffsetMapping::InlineContents inside_contents = TextOffsetMapping::FindBackwardInlineContents(inside_last);
    EXPECT_EQ("foo<input value=\"bla\"><div>^bla|</div></input>bar", GetRange(inside_contents));
    EXPECT_TRUE(TextOffsetMapping::InlineContents::PreviousOf(inside_contents).IsNull());

    const PositionInFlatTree inside_first = PositionInFlatTree::FirstPositionInNode(*input);
    EXPECT_TRUE(TextOffsetMapping::FindBackwardInlineContents(inside_first).IsNull());
}

// http://crbug.com/1295233
TEST_F(TextOffsetMappingTest, RangeWithBlockInInline)
{
    EXPECT_EQ("<div><p>ab</p><b><p>cd</p></b>^yz|</div>", GetRange("<div><p>ab</p><b><p>cd</p></b>|yz</div>"));
}
...
TEST_F(TextOffsetMappingTest, RangeWithNestedFloat)
{
    ...
    EXPECT_EQ("<b>abc <i>^def|</i> ghi</b>xyz", GetRange("<b>abc <i>d|ef</i> ghi</b>xyz"));
}
...
TEST_F(TextOffsetMappingTest, RangeWithFloatingListItem)
{
    ...
    EXPECT_EQ("<ul><li>^First|</li><li>Second</li></ul>", GetRange("<ul><li>|First</li><li>Second</li></ul>"));
    EXPECT_EQ("<ul><li>^First|</li><li>Second</li></ul>", GetRange("<ul><li>F|irst</li><li>Second</li></ul>"));
    EXPECT_EQ("<ul><li>^First|</li><li>Second</li></ul>", GetRange("<ul><li>Fir|st</li><li>Second</li></ul>"));
    EXPECT_EQ("<ul><li>^First|</li><li>Second</li></ul>", GetRange("<ul><li>Firs|t</li><li>Second</li></ul>"));
    EXPECT_EQ("<ul><li>^First|</li><li>Second</li></ul>", GetRange("<ul><li>First|</li><li>Second</li></ul>"));
    EXPECT_EQ("<ul><li>First</li><li>^Second|</li></ul>", GetRange("<ul><li>First</li><li>|Second</li></ul>"));
    EXPECT_EQ("<ul><li>First</li><li>^Second|</li></ul>", GetRange("<ul><li>First</li><li>S|econd</li></ul>"));
    EXPECT_EQ("<ul><li>First</li><li>^Second|</li></ul>", GetRange("<ul><li>First</li><li>Se|cond</li></ul>"));
    EXPECT_EQ("<ul><li>First</li><li>^Second|</li></ul>", GetRange("<ul><li>First</li><li>Sec|ond</li></ul>"));
    EXPECT_EQ("<ul><li>First</li><li>^Second|</li></ul>", GetRange("<ul><li>First</li><li>Seco|nd</li></ul>"));
    EXPECT_EQ("<ul><li>First</li><li>^Second|</li></ul>", GetRange("<ul><li>First</li><li>Secon|d</li></ul>"));
    EXPECT_EQ("<ul><li>First</li><li>^Second|</li></ul>", GetRange("<ul><li>First</li><li>Second|</li></ul>"));
}
...
TEST_F(TextOffsetMappingTest, RangeWithInlineBlockBlocks)
{
    ...
    EXPECT_EQ("^a<b><p>b</p><p>c</p></b>d|", GetRange("|a<b><p>b</p><p>c</p></b>d"));
    EXPECT_EQ("^a<b><p>b</p><p>c</p></b>d|", GetRange("a|<b><p>b</p><p>c</p></b>d"));
    EXPECT_EQ("a<b><p>^b|</p><p>c</p></b>d", GetRange("a<b>|<p>b</p><p>c</p></b>d"));
    EXPECT_EQ("a<b><p>^b|</p><p>c</p></b>d", GetRange("a<b><p>|b</p><p>c</p></b>d"));
    EXPECT_EQ("a<b><p>^b|</p><p>c</p></b>d", GetRange("a<b><p>b|</p><p>c</p></b>d"));
...
TEST_F(TextOffsetMappingTest, RangeWithSelect1)
{
    ...
    const auto& expected_outer = "^<select>"
                                 ...
                                 "</div>"
                                 "</select>foo|";
    const auto& expected_inner = "<select>"
                                 "<div aria-hidden=\"true\">^|</div>"
                                 "<slot id=\"select-options\"></slot>"
                                 "<slot id=\"select-button\"></slot>"
                                 "<div popover=\"auto\" pseudo=\"picker(select)\">"
...
<div id="div2">xyz</div>
    ...
    const TextOffsetMapping::InlineContents& div1_contents = TextOffsetMapping::FindForwardInlineContents(PositionInFlatTree::FirstPositionInNode(div1));
    EXPECT_EQ(div1.firstChild()->GetLayoutObject(), div1_contents.FirstLayoutObject());
    EXPECT_EQ(div1.lastChild()->GetLayoutObject(), div1_contents.LastLayoutObject());

    const TextOffsetMapping::InlineContents& div2_contents = TextOffsetMapping::InlineContents::NextOf(div1_contents);
    EXPECT_EQ(div2.firstChild()->GetLayoutObject(), div2_contents.FirstLayoutObject());
    EXPECT_EQ(div2.lastChild()->GetLayoutObject(), div2_contents.LastLayoutObject());
}
...
TEST_F(TextOffsetMappingTest, InlineContentsWithDocumentBoundary)
{
    ...
    SetBodyContent("");
    const PositionInFlatTree position = PositionInFlatTree::FirstPositionInNode(*GetDocument().body());
    const TextOffsetMapping::InlineContents inline_contents = TextOffsetMapping::FindForwardInlineContents(position);
    EXPECT_TRUE(inline_contents.IsNotNull());
    // Should not crash when previous/next iteration reaches document boundary.
    EXPECT_TRUE(TextOffsetMapping::InlineContents::PreviousOf(inline_contents).IsNull());
    EXPECT_TRUE(TextOffsetMapping::InlineContents::NextOf(inline_contents).IsNull());
}
...
TEST_F(TextOffsetMappingTest, ComputeTextOffsetWithBrokenImage)
{
    ...
    DCHECK(shadow);
    const Element* alt_img = shadow->getElementById(AtomicString("alttext-image"));
    DCHECK(alt_img);
    ...
    for (const TextOffsetMapping::InlineContents& inline_contents :
        { TextOffsetMapping::FindForwardInlineContents(position), TextOffsetMapping::FindBackwardInlineContents(position) }) {
