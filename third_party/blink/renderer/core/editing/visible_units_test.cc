...
PositionWithAffinity PositionWithAffinityInDOMTree(Node& anchor, int offset, TextAffinity affinity = TextAffinity::kDownstream)
{
    return PositionWithAffinity(CanonicalPositionOf(Position(&anchor, offset)), affinity);
}

VisiblePosition CreateVisiblePositionInDOMTree(Node& anchor, int offset, TextAffinity affinity = TextAffinity::kDownstream)
{
    return CreateVisiblePosition(Position(&anchor, offset), affinity);
}

PositionInFlatTreeWithAffinity PositionWithAffinityInFlatTree(Node& anchor, int offset, TextAffinity affinity = TextAffinity::kDownstream)
{
    return PositionInFlatTreeWithAffinity(CanonicalPositionOf(PositionInFlatTree(&anchor, offset)), affinity);
}

VisiblePositionInFlatTree CreateVisiblePositionInFlatTree(Node& anchor, int offset, TextAffinity affinity = TextAffinity::kDownstream)
{
...
protected:
    std::string TestSnapBackward(const std::string& selection_text, EditingBoundaryCrossingRule rule = kCannotCrossEditingBoundary)
    {
    ...
    std::string TestSnapForward(const std::string& selection_text, EditingBoundaryCrossingRule rule = kCannotCrossEditingBoundary)
    {
        const Position position = SetCaretTextToBody(selection_text);
...
{
    const char* body_content = "<style>#one:first-letter { font-size: 200%; }</style><p id=one>one</p>";
    SetBodyContent(body_content);
...
{
    const char* body_content = "<p id='host'><b slot='#one' id='one'>1</b><b slot='#two' "
                               "id='two'>22</b></p><b "
                               "id='three'>333</b>";
    const char* shadow_content = "<b id='four'>4444</b><slot name='#two'></slot><slot name=#one></slot><b "
                                 "id='five'>5555</b>";
    SetBodyContent(body_content);
    ...
    EXPECT_EQ(0, CharacterAfter(CreateVisiblePositionInDOMTree(*one->firstChild(), 1)));
    EXPECT_EQ('5', CharacterAfter(CreateVisiblePositionInFlatTree(*one->firstChild(), 1)));

    EXPECT_EQ('1', CharacterAfter(CreateVisiblePositionInDOMTree(*two->firstChild(), 2)));
...
{
    LoadAhem();
    InsertStyleElement("body { font: 10px/15px Ahem; }"
                       "b::before { content: '\\u200B'");
...
{
    const char* body_content = "<html><div id=one contenteditable>1</div><span id=two "
    ...
    EXPECT_EQ(Position(), CanonicalPositionOf(Position(GetDocument().documentElement(), 0)));
...
TEST_F(VisibleUnitsTest, canonicalPositionOfWithInputElement)
{
    ...
    EXPECT_EQ(Position::BeforeNode(*input), CanonicalPositionOf(Position::FirstPositionInNode(*GetDocument().documentElement())));

    EXPECT_EQ(PositionInFlatTree::BeforeNode(*input), CanonicalPositionOf(PositionInFlatTree::FirstPositionInNode(*GetDocument().documentElement())));
}

// http://crbug.com/1116214
TEST_F(VisibleUnitsTest, canonicalPositionOfWithCrossBlockFlowlement)
{
    const char* body_content = "<div id=one>line1<span>X</span><div>line2</div></div>"
                               "<div id=two>line3"
    ...
    EXPECT_EQ(Position::LastPositionInNode(*two->firstChild()), CanonicalPositionOf(two_text_pos));

    EXPECT_EQ(Position(*three->lastChild()->previousSibling(), 0), CanonicalPositionOf(three_text_pos));
}
...
{
    const char* body_content = "<span id=host><b slot='#one' id=one>1</b><b slot='#two' "
                               "id=two>22</b></span>";
    const char* shadow_content = "<p><slot name='#two'></slot></p><p><slot name=#one></slot></p>";
    SetBodyContent(body_content);
    SetShadowContent(shadow_content, "host");
...
TEST_F(VisibleUnitsTest, AdjustForwardPositionToAvoidCrossingEditingBoundariesNestedEditable)
{
    const SelectionInDOMTree& selection = SetSelectionTextToBody("<div contenteditable>"
                                                                 "abc"
                                                                 "<span contenteditable=\"false\">A^BC</span>"
                                                                 "d|ef"
                                                                 "</div>");
    const PositionWithAffinity& result = AdjustForwardPositionToAvoidCrossingEditingBoundaries(PositionWithAffinity(selection.Focus()), selection.Anchor());
    ASSERT_TRUE(result.IsNotNull());
    EXPECT_EQ("<div contenteditable>"
              "abc"
...
{
    const char* body_content = "<span id=host><b slot='#one' id=one contenteditable>1</b><b slot='#two' "
                               "id=two>22</b></span>";
    const char* shadow_content = "<slot name='#two'></slot></p><p><slot name='#one'></slot>";
    SetBodyContent(body_content);
    SetShadowContent(shadow_content, "host");
...
TEST_F(VisibleUnitsTest, isEndOfEditableOrNonEditableContentWithInput)
{
    ...
    Node* text = ToTextControl(GetDocument().getElementById(AtomicString("sample")))->InnerEditorElement()->firstChild();
...
TEST_F(VisibleUnitsTest, IsVisuallyEquivalentCandidateWithHTMLHtmlElement)
{
    ...
    EXPECT_FALSE(IsVisuallyEquivalentCandidate(Position(GetDocument().documentElement(), 0)));
...
{
    const char* body_content = "<div id=one contenteditable>1</div><span id=two "
                               "contenteditable=false>22</span><span id=three "
                               "contenteditable=false>333</span><span id=four "
...
{
    const char* body_content = "<p id='host'><b id='one'>1</b></p><b id='two'>22</b>";
...
TEST_F(VisibleUnitsTest, mostBackwardCaretPositionFirstLetter)
{
    ...
    const char* body_content = "<style>p:first-letter {color:red;}</style><p id=sample> (2)45 </p>";
    SetBodyContent(body_content);

    Node* sample = GetDocument().getElementById(AtomicString("sample"))->firstChild();
    ...
    EXPECT_EQ(Position::LastPositionInNode(*GetDocument().body()), MostBackwardCaretPosition(Position::LastPositionInNode(*GetDocument().body())));
}
...
TEST_F(VisibleUnitsTest, mostBackwardCaretPositionFirstLetterSplit)
{
    ...
    const char* body_content = "<style>p:first-letter {color:red;}</style><p id=sample>abc</p>";
    SetBodyContent(body_content);
    ...
    EXPECT_EQ(Position(sample, 0), MostBackwardCaretPosition(Position(first_letter, 0)));
    EXPECT_EQ(Position(first_letter, 1), MostBackwardCaretPosition(Position(first_letter, 1)));
    EXPECT_EQ(Position(first_letter, 1), MostBackwardCaretPosition(Position(remaining, 0)));
...
TEST_F(VisibleUnitsTest, mostForwardCaretPositionAfterAnchor)
{
    ...
    EXPECT_EQ(Position::AfterNode(*host), MostBackwardCaretPosition(Position::AfterNode(*host)));
...
{
    EXPECT_EQ("<div>A<svg><foreignObject height=\"10\" width=\"20\">| "
...
TEST_F(VisibleUnitsTest, mostForwardCaretPositionFirstLetter)
{
    ...
    EXPECT_EQ(Position(GetDocument().body(), 0), MostForwardCaretPosition(Position::FirstPositionInNode(*GetDocument().body())));
...
{
    const char* body_content = "<b id=zero>0</b><p id=host><b slot='#one' id=one>1</b><b slot='#two' "
                               "id=two>22</b></p><b "
                               "id=three>333</b>";
    const char* shadow_content = "<b id=four>4444</b><slot name='#two'></slot><slot name=#one></slot><b "
                                 "id=five>55555</b>";
    ...
    EXPECT_EQ(Position(two->firstChild(), 2), NextPositionOf(CreateVisiblePosition(Position(zero, 1))).DeepEquivalent());
    ...
    EXPECT_EQ(Position(three->firstChild(), 0), NextPositionOf(CreateVisiblePosition(Position(one, 0), TextAffinity::kUpstream)).DeepEquivalent());
    ...
    EXPECT_EQ(Position(two->firstChild(), 0), NextPositionOf(CreateVisiblePosition(Position(one, 1), TextAffinity::kUpstream)).DeepEquivalent());
    ...
    EXPECT_EQ(Position(three->firstChild(), 0), NextPositionOf(CreateVisiblePosition(Position(two, 1))).DeepEquivalent());
    EXPECT_EQ(PositionInFlatTree(one->firstChild(), 1), NextPositionOf(CreateVisiblePosition(PositionInFlatTree(two, 1))).DeepEquivalent());
}
...
TEST_F(VisibleUnitsTest, nextPositionOfTable)
{
    ...
    Position next = NextPositionOf(CreateVisiblePosition(position)).DeepEquivalent();
    EXPECT_NE(position, next);
    EXPECT_NE(MostBackwardCaretPosition(position), MostBackwardCaretPosition(next));
...
TEST_F(VisibleUnitsTest, previousPositionOf)
{
    ...
    Node* one = GetDocument().getElementById(AtomicString("one"))->firstChild();
    Node* two = GetDocument().getElementById(AtomicString("two"))->firstChild();
    Node* three = GetDocument().getElementById(AtomicString("three"))->firstChild();
    ...
    EXPECT_EQ(Position(two, 1), PreviousPositionOf(CreateVisiblePosition(Position(one, 0))).DeepEquivalent());
    EXPECT_EQ(PositionInFlatTree(two, 1), PreviousPositionOf(CreateVisiblePosition(PositionInFlatTree(one, 0))).DeepEquivalent());

    EXPECT_EQ(Position(two, 2), PreviousPositionOf(CreateVisiblePosition(Position(one, 1))).DeepEquivalent());
    EXPECT_EQ(PositionInFlatTree(two, 2), PreviousPositionOf(CreateVisiblePosition(PositionInFlatTree(one, 1))).DeepEquivalent());

    EXPECT_EQ(Position(one, 1), PreviousPositionOf(CreateVisiblePosition(Position(two, 0))).DeepEquivalent());
    ...
    // (one, 1) in flat tree.
    EXPECT_EQ(Position(five, 0), PreviousPositionOf(CreateVisiblePosition(Position(five, 1))).DeepEquivalent());
...
{
    const char* body_content = "<div id=sample style='font-size: 500px'>A&#x714a;&#xfa67;</div>";
    SetBodyContent(body_content);
    ...
    EXPECT_EQ(PositionWithAffinity(Position(sample, 1)),
        PreviousPositionOf(CreateVisiblePosition(Position(sample, 2), TextAffinity::kUpstream)).ToPositionWithAffinity());
}

TEST_F(VisibleUnitsTest, previousPositionOfNoPreviousPosition)
{
    SetBodyContent("<span contenteditable='true'>"
                   "<span> </span>"
                   " " // This whitespace causes no previous position.
                   "<div id='anchor'> bar</div>"
                   "</span>");
    const Position position(GetDocument().getElementById(AtomicString("anchor"))->firstChild(), 1);
...
TEST_F(VisibleUnitsTest, rendersInDifferentPositionAfterAnchor)
{
    ...
    EXPECT_FALSE(RendersInDifferentPosition(Position(), Position::AfterNode(*sample))) << "if one of position is null, the reuslt is false.";
...
{
    const char* body_content = "<p><span id=one>11</span><span id=two style='display:none'>  "
                               "</span></p>";
    SetBodyContent(body_content);
    Element* one = GetDocument().getElementById(AtomicString("one"));
    Element* two = GetDocument().getElementById(AtomicString("two"));

    EXPECT_TRUE(RendersInDifferentPosition(Position::LastPositionInNode(*one), Position(two, 0))) << "two doesn't have layout object";
}

TEST_F(VisibleUnitsTest, rendersInDifferentPositionAfterAnchorWithDifferentLayoutObjects)
{
    const char* body_content = "<p><span id=one>11</span><span id=two>  </span></p>";
    SetBodyContent(body_content);
    Element* one = GetDocument().getElementById(AtomicString("one"));
    Element* two = GetDocument().getElementById(AtomicString("two"));
...
{
    const char* body_content = "<div contenteditable><span id='sample1'>1</span><span "
...
TEST_F(VisibleUnitsTest, startOfDocument)
{
    ...
    EXPECT_EQ(Position(one, 0), CreateVisiblePosition(StartOfDocument(Position(*one, 0))).DeepEquivalent());
    EXPECT_EQ(PositionInFlatTree(two, 0), CreateVisiblePosition(StartOfDocument(PositionInFlatTree(*one, 0))).DeepEquivalent());

    EXPECT_EQ(Position(one, 0), CreateVisiblePosition(StartOfDocument(Position(*two, 1))).DeepEquivalent());
    EXPECT_EQ(PositionInFlatTree(two, 0), CreateVisiblePosition(StartOfDocument(PositionInFlatTree(*two, 1))).DeepEquivalent());
}

TEST_F(VisibleUnitsTest, endsOfNodeAreVisuallyDistinctPositionsWithInvisibleChild)
{
    // Repro case of crbug.com/582247
    const char* body_content = "<button> </button><script>document.designMode = 'on'</script>";
    SetBodyContent(body_content);
...
TEST_F(VisibleUnitsTest, endsOfNodeAreVisuallyDistinctPositionsWithEmptyLayoutChild)
{
    ...
    const char* body_content = "<button><rt><script>document.designMode = 'on'</script></rt></button>";
    SetBodyContent(body_content);
...
TEST_F(VisibleUnitsTest, canonicalizationWithCollapsedSpaceAndIsolatedCombiningCharacter)
{
...
TEST_F(VisibleUnitsTest, MostForwardCaretPositionWithInvisibleFirstLetter)
{
    ...
    const Node* foo = GetDocument().QuerySelector(AtomicString("div"))->firstChild();
...
TEST_F(VisibleUnitsTest, SnapBackwardWithZeroWidthSpace)
{
    ...
    EXPECT_EQ("<p>ab|<wbr><wbr>cd</p>", TestSnapBackward("<p>ab<wbr><wbr>|cd</p>"));
    EXPECT_EQ("<p>ab\u200B\u200B|cd</p>", TestSnapBackward("<p>ab\u200B\u200B|cd</p>"));
}
TEST_F(VisibleUnitsTest, SnapForwardWithImg)
{
    ...
    EXPECT_EQ(Position::BeforeNode(img), MostForwardCaretPosition(Position::FirstPositionInNode(body)));
    EXPECT_EQ(Position::BeforeNode(img), MostForwardCaretPosition(Position(body, 0)));
    EXPECT_EQ(Position::BeforeNode(img), MostForwardCaretPosition(Position::BeforeNode(img)));
    EXPECT_EQ(Position::BeforeNode(img), MostForwardCaretPosition(Position(img, 0)));
    EXPECT_EQ(Position::AfterNode(img), MostForwardCaretPosition(Position::LastPositionInNode(img)));
    EXPECT_EQ(Position::AfterNode(img), MostForwardCaretPosition(Position::AfterNode(img)));
}
...
TEST_F(VisibleUnitsTest, SnapForwardWithInput)
{
    ...
    EXPECT_EQ(Position::BeforeNode(input), MostForwardCaretPosition(Position::FirstPositionInNode(body)));
    EXPECT_EQ(Position::BeforeNode(input), MostForwardCaretPosition(Position(body, 0)));
    EXPECT_EQ(Position::BeforeNode(input), MostForwardCaretPosition(Position::BeforeNode(input)));
    EXPECT_EQ(Position::BeforeNode(input), MostForwardCaretPosition(Position::FirstPositionInNode(input)));
    EXPECT_EQ(Position::BeforeNode(input), MostForwardCaretPosition(Position(input, 0)));
    EXPECT_EQ(Position::AfterNode(input), MostForwardCaretPosition(Position::LastPositionInNode(input)));
    EXPECT_EQ(Position::AfterNode(input), MostForwardCaretPosition(Position::AfterNode(input)));
}

TEST_F(VisibleUnitsTest, SnapForwardWithSelect)
{
    SetBodyContent("<select><option>1</option><option>2</option><option>3</option></"
                   "select>");
    const auto& body = *GetDocument().body();
    const auto& select = *GetDocument().QuerySelector(AtomicString("select"));

    EXPECT_EQ(Position::BeforeNode(select), MostForwardCaretPosition(Position(body, 0)));
    EXPECT_EQ(Position::BeforeNode(select), MostForwardCaretPosition(Position::FirstPositionInNode(body)));
    EXPECT_EQ(Position::BeforeNode(select), MostForwardCaretPosition(Position::BeforeNode(select)));
    EXPECT_EQ(Position::BeforeNode(select), MostForwardCaretPosition(Position::FirstPositionInNode(select)));
    EXPECT_EQ(Position::BeforeNode(select), MostForwardCaretPosition(Position(select, 0)));
    ...
    EXPECT_EQ(Position::AfterNode(select), MostForwardCaretPosition(Position(select, 3)));
    EXPECT_EQ(Position::AfterNode(select), MostForwardCaretPosition(Position::LastPositionInNode(select)));
    EXPECT_EQ(Position::AfterNode(select), MostForwardCaretPosition(Position::AfterNode(select)));
    ...
    //  </select>
    EXPECT_EQ(PositionInFlatTree::BeforeNode(select), MostForwardCaretPosition(PositionInFlatTree(body, 0)));
    EXPECT_EQ(PositionInFlatTree::BeforeNode(select), MostForwardCaretPosition(PositionInFlatTree::FirstPositionInNode(body)));
    EXPECT_EQ(PositionInFlatTree::BeforeNode(select), MostForwardCaretPosition(PositionInFlatTree::BeforeNode(select)));
    ...
    EXPECT_EQ(PositionInFlatTree::BeforeNode(select), MostForwardCaretPosition(PositionInFlatTree::FirstPositionInNode(select)));
    EXPECT_EQ(PositionInFlatTree::BeforeNode(select), MostForwardCaretPosition(PositionInFlatTree(select, 0)));
    EXPECT_EQ(PositionInFlatTree::BeforeNode(select), MostForwardCaretPosition(PositionInFlatTree(select, 1)));
    EXPECT_EQ(PositionInFlatTree::BeforeNode(select), MostForwardCaretPosition(PositionInFlatTree(select, 2)));
    EXPECT_EQ(PositionInFlatTree::BeforeNode(select), MostForwardCaretPosition(PositionInFlatTree(select, 3)));
    EXPECT_EQ(PositionInFlatTree::BeforeNode(select), MostForwardCaretPosition(PositionInFlatTree(select, 4)));
    EXPECT_EQ(PositionInFlatTree::AfterNode(select), MostForwardCaretPosition(PositionInFlatTree(select, 5)));

    EXPECT_EQ(PositionInFlatTree::AfterNode(select), MostForwardCaretPosition(PositionInFlatTree::LastPositionInNode(select)));
    EXPECT_EQ(PositionInFlatTree::AfterNode(select), MostForwardCaretPosition(PositionInFlatTree::AfterNode(select)));
}
...
TEST_F(VisibleUnitsTest, SnapForwardWithTableAndImages)
{
    ...
    EXPECT_EQ(Position(body, 0), MostForwardCaretPosition(Position::FirstPositionInNode(body)));
    EXPECT_EQ(Position(table, 0), MostForwardCaretPosition(Position::BeforeNode(table)));
    EXPECT_EQ(Position(table, 0), MostForwardCaretPosition(Position::FirstPositionInNode(table)));
    ...
    EXPECT_EQ(Position::BeforeNode(img1), MostForwardCaretPosition(Position::BeforeNode(tbody)));
    EXPECT_EQ(Position::BeforeNode(img1), MostForwardCaretPosition(Position(tbody, 0)));
    EXPECT_EQ(Position::BeforeNode(img1), MostForwardCaretPosition(Position::FirstPositionInNode(tbody)));
    EXPECT_EQ(Position::BeforeNode(img2), MostForwardCaretPosition(Position(tbody, 1)));
    EXPECT_EQ(Position::LastPositionInNode(tbody), MostForwardCaretPosition(Position(tbody, 2)));
    EXPECT_EQ(Position::LastPositionInNode(tbody), MostForwardCaretPosition(Position::LastPositionInNode(tbody)));
    EXPECT_EQ(Position::LastPositionInNode(tbody), MostForwardCaretPosition(Position::AfterNode(tbody)));
    EXPECT_EQ(Position(table, 2), MostForwardCaretPosition(Position(table, 2)));
    EXPECT_EQ(Position::LastPositionInNode(table), MostForwardCaretPosition(Position(table, 3)));
    EXPECT_EQ(Position::LastPositionInNode(table), MostForwardCaretPosition(Position::LastPositionInNode(table)));
    EXPECT_EQ(Position::LastPositionInNode(table), MostForwardCaretPosition(Position::AfterNode(table)));
}
...
TEST_F(VisibleUnitsTest, SnapForwardWithZeroWidthSpace)
{
    ...
    EXPECT_EQ("<p>ab<wbr></p>", TestSnapForward("<p>ab|<wbr></p>")) << "We get <wbr>@0";
    EXPECT_EQ("<p>ab|\u200B</p>", TestSnapForward("<p>ab|\u200B</p>"));
    EXPECT_EQ("<p>ab<!-- -->|\u200B</p>", TestSnapForward("<p>ab<!-- -->|\u200B</p>"));

    EXPECT_EQ("<p>ab<wbr><wbr></p>", TestSnapForward("<p>ab|<wbr><wbr></p>")) << "We get <wbr>@0";
    EXPECT_EQ("<p>ab|\u200B\u200B</p>", TestSnapForward("<p>ab|\u200B\u200B</p>"));

    EXPECT_EQ("<p>ab<wbr>|cd</p>", TestSnapForward("<p>ab|<wbr>cd</p>"));
    EXPECT_EQ("<p>ab|\u200Bcd</p>", TestSnapForward("<p>ab|\u200Bcd</p>"));

    EXPECT_EQ("<p>ab<wbr><wbr>|cd</p>", TestSnapForward("<p>ab|<wbr><wbr>cd</p>"));
    EXPECT_EQ("<p>ab|\u200B\u200Bcd</p>", TestSnapForward("<p>ab|\u200B\u200Bcd</p>"));
}
...
TEST_F(VisibleUnitsTest, FirstRectForRangeHorizontal)
{
    ...
    const SelectionInDOMTree selection = SetSelectionTextToBody("<div>^abcdef|</div>");
...
TEST_F(VisibleUnitsTest, FirstRectForRangeHorizontalWrap)
{
    ...
    const SelectionInDOMTree selection = SetSelectionTextToBody("<div>^abc def|</div>");
    const gfx::Rect rect = FirstRectForRange(selection.ComputeRange());
