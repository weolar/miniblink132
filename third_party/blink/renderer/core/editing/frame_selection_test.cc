...
public:
    FrameSelectionTest()
        : root_paint_property_client_(MakeGarbageCollected<FakeDisplayItemClient>("root"))
        , root_paint_chunk_id_(root_paint_property_client_->Id(), DisplayItem::kUninitializedType)
    {
    }
...
protected:
    ...
    Page& GetPage() const
    {
        return GetDummyPageHolder().GetPage();
    }
    ...
    {
        ASSERT_LE(selection_index, static_cast<size_t>(std::numeric_limits<int>::max()));
        Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position(text, static_cast<int>(selection_index))).Build(), SetSelectionOptions());
    }

    // Returns whether a context menu is being displayed.
    bool HasContextMenu()
    {
        return GetDocument().GetPage()->GetContextMenuController().ContextMenuNodeForFrame(GetDocument().GetFrame());
    }

    void MoveRangeSelectionInternal(const Position& base, const Position& extent, TextGranularity granularity)
    {
        Selection().MoveRangeSelectionInternal(SelectionInDOMTree::Builder().SetBaseAndExtent(base, extent).Build(), granularity);
    }
...
{
    Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(position).Build(), SetSelectionOptions());
    return Selection().SelectWordAroundCaret();
}

bool FrameSelectionTest::SelectWordAroundCaret()
{
    return Selection().SelectAroundCaret(TextGranularity::kWord, HandleVisibility::kNotVisible, ContextMenuVisibility::kNotVisible);
}

bool FrameSelectionTest::SelectSentenceAroundCaret()
{
    return Selection().SelectAroundCaret(TextGranularity::kSentence, HandleVisibility::kNotVisible, ContextMenuVisibility::kNotVisible);
}
...
TEST_F(FrameSelectionTest, FirstEphemeralRangeOf)
{
    ...
    Selection().SetSelection(
        SelectionInDOMTree::Builder().SetBaseAndExtent(EphemeralRange(Position(text, 3), Position(text, 6))).Build(), SetSelectionOptions());
    sample->setAttribute(html_names::kStyleAttr, AtomicString("display:none"));
    // Move |VisibleSelection| before "abc".
    UpdateAllLifecyclePhasesForTest();
    const EphemeralRange& range = FirstEphemeralRangeOf(Selection().ComputeVisibleSelectionInDOMTree());
    EXPECT_EQ(Position(sample->nextSibling(), 0), range.StartPosition()) << "firstRange() should return current selection value";
...
TEST_F(FrameSelectionTest, SetValidSelection)
{
    ...
    UpdateAllLifecyclePhasesForTest();
    Selection().SetSelection(SelectionInDOMTree::Builder().SetBaseAndExtent(Position(text, 0), Position(text, 5)).Build(), SetSelectionOptions());
    EXPECT_FALSE(Selection().ComputeVisibleSelectionInDOMTree().IsNone());
}

#define EXPECT_EQ_SELECTED_TEXT(text) EXPECT_EQ(text, Selection().SelectedText().Utf8())
...
{
    SetBodyContent("<p style='width:70px; font-size:14px'>foo bar<em>+</em> baz</p>");
    // "foo bar
    //  b|az"
...
{
    Text* text = AppendTextNode("This is the first sentence. This is the second sentence. This is the "
                                "last sentence.");
    UpdateAllLifecyclePhasesForTest();
    ...
    ResetAndPlaceCaret(text, strlen("This is the first sentence."));
    EXPECT_TRUE(SelectSentenceAroundCaret());
    EXPECT_EQ_SELECTED_TEXT("This is the first sentence. This is the second sentence.");
...
TEST_F(FrameSelectionTest, SelectAroundCaret_ShouldShowHandle)
{
    ...
    EXPECT_TRUE(Selection().SelectAroundCaret(TextGranularity::kSentence, HandleVisibility::kNotVisible, ContextMenuVisibility::kNotVisible));
    EXPECT_FALSE(Selection().IsHandleVisible());

    ResetAndPlaceCaret(text, selection_index);
    EXPECT_TRUE(Selection().SelectAroundCaret(TextGranularity::kWord, HandleVisibility::kNotVisible, ContextMenuVisibility::kNotVisible));
    EXPECT_FALSE(Selection().IsHandleVisible());

    ResetAndPlaceCaret(text, selection_index);
    EXPECT_TRUE(Selection().SelectAroundCaret(TextGranularity::kSentence, HandleVisibility::kNotVisible, ContextMenuVisibility::kVisible));
    EXPECT_FALSE(Selection().IsHandleVisible());

    ResetAndPlaceCaret(text, selection_index);
    EXPECT_TRUE(Selection().SelectAroundCaret(TextGranularity::kWord, HandleVisibility::kNotVisible, ContextMenuVisibility::kVisible));
    EXPECT_FALSE(Selection().IsHandleVisible());

    // Make sure handles are always visible when the handle_visiblity param is
    // set to visible, regardless of the other parameters.
    ResetAndPlaceCaret(text, selection_index);
    EXPECT_TRUE(Selection().SelectAroundCaret(TextGranularity::kSentence, HandleVisibility::kVisible, ContextMenuVisibility::kNotVisible));
    EXPECT_TRUE(Selection().IsHandleVisible());

    ResetAndPlaceCaret(text, selection_index);
    EXPECT_TRUE(Selection().SelectAroundCaret(TextGranularity::kWord, HandleVisibility::kVisible, ContextMenuVisibility::kNotVisible));
    EXPECT_TRUE(Selection().IsHandleVisible());
}
...
TEST_F(FrameSelectionTest, SelectAroundCaret_ShouldShowContextMenu)
{
    ...
    ResetAndPlaceCaret(text, selection_index);
    EXPECT_TRUE(Selection().SelectAroundCaret(TextGranularity::kSentence, HandleVisibility::kVisible, ContextMenuVisibility::kVisible));
    EXPECT_TRUE(HasContextMenu());
    ...
    ResetAndPlaceCaret(text, selection_index);
    EXPECT_TRUE(Selection().SelectAroundCaret(TextGranularity::kWord, HandleVisibility::kVisible, ContextMenuVisibility::kVisible));
    EXPECT_TRUE(HasContextMenu());
}
...
TEST_F(FrameSelectionTest, GetSelectionRangeAroundCaret_Sentence)
{
    ...
    // sentence.
    ResetAndPlaceCaret(text, strlen(""));
    EphemeralRange range = Selection().GetSelectionRangeAroundCaretForTesting(TextGranularity::kSentence);
    EXPECT_EQ("This is the first sentence.", PlainText(range));

    // This is the first sentence|. This is the second sentence. This is the last
    // sentence.
    ResetAndPlaceCaret(text, strlen("This is the first sentence"));
    range = Selection().GetSelectionRangeAroundCaretForTesting(TextGranularity::kSentence);
    EXPECT_EQ("This is the first sentence.", PlainText(range));
    ...
    ResetAndPlaceCaret(text, strlen("This is the first sentence."));
    range = Selection().GetSelectionRangeAroundCaretForTesting(TextGranularity::kSentence);
    EXPECT_EQ("This is the first sentence. This is the second sentence.", PlainText(range));
...
TEST_F(FrameSelectionTest, ModifyExtendWithFlatTree)
{
    ...
    Selection().SetSelection(SelectionInDOMTree::Builder()
                                 ...
                                 .Extend(ToPositionInDOMTree(PositionInFlatTree(GetDocument().body(), 2)))
                                 .Build(),
        SetSelectionOptions());
    Selection().Modify(SelectionModifyAlteration::kExtend, SelectionModifyDirection::kForward, TextGranularity::kWord, SetSelectionBy::kSystem);
...
TEST_F(FrameSelectionTest, ModifyWithUserTriggered)
{
    ...
    Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(end_of_text).Build(), SetSelectionOptions());

    EXPECT_FALSE(Selection().Modify(SelectionModifyAlteration::kMove, SelectionModifyDirection::kForward, TextGranularity::kCharacter, SetSelectionBy::kSystem))
        << "Selection.modify() returns false for non-user-triggered call when "
           "selection isn't modified.";
    EXPECT_EQ(end_of_text, Selection().ComputeVisibleSelectionInDOMTree().Start()) << "Selection isn't modified";

    EXPECT_TRUE(Selection().Modify(SelectionModifyAlteration::kMove, SelectionModifyDirection::kForward, TextGranularity::kCharacter, SetSelectionBy::kUser))
...
TEST_F(FrameSelectionTest, MoveRangeSelectionTest)
{
    ...
    Selection().SetSelection(SelectionInDOMTree::Builder().SetBaseAndExtent(Position(text, 5), Position(text, 6)).Build(), SetSelectionOptions());
    EXPECT_EQ_SELECTED_TEXT("a");

    // "Foo B|ar B>az," with the Character granularity.
    MoveRangeSelectionInternal(Position(text, 5), Position(text, 9), TextGranularity::kCharacter);
    EXPECT_EQ_SELECTED_TEXT("ar B");
    // "Foo B|ar B>az," with the Word granularity.
    MoveRangeSelectionInternal(Position(text, 5), Position(text, 9), TextGranularity::kWord);
    EXPECT_EQ_SELECTED_TEXT("Bar Baz");
    // "Fo<o B|ar Baz," with the Character granularity.
    MoveRangeSelectionInternal(Position(text, 5), Position(text, 2), TextGranularity::kCharacter);
    EXPECT_EQ_SELECTED_TEXT("o B");
    // "Fo<o B|ar Baz," with the Word granularity.
    MoveRangeSelectionInternal(Position(text, 5), Position(text, 2), TextGranularity::kWord);
...
TEST_F(FrameSelectionTest, MoveRangeSelectionNoLiveness)
{
    ...
    MoveRangeSelectionInternal(Position(sample->firstChild(), 1), Position(sample->firstChild(), 1), TextGranularity::kWord);
    EXPECT_EQ("xyz", Selection().SelectedText());
    sample->insertBefore(Text::Create(GetDocument(), "abc"), sample->firstChild());
    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kTest);
    const VisibleSelection& selection = Selection().ComputeVisibleSelectionInDOMTree();
...
TEST_F(FrameSelectionTest, SelectAllWithInputElement)
{
    ...
    Selection().SelectAll();
    const SelectionInDOMTree& result_in_dom_tree = Selection().ComputeVisibleSelectionInDOMTree().AsSelection();
    const SelectionInFlatTree& result_in_flat_tree = Selection().ComputeVisibleSelectionInFlatTree().AsSelection();
    EXPECT_EQ(
        SelectionInDOMTree::Builder(result_in_dom_tree).Collapse(Position::BeforeNode(*input)).Extend(Position(last_child, 3)).Build(), result_in_dom_tree);
    EXPECT_EQ(
        SelectionInFlatTree::Builder(result_in_flat_tree).Collapse(PositionInFlatTree::BeforeNode(*input)).Extend(PositionInFlatTree(last_child, 3)).Build(),
        result_in_flat_tree);
}
...
TEST_F(FrameSelectionTest, SelectAllWithUnselectableRoot)
{
    ...
    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kTest);
    Selection().SelectAll();
    EXPECT_TRUE(Selection().ComputeVisibleSelectionInDOMTree().IsNone()) << "Nothing should be selected if the "
...
TEST_F(FrameSelectionTest, SelectAllPreservesHandle)
{
    ...
    EXPECT_FALSE(Selection().IsHandleVisible());
    Selection().SelectAll();
    EXPECT_FALSE(Selection().IsHandleVisible()) << "If handles weren't present before "
                                                   "selectAll. Then they shouldn't be present "
                                                   "after it.";

    Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(end_of_text).Build(),
        SetSelectionOptions::Builder().SetShouldCloseTyping(true).SetShouldClearTypingStyle(true).SetShouldShowHandle(true).Build());
    EXPECT_TRUE(Selection().IsHandleVisible());
    Selection().SelectAll();
    EXPECT_TRUE(Selection().IsHandleVisible()) << "If handles were present before "
...
TEST_F(FrameSelectionTest, SelectionOnRangeHidesHandles)
{
    ...
    UpdateAllLifecyclePhasesForTest();
    Selection().SetSelection(
        SelectionInDOMTree::Builder().SetBaseAndExtent(EphemeralRange(Position(text, 0), Position(text, 12))).Build(), SetSelectionOptions());

    EXPECT_FALSE(Selection().IsHandleVisible()) << "After SetSelection on Range, handles shouldn't be present.";

    Selection().SetSelection(SelectionInDOMTree::Builder().SetBaseAndExtent(Position(text, 0), Position(text, 5)).Build(),
        SetSelectionOptions::Builder().SetShouldCloseTyping(true).SetShouldClearTypingStyle(true).SetShouldShowHandle(true).Build());
...
TEST_F(FrameSelectionTest, CaretInShadowTree)
{
    ...
    ShadowRoot* shadow_root = SetShadowContent("<div contenteditable id='ce'>foo</div>", "host");
...
TEST_F(FrameSelectionTest, RangeInShadowTree)
{
    ...
    Selection().SetSelection(SelectionInDOMTree::Builder().SetBaseAndExtent(Position(text_node, 0), Position(text_node, 3)).Build(), SetSelectionOptions());
...
{
    SetBodyContent("<textarea readonly>Berlin</textarea>"
                   "<input type='submit' value='Submit'>");
    EXPECT_TRUE(Selection().GetSelectionInDOMTree().IsNone());
    EXPECT_FALSE(Selection().SelectionHasFocus());
    EXPECT_TRUE(Selection().IsHidden());

    Element* const textarea = GetDocument().QuerySelector(AtomicString("textarea"));
    textarea->Focus();
    EXPECT_TRUE(Selection().GetSelectionInDOMTree().IsCaret());
...
{
    SetBodyContent("<textarea disabled>Berlin</textarea>"
                   "<input type='submit' value='Submit'>");
    EXPECT_TRUE(Selection().GetSelectionInDOMTree().IsNone());
    EXPECT_FALSE(Selection().SelectionHasFocus());
    EXPECT_TRUE(Selection().IsHidden());
    ...
    WebMouseEvent double_click(WebMouseEvent::Type::kMouseDown, 0, WebInputEvent::GetStaticTimeStampForTests());
...
{
    SetBodyContent("<div tabindex='-1' id='parent'>"
                   "  <input id='field'>"
                   "</div>");
    EXPECT_TRUE(Selection().GetSelectionInDOMTree().IsNone());
    EXPECT_FALSE(Selection().SelectionHasFocus());
...
{
    SetBodyContent("<p id='host'></p>");
    ShadowRoot* shadow_root = SetShadowContent("<div id='ce' contenteditable>foo</div>", "host");
...
{
    SetBodyContent("<div contenteditable tabindex='-1' id='parent'>"
...
TEST_F(FrameSelectionTest, RangeContainsFocus)
{
    ...
    Selection().SetSelection(SelectionInDOMTree::Builder().SetBaseAndExtent(Position(start, 0), Position(end, 1)).Build(), SetSelectionOptions());
    EXPECT_TRUE(Selection().GetSelectionInDOMTree().IsRange());
    EXPECT_TRUE(Selection().SelectionHasFocus());
    EXPECT_FALSE(Selection().IsHidden());
...
TEST_F(FrameSelectionTest, RangeOutsideFocus)
{
    ...
    SetBodyContent("<a href='www' id='alink'>link</a>"
                   "<div>"
                   "  <div>"
                   "    <span id='start'>start</span>"
                   "  </div>"
...
TEST_F(FrameSelectionTest, InconsistentVisibleSelectionNoCrash)
{
    ...
    Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(start).Extend(end).Build(), SetSelectionOptions());
...
TEST_F(FrameSelectionTest, SelectionBounds)
{
    ...
    EXPECT_EQ(PhysicalRect(0, node_margin_top, node_width, node_height), Selection().AbsoluteUnclippedBounds());
    ...
    frame_view->LayoutViewport()->SetScrollOffset(ScrollOffset(0, scroll_offset), mojom::blink::ScrollType::kProgrammatic);
    EXPECT_EQ(PhysicalRect(0, node_margin_top, node_width, node_height), frame_view->FrameToDocument(Selection().AbsoluteUnclippedBounds()));
...
TEST_F(FrameSelectionTest, AbosluteSelectionBoundsAfterScroll)
{
    ...
    GetDocument().View()->LayoutViewport()->SetScrollOffset(ScrollOffset(0, scroll_offset), mojom::blink::ScrollType::kProgrammatic);
    ...
    EXPECT_EQ(anchor_after_scroll, initial_anchor - gfx::Vector2d(0, scroll_offset));
    EXPECT_EQ(focus_after_scroll, initial_focus - gfx::Vector2d(0, scroll_offset));
}
...
TEST_F(FrameSelectionTest, SelectionContainsBidiBoundary)
{
    ...
    // Rendered as abcFED
    Selection().SetSelection(SetSelectionTextToBody("<div dir=ltr>^abc<bdo dir=trl>DEF|</bdo></div>"), SetSelectionOptions());
...
{
    Selection().SetSelection(SetSelectionTextToBody("^foo<input value=\"bar\">baz|"), SetSelectionOptions());
...
TEST_F(FrameSelectionTest, PositionDisconnectedInFlatTree)
{
    ...
    Position positions[] = { Position::BeforeNode(*host), Position::FirstPositionInNode(*host), Position::LastPositionInNode(*host), Position::AfterNode(*host),
        Position::BeforeNode(*text), Position::FirstPositionInNode(*text), Position::LastPositionInNode(*text), Position::AfterNode(*text) };
    for (const Position& base : positions) {
        ...
        for (const Position& extent : positions) {
            const SelectionInDOMTree& selection = SelectionInDOMTree::Builder().SetBaseAndExtent(base, extent).Build();
            Selection().SetSelection(selection, SetSelectionOptions());
            EXPECT_TRUE(extent.IsConnected());
            bool flat_extent_is_connected = ToPositionInFlatTree(selection.Focus()).IsConnected();
            EXPECT_EQ(flat_base_is_connected || flat_extent_is_connected ? "<div id=\"host\"></div>|y" : "<div id=\"host\"></div>y",
                GetSelectionTextInFlatTreeFromBody(GetVisibleSelectionInFlatTree().AsSelection()));
        }
    }
}
...
TEST_F(FrameSelectionTest, PaintCaretRecordsSelectionWithNoSelectionHandles)
{
    ...
    scoped_feature_list.InitAndEnableFeature(blink::features::kHiddenSelectionBounds);
    ...
    Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position(text, 0)).Build(), SetSelectionOptions());
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(Selection().ComputeVisibleSelectionInDOMTree().IsCaret());
    EXPECT_TRUE(Selection().ShouldPaintCaret(*To<LayoutBlock>(GetDocument().body()->GetLayoutObject())));

    PaintController paint_controller;
    {
        GraphicsContext context(paint_controller);
        paint_controller.UpdateCurrentPaintChunkProperties(root_paint_chunk_id_, *root_paint_property_client_, PropertyTreeState::Root());
