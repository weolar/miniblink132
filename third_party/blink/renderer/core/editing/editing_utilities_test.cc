...
TEST_F(EditingUtilitiesTest, ComputePositionForNodeRemovalAfterChildren)
{
    ...
    EXPECT_EQ(Position(*GetElementById("a"), 0), ComputePositionForNodeRemoval(position, *GetElementById("b")));
}
...
{
    const char* body_content = "<p id='host'><b slot='#one' id='one'></b><b slot='#two' "
                               "id='two'>22</b></p>";
    const char* shadow_content = "<slot name=#two></slot><p dir=rtl><slot name=#one></slot><p>";
    ...
    EXPECT_EQ(TextDirection::kRtl, DirectionOfEnclosingBlockOf(PositionInFlatTree(one, 0)));
}

// TODO(crbug.com/1157146): This test breaks without Shadow DOM v0.
TEST_F(EditingUtilitiesTest, DISABLED_firstEditablePositionAfterPositionInRoot)
{
    const char* body_content = "<p id='host' contenteditable><b slot='#one' id='one'>1</b><b "
                               "slot='#two' id='two'>22</b></p>";
    const char* shadow_content = "<slot name=#two></slot><slot name=#one></slot><b id='three'>333</b>";
    SetBodyContent(body_content);
    ...
    // end of "22".
    EXPECT_EQ(Position(two->firstChild(), 2), CreateVisiblePosition(FirstEditablePositionAfterPositionInRoot(Position(one, 0), *host)).DeepEquivalent());
    ...
    EXPECT_EQ(PositionInFlatTree(two->firstChild(), 2),
        CreateVisiblePosition(FirstEditablePositionAfterPositionInRoot(PositionInFlatTree(one, 0), *host)).DeepEquivalent());

    EXPECT_EQ(Position::FirstPositionInNode(*host), FirstEditablePositionAfterPositionInRoot(Position(three, 0), *host));
    // TODO(crbug.com/1157146): This returns the beginning of "1" instead of the
    // beginning of "22".
    EXPECT_EQ(Position(two->firstChild(), 0), CreateVisiblePosition(FirstEditablePositionAfterPositionInRoot(Position(three, 0), *host)).DeepEquivalent());
    ...
    EXPECT_EQ(PositionInFlatTree::LastPositionInNode(*host),
        CreateVisiblePosition(FirstEditablePositionAfterPositionInRoot(PositionInFlatTree(three, 0), *host)).DeepEquivalent());
}

TEST_F(EditingUtilitiesTest, enclosingBlock)
{
    const char* body_content = "<p id='host'><b slot='#one' id='one'>11</b></p>";
    const char* shadow_content = "<slot name=#two></slot><div id='three'><slot name=#one></slot></div>";
    SetBodyContent(body_content);
    ShadowRoot* shadow_root = SetShadowContent(shadow_content, "host");
    ...
    EXPECT_EQ(three, EnclosingBlock(PositionInFlatTree(one, 0), kCannotCrossEditingBoundary));
}

TEST_F(EditingUtilitiesTest, enclosingNodeOfType)
{
    const char* body_content = "<p id='host'><b slot='#one' id='one'>11</b></p>";
    const char* shadow_content = "<slot name=#two></slot><div id='three'><slot name=#one></div></slot>";
    SetBodyContent(body_content);
    ShadowRoot* shadow_root = SetShadowContent(shadow_content, "host");
    Node* host = GetDocument().getElementById(AtomicString("host"));
    Node* one = GetDocument().getElementById(AtomicString("one"));
...
{
    SetBodyContent(R"(<div contenteditable><span inert id="target">abc</span></div>)");
...
TEST_F(EditingUtilitiesTest, PositionRespectingEditingBoundaryWithInputReadOnly)
{
    LoadAhem();
    InsertStyleElement("body { font: 10px/15px Ahem; margin: 0px; }");
    SetBodyContent("<div id=sample>012"
    ...
    ASSERT_TRUE(GetDocument().View()->GetLayoutView()->HitTest(hit_location, hit_result));
    ASSERT_EQ(PositionWithAffinity(Position(text_012, 0)), hit_result.GetPosition());

    EXPECT_EQ(PositionWithAffinity(Position(text_abc, 0)), PositionRespectingEditingBoundary(Position(text_abc, 1), hit_result))
        << "Adjust to outside of <input> to inside of <input>";
    EXPECT_EQ(PositionWithAffinity(Position(text_012, 0)), PositionRespectingEditingBoundary(Position::BeforeNode(input), hit_result))
        << "No adjustment because both position are in same enclosing element";
}

// http://crbug.com/1185089
TEST_F(EditingUtilitiesTest, PositionRespectingEditingBoundaryWithInputReadOnlyInEditable)
{
    LoadAhem();
    InsertStyleElement("body { font: 10px/15px Ahem; margin: 0px; }");
    SetBodyContent("<div contenteditable id=sample>012"
                   "<input value=abc placeholder=xyz readonly>"
                   "345</div>");
    const auto& sample = *GetElementById("sample");
    const auto& text_012 = *To<Text>(sample.firstChild());
...
TEST_F(EditingUtilitiesTest, PositionRespectingEditingBoundaryWithInputReadWrite)
{
    LoadAhem();
    InsertStyleElement("body { font: 10px/15px Ahem; margin: 0px; }");
    SetBodyContent("<div id=sample>012"
...
TEST_F(EditingUtilitiesTest, PositionRespectingEditingBoundaryWithPointerEventNone)
{
    LoadAhem();
    InsertStyleElement("body { font: 10px/15px Ahem; margin: 0px; }"
    ...
    HitTestResult hit_result(hit_request, hit_location);
    ASSERT_TRUE(GetDocument().View()->GetLayoutView()->HitTest(hit_location, hit_result));
    ASSERT_EQ(PositionWithAffinity(Position(text_abc, 1)), hit_result.GetPosition());
    // Simulates drag from "abc"@2 to "abc@1"
    EXPECT_EQ(PositionWithAffinity(Position(text_abc, 1)), PositionRespectingEditingBoundary(Position(text_abc, 2), hit_result));
}
...
{
    const char* body_content = "<div contenteditable id=host><table slot=#table "
                               "id=table><tr><td>1</td></tr></table><b slot=#two id=two>22</b></div>";
    const char* shadow_content = "<slot name=#two></slot><slot name=#table></slot>";
    SetBodyContent(body_content);
    SetShadowContent(shadow_content, "host");
    Node* host = GetDocument().getElementById(AtomicString("host"));
...
// TODO(crbug.com/1157146): This test breaks without Shadow DOM v0.
TEST_F(EditingUtilitiesTest, DISABLED_lastEditablePositionBeforePositionInRoot)
{
    const char* body_content = "<p id='host' contenteditable><b slot=#one id='one'>1</b><b slot=#two "
                               "id='two'>22</b></p>";
    const char* shadow_content = "<slot name=#two></slot><slot name=#one></slot><b "
                                 "id='three'>333</b>";
    SetBodyContent(body_content);
    ShadowRoot* shadow_root = SetShadowContent(shadow_content, "host");
    Element* host = GetDocument().getElementById(AtomicString("host"));
    ...
    // TODO(crbug.com/1157146): This returns the beginning of "1" instead of the
    // end of "22".
    EXPECT_EQ(Position(two->firstChild(), 2), CreateVisiblePosition(LastEditablePositionBeforePositionInRoot(Position(one, 0), *host)).DeepEquivalent());

    EXPECT_EQ(PositionInFlatTree(one, 0), LastEditablePositionBeforePositionInRoot(PositionInFlatTree(one, 0), *host));
    // TODO(crbug.com/1157146): This returns the beginning of "1" instead of the
    // end of "22".
    EXPECT_EQ(PositionInFlatTree(two->firstChild(), 2),
        CreateVisiblePosition(LastEditablePositionBeforePositionInRoot(PositionInFlatTree(one, 0), *host)).DeepEquivalent());

    EXPECT_EQ(Position::FirstPositionInNode(*host), LastEditablePositionBeforePositionInRoot(Position(three, 0), *host));
    // TODO(crbug.com/1157146): This returns the beginning of "1" instead of the
    // beginning of "22".
    EXPECT_EQ(Position(two->firstChild(), 0), CreateVisiblePosition(LastEditablePositionBeforePositionInRoot(Position(three, 0), *host)).DeepEquivalent());
    ...
    EXPECT_EQ(PositionInFlatTree(two->firstChild(), 0),
        CreateVisiblePosition(LastEditablePositionBeforePositionInRoot(PositionInFlatTree(three, 0), *host)).DeepEquivalent());
}
...
TEST_F(EditingUtilitiesTest, DISABLED_NextNodeIndex)
{
    ...
    EXPECT_EQ(Position(host, 3), NextPositionOf(Position(two, 1), PositionMoveType::kGraphemeCluster));
    // TODO(crbug.com/1157146): This returns the slot instead of the <p>.
    EXPECT_EQ(PositionInFlatTree(host, 1), NextPositionOf(PositionInFlatTree(two, 1), PositionMoveType::kGraphemeCluster));
}
...
TEST_F(EditingUtilitiesTest, NextVisuallyDistinctCandidate)
{
    ...
    const char* shadow_content = "<slot name=#two></slot><slot name=#one></slot><slot name=#three></slot>";
    SetBodyContent(body_content);
    SetShadowContent(shadow_content, "host");
    Node* one = GetDocument().getElementById(AtomicString("one"));
    Node* two = GetDocument().getElementById(AtomicString("two"));
    ...
    EXPECT_EQ(Position(two->firstChild(), 0), NextVisuallyDistinctCandidate(Position(one, 1)));
...
TEST_F(EditingUtilitiesTest, uncheckedPreviousNextOffset_FirstLetter)
{
    ...
    const Node& node = *GetDocument().getElementById(AtomicString("target"))->firstChild();
...
TEST_F(EditingUtilitiesTest, uncheckedPreviousNextOffset)
{
    ...
    Node* node = GetDocument().getElementById(AtomicString("target"))->firstChild();
    EXPECT_EQ(0, PreviousGraphemeBoundaryOf(*node, 1));
...
TEST_F(EditingUtilitiesTest, previousPositionOf_Backspace)
{
    ...
    Node* node = GetDocument().getElementById(AtomicString("target"))->firstChild();
    EXPECT_EQ(Position(node, 2), PreviousPositionOf(Position(node, 3), PositionMoveType::kBackwardDeletion));
    EXPECT_EQ(Position(node, 1), PreviousPositionOf(Position(node, 2), PositionMoveType::kBackwardDeletion));
    EXPECT_EQ(Position(node, 0), PreviousPositionOf(Position(node, 1), PositionMoveType::kBackwardDeletion));
}
...
TEST_F(EditingUtilitiesTest, previousPositionOf_Backspace_FirstLetter)
{
    ...
    node = GetDocument().getElementById(AtomicString("target"))->firstChild();
    EXPECT_EQ(Position(node, 4), PreviousPositionOf(Position(node, 5), PositionMoveType::kBackwardDeletion));
    EXPECT_EQ(Position(node, 3), PreviousPositionOf(Position(node, 4), PositionMoveType::kBackwardDeletion));
    EXPECT_EQ(Position(node, 2), PreviousPositionOf(Position(node, 3), PositionMoveType::kBackwardDeletion));
    EXPECT_EQ(Position(node, 1), PreviousPositionOf(Position(node, 2), PositionMoveType::kBackwardDeletion));
    EXPECT_EQ(Position(node, 0), PreviousPositionOf(Position(node, 1), PositionMoveType::kBackwardDeletion));
}
...
TEST_F(EditingUtilitiesTest, previousPositionOf_Backspace_TextTransform)
{
    ...
    SetBodyContent("<style>p {text-transform:uppercase}</style><p "
                   "id='target'>&#x00DF;abc</p>");
    Node* node = GetDocument().getElementById(AtomicString("target"))->firstChild();
    EXPECT_EQ(Position(node, 3), PreviousPositionOf(Position(node, 4), PositionMoveType::kBackwardDeletion));
    EXPECT_EQ(Position(node, 2), PreviousPositionOf(Position(node, 3), PositionMoveType::kBackwardDeletion));
...
TEST_F(EditingUtilitiesTest, previousPositionOf_Backspace_SurrogatePairs)
{
    ...
    SetBodyContent("<p id='target'>&#x1F441;&#x1F441;&#x1F441;</p>");
    Node* node = GetDocument().getElementById(AtomicString("target"))->firstChild();
    EXPECT_EQ(Position(node, 4), PreviousPositionOf(Position(node, 6), PositionMoveType::kBackwardDeletion));
    EXPECT_EQ(Position(node, 2), PreviousPositionOf(Position(node, 4), PositionMoveType::kBackwardDeletion));
    EXPECT_EQ(Position(node, 0), PreviousPositionOf(Position(node, 2), PositionMoveType::kBackwardDeletion));

    // BMP and Supplementary plane case.
    SetBodyContent("<p id='target'>&#x1F441;a&#x1F441;a</p>");
    node = GetDocument().getElementById(AtomicString("target"))->firstChild();
    EXPECT_EQ(Position(node, 5), PreviousPositionOf(Position(node, 6), PositionMoveType::kBackwardDeletion));
    EXPECT_EQ(Position(node, 3), PreviousPositionOf(Position(node, 5), PositionMoveType::kBackwardDeletion));
    EXPECT_EQ(Position(node, 2), PreviousPositionOf(Position(node, 3), PositionMoveType::kBackwardDeletion));
    EXPECT_EQ(Position(node, 0), PreviousPositionOf(Position(node, 2), PositionMoveType::kBackwardDeletion));
