...
RangeInFlatTree::RangeInFlatTree(const PositionInFlatTree& start, const PositionInFlatTree& end)
    : start_(MakeGarbageCollected<RelocatablePosition>(ToPositionInDOMTree(start)))
    , end_(MakeGarbageCollected<RelocatablePosition>(ToPositionInDOMTree(end)))
{
    DCHECK_LE(start, end);
}
...
{
    return StartPosition().ComputeContainerNode()->isConnected() && EndPosition().ComputeContainerNode()->isConnected();
}
