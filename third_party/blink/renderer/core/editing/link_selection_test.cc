...
protected:
    ...
    void EmulateMouseDrag(const gfx::Point& down_point, const gfx::Point& up_point, int modifiers, DragFlags = kSendDownEvent | kSendUpEvent);

    void EmulateMouseClick(const gfx::Point& click_point, WebMouseEvent::Button, int modifiers, int count = 1);
    void EmulateMouseDown(const gfx::Point& click_point, WebMouseEvent::Button, int modifiers, int count = 1);
...
void LinkSelectionTestBase::EmulateMouseDrag(const gfx::Point& down_point, const gfx::Point& up_point, int modifiers, DragFlags drag_flags)
{
    if (drag_flags & kSendDownEvent) {
        const auto& down_event = frame_test_helpers::CreateMouseEvent(WebMouseEvent::Type::kMouseDown, WebMouseEvent::Button::kLeft, down_point, modifiers);
        web_view_->MainFrameViewWidget()->HandleInputEvent(WebCoalescedInputEvent(down_event, ui::LatencyInfo()));
    }
    ...
    for (int i = 0; i < kMoveEventsNumber; ++i) {
        gfx::Point move_point = down_point + gfx::ToFlooredVector2d(gfx::ScaleVector2d(up_down_vector, i * kMoveIncrementFraction));
        const auto& move_event = frame_test_helpers::CreateMouseEvent(WebMouseEvent::Type::kMouseMove, WebMouseEvent::Button::kLeft, move_point, modifiers);
        web_view_->MainFrameViewWidget()->HandleInputEvent(WebCoalescedInputEvent(move_event, ui::LatencyInfo()));
    }

    if (drag_flags & kSendUpEvent) {
        const auto& up_event = frame_test_helpers::CreateMouseEvent(WebMouseEvent::Type::kMouseUp, WebMouseEvent::Button::kLeft, up_point, modifiers);
        web_view_->MainFrameViewWidget()->HandleInputEvent(WebCoalescedInputEvent(up_event, ui::LatencyInfo()));
    }
}

void LinkSelectionTestBase::EmulateMouseClick(const gfx::Point& click_point, WebMouseEvent::Button button, int modifiers, int count)
{
    auto event = frame_test_helpers::CreateMouseEvent(WebMouseEvent::Type::kMouseDown, button, click_point, modifiers);
    event.click_count = count;
    web_view_->MainFrameViewWidget()->HandleInputEvent(WebCoalescedInputEvent(event, ui::LatencyInfo()));
    event.SetType(WebMouseEvent::Type::kMouseUp);
    web_view_->MainFrameViewWidget()->HandleInputEvent(WebCoalescedInputEvent(event, ui::LatencyInfo()));
}

void LinkSelectionTestBase::EmulateMouseDown(const gfx::Point& click_point, WebMouseEvent::Button button, int modifiers, int count)
{
    auto event = frame_test_helpers::CreateMouseEvent(WebMouseEvent::Type::kMouseDown, button, click_point, modifiers);
    event.click_count = count;
    web_view_->MainFrameViewWidget()->HandleInputEvent(WebCoalescedInputEvent(event, ui::LatencyInfo()));
...
public:
    ...
    WebNavigationPolicy GetLastNavigationPolicy() const
    {
        return last_policy_;
    }
    size_t GetNumNavigations() const
    {
        return num_navigations_;
    }
...
protected:
    void SetUp() override
    {
        constexpr char kHTMLString[] = "<a id='link' href='foo.com' style='font-size:20pt'>Text to select "
                                       "foobar</a>"
        ...
        frame_test_helpers::LoadHTMLString(main_frame_, kHTMLString, url_test_helpers::ToKURL("http://foobar.com"));
        ...
        auto* link_to_select = document->getElementById(AtomicString("link"))->firstChild();
        ...
        auto* const range_to_select = MakeGarbageCollected<Range>(*document, link_to_select, 5, link_to_select, 16);
...
{
    EmulateMouseDrag(left_point_in_link_, right_point_in_link_, WebInputEvent::kAltKey);
...
TEST_F(LinkSelectionTest, HandCursorDuringLinkDrag)
{
    ...
    const auto& cursor = main_frame_->GetFrame()->GetChromeClient().LastSetCursorForTesting();
    EXPECT_EQ(ui::mojom::blink::CursorType::kHand, cursor.type());
}

TEST_F(LinkSelectionTest, DragOnNothingShowsPointer)
{
    EmulateMouseDrag(gfx::Point(100, 500), gfx::Point(300, 500), 0, kSendDownEvent);
    main_frame_->GetFrame()->LocalFrameRoot().GetEventHandler().ScheduleCursorUpdate();
    test::RunDelayedTasks(base::Milliseconds(50));
    const auto& cursor = main_frame_->GetFrame()->GetChromeClient().LastSetCursorForTesting();
...
{
    EmulateMouseDrag(right_point_in_link_, left_point_in_link_, WebInputEvent::kAltKey, kSendDownEvent);
    main_frame_->GetFrame()->LocalFrameRoot().GetEventHandler().ScheduleCursorUpdate();
    test::RunDelayedTasks(base::Milliseconds(50));
    const auto& cursor = main_frame_->GetFrame()->GetChromeClient().LastSetCursorForTesting();
...
{
    EmulateMouseClick(left_point_in_link_, WebMouseEvent::Button::kLeft, WebInputEvent::kAltKey);
    test::RunDelayedTasks(base::Milliseconds(ui::kDoubleClickTimeMs));
    EXPECT_EQ(kWebNavigationPolicyDownload, test_frame_client_.GetLastNavigationPolicy());
}
...
TEST_F(LinkSelectionTest, DoubleAltClickNotDownloadAndSelectWord)
{
    for (int click_count = 1; click_count <= 2; ++click_count) {
        ...
        EmulateMouseClick(left_point_in_link_, WebMouseEvent::Button::kLeft, WebInputEvent::kAltKey, click_count);
    }
    test::RunDelayedTasks(base::Milliseconds(ui::kDoubleClickTimeMs));
    EXPECT_EQ(0u, test_frame_client_.GetNumNavigations());
...
{
    auto* document = main_frame_->GetFrame()->GetDocument();
    auto* text_to_select = document->getElementById(AtomicString("page_text"))->firstChild();
    ASSERT_NE(nullptr, text_to_select);

    // Select some page text outside the link element.
    const auto* range_to_select = MakeGarbageCollected<Range>(*document, text_to_select, 1, text_to_select, 20);
    const auto& selection_rect = range_to_select->BoundingBox();
    main_frame_->MoveRangeSelection(selection_rect.origin(), selection_rect.bottom_right());
...
protected:
    ...
    void CheckMouseClicks(Element& element, bool double_click_event)
    {
        struct ScopedListenersCleaner {
            ...
            {
                element_->RemoveAllEventListeners();
            }
        ...
        element.addEventListener(double_click_event ? event_type_names::kDblclick : event_type_names::kClick, event_handler);
        ...
        EmulateMouseClick(elem_bounds.CenterPoint(), WebMouseEvent::Button::kLeft, 0, click_count);
