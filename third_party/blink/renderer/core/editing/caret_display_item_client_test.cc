...
protected:
    ...
    FrameCaret& GetFrameCaret()
    {
        return Selection().FrameCaretForTesting();
    }

    bool IsVisibleIfActive()
    {
        return GetFrameCaret().IsVisibleIfActive();
    }
    void SetVisibleIfActive(bool v)
    {
        GetFrameCaret().SetVisibleIfActive(v);
    }
    ...
    {
        return CaretDisplayItemClient::ComputeCaretRectAndPainterBlock(position).caret_rect;
    }
    ...
    {
        Vector<cc::Layer*> layers = CcLayersByName(GetDocument().View()->GetPaintArtifactCompositor()->RootLayer(), "Caret");
        if (layers.empty()) {
            return nullptr;
        }
...
TEST_P(CaretDisplayItemClientTest, CaretPaintInvalidation)
{
    ...
    // new carets.
    Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position(text, 5)).Build(), SetSelectionOptions());
...
TEST_P(CaretDisplayItemClientTest, CaretMovesBetweenBlocks)
{
    ...
    Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position(block_element2, 0)).Build(), SetSelectionOptions());
    ...
    Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position(block_element1, 0)).Build(), SetSelectionOptions());
...
TEST_P(CaretDisplayItemClientTest, EnsureInvalidatePreviousLayoutBlock)
{
    ...
    // Set caret into p1.
    GetDocument().body()->Focus();
    Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position(p1, 0)).Build(), SetSelectionOptions());
    ...
    Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position(p3, 0)).Build(), SetSelectionOptions());
    GetDocument().View()->UpdateLifecycleToLayoutClean(DocumentUpdateReason::kTest);
    EXPECT_EQ(p1_block, PreviousCaretLayoutBlock());
...
TEST_P(CaretDisplayItemClientTest, CompositingChange)
{
    ...
    Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(Position(editor, 0)).Build(), SetSelectionOptions());
    ...
    // Composite container.
    container->setAttribute(html_names::kStyleAttr, AtomicString("will-change: transform"));
    UpdateAllLifecyclePhasesExceptPaint();
    EXPECT_FALSE(GetCaretDisplayItemClient().IsValid());
    UpdateAllLifecyclePhasesForCaretTest();
    EXPECT_EQ(PhysicalRect(50, 50, 1, 1), CaretLocalRect());
...
TEST_P(CaretDisplayItemClientTest, PlainTextRTLCaretPosition)
{
    ...
    const Position& regular_position = Position::FirstPositionInNode(*regular_text_node);
    const PhysicalRect regular_caret_rect = ComputeCaretRect(PositionWithAffinity(regular_position));

    auto* plaintext = GetDocument().getElementById(AtomicString("plaintext"));
    auto* plaintext_text_node = plaintext->firstChild();
    const Position& plaintext_position = Position::FirstPositionInNode(*plaintext_text_node);
    const PhysicalRect plaintext_caret_rect = ComputeCaretRect(PositionWithAffinity(plaintext_position));
...
#define MAYBE_InsertSpaceToWhiteSpacePreWrapRTL DISABLED_InsertSpaceToWhiteSpacePreWrapRTL
#else
#define MAYBE_InsertSpaceToWhiteSpacePreWrapRTL InsertSpaceToWhiteSpacePreWrapRTL
#endif
...
TEST_P(CaretDisplayItemClientTest, MAYBE_InsertSpaceToWhiteSpacePreWrapRTL)
{
    ...
    const PhysicalRect& caret_from_position = ComputeCaretRect(PositionWithAffinity(position));
    ...
    int space_width = (text_layout_object->PhysicalLinesBoundingBox().Width() - width).ToInt();
    EXPECT_GT(space_width, 0);

    GetDocument().execCommand("insertText", false, " ", ASSERT_NO_EXCEPTION);
    UpdateAllLifecyclePhasesForCaretTest();
    const PhysicalSize& delta1 = caret_from_position.DistanceAsSize(caret_from_selection.MinXMinYCorner());
    EXPECT_EQ(PhysicalSize(2 * space_width, 0), delta1);

    GetDocument().execCommand("insertText", false, " ", ASSERT_NO_EXCEPTION);
    UpdateAllLifecyclePhasesForCaretTest();
    const PhysicalSize& delta2 = caret_from_position.DistanceAsSize(caret_from_selection.MinXMinYCorner());
...
TEST_P(CaretDisplayItemClientTest, CaretAtEdgeOfInlineBlock)
{
    ...
    SetBodyContent("<div id=editable contenteditable>"
                   "<span contenteditable=false>foo</span>"
                   "</div>");

    const Element& editable = *GetDocument().QuerySelector(AtomicString("#editable"));
    const LayoutBlock* editable_block = To<LayoutBlock>(editable.GetLayoutObject());

    GetDocument().GetPage()->GetFocusController().SetActive(true);
    GetDocument().GetPage()->GetFocusController().SetFocused(true);

    auto test = [this, editable_block](const Position& position, const PhysicalRect& expected_rect) {
        Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(position).Build(), SetSelectionOptions());
...
TEST_P(CaretDisplayItemClientTest, FullDocumentPaintingWithCaret)
{
    ...
    const DisplayItemClient* text_inline_box = cursor.Current().GetDisplayItemClient();
    EXPECT_THAT(ContentDisplayItems(), ElementsAre(VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM, IsSameId(text_inline_box->Id(), kForegroundType)));
    EXPECT_FALSE(CaretLayer());
    ...
    EXPECT_THAT(ContentDisplayItems(),
        ElementsAre(VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM, IsSameId(text_inline_box->Id(), kForegroundType),
            // New!
            IsSameId(GetCaretDisplayItemClient().Id(), DisplayItem::kCaret)));
    ASSERT_CARET_LAYER();
}

TEST_F(ComputeCaretRectTest, CaretRectAfterEllipsisNoCrash)
{
    SetBodyInnerHTML("<style>pre{width:30px; overflow:hidden; text-overflow:ellipsis}</style>"
...
TEST_F(ComputeCaretRectTest, CaretRectAvoidNonEditable)
{
    ...
    const PositionWithAffinity& caret_position1 = HitTestResultAtLocation(20, 5).GetPosition();
    const PhysicalRect& rect1 = ComputeCaretRect(caret_position1);
    EXPECT_EQ(PhysicalRect(10, 0, 1, 10), rect1);

    const PositionWithAffinity& caret_position2 = HitTestResultAtLocation(60, 5).GetPosition();
