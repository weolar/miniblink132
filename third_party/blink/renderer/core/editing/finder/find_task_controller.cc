...
public:
    FindTask(FindTaskController* controller, Document* document, int identifier, const String& search_text, const mojom::blink::FindOptions& options)
        : document_(document)
        ...
        , options_(options.Clone())
    {
        ...
        if (options.run_synchronously_for_testing) {
            ...
            controller_->GetLocalFrame()
                ...
                ->PostTask(FROM_HERE, WTF::BindOnce(&FindTask::Invoke, WrapWeakPersistent(this)));
        }
    }
    ...
    void Invoke()
    {
        ...
        if (!controller_->ShouldFindMatches(identifier_, search_text_, *options_)) {
            controller_->DidFinishTask(identifier_, search_text_, *options_, true /* finished_whole_request */, PositionInFlatTree(), 0 /* match_count */,
        ...
        if (!document || document_ != document)
            return;
        auto forced_activatable_display_locks = document->GetDisplayLockDocumentState().GetScopedForceActivatableLocks();
        PositionInFlatTree search_start = PositionInFlatTree::FirstPositionInNode(*document);
        PositionInFlatTree search_end;
        if (document->documentElement() && document->documentElement()->lastChild()) {
            search_end = PositionInFlatTree::AfterNode(*document->documentElement()->lastChild());
        } else {
        ...
        if (Range* resume_from_range = controller_->ResumeFindingFromRange()) {
            ...
            search_start = FromPositionInDOMTree<EditingInFlatTreeStrategy>(resume_from_range->EndPosition());
        ...
        auto find_options
            = FindOptions().SetBackwards(!options_->forward).SetCaseInsensitive(!options_->match_case).SetStartingInSelection(options_->new_session);
        ...
        while (search_start < search_end) {
            ...
            FindBuffer buffer(EphemeralRangeInFlatTree(search_start, search_end), RubySupport::kEnabledIfNecessary);
            FindResults match_results = buffer.FindMatches(search_text_, find_options);
            bool yielded_while_iterating_results = false;
            for (MatchResultICU match : match_results) {
                const EphemeralRangeInFlatTree ephemeral_match_range = buffer.RangeFromBufferIndex(match.start, match.start + match.length);
                auto* const match_range = MakeGarbageCollected<Range>(ephemeral_match_range.GetDocument(),
                    ToPositionInDOMTree(ephemeral_match_range.StartPosition()), ToPositionInDOMTree(ephemeral_match_range.EndPosition()));
                ...
                // time.
                if ((match_count % match_yield_check_interval) == 0 && time_allotment_expired()) {
        ...
        controller_->DidFinishTask(
            identifier_, search_text_, *options_, full_range_searched, next_task_start_position, match_count, false /* aborted */, task_start_time);
    }
...
FindTaskController::FindTaskController(WebLocalFrameImpl& owner_frame, TextFinder& text_finder)
...
void FindTaskController::StartRequest(int identifier, const String& search_text, const mojom::blink::FindOptions& options)
{
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("blink", "FindInPageRequest", TRACE_ID_WITH_SCOPE("FindInPageRequest", identifier));
...
void FindTaskController::RequestFindTask(int identifier, const String& search_text, const mojom::blink::FindOptions& options)
{
    DCHECK_EQ(find_task_, nullptr);
    DCHECK_EQ(identifier, current_find_identifier_);
    find_task_ = MakeGarbageCollected<FindTask>(this, GetLocalFrame()->GetDocument(), identifier, search_text, options);
}

void FindTaskController::DidFinishTask(int identifier, const String& search_text, const mojom::blink::FindOptions& options, bool finished_whole_request,
    PositionInFlatTree next_starting_position, int match_count, bool aborted, base::TimeTicks task_start_time)
{
    ...
    if (next_starting_position.IsNotNull()) {
        resume_finding_from_range_ = MakeGarbageCollected<Range>(
            *next_starting_position.GetDocument(), ToPositionInDOMTree(next_starting_position), ToPositionInDOMTree(next_starting_position));
    }

    if (match_count > 0) {
        text_finder_->UpdateMatches(identifier, match_count, finished_whole_request);
    }

    if (!finished_whole_request) {
        match_yield_check_interval_ = std::min(kMatchYieldCheckIntervalLimit, 2 * match_yield_check_interval_);
    ...
    last_find_request_completed_with_no_matches_ = !aborted && !current_match_count_;
    finding_in_progress_ = false;
    current_find_identifier_ = kInvalidFindIdentifier;
}
...
bool FindTaskController::ShouldFindMatches(int identifier, const String& search_text, const mojom::blink::FindOptions& options)
{
    ...
    if (last_find_request_completed_with_no_matches_ && !last_search_string_.empty()) {
        // Check to see if the search string prefixes match.
        String previous_search_prefix = search_text.Substring(0, last_search_string_.length());
