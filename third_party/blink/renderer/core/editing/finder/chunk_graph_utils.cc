...
const Node* FindOuterMostRubyContainerInBlockContainer(const Node& node, const Node& block)
{
    ...
    for (const auto& ancestor : FlatTreeTraversal::AncestorsOf(node)) {
        ...
        if (const ComputedStyle* style = element->GetComputedStyle()) {
            if (style->Display() == EDisplay::kRuby || style->Display() == EDisplay::kBlockRuby) {
...
String CreateLevel(const Vector<std::pair<wtf_size_t, wtf_size_t>>& depth_context)
{
...
public:
    const Node* Build(const Node& first_visible_text_node, const Node* end_node, const Node& block_ancestor, const Node* just_after_block)
    {
        bool did_see_range_start_node = false;
        bool did_see_range_end_node = false;
        const Node* node = &first_visible_text_node;
        if (const Node* ruby_container = FindOuterMostRubyContainerInBlockContainer(first_visible_text_node, block_ancestor)) {
        ...
        while (node && node != just_after_block) {
            ...
            if (style->Visibility() == EVisibility::kVisible && node->GetLayoutObject()) {
                ...
                if (last_added_text_node && !FindBuffer::IsInSameUninterruptedBlock(*last_added_text_node, *node)) {
                    did_see_range_end_node = true;
            ...
            while (!FlatTreeTraversal::NextSibling(*node) && node != &block_ancestor) {
    ...
    Vector<String> TakeLevelList()
    {
        return std::move(level_list_);
    }

private:
    CorpusChunk* PushChunk(const String& level)
    {
        auto* new_chunk = MakeGarbageCollected<CorpusChunk>(chunk_text_list_, level);
    ...
    bool HandleAnnotationEnd(const Node& node, bool did_see_range_end_node)
    {
        ...
        return !IsParentRubyContainer(node) && CloseRubyContainer(did_see_range_end_node);
    }
...
CorpusChunk::CorpusChunk()
    : level_(String(kAnyLevel))
{
}

CorpusChunk::CorpusChunk(const HeapVector<TextOrChar>& text_list, const String& level)
    : level_(level)
{
...
std::tuple<HeapVector<Member<CorpusChunk>>, Vector<String>, const Node*> BuildChunkGraph(
    const Node& first_visible_text_node, const Node* end_node, const Node& block_ancestor, const Node* just_after_block)
{
    ChunkGraphBuilder builder;
    const Node* next_node = builder.Build(first_visible_text_node, end_node, block_ancestor, just_after_block);
