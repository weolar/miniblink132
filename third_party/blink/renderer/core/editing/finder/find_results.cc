...
FindResults::FindResults(const FindBuffer* find_buffer, TextSearcherICU* text_searcher, const Vector<UChar>& buffer, const Vector<Vector<UChar>>* extra_buffers,
    const String& search_text, const FindOptions options)
{
    ...
    text_searcher_->SetText(base::span<const UChar>(buffer.data(), buffer.size()));
    ...
    if (!RuntimeEnabledFeatures::FindRubyInPageEnabled()) {
        ...
        for (const Vector<UChar>& text : *extra_buffers) {
            extra_searchers_.push_back(std::make_unique<TextSearcherICU>(TextSearcherICU::kConstructLocal));
            std::unique_ptr<TextSearcherICU>& searcher = extra_searchers_.back();
            searcher->SetPattern(search_text_, options);
            searcher->SetText(base::span<const UChar>(text.data(), text.size()));
        }
    }
}
...
FindResults::Iterator::Iterator(const FindBuffer* find_buffer, TextSearcherICU* text_searcher, const Vector<std::unique_ptr<TextSearcherICU>>& extra_searchers)
    : find_buffer_(find_buffer)
{
...
{
    auto min_iter = std::min_element(match_list_.begin(), match_list_.end(), [](const auto& a, const auto& b) {
...
void FindResults::Iterator::operator++()
{
    ...
    for (size_t i = 0; i < text_searcher_list_.size(); ++i) {
        ...
        if (optional_match.has_value() && optional_match->start == last_result.start) {
