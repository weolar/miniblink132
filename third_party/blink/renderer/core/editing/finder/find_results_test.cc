...
protected:
    ...
    template <size_t N> static Vector<UChar> MakeBuffer(const UChar (&literal)[N])
    {
        Vector<UChar> buffer;
        buffer.reserve(N);
...
TEST_F(FindResultsTest, MultipleIdenticalCorpora)
{
    ...
    FindResults results(find_buffer, &main_searcher_, base_buffer, &extra_buffers, query, FindOptions());
...
TEST_F(FindResultsTest, MultipleCorpora)
{
    ...
    String query(u"foo");
    FindBuffer* find_buffer = nullptr;
    FindResults results(find_buffer, &main_searcher_, buffer0, &extra_buffers, query, FindOptions());
