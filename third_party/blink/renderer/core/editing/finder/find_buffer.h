...
public:
    explicit FindBuffer(const EphemeralRangeInFlatTree& range, RubySupport ruby_support = RubySupport::kDisabled);

    static EphemeralRangeInFlatTree FindMatchInRange(
        const EphemeralRangeInFlatTree& range, String search_text, const FindOptions, std::optional<base::TimeDelta> timeout_ms = std::nullopt);

    // Returns the closest ancestor of |start_node| (including the node itself)
    // that is block level.
    static const Node& GetFirstBlockLevelAncestorInclusive(const Node& start_node);
    ...
    // returns false.
    static bool IsInSameUninterruptedBlock(const Node& start_node, const Node& end_node);
    ...
    FindResults FindMatches(const String& search_text, const blink::FindOptions options);

    // Gets a flat tree range corresponding to text in the [start_index,
    // end_index) of |buffer|.
    EphemeralRangeInFlatTree RangeFromBufferIndex(unsigned start_index, unsigned end_index) const;
...
private:
    ...
    void CollectTextUntilBlockBoundary(const EphemeralRangeInFlatTree& range, RubySupport ruby_support);

    // Replaces nodes that should be ignored with appropriate char constants.
    static void ReplaceNodeWithCharConstants(const Node& node, Vector<UChar>& buffer);
    ...
    Vector<UChar> SerializeLevelInGraph(const HeapVector<Member<CorpusChunk>>& chunk_list, const String& level, const EphemeralRangeInFlatTree& range);

    // Adds text in |text_node| that are located within |range| to |buffer|.
    void AddTextToBuffer(const Text& text_node, const EphemeralRangeInFlatTree& range, Vector<UChar>& buffer, Vector<BufferNodeMapping>* mappings);
