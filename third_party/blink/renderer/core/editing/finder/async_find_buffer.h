...
public:
    static std::unique_ptr<base::AutoReset<base::TimeDelta>> OverrideTimeoutForTesting(base::TimeDelta timeout_override);

    explicit AsyncFindBuffer() = default;
    ~AsyncFindBuffer() = default;

    void FindMatchInRange(RangeInFlatTree* search_range, String search_text, FindOptions options, Callback completeCallback) override;
    void Cancel() override;
    bool IsActive() override
    {
        return pending_find_match_task_.IsActive();
    }

private:
    void Run(RangeInFlatTree* search_range, String search_text, FindOptions options, Callback completeCallback);

    void NextIteration(RangeInFlatTree* search_range, String search_text, FindOptions options, Callback completeCallback);
