...
bool FindBuffer::ShouldIgnoreContents(const Node& node)
{
    ...
    if (!element)
        return false;
    return (RuntimeEnabledFeatures::InertElementNonSearchableEnabled() && element->IsInertRoot())
        || (!element->ShouldSerializeEndTag() && !IsA<HTMLInputElement>(*element))
        || (IsA<TextControlElement>(*element) && !To<TextControlElement>(*element).SuggestedValue().empty()) || IsA<HTMLIFrameElement>(*element)
        || IsA<HTMLImageElement>(*element) || IsA<HTMLMeterElement>(*element) || IsA<HTMLObjectElement>(*element) || IsA<HTMLProgressElement>(*element)
        || (IsA<HTMLSelectElement>(*element) && To<HTMLSelectElement>(*element).UsesMenuList()) || IsA<HTMLStyleElement>(*element)
        || IsA<HTMLScriptElement>(*element) || IsA<HTMLVideoElement>(*element) || IsA<HTMLAudioElement>(*element)
        || (element->GetDisplayLockContext() && element->GetDisplayLockContext()->IsLocked()
            && !element->GetDisplayLockContext()->IsActivatable(DisplayLockActivationReason::kFindInPage));
}
...
template <class Direction> Node* GetVisibleTextNode(Node& start_node)
{
    ...
    while (node) {
        ...
        if (FindBuffer::ShouldIgnoreContents(*node) || (style && style->Display() == EDisplay::kNone)) {
            // This element and its descendants are not visible, skip it.
            node = Direction::NextSkippingSubtree(*node);
            continue;
        }
        if (style && style->Visibility() == EVisibility::kVisible && node->IsTextNode()
            && (!RuntimeEnabledFeatures::FindTextSkipCollapsedTextEnabled() || node->GetLayoutObject())) {
            return node;
        }
...
{
    if (const auto* block_flow = DynamicTo<LayoutBlockFlow>(block_ancestor.GetLayoutObject())) {
...
FindBuffer::FindBuffer(const EphemeralRangeInFlatTree& range, RubySupport ruby_support)
{
...
bool FindBuffer::IsInvalidMatch(MatchResultICU match) const
{
    ...
    PositionInFlatTree start_position = PositionAtStartOfCharacterAtIndex(start_index);
    if (start_position.IsNull() || !start_position.IsOffsetInAnchor())
        return true;

    const unsigned end_index = match.start + match.length;
    DCHECK_LE(start_index, end_index);
    PositionInFlatTree end_position = PositionAtEndOfCharacterAtIndex(end_index - 1);
...
EphemeralRangeInFlatTree FindBuffer::FindMatchInRange(
    const EphemeralRangeInFlatTree& range, String search_text, FindOptions options, std::optional<base::TimeDelta> timeout_ms)
{
    ...
    while (node && node != past_last_node) {
        if (start_time.is_null()) {
            ...
            if (timeout_ms.has_value() && time_elapsed > timeout_ms.value()) {
                return EphemeralRangeInFlatTree(PositionInFlatTree::FirstPositionInNode(*node), PositionInFlatTree::FirstPositionInNode(*node));
            }
        }
        ...
        PositionInFlatTree start_position = node == first_node ? range.StartPosition() : PositionInFlatTree::FirstPositionInNode(*node);
        if (start_position >= range.EndPosition())
            break;

        FindBuffer buffer(EphemeralRangeInFlatTree(start_position, range.EndPosition()),
            options.IsRubySupported() ? RubySupport::kEnabledIfNecessary : RubySupport::kDisabled);
        ...
        if (!match_results.IsEmpty()) {
            ...
            last_match_range = buffer.RangeFromBufferIndex(match.start, match.start + match.length);
        }
...
bool FindBuffer::IsInSameUninterruptedBlock(const Node& start_node, const Node& end_node)
{
    ...
    LayoutBlockFlow& start_block_flow = *OffsetMapping::GetInlineFormattingContextOf(*start_node.GetLayoutObject());
    LayoutBlockFlow& end_block_flow = *OffsetMapping::GetInlineFormattingContextOf(*end_node.GetLayoutObject());
    ...
    for (const Node* node = &start_node; !node->isSameNode(&end_node); node = FlatTreeTraversal::Next(*node)) {
        const ComputedStyle* style = node->GetComputedStyleForElementOrLayoutObject();
        if (ShouldIgnoreContents(*node) || !style || style->Display() == EDisplay::kNone || style->Visibility() != EVisibility::kVisible) {
            continue;
        }

        if (node->GetLayoutObject() && *OffsetMapping::GetInlineFormattingContextOf(*node->GetLayoutObject()) != start_block_flow) {
            return false;
        }
    }
...
FindResults FindBuffer::FindMatches(const String& search_text, const blink::FindOptions options)
{
    ...
    if (!RuntimeEnabledFeatures::FindDecomposedInShortTextEnabled() && search_text.length() > buffer_.size()) {
        return FindResults();
    }
    String search_text_16_bit = search_text;
    search_text_16_bit.Ensure16Bit();
    FoldQuoteMarksAndSoftHyphens(search_text_16_bit);
    return FindResults(this, &text_searcher_, buffer_, &buffer_list_, search_text_16_bit, options);
}

void FindBuffer::CollectTextUntilBlockBoundary(const EphemeralRangeInFlatTree& range, RubySupport ruby_support)
{
    ...
    Node* node = ForwardVisibleTextNode(*range.StartPosition().NodeAsRangeFirstNode());
    if (!node || !node->isConnected())
        return;

    const Node& block_ancestor = GetFirstBlockLevelAncestorInclusive(*node);
    const Node* just_after_block = FlatTreeTraversal::Next(FlatTreeTraversal::LastWithinOrSelf(block_ancestor));
    ...
    if (RuntimeEnabledFeatures::FindRubyInPageEnabled()) {
        use_chunk_graph
            = ruby_support == RubySupport::kEnabledForcefully || (ruby_support == RubySupport::kEnabledIfNecessary && IsIfcWithRuby(block_ancestor));
    }
    if (use_chunk_graph) {
        auto [corpus_chunk_list, level_list, next_node] = BuildChunkGraph(*node, end_node, block_ancestor, just_after_block);
        ...
        for (const auto& level : level_list) {
            buffer_list_.push_back(SerializeLevelInGraph(corpus_chunk_list, level, range));
    ...
    while (node && node != just_after_block) {
        if (ShouldIgnoreContents(*node)) {
            if (end_node && (end_node == node || FlatTreeTraversal::IsDescendantOf(*end_node, *node))) {
        ...
        if (style->Visibility() == EVisibility::kVisible && node->GetLayoutObject()) {
            // This node is in its own sub-block separate from our starting position.
            if (last_added_text_node && last_added_text_node->GetLayoutObject() && !IsInSameUninterruptedBlock(*last_added_text_node, *node))
                break;
...
void FindBuffer::ReplaceNodeWithCharConstants(const Node& node, Vector<UChar>& buffer)
{
...
EphemeralRangeInFlatTree FindBuffer::RangeFromBufferIndex(unsigned start_index, unsigned end_index) const
{
    DCHECK_LE(start_index, end_index);
    PositionInFlatTree start_position = PositionAtStartOfCharacterAtIndex(start_index);
    PositionInFlatTree end_position = PositionAtEndOfCharacterAtIndex(end_index - 1);
...
const FindBuffer::BufferNodeMapping* FindBuffer::MappingForIndex(unsigned index) const
{
    // Get the first entry that starts at a position higher than offset, and
    // move back one entry.
    auto it = std::upper_bound(buffer_node_mappings_.begin(), buffer_node_mappings_.end(), index,
        [](const unsigned offset, const BufferNodeMapping& entry) { return offset < entry.offset_in_buffer; });
...
PositionInFlatTree FindBuffer::PositionAtStartOfCharacterAtIndex(unsigned index) const
{
    ...
    return ToPositionInFlatTree(offset_mapping_->GetLastPosition(index - entry->offset_in_buffer + entry->offset_in_mapping));
}

PositionInFlatTree FindBuffer::PositionAtEndOfCharacterAtIndex(unsigned index) const
{
    DCHECK_LT(index, buffer_.size());
    DCHECK(offset_mapping_);
    const BufferNodeMapping* entry = MappingForIndex(index);
    if (!entry)
        return PositionInFlatTree();
    return ToPositionInFlatTree(offset_mapping_->GetFirstPosition(index - entry->offset_in_buffer + entry->offset_in_mapping + 1));
}

Vector<UChar> FindBuffer::SerializeLevelInGraph(const HeapVector<Member<CorpusChunk>>& chunk_list, const String& level, const EphemeralRangeInFlatTree& range)
{
    Vector<BufferNodeMapping>* mappings = level.empty() ? &buffer_node_mappings_ : nullptr;
    Vector<UChar> buffer;
...
void FindBuffer::AddTextToBuffer(const Text& text_node, const EphemeralRangeInFlatTree& range, Vector<UChar>& buffer, Vector<BufferNodeMapping>* mappings)
{
    LayoutBlockFlow& block_flow = *OffsetMapping::GetInlineFormattingContextOf(*text_node.GetLayoutObject());
    if (!offset_mapping_) {
    ...
    Position node_start = (&text_node == range.StartPosition().ComputeContainerNode()) ? ToPositionInDOMTree(range.StartPosition().ToOffsetInAnchor())
                                                                                       : Position::FirstPositionInNode(text_node);
    Position node_end = (&text_node == range.EndPosition().ComputeContainerNode()) ? ToPositionInDOMTree(range.EndPosition().ToOffsetInAnchor())
                                                                                   : Position::LastPositionInNode(text_node);
    unsigned last_unit_end = 0;
    bool first_unit = true;
    const String mapped_text = offset_mapping_->GetText();
    for (const OffsetMappingUnit& unit : offset_mapping_->GetMappingUnitsForDOMRange(EphemeralRange(node_start, node_end))) {
        if (first_unit || last_unit_end != unit.TextContentStart()) {
            if (mappings) {
                ...
                mappings->push_back(BufferNodeMapping({ buffer.size(), unit.TextContentStart() }));
            }
            first_unit = false;
        }
        String text_for_unit = mapped_text.Substring(unit.TextContentStart(), unit.TextContentEnd() - unit.TextContentStart());
