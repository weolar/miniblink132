...
protected:
    TextFinderTest()
    {
        ...
        frame_impl.ViewImpl()->MainFrameWidget()->UpdateAllLifecyclePhases(DocumentUpdateReason::kTest);
    ...
    static gfx::RectF FindInPageRect(Node* start_container, int start_offset, Node* end_container, int end_offset);
...
protected:
    ...
    {
        return WebLocalFrameImpl::FromFrame(GetDocument().GetFrame())->EnsureTextFinder();
    }
};

v8::Local<v8::Value> TextFinderTest::EvalJs(const std::string& script)
{
    return ClassicScript::CreateUnspecifiedScript(script.c_str())->RunScriptAndReturnValue(GetDocument().domWindow()).GetSuccessValueOrEmpty();
}
...
gfx::RectF TextFinderTest::FindInPageRect(Node* start_container, int start_offset, Node* end_container, int end_offset)
{
...
TEST_F(TextFinderTest, FindTextSimple)
{
    ...
    auto find_options = mojom::blink::FindOptions::New(); // Default + add testing flag.
    bool wrap_within_frame = true;

    ASSERT_TRUE(GetTextFinder().Find(identifier, search_text, *find_options, wrap_within_frame));
...
TEST_F(TextFinderTest, FindTextAutosizing)
{
    ...
    GetDocument().GetSettings()->SetTextAutosizingWindowSizeOverride(gfx::Size(20, 20));
...
TEST_F(TextFinderTest, FindTextNotFound)
{
    ...
    EXPECT_FALSE(GetTextFinder().Find(identifier, search_text, *find_options, wrap_within_frame));
...
TEST_F(TextFinderTest, ScopeTextMatchesSimple)
{
    ...
    GetTextFinder().StartScopingStringMatches(identifier, search_text, *find_options);
    ...
    GetDocument().body()->setAttribute(html_names::kStyleAttr, AtomicString("margin: 2000px"));
    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kTest);
...
TEST_F(TextFinderTest, ScopeTextMatchesRepeated)
{
    ...
    GetTextFinder().ResetMatchCount();
    GetTextFinder().StartScopingStringMatches(identifier, search_text1, *find_options);
    GetTextFinder().StartScopingStringMatches(identifier, search_text2, *find_options);
...
TEST_F(TextFinderTest, ScopeTextMatchesWithShadowDOM)
{
    ...
    EXPECT_EQ(FindInPageRect(text_in_i_element, 0, text_in_i_element, 3), match_rects[0]);
    EXPECT_EQ(FindInPageRect(text_in_u_element, 0, text_in_u_element, 3), match_rects[1]);
    EXPECT_EQ(FindInPageRect(text_in_b_element, 0, text_in_b_element, 3), match_rects[2]);
}
...
TEST_F(TextFinderTest, FindTextJavaScriptUpdatesDOM)
{
    ...
    find_options->new_session = false;
    ASSERT_TRUE(GetTextFinder().Find(identifier, search_text, *find_options, wrap_within_frame, &active_now));
    ...
    Node* text_in_b_element = GetDocument().body()->firstChild()->firstChild();
    Node* text_in_i_element = GetDocument().body()->lastChild()->firstChild();
    EXPECT_EQ(FindInPageRect(text_in_b_element, 4, text_in_b_element, 10), match_rects[0]);
    EXPECT_EQ(FindInPageRect(text_in_i_element, 2, text_in_i_element, 8), match_rects[1]);
}
...
TEST_F(TextFinderTest, FindTextJavaScriptUpdatesDOMAfterNoMatches)
{
    ...
    ASSERT_EQ(1u, match_rects.size());
    Node* text_in_i_element = GetDocument().body()->lastChild()->firstChild();
    EXPECT_EQ(FindInPageRect(text_in_i_element, 2, text_in_i_element, 8), match_rects[0]);
}
...
TEST_F(TextFinderTest, ScopeWithTimeouts)
{
    ...
    GetTextFinder().StartScopingStringMatches(identifier, search_pattern, *find_options);
...
TEST_F(TextFinderTest, BeforeMatchEvent)
{
    ...
    v8::Local<v8::Value> beforematch_fired_on_foo = EvalJs("window.beforematchFiredOnFoo");
    ASSERT_TRUE(beforematch_fired_on_foo->IsBoolean());
    EXPECT_FALSE(beforematch_fired_on_foo->ToBoolean(v8_testing_scope.GetIsolate())->Value());

    v8::Local<v8::Value> beforematch_fired_on_bar = EvalJs("window.beforematchFiredOnBar");
    ASSERT_TRUE(beforematch_fired_on_bar->IsBoolean());
    EXPECT_TRUE(beforematch_fired_on_bar->ToBoolean(v8_testing_scope.GetIsolate())->Value());

    // Scrolling should occur after the beforematch event.
    v8::Local<v8::Context> context = v8_testing_scope.GetScriptState()->GetContext();
    v8::Local<v8::Value> beforematch_y_offset = EvalJs("window.YOffsetOnBeforematch");
    ASSERT_TRUE(beforematch_y_offset->IsNumber());
    EXPECT_TRUE(beforematch_y_offset->ToNumber(context).ToLocalChecked()->Value() == 0);
}
...
TEST_F(TextFinderTest, BeforeMatchEventRemoveElement)
{
    ...
    v8::Local<v8::Value> beforematch_fired_on_foo = EvalJs("window.beforematchFiredOnFoo");
    ASSERT_TRUE(beforematch_fired_on_foo->IsBoolean());
    EXPECT_TRUE(beforematch_fired_on_foo->ToBoolean(v8_testing_scope.GetIsolate())->Value());
...
TEST_F(TextFinderSimTest, BeforeMatchEventAsyncExpandHighlight)
{
    ...
    GetTextFinder().Find(/*identifier=*/0, "hidden", *mojom::blink::FindOptions::New(),
        /*wrap_within_frame=*/false);

    Compositor().BeginFrame();

    HeapVector<Member<DocumentMarker>> markers = GetDocument().Markers().Markers();
...
TEST_F(TextFinderSimTest, BeforeMatchExpandedHiddenMatchableUkm)
{
    ...
    EXPECT_FALSE(ukm::TestUkmRecorder::EntryHasMetric(entries[0], "BeforematchExpandedHiddenMatchable"));
    EXPECT_TRUE(ukm::TestUkmRecorder::EntryHasMetric(entries[1], "BeforematchExpandedHiddenMatchable"));
}
...
TEST_F(TextFinderSimTest, BeforeMatchExpandedHiddenMatchableUseCounter)
{
    ...
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kBeforematchRevealedHiddenMatchable));
}

TEST_F(TextFinderSimTest, BeforeMatchExpandedHiddenMatchableUseCounterNoHandler)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
    ...
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kBeforematchRevealedHiddenMatchable));
}

TEST_F(TextFinderTest, FindTextAcrossCommentNode)
{
    GetDocument().body()->setInnerHTML("<span>abc</span><!--comment--><span>def</span>");
    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kTest);
    ...
    EXPECT_TRUE(GetTextFinder().Find(identifier, search_text, *find_options, wrap_within_frame));
