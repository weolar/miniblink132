...
static VisiblePositionInFlatTree EndOfLine(const VisiblePositionInFlatTree& position)
{
    return CreateVisiblePosition(EndOfLine(position.ToPositionWithAffinity()));
}
...
protected:
    static PositionWithAffinity PositionWithAffinityInDOMTree(Node& anchor, int offset, TextAffinity affinity = TextAffinity::kDownstream)
    {
        return PositionWithAffinity(CanonicalPositionOf(Position(&anchor, offset)), affinity);
    }

    static VisiblePosition CreateVisiblePositionInDOMTree(Node& anchor, int offset, TextAffinity affinity = TextAffinity::kDownstream)
    {
        return CreateVisiblePosition(Position(&anchor, offset), affinity);
    }

    static PositionInFlatTreeWithAffinity PositionWithAffinityInFlatTree(Node& anchor, int offset, TextAffinity affinity = TextAffinity::kDownstream)
    {
        return PositionInFlatTreeWithAffinity(CanonicalPositionOf(PositionInFlatTree(&anchor, offset)), affinity);
    }

    static VisiblePositionInFlatTree CreateVisiblePositionInFlatTree(Node& anchor, int offset, TextAffinity affinity = TextAffinity::kDownstream)
    {
    ...
    std::string TestEndOfLine(const std::string& input)
    {
        ...
        const Position& result = EndOfLine(CreateVisiblePosition(caret)).DeepEquivalent();
    ...
    {
        const Position& caret = SetCaretTextToBody(input);
        const Position& result = LogicalEndOfLine(CreateVisiblePosition(caret)).DeepEquivalent();
        return GetCaretTextFromBody(result);
    }
    ...
    {
        const Position& caret = SetCaretTextToBody(input);
        const Position& result = StartOfLine(CreateVisiblePosition(caret)).DeepEquivalent();
        return GetCaretTextFromBody(result);
    }
};
...
TEST_F(VisibleUnitsLineTest, endOfLine)
{
    ...
    // 117777777
    // 3334444
    const char* body_content = "<span id=host><b slot='#one' id=one>11</b><b slot='#two' "
                               "id=two>22</b></span><i id=three>333</i><i "
                               "id=four>4444</i><br>";
    const char* shadow_content = "<div><u id=five>55555</u><slot name='#two'></slot><br><u "
    ...
    Node* seven = shadow_root->getElementById(AtomicString("seven"))->firstChild();
    ...
    EXPECT_EQ(Position(two, 2), EndOfLine(CreateVisiblePositionInDOMTree(*two, 0, TextAffinity::kUpstream)).DeepEquivalent());
    EXPECT_EQ(Position(two, 2), EndOfLine(CreateVisiblePositionInDOMTree(*two, 0)).DeepEquivalent());
    EXPECT_EQ(PositionInFlatTree(two, 2), EndOfLine(CreateVisiblePositionInFlatTree(*two, 0)).DeepEquivalent());

    EXPECT_EQ(Position(two, 2), EndOfLine(CreateVisiblePositionInDOMTree(*two, 1)).DeepEquivalent());
    EXPECT_EQ(PositionInFlatTree(two, 2), EndOfLine(CreateVisiblePositionInFlatTree(*two, 1)).DeepEquivalent());

    EXPECT_EQ(Position(four, 4), EndOfLine(CreateVisiblePositionInDOMTree(*three, 0, TextAffinity::kUpstream)).DeepEquivalent());
    EXPECT_EQ(Position(four, 4), EndOfLine(CreateVisiblePositionInDOMTree(*three, 0)).DeepEquivalent());
    ...
    EXPECT_EQ(Position(two, 2), EndOfLine(CreateVisiblePositionInDOMTree(*five, 1)).DeepEquivalent());
...
TEST_F(VisibleUnitsLineTest, isEndOfLine)
{
    ...
    EXPECT_TRUE(IsEndOfLine(CreateVisiblePositionInFlatTree(*two, 2, TextAffinity::kUpstream)));
...
TEST_F(VisibleUnitsLineTest, isLogicalEndOfLine)
{
    ...
    EXPECT_TRUE(IsLogicalEndOfLine(CreateVisiblePositionInDOMTree(*two, 2, TextAffinity::kUpstream)));
...
{
    const char* body_content = "<p id='host'>00<b slot='#one' id='one'>11</b><b slot='#two' "
                               "id='two'>22</b>33</p>";
    const char* shadow_content = "<div><span id='s4'>44</span><slot name='#two'></slot><br><span "
    ...
    EXPECT_FALSE(InSameLine(PositionWithAffinityInDOMTree(*one, 0), PositionWithAffinityInDOMTree(*two, 0)));
    EXPECT_FALSE(InSameLine(PositionWithAffinityInDOMTree(*one->firstChild(), 0), PositionWithAffinityInDOMTree(*two->firstChild(), 0)));
    EXPECT_FALSE(InSameLine(PositionWithAffinityInDOMTree(*one->firstChild(), 0), PositionWithAffinityInDOMTree(*five->firstChild(), 0)));
    EXPECT_TRUE(InSameLine(PositionWithAffinityInDOMTree(*two->firstChild(), 0), PositionWithAffinityInDOMTree(*four->firstChild(), 0)));

    EXPECT_FALSE(InSameLine(CreateVisiblePositionInDOMTree(*one, 0), CreateVisiblePositionInDOMTree(*two, 0, TextAffinity::kUpstream)));
    EXPECT_FALSE(InSameLine(CreateVisiblePositionInDOMTree(*one, 0), CreateVisiblePositionInDOMTree(*two, 0)));
    EXPECT_FALSE(
        InSameLine(CreateVisiblePositionInDOMTree(*one->firstChild(), 0), CreateVisiblePositionInDOMTree(*two->firstChild(), 0, TextAffinity::kUpstream)));
    EXPECT_FALSE(InSameLine(CreateVisiblePositionInDOMTree(*one->firstChild(), 0), CreateVisiblePositionInDOMTree(*two->firstChild(), 0)));
    EXPECT_FALSE(InSameLine(CreateVisiblePositionInDOMTree(*one->firstChild(), 0), CreateVisiblePositionInDOMTree(*five->firstChild(), 0)));
    EXPECT_TRUE(
        InSameLine(CreateVisiblePositionInDOMTree(*two->firstChild(), 0, TextAffinity::kUpstream), CreateVisiblePositionInDOMTree(*four->firstChild(), 0)));
    EXPECT_TRUE(InSameLine(CreateVisiblePositionInDOMTree(*two->firstChild(), 0), CreateVisiblePositionInDOMTree(*four->firstChild(), 0)));
...
TEST_F(VisibleUnitsLineTest, isStartOfLine)
{
    ...
    EXPECT_TRUE(IsStartOfLine(CreateVisiblePositionInDOMTree(*three, 0, TextAffinity::kUpstream)));
...
TEST_F(VisibleUnitsLineTest, logicalEndOfLine)
{
    ...
    EXPECT_EQ(Position(two, 2), LogicalEndOfLine(CreateVisiblePositionInDOMTree(*two, 0, TextAffinity::kUpstream)).DeepEquivalent());
    EXPECT_EQ(Position(two, 2), LogicalEndOfLine(CreateVisiblePositionInDOMTree(*two, 0)).DeepEquivalent());
    EXPECT_EQ(PositionInFlatTree(two, 2), LogicalEndOfLine(CreateVisiblePositionInFlatTree(*two, 0)).DeepEquivalent());

    EXPECT_EQ(Position(two, 2), LogicalEndOfLine(CreateVisiblePositionInDOMTree(*two, 1)).DeepEquivalent());
    EXPECT_EQ(PositionInFlatTree(two, 2), LogicalEndOfLine(CreateVisiblePositionInFlatTree(*two, 1)).DeepEquivalent());

    EXPECT_EQ(Position(four, 4), LogicalEndOfLine(CreateVisiblePositionInDOMTree(*three, 0, TextAffinity::kUpstream)).DeepEquivalent());
    EXPECT_EQ(Position(four, 4), LogicalEndOfLine(CreateVisiblePositionInDOMTree(*three, 0)).DeepEquivalent());
    ...
    EXPECT_EQ(Position(two, 2), LogicalEndOfLine(CreateVisiblePositionInDOMTree(*five, 1)).DeepEquivalent());
...
TEST_F(VisibleUnitsLineTest, logicalStartOfLine)
{
    ...
    EXPECT_EQ(Position(five, 0), LogicalStartOfLine(CreateVisiblePositionInDOMTree(*two, 0, TextAffinity::kUpstream)).DeepEquivalent());
    EXPECT_EQ(Position(five, 0), LogicalStartOfLine(CreateVisiblePositionInDOMTree(*two, 0)).DeepEquivalent());
    ...
    EXPECT_EQ(Position(three, 0), LogicalStartOfLine(CreateVisiblePositionInDOMTree(*three, 0, TextAffinity::kUpstream)).DeepEquivalent());
    EXPECT_EQ(Position(three, 0), LogicalStartOfLine(CreateVisiblePositionInDOMTree(*three, 0)).DeepEquivalent());
    EXPECT_EQ(PositionInFlatTree(three, 0), LogicalStartOfLine(CreateVisiblePositionInFlatTree(*three, 1)).DeepEquivalent());

    EXPECT_EQ(Position(three, 0), LogicalStartOfLine(CreateVisiblePositionInDOMTree(*four, 1, TextAffinity::kUpstream)).DeepEquivalent());
    EXPECT_EQ(Position(three, 0), LogicalStartOfLine(CreateVisiblePositionInDOMTree(*four, 1)).DeepEquivalent());
...
TEST_F(VisibleUnitsLineTest, startOfLine)
{
    ...
    EXPECT_EQ(Position(five, 0), StartOfLine(CreateVisiblePositionInDOMTree(*two, 0, TextAffinity::kUpstream)).DeepEquivalent());
    EXPECT_EQ(Position(five, 0), StartOfLine(CreateVisiblePositionInDOMTree(*two, 0)).DeepEquivalent());
    ...
    EXPECT_EQ(Position(three, 0), StartOfLine(CreateVisiblePositionInDOMTree(*three, 0, TextAffinity::kUpstream)).DeepEquivalent());
    EXPECT_EQ(Position(three, 0), StartOfLine(CreateVisiblePositionInDOMTree(*three, 0)).DeepEquivalent());
    EXPECT_EQ(PositionInFlatTree(three, 0), StartOfLine(CreateVisiblePositionInFlatTree(*three, 1)).DeepEquivalent());

    EXPECT_EQ(Position(three, 0), StartOfLine(CreateVisiblePositionInDOMTree(*four, 1, TextAffinity::kUpstream)).DeepEquivalent());
    EXPECT_EQ(Position(three, 0), StartOfLine(CreateVisiblePositionInDOMTree(*four, 1)).DeepEquivalent());
...
TEST_F(VisibleUnitsLineTest, EndOfLineWithBidi)
{
    ...
    EXPECT_EQ("<p dir=\"ltr\"><bdo dir=\"ltr\">ab cd ef|</bdo></p>", TestEndOfLine("<p dir=\"ltr\"><bdo dir=\"ltr\">a|b cd ef</bdo></p>")) << "LTR LTR";
    EXPECT_EQ("<p dir=\"ltr\"><bdo dir=\"rtl\">ab cd ef|</bdo></p>", TestEndOfLine("<p dir=\"ltr\"><bdo dir=\"rtl\">a|b cd ef</bdo></p>")) << "LTR RTL";
    EXPECT_EQ("<p dir=\"rtl\"><bdo dir=\"ltr\">ab cd ef|</bdo></p>", TestEndOfLine("<p dir=\"rtl\"><bdo dir=\"ltr\">a|b cd ef</bdo></p>")) << "RTL LTR";
    EXPECT_EQ("<p dir=\"rtl\"><bdo dir=\"rtl\">ab cd ef|</bdo></p>", TestEndOfLine("<p dir=\"rtl\"><bdo dir=\"rtl\">a|b cd ef</bdo></p>")) << "RTL RTL";
}
...
{
    LoadAhem();
    InsertStyleElement("b { position:relative; left: 30px; }"
                       "p { font: 30px/3 Ahem; }");

    EXPECT_EQ("<p>ab <b>cd</b> <b>ef|</b></p>", TestEndOfLine("<p>a|b <b>cd</b> <b>ef</b></p>"));
    EXPECT_EQ("<p><bdo dir=\"rtl\">ab <b>cd</b> <b>ef|</b></bdo></p>", TestEndOfLine("<p><bdo dir=\"rtl\">a|b <b>cd</b> <b>ef</b></bdo></p>"));
    EXPECT_EQ("<p dir=\"rtl\">ab <b>cd</b> <b>ef|</b></p>", TestEndOfLine("<p dir=\"rtl\">a|b <b>cd</b> <b>ef</b></p>"));
    EXPECT_EQ("<p dir=\"rtl\"><bdo dir=\"rtl\">ab <b>cd</b> <b>ef|</b></bdo></p>",
        TestEndOfLine("<p dir=\"rtl\"><bdo dir=\"rtl\">a|b <b>cd</b> <b>ef</b></bdo></p>"));
}
...
TEST_F(VisibleUnitsLineTest, EndOfLineWithSoftLineWrap3)
{
    ...
    EXPECT_EQ("<div dir=\"rtl\"><bdo dir=\"rtl\">abc|def</bdo></div>", TestEndOfLine("<div dir=\"rtl\"><bdo dir=\"rtl\">|abcdef</bdo></div>"));

    // Note: Both legacy and NG layout don't have text boxes for spaces cause
    // soft line wrap.
    EXPECT_EQ("<div>abc| def ghi</div>", TestEndOfLine("<div>|abc def ghi</div>"));
    EXPECT_EQ("<div>abc| def ghi</div>", TestEndOfLine("<div>ab|c def ghi</div>"));
    EXPECT_EQ("<div>abc| def ghi</div>", TestEndOfLine("<div>abc| def ghi</div>"));
    EXPECT_EQ("<div>abc def| ghi</div>", TestEndOfLine("<div>abc |def ghi</div>"));

    EXPECT_EQ("<div dir=\"rtl\"><bdo dir=\"rtl\">abc| def ghi</bdo></div>", TestEndOfLine("<div dir=\"rtl\"><bdo dir=\"rtl\">|abc def ghi</bdo></div>"));
    EXPECT_EQ("<div dir=\"rtl\"><bdo dir=\"rtl\">abc| def ghi</bdo></div>", TestEndOfLine("<div dir=\"rtl\"><bdo dir=\"rtl\">ab|c def ghi</bdo></div>"));
    EXPECT_EQ("<div dir=\"rtl\"><bdo dir=\"rtl\">abc| def ghi</bdo></div>", TestEndOfLine("<div dir=\"rtl\"><bdo dir=\"rtl\">abc| def ghi</bdo></div>"));
    EXPECT_EQ("<div dir=\"rtl\"><bdo dir=\"rtl\">abc def| ghi</bdo></div>", TestEndOfLine("<div dir=\"rtl\"><bdo dir=\"rtl\">abc |def ghi</bdo></div>"));
    ...
    EXPECT_EQ("<div contenteditable>abc |def ghi</div>", TestEndOfLine("<div contenteditable>|abc def ghi</div>"));
    EXPECT_EQ("<div contenteditable>abc |def ghi</div>", TestEndOfLine("<div contenteditable>ab|c def ghi</div>"));
    EXPECT_EQ("<div contenteditable>abc |def ghi</div>", TestEndOfLine("<div contenteditable>abc| def ghi</div>"));
    EXPECT_EQ("<div contenteditable>abc def |ghi</div>", TestEndOfLine("<div contenteditable>abc |def ghi</div>"));
}
...
TEST_F(VisibleUnitsLineTest, EndOfLineWithWhiteSpacePre)
{
    ...
    EXPECT_EQ("<p dir=\"ltr\"><bdo dir=\"ltr\">ABC DEF|\nGHI JKL</bdo></p>", TestEndOfLine("<p dir=\"ltr\"><bdo dir=\"ltr\">ABC| DEF\nGHI JKL</bdo></p>"))
        << "LTR LTR";
    EXPECT_EQ("<p dir=\"ltr\"><bdo dir=\"rtl\">ABC DEF|\nGHI JKL</bdo></p>", TestEndOfLine("<p dir=\"ltr\"><bdo dir=\"rtl\">ABC| DEF\nGHI JKL</bdo></p>"))
        << "LTR RTL";
    EXPECT_EQ("<p dir=\"rtl\"><bdo dir=\"ltr\">ABC DEF|\nGHI JKL</bdo></p>", TestEndOfLine("<p dir=\"rtl\"><bdo dir=\"ltr\">ABC| DEF\nGHI JKL</bdo></p>"))
        << "RTL LTR";
    EXPECT_EQ("<p dir=\"rtl\"><bdo dir=\"rtl\">ABC DEF|\nGHI JKL</bdo></p>", TestEndOfLine("<p dir=\"rtl\"><bdo dir=\"rtl\">ABC| DEF\nGHI JKL</bdo></p>"))
        << "RTL RTL";
}
...
TEST_F(VisibleUnitsLineTest, LogicalEndOfLineWithSoftLineWrap3)
{
    ...
    EXPECT_EQ("<div dir=\"rtl\"><bdo dir=\"rtl\">abc|def</bdo></div>", TestLogicalEndOfLine("<div dir=\"rtl\"><bdo dir=\"rtl\">|abcdef</bdo></div>"));

    EXPECT_EQ("<div>abc| def ghi</div>", TestLogicalEndOfLine("<div>|abc def ghi</div>"));
    EXPECT_EQ("<div>abc| def ghi</div>", TestLogicalEndOfLine("<div>ab|c def ghi</div>"));
    EXPECT_EQ("<div>abc| def ghi</div>", TestLogicalEndOfLine("<div>abc| def ghi</div>"));
    EXPECT_EQ("<div>abc def| ghi</div>", TestLogicalEndOfLine("<div>abc |def ghi</div>"));

    // On content editable, caret is after a space.
    EXPECT_EQ("<div contenteditable>abc |def ghi</div>", TestLogicalEndOfLine("<div contenteditable>|abc def ghi</div>"));
    EXPECT_EQ("<div contenteditable>abc |def ghi</div>", TestLogicalEndOfLine("<div contenteditable>ab|c def ghi</div>"));
    EXPECT_EQ("<div contenteditable>abc |def ghi</div>", TestLogicalEndOfLine("<div contenteditable>abc| def ghi</div>"));
    EXPECT_EQ("<div contenteditable>abc def |ghi</div>", TestLogicalEndOfLine("<div contenteditable>abc |def ghi</div>"));
}
...
TEST_F(VisibleUnitsLineTest, InSameLineSkippingEmptyEditableDiv)
{
    ...
    SetBodyContent("<p id=foo>foo</p>"
                   "<div contenteditable></div>"
                   "<p id=bar>bar</p>");
    const Node* const foo = GetElementById("foo")->firstChild();
    const Node* const bar = GetElementById("bar")->firstChild();

    EXPECT_TRUE(InSameLine(PositionWithAffinity(Position(foo, 3), TextAffinity::kDownstream), PositionWithAffinity(Position(foo, 3), TextAffinity::kUpstream)));
    EXPECT_FALSE(
        InSameLine(PositionWithAffinity(Position(bar, 0), TextAffinity::kDownstream), PositionWithAffinity(Position(foo, 3), TextAffinity::kDownstream)));
    EXPECT_TRUE(InSameLine(PositionWithAffinity(Position(bar, 3), TextAffinity::kDownstream), PositionWithAffinity(Position(bar, 3), TextAffinity::kUpstream)));
    EXPECT_FALSE(
        InSameLine(PositionWithAffinity(Position(foo, 0), TextAffinity::kDownstream), PositionWithAffinity(Position(bar, 0), TextAffinity::kDownstream)));
}

TEST_F(VisibleUnitsLineTest, InSameLineWithMixedEditability)
{
    SelectionInDOMTree selection = SetSelectionTextToBody("<span contenteditable>f^oo</span>b|ar");
...
{
    LoadAhem();
    InsertStyleElement("p { font: 10px/1 Ahem; }"
    ...
    const PositionWithAffinity after_zws_down = PositionWithAffinity(after_zws, TextAffinity::kDownstream);
    const PositionWithAffinity after_zws_up = PositionWithAffinity(after_zws, TextAffinity::kUpstream);

    EXPECT_EQ(PositionWithAffinity(Position(*GetElementById("t")->firstChild(), 8), TextAffinity::kUpstream), EndOfLine(after_zws_down));
...
TEST_F(VisibleUnitsLineTest, InSameLineWithSoftLineWrap)
{
    ...
    //    overflow-wrap: break-word;
    const SelectionInDOMTree& selection = SetSelectionTextToBody("<p contenteditable id=t>abc |xyz</p>");
    EXPECT_FALSE(
        InSameLine(PositionWithAffinity(selection.Anchor(), TextAffinity::kUpstream), PositionWithAffinity(selection.Anchor(), TextAffinity::kDownstream)));
}
...
TEST_F(VisibleUnitsLineTest, InSameLineWithZeroWidthSpace)
{
    ...
    InsertStyleElement("p { font: 10px/1 Ahem; }"
                       "p { width: 4ch; }");
    const SelectionInDOMTree& selection = SetSelectionTextToBody("<p id=t>abcd^\u200B|wxyz</p>");
    ...
    const PositionWithAffinity before_zws_down = PositionWithAffinity(before_zws, TextAffinity::kDownstream);
    const PositionWithAffinity before_zws_up = PositionWithAffinity(before_zws, TextAffinity::kUpstream);

    EXPECT_EQ(PositionWithAffinity(Position(*GetElementById("t")->firstChild(), 9), TextAffinity::kUpstream), EndOfLine(after_zws_down));
    EXPECT_EQ(after_zws_up, EndOfLine(after_zws_up));
    EXPECT_FALSE(InSameLine(after_zws_up, after_zws_down));
...
{
    SetBodyContent("<span id=one>start</span>"
                   "<span id=two style='display: inline-block;'>test</span>"
                   "<span id=three>end</span>");

    const PositionWithAffinity position = PositionWithAffinity(Position(*GetElementById("two")->firstChild(), 0), TextAffinity::kUpstream);
    EXPECT_TRUE(InSameLine(position, PositionWithAffinity(Position(*GetElementById("one")->firstChild(), 0), TextAffinity::kUpstream)));
    EXPECT_TRUE(InSameLine(position, PositionWithAffinity(Position(*GetElementById("three")->firstChild(), 0), TextAffinity::kUpstream)));
}
...
TEST_F(VisibleUnitsLineTest, StartOfLineBeforeEmptyLine)
{
    ...
    EXPECT_EQ("<p dir=\"ltr\">abc<br>|<br>xyz<br></p>", TestStartOfLine("<p dir=\"ltr\">abc<br>|<br>xyz<br></p>"));
    EXPECT_EQ("<p dir=\"ltr\">abc<br><br>|<br>xyz<br></p>", TestStartOfLine("<p dir=\"ltr\">abc<br><br>|<br>xyz<br></p>"));
    EXPECT_EQ("<p dir=\"ltr\">abc<br>|<br><br>xyz<br></p>", TestStartOfLine("<p dir=\"ltr\">abc<br>|<br><br>xyz<br></p>"));

    EXPECT_EQ("<p dir=\"rtl\">abc<br>|<br>xyz<br></p>", TestStartOfLine("<p dir=\"rtl\">abc<br>|<br>xyz<br></p>"));
    EXPECT_EQ("<p dir=\"rtl\">abc<br>|<br><br>xyz<br></p>", TestStartOfLine("<p dir=\"rtl\">abc<br>|<br><br>xyz<br></p>"));
    EXPECT_EQ("<p dir=\"rtl\">abc<br><br>|<br>xyz<br></p>", TestStartOfLine("<p dir=\"rtl\">abc<br><br>|<br>xyz<br></p>"));
}
...
TEST_F(VisibleUnitsLineTest, StartOfLineWithBidi)
{
    ...
    EXPECT_EQ("<p dir=\"ltr\"><bdo dir=\"ltr\">|abc xyz</bdo></p>", TestStartOfLine("<p dir=\"ltr\"><bdo dir=\"ltr\">abc |xyz</bdo></p>")) << "LTR LTR";
    EXPECT_EQ("<p dir=\"ltr\"><bdo dir=\"rtl\">|abc xyz</bdo></p>", TestStartOfLine("<p dir=\"ltr\"><bdo dir=\"rtl\">abc |xyz</bdo></p>")) << "LTR RTL";
    EXPECT_EQ("<p dir=\"rtl\"><bdo dir=\"ltr\">|abc xyz</bdo></p>", TestStartOfLine("<p dir=\"rtl\"><bdo dir=\"ltr\">abc |xyz</bdo></p>")) << "RTL LTR";
    EXPECT_EQ("<p dir=\"rtl\"><bdo dir=\"rtl\">|abc xyz</bdo></p>", TestStartOfLine("<p dir=\"rtl\"><bdo dir=\"rtl\">abc |xyz</bdo></p>")) << "RTL RTL";
}

TEST_F(VisibleUnitsLineTest, StartOfLineWithPositionRelative)
{
    LoadAhem();
    InsertStyleElement("b { position:relative; left: -100px; }"
                       "p { font: 30px/3 Ahem; }");

    EXPECT_EQ("<p><b>|abc</b> xyz</p>", TestStartOfLine("<p><b>abc</b> |xyz</p>")) << "LTR-LTR";
    EXPECT_EQ("<p dir=\"rtl\"><b>|abc</b> xyz</p>", TestStartOfLine("<p dir=\"rtl\"><b>abc</b> |xyz</p>")) << "RTL-LTR";
    EXPECT_EQ("<p><bdo dir=\"rtl\"><b>|abc</b> xyz</bdo></p>", TestStartOfLine("<p><bdo dir=\"rtl\"><b>abc</b> |xyz</bdo></p>")) << "LTR-RTL";
    EXPECT_EQ("<p dir=\"rtl\"><bdo dir=\"rtl\"><b>|abc</b> xyz</bdo></p>", TestStartOfLine("<p dir=\"rtl\"><bdo  dir=\"rtl\"><b>abc</b> |xyz</bdo></p>"))
        << "RTL-RTL";
}
...
TEST_F(VisibleUnitsLineTest, TextOverflowEllipsis1)
{
    ...
    EXPECT_EQ(Position(text, 0), StartOfLine(CreateVisiblePositionInDOMTree(*text, 6)).DeepEquivalent());
    EXPECT_EQ(Position(text, 7), EndOfLine(CreateVisiblePositionInDOMTree(*text, 6)).DeepEquivalent());
}
...
TEST_F(VisibleUnitsLineTest, TextOverflowEllipsis2)
{
    ...
    // Should not crash
    const PositionWithAffinity& start_of_line = StartOfLine(PositionWithAffinity(Position(span, 1)));
