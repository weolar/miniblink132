...
public:
    ...
    public:
        ...
        InlineContents(const LayoutBlockFlow& block_flow, const LayoutObject* block_in_inline_before, const LayoutObject& first, const LayoutObject& last,
    ...
    public:
        ...
        public:
            ...
            explicit Iterator(const InlineContents& current)
                : current_(current)
            {
            }
            Iterator() = default;
        ...
        explicit BackwardRange(const InlineContents& start)
            : start_(start)
        {
        }
    ...
    public:
        ...
        explicit ForwardRange(const InlineContents& start)
            : start_(start)
        {
        }
    ...
    const EphemeralRangeInFlatTree GetRange() const
    {
        return range_;
    }
    ...
    static InlineContents FindBackwardInlineContents(const PositionInFlatTree& position);
...
private:
    ...
    template <typename Traverser> static InlineContents FindInlineContentsInternal(const Node*, Traverser);
...
CORE_EXPORT std::ostream& operator<<(std::ostream&, const TextOffsetMapping::InlineContents&);
