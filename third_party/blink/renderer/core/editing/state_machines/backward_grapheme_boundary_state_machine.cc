...
std::ostream& operator<<(std::ostream& os, BackwardGraphemeBoundaryStateMachine::InternalState state)
{
...
TextSegmentationMachineState BackwardGraphemeBoundaryStateMachine::FeedPrecedingCodeUnit(UChar code_unit)
{
    ...
    case InternalState::kSearchWaitLeadSurrogate:
        ...
        {
            const UChar32 code_point = U16_GET_SUPPLEMENTARY(code_unit, trail_surrogate_);
            trail_surrogate_ = 0;
            if (Character::IsRegionalIndicator(next_code_point_) && Character::IsRegionalIndicator(code_point)) {
...
TextSegmentationMachineState BackwardGraphemeBoundaryStateMachine::TellEndOfPrecedingText()
{
    switch (internal_state_) {
    case InternalState::kStart:
        // Did nothing.
...
TextSegmentationMachineState BackwardGraphemeBoundaryStateMachine::FeedFollowingCodeUnit(UChar code_unit)
{
    NOTREACHED();
}
...
TextSegmentationMachineState BackwardGraphemeBoundaryStateMachine::MoveToNextState(InternalState next_state)
{
...
TextSegmentationMachineState BackwardGraphemeBoundaryStateMachine::StaySameState()
{
