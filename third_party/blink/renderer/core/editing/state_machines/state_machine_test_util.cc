...
char MachineStateToChar(TextSegmentationMachineState state)
{
    ...
    DCHECK_LT(static_cast<size_t>(state), kIndicators.size()) << "Unknown backspace value";
...
template <typename StateMachine> String ProcessSequence(StateMachine* machine, const Vector<UChar32>& preceding, const Vector<UChar32>& following)
{
    machine->Reset();
    StringBuilder out;
    ...
    if (preceding.empty() || state == TextSegmentationMachineState::kNeedMoreCodeUnit) {
...
String GraphemeStateMachineTestBase::ProcessSequenceBackward(BackwardGraphemeBoundaryStateMachine* machine, const Vector<UChar32>& preceding)
{
...
String GraphemeStateMachineTestBase::ProcessSequenceForward(
    ForwardGraphemeBoundaryStateMachine* machine, const Vector<UChar32>& preceding, const Vector<UChar32>& following)
{
