...
public:
    ...
    GraphemeStateMachineTestBase& operator=(const GraphemeStateMachineTestBase&) = delete;

protected:
    GraphemeStateMachineTestBase() = default;
    ~GraphemeStateMachineTestBase() override = default;

    Vector<UChar32> AsCodePoints()
    {
        return Vector<UChar32>();
    }

    template <typename... Args> Vector<UChar32> AsCodePoints(Args... args)
    {
    ...
    String ProcessSequenceBackward(BackwardGraphemeBoundaryStateMachine*, const Vector<UChar32>& preceding);

    String ProcessSequenceForward(ForwardGraphemeBoundaryStateMachine*, const Vector<UChar32>& preceding, const Vector<UChar32>& following);
};
