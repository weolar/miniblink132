...
std::ostream& operator<<(std::ostream& os, ForwardGraphemeBoundaryStateMachine::InternalState state)
{
...
TextSegmentationMachineState ForwardGraphemeBoundaryStateMachine::FeedPrecedingCodeUnit(UChar code_unit)
{
    ...
    case InternalState::kCountRISWaitLeadSurrogate:
        ...
        if (U16_IS_LEAD(code_unit)) {
            const UChar32 code_point = U16_GET_SUPPLEMENTARY(code_unit, pending_code_unit_);
    ...
    case InternalState::kSearchWaitTrailSurrogate: // Fallthrough
        NOTREACHED() << "Do not call feedPrecedingCodeUnit() once " << TextSegmentationMachineState::kNeedFollowingCodeUnit
...
TextSegmentationMachineState ForwardGraphemeBoundaryStateMachine::FeedFollowingCodeUnit(UChar code_unit)
{
    switch (internal_state_) {
    case InternalState::kCountRIS: // Fallthrough
    case InternalState::kCountRISWaitLeadSurrogate:
        NOTREACHED() << "Do not call feedFollowingCodeUnit() until " << TextSegmentationMachineState::kNeedFollowingCodeUnit
                     << " is returned. InternalState: " << internal_state_;
    ...
    case InternalState::kSearchWaitTrailSurrogate:
        ...
        {
            const UChar32 code_point = U16_GET_SUPPLEMENTARY(pending_code_unit_, code_unit);
            pending_code_unit_ = 0;
            if (Character::IsRegionalIndicator(prev_code_point_) && Character::IsRegionalIndicator(code_point)) {
...
TextSegmentationMachineState ForwardGraphemeBoundaryStateMachine::TellEndOfPrecedingText()
{
    DCHECK(internal_state_ == InternalState::kCountRIS || internal_state_ == InternalState::kCountRISWaitLeadSurrogate)
        << "Do not call tellEndOfPrecedingText() once " << TextSegmentationMachineState::kNeedFollowingCodeUnit
        << " is returned. InternalState: " << internal_state_;
...
TextSegmentationMachineState ForwardGraphemeBoundaryStateMachine::MoveToNextState(InternalState next_state)
{
...
TextSegmentationMachineState ForwardGraphemeBoundaryStateMachine::StaySameState()
{
