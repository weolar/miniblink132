...
{
    return std::ranges::binary_search(kIndicSyllabicCategoryViramaList, code_point);
}
...
bool IsGraphemeBreak(UChar32 prev_code_point, UChar32 next_code_point)
{
    ...
    int prev_prop = u_getIntPropertyValue(prev_code_point, UCHAR_GRAPHEME_CLUSTER_BREAK);
    int next_prop = u_getIntPropertyValue(next_code_point, UCHAR_GRAPHEME_CLUSTER_BREAK);
    ...
    if (prev_prop == U_GCB_CONTROL || prev_prop == U_GCB_CR || prev_prop == U_GCB_LF)
        return true;

    // Rule GB5, ÷ (Control | CR | LF)
    if (next_prop == U_GCB_CONTROL || next_prop == U_GCB_CR || next_prop == U_GCB_LF)
        return true;

    // Rule GB6, L x (L | V | LV | LVT)
    if (prev_prop == U_GCB_L && (next_prop == U_GCB_L || next_prop == U_GCB_V || next_prop == U_GCB_LV || next_prop == U_GCB_LVT))
        return false;

    // Rule GB7, (LV | V) x (V | T)
    if ((prev_prop == U_GCB_LV || prev_prop == U_GCB_V) && (next_prop == U_GCB_V || next_prop == U_GCB_T))
        return false;
    ...
    // [^RI] (RI RI)* RI x RI
    //                RI ÷ RI
    if (Character::IsRegionalIndicator(prev_code_point) && Character::IsRegionalIndicator(next_code_point)) {
        NOTREACHED() << "Do not use this function for regional indicators.";
    }

    // Rule GB9, x (Extend | ZWJ)
    // Rule GB9a, x SpacingMark
    if (next_prop == U_GCB_EXTEND || next_code_point == kZeroWidthJoinerCharacter || next_prop == U_GCB_SPACING_MARK)
        return false;
    ...
    if (IsIndicSyllabicCategoryVirama(prev_code_point) && u_getIntPropertyValue(next_code_point, UCHAR_GENERAL_CATEGORY) == U_OTHER_LETTER)
        return false;

    // GB11, ZWJ x Emoji
    if (prev_code_point == kZeroWidthJoinerCharacter && (Character::IsEmoji(next_code_point)))
        return false;
