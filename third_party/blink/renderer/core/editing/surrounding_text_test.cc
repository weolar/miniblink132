...
protected:
    ...
    EphemeralRange Select(int offset)
    {
...
EphemeralRange SurroundingTextTest::Select(int start, int end)
{
    ...
    return EphemeralRange(Position(element->firstChild(), start), Position(element->firstChild(), end));
}
...
TEST_F(SurroundingTextTest, BasicCaretSelection)
{
...

    {
        ...
        EXPECT_EQ("foo", String(surrounding_text.TextContent()).SimplifyWhiteSpace());
...

    {
        ...
        EXPECT_EQ("foo bar", String(surrounding_text.TextContent()).SimplifyWhiteSpace());
        EXPECT_EQ(1u, surrounding_text.StartOffsetInTextContent());
        EXPECT_EQ(1u, surrounding_text.EndOffsetInTextContent());
    }
...
TEST_F(SurroundingTextTest, BasicRangeSelection)
{
...

    {
        ...
        EXPECT_EQ("Lorem ip", String(surrounding_text.TextContent()).SimplifyWhiteSpace());
        EXPECT_EQ(1u, surrounding_text.StartOffsetInTextContent());
...

    {
        ...
        EXPECT_EQ("Lorem ipsum dolor sit amet", String(surrounding_text.TextContent()).SimplifyWhiteSpace());
        EXPECT_EQ(1u, surrounding_text.StartOffsetInTextContent());
        EXPECT_EQ(6u, surrounding_text.EndOffsetInTextContent());
    }
...
{
    SetHTML(String("<div>This is outside of <p id='selection'>foo bar</p> the "
                   "selected node</div>"));
...

    {
        ...
        EXPECT_EQ("This is outside of foo bar the selected node", String(surrounding_text.TextContent()).SimplifyWhiteSpace());
...
TEST_F(SurroundingTextTest, TreeRangeSelection)
{
...

    {
        ...
        EXPECT_EQ("fo", String(surrounding_text.TextContent()).SimplifyWhiteSpace());
        EXPECT_EQ(0u, surrounding_text.StartOffsetInTextContent());
        EXPECT_EQ(1u, surrounding_text.EndOffsetInTextContent());
    }

    {
        ...
        EXPECT_EQ("e of foo bar", String(surrounding_text.TextContent()).SimplifyWhiteSpace());
...

    {
        ...
        EXPECT_EQ("foo bar the se", String(surrounding_text.TextContent()).SimplifyWhiteSpace());
        EXPECT_EQ(5u, surrounding_text.StartOffsetInTextContent());
        EXPECT_EQ(8u, surrounding_text.EndOffsetInTextContent());
    }
...
{
    SetHTML(String("<p>First paragraph</p>"
                   "<textarea id='selection'>abc def ghi</textarea>"
                   "<p>Second paragraph</p>"));

    TextControlElement* text_ctrl = reinterpret_cast<TextControlElement*>(GetDocument().getElementById(AtomicString("selection")));
    ...
    EXPECT_EQ("abc def ghi", String(surrounding_text.TextContent()).SimplifyWhiteSpace());
    EXPECT_EQ(4u, surrounding_text.StartOffsetInTextContent());
    EXPECT_EQ(7u, surrounding_text.EndOffsetInTextContent());
}
...
TEST_F(SurroundingTextTest, EmptyInputElementWithChild)
{
    ...
    TextControlElement* input_element = reinterpret_cast<TextControlElement*>(GetDocument().getElementById(AtomicString("input_name")));
...
{
    SetHTML(String("<button>.</button>12345"
...
TEST_F(SurroundingTextTest, SelectElementAndText)
{
    ...
    EXPECT_EQ("\xEF\xBF\xBC\n57th Street and Lake Shore Drive\nChicago IL 60637", surrounding_text.TextContent().Utf8());
...
{
    SetHTML(String("<fieldset>.</fieldset>12345<button>abc</button>"
...
{
    SetHTML(String("<option>.</option>12345"
