...
protected:
    using AppendTrailingWhitespace = SelectionController::AppendTrailingWhitespace;
    ...
    static PositionWithAffinity GetPositionFromHitTestResult(const HitTestResult& hit_test_result)
    {
    ...
    void SetNonDirectionalSelectionIfNeeded(const SelectionInFlatTree&, TextGranularity);
};

bool SelectionControllerTest::SelectClosestWordFromHitTestResult(
    const HitTestResult& result, AppendTrailingWhitespace append_trailing_whitespace, SelectInputEventType select_input_event_type)
{
    return Controller().SelectClosestWordFromHitTestResult(result, append_trailing_whitespace, select_input_event_type);
}

void SelectionControllerTest::SetCaretAtHitTestResult(const HitTestResult& hit_test_result)
{
    GetFrame().GetEventHandler().GetSelectionController().SetCaretAtHitTestResult(hit_test_result);
}

void SelectionControllerTest::SetNonDirectionalSelectionIfNeeded(const SelectionInFlatTree& new_selection, TextGranularity granularity)
{
    GetFrame().GetEventHandler().GetSelectionController().SetNonDirectionalSelectionIfNeeded(
        new_selection, SetSelectionOptions::Builder().SetGranularity(granularity).Build(), SelectionController::kDoNotAdjustEndpoints);
}
...
TEST_F(SelectionControllerTest, setNonDirectionalSelectionIfNeeded)
{
    ...
    Node* bottom = shadow_root->getElementById(AtomicString("bottom"))->firstChild();

    // top to bottom
    SetNonDirectionalSelectionIfNeeded(
        SelectionInFlatTree::Builder().Collapse(PositionInFlatTree(top, 1)).Extend(PositionInFlatTree(bottom, 3)).Build(), TextGranularity::kCharacter);
    EXPECT_EQ(VisibleSelectionInDOMTree().Start(), VisibleSelectionInDOMTree().Anchor());
    EXPECT_EQ(VisibleSelectionInDOMTree().End(), VisibleSelectionInDOMTree().Focus());
    EXPECT_EQ(Position(top, 1), VisibleSelectionInDOMTree().Start());
    EXPECT_EQ(Position(top, 3), VisibleSelectionInDOMTree().End());

    EXPECT_EQ(PositionInFlatTree(top, 1), GetVisibleSelectionInFlatTree().Anchor());
    EXPECT_EQ(PositionInFlatTree(bottom, 3), GetVisibleSelectionInFlatTree().Focus());
    ...
    // bottom to top
    SetNonDirectionalSelectionIfNeeded(
        SelectionInFlatTree::Builder().Collapse(PositionInFlatTree(bottom, 3)).Extend(PositionInFlatTree(top, 1)).Build(), TextGranularity::kCharacter);
    EXPECT_EQ(VisibleSelectionInDOMTree().End(), VisibleSelectionInDOMTree().Anchor());
    EXPECT_EQ(VisibleSelectionInDOMTree().Start(), VisibleSelectionInDOMTree().Focus());
    EXPECT_EQ(Position(bottom, 0), VisibleSelectionInDOMTree().Start());
    EXPECT_EQ(Position(bottom, 3), VisibleSelectionInDOMTree().End());

    EXPECT_EQ(PositionInFlatTree(bottom, 3), GetVisibleSelectionInFlatTree().Anchor());
    EXPECT_EQ(PositionInFlatTree(top, 1), GetVisibleSelectionInFlatTree().Focus());
    EXPECT_EQ(PositionInFlatTree(top, 1), GetVisibleSelectionInFlatTree().Start());
    EXPECT_EQ(PositionInFlatTree(bottom, 3), GetVisibleSelectionInFlatTree().End());
}
...
TEST_F(SelectionControllerTest, setCaretAtHitTestResult)
{
    ...
    script->setInnerHTML("var sample = document.getElementById('sample');"
    ...
    GetFrame().GetEventHandler().GetSelectionController().HandleGestureLongPress(GetFrame().GetEventHandler().HitTestResultAtLocation(location));
}
...
TEST_F(SelectionControllerTest, setCaretAtHitTestResultWithNullPosition)
{
    ...
    SetCaretAtHitTestResult(GetFrame().GetEventHandler().HitTestResultAtLocation(location));

    EXPECT_TRUE(Selection().GetSelectionInDOMTree().IsNone());
}

// For http://crbug.com/759971
TEST_F(SelectionControllerTest, SetCaretAtHitTestResultWithDisconnectedPosition)
{
    GetDocument().GetSettings()->SetScriptEnabled(true);
    Element* script = GetDocument().CreateRawElement(html_names::kScriptTag);
    script->setInnerHTML("document.designMode = 'on';"
    ...
    blink::WebMouseEvent mouse_event(
        blink::WebInputEvent::Type::kMouseDown, blink::WebInputEvent::kIsCompatibilityEventForTouch, blink::WebInputEvent::GetStaticTimeStampForTests());
    ...
    GetFrame().GetEventHandler().GetSelectionController().HandleMousePressEvent(
        MouseEventWithHitTestResults(mouse_event, location, GetFrame().GetEventHandler().HitTestResultAtLocation(location)));
...
{
    SetBodyContent("<input type=checkbox>"
    ...
    const SelectionInFlatTree& selection = ExpandWithGranularity(
        SelectionInFlatTree::Builder().Collapse(PositionInFlatTree::BeforeNode(*input)).Extend(PositionInFlatTree::AfterNode(*input)).Build(),
        TextGranularity::kWord);
    const SelectionInFlatTree& result = AdjustSelectionWithTrailingWhitespace(selection);

    EXPECT_EQ(PositionInFlatTree::BeforeNode(*input), result.ComputeStartPosition());
    EXPECT_EQ(PositionInFlatTree::AfterNode(*input), result.ComputeEndPosition());
}

// For http://crbug.com/974569
TEST_F(SelectionControllerTest, SelectClosestWordFromHitTestResultAtEndOfLine1)
{
    ...
    HitTestResult result = GetFrame().GetEventHandler().HitTestResultAtLocation(location);
    ASSERT_EQ("<pre>(1)|\n(2)</pre>", GetSelectionTextFromBody(SelectionInDOMTree::Builder().Collapse(GetPositionFromHitTestResult(result)).Build()));

    // Select word by mouse
    EXPECT_TRUE(SelectClosestWordFromHitTestResult(result, AppendTrailingWhitespace::kDontAppend, SelectInputEventType::kMouse));
    EXPECT_EQ("<pre>(1)^\n|(2)</pre>", GetSelectionTextFromBody());

    // Select word by tap
    EXPECT_FALSE(SelectClosestWordFromHitTestResult(result, AppendTrailingWhitespace::kDontAppend, SelectInputEventType::kTouch));
    EXPECT_EQ("<pre>(1)^\n|(2)</pre>", GetSelectionTextFromBody()) << "selection isn't changed";
}

TEST_F(SelectionControllerTest, SelectClosestWordFromHitTestResultAtEndOfLine2)
{
    InsertStyleElement("body { margin: 0; padding: 0; font: 10px monospace; }");
    ...
    HitTestLocation location(gfx::Point(40, 10));
    HitTestResult result = GetFrame().GetEventHandler().HitTestResultAtLocation(location);
    ASSERT_EQ("<pre>ab:|\ncd</pre>", GetSelectionTextFromBody(SelectionInDOMTree::Builder().Collapse(GetPositionFromHitTestResult(result)).Build()));
    ...
    // Select word by tap
    EXPECT_FALSE(SelectClosestWordFromHitTestResult(result, AppendTrailingWhitespace::kDontAppend, SelectInputEventType::kTouch));
    EXPECT_EQ("<pre>ab:^\n|cd</pre>", GetSelectionTextFromBody()) << "selection isn't changed";
}
...
TEST_F(SelectionControllerTest, SelectWordToEndOfLine)
{
    ...
    // Select foo
    blink::WebMouseEvent double_click(blink::WebMouseEvent::Type::kMouseDown, 0, blink::WebInputEvent::GetStaticTimeStampForTests());
    // Frame scale defaults to 0, which would cause a divide-by-zero problem.
    ...
    HitTestResult result = GetFrame().GetEventHandler().HitTestResultAtLocation(location);
    GetFrame().GetEventHandler().GetSelectionController().HandleMousePressEvent(MouseEventWithHitTestResults(double_click, location, result));
    ASSERT_EQ("<div>ab|c def<br>ghi</div>", GetSelectionTextFromBody(SelectionInDOMTree::Builder().Collapse(GetPositionFromHitTestResult(result)).Build()));
    ...
    blink::WebMouseEvent single_shift_click(
        blink::WebMouseEvent::Type::kMouseDown, blink::WebInputEvent::Modifiers::kShiftKey, blink::WebInputEvent::GetStaticTimeStampForTests());
    // Frame scale defaults to 0, which would cause a divide-by-zero problem.
    ...
    HitTestResult single_click_result = GetFrame().GetEventHandler().HitTestResultAtLocation(single_click_location);
    GetFrame().GetEventHandler().GetSelectionController().HandleMousePressEvent(
        MouseEventWithHitTestResults(single_shift_click, single_click_location, single_click_result));
    EXPECT_EQ("<div>^abc def<br>|ghi</div>", GetSelectionTextFromBody());
}
...
{
    LoadAhem();
    InsertStyleElement("body { margin: 0; padding: 0; font: 10px/10px Ahem; } td {width: "
                       "200px}");
    ...
    ASSERT_EQ("<table><tbody><tr><td>fo|o</td><td>bar</td></tr></tbody></table>",
        GetSelectionTextFromBody(SelectionInDOMTree::Builder().Collapse(GetPositionFromHitTestResult(result)).Build()));
    // Select word by mouse
    EXPECT_TRUE(SelectClosestWordFromHitTestResult(result, AppendTrailingWhitespace::kDontAppend, SelectInputEventType::kMouse));
    EXPECT_EQ("<table><tbody><tr><td>^foo|</td><td>bar</td></tr></tbody></table>", GetSelectionTextFromBody());
    ...
    HitTestResult cell1_single_click_result = GetFrame().GetEventHandler().HitTestResultAtLocation(cell1_single_click_location);
    GetFrame().GetEventHandler().GetSelectionController().HandleMousePressEvent(
        MouseEventWithHitTestResults(cell1_single_shift_click, cell1_single_click_location, cell1_single_click_result));
    EXPECT_EQ("<table><tbody><tr><td>^foo|</td><td>bar</td></tr></tbody></table>", GetSelectionTextFromBody());
    ...
    HitTestResult cell2_single_click_result = GetFrame().GetEventHandler().HitTestResultAtLocation(cell2_single_click_location);
    GetFrame().GetEventHandler().GetSelectionController().HandleMousePressEvent(
        MouseEventWithHitTestResults(cell2_single_shift_click, cell2_single_click_location, cell2_single_click_result));
    EXPECT_EQ("<table><tbody><tr><td>^foo</td><td>bar|</td></tr></tbody></table>", GetSelectionTextFromBody());
}
...
TEST_F(SelectionControllerTest, AdjustSelectionByUserSelectWithInput)
{
    ...
    const SelectionInFlatTree& selection
        = ExpandWithGranularity(SelectionInFlatTree::Builder().Collapse(PositionInFlatTree(one, 0)).Build(), TextGranularity::kParagraph);
    SelectionInFlatTree adjust_selection = AdjustSelectionByUserSelect(one, selection);
...
TEST_F(SelectionControllerTest, AdjustSelectionByUserSelectWithSpan)
{
    ...
    SelectionInFlatTree adjust_selection = AdjustSelectionByUserSelect(one, selection);
    EXPECT_EQ(adjust_selection.Anchor(), selection.Anchor());
    EXPECT_EQ(adjust_selection.Focus(), PositionInFlatTree::LastPositionInNode(*two->firstChild()));
}
...
TEST_F(SelectionControllerTest, AdjustSelectionByUserSelectWithComment)
{
    ...
    SelectionInFlatTree adjust_selection = AdjustSelectionByUserSelect(one, selection);
    EXPECT_EQ(adjust_selection.Anchor(), selection.Anchor());
    EXPECT_EQ(adjust_selection.Anchor(), PositionInFlatTree::FirstPositionInNode(*one->firstChild()));
