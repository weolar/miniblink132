...
TEST_F(PositionTest, IsEquivalent)
{
    ...
    EXPECT_TRUE(Position(sample, 0).IsEquivalent(Position::FirstPositionInNode(*sample)));
    EXPECT_TRUE(Position(sample, 0).IsEquivalent(Position::BeforeNode(*sample->firstChild())));
    EXPECT_TRUE(Position(sample, 1).IsEquivalent(Position::AfterNode(*sample->firstChild())));
    EXPECT_TRUE(Position(sample, 1).IsEquivalent(Position::BeforeNode(*sample->firstChild()->nextSibling())));
    EXPECT_TRUE(Position(sample, 2).IsEquivalent(Position::BeforeNode(*sample->lastChild())));
    EXPECT_TRUE(Position(sample, 3).IsEquivalent(Position::AfterNode(*sample->lastChild())));
    EXPECT_TRUE(Position(sample, 3).IsEquivalent(Position::LastPositionInNode(*sample)));

    EXPECT_FALSE(Position(sample, 0).IsEquivalent(Position(sample, 1)));
    EXPECT_FALSE(Position(sample, 0).IsEquivalent(Position::LastPositionInNode(*sample)));
}
...
{
    const char* body_content = "<textarea id=textarea></textarea><a id=child1>1</a><b id=child2>2</b>";
...
TEST_F(PositionTest, LastPositionInOrAfterNodeNotInFlatTree)
{
    ...
    EXPECT_EQ(Position::LastPositionInNode(document_element), Position::LastPositionInOrAfterNode(document_element));
    EXPECT_EQ(PositionInFlatTree::LastPositionInNode(document_element), PositionInFlatTree::LastPositionInOrAfterNode(document_element));

    // Note: <select> isn't appeared in flat tree, because <option> doesn't
    // take it as valid child node.
    EXPECT_EQ(Position::AfterNode(select), Position::LastPositionInOrAfterNode(select));
    EXPECT_EQ(PositionInFlatTree::LastPositionInNode(select), PositionInFlatTree::LastPositionInOrAfterNode(select));
}

TEST_F(PositionTest, NodeAsRangeLastNode)
{
    const char* body_content = "<p id='p1'>11</p><p id='p2'></p><p id='p3'>33</p>";
    SetBodyContent(body_content);
...
TEST_F(PositionTest, DISABLED_NodeAsRangeLastNodeShadow)
{
    const char* body_content = "<p id='host'>00<b slot='#one' id='one'>11</b><b slot='#two' "
                               "id='two'>22</b>33</p>";
    const char* shadow_content = "<a id='a'><slot name='#two'></slot><slot name='#one'></slot></a>";
    SetBodyContent(body_content);
...
{
    const char* body_content = "<p id='host'>00<b slot='#one' id='one'>11</b>22</p>";
    const char* shadow_content = "<a id='a'><slot name=#one id='content'></slot><slot></slot></a>";
    SetBodyContent(body_content);
    ShadowRoot* shadow_root = SetShadowContent(shadow_content, "host");
    ...
    EXPECT_EQ(PositionInFlatTree::LastPositionInNode(*anchor), ToPositionInFlatTree(Position(anchor, 2)));
}
...
TEST_F(PositionTest, ToPositionInFlatTreeWithInactiveInsertionPoint)
{
    ...
    EXPECT_EQ(PositionInFlatTree(anchor, 0), ToPositionInFlatTree(Position(anchor, 0)));
    EXPECT_EQ(PositionInFlatTree::LastPositionInNode(*anchor), ToPositionInFlatTree(Position(anchor, 1)));
}
...
TEST_F(PositionTest, ToPositionInFlatTreeWithNotDistributed)
{
    ...
    EXPECT_EQ(PositionInFlatTree::FirstPositionInNode(*sample), ToPositionInFlatTree(Position(sample, 0)));
}
...
TEST_F(PositionTest, ToPositionInFlatTreeWithShadowRoot)
{
    ...
    EXPECT_EQ(PositionInFlatTree(host, 0), ToPositionInFlatTree(Position(shadow_root, 0)));
    EXPECT_EQ(PositionInFlatTree::LastPositionInNode(*host), ToPositionInFlatTree(Position(shadow_root, 1)));
    EXPECT_EQ(PositionInFlatTree::LastPositionInNode(*host), ToPositionInFlatTree(Position::LastPositionInNode(*shadow_root)));
    EXPECT_EQ(PositionInFlatTree::FirstPositionInNode(*host), ToPositionInFlatTree(Position::FirstPositionInNode(*shadow_root)));
}
...
TEST_F(PositionTest, ToPositionInFlatTreeWithEmptyShadowRoot)
{
    ...
    EXPECT_EQ(PositionInFlatTree::FirstPositionInNode(*host), ToPositionInFlatTree(Position(shadow_root, 0)));
}
...
{
    Position position = SetCaretTextToBody("<div>f|oo<template data-mode=open>bar</template></div>");
    EXPECT_TRUE(position.IsConnected());
    EXPECT_FALSE(ToPositionInFlatTree(position).IsConnected());
}
...
TEST_F(PositionTest, ComparePositionsAcrossShadowBoundary)
{
    ...
    for (unsigned i = 0; i < size; ++i) {
        for (unsigned j = 0; j < i; ++j) {
            ...
            EXPECT_LT(PositionInFlatTree(nodes[j], 0), PositionInFlatTree(nodes[i], 0));
        }
        ...
        for (unsigned j = i + 1; j < size; ++j) {
            ...
            EXPECT_GT(PositionInFlatTree(nodes[j], 0), PositionInFlatTree(nodes[i], 0));
        }
    }
}
