...
bool IsValidRange(const RangeInFlatTree* range)
{
    ...
    return range && range->IsConnected() && !range->IsCollapsed() && !range->ToEphemeralRange().IsCollapsed();
}
...
bool IsValidRangeForTextFinder(const RangeInFlatTree* range)
{
    ...
    for (; !object->IsLayoutView(); object = object->Parent()) {
        ...
        if (box->HasNonVisibleOverflow()) {
            if (box->StyleRef().OverflowX() != EOverflow::kVisible && box->Size().width.RawValue() <= 0) {
                return false;
            }

            if (box->StyleRef().OverflowY() != EOverflow::kVisible && box->Size().height.RawValue() <= 0) {
                return false;
            }
        }
        ...
        if (box->StyleRef().GetPosition() == EPosition::kFixed) {
            PhysicalRect view_rect = PhysicalRect::EnclosingRect(box->View()->AbsoluteBoundingBoxRectF());
            if (!view_rect.Intersects(common_node->GetLayoutObject()->AbsoluteBoundingBoxRectForScrollIntoView())) {
                return false;
            }
        }
    }
...
AnnotationAgentImpl::AnnotationAgentImpl(AnnotationAgentContainerImpl& owning_container, mojom::blink::AnnotationType annotation_type,
    AnnotationSelector& selector, AnnotationAgentContainerImpl::PassKey)
    : agent_host_(owning_container.GetSupplementable()->GetExecutionContext())
    , receiver_(this, owning_container.GetSupplementable()->GetExecutionContext())
...
void AnnotationAgentImpl::Bind(
    mojo::PendingRemote<mojom::blink::AnnotationAgentHost> host_remote, mojo::PendingReceiver<mojom::blink::AnnotationAgent> agent_receiver)
{
    DCHECK(!IsRemoved());

    scoped_refptr<base::SingleThreadTaskRunner> task_runner = owning_container_->GetSupplementable()->GetTaskRunner(TaskType::kInternalDefault);
    ...
    // the container.
    receiver_.set_disconnect_handler(WTF::BindOnce(&AnnotationAgentImpl::Remove, WrapWeakPersistent(this)));
}
...
void AnnotationAgentImpl::Attach(AnnotationAgentContainerImpl::PassKey)
{
    ...
    selector_->FindRange(document, AnnotationSelector::kSynchronous, WTF::BindOnce(&AnnotationAgentImpl::DidFinishFindRange, WrapWeakPersistent(this)));
}
...
void AnnotationAgentImpl::Remove()
{
    ...
    if (IsAttached()) {
        EphemeralRange dom_range = EphemeralRange(ToPositionInDOMTree(attached_range_->StartPosition()), ToPositionInDOMTree(attached_range_->EndPosition()));
        ...
        if (LocalFrame* frame = document->GetFrame()) {
            ...
            // modifications.
            frame->GetDocument()->UpdateStyleAndLayout(DocumentUpdateReason::kFindInPage);

            document->Markers().RemoveMarkersInRange(dom_range, DocumentMarker::MarkerTypes::TextFragment());
        }
    }
...
void AnnotationAgentImpl::ScrollIntoView() const
{
    ...
    Document& document = *owning_container_->GetSupplementable();
    document.EnsurePaintLocationDataValidForNode(&first_node, DocumentUpdateReason::kFindInPage);
    ...
    mojom::blink::ScrollIntoViewParamsPtr params = scroll_into_view_util::CreateScrollIntoViewParams(
        ScrollAlignment::CenterAlways(), ScrollAlignment::CenterAlways(), mojom::blink::ScrollType::kProgrammatic);
    params->cross_origin_boundaries = false;

    scroll_into_view_util::ScrollRectToVisible(*first_node.GetLayoutObject(), bounding_box, std::move(params));
}

void AnnotationAgentImpl::DidFinishFindRange(const RangeInFlatTree* range)
{
    TRACE_EVENT("blink", "AnnotationAgentImpl::DidFinishFindRange", "bound_to_host", agent_host_.is_bound());
    if (IsRemoved()) {
    ...
    if (!NeedsDOMMutationToAttach()) {
        ...
        // throttled iframe.
        Document& document = *owning_container_->GetSupplementable();
        document.EnqueueAnimationFrameTask(WTF::BindOnce(&AnnotationAgentImpl::PerformPreAttachDOMMutation, WrapPersistent(this)));
    }
}
...
void AnnotationAgentImpl::PerformPreAttachDOMMutation()
{
    if (IsValidRange(pending_range_)) {
        ...
        DisplayLockUtilities::ActivateFindInPageMatchRangeIfNeeded(pending_range_->ToEphemeralRange());
        ...
        if (HTMLDetailsElement::ExpandDetailsAncestors(first_node)) {
            UseCounter::Count(first_node.GetDocument(), WebFeature::kAutoExpandedDetailsForScrollToTextFragment);
        }
        ...
        first_node.GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kFindInPage);
    }
...
void AnnotationAgentImpl::ProcessAttachmentFinished()
{
    ...
    // here.
    bool pending_range_valid = type_ == mojom::blink::AnnotationType::kTextFinder ? IsValidRangeForTextFinder(pending_range_) : IsValidRange(pending_range_);
    ...
    if (pending_range_valid) {
        ...
        if (type_ != mojom::blink::AnnotationType::kTextFinder) {
            ...
            document->Markers().MergeOverlappingMarkers(DocumentMarker::kTextFragment);
        }
    ...
    if (agent_host_.is_bound()) {
        ...
        if (IsAttached()) {
            gfx::Rect rect_in_frame = ComputeTextRect(attached_range_->ToEphemeralRange());
