...
{
    DEFINE_STATIC_LOCAL(std::optional<AnnotationSelector::GeneratorFunc>, generator, ());
    return generator;
}
} //  namespace
...
AnnotationSelector* AnnotationSelector::Deserialize(const String& serialized)
{
    ...
    return MakeGarbageCollected<TextAnnotationSelector>(TextFragmentSelector::FromTextDirective(serialized));
}
