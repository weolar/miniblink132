...
// link generations.
class CORE_EXPORT AnnotationAgentGenerator final : public GarbageCollected<AnnotationAgentGenerator> {
public:
    ...
    using SelectorGenerationCallback = base::OnceCallback<void(mojom::blink::AnnotationType, shared_highlighting::LinkGenerationReadyStatus, const String&,
        const TextFragmentSelector&, shared_highlighting::LinkGenerationError)>;
    ...
    // by the callback.
    void GetForCurrentSelection(mojom::blink::AnnotationType type, SelectorGenerationCallback callback);

private:
    void GenerateSelector();
    void DidFinishGeneration(const TextFragmentSelector& selector, shared_highlighting::LinkGenerationError error);
    void InvokeCompletionCallbackIfNeeded(shared_highlighting::LinkGenerationReadyStatus ready_status);
