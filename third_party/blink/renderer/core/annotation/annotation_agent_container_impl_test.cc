...
protected:
    bool IsInContainer(AnnotationAgentImpl& agent, AnnotationAgentContainerImpl& container) const
    {
    ...
    {
        auto event = frame_test_helpers::CreateMouseEvent(WebMouseEvent::Type::kMouseDown, WebMouseEvent::Button::kRight, click_point, /*modifiers=*/0);
        event.click_count = 1;
        WebView().MainFrameViewWidget()->HandleInputEvent(WebCoalescedInputEvent(event, ui::LatencyInfo()));
    }
...
TEST_F(AnnotationAgentContainerImplTest, IsConstructedLazily)
{
    ...
    LocalFrame* child_frame = DynamicTo<LocalFrame>(GetDocument().GetFrame()->FirstChild());
    ...
    auto* child_container = AnnotationAgentContainerImpl::CreateIfNeeded(*child_document);
    EXPECT_TRUE(child_container);
    EXPECT_EQ(child_container, AnnotationAgentContainerImpl::FromIfExists(*child_document));
    EXPECT_FALSE(AnnotationAgentContainerImpl::FromIfExists(GetDocument()));

    // Calling the getter for the main document should now create that supplement.
    auto* main_container = AnnotationAgentContainerImpl::CreateIfNeeded(GetDocument());
    EXPECT_EQ(main_container, AnnotationAgentContainerImpl::FromIfExists(GetDocument()));
...
TEST_F(AnnotationAgentContainerImplTest, BindingCreatesContainer)
{
    ...
    AnnotationAgentContainerImpl::BindReceiver(GetDocument().GetFrame(), remote.BindNewPipeAndPassReceiver());
...
TEST_F(AnnotationAgentContainerImplTest, NavigationReplacesContainer)
{
    ...
    auto* container_next = AnnotationAgentContainerImpl::CreateIfNeeded(GetDocument());
...
TEST_F(AnnotationAgentContainerImplTest, CreateBoundAgent)
{
    ...
    auto* container = AnnotationAgentContainerImpl::CreateIfNeeded(GetDocument());
    ASSERT_TRUE(container);
    container->CreateAgent(std::move(remote_receiver_pair.first), std::move(remote_receiver_pair.second), mojom::blink::AnnotationType::kSharedHighlight,
...
TEST_F(AnnotationAgentContainerImplTest, DeferAttachmentUntilFinishedParsing)
{
    ...
    container->CreateAgent(
        std::move(remote_receiver_pair.first), std::move(remote_receiver_pair.second), mojom::blink::AnnotationType::kUserNote, "MockAnnotationSelector");
...
TEST_F(AnnotationAgentContainerImplTest, ManuallyRemoveAgent)
{
    ...
    auto* container = AnnotationAgentContainerImpl::CreateIfNeeded(GetDocument());
    ASSERT_TRUE(container);
    auto* agent1 = container->CreateUnboundAgent(mojom::blink::AnnotationType::kSharedHighlight, *MakeGarbageCollected<MockAnnotationSelector>());
    auto* agent2 = container->CreateUnboundAgent(mojom::blink::AnnotationType::kSharedHighlight, *MakeGarbageCollected<MockAnnotationSelector>());
...
TEST_F(AnnotationAgentContainerImplTest, DetachedDocumentContainerBecomesInaccessible)
{
    SimRequest request("https://example.com/test.html", "text/html");
    SimRequest request_next("https://example.com/next.html", "text/html");
    LoadURL("https://example.com/test.html");
...
// caller.
TEST_F(AnnotationAgentContainerImplTest, CreateAgentFromSelectionWithNoSelection)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
    ...
    container->CreateAgentFromSelection(mojom::blink::AnnotationType::kUserNote,
        base::BindLambdaForTesting([&did_reply](mojom::blink::SelectorCreationResultPtr selector_creation_result,
            ...
            EXPECT_EQ(error, shared_highlighting::LinkGenerationError::kEmptySelection);
            // Test that the generation was not preemptive, the result was not
            // ready by the time we called CreateAgentFromSelection.
            EXPECT_EQ(ready_status, shared_highlighting::LinkGenerationReadyStatus::kRequestedBeforeReady);
        }));
...
// show the highlight.
TEST_F(AnnotationAgentContainerImplTest, CreateAgentFromSelectionWithCollapsedSelection)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
    ...
    frame_selection.SetSelection(SelectionInDOMTree::Builder().Collapse(Position(body->firstChild(), 0)).Build(), SetSelectionOptions());
...
TEST_F(AnnotationAgentContainerImplTest, CreateAgentFromSelectionWithPreemptiveGeneration)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
    ...
    Element* body = GetDocument().body();
    EphemeralRange range = EphemeralRange(Position(body->firstChild(), 0), Position(body->firstChild(), 5));
    ASSERT_EQ("TEST ", PlainText(range));

    frame_selection.SetSelection(SelectionInDOMTree::Builder().SetBaseAndExtent(range).Build(), SetSelectionOptions());
    ...
    bool did_reply = false;
    container->CreateAgentFromSelection(mojom::blink::AnnotationType::kUserNote,
        base::BindLambdaForTesting([&did_reply, &host](mojom::blink::SelectorCreationResultPtr selector_creation_result,
                                       shared_highlighting::LinkGenerationError error, shared_highlighting::LinkGenerationReadyStatus ready_status) {
            did_reply = true;

            EXPECT_EQ(selector_creation_result->selected_text, "TEST");
            EXPECT_EQ(selector_creation_result->serialized_selector, "TEST,-PAGE");
            ...
            EXPECT_EQ(ready_status, shared_highlighting::LinkGenerationReadyStatus::kRequestedAfterReady);

            host.Bind(std::move(selector_creation_result->host_receiver), std::move(selector_creation_result->agent_remote));
        }));
...
TEST_F(AnnotationAgentContainerImplTest, CreateAgentFromSelection)
{
    ...
    base::RunLoop run_loop;
    container->CreateAgentFromSelection(mojom::blink::AnnotationType::kUserNote,
        base::BindLambdaForTesting([&run_loop, &host](mojom::blink::SelectorCreationResultPtr selector_creation_result,
                                       shared_highlighting::LinkGenerationError error, shared_highlighting::LinkGenerationReadyStatus ready_status) {
            run_loop.Quit();
...
TEST_F(AnnotationAgentContainerImplTest, ShutdownDocumentWhileGenerating)
{
    ...
    // work.
    auto auto_reset_timeout = AsyncFindBuffer::OverrideTimeoutForTesting(base::TimeDelta::Min());
    ...
    EphemeralRange range = EphemeralRange(Position(target, 0), Position(target, 1));
    ...
    container->CreateAgentFromSelection(mojom::blink::AnnotationType::kUserNote,
        base::BindLambdaForTesting([&did_finish](mojom::blink::SelectorCreationResultPtr selector_creation_result,
                                       shared_highlighting::LinkGenerationError error, shared_highlighting::LinkGenerationReadyStatus ready_status) {
            did_finish = true;
            EXPECT_FALSE(selector_creation_result);
            EXPECT_EQ(error, shared_highlighting::LinkGenerationError::kIncorrectSelector);
            // Test that the generation was preemptive, the result was ready by
            // the time we called CreateAgentFromSelection.
            EXPECT_EQ(ready_status, shared_highlighting::LinkGenerationReadyStatus::kRequestedAfterReady);
        }));
    ...
    // that task runs.
    GetDocument().GetTaskRunner(TaskType::kInternalFindInPage)->PostTask(FROM_HERE, run_loop.QuitClosure());
    run_loop.Run();
    ...
    auto* new_container = AnnotationAgentContainerImpl::CreateIfNeeded(GetDocument());
