...
class CORE_EXPORT AnnotationAgentImpl final : public GarbageCollected<AnnotationAgentImpl>, public mojom::blink::AnnotationAgent {
public:
    using PassKey = base::PassKey<AnnotationAgentImpl>;

    // Agents can only be created via AnnotationAgentContainerImpl.
    AnnotationAgentImpl(AnnotationAgentContainerImpl& owning_container, mojom::blink::AnnotationType annotation_type, AnnotationSelector& selector,
    ...
    void Bind(mojo::PendingRemote<mojom::blink::AnnotationAgentHost> host_remote, mojo::PendingReceiver<mojom::blink::AnnotationAgent> agent_receiver);
    ...
    void SetNeedsAttachment()
    {
        needs_attachment_ = true;
    }
    bool NeedsAttachment() const
    {
        return needs_attachment_;
    }
    ...
    const AnnotationSelector* GetSelector() const
    {
        return selector_.Get();
    }

    mojom::blink::AnnotationType GetType() const
    {
        return type_;
    }
...
private:
    ...
    HeapMojoReceiver<mojom::blink::AnnotationAgent, AnnotationAgentImpl> receiver_;
