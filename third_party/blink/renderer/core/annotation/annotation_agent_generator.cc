...
void AnnotationAgentGenerator::GetForCurrentSelection(mojom::blink::AnnotationType type, SelectorGenerationCallback callback)
{
    ...
    if (callback_) {
        std::move(callback_).Run(type_, shared_highlighting::LinkGenerationReadyStatus::kRequestedAfterReady, "",
            TextFragmentSelector(TextFragmentSelector::SelectorType::kInvalid), shared_highlighting::LinkGenerationError::kUnknown);
    }
    ...
    if (generation_result_.has_value()) {
        InvokeCompletionCallbackIfNeeded(shared_highlighting::LinkGenerationReadyStatus::kRequestedAfterReady);
        return;
    }
...
void AnnotationAgentGenerator::InvokeCompletionCallbackIfNeeded(shared_highlighting::LinkGenerationReadyStatus ready_status)
{
    DCHECK(callback_);
    ...
    std::move(callback_).Run(type_, ready_status, generator_ ? generator_->GetSelectorTargetText() : "", generation_result_.value(), selector_error_);
...
void AnnotationAgentGenerator::GenerateSelector()
{
    ...
    VisibleSelectionInFlatTree selection = frame_->Selection().ComputeVisibleSelectionInFlatTree();
    if (selection.IsNone() || !selection.IsRange()) {
        if (callback_) {
            generation_result_.emplace(TextFragmentSelector(TextFragmentSelector::SelectorType::kInvalid));
            selector_error_ = shared_highlighting::LinkGenerationError::kEmptySelection;
            InvokeCompletionCallbackIfNeeded(shared_highlighting::LinkGenerationReadyStatus::kRequestedBeforeReady);
        }
        return;
    }
    ...
    generator_->Generate(*current_selection_range, WTF::BindOnce(&AnnotationAgentGenerator::DidFinishGeneration, WrapWeakPersistent(this)));
}

void AnnotationAgentGenerator::DidFinishGeneration(const TextFragmentSelector& selector, shared_highlighting::LinkGenerationError error)
{
    DCHECK(!generation_result_.has_value());
