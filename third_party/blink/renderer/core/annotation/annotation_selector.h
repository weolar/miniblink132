...
class CORE_EXPORT AnnotationSelector : public GarbageCollected<AnnotationSelector> {
public:
    enum SearchType {
    ...
    using GeneratorFunc = base::RepeatingCallback<AnnotationSelector*(const String&)>;
    ...
    virtual void FindRange(Document& document, SearchType type, FinishedCallback finished_cb) = 0;

    virtual bool IsTextSelector() const
    {
        return false;
    }
};
