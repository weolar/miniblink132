...
TextAnnotationSelector::TextAnnotationSelector(const TextFragmentSelector& params)
    : params_(params)
{
}
...
void TextAnnotationSelector::FindRange(Document& document, SearchType type, FinishedCallback finished_cb)
{
    ...
    finder_ = MakeGarbageCollected<TextFragmentFinder>(*this, params_, &document, find_buffer_type);
    finished_callback_ = std::move(finished_cb);
    finder_->FindMatch();
}

void TextAnnotationSelector::DidFindMatch(const RangeInFlatTree& range, bool is_unique)
{
