...
class CORE_EXPORT AnnotationAgentContainerImpl final : public GarbageCollected<AnnotationAgentContainerImpl>,
...
public:
    ...
    static void BindReceiver(LocalFrame* frame, mojo::PendingReceiver<mojom::blink::AnnotationAgentContainer> receiver);

    // Only instantiated using static From method.
    explicit AnnotationAgentContainerImpl(Document&, base::PassKey<AnnotationAgentContainerImpl>);
    ~AnnotationAgentContainerImpl() override = default;

    // Not copyable or movable
    AnnotationAgentContainerImpl(const AnnotationAgentContainerImpl&) = delete;
    AnnotationAgentContainerImpl& operator=(const AnnotationAgentContainerImpl&) = delete;

    void Bind(mojo::PendingReceiver<mojom::blink::AnnotationAgentContainer> receiver);
    ...
    void RemoveAgent(AnnotationAgentImpl& agent, base::PassKey<AnnotationAgentImpl>);

    // Returns all annotation agents in this container of the given type.
    HeapHashSet<Member<AnnotationAgentImpl>> GetAgentsOfType(mojom::blink::AnnotationType type);

    // Use from within Blink to create an agent in this container.
    AnnotationAgentImpl* CreateUnboundAgent(mojom::blink::AnnotationType type, AnnotationSelector& selector);

    // mojom::blink::AnnotationAgentContainer
    void CreateAgent(mojo::PendingRemote<mojom::blink::AnnotationAgentHost> host_remote, mojo::PendingReceiver<mojom::blink::AnnotationAgent> agent_receiver,
        mojom::blink::AnnotationType type, const String& serialized_selector) override;
    void CreateAgentFromSelection(mojom::blink::AnnotationType type, CreateAgentFromSelectionCallback callback) override;
...
private:
    ...
    void DidFinishSelectorGeneration(CreateAgentFromSelectionCallback callback, mojom::blink::AnnotationType type,
    ...
    HeapMojoReceiverSet<mojom::blink::AnnotationAgentContainer, AnnotationAgentContainerImpl> receivers_;
