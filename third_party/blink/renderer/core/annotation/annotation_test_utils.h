...
public:
    ...
    void FindRange(Document& document, SearchType type, FinishedCallback finished_cb) override
    {
...
public:
    ...
    {
        AnnotationSelector::SetGeneratorForTesting(WTF::BindRepeating(&ScopedUseMockAnnotationSelector::Generate, WTF::Unretained(this)));
    }
...
public:
    ...
    void BindToAgent(AnnotationAgentImpl& agent)
    {
        ...
        receiver_.set_disconnect_handler(WTF::BindRepeating([](bool* did_disconnect) { *did_disconnect = true; }, WTF::Unretained(&did_disconnect_)));

        agent.Bind(std::move(pending_remote), agent_.BindNewPipeAndPassReceiver());
    }

    void Bind(mojo::PendingReceiver<mojom::blink::AnnotationAgentHost> host_receiver, mojo::PendingRemote<mojom::blink::AnnotationAgent> agent_remote)
    {
        agent_.Bind(std::move(agent_remote));
        receiver_.Bind(std::move(host_receiver));
    }

    using RemoteHostReceiverAgentPair = std::pair<mojo::PendingRemote<mojom::blink::AnnotationAgentHost>, mojo::PendingReceiver<mojom::blink::AnnotationAgent>>;
    ...
    RemoteHostReceiverAgentPair BindForCreateAgent()
    {
        ...
        return std::make_pair(std::move(pending_remote), agent_.BindNewPipeAndPassReceiver());
    }
