...
protected:
    ...
    RangeInFlatTree* CreateRangeToExpectedText(Element* element, int start_offset, int end_offset, const String& expected)
    {
        EXPECT_TRUE(element);
        if (!element)
            return nullptr;
        ...
        return MakeGarbageCollected<RangeInFlatTree>(ToPositionInFlatTree(range_start), ToPositionInFlatTree(range_end));
    }
    ...
    {
        return GetDocument().Markers().MarkersIntersectingRange(range.ToEphemeralRange(), DocumentMarker::MarkerTypes::TextFragment()).size();
    }

    // Creates an agent with a mock selector that will relect the given range
    // when attached.
    AnnotationAgentImpl* CreateAgentForRange(RangeInFlatTree* range, mojom::blink::AnnotationType type = mojom::blink::AnnotationType::kSharedHighlight)
    {
        EXPECT_TRUE(range);
        if (!range)
            return nullptr;
    ...
    AnnotationAgentImpl* CreateAgentFailsAttach()
    {
        ...
        auto* mock_selector = MakeGarbageCollected<MockAnnotationSelector>(*null_range);
        return container->CreateUnboundAgent(mojom::blink::AnnotationType::kSharedHighlight, *mock_selector);
    }
    ...
    AnnotationAgentImpl* CreateTextFinderAgent(const String& text_selector, mojom::blink::AnnotationType type = mojom::blink::AnnotationType::kSharedHighlight)
    {
        auto* selector = MakeGarbageCollected<TextAnnotationSelector>(TextFragmentSelector::FromTextDirective(text_selector));
        auto* container = AnnotationAgentContainerImpl::CreateIfNeeded(GetDocument());
    ...
    {
        VisualViewport& viewport = GetDocument().View()->GetPage()->GetVisualViewport();
        gfx::Rect rect_in_visual_viewport = viewport.RootFrameToViewport(node.GetLayoutObject()->AbsoluteBoundingBoxRect(kTraverseDocumentBoundaries));
        gfx::Rect viewport_rect(viewport.Size());

        bool is_contained = viewport_rect.Contains(rect_in_visual_viewport);
        EXPECT_TRUE(is_contained) << "Expected [" << node.DebugName() << "] to be visible in viewport. Bounds relative to viewport: ["
                                  << rect_in_visual_viewport.ToString() << "] vs. viewport bounds [ " << viewport_rect.ToString() << " ]";
        return is_contained;
    }
    ...
    bool ExpectNotInViewport(Node& node)
    {
        ...
        bool is_contained = viewport_rect.Contains(rect_in_visual_viewport);
        EXPECT_FALSE(is_contained) << "Expected [" << node.DebugName() << "] to be visible in viewport. Bounds relative to viewport: ["
                                   << rect_in_visual_viewport.ToString() << "] vs. viewport bounds [ " << viewport_rect.ToString() << " ]";
        return !is_contained;
    }
    ...
    bool IsRemoved(AnnotationAgentImpl* agent)
    {
        return agent->IsRemoved();
    }
...
TEST_F(AnnotationAgentImplTest, AgentType)
{
    ...
    auto* shared_highlight_agent
        = container->CreateUnboundAgent(mojom::blink::AnnotationType::kSharedHighlight, *MakeGarbageCollected<MockAnnotationSelector>());

    auto* user_note_agent = container->CreateUnboundAgent(mojom::blink::AnnotationType::kUserNote, *MakeGarbageCollected<MockAnnotationSelector>());

    EXPECT_EQ(GetAgentType(shared_highlight_agent), mojom::blink::AnnotationType::kSharedHighlight);
    EXPECT_EQ(GetAgentType(user_note_agent), mojom::blink::AnnotationType::kUserNote);
}
...
TEST_F(AnnotationAgentImplTest, CreatingDoesntBindOrAttach)
{
    ...
    auto* container = AnnotationAgentContainerImpl::CreateIfNeeded(GetDocument());
    ASSERT_TRUE(container);
    auto* agent = container->CreateUnboundAgent(mojom::blink::AnnotationType::kSharedHighlight, *MakeGarbageCollected<MockAnnotationSelector>());
...
TEST_F(AnnotationAgentImplTest, AgentFailsAttachment)
{
    ...
    Element* p = GetDocument().getElementById(AtomicString("text"));
    RangeInFlatTree* range = CreateRangeToExpectedText(p, 0, 17, "TEST FOO PAGE BAR");
...
TEST_F(AnnotationAgentImplTest, AttachmentToOverlappingMarkerReportsToHost)
{
    ...
    auto* agent = CreateAgentFailsAttach();
    ASSERT_TRUE(agent);
    RangeInFlatTree* range_foo = CreateRangeToExpectedText(element_text, 5, 13, "FOO PAGE");
    auto* agent_foo = CreateAgentForRange(range_foo);
    ASSERT_TRUE(agent_foo);

    RangeInFlatTree* range_bar = CreateRangeToExpectedText(element_text, 9, 17, "PAGE BAR");
    auto* agent_bar = CreateAgentForRange(range_bar);
    ASSERT_TRUE(agent_bar);
...
TEST_F(AnnotationAgentImplTest, AttachmentReportsRectsToHost)
{
    ...
    // the attached ranges are in the document coordinate space.
    {
        ...
        GetDocument().View()->GetRootFrameViewport()->SetScrollOffset(
            ScrollOffset(123, 3000), mojom::blink::ScrollType::kProgrammatic, mojom::blink::ScrollBehavior::kInstant, ScrollableArea::ScrollCallback());
        ...
        ASSERT_EQ(GetDocument().View()->LayoutViewport()->GetScrollOffset(), ScrollOffset(0, 2700));
    }
    ...
    RangeInFlatTree* range_foo = CreateRangeToExpectedText(element_foo, 0, 3, "FOO");
    auto* agent_foo = CreateAgentForRange(range_foo);
    ASSERT_TRUE(agent_foo);

    RangeInFlatTree* range_bar = CreateRangeToExpectedText(element_bar, 0, 3, "BAR");
    auto* agent_bar = CreateAgentForRange(range_bar);
    ASSERT_TRUE(agent_bar);
...
TEST_F(AnnotationAgentImplTest, AgentScrollIntoView)
{
    ...
    ASSERT_EQ(GetDocument().View()->GetRootFrameViewport()->GetScrollOffset(), ScrollOffset());
...
TEST_F(AnnotationAgentImplTest, ScrollIntoViewWithDirtyLayout)
{
    ...
    RangeInFlatTree* range_foo = CreateRangeToExpectedText(element_text, 0, 3, "FOO");
    auto* agent_foo = CreateAgentForRange(range_foo);
    ASSERT_TRUE(agent_foo);
    ...
    element_text->setAttribute(html_names::kStyleAttr, AtomicString("top: 2000px"));
    ...
    EXPECT_GT(GetDocument().View()->GetRootFrameViewport()->GetScrollOffset().y(), 1000);
}
...
TEST_F(AnnotationAgentImplTest, ScrollIntoViewCollapsedRange)
{
    ...
    const auto& range_start = Position(element_text->firstChild(), PositionAnchorType::kBeforeAnchor);
    const auto& range_end = Position(element_text, 0);

    RangeInFlatTree* range = MakeGarbageCollected<RangeInFlatTree>(ToPositionInFlatTree(range_start), ToPositionInFlatTree(range_end));
    ...
    host.FlushForTesting();
    EXPECT_EQ(GetDocument().View()->GetRootFrameViewport()->GetScrollOffset().y(), 0);
}
...
TEST_F(AnnotationAgentImplTest, OpenDetailsElement)
{
    ...
    Element* element_details = GetDocument().getElementById(AtomicString("details"));
    ...
    EXPECT_EQ(GetDocument().View()->GetRootFrameViewport()->GetScrollOffset(), ScrollOffset());
    ...
    agent->ScrollIntoView();
    EXPECT_GT(GetDocument().View()->GetRootFrameViewport()->GetScrollOffset().y(), 100.f);
}
...
TEST_F(AnnotationAgentImplTest, ActivatesContentVisibilityAuto)
{
    ...
    EXPECT_FALSE(DisplayLockUtilities::NeedsActivationForFindInPage(range->ToEphemeralRange()));
}
...
TEST_F(AnnotationAgentImplTest, TextFinderDoesntMutateDom)
{
    ...
    auto* agent_foo = CreateTextFinderAgent("FOO", mojom::blink::AnnotationType::kTextFinder);
    ASSERT_TRUE(agent_foo);
    ...
    auto* agent_bar = CreateTextFinderAgent("BAR", mojom::blink::AnnotationType::kSharedHighlight);
    Compositor().BeginFrame();
...
TEST_F(AnnotationAgentImplTest, TextFinderDoesntAddMarkers)
{
    ...
    Element* p = GetDocument().getElementById(AtomicString("text"));
    RangeInFlatTree* range_foo = CreateRangeToExpectedText(p, 5, 8, "FOO");
    auto* agent_foo = CreateAgentForRange(range_foo, mojom::blink::AnnotationType::kTextFinder);
    ASSERT_TRUE(agent_foo);
    ...
    RangeInFlatTree* range_bar = CreateRangeToExpectedText(p, 14, 17, "BAR");
    CreateAgentForRange(range_bar, mojom::blink::AnnotationType::kSharedHighlight);
    Compositor().BeginFrame();
...
TEST_F(AnnotationAgentImplTest, TextFinderDoesntFindEmptyOverflowHidden)
{
    ...
    // compute to auto).
    {
        container->setAttribute(html_names::kStyleAttr, AtomicString("height: 0px; overflow: visible hidden"));
    ...
    // the non-empty axis.
    {
        container->setAttribute(html_names::kStyleAttr, AtomicString("height: 0px; overflow: clip visible"));
    ...
    {
        container->setAttribute(html_names::kStyleAttr, AtomicString("height: 0px; overflow: clip clip"));
    ...
    {
        container->setAttribute(html_names::kStyleAttr, AtomicString("height: 0px; overflow: visible clip"));
    ...
    {
        container->setAttribute(html_names::kStyleAttr, AtomicString("height: 1px; overflow: hidden"));
    ...
    {
        container->setAttribute(html_names::kStyleAttr, AtomicString("height: 0px; overflow: visible"));
...
TEST_F(AnnotationAgentImplTest, TextFinderDoesntFindOpacityZero)
{
    ...
    {
        container->setAttribute(html_names::kStyleAttr, AtomicString("opacity: 0.1"));
...
TEST_F(AnnotationAgentImplTest, TextFinderDoesntFindOffscreenFixed)
{
    ...
    {
        container->setAttribute(html_names::kStyleAttr, AtomicString("left: 0; top: -25px"));
    ...
    {
        container->setAttribute(html_names::kStyleAttr, AtomicString("left: 0; top: -15px"));
