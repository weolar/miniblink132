...
FileBackedBlobFactoryTestHelper::FileBackedBlobFactoryTestHelper(ExecutionContext* context)
    : context_(context)
    , receiver_(&factory_)
{
    CHECK(context);
    FileBackedBlobFactoryDispatcher::From(*context)->SetFileBackedBlobFactoryForTesting(receiver_.BindNewEndpointAndPassDedicatedRemote());
}
