...
class NullExecutionContext : public GarbageCollected<NullExecutionContext>, public ExecutionContext {
public:
    ...
    KURL CompleteURL(const String&) const override
    {
        return url_;
    }
...
public:
    ...
    explicit ScopedNullExecutionContext(std::unique_ptr<FrameScheduler> scheduler)
        : execution_context_(MakeGarbageCollected<NullExecutionContext>(std::move(scheduler)))
    {
    }
