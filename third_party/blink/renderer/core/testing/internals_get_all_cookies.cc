...
// static
ScriptPromise<IDLSequence<InternalCookie>> InternalsGetAllCookies::getAllCookies(ScriptState* script_state, Internals&)
{
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLSequence<InternalCookie>>>(script_state);
    ...
    raw_cookie_manager->GetAllCookies(WTF::BindOnce(
        [](ScriptPromiseResolver<IDLSequence<InternalCookie>>* resolver, ScriptState* script_state, mojo::Remote<test::mojom::blink::CookieManagerAutomation>,
            WTF::Vector<network::mojom::blink::CookieWithAccessResultPtr> cookies) {
            HeapVector<Member<InternalCookie>> cookie_results;
            for (const auto& cookie : cookies) {
                cookie_results.push_back(CookieMojomToInternalCookie(cookie, script_state->GetIsolate()));
            }
