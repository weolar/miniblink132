...
NullExecutionContext::NullExecutionContext(std::unique_ptr<FrameScheduler> scheduler)
    : ExecutionContext(scheduler->GetAgentGroupScheduler()->Isolate(),
        MakeGarbageCollected<Agent>(scheduler->GetAgentGroupScheduler()->Isolate(), base::UnguessableToken::Create(),
            v8::MicrotaskQueue::New(scheduler->GetAgentGroupScheduler()->Isolate(), v8::MicrotasksPolicy::kScoped)))
...
void NullExecutionContext::SetUpSecurityContextForTesting()
{
    ...
    GetSecurityContext().SetSecurityOriginForTesting(SecurityOrigin::Create(url_));
...
scoped_refptr<base::SingleThreadTaskRunner> NullExecutionContext::GetTaskRunner(TaskType task_type)
{
    return scheduler_->GetTaskRunner(task_type);
}

const BrowserInterfaceBrokerProxy& NullExecutionContext::GetBrowserInterfaceBroker() const
{
