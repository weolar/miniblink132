...
void DummyModulator::FetchTree(const KURL&, ModuleType, ResourceFetcher*, mojom::blink::RequestContextType, network::mojom::RequestDestination,
    const ScriptFetchOptions&, ModuleScriptCustomFetchType, ModuleTreeClient*, String referrer)
{
    NOTREACHED();
}

void DummyModulator::FetchSingle(const ModuleScriptFetchRequest&, ResourceFetcher*, ModuleGraphLevel, ModuleScriptCustomFetchType, SingleModuleClient*)
{
    NOTREACHED();
}

void DummyModulator::FetchDescendantsForInlineScript(
    ModuleScript*, ResourceFetcher*, mojom::blink::RequestContextType, network::mojom::RequestDestination, ModuleTreeClient*)
{
    NOTREACHED();
}
...
KURL DummyModulator::ResolveModuleSpecifier(const String&, const KURL&, String*)
{
    NOTREACHED();
}
...
void DummyModulator::ResolveDynamically(const ModuleRequest& module_request, const ReferrerScriptInfo&, ScriptPromiseResolver<IDLAny>*)
{
    NOTREACHED();
}

ModuleImportMeta DummyModulator::HostGetImportMetaProperties(v8::Local<v8::Module>) const
{
    NOTREACHED();
}

ModuleType DummyModulator::ModuleTypeFromRequest(const ModuleRequest& module_request) const
{
...
ModuleScriptFetcher* DummyModulator::CreateModuleScriptFetcher(ModuleScriptCustomFetchType, base::PassKey<ModuleScriptLoader> pass_key)
{
    NOTREACHED();
}
