...
void SimNetwork::DidReceiveResponse(URLLoaderClient* client, const WebURLResponse& response)
{
...
void SimNetwork::DidReceiveData(URLLoaderClient* client, base::span<const char> data)
{
    if (!current_request_)
        client->DidReceiveDataForTesting(data);
}

void SimNetwork::DidFail(
    URLLoaderClient* client, const WebURLError& error, int64_t total_encoded_data_length, int64_t total_encoded_body_length, int64_t total_decoded_body_length)
{
    if (!current_request_) {
...
void SimNetwork::DidFinishLoading(URLLoaderClient* client, base::TimeTicks finish_time, int64_t total_encoded_data_length, int64_t total_encoded_body_length,
    int64_t total_decoded_body_length)
{
    if (!current_request_) {
        client->DidFinishLoading(finish_time, total_encoded_data_length, total_encoded_body_length, total_decoded_body_length);
        return;
    }
    current_request_ = nullptr;
}
...
void SimNetwork::AddRequest(SimRequestBase& request)
{
    ...
    // TODO(crbug.com/751425): We should use the mock functionality
    // via |SimTest::web_frame_client_| and/or |SimTest::web_view_helper_|.
    url_test_helpers::RegisterMockedURLLoadWithCustomResponse(request.url_, "", response);
}
