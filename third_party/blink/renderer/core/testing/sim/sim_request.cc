...
SimRequestBase::SimRequestBase(KURL url, String mime_type, bool start_immediately, Params params)
    : url_(std::move(url))
...
void SimRequestBase::DidReceiveResponse(URLLoaderClient* client, const WebURLResponse& response)
{
    DCHECK(!navigation_body_loader_);
    client_ = client;
    response_ = response;
...
void SimRequestBase::UsedForNavigation(StaticDataNavigationBodyLoader* navigation_body_loader)
{
...
{
    StringUTF8Adaptor utf8_str(data);
    WriteInternal(base::span<const char>(utf8_str.data(), utf8_str.size()));
}

void SimRequestBase::Write(const Vector<char>& data)
{
    WriteInternal(base::span<const char>(data.data(), data.size()));
}
...
void SimRequestBase::Finish(bool body_loader_finished)
{
    ...
    if (error_) {
        DCHECK(!navigation_body_loader_);
        client_->DidFail(*error_, base::TimeTicks::Now(), total_encoded_data_length_, total_encoded_data_length_, total_encoded_data_length_);
    } else {
        if (navigation_body_loader_) {
            if (!body_loader_finished)
                navigation_body_loader_->Finish();
        } else {
            client_->DidFinishLoading(base::TimeTicks::Now(), total_encoded_data_length_, total_encoded_data_length_, total_encoded_data_length_);
        }
    }
    Reset();
}
...
SimRequest::SimRequest(KURL url, String mime_type, Params params)
    : SimRequestBase(std::move(url), std::move(mime_type),
        /* start_immediately=*/true, params)
{
}
...
SimSubresourceRequest::SimSubresourceRequest(KURL url, String mime_type, Params params)
    : SimRequestBase(std::move(url), std::move(mime_type),
        /* start_immediately=*/false, params)
{
}

SimSubresourceRequest::SimSubresourceRequest(String url, String mime_type, Params params)
