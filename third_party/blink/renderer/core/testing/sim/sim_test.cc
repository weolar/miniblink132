...
SimTest::SimTest(std::optional<base::test::TaskEnvironment::TimeSource> time_source)
    : task_environment_(time_source.has_value() ? time_source.value() : base::test::TaskEnvironment::TimeSource::DEFAULT)
{
    ...
    bool was_threaded_animation_enabled = content::TestBlinkWebUnitTestSupport::SetThreadedAnimationEnabled(false);
...
void SimTest::SetUp()
{
    ...
    web_view_helper_ = std::make_unique<frame_test_helpers::WebViewHelper>(WTF::BindRepeating(&SimTest::CreateWebFrameWidget, base::Unretained(this)));
    ...
    compositor_->SetLayerTreeHost(local_frame_root_->FrameWidgetImpl()->LayerTreeHostForTesting());
...
void SimTest::InitializeRemote()
{
    ...
    compositor_->SetWebView(WebView());
    page_->SetPage(WebView().GetPage());
    web_frame_client_ = std::make_unique<frame_test_helpers::TestWebFrameClient>();
    local_frame_root_ = web_view_helper_->CreateLocalChild(
        *WebView().MainFrame()->ToWebRemoteFrame(), "local_frame_root", WebFrameOwnerProperties(), nullptr, web_frame_client_.get());
    compositor_->SetLayerTreeHost(local_frame_root_->FrameWidgetImpl()->LayerTreeHostForTesting());
}

void SimTest::InitializeFencedFrameRoot(blink::FencedFrame::DeprecatedFencedFrameMode mode)
{
...
void SimTest::LoadURL(const String& url_string)
{
    ...
    if (DocumentLoader::WillLoadUrlAsEmpty(url) || url.ProtocolIsData()) {
        ...
        frame_test_helpers::PumpPendingRequestsForFrameToLoad(local_frame_root_.Get());
    }
}
...
{
    return *static_cast<frame_test_helpers::TestWebFrameWidget*>(local_frame_root_->FrameWidgetImpl());
}
...
frame_test_helpers::TestWebFrameWidget* SimTest::CreateWebFrameWidget(base::PassKey<WebLocalFrame> pass_key,
...
{
    return MakeGarbageCollected<frame_test_helpers::TestWebFrameWidget>(std::move(pass_key), std::move(frame_widget_host), std::move(frame_widget),
        std::move(widget_host), std::move(widget), std::move(task_runner), frame_sink_id, hidden, never_composited, is_for_child_local_root,
        is_for_nested_main_frame, is_for_scalable_page);
}

std::unique_ptr<frame_test_helpers::TestWebFrameClient> SimTest::CreateWebFrameClientForMainFrame()
{
