...
size_t SimCanvas::Commands::DrawCount(CommandType type, const String& color_string) const
{
    Color color;
    ...
    for (auto& command : commands_) {
        if (command.type == type && (color_string.IsNull() || command.color == color.Rgb())) {
            count++;
        }
    }
    return count;
...
void SimCanvas::onDrawImage2(const SkImage* image, SkScalar left, SkScalar top, const SkSamplingOptions& sampling, const SkPaint* paint)
{
    DrawScope scope;
...
void SimCanvas::onDrawImageRect2(
