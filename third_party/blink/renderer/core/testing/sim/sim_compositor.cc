...
SimCanvas::Commands SimCompositor::BeginFrame(double time_delta_in_seconds, bool raster)
{
    DCHECK(web_view_);
    ...
    base::TimeTicks start = last_frame_time_ + base::Seconds(time_delta_in_seconds);
    ...
    LayerTreeHost()->CompositeForTest(last_frame_time_, raster, base::OnceClosure());

    const auto* main_frame = web_view_->MainFrameImpl();
    if (!main_frame || main_frame->GetFrame()->GetDocument()->Lifecycle().GetState() < DocumentLifecycle::kPaintClean) {
