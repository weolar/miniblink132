...
LocalFrame* SingleChildLocalFrameClient::CreateFrame(const AtomicString& name, HTMLFrameOwnerElement* owner_element)
{

    LocalFrame* parent_frame = owner_element->GetDocument().GetFrame();
    auto* child_client = MakeGarbageCollected<LocalFrameClientWithParent>(parent_frame);
    LocalFrame* child = MakeGarbageCollected<LocalFrame>(child_client, *parent_frame->GetPage(), owner_element, parent_frame, nullptr,
        FrameInsertType::kInsertInConstructor, LocalFrameToken(), &parent_frame->window_agent_factory(), nullptr, mojo::NullRemote());
    ...
    mojom::blink::PolicyContainerPoliciesPtr policy_container_data
        = parent_frame->GetDocument()->GetExecutionContext()->GetPolicyContainer()->GetPolicies().Clone();
    ...
    policy_container_data->sandbox_flags |= child->Owner()->GetFramePolicy().sandbox_flags;
    ...
    auto policy_container = std::make_unique<PolicyContainer>(dummy_host.Unbind(), std::move(policy_container_data));

    child->Init(/*opener=*/nullptr, DocumentToken(), std::move(policy_container), parent_frame->DomWindow()->GetStorageKey(),
...
void RenderingTestChromeClient::InjectScrollbarGestureScroll(LocalFrame& local_frame, const gfx::Vector2dF& delta, ui::ScrollGranularity granularity,
    CompositorElementId scrollable_area_element_id, WebInputEvent::Type injected_type)
{
    ...
    std::unique_ptr<WebGestureEvent> gesture_event
        = WebGestureEvent::GenerateInjectedScrollbarGestureScroll(injected_type, base::TimeTicks::Now(), gfx::PointF(0, 0), delta, granularity);
...
RenderingTest::RenderingTest(base::test::TaskEnvironment::TimeSource time_source)
...
{
    DEFINE_STATIC_LOCAL(Persistent<RenderingTestChromeClient>, client, (MakeGarbageCollected<RenderingTestChromeClient>()));
    return *client;
}
...
HitTestResult::NodeSet RenderingTest::RectBasedHitTest(const PhysicalRect& rect)
{
    HitTestLocation location(rect);
    HitTestResult result(
        HitTestRequest(HitTestRequest::kReadOnly | HitTestRequest::kActive | HitTestRequest::kAllowChildFrameContent | HitTestRequest::kListBased), location);
    GetLayoutView().HitTest(location, result);
...
void RenderingTest::SetUp()
{
    ...
    SetupPageWithClients(&GetChromeClient(), local_frame_client_, SettingOverrider());
    EXPECT_TRUE(GetDocument().GetPage()->GetScrollbarTheme().UsesOverlayScrollbars());
...
ConstraintSpace RenderingTest::ConstraintSpaceForAvailableSize(LayoutUnit inline_size) const
{
...
PhysicalRect VisualRectInDocument(const LayoutObject& object, VisualRectFlags flags)
{
    ...
    if (object.IsSVG() || IsA<LayoutSVGInline>(object)) {
        return SVGLayoutSupport::VisualRectInAncestorSpace(object, *object.View(), flags);
    }
...
{
    if (object.StyleRef().Visibility() != EVisibility::kVisible && object.VisualRectRespectsVisibility()) {
        return PhysicalRect();
    }

    if (const auto* text = DynamicTo<LayoutText>(object)) {
        return UnionRect(text->VisualOverflowRect(), text->LocalSelectionVisualRect());
