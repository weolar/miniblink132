...
public:
    static void Bind(mojo::ScopedMessagePipeHandle handle)
    {
        ...
        impl.receivers_.Add(&impl, mojo::PendingReceiver<mojom::blink::PluginRegistry>(std::move(handle)));
    }
...
ScopedFakePluginRegistry::ScopedFakePluginRegistry()
{
    Platform::Current()->GetBrowserInterfaceBroker()->SetBinderForTesting(
        mojom::blink::PluginRegistry::Name_, WTF::BindRepeating(&FakePluginRegistryImpl::Bind));
}

ScopedFakePluginRegistry::~ScopedFakePluginRegistry()
{
    Platform::Current()->GetBrowserInterfaceBroker()->SetBinderForTesting(mojom::blink::PluginRegistry::Name_, {});
}
