...
public:
    ...
    Vector<Vector<String>> identityByteStringSequenceSequence(const Vector<Vector<String>>& arg) const;
    Vector<double> identityDoubleSequence(const Vector<double>& arg) const;
    Vector<V8FoodEnum> identityFoodEnumSequence(const Vector<V8FoodEnum>& arg) const;
    Vector<int32_t> identityLongSequence(const Vector<int32_t>& arg) const;
    std::optional<Vector<uint8_t>> identityOctetSequenceOrNull(const std::optional<Vector<uint8_t>>& arg) const;

    HeapVector<Member<Element>> getElementSequence() const;
    void setElementSequence(const HeapVector<Member<Element>>& arg);
    void setElementSequenceOfSequences(const HeapVector<HeapVector<Member<Element>>>& arg);
