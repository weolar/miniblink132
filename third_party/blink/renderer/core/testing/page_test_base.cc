...
Element* GetOrCreateElement(ContainerNode* parent, const HTMLQualifiedName& tag_name)
{
    HTMLCollection* elements = parent->getElementsByTagNameNS(tag_name.NamespaceURI(), tag_name.LocalName());
    if (!elements->IsEmpty())
        return elements->item(0);
    return parent->ownerDocument()->CreateRawElement(tag_name, CreateElementFlags::ByCreateElement());
}

void ToSimpleLayoutTree(std::ostream& ostream, const LayoutObject& layout_object, int depth)
{
    ...
    ostream << std::endl;
    for (auto* child = layout_object.SlowFirstChild(); child; child = child->NextSibling()) {
        ostream << "  ";
...
PageTestBase::MockClipboardHostProvider::MockClipboardHostProvider(const blink::BrowserInterfaceBrokerProxy& interface_broker)
{
...
PageTestBase::MockClipboardHostProvider::~MockClipboardHostProvider()
{
    if (interface_broker_) {
        interface_broker_->SetBinderForTesting(blink::mojom::blink::ClipboardHost::Name_, {});
    }
}

void PageTestBase::MockClipboardHostProvider::Install(const blink::BrowserInterfaceBrokerProxy& interface_broker)
{
    interface_broker_ = &interface_broker;
    interface_broker_->SetBinderForTesting(
        blink::mojom::blink::ClipboardHost::Name_, WTF::BindRepeating(&PageTestBase::MockClipboardHostProvider::BindClipboardHost, base::Unretained(this)));
}

void PageTestBase::MockClipboardHostProvider::BindClipboardHost(mojo::ScopedMessagePipeHandle handle)
{
    host_.Bind(mojo::PendingReceiver<blink::mojom::blink::ClipboardHost>(std::move(handle)));
}
...
{
    DCHECK(!dummy_page_holder_) << "EnableCompositing() must be called before set up";
...
void PageTestBase::SetUp()
{
    ...
    dummy_page_holder_ = std::make_unique<DummyPageHolder>(gfx::Size(800, 600), nullptr, nullptr, std::move(setter), GetTickClock());
...
void PageTestBase::SetUp(gfx::Size size)
{
    ...
    dummy_page_holder_ = std::make_unique<DummyPageHolder>(size, nullptr, nullptr, std::move(setter), GetTickClock());
...
void PageTestBase::SetupPageWithClients(
    ChromeClient* chrome_client, LocalFrameClient* local_frame_client, FrameSettingOverrideFunction setting_overrider, gfx::Size size)
{
    DCHECK(!dummy_page_holder_) << "Page should be set up only once";
    auto setter = base::BindLambdaForTesting([&](Settings& settings) {
    ...
    dummy_page_holder_ = std::make_unique<DummyPageHolder>(size, chrome_client, local_frame_client, std::move(setter), GetTickClock());
...
{
    LoadFontFromFile(frame, test::CoreTestDataPath("Ahem.ttf"), AtomicString("Ahem"));
}

void PageTestBase::LoadFontFromFile(LocalFrame& frame, String font_path, const AtomicString& family_name)
{
    ...
    FontFace* ahem = FontFace::Create(frame.DomWindow(), family_name, buffer, FontFaceDescriptors::Create());

    ScriptState* script_state = ToScriptStateForMainWorld(&frame);
    DummyExceptionStateForTesting exception_state;
    FontFaceSetDocument::From(document)->addForBinding(script_state, ahem, exception_state);
}
...
{
    LoadFontFromFile(frame, blink::test::PlatformTestDataPath("third_party/Noto/NotoNaskhArabic-regular.woff2"), AtomicString("NotoArabic"));
}
...
{
    Element* const head = GetOrCreateElement(&GetDocument(), html_names::kHeadTag);
    DCHECK_EQ(head, GetOrCreateElement(&GetDocument(), html_names::kHeadTag));
    Element* const style = GetDocument().CreateRawElement(html_names::kStyleTag, CreateElementFlags::ByCreateElement());
    style->setTextContent(String(style_rules));
    head->appendChild(style);
}

void PageTestBase::NavigateTo(const KURL& url, const WTF::HashMap<String, String>& headers)
{
    ...
    for (auto& csp : ParseContentSecurityPolicies(params->response.HttpHeaderField("content-security-policy"),
             network::mojom::blink::ContentSecurityPolicyType::kEnforce, network::mojom::blink::ContentSecurityPolicySource::kHTTP, url)) {
        params->policy_container->policies.content_security_policies.emplace_back(ConvertToPublic(std::move(csp)));
    }

    GetFrame().Loader().CommitNavigation(std::move(params), nullptr /* extra_data */);
...
// static
std::string PageTestBase::ToSimpleLayoutTree(const LayoutObject& layout_object)
{
