...
InternalCookie* CookieMojomToInternalCookie(const network::mojom::blink::CookieWithAccessResultPtr& cookie, v8::Isolate* isolate)
{
    ...
    if (!cookie->cookie.ExpiryDate().is_null()) {
        ...
        result->setExpiry((cookie->cookie.ExpiryDate() - base::Time::UnixEpoch()).InSeconds());
    }
