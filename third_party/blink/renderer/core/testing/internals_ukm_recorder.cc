...
HeapVector<ScriptValue> InternalsUkmRecorder::getMetrics(ScriptState* script_state, const String& entry_name, const Vector<String>& metric_names)
{
    std::vector<std::string> names(metric_names.size());
    std::transform(metric_names.begin(), metric_names.end(), names.begin(), [](String name) { return std::string(name.Utf8()); });

    std::vector<ukm::TestUkmRecorder::HumanReadableUkmEntry> entries = recorder_.GetEntries(entry_name.Utf8(), names);
