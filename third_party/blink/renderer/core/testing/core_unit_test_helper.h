...
public:
    ...
    LocalFrame* CreateFrame(const AtomicString& name, HTMLFrameOwnerElement*) override;
};

class LocalFrameClientWithParent final : public EmptyLocalFrameClient {
public:
    explicit LocalFrameClientWithParent(LocalFrame* parent)
        : parent_(parent)
    {
    }
...
protected:
    ...
    const DisplayItemClient* GetDisplayItemClientFromLayoutObject(LayoutObject* obj) const
    {
        return obj;
    }

    const DisplayItemClient* GetDisplayItemClientFromElementId(const char* id) const
    {
...
constexpr LogicalRect::LogicalRect(int inline_offset, int block_offset, int inline_size, int block_size)
...
PhysicalRect VisualRectInDocument(const LayoutObject& object, VisualRectFlags = kDefaultVisualRectFlags);
