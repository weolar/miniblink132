...
public:
    ...
    mojo::PendingAssociatedRemote<mojom::blink::RemoteFrameHost> BindNewAssociatedRemote();
    ...
    void AdvanceFocus(blink::mojom::FocusType focus_type, const LocalFrameToken& source_frame_token) override;
    void RouteMessageEvent(const std::optional<LocalFrameToken>& source_frame_token, const scoped_refptr<const SecurityOrigin>& source_origin,
        const String& target_origin, BlinkTransferableMessage message) override;
    void PrintCrossProcessSubframe(const gfx::Rect& rect, int document_cookie) override;
    void Detach() override;
    void UpdateViewportIntersection(
        blink::mojom::blink::ViewportIntersectionStatePtr intersection_state, const std::optional<FrameVisualProperties>& visual_properties) override;
