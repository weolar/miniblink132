...
{
    MojoCreateMessagePipeResult* result_dict = MojoCreateMessagePipeResult::Create();
    ...
    if (result == MOJO_RESULT_OK) {
        result_dict->setHandle0(MakeGarbageCollected<MojoHandle>(mojo::ScopedHandle::From(std::move(handle0))));
        result_dict->setHandle1(MakeGarbageCollected<MojoHandle>(mojo::ScopedHandle::From(std::move(handle1))));
    }
    return result_dict;
}

// static
MojoCreateDataPipeResult* Mojo::createDataPipe(const MojoCreateDataPipeOptions* options_dict)
{
    ...
    if (!options_dict->hasElementNumBytes() || !options_dict->hasCapacityNumBytes() || options_dict->capacityNumBytes() < 1
    ...
    result_dict->setResult(result);
    if (result == MOJO_RESULT_OK) {
        result_dict->setProducer(MakeGarbageCollected<MojoHandle>(mojo::ScopedHandle::From(std::move(producer))));
        result_dict->setConsumer(MakeGarbageCollected<MojoHandle>(mojo::ScopedHandle::From(std::move(consumer))));
    }
    return result_dict;
}
...
MojoCreateSharedBufferResult* Mojo::createSharedBuffer(unsigned num_bytes)
{
    ...
    mojo::Handle handle;
    MojoResult result = MojoCreateSharedBuffer(num_bytes, options, handle.mutable_value());
...
// static
void Mojo::bindInterface(
    ScriptState* script_state, const String& interface_name, MojoHandle* request_handle, const V8MojoScope& scope, ExceptionState& exception_state)
{
    std::string name = interface_name.Utf8();
    auto handle = mojo::ScopedMessagePipeHandle::From(request_handle->TakeHandle());
    ...
    if (scope == V8MojoScope::Enum::kProcess) {
        Platform::Current()->GetBrowserInterfaceBroker()->GetInterface(mojo::GenericPendingReceiver(name, std::move(handle)));
        return;
    }
