...
public:
    ...
    void close();
    MojoWatcher* watch(ScriptState*, const MojoHandleSignals*, V8MojoWatchCallback*);

    // MessagePipe handle.
    MojoResult writeMessage(const V8BufferSource* buffer, const HeapVector<Member<MojoHandle>>& handles);
    MojoReadMessageResult* readMessage(const MojoReadMessageFlags*);

    // DataPipe handle.
    MojoWriteDataResult* writeData(const V8BufferSource* buffer, const MojoWriteDataOptions* options);
    MojoReadDataResult* queryData() const;
    MojoReadDataResult* discardData(unsigned num_bytes, const MojoDiscardDataOptions*);
    MojoReadDataResult* readData(const V8BufferSource* buffer, const MojoReadDataOptions* options) const;

    // SharedBuffer handle.
    MojoMapBufferResult* mapBuffer(unsigned offset, unsigned num_bytes);
    MojoCreateSharedBufferResult* duplicateBufferHandle(const MojoDuplicateBufferHandleOptions*);
