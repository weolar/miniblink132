...
// static
MojoWatcher* MojoWatcher::Create(mojo::Handle handle, const MojoHandleSignals* signals_dict, V8MojoWatchCallback* callback, ExecutionContext* context)
{
    ...
    if (result != MOJO_RESULT_OK) {
        watcher->task_runner_->PostTask(
            FROM_HERE, WTF::BindOnce(&V8MojoWatchCallback::InvokeAndReportException, WrapPersistent(callback), WrapPersistent(watcher), result));
    }
    return watcher;
}
...
MojoWatcher::MojoWatcher(ExecutionContext* context, V8MojoWatchCallback* callback)
...
MojoResult MojoWatcher::Watch(mojo::Handle handle, const MojoHandleSignals* signals_dict)
{
    ...
    MojoResult result = mojo::CreateTrap(&MojoWatcher::OnHandleReady, &trap_handle_);
    DCHECK_EQ(MOJO_RESULT_OK, result);

    result = MojoAddTrigger(
        trap_handle_.get().value(), handle.value(), signals, MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED, reinterpret_cast<uintptr_t>(this), nullptr);
    if (result != MOJO_RESULT_OK)
        return result;
    ...
    if (result == MOJO_RESULT_FAILED_PRECONDITION) {
        ...
        task_runner_->PostTask(FROM_HERE, WTF::BindOnce(&MojoWatcher::RunReadyCallback, WrapPersistent(this), ready_result));
        return MOJO_RESULT_OK;
    }
...
MojoResult MojoWatcher::Arm(MojoResult* ready_result)
{
    ...
    MojoResult result = MojoArmTrap(trap_handle_.get().value(), nullptr, &num_blocking_events, &blocking_event);
    if (result == MOJO_RESULT_OK)
        return MOJO_RESULT_OK;

    if (result == MOJO_RESULT_FAILED_PRECONDITION) {
        DCHECK_EQ(1u, num_blocking_events);
        DCHECK_EQ(reinterpret_cast<uintptr_t>(this), blocking_event.trigger_context);
...
void MojoWatcher::OnHandleReady(const MojoTrapEvent* event)
{
    ...
    PostCrossThreadTask(
        *watcher->task_runner_, FROM_HERE, CrossThreadBindOnce(&MojoWatcher::RunReadyCallback, WrapCrossThreadWeakPersistent(watcher), event->result));
}
...
void MojoWatcher::RunReadyCallback(MojoResult result)
{
    ...
    if (arm_result == MOJO_RESULT_FAILED_PRECONDITION) {
        task_runner_->PostTask(FROM_HERE, WTF::BindOnce(&MojoWatcher::RunReadyCallback, WrapWeakPersistent(this), ready_result));
        return;
    }
}
