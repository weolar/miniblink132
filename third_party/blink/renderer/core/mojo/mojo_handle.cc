...
MojoWatcher* MojoHandle::watch(ScriptState* script_state, const MojoHandleSignals* signals, V8MojoWatchCallback* callback)
{
    return MojoWatcher::Create(handle_.get(), signals, callback, ExecutionContext::From(script_state));
}

MojoResult MojoHandle::writeMessage(const V8BufferSource* buffer, const HeapVector<Member<MojoHandle>>& handles)
{
    ...
    return mojo::WriteMessageNew(mojo::MessagePipeHandle(handle_.get().value()), message.TakeMojoMessage(), MOJO_WRITE_MESSAGE_FLAG_NONE);
}

MojoReadMessageResult* MojoHandle::readMessage(const MojoReadMessageFlags* flags_dict)
{
    MojoReadMessageResult* result_dict = MojoReadMessageResult::Create();

    mojo::ScopedMessageHandle message;
    MojoResult result = mojo::ReadMessageNew(mojo::MessagePipeHandle(handle_.get().value()), &message, MOJO_READ_MESSAGE_FLAG_NONE);
    ...
    result = MojoGetMessageData(message->value(), nullptr, &bytes, &num_bytes, nullptr, &num_handles);
    if (result == MOJO_RESULT_RESOURCE_EXHAUSTED) {
        raw_handles.resize(num_handles);
        result = MojoGetMessageData(message->value(), nullptr, &bytes, &num_bytes, raw_handles.data(), &num_handles);
    }
    ...
    DOMArrayBuffer* buffer = DOMArrayBuffer::CreateUninitializedOrNull(num_bytes, 1);
    if (num_bytes) {
        CHECK(buffer);
    ...
    for (uint32_t i = 0; i < num_handles; ++i) {
        handles[i] = MakeGarbageCollected<MojoHandle>(mojo::MakeScopedHandle(mojo::Handle(raw_handles[i])));
    }
...
MojoWriteDataResult* MojoHandle::writeData(const V8BufferSource* buffer, const MojoWriteDataOptions* options_dict)
{
    ...
    MojoResult result = base::CheckedNumeric<uint32_t>(bytes.size()).AssignIfValid(&num_bytes)
        ? MojoWriteData(handle_.get().value(), bytes.data(), &num_bytes, &options)
...
MojoReadDataResult* MojoHandle::queryData() const
{
    ...
    MojoResult result = MojoReadData(handle_.get().value(), &options, nullptr, &num_bytes);
    result_dict->setResult(result);
    result_dict->setNumBytes(num_bytes);
    return result_dict;
}

MojoReadDataResult* MojoHandle::discardData(unsigned num_bytes, const MojoDiscardDataOptions* options_dict)
{
    MojoReadDataResult* result_dict = MojoReadDataResult::Create();
...
MojoReadDataResult* MojoHandle::readData(const V8BufferSource* buffer, const MojoReadDataOptions* options_dict) const
{
    ...
    uint32_t num_bytes;
    MojoResult result = base::CheckedNumeric<uint32_t>(bytes.size()).AssignIfValid(&num_bytes)
        ? MojoReadData(handle_.get().value(), &options, bytes.data(), &num_bytes)
        : MOJO_RESULT_INVALID_ARGUMENT;
...
MojoMapBufferResult* MojoHandle::mapBuffer(unsigned offset, unsigned num_bytes)
{
    ...
    auto region = mojo::UnwrapPlatformSharedMemoryRegion(mojo::ScopedSharedBufferHandle(buffer_handle));
    ...
    // mojo handle again.
    mojo::ScopedSharedBufferHandle mojo_buffer = mojo::WrapPlatformSharedMemoryRegion(std::move(region));
    handle_.reset(mojo::Handle(mojo_buffer.release().value()));

    return result_dict;
}

MojoCreateSharedBufferResult* MojoHandle::duplicateBufferHandle(const MojoDuplicateBufferHandleOptions* options_dict)
{
    MojoCreateSharedBufferResult* result_dict = MojoCreateSharedBufferResult::Create();

    ::MojoDuplicateBufferHandleOptions options = { sizeof(options), MOJO_DUPLICATE_BUFFER_HANDLE_FLAG_NONE };
    if (options_dict->readOnly())
        options.flags |= MOJO_DUPLICATE_BUFFER_HANDLE_FLAG_READ_ONLY;

    mojo::Handle handle;
    MojoResult result = MojoDuplicateBufferHandle(handle_.get().value(), &options, handle.mutable_value());
    result_dict->setResult(result);
    if (result == MOJO_RESULT_OK) {
        result_dict->setHandle(MakeGarbageCollected<MojoHandle>(mojo::MakeScopedHandle(handle)));
    }
    return result_dict;
}
