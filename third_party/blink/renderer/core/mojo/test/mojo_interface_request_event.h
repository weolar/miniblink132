...
public:
    ...
    static MojoInterfaceRequestEvent* Create(const AtomicString& type, const MojoInterfaceRequestEventInit* initializer)
    {
        return MakeGarbageCollected<MojoInterfaceRequestEvent>(type, initializer);
    }

    explicit MojoInterfaceRequestEvent(MojoHandle*);
    MojoInterfaceRequestEvent(const AtomicString& type, const MojoInterfaceRequestEventInit*);
