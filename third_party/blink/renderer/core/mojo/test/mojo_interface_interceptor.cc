...
MojoInterfaceInterceptor* MojoInterfaceInterceptor::Create(
    ExecutionContext* context, const String& interface_name, const Scope& scope, ExceptionState& exception_state)
{
    if (scope == Scope::Enum::kProcess && !context->IsWindow()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "\"process\" scope interception is unavailable outside a Document.");
        return nullptr;
    }

    if (scope == Scope::Enum::kContextJs && !context->use_mojo_js_interface_broker()) {
    ...
    return MakeGarbageCollected<MojoInterfaceInterceptor>(context, interface_name, scope.AsEnum());
}
...
void MojoInterfaceInterceptor::start(ExceptionState& exception_state)
{
    ...
    if (scope_ == Scope::Enum::kProcess) {
        ...
        if (!Platform::Current()->GetBrowserInterfaceBroker()->SetBinderForTesting(
                interface_name, WTF::BindRepeating(&MojoInterfaceInterceptor::OnInterfaceRequest, WrapWeakPersistent(this)))) {
            exception_state.ThrowDOMException(
                DOMExceptionCode::kInvalidModificationError, "Interface " + interface_name_ + " is already intercepted by another MojoInterfaceInterceptor.");
        }
...
void MojoInterfaceInterceptor::stop()
{
    ...
    if (scope_ == Scope::Enum::kProcess) {
        Platform::Current()->GetBrowserInterfaceBroker()->SetBinderForTesting(interface_name, {});
        return;
    }
...
MojoInterfaceInterceptor::MojoInterfaceInterceptor(ExecutionContext* context, const String& interface_name, Scope::Enum scope)
...
void MojoInterfaceInterceptor::OnInterfaceRequest(mojo::ScopedMessagePipeHandle handle)
{
    ...
    GetExecutionContext()
        ...
        ->PostTask(FROM_HERE, WTF::BindOnce(&MojoInterfaceInterceptor::DispatchInterfaceRequestEvent, WrapPersistent(this), std::move(handle)));
}

void MojoInterfaceInterceptor::DispatchInterfaceRequestEvent(mojo::ScopedMessagePipeHandle handle)
{
    DispatchEvent(*MakeGarbageCollected<MojoInterfaceRequestEvent>(MakeGarbageCollected<MojoHandle>(mojo::ScopedHandle::From(std::move(handle)))));
}
