...
class DOMContentLoadedListener final : public NativeEventListener, public ProcessingInstruction::DetachableEventListener {
public:
    ...
    {
        processing_instruction_ = nullptr;
    }
...
void DocumentXSLT::ApplyXSLTransform(Document& document, ProcessingInstruction* pi)
{
    ...
    if (!processor->TransformToString(&document, result_mime_type, new_source, result_encoding)) {
    ...
    processor->CreateDocumentFromSource(new_source, result_encoding, result_mime_type, &document, owner_frame);
...
bool DocumentXSLT::ProcessingInstructionInsertedIntoDocument(Document& document, ProcessingInstruction* pi)
{
    ...
    document.addEventListener(event_type_names::kDOMContentLoaded, listener, false);
...
bool DocumentXSLT::ProcessingInstructionRemovedFromDocument(Document& document, ProcessingInstruction* pi)
{
    ...
    document.removeEventListener(event_type_names::kDOMContentLoaded, pi->EventListenerForXSLT(), false);
...
bool DocumentXSLT::SheetLoaded(Document& document, ProcessingInstruction* pi)
{
    ...
    if (!document.Parsing() && !pi->IsLoading() && !DocumentXSLT::HasTransformSourceDocument(document)) {
...
void DocumentXSLT::SetHasTransformSource(Document& document)
{
    ...
    Supplement<Document>::ProvideTo(document, MakeGarbageCollected<DocumentXSLT>(document));
}
