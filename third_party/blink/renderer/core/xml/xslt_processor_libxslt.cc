...
void XSLTProcessor::ParseErrorFunc(void* user_data, /*const*/ xmlError* error)
{
    ...
    console->AddMessage(MakeGarbageCollected<ConsoleMessage>(
        mojom::ConsoleMessageSource::kXml, level, error->message, std::make_unique<SourceLocation>(error->file, String(), error->line, 0, nullptr)));
}
...
static xmlDocPtr DocLoaderFunc(const xmlChar* uri, xmlDictPtr, int options, void* ctxt, xsltLoadType type)
{
    ...
    case XSLT_LOAD_DOCUMENT: {
        XMLDocumentParserScope scope(g_global_processor->XslStylesheet()->OwnerDocument());
        ...
        Resource* resource = RawResource::FetchSynchronously(params, g_global_resource_fetcher);
        if (!g_global_processor)
            return nullptr;
        ...
        LocalFrame* frame = g_global_processor->XslStylesheet()->OwnerDocument()->GetFrame();
        if (frame)
        ...
        xmlParserCtxtPtr ctx = xmlCreatePushParserCtxt(nullptr, nullptr, nullptr, 0, reinterpret_cast<const char*>(uri));
        if (ctx && !xmlCtxtUseOptions(ctx, options)) {
            ...
            for (const auto& span : *data) {
                ...
                if (xmlParseChunk(ctx, span.data(), static_cast<int>(span.size()), final_chunk))
                    break;
                offset += span.size();
            }
    ...
    case XSLT_LOAD_STYLESHEET:
        return g_global_processor->XslStylesheet()->LocateStylesheetSubResource(((xsltStylesheetPtr)ctxt)->doc, uri);
    default:
        break;
    }
...
static int WriteToStringBuilder(void* context, const char* buffer, int len)
{
    ...
    WTF::unicode::ConversionResult result = WTF::unicode::ConvertUTF8ToUTF16(&string_current, buffer + len, &buffer_u_char, buffer_u_char_end);
    if (result != WTF::unicode::kConversionOK && result != WTF::unicode::kSourceExhausted) {
        NOTREACHED();
    }

    int utf16_length = static_cast<int>(buffer_u_char - string_buffer.Characters());
    result_output.Append(string_buffer.Characters(), utf16_length);
    return static_cast<int>(string_current - buffer);
}

static bool SaveResultToString(xmlDocPtr result_doc, xsltStylesheetPtr sheet, String& result_string)
{
    ...
    if (result_builder.length() > 0 && result_builder[result_builder.length() - 1] == '\n')
...
static char* AllocateParameterArray(const char* data)
{
    ...
    char* parameter_array = static_cast<char*>(WTF::Partitions::FastMalloc(length, WTF_HEAP_PROFILER_TYPE_NAME(XSLTProcessor)));
...
static const char** XsltParamArrayFromParameterMap(XSLTProcessor::ParameterMap& parameters)
{
    ...
    ++size;
    size *= sizeof(char*);
    const char** parameter_array = static_cast<const char**>(WTF::Partitions::FastMalloc(size.ValueOrDie(), WTF_HEAP_PROFILER_TYPE_NAME(XSLTProcessor)));

    unsigned index = 0;
    for (auto& parameter : parameters) {
        parameter_array[index++] = AllocateParameterArray(parameter.key.Utf8().c_str());
        parameter_array[index++] = AllocateParameterArray(parameter.value.Utf8().c_str());
    }
...
static xsltStylesheetPtr XsltStylesheetPointer(Document* document, Member<XSLStyleSheet>& cached_stylesheet, Node* stylesheet_root_node)
{
    if (!cached_stylesheet && stylesheet_root_node) {
        ...
        cached_stylesheet
            = MakeGarbageCollected<XSLStyleSheet>(stylesheet_root_node->parentNode() ? &stylesheet_root_node->parentNode()->GetDocument() : document,
                stylesheet_root_node, stylesheet_root_node->GetDocument().Url().GetString(), stylesheet_root_node->GetDocument().Url(),
...
static inline xmlDocPtr XmlDocPtrFromNode(Node* source_node, bool& should_delete)
{
    ...
    if (source_is_document && owner_document->GetTransformSource())
        source_doc = (xmlDocPtr)owner_document->GetTransformSource()->PlatformSource();
    if (!source_doc) {
        source_doc
            = (xmlDocPtr)XmlDocPtrForString(owner_document, CreateMarkup(source_node), source_is_document ? owner_document->Url().GetString() : String());
        should_delete = source_doc;
    }
    return source_doc;
}

static inline String ResultMIMEType(xmlDocPtr result_doc, xsltStylesheetPtr sheet)
{
...
bool XSLTProcessor::TransformToString(Node* source_node, String& mime_type, String& result_string, String& result_encoding)
{
    Document* owner_document = &source_node->GetDocument();

    SetXSLTLoadCallBack(DocLoaderFunc, this, owner_document->Fetcher());
    xsltStylesheetPtr sheet = XsltStylesheetPointer(document_.Get(), stylesheet_, stylesheet_root_node_.Get());
    if (!sheet) {
    ...
    if (xmlDocPtr source_doc = XmlDocPtrFromNode(source_node, should_free_source_doc)) {
        ...
        DCHECK(xsltMaxVars == kDoubleXsltMaxVars || xsltMaxVars == kDoubleXsltMaxVars / 2)
        ...
        xsltTransformContextPtr transform_context = xsltNewTransformContext(sheet, source_doc);
        ...
        CHECK_EQ(0, xsltSetSecurityPrefs(security_prefs, XSLT_SECPREF_WRITE_FILE, xsltSecurityForbid));
        CHECK_EQ(0, xsltSetSecurityPrefs(security_prefs, XSLT_SECPREF_CREATE_DIRECTORY, xsltSecurityForbid));
        CHECK_EQ(0, xsltSetSecurityPrefs(security_prefs, XSLT_SECPREF_WRITE_NETWORK, xsltSecurityForbid));
        ...
        xmlDocPtr result_doc = xsltApplyStylesheetUser(sheet, source_doc, nullptr, nullptr, nullptr, transform_context);
