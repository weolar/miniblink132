...
public:
    PendingStartElementNSCallback(const AtomicString& local_name, const AtomicString& prefix, const AtomicString& uri, int namespace_count,
        const xmlChar** namespaces, int attribute_count, int defaulted_count, const xmlChar** attributes, TextPosition text_position)
    ...
    {
        namespaces_ = static_cast<xmlChar**>(xmlMalloc(sizeof(xmlChar*) * namespace_count * 2));
        for (int i = 0; i < namespace_count * 2; ++i)
            namespaces_[i] = xmlStrdup(namespaces[i]);
        attributes_ = static_cast<xmlChar**>(xmlMalloc(sizeof(xmlChar*) * attribute_count * 5));
        for (int i = 0; i < attribute_count; ++i) {
            ...
            int length = static_cast<int>(attributes[i * 5 + 4] - attributes[i * 5 + 3]);
    ...
    {
        parser->StartElementNs(local_name_, prefix_, uri_, namespace_count_, const_cast<const xmlChar**>(namespaces_), attribute_count_, defaulted_count_,
...
public:
    explicit PendingEndElementNSCallback(TextPosition script_start_position, TextPosition text_position)
        : PendingCallback(text_position)
...
public:
    PendingCharactersCallback(const xmlChar* chars, int length, TextPosition text_position)
        : PendingCallback(text_position)
...
public:
    PendingProcessingInstructionCallback(const String& target, const String& data, TextPosition text_position)
        : PendingCallback(text_position)
        , target_(target)
        , data_(data)
    {
...
public:
    explicit PendingCDATABlockCallback(const String& text, TextPosition text_position)
        : PendingCallback(text_position)
        , text_(text)
    {
    }
...
public:
    explicit PendingCommentCallback(const String& text, TextPosition text_position)
        : PendingCallback(text_position)
        , text_(text)
    {
    }
...
public:
    PendingInternalSubsetCallback(const String& name, const String& external_id, const String& system_id, TextPosition text_position)
        : PendingCallback(text_position)
        , name_(name)
...
public:
    PendingErrorCallback(XMLErrors::ErrorType type, const xmlChar* message, TextPosition text_position)
        : PendingCallback(text_position)
        , type_(type)
    ...
    void Call(XMLDocumentParser* parser) override
    {
        parser->HandleError(type_, reinterpret_cast<char*>(message_), GetTextPosition());
    }
...
void XMLDocumentParser::PushCurrentNode(ContainerNode* n)
{
    ...
    if (current_node_stack_.size() > kMaxXMLTreeDepth)
        HandleError(XMLErrors::kErrorTypeFatal, "Excessive node nesting.", GetTextPosition());
}
...
void XMLDocumentParser::HandleError(XMLErrors::ErrorType type, const char* formatted_message, TextPosition position)
{
...
bool XMLDocumentParser::UpdateLeafTextNode()
{
    ...
    leaf_text_node_->ParserAppendData(ToString(buffered_text_.data(), buffered_text_.size()));
...
bool XMLDocumentParser::ParseDocumentFragment(
    const String& chunk, DocumentFragment* fragment, Element* context_element, ParserContentPolicy parser_content_policy, ExceptionState& exception_state)
{
    if (!chunk.length())
        return true;
    ...
    TryRethrowScope rethrow_scope(fragment->GetDocument().GetAgent().isolate(), exception_state);
    auto* parser = MakeGarbageCollected<XMLDocumentParser>(fragment, context_element, parser_content_policy);
...
static inline void SetAttributes(Element* element, Vector<Attribute, kAttributePrealloc>& attribute_vector, ParserContentPolicy parser_content_policy)
{
...
static void SwitchEncoding(xmlParserCtxtPtr ctxt, bool is_8bit)
{
    ...
    const unsigned char bom_high_byte = *reinterpret_cast<const unsigned char*>(&kBOM);
    xmlSwitchEncoding(ctxt, bom_high_byte == 0xFF ? XML_CHAR_ENCODING_UTF16LE : XML_CHAR_ENCODING_UTF16BE);
}
...
static bool IsLibxmlDefaultCatalogFile(const String& url_string)
{
    ...
    if (url_string.StartsWithIgnoringASCIICase("file:///") && url_string.EndsWithIgnoringASCIICase("/etc/catalog"))
        return true;
    return false;
}
...
static bool ShouldAllowExternalLoad(const KURL& url)
{
    ...
    auto* current_context = XMLDocumentParserScope::current_document_->GetExecutionContext();
    if (!current_context->GetSecurityOrigin()->CanRequest(url)) {
        ...
        // canRequest().
        if (!url.IsNull()) {
            String message = "Unsafe attempt to load URL " + url.ElidedString() + " from frame with URL " + current_context->Url().ElidedString()
                + ". Domains, protocols and ports must match.\n";
            current_context->AddConsoleMessage(
                MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kSecurity, mojom::blink::ConsoleMessageLevel::kError, message));
        }
        return false;
    }
...
static void* OpenFunc(const char* uri)
{
...

    {
        ...
        ResourceLoaderOptions options(document->GetExecutionContext()->GetCurrentWorld());
        options.initiator_info.name = fetch_initiator_type_names::kXml;
        FetchParameters params(ResourceRequest(url), options);
        params.MutableResourceRequest().SetMode(network::mojom::RequestMode::kSameOrigin);
        Resource* resource = RawResource::FetchSynchronously(params, document->Fetcher());

        if (!AllowedByNosniff::MimeTypeAsXMLExternalEntity(document->GetExecutionContext(), resource->GetResponse())) {
            return &g_global_descriptor;
        }
    ...
    UseCounter::Count(XMLDocumentParserScope::current_document_, WebFeature::kXMLExternalResourceLoad);
...
scoped_refptr<XMLParserContext> XMLParserContext::CreateStringParser(xmlSAXHandlerPtr handlers, void* user_data)
{
    InitializeLibXMLIfNecessary();
    xmlParserCtxtPtr parser = xmlCreatePushParserCtxt(handlers, nullptr, nullptr, 0, nullptr);
...
scoped_refptr<XMLParserContext> XMLParserContext::CreateMemoryParser(xmlSAXHandlerPtr handlers, void* user_data, const std::string& chunk)
{
    InitializeLibXMLIfNecessary();

    // appendFragmentSource() checks that the length doesn't overflow an int.
    xmlParserCtxtPtr parser = xmlCreateMemoryParserCtxt(chunk.c_str(), base::checked_cast<int>(chunk.length()));
...
XMLDocumentParser::XMLDocumentParser(Document& document, LocalFrameView* frame_view)
    ...
    , document_(&document)
    , script_runner_(frame_view ? MakeGarbageCollected<XMLParserScriptRunner>(this) : nullptr)
    , // Don't execute scripts for
...
XMLDocumentParser::XMLDocumentParser(DocumentFragment* fragment, Element* parent_element, ParserContentPolicy parser_content_policy)
    ...
    , script_runner_(nullptr)
    , // Don't execute scripts for document fragments.
    script_start_position_(TextPosition::BelowRangePosition())
...
void XMLDocumentParser::DoWrite(const String& parse_string)
{
    ...
    if (parse_string.length()) {
        ...
        base::AutoReset<bool> encoding_scope(&is_currently_parsing8_bit_chunk_, parse_string.Is8Bit());
...
static inline bool HandleNamespaceAttributes(
    Vector<Attribute, kAttributePrealloc>& prefixed_attributes, const xmlChar** libxml_namespaces, int nb_namespaces, ExceptionState& exception_state)
{
    xmlSAX2Namespace* namespaces = reinterpret_cast<xmlSAX2Namespace*>(libxml_namespaces);
    for (int i = 0; i < nb_namespaces; ++i) {
        AtomicString namespace_q_name = g_xmlns_atom;
        AtomicString namespace_uri = ToAtomicString(namespaces[i].uri);
        if (namespaces[i].prefix)
            namespace_q_name = WTF::g_xmlns_with_colon + ToAtomicString(namespaces[i].prefix);

        std::optional<QualifiedName> parsed_name = Element::ParseAttributeName(xmlns_names::kNamespaceURI, namespace_q_name, exception_state);
        if (!parsed_name) {
...
static inline bool HandleElementAttributes(Vector<Attribute, kAttributePrealloc>& prefixed_attributes, const xmlChar** libxml_attributes, int nb_attributes,
    const HashMap<AtomicString, AtomicString>& initial_prefix_to_namespace_map, ExceptionState& exception_state)
{
    xmlSAX2Attributes* attributes = reinterpret_cast<xmlSAX2Attributes*>(libxml_attributes);
    for (int i = 0; i < nb_attributes; ++i) {
        ...
        if (!attr_prefix.empty()) {
            ...
            if (attributes[i].uri) {
                ...
                const HashMap<AtomicString, AtomicString>::const_iterator it = initial_prefix_to_namespace_map.find(attr_prefix);
                if (it != initial_prefix_to_namespace_map.end()) {
                    attr_uri = it->value;
                } else {
                    exception_state.ThrowDOMException(
                        DOMExceptionCode::kNamespaceError, "Namespace prefix " + attr_prefix + " for attribute " + attr_value + " is not declared.");
                    return false;
                }
            }
        }
        AtomicString attr_q_name = attr_prefix.empty() ? ToAtomicString(attributes[i].localname) : attr_prefix + ":" + ToString(attributes[i].localname);

        std::optional<QualifiedName> parsed_name = Element::ParseAttributeName(attr_uri, attr_q_name, exception_state);
        if (!parsed_name) {
            return false;
        }
...
void XMLDocumentParser::StartElementNs(const AtomicString& local_name, const AtomicString& prefix, const AtomicString& uri, int nb_namespaces,
    const xmlChar** libxml_namespaces, int nb_attributes, int nb_defaulted, const xmlChar** libxml_attributes)
{
    if (IsStopped())
        return;
    ...
    if (parser_paused_) {
        ...
        pending_callbacks_.push_back(std::make_unique<PendingStartElementNSCallback>(
            local_name, prefix, uri, nb_namespaces, libxml_namespaces, nb_attributes, nb_defaulted, libxml_attributes, script_start_position_));
        return;
    }
    ...
    if (!HandleNamespaceAttributes(prefixed_attributes, libxml_namespaces, nb_namespaces, IGNORE_EXCEPTION)) {
        StopParsing();
        return;
    }
    ...
    if (!HandleElementAttributes(prefixed_attributes, libxml_attributes, nb_attributes, prefix_to_namespace_map_,
            parsing_fragment_ ? PassThroughException(isolate) : IGNORE_EXCEPTION)) {
        StopParsing();
        if (parsing_fragment_) {
    ...
    std::optional<ThrowOnDynamicMarkupInsertionCountIncrementer> throw_on_dynamic_markup_insertions;
    if (!parsing_fragment_) {
        if (HTMLConstructionSite::LookUpCustomElementDefinition(*document_, q_name, is)) {
    ...
    Element* new_element = current_node_->GetDocument().CreateElement(
        q_name, parsing_fragment_ ? CreateElementFlags::ByFragmentParser(document_) : CreateElementFlags::ByParser(document_), is);
...
void XMLDocumentParser::EndElementNs()
{
    ...
    if (parser_paused_) {
        pending_callbacks_.push_back(std::make_unique<PendingEndElementNSCallback>(script_start_position_, GetTextPosition()));
        return;
    }
    ...
    if (element->IsScriptElement() && !ScriptingContentIsAllowed(GetParserContentPolicy())) {
        PopCurrentNode();
    ...
    if (element->IsScriptElement()) {
        ...
        script_runner_->ProcessScriptElement(*GetDocument(), element, script_start_position_);
        requesting_script_ = false;
    }
...
void XMLDocumentParser::Characters(const xmlChar* chars, int length)
{
    ...
    if (parser_paused_) {
        pending_callbacks_.push_back(std::make_unique<PendingCharactersCallback>(chars, length, GetTextPosition()));
        return;
    }
...
void XMLDocumentParser::GetError(XMLErrors::ErrorType type, const char* message, va_list args)
{
    if (IsStopped())
        return;
    ...
    if (parser_paused_) {
        pending_callbacks_.push_back(std::make_unique<PendingErrorCallback>(type, reinterpret_cast<const xmlChar*>(formatted_message), GetTextPosition()));
        return;
    }
...
void XMLDocumentParser::GetProcessingInstruction(const String& target, const String& data)
{
    if (IsStopped())
        return;

    if (parser_paused_) {
        pending_callbacks_.push_back(std::make_unique<PendingProcessingInstructionCallback>(target, data, GetTextPosition()));
        return;
    }
    ...
    ProcessingInstruction* pi = current_node_->GetDocument().createProcessingInstruction(target, data, exception_state);
    ...
    if (saw_xsl_transform_ && !DocumentXSLT::HasTransformSourceDocument(*GetDocument())) {
...
void XMLDocumentParser::CdataBlock(const String& text)
{
    ...
    if (parser_paused_) {
        pending_callbacks_.push_back(std::make_unique<PendingCDATABlockCallback>(text, GetTextPosition()));
        return;
    }
    ...
    // and this REF.
    CDATASection* cdata_tail = current_node_ ? DynamicTo<CDATASection>(current_node_->lastChild()) : nullptr;
    if (cdata_tail && (RuntimeEnabledFeatures::XMLParserMergeAdjacentCDataSectionsEnabled() || is_start_of_new_chunk)) {
        cdata_tail->ParserAppendData(text);
    } else {
        current_node_->ParserAppendChild(CDATASection::Create(current_node_->GetDocument(), text));
    }
}
...
void XMLDocumentParser::Comment(const String& text)
{
    ...
    if (parser_paused_) {
        pending_callbacks_.push_back(std::make_unique<PendingCommentCallback>(text, GetTextPosition()));
        return;
    }
    ...
    current_node_->ParserAppendChild(Comment::Create(current_node_->GetDocument(), text));
}

enum StandaloneInfo { kStandaloneUnspecified = -2, kNoXMlDeclaration, kStandaloneNo, kStandaloneYes };
...
void XMLDocumentParser::StartDocument(const String& version, const String& encoding, int standalone)
{
    ...
    if (standalone != kStandaloneUnspecified)
        GetDocument()->setXMLStandalone(standalone_info == kStandaloneYes, ASSERT_NO_EXCEPTION);
    if (!encoding.IsNull())
...
void XMLDocumentParser::InternalSubset(const String& name, const String& external_id, const String& system_id)
{
    if (IsStopped())
        return;

    if (parser_paused_) {
        pending_callbacks_.push_back(std::make_unique<PendingInternalSubsetCallback>(name, external_id, system_id, GetTextPosition()));
        return;
    }

    if (GetDocument()) {
        GetDocument()->ParserAppendChild(MakeGarbageCollected<DocumentType>(GetDocument(), name, external_id, system_id));
    }
}
...
static void StartElementNsHandler(void* closure, const xmlChar* local_name, const xmlChar* prefix, const xmlChar* uri, int nb_namespaces,
    const xmlChar** namespaces, int nb_attributes, int nb_defaulted, const xmlChar** libxml_attributes)
{
    GetParser(closure)->StartElementNs(
        ToAtomicString(local_name), ToAtomicString(prefix), ToAtomicString(uri), nb_namespaces, namespaces, nb_attributes, nb_defaulted, libxml_attributes);
}
...
template <size_t N> static base::span<const char, N - 1> CopyToEntityBuffer(base::span<const char, N> expanded_entity_chars)
{
    auto entity_buffer = base::as_writable_chars(base::span(g_shared_xhtml_entity_result));
    entity_buffer.first<N>().copy_from(expanded_entity_chars);
    return entity_buffer.first<N - 1>();
}

static base::span<const char> ConvertUTF16EntityToUTF8(const DecodedHTMLEntity& entity)
{
    ...
    WTF::unicode::ConversionResult conversion_result
        = WTF::unicode::ConvertUTF16ToUTF8(&utf16_entity, utf16_entity + entity.length, &target, target + entity_buffer.size());
...
{
    std::optional<DecodedHTMLEntity> decoded_entity = DecodeNamedEntity(reinterpret_cast<const char*>(name));
    if (!decoded_entity) {
        return nullptr;
    }
    ...
    if (decoded_entity->length == 1 && decoded_entity->data[0] == '&') {
        ...
        entity_utf8 = CopyToEntityBuffer(base::span_with_nul_from_cstring("&#60;"));
    } else if (decoded_entity->length == 2 && decoded_entity->data[0] == '<' && decoded_entity->data[1] == 0x20D2) {
        entity_utf8 = CopyToEntityBuffer(base::span_with_nul_from_cstring("&#60;\xE2\x83\x92"));
    } else {
...
static void InternalSubsetHandler(void* closure, const xmlChar* name, const xmlChar* external_id, const xmlChar* system_id)
{
    GetParser(closure)->InternalSubset(ToString(name), ToString(external_id), ToString(system_id));
    xmlSAX2InternalSubset(closure, name, external_id, system_id);
}

static void ExternalSubsetHandler(void* closure, const xmlChar*, const xmlChar* external_id, const xmlChar*)
{
    // https://html.spec.whatwg.org/C/#parsing-xhtml-documents:named-character-references
    String ext_id = ToString(external_id);
    if (ext_id == "-//W3C//DTD XHTML 1.0 Transitional//EN" || ext_id == "-//W3C//DTD XHTML 1.1//EN" || ext_id == "-//W3C//DTD XHTML 1.0 Strict//EN"
        || ext_id == "-//W3C//DTD XHTML 1.0 Frameset//EN" || ext_id == "-//W3C//DTD XHTML Basic 1.0//EN"
        || ext_id == "-//W3C//DTD XHTML 1.1 plus MathML 2.0//EN" || ext_id == "-//W3C//DTD XHTML 1.1 plus MathML 2.0 plus SVG 1.1//EN"
        || ext_id == "-//W3C//DTD MathML 2.0//EN" || ext_id == "-//WAPFORUM//DTD XHTML Mobile 1.0//EN" || ext_id == "-//WAPFORUM//DTD XHTML Mobile 1.1//EN"
...
void XMLDocumentParser::DoEnd()
{
    ...
    bool xml_viewer_mode = !saw_error_ && !saw_css_ && !saw_xsl_transform_ && HasNoStyleInformation(GetDocument());
    if (xml_viewer_mode) {
        GetDocument()->SetIsViewSource(true);
        TransformDocumentToXMLTreeView(*GetDocument());
    } else if (saw_xsl_transform_) {
        xmlDocPtr doc = XmlDocPtrForString(GetDocument(), original_source_for_transform_.ToString(), GetDocument()->Url().GetString());
...
xmlDocPtr XmlDocPtrForString(Document* document, const String& source, const String& url)
{
    ...
    return xmlReadMemory(input.Data(), input.size(), url.Latin1().c_str(), input.Encoding(), XSLT_PARSE_OPTIONS | XML_PARSE_HUGE);
}
...
bool XMLDocumentParser::AppendFragmentSource(const String& chunk)
{
    ...
    if (bytes_processed == -1 || bytes_processed != static_cast<int64_t>(chunk_as_utf8.length())) {
        ...
        DCHECK(saw_error_ || (bytes_processed >= 0 && !chunk_as_utf8.data()[bytes_processed]));
        return false;
    }
#endif
...
{
    if (!IsWaitingForScripts() && !waiting_for_stylesheets_ && parser_paused_ && IsParsing()) {
        ResumeParsing();
    }
}
...
static void AttributesStartElementNsHandler(void* closure, const xmlChar* xml_local_name, const xmlChar* /*xmlPrefix*/, const xmlChar* /*xmlURI*/,
    int /*nbNamespaces*/, const xmlChar** /*namespaces*/, int nb_attributes, int /*nbDefaulted*/, const xmlChar** libxml_attributes)
{
    ...
    for (int i = 0; i < nb_attributes; ++i) {
        ...
        String attr_q_name = attr_prefix.empty() ? attr_local_name : attr_prefix + ":" + attr_local_name;
...
HashMap<String, String> ParseAttributes(const String& string, bool& attrs_ok)
{
    ...
    sax.initialized = XML_SAX2_MAGIC;
    scoped_refptr<XMLParserContext> parser = XMLParserContext::CreateStringParser(&sax, &state);
