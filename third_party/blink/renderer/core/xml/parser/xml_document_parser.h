...
public:
    static scoped_refptr<XMLParserContext> CreateMemoryParser(xmlSAXHandlerPtr, void* user_data, const std::string& chunk);
    static scoped_refptr<XMLParserContext> CreateStringParser(xmlSAXHandlerPtr, void* user_data);
    ~XMLParserContext();
...
class XMLDocumentParser final : public ScriptableDocumentParser, public XMLParserScriptRunnerHost {
public:
    ...
    {
        is_xhtml_document_ = is_xhtml;
    }
    bool IsXHTMLDocument() const
    {
        return is_xhtml_document_;
    }
    ...
    static bool ParseDocumentFragment(const String&, DocumentFragment*, Element* parent, ParserContentPolicy, ExceptionState&);
...
public:
    ...
    void StartElementNs(const AtomicString& local_name, const AtomicString& prefix, const AtomicString& uri, int namespace_count, const xmlChar** namespaces,
        int attribute_count, int defaulted_count, const xmlChar** libxml_attributes);
    void EndElementNs();
    ...
    void InternalSubset(const String& name, const String& external_id, const String& system_id);
    void EndDocument();
