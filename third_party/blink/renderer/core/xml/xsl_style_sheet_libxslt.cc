...
XSLStyleSheet::XSLStyleSheet(XSLStyleSheet* parent_style_sheet, const String& original_url, const KURL& final_url)
    : owner_node_(nullptr)
...
XSLStyleSheet::XSLStyleSheet(Node* parent_node, const String& original_url, const KURL& final_url, bool embedded)
...
XSLStyleSheet::XSLStyleSheet(Document* owner_document, Node* style_sheet_root_node, const String& original_url, const KURL& final_url, bool embedded)
...
bool XSLStyleSheet::ParseString(const String& source)
{
    ...
    stylesheet_doc_ = xmlCtxtReadMemory(ctxt, input.Data(), input.size(), final_url_.GetString().Utf8().c_str(), input.Encoding(),
...
void XSLStyleSheet::LoadChildSheets()
{
    ...
    if (embedded_) {
        ...
        xmlAttrPtr id_node = xmlGetID(GetDocument(), (const xmlChar*)(final_url_.GetString().Utf8().c_str()));
        if (!id_node)
            return;
...
void XSLStyleSheet::LoadChildSheet(const String& href)
{
    ...
    for (XSLStyleSheet* parent_sheet = parentStyleSheet(); parent_sheet; parent_sheet = parent_sheet->parentStyleSheet()) {
        if (url == parent_sheet->BaseURL())
            return;
    }

    const String& url_string = url.GetString();
    ResourceLoaderOptions fetch_options(OwnerDocument()->GetExecutionContext()->GetCurrentWorld());
    fetch_options.initiator_info.name = fetch_initiator_type_names::kXml;
    FetchParameters params(ResourceRequest(OwnerDocument()->CompleteURL(url_string)), fetch_options);
    params.MutableResourceRequest().SetMode(network::mojom::RequestMode::kSameOrigin);
    XSLStyleSheetResource* resource = XSLStyleSheetResource::FetchSynchronously(params, OwnerDocument()->Fetcher());
    if (!resource->Sheet())
        return;

    XSLStyleSheet* style_sheet = MakeGarbageCollected<XSLStyleSheet>(this, url_string, resource->GetResponse().CurrentRequestUrl());
...
{
    for (XSLStyleSheet* style_sheet = this; style_sheet; style_sheet = style_sheet->parentStyleSheet()) {
...
xmlDocPtr XSLStyleSheet::LocateStylesheetSubResource(xmlDocPtr parent_doc, const xmlChar* uri)
{
    ...
    for (unsigned i = 0; i < children_.size(); ++i) {
        ...
        if (matched_parent) {
            ...
            xmlChar* child_uri = xmlBuildURI((const xmlChar*)import_href.c_str(), base);
