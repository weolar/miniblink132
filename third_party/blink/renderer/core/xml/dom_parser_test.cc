...
TEST(DOMParserTest, DomParserDocumentUsesQuirksMode)
{
    ...
    Document* document = parser->parseFromString("<div></div>", V8SupportedType(V8SupportedType::Enum::kTextHtml));
...
TEST(DOMParserTest, DomParserDocumentUsesNoQuirksMode)
{
    ...
    auto* parser = DOMParser::Create(scope.GetScriptState());
    base::HistogramTester histogram_tester;
    Document* document = parser->parseFromString("<!doctype html>", V8SupportedType(V8SupportedType::Enum::kTextHtml));
