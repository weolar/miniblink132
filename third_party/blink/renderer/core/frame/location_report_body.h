...
private:
    ...
    static ReportLocation CreateReportLocation(const String& file, std::optional<uint32_t> line_number, std::optional<uint32_t> column_number);

    static ReportLocation CreateReportLocation(std::unique_ptr<SourceLocation> location);
...
protected:
    ...
    explicit LocationReportBody(
        ...
        : LocationReportBody(CreateReportLocation(source_file, line_number, column_number))
    {
    }
...
public:
    ...
    std::optional<uint32_t> lineNumber() const
    {
        return line_number_;
    }
    std::optional<uint32_t> columnNumber() const
    {
