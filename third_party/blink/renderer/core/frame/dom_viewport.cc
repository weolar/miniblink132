...
DOMViewport::DOMViewport(LocalDOMWindow* window)
    : window_(window)
{
}
...
std::optional<HeapVector<Member<DOMRect>>> DOMViewport::segments() const
{
    ...
    WebVector<gfx::Rect> web_segments = frame->GetWidgetForLocalRoot()->ViewportSegments();
    ...
    const float dips_to_blink = frame->GetWidgetForLocalRoot()->DIPsToBlinkSpace(1.0f);
