...
class CORE_EXPORT CachedPermissionStatus final : public GarbageCollected<CachedPermissionStatus>,
...
public:
    ...
    public:
        ...
        // duplicate IPC calls.
        virtual void OnPermissionStatusChanged(mojom::blink::PermissionStatus status)
        {
        }

        virtual void OnPermissionStatusInitialized(PermissionStatusMap initilized_map) = 0;
    };
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(CachedPermissionStatusTest, UnregisterClientRemoveObserver);
    ...
    // it.
    void RegisterClient(Client* client, const Vector<mojom::blink::PermissionDescriptorPtr>& permissions);
    void UnregisterClient(Client* client, const Vector<mojom::blink::PermissionDescriptorPtr>& permissions);
    ...
    using ClientMap = HeapHashMap<mojom::blink::PermissionName, HeapHashSet<WeakMember<Client>>>;
    using PermissionToReceiversMap = HashMap<mojom::blink::PermissionName, mojo::ReceiverId>;
    using PermissionObserverReceiverSet = HeapMojoReceiverSet<mojom::blink::PermissionObserver, CachedPermissionStatus,
        HeapMojoWrapperMode::kWithContextObserver, mojom::blink::PermissionName>;
    const ClientMap& GetClientsForTesting() const
    {
        return clients_;
    }

    const PermissionToReceiversMap& GetPermissionToReceiversMapForTesting() const
    {
