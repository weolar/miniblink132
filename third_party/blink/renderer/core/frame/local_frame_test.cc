...
#if BUILDFLAG(IS_MAC)
void RegisterMockedHttpURLLoad(const std::string& base_url, const std::string& file_name)
{
...
public:
    ...
    void Init(base::OnceClosure callback, const blink::BrowserInterfaceBrokerProxy& provider)
    {
        callback_ = std::move(callback);
        provider.SetBinderForTesting(
            mojom::blink::TextInputHost::Name_, WTF::BindRepeating(&TestTextInputHostWaiter::BindTextInputHostReceiver, WTF::Unretained(this)));
    }
    ...
    void BindTextInputHostReceiver(mojo::ScopedMessagePipeHandle message_pipe_handle)
    {
        receiver_.Bind(mojo::PendingReceiver<mojom::blink::TextInputHost>(std::move(message_pipe_handle)));
    }
...
TEST_F(LocalFrameTest, ForceSynchronousDocumentInstall_XHTMLStyleInBody)
{
    ...
    data.Append("<html xmlns='http://www.w3.org/1999/xhtml'><body><style>div { color: "
                "green }</style><div id='div'></div></body></html>",
        static_cast<size_t>(118));
    page_holder->GetFrame().ForceSynchronousDocumentInstall(AtomicString("text/xml"), data);
...
TEST_F(LocalFrameTest, ForceSynchronousDocumentInstall_XHTMLLinkInBody)
{
    ...
    SegmentedBuffer data;
    data.Append("<html xmlns='http://www.w3.org/1999/xhtml'><body><link rel='stylesheet' "
...
TEST_F(LocalFrameTest, ForceSynchronousDocumentInstall_XHTMLStyleInHead)
{
    ...
    SegmentedBuffer data;
    data.Append("<html xmlns='http://www.w3.org/1999/xhtml'><head><style>div { color: "
...
TEST_F(LocalFrameTest, ForceSynchronousDocumentInstall_XHTMLLinkInHead)
{
    ...
    SegmentedBuffer data;
    data.Append("<html xmlns='http://www.w3.org/1999/xhtml'><head><link rel='stylesheet' "
...
TEST_F(LocalFrameTest, ForceSynchronousDocumentInstall_XMLStyleSheet)
{
    ...
    SegmentedBuffer data;
    data.Append("<?xml-stylesheet type='text/css' "
