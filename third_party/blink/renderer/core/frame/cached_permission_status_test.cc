...
Vector<PermissionDescriptorPtr> CreatePermissionDescriptors(const AtomicString& permissions_string)
{
...
class MockHTMLPermissionElement : public GarbageCollected<MockHTMLPermissionElement>, public CachedPermissionStatus::Client {
public:
    MockHTMLPermissionElement() = default;

    ~MockHTMLPermissionElement() override = default;

    void OnPermissionStatusInitialized(HashMap<PermissionName, PermissionStatus> map) override
    {
    }
...
public:
    ...
    CachedPermissionStatusTest& operator=(const CachedPermissionStatusTest&) = delete;

    void SetUp() override
    {
        PageTestBase::SetUp();
        CachedPermissionStatus::From(GetDocument().domWindow())
            ->SetPermissionStatusMap(HashMap<PermissionName, PermissionStatus>({ { PermissionName::VIDEO_CAPTURE, PermissionStatus::ASK },
                { PermissionName::AUDIO_CAPTURE, PermissionStatus::ASK }, { PermissionName::GEOLOCATION, PermissionStatus::ASK } }));
    }

    bool HasClient(PermissionName permission, CachedPermissionStatus::Client* client) const
    {
        CachedPermissionStatus* cache = CachedPermissionStatus::From(GetDocument().domWindow());
    ...
    {
        CachedPermissionStatus* cache = CachedPermissionStatus::From(GetDocument().domWindow());
        const auto& permission_to_receivers_map = cache->GetPermissionToReceiversMapForTesting();
        auto it = permission_to_receivers_map.find(permission);
        if (it == permission_to_receivers_map.end()) {
            return false;
        }
        mojo::ReceiverId id = it->value;
        auto& permission_observer_receivers = cache->GetPermissionObserverReceiversForTesting();
...
TEST_F(CachedPermissionStatusTest, MAYBE_RegisterClient)
{
    ...
    CachedPermissionStatus* cache = CachedPermissionStatus::From(GetDocument().domWindow());
    cache->RegisterClient(client1, CreatePermissionDescriptors(AtomicString("geolocation")));
    ...
    cache->RegisterClient(client2, CreatePermissionDescriptors(AtomicString("geolocation")));
    EXPECT_TRUE(HasClient(PermissionName::GEOLOCATION, client2));
    auto* client3 = MakeGarbageCollected<MockHTMLPermissionElement>();
    cache->RegisterClient(client3, CreatePermissionDescriptors(AtomicString("camera")));
...
#if BUILDFLAG(IS_ANDROID)
#define MAYBE_UnregisterClientRemoveObserver DISABLED_UnregisterClientRemoveObserver
#else
...
TEST_F(CachedPermissionStatusTest, MAYBE_UnregisterClientRemoveObserver)
{
    ...
    cache->UnregisterClient(client2, CreatePermissionDescriptors(AtomicString("geolocation")));
    EXPECT_TRUE(HasPermisionObserver(PermissionName::GEOLOCATION));
    EXPECT_TRUE(HasPermisionObserver(PermissionName::VIDEO_CAPTURE));
    cache->UnregisterClient(client1, CreatePermissionDescriptors(AtomicString("geolocation")));
    EXPECT_FALSE(HasPermisionObserver(PermissionName::GEOLOCATION));
    cache->UnregisterClient(client3, CreatePermissionDescriptors(AtomicString("camera")));
