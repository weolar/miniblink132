...
struct IdentifiableTokenKey {
    ...
    {
        return token == other.token && is_deleted_value == other.is_deleted_value && is_empty_value == other.is_empty_value;
    }
    bool operator!=(const IdentifiableTokenKey& other) const
    {
        return !(*this == other);
...
struct IdentifiableTokenKeyHashTraits : WTF::SimpleClassHashTraits<IdentifiableTokenKey> {
    static unsigned GetHash(const IdentifiableTokenKey& key)
    {
        return WTF::GetHash(key.token.ToUkmMetricValue()) ^ WTF::GetHash((key.is_deleted_value << 1) + key.is_empty_value);
    }
    static const bool kEmptyValueIsZero = false;
    static IdentifiableTokenKey EmptyValue()
    {
        return IdentifiableTokenKey();
    }
};
...
public:
    ...
    FontMatchingMetrics(ExecutionContext* execution_context, scoped_refptr<base::SingleThreadTaskRunner> task_runner);
    ...
    void ReportFontLookupByUniqueOrFamilyName(const AtomicString& name, const FontDescription& font_description, const SimpleFontData* resulting_font_data);
    ...
    void ReportFontLookupByUniqueNameOnly(
        const AtomicString& name, const FontDescription& font_description, const SimpleFontData* resulting_font_data, bool is_loading_fallback = false);
    ...
    void ReportLastResortFallbackFontLookup(const FontDescription& font_description, const SimpleFontData* resulting_font_data);

    // Reports a generic font family name was matched according to the script and
    // the user's preferences to a font family name.
    void ReportFontFamilyLookupByGenericFamily(const AtomicString& generic_font_family_name, UScriptCode script,
    ...
    void ReportEmojiSegmentGlyphCoverage(unsigned num_clusters, unsigned num_broken_clusters);
...
private:
    ...
    using TokenToTokenHashMap = HashMap<IdentifiableTokenKey, IdentifiableToken, IdentifiableTokenKeyHashTraits>;
    ...
    void InsertFontHashIntoMap(IdentifiableTokenKey input_key, const SimpleFontData* font_data, TokenToTokenHashMap& hash_map);
    ...
    // family names.
    void ReportLocalFontExistenceByUniqueNameOnly(const AtomicString& font_name, bool font_exists);
    ...
    void ReportLocalFontExistenceByUniqueOrFamilyName(const AtomicString& font_name, bool font_exists);

    // Constructs a builder with a hash of the FontSelectionRequest already added.
    IdentifiableTokenBuilder GetTokenBuilderWithFontSelectionRequest(const FontDescription& font_description);
    ...
    IdentifiableToken GetPostScriptNameTokenForFontData(const SimpleFontData* font_data);
