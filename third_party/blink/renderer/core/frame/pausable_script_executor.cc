...
public:
    ...
    PromiseAggregator(ScriptState* script_state, const v8::LocalVector<v8::Value>& values, Callback callback);
...
private:
    ...
    public:
        OnSettled(PromiseAggregator* aggregator, wtf_size_t index, bool was_fulfilled)
        ...
        void React(ScriptState* script_state, ScriptValue value)
        {
            ...
            if (was_fulfilled_) {
                aggregator_->results_[index_].Reset(script_state->GetIsolate(), value.V8Value());
            }
...
PromiseAggregator::PromiseAggregator(ScriptState* script_state, const v8::LocalVector<v8::Value>& values, Callback callback)
    ...
    , callback_(std::move(callback))
{
    for (wtf_size_t i = 0; i < values.size(); ++i) {
        ...
        ToResolvedPromise<IDLAny>(script_state, values[i])
            .React(script_state, MakeGarbageCollected<OnSettled>(this, i, /*was_fulfilled=*/true),
...
public:
    WebScriptExecutor(Vector<WebScriptSource> sources, ExecuteScriptPolicy execute_script_policy)
    ...
    v8::LocalVector<v8::Value> Execute(ScriptState* script_state) override
    {
        ...
        for (const auto& source : sources_) {
            ...
            // a foreign world.
            ScriptEvaluationResult result = ClassicScript::CreateUnspecifiedScript(source, SanitizeScriptErrors::kDoNotSanitize)
                                                ->RunScriptOnScriptStateAndReturnValue(script_state, execute_script_policy_);
...
V8FunctionExecutor::V8FunctionExecutor(
    v8::Isolate* isolate, v8::Local<v8::Function> function, v8::Local<v8::Value> receiver, int argc, v8::Local<v8::Value> argv[])
...
v8::LocalVector<v8::Value> V8FunctionExecutor::Execute(ScriptState* script_state)
{
    ...
    v8::LocalVector<v8::Value> results(isolate);
    {
        ...
        if (V8ScriptRunner::CallFunction(
                function_.Get(isolate), ExecutionContext::From(script_state), receiver_.Get(isolate), static_cast<int>(args.size()), args.data(), isolate)
...
void PausableScriptExecutor::CreateAndRun(v8::Local<v8::Context> context, v8::Local<v8::Function> function, v8::Local<v8::Value> receiver, int argc,
    v8::Local<v8::Value> argv[], mojom::blink::WantResultOption want_result_option, WebScriptExecutionCallback callback)
{
    ...
    PausableScriptExecutor* executor = MakeGarbageCollected<PausableScriptExecutor>(script_state, mojom::blink::UserActivationOption::kDoNotActivate,
        mojom::blink::LoadEventBlockingOption::kDoNotBlock, want_result_option, mojom::blink::PromiseResultOption::kDoNotWait, std::move(callback),
        MakeGarbageCollected<V8FunctionExecutor>(script_state->GetIsolate(), function, receiver, argc, argv));
    executor->Run();
}

void PausableScriptExecutor::CreateAndRun(ScriptState* script_state, Vector<WebScriptSource> sources, ExecuteScriptPolicy execute_script_policy,
    mojom::blink::UserActivationOption user_activation_option, mojom::blink::EvaluationTiming evaluation_timing,
    mojom::blink::LoadEventBlockingOption blocking_option, mojom::blink::WantResultOption want_result_option,
    mojom::blink::PromiseResultOption promise_result_option, WebScriptExecutionCallback callback)
{
    auto* executor = MakeGarbageCollected<PausableScriptExecutor>(script_state, user_activation_option, blocking_option, want_result_option,
        promise_result_option, std::move(callback), MakeGarbageCollected<WebScriptExecutor>(std::move(sources), execute_script_policy));
...
PausableScriptExecutor::PausableScriptExecutor(ScriptState* script_state, mojom::blink::UserActivationOption user_activation_option,
    mojom::blink::LoadEventBlockingOption blocking_option, mojom::blink::WantResultOption want_result_option,
    mojom::blink::PromiseResultOption promise_result_option, WebScriptExecutionCallback callback, Executor* executor)
...
void PausableScriptExecutor::PostExecuteAndDestroySelf(ExecutionContext* context)
{
    task_handle_ = PostCancellableTask(*context->GetTaskRunner(TaskType::kJavascriptTimerImmediate), FROM_HERE,
        WTF::BindOnce(&PausableScriptExecutor::ExecuteAndDestroySelf, WrapPersistent(this)));
}
...
void PausableScriptExecutor::ExecuteAndDestroySelf()
{
    ...
    if (user_activation_option_ == mojom::blink::UserActivationOption::kActivate) {
        // TODO(mustaq): Need to make sure this is safe. https://crbug.com/1082273
        if (auto* window = DynamicTo<LocalDOMWindow>(GetExecutionContext())) {
            LocalFrame::NotifyUserActivation(window->GetFrame(), mojom::blink::UserActivationNotificationType::kWebScriptExec);
        }
    }
    ...
    case mojom::blink::PromiseResultOption::kAwait:
        ...
        keep_alive_ = this;
        MakeGarbageCollected<PromiseAggregator>(script_state_, results, WTF::BindOnce(&PausableScriptExecutor::HandleResults, WrapWeakPersistent(this)));
        break;
...
void PausableScriptExecutor::HandleResults(const v8::LocalVector<v8::Value>& results)
{
    // The script may have removed the frame, in which case ContextDestroyed()
    // will have handled the disposal/callback.
    if (!script_state_->ContextIsValid())
    ...
    if (callback_) {
        ...
        case mojom::blink::WantResultOption::kWantResultDateAndRegExpAllowed:
            if (!results.empty() && !results.back().IsEmpty()) {
                ...
                if (want_result_option_ == mojom::blink::WantResultOption::kWantResultDateAndRegExpAllowed) {
                    converter->SetDateAllowed(true);
                    converter->SetRegExpAllowed(true);
                }
                if (std::unique_ptr<base::Value> new_value = converter->FromV8Value(results.back(), script_state_->GetContext())) {
