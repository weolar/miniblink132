...
namespace {
const char kExtensionFeaturesHistogramName[] = "Blink.UseCounter.Extensions.Features";
...
int GetPageVisitsBucketforHistogram(const std::string& histogram_name)
{
    ...
    return static_cast<int>(blink::mojom::blink::CSSSampleId::kTotalPagesMeasured);
}
...
public:
    ...
    HashSet<CSSPropertyID> GetAlternatives() const
    {
        ...
        for (CSSPropertyID property : CSSPropertyIDList()) {
            if (CSSPropertyID alternative_id = CSSUnresolvedProperty::Get(property).GetAlternative(); alternative_id != CSSPropertyID::kInvalid) {
...
protected:
    LocalFrame* GetFrame()
    {
        return &dummy_->GetFrame();
    }
    void SetIsViewSource()
    {
        dummy_->GetDocument().SetIsViewSource(true);
    }
    void SetURL(const KURL& url)
    {
        dummy_->GetDocument().SetURL(url);
    }
    Document& GetDocument()
    {
        return dummy_->GetDocument();
    }
...
class UseCounterImplBrowserReportTest : public UseCounterImplTest, public ::testing::WithParamInterface</* URL */ const char*> { };

INSTANTIATE_TEST_SUITE_P(All, UseCounterImplBrowserReportTest,
    ::testing::Values("chrome-extension://dummysite/", "file://dummyfile", "data:;base64,", "ftp://ftp.dummy/dummy.txt", "http://foo.com", "https://bar.com"));
...
TEST_P(UseCounterImplBrowserReportTest, ReportOnlyHTTPFamily)
{
    ...
    use_counter.Count(CSSPropertyID::kHeight, UseCounterImpl::CSSPropertyType::kDefault, GetFrame());
    use_counter.Count(CSSPropertyID::kHeight, UseCounterImpl::CSSPropertyType::kAnimation, GetFrame());

    auto* dummy_client = static_cast<UseCounterImplBrowserReportTest::DummyLocalFrameClient*>(GetFrame()->Client());

    EXPECT_EQ(!dummy_client->observed_features().empty(), url.ProtocolIsInHTTPFamily());
}
...
TEST_F(UseCounterImplTest, RecordingExtensions)
{
    ...
    histogram_tester_.ExpectBucketCount(histogram, static_cast<int>(second_item), 1);
...
TEST_F(UseCounterImplTest, CSSSelectorPseudoWhere)
{
    ...
    document.documentElement()->setInnerHTML("<style>.a+:where(.b, .c+.d) { color: red; }</style>");
...
TEST_F(UseCounterImplTest, CSSSelectorPseudoAnyLink)
{
    ...
    EXPECT_FALSE(document.IsUseCounted(feature));
    document.documentElement()->setInnerHTML("<style>:any-link { color: red; }</style>");
    EXPECT_TRUE(document.IsUseCounted(feature));
}
...
TEST_F(UseCounterImplTest, CSSSelectorPseudoWebkitAnyLink)
{
    ...
    EXPECT_FALSE(document.IsUseCounted(feature));
    document.documentElement()->setInnerHTML("<style>:-webkit-any-link { color: red; }</style>");
    EXPECT_TRUE(document.IsUseCounted(feature));
}
...
TEST_F(UseCounterImplTest, CSSSelectorPseudoIs)
{
    ...
    EXPECT_FALSE(document.IsUseCounted(feature));
    document.documentElement()->setInnerHTML("<style>.a+:is(.b, .c+.d) { color: red; }</style>");
    EXPECT_TRUE(document.IsUseCounted(feature));
...
TEST_F(UseCounterImplTest, CSSSelectorPseudoDir)
{
    ...
    EXPECT_FALSE(document.IsUseCounted(feature));
    document.documentElement()->setInnerHTML("<style>:dir(ltr) { color: red; }</style>");
    EXPECT_TRUE(document.IsUseCounted(feature));
}
...
TEST_F(UseCounterImplTest, CSSSelectorNthChildOfSelector)
{
    ...
    EXPECT_FALSE(document.IsUseCounted(feature));
    document.documentElement()->setInnerHTML("<style>.a:nth-child(3) { color: red; }</style>");
    EXPECT_FALSE(document.IsUseCounted(feature));
    document.documentElement()->setInnerHTML("<style>.a:nth-child(3 of .b) { color: red; }</style>");
    EXPECT_TRUE(document.IsUseCounted(feature));
}
...
TEST_F(UseCounterImplTest, CSSGridLayoutPercentageColumnIndefiniteWidth)
{
    ...
    EXPECT_FALSE(document.IsUseCounted(feature));
    document.documentElement()->setInnerHTML("<div style='display: inline-grid; grid-template-columns: 50%;'>"
                                             "</div>");
...
TEST_F(UseCounterImplTest, CSSFlexibleBox)
{
    ...
    EXPECT_FALSE(document.IsUseCounted(feature));
    document.documentElement()->setInnerHTML("<div style='display: flex;'>flexbox</div>");
    UpdateAllLifecyclePhases(document);
    EXPECT_TRUE(document.IsUseCounted(feature));
}
...
TEST_F(UseCounterImplTest, CSSFlexibleBoxInline)
{
    ...
    WebFeature feature = WebFeature::kCSSFlexibleBox;
    EXPECT_FALSE(document.IsUseCounted(feature));
    document.documentElement()->setInnerHTML("<div style='display: inline-flex;'>flexbox</div>");
    UpdateAllLifecyclePhases(document);
    EXPECT_TRUE(document.IsUseCounted(feature));
}
...
TEST_F(UseCounterImplTest, HTMLRootContained)
{
    ...
    document.documentElement()->SetInlineStyleProperty(CSSPropertyID::kDisplay, "none");
...
TEST_F(DeprecationTest, InspectorDisablesDeprecation)
{
    ...
    WebFeature feature = WebFeature::kCSSSelectorInternalMediaControlsOverlayCastButton;
...
TEST_F(UseCounterImplTest, CSSSelectorHostContextInSnapshotProfile)
{
    ...
    Element* span = shadow_root.QuerySelector(AtomicString(":host-context(#parent) span"));
    EXPECT_TRUE(span);
    EXPECT_TRUE(document.IsUseCounted(feature));
}
...
TEST_F(UseCounterImplTest, MaximumCSSSampleId)
{
    ...
    EXPECT_EQ(static_cast<int>(mojom::blink::CSSSampleId::kMaxValue), max_sample_id);
}
...
TEST_F(UseCounterImplTest, BackgroundClip)
{
    ...
    document.documentElement()->setInnerHTML("<style>html{background-clip: border-box;}</style>");
    UpdateAllLifecyclePhases(document);
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipBorder));
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipContent));
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipPadding));

    document.documentElement()->setInnerHTML("<style>html{background-clip: content-box;}</style>");
    UpdateAllLifecyclePhases(document);
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipBorder));
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipContent));
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipPadding));

    document.documentElement()->setInnerHTML("<style>html{background-clip: padding-box;}</style>");
    UpdateAllLifecyclePhases(document);
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipBorder));
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipContent));
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipPadding));

    document.documentElement()->setInnerHTML("<style>html{-webkit-background-clip: border-box;}</style>");
    UpdateAllLifecyclePhases(document);
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipBorder));
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipContent));
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipPadding));

    document.documentElement()->setInnerHTML("<style>html{-webkit-background-clip: content-box;}</style>");
    UpdateAllLifecyclePhases(document);
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipBorder));
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipContent));
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipPadding));

    document.documentElement()->setInnerHTML("<style>html{-webkit-background-clip: padding-box;}</style>");
    UpdateAllLifecyclePhases(document);
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipBorder));
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipContent));
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipPadding));

    document.documentElement()->setInnerHTML("<style>html{-webkit-background-clip: text;}</style>");
    UpdateAllLifecyclePhases(document);
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipBorder));
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipContent));
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipPadding));
    ...
    document.documentElement()->setInnerHTML("<style>html{-webkit-background-clip: border;}</style>");
    UpdateAllLifecyclePhases(document);
    ...
    document.documentElement()->setInnerHTML("<style>html{-webkit-background-clip: content;}</style>");
    UpdateAllLifecyclePhases(document);
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipBorder));
    if (RuntimeEnabledFeatures::CSSBackgroundClipUnprefixEnabled()) {
        EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipContent));
    ...
    document.documentElement()->setInnerHTML("<style>html{-webkit-background-clip: padding;}</style>");
    UpdateAllLifecyclePhases(document);
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipBorder));
    EXPECT_FALSE(document.IsUseCounted(WebFeature::kCSSBackgroundClipContent));
    if (RuntimeEnabledFeatures::CSSBackgroundClipUnprefixEnabled()) {
...
TEST_F(UseCounterImplTest, H1UserAgentFontSizeInSectionApplied)
{
    ...
    UpdateAllLifecyclePhases(document);
    EXPECT_FALSE(document.IsUseCounted(feature)) << "Not inside sectioning element";

    document.documentElement()->setInnerHTML(R"HTML(
      <article><h1 style="font-size: 10px"></h1></article>
  )HTML");
    UpdateAllLifecyclePhases(document);
    EXPECT_FALSE(document.IsUseCounted(feature)) << "Inside sectioning element with author font-size";

    document.documentElement()->setInnerHTML(R"HTML(
      <article><h1></h1></article>
  )HTML");
    UpdateAllLifecyclePhases(document);
    EXPECT_TRUE(document.IsUseCounted(feature)) << "Inside sectioning element with UA font-size";
}
