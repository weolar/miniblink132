...
bool ViewportData::ShouldMergeWithLegacyDescription(ViewportDescription::Type origin) const
{
    return document_->GetSettings() && document_->GetSettings()->GetViewportMetaMergeContentQuirk() && legacy_viewport_description_.IsMetaViewportType()
        && legacy_viewport_description_.type == origin;
}

void ViewportData::SetViewportDescription(const ViewportDescription& viewport_description)
{
...
ViewportDescription ViewportData::GetViewportDescription() const
{
    ...
    bool viewport_meta_enabled = document_->GetSettings() && document_->GetSettings()->GetViewportMetaEnabled();
    if (legacy_viewport_description_.type != ViewportDescription::kUserAgentStyleSheet && viewport_meta_enabled)
    ...
    if (virtual_keyboard_overlays_content_) {
        applied_viewport_description.virtual_keyboard_mode = ui::mojom::blink::VirtualKeyboardMode::kOverlaysContent;
    }
...
void ViewportData::UpdateViewportDescription()
{
    ...
    mojom::ViewportFit current_viewport_fit = GetViewportDescription().GetViewportFit();
    ...
    if (viewport_fit_ != current_viewport_fit) {
        if (AssociatedInterfaceProvider* provider = document_->GetFrame()->Client()->GetRemoteNavigationAssociatedInterfaces()) {
            // Bind the mojo interface.
            if (!display_cutout_host_.is_bound()) {
                provider->GetInterface(display_cutout_host_.BindNewEndpointAndPassReceiver(provider->GetTaskRunner()));
            ...
            if (document_->GetFrame()->IsOutermostMainFrame()) {
                if (current_viewport_fit == mojom::blink::ViewportFit::kContain) {
                    ...
                    || current_viewport_fit == mojom::blink::ViewportFit::kCoverForcedByUserAgent) {
                    ...
                    UseCounter::Count(document_, WebFeature::kViewportFitCoverOrSafeAreaInsetBottom);
                }
            }
        }
    ...
    if (document_->GetFrame()->IsMainFrame() && document_->GetPage()->GetVisualViewport().IsActiveViewport()) {
