...
TEST_F(HistoryUtilTest, CanChangeToURL)
{
    ...
    for (const auto& test : cases) {
        ...
        scoped_refptr<const SecurityOrigin> document_origin = SecurityOrigin::Create(document_url);
        EXPECT_EQ(test.expected, CanChangeToUrlForHistoryApi(url, document_origin.get(), document_url));
    }
}
...
TEST_F(HistoryUtilTest, CanChangeToURLInUniqueOrigin)
{
    ...
    for (const auto& test : cases) {
        KURL url(test.url);
        KURL document_url(test.document_url);
        scoped_refptr<const SecurityOrigin> document_origin = SecurityOrigin::CreateUniqueOpaque();
        EXPECT_EQ(test.expected, CanChangeToUrlForHistoryApi(url, document_origin.get(), document_url));
    }
}
