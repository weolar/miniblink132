...
{
    DEFINE_STATIC_LOCAL(Persistent<LocalFramesByTokenMap>, map, (MakeGarbageCollected<LocalFramesByTokenMap>()));
    return *map;
}
...
mojo::PendingRemote<mojom::blink::Blob> DataURLToBlob(const String& data_url)
{
    ...
    blob_data->AppendBytes(base::span<const uint8_t>((const uint8_t*)data_url_utf8.data(), data_url_utf8.size()));
    scoped_refptr<BlobDataHandle> blob_data_handle = BlobDataHandle::Create(std::move(blob_data), data_url_utf8.size());
    return blob_data_handle->CloneBlobRemote();
}

RemoteFrame* SourceFrameForOptionalToken(const std::optional<RemoteFrameToken>& source_frame_token)
{
...
void SetViewportSegmentVariablesForRect(
    StyleEnvironmentVariables& vars, gfx::Rect segment_rect, unsigned first_dimension, unsigned second_dimension, const ExecutionContext* context)
{
    vars.SetVariable(UADefinedTwoDimensionalVariable::kViewportSegmentTop, first_dimension, second_dimension,
        StyleEnvironmentVariables::FormatPx(segment_rect.y()), context);
    vars.SetVariable(UADefinedTwoDimensionalVariable::kViewportSegmentRight, first_dimension, second_dimension,
        StyleEnvironmentVariables::FormatPx(segment_rect.right()), context);
    vars.SetVariable(UADefinedTwoDimensionalVariable::kViewportSegmentBottom, first_dimension, second_dimension,
        StyleEnvironmentVariables::FormatPx(segment_rect.bottom()), context);
    vars.SetVariable(UADefinedTwoDimensionalVariable::kViewportSegmentLeft, first_dimension, second_dimension,
        StyleEnvironmentVariables::FormatPx(segment_rect.x()), context);
    vars.SetVariable(UADefinedTwoDimensionalVariable::kViewportSegmentWidth, first_dimension, second_dimension,
        StyleEnvironmentVariables::FormatPx(segment_rect.width()), context);
    vars.SetVariable(UADefinedTwoDimensionalVariable::kViewportSegmentHeight, first_dimension, second_dimension,
        StyleEnvironmentVariables::FormatPx(segment_rect.height()), context);
}

mojom::blink::BlockingDetailsPtr CreateBlockingDetailsMojom(const FeatureAndJSLocationBlockingBFCache& blocking_details)
{
    auto feature_location_to_report = mojom::blink::BlockingDetails::New();
    feature_location_to_report->feature = static_cast<uint32_t>(blocking_details.Feature());
    // Zero line number and column number means no source location found.
    if (blocking_details.LineNumber() > 0 && blocking_details.ColumnNumber() > 0) {
        // `Url()` and `Function()` may return nullptr.
        auto source_location = mojom::blink::ScriptSourceLocation::New(blocking_details.Url() ? KURL(blocking_details.Url()) : KURL(),
            blocking_details.Function() ? blocking_details.Function() : "", blocking_details.LineNumber(), blocking_details.ColumnNumber());
        feature_location_to_report->source = std::move(source_location);
    }
    return feature_location_to_report;
}

bool IsNavigationBlockedByCoopRestrictProperties(const LocalFrame& accessing_frame, const Frame& target_frame)
{
    ...
    if (accessing_frame.GetPage()->CoopRelatedGroupToken() != target_frame.GetPage()->CoopRelatedGroupToken()) {
        return false;
    }
    ...
    if (accessing_frame.GetPage()->BrowsingContextGroupToken() != target_frame.GetPage()->BrowsingContextGroupToken()) {
        return true;
    }
...
mojom::blink::StorageTypeAccessed ToMojoStorageType(blink::WebContentSettingsClient::StorageType storage_type)
{
    switch (storage_type) {
...
LocalFrame* FromFrameTokenHash(const size_t& frame_token_hash)
{
    ...
    auto it = local_frames_map.find(frame_token_hash);
...
void LocalFrame::Init(Frame* opener, const DocumentToken& document_token, std::unique_ptr<PolicyContainer> policy_container, const StorageKey& storage_key,
    ukm::SourceId document_ukm_source_id, const KURL& creator_base_url)
{
    if (!policy_container)
    ...
    GetInterfaceRegistry()->AddInterface(WTF::BindRepeating(&LocalFrame::BindTextFragmentReceiver, WrapWeakPersistent(this)));
...
void LocalFrame::CreateView(const gfx::Size& viewport_size, const Color& background_color)
{
    DCHECK(this);
    DCHECK(GetPage());
    ...
    if (Owner()) {
        View()->SetCanHaveScrollbars(Owner()->ScrollbarMode() != mojom::blink::ScrollbarMode::kAlwaysOff);
    }
}
...
void LocalFrame::Navigate(FrameLoadRequest& request, WebFrameLoadType frame_load_type)
{
    ...
    TRACE_EVENT2(
        "navigation", "LocalFrame::Navigate", "url", request.GetResourceRequest().Url().GetString().Utf8(), "load_type", static_cast<int>(frame_load_type));

    if (request.GetClientNavigationReason() != ClientNavigationReason::kNone
        && request.GetClientNavigationReason() != ClientNavigationReason::kInitialFrameNavigation) {
        probe::FrameScheduledNavigation(this, request.GetResourceRequest().Url(), base::TimeDelta(), request.GetClientNavigationReason());
    }

    if (NavigationShouldReplaceCurrentHistoryEntry(request, frame_load_type))
        frame_load_type = WebFrameLoadType::kReplaceCurrentItem;

    const ClientNavigationReason client_redirect_reason = request.GetClientNavigationReason();
    loader_.StartNavigation(request, frame_load_type);

    if (client_redirect_reason != ClientNavigationReason::kNone && client_redirect_reason != ClientNavigationReason::kInitialFrameNavigation) {
...
// replace to JS.
bool LocalFrame::ShouldReplaceForSameUrlNavigation(const FrameLoadRequest& request)
{
    ...
    if (request.GetOriginWindow() && !request.GetOriginWindow()->GetSecurityOrigin()->CanAccess(DomWindow()->GetSecurityOrigin())) {
        return false;
    }
    ...
    // in those cases.
    if (SchemeRegistry::IsWebUIScheme(request_url.Protocol()) || request.GetNavigationPolicy() != kNavigationPolicyCurrentTab) {
        return false;
    }
...
bool LocalFrame::NavigationShouldReplaceCurrentHistoryEntry(const FrameLoadRequest& request, WebFrameLoadType frame_load_type)
{
    ...
    if (request.GetClientNavigationReason() == ClientNavigationReason::kNone || request.GetClientNavigationReason() == ClientNavigationReason::kAnchorClick) {
        return false;
    }
    return true;
}
...
{
    TRACE_EVENT1("navigation", "LocalFrame::DetachImpl", "detach_type", static_cast<int>(type));
    std::string_view histogram_suffix = (type == FrameDetachType::kRemove) ? "Remove" : "Swap";
    base::ScopedUmaHistogramTimer histogram_timer(base::StrCat({ "Navigation.LocalFrame.DetachImpl.", histogram_suffix }));
    ...
    IgnoreOpensDuringUnloadCountIncrementer ignore_opens_during_unload(GetDocument());

    loader_.DispatchUnloadEventAndFillOldDocumentInfoIfNeeded(type == FrameDetachType::kSwap);
    if (evict_cached_session_storage_on_freeze_or_unload_) {
        ...
        // process.
        CoreInitializer::GetInstance().EvictSessionStorageCachedData(GetDocument()->GetPage());
    }
    if (!Client())
        return false;
...
BackgroundColorPaintImageGenerator* LocalFrame::GetBackgroundColorPaintImageGenerator()
{
    ...
    if (Thread::CompositorThread() && !local_root.background_color_paint_image_generator_) {
        local_root.background_color_paint_image_generator_ = BackgroundColorPaintImageGenerator::Create(local_root);
    }
    return local_root.background_color_paint_image_generator_.Get();
}

void LocalFrame::SetBackgroundColorPaintImageGeneratorForTesting(BackgroundColorPaintImageGenerator* generator_for_testing)
{
    LocalFrame& local_root = LocalFrameRoot();
...
BoxShadowPaintImageGenerator* LocalFrame::GetBoxShadowPaintImageGenerator()
{
    ...
    if (!local_root.box_shadow_paint_image_generator_) {
        local_root.box_shadow_paint_image_generator_ = BoxShadowPaintImageGenerator::Create(local_root);
    }
...
ClipPathPaintImageGenerator* LocalFrame::GetClipPathPaintImageGenerator()
{
    ...
    if (!local_root.clip_path_paint_image_generator_) {
        local_root.clip_path_paint_image_generator_ = ClipPathPaintImageGenerator::Create(local_root);
    }
    return local_root.clip_path_paint_image_generator_.Get();
}

void LocalFrame::SetClipPathPaintImageGeneratorForTesting(ClipPathPaintImageGenerator* generator)
{
    LocalFrame& local_root = LocalFrameRoot();
...
static String FrameDescription(const Frame& frame)
{
    ...
    return local_frame ? "with URL '" + local_frame->GetDocument()->Url().GetString().GetString() + "'"
                       : "with origin '" + frame.GetSecurityContext()->GetSecurityOrigin()->ToString() + "'";
}

void LocalFrame::PrintNavigationErrorMessage(const Frame& target_frame, const String& reason)
{
    String message = "Unsafe attempt to initiate navigation for frame " + FrameDescription(target_frame) + " from frame with URL '"
...
{
    console_->AddMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kJavaScript, mojom::ConsoleMessageLevel::kWarning, message));
}
...
{
    DCHECK(GetDocument());
    ChildFrameDisconnector(*GetDocument(), ChildFrameDisconnector::DisconnectReason::kDisconnectParent).Disconnect();
    return !!Client();
}
...
void LocalFrame::DidAttachDocument()
{
    ...
    if (is_window_controls_overlay_visible_) {
        DocumentStyleEnvironmentVariables& vars = GetDocument()->GetStyleEngine().EnsureEnvironmentVariables();
        DCHECK(!vars.ResolveVariable(
            StyleEnvironmentVariables::GetVariableName(UADefinedVariable::kTitlebarAreaX, document->GetExecutionContext()), {}, false /* record_metrics */));
...
void LocalFrame::OnFirstPaint(bool text_painted, bool image_painted)
{
    ...
    if (text_painted || image_painted) {
        ...
        GetLocalFrameHostRemote().DidInferColorScheme(l < 0.5 ? mojom::blink::PreferredColorScheme::kDark : mojom::blink::PreferredColorScheme::kLight);
...
bool LocalFrame::CanAccessEvent(const WebInputEventAttribution& attribution) const
{
    ...
    case WebInputEventAttribution::kTargetedFrame: {
        ...
        if (auto* page = frame_document->GetPage()) {
            ...
            if (auto* element = pointer_lock_controller.GetElement()) {
                ...
                DOMNodeId target_document_id = DOMNodeIdFromCompositorElementId(element_id);
                target_document = DynamicTo<Document>(DOMNodeIds::NodeForId(target_document_id));
            }
        }
        ...
        return GetSecurityContext()->GetSecurityOrigin()->CanAccess(target_document->domWindow()->GetSecurityOrigin());
    }
    case WebInputEventAttribution::kFocusedFrame:
        return GetPage() ? GetPage()->GetFocusController().FocusedFrame() == this : false;
...
void LocalFrame::Reload(WebFrameLoadType load_type)
{
    ...
    if (!loader_.GetDocumentLoader()->GetHistoryItem())
        return;
    TRACE_EVENT1("navigation", "LocalFrame::Reload", "load_type", static_cast<int>(load_type));

    FrameLoadRequest request(DomWindow(), loader_.ResourceRequestForReload(load_type, ClientRedirectPolicy::kClientRedirect));
    request.SetClientNavigationReason(ClientNavigationReason::kReload);
    probe::FrameScheduledNavigation(this, request.GetResourceRequest().Url(), base::TimeDelta(), ClientNavigationReason::kReload);
...
LocalWindowProxy* LocalFrame::WindowProxyMaybeUninitialized(DOMWrapperWorld& world)
{
...
void LocalFrame::HookBackForwardCacheEviction()
{
    ...
    static_cast<LocalWindowProxyManager*>(GetWindowProxyManager())->SetAbortScriptExecution([](v8::Isolate* isolate, v8::Local<v8::Context> context) {
        ...
        if (frame) {
            ...
            if (base::FeatureList::IsEnabled(features::kCaptureJSExecutionLocation)) {
                // Capture the source location of the JS execution if the flag
                // is enabled.
                source_location = CaptureSourceLocation();
            }
            frame->EvictFromBackForwardCache(mojom::blink::RendererEvictionReason::kJavaScriptExecution, std::move(source_location));
            if (base::FeatureList::IsEnabled(features::kBackForwardCacheDWCOnJavaScriptExecution)) {
...
void LocalFrame::RemoveBackForwardCacheEviction()
{
    ...
    DCHECK(RuntimeEnabledFeatures::BackForwardCacheEnabled());
    static_cast<LocalWindowProxyManager*>(GetWindowProxyManager())->SetAbortScriptExecution(nullptr);
...
void LocalFrame::SetTextDirection(base::i18n::TextDirection direction)
{
    ...
    case base::i18n::TextDirection::UNKNOWN_DIRECTION:
        editor.SetBaseWritingDirection(mojo_base::mojom::blink::TextDirection::UNKNOWN_DIRECTION);
        break;

    case base::i18n::TextDirection::LEFT_TO_RIGHT:
        editor.SetBaseWritingDirection(mojo_base::mojom::blink::TextDirection::LEFT_TO_RIGHT);
        break;

    case base::i18n::TextDirection::RIGHT_TO_LEFT:
        editor.SetBaseWritingDirection(mojo_base::mojom::blink::TextDirection::RIGHT_TO_LEFT);
        break;
...
void LocalFrame::SetInheritedEffectiveTouchAction(TouchAction touch_action)
{
    ...
    GetDocument()->GetStyleEngine().MarkAllElementsForStyleRecalc(
        StyleChangeReasonForTracing::Create(style_change_reason::kInheritedStyleChangeFromParentFrame));
}

bool LocalFrame::BubbleLogicalScrollInParentFrame(mojom::blink::ScrollDirection direction, ui::ScrollGranularity granularity)
{
    bool is_embedded_main_frame = IsMainFrame() && !IsOutermostMainFrame();
    if (is_embedded_main_frame || IsA<RemoteFrame>(Parent())) {
        GetLocalFrameHostRemote().BubbleLogicalScrollInParentFrame(direction, granularity);
        return false;
    } else if (auto* local_parent = DynamicTo<LocalFrame>(Parent())) {
        return local_parent->BubbleLogicalScrollFromChildFrame(direction, granularity, this);
    }
...
bool LocalFrame::BubbleLogicalScrollFromChildFrame(mojom::blink::ScrollDirection direction, ui::ScrollGranularity granularity, Frame* child)
{
    FrameOwner* owner = child->Owner();
    auto* owner_element = DynamicTo<HTMLFrameOwnerElement>(owner);
    DCHECK(owner_element);

    return GetEventHandler().BubblingScroll(direction, granularity, owner_element);
}

mojom::blink::SuddenTerminationDisablerType SuddenTerminationDisablerTypeForEventType(const AtomicString& event_type)
{
    ...
    if (event_type == event_type_names::kVisibilitychange) {
        return mojom::blink::SuddenTerminationDisablerType::kVisibilityChangeHandler;
    }
    NOTREACHED();
}

int NumberOfSuddenTerminationEventListeners(const EventTarget& event_target, const AtomicString& event_type)
{
    if (event_type != event_type_names::kVisibilitychange)
    ...
    if (auto* dom_window = event_target.ToLocalDOMWindow()) {
        ...
        // on the document.
        total_listeners_count += dom_window->document()->NumberOfEventListeners(event_type);
    } else {
        ...
        // on the window.
        total_listeners_count += node->GetDocument().domWindow()->NumberOfEventListeners(event_type);
    }
    return total_listeners_count;
}

void LocalFrame::UpdateSuddenTerminationStatus(bool added_listener, mojom::blink::SuddenTerminationDisablerType disabler_type)
{
    ...
    if (features::IsUnloadBlocklisted()) {
        ...
        if (disabler_type == mojom::blink::SuddenTerminationDisablerType::kUnloadHandler) {
            if (added_listener) {
                if (feature_handle_for_scheduler_) {
                    return;
                }
                feature_handle_for_scheduler_
                    = GetFrameScheduler()->RegisterFeature(SchedulingPolicy::Feature::kUnloadHandler, { SchedulingPolicy::DisableBackForwardCache() });
            } else {
                feature_handle_for_scheduler_.reset();
            }
        }
    }
    GetLocalFrameHostRemote().SuddenTerminationDisablerChanged(added_listener, disabler_type);
}

void LocalFrame::AddedSuddenTerminationDisablerListener(const EventTarget& event_target, const AtomicString& event_type)
{
    if (NumberOfSuddenTerminationEventListeners(event_target, event_type) == 1) {
        // The first handler of this type was added.
        UpdateSuddenTerminationStatus(true, SuddenTerminationDisablerTypeForEventType(event_type));
    }
}

void LocalFrame::RemovedSuddenTerminationDisablerListener(const EventTarget& event_target, const AtomicString& event_type)
{
    if (NumberOfSuddenTerminationEventListeners(event_target, event_type) == 0) {
        // The last handler of this type was removed.
        UpdateSuddenTerminationStatus(false, SuddenTerminationDisablerTypeForEventType(event_type));
    }
}
...
void LocalFrame::DidChangeBackgroundColor(SkColor4f background_color, bool color_adjust)
{
    DCHECK(!Tree().Parent());
    GetLocalFrameHostRemote().DidChangeBackgroundColor(background_color, color_adjust);
}
...
void LocalFrame::StartPrinting(const WebPrintParams& print_params, float maximum_shrink_ratio)
{
    ...
    if (!print_params_.use_paginated_layout) {
        ...
        if (LayoutView* layout_view = View()->GetLayoutView()) {
            auto size = PhysicalSize::FromSizeFRound(print_params_.default_page_description.size);
...
void LocalFrame::StartPrintingSubLocalFrame()
{
    ...
    if (const LayoutView* layout_view = View()->GetLayoutView()) {
        page_size = gfx::SizeF(layout_view->GetNonPrintingLayoutSize(kIncludeScrollbars));
    }
...
void LocalFrame::SetPrinting(bool printing, float maximum_shrink_ratio)
{
    ...
    ResourceCacheValidationSuppressor validation_suppressor(GetDocument()->Fetcher());

    GetDocument()->SetPrinting(printing ? Document::kPrinting : Document::kFinishingPrinting);
...
void LocalFrame::RestoreScrollOffsets()
{
    ...
    for (auto& entry : *saved_scroll_offsets_) {
        ...
        if (!scrollable_area)
            continue;
        scrollable_area->SetScrollOffsetUnconditionally(entry.value, mojom::blink::ScrollType::kProgrammatic);
    }
...
void LocalFrame::SetLayoutAndTextZoomFactors(float layout_zoom_factor, float text_zoom_factor)
{
    if (layout_zoom_factor_ == layout_zoom_factor && text_zoom_factor_ == text_zoom_factor) {
        return;
    }
    ...
    if (!GetDocument()->StandardizedBrowserZoomEnabled()) {
        ...
        for (Frame* child = Tree().FirstChild(); child; child = child->Tree().NextSibling()) {
            if (auto* child_local_frame = DynamicTo<LocalFrame>(child)) {
                child_local_frame->SetLayoutAndTextZoomFactors(layout_zoom_factor_, text_zoom_factor_);
            } else {
    ...
    document->GetStyleEngine().MarkAllElementsForStyleRecalc(StyleChangeReasonForTracing::Create(style_change_reason::kZoom));
    if (View())
        View()->SetNeedsLayout();
}

void LocalFrame::MediaQueryAffectingValueChangedForLocalSubtree(MediaValueChange value)
{
...
void LocalFrame::ViewportSegmentsChanged(const WebVector<gfx::Rect>& viewport_segments)
{
    if (!RuntimeEnabledFeatures::ViewportSegmentsEnabled(GetDocument()->GetExecutionContext())) {
        return;
    }
    ...
    if (Element* fullscreen = Fullscreen::FullscreenElementFrom(*GetDocument())) {
        GetDocument()->GetStyleEngine().MarkAllElementsForStyleRecalc(StyleChangeReasonForTracing::Create(style_change_reason::kFullscreen));
        CSSDefaultStyleSheets::Instance().RebuildFullscreenRuleSetIfMediaQueriesChanged(*fullscreen);
    }
...
void LocalFrame::UpdateViewportSegmentCSSEnvironmentVariables(const WebVector<gfx::Rect>& viewport_segments)
{
    DCHECK(RuntimeEnabledFeatures::ViewportSegmentsEnabled(GetDocument()->GetExecutionContext()));
    ...
    UpdateViewportSegmentCSSEnvironmentVariables(StyleEnvironmentVariables::GetRootInstance(), viewport_segments);

    if (Element* fullscreen = Fullscreen::FullscreenElementFrom(*GetDocument())) {
        // Fullscreen has its own document so we need to update its variables as
        // well.
        UpdateViewportSegmentCSSEnvironmentVariables(fullscreen->GetDocument().GetStyleEngine().EnsureEnvironmentVariables(), viewport_segments);
    }
}

void LocalFrame::UpdateViewportSegmentCSSEnvironmentVariables(StyleEnvironmentVariables& vars, const WebVector<gfx::Rect>& viewport_segments)
{
    ...
    if (viewport_segments.size() >= 2) {
        ...
        unsigned x_index = 0;
        unsigned y_index = 0;
        SetViewportSegmentVariablesForRect(vars, viewport_segments[0], x_index, y_index, context);
        for (size_t i = 1; i < viewport_segments.size(); i++) {
            if (viewport_segments[i].y() == current_y_position) {
                x_index++;
                SetViewportSegmentVariablesForRect(vars, viewport_segments[i], x_index, y_index, context);
            } else {
...
void LocalFrame::OverrideDevicePostureForEmulation(mojom::blink::DevicePostureType device_posture_param)
{
...
void LocalFrame::TextSelectionChanged(const WTF::String& selection_text, uint32_t offset, const gfx::Range& range) const
{
    GetLocalFrameHostRemote().TextSelectionChanged(selection_text, offset, range);
}

PositionWithAffinity LocalFrame::PositionForPoint(const PhysicalOffset& frame_point)
{
...
Document* LocalFrame::DocumentAtPoint(const PhysicalOffset& point_in_root_frame)
{
    if (!View())
        return nullptr;
...
LocalFrame::LocalFrame(LocalFrameClient* client, Page& page, FrameOwner* owner, Frame* parent, Frame* previous_sibling, FrameInsertType insert_type,
    const LocalFrameToken& frame_token, WindowAgentFactory* inheriting_agent_factory, InterfaceRegistry* interface_registry,
    mojo::PendingRemote<mojom::blink::BrowserInterfaceBroker> interface_broker, const base::TickClock* clock)
    : Frame(client, page, owner, parent, previous_sibling, insert_type, frame_token, client->GetDevToolsFrameToken(),
        MakeGarbageCollected<LocalWindowProxyManager>(page.GetAgentGroupScheduler().Isolate(), *this), inheriting_agent_factory)
    , frame_scheduler_(page.GetPageScheduler()->CreateFrameScheduler(
          this, IsInFencedFrameTree(), IsMainFrame() ? FrameScheduler::FrameType::kMainFrame : FrameScheduler::FrameType::kSubframe))
    , loader_(this)
    ...
    , inspector_task_runner_(InspectorTaskRunner::Create(GetTaskRunner(TaskType::kInternalInspector)))
    , interface_registry_(interface_registry ? interface_registry : InterfaceRegistry::GetEmptyInterfaceRegistry())
    , v8_local_compile_hints_producer_(MakeGarbageCollected<v8_compile_hints::V8LocalCompileHintsProducer>(this))
    ,
...
{
    auto frame_tracking_result = GetLocalFramesMap().insert(FrameToken::Hasher()(GetFrameToken()), this);
    ...
    if (interface_broker.is_valid()) { // This may be invalid in unit tests.
        browser_interface_broker_proxy_.Bind(std::move(interface_broker), page.GetAgentGroupScheduler().DefaultTaskRunner());
    }
    ...
    if (IsOutermostMainFrame()) {
        intersection_state_.occlusion_state = mojom::blink::FrameOcclusionState::kGuaranteedNotOccluded;
    }

    DCHECK(ad_tracker_ ? RuntimeEnabledFeatures::AdTaggingEnabled() : !RuntimeEnabledFeatures::AdTaggingEnabled());

    // See SubresourceFilterAgent::Initialize for why we don't set this here for
    // fenced frames.
    is_frame_created_by_ad_script_ = !IsMainFrame() && ad_tracker_
...
scoped_refptr<base::SingleThreadTaskRunner> LocalFrame::GetTaskRunner(TaskType type)
{
    DCHECK(IsMainThread());
...
static bool CanAccessAncestor(const SecurityOrigin& active_security_origin, const Frame* target_frame)
{
    ...
    for (const Frame* ancestor_frame = target_frame; ancestor_frame; ancestor_frame = ancestor_frame->Tree().Parent()) {
        const SecurityOrigin* ancestor_security_origin = ancestor_frame->GetSecurityContext()->GetSecurityOrigin();
...
bool LocalFrame::CanNavigate(const Frame& target_frame, const KURL& destination_url)
{
    ...
    if (Opener() == target_frame && !HasTransientUserActivation(this)
        && !target_frame.GetSecurityContext()->GetSecurityOrigin()->CanAccess(SecurityOrigin::Create(destination_url).get())) {
        UseCounter::Count(GetDocument(), WebFeature::kOpenerNavigationWithoutGesture);
    }
    ...
    if (destination_url.ProtocolIsJavaScript()
        && (!GetSecurityContext()->GetSecurityOrigin()->CanAccess(target_frame.GetSecurityContext()->GetSecurityOrigin()))) {
    ...
    if (GetSecurityContext()->IsSandboxed(network::mojom::blink::WebSandboxFlags::kNavigation)) {
        ...
        const bool target_is_outermost_frame = target_frame.IsMainFrame() && !target_frame.GetPage()->IsMainFrameFencedFrameRoot();

        if (!target_frame.Tree().IsDescendantOf(this) && !target_is_outermost_frame) {
            PrintNavigationErrorMessage(target_frame,
                IsInFencedFrameTree() ? "The frame attempting navigation is in a fenced frame tree, "
        ...
        if (target_is_outermost_frame && target_frame != Tree().Top()
            && GetSecurityContext()->IsSandboxed(network::mojom::blink::WebSandboxFlags::kPropagatesToAuxiliaryBrowsingContexts)
            && (GetSecurityContext()->IsSandboxed(network::mojom::blink::WebSandboxFlags::kPopups) || target_frame.Opener() != this)) {
            PrintNavigationErrorMessage(target_frame,
        ...
        if ((target_frame == Tree().Top()) && !target_frame.GetPage()->IsMainFrameFencedFrameRoot()) {
            if (GetSecurityContext()->IsSandboxed(network::mojom::blink::WebSandboxFlags::kTopNavigation)
                && GetSecurityContext()->IsSandboxed(network::mojom::blink::WebSandboxFlags::kTopNavigationByUserActivation)) {
                PrintNavigationErrorMessage(target_frame,
            ...
            if (GetSecurityContext()->IsSandboxed(network::mojom::blink::WebSandboxFlags::kTopNavigation)
                ...
                if (!HasTransientUserActivation(this)) {
                    GetLocalFrameHostRemote().DidBlockNavigation(
                        destination_url, GetDocument()->Url(), mojom::blink::NavigationBlockedReason::kRedirectWithNoUserGestureSandbox);
                    PrintNavigationErrorMessage(target_frame,
                        "The frame attempting navigation of the top-level window is "
            ...
            if (!DomWindow()->GetExecutionContext()->GetPolicyContainer()->GetPolicies().can_navigate_top_without_user_gesture && !HasStickyUserActivation()) {
                String message = "The frame attempting to navigate the top-level window is "
    ...
    if (target_frame == Tree().Top()) {
        ...
        if (HasStickyUserActivation() || target_frame.GetSecurityContext()->GetSecurityOrigin()->CanAccess(SecurityOrigin::Create(destination_url).get())) {
            return true;
        }

        String target_domain
            = network_utils::GetDomainAndRegistry(target_frame.GetSecurityContext()->GetSecurityOrigin()->Domain(), network_utils::kIncludePrivateRegistries);
        String destination_domain = network_utils::GetDomainAndRegistry(destination_url.Host(), network_utils::kIncludePrivateRegistries);
        if (!target_domain.empty() && !destination_domain.empty() && target_domain == destination_domain
            && (target_frame.GetSecurityContext()->GetSecurityOrigin()->Protocol() == destination_url.Protocol())) {
            return true;
        }
        ...
        GetLocalFrameHostRemote().DidBlockNavigation(destination_url, GetDocument()->Url(), mojom::blink::NavigationBlockedReason::kRedirectWithNoUserGesture);
...
void LocalFrame::MaybeStartOutermostMainFrameNavigation(const Vector<KURL>& urls) const
{
    TRACE_EVENT0("navigation", "LocalFrame::MaybeStartOutermostMainFrameNavigation");
    mojo_handler_->NonAssociatedLocalFrameHostRemote().MaybeStartOutermostMainFrameNavigation(urls);
}
...
ContentCaptureManager* LocalFrame::GetOrResetContentCaptureManager()
{
    ...
    if (Client()->GetWebContentCaptureClient()) {
        if (!content_capture_manager_) {
            content_capture_manager_ = MakeGarbageCollected<ContentCaptureManager>(*this);
        }
...
AssociatedInterfaceProvider* LocalFrame::GetRemoteNavigationAssociatedInterfaces()
{
    DCHECK(Client());
...
scoped_refptr<network::SharedURLLoaderFactory> LocalFrame::GetURLLoaderFactory()
{
...
scoped_refptr<WebBackgroundResourceFetchAssets> LocalFrame::MaybeGetBackgroundResourceFetchAssets()
{
...
void LocalFrame::WasHidden()
{
    ...
    if (intersection_state_.occlusion_state == mojom::blink::FrameOcclusionState::kPossiblyOccluded) {
        return;
    }
...
void LocalFrame::SetViewportIntersectionFromParent(const mojom::blink::ViewportIntersectionState& intersection_state)
{
    DCHECK(IsLocalRoot());
    DCHECK(!IsOutermostMainFrame());
    // Notify the render frame observers when the main frame intersection or the
    // transform changes.
    if (intersection_state_.main_frame_intersection != intersection_state.main_frame_intersection
        || intersection_state_.main_frame_transform != intersection_state.main_frame_transform) {
        gfx::Rect rect = intersection_state.main_frame_transform.MapRect(intersection_state.main_frame_intersection);
    ...
    if (intersection_state_.viewport_intersection != intersection_state.viewport_intersection
        || intersection_state_.outermost_main_frame_size != intersection_state.outermost_main_frame_size) {
    ...
    bool needs_update = intersection_state_.viewport_intersection != intersection_state.viewport_intersection
...
{
    LocalFrame& local_root = LocalFrameRoot();
    return local_root.IsOutermostMainFrame() ? local_root.View()->LayoutViewport()->VisibleContentRect().size()
...
{
    LocalFrame& local_root = LocalFrameRoot();
    return local_root.IsOutermostMainFrame() ? gfx::ToFlooredPoint(local_root.View()->LayoutViewport()->ScrollPosition())
                                             : local_root.intersection_state_.outermost_main_frame_scroll_position;
}
...
void LocalFrame::SetOpener(Frame* opener_frame)
{
    ...
    if (web_frame && Opener() != opener_frame) {
        GetLocalFrameHostRemote().DidChangeOpener(
            opener_frame ? std::optional<blink::LocalFrameToken>(opener_frame->GetFrameToken().GetAs<LocalFrameToken>()) : std::nullopt);
    }
...
bool LocalFrame::NeedsOcclusionTracking() const
{
    if (Document* document = GetDocument()) {
        if (IntersectionObserverController* controller = document->GetIntersectionObserverController()) {
...
void LocalFrame::ForceSynchronousDocumentInstall(const AtomicString& mime_type, const SegmentedBuffer& data)
{
    ...
    Document* document = DomWindow()->InstallNewDocument(
        DocumentInit::Create().WithWindow(DomWindow(), nullptr).WithTypeFrom(mime_type).ForPrerendering(GetPage()->IsPrerendering()));
    DCHECK_EQ(document, GetDocument());
    DocumentParser* parser = document->OpenForNavigation(kForceSynchronousParsing, mime_type, AtomicString("UTF-8"));
    ...
    if (RuntimeEnabledFeatures::DocumentInstallChunkingEnabled()) {
        ...
        for (const auto& segment : data) {
            ...
            if (current_chunk.size() > kMaxDocumentChunkSize) {
                parser->AppendBytes(base::span<const uint8_t>((const uint8_t*)current_chunk.data(), current_chunk.size()));
                current_chunk.clear();
            }
        }
        parser->AppendBytes(base::span<const uint8_t>((const uint8_t*)current_chunk.data(), current_chunk.size()));
        ...
        for (const auto& segment : data) {
            parser->AppendBytes(base::span<const uint8_t>((const uint8_t*)segment.data(), segment.size()));
        }
    }
...
void LocalFrame::SetAdEvidence(const FrameAdEvidence& ad_evidence)
{
    ...
    DCHECK(!is_frame_created_by_ad_script_ || ad_evidence.created_by_ad_script() == blink::mojom::FrameCreationStackEvidence::kCreatedByAdScript);
    is_frame_created_by_ad_script_ = ad_evidence.created_by_ad_script() == blink::mojom::FrameCreationStackEvidence::kCreatedByAdScript;
    ...
    if (ad_evidence_.has_value()) {
        ...
        // by definition.
        DCHECK_LE(ad_evidence_->most_restrictive_filter_list_result(), ad_evidence.most_restrictive_filter_list_result());
    }
...
{
    return ad_tracker_ && ad_tracker_->IsAdScriptInStack(AdTracker::StackType::kBottomAndTop);
}
...
void LocalFrame::PauseSubresourceLoading(mojo::PendingReceiver<mojom::blink::PauseSubresourceLoadingHandle> receiver)
{
    auto handle = GetFrameScheduler()->GetPauseSubresourceLoadingHandle();
    if (!handle)
        return;
    pause_handle_receivers_.Add(std::move(handle), std::move(receiver), GetTaskRunner(blink::TaskType::kInternalDefault));
}
...
void LocalFrame::ReinstateSmoothScrollSequence(SmoothScrollSequencer* sequencer)
{
    if (!IsLocalRoot()) {
...
void LocalFrame::UpdateBackForwardCacheDisablingFeatures(BlockingDetails details)
{
    auto mojom_details = ConvertFeatureAndLocationToMojomStruct(*details.non_sticky_features_and_js_locations, *details.sticky_features_and_js_locations);
    GetBackForwardCacheControllerHostRemote().DidChangeBackForwardCacheDisablingFeatures(std::move(mojom_details));
}
...
BlockingDetailsList LocalFrame::ConvertFeatureAndLocationToMojomStruct(
    const BFCacheBlockingFeatureAndLocations& non_sticky, const BFCacheBlockingFeatureAndLocations& sticky)
{
...
void LocalFrame::OnTaskCompleted(base::TimeTicks start_time, base::TimeTicks end_time)
{
...
// static
void LocalFrame::NotifyUserActivation(LocalFrame* frame, mojom::blink::UserActivationNotificationType notification_type, bool need_browser_verification)
{
    if (frame) {
...
// static
bool LocalFrame::ConsumeTransientUserActivation(LocalFrame* frame, UserActivationUpdateSource update_source)
{
    return frame ? frame->ConsumeTransientUserActivation(update_source) : false;
}

void LocalFrame::NotifyUserActivation(mojom::blink::UserActivationNotificationType notification_type, bool need_browser_verification)
{
    mojom::blink::UserActivationUpdateType update_type = need_browser_verification
        ? mojom::blink::UserActivationUpdateType::kNotifyActivationPendingBrowserVerification
        : mojom::blink::UserActivationUpdateType::kNotifyActivation;

    GetLocalFrameHostRemote().UpdateUserActivationState(update_type, notification_type);
    Client()->NotifyUserActivation();
    NotifyUserActivationInFrameTree(notification_type);
}

bool LocalFrame::ConsumeTransientUserActivation(UserActivationUpdateSource update_source)
{
    if (update_source == UserActivationUpdateSource::kRenderer) {
        GetLocalFrameHostRemote().UpdateUserActivationState(
            mojom::blink::UserActivationUpdateType::kConsumeTransientActivation, mojom::blink::UserActivationNotificationType::kNone);
    }
...
void LocalFrame::SetHadUserInteraction(bool had_user_interaction)
{
    ...
    DomWindow()->closewatcher_stack()->SetHadUserInteraction(had_user_interaction);
...
public:
    ...
    SkColor GetColorForTesting() const
    {
        return color_;
    }

private:
    void PaintFrameOverlay(const FrameOverlay& frame_overlay, GraphicsContext& graphics_context, const gfx::Size&) const override
    {
        const auto* view = frame_->View();
        DCHECK(view);
        if (view->Width() == 0 || view->Height() == 0)
            return;
        ScopedPaintChunkProperties properties(graphics_context.GetPaintController(), view->GetLayoutView()->FirstFragment().LocalBorderBoxProperties(),
            frame_overlay, DisplayItem::kFrameOverlay);
        if (DrawingRecorder::UseCachedDrawingIfPossible(graphics_context, frame_overlay, DisplayItem::kFrameOverlay))
            return;
        DrawingRecorder recorder(graphics_context, frame_overlay, DisplayItem::kFrameOverlay, gfx::Rect(view->Size()));
        gfx::RectF rect(0, 0, view->Width(), view->Height());
        graphics_context.FillRect(
            rect, Color::FromSkColor(color_), PaintAutoDarkMode(view->GetLayoutView()->StyleRef(), DarkModeFilter::ElementRole::kBackground));
    }
...
void LocalFrame::SetReducedAcceptLanguage(const AtomicString& reduced_accept_language)
{
    reduced_accept_language_ = reduced_accept_language;
}

template <> struct DowncastTraits<FrameColorOverlay> {
...
std::optional<SkColor> LocalFrame::GetFrameOverlayColorForTesting() const
{
    if (!frame_color_overlay_)
        return std::nullopt;
    return DynamicTo<FrameColorOverlay>(frame_color_overlay_->GetDelegate())->GetColorForTesting();
}
...
void LocalFrame::SetFrameColorOverlay(SkColor color)
{
    ...
    frame_color_overlay_ = MakeGarbageCollected<FrameOverlay>(this, std::make_unique<FrameColorOverlay>(this, color));
}
...
void LocalFrame::OnPageLifecycleStateUpdated()
{
    ...
    mojom::blink::FrameLifecycleState frame_lifecycle_state = mojom::blink::FrameLifecycleState::kRunning;
    if (GetPage()->Paused()) {
...
{
    CHECK(IsProvisional());
    if (auto* previous_main_frame = GetPage()->GetPreviousMainFrameForLocalSwap()) {
...
bool LocalFrame::SwapIn()
{
    ...
    Frame* previous_local_main_frame = GetPage()->GetPreviousMainFrameForLocalSwap();
    if (previous_local_main_frame && !previous_local_main_frame->IsDetached()) {
        ...
        CHECK(!DynamicTo<RemoteFrame>(provisional_owner_frame)->IsRemoteFrameHostRemoteBound());
    ...
    if (!IsLocalRoot()) {
        if (auto* local_provisional_owner = DynamicTo<LocalFrame>(provisional_owner_frame)) {
            ...
            bool swap_result = client->SwapIn(WebFrame::FromCoreFrame(provisional_owner_frame));
...
void LocalFrame::LoadJavaScriptURL(const KURL& url)
{
    ...
    if (SchemeRegistry::ShouldTreatURLSchemeAsNotAllowingJavascriptURLs(
            GetSecurityContext()->GetSecurityOrigin()->GetOriginOrPrecursorOriginIfOpaque()->Protocol())) {
        return;
    }

    // TODO(mustaq): This is called only through the user typing a javascript URL
    // into the omnibox.  See https://crbug.com/1082900
    NotifyUserActivation(mojom::blink::UserActivationNotificationType::kInteraction, false);
    auto* window = DomWindow();
    window->GetScriptController().ExecuteJavaScriptURL(url, network::mojom::CSPDisposition::DO_NOT_CHECK, &DOMWrapperWorld::MainWorld(window->GetIsolate()));
}

void LocalFrame::RequestExecuteScript(int32_t world_id, base::span<const WebScriptSource> sources, mojom::blink::UserActivationOption user_gesture,
    ...
    BackForwardCacheAware back_forward_cache_aware, mojom::blink::WantResultOption want_result_option, mojom::blink::PromiseResultOption promise_behavior)
{
    ...
    if (world_id == DOMWrapperWorld::kMainWorldId) {
        ...
        execute_script_policy = ExecuteScriptPolicy::kExecuteScriptWhenScriptsDisabled;
    }

    if (back_forward_cache_aware == BackForwardCacheAware::kPossiblyDisallow) {
        GetFrameScheduler()->RegisterStickyFeature(SchedulingPolicy::Feature::kInjectedJavascript, { SchedulingPolicy::DisableBackForwardCache() });
    }
    ...
    CHECK(script_state);
    PausableScriptExecutor::CreateAndRun(script_state, std::move(script_sources), execute_script_policy, user_gesture, evaluation_timing, blocking_option,
        want_result_option, promise_behavior, std::move(callback));
}

void LocalFrame::SetEvictCachedSessionStorageOnFreezeOrUnload()
{
    DCHECK(RuntimeEnabledFeatures::Prerender2Enabled(GetDocument()->GetExecutionContext()));
...
{
    if (paused_ || frozen_) {
        if (GetPage()->GetPageScheduler()->IsInBackForwardCache() && IsInflightNetworkRequestBackForwardCacheSupportEnabled()) {
...
void LocalFrame::DidFreeze()
{
    ...
    if (auto* document_resource_coordinator = GetDocument()->GetResourceCoordinator()) {
        document_resource_coordinator->SetLifecycleState(performance_manager::mojom::LifecycleState::kFrozen);
    }
...
void LocalFrame::DidResume()
{
    ...
    if (auto* document_resource_coordinator = GetDocument()->GetResourceCoordinator()) {
        document_resource_coordinator->SetLifecycleState(performance_manager::mojom::LifecycleState::kRunning);
    }
...
void LocalFrame::CountUseIfFeatureWouldBeBlockedByPermissionsPolicy(mojom::WebFeature blocked_cross_origin, mojom::WebFeature blocked_same_origin)
{
    // Get the origin of the top-level document
    const SecurityOrigin* topOrigin = Tree().Top().GetSecurityContext()->GetSecurityOrigin();

    // Check if this frame is same-origin with the top-level or is in
    // a fenced frame tree.
    if (!GetSecurityContext()->GetSecurityOrigin()->CanAccess(topOrigin) || IsInFencedFrameTree()) {
...
void LocalFrame::UpdateFaviconURL()
{
    ...
    for (const auto& icon_url : icon_urls) {
        urls.push_back(mojom::blink::FaviconURL::New(icon_url.icon_url_, icon_url.icon_type_, icon_url.sizes_, icon_url.is_default_icon_));
    }
...
void LocalFrame::SetIsCapturingMediaCallback(IsCapturingMediaCallback callback)
{
...
{
    return is_capturing_media_callback_ ? is_capturing_media_callback_.Run() : false;
}
...
void LocalFrame::EvictFromBackForwardCache(mojom::blink::RendererEvictionReason reason, std::unique_ptr<SourceLocation> source_location)
{
    if (!GetPage()->GetPageScheduler()->IsInBackForwardCache())
        return;
    ...
    if (source_location) {
        source = mojom::blink::ScriptSourceLocation::New(source_location->Url() ? KURL(source_location->Url()) : KURL(),
            source_location->Function() ? source_location->Function() : "", source_location->LineNumber(), source_location->ColumnNumber());
    }
    GetBackForwardCacheControllerHostRemote().EvictFromBackForwardCache(std::move(reason), std::move(source));
}

void LocalFrame::DidBufferLoadWhileInBackForwardCache(bool update_process_wide_count, size_t num_bytes)
{
    DomWindow()->DidBufferLoadWhileInBackForwardCache(update_process_wide_count, num_bytes);
}
...
void LocalFrame::SetScaleFactor(float scale_factor)
{
    ...
    const PageScaleConstraints& constraints = GetPage()->GetPageScaleConstraintsSet().FinalConstraints();
...
#if !BUILDFLAG(IS_ANDROID)
void LocalFrame::UpdateWindowControlsOverlay(const gfx::Rect& bounding_rect_in_dips)
{
    ...
    const float window_to_viewport_factor = GetPage()->GetChromeClient().WindowToViewportScalar(&local_frame_root, 1.0f);
    const float zoom_factor = local_frame_root.LayoutZoomFactor();
    const float scale_factor = zoom_factor / window_to_viewport_factor;
    gfx::Rect window_controls_overlay_rect = gfx::ScaleToEnclosingRect(bounding_rect_in_dips, 1.0f / scale_factor);

    bool fire_event = (window_controls_overlay_rect != window_controls_overlay_rect_);
    ...
    if (fire_event && window_controls_overlay_changed_delegate_) {
        window_controls_overlay_changed_delegate_->WindowControlsOverlayChanged(window_controls_overlay_rect_);
    }
}

void LocalFrame::RegisterWindowControlsOverlayChangedDelegate(WindowControlsOverlayChangedDelegate* delegate)
{
    window_controls_overlay_changed_delegate_ = delegate;
}
#endif

HitTestResult LocalFrame::HitTestResultForVisualViewportPos(const gfx::Point& pos_in_viewport)
{
    gfx::Point root_frame_point(GetPage()->GetVisualViewport().ViewportToRootFrame(pos_in_viewport));
...
WebPrescientNetworking* LocalFrame::PrescientNetworking()
{
    if (!prescient_networking_) {
        ...
        if (!web_local_frame)
            return nullptr;
        prescient_networking_ = web_local_frame->Client()->CreatePrescientNetworking();
    }
    return prescient_networking_.get();
}

void LocalFrame::SetPrescientNetworkingForTesting(std::unique_ptr<WebPrescientNetworking> prescient_networking)
{
...
mojom::blink::BackForwardCacheControllerHost& LocalFrame::GetBackForwardCacheControllerHostRemote()
{
    return mojo_handler_->BackForwardCacheControllerHostRemote();
}

void LocalFrame::NotifyUserActivation(mojom::blink::UserActivationNotificationType notification_type)
{
    NotifyUserActivation(notification_type, false);
}

void LocalFrame::RegisterVirtualKeyboardOverlayChangedObserver(VirtualKeyboardOverlayChangedObserver* observer)
{
    virtual_keyboard_overlay_changed_observers_.insert(observer);
}

void LocalFrame::NotifyVirtualKeyboardOverlayRectObservers(const gfx::Rect& rect) const
{
    HeapVector<Member<VirtualKeyboardOverlayChangedObserver>, 32> observers(virtual_keyboard_overlay_changed_observers_);
...
void LocalFrame::CopyImageAtViewportPoint(const gfx::Point& viewport_point)
{
    ...
    if (!IsA<HTMLCanvasElement>(result.InnerNodeOrImageMapImage()) && result.AbsoluteImageURL().IsEmpty()) {
...
{
    gfx::Point viewport_position = GetWidgetForLocalRoot()->DIPsToRoundedBlinkSpace(window_point);
    Node* node = HitTestResultForVisualViewportPos(viewport_position).InnerNodeOrImageMapImage();
...
void LocalFrame::MediaPlayerActionAtViewportPoint(const gfx::Point& viewport_position, const mojom::blink::MediaPlayerActionType type, bool enable)
{
    ...
    case mojom::blink::MediaPlayerActionType::kSaveVideoFrameAs:
        if (auto* video = DynamicTo<HTMLVideoElement>(media_element); video) {
            ...
            ImageEncodingMimeType encoding_mime_type = ImageEncoderUtils::ToEncodingMimeType("image/png", ImageEncoderUtils::kEncodeReasonToDataURL);
            String data_url = data_buffer->ToDataURL(encoding_mime_type, /*quality=*/0);

            auto params = mojom::blink::DownloadURLParams::New();
            params->is_context_menu_save = true;
            // Suggested name always starts with "videoframe_", plus the timestamp
            // of the video frame in milliseconds.
            auto timestamp_ms = base::saturated_cast<uint32_t>(media_element->currentTime() * base::Time::kMillisecondsPerSecond);
    ...
    case mojom::blink::MediaPlayerActionType::kPictureInPicture:
        if (auto* video = DynamicTo<HTMLVideoElement>(media_element); video) {
            if (enable) {
                PictureInPictureController::From(node->GetDocument()).EnterPictureInPicture(video, /*promise=*/nullptr);
            } else {
                PictureInPictureController::From(node->GetDocument()).ExitPictureInPicture(video, nullptr);
            }
        }
        break;
    }
...
void LocalFrame::RequestVideoFrameAtWithBoundsHint(
    const gfx::Point& viewport_position, const gfx::Size& max_size, int max_area, base::OnceCallback<void(const SkBitmap&, const gfx::Rect&)> callback)
{
    HitTestResult result = HitTestResultForVisualViewportPos(viewport_position);
    Node* node = result.InnerNode();
    ...
    if ((size.width() > max_size.width() || size.height() > max_size.height()) && size.GetArea() > max_area) {
        double scale = std::min(static_cast<double>(max_size.width()) / size.width(), static_cast<double>(max_size.height()) / size.height());
    ...
    auto image = video->CreateStaticBitmapImage(/*allow_accelerated_images=*/true, size);
    if (!image) {
        std::move(callback).Run(SkBitmap(), gfx::Rect());
        return;
    }
    ...
    if (bitmap.colorType() == kN32_SkColorType) {
        ...
        if (converted_bitmap.tryAllocPixels(info)) {
            bitmap.readPixels(info, converted_bitmap.getPixels(), converted_bitmap.rowBytes(), 0, 0);
        }
    }
...
void LocalFrame::DownloadURL(const ResourceRequest& request, network::mojom::blink::RedirectMode cross_origin_redirect_behavior)
{
    mojo::PendingRemote<mojom::blink::BlobURLToken> blob_url_token;
    if (request.Url().ProtocolIs("blob")) {
        DomWindow()->GetPublicURLManager().Resolve(request.Url(), blob_url_token.InitWithNewPipeAndPassReceiver());
    }

    DownloadURL(request, cross_origin_redirect_behavior, std::move(blob_url_token));
}
...
void LocalFrame::AdvanceFocusForIME(mojom::blink::FocusType focus_type)
{
    ...
    Element* next_element = GetPage()->GetFocusController().NextFocusableElementForImeAndAutofill(element, focus_type);
    if (!next_element)
        return;
...
void LocalFrame::PostMessageEvent(
    const std::optional<RemoteFrameToken>& source_frame_token, const String& source_origin, const String& target_origin, BlinkTransferableMessage message)
{
    ...
    if (GetDocument()) {
        ports = MessagePort::EntanglePorts(*GetDocument()->GetExecutionContext(), std::move(message.ports));
    }
    ...
    if (message.user_activation) {
        user_activation = MakeGarbageCollected<UserActivation>(message.user_activation->has_been_active, message.user_activation->was_active);
    }

    message_event->initMessageEvent(event_type_names::kMessage, false, false, std::move(message.message), source_origin, "" /*lastEventId*/, window, ports,
    ...
    DomWindow()->DispatchMessageEventWithOriginCheck(
        target_security_origin.get(), message_event, std::make_unique<SourceLocation>(String(), String(), 0, 0, nullptr), message.sender_agent_cluster_id);
}
...
String CreateMarkupInRect(LocalFrame* frame, const gfx::Point& start_point, const gfx::Point& end_point)
{
    VisiblePosition start_visible_position = CreateVisiblePosition(PositionForContentsPointRespectingEditingBoundary(start_point, frame));
    VisiblePosition end_visible_position = CreateVisiblePosition(PositionForContentsPointRespectingEditingBoundary(end_point, frame));
    ...
    const CreateMarkupOptions create_markup_options
        = CreateMarkupOptions::Builder().SetShouldAnnotateForInterchange(true).SetShouldResolveURLs(kResolveNonLocalURLs).Build();
...
void LocalFrame::ExtractSmartClipDataInternal(const gfx::Rect& rect_in_viewport, String& clip_text, String& clip_html, gfx::Rect& clip_rect)
{
    ...
    gfx::Point end_point(rect_in_viewport.x() + rect_in_viewport.width(), rect_in_viewport.y() + rect_in_viewport.height());
    clip_html = CreateMarkupInRect(this, View()->ViewportToFrame(start_point), View()->ViewportToFrame(end_point));
}
...
void LocalFrame::BindTextFragmentReceiver(mojo::PendingReceiver<mojom::blink::TextFragmentReceiver> receiver)
{
    if (IsDetached())
        return;
...
void LocalFrame::WriteIntoTrace(perfetto::TracedValue ctx) const
{
    ...
    dict.Add("is_cross_origin_to_outermost_main_frame", IsCrossOriginToOutermostMainFrame());
}

mojo::PendingRemote<mojom::blink::BlobURLStore> LocalFrame::GetBlobUrlStorePendingRemote()
{
    mojo::PendingRemote<mojom::blink::BlobURLStore> pending_remote;
    GetBrowserInterfaceBroker().GetInterface(pending_remote.InitWithNewPipeAndPassReceiver());
...
void LocalFrame::SetTitlebarAreaDocumentStyleEnvironmentVariables() const
{
    ...
    DocumentStyleEnvironmentVariables& vars = GetDocument()->GetStyleEngine().EnsureEnvironmentVariables();
    vars.SetVariable(UADefinedVariable::kTitlebarAreaX, StyleEnvironmentVariables::FormatPx(window_controls_overlay_rect_.x()));
    vars.SetVariable(UADefinedVariable::kTitlebarAreaY, StyleEnvironmentVariables::FormatPx(window_controls_overlay_rect_.y()));
    vars.SetVariable(UADefinedVariable::kTitlebarAreaWidth, StyleEnvironmentVariables::FormatPx(window_controls_overlay_rect_.width()));
    vars.SetVariable(UADefinedVariable::kTitlebarAreaHeight, StyleEnvironmentVariables::FormatPx(window_controls_overlay_rect_.height()));
}
...
void LocalFrame::SetNotRestoredReasons(mojom::blink::BackForwardCacheNotRestoredReasonsPtr not_restored_reasons)
{
...
const mojom::blink::BackForwardCacheNotRestoredReasonsPtr& LocalFrame::GetNotRestoredReasons()
{
...
void LocalFrame::SetNavigationConfidence(double randomized_trigger_rate, mojom::blink::ConfidenceLevel confidence)
{
    DCHECK(IsOutermostMainFrame());
    loader_.GetDocumentLoader()->GetTiming().SetRandomizedConfidence(std::make_pair(randomized_trigger_rate, confidence));
}
...
{
    for (auto& client : scroll_snapshot_clients_) {
        if (AnchorPositionScrollData* scroll_data = DynamicTo<AnchorPositionScrollData>(client.Get())) {
...
{
    AnchorPositionVisibilityObserver::UpdateForChainedAnchorVisibility(scroll_snapshot_clients_);
}

bool LocalFrame::IsSameOrigin()
{
    const SecurityOrigin* security_origin = GetSecurityContext()->GetSecurityOrigin();
    const SecurityOrigin* top_security_origin = Tree().Top().GetSecurityContext()->GetSecurityOrigin();
...
bool LocalFrame::ImagesEnabled()
{
    ...
    bool allow_image_content_setting = loader_.GetDocumentLoader()->GetContentSettings()->allow_image;
...
bool LocalFrame::ScriptEnabled()
{
    ...
    bool allow_script_content_setting = loader_.GetDocumentLoader()->GetContentSettings()->allow_script;
...
mojo::PendingRemote<mojom::blink::NavigationStateKeepAliveHandle> LocalFrame::IssueKeepAliveHandle()
{
    mojo::PendingRemote<mojom::blink::NavigationStateKeepAliveHandle> keep_alive_remote;
    GetLocalFrameHostRemote().IssueKeepAliveHandle(keep_alive_remote.InitWithNewPipeAndPassReceiver());
...
void LocalFrame::EnsureLinkPreviewTriggererInitialized()
{
    ...
    link_preview_triggerer_ = web_local_frame->Client()->CreateLinkPreviewTriggerer();
    is_link_preivew_triggerer_initialized_ = true;
}

void LocalFrame::SetLinkPreviewTriggererForTesting(std::unique_ptr<WebLinkPreviewTriggerer> trigger)
{
    link_preview_triggerer_ = std::move(trigger);
    is_link_preivew_triggerer_initialized_ = true;
}

void LocalFrame::AllowStorageAccessAndNotify(blink::WebContentSettingsClient::StorageType storage_type, base::OnceCallback<void(bool)> callback)
{
    mojom::blink::StorageTypeAccessed mojo_storage_type = ToMojoStorageType(storage_type);
    auto wrapped_callback = WTF::BindOnce(&LocalFrame::OnStorageAccessCallback, WrapWeakPersistent(this), std::move(callback), mojo_storage_type);
    if (WebContentSettingsClient* content_settings_client = GetContentSettingsClient()) {
        content_settings_client->AllowStorageAccess(storage_type, std::move(wrapped_callback));
    } else {
        std::move(wrapped_callback).Run(true);
    }
}

bool LocalFrame::AllowStorageAccessSyncAndNotify(blink::WebContentSettingsClient::StorageType storage_type)
{
    bool allowed = true;
    if (WebContentSettingsClient* content_settings_client = GetContentSettingsClient()) {
        allowed = content_settings_client->AllowStorageAccessSync(storage_type);
    }
    GetLocalFrameHostRemote().NotifyStorageAccessed(ToMojoStorageType(storage_type), !allowed);
    return allowed;
}

void LocalFrame::OnStorageAccessCallback(base::OnceCallback<void(bool)> callback, mojom::blink::StorageTypeAccessed storage_type, bool is_allowed)
{
