...
void FrameViewAutoSizeInfo::ConfigureAutoSizeMode(const gfx::Size& min_size, const gfx::Size& max_size)
{
...
bool FrameViewAutoSizeInfo::AutoSizeIfNeeded()
{
    ...
    if (new_size.width() > max_auto_size_.width()) {
        new_size.Enlarge(0, layout_viewport->HypotheticalScrollbarThickness(kHorizontalScrollbar));
        ...
        new_size.Enlarge(layout_viewport->HypotheticalScrollbarThickness(kVerticalScrollbar), 0);
    ...
    // show.
    mojom::blink::ScrollbarMode horizontal_scrollbar_mode = mojom::blink::ScrollbarMode::kAlwaysOff;
    if (new_size.width() > max_auto_size_.width()) {
        new_size.set_width(max_auto_size_.width());
        horizontal_scrollbar_mode = mojom::blink::ScrollbarMode::kAlwaysOn;
    }
    mojom::blink::ScrollbarMode vertical_scrollbar_mode = mojom::blink::ScrollbarMode::kAlwaysOff;
    ...
    if (!running_first_autosize_ && size.height() <= max_auto_size_.height() && size.width() <= max_auto_size_.width()
        && !frame_view_->GetFrame().GetDocument()->LoadEventFinished() && (new_size.height() < size.height() || new_size.width() < size.width())) {
        change_size = false;
    }
    ...
    frame_view_->GetLayoutView()->SetAutosizeScrollbarModes(horizontal_scrollbar_mode, vertical_scrollbar_mode);
