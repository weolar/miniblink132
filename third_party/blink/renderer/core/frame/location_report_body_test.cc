...
public:
    explicit TestLocationReportBody(
        const String& source_file = g_empty_string, std::optional<uint32_t> line_number = std::nullopt, std::optional<uint32_t> column_number = std::nullopt)
...
TEST(LocationReportBodyMatchIdTest, SameInputGeneratesSameMatchId)
{
    ...
    EXPECT_EQ(TestLocationReportBody(url, line, column).MatchId(), TestLocationReportBody(url, line, column).MatchId());
...
TEST(LocationReportBodyMatchIdTest, DifferentInputsGenerateDifferentMatchId)
{
    ...
    for (const auto& input : kLocationReportBodyInputs) {
        match_ids.push_back(TestLocationReportBody(input.url, input.line_number, input.column_number).MatchId());
    }
...
TEST(LocationReportBodyMatchIdTest, MatchIdGeneratedShouldNotBeZero)
{
    std::vector<unsigned> match_ids;
    for (const auto& input : kLocationReportBodyInputs) {
        EXPECT_NE(TestLocationReportBody(input.url, input.line_number, input.column_number).MatchId(), 0u);
    }
}
...
TEST(LocationReportBodyMatchIdTest, EmptyURLGenerateSameMatchIdRegardlessOfOtherParams)
{
    const unsigned empty_hash = TestLocationReportBody("", std::nullopt, std::nullopt).MatchId();
    for (const auto& input : kLocationReportBodyInputs) {
        EXPECT_EQ(TestLocationReportBody("", input.line_number, input.column_number).MatchId(), empty_hash);
    }
}

TEST(LocationReportBodyTest, ExtensionURLsAreIdentified)
{
    const char* kExtensionUrl = "chrome-extension://abcdefghijklmnopabcdefghijklmnop/scripts/"
                                "script.js";
    const char* kAboutBlankUrl = "about:blank";
    const char* kHttpsUrl = "https://example.com/";

    EXPECT_FALSE(TestLocationReportBody(kExtensionUrl, 1, 1).IsExtensionSource());
    EXPECT_FALSE(TestLocationReportBody(kAboutBlankUrl, 1, 1).IsExtensionSource());
