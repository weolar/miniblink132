...
#if 0 // BUILDFLAG(ENABLE_PRINTING)
...
BASE_FEATURE(kSkipUnnecessaryRemoteFrameGeometryPropagation, "SkipUnnecessaryRemoteFrameGeometryPropagation", base::FEATURE_DISABLED_BY_DEFAULT);
...
LocalFrameView* RemoteFrameView::ParentLocalRootFrameView() const
{
    ...
    // |is_attached_| is only set from AttachToLayout(), which ensures that the
    // parent is a local frame.
    return To<LocalFrame>(remote_frame_->Tree().Parent())->LocalFrameRoot().View();
}
...
void RemoteFrameView::AttachToLayout()
{
    ...
    UpdateRenderThrottlingStatus(IsHiddenForThrottling(), ParentFrameView()->CanThrottleRenderingForPropagation(), IsDisplayLocked());
...
bool RemoteFrameView::UpdateViewportIntersectionsForSubtree(unsigned parent_flags, ComputeIntersectionsContext&)
{
    UpdateViewportIntersection(parent_flags, needs_occlusion_tracking_);
    return needs_occlusion_tracking_;
}

void RemoteFrameView::SetViewportIntersection(const mojom::blink::ViewportIntersectionState& intersection_state)
{
    ...
    auto is_equal
        = [](mojom::blink::ViewportIntersectionState& a, mojom::blink::ViewportIntersectionState& b, bool ignore_outermost_main_frame_scroll_position) {
              if (ignore_outermost_main_frame_scroll_position) {
                  auto b_copy = b;
                  b_copy.outermost_main_frame_scroll_position = a.outermost_main_frame_scroll_position;
    ...
    bool needs_update;
    if (base::FeatureList::IsEnabled(kSkipUnnecessaryRemoteFrameGeometryPropagation)) {
        ...
        // context switches.
        bool outside_viewport = frame_visibility()
            && (*frame_visibility() == mojom::blink::FrameVisibility::kNotRendered
                || *frame_visibility() == mojom::blink::FrameVisibility::kRenderedOutOfViewport);
        needs_update = !is_equal(last_intersection_state_, new_state, outside_viewport);
    } else {
        needs_update = !last_intersection_state_.Equals(new_state);
    }

    UMA_HISTOGRAM_BOOLEAN("Blink.UpdateViewportIntersection.RemoteFrameNeedsUpdate", needs_update);
    if (needs_update) {
...
gfx::Rect RemoteFrameView::ComputeCompositingRect() const
{
    ...
    LayoutEmbeddedContent* owner_layout_object = remote_frame_->OwnerLayoutObject();
    ...
    TransformState local_root_transform_state(TransformState::kApplyTransformDirection);
    local_root_transform_state.Move(owner_layout_object->PhysicalContentBoxOffset());
    owner_layout_object->MapLocalToAncestor(nullptr, local_root_transform_state, kTraverseDocumentBoundaries);
    gfx::Transform matrix = local_root_transform_state.AccumulatedTransform().InverseOrIdentity();
    PhysicalRect local_viewport_rect = PhysicalRect::EnclosingRect(matrix.ProjectQuad(gfx::QuadF(gfx::RectF(viewport_rect))).BoundingBox());
    ...
    compositing_rect.Outset(gfx::Outsets::VH(ceilf(local_viewport_rect.Height() * 0.15f), ceilf(local_viewport_rect.Width() * 0.15f)));
...
void RemoteFrameView::UpdateCompositingScaleFactor()
{
    ...
    gfx::Transform local_root_transform = local_root_transform_state.AccumulatedTransform();
    std::optional<gfx::Vector2dF> scale_components = gfx::TryComputeTransform2dScaleComponents(local_root_transform);
    if (!scale_components) {
        frame_to_local_root_scale_factor = gfx::ComputeApproximateMaxScale(local_root_transform);
    } else {
        frame_to_local_root_scale_factor = std::max(scale_components->x(), scale_components->y());
    }
    ...
    compositing_scale_factor_ = widget->GetCompositingScaleFactor() * frame_to_local_root_scale_factor;
    ...
    compositing_scale_factor_ = std::clamp(compositing_scale_factor_, kMinCompositingScaleFactor, kMaxCompositingScaleFactor);
...
void RemoteFrameView::UpdateFrozenSize()
{
    ...
    if (!layout_embedded_content)
        return;
    std::optional<PhysicalSize> frozen_phys_size = layout_embedded_content->FrozenFrameSize();
    if (!frozen_phys_size)
        return;
    const gfx::Size rounded_frozen_size(frozen_phys_size->width.Ceil(), frozen_phys_size->height.Ceil());
...
void RemoteFrameView::Paint(GraphicsContext& context, PaintFlags flags, const CullRect& rect, const gfx::Vector2d& paint_offset) const
{
    ...
    if (owner_layout_object.GetDocument().IsPrintingOrPaintingPreview()) {
        DrawingRecorder recorder(context, owner_layout_object, DisplayItem::kDocumentBackground);
        context.Save();
        ...
        if (owner_layout_object.GetDocument().Printing()) {
            ...
            DCHECK_NE(Document::kNotPaintingPreview, owner_layout_object.GetDocument().GetPaintPreviewState());
    ...
    if (GetFrame().GetCcLayer()) {
        RecordForeignLayer(context, owner_layout_object, DisplayItem::kForeignLayerRemoteFrame, GetFrame().GetCcLayer(), FrameRect().origin() + paint_offset);
    }
}
...
void RemoteFrameView::Hide()
{
    ...
    UpdateFrameVisibility(!last_intersection_state_.viewport_intersection.IsEmpty());
}
...
void RemoteFrameView::VisibilityForThrottlingChanged()
{
    ...
    // separate bits.
    remote_frame_->GetRemoteFrameHostRemote().UpdateRenderThrottlingStatus(IsHiddenForThrottling(), IsSubtreeThrottled(), IsDisplayLocked());
}

void RemoteFrameView::VisibilityChanged(blink::mojom::FrameVisibility visibility)
{
...
void RemoteFrameView::SetIntrinsicSizeInfo(const IntrinsicSizingInfo& size_info)
{
    intrinsic_sizing_info_ = size_info;
    has_intrinsic_sizing_info_ = true;
}

bool RemoteFrameView::GetIntrinsicSizingInfo(IntrinsicSizingInfo& sizing_info) const
{
...
uint32_t RemoteFrameView::Print(const gfx::Rect& rect, cc::PaintCanvas* canvas) const
{
#if 0 // BUILDFLAG(ENABLE_PRINTING)
    ...
    uint32_t content_id = metafile->CreateContentForRemoteFrame(rect, remote_frame_->GetFrameToken().value());

    // Inform browser to print the remote subframe.
    remote_frame_->GetRemoteFrameHostRemote().PrintCrossProcessSubframe(rect, metafile->GetDocumentCookie());
    return content_id;
#else
    return 0;
#endif
...
uint32_t RemoteFrameView::CapturePaintPreview(const gfx::Rect& rect, cc::PaintCanvas* canvas) const
{
    *(int*)1 = 1;
    return 0;
...
//     // result.
//     std::optional<base::UnguessableToken> maybe_embedding_token = remote_frame_->GetEmbeddingToken();
//     if (!maybe_embedding_token.has_value())
//         return 0;
//     uint32_t content_id = tracker->CreateContentForRemoteFrame(rect, maybe_embedding_token.value());
// 
//     // Send a request to the browser to trigger a capture of the remote frame.
//     remote_frame_->GetRemoteFrameHostRemote().CapturePaintPreviewOfCrossProcessSubframe(rect, tracker->Guid());
//     return content_id;
}
