...
public:
    ...
    void AssertFindMatchRects(int expected_version, const WebVector<gfx::RectF>& expected_rects, const gfx::RectF& expected_active_match_rect,
        int actual_version, const Vector<gfx::RectF>& actual_rects, const gfx::RectF& actual_active_match_rect)
    {
        is_called = true;
...
TEST_F(FindInPageTest, FindMatchRectsReturnsCorrectRects)
{
    ...
    GetFindInPage().FindMatchRects(rects_version - 1,
        WTF::BindOnce(&FindInPageCallbackReceiver::AssertFindMatchRects, WTF::Unretained(&callback_receiver), rects_version, GetTextFinder().FindMatchRects(),
