...
protected:
    ...
    bool TaskShouldBeReported()
    {
        return monitor_->task_should_be_reported_;
    }
...
void PerformanceMonitorTest::SetUp()
{
    ...
    monitor_ = MakeGarbageCollected<PerformanceMonitor>(GetFrame(), GetExecutionContext()->GetIsolate());
...
String PerformanceMonitorTest::FrameContextURL()
{
    ...
    return To<LocalDOMWindow>(monitor_->task_execution_context_.Get())->location()->toString();
}
...
TEST_F(PerformanceMonitorTest, NoScriptInLongTask)
{
    WillProcessTask(SecondsToTimeTicks(3719349.445172));
    WillExecuteScript(GetExecutionContext());
    DidProcessTask(SecondsToTimeTicks(3719349.445172), SecondsToTimeTicks(3719349.445182));
