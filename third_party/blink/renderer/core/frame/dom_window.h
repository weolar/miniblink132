...
public:
    ...
    Frame* GetFrame() const
    {
        ...
        //   frame.
        SECURITY_DCHECK(!frame_ || (frame_->DomWindow() == this && frame_->GetPage()));
        return frame_.Get();
    }
    ...
    void postMessage(v8::Isolate*, const ScriptValue& message, const String& target_origin, HeapVector<ScriptValue> transfer, ExceptionState&);

    void postMessage(v8::Isolate*, const ScriptValue& message, const WindowPostMessageOptions* options, ExceptionState&);
    ...
    String SanitizedCrossDomainAccessErrorMessage(const LocalDOMWindow* accessing_window, CrossDocumentAccessPolicy cross_document_access) const;
    String CrossDomainAccessErrorMessage(const LocalDOMWindow* accessing_window, CrossDocumentAccessPolicy cross_document_access) const;
    ...
    void PostMessageForTesting(
        scoped_refptr<SerializedScriptValue> message, const MessagePortArray&, const String& target_origin, LocalDOMWindow* source, ExceptionState&);
    ...
    void InstallCoopAccessMonitor(LocalFrame* accessing_frame, network::mojom::blink::CrossOriginOpenerPolicyReporterParamsPtr coop_reporter_params,
    ...
    void RecordWindowProxyAccessMetrics(mojom::blink::WebFeature property_access, mojom::blink::WebFeature property_access_from_other_page,
    ...
    std::optional<ProxyAccessBlockedReason> GetProxyAccessBlockedReason(v8::Isolate* isolate) const;
    static String GetProxyAccessBlockedExceptionMessage(ProxyAccessBlockedReason reason);
...
private:
    void DoPostMessage(scoped_refptr<SerializedScriptValue> message, const MessagePortArray&, const WindowPostMessageOptions* options, LocalDOMWindow* source,
        ExceptionState&);
    ...
    struct CoopAccessMonitor : public GarbageCollected<CoopAccessMonitor> {
        ...
        void Trace(Visitor* visitor) const
        {
            visitor->Trace(reporter);
        }

        network::mojom::blink::CoopAccessReportType report_type;
        blink::LocalFrameToken accessing_main_frame;
        HeapMojoRemote<network::mojom::blink::CrossOriginOpenerPolicyReporter> reporter;
