...
class CORE_EXPORT PermissionsPolicyViolationReportBody : public LocationReportBody {
    DEFINE_WRAPPERTYPEINFO();

public:
    PermissionsPolicyViolationReportBody(const String& feature_id, const String& message, const String& disposition)
        : feature_id_(feature_id)
        , message_("Permissions policy violation: " + (message.empty() ? feature_id + " is not allowed in this document." : message))
        , disposition_(disposition)
    {
    }

    const String& featureId() const
    {
        return feature_id_;
    }
