...
void FrameView::UpdateViewportIntersection(unsigned flags, bool needs_occlusion_tracking)
{
    ...
    DocumentLifecycle::LifecycleState parent_lifecycle_state = owner_document.Lifecycle().GetState();

    bool should_compute_occlusion = false;
    mojom::blink::FrameOcclusionState occlusion_state = owner_document.GetFrame()->GetOcclusionState();
    if (occlusion_state == mojom::blink::FrameOcclusionState::kGuaranteedNotOccluded) {
        ...
        if (needs_occlusion_tracking && parent_lifecycle_state >= DocumentLifecycle::kPrePaintClean) {
            should_compute_occlusion = true;
        }
    }

    LayoutEmbeddedContent* owner_layout_object = owner_element->GetLayoutEmbeddedContent();
    bool display_locked_in_parent_frame = DisplayLockedInParentFrame();
    if (!owner_layout_object || owner_layout_object->ContentSize().IsEmpty() || (flags & IntersectionObservation::kAncestorFrameIsDetachedFromLayout)
    ...
    } else if (parent_lifecycle_state >= DocumentLifecycle::kLayoutClean && !owner_document.View()->NeedsLayout()) {
        unsigned geometry_flags = IntersectionGeometry::kForFrameViewportIntersection;
        ...
        PhysicalRect new_rect_in_parent = PhysicalRect::FastAndLossyFromRectF(geometry.IntersectionRect());

        // Convert to DIP
        const auto& screen_info = frame.GetChromeClient().GetScreenInfo(*owner_document.GetFrame());
        new_rect_in_parent.Scale(1. / screen_info.device_scale_factor);

        // Movement as a proportion of frame size
        double horizontal_movement = new_rect_in_parent.Width() ? (new_rect_in_parent.X() - rect_in_parent_.X()).Abs() / new_rect_in_parent.Width() : 0.0;
        double vertical_movement = new_rect_in_parent.Height() ? (new_rect_in_parent.Y() - rect_in_parent_.Y()).Abs() / new_rect_in_parent.Height() : 0.0;
        if (new_rect_in_parent.size != rect_in_parent_.size || horizontal_movement > FrameVisualProperties::MaxChildFrameScreenRectMovement()
            || vertical_movement > FrameVisualProperties::MaxChildFrameScreenRectMovement()) {
        ...
        if (new_rect_in_parent.size != rect_in_parent_for_iov2_.size
            || ((new_rect_in_parent.X() - rect_in_parent_for_iov2_.X()).Abs() + (new_rect_in_parent.Y() - rect_in_parent_for_iov2_.Y()).Abs()
                > LayoutUnit(FrameVisualProperties::MaxChildFrameScreenRectMovementForIOv2()))) {
            rect_in_parent_for_iov2_ = new_rect_in_parent;
            if (Page* page = GetFrame().GetPage()) {
                rect_in_parent_stable_since_for_iov2_ = page->Animator().Clock().CurrentTime();
            } else {
        ...
        if (should_compute_occlusion) {
            occlusion_state
                = geometry.IsVisible() ? mojom::blink::FrameOcclusionState::kGuaranteedNotOccluded : mojom::blink::FrameOcclusionState::kPossiblyOccluded;
        }

        // Generate matrix to transform from the space of the containing document
        // to the space of the iframe's contents.
        TransformState parent_frame_to_iframe_content_transform(TransformState::kUnapplyInverseTransformDirection);
        // First transform to box coordinates of the iframe element...
        owner_layout_object->MapAncestorToLocal(nullptr, parent_frame_to_iframe_content_transform, 0);
        // ... then apply content_box_offset to translate to the coordinate of the
        // child frame.
        parent_frame_to_iframe_content_transform.Move(owner_layout_object->PhysicalContentBoxOffset());
        gfx::Transform matrix = parent_frame_to_iframe_content_transform.AccumulatedTransform().InverseOrIdentity();
        if (geometry.IsIntersecting()) {
            PhysicalRect intersection_rect = PhysicalRect::EnclosingRect(matrix.ProjectQuad(gfx::QuadF(gfx::RectF(geometry.IntersectionRect()))).BoundingBox());

            // Don't let EnclosingRect turn an empty rect into a non-empty one.
            if (intersection_rect.IsEmpty()) {
                viewport_intersection = gfx::Rect(ToFlooredPoint(intersection_rect.offset), gfx::Size());
            } else {
        ...
        if (!geometry.UnclippedIntersectionRect().IsEmpty()) {
            mainframe_intersection_rect = PhysicalRect::EnclosingRect(matrix.ProjectQuad(gfx::QuadF(geometry.UnclippedIntersectionRect())).BoundingBox());

            if (mainframe_intersection_rect.IsEmpty()) {
                mainframe_intersection = gfx::Rect(ToFlooredPoint(mainframe_intersection_rect.offset), gfx::Size());
            } else {
        ...
        TransformState child_frame_to_root_frame(TransformState::kUnapplyInverseTransformDirection);
        // TODO: Should this be IsOutermostMainFrame()?
        if (owner_document.GetFrame()->LocalFrameRoot().IsMainFrame()) {
            child_frame_to_root_frame.Move(PhysicalOffset::FromPointFRound(gfx::PointF(frame.GetOutermostMainFrameScrollPosition())));
        }
        if (owner_layout_object) {
            owner_layout_object->MapAncestorToLocal(nullptr, child_frame_to_root_frame, kTraverseDocumentBoundaries | kApplyRemoteMainFrameTransform);
            child_frame_to_root_frame.Move(owner_layout_object->PhysicalContentBoxOffset());
        }
        main_frame_transform_matrix = child_frame_to_root_frame.AccumulatedTransform();
    }
    ...
    SetViewportIntersection(mojom::blink::ViewportIntersectionState(viewport_intersection, mainframe_intersection, gfx::Rect(), occlusion_state,
        frame.GetOutermostMainFrameSize(), frame.GetOutermostMainFrameScrollPosition(), pixel_snapped_transform));

    UpdateFrameVisibility(!viewport_intersection.IsEmpty());

    if (ShouldReportMainFrameIntersection()) {
        gfx::Rect projected_rect = gfx::ToEnclosingRect(main_frame_transform_matrix.ProjectQuad(gfx::QuadF(gfx::RectF(mainframe_intersection))).BoundingBox());
    ...
    bool should_throttle = (is_display_none || (zero_viewport_intersection && !has_zero_area));

    bool subtree_throttled = false;
    Frame* parent_frame = GetFrame().Tree().Parent();
    if (parent_frame && parent_frame->View()) {
        subtree_throttled = parent_frame->View()->CanThrottleRenderingForPropagation();
    }
    UpdateRenderThrottlingStatus(should_throttle, subtree_throttled, display_locked_in_parent_frame);
}
...
void FrameView::UpdateFrameVisibility(bool intersects_viewport)
{
    ...
    if (LifecycleUpdatesThrottled())
        return;
    if (IsVisible()) {
        frame_visibility = intersects_viewport ? mojom::blink::FrameVisibility::kRenderedInViewport : mojom::blink::FrameVisibility::kRenderedOutOfViewport;
    } else {
...
void FrameView::UpdateRenderThrottlingStatus(bool hidden_for_throttling, bool subtree_throttled, bool display_locked, bool recurse)
{
    bool visibility_changed
        = (hidden_for_throttling_ || subtree_throttled_ || display_locked_) != (hidden_for_throttling || subtree_throttled || display_locked);
        ...
        for (Frame* child = GetFrame().Tree().FirstChild(); child; child = child->Tree().NextSibling()) {
            if (FrameView* child_view = child->View()) {
                child_view->UpdateRenderThrottlingStatus(
                    child_view->IsHiddenForThrottling(), child_view->IsAttached() && CanThrottleRenderingForPropagation(), child_view->IsDisplayLocked(), true);
            }
        }
    }
}

bool FrameView::RectInParentIsStable(const base::TimeTicks& event_timestamp) const
{
    if (event_timestamp - rect_in_parent_stable_since_ < base::Milliseconds(blink::FrameVisualProperties::MinScreenRectStableTimeMs())) {
        return false;
    }
...
bool FrameView::RectInParentIsStableForIOv2(const base::TimeTicks& event_timestamp) const
{
    if (event_timestamp - rect_in_parent_stable_since_for_iov2_ < base::Milliseconds(blink::FrameVisualProperties::MinScreenRectStableTimeMsForIOv2())) {
        return false;
    }
    LocalFrameView* parent = ParentFrameView();
    if (!parent) {
