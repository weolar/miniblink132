...
ChildFrameCompositingHelper::ChildFrameCompositingHelper(ChildFrameCompositor* child_frame_compositor)
...
void ChildFrameCompositingHelper::SetSurfaceId(
    const viz::SurfaceId& surface_id, CaptureSequenceNumberChanged capture_sequence_number_changed, AllowPaintHolding allow_paint_holding)
{
    ...
    cc::DeadlinePolicy deadline = capture_sequence_number_changed == CaptureSequenceNumberChanged::kYes ? cc::DeadlinePolicy::UseInfiniteDeadline()
    ...
    child_frame_compositor_->SetCcLayer(surface_layer_, true /* is_surface_layer */);

    UpdateVisibility(true);
}

void ChildFrameCompositingHelper::MaybeSetUpPaintHolding(const viz::SurfaceId& fallback_id, AllowPaintHolding allow_paint_holding)
{
    if (!RuntimeEnabledFeatures::PaintHoldingForIframesEnabled()) {
        return;
    }

    if (fallback_id.is_valid() && allow_paint_holding == AllowPaintHolding::kYes) {
        surface_layer_->SetOldestAcceptableFallback(fallback_id);

        paint_holding_timer_.Start(
            FROM_HERE, kNewContentRenderingDelay, WTF::BindOnce(&ChildFrameCompositingHelper::PaintHoldingTimerFired, base::Unretained(this)));
    } else {
...
scoped_refptr<cc::DisplayItemList> ChildFrameCompositingHelper::PaintContentsToDisplayList()
{
    ...
    if (sad_bitmap) {
        ...
        if (layer_size.width() >= paint_width && layer_size.height() >= paint_height) {
            ...
            if (device_scale_factor_ != 1.f) {
                ...
                display_list->push<cc::ScaleOp>(device_scale_factor_, device_scale_factor_);
                x = 0;
                y = 0;
            }
            ...
            auto image = cc::PaintImageBuilder::WithDefault()
                             ...
                             .set_image(SkImages::RasterFromBitmap(*sad_bitmap), cc::PaintImage::GetNextContentId())
                             .TakePaintImage();
