...
void SendToBrowser(ExecutionContext* context, const DeprecationInfo& info)
{
    ...
    // process.
    if (!base::CommandLine::ForCurrentProcess()->HasSwitch(blink::switches::kLegacyTechReportPolicyEnabled)) {
        return;
    }
    ...
    if (auto* window = DynamicTo<LocalDOMWindow>(context)) {
        if (LocalFrame* frame = window->GetFrame()) {
            ...
            frame->GetLocalFrameHostRemote().SendLegacyTechEvent(info.type_.ToString(),
                mojom::blink::LegacyTechEventCodeLocation::New(
                    source_location->Url() ? source_location->Url() : g_empty_string, source_location->LineNumber(), source_location->ColumnNumber()));
        }
    }
}
...
void Deprecation::CountDeprecationCrossOriginIframe(LocalDOMWindow* window, WebFeature feature)
{
    DCHECK(window);
    if (!window->GetFrame())
        return;
    ...
    if (!window->GetSecurityOrigin()->CanAccess(top.GetSecurityContext()->GetSecurityOrigin())) {
        CountDeprecation(window, feature);
    }
}

void Deprecation::CountDeprecation(ExecutionContext* context, WebFeature feature)
{
    if (!context)
        return;
    ...
    if (!deprecation || deprecation->mute_count_ || deprecation->GetReported(feature)) {
        return;
    }
    ...
    // ReportingObservers.
    DeprecationReportBody* body = MakeGarbageCollected<DeprecationReportBody>(type, std::nullopt, info.message_.ToString());
    Report* report = MakeGarbageCollected<Report>(ReportType::kDeprecation, context->Url(), body);
