...
ScriptValue DeprecationReportBody::anticipatedRemoval(ScriptState* script_state) const
{
    v8::Isolate* isolate = script_state->GetIsolate();
    if (!anticipated_removal_)
        return ScriptValue::CreateNull(isolate);
    return ScriptValue(isolate, ToV8Traits<IDLNullable<IDLDate>>::ToV8(script_state, *anticipated_removal_));
}
...
void DeprecationReportBody::BuildJSONValue(V8ObjectBuilder& builder) const
{
    ...
    if (!anticipated_removal_) {
        ...
        bool is_valid = anticipated_removal_date.SetMillisecondsSinceEpochForDateTimeLocal(anticipated_removal_->InMillisecondsFSinceUnixEpochIgnoringNull());
        if (!is_valid) {
            builder.AddNull("anticipatedRemoval");
        } else {
            ...
            String iso8601_date = anticipated_removal_date.ToString(DateComponents::SecondFormat::kMillisecond) + "Z";
