...
TEST(DeprecationReportBodyJSONTest, noAnticipatedRemoval)
{
    ...
    DeprecationReportBody* body = MakeGarbageCollected<DeprecationReportBody>("test_id", std::nullopt, "test_message");
    ...
    String expected = "{\"sourceFile\":null,\"lineNumber\":null,\"columnNumber\":null,\"id\":"
...
TEST(DeprecationReportBodyJSONTest, actualAnticipatedRemoval)
{
    ...
    DeprecationReportBody* body
        = MakeGarbageCollected<DeprecationReportBody>("test_id", base::Time::FromMillisecondsSinceUnixEpoch(1575950400000), "test_message");
    V8TestingScope scope;
    ScriptState* script_state = scope.GetScriptState();
    V8ObjectBuilder builder(script_state);
    body->BuildJSONValue(builder);
