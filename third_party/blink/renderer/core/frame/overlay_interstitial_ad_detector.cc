...
bool IsOverlayCandidate(Element* element)
{
    ...
    // on the document.
    if (style->GetPosition() == EPosition::kFixed || style->HasStickyConstrainedPosition()) {
        return true;
    }
...
void OverlayInterstitialAdDetector::MaybeFireDetection(LocalFrame* outermost_main_frame)
{
    ...
    if (PaintTiming::From(*outermost_main_frame->GetDocument()).FirstContentfulPaintIgnoringSoftNavigations().is_null()) {
        return;
    }

    base::Time current_time = base::Time::Now();
    if (started_detection_ && base::FeatureList::IsEnabled(features::kFrequencyCappingForOverlayPopupDetection)
        && current_time < last_detection_time_ + kFireInterval)
        return;

    TRACE_EVENT0("blink,benchmark", "OverlayInterstitialAdDetector::MaybeFireDetection");

    started_detection_ = true;
    last_detection_time_ = current_time;

    gfx::Size outermost_main_frame_size = outermost_main_frame->View()->LayoutViewport()->VisibleContentRect().size();
    ...
    HitTestLocation location(gfx::PointF(outermost_main_frame_size.width() / 2.0, outermost_main_frame_size.height() / 2.0));
    HitTestResult result;
    outermost_main_frame->ContentLayoutObject()->HitTestNoLifecycleUpdate(location, result);
    ...
    if (!content_has_been_stable_) {
        if (!PaintTiming::From(*outermost_main_frame->GetDocument()).FirstMeaningfulPaint().is_null()) {
    ...
    if (is_new_element && candidate_id_ != kInvalidDOMNodeId) {
        ...
        if (outermost_main_frame->GetOutermostMainFrameScrollPosition().y() == candidate_start_outermost_main_frame_scroll_position_) {
    ...
    gfx::Rect overlay_rect = element->GetLayoutObject()->AbsoluteBoundingBoxRect();

    bool is_large = (overlay_rect.size().Area64() > outermost_main_frame_size.Area64() * kLargeAdSizeToViewportSizeThreshold);

    bool has_gesture = LocalFrame::HasTransientUserActivation(outermost_main_frame);
    bool is_ad = element->IsAdRelated();

    if (!has_gesture && is_large && (!popup_detected_ || is_ad) && IsOverlayCandidate(element)) {
        ...
        // time.
        if (!outermost_main_frame->GetDocument()->GetLayoutView()->HasScrollableOverflowY()) {
        ...
        candidate_start_outermost_main_frame_scroll_position_ = outermost_main_frame->GetOutermostMainFrameScrollPosition().y();
    } else {
        last_unqualified_element_id_ = element_id;
    }
}

void OverlayInterstitialAdDetector::OnPopupDetected(LocalFrame* outermost_main_frame, bool is_ad)
{
    if (!popup_detected_) {
        UseCounter::Count(outermost_main_frame->GetDocument(), WebFeature::kOverlayPopup);
    ...
    if (is_ad) {
        ...
        UseCounter::Count(outermost_main_frame->GetDocument(), WebFeature::kOverlayPopupAd);
