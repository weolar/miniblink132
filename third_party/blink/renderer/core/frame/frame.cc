...
bool Frame::Detach(FrameDetachType type)
{
    ...
    const std::string_view histogram_suffix = (type == FrameDetachType::kRemove) ? "Remove" : "Swap";
    base::ScopedUmaHistogramTimer histogram_timer(base::StrCat({ "Navigation.Frame.Detach.", histogram_suffix }));
    DCHECK(client_);
...
bool Frame::IsCrossOriginToNearestMainFrame() const
{
    ...
    return !security_origin->CanAccess(Tree().Top().GetSecurityContext()->GetSecurityOrigin());
}
...
bool Frame::IsCrossOriginToParentOrOuterDocument() const
{
    ...
    const SecurityOrigin* parent_security_origin = parent->GetSecurityContext()->GetSecurityOrigin();
    const SecurityOrigin* security_origin = GetSecurityContext()->GetSecurityOrigin();
...
{
    DEFINE_STATIC_LOCAL(Persistent<EmptyChromeClient>, client, (MakeGarbageCollected<EmptyChromeClient>()));
    return *client;
}
...
Frame* Frame::FindUnsafeParentScrollPropagationBoundary()
{
    ...
    while (ancestor_frame) {
        if (!ancestor_frame->GetSecurityContext()->GetSecurityOrigin()->CanAccess(GetSecurityContext()->GetSecurityOrigin()))
...
{
    NotifyUserActivationInFrameTree(mojom::blink::UserActivationNotificationType::kNone,
        /*sticky_only=*/true);
}

void Frame::NotifyUserActivationInFrameTree(mojom::blink::UserActivationNotificationType notification_type, bool sticky_only)
{
    ...
    if (RuntimeEnabledFeatures::DocumentPictureInPictureUserActivationEnabled()) {
        ...
        if (local_top_frame && local_top_frame->GetDocument()) {
            LocalDOMWindow* pip_owner = PictureInPictureController::GetDocumentPictureInPictureOwner(*local_top_frame->GetDocument());
            if (pip_owner) {
                NotifyUserActivationInFrame(pip_owner->GetFrame(), notification_type, sticky_only);
            }
        }
    }
#endif // !BUILDFLAG(TARGET_OS_IS_ANDROID)
    ...
    if (local_frame && RuntimeEnabledFeatures::UserActivationSameOriginVisibilityEnabled()) {
        const SecurityOrigin* security_origin = local_frame->GetSecurityContext()->GetSecurityOrigin();

        for (Frame* node = &Tree().Top(); node; node = node->Tree().TraverseNext()) {
            auto* local_frame_node = DynamicTo<LocalFrame>(node);
            if (local_frame_node && security_origin->CanAccess(local_frame_node->GetSecurityContext()->GetSecurityOrigin())) {
                NotifyUserActivationInFrame(node, notification_type, sticky_only);
            }
        }
        ...
        if (RuntimeEnabledFeatures::DocumentPictureInPictureUserActivationEnabled()) {
            ...
            auto* local_top_frame = DynamicTo<LocalFrame>(Tree().Top());
            if (local_top_frame) {
                LocalDOMWindow* pip_window = PictureInPictureController::GetDocumentPictureInPictureWindow(*local_top_frame->GetDocument());
                for (Frame* node = pip_window ? pip_window->GetFrame() : nullptr; node; node = node->Tree().TraverseNext()) {
                    auto* local_frame_node = DynamicTo<LocalFrame>(node);
                    if (local_frame_node && security_origin->CanAccess(local_frame_node->GetSecurityContext()->GetSecurityOrigin())) {
                        NotifyUserActivationInFrame(node, notification_type, sticky_only);
                    }
...
bool Frame::ConsumeTransientUserActivationInFrameTree()
{
    ...
    if (RuntimeEnabledFeatures::DocumentPictureInPictureUserActivationEnabled()) {
        ...
        if (local_top_frame) {
            ...
            LocalDOMWindow* pip_owner = PictureInPictureController::GetDocumentPictureInPictureOwner(*local_top_frame->GetDocument());
            for (Frame* node = pip_owner ? pip_owner->GetFrame() : nullptr; node; node = node->Tree().TraverseNext()) {
                node->user_activation_state_.ConsumeIfActive();
            }
...
void Frame::RenderFallbackContent()
{
    ...
    To<HTMLObjectElement>(Owner())->RenderFallbackContent(HTMLObjectElement::ErrorEventPolicy::kDispatch);
}
...
std::optional<blink::FencedFrame::DeprecatedFencedFrameMode> Frame::GetDeprecatedFencedFrameMode() const
{
    DCHECK(!IsDetached());
...
{
    if (auto* frame_owner_element = DynamicTo<HTMLFrameOwnerElement>(owner_.Get())) {
...
void Frame::UpdateInheritedEffectiveTouchActionIfPossible()
{
    if (owner_) {
        ...
        if (owner_frame) {
            SetInheritedEffectiveTouchAction(owner_frame->InheritedEffectiveTouchAction());
        }
    }
}
...
void Frame::UpdateVisibleToHitTesting()
{
    ...
    if (auto* local_owner = DynamicTo<HTMLFrameOwnerElement>(owner_.Get())) {
        self_visible_to_hit_testing = local_owner->GetLayoutObject() ? local_owner->GetLayoutObject()->Style()->VisibleToHitTesting() : true;
    }

    bool visible_to_hit_testing = parent_visible_to_hit_testing && self_visible_to_hit_testing;
...
Frame::Frame(FrameClient* client, Page& page, FrameOwner* owner, Frame* parent, Frame* previous_sibling, FrameInsertType insert_type,
    const FrameToken& frame_token, const base::UnguessableToken& devtools_frame_token, WindowProxyManager* window_proxy_manager,
    ...
    , window_agent_factory_(inheriting_agent_factory ? inheriting_agent_factory : MakeGarbageCollected<WindowAgentFactory>(page.GetAgentGroupScheduler()))
    , is_loading_(false)
...
void Frame::FocusImpl()
{
    ...
    GetPage()->GetFocusController().FocusDocumentView(this, false /* notify_embedder */);
}

void Frame::ApplyFrameOwnerProperties(mojom::blink::FrameOwnerPropertiesPtr properties)
{
...
base::OnceClosure Frame::ScheduleFormSubmission(FrameScheduler* scheduler, FormSubmission* form_submission)
{
    form_submit_navigation_task_ = PostCancellableTask(
        *scheduler->GetTaskRunner(TaskType::kDOMManipulation), FROM_HERE, WTF::BindOnce(&FormSubmission::Navigate, WrapPersistent(form_submission)));
    form_submit_navigation_task_version_++;

    return WTF::BindOnce(&Frame::CancelFormSubmissionWithVersion, WrapWeakPersistent(this), form_submit_navigation_task_version_);
}
...
void Frame::FocusPage(LocalFrame* originating_frame)
{
    ...
    if (originating_frame && LocalFrame::HasTransientUserActivation(originating_frame)) {
...
{
    return SwapImpl(new_web_frame, mojo::NullAssociatedRemote(), mojo::NullAssociatedReceiver());
}

bool Frame::Swap(WebRemoteFrame* new_web_frame, mojo::PendingAssociatedRemote<mojom::blink::RemoteFrameHost> remote_frame_host,
    mojo::PendingAssociatedReceiver<mojom::blink::RemoteFrame> remote_frame_receiver)
{
    return SwapImpl(new_web_frame, std::move(remote_frame_host), std::move(remote_frame_receiver));
}

bool Frame::SwapImpl(WebFrame* new_web_frame, mojo::PendingAssociatedRemote<mojom::blink::RemoteFrameHost> remote_frame_host,
    mojo::PendingAssociatedReceiver<mojom::blink::RemoteFrame> remote_frame_receiver)
{
    TRACE_EVENT0("navigation", "Frame::SwapImpl");
    std::string_view histogram_suffix = (new_web_frame->IsWebLocalFrame() ? "Local" : "Remote");
    base::ScopedUmaHistogramTimer histogram_timer(base::StrCat({ "Navigation.Frame.SwapImpl.", histogram_suffix }));
    DCHECK(IsAttached());
    ...
    std::unique_ptr<IncrementLoadEventDelayCount> delay_parent_load
        = parent_local_frame ? std::make_unique<IncrementLoadEventDelayCount>(*parent_local_frame->GetDocument()) : nullptr;
    ...
    if (new_web_frame->IsWebRemoteFrame()) {
        ...
        To<WebRemoteFrameImpl>(new_web_frame)
            ->InitializeCoreFrame(*page, owner, WebFrame::FromCoreFrame(parent_), nullptr, FrameInsertType::kInsertLater, name, &window_agent_factory(),
                devtools_frame_token_, std::move(remote_frame_host), std::move(remote_frame_receiver));
    ...
    if (auto* new_local_frame = DynamicTo<LocalFrame>(new_frame)) {
        ...
        base::ScopedUmaHistogramTimer clone_state_timer("Navigation.Frame.SwapImpl.CloneState");
        ...
        if (owner) {
            ...
            if (page != new_page) {
                ...
                WebRemoteFrame* old_page_placeholder_remote_frame = WebRemoteFrame::Create(mojom::blink::TreeScopeType::kDocument, RemoteFrameToken());
                To<WebRemoteFrameImpl>(old_page_placeholder_remote_frame)
                    ->InitializeCoreFrame(*page, /*owner=*/nullptr, /*parent=*/nullptr,
                        /*previous_sibling=*/nullptr, FrameInsertType::kInsertLater, name, &window_agent_factory(), devtools_frame_token_,
                        mojo::NullAssociatedRemote(), mojo::NullAssociatedReceiver());
                page->SetMainFrame(WebFrame::ToCoreFrame(*old_page_placeholder_remote_frame));
                ...
                CHECK(!DynamicTo<RemoteFrame>(new_page->MainFrame())->IsRemoteFrameHostRemoteBound());
            ...
            TRACE_EVENT_INSTANT1("loading", "markAsMainFrame", TRACE_EVENT_SCOPE_THREAD, "frame", ::blink::GetFrameIdForTracing(new_local_frame));
        }
    }
...
// static
void Frame::NotifyUserActivationInFrame(Frame* node, mojom::blink::UserActivationNotificationType notification_type, bool sticky_only)
{
    CHECK(node);
    if (sticky_only) {
...
void Frame::RemoveChild(Frame* child)
{
    ...
    if (first_child_ == child) {
        ...
        CHECK(child->previous_sibling_) << " child " << child << " child->previous_sibling_ " << child->previous_sibling_;
...
HeapVector<Member<Resource>> Frame::AllResourcesUnderFrame()
{
    ...
    if (IsLocalFrame()) {
        if (auto* this_local_frame = DynamicTo<LocalFrame>(this)) {
            HeapHashSet<Member<Resource>> local_frame_resources = this_local_frame->GetDocument()->Fetcher()->MoveResourceStrongReferences();
