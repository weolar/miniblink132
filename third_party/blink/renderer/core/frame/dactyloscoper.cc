...
{
    return IdentifiabilityStudySettings::Get()->ShouldSampleSurface(IdentifiableSurface::FromTypeAndToken(IdentifiableSurface::Type::kWebFeature, feature));
}

// using CalledJsApi = perfetto::protos::pbzero::BlinkHighEntropyAPI::CalledJsApi;
// using JSFunctionArgument = perfetto::protos::pbzero::BlinkHighEntropyAPI::JSFunctionArgument;
// using ArgumentType = perfetto::protos::pbzero::BlinkHighEntropyAPI::JSFunctionArgument::ArgumentType;
...
// using FontLookupType = perfetto::protos::pbzero::BlinkHighEntropyAPI::FontLookup::FontLookupType;
// 
...
String V8ValueToString(v8::Local<v8::Context> current_context, v8::Isolate* isolate, const v8::Local<v8::Value>& value)
{
...
// static
void Dactyloscoper::RecordDirectSurface(ExecutionContext* context, WebFeature feature, const IdentifiableToken& value)
{
    if (!context || !ShouldSample(feature))
        return;

    IdentifiabilityMetricBuilder(context->UkmSourceID()).AddWebFeature(feature, value).Record(context->UkmRecorder());
}

// static
void Dactyloscoper::RecordDirectSurface(ExecutionContext* context, WebFeature feature, const String& str)
{
    if (!context || !ShouldSample(feature))
        return;
    Dactyloscoper::RecordDirectSurface(context, feature, IdentifiabilitySensitiveStringToken(str));
}

// static
void Dactyloscoper::RecordDirectSurface(ExecutionContext* context, WebFeature feature, const bindings::EnumerationBase& value)
{
    if (!context || !ShouldSample(feature)) {
        return;
    }
    Dactyloscoper::RecordDirectSurface(context, feature, IdentifiabilitySensitiveStringToken(value.AsString()));
}

// static
void Dactyloscoper::RecordDirectSurface(ExecutionContext* context, WebFeature feature, const Vector<String>& strs)
{
    if (!context || !ShouldSample(feature))
        return;
...
// static
void Dactyloscoper::RecordDirectSurface(ExecutionContext* context, WebFeature feature, const DOMArrayBufferView* buffer)
{
    if (!context || !ShouldSample(feature))
        return;
    IdentifiableTokenBuilder builder;
...
// static
void Dactyloscoper::RecordDirectSurface(ExecutionContext* context, WebFeature feature, SVGStringListTearOff* strings)
{
...
// 
//         ExecutionContextProto* proto_context = high_entropy_api.set_execution_context();
//         execution_context->WriteIntoTrace(ctx.Wrap(proto_context));
// 
//         std::unique_ptr<SourceLocation> source_location = CaptureSourceLocation(execution_context);
//         SourceLocationProto* proto_source_location = high_entropy_api.set_source_location();
...
//         FontSelectionRequest font_selection_request = font_description.GetFontSelectionRequest();
...
//             arg.set_value(V8ValueToString(current_context, isolate, info[i]).Utf8());
//         }
// 
//         std::unique_ptr<SourceLocation> source_location = CaptureSourceLocation(execution_context);
//         SourceLocationProto* proto_source_location = high_entropy_api.set_source_location();
...
{
    TRACE_EVENT_END(TRACE_DISABLED_BY_DEFAULT("identifiability.high_entropy_api"));
}
