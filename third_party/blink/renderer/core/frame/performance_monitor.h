...
class CORE_EXPORT PerformanceMonitor final : public GarbageCollected<PerformanceMonitor>, public base::sequence_manager::TaskTimeObserver {
    USING_PRE_FINALIZER(PerformanceMonitor, Dispose);

public:
    enum Violation : size_t { kLongTask = 0, kLongLayout, kBlockedEvent, kBlockedParser, kDiscouragedAPIUse, kHandler, kRecurringHandler, kAfterLast };

    class CORE_EXPORT Client : public GarbageCollectedMixin {
    public:
        virtual void ReportLongTask(base::TimeTicks start_time, base::TimeTicks end_time, ExecutionContext* task_context, bool has_multiple_contexts)
        {
        }
        virtual void ReportLongLayout(base::TimeDelta duration)
        {
        }
        virtual void ReportGenericViolation(Violation, const String& text, base::TimeDelta time, SourceLocation*)
        {
        }
    ...
    static void ReportGenericViolation(ExecutionContext*, Violation, const String& text, base::TimeDelta time, std::unique_ptr<SourceLocation>);
...
private:
    ...
    static PerformanceMonitor* InstrumentingMonitorExcludingLongTasks(const ExecutionContext*);

    void UpdateInstrumentation();

    void InnerReportGenericViolation(ExecutionContext*, Violation, const String& text, base::TimeDelta time, std::unique_ptr<SourceLocation>);

    // TaskTimeObserver implementation
    void WillProcessTask(base::TimeTicks start_time) override;
    void DidProcessTask(base::TimeTicks start_time, base::TimeTicks end_time) override;
    ...
    std::pair<String, DOMWindow*> SanitizedAttribution(const HeapHashSet<Member<Frame>>& frame_contexts, Frame* observer_frame);
    ...
    HeapHashMap<Violation, Member<ClientThresholds>, IntWithZeroKeyHashTraits<size_t>> subscriptions_;
