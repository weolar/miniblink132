...
PaginationState::PaginationState()
    : content_area_paint_properties_(MakeGarbageCollected<ObjectPaintProperties>())
{
}
...
LayoutBlockFlow* PaginationState::CreateAnonymousPageLayoutObject(Document& document, const ComputedStyle& style)
{
...
ObjectPaintProperties& PaginationState::EnsureContentAreaProperties(
    const TransformPaintPropertyNodeOrAlias& parent_transform, const ClipPaintPropertyNodeOrAlias& parent_clip)
{
    ...
    content_area_paint_properties_->UpdateTransform(parent_transform, TransformPaintPropertyNode::State());

    // Create clip node.
    ClipPaintPropertyNode::State clip_state(parent_transform, gfx::RectF(), FloatRoundedRect());
    content_area_paint_properties_->UpdateOverflowClip(parent_clip, std::move(clip_state));

    return *content_area_paint_properties_;
}

void PaginationState::UpdateContentAreaPropertiesForCurrentPage(const LayoutView& layout_view)
{
    DCHECK(layout_view.ShouldUsePaginatedLayout());
    auto chunk_properties = layout_view.FirstFragment().ContentsProperties();
    const PhysicalBoxFragment& page_container = *GetPageContainer(layout_view, current_page_index_);
    ...
    matrix.Translate(float(target_content_rect.offset.left), float(target_content_rect.offset.top));
    ...
    // page.
    PhysicalOffset stitched_offset = StitchedPageContentRect(layout_view, current_page_index_).offset;
    ...
    content_area_paint_properties_->UpdateTransform(chunk_properties.Transform(), std::move(transform_state));
    ...
    gfx::RectF target_page_area_rect(gfx::PointF(target_content_rect.offset), gfx::SizeF(target_content_rect.size));
    ClipPaintPropertyNode::State clip_state(chunk_properties.Transform(), target_page_area_rect, FloatRoundedRect(target_page_area_rect));
    content_area_paint_properties_->UpdateOverflowClip(chunk_properties.Clip(), std::move(clip_state));
}

PropertyTreeState PaginationState::ContentAreaPropertyTreeStateForCurrentPage(const LayoutView& layout_view) const
{
    DCHECK(content_area_paint_properties_);
    const EffectPaintPropertyNode& effect_node = layout_view.FirstFragment().ContentsEffect().Unalias();

    return PropertyTreeState(*content_area_paint_properties_->Transform(), *content_area_paint_properties_->OverflowClip(), effect_node);
}
