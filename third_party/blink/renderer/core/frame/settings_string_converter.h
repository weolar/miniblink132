...
template <typename T> struct FromString {
    ...
    {
        return static_cast<T>(s.ToInt());
    }
};

template <> struct FromString<String> {
...
template <> struct FromString<float> {
...
template <> struct FromString<double> {
...
template <> struct FromString<gfx::Size> {
    gfx::Size operator()(const String& s)
    {
        Vector<String> fields;
        s.Split(',', fields);
        return gfx::Size(fields.size() > 0 ? fields[0].ToInt() : 0, fields.size() > 1 ? fields[1].ToInt() : 0);
    }
};
