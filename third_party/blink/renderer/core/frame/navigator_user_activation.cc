...
const char NavigatorUserActivation::kSupplementName[] = "NavigatorUserActivation";

NavigatorUserActivation& NavigatorUserActivation::From(Navigator& navigator)
{
    NavigatorUserActivation* supplement = Supplement<Navigator>::From<NavigatorUserActivation>(navigator);
    if (!supplement) {
...
{
    user_activation_ = MakeGarbageCollected<UserActivation>(navigator.DomWindow());
}
