...
SelectorDirective::SelectorDirective(Type type)
    : Directive(type)
{
}
SelectorDirective::~SelectorDirective() = default;

ScriptPromise<Range> SelectorDirective::getMatchingRange(ScriptState* state, ExceptionState& exception_state) const
{
    ...
    matching_range_resolver_ = MakeGarbageCollected<ScriptPromiseResolver<Range>>(state, exception_state.GetContext());
...
void SelectorDirective::DidFinishMatching(const RangeInFlatTree* range)
{
    ...
    if (range) {
        selected_range_ = MakeGarbageCollected<RangeInFlatTree>(range->StartPosition(), range->EndPosition());
        ...
        DCHECK_EQ(selected_range_->StartPosition().GetDocument(), selected_range_->EndPosition().GetDocument());
    }
...
void SelectorDirective::ResolvePromise() const
{
    ...
    if (!selected_range_) {
        matching_range_resolver_->RejectWithDOMException(DOMExceptionCode::kNotFoundError, "Could not find range matching the given selector");
        return;
    }

    Range* dom_range = MakeGarbageCollected<Range>(*selected_range_->StartPosition().GetDocument(), ToPositionInDOMTree(selected_range_->StartPosition()),
