...
class CORE_EXPORT PictureInPictureController : public GarbageCollected<PictureInPictureController>, public Supplement<Document> {
public:
    static const char kSupplementName[];

    PictureInPictureController(const PictureInPictureController&) = delete;
    PictureInPictureController& operator=(const PictureInPictureController&) = delete;
    ...
    virtual void EnterPictureInPicture(HTMLVideoElement*, ScriptPromiseResolver<PictureInPictureWindow>*) = 0;

    // Exit Picture-in-Picture for a video element and resolve promise if any.
    virtual void ExitPictureInPicture(HTMLVideoElement*, ScriptPromiseResolver<IDLUndefined>*) = 0;
    ...
    virtual Status IsElementAllowed(const HTMLVideoElement&, bool report_failure = false) const = 0;

    // Should be called when an element has exited Picture-in-Picture.
    virtual void OnExitedPictureInPicture(ScriptPromiseResolver<IDLUndefined>*) = 0;
