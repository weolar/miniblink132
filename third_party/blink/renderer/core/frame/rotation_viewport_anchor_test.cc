...
TEST_F(RotationViewportAnchorTest, SimpleAbsolutePosition)
{
    ...
    layout_viewport->SetScrollOffset(ScrollOffset(3050 - 200, 4050), mojom::blink::ScrollType::kProgrammatic);
...
TEST_F(RotationViewportAnchorTest, PositionRelativeToViewportSize)
{
    ...
    gfx::Point target_position(5 * WebView().MainFrameViewWidget()->Size().width(), 5 * WebView().MainFrameViewWidget()->Size().height());

    // Place the target at the top-center of the viewport. This is where the
    // rotation anchor finds the node to anchor to.
    layout_viewport->SetScrollOffset(ScrollOffset(target_position.x() - WebView().MainFrameViewWidget()->Size().width() / 2 + 25, target_position.y()),
        mojom::blink::ScrollType::kProgrammatic);

    WebView().MainFrameViewWidget()->Resize(gfx::Size(600, 100));
    Compositor().BeginFrame();

    target_position = gfx::Point(5 * WebView().MainFrameViewWidget()->Size().width(), 5 * WebView().MainFrameViewWidget()->Size().height());

    gfx::Point expected_offset(target_position.x() - WebView().MainFrameViewWidget()->Size().width() / 2 + 25, target_position.y());
