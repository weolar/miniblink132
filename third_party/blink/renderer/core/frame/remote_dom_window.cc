...
void RemoteDOMWindow::SchedulePostMessage(PostedMessage* posted_message)
{
    ...
    posted_message->source->GetTaskRunner(TaskType::kInternalPostMessageForwarding)
        ->PostTask(FROM_HERE, WTF::BindOnce(&RemoteDOMWindow::ForwardPostMessage, WrapPersistent(this), WrapPersistent(posted_message)));
}
...
void RemoteDOMWindow::ForwardPostMessage(PostedMessage* posted_message)
{
    ...
    scoped_refptr<const SecurityOrigin> source_origin = posted_message->source_origin;
    scoped_refptr<const SecurityOrigin> target_origin = posted_message->target_origin;
    GetFrame()->ForwardPostMessage(std::move(*posted_message).ToBlinkTransferableMessage(), source_frame, std::move(source_origin), std::move(target_origin));
}
