...
void NavigatorLanguage::EnsureUpdatedLanguage()
{
    if (languages_dirty_) {
        ...
        probe::ApplyAcceptLanguageOverride(execution_context_, &accept_languages_override);
        ...
        if (!accept_languages_override.IsNull()) {
            ...
            if (RuntimeEnabledFeatures::DisableReduceAcceptLanguageEnabled(execution_context_)) {
                UseCounter::Count(execution_context_, WebFeature::kDisableReduceAcceptLanguage);
            } else if (base::FeatureList::IsEnabled(network::features::kReduceAcceptLanguage)) {
