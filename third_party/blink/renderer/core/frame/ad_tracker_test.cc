...
const unsigned char kSmallGifData[] = { 0x47, 0x49, 0x46, 0x38, 0x39, 0x61, 0x01, 0x00, 0x01, 0x00, 0x00, 0xff, 0x00, 0x2c, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00,
...
public:
    explicit TestAdTracker(LocalFrame* frame)
        : AdTracker(frame)
    {
    }
    ...
    void SetAdSuffix(const String& ad_suffix)
    {
        ad_suffix_ = ad_suffix;
    }
...
protected:
    ...
    bool CalculateIfAdSubresource(ExecutionContext* execution_context, const KURL& request_url, ResourceType resource_type,
        const FetchInitiatorInfo& initiator_info, bool ad_request) override
    {
        if (!ad_suffix_.empty() && request_url.GetString().EndsWith(ad_suffix_)) {
            ad_request = true;
        }

        ad_request = AdTracker::CalculateIfAdSubresource(execution_context, request_url, resource_type, initiator_info, ad_request);
...
{
    DCHECK(frame);
    blink::FrameAdEvidence ad_evidence(frame->Parent() && frame->Parent()->IsAdFrame());
...
protected:
    ...
    void WillExecuteScript(const String& script_url, int script_id = v8::Message::kNoScriptIdInfo)
    {
        auto* execution_context = GetExecutionContext();
        ad_tracker_->WillExecuteScript(execution_context, execution_context->GetIsolate()->GetCurrentContext(), String(script_url), script_id);
    }
    ...
    {
        ad_tracker_->DidExecuteScript();
    }

    bool AnyExecutingScriptsTaggedAsAdResource()
    {
        return AnyExecutingScriptsTaggedAsAdResourceWithStackType(AdTracker::StackType::kBottomAndTop);
    }

    bool AnyExecutingScriptsTaggedAsAdResourceWithStackType(AdTracker::StackType stack_type)
    {
...
TEST_F(AdTrackerTest, BottomScriptTaggedAsAdResource)
{
    ...
    EXPECT_TRUE(AnyExecutingScriptsTaggedAsAdResourceWithStackType(AdTracker::StackType::kBottomAndTop));
    EXPECT_TRUE(AnyExecutingScriptsTaggedAsAdResourceWithStackType(AdTracker::StackType::kBottomOnly));
}
...
TEST_F(AdTrackerTest, TopScriptTaggedAsAdResource)
{
    ...
    EXPECT_TRUE(AnyExecutingScriptsTaggedAsAdResourceWithStackType(AdTracker::StackType::kBottomAndTop));
    EXPECT_FALSE(AnyExecutingScriptsTaggedAsAdResourceWithStackType(AdTracker::StackType::kBottomOnly));
}
...
protected:
    ...
    {
        SimTest::SetUp();
        main_resource_ = std::make_unique<SimRequest>("https://example.com/test.html", "text/html");
...
TEST_F(AdTrackerSimTest, ScriptLoadedWhileExecutingAdScript)
{
    ...
    EXPECT_TRUE(IsKnownAdScript(GetDocument().GetExecutionContext(), kVanillaUrl));
...
TEST_F(AdTrackerSimTest, ScriptDetectedByContext)
{
    ...
    EXPECT_TRUE(ad_tracker_->IsAdScriptInStack(AdTracker::StackType::kBottomAndTop));
}
...
TEST_F(AdTrackerSimTest, EventHandlerForPostMessageFromAdFrame_NoAdInStack)
{
    ...
    SimSubresourceRequest image_resource("https://example.com/image.gif", "image/gif");
    ...
    EXPECT_FALSE(ad_tracker_->RequestWithUrlTaggedAsAd("https://example.com/image.gif"));
}
...
TEST_F(AdTrackerSimTest, RedirectToAdUrl)
{
    ...
    SimSubresourceRequest redirect_script("https://example.com/redirect_script.js", "text/javascript", params);
    SimSubresourceRequest ad_script("https://example.com/ad_script.js", "text/javascript");

    ad_tracker_->SetAdSuffix("ad_script.js");

    main_resource_->Complete("<body><script src='redirect_script.js'></script></body>");

    ad_script.Complete("");

    EXPECT_FALSE(ad_tracker_->RequestWithUrlTaggedAsAd("https://example.com/redirect_script.js"));
    EXPECT_TRUE(ad_tracker_->RequestWithUrlTaggedAsAd("https://example.com/ad_script.js"));
}
...
TEST_F(AdTrackerSimTest, AdResourceDetectedByContext)
{
    ...
    // Create an iframe that's considered an ad.
    main_resource_->Complete("<body><iframe src='ad_frame.html'></iframe></body>");
    auto* child_frame = To<LocalFrame>(GetDocument().GetFrame()->Tree().FirstChild());
    SetIsAdFrame(child_frame);
    ...
    EXPECT_TRUE(ad_tracker_->RequestWithUrlTaggedAsAd("https://example.com/foo.css"));
}
...
TEST_F(AdTrackerSimTest, InlineAdScriptRunningInNonAdContext)
{
    ...
    EXPECT_TRUE(To<LocalFrame>(GetDocument().GetFrame()->Tree().ScopedChild(AtomicString("ad_sibling")))->IsFrameCreatedByAdScript());
}
...
TEST_F(AdTrackerSimTest, ImageLoadedWhileExecutingAdScriptAsyncEnabled)
{
    ...
    HTMLImageElement* image_element = Traversal<HTMLImageElement>::FirstChild(*body_element);
...
TEST_F(AdTrackerSimTest, Contexts)
{
    ...
    SimSubresourceRequest library_resource("https://example.com/library.js", "text/javascript");
    ...
    SimSubresourceRequest library_resource_for_subframe("https://example.com/library.js", "text/javascript");
    ...
    EXPECT_TRUE(IsKnownAdScript(local_subframe->GetDocument()->GetExecutionContext(), String("https://example.com/library.js")));

    EXPECT_FALSE(IsKnownAdScript(GetDocument().GetExecutionContext(), String("https://example.com/library.js")));
}

TEST_F(AdTrackerSimTest, SameOriginSubframeFromAdScript)
{
    SimSubresourceRequest ad_resource("https://example.com/ad_script.js", "text/javascript");
    SimRequest iframe_resource("https://example.com/iframe.html", "text/html");
    ad_tracker_->SetAdSuffix("ad_script.js");
    ...
    auto* subframe = To<LocalFrame>(GetDocument().GetFrame()->Tree().FirstChild());
...
class AdTrackerVanillaOrAdSimTest : public AdTrackerSimTest, public ::testing::WithParamInterface<bool> {
public:
    bool IsAdRun()
    {
        return GetParam();
...
TEST_P(AdTrackerVanillaOrAdSimTest, VanillaExternalStylesheetLoadsResources)
{
    ...
    stylesheet.Complete(IsAdRun() ? kStylesheetWithAdResources : kStylesheetWithVanillaResources);
...
TEST_P(AdTrackerVanillaOrAdSimTest, LinkRelStylesheetAddedByScript)
{
    ...
    main_resource_->Complete(IsAdRun() ? kPageWithAdScript : kPageWithVanillaScript);
    ...
    EXPECT_EQ(ad_tracker_->RequestWithUrlTaggedAsAd(vanilla_stylesheet_url), IsAdRun());
    EXPECT_EQ(ad_tracker_->RequestWithUrlTaggedAsAd(vanilla_font_url), IsAdRun());
    EXPECT_EQ(ad_tracker_->RequestWithUrlTaggedAsAd(vanilla_image_url), IsAdRun());
}
...
TEST_P(AdTrackerVanillaOrAdSimTest, VanillaImportInStylesheet)
{
    ...
    SimSubresourceRequest imported_stylesheet(vanilla_imported_stylesheet_url, "text/css");
    SimSubresourceRequest font(vanilla_font_url, "font/woff2");
    SimSubresourceRequest image(vanilla_image_url, "image/png");

    ad_tracker_->SetAdSuffix("ad=true");

    main_resource_->Complete(IsAdRun() ? kPageWithAdExternalStylesheet : kPageWithVanillaExternalStylesheet);
    ...
    EXPECT_EQ(ad_tracker_->RequestWithUrlTaggedAsAd(vanilla_imported_stylesheet_url), IsAdRun());
    EXPECT_EQ(ad_tracker_->RequestWithUrlTaggedAsAd(vanilla_font_url), IsAdRun());
    EXPECT_EQ(ad_tracker_->RequestWithUrlTaggedAsAd(vanilla_image_url), IsAdRun());
}
...
{
    String stylesheet_url = FlipURLOnAdRun("https://example.com/style.css");
    String ad_imported_stylesheet_url = "https://example.com/imported.css?ad=true";
    String vanilla_font_url = "https://example.com/font.woff2";
    String vanilla_image_url = "https://example.com/pixel.png";
    SimSubresourceRequest stylesheet(stylesheet_url, "text/css");
    SimSubresourceRequest imported_stylesheet(ad_imported_stylesheet_url, "text/css");
    SimSubresourceRequest font(vanilla_font_url, "font/woff2");
    SimSubresourceRequest image(vanilla_image_url, "image/png");
    ...
    EXPECT_EQ(ad_tracker_->RequestWithUrlTaggedAsAd(stylesheet_url), IsAdRun());
    EXPECT_TRUE(ad_tracker_->RequestWithUrlTaggedAsAd(ad_imported_stylesheet_url));
...
TEST_F(AdTrackerSimTest, DynamicallyAddedScriptNoSrc_NotTaggedBasedOnUseByAdScript)
{
    String vanilla_script_url = "https://example.com/script.js";
    String ad_script_url = "https://example.com/script.js?ad=true";
...
class AdTrackerDisabledSimTest : public SimTest, private ScopedAdTaggingForTest {
protected:
    AdTrackerDisabledSimTest()
        : ScopedAdTaggingForTest(false)
    {
    }
    void SetUp() override
    {
...
INSTANTIATE_TEST_SUITE_P(All, AdTrackerVanillaOrAdSimTest, ::testing::Values(true, false));
