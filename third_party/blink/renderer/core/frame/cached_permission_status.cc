...
{
    CachedPermissionStatus* cache = Supplement<LocalDOMWindow>::From<CachedPermissionStatus>(window);
    if (!cache) {
...
void CachedPermissionStatus::RegisterClient(Client* client, const Vector<PermissionDescriptorPtr>& permissions)
{
    HashMap<mojom::blink::PermissionName, mojom::blink::PermissionStatus> initialized_map;
    for (const PermissionDescriptorPtr& descriptor : permissions) {
        auto status_it = permission_status_map_.find(descriptor->name);
        PermissionStatus status = status_it != permission_status_map_.end() ? status_it->value : PermissionStatus::ASK;
...
void CachedPermissionStatus::UnregisterClient(Client* client, const Vector<PermissionDescriptorPtr>& permissions)
{
    for (const PermissionDescriptorPtr& descriptor : permissions) {
...
void CachedPermissionStatus::RegisterPermissionObserver(const PermissionDescriptorPtr& descriptor, PermissionStatus current_status)
{
    mojo::PendingRemote<PermissionObserver> observer;
    mojo::ReceiverId id = permission_observer_receivers_.Add(observer.InitWithNewPipeAndPassReceiver(), descriptor->name, GetTaskRunner());
...
{
    permission_status_map_.Set(permission_observer_receivers_.current_context(), status);
}

PermissionService* CachedPermissionStatus::GetPermissionService()
{
    if (!permission_service_.is_bound()) {
        GetSupplementable()->GetBrowserInterfaceBroker().GetInterface(permission_service_.BindNewPipeAndPassReceiver(GetTaskRunner()));
    }

    return permission_service_.get();
}

scoped_refptr<base::SingleThreadTaskRunner> CachedPermissionStatus::GetTaskRunner()
{
