...
bool CanChangeToUrlForHistoryApi(const KURL& url, const SecurityOrigin* document_origin, const KURL& document_url)
{
    if (!url.IsValid()) {
        return false;
    }
    ...
    if (url.Protocol() != document_url.Protocol() || url.User() != document_url.User() || url.Pass() != document_url.Pass() || url.Host() != document_url.Host()
    ...
    is_standard = url::IsStandard(protocol.data(), url::Component(0, static_cast<int>(protocol.size())));
    if (is_standard) {
        return true;
    }
