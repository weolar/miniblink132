...
public:
    void SetUp() override
    {
        ...
        ASSERT_FALSE(GetDocument().GetFrame()->HadStickyUserActivationBeforeNavigation());
    }
    ...
    void Navigate(const String& destinationUrl, bool user_activated)
    {
        ...
        GetDocument().GetFrame()->Loader().CommitNavigation(std::move(navigation_params), nullptr /* extra_data */);
    ...
    void NavigateDifferentDomain()
    {
        Navigate("https://example.org/", false);
    }
};
...
TEST_F(FrameTest, NoGesture)
{
    ...
    LocalFrame::NotifyUserActivation(nullptr, mojom::UserActivationNotificationType::kTest);
...
TEST_F(FrameTest, NavigateDifferentDomain)
{
    LocalFrame::NotifyUserActivation(GetDocument().GetFrame(), mojom::UserActivationNotificationType::kTest);
    EXPECT_TRUE(GetDocument().GetFrame()->HasStickyUserActivation());
    EXPECT_FALSE(GetDocument().GetFrame()->HadStickyUserActivationBeforeNavigation());
...
TEST_F(FrameTest, NavigateSameDomainMultipleTimes)
{
    ...
    EXPECT_FALSE(GetDocument().GetFrame()->HasStickyUserActivation());
    EXPECT_TRUE(GetDocument().GetFrame()->HadStickyUserActivationBeforeNavigation());
...
TEST_F(FrameTest, UserActivationInterfaceTest)
{
    ...
    EXPECT_FALSE(GetDocument().GetFrame()->HasStickyUserActivation());
    EXPECT_FALSE(LocalFrame::HasTransientUserActivation(GetDocument().GetFrame()));
    ...
    EXPECT_TRUE(LocalFrame::ConsumeTransientUserActivation(GetDocument().GetFrame()));

    // After consumption, only the transient bit resets to false.
    EXPECT_TRUE(GetDocument().GetFrame()->HasStickyUserActivation());
    EXPECT_FALSE(LocalFrame::HasTransientUserActivation(GetDocument().GetFrame()));
    EXPECT_FALSE(LocalFrame::ConsumeTransientUserActivation(GetDocument().GetFrame()));
}
...
TEST_F(FrameTest, UserActivationTriggerHistograms)
{
    ...
    LocalFrame::HasTransientUserActivation(GetDocument().GetFrame());
    histograms.ExpectBucketCount("Event.UserActivation.TriggerForTransient", 9, 1);
    ...
    LocalFrame::ConsumeTransientUserActivation(GetDocument().GetFrame());
    histograms.ExpectBucketCount("Event.UserActivation.TriggerForTransient", 9, 2);
    histograms.ExpectBucketCount("Event.UserActivation.TriggerForConsuming", 9, 1);
    ...
    LocalFrame::NotifyUserActivation(GetDocument().GetFrame(), mojom::UserActivationNotificationType::kInteraction);
    GetDocument().GetFrame()->HasStickyUserActivation();
    LocalFrame::HasTransientUserActivation(GetDocument().GetFrame());
    LocalFrame::ConsumeTransientUserActivation(GetDocument().GetFrame());
    histograms.ExpectBucketCount("Event.UserActivation.TriggerForConsuming", 1, 1);
    histograms.ExpectBucketCount("Event.UserActivation.TriggerForSticky", 9, 3);
    histograms.ExpectBucketCount("Event.UserActivation.TriggerForTransient", 1, 2);
...
{
    ScrollTimeline::Create(&GetDocument(), GetDocument().ScrollingElementNoLayout(), ScrollTimeline::ScrollAxis::kBlock);

    EXPECT_EQ(GetDocument().GetFrame()->GetScrollSnapshotClientsForTesting().size(), 1U);
    NavigateSameDomain("page1");
    EXPECT_EQ(GetDocument().GetFrame()->GetScrollSnapshotClientsForTesting().size(), 0U);
}
