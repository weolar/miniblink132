...
ScreenMetricsEmulator::ScreenMetricsEmulator(WebFrameWidgetImpl* frame_widget, const display::ScreenInfos& screen_infos, const gfx::Size& widget_size,
    const gfx::Size& visible_viewport_size, const gfx::Rect& view_screen_rect, const gfx::Rect& window_screen_rect)
...
const display::ScreenInfo& ScreenMetricsEmulator::GetOriginalScreenInfo() const
{
...
void ScreenMetricsEmulator::DisableAndApply()
{
    ...
    frame_widget_->SetScreenRects(original_view_screen_rect_, original_window_screen_rect_);
    frame_widget_->SetViewportSegments(original_root_viewport_segments_);
    frame_widget_->SetScreenInfoAndSize(original_screen_infos_, original_widget_size_, original_visible_viewport_size_);
...
void ScreenMetricsEmulator::ChangeEmulationParams(const DeviceEmulationParams& params)
{
...
void ScreenMetricsEmulator::Apply()
{
    ...
    if (emulation_params_.view_size.width()) {
        ...
        widget_size.set_width(base::ClampRound(widget_size.width() / emulation_params_.scale));
    }
    if (emulation_params_.view_size.height()) {
        widget_size.set_height(emulation_params_.view_size.height());
    } else {
        widget_size.set_height(base::ClampRound(widget_size.height() / emulation_params_.scale));
    }
    ...
    const display::ScreenInfo& original_screen_info = original_screen_infos_.current();
    ...
    display::mojom::blink::ScreenOrientation orientation_type = original_screen_info.orientation_type;
    uint16_t orientation_angle = original_screen_info.orientation_angle;
    if (emulation_params_.screen_orientation_type != display::mojom::blink::ScreenOrientation::kUndefined) {
    ...
    modified_emulation_params.device_scale_factor = original_screen_info.device_scale_factor;
    frame_widget_->SetScreenMetricsEmulationParameters(true, std::move(modified_emulation_params));

    frame_widget_->SetScreenRects(gfx::Rect(widget_pos, widget_size), gfx::Rect(window_pos, window_size));
    ...
    if (has_emulated_segments) {
        ...
        std::vector<gfx::Rect> emulated_segments { { 0, 0, widget_size.width(), widget_size.height() } };
        frame_widget_->SetViewportSegments(emulated_segments);
    }

    frame_widget_->OverrideDevicePostureForEmulation(emulation_params_.device_posture);

    display::ScreenInfos emulated_screen_infos = original_screen_infos_;
    display::ScreenInfo& emulated_screen_info = emulated_screen_infos.mutable_current();
...
void ScreenMetricsEmulator::UpdateVisualProperties(const VisualProperties& visual_properties)
{
    ...
    original_root_viewport_segments_ = visual_properties.root_widget_viewport_segments;
    Apply();
    ...
    frame_widget_->UpdateSurfaceAndCompositorRect(
        visual_properties.local_surface_id.value_or(viz::LocalSurfaceId()), visual_properties.compositor_viewport_pixel_rect);
}

void ScreenMetricsEmulator::OnUpdateScreenRects(const gfx::Rect& view_screen_rect, const gfx::Rect& window_screen_rect)
{
    original_view_screen_rect_ = view_screen_rect;
    original_window_screen_rect_ = window_screen_rect;
    if (emulating_desktop()) {
