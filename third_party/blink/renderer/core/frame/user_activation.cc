...
UserActivation* UserActivation::CreateSnapshot(LocalDOMWindow* window)
{
    ...
    return MakeGarbageCollected<UserActivation>(frame ? frame->HasStickyUserActivation() : false, LocalFrame::HasTransientUserActivation(frame));
}

UserActivation::UserActivation(bool has_been_active, bool is_active)
    : has_been_active_(has_been_active)
