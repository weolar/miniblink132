...
class CORE_EXPORT AttributionSrcLoader : public GarbageCollected<AttributionSrcLoader> {
public:
    ...
    void Register(const AtomicString& attribution_src, HTMLElement* element, network::mojom::ReferrerPolicy);
    ...
    bool MaybeRegisterAttributionHeaders(const ResourceRequest& request, const ResourceResponse& response);
    ...
    [[nodiscard]] std::optional<Impression> RegisterNavigation(const KURL& navigation_url, const AtomicString& attribution_src, HTMLAnchorElementBase* element,
        bool has_transient_user_activation, network::mojom::ReferrerPolicy);
    ...
    [[nodiscard]] std::optional<Impression> RegisterNavigation(
        const KURL& navigation_url, const WebVector<WebString>& attribution_srcs, bool has_transient_user_activation, network::mojom::ReferrerPolicy);
    ...
    // `log_issues` is true.
    [[nodiscard]] bool CanRegister(const KURL& url, HTMLElement* element, std::optional<uint64_t> request_id, bool log_issues = true);
...
private:
    ...
    Vector<KURL> ParseAttributionSrc(const AtomicString& attribution_src, HTMLElement*);

    bool DoRegistration(const Vector<KURL>&, std::optional<AttributionSrcToken>, network::mojom::ReferrerPolicy);
    ...
    [[nodiscard]] std::optional<Impression> RegisterNavigationInternal(const KURL& navigation_url, Vector<KURL> attribution_src_urls, HTMLAnchorElementBase*,
        bool has_transient_user_activation, network::mojom::ReferrerPolicy);
    ...
    std::optional<attribution_reporting::SuitableOrigin> ReportingOriginForUrlIfValid(
        const KURL& url, HTMLElement* element, std::optional<uint64_t> request_id, bool log_issues = true);

    bool CreateAndSendRequests(Vector<KURL>, std::optional<AttributionSrcToken>, network::mojom::ReferrerPolicy);

    struct AttributionHeaders;

    void RegisterAttributionHeaders(attribution_reporting::mojom::blink::RegistrationEligibility, network::mojom::AttributionSupport,
        attribution_reporting::SuitableOrigin reporting_origin, const AttributionHeaders&, const attribution_reporting::RegistrationInfo&,
