...
bool operator==(const InputHandlerProxy::DidOverscrollParams& lhs, const InputHandlerProxy::DidOverscrollParams& rhs)
{
    return lhs.accumulated_overscroll == rhs.accumulated_overscroll && lhs.latest_overscroll_delta == rhs.latest_overscroll_delta
        && lhs.current_fling_velocity == rhs.current_fling_velocity && lhs.causal_event_viewport_point == rhs.causal_event_viewport_point
...
TEST_F(WebFrameWidgetSimTest, AutoResizeAllocatedLocalSurfaceId)
{
    ...
    WebView().MainFrameViewWidget()->UpdateSurfaceAndScreenInfo(
        visual_properties.local_surface_id.value(), visual_properties.compositor_viewport_pixel_rect, visual_properties.screen_infos);

    EXPECT_EQ(allocator.GetCurrentLocalSurfaceId(), WebView().MainFrameViewWidget()->LocalSurfaceIdFromParent());
    EXPECT_FALSE(WebView().MainFrameViewWidget()->LayerTreeHostForTesting()->new_local_surface_id_request_for_testing());

    constexpr gfx::Size size(200, 200);
    WebView().MainFrameViewWidget()->DidAutoResize(size);
    EXPECT_EQ(allocator.GetCurrentLocalSurfaceId(), WebView().MainFrameViewWidget()->LocalSurfaceIdFromParent());
    EXPECT_TRUE(WebView().MainFrameViewWidget()->LayerTreeHostForTesting()->new_local_surface_id_request_for_testing());
}
...
TEST_F(WebFrameWidgetSimTest, FrameSinkIdHitTestAPI)
{
    ...
    gfx::PointF point;
    viz::FrameSinkId main_frame_sink_id = WebView().MainFrameViewWidget()->GetFrameSinkIdAtPoint(gfx::PointF(10.43, 10.74), &point);
    EXPECT_EQ(WebView().MainFrameViewWidget()->GetFrameSinkId(), main_frame_sink_id);
    EXPECT_EQ(gfx::PointF(10.43, 10.74), point);

    // Targeting a child frame should also return the FrameSinkId for the main
    // widget.
    viz::FrameSinkId frame_sink_id = WebView().MainFrameViewWidget()->GetFrameSinkIdAtPoint(gfx::PointF(150.27, 150.25), &point);
...
{
    const cc::LayerTreeHost* layer_tree_host = WebView().MainFrameViewWidget()->LayerTreeHostForTesting();
    // We should not have any force send metadata requests at start.
    EXPECT_FALSE(layer_tree_host->pending_commit_state()->force_send_metadata_request);
    ...
    EXPECT_TRUE(layer_tree_host->pending_commit_state()->force_send_metadata_request);
}
...
public:
    void SetUp() override
    {
        SimTest::SetUp();
        InitializeRemote();
        CHECK(static_cast<WebFrameWidgetImpl*>(LocalFrameRoot().FrameWidget())->ForSubframe());
    }
...
// roots.
TEST_F(WebFrameWidgetImplRemoteFrameSimTest, ActivePinchGestureUpdatesLayerTreeHostSubFrame)
{
    cc::LayerTreeHost* layer_tree_host = LocalFrameRootWidget()->LayerTreeHostForTesting();
...
public:
    ...
    MOCK_METHOD4_T(Run, void(mojom::InputEventResultState, const ui::LatencyInfo&, InputHandlerProxy::DidOverscrollParams*, std::optional<cc::TouchAction>));

    WidgetBaseInputHandler::HandledEventCallback GetCallback()
    {
        return WTF::BindOnce(&MockHandledEventCallback::HandleCallback, WTF::Unretained(this));
    }

private:
    void HandleCallback(mojom::InputEventResultState ack_state, const ui::LatencyInfo& latency_info,
        std::unique_ptr<InputHandlerProxy::DidOverscrollParams> overscroll, std::optional<cc::TouchAction> touch_action)
    {
...
public:
    ...
    MOCK_METHOD1(HandleInputEvent, WebInputEventResult(const WebCoalescedInputEvent&));
...
public:
    frame_test_helpers::TestWebFrameWidget* CreateWebFrameWidget(base::PassKey<WebLocalFrame> pass_key,
    ...
    {
        return MakeGarbageCollected<MockWebFrameWidgetImpl>(pass_key, std::move(frame_widget_host), std::move(frame_widget), std::move(widget_host),
            std::move(widget), std::move(task_runner), frame_sink_id, hidden, never_composited, is_for_child_local_root, is_for_nested_main_frame,
    ...
    void SendInputEvent(const WebInputEvent& event, WidgetBaseInputHandler::HandledEventCallback callback)
    {
        MockMainFrameWidget()->ProcessInputEventSynchronouslyForTesting(WebCoalescedInputEvent(event.Clone(), {}, {}, ui::LatencyInfo()), std::move(callback));
    }
...
TEST_F(WebFrameWidgetImplSimTest, CursorChange)
{
    ...
    frame_test_helpers::TestWebFrameWidgetHost& widget_host = MockMainFrameWidget()->WidgetHost();
    ...
    EXPECT_CALL(*MockMainFrameWidget(), HandleInputEvent(_)).WillOnce(::testing::Return(WebInputEventResult::kNotHandled));
    SendInputEvent(SyntheticWebMouseEventBuilder::Build(WebInputEvent::Type::kMouseLeave), base::DoNothing());
    base::RunLoop().RunUntilIdle();
    EXPECT_EQ(widget_host.CursorSetCount(), 1u);
...
TEST_F(WebFrameWidgetImplSimTest, RenderWidgetInputEventUmaMetrics)
{
    ...
    EXPECT_CALL(*MockMainFrameWidget(), HandleInputEvent(_)).Times(5).WillRepeatedly(::testing::Return(WebInputEventResult::kNotHandled));
    EXPECT_CALL(*MockMainFrameWidget(), DispatchBufferedTouchEvents()).Times(5).WillRepeatedly(::testing::Return(WebInputEventResult::kNotHandled));
    SendInputEvent(touch, base::DoNothing());
    histogram_tester().ExpectBucketCount(EVENT_LISTENER_RESULT_HISTOGRAM, PASSIVE_LISTENER_UMA_ENUM_CANCELABLE, 1);

    touch.dispatch_type = WebInputEvent::DispatchType::kEventNonBlocking;
    SendInputEvent(touch, base::DoNothing());
    histogram_tester().ExpectBucketCount(EVENT_LISTENER_RESULT_HISTOGRAM, PASSIVE_LISTENER_UMA_ENUM_UNCANCELABLE, 1);

    touch.dispatch_type = WebInputEvent::DispatchType::kListenersNonBlockingPassive;
    SendInputEvent(touch, base::DoNothing());
    histogram_tester().ExpectBucketCount(EVENT_LISTENER_RESULT_HISTOGRAM, PASSIVE_LISTENER_UMA_ENUM_PASSIVE, 1);

    touch.dispatch_type = WebInputEvent::DispatchType::kListenersForcedNonBlockingDueToFling;
    SendInputEvent(touch, base::DoNothing());
    histogram_tester().ExpectBucketCount(EVENT_LISTENER_RESULT_HISTOGRAM, PASSIVE_LISTENER_UMA_ENUM_FORCED_NON_BLOCKING_DUE_TO_FLING, 1);

    touch.MovePoint(0, 10, 10);
    touch.touch_start_or_first_touch_move = true;
    touch.dispatch_type = WebInputEvent::DispatchType::kListenersForcedNonBlockingDueToFling;
    SendInputEvent(touch, base::DoNothing());
    histogram_tester().ExpectBucketCount(EVENT_LISTENER_RESULT_HISTOGRAM, PASSIVE_LISTENER_UMA_ENUM_FORCED_NON_BLOCKING_DUE_TO_FLING, 2);

    EXPECT_CALL(*MockMainFrameWidget(), HandleInputEvent(_)).WillOnce(::testing::Return(WebInputEventResult::kNotHandled));
    EXPECT_CALL(*MockMainFrameWidget(), DispatchBufferedTouchEvents()).WillOnce(::testing::Return(WebInputEventResult::kHandledSuppressed));
    touch.dispatch_type = WebInputEvent::DispatchType::kBlocking;
    SendInputEvent(touch, base::DoNothing());
    histogram_tester().ExpectBucketCount(EVENT_LISTENER_RESULT_HISTOGRAM, PASSIVE_LISTENER_UMA_ENUM_SUPPRESSED, 1);

    EXPECT_CALL(*MockMainFrameWidget(), HandleInputEvent(_)).WillOnce(::testing::Return(WebInputEventResult::kNotHandled));
    EXPECT_CALL(*MockMainFrameWidget(), DispatchBufferedTouchEvents()).WillOnce(::testing::Return(WebInputEventResult::kHandledApplication));
    touch.dispatch_type = WebInputEvent::DispatchType::kBlocking;
    SendInputEvent(touch, base::DoNothing());
    histogram_tester().ExpectBucketCount(EVENT_LISTENER_RESULT_HISTOGRAM, PASSIVE_LISTENER_UMA_ENUM_CANCELABLE_AND_CANCELED, 1);
}
...
{
    WebGestureEvent scroll(WebInputEvent::Type::kGestureScrollUpdate, WebInputEvent::kNoModifiers, base::TimeTicks::Now(), WebGestureDevice::kTouchpad);
    ...
    // HandleInputEvent().
    EXPECT_CALL(*MockMainFrameWidget(), ObserveGestureEventAndResult(_, _, _, _)).Times(1);
    EXPECT_CALL(*MockMainFrameWidget(), HandleInputEvent(_)).Times(testing::AnyNumber());
...
{
    WebGestureEvent scroll(WebInputEvent::Type::kGestureScrollUpdate, WebInputEvent::kNoModifiers, base::TimeTicks::Now(), WebGestureDevice::kTouchscreen);
    scroll.SetPositionInWidget(gfx::PointF(-10, 0));
    scroll.data.scroll_update.delta_y = 10;
...
TEST_F(WebFrameWidgetImplSimTest, TestStartStylusWritingForInputElement)
{
    ...
    Compositor().BeginFrame();
    Element* first = DynamicTo<Element>(GetDocument().getElementById(AtomicString("first")));
    WebPointerEvent event(WebInputEvent::Type::kPointerDown,
        WebPointerProperties(1, WebPointerProperties::PointerType::kPen, WebPointerProperties::Button::kLeft, gfx::PointF(100, 100), gfx::PointF(100, 100)), 1,
        1);
    GetEventHandler().HandlePointerEvent(event, Vector<WebPointerEvent>(), Vector<WebPointerEvent>());
    EXPECT_EQ(nullptr, GetDocument().FocusedElement());
    OnStartStylusWriting();
    EXPECT_EQ(first, GetDocument().FocusedElement());
}

TEST_F(WebFrameWidgetImplSimTest, TestStartStylusWritingForContentEditableElement)
{
    ScopedStylusHandwritingForTest enable_stylus_handwriting(true);
    WebView().MainFrameViewWidget()->Resize(gfx::Size(400, 400));
    SimRequest request("https://example.com/test.html", "text/html");
...
TEST_F(WebFrameWidgetImplSimTest, TestStartStylusWritingForContentEditableChildElement)
{
    ScopedStylusHandwritingForTest enable_stylus_handwriting(true);
    WebView().MainFrameViewWidget()->Resize(gfx::Size(400, 400));
    SimRequest request("https://example.com/test.html", "text/html");
    ...
    Compositor().BeginFrame();
    Element* first = DynamicTo<Element>(GetDocument().getElementById(AtomicString("first")));
    Element* second = DynamicTo<Element>(GetDocument().getElementById(AtomicString("second")));
    WebPointerEvent event(WebInputEvent::Type::kPointerDown,
        WebPointerProperties(1, WebPointerProperties::PointerType::kPen, WebPointerProperties::Button::kLeft, gfx::PointF(100, 100), gfx::PointF(100, 100)), 1,
        1);
    GetEventHandler().HandlePointerEvent(event, Vector<WebPointerEvent>(), Vector<WebPointerEvent>());
...
std::ostream& operator<<(std::ostream& os, const ProximateBoundsCollectionArgs& args)
{
...
struct WebFrameWidgetProximateBoundsCollectionSimTestParam {
    ...
    const std::string& GetHTMLDocument() const
    {
        return html_document_;
    }
    ...
    {
        return proximate_bounds_collection_args_.get_focus_rect_in_widget.Run(document);
    }
...
private:
    friend std::ostream& operator<<(std::ostream& os, const WebFrameWidgetProximateBoundsCollectionSimTestParam& param);
...
std::ostream& operator<<(std::ostream& os, const WebFrameWidgetProximateBoundsCollectionSimTestParam& param)
{
    return os << "\nenable_stylus_handwriting_win: " << param.enable_stylus_handwriting_win_ << "\nhtml_document: " << param.html_document_
              << "\nproximate_bounds_collection_args: {" << param.proximate_bounds_collection_args_ << "}";
}

class WebFrameWidgetProximateBoundsCollectionSimTestBase : public WebFrameWidgetImplSimTest {
public:
    void LoadDocument(const String& html_document)
    {
        WebView().MainFrameViewWidget()->Resize(gfx::Size(400, 400));
        SimRequest request("https://example.com/test.html", "text/html");
        SimSubresourceRequest style_resource("https://example.com/styles.css", "text/css");
        SimSubresourceRequest font_resource("https://example.com/Ahem.woff2", "font/woff2");
        LoadURL("https://example.com/test.html");
      ...
      #touch_fallback {
        ...
        font_resource.Complete(*test::ReadFromFile(test::CoreTestDataPath("Ahem.woff2")));
        Compositor().BeginFrame();
    }
    ...
    void HandlePointerDownEventOverTouchFallback()
    {
        ...
        const WebPointerEvent event(WebInputEvent::Type::kPointerDown,
            WebPointerProperties(
                1, WebPointerProperties::PointerType::kPen, WebPointerProperties::Button::kLeft, gfx::PointF(tap_point), gfx::PointF(tap_point)),
            1, 1);
        GetEventHandler().HandlePointerEvent(event, Vector<WebPointerEvent>(), Vector<WebPointerEvent>());
    ...
    {
        MockMainFrameWidget()->OnStartStylusWriting(focus_rect_in_widget,
            base::BindOnce(&WebFrameWidgetProximateBoundsCollectionSimTestBase::OnStartStylusWritingComplete, weak_factory_.GetWeakPtr()));
    }
    ...
    const mojom::blink::ProximateCharacterRangeBounds* GetLastProximateBounds() const
    {
        return last_proximate_bounds_.get();
    }

protected:
    explicit WebFrameWidgetProximateBoundsCollectionSimTestBase(bool enable_stylus_handwriting_win)
    {
        if (enable_stylus_handwriting_win) {
            ...
            scoped_feature_list_.InitWithFeaturesAndParameters(
                /*enabled_features=*/
                { { stylus_handwriting::win::kStylusHandwritingWin, base::FieldTrialParams() },
                    { stylus_handwriting::win::kProximateBoundsCollection,
                        base::FieldTrialParams({ { stylus_handwriting::win::kProximateBoundsCollectionHalfLimit.name, base::NumberToString(-2) } }) } },
                /*disabled_features=*/ {});
            enable_stylus_handwriting_.emplace(true);
        } else {
            scoped_feature_list_.InitWithFeaturesAndParameters(
                /*enabled_features=*/ {},
                /*disabled_features=*/ { stylus_handwriting::win::kStylusHandwritingWin });
        }
    }

private:
    void OnStartStylusWritingComplete(mojom::blink::StylusWritingFocusResultPtr focus_result)
    {
        last_proximate_bounds_ = focus_result ? std::move(focus_result->proximate_bounds) : nullptr;
    }
    ...
    base::WeakPtrFactory<WebFrameWidgetProximateBoundsCollectionSimTestBase> weak_factory_ { this };
};

class WebFrameWidgetProximateBoundsCollectionSimTestF : public WebFrameWidgetProximateBoundsCollectionSimTestBase {
public:
    ...
    {
        gfx::Rect focus_rect_in_widget(element.BoundsInWidget().CenterPoint(), gfx::Size());
        focus_rect_in_widget.Outset(gfx::Outsets(25));
        OnStartStylusWriting(focus_rect_in_widget);
    }
};

class WebFrameWidgetProximateBoundsCollectionSimTestP : public WebFrameWidgetProximateBoundsCollectionSimTestBase,
                                                        public testing::WithParamInterface<WebFrameWidgetProximateBoundsCollectionSimTestParam> {
public:
    WebFrameWidgetProximateBoundsCollectionSimTestP()
        : WebFrameWidgetProximateBoundsCollectionSimTestBase(
            /*enable_stylus_handwriting_win=*/GetParam().IsStylusHandwritingWinEnabled())
    {
    }
};

TEST_F(WebFrameWidgetProximateBoundsCollectionSimTestF, ProximateBoundsDisabled)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitWithFeaturesAndParameters(
        /*enabled_features=*/
        { { stylus_handwriting::win::kProximateBoundsCollection,
            base::FieldTrialParams({ { stylus_handwriting::win::kProximateBoundsCollectionHalfLimit.name, base::NumberToString(0) } }) } },
        /*disabled_features=*/ {});
...
INSTANTIATE_TEST_SUITE_P(All, WebFrameWidgetProximateBoundsCollectionSimTestP,
    ::testing::ConvertGenerator<WebFrameWidgetProximateBoundsCollectionSimTestParam::TupleType>(testing::Combine(
        ...
        testing::Values(
            ...
            ProximateBoundsCollectionArgs { /*get_focus_rect_in_widget=*/base::BindRepeating([](const Document& document) -> gfx::Rect {
                                               const Element* target = document.getElementById(AtomicString("target_editable"));
                                               gfx::Rect focus_rect_in_widget(target->BoundsInWidget().top_center(), gfx::Size());
                                               focus_rect_in_widget.Outset(gfx::Outsets(25));
                ...
                /*expected_bounds=*/
                { gfx::Rect(110, 0, 10, 10), gfx::Rect(120, 0, 10, 10), gfx::Rect(130, 0, 10, 10), gfx::Rect(140, 0, 10, 10) } },
            ...
            ProximateBoundsCollectionArgs { /*get_focus_rect_in_widget=*/base::BindRepeating([](const Document& document) -> gfx::Rect {
                                               const Element* target = document.getElementById(AtomicString("target_editable"));
                                               gfx::Rect focus_rect_in_widget(target->BoundsInWidget().origin(), gfx::Size());
                                               focus_rect_in_widget.Outset(gfx::Outsets(25));
                                               return focus_rect_in_widget;
                                           }),
                /*expected_focus_id=*/"target_editable",
            ...
            ProximateBoundsCollectionArgs { /*get_focus_rect_in_widget=*/base::BindRepeating([](const Document& document) -> gfx::Rect {
                                               const Element* target = document.getElementById(AtomicString("target_editable"));
                                               gfx::Rect focus_rect_in_widget(target->BoundsInWidget().top_right() - gfx::Vector2d(1, 0), gfx::Size());
                                               focus_rect_in_widget.Outset(gfx::Outsets(25));
                                               return focus_rect_in_widget;
                                           }),
                /*expected_focus_id=*/"target_editable",
            ...
            ProximateBoundsCollectionArgs { /*get_focus_rect_in_widget=*/base::BindRepeating([](const Document& document) -> gfx::Rect {
                                               const Element* target = document.getElementById(AtomicString("target_editable"));
                                               gfx::Rect focus_rect_in_widget(target->BoundsInWidget().right_center() + gfx::Vector2d(100, 0), gfx::Size());
                                               focus_rect_in_widget.Outset(gfx::Outsets(25));
                                               return focus_rect_in_widget;
                                           }),
            ...
            // determined.
            ProximateBoundsCollectionArgs { /*get_focus_rect_in_widget=*/base::BindRepeating([](const Document& document) -> gfx::Rect {
                                               const Element* target = document.getElementById(AtomicString("target_readonly"));
                                               gfx::Rect focus_rect_in_widget(target->BoundsInWidget().CenterPoint(), gfx::Size());
                                               focus_rect_in_widget.Outset(gfx::Outsets(25));
                                               return focus_rect_in_widget;
                                           }),
                /*expected_focus_id=*/"touch_fallback",
...
TEST_P(WebFrameWidgetProximateBoundsCollectionSimTestP, TestProximateBoundsCollection)
{
    ...
    // Focus expectations.
    const Element* expected_focus = GetElementById(GetParam().GetExpectedFocusId().c_str());
    ...
    if (!GetParam().ExpectNullProximateBounds()) {
        ...
        EXPECT_TRUE(std::equal(GetLastProximateBounds()->bounds.begin(), GetLastProximateBounds()->bounds.end(), GetParam().GetExpectedBounds().begin(),
...
public:
    void SetUp() override
    {
        ...
        auto* root_layer = FrameWidgetBase()->LayerTreeHostForTesting()->root_layer();
    ...
    void CompositeAndWaitForPresentation(base::TimeDelta swap_to_presentation)
    {
        ...
        static_cast<WebFrameWidgetImpl*>(MainFrame().FrameWidget())
            ->NotifySwapAndPresentationTimeForTesting({ WTF::BindOnce(
                                                            [](base::OnceClosure swap_quit_closure, base::TimeTicks* swap_time, base::TimeTicks timestamp) {
                ...
                WTF::BindOnce(
                    [](base::OnceClosure presentation_quit_closure, const viz::FrameTimingDetails& presentation_details) {
                        base::TimeTicks timestamp = presentation_details.presentation_feedback.timestamp;
                        CHECK(!timestamp.is_null());
        ...
        if (!swap_to_presentation.is_zero()) {
            timing_details.presentation_feedback = gfx::PresentationFeedback(swap_time + swap_to_presentation, base::Milliseconds(16), 0);
        }
...
TEST_F(NotifySwapTimesWebFrameWidgetTest, NotifyOnSuccessfulPresentation)
{
    ...
    WebFrameWidgetImpl::PromiseCallbacks callbacks = { base::BindLambdaForTesting([&](base::TimeTicks timestamp) {
                                                          ...
                                                          successful_presentation_time = failed_presentation_time + failed_to_successful;

                                                          swap_run_loop.Quit();
                                                      }),
        base::BindLambdaForTesting([&](const viz::FrameTimingDetails& presentation_details) {
            base::TimeTicks timestamp = presentation_details.presentation_feedback.timestamp;
            CHECK(!timestamp.is_null());
    ...
    constexpr gfx::CALayerResult ca_layer_error_code = gfx::kCALayerFailedTileNotCandidate;

    callbacks.core_animation_error_code_callback = base::BindLambdaForTesting([&](gfx::CALayerResult core_animation_error_code) {
    ...
    // Register callbacks for swap and presentation times.
    static_cast<WebFrameWidgetImpl*>(MainFrame().FrameWidget())->NotifySwapAndPresentationTimeForTesting(std::move(callbacks));
    ...
    failed_timing_details.presentation_feedback
        = gfx::PresentationFeedback(failed_presentation_time, base::Milliseconds(16), gfx::PresentationFeedback::kFailure);
    GetWebFrameWidget().LastCreatedFrameSink()->NotifyDidPresentCompositorFrame(1, failed_timing_details);

    // Respond with a successful presentation feedback.
    DCHECK(!successful_presentation_time.is_null());
    viz::FrameTimingDetails successful_timing_details;
    successful_timing_details.presentation_feedback = gfx::PresentationFeedback(successful_presentation_time, base::Milliseconds(16), 0);
#if BUILDFLAG(IS_MAC)
    successful_timing_details.presentation_feedback.ca_layer_error_code = ca_layer_error_code;
#endif
    GetWebFrameWidget().LastCreatedFrameSink()->NotifyDidPresentCompositorFrame(2, successful_timing_details);
...
TEST_F(NotifySwapTimesWebFrameWidgetTest, ReportPresentationOnlyOnSuccessfulCommit)
{
    base::HistogramTester histograms;
    ...
    // Register callbacks for swap and presentation times.
    static_cast<WebFrameWidgetImpl*>(MainFrame().FrameWidget())
        ->NotifySwapAndPresentationTimeForTesting({ base::BindLambdaForTesting([&](base::TimeTicks timestamp) {
                                                       DCHECK(!timestamp.is_null());
    ...
    layer_tree_host->GetSwapPromiseManager()->BreakSwapPromises(cc::SwapPromise::DidNotSwapReason::COMMIT_FAILS);
    ...
    frame_timing_details.presentation_feedback = gfx::PresentationFeedback(presentation_time, delta, 0);
    GetWebFrameWidget().LastCreatedFrameSink()->NotifyDidPresentCompositorFrame(1, frame_timing_details);
...
{
    auto* layer_tree_host = WebView().MainFrameViewWidget()->LayerTreeHostForTesting();
    EXPECT_FALSE(layer_tree_host->is_external_pinch_gesture_active_for_testing());
    VisualProperties visual_properties;
    visual_properties.screen_infos = display::ScreenInfos(display::ScreenInfo());
...
class WebFrameWidgetInputEventsSimTest : public WebFrameWidgetSimTest, public testing::WithParamInterface<bool> {
public:
    WebFrameWidgetInputEventsSimTest()
    {
        if (GetParam()) {
            feature_list_.InitAndEnableFeature(features::kPausePagesPerBrowsingContextGroup);
        } else {
            feature_list_.InitAndDisableFeature(features::kPausePagesPerBrowsingContextGroup);
        }
    }
...
INSTANTIATE_TEST_SUITE_P(All, WebFrameWidgetInputEventsSimTest, testing::Values(true, false));
...
TEST_P(WebFrameWidgetInputEventsSimTest, DispatchBufferedTouchEvents)
{
    ...
    Window().addEventListener(event_type_names::kTouchmove, listener, MakeGarbageCollected<AddEventListenerOptionsResolved>());
    ...
    touch.PressPoint(10, 10);
    touch.touch_start_or_first_touch_move = true;
    widget->ProcessInputEventSynchronouslyForTesting(WebCoalescedInputEvent(touch.Clone(), {}, {}, ui::LatencyInfo()), base::DoNothing());
    ...
    const base::UnguessableToken browsing_context_group_token = WebView().GetPage()->BrowsingContextGroupToken();
    ...
    widget->ProcessInputEventSynchronouslyForTesting(WebCoalescedInputEvent(touch.Clone(), {}, {}, ui::LatencyInfo()), base::DoNothing());
    EXPECT_TRUE(WebFrameWidgetImpl::IgnoreInputEvents(browsing_context_group_token));
    ...
    widget->StartDragging(MainFrame().GetFrame(), WebDragData(), kDragOperationCopy, SkBitmap(), gfx::Vector2d(), gfx::Rect());
    widget->ProcessInputEventSynchronouslyForTesting(WebCoalescedInputEvent(touch.Clone(), {}, {}, ui::LatencyInfo()), base::DoNothing());
    EXPECT_TRUE(widget->DoingDragAndDrop());
    EXPECT_FALSE(WebFrameWidgetImpl::IgnoreInputEvents(browsing_context_group_token));
    EXPECT_FALSE(listener->GetInvokedStateAndReset());
}
...
TEST_F(WebFrameWidgetSimTest, PropagateScaleToRemoteFrames)
{
    ...
    EXPECT_TRUE(WebView().MainFrame()->FirstChild());
    {
        ...
        frame_test_helpers::SwapRemoteFrame(grandchild, frame_test_helpers::CreateRemote());
    }
    auto* widget = WebView().MainFrameViewWidget();
    widget->SetPageScaleStateAndLimits(1.3f, true, 1.0f, 3.0f);
    EXPECT_EQ(
        To<WebRemoteFrameImpl>(WebView().MainFrame()->FirstChild()->FirstChild())->GetFrame()->GetPendingVisualPropertiesForTesting().page_scale_factor, 1.3f);
    WebView().MainFrame()->FirstChild()->FirstChild()->Detach();
}

TEST_F(WebFrameWidgetSimTest, TestLineBoundsAreEmptyBeforeFocus)
{
    std::unique_ptr<ScopedReportVisibleLineBoundsForTest> enabled = std::make_unique<ScopedReportVisibleLineBoundsForTest>(true);
...
TEST_F(WebFrameWidgetSimTest, TestLineBoundsAreCorrectAfterFocusChange)
{
    ...
    font_resource.Complete(*test::ReadFromFile(test::CoreTestDataPath("Ahem.woff2")));
    Compositor().BeginFrame();
    HTMLInputElement* first = DynamicTo<HTMLInputElement>(GetDocument().getElementById(AtomicString("first")));
    HTMLInputElement* second = DynamicTo<HTMLInputElement>(GetDocument().getElementById(AtomicString("second")));
    ...
    second->Focus();
    gfx::Point origin = second->GetBoundingClientRect()->ToEnclosingRect().origin();
    widget->UpdateAllLifecyclePhases(DocumentUpdateReason::kTest);
...
{
    base::test::ScopedFeatureList feature_list(ScopedDesktopPWAsAdditionalWindowingControlsForTest);
...
TEST_F(WebFrameWidgetSimTest, TestLineBoundsAreCorrectAfterLayoutChange)
{
    ...
    GetDocument().getElementById(AtomicString("d"))->setAttribute(html_names::kStyleAttr, AtomicString("height: 200px"));
    widget->UpdateAllLifecyclePhases(DocumentUpdateReason::kTest);
    Vector<gfx::Rect>& actual = widget->GetVisibleLineBoundsOnScreen();
    for (wtf_size_t i = 0; i < expected.size(); ++i) {
        EXPECT_EQ(expected.at(i), actual.at(i));
...
TEST_F(WebFrameWidgetSimTest, TestLineBoundsAreCorrectAfterPageScroll)
{
    ...
    font_resource.Complete(*test::ReadFromFile(test::CoreTestDataPath("Ahem.woff2")));
    Compositor().BeginFrame();
    HTMLTextAreaElement* first = DynamicTo<HTMLTextAreaElement>(GetDocument().getElementById(AtomicString("first")));
    // Focus the element and check the line bounds.
    first->Focus();
    widget->UpdateAllLifecyclePhases(DocumentUpdateReason::kTest);
    ...
    widget->FocusedLocalFrameInWidget()->View()->LayoutViewport()->ScrollBy(ScrollOffset(0, 50), mojom::blink::ScrollType::kUser);
    widget->UpdateAllLifecyclePhases(DocumentUpdateReason::kTest);
...
TEST_F(WebFrameWidgetSimTest, TestLineBoundsAreCorrectAfterCommit)
{
    ...
    // Focus the element and check the line bounds.
    first->Focus();
    gfx::Point origin = first->GetBoundingClientRect()->ToEnclosingRect().origin();
    ...
    for (wtf_size_t i = 0; i < text.length(); ++i) {
        ...
        EXPECT_EQ(gfx::Rect(origin.x(), origin.y(), 10 * (i + 1), 10), widget->GetVisibleLineBoundsOnScreen().at(0));
    }
    ...
    for (wtf_size_t i = 0; i < new_text.length(); ++i) {
        ...
        EXPECT_EQ(gfx::Rect(origin.x(), origin.y() + 10, 10 * (i + 1), 10), widget->GetVisibleLineBoundsOnScreen().at(1));
    }
}
...
TEST_F(WebFrameWidgetSimTest, TestLineBoundsAreCorrectAfterDelete)
{
    ...
    for (wtf_size_t i = last_line.length() - 1; i > 0; --i) {
        ...
        widget->UpdateAllLifecyclePhases(DocumentUpdateReason::kTest);
        EXPECT_EQ(2U, widget->GetVisibleLineBoundsOnScreen().size());
        EXPECT_EQ(gfx::Rect(origin.x(), origin.y() + 10, 10 * i, 10), widget->GetVisibleLineBoundsOnScreen().at(1));
    }
    ...
    for (wtf_size_t i = first_line.length() - 1; i > 0; --i) {
        ...
        widget->UpdateAllLifecyclePhases(DocumentUpdateReason::kTest);
        EXPECT_EQ(1U, widget->GetVisibleLineBoundsOnScreen().size());
        EXPECT_EQ(gfx::Rect(origin.x(), origin.y(), 10 * i, 10), widget->GetVisibleLineBoundsOnScreen().at(0));
    }
...
TEST_F(WebFrameWidgetSimTest, TestLineBoundsInFrame)
{
    ...
    SimRequest child_frame_resource("https://example.com/child_frame.html", "text/html");
    SimSubresourceRequest child_font_resource("https://example.com/Ahem.woff2", "font/woff2");
    ...
    child_font_resource.Complete(*test::ReadFromFile(test::CoreTestDataPath("Ahem.woff2")));
    Compositor().BeginFrame();
...
TEST_F(WebFrameWidgetSimTest, TestLineBoundsWithDifferentZoom)
{
    ...
    Vector<gfx::Rect> expected(Vector({ gfx::Rect(0, /* 70*1.2+40*1.2*1.5= */ 156, /* 40*1.2*1.5= */ 72,
        /* 10*1.2*1.5= */ 18) }));
    Vector<gfx::Rect>& actual = widget->GetVisibleLineBoundsOnScreen();
    EXPECT_EQ(expected.size(), actual.size());
    for (wtf_size_t i = 0; i < expected.size(); ++i) {
...
public:
    ...
    frame_test_helpers::TestWebFrameWidget* CreateWebFrameWidget(base::PassKey<WebLocalFrame> pass_key,
        ...
        bool is_for_scalable_page) override
    {
        return MakeGarbageCollected<TestWebFrameWidget>(pass_key, std::move(frame_widget_host), std::move(frame_widget), std::move(widget_host),
            std::move(widget), std::move(task_runner), frame_sink_id, hidden, never_composited, is_for_child_local_root, is_for_nested_main_frame,
            is_for_scalable_page);
    }
...
protected:
    ...
    public:
        ...
        explicit TestSwapPromise(State* state)
            : state_(state)
        {
            DCHECK(state_);
        ...
        DidNotSwapAction DidNotSwap(DidNotSwapReason reason, base::TimeTicks) override
        {
            DCHECK_EQ(State::kPending, *state_);
    ...
    public:
        ...
        WebInputEventResult HandleInputEvent(const WebCoalescedInputEvent& coalesced_event) override
        {
        ...
        {
            LayerTreeHost()->GetSwapPromiseManager()->QueueSwapPromise(std::make_unique<TestSwapPromise>(state));
        }

        void SendInputEventAndWaitForDispatch(std::unique_ptr<WebInputEvent> event)
        {
            MainThreadEventQueue* input_event_queue = GetWidgetInputHandlerManager()->input_event_queue();
            input_event_queue->HandleEvent(std::make_unique<WebCoalescedInputEvent>(std::move(event), ui::LatencyInfo()),
                MainThreadEventQueue::DispatchType::kNonBlocking, mojom::blink::InputEventResultState::kSetNonBlocking, WebInputEventAttribution(), nullptr,
                base::DoNothing());
        ...
        void CompositeAndWaitForPresentation(SimCompositor& compositor)
        {
            ...
            // Present and wait for it to complete.
            viz::FrameTimingDetails timing_details;
            timing_details.presentation_feedback = gfx::PresentationFeedback(swap_time + base::Milliseconds(2), base::Milliseconds(16), 0);
            LastCreatedFrameSink()->NotifyDidPresentCompositorFrame(1, timing_details);
            presentation_run_loop.Run();
        }
...
TEST_F(EventHandlingWebFrameWidgetSimTest, NonRafAlignedEventWithoutUpdate)
{
    ...
    GetTestWebFrameWidget().SendInputEventAndWaitForDispatch(
        std::make_unique<WebKeyboardEvent>(WebInputEvent::Type::kRawKeyDown, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests()));
...
TEST_F(EventHandlingWebFrameWidgetSimTest, NonRafAlignedEventWithoutUpdateAfterUpdate)
{
...
TEST_F(EventHandlingWebFrameWidgetSimTest, RafAlignedEventWithoutUpdate)
{
    ...
    GetTestWebFrameWidget().SendInputEventAndWaitForDispatch(std::make_unique<WebMouseEvent>(WebInputEvent::Type::kMouseMove, 0, base::TimeTicks::Now()));
    EXPECT_EQ(TestSwapPromise::State::kPending, swap_promise_state);
