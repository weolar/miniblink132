...
// static
base::TimeDelta PerformanceMonitor::Threshold(ExecutionContext* context, Violation violation)
{
    ...
    PerformanceMonitor* monitor = PerformanceMonitor::InstrumentingMonitorExcludingLongTasks(context);
    return monitor ? monitor->thresholds_[violation] : base::TimeDelta();
}

// static
void PerformanceMonitor::ReportGenericViolation(
    ExecutionContext* context, Violation violation, const String& text, base::TimeDelta time, std::unique_ptr<SourceLocation> location)
{
    // Calling InstrumentingMonitorExcludingLongTasks wouldn't work properly if
    ...
    if (!monitor)
        return;
    monitor->InnerReportGenericViolation(context, violation, text, time, std::move(location));
}

// static
PerformanceMonitor* PerformanceMonitor::Monitor(const ExecutionContext* context)
{
...
// static
PerformanceMonitor* PerformanceMonitor::InstrumentingMonitorExcludingLongTasks(const ExecutionContext* context)
{
    PerformanceMonitor* monitor = PerformanceMonitor::Monitor(context);
    return monitor && monitor->enabled_ ? monitor : nullptr;
}

PerformanceMonitor::PerformanceMonitor(LocalFrame* local_root, v8::Isolate* isolate)
...
void PerformanceMonitor::Subscribe(Violation violation, base::TimeDelta threshold, Client* client)
{
...
void PerformanceMonitor::UpdateInstrumentation()
{
    ...
    for (const auto& it : subscriptions_) {
        ...
        for (const auto& client_threshold : *client_thresholds) {
            if (thresholds_[violation].is_zero() || thresholds_[violation] > client_threshold.value)
    ...
    // begin(thresholds_).
    enabled_ = std::count(std::begin(thresholds_) + 1, std::end(thresholds_), base::TimeDelta()) < static_cast<int>(kAfterLast) - 1;
}
...
{
    UpdateTaskShouldBeReported(probe.document ? probe.document->GetFrame() : nullptr);
...
void PerformanceMonitor::Did(const probe::CallFunction& probe)
{
    ...
    const probe::UserCallback* user_callback = static_cast<const probe::UserCallback*>(user_callback_);
    Violation handler_type = user_callback->recurring ? kRecurringHandler : kHandler;
    ...
    String name = user_callback->name ? String(user_callback->name) : String(user_callback->atomic_name);
    String text = String::Format("'%s' handler took %" PRId64 "ms", name.Utf8().c_str(), duration.InMilliseconds());
    InnerReportGenericViolation(probe.context, handler_type, text, duration, CaptureSourceLocation(probe.context->GetIsolate(), probe.function));
}
...
void PerformanceMonitor::Will(const probe::UserCallback& probe)
{
    ...
    UpdateTaskAttribution(probe.context);
    if (!enabled_ || user_callback_depth_ != 1 || thresholds_[probe.recurring ? kRecurringHandler : kHandler].is_zero())
        return;
...
void PerformanceMonitor::DocumentWriteFetchScript(Document* document)
{
    ...
    InnerReportGenericViolation(document->GetExecutionContext(), kBlockedParser, text, base::TimeDelta(), nullptr);
}
...
void PerformanceMonitor::DidProcessTask(base::TimeTicks start_time, base::TimeTicks end_time)
{
                ...
                for (const auto& it : *client_thresholds) {
                    if (it.value < task_time) {
                        it.key->ReportLongTask(
                            start_time, end_time, task_has_multiple_contexts_ ? nullptr : task_execution_context_, task_has_multiple_contexts_);
                    }
                }
            }
        }
