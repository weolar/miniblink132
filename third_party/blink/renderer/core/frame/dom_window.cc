...
v8::Local<v8::Value> DOMWindow::Wrap(ScriptState* script_state)
{
    ...
    v8::MaybeLocal<v8::Object> proxy = frame->GetWindowProxy(script_state->World())->GlobalProxyIfNotDetached();
    if (proxy.IsEmpty()) {
...
v8::Local<v8::Object> DOMWindow::AssociateWithWrapper(
    v8::Isolate* isolate, DOMWrapperWorld* world, const WrapperTypeInfo* wrapper_type_info, v8::Local<v8::Object> wrapper)
{
    // Using the world directly avoids fetching it from a potentially
    // half-initialized context.
    if (world->DomDataStore().Set</*entered_context=*/false>(isolate, this, wrapper_type_info, wrapper)) {
...
{
    RecordWindowProxyAccessMetrics(WebFeature::kWindowProxyCrossOriginAccessLocation, WebFeature::kWindowProxyCrossOriginAccessFromOtherPageLocation,
...
{
    RecordWindowProxyAccessMetrics(WebFeature::kWindowProxyCrossOriginAccessClosed, WebFeature::kWindowProxyCrossOriginAccessFromOtherPageClosed,
...
{
    RecordWindowProxyAccessMetrics(WebFeature::kWindowProxyCrossOriginAccessLength, WebFeature::kWindowProxyCrossOriginAccessFromOtherPageLength,
...
DOMWindow* DOMWindow::self() const
{
    ...
    RecordWindowProxyAccessMetrics(
        WebFeature::kWindowProxyCrossOriginAccessSelf, WebFeature::kWindowProxyCrossOriginAccessFromOtherPageSelf, mojom::blink::WindowProxyAccessType::kSelf);
...
DOMWindow* DOMWindow::window() const
{
    ...
    RecordWindowProxyAccessMetrics(WebFeature::kWindowProxyCrossOriginAccessWindow, WebFeature::kWindowProxyCrossOriginAccessFromOtherPageWindow,
...
DOMWindow* DOMWindow::frames() const
{
    ...
    RecordWindowProxyAccessMetrics(WebFeature::kWindowProxyCrossOriginAccessFrames, WebFeature::kWindowProxyCrossOriginAccessFromOtherPageFrames,
...
{
    RecordWindowProxyAccessMetrics(WebFeature::kWindowProxyCrossOriginAccessOpener, WebFeature::kWindowProxyCrossOriginAccessFromOtherPageOpener,
        mojom::blink::WindowProxyAccessType::kOpener);
    ScriptState* script_state = ScriptState::ForCurrentRealm(isolate);
    return ScriptValue(isolate, ToV8Traits<IDLNullable<DOMWindow>>::ToV8(script_state, opener()));
}
...
void DOMWindow::setOpenerForBindings(v8::Isolate* isolate, ScriptValue opener, ExceptionState& exception_state)
{
    ...
    v8::Local<v8::Object> this_wrapper = ToV8Traits<DOMWindow>::ToV8(ScriptState::From(isolate, context), this).As<v8::Object>();
    v8::PropertyDescriptor desc(opener.V8Value(), /*writable=*/true);
    desc.set_enumerable(true);
    desc.set_configurable(true);
    bool result = false;
    if (!this_wrapper->DefineProperty(context, V8AtomicString(isolate, "opener"), desc).To(&result)) {
        return;
    }
    if (!result) {
...
DOMWindow* DOMWindow::parent() const
{
    ...
    RecordWindowProxyAccessMetrics(WebFeature::kWindowProxyCrossOriginAccessParent, WebFeature::kWindowProxyCrossOriginAccessFromOtherPageParent,
...
DOMWindow* DOMWindow::top() const
{
    ...
    RecordWindowProxyAccessMetrics(
        WebFeature::kWindowProxyCrossOriginAccessTop, WebFeature::kWindowProxyCrossOriginAccessFromOtherPageTop, mojom::blink::WindowProxyAccessType::kTop);

    return GetFrame()->Tree().Top().DomWindow();
}

void DOMWindow::postMessage(
    v8::Isolate* isolate, const ScriptValue& message, const String& target_origin, HeapVector<ScriptValue> transfer, ExceptionState& exception_state)
{
...
void DOMWindow::postMessage(v8::Isolate* isolate, const ScriptValue& message, const WindowPostMessageOptions* options, ExceptionState& exception_state)
{
    RecordWindowProxyAccessMetrics(WebFeature::kWindowProxyCrossOriginAccessPostMessage, WebFeature::kWindowProxyCrossOriginAccessFromOtherPagePostMessage,
        mojom::blink::WindowProxyAccessType::kPostMessage);
    LocalDOMWindow* incumbent_window = IncumbentDOMWindow(isolate);
    UseCounter::Count(incumbent_window->document(), WebFeature::kWindowPostMessage);

    Transferables transferables;
    scoped_refptr<SerializedScriptValue> serialized_message
        = PostMessageHelper::SerializeMessageByMove(isolate, message, options, transferables, exception_state);
    if (exception_state.HadException())
        return;
    DCHECK(serialized_message);
    DoPostMessage(std::move(serialized_message), transferables.message_ports, options, incumbent_window, exception_state);
}

DOMWindow* DOMWindow::AnonymousIndexedGetter(uint32_t index)
{
    RecordWindowProxyAccessMetrics(WebFeature::kWindowProxyCrossOriginAccessIndexedGetter, WebFeature::kWindowProxyCrossOriginAccessFromOtherPageIndexedGetter,
...
String DOMWindow::SanitizedCrossDomainAccessErrorMessage(const LocalDOMWindow* accessing_window, CrossDocumentAccessPolicy cross_document_access) const
{
    ...
    if (cross_document_access == CrossDocumentAccessPolicy::kDisallowed) {
        message = "Blocked a restricted frame with origin \"" + active_origin->ToString() + "\" from accessing another frame.";
    } else {
        message = "Blocked a frame with origin \"" + active_origin->ToString() + "\" from accessing a cross-origin frame.";
    }
...
String DOMWindow::CrossDomainAccessErrorMessage(const LocalDOMWindow* accessing_window, CrossDocumentAccessPolicy cross_document_access) const
{
    if (!accessing_window || !GetFrame())
        return String();
    ...
    const SecurityOrigin* active_origin = accessing_window->GetSecurityOrigin();
    const SecurityOrigin* target_origin = GetFrame()->GetSecurityContext()->GetSecurityOrigin();
    ...
    DCHECK(GetFrame()->IsRemoteFrame() || !active_origin->CanAccess(target_origin)
        || (local_dom_window && accessing_window->GetAgent() != local_dom_window->GetAgent()));

    String message
        = "Blocked a frame with origin \"" + active_origin->ToString() + "\" from accessing a frame with origin \"" + target_origin->ToString() + "\". ";
    ...
    KURL target_url = local_dom_window ? local_dom_window->Url() : KURL(NullURL(), target_origin->ToString());
    using SandboxFlags = network::mojom::blink::WebSandboxFlags;
    if (GetFrame()->GetSecurityContext()->IsSandboxed(SandboxFlags::kOrigin) || accessing_window->IsSandboxed(SandboxFlags::kOrigin)) {
        message = "Blocked a frame at \"" + SecurityOrigin::Create(active_url)->ToString() + "\" from accessing a frame at \""
    ...
    if (target_origin->Protocol() != active_origin->Protocol())
        return message + " The frame requesting access has a protocol of \"" + active_url.Protocol() + "\", the frame being accessed has a protocol of \""
            + target_url.Protocol() + "\". Protocols must match.";

    // 'document.domain' errors.
    if (target_origin->DomainWasSetInDOM() && active_origin->DomainWasSetInDOM())
        return message + "The frame requesting access set \"document.domain\" to \"" + active_origin->Domain() + "\", the frame being accessed set it to \""
            + target_origin->Domain()
            + "\". Both must set \"document.domain\" to the same value to allow "
              "access.";
    if (active_origin->DomainWasSetInDOM())
        return message + "The frame requesting access set \"document.domain\" to \"" + active_origin->Domain()
    ...
    if (target_origin->DomainWasSetInDOM())
        return message + "The frame being accessed set \"document.domain\" to \"" + target_origin->Domain()
...
void DOMWindow::Close(LocalDOMWindow* incumbent_window)
{
    ...
    if (!(active_document && active_document->GetFrame() && active_document->GetFrame()->CanNavigate(*GetFrame()))) {
        return;
    }

    RecordWindowProxyAccessMetrics(WebFeature::kWindowProxyCrossOriginAccessClose, WebFeature::kWindowProxyCrossOriginAccessFromOtherPageClose,
        mojom::blink::WindowProxyAccessType::kClose);

    Settings* settings = GetFrame()->GetSettings();
    bool allow_scripts_to_close_windows = settings && settings->GetAllowScriptsToCloseWindows();

    if (!page->OpenedByDOM() && !allow_scripts_to_close_windows) {
        if (GetFrame()->Client()->BackForwardLength() > 1) {
            active_document->domWindow()->GetFrameConsole()->AddMessage(MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kJavaScript,
                mojom::blink::ConsoleMessageLevel::kWarning, "Scripts may close only the windows that were opened by them."));
            return;
        } else {
            ...
            UseCounter::Count(active_document, WebFeature::kWindowCloseHistoryLengthOne);
        }
    }
...
void DOMWindow::focus(v8::Isolate* isolate)
{
    ...
    bool allow_focus_without_user_activation = frame->AllowFocusWithoutUserActivation();

    if (!allow_focus_without_user_activation && !frame->HasTransientUserActivation()) {
    ...
    RecordWindowProxyAccessMetrics(WebFeature::kWindowProxyCrossOriginAccessFocus, WebFeature::kWindowProxyCrossOriginAccessFromOtherPageFocus,
    ...
    if (allow_focus) {
        ...
        if (local_dom_window) {
            ...
            LocalDOMWindow* pip_window = document ? PictureInPictureController::GetDocumentPictureInPictureWindow(*document) : nullptr;
            if (pip_window && LocalFrame::HasTransientUserActivation(pip_window->GetFrame())) {
                allow_focus = true;
    ...
    if (frame->IsOutermostMainFrame() && allow_focus) {
        ...
        if (local_frame->GetDocument() && !local_frame->GetDocument()->IsFocusAllowed()) {
            return;
        }
    }

    page->GetFocusController().FocusDocumentView(GetFrame(), true /* notifyEmbedder */);
...
{
    RecordWindowProxyAccessMetrics(
        WebFeature::kWindowProxyCrossOriginAccessBlur, WebFeature::kWindowProxyCrossOriginAccessFromOtherPageBlur, mojom::blink::WindowProxyAccessType::kBlur);
}
...
void DOMWindow::PostMessageForTesting(scoped_refptr<SerializedScriptValue> message, const MessagePortArray& ports, const String& target_origin,
    LocalDOMWindow* source, ExceptionState& exception_state)
{
    WindowPostMessageOptions* options = WindowPostMessageOptions::Create();
    options->setTargetOrigin(target_origin);
...
void DOMWindow::InstallCoopAccessMonitor(LocalFrame* accessing_frame, network::mojom::blink::CrossOriginOpenerPolicyReporterParamsPtr coop_reporter_params,
    bool is_in_same_virtual_coop_related_group)
{
    ExecutionContext* execution_context = accessing_frame->DomWindow()->GetExecutionContext();
    CoopAccessMonitor* monitor = MakeGarbageCollected<CoopAccessMonitor>(execution_context);
    ...
    monitor->reported_window_url = std::move(coop_reporter_params->reported_window_url);
    monitor->is_in_same_virtual_coop_related_group = is_in_same_virtual_coop_related_group;

    // `task_runner` is used for handling disconnect, and it uses
    // `TaskType::kInternalDefault` to match the main frame receiver.
    scoped_refptr<base::SingleThreadTaskRunner> task_runner = execution_context->GetTaskRunner(TaskType::kInternalDefault);
    monitor->reporter.Bind(std::move(coop_reporter_params->reporter), std::move(task_runner));
    ...
    // instead.
    monitor->reporter.set_disconnect_handler(WTF::BindOnce(&DOMWindow::DisconnectCoopAccessMonitor, WrapWeakPersistent(this), monitor->accessing_main_frame));
    ...
    for (Member<CoopAccessMonitor>& old : coop_access_monitor_) {
        if (old->accessing_main_frame == monitor->accessing_main_frame
            && network::IsAccessFromCoopPage(old->report_type) == network::IsAccessFromCoopPage(monitor->report_type)) {
...
void DOMWindow::ReportCoopAccess(const char* property_name)
{
    ...
    LocalFrame& accessing_main_frame = To<LocalFrame>(accessing_frame->Tree().Top());
    const LocalFrameToken accessing_main_frame_token = accessing_main_frame.GetLocalFrameToken();
    ...
    while (it != coop_access_monitor_.end()) {
        ...
        if ((*it)->is_in_same_virtual_coop_related_group && (property_name_as_string == "postMessage" || property_name_as_string == "closed")) {
            ++it;
            continue;
        }
        ...
        auto source_location
            = network::mojom::blink::SourceLocation::New(location->Url() ? location->Url() : "", location->LineNumber(), location->ColumnNumber());

        accessing_window->GetFrameConsole()->AddMessage(MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kJavaScript,
            mojom::blink::ConsoleMessageLevel::kError, CoopReportOnlyErrorMessage(property_name), location->Clone()));
        ...
        if (monitor->endpoint_defined) {
            if (monitor->reporter.is_bound()) {
                monitor->reporter->QueueAccessReport(monitor->report_type, property_name, std::move(source_location), std::move(monitor->reported_window_url));
            }
            ...
            if (network::IsAccessFromCoopPage(monitor->report_type)) {
                ReportingContext::From(accessing_main_frame.DomWindow())
                    ->QueueReport(MakeGarbageCollected<Report>(ReportType::kCoopAccessViolation, accessing_main_frame.GetDocument()->Url().GetString(),
                        MakeGarbageCollected<CoopAccessViolationReportBody>(
                            std::move(location), monitor->report_type, String(property_name), monitor->reported_window_url)));
            }
        }
...
void DOMWindow::DoPostMessage(scoped_refptr<SerializedScriptValue> message, const MessagePortArray& ports, const WindowPostMessageOptions* options,
    LocalDOMWindow* source, ExceptionState& exception_state)
{
    TRACE_EVENT0("blink", "DOMWindow::DoPostMessage");
    auto* source_frame = source->GetFrame();
    bool unload_event_in_progress = source_frame && source_frame->GetDocument() && source_frame->GetDocument()->UnloadEventInProgress();
    if (!unload_event_in_progress && source_frame && source_frame->GetPage() && source_frame->GetPage()->DispatchedPagehideAndStillHidden()) {
    }
    ...
    scoped_refptr<const SecurityOrigin> target = PostMessageHelper::GetTargetOrigin(options, *source, exception_state);
    if (exception_state.HadException())
        return;
    if (!target) {
    ...
    auto channels = MessagePort::DisentanglePorts(GetExecutionContext(), ports, exception_state);
    if (exception_state.HadException())
        return;

    const SecurityOrigin* target_security_origin = GetFrame()->GetSecurityContext()->GetSecurityOrigin();
    ...
    if (is_target_secure) {
        if (is_source_secure) {
            ...
            if (!GetFrame()->Tree().Top().GetSecurityContext()->GetSecurityOrigin()->IsPotentiallyTrustworthy()) {
                UseCounter::Count(source, WebFeature::kPostMessageFromInsecureToSecureToplevel);
            }
        }
    } else {
        if (is_source_secure) {
    ...
    if (source->GetFrame() && source->GetFrame()->Tree().Top() != GetFrame()->Tree().Top()) {
        if ((!target_security_origin->RegistrableDomain() && target_security_origin->Host() == source_security_origin->Host())
            || (target_security_origin->RegistrableDomain() && target_security_origin->RegistrableDomain() == source_security_origin->RegistrableDomain())) {
            if (target_security_origin->Protocol() == source_security_origin->Protocol()) {
                ...
                if (is_source_secure && !is_target_secure) {
                    UseCounter::Count(source, WebFeature::kSchemelesslySameSitePostMessageSecureToInsecure);
                } else if (!is_source_secure && is_target_secure) {
                    UseCounter::Count(source, WebFeature::kSchemelesslySameSitePostMessageInsecureToSecure);
                }
            }
        } else {
    ...
    auto* local_dom_window = DynamicTo<LocalDOMWindow>(this);
    KURL target_url = local_dom_window ? local_dom_window->Url() : KURL(NullURL(), target_security_origin->ToString());
    if (!source->GetContentSecurityPolicy()->AllowConnectToSource(
            target_url, target_url, RedirectStatus::kNoRedirect, ReportingDisposition::kSuppressReporting)) {
        UseCounter::Count(source, WebFeature::kPostMessageOutgoingWouldBeBlockedByConnectSrc);
    }
    ...
    mojom::blink::DelegatedCapability delegated_capability = mojom::blink::DelegatedCapability::kNone;
    if (options->hasDelegate()) {
        ...
        if (capability_list.Contains("payment")) {
            ...
            delegated_capability = mojom::blink::DelegatedCapability::kFullscreenRequest;
        } else if (capability_list.Contains("display-capture")) {
            delegated_capability = mojom::blink::DelegatedCapability::kDisplayCaptureRequest;
        } else {
            exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "Delegation of \'" + options->delegate() + "\' is not supported.");
            return;
        }
        ...
        if (!target) {
            exception_state.ThrowDOMException(DOMExceptionCode::kNotAllowedError, "Delegation to target origin '*' is not allowed.");
            return;
        }

        if (!LocalFrame::HasTransientUserActivation(source_frame)) {
            exception_state.ThrowDOMException(DOMExceptionCode::kNotAllowedError, "Delegation is not allowed without transient user activation.");
            return;
        }
...
void DOMWindow::RecordWindowProxyAccessMetrics(
    WebFeature property_access, WebFeature property_access_from_other_page, mojom::blink::WindowProxyAccessType access_type) const
{
    if (!GetFrame())
        return;
    ...
    if (GetFrame() != accessing_frame) {
        ...
        if (!base::FeatureList::IsEnabled(features::kSubSampleWindowProxyUsageMetrics) || metrics_sub_sampler_.ShouldSample(0.0001)) {
            accessing_frame->GetLocalFrameHostRemote().RecordWindowProxyUsageMetrics(GetFrame()->GetFrameToken(), access_type);
        }
    }

    // Note that SecurityOrigin can be null in unit tests.
    if (!GetFrame()->GetSecurityContext()->GetSecurityOrigin() || !accessing_frame->GetSecurityContext()->GetSecurityOrigin()
        || accessing_frame->GetSecurityContext()->GetSecurityOrigin()->IsSameOriginWith(GetFrame()->GetSecurityContext()->GetSecurityOrigin())) {
        return;
    }
...
std::optional<DOMWindow::ProxyAccessBlockedReason> DOMWindow::GetProxyAccessBlockedReason(v8::Isolate* isolate) const
{
    if (!GetFrame()) {
    ...
    if (GetFrame()->GetPage() != accessing_frame->GetPage()
        && (accessing_frame->GetPage()->IsPartitionedPopin() || GetFrame()->GetPage()->IsPartitionedPopin())) {
    ...
    if (accessing_frame->GetPage()->CoopRelatedGroupToken() == GetFrame()->GetPage()->CoopRelatedGroupToken()
        && accessing_frame->GetPage()->BrowsingContextGroupToken() != GetFrame()->GetPage()->BrowsingContextGroupToken()) {
...
// static
String DOMWindow::GetProxyAccessBlockedExceptionMessage(DOMWindow::ProxyAccessBlockedReason reason)
{
    switch (reason) {
...
BlinkTransferableMessage DOMWindow::PostedMessage::ToBlinkTransferableMessage() &&
{
    BlinkTransferableMessage result;

    result.message = std::move(data);
    result.sender_agent_cluster_id = source->GetAgentClusterID();
    result.locked_to_sender_agent_cluster = result.message->IsLockedToAgentCluster();

    result.ports = std::move(channels);

    if (user_activation) {
        result.user_activation = mojom::blink::UserActivationSnapshot::New(user_activation->hasBeenActive(), user_activation->isActive());
    }
...
void DOMWindow::DisconnectCoopAccessMonitor(const LocalFrameToken& accessing_main_frame)
{
    auto it = coop_access_monitor_.begin();
    while (it != coop_access_monitor_.end()) {
