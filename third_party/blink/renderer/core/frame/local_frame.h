...
public:
    ...
    // Frame constructor.
    LocalFrame(LocalFrameClient*, Page&, FrameOwner*, Frame* parent, Frame* previous_sibling, FrameInsertType insert_type, const LocalFrameToken& frame_token,
        WindowAgentFactory* inheriting_agent_factory, InterfaceRegistry*, mojo::PendingRemote<mojom::blink::BrowserInterfaceBroker>,
    ...
    void Init(Frame* opener, const DocumentToken& document_token, std::unique_ptr<PolicyContainer> policy_container, const StorageKey& storage_key,
    ...
    void EvictFromBackForwardCache(mojom::blink::RendererEvictionReason reason, std::unique_ptr<SourceLocation> source_location) override;
    ...
    {
        return page_popup_owner_.Get();
    }
    bool HasPagePopupOwner() const
    {
        return page_popup_owner_ != nullptr;
    }
    ...
    static void NotifyUserActivation(LocalFrame*, mojom::blink::UserActivationNotificationType notification_type, bool need_browser_verification = false);
    ...
    static bool ConsumeTransientUserActivation(LocalFrame*, UserActivationUpdateSource update_source = UserActivationUpdateSource::kRenderer);
    ...
    void RegisterVirtualKeyboardOverlayChangedObserver(VirtualKeyboardOverlayChangedObserver*);
    ...
    bool BubbleLogicalScrollInParentFrame(mojom::blink::ScrollDirection direction, ui::ScrollGranularity granularity);
    ...
    // Returns true if the scroll is locally consumed, false otherwise.
    bool BubbleLogicalScrollFromChildFrame(mojom::blink::ScrollDirection direction, ui::ScrollGranularity granularity, Frame* child);
    ...
    void SetLayoutAndTextZoomFactors(float layout_zoom_factor, float text_zoom_factor);
    ...
    void UpdateViewportSegmentCSSEnvironmentVariables(const WebVector<gfx::Rect>& viewport_segments);
    void UpdateViewportSegmentCSSEnvironmentVariables(StyleEnvironmentVariables& vars, const WebVector<gfx::Rect>& viewport_segments);
    ...
    void TextSelectionChanged(const WTF::String& selection_text, uint32_t offset, const gfx::Range& range) const;

    PositionWithAffinityTemplate<EditingAlgorithm<NodeTraversal>> PositionForPoint(const PhysicalOffset& frame_point);
    ...
    {
        return navigation_disable_count_ == 0;
    }
    ...
    bool NavigationShouldReplaceCurrentHistoryEntry(const FrameLoadRequest& request, WebFrameLoadType frame_load_type);
    ...
    IdlenessDetector* GetIdlenessDetector()
    {
        return idleness_detector_.Get();
    }
    AdTracker* GetAdTracker()
    {
        return ad_tracker_.Get();
    }
    void SetAdTrackerForTesting(AdTracker* ad_tracker);
    LCPScriptObserver* GetScriptObserver()
    {
        return script_observer_.Get();
    }
    ...
    scoped_refptr<WebBackgroundResourceFetchAssets> MaybeGetBackgroundResourceFetchAssets();
    ...
    void SetViewportIntersectionFromParent(const mojom::blink::ViewportIntersectionState& intersection_state);
    ...
    void ForceSynchronousDocumentInstall(const AtomicString& mime_type, const SegmentedBuffer& data);
    ...
    void UpdateSuddenTerminationStatus(bool added_listener, mojom::blink::SuddenTerminationDisablerType disabler_type);

    // Called when we added an event listener that might affect sudden termination
    // disabling of the page.
    void AddedSuddenTerminationDisablerListener(const EventTarget& event_target, const AtomicString& event_type);
    // Called when we removed event listeners that might affect sudden termination
    // disabling of the page.
    void RemovedSuddenTerminationDisablerListener(const EventTarget& event_target, const AtomicString& event_type);
    ...
    void PauseSubresourceLoading(mojo::PendingReceiver<blink::mojom::blink::PauseSubresourceLoadingHandle> receiver);
    ...
    mojom::blink::BackForwardCacheControllerHost& GetBackForwardCacheControllerHostRemote();

    // Sets back/forward cache NotRestoredReasons for this frame. Only set for
    // outermost main frame.
    void SetNotRestoredReasons(mojom::blink::BackForwardCacheNotRestoredReasonsPtr);
    ...
    // main frame.
    void SetNavigationConfidence(double randomized_trigger_rate, mojom::blink::ConfidenceLevel confidence);
    ...
    // (A->B->A embedding)
    void CountUseIfFeatureWouldBeBlockedByPermissionsPolicy(mojom::WebFeature blocked_cross_origin, mojom::WebFeature blocked_same_origin);
    ...
    void SetPrescientNetworkingForTesting(std::unique_ptr<WebPrescientNetworking> prescient_networking);

    void CopyImageAtViewportPoint(const gfx::Point& viewport_point);
    void MediaPlayerActionAtViewportPoint(const gfx::Point& viewport_position, const blink::mojom::blink::MediaPlayerActionType type, bool enable);
    ...
    void DownloadURL(const ResourceRequest& request, network::mojom::blink::RedirectMode cross_origin_redirect_behavior);
    void DownloadURL(const ResourceRequest& request, network::mojom::blink::RedirectMode cross_origin_redirect_behavior,
        mojo::PendingRemote<mojom::blink::BlobURLToken> blob_url_token);

    void NotifyUserActivation(mojom::blink::UserActivationNotificationType notification_type);
    ...
    void RegisterWindowControlsOverlayChangedDelegate(WindowControlsOverlayChangedDelegate*);
    ...
    void BindTextFragmentReceiver(mojo::PendingReceiver<mojom::blink::TextFragmentReceiver> receiver);
    ...
    {
        return ancestor_or_self_has_cspee_;
    }
    ...
    void SetBackgroundColorPaintImageGeneratorForTesting(BackgroundColorPaintImageGenerator* generator);
    ...
    mojo::PendingRemote<mojom::blink::BlobURLStore> GetBlobUrlStorePendingRemote();
    ...
    const HeapHashSet<WeakMember<ScrollSnapshotClient>>& GetScrollSnapshotClientsForTesting()
    {
    ...
    static BlockingDetailsList ConvertFeatureAndLocationToMojomStruct(const BFCacheBlockingFeatureAndLocations&, const BFCacheBlockingFeatureAndLocations&);

    bool IsSameOrigin();

    v8_compile_hints::V8LocalCompileHintsProducer& GetV8LocalCompileHintsProducer()
    {
    ...
    mojo::PendingRemote<mojom::blink::NavigationStateKeepAliveHandle> IssueKeepAliveHandle();

    WebLinkPreviewTriggerer* GetOrCreateLinkPreviewTriggerer();
    void SetLinkPreviewTriggererForTesting(std::unique_ptr<WebLinkPreviewTriggerer> trigger);

    void AllowStorageAccessAndNotify(blink::WebContentSettingsClient::StorageType storage_type, base::OnceCallback<void(bool)> callback);

    bool AllowStorageAccessSyncAndNotify(blink::WebContentSettingsClient::StorageType storage_type);
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(WebFrameTest, SmartClipReturnsEmptyStringsWhenUserSelectIsNone);
    ...
    // already LocalFrame.
    bool IsLocalFrame() const override
    {
        return true;
    }
    ...
    void UpdateBackForwardCacheDisablingFeatures(BlockingDetails details) override;
    const base::UnguessableToken& GetAgentClusterId() const override;
    void OnTaskCompleted(base::TimeTicks start_time, base::TimeTicks end_time) override;
    ...
    //
    // The |notification_type| parameter is used for histograms only.
    void NotifyUserActivation(mojom::blink::UserActivationNotificationType notification_type, bool need_browser_verification);
    ...
    HitTestResult HitTestResultForVisualViewportPos(const gfx::Point& pos_in_viewport);

    bool ShouldThrottleDownload();

    void ExtractSmartClipDataInternal(const gfx::Rect& rect_in_viewport, String& clip_text, String& clip_html, gfx::Rect& clip_rect);
    ...
    HeapMojoUniqueReceiverSet<blink::mojom::blink::PauseSubresourceLoadingHandle> pause_handle_receivers_ { nullptr };

    // Keeps track of all the registered VK observers.
    HeapHashSet<WeakMember<VirtualKeyboardOverlayChangedObserver>> virtual_keyboard_overlay_changed_observers_;
    ...
    Member<BackgroundColorPaintImageGenerator> background_color_paint_image_generator_;
    ...
    Member<v8_compile_hints::V8LocalCompileHintsProducer> v8_local_compile_hints_producer_;

    // This handle notifies the scheduler of whether the unload handler is used or
    // not so it can block BFCache.
    FrameScheduler::SchedulingAffectingFeatureHandle feature_handle_for_scheduler_;
...
template <> struct DowncastTraits<LocalFrame> {
    static bool AllowFrom(const Frame& frame)
    {
        return frame.IsLocalFrame();
    }
};
