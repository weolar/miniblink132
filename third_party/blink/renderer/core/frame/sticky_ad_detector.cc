...
void StickyAdDetector::MaybeFireDetection(LocalFrame* outermost_main_frame)
{
    ...
    if (last_detection_time_.has_value() && base::FeatureList::IsEnabled(features::kFrequencyCappingForLargeStickyAdDetection)
    ...
    HitTestLocation location(gfx::PointF(outermost_main_frame_size.width() / 2.0, outermost_main_frame_size.height() * 9.0 / 10));
    ...
    if (element_id == candidate_id_) {
        ...
        if (std::abs(candidate_start_outermost_main_frame_scroll_position_ - outermost_main_frame->GetOutermostMainFrameScrollPosition().y())
            > candidate_height_) {
    ...
    bool is_main_page_scrollable = element->GetDocument().GetLayoutView()->HasScrollableOverflowY();
...
void StickyAdDetector::OnLargeStickyAdDetected(LocalFrame* outermost_main_frame)
{
    outermost_main_frame->Client()->OnLargeStickyAdDetected();
    UseCounter::Count(outermost_main_frame->GetDocument(), WebFeature::kLargeStickyAd);
