...
void LogCursorSizeCounter(LocalFrame* frame, const ui::Cursor& cursor)
{
    ...
    auto scaled_size = gfx::ScaleToFlooredSize(gfx::Size(bitmap.width(), bitmap.height()), 1 / cursor.image_scale_factor());
...
LocalFrameView::LocalFrameView(LocalFrame& frame, gfx::Rect frame_rect)
    ...
    update_plugins_timer_(frame.GetTaskRunner(TaskType::kInternalLoading), this, &LocalFrameView::UpdatePluginsTimerFired)
...
#endif
{
    ...
    if (frame_->Owner() && frame_->Owner()->ScrollbarMode() == mojom::blink::ScrollbarMode::kAlwaysOff)
...
void LocalFrameView::ForAllChildViewsAndPlugins(base::FunctionRef<void(EmbeddedContentView&)> function)
{
    for (Frame* child = frame_->Tree().FirstChild(); child; child = child->Tree().NextSibling()) {
        if (child->View())
            function(*child->View());
    }
    ...
    if (Document* document = frame_->GetDocument()) {
        if (DocumentFencedFrames* fenced_frames = DocumentFencedFrames::Get(*document)) {
...
void LocalFrameView::ForAllChildLocalFrameViews(base::FunctionRef<void(LocalFrameView&)> function)
{
    for (Frame* child = frame_->Tree().FirstChild(); child; child = child->Tree().NextSibling()) {
...
void LocalFrameView::ForAllNonThrottledLocalFrameViews(base::FunctionRef<void(LocalFrameView&)> function, TraversalOrder order)
{
    ...
    ForAllChildLocalFrameViews([&function, order](LocalFrameView& child_view) { child_view.ForAllNonThrottledLocalFrameViews(function, order); });
...
// need to be updated as well.
void LocalFrameView::ForAllThrottledLocalFrameViews(base::FunctionRef<void(LocalFrameView&)> function)
{
    if (ShouldThrottleRendering())
        function(*this);

    ForAllChildLocalFrameViews([&function](LocalFrameView& child_view) { child_view.ForAllThrottledLocalFrameViews(function); });
}

void LocalFrameView::ForAllRemoteFrameViews(base::FunctionRef<void(RemoteFrameView&)> function)
{
    for (Frame* child = frame_->Tree().FirstChild(); child; child = child->Tree().NextSibling()) {
...
void LocalFrameView::Dispose()
{
    ...
    if (ukm_aggregator_) {
        ...
        if (root_document) {
            ukm_aggregator_->TransmitFinalSample(root_document->UkmSourceID(), root_document->UkmRecorder(), root_frame.IsMainFrame());
        }
...
{
    bool uses_window_inactive_selector = frame_->GetDocument()->GetStyleEngine().UsesWindowInactiveSelector();

    ForAllChildLocalFrameViews([](LocalFrameView& frame_view) { frame_view.InvalidateAllCustomScrollbarsOnActiveChanged(); });
    ...
    for (const auto& scrollable_area : user_scrollable_areas_->Values()) {
        if (scrollable_area->ScrollsOverflow() || scrollable_area->HasScrollbar()) {
            ...
            if (auto* layout_box = scrollable_area->GetLayoutBox()) {
                layout_box->SetNeedsLayout(layout_invalidation_reason::kScrollbarChanged);
            }
        }
    }
}
...
void LocalFrameView::CountObjectsNeedingLayout(unsigned& needs_layout_objects, unsigned& total_objects, bool& is_subtree)
{
    needs_layout_objects = 0;
    total_objects = 0;
    is_subtree = IsSubtreeLayout();
    if (is_subtree) {
        layout_subtree_root_list_.CountObjectsNeedingLayout(needs_layout_objects, total_objects);
    } else {
        LayoutSubtreeRootList::CountObjectsNeedingLayoutInRoot(GetLayoutView(), needs_layout_objects, total_objects);
    }
}
...
bool LocalFrameView::LayoutFromRootObject(LayoutObject& root)
{
    ...
    if (scroll_anchoring_scrollable_areas_) {
        for (auto& scrollable_area : *scroll_anchoring_scrollable_areas_) {
            if (scrollable_area->GetScrollAnchor() && scrollable_area->ShouldPerformScrollAnchoring())
...
void LocalFrameView::PerformLayout()
{
    ...
    base::AutoReset<bool> change_scheduling_enabled(&layout_scheduling_enabled_, false);
    ...
    TRACE_EVENT_OBJECT_SNAPSHOT_WITH_ID(
        TRACE_DISABLED_BY_DEFAULT("blink.debug.layout.trees"), "LayoutTree", this, TracedLayoutObject::Create(*GetLayoutView(), false));

    gfx::Size old_size(Size());

    DCHECK(in_subtree_layout || layout_subtree_root_list_.IsEmpty());

    double contents_height_before_layout = GetLayoutView()->DocumentRect().Height();
    TRACE_EVENT_BEGIN1(PERFORM_LAYOUT_TRACE_CATEGORIES, "LocalFrameView::performLayout", "contentsHeightBeforeLayout", contents_height_before_layout);
        ...
        if (in_subtree_layout) {
            ...
            for (auto& root : layout_subtree_root_list_.Ordered()) {
                ...
                if (container_box) {
                    ...
                    should_rebuild_fragments = it != fragment_tree_spines.end() && --it->value == 0;
                }
    ...
    TRACE_EVENT_END0(PERFORM_LAYOUT_TRACE_CATEGORIES, "LocalFrameView::performLayout");
    FirstMeaningfulPaintDetector::From(*document).MarkNextPaintAsMeaningfulIfNeeded(
        layout_object_counter_, contents_height_before_layout, GetLayoutView()->DocumentRect().Height(), Height());
...
void LocalFrameView::UpdateLayout()
{
    ...
    if (RuntimeEnabledFeatures::BlinkRuntimeCallStatsEnabled()) [[unlikely]] {
        rcs_scope.emplace(RuntimeCallStats::From(isolate), RuntimeCallStats::CounterId::kUpdateLayout);
    }
    ...
    TRACE_EVENT_BEGIN1(
        "devtools.timeline", "Layout", "beginData", [&](perfetto::TracedValue context) { inspector_layout_event::BeginData(std::move(context), this); });
    ...
    TRACE_EVENT_END1(
        "devtools.timeline", "Layout", "endData", [&](perfetto::TracedValue context) { inspector_layout_event::EndData(std::move(context), layout_roots); });
...
void LocalFrameView::WillStartForcedLayout(DocumentUpdateReason reason)
{
    ...
    if (auto* metrics_aggregator = GetUkmAggregator()) {
        ...
        forced_layout_timer_ = metrics_aggregator->GetScopedForcedLayoutTimer(reason);
    }
}
...
gfx::SizeF LocalFrameView::LargeViewportSizeForViewportUnits() const
{
    ...
    if (browser_controls.PermittedState() != cc::BrowserControlsState::kHidden) {
        ...
        if (frame_->IsOutermostMainFrame() && layout_size.width() && viewport_width) {
            float layout_to_viewport_width_scale_factor = viewport_width / layout_size.width();
            layout_size.Enlarge(0, (browser_controls.TotalHeight() - browser_controls.TotalMinHeight()) / layout_to_viewport_width_scale_factor);
        }
    }
...
{
    BrowserControls& browser_controls = frame_->GetPage()->GetBrowserControls();
    return browser_controls.ShrinkViewport() ? SmallViewportSizeForViewportUnits() : LargeViewportSizeForViewportUnits();
}
...
void LocalFrameView::RunPostLifecycleSteps()
{
    ...
    ForAllRemoteFrameViews([](RemoteFrameView& frame_view) { frame_view.UpdateCompositingScaleFactor(); });
}
...
void LocalFrameView::RunIntersectionObserverSteps()
{
...
#endif
    if ((intersection_observation_state_ < kRequired && ShouldThrottleRendering()) || Lifecycle().LifecyclePostponed() || !frame_->GetDocument()->IsActive()) {
        return;
    }
    ...
    if (frame_->IsOutermostMainFrame()) {
        ...
        gfx::Rect main_frame_dimensions(ToRoundedSize(To<LayoutBox>(layout_object)->ScrollableOverflowRect().size));
        GetFrame().Client()->OnMainFrameIntersectionChanged(main_frame_dimensions);
        GetFrame().Client()->OnMainFrameViewportRectangleChanged(gfx::Rect(frame_->GetOutermostMainFrameScrollPosition(), frame_->GetOutermostMainFrameSize()));
    }

    TRACE_EVENT0("blink,benchmark", "LocalFrameView::UpdateViewportIntersectionsForSubtree");
    SCOPED_UMA_AND_UKM_TIMER(GetUkmAggregator(), LocalFrameUkmAggregator::kIntersectionObservation);

    ComputeIntersectionsContext context;
    bool needs_occlusion_tracking = UpdateViewportIntersectionsForSubtree(0, context);
...
void LocalFrameView::ForceUpdateViewportIntersections()
{
    ...
    UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason::kIntersectionObservation);
    ComputeIntersectionsContext context;
    UpdateViewportIntersectionsForSubtree(IntersectionObservation::kImplicitRootObserversNeedUpdate | IntersectionObservation::kIgnoreDelay, context);
}
...
bool LocalFrameView::GetIntrinsicSizingInfo(IntrinsicSizingInfo& intrinsic_sizing_info) const
{
...
#if DCHECK_IS_ON()
    if (new_frame.Width() != LayoutUnit::Max().RawValue() && new_frame.Height() != LayoutUnit::Max().RawValue())
...
void LocalFrameView::AddPartToUpdate(LayoutEmbeddedObject& object)
{
    ...
    // recalc).
    DCHECK(IsInPerformLayout() || (DisplayLockUtilities::LockedAncestorPreventingLayout(object) && frame_->GetDocument()->InStyleRecalc()));
...
void LocalFrameView::SetMediaType(const AtomicString& media_type)
{
    ...
    frame_->GetDocument()->MediaQueryAffectingValueChanged(MediaValueChange::kOther);
}

AtomicString LocalFrameView::MediaType() const
{
    // See if we have an override type.
    if (frame_->GetSettings() && !frame_->GetSettings()->GetMediaTypeOverride().empty())
...
void LocalFrameView::AddBackgroundAttachmentFixedObject(LayoutBoxModelObject& object)
{
...
void LocalFrameView::RemoveBackgroundAttachmentFixedObject(LayoutBoxModelObject& object)
{
    background_attachment_fixed_objects_.erase(&object);
    SetNeedsPaintPropertyUpdate();
}

static bool BackgroundAttachmentFixedNeedsRepaintOnScroll(const LayoutObject& object)
{
    ...
    if (const auto* view = DynamicTo<LayoutView>(object)) {
        if (view->GetBackgroundPaintLocation() == kBackgroundPaintInBorderBoxSpace) {
            return false;
        }
    }
...
bool LocalFrameView::RequiresMainThreadScrollingForBackgroundAttachmentFixed() const
{
...
{
    DCHECK(frame_->GetPage());
    if (frame_->GetDocument() && frame_->GetDocument()->Lifecycle().LifecyclePostponed())
        return;
    ...
    if (layout_view && frame_->IsMainFrame() && frame_->GetPage()->GetBrowserControls().TotalHeight())
        layout_view->SetShouldCheckForPaintInvalidation();
...
{
    Page* page = GetFrame().GetPage();
    return page && page->IsPageVisible() && !frame_->GetEventHandler().IsMousePositionUnknown() && page->GetFocusController().IsActive();
}
...
void LocalFrameView::UpdateCanCompositeBackgroundAttachmentFixed()
{
    ...
    bool enable_composited_background_attachment_fixed = background_attachment_fixed_objects_.size() <= kMaxCompositedBackgroundAttachmentFixed;
    for (const auto& object : background_attachment_fixed_objects_) {
        object->UpdateCanCompositeBackgroundAttachmentFixed(enable_composited_background_attachment_fixed);
    }
}

void LocalFrameView::InvalidateBackgroundAttachmentFixedDescendantsOnScroll(const LayoutBox& scroller)
{
    for (const auto& layout_object : background_attachment_fixed_objects_) {
        if (scroller != GetLayoutView() && !layout_object->IsDescendantOf(&scroller)) {
            continue;
        }
...
HitTestResult LocalFrameView::HitTestWithThrottlingAllowed(const HitTestLocation& location, HitTestRequest::HitTestRequestType request_type) const
{
    AllowThrottlingScope allow_throttling(*this);
    return GetFrame().GetEventHandler().HitTestResultAtLocation(location, request_type);
}

void LocalFrameView::ProcessUrlFragment(const KURL& url, bool same_document_navigation, bool should_scroll)
{
    ...
    FragmentAnchor* anchor = FragmentAnchor::TryCreate(url, *frame_, should_scroll);
...
{
    TRACE_EVENT1("blink", "LocalFrameView::HandleLoadCompleted", "has_auto_size_info", !!auto_size_info_);
...
bool LocalFrameView::RunPostLayoutIntersectionObserverSteps()
{
    ...
    ForAllNonThrottledLocalFrameViews([&needs_more_lifecycle_steps](LocalFrameView& frame_view) {
        if (auto* controller = frame_view.GetFrame().GetDocument()->GetIntersectionObserverController()) {
            controller->DeliverNotifications(IntersectionObserver::kDeliverDuringPostLayoutSteps);
        }
        // If the lifecycle state changed as a result of the notifications, we
        // should run the lifecycle again.
        needs_more_lifecycle_steps |= frame_view.Lifecycle().GetState() < DocumentLifecycle::kPrePaintClean || frame_view.NeedsLayout();
    });

    return needs_more_lifecycle_steps;
}

void LocalFrameView::ComputePostLayoutIntersections(unsigned parent_flags, ComputeIntersectionsContext& context)
{
    if (ShouldThrottleRendering())
        return;

    unsigned flags = GetIntersectionObservationFlags(parent_flags) | IntersectionObservation::kPostLayoutDeliveryOnly;

    if (auto* controller = GetFrame().GetDocument()->GetIntersectionObserverController()) {
        controller->ComputeIntersections(flags, *this, accumulated_scroll_delta_since_last_intersection_update_, context);
...
void LocalFrameView::ScheduleRelayout()
{
    ...
    if (!frame_->GetDocument()->ShouldScheduleLayout())
        return;
    DEVTOOLS_TIMELINE_TRACE_EVENT_INSTANT_WITH_CATEGORIES(TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "InvalidateLayout",
        inspector_invalidate_layout_event::Data, frame_.Get(), GetLayoutView()->OwnerNodeId());
...
void LocalFrameView::ScheduleRelayoutOfSubtree(LayoutObject* relayout_root)
{
    ...
    DEVTOOLS_TIMELINE_TRACE_EVENT_INSTANT_WITH_CATEGORIES(TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "InvalidateLayout",
        inspector_invalidate_layout_event::Data, frame_.Get(), relayout_root->OwnerNodeId());
}
...
bool LocalFrameView::NeedsLayout() const
{
    ...
    auto* layout_view = GetLayoutView();
    return LayoutPending() || (layout_view && layout_view->NeedsLayout()) || IsSubtreeLayout();
}
...
bool LocalFrameView::ShouldUseColorAdjustBackground() const
{
    return use_color_adjust_background_ == UseColorAdjustBackground::kYes
        || (use_color_adjust_background_ == UseColorAdjustBackground::kIfBaseNotTransparent && base_background_color_ != Color::kTransparent);
}
...
void LocalFrameView::SetUseColorAdjustBackground(UseColorAdjustBackground use, bool color_scheme_changed)
{
    ...
    if (GetFrame().IsMainFrame() && ShouldUseColorAdjustBackground()) {
        ...
        GetFrame().DidChangeBackgroundColor(BaseBackgroundColor().toSkColor4f(), true /* color_adjust */);
    }
...
{
    return ShouldUseColorAdjustBackground() || frame_->GetDocument()->IsInMainFrame();
}

void LocalFrameView::UpdateBaseBackgroundColorRecursively(const Color& base_background_color)
{
    ForAllNonThrottledLocalFrameViews([base_background_color](LocalFrameView& frame_view) { frame_view.SetBaseBackgroundColor(base_background_color); });
}
...
void LocalFrameView::PerformPostLayoutTasks(bool visual_viewport_size_changed)
{
    ...
    TRACE_EVENT_OBJECT_SNAPSHOT_WITH_ID(
        TRACE_DISABLED_BY_DEFAULT("blink.debug.layout.trees"), "LayoutTree", this, TracedLayoutObject::Create(*GetLayoutView(), true));
...
float LocalFrameView::InputEventsScaleFactor() const
{
    ...
    return page_scale * frame_->GetPage()->GetChromeClient().InputEventsScaleForEmulation();
}
...
void LocalFrameView::UpdateDocumentDraggableRegions() const
{
    ...
    if (!document->HasDraggableRegions() || !frame_->GetPage()->GetChromeClient().SupportsDraggableRegions()) {
        return;
    }
...
void LocalFrameView::InitializeRootScroller()
{
    ...
    auto* root_frame_viewport = MakeGarbageCollected<RootFrameViewport>(visual_viewport, *layout_viewport);
    viewport_scrollable_area_ = root_frame_viewport;

    DCHECK(frame_->GetDocument());
    page->GlobalRootScrollerController().Initialize(*root_frame_viewport, *frame_->GetDocument());
}
...
Color LocalFrameView::DocumentBackgroundColor()
{
    ...
    Color doc_bg = background_source->ResolveColor(GetCSSPropertyBackgroundColor());
    if (background_source->StyleRef().ColorSchemeForced()) {
        ...
        doc_bg = Color::FromSkColor4f(EnsureDarkModeFilter().InvertColorIfNeeded(doc_bg.toSkColor4f(), DarkModeFilter::ElementRole::kBackground));
    }
    if (blend_with_base)
...
void LocalFrameView::UpdateGeometriesIfNeeded()
{
    ...
    ForAllChildViewsAndPlugins([&](EmbeddedContentView& view) { views.push_back(view); });
...
{
    AllowThrottlingScope allow_throttling(*this);
    bool updated = GetFrame().LocalFrameRoot().View()->UpdateLifecyclePhases(DocumentLifecycle::kPaintClean, reason);

#if DCHECK_IS_ON()
    if (updated) {
        // This function should return true iff all non-throttled frames are in the
        // kPaintClean lifecycle state.
        ForAllNonThrottledLocalFrameViews([](LocalFrameView& frame_view) { DCHECK_EQ(frame_view.Lifecycle().GetState(), DocumentLifecycle::kPaintClean); });
        ...
        ForAllThrottledLocalFrameViews([](LocalFrameView& frame_view) {
            DCHECK(frame_view.intersection_observation_state_ != kRequired || frame_view.IsDisplayLocked()
                || frame_view.Lifecycle().GetState() >= DocumentLifecycle::kLayoutClean);
        });
    }
#endif
...
bool LocalFrameView::UpdateLifecycleToCompositingInputsClean(DocumentUpdateReason reason)
{
    return GetFrame().LocalFrameRoot().View()->UpdateLifecyclePhases(DocumentLifecycle::kCompositingInputsClean, reason);
}

bool LocalFrameView::UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason reason)
{
    return GetFrame().LocalFrameRoot().View()->UpdateLifecyclePhases(DocumentLifecycle::kPrePaintClean, reason);
}
...
void LocalFrameView::DryRunPaintingForPrerender()
{
    ...
    bool update_result = GetFrame().LocalFrameRoot().View()->UpdateLifecyclePhases(DocumentLifecycle::kPrePaintClean, DocumentUpdateReason::kPrerender);
    if (!update_result) {
        return;
    }
    if (paint_artifact_compositor_) {
...
void LocalFrameView::UpdateLifecyclePhasesForPrinting()
{
    ...
    local_frame_view_root->UpdateLifecyclePhases(DocumentLifecycle::kPrePaintClean, DocumentUpdateReason::kPrinting);
    ...
    if (local_frame_view_root != this && !IsAttached()) {
        ...
        UpdateLifecyclePhases(DocumentLifecycle::kPrePaintClean, DocumentUpdateReason::kPrinting);
    }
}

bool LocalFrameView::UpdateLifecycleToLayoutClean(DocumentUpdateReason reason)
{
    return GetFrame().LocalFrameRoot().View()->UpdateLifecyclePhases(DocumentLifecycle::kLayoutClean, reason);
}

LocalFrameView::InvalidationDisallowedScope::InvalidationDisallowedScope(const LocalFrameView& frame_view)
    : resetter_(&frame_view.GetFrame().LocalFrameRoot().View()->invalidation_disallowed_, true)
{
...
void LocalFrameView::ScheduleVisualUpdateForVisualOverflowIfNeeded()
{
    ...
    if (local_frame_root.View()->target_state_ < DocumentLifecycle::kPaintClean || Lifecycle().GetState() >= DocumentLifecycle::kCompositingInputsClean) {
...
void LocalFrameView::ScheduleVisualUpdateForPaintInvalidationIfNeeded()
{
    ...
    if (local_frame_root.View()->target_state_ < DocumentLifecycle::kPaintClean || Lifecycle().GetState() >= DocumentLifecycle::kPrePaintClean) {
        // Schedule visual update to process the paint invalidation in the next
        // cycle.
        local_frame_root.ScheduleVisualUpdateUnlessThrottled();
    }
...
bool LocalFrameView::NotifyResizeObservers()
{
    ...
    ResizeObserverController* resize_controller = ResizeObserverController::FromIfExists(*GetFrame().DomWindow());
    ...
    if (min_depth != ResizeObserverController::kDepthBottom) {
        ...
        if (resize_controller->SkippedObservations() && !resize_controller->IsLoopLimitErrorDispatched()) {
            resize_controller->ClearObservations();

            if (auto* script_state = ToScriptStateForMainWorld(frame_->DomWindow())) {
                ScriptState::Scope scope(script_state);
                const String message = "ResizeObserver loop completed with undelivered notifications.";
                ScriptValue value(script_state->GetIsolate(), V8String(script_state->GetIsolate(), message));
                // TODO(pdr): We could report the source location of one of the
                // observers which had skipped observations.
                ErrorEvent* error = ErrorEvent::Create(message, CaptureSourceLocation(), value, &script_state->World());
                // We're using |SanitizeScriptErrors::kDoNotSanitize| as the error is
                // made by blink itself.
                // TODO(yhirano): Reconsider this.
                frame_->DomWindow()->DispatchErrorEvent(error, SanitizeScriptErrors::kDoNotSanitize);
            }
...
void LocalFrameView::PrepareForLifecycleUpdateRecursive()
{
    ...
    if (!ShouldThrottleRendering() || intersection_observation_state_ == kRequired) {
        Lifecycle().EnsureStateAtMost(DocumentLifecycle::kVisualUpdatePending);
        ForAllChildLocalFrameViews([](LocalFrameView& child) { child.PrepareForLifecycleUpdateRecursive(); });
    }
}

// TODO(leviw): We don't assert lifecycle information from documents in child
// WebPluginContainerImpls.
bool LocalFrameView::UpdateLifecyclePhases(DocumentLifecycle::LifecycleState target_state, DocumentUpdateReason reason)
{
    // If the lifecycle is postponed, which can happen if the inspector requests
    // it, then we shouldn't update any lifecycle phases.
    if (frame_->GetDocument() && frame_->GetDocument()->Lifecycle().LifecyclePostponed()) [[unlikely]] {
        return false;
    }
    ...
    if (IsUpdatingLifecycle()) [[unlikely]] {
        DUMP_WILL_BE_NOTREACHED() << "LocalFrameView::updateLifecyclePhasesInternal() reentrance";
        return false;
    }
    ...
    DCHECK(LocalFrameTreeAllowsThrottling() || (target_state < DocumentLifecycle::kPaintClean));

    // Only the following target states are supported.
    DCHECK(target_state == DocumentLifecycle::kLayoutClean || target_state == DocumentLifecycle::kCompositingInputsClean
        || target_state == DocumentLifecycle::kPrePaintClean || target_state == DocumentLifecycle::kPaintClean);
    ...
    base::AutoReset<DocumentLifecycle::LifecycleState> target_state_scope(&target_state_, target_state);
    ...
    if (target_state == DocumentLifecycle::kPaintClean) {
        ...
        {
            TRACE_EVENT0("blink", "LocalFrameView::UpdateLifecyclePhases - start of lifecycle tasks");
            ForAllNonThrottledLocalFrameViews([](LocalFrameView& frame_view) {
    ...
    if (target_state == DocumentLifecycle::kPaintClean) {
        ...
        if (frame_->GetWidgetForLocalRoot() && RuntimeEnabledFeatures::ReportVisibleLineBoundsEnabled()) {
    ...
    if (reason == DocumentUpdateReason::kHitTest) {
        if (auto* metrics_aggregator = GetUkmAggregator()) {
            metrics_aggregator->RecordTimerSample(
                static_cast<size_t>(LocalFrameUkmAggregator::kHitTestDocumentUpdate), lifecycle_data_.start_time, base::TimeTicks::Now());
        }
    }
...
void LocalFrameView::UpdateLifecyclePhasesInternal(DocumentLifecycle::LifecycleState target_state)
{
    ...
    ForAllNonThrottledLocalFrameViews([&unthrottled_frame_views](LocalFrameView& frame_view) { unthrottled_frame_views.push_back(&frame_view); });
    ...
    while (true) {
        for (LocalFrameView* frame_view : unthrottled_frame_views) {
            ...
            if (frame_view->GetFrame().IsAttached()) {
                frame_view->Lifecycle().EnsureStateAtMost(DocumentLifecycle::kVisualUpdatePending);
            }
        }
...

        {
            ...
            DEVTOOLS_TIMELINE_TRACE_EVENT_INSTANT_WITH_CATEGORIES(
                TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "SetLayerTreeId", inspector_set_layer_tree_id::Data, frame_.Get());
            ...
            // under PrePaint.
            DEVTOOLS_TIMELINE_TRACE_EVENT("PrePaint", inspector_pre_paint_event::Data, frame_.Get());
        ...
        DCHECK(ShouldThrottleRendering() || Lifecycle().GetState() >= DocumentLifecycle::kPrePaintClean);
...

        {
            ...
            base::AutoReset<DocumentLifecycle::LifecycleState> saved_target_state(&target_state_, DocumentLifecycle::kUninitialized);
    ...
    ForAllNonThrottledLocalFrameViews([](LocalFrameView& frame_view) { frame_view.frame_->CheckPositionAnchorsForChainedVisibilityChanges(); });
    ...
    // on cc::Layer bounds.
    ForAllRemoteFrameViews([](RemoteFrameView& frame_view) { frame_view.UpdateCompositingRect(); });

    DCHECK_EQ(target_state, DocumentLifecycle::kPaintClean);
    RunPaintLifecyclePhase(PaintBenchmarkMode::kNormal);
    DCHECK(ShouldThrottleRendering() || AnyFrameIsPrintingOrPaintingPreview() || Lifecycle().GetState() == DocumentLifecycle::kPaintClean);
}
...
bool LocalFrameView::RunScrollSnapshotClientSteps()
{
    ...
    ForAllNonThrottledLocalFrameViews([&re_run_lifecycles](LocalFrameView& frame_view) {
...
bool LocalFrameView::RunViewTransitionSteps(DocumentLifecycle::LifecycleState target_state)
{
    ...
    ForAllNonThrottledLocalFrameViews([&re_run_lifecycle, target_state](LocalFrameView& frame_view) {
        ...
        re_run_lifecycle |= document->Lifecycle().GetState() < DocumentLifecycle::kPrePaintClean || frame_view.NeedsLayout();
    });

    return re_run_lifecycle;
}

bool LocalFrameView::RunResizeObserverSteps(DocumentLifecycle::LifecycleState target_state)
{
    if (target_state != DocumentLifecycle::kPaintClean)
        return false;
...
{
    ForAllNonThrottledLocalFrameViews([](LocalFrameView& frame_view) {
        ResizeObserverController* resize_controller = ResizeObserverController::From(*frame_view.frame_->DomWindow());
...
{
    TRACE_EVENT0("blink,benchmark", "LocalFrameView::RunStyleAndLayoutLifecyclePhases");
    UpdateStyleAndLayoutIfNeededRecursive();
    DCHECK(ShouldThrottleRendering() || Lifecycle().GetState() >= DocumentLifecycle::kLayoutClean);
    ...
    if (frame_->GetDocument()->GetRootScrollerController().PerformRootScrollerSelection() && RuntimeEnabledFeatures::ImplicitRootScrollerEnabled()) {
        UpdateStyleAndLayoutIfNeededRecursive();
    }
    ...
    ForAllNonThrottledLocalFrameViews([](LocalFrameView& frame_view) { frame_view.PerformScrollAnchoringAdjustments(); });
    ...
    if (target_state == DocumentLifecycle::kPaintClean) {
        ForAllNonThrottledLocalFrameViews([](LocalFrameView& frame_view) { frame_view.NotifyFrameRectsChangedIfNeeded(); });
    }
...
bool LocalFrameView::RunCompositingInputsLifecyclePhase(DocumentLifecycle::LifecycleState target_state)
{
    TRACE_EVENT0("blink,benchmark", "LocalFrameView::RunCompositingInputsLifecyclePhase");
    auto* layout_view = GetLayoutView();
    DCHECK(layout_view);

    SCOPED_UMA_AND_UKM_TIMER(GetUkmAggregator(), LocalFrameUkmAggregator::kCompositingInputs);
...
#endif
        ForAllNonThrottledLocalFrameViews([](LocalFrameView& frame_view) {
            frame_view.Lifecycle().AdvanceTo(DocumentLifecycle::kInCompositingInputsUpdate);
            ...
            if (LocalDOMWindow* window = frame_view.GetFrame().DomWindow()) {
                if (HighlightRegistry* highlight_registry = window->Supplementable<LocalDOMWindow>::RequireSupplement<HighlightRegistry>()) {
    ...
    ForAllNonThrottledLocalFrameViews([](LocalFrameView& frame_view) { frame_view.Lifecycle().AdvanceTo(DocumentLifecycle::kCompositingInputsClean); });

    return target_state > DocumentLifecycle::kCompositingInputsClean;
}

bool LocalFrameView::RunPrePaintLifecyclePhase(DocumentLifecycle::LifecycleState target_state)
{
        ...
        [](LocalFrameView& frame_view) {
            ...
            if (LayoutView* layout_view = frame_view.GetLayoutView()) {
                if (auto* owner = frame_view.GetFrame().OwnerLayoutObject()) {
                    if (layout_view->NeedsPaintPropertyUpdate() || layout_view->DescendantNeedsPaintPropertyUpdate()) {
                        owner->SetDescendantNeedsPaintPropertyUpdate();
                    }
                    if (layout_view->ShouldCheckForPaintInvalidation()) {
                        owner->SetShouldCheckForPaintInvalidation();
                    }
                    if (layout_view->EffectiveAllowedTouchActionChanged() || layout_view->DescendantEffectiveAllowedTouchActionChanged()) {
                        owner->MarkDescendantEffectiveAllowedTouchActionChanged();
                    }
                    if (layout_view->BlockingWheelEventHandlerChanged() || layout_view->DescendantBlockingWheelEventHandlerChanged()) {
    ...
    {
        SCOPED_UMA_AND_UKM_TIMER(GetUkmAggregator(), LocalFrameUkmAggregator::kPrePaint);
    ...
    ForAllNonThrottledLocalFrameViews([](LocalFrameView& frame_view) { frame_view.Lifecycle().AdvanceTo(DocumentLifecycle::kPrePaintClean); });
...
bool LocalFrameView::AnyFrameIsPrintingOrPaintingPreview()
{
    ...
    ForAllNonThrottledLocalFrameViews([&any_frame_is_printing_or_painting_preview](LocalFrameView& frame_view) {
...
void LocalFrameView::RunPaintLifecyclePhase(PaintBenchmarkMode benchmark_mode)
{
    ...
    bool needed_full_update = false;
    {
        ...
        if (paint_artifact_compositor_ && benchmark_mode == PaintBenchmarkMode::kForcePaintArtifactCompositorUpdate) {
            paint_artifact_compositor_->SetNeedsUpdate();
        }
        needed_full_update = !paint_artifact_compositor_ || paint_artifact_compositor_->NeedsUpdate() == PaintArtifactCompositor::UpdateType::kFull;
        PushPaintArtifactToCompositor(paint_controller.has_value());
    }

    size_t total_animations_count = 0;
    ForAllNonThrottledLocalFrameViews([this, needed_full_update, &total_animations_count](LocalFrameView& frame_view) {
        if (auto* scrollable_area = frame_view.GetScrollableArea())
            scrollable_area->UpdateCompositorScrollAnimations();
        if (const auto* animating_scrollable_areas = frame_view.AnimatingScrollableAreas()) {
            for (PaintLayerScrollableArea* area : *animating_scrollable_areas)
                area->UpdateCompositorScrollAnimations();
        }
        frame_view.GetPage()->GetLinkHighlight().UpdateAfterPaint(paint_artifact_compositor_.Get());
        ...
        document.AttachCompositorTimeline(document.Timeline().CompositorTimeline());
        {
            ...
            ScriptForbiddenScope forbid_script;
            document.GetDocumentAnimations().UpdateAnimations(DocumentLifecycle::kPaintClean, paint_artifact_compositor_.Get(), needed_full_update);
        }
        total_animations_count += document.GetDocumentAnimations().GetAnimationsCount();
    });
...
void LocalFrameView::RunAccessibilitySteps()
{
    ...
    SCOPED_UMA_AND_UKM_TIMER(GetUkmAggregator(), LocalFrameUkmAggregator::kAccessibility);

    // Reduce redundant ancestor chain walking for display lock computations.
    auto display_lock_memoization_scope = DisplayLockUtilities::CreateLockCheckMemoizationScope();
...
void LocalFrameView::EnqueueScrollAnchoringAdjustment(ScrollableArea* scrollable_area)
{
    anchoring_adjustment_queue_.insert(scrollable_area);
}

void LocalFrameView::DequeueScrollAnchoringAdjustment(ScrollableArea* scrollable_area)
{
    anchoring_adjustment_queue_.erase(scrollable_area);
}

void LocalFrameView::SetNeedsEnqueueScrollEvent(PaintLayerScrollableArea* scrollable_area)
{
...
{
    ForAllNonThrottledLocalFrameViews([](LocalFrameView& frame_view) {
        for (const WeakMember<PaintLayerScrollableArea>& scroller : frame_view.scroll_event_queue_) {
            if (scroller)
...
void LocalFrameView::PaintTree(PaintBenchmarkMode benchmark_mode, std::optional<PaintController>& paint_controller)
{
    ...
    bool debug_info_newly_enabled = UpdatePaintDebugInfoEnabled() && PaintDebugInfoEnabled();
    ...
    ForAllThrottledLocalFrameViews([](LocalFrameView& frame_view) { frame_view.MarkIneligibleToPaint(); });

    bool needs_clear_repaint_flags = false;

    if (benchmark_mode >= PaintBenchmarkMode::kForcePaint || !paint_controller_persistent_data_ || GetLayoutView()->Layer()->SelfOrDescendantNeedsRepaint()
        || visual_viewport_or_overlay_needs_repaint_) {
        const PaintArtifact& previous_artifact = EnsurePaintControllerPersistentData().GetPaintArtifact();
        paint_controller.emplace(PaintDebugInfoEnabled(), paint_controller_persistent_data_.Get(), benchmark_mode);
        ...
        if (PaintLayer* full_screen_layer = GetXROverlayLayer()) {
            ...
            ForAllNonThrottledLocalFrameViews([&graphics_context](LocalFrameView& view) { view.frame_->PaintFrameColorOverlay(graphics_context); });
        ...
        if (paint_artifact_compositor_) {
            paint_artifact_compositor_->SetNeedsUpdateAfterRepaint(previous_artifact, paint_controller_persistent_data_->GetPaintArtifact());
        }
    }

    visual_viewport_or_overlay_needs_repaint_ = false;

    ForAllNonThrottledLocalFrameViews([needs_clear_repaint_flags](LocalFrameView& frame_view) {
...
{
    return paint_artifact_compositor_ ? paint_artifact_compositor_->RootLayer() : nullptr;
}
    ...
    if (const cc::Layer* root_layer = paint_artifact_compositor_->RootLayer()) {
        for (const auto& layer : root_layer->children()) {
            if (!layer->update_rect().IsEmpty()) {
                DEVTOOLS_TIMELINE_TRACE_EVENT_INSTANT_WITH_CATEGORIES("devtools.timeline,rail", "Paint", inspector_paint_event::Data, &GetFrame(),
                    /*layout_object=*/nullptr, GetQuadForTimelinePaintEvent(layer), layer->id());
            }
        }
    }
}
...
void LocalFrameView::PushPaintArtifactToCompositor(bool repainted)
{
    ...
    if (!paint_artifact_compositor_) {
        paint_artifact_compositor_ = MakeGarbageCollected<PaintArtifactCompositor>(page->GetScrollingCoordinator()->GetScrollCallbacks());
        page->GetChromeClient().AttachRootLayer(paint_artifact_compositor_->RootLayer(), &GetFrame());
    }

    paint_artifact_compositor_->SetLCDTextPreference(page->GetSettings().GetLCDTextPreference());

    SCOPED_UMA_AND_UKM_TIMER(GetUkmAggregator(), LocalFrameUkmAggregator::kCompositingCommit);
    DEVTOOLS_TIMELINE_TRACE_EVENT("Layerize", inspector_layerize_event::Data, frame_.Get());

    // Skip updating property trees, pushing cc::Layers, and issuing raster
    // invalidations if possible.
    if (paint_artifact_compositor_->TryFastPathUpdate(paint_controller_persistent_data_->GetPaintArtifact())) {
        if (repainted) {
    ...
    paint_artifact_compositor_->SetLayerDebugInfoEnabled(paint_debug_info_enabled_);

    PaintArtifactCompositor::ViewportProperties viewport_properties;
    if (const auto& viewport = page->GetVisualViewport(); GetFrame().IsMainFrame() && viewport.IsActiveViewport()) {
        viewport_properties.overscroll_elasticity_transform = viewport.GetOverscrollElasticityTransformNode();
        viewport_properties.page_scale = viewport.GetPageScaleNode();

        if (const auto* root_scroller = GetPage()->GlobalRootScrollerController().GlobalRootScroller()) {
            if (const auto* layout_object = root_scroller->GetLayoutObject()) {
                if (const auto* paint_properties = layout_object->FirstFragment().PaintProperties()) {
                    if (paint_properties->Scroll()) {
                        viewport_properties.outer_clip = paint_properties->OverflowClip();
                        viewport_properties.outer_scroll_translation = paint_properties->ScrollTranslation();
                        viewport_properties.inner_scroll_translation = viewport.GetScrollTranslationNode();
                    }
                }
            }
        }
    ...
    PaintArtifactCompositor::StackScrollTranslationVector scroll_translation_nodes;
    ForAllNonThrottledLocalFrameViews([&scroll_translation_nodes](LocalFrameView& frame_view) {
        if (const auto* scrollable_areas = frame_view.UserScrollableAreas()) {
            for (const auto& area : scrollable_areas->Values()) {
                const auto* paint_properties = area->GetLayoutBox()->FirstFragment().PaintProperties();
    ...
    paint_artifact_compositor_->Update(
        paint_controller_persistent_data_->GetPaintArtifact(), viewport_properties, scroll_translation_nodes, std::move(view_transition_requests));

    CreatePaintTimelineEvents();
}

void LocalFrameView::AppendViewTransitionRequests(WTF::Vector<std::unique_ptr<ViewTransitionRequest>>& requests)
{
    DCHECK(frame_ && frame_->GetDocument());
    DCHECK(frame_->IsLocalRoot());
    ...
    ForAllNonThrottledLocalFrameViews([&requests](LocalFrameView& frame_view) {
        ...
        auto pending_requests = ViewTransitionUtils::GetPendingRequests(*frame_view.GetFrame().GetDocument());
        for (auto& pending_request : pending_requests)
            requests.push_back(std::move(pending_request));
    });
}

std::unique_ptr<JSONObject> LocalFrameView::CompositedLayersAsJSON(LayerTreeFlags flags)
{
...
void LocalFrameView::UpdateStyleAndLayoutIfNeededRecursive()
{
    ...
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("blink.debug"), "LocalFrameView::updateStyleAndLayoutIfNeededRecursive");
    ...
    if (GetFrame().GetDocument()->HasFinishedParsing() && !GetFrame().GetDocument()->IsInitialEmptyDocument())
...
void LocalFrameView::UpdateStyleAndLayout()
{
    ...
    if (IsInPerformLayout() || ShouldThrottleRendering() || !frame_->GetDocument()->IsActive() || frame_->IsProvisional() || Lifecycle().LifecyclePostponed()) {
        return;
    }

    gfx::Size visual_viewport_size = GetScrollableArea()->VisibleContentRect().size();
    ...
    if (did_layout) {
        gfx::Size new_visual_viewport_size = GetScrollableArea()->VisibleContentRect().size();
        bool visual_viewport_size_changed = (new_visual_viewport_size != visual_viewport_size);
...
bool LocalFrameView::UpdateStyleAndLayoutInternal()
{
    ...
    do {
        ...
        if (NeedsLayout()) {
            SCOPED_UMA_AND_UKM_TIMER(GetUkmAggregator(), LocalFrameUkmAggregator::kLayout);
            UpdateLayout();
            layout_updated = true;
        }
...
void LocalFrameView::EnableAutoSizeMode(const gfx::Size& min_size, const gfx::Size& max_size)
{
    if (!auto_size_info_)
...
void LocalFrameView::DisableAutoSizeMode()
{
    ...
    GetLayoutView()->SetAutosizeScrollbarModes(mojom::blink::ScrollbarMode::kAuto, mojom::blink::ScrollbarMode::kAuto);
    auto_size_info_.Clear();
}
...
void LocalFrameView::ForceLayoutForPagination(float maximum_shrink_factor)
{
    ...
    auto LayoutForPrinting = [&layout_view, &document]() {
        ...
        layout_view->SetNeedsLayoutAndIntrinsicWidthsRecalcAndFullPaintInvalidation(layout_invalidation_reason::kPrintingChanged);
    ...
    PhysicalSize initial_containing_block_size = CalculateInitialContainingBlockSizeForPagination(document);
    layout_view->SetInitialContainingBlockSizeForPrinting(initial_containing_block_size);

    LayoutForPrinting();

    PhysicalSize new_initial_containing_block_size = CalculateInitialContainingBlockSizeForPagination(document);
    if (new_initial_containing_block_size != initial_containing_block_size) {
        ...
        layout_view->SetInitialContainingBlockSizeForPrinting(new_initial_containing_block_size);
    ...
    float overall_scale_factor = CalculateOverflowShrinkForPrinting(*layout_view, maximum_shrink_factor);
    ...
    if (overall_scale_factor > 1.0) {
        ...
        layout_view->SetPaginationScaleFactor(layout_view->PaginationScaleFactor() * overall_scale_factor);
        PhysicalSize new_size = CalculateInitialContainingBlockSizeForPagination(document);
...
gfx::Rect LocalFrameView::RootFrameToDocument(const gfx::Rect& rect_in_root_frame)
{
...
gfx::Point LocalFrameView::RootFrameToDocument(const gfx::Point& point_in_root_frame)
{
    return gfx::ToFlooredPoint(RootFrameToDocument(gfx::PointF(point_in_root_frame)));
}

gfx::PointF LocalFrameView::RootFrameToDocument(const gfx::PointF& point_in_root_frame)
{
    ScrollableArea* layout_viewport = LayoutViewport();
    if (!layout_viewport)
...
gfx::Rect LocalFrameView::DocumentToFrame(const gfx::Rect& rect_in_document) const
{
...
gfx::Point LocalFrameView::DocumentToFrame(const gfx::Point& point_in_document) const
{
    return gfx::ToFlooredPoint(DocumentToFrame(gfx::PointF(point_in_document)));
}

gfx::PointF LocalFrameView::DocumentToFrame(const gfx::PointF& point_in_document) const
{
    ScrollableArea* layout_viewport = LayoutViewport();
    if (!layout_viewport)
...
PhysicalOffset LocalFrameView::DocumentToFrame(const PhysicalOffset& offset_in_document) const
{
    ScrollableArea* layout_viewport = LayoutViewport();
    if (!layout_viewport)
        return offset_in_document;

    return offset_in_document - PhysicalOffset::FromVector2dFRound(layout_viewport->GetScrollOffset());
}

PhysicalRect LocalFrameView::DocumentToFrame(const PhysicalRect& rect_in_document) const
{
    return PhysicalRect(DocumentToFrame(rect_in_document.offset), rect_in_document.size);
}

gfx::Point LocalFrameView::FrameToDocument(const gfx::Point& point_in_frame) const
{
    return ToFlooredPoint(FrameToDocument(PhysicalOffset(point_in_frame)));
}

PhysicalOffset LocalFrameView::FrameToDocument(const PhysicalOffset& offset_in_frame) const
{
    ScrollableArea* layout_viewport = LayoutViewport();
    if (!layout_viewport)
        return offset_in_frame;

    return offset_in_frame + PhysicalOffset::FromVector2dFRound(layout_viewport->GetScrollOffset());
}

gfx::Rect LocalFrameView::FrameToDocument(const gfx::Rect& rect_in_frame) const
{
    return gfx::Rect(FrameToDocument(rect_in_frame.origin()), rect_in_frame.size());
}

PhysicalRect LocalFrameView::FrameToDocument(const PhysicalRect& rect_in_frame) const
{
    return PhysicalRect(FrameToDocument(rect_in_frame.offset), rect_in_frame.size);
}

gfx::Rect LocalFrameView::ConvertToContainingEmbeddedContentView(const gfx::Rect& local_rect) const
{
    if (ParentFrameView()) {
        ...
        return ToPixelSnappedRect(layout_object->LocalToAbsoluteRect(PhysicalRect(rect)));
    }

    return local_rect;
}

gfx::Rect LocalFrameView::ConvertFromContainingEmbeddedContentView(const gfx::Rect& parent_rect) const
{
    if (ParentFrameView()) {
...
PhysicalOffset LocalFrameView::ConvertToContainingEmbeddedContentView(const PhysicalOffset& local_offset) const
{
    if (ParentFrameView()) {
        auto* layout_object = GetLayoutEmbeddedContent();
        if (!layout_object)
...
gfx::PointF LocalFrameView::ConvertToContainingEmbeddedContentView(const gfx::PointF& local_point) const
{
    if (ParentFrameView()) {
        auto* layout_object = GetLayoutEmbeddedContent();
        if (!layout_object)
...
PhysicalOffset LocalFrameView::ConvertFromContainingEmbeddedContentView(const PhysicalOffset& parent_offset) const
{
    return PhysicalOffset::FromPointFRound(ConvertFromContainingEmbeddedContentView(gfx::PointF(parent_offset)));
}

gfx::PointF LocalFrameView::ConvertFromContainingEmbeddedContentView(const gfx::PointF& parent_point) const
{
    if (ParentFrameView()) {
...
void LocalFrameView::SetTracksRasterInvalidations(bool track_raster_invalidations)
{
    if (!GetFrame().IsLocalRoot()) {
        GetFrame().LocalFrameRoot().View()->SetTracksRasterInvalidations(track_raster_invalidations);
        return;
    }
    ...
    if (paint_artifact_compositor_) {
        paint_artifact_compositor_->SetTracksRasterInvalidations(track_raster_invalidations);
    }

    TRACE_EVENT_INSTANT1(TRACE_DISABLED_BY_DEFAULT("blink.invalidation"), "LocalFrameView::setTracksPaintInvalidations", TRACE_EVENT_SCOPE_GLOBAL, "enabled",
...
void LocalFrameView::ServiceScrollAnimations(base::TimeTicks start_time)
{
    ...
    if (!can_throttle) {
        if (ScrollableArea* scrollable_area = GetScrollableArea()) {
            scrollable_area->ServiceScrollAnimations(start_time.since_origin().InSecondsF());
        }
        if (const ScrollableAreaSet* animating_scrollable_areas = AnimatingScrollableAreas()) {
            // Iterate over a copy, since ScrollableAreas may deregister
            // themselves during the iteration.
            HeapVector<Member<PaintLayerScrollableArea>> animating_scrollable_areas_copy(*animating_scrollable_areas);
            for (PaintLayerScrollableArea* scrollable_area : animating_scrollable_areas_copy) {
                scrollable_area->ServiceScrollAnimations(start_time.since_origin().InSecondsF());
            }
        }
...
void LocalFrameView::ScheduleAnimation(base::TimeDelta delay, base::Location location)
{
    TRACE_EVENT("cc", "LocalFrameView::ScheduleAnimation", "frame", GetFrame(), "delay", delay, "location", location);
...
{
    DCHECK(frame_->IsLocalRoot());
    if (frame_->GetDocument() && !frame_->GetDocument()->IsInitialEmptyDocument() && GetUkmAggregator()) {
        GetUkmAggregator()->OnCommitRequested();
    }
}

void LocalFrameView::AddScrollAnchoringScrollableArea(PaintLayerScrollableArea* scrollable_area)
{
    DCHECK(scrollable_area);
    if (!scroll_anchoring_scrollable_areas_) {
        scroll_anchoring_scrollable_areas_ = MakeGarbageCollected<ScrollableAreaSet>();
    }
    scroll_anchoring_scrollable_areas_->insert(scrollable_area);
}

void LocalFrameView::RemoveScrollAnchoringScrollableArea(PaintLayerScrollableArea* scrollable_area)
{
    if (scroll_anchoring_scrollable_areas_)
        scroll_anchoring_scrollable_areas_->erase(scrollable_area);
}

void LocalFrameView::AddAnimatingScrollableArea(PaintLayerScrollableArea* scrollable_area)
{
    DCHECK(scrollable_area);
...
void LocalFrameView::RemoveAnimatingScrollableArea(PaintLayerScrollableArea* scrollable_area)
{
    if (!animating_scrollable_areas_)
        return;
...
void LocalFrameView::AddUserScrollableArea(PaintLayerScrollableArea* scrollable_area)
{
    DCHECK(scrollable_area);
    if (!user_scrollable_areas_)
        user_scrollable_areas_ = MakeGarbageCollected<ScrollableAreaMap>();
    user_scrollable_areas_->insert(scrollable_area->GetScrollElementId(), scrollable_area);
}

void LocalFrameView::RemoveUserScrollableArea(PaintLayerScrollableArea* scrollable_area)
{
    if (user_scrollable_areas_) {
...
void LocalFrameView::AttachToLayout()
{
    ...
    UpdateRenderThrottlingStatus(IsHiddenForThrottling(), parent_view->CanThrottleRendering(), IsDisplayLocked());
    ...
    if (auto* layout_view = GetLayoutView()) {
        layout_view->AddSubtreePaintPropertyUpdateReason(SubtreePaintPropertyUpdateReason::kPrinting);
    }
}
...
bool LocalFrameView::VisualViewportSuppliesScrollbars()
{
    ...
    if (!frame_->GetSettings() || !frame_->GetSettings()->GetViewportEnabled() || !frame_->GetDocument() || !frame_->GetPage())
        return false;

    if (!LayoutViewport())
        return false;

    const TopDocumentRootScrollerController& controller = frame_->GetPage()->GlobalRootScrollerController();
...
void LocalFrameView::DidChangeScrollOffset()
{
    ...
    if (GetFrame().IsOutermostMainFrame()) {
        GetFrame().GetPage()->GetChromeClient().OutermostMainFrameScrollOffsetChanged();
    }
}

ScrollableArea* LocalFrameView::ScrollableAreaWithElementId(const CompositorElementId& id)
{
...
void LocalFrameView::ScrollRectToVisibleInRemoteParent(const PhysicalRect& rect_to_scroll, mojom::blink::ScrollIntoViewParamsPtr params)
{
    DCHECK(GetFrame().IsLocalRoot());
    ...
    DCHECK(params->cross_origin_boundaries || (!GetFrame().IsMainFrame() || GetFrame().IsOutermostMainFrame()));

    DCHECK(params->cross_origin_boundaries
        || GetFrame().Tree().Parent()->GetSecurityContext()->GetSecurityOrigin()->CanAccess(GetFrame().GetSecurityContext()->GetSecurityOrigin()));
    PhysicalRect new_rect = ConvertToRootFrame(rect_to_scroll);
    GetFrame().GetLocalFrameHostRemote().ScrollRectToVisibleInParentFrame(gfx::RectF(new_rect), std::move(params));
}
...
PhysicalOffset LocalFrameView::ViewportToFrame(const PhysicalOffset& point_in_viewport) const
{
    PhysicalOffset point_in_root_frame
        = PhysicalOffset::FromPointFRound(frame_->GetPage()->GetVisualViewport().ViewportToRootFrame(gfx::PointF(point_in_viewport)));
    return ConvertFromRootFrame(point_in_root_frame);
}

gfx::PointF LocalFrameView::ViewportToFrame(const gfx::PointF& point_in_viewport) const
{
    gfx::PointF point_in_root_frame(frame_->GetPage()->GetVisualViewport().ViewportToRootFrame(point_in_viewport));
    return ConvertFromRootFrame(point_in_root_frame);
}

gfx::Rect LocalFrameView::ViewportToFrame(const gfx::Rect& rect_in_viewport) const
{
    gfx::Rect rect_in_root_frame = frame_->GetPage()->GetVisualViewport().ViewportToRootFrame(rect_in_viewport);
    return ConvertFromRootFrame(rect_in_root_frame);
}

gfx::Point LocalFrameView::ViewportToFrame(const gfx::Point& point_in_viewport) const
{
    return ToRoundedPoint(ViewportToFrame(PhysicalOffset(point_in_viewport)));
}

gfx::Rect LocalFrameView::FrameToViewport(const gfx::Rect& rect_in_frame) const
{
    gfx::Rect rect_in_root_frame = ConvertToRootFrame(rect_in_frame);
    return frame_->GetPage()->GetVisualViewport().RootFrameToViewport(rect_in_root_frame);
}

gfx::Point LocalFrameView::FrameToViewport(const gfx::Point& point_in_frame) const
{
    gfx::Point point_in_root_frame = ConvertToRootFrame(point_in_frame);
    return frame_->GetPage()->GetVisualViewport().RootFrameToViewport(point_in_root_frame);
}

gfx::PointF LocalFrameView::FrameToViewport(const gfx::PointF& point_in_frame) const
{
...
gfx::Point LocalFrameView::SoonToBeRemovedUnscaledViewportToContents(const gfx::Point& point_in_viewport) const
{
    gfx::Point point_in_root_frame = gfx::ToFlooredPoint(frame_->GetPage()->GetVisualViewport().ViewportCSSPixelsToRootFrame(gfx::PointF(point_in_viewport)));
    return ConvertFromRootFrame(point_in_root_frame);
}

LocalFrameView::AllowThrottlingScope::AllowThrottlingScope(const LocalFrameView& frame_view)
    : value_(&frame_view.GetFrame().LocalFrameRoot().View()->allow_throttling_, true)
{
}

LocalFrameView::DisallowThrottlingScope::DisallowThrottlingScope(const LocalFrameView& frame_view)
    : value_(&frame_view.GetFrame().LocalFrameRoot().View()->allow_throttling_, false)
{
}

LocalFrameView::ForceThrottlingScope::ForceThrottlingScope(const LocalFrameView& frame_view)
    : allow_scope_(frame_view)
    , value_(&frame_view.GetFrame().LocalFrameRoot().View()->force_throttling_, true)
{
}

PaintControllerPersistentData& LocalFrameView::EnsurePaintControllerPersistentData()
{
...
bool LocalFrameView::CapturePaintPreview(GraphicsContext& context, const gfx::Vector2d& paint_offset) const
{
    *(int*)1 = 1;
//     std::optional<base::UnguessableToken> maybe_embedding_token = GetFrame().GetEmbeddingToken();
// 
...
//     DrawingRecorder recorder(context, *GetFrame().OwnerLayoutObject(), DisplayItem::kDocumentBackground);
//     context.Save();
...
//     context.Canvas()->recordCustomData(tracker->CreateContentForRemoteFrame(FrameRect(), maybe_embedding_token.value()));
//     context.Restore();
// 
//     // Send a request to the browser to trigger a capture of the frame.
//     GetFrame().GetLocalFrameHostRemote().CapturePaintPreviewOfSubframe(FrameRect(), tracker->Guid());
    return true;
}

void LocalFrameView::Paint(GraphicsContext& context, PaintFlags paint_flags, const CullRect& cull_rect, const gfx::Vector2d& paint_offset) const
{
    const auto* owner_layout_object = GetFrame().OwnerLayoutObject();
    std::optional<Document::PaintPreviewScope> paint_preview;
    if (owner_layout_object && owner_layout_object->GetDocument().GetPaintPreviewState() != Document::kNotPaintingPreview) {
        paint_preview.emplace(*GetFrame().GetDocument(), owner_layout_object->GetDocument().GetPaintPreviewState());
...
void LocalFrameView::PaintFrame(GraphicsContext& context, PaintFlags paint_flags) const
{
    FramePainter(*this).Paint(context, paint_flags);
}

void LocalFrameView::PrintPage(GraphicsContext& context, wtf_size_t page_index, const CullRect& cull_rect)
{
    DCHECK(GetFrame().GetDocument()->Printing());
    if (pagination_state_) {
        pagination_state_->SetCurrentPageIndex(page_index);
    }
    const PaintFlags flags = PaintFlag::kOmitCompositingInfo | PaintFlag::kAddUrlMetadata;
    PaintOutsideOfLifecycle(context, flags, cull_rect);
}

static bool PaintOutsideOfLifecycleIsAllowed(GraphicsContext& context, const LocalFrameView& frame_view)
{
...
void LocalFrameView::PaintOutsideOfLifecycle(GraphicsContext& context, const PaintFlags paint_flags, const CullRect& cull_rect)
{
    ...
    ForAllNonThrottledLocalFrameViews([](LocalFrameView& frame_view) { frame_view.Lifecycle().AdvanceTo(DocumentLifecycle::kInPaint); });

    {
        if (pagination_state_) {
            pagination_state_->UpdateContentAreaPropertiesForCurrentPage(*GetLayoutView());
        }

        bool disable_expansion = paint_flags & PaintFlag::kOmitCompositingInfo;
        OverriddenCullRectScope force_cull_rect(*GetLayoutView()->Layer(), cull_rect, disable_expansion);
        context.GetPaintController().SetRecordDebugInfo(PaintDebugInfoEnabled());
        PaintFrame(context, paint_flags);
    }

    ForAllNonThrottledLocalFrameViews([](LocalFrameView& frame_view) { frame_view.Lifecycle().AdvanceTo(DocumentLifecycle::kPaintClean); });
}

void LocalFrameView::PaintOutsideOfLifecycleWithThrottlingAllowed(GraphicsContext& context, const PaintFlags paint_flags, const CullRect& cull_rect)
{
    AllowThrottlingScope allow_throttling(*this);
...
void LocalFrameView::PaintForTest(const CullRect& cull_rect)
{
    ...
    if (GetLayoutView()->Layer()->SelfOrDescendantNeedsRepaint()) {
        PaintController paint_controller(PaintDebugInfoEnabled(), &EnsurePaintControllerPersistentData());
        GraphicsContext graphics_context(paint_controller);
        PaintFrame(graphics_context);
        paint_controller.CommitNewDisplayItems();
    }
    Lifecycle().AdvanceTo(DocumentLifecycle::kPaintClean);
    CullRectUpdater(*GetLayoutView()->Layer()).UpdateForTesting(CullRect::Infinite());
}
...
PaintRecord LocalFrameView::GetPaintRecord(const gfx::Rect* cull_rect) const
{
    ...
    return paint_controller_persistent_data_->GetPaintArtifact().GetPaintRecord(PropertyTreeState::Root(), cull_rect);
}

const PaintArtifact& LocalFrameView::GetPaintArtifact() const
{
    CHECK_EQ(DocumentLifecycle::kPaintClean, Lifecycle().GetState());
    return GetFrame().LocalFrameRoot().View()->EnsurePaintControllerPersistentData().GetPaintArtifact();
}

gfx::Rect LocalFrameView::ConvertToRootFrame(const gfx::Rect& local_rect) const
{
...
gfx::Point LocalFrameView::ConvertToRootFrame(const gfx::Point& local_point) const
{
    return ToRoundedPoint(ConvertToRootFrame(PhysicalOffset(local_point)));
}

PhysicalOffset LocalFrameView::ConvertToRootFrame(const PhysicalOffset& local_offset) const
{
    if (LocalFrameView* parent = ParentFrameView()) {
        PhysicalOffset parent_offset = ConvertToContainingEmbeddedContentView(local_offset);
        return parent->ConvertToRootFrame(parent_offset);
    }
    return local_offset;
}

gfx::PointF LocalFrameView::ConvertToRootFrame(const gfx::PointF& local_point) const
{
    if (LocalFrameView* parent = ParentFrameView()) {
        gfx::PointF parent_point = ConvertToContainingEmbeddedContentView(local_point);
        return parent->ConvertToRootFrame(parent_point);
    }
    return local_point;
}

PhysicalRect LocalFrameView::ConvertToRootFrame(const PhysicalRect& local_rect) const
{
    if (LocalFrameView* parent = ParentFrameView()) {
        PhysicalOffset parent_offset = ConvertToContainingEmbeddedContentView(local_rect.offset);
...
gfx::Rect LocalFrameView::ConvertFromRootFrame(const gfx::Rect& rect_in_root_frame) const
{
    if (LocalFrameView* parent = ParentFrameView()) {
...
gfx::Point LocalFrameView::ConvertFromRootFrame(const gfx::Point& point_in_root_frame) const
{
    return ToRoundedPoint(ConvertFromRootFrame(PhysicalOffset(point_in_root_frame)));
}

PhysicalOffset LocalFrameView::ConvertFromRootFrame(const PhysicalOffset& offset_in_root_frame) const
{
    if (LocalFrameView* parent = ParentFrameView()) {
        PhysicalOffset parent_point = parent->ConvertFromRootFrame(offset_in_root_frame);
        return ConvertFromContainingEmbeddedContentView(parent_point);
    }
    return offset_in_root_frame;
}

gfx::PointF LocalFrameView::ConvertFromRootFrame(const gfx::PointF& point_in_root_frame) const
{
    if (LocalFrameView* parent = ParentFrameView()) {
        gfx::PointF parent_point = parent->ConvertFromRootFrame(point_in_root_frame);
        return ConvertFromContainingEmbeddedContentView(parent_point);
    }
    return point_in_root_frame;
}
...
void LocalFrameView::ParentVisibleChanged()
{
    ...
    ForAllChildViewsAndPlugins([visible](EmbeddedContentView& embedded_content_view) { embedded_content_view.SetParentVisible(visible); });
}
...
void LocalFrameView::Show()
{
    if (!IsSelfVisible()) {
        SetSelfVisible(true);
        if (IsParentVisible()) {
            ForAllChildViewsAndPlugins([](EmbeddedContentView& embedded_content_view) { embedded_content_view.SetParentVisible(true); });
        }
    }
}
...
void LocalFrameView::Hide()
{
    if (IsSelfVisible()) {
        if (IsParentVisible()) {
            ForAllChildViewsAndPlugins([](EmbeddedContentView& embedded_content_view) { embedded_content_view.SetParentVisible(false); });
        }
...
void LocalFrameView::CollectDraggableRegions(LayoutObject& layout_object, Vector<DraggableRegionValue>& regions) const
{
    ...
    for (LayoutObject* curr = layout_object.SlowFirstChild(); curr; curr = curr->NextSibling())
        CollectDraggableRegions(*curr, regions);
}

bool LocalFrameView::UpdateViewportIntersectionsForSubtree(unsigned parent_flags, ComputeIntersectionsContext& context)
{
    // This will be recomputed, but default to the previous computed value if
    // there's an early return.
    bool needs_occlusion_tracking = false;
    IntersectionObserverController* controller = GetFrame().GetDocument()->GetIntersectionObserverController();
    if (controller) {
    ...
    if (!NeedsLayout() || IsDisplayLocked()) {
        ...
        if (controller) {
            needs_occlusion_tracking = controller->ComputeIntersections(flags, *this, accumulated_scroll_delta_since_last_intersection_update_, context);
            accumulated_scroll_delta_since_last_intersection_update_ = gfx::Vector2dF();
        }
        intersection_observation_state_ = kNotNeeded;
    }
    ...
    {
        SCOPED_UMA_AND_UKM_TIMER(GetUkmAggregator(), LocalFrameUkmAggregator::kUpdateViewportIntersection);
        UpdateViewportIntersection(flags, needs_occlusion_tracking);
    }

    for (Frame* child = frame_->Tree().FirstChild(); child; child = child->Tree().NextSibling()) {
        needs_occlusion_tracking |= child->View()->UpdateViewportIntersectionsForSubtree(flags, context);
    }

    if (DocumentFencedFrames* fenced_frames = DocumentFencedFrames::Get(*frame_->GetDocument())) {
        for (HTMLFencedFrameElement* fenced_frame : fenced_frames->GetFencedFrames()) {
            if (Frame* frame = fenced_frame->ContentFrame()) {
                needs_occlusion_tracking |= frame->View()->UpdateViewportIntersectionsForSubtree(flags, context);
            }
        }
    }
...
{
    if (IntersectionObserverController* controller = GetFrame().GetDocument()->GetIntersectionObserverController()) {
        controller->DeliverNotifications(IntersectionObserver::kDeliverDuringPostLifecycleSteps);
    }
    ForAllChildLocalFrameViews([](LocalFrameView& frame_view) { frame_view.DeliverSynchronousIntersectionObservations(); });
}
...
void LocalFrameView::CrossOriginToNearestMainFrameChanged()
{
    ...
    if (lifecycle_updates_throttled_ || IsSubtreeThrottled() || IsDisplayLocked() || !IsHiddenForThrottling()) {
        return;
    }
    RenderThrottlingStatusChanged();
    // Immediately propagate changes to children.
    UpdateRenderThrottlingStatus(IsHiddenForThrottling(), IsSubtreeThrottled(), IsDisplayLocked(), true);
}
...
void LocalFrameView::SetViewportIntersection(const mojom::blink::ViewportIntersectionState& intersection_state)
{
    if (!last_intersection_state_.Equals(intersection_state)) {
        last_intersection_state_ = intersection_state;
        int viewport_intersect_area = intersection_state.viewport_intersection.size().GetCheckedArea().ValueOrDefault(INT_MAX);
        int outermost_main_frame_area = intersection_state.outermost_main_frame_size.GetCheckedArea().ValueOrDefault(INT_MAX);
        float ratio = 1.0f * viewport_intersect_area / outermost_main_frame_area;
        const float ratio_threshold = 1.0f * features::kLargeFrameSizePercentThreshold.Get() / 100;
...
void LocalFrameView::VisibilityChanged(blink::mojom::FrameVisibility visibility)
{
    frame_->GetLocalFrameHostRemote().VisibilityChanged(visibility);

    // LocalFrameClient member may not be valid in some tests.
    if (frame_->Client() && frame_->Client()->GetWebFrame() && frame_->Client()->GetWebFrame()->Client()) {
        frame_->Client()->GetWebFrame()->Client()->OnFrameVisibilityChanged(visibility);
    }
}
...
void LocalFrameView::UpdateIntersectionObservationStateOnScroll(gfx::Vector2dF scroll_delta)
{
    accumulated_scroll_delta_since_last_intersection_update_ += gfx::Vector2dF(std::abs(scroll_delta.x()), std::abs(scroll_delta.y()));
...
unsigned LocalFrameView::GetIntersectionObservationFlags(unsigned parent_flags) const
{
    unsigned flags = 0;
    ...
    if (&root_frame == &target_frame
        || target_frame.GetSecurityContext()->GetSecurityOrigin()->CanAccess(root_frame.GetSecurityContext()->GetSecurityOrigin())) {
    ...
    if (intersection_observation_state_ != kNotNeeded) {
        flags |= (IntersectionObservation::kExplicitRootObserversNeedUpdate | IntersectionObservation::kImplicitRootObserversNeedUpdate);
    ...
    flags |= (parent_flags & IntersectionObservation::kImplicitRootObserversNeedUpdate);
...
bool LocalFrameView::ShouldThrottleRendering() const
{
    if (LocalFrameTreeForcesThrottling())
        return true;
    bool throttled_for_global_reasons = LocalFrameTreeAllowsThrottling() && CanThrottleRendering() && frame_->GetDocument();
    ...
    if (local_frame_root_view->IsUpdatingLifecycle() && intersection_observation_state_ == kRequired && !IsDisplayLocked()) {
...
{
    if (lifecycle_updates_throttled_ || IsSubtreeThrottled() || IsDisplayLocked() || throttled_for_view_transition_) {
        return true;
    }
...
void LocalFrameView::UpdateRenderThrottlingStatus(bool hidden_for_throttling, bool subtree_throttled, bool display_locked, bool recurse)
{
    bool was_throttled = CanThrottleRendering();
    FrameView::UpdateRenderThrottlingStatus(hidden_for_throttling, subtree_throttled, display_locked, recurse);
...
void LocalFrameView::SetThrottledForViewTransition(bool throttled)
{
    ...
    // local iframes.
    if (RuntimeEnabledFeatures::PaintHoldingForLocalIframesEnabled() && was_throttled != CanThrottleRendering()) {
        RenderThrottlingStatusChanged();
    }
}
...
void LocalFrameView::BeginLifecycleUpdates()
{
    ...
    if (layout_view_is_empty && !DidFirstLayout() && !NeedsLayout()) {
        ...
        layout_view->SetNeedsLayout(layout_invalidation_reason::kAddedToLayout, kMarkOnlyThis);
    }
    ...
    if (WillDoPaintHoldingForFCP()) {
        ...
        chrome_client.StartDeferringCommits(GetFrame(), base::Milliseconds(kCommitDelayDefaultInMs), cc::PaintHoldingReason::kFirstContentfulPaint);
    }
...
{
    Document* document = GetFrame().GetDocument();
    return document && document->DeferredCompositorCommitIsAllowed() && !have_deferred_main_frame_commits_ && GetFrame().IsOutermostMainFrame();
}
...
String LocalFrameView::MainThreadScrollingReasonsAsText()
{
    ...
    if (properties && properties->Scroll()) {
        const auto* compositor = GetFrame().LocalFrameRoot().View()->paint_artifact_compositor_.Get();
        CHECK(compositor);
...
bool LocalFrameView::MapToVisualRectInRemoteRootFrame(PhysicalRect& rect, bool apply_overflow_clip)
{
    DCHECK(frame_->IsLocalRoot());
    // This is the top-level frame, so no mapping necessary.
    if (frame_->IsOutermostMainFrame())
        return true;
    bool result = rect.InclusiveIntersect(PhysicalRect(apply_overflow_clip ? frame_->RemoteViewportIntersection() : frame_->RemoteMainFrameIntersection()));
    if (result) {
        if (LayoutView* layout_view = GetLayoutView()) {
            rect = layout_view->LocalToAncestorRect(rect, nullptr, kTraverseDocumentBoundaries | kApplyRemoteMainFrameTransform);
        }
    }
    return result;
}

void LocalFrameView::MapLocalToRemoteMainFrame(TransformState& transform_state)
{
    DCHECK(frame_->IsLocalRoot());
    // This is the top-level frame, so no mapping necessary.
    if (frame_->IsOutermostMainFrame())
        return;
    transform_state.ApplyTransform(GetFrame().RemoteMainFrameTransform(), TransformState::kAccumulateTransform);
}

LayoutUnit LocalFrameView::CaretWidth() const
{
    return LayoutUnit(std::max<float>(1.0f, GetChromeClient()->WindowToViewportScalar(&GetFrame(), 1.0f)));
}
...
void LocalFrameView::OnFirstContentfulPaint()
{
    if (frame_->IsMainFrame()) {
        ...
        GetPage()->GetChromeClient().StopDeferringCommits(*frame_, cc::PaintHoldingCommitTrigger::kFirstContentfulPaint);
...
void LocalFrameView::RegisterForLifecycleNotifications(LifecycleNotificationObserver* observer)
{
    lifecycle_observers_.insert(observer);
}

void LocalFrameView::UnregisterFromLifecycleNotifications(LifecycleNotificationObserver* observer)
{
...
void LocalFrameView::NotifyVideoIsDominantVisibleStatus(HTMLVideoElement* element, bool is_dominant)
{
    if (is_dominant) {
...
#if DCHECK_IS_ON()
LocalFrameView::DisallowLayoutInvalidationScope::DisallowLayoutInvalidationScope(LocalFrameView* view)
...
LocalFrameView::DisallowLayoutInvalidationScope::~DisallowLayoutInvalidationScope()
{
...
#else
    bool should_enable = cc::frame_viewer_instrumentation::IsTracingLayerTreeSnapshots() || RuntimeEnabledFeatures::PaintUnderInvalidationCheckingEnabled()
        || WebTestSupport::IsRunningWebTest() || CoreProbeSink::HasAgentsGlobal(CoreProbeSink::kInspectorLayerTreeAgent);
...
OverlayInterstitialAdDetector& LocalFrameView::EnsureOverlayInterstitialAdDetector()
{
    if (!overlay_interstitial_ad_detector_) {
        overlay_interstitial_ad_detector_ = std::make_unique<OverlayInterstitialAdDetector>();
    }
...
static PaintLayer* GetXrOverlayLayer(Document& document)
{
    ...
    Element* fullscreen_element = Fullscreen::FullscreenElementFrom(*content_document);
    while (auto* frame_owner = DynamicTo<HTMLFrameOwnerElement>(fullscreen_element)) {
        ...
        [disable_expansion](LocalFrameView& frame_view) {
            ...
            if (auto* frame_layout_view = frame_view.GetLayoutView()) {
                if (auto* owner = frame_view.GetFrame().OwnerLayoutObject()) {
                    ...
                    DCHECK(frame_root_layer);
                    DCHECK(owner->Layer());
                    if (frame_root_layer->NeedsCullRectUpdate() || frame_root_layer->DescendantNeedsCullRectUpdate()) {
...
void LocalFrameView::RunPaintBenchmark(int repeat_count, cc::PaintBenchmarkResult& result)
{
    ...
    result.record_time_caching_disabled_ms = run_benchmark(PaintBenchmarkMode::kCachingDisabled);
    result.record_time_subsequence_caching_disabled_ms = run_benchmark(PaintBenchmarkMode::kSubsequenceCachingDisabled);
    result.raster_invalidation_and_convert_time_ms = run_benchmark(PaintBenchmarkMode::kForceRasterInvalidationAndConvert);
    result.paint_artifact_compositor_update_time_ms = run_benchmark(PaintBenchmarkMode::kForcePaintArtifactCompositorUpdate);

    result.painter_memory_usage = 0;
    if (paint_controller_persistent_data_) {
        result.painter_memory_usage += paint_controller_persistent_data_->ApproximateUnsharedMemoryUsage();
    }
    if (paint_artifact_compositor_) {
        result.painter_memory_usage += paint_artifact_compositor_->ApproximateUnsharedMemoryUsage();
    }
}
...
DarkModeFilter& LocalFrameView::EnsureDarkModeFilter()
{
    if (!dark_mode_filter_) {
        dark_mode_filter_ = std::make_unique<DarkModeFilter>(GetCurrentDarkModeSettings());
    }
...
void LocalFrameView::AddPendingTransformUpdate(LayoutObject& object)
{
    if (!pending_transform_updates_) {
        pending_transform_updates_ = MakeGarbageCollected<HeapHashSet<Member<LayoutObject>>>();
    }
...
bool LocalFrameView::RemovePendingTransformUpdate(const LayoutObject& object)
{
    if (!pending_transform_updates_)
        return false;
    auto it = pending_transform_updates_->find(const_cast<LayoutObject*>(&object));
...
void LocalFrameView::AddPendingOpacityUpdate(LayoutObject& object)
{
    if (!pending_opacity_updates_) {
        pending_opacity_updates_ = MakeGarbageCollected<HeapHashSet<Member<LayoutObject>>>();
    }
...
bool LocalFrameView::ExecuteAllPendingUpdates()
{
    ...
    ForAllNonThrottledLocalFrameViews([&updated](LocalFrameView& frame_view) {
        if (frame_view.pending_opacity_updates_ && !frame_view.pending_opacity_updates_->empty()) {
            for (LayoutObject* object : *frame_view.pending_opacity_updates_) {
                DCHECK(!DisplayLockUtilities::LockedAncestorPreventingPrePaint(*object));
                PaintPropertyTreeBuilder::DirectlyUpdateOpacityValue(*object);
            }
            updated = true;
            frame_view.pending_opacity_updates_->clear();
        }
        if (frame_view.pending_transform_updates_ && !frame_view.pending_transform_updates_->empty()) {
...
void LocalFrameView::AddPendingStickyUpdate(PaintLayerScrollableArea* object)
{
    if (!pending_sticky_updates_) {
        pending_sticky_updates_ = MakeGarbageCollected<HeapHashSet<Member<PaintLayerScrollableArea>>>();
    }
    pending_sticky_updates_->insert(object);
}

bool LocalFrameView::HasPendingStickyUpdate(PaintLayerScrollableArea* object) const
{
    if (pending_sticky_updates_) {
...
void LocalFrameView::AddPendingSnapUpdate(PaintLayerScrollableArea* object)
{
    if (!pending_snap_updates_) {
        pending_snap_updates_ = MakeGarbageCollected<HeapHashSet<Member<PaintLayerScrollableArea>>>();
    }
        ...
        for (PaintLayerScrollableArea* scrollable_area : *pending_snap_updates_) {
            ...
            if (SnapCoordinator::UpdateSnapContainerData(*snap_container)) {
                if (!pending_perform_snap_) {
                    pending_perform_snap_ = MakeGarbageCollected<HeapHashSet<Member<PaintLayerScrollableArea>>>();
                }
...
void LocalFrameView::NotifyElementWithRememberedSizeDisconnected(Element* element)
{
...
{
    return GetFrame().GetDocument()->GetStyleEngine().UpdateLastSuccessfulPositionFallbacks();
}
