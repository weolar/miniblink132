...
public:
    AnimationMockChromeClient()
        : has_scheduled_animation_(false)
    {
    }
    ...
    void ScheduleAnimation(const LocalFrameView*, base::TimeDelta = base::TimeDelta()) override
    {
...
TEST_F(LocalFrameViewTest, SetPaintInvalidationDuringUpdateAllLifecyclePhases)
{
    ...
    GetDocument().getElementById(AtomicString("a"))->setAttribute(html_names::kStyleAttr, AtomicString("color: green"));
...
TEST_F(LocalFrameViewTest, SetPaintInvalidationDuringUpdateLifecyclePhasesToPrePaintClean)
{
    SetBodyInnerHTML("<div id='a' style='color: blue'>A</div>");
    GetDocument().getElementById(AtomicString("a"))->setAttribute(html_names::kStyleAttr, AtomicString("color: green"));
    GetAnimationMockChromeClient().has_scheduled_animation_ = false;
...
TEST_F(LocalFrameViewTest, SetPaintInvalidationOutOfUpdateAllLifecyclePhases)
{
    SetBodyInnerHTML("<div id='a' style='color: blue'>A</div>");
    GetAnimationMockChromeClient().has_scheduled_animation_ = false;
    GetDocument().getElementById(AtomicString("a"))->GetLayoutObject()->SetShouldDoFullPaintInvalidation();
    EXPECT_TRUE(GetAnimationMockChromeClient().has_scheduled_animation_);
    GetAnimationMockChromeClient().has_scheduled_animation_ = false;
    UpdateAllLifecyclePhasesForTest();
    GetDocument().getElementById(AtomicString("a"))->GetLayoutObject()->SetShouldDoFullPaintInvalidation();
...
TEST_F(LocalFrameViewTest, HideTooltipWhenScrollPositionChanges)
{
    ...
    EXPECT_CALL(GetAnimationMockChromeClient(), MockUpdateTooltipUnderCursor(GetDocument().GetFrame(), String(), _));
    GetDocument().View()->LayoutViewport()->SetScrollOffset(ScrollOffset(1, 1), mojom::blink::ScrollType::kUser);
    ...
    EXPECT_CALL(GetAnimationMockChromeClient(), MockUpdateTooltipUnderCursor(GetDocument().GetFrame(), String(), _)).Times(0);
    GetDocument().View()->LayoutViewport()->SetScrollOffset(ScrollOffset(2, 2), mojom::blink::ScrollType::kProgrammatic);
}
...
TEST_F(LocalFrameViewTest, NoOverflowInIncrementVisuallyNonEmptyPixelCount)
{
    ...
    GetDocument().View()->IncrementVisuallyNonEmptyPixelCount(gfx::Size(65536, 65536));
...
TEST_F(LocalFrameViewTest, UpdateLifecyclePhasesForPrintingDetachedFrame)
{
    ...
    EXPECT_EQ(DocumentLifecycle::kPrePaintClean, GetDocument().Lifecycle().GetState());
    EXPECT_EQ(DocumentLifecycle::kPrePaintClean, ChildDocument().Lifecycle().GetState());
...
TEST_F(LocalFrameViewTest, PrintFrameUpdateAllLifecyclePhases)
{
    ...
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(DocumentLifecycle::kPaintClean, GetDocument().Lifecycle().GetState());
    EXPECT_EQ(DocumentLifecycle::kPaintClean, ChildDocument().Lifecycle().GetState());
}
...
TEST_F(LocalFrameViewTest, CanHaveScrollbarsIfScrollingAttrEqualsNoChanged)
{
    ...
    ChildDocument().WillChangeFrameOwnerProperties(
        0, 0, mojom::blink::ScrollbarMode::kAlwaysOn, false, mojom::blink::ColorScheme::kLight, mojom::blink::PreferredColorScheme::kLight);
    EXPECT_TRUE(ChildDocument().View()->CanHaveScrollbars());
}

TEST_F(LocalFrameViewTest, MainThreadScrollingForBackgroundFixedAttachmentWithCompositing)
{
    ...
    EXPECT_FALSE(frame_view->RequiresMainThreadScrollingForBackgroundAttachmentFixed());
    ...
    EXPECT_TRUE(frame_view->RequiresMainThreadScrollingForBackgroundAttachmentFixed());
...
TEST_F(LocalFrameViewTest, MainThreadScrollingForBackgroundFixedAttachmentWithoutCompositing)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      .fixed-background {
...
TEST_F(LocalFrameViewSimTest, FragmentNavChangesFocusWhileRenderingBlocked)
{
    ...
    //  navigation.
    auto* anchor = To<HTMLAnchorElement>(GetDocument().getElementById(AtomicString("anchorlink")));
    anchor->click();

    // Even though the navigation is synchronous, the active element shouldn't be
    // changed.
    EXPECT_EQ(GetDocument().body(), GetDocument().ActiveElement()) << "Active element changed while rendering is blocked";
    EXPECT_EQ(ScrollOffset(), viewport->GetScrollOffset()) << "Scroll offset changed while rendering is blocked";

    // Force a layout.
    anchor->style()->setProperty(GetDocument().GetExecutionContext(), "display", "block", String(), ASSERT_NO_EXCEPTION);
    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kTest);

    EXPECT_EQ(GetDocument().body(), GetDocument().ActiveElement()) << "Active element changed due to layout while rendering is blocked";
    EXPECT_EQ(ScrollOffset(), viewport->GetScrollOffset()) << "Scroll offset changed due to layout while rendering is blocked";
    ...
    EXPECT_EQ(GetDocument().getElementById(AtomicString("bottom")), GetDocument().ActiveElement()) << "Active element wasn't changed after load completed.";
    EXPECT_NE(ScrollOffset(), viewport->GetScrollOffset()) << "Scroll offset wasn't changed after load completed.";
}
...
TEST_F(LocalFrameViewSimTest, NestedCrossOriginPaintEligibility)
{
    ...
    auto* outer_frame_element = To<HTMLIFrameElement>(GetDocument().getElementById(AtomicString("outer")));
    ...
    auto* inner_frame_element = To<HTMLIFrameElement>(outer_frame_document->getElementById(AtomicString("inner")));
...
TEST_F(LocalFrameViewRemoteParentSimTest, ThrottledLocalRootAnimationUpdate)
{
    ...
    ClassicScript::CreateUnspecifiedScript(source)->RunScript(document->domWindow());
    Compositor().BeginFrame();
...
class TestLifecycleObserver : public GarbageCollected<TestLifecycleObserver>, public LocalFrameView::LifecycleNotificationObserver {
public:
    ...
    int will_start_lifecycle_count() const
    {
        return will_start_lifecycle_count_;
    }
    int did_finish_lifecycle_count() const
    {
        return did_finish_lifecycle_count_;
    }
...
TEST_F(LocalFrameViewTest, StartOfLifecycleTaskRunsOnFullLifecycle)
{
    ...
    frame_view->EnqueueStartOfLifecycleTask(WTF::BindOnce(&TestCallback::Increment, WTF::Unretained(&callback)));
...
TEST_F(LocalFrameViewTest, DarkModeDocumentBackground)
{
    ...
    GetDocument().documentElement()->SetInlineStyleProperty(CSSPropertyID::kBackgroundColor, "transparent");
    UpdateAllLifecyclePhasesForTest();
    frame_view->SetBaseBackgroundColor(Color(255, 0, 0));
    EXPECT_EQ(frame_view->DocumentBackgroundColor(), Color(18, 18, 18));
}

TEST_F(LocalFrameViewTest, AdjustMediaTypeForPrintingRestoresMediaTypeCorrectly)
{
    auto* frame_view = GetDocument().View();
...
class FencedFrameLocalFrameViewTest : private ScopedFencedFramesForTest, public SimTest {
public:
    FencedFrameLocalFrameViewTest()
...
class PrerenderLocalFrameViewTest : public base::test::WithFeatureOverride, public SimTest {
public:
    PrerenderLocalFrameViewTest()
        : base::test::WithFeatureOverride(features::kPrerender2EarlyDocumentLifecycleUpdate)
    {
    }
};
...
TEST_P(PrerenderLocalFrameViewTest, DryRunPaintBeforePrerenderActivation)
{
    ...
    PaintControllerPersistentData& pd = GetDocument().View()->GetPaintControllerPersistentDataForTesting();

    if (base::FeatureList::IsEnabled(features::kPrerender2EarlyDocumentLifecycleUpdate)) {
        EXPECT_EQ(DocumentLifecycle::kPaintClean, GetDocument().Lifecycle().GetState());
