...
{
    return os << "Context menu location: [" << data.mouse_position.x() << ", " << data.mouse_position.y() << "]";
}
...
{
    return layer->layer_tree_host()->property_trees()->effect_tree().Node(layer->effect_tree_index());
}

class VisualViewportTest : public testing::Test, public PaintTestConfigurations {
public:
    VisualViewportTest()
    ...
    void InitializeWithAndroidSettings(void (*override_settings_func)(WebSettings*) = nullptr)
    {
    ...
    {
        WebView()->MainFrameViewWidget()->UpdateLifecycle(WebLifecycleUpdate::kPrePaint, DocumentUpdateReason::kTest);
    }
    ...
    void RegisterMockedHttpURLLoad(const std::string& fileName)
    {
        ...
        // via |helper_|.
        url_test_helpers::RegisterMockedURLLoadFromBase(WebString::FromUTF8(base_url_), blink::test::CoreTestDataPath(), WebString::FromUTF8(fileName));
    }

    void RegisterMockedHttpURLLoad(const std::string& url, const std::string& fileName)
    {
        // TODO(crbug.com/751425): We should use the mock functionality
        // via |helper_|.
        url_test_helpers::RegisterMockedURLLoad(ToKURL(url), blink::test::CoreTestDataPath(WebString::FromUTF8(fileName)));
    }
    ...
    {
        return document->GetLayoutView()->GetScrollableArea()->GetScrollingBackgroundDisplayItemClient();
    }
...
TEST_P(VisualViewportTest, TestResize)
{
    ...
    WebView()->ResizeWithBrowserControls(gfx::Size(320, 240), gfx::Size(320, 240), WebView()->GetBrowserControls().Params());
    UpdateAllLifecyclePhases();
    ...
    WebView()->ResizeWithBrowserControls(web_view_size, web_view_size, WebView()->GetBrowserControls().Params());
    UpdateAllLifecyclePhases();
...
TEST_P(VisualViewportTest, TestVisibleContentRect)
{
    ...
    WebView()->ResizeWithBrowserControls(size, size, WebView()->GetBrowserControls().Params());
    UpdateAllLifecyclePhases();
    ...
    VisualViewport& visual_viewport = GetFrame()->GetPage()->GetVisualViewport();
    EXPECT_EQ(gfx::Rect(gfx::Point(0, 0), size - scrollbar_size), visual_viewport.VisibleContentRect(kExcludeScrollbars));
    EXPECT_EQ(gfx::Rect(gfx::Point(0, 0), size), visual_viewport.VisibleContentRect(kIncludeScrollbars));
    ...
    EXPECT_EQ(gfx::Rect(gfx::Point(10, 10), size - scrollbar_size), visual_viewport.VisibleContentRect(kExcludeScrollbars));
    EXPECT_EQ(gfx::Rect(gfx::Point(10, 10), size), visual_viewport.VisibleContentRect(kIncludeScrollbars));
}
...
{
    InitializeWithDesktopSettings();
    WebView()->ResizeWithBrowserControls(gfx::Size(800, 600), gfx::Size(800, 600), WebView()->GetBrowserControls().Params());
    UpdateAllLifecyclePhases();
    ...
    frame_view.LayoutViewport()->SetScrollOffset(ScrollOffset(10000, 10000), mojom::blink::ScrollType::kProgrammatic);
    visual_viewport.Move(gfx::Vector2dF(10000, 10000));

    // Sanity check.
    ASSERT_EQ(ScrollOffset(400, 300), visual_viewport.GetScrollOffset());
    ASSERT_EQ(ScrollOffset(200, 1400), frame_view.LayoutViewport()->GetScrollOffset());

    gfx::Point expected_location = frame_view.GetScrollableArea()->VisibleContentRect().origin();
    ...
    // location.
    WebView()->ResizeWithBrowserControls(gfx::Size(700, 550), gfx::Size(800, 600), WebView()->GetBrowserControls().Params());
    UpdateAllLifecyclePhases();

    EXPECT_EQ(expected_location, frame_view.GetScrollableArea()->VisibleContentRect().origin());
...
TEST_P(VisualViewportTest, TestResizeAfterVerticalScroll)
{
    ...
    EXPECT_EQ(ScrollOffset(0, 400), GetFrame()->View()->LayoutViewport()->GetScrollOffset());
    ...
    {
        UpdateAllLifecyclePhases();
        EXPECT_EQ(gfx::Transform::MakeScale(2), visual_viewport.GetPageScaleNode()->Matrix());
        EXPECT_EQ(gfx::Vector2dF(0, -300), visual_viewport.GetScrollTranslationNode()->Get2dTranslation());
        auto expected_projection = gfx::Transform::MakeScale(2);
        expected_projection.Translate(0, -300);
        EXPECT_EQ(expected_projection,
            GeometryMapper::SourceToDestinationProjection(*visual_viewport.GetScrollTranslationNode(), TransformPaintPropertyNode::Root()));
    }
    ...
    EXPECT_EQ(ScrollOffset(0, 625), GetFrame()->View()->LayoutViewport()->GetScrollOffset());
    EXPECT_VECTOR2DF_EQ(ScrollOffset(0, 75), visual_viewport.GetScrollOffset());

    // Verify the paint property nodes and GeometryMapper cache.
    {
        UpdateAllLifecyclePhases();
        EXPECT_EQ(gfx::Transform::MakeScale(4), visual_viewport.GetPageScaleNode()->Matrix());
        EXPECT_EQ(gfx::Vector2dF(0, -75), visual_viewport.GetScrollTranslationNode()->Get2dTranslation());
...
TEST_P(VisualViewportTest, TestResizeAfterHorizontalScroll)
{
    ...
    // Verify the paint property nodes and GeometryMapper cache.
    {
        UpdateAllLifecyclePhases();
        EXPECT_EQ(gfx::Transform::MakeScale(2), visual_viewport.GetPageScaleNode()->Matrix());
        EXPECT_EQ(gfx::Vector2dF(-150, 0), visual_viewport.GetScrollTranslationNode()->Get2dTranslation());
        auto expected_projection = gfx::Transform::MakeScale(2);
    ...
    EXPECT_EQ(ScrollOffset(0, 0), GetFrame()->View()->LayoutViewport()->GetScrollOffset());
    EXPECT_VECTOR2DF_EQ(ScrollOffset(150, 0), visual_viewport.GetScrollOffset());
...
TEST_P(VisualViewportTest, TestFractionalScrollOffsetIsNotOverwritten)
{
    ...
    LocalFrameView& frame_view = *WebView()->MainFrameImpl()->GetFrameView();
    frame_view.LayoutViewport()->SetScrollOffset(ScrollOffset(0, 10.5), mojom::blink::ScrollType::kProgrammatic);
    frame_view.LayoutViewport()->ScrollableArea::SetScrollOffset(ScrollOffset(10, 30.5), mojom::blink::ScrollType::kCompositor);
...
TEST_P(VisualViewportTest, TestOffsetClamping)
{
    ...
    EXPECT_POINTF_EQ(gfx::PointF(1120, 840), visual_viewport.VisibleRect().origin());
    ...
    EXPECT_POINTF_EQ(gfx::PointF(1024, 768), visual_viewport.VisibleRect().origin());
...
TEST_P(VisualViewportTest, TestOffsetClampingWithResizeAndScale)
{
    ...
    EXPECT_POINTF_EQ(gfx::PointF(160, 120), visual_viewport.VisibleRect().origin());
    ...
    visual_viewport.SetLocation(gfx::PointF(200, 200));
    EXPECT_POINTF_EQ(gfx::PointF(165, 125), visual_viewport.VisibleRect().origin());
    ...
    UpdateAllLifecyclePhases();
    EXPECT_EQ(WebView()->MainFrameViewWidget()->Size(), visual_viewport.Size());
    EXPECT_EQ(WebView()->MainFrameViewWidget()->Size(), GetFrame()->View()->FrameRect().size());
    visual_viewport.SetLocation(gfx::PointF(1000, 1000));
    EXPECT_POINTF_EQ(gfx::PointF(320, 240), visual_viewport.VisibleRect().origin());
    ...
    EXPECT_POINTF_EQ(gfx::PointF(200, 200), visual_viewport.VisibleRect().origin());
}
...
TEST_P(VisualViewportTest, TestFrameViewSizedToContent)
{
    ...
    // LayerImpl::bounds().
    EXPECT_EQ(gfx::Size(200, 267), WebView()->MainFrameImpl()->GetFrameView()->FrameRect().size());
}
...
TEST_P(VisualViewportTest, TestFrameViewSizedToMinimumScale)
{
    ...
    EXPECT_EQ(gfx::Size(100, 160), WebView()->MainFrameImpl()->GetFrameView()->FrameRect().size());
}
...
TEST_P(VisualViewportTest, TestAttachingNewFrameSetsInnerScrollLayerSize)
{
    ...
    EXPECT_EQ(gfx::Rect(0, 0, 320, 240), visual_viewport.GetScrollNode()->ContentsRect());
...
TEST_P(VisualViewportTest, TestFrameViewSizedToViewportMetaMinimumScale)
{
    ...
    EXPECT_EQ(gfx::Size(50, 80), WebView()->MainFrameImpl()->GetFrameView()->FrameRect().size());
}
...
TEST_P(VisualViewportTest, TestVisualViewportGetsSizeInAutoSizeMode)
{
    ...
    EXPECT_EQ(gfx::Size(200, 300), GetFrame()->GetPage()->GetVisualViewport().Size());
}
...
TEST_P(VisualViewportTest, TestTextSelectionHandles)
{
    ...
    WebView()->MainFrameViewWidget()->CalculateSelectionBounds(original_anchor, original_focus);
    ...
    expected -= gfx::ToFlooredVector2d(visual_viewport.VisibleRect().OffsetFromOrigin());
...
TEST_P(VisualViewportTest, TestSavedToHistoryItem)
{
    ...
    EXPECT_FALSE(To<LocalFrame>(WebView()->GetPage()->MainFrame())->Loader().GetDocumentLoader()->GetHistoryItem()->GetViewState().has_value());

    VisualViewport& visual_viewport = GetFrame()->GetPage()->GetVisualViewport();
    visual_viewport.SetScale(2);

    EXPECT_EQ(2, To<LocalFrame>(WebView()->GetPage()->MainFrame())->Loader().GetDocumentLoader()->GetHistoryItem()->GetViewState()->page_scale_factor_);

    visual_viewport.SetLocation(gfx::PointF(10, 20));

    EXPECT_EQ(ScrollOffset(10, 20),
        To<LocalFrame>(WebView()->GetPage()->MainFrame())->Loader().GetDocumentLoader()->GetHistoryItem()->GetViewState()->visual_viewport_scroll_offset_);
}
...
TEST_P(VisualViewportTest, TestRestoredFromHistoryItem)
{
    ...
    frame_test_helpers::LoadHistoryItem(WebView()->MainFrameImpl(), item, mojom::FetchCacheMode::kDefault);
    UpdateAllLifecyclePhases();
    VisualViewport& visual_viewport = GetFrame()->GetPage()->GetVisualViewport();
    EXPECT_EQ(2, visual_viewport.Scale());

    EXPECT_POINTF_EQ(gfx::PointF(100, 120), visual_viewport.VisibleRect().origin());
}
...
TEST_P(VisualViewportTest, TestRestoredFromLegacyHistoryItem)
{
    ...
    HistoryItem* item = MakeGarbageCollected<HistoryItem>();
    item->SetURL(url_test_helpers::ToKURL(base_url_ + "200-by-300-viewport.html"));
    ...
    VisualViewport& visual_viewport = GetFrame()->GetPage()->GetVisualViewport();
    EXPECT_EQ(2, visual_viewport.Scale());
    EXPECT_EQ(ScrollOffset(100, 150), GetFrame()->View()->LayoutViewport()->GetScrollOffset());
...
TEST_P(VisualViewportTest, TestNavigateToSmallerFrameViewHistoryItemClobberBug)
{
    InitializeWithAndroidSettings();
    ...
    LocalFrameView* frame_view = WebView()->MainFrameImpl()->GetFrameView();
    frame_view->LayoutViewport()->SetScrollOffset(ScrollOffset(0, 1000), mojom::blink::ScrollType::kProgrammatic);
    ...
    Persistent<HistoryItem> firstItem = WebView()->MainFrameImpl()->GetFrame()->Loader().GetDocumentLoader()->GetHistoryItem();
    ...
    EXPECT_NE(firstItem, WebView()->MainFrameImpl()->GetFrame()->Loader().GetDocumentLoader()->GetHistoryItem());
...
TEST_P(VisualViewportTest, DISABLED_TestWebFrameRangeAccountsForVisualViewportScroll)
{
    InitializeWithDesktopSettings();
    ...
    WebView()->MainFrameViewWidget()->CalculateSelectionBounds(base_rect, extent_rect);
...
class VisualViewportMockWebFrameClient : public frame_test_helpers::TestWebFrameClient {
public:
    MOCK_METHOD2(UpdateContextMenuDataForTesting, void(const ContextMenuData&, const std::optional<gfx::Point>&));
    MOCK_METHOD0(DidChangeScrollOffset, void());
};

MATCHER_P2(ContextMenuAtLocation, x, y, std::string(negation ? "is" : "isn't") + " at expected location [" + PrintToString(x) + ", " + PrintToString(y) + "]")
{
...
TEST_P(VisualViewportTest, TestContextMenuShownInCorrectLocation)
{
    ...
    WebMouseEvent mouse_down_event(WebInputEvent::Type::kMouseDown, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests());
    ...
    EXPECT_CALL(mock_web_frame_client,
        UpdateContextMenuDataForTesting(ContextMenuAtLocation(mouse_down_event.PositionInWidget().x(), mouse_down_event.PositionInWidget().y()), _));

    // Do a sanity check with no scale applied.
    WebView()->MainFrameImpl()->SetClient(&mock_web_frame_client);
    WebView()->MainFrameViewWidget()->HandleInputEvent(WebCoalescedInputEvent(mouse_down_event, ui::LatencyInfo()));
    WebView()->MainFrameViewWidget()->HandleInputEvent(WebCoalescedInputEvent(mouse_up_event, ui::LatencyInfo()));
...
TEST_P(VisualViewportTest, ScrollIntoViewFractionalOffset)
{
    ...
    VisualViewport& visual_viewport = GetFrame()->GetPage()->GetVisualViewport();
    Element* inputBox = GetFrame()->GetDocument()->getElementById(AtomicString("box"));
    ...
    layout_viewport_scrollable_area->SetScrollOffset(ScrollOffset(0, 900.75), mojom::blink::ScrollType::kProgrammatic);
    inputBox->scrollIntoViewIfNeeded(false);

    if (RuntimeEnabledFeatures::FractionalScrollOffsetsEnabled()) {
        EXPECT_EQ(ScrollOffset(0, 900.75), layout_viewport_scrollable_area->GetScrollOffset());
    } else {
        EXPECT_EQ(ScrollOffset(0, 900), layout_viewport_scrollable_area->GetScrollOffset());
    }
    ...
    layout_viewport_scrollable_area->SetScrollOffset(ScrollOffset(0, 900.125), mojom::blink::ScrollType::kProgrammatic);
    inputBox->scrollIntoViewIfNeeded(false);

    if (RuntimeEnabledFeatures::FractionalScrollOffsetsEnabled()) {
        EXPECT_EQ(ScrollOffset(0, 900.125), layout_viewport_scrollable_area->GetScrollOffset());
    } else {
        EXPECT_EQ(ScrollOffset(0, 900), layout_viewport_scrollable_area->GetScrollOffset());
    }
    EXPECT_EQ(ScrollOffset(250.25f, 100.25f), visual_viewport.GetScrollOffset());
    ...
    EXPECT_EQ(ScrollOffset(250.875f, 100.875f), visual_viewport.GetScrollOffset());
    ...
    layout_viewport_scrollable_area->SetScrollOffset(ScrollOffset(0, 900.5), mojom::blink::ScrollType::kProgrammatic);
    inputBox->scrollIntoViewIfNeeded(false);

    if (RuntimeEnabledFeatures::FractionalScrollOffsetsEnabled()) {
        EXPECT_EQ(ScrollOffset(0, 900.5), layout_viewport_scrollable_area->GetScrollOffset());
    } else {
        EXPECT_EQ(ScrollOffset(0, 900), layout_viewport_scrollable_area->GetScrollOffset());
    }
...
static ScrollOffset expectedMaxLayoutViewportScrollOffset(VisualViewport& visual_viewport, LocalFrameView& frame_view)
{
    float aspect_ratio = visual_viewport.VisibleRect().width() / visual_viewport.VisibleRect().height();
    float new_height = frame_view.FrameRect().width() / aspect_ratio;
    gfx::Size contents_size = frame_view.LayoutViewport()->ContentsSize();
    return ScrollOffset(contents_size.width() - frame_view.FrameRect().width(), contents_size.height() - new_height);
}
...
TEST_P(VisualViewportTest, TestBrowserControlsAdjustment)
{
    ...
    WebView()->MainFrameViewWidget()->ApplyViewportChangesForTesting({ gfx::Vector2dF(), gfx::Vector2dF(), 1, false, 1, 0, cc::BrowserControlsState::kBoth });
    ...
    frame_view.LayoutViewport()->ScrollBy(ScrollOffset(10000, 10000), mojom::blink::ScrollType::kUser);
    EXPECT_EQ(expectedMaxLayoutViewportScrollOffset(visual_viewport, frame_view), frame_view.LayoutViewport()->GetScrollOffset());

    // Simulate bringing up the browser controls by 10.5px.
    WebView()->MainFrameViewWidget()->ApplyViewportChangesForTesting(
        { gfx::Vector2dF(), gfx::Vector2dF(), 1, false, -10.5f / 20, 0, cc::BrowserControlsState::kBoth });
    EXPECT_SIZEF_EQ(gfx::SizeF(500, 440.5f), visual_viewport.VisibleRect().size());

    // maximumScrollPosition |ceil|s the browser controls adjustment.
    visual_viewport.Move(ScrollOffset(10000, 10000));
    EXPECT_VECTOR2DF_EQ(ScrollOffset(500, 881 - 441), visual_viewport.GetScrollOffset());
...
TEST_P(VisualViewportTest, TestBrowserControlsAdjustmentWithScale)
{
    ...
    // The outer viewport (LocalFrameView) should be affected as well.
    frame_view.LayoutViewport()->ScrollBy(ScrollOffset(10000, 10000), mojom::blink::ScrollType::kUser);
    ScrollOffset expected = expectedMaxLayoutViewportScrollOffset(visual_viewport, frame_view);
    ...
    WebView()->MainFrameViewWidget()->ApplyViewportChangesForTesting(
        { gfx::Vector2dF(), gfx::Vector2dF(), 0.5f, false, 0, 0, cc::BrowserControlsState::kBoth });
    EXPECT_EQ(1, visual_viewport.Scale());
    EXPECT_EQ(expected, frame_view.LayoutViewport()->GetScrollOffset());
    frame_view.LayoutViewport()->ScrollBy(ScrollOffset(10000, 10000), mojom::blink::ScrollType::kUser);
    EXPECT_EQ(expected, frame_view.LayoutViewport()->GetScrollOffset());
    ...
    WebView()->MainFrameViewWidget()->ApplyViewportChangesForTesting(
        { gfx::Vector2dF(), gfx::Vector2dF(), 0.8f, false, -1, 0, cc::BrowserControlsState::kBoth });
    EXPECT_EQ(gfx::SizeF(625, 562.5), visual_viewport.VisibleRect().size());

    // Bring out the browser controls by 11
    WebView()->MainFrameViewWidget()->ApplyViewportChangesForTesting(
        { gfx::Vector2dF(), gfx::Vector2dF(), 1, false, 11 / 20.f, 0, cc::BrowserControlsState::kBoth });
    EXPECT_EQ(gfx::SizeF(625, 548.75), visual_viewport.VisibleRect().size());

    // Ensure max scroll offsets are updated properly.
    visual_viewport.Move(ScrollOffset(10000, 10000));
    EXPECT_VECTOR2DF_EQ(ScrollOffset(375, 877.5 - 548.75), visual_viewport.GetScrollOffset());
...
TEST_P(VisualViewportTest, TestBrowserControlsAdjustmentAndResize)
{
    ...
    WebView()->ResizeWithBrowserControls(
        gfx::Size(500, visual_viewport_height - browser_controls_height), gfx::Size(500, visual_viewport_height - browser_controls_height), controls);
    UpdateAllLifecyclePhases();
    ...
    EXPECT_EQ(gfx::SizeF(250, (visual_viewport_height - browser_controls_height) / page_scale), visual_viewport.VisibleRect().size());
    EXPECT_EQ(gfx::Size(1000, layout_viewport_height - browser_controls_height / min_page_scale), frame_view.FrameRect().size());
    EXPECT_EQ(gfx::Size(500, visual_viewport_height - browser_controls_height), visual_viewport.Size());
    ...
    EXPECT_EQ(gfx::SizeF(250, visual_viewport_height / page_scale), visual_viewport.VisibleRect().size());

    ScrollOffset frame_view_expected = expectedMaxLayoutViewportScrollOffset(visual_viewport, frame_view);
    ScrollOffset visual_viewport_expected = ScrollOffset(750, layout_viewport_height - visual_viewport_height / page_scale);

    EXPECT_EQ(visual_viewport_expected, visual_viewport.GetScrollOffset());
    EXPECT_EQ(frame_view_expected, frame_view.LayoutViewport()->GetScrollOffset());
    ...
    WebView()->ResizeWithBrowserControls(gfx::Size(500, visual_viewport_height), gfx::Size(500, visual_viewport_height), controls);
    UpdateAllLifecyclePhases();

    EXPECT_EQ(gfx::Size(500, visual_viewport_height), visual_viewport.Size());
    EXPECT_EQ(gfx::SizeF(250, visual_viewport_height / page_scale), visual_viewport.VisibleRect().size());
    EXPECT_EQ(gfx::Size(1000, layout_viewport_height), frame_view.FrameRect().size());

    EXPECT_EQ(total_expected, visual_viewport.GetScrollOffset() + frame_view.LayoutViewport()->GetScrollOffset());
...
TEST_P(VisualViewportTest, TestBrowserControlsShrinkAdjustmentAndResize)
{
    ...
    WebView()->ResizeWithBrowserControls(gfx::Size(500, visual_viewport_height), 20, 0, false);
    UpdateAllLifecyclePhases();
    WebView()->GetBrowserControls().SetShownRatio(0, 0);
    ...
    ScrollOffset frame_view_expected(0, content_height - (layout_viewport_height - browser_controls_height / min_page_scale));
    ScrollOffset visual_viewport_expected
        = ScrollOffset(750, (layout_viewport_height - browser_controls_height / min_page_scale - visual_viewport.VisibleRect().height()));
    ...
    WebView()->ResizeWithBrowserControls(gfx::Size(500, visual_viewport_height - browser_controls_height), 20, 0, true);
    UpdateAllLifecyclePhases();
...
{
    InitializeWithAndroidSettings();
    WebView()->ResizeWithBrowserControls(gfx::Size(WebView()->MainFrameViewWidget()->Size()), 500, 0, false);
    UpdateAllLifecyclePhases();
    WebView()->MainFrameViewWidget()->ApplyViewportChangesForTesting({ gfx::Vector2dF(), gfx::Vector2dF(), 1, false, 1, 0, cc::BrowserControlsState::kBoth });
    ...
    WebView()->MainFrameViewWidget()->ApplyViewportChangesForTesting({ gfx::Vector2dF(), gfx::Vector2dF(), 1, false, -1, 0, cc::BrowserControlsState::kBoth });
    LocalFrameView& frame_view = *WebView()->MainFrameImpl()->GetFrameView();
    frame_view.LayoutViewport()->SetScrollOffset(ScrollOffset(0, 10000), mojom::blink::ScrollType::kProgrammatic);
...
TEST_P(VisualViewportTest, TestScrollbarsNotAttachedWhenHideScrollbarsSettingIsTrue)
{
...
TEST_P(VisualViewportTest, TestScrollbarsAttachedWhenHideScrollbarsSettingIsFalse)
{
    InitializeWithAndroidSettings();
    WebView()->MainFrameViewWidget()->Resize(gfx::Size(100, 150));
    UpdateAllLifecyclePhases();
...
TEST_P(VisualViewportTest, TestChangingContentSizeAffectsScrollBounds)
{
    ...
    WebView()->MainFrameImpl()->ExecuteScript(WebScriptSource("var content = document.getElementById(\"content\");"
                                                              "content.style.width = \"1500px\";"
                                                              "content.style.height = \"2400px\";"));
    UpdateAllLifecyclePhases();

    const auto* scroll_node = frame_view.GetLayoutView()->FirstFragment().PaintProperties()->Scroll();
    float scale = GetFrame()->GetPage()->GetVisualViewport().Scale();
    EXPECT_EQ(gfx::Size(100 / scale, 150 / scale), scroll_node->ContainerRect().size());
...
TEST_P(VisualViewportTest, ElementBoundsInWidgetSpaceAccountsForViewport)
{
    ...
    gfx::Rect bounds = input_element->GetLayoutObject()->AbsoluteBoundingBoxRect();
    ...
    EXPECT_EQ(gfx::ToRoundedPoint(gfx::PointF(expected_bounds.origin()) - expected_scroll_delta), bounds_in_viewport.origin());
...
TEST_P(VisualViewportTest, visualViewportIsInert)
{
    ...
    frame_test_helpers::LoadHTMLString(web_view_impl->MainFrameImpl(),
        ...
        base_url);
    UpdateAllLifecyclePhases();
    LocalDOMWindow* window = web_view_impl->MainFrameImpl()->GetFrame()->DomWindow();
    ...
    VisualViewport& visual_viewport = web_view_impl->MainFrameImpl()->GetFrame()->GetPage()->GetVisualViewport();
    visual_viewport.SetScale(2);
    ...
    visual_viewport.SetScrollOffset(
        ScrollOffset(10, 15), mojom::blink::ScrollType::kProgrammatic, mojom::blink::ScrollBehavior::kInstant, ScrollableArea::ScrollCallback());
...
TEST_P(VisualViewportTest, FractionalMaxScrollOffset)
{
    ...
    WebView()->SetPageScaleFactor(2);
    EXPECT_EQ(ScrollOffset(101. / 2., 201. / 2.), scrollable_area->MaximumScrollOffset());
}
...
TEST_P(VisualViewportTest, MaxScrollOffsetAtScale)
{
    ...
    EXPECT_EQ(ScrollOffset(101. / 2., 201. / 2.), visual_viewport.MaximumScrollOffsetAtScale(2.0));
}
...
TEST_P(VisualViewportTest, AccessibilityHitTestWhileZoomedIn)
{
    ...
    frame_view.LayoutViewport()->SetScrollOffset(ScrollOffset(400, 1100), mojom::blink::ScrollType::kProgrammatic);
    ...
    WebAXObject hitNode = WebAXObject::FromWebDocument(web_doc).HitTest(gfx::Point(154, 165));
    ax::mojom::NameFrom name_from;
    WebVector<WebAXObject> name_objects;
    EXPECT_EQ(std::string("Target4"), hitNode.GetName(name_from, name_objects).Utf8());
}
...
TEST_P(VisualViewportTest, TestCoordinateTransforms)
{
    ...
    visual_viewport.SetScale(1);
    EXPECT_POINTF_EQ(gfx::PointF(314, 273), visual_viewport.ViewportToRootFrame(gfx::PointF(314, 273)));
    EXPECT_POINTF_EQ(gfx::PointF(314, 273), visual_viewport.RootFrameToViewport(gfx::PointF(314, 273)));

    // At scale = 2.
    visual_viewport.SetScale(2);
    EXPECT_POINTF_EQ(gfx::PointF(55, 75), visual_viewport.ViewportToRootFrame(gfx::PointF(110, 150)));
    EXPECT_POINTF_EQ(gfx::PointF(110, 150), visual_viewport.RootFrameToViewport(gfx::PointF(55, 75)));

    // At scale = 2 and with the visual viewport offset.
    visual_viewport.SetLocation(gfx::PointF(10, 12));
    EXPECT_POINTF_EQ(gfx::PointF(50, 62), visual_viewport.ViewportToRootFrame(gfx::PointF(80, 100)));
    EXPECT_POINTF_EQ(gfx::PointF(80, 100), visual_viewport.RootFrameToViewport(gfx::PointF(50, 62)));

    // Test points that will cause non-integer values.
    EXPECT_POINTF_EQ(gfx::PointF(50.5, 62.4), visual_viewport.ViewportToRootFrame(gfx::PointF(81, 100.8)));
    EXPECT_POINTF_EQ(gfx::PointF(81, 100.8), visual_viewport.RootFrameToViewport(gfx::PointF(50.5, 62.4)));

    // Scrolling the main frame should have no effect.
    frame_view.LayoutViewport()->SetScrollOffset(ScrollOffset(100, 120), mojom::blink::ScrollType::kProgrammatic);
    EXPECT_POINTF_EQ(gfx::PointF(50, 62), visual_viewport.ViewportToRootFrame(gfx::PointF(80, 100)));
    EXPECT_POINTF_EQ(gfx::PointF(80, 100), visual_viewport.RootFrameToViewport(gfx::PointF(50, 62)));
}
...
TEST_P(VisualViewportTest, WindowDimensionsOnLoad)
{
    ...
    Element* output = GetFrame()->GetDocument()->getElementById(AtomicString("output"));
    DCHECK(output);
...
TEST_P(VisualViewportTest, ResizeWithScrollAnchoring)
{
    ...
    LocalFrameView& frame_view = *WebView()->MainFrameImpl()->GetFrameView();
    frame_view.LayoutViewport()->SetScrollOffset(ScrollOffset(700, 500), mojom::blink::ScrollType::kProgrammatic);

    WebView()->MainFrameViewWidget()->Resize(gfx::Size(800, 300));
    UpdateAllLifecyclePhases();
    EXPECT_EQ(ScrollOffset(700, 200), frame_view.LayoutViewport()->GetScrollOffset());
}
...
TEST_P(VisualViewportTest, ResizeCompositedAndFixedBackground)
{
    ...
    web_view_impl->ResizeWithBrowserControls(gfx::Size(page_width, page_height), browser_controls_height, 0, false);
    UpdateAllLifecyclePhases();
    ...
    VisualViewport& visual_viewport = web_view_impl->GetPage()->GetVisualViewport();
    ...
    web_view_impl->ResizeWithBrowserControls(gfx::Size(page_width, smallest_height), browser_controls_height, 0, true);
    UpdateAllLifecyclePhases();
    ...
    web_view_impl->ResizeWithBrowserControls(gfx::Size(page_width, page_height), browser_controls_height, 0, true);
    UpdateAllLifecyclePhases();
...
{
    WebViewImpl* web_view_impl = helper_.InitializeWithSettings(&ConfigureViewportNonCompositing);
    ...
    {
        const auto& raster_invalidations = GetRasterInvalidationTracking(*GetFrame()->View())->Invalidations();
        EXPECT_THAT(raster_invalidations,
            UnorderedElementsAre(RasterInvalidationInfo { ScrollingBackgroundClient(document).Id(), ScrollingBackgroundClient(document).DebugName(),
...
TEST_P(VisualViewportTest, ResizeNonFixedBackgroundNoLayoutOrInvalidation)
{
    ...
    document->View()->CountObjectsNeedingLayout(needs_layout_objects, total_objects, is_subtree);
    ...
    EXPECT_FALSE(GetRasterInvalidationTracking(*GetFrame()->View())->HasInvalidations());
...
TEST_P(VisualViewportTest, InvalidateLayoutViewWhenDocumentSmallerThanView)
{
    ...
    WebURL base_url = url_test_helpers::ToKURL("http://example.com/");
    frame_test_helpers::LoadHTMLString(web_view_impl->MainFrameImpl(), "<div style='height: 20px'>Text</div>", base_url);
    UpdateAllLifecyclePhases();
    Document* document = To<LocalFrame>(web_view_impl->GetPage()->MainFrame())->GetDocument();

    // Do a resize to check for invalidations.
    document->View()->SetTracksRasterInvalidations(true);
    web_view_impl->ResizeWithBrowserControls(gfx::Size(page_width, largest_height), browser_controls_height, 0, false);
    UpdateAllLifecyclePhases();
    ...
    // unpainted area of background.
    {
        ...
        EXPECT_THAT(raster_invalidations,
            UnorderedElementsAre(RasterInvalidationInfo { ScrollingBackgroundClient(document).Id(), ScrollingBackgroundClient(document).DebugName(),
                gfx::Rect(0, 590, 320, 50), PaintInvalidationReason::kIncremental }));
    }
...
TEST_P(VisualViewportTest, EnsureOverscrollElasticityTransformNode)
{
    ...
    VisualViewport& visual_viewport = GetFrame()->GetPage()->GetVisualViewport();
    EXPECT_EQ(visual_viewport.GetOverscrollType() == OverscrollType::kTransform, !!visual_viewport.GetOverscrollElasticityTransformNode());
...
TEST_P(VisualViewportTest, EnsureEffectNodeForScrollbars)
{
    ...
    int scrollbar_thickness = theme.ScrollbarThickness(visual_viewport.ScaleFromDIP(), EScrollbarWidth::kAuto);

    EXPECT_EQ(vertical_scrollbar->effect_tree_index(),
        vertical_scrollbar->layer_tree_host()
            ->property_trees()
            ->effect_tree()
            .FindNodeFromElementId((visual_viewport.GetScrollbarElementId(ScrollbarOrientation::kVerticalScrollbar)))
            ->id);
    EXPECT_EQ(vertical_scrollbar->offset_to_transform_parent(), gfx::Vector2dF(400 - scrollbar_thickness, 0));

    EXPECT_EQ(horizontal_scrollbar->effect_tree_index(),
        horizontal_scrollbar->layer_tree_host()
            ->property_trees()
            ->effect_tree()
            .FindNodeFromElementId(visual_viewport.GetScrollbarElementId(ScrollbarOrientation::kHorizontalScrollbar))
            ->id);
    EXPECT_EQ(horizontal_scrollbar->offset_to_transform_parent(), gfx::Vector2dF(0, 400 - scrollbar_thickness));

    EXPECT_EQ(GetEffectNode(vertical_scrollbar)->parent_id, GetEffectNode(horizontal_scrollbar)->parent_id);
}
...
TEST_P(VisualViewportTest, SwapMainFrame)
{
    ...
    WebLocalFrame* local_frame = helper_.CreateProvisional(*helper_.LocalMainFrame());
...
public:
    ...
    {
        SimTest::SetUp();
        frame_test_helpers::WebViewHelper::UpdateAndroidCompositingSettings(WebView().GetSettings());
        WebView().SetDefaultPageScaleLimits(0.25f, 5);
    }
};
...
TEST_F(VisualViewportSimTest, ScrollingContentsSmallerThanContainer)
{
    ...
    EXPECT_EQ(gfx::Rect(0, 0, 400, 600), visual_viewport.GetScrollNode()->ContainerRect());
    EXPECT_EQ(gfx::Rect(0, 0, 320, 480), visual_viewport.GetScrollNode()->ContentsRect());

    WebView().MainFrameViewWidget()->ApplyViewportChangesForTesting(
        { gfx::Vector2dF(1, 1), gfx::Vector2dF(), 2, false, 1, 0, cc::BrowserControlsState::kBoth });
    EXPECT_EQ(gfx::Size(320, 480), visual_viewport.LayerForScrolling()->bounds());
...
class VisualViewportScrollIntoViewTest : public VisualViewportSimTest, public ::testing::WithParamInterface<std::vector<base::test::FeatureRef>> {
public:
    VisualViewportScrollIntoViewTest()
    {
        feature_list_.InitWithFeatures(GetParam(),
...
INSTANTIATE_TEST_SUITE_P(, VisualViewportScrollIntoViewTest,
    testing::Values(std::vector<base::test::FeatureRef> {}, std::vector<base::test::FeatureRef> { features::kMultiSmoothScrollIntoView }));
...
TEST_P(VisualViewportScrollIntoViewTest, ScrollingToFixed)
{
    ...
    bool is_for_scroll_sequence = !RuntimeEnabledFeatures::MultiSmoothScrollIntoViewEnabled();
    auto scroll_params = scroll_into_view_util::CreateScrollIntoViewParams(ScrollAlignment::ToEdgeIfNeeded(), ScrollAlignment::ToEdgeIfNeeded(),
        mojom::blink::ScrollType::kProgrammatic,
        /*make_visible_in_visual_viewport=*/true, mojom::blink::ScrollBehavior::kInstant, is_for_scroll_sequence);
    if (is_for_scroll_sequence) {
        GetDocument().GetFrame()->CreateNewSmoothScrollSequence();
    }
    WebView().GetPage()->GetVisualViewport().ScrollIntoView(bottom_element->BoundingBox(), PhysicalBoxStrut(), scroll_params);
    if (is_for_scroll_sequence) {
...
TEST_P(VisualViewportTest, DeviceEmulation)
{
    ...
    EXPECT_FALSE(GetFrame()->View()->VisualViewportOrOverlayNeedsRepaintForTesting());
    ...
    UpdateAllLifecyclePhasesExceptPaint();
    EXPECT_TRUE(GetFrame()->View()->VisualViewportOrOverlayNeedsRepaintForTesting());
    ASSERT_TRUE(visual_viewport.GetDeviceEmulationTransformNode());
    EXPECT_EQ(
        gfx::Transform::MakeTranslation(-params.viewport_offset.x(), -params.viewport_offset.y()), visual_viewport.GetDeviceEmulationTransformNode()->Matrix());
    UpdateAllLifecyclePhases();
    EXPECT_FALSE(GetFrame()->View()->VisualViewportOrOverlayNeedsRepaintForTesting());
    ...
    EXPECT_FALSE(GetFrame()->View()->VisualViewportOrOverlayNeedsRepaintForTesting());
    ASSERT_TRUE(visual_viewport.GetDeviceEmulationTransformNode());
    EXPECT_EQ(gfx::Transform::MakeScale(1.5f), visual_viewport.GetDeviceEmulationTransformNode()->Matrix());
    UpdateAllLifecyclePhases();
    EXPECT_FALSE(GetFrame()->View()->VisualViewportOrOverlayNeedsRepaintForTesting());
...
TEST_P(VisualViewportTest, PaintScrollbar)
{
    ...
    auto check_scrollbar = [](const cc::Layer* scrollbar, float scale) {
        ...
        EXPECT_EQ(cc::HitTestOpaqueness::kTransparent, scrollbar->hit_test_opaqueness());
        EXPECT_TRUE(scrollbar->IsScrollbarLayerForTesting());
        EXPECT_EQ(cc::ScrollbarOrientation::kVertical, static_cast<const cc::ScrollbarLayerBase*>(scrollbar)->orientation());
        ...
        EXPECT_EQ(transform, scrollbar->layer_tree_host()->property_trees()->transform_tree().Node(scrollbar->transform_tree_index())->local);
    };

    // The last layer should be the vertical scrollbar.
    const cc::Layer* scrollbar = GetFrame()->View()->RootCcLayer()->children().back().get();
    ...
    WebView()->EnableDeviceEmulation(params);
    UpdateAllLifecyclePhases();
    ASSERT_EQ(scrollbar, GetFrame()->View()->RootCcLayer()->children().back().get());
...
TEST_P(VisualViewportTest, DirectPinchZoomPropertyUpdate)
{
    ...
    UpdateAllLifecyclePhases();
    EXPECT_EQ(paint_artifact_compositor()->NeedsUpdate(), PaintArtifactCompositor::UpdateType::kNone);
...
TEST_P(VisualViewportTest, InSubtreeOfPageScale)
{
    ...
    for (const auto* ancestor = page_scale->UnaliasedParent(); ancestor; ancestor = ancestor->UnaliasedParent()) {
        EXPECT_FALSE(ancestor->IsInSubtreeOfPageScale());
    }

    const auto* view = GetFrame()->View()->GetLayoutView();
    const auto& view_contents_transform = view->FirstFragment().ContentsProperties().Transform();
    ...
    for (const auto* ancestor = view_contents_transform.UnaliasedParent(); ancestor != page_scale; ancestor = ancestor->UnaliasedParent()) {
...
TEST_F(VisualViewportSimTest, UsedColorSchemeFromRootElement)
{
    ...
    const VisualViewport& visual_viewport = WebView().GetPage()->GetVisualViewport();

    EXPECT_EQ(mojom::blink::ColorScheme::kLight, visual_viewport.UsedColorSchemeScrollbars());
    ...
    EXPECT_EQ(mojom::blink::ColorScheme::kDark, visual_viewport.UsedColorSchemeScrollbars());
}
...
TEST_P(VisualViewportTest, SetLocationBeforePrePaint)
{
    ...
    EXPECT_EQ(gfx::PointF(12, 34), layer_tree_host->property_trees()->scroll_tree().current_scroll_offset(visual_viewport.GetScrollElementId()));
}
...
TEST_P(VisualViewportTest, ScrollbarGeometryOnSizeChange)
{
    ...
    ASSERT_TRUE(horizontal_scrollbar);
    ASSERT_TRUE(vertical_scrollbar);
    EXPECT_EQ(gfx::Vector2dF(0, 93), horizontal_scrollbar->offset_to_transform_parent());
    EXPECT_EQ(gfx::Vector2dF(93, 0), vertical_scrollbar->offset_to_transform_parent());
    ...
    ASSERT_EQ(horizontal_scrollbar, visual_viewport.LayerForHorizontalScrollbar());
    ASSERT_EQ(vertical_scrollbar, visual_viewport.LayerForVerticalScrollbar());
    EXPECT_EQ(gfx::Vector2dF(0, 113), horizontal_scrollbar->offset_to_transform_parent());
    EXPECT_EQ(gfx::Vector2dF(93, 0), vertical_scrollbar->offset_to_transform_parent());
    EXPECT_EQ(gfx::Size(93, 7), horizontal_scrollbar->bounds());
...
TEST_F(VisualViewportSimTest, PreferredOverlayScrollbarColorTheme)
{
    ...
    const VisualViewport& visual_viewport = WebView().GetPage()->GetVisualViewport();
    EXPECT_EQ(mojom::blink::ColorScheme::kDark, visual_viewport.GetOverlayScrollbarColorScheme());

    color_scheme_helper.SetPreferredColorScheme(mojom::blink::PreferredColorScheme::kLight);
    Compositor().BeginFrame();
    EXPECT_EQ(mojom::blink::ColorScheme::kLight, visual_viewport.GetOverlayScrollbarColorScheme());
}
