...
DeviceSingleWindowEventController::DeviceSingleWindowEventController(LocalDOMWindow& window)
...
DeviceSingleWindowEventController::~DeviceSingleWindowEventController() = default;
...
void DeviceSingleWindowEventController::DidAddEventListener(LocalDOMWindow* window, const AtomicString& event_type)
{
...
void DeviceSingleWindowEventController::DidRemoveEventListener(LocalDOMWindow* window, const AtomicString& event_type)
{
    if (event_type != EventTypeName() || window->HasEventListeners(EventTypeName()))
        return;
...
bool DeviceSingleWindowEventController::CheckPolicyFeatures(const Vector<mojom::blink::PermissionsPolicyFeature>& features) const
{
    LocalDOMWindow& window = GetWindow();
    return base::ranges::all_of(
        features, [&window](mojom::blink::PermissionsPolicyFeature feature) { return window.IsFeatureEnabled(feature, ReportOptions::kReportOnFailure); });
}
