...
ReportingObserver* ReportingObserver::Create(ExecutionContext* execution_context, V8ReportingObserverCallback* callback, ReportingObserverOptions* options)
{
    return MakeGarbageCollected<ReportingObserver>(execution_context, callback, options);
}

ReportingObserver::ReportingObserver(ExecutionContext* execution_context, V8ReportingObserverCallback* callback, ReportingObserverOptions* options)
...
void ReportingObserver::QueueReport(Report* report)
{
    ...
    if (report_queue_.size() == 1) {
        execution_context_->GetTaskRunner(TaskType::kMiscPlatformAPI)
            ->PostTask(FROM_HERE, WTF::BindOnce(&ReportingObserver::ReportToCallback, WrapWeakPersistent(this)));
    }
}

bool ReportingObserver::ObservedType(const String& type)
{
    return !options_->hasTypesNonNull() || options_->typesNonNull().empty() || options_->typesNonNull().Find(type) != kNotFound;
}
