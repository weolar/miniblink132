...
{
    WindowPopin* supplement = Supplement<LocalDOMWindow>::From<WindowPopin>(window);
    if (!supplement) {
...
Vector<V8PopinContextType> WindowPopin::popinContextTypesSupported(LocalDOMWindow& window)
{
...
Vector<V8PopinContextType> WindowPopin::popinContextTypesSupported()
{
    ...
    if (!frame->GetPage()->IsPartitionedPopin() && frame->GetSecurityContext()->GetSecurityOrigin()->Protocol() == "https") {
        out.push_back(V8PopinContextType(V8PopinContextType::Enum::kPartitioned));
    }
    return out;
}

std::optional<V8PopinContextType> WindowPopin::popinContextType(LocalDOMWindow& window)
{
