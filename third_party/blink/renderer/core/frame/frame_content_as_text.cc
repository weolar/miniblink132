...
void FrameContentAsText(wtf_size_t max_chars, LocalFrame* frame, StringBuilder& output)
{
    ...
    for (Frame* cur_child = frame_tree.FirstChild(); cur_child; cur_child = cur_child->Tree().NextSibling()) {
        ...
        if (!layout_view || !layout_view->Size().width || !layout_view->Size().height || (layout_view->PhysicalLocation().left + layout_view->Size().width <= 0)
            || (layout_view->PhysicalLocation().top + layout_view->Size().height <= 0)
            || (owner_layout_object && owner_layout_object->Style() && owner_layout_object->Style()->Visibility() != EVisibility::kVisible)) {
            continue;
        }
