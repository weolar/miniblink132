...
ExecutionContextCSPDelegate::ExecutionContextCSPDelegate(ExecutionContext& execution_context)
...
void ExecutionContextCSPDelegate::SetSandboxFlags(network::mojom::blink::WebSandboxFlags mask)
{
    ...
    WorkerOrWorkletGlobalScope* worklet_or_worker = DynamicTo<WorkerOrWorkletGlobalScope>(execution_context_.Get());
    ...
    network::mojom::blink::WebSandboxFlags flags = execution_context_->GetSandboxFlags();
...
void ExecutionContextCSPDelegate::AddInsecureRequestPolicy(mojom::blink::InsecureRequestPolicy policy)
{
    ...
    security_context.SetInsecureRequestPolicy(security_context.GetInsecureRequestPolicy() | policy);
    if (window && window->GetFrame()) {
        window->GetFrame()->GetLocalFrameHostRemote().EnforceInsecureRequestPolicy(security_context.GetInsecureRequestPolicy());
    }

    // Upgrade Insecure Requests: Update the set of insecure URLs to upgrade.
    if ((policy & mojom::blink::InsecureRequestPolicy::kUpgradeInsecureRequests) != mojom::blink::InsecureRequestPolicy::kLeaveInsecureRequestsAlone) {
        ...
        if (window && !Url().Host().empty()) {
            ...
            if (auto* frame = window->GetFrame()) {
                frame->GetLocalFrameHostRemote().EnforceInsecureNavigationsSet(
                    SecurityContext::SerializeInsecureNavigationSet(GetSecurityContext().InsecureNavigationsToUpgrade()));
            }
        }
    }
}

std::unique_ptr<SourceLocation> ExecutionContextCSPDelegate::GetSourceLocation()
{
...
void ExecutionContextCSPDelegate::DispatchViolationEvent(const SecurityPolicyViolationEventInit& violation_data, Element* element)
{
    execution_context_->GetTaskRunner(TaskType::kNetworking)
        ->PostTask(FROM_HERE,
            WTF::BindOnce(
                &ExecutionContextCSPDelegate::DispatchViolationEventInternal, WrapPersistent(this), WrapPersistent(&violation_data), WrapPersistent(element)));
}

void ExecutionContextCSPDelegate::PostViolationReport(const SecurityPolicyViolationEventInit& violation_data, const String& stringified_report,
    bool is_frame_ancestors_violation, const Vector<String>& report_endpoints, bool use_reporting_api)
{
    DCHECK_EQ(is_frame_ancestors_violation,
        network::mojom::blink::CSPDirectiveName::FrameAncestors == ContentSecurityPolicy::GetDirectiveType(violation_data.effectiveDirective()));
    ...
    scoped_refptr<EncodedFormData> report = EncodedFormData::Create(stringified_report.Utf8());
    ...
    String url_sending_report = is_frame_ancestors_violation ? violation_data.documentURI() : Url().GetString();
    Report* observed_report = MakeGarbageCollected<Report>(ReportType::kCSPViolation, url_sending_report, body);
    ReportingContext::From(execution_context_.Get())->QueueReport(observed_report, use_reporting_api ? report_endpoints : Vector<String>());

    if (use_reporting_api)
        return;

    for (const auto& report_endpoint : report_endpoints) {
        PingLoader::SendViolationReport(execution_context_.Get(), KURL(report_endpoint), report, is_frame_ancestors_violation);
    }
}
...
void ExecutionContextCSPDelegate::AddConsoleMessage(ConsoleMessage* console_message)
{
...
void ExecutionContextCSPDelegate::SetWasmEvalErrorMessage(const String& error_message)
{
    execution_context_->SetWasmEvalErrorMessage(error_message);
}

void ExecutionContextCSPDelegate::ReportBlockedScriptExecutionToInspector(const String& directive_text)
{
    probe::ScriptExecutionBlockedByCSP(execution_context_, directive_text);
}

void ExecutionContextCSPDelegate::DidAddContentSecurityPolicies(WTF::Vector<network::mojom::blink::ContentSecurityPolicyPtr> policies)
{
    auto* window = DynamicTo<LocalDOMWindow>(execution_context_.Get());
    if (!window)
        return;
...
void ExecutionContextCSPDelegate::DispatchViolationEventInternal(const SecurityPolicyViolationEventInit* violation_data, Element* element)
{
    ...
    SecurityPolicyViolationEvent& event = *SecurityPolicyViolationEvent::Create(event_type_names::kSecuritypolicyviolation, violation_data);
