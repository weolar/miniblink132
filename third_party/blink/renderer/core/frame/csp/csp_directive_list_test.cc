...
public:
    ...
    network::mojom::blink::ContentSecurityPolicyPtr CreateList(
        const String& list, ContentSecurityPolicyType type, ContentSecurityPolicySource source = ContentSecurityPolicySource::kHTTP)
    {
        Vector<network::mojom::blink::ContentSecurityPolicyPtr> parsed
            = ParseContentSecurityPolicies(list, type, source, KURL("https://example.test/index.html"));
...
TEST_F(CSPDirectiveListTest, Header)
{
    struct TestCase {
        const char* list;
        const char* expected;
    } cases[] = { { "script-src 'self'", "script-src 'self'" }, { "  script-src 'self'  ", "script-src 'self'" },
        { "\t\tscript-src 'self'", "script-src 'self'" }, { "script-src 'self' \t", "script-src 'self'" } };

    for (const auto& test : cases) {
        network::mojom::blink::ContentSecurityPolicyPtr directive_list = CreateList(test.list, ContentSecurityPolicyType::kReport);
...
TEST_F(CSPDirectiveListTest, IsMatchingNoncePresent)
{
    ...
    ContentSecurityPolicy* context = MakeGarbageCollected<ContentSecurityPolicy>();
    ...
    for (const auto& test : cases) {
        for (auto reporting_disposition : { ReportingDisposition::kSuppressReporting, ReportingDisposition::kReport }) {
            // Report-only
            network::mojom::blink::ContentSecurityPolicyPtr directive_list = CreateList(test.list, ContentSecurityPolicyType::kReport);

            EXPECT_TRUE(CSPDirectiveListAllowFromSource(*directive_list, context, CSPDirectiveName::ScriptSrcElem, blocked_url, blocked_url,
                ResourceRequest::RedirectStatus::kNoRedirect, reporting_disposition, test.nonce));

            // Enforce
            directive_list = CreateList(test.list, ContentSecurityPolicyType::kEnforce);
            EXPECT_EQ(CSPCheckResult(test.expected),
                CSPDirectiveListAllowFromSource(*directive_list, context, CSPDirectiveName::ScriptSrcElem, blocked_url, blocked_url,
                    ResourceRequest::RedirectStatus::kNoRedirect, reporting_disposition, test.nonce));
        }
    }
}
...
TEST_F(CSPDirectiveListTest, AllowScriptFromSourceNoNonce)
{
    ...
    for (const auto& test : cases) {
        ...
        // Report-only
        network::mojom::blink::ContentSecurityPolicyPtr directive_list = CreateList(test.list, ContentSecurityPolicyType::kReport);
        EXPECT_TRUE(CSPDirectiveListAllowFromSource(*directive_list, context, CSPDirectiveName::ScriptSrcElem, script_src, script_src,
            ResourceRequest::RedirectStatus::kNoRedirect, ReportingDisposition::kSuppressReporting, String(), IntegrityMetadataSet(), kParserInserted));

        // Enforce
        directive_list = CreateList(test.list, ContentSecurityPolicyType::kEnforce);
        EXPECT_EQ(CSPCheckResult(test.expected),
            CSPDirectiveListAllowFromSource(*directive_list, context, CSPDirectiveName::ScriptSrcElem, script_src, script_src,
                ResourceRequest::RedirectStatus::kNoRedirect, ReportingDisposition::kSuppressReporting, String(), IntegrityMetadataSet(), kParserInserted));
    }
}
...
TEST_F(CSPDirectiveListTest, AllowFromSourceWithNonce)
{
    ...
    for (const auto& test : cases) {
        ...
        network::mojom::blink::ContentSecurityPolicyPtr directive_list = CreateList(String("script-src ") + test.list, ContentSecurityPolicyType::kReport);
        EXPECT_TRUE(CSPDirectiveListAllowFromSource(*directive_list, context, CSPDirectiveName::ScriptSrcElem, resource, resource,
            ResourceRequest::RedirectStatus::kNoRedirect, ReportingDisposition::kSuppressReporting, String(test.nonce), IntegrityMetadataSet(),
            kParserInserted));

        // Enforce 'script-src'
        directive_list = CreateList(String("script-src ") + test.list, ContentSecurityPolicyType::kEnforce);
        EXPECT_EQ(CSPCheckResult(test.expected),
            CSPDirectiveListAllowFromSource(*directive_list, context, CSPDirectiveName::ScriptSrcElem, resource, resource,
                ResourceRequest::RedirectStatus::kNoRedirect, ReportingDisposition::kSuppressReporting, String(test.nonce), IntegrityMetadataSet(),
                kParserInserted));

        // Report-only 'style-src'
        directive_list = CreateList(String("style-src ") + test.list, ContentSecurityPolicyType::kReport);
        EXPECT_TRUE(CSPDirectiveListAllowFromSource(*directive_list, context, CSPDirectiveName::StyleSrcElem, resource, resource,
            ResourceRequest::RedirectStatus::kNoRedirect, ReportingDisposition::kSuppressReporting, String(test.nonce)));

        // Enforce 'style-src'
        directive_list = CreateList(String("style-src ") + test.list, ContentSecurityPolicyType::kEnforce);
        EXPECT_EQ(CSPCheckResult(test.expected),
            CSPDirectiveListAllowFromSource(*directive_list, context, CSPDirectiveName::StyleSrcElem, resource, resource,
                ResourceRequest::RedirectStatus::kNoRedirect, ReportingDisposition::kSuppressReporting, String(test.nonce)));

        // Report-only 'style-src'
        directive_list = CreateList(String("default-src ") + test.list, ContentSecurityPolicyType::kReport);
        EXPECT_TRUE(CSPDirectiveListAllowFromSource(*directive_list, context, CSPDirectiveName::ScriptSrcElem, resource, resource,
            ResourceRequest::RedirectStatus::kNoRedirect, ReportingDisposition::kSuppressReporting, String(test.nonce)));
        EXPECT_TRUE(CSPDirectiveListAllowFromSource(*directive_list, context, CSPDirectiveName::StyleSrcElem, resource, resource,
            ResourceRequest::RedirectStatus::kNoRedirect, ReportingDisposition::kSuppressReporting, String(test.nonce)));
        ...
        // Enforce 'style-src'
        directive_list = CreateList(String("default-src ") + test.list, ContentSecurityPolicyType::kEnforce);
        EXPECT_EQ(CSPCheckResult(test.expected),
            CSPDirectiveListAllowFromSource(*directive_list, context, CSPDirectiveName::ScriptSrcElem, resource, resource,
                ResourceRequest::RedirectStatus::kNoRedirect, ReportingDisposition::kSuppressReporting, String(test.nonce), IntegrityMetadataSet(),
                kParserInserted));
...
TEST_F(CSPDirectiveListTest, AllowScriptFromSourceWithHash)
{
    struct TestCase {
        ...
        { "https://example.com", "https://example.com/file", "", true },
        { "https://example.com", "https://not.example.com/file", "sha256-yay", false },
        { "https://example.com", "https://not.example.com/file", "sha256-boo", false },
        { "https://example.com", "https://not.example.com/file", "", false },

        // Doesn't affect URLs that match the allowlist.
        { "https://example.com 'sha256-yay'", "https://example.com/file", "sha256-yay", true },
        { "https://example.com 'sha256-yay'", "https://example.com/file", "sha256-boo", true },
        { "https://example.com 'sha256-yay'", "https://example.com/file", "", true },

        // Does affect URLs that don't match the allowlist.
        { "https://example.com 'sha256-yay'", "https://not.example.com/file", "sha256-yay", true },
        { "https://example.com 'sha256-yay'", "https://not.example.com/file", "sha256-boo", false },
        { "https://example.com 'sha256-yay'", "https://not.example.com/file", "", false },
        ...
        { "'sha256-yay' 'sha384-boo'", "https://a.com/file", "sha256-yay sha384-yay", false },
        { "'sha256-yay' 'sha384-boo'", "https://a.com/file", "sha256-boo sha384-boo", false },
        { "'sha256-yay' 'sha384-boo'", "https://a.com/file", "sha256-yay sha384-boo", true },
    ...
    for (const auto& test : cases) {
        SCOPED_TRACE(testing::Message() << "List: `" << test.list << "`, URL: `" << test.url << "`, Integrity: `" << test.integrity << "`");
        const KURL resource(test.url);

        IntegrityMetadataSet integrity_metadata;
        SubresourceIntegrity::ParseIntegrityAttribute(test.integrity, SubresourceIntegrity::IntegrityFeatures::kDefault, integrity_metadata);

        // Report-only 'script-src'
        network::mojom::blink::ContentSecurityPolicyPtr directive_list = CreateList(String("script-src ") + test.list, ContentSecurityPolicyType::kReport);
        EXPECT_TRUE(CSPDirectiveListAllowFromSource(*directive_list, context, CSPDirectiveName::ScriptSrcElem, resource, resource,
            ResourceRequest::RedirectStatus::kNoRedirect, ReportingDisposition::kSuppressReporting, String(), integrity_metadata, kParserInserted));
...
TEST_F(CSPDirectiveListTest, WorkerSrc)
{
    struct TestCase {
        ...
        { "default-src *; script-src *; worker-src http://not.example.test", false },
    ...
    for (const auto& test : cases) {
        ...
        network::mojom::blink::ContentSecurityPolicyPtr directive_list = CreateList(test.list, ContentSecurityPolicyType::kEnforce);
        EXPECT_EQ(CSPCheckResult(test.allowed),
            CSPDirectiveListAllowFromSource(*directive_list, context, CSPDirectiveName::WorkerSrc, resource, resource,
...
TEST_F(CSPDirectiveListTest, WorkerSrcChildSrcFallback)
{
    ...
    struct TestCase {
        ...
        { "child-src https://example.test; script-src https://not-example.test", true },
        { "child-src https://not-example.test; script-src https://example.test", false },
        ...
        { "worker-src https://example.test; child-src https://not-example.test", true },
        { "worker-src https://not-example.test; child-src https://example.test", false },
...
TEST_F(CSPDirectiveListTest, OperativeDirectiveGivenType)
{
    struct TestCase {
        ...
        { CSPDirectiveName::ScriptSrcAttr, { CSPDirectiveName::ScriptSrc, CSPDirectiveName::DefaultSrc } },
        { CSPDirectiveName::ScriptSrcElem, { CSPDirectiveName::ScriptSrc, CSPDirectiveName::DefaultSrc } },
        { CSPDirectiveName::FrameSrc, { CSPDirectiveName::ChildSrc, CSPDirectiveName::DefaultSrc } },
        { CSPDirectiveName::WorkerSrc, { CSPDirectiveName::ChildSrc, CSPDirectiveName::ScriptSrc, CSPDirectiveName::DefaultSrc } },
    };
    ...
    network::mojom::blink::ContentSecurityPolicyPtr empty = CreateList("nonexistent-directive", ContentSecurityPolicyType::kEnforce);
    ...
    for (auto& test : cases) {
        ...
        // null.
        EXPECT_FALSE(CSPDirectiveListOperativeDirective(*empty, test.directive).source_list);
        ...
        while (!test.fallback_list.empty()) {
            directive_list = CreateList(directive_string.c_str(), ContentSecurityPolicyType::kEnforce);

            CSPOperativeDirective operative_directive = CSPDirectiveListOperativeDirective(*directive_list, test.directive);
            ...
            const char* current_directive_name = ContentSecurityPolicy::GetDirectiveName(test.fallback_list.front());

            std::stringstream current_directive;
            current_directive << current_directive_name << " http://" << current_directive_name << ".com; ";
        ...
        directive_list = CreateList(directive_string.c_str(), ContentSecurityPolicyType::kEnforce);
        EXPECT_FALSE(CSPDirectiveListOperativeDirective(*directive_list, test.directive).source_list);
    }
}
...
TEST_F(CSPDirectiveListTest, ReportEndpointsProperlyParsed)
{
    struct TestCase {
        ...
        { "script-src 'self'; report-uri https://example.com", ContentSecurityPolicySource::kHTTP, { "https://example.com/" }, false },
        { "script-src 'self'; report-uri https://example.com "
          "https://example2.com",
            ContentSecurityPolicySource::kHTTP, { "https://example.com/", "https://example2.com/" }, false },
        { "script-src 'self'; report-uri https://example.com "
          "http://example2.com /relative/path",
            // Mixed Content report-uri endpoint is ignored.
            ContentSecurityPolicySource::kHTTP, { "https://example.com/", "https://example.test/relative/path" }, false },
        { "script-src 'self'; report-uri https://example.com", ContentSecurityPolicySource::kMeta, {}, false },
        { "script-src 'self'; report-to group", ContentSecurityPolicySource::kHTTP, { "group" }, true },
        // report-to supersedes report-uri
        { "script-src 'self'; report-to group; report-uri https://example.com", ContentSecurityPolicySource::kHTTP, { "group" }, true },
        { "script-src 'self'; report-to group", ContentSecurityPolicySource::kMeta, { "group" }, true },
        { "script-src 'self'; report-to group group2", ContentSecurityPolicySource::kHTTP,
            // Only the first report-to endpoint is used. The other ones are ignored.
            { "group" }, true },
        { "script-src 'self'; report-to group; report-to group2;", ContentSecurityPolicySource::kHTTP, { "group" }, true },
        { "script-src 'self'; report-to group; report-uri https://example.com; "
          "report-to group2",
            ContentSecurityPolicySource::kHTTP, { "group" }, true },
    ...
    for (const auto& test : cases) {
        ...
        for (const auto& header_type : { ContentSecurityPolicyType::kEnforce, ContentSecurityPolicyType::kReport }) {
            network::mojom::blink::ContentSecurityPolicyPtr directive_list = CreateList(test.policy, header_type, test.header_source);

            EXPECT_EQ(directive_list->use_reporting_api, test.expected_use_reporting_api);
            EXPECT_EQ(directive_list->report_endpoints.size(), test.expected_endpoints.size());
...
TEST_F(CSPDirectiveListTest, ReasonableObjectRestriction)
{
    struct TestCase {
        ...
        { "img-src *", false }, { "object-src *", false }, { "object-src https://very.safe.test/", false }, { "object-src https:", false },
        { "script-src *", false }, { "script-src https://very.safe.test/", false }, { "script-src https:", false },
        { "script-src 'none'; object-src *", false }, { "script-src 'none'; object-src https://very.safe.test/", false },
        { "script-src 'none'; object-src https:", false },

        // Sufficient restrictions!
        { "default-src 'none'", true }, { "object-src 'none'", true }, { "object-src 'none'; script-src 'unsafe-inline'", true },
    ...
    for (const auto& test : cases) {
        ...
        network::mojom::blink::ContentSecurityPolicyPtr directive_list = CreateList(test.list, ContentSecurityPolicyType::kReport);
        EXPECT_EQ(test.expected, CSPDirectiveListIsObjectRestrictionReasonable(*directive_list));
        directive_list = CreateList(test.list, ContentSecurityPolicyType::kEnforce);
        EXPECT_EQ(test.expected, CSPDirectiveListIsObjectRestrictionReasonable(*directive_list));
    }
}
...
TEST_F(CSPDirectiveListTest, ReasonableBaseRestriction)
{
    struct TestCase {
        ...
        { "default-src 'none'", false }, { "base-uri https://very.safe.test/", false }, { "base-uri *", false }, { "base-uri https:", false },

        // Sufficient restrictions!
        { "base-uri 'none'", true }, { "base-uri 'self'", true }
    };

    for (const auto& test : cases) {
        SCOPED_TRACE(testing::Message() << "List: `" << test.list << "`");
        network::mojom::blink::ContentSecurityPolicyPtr directive_list = CreateList(test.list, ContentSecurityPolicyType::kReport);
        EXPECT_EQ(test.expected, CSPDirectiveListIsBaseRestrictionReasonable(*directive_list));
        directive_list = CreateList(test.list, ContentSecurityPolicyType::kEnforce);
        EXPECT_EQ(test.expected, CSPDirectiveListIsBaseRestrictionReasonable(*directive_list));
    }
}
...
TEST_F(CSPDirectiveListTest, ReasonableScriptRestriction)
{
    struct TestCase {
        ...
        { "img-src *", false }, { "script-src *", false }, { "script-src https://very.safe.test/", false }, { "script-src https:", false },
        { "default-src 'none'; script-src *", false }, { "default-src 'none'; script-src https://very.safe.test/", false },
        { "default-src 'none'; script-src https:", false },

        // Sufficient restrictions!
        { "default-src 'none'", true }, { "script-src 'none'", true }, { "script-src 'nonce-abc'", true }, { "script-src 'sha256-abc'", true },
        { "script-src 'nonce-abc' 'unsafe-inline'", true }, { "script-src 'sha256-abc' 'unsafe-inline'", true },
        { "script-src 'nonce-abc' 'strict-dynamic'", true }, { "script-src 'sha256-abc' 'strict-dynamic'", true },
        { "script-src 'nonce-abc' 'unsafe-inline' 'strict-dynamic'", true }, { "script-src 'sha256-abc' 'unsafe-inline' 'strict-dynamic'", true },
        { "script-src 'nonce-abc' 'unsafe-inline' 'unsafe-eval' 'unsafe-hashes'", true },
        { "script-src 'sha256-abc' 'unsafe-inline' 'unsafe-eval' 'unsafe-hashes'", true },
        { "script-src 'nonce-abc' 'strict-dynamic' 'unsafe-eval' 'unsafe-hashes'", true },
        { "script-src 'sha256-abc' 'strict-dynamic' 'unsafe-eval' 'unsafe-hashes'", true },
    ...
    for (const auto& test : cases) {
        SCOPED_TRACE(testing::Message() << "List: `" << test.list << "`");
        network::mojom::blink::ContentSecurityPolicyPtr directive_list = CreateList(test.list, ContentSecurityPolicyType::kReport);
        EXPECT_EQ(test.expected, CSPDirectiveListIsScriptRestrictionReasonable(*directive_list));
        directive_list = CreateList(test.list, ContentSecurityPolicyType::kEnforce);
        EXPECT_EQ(test.expected, CSPDirectiveListIsScriptRestrictionReasonable(*directive_list));
    }
}
...
TEST_F(CSPDirectiveListTest, ReportURIInMeta)
{
    ...
    network::mojom::blink::ContentSecurityPolicyPtr directive_list
        = CreateList(policy, ContentSecurityPolicyType::kEnforce, ContentSecurityPolicySource::kMeta);
    EXPECT_TRUE(directive_list->report_endpoints.empty());
    directive_list = CreateList(policy, ContentSecurityPolicyType::kEnforce, ContentSecurityPolicySource::kHTTP);
...
TEST_F(CSPDirectiveListTest, StrictDynamicIgnoresAllowlistWarning)
{
    ...
    network::mojom::blink::ContentSecurityPolicyPtr directive_list_with_blocked
        = CreateList("script-src 'nonce-abc' https://blocked.com 'strict-dynamic'", ContentSecurityPolicyType::kEnforce);
    network::mojom::blink::ContentSecurityPolicyPtr directive_list_without_blocked
        = CreateList("script-src 'nonce-abc' 'strict-dynamic'", ContentSecurityPolicyType::kEnforce);
    ...
    for (const auto& testCase : testCases) {
        ...
        context->BindToDelegate(*test_delegate);
        for (auto reporting_disposition : { ReportingDisposition::kSuppressReporting, ReportingDisposition::kReport }) {
            EXPECT_EQ(CSPCheckResult(testCase.allowed),
                CSPDirectiveListAllowFromSource(*testCase.directive_list, context, CSPDirectiveName::ScriptSrcElem, testCase.script_url, testCase.script_url,
                    ResourceRequest::RedirectStatus::kNoRedirect, reporting_disposition, testCase.script_nonce));
        }
        static const char* message = "Note that 'strict-dynamic' is present, so "
                                     "host-based allowlisting is disabled.";
        if (testCase.console_message) {
            EXPECT_THAT(test_delegate->console_messages(), testing::Contains(HasSubstr(message)));
        } else {
            EXPECT_THAT(test_delegate->console_messages(), testing::Not(testing::Contains(HasSubstr(message))));
        }
    }
}
