...
bool DoesFrameContainHtmlDocument(Frame* frame, Element* element)
{
    ...
    // files.
    return element->HasTagName(html_names::kIFrameTag) || element->HasTagName(html_names::kFrameTag);
}
...
void GetSavableResourceLinkForElement(Element* element, const Document& current_document, SavableResources::Result* result)
{
    // Get absolute URL.
    String link_attribute_value = SavableResources::GetSubResourceLinkFromElement(element);
    ...
    if (auto* frame_owner = DynamicTo<HTMLFrameOwnerElement>(element)) {
        ...
        if (content_frame && DoesFrameContainHtmlDocument(content_frame, element)) {
            mojom::blink::SavableSubframePtr subframe = mojom::blink::SavableSubframe::New(element_url, content_frame->GetFrameToken());
    ...
    if (!element_url.ProtocolIsInHTTPFamily() && !element_url.ProtocolIs(url::kFileScheme))
        return;
...
// static
bool SavableResources::GetSavableResourceLinksForFrame(LocalFrame* current_frame, SavableResources::Result* result)
{
    ...
    for (unsigned i = 0; i < collection->length(); ++i) {
        GetSavableResourceLinkForElement(collection->item(i), *current_document, result);
    }
...
String SavableResources::GetSubResourceLinkFromElement(Element* element)
{
    ...
    if (element->HasTagName(html_names::kImgTag) || element->HasTagName(html_names::kFrameTag) || element->HasTagName(html_names::kIFrameTag)
    ...
    } else if (element->HasTagName(html_names::kBodyTag) || element->HasTagName(html_names::kTableTag) || element->HasTagName(html_names::kTrTag)
        || element->HasTagName(html_names::kTdTag)) {
        attribute_name = &html_names::kBackgroundAttr;
    } else if (element->HasTagName(html_names::kBlockquoteTag) || element->HasTagName(html_names::kQTag) || element->HasTagName(html_names::kDelTag)
        ...
        if (EqualIgnoringASCIICase(type, "text/css") || EqualIgnoringASCIICase(rel, "stylesheet")) {
    ...
    if (!value.IsNull() && !value.empty() && !value.StartsWith("javascript:", kTextCaseASCIIInsensitive))
        return value;

    return String();
}

void SavableResources::Result::AppendSubframe(mojom::blink::SavableSubframePtr subframe)
{
