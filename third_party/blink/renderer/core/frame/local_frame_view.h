...
class CORE_EXPORT LocalFrameView final : public GarbageCollected<LocalFrameView>, public FrameView {
    friend class PaintControllerPaintTestBase;
    friend class Internals;

public:
    class CORE_EXPORT LifecycleNotificationObserver : public GarbageCollectedMixin {
    public:
    ...
    unsigned LayoutCountForTesting() const
    {
        return layout_count_for_testing_;
    }
    ...
    void IncBlockLayoutCount()
    {
        ++block_layout_count_for_testing_;
    }

    void CountObjectsNeedingLayout(unsigned& needs_layout_objects, unsigned& total_objects, bool& is_partial);

    bool NeedsLayout() const;
    bool CheckDoesNotNeedLayout() const;
    void SetNeedsLayout();

    void SetNeedsUpdateGeometries()
    {
        needs_update_geometries_ = true;
    }
    ...
    IntersectionObservationState GetIntersectionObservationStateForTesting() const
    {
    ...
    void SetUseColorAdjustBackground(UseColorAdjustBackground use, bool color_scheme_changed);
    ...
    void InvalidateBackgroundAttachmentFixedDescendantsOnScroll(const LayoutBox& scroller);
    ...
    public:
        ...
        InvalidationDisallowedScope& operator=(const InvalidationDisallowedScope&) = delete;
    ...
    // throttling enabled.
    HitTestResult HitTestWithThrottlingAllowed(const HitTestLocation&, HitTestRequest::HitTestRequestType) const;

    void IncrementLayoutObjectCount()
    {
        layout_object_counter_.Increment();
    }
    void IncrementVisuallyNonEmptyCharacterCount(unsigned);
    void IncrementVisuallyNonEmptyPixelCount(const gfx::Size&);
    bool IsVisuallyNonEmpty() const
    {
        return is_visually_non_empty_;
    }
    ...
    void ProcessUrlFragment(const KURL&, bool same_document_navigation, bool should_scroll = true);
    FragmentAnchor* GetFragmentAnchor()
    {
        return fragment_anchor_.Get();
    }
    ...
    using ScrollableAreaMap = HeapHashMap<CompositorElementId, Member<PaintLayerScrollableArea>>;
    ...
    void ScheduleAnimation(base::TimeDelta = base::TimeDelta(), base::Location location = base::Location::Current());
    ...
    void SetRootLayerDidScroll()
    {
        root_layer_did_scroll_ = true;
    }
    ...
    void PaintOutsideOfLifecycleWithThrottlingAllowed(GraphicsContext&, PaintFlags, const CullRect& cull_rect = CullRect::Infinite());
    ...
    void PaintOutsideOfLifecycle(GraphicsContext&, PaintFlags, const CullRect& cull_rect = CullRect::Infinite());
    ...
    bool IsLocalFrameView() const override
    {
        return true;
    }
    bool ShouldReportMainFrameIntersection() const override
    {
        return true;
    }
    ...
    void UpdateRenderThrottlingStatus(bool hidden_for_throttling, bool subtree_throttled, bool display_locked, bool recurse = false) override;
    ...
    bool MapToVisualRectInRemoteRootFrame(PhysicalRect& rect, bool apply_overflow_clip = true);
    ...
    size_t PaintFrameCount() const
    {
        return paint_frame_count_;
    }
    ...
    void ScrollRectToVisibleInRemoteParent(const PhysicalRect&, mojom::blink::ScrollIntoViewParamsPtr);
    ...
    void NotifyVideoIsDominantVisibleStatus(HTMLVideoElement* element, bool is_dominant);
    ...
    {
        return paint_debug_info_enabled_;
    }
...
protected:
    ...
    void SetViewportIntersection(const mojom::blink::ViewportIntersectionState& intersection_state) override;
...
private:
    ...
    bool CapturePaintPreview(GraphicsContext& context, const gfx::Vector2d& paint_offset) const;

    // EmbeddedContentView implementation
    void Paint(GraphicsContext&, PaintFlags, const CullRect&, const gfx::Vector2d&) const final;
    ...
    // Returns whether the lifecycle was successfully updated to the
    // target state.
    bool UpdateLifecyclePhases(DocumentLifecycle::LifecycleState target_state, DocumentUpdateReason reason);
    // The internal version that does the work after the proper context and checks
    // have passed in the above function call.
    void UpdateLifecyclePhasesInternal(DocumentLifecycle::LifecycleState target_state);
    ...
    bool RunCompositingInputsLifecyclePhase(DocumentLifecycle::LifecycleState target_state);
    bool RunPrePaintLifecyclePhase(DocumentLifecycle::LifecycleState target_state);
    ...
    PhysicalOffset ConvertToContainingEmbeddedContentView(const PhysicalOffset&) const;
    ...
    PhysicalOffset ConvertFromContainingEmbeddedContentView(const PhysicalOffset&) const;
    gfx::PointF ConvertFromContainingEmbeddedContentView(const gfx::PointF&) const;
    ...
    void CollectDraggableRegions(LayoutObject&, Vector<DraggableRegionValue>&) const;

    void ForAllChildViewsAndPlugins(base::FunctionRef<void(EmbeddedContentView&)>);

    enum TraversalOrder { kPreOrder, kPostOrder };
    void ForAllNonThrottledLocalFrameViews(base::FunctionRef<void(LocalFrameView&)>, TraversalOrder = kPreOrder);
    ...
    bool UpdateViewportIntersectionsForSubtree(unsigned parent_flags, ComputeIntersectionsContext&) override;
    ...
    void ComputePostLayoutIntersections(unsigned parent_flags, ComputeIntersectionsContext&);
    ...
    void AppendViewTransitionRequests(WTF::Vector<std::unique_ptr<ViewTransitionRequest>>&);
    ...
    UseColorAdjustBackground use_color_adjust_background_ { UseColorAdjustBackground::kNo };
    ...
    std::optional<LocalFrameUkmAggregator::ScopedForcedLayoutTimer> forced_layout_timer_;
    ...
    std::unique_ptr<OverlayInterstitialAdDetector> overlay_interstitial_ad_detector_;
...
inline void LocalFrameView::IncrementVisuallyNonEmptyCharacterCount(unsigned count)
{
...
inline void LocalFrameView::IncrementVisuallyNonEmptyPixelCount(const gfx::Size& size)
{
    if (is_visually_non_empty_)
        return;
...
template <> struct DowncastTraits<LocalFrameView> {
