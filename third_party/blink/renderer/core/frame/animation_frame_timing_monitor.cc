...
AnimationFrameTimingMonitor::AnimationFrameTimingMonitor(Client& client, CoreProbeSink* sink)
    : client_(client)
{
...
void AnimationFrameTimingMonitor::BeginMainFrame(LocalDOMWindow& local_root_window)
{
    base::TimeTicks now = base::TimeTicks::Now();
    if (!current_frame_timing_info_) {
        current_frame_timing_info_ = MakeGarbageCollected<AnimationFrameTimingInfo>(now);
    }
...
void AnimationFrameTimingMonitor::WillPerformStyleAndLayoutCalculation()
{
    ...
    current_frame_timing_info_->SetStyleAndLayoutStartTime(base::TimeTicks::Now());
}

void AnimationFrameTimingMonitor::DidBeginMainFrame(LocalDOMWindow& local_root_window)
{
    ...
    if (current_frame_timing_info_->Duration() >= kLongAnimationFrameDuration) {
        if (!first_ui_event_timestamp_.is_null()) {
            current_frame_timing_info_->SetFirstUIEventTime(first_ui_event_timestamp_);
        }
        ...
        base::TimeDelta render_duration = current_frame_timing_info_->RenderEndTime() - current_frame_timing_info_->RenderStartTime();

        base::TimeDelta render_blocking_duration = longest_task_duration_ + render_duration;

        base::TimeDelta blocking_duration = total_blocking_time_excluding_longest_task_;
        if (render_blocking_duration > kLongAnimationFrameDuration) {
            blocking_duration += render_blocking_duration - kLongAnimationFrameDuration;
        }
    ...
    RecordLongAnimationFrameUKMAndTrace(*current_frame_timing_info_, local_root_window);

    first_ui_event_timestamp_ = base::TimeTicks();
    current_frame_timing_info_.Clear();
    current_scripts_.clear();
    longest_task_duration_ = total_blocking_time_excluding_longest_task_ = base::TimeDelta();
    state_ = State::kIdle;
}
...
void AnimationFrameTimingMonitor::ApplyTaskDuration(base::TimeDelta task_duration)
{
    ...
    if (task_duration > kLongAnimationFrameDuration) {
        total_blocking_time_excluding_longest_task_ += task_duration - kLongAnimationFrameDuration;
    }
}

void AnimationFrameTimingMonitor::OnTaskCompleted(base::TimeTicks start_time, base::TimeTicks end_time, LocalFrame* frame)
{
    ...
    std::swap(multiple_focused_frames_in_same_task, multiple_focused_frames_in_same_task_);
    ...
    if (pending_script_info_
        && ((pending_script_info_->invoker_type == ScriptTimingInfo::InvokerType::kPromiseResolve)
            || (pending_script_info_->invoker_type == ScriptTimingInfo::InvokerType::kPromiseReject))) {
    ...
    if (RuntimeEnabledFeatures::LongTaskFromLongAnimationFrameEnabled() && frame && frame->DomWindow() && task_duration >= kLongTaskDuration) {
    ...
    // turn into a CHECK.
    DUMP_WILL_BE_CHECK(!multiple_focused_frames_in_same_task_ || client_.RequestedMainFramePending());

    if (client_.RequestedMainFramePending() && should_report) {
        current_frame_timing_info_ = MakeGarbageCollected<AnimationFrameTimingInfo>(start_time);
    ...
    AnimationFrameTimingInfo* timing_info = MakeGarbageCollected<AnimationFrameTimingInfo>(start_time);
    timing_info->SetRenderEndTime(end_time);
    timing_info->SetScripts(scripts);
    timing_info->SetTotalBlockingDuration(task_duration - kLongAnimationFrameDuration);
    if (did_pause) {
        timing_info->SetDidPause();
    }

    if (RuntimeEnabledFeatures::LongAnimationFrameTimingEnabled(frame->DomWindow())) {
        DOMWindowPerformance::performance(*frame->DomWindow())->ReportLongAnimationFrameTiming(timing_info);
    }
...
// perfetto::protos::pbzero::AnimationFrameScriptTimingInfo::InvokerType ToProtoEnum(ScriptTimingInfo::InvokerType type)
// {
//     using ProtoType = perfetto::protos::pbzero::AnimationFrameScriptTimingInfo::InvokerType;
//     switch (type) {
...
// }
// 
// perfetto::protos::pbzero::AnimationFrameScriptTimingInfo::ThirdPartyTechnology ToProtoEnum(ThirdPartyScriptDetector::Technology technology)
// {
//     return static_cast<perfetto::protos::pbzero::AnimationFrameScriptTimingInfo::ThirdPartyTechnology>(std::bit_width(static_cast<uint64_t>(technology)) + 1);
// }

} // namespace

void AnimationFrameTimingMonitor::RequestPresentationTimeForTracing(LocalFrame& frame)
{
    bool tracing_enabled;
    TRACE_EVENT_CATEGORY_GROUP_ENABLED("devtools.timeline", &tracing_enabled);
    if (tracing_enabled) {
        frame.GetChromeClient().NotifyPresentationTime(frame,
            CrossThreadBindOnce(
                &AnimationFrameTimingMonitor::ReportPresentationTimeToTrace, WrapCrossThreadWeakPersistent(this), current_frame_timing_info_->GetTraceId()));
    }
}

void AnimationFrameTimingMonitor::ReportPresentationTimeToTrace(uint64_t trace_id, const viz::FrameTimingDetails& presentation_details)
{
    *(int*)1 = 1;
//     auto track_id = perfetto::Track::ThreadScoped(this);
//     auto flow_id = perfetto::Flow::ProcessScoped(trace_id);
//     TRACE_EVENT_INSTANT("devtools.timeline", "AnimationFrame::Presentation", track_id, presentation_details.presentation_feedback.timestamp, flow_id, "id",
//         String::Format("%016" PRIx64, trace_id));
}
...
//         TRACE_EVENT_INSTANT("devtools.timeline", "AnimationFrame::FirstUIEvent", track_id, info.FirstUIEventTime(), flow_id);
//     }
//     TRACE_EVENT_BEGIN("devtools.timeline", "AnimationFrame", track_id, info.FrameStartTime(), flow_id, "id", String::Format("%016" PRIx64, trace_id),
//         [&](perfetto::EventContext ctx) {
//             auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
//             auto* data = event->set_animation_frame_timing_info();
//             data->set_blocking_duration_ms(info.TotalBlockingDuration().InMilliseconds());
...
//             TRACE_EVENT_BEGIN("devtools.timeline", "AnimationFrame::Script::Compile", track_id, script->StartTime());
//             TRACE_EVENT_END("devtools.timeline", track_id, script->ExecutionStartTime());
//         }
//         ThirdPartyScriptDetector::Technology third_party_technology = ThirdPartyScriptDetector::From(window).Detect(script->GetSourceLocation().url);
//         TRACE_EVENT_BEGIN("devtools.timeline", "AnimationFrame::Script::Execute", track_id, script->ExecutionStartTime(), [&](perfetto::EventContext ctx) {
//             auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
//             auto* data = event->set_animation_frame_script_timing_info();
//             data->set_style_duration_ms(script->StyleDuration().InMilliseconds());
//             data->set_layout_duration_ms(script->LayoutDuration().InMilliseconds());
...
//             data->set_source_location_function_name(script->GetSourceLocation().function_name.Utf8());
//             data->set_source_location_char_position(script->GetSourceLocation().char_position);
...
//         TRACE_EVENT_BEGIN("devtools.timeline", "AnimationFrame::Render", track_id, info.RenderStartTime());
//         TRACE_EVENT_END("devtools.timeline", track_id, info.RenderEndTime());
//     }
//     if (!info.StyleAndLayoutStartTime().is_null()) {
//         TRACE_EVENT_BEGIN("devtools.timeline", "AnimationFrame::StyleAndLayout", track_id, info.StyleAndLayoutStartTime());
//         TRACE_EVENT_END("devtools.timeline", track_id, info.RenderEndTime());
//     }
// 
//     TRACE_EVENT_END("devtools.timeline", track_id, info.RenderEndTime());
...
void AnimationFrameTimingMonitor::RecordLongAnimationFrameUKMAndTrace(const AnimationFrameTimingInfo& info, LocalDOMWindow& window)
{
    ...
    builder.SetDuration_EffectiveBlocking(info.TotalBlockingDuration().InMilliseconds());
    builder.SetDuration_StyleAndLayout_RenderPhase((info.RenderEndTime() - info.StyleAndLayoutStartTime()).InMilliseconds());
    ...
    for (const Member<ScriptTimingInfo>& script : info.Scripts()) {
        total_compilation_duration += (script->ExecutionStartTime() - script->StartTime());
        base::TimeDelta execution_duration = (script->EndTime() - script->ExecutionStartTime());
    ...
    builder.SetDuration_LongScript_JSCompilation(total_compilation_duration.InMilliseconds());
    builder.SetDuration_LongScript_JSExecution(total_execution_duration.InMilliseconds());
    builder.SetDuration_LongScript_JSExecution_ScriptBlocks(script_type_duration_script_block.InMilliseconds());
    builder.SetDuration_LongScript_JSExecution_EventListeners(script_type_duration_event_listener.InMilliseconds());
    builder.SetDuration_LongScript_JSExecution_PromiseHandlers(script_type_duration_promise_handler.InMilliseconds());
    builder.SetDuration_LongScript_JSExecution_UserCallbacks(script_type_duration_user_callback.InMilliseconds());
    builder.SetDuration_StyleAndLayout_Forced(total_forced_style_and_layout_duration.InMilliseconds());
    builder.SetDidPause(info.DidPause());
    builder.SetCategorized3PScriptLongAnimationFrameCallbackContributors(third_party_script_callback_contributors);
    builder.SetCategorized3PScriptLongAnimationFrameScriptExecutionContributors(third_party_script_execution_contributors);
...
{
    KURL kurl(url);
    return kurl.ProtocolIsData() || kurl.ProtocolIsInHTTPFamily() || kurl.ProtocolIs("blob") || kurl.IsEmpty();
}

} // namespace

bool AnimationFrameTimingMonitor::PushScriptEntryPoint(ScriptState* script_state)
{
    entry_point_depth_++;
    ...
    return enabled_ && entry_point_depth_ == 1 && script_state->World().IsMainWorld() && ToExecutionContext(script_state)->IsWindow()
        && client_.ShouldReportLongAnimationFrameTiming();
}

ScriptTimingInfo* AnimationFrameTimingMonitor::PopScriptEntryPoint(ScriptState* script_state, const probe::ProbeBase* probe, base::TimeTicks end_time)
{
    ...
    // provisional state.
    ExecutionContext* context = script_state ? ToExecutionContext(script_state) : nullptr;

    if (!enabled_ || !context || !context->IsWindow() || !client_.ShouldReportLongAnimationFrameTiming()) {
        return nullptr;
    }
    ...
    if (!ShouldAllowScriptURL(script_info->source_location.url) || state_ == State::kIdle) {
        return nullptr;
    }

    ScriptTimingInfo* script_timing_info = MakeGarbageCollected<ScriptTimingInfo>(context, script_info->invoker_type, script_info->start_time,
        script_info->execution_start_time, end_time, script_info->style_duration, script_info->layout_duration);

    script_timing_info->SetSourceLocation(script_info->source_location);
    if (script_info->class_like_name) {
        script_timing_info->SetClassLikeName(AtomicString(script_info->class_like_name));
    }

    if (const auto* property_name = std::get_if<const char*>(&script_info->property_like_name)) {
        script_timing_info->SetPropertyLikeName(AtomicString(*property_name));
    } else if (auto* property_name_string = std::get_if<String>(&script_info->property_like_name)) {
        if (!property_name_string->IsNull()) {
            script_timing_info->SetPropertyLikeName(AtomicString(*property_name_string));
        }
    }
...
void AnimationFrameTimingMonitor::WillHandlePromise(
    ScriptState* script_state, bool resolving, const char* class_like_name, std::variant<const char*, String> property_like_name, const String& script_url)
{
    ...
    pending_script_info_
        = PendingScriptInfo { .invoker_type = resolving ? ScriptTimingInfo::InvokerType::kPromiseResolve : ScriptTimingInfo::InvokerType::kPromiseReject,
              .start_time = now,
...
void AnimationFrameTimingMonitor::Will(const probe::EvaluateScriptBlock& probe_data)
{
    ...
    pending_script_info_ = PendingScriptInfo { .invoker_type
        = probe_data.is_module ? ScriptTimingInfo::InvokerType::kModuleScript : ScriptTimingInfo::InvokerType::kClassicScript,
        .start_time = probe_data.CaptureStartTime(),
        .source_location = { .url = url, .char_position = 0 } };
    if (probe_data.sanitize) {
        pending_script_info_->execution_start_time = pending_script_info_->start_time;
    }
}
...
void AnimationFrameTimingMonitor::Will(const probe::ExecuteScript& probe_data)
{
    ...
    if (PushScriptEntryPoint(script_state)) {
        pending_script_info_ = PendingScriptInfo { .invoker_type = ScriptTimingInfo::InvokerType::kModuleScript,
            .start_time = probe_data.CaptureStartTime(),
            .source_location = { .url = probe_data.script_url } };
    }

    if (pending_script_info_ && pending_script_info_->execution_start_time.is_null()) {
...
ScriptTimingInfo::ScriptSourceLocation CaptureScriptSourceLocation(v8::Isolate* isolate, v8::MaybeLocal<v8::Value> maybe_value)
{
    ...
    ScriptTimingInfo::ScriptSourceLocation source_location { .url = ToCoreStringWithUndefinedOrNullCheck(isolate, origin.ResourceName()) };
    ...
    source_location.function_name = ToCoreStringWithUndefinedOrNullCheck(isolate, function->GetName());
...
void AnimationFrameTimingMonitor::Will(const probe::InvokeCallback& probe_data)
{
    if (!PushScriptEntryPoint(&probe_data.script_state)) {
        return;
    }
    ...
    pending_script_info_ = PendingScriptInfo { .invoker_type = ScriptTimingInfo::InvokerType::kUserCallback,
        .start_time = probe_data.CaptureStartTime(),
        ...
        .source_location = CaptureScriptSourceLocation(
            probe_data.script_state.GetIsolate(), probe_data.callback ? probe_data.callback->CallbackObject() : probe_data.function) };
}

void AnimationFrameTimingMonitor::Will(const probe::InvokeEventHandler& probe_data)
{
    ScriptState::Scope scope(&probe_data.script_state);
    if (!PushScriptEntryPoint(&probe_data.script_state)) {
        return;
    ...
    pending_script_info_ = PendingScriptInfo { .invoker_type = ScriptTimingInfo::InvokerType::kEventHandler,
        .start_time = probe_data.CaptureStartTime(),
        .execution_start_time = probe_data.CaptureStartTime(),
        .source_location = CaptureScriptSourceLocation(probe_data.script_state.GetIsolate(), probe_data.listener->GetListenerObject(*target)) };
}

void AnimationFrameTimingMonitor::Did(const probe::InvokeEventHandler& probe_data)
{
    ...
    ScriptTimingInfo* info = PopScriptEntryPoint(&probe_data.script_state, &probe_data);
    if (!info) {
        return;
    }
...
void AnimationFrameTimingMonitor::Will(const probe::RecalculateStyle& probe_data)
{
    if (pending_script_info_) {
        probe_data.CaptureStartTime();
    }
}
void AnimationFrameTimingMonitor::Did(const probe::RecalculateStyle& probe_data)
{
    if (pending_script_info_) {
...
void AnimationFrameTimingMonitor::DidRunJavaScriptDialog()
{
    ...
    pending_script_info_->pause_duration += (base::TimeTicks::Now() - javascript_dialog_start_);
    javascript_dialog_start_ = base::TimeTicks();
}

void AnimationFrameTimingMonitor::DidFinishSyncXHR(base::TimeDelta blocking_time)
{
    if (pending_script_info_) {
