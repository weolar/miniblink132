...
class RemoteFrameView final : public GarbageCollected<RemoteFrameView>, public FrameView {
public:
    ...
    gfx::Rect GetCompositingRect() const
    {
        return compositing_rect_;
    }

    void UpdateCompositingScaleFactor();
    float GetCompositingScaleFactor() const
    {
    ...
    void ResetFrozenSize()
    {
        frozen_size_ = std::nullopt;
    }

protected:
    bool NeedsViewportOffset() const override
    {
        return true;
    }
...
template <> struct DowncastTraits<RemoteFrameView> {
    static bool AllowFrom(const EmbeddedContentView& embedded_content_view)
    {
        return !embedded_content_view.IsLocalFrameView() && !embedded_content_view.IsPluginView();
    }
};
