...
public:
    ...
    void SetUp() override
    {
        test_task_runner_ = base::MakeRefCounted<base::TestMockTimeTaskRunner>(base::Time::UnixEpoch(), base::TimeTicks::Now());
        RestartAggregator();
    }
    ...
    {
        aggregator().ChooseNextFrameForTest();
    }
...
protected:
    ...
    void VerifyUpdateEntry(unsigned index, unsigned expected_primary_metric, unsigned expected_sub_metric, unsigned expected_begin_main_frame,
        unsigned expected_reasons, bool expected_before_fcp)
    {
        auto entries = recorder().GetEntriesByName("Blink.UpdateTime");
        EXPECT_GT(entries.size(), index);

        auto* entry = entries[index].get();
        EXPECT_TRUE(ukm::TestUkmRecorder::EntryHasMetric(entry, GetPrimaryMetricName()));
        ...
        for (int i = 0; i < LocalFrameUkmAggregator::kForcedStyleAndLayout; ++i) {
            EXPECT_TRUE(ukm::TestUkmRecorder::EntryHasMetric(entry, GetMetricName(i)));
            const int64_t* metric_value = ukm::TestUkmRecorder::GetEntryMetric(entry, GetMetricName(i));
            EXPECT_NEAR(*metric_value, LocalFrameUkmAggregator::ApplyBucketIfNecessary(expected_sub_metric * 1e3, i), 1);

            EXPECT_TRUE(ukm::TestUkmRecorder::EntryHasMetric(entry, GetBeginMainFrameMetricName(i)));
            const int64_t* metric_begin_main_frame = ukm::TestUkmRecorder::GetEntryMetric(entry, GetBeginMainFrameMetricName(i));
            EXPECT_NEAR(*metric_begin_main_frame, LocalFrameUkmAggregator::ApplyBucketIfNecessary(expected_begin_main_frame * 1e3, i), 1);
        }
        EXPECT_TRUE(ukm::TestUkmRecorder::EntryHasMetric(entry, "MainFrameIsBeforeFCP"));
        EXPECT_EQ(expected_before_fcp, *ukm::TestUkmRecorder::GetEntryMetric(entry, "MainFrameIsBeforeFCP"));
        EXPECT_TRUE(ukm::TestUkmRecorder::EntryHasMetric(entry, "MainFrameReasons"));
        EXPECT_EQ(expected_reasons, *ukm::TestUkmRecorder::GetEntryMetric(entry, "MainFrameReasons"));
    }

    void VerifyAggregatedEntries(unsigned expected_num_entries, unsigned expected_primary_metric, unsigned expected_sub_metric)
    {
    ...
    void SimulateFrame(base::TimeTicks start_time, unsigned millisecond_per_step, cc::ActiveFrameSequenceTrackers trackers, bool mark_fcp = false)
    {
        ...
        for (int i = 0; i < LocalFrameUkmAggregator::kForcedStyleAndLayout; ++i) {
            ...
            test_task_runner_->FastForwardBy(base::Milliseconds(millisecond_per_step));
        }
        aggregator().RecordEndOfFrameMetrics(start_time, Now(), trackers, source_id(), &recorder());
    }
    ...
    void SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason reason, LocalFrameUkmAggregator::MetricId target_metric, unsigned expected_num_entries)
    {
        base::TimeTicks start_time = Now();
        aggregator().BeginMainFrame();
        {
            LocalFrameUkmAggregator::ScopedForcedLayoutTimer timer = aggregator().GetScopedForcedLayoutTimer(reason);
            test_task_runner_->FastForwardBy(base::Milliseconds(10));
        }
        aggregator().RecordEndOfFrameMetrics(start_time, Now(), 0, source_id(), &recorder());
        ResetAggregator();
        ...
        EXPECT_TRUE(ukm::TestUkmRecorder::EntryHasMetric(entry, GetMetricName(LocalFrameUkmAggregator::kForcedStyleAndLayout)));
        const int64_t* metric_value = ukm::TestUkmRecorder::GetEntryMetric(entry, GetMetricName(LocalFrameUkmAggregator::kForcedStyleAndLayout));
        EXPECT_NEAR(*metric_value, 10000, 1);

        if (target_metric != LocalFrameUkmAggregator::kCount) {
            EXPECT_TRUE(ukm::TestUkmRecorder::EntryHasMetric(entry, GetMetricName(target_metric)));
            metric_value = ukm::TestUkmRecorder::GetEntryMetric(entry, GetMetricName(target_metric));
            EXPECT_NEAR(*metric_value, 10000, 1);
        }
        for (int i = LocalFrameUkmAggregator::kForcedStyleAndLayout + 1; i < LocalFrameUkmAggregator::kCount; ++i) {
            if (i != target_metric) {
                EXPECT_TRUE(ukm::TestUkmRecorder::EntryHasMetric(entry, GetMetricName(i)));
                metric_value = ukm::TestUkmRecorder::GetEntryMetric(entry, GetMetricName(i));
    ...
    {
        return aggregator().current_sample_.sub_metrics_counts[0] / 1000 == iteration_count;
    }
...
TEST_F(LocalFrameUkmAggregatorTest, FirstFrameIsRecorded)
{
    ...
    float expected_primary_metric = millisecond_for_step * LocalFrameUkmAggregator::kForcedStyleAndLayout;
    float expected_sub_metric = millisecond_for_step;
    float expected_begin_main_frame = millisecond_for_step;

    VerifyUpdateEntry(0u, expected_primary_metric, expected_sub_metric, expected_begin_main_frame, 12, true);
}
...
TEST_F(LocalFrameUkmAggregatorTest, PreFrameWorkIsRecorded)
{
    ...
    // for the initial frame, regardless of the initial interval.
    unsigned millisecond_for_step = 1;
    base::TimeTicks start_time = Now() + base::Milliseconds(millisecond_for_step) * LocalFrameUkmAggregator::kForcedStyleAndLayout;
...
TEST_F(LocalFrameUkmAggregatorTest, PreAndPostFCPAreRecorded)
{
    ...
    // for the initial frame, regardless of the initial interval.
    base::TimeTicks start_time = Now();
    unsigned millisecond_per_step = 50 / (LocalFrameUkmAggregator::kForcedStyleAndLayout + 1);
    ...
    float expected_primary_metric = millisecond_per_step * LocalFrameUkmAggregator::kForcedStyleAndLayout;
    float expected_sub_metric = millisecond_per_step;
    float expected_begin_main_frame = millisecond_per_step;

    VerifyUpdateEntry(0u, expected_primary_metric, expected_sub_metric, expected_begin_main_frame, 4, true);
    ...
    unsigned millisecond_per_frame = millisecond_per_step * LocalFrameUkmAggregator::kForcedStyleAndLayout;
    ...
    VerifyUpdateEntry(1u, millisecond_per_frame, millisecond_per_step, expected_begin_main_frame, 4, false);
}
...
TEST_F(LocalFrameUkmAggregatorTest, AggregatedPreFCPEventRecorded)
{
    ...
    EXPECT_EQ(recorder().entries_count(), 2u);
    VerifyAggregatedEntries(1u, 2 * millisecond_per_frame, 2 * millisecond_per_step);
    ResetAggregator();
}
...
TEST_F(LocalFrameUkmAggregatorTest, ForcedLayoutReasonsReportOnlyMetric)
{
    ...
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kContextMenu, LocalFrameUkmAggregator::kUserDrivenDocumentUpdate, 1u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kEditing, LocalFrameUkmAggregator::kUserDrivenDocumentUpdate, 2u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kEditing, LocalFrameUkmAggregator::kUserDrivenDocumentUpdate, 3u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kFindInPage, LocalFrameUkmAggregator::kUserDrivenDocumentUpdate, 4u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kFocus, LocalFrameUkmAggregator::kUserDrivenDocumentUpdate, 5u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kForm, LocalFrameUkmAggregator::kUserDrivenDocumentUpdate, 6u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kInput, LocalFrameUkmAggregator::kUserDrivenDocumentUpdate, 7u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kInspector, LocalFrameUkmAggregator::kUserDrivenDocumentUpdate, 8u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kPrinting, LocalFrameUkmAggregator::kUserDrivenDocumentUpdate, 9u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kSelection, LocalFrameUkmAggregator::kUserDrivenDocumentUpdate, 10u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kSpatialNavigation, LocalFrameUkmAggregator::kUserDrivenDocumentUpdate, 11u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kTapHighlight, LocalFrameUkmAggregator::kUserDrivenDocumentUpdate, 12u);

    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kAccessibility, LocalFrameUkmAggregator::kServiceDocumentUpdate, 13u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kBaseColor, LocalFrameUkmAggregator::kServiceDocumentUpdate, 14u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kDisplayLock, LocalFrameUkmAggregator::kServiceDocumentUpdate, 15u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kIntersectionObservation, LocalFrameUkmAggregator::kServiceDocumentUpdate, 16u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kOverlay, LocalFrameUkmAggregator::kServiceDocumentUpdate, 17u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kPagePopup, LocalFrameUkmAggregator::kServiceDocumentUpdate, 18u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kSizeChange, LocalFrameUkmAggregator::kServiceDocumentUpdate, 19u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kSpellCheck, LocalFrameUkmAggregator::kServiceDocumentUpdate, 20u);

    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kCanvas, LocalFrameUkmAggregator::kContentDocumentUpdate, 21u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kPlugin, LocalFrameUkmAggregator::kContentDocumentUpdate, 22u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kSVGImage, LocalFrameUkmAggregator::kContentDocumentUpdate, 23u);

    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kHitTest, LocalFrameUkmAggregator::kHitTestDocumentUpdate, 24u);

    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kJavaScript, LocalFrameUkmAggregator::kJavascriptDocumentUpdate, 25u);

    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kBeginMainFrame, LocalFrameUkmAggregator::kCount, 26u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kTest, LocalFrameUkmAggregator::kCount, 27u);
    SimulateAndVerifyForcedLayoutReason(DocumentUpdateReason::kUnknown, LocalFrameUkmAggregator::kCount, 28u);
}
...
TEST_F(LocalFrameUkmAggregatorTest, LatencyDataIsPopulated)
{
    ...
    aggregator().BeginMainFrame();
    for (int i = 0; i < LocalFrameUkmAggregator::kForcedStyleAndLayout; ++i) {
        auto timer = aggregator().GetScopedTimer(i % LocalFrameUkmAggregator::kForcedStyleAndLayout);
        test_task_runner_->FastForwardBy(base::Milliseconds(millisecond_for_step));
    }

    std::unique_ptr<cc::BeginMainFrameMetrics> metrics_data = aggregator().GetBeginMainFrameMetrics();
    EXPECT_EQ(metrics_data->handle_input_events.InMillisecondsF(), millisecond_for_step);
...
TEST_F(LocalFrameUkmAggregatorTest, IntersectionObserverSamplePeriod)
{
    ...
    cc::ActiveFrameSequenceTrackers trackers = 1 << static_cast<unsigned>(cc::FrameSequenceTrackerType::kSETMainThreadAnimation);
    ...
    aggregator().BeginMainFrame();
    {
        ...
        timer.StartInterval(LocalFrameUkmAggregator::kDisplayLockIntersectionObserver);
        test_task_runner_->FastForwardBy(base::Milliseconds(1));
    }
    aggregator().RecordEndOfFrameMetrics(start_time, Now(), trackers, source_id(), &recorder());
    histogram_tester.ExpectUniqueSample("Blink.Layout.UpdateTime.PreFCP", 1000, 1);
    histogram_tester.ExpectUniqueSample("Blink.DisplayLockIntersectionObserver.UpdateTime.PreFCP", 1000, 1);
    ...
    aggregator().RecordEndOfFrameMetrics(start_time, Now(), trackers, source_id(), &recorder());
    histogram_tester.ExpectUniqueSample("Blink.Layout.UpdateTime.PreFCP", 1000, 2);
    histogram_tester.ExpectUniqueSample("Blink.DisplayLockIntersectionObserver.UpdateTime.PreFCP", 1000, 1);
    ...
    aggregator().RecordEndOfFrameMetrics(start_time, Now(), trackers, source_id(), &recorder());
    histogram_tester.ExpectUniqueSample("Blink.Layout.UpdateTime.PreFCP", 1000, 3);
    histogram_tester.ExpectUniqueSample("Blink.DisplayLockIntersectionObserver.UpdateTime.PreFCP", 1000, 2);
}
...
protected:
    ...
    void TestIntersectionObserverCounts(Document& document)
    {
        ...
        observer_init->setRoot(MakeGarbageCollected<V8UnionDocumentOrElement>(&document));
        TestIntersectionObserverDelegate* internal_delegate = MakeGarbageCollected<TestIntersectionObserverDelegate>(document);
        IntersectionObserver* internal_observer
            = IntersectionObserver::Create(observer_init, *internal_delegate, LocalFrameUkmAggregator::kLazyLoadIntersectionObserver);
        DCHECK(!Compositor().NeedsBeginFrame());
        internal_observer->observe(target1);
        internal_observer->observe(target2);
        Compositor().BeginFrame();
        EXPECT_EQ(histogram_tester.GetTotalSum("Blink.IntersectionObservationInternalCount.UpdateTime.PreFCP"), 2);
        EXPECT_EQ(histogram_tester.GetTotalSum("Blink.IntersectionObservationJavascriptCount.UpdateTime.PreFCP"), 0);

        TestIntersectionObserverDelegate* javascript_delegate = MakeGarbageCollected<TestIntersectionObserverDelegate>(document);
        IntersectionObserver* javascript_observer
            = IntersectionObserver::Create(observer_init, *javascript_delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver);
        javascript_observer->observe(target1);
        javascript_observer->observe(target2);
        Compositor().BeginFrame();
        EXPECT_EQ(histogram_tester.GetTotalSum("Blink.IntersectionObservationInternalCount.UpdateTime.PreFCP"), 4);
        EXPECT_EQ(histogram_tester.GetTotalSum("Blink.IntersectionObservationJavascriptCount.UpdateTime.PreFCP"), 2);
        ...
        document.View()->GetUkmAggregator()->RecordEndOfFrameMetrics(
            base::TimeTicks(), base::TimeTicks() + base::Microseconds(10), 0, root_document->UkmSourceID(), root_document->UkmRecorder());

        target1->setAttribute(html_names::kStyleAttr, AtomicString("height: 60px"));
        Compositor().BeginFrame();
        EXPECT_EQ(histogram_tester.GetTotalSum("Blink.IntersectionObservationInternalCount.UpdateTime.PreFCP"), 4);
        EXPECT_EQ(histogram_tester.GetTotalSum("Blink.IntersectionObservationJavascriptCount.UpdateTime.PreFCP"), 2);
        EXPECT_EQ(histogram_tester.GetTotalSum("Blink.IntersectionObservationInternalCount.UpdateTime.PostFCP"), 2);
        EXPECT_EQ(histogram_tester.GetTotalSum("Blink.IntersectionObservationJavascriptCount.UpdateTime.PostFCP"), 2);
    }
...
TEST_F(LocalFrameUkmAggregatorSimTest, GetUkmAggregator)
{
    ...
    auto* subframe_view = To<HTMLFrameOwnerElement>(GetDocument().getElementById(AtomicString("frame")))->contentDocument()->View();
...
{
    std::unique_ptr<base::StatisticsRecorder> statistics_recorder = base::StatisticsRecorder::CreateTemporaryForTesting();
...
TEST_F(LocalFrameUkmAggregatorSimTest, IntersectionObserverCountsInChildFrame)
{
    ...
    Compositor().BeginFrame();
    ChooseNextFrameForTest();
    TestIntersectionObserverCounts(*To<HTMLFrameOwnerElement>(GetDocument().getElementById(AtomicString("frame")))->contentDocument());
}
...
TEST_F(LocalFrameUkmAggregatorSimTest, PrePostFCPMetricsWithChildFrameFCP)
{
    ...
    auto* subframe_document = To<HTMLFrameOwnerElement>(GetDocument().getElementById(AtomicString("frame")))->contentDocument();
...
TEST_F(LocalFrameUkmAggregatorSimTest, VisualUpdateDelay)
{
    ...
    Compositor().BeginFrame();
    histogram_tester.ExpectTotalCount("Blink.VisualUpdateDelay.UpdateTime.PreFCP", 0);
    ...
    Compositor().BeginFrame();
    histogram_tester.ExpectTotalCount("Blink.VisualUpdateDelay.UpdateTime.PreFCP", 1);
    base::HistogramBase::Sample delay = base::saturated_cast<base::HistogramBase::Sample>(
        (Compositor().LastFrameTime() - local_root_aggregator().LastFrameRequestTimeForTest()).InMicroseconds());
    EXPECT_GT(delay, 3000);
    histogram_tester.ExpectUniqueSample("Blink.VisualUpdateDelay.UpdateTime.PreFCP", delay, 1);
}
...
using SyncScrollHeuristicTestConfig = ::testing::tuple<SyncScrollMutation, SyncScrollPositionAccess, SyncScrollHandlerStrategy>;

class LocalFrameUkmAggregatorSyncScrollTest : public LocalFrameUkmAggregatorSimTest, public ::testing::WithParamInterface<SyncScrollHeuristicTestConfig> {
public:
    static std::string PrintTestName(const ::testing::TestParamInfo<SyncScrollHeuristicTestConfig>& info)
    {
        std::stringstream ss;
...
protected:
    static SyncScrollMutation GetSyncScrollMutation(const SyncScrollHeuristicTestConfig& config)
    {
        return ::testing::get<0>(config);
    }

    static SyncScrollPositionAccess GetSyncScrollPositionAccess(const SyncScrollHeuristicTestConfig& config)
    {
        return ::testing::get<1>(config);
    }

    static SyncScrollHandlerStrategy GetSyncScrollHandlerStrategy(const SyncScrollHeuristicTestConfig& config)
    {
    ...
    bool ShouldTriggerSyncScrollHeuristic() const
    {
        ...
        return GetSyncScrollMutation(GetParam()) != SyncScrollMutation::kSyncScrollMutatesNothing
            && GetSyncScrollMutation(GetParam()) != SyncScrollMutation::kSyncScrollMutatesPositionBeforeAccess
            && GetSyncScrollPositionAccess(GetParam()) == SyncScrollPositionAccess::kSyncScrollAccessScrollOffset
            && (GetSyncScrollHandlerStrategy(GetParam()) == SyncScrollHandlerStrategy::kSyncScrollWithEventHandler
                || GetSyncScrollHandlerStrategy(GetParam()) == SyncScrollHandlerStrategy::kSyncScrollWithEventHandlerSchedulingRAF);
    }
    ...
    std::string GenerateMutation()
    {
        ...
        case SyncScrollMutation::kSyncScrollMutatesTransform:
            return base::StringPrintf("card.style.transform = 'translateY(' + %s + 'px)'", pos.c_str());
        case SyncScrollMutation::kSyncScrollMutatesScrollOffset:
            return base::StringPrintf("subscroller.scrollTop = %s + 'px'", pos.c_str());
        case SyncScrollMutation::kSyncScrollMutatesPositionBeforeAccess:
            return base::StringPrintf("card.style.top = Math.floor(Math.random() * 100) + 'px'; var "
                                      "unused = %s",
                pos.c_str());
        case SyncScrollMutation::kSyncScrollMutatesNothing:
            return "";
    ...
    ScopedTestingPlatformSupport<TestingPlatformSupportWithMockScheduler> platform_;
};
...
TEST_P(LocalFrameUkmAggregatorSyncScrollTest, SyncScrollHeuristicRAFSetTop)
{
    ...
    histogram_tester.ExpectTotalCount("Blink.PossibleSynchronizedScrollCount2.UpdateTime.PreFCP", 0);

    // Cause a pre-FCP scroll.
    auto* scrolling_element = LocalFrameRoot().GetFrame()->GetDocument()->scrollingElement();
    ...
    // are met.
    histogram_tester.ExpectTotalCount("Blink.PossibleSynchronizedScrollCount2.UpdateTime.PreFCP", should_trigger ? 1 : 0);
    ...
    scrolling_element = LocalFrameRoot().GetFrame()->GetDocument()->scrollingElement();
    ...
    if (should_trigger) {
        ...
        EXPECT_THAT(histogram_tester.GetAllSamples("Blink.PossibleSynchronizedScrollCount2."
                                                   "UpdateTime.AggregatedPreFCP"),
            base::BucketsAre(base::Bucket(1, 1)));
        // Should only have triggered for the one post FCP scroll.
        histogram_tester.ExpectTotalCount("Blink.PossibleSynchronizedScrollCount2.UpdateTime.PostFCP", 1);
    } else {
        // Should never trigger.
        EXPECT_THAT(histogram_tester.GetAllSamples("Blink.PossibleSynchronizedScrollCount2."
                                                   "UpdateTime.AggregatedPreFCP"),
            base::BucketsAre(base::Bucket(0, 1)));
        histogram_tester.ExpectTotalCount("Blink.PossibleSynchronizedScrollCount2.UpdateTime.PostFCP", 0);
    }
}

INSTANTIATE_TEST_SUITE_P(P, LocalFrameUkmAggregatorSyncScrollTest,
    ::testing::Combine(::testing::Values(SyncScrollMutation::kSyncScrollMutatesPosition, SyncScrollMutation::kSyncScrollMutatesTransform,
                           SyncScrollMutation::kSyncScrollMutatesScrollOffset, SyncScrollMutation::kSyncScrollMutatesPositionBeforeAccess,
                           SyncScrollMutation::kSyncScrollMutatesNothing),
        ::testing::Values(SyncScrollPositionAccess::kSyncScrollAccessScrollOffset, SyncScrollPositionAccess::kSyncScrollDoesNotAccessScrollOffset),
        ::testing::Values(SyncScrollHandlerStrategy::kSyncScrollWithEventHandler, SyncScrollHandlerStrategy::kSyncScrollWithEventHandlerSchedulingRAF,
            SyncScrollHandlerStrategy::kSyncScrollNoEventHandlerWithRAF, SyncScrollHandlerStrategy::kSyncScrollNoEventHandler)),
