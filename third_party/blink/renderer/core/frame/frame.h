...
public:
    ...
    {
        DCHECK(IsMainFrame());
        DCHECK_EQ(network::mojom::blink::WebSandboxFlags::kNone, opener_sandbox_flags_);
    ...
    void SetRequiredDocumentPolicy(const DocumentPolicyFeatureState& required_document_policy)
    {
    ...
    const FrameToken& GetFrameToken() const
    {
        return frame_token_;
    }

    bool GetVisibleToHitTesting() const
    {
        return visible_to_hit_testing_;
    }
    void UpdateVisibleToHitTesting();

    base::OnceClosure ScheduleFormSubmission(FrameScheduler* scheduler, FormSubmission* form_submission);
    ...
    // WebLocalFrame uses.
    bool Swap(WebRemoteFrame*, mojo::PendingAssociatedRemote<mojom::blink::RemoteFrameHost> remote_frame_host,
        mojo::PendingAssociatedReceiver<mojom::blink::RemoteFrame> remote_frame_receiver);

    // Removes the given child from this frame.
    void RemoveChild(Frame* child);

    LocalFrame* ProvisionalFrame() const
    {
        return provisional_frame_.Get();
    }
    ...
    // on a detached frame.
    std::optional<blink::FencedFrame::DeprecatedFencedFrameMode> GetDeprecatedFencedFrameMode() const;
...
protected:
    ...
    // created.
    Frame(FrameClient*, Page&, FrameOwner*, Frame* parent, Frame* previous_sibling, FrameInsertType insert_type, const FrameToken& frame_token,
        const base::UnguessableToken& devtools_frame_token, WindowProxyManager*, WindowAgentFactory* inheriting_agent_factory);
    ...
    void ApplyFrameOwnerProperties(mojom::blink::FrameOwnerPropertiesPtr properties);

    void NotifyUserActivationInFrameTreeStickyOnly();
    void NotifyUserActivationInFrameTree(mojom::blink::UserActivationNotificationType notification_type, bool sticky_only = false);
...
private:
    ...
    bool SwapImpl(WebFrame*, mojo::PendingAssociatedRemote<mojom::blink::RemoteFrameHost> remote_frame_host,
        mojo::PendingAssociatedReceiver<mojom::blink::RemoteFrame> remote_frame_receiver);
    ...
    static void NotifyUserActivationInFrame(Frame* node, mojom::blink::UserActivationNotificationType notification_type, bool sticky_only);
