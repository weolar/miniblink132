...
public:
    ...
    void PaintFrameOverlay(const FrameOverlay& frame_overlay, GraphicsContext& graphics_context, const gfx::Size& size) const override
    {
        ...
        DrawingRecorder recorder(graphics_context, frame_overlay, DisplayItem::kFrameOverlay, gfx::Rect(size));
...
protected:
    ...
    FrameOverlayTest()
    {
        ...
        GetWebView()->MainFrameViewWidget()->Resize(gfx::Size(kViewportWidth, kViewportHeight));
    ...
    FrameOverlay* CreateSolidYellowOverlay()
    {
        return MakeGarbageCollected<FrameOverlay>(
            GetWebView()->MainFrameImpl()->GetFrame(), std::make_unique<SolidColorOverlay>(Color::FromSkColor(SK_ColorYELLOW)));
    }

    template <typename OverlayType> void RunFrameOverlayTestWithAcceleratedCompositing();
...
TEST_P(FrameOverlayTest, AcceleratedCompositing)
{
    ...
    EXPECT_EQ(PropertyTreeState::Root(), frame_overlay->DefaultPropertyTreeState());
    ...
    EXPECT_CALL(canvas, onDrawRect(SkRect::MakeWH(kViewportWidth, kViewportHeight), Property(&SkPaint::getColor, SK_ColorYELLOW)));
...
TEST_P(FrameOverlayTest, DeviceEmulationScale)
{
    ...
    FrameOverlay* frame_overlay = CreateSolidYellowOverlay();
    frame_overlay->UpdatePrePaint();
    auto* transform = GetWebView()->MainFrameImpl()->GetFrame()->GetPage()->GetVisualViewport().GetDeviceEmulationTransformNode();
    ...
    EXPECT_THAT(paint_artifact.GetDisplayItemList(), ElementsAre(IsSameId(frame_overlay->Id(), DisplayItem::kFrameOverlay)));
    EXPECT_EQ(gfx::Rect(0, 0, 800, 600), paint_artifact.GetDisplayItemList()[0].VisualRect());
    EXPECT_THAT(paint_artifact.GetPaintChunks(),
        ElementsAre(IsPaintChunk(0, 1, PaintChunk::Id(frame_overlay->Id(), DisplayItem::kFrameOverlay), state, nullptr, gfx::Rect(0, 0, 800, 600))));
    frame_overlay->Destroy();
}
