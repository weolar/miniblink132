...
public:
    ...
    virtual bool UpdateViewportIntersectionsForSubtree(unsigned parent_flags, ComputeIntersectionsContext&) = 0;
    ...
    bool IsFrameView() const override
    {
        return true;
    }
    virtual bool ShouldReportMainFrameIntersection() const
    {
        return false;
    }
    ...
    virtual void UpdateRenderThrottlingStatus(bool hidden_for_throttling, bool subtree_throttled, bool display_locked, bool recurse = false);
...
protected:
    virtual bool NeedsViewportOffset() const
    {
        return false;
    }
    virtual void SetViewportIntersection(const mojom::blink::ViewportIntersectionState& intersection_state) = 0;
    virtual void VisibilityForThrottlingChanged() = 0;
    virtual bool LifecycleUpdatesThrottled() const
    {
        return false;
    }
    void UpdateViewportIntersection(unsigned flags, bool needs_occlusion_tracking);
...
template <> struct DowncastTraits<FrameView> {
