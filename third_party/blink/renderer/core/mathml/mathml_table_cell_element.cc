...
{
    const AtomicString& col_span_value = FastGetAttribute(mathml_names::kColumnspanAttr);
    unsigned value = 0;
...
{
    const AtomicString& row_span_value = FastGetAttribute(mathml_names::kRowspanAttr);
    unsigned value = 0;
...
void MathMLTableCellElement::ParseAttribute(const AttributeModificationParams& params)
{
    if (params.name == mathml_names::kRowspanAttr || params.name == mathml_names::kColumnspanAttr) {
...
LayoutObject* MathMLTableCellElement::CreateLayoutObject(const ComputedStyle& style)
{
