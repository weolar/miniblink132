...
TEST(MathMLTableCellElementTest, colSpan_parsing)
{
    ...
    for (unsigned colSpan : { 1, 2, 16, 256, 999, 1000 }) {
        cell->setAttribute(mathml_names::kColumnspanAttr, AtomicString::Number(colSpan));
        EXPECT_EQ(colSpan, cell->colSpan()) << "valid columnspan value '" << colSpan << "' is properly parsed";
    }
    ...
    EXPECT_EQ(1000u, cell->colSpan()) << "columnspan is clamped to max value 1000";
...
TEST(MathMLTableCellElementTest, rowspan_parsing)
{
    ...
    for (unsigned rowspan : { 0, 1, 16, 256, 4096, 65533, 65534 }) {
        cell->setAttribute(mathml_names::kRowspanAttr, AtomicString::Number(rowspan));
        EXPECT_EQ(rowspan, cell->rowSpan()) << "valid rowspan value '" << rowspan << "' is properly parsed";
    }
