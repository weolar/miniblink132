...
enum class MathScriptType { kSub, kSuper, kSubSup, kMultiscripts, kUnder, kOver, kUnderOver };

class CORE_EXPORT MathMLScriptsElement : public MathMLElement {
public:
    MathScriptType GetScriptType() const
    {
        return script_type_;
    }
...
template <> struct DowncastTraits<MathMLScriptsElement> {
    ...
    {
        return mathml_element.HasTagName(mathml_names::kMunderTag) || mathml_element.HasTagName(mathml_names::kMoverTag)
            || mathml_element.HasTagName(mathml_names::kMunderoverTag) || mathml_element.HasTagName(mathml_names::kMsubTag)
            || mathml_element.HasTagName(mathml_names::kMsupTag) || mathml_element.HasTagName(mathml_names::kMsubsupTag)
