...
template <> struct DowncastTraits<MathMLTokenElement> {
    ...
    {
        return mathml_element.HasTagName(mathml_names::kMiTag) || mathml_element.HasTagName(mathml_names::kMoTag)
            || mathml_element.HasTagName(mathml_names::kMnTag) || mathml_element.HasTagName(mathml_names::kMtextTag)
