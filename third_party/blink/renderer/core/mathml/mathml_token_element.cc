...
UChar32 TokenCodePoint(const String& text_content)
{
    ...
    // codepoint.
    if ((content_length > 2) || (content_length == 0) || (content_length == 1 && !U16_IS_SINGLE(text_content[0]))
...
bool MathMLTokenElement::IsPresentationAttribute(const QualifiedName& name) const
{
    if (Node::HasTagName(mathml_names::kMiTag) && name == mathml_names::kMathvariantAttr) {
        return true;
    }
    return MathMLElement::IsPresentationAttribute(name);
}

void MathMLTokenElement::CollectStyleForPresentationAttribute(const QualifiedName& name, const AtomicString& value, MutableCSSPropertyValueSet* style)
{
    if (name == mathml_names::kMathvariantAttr && EqualIgnoringASCIICase(value, "normal")) {
        AddPropertyToPresentationAttributeStyle(style, CSSPropertyID::kTextTransform, CSSValueID::kNone);
    } else {
...
void MathMLTokenElement::ChildrenChanged(const ChildrenChange& children_change)
{
    token_content_ = std::nullopt;
