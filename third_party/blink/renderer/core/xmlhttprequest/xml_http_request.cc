...
void FindCharsetInMediaType(const String& media_type, unsigned& charset_pos, unsigned& charset_len)
{
    charset_len = 0;
    ...
    while (pos < length && (media_type[pos] <= ' ' || media_type[pos] == '"' || media_type[pos] == '\''))
        ++pos;
    ...
    while (pos < length && media_type[pos] > ' ' && media_type[pos] != '"' && media_type[pos] != '\'' && media_type[pos] != ';')
        ++pos;
...
void LogConsoleError(ExecutionContext* context, const String& message)
{
    ...
    auto* console_message = MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kJavaScript, mojom::ConsoleMessageLevel::kError, message);
    context->AddConsoleMessage(console_message);
}

bool ValidateOpenArguments(const AtomicString& method, const KURL& url, ExceptionState& exception_state)
{
    if (!IsValidHTTPToken(method)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "'" + method + "' is not a valid HTTP method.");
        return false;
    }

    if (FetchUtils::IsForbiddenMethod(method)) {
        exception_state.ThrowSecurityError("'" + method + "' HTTP method is unsupported.");
        return false;
    }

    if (!url.IsValid()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "Invalid URL");
        return false;
    }
...
class XMLHttpRequest::BlobLoader final : public GarbageCollected<XMLHttpRequest::BlobLoader>, public FileReaderClient {
public:
    BlobLoader(XMLHttpRequest* xhr, scoped_refptr<BlobDataHandle> handle)
        : xhr_(xhr)
        , loader_(MakeGarbageCollected<FileReaderLoader>(this, xhr->GetExecutionContext()->GetTaskRunner(TaskType::kFileReading)))
    {
...
{
    return MakeGarbageCollected<XMLHttpRequest>(ExecutionContext::From(script_state), &script_state->World());
}
...
XMLHttpRequest::XMLHttpRequest(ExecutionContext* context, const DOMWrapperWorld* world)
    : ActiveScriptWrappable<XMLHttpRequest>({})
    , ExecutionContextLifecycleObserver(context)
    , progress_event_throttle_(MakeGarbageCollected<XMLHttpRequestProgressEventThrottle>(this))
    , world_(world)
    , isolated_world_security_origin_(world_ && world_->IsIsolatedWorld() ? world_->IsolatedWorldSecurityOrigin(context->GetAgentClusterID()) : nullptr)
{
}
...
{
    if (response_type_code_ != kResponseTypeDefault && response_type_code_ != V8XMLHttpRequestResponseType::Enum::kText) {
...
void XMLHttpRequest::InitResponseDocument()
{
    ...
    if ((response_.IsHTTP() && !ResponseIsXML() && !is_html) || (is_html && response_type_code_ == kResponseTypeDefault) || !GetExecutionContext()
    ...
    DocumentInit init
        = DocumentInit::Create().WithExecutionContext(GetExecutionContext()).WithAgent(*GetExecutionContext()->GetAgent()).WithURL(response_.ResponseUrl());
    if (is_html) {
...
{
    if (response_type_code_ != kResponseTypeDefault && response_type_code_ != V8XMLHttpRequestResponseType::Enum::kDocument) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError,
            "The value is only accessible if the "
    ...
    if (!parsed_response_) {
        ...
        if (!response_document_->WellFormed()) {
            ...
            response_document_->OverrideLastModified(response_.HttpHeaderField(http_names::kLastModified));
        }
...
v8::Local<v8::Value> XMLHttpRequest::ResponseJSON(ScriptState* script_state)
{
    ...
    v8::Local<v8::Value> json = FromJSONString(script_state, response_text_.ToString());
...
Blob* XMLHttpRequest::ResponseBlob()
{
    ...
    if (!response_blob_) {
        ...
        response_blob_ = MakeGarbageCollected<Blob>(BlobDataHandle::Create(std::move(blob_data), size));
    }
...
{
    DCHECK_EQ(response_type_code_, V8XMLHttpRequestResponseType::Enum::kArraybuffer);
    DCHECK(!error_);
    DCHECK_EQ(state_, kDone);

    if (!response_array_buffer_ && !response_array_buffer_failure_) {
        if (binary_response_builder_ && binary_response_builder_->size()) {
            DOMArrayBuffer* buffer = DOMArrayBuffer::CreateUninitializedOrNull(binary_response_builder_->size(), 1);
            if (buffer) {
...
v8::Local<v8::Value> XMLHttpRequest::response(ScriptState* script_state)
{
    ...
    if (response_type_code_ == kResponseTypeDefault || response_type_code_ == V8XMLHttpRequestResponseType::Enum::kText) {
        return ToV8Traits<IDLString>::ToV8(script_state, responseText(ASSERT_NO_EXCEPTION));
    }
    ...
    case V8XMLHttpRequestResponseType::Enum::kDocument: {
        return ToV8Traits<IDLNullable<Document>>::ToV8(script_state, responseXML(ASSERT_NO_EXCEPTION));
    }
    ...
    case V8XMLHttpRequestResponseType::Enum::kArraybuffer:
        return ToV8Traits<IDLNullable<DOMArrayBuffer>>::ToV8(script_state, ResponseArrayBuffer());
    default:
        NOTREACHED();
    }
}

void XMLHttpRequest::setTimeout(unsigned timeout, ExceptionState& exception_state)
{
...
void XMLHttpRequest::setResponseType(const V8XMLHttpRequestResponseType& response_type, ExceptionState& exception_state)
{
    const bool is_window = GetExecutionContext() && GetExecutionContext()->IsWindow();
...
void XMLHttpRequest::DispatchReadyStateChangeEvent()
{
    ...
    if (async_ || (state_ <= kOpened || state_ == kDone)) {
        DEVTOOLS_TIMELINE_TRACE_EVENT("XHRReadyStateChange", inspector_xhr_ready_state_change_event::Data, GetExecutionContext(), this);
        XMLHttpRequestProgressEventThrottle::DeferredEventAction action = XMLHttpRequestProgressEventThrottle::kIgnore;
        if (state_ == kDone) {
            if (error_)
        ...
        std::optional<scheduler::TaskAttributionTracker::TaskScope> task_attribution_scope = MaybeCreateTaskAttributionScope();
        progress_event_throttle_->DispatchReadyStateChangeEvent(Event::Create(event_type_names::kReadystatechange), action);
    }

    if (state_ == kDone && !error_) {
        DEVTOOLS_TIMELINE_TRACE_EVENT("XHRLoad", inspector_xhr_load_event::Data, GetExecutionContext(), this);
        DispatchProgressEventFromSnapshot(event_type_names::kLoad);
        DispatchProgressEventFromSnapshot(event_type_names::kLoadend);
    }
}

void XMLHttpRequest::setWithCredentials(bool value, ExceptionState& exception_state)
{
    if (state_ > kOpened || send_flag_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The value may only be set if the object's state is UNSENT or OPENED.");
        return;
    }
...
void XMLHttpRequest::open(const AtomicString& method, const String& url_string, ExceptionState& exception_state)
{
    if (!GetExecutionContext())
        return;
...
void XMLHttpRequest::open(
    const AtomicString& method, const String& url_string, bool async, const String& username, const String& password, ExceptionState& exception_state)
{
    if (!GetExecutionContext())
        return;
...
void XMLHttpRequest::open(const AtomicString& method, const KURL& url, bool async, ExceptionState& exception_state)
{
    DVLOG(1) << this << " open(" << method << ", " << url.ElidedString() << ", " << async << ")";
    ...
    if (!async && window) {
        if (window->GetFrame() && !window->GetFrame()->GetSettings()->GetSyncXHRInDocumentsEnabled()) {
            exception_state.ThrowDOMException(DOMExceptionCode::kInvalidAccessError, "Synchronous requests are disabled for this page.");
            return;
        }
        ...
        if (response_type_code_ != V8XMLHttpRequestResponseType::Enum::k) {
            exception_state.ThrowDOMException(DOMExceptionCode::kInvalidAccessError, "Synchronous requests from a document must not set a response type.");
            return;
        }

        // Similarly, timeouts are disabled for synchronous requests as well.
        if (!timeout_.is_zero()) {
            exception_state.ThrowDOMException(DOMExceptionCode::kInvalidAccessError, "Synchronous requests must not set a timeout.");
            return;
        }
        ...
        if (!window->document()->ProcessingBeforeUnload()) {
            Deprecation::CountDeprecation(GetExecutionContext(), WebFeature::kXMLHttpRequestSynchronousInNonWorkerOutsideBeforeUnload);
        }
    }
    ...
    if (url_.ProtocolIs("blob")) {
        GetExecutionContext()->GetPublicURLManager().Resolve(url_, blob_url_loader_factory_.InitWithNewPipeAndPassReceiver());
    }
...
bool XMLHttpRequest::InitSend(ExceptionState& exception_state)
{
    ...
    if (!GetExecutionContext() || GetExecutionContext()->IsContextDestroyed()) {
        HandleNetworkError();
        ThrowForLoadFailureIfNeeded(exception_state, "Document is already detached.");
        return false;
    }

    if (state_ != kOpened || send_flag_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The object's state must be OPENED.");
        return false;
    }

    if (!async_) {
        if (GetExecutionContext()->IsWindow()) {
            bool sync_xhr_disabled_by_permissions_policy = !GetExecutionContext()->IsFeatureEnabled(
                mojom::blink::PermissionsPolicyFeature::kSyncXHR, ReportOptions::kReportOnFailure, "Synchronous requests are disabled by permissions policy.");

            bool sync_xhr_disabled_by_document_policy = !GetExecutionContext()->IsFeatureEnabled(
                mojom::blink::DocumentPolicyFeature::kSyncXHR, ReportOptions::kReportOnFailure, "Synchronous requests are disabled by document policy.");
            ...
            if (sync_xhr_disabled_by_permissions_policy || sync_xhr_disabled_by_document_policy) {
                HandleNetworkError();
        ...
        v8::MicrotaskQueue* microtask_queue = ToMicrotaskQueue(GetExecutionContext());
        if (isolate
            && ((microtask_queue && microtask_queue->IsRunningMicrotasks()) || (!microtask_queue && v8::MicrotasksScope::IsRunningMicrotasks(isolate)))) {
            UseCounter::Count(GetExecutionContext(), WebFeature::kDuring_Microtask_SyncXHR);
        }
    }
...
void XMLHttpRequest::send(const V8UnionDocumentOrXMLHttpRequestBodyInit* body, ExceptionState& exception_state)
{
...
{
    return method_ != http_names::kGET && method_ != http_names::kHEAD && SchemeRegistry::ShouldTreatURLSchemeAsSupportingFetchAPI(url_.Protocol());
}
...
void XMLHttpRequest::send(Document* document, ExceptionState& exception_state)
{
    ...
    if (AreMethodAndURLValidForSend()) {
        if (IsA<HTMLDocument>(document)) {
            UpdateContentTypeAndCharset(AtomicString("text/html;charset=UTF-8"), "UTF-8");
        } else if (IsA<XMLDocument>(document)) {
            UpdateContentTypeAndCharset(AtomicString("application/xml;charset=UTF-8"), "UTF-8");
        }

        String body = CreateMarkup(document);

        http_body = EncodedFormData::Create(UTF8Encoding().Encode(body, WTF::kNoUnencodables));
    }
...
void XMLHttpRequest::send(const String& body, ExceptionState& exception_state)
{
    ...
    if (!body.IsNull() && AreMethodAndURLValidForSend()) {
        http_body = EncodedFormData::Create(UTF8Encoding().Encode(body, WTF::kNoUnencodables));
        UpdateContentTypeAndCharset(AtomicString("text/plain;charset=UTF-8"), "UTF-8");
    }
    ...
    if (AreMethodAndURLValidForSend()) {
        if (!HasContentTypeRequestHeader()) {
            ...
            if (!blob_type.empty() && ParsedContentType(blob_type).IsValid()) {
                SetRequestHeaderInternal(http_names::kContentType, AtomicString(blob_type));
            }
        }
...
void XMLHttpRequest::send(FormData* body, ExceptionState& exception_state)
{
    ...
    if (AreMethodAndURLValidForSend()) {
        ...
        if (!HasContentTypeRequestHeader()) {
            AtomicString content_type = AtomicString("multipart/form-data; boundary=") + FetchUtils::NormalizeHeaderValue(http_body->Boundary().data());
...
void XMLHttpRequest::send(URLSearchParams* body, ExceptionState& exception_state)
{
    if (!InitSend(exception_state))
        return;
    ...
    if (AreMethodAndURLValidForSend()) {
        ...
        UpdateContentTypeAndCharset(AtomicString("application/x-www-form-urlencoded;charset=UTF-8"), "UTF-8");
    }

    CreateRequest(std::move(http_body), exception_state);
}

void XMLHttpRequest::send(DOMArrayBuffer* body, ExceptionState& exception_state)
{
    SendBytesData(body->Data(), body->ByteLength(), exception_state);
}

void XMLHttpRequest::send(DOMArrayBufferView* body, ExceptionState& exception_state)
{
    SendBytesData(body->BaseAddress(), body->byteLength(), exception_state);
}

void XMLHttpRequest::SendBytesData(const void* data, size_t length, ExceptionState& exception_state)
{
    if (!InitSend(exception_state))
        return;
    ...
    if (AreMethodAndURLValidForSend()) {
        http_body = EncodedFormData::Create(data, base::checked_cast<wtf_size_t>(length));
    }

    CreateRequest(std::move(http_body), exception_state);
}

void XMLHttpRequest::SendForInspectorXHRReplay(scoped_refptr<EncodedFormData> form_data, ExceptionState& exception_state)
{
    CreateRequest(form_data ? form_data->DeepCopy() : nullptr, exception_state);
}

void XMLHttpRequest::ThrowForLoadFailureIfNeeded(ExceptionState& exception_state, const String& reason)
{
...
void XMLHttpRequest::CreateRequest(scoped_refptr<EncodedFormData> http_body, ExceptionState& exception_state)
{
    ...
    if (url_.ProtocolIs("blob") && method_ != http_names::kGET) {
        ...
        if (!async_) {
            ThrowForLoadFailureIfNeeded(exception_state, "'GET' is the only method allowed for 'blob:' URLs.");
        }
        return;
    }
    ...
    if (url_.ProtocolIs("ftp")) {
        ...
        HandleNetworkError();
        if (!async_) {
            ThrowForLoadFailureIfNeeded(exception_state, "Making a request to a FTP URL is not supported.");
        }
        return;
    }
    ...
    if (async_) {
        ...
        if (http_body && upload_) {
            ...
            upload_->DispatchEvent(*ProgressEvent::Create(event_type_names::kLoadstart, true, 0, http_body->SizeInBytes()));
            // See above.
            if (!send_flag_ || loader_)
                return;
        }
    ...
    upload_events_allowed_ = GetExecutionContext()->GetSecurityOrigin()->CanRequest(url_)
        || (isolated_world_security_origin_ && isolated_world_security_origin_->CanRequest(url_)) || upload_events || !cors::IsCorsSafelistedMethod(method_)
    ...
    request.SetMode(upload_events ? network::mojom::RequestMode::kCorsWithForcedPreflight : network::mojom::RequestMode::kCors);
    request.SetTargetAddressSpace(network::mojom::IPAddressSpace::kUnknown);
    request.SetCredentialsMode(with_credentials_ ? network::mojom::CredentialsMode::kInclude : network::mojom::CredentialsMode::kSameOrigin);
    ...
    request.SetAttributionReportingEligibility(attribution_reporting_eligibility_);

    probe::WillLoadXHR(&execution_context, method_, url_, async_, request_headers_, with_credentials_);
    ...
    resource_loader_options.initiator_info.name = fetch_initiator_type_names::kXmlhttprequest;
    if (blob_url_loader_factory_) {
        resource_loader_options.url_loader_factory
            = base::MakeRefCounted<base::RefCountedData<mojo::PendingRemote<network::mojom::blink::URLLoaderFactory>>>(std::move(blob_url_loader_factory_));
    }
    ...
    downloading_to_blob_ = GetResponseTypeCode() == V8XMLHttpRequestResponseType::Enum::kBlob && !url_.ProtocolIsData();
    ...
    if (async_) {
        UseCounter::Count(&execution_context, WebFeature::kXMLHttpRequestAsynchronous);
        if (upload_)
        ...
        if (auto* window = DynamicTo<LocalDOMWindow>(GetExecutionContext())) {
            if (Frame* frame = window->GetFrame()) {
                if (frame->IsCrossOriginToOutermostMainFrame()) {
                    UseCounter::Count(&execution_context, WebFeature::kXMLHttpRequestSynchronousInCrossOriginSubframe);
                } else if (frame->IsMainFrame()) {
                    UseCounter::Count(&execution_context, WebFeature::kXMLHttpRequestSynchronousInMainFrame);
                } else {
                    UseCounter::Count(&execution_context, WebFeature::kXMLHttpRequestSynchronousInSameOriginSubframe);
                }
            }
            ...
            UseCounter::Count(&execution_context, WebFeature::kXMLHttpRequestSynchronousInWorker);
        }
    ...
    loader_ = MakeGarbageCollected<ThreadableLoader>(execution_context, this, resource_loader_options);
    loader_->SetTimeout(timeout_);
...
void XMLHttpRequest::abort()
{
    ...
    if (async_) {
        if ((state_ == kOpened && send_flag_) || state_ == kHeadersReceived || state_ == kLoading) {
            DCHECK(!loader_);
...
void XMLHttpRequest::DispatchProgressEvent(const AtomicString& type, int64_t received_length, int64_t expected_length)
{
    bool length_computable = expected_length > 0 && received_length <= expected_length;
    uint64_t loaded = received_length >= 0 ? static_cast<uint64_t>(received_length) : 0;
    uint64_t total = length_computable ? static_cast<uint64_t>(expected_length) : 0;

    std::optional<scheduler::TaskAttributionTracker::TaskScope> task_attribution_scope = MaybeCreateTaskAttributionScope();
    ExecutionContext* context = GetExecutionContext();
    probe::AsyncTask async_task(context, &async_task_context_, type == event_type_names::kLoadend ? nullptr : "progress", async_);
    progress_event_throttle_->DispatchProgressEvent(type, length_computable, loaded, total);
}

void XMLHttpRequest::DispatchProgressEventFromSnapshot(const AtomicString& type)
{
    DispatchProgressEvent(type, received_length_, response_.ExpectedContentLength());
}
...
void XMLHttpRequest::HandleDidCancel()
{
    ...
    pending_abort_event_ = PostCancellableTask(*GetExecutionContext()->GetTaskRunner(TaskType::kNetworking), FROM_HERE,
        WTF::BindOnce(&XMLHttpRequest::HandleRequestError, WrapPersistent(this), DOMExceptionCode::kAbortError, event_type_names::kAbort));
}

void XMLHttpRequest::HandleRequestError(DOMExceptionCode exception_code, const AtomicString& type)
{
...
void XMLHttpRequest::overrideMimeType(const AtomicString& mime_type, ExceptionState& exception_state)
{
    if (state_ == kLoading || state_ == kDone) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "MimeType cannot be overridden when the state is LOADING or DONE.");
        return;
    }
...
void XMLHttpRequest::setRequestHeader(const AtomicString& name, const AtomicString& value, ExceptionState& exception_state)
{
    ...
    if (!IsValidHTTPToken(name)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "'" + name + "' is not a valid HTTP header field name.");
        return;
    }
    if (!IsValidHTTPHeaderValue(normalized_value)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "'" + normalized_value + "' is not a valid HTTP header field value.");
        return;
    }
...
{
    DCHECK_EQ(value, FetchUtils::NormalizeHeaderValue(value)) << "Header values must be normalized";
...
void XMLHttpRequest::setPrivateToken(const PrivateToken* trust_token, ExceptionState& exception_state)
{
    ...
    if (!ConvertTrustTokenToMojomAndCheckPermissions(*trust_token, GetPSTFeatures(*GetExecutionContext()), &exception_state, params.get())) {
...
void XMLHttpRequest::setAttributionReporting(const AttributionReportingRequestOptions* options, ExceptionState& exception_state)
{
    // These precondition checks are copied from |setRequestHeader|.
    if (state_ != kOpened || send_flag_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "The object's state must be OPENED.");
    ...
    attribution_reporting_eligibility_ = ConvertAttributionReportingRequestOptionsToMojom(*options, *GetExecutionContext(), exception_state);
}

bool XMLHttpRequest::HasContentTypeRequestHeader() const
{
    return request_headers_.Find(http_names::kContentType) != request_headers_.end();
}
...
String XMLHttpRequest::getAllResponseHeaders() const
{
    ...
    HTTPHeaderSet access_control_expose_header_set = cors::ExtractCorsExposedHeaderNamesList(
        with_credentials_ ? network::mojom::CredentialsMode::kInclude : network::mojom::CredentialsMode::kSameOrigin, response_);
    ...
    for (auto it = response_.HttpHeaderFields().begin(); it != response_.HttpHeaderFields().end(); ++it) {
        ...
        if (response_.GetType() == network::mojom::FetchResponseType::kCors && !cors::IsCorsSafelistedResponseHeader(it->key)
            && access_control_expose_header_set.find(it->key.Ascii()) == access_control_expose_header_set.end()) {
            continue;
        }
    ...
    std::sort(headers.begin(), headers.end(),
        [](const std::pair<String, String>& x, const std::pair<String, String>& y) { return CodeUnitCompareLessThan(x.first, y.first); });
...
const AtomicString& XMLHttpRequest::getResponseHeader(const AtomicString& name) const
{
    ...
    if (response_.GetType() == network::mojom::FetchResponseType::kCors && !cors::IsCorsSafelistedResponseHeader(name)
...
{
    std::optional<std::string> overridden_type = net::ExtractMimeTypeFromMediaType(mime_type_override_.Utf8(),
        /*accept_comma_separated=*/false);
    ...
    if (response_.IsHTTP()) {
        ...
        std::optional<std::string> extracted_type = net::ExtractMimeTypeFromMediaType(header.Utf8(),
...
WTF::TextEncoding XMLHttpRequest::FinalResponseCharset() const
{
    ...
    // it. [spec text]
    String override_response_charset = ExtractCharsetFromMediaType(mime_type_override_);
...
void XMLHttpRequest::UpdateContentTypeAndCharset(const AtomicString& default_content_type, const String& charset)
{
    ...
    if (original_content_type != content_type) {
        ...
        if (!EqualIgnoringASCIICase(original_content_type, content_type)) {
            UseCounter::Count(GetExecutionContext(), WebFeature::kReplaceCharsetInXHRIgnoringCase);
        }
    }
}
...
void XMLHttpRequest::DidFinishLoading(uint64_t identifier)
{
    ...
    if (downloading_to_blob_ && response_type_code_ != V8XMLHttpRequestResponseType::Enum::kBlob && response_blob_) {
        ...
        blob_loader_ = MakeGarbageCollected<BlobLoader>(this, response_blob_->GetBlobDataHandle());
    } else {
...
void XMLHttpRequest::DidSendData(uint64_t bytes_sent, uint64_t total_bytes_to_be_sent)
{
    DVLOG(1) << this << " didSendData(" << bytes_sent << ", " << total_bytes_to_be_sent << ")";
    if (!upload_)
        return;
    ...
    if (bytes_sent == total_bytes_to_be_sent && !upload_complete_) {
        upload_complete_ = true;
        if (upload_events_allowed_) {
            upload_->DispatchEventAndLoadEnd(event_type_names::kLoad, true, bytes_sent, total_bytes_to_be_sent);
        }
    }
}

void XMLHttpRequest::DidReceiveResponse(uint64_t identifier, const ResourceResponse& response)
{
    // TODO(yhirano): Remove this CHECK: see https://crbug.com/570946.
    CHECK(&response);
...
void XMLHttpRequest::ParseDocumentChunk(base::span<const uint8_t> data)
{
    if (!response_document_parser_) {
        ...
        response_document_parser_ = response_document_->ImplicitOpen(kAllowDeferredParsing);
...
std::unique_ptr<TextResourceDecoder> XMLHttpRequest::CreateDecoder() const
{
    if (response_type_code_ == V8XMLHttpRequestResponseType::Enum::kJson) {
        return std::make_unique<TextResourceDecoder>(TextResourceDecoderOptions(TextResourceDecoderOptions::CreateUTF8Decode()));
    }
    ...
    if (final_response_charset.IsValid()) {
        ...
        return std::make_unique<TextResourceDecoder>(TextResourceDecoderOptions(TextResourceDecoderOptions::kPlainTextContent, final_response_charset));
    }

    TextResourceDecoderOptions decoder_options_for_xml(TextResourceDecoderOptions::kXMLContent);
...
void XMLHttpRequest::DidReceiveData(base::span<const char> data)
{
    ...
    if (response_type_code_ == V8XMLHttpRequestResponseType::Enum::kDocument && ResponseIsHTML()) {
        ParseDocumentChunk(base::as_bytes(data));
    } else if (response_type_code_ == kResponseTypeDefault || response_type_code_ == V8XMLHttpRequestResponseType::Enum::kText
        || response_type_code_ == V8XMLHttpRequestResponseType::Enum::kJson || response_type_code_ == V8XMLHttpRequestResponseType::Enum::kDocument) {
        if (!decoder_)
            decoder_ = CreateDecoder();

        if (!response_text_overflow_) {
            if (response_text_.DoesAppendCauseOverflow(base::checked_cast<unsigned>(data.size()))) {
                response_text_overflow_ = true;
                response_text_.Clear();
            } else {
    ...
    } else if (response_type_code_ == V8XMLHttpRequestResponseType::Enum::kArraybuffer || response_type_code_ == V8XMLHttpRequestResponseType::Enum::kBlob) {
        // Buffer binary data.
...
void XMLHttpRequest::DidDownloadToBlob(scoped_refptr<BlobDataHandle> blob)
{
    ...
    if (!blob) {
        ...
        if (blob->GetType() != mime_type) {
            ...
            response_blob_ = MakeGarbageCollected<Blob>(BlobDataHandle::Create(std::move(blob_data), blob_size));
        } else {
...
void XMLHttpRequest::HandleDidTimeout()
{
    ...
    HandleRequestError(DOMExceptionCode::kTimeoutError, event_type_names::kTimeout);
}
...
void XMLHttpRequest::ReportMemoryUsageToV8()
{
    ...
    int64_t diff = static_cast<int64_t>(size) - static_cast<int64_t>(binary_response_builder_last_reported_size_);
    binary_response_builder_last_reported_size_ = size;

    // Blob (downloading_to_blob_, length_downloaded_to_blob_)
    diff += static_cast<int64_t>(length_downloaded_to_blob_) - static_cast<int64_t>(length_downloaded_to_blob_last_reported_);
    length_downloaded_to_blob_last_reported_ = length_downloaded_to_blob_;

    // Text
    const size_t response_text_size = response_text_.Capacity() * (response_text_.Is8Bit() ? sizeof(LChar) : sizeof(UChar));
    diff += static_cast<int64_t>(response_text_size) - static_cast<int64_t>(response_text_last_reported_size_);
...
std::optional<scheduler::TaskAttributionTracker::TaskScope> XMLHttpRequest::MaybeCreateTaskAttributionScope()
{
    if (!parent_task_ || !GetExecutionContext() || GetExecutionContext()->IsContextDestroyed()) {
        return std::nullopt;
    }
    ...
    return tracker->CreateTaskScope(script_state, parent_task_, scheduler::TaskAttributionTracker::TaskScopeType::kXMLHttpRequest);
}
