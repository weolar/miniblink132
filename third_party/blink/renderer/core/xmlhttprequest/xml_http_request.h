...
class CORE_EXPORT XMLHttpRequest final : public XMLHttpRequestEventTarget,
...
public:
    ...
    enum State { kUnsent = 0, kOpened = 1, kHeadersReceived = 2, kLoading = 3, kDone = 4 };
    ...
    void send(const V8UnionDocumentOrXMLHttpRequestBodyInit* body, ExceptionState& exception_state);
    void abort();
    void Dispose();
    ...
    void setAttributionReporting(const AttributionReportingRequestOptions*, ExceptionState&);
    ...
    // For Inspector.
    void SendForInspectorXHRReplay(scoped_refptr<EncodedFormData>, ExceptionState&);
...
private:
    ...
    void DidReceiveResponse(uint64_t identifier, const ResourceResponse&) override;
    ...
    void UpdateContentTypeAndCharset(const AtomicString& content_type, const String& charset);
    ...
    std::optional<scheduler::TaskAttributionTracker::TaskScope> MaybeCreateTaskAttributionScope();

    Member<XMLHttpRequestUpload> upload_;

    KURL url_;
    mojo::PendingRemote<network::mojom::blink::URLLoaderFactory> blob_url_loader_factory_;
    ...
    static constexpr auto kResponseTypeDefault = V8XMLHttpRequestResponseType::Enum::k;
