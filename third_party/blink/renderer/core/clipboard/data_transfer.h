...
class CORE_EXPORT DataTransfer final : public ScriptWrappable, public DataObject::Observer {
...
public:
    ...
    bool IsForCopyAndPaste() const
    {
        return transfer_type_ == kCopyAndPaste;
    }
    bool IsForDragAndDrop() const
    {
        return transfer_type_ == kDragAndDrop;
    }
    ...
    bool DropEffectIsInitialized() const
    {
        return !drop_effect_.IsNull();
    }
    AtomicString effectAllowed() const
    {
    ...
    std::unique_ptr<DragImage> CreateDragImage(gfx::Point& drag_location, float device_scale_factor, LocalFrame*) const;
    void DeclareAndWriteDragImage(Element*, const KURL& link_url, const KURL& image_url, const String& title);
    ...
    static gfx::RectF ClipByVisualViewport(const gfx::RectF& rect_in_document, const LocalFrame&);
    ...
    static std::unique_ptr<DragImage> CreateDragImageForFrame(
        LocalFrame&, float, const gfx::SizeF& layout_size, const gfx::Vector2dF& paint_offset, PaintRecordBuilder&, const PropertyTreeState&);
