...
TEST(ClipboardUtilitiesTest, URLToImageMarkupNonASCII)
{
    ...
    String markup_with_non_ascii = URLToImageMarkup(KURL(NullURL(),
                                                        "http://test.example/fran\xe7"
                                                        "ais.png"),
        "Fran\xe7"
        "ais");
    EXPECT_EQ("<img src=\"http://test.example/fran%C3%A7ais.png\" alt=\"Fran\xe7"
              "ais\"/>",
        markup_with_non_ascii);
    EXPECT_EQ("<img src=\"http://test.example/fran%C3%A7ais.png\" alt=\"Fran\xc3\xa7"
              "ais\"/>",
...
TEST(ClipboardUtilitiesTest, URLToImageMarkupEmbeddedNull)
{
    ...
    const char kExpectedOutputWithNull[] = "<img src=\"http://test.example/%00.png\" alt=\"\0\"/>";
    EXPECT_EQ(String(base::span_from_cstring(kExpectedOutputWithNull)),
        URLToImageMarkup(KURL(NullURL(), String(base::span_from_cstring(kURLWithNull))), String(base::span_from_cstring(kTitleWithNull))));
}
