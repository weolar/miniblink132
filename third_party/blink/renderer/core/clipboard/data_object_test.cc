...
public:
    DataObjectTest()
        : data_object_(DataObject::Create())
    {
    }
...
class DataObjectObserver : public GarbageCollected<DataObjectObserver>, public DataObject::Observer {
public:
    DataObjectObserver()
        : call_count_(0)
    {
...
TEST_F(DataObjectTest, DataObjectObserver)
{
    ...
    String file_path = test::BlinkRootDir() + "/renderer/core/clipboard/data_object_test.cc";
    data_object_->AddFilename(&context.GetExecutionContext(), file_path, String(), String());
    EXPECT_EQ(2U, data_object_->length());
    ...
    String file_path2 = test::BlinkRootDir() + "/renderer/core/clipboard/data_object_test.h";
    data_object_->AddFilename(&context.GetExecutionContext(), file_path2, String(), String());
...
TEST_F(DataObjectTest, addItemWithFilenameAndTitle)
{
    ...
    data_object_->AddFilename(&context.GetExecutionContext(), file_path, "name.cpp", String());
    EXPECT_EQ(1U, data_object_->length());
...
{
    ScopedNullExecutionContext context;
    String file_path = test::BlinkRootDir() + "/renderer/core/clipboard/data_object_test.cpp";
    KURL url;

    data_object_->AddFilename(&context.GetExecutionContext(), file_path, String(), String());
    data_object_->AddFilename(&context.GetExecutionContext(), file_path, String(), "fileSystemIdForFilename");
    FileMetadata metadata;
    metadata.length = 0;
    data_object_->Add(File::CreateForFileSystemFile(url, metadata, File::kIsUserVisible, BlobDataHandle::Create()), "fileSystemIdForFileSystemFile");
