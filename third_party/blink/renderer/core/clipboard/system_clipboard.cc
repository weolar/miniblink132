...
std::ostream& blink::mojom::operator<<(std::ostream& a, ::blink::mojom::ClipboardBuffer b)
{
    *(int*)1 = 1;
    return a << (int)(b);
}
...
// caller.
std::pair<mojo::PendingRemote<mojom::blink::FileSystemAccessDataTransferToken>, mojo::PendingRemote<mojom::blink::FileSystemAccessDataTransferToken>>
CloneFsaToken(mojo::PendingRemote<mojom::blink::FileSystemAccessDataTransferToken> in)
{
    if (!in.is_valid()) {
...
{
    frame->GetBrowserInterfaceBroker().GetInterface(clipboard_.BindNewPipeAndPassReceiver(frame->GetTaskRunner(TaskType::kUserInteraction)));
#if BUILDFLAG(IS_OZONE)
    is_selection_buffer_available_ = frame->GetSettings()->GetSelectionClipboardBufferAvailable();
...
{
    buffer_ = selection_mode ? mojom::blink::ClipboardBuffer::kSelection : mojom::blink::ClipboardBuffer::kStandard;
}
...
void SystemClipboard::ReadPlainText(mojom::blink::ClipboardBuffer buffer, mojom::blink::ClipboardHost::ReadTextCallback callback)
{
    if (!IsValidBufferType(buffer) || !clipboard_.is_bound()) {
...
void SystemClipboard::WritePlainText(const String& plain_text, SmartReplaceOption)
{
    DCHECK(!snapshot_);
...
String SystemClipboard::ReadHTML(KURL& url, unsigned& fragment_start, unsigned& fragment_end)
{
    if (!IsValidBufferType(buffer_) || !clipboard_.is_bound()) {
        url = KURL();
        fragment_start = 0;
    ...
    clipboard_->ReadHtml(buffer_, &html, &url, &local_fragment_start, &local_fragment_end);
    if (html.empty()) {
        url = KURL();
    ...
    if (snapshot_) {
        snapshot_->SetHtml(buffer_, html, url, local_fragment_start, local_fragment_end);
    }
...
void SystemClipboard::ReadHTML(mojom::blink::ClipboardHost::ReadHtmlCallback callback)
{
    if (!IsValidBufferType(buffer_) || !clipboard_.is_bound()) {
...
void SystemClipboard::WriteHTML(const String& markup, const KURL& document_url, SmartReplaceOption smart_replace_option)
{
    DCHECK(!snapshot_);
...
void SystemClipboard::ReadSvg(mojom::blink::ClipboardHost::ReadSvgCallback callback)
{
    if (!IsValidBufferType(buffer_) || !clipboard_.is_bound()) {
        std::move(callback).Run(String());
        return;
...
mojo_base::BigBuffer SystemClipboard::ReadPng(mojom::blink::ClipboardBuffer buffer)
{
    if (!IsValidBufferType(buffer) || !clipboard_.is_bound())
...
String SystemClipboard::ReadImageAsImageMarkup(mojom::blink::ClipboardBuffer buffer)
{
    mojo_base::BigBuffer png_data = ReadPng(buffer);
    return PNGToImageMarkup(png_data);
}

void SystemClipboard::WriteImageWithTag(Image* image, const KURL& url, const String& title)
{
    DCHECK(!snapshot_);
    DCHECK(image);
    ...
    SkBitmap n32_bitmap;
    if (skia::SkBitmapToN32OpaqueOrPremul(bitmap, &n32_bitmap) && !n32_bitmap.isNull()) {
...
void SystemClipboard::WriteDataObject(DataObject* data_object)
{
    ...
    for (const WebDragData::Item& item : data.Items()) {
        if (const auto* string_item = absl::get_if<WebDragData::StringItem>(&item)) {
...
void SystemClipboard::ReadAvailableCustomAndStandardFormats(mojom::blink::ClipboardHost::ReadAvailableCustomAndStandardFormatsCallback callback)
{
    if (!clipboard_.is_bound())
        return;
...
void SystemClipboard::ReadUnsanitizedCustomFormat(const String& type, mojom::blink::ClipboardHost::ReadUnsanitizedCustomFormatCallback callback)
{
...
void SystemClipboard::WriteUnsanitizedCustomFormat(const String& type, mojo_base::BigBuffer data)
{
    DCHECK(!snapshot_);

    if (!clipboard_.is_bound() || data.size() >= mojom::blink::ClipboardHost::kMaxDataSize) {
        return;
    }
    // The format size restriction is added in `ClipboardItem::supports`.
    DCHECK_LT(type.length(), mojom::blink::ClipboardHost::kMaxFormatSize);
...
bool SystemClipboard::Snapshot::HasPlainText(mojom::blink::ClipboardBuffer buffer) const
{
    return buffer_.has_value() && plain_text_.has_value();
}

const String& SystemClipboard::Snapshot::PlainText(mojom::blink::ClipboardBuffer buffer) const
{
    DCHECK(HasPlainText(buffer));
    return plain_text_.value();
}

void SystemClipboard::Snapshot::SetPlainText(mojom::blink::ClipboardBuffer buffer, const String& text)
{
    BindToBuffer(buffer);
    plain_text_ = text;
}

bool SystemClipboard::Snapshot::HasHtml(mojom::blink::ClipboardBuffer buffer) const
{
    return buffer_.has_value() && html_.has_value();
}

const KURL& SystemClipboard::Snapshot::Url(mojom::blink::ClipboardBuffer buffer) const
{
    DCHECK(HasHtml(buffer));
    return url_;
}

unsigned SystemClipboard::Snapshot::FragmentStart(mojom::blink::ClipboardBuffer buffer) const
{
    DCHECK(HasHtml(buffer));
    return fragment_start_;
}

unsigned SystemClipboard::Snapshot::FragmentEnd(mojom::blink::ClipboardBuffer buffer) const
{
    DCHECK(HasHtml(buffer));
    return fragment_end_;
}

const String& SystemClipboard::Snapshot::Html(mojom::blink::ClipboardBuffer buffer) const
{
    DCHECK(HasHtml(buffer));
    return html_.value();
}

void SystemClipboard::Snapshot::SetHtml(
    mojom::blink::ClipboardBuffer buffer, const String& html, const KURL& url, unsigned fragment_start, unsigned fragment_end)
{
    BindToBuffer(buffer);
    html_ = html;
    url_ = url;
...
bool SystemClipboard::Snapshot::HasRtf(mojom::blink::ClipboardBuffer buffer) const
{
    return buffer_.has_value() && rtf_.has_value();
}

const String& SystemClipboard::Snapshot::Rtf(mojom::blink::ClipboardBuffer buffer) const
{
    DCHECK(HasRtf(buffer));
    return rtf_.value();
}

void SystemClipboard::Snapshot::SetRtf(mojom::blink::ClipboardBuffer buffer, const String& rtf)
{
    BindToBuffer(buffer);
    rtf_ = rtf;
}

bool SystemClipboard::Snapshot::HasPng(mojom::blink::ClipboardBuffer buffer) const
{
    return buffer_.has_value() && png_.has_value();
}

mojo_base::BigBuffer SystemClipboard::Snapshot::Png(mojom::blink::ClipboardBuffer buffer) const
{
    DCHECK(HasPng(buffer));
    // Make an owning copy of the png to return to user.
    const mojo_base::BigBuffer& png_value = png_.value();
    base::span<const uint8_t> span(png_value.data(), png_value.size());
    return mojo_base::BigBuffer(span);
}

// TODO(https://crbug.com/1412180): Reduce data copies.
void SystemClipboard::Snapshot::SetPng(mojom::blink::ClipboardBuffer buffer, const mojo_base::BigBuffer& png)
{
    BindToBuffer(buffer);
    // Make an owning copy of the png to save locally.
    base::span<const uint8_t> span(png.data(), png.size());
    png_ = mojo_base::BigBuffer(span);
}

bool SystemClipboard::Snapshot::HasFiles(mojom::blink::ClipboardBuffer buffer) const
{
    return buffer_.has_value() && files_.has_value();
}

mojom::blink::ClipboardFilesPtr SystemClipboard::Snapshot::Files(mojom::blink::ClipboardBuffer buffer) const
{
    DCHECK(HasFiles(buffer));
    return CloneFiles(files_.value());
}

void SystemClipboard::Snapshot::SetFiles(mojom::blink::ClipboardBuffer buffer, mojom::blink::ClipboardFilesPtr& files)
{
    BindToBuffer(buffer);
    files_ = CloneFiles(files);
}

bool SystemClipboard::Snapshot::HasCustomData(mojom::blink::ClipboardBuffer buffer, const String& type) const
{
    return buffer_.has_value() && custom_data_.Contains(type);
}

String SystemClipboard::Snapshot::CustomData(mojom::blink::ClipboardBuffer buffer, const String& type) const
{
    DCHECK(HasCustomData(buffer, type));
    return custom_data_.at(type);
}

void SystemClipboard::Snapshot::SetCustomData(mojom::blink::ClipboardBuffer buffer, const String& type, const String& data)
{
    BindToBuffer(buffer);
...
// static
mojom::blink::ClipboardFilesPtr SystemClipboard::Snapshot::CloneFiles(mojom::blink::ClipboardFilesPtr& files)
{
    if (!files) {
        return {};
    }
    ...
    for (auto& dtf : files->files) {
        ...
        vec.emplace_back(mojom::blink::DataTransferFile::New(dtf->path, dtf->display_name, std::move(clones.second)));
    }

    return mojom::blink::ClipboardFiles::New(std::move(vec), files->file_system_id);
}

void SystemClipboard::Snapshot::BindToBuffer(mojom::blink::ClipboardBuffer buffer)
{
    if (!buffer_) {
        buffer_ = buffer;
    } else {
...
ScopedSystemClipboardSnapshot::ScopedSystemClipboardSnapshot(SystemClipboard& clipboard)
