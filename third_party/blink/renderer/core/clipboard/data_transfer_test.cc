...
TEST_P(DataTransferTest, NodeImage)
{
    ...
    const std::unique_ptr<DragImage> image = DataTransfer::NodeImage(GetFrame(), *sample);
...
TEST_P(DataTransferTest, NodeImageWithNestedElement)
{
    ...
    Element* sample = GetDocument().getElementById(AtomicString("sample"));
    const std::unique_ptr<DragImage> image = DataTransfer::NodeImage(GetFrame(), *sample);
    EXPECT_EQ(Color::FromRGB(0, 255, 0), sample->firstChild()->GetLayoutObject()->ResolveColor(GetCSSPropertyColor()))
...
TEST_P(DataTransferTest, NodeImageWithPsuedoClassWebKitDrag)
{
    ...
    Element* sample = GetDocument().getElementById(AtomicString("sample"));
    const std::unique_ptr<DragImage> image = DataTransfer::NodeImage(GetFrame(), *sample);
    EXPECT_EQ(gfx::Size(200, 200), image->Size()) << ":-webkit-drag should affect dragged image.";
}
...
TEST_P(DataTransferTest, NodeImageWithChangingLayoutObject)
{
    ...
    EXPECT_TRUE(sample->GetLayoutObject() != before_layout_object) << ":-webkit-drag causes sample to have different layout object.";
    EXPECT_EQ(Color::FromRGB(255, 0, 0), sample->GetLayoutObject()->ResolveColor(GetCSSPropertyColor())) << "#sample has :-webkit-drag.";

    // Layout w/o :-webkit-drag
    UpdateAllLifecyclePhasesForTest();

    EXPECT_EQ(Color::FromRGB(0, 0, 255), sample->GetLayoutObject()->ResolveColor(GetCSSPropertyColor())) << "#sample doesn't have :-webkit-drag.";
}
...
TEST_P(DataTransferTest, NodeImageUnderScrollOffset)
{
    ...
    frame_view->LayoutViewport()->SetScrollOffset(ScrollOffset(0, scroll_amount), mojom::blink::ScrollType::kProgrammatic);
...
TEST_P(DataTransferTest, NodeImageSizeWithPageScaleFactor)
{
    ...
    EXPECT_EQ(gfx::Size(node_width * page_scale_factor, node_height * page_scale_factor), image->Size());
    ...
    EXPECT_EQ(gfx::Size(node_width * page_scale_factor, node_height * page_scale_factor), image_with_offset->Size());
}
...
TEST_P(DataTransferTest, NodeImageWithPageScaleFactor)
{
    ...
    const int page_scale_factor = 2;
    GetPage().SetPageScaleFactor(page_scale_factor);
    Element& blue_green = *GetDocument().getElementById(AtomicString("bluegreen"));
    const auto image = DataTransfer::NodeImage(GetFrame(), blue_green);
    const int blue_green_width = 2;
    const int blue_green_height = 1;
    EXPECT_EQ(gfx::Size(blue_green_width * page_scale_factor, blue_green_height * page_scale_factor), image->Size());
...
TEST_P(DataTransferTest, NodeImageOutOfView)
{
    ...
    auto image = DataTransfer::NodeImage(GetFrame(), *GetDocument().getElementById(AtomicString("drag")));
    EXPECT_EQ(gfx::Size(100, 100), image->Size());
...
TEST_P(DataTransferTest, CreateDragImageWithEmptyImageResource)
{
    ...
    data_transfer->SetDragImageResource(MakeGarbageCollected<ImageResourceContent>(nullptr), gfx::Point());

    gfx::Point drag_offset;
    std::unique_ptr<DragImage> drag_image = data_transfer->CreateDragImage(drag_offset, /* device_scale_factor*/ 1, &GetFrame());
...
TEST_P(DataTransferTest, NodeImageTranslatedOutOfView)
{
    ...
    EXPECT_EQ(gfx::Size(viewport_width + 20, viewport_height + 20), image->Size());
