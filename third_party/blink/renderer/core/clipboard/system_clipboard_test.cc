...
mojom::blink::ClipboardFilesPtr CreateFiles(int count)
{
    ...
    for (int i = 0; i < count; ++i) {
        vec.emplace_back(mojom::blink::DataTransferFile::New(base::FilePath(FILE_PATH_LITERAL("path")), base::FilePath(FILE_PATH_LITERAL("name")),
...
public:
    SystemClipboardTest()
    {
        ...
        clipboard_provider_ = std::make_unique<PageTestBase::MockClipboardHostProvider>(page_holder_.get()->GetFrame().GetBrowserInterfaceBroker());
    }
...
protected:
    ...
    void reset_remote_and_validate_buffer()
    {
        ...
        EXPECT_TRUE(system_clipboard().IsValidBufferType(system_clipboard().buffer_));
    }
...
TEST_F(SystemClipboardTest, Html)
{
    ...
    EXPECT_EQ(system_clipboard().ReadHTML(url, fragment_start, fragment_end), "first");
    ...
    // remembers the result, even if the underlying clipboard host changes.
    {
        ...
        EXPECT_EQ(system_clipboard().ReadHTML(url, fragment_start, fragment_end), "second");
    ...
    // Now that the snapshot is out of scope, reads from the system clipboard
    // reflect the state of the clipboard host.
    EXPECT_EQ(system_clipboard().ReadHTML(url, fragment_start, fragment_end), "third");
...
TEST_F(SystemClipboardTest, ReadHtml_SameFragmentArgs)
{
    ...
    EXPECT_EQ(system_clipboard().ReadHTML(url, fragment_start, fragment_end), kHtmlText);
    ...
    EXPECT_EQ(system_clipboard().ReadHTML(url, fragment_start2, fragment_end2), kHtmlText);
...
TEST_F(SystemClipboardTest, Png)
{
    ...
    SkBitmap bitmap2;
    SkBitmap bitmap3;
    ASSERT_TRUE(bitmap1.tryAllocPixelsFlags(SkImageInfo::Make(4, 3, kN32_SkColorType, kOpaque_SkAlphaType), 0));
    ASSERT_TRUE(bitmap2.tryAllocPixelsFlags(SkImageInfo::Make(40, 30, kN32_SkColorType, kOpaque_SkAlphaType), 0));
    ASSERT_TRUE(bitmap3.tryAllocPixelsFlags(SkImageInfo::Make(400, 300, kN32_SkColorType, kOpaque_SkAlphaType), 0));
...
TEST_F(SystemClipboardTest, CustomData)
{
    ...
    // remembers the result, even if the underlying clipboard host changes.
    {
        ...
        clipboard_host()->WriteDataTransferCustomData({ { "a", "third" }, { "b", "fourth" } });
        EXPECT_EQ(system_clipboard().ReadDataTransferCustomData("a"), "second");
...
TEST_F(SystemClipboardTest, ReadHtmlWithUnboundClipboardHost)
{
    ...
    // Now the Reads should return null string.
    EXPECT_EQ(system_clipboard().ReadHTML(url, fragment_start, fragment_end), String());
    // Writes will fail since the mojo remote is unbound.
    clipboard_host()->WriteHtml("first", KURL("http://first.com"));
    EXPECT_EQ(system_clipboard().ReadHTML(url, fragment_start, fragment_end), String());
...
TEST_F(SystemClipboardTest, ReadPngWithUnboundClipboardHost)
{
    ...
    // Create test bitmaps to put into the clipboard.
    SkBitmap bitmapIn;
    ASSERT_TRUE(bitmapIn.tryAllocPixelsFlags(SkImageInfo::Make(4, 3, kN32_SkColorType, kOpaque_SkAlphaType), 0));
...
{
    // Clipboard starts empty.
    EXPECT_FALSE(system_clipboard().IsFormatAvailable(blink::mojom::ClipboardFormat::kPlaintext));

    // Setting text in the host is visible in system clipboard.
    clipboard_host()->WriteText("first");
    EXPECT_TRUE(system_clipboard().IsFormatAvailable(blink::mojom::ClipboardFormat::kPlaintext));
