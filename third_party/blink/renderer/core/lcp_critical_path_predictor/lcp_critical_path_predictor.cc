...
{
    if (base::FeatureList::IsEnabled(blink::features::kLCPTimingPredictorPrerender2)) {
        return true;
    }
...
{
    return !lcp_element_locators_.empty() || !lcp_influencer_scripts_.empty() || !preconnected_origins_.empty();
}

void LCPCriticalPathPredictor::set_lcp_element_locators(const std::vector<std::string>& lcp_element_locator_strings)
{
    // Clear current set of locators before receiving replacements.
    lcp_element_locators_.clear();
    lcp_element_locator_strings_.clear();
    const wtf_size_t reserved_size = base::checked_cast<wtf_size_t>(lcp_element_locator_strings.size());
    ...
    for (const std::string& serialized_locator : lcp_element_locator_strings) {
        ...
        bool result = lcp_element_locators_.back().ParseFromString(serialized_locator);
        if (!result) {
...
void LCPCriticalPathPredictor::set_lcp_influencer_scripts(HashSet<KURL> scripts)
{
...
void LCPCriticalPathPredictor::set_preconnected_origins(const Vector<url::Origin>& origins)
{
...
void LCPCriticalPathPredictor::AddLCPPredictedCallback(base::OnceClosure callback)
{
    LCPCallback lcp_callback = WTF::BindOnce([](base::OnceClosure callback, const Element*) { std::move(callback).Run(); }, std::move(callback));
    AddLCPPredictedCallback(std::move(lcp_callback));
}

void LCPCriticalPathPredictor::MayRunPredictedCallbacks(const Element* lcp_element)
{
    if (are_predicted_callbacks_called_) {
        return;
    }
...
bool LCPCriticalPathPredictor::IsElementMatchingLocator(const Element& element)
{
    std::string lcp_element_locator_string = element_locator::OfElement(element).SerializeAsString();
    return lcp_element_locator_strings_.Contains(lcp_element_locator_string);
}

void LCPCriticalPathPredictor::OnLargestContentfulPaintUpdated(const Element& lcp_element, std::optional<const KURL> maybe_image_url)
{
    if (base::FeatureList::IsEnabled(features::kLCPCriticalPathPredictor) || base::FeatureList::IsEnabled(features::kLCPPLazyLoadImagePreload)
        || IsTimingPredictorEnabled()) {
        std::string lcp_element_locator_string = element_locator::OfElement(lcp_element).SerializeAsString();
        ...
        const wtf_size_t predicted_lcp_index = lcp_element_locator_strings_.Find(lcp_element_locator_string);
        ...
        features::LcppRecordedLcpElementTypes recordable_lcp_element_type = features::kLCPCriticalPathPredictorRecordedLcpElementTypes.Get();
        bool should_record_element_locator = (recordable_lcp_element_type == features::LcppRecordedLcpElementTypes::kAll)
            || (recordable_lcp_element_type == features::LcppRecordedLcpElementTypes::kImageOnly && IsA<HTMLImageElement>(lcp_element));

        if (should_record_element_locator) {
            base::UmaHistogramCounts10000("Blink.LCPP.LCPElementLocatorSize", base::checked_cast<int>(lcp_element_locator_string.size()));

            if (lcp_element_locator_string.size() <= features::kLCPCriticalPathPredictorMaxElementLocatorLength.Get()) {
                GetHost().SetLcpElementLocator(
                    lcp_element_locator_string, predicted_lcp_index == kNotFound ? std::nullopt : std::optional<wtf_size_t>(predicted_lcp_index));
            }
        }
    }

    if (base::FeatureList::IsEnabled(features::kLCPPAutoPreconnectLcpOrigin)) {
        auto root_origin = url::Origin::Create((GURL)lcp_element.GetDocument().Url());
        if (maybe_image_url.has_value()) {
            const KURL& lcp_image_url = *maybe_image_url;
            if (!lcp_image_url.IsEmpty() && lcp_image_url.IsValid() && lcp_image_url.ProtocolIsInHTTPFamily()) {
                auto lcp_origin = url::Origin::Create((GURL)lcp_image_url);
                bool is_lcp_cross_origin = !lcp_origin.IsSameOriginWith(root_origin);
                base::UmaHistogramBoolean("Blink.LCPP.CrossOriginLcpImage", is_lcp_cross_origin);
                ...
                base::UmaHistogramCounts1000("Blink.LCPP.PreconnectPredictionMatchCount", base::checked_cast<int>(preconnected_origins_.size()));
                if (!preconnected_origins_.empty()) {
                    base::UmaHistogramCounts100("Blink.LCPP.PreconnectPredictionMatchPercent",
                        base::checked_cast<int>((double)count_prediction_matches / preconnected_origins_.size() * 100));
                }
            }
        }
    }
    ...
    if (blink::LcppScriptObserverEnabled()) {
        if (const HTMLImageElement* image_element = DynamicTo<HTMLImageElement>(lcp_element)) {
            auto& creators = image_element->creator_scripts();
            size_t max_allowed_url_length = features::kLCPScriptObserverMaxUrlLength.Get();
            size_t max_allowed_url_count = features::kLCPScriptObserverMaxUrlCountPerOrigin.Get();
            ...
            for (auto& url : creators) {
                max_url_length_encountered = std::max<size_t>(max_url_length_encountered, url.length());
                if (url.length() >= max_allowed_url_length) {
                    continue;
                }
                KURL parsed_url(url);
                if (parsed_url.IsEmpty() || !parsed_url.IsValid() || !parsed_url.ProtocolIsInHTTPFamily()) {
                    continue;
                }
            ...
            base::UmaHistogramCounts10000("Blink.LCPP.LCPInfluencerUrlsCount", base::checked_cast<int>(filtered_script_urls.size()));
            base::UmaHistogramCounts10000("Blink.LCPP.LCPInfluencerUrlsMaxLength", base::checked_cast<int>(max_url_length_encountered));
            base::UmaHistogramCounts10000("Blink.LCPP.LCPInfluencerUrlsPredictionMatchCount", base::checked_cast<int>(prediction_match_count));
            if (!lcp_influencer_scripts_.empty()) {
                base::UmaHistogramCounts10000("Blink.LCPP.LCPInfluencerUrlsPredictionMatchPercent",
                    base::checked_cast<int>((double)prediction_match_count / lcp_influencer_scripts_.size() * 100));
            }
        }
    }
}
...
void LCPCriticalPathPredictor::OnStartPreload(const KURL& url, const ResourceType& resource_type)
{
    if (!base::FeatureList::IsEnabled(blink::features::kHttpDiskCachePrewarming) && !base::FeatureList::IsEnabled(blink::features::kLCPPPrefetchSubresource)) {
        return;
    }
    ...
    if (url.GetString().length() > features::kHttpDiskCachePrewarmingMaxUrlLength.Get()) {
        return;
    }
    ...
    base::TimeDelta resource_load_start = base::TimeTicks::Now() - document->Loader()->GetTiming().NavigationStart();
    CHECK_GE(resource_load_start, base::Seconds(0));
    GetHost().NotifyFetchedSubresource(url, resource_load_start, ResourceFetcher::DetermineRequestDestination(resource_type));
}

mojom::blink::LCPCriticalPathPredictorHost& LCPCriticalPathPredictor::GetHost()
{
    if (!host_.is_bound() || !host_.is_connected()) {
        host_.reset();
        GetFrame().GetBrowserInterfaceBroker().GetInterface(host_.BindNewPipeAndPassReceiver(task_runner_));
    }
    return *host_.get();
}
...
void LCPCriticalPathPredictor::OnWarnedUnusedPreloads(const Vector<KURL>& unused_preloads)
{
    ...
    if (!base::FeatureList::IsEnabled(features::kLCPPDeferUnusedPreload) || has_sent_unused_preloads_) {
        return;
    }
