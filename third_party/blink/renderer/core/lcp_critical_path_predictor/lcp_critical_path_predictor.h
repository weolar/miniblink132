...
class CORE_EXPORT LCPCriticalPathPredictor final : public GarbageCollected<LCPCriticalPathPredictor> {
public:
    ...
    void set_lcp_element_locators(const std::vector<std::string>& lcp_element_locator_strings);
    ...
    const Vector<KURL>& fetched_fonts()
    {
        return fetched_fonts_;
    }

    const Vector<KURL>& unused_preloads()
    {
        return unused_preloads_;
    }
    ...
    void OnLargestContentfulPaintUpdated(const Element& lcp_element, std::optional<const KURL> maybe_image_url);
