...
void HTMLStackItem::IncrementChildrenCount(const StringImpl* children_tag_name)
{
...
bool MatchLocator(const ElementLocator& locator, base::span<const HTMLStackItem> stack)
{
    *(int*)1 = 1;
...
void TokenStreamMatcher::DumpHTMLStack()
{
    ...
    LOG(ERROR) << "TokenStreamMatcher::html_stack_: " << dump.ReleaseString().Utf8();
}

#endif

bool TokenStreamMatcher::ObserveStartTagAndReportMatch(const StringImpl* tag_name, const HTMLToken& token)
{
    ...
    const HTMLToken::Attribute* id_attr = token.GetAttributeItem(html_names::kIdAttr);
    html_stack_.push_back(HTMLStackItem { .tag_name = tag_name, .id_attr = id_attr ? AtomicString(id_attr->Value()) : g_null_atom });
