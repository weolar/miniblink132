...
public:
    ...
    // match.
    bool ObserveStartTagAndReportMatch(const StringImpl* tag_name, const HTMLToken& token);
...
private:
    ...
    // https://html.spec.whatwg.org/multipage/parsing.html#stack-of-open-elements
    Vector<HTMLStackItem> html_stack_ { HTMLStackItem() }; // populate root node stack item.
};
