...
TEST_F(ElementLocatorTest, OfElement)
{
    ...
    constexpr TestCase test_cases[] = {
        ...
        { "<div id='container'><div data-locate-me></div></div>", "/div[0]/#container" },

        // No id on the document, so stop at BODY.
        { "<div data-locate-me></div>", "/div[0]/body[0]" },

        // Siblings
        { "<div id='container'><p><p><p><p data-locate-me><p></div>", "/p[3]/#container" },

        // Siblings with different tag names
        { "<div id='container'><h1></h1><p><p data-locate-me><p><a></a></div>", "/p[1]/#container" },
    ...
    for (const auto& test_case : test_cases) {
        SCOPED_TRACE(testing::Message() << std::endl << "body_html = " << test_case.body_html);

        SetBodyContent(test_case.body_html);
        Element* target = Traversal<Element>::FirstWithin(GetDocument(), HasDataLocateMe);
        ASSERT_TRUE(target);
...
public:
    ...
    void TestMatch(element_locator::TokenStreamMatcher& matcher, const Vector<Expectation>& exps)
    {
        size_t i = 0;
        for (const Expectation& exp : exps) {
            ...
            case Expectation::Type::kStartTag: {
                ...
                bool matched = matcher.ObserveStartTagAndReportMatch(tag_name.Impl(), token);
...
TEST_F(TokenStreamMatcherTest, CloseAPElement)
{
    ...
    EXPECT_EQ(String("/img[0]/p[2]/#container"), element_locator::ToStringForTesting(locator));

    element_locator::TokenStreamMatcher matcher({ locator });
    Vector<Expectation> exps = {

        { .tag_name = "div", .id_attr = "container" }, { .tag_name = "p" }, { .tag_name = "img" }, { .tag_name = "p" }, { .tag_name = "p" },
        { .tag_name = "img", .should_match = true }, { .type = kEndTag, .tag_name = "div" }
    };
...
TEST_F(TokenStreamMatcherTest, Complicated)
{
    ...
    EXPECT_EQ(String("/img[1]/article[2]/#container"), element_locator::ToStringForTesting(locator));
...
TEST_F(TokenStreamMatcherTest, DontMatchNonImg)
{
    ...
    EXPECT_EQ(String("/p[2]/#container"), element_locator::ToStringForTesting(locator));
