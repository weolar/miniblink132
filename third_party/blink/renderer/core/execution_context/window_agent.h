...
public:
    ...
    WindowAgent(AgentGroupScheduler& agent_group_scheduler, bool is_origin_agent_cluster, bool origin_agent_cluster_left_as_default);
...
template <> struct DowncastTraits<WindowAgent> {
    static bool AllowFrom(const Agent& agent)
    {
        return agent.IsWindowAgent();
    }
};
