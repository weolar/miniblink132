...
public:
    ...
    WindowAgent* GetAgentForOrigin(
        bool has_potential_universal_access_privilege, const SecurityOrigin* origin, bool is_origin_agent_cluster, bool origin_agent_cluster_left_as_default);
...
private:
    struct SchemeAndRegistrableDomain {
        ...
        SchemeAndRegistrableDomain(const String& scheme, const String& registrable_domain)
            : scheme(scheme)
    ...
    struct SchemeAndRegistrableDomainTraits
        : TwoFieldsHashTraits<SchemeAndRegistrableDomain, &SchemeAndRegistrableDomain::scheme, &SchemeAndRegistrableDomain::registrable_domain> { };
    ...
    HeapHashMap<scoped_refptr<const SecurityOrigin>, WeakMember<WindowAgent>> opaque_origin_agents_;
    ...
    HeapHashMap<scoped_refptr<const SecurityOrigin>, WeakMember<WindowAgent>> origin_keyed_agent_cluster_agents_;

    // Use registerable domain as the key for general tuple origins.
    using TupleOriginAgents = HeapHashMap<SchemeAndRegistrableDomain, WeakMember<WindowAgent>, SchemeAndRegistrableDomainTraits>;
