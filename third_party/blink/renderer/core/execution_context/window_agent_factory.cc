...
WindowAgentFactory::WindowAgentFactory(AgentGroupScheduler& agent_group_scheduler)
    ...
    bool has_potential_universal_access_privilege, const SecurityOrigin* origin, bool is_origin_agent_cluster, bool origin_agent_cluster_left_as_default)
{
    if (has_potential_universal_access_privilege) {
        ...
        if (!universal_access_agent_) {
            universal_access_agent_ = MakeGarbageCollected<WindowAgent>(*agent_group_scheduler_);
        }
    ...
    if (origin->IsLocal()) {
        ...
        DCHECK(!is_origin_agent_cluster);
        if (!file_url_agent_) {
            file_url_agent_ = MakeGarbageCollected<WindowAgent>(*agent_group_scheduler_);
        }
    ...
    if (origin->IsOpaque()) {
        ...
        if (inserted.is_new_entry) {
            inserted.stored_value->value = MakeGarbageCollected<WindowAgent>(*agent_group_scheduler_);
        }
    ...
    if (is_origin_agent_cluster) {
        ...
        if (inserted.is_new_entry) {
            inserted.stored_value->value
                = MakeGarbageCollected<WindowAgent>(*agent_group_scheduler_, is_origin_agent_cluster, origin_agent_cluster_left_as_default);
        }
        return inserted.stored_value->value.Get();
    }
    ...
    if (CommonSchemeRegistry::IsExtensionScheme(origin->Protocol().Ascii())) {
        DEFINE_STATIC_LOCAL(Persistent<TupleOriginAgents>, static_origin_agents, (MakeGarbageCollected<TupleOriginAgents>()));
