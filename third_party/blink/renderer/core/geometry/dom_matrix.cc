...
DOMMatrix* DOMMatrix::Create(ExecutionContext* execution_context, ExceptionState& exception_state)
{
    return MakeGarbageCollected<DOMMatrix>(gfx::Transform());
}

DOMMatrix* DOMMatrix::Create(ExecutionContext* execution_context, const V8UnionStringOrUnrestrictedDoubleSequence* init, ExceptionState& exception_state)
{
    DCHECK(init);
    ...
    case V8UnionStringOrUnrestrictedDoubleSequence::ContentType::kUnrestrictedDoubleSequence: {
        ...
        return MakeGarbageCollected<DOMMatrix>(base::span<const double>(sequence.data(), sequence.size()));
    }
    }
...
DOMMatrix* DOMMatrix::Create(DOMMatrixReadOnly* other, ExceptionState& exception_state)
{
    return MakeGarbageCollected<DOMMatrix>(other->Matrix(), other->is2D());
}

DOMMatrix* DOMMatrix::CreateForSerialization(base::span<const double> sequence)
{
    return MakeGarbageCollected<DOMMatrix>(sequence);
}

DOMMatrix* DOMMatrix::fromFloat32Array(NotShared<DOMFloat32Array> float32_array, ExceptionState& exception_state)
{
...
DOMMatrix* DOMMatrix::fromFloat64Array(NotShared<DOMFloat64Array> float64_array, ExceptionState& exception_state)
{
...
template <typename T>
DOMMatrix::DOMMatrix(base::span<T> sequence)
    : DOMMatrixReadOnly(sequence)
{
}
...
DOMMatrix* DOMMatrix::fromMatrix(DOMMatrixInit* other, ExceptionState& exception_state)
{
    ...
    if (other->is2D()) {
        return MakeGarbageCollected<DOMMatrix>(
            gfx::Transform::Affine(other->m11(), other->m12(), other->m21(), other->m22(), other->m41(), other->m42()), other->is2D());
    }

    return MakeGarbageCollected<DOMMatrix>(
        gfx::Transform::ColMajor(other->m11(), other->m12(), other->m13(), other->m14(), other->m21(), other->m22(), other->m23(), other->m24(), other->m31(),
            other->m32(), other->m33(), other->m34(), other->m41(), other->m42(), other->m43(), other->m44()),
        other->is2D());
}
...
{
    matrix_ = gfx::Transform::ColMajor(NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN);
}

DOMMatrix* DOMMatrix::multiplySelf(DOMMatrixInit* other, ExceptionState& exception_state)
{
...
DOMMatrix* DOMMatrix::preMultiplySelf(DOMMatrixInit* other, ExceptionState& exception_state)
{
    DOMMatrix* other_matrix = DOMMatrix::fromMatrix(other, exception_state);
    if (!other_matrix) {
        DCHECK(exception_state.HadException());
...
DOMMatrix* DOMMatrix::scaleSelf(double sx, double sy, double sz, double ox, double oy, double oz)
{
    if (sz != 1 || oz)
        is2d_ = false;
...
DOMMatrix* DOMMatrix::setMatrixValue(const ExecutionContext* execution_context, const String& input_string, ExceptionState& exception_state)
{
