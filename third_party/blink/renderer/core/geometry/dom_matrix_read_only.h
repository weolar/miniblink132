...
public:
    ...
    static DOMMatrixReadOnly* fromFloat32Array(NotShared<DOMFloat32Array>, ExceptionState&);
    static DOMMatrixReadOnly* fromFloat64Array(NotShared<DOMFloat64Array>, ExceptionState&);
    ...
    template <typename T, size_t N> explicit DOMMatrixReadOnly(base::span<T, N> sequence)
    {
        if (sequence.size() == 6) {
            matrix_ = gfx::Transform::Affine(sequence[0], sequence[1], sequence[2], sequence[3], sequence[4], sequence[5]);
            is2d_ = true;
        } else if (sequence.size() == 16) {
            matrix_ = gfx::Transform::ColMajor(sequence[0], sequence[1], sequence[2], sequence[3], sequence[4], sequence[5], sequence[6], sequence[7],
                sequence[8], sequence[9], sequence[10], sequence[11], sequence[12], sequence[13], sequence[14], sequence[15]);
            is2d_ = false;
        } else {
            NOTREACHED();
        }
    ...
    double m11() const
    {
        return matrix_.rc(0, 0);
    }
    double m12() const
    {
        return matrix_.rc(1, 0);
    }
    double m13() const
    {
        return matrix_.rc(2, 0);
    }
    double m14() const
    {
        return matrix_.rc(3, 0);
    }
    double m21() const
    {
        return matrix_.rc(0, 1);
    }
    double m22() const
    {
        return matrix_.rc(1, 1);
    }
    double m23() const
    {
        return matrix_.rc(2, 1);
    }
    double m24() const
    {
        return matrix_.rc(3, 1);
    }
    double m31() const
    {
        return matrix_.rc(0, 2);
    }
    double m32() const
    {
        return matrix_.rc(1, 2);
    }
    double m33() const
    {
        return matrix_.rc(2, 2);
    }
    double m34() const
    {
        return matrix_.rc(3, 2);
    }
    double m41() const
    {
    ...
    DOMMatrix* scale(double sx, double sy, double sz = 1, double ox = 0, double oy = 0, double oz = 0);
    DOMMatrix* scaleNonUniform(double sx = 1, double sy = 1);
    DOMMatrix* scale3d(double scale = 1, double ox = 0, double oy = 0, double oz = 0);
...
protected:
    void SetMatrixValueFromString(const ExecutionContext*, const String&, ExceptionState&);
