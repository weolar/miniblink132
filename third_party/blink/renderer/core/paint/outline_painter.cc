...
float FocusRingStrokeWidth(const ComputedStyle& style)
{
    ...
    if (style.EffectiveZoom() >= 1.0f) {
        width = ui::NativeTheme::GetInstanceForWeb()->AdjustBorderWidthByZoom(width, style.EffectiveZoom());
        DCHECK_GE(width, 3.f);
    }
...
int FocusRingOffset(const ComputedStyle& style, const LayoutObject::OutlineInfo& info)
{
    DCHECK(style.OutlineStyleIsAuto());
    // How much space the focus ring would like to take from the actual border.
    const float max_inside_border_width = ui::NativeTheme::GetInstanceForWeb()->AdjustBorderWidthByZoom(1.0f, style.EffectiveZoom());
    ...
    float min_border_width = std::min({ style.BorderTopWidth(), style.BorderBottomWidth(), style.BorderLeftWidth(), style.BorderRightWidth() });
...
{
    return gfx::Outsets::VH(std::max(offset, -rect.height() / 2), std::max(offset, -rect.width() / 2));
}
...
bool ComputeRightAnglePath(SkPath& path, const Vector<gfx::Rect>& rects, int outline_offset, int additional_outset)
{
...
bool MergeLineIfPossible(Line& line1, const Line& line2)
{
    ...
    if ((line1.start.x() == line1.end.x() && line1.start.x() == line2.end.x()) || (line1.start.y() == line1.end.y() && line1.start.y() == line2.end.y())) {
        line1.end = line2.end;
        return true;
    }
    return false;
...
template <typename Action> void IterateRightAnglePath(const SkPath& path, const Action& contour_action)
{
    ...
    for (SkPath::Verb verb = iter.next(points); verb != SkPath::kDone_Verb; verb = iter.next(points)) {
        ...
        case SkPath::kLine_Verb: {
            ...
            if (lines.empty() || !MergeLineIfPossible(lines.back(), new_line)) {
                ...
                DCHECK(lines.size() == 1 || lines.back().start == lines[lines.size() - 2].end);
            }
            break;
        }
...
SkPoint ShrinkCorner(const SkPoint& p1, const SkPoint& p2, const SkPoint& p3, int inset)
{
    if (p1.x() == p2.x()) {
        if (p1.y() < p2.y()) {
            return p2.x() < p3.x() ? p2 + SkVector::Make(-inset, inset) : p2 + SkVector::Make(-inset, -inset);
        }
        return p2.x() < p3.x() ? p2 + SkVector::Make(inset, inset) : p2 + SkVector::Make(inset, -inset);
    }
    if (p1.x() < p2.x()) {
        return p2.y() < p3.y() ? p2 + SkVector::Make(-inset, inset) : p2 + SkVector::Make(inset, inset);
    }
    return p2.y() < p3.y() ? p2 + SkVector::Make(-inset, -inset) : p2 + SkVector::Make(inset, -inset);
}
...
void ShrinkRightAnglePath(SkPath& path, int inset)
{
    ...
    IterateRightAnglePath(input, [&path, inset](const Vector<Line>& lines) {
        for (wtf_size_t i = 0; i < lines.size(); i++) {
            const SkPoint& prev_point = lines[i == 0 ? lines.size() - 1 : i - 1].start;
            SkPoint new_point = ShrinkCorner(prev_point, lines[i].start, lines[i].end, inset);
            if (i == 0) {
...
FloatRoundedRect::Radii ComputeCornerRadii(const ComputedStyle& style, const PhysicalRect& reference_border_rect, float offset)
{
    return RoundedBorderGeometry::PixelSnappedRoundedBorderWithOutsets(style, reference_border_rect, PhysicalBoxStrut(LayoutUnit(offset))).GetRadii();
}

// Given 3 points defining a right angle corner, returns the corresponding
// corner in |convex_radii| or |concave_radii|.
gfx::SizeF GetRadiiCorner(
    const FloatRoundedRect::Radii& convex_radii, const FloatRoundedRect::Radii& concave_radii, const SkPoint& p1, const SkPoint& p2, const SkPoint& p3)
{
    if (p1.x() == p2.x()) {
        ...
        DCHECK_EQ(p2.y(), p3.y());
        if (p1.y() < p2.y()) {
            return p2.x() < p3.x() ? concave_radii.BottomLeft() : convex_radii.BottomRight();
        }
    ...
    if (p2.x() != p3.x() || p2.y() == p3.y())
        return gfx::SizeF();
    if (p1.x() < p2.x()) {
        return p2.y() < p3.y() ? convex_radii.TopRight() : concave_radii.BottomRight();
    }
    return p2.y() < p3.y() ? concave_radii.TopLeft() : convex_radii.BottomLeft();
}

// Shorten |line| between rounded corners.
void AdjustLineBetweenCorners(
    Line& line, const FloatRoundedRect::Radii& convex_radii, const FloatRoundedRect::Radii& concave_radii, const SkPoint& prev_point, const SkPoint& next_point)
{
    gfx::SizeF corner1 = GetRadiiCorner(convex_radii, concave_radii, prev_point, line.start, line.end);
    gfx::SizeF corner2 = GetRadiiCorner(convex_radii, concave_radii, line.start, line.end, next_point);
...
void AddCornerRadiiToPath(SkPath& path, const FloatRoundedRect::Radii& convex_radii, const FloatRoundedRect::Radii& concave_radii)
{
    SkPath input;
    input.swap(path);
    IterateRightAnglePath(input, [&](const Vector<Line>& lines) {
        ...
        for (wtf_size_t i = 0; i < lines.size(); i++) {
            ...
            AdjustLineBetweenCorners(new_lines[i], convex_radii, concave_radii, prev_point, next_point);
        }
    ...
    SkPath Next()
    {
        ...
        while (true) {
            ...
            case SkPath::kClose_Verb:
                ...
                GenerateEdgeStrokePath(edge_stroke_path, prev_arc_points_, first_arc_points_);
...
private:
    ...
    void GenerateEdgeStrokePath(SkPath& edge_stroke_path, base::span<const SkPoint> starting_arc_points, base::span<const SkPoint> ending_arc_points)
    {
        ...
        if (starting_arc_points[0] == line_start) {
            ...
            edge_stroke_path.conicTo(starting_arc_points[1], line_start, kCornerConicWeight);
        }
...
public:
    ComplexOutlinePainter(GraphicsContext& context, const Vector<gfx::Rect>& rects, const PhysicalRect& reference_border_rect, const ComputedStyle& style,
        ...
        , is_rounded_(style.HasBorderRadius())
    {
        ...
        } else if (width_ == 1 && (outline_style_ == EBorderStyle::kRidge || outline_style_ == EBorderStyle::kGroove)) {
            outline_style_ = EBorderStyle::kSolid;
            Color dark = color_.Dark();
            color_ = Color((color_.Red() + dark.Red()) / 2, (color_.Green() + dark.Green()) / 2, (color_.Blue() + dark.Blue()) / 2, color_.AlphaAsInteger());
        }
    }

    void Paint()
    {
        if (!ComputeRightAnglePath(right_angle_outer_path_, rects_, offset_, width_)) {
            return;
        }

        bool use_alpha_layer = !color_.IsOpaque() && outline_style_ != EBorderStyle::kSolid && outline_style_ != EBorderStyle::kDouble;
        if (use_alpha_layer) {
        ...
        case EBorderStyle::kSolid:
            context_.FillRect(gfx::SkRectToRectF(outer_path.getBounds()), PaintAutoDarkMode(style_, DarkModeFilter::ElementRole::kBackground));
            break;
        ...
        case EBorderStyle::kOutset:
            PaintInsetOrOutsetOutline(CenterPath(), outline_style_ == EBorderStyle::kInset);
            break;
        default:
            NOTREACHED();
        }
...
private:
    void PaintDoubleOutline()
    {
        ...
        if (is_rounded_) {
            ...
            AddCornerRadiiToPath(inner_third_path, inner_third_radii, outer_third_radii);
            AddCornerRadiiToPath(outer_third_path, outer_third_radii, inner_third_radii);
        }
        AutoDarkMode auto_dark_mode(PaintAutoDarkMode(style_, DarkModeFilter::ElementRole::kBackground));
        ...
        context_.FillRect(gfx::SkRectToRectF(right_angle_outer_path_.getBounds()), auto_dark_mode);
    }

    void PaintDottedOrDashedOutline()
    {
        auto stroke_style = outline_style_ == EBorderStyle::kDashed ? kDashedStroke : kDottedStroke;
        StyledStrokeData styled_stroke;
        styled_stroke.SetStyle(stroke_style);
        if ((width_ % 2) && StyledStrokeData::StrokeIsDashed(width_, stroke_style)) {
        ...
        if (is_rounded_) {
            ...
            const StrokeData stroke_data = styled_stroke.ConvertToStrokeData({ static_cast<int>(path.length()), width_, path.IsClosed() });
            ...
            IterateRightAnglePath(center_path, [this, &styled_stroke, &auto_dark_mode](const Vector<Line>& lines) {
    ...
    void PaintGrooveOrRidgeOutline()
    {
        ...
        PaintInsetOrOutsetOutline(center_path, outline_style_ == EBorderStyle::kGroove);
        // Paint dark color in the inner half.
        context_.ClipPath(center_path, kAntiAliased);
        context_.SetStrokeColor(color_.Dark());
        PaintTopLeftOrBottomRight(center_path, outline_style_ == EBorderStyle::kRidge);
        ...
        context_.SetStrokeColor(color_);
        PaintTopLeftOrBottomRight(center_path, outline_style_ == EBorderStyle::kGroove);
    }
    ...
    void PaintTopLeftOrBottomRight(const SkPath& center_path, bool top_left_or_bottom_right)
    {
        StyledStrokeData styled_stroke;
        // If width_ is odd, draw wider to fill the clip area.
        ...
        AutoDarkMode auto_dark_mode(PaintAutoDarkMode(style_, DarkModeFilter::ElementRole::kBackground));
        IterateRightAnglePath(is_rounded_ ? right_angle_outer_path_ : center_path,
            [this, top_left_or_bottom_right, &rounded_edge_path_iterator, &styled_stroke, &auto_dark_mode](const Vector<Line>& lines) {
                for (wtf_size_t i = 0; i < lines.size(); i++) {
                    const Line& line = lines[i];
                    std::optional<SkPath> rounded_edge_path;
                    if (rounded_edge_path_iterator)
                        rounded_edge_path = rounded_edge_path_iterator->Next();
                    bool is_top_or_left = line.start.x() < line.end.x() || line.start.y() > line.end.y();
                    ...
                    context_.ClipPath(MiterClipPath(prev_line.start, line, next_line.end), kNotAntiAliased);
                    if (is_rounded_) {
    ...
    static int MiterSlope(const SkPoint& p1, const SkPoint& p2, const SkPoint& p3)
    {
        if (p1.x() == p2.x())
    ...
    // the miters.
    SkPath MiterClipPath(const SkPoint& prev_point, const Line& line, const SkPoint& next_point) const
    {
        ...
        SkPoint p1 = SkPoint::Make(line.start.x() + start_miter_slope * (line.start.y() - bounds.top()), bounds.top());
        SkPoint p2 = SkPoint::Make(line.end.x() + end_miter_slope * (line.end.y() - bounds.top()), bounds.top());
        SkPoint p3 = SkPoint::Make(line.end.x() - end_miter_slope * (bounds.bottom() - line.end.y()), bounds.bottom());
        SkPoint p4 = SkPoint::Make(line.start.x() - start_miter_slope * (bounds.bottom() - line.start.y()), bounds.bottom());
    ...
    void PaintStraightEdge(const Line& line, const StyledStrokeData& styled_stroke, const AutoDarkMode& auto_dark_mode)
    {
        ...
        context_.DrawLine(gfx::ToRoundedPoint(gfx::SkPointToPointF(adjusted_line.start)), gfx::ToRoundedPoint(gfx::SkPointToPointF(adjusted_line.end)),
...
FloatRoundedRect::Radii GetFocusRingCornerRadii(const ComputedStyle& style, const PhysicalRect& reference_border_rect, const LayoutObject::OutlineInfo& info)
{
    if (style.HasBorderRadius() && (!style.HasEffectiveAppearance() || style.HasAuthorBorderRadius())) {
    ...
    if (!style.HasAuthorBorder() && style.HasEffectiveAppearance()) {
        ...
        if (part) {
            float corner_radius = ui::NativeTheme::GetInstanceForWeb()->GetBorderRadiusForPart(
                part.value(), reference_border_rect.size.width, reference_border_rect.size.height);
            corner_radius = ui::NativeTheme::GetInstanceForWeb()->AdjustBorderRadiusByZoom(part.value(), corner_radius, style.EffectiveZoom());
...
void PaintSingleFocusRing(GraphicsContext& context, const Vector<gfx::Rect>& rects, float width, int offset, const FloatRoundedRect::Radii& corner_radii,
    const Color& color, const AutoDarkMode& auto_dark_mode)
{
    DCHECK(!rects.empty());
    SkPath path;
    ...
    SkRect rect;
    if (path.isRect(&rect)) {
        context.DrawFocusRingRect(SkRRect(FloatRoundedRect(gfx::SkRectToRectF(rect), corner_radii)), color, width, auto_dark_mode);
        return;
    }
...
void PaintFocusRing(GraphicsContext& context, const Vector<gfx::Rect>& rects, const ComputedStyle& style, const FloatRoundedRect::Radii& corner_radii,
    const LayoutObject::OutlineInfo& info)
{
    ...
    Color outer_color = style.DarkColorScheme() ? Color(0x10, 0x10, 0x10) : Color::kWhite;
    PaintSingleFocusRing(context, rects, outer_ring_width, offset + std::ceil(inner_ring_width), corner_radii, outer_color, AutoDarkMode::Disabled());
    ...
    PaintSingleFocusRing(context, rects, outer_ring_width, offset, corner_radii, inner_color, AutoDarkMode::Disabled());
}

} // anonymous namespace

void OutlinePainter::PaintOutlineRects(const PaintInfo& paint_info, const DisplayItemClient& client, const Vector<PhysicalRect>& outline_rects,
    const LayoutObject::OutlineInfo& info, const ComputedStyle& style)
{
    DCHECK(style.HasOutline());
    DCHECK(!outline_rects.empty());

    if (DrawingRecorder::UseCachedDrawingIfPossible(paint_info.context, client, paint_info.phase))
        return;
    ...
    DrawingRecorder recorder(paint_info.context, client, paint_info.phase, visual_rect);

    if (style.OutlineStyleIsAuto()) {
        auto corner_radii = GetFocusRingCornerRadii(style, outline_rects[0], info);
        PaintFocusRing(paint_info.context, pixel_snapped_outline_rects, style, corner_radii, info);
        return;
    }
    ...
    if (*united_outline_rect == pixel_snapped_outline_rects[0]) {
        ...
        BoxBorderPainter::PaintSingleRectOutline(
            paint_info.context, style, outline_rects[0], info.width, PhysicalBoxStrut(offset.top(), offset.right(), offset.bottom(), offset.left()));
        return;
    }

    ComplexOutlinePainter(paint_info.context, pixel_snapped_outline_rects, outline_rects[0], style, info).Paint();
}

void OutlinePainter::PaintFocusRingPath(GraphicsContext& context, const Path& focus_ring_path, const ComputedStyle& style)
{
    ...
    context.DrawFocusRingPath(focus_ring_path.GetSkPath(), style.VisitedDependentColor(GetCSSPropertyOutlineColor()), FocusRingStrokeWidth(style),
        DefaultFocusRingCornerRadius(style), PaintAutoDarkMode(style, DarkModeFilter::ElementRole::kBackground));
}

int OutlinePainter::OutlineOutsetExtent(const ComputedStyle& style, const LayoutObject::OutlineInfo& info)
{
    if (!style.HasOutline())
        return 0;
    if (style.OutlineStyleIsAuto()) {
        ...
        return FocusRingOffset(style, info) + std::ceil(FocusRingStrokeWidth(style) / 3.f) * 2;
    }
    return base::ClampAdd(info.width, info.offset).Max(0);
}

void OutlinePainter::IterateRightAnglePathForTesting(const SkPath& path, const base::RepeatingCallback<void(const Vector<Line>&)>& contour_action)
{
    IterateRightAnglePath(path, [contour_action](const Vector<Line>& lines) { contour_action.Run(lines); });
}
