...
PaintLayerScrollableArea::PaintLayerScrollableArea(PaintLayer& layer)
    : ScrollableArea(layer.GetLayoutBox()->GetDocument().GetPage()->GetAgentGroupScheduler().CompositorTaskRunner())
    , layer_(&layer)
...
void PaintLayerScrollableArea::DisposeImpl()
{
    ...
    GetLayoutBox()->GetDocument().GetPage()->GlobalRootScrollerController().DidDisposeScrollableArea(*this);
...
void PaintLayerScrollableArea::ApplyPendingHistoryRestoreScrollOffset()
{
    ...
    bool did_restore = RestoreScrollAnchor({ pending_view_state_->state.scroll_anchor_data_, *this });
    if (!did_restore) {
        SetScrollOffset(pending_view_state_->state.scroll_offset_, mojom::blink::ScrollType::kProgrammatic, pending_view_state_->scroll_behavior);
    }

    pending_view_state_.reset();
}

void PaintLayerScrollableArea::SetTickmarksOverride(Vector<gfx::Rect> tickmarks)
{
...
SmoothScrollSequencer* PaintLayerScrollableArea::GetSmoothScrollSequencer() const
{
    if (HasBeenDisposed())
        return nullptr;
...
static int CornerStart(const LayoutBox& box, int min_x, int max_x, int thickness)
{
...
gfx::Rect PaintLayerScrollableArea::CornerRect() const
{
    ...
    if (!VerticalScrollbar() && !HorizontalScrollbar()) {
        ...
        horizontal_thickness = GetPageScrollbarTheme().ScrollbarThickness(ScaleFromDIP(), EScrollbarWidth::kAuto);
    ...
    return gfx::Rect(CornerStart(*GetLayoutBox(), 0, border_box_size.width(), horizontal_thickness),
        border_box_size.height() - vertical_thickness - GetLayoutBox()->StyleRef().BorderBottomWidth(), horizontal_thickness, vertical_thickness);
}
...
gfx::Rect PaintLayerScrollableArea::ScrollCornerRect() const
{
    ...
    if ((has_horizontal_bar && has_vertical_bar) || (has_resizer && (has_horizontal_bar || has_vertical_bar))) {
        return CornerRect();
    }
    return gfx::Rect();
}
...
gfx::Rect PaintLayerScrollableArea::ConvertFromScrollbarToContainingEmbeddedContentView(const Scrollbar& scrollbar, const gfx::Rect& scrollbar_rect) const
{
    ...
    return ToPixelSnappedRect(GetLayoutBox()->LocalToAbsoluteRect(PhysicalRect(rect)));
}

gfx::Point PaintLayerScrollableArea::ConvertFromScrollbarToContainingEmbeddedContentView(const Scrollbar& scrollbar, const gfx::Point& scrollbar_point) const
{
    LayoutView* view = GetLayoutBox()->View();
    if (!view)
    ...
    return ToRoundedPoint(GetLayoutBox()->LocalToAbsolutePoint(PhysicalOffset(point)));
}

gfx::Point PaintLayerScrollableArea::ConvertFromContainingEmbeddedContentViewToScrollbar(const Scrollbar& scrollbar, const gfx::Point& parent_point) const
{
    LayoutView* view = GetLayoutBox()->View();
    if (!view)
        return parent_point;

    gfx::Point point = ToRoundedPoint(GetLayoutBox()->AbsoluteToLocalPoint(PhysicalOffset(parent_point)));
    point -= ScrollbarOffset(scrollbar);
    return point;
}

gfx::Point PaintLayerScrollableArea::ConvertFromRootFrame(const gfx::Point& point_in_root_frame) const
{
    LayoutView* view = GetLayoutBox()->View();
    if (!view)
...
gfx::Point PaintLayerScrollableArea::ConvertFromRootFrameToVisualViewport(const gfx::Point& point_in_root_frame) const
{
...
{
    gfx::Vector2d scroll_dimensions = MaximumScrollOffsetInt() - MinimumScrollOffsetInt();
    return (orientation == kHorizontalScrollbar) ? scroll_dimensions.x() : scroll_dimensions.y();
}

void PaintLayerScrollableArea::UpdateScrollOffset(const ScrollOffset& new_offset, mojom::blink::ScrollType scroll_type)
{
    if (HasBeenDisposed() || GetScrollOffset() == new_offset)
        return;

    TRACE_EVENT2("blink", "PaintLayerScrollableArea::UpdateScrollOffset", "x", new_offset.x(), "y", new_offset.y());
    TRACE_EVENT_INSTANT1("blink", "Type", TRACE_EVENT_SCOPE_THREAD, "type", scroll_type);
    ...
    frame_view->UpdateIntersectionObservationStateOnScroll(new_offset - scroll_offset_);
    ...
    DEVTOOLS_TIMELINE_TRACE_EVENT("ScrollLayer", inspector_scroll_layer_event::Data, GetLayoutBox());
    ...
    if (scroll_type == mojom::blink::ScrollType::kUser || scroll_type == mojom::blink::ScrollType::kCompositor) {
    ...
    if (scroll_type == mojom::blink::ScrollType::kClamping || scroll_type == mojom::blink::ScrollType::kAnchoring) {
    ...
    if (is_root_layer) {
        ...
        if (scroll_type == mojom::blink::ScrollType::kCompositor || scroll_type == mojom::blink::ScrollType::kUser) {
    ...
    if (IsExplicitScrollType(scroll_type) || scroll_type == mojom::blink::ScrollType::kScrollStart) {
        ShowNonMacOverlayScrollbars();
        GetScrollAnchor()->Clear();
    }
    if (ContentCaptureManager* manager = frame_view->GetFrame().LocalFrameRoot().GetOrResetContentCaptureManager()) {
        manager->OnScrollPositionChanged();
    }
    if (AXObjectCache* cache = GetLayoutBox()->GetDocument().ExistingAXObjectCache())
...
void PaintLayerScrollableArea::InvalidatePaintForScrollOffsetChange()
{
    ...
    if (!box->BackgroundNeedsFullPaintInvalidation() && BackgroundNeedsRepaintOnScroll()) {
...
bool PaintLayerScrollableArea::BackgroundNeedsRepaintOnScroll() const
{
    ...
    bool background_paint_in_border_box = background_paint_location & kBackgroundPaintInBorderBoxSpace;
    bool background_paint_in_scrolling_contents = background_paint_location & kBackgroundPaintInContentsSpace;

    const auto& background_layers = box->StyleRef().BackgroundLayers();
    if (background_layers.AnyLayerHasLocalAttachmentImage() && background_paint_in_border_box) {
        // Local-attachment background image scrolls, so needs invalidation if it
        // paints in non-scrolling space.
        return true;
    }
    if (background_layers.AnyLayerHasDefaultAttachmentImage() && background_paint_in_scrolling_contents) {
        // Normal attachment background image doesn't scroll, so needs
        // invalidation if it paints in scrolling contents.
        return true;
    }
    if (background_layers.AnyLayerHasLocalAttachment() && background_layers.AnyLayerUsesContentBox() && background_paint_in_border_box
        && (box->PaddingLeft() || box->PaddingTop() || box->PaddingRight() || box->PaddingBottom())) {
...
{
    if (scroll_offset_ == last_committed_scroll_offset_ && has_last_committed_scroll_offset_)
        return;
...
gfx::Vector2d PaintLayerScrollableArea::MaximumScrollOffsetInt() const
{
    ...
    DCHECK(page);
    TopDocumentRootScrollerController& controller = page->GlobalRootScrollerController();
    ...
    if (this == controller.RootScrollerArea()) {
        ...
        visible_size = ToRoundedSize(GetLayoutBox()->OverflowClipRect(PhysicalOffset(), kIgnoreOverlayScrollbarSize).size);
    }
    ...
    return -ScrollOrigin().OffsetFromOrigin() + gfx::Vector2d(content_size.width() - visible_size.width(), content_size.height() - visible_size.height());
}
...
PhysicalRect PaintLayerScrollableArea::LayoutContentRect(IncludeScrollbarsInRect scrollbar_inclusion) const
{
    ...
    PhysicalSize size(layer_size.width - border_width - scrollbars.HorizontalSum(), layer_size.height - border_height - scrollbars.VerticalSum());
    size.ClampNegativeToZero();
    return PhysicalRect(PhysicalOffset::FromPointFRound(ScrollPosition()), size);
}

gfx::Rect PaintLayerScrollableArea::VisibleContentRect(IncludeScrollbarsInRect scrollbar_inclusion) const
{
...
PhysicalRect PaintLayerScrollableArea::VisibleScrollSnapportRect(IncludeScrollbarsInRect scrollbar_inclusion) const
{
    const ComputedStyle* style = GetLayoutBox()->Style();
    PhysicalRect layout_content_rect(LayoutContentRect(scrollbar_inclusion));
    layout_content_rect.Move(PhysicalOffset(-ScrollOrigin().OffsetFromOrigin()));
    PhysicalBoxStrut padding(MinimumValueForLength(style->ScrollPaddingTop(), layout_content_rect.Height()),
        MinimumValueForLength(style->ScrollPaddingRight(), layout_content_rect.Width()),
        MinimumValueForLength(style->ScrollPaddingBottom(), layout_content_rect.Height()),
        MinimumValueForLength(style->ScrollPaddingLeft(), layout_content_rect.Width()));
...
gfx::Size PaintLayerScrollableArea::ContentsSize() const
{
    ...
    PhysicalOffset offset(GetLayoutBox()->ClientLeft(), GetLayoutBox()->ClientTop());
    // TODO(crbug.com/962299): The pixel snapping is incorrect in some cases.
    return PixelSnappedContentsSize(offset);
}

gfx::Size PaintLayerScrollableArea::PixelSnappedContentsSize(const PhysicalOffset& paint_offset) const
{
    ...
    if (IsA<LayoutView>(GetLayoutBox())) {
        if (auto* transition = ViewTransitionUtils::GetTransition(GetLayoutBox()->GetDocument()); transition && transition->IsRootTransitioning()) {
...
void PaintLayerScrollableArea::ContentsResized()
{
    ...
    GetLayoutBox()->GetFrameView()->SetIntersectionObservationState(LocalFrameView::kDesired);
}

gfx::Point PaintLayerScrollableArea::LastKnownMousePosition() const
{
    return GetLayoutBox()->GetFrame() ? gfx::ToFlooredPoint(GetLayoutBox()->GetFrame()->GetEventHandler().LastKnownMousePositionInRootFrame()) : gfx::Point();
}
...
void PaintLayerScrollableArea::ScrollbarFrameRectChanged()
{
    ...
    if (GetDocument()->Lifecycle().GetState() == DocumentLifecycle::kInPrePaint) {
        ...
        GetLayoutBox()->GetMutableForPainting().SetOnlyThisNeedsPaintPropertyUpdate();
        return;
    }
...
{
    if (orientation == kVerticalScrollbar && GetLayoutBox()->GetDocument().IsVerticalScrollEnforced()) {
        return false;
    }
    ...
    if (IsA<LayoutView>(GetLayoutBox())) {
        ...
        mojom::blink::ScrollbarMode mode = (orientation == kHorizontalScrollbar) ? h_mode : v_mode;
        return mode == mojom::blink::ScrollbarMode::kAuto || mode == mojom::blink::ScrollbarMode::kAlwaysOn;
    }

    EOverflow overflow_style = (orientation == kHorizontalScrollbar) ? GetLayoutBox()->StyleRef().OverflowX() : GetLayoutBox()->StyleRef().OverflowY();
    return (overflow_style == EOverflow::kScroll || overflow_style == EOverflow::kAuto || overflow_style == EOverflow::kOverlay);
}
...
int PaintLayerScrollableArea::PageStep(ScrollbarOrientation orientation) const
{
    ...
    int length = (orientation == kHorizontalScrollbar) ? snapport_size.width() : snapport_size.height();
    int min_page_step = static_cast<float>(length) * ScrollableArea::MinFractionToStepWhenPaging();
...
bool PaintLayerScrollableArea::IsRootFrameLayoutViewport() const
{
    ...
    RootFrameViewport* root_frame_viewport = frame->View()->GetRootFrameViewport();
...
{
    return layer_->IsRootLayer() ? PhysicalSize(GetLayoutBox()->GetFrameView()->Size()) : GetLayoutBox()->Size();
}
...
void PaintLayerScrollableArea::UpdateScrollOrigin()
{
    ...
    scrollable_overflow.Move(-PhysicalOffset(GetLayoutBox()->BorderLeft(), GetLayoutBox()->BorderTop()));
    gfx::Point new_origin = ToFlooredPoint(-scrollable_overflow.offset) + GetLayoutBox()->OriginAdjustmentForScrollbars();
...
void PaintLayerScrollableArea::UpdateScrollDimensions()
{
    ...
    new_overflow_rect.Unite(PhysicalRect(new_overflow_rect.offset, LayoutContentRect(kExcludeScrollbars).size));
...
void PaintLayerScrollableArea::UpdateScrollbarEnabledState(bool is_horizontal_scrollbar_frozen, bool is_vertical_scrollbar_frozen)
{
    bool force_disable = GetPageScrollbarTheme().ShouldDisableInvisibleScrollbars() && ScrollbarsHiddenIfOverlay();
    ...
    if (Scrollbar* horizontal_scrollbar = HorizontalScrollbar()) {
        if (!horizontal_scrollbar->IsCustomScrollbar() || !is_horizontal_scrollbar_frozen) {
            horizontal_scrollbar->SetEnabled(HasHorizontalOverflow() && !force_disable);
        }
    }

    if (Scrollbar* vertical_scrollbar = VerticalScrollbar()) {
        if (!vertical_scrollbar->IsCustomScrollbar() || !is_vertical_scrollbar_frozen) {
...
void PaintLayerScrollableArea::SetScrollOffsetUnconditionally(const ScrollOffset& offset, mojom::blink::ScrollType scroll_type)
{
...
void PaintLayerScrollableArea::UpdateAfterLayout()
{
    ...
    if (!is_horizontal_scrollbar_frozen && !is_vertical_scrollbar_frozen && TryRemovingAutoScrollbars(needs_horizontal_scrollbar, needs_vertical_scrollbar)) {
        needs_horizontal_scrollbar = needs_vertical_scrollbar = false;
    }

    bool horizontal_scrollbar_should_change = needs_horizontal_scrollbar != had_horizontal_scrollbar;
    bool vertical_scrollbar_should_change = needs_vertical_scrollbar != had_vertical_scrollbar;

    bool scrollbars_will_change
        = (horizontal_scrollbar_should_change && !is_horizontal_scrollbar_frozen) || (vertical_scrollbar_should_change && !is_vertical_scrollbar_frozen);
    if (scrollbars_will_change) {
        ...
        if (LocalFrameView* frame_view = GetLayoutBox()->GetFrameView()) {
            VisualViewport& visual_viewport = GetLayoutBox()->GetFrame()->GetPage()->GetVisualViewport();
            if (this == frame_view->LayoutViewport() && visual_viewport.IsActiveViewport()) {
        ...
        if (((horizontal_scrollbar_should_change && GetLayoutBox()->StyleRef().OverflowX() != EOverflow::kOverlay)
                || (vertical_scrollbar_should_change && GetLayoutBox()->StyleRef().OverflowY() != EOverflow::kOverlay))) {
            if ((vertical_scrollbar_should_change && GetLayoutBox()->IsHorizontalWritingMode())
                || (horizontal_scrollbar_should_change && !GetLayoutBox()->IsHorizontalWritingMode())) {
    ...
    {
        UpdateScrollbarEnabledState(is_horizontal_scrollbar_frozen, is_vertical_scrollbar_frozen);
    ...
    if (NeedsHypotheticalScrollbarThickness(kHorizontalScrollbar)) {
        hypothetical_horizontal_scrollbar_thickness_ = ComputeHypotheticalScrollbarThickness(kHorizontalScrollbar, true);
    }
    hypothetical_vertical_scrollbar_thickness_ = 0;
    if (NeedsHypotheticalScrollbarThickness(kVerticalScrollbar)) {
        hypothetical_vertical_scrollbar_thickness_ = ComputeHypotheticalScrollbarThickness(kVerticalScrollbar, true);
    }
...
void PaintLayerScrollableArea::SetShouldCheckForPaintInvalidation()
{
    ...
    if (GetLayoutBox()->GetDocument().Lifecycle().GetState() == DocumentLifecycle::DocumentLifecycle::kInPrePaint) {
...
bool PaintLayerScrollableArea::IsApplyingScrollStart() const
{
    if (Element* element = GetElementForScrollStart()) {
        ...
        if (RuntimeEnabledFeatures::CSSScrollStartTargetEnabled() && GetScrollStartTarget()) {
            return true;
        }
        return RuntimeEnabledFeatures::CSSScrollStartEnabled() && !ScrollStartIsDefault();
    }
    return false;
}
...
void PaintLayerScrollableArea::ClampScrollOffsetAfterOverflowChangeInternal()
{
    ...
    if (ScrollOriginChanged()) {
        ...
        ScrollableArea::SetScrollOffset(GetScrollOffset(), mojom::blink::ScrollType::kClamping);
    }
...
void PaintLayerScrollableArea::DidChangeGlobalRootScroller()
{
    ...
    if (GetLayoutBox()->GetFrame()->GetSettings() && GetLayoutBox()->GetFrame()->GetSettings()->GetViewportEnabled()) {
        bool needs_horizontal_scrollbar;
        bool needs_vertical_scrollbar;
        ComputeScrollbarExistence(needs_horizontal_scrollbar, needs_vertical_scrollbar);
        SetHasHorizontalScrollbar(needs_horizontal_scrollbar);
...
{
    return scroll_anchor_.HasScroller() && GetLayoutBox() && GetLayoutBox()->StyleRef().OverflowAnchor() != EOverflowAnchor::kNone
        && !GetLayoutBox()->GetDocument().FinishingOrIsPrinting();
}

bool PaintLayerScrollableArea::RestoreScrollAnchor(const SerializedAnchor& serialized_anchor)
{
    return ShouldPerformScrollAnchoring() && scroll_anchor_.RestoreAnchor(serialized_anchor);
}

gfx::QuadF PaintLayerScrollableArea::LocalToVisibleContentQuad(const gfx::QuadF& quad, const LayoutObject* local_object, MapCoordinatesFlags flags) const
{
    LayoutBox* box = GetLayoutBox();
    if (!box)
        return quad;
...
scoped_refptr<base::SingleThreadTaskRunner> PaintLayerScrollableArea::GetTimerTaskRunner() const
{
    return GetLayoutBox()->GetFrame()->GetTaskRunner(TaskType::kInternalDefault);
}

mojom::blink::ScrollBehavior PaintLayerScrollableArea::ScrollBehaviorStyle() const
{
    return GetLayoutBox()->StyleRef().GetScrollBehavior();
}

mojom::blink::ColorScheme PaintLayerScrollableArea::UsedColorSchemeScrollbars() const
{
    ...
    if (IsGlobalRootNonOverlayScroller() && layout_box->StyleRef().ColorSchemeFlagsIsNormal()) {
        const auto& document = layout_box->GetDocument();
        if (document.GetPreferredColorScheme() == mojom::blink::PreferredColorScheme::kDark
            && document.GetSettings()->GetPreferredRootScrollbarColorScheme() == mojom::blink::PreferredColorScheme::kDark) {
            UseCounter::Count(GetLayoutBox()->GetDocument(), WebFeature::kUsedColorSchemeRootScrollbarsDark);
...
bool PaintLayerScrollableArea::UsedColorSchemeScrollbarsChanged(const ComputedStyle* old_style) const
{
    if (!old_style) {
        return false;
    }

    if (old_style->UsedColorScheme() != GetLayoutBox()->StyleRef().UsedColorScheme()) {
        return true;
    }
    ...
    if (IsGlobalRootNonOverlayScroller() && old_style->ColorSchemeFlagsIsNormal() != GetLayoutBox()->StyleRef().ColorSchemeFlagsIsNormal()) {
        return true;
    }
...
{
    return GetLayoutBox()->IsGlobalRootScroller() && !GetPageScrollbarTheme().UsesOverlayScrollbars();
}
...
bool PaintLayerScrollableArea::HasHorizontalOverflow() const
{
    ...
    LayoutUnit client_width = LayoutContentRect(kIncludeScrollbars).Width() - VerticalScrollbarWidth(kIgnoreOverlayScrollbarSize);
...
{
    LayoutUnit client_height = LayoutContentRect(kIncludeScrollbars).Height() - HorizontalScrollbarHeight(kIgnoreOverlayScrollbarSize);
...
void PaintLayerScrollableArea::UpdateAfterStyleChange(const ComputedStyle* old_style)
{
    ...
    bool needs_horizontal_scrollbar;
    bool needs_vertical_scrollbar;
    ComputeScrollbarExistence(needs_horizontal_scrollbar, needs_vertical_scrollbar, kOverflowIndependent);
    ...
    if (!old_style || UsedColorSchemeScrollbarsChanged(old_style)
        || old_style->ScrollbarThumbColorResolved() != GetLayoutBox()->StyleRef().ScrollbarThumbColorResolved()
        || old_style->ScrollbarTrackColorResolved() != GetLayoutBox()->StyleRef().ScrollbarTrackColorResolved()) {
...
void PaintLayerScrollableArea::UpdateAfterOverflowRecalc()
{
    ...
    bool horizontal_scrollbar_should_change = needs_horizontal_scrollbar != HasHorizontalScrollbar();
    bool vertical_scrollbar_should_change = needs_vertical_scrollbar != HasVerticalScrollbar();

    if ((GetLayoutBox()->HasAutoHorizontalScrollbar() && horizontal_scrollbar_should_change)
        || (GetLayoutBox()->HasAutoVerticalScrollbar() && vertical_scrollbar_should_change)) {
        GetLayoutBox()->SetNeedsLayoutAndFullPaintInvalidation(layout_invalidation_reason::kUnknown);
    }
...
gfx::Rect PaintLayerScrollableArea::RectForHorizontalScrollbar() const
{
    ...
    gfx::Size border_box_size = PixelSnappedBorderBoxSize();
    return gfx::Rect(HorizontalScrollbarStart(),
        border_box_size.height() - GetLayoutBox()->BorderBottom().ToInt() - HorizontalScrollbar()->ScrollbarThickness(),
        border_box_size.width() - (GetLayoutBox()->BorderLeft() + GetLayoutBox()->BorderRight()).ToInt() - scroll_corner.width(),
...
gfx::Rect PaintLayerScrollableArea::RectForVerticalScrollbar() const
{
    ...
    const gfx::Rect& scroll_corner = ScrollCornerRect();
    return gfx::Rect(VerticalScrollbarStart(), GetLayoutBox()->BorderTop().ToInt(), VerticalScrollbar()->ScrollbarThickness(),
        PixelSnappedBorderBoxSize().height() - (GetLayoutBox()->BorderTop() + GetLayoutBox()->BorderBottom()).ToInt() - scroll_corner.height());
}
...
int PaintLayerScrollableArea::VerticalScrollbarStart() const
{
    ...
    return PixelSnappedBorderBoxSize().width() - GetLayoutBox()->BorderRight().ToInt() - VerticalScrollbar()->ScrollbarThickness();
}
...
int PaintLayerScrollableArea::HorizontalScrollbarStart() const
{
    ...
    if (GetLayoutBox()->ShouldPlaceBlockDirectionScrollbarOnLogicalLeft()) {
        x += HasVerticalScrollbar() ? VerticalScrollbar()->ScrollbarThickness() : ResizerCornerRect(kResizerForPointer).width();
    }
    return x;
}

gfx::Vector2d PaintLayerScrollableArea::ScrollbarOffset(const Scrollbar& scrollbar) const
{
    ...
    if (&scrollbar == VerticalScrollbar()) {
        return gfx::Vector2d(VerticalScrollbarStart(), GetLayoutBox()->BorderTop().ToInt());
    }

    if (&scrollbar == HorizontalScrollbar()) {
        return gfx::Vector2d(HorizontalScrollbarStart(),
            GetLayoutBox()->BorderTop().ToInt() + VisibleContentRect(kIncludeScrollbars).height() - HorizontalScrollbar()->ScrollbarThickness());
    }

    NOTREACHED();
}

static inline const LayoutObject& ScrollbarStyleSource(const LayoutBox& layout_box)
{
    if (IsA<LayoutView>(layout_box)) {
        ...
        if (Settings* settings = doc.GetSettings()) {
            ...
            if (!settings->GetAllowCustomScrollbarInMainFrame() && frame->IsMainFrame() && viewport.IsActiveViewport()) {
                return layout_box;
            }
        }
        ...
        if (body && body->GetLayoutObject() && body->GetLayoutObject()->IsBox() && body->GetLayoutObject()->StyleRef().HasCustomScrollbarStyle(body)) {
            return *body->GetLayoutObject();
        }

        // If the <body> didn't have a custom style, then the root element might.
        Element* doc_element = doc.documentElement();
        if (doc_element && doc_element->GetLayoutObject() && doc_element->GetLayoutObject()->StyleRef().HasCustomScrollbarStyle(doc_element)
...
int PaintLayerScrollableArea::HypotheticalScrollbarThickness(ScrollbarOrientation orientation, bool should_include_overlay_thickness) const
{
    ...
    if (should_include_overlay_thickness && GetLayoutBox()->GetDocument().Lifecycle().GetState() >= DocumentLifecycle::kLayoutClean) {
        return orientation == kHorizontalScrollbar ? hypothetical_horizontal_scrollbar_thickness_ : hypothetical_vertical_scrollbar_thickness_;
    }
    return ComputeHypotheticalScrollbarThickness(orientation, should_include_overlay_thickness);
}
...
bool PaintLayerScrollableArea::NeedsHypotheticalScrollbarThickness(ScrollbarOrientation orientation) const
{
    return GetLayoutBox()->IsLayoutView() || GetLayoutBox()->HasScrollbarGutters(orientation);
}

int PaintLayerScrollableArea::ComputeHypotheticalScrollbarThickness(ScrollbarOrientation orientation, bool should_include_overlay_thickness) const
{
    Scrollbar* scrollbar = orientation == kHorizontalScrollbar ? HorizontalScrollbar() : VerticalScrollbar();
    if (scrollbar)
        return scrollbar->ScrollbarThickness();

    const LayoutObject& style_source = ScrollbarStyleSource(*GetLayoutBox());
    if (style_source.StyleRef().HasCustomScrollbarStyle(GetElementForScrollStart())) {
        return CustomScrollbar::HypotheticalScrollbarThickness(this, orientation, &style_source);
    }

    ScrollbarTheme& theme = GetPageScrollbarTheme();
    if (theme.UsesOverlayScrollbars() && !should_include_overlay_thickness)
        return 0;
    return theme.ScrollbarThickness(ScaleFromDIP(), style_source.StyleRef().UsedScrollbarWidth());
}
...
bool PaintLayerScrollableArea::NeedsScrollbarReconstruction() const
{
    ...
    const LayoutObject& style_source = ScrollbarStyleSource(*GetLayoutBox());
    bool needs_custom = style_source.IsBox() && style_source.StyleRef().HasCustomScrollbarStyle(GetElementForScrollStart());
...
void PaintLayerScrollableArea::ComputeScrollbarExistence(
    bool& needs_horizontal_scrollbar, bool& needs_vertical_scrollbar, ComputeScrollbarExistenceOption option) const
{
    // Scrollbars may be hidden or provided by visual viewport or frame instead.
    DCHECK(GetLayoutBox()->GetFrame()->GetSettings());
    if (VisualViewportSuppliesScrollbars() || !CanHaveOverflowScrollbars(*GetLayoutBox()) || GetLayoutBox()->GetFrame()->GetSettings()->GetHideScrollbars()
        || GetLayoutBox()->IsFieldset() || GetLayoutBox()->IsFrameSet() || GetLayoutBox()->StyleRef().UsedScrollbarWidth() == EScrollbarWidth::kNone) {
    ...
    // First, determine what behavior the scrollbars say they should have.
    {
        if (auto* layout_view = DynamicTo<LayoutView>(GetLayoutBox())) {
            ...
            } else if (overflow_x == EOverflow::kHidden || overflow_x == EOverflow::kVisible) {
            ...
            } else if (overflow_y == EOverflow::kHidden || overflow_y == EOverflow::kVisible) {
        ...
        bool has_custom_scrollbar_style = ScrollbarStyleSource(*GetLayoutBox()).StyleRef().HasCustomScrollbarStyle(GetElementForScrollStart());
        bool will_be_overlay = GetPageScrollbarTheme().UsesOverlayScrollbars() && !has_custom_scrollbar_style;
        if (will_be_overlay) {
    ...
    // depend on overflow.
    {
        if (h_mode == mojom::blink::ScrollbarMode::kAuto) {
            ...
            needs_horizontal_scrollbar = GetLayoutBox()->IsRooted() && HasHorizontalOverflow() && VisibleContentRect(kIncludeScrollbars).height();
        }
        if (v_mode == mojom::blink::ScrollbarMode::kAuto) {
            needs_vertical_scrollbar = GetLayoutBox()->IsRooted() && HasVerticalOverflow() && VisibleContentRect(kIncludeScrollbars).width();
        }
    }
}

bool PaintLayerScrollableArea::TryRemovingAutoScrollbars(const bool& needs_horizontal_scrollbar, const bool& needs_vertical_scrollbar)
{
    ...
    if (auto* layout_view = DynamicTo<LayoutView>(GetLayoutBox())) {
        ...
        mojom::blink::ScrollbarMode v_mode;
        layout_view->CalculateScrollbarModes(h_mode, v_mode);
        if (h_mode != mojom::blink::ScrollbarMode::kAuto || v_mode != mojom::blink::ScrollbarMode::kAuto)
            return false;

        gfx::Size visible_size_with_scrollbars = VisibleContentRect(kIncludeScrollbars).size();
        if (ScrollWidth() <= visible_size_with_scrollbars.width() && ScrollHeight() <= visible_size_with_scrollbars.height()) {
            return true;
        }
    } else {
        if (!GetLayoutBox()->HasAutoVerticalScrollbar() || !GetLayoutBox()->HasAutoHorizontalScrollbar())
            return false;

        PhysicalSize client_size_with_scrollbars = LayoutContentRect(kIncludeScrollbars).size;
        if (ScrollWidth() <= client_size_with_scrollbars.width && ScrollHeight() <= client_size_with_scrollbars.height) {
            return true;
        }
    }
...
CompositorElementId PaintLayerScrollableArea::GetScrollCornerElementId() const
{
    ...
    return CompositorElementIdWithNamespace(scrollable_element_id, CompositorElementIdNamespace::kScrollCorner);
}
...
int PaintLayerScrollableArea::VerticalScrollbarWidth(OverlayScrollbarClipBehavior overlay_scrollbar_clip_behavior) const
{
    if (!HasVerticalScrollbar())
        return 0;
    if (overlay_scrollbar_clip_behavior == kIgnoreOverlayScrollbarSize && GetLayoutBox()->StyleRef().OverflowY() == EOverflow::kOverlay) {
        return 0;
    }
    if ((overlay_scrollbar_clip_behavior == kIgnoreOverlayScrollbarSize || !VerticalScrollbar()->ShouldParticipateInHitTesting())
...
int PaintLayerScrollableArea::HorizontalScrollbarHeight(OverlayScrollbarClipBehavior overlay_scrollbar_clip_behavior) const
{
    if (!HasHorizontalScrollbar())
        return 0;
    if (overlay_scrollbar_clip_behavior == kIgnoreOverlayScrollbarSize && GetLayoutBox()->StyleRef().OverflowX() == EOverflow::kOverlay) {
        return 0;
    }
    if ((overlay_scrollbar_clip_behavior == kIgnoreOverlayScrollbarSize || !HorizontalScrollbar()->ShouldParticipateInHitTesting())
...
const cc::SnapContainerData* PaintLayerScrollableArea::GetSnapContainerData() const
{
    return RareData() && RareData()->snap_container_data_ ? &RareData()->snap_container_data_.value() : nullptr;
}

void PaintLayerScrollableArea::SetSnapContainerData(std::optional<cc::SnapContainerData> data)
{
    EnsureRareData().snap_container_data_ = data;
}

bool PaintLayerScrollableArea::SetTargetSnapAreaElementIds(cc::TargetSnapAreaElementIds snap_target_ids)
{
    if (!RareData() || !RareData()->snap_container_data_)
        return false;
    if (RareData()->snap_container_data_.value().SetTargetSnapAreaElementIds(snap_target_ids)) {
        GetLayoutBox()->SetNeedsPaintPropertyUpdate();
        return true;
    }
    return false;
...
void PaintLayerScrollableArea::UpdateFocusDataForSnapAreas()
{
    ...
    std::optional<cc::SnapContainerData>& container_data = RareData()->snap_container_data_;
    ...
    for (auto& fragment : layout_box->PhysicalFragments()) {
        if (auto* snap_areas = fragment.SnapAreas()) {
            for (Element* snap_area : *snap_areas) {
                cc::ElementId element_id = CompositorElementIdFromDOMNodeId(snap_area->GetDomNodeId());
                container_data->UpdateSnapAreaFocus(id_to_index.at(element_id), snap_area->HasFocusWithin());
            }
        }
    }
}

std::optional<cc::TargetSnapAreaElementIds> PaintLayerScrollableArea::GetScrollsnapchangingTargetIds() const
{
    return RareData() ? RareData()->scrollsnapchanging_target_ids_ : std::nullopt;
}

const cc::SnapSelectionStrategy* PaintLayerScrollableArea::GetImplSnapStrategy() const
{
    return RareData() ? RareData()->impl_snap_strategy_.get() : nullptr;
}

void PaintLayerScrollableArea::SetImplSnapStrategy(std::unique_ptr<cc::SnapSelectionStrategy> strategy)
{
    EnsureRareData().impl_snap_strategy_ = std::move(strategy);
}

std::optional<gfx::PointF> PaintLayerScrollableArea::GetSnapPositionAndSetTarget(const cc::SnapSelectionStrategy& strategy)
{
    if (!RareData() || !RareData()->snap_container_data_)
        return std::nullopt;
...
bool PaintLayerScrollableArea::HasOverlayOverflowControls() const
{
    ...
    if (!HasScrollbar() && GetLayoutBox()->CanResize())
        return true;
    if (GetLayoutBox()->StyleRef().OverflowX() == EOverflow::kOverlay || GetLayoutBox()->StyleRef().OverflowY() == EOverflow::kOverlay)
        return true;
    return false;
}
...
bool PaintLayerScrollableArea::ShouldOverflowControlsPaintAsOverlay() const
{
    ...
    return layer_->IsRootLayer() || (GetLayoutBox() && GetLayoutBox()->IsGlobalRootScroller());
}
...
void PaintLayerScrollableArea::PositionOverflowControls()
{
    ...
    if (Scrollbar* horizontal_scrollbar = HorizontalScrollbar()) {
        ...
        if (auto* custom_scrollbar = DynamicTo<CustomScrollbar>(horizontal_scrollbar))
            custom_scrollbar->PositionScrollbarParts();
    }
    ...
    if (scroll_corner_) {
        ...
        scroll_corner_->GetMutableForPainting().FirstFragment().SetPaintOffset(rect.offset);
    }
    ...
    if (resizer_) {
        ...
        // TODO(crbug.com/1020913): This should be part of PaintPropertyTreeBuilder
        // when we support subpixel layout of overflow controls.
        resizer_->GetMutableForPainting().FirstFragment().SetPaintOffset(rect.offset);
    }
}
...
void PaintLayerScrollableArea::UpdateScrollCornerStyle()
{
    ...
    const LayoutObject& style_source = ScrollbarStyleSource(*GetLayoutBox());
    bool uses_standard_scrollbar_style = style_source.StyleRef().UsesStandardScrollbarStyle();
    const ComputedStyle* corner = (GetLayoutBox()->IsScrollContainer() && !uses_standard_scrollbar_style)
        ? style_source.GetUncachedPseudoElementStyle(StyleRequest(kPseudoIdScrollbarCorner, style_source.Style()))
        : nullptr;
    if (corner) {
        if (!scroll_corner_) {
            scroll_corner_ = LayoutCustomScrollbarPart::CreateAnonymous(&GetLayoutBox()->GetDocument(), this);
        }
...
bool PaintLayerScrollableArea::HitTestOverflowControls(HitTestResult& result, const gfx::Point& local_point)
{
    if (!HasOverflowControls())
        return false;
    ...
    if (HasVerticalScrollbar() && VerticalScrollbar()->ShouldParticipateInHitTesting()) {
        gfx::Rect v_bar_rect(VerticalScrollbarStart(), GetLayoutBox()->BorderTop().ToInt(), VerticalScrollbar()->ScrollbarThickness(),
            visible_rect.height() - (HasHorizontalScrollbar() ? HorizontalScrollbar()->ScrollbarThickness() : resize_control_size));
    ...
    if (HasHorizontalScrollbar() && HorizontalScrollbar()->ShouldParticipateInHitTesting()) {
        ...
        gfx::Rect h_bar_rect(HorizontalScrollbarStart(), GetLayoutBox()->BorderTop().ToInt() + visible_rect.height() - h_scrollbar_thickness,
            visible_rect.width() - (HasVerticalScrollbar() ? VerticalScrollbar()->ScrollbarThickness() : resize_control_size), h_scrollbar_thickness);
    ...
    if (scroll_corner_ && ScrollCornerRect().Contains(local_point)) {
        if (GetLayoutBox() && GetLayoutBox()->GetFrame()) {
            base::debug::CrashKeyString* crash_key = GetLayoutBox()->GetFrame()->GetEventHandler().CrashKeyForBug1519197();
            base::debug::SetCrashKeyString(crash_key, GetLayoutBox()->DebugName().Utf8());
        }
...
gfx::Rect PaintLayerScrollableArea::ResizerCornerRect(ResizerHitTestType resizer_hit_test_type) const
{
    if (!GetLayoutBox()->CanResize())
        return gfx::Rect();
    ...
    if (resizer_hit_test_type == kResizerForTouch) {
        ...
        corner.Offset(-corner.width() * (kResizerControlExpandRatioForTouch - 1), -corner.height() * (kResizerControlExpandRatioForTouch - 1));
        corner.set_size(gfx::Size(corner.width() * kResizerControlExpandRatioForTouch, corner.height() * kResizerControlExpandRatioForTouch));
    }
...
bool PaintLayerScrollableArea::IsAbsolutePointInResizeControl(const gfx::Point& absolute_point, ResizerHitTestType resizer_hit_test_type) const
{
    if (GetLayoutBox()->StyleRef().Visibility() != EVisibility::kVisible || !GetLayoutBox()->CanResize()) {
        return false;
    }

    gfx::Point local_point = ToRoundedPoint(GetLayoutBox()->AbsoluteToLocalPoint(PhysicalOffset(absolute_point)));
    return ResizerCornerRect(resizer_hit_test_type).Contains(local_point);
}

bool PaintLayerScrollableArea::IsLocalPointInResizeControl(const gfx::Point& local_point, ResizerHitTestType resizer_hit_test_type) const
{
    if (GetLayoutBox()->StyleRef().Visibility() != EVisibility::kVisible || !GetLayoutBox()->CanResize()) {
        return false;
    }
...
void PaintLayerScrollableArea::UpdateResizerStyle(const ComputedStyle* old_style)
{
    ...
    const ComputedStyle* resizer
        = GetLayoutBox()->IsScrollContainer() ? style_source.GetUncachedPseudoElementStyle(StyleRequest(kPseudoIdResizer, style_source.Style())) : nullptr;
    if (resizer) {
        if (!resizer_) {
            resizer_ = LayoutCustomScrollbarPart::CreateAnonymous(&GetLayoutBox()->GetDocument(), this);
        }
    ...
    for (const auto& fragment : GetLayoutBox()->PhysicalFragments()) {
        if (auto* sticky_descendants = fragment.StickyDescendants()) {
            for (auto& sticky_descendant : *sticky_descendants) {
                auto* constraints = sticky_descendant->ComputeStickyPositionConstraints();
    ...
    for (const auto& fragment : GetLayoutBox()->PhysicalFragments()) {
        if (auto* sticky_descendants = fragment.StickyDescendants()) {
            for (auto& sticky_descendant : *sticky_descendants) {
                ...
                sticky_descendant->StickyConstraints()->ComputeStickyOffset(ScrollPosition());
            }
        }
    }
}

gfx::Vector2d PaintLayerScrollableArea::OffsetFromResizeCorner(const gfx::Point& absolute_point) const
{
    ...
    gfx::Point local_point = ToRoundedPoint(GetLayoutBox()->AbsoluteToLocalPoint(PhysicalOffset(absolute_point)));
    return gfx::Vector2d(local_point.x() - element_size.width(), local_point.y() - element_size.height());
}

void PaintLayerScrollableArea::Resize(const gfx::Point& pos, const gfx::Vector2d& old_offset)
{
    // FIXME: This should be possible on generated content but is not right now.
    if (!InResizeMode() || !GetLayoutBox()->CanResize() || !GetLayoutBox()->GetNode())
        return;
    ...
    gfx::Vector2d new_offset = OffsetFromResizeCorner(document.View()->ConvertFromRootFrame(pos));
    ...
    PhysicalSize new_size(current_size.width + offset.left, current_size.height + offset.top);
    ...
    bool is_box_sizing_border = GetLayoutBox()->StyleRef().BoxSizing() == EBoxSizing::kBorderBox;

    EResize resize = GetLayoutBox()->StyleRef().UsedResize();
    if (resize != EResize::kVertical && difference.width) {
        LayoutUnit base_width = GetLayoutBox()->Size().width - (is_box_sizing_border ? LayoutUnit() : GetLayoutBox()->BorderAndPaddingWidth());
        base_width = LayoutUnit(base_width / zoom_factor);
        element->SetInlineStyleProperty(CSSPropertyID::kWidth, RoundToInt(base_width + difference.width), CSSPrimitiveValue::UnitType::kPixels);
    }

    if (resize != EResize::kHorizontal && difference.height) {
        LayoutUnit base_height = GetLayoutBox()->Size().height - (is_box_sizing_border ? LayoutUnit() : GetLayoutBox()->BorderAndPaddingHeight());
        base_height = LayoutUnit(base_height / zoom_factor);
        element->SetInlineStyleProperty(CSSPropertyID::kHeight, RoundToInt(base_height + difference.height), CSSPrimitiveValue::UnitType::kPixels);
    }
...
{
    PhysicalOffset border_origin_to_scroll_origin(-GetLayoutBox()->BorderLeft(), -GetLayoutBox()->BorderTop());
    // There might be scroll bar between border_origin and scroll_origin.
    gfx::Vector2d scroll_bar_adjustment = GetLayoutBox()->OriginAdjustmentForScrollbars();
    border_origin_to_scroll_origin.left -= scroll_bar_adjustment.x();
    border_origin_to_scroll_origin.top -= scroll_bar_adjustment.y();
    border_origin_to_scroll_origin += PhysicalOffset::FromVector2dFFloor(GetScrollOffset());
    return border_origin_to_scroll_origin;
}

PhysicalRect PaintLayerScrollableArea::ScrollIntoView(
    const PhysicalRect& absolute_rect, const PhysicalBoxStrut& scroll_margin, const mojom::blink::ScrollIntoViewParamsPtr& params)
{
    ...
    PhysicalRect local_expose_rect = GetLayoutBox()->AbsoluteToLocalRect(absolute_rect);
    ScrollOffset target_offset
        = scroll_into_view_util::GetScrollOffsetToExpose(*this, local_expose_rect, scroll_margin, *params->align_x.get(), *params->align_y.get());
    ScrollOffset new_scroll_offset(ClampScrollOffset(gfx::ToRoundedVector2d(target_offset)));
    ...
    std::unique_ptr<cc::SnapSelectionStrategy> strategy = cc::SnapSelectionStrategy::CreateForEndPosition(end_point, true, true);
    ...
    if (params->is_for_scroll_sequence) {
        mojom::blink::ScrollBehavior behavior = DetermineScrollBehavior(params->behavior, GetLayoutBox()->StyleRef().GetScrollBehavior());
        if (RuntimeEnabledFeatures::MultiSmoothScrollIntoViewEnabled()) {
            ...
            DCHECK(params->type == mojom::blink::ScrollType::kProgrammatic || params->type == mojom::blink::ScrollType::kUser);
            GetSmoothScrollSequencer()->QueueAnimation(this, new_scroll_offset, behavior);
        }
    } else {
        SetScrollOffset(new_scroll_offset, params->type, mojom::blink::ScrollBehavior::kInstant);
    }
    ...
    local_expose_rect.Move(-PhysicalOffset::FromVector2dFRound(scroll_offset_difference));

    // Represent the rects in the container's border-box coordinate.
    PhysicalRect scroll_snapport_rect = VisibleScrollSnapportRect() - LocalToScrollOriginOffset();
    PhysicalRect intersect = Intersection(scroll_snapport_rect, local_expose_rect);

    if (intersect.IsEmpty() && !scroll_snapport_rect.IsEmpty() && !local_expose_rect.IsEmpty()) {
...
void PaintLayerScrollableArea::UpdateScrollableAreaSet()
{
    ...
    bool has_overflow = !GetLayoutBox()->Size().IsZero()
        && ((has_horizontal_overflow && GetLayoutBox()->ScrollsOverflowX()) || (has_vertical_overflow && GetLayoutBox()->ScrollsOverflowY()));

    bool overflows_in_block_direction = GetLayoutBox()->IsHorizontalWritingMode() ? has_vertical_overflow : has_horizontal_overflow;
    ...
    bool is_visible = GetLayoutBox()->StyleRef().Visibility() == EVisibility::kVisible;
    bool did_scroll_overflow = scrolls_overflow_;
    if (auto* layout_view = DynamicTo<LayoutView>(GetLayoutBox())) {
        mojom::blink::ScrollbarMode h_mode;
        mojom::blink::ScrollbarMode v_mode;
        layout_view->CalculateScrollbarModes(h_mode, v_mode);
        if (h_mode == mojom::blink::ScrollbarMode::kAlwaysOff && v_mode == mojom::blink::ScrollbarMode::kAlwaysOff)
            has_overflow = false;
    }
    ...
    if (RuntimeEnabledFeatures::ImplicitRootScrollerEnabled() && scrolls_overflow_) {
        if (IsA<LayoutView>(GetLayoutBox())) {
            if (Element* owner = GetLayoutBox()->GetDocument().LocalOwner()) {
                owner->GetDocument().GetRootScrollerController().ConsiderForImplicit(*owner);
            }
        } else {
            ...
            if (GetLayoutBox()->GetNode()) {
                GetLayoutBox()->GetDocument().GetRootScrollerController().ConsiderForImplicit(*GetLayoutBox()->GetNode());
            }
        }
    }
...
ScrollingCoordinator* PaintLayerScrollableArea::GetScrollingCoordinator() const
{
    LocalFrame* frame = GetLayoutBox()->GetFrame();
    if (!frame)
        return nullptr;
...
{
    DCHECK_GE(GetDocument()->Lifecycle().GetState(), DocumentLifecycle::kPaintClean);
    if (HasBeenDisposed()) {
        return true;
    }
    ...
    if (const auto* paint_artifact_compositor = GetLayoutBox()->GetFrameView()->GetPaintArtifactCompositor()) {
        if (const auto* properties = GetLayoutBox()->FirstFragment().PaintProperties()) {
            if (const auto* scroll = properties->Scroll()) {
                return paint_artifact_compositor->GetMainThreadRepaintReasons(*scroll) != cc::MainThreadScrollingReason::kNotScrollingOnMain;
            }
        }
    }
    return true;
...
bool PaintLayerScrollableArea::UsesCompositedScrolling() const
{
    ...
    const auto* paint_artifact_compositor = GetLayoutBox()->GetFrameView()->GetPaintArtifactCompositor();
    return paint_artifact_compositor && paint_artifact_compositor->UsesCompositedScrolling(*properties->Scroll());
}
...
bool PaintLayerScrollableArea::VisualViewportSuppliesScrollbars() const
{
    ...
    const TopDocumentRootScrollerController& controller = GetLayoutBox()->GetDocument().GetPage()->GlobalRootScrollerController();
    return controller.RootScrollerArea() == this;
}

bool PaintLayerScrollableArea::ScheduleAnimation()
{
    if (ChromeClient* client = GetLayoutBox()->GetFrameView()->GetChromeClient()) {
...
cc::AnimationHost* PaintLayerScrollableArea::GetCompositorAnimationHost() const
{
    return layer_->GetLayoutObject().GetFrameView()->GetCompositorAnimationHost();
}

cc::AnimationTimeline* PaintLayerScrollableArea::GetCompositorAnimationTimeline() const
{
...
bool PaintLayerScrollableArea::HasTickmarks() const
{
    if (RareData() && !RareData()->tickmarks_override_.empty())
        return true;
    return layer_->IsRootLayer() && To<LayoutView>(GetLayoutBox())->HasTickmarks();
}
...
void PaintLayerScrollableArea::ScrollbarManager::SetHasHorizontalScrollbar(bool has_scrollbar)
{
    if (has_scrollbar) {
        if (!h_bar_) {
...
void PaintLayerScrollableArea::ScrollbarManager::SetHasVerticalScrollbar(bool has_scrollbar)
{
    if (has_scrollbar) {
        if (!v_bar_) {
...
{
    DCHECK(orientation == kHorizontalScrollbar ? !h_bar_is_attached_ : !v_bar_is_attached_);
    Scrollbar* scrollbar = nullptr;
    Element* element = nullptr;
    const LayoutObject& style_source = ScrollbarStyleSource(*ScrollableArea()->GetLayoutBox());
    ...
    if (style_source.StyleRef().HasCustomScrollbarStyle(element)) {
        DCHECK(element);
        scrollbar = MakeGarbageCollected<CustomScrollbar>(ScrollableArea(), orientation, &style_source);
    } else {
        scrollbar = MakeGarbageCollected<Scrollbar>(ScrollableArea(), orientation, &style_source);
    }
    ScrollableArea()->GetLayoutBox()->GetDocument().View()->AddScrollbar(scrollbar);
    return scrollbar;
}

void PaintLayerScrollableArea::ScrollbarManager::DestroyScrollbar(ScrollbarOrientation orientation)
{
    Member<Scrollbar>& scrollbar = orientation == kHorizontalScrollbar ? h_bar_ : v_bar_;
    DCHECK(orientation == kHorizontalScrollbar ? !h_bar_is_attached_ : !v_bar_is_attached_);
    if (!scrollbar)
        return;
    ...
    ScrollableArea()->GetLayoutBox()->GetDocument().View()->RemoveScrollbar(scrollbar);
    scrollbar->DisconnectFromScrollableArea();
    ScrollableArea()->GetLayoutBox()->GetFrame()->GetEventHandler().OnScrollbarDestroyed(*scrollbar);
    scrollbar = nullptr;
}
...
void PaintLayerScrollableArea::ScrollbarManager::Trace(blink::Visitor* visitor) const
{
...
PaintLayerScrollableArea::FreezeScrollbarsRootScope::FreezeScrollbarsRootScope(const LayoutBox& box, bool freeze_horizontal, bool freeze_vertical)
    : scrollable_area_(box.GetScrollableArea())
{
    if (scrollable_area_ && !FreezeScrollbarsScope::ScrollbarsAreFrozen() && (freeze_horizontal || freeze_vertical)) {
        scrollable_area_->EstablishScrollbarRoot(freeze_horizontal, freeze_vertical);
        freezer_.emplace();
    }
}
...
PaintLayerScrollableArea::DelayScrollOffsetClampScope::DelayScrollOffsetClampScope()
{
...
void PaintLayerScrollableArea::DelayScrollOffsetClampScope::SetNeedsClamp(PaintLayerScrollableArea* scrollable_area)
{
...
void PaintLayerScrollableArea::DelayScrollOffsetClampScope::ClampScrollableAreas()
{
...
HeapVector<Member<PaintLayerScrollableArea>>& PaintLayerScrollableArea::DelayScrollOffsetClampScope::NeedsClampList()
{
    DEFINE_STATIC_LOCAL(
        Persistent<HeapVector<Member<PaintLayerScrollableArea>>>, needs_clamp_list, (MakeGarbageCollected<HeapVector<Member<PaintLayerScrollableArea>>>()));
...
void PaintLayerScrollableArea::DidAddScrollbar(Scrollbar& scrollbar, ScrollbarOrientation orientation)
{
    if (HasOverlayOverflowControls() || layer_->NeedsReorderOverlayOverflowControls()) {
...
void PaintLayerScrollableArea::WillRemoveScrollbar(Scrollbar& scrollbar, ScrollbarOrientation orientation)
{
    ...
    if (!scrollbar.IsCustomScrollbar()) {
        ObjectPaintInvalidator(*GetLayoutBox())
            .SlowSetPaintingLayerNeedsRepaintAndInvalidateDisplayItemClient(scrollbar, PaintInvalidationReason::kScrollControl);
    }
...
static bool ScrollControlNeedsPaintInvalidation(const gfx::Rect& new_visual_rect, const gfx::Rect& previous_visual_rect, bool needs_paint_invalidation)
{
...
bool PaintLayerScrollableArea::MayCompositeScrollbar(const Scrollbar& scrollbar) const
{
...
void PaintLayerScrollableArea::EstablishScrollbarRoot(bool freeze_horizontal, bool freeze_vertical)
{
...
void PaintLayerScrollableArea::InvalidatePaintOfScrollbarIfNeeded(const PaintInvalidatorContext& context, bool needs_paint_invalidation, Scrollbar* scrollbar,
    bool& previously_was_overlay, bool& previously_might_be_composited, gfx::Rect& visual_rect)
{
    ...
    if (scrollbar) {
        ...
        new_visual_rect.Offset(ToRoundedVector2d(context.fragment_data->PaintOffset()));
    }
    ...
    if (!(new_scrollbar_used_space_in_box.IsEmpty() && previous_scrollbar_used_space_in_box.IsEmpty())
        && new_scrollbar_used_space_in_box != previous_scrollbar_used_space_in_box) {
        context.painting_layer->SetNeedsRepaint();
        const auto& box = *GetLayoutBox();
        ObjectPaintInvalidator(box).InvalidateDisplayItemClient(box, PaintInvalidationReason::kLayout);
    }
    ...
    if (scrollbar) {
        ...
        } else if (may_be_composited && (RuntimeEnabledFeatures::RasterInducingScrollEnabled() || UsesCompositedScrolling())) {
    ...
    if (scrollbar && ScrollControlNeedsPaintInvalidation(new_visual_rect, visual_rect, needs_paint_invalidation)) {
        context.painting_layer->SetNeedsRepaint();
...
void PaintLayerScrollableArea::InvalidatePaintOfScrollControlsIfNeeded(const PaintInvalidatorContext& context)
{
    if (context.subtree_flags & PaintInvalidatorContext::kSubtreeFullInvalidation)
        SetScrollControlsNeedFullPaintInvalidation();

    InvalidatePaintOfScrollbarIfNeeded(context, HorizontalScrollbarNeedsPaintInvalidation(), HorizontalScrollbar(),
        horizontal_scrollbar_previously_was_overlay_, horizontal_scrollbar_previously_might_be_composited_, horizontal_scrollbar_visual_rect_);
    InvalidatePaintOfScrollbarIfNeeded(context, VerticalScrollbarNeedsPaintInvalidation(), VerticalScrollbar(), vertical_scrollbar_previously_was_overlay_,
        vertical_scrollbar_previously_might_be_composited_, vertical_scrollbar_visual_rect_);

    gfx::Rect new_scroll_corner_and_resizer_visual_rect = ScrollCornerAndResizerRect();
    // TODO(crbug.com/1020913): We should not round paint_offset but should
    // consider subpixel accumulation when painting scrollbars.
    new_scroll_corner_and_resizer_visual_rect.Offset(ToRoundedVector2d(context.fragment_data->PaintOffset()));
    if (ScrollControlNeedsPaintInvalidation(
            new_scroll_corner_and_resizer_visual_rect, scroll_corner_and_resizer_visual_rect_, ScrollCornerNeedsPaintInvalidation())) {
        scroll_corner_and_resizer_visual_rect_ = new_scroll_corner_and_resizer_visual_rect;
        if (LayoutCustomScrollbarPart* scroll_corner = ScrollCorner()) {
            DCHECK(!scroll_corner->PaintingLayer());
            ObjectPaintInvalidator(*scroll_corner).InvalidateDisplayItemClient(*scroll_corner, PaintInvalidationReason::kScrollControl);
        }
        if (LayoutCustomScrollbarPart* resizer = Resizer()) {
            DCHECK(!resizer->PaintingLayer());
            ObjectPaintInvalidator(*resizer).InvalidateDisplayItemClient(*resizer, PaintInvalidationReason::kScrollControl);
        }

        context.painting_layer->SetNeedsRepaint();
        ObjectPaintInvalidator(*GetLayoutBox()).InvalidateDisplayItemClient(GetScrollCornerDisplayItemClient(), PaintInvalidationReason::kLayout);
    }
...
void PaintLayerScrollableArea::DidScrollWithScrollbar(ScrollbarPart part, ScrollbarOrientation orientation, WebInputEvent::Type type)
{
    WebFeature scrollbar_use_uma;
    switch (part) {
    case kBackButtonEndPart:
    case kForwardButtonStartPart:
        UseCounter::Count(GetLayoutBox()->GetDocument(), WebFeature::kScrollbarUseScrollbarButtonReversedDirection);
        [[fallthrough]];
    case kBackButtonStartPart:
    case kForwardButtonEndPart:
        scrollbar_use_uma
            = (orientation == kVerticalScrollbar ? WebFeature::kScrollbarUseVerticalScrollbarButton : WebFeature::kScrollbarUseHorizontalScrollbarButton);
        break;
    case kThumbPart:
        if (orientation == kVerticalScrollbar) {
            scrollbar_use_uma = (WebInputEvent::IsMouseEventType(type) ? WebFeature::kVerticalScrollbarThumbScrollingWithMouse
                                                                       : WebFeature::kVerticalScrollbarThumbScrollingWithTouch);
        } else {
            scrollbar_use_uma = (WebInputEvent::IsMouseEventType(type) ? WebFeature::kHorizontalScrollbarThumbScrollingWithMouse
    ...
    case kForwardTrackPart:
        scrollbar_use_uma
            = (orientation == kVerticalScrollbar ? WebFeature::kScrollbarUseVerticalScrollbarTrack : WebFeature::kScrollbarUseHorizontalScrollbarTrack);
        break;
    default:
        return;
    }
...
{
    return CompositorElementIdFromUniqueObjectId(GetLayoutBox()->UniqueId(), CompositorElementIdNamespace::kScroll);
}
...
gfx::Size PaintLayerScrollableArea::PixelSnappedBorderBoxSize() const
{
    ...
    return PhysicalRect(GetLayoutBox()->FirstFragment().PaintOffset(), GetLayoutBox()->Size()).PixelSnappedSize();
}
...
void PaintLayerScrollableArea::DropCompositorScrollDeltaNextCommit()
{
    ...
    CHECK(frame_view);
    if (auto* paint_artifact_compositor = frame_view->GetPaintArtifactCompositor()) {
        paint_artifact_compositor->DropCompositorScrollDeltaNextCommit(GetScrollElementId());
    }
}

gfx::Rect PaintLayerScrollableArea::ScrollingBackgroundVisualRect(const PhysicalOffset& paint_offset) const
{
    const auto* box = GetLayoutBox();
    ...
    if (IsA<LayoutView>(box) && (document.IsXMLDocument() || document.IsHTMLDocument())) {
        if (const auto* document_element = document.documentElement()) {
            if (const auto* document_element_object = document_element->GetLayoutObject()) {
                const auto& document_element_state = document_element_object->FirstFragment().LocalBorderBoxProperties();
                const auto& view_contents_state = box->FirstFragment().ContentsProperties();
                gfx::Rect result_in_view = result;
                GeometryMapper::SourceToDestinationRect(view_contents_state.Transform(), document_element_state.Transform(), result_in_view);
...
String PaintLayerScrollableArea::ScrollingBackgroundDisplayItemClient::DebugName() const
{
    return "Scrolling background of " + scrollable_area_->GetLayoutBox()->DebugName();
}

DOMNodeId PaintLayerScrollableArea::ScrollingBackgroundDisplayItemClient::OwnerNodeId() const
{
    return static_cast<const DisplayItemClient*>(scrollable_area_->GetLayoutBox())->OwnerNodeId();
}

String PaintLayerScrollableArea::ScrollCornerDisplayItemClient::DebugName() const
{
    return "Scroll corner of " + scrollable_area_->GetLayoutBox()->DebugName();
}

DOMNodeId PaintLayerScrollableArea::ScrollCornerDisplayItemClient::OwnerNodeId() const
{
    return static_cast<const DisplayItemClient*>(scrollable_area_->GetLayoutBox())->OwnerNodeId();
}
...
{
    if (!RuntimeEnabledFeatures::CSSScrollSnapChangeEventEnabled() && !RuntimeEnabledFeatures::CSSSnapContainerQueriesEnabled()) {
        return;
    }
    ...
    cc::TargetSnapAreaElementIds new_target_ids = container_data->GetTargetSnapAreaElementIds();

    CreateAndSetSnappedQueryScrollSnapshotIfNeeded(new_target_ids);

    auto& rare_data = EnsureRareData();
    bool scrollsnapchange = (rare_data.scrollsnapchange_target_ids_
            ? (new_target_ids.x != rare_data.scrollsnapchange_target_ids_->x || new_target_ids.y != rare_data.scrollsnapchange_target_ids_->y)
            : true);
    if (scrollsnapchange) {
...
void PaintLayerScrollableArea::SetScrollsnapchangingTargetIds(std::optional<cc::TargetSnapAreaElementIds> ids)
{
    EnsureRareData().scrollsnapchanging_target_ids_ = ids;
}

void PaintLayerScrollableArea::UpdateScrollSnapChangingTargetsAndEnqueueScrollSnapChanging(const cc::TargetSnapAreaElementIds& new_target_ids)
{
    ...
    auto& rare_data = EnsureRareData();
    bool scrollsnapchanging = (rare_data.scrollsnapchanging_target_ids_
            ? (new_target_ids.x != rare_data.scrollsnapchanging_target_ids_->x || new_target_ids.y != rare_data.scrollsnapchanging_target_ids_->y)
            : true);
...
void PaintLayerScrollableArea::EnqueueScrollSnapChangingEventFromImplIfNeeded()
{
    ...
    UpdateScrollSnapChangingTargetsAndEnqueueScrollSnapChanging(snap.target_element_ids);
}

Node* PaintLayerScrollableArea::GetSnapTargetAlongAxis(cc::TargetSnapAreaElementIds ids, cc::SnapAxis axis) const
{
    ...
    if (ids.y && (axis == kY || (axis == kBlock && horiz) || (axis == kInline && !horiz))) {
        return DOMNodeIds::NodeForId(DOMNodeIdFromCompositorElementId(ids.y));
    }
    if (ids.x && (axis == kX || (axis == kInline && horiz) || (axis == kBlock && !horiz))) {
        return DOMNodeIds::NodeForId(DOMNodeIdFromCompositorElementId(ids.x));
    }
    return nullptr;
}

Node* PaintLayerScrollableArea::GetSnapEventTargetAlongAxis(const AtomicString& event_type, cc::SnapAxis axis) const
{
...
Element* PaintLayerScrollableArea::GetSnappedQueryTargetAlongAxis(cc::SnapAxis axis) const
{
    if (RareData()) {
        std::optional<cc::TargetSnapAreaElementIds> ids = RareData()->snapped_query_target_ids_;
        if (ids) {
...
void PaintLayerScrollableArea::SetScrollsnapchangeTargetIds(std::optional<cc::TargetSnapAreaElementIds> ids)
{
    EnsureRareData().scrollsnapchange_target_ids_ = ids;
}

SnappedQueryScrollSnapshot& PaintLayerScrollableArea::EnsureSnappedQueryScrollSnapshot()
{
    PaintLayerScrollableAreaRareData& rare_data = EnsureRareData();
    if (rare_data.snapped_query_snapshot_ == nullptr) {
        rare_data.snapped_query_snapshot_ = MakeGarbageCollected<SnappedQueryScrollSnapshot>(*this);
    }
    return *rare_data.snapped_query_snapshot_;
}

SnappedQueryScrollSnapshot* PaintLayerScrollableArea::GetSnappedQueryScrollSnapshot()
{
    return RareData() ? RareData()->snapped_query_snapshot_ : nullptr;
}

void PaintLayerScrollableArea::CreateAndSetSnappedQueryScrollSnapshotIfNeeded(cc::TargetSnapAreaElementIds ids)
{
    ...
    if (ids.x) {
        target_x = DynamicTo<Element>(DOMNodeIds::NodeForId(DOMNodeIdFromCompositorElementId(ids.x)));
    }
    Element* target_y = nullptr;
    if (ids.y) {
        target_y = DynamicTo<Element>(DOMNodeIds::NodeForId(DOMNodeIdFromCompositorElementId(ids.y)));
    }
    for (Element* target : { target_x, target_y }) {
        if (!target) {
            continue;
        }
        if (ContainerQueryEvaluator* evaluator = target->GetContainerQueryEvaluator()) {
...
void PaintLayerScrollableArea::SetSnappedQueryTargetIds(std::optional<cc::TargetSnapAreaElementIds> ids)
{
...
void PaintLayerScrollableArea::UpdateScrollMarkers(const ScrollOffset& offset)
{
    if (Element* element = DynamicTo<Element>(GetLayoutBox()->GetNode())) {
        if (PseudoElement* before = element->GetPseudoElement(kPseudoIdScrollMarkerGroupBefore)) {
            auto* group_before = DynamicTo<ScrollMarkerGroupPseudoElement>(before);
            group_before->UpdateSelectedScrollMarker(offset);
        } else if (PseudoElement* after = element->GetPseudoElement(kPseudoIdScrollMarkerGroupAfter)) {
