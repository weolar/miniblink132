...
{
    if (!should_paint_background_ || paint_info_.IsPaintingBackgroundInContentsSpace() || layout_box_.IsDocumentElement())
        return kBackgroundBleedNone;

    const bool has_border_radius = style_.HasBorderRadius();
    if (!should_paint_border_ || !has_border_radius || style_.CanRenderBorderImage()) {
        ...
        if (style_.HasBackgroundImage() && has_border_radius) {
            ...
            if ((!BackgroundColor().IsFullyTransparent() || fill_layer.Next()) && !fill_layer.ImageOccludesNextLayers(layout_box_.GetDocument(), style_)) {
