...
FieldsetPaintInfo::FieldsetPaintInfo(
    const ComputedStyle& fieldset_style, const PhysicalSize& fieldset_size, const PhysicalBoxStrut& fieldset_borders, const PhysicalRect& legend_border_box)
{
    if (fieldset_style.IsHorizontalWritingMode()) {
        ...
        legend_cutout_rect = PhysicalRect(legend_border_box.X(), LayoutUnit(), legend_border_box.Width(), std::max(legend_size, border_size));
    } else {
        ...
        legend_cutout_rect = PhysicalRect(LayoutUnit(), legend_border_box.offset.top, legend_total_block_size, legend_border_box.size.height);
        if (fieldset_style.IsFlippedBlocksWritingMode()) {
