...
TEST_F(FragmentDataTest, PreClip)
{
    ...
    const ObjectPaintProperties* properties = target->FirstFragment().PaintProperties();
    EXPECT_TRUE(properties->ClipPathClip());
    EXPECT_TRUE(properties->CssClip());
    EXPECT_TRUE(properties->PixelMovingFilterClipExpander());
    EXPECT_EQ(properties->CssClip(), properties->PixelMovingFilterClipExpander()->Parent());
    EXPECT_EQ(properties->ClipPathClip(), properties->CssClip()->Parent());
    EXPECT_EQ(properties->ClipPathClip()->Parent(), &target->FirstFragment().PreClip());
}
