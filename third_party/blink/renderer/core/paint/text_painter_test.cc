...
protected:
    const LayoutText& GetLayoutText()
    {
        return *layout_text_;
    }
...
protected:
    ...
    {
        layout_text_ = To<LayoutText>(GetDocument().body()->firstChild()->GetLayoutObject());
...
{
    GetDocument().body()->SetInlineStyleProperty(CSSPropertyID::kColor, CSSValueID::kBlue);
    UpdateAllLifecyclePhasesForTest();

    PaintController controller;
    GraphicsContext context(controller);
    TextPaintStyle text_style
        = TextPainter::TextPaintingStyle(GetLayoutText().GetDocument(), GetLayoutText().StyleRef(), CreatePaintInfoForBackground(context));
...
{
    GetDocument().body()->SetInlineStyleProperty(CSSPropertyID::kWebkitTextFillColor, CSSValueID::kRed);
    GetDocument().body()->SetInlineStyleProperty(CSSPropertyID::kWebkitTextStrokeColor, CSSValueID::kLime);
    GetDocument().body()->SetInlineStyleProperty(CSSPropertyID::kTextEmphasisColor, CSSValueID::kBlue);
    GetDocument().body()->SetInlineStyleProperty(CSSPropertyID::kWebkitTextStrokeWidth, 4, CSSPrimitiveValue::UnitType::kPixels);
    GetDocument().body()->SetInlineStyleProperty(CSSPropertyID::kTextShadow, "1px 2px 3px yellow");
    UpdateAllLifecyclePhasesForTest();
    ...
    EXPECT_EQ(Color(255, 255, 0), text_style.shadow->Shadows()[0].GetColor().GetColor());
}
...
TEST_F(TextPainterTest, TextPaintingStyle_UsesTextAsClip)
{
    ...
    PaintController controller;
    GraphicsContext context(controller);
    TextPaintStyle text_style = TextPainter::TextPaintingStyle(GetLayoutText().GetDocument(), GetLayoutText().StyleRef(), CreatePaintInfoForTextClip(context));
...
TEST_F(TextPainterTest, TextPaintingStyle_ForceBackgroundToWhite_NoAdjustmentNeeded)
{
    ...
    GetDocument().body()->SetInlineStyleProperty(CSSPropertyID::kWebkitTextStrokeColor, CSSValueID::kLime);
    GetDocument().body()->SetInlineStyleProperty(CSSPropertyID::kTextEmphasisColor, CSSValueID::kBlue);
    GetDocument().body()->SetInlineStyleProperty(CSSPropertyID::kWebkitPrintColorAdjust, CSSValueID::kEconomy);
...
{
    GetDocument().body()->SetInlineStyleProperty(CSSPropertyID::kWebkitTextFillColor, "rgb(255, 220, 220)");
    GetDocument().body()->SetInlineStyleProperty(CSSPropertyID::kWebkitTextStrokeColor, "rgb(220, 255, 220)");
    GetDocument().body()->SetInlineStyleProperty(CSSPropertyID::kTextEmphasisColor, "rgb(220, 220, 255)");
    GetDocument().body()->SetInlineStyleProperty(CSSPropertyID::kWebkitPrintColorAdjust, CSSValueID::kEconomy);
    GetDocument().GetSettings()->SetShouldPrintBackgrounds(false);
    gfx::SizeF page_size(500, 800);
    GetFrame().StartPrinting(WebPrintParams(page_size));
...
{
    auto& persistent_data = GetDocument().View()->GetPaintControllerPersistentDataForTesting();
    auto* item = DynamicTo<DrawingDisplayItem>(persistent_data.GetDisplayItemList()[1]);
    ASSERT_TRUE(item);
    auto* op = static_cast<const cc::DrawTextBlobOp*>(&item->GetPaintRecord().GetFirstOp());
    ...
    ASSERT_TRUE(item);
    op = static_cast<const cc::DrawTextBlobOp*>(&item->GetPaintRecord().GetFirstOp());
    ASSERT_EQ(cc::PaintOpType::kDrawTextBlob, op->GetType());
    ...
    GetDocument().body()->SetInlineStyleProperty(CSSPropertyID::kFontSize, "30px");
    UpdateAllLifecyclePhasesForTest();
    item = DynamicTo<DrawingDisplayItem>(persistent_data.GetDisplayItemList()[1]);
    ASSERT_TRUE(item);
    op = static_cast<const cc::DrawTextBlobOp*>(&item->GetPaintRecord().GetFirstOp());
