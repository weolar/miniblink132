...
TEST_P(SVGContainerPainterTest, FilterPaintProperties)
{
    ...
    const DisplayItem::Type kSVGEffectPaintPhaseForeground = static_cast<DisplayItem::Type>(DisplayItem::kSVGEffectPaintPhaseFirst + 5);
    ...
    const auto& container_properties = container->FirstFragment().ContentsProperties();
    ...
    if (RuntimeEnabledFeatures::HitTestOpaquenessEnabled()) {
        EXPECT_THAT(ContentPaintChunks(),
            ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK_COMMON, IsPaintChunk(1, 1), // Hit test for svg.
                IsPaintChunk(1, 2, before_id, before_properties), IsPaintChunk(2, 3, rect_id, container_properties),
                IsPaintChunk(3, 4, after_id, after_properties)));
    } else {
        EXPECT_THAT(ContentPaintChunks(),
            ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK_COMMON, IsPaintChunk(1, 2, before_id, before_properties),
                IsPaintChunk(2, 3, rect_id, container_properties), IsPaintChunk(3, 4, after_id, after_properties)));
    }
}
...
TEST_P(SVGContainerPainterTest, ScaleAnimationFrom0)
{
    ...
    if (RuntimeEnabledFeatures::HitTestOpaquenessEnabled()) {
        EXPECT_THAT(ContentPaintChunks(), ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK_COMMON, IsPaintChunk(1, 1))); // Svg hit test.
    } else {
    ...
    rect2_element->setAttribute(html_names::kClassAttr, AtomicString("transform-scale"));
    UpdateAllLifecyclePhasesForTest();

    // Start animations on the rects.
    const DisplayItem::Type kSVGTransformPaintPhaseForeground = static_cast<DisplayItem::Type>(DisplayItem::kSVGTransformPaintPhaseFirst + 5);
    ...
    if (RuntimeEnabledFeatures::HitTestOpaquenessEnabled()) {
        EXPECT_THAT(ContentPaintChunks(),
            ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK_COMMON, IsPaintChunk(1, 1), // Svg hit test.
                IsPaintChunk(1, 2, rect1_id, rect1_properties), IsPaintChunk(2, 3, rect2_id, rect2_properties)));
    } else {
        EXPECT_THAT(ContentPaintChunks(),
            ElementsAre(
                VIEW_SCROLLING_BACKGROUND_CHUNK_COMMON, IsPaintChunk(1, 2, rect1_id, rect1_properties), IsPaintChunk(2, 3, rect2_id, rect2_properties)));
    }
