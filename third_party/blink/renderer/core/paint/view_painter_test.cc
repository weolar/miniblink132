...
void ViewPainterFixedBackgroundTest::RunFixedBackgroundTest(bool prefer_compositing_to_lcd_text)
{
    ...
    layout_viewport->SetScrollOffset(scroll_offset, mojom::blink::ScrollType::kUser);
    frame_view->UpdateAllLifecyclePhasesForTest();

    const auto& display_items = GetPersistentData().GetDisplayItemList();
    const auto& background_client = prefer_compositing_to_lcd_text ? GetLayoutView() : ViewScrollingBackgroundClient();
    const DisplayItem* background_display_item = &display_items[0];
    EXPECT_THAT(*background_display_item, IsSameId(background_client.Id(), DisplayItem::kDocumentBackground));

    PaintRecord record = To<DrawingDisplayItem>(background_display_item)->GetPaintRecord();

    SkRect expected_rect = prefer_compositing_to_lcd_text ? SkRect::MakeXYWH(0, 0, 800, 600) : SkRect::MakeXYWH(scroll_offset.x(), scroll_offset.y(), 800, 600);
    EXPECT_THAT(record,
        ElementsAre(
            _, AllOf(PaintOpIs<cc::DrawRectOp>(), ResultOf([](const cc::PaintOp& op) { return static_cast<const cc::DrawRectOp&>(op).rect; }, expected_rect))));
}
...
TEST_P(ViewPainterFixedBackgroundTest, DocumentFixedBackgroundPreferCompositing)
{
...
TEST_P(ViewPainterTest, DocumentBackgroundWithScroll)
{
    ...
    scroll_hit_test_data->scroll_translation = GetLayoutView().FirstFragment().PaintProperties()->ScrollTranslation();
    scroll_hit_test_data->scrolling_contents_cull_rect = gfx::Rect(0, 0, 800, 4600);
...
TEST_P(ViewPainterTest, FrameScrollHitTestProperties)
{
    ...
    EXPECT_THAT(ContentDisplayItems(), ElementsAre(VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM, IsSameId(child.Id(), kBackgroundType)));
    ...
    EXPECT_THAT(scroll_hit_test_chunk,
        IsPaintChunk(0, 0, PaintChunk::Id(GetLayoutView().Id(), DisplayItem::kScrollHitTest), GetLayoutView().FirstFragment().LocalBorderBoxProperties(),
    ...
    EXPECT_EQ(gfx::RectF(InfiniteIntRect()), scroll_hit_test_clip.PaintClipRect().Rect());
...
TEST_P(ViewPainterTest, TouchActionRect)
{
    ...
    auto non_scrolling_properties = view->FirstFragment().LocalBorderBoxProperties();
    auto* view_hit_test_data = MakeGarbageCollected<HitTestData>();
    view_hit_test_data->touch_action_rects = { { gfx::Rect(0, 0, 800, 600), TouchAction::kPinchZoom } };
    ...
    scrolling_hit_test_data->touch_action_rects = { { gfx::Rect(0, 0, 800, 3000), TouchAction::kPinchZoom }, { gfx::Rect(0, 0, 10, 100), TouchAction::kNone } };
    ...
    EXPECT_THAT(GetPersistentData().GetPaintChunks()[0],
        IsPaintChunk(
            0, 1, PaintChunk::Id(view->Layer()->Id(), DisplayItem::kLayerChunk), non_scrolling_properties, view_hit_test_data, gfx::Rect(0, 0, 800, 600)));
    EXPECT_THAT(GetPersistentData().GetPaintChunks()[1],
        IsPaintChunk(1, 1, PaintChunk::Id(view->Id(), DisplayItem::kScrollHitTest), non_scrolling_properties, scroll_hit_test_data, gfx::Rect(0, 0, 800, 600)));
    EXPECT_THAT(ContentPaintChunks(),
        ElementsAre(IsPaintChunk(1, 1,
            RuntimeEnabledFeatures::HitTestOpaquenessEnabled()
                ? PaintChunk::Id(view->GetScrollableArea()->GetScrollingBackgroundDisplayItemClient().Id(), DisplayItem::kDocumentBackground)
