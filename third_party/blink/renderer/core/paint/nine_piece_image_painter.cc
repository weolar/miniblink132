...
std::optional<float> CalculateSpaceNeeded(const float destination, const float source)
{
    DCHECK_GT(source, 0);
...
std::optional<TileParameters> ComputeTileParameters(ENinePieceImageRule tile_rule, float dst_extent, float src_extent)
{
    switch (tile_rule) {
...
bool ShouldTile(const NinePieceImageGrid::NinePieceDrawInfo& draw_info)
{
    ...
    // calculations.
    if (draw_info.tile_rule.horizontal == kStretchImageRule && draw_info.tile_rule.vertical == kStretchImageRule)
        return false;
    return true;
}

void PaintPieces(GraphicsContext& context, const PhysicalRect& border_image_rect, const ComputedStyle& style, const Document& document,
    const NinePieceImage& nine_piece_image, Image& image, const gfx::SizeF& unzoomed_image_size, PhysicalBoxSides sides_to_include)
{
    const RespectImageOrientationEnum respect_orientation = style.ImageOrientation();
    ...
    gfx::Vector2dF slice_scale(image_size.width() / unzoomed_image_size.width(), image_size.height() / unzoomed_image_size.height());

    auto border_widths
        = gfx::Outsets().set_left_right(style.BorderLeftWidth(), style.BorderRightWidth()).set_top_bottom(style.BorderTopWidth(), style.BorderBottomWidth());
    NinePieceImageGrid grid(
        nine_piece_image, image_size, slice_scale, style.EffectiveZoom(), ToPixelSnappedRect(border_image_rect), border_widths, sides_to_include);
    ...
    ScopedImageRenderingSettings image_rendering_settings_scope(context, style.GetInterpolationQuality(), style.GetDynamicRangeLimit());
    for (NinePiece piece = kMinPiece; piece < kMaxPiece; ++piece) {
        ...
        if (!ShouldTile(draw_info)) {
            ...
            context.DrawImage(image, Image::kSyncDecode, image_auto_dark_mode, ImagePaintTimingInfo(), draw_info.destination, &src_rect, SkBlendMode::kSrcOver,
                respect_orientation);
            continue;
        }

        // TODO(cavalcantii): see crbug.com/662513.
        const std::optional<TileParameters> h_tile
            = ComputeTileParameters(draw_info.tile_rule.horizontal, draw_info.destination.width(), draw_info.source.width() * draw_info.tile_scale.x());
        const std::optional<TileParameters> v_tile
            = ComputeTileParameters(draw_info.tile_rule.vertical, draw_info.destination.height(), draw_info.source.height() * draw_info.tile_scale.y());
        if (!h_tile || !v_tile)
            continue;

        ImageTilingInfo tiling_info;
        tiling_info.image_rect = draw_info.source;
        tiling_info.scale = gfx::ScaleVector2d(draw_info.tile_scale, h_tile->scale_factor, v_tile->scale_factor);
        ...
        tiling_info.phase = draw_info.destination.origin() + (gfx::PointF(h_tile->phase, v_tile->phase) - tile_origin_in_dest_space);
        tiling_info.spacing = gfx::SizeF(h_tile->spacing, v_tile->spacing);
        // TODO(sohom): Per crbug.com/1351498 investigate and set
        // ImagePaintTimingInfo parameters correctly
        context.DrawImageTiled(
            image, draw_info.destination, tiling_info, image_auto_dark_mode, ImagePaintTimingInfo(), SkBlendMode::kSrcOver, respect_orientation);
    }
}

} // anonymous namespace

bool NinePieceImagePainter::Paint(GraphicsContext& graphics_context, const ImageResourceObserver& observer, const Document& document, Node* node,
    const PhysicalRect& rect, const ComputedStyle& style, const NinePieceImage& nine_piece_image, PhysicalBoxSides sides_to_include)
{
    ...
    gfx::SizeF image_size = style_image->ImageSize(style.EffectiveZoom(), default_object_size, respect_orientation);
    scoped_refptr<Image> image = style_image->GetImage(observer, document, style, image_size);
    if (!image)
        return true;
    ...
    gfx::SizeF unzoomed_image_size = style_image->ImageSize(1, gfx::ScaleSize(default_object_size, 1 / style.EffectiveZoom()), respect_orientation);
    ...
    DEVTOOLS_TIMELINE_TRACE_EVENT_WITH_CATEGORIES(TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "PaintImage", inspector_paint_image_event::Data, node,
        *style_image, gfx::RectF(image->Rect()), gfx::RectF(border_image_rect));
    PaintPieces(graphics_context, border_image_rect, style, document, nine_piece_image, *image, unzoomed_image_size, sides_to_include);
    return true;
}
