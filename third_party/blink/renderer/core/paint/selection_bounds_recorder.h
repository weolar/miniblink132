...
public:
    SelectionBoundsRecorder(SelectionState, PhysicalRect, PaintController&, TextDirection, WritingMode, const LayoutObject&);

    ~SelectionBoundsRecorder();

    static bool ShouldRecordSelection(const FrameSelection&, SelectionState);

    static bool IsVisible(const LayoutObject& rect_layout_object, const PhysicalOffset& edge_start_in_layer, const PhysicalOffset& edge_end_in_layer);
