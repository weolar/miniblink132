...
BoundEdges GetBoundEdges(WritingMode writing_mode, bool is_ltr)
{
    ...
    case WritingMode::kHorizontalTb:
        return is_ltr ? BoundEdges { RectEdge::kTopLeftToBottomLeft, RectEdge::kTopRightToBottomRight }
                      : BoundEdges { RectEdge::kTopRightToBottomRight, RectEdge::kTopLeftToBottomLeft };
    case WritingMode::kVerticalRl:
    case WritingMode::kSidewaysRl:
        return is_ltr ? BoundEdges { RectEdge::kTopLeftToTopRight, RectEdge::kBottomRightToBottomLeft }
                      : BoundEdges { RectEdge::kBottomLeftToBottomRight, RectEdge::kTopRightToTopLeft };
    case WritingMode::kVerticalLr:
        return is_ltr ? BoundEdges { RectEdge::kTopRightToTopLeft, RectEdge::kBottomLeftToBottomRight }
                      : BoundEdges { RectEdge::kBottomRightToBottomLeft, RectEdge::kTopLeftToTopRight };
    case WritingMode::kSidewaysLr:
        return is_ltr ? BoundEdges { RectEdge::kBottomLeftToBottomRight, RectEdge::kTopLeftToTopRight }
                      : BoundEdges { RectEdge::kTopLeftToTopRight, RectEdge::kBottomLeftToBottomRight };
    }
}

// Set the given bound's edge_start and edge_end, based on the provided
// selection rect and edge.
void SetBoundEdge(gfx::Rect selection_rect, RectEdge edge, PaintedSelectionBound& bound)
{
    switch (edge) {
...
PhysicalOffset GetSamplePointForVisibility(const PhysicalOffset& edge_start, const PhysicalOffset& edge_end, float zoom_factor)
{
...
SelectionBoundsRecorder::SelectionBoundsRecorder(SelectionState state, PhysicalRect selection_rect, PaintController& paint_controller,
    TextDirection text_direction, WritingMode writing_mode, const LayoutObject& layout_object)
    : state_(state)
...
SelectionBoundsRecorder::~SelectionBoundsRecorder()
{
    ...
    if (state_ == SelectionState::kStart || state_ == SelectionState::kStartAndEnd) {
        start.emplace();
        start->type = is_ltr ? gfx::SelectionBound::Type::LEFT : gfx::SelectionBound::Type::RIGHT;
        SetBoundEdge(selection_rect, edges.start, *start);
        start->hidden = !IsVisible(selection_layout_object_, PhysicalOffset(start->edge_start), PhysicalOffset(start->edge_end));
    }

    if (state_ == SelectionState::kStartAndEnd || state_ == SelectionState::kEnd) {
        end.emplace();
        end->type = is_ltr ? gfx::SelectionBound::Type::RIGHT : gfx::SelectionBound::Type::LEFT;
        SetBoundEdge(selection_rect, edges.end, *end);
        end->hidden = !IsVisible(selection_layout_object_, PhysicalOffset(end->edge_start), PhysicalOffset(end->edge_end));
    }

    paint_controller_.RecordSelection(start, end, "");
}

bool SelectionBoundsRecorder::ShouldRecordSelection(const FrameSelection& frame_selection, SelectionState state)
{
    ...
    LocalFrame* focused_frame = local_frame->GetPage()->GetFocusController().FocusedFrame();
...
// clipped out).
bool SelectionBoundsRecorder::IsVisible(const LayoutObject& rect_layout_object, const PhysicalOffset& edge_start, const PhysicalOffset& edge_end)
{
    ...
    PhysicalOffset sample_point = GetSamplePointForVisibility(edge_start, edge_end, rect_layout_object.GetFrame()->LayoutZoomFactor());
    ...
    const PhysicalOffset position_in_input = rect_layout_object.LocalToAncestorPoint(sample_point, text_control_object, kTraverseDocumentBoundaries);
    return text_control_object->PhysicalBorderBoxRect().Contains(position_in_input);
}
