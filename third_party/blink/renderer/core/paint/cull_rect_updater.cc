...
float ExpansionRatio(const LayoutObject& object)
{
    ...
    float device_pixel_ratio = object.GetFrame()->LocalFrameRoot().GetDocument()->DevicePixelRatio();
...
{
    if (layer.PreviousPaintResult() == kMayBeClippedByCullRect || RuntimeEnabledFeatures::PaintUnderInvalidationCheckingEnabled()) {
        layer.SetNeedsRepaint();
    }
}

void SetFragmentCullRect(PaintLayer& layer, FragmentData& fragment, const CullRect& cull_rect)
{
...
bool SetFragmentContentsCullRect(PaintLayer& layer, FragmentData& fragment, const CullRect& contents_cull_rect)
{
    if (contents_cull_rect == fragment.GetContentsCullRect())
        return false;

    if (g_original_cull_rects) {
        if (g_original_cull_rects->empty() || g_original_cull_rects->back().fragment != &fragment) {
            g_original_cull_rects->emplace_back(fragment);
        }
    } else {
        SetLayerNeedsRepaintOnCullRectChange(layer);
...
bool ShouldUseInfiniteCullRect(const PaintLayer& layer, ViewTransitionSupplement* view_transition_supplement, bool& subtree_should_use_infinite_cull_rect)
{
    ...
    if (const auto* properties = object.FirstFragment().PaintProperties()) {
        ...
        if (properties->Filter() && properties->Filter()->HasFilterThatMovesPixels() &&
        ...
        const TransformPaintPropertyNode* transform_nodes[]
            = { properties->Transform(), properties->Offset(), properties->Scale(), properties->Rotate(), properties->Translate() };
        for (const auto* transform : transform_nodes) {
            ...
            if (!transform->HasDirectCompositingReasons() && layer.PreviousPaintResult() == kFullyPainted) {
                return true;
            }
        }
    }
...
bool HasScrolledEnough(const LayoutObject& object)
{
    if (const auto* properties = object.FirstFragment().PaintProperties()) {
        if (const auto* scroll_translation = properties->ScrollTranslation()) {
            ...
            gfx::Vector2dF delta = -scroll_translation->Get2dTranslation() - scrollable_area->LastCullRectUpdateScrollPosition().OffsetFromOrigin();
            return object.FirstFragment().GetContentsCullRect().HasScrolledEnough(delta, *scroll_translation, ExpansionRatio(object));
        }
    }
    return false;
}
...
CullRectUpdater::CullRectUpdater(PaintLayer& starting_layer, bool disable_expansion)
    : starting_layer_(starting_layer)
    , expansion_ratio_(disable_expansion ? 0.f : ExpansionRatio(starting_layer.GetLayoutObject()))
{
    view_transition_supplement_ = ViewTransitionSupplement::FromIfExists(starting_layer.GetLayoutObject().GetDocument());
}
...
void CullRectUpdater::UpdateInternal(const CullRect& input_cull_rect)
{
    ...
    if (!starting_layer_.NeedsCullRectUpdate() && !starting_layer_.DescendantNeedsCullRectUpdate() &&
    ...
    root_state_ = object.View()->FirstFragment().LocalBorderBoxProperties().Unalias();
    Context context;
    context.current.container = &starting_layer_;
    bool should_use_infinite = ShouldUseInfiniteCullRect(starting_layer_, view_transition_supplement_, context.current.subtree_should_use_infinite_cull_rect);

    auto& fragment = object.GetMutableForPainting().FirstFragment();
    SetFragmentCullRect(starting_layer_, fragment, should_use_infinite ? CullRect::Infinite() : input_cull_rect);
    context.current.force_update_children = SetFragmentContentsCullRect(starting_layer_, fragment,
        should_use_infinite ? CullRect::Infinite() : ComputeFragmentContentsCullRect(context, starting_layer_, fragment, input_cull_rect));
...
void CullRectUpdater::UpdateRecursively(const Context& parent_context, PaintLayer& layer)
{
    ...
    context.current.force_update_children = should_proactively_update || layer.ForcesChildrenCullRectUpdate();

    if (force_update_self || should_proactively_update || layer.NeedsCullRectUpdate()) {
        context.current.force_update_children |= UpdateForSelf(context, layer);
    }

    if (!context.current.subtree_is_out_of_cull_rect && object.ShouldClipOverflowAlongBothAxis() && !object.IsFragmented()) {
        const auto* box = layer.GetLayoutBox();
        DCHECK(box);
        PhysicalRect clip_rect = box->OverflowClipRect(box->FirstFragment().PaintOffset());
        if (!box->FirstFragment().GetCullRect().Intersects(ToEnclosingRect(clip_rect))) {
            context.current.subtree_is_out_of_cull_rect = true;
        }
    }

    bool should_traverse_children = context.current.force_update_children || layer.DescendantNeedsCullRectUpdate()
        || (context.absolute.force_update_children && layer.HasNonContainedAbsolutePositionDescendant())
        || (context.fixed.force_update_children && !object.CanContainFixedPositionObjects() && layer.HasFixedPositionDescendant());
    if (should_traverse_children) {
        context.current.container = &layer;
        // We pretend the starting layer can contain all descendants.
        if (&layer == &starting_layer_ || object.CanContainAbsolutePositionObjects()) {
...
// changed.
void CullRectUpdater::UpdateForDescendants(const Context& context, PaintLayer& layer)
{
    const auto& object = layer.GetLayoutObject();
    ...
    if (auto* embedded_content = DynamicTo<LayoutEmbeddedContent>(object)) {
        if (auto* embedded_view = embedded_content->GetEmbeddedContentView()) {
            if (auto* embedded_frame_view = DynamicTo<LocalFrameView>(embedded_view)) {
                ...
                if (embedded_frame_view->ShouldThrottleRendering()) {
                    ...
                    Context subframe_context = { context.current, context.current, context.current };
...
bool CullRectUpdater::UpdateForSelf(Context& context, PaintLayer& layer)
{
    ...
    bool should_use_infinite_cull_rect = !context.current.subtree_is_out_of_cull_rect
        && ShouldUseInfiniteCullRect(layer, view_transition_supplement_, context.current.subtree_should_use_infinite_cull_rect);

    for (FragmentData& fragment : MutableFragmentDataIterator(layer.GetLayoutObject())) {
        CullRect cull_rect;
        ...
        if (context.current.subtree_is_out_of_cull_rect) {
            ...
            if (!should_use_infinite_cull_rect) {
                if (should_match_fragments) {
                    for (const FragmentData& walker : FragmentDataIterator(parent_object)) {
            ...
            if (should_use_infinite_cull_rect || !parent_fragment) {
                ...
                cull_rect = ComputeFragmentCullRect(context, layer, fragment, *parent_fragment);
                contents_cull_rect = ComputeFragmentContentsCullRect(context, layer, fragment, cull_rect);
            }
        }

        SetFragmentCullRect(layer, fragment, cull_rect);
        force_update_children |= SetFragmentContentsCullRect(layer, fragment, contents_cull_rect);
    }

    return force_update_children;
}

CullRect CullRectUpdater::ComputeFragmentCullRect(Context& context, PaintLayer& layer, const FragmentData& fragment, const FragmentData& parent_fragment)
{
    ...
    if (parent_object.IsLayoutView() && pagination_state) {
        parent_state = pagination_state->ContentAreaPropertyTreeStateForCurrentPage(layout_view);
    }
    ...
    if (object.IsFixedPositioned()) {
        if (const auto* properties = fragment.PaintProperties()) {
            if (const auto* translation = properties->PaintOffsetTranslation()) {
                ...
                auto root_contents_state = view_fragment.LocalBorderBoxProperties().Unalias();
                if (pagination_state) {
                    // Document contents are parented under the pagination properties,
                    // which in turn are parented under the LayoutView.
                    root_contents_state = pagination_state->ContentAreaPropertyTreeStateForCurrentPage(layout_view);
                }
    ...
    if (parent_state != local_state) {
        ...
        bool expanded = cull_rect.ApplyPaintProperties(root_state_, parent_state, local_state, old_cull_rect, expansion_ratio_);
...
CullRect CullRectUpdater::ComputeFragmentContentsCullRect(Context& context, PaintLayer& layer, const FragmentData& fragment, const CullRect& cull_rect)
{
    auto local_state = fragment.LocalBorderBoxProperties().Unalias();
    ...
    if (contents_state != local_state) {
        ...
        bool expanded = contents_cull_rect.ApplyPaintProperties(root_state_, local_state, contents_state, old_contents_cull_rect, expansion_ratio_);
...
bool CullRectUpdater::ShouldProactivelyUpdate(const Context& context, const PaintLayer& layer) const
{
...
void CullRectUpdater::PaintPropertiesChanged(const LayoutObject& object, const PaintPropertiesChangeInfo& properties_changed)
{
    ...
    // below.
    DCHECK_NE(properties_changed.clip_changed, PaintPropertyChangeType::kChangedOnlyCompositedValues);
    DCHECK_NE(properties_changed.scroll_changed, PaintPropertyChangeType::kChangedOnlyCompositedValues);

    bool should_use_infinite_cull_rect = false;
    if (object.HasLayer()) {
        bool subtree_should_use_infinite_cull_rect = false;
        auto* view_transition_supplement = ViewTransitionSupplement::FromIfExists(object.GetDocument());
        should_use_infinite_cull_rect
            = ShouldUseInfiniteCullRect(*To<LayoutBoxModelObject>(object).Layer(), view_transition_supplement, subtree_should_use_infinite_cull_rect);
        if (should_use_infinite_cull_rect && object.FirstFragment().GetCullRect().IsInfinite() && object.FirstFragment().GetContentsCullRect().IsInfinite()) {
            return;
        }
    }
    ...
    // scroll offsets.
    bool needs_cull_rect_update = properties_changed.transform_changed >= PaintPropertyChangeType::kChangedOnlySimpleValues
        || properties_changed.clip_changed >= PaintPropertyChangeType::kChangedOnlySimpleValues
        || properties_changed.scroll_changed >= PaintPropertyChangeType::kChangedOnlySimpleValues || HasScrolledEnough(object);
    ...
    if (!needs_cull_rect_update) {
        ...
        DCHECK(properties_changed.transform_changed != PaintPropertyChangeType::kChangedOnlyCompositedValues || object.IsSVGChild()
            || should_use_infinite_cull_rect || !HasScrolledEnough(object));
        return;
    }
    ...
    if (object.HasLayer()) {
        ...
        if (const auto* layout_view = DynamicTo<LayoutView>(object)) {
            if (const auto* clip_node = object.FirstFragment().PaintProperties()->OverflowClip()) {
                if (clip_node->NodeChanged() != PaintPropertyChangeType::kUnchanged) {
                    for (const auto& fragment : layout_view->PhysicalFragments()) {
                        ...
                        for (const auto& fragment_child : fragment.Children()) {
                            ...
                            To<LayoutBox>(fragment_child->GetLayoutObject())->Layer()->SetNeedsCullRectUpdate();
                        }
                    }
                }
            }
...
OverriddenCullRectScope::OverriddenCullRectScope(PaintLayer& starting_layer, const CullRect& cull_rect, bool disable_expansion)
{
    outer_original_cull_rects_ = g_original_cull_rects;

    if (starting_layer.IsRootLayer() && starting_layer.GetLayoutObject().GetFrame()->IsLocalRoot() && !starting_layer.NeedsCullRectUpdate()
        && !starting_layer.DescendantNeedsCullRectUpdate() && cull_rect == starting_layer.GetLayoutObject().FirstFragment().GetCullRect()) {
