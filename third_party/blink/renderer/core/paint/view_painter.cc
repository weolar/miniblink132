...
void ViewPainter::PaintRootGroup(const PaintInfo& paint_info, const gfx::Rect& pixel_snapped_background_rect, const Document& document,
    const DisplayItemClient& client, const PropertyTreeStateOrAlias& state)
{
    ...
    Color base_background_color = layout_view.GetFrameView()->BaseBackgroundColor();
    ...
    bool should_clear_canvas = document.GetSettings() && document.GetSettings()->GetShouldClearDocumentBackground();

    ScopedPaintChunkProperties frame_view_background_state(paint_info.context.GetPaintController(), state, client, DisplayItem::kDocumentRootBackdrop);
    GraphicsContext& context = paint_info.context;
    if (!DrawingRecorder::UseCachedDrawingIfPossible(context, client, DisplayItem::kDocumentRootBackdrop)) {
        DrawingRecorder recorder(context, client, DisplayItem::kDocumentRootBackdrop, pixel_snapped_background_rect);
        context.FillRect(pixel_snapped_background_rect, base_background_color,
            PaintAutoDarkMode(box_fragment_.Style(), DarkModeFilter::ElementRole::kBackground),
...
void ViewPainter::PaintBoxDecorationBackground(const PaintInfo& paint_info)
{
    ...
    bool paints_hit_test_data = (RuntimeEnabledFeatures::HitTestOpaquenessEnabled() && painting_background_in_contents_space)
        || ObjectPainter(layout_view).ShouldRecordSpecialHitTestData(paint_info);

    Element* element = DynamicTo<Element>(layout_view.GetNode());
    bool paints_region_capture_data = element && element->GetRegionCaptureCropId() &&
        // TODO(wangxianzhu): This is to avoid the side-effect of
        // HitTestOpaqueness on region capture data. Verify if the side-effect
        // really matters.
        !(painting_background_in_contents_space && paint_info.ShouldSkipBackground());
    bool paints_scroll_hit_test = !painting_background_in_contents_space && layout_view.FirstFragment().PaintProperties()->Scroll();
    bool is_represented_via_pseudo_elements = [&layout_view]() {
        if (auto* transition = ViewTransitionUtils::GetTransition(layout_view.GetDocument())) {
            return transition->IsRepresentedViaPseudoElements(layout_view);
        }
        return false;
    }();
    if (!layout_view.HasBoxDecorationBackground() && !paints_hit_test_data && !paints_scroll_hit_test && !paints_region_capture_data
    ...
    // content area.
    if (document.IsPrintingOrPaintingPreview() || !layout_view.GetFrameView()->GetFrame().ClipsContent()) {
    ...
    if (painting_background_in_contents_space) {
        ...
        background_client = &layout_view.GetScrollableArea()->GetScrollingBackgroundDisplayItemClient();
    }

    gfx::Rect pixel_snapped_background_rect = ToPixelSnappedRect(background_rect);

    auto root_element_background_painting_state = layout_view.FirstFragment().ContentsProperties();
    ...
    bool should_apply_root_background_behavior = document.IsHTMLDocument() || document.IsXHTMLDocument();

    bool should_paint_background = !paint_info.ShouldSkipBackground() && (layout_view.HasBoxDecorationBackground() || is_represented_via_pseudo_elements);
    ...
    if (should_paint_background && painting_background_in_contents_space && should_apply_root_background_behavior && root_object) {
        auto document_element_state = root_object->FirstFragment().LocalBorderBoxProperties();
        document_element_state.SetTransform(root_object->FirstFragment().PreTransform());
        ...
        if (document_element_state != root_element_background_painting_state) {
            if (&document_element_state.Effect() != &root_element_background_painting_state.Effect())
                painted_separate_effect = true;

            root_element_background_painting_state = document_element_state;
            PaintRootGroup(paint_info, pixel_snapped_background_rect, document, *background_client, layout_view.FirstFragment().ContentsProperties());
    ...
    if (painting_background_in_contents_space) {
        scoped_properties.emplace(
            paint_info.context.GetPaintController(), root_element_background_painting_state, *background_client, DisplayItem::kDocumentBackground);
    }

    if (should_paint_background) {
        PaintRootElementGroup(paint_info, pixel_snapped_background_rect, root_element_background_painting_state, *background_client, painted_separate_backdrop,
            painted_separate_effect);
    }
    if (paints_hit_test_data) {
        ObjectPainter(layout_view).RecordHitTestData(paint_info, pixel_snapped_background_rect, *background_client);
    }

    if (paints_region_capture_data) {
        BoxPainter(layout_view).RecordRegionCaptureData(paint_info, PhysicalRect(pixel_snapped_background_rect), *background_client);
    }
    ...
    if (paints_scroll_hit_test) {
        ...
        BoxPainter(layout_view).RecordScrollHitTestData(paint_info, *background_client, &layout_view.FirstFragment());
    }
}
...
//    possible.
void ViewPainter::PaintRootElementGroup(const PaintInfo& paint_info, const gfx::Rect& pixel_snapped_background_rect,
    const PropertyTreeStateOrAlias& background_paint_state, const DisplayItemClient& background_client, bool painted_separate_backdrop,
    bool painted_separate_effect)
{
    GraphicsContext& context = paint_info.context;
    if (DrawingRecorder::UseCachedDrawingIfPossible(context, background_client, DisplayItem::kDocumentBackground)) {
        return;
    }
    DrawingRecorder recorder(context, background_client, DisplayItem::kDocumentBackground, pixel_snapped_background_rect);

    const LayoutView& layout_view = GetLayoutView();
    const Document& document = layout_view.GetDocument();
    const LocalFrameView& frame_view = *layout_view.GetFrameView();
    const ComputedStyle& style = box_fragment_.Style();
    bool paints_base_background = frame_view.ShouldPaintBaseBackgroundColor() && !frame_view.BaseBackgroundColor().IsFullyTransparent();
    Color base_background_color = paints_base_background ? frame_view.BaseBackgroundColor() : Color();
    if (document.Printing() && base_background_color == Color::kWhite) {
        // Leave a transparent background, assuming the paper or the PDF viewer
        // background is white by default. This allows further customization of the
        // background, e.g. in the case of https://crbug.com/498892.
    ...
    Color root_element_background_color = style.VisitedDependentColor(GetCSSPropertyBackgroundColor());

    const LayoutObject* root_object = document.documentElement() ? document.documentElement()->GetLayoutObject() : nullptr;

    // Special handling for print economy mode.
    bool force_background_to_white = BoxModelObjectPainter::ShouldForceWhiteBackgroundForPrintEconomy(document, style);
    ...
    if (!root_object || !root_object->IsBox()) {
        ...
        const auto& view_contents_state = layout_view.FirstFragment().ContentsProperties();
        if (view_contents_state != background_paint_state) {
            GeometryMapper::SourceToDestinationRect(view_contents_state.Transform(), background_paint_state.Transform(), paint_rect);
    ...
    bool should_clear_canvas = paints_base_background && (document.GetSettings() && document.GetSettings()->GetShouldClearDocumentBackground());

    if (!background_renderable) {
        if (!painted_separate_backdrop) {
            if (!base_background_color.IsFullyTransparent()) {
                context.FillRect(
                    pixel_snapped_background_rect, base_background_color, auto_dark_mode, should_clear_canvas ? SkBlendMode::kSrc : SkBlendMode::kSrcOver);
            } else if (should_clear_canvas) {
                context.FillRect(pixel_snapped_background_rect, Color(), auto_dark_mode, SkBlendMode::kClear);
            }
        }
        return;
    }
    ...
    bool should_draw_background_in_separate_buffer
        = BoxModelObjectPainter(layout_view).CalculateFillLayerOcclusionCulling(reversed_paint_list, style.BackgroundLayers());
    ...
    if (should_draw_background_in_separate_buffer && !painted_separate_effect) {
        if (!base_background_color.IsFullyTransparent()) {
            context.FillRect(paint_rect, base_background_color, auto_dark_mode, should_clear_canvas ? SkBlendMode::kSrc : SkBlendMode::kSrcOver);
        }
        context.BeginLayer();
    }

    Color combined_background_color
        = should_draw_background_in_separate_buffer ? root_element_background_color : base_background_color.Blend(root_element_background_color);
    ...
    if (!combined_background_color.IsFullyTransparent()) {
        context.FillRect(paint_rect, combined_background_color, auto_dark_mode,
            (should_draw_background_in_separate_buffer || should_clear_canvas) ? SkBlendMode::kSrc : SkBlendMode::kSrcOver);
    } else if (should_clear_canvas && !should_draw_background_in_separate_buffer) {
        context.FillRect(paint_rect, Color(), auto_dark_mode, SkBlendMode::kClear);
    }

    BoxBackgroundPaintContext bg_paint_context(layout_view, &box_fragment_, background_image_offset);
    BoxModelObjectPainter box_model_painter(layout_view);
    for (const auto* fill_layer : base::Reversed(reversed_paint_list)) {
        box_model_painter.PaintFillLayer(paint_info, Color(), *fill_layer, PhysicalRect(paint_rect), kBackgroundBleedNone, bg_paint_context);
    }
