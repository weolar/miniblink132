...
enum PaintLayerIteration {
    ...
    kNormalFlowAndPositiveZOrderChildren = kNormalFlowChildren | kPositiveZOrderChildren,
    kAllChildren = kNegativeZOrderChildren | kNormalFlowChildren | kPositiveZOrderChildren
};
...
class CORE_EXPORT PaintLayer : public GarbageCollected<PaintLayer>, public DisplayItemClient {
public:
    ...
    PaintLayer* Parent() const
    {
        return parent_.Get();
    }
    PaintLayer* PreviousSibling() const
    {
        return previous_.Get();
    }
    PaintLayer* NextSibling() const
    {
        return next_.Get();
    }
    PaintLayer* FirstChild() const
    {
        return first_.Get();
    }
    PaintLayer* LastChild() const
    {
        return last_.Get();
    }
    ...
    {
        return is_self_painting_layer_;
    }

    bool IsTransparent() const
    {
        return GetLayoutObject().StyleRef().HasOpacity() || GetLayoutObject().StyleRef().HasBlendMode() || GetLayoutObject().HasMask();
    }
    ...
    bool HitTest(const HitTestLocation& location, HitTestResult&, const PhysicalRect& hit_test_area);
    ...
    {
        return GetLayoutObject().Preserves3D();
    }
    ...
    void UpdateCompositorFilterOperationsForFilter(CompositorFilterOperations& operations);
    void SetFilterOnEffectNodeDirty()
    {
    ...
    void UpdateCompositorFilterOperationsForBackdropFilter(CompositorFilterOperations& operations, gfx::RRectF& backdrop_filter_bounds);
    ...
    void UpdateFilters(StyleDifference, const ComputedStyle* old_style, const ComputedStyle& new_style);
    void UpdateBackdropFilters(const ComputedStyle* old_style, const ComputedStyle& new_style);
    void UpdateClipPath(const ComputedStyle* old_style, const ComputedStyle& new_style);
    void UpdateOffsetPath(const ComputedStyle* old_style, const ComputedStyle& new_style);
    ...
    const PaintLayer* ContainingScrollContainerLayer(bool* is_fixed_to_view = nullptr) const;
    ...
    bool SelfNeedsRepaint() const
    {
        return self_needs_repaint_;
    }
    bool DescendantNeedsRepaint() const
    {
        return descendant_needs_repaint_;
    }
    ...
    {
        return needs_paint_phase_float_;
    }
    ...
    {
        return layer_list_mutation_allowed_;
    }
#endif
    ...
    // tracked separately.
    void SetInvisibleForPositionVisibility(LayerPositionVisibility visibility, bool invisible);
...
private:
    ...
    void SetNextSibling(PaintLayer* next)
    {
        next_ = next;
    }
    void SetPreviousSibling(PaintLayer* prev)
    {
        previous_ = prev;
    }
    void SetFirstChild(PaintLayer* first)
    {
        first_ = first;
    }
    void SetLastChild(PaintLayer* last)
    {
        last_ = last;
    }

    void AppendSingleFragmentForHitTesting(PaintLayerFragments&, const PaintLayerFragment* container_fragment, ShouldRespectOverflowClipType) const;

    void CollectFragments(PaintLayerFragments&, const PaintLayer* root_layer, ShouldRespectOverflowClipType = kRespectOverflowClip,
    ...
    public:
        ...
        HitTestRecursionData(const PhysicalRect& rect_arg, const HitTestLocation& location_arg, const HitTestLocation& original_location_arg);
    };

    PaintLayer* HitTestLayer(const PaintLayer& transform_container, const PaintLayerFragment* container_fragment, HitTestResult&,
        const HitTestRecursionData& recursion_data, bool applied_transform = false, HitTestingTransformState* = nullptr, double* z_offset = nullptr,
        bool overflow_controls_only = false);
    PaintLayer* HitTestLayerByApplyingTransform(const PaintLayer& transform_container, const PaintLayerFragment* container_fragment,
        const PaintLayerFragment& local_fragment, HitTestResult&, const HitTestRecursionData& recursion_data, HitTestingTransformState*, double* z_offset,
        bool overflow_controls_only, const PhysicalOffset& translation_offset = PhysicalOffset());
    PaintLayer* HitTestChildren(PaintLayerIteration, const PaintLayer& transform_container, const PaintLayerFragment* container_fragment, HitTestResult&,
        const HitTestRecursionData& recursion_data, HitTestingTransformState* container_transform_state, double* z_offset_for_descendants, double* z_offset,
        HitTestingTransformState* local_transform_state, bool depth_sort_descendants);

    HitTestingTransformState CreateLocalTransformState(const PaintLayer& transform_container, const FragmentData& container_fragment,
        const FragmentData& local_fragment, const HitTestRecursionData& recursion_data, const HitTestingTransformState* root_transform_state) const;

    bool HitTestFragmentWithPhase(
        HitTestResult&, const PhysicalBoxFragment*, const PhysicalOffset& fragment_offset, const HitTestLocation&, HitTestPhase phase) const;
    bool HitTestFragmentsWithPhase(const PaintLayerFragments&, HitTestResult&, const HitTestLocation&, HitTestPhase, bool& inside_clip_rect) const;
    bool HitTestForegroundForFragments(const PaintLayerFragments&, HitTestResult&, const HitTestLocation&, bool& inside_clip_rect) const;
    PaintLayer* HitTestTransformedLayerInFragments(const PaintLayer& transform_container, const PaintLayerFragment* container_fragment, HitTestResult&,
        const HitTestRecursionData&, HitTestingTransformState*, double* z_offset, bool overflow_controls_only, ShouldRespectOverflowClipType);
    bool HitTestClippedOutByClipPath(const PaintLayer& root_layer, const HitTestLocation&) const;
    ...
    // be set.
    enum DescendantDependentFlagsUpdateFlag { kNeedsDescendantDependentUpdate, kDoesNotNeedDescendantDependentUpdate };

    // Marks the ancestor chain for paint property update, and if
    // the flag is set, the descendant-dependent tree walk as well.
    void MarkAncestorChainForFlagsUpdate(DescendantDependentFlagsUpdateFlag = kNeedsDescendantDependentUpdate);

    void SetNeedsDescendantDependentFlagsUpdate();

    void UpdateTransformAfterStyleChange(StyleDifference, const ComputedStyle* old_style, const ComputedStyle& new_style);

    void MarkCompositingContainerChainForNeedsRepaint();

    void MergeNeedsPaintPhaseFlagsFrom(const PaintLayer& layer)
    {
        needs_paint_phase_descendant_outlines_ |= layer.needs_paint_phase_descendant_outlines_;
    ...
    PaintLayerStackingNode* StackingNode() const
    {
    ...
    static_assert(kMaxPaintResult < 2, "Should update number of bits of previous_paint_result_");
    ...
    FRIEND_TEST_ALL_PREFIXES(PaintLayerTest, DescendantDependentFlagsStopsAtThrottledFrames);
    FRIEND_TEST_ALL_PREFIXES(PaintLayerTest, PaintLayerTransformUpdatedOnStyleTransformAnimation);
    FRIEND_TEST_ALL_PREFIXES(PaintLayerOverlapTest, FixedUnderTransformDoesNotExpandBoundingBoxForOverlap);
    FRIEND_TEST_ALL_PREFIXES(PaintLayerOverlapTest, FixedUsesExpandedBoundingBoxForOverlap);
    FRIEND_TEST_ALL_PREFIXES(PaintLayerOverlapTest, FixedInScrollerUsesExpandedBoundingBoxForOverlap);
    FRIEND_TEST_ALL_PREFIXES(PaintLayerOverlapTest, NestedFixedUsesExpandedBoundingBoxForOverlap);
    FRIEND_TEST_ALL_PREFIXES(PaintLayerOverlapTest, FixedWithExpandedBoundsForChild);
    FRIEND_TEST_ALL_PREFIXES(PaintLayerOverlapTest, FixedWithClippedExpandedBoundsForChild);
    FRIEND_TEST_ALL_PREFIXES(PaintLayerOverlapTest, FixedWithExpandedBoundsForGrandChild);
    FRIEND_TEST_ALL_PREFIXES(PaintLayerOverlapTest, FixedWithExpandedBoundsForFixedChild);
};
...
public:
    explicit CheckAncestorPositionVisibilityScope(const PaintLayer& stacking_context)
        : reset_(&should_check_, stacking_context.descendant_needs_check_position_visibility_)
    {
...
template <typename T> struct SpaceTrait<T, std::enable_if_t<std::is_base_of<blink::PaintLayer, T>::value>> {
