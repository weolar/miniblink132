...
enum BorderEdgeFlag {
    ...
    kAllBorderEdges = kTopBorderEdge | kBottomBorderEdge | kLeftBorderEdge | kRightBorderEdge
};
...
inline bool IncludesAdjacentEdges(BorderEdgeFlags flags)
{
    ...
    // one vertical edge.
    return (flags & (kTopBorderEdge | kBottomBorderEdge)) && (flags & (kLeftBorderEdge | kRightBorderEdge));
}
...
{
    return !(style == EBorderStyle::kDotted || style == EBorderStyle::kDashed || style == EBorderStyle::kDouble);
}

inline bool BorderStyleHasInnerDetail(EBorderStyle style)
{
    return style == EBorderStyle::kGroove || style == EBorderStyle::kRidge || style == EBorderStyle::kDouble;
}
...
// bottom and right).
inline bool BorderStyleHasUnmatchedColorsAtCorner(EBorderStyle style, BoxSide side, BoxSide adjacent_side)
{
    // These styles match at the top/left and bottom/right.
    if (style == EBorderStyle::kInset || style == EBorderStyle::kGroove || style == EBorderStyle::kRidge || style == EBorderStyle::kOutset) {
        const BorderEdgeFlags top_right_flags = EdgeFlagForSide(BoxSide::kTop) | EdgeFlagForSide(BoxSide::kRight);
        const BorderEdgeFlags bottom_left_flags = EdgeFlagForSide(BoxSide::kBottom) | EdgeFlagForSide(BoxSide::kLeft);

        BorderEdgeFlags flags = EdgeFlagForSide(side) | EdgeFlagForSide(adjacent_side);
        return flags == top_right_flags || flags == bottom_left_flags;
    }
    return false;
}

inline bool BorderWillArcInnerEdge(const gfx::SizeF& first_radius, const gfx::SizeF& second_radius)
{
    return !first_radius.IsZero() || !second_radius.IsZero();
}

inline bool WillOverdraw(BoxSide side, EBorderStyle style, BorderEdgeFlags completed_edges)
{
...
inline bool BorderStylesRequireMiter(BoxSide side, BoxSide adjacent_side, EBorderStyle style, EBorderStyle adjacent_style)
{
    if (style == EBorderStyle::kDouble || adjacent_style == EBorderStyle::kDouble || adjacent_style == EBorderStyle::kGroove
        || adjacent_style == EBorderStyle::kRidge)
        return true;

    if (BorderStyleIsDottedOrDashed(style) != BorderStyleIsDottedOrDashed(adjacent_style))
        return true;
...
gfx::Rect CalculateSideRect(const FloatRoundedRect& outer_border, const BorderEdge& edge, BoxSide side)
{
...
FloatRoundedRect CalculateAdjustedInnerBorder(const FloatRoundedRect& inner_border, BoxSide side)
{
    ...
    switch (side) {
    case BoxSide::kTop:
        overshoot = new_radii.TopLeft().width() + new_radii.TopRight().width() - new_rect.width();
        ...
        max_radii = std::max(new_radii.TopLeft().height(), new_radii.TopRight().height());
        if (max_radii > new_rect.height())
            new_rect.set_height(max_radii);
        break;

    case BoxSide::kBottom:
        overshoot = new_radii.BottomLeft().width() + new_radii.BottomRight().width() - new_rect.width();
        if (overshoot > 0.1) {
            new_rect.set_width(new_rect.width() + overshoot);
        ...
        max_radii = std::max(new_radii.BottomLeft().height(), new_radii.BottomRight().height());
        if (max_radii > new_rect.height()) {
    ...
    case BoxSide::kLeft:
        overshoot = new_radii.TopLeft().height() + new_radii.BottomLeft().height() - new_rect.height();
        if (overshoot > 0.1) {
        ...
        new_radii.SetTopRight(gfx::SizeF(0, 0));
        new_radii.SetBottomRight(gfx::SizeF(0, 0));
        max_radii = std::max(new_radii.TopLeft().width(), new_radii.BottomLeft().width());
        if (max_radii > new_rect.width())
            new_rect.set_width(max_radii);
        break;

    case BoxSide::kRight:
        overshoot = new_radii.TopRight().height() + new_radii.BottomRight().height() - new_rect.height();
        if (overshoot > 0.1) {
            new_rect.set_height(new_rect.height() + overshoot);
        ...
        new_radii.SetTopLeft(gfx::SizeF(0, 0));
        new_radii.SetBottomLeft(gfx::SizeF(0, 0));
        max_radii = std::max(new_radii.TopRight().width(), new_radii.BottomRight().width());
        if (max_radii > new_rect.width()) {
...
void DrawSolidBorderRect(GraphicsContext& context, const gfx::Rect& border_rect, int border_width, const Color& color, const AutoDarkMode& auto_dark_mode)
{
...
void DrawBleedAdjustedDRRect(GraphicsContext& context, BackgroundBleedAvoidance bleed_avoidance, const FloatRoundedRect& outer, const FloatRoundedRect& inner,
    Color color, const AutoDarkMode& auto_dark_mode)
{
...
static_assert(EBorderStyle::kNone == static_cast<EBorderStyle>(0), "unexpected EBorderStyle value");
static_assert(EBorderStyle::kHidden == static_cast<EBorderStyle>(1), "unexpected EBorderStyle value");
static_assert(EBorderStyle::kInset == static_cast<EBorderStyle>(2), "unexpected EBorderStyle value");
static_assert(EBorderStyle::kGroove == static_cast<EBorderStyle>(3), "unexpected EBorderStyle value");
static_assert(EBorderStyle::kOutset == static_cast<EBorderStyle>(4), "unexpected EBorderStyle value");
static_assert(EBorderStyle::kRidge == static_cast<EBorderStyle>(5), "unexpected EBorderStyle value");
static_assert(EBorderStyle::kDotted == static_cast<EBorderStyle>(6), "unexpected EBorderStyle value");
static_assert(EBorderStyle::kDashed == static_cast<EBorderStyle>(7), "unexpected EBorderStyle value");
static_assert(EBorderStyle::kSolid == static_cast<EBorderStyle>(8), "unexpected EBorderStyle value");
static_assert(EBorderStyle::kDouble == static_cast<EBorderStyle>(9), "unexpected EBorderStyle value");

static_assert(static_cast<unsigned>(BoxSide::kTop) == 0, "unexpected BoxSide value");
static_assert(static_cast<unsigned>(BoxSide::kRight) == 1, "unexpected BoxSide value");
static_assert(static_cast<unsigned>(BoxSide::kBottom) == 2, "unexpected BoxSide value");
static_assert(static_cast<unsigned>(BoxSide::kLeft) == 3, "unexpected BoxSide value");
...
public:
    explicit OpacityGroup(float alpha)
        : edge_flags(0)
        , alpha(alpha)
    {
    }
...
void ClipPolygon(GraphicsContext& context, base::span<const gfx::PointF> vertices, bool antialiased)
{
    SkPath path;
...
void DrawDashedOrDottedBoxSide(GraphicsContext& context, int x1, int y1, int x2, int y2, BoxSide side, Color color, int thickness, EBorderStyle style,
    bool antialias, const AutoDarkMode& auto_dark_mode)
{
    ...
    styled_stroke.SetStyle(style == EBorderStyle::kDashed ? kDashedStroke : kDottedStroke);

    switch (side) {
    case BoxSide::kBottom:
    case BoxSide::kTop: {
        int mid_y = y1 + thickness / 2;
        context.DrawLine(gfx::Point(x1, mid_y), gfx::Point(x2, mid_y), styled_stroke, auto_dark_mode);
        break;
    }
    case BoxSide::kRight:
    case BoxSide::kLeft: {
        int mid_x = x1 + thickness / 2;
        context.DrawLine(gfx::Point(mid_x, y1), gfx::Point(mid_x, y2), styled_stroke, auto_dark_mode);
        break;
    }
    }
}

void DrawLineForBoxSide(GraphicsContext& context, int x1, int y1, int x2, int y2, BoxSide side, Color color, EBorderStyle style, int adjacent_width1,
    int adjacent_width2, bool antialias, const AutoDarkMode& auto_dark_mode);

Color CalculateBorderStyleColor(const EBorderStyle& style, const BoxSide& side, const Color& color)
{
    bool is_darken = (side == BoxSide::kTop || side == BoxSide::kLeft) == (style == EBorderStyle::kInset);
    ...
    auto should_lighten_color = [color, dark_color]() -> bool {
        ...
        return color_utils::GetContrastRatio(color.toSkColor4f(), dark_color.toSkColor4f()) < kMinimumBorderEdgeContrastRatio;
    };
...
void DrawDoubleBoxSide(GraphicsContext& context, int x1, int y1, int x2, int y2, int length, BoxSide side, Color color, int thickness, int adjacent_width1,
    int adjacent_width2, bool antialias, const AutoDarkMode& auto_dark_mode)
{
    ...
    if (!adjacent_width1 && !adjacent_width2) {
        ...
        case BoxSide::kTop:
        case BoxSide::kBottom:
            context.FillRect(gfx::Rect(x1, y1, length, third_of_thickness), auto_dark_mode);
            context.FillRect(gfx::Rect(x1, y2 - third_of_thickness, length, third_of_thickness), auto_dark_mode);
            break;
        case BoxSide::kLeft:
        case BoxSide::kRight:
            context.FillRect(gfx::Rect(x1, y1, third_of_thickness, length), auto_dark_mode);
            context.FillRect(gfx::Rect(x2 - third_of_thickness, y1, third_of_thickness, length), auto_dark_mode);
            break;
        }
    ...
    int adjacent1_big_third = ((adjacent_width1 > 0) ? adjacent_width1 + 1 : adjacent_width1 - 1) / 3;
    int adjacent2_big_third = ((adjacent_width2 > 0) ? adjacent_width2 + 1 : adjacent_width2 - 1) / 3;

    switch (side) {
    case BoxSide::kTop:
        DrawLineForBoxSide(context, x1 + std::max((-adjacent_width1 * 2 + 1) / 3, 0), y1, x2 - std::max((-adjacent_width2 * 2 + 1) / 3, 0),
            y1 + third_of_thickness, side, color, EBorderStyle::kSolid, adjacent1_big_third, adjacent2_big_third, antialias, auto_dark_mode);
        DrawLineForBoxSide(context, x1 + std::max((adjacent_width1 * 2 + 1) / 3, 0), y2 - third_of_thickness, x2 - std::max((adjacent_width2 * 2 + 1) / 3, 0),
            y2, side, color, EBorderStyle::kSolid, adjacent1_big_third, adjacent2_big_third, antialias, auto_dark_mode);
        break;
    case BoxSide::kLeft:
        DrawLineForBoxSide(context, x1, y1 + std::max((-adjacent_width1 * 2 + 1) / 3, 0), x1 + third_of_thickness,
            y2 - std::max((-adjacent_width2 * 2 + 1) / 3, 0), side, color, EBorderStyle::kSolid, adjacent1_big_third, adjacent2_big_third, antialias,
            auto_dark_mode);
        DrawLineForBoxSide(context, x2 - third_of_thickness, y1 + std::max((adjacent_width1 * 2 + 1) / 3, 0), x2,
            y2 - std::max((adjacent_width2 * 2 + 1) / 3, 0), side, color, EBorderStyle::kSolid, adjacent1_big_third, adjacent2_big_third, antialias,
            auto_dark_mode);
        break;
    case BoxSide::kBottom:
...
void DrawRidgeOrGrooveBoxSide(GraphicsContext& context, int x1, int y1, int x2, int y2, BoxSide side, Color color, EBorderStyle style, int adjacent_width1,
    int adjacent_width2, bool antialias, const AutoDarkMode& auto_dark_mode)
{
    EBorderStyle s1;
    EBorderStyle s2;
    ...
    int adjacent1_big_half = ((adjacent_width1 > 0) ? adjacent_width1 + 1 : adjacent_width1 - 1) / 2;
    int adjacent2_big_half = ((adjacent_width2 > 0) ? adjacent_width2 + 1 : adjacent_width2 - 1) / 2;

    switch (side) {
    case BoxSide::kTop:
        DrawLineForBoxSide(context, x1 + std::max(-adjacent_width1, 0) / 2, y1, x2 - std::max(-adjacent_width2, 0) / 2, (y1 + y2 + 1) / 2, side, color, s1,
            adjacent1_big_half, adjacent2_big_half, antialias, auto_dark_mode);
        DrawLineForBoxSide(context, x1 + std::max(adjacent_width1 + 1, 0) / 2, (y1 + y2 + 1) / 2, x2 - std::max(adjacent_width2 + 1, 0) / 2, y2, side, color,
            s2, adjacent_width1 / 2, adjacent_width2 / 2, antialias, auto_dark_mode);
        break;
    case BoxSide::kLeft:
        DrawLineForBoxSide(context, x1, y1 + std::max(-adjacent_width1, 0) / 2, (x1 + x2 + 1) / 2, y2 - std::max(-adjacent_width2, 0) / 2, side, color, s1,
            adjacent1_big_half, adjacent2_big_half, antialias, auto_dark_mode);
        DrawLineForBoxSide(context, (x1 + x2 + 1) / 2, y1 + std::max(adjacent_width1 + 1, 0) / 2, x2, y2 - std::max(adjacent_width2 + 1, 0) / 2, side, color,
            s2, adjacent_width1 / 2, adjacent_width2 / 2, antialias, auto_dark_mode);
        break;
    case BoxSide::kBottom:
        DrawLineForBoxSide(context, x1 + std::max(adjacent_width1, 0) / 2, y1, x2 - std::max(adjacent_width2, 0) / 2, (y1 + y2 + 1) / 2, side, color, s2,
            adjacent1_big_half, adjacent2_big_half, antialias, auto_dark_mode);
        DrawLineForBoxSide(context, x1 + std::max(-adjacent_width1 + 1, 0) / 2, (y1 + y2 + 1) / 2, x2 - std::max(-adjacent_width2 + 1, 0) / 2, y2, side, color,
            s1, adjacent_width1 / 2, adjacent_width2 / 2, antialias, auto_dark_mode);
        break;
    case BoxSide::kRight:
        DrawLineForBoxSide(context, x1, y1 + std::max(adjacent_width1, 0) / 2, (x1 + x2 + 1) / 2, y2 - std::max(adjacent_width2, 0) / 2, side, color, s2,
            adjacent1_big_half, adjacent2_big_half, antialias, auto_dark_mode);
        DrawLineForBoxSide(context, (x1 + x2 + 1) / 2, y1 + std::max(-adjacent_width1 + 1, 0) / 2, x2, y2 - std::max(-adjacent_width2 + 1, 0) / 2, side, color,
            s1, adjacent_width1 / 2, adjacent_width2 / 2, antialias, auto_dark_mode);
        break;
...
void FillQuad(GraphicsContext& context, const gfx::QuadF& quad, const Color& color, bool antialias, const AutoDarkMode& auto_dark_mode)
{
    SkPath path;
...
void DrawSolidBoxSide(GraphicsContext& context, int x1, int y1, int x2, int y2, BoxSide side, Color color, int adjacent_width1, int adjacent_width2,
    bool antialias, const AutoDarkMode& auto_dark_mode)
{
    DCHECK_GE(x2, x1);
    DCHECK_GE(y2, y1);
    ...
    if (!adjacent_width1 && !adjacent_width2) {
        ...
        if (antialias != was_antialiased)
            context.SetShouldAntialias(antialias);
        context.FillRect(gfx::Rect(x1, y1, x2 - x1, y2 - y1), color, auto_dark_mode);
        if (antialias != was_antialiased)
            context.SetShouldAntialias(was_antialiased);
        return;
    }
    ...
    int adjacent_width2, bool antialias, const AutoDarkMode& auto_dark_mode)
{
    ...
    case EBorderStyle::kDashed:
        DrawDashedOrDottedBoxSide(context, x1, y1, x2, y2, side, color, thickness, style, antialias, auto_dark_mode);
        break;
    case EBorderStyle::kDouble:
        DrawDoubleBoxSide(context, x1, y1, x2, y2, length, side, color, thickness, adjacent_width1, adjacent_width2, antialias, auto_dark_mode);
        break;
    case EBorderStyle::kRidge:
    case EBorderStyle::kGroove:
        DrawRidgeOrGrooveBoxSide(context, x1, y1, x2, y2, side, color, style, adjacent_width1, adjacent_width2, antialias, auto_dark_mode);
        break;
    ...
    case EBorderStyle::kSolid:
        DrawSolidBoxSide(context, x1, y1, x2, y2, side, color, adjacent_width1, adjacent_width2, antialias, auto_dark_mode);
        break;
    }
}

void FindIntersection(const gfx::PointF& p1, const gfx::PointF& p2, const gfx::PointF& d1, const gfx::PointF& d2, gfx::PointF& intersection)
{
    ...
    float param = ((d1.x() - p1.x()) * dy_length - (d1.y() - p1.y()) * dx_length) / denom;
...
public:
    explicit ComplexBorderInfo(const BoxBorderPainter& border_painter)
    {
        ...
        // alpha, style, side.
        std::sort(sorted_sides.begin(), sorted_sides.end(), [&border_painter](BoxSide a, BoxSide b) -> bool {
            ...
            const unsigned style_priority_a = kStylePriority[static_cast<unsigned>(edge_a.BorderStyle())];
            const unsigned style_priority_b = kStylePriority[static_cast<unsigned>(edge_b.BorderStyle())];
            if (style_priority_a != style_priority_b)
                return style_priority_a < style_priority_b;

            return kSidePriority[static_cast<unsigned>(a)] < kSidePriority[static_cast<unsigned>(b)];
        });
...
private:
    void BuildOpacityGroups(const BoxBorderPainter& border_painter, const Vector<BoxSide, 4>& sorted_sides)
    {
...
void BoxBorderPainter::DrawDoubleBorder() const
{
    ...
    // outer stripe
    const PhysicalBoxStrut outer_third_outsets = DoubleStripeOutsets(BorderEdge::kDoubleBorderStripeOuter);
    FloatRoundedRect outer_third_rect
        = RoundedBorderGeometry::PixelSnappedRoundedBorderWithOutsets(style_, border_rect_, outer_third_outsets, sides_to_include_);
    if (force_rectangular)
        outer_third_rect.SetRadii(FloatRoundedRect::Radii());
    DrawBleedAdjustedDRRect(context_, bleed_avoidance_, outer_, outer_third_rect, color, auto_dark_mode);

    // inner stripe
    const PhysicalBoxStrut inner_third_outsets = DoubleStripeOutsets(BorderEdge::kDoubleBorderStripeInner);
    FloatRoundedRect inner_third_rect
        = RoundedBorderGeometry::PixelSnappedRoundedBorderWithOutsets(style_, border_rect_, inner_third_outsets, sides_to_include_);
    if (force_rectangular)
...
bool BoxBorderPainter::PaintBorderFastPath() const
{
    ...
    if (FirstEdge().BorderStyle() != EBorderStyle::kSolid && FirstEdge().BorderStyle() != EBorderStyle::kDouble)
        return false;
    ...
    if (visible_edge_set_ == kAllBorderEdges) {
        if (FirstEdge().BorderStyle() == EBorderStyle::kSolid) {
            if (is_uniform_width_ && !outer_.IsRounded()) {
                ...
                DrawSolidBorderRect(
                    context_, gfx::ToRoundedRect(outer_.Rect()), FirstEdge().Width(), FirstEdge().GetColor(), PaintAutoDarkMode(style_, element_role_));
            } else {
                // 4-side, solid border => one drawDRRect()
                DrawBleedAdjustedDRRect(context_, bleed_avoidance_, outer_, inner_, FirstEdge().GetColor(), PaintAutoDarkMode(style_, element_role_));
            }
        } else {
    ...
    if (FirstEdge().BorderStyle() == EBorderStyle::kSolid && !outer_.IsRounded() && has_transparency_) {
        DCHECK(visible_edge_set_ != kAllBorderEdges);
...
BoxBorderPainter::BoxBorderPainter(GraphicsContext& context, const PhysicalRect& border_rect, const ComputedStyle& style,
    BackgroundBleedAvoidance bleed_avoidance, PhysicalBoxSides sides_to_include)
    : context_(context)
    ...
    , has_transparency_(false)
{
    ...
    outer_ = RoundedBorderGeometry::PixelSnappedRoundedBorder(style_, border_rect, sides_to_include);
    inner_ = RoundedBorderGeometry::PixelSnappedRoundedInnerBorder(style_, border_rect, sides_to_include);
...
BoxBorderPainter::BoxBorderPainter(
    GraphicsContext& context, const ComputedStyle& style, const PhysicalRect& border_rect, int width, const PhysicalBoxStrut& inner_outsets)
    : context_(context)
    , border_rect_(border_rect)
    ...
    , has_transparency_(false)
{
    ...
    BorderEdge edge(width, style.VisitedDependentColor(GetCSSPropertyOutlineColor()), style.OutlineStyle());
    for (auto& e : edges_)
        e = edge;
    ComputeBorderProperties();

    outer_ = RoundedBorderGeometry::PixelSnappedRoundedBorderWithOutsets(style, border_rect, outer_outsets_);
    is_rounded_ = outer_.IsRounded();

    inner_ = RoundedBorderGeometry::PixelSnappedRoundedBorderWithOutsets(style, border_rect, inner_outsets);
...
void BoxBorderPainter::ComputeBorderProperties()
{
    for (unsigned i = 0; i < std::size(edges_); ++i) {
        ...
        is_uniform_style_ &= edge.BorderStyle() == edges_[first_visible_edge_].BorderStyle();
...
//
BorderEdgeFlags BoxBorderPainter::PaintOpacityGroup(const ComplexBorderInfo& border_info, unsigned index, float effective_opacity) const
{
    ...
    const OpacityGroup& group = border_info.opacity_groups[opacity_group_count - index - 1];
    ...
    // possibility).
    bool needs_layer = group.alpha != 1.0f && (IncludesAdjacentEdges(group.edge_flags) || (index + 1 < border_info.opacity_groups.size()));
    ...
    BorderEdgeFlags completed_edges = PaintOpacityGroup(border_info, index + 1, effective_opacity);
...
void BoxBorderPainter::PaintSide(const ComplexBorderInfo& border_info, BoxSide side, float alpha, BorderEdgeFlags completed_edges) const
{
    const BorderEdge& edge = Edge(side);
    DCHECK(edge.ShouldRender());
    const Color color
        = Color::FromColorSpace(edge.GetColor().GetColorSpace(), edge.GetColor().Param0(), edge.GetColor().Param1(), edge.GetColor().Param2(), alpha);
    ...
    case BoxSide::kTop: {
        bool use_path = is_rounded_
            && (BorderStyleHasInnerDetail(edge.BorderStyle()) || BorderWillArcInnerEdge(inner_.GetRadii().TopLeft(), inner_.GetRadii().TopRight()));
        if (use_path) {
        ...
        PaintOneBorderSide(side_rect, BoxSide::kTop, BoxSide::kLeft, BoxSide::kRight, path, color, completed_edges);
        break;
    }
    case BoxSide::kBottom: {
        bool use_path = is_rounded_
            && (BorderStyleHasInnerDetail(edge.BorderStyle()) || BorderWillArcInnerEdge(inner_.GetRadii().BottomLeft(), inner_.GetRadii().BottomRight()));
        if (use_path) {
            path = &border_info.rounded_border_path;
        } else {
        ...
        PaintOneBorderSide(side_rect, BoxSide::kBottom, BoxSide::kLeft, BoxSide::kRight, path, color, completed_edges);
        break;
    }
    case BoxSide::kLeft: {
        bool use_path = is_rounded_
            && (BorderStyleHasInnerDetail(edge.BorderStyle()) || BorderWillArcInnerEdge(inner_.GetRadii().BottomLeft(), inner_.GetRadii().TopLeft()));
        if (use_path) {
            path = &border_info.rounded_border_path;
        } else {
        ...
        PaintOneBorderSide(side_rect, BoxSide::kLeft, BoxSide::kTop, BoxSide::kBottom, path, color, completed_edges);
        break;
    }
    case BoxSide::kRight: {
        bool use_path = is_rounded_
            && (BorderStyleHasInnerDetail(edge.BorderStyle()) || BorderWillArcInnerEdge(inner_.GetRadii().BottomRight(), inner_.GetRadii().TopRight()));
        if (use_path) {
            path = &border_info.rounded_border_path;
        } else {
        ...
        PaintOneBorderSide(side_rect, BoxSide::kRight, BoxSide::kTop, BoxSide::kBottom, path, color, completed_edges);
        break;
    }
    default:
        NOTREACHED();
...
BoxBorderPainter::MiterType BoxBorderPainter::ComputeMiter(BoxSide side, BoxSide adjacent_side, BorderEdgeFlags completed_edges) const
{
    ...
    // style.
    if (BorderStylesRequireMiter(side, adjacent_side, Edge(side).BorderStyle(), adjacent_edge.BorderStyle()))
        return kHardMiter;
...
bool BoxBorderPainter::MitersRequireClipping(MiterType miter1, MiterType miter2, EBorderStyle style)
{
    ...
    should_clip = should_clip || ((miter1 != kNoMiter || miter2 != kNoMiter) && StyleRequiresClipPolygon(style));

    return should_clip;
}

void BoxBorderPainter::PaintOneBorderSide(const gfx::Rect& side_rect, BoxSide side, BoxSide adjacent_side1, BoxSide adjacent_side2, const Path* path,
    Color color, BorderEdgeFlags completed_edges) const
{
    ...
    if (path) {
        ...
        if (!inner_.IsRenderable()) {
            FloatRoundedRect adjusted_inner_rect = CalculateAdjustedInnerBorder(inner_, side);
            if (!adjusted_inner_rect.IsEmpty()) {
                context_.ClipOutRoundedRect(adjusted_inner_rect);
            }
        }

        int stroke_thickness = std::max(std::max(edge_to_render.Width(), adjacent_edge1.Width()), adjacent_edge2.Width());
        DrawBoxSideFromPath(*path, edge_to_render.Width(), stroke_thickness, side, color, edge_to_render.BorderStyle());
    } else {
        ...
        DrawLineForBoxSide(context_, side_rect.x(), side_rect.y(), side_rect.right(), side_rect.bottom(), side, color, edge_to_render.BorderStyle(),
            miter1 != kNoMiter ? adjacent_edge1.Width() : 0, miter2 != kNoMiter ? adjacent_edge2.Width() : 0,
            /*antialias*/ true, PaintAutoDarkMode(style_, element_role_));
    }
}

void BoxBorderPainter::DrawBoxSideFromPath(
    const Path& border_path, int border_thickness, int stroke_thickness, BoxSide side, Color color, EBorderStyle border_style) const
{
    if (border_thickness <= 0)
        return;

    // The caller should have adjusted border_style.
    DCHECK_EQ(border_style, BorderEdge::EffectiveStyle(border_style, border_thickness));
    ...
    case EBorderStyle::kDotted:
    case EBorderStyle::kDashed: {
        DrawDashedDottedBoxSideFromPath(border_thickness, stroke_thickness, color, border_style);
        return;
    }
    case EBorderStyle::kDouble: {
        DrawDoubleBoxSideFromPath(border_path, border_thickness, stroke_thickness, side, color);
        return;
    }
    case EBorderStyle::kRidge:
    case EBorderStyle::kGroove: {
        DrawRidgeGrooveBoxSideFromPath(border_path, border_thickness, stroke_thickness, side, color, border_style);
        return;
    }
    case EBorderStyle::kInset:
    case EBorderStyle::kOutset:
...
void BoxBorderPainter::DrawDashedDottedBoxSideFromPath(int border_thickness, int stroke_thickness, Color color, EBorderStyle border_style) const
{
    // Convert the path to be down the middle of the dots or dashes.
    Path centerline_path;
    centerline_path.AddRoundedRect(RoundedBorderGeometry::PixelSnappedRoundedBorderWithOutsets(style_, border_rect_, CenterOutsets(), sides_to_include_));

    context_.SetStrokeColor(color);

    const StrokeStyle stroke_style = border_style == EBorderStyle::kDashed ? kDashedStroke : kDottedStroke;
    ...
    const StrokeData stroke_data
        = styled_stroke.ConvertToStrokeData({ static_cast<int>(centerline_path.length()), border_thickness, centerline_path.IsClosed() });
    context_.SetStroke(stroke_data);
    context_.StrokePath(centerline_path, PaintAutoDarkMode(style_, element_role_));
}

void BoxBorderPainter::DrawWideDottedBoxSideFromPath(const Path& border_path, int border_thickness) const
{
    StyledStrokeData styled_stroke;
    ...
    // TODO(crbug.com/344234): stroking the border path causes issues with
    // tight corners.
    const StrokeData stroke_data = styled_stroke.ConvertToStrokeData({ static_cast<int>(border_path.length()), border_thickness, border_path.IsClosed() });
    context_.SetStroke(stroke_data);
    context_.StrokePath(border_path, PaintAutoDarkMode(style_, element_role_));
}

void BoxBorderPainter::DrawDoubleBoxSideFromPath(const Path& border_path, int border_thickness, int stroke_thickness, BoxSide side, Color color) const
{
    // Draw inner border line
    {
        GraphicsContextStateSaver state_saver(context_);
        const PhysicalBoxStrut inner_outsets = DoubleStripeOutsets(BorderEdge::kDoubleBorderStripeInner);
        FloatRoundedRect inner_clip = RoundedBorderGeometry::PixelSnappedRoundedBorderWithOutsets(style_, border_rect_, inner_outsets, sides_to_include_);

        context_.ClipRoundedRect(inner_clip);
        DrawBoxSideFromPath(border_path, border_thickness, stroke_thickness, side, color, EBorderStyle::kSolid);
    }
    ...
    // Draw outer border line
    {
        ...
        PhysicalBoxStrut outer_outsets = DoubleStripeOutsets(BorderEdge::kDoubleBorderStripeOuter);
        ...
        FloatRoundedRect outer_clip = RoundedBorderGeometry::PixelSnappedRoundedBorderWithOutsets(style_, used_border_rect, outer_outsets, sides_to_include_);
    ...
    const Path& border_path, int border_thickness, int stroke_thickness, BoxSide side, Color color, EBorderStyle border_style) const
{
    ...
    // Paint full border
    DrawBoxSideFromPath(border_path, border_thickness, stroke_thickness, side, color, s1);

    // Paint inner only
    GraphicsContextStateSaver state_saver(context_);
    FloatRoundedRect clip_rect = RoundedBorderGeometry::PixelSnappedRoundedBorderWithOutsets(style_, border_rect_, CenterOutsets(), sides_to_include_);

    context_.ClipRoundedRect(clip_rect);
    DrawBoxSideFromPath(border_path, border_thickness, stroke_thickness, side, color, s2);
}

gfx::Rect BoxBorderPainter::CalculateSideRectIncludingInner(BoxSide side) const
{
    gfx::Rect side_rect = gfx::ToRoundedRect(outer_.Rect());
    int width;
...
void BoxBorderPainter::ClipBorderSidePolygon(BoxSide side, MiterType first_miter, MiterType second_miter) const
{
    ...
    case BoxSide::kTop:
        ...
        if (!inner_.GetRadii().TopLeft().IsZero()) {
            FindIntersection(edge_quad[0], edge_quad[1], gfx::PointF(edge_quad[1].x() + inner_.GetRadii().TopLeft().width(), edge_quad[1].y()),
                gfx::PointF(edge_quad[1].x(), edge_quad[1].y() + inner_.GetRadii().TopLeft().height()), edge_quad[1]);
            ...
            if (edge_quad[1].y() > inner_points[2].y()) {
                FindIntersection(edge_quad[0], edge_quad[1], inner_points[3], inner_points[2], edge_quad[1]);
            }
            if (edge_quad[1].x() > inner_points[2].x()) {
                FindIntersection(edge_quad[0], edge_quad[1], inner_points[1], inner_points[2], edge_quad[1]);
            }
            if (edge_quad[2].y() < edge_quad[1].y() && edge_quad[2].x() > edge_quad[1].x()) {
                edge_pentagon = { edge_quad[0], edge_quad[1], gfx::PointF(edge_quad[2].x(), edge_quad[1].y()), edge_quad[2], edge_quad[3] };
            }
        }

        if (!inner_.GetRadii().TopRight().IsZero()) {
            FindIntersection(edge_quad[3], edge_quad[2], gfx::PointF(edge_quad[2].x() - inner_.GetRadii().TopRight().width(), edge_quad[2].y()),
                gfx::PointF(edge_quad[2].x(), edge_quad[2].y() + inner_.GetRadii().TopRight().height()), edge_quad[2]);
            ...
            if (edge_quad[2].y() > inner_points[3].y()) {
                FindIntersection(edge_quad[3], edge_quad[2], inner_points[3], inner_points[2], edge_quad[2]);
            }
            if (edge_quad[2].x() < inner_points[3].x()) {
                FindIntersection(edge_quad[3], edge_quad[2], inner_points[0], inner_points[3], edge_quad[2]);
            }
            if (edge_quad[2].y() > edge_quad[1].y() && edge_quad[2].x() > edge_quad[1].x()) {
                edge_pentagon = { edge_quad[0], edge_quad[1], gfx::PointF(edge_quad[1].x(), edge_quad[2].y()), edge_quad[2], edge_quad[3] };
            }
        }
        break;
    ...
    case BoxSide::kLeft:
        ...
        if (!inner_.GetRadii().TopLeft().IsZero()) {
            FindIntersection(edge_quad[3], edge_quad[2], gfx::PointF(edge_quad[2].x() + inner_.GetRadii().TopLeft().width(), edge_quad[2].y()),
                gfx::PointF(edge_quad[2].x(), edge_quad[2].y() + inner_.GetRadii().TopLeft().height()), edge_quad[2]);
            ...
            if (edge_quad[2].x() > inner_points[2].x()) {
                FindIntersection(edge_quad[3], edge_quad[2], inner_points[1], inner_points[2], edge_quad[2]);
            }
            if (edge_quad[2].y() < edge_quad[1].y() && edge_quad[2].x() > edge_quad[1].x()) {
                edge_pentagon = { edge_quad[0], edge_quad[1], gfx::PointF(edge_quad[2].x(), edge_quad[1].y()), edge_quad[2], edge_quad[3] };
            }
        ...
        if (!inner_.GetRadii().BottomLeft().IsZero()) {
            FindIntersection(edge_quad[0], edge_quad[1], gfx::PointF(edge_quad[1].x() + inner_.GetRadii().BottomLeft().width(), edge_quad[1].y()),
                gfx::PointF(edge_quad[1].x(), edge_quad[1].y() - inner_.GetRadii().BottomLeft().height()), edge_quad[1]);
            ...
            if (edge_quad[1].y() < inner_points[1].y()) {
                FindIntersection(edge_quad[0], edge_quad[1], inner_points[0], inner_points[1], edge_quad[1]);
            }
    ...
    case BoxSide::kBottom:
        ...
        if (!inner_.GetRadii().BottomLeft().IsZero()) {
            FindIntersection(edge_quad[3], edge_quad[2], gfx::PointF(edge_quad[2].x() + inner_.GetRadii().BottomLeft().width(), edge_quad[2].y()),
                gfx::PointF(edge_quad[2].x(), edge_quad[2].y() - inner_.GetRadii().BottomLeft().height()), edge_quad[2]);
            DCHECK(bound_quad2.y() >= edge_quad[2].y());
            bound_quad1.set_y(edge_quad[2].y());
            bound_quad2.set_y(edge_quad[2].y());

            if (edge_quad[2].y() < inner_points[1].y()) {
                FindIntersection(edge_quad[3], edge_quad[2], inner_points[0], inner_points[1], edge_quad[2]);
            }
        ...
        if (!inner_.GetRadii().BottomRight().IsZero()) {
            FindIntersection(edge_quad[0], edge_quad[1], gfx::PointF(edge_quad[1].x() - inner_.GetRadii().BottomRight().width(), edge_quad[1].y()),
                gfx::PointF(edge_quad[1].x(), edge_quad[1].y() - inner_.GetRadii().BottomRight().height()), edge_quad[1]);
            ...
            if (edge_quad[1].x() < inner_points[0].x()) {
                FindIntersection(edge_quad[0], edge_quad[1], inner_points[0], inner_points[3], edge_quad[1]);
            }
            if (edge_quad[2].x() < edge_quad[1].x() && edge_quad[2].y() > edge_quad[1].y()) {
                edge_pentagon = { edge_quad[0], edge_quad[1], gfx::PointF(edge_quad[2].x(), edge_quad[1].y()), edge_quad[2], edge_quad[3] };
            }
        }
        break;
    ...
    case BoxSide::kRight:
        ...
        if (!inner_.GetRadii().TopRight().IsZero()) {
            FindIntersection(edge_quad[0], edge_quad[1], gfx::PointF(edge_quad[1].x() - inner_.GetRadii().TopRight().width(), edge_quad[1].y()),
                gfx::PointF(edge_quad[1].x(), edge_quad[1].y() + inner_.GetRadii().TopRight().height()), edge_quad[1]);
        ...
        if (!inner_.GetRadii().BottomRight().IsZero()) {
            FindIntersection(edge_quad[3], edge_quad[2], gfx::PointF(edge_quad[2].x() - inner_.GetRadii().BottomRight().width(), edge_quad[2].y()),
                gfx::PointF(edge_quad[2].x(), edge_quad[2].y() - inner_.GetRadii().BottomRight().height()), edge_quad[2]);
    ...
    if (first_miter == second_miter) {
        if (!edge_pentagon.empty() && !inner_.IsRenderable()) {
            ...
            ClipPolygon(context_, base::span<const gfx::PointF>(edge_pentagon.data(), edge_pentagon.size()), first_miter == kSoftMiter);
            return;
        }

        ClipPolygon(context_, base::span<const gfx::PointF>(edge_quad, std::size(edge_quad)), first_miter == kSoftMiter);
        return;
    }
    ...
    if (first_miter != kNoMiter) {
        ...
        FindIntersection(edge_quad[0], edge_quad[1], bound_quad1, bound_quad2, clipping_quad[1]);
    ...
    if (second_miter != kNoMiter) {
        ...
        FindIntersection(edge_quad[2], edge_quad[3], bound_quad1, bound_quad2, clipping_quad[2]);
...
PhysicalBoxStrut BoxBorderPainter::DoubleStripeOutsets(BorderEdge::DoubleBorderStripe stripe) const
{
    return outer_outsets_
        - PhysicalBoxStrut(Edge(BoxSide::kTop).GetDoubleBorderStripeWidth(stripe), Edge(BoxSide::kRight).GetDoubleBorderStripeWidth(stripe),
            Edge(BoxSide::kBottom).GetDoubleBorderStripeWidth(stripe), Edge(BoxSide::kLeft).GetDoubleBorderStripeWidth(stripe));
}

PhysicalBoxStrut BoxBorderPainter::CenterOutsets() const
{
    return outer_outsets_
        - PhysicalBoxStrut(Edge(BoxSide::kTop).UsedWidth() * 0.5, Edge(BoxSide::kRight).UsedWidth() * 0.5, Edge(BoxSide::kBottom).UsedWidth() * 0.5,
...
void BoxBorderPainter::DrawBoxSide(
    GraphicsContext& context, const gfx::Rect& snapped_edge_rect, BoxSide side, Color color, EBorderStyle style, const AutoDarkMode& auto_dark_mode)
{
    DrawLineForBoxSide(context, snapped_edge_rect.x(), snapped_edge_rect.y(), snapped_edge_rect.right(), snapped_edge_rect.bottom(), side, color, style, 0, 0,
        true, auto_dark_mode);
}
