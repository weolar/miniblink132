...
class CORE_EXPORT ObjectPaintProperties : public GarbageCollected<ObjectPaintProperties> {
public:
...
public:                                                                                                                                                        \
    ...
    {                                                                                                                                                          \
        return GetNode<type##PaintPropertyNode>(field_id);                                                                                                     \
                                                                                                                                                               ...
                                                                                                                                                               \
    PaintPropertyChangeType Update##function(const type##PaintPropertyNodeOrAlias& parent, type##PaintPropertyNode::State&& state,                             \
        const type##PaintPropertyNode::AnimationState& animation_state = type##PaintPropertyNode::AnimationState())                                            \
    {                                                                                                                                                          \
        return Update<type##PaintPropertyNode, type##PaintPropertyNodeOrAlias>(field_id, parent, std::move(state), animation_state);                           \
    }                                                                                                                                                          \
                                                                                                                                                               \
    bool Clear##function()                                                                                                                                     \
    {                                                                                                                                                          \
        return nodes_.EraseField(field_id);                                                                                                                    \
    }                                                                                                                                                          \
...
public:                                                                                                                                                        \
    ...
    {                                                                                                                                                          \
        return GetNode<type##PaintPropertyNodeAlias>(field_id);                                                                                                \
                                                                                                                                                               ...
                                                                                                                                                               \
    PaintPropertyChangeType Update##function(const type##PaintPropertyNodeOrAlias& parent)                                                                     \
    {                                                                                                                                                          \
...
#define ADD_TRANSFORM(function, field_id) ADD_NODE(Transform, function, field_id)
...
#define ADD_CLIP(function, field_id) ADD_NODE(Clip, function, field_id)
    ...
    std::array<const TransformPaintPropertyNode*, 5> AllCSSTransformPropertiesOutsideToInside() const
    {
        return { Translate(), Rotate(), Scale(), Offset(), Transform() };
    }
    ...
    ADD_TRANSFORM(AnchorPositionScrollTranslation, NodeId::kAnchorPositionScrollTranslation)
    ...
    ADD_EFFECT(ViewTransitionSubframeRootEffect, NodeId::kViewTransitionSubframeRootEffect)
    ...
    ADD_CLIP(PixelMovingFilterClipExpander, NodeId::kPixelMovingFilterClipExpander)
...
// change.
    ...
    void Validate()
    {
        DCHECK(!ScrollTranslation() || !ReplacedContentTransform()) << "Replaced elements don't scroll so there should never be both a "
                                                                       "scroll translation and a replaced content transform.";
        DCHECK(!ClipPathClip() || !ClipPathMask()) << "ClipPathClip and ClipPathshould be mutually exclusive.";
        DCHECK((!TransformIsolationNode() && !ClipIsolationNode() && !EffectIsolationNode())
            || (TransformIsolationNode() && ClipIsolationNode() && EffectIsolationNode()))
    ...
    PaintPropertyChangeType DirectlyUpdateTransformAndOrigin(
        TransformPaintPropertyNode::TransformAndOrigin&& transform_and_origin, const TransformPaintPropertyNode::AnimationState& animation_state)
    {
        CHECK(nodes_.HasField(NodeId::kTransform));
        return GetNode<TransformPaintPropertyNode>(NodeId::kTransform)->DirectlyUpdateTransformAndOrigin(std::move(transform_and_origin), animation_state);
    }
    PaintPropertyChangeType DirectlyUpdateOpacity(float opacity, const EffectPaintPropertyNode::AnimationState& animation_state)
    {
        ...
        return GetNode<EffectPaintPropertyNode>(NodeId::kEffect)->DirectlyUpdateOpacity(opacity, animation_state);
    }
...
private:
    ...
    PaintPropertyChangeType Update(NodeId node_id, const ParentType& parent, NodeType::State&& state, const NodeType::AnimationState& animation_state)
    {
        ...
        const PaintPropertyChangeType changed = node->Update(parent, std::move(state), animation_state);
#if DCHECK_IS_ON()
        DCHECK(!is_immutable_ || changed == PaintPropertyChangeType::kUnchanged) << "Value changed while immutable.";
#endif
        return changed;
    }

    template <typename AliasType, typename ParentType> PaintPropertyChangeType UpdateAlias(NodeId node_id, const ParentType& parent)
    {
...
#if DCHECK_IS_ON()
        DCHECK(!is_immutable_ || changed == PaintPropertyChangeType::kUnchanged) << "Parent changed while immutable. New state:\n" << *node;
#endif
        return changed;
    }

    template <typename NodeType> const NodeType* GetNode(NodeId node_id) const
    {
        if (nodes_.HasField(node_id)) {
    ...
    template <typename NodeType> NodeType* GetNode(NodeId node_id)
    {
        if (nodes_.HasField(node_id)) {
    ...
    {
        for (NodeId i = first_id; i <= last_id; i = static_cast<NodeId>(static_cast<int>(i) + 1)) {
...
#if DCHECK_IS_ON()
    void AddNodesToPrinter(NodeId first_id, NodeId last_id, PropertyTreePrinter& printer) const
    {
        for (NodeId i = first_id; i <= last_id; i = static_cast<NodeId>(static_cast<int>(i) + 1)) {
            if (nodes_.HasField(i)) {
