...
void PrePaintTreeWalk::WalkTree(LocalFrameView& root_frame_view)
{
    ...
    DCHECK_EQ(root_frame_view.GetFrame().GetDocument()->Lifecycle().GetState(), DocumentLifecycle::kInPrePaint);

    PrePaintTreeWalkContext context;

#if DCHECK_IS_ON()
    bool needed_tree_builder_context_update = NeedsTreeBuilderContextUpdate(root_frame_view, context);
#endif

    VisualViewport& visual_viewport = root_frame_view.GetPage()->GetVisualViewport();
    if (visual_viewport.IsActiveViewport() && root_frame_view.GetFrame().IsMainFrame()) {
        VisualViewportPaintPropertyTreeBuilder::Update(root_frame_view, visual_viewport, *context.tree_builder_context);
    }
...
#if DCHECK_IS_ON()
    if ((needed_tree_builder_context_update || updates_executed) && VLOG_IS_ON(1)) {
    ...
    if (needs_invalidate_chrome_client_and_intersection_) {
        ...
        // walk or earlier.
        root_frame_view.SetIntersectionObservationState(LocalFrameView::kScrollAndVisibilityOnly);
    }
}

void PrePaintTreeWalk::Walk(LocalFrameView& frame_view, const PrePaintTreeWalkContext& parent_context)
{
    bool needs_tree_builder_context_update = NeedsTreeBuilderContextUpdate(frame_view, parent_context);
    ...
    if (frame_view.ShouldThrottleRendering()) {
        ...
        if (LayoutView* layout_view = frame_view.GetLayoutView()) {
            if (needs_tree_builder_context_update) {
                layout_view->AddSubtreePaintPropertyUpdateReason(SubtreePaintPropertyUpdateReason::kPreviouslySkipped);
            }
    ...
    if (context.tree_builder_context) {
        PaintPropertyTreeBuilder::SetupContextForFrame(frame_view, *context.tree_builder_context);
    }
...
bool HasBlockingEventHandlerHelper(const LocalFrame& frame, EventTarget& target, BlockingEventHandlerType event_type)
{
    if (!target.HasEventListeners())
        return false;
    const auto& registry = frame.GetEventHandlerRegistry();
    if (BlockingEventHandlerType::kTouchStartOrMoveBlockingEventHandler == event_type) {
        const auto* blocking = registry.EventHandlerTargets(EventHandlerRegistry::kTouchStartOrMoveEventBlocking);
    ...
    } else if (BlockingEventHandlerType::kWheelBlockingEventHandler == event_type) {
        const auto* blocking = registry.EventHandlerTargets(EventHandlerRegistry::kWheelEventBlocking);
        return blocking->Contains(&target);
    }
    NOTREACHED();
}

bool HasBlockingEventHandlerHelper(const LayoutObject& object, BlockingEventHandlerType event_type)
{
...
{
    return HasBlockingEventHandlerHelper(object, BlockingEventHandlerType::kTouchStartOrMoveBlockingEventHandler);
}

bool HasBlockingWheelEventHandler(const LayoutObject& object)
{
    return HasBlockingEventHandlerHelper(object, BlockingEventHandlerType::kWheelBlockingEventHandler);
}
} // namespace

void PrePaintTreeWalk::UpdateEffectiveAllowedTouchAction(const LayoutObject& object, PrePaintTreeWalk::PrePaintTreeWalkContext& context)
{
    ...
    if (context.effective_allowed_touch_action_changed) {
        object.GetMutableForPainting().UpdateInsideBlockingTouchEventHandler(
            context.inside_blocking_touch_event_handler || HasBlockingTouchEventHandler(object));
    }
...
void PrePaintTreeWalk::UpdateBlockingWheelEventHandler(const LayoutObject& object, PrePaintTreeWalk::PrePaintTreeWalkContext& context)
{
    ...
    if (context.blocking_wheel_event_handler_changed) {
        object.GetMutableForPainting().UpdateInsideBlockingWheelEventHandler(
            context.inside_blocking_wheel_event_handler || HasBlockingWheelEventHandler(object));
    }
...
void PrePaintTreeWalk::InvalidatePaintForHitTesting(const LayoutObject& object, PrePaintTreeWalk::PrePaintTreeWalkContext& context)
{
    if (context.paint_invalidator_context.subtree_flags & PaintInvalidatorContext::kSubtreeNoInvalidation)
        return;

    if (!context.effective_allowed_touch_action_changed && !context.blocking_wheel_event_handler_changed && !object.ShouldInvalidatePaintForHitTestOnly()) {
        return;
    }
    ...
    if (!RuntimeEnabledFeatures::HitTestOpaquenessEnabled()) {
        ObjectPaintInvalidator(object).InvalidateDisplayItemClient(object, PaintInvalidationReason::kHitTest);
    }
}

bool PrePaintTreeWalk::NeedsTreeBuilderContextUpdate(const LocalFrameView& frame_view, const PrePaintTreeWalkContext& context)
{
    if (frame_view.GetFrame().IsMainFrame() && frame_view.GetPage()->GetVisualViewport().IsActiveViewport()
        && frame_view.GetPage()->GetVisualViewport().NeedsPaintPropertyUpdate()) {
        return true;
    }

    return frame_view.GetLayoutView() && NeedsTreeBuilderContextUpdate(*frame_view.GetLayoutView(), context);
}

bool PrePaintTreeWalk::NeedsTreeBuilderContextUpdate(const LayoutObject& object, const PrePaintTreeWalkContext& parent_context)
{
    return ContextRequiresChildTreeBuilderContext(parent_context) || ObjectRequiresTreeBuilderContext(object);
}

bool PrePaintTreeWalk::ObjectRequiresPrePaint(const LayoutObject& object)
{
    return object.ShouldCheckForPaintInvalidation() || object.EffectiveAllowedTouchActionChanged() || object.DescendantEffectiveAllowedTouchActionChanged()
        || object.BlockingWheelEventHandlerChanged() || object.DescendantBlockingWheelEventHandlerChanged();
}

bool PrePaintTreeWalk::ContextRequiresChildPrePaint(const PrePaintTreeWalkContext& context)
{
    return context.paint_invalidator_context.NeedsSubtreeWalk() || context.effective_allowed_touch_action_changed
        || context.blocking_wheel_event_handler_changed;
}

bool PrePaintTreeWalk::ObjectRequiresTreeBuilderContext(const LayoutObject& object)
{
    return object.NeedsPaintPropertyUpdate() || object.ShouldCheckLayoutForPaintInvalidation()
        || (!object.ChildPrePaintBlockedByDisplayLock()
            && (object.DescendantNeedsPaintPropertyUpdate() || object.DescendantShouldCheckLayoutForPaintInvalidation()));
}

bool PrePaintTreeWalk::ContextRequiresChildTreeBuilderContext(const PrePaintTreeWalkContext& context)
{
    if (!context.NeedsTreeBuilderContext()) {
        DCHECK(!context.tree_builder_context || !context.tree_builder_context->force_subtree_update_reasons);
...
#if DCHECK_IS_ON()
void PrePaintTreeWalk::CheckTreeBuilderContextState(const LayoutObject& object, const PrePaintTreeWalkContext& parent_context)
{
    if (parent_context.tree_builder_context || (!ObjectRequiresTreeBuilderContext(object) && !ContextRequiresChildTreeBuilderContext(parent_context))) {
        return;
    }
...
PrePaintInfo PrePaintTreeWalk::CreatePrePaintInfo(const PhysicalFragmentLink& child, const PrePaintTreeWalkContext& context)
{
    const auto* fragment = To<PhysicalBoxFragment>(child.fragment.Get());
    return PrePaintInfo(fragment, child.offset, context.current_container.fragmentainer_idx, fragment->IsFirstForNode(), !fragment->GetBreakToken(),
        /* is_inside_fragment_child */ false, context.current_container.IsInFragmentationContext());
}

FragmentData* PrePaintTreeWalk::GetOrCreateFragmentData(const LayoutObject& object, const PrePaintTreeWalkContext& context, const PrePaintInfo& pre_paint_info)
{
    ...
    FragmentDataList& fragment_list = object.GetMutableForPainting().FragmentList();
    ...
    if (pre_paint_info.is_first_for_node) {
        ...
        if (pre_paint_info.is_inside_fragment_child) {
            ...
            InlineCursor cursor(*To<LayoutBlockFlow>(parent_fragment.GetLayoutObject()));
            cursor.MoveToIncludingCulledInline(object);
            DCHECK_GE(BoxFragmentIndex(parent_fragment), cursor.ContainerFragmentIndex());
            wtf_size_t parent_fragment_idx = BoxFragmentIndex(parent_fragment);

            const auto& container = *To<LayoutBlockFlow>(parent_fragment.GetLayoutObject());
            if (container.MayBeNonContiguousIfc()) {
                ...
                fragment_data_idx = parent_fragment_idx - cursor.ContainerFragmentIndex();
            }
        } else {
...
void PrePaintTreeWalk::UpdateContextForOOFContainer(const LayoutObject& object, PrePaintTreeWalkContext& context, const PhysicalBoxFragment* fragment)
{
    ...
    if (!context.current_container.IsInFragmentationContext() || (fragment && fragment->IsMonolithic())) {
...
void PrePaintTreeWalk::WalkInternal(const LayoutObject& object, PrePaintTreeWalkContext& context, PrePaintInfo* pre_paint_info)
{
    PaintInvalidatorContext& paint_invalidator_context = context.paint_invalidator_context;
    ...
    if (pre_paint_info) {
        ...
        pre_paint_info->fragment_data = GetOrCreateFragmentData(object, context, *pre_paint_info);
    ...
    if (context.tree_builder_context) {
        property_tree_builder.emplace(object, pre_paint_info, *context.tree_builder_context);
    ...
    if (paint_invalidator_.InvalidatePaint(object, pre_paint_info, base::OptionalToPtr(context.tree_builder_context), paint_invalidator_context)) {
        needs_invalidate_chrome_client_and_intersection_ = true;
    }
    ...
    if (context.tree_builder_context) {
        ...
        needs_invalidate_chrome_client_and_intersection_ |= property_tree_builder->PropertiesChanged();
    }
}

bool PrePaintTreeWalk::CollectMissableChildren(PrePaintTreeWalkContext& context, const PhysicalBoxFragment& parent)
{
    ...
    for (const PhysicalFragmentLink& child : parent.Children()) {
        ...
        if (child->IsOutOfFlowPositioned() && (context.current_container.fragment || child->IsFixedPositioned())) {
...
const PhysicalBoxFragment* PrePaintTreeWalk::RebuildContextForMissedDescendant(
    const PhysicalBoxFragment& ancestor, const LayoutObject& object, bool update_tree_builder_context, PrePaintTreeWalkContext& context)
{
    ...
    const PhysicalBoxFragment* search_fragment = RebuildContextForMissedDescendant(ancestor, *object.Parent(), update_tree_builder_context, context);
    ...
    if (context.tree_builder_context && update_tree_builder_context) {
        ...
        if (object.IsOutOfFlowPositioned()) {
            ...
            const ContainingFragment& oof_containing_fragment_info
                = object.IsFixedPositioned() ? context.fixed_positioned_container : context.absolute_positioned_container;
        ...
        PrePaintInfo pre_paint_info(box_fragment, paint_offset, fragmentainer_idx,
        ...
        PaintPropertyTreeBuilderContext& builder_context = context.tree_builder_context.value();
        ...
        builder_context.force_subtree_update_reasons |= PaintPropertyTreeBuilderContext::kSubtreeUpdateIsolationPiercing;

        PaintPropertyTreeBuilder property_tree_builder(object, &pre_paint_info, builder_context);
        property_tree_builder.UpdateForSelf();
        property_tree_builder.UpdateForChildren();
    ...
    if (!object.CanContainAbsolutePositionObjects() || !context.tree_builder_context) {
        return box_fragment;
    }

    PaintPropertyTreeBuilderContext& property_context = *context.tree_builder_context;
    PaintPropertyTreeBuilderFragmentContext& fragment_context = property_context.fragment_context;
...
void PrePaintTreeWalk::WalkMissedChildren(const PhysicalBoxFragment& fragment, bool is_in_fragment_traversal, const PrePaintTreeWalkContext& context)
{
    ...
    if (fragment.IsFragmentainerBox() && fragment.Style().IsFlippedBlocksWritingMode()) {
    ...
    for (const PhysicalFragmentLink& child : fragment.Children()) {
        ...
        PrePaintTreeWalkContext descendant_context(context, NeedsTreeBuilderContextUpdate(descendant_object, context));
        if (child->IsOutOfFlowPositioned()) {
            if (descendant_context.tree_builder_context.has_value()) {
                PaintPropertyTreeBuilderContext* builder_context = &descendant_context.tree_builder_context.value();
                builder_context->fragment_context.current.paint_offset += offset_to_block_start_edge;
            }

            bool update_tree_builder_context = NeedsTreeBuilderContextUpdate(descendant_object, descendant_context);

            RebuildContextForMissedDescendant(fragment, *descendant_object.Parent(), update_tree_builder_context, descendant_context);
        }

        if (is_in_fragment_traversal) {
            PrePaintInfo pre_paint_info = CreatePrePaintInfo(child, descendant_context);
...
LocalFrameView* FindWebViewPluginContentFrameView(const LayoutEmbeddedContent& embedded_content)
{
    for (Frame* frame = embedded_content.GetFrame()->Tree().FirstChild(); frame; frame = frame->Tree().NextSibling()) {
        if (frame->IsLocalFrame() && To<LocalFrame>(frame)->OwnerLayoutObject() == &embedded_content)
...
void PrePaintTreeWalk::WalkFragmentationContextRootChildren(
    const LayoutObject& object, const PhysicalBoxFragment& fragment, const PrePaintTreeWalkContext& parent_context)
{
    ...
    for (const LayoutObject* child = object.SlowFirstChild(); child; child = child->NextSibling()) {
        DCHECK(child->IsLayoutFlowThread() || child->IsLayoutMultiColumnSet() || child->IsLayoutMultiColumnSpannerPlaceholder());
        child->GetMutableForPainting().ClearPaintFlags();
    }
}

void PrePaintTreeWalk::WalkPageContainer(const PhysicalFragmentLink& page_container_link, const LayoutObject& parent_object,
    const PrePaintTreeWalkContext& parent_context, wtf_size_t fragmentainer_idx)
{
    // In paginated layout, each fragmentainer (page area) is wrapped inside a
    // page box and a page border box.
    DCHECK_EQ(page_container_link->GetBoxType(), PhysicalFragment::kPageContainer);
    const auto& page_container = To<PhysicalBoxFragment>(*page_container_link.get());

    PrePaintTreeWalkContext page_container_context(parent_context, parent_context.NeedsTreeBuilderContext());
    PrePaintInfo container_pre_paint_info = CreatePrePaintInfo(page_container_link, page_container_context);
    WalkInternal(*page_container_link->GetLayoutObject(), page_container_context, &container_pre_paint_info);
    ...
    PhysicalOffset pagination_adjustment = StitchedPageContentRect(page_container).offset;
    ...
    for (const PhysicalFragmentLink& grandchild : page_container.Children()) {
        if (grandchild->GetBoxType() == PhysicalFragment::kPageMargin) {
            ...
            // headers or footers.
            PrePaintTreeWalkContext margin_box_context(parent_context, parent_context.NeedsTreeBuilderContext());
            PrePaintInfo margin_pre_paint_info = CreatePrePaintInfo(grandchild, margin_box_context);
            Walk(*grandchild->GetLayoutObject(), margin_box_context, &margin_pre_paint_info);
            continue;
        }
        ...
        // contents).
        PrePaintTreeWalkContext page_border_box_context(page_container_context, page_container_context.NeedsTreeBuilderContext());
        if (page_border_box_context.tree_builder_context) {
            PrePaintInfo border_box_pre_paint_info = CreatePrePaintInfo(grandchild, page_border_box_context);
            PaintPropertyTreeBuilder builder(*grandchild->GetLayoutObject(), &border_box_pre_paint_info, page_border_box_context.tree_builder_context.value());
        ...
        PrePaintTreeWalkContext page_area_context(parent_context, parent_context.NeedsTreeBuilderContext());
        PaintPropertyTreeBuilderFragmentContext::ContainingBlockContext* containing_block_context = nullptr;

        if (page_area_context.tree_builder_context) {
            PaintPropertyTreeBuilderFragmentContext& fragment_context = page_area_context.tree_builder_context->fragment_context;
            containing_block_context = &fragment_context.current;
            containing_block_context->paint_offset += pagination_adjustment;

            PaginationState* pagination_state = parent_object.GetFrameView()->GetPaginationState();
            ObjectPaintProperties& pagination_paint_properties
                = pagination_state->EnsureContentAreaProperties(*containing_block_context->transform, *containing_block_context->clip);
            ...
            // is painted.
            containing_block_context->transform = pagination_paint_properties.Transform();
            containing_block_context->clip = pagination_paint_properties.OverflowClip();
        }

        WalkFragmentainer(parent_object, page_area, page_area_context, fragmentainer_idx);
...
void PrePaintTreeWalk::WalkFragmentainer(
    const LayoutObject& parent_object, const PhysicalFragmentLink& child_link, const PrePaintTreeWalkContext& parent_context, wtf_size_t fragmentainer_idx)
{
    DCHECK(child_link->IsFragmentainerBox());
    const auto& fragmentainer = To<PhysicalBoxFragment>(*child_link.get());

    PrePaintTreeWalkContext fragmentainer_context(parent_context, parent_context.NeedsTreeBuilderContext());

    fragmentainer_context.current_container.fragmentation_nesting_level++;
    fragmentainer_context.is_parent_first_for_node = fragmentainer.IsFirstForNode();
    ...
    if (fragmentainer_context.tree_builder_context) {
        PaintPropertyTreeBuilderFragmentContext& fragment_context = fragmentainer_context.tree_builder_context->fragment_context;
        containing_block_context = &fragment_context.current;
        ...
        containing_block_context->paint_offset_for_oof_in_fragmentainer = containing_block_context->paint_offset;
    ...
    const auto* flow_thread = To<LayoutBlockFlow>(&parent_object)->MultiColumnFlowThread();
...
void PrePaintTreeWalk::WalkLayoutObjectChildren(
    const LayoutObject& parent_object, const PhysicalBoxFragment* parent_fragment, const PrePaintTreeWalkContext& context)
{
    ...
    for (const LayoutObject* child = parent_object.SlowFirstChild(); child;
        ...
        if (!inline_cursor && parent_fragment->HasItems() && child->HasInlineFragments()) {
        ...
        if (inline_cursor) {
            for (; inline_cursor->Current(); inline_cursor->MoveToNextForSameLayoutObject()) {
            ...
            if (!layout_inline_child) {
                ...
                for (const LayoutObject* fragmentless = child; fragmentless; fragmentless = fragmentless->NextInPreOrder(child)) {
            ...
            if (layout_inline_child->FirstChild()) {
                ...
                if (!parent_fragment->Items()->IsContainerForCulledInline(*layout_inline_child, &is_first_for_node, &is_last_for_node, &child_has_any_items)) {
            ...
            if (child_box->IsOutOfFlowPositioned()) {
                oof_containing_fragment_info = child_box->IsFixedPositioned() ? &context.fixed_positioned_container : &context.absolute_positioned_container;
                if (context.current_container.fragmentation_nesting_level != oof_containing_fragment_info->fragmentation_nesting_level) {
        ...
        if (box_fragment) {
            const ContainingFragment* container_for_child = &context.current_container;
            bool is_in_different_fragmentation_context = false;
            if (oof_containing_fragment_info
                && context.current_container.fragmentation_nesting_level != oof_containing_fragment_info->fragmentation_nesting_level) {
            ...
            PrePaintInfo pre_paint_info(box_fragment, paint_offset, fragmentainer_idx, is_first_for_node, is_last_for_node, is_inside_fragment_child,
                container_for_child->IsInFragmentationContext());
            if (is_in_different_fragmentation_context) {
                PrePaintTreeWalkContext oof_context(context, NeedsTreeBuilderContextUpdate(*child, context));
...
void PrePaintTreeWalk::WalkChildren(
    const LayoutObject& object, const PhysicalBoxFragment* traversable_fragment, PrePaintTreeWalkContext& context, bool is_inside_fragment_child)
{
    const LayoutBox* box = DynamicTo<LayoutBox>(&object);
    if (box) {
        if (traversable_fragment) {
            if (!box->IsLayoutFlowThread() && (!box->IsLayoutNGObject() || !box->PhysicalFragmentCount())) {
            ...
            const auto* first_fragment = To<PhysicalBoxFragment>(box->GetPhysicalFragment(0));
            DCHECK(!first_fragment->GetBreakToken());
            if (first_fragment->IsFragmentationContextRoot() && box->CanTraversePhysicalFragments())
    ...
    if (!fragment) {
        ...
        if (box && box->CanContainAbsolutePositionObjects() && box->IsLayoutNGObject() && box->PhysicalFragmentCount()) {
            DCHECK_EQ(box->PhysicalFragmentCount(), 1u);
    ...
    if (traversable_fragment && traversable_fragment->IsFragmentationContextRoot()) {
        WalkFragmentationContextRootChildren(object, *traversable_fragment, context);
    } else {
...
void PrePaintTreeWalk::Walk(const LayoutObject& object, const PrePaintTreeWalkContext& parent_context, PrePaintInfo* pre_paint_info)
{
    ...
    bool needs_tree_builder_context_update = NeedsTreeBuilderContextUpdate(object, parent_context);
    ...
    if (!needs_tree_builder_context_update && !ObjectRequiresPrePaint(object) && !ContextRequiresChildPrePaint(parent_context)) {
    ...
    if (child_walk_blocked && (ContextRequiresChildTreeBuilderContext(context) || ContextRequiresChildPrePaint(context))) {
        ...
        object.GetDisplayLockContext()->SetNeedsPrePaintSubtreeWalk(
            context.effective_allowed_touch_action_changed, context.blocking_wheel_event_handler_changed);
    }
    ...
    if (!child_walk_blocked) {
        ...
        if (const auto* layout_embedded_content = DynamicTo<LayoutEmbeddedContent>(object)) {
            if (auto* embedded_view = layout_embedded_content->GetEmbeddedContentView()) {
                ...
                if (!physical_fragment || physical_fragment->IsFirstForNode() || CanPaintMultipleFragments(*physical_fragment)) {
                    if (context.tree_builder_context) {
                        auto& current = context.tree_builder_context->fragment_context.current;
                        current.paint_offset = PhysicalOffset(ToRoundedPoint(current.paint_offset + layout_embedded_content->ReplacedContentRect().offset
                            - PhysicalOffset(embedded_view->FrameRect().origin())));
                        // Subpixel accumulation doesn't propagate across embedded view.
                        current.directly_composited_container_paint_offset_subpixel_delta = PhysicalOffset();
                    }
                    if (embedded_view->IsLocalFrameView()) {
                        ...
                        if (auto* plugin_content_frame_view = FindWebViewPluginContentFrameView(*layout_embedded_content)) {
