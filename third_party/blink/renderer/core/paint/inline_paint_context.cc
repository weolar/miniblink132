...
void InlinePaintContext::ClearDecoratingBoxes(DecoratingBoxList* saved_decorating_boxes)
{
...
InlinePaintContext::ScopedInlineItem::ScopedInlineItem(const FragmentItem& item, InlinePaintContext* inline_context)
{
    DCHECK(inline_context);
    inline_context_ = inline_context;
    last_decorations_ = inline_context->last_decorations_;
    push_count_ = inline_context->SyncDecoratingBox(item, &saved_decorating_boxes_);
    DCHECK_EQ(inline_context->decorating_boxes_.size(), item.Style().AppliedTextDecorations().size());
}
...
wtf_size_t InlinePaintContext::SyncDecoratingBox(const FragmentItem& item, DecoratingBoxList* saved_decorating_boxes)
{
    ...
    const Vector<AppliedTextDecoration, 1>* decorations = &style->AppliedTextDecorations();
    ...
    public:
        DecorationBoxSynchronizer(InlinePaintContext* inline_context, const FragmentItem& item, const Vector<AppliedTextDecoration, 1>* stop_at,
        ...
        wtf_size_t Sync(
            const FragmentItem* item, const LayoutObject* layout_object, const ComputedStyle* style, const Vector<AppliedTextDecoration, 1>* decorations)
        {
            for (;;) {
                ...
                DCHECK(parent);
                const ComputedStyle& parent_style = parent->EffectiveStyle(style_variant_);
                const Vector<AppliedTextDecoration, 1>& parent_decorations = parent_style.AppliedTextDecorations();
                ...
                if (decorations != &parent_decorations) {
                    ...
                    if (decorations->size() > parent_decorations.size()) {
                        ...
                        if (&parent_decorations != stop_at_) {
                            num_pushes = Sync(/* item */ nullptr, parent, &parent_style, &parent_decorations);
                        }

                        num_pushes += PushDecoratingBoxesUntilParent(item, *layout_object, *style, *decorations, parent_decorations);
                        return num_pushes;
                    }
                    ...
                    if (decorations->size() == parent_decorations.size()
                        ...
                        if (&parent_decorations == stop_at_)
                            return 0;
                        return Sync(/* item */ nullptr, parent, &parent_style, &parent_decorations);
                    }
                    ...
                    const Vector<AppliedTextDecoration, 1>* base_decorations = style->BaseAppliedTextDecorations();
                    if (base_decorations != &parent_decorations) {
                        inline_context_->ClearDecoratingBoxes(saved_decorating_boxes_);
                        const wtf_size_t size = std::min(saved_decorating_boxes_->size(), decorations->size());
                        inline_context_->PushDecoratingBoxes(base::span(*saved_decorating_boxes_).first(size));
                        return size;
                    }
...
#endif
                    NOTREACHED() << "size=" << decorations->size() << ", parent=" << parent_decorations.size()
                                 << ", TextDecorationLine=" << static_cast<int>(style->GetTextDecorationLine());
                }
                ...
                // shared, see above.
                if (!(parent_decorations.size() == parent->Parent()->StyleRef().AppliedTextDecorations().size()
                        && parent_style.GetTextDecorationLine() == TextDecorationLine::kNone)
                    && !IsA<LayoutText>(layout_object)) {
        ...
        wtf_size_t PushDecoratingBoxesUntilParent(const FragmentItem* item, const LayoutObject& layout_object, const ComputedStyle& style,
            const Vector<AppliedTextDecoration, 1>& decorations, const Vector<AppliedTextDecoration, 1>& parent_decorations)
        {
            const Vector<AppliedTextDecoration, 1>* base_decorations = style.BaseAppliedTextDecorations();
            if (base_decorations == &parent_decorations) {
            ...
            if (base_decorations && base_decorations != &decorations && decorations.size() == parent_decorations.size() + 2) {
            ...
            const FragmentItem* item, const LayoutObject& layout_object, const ComputedStyle& style, const Vector<AppliedTextDecoration, 1>& decorations)
        {
            ...
            DCHECK(item);
            inline_context_->PushDecoratingBox(item->ContentOffsetInContainerFragment(), style, &decorations);
        }
    ...
    const wtf_size_t push_count
        = DecorationBoxSynchronizer(this, item, last_decorations_, saved_decorating_boxes).Sync(&item, item.GetLayoutObject(), style, decorations);
    last_decorations_ = decorations;
    return push_count;
}

InlinePaintContext::ScopedInlineBoxAncestors::ScopedInlineBoxAncestors(const InlineCursor& inline_box, InlinePaintContext* inline_context)
{
    DCHECK(inline_context);
    inline_context_ = inline_context;
...
void InlinePaintContext::PushDecoratingBoxAncestors(const InlineCursor& inline_box)
{
...
void InlinePaintContext::PushDecoratingBoxes(const base::span<DecoratingBox>& boxes)
{
    decorating_boxes_.AppendRange(boxes.begin(), boxes.end());
}

InlinePaintContext::ScopedLineBox::ScopedLineBox(const InlineCursor& line_cursor, InlinePaintContext* inline_context)
{
    DCHECK(inline_context);
    inline_context_ = inline_context;
...
void InlinePaintContext::SetLineBox(const InlineCursor& line_cursor)
{
    ...
    const Vector<AppliedTextDecoration, 1>& applied_text_decorations = style.AppliedTextDecorations();
