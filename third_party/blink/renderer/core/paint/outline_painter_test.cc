...
TEST_F(OutlinePainterTest, FocusRingOutset)
{
    ...
    LayoutObject::OutlineInfo info = LayoutObject::OutlineInfo::GetFromStyle(*style);
...
TEST_F(OutlinePainterTest, HugeOutlineWidthOffset)
{
    ...
    GetLayoutObjectByElementId("target")->OutlineRects(&info, PhysicalOffset(), OutlineType::kDontIncludeBlockInkOverflow);
    const auto& style = GetLayoutObjectByElementId("target")->StyleRef();
    EXPECT_TRUE(style.HasOutline());
    EXPECT_EQ(LayoutUnit::Max().ToInt() * 2, OutlinePainter::OutlineOutsetExtent(style, info));
}
...
TEST_F(OutlinePainterTest, IterateCollapsedPath)
{
    ...
    OutlinePainter::IterateRightAnglePathForTesting(path, WTF::BindRepeating([](const Vector<OutlinePainter::Line>&) { NOTREACHED(); }));
}
