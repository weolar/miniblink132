...
struct CORE_EXPORT PaintLayerScrollableAreaRareData final : public GarbageCollected<PaintLayerScrollableAreaRareData> {
public:
    PaintLayerScrollableAreaRareData();
    PaintLayerScrollableAreaRareData(const PaintLayerScrollableAreaRareData&) = delete;
    PaintLayerScrollableAreaRareData& operator=(const PaintLayerScrollableAreaRareData&) = delete;
...
// scrolling contents.
class CORE_EXPORT PaintLayerScrollableArea final : public GarbageCollected<PaintLayerScrollableArea>, public ScrollableArea {
...
public:
    ...
    public:
        ...
        static bool ScrollbarsAreFrozen()
        {
            return count_;
        }
    ...
    public:
        ...
        FreezeScrollbarsRootScope(const LayoutBox&, bool freeze_horizontal, bool freeze_vertical);
    ...
    gfx::Rect ConvertFromScrollbarToContainingEmbeddedContentView(const Scrollbar&, const gfx::Rect&) const override;
    gfx::Point ConvertFromScrollbarToContainingEmbeddedContentView(const Scrollbar&, const gfx::Point&) const override;
    gfx::Point ConvertFromContainingEmbeddedContentViewToScrollbar(const Scrollbar&, const gfx::Point&) const override;
    gfx::Point ConvertFromRootFrame(const gfx::Point&) const override;
    gfx::Point ConvertFromRootFrameToVisualViewport(const gfx::Point&) const override;
    ...
    ScrollOffset ScrollPositionToOffset(const gfx::PointF& position) const override
    {
    ...
    PhysicalRect LayoutContentRect(IncludeScrollbarsInRect = kExcludeScrollbars) const;
    gfx::Rect VisibleContentRect(IncludeScrollbarsInRect = kExcludeScrollbars) const override;
    PhysicalRect VisibleScrollSnapportRect(IncludeScrollbarsInRect = kExcludeScrollbars) const override;
    ...
    {
        return scroll_origin_changed_;
    }

    void ScrollToAbsolutePosition(const gfx::PointF& position, mojom::blink::ScrollBehavior scroll_behavior = mojom::blink::ScrollBehavior::kInstant,
        mojom::blink::ScrollType scroll_type = mojom::blink::ScrollType::kProgrammatic)
    {
    ...
    void SetScrollOffsetUnconditionally(const ScrollOffset&, mojom::blink::ScrollType = mojom::blink::ScrollType::kProgrammatic);
    ...
    int VerticalScrollbarWidth(OverlayScrollbarClipBehavior = kIgnoreOverlayScrollbarSize) const override;
    int HorizontalScrollbarHeight(OverlayScrollbarClipBehavior = kIgnoreOverlayScrollbarSize) const override;

    void PositionOverflowControls();

    // Test if a pointer/touch position is in the resize control area.
    bool IsAbsolutePointInResizeControl(const gfx::Point& absolute_point, ResizerHitTestType) const;
    bool IsLocalPointInResizeControl(const gfx::Point& local_point, ResizerHitTestType) const;
    ...
    PhysicalRect ScrollIntoView(const PhysicalRect&, const PhysicalBoxStrut& scroll_margin, const mojom::blink::ScrollIntoViewParamsPtr&) override;
    ...
    LayoutCustomScrollbarPart* Resizer() const
    {
        return resizer_.Get();
    }
    ...
    ScrollAnchor* GetScrollAnchor() override
    {
        return &scroll_anchor_;
    }
    bool IsPaintLayerScrollableArea() const override
    {
        return true;
    }
    ...
    gfx::QuadF LocalToVisibleContentQuad(const gfx::QuadF&, const LayoutObject*, unsigned = 0) const final;
    ...
    bool NeedsScrollOffsetClamp() const
    {
        return needs_scroll_offset_clamp_;
    }
    void SetNeedsScrollOffsetClamp(bool val)
    {
        needs_scroll_offset_clamp_ = val;
    }
    ...
    {
        return needs_relayout_;
    }
    void SetNeedsRelayout(bool val)
    {
        needs_relayout_ = val;
    }
    ...
    int HypotheticalScrollbarThickness(ScrollbarOrientation, bool should_include_overlay_thickness = false) const;
    ...
    void DidScrollWithScrollbar(ScrollbarPart, ScrollbarOrientation, WebInputEvent::Type) override;
    ...
    gfx::Rect ScrollingBackgroundVisualRect(const PhysicalOffset& paint_offset) const;
    ...
    std::optional<gfx::PointF> GetSnapPositionAndSetTarget(const cc::SnapSelectionStrategy& strategy) override;
    // Functions related to firing scrollsnapchange events.
    void SetScrollsnapchangeTargetIds(std::optional<cc::TargetSnapAreaElementIds>) override;
    void UpdateSnappedTargetsAndEnqueueScrollSnapChange() override;

    // Functions related to firing scrollsnapchanging events.
    std::optional<cc::TargetSnapAreaElementIds> GetScrollsnapchangingTargetIds() const override;
    void SetScrollsnapchangingTargetIds(std::optional<cc::TargetSnapAreaElementIds>) override;
    void UpdateScrollSnapChangingTargetsAndEnqueueScrollSnapChanging(const cc::TargetSnapAreaElementIds& new_target_ids) override;
    const cc::SnapSelectionStrategy* GetImplSnapStrategy() const override;
    void SetImplSnapStrategy(std::unique_ptr<cc::SnapSelectionStrategy> strategy) override;
    void EnqueueScrollSnapChangingEventFromImplIfNeeded() override;

    // Functions related to scroll-state(snapped) queries.
    void SetSnappedQueryTargetIds(std::optional<cc::TargetSnapAreaElementIds>) override;

    void DisposeImpl() override;

    void SetPendingHistoryRestoreScrollOffset(
        const HistoryItem::ViewState& view_state, bool should_restore_scroll, mojom::blink::ScrollBehavior scroll_behavior) override
    {
...
private:
    ...
    int ComputeHypotheticalScrollbarThickness(ScrollbarOrientation, bool should_include_overlay_thickness) const;
    ...
    void UpdateScrollbarEnabledState(bool is_horizontal_scrollbar_frozen = false, bool is_vertical_scrollbar_frozen = false);

    // Update the proportions used for thumb rect dimensions.
    void UpdateScrollbarProportions();

    void UpdateScrollOffset(const ScrollOffset&, mojom::blink::ScrollType) override;
    ...
    enum ComputeScrollbarExistenceOption { kDependsOnOverflow, kOverflowIndependent };
    void ComputeScrollbarExistence(
        bool& needs_horizontal_scrollbar, bool& needs_vertical_scrollbar, ComputeScrollbarExistenceOption = kDependsOnOverflow) const;
    ...
    bool TryRemovingAutoScrollbars(const bool& needs_horizontal_scrollbar, const bool& needs_vertical_scrollbar);
    ...
    void InvalidatePaintOfScrollbarIfNeeded(const PaintInvalidatorContext&, bool needs_paint_invalidation, Scrollbar* scrollbar, bool& previously_was_overlay,
        bool& previously_might_be_composited, gfx::Rect& visual_rect);
    ...
    Node* GetSnapTargetAlongAxis(cc::TargetSnapAreaElementIds, cc::SnapAxis) const;

    // Get the current target for a snap event of |type| (either
    // "scrollsnapchange" or scrollsnapchanging) along axis |axis|.
    Node* GetSnapEventTargetAlongAxis(const AtomicString& type, cc::SnapAxis) const override;
    ...
    void CreateAndSetSnappedQueryScrollSnapshotIfNeeded(cc::TargetSnapAreaElementIds);
    ...
    class ScrollingBackgroundDisplayItemClient final : public GarbageCollected<ScrollingBackgroundDisplayItemClient>, public DisplayItemClient {
    public:
        explicit ScrollingBackgroundDisplayItemClient(const PaintLayerScrollableArea& scrollable_area)
            : scrollable_area_(&scrollable_area)
        {
        }
    ...
    class ScrollCornerDisplayItemClient final : public GarbageCollected<ScrollCornerDisplayItemClient>, public DisplayItemClient {
    public:
        explicit ScrollCornerDisplayItemClient(const PaintLayerScrollableArea& scrollable_area)
            : scrollable_area_(&scrollable_area)
        {
        }
    ...
    Member<ScrollingBackgroundDisplayItemClient> scrolling_background_display_item_client_ = MakeGarbageCollected<ScrollingBackgroundDisplayItemClient>(*this);
    Member<ScrollCornerDisplayItemClient> scroll_corner_display_item_client_ = MakeGarbageCollected<ScrollCornerDisplayItemClient>(*this);

    struct PendingViewState {
        HistoryItem::ViewState state;
        mojom::blink::ScrollBehavior scroll_behavior = mojom::blink::ScrollBehavior::kAuto;
    };
