...
static bool ShouldFullyInvalidateFillLayersOnWidthChange(const FillLayer& layer)
{
    ...
    if (layer.Repeat().x != EFillRepeat::kRepeatFill && layer.Repeat().x != EFillRepeat::kNoRepeatFill) {
        return true;
    }
    ...
    if (size_type == EFillSizeType::kContain || size_type == EFillSizeType::kCover)
        return true;
...
static bool ShouldFullyInvalidateFillLayersOnHeightChange(const FillLayer& layer)
{
    // Nobody will use multiple layers without wanting fancy positioning.
    if (layer.Next())
        return true;
    ...
    if (layer.Repeat().y != EFillRepeat::kRepeatFill && layer.Repeat().y != EFillRepeat::kNoRepeatFill) {
        return true;
    }
...
bool ShouldFullyInvalidateFillLayersOnSizeChange(const FillLayer& layer, const PhysicalSize& old_size, const PhysicalSize& new_size)
{
    return (old_size.width != new_size.width && ShouldFullyInvalidateFillLayersOnWidthChange(layer))
        || (old_size.height != new_size.height && ShouldFullyInvalidateFillLayersOnHeightChange(layer));
}

PaintInvalidationReason BoxPaintInvalidator::ComputePaintInvalidationReason()
{
    PaintInvalidationReason reason = ObjectPaintInvalidatorWithContext(box_, context_).ComputePaintInvalidationReason();
    ...
    if (IsFullPaintInvalidationReason(reason) && !box_.ShouldCheckLayoutForPaintInvalidation()) {
        return reason;
    }

    const ComputedStyle& style = box_.StyleRef();

    if (style.MaskLayers().AnyLayerUsesContentBox() && box_.PreviousPhysicalContentBoxRect() != box_.PhysicalContentBoxRect())
        return PaintInvalidationReason::kLayout;

    if (const auto* layout_replaced = DynamicTo<LayoutReplaced>(box_)) {
        if (layout_replaced->ReplacedContentRect() != layout_replaced->ReplacedContentRectFrom(box_.PreviousPhysicalContentBoxRect())) {
            return PaintInvalidationReason::kLayout;
        }
    }
...
#endif
    if (box_.PreviousSize() == box_.Size() && box_.PreviousSelfVisualOverflowRect() == box_.SelfVisualOverflowRect()) {
        return IsFullPaintInvalidationReason(reason) ? reason : PaintInvalidationReason::kNone;
    }

    // Incremental invalidation is not applicable if there is visual overflow.
    if (box_.PreviousSelfVisualOverflowRect().size != box_.PreviousSize() || box_.SelfVisualOverflowRect().size != box_.Size()) {
        return PaintInvalidationReason::kLayout;
    }

    // Incremental invalidation is not applicable if paint offset or size has
    // fraction.
    if (context_.old_paint_offset.HasFraction() || context_.fragment_data->PaintOffset().HasFraction() || box_.PreviousSize().HasFraction()
    ...
    if (style.HasVisualOverflowingEffect() || style.HasEffectiveAppearance() || style.HasFilterInducingProperty() || style.HasMask() || style.HasClipPath())
        return PaintInvalidationReason::kLayout;
...
{
    return HasEffectiveBackground() && box_.StyleRef().BackgroundLayers().AnyLayerHasLocalAttachmentImage();
}
...
bool BoxPaintInvalidator::ShouldFullyInvalidateBackgroundOnScrollableOverflowChange(
    const PhysicalRect& old_scrollable_overflow, const PhysicalRect& new_scrollable_overflow)
{
    ...
    return ShouldFullyInvalidateFillLayersOnSizeChange(box_.StyleRef().BackgroundLayers(), old_scrollable_overflow.size, new_scrollable_overflow.size);
}

BoxPaintInvalidator::BackgroundInvalidationType BoxPaintInvalidator::ComputeViewBackgroundInvalidation()
{
    ...
    bool background_location_changed = new_background_rect.offset != old_background_rect.offset;
    bool background_size_changed = new_background_rect.size != old_background_rect.size;
    if (background_location_changed || background_size_changed) {
        for (const auto& object : layout_view.GetFrameView()->BackgroundAttachmentFixedObjects())
            object->SetBackgroundNeedsFullPaintInvalidation();
    }

    if (background_location_changed || layout_view.BackgroundNeedsFullPaintInvalidation()
        || (context_.subtree_flags & PaintInvalidatorContext::kSubtreeFullInvalidation)) {
    ...
    if (Element* root_element = box_.GetDocument().documentElement()) {
        if (const auto* root_object = root_element->GetLayoutObject()) {
            if (root_object->IsBox()) {
                ...
                if (ShouldFullyInvalidateFillLayersOnSizeChange(background_layers, root_box->PreviousSize(), root_box->Size())) {
                    return BackgroundInvalidationType::kFull;
                }
                if (BackgroundGeometryDependsOnScrollableOverflowRect()
                    && ShouldFullyInvalidateBackgroundOnScrollableOverflowChange(
                        root_box->PreviousScrollableOverflowRect(), root_box->ScrollableOverflowRect())) {
                    return BackgroundInvalidationType::kFull;
                }
                ...
                if (background_layers.AnyLayerUsesContentBox() && root_box->PreviousPhysicalContentBoxRect() != root_box->PhysicalContentBoxRect()) {
                    return BackgroundInvalidationType::kFull;
                }
            }
    ...
    return background_size_changed ? BackgroundInvalidationType::kIncremental : BackgroundInvalidationType::kNone;
}

BoxPaintInvalidator::BackgroundInvalidationType BoxPaintInvalidator::ComputeBackgroundInvalidation(bool& should_invalidate_all_layers)
{
    ...
    if (box_.BackgroundNeedsFullPaintInvalidation() || (context_.subtree_flags & PaintInvalidatorContext::kSubtreeFullInvalidation)) {
    ...
    if (background_layers.AnyLayerHasDefaultAttachmentImage()
        && ShouldFullyInvalidateFillLayersOnSizeChange(background_layers, box_.PreviousSize(), box_.Size())) {
        return BackgroundInvalidationType::kFull;
    }

    if (background_layers.AnyLayerUsesContentBox() && box_.PreviousPhysicalContentBoxRect() != box_.PhysicalContentBoxRect())
        return BackgroundInvalidationType::kFull;

    bool scrollable_overflow_change_causes_invalidation = (BackgroundGeometryDependsOnScrollableOverflowRect() || BackgroundPaintsInContentsSpace());
    ...
    if (ShouldFullyInvalidateBackgroundOnScrollableOverflowChange(old_scrollable_overflow, new_scrollable_overflow)) {
        return BackgroundInvalidationType::kFull;
    }
...
void BoxPaintInvalidator::InvalidateBackground()
{
    ...
    auto background_invalidation_type = ComputeBackgroundInvalidation(should_invalidate_in_both_spaces);
    if (IsA<LayoutView>(box_)) {
        background_invalidation_type = std::max(background_invalidation_type, ComputeViewBackgroundInvalidation());
    }

    if (box_.GetScrollableArea()) {
        if (should_invalidate_in_both_spaces || (BackgroundPaintsInContentsSpace() && background_invalidation_type != BackgroundInvalidationType::kNone)) {
            auto reason = background_invalidation_type == BackgroundInvalidationType::kFull ? PaintInvalidationReason::kBackground
                                                                                            : PaintInvalidationReason::kIncremental;
            context_.painting_layer->SetNeedsRepaint();
            ObjectPaintInvalidator(box_).InvalidateDisplayItemClient(box_.GetScrollableArea()->GetScrollingBackgroundDisplayItemClient(), reason);
        }
    }

    if (should_invalidate_in_both_spaces || (BackgroundPaintsInBorderBoxSpace() && background_invalidation_type == BackgroundInvalidationType::kFull)) {
        box_.GetMutableForPainting().SetShouldDoFullPaintInvalidationWithoutLayoutChange(PaintInvalidationReason::kBackground);
    }

    if (background_invalidation_type == BackgroundInvalidationType::kNone && box_.ScrollsOverflow()
...
void BoxPaintInvalidator::InvalidatePaint()
{
    ...
    ObjectPaintInvalidatorWithContext(box_, context_).InvalidatePaintWithComputedReason(ComputePaintInvalidationReason());
...
bool BoxPaintInvalidator::NeedsToSavePreviousContentBoxRect()
{
    ...
    // crbug.com/490533
    if ((style.BackgroundLayers().AnyLayerUsesContentBox() || style.MaskLayers().AnyLayerUsesContentBox()) && box_.ContentSize() != box_.Size()) {
        return true;
    }
...
bool BoxPaintInvalidator::NeedsToSavePreviousOverflowData()
{
    ...
    if ((BackgroundGeometryDependsOnScrollableOverflowRect() || BackgroundPaintsInContentsSpace() || box_.IsDocumentElement())
