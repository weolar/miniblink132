...
public:
    BoxDecorationData(const PaintInfo& paint_info, const LayoutReplaced& layout_replaced)
        : BoxDecorationData(paint_info, layout_replaced, layout_replaced.StyleRef(), layout_replaced.StyleRef().HasBorderDecoration())
    {
    }

    BoxDecorationData(const PaintInfo& paint_info, const PhysicalFragment& fragment, const ComputedStyle& style)
        : BoxDecorationData(paint_info, To<LayoutBox>(*fragment.GetLayoutObject()), style, !fragment.HasCollapsedBorders() && style.HasBorderDecoration())
    {
    }

    BoxDecorationData(const PaintInfo& paint_info, const PhysicalFragment& fragment)
    ...
    {
        return should_paint_background_;
    }
    bool ShouldPaintBorder() const
    {
        return should_paint_border_;
    }
    bool ShouldPaintShadow() const
    {
        return should_paint_shadow_;
    }
    ...
    {
        return HasAppearance() || ShouldPaintBackground() || ShouldPaintBorder() || ShouldPaintShadow();
    }
...
private:
    BoxDecorationData(const PaintInfo& paint_info, const LayoutBox& layout_box, const ComputedStyle& style, const bool has_non_collapsed_border_decoration)
        ...
        , should_paint_border_(ComputeShouldPaintBorder(has_non_collapsed_border_decoration))
        , should_paint_shadow_(ComputeShouldPaintShadow())
    {
    }

    // For BackgroundOnly() and BorderOnly().
    BoxDecorationData(const BoxDecorationData& data, bool should_paint_background, bool should_paint_border)
    ...
    bool ComputeShouldPaintBackground() const
    {
        ...
        // background.
        bool has_background = style_.HasBackground() || GetBoxFragmentType() == PhysicalFragment::kPageBorderBox;
        return has_background && !layout_box_.BackgroundTransfersToView() && !paint_info_.ShouldSkipBackground();
    }

    bool ComputeShouldPaintBorder(bool has_non_collapsed_border_decoration) const
    {
    ...
    {
        return !paint_info_.IsPaintingBackgroundInContentsSpace() && style_.BoxShadow();
    }
