...
public:
    ClipRectsContext(const PaintLayer* root, const FragmentData* fragment,
        OverlayScrollbarClipBehavior overlay_scrollbar_clip_behavior = kIgnoreOverlayScrollbarSize,
        ShouldRespectOverflowClipType root_layer_clip_behavior = kRespectOverflowClip, const PhysicalOffset& sub_pixel_accumulation = PhysicalOffset())
        : root_layer(root)
...
public:
    ...
    void CalculateBackgroundClipRect(const ClipRectsContext&, ClipRect& output) const;
    ...
    void CalculateRects(const ClipRectsContext& context, const FragmentData& fragment_data, PhysicalOffset& layer_offset, ClipRect& background_rect,
        ClipRect& foreground_rect) const;

private:
    ALWAYS_INLINE bool ShouldClipOverflowAlongEitherAxis(const ClipRectsContext&) const;
    ...
    ALWAYS_INLINE void CalculateBackgroundClipRectInternal(
        const ClipRectsContext&, const FragmentData&, ShouldRespectOverflowClipType should_apply_self_overflow_clip, ClipRect& output) const;
