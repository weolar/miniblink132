...
public:
    ...
    explicit PaintInvalidatorContext(const PaintInvalidatorContext& parent)
        ...
        , inside_opaque_layout_shift_root(parent.inside_opaque_layout_shift_root)
    {
    }

    bool NeedsSubtreeWalk() const
    {
        return subtree_flags & (kSubtreeInvalidationChecking | kSubtreeFullInvalidation | kSubtreeFullInvalidationForStackedContents);
    }
...
private:
    ...
    std::optional<LayoutShiftTracker::ContainingBlockScope> containing_block_scope_;
...
public:
    ...
    bool InvalidatePaint(const LayoutObject&, const PrePaintInfo*, const PaintPropertyTreeBuilderContext*, PaintInvalidatorContext&);
...
private:
    ...
    ALWAYS_INLINE void UpdatePaintingLayer(const LayoutObject&, PaintInvalidatorContext&);
    ALWAYS_INLINE void UpdateFromTreeBuilderContext(const PaintPropertyTreeBuilderFragmentContext&, PaintInvalidatorContext&);
    ALWAYS_INLINE void UpdateLayoutShiftTracking(const LayoutObject&, const PaintPropertyTreeBuilderFragmentContext&, PaintInvalidatorContext&);
