...
TEST_F(ObjectPaintInvalidatorTest, Selection)
{
    ...
    const auto* invalidations = &GetRasterInvalidationTracking(*GetDocument().View())->Invalidations();
    ...
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(GetRasterInvalidationTracking(*GetDocument().View())->Invalidations().empty());
    GetDocument().View()->SetTracksRasterInvalidations(false);
    ...
    UpdateAllLifecyclePhasesForTest();
    invalidations = &GetRasterInvalidationTracking(*GetDocument().View())->Invalidations();
    ASSERT_EQ(1u, invalidations->size());
    EXPECT_EQ(gfx::Rect(8, 8, 102, 102), (*invalidations)[0].rect);
    EXPECT_EQ(PaintInvalidationReason::kSelection, (*invalidations)[0].reason);
    GetDocument().View()->SetTracksRasterInvalidations(false);
...
TEST_F(ObjectPaintInvalidatorTest, VisibilityHidden)
{
    ...
    target_element->setAttribute(html_names::kStyleAttr, AtomicString("width: 200px"));
    GetDocument().View()->UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason::kTest);
    EXPECT_TRUE(IsValidDisplayItemClient(target));
    UpdateAllLifecyclePhasesForTest();

    target_element->setAttribute(html_names::kStyleAttr, AtomicString("width: 200px; visibility: visible"));
    GetDocument().View()->UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason::kTest);
    EXPECT_FALSE(IsValidDisplayItemClient(target));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(IsValidDisplayItemClient(target));

    target_element->setAttribute(html_names::kStyleAttr, AtomicString("width: 200px; visibility: hidden"));
    GetDocument().View()->UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason::kTest);
    EXPECT_FALSE(IsValidDisplayItemClient(target));
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(ObjectPaintInvalidatorTest, DirectPaintInvalidationSkipsPaintInvalidationChecking)
{
    SetBodyInnerHTML(R"HTML(
    ...
    div->setAttribute(html_names::kStyleAttr, AtomicString("color: rgb(80, 100, 175)"));
