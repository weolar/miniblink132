...
TEST_F(MarkerRangeMappingContextTest, FullNodeOffsetsCorrect)
{
    ...
    TextFragmentMarker* marker_pre = MakeGarbageCollected<TextFragmentMarker>(1, 5); // Before text
    auto offsets = mapping_context.GetTextContentOffsets(*marker_pre);
    ASSERT_FALSE(offsets.has_value());
    offsets.reset();

    TextFragmentMarker* marker_a = MakeGarbageCollected<TextFragmentMarker>(7, 10); // Partially before
    ...
    TextFragmentMarker* marker_b = MakeGarbageCollected<TextFragmentMarker>(11, 12); // 'b'
    ...
    TextFragmentMarker* marker_ij = MakeGarbageCollected<TextFragmentMarker>(25, 28); // Overlaps 1st and 2nd line
    ...
    TextFragmentMarker* marker_post = MakeGarbageCollected<TextFragmentMarker>(30, 35); // After text
