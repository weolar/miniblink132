...
class CORE_EXPORT PaintLayerStackingNode : public GarbageCollected<PaintLayerStackingNode> {
public:
    ...
    static bool StyleDidChange(PaintLayer& paint_layer, const ComputedStyle* old_style);
    ...
    const PaintLayers* LayersPaintingOverlayOverflowControlsAfter(const PaintLayer* layer) const
    {
        DCHECK(!z_order_lists_dirty_);
        auto it = layer_to_overlay_overflow_controls_painting_after_.find(layer);
        return it == layer_to_overlay_overflow_controls_painting_after_.end() ? nullptr : it->value.Get();
    }
...
private:
  ...
  //           target  child  |  other
    ...
    HeapHashMap<Member<const PaintLayer>, Member<PaintLayers>> layer_to_overlay_overflow_controls_painting_after_;
