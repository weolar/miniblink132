...
void PaintLayerClipper::CalculateRects(const ClipRectsContext& context, const FragmentData& fragment_data, PhysicalOffset& layer_offset,
    ClipRect& background_rect, ClipRect& foreground_rect) const
{
    ...
    if (layer_ == context.root_layer) {
        ...
        auto projection
            = GeometryMapper::SourceToDestinationProjection(fragment_data.PreTransform(), context.root_fragment->LocalBorderBoxProperties().Transform());
        layer_offset = PhysicalOffset::FromPointFRound(projection.MapPoint(gfx::PointF(layer_offset)));
        layer_offset -= context.root_fragment->PaintOffset();
    }

    CalculateBackgroundClipRectInternal(context, fragment_data, kRespectOverflowClip, background_rect);
    ...
    if (ShouldClipOverflowAlongEitherAxis(context)) {
        ...
        foreground_rect = To<LayoutBox>(layout_object).OverflowClipRect(layer_offset, context.overlay_scrollbar_clip_behavior);
...
void PaintLayerClipper::CalculateBackgroundClipRectInternal(
    const ClipRectsContext& context, const FragmentData& fragment_data, ShouldRespectOverflowClipType should_apply_self_overflow_clip, ClipRect& output) const
{
    output.Reset();
    ...
    auto destination_property_tree_state = context.root_fragment->LocalBorderBoxProperties();
    if (context.ShouldRespectRootLayerClip()) {
        destination_property_tree_state.SetClip(context.root_fragment->PreClip());
        destination_property_tree_state.SetEffect(context.root_fragment->PreEffect());
    } else {
        destination_property_tree_state.SetClip(context.root_fragment->ContentsClip());
    }
    ...
    if ((should_apply_self_overflow_clip == kRespectOverflowClip) && HasNonVisibleOverflow(*layer_)) {
        ...
        OverlayScrollbarClipBehavior clip_behavior = context.overlay_scrollbar_clip_behavior;
        ...
        GeometryMapper::LocalToAncestorVisualRect(source_property_tree_state, destination_property_tree_state, clip_rect, clip_behavior);
        output.SetRect(clip_rect);
    } else if (&source_property_tree_state.Clip() != &destination_property_tree_state.Clip()) {
        const FloatClipRect& clipped_rect_in_root_layer_space
            = GeometryMapper::LocalToAncestorClipRect(source_property_tree_state, destination_property_tree_state, context.overlay_scrollbar_clip_behavior);
...
PhysicalRect PaintLayerClipper::LocalVisualRect(const ClipRectsContext& context) const
{
    ...
    PhysicalRect layer_bounds_with_visual_overflow = affected_by_url_bar ? layout_object.View()->ViewRect() : To<LayoutBox>(layout_object).VisualOverflowRect();
    return layer_bounds_with_visual_overflow;
}

void PaintLayerClipper::CalculateBackgroundClipRect(const ClipRectsContext& context, ClipRect& output) const
{
    ...
    CalculateBackgroundClipRectInternal(context, fragment_data, kIgnoreOverflowClip, output);
}

bool PaintLayerClipper::ShouldClipOverflowAlongEitherAxis(const ClipRectsContext& context) const
{
    ...
    // phases.
    return HasNonVisibleOverflow(*layer_) || (layer_->GetLayoutObject().IsLayoutEmbeddedContent() && layer_->GetLayoutObject().StyleRef().HasBorderRadius());
}
