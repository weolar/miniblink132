...
INSTANTIATE_TEST_SUITE_P(All, PaintPropertyTreeUpdateTest, ::testing::Values(0, kUnderInvalidationChecking));

TEST_P(PaintPropertyTreeUpdateTest, BackgroundAttachmentFixedMainThreadScrollReasonsWithNestedScrollers)
{
    ...
    EXPECT_TRUE(
        overflow_a->GetLayoutObject()->FirstFragment().PaintProperties()->ScrollTranslation()->ScrollNode()->RequiresMainThreadForBackgroundAttachmentFixed());
    EXPECT_TRUE(
        overflow_b->GetLayoutObject()->FirstFragment().PaintProperties()->ScrollTranslation()->ScrollNode()->RequiresMainThreadForBackgroundAttachmentFixed());
    ...
    overflow_b->setAttribute(html_names::kClassAttr, AtomicString("backgroundAttachmentFixed"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(DocScroll()->RequiresMainThreadForBackgroundAttachmentFixed());
    EXPECT_TRUE(
        overflow_a->GetLayoutObject()->FirstFragment().PaintProperties()->ScrollTranslation()->ScrollNode()->RequiresMainThreadForBackgroundAttachmentFixed());
...
TEST_P(PaintPropertyTreeUpdateTest, ParentFrameMainThreadScrollReasons)
{
    ...
    SetChildFrameHTML("<style>body { margin: 0; }</style>"
                      "<div id='forceScroll' style='height: 8888px;'></div>");
    UpdateAllLifecyclePhasesForTest();
    Document* parent = &GetDocument();
    EXPECT_TRUE(DocScroll(parent)->RequiresMainThreadForBackgroundAttachmentFixed());
    Document* child = &ChildDocument();
    EXPECT_TRUE(DocScroll(child)->RequiresMainThreadForBackgroundAttachmentFixed());

    // Removing a main thread scrolling reason should update the entire tree.
    auto* fixed_background = GetDocument().getElementById(AtomicString("fixedBackground"));
    fixed_background->removeAttribute(html_names::kClassAttr);
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(DocScroll(parent)->RequiresMainThreadForBackgroundAttachmentFixed());
    EXPECT_FALSE(DocScroll(child)->RequiresMainThreadForBackgroundAttachmentFixed());

    // Adding a main thread scrolling reason should update the entire tree.
    fixed_background->setAttribute(html_names::kClassAttr, AtomicString("fixedBackground"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(DocScroll(parent)->RequiresMainThreadForBackgroundAttachmentFixed());
    EXPECT_TRUE(DocScroll(child)->RequiresMainThreadForBackgroundAttachmentFixed());
}
...
TEST_P(PaintPropertyTreeUpdateTest, ChildFrameMainThreadScrollReasons)
{
    ...
    // Removing a main thread scrolling reason should update the entire tree.
    auto* fixed_background = ChildDocument().getElementById(AtomicString("fixedBackground"));
    fixed_background->removeAttribute(html_names::kClassAttr);
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(DocScroll(parent)->RequiresMainThreadForBackgroundAttachmentFixed());
    EXPECT_FALSE(DocScroll(child)->RequiresMainThreadForBackgroundAttachmentFixed());
...
TEST_P(PaintPropertyTreeUpdateTest, BackgroundAttachmentFixedMainThreadScrollReasonsWithFixedScroller)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      #overflowA {
    ...
    EXPECT_TRUE(
        overflow_b->GetLayoutObject()->FirstFragment().PaintProperties()->ScrollTranslation()->ScrollNode()->RequiresMainThreadForBackgroundAttachmentFixed());
    EXPECT_EQ(DocScroll(), overflow_b->GetLayoutObject()->FirstFragment().PaintProperties()->ScrollTranslation()->ScrollNode()->Parent());
...
TEST_P(PaintPropertyTreeUpdateTest, DescendantNeedsUpdateAcrossFrames)
{
    ...
    SetChildFrameHTML("<style>body { margin: 0; }</style><div id='transform' style='transform: "
    ...
    EXPECT_FALSE(child_frame_view->GetLayoutView()->DescendantNeedsPaintPropertyUpdate());
...
TEST_P(PaintPropertyTreeUpdateTest, BuildingStopsAtThrottledFrames)
{
    ...
    auto* iframe = To<HTMLIFrameElement>(GetDocument().getElementById(AtomicString("iframe")));
    iframe->setAttribute(html_names::kStyleAttr, AtomicString("transform: translateY(5555px)"));
    UpdateAllLifecyclePhasesForTest();
    ...
    auto* iframe_transform = ChildDocument().getElementById(AtomicString("iframeTransform"))->GetLayoutObject();
...
TEST_P(PaintPropertyTreeUpdateTest, ClipChangesUpdateOverflowClip)
{
    ...
    div->setAttribute(html_names::kStyleAttr, AtomicString("display:inline-block; width:7px;"));
    UpdateAllLifecyclePhasesForTest();
    auto* clip_properties = div->GetLayoutObject()->FirstFragment().PaintProperties()->OverflowClip();
    EXPECT_CLIP_RECT(gfx::RectF(0, 0, 7, 0), clip_properties);

    // Width changes should update the overflow clip.
    div->setAttribute(html_names::kStyleAttr, AtomicString("display:inline-block; width:7px;"));
    UpdateAllLifecyclePhasesForTest();
    clip_properties = div->GetLayoutObject()->FirstFragment().PaintProperties()->OverflowClip();
    EXPECT_CLIP_RECT(gfx::RectF(0, 0, 7, 0), clip_properties);
    div->setAttribute(html_names::kStyleAttr, AtomicString("display:inline-block; width:9px;"));
    UpdateAllLifecyclePhasesForTest();
    ...
    div->setAttribute(html_names::kStyleAttr, AtomicString("display:inline-block; width:7px; padding-right:3px;"));
    UpdateAllLifecyclePhasesForTest();
    clip_properties = div->GetLayoutObject()->FirstFragment().PaintProperties()->OverflowClip();
    EXPECT_CLIP_RECT(gfx::RectF(0, 0, 10, 0), clip_properties);
    div->setAttribute(html_names::kStyleAttr, AtomicString("display:inline-block; width:8px; padding-right:2px;"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_CLIP_RECT(gfx::RectF(0, 0, 10, 0), clip_properties);
    ...
    div->setAttribute(html_names::kStyleAttr, AtomicString("border-right:3px solid red;"));
    UpdateAllLifecyclePhasesForTest();
    clip_properties = div->GetLayoutObject()->FirstFragment().PaintProperties()->OverflowClip();
    EXPECT_CLIP_RECT(gfx::RectF(0, 0, 797, 0), clip_properties);
    div->setAttribute(html_names::kStyleAttr, AtomicString("border-right:5px solid red;"));
    UpdateAllLifecyclePhasesForTest();
    ...
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(!div->GetLayoutObject()->FirstFragment().PaintProperties() || !div->GetLayoutObject()->FirstFragment().PaintProperties()->OverflowClip());
}
...
TEST_P(PaintPropertyTreeUpdateTest, ContainPaintChangesUpdateOverflowClip)
{
    ...
    UpdateAllLifecyclePhasesForTest();
    auto* div = GetDocument().getElementById(AtomicString("div"));
    auto* properties = div->GetLayoutObject()->FirstFragment().PaintProperties()->OverflowClip();
...
TEST_P(PaintPropertyTreeUpdateTest, NoPaintPropertyUpdateOnBackgroundChange)
{
    ...
    UpdateAllLifecyclePhasesForTest();
    div->setAttribute(html_names::kStyleAttr, AtomicString("background-color: green"));
    GetDocument().View()->UpdateLifecycleToLayoutClean(DocumentUpdateReason::kTest);
...
TEST_P(PaintPropertyTreeUpdateTest, DISABLED_FrameVisibilityChangeUpdatesProperties)
{
    SetBodyInnerHTML(R"HTML(
    <style>body { margin: 0; }</style>
    ...
    auto* iframe_container = GetDocument().getElementById(AtomicString("iframeContainer"));
    iframe_container->setAttribute(html_names::kStyleAttr, AtomicString("visibility: hidden;"));
    frame_view->UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintPropertyTreeUpdateTest, TransformNodeWithAnimationLosesNodeWhenAnimationRemoved)
{
    ...
    EXPECT_EQ(nullptr, target->GetLayoutObject()->FirstFragment().PaintProperties());
}

TEST_P(PaintPropertyTreeUpdateTest, EffectNodeWithAnimationLosesNodeWhenAnimationRemoved)
{
...
TEST_P(PaintPropertyTreeUpdateTest, TransformNodeDoesNotLoseCompositorElementIdWhenAnimationRemoved)
{
    LoadTestData("transform-animation.html");

    Element* target = GetDocument().getElementById(AtomicString("target"));
    target->setAttribute(html_names::kStyleAttr, AtomicString("transform: translateX(2em)"));
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintPropertyTreeUpdateTest, EffectNodeDoesNotLoseCompositorElementIdWhenAnimationRemoved)
{
    LoadTestData("opacity-animation.html");
...
TEST_P(PaintPropertyTreeUpdateTest, PerspectiveOriginUpdatesOnSizeChanges)
{
    ...
    EXPECT_EQ(matrix, perspective->FirstFragment().PaintProperties()->Perspective()->Matrix());
    EXPECT_EQ(gfx::Point3F(50, 0, 0), perspective->FirstFragment().PaintProperties()->Perspective()->Origin());

    auto* contents = GetDocument().getElementById(AtomicString("contents"));
    contents->setAttribute(html_names::kStyleAttr, AtomicString("height: 200px;"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(matrix, perspective->FirstFragment().PaintProperties()->Perspective()->Matrix());
    EXPECT_EQ(gfx::Point3F(50, 100, 0), perspective->FirstFragment().PaintProperties()->Perspective()->Origin());
}
...
TEST_P(PaintPropertyTreeUpdateTest, TransformUpdatesOnRelativeLengthChanges)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(50, 100), transform_object->FirstFragment().PaintProperties()->Transform()->Get2dTranslation());

    transform->setAttribute(html_names::kStyleAttr, AtomicString("width: 200px; height: 300px;"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(gfx::Vector2dF(100, 150), transform_object->FirstFragment().PaintProperties()->Transform()->Get2dTranslation());
}
...
TEST_P(PaintPropertyTreeUpdateTest, CSSClipDependingOnSize)
{
    ...
    EXPECT_CLIP_RECT(gfx::RectF(45, 50, 105, 100), clip->FirstFragment().PaintProperties()->CssClip());

    outer->setAttribute(html_names::kStyleAttr, AtomicString("height: 200px"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_CLIP_RECT(gfx::RectF(45, 50, 105, 200), clip->FirstFragment().PaintProperties()->CssClip());
}
...
TEST_P(PaintPropertyTreeUpdateTest, ScrollBoundsChange)
{
    ...
    auto* scroll_node = container->FirstFragment().PaintProperties()->ScrollTranslation()->ScrollNode();
    EXPECT_EQ(gfx::Rect(0, 0, 100, 100), scroll_node->ContainerRect());
    EXPECT_EQ(gfx::Rect(0, 0, 200, 200), scroll_node->ContentsRect());

    GetDocument().getElementById(AtomicString("content"))->setAttribute(html_names::kStyleAttr, AtomicString("width: 200px; height: 300px"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(scroll_node, container->FirstFragment().PaintProperties()->ScrollTranslation()->ScrollNode());
    EXPECT_EQ(gfx::Rect(0, 0, 100, 100), scroll_node->ContainerRect());
...
TEST_P(PaintPropertyTreeUpdateTest, ViewportContentsAndContainerRectsIncludeScrollbar)
{
    SetBodyInnerHTML(R"HTML(
    <style>
    ...
    VisualViewport& visual_viewport = GetDocument().GetPage()->GetVisualViewport();

    EXPECT_EQ(gfx::Rect(0, 0, 800, 600), visual_viewport.GetScrollNode()->ContainerRect());
    EXPECT_EQ(gfx::Rect(0, 0, 800, 600), visual_viewport.GetScrollNode()->ContentsRect());
}

TEST_P(PaintPropertyTreeUpdateTest, ViewportAddRemoveDeviceEmulationNode)
{
    SetBodyInnerHTML("<style>body {height: 2000px; width: 2000px; margin: 0;}</style>");
    ...
    ASSERT_TRUE(GetLayoutView().GetScrollableArea());
    {
        ...
        EXPECT_EQ(&TransformPaintPropertyNode::Root(), &chunk.properties.Transform());
    }
    ...
    EXPECT_TRUE(visual_viewport.GetDeviceEmulationTransformNode());
    {
        auto& chunk = ContentPaintChunks()[1];
        EXPECT_EQ(DisplayItem::kScrollbarHorizontal, chunk.id.type);
        EXPECT_EQ(visual_viewport.GetDeviceEmulationTransformNode(), &chunk.properties.Transform());
    }
...
TEST_P(PaintPropertyTreeUpdateTest, ScrollbarWidthChange)
{
    ...
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(overflow_clip, container->FirstFragment().PaintProperties()->OverflowClip());
...
TEST_P(PaintPropertyTreeUpdateTest, Preserve3DChange)
{
    ...
    GetDocument().getElementById(AtomicString("parent"))->setAttribute(html_names::kStyleAttr, AtomicString("transform-style: preserve-3d"));
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintPropertyTreeUpdateTest, BoxAddRemoveMask)
{
    ...
    target->setAttribute(html_names::kStyleAttr, AtomicString("-webkit-mask: linear-gradient(red, blue)"));
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintPropertyTreeUpdateTest, MaskClipNodeBoxSizeChange)
{
    ...
    GetDocument().getElementById(AtomicString("target"))->setAttribute(html_names::kStyleAttr, AtomicString("height: 200px"));
    UpdateAllLifecyclePhasesForTest();
...
{
    SetBodyInnerHTML("<span id='target'><img id='img' style='width: 50px'></span>");
...
TEST_P(PaintPropertyTreeUpdateTest, MaskClipNodeInlineBoundsChange)
{
    ...
    GetDocument().getElementById(AtomicString("img"))->setAttribute(html_names::kStyleAttr, AtomicString("width: 100px"));
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintPropertyTreeUpdateTest, AddRemoveSVGMask)
{
    ...
    GetDocument().getElementById(AtomicString("rect"))->setAttribute(svg_names::kMaskAttr, AtomicString("url(#mask)"));
    UpdateAllLifecyclePhasesForTest();
    ...
    GetDocument().getElementById(AtomicString("rect"))->removeAttribute(svg_names::kMaskAttr);
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(nullptr, PaintPropertiesForElement("rect"));
}
...
TEST_P(PaintPropertyTreeUpdateTest, SVGMaskTargetBoundsChange)
{
    ...
    GetDocument().getElementById(AtomicString("rect"))->setAttribute(svg_names::kWidthAttr, AtomicString("200"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_NE(nullptr, properties->Effect());
    EXPECT_NE(nullptr, properties->Mask());
...
TEST_P(PaintPropertyTreeUpdateTest, WillTransformChangeAboveFixed)
{
    ...
    EXPECT_EQ(container->FirstFragment().PaintProperties()->Transform(), &fixed->FirstFragment().LocalBorderBoxProperties().Transform());

    To<Element>(container->GetNode())->setAttribute(html_names::kStyleAttr, AtomicString("will-change: top"));
    UpdateAllLifecyclePhasesForTest();
    ...
    To<Element>(container->GetNode())->setAttribute(html_names::kStyleAttr, AtomicString("will-change: transform"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(container->FirstFragment().PaintProperties()->Transform(), &fixed->FirstFragment().LocalBorderBoxProperties().Transform());
}
...
TEST_P(PaintPropertyTreeUpdateTest, CompositingReasonForAnimation)
{
    ...
    auto* target = GetDocument().getElementById(AtomicString("target"));
    auto* transform = target->GetLayoutObject()->FirstFragment().PaintProperties()->Transform();
    ASSERT_TRUE(transform);
    EXPECT_FALSE(transform->HasDirectCompositingReasons());

    auto* filter = target->GetLayoutObject()->FirstFragment().PaintProperties()->Filter();
    ASSERT_TRUE(filter);
    EXPECT_FALSE(filter->HasDirectCompositingReasons());

    target->setAttribute(html_names::kStyleAttr, AtomicString("transform: translateX(11px)"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(transform->HasDirectCompositingReasons());
    EXPECT_TRUE(transform->HasActiveTransformAnimation());
    EXPECT_FALSE(filter->HasDirectCompositingReasons());

    target->setAttribute(html_names::kStyleAttr, AtomicString("transform: translateX(11px); filter: opacity(40%)"));
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintPropertyTreeUpdateTest, SVGViewportContainerOverflowChange)
{
    ...
    GetDocument().getElementById(AtomicString("target"))->setAttribute(svg_names::kOverflowAttr, AtomicString("visible"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(nullptr, PaintPropertiesForElement("target"));

    GetDocument().getElementById(AtomicString("target"))->setAttribute(svg_names::kOverflowAttr, AtomicString("hidden"));
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintPropertyTreeUpdateTest, PropertyTreesRebuiltAfterSVGBlendModeChange)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      #blended {
    ...
    const auto* props = blended_element->GetLayoutObject()->FirstFragment().PaintProperties();
    ASSERT_TRUE(props->Effect());
    EXPECT_EQ(props->Effect()->BlendMode(), SkBlendMode::kDarken);

    blended_element->setAttribute(html_names::kStyleAttr, AtomicString("mix-blend-mode: lighten;"));
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintPropertyTreeUpdateTest, EnsureSnapContainerData)
{
    ...
    EXPECT_EQ(doc_snap_container_data->scroll_snap_type().axis, cc::SnapAxis::kBoth);
    EXPECT_EQ(doc_snap_container_data->scroll_snap_type().strictness, cc::SnapStrictness::kProximity);
    EXPECT_EQ(doc_snap_container_data->rect(), gfx::RectF(0, 0, 300, 300));
    EXPECT_EQ(doc_snap_container_data->size(), 1u);
    EXPECT_EQ(doc_snap_container_data->at(0).rect, gfx::RectF(100, 700, 200, 200));
}

TEST_P(PaintPropertyTreeUpdateTest, EffectAndClipWithNonContainedOutOfFlowDescendant)
{
    SetBodyInnerHTML(R"HTML(
    ...
    descendant->setAttribute(html_names::kStyleAttr, AtomicString("position: relative"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(clip_properties->OverflowClip(), effect_properties->Effect()->OutputClip());

    descendant->setAttribute(html_names::kStyleAttr, AtomicString("position: absolute"));
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintPropertyTreeUpdateTest, ForwardReferencedSVGElementUpdate)
{
    ...
    EXPECT_EQ(svg2_properties->PaintOffsetTranslation(), svg2_properties->Perspective()->Parent());

    const auto* rect_properties = PaintPropertiesForElement("rect");
    ASSERT_NE(nullptr, rect_properties->Transform());
    EXPECT_EQ(svg2_properties->Perspective(), rect_properties->Transform()->Parent());
    EXPECT_EQ(
        MakeTranslationMatrix(1, 0), GeometryMapper::SourceToDestinationProjection(*rect_properties->Transform(), *svg2_properties->PaintOffsetTranslation()));

    // Change filter which forward references rect, and insert a transform
    // node above rect's transform.
    GetDocument().getElementById(AtomicString("filter"))->setAttribute(svg_names::kWidthAttr, AtomicString("20"));
    GetDocument().getElementById(AtomicString("svg2"))->setAttribute(svg_names::kTransformAttr, AtomicString("translate(2)"));
    UpdateAllLifecyclePhasesForTest();

    EXPECT_NE(nullptr, svg2_properties->Transform());
    EXPECT_EQ(svg2_properties->PaintOffsetTranslation(), svg2_properties->Transform()->Parent());
    EXPECT_EQ(svg2_properties->Transform(), svg2_properties->Perspective()->Parent());
    EXPECT_EQ(svg2_properties->Perspective(), rect_properties->Transform()->Parent());

    // Ensure that GeometryMapper's cache is properly invalidated and updated.
    EXPECT_EQ(
        MakeTranslationMatrix(3, 0), GeometryMapper::SourceToDestinationProjection(*rect_properties->Transform(), *svg2_properties->PaintOffsetTranslation()));
}
...
TEST_P(PaintPropertyTreeUpdateTest, OverflowClipUpdateForImage)
{
    ...
    target->setAttribute(html_names::kStyleAttr, AtomicString("object-fit: cover"));
    UpdateAllLifecyclePhasesForTest();
    properties = PaintPropertiesForElement("target");
    // Image elements don't need a clip node if the image is clipped to its
    // content box.
    ...
    target->setAttribute(html_names::kStyleAttr, AtomicString("object-fit: none"));
    UpdateAllLifecyclePhasesForTest();
    properties = PaintPropertiesForElement("target");
    // Ditto.
    EXPECT_EQ(nullptr, properties);

    target->setAttribute(html_names::kStyleAttr, AtomicString("overflow-clip-margin: padding-box;"));
    UpdateAllLifecyclePhasesForTest();
    properties = PaintPropertiesForElement("target");
    // Changing overflow-clip-margin induces a clip node.
    EXPECT_NE(nullptr, properties);

    target->setAttribute(html_names::kStyleAttr, AtomicString("object-fit: none; border-radius: 2px"));
    UpdateAllLifecyclePhasesForTest();
    properties = PaintPropertiesForElement("target");
    ASSERT_TRUE(properties);
    ASSERT_TRUE(properties->OverflowClip());
    EXPECT_CLIP_RECT(FloatRoundedRect(gfx::RectF(8, 8, 8, 8), 0), properties->OverflowClip());
    EXPECT_CLIP_RECT(FloatRoundedRect(gfx::RectF(8, 8, 8, 8), 2), properties->InnerBorderRadiusClip());

    // We should update clip rect on border radius change.
    target->setAttribute(html_names::kStyleAttr, AtomicString("object-fit: none; border-radius: 3px"));
    UpdateAllLifecyclePhasesForTest();
    ASSERT_EQ(properties, PaintPropertiesForElement("target"));
    ASSERT_TRUE(properties->OverflowClip());
    EXPECT_CLIP_RECT(FloatRoundedRect(gfx::RectF(8, 8, 8, 8), 0), properties->OverflowClip());
    EXPECT_CLIP_RECT(FloatRoundedRect(gfx::RectF(8, 8, 8, 8), 3), properties->InnerBorderRadiusClip());

    // We should update clip rect on padding change.
    target->setAttribute(html_names::kStyleAttr, AtomicString("object-fit: none; border-radius: 3px; padding: 1px 2px 3px 4px"));
    UpdateAllLifecyclePhasesForTest();
    ASSERT_EQ(properties, PaintPropertiesForElement("target"));
    ASSERT_TRUE(properties->OverflowClip());
    ...
    EXPECT_CLIP_RECT(
        FloatRoundedRect(gfx::RectF(12, 9, 2, 4), gfx::SizeF(0, 2), gfx::SizeF(1, 2), gfx::SizeF(), gfx::SizeF(1, 0)), properties->InnerBorderRadiusClip());
}
...
TEST_P(PaintPropertyTreeUpdateTest, OverflowClipUpdateForVideo)
{
    ...
    // We should update clip rect on padding change.
    target->setAttribute(html_names::kStyleAttr, AtomicString("object-fit: none; padding: 1px 2px 3px 4px"));
    UpdateAllLifecyclePhasesForTest();
    ASSERT_EQ(properties, PaintPropertiesForElement("target"));
    ASSERT_TRUE(properties->OverflowClip());
...
TEST_P(PaintPropertyTreeUpdateTest, OverflowClipWithBorderRadiusForVideo)
{
    ...
    target->setAttribute(html_names::kStyleAttr, AtomicString("border-radius: 5px"));
    UpdateAllLifecyclePhasesForTest();
    ASSERT_EQ(properties, PaintPropertiesForElement("target"));
    ASSERT_TRUE(properties->OverflowClip());
    EXPECT_CLIP_RECT(FloatRoundedRect(4, 1, 8, 8), properties->OverflowClip());
    ASSERT_TRUE(properties->InnerBorderRadiusClip());
    EXPECT_CLIP_RECT(FloatRoundedRect(gfx::RectF(4, 1, 8, 8), FloatRoundedRect::Radii(gfx::SizeF(1, 4), gfx::SizeF(3, 4), gfx::SizeF(1, 2), gfx::SizeF(3, 2))),
...
TEST_P(PaintPropertyTreeUpdateTest, SubpixelAccumulationAcrossIsolation)
{
    ...
    auto* child = GetLayoutObjectByElementId("child");
    EXPECT_EQ(PhysicalOffset(LayoutUnit(10.25), LayoutUnit()), parent->FirstFragment().PaintOffset());
    EXPECT_EQ(gfx::Vector2dF(10, 0), isolation_properties->PaintOffsetTranslation()->Get2dTranslation());
    EXPECT_EQ(PhysicalOffset(), child->FirstFragment().PaintOffset());

    parent_element->setAttribute(html_names::kStyleAttr, AtomicString("margin-left: 12.75px"));
    UpdateAllLifecyclePhasesForTest();

    EXPECT_EQ(PhysicalOffset(LayoutUnit(12.75), LayoutUnit()), parent->FirstFragment().PaintOffset());
    EXPECT_EQ(gfx::Vector2dF(13, 0), isolation_properties->PaintOffsetTranslation()->Get2dTranslation());
    EXPECT_EQ(PhysicalOffset(), child->FirstFragment().PaintOffset());
}
...
TEST_P(PaintPropertyTreeUpdateTest, ChangeDuringAnimation)
{
    ...
    const auto* transform_node = target->FirstFragment().PaintProperties()->Transform();
    ...
    EXPECT_EQ(paint_artifact_compositor->NeedsUpdate(), PaintArtifactCompositor::UpdateType::kFull);
    UpdateAllLifecyclePhasesForTest();
    ...
    transform.Operations().push_back(MakeGarbageCollected<RotateTransformOperation>(10, TransformOperation::kRotate));
    builder.SetTransform(transform);
    builder.SetTransformOrigin(TransformOrigin(Length::Fixed(70), Length::Fixed(30), 0));
    target->SetStyle(builder.TakeStyle());
    EXPECT_TRUE(target->NeedsPaintPropertyUpdate());
    GetDocument().Lifecycle().AdvanceTo(DocumentLifecycle::kStyleClean);
    {
    ...
    ASSERT_EQ(transform_node, target->FirstFragment().PaintProperties()->Transform());
    EXPECT_TRUE(transform_node->HasActiveTransformAnimation());
...
TEST_P(PaintPropertyTreeUpdateTest, BackfaceVisibilityInvalidatesProperties)
{
    ...
    span->setAttribute(html_names::kStyleAttr, AtomicString("backface-visibility: hidden;"));
    GetDocument().View()->UpdateLifecycleToLayoutClean(DocumentUpdateReason::kTest);
...
TEST_P(PaintPropertyTreeUpdateTest, InlineFilterReferenceBoxChange)
{
    ...
    EXPECT_EQ(gfx::PointF(0, 20), properties->Filter()->Filter().ReferenceBox().origin());

    GetDocument().getElementById(AtomicString("spacer"))->setAttribute(html_names::kStyleAttr, AtomicString("display: inline-block; height: 100px"));
    UpdateAllLifecyclePhasesForTest();
    ASSERT_EQ(properties, PaintPropertiesForElement("span"));
    EXPECT_EQ(gfx::PointF(0, 100), properties->Filter()->Filter().ReferenceBox().origin());
}
...
TEST_P(PaintPropertyTreeUpdateTest, StartSVGAnimation)
{
    ...
    GetDocument().getElementById(AtomicString("line"))->setAttribute(html_names::kStyleAttr, AtomicString("transform: translateY(100px)"));
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintPropertyTreeUpdateTest, ScrollNonStackingContextContainingStacked)
{
    ...
    scroller->setAttribute(html_names::kStyleAttr, AtomicString("position: absolute; will-change: transform"));
    content->setAttribute(html_names::kStyleAttr, AtomicString("position: absolute"));
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintPropertyTreeUpdateTest, ScrollOriginChange)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(-20, 0), container_properties->ScrollTranslation()->Get2dTranslation());
    EXPECT_EQ(PhysicalOffset(), child1->FirstFragment().PaintOffset());
    EXPECT_EQ(PhysicalOffset(), child2->FirstFragment().PaintOffset());

    To<Element>(child2->GetNode())->setAttribute(html_names::kStyleAttr, AtomicString("width: 100px"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(gfx::Vector2dF(-120, 0), container_properties->ScrollTranslation()->Get2dTranslation());
...
TEST_P(PaintPropertyTreeUpdateTest, IFrameContainStrictChangeBorderTopWidth)
{
    ...
    auto* child_view_properties = ChildDocument().GetLayoutView()->FirstFragment().PaintProperties();
    ASSERT_TRUE(child_view_properties);
    ASSERT_TRUE(child_view_properties->PaintOffsetTranslation());
    EXPECT_EQ(gfx::Vector2dF(2, 2), child_view_properties->PaintOffsetTranslation()->Get2dTranslation());

    GetDocument().getElementById(AtomicString("iframe"))->setAttribute(html_names::kStyleAttr, AtomicString("border-top-width: 10px"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(gfx::Vector2dF(2, 10), child_view_properties->PaintOffsetTranslation()->Get2dTranslation());
}
...
TEST_P(PaintPropertyTreeUpdateTest, LocalBorderBoxPropertiesChange)
{
    ...
    Element* opacity_element = GetDocument().getElementById(AtomicString("opacity"));
    ...
    opacity_element->setAttribute(html_names::kStyleAttr, AtomicString("opacity: 0.5"));
    UpdateAllLifecyclePhasesExceptPaint();
...
TEST_P(PaintPropertyTreeUpdateTest, DirectTransformUpdateSkipsPropertyTreeBuilder)
{
    SetBodyInnerHTML(R"HTML(
    ...
    div->setAttribute(html_names::kStyleAttr, AtomicString("transform: translateX(200px)"));
    GetDocument().View()->UpdateLifecycleToLayoutClean(DocumentUpdateReason::kTest);
    EXPECT_FALSE(div->GetLayoutObject()->NeedsPaintPropertyUpdate());
...
TEST_P(PaintPropertyTreeUpdateTest, ChangeMaskOutputClip)
{
    ...
    EXPECT_EQ(container_properties->OverflowClip(), masked_properties->Mask()->OutputClip());

    GetDocument().getElementById(AtomicString("container"))->setAttribute(html_names::kStyleAttr, AtomicString("width: 100px; height: 100px"));
    UpdateAllLifecyclePhasesExceptPaint();
...
TEST_P(PaintPropertyTreeUpdateTest, DirectOpacityUpdateSkipsPropertyTreeBuilder)
{
    SetBodyInnerHTML(R"HTML(
...
TEST_P(PaintPropertyTreeUpdateTest, DirectOpacityAndTransformUpdatesBothExecuted)
{
    SetBodyInnerHTML(R"HTML(
    ...
    div->setAttribute(html_names::kStyleAttr, AtomicString("opacity:0.8; transform: translateX(200px)"));
    GetDocument().View()->UpdateLifecycleToLayoutClean(DocumentUpdateReason::kTest);
    EXPECT_FALSE(div->GetLayoutObject()->NeedsPaintPropertyUpdate());
...
TEST_P(PaintPropertyTreeUpdateTest, DirectTransformUpdateSkipsPropertyTreeBuilderForAncestors)
{
    SetBodyInnerHTML(R"HTML(
    ...
    auto* div = GetDocument().getElementById(AtomicString("div"));
    EXPECT_FALSE(div->GetLayoutObject()->NeedsPaintPropertyUpdate());
    auto* dom_ancestor = GetDocument().getElementById(AtomicString("dom_ancestor"));
    EXPECT_FALSE(dom_ancestor->GetLayoutObject()->NeedsPaintPropertyUpdate());
    auto* positioned_ancestor = GetDocument().getElementById(AtomicString("positioned_ancestor"));
    EXPECT_FALSE(positioned_ancestor->GetLayoutObject()->NeedsPaintPropertyUpdate());
...
TEST_P(PaintPropertyTreeUpdateTest, BackdropFilterBounds)
{
    ...
    EXPECT_EQ(gfx::RRectF(0, 0, 100, 100, 0), properties->Effect()->BackdropFilterBounds());

    GetDocument().getElementById(AtomicString("target"))->SetInlineStyleProperty(CSSPropertyID::kWidth, "200px");
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(gfx::RRectF(0, 0, 200, 100, 0), properties->Effect()->BackdropFilterBounds());
}
...
TEST_P(PaintPropertyTreeUpdateTest, UpdatesInLockedDisplayHandledCorrectly)
{
    ...
    auto* fast_path_div = GetDocument().getElementById(AtomicString("fast_path_div"));
    ...
    GetDocument().getElementById(AtomicString("fast_path_div"))->setAttribute(html_names::kStyleAttr, AtomicString("opacity:0.8"));
    GetDocument().getElementById(AtomicString("regular_update_div"))->setAttribute(html_names::kStyleAttr, AtomicString("background:purple"));
    GetDocument().getElementById(AtomicString("locked_display_inner"))->GetBoundingClientRect();
...
TEST_P(PaintPropertyTreeUpdateTest, AnchorPositioningScrollUpdate)
{
    ...
    EXPECT_EQ(PaintPropertiesForElement("target")->AnchorPositionScrollTranslation()->Get2dTranslation(), gfx::Vector2dF(0, -300));

    // Anchor positioning scroll update should not require main thread commits.
    EXPECT_EQ(GetFrame().View()->GetPaintArtifactCompositor()->NeedsUpdate(), PaintArtifactCompositor::UpdateType::kNone);
}
...
TEST_P(PaintPropertyTreeUpdateTest, ElementCaptureUpdate)
{
   ...
   <style>
    ...
    paint_properties = element->GetLayoutObject()->FirstFragment().PaintProperties();
