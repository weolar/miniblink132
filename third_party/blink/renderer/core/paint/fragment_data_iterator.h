...
template <typename Iterator, typename Data, typename Head> class FragmentDataIteratorBase {
    STACK_ALLOCATED();

public:
    explicit FragmentDataIteratorBase(Head& head)
        : fragment_head_(head)
    {
    }
...
class FragmentDataIterator : public FragmentDataIteratorBase<FragmentDataIterator, const FragmentData, const FragmentDataList> {
public:
...
class MutableFragmentDataIterator : public FragmentDataIteratorBase<MutableFragmentDataIterator, FragmentData, FragmentDataList> {
public:
    explicit MutableFragmentDataIterator(const LayoutObject& object)
        : FragmentDataIteratorBase(object.GetMutableForPainting().FragmentList())
    {
    }
};
...
public:
    ...
    {
        return cursor_ ? &(*cursor_) : nullptr;
    }
