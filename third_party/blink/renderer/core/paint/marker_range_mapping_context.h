...
private:
    ...
    private:
        base::span<const OffsetMappingUnit> GetMappingUnits(const LayoutObject* layout_object);

        // Find the mapping unit for `dom_offset`, starting from `begin`.
        base::span<const OffsetMappingUnit>::iterator FindUnit(base::span<const OffsetMappingUnit>::iterator begin, unsigned dom_offset) const;
...
public:
    ...
    explicit MarkerRangeMappingContext(const Text& text_node, const TextOffsetRange& fragment_dom_range)
    ...
    std::optional<TextOffsetRange> GetTextContentOffsets(const DocumentMarker&) const;
