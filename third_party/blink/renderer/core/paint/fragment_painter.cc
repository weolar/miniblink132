...
void FragmentPainter::PaintOutline(const PaintInfo& paint_info, const PhysicalOffset& paint_offset, const ComputedStyle& style_to_use)
{
    ...
    fragment.AddSelfOutlineRects(paint_offset, style_to_use.OutlineRectsShouldIncludeBlockInkOverflow(), collector, &info);

    VectorOf<PhysicalRect> outline_rects = collector.TakeRects();
    if (outline_rects.empty())
        return;

    OutlinePainter::PaintOutlineRects(paint_info, GetDisplayItemClient(), outline_rects, info, style_to_use);
}

void FragmentPainter::AddURLRectIfNeeded(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    ...
    auto outline_rects = fragment.GetLayoutObject()->OutlineRects(nullptr, paint_offset, OutlineType::kIncludeBlockInkOverflow);
    gfx::Rect rect = ToPixelSnappedRect(UnionRect(outline_rects));
    if (rect.IsEmpty())
        return;

    const DisplayItemClient& display_item_client = GetDisplayItemClient();
    if (DrawingRecorder::UseCachedDrawingIfPossible(paint_info.context, display_item_client, DisplayItem::kPrintedContentPDFURLRect))
        return;

    DrawingRecorder recorder(paint_info.context, display_item_client, DisplayItem::kPrintedContentPDFURLRect);
