...
void HTMLCanvasPainter::PaintReplaced(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    ...
    if (!canvas->IsCanvasClear()) {
        PaintTiming::From(layout_html_canvas_.GetDocument()).MarkFirstContentfulPaint();
    }
    ...
    if (auto* layer = canvas->ContentsCcLayer()) {
        ...
        if (layout_html_canvas_.DrawsBackgroundOntoContentLayer()) {
            Color background_color = layout_html_canvas_.ResolveColor(GetCSSPropertyBackgroundColor());
        ...
        if (!paint_info.ShouldOmitCompositingInfo() && !canvas->IsPrinting()) {
            ...
            RecordForeignLayer(context, layout_html_canvas_, DisplayItem::kForeignLayerCanvas, layer, pixel_snapped_rect.origin());
            return;
        }
    }

    if (DrawingRecorder::UseCachedDrawingIfPossible(context, layout_html_canvas_, paint_info.phase))
        return;

    BoxDrawingRecorder recorder(context, layout_html_canvas_, paint_info.phase, paint_offset);
    ScopedImageRenderingSettings image_rendering_settings_scope(
        context, InterpolationQualityForCanvas(layout_html_canvas_.StyleRef()), layout_html_canvas_.StyleRef().GetDynamicRangeLimit());
