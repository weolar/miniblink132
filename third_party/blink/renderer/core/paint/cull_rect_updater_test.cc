...
protected:
    ...
    {
        return GetLayoutObjectByElementId(id)->FirstFragment().GetContentsCullRect();
    }
...
TEST_F(CullRectUpdaterTest, VerticalRLWritingModeScrollDiv)
{
    ...
    EXPECT_EQ(gfx::Rect(800, 0, 8200, 200), GetContentsCullRect("scroller").Rect());
}
...
TEST_F(CullRectUpdaterTest, ScaledAndRotatedCullRect)
{
    ...
    // to local).
    EXPECT_EQ(gfx::Rect(-6748, -6836, 14236, 14236), GetCullRect("target").Rect());
}
...
TEST_F(CullRectUpdaterTest, ScaledAndRotatedCullRectUnderCompositedScroller)
{
    ...
    // The expansion 6599 is 4000 * max_dimension(1x1 rect projected from screen
    // to local).
    EXPECT_EQ(gfx::Rect(-6599, -6599, 16697, 16797), GetCullRect("target").Rect());
    EXPECT_EQ(gfx::Rect(-6599, -6599, 16697, 16797), GetContentsCullRect("target").Rect());
}
...
TEST_F(CullRectUpdaterTest, OptimizeNonCompositedTransformUpdate)
{
    ...
    GetDocument().getElementById(AtomicString("target"))->setAttribute(html_names::kStyleAttr, AtomicString("transform: rotate(10deg);"));
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(CullRectUpdaterTest, PerspectiveCullRect)
{
    ...
    EXPECT_TRUE(GetCullRect("target").Rect().Contains(gfx::Rect(0, 0, 2000, 3000)));
}
...
TEST_F(CullRectUpdaterTest, LayerOffscreenFarCullRect)
{
    ...
    EXPECT_FALSE(GetCullRect("target").Rect().Intersects(gfx::Rect(0, 0, 200, 300)));
}
...
TEST_F(CullRectUpdaterTest, TallScrolledLayerCullRect)
{
    ...
    GetDocument().View()->LayoutViewport()->SetScrollOffset(ScrollOffset(0, 4000), mojom::blink::ScrollType::kProgrammatic);
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(gfx::Rect(0, 0, 800, 8600), GetCullRect("target").Rect());

    GetDocument().View()->LayoutViewport()->SetScrollOffset(ScrollOffset(0, 4500), mojom::blink::ScrollType::kProgrammatic);
    UpdateAllLifecyclePhasesForTest();
    // Used the previous cull rect because the scroll amount is small.
    EXPECT_EQ(gfx::Rect(0, 0, 800, 8600), GetCullRect("target").Rect());

    GetDocument().View()->LayoutViewport()->SetScrollOffset(ScrollOffset(0, 4600), mojom::blink::ScrollType::kProgrammatic);
    UpdateAllLifecyclePhasesForTest();
    // Used new cull rect.
...
TEST_F(CullRectUpdaterTest, LayerUnderSVGHiddenContainer)
{
    ...
    GetDocument().getElementById(AtomicString("defs"))->appendChild(GetDocument().getElementById(AtomicString("div")));
...
TEST_F(CullRectUpdaterTest, StickyPositionInCompositedScroller)
{
    ...
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(gfx::Rect(0, 3200, 300, 7650), GetContentsCullRect("scroller").Rect());
...
TEST_F(CullRectUpdaterTest, NestedOverriddenCullRectScopes)
{
    ...
    {
        OverriddenCullRectScope scope1(layer1, special_cull_rect1, disable_expansion);
        {
            OverriddenCullRectScope scope2(layer2, cull_rect2, disable_expansion);
            EXPECT_EQ(cull_rect2, GetCullRect(layer2));
        }
...

    {
        OverriddenCullRectScope scope1(layer1, cull_rect1, disable_expansion);
        {
            OverriddenCullRectScope scope2(layer2, special_cull_rect2, disable_expansion);
...
TEST_F(CullRectUpdaterTest, OverriddenCullRectWithoutExpansion)
{
...

    {
        ...
        OverriddenCullRectScope scope(*GetLayoutView().Layer(), CullRect(gfx::Rect(100, 100, 400, 400)), disable_expansion);
        ...
        EXPECT_EQ(gfx::Rect(100, 100, 200, 200), GetContentsCullRect(scroller).Rect());
    }
...
TEST_F(CullRectUpdaterTest, ViewScrollNeedsCullRectUpdate)
{
    ...
    EXPECT_EQ(gfx::PointF(), layer.GetScrollableArea()->LastCullRectUpdateScrollPosition());
    ...
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(gfx::PointF(0, 600), layer.GetScrollableArea()->LastCullRectUpdateScrollPosition());
...
TEST_F(CullRectUpdaterTest, InputDoesntExpandCullRect)
{
    ...
    const LayoutObject* editor = GetLayoutObjectByElementId("input")->SlowFirstChild();
    ASSERT_TRUE(editor);
...
protected:
    void Check(const String& old_style, const String& new_style, bool expected_needs_repaint, bool expected_needs_cull_rect_update,
        bool expected_needs_repaint_after_cull_rect_update)
    {
        UpdateAllLifecyclePhasesExceptPaint(/*update_cull_rects*/ false);
        const auto* target_layer = GetPaintLayerByElementId("target");
        EXPECT_EQ(expected_needs_repaint, target_layer->SelfNeedsRepaint()) << old_style << " -> " << new_style;
        EXPECT_EQ(expected_needs_cull_rect_update, target_layer->NeedsCullRectUpdate()) << old_style << " -> " << new_style;
        UpdateCullRects();
        EXPECT_EQ(expected_needs_repaint_after_cull_rect_update, target_layer->SelfNeedsRepaint()) << old_style << " -> " << new_style;
    }

    void TestTargetChange(const char* old_style, const char* new_style, bool expected_needs_repaint, bool expected_needs_cull_rect_update,
        bool expected_needs_repaint_after_cull_rect_update)
    {
        ...
        Check(old_style, new_style, expected_needs_repaint, expected_needs_cull_rect_update, expected_needs_repaint_after_cull_rect_update);
    }

    void TestChildChange(const char* old_style, const char* new_style, bool expected_needs_repaint, bool expected_needs_cull_rect_update,
        bool expected_needs_repaint_after_cull_rect_update)
    {
        SetBodyInnerHTML(html_);
    ...
    void TestTargetScroll(const ScrollOffset& old_scroll_offset, const ScrollOffset& new_scroll_offset, bool expected_needs_repaint,
        bool expected_needs_cull_rect_update, bool expected_needs_repaint_after_cull_rect_update)
    {
        SetBodyInnerHTML(html_);
        auto* target = GetDocument().getElementById(AtomicString("target"));
        target->scrollTo(old_scroll_offset.x(), old_scroll_offset.y()), UpdateAllLifecyclePhasesForTest();
        target->scrollTo(new_scroll_offset.x(), new_scroll_offset.y()),
            Check(String(old_scroll_offset.ToString()), String(new_scroll_offset.ToString()), expected_needs_repaint, expected_needs_cull_rect_update,
...
TEST_F(CullRectUpdateOnPaintPropertyChangeTest, Opacity)
{
    ...
    TestTargetChange("", "opacity: 0.5", true, false, true);
    TestTargetChange("will-change: opacity", "will-change: opacity; opacity: 0.5", false, false, false);
    TestTargetChange("will-change: opacity; opacity: 0.5", "will-change: opacity", false, false, false);
}

TEST_F(CullRectUpdateOnPaintPropertyChangeTest, NonPixelMovingFilter)
{
    TestTargetChange("filter: invert(5%)", "filter: invert(8%)", false, false, false);
    TestTargetChange("filter: invert(5%)", "", true, false, true);
    TestTargetChange("", "filter: invert(5%)", true, false, true);
    TestTargetChange("will-change: filter; filter: invert(5%)", "will-change: filter", false, false, false);
    TestTargetChange("will-change: filter", "will-change: filter; filter: invert(5%)", false, false, false);
}

TEST_F(CullRectUpdateOnPaintPropertyChangeTest, PixelMovingFilter)
{
    TestTargetChange("filter: blur(5px)", "filter: blur(8px)", false, false, false);
    TestTargetChange("filter: blur(5px)", "", true, true, true);
    TestTargetChange("", "filter: blur(5px)", true, true, true);
    TestTargetChange("will-change: filter; filter: blur(5px)", "will-change: filter", true, false, true);
    TestTargetChange("will-change: filter", "will-change: filter; filter: blur(5px)", true, false, true);
}
...
TEST_F(CullRectUpdateOnPaintPropertyChangeTest, Transform)
{
    ...
    TestTargetChange("transform: translateX(10px)", "transform: translateX(20px)", false, false, false);
    ...
    TestTargetChange("will-change: transform; transform: translateX(10px)", "will-change: transform; transform: translateX(20px)", false, true, false);
    TestTargetChange("will-change: transform; transform: translateX(10px)", "will-change: transform", false, true, false);
    TestTargetChange("will-change: transform", "will-change: transform; transform: translateX(10px)", false, true, false);
}
...
TEST_F(CullRectUpdateOnPaintPropertyChangeTest, SmallContentsScroll)
{
    ...
    TestTargetScroll(ScrollOffset(), ScrollOffset(100, 200), false, needs_cull_rect_update, false);
    TestTargetScroll(ScrollOffset(100, 200), ScrollOffset(1000, 1000), false, needs_cull_rect_update, false);
    TestTargetScroll(ScrollOffset(1000, 1000), ScrollOffset(), false, needs_cull_rect_update, false);
}

TEST_F(CullRectUpdateOnPaintPropertyChangeTest, LargeContentsScrollSmallDeltaOrNotExposingNewContents1)
{
    ...
    bool needs_cull_rect_update = false;
    TestTargetScroll(ScrollOffset(), ScrollOffset(0, 200), false, needs_cull_rect_update, false);
    TestTargetScroll(ScrollOffset(0, 200), ScrollOffset(), false, needs_cull_rect_update, false);
    TestTargetScroll(ScrollOffset(0, 2000), ScrollOffset(), false, needs_cull_rect_update, false);
    TestTargetScroll(ScrollOffset(0, 7000), ScrollOffset(0, 8000), false, needs_cull_rect_update, false);
}

TEST_F(CullRectUpdateOnPaintPropertyChangeTest, LargeContentsScrollSmallDeltaOrNotExposingNewContents2)
{
    html_ = html_ + "<style>#child { width: 10000px; height: 10000px; }</style>";
    // Scroll offset changes that are small or won't expose new contents don't
    // need cull rect update.
    bool needs_cull_rect_update = false;
    TestTargetScroll(ScrollOffset(), ScrollOffset(200, 200), false, needs_cull_rect_update, false);
    TestTargetScroll(ScrollOffset(200, 200), ScrollOffset(), false, needs_cull_rect_update, false);
    TestTargetScroll(ScrollOffset(2000, 2000), ScrollOffset(), false, needs_cull_rect_update, false);
    TestTargetScroll(ScrollOffset(7000, 7000), ScrollOffset(7500, 7500), false, needs_cull_rect_update, false);
}

TEST_F(CullRectUpdateOnPaintPropertyChangeTest, LargeContentsScrollExposingNewContents)
{
    html_ = html_ + "<style>#child { width: 10000px; height: 10000px; }</style>";
    // Big scroll offset changes that will expose new contents to paint need cull
    // rect update.
    TestTargetScroll(ScrollOffset(100, 200), ScrollOffset(100, 800), false, true, true);
    TestTargetScroll(ScrollOffset(100, 800), ScrollOffset(700, 800), false, true, true);
    TestTargetScroll(ScrollOffset(700, 800), ScrollOffset(1700, 1800), false, true, true);
    TestTargetScroll(ScrollOffset(8000, 8000), ScrollOffset(0, 8000), false, true, true);
