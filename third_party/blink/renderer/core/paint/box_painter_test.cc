...
TEST_P(BoxPainterTest, EmptyDecorationBackground)
{
    ...
    EXPECT_THAT(ContentDisplayItems(),
        ElementsAre(VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM, IsSameId(div1->Id(), kBackgroundType),
    ...
    EXPECT_THAT(ContentPaintChunks(),
        ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK_COMMON,
            ...
            IsPaintChunk(1, 3, PaintChunk::Id(body->Layer()->Id(), DisplayItem::kLayerChunk), body->FirstFragment().LocalBorderBoxProperties(), nullptr,
                gfx::Rect(-2, 0, 202, 350))));
}
...
TEST_P(BoxPainterTest, ScrollHitTestOrderWithScrollBackgroundAttachment)
{
    ...
    EXPECT_THAT(ContentDisplayItems(),
        ElementsAre(VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM, IsSameId(container.Id(), kBackgroundType), IsSameId(child.Id(), kBackgroundType)));
    ...
    EXPECT_THAT(ContentPaintChunks(),
        ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK_COMMON,
            IsPaintChunk(1, 2, PaintChunk::Id(container.Layer()->Id(), DisplayItem::kLayerChunk), container.FirstFragment().LocalBorderBoxProperties()),
...
TEST_P(BoxPainterTest, ScrollHitTestOrderWithLocalBackgroundAttachment)
{
    ...
    auto& container = *GetLayoutBoxByElementId("container");
    auto& child = *GetLayoutObjectByElementId("child");
    auto* container_scrolling_client = &container.GetScrollableArea()->GetScrollingBackgroundDisplayItemClient();
    ...
    EXPECT_THAT(ContentDisplayItems(),
        ElementsAre(
            VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM, IsSameId(container_scrolling_client->Id(), kBackgroundType), IsSameId(child.Id(), kBackgroundType)));
    auto* scroll_hit_test = MakeGarbageCollected<HitTestData>();
    scroll_hit_test->scroll_translation = container.FirstFragment().PaintProperties()->ScrollTranslation();
    scroll_hit_test->scroll_hit_test_rect = gfx::Rect(0, 0, 200, 200);
    EXPECT_THAT(ContentPaintChunks(),
        ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK_COMMON,
            IsPaintChunk(1, 1, PaintChunk::Id(container.Layer()->Id(), DisplayItem::kLayerChunk), container.FirstFragment().LocalBorderBoxProperties()),
            IsPaintChunk(1, 1, PaintChunk::Id(container.Id(), DisplayItem::kScrollHitTest), container.FirstFragment().LocalBorderBoxProperties(),
                scroll_hit_test, gfx::Rect(0, 0, 200, 200)),
            IsPaintChunk(1, 3, PaintChunk::Id(container.Id(), kScrollingBackgroundChunkType), container.FirstFragment().ContentsProperties())));
}
...
TEST_P(BoxPainterTest, ScrollHitTestProperties)
{
    ...
    // scrolled contents.
    EXPECT_EQ(kBackgroundPaintInBorderBoxSpace, container.GetBackgroundPaintLocation());
    EXPECT_THAT(ContentDisplayItems(),
        ElementsAre(VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM, IsSameId(container.Id(), kBackgroundType), IsSameId(child.Id(), kBackgroundType)));

    auto* scroll_hit_test_data = MakeGarbageCollected<HitTestData>();
    const auto& scrolling_contents_properties = container.FirstFragment().ContentsProperties();
    scroll_hit_test_data->scroll_translation = container.FirstFragment().PaintProperties()->ScrollTranslation();
    scroll_hit_test_data->scroll_hit_test_rect = gfx::Rect(0, 0, 200, 200);
    EXPECT_THAT(paint_chunks,
        ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK_COMMON,
            IsPaintChunk(1, 2, PaintChunk::Id(container.Id(), kBackgroundChunkType), container.FirstFragment().LocalBorderBoxProperties()),
            IsPaintChunk(2, 2, PaintChunk::Id(container.Id(), DisplayItem::kScrollHitTest), container.FirstFragment().LocalBorderBoxProperties(),
    ...
    const auto& root_transform = ToUnaliased(paint_chunks[0].properties.Transform());
    ...
    // transform.
    const auto& container_transform = ToUnaliased(paint_chunks[1].properties.Transform());
    ...
    const auto& scroll_hit_test_transform = ToUnaliased(scroll_hit_test_chunk.properties.Transform());
    EXPECT_EQ(nullptr, scroll_hit_test_transform.ScrollNode());
    EXPECT_EQ(&root_transform, scroll_hit_test_transform.Parent());
    const auto& scroll_hit_test_clip = ToUnaliased(scroll_hit_test_chunk.properties.Clip());
    EXPECT_EQ(gfx::RectF(0, 0, 800, 600), scroll_hit_test_clip.PaintClipRect().Rect());

    // The scrolled contents should be scrolled and clipped.
    const auto& contents_chunk = paint_chunks[3];
    const auto& contents_transform = ToUnaliased(contents_chunk.properties.Transform());
    ...
    EXPECT_EQ(&contents_transform, scroll_hit_test_chunk.hit_test_data->scroll_translation);
}
...
size_t CountDrawImagesWithConstraint(const cc::PaintRecord& record, SkCanvas::SrcRectConstraint constraint)
{
    size_t count = 0;
...
TEST_P(BoxPainterTest, ImageClampingMode)
{
    ...
    EXPECT_EQ(1U, CountDrawImagesWithConstraint(record, SkCanvas::kFast_SrcRectConstraint));
}
