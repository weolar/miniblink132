...
public:
    ...
    ScopedPaintState(const LayoutObject& object, const PaintInfo& paint_info)
        : ScopedPaintState(object, paint_info, DetermineFragmentToPaint(object, paint_info))
    {
    }

    ScopedPaintState(const PhysicalBoxFragment& fragment, const PaintInfo& paint_info)
        : ScopedPaintState(*fragment.GetLayoutObject(), paint_info, fragment.GetFragmentData())
    {
    }
    ...
    PhysicalOffset PaintOffset() const
    {
        return paint_offset_;
    }

    const FragmentData* FragmentToPaint() const
    {
...
protected:
    ...
    ScopedPaintState(const PaintInfo& paint_info, const PhysicalOffset& paint_offset, const LayoutObject& object, const FragmentData* fragment_data)
    ...
    const FragmentData* DetermineFragmentToPaint(const LayoutObject& object, const PaintInfo& paint_info)
    {
...
public:
    ScopedBoxContentsPaintState(const ScopedPaintState& input, const LayoutBox& box)
        : ScopedPaintState(input)
    {
        AdjustForBoxContents(box);
    }

    ScopedBoxContentsPaintState(const PaintInfo& paint_info, const PhysicalOffset& paint_offset, const LayoutBox& box, const FragmentData* fragment_data)
