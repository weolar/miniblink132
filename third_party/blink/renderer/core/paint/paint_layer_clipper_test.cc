...
TEST_F(PaintLayerClipperTest, ParentBackgroundClipRectSubpixelAccumulation)
{
    ...
    ClipRectsContext context(GetDocument().GetLayoutView()->Layer(), &GetDocument().GetLayoutView()->FirstFragment(), kIgnoreOverlayScrollbarSize,
        kIgnoreOverflowClip, PhysicalOffset(LayoutUnit(0.25), LayoutUnit(0.35)));

    ClipRect background_rect_gm;
    target_paint_layer->Clipper().CalculateBackgroundClipRect(context, background_rect_gm);

    EXPECT_EQ(PhysicalRect(LayoutUnit(8.25), LayoutUnit(8.34375), LayoutUnit(300), LayoutUnit(300)), background_rect_gm.Rect());
}
...
TEST_F(PaintLayerClipperTest, LayoutSVGRoot)
{
    ...
    target_paint_layer->Clipper().CalculateRects(
        context, target_paint_layer->GetLayoutObject().FirstFragment(), layer_offset, background_rect, foreground_rect);

    EXPECT_EQ(PhysicalRect(LayoutUnit(8.25), LayoutUnit(8.35), LayoutUnit(200), LayoutUnit(300)), background_rect.Rect());
    EXPECT_EQ(PhysicalRect(LayoutUnit(8.25), LayoutUnit(8.35), LayoutUnit(200), LayoutUnit(300)), foreground_rect.Rect());
...
TEST_F(PaintLayerClipperTest, ControlClip)
{
    ...
    ClipRectsContext context(
        GetDocument().GetLayoutView()->Layer(), &GetDocument().GetLayoutView()->FirstFragment(), kIgnoreOverlayScrollbarSize, kIgnoreOverflowClip);
    PhysicalOffset layer_offset;
    ClipRect background_rect, foreground_rect;
...
TEST_F(PaintLayerClipperTest, RoundedClipNested)
{
    ...
    ClipRectsContext context(parent_paint_layer, &parent_paint_layer->GetLayoutObject().FirstFragment());

    PhysicalOffset layer_offset;
    ClipRect background_rect, foreground_rect;

    child_paint_layer->Clipper().CalculateRects(context, child_paint_layer->GetLayoutObject().FirstFragment(), layer_offset, background_rect, foreground_rect);
...
TEST_F(PaintLayerClipperTest, ControlClipSelect)
{
    ...
    EXPECT_GT(foreground_rect.Rect().X(), content_box_rect.X() + target->PhysicalLocation().left);
...
TEST_F(PaintLayerClipperTest, LayoutSVGRootChild)
{
    ...
    PaintLayer* target_paint_layer = GetPaintLayerByElementId("target");
    ClipRectsContext context(GetDocument().GetLayoutView()->Layer(), &GetDocument().GetLayoutView()->FirstFragment());
    PhysicalOffset layer_offset;
    ClipRect background_rect, foreground_rect;
...
TEST_F(PaintLayerClipperTest, ContainPaintClip)
{
    ...
    ClipRectsContext context(layer, &layer->GetLayoutObject().FirstFragment(), kIgnoreOverlayScrollbarSize, kIgnoreOverflowClip);
    PhysicalOffset layer_offset;
    ClipRect background_rect, foreground_rect;

    layer->Clipper().CalculateRects(context, layer->GetLayoutObject().FirstFragment(), layer_offset, background_rect, foreground_rect);
    EXPECT_TRUE(background_rect.IsInfinite()) << background_rect;
    EXPECT_EQ(background_rect.Rect(), foreground_rect.Rect());
    EXPECT_EQ(PhysicalOffset(), layer_offset);

    ClipRectsContext context_clip(layer, &layer->GetLayoutObject().FirstFragment());

    layer->Clipper().CalculateRects(context_clip, layer->GetLayoutObject().FirstFragment(), layer_offset, background_rect, foreground_rect);
...
TEST_F(PaintLayerClipperTest, NestedContainPaintClip)
{
    ...
    PaintLayer* layer = GetPaintLayerByElementId("target");
    ClipRectsContext context(layer->Parent(), &layer->Parent()->GetLayoutObject().FirstFragment(), kIgnoreOverlayScrollbarSize, kIgnoreOverflowClip);
    PhysicalOffset layer_offset;
    ClipRect background_rect, foreground_rect;
    ...
    ClipRectsContext context_clip(layer->Parent(), &layer->Parent()->GetLayoutObject().FirstFragment());
...
TEST_F(PaintLayerClipperTest, CSSClip)
{
    ...
    target->Clipper().CalculateRects(context, target->GetLayoutObject().FirstFragment(), layer_offset, background_rect, foreground_rect);
...
TEST_F(PaintLayerClipperTest, Filter)
{
    ...
    ClipRectsContext root_context(GetLayoutView().Layer(), &GetLayoutView().FirstFragment());
    background_rect = infinite_rect;
    foreground_rect = infinite_rect;
    target->Clipper().CalculateRects(root_context, target->GetLayoutObject().FirstFragment(), layer_offset, background_rect, foreground_rect);
...
TEST_F(PaintLayerClipperTest, IgnoreRootLayerClipWithCSSClip)
{
    ...
    PaintLayer* target = GetPaintLayerByElementId("target");
    ClipRectsContext context(root, &root->GetLayoutObject().FirstFragment(), kIgnoreOverlayScrollbarSize, kIgnoreOverflowClip);
    PhysicalRect infinite_rect(InfiniteIntRect());
    PhysicalOffset layer_offset = infinite_rect.offset;
    ClipRect background_rect(infinite_rect);
    ClipRect foreground_rect(infinite_rect);
...
TEST_F(PaintLayerClipperTest, Fragmentation)
{
    ...
    ClipRectsContext context(root_paint_layer, &root_paint_layer->GetLayoutObject().FirstFragment(), kIgnoreOverlayScrollbarSize);
    PhysicalOffset layer_offset;
    ClipRect background_rect, foreground_rect;
    ...
    target_paint_layer->Clipper().CalculateRects(context, *second_fragment, layer_offset, background_rect, foreground_rect);
...
TEST_F(PaintLayerClipperTest, ScrollbarClipBehaviorChild)
{
    ...
    ClipRectsContext context(parent_paint_layer, &parent_paint_layer->GetLayoutObject().FirstFragment(), kExcludeOverlayScrollbarSizeForHitTesting);
...
TEST_F(PaintLayerClipperTest, ScrollbarClipBehaviorChildScrollBetween)
{
    ...
    PaintLayer* child_paint_layer = GetPaintLayerByElementId("child");
    parent->GetLayoutBox()->GetScrollableArea()->SetScrollbarsHiddenIfOverlay(false);
    UpdateAllLifecyclePhasesForTest();

    ClipRectsContext context(root_paint_layer, &root_paint_layer->GetLayoutObject().FirstFragment(), kExcludeOverlayScrollbarSizeForHitTesting);
...
TEST_F(PaintLayerClipperTest, ScrollbarClipBehaviorParent)
{
    ...
    parent_paint_layer->Clipper().CalculateRects(
        context, parent_paint_layer->GetLayoutObject().FirstFragment(), layer_offset, background_rect, foreground_rect);
