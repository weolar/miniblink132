...
void ObjectPainter::PaintOutline(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    ...
    if (!style_to_use.HasOutline() || style_to_use.Visibility() != EVisibility::kVisible) {
        return;
    }

    // Only paint the focus ring by hand if the theme isn't able to draw the focus
    // ring.
    if (style_to_use.OutlineStyleIsAuto() && !LayoutTheme::GetTheme().ShouldDrawDefaultFocusRing(layout_object_.GetNode(), style_to_use)) {
        return;
    }

    LayoutObject::OutlineInfo info;
    auto outline_rects = layout_object_.OutlineRects(&info, paint_offset, style_to_use.OutlineRectsShouldIncludeBlockInkOverflow());
    if (outline_rects.empty())
        return;

    OutlinePainter::PaintOutlineRects(paint_info, layout_object_, outline_rects, info, style_to_use);
}
...
void ObjectPainter::PaintInlineChildrenOutlines(const PaintInfo& paint_info)
{
    ...
    for (LayoutObject* child = layout_object_.SlowFirstChild(); child; child = child->NextSibling()) {
        if (child->IsLayoutInline() && !To<LayoutInline>(child)->HasSelfPaintingLayer())
            child->Paint(paint_info_for_descendants);
    }
}

void ObjectPainter::AddURLRectIfNeeded(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    DCHECK(paint_info.ShouldAddUrlMetadata());
    if (!layout_object_.GetNode() || !layout_object_.GetNode()->IsLink() || layout_object_.StyleRef().Visibility() != EVisibility::kVisible) {
        return;
    }
    ...
    auto outline_rects = layout_object_.OutlineRects(nullptr, paint_offset, OutlineType::kIncludeBlockInkOverflow);
    gfx::Rect bounding_rect = ToPixelSnappedRect(UnionRect(outline_rects));
    if (bounding_rect.IsEmpty()) {
        return;
    }

    if (DrawingRecorder::UseCachedDrawingIfPossible(paint_info.context, layout_object_, DisplayItem::kPrintedContentPDFURLRect))
        return;

    DrawingRecorder recorder(paint_info.context, layout_object_, DisplayItem::kPrintedContentPDFURLRect, bounding_rect);
...
void ObjectPainter::PaintAllPhasesAtomically(const PaintInfo& paint_info)
{
    ...
    if (paint_info.phase == PaintPhase::kSelectionDragImage || paint_info.phase == PaintPhase::kTextClip) {
...
void ObjectPainter::RecordHitTestData(const PaintInfo& paint_info, const gfx::Rect& paint_rect, const DisplayItemClient& background_client)
{
    ...
    if (!RuntimeEnabledFeatures::HitTestOpaquenessEnabled() && paint_info.IsPaintingBackgroundInContentsSpace()
    ...
    paint_info.context.GetPaintController().RecordHitTestData(
        background_client, paint_rect, layout_object_.EffectiveAllowedTouchAction(), layout_object_.InsideBlockingWheelEventHandler(), GetHitTestOpaqueness());
}
...
cc::HitTestOpaqueness ObjectPainter::GetHitTestOpaqueness() const
{
    ...
    if (!layout_object_.VisibleToHitTesting() || !layout_object_.GetFrame()->GetVisibleToHitTesting()) {
...
bool ObjectPainter::ShouldRecordSpecialHitTestData(const PaintInfo& paint_info)
{
    ...
    if (RuntimeEnabledFeatures::HitTestOpaquenessEnabled()) {
        ...
        if (paint_info.context.GetPaintController().CurrentChunkIsNonEmptyAndTransparentToHitTest()) {
