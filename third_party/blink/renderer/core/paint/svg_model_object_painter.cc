...
void SVGModelObjectPainter::RecordHitTestData(const LayoutObject& svg_object, const PaintInfo& paint_info)
{
    DCHECK(svg_object.IsSVGChild());
    DCHECK_EQ(paint_info.phase, PaintPhase::kForeground);
    ObjectPainter(svg_object).RecordHitTestData(paint_info, gfx::ToEnclosingRect(svg_object.VisualRectInLocalSVGCoordinates()), svg_object);
}

void SVGModelObjectPainter::RecordRegionCaptureData(const LayoutObject& svg_object, const PaintInfo& paint_info)
{
    DCHECK(svg_object.IsSVGChild());
    ...
    if (element) {
        ...
        if (crop_id) {
            paint_info.context.GetPaintController().RecordRegionCaptureData(
                svg_object, *crop_id, gfx::ToEnclosingRect(svg_object.VisualRectInLocalSVGCoordinates()));
        }
    }
}
...
void SVGModelObjectPainter::PaintOutline(const PaintInfo& paint_info)
{
    ...
    ObjectPainter(layout_svg_model_object_).PaintOutline(outline_paint_info, PhysicalOffset::FromPointFRound(visual_rect.origin()));
}
