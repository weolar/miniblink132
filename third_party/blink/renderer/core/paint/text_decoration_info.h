...
enum class ResolvedUnderlinePosition { kNearAlphabeticBaselineAuto, kNearAlphabeticBaselineFromFont, kUnder, kOver };
...
public:
    TextDecorationInfo(LineRelativeOffset local_origin, LayoutUnit width, const ComputedStyle& target_style, const InlinePaintContext* inline_context,
        const TextDecorationLine selection_decoration_line, const Color selection_decoration_color, const AppliedTextDecoration* decoration_override = nullptr,
        const Font* font_override = nullptr, MinimumThickness1 minimum_thickness1 = MinimumThickness1(true), float scaling_factor = 1.0f);

    wtf_size_t AppliedDecorationCount() const;
    const AppliedTextDecoration& AppliedDecoration(wtf_size_t) const;
    bool HasDecorationOverride() const
    {
...
private:
    ...
    bool Has(TextDecorationLine line) const
    {
        return EnumHasFlags(lines_, line);
    }
...
public:
    ...
    {
        return has_underline_;
    }
    bool HasOverline() const
    {
        return has_overline_;
    }
    bool HasLineThrough() const
    {
        return Has(TextDecorationLine::kLineThrough);
    }
    ...
    const ComputedStyle& TargetStyle() const
    {
        return target_style_;
    }
    float TargetAscent() const
    {
        return target_ascent_;
    }
...
private:
    ...
    float ComputeUnderlineThickness(const TextDecorationThickness& applied_decoration_thickness, const ComputedStyle* decorating_box_style) const;
    void ComputeWavyLineData(gfx::RectF& pattern_rect, cc::PaintRecord& tile_record) const;
    ...
    {
        return line_data_.line == TextDecorationLine::kSpellingError || line_data_.line == TextDecorationLine::kGrammarError;
    }
    ...
    const TextDecorationLine selection_decoration_line_ = TextDecorationLine::kNone;
    ...
    ResolvedUnderlinePosition original_underline_position_ = ResolvedUnderlinePosition::kNearAlphabeticBaselineAuto;
    ResolvedUnderlinePosition flipped_underline_position_ = ResolvedUnderlinePosition::kNearAlphabeticBaselineAuto;
