...
public:
    TextCombinePainter(GraphicsContext& context, const SvgContextPaints* svg_context_paints, const gfx::Rect& visual_rect, const ComputedStyle& style,
        const LineRelativeOffset& text_origin);
    ~TextCombinePainter();

    static void Paint(const PaintInfo& paint_info, const PhysicalOffset& paint_offset, const LayoutTextCombine& text_combine);

    static bool ShouldPaint(const LayoutTextCombine& text_combine);

protected:
    void ClipDecorationsStripe(const TextFragmentPaintInfo&, float upper, float stripe_width, float dilation) override;
...
private:
    ...
    void PaintEmphasisMark(const TextPaintStyle& text_style, const Font& emphasis_mark_font);
