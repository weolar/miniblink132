...
bool ShouldPaintBorderAfter(const Vector<bool>& allow_border, wtf_size_t index)
{
...
void FrameSetPainter::PaintObject(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
...
void FrameSetPainter::PaintChildren(const PaintInfo& paint_info)
{
    ...
    for (const PhysicalFragmentLink& link : box_fragment_.Children()) {
        ...
        if (To<PhysicalBoxFragment>(child_fragment).CanTraverse()) {
            BoxFragmentPainter(To<PhysicalBoxFragment>(child_fragment)).Paint(paint_info);
        } else {
...
void FrameSetPainter::PaintBorders(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    if (DrawingRecorder::UseCachedDrawingIfPossible(paint_info.context, display_item_client_, paint_info.phase))
        return;

    DrawingRecorder recorder(
        paint_info.context, display_item_client_, paint_info.phase, BoxPainter(*To<LayoutBox>(box_fragment_.GetLayoutObject())).VisualRect(paint_offset));
    ...
    Color border_fill_color = layout_data->has_border_color ? style.VisitedDependentColor(GetCSSPropertyBorderLeftColor()) : kBorderFillColor;
    auto auto_dark_mode = PaintAutoDarkMode(style, DarkModeFilter::ElementRole::kBackground);
    ...
    for (wtf_size_t row = 0; row < row_sizes.size(); ++row) {
        ...
        for (wtf_size_t col = 0; col < col_sizes.size(); ++col) {
            ...
            if (ShouldPaintBorderAfter(layout_data->col_allow_border, col)) {
                gfx::Rect rect
                    = ToPixelSnappedRect(PhysicalRect(paint_offset.left + x, paint_offset.top + y, border_thickness, box_fragment_.Size().height - y));
        ...
        if (ShouldPaintBorderAfter(layout_data->row_allow_border, row)) {
            gfx::Rect rect = ToPixelSnappedRect(PhysicalRect(paint_offset.left, paint_offset.top + y, box_fragment_.Size().width, border_thickness));
...
void FrameSetPainter::PaintRowBorder(const PaintInfo& paint_info, const gfx::Rect& border_rect, const Color& fill_color, const AutoDarkMode& auto_dark_mode)
{
    // Fill first.
    ...
    if (border_rect.height() < 3)
        return;
    context.FillRect(gfx::Rect(border_rect.origin(), gfx::Size(border_rect.width(), 1)), kBorderStartEdgeColor, auto_dark_mode);
    context.FillRect(gfx::Rect(border_rect.x(), border_rect.bottom() - 1, border_rect.width(), 1), kBorderEndEdgeColor, auto_dark_mode);
}

void FrameSetPainter::PaintColumnBorder(const PaintInfo& paint_info, const gfx::Rect& border_rect, const Color& fill_color, const AutoDarkMode& auto_dark_mode)
{
    ...
    if (border_rect.width() < 3)
        return;
    context.FillRect(gfx::Rect(border_rect.origin(), gfx::Size(1, border_rect.height())), kBorderStartEdgeColor, auto_dark_mode);
    context.FillRect(gfx::Rect(border_rect.right() - 1, border_rect.y(), 1, border_rect.height()), kBorderEndEdgeColor, auto_dark_mode);
}
