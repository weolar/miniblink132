...
const TransformPaintPropertyNode* PaintPropertyTreeBuilderTest::DocPreTranslation(const Document* document)
{
    if (!document)
        document = &GetDocument();
    return document->GetLayoutView()->FirstFragment().PaintProperties()->PaintOffsetTranslation();
}

const TransformPaintPropertyNode* PaintPropertyTreeBuilderTest::DocScrollTranslation(const Document* document)
{
    if (!document)
        document = &GetDocument();
    return document->GetLayoutView()->FirstFragment().PaintProperties()->ScrollTranslation();
}

const ClipPaintPropertyNode* PaintPropertyTreeBuilderTest::DocContentClip(const Document* document)
{
    if (!document)
        document = &GetDocument();
    return document->GetLayoutView()->FirstFragment().PaintProperties()->OverflowClip();
}

const ScrollPaintPropertyNode* PaintPropertyTreeBuilderTest::DocScroll(const Document* document)
{
    if (!document)
        document = &GetDocument();
...
const EffectPaintPropertyNode* PaintPropertyTreeBuilderTest::DocEffect(const Document* document)
{
    if (!document) {
        document = &GetDocument();
    }
    return document->GetLayoutView()->FirstFragment().PaintProperties()->ViewTransitionEffect();
}

const ObjectPaintProperties* PaintPropertyTreeBuilderTest::PaintPropertiesForElement(const char* name)
{
    return GetDocument().getElementById(AtomicString(name))->GetLayoutObject()->FirstFragment().PaintProperties();
}

const GeometryMapperTransformCache& PaintPropertyTreeBuilderTest::GetTransformCache(const TransformPaintPropertyNode& transform)
{
...
#define CHECK_VISUAL_RECT(expected, source_object, ancestor, slop_factor)                                                                                      \
    do {                                                                                                                                                       \
        if ((source_object)->HasLayer() && (ancestor)->HasLayer()) {                                                                                           \
            ...
            (source_object)->MapToVisualRectInAncestorSpace(ancestor, actual, kUseGeometryMapper);                                                             \
        ...
        if (slop_factor) {                                                                                                                                     \
            ...
            SCOPED_TRACE(String::Format("Slow path rect: %s, Expected: %s, Inflated expected: %s", slow_path_rect.ToString().Ascii().c_str(),                  \
                expected.ToString().Ascii().c_str(), inflated_expected.ToString().Ascii().c_str()));                                                           \
            EXPECT_TRUE(PhysicalRect(ToEnclosingRect(slow_path_rect)).Contains(expected));                                                                     \
...
#define CHECK_EXACT_VISUAL_RECT(expected, source_object, ancestor) CHECK_VISUAL_RECT(expected, source_object, ancestor, 0)

INSTANTIATE_TEST_SUITE_P(All, PaintPropertyTreeBuilderTest, ::testing::Values(0, kUnderInvalidationChecking, kElementCapture));

TEST_P(PaintPropertyTreeBuilderTest, FixedPosition)
{
    LoadTestData("fixed-position.html");

    Element* positioned_scroll = GetDocument().getElementById(AtomicString("positionedScroll"));
    positioned_scroll->setScrollTop(3);
    Element* transformed_scroll = GetDocument().getElementById(AtomicString("transformedScroll"));
    ...
    const ObjectPaintProperties* target1_properties = target1->GetLayoutObject()->FirstFragment().PaintProperties();
    EXPECT_CLIP_RECT(FloatRoundedRect(0, 0, 100, 100), target1_properties->OverflowClip());
    ...
    // positionedScroll's.
    const ObjectPaintProperties* positioned_scroll_properties = positioned_scroll->GetLayoutObject()->FirstFragment().PaintProperties();
    auto* positioned_scroll_translation = positioned_scroll_properties->ScrollTranslation();
    auto* positioned_scroll_node = positioned_scroll_translation->ScrollNode();
    EXPECT_EQ(DocScroll(), positioned_scroll_node->Parent());
    EXPECT_EQ(gfx::Vector2dF(0, -3), positioned_scroll_translation->Get2dTranslation());
    EXPECT_EQ(nullptr, target1_properties->ScrollTranslation());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(200, 150, 100, 100), target1->GetLayoutObject(), frame_view->GetLayoutView());
    ...
    const ObjectPaintProperties* target2_properties = target2->GetLayoutObject()->FirstFragment().PaintProperties();
    Element* scroller = GetDocument().getElementById(AtomicString("transformedScroll"));
    const ObjectPaintProperties* scroller_properties = scroller->GetLayoutObject()->FirstFragment().PaintProperties();
    EXPECT_CLIP_RECT(FloatRoundedRect(200, 150, 100, 100), target2_properties->OverflowClip());
    EXPECT_EQ(scroller_properties->OverflowClip(), target2_properties->OverflowClip()->Parent());
    // target2 should not have it's own scroll node and instead should inherit
    // transformedScroll's.
    const ObjectPaintProperties* transformed_scroll_properties = transformed_scroll->GetLayoutObject()->FirstFragment().PaintProperties();
    auto* transformed_scroll_translation = transformed_scroll_properties->ScrollTranslation();
    auto* transformed_scroll_node = transformed_scroll_translation->ScrollNode();
    EXPECT_EQ(DocScroll(), transformed_scroll_node->Parent());
    EXPECT_EQ(gfx::Vector2dF(0, -5), transformed_scroll_translation->Get2dTranslation());
    EXPECT_EQ(nullptr, target2_properties->ScrollTranslation());

    CHECK_EXACT_VISUAL_RECT(PhysicalRect(208, 153, 200, 100), target2->GetLayoutObject(), frame_view->GetLayoutView());
}
...
TEST_P(PaintPropertyTreeBuilderTest, PositionAndScroll)
{
    ...
    frame_view->UpdateAllLifecyclePhasesForTest();
    const ObjectPaintProperties* scroller_properties = scroller->GetLayoutObject()->FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(0, -100), scroller_properties->ScrollTranslation()->Get2dTranslation());
    EXPECT_EQ(scroller_properties->PaintOffsetTranslation(), scroller_properties->ScrollTranslation()->Parent());
    EXPECT_EQ(DocScrollTranslation(), scroller_properties->PaintOffsetTranslation()->Parent());
    EXPECT_EQ(scroller_properties->PaintOffsetTranslation(), &scroller_properties->OverflowClip()->LocalTransformSpace());
    ...
    EXPECT_EQ(gfx::Vector2dF(120, 340), scroller_properties->PaintOffsetTranslation()->Get2dTranslation());
    EXPECT_CLIP_RECT(FloatRoundedRect(0, 0, 413, 317), scroller_properties->OverflowClip());
    EXPECT_EQ(DocContentClip(), scroller_properties->OverflowClip()->Parent());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(120, 340, 413, 317), scroller->GetLayoutObject(), frame_view->GetLayoutView());
    ...
    const ObjectPaintProperties* rel_pos_properties = rel_pos->GetLayoutObject()->FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(560, 780), rel_pos_properties->PaintOffsetTranslation()->Get2dTranslation());
    EXPECT_EQ(scroller_properties->ScrollTranslation(), rel_pos_properties->PaintOffsetTranslation()->Parent());
    EXPECT_EQ(rel_pos_properties->Transform(), &rel_pos_properties->OverflowClip()->LocalTransformSpace());
    EXPECT_CLIP_RECT(FloatRoundedRect(0, 0, 100, 200), rel_pos_properties->OverflowClip());
    EXPECT_EQ(scroller_properties->OverflowClip(), rel_pos_properties->OverflowClip()->Parent());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(), rel_pos->GetLayoutObject(), frame_view->GetLayoutView());
    ...
    const ObjectPaintProperties* abs_pos_properties = abs_pos->GetLayoutObject()->FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(123, 456), abs_pos_properties->PaintOffsetTranslation()->Get2dTranslation());
    EXPECT_EQ(DocScrollTranslation(), abs_pos_properties->PaintOffsetTranslation()->Parent());
    EXPECT_EQ(abs_pos_properties->Transform(), &abs_pos_properties->OverflowClip()->LocalTransformSpace());
    EXPECT_CLIP_RECT(FloatRoundedRect(0, 0, 300, 400), abs_pos_properties->OverflowClip());
    EXPECT_EQ(DocContentClip(), abs_pos_properties->OverflowClip()->Parent());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(123, 456, 300, 400), abs_pos->GetLayoutObject(), frame_view->GetLayoutView());
}
...
TEST_P(PaintPropertyTreeBuilderTest, OverflowScrollExcludeScrollbars)
{
    ...
    EXPECT_EQ(properties->PaintOffsetTranslation(), &overflow_clip->LocalTransformSpace());
    EXPECT_EQ(FloatClipRect(gfx::RectF(10, 10, 100, 100)), overflow_clip->LayoutClipRect());

    PaintLayer* paint_layer = GetPaintLayerByElementId("scroller");
    EXPECT_TRUE(paint_layer->GetScrollableArea()->VerticalScrollbar()->IsOverlayScrollbar());

    EXPECT_EQ(FloatClipRect(gfx::RectF(10, 10, 93, 93)), overflow_clip->LayoutClipRectExcludingOverlayScrollbars());
}
...
TEST_P(PaintPropertyTreeBuilderTest, OverflowScrollExcludeScrollbarsSubpixel)
{
    ...
    EXPECT_EQ(DocContentClip(), overflow_clip->Parent());
    EXPECT_EQ(properties->PaintOffsetTranslation(), &overflow_clip->LocalTransformSpace());
    EXPECT_EQ(FloatClipRect(gfx::RectF(10, 10, 100.5, 100)), overflow_clip->LayoutClipRect());
    EXPECT_EQ(FloatRoundedRect(10, 10, 101, 100), overflow_clip->PaintClipRect());

    EXPECT_TRUE(To<LayoutBox>(scroller)->GetScrollableArea()->VerticalScrollbar()->IsOverlayScrollbar());

    EXPECT_EQ(FloatClipRect(gfx::RectF(10, 10, 93.5, 93)), overflow_clip->LayoutClipRectExcludingOverlayScrollbars());
}
...
TEST_P(PaintPropertyTreeBuilderTest, OverflowScrollVerticalRL)
{
    ...
    scroller->GetScrollableArea()->ScrollBy(ScrollOffset(-100, 0), mojom::blink::ScrollType::kUser);
...
TEST_P(PaintPropertyTreeBuilderTest, OverflowScrollVerticalRLMulticol)
{
    ...
    GetLayoutBoxByElementId("scroller")->GetScrollableArea()->ScrollBy(ScrollOffset(-100, 200), mojom::blink::ScrollType::kUser);
    UpdateAllLifecyclePhasesForTest();
    check_fragments();
}
...
TEST_P(PaintPropertyTreeBuilderTest, DocScrollingTraditional)
{
    ...
    EXPECT_EQ(GetDocument().GetPage()->GetVisualViewport().GetScrollTranslationNode(), DocPreTranslation()->Parent());
    EXPECT_EQ(gfx::Vector2dF(0, -100), DocScrollTranslation()->Get2dTranslation());
    ...
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(8, 8, 784, 10000), GetDocument().body()->GetLayoutObject(), frame_view->GetLayoutView());
}
...
TEST_P(PaintPropertyTreeBuilderTest, Perspective)
{
    ...
    Element* perspective = GetDocument().getElementById(AtomicString("perspective"));
    const ObjectPaintProperties* perspective_properties = perspective->GetLayoutObject()->FirstFragment().PaintProperties();
    ...
    // paint offset.
    EXPECT_EQ(gfx::Point3F(250, 250, 0), perspective_properties->Perspective()->Origin());
    EXPECT_EQ(DocScrollTranslation(), perspective_properties->Perspective()->Parent());
    ...
    const ObjectPaintProperties* inner_properties = inner->GetLayoutObject()->FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(50, 100), inner_properties->PaintOffsetTranslation()->Get2dTranslation());
    EXPECT_EQ(perspective_properties->Perspective(), inner_properties->PaintOffsetTranslation()->Parent());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(50, 100, 100, 200), inner->GetLayoutObject(), GetDocument().View()->GetLayoutView());

    perspective->setAttribute(html_names::kStyleAttr, AtomicString("perspective: 200px"));
    UpdateAllLifecyclePhasesForTest();
    ...
    perspective->setAttribute(html_names::kStyleAttr, AtomicString("perspective-origin: 5% 20%"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(matrix, perspective_properties->Perspective()->Matrix());
    EXPECT_EQ(gfx::Point3F(70, 160, 0), perspective_properties->Perspective()->Origin());
    EXPECT_EQ(DocScrollTranslation(), perspective_properties->Perspective()->Parent());
}
...
TEST_P(PaintPropertyTreeBuilderTest, Transform)
{
    ...
    const ObjectPaintProperties* transform_properties = transform->GetLayoutObject()->FirstFragment().PaintProperties();

    EXPECT_EQ(MakeTranslationMatrix(123, 456, 789), transform_properties->Transform()->Matrix());
    EXPECT_EQ(gfx::Point3F(200, 150, 0), transform_properties->Transform()->Origin());
    EXPECT_EQ(transform_properties->PaintOffsetTranslation(), transform_properties->Transform()->Parent());
    EXPECT_EQ(gfx::Vector2dF(50, 100), transform_properties->PaintOffsetTranslation()->Get2dTranslation());
    EXPECT_EQ(DocScrollTranslation(), transform_properties->PaintOffsetTranslation()->Parent());
    EXPECT_TRUE(transform_properties->Transform()->HasDirectCompositingReasons());

    CHECK_EXACT_VISUAL_RECT(PhysicalRect(173, 556, 400, 300), transform->GetLayoutObject(), GetDocument().View()->GetLayoutView());

    transform->setAttribute(html_names::kStyleAttr,
        AtomicString("margin-left: 50px; margin-top: 100px; "
                     "width: 400px; height: 300px;"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(nullptr, transform->GetLayoutObject()->FirstFragment().PaintProperties());

    transform->setAttribute(html_names::kStyleAttr,
        AtomicString("margin-left: 50px; margin-top: 100px; width: 400px; height: 300px; "
                     "transform: translate3d(123px, 456px, 789px)"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(MakeTranslationMatrix(123, 456, 789), transform->GetLayoutObject()->FirstFragment().PaintProperties()->Transform()->Matrix());
}
...
TEST_P(PaintPropertyTreeBuilderTest, Preserve3D3DTransformedDescendant)
{
    ...
    const ObjectPaintProperties* preserve_properties = preserve->GetLayoutObject()->FirstFragment().PaintProperties();
...
TEST_P(PaintPropertyTreeBuilderTest, Perspective3DTransformedDescendant)
{
    ...
    EXPECT_TRUE(perspective_properties->Transform()->HasDirectCompositingReasons());
}

TEST_P(PaintPropertyTreeBuilderTest, TransformPerspective3DTransformedDescendant)
{
    SetBodyInnerHTML(R"HTML(
    <style> body { margin: 0 } </style>
...
TEST_P(PaintPropertyTreeBuilderTest, TransformNodeWithActiveAnimationHasDirectCompositingReason)
{
    LoadTestData("transform-animation.html");
    EXPECT_TRUE(PaintPropertiesForElement("target")->Transform()->HasDirectCompositingReasons());
}

TEST_P(PaintPropertyTreeBuilderTest, TransformAnimationDoesNotCreateEffectAndFilterNodes)
{
    LoadTestData("transform-animation.html");
...
TEST_P(PaintPropertyTreeBuilderTest, OpacityAnimationDoesNotCreateTransformAndFilterNodes)
{
...
TEST_P(PaintPropertyTreeBuilderTest, EffectNodeWithActiveAnimationHasDirectCompositingReason)
{
    LoadTestData("opacity-animation.html");
    EXPECT_TRUE(PaintPropertiesForElement("target")->Effect()->HasDirectCompositingReasons());
}
...
TEST_P(PaintPropertyTreeBuilderTest, WillChangeTransform)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(), transform_properties->Transform()->Get2dTranslation());
    EXPECT_EQ(gfx::Point3F(), transform_properties->Transform()->Origin());
    EXPECT_EQ(gfx::Vector2dF(50, 100), transform_properties->PaintOffsetTranslation()->Get2dTranslation());
    EXPECT_TRUE(transform_properties->Transform()->HasDirectCompositingReasons());

    CHECK_EXACT_VISUAL_RECT(PhysicalRect(50, 100, 400, 300), transform->GetLayoutObject(), GetDocument().View()->GetLayoutView());
    ...
    transform->setAttribute(html_names::kStyleAttr,
        AtomicString("margin-left: 50px; margin-top: 100px; width: 400px; height: 300px; "
                     "will-change: transform"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(transform->GetLayoutObject()->FirstFragment().PaintProperties()->Transform()->IsIdentity());
}
...
TEST_P(PaintPropertyTreeBuilderTest, BackfaceVisibilityWithPseudoStacking3DChildren)
{
    ...
    EXPECT_EQ(PaintPropertiesForElement("child")->Transform()->GetBackfaceVisibilityForTesting(), TransformPaintPropertyNode::BackfaceVisibility::kVisible);
}
...
TEST_P(PaintPropertyTreeBuilderTest, RelativePositionInline)
{
    ...
    Element* inline_block = GetDocument().getElementById(AtomicString("inline-block"));
    const ObjectPaintProperties* inline_block_properties = inline_block->GetLayoutObject()->FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(135, 490), inline_block_properties->PaintOffsetTranslation()->Get2dTranslation());
    EXPECT_EQ(DocScrollTranslation(), inline_block_properties->PaintOffsetTranslation()->Parent());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(135, 490, 10, 20), inline_block->GetLayoutObject(), GetDocument().View()->GetLayoutView());
}
...
TEST_P(PaintPropertyTreeBuilderTest, NestedOpacityEffect)
{
    ...
    LayoutObject* node_without_opacity = GetLayoutObjectByElementId("nodeWithoutOpacity");
    const auto* data_without_opacity_properties = node_without_opacity->FirstFragment().PaintProperties();
    EXPECT_EQ(nullptr, data_without_opacity_properties);
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(8, 8, 100, 200), node_without_opacity, GetDocument().View()->GetLayoutView());

    LayoutObject* child_with_opacity = GetLayoutObjectByElementId("childWithOpacity");
    const ObjectPaintProperties* child_with_opacity_properties = child_with_opacity->FirstFragment().PaintProperties();
    ...
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(8, 8, 50, 60), child_with_opacity, GetDocument().View()->GetLayoutView());

    LayoutObject* grand_child_without_opacity = GetDocument().getElementById(AtomicString("grandChildWithoutOpacity"))->GetLayoutObject();
    EXPECT_EQ(nullptr, grand_child_without_opacity->FirstFragment().PaintProperties());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(8, 8, 20, 30), grand_child_without_opacity, GetDocument().View()->GetLayoutView());

    LayoutObject* great_grand_child_with_opacity = GetDocument().getElementById(AtomicString("greatGrandChildWithOpacity"))->GetLayoutObject();
    const ObjectPaintProperties* great_grand_child_with_opacity_properties = great_grand_child_with_opacity->FirstFragment().PaintProperties();
    EXPECT_EQ(0.2f, great_grand_child_with_opacity_properties->Effect()->Opacity());
    EXPECT_EQ(child_with_opacity_properties->Effect(), great_grand_child_with_opacity_properties->Effect()->Parent());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(8, 8, 10, 15), great_grand_child_with_opacity, GetDocument().View()->GetLayoutView());
}
...
TEST_P(PaintPropertyTreeBuilderTest, TransformNodeDoesNotAffectEffectNodes)
{
    ...
    LayoutObject* node_with_opacity = GetLayoutObjectByElementId("nodeWithOpacity");
    const ObjectPaintProperties* node_with_opacity_properties = node_with_opacity->FirstFragment().PaintProperties();
    EXPECT_EQ(0.6f, node_with_opacity_properties->Effect()->Opacity());
    EXPECT_EQ(DocContentClip(), node_with_opacity_properties->Effect()->OutputClip());
    EXPECT_NE(nullptr, node_with_opacity_properties->Effect()->Parent());
    EXPECT_EQ(nullptr, node_with_opacity_properties->Transform());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(8, 8, 100, 200), node_with_opacity, GetDocument().View()->GetLayoutView());

    LayoutObject* child_with_transform = GetLayoutObjectByElementId("childWithTransform");
    const ObjectPaintProperties* child_with_transform_properties = child_with_transform->FirstFragment().PaintProperties();
    EXPECT_EQ(nullptr, child_with_transform_properties->Effect());
    EXPECT_EQ(gfx::Vector2dF(10, 10), child_with_transform_properties->Transform()->Get2dTranslation());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(18, 18, 50, 60), child_with_transform, GetDocument().View()->GetLayoutView());

    LayoutObject* grand_child_with_opacity = GetLayoutObjectByElementId("grandChildWithOpacity");
    const ObjectPaintProperties* grand_child_with_opacity_properties = grand_child_with_opacity->FirstFragment().PaintProperties();
    EXPECT_EQ(0.4f, grand_child_with_opacity_properties->Effect()->Opacity());
    EXPECT_EQ(DocContentClip(), grand_child_with_opacity_properties->Effect()->OutputClip());
    EXPECT_EQ(node_with_opacity_properties->Effect(), grand_child_with_opacity_properties->Effect()->Parent());
    EXPECT_EQ(nullptr, grand_child_with_opacity_properties->Transform());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(18, 18, 20, 30), grand_child_with_opacity, GetDocument().View()->GetLayoutView());
}
...
TEST_P(PaintPropertyTreeBuilderTest, EffectNodesAcrossStackingContext)
{
    ...
    LayoutObject* child_with_stacking_context = GetDocument().getElementById(AtomicString("childWithStackingContext"))->GetLayoutObject();
    const ObjectPaintProperties* child_with_stacking_context_properties = child_with_stacking_context->FirstFragment().PaintProperties();
    EXPECT_EQ(nullptr, child_with_stacking_context_properties);
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(8, 8, 50, 60), child_with_stacking_context, GetDocument().View()->GetLayoutView());
    ...
    EXPECT_EQ(node_with_opacity_properties->Effect(), grand_child_with_opacity_properties->Effect()->Parent());
    EXPECT_EQ(nullptr, grand_child_with_opacity_properties->Transform());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(8, 8, 20, 30), grand_child_with_opacity, GetDocument().View()->GetLayoutView());
}
...
TEST_P(PaintPropertyTreeBuilderTest, EffectNodesInSVG)
{
    ...
    const auto* group_with_opacity_properties = PaintPropertiesForElement("groupWithOpacity");
    ...
    const auto* rect_with_opacity_properties = PaintPropertiesForElement("rectWithOpacity");
    ...
    EXPECT_EQ(group_with_opacity_properties->Effect(), rect_with_opacity_properties->Effect()->Parent());
    ...
    const auto* text_with_opacity_properties = PaintPropertiesForElement("textWithOpacity");
    ...
    EXPECT_EQ(group_with_opacity_properties->Effect(), text_with_opacity_properties->Effect()->Parent());
    ...
    const auto* tspan_with_opacity_properties = PaintPropertiesForElement("tspanWithOpacity");
    ...
    EXPECT_EQ(text_with_opacity_properties->Effect(), tspan_with_opacity_properties->Effect()->Parent());
}
...
TEST_P(PaintPropertyTreeBuilderTest, EffectNodesAcrossHTMLSVGBoundary)
{
    ...
    const auto* div_with_opacity_properties = PaintPropertiesForElement("divWithOpacity");
    EXPECT_EQ(0.2f, div_with_opacity_properties->Effect()->Opacity());
    EXPECT_EQ(DocContentClip(), div_with_opacity_properties->Effect()->OutputClip());
    EXPECT_NE(nullptr, div_with_opacity_properties->Effect()->Parent());

    const auto* svg_root_with_opacity_properties = PaintPropertiesForElement("svgRootWithOpacity");
    EXPECT_EQ(0.3f, svg_root_with_opacity_properties->Effect()->Opacity());
    EXPECT_EQ(DocContentClip(), svg_root_with_opacity_properties->Effect()->OutputClip());
    EXPECT_EQ(div_with_opacity_properties->Effect(), svg_root_with_opacity_properties->Effect()->Parent());

    const auto* rect_with_opacity_properties = PaintPropertiesForElement("rectWithOpacity");
    EXPECT_EQ(0.4f, rect_with_opacity_properties->Effect()->Opacity());
    EXPECT_EQ(svg_root_with_opacity_properties->OverflowClip(), rect_with_opacity_properties->Effect()->OutputClip());
    EXPECT_EQ(svg_root_with_opacity_properties->Effect(), rect_with_opacity_properties->Effect()->Parent());
}
...
TEST_P(PaintPropertyTreeBuilderTest, EffectNodesAcrossSVGHTMLBoundary)
{
    ...
    const auto* foreign_object_with_opacity_properties = PaintPropertiesForElement("foreignObjectWithOpacity");
    EXPECT_EQ(0.4f, foreign_object_with_opacity_properties->Effect()->Opacity());
    EXPECT_EQ(svg_root_with_opacity_properties->OverflowClip(), foreign_object_with_opacity_properties->Effect()->OutputClip());
    EXPECT_EQ(svg_root_with_opacity_properties->Effect(), foreign_object_with_opacity_properties->Effect()->Parent());

    const auto* span_with_opacity_properties = PaintPropertiesForElement("spanWithOpacity");
    EXPECT_EQ(0.5f, span_with_opacity_properties->Effect()->Opacity());
    EXPECT_EQ(svg_root_with_opacity_properties->OverflowClip(), span_with_opacity_properties->Effect()->OutputClip());
    EXPECT_EQ(foreign_object_with_opacity_properties->Effect(), span_with_opacity_properties->Effect()->Parent());
}
...
TEST_P(PaintPropertyTreeBuilderTest, TransformNodesInSVG)
{
    ...
    LayoutObject& svg_root_with3d_transform = *GetDocument().getElementById(AtomicString("svgRootWith3dTransform"))->GetLayoutObject();
    const ObjectPaintProperties* svg_root_with3d_transform_properties = svg_root_with3d_transform.FirstFragment().PaintProperties();
    EXPECT_EQ(MakeTranslationMatrix(1, 2, 3), svg_root_with3d_transform_properties->Transform()->Matrix());
    EXPECT_EQ(gfx::Point3F(50, 50, 0), svg_root_with3d_transform_properties->Transform()->Origin());
    EXPECT_EQ(svg_root_with3d_transform_properties->PaintOffsetTranslation(), svg_root_with3d_transform_properties->Transform()->Parent());
    EXPECT_EQ(gfx::Vector2dF(70, 25), svg_root_with3d_transform_properties->PaintOffsetTranslation()->Get2dTranslation());
    EXPECT_EQ(DocScrollTranslation(), svg_root_with3d_transform_properties->PaintOffsetTranslation()->Parent());

    LayoutObject& rect_with2d_transform = *GetLayoutObjectByElementId("rectWith2dTransform");
    const ObjectPaintProperties* rect_with2d_transform_properties = rect_with2d_transform.FirstFragment().PaintProperties();
    gfx::Transform matrix;
    ...
    EXPECT_EQ(gfx::Point3F(0, 0, 0), rect_with2d_transform_properties->Transform()->Origin());
    // SVG does not use paint offset.
    EXPECT_EQ(nullptr, rect_with2d_transform_properties->PaintOffsetTranslation());
}
...
TEST_P(PaintPropertyTreeBuilderTest, SVGViewBoxTransform)
{
    ...
    LayoutObject& svg_with_view_box = *GetLayoutObjectByElementId("svgWithViewBox");
    const ObjectPaintProperties* svg_with_view_box_properties = svg_with_view_box.FirstFragment().PaintProperties();
    EXPECT_EQ(MakeTranslationMatrix(1, 2, 3), svg_with_view_box_properties->Transform()->Matrix());
    EXPECT_EQ(gfx::Vector2dF(-50, -50), svg_with_view_box_properties->ReplacedContentTransform()->Get2dTranslation());
    EXPECT_EQ(svg_with_view_box_properties->ReplacedContentTransform()->Parent(), svg_with_view_box_properties->Transform());

    LayoutObject& rect = *GetLayoutObjectByElementId("rect");
    const ObjectPaintProperties* rect_properties = rect.FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(100, 100), rect_properties->Transform()->Get2dTranslation());
    EXPECT_EQ(svg_with_view_box_properties->ReplacedContentTransform(), rect_properties->Transform()->Parent());
}
...
TEST_P(PaintPropertyTreeBuilderTest, SVGRootPaintOffsetTransformNode)
{
    ...
    const ObjectPaintProperties* svg_properties = svg.FirstFragment().PaintProperties();
    EXPECT_TRUE(svg_properties->PaintOffsetTranslation());
    EXPECT_EQ(gfx::Vector2dF(50, 25), svg_properties->PaintOffsetTranslation()->Get2dTranslation());
    EXPECT_EQ(nullptr, svg_properties->ReplacedContentTransform());
    EXPECT_EQ(DocScrollTranslation(), svg_properties->PaintOffsetTranslation()->Parent());
}
...
TEST_P(PaintPropertyTreeBuilderTest, SVGRootLocalToBorderBoxTransformNode)
{
    ...
    LayoutObject& svg = *GetLayoutObjectByElementId("svg");
    const ObjectPaintProperties* svg_properties = svg.FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(2, 3), svg_properties->PaintOffsetTranslation()->Get2dTranslation());
    EXPECT_EQ(gfx::Vector2dF(5, 7), svg_properties->Transform()->Get2dTranslation());
    auto matrix = MakeTranslationMatrix(11, 11);
    matrix.Scale(100.0 / 13.0);
    EXPECT_TRANSFORM_EQ(matrix, svg_properties->ReplacedContentTransform()->Matrix());
    EXPECT_EQ(svg_properties->PaintOffsetTranslation(), svg_properties->Transform()->Parent());
    EXPECT_EQ(svg_properties->Transform(), svg_properties->ReplacedContentTransform()->Parent());

    // Ensure the rect's transform is a child of the local to border box
    // transform.
    LayoutObject& rect = *GetLayoutObjectByElementId("rect");
    const ObjectPaintProperties* rect_properties = rect.FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(17, 19), rect_properties->Transform()->Get2dTranslation());
    EXPECT_EQ(svg_properties->ReplacedContentTransform(), rect_properties->Transform()->Parent());
}

TEST_P(PaintPropertyTreeBuilderTest, SVGRootLocalToBorderBoxSnappingScale)
{
    ScopedSvgInlineRootPixelSnappingScaleAdjustmentForTest snapscale_enabled(true);
    SetBodyInnerHTML(R"HTML(
    <style>
      svg {
        height: 99.99px;
    ...
    const ObjectPaintProperties* svg_properties = svg.FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(8, 9), svg_properties->PaintOffsetTranslation()->Get2dTranslation());
    ...
    EXPECT_TRANSFORM_EQ(MakeScaleMatrix(snapped_height / unsnapped_height), svg_properties->ReplacedContentTransform()->Matrix());
}
...
TEST_P(PaintPropertyTreeBuilderTest, SVGNestedViewboxTransforms)
{
    ...
    LayoutObject& svg = *GetLayoutObjectByElementId("svg");
    const ObjectPaintProperties* svg_properties = svg.FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(11, 11), svg_properties->Transform()->Get2dTranslation());
    EXPECT_EQ(MakeScaleMatrix(2), svg_properties->ReplacedContentTransform()->Matrix());

    LayoutObject& nested_svg = *GetLayoutObjectByElementId("nestedSvg");
    const ObjectPaintProperties* nested_svg_properties = nested_svg.FirstFragment().PaintProperties();
    ...
    EXPECT_EQ(svg_properties->ReplacedContentTransform(), nested_svg_properties->Transform()->Parent());

    LayoutObject& rect = *GetLayoutObjectByElementId("rect");
    const ObjectPaintProperties* rect_properties = rect.FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(13, 13), rect_properties->Transform()->Get2dTranslation());
    EXPECT_EQ(nested_svg_properties->Transform(), rect_properties->Transform()->Parent());
}
...
TEST_P(PaintPropertyTreeBuilderTest, TransformNodesAcrossSVGHTMLBoundary)
{
    ...
    LayoutObject& svg_with_transform = *GetLayoutObjectByElementId("svgWithTransform");
    const ObjectPaintProperties* svg_with_transform_properties = svg_with_transform.FirstFragment().PaintProperties();
    EXPECT_EQ(MakeTranslationMatrix(1, 2, 3), svg_with_transform_properties->Transform()->Matrix());

    LayoutObject& div_with_transform = *GetLayoutObjectByElementId("divWithTransform");
    const ObjectPaintProperties* div_with_transform_properties = div_with_transform.FirstFragment().PaintProperties();
    EXPECT_EQ(MakeTranslationMatrix(3, 4, 5), div_with_transform_properties->Transform()->Matrix());
    // Ensure the div's transform node is a child of the svg's transform node.
    EXPECT_EQ(svg_with_transform_properties->Transform(), div_with_transform_properties->Transform()->Parent()->Parent());
}
...
TEST_P(PaintPropertyTreeBuilderTest, ForeignObjectWithTransformAndOffset)
{
    ...
    const ObjectPaintProperties* foreign_object_properties = foreign_object.FirstFragment().PaintProperties();
    EXPECT_EQ(MakeScaleMatrix(5), foreign_object_properties->Transform()->Matrix());
    EXPECT_EQ(PhysicalOffset(10, 10), foreign_object.FirstFragment().PaintOffset());
...
TEST_P(PaintPropertyTreeBuilderTest, ForeignObjectWithMask)
{
    ...
    EXPECT_EQ(foreign_object_properties->MaskClip()->Parent(), foreign_object_properties->Mask()->OutputClip());
    EXPECT_EQ(&svg.FirstFragment().LocalBorderBoxProperties().Transform(), &foreign_object_properties->Mask()->LocalTransformSpace());
}
...
TEST_P(PaintPropertyTreeBuilderTest, PaintOffsetTranslationSVGHTMLBoundary)
{
    ...
    LayoutObject& svg = *GetLayoutObjectByElementId("svg");
    const ObjectPaintProperties* svg_properties = svg.FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(8, 8), svg_properties->PaintOffsetTranslation()->Get2dTranslation());

    LayoutObject& div_with_transform = *GetLayoutObjectByElementId("divWithTransform");
    const ObjectPaintProperties* div_with_transform_properties = div_with_transform.FirstFragment().PaintProperties();
    EXPECT_EQ(MakeTranslationMatrix(3, 4, 5), div_with_transform_properties->Transform()->Matrix());
    EXPECT_EQ(gfx::Vector2dF(8, 158), div_with_transform_properties->PaintOffsetTranslation()->Get2dTranslation());
    EXPECT_EQ(div_with_transform_properties->PaintOffsetTranslation(), div_with_transform_properties->Transform()->Parent());
}
...
TEST_P(PaintPropertyTreeBuilderTest, PaintOffsetTranslationSVGHTMLBoundaryMulticol)
{
    SetBodyInnerHTML(R"HTML(
    <svg id='svg'>
      <foreignObject>
    ...
    const ObjectPaintProperties* svg_properties = svg.FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(8, 8), svg_properties->PaintOffsetTranslation()->Get2dTranslation());
    LayoutObject& div_with_columns = *GetLayoutObjectByElementId("divWithColumns")->SlowFirstChild();
...
TEST_P(PaintPropertyTreeBuilderTest, FixedTransformAncestorAcrossSVGHTMLBoundary)
{
    ...
    LayoutObject& svg = *GetLayoutObjectByElementId("svg");
    const ObjectPaintProperties* svg_properties = svg.FirstFragment().PaintProperties();
    EXPECT_EQ(MakeTranslationMatrix(1, 2, 3), svg_properties->Transform()->Matrix());

    LayoutObject& container = *GetLayoutObjectByElementId("container");
    const ObjectPaintProperties* container_properties = container.FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(20, 30), container_properties->Transform()->Get2dTranslation());
    EXPECT_EQ(svg_properties->Transform(), container_properties->Transform()->Parent());
    ...
    // container.
    EXPECT_EQ(container_properties->Transform(), &fixed->GetLayoutObject()->FirstFragment().LocalBorderBoxProperties().Transform());
}
...
TEST_P(PaintPropertyTreeBuilderTest, ControlClip)
{
    ...
    const ObjectPaintProperties* button_properties = button.FirstFragment().PaintProperties();
    ...
    EXPECT_EQ(DocScrollTranslation(), &button_properties->OverflowClip()->LocalTransformSpace());

    EXPECT_CLIP_RECT(FloatRoundedRect(5, 5, 335, 113), button_properties->OverflowClip());
    EXPECT_EQ(DocContentClip(), button_properties->OverflowClip()->Parent());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(0, 0, 345, 123), &button, GetDocument().View()->GetLayoutView());
}
...
TEST_P(PaintPropertyTreeBuilderTest, ControlClipInsideForeignObject)
{
    ...
    // not scroll (not enough content).
    EXPECT_TRUE(DocScrollTranslation());
    EXPECT_CLIP_RECT(FloatRoundedRect(2, 2, 341, 119), button_properties->OverflowClip());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(8, 8, 345, 123), &button, GetDocument().View()->GetLayoutView());
}
...
TEST_P(PaintPropertyTreeBuilderTest, BorderRadiusClip)
{
    ...
    LayoutObject& div = *GetLayoutObjectByElementId("div");
    const ObjectPaintProperties* div_properties = div.FirstFragment().PaintProperties();

    // Always create scroll translation for layout view even the document does
    // not scroll (not enough content).
    EXPECT_TRUE(DocScrollTranslation());
    EXPECT_EQ(DocScrollTranslation(), &div_properties->OverflowClip()->LocalTransformSpace());
    ...
    EXPECT_CLIP_RECT(FloatRoundedRect(60, 45, 500, 400), div_properties->OverflowClip());
    auto& border_radius_clip = ToUnaliased(*div_properties->OverflowClip()->Parent());
    ...
    EXPECT_CLIP_RECT(FloatRoundedRect(border_box_minus_border_outset,
    ...
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(0, 0, 610, 500), &div, GetDocument().View()->GetLayoutView());
}
...
TEST_P(PaintPropertyTreeBuilderTest, SubpixelBorderRadiusClip)
{
    ...
    const ClipPaintPropertyNode* border_radius_clip = div_properties->InnerBorderRadiusClip();
    ...
    EXPECT_EQ(FloatRoundedRect(gfx::RectF(0, 1, 100, 100), FloatRoundedRect::Radii(50)), border_radius_clip->PaintClipRect());
}
...
TEST_P(PaintPropertyTreeBuilderTest, TransformNodesAcrossSubframes)
{
    ...
    LayoutObject* div_with_transform = GetLayoutObjectByElementId("divWithTransform");
    const ObjectPaintProperties* div_with_transform_properties = div_with_transform->FirstFragment().PaintProperties();
    EXPECT_EQ(MakeTranslationMatrix(1, 2, 3), div_with_transform_properties->Transform()->Matrix());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(1, 2, 800, 164), div_with_transform, frame_view->GetLayoutView());

    LayoutObject* inner_div_with_transform = ChildDocument().getElementById(AtomicString("innerDivWithTransform"))->GetLayoutObject();
    const ObjectPaintProperties* inner_div_with_transform_properties = inner_div_with_transform->FirstFragment().PaintProperties();
    ...
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(12, 14, 100, 145), inner_div_with_transform, frame_view->GetLayoutView());
    ...
    auto* inner_document_scroll_translation = inner_div_transform->UnaliasedParent();
    EXPECT_TRUE(inner_document_scroll_translation->IsIdentity());
    auto* paint_offset_translation = inner_document_scroll_translation->UnaliasedParent();
    ...
    EXPECT_EQ(div_with_transform_properties->Transform(), iframe_pre_translation->Parent());
}
...
TEST_P(PaintPropertyTreeBuilderTest, FramesEstablishIsolation)
{
    ...
    const auto& frame_contents_properties = frame->FirstFragment().ContentsProperties();

    LayoutObject* child = ChildDocument().getElementById(AtomicString("child"))->GetLayoutObject();
    const auto& child_local_border_box_properties = child->FirstFragment().LocalBorderBoxProperties();
    auto* child_properties = child->GetMutableForPainting().FirstFragment().PaintProperties();

    // From the frame content's properties, we have:
    //  - transform isolation node
    //    - paint offset translation
    //      - transform
    EXPECT_EQ(gfx::Vector2dF(50, 0), ToUnaliased(child_local_border_box_properties.Transform()).Get2dTranslation());
    EXPECT_EQ(child_local_border_box_properties.Transform().Parent(), child_properties->PaintOffsetTranslation());
    EXPECT_EQ(child_local_border_box_properties.Transform().Parent()->Parent(), &frame_contents_properties.Transform());
    ...
    // clip and effect.
    EXPECT_EQ(&child_local_border_box_properties.Clip(), &frame_contents_properties.Clip());
    EXPECT_TRUE(frame_contents_properties.Clip().Parent());
    EXPECT_TRUE(frame_contents_properties.Clip().IsParentAlias());

    EXPECT_EQ(&child_local_border_box_properties.Effect(), &frame_contents_properties.Effect());
...
// this only runs if we don't have DCHECKs enabled.
    ...
    child_properties->UpdateTransform(*child_local_border_box_properties.Transform().Parent(), std::move(state));
    // Verify that we clobbered it correctly.
    EXPECT_EQ(gfx::Vector2dF(123, 321), ToUnaliased(child_local_border_box_properties.Transform()).Get2dTranslation());

    // This causes a tree topology change which forces the subtree to be updated.
    // However, isolation stops this recursion.
    GetDocument().getElementById(AtomicString("parent"))->setAttribute(html_names::kClassAttr, AtomicString("transformed"));
    frame_view->UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintPropertyTreeBuilderTest, TransformNodesInTransformedSubframes)
{
    ...
    LayoutObject* inner_div_with_transform = ChildDocument().getElementById(AtomicString("transform"))->GetLayoutObject();
    auto* inner_div_transform = inner_div_with_transform->FirstFragment().PaintProperties()->Transform();
    EXPECT_EQ(MakeTranslationMatrix(7, 8, 9), inner_div_transform->Matrix());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(92, 95, 100, 111), inner_div_with_transform, frame_view->GetLayoutView());

    auto* inner_document_paint_offset_translation = inner_div_transform->UnaliasedParent();
    EXPECT_EQ(gfx::Vector2dF(31, 31), inner_document_paint_offset_translation->Get2dTranslation());
    auto* inner_document_scroll_translation = inner_document_paint_offset_translation->UnaliasedParent();
    EXPECT_TRUE(inner_document_scroll_translation->IsIdentity());
    auto* iframe_pre_translation = inner_document_scroll_translation->UnaliasedParent();
    ...
    EXPECT_EQ(gfx::Vector2dF(7, 7), iframe_paint_offset_translation->Get2dTranslation());
    auto* div_with_transform_transform = iframe_paint_offset_translation->UnaliasedParent();
    EXPECT_EQ(MakeTranslationMatrix(1, 2, 3), div_with_transform_transform->Matrix());

    LayoutObject* div_with_transform = GetLayoutObjectByElementId("divWithTransform");
    EXPECT_EQ(div_with_transform_transform, div_with_transform->FirstFragment().PaintProperties()->Transform());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(1, 2, 800, 248), div_with_transform, frame_view->GetLayoutView());
}
...
TEST_P(PaintPropertyTreeBuilderTest, TreeContextClipByNonStackingContext)
{
    ...
    const ObjectPaintProperties* scroller_properties = scroller->FirstFragment().PaintProperties();
    LayoutObject* child = GetLayoutObjectByElementId("child");

    EXPECT_EQ(scroller_properties->OverflowClip(), &child->FirstFragment().LocalBorderBoxProperties().Clip());
    EXPECT_EQ(scroller_properties->ScrollTranslation(), &child->FirstFragment().LocalBorderBoxProperties().Transform());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(0, 0, 400, 300), scroller, frame_view->GetLayoutView());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(0, 0, 100, 200), child, frame_view->GetLayoutView());
}
...
TEST_P(PaintPropertyTreeBuilderTest, TreeContextUnclipFromParentStackingContext)
{
    ...
    EXPECT_EQ(DocContentClip(), &child.FirstFragment().LocalBorderBoxProperties().Clip());
    EXPECT_EQ(DocScrollTranslation(), &child.FirstFragment().LocalBorderBoxProperties().Transform());
    EXPECT_EQ(scroller_properties->Effect(), &child.FirstFragment().LocalBorderBoxProperties().Effect());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(0, 0, 800, 10000), &scroller, GetDocument().View()->GetLayoutView());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(0, 0, 100, 200), &child, GetDocument().View()->GetLayoutView());
}
...
TEST_P(PaintPropertyTreeBuilderTest, TableCellLayoutLocation)
{
    ...
    EXPECT_EQ(DocScrollTranslation(), &target.FirstFragment().LocalBorderBoxProperties().Transform());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(170, 170, 100, 100), &target, GetDocument().View()->GetLayoutView());
}
...
TEST_P(PaintPropertyTreeBuilderTest, CSSClipFixedPositionDescendant)
{
    ...
    const ObjectPaintProperties* clip_properties = clip.FirstFragment().PaintProperties();
    EXPECT_EQ(DocContentClip(), clip_properties->CssClip()->Parent());
    EXPECT_EQ(DocScrollTranslation(), &clip_properties->CssClip()->LocalTransformSpace());
    EXPECT_CLIP_RECT(gfx::RectF(absolute_clip_rect), clip_properties->CssClip());
    CHECK_VISUAL_RECT(absolute_clip_rect, &clip, GetDocument().View()->GetLayoutView(),
    ...
    EXPECT_EQ(clip_properties->CssClip(), &fixed->FirstFragment().LocalBorderBoxProperties().Clip());
    EXPECT_EQ(fixed->FirstFragment().PaintProperties()->PaintOffsetTranslation(), &fixed->FirstFragment().LocalBorderBoxProperties().Transform());
    EXPECT_EQ(PhysicalOffset(0, 0), fixed->FirstFragment().PaintOffset());
    CHECK_VISUAL_RECT(PhysicalRect(), fixed, GetDocument().View()->GetLayoutView(),
...
TEST_P(PaintPropertyTreeBuilderTest, CSSClipAbsPositionDescendant)
{
    ...
    EXPECT_EQ(clip_properties->CssClip(), &absolute->FirstFragment().LocalBorderBoxProperties().Clip());
    EXPECT_TRUE(DocScrollTranslation());
    EXPECT_EQ(DocScrollTranslation(), &absolute->FirstFragment().LocalBorderBoxProperties().Transform());
    EXPECT_EQ(PhysicalOffset(777, 777), absolute->FirstFragment().PaintOffset());
    CHECK_VISUAL_RECT(PhysicalRect(), absolute, GetDocument().View()->GetLayoutView(),
        // TODO(crbug.com/599939): CSS clip of fixed-position
        // descendants is broken in
        // mapToVisualRectInAncestorSpace().
        LayoutUnit::Max());
...
TEST_P(PaintPropertyTreeBuilderTest, CSSClipSubpixel)
{
    ...
    absolute_clip_rect.offset += PhysicalOffset(LayoutUnit(123.5), LayoutUnit(456));
    ...
    EXPECT_TRUE(DocScrollTranslation());
    EXPECT_EQ(DocScrollTranslation(), &clip_properties->CssClip()->LocalTransformSpace());
    EXPECT_EQ(FloatClipRect(gfx::RectF(absolute_clip_rect)), clip_properties->CssClip()->LayoutClipRect());
    EXPECT_EQ(FloatRoundedRect(ToPixelSnappedRect((absolute_clip_rect))), clip_properties->CssClip()->PaintClipRect());
}
...
TEST_P(PaintPropertyTreeBuilderTest, CSSClipFixedPositionDescendantNonShared)
{
    ...
    const ObjectPaintProperties* overflow_properties = overflow.FirstFragment().PaintProperties();
    EXPECT_EQ(DocContentClip(), overflow_properties->OverflowClip()->Parent());
    // Always create scroll translation for layout view even the document does
    // not scroll (not enough content).
    EXPECT_TRUE(DocScrollTranslation());
    EXPECT_EQ(DocScrollTranslation(), overflow_properties->ScrollTranslation()->Parent()->Parent());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(0, 0, 50, 50), &overflow, GetDocument().View()->GetLayoutView());

    LayoutObject* clip = GetLayoutObjectByElementId("clip");
    const ObjectPaintProperties* clip_properties = clip->FirstFragment().PaintProperties();
    EXPECT_EQ(overflow_properties->OverflowClip(), clip_properties->CssClip()->Parent());
    EXPECT_EQ(overflow_properties->ScrollTranslation(), &clip_properties->CssClip()->LocalTransformSpace());
    EXPECT_CLIP_RECT(FloatRoundedRect(gfx::RectF(absolute_clip_rect)), clip_properties->CssClip());
    EXPECT_EQ(DocContentClip(), clip_properties->CssClipFixedPosition()->Parent());
    EXPECT_EQ(overflow_properties->ScrollTranslation(), &clip_properties->CssClipFixedPosition()->LocalTransformSpace());
    EXPECT_CLIP_RECT(FloatRoundedRect(gfx::RectF(absolute_clip_rect)), clip_properties->CssClipFixedPosition());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(), clip, GetDocument().View()->GetLayoutView());

    LayoutObject* fixed = GetLayoutObjectByElementId("fixed");
    EXPECT_EQ(clip_properties->CssClipFixedPosition(), &fixed->FirstFragment().LocalBorderBoxProperties().Clip());
    EXPECT_EQ(fixed->FirstFragment().PaintProperties()->PaintOffsetTranslation(), &fixed->FirstFragment().LocalBorderBoxProperties().Transform());
    EXPECT_EQ(PhysicalOffset(0, 0), fixed->FirstFragment().PaintOffset());
    CHECK_VISUAL_RECT(PhysicalRect(), fixed, GetDocument().View()->GetLayoutView(),
        // TODO(crbug.com/599939): CSS clip of fixed-position
...
TEST_P(PaintPropertyTreeBuilderTest, ColumnSpannerUnderRelativePositioned)
{
    ...
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(55, 44, 100, 100), spanner, GetDocument().View()->GetLayoutView());
}
...
TEST_P(PaintPropertyTreeBuilderTest, FractionalPaintOffset)
{
    ...
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(LayoutUnit(0.1), LayoutUnit(0.3), LayoutUnit(70), LayoutUnit(70)), a, frame_view->GetLayoutView());

    LayoutObject* b = GetLayoutObjectByElementId("b");
    PhysicalOffset b_paint_offset = a_paint_offset + PhysicalOffset(LayoutUnit(0.5), LayoutUnit(11.1));
...
TEST_P(PaintPropertyTreeBuilderTest, PaintOffsetWithBasicPixelSnapping)
{
    ...
    LayoutObject* b = GetLayoutObjectByElementId("b");
    const ObjectPaintProperties* b_properties = b->FirstFragment().PaintProperties();
    ...
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(LayoutUnit(0.3), LayoutUnit(0.3), LayoutUnit(40), LayoutUnit(40)), b, frame_view->GetLayoutView());

    // c's painted should start at subpixelAccumulation + (0.1,0.1) = (0.4,0.4).
    LayoutObject* c = GetLayoutObjectByElementId("c");
    PhysicalOffset c_paint_offset = subpixel_accumulation + PhysicalOffset(LayoutUnit(0.1), LayoutUnit(0.1));
    ...
    CHECK_VISUAL_RECT(PhysicalRect(LayoutUnit(0.4), LayoutUnit(0.4), LayoutUnit(40), LayoutUnit(40)), c, frame_view->GetLayoutView(), 1);
}
...
TEST_P(PaintPropertyTreeBuilderTest, PaintOffsetWithPixelSnappingThroughTransform)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(1, 1), ToUnaliased(*b_properties->Transform()->Parent()).Get2dTranslation());
    // The residual subpixel adjustment should be (0.7,0.7) - (1,1) = (-0.3,-0.3).
    PhysicalOffset subpixel_accumulation = PhysicalOffset(LayoutUnit(0.7), LayoutUnit(0.7)) - PhysicalOffset(1, 1);
    EXPECT_EQ(subpixel_accumulation, b->FirstFragment().PaintOffset());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(LayoutUnit(0.7), LayoutUnit(0.7), LayoutUnit(40), LayoutUnit(40)), b, frame_view->GetLayoutView());

    // c's painting should start at subpixelAccumulation + (0.7,0.7) = (0.4,0.4).
    LayoutObject* c = GetLayoutObjectByElementId("c");
    PhysicalOffset c_paint_offset = subpixel_accumulation + PhysicalOffset(LayoutUnit(0.7), LayoutUnit(0.7));
    EXPECT_EQ(c_paint_offset, c->FirstFragment().PaintOffset());
    // Visual rects via the non-paint properties system use enclosingIntRect
    // before applying transforms, because they are computed bottom-up and
    // therefore can't apply pixel snapping. Therefore apply a slop of 1px.
    CHECK_VISUAL_RECT(
        PhysicalRect(LayoutUnit(0.7) + LayoutUnit(0.7), LayoutUnit(0.7) + LayoutUnit(0.7), LayoutUnit(40), LayoutUnit(40)), c, frame_view->GetLayoutView(), 1);
}

TEST_P(PaintPropertyTreeBuilderTest, NonTranslationTransformShouldResetSubpixelPaintOffset)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      * { margin: 0; }
    ...
    // before applying transforms, because they are computed bottom-up and
    // therefore can't apply pixel snapping. Therefore apply a slop of 1px.
    CHECK_VISUAL_RECT(PhysicalRect(LayoutUnit(1), LayoutUnit(1), LayoutUnit(400), LayoutUnit(400)), b, frame_view->GetLayoutView(), 1);

    // c's painting should start at c_offset.
    LayoutObject* c = GetLayoutObjectByElementId("c");
    LayoutUnit c_offset = LayoutUnit(0.6);
    EXPECT_EQ(PhysicalOffset(c_offset, c_offset), c->FirstFragment().PaintOffset());
    // Visual rects via the non-paint properties system use enclosingIntRect
    // before applying transforms, because they are computed bottom-up and
    // therefore can't apply pixel snapping. Therefore apply a slop of 1px
    // in the transformed space (c_offset * 10 in view space) and 1px in the
    // view space.
    CHECK_VISUAL_RECT(PhysicalRect(c_offset * 10 + 1, c_offset * 10 + 1, LayoutUnit(400), LayoutUnit(400)), c, frame_view->GetLayoutView(), c_offset * 10 + 1);
}

TEST_P(PaintPropertyTreeBuilderTest, WillChangeTransformShouldResetSubpixelPaintOffset)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      * { margin: 0; }
    ...
    EXPECT_TRUE(b_properties->Transform()->RequiresCompositingForWillChangeTransform());
    // The paint offset transform should not be snapped.
    EXPECT_EQ(gfx::Vector2dF(1, 1), ToUnaliased(*b_properties->Transform()->Parent()).Get2dTranslation());
    EXPECT_EQ(PhysicalOffset(), b->FirstFragment().PaintOffset());
    // c's painting should start at c_offset.
...
TEST_P(PaintPropertyTreeBuilderTest, TransformAnimationShouldResetSubpixelPaintOffset)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      * { margin: 0; }
...
TEST_P(PaintPropertyTreeBuilderTest, PaintOffsetWithPixelSnappingThroughMultipleTransforms)
{
    ...
    LayoutObject* b = GetLayoutObjectByElementId("b");
    const ObjectPaintProperties* b_properties = b->FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(5, 7), b_properties->Transform()->Get2dTranslation());
    // The paint offset transform should be snapped from (0.7,0.7) to (1,1).
    EXPECT_EQ(gfx::Vector2dF(1, 1), ToUnaliased(*b_properties->Transform()->Parent()).Get2dTranslation());
    // The residual subpixel adjustment should be (0.7,0.7) - (1,1) = (-0.3,-0.3).
    PhysicalOffset subpixel_accumulation = PhysicalOffset(LayoutUnit(0.7), LayoutUnit(0.7)) - PhysicalOffset(1, 1);
    EXPECT_EQ(subpixel_accumulation, b->FirstFragment().PaintOffset());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(LayoutUnit(5.7), LayoutUnit(7.7), LayoutUnit(40), LayoutUnit(40)), b, frame_view->GetLayoutView());

    LayoutObject* c = GetLayoutObjectByElementId("c");
    const ObjectPaintProperties* c_properties = c->FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(11, 13), c_properties->Transform()->Get2dTranslation());
    // The paint offset should be (-0.3,-0.3) but the paint offset transform
    // should still be at (0,0) because it should be snapped.
    EXPECT_EQ(gfx::Vector2dF(), ToUnaliased(*c_properties->Transform()->Parent()).Get2dTranslation());
    // The residual subpixel adjustment should still be (-0.3,-0.3).
    EXPECT_EQ(subpixel_accumulation, c->FirstFragment().PaintOffset());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(LayoutUnit(16.7), LayoutUnit(20.7), LayoutUnit(40), LayoutUnit(40)), c, frame_view->GetLayoutView());

    // d should be painted starting at subpixelAccumulation + (0.7,0.7) =
    // (0.4,0.4).
    LayoutObject* d = GetLayoutObjectByElementId("d");
    PhysicalOffset d_paint_offset = subpixel_accumulation + PhysicalOffset(LayoutUnit(0.7), LayoutUnit(0.7));
    EXPECT_EQ(d_paint_offset, d->FirstFragment().PaintOffset());
    // Visual rects via the non-paint properties system use enclosingIntRect
    // before applying transforms, because they are computed bottom-up and
    // therefore can't apply pixel snapping. Therefore apply a slop of 1px.
    CHECK_VISUAL_RECT(PhysicalRect(LayoutUnit(16.7) + LayoutUnit(0.7), LayoutUnit(20.7) + LayoutUnit(0.7), LayoutUnit(40), LayoutUnit(40)), d,
...
TEST_P(PaintPropertyTreeBuilderTest, PaintOffsetWithPixelSnappingWithFixedPos)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(1, 0), ToUnaliased(*b_properties->Transform()->Parent()).Get2dTranslation());
    // The residual subpixel adjustment should be (0.7,0) - (1,0) = (-0.3,0).
    PhysicalOffset subpixel_accumulation = PhysicalOffset(LayoutUnit(0.7), LayoutUnit()) - PhysicalOffset(1, 0);
    EXPECT_EQ(subpixel_accumulation, b->FirstFragment().PaintOffset());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(LayoutUnit(0.7), LayoutUnit(0), LayoutUnit(40), LayoutUnit(40)), b, frame_view->GetLayoutView());

    LayoutObject* fixed = GetLayoutObjectByElementId("fixed");
    // The residual subpixel adjustment should still be (-0.3,0).
    EXPECT_EQ(subpixel_accumulation, fixed->FirstFragment().PaintOffset());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(LayoutUnit(0.7), LayoutUnit(0), LayoutUnit(40), LayoutUnit(40)), fixed, frame_view->GetLayoutView());

    // d should be painted starting at subpixelAccumulation + (0.7,0) = (0.4,0).
    LayoutObject* d = GetLayoutObjectByElementId("d");
    PhysicalOffset d_paint_offset = subpixel_accumulation + PhysicalOffset(LayoutUnit(0.7), LayoutUnit());
    EXPECT_EQ(d_paint_offset, d->FirstFragment().PaintOffset());
    // Visual rects via the non-paint properties system use enclosingIntRect
    // before applying transforms, because they are computed bottom-up and
    // therefore can't apply pixel snapping. Therefore apply a slop of 1px.
    CHECK_VISUAL_RECT(PhysicalRect(LayoutUnit(0.7) + LayoutUnit(0.7), LayoutUnit(), LayoutUnit(40), LayoutUnit(40)), d, frame_view->GetLayoutView(), 1);
}
...
TEST_P(PaintPropertyTreeBuilderTest, SvgPixelSnappingShouldResetPaintOffset)
{
    ...
    EXPECT_EQ(PhysicalOffset(LayoutUnit(0.1), LayoutUnit()), svg_with_transform.FirstFragment().PaintOffset());
    EXPECT_TRUE(svg_with_transform_properties->ReplacedContentTransform() == nullptr);

    LayoutObject& rect_with_transform = *GetLayoutObjectByElementId("rect");
    const ObjectPaintProperties* rect_with_transform_properties = rect_with_transform.FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(1, 1), rect_with_transform_properties->Transform()->Get2dTranslation());

    // Ensure there is no PaintOffset transform between the rect and the svg's
    // transform.
    EXPECT_EQ(svg_with_transform_properties->Transform(), rect_with_transform_properties->Transform()->Parent());
}
...
TEST_P(PaintPropertyTreeBuilderTest, SvgRootAndForeignObjectPixelSnapping)
{
    ...
    // remains PaintOffset.
    EXPECT_EQ(gfx::Vector2dF(9, 8), svg_properties->PaintOffsetTranslation()->Get2dTranslation());
    EXPECT_EQ(PhysicalOffset(LayoutUnit(-0.40625), LayoutUnit(0.3)), svg->FirstFragment().PaintOffset());
    EXPECT_EQ(nullptr, svg_properties->ReplacedContentTransform());
    const auto* foreign_object = GetLayoutObjectByElementId("foreign");
    const auto* foreign_object_properties = foreign_object->FirstFragment().PaintProperties();
    EXPECT_EQ(nullptr, foreign_object_properties->PaintOffsetTranslation());
...
TEST_P(PaintPropertyTreeBuilderTest, NoRenderingContextByDefault)
{
    ...
    const ObjectPaintProperties* properties = GetDocument().body()->firstChild()->GetLayoutObject()->FirstFragment().PaintProperties();
...
TEST_P(PaintPropertyTreeBuilderTest, Preserve3DCreatesSharedRenderingContext)
{
    ...
    LayoutObject* a = GetLayoutObjectByElementId("a");
    const ObjectPaintProperties* a_properties = a->FirstFragment().PaintProperties();
    LayoutObject* b = GetLayoutObjectByElementId("b");
    const ObjectPaintProperties* b_properties = b->FirstFragment().PaintProperties();
    ...
    EXPECT_EQ(a_properties->Transform()->RenderingContextId(), b_properties->Transform()->RenderingContextId());
...
TEST_P(PaintPropertyTreeBuilderTest, IntermediateElementPreventsSharedRenderingContext)
{
    SetBodyInnerHTML(R"HTML(
    ...
    const ObjectPaintProperties* parent_properties = b->FirstFragment().PaintProperties();

    EXPECT_FALSE(a_properties->Transform()->HasRenderingContext());
    EXPECT_TRUE(a_properties->Transform()->FlattensInheritedTransform());
    EXPECT_TRUE(b_properties->Transform()->HasRenderingContext());
    EXPECT_FALSE(b_properties->Transform()->FlattensInheritedTransform());
    EXPECT_NE(a_properties->Transform()->RenderingContextId(), b_properties->Transform()->RenderingContextId());

    EXPECT_EQ(parent_properties->Transform()->RenderingContextId(), b_properties->Transform()->RenderingContextId());

    CHECK_EXACT_VISUAL_RECT(PhysicalRect(8, 8, 30, 40), a, frame_view->GetLayoutView());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(8, 48, 20, 10), b, frame_view->GetLayoutView());
}

TEST_P(PaintPropertyTreeBuilderTest, IntermediateElementWithPropertiesPreventsSharedRenderingContext)
{
    SetBodyInnerHTML(R"HTML(
    <div id='parent' style='transform-style: preserve-3d'>
...
TEST_P(PaintPropertyTreeBuilderTest, FlatTransformStyleEndsRenderingContext)
{
    ...
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(8, 8, 30, 40), a, frame_view->GetLayoutView());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(8, 8, 10, 20), b, frame_view->GetLayoutView());
}
...
TEST_P(PaintPropertyTreeBuilderTest, NestedRenderingContexts)
{
    ...
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(8, 8, 50, 60), a, frame_view->GetLayoutView());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(8, 8, 10, 20), b, frame_view->GetLayoutView());
}
...
TEST_P(PaintPropertyTreeBuilderTest, FlatTransformStylePropagatesToChildren)
{
    ...
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(8, 8, 30, 40), a, frame_view->GetLayoutView());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(8, 8, 10, 10), b, frame_view->GetLayoutView());
}
...
TEST_P(PaintPropertyTreeBuilderTest, FlatteningIn3DContext)
{
    ...
    EXPECT_EQ(MakeTranslationMatrix(0, 0, 33), b_properties->Transform()->Matrix());
    EXPECT_EQ(a_properties->Transform()->RenderingContextId(), b_properties->Transform()->RenderingContextId());
    EXPECT_FALSE(b_properties->Transform()->FlattensInheritedTransform());
    ...
    EXPECT_EQ(b_properties->Transform(), &b_properties->Effect()->LocalTransformSpace());
    ...
    EXPECT_EQ(MakeTranslationMatrix(0, 0, -10), c_properties->Transform()->Matrix());
    ...
    EXPECT_EQ(MakeTranslationMatrix(0, -10, 22), d_properties->Transform()->Matrix());
    EXPECT_EQ(a_properties->Transform()->RenderingContextId(), d_properties->Transform()->RenderingContextId());
...
TEST_P(PaintPropertyTreeBuilderTest, CachedProperties)
{
    ...
    const ObjectPaintProperties* a_properties = a->GetLayoutObject()->FirstFragment().PaintProperties();
    const TransformPaintPropertyNode* a_transform_node = a_properties->Transform();
    EXPECT_EQ(gfx::Vector2dF(33, 44), a_transform_node->Get2dTranslation());

    Element* b = GetDocument().getElementById(AtomicString("b"));
    const ObjectPaintProperties* b_properties = b->GetLayoutObject()->FirstFragment().PaintProperties();
    const TransformPaintPropertyNode* b_transform_node = b_properties->Transform();
    EXPECT_EQ(gfx::Vector2dF(55, 66), b_transform_node->Get2dTranslation());

    Element* c = GetDocument().getElementById(AtomicString("c"));
    const ObjectPaintProperties* c_properties = c->GetLayoutObject()->FirstFragment().PaintProperties();
    const TransformPaintPropertyNode* c_transform_node = c_properties->Transform();
    EXPECT_EQ(gfx::Vector2dF(77, 88), c_transform_node->Get2dTranslation());

    CHECK_EXACT_VISUAL_RECT(PhysicalRect(33, 44, 50, 60), a->GetLayoutObject(), frame_view->GetLayoutView());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(88, 110, 30, 40), b->GetLayoutObject(), frame_view->GetLayoutView());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(165, 198, 10, 20), c->GetLayoutObject(), frame_view->GetLayoutView());
    ...
    // adjusted).
    b->setAttribute(html_names::kStyleAttr, AtomicString("transform: translate(111px, 222px)"));
    UpdateAllLifecyclePhasesForTest();

    EXPECT_EQ(a_properties, a->GetLayoutObject()->FirstFragment().PaintProperties());
    EXPECT_EQ(a_transform_node, a_properties->Transform());

    EXPECT_EQ(b_properties, b->GetLayoutObject()->FirstFragment().PaintProperties());
    ...
    EXPECT_EQ(c_properties, c->GetLayoutObject()->FirstFragment().PaintProperties());
    EXPECT_EQ(c_transform_node, c_properties->Transform());
    EXPECT_EQ(b_transform_node, c_transform_node->Parent()->Parent());

    CHECK_EXACT_VISUAL_RECT(PhysicalRect(33, 44, 50, 60), a->GetLayoutObject(), frame_view->GetLayoutView());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(144, 266, 50, 20), b->GetLayoutObject(), frame_view->GetLayoutView());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(221, 354, 10, 20), c->GetLayoutObject(), frame_view->GetLayoutView());
    ...
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(33, 44, 50, 60), a->GetLayoutObject(), frame_view->GetLayoutView());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(33, 44, 50, 20), b->GetLayoutObject(), frame_view->GetLayoutView());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(110, 132, 10, 20), c->GetLayoutObject(), frame_view->GetLayoutView());
    ...
    // adjusted).
    b->setAttribute(html_names::kStyleAttr, AtomicString("transform: translate(4px, 5px)"));
    UpdateAllLifecyclePhasesForTest();
    ...
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(33, 44, 50, 60), a->GetLayoutObject(), frame_view->GetLayoutView());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(37, 49, 50, 20), b->GetLayoutObject(), frame_view->GetLayoutView());
    CHECK_EXACT_VISUAL_RECT(PhysicalRect(114, 137, 10, 20), c->GetLayoutObject(), frame_view->GetLayoutView());
}
...
TEST_P(PaintPropertyTreeBuilderTest, OverflowClipContentsTreeState)
{
    ...
    LayoutBoxModelObject* clipper = To<LayoutBoxModelObject>(GetLayoutObjectByElementId("clipper"));
    const ObjectPaintProperties* clip_properties = clipper->FirstFragment().PaintProperties();
    LayoutObject* child = GetLayoutObjectByElementId("child");

    // Always create scroll translation for layout view even the document does
    // not scroll (not enough content).
    EXPECT_TRUE(DocScrollTranslation());
    EXPECT_EQ(DocScrollTranslation(), &clipper->FirstFragment().LocalBorderBoxProperties().Transform());
    EXPECT_EQ(DocContentClip(), &clipper->FirstFragment().LocalBorderBoxProperties().Clip());
    ...
    EXPECT_EQ(DocScrollTranslation(), &child->FirstFragment().LocalBorderBoxProperties().Transform());
    EXPECT_EQ(clip_properties->OverflowClip(), &child->FirstFragment().LocalBorderBoxProperties().Clip());
...
TEST_P(PaintPropertyTreeBuilderTest, ReplacedSvgContentWithIsolation)
{
    ...
    LayoutBoxModelObject* svg = To<LayoutBoxModelObject>(GetLayoutObjectByElementId("replacedsvg"));
    const ObjectPaintProperties* svg_properties = svg->FirstFragment().PaintProperties();

    EXPECT_TRUE(svg_properties->TransformIsolationNode());
    EXPECT_TRUE(svg_properties->ReplacedContentTransform());
    EXPECT_EQ(svg_properties->TransformIsolationNode()->Parent(), svg_properties->ReplacedContentTransform());
}
...
TEST_P(PaintPropertyTreeBuilderTest, ReplacedContentTransformFlattening)
{
    ...
    EXPECT_TRUE(ToUnaliased(*replaced_transform->Parent()).FlattensInheritedTransform());
}
...
TEST_P(PaintPropertyTreeBuilderTest, ContainPaintOrStyleLayoutTreeState)
{
    for (const char* containment : { "paint", "style layout" }) {
      ...
      <div id='clipper'
        ...
        auto* clipper = To<LayoutBoxModelObject>(GetLayoutObjectByElementId("clipper"));
        const ObjectPaintProperties* clip_properties = clipper->FirstFragment().PaintProperties();
        LayoutObject* child = GetLayoutObjectByElementId("child");
        const auto& clip_local_properties = clipper->FirstFragment().LocalBorderBoxProperties();
        ...
        EXPECT_EQ(clip_properties->TransformIsolationNode()->Parent(), &clip_local_properties.Transform());
        EXPECT_EQ(clip_properties->TransformIsolationNode()->Parent(), clip_properties->PaintOffsetTranslation());
        // Similarly, effect isolation node is parented to the local border box
        // properties effect.
        EXPECT_EQ(clip_properties->EffectIsolationNode()->Parent(), &clip_local_properties.Effect());
        if (strcmp(containment, "paint") == 0) {
            ...
            // properties clip.
            EXPECT_EQ(clip_properties->ClipIsolationNode()->Parent(), clip_properties->OverflowClip());
            EXPECT_EQ(clip_properties->OverflowClip()->Parent(), &clip_local_properties.Clip());
        } else {
            // Otherwise, the clip isolation node is parented to the local border box
            // properties clip directly.
            EXPECT_EQ(clip_properties->ClipIsolationNode()->Parent(), &clip_local_properties.Clip());
        }
        ...
        // node.
        EXPECT_EQ(DocScrollTranslation(), clipper->FirstFragment().LocalBorderBoxProperties().Transform().Parent());
        ...
        EXPECT_EQ(clip_properties->TransformIsolationNode(), &contents_properties.Transform());
        EXPECT_EQ(clip_properties->ClipIsolationNode(), &contents_properties.Clip());
        EXPECT_EQ(clip_properties->EffectIsolationNode(), &contents_properties.Effect());

        // Child should be using isolation nodes as its local border box properties.
        EXPECT_EQ(&contents_properties.Transform(), &child->FirstFragment().LocalBorderBoxProperties().Transform());
        EXPECT_EQ(&contents_properties.Clip(), &child->FirstFragment().LocalBorderBoxProperties().Clip());
        EXPECT_EQ(&contents_properties.Effect(), &child->FirstFragment().LocalBorderBoxProperties().Effect());
...
TEST_P(PaintPropertyTreeBuilderTest, OverflowScrollContentsTreeState)
{
    ...
    Element* clipper_element = GetDocument().getElementById(AtomicString("clipper"));
    ...
    EXPECT_EQ(DocScrollTranslation(), clipper->FirstFragment().LocalBorderBoxProperties().Transform().Parent());
    EXPECT_EQ(clip_properties->PaintOffsetTranslation(), &clipper->FirstFragment().LocalBorderBoxProperties().Transform());
    EXPECT_EQ(DocContentClip(), &clipper->FirstFragment().LocalBorderBoxProperties().Clip());

    auto contents_properties = clipper->FirstFragment().ContentsProperties();
    EXPECT_EQ(gfx::Vector2dF(30, 20), clip_properties->PaintOffsetTranslation()->Get2dTranslation());
    EXPECT_EQ(PhysicalOffset(), clipper->FirstFragment().PaintOffset());
    EXPECT_EQ(clip_properties->ScrollTranslation(), &contents_properties.Transform());
    EXPECT_EQ(clip_properties->OverflowClip(), &contents_properties.Clip());

    EXPECT_EQ(clip_properties->ScrollTranslation(), &child->FirstFragment().LocalBorderBoxProperties().Transform());
    EXPECT_EQ(clip_properties->OverflowClip(), &child->FirstFragment().LocalBorderBoxProperties().Clip());
...
TEST_P(PaintPropertyTreeBuilderTest, OverflowScrollWithRoundedRect)
{
    ...
    const ObjectPaintProperties* rounded_box_properties = rounded_box.FirstFragment().PaintProperties();
    EXPECT_CLIP_RECT(FloatRoundedRect(gfx::RectF(50, 50, 200, 200), FloatRoundedRect::Radii(50)), rounded_box_properties->InnerBorderRadiusClip());

    // Unlike the inner border radius clip, the overflow clip is inset by the
    // scrollbars (13px).
    EXPECT_CLIP_RECT(FloatRoundedRect(50, 50, 187, 187), rounded_box_properties->OverflowClip());
    EXPECT_EQ(DocContentClip(), rounded_box_properties->InnerBorderRadiusClip()->Parent());
    EXPECT_EQ(rounded_box_properties->InnerBorderRadiusClip(), rounded_box_properties->OverflowClip()->Parent());
}
...
TEST_P(PaintPropertyTreeBuilderTest, CssClipContentsTreeState)
{
    ...
    EXPECT_EQ(clip_properties->CssClip(), &clipper->FirstFragment().LocalBorderBoxProperties().Clip());
...
TEST_P(PaintPropertyTreeBuilderTest, ReplacedContentTransformContentsTreeState)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      body {
    ...
    LayoutObject& svg_with_view_box = *GetLayoutObjectByElementId("svgWithViewBox");
    const auto* paint_offset_translation = svg_with_view_box.FirstFragment().PaintProperties()->PaintOffsetTranslation();
    EXPECT_EQ(paint_offset_translation, &svg_with_view_box.FirstFragment().LocalBorderBoxProperties().Transform());
    EXPECT_EQ(DocScrollTranslation(), paint_offset_translation->Parent());
    EXPECT_EQ(gfx::Vector2dF(30, 20), paint_offset_translation->Get2dTranslation());
    EXPECT_EQ(PhysicalOffset(), svg_with_view_box.FirstFragment().PaintOffset());

    const auto* replaced_content_transform = svg_with_view_box.FirstFragment().PaintProperties()->ReplacedContentTransform();
    EXPECT_EQ(replaced_content_transform, &svg_with_view_box.FirstFragment().ContentsProperties().Transform());
    EXPECT_EQ(paint_offset_translation, replaced_content_transform->Parent());
    EXPECT_EQ(gfx::Vector2dF(-50, -50), replaced_content_transform->Get2dTranslation());
}
...
TEST_P(PaintPropertyTreeBuilderTest, OverflowHiddenScrollProperties)
{
    ...
    Element* overflow_hidden = GetDocument().getElementById(AtomicString("overflowHidden"));

    const ObjectPaintProperties* overflow_hidden_scroll_properties = overflow_hidden->GetLayoutObject()->FirstFragment().PaintProperties();
    ...
    UpdateAllLifecyclePhasesForTest();
    auto* scroll_translation = overflow_hidden_scroll_properties->ScrollTranslation();
...
TEST_P(PaintPropertyTreeBuilderTest, NestedScrollProperties)
{
    ...
    const ObjectPaintProperties* overflow_a_scroll_properties = overflow_a->GetLayoutObject()->FirstFragment().PaintProperties();
    // Because the frameView is does not scroll, overflowA's scroll should be
    // under the root.
    auto* scroll_a_translation = overflow_a_scroll_properties->ScrollTranslation();
    ...
    const ObjectPaintProperties* overflow_b_scroll_properties = overflow_b->GetLayoutObject()->FirstFragment().PaintProperties();
    // The overflow child's scroll node should be a child of the parent's
    // (overflowA) scroll node.
    auto* scroll_b_translation = overflow_b_scroll_properties->ScrollTranslation();
...
TEST_P(PaintPropertyTreeBuilderTest, PositionedScrollerIsNotNested)
{
    ...
    Element* abspos_overflow = GetDocument().getElementById(AtomicString("absposOverflow"));
    abspos_overflow->setScrollTop(41);
    Element* fixed_overflow = GetDocument().getElementById(AtomicString("fixedOverflow"));
    ...
    const ObjectPaintProperties* overflow_scroll_properties = overflow->GetLayoutObject()->FirstFragment().PaintProperties();
    ...
    EXPECT_EQ(DocScroll(), overflow_scroll_properties->ScrollTranslation()->ScrollNode()->Parent());
    EXPECT_EQ(gfx::Vector2dF(0, -37), scroll_translation->Get2dTranslation());
    ...
    const ObjectPaintProperties* abspos_overflow_scroll_properties = abspos_overflow->GetLayoutObject()->FirstFragment().PaintProperties();
    auto* abspos_scroll_translation = abspos_overflow_scroll_properties->ScrollTranslation();
    ...
    EXPECT_EQ(gfx::Vector2dF(0, -41), abspos_scroll_translation->Get2dTranslation());
    EXPECT_EQ(gfx::Rect(0, 0, 9, 7), abspos_overflow_scroll_node->ContainerRect());
    EXPECT_EQ(gfx::Rect(0, 0, 9, 4000), abspos_overflow_scroll_node->ContentsRect());

    const ObjectPaintProperties* fixed_overflow_scroll_properties = fixed_overflow->GetLayoutObject()->FirstFragment().PaintProperties();
    auto* fixed_scroll_translation = fixed_overflow_scroll_properties->ScrollTranslation();
    ...
    EXPECT_EQ(gfx::Vector2dF(0, -43), fixed_scroll_translation->Get2dTranslation());
    EXPECT_EQ(gfx::Rect(0, 0, 13, 11), fixed_overflow_scroll_node->ContainerRect());
    EXPECT_EQ(gfx::Rect(0, 0, 13, 4000), fixed_overflow_scroll_node->ContentsRect());
}
...
TEST_P(PaintPropertyTreeBuilderTest, SVGRootClip)
{
    ...
    const ClipPaintPropertyNode* clip = GetLayoutObjectByElementId("svg")->FirstFragment().PaintProperties()->OverflowClip();
    EXPECT_EQ(DocContentClip(), clip->Parent());
    EXPECT_EQ(gfx::Vector2dF(8, 8), GetLayoutObjectByElementId("svg")->FirstFragment().PaintProperties()->PaintOffsetTranslation()->Get2dTranslation());
...
TEST_P(PaintPropertyTreeBuilderTest, SVGRootNoClip)
{
    ...
    EXPECT_FALSE(GetLayoutObjectByElementId("svg")->FirstFragment().PaintProperties()->OverflowClip());
}
...
TEST_P(PaintPropertyTreeBuilderTest, MainThreadScrollReasonsWithoutScrolling)
{
    ...
    EXPECT_EQ(overflow->GetLayoutObject()->FirstFragment().PaintProperties()->ScrollTranslation(), nullptr);
}
...
TEST_P(PaintPropertyTreeBuilderTest, PaintOffsetsUnderMultiColumnScrolled)
{
    ...
    scroller->GetScrollableArea()->ScrollBy(ScrollOffset(0, 300), mojom::blink::ScrollType::kUser);
    UpdateAllLifecyclePhasesForTest();

    EXPECT_EQ(gfx::Vector2dF(8, 8), scroller->FirstFragment().PaintProperties()->PaintOffsetTranslation()->Get2dTranslation());
}

TEST_P(PaintPropertyTreeBuilderTest, PaintOffsetsUnderMultiColumnWithVisualOverflow)
{
    SetBodyInnerHTML(R"HTML(
...
TEST_P(PaintPropertyTreeBuilderTest, PaintOffsetsUnderMultiColumnWithScrollableOverflow)
{
    SetBodyInnerHTML(R"HTML(
    <div style='columns: 2; width: 300px; column-gap: 0; height: 100px'>
...
TEST_P(PaintPropertyTreeBuilderTest, PaintOffsetUnderMulticolumnScrollFixedPos)
{
    SetBodyInnerHTML(R"HTML(
    ...
    GetDocument().View()->LayoutViewport()->ScrollBy(ScrollOffset(0, 25), mojom::blink::ScrollType::kUser);
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintPropertyTreeBuilderTest, FragmentsUnderMultiColumn)
{
    ...
    EXPECT_EQ(PhysicalOffset(380, 380), bottom_right->FirstFragment().PaintOffset());
}

TEST_P(PaintPropertyTreeBuilderTest, FragmentsUnderMultiColumnVerticalRLWithOverflow)
{
    SetBodyInnerHTML(R"HTML(
    <style>body { margin: 0; }</style>
    ...
    LayoutObject* thread = GetLayoutObjectByElementId("multicol")->SlowFirstChild();
    LayoutObject* container = GetLayoutObjectByElementId("container");
...
TEST_P(PaintPropertyTreeBuilderTest, CompositedUnderMultiColumn)
{
    ...
    LayoutObject* non_composited_child = GetLayoutObjectByElementId("non-composited-child");
    LayoutObject* composited_child = GetLayoutObjectByElementId("composited-child");
    ...
    EXPECT_EQ(PhysicalOffset(0, 0), FragmentAt(non_composited_child, 0).PaintOffset());
    EXPECT_EQ(1u, FragmentAt(non_composited_child, 0).FragmentID());
    EXPECT_EQ(PhysicalOffset(0, 0), FragmentAt(non_composited_child, 1).PaintOffset());
    EXPECT_EQ(2u, FragmentAt(non_composited_child, 1).FragmentID());
    EXPECT_EQ(1u, NumFragments(composited_child));
    EXPECT_EQ(PhysicalOffset(0, 0), FragmentAt(composited_child, 0).PaintOffset());
...
TEST_P(PaintPropertyTreeBuilderTest, MultiColumnInlineRelativeAndSpannerAndAbsPos)
{
    SetBodyInnerHTML(R"HTML(
    <div style='columns:2; width: 200px; column-gap: 0'>
    ...
    EXPECT_TRUE(GetLayoutObjectByElementId("absolute")->Container()->IsLayoutBlock());
}
...
TEST_P(PaintPropertyTreeBuilderTest, BecomingUnfragmented)
{
    ...
    EXPECT_EQ(PhysicalOffset(LayoutUnit(208), LayoutUnit(8)), target->FirstFragment().PaintOffset());
    Element* target_element = GetDocument().getElementById(AtomicString("target"));

    target_element->setAttribute(html_names::kStyleAttr, AtomicString("position: absolute"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(0u, target->FirstFragment().FragmentID());
    EXPECT_EQ(PhysicalOffset(LayoutUnit(8), LayoutUnit(28)), target->FirstFragment().PaintOffset());
}

TEST_P(PaintPropertyTreeBuilderTest, Reflection)
{
    SetBodyInnerHTML("<div id='filter' style='-webkit-box-reflect: below; height:1000px;'>"
                     "</div>");
    const ObjectPaintProperties* filter_properties = GetLayoutObjectByElementId("filter")->FirstFragment().PaintProperties();
    EXPECT_EQ(DocScrollTranslation(), filter_properties->PaintOffsetTranslation()->Parent());
    EXPECT_EQ(gfx::Vector2dF(8, 8), filter_properties->PaintOffsetTranslation()->Get2dTranslation());
    EXPECT_EQ(filter_properties->Filter()->Parent(), DocEffect());
    EXPECT_EQ(filter_properties->PaintOffsetTranslation(), &filter_properties->Filter()->LocalTransformSpace());
    EXPECT_EQ(DocContentClip(), filter_properties->Filter()->OutputClip());
}

TEST_P(PaintPropertyTreeBuilderTest, SimpleFilter)
{
    SetBodyInnerHTML("<div id='filter' style='filter:opacity(0.5); height:1000px;'>"
                     "</div>");
    const ObjectPaintProperties* filter_properties = GetLayoutObjectByElementId("filter")->FirstFragment().PaintProperties();
    EXPECT_FALSE(filter_properties->PaintOffsetTranslation());
    EXPECT_EQ(filter_properties->Filter()->Parent(), DocEffect());
    EXPECT_FALSE(filter_properties->PixelMovingFilterClipExpander());
    EXPECT_EQ(DocScrollTranslation(), &filter_properties->Filter()->LocalTransformSpace());
    EXPECT_EQ(DocContentClip(), filter_properties->Filter()->OutputClip());
}

TEST_P(PaintPropertyTreeBuilderTest, PixelMovingFilter)
{
    SetBodyInnerHTML("<div id='filter' style='filter:blur(10px); height:1000px;'>"
                     "</div>");
    const ObjectPaintProperties* filter_properties = GetLayoutObjectByElementId("filter")->FirstFragment().PaintProperties();
    EXPECT_FALSE(filter_properties->PaintOffsetTranslation());
...
{
    SetBodyInnerHTML("<div id='filter' style='height:1000px; will-change: filter'>");
...
TEST_P(PaintPropertyTreeBuilderTest, FilterReparentClips)
{
    ...
    const ObjectPaintProperties* clip_properties = GetLayoutObjectByElementId("clip")->FirstFragment().PaintProperties();
    const ObjectPaintProperties* filter_properties = GetLayoutObjectByElementId("filter")->FirstFragment().PaintProperties();
    EXPECT_TRUE(DocEffect());
    EXPECT_EQ(clip_properties->OverflowClip(), filter_properties->Filter()->OutputClip());
    EXPECT_EQ(DocScrollTranslation(), &filter_properties->Filter()->LocalTransformSpace());

    const auto& child_paint_state = GetLayoutObjectByElementId("child")->FirstFragment().LocalBorderBoxProperties();

    // This will change once we added clip expansion node.
    EXPECT_EQ(filter_properties->Filter()->OutputClip(), &child_paint_state.Clip());
...
TEST_P(PaintPropertyTreeBuilderTest, TransformOriginWithAndWithoutMotionPath)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(50, 150), motion_path_properties->Offset()->Get2dTranslation());
    EXPECT_EQ(gfx::Point3F(50, 50, 0), motion_path_properties->Offset()->Origin());
...
TEST_P(PaintPropertyTreeBuilderTest, ChangePositionUpdateDescendantProperties)
{
    ...
    EXPECT_EQ(ancestor->FirstFragment().PaintProperties()->OverflowClip(), &descendant->FirstFragment().LocalBorderBoxProperties().Clip());

    To<Element>(ancestor->GetNode())->setAttribute(html_names::kStyleAttr, AtomicString("position: static"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_NE(ancestor->FirstFragment().PaintProperties()->OverflowClip(), &descendant->FirstFragment().LocalBorderBoxProperties().Clip());
}

TEST_P(PaintPropertyTreeBuilderTest, TransformNodeNotAnimatedStillHasCompositorElementId)
{
    ...
    EXPECT_NE(CompositorElementId(), properties->Transform()->GetCompositorElementId());
}

TEST_P(PaintPropertyTreeBuilderTest, EffectNodeNotAnimatedStillHasCompositorElementId)
{
...
TEST_P(PaintPropertyTreeBuilderTest, TransformNodeAnimatedHasCompositorElementId)
{
    LoadTestData("transform-animation.html");
    const ObjectPaintProperties* properties = PaintPropertiesForElement("target");
    EXPECT_TRUE(properties->Transform());
...
TEST_P(PaintPropertyTreeBuilderTest, FloatUnderInline)
{
    ...
    EXPECT_EQ(effect, &target->FirstFragment().LocalBorderBoxProperties().Effect());
}
...
TEST_P(PaintPropertyTreeBuilderTest, ScrollNodeHasCompositorElementId)
{
    ...
    EXPECT_EQ(CompositorElementId(), properties->ScrollTranslation()->GetCompositorElementId());
    EXPECT_NE(CompositorElementId(), properties->Scroll()->GetCompositorElementId());
}
...
TEST_P(PaintPropertyTreeBuilderTest, OverflowClipSubpixelPosition)
{
    ...
    EXPECT_EQ(PhysicalOffset(LayoutUnit(31.5), LayoutUnit(20)), clipper->FirstFragment().PaintOffset());
    // Result is pixel-snapped.
    EXPECT_EQ(FloatClipRect(gfx::RectF(31.5, 20, 400, 300)), clip_properties->OverflowClip()->LayoutClipRect());
    EXPECT_EQ(FloatRoundedRect(32, 20, 400, 300), clip_properties->OverflowClip()->PaintClipRect());
}
...
TEST_P(PaintPropertyTreeBuilderTest, MaskSimple)
{
    ...
    EXPECT_EQ(mask_clip, &target->FirstFragment().LocalBorderBoxProperties().Clip());
    EXPECT_EQ(DocContentClip(), mask_clip->Parent());
    EXPECT_EQ(FloatClipRect(gfx::RectF(8, 8, 300, 200.5)), mask_clip->LayoutClipRect());
    EXPECT_EQ(FloatRoundedRect(8, 8, 300, 201), mask_clip->PaintClipRect());

    EXPECT_EQ(properties->Effect(), &target->FirstFragment().LocalBorderBoxProperties().Effect());
    EXPECT_TRUE(DocEffect());
...
TEST_P(PaintPropertyTreeBuilderTest, MaskEscapeClip)
{
    ...
    const ObjectPaintProperties* target_properties = PaintPropertiesForElement("target");
    ...
    const auto* target = GetLayoutObjectByElementId("target");
    const auto& scroll_translation = target->FirstFragment().LocalBorderBoxProperties().Transform();

    const ObjectPaintProperties* scroll_properties = PaintPropertiesForElement("scroll");

    EXPECT_EQ(DocContentClip(), overflow_clip1->Parent());
    EXPECT_CLIP_RECT(FloatRoundedRect(0, 0, 300, 200), &ToUnaliased(*overflow_clip1));
    EXPECT_EQ(scroll_properties->PaintOffsetTranslation(), &ToUnaliased(*overflow_clip1).LocalTransformSpace());
    ...
    EXPECT_EQ(target_properties->Effect(), &target->FirstFragment().LocalBorderBoxProperties().Effect());
    EXPECT_TRUE(DocEffect());
    ...
    EXPECT_EQ(DocScrollTranslation(), &absolute->FirstFragment().LocalBorderBoxProperties().Transform());
    EXPECT_EQ(mask_clip, &absolute->FirstFragment().LocalBorderBoxProperties().Clip());
}
...
TEST_P(PaintPropertyTreeBuilderTest, MaskInline)
{
    ...
    EXPECT_EQ(mask_clip, &overflowing->FirstFragment().LocalBorderBoxProperties().Clip());
    EXPECT_EQ(properties->Effect(), &overflowing->FirstFragment().LocalBorderBoxProperties().Effect());
}
...
TEST_P(PaintPropertyTreeBuilderTest, SVGResource)
{
    ...
    const ObjectPaintProperties* transform_inside_marker_properties = PaintPropertiesForElement("transformInsideMarker");
    const ObjectPaintProperties* transform_outside_path_properties = PaintPropertiesForElement("transformOutsidePath");
    const ObjectPaintProperties* svg_properties = PaintPropertiesForElement("svg");

    // The <marker> object resets to a new paint property tree, so the
    // transform within it should have the root as parent.
    EXPECT_EQ(&TransformPaintPropertyNode::Root(), transform_inside_marker_properties->Transform()->Parent());

    // Whereas this is not true of the transform above the path.
    EXPECT_EQ(svg_properties->PaintOffsetTranslation(), transform_outside_path_properties->Transform()->Parent());
}
...
TEST_P(PaintPropertyTreeBuilderTest, SVGHiddenResource)
{
    ...
    const ObjectPaintProperties* transform_inside_symbol_properties = PaintPropertiesForElement("transformInsideSymbol");
    const ObjectPaintProperties* transform_outside_use_properties = PaintPropertiesForElement("transformOutsideUse");
    const ObjectPaintProperties* svg_properties = PaintPropertiesForElement("svg");

    // The <marker> object resets to a new paint property tree, so the
    // transform within it should have the root as parent.
    EXPECT_EQ(&TransformPaintPropertyNode::Root(), transform_inside_symbol_properties->Transform()->Parent());

    // Whereas this is not true of the transform above the path.
    EXPECT_EQ(svg_properties->PaintOffsetTranslation(), transform_outside_use_properties->Transform()->Parent());
}
...
TEST_P(PaintPropertyTreeBuilderTest, SVGRootBlending)
{
    ...
    const auto* html_properties = GetDocument().documentElement()->GetLayoutObject()->FirstFragment().PaintProperties();
...
TEST_P(PaintPropertyTreeBuilderTest, ScrollBoundsOffset)
{
    ...
    const ObjectPaintProperties* scroll_properties = scroller->GetLayoutObject()->FirstFragment().PaintProperties();
    ...
    EXPECT_EQ(gfx::Vector2dF(7, 11), paint_offset_translation->Get2dTranslation());
    // And the scroll node should not.
    EXPECT_EQ(gfx::Rect(0, 0, 100, 100), scroll_node->ContainerRect());

    scroller->setAttribute(html_names::kStyleAttr, AtomicString("border: 20px solid black;"));
    UpdateAllLifecyclePhasesForTest();
    // The paint offset node should be offset by the margin.
    EXPECT_EQ(gfx::Vector2dF(7, 11), paint_offset_translation->Get2dTranslation());
    ...
    EXPECT_EQ(gfx::Rect(20, 20, 100, 100), scroll_node->ContainerRect());
    EXPECT_EQ(gfx::Vector2dF(7, 11), scroll_properties->PaintOffsetTranslation()->Get2dTranslation());
}
...
TEST_P(PaintPropertyTreeBuilderTest, BackfaceHidden)
{
    ...
    const auto* paint_offset_translation = target_properties->PaintOffsetTranslation();
    ASSERT_NE(nullptr, paint_offset_translation);
    EXPECT_EQ(gfx::Vector2dF(60, 50), paint_offset_translation->Get2dTranslation());
    EXPECT_EQ(TransformPaintPropertyNode::BackfaceVisibility::kInherited, paint_offset_translation->GetBackfaceVisibilityForTesting());

    const auto* transform = target_properties->Transform();
    ASSERT_NE(nullptr, transform);
    EXPECT_TRUE(transform->IsIdentity());
    EXPECT_EQ(TransformPaintPropertyNode::BackfaceVisibility::kHidden, transform->GetBackfaceVisibilityForTesting());

    To<Element>(target->GetNode())->setAttribute(html_names::kStyleAttr, g_empty_atom);
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintPropertyTreeBuilderTest, FrameBorderRadius)
{
    ...
    EXPECT_CLIP_RECT(FloatRoundedRect(gfx::RectF(28, 28, 200, 200), FloatRoundedRect::Radii(30)), border_radius_clip);
...
TEST_P(PaintPropertyTreeBuilderTest, ImageBorderRadius)
{
    ...
    EXPECT_CLIP_RECT(FloatRoundedRect(gfx::RectF(18, 18, 50, 50), FloatRoundedRect::Radii(0)), overflow_clip);
    ...
    ASSERT_NE(nullptr, border_radius_clip);
    EXPECT_EQ(DocContentClip(), border_radius_clip->Parent());
    EXPECT_CLIP_RECT(FloatRoundedRect(gfx::RectF(18, 18, 50, 50), FloatRoundedRect::Radii(20)), border_radius_clip);
}
...
TEST_P(PaintPropertyTreeBuilderTest, OverflowControlsClipSubpixel)
{
    ...
    ASSERT_NE(nullptr, properties1);
    const auto* overflow_controls_clip = properties1->OverflowControlsClip();
    EXPECT_EQ(FloatClipRect(gfx::RectF(0, 0, 5.5, 50)), overflow_controls_clip->LayoutClipRect());
    EXPECT_EQ(FloatRoundedRect(0, 0, 6, 50), overflow_controls_clip->PaintClipRect());
...
TEST_P(PaintPropertyTreeBuilderTest, FragmentPaintOffsetUnderOverflowScroll)
{
    ...
    EXPECT_NE(nullptr, PaintPropertiesForElement("container")->PaintOffsetTranslation());
...
TEST_P(PaintPropertyTreeBuilderTest, ElementCaptureEffectNode)
{
    ...
    element->SetRestrictionTargetId(std::make_unique<RestrictionTargetId>(base::Token::CreateRandom()));
    UpdateAllLifecyclePhasesForTest();
    ...
    UpdateAllLifecyclePhasesForTest();
    const ObjectPaintProperties* paint_properties = element->GetLayoutObject()->FirstFragment().PaintProperties();
...
TEST_P(PaintPropertyTreeBuilderTest, EmptyClipPathSubpixelOffset)
{
    ...
    const auto* clip_path_clip = target->FirstFragment().PaintProperties()->ClipPathClip();
    ASSERT_TRUE(clip_path_clip);
    EXPECT_EQ(gfx::RectF(0.25, 0.75, 0, 0), clip_path_clip->LayoutClipRect().Rect());
...
TEST_P(PaintPropertyTreeBuilderTest, RootHasCompositedScrolling)
{
    ...
    Element* force_scroll_element = GetDocument().getElementById(AtomicString("forceScroll"));
...
TEST_P(PaintPropertyTreeBuilderTest, IframeDoesNotRequireCompositedScrolling)
{
    ...
    EXPECT_FALSE(DocScrollTranslation(&ChildDocument())->HasDirectCompositingReasons());
}
...
TEST_P(PaintPropertyTreeBuilderTest, ClipHitTestChangeDoesNotCauseFullRepaint)
{
    ...
    GetDocument().body()->setAttribute(html_names::kClassAttr, AtomicString("noscrollbars"));
    UpdateAllLifecyclePhasesExceptPaint();
    EXPECT_FALSE(child_layer->SelfNeedsRepaint());
}
...
TEST_P(PaintPropertyTreeBuilderTest, ClipPathInheritanceWithoutMutation)
{
    ...
    const auto& old_clip_state = child->FirstFragment().LocalBorderBoxProperties().Clip();

    child->SetNeedsPaintPropertyUpdate();
    UpdateAllLifecyclePhasesForTest();

    const auto& new_clip_state = child->FirstFragment().LocalBorderBoxProperties().Clip();
...
TEST_P(PaintPropertyTreeBuilderTest, CompositedLayerSkipsFragmentClip)
{
    ...
    const auto* composited_with_clip_properties = PaintPropertiesForElement("composited-with-clip");
    EXPECT_EQ(DocContentClip(), composited_with_clip_properties->OverflowClip()->Parent());
    EXPECT_EQ(composited_with_clip_properties->OverflowClip(), &GetLayoutObjectByElementId("child-clipped")->FirstFragment().LocalBorderBoxProperties().Clip());

    EXPECT_EQ(DocContentClip(), &GetLayoutObjectByElementId("composited-without-clip")->FirstFragment().LocalBorderBoxProperties().Clip());
    EXPECT_EQ(DocContentClip(), &GetLayoutObjectByElementId("child-unclipped")->FirstFragment().LocalBorderBoxProperties().Clip());
}
...
TEST_P(PaintPropertyTreeBuilderTest, CompositedLayerUnderClipUnderMulticol)
{
    ...
    EXPECT_EQ(clip_properties->OverflowClip(), &composited->FirstFragment().LocalBorderBoxProperties().Clip());
}
...
TEST_P(PaintPropertyTreeBuilderTest, RepeatingFixedPositionInPagedMedia)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        EXPECT_EQ(gfx::Vector2dF(20, 20 + 400 * i), properties->PaintOffsetTranslation()->Get2dTranslation());
    ...
    for (int i = 0; i < 3; i++) {
        ...
        EXPECT_EQ(FragmentAt(fixed, i).PaintOffset() + PhysicalOffset(0, 10), fragment.PaintOffset());
    }
...
TEST_P(PaintPropertyTreeBuilderTest, RepeatingFixedPositionWithTransformInPagedMedia)
{
    SetBodyInnerHTML(R"HTML(
    <div id="fixed" style="position: fixed; top: 20px; left: 20px;
    ...
    for (wtf_size_t i = 0; i < 3; i++) {
        ...
        EXPECT_EQ(gfx::Vector2dF(20, 20 + i * 400), properties->PaintOffsetTranslation()->Get2dTranslation());
        EXPECT_EQ(gfx::Vector2dF(10, 0), properties->Transform()->Get2dTranslation());
        EXPECT_EQ(properties->PaintOffsetTranslation(), properties->Transform()->Parent());
    ...
    for (wtf_size_t i = 0; i < 3; i++) {
        ...
        EXPECT_EQ(FragmentAt(fixed, i).PaintProperties()->Transform(), &fragment.LocalBorderBoxProperties().Transform());
        EXPECT_EQ(fragment.FragmentID(), i);
    }
...
TEST_P(PaintPropertyTreeBuilderTest, FloatPaintOffsetInContainerWithScrollbars)
{
    SetBodyInnerHTML(R"HTML(
    <style>
    ...
    auto paint_offset = [this](const char* id) { return GetLayoutObjectByElementId(id)->FirstFragment().PaintOffset(); };
...
TEST_P(PaintPropertyTreeBuilderTest, LayoutMenuListHasOverlowAndLocalBorderBoxProperties)
{
    SetBodyInnerHTML(R"HTML(
    <!doctype HTML>
    ...
    const auto& fragment = GetDocument().getElementById(AtomicString("selection"))->GetLayoutObject()->FirstFragment();
    ...
    EXPECT_EQ(&fragment.ContentsProperties().Clip(), fragment.PaintProperties()->OverflowClip());
}
...
TEST_P(PaintPropertyTreeBuilderTest, EmptyClipFragments)
{
    ...
    const auto* wrapper = GetDocument().getElementById(AtomicString("wrapper"))->GetLayoutObject();
...
TEST_P(PaintPropertyTreeBuilderTest, StickyConstraintChain)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(0, 60), outer_properties->StickyTranslation()->Get2dTranslation());
    ASSERT_NE(nullptr, outer_properties->StickyTranslation()->GetStickyConstraint());
    EXPECT_EQ(CompositorElementId(), outer_properties->StickyTranslation()->GetStickyConstraint()->nearest_element_shifting_sticky_box);
    ...
    EXPECT_TRUE(middle_properties->StickyTranslation()->RequiresCompositingForStickyPosition());
    EXPECT_EQ(gfx::Vector2dF(0, 15), middle_properties->StickyTranslation()->Get2dTranslation());
    ASSERT_NE(nullptr, middle_properties->StickyTranslation()->GetStickyConstraint());
    EXPECT_EQ(CompositorElementId(), middle_properties->StickyTranslation()->GetStickyConstraint()->nearest_element_shifting_sticky_box);
    ...
    EXPECT_TRUE(inner_properties->StickyTranslation()->RequiresCompositingForStickyPosition());
    EXPECT_EQ(gfx::Vector2dF(0, 20), inner_properties->StickyTranslation()->Get2dTranslation());
    ASSERT_NE(nullptr, inner_properties->StickyTranslation()->GetStickyConstraint());
    EXPECT_EQ(middle_properties->StickyTranslation()->GetCompositorElementId(),
        inner_properties->StickyTranslation()->GetStickyConstraint()->nearest_element_shifting_sticky_box);
    EXPECT_EQ(outer_properties->StickyTranslation()->GetCompositorElementId(),
...
TEST_P(PaintPropertyTreeBuilderTest, StickyUnderOverflowHidden)
{
    ...
    // offset animation.
    EXPECT_TRUE(outer_properties->StickyTranslation()->RequiresCompositingForStickyPosition());
    EXPECT_EQ(gfx::Vector2dF(0, 10), outer_properties->StickyTranslation()->Get2dTranslation());
    EXPECT_EQ(nullptr, outer_properties->StickyTranslation()->GetStickyConstraint());

    const auto* middle_properties = PaintPropertiesForElement("middle");
    ASSERT_TRUE(middle_properties && middle_properties->StickyTranslation());
    EXPECT_TRUE(middle_properties->StickyTranslation()->RequiresCompositingForStickyPosition());
    EXPECT_EQ(gfx::Vector2dF(0, 15), middle_properties->StickyTranslation()->Get2dTranslation());
    EXPECT_EQ(nullptr, middle_properties->StickyTranslation()->GetStickyConstraint());

    const auto* inner_properties = PaintPropertiesForElement("inner");
    ASSERT_TRUE(inner_properties && inner_properties->StickyTranslation());
    EXPECT_TRUE(inner_properties->StickyTranslation()->RequiresCompositingForStickyPosition());
    EXPECT_EQ(gfx::Vector2dF(0, 20), inner_properties->StickyTranslation()->Get2dTranslation());
    EXPECT_EQ(nullptr, inner_properties->StickyTranslation()->GetStickyConstraint());
    ...
    EXPECT_TRUE(outer_properties->StickyTranslation()->RequiresCompositingForStickyPosition());
    EXPECT_EQ(gfx::Vector2dF(0, 60), outer_properties->StickyTranslation()->Get2dTranslation());
    EXPECT_NE(nullptr, outer_properties->StickyTranslation()->GetStickyConstraint());

    ASSERT_TRUE(middle_properties && middle_properties->StickyTranslation());
    EXPECT_TRUE(middle_properties->StickyTranslation()->RequiresCompositingForStickyPosition());
    EXPECT_EQ(gfx::Vector2dF(0, 15), middle_properties->StickyTranslation()->Get2dTranslation());
    EXPECT_NE(nullptr, middle_properties->StickyTranslation()->GetStickyConstraint());

    ASSERT_TRUE(inner_properties && inner_properties->StickyTranslation());
    EXPECT_TRUE(inner_properties->StickyTranslation()->RequiresCompositingForStickyPosition());
    EXPECT_EQ(gfx::Vector2dF(0, 20), inner_properties->StickyTranslation()->Get2dTranslation());
    EXPECT_NE(nullptr, inner_properties->StickyTranslation()->GetStickyConstraint());
}
...
TEST_P(PaintPropertyTreeBuilderTest, StickyUnderScrollerWithoutOverflow)
{
    ...
    const auto* outer_properties = PaintPropertiesForElement("outer");
    ASSERT_TRUE(outer_properties && outer_properties->StickyTranslation());
    EXPECT_FALSE(outer_properties->StickyTranslation()->RequiresCompositingForStickyPosition());
    EXPECT_EQ(gfx::Vector2dF(0, 10), outer_properties->StickyTranslation()->Get2dTranslation());
    EXPECT_EQ(nullptr, outer_properties->StickyTranslation()->GetStickyConstraint());

    const auto* middle_properties = PaintPropertiesForElement("middle");
    ASSERT_TRUE(middle_properties && middle_properties->StickyTranslation());
    EXPECT_FALSE(middle_properties->StickyTranslation()->RequiresCompositingForStickyPosition());
    EXPECT_EQ(gfx::Vector2dF(0, 15), middle_properties->StickyTranslation()->Get2dTranslation());
    EXPECT_EQ(nullptr, middle_properties->StickyTranslation()->GetStickyConstraint());

    const auto* inner_properties = PaintPropertiesForElement("inner");
    ASSERT_TRUE(inner_properties && inner_properties->StickyTranslation());
    EXPECT_FALSE(inner_properties->StickyTranslation()->RequiresCompositingForStickyPosition());
    EXPECT_EQ(gfx::Vector2dF(0, 20), inner_properties->StickyTranslation()->Get2dTranslation());
    EXPECT_EQ(nullptr, inner_properties->StickyTranslation()->GetStickyConstraint());
}
...
TEST_P(PaintPropertyTreeBuilderTest, WillChangeOpacityInducesAnEffectNode)
{
    ...
    UpdateAllLifecyclePhasesExceptPaint();
    EXPECT_FALSE(To<LayoutBox>(div->GetLayoutObject())->Layer()->SelfNeedsRepaint());
...
TEST_P(PaintPropertyTreeBuilderTest, EffectOutputClipWithFixedDescendant)
{
    ...
    EXPECT_EQ(DocContentClip(), PaintPropertiesForElement("target1")->Effect()->OutputClip());
    // OutputClip is null because the fixed descendant escapes the effect's
    // current clip.
    EXPECT_EQ(nullptr, PaintPropertiesForElement("target2")->Effect()->OutputClip());
    EXPECT_EQ(PaintPropertiesForElement("clip3")->OverflowClip(), PaintPropertiesForElement("target3")->Effect()->OutputClip());
    EXPECT_EQ(PaintPropertiesForElement("clip4")->OverflowClip(), PaintPropertiesForElement("target4")->Effect()->OutputClip());
    EXPECT_EQ(DocContentClip(), PaintPropertiesForElement("target5")->Effect()->OutputClip());
}
...
TEST_P(PaintPropertyTreeBuilderTest, MainFrameDoesntClipContent)
{
    ...
    EXPECT_TRUE(GetLayoutView().FirstFragment().PaintProperties()->OverflowClip());

    GetPage().GetSettings().SetMainFrameClipsContent(false);
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(GetLayoutView().FirstFragment().PaintProperties()->OverflowClip());
...
TEST_P(PaintPropertyTreeBuilderTest, SVGRootCompositedClipPathSimple)
{
    ...
    EXPECT_CLIP_RECT(FloatRoundedRect(gfx::RectF(75, 0, 150, 150), 75), clip_path_clip);
...
TEST_P(PaintPropertyTreeBuilderTest, SimpleOpacityChangeDoesNotCausePacUpdate)
{
    ...
    cc::EffectNode* cc_effect
        = GetChromeClient().layer_tree_host()->property_trees()->effect_tree_mutable().FindNodeFromElementId(properties->Effect()->GetCompositorElementId());
    ASSERT_TRUE(cc_effect);
    EXPECT_FLOAT_EQ(cc_effect->opacity, 0.5f);
    EXPECT_TRUE(cc_effect->effect_changed);
    EXPECT_FALSE(GetChromeClient().layer_tree_host()->property_trees()->effect_tree().needs_update());
    ...
    EXPECT_TRUE(cc_effect->effect_changed);
    EXPECT_EQ(pac->NeedsUpdate(), PaintArtifactCompositor::UpdateType::kNone);
    EXPECT_TRUE(GetChromeClient().layer_tree_host()->property_trees()->effect_tree().needs_update());
}
...
TEST_P(PaintPropertyTreeBuilderTest, SimpleScrollChangeDoesNotCausePacUpdate)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(0, 0), properties->ScrollTranslation()->Get2dTranslation());
    EXPECT_EQ(pac->NeedsUpdate(), PaintArtifactCompositor::UpdateType::kNone);

    auto* property_trees = GetChromeClient().layer_tree_host()->property_trees();
    const auto* cc_scroll_node = property_trees->scroll_tree().FindNodeFromElementId(properties->ScrollTranslation()->ScrollNode()->GetCompositorElementId());
    ASSERT_TRUE(cc_scroll_node);

    const auto* cc_transform_node = property_trees->transform_tree().Node(cc_scroll_node->transform_id);
    ...
    auto current_scroll_offset = property_trees->scroll_tree().current_scroll_offset(properties->ScrollTranslation()->ScrollNode()->GetCompositorElementId());
    ...
    EXPECT_EQ(gfx::Vector2dF(0, -10), properties->ScrollTranslation()->Get2dTranslation());
    EXPECT_EQ(pac->NeedsUpdate(), PaintArtifactCompositor::UpdateType::kNone);
    EXPECT_TRUE(cc_transform_node->local.IsIdentity());
    EXPECT_FLOAT_EQ(cc_transform_node->scroll_offset.x(), 0);
    EXPECT_FLOAT_EQ(cc_transform_node->scroll_offset.y(), 10);
    current_scroll_offset = property_trees->scroll_tree().current_scroll_offset(properties->ScrollTranslation()->ScrollNode()->GetCompositorElementId());
    EXPECT_FLOAT_EQ(current_scroll_offset.x(), 0);
...
TEST_P(PaintPropertyTreeBuilderTest, SimpleStickyTranslationChangeDoesNotCausePacUpdate)
{
    SetBodyInnerHTML(R"HTML(
    ...
    auto* property_trees = GetChromeClient().layer_tree_host()->property_trees();
    const auto* cc_transform_node = property_trees->transform_tree().FindNodeFromElementId(sticky_translation->GetCompositorElementId());
    ASSERT_TRUE(cc_transform_node);
...
TEST_P(PaintPropertyTreeBuilderTest, NonCompositedTransformChangeCausesPacUpdate)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      #outer {
    ...
    EXPECT_EQ(GetDocument().View()->GetPaintArtifactCompositor()->NeedsUpdate(), PaintArtifactCompositor::UpdateType::kNone);

    Element* outer = GetDocument().getElementById(AtomicString("outer"));
    outer->setAttribute(html_names::kStyleAttr, AtomicString("transform: translateY(10px)"));
    UpdateAllLifecyclePhasesExceptPaint();

    EXPECT_EQ(GetDocument().View()->GetPaintArtifactCompositor()->NeedsUpdate(), PaintArtifactCompositor::UpdateType::kFull);
}
...
TEST_P(PaintPropertyTreeBuilderTest, VideoClipRect)
{
    ...
    LocalFrameView* frame_view = GetDocument().View();
    frame_view->UpdateAllLifecyclePhasesForTest();
    const ObjectPaintProperties* video_element_properties = video_element->GetLayoutObject()->FirstFragment().PaintProperties();
    ...
    EXPECT_CLIP_RECT(FloatRoundedRect(0, 0, 320, 240), video_element_properties->OverflowClip());
    ...
    frame_view->UpdateAllLifecyclePhasesForTest();
    EXPECT_CLIP_RECT(FloatRoundedRect(10, 10, 320, 240), video_element_properties->OverflowClip());
}
...
{
    SetBodyInnerHTML("<div id='container' style='" ALL_PROPERTY_STYLES "'>T</div>");
    EXPECT_TRUE(PaintPropertiesForElement("container"));
    auto* text = GetDocument().getElementById(AtomicString("container"))->firstChild()->GetLayoutObject();
...
{
    SetBodyInnerHTML("<span id='container' style='" ALL_PROPERTY_STYLES "'>T</span>");
    EXPECT_TRUE(PaintPropertiesForElement("container"));
    auto* text = GetDocument().getElementById(AtomicString("container"))->firstChild()->GetLayoutObject();
    ASSERT_TRUE(text->IsText());
    EXPECT_FALSE(text->FirstFragment().PaintProperties());
...
{
    SetBodyInnerHTML("<svg><text id='container' style='" ALL_PROPERTY_STYLES "'>T</text>");
    EXPECT_TRUE(PaintPropertiesForElement("container"));
    auto* text = GetDocument().getElementById(AtomicString("container"))->firstChild()->GetLayoutObject();
    ASSERT_TRUE(text->IsText());
    EXPECT_FALSE(text->FirstFragment().PaintProperties());
...
TEST_P(PaintPropertyTreeBuilderTest, IsAffectedByOuterViewportBoundsDelta)
{
    ...
    auto check_result = [&](const ObjectPaintProperties* properties, bool expected) {
        ASSERT_TRUE(properties);
        ASSERT_TRUE(properties->PaintOffsetTranslation());
        EXPECT_EQ(expected, properties->PaintOffsetTranslation()->IsAffectedByOuterViewportBoundsDelta());
    };
    ...
    check_result(ChildDocument().getElementById(AtomicString("fixed"))->GetLayoutObject()->FirstFragment().PaintProperties(), false);
}
...
TEST_P(PaintPropertyTreeBuilderTest, TransformAnimationAxisAlignment)
{
    ...
    const auto* translation = PaintPropertiesForElement("translation_animation")->Transform();
    EXPECT_TRUE(translation->HasActiveTransformAnimation());
    EXPECT_TRUE(translation->TransformAnimationIsAxisAligned());

    const auto* rotation = PaintPropertiesForElement("rotation_animation")->Transform();
...
TEST_P(PaintPropertyTreeBuilderTest, OverflowScrollPropertyHierarchy)
{
    ...
    EXPECT_EQ(top_properties->ScrollTranslation()->ScrollNode(), top_properties->Scroll());

    auto* middle_properties = PaintPropertiesForElement("middle-scroller");
    EXPECT_EQ(middle_properties->PaintOffsetTranslation(), &middle_properties->OverflowClip()->LocalTransformSpace());
    EXPECT_EQ(top_properties->OverflowClip(), middle_properties->OverflowClip()->Parent());
    EXPECT_EQ(top_properties->Scroll(), middle_properties->Scroll()->Parent());
    EXPECT_EQ(middle_properties->ScrollTranslation()->ScrollNode(), middle_properties->Scroll());
    EXPECT_EQ(top_properties->ScrollTranslation(), middle_properties->ScrollTranslation()->Parent()->Parent());
    EXPECT_EQ(middle_properties->PaintOffsetTranslation(), &middle_properties->Effect()->LocalTransformSpace());

    // |fixed| escapes both top and middle scrollers.
    auto& fixed_fragment = GetLayoutObjectByElementId("fixed")->FirstFragment();
    EXPECT_EQ(fixed_fragment.PaintProperties()->PaintOffsetTranslation(), &fixed_fragment.PreTransform());
    ...
    auto& absolute_fragment = GetLayoutObjectByElementId("absolute")->FirstFragment();
    EXPECT_EQ(top_properties->ScrollTranslation(), &absolute_fragment.PreTransform());
    EXPECT_EQ(top_properties->OverflowClip(), &absolute_fragment.PreClip());

    // |relative| is contained by |middle-scroller|.
    auto& relative_fragment = GetLayoutObjectByElementId("relative")->FirstFragment();
    EXPECT_EQ(middle_properties->ScrollTranslation(), &relative_fragment.PreTransform());
    EXPECT_EQ(middle_properties->OverflowClip(), &relative_fragment.PreClip());

    // The opacity on |middle-scroller| applies to all children.
    EXPECT_EQ(middle_properties->Effect(), &fixed_fragment.LocalBorderBoxProperties().Effect());
    EXPECT_EQ(middle_properties->Effect(), &absolute_fragment.LocalBorderBoxProperties().Effect());
    EXPECT_EQ(middle_properties->Effect(), &relative_fragment.LocalBorderBoxProperties().Effect());
}
...
TEST_P(PaintPropertyTreeBuilderTest, OutOfFlowContainedInMulticol)
{
    ...
    for (unsigned i = 0; i < NumFragments(relative); i++) {
        ...
        const auto* relative_transform = FragmentAt(relative, i).PaintProperties()->Transform();
        const auto& absolute_properties = FragmentAt(absolute, i).LocalBorderBoxProperties();
        const auto& fixed_properties = FragmentAt(fixed, i).LocalBorderBoxProperties();
...
TEST_P(PaintPropertyTreeBuilderTest, SVGChildBackdropFilter)
{
    ...
    EXPECT_FALSE(svg_properties->PaintOffsetTranslation()->HasDirectCompositingReasons());
    ...
    EXPECT_FALSE(GetLayoutObjectByElementId("text")->SlowFirstChild()->FirstFragment().PaintProperties());
}
...
TEST_P(PaintPropertyTreeBuilderTest, WillChangeBackdropFilter)
{
    ...
    EXPECT_TRUE(properties->Effect()->RequiresCompositingForWillChangeBackdropFilter());
...
TEST_P(PaintPropertyTreeBuilderTest, WillChangeBackdropFilterWithTransformAndFilter)
{
    SetBodyInnerHTML(R"HTML(
...
TEST_P(PaintPropertyTreeBuilderTest, EffectCanUseCurrentClipAsOutputClipCrash)
{
    ...
    EXPECT_TRUE(GetLayoutObjectByElementId("f")->SlowFirstChild()->FirstFragment().HasLocalBorderBoxProperties());
}
...
TEST_P(PaintPropertyTreeBuilderTest, GeometryMapperCacheInvalidationAcrossIsolationNodes)
{
    SetBodyInnerHTML(R"HTML(
    <style>
    ...
    EXPECT_EQ(target_properties->Transform()->NearestDirectlyCompositedAncestor(), container_properties->Transform());

    // Remove the direct compositing reason from #container.
    auto* container_element = GetDocument().getElementById(AtomicString("container"));
    ...
    EXPECT_EQ(target_properties->Transform()->NearestDirectlyCompositedAncestor(), composited_properties->Transform());
}
...
TEST_P(PaintPropertyTreeBuilderTest, PromoteTrivial3DWithHighEndDevice)
{
    ...
    EXPECT_TRUE(non_scroll_properties->Transform()->HasDirectCompositingReasons());
    ...
    EXPECT_EQ(CompositedScrollingPreference::kPreferred, scroll_properties->Scroll()->GetCompositedScrollingPreference());
...
TEST_P(PaintPropertyTreeBuilderTest, DontPromoteTrivial3DWithLowEndDevice)
{
    ...
    const auto* non_scroll_properties = PaintPropertiesForElement("non-scroll");
    EXPECT_FALSE(non_scroll_properties->Transform()->HasDirectCompositingReasons());
    EXPECT_FALSE(non_scroll_properties->Effect());
...
TEST_P(PaintPropertyTreeBuilderTest, BackgroundClip)
{
    ...
    EXPECT_BACKGROUND_CLIP(PaintPropertiesForElement("target"), gfx::RectF(0, 0, 300, 200));

    target->setAttribute(html_names::kStyleAttr, AtomicString("background-clip: padding-box"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_BACKGROUND_CLIP(PaintPropertiesForElement("target"), gfx::RectF(20, 20, 260, 160));

    target->setAttribute(html_names::kStyleAttr, AtomicString("background-clip: padding-box; border-top-width: 15px"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_BACKGROUND_CLIP(PaintPropertiesForElement("target"), gfx::RectF(20, 15, 260, 165));

    target->setAttribute(html_names::kStyleAttr, AtomicString("background-clip: content-box"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_BACKGROUND_CLIP(PaintPropertiesForElement("target"), gfx::RectF(30, 30, 240, 140));

    target->setAttribute(html_names::kStyleAttr, AtomicString("background-clip: content-box; padding-left: 25px"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_BACKGROUND_CLIP(PaintPropertiesForElement("target"), gfx::RectF(45, 30, 225, 140));

    target->setAttribute(html_names::kStyleAttr, g_empty_atom);
    UpdateAllLifecyclePhasesForTest();
    EXPECT_BACKGROUND_CLIP(PaintPropertiesForElement("target"), gfx::RectF(0, 0, 300, 200));

    target->setAttribute(html_names::kStyleAttr, AtomicString("border-radius: 20px"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(PaintPropertiesForElement("target"));
    ...
    target->setAttribute(html_names::kStyleAttr, AtomicString("box-shadow: 10px 20px blue"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(PaintPropertiesForElement("target"));
    ...
    target->setAttribute(html_names::kStyleAttr, AtomicString("background-attachment: local"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(PaintPropertiesForElement("target"));
}
...
TEST_P(PaintPropertyTreeBuilderTest, BackgroundClipFragmented)
{
    ...
    EXPECT_BACKGROUND_CLIP(FragmentAt(target, 0).PaintProperties(), gfx::RectF(30, 30, 40, 170));
    EXPECT_BACKGROUND_CLIP(FragmentAt(target, 1).PaintProperties(), gfx::RectF(130, 0, 40, 200));
    EXPECT_BACKGROUND_CLIP(FragmentAt(target, 2).PaintProperties(), gfx::RectF(230, 0, 40, 170));

    GetDocument().getElementById(AtomicString("target"))->setAttribute(html_names::kStyleAttr, AtomicString("background-clip: padding-box"));
    UpdateAllLifecyclePhasesForTest();
    ASSERT_EQ(3u, NumFragments(target));
    EXPECT_BACKGROUND_CLIP(FragmentAt(target, 0).PaintProperties(), gfx::RectF(20, 20, 60, 180));
    EXPECT_BACKGROUND_CLIP(FragmentAt(target, 1).PaintProperties(), gfx::RectF(120, 0, 60, 200));
    EXPECT_BACKGROUND_CLIP(FragmentAt(target, 2).PaintProperties(), gfx::RectF(220, 0, 60, 180));

    GetDocument().getElementById(AtomicString("target"))->setAttribute(html_names::kStyleAttr, AtomicString("background-clip: border-box"));
    UpdateAllLifecyclePhasesForTest();
    ASSERT_EQ(3u, NumFragments(target));
    EXPECT_BACKGROUND_CLIP(FragmentAt(target, 0).PaintProperties(), gfx::RectF(0, 0, 100, 200));
    EXPECT_BACKGROUND_CLIP(FragmentAt(target, 1).PaintProperties(), gfx::RectF(100, 0, 100, 200));
    EXPECT_BACKGROUND_CLIP(FragmentAt(target, 2).PaintProperties(), gfx::RectF(200, 0, 100, 200));
}
...
TEST_P(PaintPropertyTreeBuilderTest, OverlayScrollbarEffects)
{
    ...
    EXPECT_EQ(properties->OverflowClip()->Parent(), properties->VerticalScrollbarEffect()->OutputClip());
}
...
TEST_P(PaintPropertyTreeBuilderTest, OverlayScrollbarEffectsWithRadius)
{
    ...
    EXPECT_FALSE(properties->HorizontalScrollbarEffect());
    ASSERT_TRUE(properties->VerticalScrollbarEffect());
    EXPECT_EQ(properties->InnerBorderRadiusClip()->Parent(), properties->VerticalScrollbarEffect()->OutputClip());
}
