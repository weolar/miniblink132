...
{
    DCHECK(!object_.PaintingLayer() || object_.PaintingLayer()->SelfNeedsRepaint());
}
#endif
...
#endif
    TRACE_EVENT_INSTANT2(TRACE_DISABLED_BY_DEFAULT("blink.invalidation"), "InvalidateDisplayItemClient", TRACE_EVENT_SCOPE_GLOBAL, "client",
        client.DebugName().Utf8(), "reason", PaintInvalidationReasonToString(reason));
...
PaintInvalidationReason ObjectPaintInvalidatorWithContext::ComputePaintInvalidationReason()
{
    ...
    if (object_.VisualRectRespectsVisibility() && !previous_visibility_visible && object_.StyleRef().Visibility() != EVisibility::kVisible) {
    ...
    if (context_.subtree_flags & PaintInvalidatorContext::kSubtreeFullInvalidation)
...
DISABLE_CFI_PERF
void ObjectPaintInvalidatorWithContext::InvalidatePaintWithComputedReason(PaintInvalidationReason reason)
{
    DCHECK(!(context_.subtree_flags & PaintInvalidatorContext::kSubtreeNoInvalidation));
    ...
    if (reason == PaintInvalidationReason::kNone) {
        ...
        if (const auto* selection_client = object_.GetSelectionDisplayItemClient()) {
