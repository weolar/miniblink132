...
std::pair<gfx::Point, gfx::Point> GetSnappedPointsForTextLine(const gfx::RectF& decoration_rect)
{
    int mid_y = floorf(decoration_rect.y() + std::max(decoration_rect.height() / 2.0f, 0.5f));
    return { gfx::Point(decoration_rect.x(), mid_y), gfx::Point(decoration_rect.right(), mid_y) };
}
...
void DecorationLinePainter::DrawLineForText(GraphicsContext& context, const gfx::PointF& pt, float width, const StyledStrokeData& styled_stroke,
    const AutoDarkMode& auto_dark_mode, const cc::PaintFlags* paint_flags)
{
    if (width <= 0) {
        return;
    }
    ...
    if (ShouldUseStrokeForTextLine(stroke_style)) {
        ...
        context.DrawLine(start, end, styled_stroke, auto_dark_mode, true, paint_flags);
    } else {
        if (paint_flags) {
            // In SVG (inferred by a non-null `paint_flags`), we don't snap the line
            // to get better scaling behavior. See crbug.com/1270336.
            context.DrawRect(gfx::RectFToSkRect(line_rect), *paint_flags, auto_dark_mode);
        } else {
...
Path DecorationLinePainter::GetPathForTextLine(const gfx::PointF& pt, float width, float stroke_thickness, StrokeStyle stroke_style)
{
    DCHECK_NE(stroke_style, kWavyStroke);
...
void DecorationLinePainter::Paint(const Color& color, const cc::PaintFlags* flags)
{
    ...
    AutoDarkMode auto_dark_mode(PaintAutoDarkMode(decoration_info_.TargetStyle(), DarkModeFilter::ElementRole::kForeground));
    ...
    case ETextDecorationStyle::kDashed:
        ...
        [[fallthrough]];
    default:
        DrawLineForText(context_, decoration_info_.StartPoint(), decoration_info_.Width(), styled_stroke, auto_dark_mode, flags);

        if (decoration_info_.DecorationStyle() == ETextDecorationStyle::kDouble) {
            DrawLineForText(context_, decoration_info_.StartPoint() + gfx::Vector2dF(0, decoration_info_.DoubleOffset()), decoration_info_.Width(),
                styled_stroke, auto_dark_mode, flags);
        }
    }
}

void DecorationLinePainter::PaintWavyTextDecoration(const AutoDarkMode& auto_dark_mode)
{
    ...
    flags.setShader(PaintShader::MakePaintRecord(
        decoration_info_.WavyTileRecord(), gfx::RectFToSkRect(decoration_info_.WavyTileRect()), SkTileMode::kRepeat, SkTileMode::kDecal, nullptr));
    ...
    context_.DrawRect(gfx::RectFToSkRect(decoration_info_.WavyPaintRect()), flags, auto_dark_mode);
}
