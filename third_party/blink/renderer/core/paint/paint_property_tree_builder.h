...
public:
    ...
    //   isolation.
    enum SubtreeUpdateReason : unsigned { kSubtreeUpdateIsolationPiercing = 1 << 0, kSubtreeUpdateIsolationBlocked = 1 << 1 };
    ...
    unsigned composited_scrolling_preference : 2 = static_cast<unsigned>(CompositedScrollingPreference::kDefault);
...
public:
    ...
    // is up to date.
    static void Update(LocalFrameView& main_frame_view, VisualViewport&, PaintPropertyTreeBuilderContext&);
};
...
public:
    PrePaintInfo(const PhysicalBoxFragment* box_fragment, PhysicalOffset paint_offset, wtf_size_t fragmentainer_idx, bool is_first_for_node,
        bool is_last_for_node, bool is_inside_fragment_child, bool fragmentainer_is_oof_containing_block)
        ...
        , fragmentainer_is_oof_containing_block(fragmentainer_is_oof_containing_block)
    {
    }
...
public:
    PaintPropertyChangeType transform_changed = PaintPropertyChangeType::kUnchanged;
    ...
    void Merge(const PaintPropertiesChangeInfo& other)
    {
        ...
        transform_change_is_scroll_translation_only &= other.transform_change_is_scroll_translation_only;
    ...
    {
        return std::max({ transform_changed, clip_changed, effect_changed, scroll_changed });
    }
};
...
public:
    static void SetupContextForFrame(LocalFrameView&, PaintPropertyTreeBuilderContext&);

    PaintPropertyTreeBuilder(const LayoutObject& object, PrePaintInfo* pre_paint_info, PaintPropertyTreeBuilderContext& context)
        : object_(object)
