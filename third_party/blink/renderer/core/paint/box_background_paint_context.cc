...
PhysicalOffset OffsetInStitchedTableGrid(const PhysicalBoxFragment& fragment, PhysicalSize* stitched_grid_size)
{
...
BoxBackgroundPaintContext::BoxBackgroundPaintContext(
    const LayoutView& view, const PhysicalBoxFragment* box_fragment, const PhysicalOffset& element_positioning_area_offset)
    : box_(&view)
{
    if (box_fragment && box_fragment->GetBoxType() == PhysicalFragment::kPageContainer) {
        ...
        positioning_box_ = To<LayoutBox>(GetPageBorderBox(*box_fragment).GetLayoutObject());
    } else {
...
BoxBackgroundPaintContext::BoxBackgroundPaintContext(const LayoutBoxModelObject& obj)
...
BoxBackgroundPaintContext::BoxBackgroundPaintContext(const PhysicalBoxFragment& fragment)
    : BoxBackgroundPaintContext(To<LayoutBoxModelObject>(fragment.GetLayoutObject()), To<LayoutBoxModelObject>(fragment.GetLayoutObject()))
{
    DCHECK(box_->IsBox());
    ...
    if (fragment.GetBoxType() == PhysicalFragment::kPageBorderBox) {
        ...
        wtf_size_t page_index = view->GetFrameView()->GetPaginationState()->CurrentPageIndex();
        element_positioning_area_offset_ = StitchedPageContentRect(*view, page_index).offset;
    } else {
...
BoxBackgroundPaintContext::BoxBackgroundPaintContext(const LayoutBoxModelObject* box, const LayoutBoxModelObject* positioning_box)
    : box_(box)
    , positioning_box_(positioning_box)
    , has_background_fixed_to_viewport_(HasBackgroundFixedToViewport(*positioning_box))
{
...
PhysicalBoxStrut BoxBackgroundPaintContext::VisualOverflowOutsets() const
{
    ...
    PhysicalRect visual_overflow = positioning_box_->Layer()->LocalBoundingBoxIncludingSelfPaintingDescendants();
    return PhysicalBoxStrut(visual_overflow.Y() - border_box.Y(), border_box.Right() - visual_overflow.Right(), border_box.Bottom() - visual_overflow.Bottom(),
        visual_overflow.X() - border_box.X());
}

PhysicalBoxStrut BoxBackgroundPaintContext::InnerBorderOutsets(const PhysicalRect& dest_rect, const PhysicalRect& positioning_area) const
{
    gfx::RectF inner_border_rect = RoundedBorderGeometry::PixelSnappedRoundedInnerBorder(positioning_box_->StyleRef(), positioning_area).Rect();
...
SnappedAndUnsnappedOutsets BoxBackgroundPaintContext::ObscuredBorderOutsets(const PhysicalRect& dest_rect, const PhysicalRect& positioning_area) const
{
    const ComputedStyle& style = positioning_box_->StyleRef();
    gfx::RectF inner_border_rect = RoundedBorderGeometry::PixelSnappedRoundedInnerBorder(style, positioning_area).Rect();
    ...
    if (edges[static_cast<unsigned>(BoxSide::kRight)].ObscuresBackground()) {
        adjust.snapped.right = dest_rect.Right() - LayoutUnit(inner_border_rect.right());
        adjust.unsnapped.right = box_outsets.right;
    }
    if (edges[static_cast<unsigned>(BoxSide::kBottom)].ObscuresBackground()) {
        adjust.snapped.bottom = dest_rect.Bottom() - LayoutUnit(inner_border_rect.bottom());
...
PhysicalRect BoxBackgroundPaintContext::ComputePositioningArea(const PaintInfo& paint_info, const FillLayer& fill_layer, const PhysicalRect& paint_rect) const
{
...
PhysicalRect BoxBackgroundPaintContext::NormalPositioningArea(const PhysicalRect& paint_rect) const
{
    if (painting_view_ || cell_using_container_background_ || box_has_multiple_fragments_) {
...
{
    return painting_view_ || painting_table_cell_ || box_has_multiple_fragments_ || positioning_box_->StyleRef().BorderImage().GetImage()
        || positioning_box_->StyleRef().BorderCollapse() == EBorderCollapse::kCollapse;
}

bool BoxBackgroundPaintContext::CanCompositeBackgroundAttachmentFixed() const
{
    return !painting_view_ && has_background_fixed_to_viewport_ && positioning_box_->CanCompositeBackgroundAttachmentFixed();
}
...
bool BoxBackgroundPaintContext::ShouldUseFixedAttachment(const FillLayer& fill_layer) const
{
    ...
    return has_background_fixed_to_viewport_ &&
        ...
        fill_layer.GetImage() && fill_layer.Attachment() == EFillAttachment::kFixed;
}

bool BoxBackgroundPaintContext::HasBackgroundFixedToViewport(const LayoutBoxModelObject& object)
{
    ...
    for (const PaintLayer* layer = object.EnclosingLayer(); layer && !layer->IsRootLayer(); layer = layer->Parent()) {
        ...
        if (ancestor.HasTransformRelatedProperty() && (layer->Transform() || ancestor.StyleRef().HasWillChangeHintForAnyTransformProperty())) {
            return false;
        }
    }
    return true;
...
PhysicalRect BoxBackgroundPaintContext::FixedAttachmentPositioningArea(const PaintInfo& paint_info) const
{
    const ScrollableArea* layout_viewport = box_->GetFrameView()->LayoutViewport();
    ...
    gfx::PointF viewport_origin_in_local_space = GeometryMapper::SourceToDestinationProjection(
        box_->View()->FirstFragment().LocalBorderBoxProperties().Transform(), paint_info.context.GetPaintController().CurrentPaintChunkProperties().Transform())
                                                     .MapPoint(gfx::PointF());
    return PhysicalRect(PhysicalOffset::FromPointFRound(viewport_origin_in_local_space), PhysicalSize(layout_viewport->VisibleContentRect().size()));
}
...
const ComputedStyle& BoxBackgroundPaintContext::ImageStyle(const ComputedStyle& fragment_style) const
{
...
bool BoxBackgroundPaintContext::ShouldSkipBackgroundIfWhite() const
{
    ...
    return box_fragment_ && box_fragment_->GetBoxType() == PhysicalFragment::kPageBorderBox;
}

PhysicalOffset BoxBackgroundPaintContext::OffsetInBackground(const FillLayer& fill_layer) const
{
    if (ShouldUseFixedAttachment(fill_layer)) {
