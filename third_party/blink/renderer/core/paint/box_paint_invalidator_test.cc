...
protected:
    PaintInvalidationReason ComputePaintInvalidationReason(LayoutBox& box, const PhysicalOffset& old_paint_offset)
    {
    ...
    void ExpectFullPaintInvalidationOnGeometryChange(const char* test_title)
    {
        ...
        // No geometry change.
        EXPECT_EQ(PaintInvalidationReason::kNone, ComputePaintInvalidationReason(box, paint_offset));

        target.setAttribute(html_names::kStyleAttr, target.getAttribute(html_names::kStyleAttr) + "; width: 200px");
        GetDocument().View()->UpdateLifecycleToLayoutClean(DocumentUpdateReason::kTest);

        EXPECT_EQ(PaintInvalidationReason::kLayout, ComputePaintInvalidationReason(box, paint_offset));
    }
...
private:
    Persistent<FragmentData> fragment_data_ = MakeGarbageCollected<FragmentData>();
};
...
TEST_P(BoxPaintInvalidatorTest, ComputePaintInvalidationReasonEmptyContent)
{
    ...
    EXPECT_EQ(PaintInvalidationReason::kLayout, ComputePaintInvalidationReason(box, old_paint_offset));

    // Size change.
    target.setAttribute(html_names::kStyleAttr, AtomicString("width: 200px"));
    GetDocument().View()->UpdateLifecycleToLayoutClean(DocumentUpdateReason::kTest);
    EXPECT_EQ(PaintInvalidationReason::kIncremental, ComputePaintInvalidationReason(box, paint_offset));
}
...
TEST_P(BoxPaintInvalidatorTest, ComputePaintInvalidationReasonBasic)
{
    ...
    // Size change.
    target.setAttribute(html_names::kStyleAttr, AtomicString("background: blue; width: 200px"));
    GetDocument().View()->UpdateLifecycleToLayoutClean(DocumentUpdateReason::kTest);
    EXPECT_EQ(PaintInvalidationReason::kIncremental, ComputePaintInvalidationReason(box, paint_offset));

    // Add visual overflow.
    target.setAttribute(html_names::kStyleAttr, AtomicString("background: blue; width: 200px; outline: 5px solid red"));
    UpdateAllLifecyclePhasesForTest();

    // Size change with visual overflow.
    target.setAttribute(html_names::kStyleAttr, AtomicString("background: blue; width: 100px; outline: 5px solid red"));
    GetDocument().View()->UpdateLifecycleToLayoutClean(DocumentUpdateReason::kTest);
    ...
    box.SetShouldDoFullPaintInvalidationWithoutLayoutChange(PaintInvalidationReason::kStyle);
    EXPECT_EQ(PaintInvalidationReason::kLayout, ComputePaintInvalidationReason(box, paint_offset));
    ...
    EXPECT_EQ(PaintInvalidationReason::kSVGResource, ComputePaintInvalidationReason(box, paint_offset));
}

TEST_P(BoxPaintInvalidatorTest, InvalidateLineBoxHitTestOnCompositingStyleChange)
{
...
TEST_P(BoxPaintInvalidatorTest, ComputePaintInvalidationReasonOtherCases)
{
    ...
    target.setAttribute(html_names::kStyleAttr, AtomicString("-webkit-mask: url(#)"));
    ExpectFullPaintInvalidationOnGeometryChange("With mask");

    target.setAttribute(html_names::kStyleAttr, AtomicString("filter: blur(5px)"));
    ExpectFullPaintInvalidationOnGeometryChange("With filter");

    target.setAttribute(html_names::kStyleAttr, AtomicString("box-shadow: inset 3px 2px"));
    ExpectFullPaintInvalidationOnGeometryChange("With box-shadow");

    target.setAttribute(html_names::kStyleAttr, AtomicString("clip-path: circle(50% at 0 50%)"));
...
TEST_P(BoxPaintInvalidatorTest, ComputePaintInvalidationReasonOutline)
{
    ...
    target.setAttribute(html_names::kStyleAttr, AtomicString("outline: 2px solid blue;"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_THAT(GetRasterInvalidationTracking()->Invalidations(),
        UnorderedElementsAre(RasterInvalidationInfo { object->Id(), object->DebugName(), gfx::Rect(0, 0, 72, 142), PaintInvalidationReason::kLayout }));
    GetDocument().View()->SetTracksRasterInvalidations(false);

    GetDocument().View()->SetTracksRasterInvalidations(true);
    target.setAttribute(html_names::kStyleAttr, AtomicString("outline: 2px solid blue; width: 100px;"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_THAT(GetRasterInvalidationTracking()->Invalidations(),
        UnorderedElementsAre(RasterInvalidationInfo { object->Id(), object->DebugName(), gfx::Rect(0, 0, 122, 142), PaintInvalidationReason::kLayout }));
    GetDocument().View()->SetTracksRasterInvalidations(false);
}
