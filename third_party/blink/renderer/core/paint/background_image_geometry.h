...
public:
    ...
    // PrePaintClean.
    void Calculate(const FillLayer&, const BoxBackgroundPaintContext&, const PhysicalRect& paint_rect, const PaintInfo& paint_info);
    ...
    // size is given.
    const PhysicalRect& UnsnappedDestRect() const
    {
        return unsnapped_dest_rect_;
    }
    const PhysicalRect& SnappedDestRect() const
    {
        return snapped_dest_rect_;
    }
    ...
    const PhysicalSize& TileSize() const
    {
        return tile_size_;
    }
    ...
    // in the scaled image.
    const PhysicalOffset& Phase() const
    {
        return phase_;
    }
    ...
    const PhysicalSize& SpaceSize() const
    {
        return repeat_spacing_;
    }
...
private:
    ...
    void SetPhaseX(LayoutUnit x)
    {
        phase_.left = x;
    }
    void SetPhaseY(LayoutUnit y)
    {
        phase_.top = y;
    }

    void SetNoRepeatX(const FillLayer&, LayoutUnit x_offset, LayoutUnit snapped_x_offset);
    void SetNoRepeatY(const FillLayer&, LayoutUnit y_offset, LayoutUnit snapped_y_offset);
    ...
    SnappedAndUnsnappedOutsets ComputeDestRectAdjustments(
        const FillLayer&, const BoxBackgroundPaintContext&, const PhysicalRect& unsnapped_positioning_area, bool disallow_border_derived_adjustment) const;
    ...
    gfx::RectF ComputePositioningArea(const FillLayer&, const SVGBackgroundPaintContext&) const;
    gfx::RectF ComputePaintingArea(const FillLayer&, const SVGBackgroundPaintContext&, const gfx::RectF& positioning_area) const;

    void AdjustPositioningArea(
        const FillLayer&, const BoxBackgroundPaintContext&, const PaintInfo&, PhysicalRect&, PhysicalRect&, PhysicalOffset&, PhysicalOffset&);
    void CalculateFillTileSize(const FillLayer&, const ComputedStyle&, const PhysicalSize&, const PhysicalSize&);
    void CalculateRepeatAndPosition(const FillLayer&, const PhysicalOffset& offset_in_background, const PhysicalSize& unsnapped_positioning_area_size,
        const PhysicalSize& snapped_positioning_area_size, const PhysicalOffset& unsnapped_box_offset, const PhysicalOffset& snapped_box_offset);
