...
public:
    ...
    static void RecordHitTestData(const LayoutObject& svg_object, const PaintInfo&);
    ...
    // content.
    static void RecordRegionCaptureData(const LayoutObject& svg_object, const PaintInfo&);

    explicit SVGModelObjectPainter(const LayoutSVGModelObject& layout_svg_model_object)
...
public:
    ...
    SVGDrawingRecorder(GraphicsContext& context, const LayoutObjectType& object, DisplayItem::Type type)
        : DrawingRecorder(context, object, type, gfx::ToEnclosingRect(object.VisualRectInLocalSVGCoordinates()))
    {
    ...
    template <typename LayoutObjectType>
    SVGDrawingRecorder(GraphicsContext& context, const LayoutObjectType& object, PaintPhase phase)
        : SVGDrawingRecorder(context, object, DisplayItem::PaintPhaseToDrawingType(phase))
    {
    }
};
