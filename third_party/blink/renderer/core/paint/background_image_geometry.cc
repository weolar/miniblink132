...
inline LayoutUnit GetSpaceBetweenImageTiles(LayoutUnit area_size, LayoutUnit tile_size)
{
...
LayoutUnit ComputeTilePhase(LayoutUnit position, LayoutUnit tile_extent)
{
    ...
    return tile_extent ? tile_extent - IntMod(position, tile_extent) : LayoutUnit();
}

LayoutUnit ResolveWidthForRatio(LayoutUnit height, const PhysicalSize& natural_ratio)
{
    LayoutUnit resolved_width = height.MulDiv(natural_ratio.width, natural_ratio.height);
...
LayoutUnit ResolveHeightForRatio(LayoutUnit width, const PhysicalSize& natural_ratio)
{
    LayoutUnit resolved_height = width.MulDiv(natural_ratio.height, natural_ratio.width);
...
LayoutUnit ResolveXPosition(const FillLayer& fill_layer, LayoutUnit available_width, LayoutUnit offset)
{
    const LayoutUnit edge_relative_position = MinimumValueForLength(fill_layer.PositionX(), available_width);
    // Convert from edge-relative form to absolute.
    const LayoutUnit absolute_position
        = fill_layer.BackgroundXOrigin() == BackgroundEdgeOrigin::kRight ? available_width - edge_relative_position : edge_relative_position;
    return absolute_position - offset;
}

LayoutUnit ResolveYPosition(const FillLayer& fill_layer, LayoutUnit available_height, LayoutUnit offset)
{
    const LayoutUnit edge_relative_position = MinimumValueForLength(fill_layer.PositionY(), available_height);
    // Convert from edge-relative form to absolute.
    const LayoutUnit absolute_position
        = fill_layer.BackgroundYOrigin() == BackgroundEdgeOrigin::kBottom ? available_height - edge_relative_position : edge_relative_position;
    return absolute_position - offset;
}
...
bool NeedsFullSizeDestination(const FillLayer& fill_layer)
{
    ...
    return fill_layer.GetType() == EFillLayerType::kMask && fill_layer.Next() && fill_layer.Composite() != kCompositeSourceOver;
}

void BackgroundImageGeometry::SetNoRepeatX(const FillLayer& fill_layer, LayoutUnit x_offset, LayoutUnit snapped_x_offset)
{
    if (NeedsFullSizeDestination(fill_layer)) {
        SetPhaseX(-x_offset);
        SetSpaceSize(PhysicalSize(unsnapped_dest_rect_.Width(), SpaceSize().height));
        return;
    }
...
void BackgroundImageGeometry::SetNoRepeatY(const FillLayer& fill_layer, LayoutUnit y_offset, LayoutUnit snapped_y_offset)
{
    if (NeedsFullSizeDestination(fill_layer)) {
        SetPhaseY(-y_offset);
        SetSpaceSize(PhysicalSize(SpaceSize().width, unsnapped_dest_rect_.Height()));
        return;
    }
...
void BackgroundImageGeometry::SetSpaceX(LayoutUnit space, LayoutUnit extra_offset)
{
...
void BackgroundImageGeometry::SetSpaceY(LayoutUnit space, LayoutUnit extra_offset)
{
...
SnappedAndUnsnappedOutsets BackgroundImageGeometry::ComputeDestRectAdjustments(const FillLayer& fill_layer, const BoxBackgroundPaintContext& paint_context,
    const PhysicalRect& unsnapped_positioning_area, bool disallow_border_derived_adjustment) const
{
    ...
    case EFillBox::kPadding:
        ...
        if (disallow_border_derived_adjustment) {
            ...
            dest_adjust.snapped = paint_context.InnerBorderOutsets(unsnapped_dest_rect_, unsnapped_positioning_area);
        }
        break;
    ...
    case EFillBox::kBorder: {
        ...
        // zoom).
        dest_adjust = paint_context.ObscuredBorderOutsets(unsnapped_dest_rect_, unsnapped_positioning_area);
        break;
    }
    case EFillBox::kText:
        break;
...
SnappedAndUnsnappedOutsets BackgroundImageGeometry::ComputePositioningAreaAdjustments(const FillLayer& fill_layer,
    const BoxBackgroundPaintContext& paint_context, const PhysicalRect& unsnapped_positioning_area, bool disallow_border_derived_adjustment) const
{
    ...
    case EFillBox::kPadding:
        ...
        if (disallow_border_derived_adjustment) {
            ...
            // border by more than a pixel when done (particularly under magnifying
            // zoom).
            box_outset.snapped = paint_context.InnerBorderOutsets(unsnapped_positioning_area, unsnapped_positioning_area);
        }
        break;
    case EFillBox::kStrokeBox:
    case EFillBox::kViewBox:
...
void BackgroundImageGeometry::AdjustPositioningArea(const FillLayer& fill_layer, const BoxBackgroundPaintContext& paint_context, const PaintInfo& paint_info,
    PhysicalRect& unsnapped_positioning_area, PhysicalRect& snapped_positioning_area, PhysicalOffset& unsnapped_box_offset, PhysicalOffset& snapped_box_offset)
{
    ...
    bool disallow_border_derived_adjustment = !ShouldPaintSelfBlockBackground(paint_info.phase)
        || fill_layer.Composite() != CompositeOperator::kCompositeSourceOver || paint_context.DisallowBorderDerivedAdjustment();
    ...
    const SnappedAndUnsnappedOutsets dest_adjust
        = ComputeDestRectAdjustments(fill_layer, paint_context, unsnapped_positioning_area, disallow_border_derived_adjustment);
    const SnappedAndUnsnappedOutsets box_outset
        = ComputePositioningAreaAdjustments(fill_layer, paint_context, unsnapped_positioning_area, disallow_border_derived_adjustment);
    ...
    snapped_positioning_area = PhysicalRect(ToPixelSnappedRect(snapped_positioning_area));
...
void BackgroundImageGeometry::CalculateFillTileSize(const FillLayer& fill_layer, const ComputedStyle& style,
    const PhysicalSize& unsnapped_positioning_area_size, const PhysicalSize& snapped_positioning_area_size)
{
    ...
    IntrinsicSizingInfo sizing_info = image->GetNaturalSizingInfo(style.EffectiveZoom(), style.ImageOrientation());
    PhysicalSize image_aspect_ratio = PhysicalSize::FromSizeFFloor(sizing_info.aspect_ratio);
    PhysicalSize positioning_area_size = !image->HasIntrinsicSize() ? snapped_positioning_area_size : unsnapped_positioning_area_size;
    switch (type) {
    case EFillSizeType::kSizeLength: {
        ...
        if (layer_width.IsFixed()) {
            ...
            tile_size_.width = ValueForLength(layer_width, positioning_area_size.width);
        }
        ...
        if (layer_height.IsFixed()) {
            ...
            tile_size_.height = ValueForLength(layer_height, positioning_area_size.height);
        }
        ...
        if (layer_width.IsAuto() && !layer_height.IsAuto()) {
            if (!image_aspect_ratio.IsEmpty()) {
                tile_size_.width = ResolveWidthForRatio(tile_size_.height, image_aspect_ratio);
            } else if (sizing_info.has_width) {
                tile_size_.width = LayoutUnit::FromFloatFloor(sizing_info.size.width());
            } else {
                tile_size_.width = positioning_area_size.width;
            }
        } else if (!layer_width.IsAuto() && layer_height.IsAuto()) {
            if (!image_aspect_ratio.IsEmpty()) {
                tile_size_.height = ResolveHeightForRatio(tile_size_.width, image_aspect_ratio);
            } else if (sizing_info.has_height) {
                tile_size_.height = LayoutUnit::FromFloatFloor(sizing_info.size.height());
            } else {
                tile_size_.height = positioning_area_size.height;
            }
        } else if (layer_width.IsAuto() && layer_height.IsAuto()) {
            PhysicalSize concrete_image_size
                = PhysicalSize::FromSizeFFloor(image->ImageSize(style.EffectiveZoom(), gfx::SizeF(positioning_area_size), style.ImageOrientation()));
    ...
    case EFillSizeType::kCover: {
        ...
        tile_size_
            = snapped_positioning_area_size.FitToAspectRatio(image_aspect_ratio, type == EFillSizeType::kCover ? kAspectRatioFitGrow : kAspectRatioFitShrink);
        ...
        if (type == EFillSizeType::kContain) {
            ...
            if (tile_size_.height != snapped_positioning_area_size.height) {
                tile_size_.height = LayoutUnit(std::max(1, tile_size_.height.Round()));
            }
        } else {
            if (tile_size_.width != snapped_positioning_area_size.width)
...
void BackgroundImageGeometry::CalculateRepeatAndPosition(const FillLayer& fill_layer, const PhysicalOffset& offset_in_background,
    const PhysicalSize& unsnapped_positioning_area_size, const PhysicalSize& snapped_positioning_area_size, const PhysicalOffset& unsnapped_box_offset,
    const PhysicalOffset& snapped_box_offset)
{
    ...
    const LayoutUnit unsnapped_available_width = unsnapped_positioning_area_size.width - tile_size_.width;
    const LayoutUnit unsnapped_available_height = unsnapped_positioning_area_size.height - tile_size_.height;
    const LayoutUnit snapped_available_width = snapped_positioning_area_size.width - tile_size_.width;
    const LayoutUnit snapped_available_height = snapped_positioning_area_size.height - tile_size_.height;

    if (background_repeat_x == EFillRepeat::kRoundFill && snapped_positioning_area_size.width > LayoutUnit() && tile_size_.width > LayoutUnit()) {
        LayoutUnit rounded_width = ComputeRoundedTileSize(snapped_positioning_area_size.width, tile_size_.width);
        // Maintain aspect ratio if background-size: auto is set
        if (fill_layer.SizeLength().Height().IsAuto() && background_repeat_y != EFillRepeat::kRoundFill) {
        ...
        const LayoutUnit x_offset = ResolveXPosition(fill_layer, snapped_available_width, offset_in_background.left);
        SetPhaseX(ComputeTilePhase(x_offset + unsnapped_box_offset.left, tile_size_.width));
        SetSpaceSize(PhysicalSize());
    }

    if (background_repeat_y == EFillRepeat::kRoundFill && snapped_positioning_area_size.height > LayoutUnit() && tile_size_.height > LayoutUnit()) {
        LayoutUnit rounded_height = ComputeRoundedTileSize(snapped_positioning_area_size.height, tile_size_.height);
        // Maintain aspect ratio if background-size: auto is set
        if (fill_layer.SizeLength().Width().IsAuto() && background_repeat_x != EFillRepeat::kRoundFill) {
            tile_size_.width = ResolveWidthForRatio(rounded_height, tile_size_);
        }
        tile_size_.height = rounded_height;

        // Force the first tile to line up with the edge of the positioning area.
        const LayoutUnit y_offset = ResolveYPosition(fill_layer, snapped_available_height, offset_in_background.top);
        SetPhaseY(ComputeTilePhase(y_offset + unsnapped_box_offset.top, tile_size_.height));
        SetSpaceSize(PhysicalSize());
    }
    ...
    if (background_repeat_x == EFillRepeat::kRepeatFill) {
        ...
        const LayoutUnit x_offset = ResolveXPosition(fill_layer, unsnapped_available_width, offset_in_background.left);
        SetRepeatX(unsnapped_box_offset.left + x_offset);
    } else if (background_repeat_x == EFillRepeat::kSpaceFill && tile_size_.width > LayoutUnit()) {
        // SpaceFill uses snapped values to fill the painted area.
        LayoutUnit space = GetSpaceBetweenImageTiles(snapped_positioning_area_size.width, tile_size_.width);
        if (space >= LayoutUnit())
    ...
    if (background_repeat_x == EFillRepeat::kNoRepeatFill) {
        ...
        const LayoutUnit x_offset = ResolveXPosition(fill_layer, unsnapped_available_width, offset_in_background.left);
        const LayoutUnit snapped_x_offset = ResolveXPosition(fill_layer, snapped_available_width, offset_in_background.left);
        SetNoRepeatX(fill_layer, unsnapped_box_offset.left + x_offset, snapped_box_offset.left + snapped_x_offset);
    }
    ...
    if (background_repeat_y == EFillRepeat::kRepeatFill) {
        ...
        // Recompute computed position because here we need to resolve against
        // unsnapped widths to correctly set the phase.
        const LayoutUnit y_offset = ResolveYPosition(fill_layer, unsnapped_available_height, offset_in_background.top);
        SetRepeatY(unsnapped_box_offset.top + y_offset);
    } else if (background_repeat_y == EFillRepeat::kSpaceFill && tile_size_.height > LayoutUnit()) {
        // SpaceFill uses snapped values to fill the painted area.
        LayoutUnit space = GetSpaceBetweenImageTiles(snapped_positioning_area_size.height, tile_size_.height);
        if (space >= LayoutUnit())
    ...
    if (background_repeat_y == EFillRepeat::kNoRepeatFill) {
        ...
        // unsnapped parameters.
        const LayoutUnit y_offset = ResolveYPosition(fill_layer, unsnapped_available_height, offset_in_background.top);
        const LayoutUnit snapped_y_offset = ResolveYPosition(fill_layer, snapped_available_height, offset_in_background.top);
        SetNoRepeatY(fill_layer, unsnapped_box_offset.top + y_offset, snapped_box_offset.top + snapped_y_offset);
    }
}

void BackgroundImageGeometry::Calculate(
    const FillLayer& fill_layer, const BoxBackgroundPaintContext& paint_context, const PhysicalRect& paint_rect, const PaintInfo& paint_info)
{
    ...
    if (paint_context.ShouldUseFixedAttachment(fill_layer)) {
        unsnapped_positioning_area = paint_context.FixedAttachmentPositioningArea(paint_info);
        unsnapped_dest_rect_ = snapped_dest_rect_ = snapped_positioning_area = unsnapped_positioning_area;
    } else {
        unsnapped_positioning_area = paint_context.NormalPositioningArea(paint_rect);
        unsnapped_dest_rect_ = paint_rect;

        // This method adjusts `unsnapped_dest_rect_` and sets
        // `snapped_dest_rect_`.
        AdjustPositioningArea(
            fill_layer, paint_context, paint_info, unsnapped_positioning_area, snapped_positioning_area, unsnapped_box_offset, snapped_box_offset);
    }

    // Sets the tile_size_.
    CalculateFillTileSize(fill_layer, paint_context.Style(), unsnapped_positioning_area.size, snapped_positioning_area.size);

    // Applies *-repeat and *-position.
    const PhysicalOffset offset_in_background = paint_context.OffsetInBackground(fill_layer);
    CalculateRepeatAndPosition(
        fill_layer, offset_in_background, unsnapped_positioning_area.size, snapped_positioning_area.size, unsnapped_box_offset, snapped_box_offset);

    if (paint_context.ShouldUseFixedAttachment(fill_layer)) {
        PhysicalOffset fixed_adjustment = paint_rect.offset - unsnapped_dest_rect_.offset;
...
gfx::RectF BackgroundImageGeometry::ComputePositioningArea(const FillLayer& layer, const SVGBackgroundPaintContext& paint_context) const
{
    switch (layer.Origin()) {
    case EFillBox::kNoClip:
    case EFillBox::kText:
...
gfx::RectF BackgroundImageGeometry::ComputePaintingArea(
    const FillLayer& layer, const SVGBackgroundPaintContext& paint_context, const gfx::RectF& positioning_area) const
{
    switch (layer.Clip()) {
    case EFillBox::kText:
    case EFillBox::kNoClip:
...
void BackgroundImageGeometry::Calculate(const FillLayer& fill_layer, const SVGBackgroundPaintContext& paint_context)
{
    const gfx::RectF positioning_area = ComputePositioningArea(fill_layer, paint_context);
    const gfx::RectF painting_area = ComputePaintingArea(fill_layer, paint_context, positioning_area);
    // Unsnapped positioning area is used to derive quantities
    // that reference source image maps and define non-integer values, such
    // as phase and position.
    PhysicalRect unsnapped_positioning_area = PhysicalRect::EnclosingRect(positioning_area);
    unsnapped_dest_rect_ = PhysicalRect::EnclosingRect(painting_area);

    // Additional offset from the corner of the positioning_box_
    PhysicalOffset unsnapped_box_offset = unsnapped_positioning_area.offset - unsnapped_dest_rect_.offset;

    snapped_dest_rect_ = unsnapped_dest_rect_;

    // Sets the tile_size_.
    CalculateFillTileSize(fill_layer, paint_context.Style(), unsnapped_positioning_area.size, unsnapped_positioning_area.size);

    // Applies *-repeat and *-position.
    CalculateRepeatAndPosition(
        fill_layer, PhysicalOffset(), unsnapped_positioning_area.size, unsnapped_positioning_area.size, unsnapped_box_offset, unsnapped_box_offset);
}
...
PhysicalOffset BackgroundImageGeometry::ComputePhase() const
{
    ...
    const PhysicalOffset phase = { IntMod(-phase_.left, step_per_tile.width), IntMod(-phase_.top, step_per_tile.height) };
    return phase;
}
