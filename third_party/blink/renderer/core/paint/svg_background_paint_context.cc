...
SVGBackgroundPaintContext::SVGBackgroundPaintContext(const LayoutObject& layout_object)
    : object_(layout_object)
{
}

gfx::RectF SVGBackgroundPaintContext::ReferenceBox(GeometryBox geometry_box) const
{
    const gfx::RectF reference_box = SVGResources::ReferenceBoxForEffects(object_, geometry_box, SVGResources::ForeignObjectQuirk::kDisabled);
...
gfx::RectF SVGBackgroundPaintContext::VisualOverflowRect() const
{
    ...
    if (auto* svg_fo = DynamicTo<LayoutSVGForeignObject>(object_)) {
        const PhysicalRect visual_overflow = svg_fo->Layer()->LocalBoundingBoxIncludingSelfPaintingDescendants();
