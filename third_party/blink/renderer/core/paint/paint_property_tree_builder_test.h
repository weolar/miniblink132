...
protected:
    ...
    const TransformPaintPropertyNode* DocPreTranslation(const Document* = nullptr);
    const TransformPaintPropertyNode* DocScrollTranslation(const Document* = nullptr);
    ...
    const GeometryMapperTransformCache& GetTransformCache(const TransformPaintPropertyNode&);
    ...
    static const FragmentData& FragmentAt(const LayoutObject* obj, unsigned index)
    {
...
#define EXPECT_CLIP_RECT(expected_arg, clip_node)                                                                                                              \
    do {                                                                                                                                                       \
        ...
        EXPECT_EQ(expected.IsRounded(), (clip_node)->LayoutClipRect().HasRadius());                                                                            \
