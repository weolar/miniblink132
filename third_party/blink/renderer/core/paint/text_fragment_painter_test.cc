...
public:
    explicit TextFragmentPainterTest(LocalFrameClient* local_frame_client = nullptr)
...
{
    SetBodyInnerHTML("<div id='div' style='font-size: 100px; width: 300px'><div>");
    auto* div = GetDocument().getElementById(AtomicString("div"));
    for (int i = 0; i < 1000; i++) {
        div->appendChild(GetDocument().createTextNode("XX"));
        div->appendChild(GetDocument().CreateRawElement(QualifiedName(AtomicString("br"))));
    }
    UpdateAllLifecyclePhasesForTest();
    Selection().SelectAll();
    UpdateAllLifecyclePhasesForTest();
