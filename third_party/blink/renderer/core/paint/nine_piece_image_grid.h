...
public:
    NinePieceImageGrid(const NinePieceImage&, const gfx::SizeF& image_size, const gfx::Vector2dF& slice_scale, float zoom, const gfx::Rect& border_image_area,
        const gfx::Outsets& border_widths, PhysicalBoxSides sides_to_include = PhysicalBoxSides());
    ...
    struct Edge {
        ...
        {
            return slice > 0 && width > 0;
        }
        float Scale() const
        {
            return IsDrawable() ? width / slice : 1;
        }
        float slice;
        int width;
    };
