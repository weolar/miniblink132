...
public:
    ...
    SvgContextPaints(const ContextPaint& f, const ContextPaint& s, const AffineTransform& t)
        : fill(f)
        , stroke(s)
        , transform(t)
    {
...
public:
    PaintInfo(GraphicsContext& context, const CullRect& cull_rect, PaintPhase phase, bool descendant_painting_blocked,
        PaintFlags paint_flags = PaintFlag::kNoFlag, const SvgContextPaints* context_paints = nullptr)
        : context(context)
        , phase(phase)
    ...
    const CullRect& GetCullRect() const
    {
        return cull_rect_;
    }
    ...
    bool IntersectsCullRect(const PhysicalRect& rect, const PhysicalOffset& offset = PhysicalOffset()) const
    {
        return cull_rect_.Intersects(ToEnclosingRect(PhysicalRect(rect.offset + offset, rect.size)));
    }

    void ApplyInfiniteCullRect()
    {
        cull_rect_ = CullRect::Infinite();
    }
    ...
    void SetDescendantPaintingBlocked()
    {
        descendant_painting_blocked_ = true;
    }
