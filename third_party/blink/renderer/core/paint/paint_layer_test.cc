...
public:
    PaintLayerTest()
        : PaintControllerPaintTest(MakeGarbageCollected<SingleChildLocalFrameClient>())
    {
    }
...
{
    SetBodyInnerHTML("<div id='target' style='width: 200px; height: 200px;'></div>");
...
TEST_P(PaintLayerTest, RootLayerScrollBounds)
{
    ...
    SetBodyInnerHTML("<style> body { width: 1000px; height: 1000px; margin: 0 } </style>");
    ...
    EXPECT_EQ(ScrollOffset(200 + scrollbarThickness, 400 + scrollbarThickness), plsa->MaximumScrollOffset());

    EXPECT_EQ(gfx::Rect(0, 0, 800 - scrollbarThickness, 600 - scrollbarThickness), plsa->VisibleContentRect());
    EXPECT_EQ(gfx::Rect(0, 0, 800, 600), plsa->VisibleContentRect(kIncludeScrollbars));
}
...
TEST_P(PaintLayerTest, CompositedScrollingNoNeedsRepaint)
{
    ...
    scroll_layer->GetScrollableArea()->SetScrollOffset(ScrollOffset(1000, 1000), mojom::blink::ScrollType::kProgrammatic);
    UpdateAllLifecyclePhasesExceptPaint();
    EXPECT_EQ(gfx::Vector2d(1000, 1000), content_layer->ContainingLayer()->PixelSnappedScrolledContentOffset());
...
TEST_P(PaintLayerTest, NonCompositedScrollingNeedsRepaint)
{
    ...
    PaintLayer* scroll_layer = GetPaintLayerByElementId("scroll");
    EXPECT_FALSE(scroll_layer->GetLayoutObject().FirstFragment().PaintProperties()->ScrollTranslation()->HasDirectCompositingReasons());
...
TEST_P(PaintLayerTest, HasNonIsolatedDescendantWithBlendMode)
{
    ...
    PaintLayer* stacking_grandparent = GetPaintLayerByElementId("stacking-grandparent");
...
TEST_P(PaintLayerTest, HasFixedPositionDescendant)
{
    ...
    GetDocument().getElementById(AtomicString("child"))->setAttribute(html_names::kStyleAttr, AtomicString("position: relative"));
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintLayerTest, HasNonContainedAbsolutePositionDescendant)
{
    ...
    GetDocument().getElementById(AtomicString("child"))->setAttribute(html_names::kStyleAttr, AtomicString("position: absolute"));
    UpdateAllLifecyclePhasesForTest();

    EXPECT_TRUE(parent->HasNonContainedAbsolutePositionDescendant());
    EXPECT_FALSE(child->HasNonContainedAbsolutePositionDescendant());

    GetDocument().getElementById(AtomicString("parent"))->setAttribute(html_names::kStyleAttr, AtomicString("position: relative"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(parent->HasNonContainedAbsolutePositionDescendant());
    EXPECT_FALSE(child->HasNonContainedAbsolutePositionDescendant());
}
...
static const HeapVector<Member<PaintLayer>>* LayersPaintingOverlayOverflowControlsAfter(const PaintLayer* layer)
{
    return PaintLayerPaintOrderIterator(layer->AncestorStackingContext(), kPositiveZOrderChildren).LayersPaintingOverlayOverflowControlsAfter(layer);
}
...
class ReorderOverlayOverflowControlsTest : public testing::WithParamInterface<OverlayType>, public PaintControllerPaintTestBase {
public:
    ReorderOverlayOverflowControlsTest()
        : PaintControllerPaintTestBase(MakeGarbageCollected<SingleChildLocalFrameClient>())
    {
    }
    ...
    void InitOverflowStyle(const char* id)
    {
        ...
        element->setAttribute(
            html_names::kStyleAttr, AtomicString(GetOverlayType() == kOverlayScrollbars ? "overflow: auto" : "overflow: hidden; resize: both"));
        UpdateAllLifecyclePhasesForTest();
        if (GetOverlayType() == kOverlayScrollbars) {
            element->GetLayoutBox()->GetScrollableArea()->SetScrollbarsHiddenIfOverlay(false);
            UpdateAllLifecyclePhasesForTest();
        }
    }
    ...
    {
        GetElementById(id)->setAttribute(html_names::kStyleAttr, AtomicString("overflow: visible"));
        UpdateAllLifecyclePhasesForTest();
    }
};

INSTANTIATE_TEST_SUITE_P(All, ReorderOverlayOverflowControlsTest, ::testing::Values(kOverlayScrollbars, kOverlayResizer));
...
TEST_P(ReorderOverlayOverflowControlsTest, StackedWithInFlowDescendant)
{
    ...
    EXPECT_THAT(LayersPaintingOverlayOverflowControlsAfter(child), Pointee(ElementsAre(parent)));
    EXPECT_EQ(parent->GetLayoutObject().GetNode(), HitTest(99, 99));

    GetDocument().getElementById(AtomicString("child"))->setAttribute(html_names::kStyleAttr, AtomicString("position: relative; height: 80px"));
    UpdateAllLifecyclePhasesForTest();
    if (GetOverlayType() == kOverlayScrollbars) {
    ...
    GetDocument().getElementById(AtomicString("child"))->setAttribute(html_names::kStyleAttr, AtomicString("position: relative; width: 200px; height: 80px"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(parent->NeedsReorderOverlayOverflowControls());
    EXPECT_THAT(LayersPaintingOverlayOverflowControlsAfter(child), Pointee(ElementsAre(parent)));
    EXPECT_EQ(parent->GetLayoutObject().GetNode(), HitTest(99, 99));

    GetDocument().getElementById(AtomicString("child"))->setAttribute(html_names::kStyleAttr, AtomicString("width: 200px; height: 80px"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(parent->NeedsReorderOverlayOverflowControls());
    EXPECT_EQ(parent->GetLayoutObject().GetNode(), HitTest(99, 99));
...
TEST_P(ReorderOverlayOverflowControlsTest, StackedWithOutOfFlowDescendant)
{
    ...
    GetDocument().getElementById(AtomicString("child"))->setAttribute(html_names::kStyleAttr, g_empty_atom);
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(parent->NeedsReorderOverlayOverflowControls());
...
TEST_P(ReorderOverlayOverflowControlsTest, StackedWithZIndexDescendant)
{
    ...
    GetDocument().getElementById(AtomicString("child"))->setAttribute(html_names::kStyleAttr, AtomicString("z-index: -1"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(parent->NeedsReorderOverlayOverflowControls());
    EXPECT_FALSE(LayersPaintingOverlayOverflowControlsAfter(child));
    EXPECT_EQ(parent->GetLayoutObject().GetNode(), HitTest(99, 99));

    GetDocument().getElementById(AtomicString("child"))->setAttribute(html_names::kStyleAttr, AtomicString("z-index: 2"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(parent->NeedsReorderOverlayOverflowControls());
    EXPECT_THAT(LayersPaintingOverlayOverflowControlsAfter(child), Pointee(ElementsAre(parent)));
    EXPECT_EQ(parent->GetLayoutObject().GetNode(), HitTest(99, 99));
...
TEST_P(ReorderOverlayOverflowControlsTest, NestedStackedWithInFlowStackedChild)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      body { margin: 0; }
    ...
    EXPECT_TRUE(parent->NeedsReorderOverlayOverflowControls());
    EXPECT_FALSE(child->NeedsReorderOverlayOverflowControls());
    EXPECT_THAT(LayersPaintingOverlayOverflowControlsAfter(child), Pointee(ElementsAre(parent, ancestor)));
    EXPECT_EQ(ancestor->GetLayoutObject().GetNode(), HitTest(99, 99));
}

TEST_P(ReorderOverlayOverflowControlsTest, NestedStackedWithOutOfFlowStackedChild)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      body { margin: 0; }
...
TEST_P(ReorderOverlayOverflowControlsTest, MultipleChildren)
{
    ...
    EXPECT_THAT(LayersPaintingOverlayOverflowControlsAfter(middle_child), Pointee(ElementsAre(parent)));
    ...
    std::string extra_style = GetOverlayType() == kOverlayScrollbars ? "overflow: auto;" : "overflow: hidden; resize: both;";
    std::string new_style = extra_style + "position: absolute; z-index: 1";
    GetDocument().getElementById(AtomicString("parent"))->setAttribute(html_names::kStyleAttr, AtomicString(new_style.c_str()));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(parent->NeedsReorderOverlayOverflowControls());
    EXPECT_FALSE(LayersPaintingOverlayOverflowControlsAfter(low_child));
    ...
    EXPECT_FALSE(LayersPaintingOverlayOverflowControlsAfter(low_child));
    EXPECT_FALSE(LayersPaintingOverlayOverflowControlsAfter(middle_child));
    EXPECT_THAT(LayersPaintingOverlayOverflowControlsAfter(high_child), Pointee(ElementsAre(parent)));
    EXPECT_EQ(parent->GetLayoutObject().GetNode(), HitTest(99, 99));
}
...
TEST_P(ReorderOverlayOverflowControlsTest, NonStackedWithZIndexInFlowDescendant)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      body { margin: 0; }
...
TEST_P(ReorderOverlayOverflowControlsTest, NestedNonStackedWithInFlowStackedChild)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      body { margin: 0; }
...
TEST_P(ReorderOverlayOverflowControlsTest, NestedNonStackedWithOutOfFlowStackedChild)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      body { margin: 0; }
...
TEST_P(ReorderOverlayOverflowControlsTest, AdjustAccessingOrderForSubtreeHighestLayers)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      body { margin: 0; }
...
TEST_P(ReorderOverlayOverflowControlsTest, AddRemoveStackedChild)
{
    ...
    child_element->setAttribute(html_names::kStyleAttr, AtomicString("display: block"));
    UpdateAllLifecyclePhasesExceptPaint();
    EXPECT_TRUE(parent->NeedsReorderOverlayOverflowControls());
    EXPECT_THAT(LayersPaintingOverlayOverflowControlsAfter(GetPaintLayerByElementId("child")), Pointee(ElementsAre(parent)));
...
TEST_P(PaintLayerTest, NegativeZIndexChangeToPositive)
{
    ...
    EXPECT_TRUE(PaintLayerPaintOrderIterator(target, kNegativeZOrderChildren).Next());
    EXPECT_FALSE(PaintLayerPaintOrderIterator(target, kPositiveZOrderChildren).Next());

    GetDocument().getElementById(AtomicString("child"))->setAttribute(html_names::kStyleAttr, AtomicString("z-index: 1"));
    UpdateAllLifecyclePhasesForTest();

    EXPECT_FALSE(PaintLayerPaintOrderIterator(target, kNegativeZOrderChildren).Next());
    EXPECT_TRUE(PaintLayerPaintOrderIterator(target, kPositiveZOrderChildren).Next());
}
...
TEST_P(PaintLayerTest, Has3DTransformedDescendantChangeStyle)
{
    ...
    GetDocument().getElementById(AtomicString("child"))->setAttribute(html_names::kStyleAttr, AtomicString("transform: translateZ(1px)"));
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintLayerTest, DescendantDependentFlagsStopsAtThrottledFrames)
{
    ...
    EXPECT_TRUE(ChildDocument().View()->GetLayoutView()->Layer()->needs_descendant_dependent_flags_update_);
    ...
    EXPECT_FALSE(ChildDocument().View()->GetLayoutView()->Layer()->needs_descendant_dependent_flags_update_);
}
...
TEST_P(PaintLayerTest, CompositingContainerColumnSpanAll)
{
    ...
    EXPECT_EQ(GetPaintLayerByElementId("paintContainer"), columnSpan->CompositingContainer());
    EXPECT_EQ(GetPaintLayerByElementId("multicol"), columnSpan->ContainingLayer());
}
...
TEST_P(PaintLayerTest, CompositingContainerNonStackedFloatUnderStackingCompositedInline)
{
    SetBodyInnerHTML(R"HTML(
    <div id='compositedContainer' style='position: relative;
        will-change: transform'>
...
TEST_P(PaintLayerTest, CompositingContainerStackedUnderFloatUnderStackingInline)
{
    SetBodyInnerHTML(R"HTML(
    <div id='compositedContainer' style='position: relative;
        will-change: transform'>
...
TEST_P(PaintLayerTest, CompositingContainerStackedUnderFloatUnderStackingCompositedInline)
{
    SetBodyInnerHTML(R"HTML(
    <div id='compositedContainer' style='position: relative;
        will-change: transform'>
...
TEST_P(PaintLayerTest, CompositingContainerNonStackedUnderFloatUnderStackingInline)
{
    SetBodyInnerHTML(R"HTML(
    <div id='compositedContainer' style='position: relative;
        will-change: transform'>
...
TEST_P(PaintLayerTest, CompositingContainerNonStackedUnderFloatUnderStackingCompositedInline)
{
    SetBodyInnerHTML(R"HTML(
    <div id='compositedContainer' style='position: relative;
        will-change: transform'>
...
TEST_P(PaintLayerTest, FloatLayerUnderInlineLayerScrolled)
{
    ...
    PaintLayer* span = GetPaintLayerByElementId("span");
    PaintLayer* container = GetPaintLayerByElementId("container");
    container->GetScrollableArea()->SetScrollOffset(ScrollOffset(0, 400), mojom::blink::ScrollType::kProgrammatic);

    EXPECT_EQ(span, floating->Parent());
    EXPECT_EQ(span, floating->ContainingLayer());
    EXPECT_EQ(container, span->Parent());
    EXPECT_EQ(container, span->ContainingLayer());
    EXPECT_EQ(gfx::Vector2d(0, 400), span->ContainingLayer()->PixelSnappedScrolledContentOffset());
}
...
TEST_P(PaintLayerTest, ColumnSpanLayerUnderExtraLayerScrolled)
{
    ...
    columns->GetScrollableArea()->SetScrollOffset(ScrollOffset(200, 0), mojom::blink::ScrollType::kProgrammatic);
    ...
    EXPECT_EQ(gfx::Vector2d(200, 0), spanner->ContainingLayer()->PixelSnappedScrolledContentOffset());
}
...
TEST_P(PaintLayerTest, PaintLayerTransformUpdatedOnStyleTransformAnimation)
{
    ...
    LayoutObject* target_object = GetDocument().getElementById(AtomicString("target"))->GetLayoutObject();
    PaintLayer* target_paint_layer = To<LayoutBoxModelObject>(target_object)->Layer();
...
TEST_P(PaintLayerTest, NeedsRepaintOnSelfPaintingStatusChange)
{
    ...
    target_element->setAttribute(html_names::kStyleAttr, AtomicString("overflow: hidden; float: left"));
...
{
    SetBodyInnerHTML("<style>body {margin-top: 200px; backface-visibility: hidden}</style>"
...
{
    SetBodyInnerHTML("<style> body { width: 1200px; height: 900px; margin: 0 } </style>");
...
TEST_P(PaintLayerTest, HitTestWithIgnoreClipping)
{
    ...
    EXPECT_EQ(GetDocument().getElementById(AtomicString("hit")), result.InnerNode());
}
...
TEST_P(PaintLayerTest, HitTestWithStopNode)
{
    ...
    request = HitTestRequest(HitTestRequest::kReadOnly | HitTestRequest::kActive, hit->GetLayoutObject());
    ...
    request = HitTestRequest(HitTestRequest::kReadOnly | HitTestRequest::kActive | HitTestRequest::kListBased, hit->GetLayoutObject());
...
TEST_P(PaintLayerTest, HitTestTableWithStopNode)
{
    ...
    request = HitTestRequest(HitTestRequest::kReadOnly | HitTestRequest::kActive, table->GetLayoutObject());
    result = HitTestResult(request, location);
    GetDocument().GetLayoutView()->HitTest(location, result);
...
TEST_P(PaintLayerTest, HitTestSVGWithStopNode)
{
    ...
    request = HitTestRequest(HitTestRequest::kReadOnly | HitTestRequest::kActive, svg->GetLayoutObject());
    result = HitTestResult(request, location);
    GetDocument().GetLayoutView()->HitTest(location, result);
...
TEST_P(PaintLayerTest, SetNeedsRepaintSelfPaintingUnderNonSelfPainting)
{
    ...
    auto* html_layer = To<LayoutBoxModelObject>(GetDocument().documentElement()->GetLayoutObject())->Layer();
    auto* span_layer = GetPaintLayerByElementId("span");
...
TEST_P(PaintLayerTest, HitTestPseudoElementWithContinuation)
{
    ...
    EXPECT_EQ(target->GetPseudoElement(kPseudoIdBefore), result.InnerPossiblyPseudoNode());
}
...
TEST_P(PaintLayerTest, HitTestFirstLetterPseudoElement)
{
    ...
    GetDocument().GetLayoutView()->HitTest(location, result);
    EXPECT_EQ(target, result.InnerNode());
    EXPECT_EQ(container->GetPseudoElement(kPseudoIdFirstLetter), result.InnerPossiblyPseudoNode());
}
...
TEST_P(PaintLayerTest, HitTestFloatInsideInlineBoxContainer)
{
    ...
    Node* target = GetDocument().getElementById(AtomicString("target"))->firstChild();
    HitTestRequest request(HitTestRequest::kReadOnly | HitTestRequest::kActive);
...
TEST_P(PaintLayerTest, HitTestOverlayResizer)
{
    ...
    for (int i = 0; i < 6; i++) {
        Element* target_element = GetDocument().getElementById(AtomicString(String::Format("target_%d", i)));
        target_element->setAttribute(html_names::kStyleAttr, AtomicString("display: block"));
        UpdateAllLifecyclePhasesForTest();
        ...
        target_element->setAttribute(html_names::kStyleAttr, AtomicString("display: none"));
    }
}
...
TEST_P(PaintLayerTest, HitTestScrollbarUnderClip)
{
    ...
    EXPECT_EQ(GetDocument().getElementById(AtomicString("target")), HitTest(245, 100));
    // Should not hit the hidden part of the vertical scrollbar, the hidden
    // horizontal scrollbar, or the hidden scroll corner.
    EXPECT_EQ(GetDocument().getElementById(AtomicString("below")), HitTest(245, 200));
    EXPECT_EQ(GetDocument().getElementById(AtomicString("below")), HitTest(150, 245));
    EXPECT_EQ(GetDocument().getElementById(AtomicString("below")), HitTest(245, 245));
}
...
TEST_P(PaintLayerTest, HitTestPerspectiveBackfaceHiddenNotInverted)
{
    ...
    EXPECT_EQ(GetDocument().getElementById(AtomicString("target")), HitTest(50, 80));
    EXPECT_EQ(GetDocument().getElementById(AtomicString("target")), HitTest(149, 179));
...
TEST_P(PaintLayerTest, HitTestObscuredOverlayScrollbar)
{
    ...
    scroller->GetLayoutBox()->GetScrollableArea()->SetScrollbarsHiddenForTesting(false);
    EXPECT_EQ(scroller, HitTest(199, 1));
    EXPECT_EQ(GetDocument().getElementById(AtomicString("above")), HitTest(199, 101));
}

TEST_P(PaintLayerTest, InlineWithBackdropFilterHasPaintLayer)
{
    SetBodyInnerHTML("<map id='target' style='backdrop-filter: invert(1);'></map>");
    PaintLayer* paint_layer = GetPaintLayerByElementId("target");
    PaintLayer* root_layer = GetLayoutView().Layer();
...
#define TEST_SCROLL_CONTAINER(name, expected_scroll_container, expected_is_fixed_to_view)                                                                      \
    do {                                                                                                                                                       \
        auto* layer = GetPaintLayerByElementId(name);                                                                                                          \
        bool is_fixed_to_view = false;                                                                                                                         \
        EXPECT_EQ(expected_scroll_container, layer->ContainingScrollContainerLayer(&is_fixed_to_view));                                                        \
...
TEST_P(PaintLayerTest, ScrollContainerLayerRootScroller)
{
    ...
    {
        bool is_fixed_to_view = false;
        EXPECT_EQ(nullptr, view_layer->ContainingScrollContainerLayer(&is_fixed_to_view));
...
TEST_P(PaintLayerTest, ScrollContainerLayerNestedScroller)
{
    ...
    ASSERT_FALSE(scroller2->GetLayoutObject().CanContainAbsolutePositionObjects());
...
{
    GetDocument().body()->setInnerHTML("<style>"
    ...
    Element* second_div = GetDocument().getElementById(AtomicString("second_div"));
    PseudoElement* second_scroll_marker = second_div->GetPseudoElement(kPseudoIdScrollMarker);
