...
public:
    explicit TextFragmentPainter(const InlineCursor& cursor)
        : cursor_(cursor)
    {
    }
    TextFragmentPainter(const InlineCursor& cursor, const PhysicalOffset& parent_offset, InlinePaintContext* inline_context)
        : cursor_(cursor)
...
private:
    void Paint(const TextFragmentPaintInfo& fragment_paint_info, const LayoutObject* layout_object, const DisplayItemClient& display_item_client,
        const ComputedStyle& style, PhysicalRect box_rect, const gfx::Rect& visual_rect, bool is_ellipsis, bool is_symbol_marker, const PaintInfo& paint_info,
        const PhysicalOffset& paint_offset);

    static void PaintSymbol(const LayoutObject* layout_object, const ComputedStyle& style, const PhysicalSize box_size, const PaintInfo& paint_info,
        const PhysicalOffset& paint_offset);
