...
TEST_F(ScrollableAreaPainterTest, OverlayScrollbars)
{
    ...
    ASSERT_TRUE(GetDocument().GetPage()->GetScrollbarTheme().UsesOverlayScrollbars());
    const auto* target = To<LayoutBox>(GetLayoutObjectByElementId("target"));
    const auto* properties = GetLayoutObjectByElementId("target")->FirstFragment().PaintProperties();
    ASSERT_TRUE(properties);
    ASSERT_TRUE(properties->HorizontalScrollbarEffect());
    ASSERT_TRUE(properties->VerticalScrollbarEffect());

    PaintChunk::Id horizontal_id(target->GetScrollableArea()->HorizontalScrollbar()->Id(), DisplayItem::kScrollbarHorizontal);
    ...
    PaintChunk::Id vertical_id(target->GetScrollableArea()->VerticalScrollbar()->Id(), DisplayItem::kScrollbarVertical);
    ...
    EXPECT_THAT(ContentPaintChunks(),
        ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK_COMMON, _, _, _, IsPaintChunk(1, 2, horizontal_id, horizontal_state),
