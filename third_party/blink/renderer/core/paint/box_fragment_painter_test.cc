...
void ExtractLinks(const PaintRecord& record, std::vector<GURL>* links)
{
    for (const cc::PaintOp& op : record) {
        if (op.GetType() == cc::PaintOpType::kAnnotate) {
            ...
            links->push_back(GURL(std::string(reinterpret_cast<const char*>(annotate_op.data->data()), annotate_op.data->size())));
...
public:
    explicit BoxFragmentPainterTest(LocalFrameClient* local_frame_client = nullptr)
...
TEST_P(BoxFragmentPainterTest, ScrollHitTestOrder)
{
    ...
    const DisplayItemClient& text_fragment = *cursor.Current().GetDisplayItemClient();

    EXPECT_THAT(ContentDisplayItems(), ElementsAre(VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM, IsSameId(text_fragment.Id(), kForegroundType)));
    auto* scroll_hit_test = MakeGarbageCollected<HitTestData>();
    scroll_hit_test->scroll_translation = scroller.FirstFragment().PaintProperties()->ScrollTranslation();
    ...
    EXPECT_THAT(ContentPaintChunks(),
        ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK_COMMON,
            ...
            IsPaintChunk(1, 1, PaintChunk::Id(root_fragment.Id(), DisplayItem::kScrollHitTest), scroller.FirstFragment().LocalBorderBoxProperties(),
...
TEST_P(BoxFragmentPainterTest, AddUrlRects)
{
    ...
    // annotated.
    Document::PaintPreviewScope paint_preview(GetDocument(), Document::kPaintingPreview);
    UpdateAllLifecyclePhasesForTest();

    paint_preview::PaintPreviewTracker tracker(base::UnguessableToken::Create(), std::nullopt, true);
    PaintRecordBuilder builder;
    builder.Context().SetPaintPreviewTracker(&tracker);

    GetDocument().View()->PaintOutsideOfLifecycle(builder.Context(), PaintFlag::kAddUrlMetadata | PaintFlag::kOmitCompositingInfo, CullRect::Infinite());
...
TEST_P(BoxFragmentPainterTest, SelectionTablePainting)
{
    ...
    GetDocument().View()->PaintOutsideOfLifecycle(
        builder.Context(), PaintFlag::kSelectionDragImageOnly | PaintFlag::kOmitCompositingInfo, CullRect::Infinite());
...
<svg xmlns="http://www.w3.org/2000/svg" width="900" height="900"
 ...
 <g font-size="13">
  ...
  <tspan id="fail">Should not be selected.</tspan>
    ...
    auto* root = GetDocument().getElementById(AtomicString("svg"))->GetLayoutBox();
    HitTestResult result;
    root->NodeAtPoint(result, HitTestLocation(gfx::PointF(256, 192)), PhysicalOffset(0, 0), HitTestPhase::kForeground);
    EXPECT_EQ(GetDocument().getElementById(AtomicString("pass")), result.InnerElement());
}
...
TEST_P(BoxFragmentPainterTest, TextareaBoxDecorationBackground)
{
    ...
    EXPECT_THAT(ContentDisplayItems(), ElementsAre(VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM, IsSameId(textarea->Id(), kBackgroundType)));
}
