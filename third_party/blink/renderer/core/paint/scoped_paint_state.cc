...
ScopedPaintState::ScopedPaintState(const LayoutObject& object, const PaintInfo& paint_info, const FragmentData* fragment_data)
    ...
    , input_paint_info_(paint_info)
{
    if (!fragment_to_paint_) {
        ...
        paint_offset_ = PhysicalOffset(LayoutUnit::NearlyMax(), LayoutUnit::NearlyMax());
        return;
    }

    paint_offset_ = fragment_to_paint_->PaintOffset();
    if (paint_info.phase == PaintPhase::kOverlayOverflowControls || (object.HasLayer() && To<LayoutBoxModelObject>(object).HasSelfPaintingLayer())) {
...
void ScopedPaintState::AdjustForPaintProperties(const LayoutObject& object)
{
    ...
    if (!object.Parent() && !object.HasLayer()) {
        ...
        DCHECK(object.IsBox());
        DCHECK_EQ(To<LayoutBox>(object).GetPhysicalFragment(0)->GetBoxType(), PhysicalFragment::kPageBorderBox);
#endif
        ...
        chunk_properties_.emplace(input_paint_info_.context.GetPaintController(), fragment_to_paint_->LocalBorderBoxProperties(), object,
            DisplayItem::PaintPhaseToDrawingType(input_paint_info_.phase));
        return;
    }

    auto new_chunk_properties = input_paint_info_.context.GetPaintController().CurrentPaintChunkProperties();
    bool needs_new_chunk_properties = false;

    if (const auto* paint_offset_translation = properties->PaintOffsetTranslation()) {
    ...
    if (needs_new_chunk_properties) {
        chunk_properties_.emplace(
            input_paint_info_.context.GetPaintController(), new_chunk_properties, object, DisplayItem::PaintPhaseToDrawingType(input_paint_info_.phase));
    }
}
...
{
    DCHECK(input_paint_info_.phase != PaintPhase::kSelfOutlineOnly && input_paint_info_.phase != PaintPhase::kMask);
    ...
    chunk_properties_.emplace(
        input_paint_info_.context.GetPaintController(), fragment_to_paint_->ContentsProperties(), box, input_paint_info_.DisplayItemTypeForClipping());
    ...
    if (box.Layer()->PreviousPaintResult() == kFullyPainted) {
        PhysicalRect contents_visual_rect = PaintLayerPainter::ContentsVisualRect(*fragment_to_paint_, box);
        if (!PhysicalRect(fragment_to_paint_->GetContentsCullRect().Rect()).Contains(contents_visual_rect)) {
            ...
            if (!box.IsLayoutView()) {
                if (auto* scrollable_area = box.GetScrollableArea()) {
                    if (scrollable_area->MaximumScrollOffset().x() != 0) {
                        ...
                        content_rect.Intersect(PhysicalRect(input_paint_info_.GetCullRect().Rect()));
                        mf_checker->NotifyPaintReplaced(content_rect, input_paint_info_.context.GetPaintController().CurrentPaintChunkProperties().Transform());
