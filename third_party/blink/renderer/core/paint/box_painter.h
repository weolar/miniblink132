...
public:
    BoxPainter(const LayoutBox& layout_box)
    ...
    void RecordRegionCaptureData(const PaintInfo& paint_info, const PhysicalRect& paint_rect, const DisplayItemClient& background_client);

    // This should be called in the background paint phase even if there is no
    // other painted content.
    void RecordScrollHitTestData(const PaintInfo& paint_info, const DisplayItemClient& background_client, const FragmentData*);
...
public:
    BoxDrawingRecorder(GraphicsContext& context, const LayoutBox& box, DisplayItem::Type type, const PhysicalOffset& paint_offset)
        : DrawingRecorder(context, box, type, BoxPainter(box).VisualRect(paint_offset))
    {
    }

    BoxDrawingRecorder(GraphicsContext& context, const LayoutBox& box, PaintPhase phase, const PhysicalOffset& paint_offset)
        : BoxDrawingRecorder(context, box, DisplayItem::PaintPhaseToDrawingType(phase), paint_offset)
    {
    }
};
