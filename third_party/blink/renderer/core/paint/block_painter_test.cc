...
TEST_P(BlockPainterTest, BlockingWheelRectsWithoutPaint)
{
    ...
    hit_test_data->wheel_event_rects = { { gfx::Rect(0, 0, 100, 100) }, { gfx::Rect(0, 0, 200, 25) } };
    ContentPaintChunks(), ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK(1, hit_test_data));
...
TEST_P(BlockPainterTest, BlockingWheelEventRectSubsequenceCaching)
{
    ...
    EXPECT_SUBSEQUENCE_FROM_CHUNK(hit_test_client, ContentPaintChunks().begin() + 1, 1);

    PaintChunk::Id hit_test_chunk_id(hit_test_client.Id(), DisplayItem::kLayerChunk);
    auto hit_test_chunk_properties = wheelhandler->EnclosingLayer()->GetLayoutObject().FirstFragment().ContentsProperties();
    auto* hit_test_data = MakeGarbageCollected<HitTestData>();
    ...
    EXPECT_THAT(ContentPaintChunks(),
        ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK_COMMON,
            IsPaintChunk(1, 1, hit_test_chunk_id, hit_test_chunk_properties, hit_test_data, gfx::Rect(0, 0, 100, 100))));
...
TEST_P(BlockPainterTest, WheelEventRectPaintCaching)
{
    ...
    EXPECT_THAT(ContentDisplayItems(), ElementsAre(VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM, IsSameId(sibling->Id(), kBackgroundType)));

    auto* hit_test_data = MakeGarbageCollected<HitTestData>();
    hit_test_data->wheel_event_rects = { { gfx::Rect(0, 0, 100, 100) } };

    EXPECT_THAT(ContentPaintChunks(), ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK(2, hit_test_data)));

    sibling_element->setAttribute(html_names::kStyleAttr, AtomicString("background: green;"));
    PaintController::CounterForTesting counter;
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(BlockPainterTest, BlockingWheelRectOverflowingContents)
{
    ...
    auto* hit_test_data = MakeGarbageCollected<HitTestData>();
    hit_test_data->wheel_event_rects = { gfx::Rect(0, 0, 100, 100), gfx::Rect(0, 0, 10, 400) };
    auto* parent = GetLayoutBoxByElementId("parent");
    EXPECT_THAT(ContentPaintChunks(),
        ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK_COMMON,
            IsPaintChunk(1, 2, PaintChunk::Id(parent->Layer()->Id(), DisplayItem::kLayerChunk), parent->FirstFragment().ContentsProperties(), hit_test_data,
                gfx::Rect(0, 0, 100, 400))));
}
...
TEST_P(BlockPainterTest, BlockingWheelRectScrollingContents)
{
    ...
    auto* scroller = To<LayoutBoxModelObject>(scroller_element->GetLayoutObject());
    const auto& scroller_scrolling_client = scroller->GetScrollableArea()->GetScrollingBackgroundDisplayItemClient();
    ...
    EXPECT_THAT(ContentDisplayItems(),
        ElementsAre(
            VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM, IsSameId(scroller->Id(), kBackgroundType), IsSameId(scroller_scrolling_client.Id(), kBackgroundType)));
    EXPECT_THAT(ContentPaintChunks(),
        ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK_COMMON, IsPaintChunk(1, 2), // scroller background.
            IsPaintChunk(2, 2), // scroller scroll hit test.
            IsPaintChunk(2, 3, PaintChunk::Id(scroller->Id(), kScrollingBackgroundChunkType), scroller->FirstFragment().ContentsProperties(), hit_test_data)));
}
...
TEST_P(BlockPainterTest, WheelEventRectPaintChunkChanges)
{
    ...
    EXPECT_THAT(ContentPaintChunks(), ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK(1, hit_test_data)));
...
TEST_P(BlockPainterTest, TouchActionRectsWithoutPaint)
{
    ...
    auto* parent_element = GetElementById("parent");
    parent_element->setAttribute(html_names::kClassAttr, AtomicString("touchActionNone"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_THAT(ContentDisplayItems(), ElementsAre(VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM));
    auto* hit_test_data = MakeGarbageCollected<HitTestData>();
    hit_test_data->touch_action_rects = { { gfx::Rect(0, 0, 100, 100) }, { gfx::Rect(0, 0, 200, 25) } };
    ContentPaintChunks(), ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK(1, hit_test_data));
...
TEST_P(BlockPainterTest, TouchActionRectSubsequenceCaching)
{
    ...
    PaintChunk::Id hit_test_chunk_id(hit_test_client.Id(), DisplayItem::kLayerChunk);
    auto hit_test_chunk_properties = touchaction->EnclosingLayer()->GetLayoutObject().FirstFragment().ContentsProperties();
    auto* hit_test_data = MakeGarbageCollected<HitTestData>();
...
TEST_P(BlockPainterTest, TouchActionRectScrollingContents)
{
    ...
    const auto& scroller_scrolling_client = scroller->GetScrollableArea()->GetScrollingBackgroundDisplayItemClient();
    auto* hit_test_data = MakeGarbageCollected<HitTestData>();
    hit_test_data->touch_action_rects = { { gfx::Rect(0, 0, 100, 400), TouchAction::kPinchZoom }, { gfx::Rect(0, 200, 10, 200), TouchAction::kNone } };
    EXPECT_THAT(ContentDisplayItems(),
        ElementsAre(
            VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM, IsSameId(scroller->Id(), kBackgroundType), IsSameId(scroller_scrolling_client.Id(), kBackgroundType)));
    EXPECT_THAT(ContentPaintChunks(),
...
TEST_P(BlockPainterTest, TouchActionRectPaintChunkChanges)
{
    ...
    touchaction_element->setAttribute(html_names::kStyleAttr, AtomicString("touch-action: none;"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_THAT(ContentDisplayItems(), ElementsAre(VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM));

    PaintChunk::Id hit_test_chunk_id(touchaction->EnclosingLayer()->Id(), kHitTestChunkType);
    auto* hit_test_data = MakeGarbageCollected<HitTestData>();
    hit_test_data->touch_action_rects = { { gfx::Rect(0, 0, 100, 100) } };
...
TEST_P(BlockPainterTest, TouchHandlerRectsWithoutPaint)
{
    ...
    BlockPainterMockEventListener* callback = MakeGarbageCollected<BlockPainterMockEventListener>();
    auto* parent_element = GetElementById("parent");
    ...
    EXPECT_THAT(ContentDisplayItems(), ElementsAre(VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM));
    auto* hit_test_data = MakeGarbageCollected<HitTestData>();
    hit_test_data->touch_action_rects = { { gfx::Rect(0, 0, 100, 100) }, { gfx::Rect(0, 0, 200, 50) } };
    EXPECT_THAT(ContentPaintChunks(), ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK(1, hit_test_data)));
...
TEST_P(BlockPainterTest, TouchActionRectsAcrossPaintChanges)
{
    ...
    auto* hit_test_data = MakeGarbageCollected<HitTestData>();
    hit_test_data->touch_action_rects = { { gfx::Rect(0, 0, 100, 100) }, { gfx::Rect(0, 0, 200, 50) } };
    EXPECT_THAT(ContentPaintChunks(), ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK(1, hit_test_data, gfx::Rect(0, 0, 800, 600))));

    auto* child_element = GetElementById("child");
    child_element->setAttribute(html_names::kStyleAttr, AtomicString("background: blue;"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_THAT(ContentDisplayItems(), ElementsAre(VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM, IsSameId(child_element->GetLayoutObject()->Id(), kBackgroundType)));
    EXPECT_THAT(ContentPaintChunks(), ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK(2, hit_test_data)));
}
...
TEST_P(BlockPainterTest, ScrolledHitTestChunkProperties)
{
    ...
    const auto* scroller = To<LayoutBlock>(GetLayoutObjectByElementId("scroller"));
    EXPECT_THAT(ContentDisplayItems(), ElementsAre(VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM));

    auto* scroller_touch_action_hit_test_data = MakeGarbageCollected<HitTestData>();
    scroller_touch_action_hit_test_data->touch_action_rects = { { gfx::Rect(0, 0, 100, 100) } };
    auto* scroll_hit_test_data = MakeGarbageCollected<HitTestData>();
    scroll_hit_test_data->scroll_translation = scroller->FirstFragment().PaintProperties()->ScrollTranslation();
    ...
    scrolled_hit_test_data->touch_action_rects
        = { { RuntimeEnabledFeatures::HitTestOpaquenessEnabled() ? gfx::Rect(0, 0, 200, 100) : gfx::Rect(0, 0, 200, 50) } };

    const auto& paint_chunks = ContentPaintChunks();
    EXPECT_THAT(paint_chunks,
        ElementsAre(VIEW_SCROLLING_BACKGROUND_CHUNK_COMMON,
            IsPaintChunk(1, 1, PaintChunk::Id(scroller->Id(), kBackgroundChunkType), scroller->FirstFragment().LocalBorderBoxProperties(),
                scroller_touch_action_hit_test_data, gfx::Rect(0, 0, 100, 100)),
            IsPaintChunk(1, 1, PaintChunk::Id(scroller->Id(), DisplayItem::kScrollHitTest), scroller->FirstFragment().LocalBorderBoxProperties(),
                scroll_hit_test_data, gfx::Rect(0, 0, 100, 100)),
            IsPaintChunk(1, 1,
                PaintChunk::Id(
                    scroller->Id(), RuntimeEnabledFeatures::HitTestOpaquenessEnabled() ? kScrollingBackgroundChunkType : kClippedContentsBackgroundChunkType),
    ...
    EXPECT_FALSE(ToUnaliased(scroller_paint_chunk.properties.Transform()).ScrollNode());

    const auto& scrolled_paint_chunk = paint_chunks[3];
    // The hit test rect for the scrolled contents should be scrolled.
    EXPECT_TRUE(ToUnaliased(scrolled_paint_chunk.properties.Transform()).ScrollNode());
}
