...
public:
    ...
    {
        return GetDocument().View()->GetLayoutView()->FirstFragment().PaintProperties()->PaintOffsetTranslation();
    }

    const TransformPaintPropertyNode* FrameScrollTranslation()
    {
        return GetDocument().View()->GetLayoutView()->FirstFragment().PaintProperties()->ScrollTranslation();
    }
...
TEST_P(PrePaintTreeWalkTest, PropertyTreesRebuiltWithBorderInvalidation)
{
    ...
    auto* transformed_element = GetDocument().getElementById(AtomicString("transformed"));
    const auto* transformed_properties = transformed_element->GetLayoutObject()->FirstFragment().PaintProperties();
    EXPECT_EQ(gfx::Vector2dF(100, 100), transformed_properties->Transform()->Get2dTranslation());
    ...
    transformed_element->setAttribute(html_names::kClassAttr, AtomicString("border"));
...
TEST_P(PrePaintTreeWalkTest, PropertyTreesRebuiltWithFrameScroll)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(0, -100), FrameScrollTranslation()->Get2dTranslation());
}
...
TEST_P(PrePaintTreeWalkTest, PropertyTreesRebuiltWithCSSTransformInvalidation)
{
    ...
    transformed_element->setAttribute(html_names::kClassAttr, AtomicString("transformB"));
    UpdateAllLifecyclePhasesForTest();

    // The transform should have changed.
    EXPECT_EQ(gfx::Vector2dF(200, 200), transformed_properties->Transform()->Get2dTranslation());
}
...
TEST_P(PrePaintTreeWalkTest, PropertyTreesRebuiltWithOpacityInvalidation)
{
    ...
    auto* transparent_element = GetDocument().getElementById(AtomicString("transparent"));
    const auto* transparent_properties = transparent_element->GetLayoutObject()->FirstFragment().PaintProperties();
    EXPECT_EQ(0.9f, transparent_properties->Effect()->Opacity());

    // Invalidate the opacity property.
    transparent_element->setAttribute(html_names::kClassAttr, AtomicString("opacityB"));
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(PrePaintTreeWalkTest, ClipChangeRepaintsDescendants)
{
    ...
    GetDocument().getElementById(AtomicString("parent"))->setAttribute(html_names::kStyleAttr, AtomicString("height: 100px"));
    UpdateAllLifecyclePhasesExceptPaint();
...
TEST_P(PrePaintTreeWalkTest, InsideBlockingTouchEventHandlerUpdate)
{
    ...
    PrePaintTreeWalkMockEventListener* callback = MakeGarbageCollected<PrePaintTreeWalkMockEventListener>();
...
TEST_P(PrePaintTreeWalkTest, EffectiveTouchActionStyleUpdate)
{
    ...
    GetDocument().getElementById(AtomicString("touchaction"))->setAttribute(html_names::kClassAttr, AtomicString("touchaction"));
...
TEST_P(PrePaintTreeWalkTest, CullRectUpdateOnSVGTransformChange)
{
    ...
    EXPECT_EQ(gfx::Rect(0, 0, 200, 200), foreign.FirstFragment().GetCullRect().Rect());

    GetDocument().getElementById(AtomicString("rect"))->setAttribute(html_names::kStyleAttr, AtomicString("transform: translateX(20px)"));
    UpdateAllLifecyclePhasesExceptPaint();
    EXPECT_EQ(gfx::Rect(0, 0, 200, 200), foreign.FirstFragment().GetCullRect().Rect());

    GetDocument().getElementById(AtomicString("g"))->setAttribute(html_names::kStyleAttr, AtomicString("transform: translateY(20px)"));
    UpdateAllLifecyclePhasesExceptPaint();
    EXPECT_EQ(gfx::Rect(0, -20, 200, 200), foreign.FirstFragment().GetCullRect().Rect());
}
...
TEST_P(PrePaintTreeWalkTest, InlineOutlineWithContinuationPaintInvalidation)
{
    ...
    GetDocument().getElementById(AtomicString("child-span"))->setAttribute(html_names::kStyleAttr, AtomicString("color: blue"));
    UpdateAllLifecyclePhasesForTest();
}
