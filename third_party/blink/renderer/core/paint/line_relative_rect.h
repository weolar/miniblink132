...
struct CORE_EXPORT LineRelativeRect {
    ...
    static LineRelativeRect CreateFromLineBox(const PhysicalRect& rect, bool is_horizontal)
    {
        return { LineRelativeOffset::CreateFromBoxOrigin(rect.offset),
            LogicalSize { is_horizontal ? rect.size.width : rect.size.height, is_horizontal ? rect.size.height : rect.size.width } };
    }
    ...
    static LineRelativeRect Create(const PhysicalRect& rect, const std::optional<AffineTransform>& rotation)
    {
        if (!rotation || rotation == AffineTransform()) {
            return { { rect.offset.left, rect.offset.top }, { rect.size.width, rect.size.height } };
        }
