...
static ResolvedUnderlinePosition ResolveUnderlinePosition(const ComputedStyle& style)
{
...
static float ComputeDecorationThickness(
    const TextDecorationThickness text_decoration_thickness, float computed_font_size, float minimum_thickness, const SimpleFontData* font_data)
{
    float auto_underline_thickness = std::max(minimum_thickness, computed_font_size / 10.f);
    ...
    if (text_decoration_thickness.IsFromFont()) {
        std::optional<float> font_underline_thickness = font_data->GetFontMetrics().UnderlineThickness();
    ...
    float text_decoration_thickness_pixels = FloatValueForLength(thickness_length, computed_font_size);

    return std::max(minimum_thickness, roundf(text_decoration_thickness_pixels));
}

static enum StrokeStyle TextDecorationStyleToStrokeStyle(ETextDecorationStyle decoration_style)
{
...
gfx::RectF ComputeWavyPatternRect(const WavyParams& params, const Path& stroke_path)
{
...
cc::PaintRecord PrepareWavyTileRecord(const WavyParams& params, const Path& stroke_path, const gfx::RectF& pattern_rect)
{
    cc::PaintFlags flags;
...
TextDecorationInfo::TextDecorationInfo(LineRelativeOffset local_origin, LayoutUnit width, const ComputedStyle& target_style,
    const InlinePaintContext* inline_context, const TextDecorationLine selection_decoration_line, const Color selection_decoration_color,
    const AppliedTextDecoration* decoration_override, const Font* font_override, MinimumThickness1 minimum_thickness1, float scaling_factor)
    ...
    , font_override_(font_override && font_override != &target_style.GetFont() ? font_override : nullptr)
    , local_origin_(local_origin)
    , width_(width)
    , target_ascent_(GetAscent(target_style, font_override))
    , scaling_factor_(scaling_factor)
    , use_decorating_box_(inline_context && !decoration_override_ && !font_override_ && ShouldUseDecoratingBox(target_style))
    , minimum_thickness_is_one_(minimum_thickness1)
{
    ...
    for (wtf_size_t i = 0; i < AppliedDecorationCount(); i++) {
        if (AppliedDecoration(i).Style() == ETextDecorationStyle::kDotted || AppliedDecoration(i).Style() == ETextDecorationStyle::kDashed) {
            antialias_ = true;
            break;
        }
    }
...
const AppliedTextDecoration& TextDecorationInfo::AppliedDecoration(wtf_size_t index) const
{
    if (HasDecorationOverride())
...
void TextDecorationInfo::UpdateForDecorationIndex()
{
    ...
    if (use_decorating_box_) {
        ...
        DCHECK_EQ(inline_context_->DecoratingBoxes().size(), AppliedDecorationCount());
    ...
    if (decorating_box_style != decorating_box_style_) {
        ...
        original_underline_position_ = ResolveUnderlinePosition(*decorating_box_style);

        // text-underline-position may flip underline and overline.
        flip_underline_and_overline_ = original_underline_position_ == ResolvedUnderlinePosition::kOver;
    }
    ...
    const Font* font = font_override_ ? font_override_ : &decorating_box_style_->GetFont();
    DCHECK(font);
    if (font != font_) {
        font_ = font;
...
void TextDecorationInfo::SetLineData(TextDecorationLine line, float line_offset)
{
    ...
    case ETextDecorationStyle::kWavy:
        ...
        ComputeWavyLineData(line_data_.wavy_pattern_rect, line_data_.wavy_tile_record);
        break;
    default:
        line_data_.stroke_path.reset();
        line_data_.wavy_tile_record = cc::PaintRecord();
...
LayoutUnit TextDecorationInfo::OffsetFromDecoratingBox() const
{
    ...
    const LayoutUnit decorating_box_paint_offset = decorating_box_->ContentOffsetInContainer().top + inline_context_->PaintOffset().top;
    return decorating_box_paint_offset - local_origin_.line_over;
}

void TextDecorationInfo::SetUnderlineLineData(const TextDecorationOffset& decoration_offset)
{
    DCHECK(HasUnderline());
    ...
    float paint_underline_offset
        = decoration_offset.ComputeUnderlineOffset(FlippedUnderlinePosition(), ComputedFontSize(), FontData(), line_offset, ResolvedThickness());
    if (use_decorating_box_) {
...
void TextDecorationInfo::SetOverlineLineData(const TextDecorationOffset& decoration_offset)
{
    DCHECK(HasOverline());
    ...
    const int paint_overline_offset
        = decoration_offset.ComputeUnderlineOffsetForUnder(line_offset, TargetStyle().ComputedFontSize(), FontData(), ResolvedThickness(), position);
...
void TextDecorationInfo::SetSpellingOrGrammarErrorLineData(const TextDecorationOffset& decoration_offset)
{
    ...
    const int paint_underline_offset
        = decoration_offset.ComputeUnderlineOffset(FlippedUnderlinePosition(), TargetStyle().ComputedFontSize(), FontData(), Length(), ResolvedThickness());
    SetLineData(HasSpellingError() ? TextDecorationLine::kSpellingError : TextDecorationLine::kGrammarError, paint_underline_offset);
}

bool TextDecorationInfo::ShouldAntialias() const
{
#if BUILDFLAG(IS_APPLE)
    if (line_data_.line == TextDecorationLine::kSpellingError || line_data_.line == TextDecorationLine::kGrammarError) {
        return true;
    }
#endif
    return antialias_;
...
float TextDecorationInfo::ComputeThickness() const
{
    ...
    return ComputeUnderlineThickness(decoration.Thickness(), decorating_box_style_);
}

float TextDecorationInfo::ComputeUnderlineThickness(
    const TextDecorationThickness& applied_decoration_thickness, const ComputedStyle* decorating_box_style) const
{
    const float minimum_thickness = minimum_thickness_is_one_ ? 1.0f : 0.0f;
    float thickness = 0;
    if (flipped_underline_position_ == ResolvedUnderlinePosition::kNearAlphabeticBaselineAuto
        || flipped_underline_position_ == ResolvedUnderlinePosition::kNearAlphabeticBaselineFromFont) {
        thickness = ComputeDecorationThickness(applied_decoration_thickness, computed_font_size_, minimum_thickness, font_data_);
    } else {
        ...
        if (decorating_box_style) {
            thickness = ComputeDecorationThickness(
                applied_decoration_thickness, decorating_box_style->ComputedFontSize(), minimum_thickness, decorating_box_style->GetFont().PrimaryFont());
        } else {
            thickness = ComputeDecorationThickness(applied_decoration_thickness, computed_font_size_, minimum_thickness, font_data_);
        }
    }
...
void TextDecorationInfo::ComputeWavyLineData(gfx::RectF& pattern_rect, cc::PaintRecord& tile_record) const
{
    struct WavyCache {
        WavyParams key;
    ...
    if (wavy_cache && wavy_cache->key.resolved_thickness == ResolvedThickness() && wavy_cache->key.effective_zoom == decorating_box_style_->EffectiveZoom()
        && wavy_cache->key.spelling_grammar == IsSpellingOrGrammarError() && wavy_cache->key.color == LineColor()) {
        pattern_rect = wavy_cache->pattern_rect;
        tile_record = wavy_cache->tile_record;
        return;
    }

    WavyParams params { ResolvedThickness(), decorating_box_style_->EffectiveZoom(), IsSpellingOrGrammarError(), LineColor() };
...
gfx::RectF TextDecorationInfo::Bounds() const
{
    ...
    case ETextDecorationStyle::kDouble:
        if (DoubleOffset() > 0) {
            return gfx::RectF(start_point.x(), start_point.y(), width_, DoubleOffset() + ResolvedThickness());
        }
        return gfx::RectF(start_point.x(), start_point.y() + DoubleOffset(), width_, -DoubleOffset() + ResolvedThickness());
    case ETextDecorationStyle::kSolid:
        return gfx::RectF(start_point.x(), start_point.y(), width_, ResolvedThickness());
    default:
        break;
    }
    NOTREACHED();
...
gfx::RectF TextDecorationInfo::BoundsForDottedOrDashed() const
{
    ...
    return line_data_.stroke_path.value().StrokeBoundingRect(styled_stroke.ConvertToStrokeData({}));
}
...
void TextDecorationInfo::SetHighlightOverrideColor(const std::optional<Color>& color)
{
...
Path TextDecorationInfo::PrepareDottedOrDashedStrokePath() const
{
    ...
    gfx::PointF start_point = StartPoint();
    return DecorationLinePainter::GetPathForTextLine(start_point, width_, ResolvedThickness(), TextDecorationStyleToStrokeStyle(DecorationStyle()));
}
