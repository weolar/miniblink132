...
PaintRecord RecordMarker(Color blink_color)
{
    ...
    path.cubicTo(kMarkerWidth * -1 / 8, kMarkerHeight * 3 / 4, kMarkerWidth * -1 / 8, kMarkerHeight * 1 / 4, kMarkerWidth * 1 / 8, kMarkerHeight * 1 / 4);
    path.cubicTo(kMarkerWidth * 3 / 8, kMarkerHeight * 1 / 4, kMarkerWidth * 3 / 8, kMarkerHeight * 3 / 4, kMarkerWidth * 5 / 8, kMarkerHeight * 3 / 4);
    path.cubicTo(kMarkerWidth * 7 / 8, kMarkerHeight * 3 / 4, kMarkerWidth * 7 / 8, kMarkerHeight * 1 / 4, kMarkerWidth * 9 / 8, kMarkerHeight * 1 / 4);
...
PaintRecord RecordMarker(Color blink_color)
{
    ...
    PaintRecorder recorder;
    recorder.beginRecording();
    recorder.getRecordingCanvas()->drawOval(SkRect::MakeWH(2 * kR, 2 * kR), flags);
    return recorder.finishRecordingAsPicture();
}

#endif // !BUILDFLAG(IS_APPLE)

void DrawDocumentMarker(GraphicsContext& context, const gfx::PointF& pt, float width, float zoom, PaintRecord marker)
{
    ...
    flags.setAntiAlias(true);
    flags.setShader(
        PaintShader::MakePaintRecord(std::move(marker), SkRect::MakeWH(kMarkerWidth, kMarkerHeight), SkTileMode::kRepeat, SkTileMode::kClamp, &local_matrix));
...
bool StyleableMarkerPainter::ShouldPaintUnderline(const StyleableMarker& marker)
{
    if (marker.HasThicknessNone() || (marker.UnderlineColor() == Color::kTransparent && !marker.UseTextColor())
...
void StyleableMarkerPainter::PaintUnderline(const StyleableMarker& marker, GraphicsContext& context, const PhysicalOffset& box_origin,
    const ComputedStyle& style, const LineRelativeRect& marker_rect, LayoutUnit logical_height, bool in_dark_mode)
{
    ...
    Color marker_color = (marker.UseTextColor() || in_dark_mode) ? style.VisitedDependentColor(GetCSSPropertyWebkitTextFillColor()) : marker.UnderlineColor();
    ...
    if (marker.UnderlineStyle() != UnderlineStyle::kSquiggle) {
        ...
        DecorationLinePainter::DrawLineForText(context,
            gfx::PointF(box_origin.left + start, (box_origin.top + logical_height.ToInt() - line_thickness).ToFloat()), width, styled_stroke,
        ...
        if (marker.GetType() == DocumentMarker::kComposition) {
            ...
            DrawDocumentMarker(context, gfx::PointF((box_origin.left + start).ToFloat(), (box_origin.top + logical_height.ToInt() - line_thickness).ToFloat()),
