...
AffineTransform MaskToContentTransform(const LayoutSVGResourceMasker& masker, const gfx::RectF& reference_box, float zoom)
{
    AffineTransform content_transformation;
    if (masker.MaskContentUnits() == SVGUnitTypes::kSvgUnitTypeObjectboundingbox) {
        content_transformation.Translate(reference_box.x(), reference_box.y());
        content_transformation.ScaleNonUniform(reference_box.width(), reference_box.height());
    } else if (zoom != 1) {
...
LayoutSVGResourceMasker* ResolveElementReference(const StyleMaskSourceImage& mask_source, const ImageResourceObserver& observer)
{
    ...
    auto* masker = GetSVGResourceAsType<LayoutSVGResourceMasker>(*client, mask_resource);
    if (!masker) {
        return nullptr;
    }
...
void PaintMaskLayer(const FillLayer& layer, const LayoutObject& object, const SVGBackgroundPaintContext& bg_paint_context, GraphicsContext& context)
{
    ...
    if (layer.Next()) {
        composite_op = WebCoreCompositeToSkiaComposite(layer.Composite(), layer.GetBlendMode());
    }
    ...
    if (const auto* mask_source = ToMaskSourceIfSVGMask(*style_image)) {
        ...
        gfx::RectF reference_box = SVGResources::ReferenceBoxForEffects(object, GeometryBox::kFillBox, SVGResources::ForeignObjectQuirk::kDisabled);
        reference_box.Scale(zoom);

        saver.Save();
        SVGMaskPainter::PaintSVGMaskLayer(context, *mask_source, observer, reference_box, zoom, composite_op, layer.MaskMode() == EFillMaskMode::kMatchSource);
        return;
    }
    ...
    scoped_refptr<Image> image = style_image->GetImage(observer, document, style, gfx::SizeF(geometry.TileSize()));
    if (!image) {
        return;
    }

    ScopedImageRenderingSettings image_rendering_settings_context(context, style.GetInterpolationQuality(), style.GetDynamicRangeLimit());
    ...
    if (clip_box) {
        gfx::RectF clip_rect = SVGResources::ReferenceBoxForEffects(object, *clip_box, SVGResources::ForeignObjectQuirk::kDisabled);
    ...
    const RespectImageOrientationEnum respect_orientation = style_image->ForceOrientationIfNecessary(style.ImageOrientation());
    ...
    tiling_info.phase = dest_rect.origin() + gfx::Vector2dF(geometry.ComputePhase());
    tiling_info.spacing = gfx::SizeF(geometry.SpaceSize());
    tiling_info.scale = { geometry.TileSize().width / tiling_info.image_rect.width(), geometry.TileSize().height / tiling_info.image_rect.height() };

    auto image_auto_dark_mode = ImageClassifierHelper::GetImageAutoDarkMode(*document.GetFrame(), style, dest_rect, tiling_info.image_rect);
    ...
    context.DrawImageTiled(*image, dest_rect, tiling_info, image_auto_dark_mode, paint_timing_info, composite_op, respect_orientation);
}

} // namespace

void SVGMaskPainter::Paint(GraphicsContext& context, const LayoutObject& layout_object, const DisplayItemClient& display_item_client)
{
    const auto* properties = layout_object.FirstFragment().PaintProperties();
    DCHECK(properties);
    DCHECK(properties->Mask());
    DCHECK(properties->MaskClip());
    PropertyTreeStateOrAlias property_tree_state(properties->Mask()->LocalTransformSpace(), *properties->MaskClip(), *properties->Mask());
    ScopedPaintChunkProperties scoped_paint_chunk_properties(context.GetPaintController(), property_tree_state, display_item_client, DisplayItem::kSVGMask);

    if (DrawingRecorder::UseCachedDrawingIfPossible(context, display_item_client, DisplayItem::kSVGMask))
        return;
    ...
    DrawingRecorder recorder(context, display_item_client, DisplayItem::kSVGMask, gfx::ToEnclosingRect(visual_rect));
...
void SVGMaskPainter::PaintSVGMaskLayer(GraphicsContext& context, const StyleMaskSourceImage& mask_source, const ImageResourceObserver& observer,
    const gfx::RectF& reference_box, const float zoom, const SkBlendMode composite_op, const bool apply_mask_type)
{
    LayoutSVGResourceMasker* masker = ResolveElementReference(mask_source, observer);
    if (!masker) {
        return;
    }
    const AffineTransform content_transformation = MaskToContentTransform(*masker, reference_box, zoom);
    ...
    if (apply_mask_type && masker->StyleRef().MaskType() == EMaskType::kLuminance) {
        context.BeginLayer(cc::ColorFilter::MakeLuma(), &composite_op);
        has_layer = true;
...
bool SVGMaskPainter::MaskIsValid(const StyleMaskSourceImage& mask_source, const ImageResourceObserver& observer)
{
    return ResolveElementReference(mask_source, observer);
}

gfx::RectF SVGMaskPainter::ResourceBoundsForSVGChild(const LayoutObject& object)
{
    const ComputedStyle& style = object.StyleRef();
    const gfx::RectF reference_box = SVGResources::ReferenceBoxForEffects(object, GeometryBox::kFillBox, SVGResources::ForeignObjectQuirk::kDisabled);
    const float reference_box_zoom = object.IsSVGForeignObject() ? style.EffectiveZoom() : 1;
    gfx::RectF bounds;
    for (const FillLayer* layer = &style.MaskLayers(); layer; layer = layer->Next()) {
        const auto* mask_source = DynamicTo<StyleMaskSourceImage>(layer->GetImage());
        if (!mask_source) {
            continue;
        }
        LayoutSVGResourceMasker* masker = ResolveElementReference(*mask_source, object);
        if (!masker) {
            continue;
        }
        const gfx::RectF svg_mask_bounds = masker->ResourceBoundingBox(reference_box, reference_box_zoom);
