...
void ExtractLinks(const PaintRecord& record, std::vector<std::pair<GURL, SkRect>>* links)
{
    for (const cc::PaintOp& op : record) {
        if (op.GetType() == cc::PaintOpType::kAnnotate) {
            ...
            links->push_back(
                std::make_pair(GURL(std::string(reinterpret_cast<const char*>(annotate_op.data->data()), annotate_op.data->size())), annotate_op.rect));
...
public:
    VideoPainterTest()
        : PaintControllerPaintTestBase(MakeGarbageCollected<VideoStubLocalFrameClient>())
    {
    }
...
TEST_F(VideoPainterTest, VideoLayerAppearsInLayerTree)
{
    ...
    StubWebMediaPlayer* player = static_cast<StubWebMediaPlayer*>(element->GetWebMediaPlayer());
...
public:
    ...
    MOCK_METHOD3(Paint, void(cc::PaintCanvas*, const gfx::Rect&, cc::PaintFlags&));
};
...
public:
        ...
        scoped_refptr<base::TaskRunner> compositor_worker_task_runner) override
    {
        ...
        EXPECT_CALL(*player, HasAvailableVideoFrame).WillRepeatedly(testing::Return(false));
        return player;
    }
};

class VideoPaintPreviewTest : public testing::Test, public PaintTestConfigurations {
public:
    ...
    void SetUp() override
    {
        ...
        frame_impl.ViewImpl()->MainFrameViewWidget()->Resize(gfx::Size(bounds().size()));
    ...
    {
        frame_test_helpers::LoadHTMLString(&GetLocalMainFrame(), content, KURL("http://test.com"));
    }
    ...
    bool PlayVideo()
    {
        ...
        auto* element = To<HTMLMediaElement>(GetDocument().body()->firstChild());
        MockWebMediaPlayer* player = static_cast<MockWebMediaPlayer*>(element->GetWebMediaPlayer());
        EXPECT_CALL(*player, HasAvailableVideoFrame).WillRepeatedly(testing::Return(true));
        auto play_result = element->Play();
        EXPECT_FALSE(play_result.has_value()) << "DOM Exception when playing: " << static_cast<int>(play_result.value());
    ...
    cc::PaintRecord CapturePaintPreview(bool skip_accelerated_content)
    {
        ...
        const base::UnguessableToken embedding_token = base::UnguessableToken::Create();
        const bool is_main_frame = true;

        cc::PaintRecorder recorder;
        paint_preview::PaintPreviewTracker tracker(token, embedding_token, is_main_frame);
...
private:
    ...
    LocalFrame* GetFrame()
    {
        return GetLocalMainFrame().GetFrame();
    }
