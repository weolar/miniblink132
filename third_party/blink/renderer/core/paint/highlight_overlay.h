...
public:
    ...
    public:
        explicit HighlightLayer(HighlightLayerType type, const AtomicString& name = g_null_atom);
        ...
        int8_t ComparePaintOrder(const HighlightLayer&, const HighlightRegistry*) const;
    ...
    public:
        HighlightEdge(HighlightRange range, HighlightLayerType layer_type, uint16_t layer_index, HighlightEdgeType edge_type)
            : range(range)
        ...
        bool LessThan(const HighlightEdge&, const HeapVector<HighlightLayer>& layers, const HighlightRegistry*) const;
    ...
    public:
        HighlightDecoration(HighlightLayerType type, uint16_t layer_index, HighlightRange range, Color override_color);
    ...
    public:
        HighlightPart(HighlightLayerType, uint16_t, HighlightRange, TextPaintStyle, float, Vector<HighlightDecoration>, Vector<HighlightBackground>,
            Vector<HighlightTextShadow>);
        HighlightPart(HighlightLayerType, uint16_t, HighlightRange, TextPaintStyle, float, Vector<HighlightDecoration>);
    ...
    static HeapVector<HighlightLayer> ComputeLayers(const Document& document, Node* node, const ComputedStyle& originating_style,
    ...
    static Vector<HighlightEdge> ComputeEdges(const Node*, bool is_generated_text_fragment, std::optional<TextOffsetRange> dom_offsets,
        const HeapVector<HighlightLayer>& layers, const LayoutSelectionStatus* selection, const DocumentMarkerVector& custom,
        const DocumentMarkerVector& grammar, const DocumentMarkerVector& spelling, const DocumentMarkerVector& target, const DocumentMarkerVector& search);
    ...
    static HeapVector<HighlightPart> ComputeParts(
        const TextFragmentPaintInfo& originating, const HeapVector<HighlightLayer>& layers, const Vector<HighlightEdge>& edges);
};

CORE_EXPORT std::ostream& operator<<(std::ostream&, const HighlightOverlay::HighlightLayer&);
CORE_EXPORT std::ostream& operator<<(std::ostream&, const HighlightOverlay::HighlightEdge&);
CORE_EXPORT std::ostream& operator<<(std::ostream&, const HighlightOverlay::HighlightPart&);

} // namespace blink

WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::HighlightOverlay::HighlightLayer)
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::HighlightOverlay::HighlightPart)
