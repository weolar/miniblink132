...
ImagePaintTimingInfo ComputeImagePaintTimingInfo(const LayoutImage& layout_image, const Image& image, const ImageResourceContent* image_content,
    const GraphicsContext& context, const gfx::Rect& image_border)
{
    ...
    return ImagePaintTimingInfo(PaintTimingDetector::NotifyImagePaint(
        layout_image, image.Size(), *image_content, context.GetPaintController().CurrentPaintChunkProperties(), image_border));
}
...
void ImagePainter::PaintAreaElementFocusRing(const PaintInfo& paint_info)
{
    ...
    if (document.Printing() || !document.GetFrame()->Selection().FrameIsFocusedAndActive())
        return;
    ...
    if (DrawingRecorder::UseCachedDrawingIfPossible(paint_info.context, layout_image_, DisplayItem::kImageAreaFocusRing))
        return;

    BoxDrawingRecorder recorder(paint_info.context, layout_image_, DisplayItem::kImageAreaFocusRing, paint_offset);
    ...
    OutlinePainter::PaintFocusRingPath(paint_info.context, path, *area_element_style);
    paint_info.context.Restore();
}

void ImagePainter::PaintReplaced(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    ...
    PhysicalRect content_rect(paint_offset + layout_image_.PhysicalContentBoxOffset(), content_size);
    ...
    // property nodes.
    PhysicalRect visual_rect = layout_image_.ClipsToContentBox() ? content_rect : paint_rect;
    ...
    if (layout_image_.CachedImage() && layout_image_.CachedImage()->GetImage()->IsSVGImage()) {
    ...
    if (DrawingRecorder::UseCachedDrawingIfPossible(context, layout_image_, paint_info.phase))
        return;
    ...
    if (RuntimeEnabledFeatures::PaintUnderInvalidationCheckingEnabled() && layout_image_.ImageResource() && layout_image_.ImageResource()->MaybeAnimated())
        cache_skipper.emplace(context);

    if (!has_image) {
        // Draw an outline rect where the image should be.
        BoxDrawingRecorder recorder(context, layout_image_, paint_info.phase, paint_offset);
        ...
        context.StrokeRect(outline_rect, PaintAutoDarkMode(layout_image_.StyleRef(), DarkModeFilter::ElementRole::kBorder));
        return;
    }

    DrawingRecorder recorder(context, layout_image_, paint_info.phase, ToEnclosingRect(visual_rect));
    PaintIntoRect(context, paint_rect, visual_rect);
}

void ImagePainter::PaintIntoRect(GraphicsContext& context, const PhysicalRect& dest_rect, const PhysicalRect& content_rect)
{
    ...
    scoped_refptr<Image> image = image_resource.GetImage(gfx::SizeF(dest_rect.size));
    ...
    auto respect_orientation = image->HasDefaultOrientation() ? kDoNotRespectImageOrientation : image_resource.ImageOrientation();
    ...
    if (!content_rect.Contains(dest_rect)) {
        ...
        if (pixel_snapped_content_rect.IsEmpty())
            return;
        src_rect = gfx::MapRect(gfx::RectF(pixel_snapped_content_rect), gfx::RectF(pixel_snapped_dest_rect), src_rect);
    ...
    if (respect_orientation == kRespectImageOrientation) {
        src_rect = image->CorrectSrcRectForImageOrientation(image->SizeAsFloat(respect_orientation), src_rect);
    }

    DEVTOOLS_TIMELINE_TRACE_EVENT_WITH_CATEGORIES(
        TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "PaintImage", inspector_paint_image_event::Data, layout_image_, src_rect, gfx::RectF(dest_rect));

    ScopedImageRenderingSettings image_rendering_settings_scope(
        context, layout_image_.StyleRef().GetInterpolationQuality(), layout_image_.StyleRef().GetDynamicRangeLimit());

    Node* node = layout_image_.GetNode();
    auto* image_element = DynamicTo<HTMLImageElement>(node);
    Image::ImageDecodingMode decode_mode = image_element ? image_element->GetDecodingModeForPainting(image->paint_image_id()) : Image::kUnspecifiedDecode;

    auto image_auto_dark_mode
        = ImageClassifierHelper::GetImageAutoDarkMode(*layout_image_.GetFrame(), layout_image_.StyleRef(), gfx::RectF(pixel_snapped_dest_rect), src_rect);
    ...
    if (image_content && (IsA<HTMLImageElement>(node) || IsA<HTMLVideoElement>(node)) && image_content->IsLoaded()) {
        LocalDOMWindow* window = layout_image_.GetDocument().domWindow();
        DCHECK(window);
        ImageElementTiming::From(*window).NotifyImagePainted(
            layout_image_, *image_content, context.GetPaintController().CurrentPaintChunkProperties(), pixel_snapped_dest_rect);
    }

    context.DrawImage(*image, decode_mode, image_auto_dark_mode,
        ComputeImagePaintTimingInfo(layout_image_, *image, image_content, context, pixel_snapped_dest_rect), gfx::RectF(pixel_snapped_dest_rect), &src_rect,
