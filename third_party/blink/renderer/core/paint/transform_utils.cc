...
PhysicalRect ComputeReferenceBoxInternal(const PhysicalBoxFragment& fragment, PhysicalRect border_box_rect)
{
    PhysicalRect fragment_reference_box = border_box_rect;
    switch (fragment.Style().UsedTransformBox(ComputedStyle::TransformBoxContext::kLayoutBox)) {
...
PhysicalRect ComputeReferenceBox(const LayoutBox& box)
{
    ...
    return ComputeReferenceBoxInternal(*box.GetPhysicalFragment(0), box.PhysicalBorderBoxRect());
}
