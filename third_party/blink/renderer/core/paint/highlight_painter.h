...
public:
    ...
    public:
        ...
        explicit SelectionPaintState(const InlineCursor& containing_block, const PhysicalOffset& box_offset,
            const std::optional<AffineTransform> writing_mode_rotation, const FrameSelection&);

        const LayoutSelectionStatus& Status() const
        {
            return selection_status_;
        }

        const TextPaintStyle& GetSelectionStyle() const
        {
            return selection_style_.style;
        }
        ...
        void PaintSelectedText(
            TextPainter& text_painter, const TextFragmentPaintInfo&, const TextPaintStyle& text_style, DOMNodeId node_id, const AutoDarkMode& auto_dark_mode);
    ...
    HighlightPainter(const TextFragmentPaintInfo& fragment_paint_info, TextPainter& text_painter, TextDecorationPainter& decoration_painter,
        const PaintInfo& paint_info, const InlineCursor& cursor, const FragmentItem& fragment_item, const PhysicalOffset& box_origin,
        const ComputedStyle& style, const TextPaintStyle& text_style, SelectionPaintState*);
    ...
    static TextOffsetRange GetFragmentDOMOffsets(const Text& text, unsigned from, unsigned to);
    ...
    void PaintHighlightOverlays(const TextPaintStyle&, DOMNodeId, bool paint_marker_backgrounds, std::optional<AffineTransform> rotation);
...
private:
    ...
    const PhysicalRect ComputeBackgroundRect(StringView text, unsigned start_offset, unsigned end_offset);
    const PhysicalRect ComputeBackgroundRectForSelection(unsigned start_offset, unsigned end_offset);
    Vector<LayoutSelectionStatus> GetHighlights(const HighlightLayer& layer);
    void FastPaintSpellingGrammarDecorations(const Text& text_node, const StringView& text, const DocumentMarkerVector& markers);
    void PaintOneSpellingGrammarDecoration(DocumentMarker::MarkerType, const StringView& text, unsigned paint_start_offset, unsigned paint_end_offset);
    void PaintOneSpellingGrammarDecoration(DocumentMarker::MarkerType, const StringView& text, unsigned paint_start_offset, unsigned paint_end_offset,
        const ComputedStyle& style, const TextPaintStyle& text_style, const AppliedTextDecoration* decoration_override);
    LineRelativeRect LineRelativeWorldRect(const HighlightOverlay::HighlightRange&);
    LineRelativeRect LocalRectInWritingModeSpace(unsigned from, unsigned to) const;
    void ClipToPartRect(const LineRelativeRect& part_rect);
    void PaintDecorationsExceptLineThrough(const HighlightOverlay::HighlightPart&, const LineRelativeRect&);
    void PaintDecorationsExceptLineThrough(const HighlightOverlay::HighlightPart&, const LineRelativeRect&, TextDecorationLine lines_to_paint);
    void PaintDecorationsOnlyLineThrough(const HighlightOverlay::HighlightPart&, const LineRelativeRect&);

    // Paints originating text and decorations (again) with the given color.
    // Used for composition markers only.
    void PaintTextForCompositionMarker(const StringView& text, const Color& text_color, unsigned paint_start_offset, unsigned paint_end_offset);
