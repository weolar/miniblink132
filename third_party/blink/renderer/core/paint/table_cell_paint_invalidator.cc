...
void InvalidateContainerForCellGeometryChange(const LayoutObject& container, const PaintInvalidatorContext& container_context)
{
...
void TableCellPaintInvalidator::InvalidatePaint()
{
    ...
    if (context_.old_paint_offset != context_.fragment_data->PaintOffset() || cell_.Size() != cell_.PreviousSize()) {
        // Table row background is painted inside cell's geometry.
        const auto& row = *cell_.Parent();
        DCHECK(row.IsTableRow());
        if (!DisplayItemClientIsFullyInvalidated(row) && row.StyleRef().HasBackground()) {
        ...
        if (!DisplayItemClientIsFullyInvalidated(section) && section.StyleRef().HasBackground()) {
            InvalidateContainerForCellGeometryChange(section, *context_.ParentContext()->ParentContext());
        }
        // Table paints its background, and column backgrounds inside cell's
        // geometry.
        const auto& table = *cell_.Table();
        if (!DisplayItemClientIsFullyInvalidated(table) && (table.HasBackgroundForPaint() || table.HasCollapsedBorders())) {
            InvalidateContainerForCellGeometryChange(table, *context_.ParentContext()->ParentContext()->ParentContext());
        }
    }
