...
public:
    ...
    struct PrePaintTreeWalkContext : public PrePaintTreeWalkContextBase {
        ...
        PrePaintTreeWalkContext(const PrePaintTreeWalkContext& parent_context, bool needs_tree_builder_context)
...
#if DCHECK_IS_ON()
                DCHECK(!needs_tree_builder_context || parent_context.tree_builder_context->is_actually_needed);
    ...
    static bool ContextRequiresChildTreeBuilderContext(const PrePaintTreeWalkContext&);

#if DCHECK_IS_ON()
    void CheckTreeBuilderContextState(const LayoutObject&, const PrePaintTreeWalkContext&);
#endif
    ...
    // at all).
    PrePaintInfo CreatePrePaintInfo(const PhysicalFragmentLink& child, const PrePaintTreeWalkContext& context);

    // Locate and/or set up a FragmentData object for the current object /
    // physical fragment.
    FragmentData* GetOrCreateFragmentData(const LayoutObject&, const PrePaintTreeWalkContext&, const PrePaintInfo&);

    void UpdateContextForOOFContainer(const LayoutObject&, PrePaintTreeWalkContext&, const PhysicalBoxFragment*);
    ...
    NOINLINE void WalkInternal(const LayoutObject&, PrePaintTreeWalkContext&, PrePaintInfo*);
    ...
    // otherwise.
    bool CollectMissableChildren(PrePaintTreeWalkContext&, const PhysicalBoxFragment&);
    ...
    const PhysicalBoxFragment* RebuildContextForMissedDescendant(
        const PhysicalBoxFragment& ancestor, const LayoutObject& object, bool update_tree_builder_context, PrePaintTreeWalkContext&);
    ...
    void WalkMissedChildren(const PhysicalBoxFragment&, bool is_in_fragment_traversal, const PrePaintTreeWalkContext&);

    void WalkFragmentationContextRootChildren(const LayoutObject&, const PhysicalBoxFragment&, const PrePaintTreeWalkContext&);
    void WalkPageContainer(const PhysicalFragmentLink& page_container_link, const LayoutObject& parent_object, const PrePaintTreeWalkContext& parent_context,
        wtf_size_t fragmentainer_idx);
    void WalkFragmentainer(
    ...
    void WalkLayoutObjectChildren(const LayoutObject&, const PhysicalBoxFragment*, const PrePaintTreeWalkContext&);
    void WalkChildren(const LayoutObject&, const PhysicalBoxFragment*, PrePaintTreeWalkContext&, bool is_inside_fragment_child = false);
    void Walk(const LayoutObject&, const PrePaintTreeWalkContext& parent_context, PrePaintInfo*);

    bool NeedsTreeBuilderContextUpdate(const LocalFrameView&, const PrePaintTreeWalkContext&);
    bool NeedsTreeBuilderContextUpdate(const LayoutObject&, const PrePaintTreeWalkContext&);
    ...
    void UpdateEffectiveAllowedTouchAction(const LayoutObject&, PrePaintTreeWalkContext&);
    ...
    // which will ensure the subtree is updated too.
    void UpdateBlockingWheelEventHandler(const LayoutObject&, PrePaintTreeWalkContext&);
    void InvalidatePaintForHitTesting(const LayoutObject&, PrePaintTreeWalkContext&);
...
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::PrePaintTreeWalk::PrePaintTreeWalkContext)
