...
bool HasSingleInvalidSVGMaskReferenceMaskLayer(const LayoutObject& object, const FillLayer& first_layer)
{
    if (first_layer.Next()) {
        return false;
    }
    const auto* mask_source = DynamicTo<StyleMaskSourceImage>(first_layer.GetImage());
...
std::optional<gfx::RectF> CSSMaskPainter::MaskBoundingBox(const LayoutObject& object, const PhysicalOffset& paint_offset)
{
    ...
    if (object.IsBox()) {
        if (maximum_mask_clip == EFillBox::kNoClip) {
            maximum_mask_region = To<LayoutBox>(object).Layer()->LocalBoundingBoxIncludingSelfPaintingDescendants();
        } else {
        ...
        if (maximum_mask_clip == EFillBox::kNoClip) {
            maximum_mask_region = To<LayoutInline>(object).Layer()->LocalBoundingBoxIncludingSelfPaintingDescendants();
        } else {
            // We could use a tighter rect for padding-box/content-box.
