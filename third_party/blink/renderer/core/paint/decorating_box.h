...
public:
    DecoratingBox(const PhysicalOffset& content_offset_in_container, const ComputedStyle& style, const Vector<AppliedTextDecoration, 1>* decorations)
        : content_offset_in_container_(content_offset_in_container)
        , style_(&style)
        , decorations_(decorations ? decorations : &style.AppliedTextDecorations())
    {
    }
    DecoratingBox(const FragmentItem& item, const ComputedStyle& style, const Vector<AppliedTextDecoration, 1>* decorations)
        : DecoratingBox(item.ContentOffsetInContainerFragment(), style, decorations)
    {
    }
