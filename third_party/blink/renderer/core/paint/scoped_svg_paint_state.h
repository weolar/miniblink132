...
public:
    ScopedSVGTransformState(const PaintInfo& paint_info, const LayoutObject& object)
        : content_paint_info_(paint_info)
    {
        ...
        if (const auto* transform_node = properties->Transform()) {
            transform_property_scope_.emplace(
                paint_info.context.GetPaintController(), *transform_node, object, DisplayItem::PaintPhaseToSVGTransformType(paint_info.phase));
            if (auto* context_paints = paint_info.GetSvgContextPaints()) {
                transformed_context_paints_.emplace(
                    context_paints->fill, context_paints->stroke, context_paints->transform * AffineTransform::FromTransform(transform_node->Matrix()));
                content_paint_info_.SetSvgContextPaints(base::OptionalToPtr(transformed_context_paints_));
            }
        }
    }

    PaintInfo& ContentPaintInfo()
    {
        return content_paint_info_;
    }
...
public:
    ...
    ScopedSVGPaintState(const LayoutObject& object, const PaintInfo& paint_info, const DisplayItemClient& display_item_client)
        : object_(object)
