...
public:
    ImageRecord(DOMNodeId new_node_id, const MediaTiming* new_media_timing, uint64_t new_recorded_size, const gfx::Rect& frame_visual_rect,
        const gfx::RectF& root_visual_rect, MediaRecordIdHash hash)
        : node_id(new_node_id)
        ...
        , recorded_size(new_recorded_size)
    {
        if (PaintTimingVisualizer::IsTracingEnabled()) {
            lcp_rect_info_ = std::make_unique<LCPRectInfo>(frame_visual_rect, gfx::ToRoundedRect(root_visual_rect));
        }
    }
...
class CORE_EXPORT ImageRecordsManager {
    ...
    FRIEND_TEST_ALL_PREFIXES(ImagePaintTimingDetectorTest, LargestImagePaint_Detached_Frame);
...
private:
    ...
    bool OnFirstAnimatedFramePainted(MediaRecordIdHash, unsigned current_frame_index);
    void OnImageLoaded(MediaRecordIdHash, unsigned current_frame_index, const StyleImage*);
    ...
    // larger size.
    void MaybeUpdateLargestIgnoredImage(
        const MediaRecordId&, const uint64_t& visual_size, const gfx::Rect& frame_visual_rect, const gfx::RectF& root_visual_rect);
    void ReportLargestIgnoredImage(unsigned current_frame_index);

    void AssignPaintTimeToRegisteredQueuedRecords(const base::TimeTicks&, unsigned last_queued_frame_index);

    void AddPendingImage(ImageRecord* record);
    void ClearImagesQueuedForPaintTime();
    void Clear();

    ImageRecord* CreateImageRecord(const LayoutObject& object, const MediaTiming* media_timing, const uint64_t& visual_size, const gfx::Rect& frame_visual_rect,
        const gfx::RectF& root_visual_rect, MediaRecordIdHash hash);
    inline void QueueToMeasurePaintTime(ImageRecord* record, unsigned current_frame_index)
    {
        CHECK(record);
...
class CORE_EXPORT ImagePaintTimingDetector final : public GarbageCollected<ImagePaintTimingDetector> {
public:
    ...
    bool RecordImage(const LayoutObject&, const gfx::Size& intrinsic_size, const MediaTiming&, const PropertyTreeStateOrAlias& current_paint_properties,
        const StyleImage*, const gfx::Rect& image_border);
    ...
    void ReportPresentationTime(unsigned last_queued_frame_index, base::TimeTicks);
...
private:
    ...
    uint64_t ComputeImageRectSize(const gfx::Rect& image_border, const gfx::RectF& mapped_visual_rect, const gfx::Size&, const PropertyTreeStateOrAlias&,
