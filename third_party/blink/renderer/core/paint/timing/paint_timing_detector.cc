...
bool IsBackgroundImageContentful(const LayoutObject& object, const Image& image)
{
...
void ReportImagePixelInaccuracy(HTMLImageElement* image_element)
{
    ...
    gfx::Size intrinsic_dimensions = image_content->IntrinsicSize(kRespectImageOrientation);
    ...
    uint64_t needed_pixels = base::saturated_cast<uint64_t>((layout_width * document_dpr) * (layout_height * document_dpr));
    uint64_t capped_pixels = base::saturated_cast<uint64_t>((layout_width * capped_dpr) * (layout_height * capped_dpr));

    bool has_overfetched_pixels = fetched_pixels > needed_pixels;
    base::UmaHistogramBoolean("Renderer.Images.HasOverfetchedPixels", has_overfetched_pixels);
    if (has_overfetched_pixels) {
        uint64_t overfetched_pixels = fetched_pixels - needed_pixels;
        base::UmaHistogramCounts10M("Renderer.Images.OverfetchedPixels", base::saturated_cast<int>(overfetched_pixels));
    }

    bool has_overfetched_capped_pixels = fetched_pixels > capped_pixels;
    base::UmaHistogramBoolean("Renderer.Images.HasOverfetchedCappedPixels", has_overfetched_capped_pixels);
    if (has_overfetched_capped_pixels) {
        uint64_t overfetched_capped_pixels = fetched_pixels - capped_pixels;
        base::UmaHistogramCounts10M("Renderer.Images.OverfetchedCappedPixels", base::saturated_cast<int>(overfetched_capped_pixels));
    }

    // Report offset in pixels between layout width and sizes result
    if (sizes_width) {
        int sizes_miss = base::saturated_cast<int>(sizes_width.value() - layout_width);

        base::UmaHistogramBoolean("Renderer.Images.HasSizesAttributeMiss", sizes_miss > 0);
        if (sizes_miss > 0) {
            base::UmaHistogramCounts10000("Renderer.Images.SizesAttributeMiss", sizes_miss);
        }
    }
}
...
PaintTimingDetector::PaintTimingDetector(LocalFrameView* frame_view)
    ...
    , text_paint_timing_detector_(MakeGarbageCollected<TextPaintTimingDetector>(frame_view, this, nullptr /*set later*/))
    , image_paint_timing_detector_(MakeGarbageCollected<ImagePaintTimingDetector>(frame_view, nullptr /*set later*/))
    , callback_manager_(MakeGarbageCollected<PaintTimingCallbackManagerImpl>(frame_view))
{
...
// static
bool PaintTimingDetector::NotifyBackgroundImagePaint(const Node& node, const Image& image, const StyleImage& style_image,
    const PropertyTreeStateOrAlias& current_paint_chunk_properties, const gfx::Rect& image_border)
{
    ...
    PaintTimingDetector& paint_timing_detector = frame_view->GetPaintTimingDetector();
    ...
    return image_paint_timing_detector.RecordImage(*object, image.Size(), *cached_image, current_paint_chunk_properties, &style_image, image_border);
}

// static
bool PaintTimingDetector::NotifyImagePaint(const LayoutObject& object, const gfx::Size& intrinsic_size, const MediaTiming& media_timing,
    const PropertyTreeStateOrAlias& current_paint_chunk_properties, const gfx::Rect& image_border)
{
    ...
    if (image_node && paint_timing_detector.IsUnrelatedSoftNavigationPaint(*image_node)) {
        return false;
    }
    ...
    return image_paint_timing_detector.RecordImage(object, intrinsic_size, media_timing, current_paint_chunk_properties, nullptr, image_border);
}

void PaintTimingDetector::NotifyImageFinished(const LayoutObject& object, const MediaTiming* media_timing)
{
    if (IgnorePaintTimingScope::ShouldIgnore() || !image_paint_timing_detector_->IsRecordingLargestImagePaint()) {
        return;
    }
    image_paint_timing_detector_->NotifyImageFinished(object, media_timing);
}

void PaintTimingDetector::LayoutObjectWillBeDestroyed(const LayoutObject& object)
{
    text_paint_timing_detector_->LayoutObjectWillBeDestroyed(object);
}

void PaintTimingDetector::NotifyImageRemoved(const LayoutObject& object, const ImageResourceContent* cached_image)
{
...
void PaintTimingDetector::NotifyInputEvent(WebInputEvent::Type type)
{
    ...
    if (type == WebInputEvent::Type::kMouseMove || type == WebInputEvent::Type::kMouseEnter || type == WebInputEvent::Type::kMouseLeave
        || type == WebInputEvent::Type::kKeyUp || WebInputEvent::IsPinchGestureEventType(type)) {
        return;
    }
    OnInputOrScroll();
}
...
bool PaintTimingDetector::NeedToNotifyInputOrScroll() const
{
    ...
    return text_paint_timing_detector_->IsRecordingLargestTextPaint() || image_paint_timing_detector_;
}
...
void PaintTimingDetector::SoftNavigationDetected(LocalDOMWindow* window)
{
    ...
    // will be called.
    if (!lcp_calculator || (!potential_soft_navigation_text_record_ && !potential_soft_navigation_image_record_)) {
        return;
    }
    if (!lcp_was_restarted_ || RuntimeEnabledFeatures::SoftNavigationHeuristicsEnabled(window)) {
        lcp_calculator->UpdateWebExposedLargestContentfulPaintIfNeeded(potential_soft_navigation_text_record_, potential_soft_navigation_image_record_,
    ...
    soft_navigation_lcp_details_for_metrics_ = largest_contentful_paint_calculator_->LatestLcpDetails();
    DidChangePerformanceTiming();
}
...
LargestContentfulPaintCalculator* PaintTimingDetector::GetLargestContentfulPaintCalculator()
{
    ...
    largest_contentful_paint_calculator_ = MakeGarbageCollected<LargestContentfulPaintCalculator>(DOMWindowPerformance::performance(*dom_window));
    return largest_contentful_paint_calculator_.Get();
}
...
void PaintTimingDetector::UpdateMetricsLcp()
{
    ...
    if (record_lcp_to_metrics_) {
        auto latest_lcp_details = GetLargestContentfulPaintCalculator()->LatestLcpDetails();
        lcp_details_for_metrics_ = latest_lcp_details;
    }

    // If we're waiting on a softnav and it wasn't detected yet, keep on waiting
    // and don't update.
    if (record_soft_navigation_lcp_for_metrics_ && soft_navigation_was_detected_) {
        auto latest_lcp_details = GetLargestContentfulPaintCalculator()->LatestLcpDetails();
...
gfx::RectF PaintTimingDetector::CalculateVisualRect(const gfx::Rect& visual_rect, const PropertyTreeStateOrAlias& current_paint_chunk_properties) const
{
    ...
    GeometryMapper::LocalToAncestorVisualRect(
        current_paint_chunk_properties, local_root.ContentLayoutObject()->FirstFragment().LocalBorderBoxProperties(), float_clip_visual_rect);
    ...
    auto layout_visual_rect = PhysicalRect::EnclosingRect(float_clip_visual_rect.Rect());
    frame_view_->GetFrame().LocalFrameRoot().View()->MapToVisualRectInRemoteRootFrame(layout_visual_rect);
...
void PaintTimingDetector::UpdateLcpCandidate()
{
    ...
    if (image_paint_timing_detector_->IsRecordingLargestImagePaint()) {
        image_update_result = image_paint_timing_detector_->UpdateMetricsCandidate();
    }
    ...
    if (record_lcp_to_metrics_ || lcp_was_restarted_) {
        lcp_calculator->UpdateWebExposedLargestContentfulPaintIfNeeded(text_update_result.first, image_update_result.first,
            /*is_triggered_by_soft_navigation=*/lcp_was_restarted_);
    }
}
...
const LargestContentfulPaintDetails& PaintTimingDetector::LatestLcpDetailsForTest() const
{
...
{
    return (WasLCPRestarted() && !(IsSoftNavigationDetected() || node.IsModifiedBySoftNavigation()));
}

ScopedPaintTimingDetectorBlockPaintHook* ScopedPaintTimingDetectorBlockPaintHook::top_ = nullptr;

void ScopedPaintTimingDetectorBlockPaintHook::EmplaceIfNeeded(const LayoutBoxModelObject& aggregator, const PropertyTreeStateOrAlias& property_tree_state)
{
    ...
    TextPaintTimingDetector& detector = aggregator.GetFrameView()->GetPaintTimingDetector().GetTextPaintTimingDetector();
...
ScopedPaintTimingDetectorBlockPaintHook::Data::Data(
    const LayoutBoxModelObject& aggregator, const PropertyTreeStateOrAlias& property_tree_state, TextPaintTimingDetector* detector)
...
ScopedPaintTimingDetectorBlockPaintHook::~ScopedPaintTimingDetectorBlockPaintHook()
{
    ...
    if (!data_->detector_ || !data_->detector_->ShouldWalkObject(data_->aggregator_)) {
        return;
    }
    data_->detector_->RecordAggregatedText(data_->aggregator_, data_->aggregated_visual_rect_, data_->property_tree_state_);
}
