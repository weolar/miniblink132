...
class CORE_EXPORT TextElementTiming final : public GarbageCollected<TextElementTiming>, public Supplement<LocalDOMWindow> {
public:
    ...
    static inline bool NeededForElementTiming(Node& node)
    {
        ...
        return !node.IsInShadowTree() && element && element->FastHasAttribute(html_names::kElementtimingAttr);
    }

    static gfx::RectF ComputeIntersectionRect(
        const LayoutObject&, const gfx::Rect& aggregated_visual_rect, const PropertyTreeStateOrAlias&, const LocalFrameView*);
