...
FirstMeaningfulPaintDetector::FirstMeaningfulPaintDetector(PaintTiming* paint_timing)
...
void FirstMeaningfulPaintDetector::MarkNextPaintAsMeaningfulIfNeeded(
    const LayoutObjectCounter& counter, double contents_height_before_layout, double contents_height_after_layout, int visible_height)
{
    ...
    double ratio_before = std::max(1.0, contents_height_before_layout / visible_height);
    double ratio_after = std::max(1.0, contents_height_after_layout / visible_height);
    ...
    size_t approximate_blank_character_count = FontFaceSetDocument::ApproximateBlankCharacterCount(*GetDocument());
...
{
    if (!GetDocument() || network_quiet_reached_ || paint_timing_->FirstContentfulPaintRenderedButNotPresentedAsMonotonicTime().is_null())
        return;
    ...
    if (!provisional_first_meaningful_paint_.is_null()) {
        ...
        if (provisional_first_meaningful_paint_ < paint_timing_->FirstContentfulPaintRenderedButNotPresentedAsMonotonicTime()) {
            first_meaningful_paint_ = paint_timing_->FirstContentfulPaintRenderedButNotPresentedAsMonotonicTime();
            first_meaningful_paint_presentation = paint_timing_->FirstContentfulPaintIgnoringSoftNavigations();
            ...
            first_meaningful_paint_presentation = provisional_first_meaningful_paint_presentation_;
...
void FirstMeaningfulPaintDetector::RegisterNotifyPresentationTime(PaintEvent event)
{
    ++outstanding_presentation_promise_count_;
    paint_timing_->RegisterNotifyPresentationTime(
        CrossThreadBindOnce(&FirstMeaningfulPaintDetector::ReportPresentationTime, WrapCrossThreadWeakPersistent(this), event));
}

void FirstMeaningfulPaintDetector::ReportPresentationTime(PaintEvent event, const viz::FrameTimingDetails& presentation_details)
{
    ...
    probe::PaintTiming(GetDocument(), "firstMeaningfulPaintCandidate", timestamp.since_origin().InSecondsF());
    ...
    if (!seen_first_meaningful_paint_candidate_) {
        ...
        paint_timing_->SetFirstMeaningfulPaintCandidate(provisional_first_meaningful_paint_presentation_);
    }

    if (defer_first_meaningful_paint_ == kDeferOutstandingPresentationPromises && outstanding_presentation_promise_count_ == 0) {
        DCHECK(!first_meaningful_paint_.is_null());
        SetFirstMeaningfulPaint(provisional_first_meaningful_paint_presentation_);
    }
}

void FirstMeaningfulPaintDetector::NotifyFirstContentfulPaint(base::TimeTicks presentation_time)
{
...
void FirstMeaningfulPaintDetector::SetFirstMeaningfulPaint(base::TimeTicks presentation_time)
{
    DCHECK(paint_timing_->FirstMeaningfulPaint().is_null());
    DCHECK(!presentation_time.is_null());
    DCHECK(network_quiet_reached_);

    double presentation_time_seconds = presentation_time.since_origin().InSecondsF();
    probe::PaintTiming(GetDocument(), "firstMeaningfulPaint", presentation_time_seconds);
    ...
    paint_timing_->SetFirstMeaningfulPaint(presentation_time, had_user_input_before_provisional_first_meaningful_paint_);
}

// static
void FirstMeaningfulPaintDetector::SetTickClockForTesting(const base::TickClock* clock)
{
    g_clock = clock;
}
