...
class CORE_EXPORT PaintTimingDetector : public GarbageCollected<PaintTimingDetector> {
...
public:
    ...
    static bool NotifyBackgroundImagePaint(
        const Node&, const Image&, const StyleImage&, const PropertyTreeStateOrAlias& current_paint_chunk_properties, const gfx::Rect& image_border);
    ...
    bool NotifyMetricsIfLargestImagePaintChanged(
        base::TimeTicks image_paint_time, uint64_t image_size, ImageRecord* image_record, double image_bpp, std::optional<WebURLRequest::Priority> priority);
    ...
    gfx::RectF CalculateVisualRect(const gfx::Rect& visual_rect, const PropertyTreeStateOrAlias&) const;
    ...
    bool WasLCPRestarted() const
    {
        return lcp_was_restarted_;
    }
    ...
    const LargestContentfulPaintDetails& LargestContentfulPaintDetailsForMetrics() const
    {
        return lcp_details_for_metrics_;
    }

    const LargestContentfulPaintDetails& SoftNavigationLargestContentfulPaintDetailsForMetrics() const
    {
    ...
    std::optional<PaintTimingVisualizer>& Visualizer()
    {
        return visualizer_;
    }
...
private:
    ...
    bool HasLargestTextPaintChangedForMetrics(base::TimeTicks, uint64_t size) const;
...
public:
    ...
    ScopedPaintTimingDetectorBlockPaintHook(const ScopedPaintTimingDetectorBlockPaintHook&) = delete;
    ScopedPaintTimingDetectorBlockPaintHook& operator=(const ScopedPaintTimingDetectorBlockPaintHook&) = delete;

    void EmplaceIfNeeded(const LayoutBoxModelObject&, const PropertyTreeStateOrAlias&);
    ~ScopedPaintTimingDetectorBlockPaintHook();
...
private:
    ...
    std::optional<base::AutoReset<ScopedPaintTimingDetectorBlockPaintHook*>> reset_top_;
    struct Data {
        STACK_ALLOCATED();

    public:
        Data(const LayoutBoxModelObject& aggregator, const PropertyTreeStateOrAlias&, TextPaintTimingDetector*);
...
// static
inline void PaintTimingDetector::NotifyTextPaint(const gfx::Rect& text_visual_rect)
{
