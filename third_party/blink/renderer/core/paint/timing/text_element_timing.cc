...
{
    TextElementTiming* timing = Supplement<LocalDOMWindow>::From<TextElementTiming>(window);
    if (!timing) {
...
gfx::RectF TextElementTiming::ComputeIntersectionRect(
    const LayoutObject& object, const gfx::Rect& aggregated_visual_rect, const PropertyTreeStateOrAlias& property_tree_state, const LocalFrameView* frame_view)
{
    ...
    return ElementTimingUtils::ComputeIntersectionRect(&frame_view->GetFrame(), aggregated_visual_rect, property_tree_state);
}

bool TextElementTiming::CanReportElements() const
{
    DCHECK(performance_);
    return performance_->HasObserverFor(PerformanceEntry::kElement) || !performance_->IsElementTimingBufferFull();
}
...
void TextElementTiming::OnTextObjectPainted(const TextRecord& record)
{
    ...
    performance_->AddElementTiming(kTextPaint, g_empty_string, record.element_timing_rect_, record.paint_time, base::TimeTicks(),
        element->FastGetAttribute(html_names::kElementtimingAttr), gfx::Size(), id, element);
}
