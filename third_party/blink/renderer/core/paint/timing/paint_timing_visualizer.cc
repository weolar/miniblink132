...
void PaintTimingVisualizer::RecordRects(const gfx::Rect& rect, std::unique_ptr<TracedValue>& value)
{
    CreateQuad(value.get(), "rect", gfx::QuadF(gfx::RectF(rect)));
}
void PaintTimingVisualizer::RecordObject(const LayoutObject& object, std::unique_ptr<TracedValue>& value)
{
    ...
    value->SetBoolean("is_in_outermost_main_frame", object.GetFrame()->IsOutermostMainFrame());
    if (object.GetNode())
        value->SetInteger("dom_node_id", object.GetNode()->GetDomNodeId());
}

void PaintTimingVisualizer::DumpTextDebuggingRect(const LayoutObject& object, const gfx::RectF& rect)
{
...
void PaintTimingVisualizer::DumpImageDebuggingRect(const LayoutObject& object, const gfx::RectF& rect, bool is_loaded, const KURL& url)
{
    std::unique_ptr<TracedValue> value = std::make_unique<TracedValue>();
    RecordObject(object, value);
    RecordRects(gfx::ToRoundedRect(rect), value);
...
{
    TRACE_EVENT_INSTANT1("loading", "PaintTimingVisualizer::LayoutObjectPainted", TRACE_EVENT_SCOPE_THREAD, "data", std::move(value));
}

void PaintTimingVisualizer::RecordMainFrameViewport(LocalFrameView& frame_view)
{
    ...
    gfx::RectF float_visual_rect = frame_view.GetPaintTimingDetector().BlinkSpaceToDIPs(float_clip_visual_rect.Rect());

    std::unique_ptr<TracedValue> value = std::make_unique<TracedValue>();
    CreateQuad(value.get(), "viewport_rect", gfx::QuadF(float_visual_rect));
    value->SetDouble("dpr", frame_view.GetFrame().DevicePixelRatio());
    TRACE_EVENT_INSTANT1("loading", "PaintTimingVisualizer::Viewport", TRACE_EVENT_SCOPE_THREAD, "data", std::move(value));
