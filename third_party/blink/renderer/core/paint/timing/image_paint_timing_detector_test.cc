...
class ImagePaintTimingDetectorTest : public testing::Test, public PaintTestConfigurations {
public:
    ...
    void SetUp() override
    {
        ...
        web_view_helper_.GetWebView()->GetPage()->GetSettings().SetAcceleratedCompositingEnabled(true);
...
protected:
    LocalFrameView& GetFrameView()
    {
        return *GetFrame()->View();
    }
    LocalFrameView& GetChildFrameView()
    {
        return *GetChildFrame()->View();
    }
    ...
    {
        return GetChildFrame()->GetDocument();
    }
    ...
    {
        PerformanceTimingForReporting* performance_for_reporting = DOMWindowPerformance::performance(*GetFrame()->DomWindow())->timingForReporting();
    ...
    {
        return GetPaintTimingDetector().GetImagePaintTimingDetector().records_manager_.LargestImage();
    }

    ImageRecord* LargestPaintedImage()
    {
        return GetPaintTimingDetector().GetImagePaintTimingDetector().records_manager_.largest_painted_image_.Get();
    }

    ImageRecord* ChildFrameLargestImage()
    {
        return GetChildFrameView().GetPaintTimingDetector().GetImagePaintTimingDetector().records_manager_.LargestImage();
    }

    size_t CountImageRecords()
    {
        return GetPaintTimingDetector().GetImagePaintTimingDetector().records_manager_.recorded_images_.size();
    }

    size_t ContainerTotalSize()
    {
        size_t result = GetPaintTimingDetector().GetImagePaintTimingDetector().records_manager_.recorded_images_.size()
            + GetPaintTimingDetector().GetImagePaintTimingDetector().records_manager_.pending_images_.size()
            + GetPaintTimingDetector().GetImagePaintTimingDetector().records_manager_.images_queued_for_paint_time_.size()
            + GetPaintTimingDetector().GetImagePaintTimingDetector().records_manager_.image_finished_times_.size();
    ...
    {
        return GetChildPaintTimingDetector().GetImagePaintTimingDetector().records_manager_.recorded_images_.size();
    }
    ...
    {
        return GetPaintTimingDetector().LatestLcpDetailsForTest().largest_image_paint_time;
    }

    uint64_t LargestPaintSize()
    {
        return GetPaintTimingDetector().LatestLcpDetailsForTest().largest_image_paint_size;
    }
    ...
    {
        frame_test_helpers::LoadHTMLString(web_view_helper_.GetWebView()->MainFrameImpl(), content, KURL("http://test.com"));
        mock_callback_manager_ = MakeGarbageCollected<MockPaintTimingCallbackManager>();
        GetPaintTimingDetector().GetImagePaintTimingDetector().ResetCallbackManager(mock_callback_manager_);
        UpdateAllLifecyclePhases();
    }
    ...
    void SetChildBodyInnerHTML(const String& content)
    {
        ...
        child_mock_callback_manager_ = MakeGarbageCollected<MockPaintTimingCallbackManager>();
        GetChildPaintTimingDetector().GetImagePaintTimingDetector().ResetCallbackManager(child_mock_callback_manager_);
        UpdateAllLifecyclePhases();
    }
    ...
    void InvokePresentationTimeCallback(MockPaintTimingCallbackManager* image_callback_manager)
    {
        image_callback_manager->InvokePresentationTimeCallback(test_task_runner_->NowTicks());
        UpdateCandidate();
    }
...
private:
    ...
    ImageResourceContent* CreateImageForTest(int width, int height)
    {
        ...
        SkImageInfo raster_image_info = SkImageInfo::MakeN32Premul(width, height, src_rgb_color_space);
        sk_sp<SkSurface> surface(SkSurfaces::Raster(raster_image_info));
        sk_sp<SkImage> image = surface->makeImageSnapshot();
        scoped_refptr<UnacceleratedStaticBitmapImage> original_image_data = UnacceleratedStaticBitmapImage::Create(image);
        ...
        scoped_refptr<SharedBuffer> shared_buffer = SharedBuffer::Create(Vector<char>(bytes));
        original_image_data->SetData(shared_buffer, /*all_data_received=*/true);
        ImageResourceContent* original_image_content = ImageResourceContent::CreateLoaded(original_image_data.get());
...
TEST_P(ImagePaintTimingDetectorTest, LargestImagePaint_OneImage)
{
    ...
    test_ukm_recorder.ExpectEntryMetric(entry, UkmPaintTiming::kLCPDebugging_HasViewportImageName, false);
}
...
TEST_P(ImagePaintTimingDetectorTest, LargestImagePaint_TraceEvent_Candidate)
{
    ...
    std::optional<bool> is_outermost_main_frame = arg_dict.FindBool("isOutermostMainFrame");
...
TEST_P(ImagePaintTimingDetectorTest, LargestImagePaint_TraceEvent_Candidate_Frame)
{
    using trace_analyzer::Query;
    trace_analyzer::Start("loading");
    {
...
{
    LargestContentfulPaintDetailsForReporting largest_contentful_paint_details = GetPerformanceTimingForReporting().LargestContentfulPaintDetailsForMetrics();
    ...
    SetImageAndPaint("target", 5, 5);
    UpdateAllLifecyclePhasesAndInvokeCallbackIfAny();
    largest_contentful_paint_details = GetPerformanceTimingForReporting().LargestContentfulPaintDetailsForMetrics();
...
TEST_P(ImagePaintTimingDetectorTest, UpdatePerformanceTimingToZero)
{
    ...
    SetImageAndPaint("target", 5, 5);
    UpdateAllLifecyclePhasesAndInvokeCallbackIfAny();
    auto largest_contentful_paint_details = GetPerformanceTimingForReporting().LargestContentfulPaintDetailsForMetrics();
    EXPECT_EQ(largest_contentful_paint_details.image_paint_size, 25u);
    EXPECT_GT(largest_contentful_paint_details.image_paint_time, 0u);
    GetDocument().body()->RemoveChild(GetDocument().getElementById(AtomicString("target")));
    UpdateAllLifecyclePhasesAndInvokeCallbackIfAny();
    EXPECT_EQ(largest_contentful_paint_details.image_paint_size, 25u);
    EXPECT_GT(largest_contentful_paint_details.image_paint_time, 0u);
}
...
TEST_P(ImagePaintTimingDetectorTest, IgnoreImageUntilInvalidatedRectSizeNonZero)
{
    SetBodyInnerHTML(R"HTML(
    <img id="target"></img>
  )HTML");
...
TEST_P(ImagePaintTimingDetectorTest, LargestImagePaint_IgnoreThoseOutsideViewport)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      img {
...
TEST_P(ImagePaintTimingDetectorTest, LargestImagePaint_UpdateOnRemovingTheLastImage)
{
    SetBodyInnerHTML(R"HTML(
    <div id="parent">
      <img id="target"></img>
    ...
    GetDocument().getElementById(AtomicString("parent"))->RemoveChild(GetDocument().getElementById(AtomicString("target")));
    UpdateAllLifecyclePhasesAndInvokeCallbackIfAny();
    record = LargestImage();
    EXPECT_TRUE(record);
    EXPECT_NE(LargestPaintTime(), base::TimeTicks());
...
TEST_P(ImagePaintTimingDetectorTest, LargestImagePaint_UpdateOnRemoving)
{
    ...
    GetDocument().getElementById(AtomicString("parent"))->RemoveChild(GetDocument().getElementById(AtomicString("target2")));
    UpdateAllLifecyclePhasesAndInvokeCallbackIfAny();
...
TEST_P(ImagePaintTimingDetectorTest, LargestImagePaint_NodeRemovedBetweenRegistrationAndInvocation)
{
    SetBodyInnerHTML(R"HTML(
    <div id="parent">
      <img id="target"></img>
...
TEST_P(ImagePaintTimingDetectorTest, RemoveRecordFromAllContainersAfterImageRemoval)
{
    SetBodyInnerHTML(R"HTML(
    <div id="parent">
      <img id="target"></img>
...
TEST_P(ImagePaintTimingDetectorTest, RemoveRecordFromAllContainersAfterInvisibleImageRemoved)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      #target {
    ...
    GetDocument().body()->RemoveChild(GetDocument().getElementById(AtomicString("parent")));
    EXPECT_EQ(ContainerTotalSize(), 0u);
}

TEST_P(ImagePaintTimingDetectorTest, RemoveRecordFromAllContainersAfterBackgroundImageRemoval)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      #target {
...
TEST_P(ImagePaintTimingDetectorTest, RemoveRecordFromAllContainersAfterImageRemovedAndCallbackInvoked)
{
    SetBodyInnerHTML(R"HTML(
    <div id="parent">
      <img id="target"></img>
...
TEST_P(ImagePaintTimingDetectorTest, LargestImagePaint_ReattachedNodeNotTreatedAsNew)
{
    SetBodyInnerHTML(R"HTML(
    <div id="parent">
    </div>
    ...
    EXPECT_EQ(record->paint_time, base::TimeTicks() + base::Seconds(1) + kQuantumOfTime);
...
TEST_P(ImagePaintTimingDetectorTest, MatchPresentationTimeToNodesOfDifferentFrames)
{
    SetBodyInnerHTML(R"HTML(
    <div id="parent">
...
TEST_P(ImagePaintTimingDetectorTest, LargestImagePaint_UpdateResultWhenLargestChanged)
{
...
TEST_P(ImagePaintTimingDetectorTest, BackgroundImageAndLayoutImageTrackedDifferently)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      img {
...
{
    SetBodyInnerHTML("<style>body { background-image: url(" SIMPLE_IMAGE ")}</style>");
    EXPECT_EQ(CountImageRecords(), 0u);
}

TEST_P(ImagePaintTimingDetectorTest, BackgroundImage_IgnoreHtml)
{
    SetBodyInnerHTML("<style>html { background-image: url(" SIMPLE_IMAGE ")}</style>");
    EXPECT_EQ(CountImageRecords(), 0u);
}
...
TEST_P(ImagePaintTimingDetectorTest, DeactivateAfterUserInput)
{
    ...
    SetImageAndPaint("target", 5, 5);
    UpdateAllLifecyclePhasesAndInvokeCallbackIfAny();
    EXPECT_FALSE(GetPaintTimingDetector().GetImagePaintTimingDetector().IsRecordingLargestImagePaint());
}
...
TEST_P(ImagePaintTimingDetectorTest, ContinueAfterKeyUp)
{
    ...
    SetImageAndPaint("target", 5, 5);
    UpdateAllLifecyclePhasesAndInvokeCallbackIfAny();
    EXPECT_TRUE(GetPaintTimingDetector().GetImagePaintTimingDetector().IsRecordingLargestImagePaint());
}
...
TEST_P(ImagePaintTimingDetectorTest, UseIntrinsicSizeIfSmaller_BackgroundImage)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      #d {
...
TEST_P(ImagePaintTimingDetectorTest, NotUseIntrinsicSizeIfLarger_BackgroundImage)
{
...
TEST_P(ImagePaintTimingDetectorTest, OpacityZeroHTML)
{
    ...
    GetDocument().documentElement()->setAttribute(html_names::kStyleAttr, AtomicString("opacity: 1"));
    UpdateAllLifecyclePhasesAndInvokeCallbackIfAny();
    EXPECT_EQ(CountImageRecords(), 1u);
    auto largest_contentful_paint_details = GetPerformanceTimingForReporting().LargestContentfulPaintDetailsForMetrics();
    EXPECT_EQ(largest_contentful_paint_details.image_paint_size, 25u);
...
TEST_P(ImagePaintTimingDetectorTest, OpacityZeroHTML2)
{
    ...
    GetDocument().documentElement()->setAttribute(html_names::kStyleAttr, AtomicString("opacity: 0"));
    UpdateAllLifecyclePhasesAndInvokeCallbackIfAny();
    EXPECT_EQ(CountImageRecords(), 0u);
...
TEST_P(ImagePaintTimingDetectorTest, LargestImagePaint_FullViewportImage)
{
    ...
    EXPECT_EQ(1ul, entries.size());
    auto* entry = entries[0].get();
    test_ukm_recorder.ExpectEntryMetric(entry, UkmPaintTiming::kLCPDebugging_HasViewportImageName, true);
}
...
#define MAYBE_LargestImagePaint_Detached_Frame DISABLED_LargestImagePaint_Detached_Frame
#else
...
TEST_P(ImagePaintTimingDetectorTest, MAYBE_LargestImagePaint_Detached_Frame)
{
    ...
    PaintTimingDetector* child_detector = &child_frame->View()->GetPaintTimingDetector();
    ...
    presentation_details.presentation_feedback.timestamp = test_task_runner_->NowTicks();
    child_detector->callback_manager_->ReportPaintTime(std::make_unique<PaintTimingCallbackManager::CallbackQueue>(), presentation_details);
...
class ImagePaintTimingDetectorFencedFrameTest : private ScopedFencedFramesForTest, public ImagePaintTimingDetectorTest {
public:
    ImagePaintTimingDetectorFencedFrameTest()
        : ScopedFencedFramesForTest(true)
    {
        scoped_feature_list_.InitAndEnableFeatureWithParameters(features::kFencedFrames, { { "implementation_type", "mparch" } });
    }

    void InitializeFencedFrameRoot(blink::FencedFrame::DeprecatedFencedFrameMode mode)
    {
        web_view_helper_.InitializeWithOpener(/*opener=*/nullptr,
            /*frame_client=*/nullptr,
            /*view_client=*/nullptr,
            /*update_settings_func=*/nullptr, mode);
        // Enable compositing on the page before running the document lifecycle.
        web_view_helper_.GetWebView()->GetPage()->GetSettings().SetAcceleratedCompositingEnabled(true);
...
class ImagePaintTimingDetectorTransparentPlaceholderImageTest : public ImagePaintTimingDetectorTest {
public:
...
protected:
    ...
    {
        Element* element = GetDocument().getElementById(AtomicString(id));
        ImageResource* resource = ImageResource::CreateForTest(url_test_helpers::ToKURL(TRANSPARENT_PLACEHOLDER_IMAGE));
...
INSTANTIATE_PAINT_TEST_SUITE_P(ImagePaintTimingDetectorTransparentPlaceholderImageTest);

TEST_P(ImagePaintTimingDetectorTransparentPlaceholderImageTest, LargestImagePaint)
{
    LargestContentfulPaintDetailsForReporting largest_contentful_paint_details = GetPerformanceTimingForReporting().LargestContentfulPaintDetailsForMetrics();
    EXPECT_EQ(largest_contentful_paint_details.image_paint_size, 0u);
    EXPECT_EQ(largest_contentful_paint_details.image_paint_time, 0u);
