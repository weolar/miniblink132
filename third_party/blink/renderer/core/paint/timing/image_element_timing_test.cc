...
class ImageElementTimingTest : public testing::Test, public PaintTestConfigurations {
protected:
    void SetUp() override
    {
    ...
    {
        ImageResourceContent* content = CreateImageForTest(width, height);
        if (auto* layout_image = DynamicTo<LayoutSVGImage>(GetLayoutObjectById(id))) {
            layout_image->ImageResource()->SetImageResource(content);
            return layout_image;
        }
        return nullptr;
    ...
    {
        return ImageElementTiming::From(*GetDoc()->domWindow()).images_notified_.Contains(record_id_hash);
    }

    unsigned ImagesNotifiedSize()
    {
        return ImageElementTiming::From(*GetDoc()->domWindow()).images_notified_.size();
    }
    ...
    {
        web_view_helper_.GetWebView()->MainFrameImpl()->GetFrame()->View()->UpdateAllLifecyclePhasesForTest();
    }
...
{
    frame_test_helpers::LoadHTMLString(web_view_helper_.GetWebView()->MainFrameImpl(), R"HTML(
    ...
    LayoutObject* with_undefined_attribute = GetLayoutObjectById("unset-attribute");
    ...
    LayoutObject* with_explicit_element_timing = GetLayoutObjectById("valid-attribute");
    actual = internal::IsExplicitlyRegisteredForTiming(*with_explicit_element_timing);
...
TEST_P(ImageElementTimingTest, IgnoresUnmarkedElement)
{
    ...
    EXPECT_FALSE(ImagesNotifiedContains(MediaRecordId::GenerateHash(layout_image, layout_image->CachedImage())));
}
...
TEST_P(ImageElementTimingTest, ImageInsideSVG)
{
    ...
    EXPECT_TRUE(ImagesNotifiedContains(MediaRecordId::GenerateHash(layout_image, layout_image->CachedImage())));
}
...
TEST_P(ImageElementTimingTest, SVGImageRemoved)
{
    ...
    ASSERT_TRUE(layout_image);
    UpdateAllLifecyclePhases();
    EXPECT_TRUE(ImagesNotifiedContains(MediaRecordId::GenerateHash(layout_image, layout_image->ImageResource()->CachedImage())));
...
TEST_P(ImageElementTimingTest, BackgroundImageRemoved)
{
    ...
    ImageResourceContent* content = object->Style()->BackgroundLayers().GetImage()->CachedImage();
    UpdateAllLifecyclePhases();
    EXPECT_EQ(ImagesNotifiedSize(), 1u);
    EXPECT_TRUE(ImagesNotifiedContains(MediaRecordId::GenerateHash(object, content)));
