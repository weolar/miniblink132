...
public:
    void SetUp() override
    {
        ...
        mock_text_callback_manager_ = MakeGarbageCollected<MockPaintTimingCallbackManager>();
        GetTextPaintTimingDetector()->ResetCallbackManager(mock_text_callback_manager_);
        mock_image_callback_manager_ = MakeGarbageCollected<MockPaintTimingCallbackManager>();
        GetImagePaintTimingDetector()->ResetCallbackManager(mock_image_callback_manager_);
    ...
    {
        return &GetFrame().View()->GetPaintTimingDetector().GetImagePaintTimingDetector();
    }
    TextPaintTimingDetector* GetTextPaintTimingDetector()
    {
        return &GetFrame().View()->GetPaintTimingDetector().GetTextPaintTimingDetector();
    }

    void SetImage(const char* id, int width, int height, int bytes = 0)
    {
        To<HTMLImageElement>(GetElementById(id))->SetImageForTest(CreateImageForTest(width, height, bytes));
    }

    ImageResourceContent* CreateImageForTest(int width, int height, int bytes = 0)
    {
    ...
    {
        mock_text_callback_manager_->InvokePresentationTimeCallback(simulated_clock_.NowTicks());
        mock_image_callback_manager_->InvokePresentationTimeCallback(simulated_clock_.NowTicks());
...
private:
    ...
    {
        return GetFrame().View()->GetPaintTimingDetector().GetLargestContentfulPaintCalculator();
    }
...
TEST_F(LargestContentfulPaintCalculatorTest, SingleImageExcludedForEntropy)
{
    ...
    scoped_features.InitAndEnableFeatureWithParameters(blink::features::kExcludeLowEntropyImagesFromLCP, { { "min_bpp", "2.0" } });
    SetBodyInnerHTML(R"HTML(
    <!DOCTYPE html>
    <img id='target'/>
  )HTML");
...
TEST_F(LargestContentfulPaintCalculatorTest, LowEntropyImageNotExcludedAtLowerThreshold)
{
    base::test::ScopedFeatureList scoped_features;
    scoped_features.InitAndEnableFeatureWithParameters(blink::features::kExcludeLowEntropyImagesFromLCP, { { "min_bpp", "0.02" } });
    SetBodyInnerHTML(R"HTML(
    <!DOCTYPE html>
    <img id='small'/>
    <img id='large'/>
