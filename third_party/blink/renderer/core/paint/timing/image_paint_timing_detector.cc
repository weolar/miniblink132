...
// coordinate.
uint64_t DownScaleIfIntrinsicSizeIsSmaller(uint64_t visual_size, const uint64_t& intrinsic_image_size, const uint64_t& displayed_image_size)
{
    ...
    if (intrinsic_image_size < displayed_image_size) {
        ...
        return static_cast<double>(visual_size) * intrinsic_image_size / displayed_image_size;
    }
    return visual_size;
}

void RecordPotentialSoftNavigationPaint(LocalFrameView* frame_view, gfx::RectF rect, Node* node)
{
    ...
    if (LocalDOMWindow* window = frame.DomWindow()) {
        if (SoftNavigationHeuristics* heuristics = SoftNavigationHeuristics::From(*window)) {
            heuristics->RecordPaint(&frame, rect.size().GetArea(), node->IsModifiedBySoftNavigation());
        }
    }
}
...
ImagePaintTimingDetector::ImagePaintTimingDetector(LocalFrameView* frame_view, PaintTimingCallbackManager* callback_manager)
    : uses_page_viewport_(base::FeatureList::IsEnabled(features::kUsePageViewportInLCP))
...
{
    if (!largest_painted_image_ || (largest_pending_image_ && (largest_painted_image_->recorded_size < largest_pending_image_->recorded_size))) {
        return largest_pending_image_.Get();
    }
    return largest_painted_image_.Get();
}

void ImagePaintTimingDetector::PopulateTraceValue(TracedValue& value, const ImageRecord& first_image_paint)
{
    ...
    value.SetString("imageUrl", first_image_paint.media_timing ? String(first_image_paint.media_timing->Url()) : "(deleted)");
    value.SetInteger("size", static_cast<int>(first_image_paint.recorded_size));
    value.SetInteger("candidateIndex", ++count_candidates_);
    value.SetBoolean("isMainFrame", frame_view_->GetFrame().IsMainFrame());
    value.SetBoolean("isOutermostMainFrame", frame_view_->GetFrame().IsOutermostMainFrame());
    value.SetBoolean("isEmbeddedFrame", !frame_view_->GetFrame().LocalFrameRoot().IsMainFrame() || frame_view_->GetFrame().IsInFencedFrameTree());
    if (first_image_paint.lcp_rect_info_) {
        first_image_paint.lcp_rect_info_->OutputToTraceValue(value);
    }
}

void ImagePaintTimingDetector::ReportCandidateToTrace(ImageRecord& largest_image_record, base::TimeTicks time)
{
    ...
    TRACE_EVENT_MARK_WITH_TIMESTAMP2(
        "loading", "LargestImagePaint::Candidate", time, "data", std::move(value), "frame", GetFrameIdForTracing(&frame_view_->GetFrame()));
}
...
void ImagePaintTimingDetector::ReportNoCandidateToTrace()
{
    ...
    value->SetBoolean("isOutermostMainFrame", frame_view_->GetFrame().IsOutermostMainFrame());
    value->SetBoolean("isEmbeddedFrame", !frame_view_->GetFrame().LocalFrameRoot().IsMainFrame() || frame_view_->GetFrame().IsInFencedFrameTree());
    TRACE_EVENT2("loading", "LargestImagePaint::NoCandidate", "data", std::move(value), "frame", GetFrameIdForTracing(&frame_view_->GetFrame()));
}

std::pair<ImageRecord*, bool> ImagePaintTimingDetector::UpdateMetricsCandidate()
{
    ImageRecord* largest_image_record = records_manager_.LargestImage();
    base::TimeTicks time = largest_image_record ? largest_image_record->paint_time : base::TimeTicks();
    bool animated_first_frame_ready = largest_image_record && !largest_image_record->first_animated_frame_time.is_null();
    if (animated_first_frame_ready) {
        time = largest_image_record->first_animated_frame_time;
    }

    const uint64_t size = largest_image_record ? largest_image_record->recorded_size : 0;

    double bpp = largest_image_record ? largest_image_record->EntropyForLCP() : 0.0;

    std::optional<WebURLRequest::Priority> priority = largest_image_record ? largest_image_record->RequestPriority() : std::nullopt;
    ...
    bool changed
        = detector.GetLargestContentfulPaintCalculator()->NotifyMetricsIfLargestImagePaintChanged(time, size, largest_image_record, bpp, std::move(priority));
    if (changed) {
...
void ImagePaintTimingDetector::NotifyImageRemoved(const LayoutObject& object, const MediaTiming* media_timing)
{
    records_manager_.RemoveRecord(MediaRecordId::GenerateHash(&object, media_timing));
}
...
{
    auto callback = WTF::BindOnce(&ImagePaintTimingDetector::ReportPresentationTime, WrapWeakPersistent(this), last_registered_frame_index_);
    callback_manager_->RegisterCallback(std::move(callback));
}

void ImagePaintTimingDetector::ReportPresentationTime(unsigned last_queued_frame_index, base::TimeTicks timestamp)
{
    // The callback is safe from race-condition only when running on main-thread.
    DCHECK(ThreadState::Current()->IsMainThread());
    records_manager_.AssignPaintTimeToRegisteredQueuedRecords(timestamp, last_queued_frame_index);
}

void ImageRecordsManager::AssignPaintTimeToRegisteredQueuedRecords(const base::TimeTicks& timestamp, unsigned last_queued_frame_index)
{
    while (!images_queued_for_paint_time_.empty()) {
        ...
        if (!record->loaded || !record->paint_time.is_null() || it == pending_images_.end()) {
            continue;
        }
        record->paint_time = timestamp;
        if (!largest_painted_image_ || largest_painted_image_->recorded_size < record->recorded_size) {
...
bool ImagePaintTimingDetector::RecordImage(const LayoutObject& object, const gfx::Size& intrinsic_size, const MediaTiming& media_timing,
    const PropertyTreeStateOrAlias& current_paint_chunk_properties, const StyleImage* style_image, const gfx::Rect& image_border)
{
    ...
    if (int depth = IgnorePaintTimingScope::IgnoreDepth()) {
        ...
        // 1).
        if (depth == 1 && IgnorePaintTimingScope::IsDocumentElementInvisible() && media_timing.IsSufficientContentLoadedForPaint()) {
            gfx::RectF mapped_visual_rect = frame_view_->GetPaintTimingDetector().CalculateVisualRect(image_border, current_paint_chunk_properties);
            uint64_t rect_size = ComputeImageRectSize(image_border, mapped_visual_rect, intrinsic_size, current_paint_chunk_properties, object, media_timing);
            records_manager_.MaybeUpdateLargestIgnoredImage(record_id, rect_size, image_border, mapped_visual_rect);
        }
        return false;
    }
    ...
    if (records_manager_.IsRecordedImage(record_id_hash)) {
        ...
        if (!record)
            return false;
        if (media_timing.IsPaintedFirstFrame() && RuntimeEnabledFeatures::LCPAnimatedImagesWebExposedEnabled()) {
            added_entry_in_latest_frame_ |= records_manager_.OnFirstAnimatedFramePainted(record_id_hash, frame_index_);
        }
        if (!record->loaded && media_timing.IsSufficientContentLoadedForPaint()) {
            ...
            if (std::optional<PaintTimingVisualizer>& visualizer = frame_view_->GetPaintTimingDetector().Visualizer()) {
                gfx::RectF mapped_visual_rect = frame_view_->GetPaintTimingDetector().CalculateVisualRect(image_border, current_paint_chunk_properties);
                visualizer->DumpImageDebuggingRect(object, mapped_visual_rect, media_timing.IsSufficientContentLoadedForPaint(), media_timing.Url());
            }
            return true;
        }
        return false;
    ...
    double bpp = (rect_size > 0) ? media_timing.ContentSizeForEntropy() * 8.0 / rect_size : 0.0;

    bool added_pending = records_manager_.RecordFirstPaintAndReturnIsPending(record_id, rect_size, image_border, mapped_visual_rect, bpp);
    if (!added_pending)
        return false;
...
uint64_t ImagePaintTimingDetector::ComputeImageRectSize(const gfx::Rect& image_border, const gfx::RectF& mapped_visual_rect, const gfx::Size& intrinsic_size,
    const PropertyTreeStateOrAlias& current_paint_chunk_properties, const LayoutObject& object, const MediaTiming& media_timing)
{
    if (std::optional<PaintTimingVisualizer>& visualizer = frame_view_->GetPaintTimingDetector().Visualizer()) {
        visualizer->DumpImageDebuggingRect(object, mapped_visual_rect, media_timing.IsSufficientContentLoadedForPaint(), media_timing.Url());
    }
    ...
    gfx::RectF float_visual_rect = frame_view_->GetPaintTimingDetector().BlinkSpaceToDIPs(gfx::RectF(image_border));
    if (!viewport_size_.has_value()) {
        ...
        gfx::RectF viewport = frame_view_->GetPaintTimingDetector().BlinkSpaceToDIPs(gfx::RectF(viewport_int_rect));
    ...
    rect_size = DownScaleIfIntrinsicSizeIsSmaller(rect_size, intrinsic_size.Area64(), float_visual_rect.size().GetArea());
    return rect_size;
}

void ImagePaintTimingDetector::NotifyImageFinished(const LayoutObject& object, const MediaTiming* media_timing)
{
    records_manager_.NotifyImageFinished(MediaRecordId::GenerateHash(&object, media_timing));
}
...
bool ImageRecordsManager::OnFirstAnimatedFramePainted(MediaRecordIdHash record_id_hash, unsigned current_frame_index)
{
...
void ImageRecordsManager::OnImageLoaded(MediaRecordIdHash record_id_hash, unsigned current_frame_index, const StyleImage* style_image)
{
    ImageRecord* record = GetPendingImage(record_id_hash);
    DCHECK(record);
    if (!style_image) {
        ...
        if (document && document->domWindow()) {
            record->load_time = ImageElementTiming::From(*document->domWindow()).GetBackgroundImageLoadTime(style_image);
...
void ImageRecordsManager::ReportLargestIgnoredImage(unsigned current_frame_index)
{
    if (!largest_ignored_image_)
        return;
    Node* node = DOMNodeIds::NodeForId(largest_ignored_image_->node_id);
    if (!node || !node->GetLayoutObject() || !largest_ignored_image_->media_timing) {
...
void ImageRecordsManager::OnImageLoadedInternal(ImageRecord* record, unsigned current_frame_index)
{
    SetLoaded(record);
    QueueToMeasurePaintTime(record, current_frame_index);
}
...
void ImageRecordsManager::MaybeUpdateLargestIgnoredImage(
    const MediaRecordId& record_id, const uint64_t& visual_size, const gfx::Rect& frame_visual_rect, const gfx::RectF& root_visual_rect)
{
    if (visual_size && (!largest_ignored_image_ || visual_size > largest_ignored_image_->recorded_size)) {
        largest_ignored_image_ = CreateImageRecord(
            *record_id.GetLayoutObject(), record_id.GetMediaTiming(), visual_size, frame_visual_rect, root_visual_rect, record_id.GetHash());
        largest_ignored_image_->load_time = base::TimeTicks::Now();
    }
}

bool ImageRecordsManager::RecordFirstPaintAndReturnIsPending(
    const MediaRecordId& record_id, const uint64_t& visual_size, const gfx::Rect& frame_visual_rect, const gfx::RectF& root_visual_rect, double bpp)
{
    ...
    if (visual_size == 0u || (largest_painted_image_ && largest_painted_image_->recorded_size > visual_size)) {
        return false;
    }
    if (base::FeatureList::IsEnabled(features::kExcludeLowEntropyImagesFromLCP) && bpp < features::kMinimumEntropyForLCP.Get()) {
        return false;
    }

    ImageRecord* record
        = CreateImageRecord(*record_id.GetLayoutObject(), record_id.GetMediaTiming(), visual_size, frame_visual_rect, root_visual_rect, record_id.GetHash());
    AddPendingImage(record);
    return true;
}
void ImageRecordsManager::AddPendingImage(ImageRecord* record)
{
    if (!largest_pending_image_ || (largest_pending_image_->recorded_size < record->recorded_size)) {
        largest_pending_image_ = record;
    }
    pending_images_.insert(record->hash, record);
}

ImageRecord* ImageRecordsManager::CreateImageRecord(const LayoutObject& object, const MediaTiming* media_timing, const uint64_t& visual_size,
    const gfx::Rect& frame_visual_rect, const gfx::RectF& root_visual_rect, MediaRecordIdHash hash)
{
    DCHECK_GT(visual_size, 0u);
    Node* node = object.GetNode();
    DOMNodeId node_id = node->GetDomNodeId();
    return MakeGarbageCollected<ImageRecord>(node_id, media_timing, visual_size, frame_visual_rect, root_visual_rect, hash);
}
