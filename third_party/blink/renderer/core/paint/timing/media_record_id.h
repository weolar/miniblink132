...
public:
    static MediaRecordIdHash CORE_EXPORT GenerateHash(const LayoutObject* layout, const MediaTiming* media);

    MediaRecordId(const LayoutObject* layout, const MediaTiming* media);

    MediaRecordIdHash GetHash() const
    {
        return hash_;
    }
    ...
    const MediaTiming* GetMediaTiming() const
    {
        return media_timing_;
    }
