...
void PaintTimingCallbackManagerImpl::ReportPaintTime(
    std::unique_ptr<PaintTimingCallbackManager::CallbackQueue> frame_callbacks, const viz::FrameTimingDetails& presentation_details)
{
    ...
    while (!frame_callbacks->empty()) {
        std::move(frame_callbacks->front()).Run(presentation_details.presentation_feedback.timestamp);
        frame_callbacks->pop();
    }
    frame_view_->GetPaintTimingDetector().UpdateLcpCandidate();
}

void PaintTimingCallbackManagerImpl::RegisterPaintTimeCallbackForCombinedCallbacks()
{
    ...
    auto combined_callback
        = CrossThreadBindOnce(&PaintTimingCallbackManagerImpl::ReportPaintTime, WrapCrossThreadWeakPersistent(this), std::move(frame_callbacks_));
    frame_callbacks_ = std::make_unique<PaintTimingCallbackManager::CallbackQueue>();
    ...
    // or fails to happen.
    frame.GetPage()->GetChromeClient().NotifyPresentationTime(frame, std::move(combined_callback));
}
