...
protected:
    ...
    {
        return GetChildFrameView().GetPaintTimingDetector().GetTextPaintTimingDetector();
    }
    ...
    {
        return view.GetPaintTimingDetector().GetTextPaintTimingDetector().texts_queued_for_paint_time_.size();
    }
    ...
    void InvokePresentationTimeCallback(MockPaintTimingCallbackManager* callback_manager)
    {
        callback_manager->InvokePresentationTimeCallback(test_task_runner_->NowTicks());
    }

    base::TimeTicks LargestPaintTime()
    {
        return GetPaintTimingDetector().LatestLcpDetailsForTest().largest_text_paint_time;
    }

    uint64_t LargestPaintSize()
    {
        return GetPaintTimingDetector().LatestLcpDetailsForTest().largest_text_paint_size;
    }
    ...
    void SetChildBodyInnerHTML(const String& content)
    {
        ...
        child_frame_mock_callback_manager_ = MakeGarbageCollected<MockPaintTimingCallbackManager>();
        GetChildFrameTextPaintTimingDetector().ResetCallbackManager(child_frame_mock_callback_manager_);
        UpdateAllLifecyclePhases();
    }
    ...
    void CheckSizeOfTextQueuedForPaintTimeAfterUpdateLifecyclePhases(wtf_size_t size)
    {
        UpdateAllLifecyclePhases();
    ...
    {
        return GetChildFrameView().GetPaintTimingDetector().GetTextPaintTimingDetector().ltp_manager_->LargestText();
    }

    void SetFontSize(Element* font_element, uint16_t font_size)
    {
        DCHECK_EQ(font_element->nodeName(), "FONT");
        font_element->setAttribute(html_names::kSizeAttr, AtomicString(WTF::String::Number(font_size)));
    }
...
TEST_F(TextPaintTimingDetectorTest, LargestTextPaint_FontSizeChange_MultipleUpdates)
{
...
TEST_F(TextPaintTimingDetectorTest, LargestTextPaint_TraceEvent_Candidate_Frame)
{
    using trace_analyzer::Query;
    trace_analyzer::Start("loading");
    {
...
TEST_F(TextPaintTimingDetectorTest, NodeRemovedBeforeAssigningPresentationTime)
{
    SetBodyInnerHTML(R"HTML(
    <div id="parent">
    ...
    UpdateAllLifecyclePhases();
    GetDocument().getElementById(AtomicString("parent"))->RemoveChild(GetDocument().getElementById(AtomicString("remove")));
    InvokeCallback();
    EXPECT_EQ(TextRecordOfLargestTextPaint(), nullptr);
}
...
TEST_F(TextPaintTimingDetectorTest, LargestTextPaint_ReportFirstPaintTime)
{
    ...
    UpdateAllLifecyclePhasesAndSimulatePresentationTime();
    AdvanceClock(base::Seconds(1));
    text->setAttribute(html_names::kStyleAttr, AtomicString("position:fixed;left:30px"));
    UpdateAllLifecyclePhasesAndSimulatePresentationTime();
    AdvanceClock(base::Seconds(1));
...
TEST_F(TextPaintTimingDetectorTest, LargestTextPaint_IgnoreTextOutsideViewport)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      div.out {
...
TEST_F(TextPaintTimingDetectorTest, LargestTextPaint_RemovedText)
{
    ...
    Element* large_text = AppendDivElementToBody("(large text)(large text)(large text)(large text)(large text)(large "
                                                 "text)");
...
TEST_F(TextPaintTimingDetectorTest, RemoveRecordFromAllContainerAfterTextRemoval)
{
    SetBodyInnerHTML(R"HTML(
  )HTML");
    Element* text = AppendDivElementToBody("text");
...
TEST_F(TextPaintTimingDetectorTest, RemoveRecordFromAllContainerAfterRepeatedAttachAndDetach)
{
    SetBodyInnerHTML(R"HTML(
  )HTML");
...
TEST_F(TextPaintTimingDetectorTest, DestroyLargestTextPaintMangerAfterUserInput)
{
    SetBodyInnerHTML(R"HTML(
  )HTML");
    AppendDivElementToBody("text");
...
TEST_F(TextPaintTimingDetectorTest, LargestTextPaint_CompareVisualSizeNotActualSize)
{
    SetBodyInnerHTML(R"HTML(
  )HTML");
...
TEST_F(TextPaintTimingDetectorTest, CaptureSVGText)
{
    ...
    auto* elem = To<SVGTextContentElement>(GetDocument().QuerySelector(AtomicString("text")));
    UpdateAllLifecyclePhasesAndSimulatePresentationTime();
    EXPECT_EQ(CountRecordedSize(), 1u);
...
TEST_F(TextPaintTimingDetectorTest, ClippedByParentVisibleRect)
{
    ...
    GetDocument().body()->getElementById(AtomicString("outer1"))->AppendChild(div1);
    ...
    GetDocument().body()->getElementById(AtomicString("outer2"))->AppendChild(div2);
