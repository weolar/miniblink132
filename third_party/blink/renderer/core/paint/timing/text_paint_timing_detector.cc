...
TextPaintTimingDetector::TextPaintTimingDetector(
    LocalFrameView* frame_view, PaintTimingDetector* paint_timing_detector, PaintTimingCallbackManager* callback_manager)
    : callback_manager_(callback_manager)
    , frame_view_(frame_view)
    , ltp_manager_(MakeGarbageCollected<LargestTextPaintManager>(frame_view, paint_timing_detector))
{
}

void LargestTextPaintManager::PopulateTraceValue(TracedValue& value, const TextRecord& first_text_paint)
{
    value.SetInteger("DOMNodeId", static_cast<int>(first_text_paint.node_->GetDomNodeId()));
...
void LargestTextPaintManager::ReportCandidateToTrace(const TextRecord& largest_text_record)
{
    if (!PaintTimingDetector::IsTracing())
        return;
    auto value = std::make_unique<TracedValue>();
    PopulateTraceValue(*value, largest_text_record);
    TRACE_EVENT_MARK_WITH_TIMESTAMP2("loading", "LargestTextPaint::Candidate", largest_text_record.paint_time, "data", std::move(value), "frame",
...
std::pair<TextRecord*, bool> LargestTextPaintManager::UpdateMetricsCandidate()
{
    ...
    bool changed = paint_timing_detector_->GetLargestContentfulPaintCalculator()->NotifyMetricsIfLargestTextPaintChanged(time, size);
    if (changed) {
...
void TextPaintTimingDetector::OnPaintFinished()
{
    ...
    RegisterNotifyPresentationTime(WTF::BindOnce(&TextPaintTimingDetector::ReportPresentationTime, WrapWeakPersistent(this), frame_index_++));
    added_entry_in_latest_frame_ = false;
}

void TextPaintTimingDetector::LayoutObjectWillBeDestroyed(const LayoutObject& object)
{
...
void TextPaintTimingDetector::RegisterNotifyPresentationTime(PaintTimingCallbackManager::LocalThreadCallback callback)
{
    callback_manager_->RegisterCallback(std::move(callback));
}

void TextPaintTimingDetector::ReportPresentationTime(uint32_t frame_index, base::TimeTicks timestamp)
{
...
bool TextPaintTimingDetector::ShouldWalkObject(const LayoutBoxModelObject& object) const
{
    ...
    // its text.
    if (!IsRecordingLargestTextPaint() && !TextElementTiming::NeededForElementTiming(*node)) {
        return false;
    }
...
void TextPaintTimingDetector::RecordAggregatedText(
    const LayoutBoxModelObject& aggregator, const gfx::Rect& aggregated_visual_rect, const PropertyTreeStateOrAlias& property_tree_state)
{
    if (RuntimeEnabledFeatures::ExcludeTransparentTextsFromBeingLcpEligibleEnabled()) {
        bool is_color_transparent = aggregator.StyleRef().VisitedDependentColor(GetCSSPropertyColor()).IsFullyTransparent();
    ...
    gfx::RectF mapped_visual_rect = frame_view_->GetPaintTimingDetector().CalculateVisualRect(aggregated_visual_rect, property_tree_state);
    ...
    if (IgnorePaintTimingScope::IgnoreDepth() == 1) {
        if (IgnorePaintTimingScope::IsDocumentElementInvisible() && IsRecordingLargestTextPaint()) {
            ltp_manager_->MaybeUpdateLargestIgnoredText(aggregator, aggregated_size, aggregated_visual_rect, mapped_visual_rect);
        }
        return;
    }
    ...
    if (LocalDOMWindow* window = frame.DomWindow()) {
        if (SoftNavigationHeuristics* heuristics = SoftNavigationHeuristics::From(*window)) {
            heuristics->RecordPaint(&frame, mapped_visual_rect.size().GetArea(), aggregator.GetNode()->IsModifiedBySoftNavigation());
        }
    }
    recorded_set_.insert(&aggregator);
    MaybeRecordTextRecord(aggregator, aggregated_size, property_tree_state, aggregated_visual_rect, mapped_visual_rect);
...
LargestTextPaintManager::LargestTextPaintManager(LocalFrameView* frame_view, PaintTimingDetector* paint_timing_detector)
    : frame_view_(frame_view)
...
void LargestTextPaintManager::MaybeUpdateLargestIgnoredText(
    const LayoutObject& object, const uint64_t& size, const gfx::Rect& frame_visual_rect, const gfx::RectF& root_visual_rect)
{
    if (size && (!largest_ignored_text_ || size > largest_ignored_text_->recorded_size)) {
        ...
        largest_ignored_text_ = MakeGarbageCollected<TextRecord>(*object.GetNode(), size, gfx::RectF(), frame_visual_rect, root_visual_rect, 0u);
    }
}
...
void TextPaintTimingDetector::AssignPaintTimeToQueuedRecords(uint32_t frame_index, const base::TimeTicks& timestamp)
{
    bool can_report_element_timing = text_element_timing_ ? text_element_timing_->CanReportElements() : false;
    ...
    for (const auto& it : texts_queued_for_paint_time_) {
        ...
        if (ltp_manager_ && (record->recorded_size > 0u) && !(record->node_ && ltp_manager_->IsUnrelatedSoftNavigationPaint(*(record->node_)))) {
...
void TextPaintTimingDetector::MaybeRecordTextRecord(const LayoutObject& object, const uint64_t& visual_size,
    const PropertyTreeStateOrAlias& property_tree_state, const gfx::Rect& frame_visual_rect, const gfx::RectF& root_visual_rect)
{
    Node* node = object.GetNode();
    DCHECK(node);
    // If the node is not required by LCP and not required by ElementTiming, we
    // can bail out early.
    if ((visual_size == 0u || !IsRecordingLargestTextPaint()) && !TextElementTiming::NeededForElementTiming(*node)) {
        return;
    }
    TextRecord* record;
    if (visual_size == 0u) {
        record = MakeGarbageCollected<TextRecord>(*node, 0, gfx::RectF(), gfx::Rect(), gfx::RectF(), frame_index_);
    } else {
        record = MakeGarbageCollected<TextRecord>(*object.GetNode(), visual_size,
            TextElementTiming::ComputeIntersectionRect(object, frame_visual_rect, property_tree_state, frame_view_), frame_visual_rect, root_visual_rect,
            frame_index_);
    }
