...
class CORE_EXPORT FirstMeaningfulPaintDetector : public GarbageCollected<FirstMeaningfulPaintDetector> {
public:
    ...
    FirstMeaningfulPaintDetector& operator=(const FirstMeaningfulPaintDetector&) = delete;
    virtual ~FirstMeaningfulPaintDetector() = default;

    void MarkNextPaintAsMeaningfulIfNeeded(
        const LayoutObjectCounter&, double contents_height_before_layout, double contents_height_after_layout, int visible_height);
...
private:
    ...
    enum DeferFirstMeaningfulPaint { kDoNotDefer, kDeferOutstandingPresentationPromises, kDeferFirstContentfulPaintNotSet };
    ...
    HadUserInput had_user_input_before_provisional_first_meaningful_paint_ = kNoUserInput;
