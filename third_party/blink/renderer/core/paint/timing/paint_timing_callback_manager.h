...
public:
    ...
    virtual void RegisterCallback(PaintTimingCallbackManager::LocalThreadCallback) = 0;
};
...
// registration.
class CORE_EXPORT PaintTimingCallbackManagerImpl final : public GarbageCollected<PaintTimingCallbackManagerImpl>, public PaintTimingCallbackManager {
public:
    PaintTimingCallbackManagerImpl(LocalFrameView* frame_view)
        : frame_view_(frame_view)
        , frame_callbacks_(std::make_unique<std::queue<PaintTimingCallbackManager::LocalThreadCallback>>())
    {
    }
    ~PaintTimingCallbackManagerImpl()
    {
        frame_callbacks_.reset();
    }
    ...
    // `frame_callbacks_`.
    void RegisterCallback(PaintTimingCallbackManager::LocalThreadCallback callback) override
    {
    ...
    inline size_t CountCallbacks()
    {
        return frame_callbacks_->size();
    }

    void ReportPaintTime(
        std::unique_ptr<std::queue<PaintTimingCallbackManager::LocalThreadCallback>> frame_callbacks, const viz::FrameTimingDetails& presentation_details);
