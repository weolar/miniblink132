...
TEST_F(CSSMaskPainterTest, MaskBoundingBoxSVG)
{
    ...
    std::optional<gfx::RectF> mask_bounding_box = CSSMaskPainter::MaskBoundingBox(masked, PhysicalOffset());
...
TEST_F(CSSMaskPainterTest, MaskBoundingBoxCSSBlock)
{
    ...
    auto& masked = *GetLayoutObjectByElementId("masked");
    std::optional<gfx::RectF> mask_bounding_box = CSSMaskPainter::MaskBoundingBox(masked, PhysicalOffset(8, 8));
    ASSERT_TRUE(mask_bounding_box.has_value());
    EXPECT_EQ(gfx::RectF(8, 8, 300, 200), *mask_bounding_box);

    mask_bounding_box = CSSMaskPainter::MaskBoundingBox(masked, PhysicalOffset(LayoutUnit(8.25f), LayoutUnit(8.75f)));
    ASSERT_TRUE(mask_bounding_box.has_value());
