...
bool VisibleToHitTesting(const LayoutBox& box)
{
    if (RuntimeEnabledFeatures::HitTestOpaquenessEnabled()) {
        return ObjectPainter(box).GetHitTestOpaqueness() != cc::HitTestOpaqueness::kTransparent;
    }
...
void ScrollableAreaPainter::PaintResizer(GraphicsContext& context, const PhysicalOffset& paint_offset, const CullRect& cull_rect)
{
    ...
    gfx::Rect visual_rect = scrollable_area_.ResizerCornerRect(kResizerForPointer);
    ...
    if (const auto* resizer = scrollable_area_.Resizer()) {
        CustomScrollbarTheme::PaintIntoRect(*resizer, context, PhysicalRect(visual_rect));
        return;
    }
    ...
    if (scrollable_area_.NeedsScrollCorner()) {
        ...
        context.StrokeRect(corner_outline, PaintAutoDarkMode(box->StyleRef(), DarkModeFilter::ElementRole::kBackground));
    }
}

void ScrollableAreaPainter::RecordResizerScrollHitTestData(GraphicsContext& context, const PhysicalOffset& paint_offset)
{
    const auto* box = scrollable_area_.GetLayoutBox();
    ...
    context.GetPaintController().RecordScrollHitTestData(scrollable_area_.GetScrollCornerDisplayItemClient(), DisplayItem::kResizerScrollHitTest, nullptr,
        touch_rect,
        // Assume hit testing in some area may pass though.
        cc::HitTestOpaqueness::kMixed);
}

void ScrollableAreaPainter::DrawPlatformResizerImage(GraphicsContext& context, const gfx::Rect& resizer_corner_rect)
{
    gfx::Point points[4];
    bool on_left = false;
    float paint_scale = scrollable_area_.ScaleFromDIP();
    int edge_offset = std::ceil(paint_scale);
    if (scrollable_area_.GetLayoutBox()->ShouldPlaceBlockDirectionScrollbarOnLogicalLeft()) {
        on_left = true;
        points[0].set_x(resizer_corner_rect.x() + edge_offset);
        points[1].set_x(resizer_corner_rect.x() + resizer_corner_rect.width() - resizer_corner_rect.width() / 2);
        points[2].set_x(points[0].x());
        points[3].set_x(resizer_corner_rect.x() + resizer_corner_rect.width() - resizer_corner_rect.width() * 3 / 4);
    } else {
        points[0].set_x(resizer_corner_rect.x() + resizer_corner_rect.width() - edge_offset);
        points[1].set_x(resizer_corner_rect.x() + resizer_corner_rect.width() / 2);
        points[2].set_x(points[0].x());
        points[3].set_x(resizer_corner_rect.x() + resizer_corner_rect.width() * 3 / 4);
    }
    points[0].set_y(resizer_corner_rect.y() + resizer_corner_rect.height() / 2);
    points[1].set_y(resizer_corner_rect.y() + resizer_corner_rect.height() - edge_offset);
    points[2].set_y(resizer_corner_rect.y() + resizer_corner_rect.height() * 3 / 4);
    ...
    AutoDarkMode auto_dark_mode(PaintAutoDarkMode(scrollable_area_.GetLayoutBox()->StyleRef(), DarkModeFilter::ElementRole::kBackground));
...
bool ScrollableAreaPainter::PaintOverflowControls(const PaintInfo& paint_info, const PhysicalOffset& paint_offset, const FragmentData* fragment)
{
    if (!fragment) {
        return false;
    }
    ...
    if (scrollable_area_.ShouldOverflowControlsPaintAsOverlay()) {
        if (box.HasSelfPaintingLayer() || box.Layer()->NeedsReorderOverlayOverflowControls()) {
    ...
    if (box.IsGlobalRootScroller()) {
        ...
        if (const auto* scrollbar_transform = viewport.TransformNodeForViewportScrollbars()) {
    ...
    if (clip || transform) {
        ...
        PropertyTreeStateOrAlias modified_properties(paint_controller.CurrentPaintChunkProperties());
        if (clip)
            modified_properties.SetClip(*clip);
        if (transform)
            modified_properties.SetTransform(*transform);

        scoped_paint_chunk_properties.emplace(paint_controller, modified_properties, box, DisplayItem::kOverflowControls);
    }

    if (scrollable_area_.HorizontalScrollbar()) {
        PaintScrollbar(context, *scrollable_area_.HorizontalScrollbar(), paint_offset, paint_info.GetCullRect());
    }
    if (scrollable_area_.VerticalScrollbar()) {
        PaintScrollbar(context, *scrollable_area_.VerticalScrollbar(), paint_offset, paint_info.GetCullRect());
    }
...
void ScrollableAreaPainter::PaintScrollbar(GraphicsContext& context, Scrollbar& scrollbar, const PhysicalOffset& paint_offset, const CullRect& cull_rect)
{
    // Don't paint overlay scrollbars when printing otherwise all scrollbars will
    // be visible and cover contents.
    if (scrollbar.IsOverlayScrollbar() && scrollable_area_.GetLayoutBox()->GetDocument().Printing()) {
        return;
    }
    ...
    CHECK(properties);
    auto type = scrollbar.Orientation() == kHorizontalScrollbar ? DisplayItem::kScrollbarHorizontal : DisplayItem::kScrollbarVertical;
    std::optional<ScopedPaintChunkProperties> chunk_properties;
    if (const auto* effect
        = scrollbar.Orientation() == kHorizontalScrollbar ? properties->HorizontalScrollbarEffect() : properties->VerticalScrollbarEffect()) {
    ...
    if (scrollbar.IsCustomScrollbar()) {
        ...
        if (VisibleToHitTesting(*scrollable_area_.GetLayoutBox())) {
            context.GetPaintController().RecordScrollHitTestData(scrollbar, DisplayItem::kScrollbarHitTest, nullptr, visual_rect,
                // Assume hit testing in some area may pass though.
                cc::HitTestOpaqueness::kMixed);
        }
    } else {
...
void ScrollableAreaPainter::PaintNativeScrollbar(GraphicsContext& context, Scrollbar& scrollbar, gfx::Rect visual_rect)
{
    auto type = scrollbar.Orientation() == kHorizontalScrollbar ? DisplayItem::kScrollbarHorizontal : DisplayItem::kScrollbarVertical;
    ...
    if (scrollbar.GetTheme().AllowsHitTest()) {
        hit_test_opaqueness = ObjectPainter(*scrollable_area_.GetLayoutBox()).GetHitTestOpaqueness();
        if (RuntimeEnabledFeatures::HitTestOpaquenessEnabled() && hit_test_opaqueness == cc::HitTestOpaqueness::kMixed) {
    ...
    ScrollbarDisplayItem::Record(context, scrollbar, type, std::move(delegate), visual_rect, scroll_translation, scrollbar.GetElementId(), hit_test_opaqueness);
}

void ScrollableAreaPainter::PaintScrollCorner(GraphicsContext& context, const PhysicalOffset& paint_offset, const CullRect& cull_rect)
{
    ...
    if (const auto* effect = properties->ScrollCornerEffect()) {
        chunk_properties.emplace(context.GetPaintController(), *effect, client, DisplayItem::kScrollCorner);
    }

    if (const auto* scroll_corner = scrollable_area_.ScrollCorner()) {
        CustomScrollbarTheme::PaintIntoRect(*scroll_corner, context, PhysicalRect(visual_rect));
        return;
    }
