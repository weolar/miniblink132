...
{
    return properties.Filter() && properties.Filter()->Filter().HasReferenceFilter();
}
...
bool SVGContainerPainter::CanUseCullRect() const
{
    ...
    return SVGModelObjectPainter::CanUseCullRect(layout_svg_container_.StyleRef());
}
...
void SVGContainerPainter::Paint(const PaintInfo& paint_info)
{
    ...
    auto* svg_svg_element = DynamicTo<SVGSVGElement>(*layout_svg_container_.GetElement());
    if (svg_svg_element && svg_svg_element->HasEmptyViewBox())
        return;

    const auto* properties = layout_svg_container_.FirstFragment().PaintProperties();
    PaintInfo paint_info_before_filtering(paint_info);
    if (CanUseCullRect()) {
        if (!paint_info.GetCullRect().IntersectsTransformed(
                layout_svg_container_.LocalToSVGParentTransform(), layout_svg_container_.VisualRectInLocalSVGCoordinates()))
            return;
        if (properties) {
    ...
    ScopedSVGTransformState transform_state(paint_info_before_filtering, layout_svg_container_);
    {
        std::optional<ScopedPaintChunkProperties> scoped_paint_chunk_properties;
        if (layout_svg_container_.IsSVGViewportContainer() && SVGLayoutSupport::IsOverflowHidden(layout_svg_container_)) {
            ...
            if (properties && properties->OverflowClip()) {
                scoped_paint_chunk_properties.emplace(paint_info_before_filtering.context.GetPaintController(), *properties->OverflowClip(),
                    layout_svg_container_, paint_info_before_filtering.DisplayItemTypeForClipping());
            }
        }

        ScopedSVGPaintState paint_state(layout_svg_container_, paint_info_before_filtering);
        // When a filter applies to the container we need to make sure
        // that it is applied even if nothing is painted.
        if (paint_info_before_filtering.phase == PaintPhase::kForeground && properties && HasReferenceFilterEffect(*properties))
        ...
        if (IsA<SVGUseElement>(layout_svg_container_.GetElement())) {
            SVGObjectPainter object_painter(layout_svg_container_, child_paint_info.GetSvgContextPaints());
            ...
            // context paints.
            child_context_paints.emplace(object_painter.ResolveContextPaint(layout_svg_container_.StyleRef().FillPaint()),
                object_painter.ResolveContextPaint(layout_svg_container_.StyleRef().StrokePaint()));
            child_paint_info.SetSvgContextPaints(&(*child_context_paints));
        }

        for (LayoutObject* child = layout_svg_container_.FirstChild(); child; child = child->NextSibling()) {
            if (auto* foreign_object = DynamicTo<LayoutSVGForeignObject>(child)) {
                SVGForeignObjectPainter(*foreign_object).PaintLayer(paint_info_before_filtering);
            } else {
    ...
    if (layout_svg_container_.FirstChild()) {
        SVGModelObjectPainter(layout_svg_container_).PaintOutline(paint_info_before_filtering);
    }

    if (paint_info_before_filtering.ShouldAddUrlMetadata() && paint_info_before_filtering.phase == PaintPhase::kForeground) {
        ObjectPainter(layout_svg_container_).AddURLRectIfNeeded(paint_info_before_filtering, PhysicalOffset());
    }
}
