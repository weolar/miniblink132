...
public:
    MOCK_METHOD3(MockUpdateTooltipUnderCursor, void(LocalFrame*, const String&, TextDirection));
    void UpdateTooltipUnderCursor(LocalFrame& frame, const String& tooltip_text, TextDirection dir) override
    {
...
public:
    ...
    bool HasDirectCompositingReasons(const LayoutObject* scroller)
    {
        ...
        return paint_properties && paint_properties->Transform() && paint_properties->Transform()->HasDirectCompositingReasons();
    }
    ...
    bool UsesCompositedScrolling(const LayoutBox* scroller)
    {
        ...
        if (auto* scrollable_area = scroller->GetScrollableArea()) {
            const auto* property_trees = GetFrame().View()->RootCcLayer()->layer_tree_host()->property_trees();
            if (const auto* scroll_node = property_trees->scroll_tree().FindNodeFromElementId(scrollable_area->GetScrollElementId())) {
    ...
    {
        color_scheme_helper.SetPreferredRootScrollbarColorScheme(mojom::blink::PreferredColorScheme::kDark);
    ...
    {
        ASSERT_EQ(GetDocument().GetPreferredColorScheme(), mojom::blink::PreferredColorScheme::kLight);
        ASSERT_EQ(GetDocument().GetSettings()->GetPreferredRootScrollbarColorScheme(), mojom::blink::PreferredColorScheme::kLight);
    }

    void ExpectEqAllScrollControlsNeedPaintInvalidation(const PaintLayerScrollableArea* area, bool expectation) const
    {
...
TEST_P(PaintLayerScrollableAreaTest, OpaqueLayersDepromotedOnStyleChange)
{
    ...
    scroller->setAttribute(html_names::kStyleAttr, AtomicString("background: rgba(255,255,255,0.5) local content-box;"));
...
TEST_P(PaintLayerScrollableAreaTest, OpaqueLayersPromotedOnStyleChange)
{
    ...
    scroller->setAttribute(html_names::kStyleAttr, AtomicString("background: white local content-box;"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(UsesCompositedScrolling(scroller->GetLayoutBox()));
}
...
// promotion.
TEST_P(PaintLayerScrollableAreaTest, TransformDoesNotPreventCompositedScrolling)
{
    SetBodyInnerHTML(R"HTML(
    <style>
    #scroller { overflow: scroll; height: 200px; width: 200px; background:
    ...
    parent->setAttribute(html_names::kStyleAttr, AtomicString("transform: translate(1px, 0);"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(UsesCompositedScrolling(scroller->GetLayoutBox()));
    ...
    scroller->setAttribute(html_names::kStyleAttr, AtomicString("transform: translate(1px, 0);"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(UsesCompositedScrolling(scroller->GetLayoutBox()));
}

TEST_P(PaintLayerScrollableAreaTest, PromoteLayerRegardlessOfSelfAndAncestorOpacity)
{
    SetBodyInnerHTML(R"HTML(
    <style>
    #scroller { overflow: scroll; height: 200px; width: 200px; background:
...
TEST_P(PaintLayerScrollableAreaTest, CompositedScrollOnWillChangeTransform)
{
    ...
    scroller->setAttribute(html_names::kStyleAttr, AtomicString("will-change: transform"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(UsesCompositedScrolling(scroller->GetLayoutBox()));
...
TEST_P(PaintLayerScrollableAreaTest, ScrollLayerOnPointerEvents)
{
    ...
    // present.
    scroller->setAttribute(html_names::kStyleAttr, AtomicString("pointer-events: none"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(UsesCompositedScrolling(scroller->GetLayoutBox()));
    ...
    scroller->setAttribute(html_names::kStyleAttr, AtomicString("visibility: hidden"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_FALSE(UsesCompositedScrolling(scroller->GetLayoutBox()));
...
TEST_P(PaintLayerScrollableAreaTest, OverlayScrollbarColorThemeUpdated)
{
    ...
    ASSERT_EQ(mojom::blink::ColorScheme::kLight, none_layer->GetScrollableArea()->GetOverlayScrollbarColorScheme());
    ASSERT_EQ(mojom::blink::ColorScheme::kLight, white_layer->GetScrollableArea()->GetOverlayScrollbarColorScheme());
    ASSERT_EQ(mojom::blink::ColorScheme::kDark, black_layer->GetScrollableArea()->GetOverlayScrollbarColorScheme());
}

TEST_P(PaintLayerScrollableAreaTest, RecalculatesScrollbarOverlayIfBackgroundChanges)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      #scroller {
    ...
    EXPECT_EQ(mojom::blink::ColorScheme::kLight, scroll_paint_layer->GetScrollableArea()->GetOverlayScrollbarColorScheme());

    GetElementById("scroller")->setAttribute(html_names::kStyleAttr, AtomicString("background: rgb(34, 85, 51);"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(mojom::blink::ColorScheme::kDark, scroll_paint_layer->GetScrollableArea()->GetOverlayScrollbarColorScheme());

    GetElementById("scroller")->setAttribute(html_names::kStyleAttr, AtomicString("background: rgb(236, 143, 185);"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(mojom::blink::ColorScheme::kLight, scroll_paint_layer->GetScrollableArea()->GetOverlayScrollbarColorScheme());
}
...
TEST_P(PaintLayerScrollableAreaTest, PreferredOverlayScrollbarColorTheme)
{
    ...
    PaintLayer* white_layer = GetPaintLayerByElementId("white");
    PaintLayer* black_layer = GetPaintLayerByElementId("black");
    EXPECT_EQ(mojom::blink::ColorScheme::kDark, none_layer->GetScrollableArea()->GetOverlayScrollbarColorScheme());
    EXPECT_EQ(mojom::blink::ColorScheme::kLight, white_layer->GetScrollableArea()->GetOverlayScrollbarColorScheme());
    EXPECT_EQ(mojom::blink::ColorScheme::kDark, black_layer->GetScrollableArea()->GetOverlayScrollbarColorScheme());

    color_scheme_helper.SetPreferredColorScheme(mojom::blink::PreferredColorScheme::kLight);
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(mojom::blink::ColorScheme::kLight, none_layer->GetScrollableArea()->GetOverlayScrollbarColorScheme());
    EXPECT_EQ(mojom::blink::ColorScheme::kLight, white_layer->GetScrollableArea()->GetOverlayScrollbarColorScheme());
    EXPECT_EQ(mojom::blink::ColorScheme::kDark, black_layer->GetScrollableArea()->GetOverlayScrollbarColorScheme());
}
...
TEST_P(PaintLayerScrollableAreaTest, HideTooltipWhenScrollPositionChanges)
{
    ...
    Element* scroller = GetDocument().getElementById(AtomicString("scroller"));
    PaintLayerScrollableArea* scrollable_area = scroller->GetLayoutBox()->GetScrollableArea();
    ASSERT_TRUE(scrollable_area);

    EXPECT_CALL(GetChromeClient(), MockUpdateTooltipUnderCursor(GetDocument().GetFrame(), String(), _)).Times(1);
    scrollable_area->SetScrollOffset(ScrollOffset(1, 1), mojom::blink::ScrollType::kUser);
    ...
    EXPECT_CALL(GetChromeClient(), MockUpdateTooltipUnderCursor(GetDocument().GetFrame(), String(), _)).Times(0);
    scrollable_area->SetScrollOffset(ScrollOffset(2, 2), mojom::blink::ScrollType::kProgrammatic);
}
...
TEST_P(PaintLayerScrollableAreaTest, IncludeOverlayScrollbarsInVisibleWidth)
{
    ...
    PaintLayerScrollableArea* scrollable_area = scroller->GetLayoutBox()->GetScrollableArea();
    ASSERT_TRUE(scrollable_area);
    scrollable_area->SetScrollOffset(ScrollOffset(100, 0), mojom::blink::ScrollType::kClamping);
...
TEST_P(PaintLayerScrollableAreaTest, ShowAutoScrollbarsForVisibleContent)
{
    ...
    UpdateAllLifecyclePhasesForTest();
    PaintLayerScrollableArea* scrollable_area = outer_div->GetLayoutBox()->GetScrollableArea();
    ASSERT_TRUE(scrollable_area);
...
TEST_P(PaintLayerScrollableAreaTest, FloatOverflowInRtlContainer)
{
    ...
    PaintLayerScrollableArea* scrollable_area = container->GetLayoutBox()->GetScrollableArea();
    ASSERT_TRUE(scrollable_area);
...
TEST_P(PaintLayerScrollableAreaTest, OverflowHiddenScrollOffsetInvalidation)
{
    ...
    scrollable_area->SetScrollOffset(ScrollOffset(0, 1), mojom::blink::ScrollType::kProgrammatic);
    ...
    scrollable_area->SetScrollOffset(ScrollOffset(0, 2), mojom::blink::ScrollType::kProgrammatic);
    EXPECT_TRUE(scroller->NeedsPaintPropertyUpdate());
    UpdateAllLifecyclePhasesExceptPaint();
    EXPECT_TRUE(scroller->PaintingLayer()->SelfNeedsRepaint());
...
TEST_P(PaintLayerScrollableAreaTest, ScrollWithFixedDoesNotNeedCompositingUpdate)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      * {
...
TEST_P(PaintLayerScrollableAreaTest, ScrollWithLocalAttachmentBackgroundInScrollingContents)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      #scroller {
    ...
    auto* scroller = GetLayoutBoxByElementId("scroller");
    auto* scrollable_area = scroller->GetScrollableArea();
    EXPECT_EQ(kBackgroundPaintInContentsSpace, scroller->GetBackgroundPaintLocation());
...
TEST_P(PaintLayerScrollableAreaTest, ScrollWith3DPreserveParent)
{
    ...
    auto* scroller = GetLayoutBoxByElementId("scroller");
    EXPECT_EQ(kBackgroundPaintInBorderBoxSpace, scroller->GetBackgroundPaintLocation());
}

TEST_P(PaintLayerScrollableAreaTest, ScrollWithLocalAttachmentBackgroundInMainLayer)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      #scroller {
    ...
    auto* scroller = GetLayoutBoxByElementId("scroller");
    auto* scrollable_area = scroller->GetScrollableArea();
    EXPECT_EQ(kBackgroundPaintInBothSpaces, scroller->GetBackgroundPaintLocation());
...
TEST_P(PaintLayerScrollableAreaTest, ViewScrollWithFixedAttachmentBackground)
{
    ...
    EXPECT_EQ(kBackgroundPaintInBorderBoxSpace, fixed_background_div->GetBackgroundPaintLocation());
    ...
    view_scrollable_area->SetScrollOffset(ScrollOffset(0, 1), mojom::blink::ScrollType::kProgrammatic);
    ...
    div_scrollable_area->SetScrollOffset(ScrollOffset(0, 1), mojom::blink::ScrollType::kProgrammatic);
    EXPECT_TRUE(fixed_background_div->ShouldDoFullPaintInvalidation());
    EXPECT_TRUE(fixed_background_div->BackgroundNeedsFullPaintInvalidation());
...
TEST_P(PaintLayerScrollableAreaTest, ViewScrollWithSolidColorFixedAttachmentBackground)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      html, #fixed-background {
    ...
    EXPECT_EQ(kBackgroundPaintInContentsSpace, GetLayoutView().GetBackgroundPaintLocation());
    auto* fixed_background_div = GetLayoutBoxByElementId("fixed-background");
    EXPECT_EQ(kBackgroundPaintInContentsSpace, fixed_background_div->GetBackgroundPaintLocation());
    auto* div_scrollable_area = fixed_background_div->GetScrollableArea();
    auto* view_scrollable_area = GetLayoutView().GetScrollableArea();
...
TEST_P(PaintLayerScrollableAreaTest, ViewScrollWithFixedAttachmentBackgroundPreferCompositingToLCDText)
{
    SetPreferCompositingToLCDText(true);
    SetBodyInnerHTML(R"HTML(
    <style>
    ...
    EXPECT_EQ(kBackgroundPaintInBorderBoxSpace, GetLayoutView().GetBackgroundPaintLocation());
    auto* fixed_background_div = GetLayoutBoxByElementId("fixed-background");
    EXPECT_EQ(kBackgroundPaintInBorderBoxSpace, fixed_background_div->GetBackgroundPaintLocation());
    auto* div_scrollable_area = fixed_background_div->GetScrollableArea();
    auto* view_scrollable_area = GetLayoutView().GetScrollableArea();
...
TEST_P(PaintLayerScrollableAreaTest, ShowNonCompositedScrollbarOnCompositorScroll)
{
    ...
    scrollable_area->SetScrollOffset(ScrollOffset(0, 20), mojom::blink::ScrollType::kCompositor);
...
TEST_P(PaintLayerScrollableAreaTest, CompositedStickyDescendant)
{
    ...
    EXPECT_EQ(&sticky->FirstFragment().LocalBorderBoxProperties().Transform(), sticky->FirstFragment().PaintProperties()->StickyTranslation());
    EXPECT_TRUE(sticky->FirstFragment().PaintProperties()->StickyTranslation()->IsIdentity());

    scrollable_area->SetScrollOffset(ScrollOffset(0, 50), mojom::blink::ScrollType::kUser);
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintLayerScrollableAreaTest, StickyPositionUseCounter)
{
    ...
    test->setAttribute(html_names::kStyleAttr, AtomicString("top: 0; position: sticky;"));
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintLayerScrollableAreaTest, IgnoreDelayedScrollOnDestroyedLayer)
{
    ...
    Element* scroller = GetDocument().getElementById(AtomicString("scroller"));
    {
        ...
        PaintLayerScrollableArea::DelayScrollOffsetClampScope::SetNeedsClamp(scroller->GetLayoutBox()->GetScrollableArea());
        scroller->SetInlineStyleProperty(CSSPropertyID::kDisplay, CSSValueID::kNone);
        UpdateAllLifecyclePhasesForTest();
    }
}
...
TEST_P(PaintLayerScrollableAreaTest, ScrollbarMaximum)
{
    ...
    scrollable_area->ScrollBy(ScrollOffset(0, 1000), mojom::blink::ScrollType::kProgrammatic);
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(PaintLayerScrollableAreaTest, ScrollingBackgroundVisualRect)
{
    ...
    EXPECT_EQ(gfx::Rect(2, 2, 101, 200), GetLayoutBoxByElementId("scroller")->GetScrollableArea()->ScrollingBackgroundVisualRect(PhysicalOffset()));
}
...
TEST_P(PaintLayerScrollableAreaTest, ApplyPendingHistoryRestoreScrollOffsetTwice)
{
    GetPage().GetSettings().SetTextAreasAreResizable(true);
    SetBodyInnerHTML(R"HTML(
    <!doctype HTML>
    ...
    scrollable_area->SetPendingHistoryRestoreScrollOffset(view_state, true, mojom::blink::ScrollBehavior::kAuto);
...
class PaintLayerScrollableAreaTestLowEndPlatform : public TestingPlatformSupport {
public:
...
{
    ScopedTestingPlatformSupport<PaintLayerScrollableAreaTestLowEndPlatform> platform;
    SetBodyInnerHTML(R"HTML(
    <style>
      #scroller {
        width: 100px;
...
TEST_P(PaintLayerScrollableAreaTest, RootScrollbarShouldUseParentOfOverscrollNodeAsTransformNode)
{
    SetPreferCompositingToLCDText(true);
    SetBodyInnerHTML(R"HTML(
    <style>
...

    {
        ...
        const auto* parent_transform = visual_viewport.GetOverscrollElasticityTransformNode() ? visual_viewport.GetOverscrollElasticityTransformNode()->Parent()
                                                                                              : visual_viewport.GetPageScaleNode()->Parent();
        for (const auto& chunk : paint_chunks) {
            if (chunk.id == PaintChunk::Id(root_scrollable->VerticalScrollbar()->Id(), DisplayItem::kScrollbarHitTest)) {
    ...
    // Non root scrollbar should use scroller's transform node.
    {
        ...
        PaintLayerScrollableArea* scrollable_area = scroller_layer->GetScrollableArea();
        ASSERT_TRUE(scrollable_area);

        auto paint_properties = scroller_layer->GetLayoutObject().FirstFragment().LocalBorderBoxProperties();

        const auto& paint_chunks = ContentPaintChunks();
        bool found_subscroller_scrollbar = false;
        for (const auto& chunk : paint_chunks) {
            if (chunk.id == PaintChunk::Id(scrollable_area->VerticalScrollbar()->Id(), DisplayItem::kScrollbarHitTest)) {
...
TEST_P(PaintLayerScrollableAreaTest, ResizeSmallerToBeScrollableWithResizerAndStackedChild)
{
    USE_NON_OVERLAY_SCROLLBARS_OR_QUIT();
    ...
    EXPECT_TRUE(scroller->GetLayoutBox()->Layer()->NeedsReorderOverlayOverflowControls());
    ...
    EXPECT_FALSE(scroller->GetLayoutBox()->Layer()->NeedsReorderOverlayOverflowControls());
}
...
TEST_P(PaintLayerScrollableAreaTest, UsedColorSchemeRootScrollbarsDark)
{
    ...
    const auto* non_root_scrollable_area_dark = GetPaintLayerByElementId("dark")->GetScrollableArea();
    ASSERT_TRUE(non_root_scrollable_area_dark);
    const auto* non_root_scrollable_area_normal = GetPaintLayerByElementId("normal")->GetScrollableArea();
    ASSERT_TRUE(non_root_scrollable_area_normal);

    EXPECT_EQ(root_scrollable_area->UsedColorSchemeScrollbars(), mojom::blink::ColorScheme::kLight);
    EXPECT_EQ(non_root_scrollable_area_dark->UsedColorSchemeScrollbars(), mojom::blink::ColorScheme::kLight);
    EXPECT_EQ(non_root_scrollable_area_normal->UsedColorSchemeScrollbars(), mojom::blink::ColorScheme::kLight);
    ...
    // scheme is light.
    EXPECT_EQ(root_scrollable_area->UsedColorSchemeScrollbars(), mojom::blink::ColorScheme::kLight);
    EXPECT_EQ(non_root_scrollable_area_dark->UsedColorSchemeScrollbars(), mojom::blink::ColorScheme::kDark);
    EXPECT_EQ(non_root_scrollable_area_normal->UsedColorSchemeScrollbars(), mojom::blink::ColorScheme::kLight);
    ...
    EXPECT_EQ(root_scrollable_area->UsedColorSchemeScrollbars(), mojom::blink::ColorScheme::kDark);
    EXPECT_EQ(non_root_scrollable_area_dark->UsedColorSchemeScrollbars(), mojom::blink::ColorScheme::kDark);
    EXPECT_EQ(non_root_scrollable_area_normal->UsedColorSchemeScrollbars(), mojom::blink::ColorScheme::kLight);
}

TEST_P(PaintLayerScrollableAreaTest, UsedColorSchemeRootScrollbarsMetaLightDark)
{
    USE_NON_OVERLAY_SCROLLBARS_OR_QUIT();
...
TEST_P(PaintLayerScrollableAreaTest, UsedColorSchemeRootScrollbarsInvalidateOnPreferredColorSchemeChange)
{
    USE_NON_OVERLAY_SCROLLBARS_OR_QUIT();
...
TEST_P(PaintLayerScrollableAreaTest, UsedColorSchemeRootScrollbarsInvalidateOnNormalToLightChange)
{
    USE_NON_OVERLAY_SCROLLBARS_OR_QUIT();
    ...
    const auto* root_scrollable_area = GetLayoutView().GetScrollableArea();
    ASSERT_TRUE(root_scrollable_area);
    const auto* non_root_scrollable_area = GetPaintLayerByElementId("normal")->GetScrollableArea();
    ...
    GetDocument().documentElement()->SetInlineStyleProperty(CSSPropertyID::kColorScheme, AtomicString("light"));

    // Update lifecycle up until the pre-paint before the scrollbars paint is
    // invalidated.
    GetDocument().View()->UpdateLifecycleToCompositingInputsClean(DocumentUpdateReason::kTest);
    ...
    // Non root scrollbars should not change.
    ExpectEqAllScrollControlsNeedPaintInvalidation(non_root_scrollable_area, false);

    EXPECT_EQ(root_scrollable_area->UsedColorSchemeScrollbars(), mojom::blink::ColorScheme::kLight);
}

TEST_P(PaintLayerScrollableAreaTest, UsedColorSchemeRootScrollbarsInvalidateOnLightToNormalChange)
{
    USE_NON_OVERLAY_SCROLLBARS_OR_QUIT();
    ...
    GetDocument().documentElement()->SetInlineStyleProperty(CSSPropertyID::kColorScheme, AtomicString("normal"));
...
TEST_P(PaintLayerScrollableAreaTest, UsedColorSchemeRootScrollbarsUseCounterUpdated)
{
    USE_NON_OVERLAY_SCROLLBARS_OR_QUIT();
    ...
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kUsedColorSchemeRootScrollbarsDark));
}
...
TEST_P(PaintLayerScrollableAreaTest, NotScrollsOverflowWithScrollableScrollbar)
{
    USE_NON_OVERLAY_SCROLLBARS_OR_QUIT();
...
TEST_P(PaintLayerScrollableAreaTest, ScrollbarGutterBothEdgesWithHorizontalScrollbar)
{
    USE_NON_OVERLAY_SCROLLBARS_OR_QUIT();
    SetBodyInnerHTML(R"HTML(
    ...
    EXPECT_EQ(PhysicalRect(0, 0, 90, 105), scroll->LayoutContentRect(kExcludeScrollbars));
    EXPECT_EQ(PhysicalRect(0, 0, 120, 120), scroll->LayoutContentRect(kIncludeScrollbars));
    EXPECT_EQ(gfx::Size(320, 105), scroll->ContentsSize());
    EXPECT_EQ(PhysicalRect(35, 20, 320, 105), scroll->GetLayoutBox()->ScrollableOverflowRect());
    EXPECT_EQ(PhysicalRect(20, 20, 120, 105), scroll->GetLayoutBox()->OverflowClipRect(PhysicalOffset()));
    EXPECT_EQ(gfx::Point(), scroll->ScrollOrigin());
}

// TODO(crbug.com/340578714): The expectations match the current actual outputs
// which may not be fully correct.
TEST_P(PaintLayerScrollableAreaTest, ScrollbarGutterBothEdgesWithVerticalScrollbars)
{
    USE_NON_OVERLAY_SCROLLBARS_OR_QUIT();
    SetBodyInnerHTML(R"HTML(
    <div id="scroll" style="width: 100px; height: 100px; overflow: auto;
    ...
    auto* scroll = GetLayoutBoxByElementId("scroll")->GetScrollableArea();
    EXPECT_EQ(PhysicalRect(0, 0, 90, 120), scroll->LayoutContentRect(kExcludeScrollbars));
    EXPECT_EQ(PhysicalRect(0, 0, 120, 120), scroll->LayoutContentRect(kIncludeScrollbars));
    EXPECT_EQ(gfx::Size(90, 320), scroll->ContentsSize());
    EXPECT_EQ(PhysicalRect(35, 20, 90, 320), scroll->GetLayoutBox()->ScrollableOverflowRect());
    EXPECT_EQ(PhysicalRect(20, 20, 105, 120), scroll->GetLayoutBox()->OverflowClipRect(PhysicalOffset()));
    EXPECT_EQ(gfx::Point(), scroll->ScrollOrigin());
}

// TODO(crbug.com/340578714): The expectations match the current actual outputs
// which may not be fully correct.
TEST_P(PaintLayerScrollableAreaTest, ScrollbarGutterBothEdgesWithBothScrollbars)
{
    USE_NON_OVERLAY_SCROLLBARS_OR_QUIT();
    SetBodyInnerHTML(R"HTML(
    <div id="scroll" style="width: 100px; height: 100px; overflow: auto;
    ...
    EXPECT_EQ(PhysicalRect(35, 20, 320, 320), scroll->GetLayoutBox()->ScrollableOverflowRect());
    EXPECT_EQ(PhysicalRect(20, 20, 105, 105), scroll->GetLayoutBox()->OverflowClipRect(PhysicalOffset()));
    EXPECT_EQ(gfx::Point(), scroll->ScrollOrigin());
}

// TODO(crbug.com/340578714): The expectations match the current actual outputs
// which may not be fully correct.
TEST_P(PaintLayerScrollableAreaTest, ScrollbarGutterBothEdgesOverflowIntoGutter)
{
    USE_NON_OVERLAY_SCROLLBARS_OR_QUIT();
    SetBodyInnerHTML(R"HTML(
    <div id="scroll" style="width: 100px; height: 100px; overflow: auto;
    ...
    EXPECT_EQ(PhysicalRect(35, 20, 120, 120), scroll->GetLayoutBox()->ScrollableOverflowRect());
    EXPECT_EQ(PhysicalRect(20, 20, 120, 120), scroll->GetLayoutBox()->OverflowClipRect(PhysicalOffset()));
    EXPECT_EQ(gfx::Point(), scroll->ScrollOrigin());
}

// TODO(crbug.com/340578714): The expectations match the current actual outputs
// which may not be fully correct.
TEST_P(PaintLayerScrollableAreaTest, ScrollbarGutterBothEdgesRtlWithHorizontalScrollbar)
{
    USE_NON_OVERLAY_SCROLLBARS_OR_QUIT();
    SetBodyInnerHTML(R"HTML(
    <div id="scroll" style="width: 100px; height: 100px; overflow: auto;
    ...
    auto* scroll = GetLayoutBoxByElementId("scroll")->GetScrollableArea();
    EXPECT_EQ(PhysicalRect(200, 0, 90, 105), scroll->LayoutContentRect(kExcludeScrollbars));
    EXPECT_EQ(PhysicalRect(200, 0, 120, 120), scroll->LayoutContentRect(kIncludeScrollbars));
    EXPECT_EQ(gfx::Size(320, 105), scroll->ContentsSize());
    EXPECT_EQ(PhysicalRect(-195, 20, 320, 105), scroll->GetLayoutBox()->ScrollableOverflowRect());
    EXPECT_EQ(PhysicalRect(20, 20, 120, 105), scroll->GetLayoutBox()->OverflowClipRect(PhysicalOffset()));
...
// TODO(crbug.com/340578714): The expectations match the current actual outputs
// which may not be fully correct.
TEST_P(PaintLayerScrollableAreaTest, ScrollbarGutterBothEdgesRtlWithVerticalScrollbar)
{
    USE_NON_OVERLAY_SCROLLBARS_OR_QUIT();
    SetBodyInnerHTML(R"HTML(
    <div id="scroll" style="width: 100px; height: 100px; overflow: auto;
    ...
    EXPECT_EQ(gfx::Size(90, 320), scroll->ContentsSize());
    EXPECT_EQ(PhysicalRect(35, 20, 90, 320), scroll->GetLayoutBox()->ScrollableOverflowRect());
    EXPECT_EQ(PhysicalRect(35, 20, 105, 120), scroll->GetLayoutBox()->OverflowClipRect(PhysicalOffset()));
    EXPECT_EQ(gfx::Point(0, 0), scroll->ScrollOrigin());
}

// TODO(crbug.com/340578714): The expectations match the current actual outputs
// which may not be fully correct.
TEST_P(PaintLayerScrollableAreaTest, ScrollbarGutterBothEdgesRtlWithBothScrollbars)
{
    USE_NON_OVERLAY_SCROLLBARS_OR_QUIT();
    SetBodyInnerHTML(R"HTML(
    <div id="scroll" style="width: 100px; height: 100px; overflow: auto;
    ...
    auto* scroll = GetLayoutBoxByElementId("scroll")->GetScrollableArea();
    EXPECT_EQ(PhysicalRect(215, 0, 90, 105), scroll->LayoutContentRect(kExcludeScrollbars));
    EXPECT_EQ(PhysicalRect(215, 0, 120, 120), scroll->LayoutContentRect(kIncludeScrollbars));
    EXPECT_EQ(gfx::Size(320, 320), scroll->ContentsSize());
    EXPECT_EQ(PhysicalRect(-195, 20, 320, 320), scroll->GetLayoutBox()->ScrollableOverflowRect());
    EXPECT_EQ(PhysicalRect(35, 20, 105, 105), scroll->GetLayoutBox()->OverflowClipRect(PhysicalOffset()));
    EXPECT_EQ(gfx::Point(230, 0), scroll->ScrollOrigin());
}
...
TEST_F(PaintLayerScrollableAreaWithWebFrameTest, UpdateShouldAnimateScrollOnMainThread)
{
    ...
    scroller->SetInlineStyleProperty(CSSPropertyID::kOverflow, CSSValueID::kHidden);
    GetDocument().View()->UpdateAllLifecyclePhasesForTest();
    EXPECT_TRUE(scrollable_area->ShouldScrollOnMainThread());
    EXPECT_FALSE(box->FirstFragment().PaintProperties()->Scroll());
