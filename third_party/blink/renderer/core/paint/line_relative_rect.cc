...
LineRelativeRect LineRelativeRect::EnclosingRect(const gfx::RectF& rect)
{
    ...
    LogicalSize size { LayoutUnit::FromFloatCeil(rect.right()) - offset.line_left, LayoutUnit::FromFloatCeil(rect.bottom()) - offset.line_over };
    return { offset, size };
}
...
AffineTransform LineRelativeRect::ComputeRelativeToPhysicalTransform(WritingMode writing_mode) const
{
    ...
    return writing_mode != WritingMode::kSidewaysLr ? AffineTransform(0, 1, -1, 0, LineLeft() + LineOver() + BlockSize(), LineOver() - LineLeft())
...
void LineRelativeRect::AdjustLineStartToInkOverflow(const FragmentItem& fragment)
{
...
void LineRelativeRect::AdjustLineEndToInkOverflow(const FragmentItem& fragment)
{
    WritingMode writing_mode = fragment.GetWritingMode();
    // Offset from the inline-start position of `fragment`.
