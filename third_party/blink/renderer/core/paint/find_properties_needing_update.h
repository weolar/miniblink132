...
public:
    FindPropertiesNeedingUpdateScope(const LayoutObject& object, const FragmentData& fragment_data, bool force_subtree_update)
        : object_(object)
        ...
        , needed_forced_subtree_update_(force_subtree_update)
    {
        ...
        if (fragment_data_.HasLocalBorderBoxProperties()) {
            original_local_border_box_properties_ = fragment_data_.LocalBorderBoxProperties();
        }
    }
    ...
    ~FindPropertiesNeedingUpdateScope()
    {
        ...
        if (!original_local_border_box_properties_.IsInitialized() && fragment_data_.HasLocalBorderBoxProperties()) {
            const auto object_border_box = fragment_data_.LocalBorderBoxProperties();
            DCHECK_EQ(&original_local_border_box_properties_.Transform(), &object_border_box.Transform()) << object_.DebugName();
            DCHECK_EQ(&original_local_border_box_properties_.Clip(), &object_border_box.Clip()) << object_.DebugName();
            DCHECK_EQ(&original_local_border_box_properties_.Effect(), &object_border_box.Effect()) << object_.DebugName();
        } else {
            DCHECK_EQ(original_local_border_box_properties_.IsInitialized(), fragment_data_.HasLocalBorderBoxProperties()) << object_.DebugName();
        }
...
private:
    ...
    PropertyTreeStateOrAlias original_local_border_box_properties_ { PropertyTreeState::kUninitialized };
