...
public:
    ...
    SVGObjectPainter(const LayoutObject& layout_object, const SvgContextPaints* context_paints)
        : layout_object_(layout_object)
        , context_paints_(context_paints)
    {
    ...
    // using |paint_flags|.
    bool PreparePaint(PaintFlags, const ComputedStyle&, LayoutSVGResourceMode, cc::PaintFlags& paint_flags,
        const AffineTransform* additional_paint_server_transform = nullptr);

    void PaintResourceSubtree(GraphicsContext&, PaintFlags additional_flags = PaintFlag::kNoFlag);

    SvgContextPaints::ContextPaint ResolveContextPaint(const SVGPaint& initial_paint);
    std::optional<AffineTransform> ResolveContextTransform(const SVGPaint& initial_paint, const AffineTransform* additional_paint_server_transform);
