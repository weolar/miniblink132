...
static std::optional<AffineTransform> SetupNonScalingStrokeContext(const LayoutSVGShape& layout_svg_shape, GraphicsContextStateSaver& state_saver)
{
    const AffineTransform& non_scaling_stroke_transform = layout_svg_shape.NonScalingStrokeTransform();
...
{
    if (paint_info.phase != PaintPhase::kForeground || layout_svg_shape_.StyleRef().Visibility() != EVisibility::kVisible || layout_svg_shape_.IsShapeEmpty()) {
        return;
    }

    if (SVGModelObjectPainter::CanUseCullRect(layout_svg_shape_.StyleRef())) {
        if (!paint_info.GetCullRect().IntersectsTransformed(layout_svg_shape_.LocalSVGTransform(), layout_svg_shape_.VisualRectInLocalSVGCoordinates()))
            return;
    }
...

    {
        ...
        SVGModelObjectPainter::RecordHitTestData(layout_svg_shape_, content_paint_info);
        SVGModelObjectPainter::RecordRegionCaptureData(layout_svg_shape_, content_paint_info);
        if (!DrawingRecorder::UseCachedDrawingIfPossible(content_paint_info.context, layout_svg_shape_, content_paint_info.phase)) {
            SVGDrawingRecorder recorder(content_paint_info.context, layout_svg_shape_, content_paint_info.phase);
...
void SVGShapePainter::PaintShape(const PaintInfo& paint_info)
{
    ...
    const bool should_anti_alias = style.ShapeRendering() != EShapeRendering::kCrispedges && style.ShapeRendering() != EShapeRendering::kOptimizespeed;
    ...
    for (unsigned i = 0; i < 3; i++) {
        ...
        case PT_FILL: {
            if (SVGObjectPainter::HasFill(style, paint_info.GetSvgContextPaints())) {
                cc::PaintFlags fill_flags;
                if (!SVGObjectPainter(layout_svg_shape_, paint_info.GetSvgContextPaints())
                         .PreparePaint(paint_info.GetPaintFlags(), style, kApplyToFillMode, fill_flags)) {
                    break;
                }
        ...
        case PT_STROKE:
            if (SVGObjectPainter::HasVisibleStroke(style, paint_info.GetSvgContextPaints())) {
                ...
                if (layout_svg_shape_.HasNonScalingStroke()) {
                    ...
                    // host transform.
                    non_scaling_transform = SetupNonScalingStrokeContext(layout_svg_shape_, state_saver);
                ...
                if (!SVGObjectPainter(layout_svg_shape_, paint_info.GetSvgContextPaints())
                         .PreparePaint(paint_info.GetPaintFlags(), style, kApplyToStrokeMode, stroke_flags, base::OptionalToPtr(non_scaling_transform))) {
                    break;
                }
                stroke_flags.setAntiAlias(should_anti_alias);

                StrokeData stroke_data;
                SVGLayoutSupport::ApplyStrokeStyleToStrokeData(stroke_data, style, layout_svg_shape_, layout_svg_shape_.DashScaleFactor());
...
public:
    ...
    {
        path_.setFillType(saved_fill_type_);
    }
...
void SVGShapePainter::FillShape(GraphicsContext& context, const cc::PaintFlags& flags, WindRule wind_rule)
{
    const SkPathFillType sk_fill_type = WebCoreWindRuleToSkFillType(wind_rule);
    AutoDarkMode auto_dark_mode(PaintAutoDarkMode(layout_svg_shape_.StyleRef(), DarkModeFilter::ElementRole::kSVG));
    switch (layout_svg_shape_.GetGeometryType()) {
    case LayoutSVGShape::GeometryType::kRectangle:
        context.DrawRect(gfx::RectFToSkRect(layout_svg_shape_.ObjectBoundingBox()), flags, auto_dark_mode);
        break;
    case LayoutSVGShape::GeometryType::kCircle:
    case LayoutSVGShape::GeometryType::kEllipse:
        context.DrawOval(gfx::RectFToSkRect(layout_svg_shape_.ObjectBoundingBox()), flags, auto_dark_mode);
        break;
    default: {
        DCHECK(layout_svg_shape_.HasPath());
        PathWithTemporaryWindingRule path_with_winding(layout_svg_shape_.GetPath(), sk_fill_type);
...
void SVGShapePainter::StrokeShape(GraphicsContext& context, const cc::PaintFlags& flags)
{
    AutoDarkMode auto_dark_mode(PaintAutoDarkMode(layout_svg_shape_.StyleRef(), DarkModeFilter::ElementRole::kSVG));

    // Remap all geometry types to 'path' when non-scaling-stroke is in effect.
    LayoutSVGShape::GeometryType geometry_type = layout_svg_shape_.GetGeometryType();
    if (layout_svg_shape_.HasNonScalingStroke()) {
...
{
    const Vector<MarkerPosition>* marker_positions = layout_svg_shape_.MarkerPositions();
    if (!marker_positions || marker_positions->empty())
        return;
    SVGResourceClient* client = SVGResources::GetClient(layout_svg_shape_);
    const ComputedStyle& style = layout_svg_shape_.StyleRef();
    auto* marker_start = GetSVGResourceAsType<LayoutSVGResourceMarker>(*client, style.MarkerStartResource());
    auto* marker_mid = GetSVGResourceAsType<LayoutSVGResourceMarker>(*client, style.MarkerMidResource());
    auto* marker_end = GetSVGResourceAsType<LayoutSVGResourceMarker>(*client, style.MarkerEndResource());
    ...
    for (const MarkerPosition& marker_position : *marker_positions) {
        if (LayoutSVGResourceMarker* marker = marker_position.SelectMarker(marker_start, marker_mid, marker_end)) {
            PaintMarker(paint_info, *marker, marker_position, stroke_width);
        }
    }
}

void SVGShapePainter::PaintMarker(const PaintInfo& paint_info, LayoutSVGResourceMarker& marker, const MarkerPosition& position, float stroke_width)
{
    ...
    AffineTransform transform = marker.MarkerTransformation(position, stroke_width);
    ...
    SvgContextPaints marker_context_paints(object_painter.ResolveContextPaint(layout_svg_shape_.StyleRef().FillPaint()),
        object_painter.ResolveContextPaint(layout_svg_shape_.StyleRef().StrokePaint()), transform);
    PaintInfo marker_paint_info(
        builder.Context(), CullRect::Infinite(), paint_info.phase, paint_info.DescendantPaintingBlocked(), paint_info.GetPaintFlags(), &marker_context_paints);
