...
bool IsMultipleFieldsTemporalInput(FormControlType type)
{
    ...
    return type == FormControlType::kInputDate || type == FormControlType::kInputDatetimeLocal || type == FormControlType::kInputMonth
        || type == FormControlType::kInputTime || type == FormControlType::kInputWeek;
#else
    return false;
#endif
}
...
#define COUNT_APPEARANCE(doc, feature) doc.CountUse(WebFeature::kCSSValueAppearance##feature##Rendered)
...
void CountAppearanceTextFieldPart(const Element& element)
{
    if (auto* input = DynamicTo<HTMLInputElement>(element)) {
        ...
        if (type == FormControlType::kInputSearch) {
            UseCounter::Count(element.GetDocument(), WebFeature::kCSSValueAppearanceTextFieldForSearch);
        } else if (input->IsTextField()) {
            UseCounter::Count(element.GetDocument(), WebFeature::kCSSValueAppearanceTextFieldForTextField);
        } else if (IsMultipleFieldsTemporalInput(type)) {
            UseCounter::Count(element.GetDocument(), WebFeature::kCSSValueAppearanceTextFieldForTemporalRendered);
        }
    }
}

// Returns true; Needs CSS painting and/or PaintBorderOnly().
bool ThemePainter::Paint(const LayoutObject& o, const PaintInfo& paint_info, const gfx::Rect& r)
{
    ...
    if (part == kButtonPart) {
        ...
        } else if (auto* input_element = DynamicTo<HTMLInputElement>(element); input_element && input_element->IsTextButton()) {
            // Text buttons (type=button, reset, submit) has
            // -webkit-appearance:push-button by default.
            UseCounter::Count(doc, WebFeature::kCSSValueAppearanceButtonForOtherButtons);
        }
...
bool ThemePainter::PaintBorderOnly(const Node* node, const ComputedStyle& style, const PaintInfo& paint_info, const gfx::Rect& r)
{
...
bool ThemePainter::PaintDecorations(const Node* node, const Document& document, const ComputedStyle& style, const PaintInfo& paint_info, const gfx::Rect& r)
{
    DCHECK(node);
    // Call the appropriate paint method based off the appearance value.
    switch (style.EffectiveAppearance()) {
    case kMenulistButtonPart:
        ...
        return PaintMenuListButton(*To<Element>(node), document, style, paint_info, r);
    case kTextFieldPart:
    case kTextAreaPart:
    case kCheckboxPart:
    case kRadioPart:
...
void ThemePainter::PaintSliderTicks(const LayoutObject& o, const PaintInfo& paint_info, const gfx::Rect& rect)
{
    auto* input = DynamicTo<HTMLInputElement>(o.GetNode());
    if (!input)
        return;

    if (input->FormControlType() != FormControlType::kInputRange || !input->UserAgentShadowRoot()->HasChildren()) {
        return;
    }
    ...
    bool is_slider_vertical = RuntimeEnabledFeatures::NonStandardAppearanceValueSliderVerticalEnabled() && part == kSliderVerticalPart;
    ...
    gfx::Size thumb_size;
    LayoutObject* thumb_layout_object = input->UserAgentShadowRoot()->getElementById(shadow_element_names::kIdSliderThumb)->GetLayoutObject();
    ...
    LayoutObject* track_layout_object = input->UserAgentShadowRoot()->getElementById(shadow_element_names::kIdSliderTrack)->GetLayoutObject();
    if (track_layout_object && track_layout_object->IsBox()) {
        track_bounds = gfx::Rect(ToCeiledPoint(track_layout_object->FirstFragment().PaintOffset()), ToFlooredSize(To<LayoutBox>(track_layout_object)->Size()));
    }

    const float tick_offset_from_center = LayoutTheme::GetTheme().SliderTickOffsetFromTrackCenter() * zoom_factor;
    ...
    if (is_horizontal) {
        ...
        tick_rect.set_y(floor(rect.y() + rect.height() / 2.0 + tick_offset_from_center));
        tick_region_side_margin = track_bounds.x() + (thumb_size.width() - tick_inline_size) / 2.0;
        ...
        const float tick_x = (style.IsHorizontalTypographicMode() && writing_direction.LineUnder() == PhysicalDirection::kLeft)
            ? (slider_center - tick_offset_from_center - tick_block_size)
            : (slider_center + tick_offset_from_center);
        tick_rect.set_x(floor(tick_x));
        tick_region_side_margin = track_bounds.y() + (thumb_size.height() - tick_inline_size) / 2.0;
    ...
    if (is_horizontal || is_writing_mode_vertical) {
        ...
        flip_tick_direction = inline_end == PhysicalDirection::kLeft || inline_end == PhysicalDirection::kUp;
    }
    for (unsigned i = 0; HTMLOptionElement* option_element = options->Item(i); i++) {
        String value = option_element->value();
        if (option_element->IsDisabledFormControl() || value.empty())
            continue;
        if (!input->IsValidValue(value))
            continue;
        double parsed_value = ParseToDoubleForNumberType(input->SanitizeValue(value));
        double tick_fraction = (parsed_value - min) / (max - min);
        double tick_ratio = flip_tick_direction ? 1.0 - tick_fraction : tick_fraction;
        double tick_position = round(tick_region_side_margin + tick_region_width * tick_ratio);
        if (is_horizontal)
            tick_rect.set_x(tick_position);
        else
            tick_rect.set_y(tick_position);
        paint_info.context.FillRect(tick_rect, o.ResolveColor(GetCSSPropertyColor()), PaintAutoDarkMode(style, DarkModeFilter::ElementRole::kBackground));
    }
}
