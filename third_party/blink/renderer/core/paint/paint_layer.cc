...
inline PhysicalRect PhysicalVisualOverflowRectAllowingUnset(const LayoutBoxModelObject& layout_object)
{
#if DCHECK_IS_ON()
...
{
    if (const auto* layout_inline = DynamicTo<LayoutInline>(layer.GetLayoutObject())) {
...
void PaintLayer::Destroy()
{
    ...
    if (resource_info_) {
        ...
        if (auto* reference_clip = DynamicTo<ReferenceClipPathOperation>(style.ClipPath()))
            reference_clip->RemoveClient(*resource_info_);
        if (auto* reference_offset = DynamicTo<ReferenceOffsetPathOperation>(style.OffsetPath())) {
...
const PaintLayer* PaintLayer::ContainingScrollContainerLayer(bool* is_fixed_to_view) const
{
    bool is_fixed = GetLayoutObject().IsFixedPositioned();
    for (const PaintLayer* container = ContainingLayer(); container; container = container->ContainingLayer()) {
...
void PaintLayer::UpdateTransform()
{
    if (gfx::Transform* transform = Transform()) {
        ...
        box->StyleRef().ApplyTransform(*transform, box, reference_box, ComputedStyle::kIncludeTransformOperations, ComputedStyle::kIncludeTransformOrigin,
            ComputedStyle::kIncludeMotionPath, ComputedStyle::kIncludeIndependentTransformProperties);
    }
}

void PaintLayer::UpdateTransformAfterStyleChange(StyleDifference diff, const ComputedStyle* old_style, const ComputedStyle& new_style)
{
    ...
    if (had_transform == has_transform && old_style && !diff.TransformDataChanged()) {
        return;
    }
...
void PaintLayer::MarkAncestorChainForFlagsUpdate(DescendantDependentFlagsUpdateFlag flag)
{
#if DCHECK_IS_ON()
    DCHECK(flag == kDoesNotNeedDescendantDependentUpdate || !layout_object_->GetDocument().View()->IsUpdatingDescendantDependentFlags());
#endif
    for (PaintLayer* layer = this; layer; layer = layer->Parent()) {
        if (layer->needs_descendant_dependent_flags_update_ && layer->GetLayoutObject().NeedsPaintPropertyUpdate())
            break;
...
void PaintLayer::UpdateDescendantDependentFlags()
{
    if (needs_descendant_dependent_flags_update_) {
        bool old_has_non_isolated_descendant_with_blend_mode = has_non_isolated_descendant_with_blend_mode_;
        has_visible_self_painting_descendant_ = false;
        has_non_isolated_descendant_with_blend_mode_ = false;
        has_fixed_position_descendant_ = false;
        has_non_contained_absolute_position_descendant_ = false;
        ...
        bool can_contain_abs = GetLayoutObject().CanContainAbsolutePositionObjects();

        auto* first_child = [this]() -> PaintLayer* {
            if (GetLayoutObject().ChildPrePaintBlockedByDisplayLock()) {
                GetLayoutObject().GetDisplayLockContext()->NotifyCompositingDescendantDependentFlagUpdateWasBlocked();
                return nullptr;
            }
            return FirstChild();
        }();
        ...
        for (PaintLayer* child = first_child; child; child = child->NextSibling()) {
            ...
            if ((child->has_visible_content_ && child->IsSelfPaintingLayer()) || child->has_visible_self_painting_descendant_)
                has_visible_self_painting_descendant_ = true;

            has_non_isolated_descendant_with_blend_mode_
                |= (!child->GetLayoutObject().IsStackingContext() && child->HasNonIsolatedDescendantWithBlendMode()) || child_style.HasBlendMode();

            has_fixed_position_descendant_ |= child->HasFixedPositionDescendant() || child_style.GetPosition() == EPosition::kFixed;

            if (!can_contain_abs) {
                has_non_contained_absolute_position_descendant_
                    |= (child->HasNonContainedAbsolutePositionDescendant() || child_style.GetPosition() == EPosition::kAbsolute);
            }
            ...
            if (!has_stacked_descendant_in_current_stacking_context_) {
                if (child->GetLayoutObject().IsStacked()) {
                    ...
                    has_stacked_descendant_in_current_stacking_context_ = child->has_stacked_descendant_in_current_stacking_context_;
                }
            }

            has_self_painting_layer_descendant_
                = has_self_painting_layer_descendant_ || child->HasSelfPaintingLayerDescendant() || child->IsSelfPaintingLayer();
        }

        // See SetInvisibleForPositionVisibility() for explanation for
        // descendant_needs_check_position_visibility_.
        if (InvisibleForPositionVisibility() && !GetLayoutObject().IsStackingContext() && has_self_painting_layer_descendant_) {
            AncestorStackingContext()->descendant_needs_check_position_visibility_ = true;
        }

        UpdateStackingNode();

        if (old_has_non_isolated_descendant_with_blend_mode != static_cast<bool>(has_non_isolated_descendant_with_blend_mode_)) {
        ...
        if (IsSelfPaintingLayer() && needs_visual_overflow_recalc_) {
            PhysicalRect old_visual_rect = PhysicalVisualOverflowRectAllowingUnset(GetLayoutObject());
    ...
    if (GetLayoutObject().StyleRef().Visibility() == EVisibility::kVisible) {
        ...
        while (r) {
            if (r->StyleRef().Visibility() == EVisibility::kVisible && (!r->HasLayer() || !r->EnclosingLayer()->IsSelfPaintingLayer())) {
                has_visible_content_ = true;
                break;
            }
            LayoutObject* layout_object_first_child = r->SlowFirstChild();
            if (layout_object_first_child && (!r->HasLayer() || !r->EnclosingLayer()->IsSelfPaintingLayer())) {
...
void PaintLayer::Update3DTransformedDescendantStatus()
{
    ...
    while (PaintLayer* child_layer = iterator.Next()) {
        ...
        if (child_layer->Preserves3D() && (child_layer->Has3DTransform() || child_layer->Has3DTransformedDescendant()))
            child_has3d = true;
...
PaintLayer* PaintLayer::ContainingLayer() const
{
    ...
    if (layout_object.IsOutOfFlowPositioned()) {
        ...
        auto can_contain_this_layer
            = layout_object.IsFixedPositioned() ? &LayoutObject::CanContainFixedPositionObjects : &LayoutObject::CanContainAbsolutePositionObjects;
    ...
    if ((!Parent() || Parent()->GetLayoutObject().IsLayoutBlock()) && !layout_object.IsColumnSpanAll())
        return Parent();
...
{
    for (PaintLayer* ancestor = Parent(); ancestor; ancestor = ancestor->Parent()) {
...
bool PaintLayer::HasNonIsolatedDescendantWithBlendMode() const
{
    ...
    if (GetLayoutObject().IsSVGRoot()) {
        return To<LayoutSVGRoot>(GetLayoutObject()).HasNonIsolatedBlendingDescendants();
    }
    return false;
}
...
void PaintLayer::AddChild(PaintLayer* child, PaintLayer* before_child)
{
    ...
    PaintLayer* prev_sibling = before_child ? before_child->PreviousSibling() : LastChild();
    if (prev_sibling) {
...
void PaintLayer::RemoveChild(PaintLayer* old_child)
{
    ...
    if (old_child->has_visible_content_ || old_child->has_visible_self_painting_descendant_)
        MarkAncestorChainForFlagsUpdate();
}

void PaintLayer::RemoveOnlyThisLayerAfterStyleChange(const ComputedStyle* old_style)
{
    if (!parent_)
        return;
    ...
    if (IsSelfPaintingLayer()) {
        if (PaintLayer* enclosing_self_painting_layer = parent_->EnclosingSelfPaintingLayer())
...
void PaintLayer::InsertOnlyThisLayerAfterStyleChange()
{
    if (!parent_ && GetLayoutObject().Parent()) {
        ...
        DCHECK(parent_layer);
        PaintLayer* before_child = GetLayoutObject().Parent()->FindNextLayer(parent_layer, &GetLayoutObject());
        parent_layer->AddChild(this, before_child);
    }

    // Remove all descendant layers from the hierarchy and add them to the new
    // position.
    for (LayoutObject* curr = GetLayoutObject().SlowFirstChild(); curr; curr = curr->NextSibling())
...
void PaintLayer::UpdateStackingNode()
{
    ...
    bool needs_stacking_node = has_stacked_descendant_in_current_stacking_context_ && GetLayoutObject().IsStackingContext();
...
void PaintLayer::AppendSingleFragmentForHitTesting(
    PaintLayerFragments& fragments, const PaintLayerFragment* container_fragment, ShouldRespectOverflowClipType respect_overflow_clip) const
{
    ...
    ClipRectsContext clip_rects_context(this, fragment.fragment_data, kExcludeOverlayScrollbarSizeForHitTesting, respect_overflow_clip);
    Clipper().CalculateRects(clip_rects_context, *fragment.fragment_data, fragment.layer_offset, fragment.background_rect, fragment.foreground_rect);
...
void PaintLayer::CollectFragments(PaintLayerFragments& fragments, const PaintLayer* root_layer, ShouldRespectOverflowClipType respect_overflow_clip,
    const FragmentData* root_fragment_arg) const
{
    PaintLayerFragment fragment;
    const auto& first_root_fragment_data = root_layer->GetLayoutObject().FirstFragment();
    ...
    bool multiple_fragments_allowed = layout_box_with_fragments || CanPaintMultipleFragments(GetLayoutObject());
    ...
    for (FragmentDataIterator iterator(GetLayoutObject()); !iterator.IsDone(); ++iterator, physical_fragment_idx++) {
        ...
        if (root_fragment_arg) {
            ...
            if (!root_fragment_arg->ContentsProperties().Transform().IsAncestorOf(fragment_data->LocalBorderBoxProperties().Transform())) {
        ...
        ClipRectsContext clip_rects_context(root_layer, root_fragment_data, kExcludeOverlayScrollbarSizeForHitTesting, respect_overflow_clip, PhysicalOffset());

        Clipper().CalculateRects(clip_rects_context, *fragment_data, fragment.layer_offset, fragment.background_rect, fragment.foreground_rect);

        fragment.fragment_data = fragment_data;

        if (layout_box_with_fragments) {
            fragment.physical_fragment = layout_box_with_fragments->GetPhysicalFragment(physical_fragment_idx);
...
PaintLayer::HitTestRecursionData::HitTestRecursionData(
    const PhysicalRect& rect_arg, const HitTestLocation& location_arg, const HitTestLocation& original_location_arg)
    : rect(rect_arg)
...
bool PaintLayer::HitTest(const HitTestLocation& hit_test_location, HitTestResult& result, const PhysicalRect& hit_test_area)
{
    ...
    HitTestRecursionData recursion_data(hit_test_area, hit_test_location, hit_test_location);
    PaintLayer* inside_layer = HitTestLayer(*this, /*container_fragment*/ nullptr, result, recursion_data);
...
static double ComputeZOffset(const HitTestingTransformState& transform_state)
{
    ...
    gfx::Point3F backmapped_point = transform_state.AccumulatedTransform().MapPoint(gfx::Point3F(target_point));
    return backmapped_point.z();
}

HitTestingTransformState PaintLayer::CreateLocalTransformState(const PaintLayer& transform_container, const FragmentData& transform_container_fragment,
    const FragmentData& local_fragment, const HitTestRecursionData& recursion_data, const HitTestingTransformState* container_transform_state) const
{
    ...
    HitTestingTransformState transform_state = container_transform_state
        ? *container_transform_state
        : HitTestingTransformState(
            recursion_data.location.TransformedPoint(), recursion_data.location.TransformedRect(), gfx::QuadF(gfx::RectF(recursion_data.rect)));
    ...
    if (container_transform_state
        && (!transform_container.Preserves3D() || &transform_container.GetLayoutObject() != GetLayoutObject().NearestAncestorForElement())) {
    ...
    const auto* container_transform = &transform_container_fragment.LocalBorderBoxProperties().Transform();
    ...
    transform_state.Translate(gfx::Vector2dF(-transform_container_fragment.PaintOffset()));
    transform_state.ApplyTransform(GeometryMapper::SourceToDestinationProjection(local_fragment.PreTransform(), *container_transform));
...
static bool IsHitCandidateForDepthOrder(const PaintLayer* hit_layer, bool can_depth_sort, double* z_offset, const HitTestingTransformState* transform_state)
{
    if (!hit_layer)
        return false;
    ...
    DCHECK(!z_offset || transform_state || hit_layer->GetLayoutObject().IsSVGForeignObject());
...
static bool IsHitCandidateForStopNode(const LayoutObject& candidate, const LayoutObject* stop_node)
{
    return !stop_node || (&candidate == stop_node) || !candidate.IsDescendantOf(stop_node);
}
...
PaintLayer* PaintLayer::HitTestLayer(const PaintLayer& transform_container, const PaintLayerFragment* container_fragment, HitTestResult& result,
    const HitTestRecursionData& recursion_data, bool applied_transform, HitTestingTransformState* container_transform_state, double* z_offset,
    bool overflow_controls_only)
{
    const FragmentData* container_fragment_data = container_fragment ? container_fragment->fragment_data : nullptr;
    ...
    DCHECK_GE(layout_object.GetDocument().Lifecycle().GetState(), DocumentLifecycle::kPrePaintClean);

    if (layout_object.NeedsLayout() && !layout_object.ChildLayoutBlockedByDisplayLock()) [[unlikely]] {
    ...
    if ((result.GetHitTestRequest().GetType() & HitTestRequest::kIgnoreZeroOpacityObjects) && !layout_object.HasNonZeroEffectiveOpacity()) {
        return nullptr;
    }

    std::optional<CheckAncestorPositionVisibilityScope> check_position_visibility_scope;
    if (InvisibleForPositionVisibility() || HasAncestorInvisibleForPositionVisibility()) {
        return nullptr;
    }
    ...
    // each hit test.
    if (auto* transition = ViewTransitionUtils::GetTransition(layout_object.GetDocument())) {
    ...
    if (layout_object.IsGlobalRootScroller()) {
        ...
        if (scrollable_area_) {
            gfx::Point point = scrollable_area_->ConvertFromRootFrameToVisualViewport(ToRoundedPoint(recursion_data.location.Point()));

            DCHECK(GetLayoutBox());
            if (GetLayoutBox()->HitTestOverflowControl(result, HitTestLocation(point), PhysicalOffset()))
                return this;
        }
    }
    ...
    if (!layout_object.IsSVGForeignObject() &&
        ...
        if (const auto* properties = layout_object.FirstFragment().PaintProperties()) {
            if (properties->HasCSSTransformPropertyNode() || properties->Perspective())
                use_transform = true;
        }
    }
    ...
    if (use_transform && !applied_transform) {
        return HitTestTransformedLayerInFragments(
            transform_container, container_fragment, result, recursion_data, container_transform_state, z_offset, overflow_controls_only, clip_behavior);
    }
    ...
    // hit test).
    bool is_occlusion_test = result.GetHitTestRequest().GetType() & HitTestRequest::kHitTestVisualOverflow;
    if (!is_occlusion_test && layout_object.HasClipPath() && HitTestClippedOutByClipPath(transform_container, recursion_data.location)) {
        return nullptr;
    }
    ...
    if (applied_transform) {
        ...
        const FragmentData* local_fragment_for_transform_state = iterator.GetFragmentData();
        ...
        if (container_fragment_data) {
            ...
            const auto& container_transform = container_fragment_data->ContentsProperties().Transform();
            while (!iterator.IsDone()) {
                // Find the first local fragment that is a descendant of
                // container_fragment.
                if (container_transform.IsAncestorOf(local_fragment_for_transform_state->LocalBorderBoxProperties().Transform())) {
                    break;
                }
                ++iterator;
            ...
            container_fragment_for_transform_state = &container_layout_object.FirstFragment();
        }
        storage = CreateLocalTransformState(
            transform_container, *container_fragment_for_transform_state, *local_fragment_for_transform_state, recursion_data, container_transform_state);
        local_transform_state = &*storage;
    }

    // Check for hit test on backface if backface-visibility is 'hidden'
    if (local_transform_state && layout_object.StyleRef().BackfaceVisibility() == EBackfaceVisibility::kHidden
    ...
    if (recursion_data.intersects_location) {
        if (applied_transform) {
            ...
            AppendSingleFragmentForHitTesting(layer_fragments, container_fragment, clip_behavior);
        } else {
            CollectFragments(layer_fragments, &transform_container, clip_behavior, container_fragment_data);
        }
        ...
        if (scrollable_area_ && layer_fragments[0].background_rect.Intersects(recursion_data.location)
    ...
    // the lowest z-index.
    PaintLayer* hit_layer = HitTestChildren(kPositiveZOrderChildren, transform_container, container_fragment, result, recursion_data, container_transform_state,
        z_offset_for_descendants_ptr, z_offset, local_transform_state, depth_sort_descendants);
    if (hit_layer) {
    ...
    hit_layer = HitTestChildren(kNormalFlowChildren, transform_container, container_fragment, result, recursion_data, container_transform_state,
        z_offset_for_descendants_ptr, z_offset, local_transform_state, depth_sort_descendants);
    if (hit_layer) {
        if (!depth_sort_descendants)
            return hit_layer;
    ...
    if (recursion_data.intersects_location) {
        ...
        if (IsSelfPaintingLayer() && !layout_object.ChildPaintBlockedByDisplayLock()) {
            // Hit test with a temporary HitTestResult, because we only want to commit
            // to 'result' if we know we're frontmost.
            STACK_UNINITIALIZED HitTestResult temp_result(result.GetHitTestRequest(), recursion_data.original_location);
            bool inside_fragment_foreground_rect = false;

            if (HitTestForegroundForFragments(layer_fragments, temp_result, recursion_data.location, inside_fragment_foreground_rect)
                && IsHitCandidateForDepthOrder(this, false, z_offset_for_contents_ptr, local_transform_state)
            ...
            } else if (inside_fragment_foreground_rect && result.GetHitTestRequest().ListBased() && IsHitCandidateForStopNode(GetLayoutObject(), stop_node)) {
                result.Append(temp_result);
            }
        }
    }
    ...
    hit_layer = HitTestChildren(kNegativeZOrderChildren, transform_container, container_fragment, result, recursion_data, container_transform_state,
        z_offset_for_descendants_ptr, z_offset, local_transform_state, depth_sort_descendants);
    if (hit_layer) {
        if (!depth_sort_descendants)
            return hit_layer;
    ...
    if (recursion_data.intersects_location && IsSelfPaintingLayer()) {
        ...
        if (HitTestFragmentsWithPhase(
                layer_fragments, temp_result, recursion_data.location, HitTestPhase::kSelfBlockBackground, inside_fragment_background_rect)
            && IsHitCandidateForDepthOrder(this, false, z_offset_for_contents_ptr, local_transform_state)
            && IsHitCandidateForStopNode(GetLayoutObject(), stop_node)) {
            if (result.GetHitTestRequest().ListBased())
                result.Append(temp_result);
        ...
        if (inside_fragment_background_rect && result.GetHitTestRequest().ListBased() && IsHitCandidateForStopNode(GetLayoutObject(), stop_node)) {
            result.Append(temp_result);
        }
    }
...
bool PaintLayer::HitTestForegroundForFragments(
    const PaintLayerFragments& layer_fragments, HitTestResult& result, const HitTestLocation& hit_test_location, bool& inside_clip_rect) const
{
    if (HitTestFragmentsWithPhase(layer_fragments, result, hit_test_location, HitTestPhase::kForeground, inside_clip_rect)) {
        return true;
    }
    if (inside_clip_rect && HitTestFragmentsWithPhase(layer_fragments, result, hit_test_location, HitTestPhase::kFloat, inside_clip_rect)) {
        return true;
    }
    if (inside_clip_rect
        && HitTestFragmentsWithPhase(layer_fragments, result, hit_test_location, HitTestPhase::kDescendantBlockBackgrounds, inside_clip_rect)) {
        return true;
    }
    return false;
}

bool PaintLayer::HitTestFragmentsWithPhase(const PaintLayerFragments& layer_fragments, HitTestResult& result, const HitTestLocation& hit_test_location,
    HitTestPhase phase, bool& inside_clip_rect) const
{
    ...
    for (int i = layer_fragments.size() - 1; i >= 0; --i) {
        ...
        const ClipRect& bounds = phase == HitTestPhase::kSelfBlockBackground ? fragment.background_rect : fragment.foreground_rect;
        if (!bounds.Intersects(hit_test_location))
            continue;

        inside_clip_rect = true;

        if (GetLayoutObject().IsLayoutInline() && GetLayoutObject().CanTraversePhysicalFragments()) [[unlikely]] {
            ...
            HitTestLocation location_for_fragment(hit_test_location, fragment.fragment_idx);
            if (HitTestFragmentWithPhase(result, fragment.physical_fragment, fragment.layer_offset, location_for_fragment, phase))
                return true;
        } else if (HitTestFragmentWithPhase(result, fragment.physical_fragment, fragment.layer_offset, hit_test_location, phase)) {
            return true;
        }
    }
...
PaintLayer* PaintLayer::HitTestTransformedLayerInFragments(const PaintLayer& transform_container, const PaintLayerFragment* container_fragment,
    HitTestResult& result, const HitTestRecursionData& recursion_data, HitTestingTransformState* container_transform_state, double* z_offset,
    bool overflow_controls_only, ShouldRespectOverflowClipType clip_behavior)
{
    const FragmentData* container_fragment_data = container_fragment ? container_fragment->fragment_data : nullptr;
    PaintLayerFragments fragments;
    ClearCollectionScope<PaintLayerFragments> scope(&fragments);

    CollectFragments(fragments, &transform_container, clip_behavior, container_fragment_data);
    ...
    for (const auto& fragment : fragments) {
        ...
        PaintLayer* hit_layer = HitTestLayerByApplyingTransform(
            transform_container, container_fragment, fragment, result, recursion_data, container_transform_state, z_offset, overflow_controls_only);
        if (hit_layer)
            return hit_layer;
    }
...
PaintLayer* PaintLayer::HitTestLayerByApplyingTransform(const PaintLayer& transform_container, const PaintLayerFragment* container_fragment,
    const PaintLayerFragment& local_fragment, HitTestResult& result, const HitTestRecursionData& recursion_data, HitTestingTransformState* root_transform_state,
    double* z_offset, bool overflow_controls_only, const PhysicalOffset& translation_offset)
{
    // Create a transform state to accumulate this transform.
    HitTestingTransformState new_transform_state = CreateLocalTransformState(transform_container,
        container_fragment ? *container_fragment->fragment_data : transform_container.GetLayoutObject().FirstFragment(), *local_fragment.fragment_data,
        recursion_data, root_transform_state);
    ...
    HitTestRecursionData new_recursion_data(bounds_of_mapped_area, *new_location, recursion_data.original_location);
    ...
    const auto* new_container_fragment = GetLayoutObject().IsFragmented() ? &local_fragment : nullptr;
    return HitTestLayer(*this, new_container_fragment, result, new_recursion_data,
        /*applied_transform*/ true, &new_transform_state, z_offset, overflow_controls_only);
}

bool PaintLayer::HitTestFragmentWithPhase(HitTestResult& result, const PhysicalBoxFragment* physical_fragment, const PhysicalOffset& fragment_offset,
    const HitTestLocation& hit_test_location, HitTestPhase phase) const
{
    DCHECK(IsSelfPaintingLayer() || HasSelfPaintingLayerDescendant());

    bool did_hit;
    if (physical_fragment) {
        if (!physical_fragment->MayIntersect(result, hit_test_location, fragment_offset)) {
            did_hit = false;
        } else {
            did_hit = BoxFragmentPainter(*physical_fragment).NodeAtPoint(result, hit_test_location, fragment_offset, phase);
        }
    } else {
        did_hit = GetLayoutObject().NodeAtPoint(result, hit_test_location, fragment_offset, phase);
    }

    if (!did_hit) {
        // It's wrong to set innerNode, but then claim that you didn't hit anything,
        // unless it is a list-based test.
        DCHECK(!result.InnerNode() || (result.GetHitTestRequest().ListBased() && result.ListBasedTestResult().size()));
        return false;
    }
...
PaintLayer* PaintLayer::HitTestChildren(PaintLayerIteration children_to_visit, const PaintLayer& transform_container,
    const PaintLayerFragment* container_fragment, HitTestResult& result, const HitTestRecursionData& recursion_data,
    HitTestingTransformState* container_transform_state, double* z_offset_for_descendants, double* z_offset, HitTestingTransformState* local_transform_state,
    bool depth_sort_descendants)
{
    ...
    auto hit_test_child = [&](PaintLayer* child_layer, bool overflow_controls_only) -> bool {
        if (child_layer->IsReplacedNormalFlowStacking())
            return false;

        // Avoid the call to child_layer.HitTestLayer() if possible.
        if (stop_layer == this && !IsHitCandidateForStopNode(child_layer->GetLayoutObject(), stop_node)) {
            return false;
        }

        STACK_UNINITIALIZED HitTestResult temp_result(result.GetHitTestRequest(), recursion_data.original_location);
        PaintLayer* hit_layer = child_layer->HitTestLayer(transform_container, container_fragment, temp_result, recursion_data,
            /*applied_transform*/ false, container_transform_state, z_offset_for_descendants, overflow_controls_only);
        ...
        if (IsHitCandidateForDepthOrder(hit_layer, depth_sort_descendants, z_offset, local_transform_state)) {
    ...
    while (PaintLayer* child_layer = iterator.Next()) {
        if (stacking_node_) {
            if (const auto* layers_painting_overlay_overflow_controls_after = stacking_node_->LayersPaintingOverlayOverflowControlsAfter(child_layer)) {
                bool break_loop = false;
                for (auto& reparent_overflow_controls_layer : base::Reversed(*layers_painting_overlay_overflow_controls_after)) {
                    DCHECK(reparent_overflow_controls_layer->NeedsReorderOverlayOverflowControls());
...
void PaintLayer::UpdateFilterReferenceBox()
{
    ...
    if (!ResourceInfo() || ResourceInfo()->FilterReferenceBox() != reference_box || ResourceInfo()->FilterViewport() != viewport) {
        if (GetLayoutObject().GetDocument().Lifecycle().GetState() == DocumentLifecycle::kInPrePaint) {
...
{
#if DCHECK_IS_ON()
    DCHECK_GE(GetLayoutObject().GetDocument().Lifecycle().GetState(), DocumentLifecycle::kInPrePaint);
#endif
    if (ResourceInfo())
...
gfx::RectF PaintLayer::BackdropFilterReferenceBox() const
{
    if (const auto* layout_inline = DynamicTo<LayoutInline>(GetLayoutObject())) {
        return gfx::RectF(gfx::SizeF(layout_inline->PhysicalLinesBoundingBox().size));
    }
...
gfx::RRectF PaintLayer::BackdropFilterBounds() const
{
    gfx::RRectF backdrop_filter_bounds(
        SkRRect(RoundedBorderGeometry::PixelSnappedRoundedBorder(GetLayoutObject().StyleRef(), PhysicalRect::EnclosingRect(BackdropFilterReferenceBox()))));
    return backdrop_filter_bounds;
}

bool PaintLayer::HitTestClippedOutByClipPath(const PaintLayer& root_layer, const HitTestLocation& hit_test_location) const
{
    // TODO(crbug.com/1270522): Support LayoutNGBlockFragmentation.
    DCHECK(GetLayoutObject().HasClipPath());
    DCHECK(IsSelfPaintingLayer());

    PhysicalOffset origin = GetLayoutObject().LocalToAncestorPoint(PhysicalOffset(), &root_layer.GetLayoutObject());
...
PhysicalRect PaintLayer::LocalBoundingBox() const
{
    ...
    if (GetLayoutObject().IsEffectiveRootScroller() || IsRootLayer()) {
        rect.Unite(PhysicalRect(rect.offset, GetLayoutObject().View()->ViewRect().size));
    }
    return rect;
}

void PaintLayer::ExpandRectForSelfPaintingDescendants(PhysicalRect& result) const
{
    ...
    while (PaintLayer* child_layer = iterator.Next()) {
        ...
        if (child_layer->Transform()) {
            added_rect = PhysicalRect::EnclosingRect(child_layer->Transform()->MapRect(gfx::RectF(added_rect)));
        }

        PhysicalOffset delta = child_layer->GetLayoutObject().LocalToAncestorPoint(PhysicalOffset(), &GetLayoutObject(), kIgnoreTransforms);
...
PhysicalRect PaintLayer::LocalBoundingBoxIncludingSelfPaintingDescendants() const
{
...
void PaintLayer::UpdateSelfPaintingLayer()
{
    ...
    if (PaintLayer* parent = Parent()) {
        ...
        if (PaintLayer* enclosing_self_painting_layer = parent->EnclosingSelfPaintingLayer()) {
            if (is_self_painting_layer)
                MergeNeedsPaintPhaseFlagsFrom(*enclosing_self_painting_layer);
            else
                enclosing_self_painting_layer->MergeNeedsPaintPhaseFlagsFrom(*this);
...
void PaintLayer::UpdateFilters(StyleDifference diff, const ComputedStyle* old_style, const ComputedStyle& new_style)
{
    if (!filter_on_effect_node_dirty_) {
        filter_on_effect_node_dirty_ = old_style ? diff.FilterChanged() : new_style.HasFilterInducingProperty();
    }

    if (!new_style.HasFilterInducingProperty() && (!old_style || !old_style->HasFilterInducingProperty()))
        return;
...
void PaintLayer::UpdateBackdropFilters(const ComputedStyle* old_style, const ComputedStyle& new_style)
{
    if (!backdrop_filter_on_effect_node_dirty_) {
        backdrop_filter_on_effect_node_dirty_ = old_style ? old_style->BackdropFilter() != new_style.BackdropFilter() : new_style.HasBackdropFilter();
    }

    if (!new_style.HasBackdropFilter() && (!old_style || !old_style->HasBackdropFilter())) {
        return;
    }
...
void PaintLayer::UpdateClipPath(const ComputedStyle* old_style, const ComputedStyle& new_style)
{
    ...
    if (had_resource_info) {
        if (auto* old_reference_clip = DynamicTo<ReferenceClipPathOperation>(old_clip))
            old_reference_clip->RemoveClient(*ResourceInfo());
    }
}

void PaintLayer::UpdateOffsetPath(const ComputedStyle* old_style, const ComputedStyle& new_style)
{
    OffsetPathOperation* new_offset = new_style.OffsetPath();
    OffsetPathOperation* old_offset = old_style ? old_style->OffsetPath() : nullptr;
    if (!new_offset && !old_offset) {
        return;
    }
    const bool had_resource_info = ResourceInfo();
    if (auto* reference_offset = DynamicTo<ReferenceOffsetPathOperation>(new_offset)) {
        reference_offset->AddClient(EnsureResourceInfo());
    }
    if (had_resource_info) {
        if (auto* old_reference_offset = DynamicTo<ReferenceOffsetPathOperation>(old_offset)) {
...
void PaintLayer::StyleDidChange(StyleDifference diff, const ComputedStyle* old_style)
{
    ...
    // screens.
    if (GetScrollableArea() && GetScrollableArea()->ScrollsOverflow() && diff.HasDifference()) {
        MarkAncestorChainForFlagsUpdate();
    }
    ...
    if (needs_full_transform_update) {
        ...
        if (!diff.OtherTransformPropertyChanged() && PaintPropertyTreeBuilder::ScheduleDeferredTransformNodeUpdate(GetLayoutObject())) {
    ...
    if (needs_full_opacity_update) {
        if (PaintPropertyTreeBuilder::ScheduleDeferredOpacityNodeUpdate(GetLayoutObject())) {
    ...
    if (needs_full_transform_update || needs_full_opacity_update || diff.ZIndexChanged() || diff.FilterChanged() || diff.CssClipChanged()
        || diff.BlendModeChanged() || diff.MaskChanged() || diff.CompositingReasonsChanged()) {
        GetLayoutObject().SetNeedsPaintPropertyUpdate();
        MarkAncestorChainForFlagsUpdate();
    }
    ...
    if (old_style) {
        bool new_painted_output_invisible = PaintLayerPainter::PaintedOutputInvisible(new_style);
        if (PaintLayerPainter::PaintedOutputInvisible(*old_style) != new_painted_output_invisible) {
...
FilterOperations PaintLayer::FilterOperationsIncludingReflection() const
{
    ...
    if (GetLayoutObject().HasReflection() && GetLayoutObject().IsBox()) {
        ...
        filter_operations.Operations().push_back(MakeGarbageCollected<BoxReflectFilterOperation>(reflection));
    }
    return filter_operations;
}

void PaintLayer::UpdateCompositorFilterOperationsForFilter(CompositorFilterOperations& operations)
{
    ...
    if (!operations.IsEmpty() && !filter_on_effect_node_dirty_ && reference_box == operations.ReferenceBox())
        return;

    operations = FilterEffectBuilder(reference_box, FilterViewport(), zoom, style.VisitedDependentColor(GetCSSPropertyColor()), style.UsedColorScheme())
                     .BuildFilterOperations(filter);
    filter_on_effect_node_dirty_ = false;
}

void PaintLayer::UpdateCompositorFilterOperationsForBackdropFilter(CompositorFilterOperations& operations, gfx::RRectF& backdrop_filter_bounds)
{
    const auto& style = GetLayoutObject().StyleRef();
    ...
    // Use the existing |operations| if there is no change.
    if (!operations.IsEmpty() && !backdrop_filter_on_effect_node_dirty_ && reference_box == operations.ReferenceBox())
        return;
    ...
    operations = FilterEffectBuilder(
        reference_box, FilterViewport(), zoom, style.VisitedDependentColor(GetCSSPropertyColor()), style.UsedColorScheme(), nullptr, nullptr)
...
void PaintLayer::MarkCompositingContainerChainForNeedsRepaint()
{
    ...
    while (true) {
        ...
        if (!layer->SelfNeedsRepaint() && layer->GetLayoutObject().ChildPaintBlockedByDisplayLock())
            break;
...
void PaintLayer::SetInvisibleForPositionVisibility(LayerPositionVisibility visibility, bool invisible)
{
    ...
    if (InvisibleForPositionVisibility() != already_invisible) {
        ...
        if (!already_invisible && !GetLayoutObject().IsStackingContext() &&
            ...
            !needs_descendant_dependent_flags_update_ && HasSelfPaintingLayerDescendant()) {
...
bool PaintLayer::HasAncestorInvisibleForPositionVisibility() const
{
    ...
    for (auto* layer = Parent(); layer && !layer->GetLayoutObject().IsStackingContext(); layer = layer->Parent()) {
...
void ShowLayerTree(const blink::PaintLayer* layer)
{
    ...
    if (blink::LocalFrame* frame = layer->GetLayoutObject().GetFrame()) {
        WTF::String output = ExternalRepresentation(frame,
            blink::kLayoutAsTextShowLayerNesting | blink::kLayoutAsTextShowAddresses | blink::kLayoutAsTextShowIDAndClass | blink::kLayoutAsTextDontUpdateLayout
                | blink::kLayoutAsTextShowLayoutState | blink::kLayoutAsTextShowPaintProperties,
            layer);
