...
public:
    DecorationLinePainter(GraphicsContext& context, const TextDecorationInfo& decoration_info)
        : context_(context)
    ...
    static void DrawLineForText(GraphicsContext&, const gfx::PointF& pt, float width, const StyledStrokeData& styled_stroke, const AutoDarkMode& auto_dark_mode,
        const cc::PaintFlags* paint_flags = nullptr);
    static Path GetPathForTextLine(const gfx::PointF& pt, float width, float stroke_thickness, StrokeStyle stroke_style);
