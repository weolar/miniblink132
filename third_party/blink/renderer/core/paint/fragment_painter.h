...
public:
    FragmentPainter(const PhysicalBoxFragment& box, const DisplayItemClient& display_item_client)
        : box_fragment_(box)
    ...
    void PaintOutline(const PaintInfo&, const PhysicalOffset& paint_offset, const ComputedStyle& style_to_use);

    void AddURLRectIfNeeded(const PaintInfo&, const PhysicalOffset& paint_offset);

private:
    const PhysicalBoxFragment& PhysicalFragment() const
    {
