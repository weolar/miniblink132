...
protected:
    ...
    {
        return GetLayoutView().GetScrollableArea()->GetScrollingBackgroundDisplayItemClient();
    }
    ...
    {
        DCHECK_EQ(GetDocument().Lifecycle().GetState(), DocumentLifecycle::kPrePaintClean);
    ...
    DisplayItemRange ContentDisplayItems()
    {
        ...
        while (begin_index < end_index && display_item_list[begin_index].ClientId() == GetLayoutView().Id()) {
            begin_index++;
        }
        while (end_index > begin_index && IsNotContentType(display_item_list[end_index - 1].GetType())) {
            end_index--;
        }
    ...
    PaintChunkSubset ContentPaintChunks()
    {
        ...
        while (begin_index < end_index) {
            ...
            if (client_id != GetLayoutView().Id() && client_id != GetLayoutView().Layer()->Id()) {
                break;
            }
            begin_index++;
        }
        while (end_index > begin_index && IsNotContentType(chunks[end_index - 1].id.type)) {
            end_index--;
        }
    ...
    void SetWheelEventListener(const char* element_id)
    {
        ...
        element->addEventListener(event_type_names::kWheel, listener, resolved_options);
        UpdateAllLifecyclePhasesForTest();
    }
};

class PaintControllerPaintTest : public PaintTestConfigurations, public PaintControllerPaintTestBase {
public:
...
const DisplayItem::Type kBackgroundChunkType = DisplayItem::PaintPhaseToDrawingType(PaintPhase::kBlockBackground);
const DisplayItem::Type kHitTestChunkType = DisplayItem::PaintPhaseToDrawingType(PaintPhase::kSelfBlockBackgroundOnly);
const DisplayItem::Type kScrollingBackgroundChunkType = DisplayItem::PaintPhaseToClipType(PaintPhase::kSelfBlockBackgroundOnly);
const DisplayItem::Type kClippedContentsBackgroundChunkType = DisplayItem::PaintPhaseToClipType(PaintPhase::kDescendantBlockBackgroundsOnly);

#define VIEW_SCROLLING_BACKGROUND_DISPLAY_ITEM IsSameId(ViewScrollingBackgroundClient().Id(), DisplayItem::kDocumentBackground)
...
#define VIEW_SCROLLING_BACKGROUND_CHUNK_COMMON                                                                                                                 \
    IsPaintChunk(                                                                                                                                              \
        0, 1, PaintChunk::Id(ViewScrollingBackgroundClient().Id(), DisplayItem::kDocumentBackground), GetLayoutView().FirstFragment().ContentsProperties())
...
#define VIEW_SCROLLING_BACKGROUND_CHUNK(display_item_count, ...)                                                                                               \
    IsPaintChunk(0, display_item_count, PaintChunk::Id(ViewScrollingBackgroundClient().Id(), DisplayItem::kDocumentBackground),                                \
        GetLayoutView().FirstFragment().ContentsProperties(), __VA_ARGS__)
