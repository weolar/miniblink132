...
void UpdatePropertyChange(PaintPropertyChangeType& target, PaintPropertyChangeType new_change)
{
...
bool AreSubtreeUpdateReasonsIsolationPiercing(unsigned reasons)
{
    ...
    return reasons & ~(static_cast<unsigned>(SubtreePaintPropertyUpdateReason::kContainerChainMayChange));
}
...
{
    current.clip = absolute_position.clip = fixed_position.clip = &ClipPaintPropertyNode::Root();
    current.transform = absolute_position.transform = fixed_position.transform = &TransformPaintPropertyNode::Root();
    current.scroll = absolute_position.scroll = fixed_position.scroll = &ScrollPaintPropertyNode::Root();
}

void VisualViewportPaintPropertyTreeBuilder::Update(
    LocalFrameView& main_frame_view, VisualViewport& visual_viewport, PaintPropertyTreeBuilderContext& full_context)
{
    PaintPropertyTreeBuilderFragmentContext& context = full_context.fragment_context;

    auto property_changed = visual_viewport.UpdatePaintPropertyNodesIfNeeded(context);

    context.current.transform = visual_viewport.GetScrollTranslationNode();
    context.absolute_position.transform = visual_viewport.GetScrollTranslationNode();
    ...
    if (property_changed >= PaintPropertyChangeType::kNodeAddedOrRemoved) {
        ...
        full_context.force_subtree_update_reasons |= PaintPropertyTreeBuilderContext::kSubtreeUpdateIsolationPiercing;
    ...
    if (property_changed > PaintPropertyChangeType::kChangedOnlyCompositedValues) {
...
void PaintPropertyTreeBuilder::SetupContextForFrame(LocalFrameView& frame_view, PaintPropertyTreeBuilderContext& full_context)
{
    PaintPropertyTreeBuilderFragmentContext& context = full_context.fragment_context;
...
public:
    FragmentPaintPropertyTreeBuilder(
        const LayoutObject& object, PrePaintInfo* pre_paint_info, PaintPropertyTreeBuilderContext& full_context, FragmentData& fragment_data)
        : object_(object)
    ...
    bool HasIsolationNodes() const
    {
        ...
        DCHECK(!properties_ || (properties_->TransformIsolationNode() && properties_->ClipIsolationNode() && properties_->EffectIsolationNode())
            || (!properties_->TransformIsolationNode() && !properties_->ClipIsolationNode() && !properties_->EffectIsolationNode()));
...
private:
    ...
    ALWAYS_INLINE void UpdateForPaintOffsetTranslation(std::optional<gfx::Vector2d>&);
    ALWAYS_INLINE void UpdatePaintOffsetTranslation(const std::optional<gfx::Vector2d>&);
    ALWAYS_INLINE void SetNeedsPaintPropertyUpdateIfNeeded();
    ALWAYS_INLINE void UpdateForObjectLocation(std::optional<gfx::Vector2d>& paint_offset_translation);
    ALWAYS_INLINE void UpdateStickyTranslation();
    ALWAYS_INLINE void UpdateAnchorPositionScrollTranslation();

    void UpdateIndividualTransform(bool (*needs_property)(const LayoutObject&, CompositingReasons),
        void (*compute_matrix)(const LayoutBox& box, const PhysicalRect& reference_box, gfx::Transform& matrix),
        CompositingReasons compositing_reasons_for_property, CompositorElementIdNamespace compositor_namespace,
        bool (ComputedStyle::*running_on_compositor_test)() const, const TransformPaintPropertyNode* (ObjectPaintProperties::*getter)() const,
        PaintPropertyChangeType (ObjectPaintProperties::*updater)(
            const TransformPaintPropertyNodeOrAlias&, TransformPaintPropertyNode::State&&, const TransformPaintPropertyNode::AnimationState&),
    ...
    ALWAYS_INLINE TransformPaintPropertyNode::TransformAndOrigin TransformAndOriginForSVGChild() const;
    ALWAYS_INLINE void UpdateLayoutShiftRootChanged(bool is_layout_shift_root);

    bool NeedsPaintPropertyUpdate() const
    {
        return object_.NeedsPaintPropertyUpdate() || full_context_.force_subtree_update_reasons;
    }
    ...
    bool IsInNGFragmentTraversal() const
    {
        return pre_paint_info_;
    }

    void SwitchToOOFContext(PaintPropertyTreeBuilderFragmentContext::ContainingBlockContext& oof_context) const
    {
        ...
        context_.current.paint_offset = context_.current.paint_offset_for_oof_in_fragmentainer;
    }

    void ResetPaintOffset(PhysicalOffset new_offset = PhysicalOffset())
    {
        context_.current.paint_offset_for_oof_in_fragmentainer -= context_.current.paint_offset - new_offset;
    ...
    {
        if (cleared) {
            UpdatePropertyChange(properties_changed_.transform_changed, PaintPropertyChangeType::kNodeAddedOrRemoved);
            properties_changed_.transform_change_is_scroll_translation_only = false;
        }
    }
    ...
    {
        if (cleared) {
            UpdatePropertyChange(properties_changed_.clip_changed, PaintPropertyChangeType::kNodeAddedOrRemoved);
        }
    }
    ...
    {
        if (cleared) {
            UpdatePropertyChange(properties_changed_.effect_changed, PaintPropertyChangeType::kNodeAddedOrRemoved);
        }
    }
    ...
    {
        if (cleared) {
            UpdatePropertyChange(properties_changed_.scroll_changed, PaintPropertyChangeType::kNodeAddedOrRemoved);
        }
    }

    CompositorElementId GetCompositorElementId(CompositorElementIdNamespace namespace_id) const
    {
        return CompositorElementIdFromUniqueObjectId(fragment_data_.UniqueId(), namespace_id);
    }

    MainThreadScrollingReasons GetMainThreadRepaintReasonsForScroll(bool user_scrollable) const;
...
static bool NeedsScrollAndScrollTranslation(const LayoutObject& object, CompositingReasons direct_compositing_reasons)
{
    ...
    if (!scrollable_area->ScrollPosition().IsOrigin() || !scrollable_area->GetScrollOffset().IsZero()) {
        return true;
    }
    return false;
}
...
static bool NeedsPaintOffsetTranslationForOverflowControls(const LayoutBoxModelObject& object)
{
    if (auto* area = object.GetScrollableArea()) {
        if (area->HorizontalScrollbar() || area->VerticalScrollbar() || area->Resizer()) {
            return true;
        }
    }
    return false;
...
static bool NeedsIsolationNodes(const LayoutObject& object)
{
    ...
    if (object.ShouldApplyPaintContainment() || (object.ShouldApplyStyleContainment() && object.ShouldApplyLayoutContainment())) {
        return true;
    }
...
static bool NeedsPaintOffsetTranslation(const LayoutObject& object, CompositingReasons direct_compositing_reasons,
    const LayoutObject* container_for_fixed_position, const PaintLayer* painting_layer)
{
    if (!object.IsBoxModelObject())
        return false;
    ...
    // zero paint offset.
    if (box_model.HasLayer() && (object.StyleRef().Filter().HasReferenceFilter() || object.HasReflection()))
        return true;
    ...
    bool has_paint_offset_compositing_reason
        = direct_compositing_reasons != CompositingReason::kNone || box_model.StyleRef().BackfaceVisibility() == EBackfaceVisibility::kHidden;
    if (has_paint_offset_compositing_reason) {
        ...
        // and column spans.
        if (box_model.IsLayoutBlock() || object.IsLayoutReplaced() || (direct_compositing_reasons & CompositingReason::kViewTransitionElement)
...
bool FragmentPaintPropertyTreeBuilder::CanPropagateSubpixelAccumulation() const
{
    if (!object_.HasLayer())
        return true;

    if (full_context_.direct_compositing_reasons & CompositingReason::kPreventingSubpixelAccumulationReasons) {
        return false;
    }
    if (full_context_.direct_compositing_reasons
        & (CompositingReason::kActiveTransformAnimation | CompositingReason::kActiveRotateAnimation | CompositingReason::kActiveScaleAnimation)) {
...
void FragmentPaintPropertyTreeBuilder::UpdateForPaintOffsetTranslation(std::optional<gfx::Vector2d>& paint_offset_translation)
{
    if (!NeedsPaintOffsetTranslation(
            object_, full_context_.direct_compositing_reasons, full_context_.container_for_fixed_position, full_context_.painting_layer)) {
        return;
    }
    ...
    if (NeedsIsolationNodes(object_)) {
        ResetPaintOffset();
        context_.current.directly_composited_container_paint_offset_subpixel_delta = PhysicalOffset();
        return;
    }

    PhysicalOffset subpixel_accumulation = context_.current.paint_offset - PhysicalOffset(*paint_offset_translation);
    if (!subpixel_accumulation.IsZero() || !context_.current.directly_composited_container_paint_offset_subpixel_delta.IsZero()) {
    ...
    if (paint_offset_translation && properties_ && properties_->PaintOffsetTranslation()) {
        ...
        context_.current.directly_composited_container_paint_offset_subpixel_delta = context_.current.paint_offset - fragment_data_.PaintOffset();
    } else {
...
void FragmentPaintPropertyTreeBuilder::UpdatePaintOffsetTranslation(const std::optional<gfx::Vector2d>& paint_offset_translation)
{
    DCHECK(properties_);

    if (paint_offset_translation) {
        TransformPaintPropertyNode::State state { { gfx::Transform::MakeTranslation(*paint_offset_translation) } };
        state.flattens_inherited_transform = context_.should_flatten_inherited_transform;
        state.rendering_context_id = context_.rendering_context_id;
        state.direct_compositing_reasons = full_context_.direct_compositing_reasons & CompositingReason::kDirectReasonsForPaintOffsetTranslationProperty;
        if (auto* box = DynamicTo<LayoutBox>(object_)) {
            if (box->IsFixedToView(full_context_.container_for_fixed_position) && object_.View()->FirstFragment().PaintProperties()->Scroll()) {
                state.scroll_translation_for_fixed = object_.View()->FirstFragment().PaintProperties()->ScrollTranslation();
            }
        }
        ...
        if (IsA<LayoutView>(object_)) {
            ...
            state.visible_frame_element_id = object_.GetFrame()->GetVisibleToHitTesting()
                ? CompositorElementIdFromUniqueObjectId(object_.GetDocument().GetDomNodeId(), CompositorElementIdNamespace::kDOMNodeId)
                : cc::ElementId();
        }
        OnUpdateTransform(properties_->UpdatePaintOffsetTranslation(*context_.current.transform, std::move(state)));
        context_.current.transform = properties_->PaintOffsetTranslation();
        if (IsA<LayoutView>(object_)) {
            context_.absolute_position.transform = properties_->PaintOffsetTranslation();
        ...
        if (!object_.ShouldAssumePaintOffsetTranslationForLayoutShiftTracking()) {
            context_.current.additional_offset_to_layout_shift_root_delta += PhysicalOffset(*paint_offset_translation);
        }
    } else {
...
void FragmentPaintPropertyTreeBuilder::UpdateStickyTranslation()
{
    ...
    if (NeedsPaintPropertyUpdate()) {
        if (NeedsStickyTranslation(object_)) {
            ...
            TransformPaintPropertyNode::State state { { gfx::Transform::MakeTranslation(gfx::Vector2dF(box_model.StickyPositionOffset())) } };
            state.direct_compositing_reasons = full_context_.direct_compositing_reasons & CompositingReason::kStickyPosition;
            ...
            state.compositor_element_id = CompositorElementIdFromUniqueObjectId(box_model.UniqueId(), CompositorElementIdNamespace::kStickyTranslation);
            state.rendering_context_id = context_.rendering_context_id;
            state.flattens_inherited_transform = context_.should_flatten_inherited_transform;
            ...
            if (state.direct_compositing_reasons) {
                ...
                const auto* scroll_container_properties
                    = layout_constraint->containing_scroll_container_layer->GetLayoutObject().FirstFragment().PaintProperties();
                ...
                bool scroll_container_scrolls = scroll_container_properties && scroll_container_properties->Scroll() == context_.current.scroll;
                if (scroll_container_scrolls) {
                    auto constraint = std::make_unique<CompositorStickyConstraint>();
                    constraint->is_anchored_left = layout_constraint->left_inset.has_value();
                    constraint->is_anchored_right = layout_constraint->right_inset.has_value();
                    constraint->is_anchored_top = layout_constraint->top_inset.has_value();
                    constraint->is_anchored_bottom = layout_constraint->bottom_inset.has_value();

                    constraint->left_offset = layout_constraint->left_inset.value_or(LayoutUnit()).ToFloat();
                    constraint->right_offset = layout_constraint->right_inset.value_or(LayoutUnit()).ToFloat();
                    constraint->top_offset = layout_constraint->top_inset.value_or(LayoutUnit()).ToFloat();
                    constraint->bottom_offset = layout_constraint->bottom_inset.value_or(LayoutUnit()).ToFloat();
                    constraint->constraint_box_rect = gfx::RectF(layout_constraint->constraining_rect);
                    constraint->scroll_container_relative_sticky_box_rect = gfx::RectF(layout_constraint->scroll_container_relative_sticky_box_rect);
                    constraint->scroll_container_relative_containing_block_rect
                        = gfx::RectF(layout_constraint->scroll_container_relative_containing_block_rect);
                    if (const LayoutBoxModelObject* sticky_box_shifting_ancestor = layout_constraint->nearest_sticky_layer_shifting_sticky_box) {
                        constraint->nearest_element_shifting_sticky_box
                            = CompositorElementIdFromUniqueObjectId(sticky_box_shifting_ancestor->UniqueId(), CompositorElementIdNamespace::kStickyTranslation);
                    }
                    if (const LayoutBoxModelObject* containing_block_shifting_ancestor = layout_constraint->nearest_sticky_layer_shifting_containing_block) {
                        constraint->nearest_element_shifting_containing_block = CompositorElementIdFromUniqueObjectId(
                            containing_block_shifting_ancestor->UniqueId(), CompositorElementIdNamespace::kStickyTranslation);
                    }
                    state.sticky_constraint = std::move(constraint);
                }
            }

            OnUpdateTransform(properties_->UpdateStickyTranslation(*context_.current.transform, std::move(state)));
        } else {
...
void FragmentPaintPropertyTreeBuilder::UpdateAnchorPositionScrollTranslation()
{
    ...
    if (NeedsPaintPropertyUpdate()) {
        if (NeedsAnchorPositionScrollTranslation(object_)) {
            const auto& box = To<LayoutBox>(object_);
            const AnchorPositionScrollData& anchor_position_scroll_data = *box.GetAnchorPositionScrollData();
            gfx::Vector2dF translation_offset = -anchor_position_scroll_data.AccumulatedAdjustment();
            TransformPaintPropertyNode::State state { { gfx::Transform::MakeTranslation(translation_offset) } };
            ...
            DCHECK(full_context_.direct_compositing_reasons & CompositingReason::kAnchorPosition);
            ...
            state.compositor_element_id = CompositorElementIdFromUniqueObjectId(box.UniqueId(), CompositorElementIdNamespace::kAnchorPositionScrollTranslation);
            state.rendering_context_id = context_.rendering_context_id;
            state.flattens_inherited_transform = context_.should_flatten_inherited_transform;

            state.anchor_position_scroll_data = std::make_unique<cc::AnchorPositionScrollData>();
            state.anchor_position_scroll_data->adjustment_container_ids = std::vector<CompositorElementId>(
                anchor_position_scroll_data.AdjustmentContainerIds().begin(), anchor_position_scroll_data.AdjustmentContainerIds().end());
            state.anchor_position_scroll_data->accumulated_scroll_origin = anchor_position_scroll_data.AccumulatedAdjustmentScrollOrigin();
            state.anchor_position_scroll_data->needs_scroll_adjustment_in_x = anchor_position_scroll_data.NeedsScrollAdjustmentInX();
            state.anchor_position_scroll_data->needs_scroll_adjustment_in_y = anchor_position_scroll_data.NeedsScrollAdjustmentInY();

            OnUpdateTransform(properties_->UpdateAnchorPositionScrollTranslation(*context_.current.transform, std::move(state)));
        } else {
...
static void DirectlyUpdateCcTransform(const TransformPaintPropertyNode& transform, const LayoutObject& object, PaintPropertyChangeType& change_type)
{
    ...
    if (change_type == PaintPropertyChangeType::kChangedOnlySimpleValues && transform.HasActiveTransformAnimation()) {
        if (auto* paint_artifact_compositor = object.GetFrameView()->GetPaintArtifactCompositor()) {
            bool updated = paint_artifact_compositor->DirectlyUpdateTransform(transform);
            if (updated) {
...
static void DirectlyUpdateCcOpacity(const LayoutObject& object, ObjectPaintProperties& properties, PaintPropertyChangeType& change_type)
{
    if (change_type == PaintPropertyChangeType::kChangedOnlySimpleValues && properties.Effect()->HasDirectCompositingReasons()) {
        if (auto* paint_artifact_compositor = object.GetFrameView()->GetPaintArtifactCompositor()) {
            bool updated = paint_artifact_compositor->DirectlyUpdateCompositedOpacityValue(*properties.Effect());
            if (updated) {
                change_type = PaintPropertyChangeType::kChangedOnlyCompositedValues;
...
{
    CompositingReasons reasons = CompositingReason::kDirectReasonsForTransformProperty;
...
static bool NeedsTransformForSVGChild(const LayoutObject& object, CompositingReasons direct_compositing_reasons)
{
    if (!object.IsSVGChild() || object.IsText())
        return false;
    if (direct_compositing_reasons
        & (CompositingReasonsForTransformProperty() | CompositingReason::kDirectReasonsForTranslateProperty | CompositingReason::kDirectReasonsForRotateProperty
            | CompositingReason::kDirectReasonsForScaleProperty))
        return true;
    return !object.LocalToSVGParentTransform().IsIdentity();
}

TransformPaintPropertyNode::TransformAndOrigin FragmentPaintPropertyTreeBuilder::TransformAndOriginForSVGChild() const
{
    if (full_context_.direct_compositing_reasons & CompositingReason::kActiveTransformAnimation) {
        if (CompositorAnimations::CanStartTransformAnimationOnCompositorForSVG(*To<SVGElement>(object_.GetNode()))) {
            const gfx::RectF reference_box = TransformHelper::ComputeReferenceBox(object_);
            ...
            DCHECK_EQ(TransformHelper::ComputeTransform(object_.GetDocument(), object_.StyleRef(), reference_box, ComputedStyle::kIncludeTransformOrigin),
            ...
            return { TransformHelper::ComputeTransform(object_.GetDocument(), object_.StyleRef(), reference_box, ComputedStyle::kExcludeTransformOrigin)
                         .ToTransform(),
                gfx::Point3F(TransformHelper::ComputeTransformOrigin(object_.StyleRef(), reference_box)) };
        }
    }
...
// TODO(crbug.com/1278452): Merge SVG handling into the primary codepath.
void FragmentPaintPropertyTreeBuilder::UpdateTransformForSVGChild(CompositingReasons direct_compositing_reasons)
{
    DCHECK(properties_);
    ...
    DCHECK(object_.IsSVGForeignObject() || context_.current.paint_offset.IsZero());
    ...
    if (NeedsPaintPropertyUpdate()) {
        if (NeedsTransformForSVGChild(object_, direct_compositing_reasons)) {
            ...
            state.direct_compositing_reasons = direct_compositing_reasons & CompositingReasonsForTransformProperty();
            state.flattens_inherited_transform = context_.should_flatten_inherited_transform;
            state.rendering_context_id = context_.rendering_context_id;
            state.is_for_svg_child = true;
            state.compositor_element_id = GetCompositorElementId(CompositorElementIdNamespace::kPrimaryTransform);

            TransformPaintPropertyNode::AnimationState animation_state;
            animation_state.is_running_animation_on_compositor = object_.StyleRef().IsRunningTransformAnimationOnCompositor();
            auto effective_change_type = properties_->UpdateTransform(*context_.current.transform, std::move(state), animation_state);
            DirectlyUpdateCcTransform(*properties_->Transform(), object_, effective_change_type);
...
static gfx::Point3F GetTransformOrigin(const LayoutBox& box, const PhysicalRect& reference_box)
{
    ...
    return gfx::Point3F(FloatValueForLength(style.GetTransformOrigin().X(), reference_box_size.width()) + reference_box.X().ToFloat(),
        FloatValueForLength(style.GetTransformOrigin().Y(), reference_box_size.height()) + reference_box.Y().ToFloat(), style.GetTransformOrigin().Z());
}

static bool NeedsIndividualTransform(const LayoutObject& object, CompositingReasons relevant_compositing_reasons, bool (*style_test)(const ComputedStyle&))
{
...
static bool NeedsTranslate(const LayoutObject& object, CompositingReasons direct_compositing_reasons)
{
    return NeedsIndividualTransform(object, direct_compositing_reasons & CompositingReason::kDirectReasonsForTranslateProperty,
        [](const ComputedStyle& style) { return style.Translate() || style.HasCurrentTranslateAnimation(); });
}

static bool NeedsRotate(const LayoutObject& object, CompositingReasons direct_compositing_reasons)
{
    return NeedsIndividualTransform(object, direct_compositing_reasons & CompositingReason::kDirectReasonsForRotateProperty,
        [](const ComputedStyle& style) { return style.Rotate() || style.HasCurrentRotateAnimation(); });
}

static bool NeedsScale(const LayoutObject& object, CompositingReasons direct_compositing_reasons)
{
    return NeedsIndividualTransform(object, direct_compositing_reasons & CompositingReason::kDirectReasonsForScaleProperty,
        [](const ComputedStyle& style) { return style.Scale() || style.HasCurrentScaleAnimation(); });
}

static bool NeedsOffset(const LayoutObject& object, CompositingReasons direct_compositing_reasons)
{
    return NeedsIndividualTransform(object, CompositingReason::kNone, [](const ComputedStyle& style) { return style.HasOffset(); });
}

static bool NeedsTransform(const LayoutObject& object, CompositingReasons direct_compositing_reasons)
{
    if (object.IsText() || object.IsSVGChild())
        return false;
    ...
    if (object.StyleRef().HasTransformOperations() || object.StyleRef().HasCurrentTransformAnimation() || object.StyleRef().Preserves3D())
        return true;

    return false;
}

static bool UpdateBoxSizeAndCheckActiveAnimationAxisAlignment(const LayoutBox& object, CompositingReasons compositing_reasons)
{
    if (!(compositing_reasons
            & (CompositingReason::kActiveTransformAnimation | CompositingReason::kActiveScaleAnimation | CompositingReason::kActiveRotateAnimation
    ...
    DCHECK(animations);
    return animations->UpdateBoxSizeAndCheckTransformAxisAlignment(gfx::SizeF(object.Size()));
}

static TransformPaintPropertyNode::TransformAndOrigin TransformAndOriginState(const LayoutBox& box, const PhysicalRect& reference_box,
    void (*compute_matrix)(const LayoutBox& box, const PhysicalRect& reference_box, gfx::Transform& matrix))
{
...
static bool IsLayoutShiftRootTransform(const TransformPaintPropertyNode& transform)
{
    // This is to keep the layout shift behavior before crrev.com/c/4024030.
    return transform.HasActiveTransformAnimation() || !transform.IsIdentityOr2dTranslation();
}

void FragmentPaintPropertyTreeBuilder::UpdateIndividualTransform(bool (*needs_property)(const LayoutObject&, CompositingReasons),
    void (*compute_matrix)(const LayoutBox& box, const PhysicalRect& reference_box, gfx::Transform& matrix),
    CompositingReasons compositing_reasons_for_property, CompositorElementIdNamespace compositor_namespace,
    bool (ComputedStyle::*running_on_compositor_test)() const, const TransformPaintPropertyNode* (ObjectPaintProperties::*getter)() const,
    PaintPropertyChangeType (ObjectPaintProperties::*updater)(
    ...
    bool (ObjectPaintProperties::*clearer)())
{
    ...
    if (NeedsPaintPropertyUpdate()) {
        ...
        if ((*needs_property)(object_, full_context_.direct_compositing_reasons)) {
            ...
            bool handling_transform_property = compositor_namespace == CompositorElementIdNamespace::kPrimaryTransform;
            ...
            if (object_.IsBox()) {
                ...
                state.transform_and_origin = TransformAndOriginState(box, reference_box, compute_matrix);
                ...
                // new one.
                state.rendering_context_id = context_.rendering_context_id;
                if (handling_transform_property && style.Preserves3D() && !state.rendering_context_id) {
            ...
            state.direct_compositing_reasons = full_context_.direct_compositing_reasons & compositing_reasons_for_property;

            state.flattens_inherited_transform = context_.should_flatten_inherited_transform;
            if (running_on_compositor_test) {
                state.compositor_element_id = GetCompositorElementId(compositor_namespace);
            }

            if (handling_transform_property) {
                if (object_.HasHiddenBackface()) {
                    state.backface_visibility = TransformPaintPropertyNode::BackfaceVisibility::kHidden;
                } else if (!context_.can_inherit_backface_visibility || style.Has3DTransformOperation()) {
                    ...
                    state.backface_visibility = TransformPaintPropertyNode::BackfaceVisibility::kVisible;
                } else {
                    // Otherwise we want to inherit backface-visibility.
                    DCHECK_EQ(state.backface_visibility, TransformPaintPropertyNode::BackfaceVisibility::kInherited);
                }
            }

            TransformPaintPropertyNode::AnimationState animation_state;
            animation_state.is_running_animation_on_compositor = running_on_compositor_test && (style.*running_on_compositor_test)();
            auto effective_change_type = (properties_->*updater)(*context_.current.transform, std::move(state), animation_state);
            DirectlyUpdateCcTransform(*(properties_->*getter)(), object_, effective_change_type);
            OnUpdateTransform(effective_change_type);
        } else {
    ...
    if (const auto* transform = (properties_->*getter)()) {
        ...
        if (!IsLayoutShiftRootTransform(*transform)) {
            context_.translation_2d_to_layout_shift_root_delta += transform->Get2dTranslation();
        }
    }
}
...
void FragmentPaintPropertyTreeBuilder::UpdateTranslate()
{
    UpdateIndividualTransform(
        &NeedsTranslate,
        [](const LayoutBox& box, const PhysicalRect& reference_box, gfx::Transform& matrix) {
            const ComputedStyle& style = box.StyleRef();
            if (style.Translate())
                style.Translate()->Apply(matrix, gfx::SizeF(reference_box.size));
        },
        CompositingReason::kDirectReasonsForTranslateProperty, CompositorElementIdNamespace::kTranslateTransform,
        &ComputedStyle::IsRunningTranslateAnimationOnCompositor, &ObjectPaintProperties::Translate, &ObjectPaintProperties::UpdateTranslate,
...
void FragmentPaintPropertyTreeBuilder::UpdateRotate()
{
    UpdateIndividualTransform(
        ...
        CompositingReason::kDirectReasonsForRotateProperty, CompositorElementIdNamespace::kRotateTransform,
        &ComputedStyle::IsRunningRotateAnimationOnCompositor, &ObjectPaintProperties::Rotate, &ObjectPaintProperties::UpdateRotate,
...
void FragmentPaintPropertyTreeBuilder::UpdateScale()
{
    UpdateIndividualTransform(
        ...
        CompositingReason::kDirectReasonsForScaleProperty, CompositorElementIdNamespace::kScaleTransform, &ComputedStyle::IsRunningScaleAnimationOnCompositor,
        &ObjectPaintProperties::Scale, &ObjectPaintProperties::UpdateScale, &ObjectPaintProperties::ClearScale);
}
...
void FragmentPaintPropertyTreeBuilder::UpdateOffset()
{
    UpdateIndividualTransform(
        ...
        [](const LayoutBox& box, const PhysicalRect& reference_box, gfx::Transform& matrix) {
            const ComputedStyle& style = box.StyleRef();
            style.ApplyTransform(matrix, &box, reference_box, ComputedStyle::kExcludeTransformOperations, ComputedStyle::kExcludeTransformOrigin,
                ComputedStyle::kIncludeMotionPath, ComputedStyle::kExcludeIndependentTransformProperties);
        },
        CompositingReason::kNone,
        // TODO(dbaron): When we support animating offset on the
        // compositor, we need to use an element ID specific to offset.
        // This is currently unused.
        CompositorElementIdNamespace::kPrimary, nullptr, &ObjectPaintProperties::Offset, &ObjectPaintProperties::UpdateOffset,
...
void FragmentPaintPropertyTreeBuilder::UpdateTransform()
{
    UpdateIndividualTransform(
        ...
        [](const LayoutBox& box, const PhysicalRect& reference_box, gfx::Transform& matrix) {
            const ComputedStyle& style = box.StyleRef();
            style.ApplyTransform(matrix, &box, reference_box, ComputedStyle::kIncludeTransformOperations, ComputedStyle::kExcludeTransformOrigin,
                ComputedStyle::kExcludeMotionPath, ComputedStyle::kExcludeIndependentTransformProperties);
        },
        CompositingReasonsForTransformProperty(), CompositorElementIdNamespace::kPrimaryTransform, &ComputedStyle::IsRunningTransformAnimationOnCompositor,
        &ObjectPaintProperties::Transform, &ObjectPaintProperties::UpdateTransform, &ObjectPaintProperties::ClearTransform);
...
static bool NeedsClipPathClipOrMask(const LayoutObject& object)
{
    ...
    return !object.IsText()
        && (ClipPathClipper::HasCompositeClipPathAnimation(object) || (object.StyleRef().HasClipPath() && (object.HasLayer() || object.IsSVGChild())));
}
...
static bool NeedsEffectForViewTransition(const LayoutObject& object)
{
    ...
    if (style.ElementIsViewTransitionParticipant()) {
        DCHECK(ViewTransitionUtils::IsViewTransitionElementExcludingRootFromSupplement(*To<Element>(object.GetNode())));
        return true;
    } else {
#if DCHECK_IS_ON()
        auto* element = DynamicTo<Element>(object.GetNode());
        DCHECK(!element || !ViewTransitionUtils::IsViewTransitionElementExcludingRootFromSupplement(*element)) << element;
#endif
    }

    return style.ViewTransitionName() && !object.IsDocumentElement() && !object.IsLayoutView();
}

static bool NeedsEffectIgnoringClipPath(const LayoutObject& object, CompositingReasons direct_compositing_reasons)
{
    if (object.IsText()) {
        DCHECK(!(direct_compositing_reasons & CompositingReason::kDirectReasonsForEffectProperty));
        return false;
    }

    if (direct_compositing_reasons & CompositingReason::kDirectReasonsForEffectProperty)
        return true;
    ...
    const bool is_css_isolated_group = object.HasLayer() && object.IsStackingContext();
    ...
    if (object.IsBlendingAllowed() && WebCoreCompositeToSkiaComposite(kCompositeSourceOver, style.GetBlendMode()) != SkBlendMode::kSrcOver)
        return true;
...
bool FragmentPaintPropertyTreeBuilder::NeedsEffect() const
{
    ...
    if (needs_mask_based_clip_path_)
        return true;
    return NeedsEffectIgnoringClipPath(object_, full_context_.direct_compositing_reasons);
}
...
bool FragmentPaintPropertyTreeBuilder::EffectCanUseCurrentClipAsOutputClip() const
{
    DCHECK(NeedsEffect());
    ...
    if (layer->HasNonContainedAbsolutePositionDescendant()) {
        ...
        if (!container->FirstFragment().HasLocalBorderBoxProperties() || &container->FirstFragment().ContentsClip() != context_.current.clip) {
            return false;
        }
    }
    if (layer->HasFixedPositionDescendant() && !object_.CanContainFixedPositionObjects()) {
...
void FragmentPaintPropertyTreeBuilder::UpdateEffect()
{
    ...
    if (NeedsPaintPropertyUpdate()) {
        if (NeedsEffect()) {
            std::optional<gfx::RectF> mask_clip = CSSMaskPainter::MaskBoundingBox(object_, context_.current.paint_offset);
            if (mask_clip || needs_mask_based_clip_path_) {
                DCHECK(mask_clip || clip_path_bounding_box_.has_value());
                gfx::RectF combined_clip = mask_clip ? *mask_clip : *clip_path_bounding_box_;
                if (mask_clip && needs_mask_based_clip_path_)
                    combined_clip.Intersect(*clip_path_bounding_box_);
                OnUpdateClip(properties_->UpdateMaskClip(*context_.current.clip,
                    ClipPaintPropertyNode::State(*context_.current.transform, combined_clip, FloatRoundedRect(gfx::ToEnclosingRect(combined_clip)))));
            ...
            if (mask_clip) {
                mask_compositor_element_id = GetCompositorElementId(CompositorElementIdNamespace::kEffectMask);
            }
            ...
            if (object_.IsBlendingAllowed()) {
                state.blend_mode = WebCoreCompositeToSkiaComposite(kCompositeSourceOver, style.GetBlendMode());
            }
            if (object_.IsBoxModelObject()) {
                if (auto* layer = To<LayoutBoxModelObject>(object_).Layer()) {
                    CompositorFilterOperations operations;
                    gfx::RRectF bounds;
                    layer->UpdateCompositorFilterOperationsForBackdropFilter(operations, bounds);
                    if (!operations.IsEmpty()) {
                        state.backdrop_filter_info
                            = base::WrapUnique(new EffectPaintPropertyNode::BackdropFilterInfo { std::move(operations), bounds, mask_compositor_element_id });
                    }
                }
            }

            state.direct_compositing_reasons = full_context_.direct_compositing_reasons & CompositingReason::kDirectReasonsForEffectProperty;
            ...
            state.direct_compositing_reasons |= (full_context_.direct_compositing_reasons & CompositingReason::kAdditionalEffectCompositingTrigger);
            ...
            if (state.direct_compositing_reasons) {
                state.compositor_element_id = GetCompositorElementId(CompositorElementIdNamespace::kPrimaryEffect);
            } else {
                ...
                state.compositor_element_id = GetCompositorElementId(CompositorElementIdNamespace::kPrimary);
            }

            state.self_or_ancestor_participates_in_view_transition = context_.self_or_ancestor_participates_in_view_transition;

            EffectPaintPropertyNode::AnimationState animation_state;
            animation_state.is_running_opacity_animation_on_compositor = style.IsRunningOpacityAnimationOnCompositor();
            animation_state.is_running_backdrop_filter_animation_on_compositor = style.IsRunningBackdropFilterAnimationOnCompositor();
            ...
            if (object_.GetNode() && object_.GetNode()->GetPseudoId() == kPseudoIdViewTransition) {
                if (IsInLocalSubframe(object_)) {
                    parent_effect = object_.GetDocument().GetLayoutView()->FirstFragment().PaintProperties()->ViewTransitionSubframeRootEffect();
                } else {
            ...
            auto effective_change_type = properties_->UpdateEffect(*parent_effect, std::move(state), animation_state);
            ...
            auto mask_direct_compositing_reasons = full_context_.direct_compositing_reasons & CompositingReason::kDirectReasonsForBackdropFilter
            ...
            if (mask_clip) {
                ...
                OnUpdateEffect(properties_->UpdateMask(*properties_->Effect(), std::move(mask_state)));
            } else {
            ...
            if (needs_mask_based_clip_path_) {
                ...
                clip_path_state.compositor_element_id = GetCompositorElementId(CompositorElementIdNamespace::kEffectClipPath);
                if (!mask_clip) {
                    clip_path_state.direct_compositing_reasons = mask_direct_compositing_reasons;
                }
                OnUpdateEffect(
                    properties_->UpdateClipPathMask(properties_->Mask() ? *properties_->Mask() : *properties_->Effect(), std::move(clip_path_state)));
            } else {
    ...
    if (const auto* effect = properties_->Effect()) {
        ...
        if (properties_->MaskClip()) {
            context_.current.clip = context_.absolute_position.clip = context_.fixed_position.clip = properties_->MaskClip();
        }
    }
}
...
void FragmentPaintPropertyTreeBuilder::UpdateElementCaptureEffect()
{
    ...
    if (!(full_context_.direct_compositing_reasons & CompositingReason::kElementCapture)) {
    ...
    state.compositor_element_id = CompositorElementIdFromUniqueObjectId(object_.UniqueId(), CompositorElementIdNamespace::kElementCapture);

    OnUpdateEffect(properties_->UpdateElementCaptureEffect(*context_.current_effect, std::move(state), {}));
    context_.current_effect = properties_->ElementCaptureEffect();
}

void FragmentPaintPropertyTreeBuilder::UpdateViewTransitionSubframeRootEffect()
{
    if (NeedsPaintPropertyUpdate()) {
        const bool needs_node = object_.IsLayoutView() && IsInLocalSubframe(object_) && ViewTransitionUtils::GetTransition(object_.GetDocument());
        ...
        if (needs_node) {
            ...
            state.local_transform_space = context_.current.transform;
            state.output_clip = context_.current.clip;
            state.compositor_element_id = CompositorElementIdFromUniqueObjectId(object_.UniqueId(), CompositorElementIdNamespace::kViewTransitionSubframeRoot);
            if (const auto& layer = ViewTransitionUtils::GetTransition(object_.GetDocument())->GetSubframeSnapshotLayer()) {
                state.view_transition_element_resource_id = layer->ViewTransitionResourceId();
            }

            auto change_type = properties_->UpdateViewTransitionSubframeRootEffect(*context_.current_effect, std::move(state), {});
            needs_full_invalidation = change_type >= PaintPropertyChangeType::kNodeAddedOrRemoved;
        ...
        if (needs_full_invalidation) {
            full_context_.force_subtree_update_reasons |= PaintPropertyTreeBuilderContext::kSubtreeUpdateIsolationPiercing;
        }
    }
...
void FragmentPaintPropertyTreeBuilder::UpdateViewTransitionEffect()
{
    if (NeedsPaintPropertyUpdate()) {
        const bool old_self_or_ancestor_participates_in_view_transition
            = properties_->ViewTransitionEffect() && properties_->ViewTransitionEffect()->SelfOrAncestorParticipatesInViewTransition();

        const bool is_view_transition_element = full_context_.direct_compositing_reasons & CompositingReason::kViewTransitionElement;

        const bool needs_view_transition_effect
            = is_view_transition_element || (object_.IsLayoutView() && !IsInLocalSubframe(object_) && !object_.GetDocument().IsSVGDocument());

        if (needs_view_transition_effect) {
            auto* transition = ViewTransitionUtils::GetTransition(object_.GetDocument());
            DCHECK(!is_view_transition_element || transition);

            EffectPaintPropertyNode::State state;
            state.local_transform_space = context_.current.transform;
            state.output_clip = context_.current.clip;
            state.compositor_element_id = CompositorElementIdFromUniqueObjectId(object_.UniqueId(), CompositorElementIdNamespace::kViewTransitionElement);
            if (is_view_transition_element) {
                state.direct_compositing_reasons = CompositingReason::kViewTransitionElement;
                state.view_transition_element_resource_id = transition->GetSnapshotId(object_);
            }
            ...
            if (object_.IsLayoutView()) {
                ...
                CHECK(!context_.self_or_ancestor_participates_in_view_transition || IsInLocalSubframe(object_));
                state.self_or_ancestor_participates_in_view_transition = context_.self_or_ancestor_participates_in_view_transition;
            } else {
                state.self_or_ancestor_participates_in_view_transition = true;
            }

            OnUpdateEffect(properties_->UpdateViewTransitionEffect(*context_.current_effect, std::move(state), {}));
        } else {
        ...
        bool new_self_or_ancestor_participates_in_view_transition = new_effect && new_effect->SelfOrAncestorParticipatesInViewTransition();

        if (old_self_or_ancestor_participates_in_view_transition != new_self_or_ancestor_participates_in_view_transition) {
            full_context_.force_subtree_update_reasons |= PaintPropertyTreeBuilderContext::kSubtreeUpdateIsolationPiercing;
        }
    }
    ...
    if (auto* effect = properties_->ViewTransitionEffect()) {
        context_.current_effect = effect;
        context_.self_or_ancestor_participates_in_view_transition |= effect->SelfOrAncestorParticipatesInViewTransition();
    }
}
...
{
    if (NeedsPaintPropertyUpdate()) {
        if (full_context_.direct_compositing_reasons & CompositingReason::kViewTransitionElement) {
            auto* transition = ViewTransitionUtils::GetTransition(object_.GetDocument());
            DCHECK(transition);
            ...
            OnUpdateClip(transition->UpdateCaptureClip(object_, context_.current.clip, context_.current.transform));
...
{
    return object.GetFrame()->GetPage()->GetLinkHighlight().IsHighlighting(object);
}
...
static bool NeedsFilter(const LayoutObject& object, const PaintPropertyTreeBuilderContext& full_context)
{
    if (full_context.direct_compositing_reasons & CompositingReason::kDirectReasonsForFilterProperty)
        return true;

    if (object.IsBoxModelObject() && To<LayoutBoxModelObject>(object).HasLayer()) {
        if (object.StyleRef().HasFilter() || object.HasReflection()) {
            return true;
        }
    } else if (object.IsSVGChild() && !object.IsText() && SVGResources::GetClient(object)) {
        if (object.StyleRef().HasFilter()) {
            // Filters don't apply to elements that are descendants of a <clipPath>.
            if (!full_context.has_svg_hidden_container_ancestor || !IsClipPathDescendant(object))
                return true;
        }
    }
    return false;
...
static void UpdateFilterEffect(const LayoutObject& object, const EffectPaintPropertyNode* effect_node, CompositorFilterOperations& filter)
{
    if (object.HasLayer()) {
...
void FragmentPaintPropertyTreeBuilder::UpdateFilter()
{
    ...
    if (NeedsPaintPropertyUpdate()) {
        if (NeedsFilter(object_, full_context_)) {
            ...
            // current.
            state.direct_compositing_reasons = full_context_.direct_compositing_reasons & CompositingReason::kDirectReasonsForFilterProperty;

            // If a filter node exists, add an additional direct compositing reason
            // for 3d transforms and will-change:transform to ensure it is composited.
            state.direct_compositing_reasons |= (full_context_.direct_compositing_reasons & CompositingReason::kAdditionalEffectCompositingTrigger);

            state.compositor_element_id = GetCompositorElementId(CompositorElementIdNamespace::kEffectFilter);

            state.self_or_ancestor_participates_in_view_transition = context_.self_or_ancestor_participates_in_view_transition;

            // This must be computed before std::move(state) below.
            bool needs_pixel_moving_filter_clip_expander
                = (state.direct_compositing_reasons & (CompositingReason::kWillChangeFilter | CompositingReason::kActiveFilterAnimation))
                || state.filter.HasFilterThatMovesPixels();

            EffectPaintPropertyNode::AnimationState animation_state;
            animation_state.is_running_filter_animation_on_compositor = object_.StyleRef().IsRunningFilterAnimationOnCompositor();
            OnUpdateEffect(properties_->UpdateFilter(*context_.current_effect, std::move(state), animation_state));

            if (needs_pixel_moving_filter_clip_expander) {
                OnUpdateClip(properties_->UpdatePixelMovingFilterClipExpander(
                    *context_.current.clip, ClipPaintPropertyNode::State(*context_.current.transform, properties_->Filter())));
            } else {
...
void FragmentPaintPropertyTreeBuilder::UpdateCssClip()
{
    ...
    if (NeedsPaintPropertyUpdate()) {
        if (NeedsCssClip(object_)) {
            ...
            const auto& clip_rect = To<LayoutBox>(object_).ClipRect(context_.current.paint_offset);
            OnUpdateClip(properties_->UpdateCssClip(
                *context_.current.clip, ClipPaintPropertyNode::State(*context_.current.transform, gfx::RectF(clip_rect), ToSnappedClipRect(clip_rect))));
        } else {
...
void FragmentPaintPropertyTreeBuilder::UpdateClipPathClip()
{
    if (NeedsPaintPropertyUpdate()) {
        ...
        if (NeedsClipPathClipOrMask(object_)) {
            clip_path_bounding_box_ = ClipPathClipper::LocalClipPathBoundingBox(object_);
            if (clip_path_bounding_box_) {
                ...
                const gfx::Vector2dF paint_offset = !object_.IsSVGChild() ? gfx::Vector2dF(context_.current.paint_offset) : gfx::Vector2dF();
                clip_path_bounding_box_->Offset(paint_offset);
                if (std::optional<Path> path = ClipPathClipper::PathBasedClip(object_)) {
                    ...
                    if (!(full_context_.direct_compositing_reasons
                            & (CompositingReason::kViewTransitionElement | CompositingReason::kViewTransitionElementDescendantWithClipPath))) {
                        rrect = PathToRRect(*path);
                    }
                    ClipPaintPropertyNode::State state(*context_.current.transform, *clip_path_bounding_box_,
                        rrect.value_or(FloatRoundedRect(gfx::ToEnclosingRect(*clip_path_bounding_box_))));
                    if (!rrect) {
                        state.clip_path = path;
                    }
                    OnUpdateClip(properties_->UpdateClipPathClip(*context_.current.clip, std::move(state)));
                } else {
...
static bool ReplacedElementAlwaysClipsToContentBox(const LayoutReplaced& replaced)
{
...
// this function.
static bool NeedsOverflowClipForReplacedContents(const LayoutReplaced& replaced)
{
...
static bool NeedsOverflowClip(const LayoutObject& object)
{
    if (const auto* replaced = DynamicTo<LayoutReplaced>(object)) {
        if (ReplacedElementAlwaysClipsToContentBox(*replaced) || replaced->ClipsToContentBox())
            return NeedsOverflowClipForReplacedContents(*replaced);
    }

    if (object.IsSVGViewportContainer() && SVGLayoutSupport::IsOverflowHidden(object))
        return true;
...
void FragmentPaintPropertyTreeBuilder::UpdateLocalBorderBoxContext()
{
    ...
    if (object_.HasLayer() || properties_ || IsLinkHighlighted(object_) || object_.CanContainFixedPositionObjects()
        ...
        fragment_data_.SetLocalBorderBoxProperties(PropertyTreeStateOrAlias(*new_transform, *new_clip, *new_effect));
    } else {
...
bool FragmentPaintPropertyTreeBuilder::NeedsOverflowControlsClip() const
{
    ...
    gfx::Rect scroll_controls_bounds = scrollable_area->ScrollCornerAndResizerRect();
    ...
    if (const auto* scrollbar = scrollable_area->VerticalScrollbar())
        scroll_controls_bounds.Union(scrollbar->FrameRect());
    gfx::Rect pixel_snapped_border_box_rect(gfx::Point(), scrollable_area->PixelSnappedBorderBoxSize());
...
static bool NeedsInnerBorderRadiusClip(const LayoutObject& object)
{
    ...
    if (object.IsLayoutReplaced() && ReplacedElementAlwaysClipsToContentBox(To<LayoutReplaced>(object))) {
        return false;
    }
    ...
    return object.StyleRef().HasBorderRadius() && object.IsBox() && NeedsOverflowClip(object) && object.ShouldClipOverflowAlongBothAxis();
}
...
void FragmentPaintPropertyTreeBuilder::UpdateOverflowControlsClip()
{
    ...
    if (NeedsOverflowControlsClip()) {
        ...
        const auto& clip_rect = PhysicalRect(context_.current.paint_offset, To<LayoutBox>(object_).Size());
...
static void AdjustRoundedClipForOverflowClipMargin(const LayoutBox& box, gfx::RectF& layout_clip_rect, FloatRoundedRect& paint_clip_rect)
{
    const auto& style = box.StyleRef();
...
void FragmentPaintPropertyTreeBuilder::UpdateInnerBorderRadiusClip()
{
    ...
    if (NeedsPaintPropertyUpdate()) {
        ...
        if (NeedsInnerBorderRadiusClip(object_)) {
            ...
            gfx::RectF layout_clip_rect = RoundedBorderGeometry::RoundedInnerBorder(box.StyleRef(), box_rect).Rect();
            FloatRoundedRect paint_clip_rect = RoundedBorderGeometry::PixelSnappedRoundedInnerBorder(box.StyleRef(), box_rect);

            gfx::Vector2dF offset(-OffsetInStitchedFragments(BoxFragment()));
            layout_clip_rect.Offset(offset);
            paint_clip_rect.Move(offset);

            AdjustRoundedClipForOverflowClipMargin(box, layout_clip_rect, paint_clip_rect);
            ClipPaintPropertyNode::State state(*context_.current.transform, layout_clip_rect, paint_clip_rect);
            OnUpdateClip(properties_->UpdateInnerBorderRadiusClip(*context_.current.clip, std::move(state)));
        } else {
...
void FragmentPaintPropertyTreeBuilder::UpdateOverflowClip()
{
    ...
    if (NeedsPaintPropertyUpdate()) {
        ...
        if (NeedsOverflowClip(object_)) {
            ClipPaintPropertyNode::State state(*context_.current.transform, gfx::RectF(), FloatRoundedRect());

            if (object_.IsLayoutReplaced() && ReplacedElementAlwaysClipsToContentBox(To<LayoutReplaced>(object_))) {
                ...
                if (IsA<LayoutVideo>(replaced)) {
                    content_rect = LayoutReplaced::PreSnappedRectForPersistentSizing(content_rect);
                }
                // LayoutReplaced clips the foreground by rounded content box.
                auto clip_rect = RoundedBorderGeometry::PixelSnappedRoundedBorderWithOutsets(replaced.StyleRef(), content_rect,
                    PhysicalBoxStrut(-(replaced.PaddingTop() + replaced.BorderTop()), -(replaced.PaddingRight() + replaced.BorderRight()),
                        -(replaced.PaddingBottom() + replaced.BorderBottom()), -(replaced.PaddingLeft() + replaced.BorderLeft())));
                if (replaced.IsLayoutEmbeddedContent()) {
                    ...
                    clip_rect.SetRect(gfx::RectF(clip_rect.Rect().origin(), gfx::SizeF(replaced.ReplacedContentRect().size)));
                }
                ...
                PhysicalRect clip_rect = box_fragment.OverflowClipRect(context_.current.paint_offset, FindPreviousBreakToken(box_fragment));
                ...
                state.layout_clip_rect_excluding_overlay_scrollbars = FloatClipRect(
                    gfx::RectF(To<LayoutBox>(object_).OverflowClipRect(context_.current.paint_offset, kExcludeOverlayScrollbarSizeForHitTesting)));
            } else {
                DCHECK(object_.IsSVGViewportContainer());
                const auto& viewport_container = To<LayoutSVGViewportContainer>(object_);
                const auto clip_rect = viewport_container.LocalToSVGParentTransform().Inverse().MapRect(viewport_container.Viewport());
                // TODO(crbug.com/1248598): Should we use non-snapped clip rect for
                // the first parameter?
                state.SetClipRect(clip_rect, FloatRoundedRect(clip_rect));
            }
            OnUpdateClip(properties_->UpdateOverflowClip(*context_.current.clip, std::move(state)));
        } else {
...
void FragmentPaintPropertyTreeBuilder::UpdatePerspective()
{
    ...
    if (NeedsPaintPropertyUpdate()) {
        if (NeedsPerspective(object_)) {
            ...
            TransformPaintPropertyNode::State state { { matrix,
                gfx::Point3F(PerspectiveOrigin(To<LayoutBox>(object_)) + gfx::Vector2dF(context_.current.paint_offset)) } };
            state.flattens_inherited_transform = context_.should_flatten_inherited_transform;
            state.rendering_context_id = context_.rendering_context_id;
            OnUpdateTransform(properties_->UpdatePerspective(*context_.current.transform, std::move(state)));
        } else {
...
void FragmentPaintPropertyTreeBuilder::UpdateReplacedContentTransform()
{
    ...
    if (NeedsPaintPropertyUpdate() && !NeedsReplacedContentTransform(object_)) {
        ...
        if (object_.IsSVGRoot()) {
            content_to_parent_space = SVGRootPainter(To<LayoutSVGRoot>(object_)).TransformToPixelSnappedBorderBox(context_.current.paint_offset);
        } else if (object_.IsLayoutEmbeddedContent()) {
            content_to_parent_space = To<LayoutEmbeddedContent>(object_).EmbeddedContentTransform();
        }
        if (!content_to_parent_space.IsIdentity()) {
            ...
            state.flattens_inherited_transform = context_.should_flatten_inherited_transform;
            state.rendering_context_id = context_.rendering_context_id;
            OnUpdateTransform(properties_->UpdateReplacedContentTransform(*context_.current.transform, std::move(state)));
        } else {
            OnClearTransform(properties_->ClearReplacedContentTransform());
        }
    }
...
MainThreadScrollingReasons FragmentPaintPropertyTreeBuilder::GetMainThreadRepaintReasonsForScroll(bool user_scrollable) const
{
    ...
    if (full_context_.requires_main_thread_for_background_attachment_fixed) {
        reasons |= cc::MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects;
    }
...
void FragmentPaintPropertyTreeBuilder::UpdateScrollAndScrollTranslation()
{
    ...
    if (NeedsPaintPropertyUpdate()) {
        if (NeedsScrollAndScrollTranslation(object_, full_context_.direct_compositing_reasons)) {
            UpdateScrollNode();
    ...
    if (const auto* scroll_translation = properties_->ScrollTranslation()) {
        ...
        context_.current.paint_offset += PhysicalOffset(To<LayoutBox>(object_).ScrollOrigin());
        // A scroller creates a layout shift root, so we just calculate one scroll
        // offset delta without accumulation.
        context_.current.scroll_offset_to_layout_shift_root_delta = scroll_translation->Get2dTranslation() - full_context_.old_scroll_offset;
    }
}
...
{
    DCHECK(NeedsPaintPropertyUpdate());
    DCHECK(NeedsScrollAndScrollTranslation(object_, full_context_.direct_compositing_reasons));
    ...
    state.contents_size = scrollable_area->PixelSnappedContentsSize(clip_rect.offset);
    state.overflow_clip_node = properties_->OverflowClip();
    state.user_scrollable_horizontal = scrollable_area->UserInputScrollable(kHorizontalScrollbar);
    state.user_scrollable_vertical = scrollable_area->UserInputScrollable(kVerticalScrollbar);
    ...
    state.composited_scrolling_preference = static_cast<CompositedScrollingPreference>(full_context_.composited_scrolling_preference);
    state.main_thread_repaint_reasons = GetMainThreadRepaintReasonsForScroll(state.user_scrollable_horizontal || state.user_scrollable_vertical);

    state.compositor_element_id = scrollable_area->GetScrollElementId();

    state.overscroll_behavior = cc::OverscrollBehavior(static_cast<cc::OverscrollBehavior::Type>(box.StyleRef().OverscrollBehaviorX()),
        static_cast<cc::OverscrollBehavior::Type>(box.StyleRef().OverscrollBehaviorY()));

    if (auto* data = scrollable_area->GetSnapContainerData()) {
        state.snap_container_data = *data;
    }

    OnUpdateScroll(properties_->UpdateScroll(*context_.current.scroll, std::move(state)));
}
...
void FragmentPaintPropertyTreeBuilder::UpdateOverflowControlEffects()
{
    ...
    bool transition_forces_scrollbar_effect_nodes = object_.IsLayoutView() && ViewTransitionUtils::GetTransition(object_.GetDocument());
    ...
    if (const auto* clip_to_skip = properties_->InnerBorderRadiusClip() ? properties_->InnerBorderRadiusClip() : properties_->OverflowClip()) {
        output_clip = clip_to_skip->Parent();
    }

    auto* scrollable_area = To<LayoutBox>(object_).GetScrollableArea();
    auto setup_scrollbar_effect_node = [this, scrollable_area, transition_forces_scrollbar_effect_nodes, output_clip](ScrollbarOrientation orientation) {
        ...
        bool needs_effect_node = scrollbar && (transition_forces_scrollbar_effect_nodes || scrollbar_is_overlay);
        ...
        if (needs_effect_node) {
            ...
            effect_state.compositor_element_id = scrollable_area->GetScrollbarElementId(orientation);

            if (scrollbar_is_overlay) {
                effect_state.direct_compositing_reasons = CompositingReason::kActiveOpacityAnimation;
            }

            const EffectPaintPropertyNodeOrAlias* parent
                = transition_forces_scrollbar_effect_nodes ? &EffectPaintPropertyNode::Root() : context_.current_effect;

            PaintPropertyChangeType change_type = orientation == ScrollbarOrientation::kHorizontalScrollbar
                ? properties_->UpdateHorizontalScrollbarEffect(*parent, std::move(effect_state))
                : properties_->UpdateVerticalScrollbarEffect(*parent, std::move(effect_state));
            OnUpdateEffect(change_type);
        } else {
            bool result = orientation == ScrollbarOrientation::kHorizontalScrollbar ? properties_->ClearHorizontalScrollbarEffect()
    ...
    bool has_scroll_corner
        = scrollable_area->HorizontalScrollbar() && scrollable_area->VerticalScrollbar() && !scrollable_area->VerticalScrollbar()->IsOverlayScrollbar();
    DCHECK(!has_scroll_corner || !scrollable_area->HorizontalScrollbar()->IsOverlayScrollbar());
    ...
    if (transition_forces_scrollbar_effect_nodes && has_scroll_corner) {
        ...
        effect_state.local_transform_space = context_.current.transform;
        effect_state.output_clip = output_clip;
        effect_state.compositor_element_id = scrollable_area->GetScrollCornerElementId();
        OnUpdateEffect(properties_->UpdateScrollCornerEffect(EffectPaintPropertyNode::Root(), std::move(effect_state)));
    } else {
        OnClearEffect(properties_->ClearScrollCornerEffect());
    }
}
...
void FragmentPaintPropertyTreeBuilder::UpdateScrollTranslation()
{
    ...
    TransformPaintPropertyNode::State state { { gfx::Transform::MakeTranslation(-scroll_position.OffsetFromOrigin()) } };
    if (!scrollable_area->PendingScrollAnchorAdjustment().IsZero()) {
        context_.current.pending_scroll_anchor_adjustment += scrollable_area->PendingScrollAnchorAdjustment();
        scrollable_area->ClearPendingScrollAnchorAdjustment();
    }
    state.flattens_inherited_transform = context_.should_flatten_inherited_transform;
    state.rendering_context_id = context_.rendering_context_id;
    state.direct_compositing_reasons = full_context_.direct_compositing_reasons & CompositingReason::kDirectReasonsForScrollTranslationProperty;
    ...
    auto effective_change_type = properties_->UpdateScrollTranslation(*context_.current.transform, std::move(state));
    ...
    if (effective_change_type <= PaintPropertyChangeType::kChangedOnlySimpleValues &&
        ...
        properties_->Scroll()) {
        if (auto* paint_artifact_compositor = object_.GetFrameView()->GetPaintArtifactCompositor()) {
            bool updated = paint_artifact_compositor->DirectlyUpdateScrollOffsetTransform(*properties_->ScrollTranslation());
            if (updated && effective_change_type == PaintPropertyChangeType::kChangedOnlySimpleValues) {
                effective_change_type = PaintPropertyChangeType::kChangedOnlyCompositedValues;
                properties_->ScrollTranslation()->CompositorSimpleValuesUpdated();
                if (paint_artifact_compositor->UsesRasterInducingScroll(*properties_->Scroll())) {
...
void FragmentPaintPropertyTreeBuilder::UpdateOutOfFlowContext()
{
    ...
    if (IsA<LayoutView>(object_)) {
        ...
        if (properties_->ScrollTranslation()) {
            ...
            context_.fixed_position.paint_offset -= PhysicalOffset(To<LayoutBox>(object_).ScrollOrigin());
        }
        ...
        if (context_.fixed_position.clip == css_clip->Parent()) {
            ...
            if (NeedsPaintPropertyUpdate()) {
                OnUpdateClip(properties_->UpdateCssClipFixedPosition(*context_.fixed_position.clip,
                    ClipPaintPropertyNode::State(css_clip->LocalTransformSpace(), css_clip->LayoutClipRect().Rect(), css_clip->PaintClipRect())));
            }
...
void FragmentPaintPropertyTreeBuilder::UpdateTransformIsolationNode()
{
    if (NeedsPaintPropertyUpdate()) {
        if (NeedsIsolationNodes(object_)) {
            OnUpdateTransform(properties_->UpdateTransformIsolationNode(*context_.current.transform));
        } else {
...
void FragmentPaintPropertyTreeBuilder::UpdateEffectIsolationNode()
{
    if (NeedsPaintPropertyUpdate()) {
        if (NeedsIsolationNodes(object_)) {
            OnUpdateEffect(properties_->UpdateEffectIsolationNode(*context_.current_effect));
        } else {
...
void FragmentPaintPropertyTreeBuilder::UpdateClipIsolationNode()
{
    if (NeedsPaintPropertyUpdate()) {
        if (NeedsIsolationNodes(object_)) {
            OnUpdateClip(properties_->UpdateClipIsolationNode(*context_.current.clip));
        } else {
...
void FragmentPaintPropertyTreeBuilder::UpdatePaintOffset()
{
    if (object_.IsBoxModelObject()) {
        ...
        case EPosition::kAbsolute: {
            DCHECK_EQ(full_context_.container_for_absolute_position, box_model_object.Container());
        ...
        case EPosition::kFixed: {
            DCHECK_EQ(full_context_.container_for_fixed_position, box_model_object.Container());
    ...
    if (const auto* box = DynamicTo<LayoutBox>(&object_)) {
        if (pre_paint_info_) {
            ...
            context_.current.is_in_block_fragmentation = pre_paint_info_->fragmentainer_is_oof_containing_block && !BoxFragment().IsMonolithic();
        } else {
    ...
    context_.current.additional_offset_to_layout_shift_root_delta += context_.pending_additional_offset_to_layout_shift_root_delta;
    context_.pending_additional_offset_to_layout_shift_root_delta = PhysicalOffset();
}
...
void FragmentPaintPropertyTreeBuilder::SetNeedsPaintPropertyUpdateIfNeeded()
{
    ...
    if (box.IsLayoutReplaced() && box.PreviousPhysicalContentBoxRect() != box.PhysicalContentBoxRect()) {
        box.GetMutableForPainting().SetOnlyThisNeedsPaintPropertyUpdate();
        if (box.IsLayoutEmbeddedContent()) {
            if (const auto* child_view = To<LayoutEmbeddedContent>(box).ChildLayoutView()) {
                child_view->GetMutableForPainting().SetOnlyThisNeedsPaintPropertyUpdate();
            }
        }
    }
...
void FragmentPaintPropertyTreeBuilder::UpdateForObjectLocation(std::optional<gfx::Vector2d>& paint_offset_translation)
{
    context_.old_paint_offset = fragment_data_.PaintOffset();
    UpdatePaintOffset();
    UpdateForPaintOffsetTranslation(paint_offset_translation);

    PhysicalOffset paint_offset_delta = fragment_data_.PaintOffset() - context_.current.paint_offset;
    if (!paint_offset_delta.IsZero() && !PrePaintDisableSideEffectsScope::IsDisabled()) {
        // Many paint properties depend on paint offset so we force an update of
        // the entire subtree on paint offset changes.
        full_context_.force_subtree_update_reasons |= PaintPropertyTreeBuilderContext::kSubtreeUpdateIsolationBlocked;
...
static bool IsLayoutShiftRoot(const LayoutObject& object, const FragmentData& fragment)
{
    ...
    if (IsA<LayoutView>(object))
        return true;
    for (const TransformPaintPropertyNode* transform : properties->AllCSSTransformPropertiesOutsideToInside()) {
...
void FragmentPaintPropertyTreeBuilder::UpdateForSelf()
{
    ...
    bool should_check_paint_under_invalidation
        = RuntimeEnabledFeatures::PaintUnderInvalidationCheckingEnabled() && !PrePaintDisableSideEffectsScope::IsDisabled();
    std::optional<FindPaintOffsetNeedingUpdateScope> check_paint_offset;
    if (should_check_paint_under_invalidation) {
        check_paint_offset.emplace(object_, fragment_data_, full_context_.is_actually_needed);
    }
#endif
    ...
    if (should_check_paint_under_invalidation) {
        ...
        check_paint_properties.emplace(object_, fragment_data_, force_subtree_update);
    }
#endif
    ...
    if (properties_) {
        ...
        const bool delegate_effects_to_view_transition = ViewTransitionUtils::ShouldDelegateEffectsAndBoxDecorationsToViewTransitionGroup(object_);
    ...
    if (IsA<LayoutView>(object_)) {
        context_.current.additional_offset_to_layout_shift_root_delta = PhysicalOffset();
        context_.translation_2d_to_layout_shift_root_delta = gfx::Vector2dF();
        context_.current.scroll_offset_to_layout_shift_root_delta = gfx::Vector2dF();
    }
}
...
void FragmentPaintPropertyTreeBuilder::UpdateForChildren()
{
    ...
    std::optional<FindPaintOffsetNeedingUpdateScope> check_paint_offset;
    std::optional<FindPropertiesNeedingUpdateScope> check_paint_properties;
    if (RuntimeEnabledFeatures::PaintUnderInvalidationCheckingEnabled() && !PrePaintDisableSideEffectsScope::IsDisabled()) {
        check_paint_offset.emplace(object_, fragment_data_, needs_paint_offset_update);
        bool force_subtree_update = full_context_.force_subtree_update_reasons;
        check_paint_properties.emplace(object_, fragment_data_, force_subtree_update);
    }
#endif
    ...
    context_.can_inherit_backface_visibility = context_.should_flatten_inherited_transform;
    ...
    if (full_context_.was_layout_shift_root || is_layout_shift_root) {
        ...
        context_.current.additional_offset_to_layout_shift_root_delta = context_.old_paint_offset - fragment_data_.PaintOffset();
        context_.translation_2d_to_layout_shift_root_delta = gfx::Vector2dF();
...
void FragmentPaintPropertyTreeBuilder::UpdateLayoutShiftRootChanged(bool is_layout_shift_root)
{
...
{
    bool needs_paint_properties = ObjectTypeMightNeedPaintProperties()
        && (NeedsPaintOffsetTranslation(object_, context_.direct_compositing_reasons, context_.container_for_fixed_position, context_.painting_layer)
            || NeedsStickyTranslation(object_) || NeedsAnchorPositionScrollTranslation(object_) || NeedsTranslate(object_, context_.direct_compositing_reasons)
            || NeedsRotate(object_, context_.direct_compositing_reasons) || NeedsScale(object_, context_.direct_compositing_reasons)
            || NeedsOffset(object_, context_.direct_compositing_reasons) || NeedsTransform(object_, context_.direct_compositing_reasons)
            || NeedsEffectIgnoringClipPath(object_, context_.direct_compositing_reasons) || NeedsClipPathClipOrMask(object_)
            || NeedsTransformForSVGChild(object_, context_.direct_compositing_reasons) || NeedsFilter(object_, context_) || NeedsCssClip(object_)
            || NeedsBackgroundClip(object_) || NeedsInnerBorderRadiusClip(object_) || NeedsOverflowClip(object_) || NeedsPerspective(object_)
            || NeedsReplacedContentTransform(object_) || NeedsScrollAndScrollTranslation(object_, context_.direct_compositing_reasons));
    ...
    if (const auto* properties = fragment.PaintProperties()) {
        if (const auto* translation = properties->PaintOffsetTranslation()) {
            ...
            context_.fragment_context.pending_additional_offset_to_layout_shift_root_delta
        ...
        context_.fragment_context.translation_2d_to_layout_shift_root_delta -= translation2d;
    }
    ...
    if (object_.IsSVGHiddenContainer()) {
        ...
        PaintPropertyTreeBuilderFragmentContext& fragment_context = context_.fragment_context;
        fragment_context.current.paint_offset_root = fragment_context.absolute_position.paint_offset_root = fragment_context.fixed_position.paint_offset_root
            = &object_;
    ...
    if (object_.HasLayer()) {
        To<LayoutBoxModelObject>(object_).Layer()->SetIsUnderSVGHiddenContainer(context_.has_svg_hidden_container_ancestor);
    }
}
...
void PaintPropertyTreeBuilder::UpdateFragmentData()
{
    ...
    if (IsInNGFragmentTraversal()) {
        context_.fragment_context.current.fragmentainer_idx = pre_paint_info_->fragmentainer_idx;
    } else {
        DCHECK_EQ(&fragment, &object_.FirstFragment());
        const FragmentDataList& fragment_list = object_.GetMutableForPainting().FragmentList();
        ...
        if (context_.fragment_context.current.fragmentainer_idx == WTF::kNotFound) {
            ...
            fragment.SetFragmentID(context_.fragment_context.current.fragmentainer_idx);
        }
    }
}
...
{
    if (object_.HasLayer() && To<LayoutBoxModelObject>(object_).HasSelfPaintingLayer()) {
...
void PaintPropertyTreeBuilder::UpdateForSelf()
{
    ...
    context_.direct_compositing_reasons = CompositingReasonFinder::DirectReasonsForPaintProperties(object_, context_.container_for_fixed_position);
    if (const auto* box = DynamicTo<LayoutBox>(object_)) {
        box->GetMutableForPainting().UpdateBackgroundPaintLocation();
        if (auto* scrollable_area = box->GetScrollableArea()) {
            bool force_prefer_compositing = CompositingReasonFinder::ShouldForcePreferCompositingToLCDText(object_, context_.direct_compositing_reasons);
            context_.composited_scrolling_preference = static_cast<unsigned>(force_prefer_compositing ? CompositedScrollingPreference::kPreferred
                    : scrollable_area->PrefersNonCompositedScrolling()                                ? CompositedScrollingPreference::kNotPreferred
    ...
    if (Platform::Current()->IsLowEndDevice()) {
        ...
        context_.direct_compositing_reasons &= ~CompositingReason::kTrivial3DTransform;
    }

    if (context_.fragment_context.self_or_ancestor_participates_in_view_transition && object_.StyleRef().HasClipPath()) {
        context_.direct_compositing_reasons |= CompositingReason::kViewTransitionElementDescendantWithClipPath;
    }

    context_.was_layout_shift_root = IsLayoutShiftRoot(object_, object_.FirstFragment());
    ...
    // object.
    const bool is_in_fragment_container = pre_paint_info_ && pre_paint_info_->fragmentainer_is_oof_containing_block && IsA<LayoutBox>(object_)
    ...
    FragmentPaintPropertyTreeBuilder builder(object_, pre_paint_info_, context_, GetFragmentData());
    builder.UpdateForSelf();
    properties_changed_.Merge(builder.PropertiesChanged());

    if (!PrePaintDisableSideEffectsScope::IsDisabled()) {
        object_.GetMutableForPainting().SetShouldAssumePaintOffsetTranslationForLayoutShiftTracking(false);
    }
}
...
void PaintPropertyTreeBuilder::UpdateGlobalMainThreadRepaintReasonsForScroll()
{
    ...
    if (object_.GetFrameView()->RequiresMainThreadScrollingForBackgroundAttachmentFixed()) {
    ...
    if (auto* properties = object_.FirstFragment().PaintProperties()) {
        if (auto* scroll = properties->Scroll()) {
            if (scroll->RequiresMainThreadForBackgroundAttachmentFixed() != context_.requires_main_thread_for_background_attachment_fixed) {
                // The changed requires_main_thread_for_background_attachment_fixed
                // needs to propagate to all scroll nodes in this view.
                context_.force_subtree_update_reasons |= PaintPropertyTreeBuilderContext::kSubtreeUpdateIsolationPiercing;
            }
        }
    }
}
...
void PaintPropertyTreeBuilder::UpdateForChildren()
{
    ...
    if (properties_changed_.Max() >= PaintPropertyChangeType::kNodeAddedOrRemoved
        || object_.SubtreePaintPropertyUpdateReasons() != static_cast<unsigned>(SubtreePaintPropertyUpdateReason::kNone)) {
        ...
        if (properties_changed_.scroll_changed >= PaintPropertyChangeType::kNodeAddedOrRemoved
            || AreSubtreeUpdateReasonsIsolationPiercing(object_.SubtreePaintPropertyUpdateReasons())) {
            context_.force_subtree_update_reasons |= PaintPropertyTreeBuilderContext::kSubtreeUpdateIsolationPiercing;
        } else {
            context_.force_subtree_update_reasons |= PaintPropertyTreeBuilderContext::kSubtreeUpdateIsolationBlocked;
        }
    }

    if (properties_changed_.transform_changed > (properties_changed_.transform_change_is_scroll_translation_only
                ? PaintPropertyChangeType::kChangedOnlySimpleValues
                : PaintPropertyChangeType::kUnchanged)
        || properties_changed_.clip_changed > PaintPropertyChangeType::kUnchanged || properties_changed_.scroll_changed > PaintPropertyChangeType::kUnchanged) {
        object_.GetFrameView()->SetIntersectionObservationState(LocalFrameView::kDesired);
    }
...
void PaintPropertyTreeBuilder::UpdateForPageBorderBox(const PhysicalBoxFragment& page_container)
{
    ...
    gfx::Transform matrix = gfx::Transform::MakeTranslation(gfx::Vector2dF(page_border_box_offset));
    matrix.Scale(scale);
    ...
    fragment_data.EnsurePaintProperties().UpdateTransform(*fragment_context.current.transform, std::move(transform_state));
    fragment_data.SetLocalBorderBoxProperties(
        PropertyTreeStateOrAlias(*fragment_data.PaintProperties()->Transform(), *fragment_context.current.clip, *fragment_context.current_effect));
}

bool PaintPropertyTreeBuilder::ScheduleDeferredTransformNodeUpdate(LayoutObject& object)
{
...
bool PaintPropertyTreeBuilder::ScheduleDeferredOpacityNodeUpdate(LayoutObject& object)
{
...
void PaintPropertyTreeBuilder::DirectlyUpdateTransformMatrix(const LayoutObject& object)
{
    ...
    auto transform_and_origin
        = TransformAndOriginState(box, reference_box, [](const LayoutBox& box, const PhysicalRect& reference_box, gfx::Transform& matrix) {
              const ComputedStyle& style = box.StyleRef();
              style.ApplyTransform(matrix, &box, reference_box, ComputedStyle::kIncludeTransformOperations, ComputedStyle::kExcludeTransformOrigin,
                  ComputedStyle::kExcludeMotionPath, ComputedStyle::kExcludeIndependentTransformProperties);
          });
    ...
    TransformPaintPropertyNode::AnimationState animation_state;
    animation_state.is_running_animation_on_compositor = box.StyleRef().IsRunningTransformAnimationOnCompositor();
    auto effective_change_type = properties->DirectlyUpdateTransformAndOrigin(std::move(transform_and_origin), animation_state);
    ...
    if (effective_change_type > PaintPropertyChangeType::kUnchanged) {
        object.GetFrameView()->SetIntersectionObservationState(LocalFrameView::kDesired);
    }
...
void PaintPropertyTreeBuilder::DirectlyUpdateOpacityValue(const LayoutObject& object)
{
    ...
    FragmentData* fragment_data = &object.GetMutableForPainting().FirstFragment();
    auto* properties = fragment_data->PaintProperties();
    auto effective_change_type = properties->DirectlyUpdateOpacity(style.Opacity(), animation_state);
    // If we have simple value change, which means opacity, we should try to
    // directly update it on the PaintArtifactCompositor in order to avoid
    ...
    if (max_change >= PaintPropertyChangeType::kNodeAddedOrRemoved) {
        ...
        if (object_.IsDocumentElement()) {
            ...
            if (auto* scrollable_area = layout_view->GetScrollableArea()) {
                scrollable_area->GetScrollingBackgroundDisplayItemClient().Invalidate(reason);
            }
        }
    }
...
bool PaintPropertyTreeBuilder::CanDoDeferredTransformNodeUpdate(const LayoutObject& object)
{
    // If we already need a full update, do not do the direct update.
    if (object.NeedsPaintPropertyUpdate() || object.DescendantNeedsPaintPropertyUpdate()) {
        return false;
    }
...
bool PaintPropertyTreeBuilder::CanDoDeferredOpacityNodeUpdate(const LayoutObject& object)
{
    // If we already need a full update, do not do the direct update.
    if (object.NeedsPaintPropertyUpdate() || object.DescendantNeedsPaintPropertyUpdate()) {
        return false;
