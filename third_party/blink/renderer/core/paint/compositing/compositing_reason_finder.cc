...
bool ShouldPreferCompositingForLayoutView(const LayoutView& layout_view)
{
    ...
    auto has_direct_compositing_reasons = [](const LayoutObject* object) -> bool {
        return object && CompositingReasonFinder::DirectReasonsForPaintProperties(*object) != CompositingReason::kNone;
    };
    if (has_direct_compositing_reasons(layout_view.GetFrame()->OwnerLayoutObject()))
        return true;
...
CompositingReasons BackfaceInvisibility3DAncestorReason(const PaintLayer& layer)
{
    if (RuntimeEnabledFeatures::BackfaceVisibilityInteropEnabled()) {
        if (auto* compositing_container = layer.CompositingContainer()) {
            if (compositing_container->GetLayoutObject().StyleRef().BackfaceVisibility() == EBackfaceVisibility::kHidden)
...
CompositingReasons CompositingReasonsForWillChange(const ComputedStyle& style)
{
    ...
    // reasons are set.
    if (reasons == CompositingReason::kNone && style.HasWillChangeCompositingHint())
        reasons |= CompositingReason::kWillChangeOther;

    return reasons;
}

CompositingReasons CompositingReasonsFor3DTransform(const LayoutObject& layout_object)
{
    ...
    CompositingReasons reasons = CompositingReasonFinder::PotentialCompositingReasonsFor3DTransform(style);
    if (reasons != CompositingReason::kNone && layout_object.IsBox()) {
        ...
        style.ApplyTransform(matrix, &box, reference_box, ComputedStyle::kIncludeTransformOperations, ComputedStyle::kExcludeTransformOrigin,
            ComputedStyle::kExcludeMotionPath, ComputedStyle::kIncludeIndependentTransformProperties);
        ...
        if (matrix.Creates3d()) {
            LayoutObject* parent_for_element = layout_object.NearestAncestorForElement();
            if (parent_for_element && parent_for_element->Preserves3D()) {
                UseCounter::Count(layout_object.GetDocument(), WebFeature::kTransform3dScene);
            }
        }
    }
    return reasons;
...
CompositingReasons CompositingReasonsFor3DSceneLeaf(const LayoutObject& layout_object)
{
    ...
    if (!layout_object.IsAnonymous() && !layout_object.StyleRef().Preserves3D()) {
        const LayoutObject* parent_object = layout_object.NearestAncestorForElement();
...
CompositingReasons DirectReasonsForSVGChildPaintProperties(const LayoutObject& object)
{
...
CompositingReasons CompositingReasonsForViewportScrollEffect(const LayoutObject& layout_object, const LayoutObject* container_for_fixed_position)
{
    ...
    if (!frame->IsMainFrame() && frame->GetDocument() != controller.GlobalRootScroller()) {
        return CompositingReason::kNone;
    }
    ...
    if (frame->GetPage()->GetVisualViewport().GetOverscrollType() == OverscrollType::kTransform) {
        reasons |= CompositingReason::kFixedPosition;
        if (!To<LayoutBox>(layout_object).AnchorPositionScrollAdjustmentAfectedByViewportScrolling()) {
            reasons |= CompositingReason::kUndoOverscroll;
        }
    }

    if (layout_object.StyleRef().IsFixedToBottom()) {
        reasons |= CompositingReason::kFixedPosition | CompositingReason::kAffectedByOuterViewportBoundsDelta;
    }

    return reasons;
}

CompositingReasons CompositingReasonsForScrollDependentPosition(const PaintLayer& layer, const LayoutObject* container_for_fixed_position)
{
    CompositingReasons reasons = CompositingReason::kNone;
    ...
    if (const auto* constraints = layer.GetLayoutObject().StickyConstraints()) {
        if (!constraints->is_fixed_to_view && constraints->containing_scroll_container_layer->GetScrollableArea()->HasOverflow())
...
bool ObjectTypeSupportsCompositedTransformAnimation(const LayoutObject& object)
{
    if (object.IsSVGChild()) {
        // Transforms are not supported on hidden containers, inlines, text, or
        // filter primitives.
        return !object.IsSVGHiddenContainer() && !object.IsLayoutInline() && !object.IsText() && !object.IsSVGFilterPrimitive();
    }
...
CompositingReasons CompositingReasonFinder::DirectReasonsForPaintProperties(const LayoutObject& object, const LayoutObject* container_for_fixed_position)
{
    ...
    const ComputedStyle& style = object.StyleRef();
    reasons |= CompositingReasonsForAnimation(object) | CompositingReasonsForWillChange(style);
    ...
    reasons |= CompositingReasonsForScrollDependentPosition(*layer, container_for_fixed_position);

    reasons |= CompositingReasonsForViewportScrollEffect(object, container_for_fixed_position);
    ...
    if (auto* transition = ViewTransitionUtils::GetTransition(object.GetDocument())) {
...
bool CompositingReasonFinder::ShouldForcePreferCompositingToLCDText(const LayoutObject& object, CompositingReasons reasons)
{
...
CompositingReasons CompositingReasonFinder::PotentialCompositingReasonsFor3DTransform(const ComputedStyle& style)
{
    CompositingReasons reasons = CompositingReason::kNone;
    ...
    if (style.Rotate() && (style.Rotate()->X() != 0 || style.Rotate()->Y() != 0)) {
...
CompositingReasons CompositingReasonFinder::CompositingReasonsForAnimation(const LayoutObject& object)
{
    CompositingReasons reasons = CompositingReason::kNone;
    ...
    if (style.HasCurrentTransformAnimation() && ObjectTypeSupportsCompositedTransformAnimation(object))
        reasons |= CompositingReason::kActiveTransformAnimation;
    if (style.HasCurrentScaleAnimation() && ObjectTypeSupportsCompositedTransformAnimation(object))
        reasons |= CompositingReason::kActiveScaleAnimation;
    if (style.HasCurrentRotateAnimation() && ObjectTypeSupportsCompositedTransformAnimation(object))
        reasons |= CompositingReason::kActiveRotateAnimation;
    if (style.HasCurrentTranslateAnimation() && ObjectTypeSupportsCompositedTransformAnimation(object))
...
bool CompositingReasonFinder::RequiresCompositingForRootScroller(const LayoutObject& object)
{
