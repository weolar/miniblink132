...
public:
    void SetUp() override
    {
        ...
        GetLocalFrameView()->GetFrame().GetSettings()->SetPreferCompositingToLCDTextForTesting(true);
    ...
    {
        return web_view_helper_->LocalMainFrame()->FrameWidgetImpl()->LayerTreeHostForTesting();
    }
    ...
    {
        GetLocalFrameView()->UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason::kTest);
    }
    ...
    {
        return GetPropertyTrees()->transform_tree_mutable().Node(layer->transform_tree_index());
    }
...
TEST_P(CompositingTest, DidScrollCallbackAfterScrollableAreaChanges)
{
    ...
    auto* overflow_scroll_layer = CcLayerByCcElementId(RootCcLayer(), scroll_element_id);
    const auto* scroll_node = RootCcLayer()->layer_tree_host()->property_trees()->scroll_tree().FindNodeFromElementId(scroll_element_id);
    ...
    commit_data.scrolls.push_back({ scroll_element_id, gfx::Vector2dF(0, 1), std::nullopt });
    overflow_scroll_layer->layer_tree_host()->ApplyCompositorChanges(&commit_data);
    UpdateAllLifecyclePhases();
    EXPECT_EQ(ScrollOffset(0, 1), scrollable_area->GetScrollOffset());

    // Make the scrollable area non-scrollable.
    scrollable->setAttribute(html_names::kStyleAttr, AtomicString("overflow: visible"));
    ...
    ASSERT_EQ(overflow_scroll_layer, CcLayerByCcElementId(RootCcLayer(), scroll_element_id));
    commit_data.scrolls[0] = { scroll_element_id, gfx::Vector2dF(0, 1), std::nullopt };
    overflow_scroll_layer->layer_tree_host()->ApplyCompositorChanges(&commit_data);
...
TEST_P(CompositingTest, FrameViewScroll)
{
    ...
    const auto* scroll_node = RootCcLayer()->layer_tree_host()->property_trees()->scroll_tree().FindNodeFromElementId(scrollable_area->GetScrollElementId());
    ...
    EXPECT_EQ(ScrollOffset(), scrollable_area->GetScrollOffset());
    cc::CompositorCommitData commit_data;
    commit_data.scrolls.push_back({ scrollable_area->GetScrollElementId(), gfx::Vector2dF(0, 1), std::nullopt });
...
TEST_P(CompositingTest, Compositing3DTransformOnSVGModelObject)
{
    ...
    target_element->setAttribute(html_names::kStyleAttr, AtomicString("transform: translate3d(0, 0, 1px)"));
    UpdateAllLifecyclePhases();
    ...
    target_element->setAttribute(html_names::kStyleAttr, AtomicString("transform: none"));
    UpdateAllLifecyclePhases();
    // |HasTransformRelatedProperty| is used in |CompositingReasonsFor3DTransform|
    // and must be set correctly.
    ...
    target_element->setAttribute(html_names::kStyleAttr, AtomicString("transform: translate(1px, 0)"));
    UpdateAllLifecyclePhases();
    EXPECT_FALSE(CcLayerByDOMElementId("target"));
...
TEST_P(CompositingTest, NotCompositing3DTransformOnSVGInline)
{
    ...
    inline_element->setAttribute(html_names::kStyleAttr, AtomicString("transform: translate3d(0, 0, 1px)"));
    UpdateAllLifecyclePhases();
    // |HasTransformRelatedProperty| is used in |CompositingReasonsFor3DTransform|
    // and must be set correctly.
...
TEST_P(CompositingTest, PaintPropertiesWhenCompositingSVG)
{
    ...
    auto* ancestor_effect_node = GetPropertyTrees()->effect_tree_mutable().Node(ancestor->effect_tree_index());
    EXPECT_EQ(ancestor_effect_node->opacity, 0.9f);

    auto* svg_root = CcLayerByDOMElementId("svg");
    const auto* svg_root_effect_node = GetPropertyTrees()->effect_tree().Node(svg_root->effect_tree_index());
    ...
    const auto* rect_effect_node = GetPropertyTrees()->effect_tree().Node(rect->effect_tree_index());
...
TEST_P(CompositingTest, BackgroundColorInScrollingContentsLayer)
{
    ...
    ASSERT_TRUE(layout_view->GetBackgroundPaintLocation() == kBackgroundPaintInContentsSpace);
    ASSERT_TRUE(scroller_box->GetBackgroundPaintLocation() == kBackgroundPaintInContentsSpace);
...
TEST_P(CompositingTest, BackgroundColorInGraphicsLayer)
{
    ...
    Element* scroller = GetElementById("scroller");
    LayoutBox* scroller_box = scroller->GetLayoutBox();
    ASSERT_TRUE(layout_view->GetBackgroundPaintLocation() == kBackgroundPaintInBorderBoxSpace);
    ASSERT_TRUE(scroller_box->GetBackgroundPaintLocation() == kBackgroundPaintInBorderBoxSpace);
    ...
    // Non-root layers set background_color based on the CSS background color of
    // the layer-defining element.
    SkColor4f expected_color = SkColor4f::FromColor(SkColorSetARGB(roundf(255. * 0.6), 30, 40, 50));
    layer = CcLayerByDOMElementId("scroller");
    EXPECT_SKCOLOR4F_NEAR(layer->background_color(), expected_color, 0.005f);
    scrollable_area = scroller_box->GetScrollableArea();
    layer = ScrollingContentsCcLayerByScrollElementId(RootCcLayer(), scrollable_area->GetScrollElementId());
...
TEST_P(CompositingTest, CompositedOverlayScrollbarUnderNonFastBorderRadius)
{
    ...
    ASSERT_TRUE(GetLayoutObjectById("scroll1")->FirstFragment().PaintProperties()->VerticalScrollbarEffect());
    EXPECT_EQ(1u, CcLayersByName(RootCcLayer(), "Synthesized Clip").size());
}

// https://crbug.com/1459318
TEST_P(CompositingTest, FullPACUpdateOnScrollWithSyntheticClipAcrossScrollerSimpleRadius)
{
    InitializeWithHTML(*WebView()->MainFrameImpl()->GetFrame(), R"HTML(
...
// https://crbug.com/1459318
TEST_P(CompositingTest, FullPACUpdateOnScrollWithSyntheticClipAcrossScrollerComplexRadius)
{
    InitializeWithHTML(*WebView()->MainFrameImpl()->GetFrame(), R"HTML(
    <div id="scroll" style="width: 200px; height: 200px;
...
TEST_P(CompositingTest, HitTestOpaqueness)
{
    ...
    const auto hit_test_transparent = RuntimeEnabledFeatures::HitTestOpaquenessEnabled() ? cc::HitTestOpaqueness::kTransparent : cc::HitTestOpaqueness::kMixed;
    EXPECT_EQ(hit_test_transparent, CcLayersByDOMElementId(RootCcLayer(), "transparent1")[0]->hit_test_opaqueness());
    EXPECT_EQ(hit_test_transparent, CcLayersByDOMElementId(RootCcLayer(), "transparent2")[0]->hit_test_opaqueness());
    EXPECT_EQ(cc::HitTestOpaqueness::kMixed, CcLayersByDOMElementId(RootCcLayer(), "mixed1")[0]->hit_test_opaqueness());
    EXPECT_EQ(cc::HitTestOpaqueness::kMixed, CcLayersByDOMElementId(RootCcLayer(), "mixed2")[0]->hit_test_opaqueness());
    EXPECT_EQ(cc::HitTestOpaqueness::kMixed, CcLayersByDOMElementId(RootCcLayer(), "mixed3")[0]->hit_test_opaqueness());
    const auto hit_test_opaque = RuntimeEnabledFeatures::HitTestOpaquenessEnabled() ? cc::HitTestOpaqueness::kOpaque : cc::HitTestOpaqueness::kMixed;
    EXPECT_EQ(hit_test_opaque, CcLayersByDOMElementId(RootCcLayer(), "opaque1")[0]->hit_test_opaqueness());
    EXPECT_EQ(hit_test_opaque, CcLayersByDOMElementId(RootCcLayer(), "opaque2")[0]->hit_test_opaqueness());
    EXPECT_EQ(hit_test_opaque, CcLayersByDOMElementId(RootCcLayer(), "opaque3")[0]->hit_test_opaqueness());
    EXPECT_EQ(hit_test_opaque, CcLayersByDOMElementId(RootCcLayer(), "opaque4")[0]->hit_test_opaqueness());
    EXPECT_EQ(hit_test_opaque, CcLayersByDOMElementId(RootCcLayer(), "opaque5")[0]->hit_test_opaqueness());
    EXPECT_EQ(hit_test_opaque, CcLayersByDOMElementId(RootCcLayer(), "opaque6")[0]->hit_test_opaqueness());
}
...
TEST_P(CompositingTest, HitTestOpaquenessOfEmptyInline)
{
    ...
    auto* scrolling_contents = CcLayersByDOMElementId(RootCcLayer(), "scrollable").back();
...
TEST_P(CompositingTest, HitTestOpaquenessOnChangeOfUsedPointerEvents)
{
    ...
    const cc::Layer* target_layer = CcLayersByDOMElementId(RootCcLayer(), "target")[0];
...
TEST_P(CompositingTest, NonStackedScrollerWithRelativeChildAboveFixedAndAbsolute)
{
    GetLocalFrameView()->GetFrame().GetSettings()->SetPreferCompositingToLCDTextForTesting(false);
    ...
    GetElementById("fixed")->SetInlineStyleProperty(CSSPropertyID::kPosition, "absolute");
    UpdateAllLifecyclePhases();
...
TEST_P(CompositingTest, AnchorPositionAdjustmentTransformIdReference)
{
    ...
    int scroll_translation_id = GetElementById("scroller")
                                    ->GetLayoutObject()
                                    ->FirstFragment()
                                    .PaintProperties()
                                    ->ScrollTranslation()
                                    ->CcNodeId(LayerTreeHost()->property_trees()->sequence_number());
    EXPECT_LT(scroll_translation_id, CcLayersByDOMElementId(RootCcLayer(), "anchored1")[0]->transform_tree_index());
    EXPECT_LT(scroll_translation_id, CcLayersByDOMElementId(RootCcLayer(), "anchored2")[0]->transform_tree_index());
}
...
protected:
    ...
    {
        const gfx::Rect* actual = GetPropertyTrees()->scroll_tree().ScrollingContentsCullRect(
            GetLayoutObjectById(id)->FirstFragment().PaintProperties()->Scroll()->GetCompositorElementId());
        if (expected) {
            ASSERT_TRUE(actual);
...
TEST_P(ScrollingContentsCullRectTest, Basics)
{
    ...
    if (RuntimeEnabledFeatures::RasterInducingScrollEnabled()) {
        ...
        EXPECT_EQ(paint_artifact_compositor()->NeedsUpdate(), PaintArtifactCompositor::UpdateType::kRasterInducingScroll);
    } else {
...
TEST_P(ScrollingContentsCullRectTest, RepaintOnlyScroll)
{
    ...
    EXPECT_EQ(paint_artifact_compositor()->NeedsUpdate(), PaintArtifactCompositor::UpdateType::kNone);
    UpdateAllLifecyclePhases();
    EXPECT_EQ(paint_artifact_compositor()->PreviousUpdateForTesting(), PaintArtifactCompositor::UpdateType::kRepaint);
...
public:
    ...
    Element* GetElementById(const char* id)
    {
        return MainFrame().GetFrame()->GetDocument()->getElementById(AtomicString(id));
    }
    ...
    void UpdateAllLifecyclePhasesExceptPaint()
    {
        WebView().MainFrameWidget()->UpdateLifecycle(WebLifecycleUpdate::kPrePaint, DocumentUpdateReason::kTest);
    }
    ...
    {
        return GetPropertyTrees()->effect_tree_mutable().Node(layer->effect_tree_index());
    }
...
TEST_P(CompositingSimTest, LayerUpdatesDoNotInvalidateEarlierLayers)
{
    ...
    EXPECT_FALSE(host.pending_commit_state()->layers_that_should_push_properties.count(a_layer));
    EXPECT_FALSE(host.pending_commit_state()->layers_that_should_push_properties.count(b_layer));
    ...
    UpdateAllLifecyclePhases();
    EXPECT_FALSE(host.pending_commit_state()->layers_that_should_push_properties.count(a_layer));
    EXPECT_TRUE(host.pending_commit_state()->layers_that_should_push_properties.count(b_layer));
...
TEST_P(CompositingSimTest, LayerUpdatesDoNotInvalidateLaterLayers)
{
    ...
    EXPECT_FALSE(host.pending_commit_state()->layers_that_should_push_properties.count(a_layer));
    EXPECT_FALSE(host.pending_commit_state()->layers_that_should_push_properties.count(b_layer));
    EXPECT_FALSE(host.pending_commit_state()->layers_that_should_push_properties.count(c_layer));
    ...
    UpdateAllLifecyclePhases();
    EXPECT_TRUE(host.pending_commit_state()->layers_that_should_push_properties.count(a_layer));
    EXPECT_TRUE(host.pending_commit_state()->layers_that_should_push_properties.count(b_layer));
    EXPECT_FALSE(host.pending_commit_state()->layers_that_should_push_properties.count(c_layer));
...
TEST_P(CompositingSimTest, NoopChangeDoesNotCauseFullTreeSyncOrPropertyTreeUpdate)
{
    InitializeWithHTML(R"HTML(
      <!DOCTYPE html>
      <style>
...
TEST_P(CompositingSimTest, LayerSubtreeTransformPropertyChanged)
{
    ...
    // both layers.
    outer_element->setAttribute(html_names::kStyleAttr, AtomicString("transform: rotate(10deg)"));
    UpdateAllLifecyclePhases();
...
TEST_P(CompositingSimTest, DirectTransformPropertyUpdate)
{
    ...
    const auto* transform_node = GetPropertyTrees()->transform_tree().Node(transform_tree_index);
    ...
    outer_element->setAttribute(html_names::kStyleAttr, AtomicString("animation-name: animateTransformB"));
    UpdateAllLifecyclePhasesExceptPaint();
...
TEST_P(CompositingSimTest, FastPathTransformUpdateFromStyle)
{
    ...
    auto* div_properties = div->GetLayoutObject()->FirstFragment().PaintProperties();
    ASSERT_TRUE(div_properties);
    EXPECT_EQ(gfx::Transform::MakeTranslation(100, 0), div_properties->Transform()->Matrix());
    ...
    div->setAttribute(html_names::kStyleAttr, AtomicString("transform: translateX(400px)"));
    ...
    // performed.
    UpdateAllLifecyclePhasesExceptPaint();
    EXPECT_EQ(gfx::Transform::MakeTranslation(400, 0), div_properties->Transform()->Matrix());
...
TEST_P(CompositingSimTest, FastPathOpacityUpdateFromStyle)
{
    ...
    const auto* effect_node = GetPropertyTrees()->effect_tree().Node(effect_tree_index);
...
TEST_P(CompositingSimTest, DirectSVGTransformPropertyUpdate)
{
    ...
    will_change_element->setAttribute(html_names::kStyleAttr, AtomicString("animation-name: animateTransformB"));
    UpdateAllLifecyclePhasesExceptPaint();
    EXPECT_TRUE(transform_node->transform_changed);
    EXPECT_EQ(paint_artifact_compositor()->NeedsUpdate(), PaintArtifactCompositor::UpdateType::kNone);
...
TEST_P(CompositingSimTest, DirectTransformPropertyUpdateCausesChange)
{
    ...
    const auto* inner_transform_node = GetPropertyTrees()->transform_tree().Node(inner_transform_tree_index);
    ...
    outer_element->setAttribute(html_names::kStyleAttr, AtomicString("animation-name: animateTransformB"));
    inner_element->setAttribute(html_names::kStyleAttr, AtomicString("transform: rotate(30deg)"));
    UpdateAllLifecyclePhasesExceptPaint();
...
TEST_P(CompositingSimTest, AffectedByOuterViewportBoundsDelta)
{
    ...
    // delta changes.
    {
        fixed_element->setAttribute(html_names::kStyleAttr, AtomicString("bottom: 0"));
        Compositor().BeginFrame();
...
TEST_P(CompositingSimTest, DirectTransformOriginPropertyUpdate)
{
    ...
    box_element->setAttribute(html_names::kStyleAttr, AtomicString("animation-name: animateTransformB"));
    UpdateAllLifecyclePhasesExceptPaint();
    EXPECT_TRUE(transform_node->transform_changed);
    EXPECT_EQ(paint_artifact_compositor()->NeedsUpdate(), PaintArtifactCompositor::UpdateType::kNone);
...
TEST_P(CompositingSimTest, LayerSubtreeEffectPropertyChanged)
{
    ...
    // both layers.
    outer_element->setAttribute(html_names::kStyleAttr, AtomicString("filter: blur(20px)"));
    UpdateAllLifecyclePhases();
    EXPECT_TRUE(outer_element_layer->subtree_property_changed());
    // Set by blink::PropertyTreeManager.
...
TEST_P(CompositingSimTest, LayerSubtreeClipPropertyChanged)
{
    ...
    // both layers.
    outer_element->setAttribute(html_names::kStyleAttr, AtomicString("clip: rect(1px, 8px, 7px, 4px);"));
    UpdateAllLifecyclePhases();
    EXPECT_TRUE(outer_element_layer->subtree_property_changed());
    EXPECT_TRUE(inner_element_layer->subtree_property_changed());
...
TEST_P(CompositingSimTest, LayerSubtreeOverflowClipPropertyChanged)
{
    ...
    // both layers.
    outer_element->setAttribute(html_names::kStyleAttr, AtomicString("width: 200px;"));
    UpdateAllLifecyclePhases();
...
TEST_P(CompositingSimTest, SafeOpaqueBackgroundColor)
{
    ...
    auto* opaque_image_translucent_color = CcLayerByDOMElementId("opaque-image-translucent-color");
    EXPECT_TRUE(opaque_image_translucent_color->contents_opaque());
    EXPECT_EQ(opaque_image_translucent_color->background_color(), kTranslucentCyan);
    // Use background_color() with the alpha channel forced to be opaque.
    EXPECT_EQ(opaque_image_translucent_color->SafeOpaqueBackgroundColor(), SkColors::kCyan);
...
TEST_P(CompositingSimTest, SquashingLayerSafeOpaqueBackgroundColor)
{
    ...
    // SafeOpaqueBackgroundColor() returns SK_ColorTRANSPARENT when
    // background_color() is opaque and contents_opaque() is false.
    EXPECT_EQ(squashing_layer->SafeOpaqueBackgroundColor(), SkColors::kTransparent);
}
...
TEST_P(CompositingSimTest, RootScrollingContentsSafeOpaqueBackgroundColor)
{
    ...
    auto* scrolling_contents = ScrollingContentsCcLayerByScrollElementId(RootCcLayer(), MainFrame().GetFrameView()->LayoutViewport()->GetScrollElementId());
...
TEST_P(CompositingSimTest, NoRenderSurfaceWithAxisAlignedTransformAnimation)
{
    ...
    for (const auto& effect_node : GetPropertyTrees()->effect_tree().nodes()) {
        EXPECT_NE(cc::RenderSurfaceReason::kClipAxisAlignment, effect_node.render_surface_reason);
    }
}
...
TEST_P(CompositingSimTest, PromoteCrossOriginIframe)
{
    ...
    Compositor().BeginFrame();
    Document* iframe_doc = To<HTMLFrameOwnerElement>(GetElementById("iframe"))->contentDocument();
...
TEST_P(CompositingSimTest, PromoteCrossOriginToParent)
{
    ...
    Document* iframe_doc = To<HTMLFrameOwnerElement>(GetElementById("main_iframe"))->contentDocument();
    EXPECT_TRUE(CcLayerByOwnerNode(iframe_doc));

    iframe_doc = To<HTMLFrameOwnerElement>(iframe_doc->getElementById(AtomicString("child_iframe")))->contentDocument();
    EXPECT_TRUE(CcLayerForIFrameContent(iframe_doc));
}
...
TEST_P(CompositingSimTest, PromoteCrossOriginIframeAfterDomainChange)
{
    ...
    iframe_element->contentDocument()->setDomain(String("origin-a.com"), exception_state);
    // We may not have scheduled a visual update so force an update instead of
    // using BeginFrame.
    UpdateAllLifecyclePhases();

    iframe_doc = To<HTMLFrameOwnerElement>(GetElementById("iframe"))->contentDocument();
...
TEST_P(CompositingSimTest, PromoteCrossOriginToParentIframeAfterDomainChange)
{
    ...
    auto* main_iframe_element = To<HTMLIFrameElement>(GetDocument().getElementById(AtomicString("main_iframe")));
    NonThrowableExceptionState exception_state;

    GetDocument().setDomain(String("origin-a.com"), exception_state);
    auto* child_iframe_element = To<HTMLIFrameElement>(main_iframe_element->contentDocument()->getElementById(AtomicString("child_iframe")));
    child_iframe_element->contentDocument()->setDomain(String("origin-a.com"), exception_state);
    main_iframe_element->contentDocument()->setDomain(String("origin-a.com"), exception_state);

    // We may not have scheduled a visual update so force an update instead of
    // using BeginFrame.
    UpdateAllLifecyclePhases();
    iframe_doc = To<HTMLFrameOwnerElement>(GetElementById("main_iframe"))->contentDocument();
...
TEST_P(CompositingSimTest, ImplSideScrollSkipsCommit)
{
    ...
    cc::CompositorCommitData commit_data;
    commit_data.scrolls.emplace_back(element_id, gfx::Vector2dF(0, 10), std::nullopt);
    Compositor().LayerTreeHost()->ApplyCompositorChanges(&commit_data);
    EXPECT_EQ(gfx::PointF(0, 10), scrollable_area->ScrollPosition());
    EXPECT_EQ(gfx::PointF(0, 10), GetPropertyTrees()->scroll_tree().current_scroll_offset(element_id));
...
TEST_P(CompositingSimTest, ImplSideScrollUnpaintedSkipsCommit)
{
    ...
    auto* scroll_node = GetPropertyTrees()->scroll_tree().FindNodeFromElementId(element_id);
    ASSERT_TRUE(scroll_node);
...
TEST_P(CompositingSimTest, MainThreadScaleUpdatesTransformTree)
{
    ...
    const cc::TransformNode* scale_node = GetPropertyTrees()->transform_tree().FindNodeFromElementId(viewport.GetPageScaleNode()->GetCompositorElementId());
...
TEST_P(CompositingSimTest, ScrollWithMainThreadReasonsNeedsCommit)
{
    ...
    cc::CompositorCommitData commit_data;
    commit_data.scrolls.emplace_back(MainFrame().GetFrameView()->LayoutViewport()->GetScrollElementId(), gfx::Vector2dF(0, 100.f), std::nullopt);
...
TEST_P(CompositingSimTest, FrameAttribution)
{
    ...
    while (current_transform_node) {
        ...
        if (visible_frame_element_id)
            break;
        current_transform_node = GetPropertyTrees()->transform_tree().parent(current_transform_node);
    }

    EXPECT_EQ(visible_frame_element_id, CompositorElementIdFromUniqueObjectId(GetDocument().GetDomNodeId(), CompositorElementIdNamespace::kDOMNodeId));
    ...
    EXPECT_EQ(iframe_transform_node->visible_frame_element_id,
        CompositorElementIdFromUniqueObjectId(iframe_doc->GetDomNodeId(), CompositorElementIdNamespace::kDOMNodeId));
}
...
TEST_P(CompositingSimTest, VisibleFrameRootLayers)
{
    ...
    GetElementById("iframe")->SetInlineStyleProperty(CSSPropertyID::kPointerEvents, "none");
...
TEST_P(CompositingSimTest, BackgroundColorChangeUsesRepaintUpdate)
{
    ...
    EXPECT_EQ(CcLayerByDOMElementId("target")->background_color(), SkColors::kWhite);
    ...
    auto* target_element = GetElementById("target");
    target_element->setAttribute(html_names::kStyleAttr, AtomicString("background: black"));
    Compositor().BeginFrame();
    EXPECT_EQ(paint_artifact_compositor()->PreviousUpdateForTesting(), PaintArtifactCompositor::UpdateType::kRepaint);

    // Though a repaint-only update was done, the background color should still
    // be updated.
    EXPECT_EQ(CcLayerByDOMElementId("target")->background_color(), SkColors::kBlack);
}
...
TEST_P(CompositingSimTest, MultipleChunkBackgroundColorChangeRepaintUpdate)
{
    ...
    background_element->setAttribute(html_names::kStyleAttr, AtomicString("background: white"));
    Compositor().BeginFrame();
    EXPECT_EQ(paint_artifact_compositor()->PreviousUpdateForTesting(), PaintArtifactCompositor::UpdateType::kRepaint);
...
TEST_P(CompositingSimTest, SVGColorChangeUsesRepaintUpdate)
{
    ...
    div_element->setAttribute(html_names::kStyleAttr, AtomicString("background: black"));
    Compositor().BeginFrame();
    EXPECT_EQ(paint_artifact_compositor()->PreviousUpdateForTesting(), PaintArtifactCompositor::UpdateType::kRepaint);
}
...
TEST_P(CompositingSimTest, ChangingOpaquenessRequiresFullUpdate)
{
    ...
    auto* target_element = GetElementById("target");
    target_element->setAttribute(html_names::kStyleAttr, AtomicString("background: rgba(1, 0, 0, 0.1)"));
    Compositor().BeginFrame();
    EXPECT_EQ(paint_artifact_compositor()->PreviousUpdateForTesting(), PaintArtifactCompositor::UpdateType::kFull);
...
TEST_P(CompositingSimTest, ChangingContentsOpaqueForTextRequiresFullUpdate)
{
    ...
    text_container_element->setAttribute(html_names::kStyleAttr, AtomicString("background: rgba(1, 0, 0, 0.1)"));
    Compositor().BeginFrame();
    EXPECT_EQ(paint_artifact_compositor()->PreviousUpdateForTesting(), PaintArtifactCompositor::UpdateType::kFull);
    EXPECT_FALSE(CcLayerByDOMElementId("target")->contents_opaque());
...
TEST_P(CompositingSimTest, ChangingDrawsContentRequiresFullUpdate)
{
    ...
    target->setAttribute(html_names::kStyleAttr, AtomicString("background: rgba(0,0,0,0.5)"));
    Compositor().BeginFrame();
    EXPECT_EQ(paint_artifact_compositor()->PreviousUpdateForTesting(), PaintArtifactCompositor::UpdateType::kFull);
...
TEST_P(CompositingSimTest, FullCompositingUpdateReasons)
{
    ...
    b_element->setAttribute(html_names::kStyleAttr, AtomicString("display: none"));
    Compositor().BeginFrame();
    EXPECT_EQ(paint_artifact_compositor()->PreviousUpdateForTesting(), PaintArtifactCompositor::UpdateType::kFull);
...
TEST_P(CompositingSimTest, FullCompositingUpdateForJustCreatedChunks)
{
    ...
    auto* target = GetElementById("target");
    target->setAttribute(html_names::kClassAttr, AtomicString("firstLetterStyle"));
    Compositor().BeginFrame();
    EXPECT_EQ(paint_artifact_compositor()->PreviousUpdateForTesting(), PaintArtifactCompositor::UpdateType::kFull);
}
...
TEST_P(CompositingSimTest, FullCompositingUpdateForUncachableChunks)
{
    ...
    // paint chunks.
    auto* target = GetElementById("target");
    target->setAttribute(html_names::kStyleAttr, AtomicString("background: lightgreen"));
    Compositor().BeginFrame();
    EXPECT_EQ(paint_artifact_compositor()->PreviousUpdateForTesting(), PaintArtifactCompositor::UpdateType::kFull);
}
...
TEST_P(CompositingSimTest, DecompositeScrollerInHiddenIframe)
{
    ...
    LocalFrame& middle_frame = *To<LocalFrame>(GetDocument().GetFrame()->Tree().FirstChild());
    ...
    GetDocument().getElementById(AtomicString("middle"))->SetInlineStyleProperty(CSSPropertyID::kVisibility, CSSValueID::kHidden);
    Compositor().BeginFrame();
...
TEST_P(CompositingSimTest, ForeignLayersInMovedSubsequence)
{
    ...
    // Modifying paint in a simple way only requires a repaint update.
    auto* target_element = GetElementById("target");
    target_element->setAttribute(html_names::kStyleAttr, AtomicString("background: green;"));
    Compositor().BeginFrame();
    EXPECT_EQ(paint_artifact_compositor()->PreviousUpdateForTesting(), PaintArtifactCompositor::UpdateType::kRepaint);
...
{
    return "data:image/jpeg;base64," + Base64Encode(base::as_byte_span(*test::ReadFromFile(test::CoreTestDataPath(filename))));
}

TEST_P(CompositingSimTest, CompositedImageWithSubpixelOffset)
{
    // The image is 100x50 with normal orientation.
    InitializeWithHTML("<!DOCTYPE html><img id='image' src='" + ImageFileAsDataURL("exif-orientation-1-ul.jpg")
        + "' style='position: absolute; width: 400px; height: 800px;"
          "         top: 10.6px; will-change: top'>");
    Compositor().BeginFrame();
    auto* image_layer = static_cast<const cc::PictureLayer*>(CcLayerByDOMElementId("image"));
    ASSERT_TRUE(image_layer);
    EXPECT_EQ(gfx::Vector2dF(0.25f, 0.0625f), image_layer->GetRecordingSourceForTesting().directly_composited_image_info()->default_raster_scale);
}

TEST_P(CompositingSimTest, CompositedImageWithSubpixelOffsetAndOrientation)
{
    // The image is 50x100 after transposed.
    InitializeWithHTML("<!DOCTYPE html><img id='image' src='" + ImageFileAsDataURL("exif-orientation-5-lu.jpg")
        + "' style='position: absolute; width: 800px; height: 400px;"
          "         top: 10.6px; will-change: top'>");
    Compositor().BeginFrame();
    auto* image_layer = static_cast<const cc::PictureLayer*>(CcLayerByDOMElementId("image"));
    ASSERT_TRUE(image_layer);
    EXPECT_EQ(gfx::Vector2dF(0.0625f, 0.25f), image_layer->GetRecordingSourceForTesting().directly_composited_image_info()->default_raster_scale);
}
...
TEST_P(CompositingSimTest, ScrollingContentsLayerRecordedBounds)
{
    ...
    auto* layer = static_cast<const cc::PictureLayer*>(
        ScrollingContentsCcLayerByScrollElementId(RootCcLayer(), GetElementById("scroller")->GetLayoutBox()->GetScrollableArea()->GetScrollElementId()));
    ASSERT_TRUE(layer);
    if (RuntimeEnabledFeatures::HitTestOpaquenessEnabled()) {
        EXPECT_EQ(gfx::Size(2000, 16000), layer->bounds());
        EXPECT_EQ(gfx::Rect(0, 0, 2000, 16000), layer->GetRecordingSourceForTesting().recorded_bounds());
    } else {
        EXPECT_EQ(gfx::Size(2000, 2000), layer->bounds());
        EXPECT_EQ(gfx::Rect(0, 0, 2000, 2000), layer->GetRecordingSourceForTesting().recorded_bounds());
    }
}
...
TEST_P(CompositingSimTest, ScrollbarLayerWithDecompositedTransform)
{
    ...
    MainFrame().GetFrame()->GetSettings()->SetPreferCompositingToLCDTextForTesting(false);
    InitializeWithHTML(R"HTML(
    <!DOCTYPE html>
    ...
    EXPECT_EQ(gfx::Vector2dF(285, 100), scrollbar_layer->offset_to_transform_parent());
