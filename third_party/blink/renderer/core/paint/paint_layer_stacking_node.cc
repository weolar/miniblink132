...
void PaintLayerStackingNode::DirtyZOrderLists()
{
    ...
    for (auto& entry : layer_to_overlay_overflow_controls_painting_after_.Values()) {
...
static bool ZIndexLessThan(const PaintLayer* first, const PaintLayer* second)
{
    ...
    return first->GetLayoutObject().StyleRef().EffectiveZIndex() < second->GetLayoutObject().StyleRef().EffectiveZIndex();
}
...
public:
    enum LayerType { kAbsolutePosition, kFixedPosition, kInFlowStacked, kLayerTypeCount };
    std::array<const PaintLayer*, kLayerTypeCount> highest_layers = { nullptr, nullptr, nullptr };
    Vector<LayerType, kLayerTypeCount> highest_layers_order;

    void UpdateOrderForSubtreeHighestLayers(LayerType type, const PaintLayer* layer)
    {
        if (SetIfHigher(highest_layers[type], layer)) {
            auto new_end = std::remove(highest_layers_order.begin(), highest_layers_order.end(), type);
    ...
    void Merge(HighestLayers& child, const PaintLayer& current_layer)
    {
        ...
        for (auto layer_type : child.highest_layers_order) {
            ...
            if (object.IsStacked()) {
                if ((layer_type == kAbsolutePosition && object.CanContainAbsolutePositionObjects())
                    || (layer_type == kFixedPosition && object.CanContainFixedPositionObjects()) || layer_type == kInFlowStacked) {
            ...
            UpdateOrderForSubtreeHighestLayers(layer_type_for_propagation, child.highest_layers[layer_type]);
        }
    }
};

static LayoutObject* ChildOfFlexboxOrGridParentOrGrandparent(const PaintLayer* layer)
{
...
static bool OrderLessThan(const PaintLayer* first, const PaintLayer* second)
{
    ...
    LayoutObject* second_ancestor = ChildOfFlexboxOrGridParentOrGrandparent(second);
    ...
    if (first_ancestor != first->GetLayoutObject() || !first_ancestor->IsOutOfFlowPositioned()) {
        first_order = first_style.Order();
    }
    if (second_ancestor != second->GetLayoutObject() || !second_ancestor->IsOutOfFlowPositioned()) {
...
static void GetOrderSortedChildren(PaintLayer* paint_layer, PaintLayerStackingNode::PaintLayers& sorted_children)
{
    for (PaintLayer* child = paint_layer->FirstChild(); child; child = child->NextSibling()) {
        sorted_children.push_back(child);
    }

    std::stable_sort(sorted_children.begin(), sorted_children.end(), OrderLessThan);
}
...
void PaintLayerStackingNode::RebuildZOrderLists()
{
    ...
    // Sort the two lists.
    std::stable_sort(pos_z_order_list_.begin(), pos_z_order_list_.end(), ZIndexLessThan);
    std::stable_sort(neg_z_order_list_.begin(), neg_z_order_list_.end(), ZIndexLessThan);
    ...
    if (layer_->IsRootLayer()) {
        ...
        // that case.
        if (LayoutBlockFlow* multi_column_flow_thread = root_block->MultiColumnFlowThread())
            root_block = multi_column_flow_thread;
        for (LayoutObject* child = root_block->FirstChild(); child; child = child->NextSibling()) {
...
void PaintLayerStackingNode::CollectLayers(PaintLayer& paint_layer, HighestLayers* highest_layers)
{
    ...
    if (object.IsStacked()) {
        auto& list = style.EffectiveZIndex() >= 0 ? pos_z_order_list_ : neg_z_order_list_;
    ...
    bool has_overlay_overflow_controls = paint_layer.GetScrollableArea() && paint_layer.GetScrollableArea()->HasOverlayOverflowControls();
    ...
    if (has_overlay_overflow_controls) {
        ...
        for (auto layer_type : subtree_highest_layers->highest_layers_order) {
            if (layer_type == HighestLayers::kFixedPosition && !object.CanContainFixedPositionObjects())
                continue;
            if (layer_type == HighestLayers::kAbsolutePosition && !object.CanContainAbsolutePositionObjects())
                continue;
            SetIfHigher(layer_to_paint_overlay_overflow_controls_after, subtree_highest_layers->highest_layers[layer_type]);
        }

        if (layer_to_paint_overlay_overflow_controls_after) {
            layer_to_overlay_overflow_controls_painting_after_.insert(layer_to_paint_overlay_overflow_controls_after, MakeGarbageCollected<PaintLayers>())
                .stored_value->value->push_back(paint_layer);
        }
        paint_layer.SetNeedsReorderOverlayOverflowControls(!!layer_to_paint_overlay_overflow_controls_after);
    }
...
bool PaintLayerStackingNode::StyleDidChange(PaintLayer& paint_layer, const ComputedStyle* old_style)
{
    ...
    int old_order = 0;
    if (old_style) {
        was_stacking_context = paint_layer.GetLayoutObject().IsStackingContext(*old_style);
    ...
    bool should_be_stacking_context = paint_layer.GetLayoutObject().IsStackingContext();
    bool should_be_stacked = paint_layer.GetLayoutObject().IsStacked();
    if (should_be_stacking_context == was_stacking_context && was_stacked == should_be_stacked && old_z_index == new_style.EffectiveZIndex()
