...
inline const DisplayItemClient& AsDisplayItemClient(const InlineCursor& cursor, bool for_selection)
{
    if (for_selection) [[unlikely]] {
        if (const auto* selection_client = cursor.Current().GetSelectionDisplayItemClient())
            return *selection_client;
    }
    return *cursor.Current().GetDisplayItemClient();
}

inline PhysicalRect PhysicalBoxRect(
    const InlineCursor& cursor, const PhysicalOffset& paint_offset, const PhysicalOffset& parent_offset, const LayoutTextCombine* text_combine)
{
    ...
    box_rect.offset.top = LayoutUnit((paint_offset.top + parent_offset.top).Round()) + (box_rect.offset.top - parent_offset.top);
    if (text_combine) {
...
inline const InlineCursor& InlineCursorForBlockFlow(const InlineCursor& cursor, std::optional<InlineCursor>* storage)
{
    if (*storage)
        return **storage;
    *storage = cursor;
...
// > the ruby.
bool ShouldPaintEmphasisMark(const ComputedStyle& style, const LayoutObject& layout_object, const FragmentItem& text_item)
{
...
PhysicalDirection GetDisclosureOrientation(const ComputedStyle& style, bool is_open)
{
...
{
    constexpr gfx::PointF kLeftPoints[4] = { { 1.0f, 0.0f }, { 0.14f, 0.5f }, { 1.0f, 1.0f }, { 1.0f, 0.0f } };
    constexpr gfx::PointF kRightPoints[4] = { { 0.0f, 0.0f }, { 0.86f, 0.5f }, { 0.0f, 1.0f }, { 0.0f, 0.0f } };
    constexpr gfx::PointF kUpPoints[4] = { { 0.0f, 0.93f }, { 0.5f, 0.07f }, { 1.0f, 0.93f }, { 0.0f, 0.93f } };
    constexpr gfx::PointF kDownPoints[4] = { { 0.0f, 0.07f }, { 0.5f, 0.93f }, { 1.0f, 0.07f }, { 0.0f, 0.07f } };
...
void TextFragmentPainter::PaintSymbol(
    const LayoutObject* layout_object, const ComputedStyle& style, const PhysicalSize box_size, const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    const AtomicString& type = LayoutCounter::ListStyle(layout_object, style);
    PhysicalRect marker_rect(ListMarker::RelativeSymbolMarkerRect(style, type, box_size.width));
    ...
    if (BoxModelObjectPainter::ShouldForceWhiteBackgroundForPrintEconomy(layout_object->GetDocument(), style)) {
    ...
    AutoDarkMode auto_dark_mode(PaintAutoDarkMode(style, DarkModeFilter::ElementRole::kListSymbol));
    ...
    } else if (type == keywords::kDisclosureOpen || type == keywords::kDisclosureClosed) {
        Path path = GetCanonicalDisclosurePath(style, type == keywords::kDisclosureOpen);
        path.Transform(AffineTransform::MakeScaleNonUniform(marker_rect.Width(), marker_rect.Height()));
...
void TextFragmentPainter::Paint(const PaintInfo& paint_info, const PhysicalOffset& paint_offset)
{
    ...
    const TextFragmentPaintInfo& fragment_paint_info = cursor_.Current()->TextPaintInfo(cursor_.Items());
    ...
    const auto* const text_combine = DynamicTo<LayoutTextCombine>(layout_object->Parent());
    const PhysicalRect physical_box = PhysicalBoxRect(cursor_, paint_offset, parent_offset_, text_combine);
#if DCHECK_IS_ON()
    ...
    if (object_painter.ShouldRecordSpecialHitTestData(paint_info)) {
        object_painter.RecordHitTestData(paint_info, ToPixelSnappedRect(physical_box), *text_item.GetDisplayItemClient());
    }
    ...
    std::optional<HighlightPainter::SelectionPaintState> selection_for_bounds_recording;
    if (!is_printing && !is_rendering_resource && paint_info.phase != PaintPhase::kTextClip && layout_object->IsSelected()) [[unlikely]] {
        const InlineCursor& root_inline_cursor = InlineCursorForBlockFlow(cursor_, &inline_cursor_for_block_flow_);
        ...
        // has a valid range.
        selection_for_bounds_recording.emplace(root_inline_cursor, physical_box.offset, rotation);
    ...
    const auto* const svg_inline_text = DynamicTo<LayoutSVGInlineText>(layout_object);
    ...
    if (svg_inline_text) [[unlikely]] {
        ...
        visual_rect = gfx::ToEnclosingRect(svg_inline_text->Parent()->VisualRectInLocalSVGCoordinates());
    } else {
        DCHECK(!text_item.IsSvgText());
    ...
    if (selection_for_bounds_recording && paint_info.phase == PaintPhase::kForeground && !is_printing) [[unlikely]] {
        if (SelectionBoundsRecorder::ShouldRecordSelection(
                cursor_.Current().GetLayoutObject()->GetFrame()->Selection(), selection_for_bounds_recording->State())) {
            selection_recorder.emplace(selection_for_bounds_recording->State(), selection_for_bounds_recording->PhysicalSelectionRect(),
                paint_info.context.GetPaintController(), cursor_.Current().ResolvedDirection(), style.GetWritingMode(), *cursor_.Current().GetLayoutObject());
        }
    }
    ...
    const auto& display_item_client = AsDisplayItemClient(cursor_, selection != nullptr);
    ...
    if (paint_info.phase != PaintPhase::kTextClip) {
        if (!paint_info.context.InDrawingRecorder()) [[likely]] {
            ...
            recorder.emplace(paint_info.context, display_item_client, paint_info.phase, visual_rect);
        }
    }

    if (text_item.IsSymbolMarker()) [[unlikely]] {
        PaintSymbol(layout_object, style, physical_box.size, paint_info, physical_box.offset);
        return;
    }
    ...
    TextPaintStyle text_style = TextPainter::TextPaintingStyle(document, style, paint_info);
    if (selection) [[unlikely]] {
        selection->ComputeSelectionStyle(document, style, node, paint_info, text_style);
    }
    ...
    LineRelativeOffset text_origin { physical_box.offset.left, physical_box.offset.top + ascent };
    if (text_combine) [[unlikely]] {
        text_origin.line_over = text_combine->AdjustTextTopForPaint(physical_box.offset.top);
    }

    TextPainter text_painter(context, paint_info.GetSvgContextPaints(), *font, visual_rect, text_origin, is_horizontal);
    TextDecorationPainter decoration_painter(text_painter, inline_context_, paint_info, style, text_style, rotated_box, selection);
    HighlightPainter highlight_painter(
        fragment_paint_info, text_painter, decoration_painter, paint_info, cursor_, text_item, physical_box.offset, style, text_style, selection);
        ...
        if (auto* mf_checker = MobileFriendlinessChecker::From(document)) {
            if (auto* text = DynamicTo<LayoutText>(*layout_object)) {
                ...
                mf_checker->NotifyPaintTextFragment(
                    clipped_rect, text->StyleRef().FontSize(), paint_info.context.GetPaintController().CurrentPaintChunkProperties().Transform());
            }
        }
    }

    if (svg_inline_text) {
        TextPainter::SvgTextPaintState& svg_state = text_painter.SetSvgState(*svg_inline_text, style, text_item.GetStyleVariant(), paint_info.GetPaintFlags());
    ...
    const bool paint_marker_backgrounds = paint_info.phase != PaintPhase::kSelectionDragImage && paint_info.phase != PaintPhase::kTextClip && !is_printing;
    ...
    if (highlight_painter.Selection()) [[unlikely]] {
        PhysicalRect physical_selection = highlight_painter.Selection()->PhysicalSelectionRect();
        if (scaling_factor != 1.0f) {
    ...
    if (ShouldPaintEmphasisMark(style, *layout_object, text_item)) {
        text_painter.SetEmphasisMark(style.TextEmphasisMarkString(), style.GetTextEmphasisPosition());
    }
    ...
    AutoDarkMode auto_dark_mode(PaintAutoDarkMode(style, DarkModeFilter::ElementRole::kForeground));
    ...
    case HighlightPainter::kFastSelection:
        highlight_painter.Selection()->PaintSuppressingTextProperWhereSelected(text_painter, fragment_paint_info, text_style, node_id, auto_dark_mode);
        break;
    case HighlightPainter::kOverlay:
        ...
        highlight_painter.PaintHighlightOverlays(text_style, node_id, paint_marker_backgrounds, rotation);
        break;
    ...
    if (highlight_painter.Selection() && paint_marker_backgrounds) [[unlikely]] {
        if (highlight_case == HighlightPainter::kFastSelection) {
            highlight_painter.Selection()->PaintSelectionBackground(context, node, document, style, rotation);
        }
    }
    ...
    if (highlight_painter.Selection()) [[unlikely]] {
        switch (highlight_case) {
        case HighlightPainter::kFastSelection:
            highlight_painter.Selection()->PaintSelectedText(text_painter, fragment_paint_info, text_style, node_id, auto_dark_mode);
            break;
        case HighlightPainter::kSelectionOnly:
