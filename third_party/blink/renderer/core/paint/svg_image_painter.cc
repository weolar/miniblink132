...
namespace {
ImagePaintTimingInfo ComputeImagePaintTimingInfo(const LayoutSVGImage& layout_image, const Image& image, const ImageResourceContent* image_content,
...
{
    if (paint_info.phase != PaintPhase::kForeground || layout_svg_image_.StyleRef().Visibility() != EVisibility::kVisible
        || !layout_svg_image_.ImageResource()->HasImage()) {
        return;
    }

    if (SVGModelObjectPainter::CanUseCullRect(layout_svg_image_.StyleRef())) {
        if (!paint_info.GetCullRect().IntersectsTransformed(layout_svg_image_.LocalSVGTransform(), layout_svg_image_.VisualRectInLocalSVGCoordinates()))
            return;
    }
    ...
    ScopedSVGTransformState transform_state(paint_info, layout_svg_image_);
    {
        ...
        SVGModelObjectPainter::RecordRegionCaptureData(layout_svg_image_, paint_info);
        if (!DrawingRecorder::UseCachedDrawingIfPossible(paint_info.context, layout_svg_image_, paint_info.phase)) {
            SVGDrawingRecorder recorder(paint_info.context, layout_svg_image_, paint_info.phase);
...
void SVGImagePainter::PaintForeground(const PaintInfo& paint_info)
{
    ...
    if (respect_orientation && !image->HasDefaultOrientation()) {
        ...
        image_element->preserveAspectRatio()->CurrentValue()->TransformRect(dest_rect, src_rect);

        // Map the oriented_src_rect back into the src_rect space
        src_rect = image->CorrectSrcRectForImageOrientation(unadjusted_size, src_rect);
    } else {
        image_element->preserveAspectRatio()->CurrentValue()->TransformRect(dest_rect, src_rect);
    }
    ...
    if (image_content->IsLoaded()) {
        ...
        ImageElementTiming::From(*window).NotifyImagePainted(
            layout_svg_image_, *image_content, paint_info.context.GetPaintController().CurrentPaintChunkProperties(), gfx::ToEnclosingRect(dest_rect));
    }
    ...
    ScopedImageRenderingSettings image_rendering_settings_scope(
        paint_info.context, layout_svg_image_.StyleRef().GetInterpolationQuality(), layout_svg_image_.StyleRef().GetDynamicRangeLimit());
    Image::ImageDecodingMode decode_mode = image_element->GetDecodingModeForPainting(image->paint_image_id());
    auto image_auto_dark_mode = ImageClassifierHelper::GetImageAutoDarkMode(*layout_svg_image_.GetFrame(), layout_svg_image_.StyleRef(), dest_rect, src_rect);
    paint_info.context.DrawImage(*image, decode_mode, image_auto_dark_mode,
        ComputeImagePaintTimingInfo(layout_svg_image_, *image, image_content, paint_info.context, gfx::ToEnclosingRect(dest_rect)), dest_rect, &src_rect,
...
gfx::SizeF SVGImagePainter::ComputeImageViewportSize() const
{
    ...
    const gfx::SizeF default_object_size = gfx::ScaleSize(layout_svg_image_.ObjectBoundingBox().size(), zoom);

    if (To<SVGImageElement>(layout_svg_image_.GetElement())->preserveAspectRatio()->CurrentValue()->Align()
