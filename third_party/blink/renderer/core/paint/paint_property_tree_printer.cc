...
public:
    ...
    virtual void AddVisualViewportProperties(const VisualViewport&, PropertyTreePrinter&) const
    {
    }
    virtual void AddOtherProperties(const LocalFrameView&, PropertyTreePrinter&) const
    {
    }
    virtual void AddObjectPaintProperties(const ObjectPaintProperties&, PropertyTreePrinter&) const
    {
    }
};
...
private:
    ...
    {
        collector_.AddVisualViewportProperties(frame_view.GetPage()->GetVisualViewport(), *this);
        if (LayoutView* layout_view = frame_view.GetLayoutView())
            CollectNodes(*layout_view);
        for (Frame* child = frame_view.GetFrame().Tree().FirstChild(); child; child = child->Tree().NextSibling()) {
    ...
    void CollectNodes(const LayoutObject& object)
    {
        ...
        for (const auto* child = object.SlowFirstChild(); child; child = child->NextSibling()) {
            CollectNodes(*child);
        }
    }
...
public:
    void AddVisualViewportProperties(const VisualViewport& visual_viewport, PropertyTreePrinter& printer) const override
    {
    ...
    void AddObjectPaintProperties(const ObjectPaintProperties& properties, PropertyTreePrinter& printer) const override
    {
...
public:
    ...
    void AddOtherProperties(const LocalFrameView& frame_view, PropertyTreePrinter& printer) const override
    {
...
void SetDebugName(const PaintPropertyNode* node, const String& name, const LayoutObject& object)
{
    if (node) {
...
void UpdateDebugNames(const VisualViewport& viewport)
{
    ...
    SetDebugName(viewport.GetScrollTranslationNode(), "VisualViewport Translate Node");
    SetDebugName(viewport.GetScrollNode(), "VisualViewport Scroll Node");
}

void UpdateDebugNames(const LayoutObject& object, ObjectPaintProperties& properties)
{
    SetDebugName(properties.PaintOffsetTranslation(), "PaintOffsetTranslation", object);
    SetDebugName(properties.StickyTranslation(), "StickyTranslation", object);
    SetDebugName(properties.AnchorPositionScrollTranslation(), "AnchorPositionScrollTranslation", object);
    ...
    SetDebugName(properties.ReplacedContentTransform(), "ReplacedContentTransform", object);
    SetDebugName(properties.ScrollTranslation(), "ScrollTranslation", object);
    SetDebugName(properties.TransformIsolationNode(), "TransformIsolationNode", object);
    ...
    SetDebugName(properties.CssClipFixedPosition(), "CssClipFixedPosition", object);
    SetDebugName(properties.PixelMovingFilterClipExpander(), "PixelMovingFilterClip", object);
    SetDebugName(properties.OverflowControlsClip(), "OverflowControlsClip", object);
    SetDebugName(properties.BackgroundClip(), "BackgroundClip", object);
    SetDebugName(properties.InnerBorderRadiusClip(), "InnerBorderRadiusClip", object);
    ...
    SetDebugName(properties.VerticalScrollbarEffect(), "VerticalScrollbarEffect", object);
    SetDebugName(properties.HorizontalScrollbarEffect(), "HorizontalScrollbarEffect", object);
    ...
    SetDebugName(properties.ElementCaptureEffect(), "ElementCaptureEffect", object);
...
{
    return blink::FrameViewPropertyTreePrinter(blink::TransformNodeCollector()).TreeAsString(rootFrame);
}

String ClipPropertyTreeAsString(const blink::LocalFrameView& rootFrame)
{
    return blink::FrameViewPropertyTreePrinter(blink::ClipNodeCollector()).TreeAsString(rootFrame);
}

String EffectPropertyTreeAsString(const blink::LocalFrameView& rootFrame)
{
    return blink::FrameViewPropertyTreePrinter(blink::EffectNodeCollector()).TreeAsString(rootFrame);
}

String ScrollPropertyTreeAsString(const blink::LocalFrameView& rootFrame)
{
    return blink::FrameViewPropertyTreePrinter(blink::ScrollNodeCollector()).TreeAsString(rootFrame);
}
