...
class CORE_EXPORT RenderBlockingResourceManager final : public GarbageCollected<RenderBlockingResourceManager> {
public:
    ...
    RenderBlockingResourceManager& operator=(const RenderBlockingResourceManager&) = delete;
    ...
    {
        return pending_stylesheet_owner_nodes_.size() || pending_scripts_.size() || element_render_blocking_links_.size();
    }
    ...
    void AddPendingParsingElementLink(const AtomicString& id, const HTMLLinkElement* element);
    void RemovePendingParsingElement(const AtomicString& id, Element* element);
    void RemovePendingParsingElementLink(const AtomicString& id, const HTMLLinkElement* element);
...
private:
    ...
    HeapHashMap<AtomicString, Member<HeapHashSet<WeakMember<const HTMLLinkElement>>>> element_render_blocking_links_;

    Member<Document> document_;

    unsigned imperative_font_loading_count_ = 0;

    HeapTaskRunnerTimer<RenderBlockingResourceManager> font_preload_max_blocking_timer_;
    HeapTaskRunnerTimer<RenderBlockingResourceManager> font_preload_max_fcp_delay_timer_;
