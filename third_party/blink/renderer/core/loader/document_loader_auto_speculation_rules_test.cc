...
protected:
    void SetUp() override
    {
        ...
        url_test_helpers::RegisterMockedURLLoad(url_test_helpers::ToKURL("https://start.example.com/foo.html"), test::CoreTestDataPath("foo.html"));
        web_view_impl_ = web_view_helper_.InitializeAndLoad("https://start.example.com/foo.html");
    ...
    {
        return *GetLocalFrame().GetDocument();
    }
...
class DocumentLoaderAutoSpeculationRulesOptOutTest : public DocumentLoaderAutoSpeculationRulesTest, public testing::WithParamInterface<OptOutRuleSetType> {
public:
    SpeculationRuleSet* GetOptOutRuleSet() const
    {
        switch (GetParam()) {
        case OptOutRuleSetType::kInline:
            return SpeculationRuleSet::Parse(SpeculationRuleSet::Source::FromInlineScript("{}", GetDocument(), 0), GetLocalFrame().DomWindow());
        case OptOutRuleSetType::kExternal:
            return SpeculationRuleSet::Parse(
                SpeculationRuleSet::Source::FromRequest("{}", KURL("https://example.com/speculation-rules.json"), 0u), GetLocalFrame().DomWindow());
        }
    }
};
...
TEST_F(DocumentLoaderAutoSpeculationRulesTest, InvalidJSON)
{
    test::AutoSpeculationRulesConfigOverride override(R"(
  {
    ...
    static_assert(base::to_underlying(mojom::JavaScriptFramework::kVuePress) == 1);
    GetDocumentLoader().DidObserveJavaScriptFrameworks({ { { mojom::JavaScriptFramework::kVuePress, kNoFrameworkVersionDetected } } });
...
TEST_F(DocumentLoaderAutoSpeculationRulesTest, MultipleFrameworkRules)
{
    test::AutoSpeculationRulesConfigOverride override(R"(
  {
    ...
    GetDocumentLoader().DidObserveJavaScriptFrameworks(
        { { { mojom::JavaScriptFramework::kVuePress, kNoFrameworkVersionDetected }, { mojom::JavaScriptFramework::kGatsby, kNoFrameworkVersionDetected } } });

    // Test that we got the rules we expect from the framework mapping, and not
    // any more.
    EXPECT_EQ(rules.rule_sets().size(), 2u);
    EXPECT_EQ(rules.rule_sets().at(0)->prefetch_rules().at(0)->urls().at(0).GetString(), "https://example.com/foo.html");
    EXPECT_EQ(rules.rule_sets().at(1)->prefetch_rules().at(0)->urls().at(0).GetString(), "https://example.com/baz.html");
}
...
TEST_P(DocumentLoaderAutoSpeculationRulesOptOutTest, ExistingRuleSetOptsOut)
{
    test::AutoSpeculationRulesConfigOverride override(R"(
  {
    ...
    EXPECT_EQ(rules.rule_sets().size(), 1u);
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kAutoSpeculationRulesOptedOut));
    ...
    EXPECT_EQ(rules.rule_sets().size(), 1u);
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kAutoSpeculationRulesOptedOut));
    histogram_tester.ExpectUniqueSample("Blink.SpeculationRules.LoadOutcome", SpeculationRulesLoadOutcome::kAutoSpeculationRulesOptedOut,
        /*expected_bucket_count=*/2);
}

TEST_P(DocumentLoaderAutoSpeculationRulesOptOutTest, ExistingRuleSetOptOutIgnored)
{
    test::AutoSpeculationRulesConfigOverride override(R"(
  {
    ...
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kAutoSpeculationRulesOptedOut));
    histogram_tester.ExpectUniqueSample("Blink.SpeculationRules.LoadOutcome", SpeculationRulesLoadOutcome::kSuccess,
...
TEST_P(DocumentLoaderAutoSpeculationRulesOptOutTest, AddedLaterRuleSetOptOutIgnored)
{
    test::AutoSpeculationRulesConfigOverride override(R"(
  {
    "url_match_pattern_to_speculation_rules_ignore_opt_out": {
...
INSTANTIATE_TEST_SUITE_P(
    FromInlineOrExternal, DocumentLoaderAutoSpeculationRulesOptOutTest, testing::Values(OptOutRuleSetType::kInline, OptOutRuleSetType::kExternal));
