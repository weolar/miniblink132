...
static void SetReferrerForRequest(LocalDOMWindow* origin_window, ResourceRequest& request)
{
    ...
    Referrer referrer = SecurityPolicy::GenerateReferrer(referrer_policy_to_use, request.Url(), referrer_to_use);
...
void LogDanglingMarkupHistogram(LocalDOMWindow* origin_window, const AtomicString& target)
{
    DCHECK(origin_window);
    ...
    if (!target.EndsWith('>')) {
        ...
        if (!target.EndsWith('\n')) {
            origin_window->CountUse(WebFeature::kDanglingMarkupInTargetNotEndsWithNewLineOrGT);
        }
    }
}
...
FrameLoadRequest::FrameLoadRequest(LocalDOMWindow* origin_window, const ResourceRequest& resource_request)
    ...
    , should_send_referrer_(kMaybeSendReferrer)
{
    ...
    resource_request_.SetTargetAddressSpace(network::mojom::IPAddressSpace::kUnknown);
    resource_request_.SetCredentialsMode(network::mojom::CredentialsMode::kInclude);
    ...
    if (origin_window) {
        ...
        if (resource_request_.Url().IsAboutBlankURL() || resource_request_.Url().IsAboutSrcdocURL() || resource_request_.Url().IsEmpty()) {
            requestor_base_url_ = origin_window->BaseURL();
        }

        if (resource_request.Url().ProtocolIs("blob")) {
            blob_url_token_ = base::MakeRefCounted<base::RefCountedData<mojo::Remote<mojom::blink::BlobURLToken>>>();
            origin_window->GetPublicURLManager().Resolve(resource_request.Url(), blob_url_token_->data.BindNewPipeAndPassReceiver());
        }
...
FrameLoadRequest::FrameLoadRequest(LocalDOMWindow* origin_window, const ResourceRequestHead& resource_request_head)
...
{
    DCHECK(!origin_window_ || origin_window_->GetSecurityOrigin() == resource_request_.RequestorOrigin());
...
const AtomicString& FrameLoadRequest::CleanNavigationTarget(const AtomicString& target) const
{
