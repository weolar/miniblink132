...
TEST_F(DocumentLoadTimingTest, ensureValidNavigationStartAfterEmbedder)
{
    ...
    double delta = -1000;
    double embedder_navigation_start = base::TimeTicks::Now().since_origin().InSecondsF() + delta;
    timing.SetNavigationStart(base::TimeTicks() + base::Seconds(embedder_navigation_start));

    double real_wall_time = base::Time::Now().InSecondsFSinceUnixEpoch();
    base::TimeDelta adjusted_wall_time = timing.MonotonicTimeToPseudoWallTime(timing.NavigationStart());
...
TEST_F(DocumentLoadTimingTest, correctTimingDeltas)
{
    ...
    double current_monotonic_time = base::TimeTicks::Now().since_origin().InSecondsF();
    double embedder_navigation_start = current_monotonic_time + navigation_start_delta;
    ...
    double real_wall_load_event_end = base::Time::Now().InSecondsFSinceUnixEpoch();
    base::TimeDelta adjusted_load_event_end = timing.MonotonicTimeToPseudoWallTime(timing.LoadEventEnd());

    EXPECT_NEAR(adjusted_load_event_end.InSecondsF(), real_wall_load_event_end, .001);

    base::TimeDelta adjusted_navigation_start = timing.MonotonicTimeToPseudoWallTime(timing.NavigationStart());
    EXPECT_NEAR((adjusted_load_event_end - adjusted_navigation_start).InSecondsF(), -navigation_start_delta, .001);
}
...
TEST_F(DocumentLoadTimingTest, ensureRedirectEndExcludesNextFetch)
{
    ...
    // should be just |t1|.
    double redirect_time_ms = (timing.RedirectEnd() - timing.RedirectStart()).InMillisecondsF();
