...
public:
    ...
    const PartialResourceRequest& PingRequest() const
    {
...
public:
    ...
    {
        GetFrame().Loader().CommitNavigation(WebNavigationParams::CreateWithEmptyHTMLForTesting(url), nullptr /* extra_data */);
    ...
    const PartialResourceRequest& PingAndGetRequest(const KURL& ping_url)
    {
        ...
        url_test_helpers::RegisterMockedURLLoad(ping_url, test::CoreTestDataPath("bar.html"), "text/html");
        ...
        if (!ping_request.IsNull()) {
            EXPECT_EQ(destination_url.GetString(), ping_request.HttpHeaderField(AtomicString("Ping-To")));
        }
...
TEST_F(PingLoaderTest, HTTPToHTTPS)
{
    ...
    ASSERT_FALSE(ping_request.IsNull());
    EXPECT_EQ(ping_url, ping_request.Url());
    EXPECT_EQ(document_url.GetString(), ping_request.HttpHeaderField(AtomicString("Ping-From")));
}

TEST_F(PingLoaderTest, NonHTTPPingTarget)
{
    SetDocumentURL(KURL("http://127.0.0.1:8000/foo.html"));
    const PartialResourceRequest& ping_request = PingAndGetRequest(KURL("ftp://localhost/bar.html"));
...
TEST_F(PingLoaderTest, ViolationPriority)
{
    ...
    // via |PageTestBase::dummy_page_holder_|.
    url_test_helpers::RegisterMockedURLLoad(ping_url, test::CoreTestDataPath("bar.html"), "text/html");
    PingLoader::SendViolationReport(GetFrame().DomWindow(), ping_url, EncodedFormData::Create(), false);
    url_test_helpers::ServeAsynchronousRequests();
    const PartialResourceRequest& request = client_->PingRequest();
    ASSERT_FALSE(request.IsNull());
    ASSERT_EQ(request.Url(), ping_url);
...
TEST_F(PingLoaderTest, FrameAncestorsViolationHasOpaqueOrigin)
{
    ...
    // via |PageTestBase::dummy_page_holder_|.
    url_test_helpers::RegisterMockedURLLoad(ping_url, test::CoreTestDataPath("bar.html"), "text/html");
    PingLoader::SendViolationReport(GetFrame().DomWindow(), ping_url, EncodedFormData::Create(), true);
    url_test_helpers::ServeAsynchronousRequests();
    const PartialResourceRequest& request = client_->PingRequest();
    ASSERT_FALSE(request.IsNull());
    ASSERT_EQ(request.Url(), ping_url);
...
TEST_F(PingLoaderTest, BeaconPriority)
{
    ...
    // via |PageTestBase::dummy_page_holder_|.
    url_test_helpers::RegisterMockedURLLoad(ping_url, test::CoreTestDataPath("bar.html"), "text/html");
    PingLoader::SendBeacon(*ToScriptStateForMainWorld(&GetFrame()), &GetFrame(), ping_url, "hello");
    url_test_helpers::ServeAsynchronousRequests();
    const PartialResourceRequest& request = client_->PingRequest();
    ASSERT_FALSE(request.IsNull());
    ASSERT_EQ(request.Url(), ping_url);
