...
protected:
    ...
    ResourceFetcher* MakeResourceFetcher(TestResourceFetcherProperties* properties, FetchContext* context, ResourceFetcher::LoaderFactory* loader_factory)
    {
        return MakeGarbageCollected<ResourceFetcher>(ResourceFetcherInit(properties->MakeDetachable(), context, CreateTaskRunner(), CreateTaskRunner(),
            loader_factory, MakeGarbageCollected<MockContextLifecycleNotifier>(),
    ...
    void CommonSetup(v8::Isolate* isolate, const char* url_string = nullptr)
    {
        ...
        KURL url(url_string ? url_string : "codecachewithhashing://www.example.com/");
        ...
        resource_ = ScriptResource::Fetch(params, fetcher, nullptr, isolate, ScriptResource::kNoStreaming, kNoCompileHintsProducer, kNoCompileHintsConsumer,
    ...
    {
        const size_t kSerializedDataSize = sizeof(CachedMetadataHeader) + data.size();
        std::vector<uint8_t> serialized_data(kSerializedDataSize);
        CachedMetadataHeader* header = reinterpret_cast<CachedMetadataHeader*>(&serialized_data[0]);
        header->marker = CachedMetadataHandler::kSingleEntryWithTag;
        header->type = 0;
        memcpy(&serialized_data[sizeof(CachedMetadataHeader)], data.data(), data.size());
        return serialized_data;
    }

    std::vector<uint8_t> MakeSerializedCodeCacheDataWithHash(base::span<uint8_t> data, std::optional<String> source_text = {})
    {
        const size_t kSerializedDataSize = sizeof(CachedMetadataHeaderWithHash) + sizeof(CachedMetadataHeader) + data.size();
        std::vector<uint8_t> serialized_data(kSerializedDataSize);
        CachedMetadataHeaderWithHash* outer_header = reinterpret_cast<CachedMetadataHeaderWithHash*>(&serialized_data[0]);
        outer_header->marker = CachedMetadataHandler::kSingleEntryWithHashAndPadding;
        if (source_text.has_value()) {
            std::unique_ptr<ParkableStringImpl::SecureDigest> hash = ParkableStringImpl::HashString(source_text->Impl());
            CHECK_EQ(hash->size(), kSha256Bytes);
            memcpy(outer_header->hash, hash->data(), kSha256Bytes);
        }
        CachedMetadataHeader* inner_header = reinterpret_cast<CachedMetadataHeader*>(&serialized_data[sizeof(CachedMetadataHeaderWithHash)]);
        inner_header->marker = CachedMetadataHandler::kSingleEntryWithTag;
        inner_header->type = 0;
        memcpy(&serialized_data[sizeof(CachedMetadataHeaderWithHash) + sizeof(CachedMetadataHeader)], data.data(), data.size());
        return serialized_data;
    }
...
TEST_F(ResourceLoaderCodeCacheTest, WebUICodeCacheFullResponse)
{
    ...
    loader_->DidReceiveResponse(WrappedResourceResponse(response_),
        /*body=*/mojo::ScopedDataPipeConsumerHandle(), mojo_base::BigBuffer(MakeSerializedCodeCacheDataWithHash(cache_data)));

    // Code cache data was present.
    EXPECT_EQ(cache_data.size() + sizeof(CachedMetadataHeader), resource_->CodeCacheSize());
}
...
TEST_F(ResourceLoaderCodeCacheTest, CodeCacheFullHttpsScheme)
{
    ...
    std::vector<uint8_t> cache_data { 2, 3, 4, 5, 6 };
    loader_->DidReceiveResponse(WrappedResourceResponse(response_),
        /*body=*/mojo::ScopedDataPipeConsumerHandle(), mojo_base::BigBuffer(MakeSerializedCodeCacheData(cache_data)));
...
TEST_F(ResourceLoaderCodeCacheTest, CodeCacheFullHttpsSchemeWithResponseFlag)
{
    ...
    // Code cache data was present.
    EXPECT_EQ(resource_->CodeCacheSize(), cache_data.size() + sizeof(CachedMetadataHeader));
}
...
TEST_F(ResourceLoaderCodeCacheTest, WebUICodeCacheHashCheckSuccess)
{
    ...
    loader_->DidReceiveResponse(WrappedResourceResponse(response_),
        /*body=*/mojo::ScopedDataPipeConsumerHandle(), mojo_base::BigBuffer(MakeSerializedCodeCacheDataWithHash(cache_data, source_text)));

    // Code cache data was present.
    EXPECT_EQ(cache_data.size() + sizeof(CachedMetadataHeader), resource_->CodeCacheSize());

    // Successful check.
    resource_->CacheHandler()->Check(loader_factory_->GetCodeCacheHost(), ParkableString(source_text.Impl()));

    // Now the metadata can be accessed.
    scoped_refptr<CachedMetadata> cached_metadata = resource_->CacheHandler()->GetCachedMetadata(0);
...
TEST_F(ResourceLoaderCodeCacheTest, WebUICodeCacheHashCheckFailure)
{
    ...
    resource_->CacheHandler()->Check(loader_factory_->GetCodeCacheHost(), ParkableString(source_text_2.Impl()));
...
public:
    ...
    void set_should_use_code_cache_with_hashing(bool should_use_code_cache_with_hashing)
    {
