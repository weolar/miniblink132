...
bool IsRequestContextSupported(mojom::blink::RequestContextType request_context)
{
...
ScriptResource* ScriptResource::Fetch(FetchParameters& params, ResourceFetcher* fetcher, ResourceClient* client, v8::Isolate* isolate,
    StreamingAllowed streaming_allowed, v8_compile_hints::V8CrowdsourcedCompileHintsProducer* v8_compile_hints_producer,
    v8_compile_hints::V8CrowdsourcedCompileHintsConsumer* v8_compile_hints_consumer, v8_compile_hints::MagicCommentMode magic_comment_mode)
{
    DCHECK(IsRequestContextSupported(params.GetResourceRequest().GetRequestContext()));
    auto* resource = To<ScriptResource>(fetcher->RequestResource(params,
        ScriptResourceFactory(isolate, streaming_allowed, v8_compile_hints_producer, v8_compile_hints_consumer, magic_comment_mode, params.GetScriptType()),
        client));
    return resource;
}

ScriptResource* ScriptResource::CreateForTest(v8::Isolate* isolate, const KURL& url, const WTF::TextEncoding& encoding, mojom::blink::ScriptType script_type)
{
    ResourceRequest request(url);
    request.SetCredentialsMode(network::mojom::CredentialsMode::kOmit);
    ResourceLoaderOptions options(nullptr /* world */);
    TextResourceDecoderOptions decoder_options(TextResourceDecoderOptions::kPlainTextContent, encoding);
    return MakeGarbageCollected<ScriptResource>(request, options, decoder_options, isolate, kNoStreaming,
        /*v8_compile_hints_producer=*/nullptr,
        /*v8_compile_hints_consumer=*/nullptr, v8_compile_hints::MagicCommentMode::kNever, script_type);
}

ScriptResource::ScriptResource(const ResourceRequest& resource_request, const ResourceLoaderOptions& options, const TextResourceDecoderOptions& decoder_options,
    v8::Isolate* isolate, StreamingAllowed streaming_allowed, v8_compile_hints::V8CrowdsourcedCompileHintsProducer* v8_compile_hints_producer,
    v8_compile_hints::V8CrowdsourcedCompileHintsConsumer* v8_compile_hints_consumer, v8_compile_hints::MagicCommentMode magic_comment_mode,
    mojom::blink::ScriptType initial_request_script_type)
    : TextResource(resource_request, ResourceType::kScript, options, decoder_options)
    ,
    ...
    , stream_text_decoder_(std::make_unique<TextResourceDecoder>(decoder_options))
...
{
    static bool script_streaming_enabled = base::FeatureList::IsEnabled(features::kScriptStreaming);
    static bool script_streaming_for_non_http_enabled = base::FeatureList::IsEnabled(features::kScriptStreamingForNonHTTP);
    ...
    // unit tests.
    bool consume_code_cache_off_thread_enabled = base::FeatureList::IsEnabled(features::kConsumeCodeCacheOffThread);

    if (!script_streaming_enabled) {
        DisableStreaming(ScriptStreamer::NotStreamingReason::kDisabledByFeatureList);
    } else if (streaming_allowed == kNoStreaming) {
        DisableStreaming(ScriptStreamer::NotStreamingReason::kStreamingDisabled);
    } else if (!Url().ProtocolIsInHTTPFamily() && !script_streaming_for_non_http_enabled) {
...
void ScriptResource::OnMemoryDump(WebMemoryDumpLevelOfDetail level_of_detail, WebProcessMemoryDump* memory_dump) const
{
...
void ScriptResource::SetSerializedCachedMetadata(mojo_base::BigBuffer data)
{
    ...
    if (consume_cache_state_ == ConsumeCacheState::kWaitingForCache) {
        ...
        if (background_streamer_ && background_streamer_->HasConsumeCodeCacheTask()) {
            cache_consumer_ = MakeGarbageCollected<ScriptCacheConsumer>(isolate_if_main_thread_,
                V8CodeCache::GetCachedMetadata(CacheHandler(), CachedMetadataHandler::kAllowUnchecked), background_streamer_->TakeConsumeCodeCacheTask(), Url(),
                InspectorId());
        ...
        if (V8CodeCache::HasCodeCache(cached_metadata_handler_,
            ...
            cache_consumer_ = MakeGarbageCollected<ScriptCacheConsumer>(
                isolate_if_main_thread_, V8CodeCache::GetCachedMetadata(CacheHandler(), CachedMetadataHandler::kAllowUnchecked), Url(), InspectorId());
            AdvanceConsumeCacheState(ConsumeCacheState::kRunningOffThread);
            return;
        }
    }
...
void ScriptResource::SetRevalidatingRequest(const ResourceRequestHead& request)
{
    CHECK(IsLoaded());
    if (streamer_) {
...
{
    return cached_metadata_handler_ ? cached_metadata_handler_->GetCodeCacheSize() : 0;
}

void ScriptResource::ResponseReceived(const ResourceResponse& response)
{
    const bool is_successful_revalidation = IsSuccessfulRevalidationResponse(response);
    ...
    bool http_family = GetResourceRequest().Url().ProtocolIsInHTTPFamily() && response.CurrentRequestUrl().ProtocolIsInHTTPFamily();
    bool code_cache_with_hashing_supported = SchemeRegistry::SchemeSupportsCodeCacheWithHashing(GetResourceRequest().Url().Protocol())
        && GetResourceRequest().Url().ProtocolIs(response.CurrentRequestUrl().Protocol());
    ...
    code_cache_with_hashing_supported |= response.ShouldUseSourceHashForJSCodeCache();

    // Embedders may override whether hash-based code caching can be used for a
    // given resource request.
    code_cache_with_hashing_supported &= Platform::Current()->ShouldUseCodeCacheWithHashing(WebURL(GetResourceRequest().Url()));
    ...
    if (code_cache_supported) {
        std::unique_ptr<CachedMetadataSender> sender
            = CachedMetadataSender::Create(response, mojom::blink::CodeCacheType::kJavascript, GetResourceRequest().RequestorOrigin());
        if (code_cache_with_hashing_supported) {
            cached_metadata_handler_ = MakeGarbageCollected<ScriptCachedMetadataHandlerWithHashing>(Encoding(), std::move(sender));
        } else {
            cached_metadata_handler_ = MakeGarbageCollected<ScriptCachedMetadataHandler>(Encoding(), std::move(sender));
        }
    }
}

void ScriptResource::ResponseBodyReceived(ResponseBodyLoaderDrainableInterface& body_loader, scoped_refptr<base::SingleThreadTaskRunner> loader_task_runner)
{
    ...
    CHECK(Url().ProtocolIsInHTTPFamily() || base::FeatureList::IsEnabled(features::kScriptStreamingForNonHTTP));
    CHECK(base::FeatureList::IsEnabled(features::kScriptStreaming));

    ResponseBodyLoaderClient* response_body_loader_client;
    mojo::ScopedDataPipeConsumerHandle data_pipe = body_loader.DrainAsDataPipe(&response_body_loader_client);
    if (!data_pipe) {
    ...
    streamer_ = MakeGarbageCollected<ResourceScriptStreamer>(
        this, std::move(data_pipe), response_body_loader_client, std::move(stream_text_decoder_), loader_task_runner);
    CHECK_EQ(no_streamer_reason_, ScriptStreamer::NotStreamingReason::kInvalid);
    AdvanceStreamingState(StreamingState::kStreaming);
}

void ScriptResource::DidReceiveDecodedData(const String& data, std::unique_ptr<ParkableStringImpl::SecureDigest> digest)
{
...
void ScriptResource::SetEncoding(const String& chs)
{
    ...
    if (stream_text_decoder_) {
        stream_text_decoder_->SetEncoding(WTF::TextEncoding(chs), TextResourceDecoder::kEncodingFromHTTPHeader);
    }
}
...
ScriptStreamer* ScriptResource::TakeStreamer()
{
    ...
    DisableStreaming(ScriptStreamer::NotStreamingReason::kSecondScriptResourceUse);
    return streamer;
}

void ScriptResource::DisableStreaming(ScriptStreamer::NotStreamingReason no_streamer_reason)
{
...
void ScriptResource::AdvanceStreamingState(StreamingState new_state)
{
    switch (streaming_state_) {
    case StreamingState::kWaitingForDataPipe:
        CHECK(new_state == StreamingState::kStreaming || new_state == StreamingState::kStreamingDisabled);
        break;
    case StreamingState::kStreaming:
...
void ScriptResource::CheckStreamingState() const
{
    ...
    case StreamingState::kStreaming:
        CHECK(streamer_);
        CHECK(streamer_->CanStartStreaming() || streamer_->IsStreamingStarted() || streamer_->IsStreamingSuppressed());
        CHECK(IsLoading() || streamer_->IsFinished());
        break;
    case StreamingState::kStreamingDisabled:
        CHECK(!streamer_);
        CHECK_NE(no_streamer_reason_, ScriptStreamer::NotStreamingReason::kInvalid);
        break;
    }
}
...
void ScriptResource::AdvanceConsumeCacheState(ConsumeCacheState new_state)
{
    ...
    case ConsumeCacheState::kWaitingForCache:
        CHECK(new_state == ConsumeCacheState::kRunningOffThread || new_state == ConsumeCacheState::kOffThreadConsumeCacheDisabled);
        break;
...
std::unique_ptr<BackgroundResponseProcessorFactory> ScriptResource::MaybeCreateBackgroundResponseProcessorFactory()
{
    ...
    DisableStreaming(ScriptStreamer::NotStreamingReason::kBackgroundResponseProcessorWillBeUsed);

    background_streamer_ = MakeGarbageCollected<BackgroundResourceScriptStreamer>(this);
