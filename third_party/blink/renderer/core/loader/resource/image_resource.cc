...
{
    CHECK(IsMainThread());
    DEFINE_THREAD_SAFE_STATIC_LOCAL(Vector<String>, known_transparent_urls,
...
scoped_refptr<SharedBuffer> GetDataForTransparentPlaceholderImageIndex(wtf_size_t index)
{
    CHECK(index >= 0 && index < 2);
    CHECK(IsMainThread());
    DEFINE_THREAD_SAFE_STATIC_LOCAL(Vector<scoped_refptr<SharedBuffer>>, known_transparent_encoded_gifs,
        ({ SharedBuffer::Create("\x47\x49\x46\x38\x39\x61\x01\x00\x01\x00\x80\x00\x00\xff\xff\xff"
            ...
            SharedBuffer::Create("\x47\x49\x46\x38\x39\x61\x01\x00\x01\x00\x80\xff\x00\xc0\xc0\xc0"
...
void MarkKnownTransparentPlaceholderResourceRequestIfNeeded(ResourceRequest& resource_request)
{
...
ImageResource* CreateResourceForTransparentPlaceholderImage(const ResourceRequest& request, const ResourceLoaderOptions& options)
{
    const wtf_size_t index = request.GetKnownTransparentPlaceholderImageIndex();
    CHECK_NE(index, kNotFound);
    CHECK(index >= 0 && index < 2);
    scoped_refptr<SharedBuffer> data = GetDataForTransparentPlaceholderImageIndex(index);
    CHECK(data->size());
    ...
    auto* resource = MakeGarbageCollected<ImageResource>(request, options, image_content);
...
class ImageResource::ImageResourceInfoImpl final : public GarbageCollected<ImageResourceInfoImpl>, public ImageResourceInfo {
public:
...
private:
    ...
    {
        if (ResourceLoadTiming* load_timing = resource_->GetResponse().GetResourceLoadTiming()) {
    ...
    bool IsAccessAllowed(DoesCurrentFrameHaveSingleSecurityOrigin does_current_frame_has_single_security_origin) const override
    {
        return resource_->IsAccessAllowed(does_current_frame_has_single_security_origin);
    }
    ...
    void EmulateLoadStartedForInspector(ResourceFetcher* fetcher, const AtomicString& initiator_name) override
    {
        fetcher->EmulateLoadStartedForInspector(
            resource_.Get(), mojom::blink::RequestContextType::IMAGE, network::mojom::RequestDestination::kImage, initiator_name);
    }

    void LoadDeferredImage(ResourceFetcher* fetcher) override
    {
        if (resource_->GetType() == ResourceType::kImage && resource_->StillNeedsLoad() && !fetcher->ShouldDeferImageLoad(resource_->Url())) {
...
public:
    explicit ImageResourceFactory(bool transparent_image_optimization_enabled)
        ...
        , transparent_image_optimization_enabled_(transparent_image_optimization_enabled)
    {
    }

    Resource* Create(const ResourceRequest& request, const ResourceLoaderOptions& options) const override
    {
        if (transparent_image_optimization_enabled_ && (request.GetKnownTransparentPlaceholderImageIndex() != kNotFound)) {
            return CreateResourceForTransparentPlaceholderImage(request, options);
        }

        return MakeGarbageCollected<ImageResource>(request, options, ImageResourceContent::CreateNotStarted());
    }
...
ImageResource* ImageResource::Fetch(FetchParameters& params, ResourceFetcher* fetcher)
{
    MarkKnownTransparentPlaceholderResourceRequestIfNeeded(params.MutableResourceRequest());

    if (params.GetResourceRequest().GetRequestContext() == mojom::blink::RequestContextType::UNSPECIFIED) {
    ...
    bool is_user_agent_resource = params.Options().initiator_info.name == fetch_initiator_type_names::kUacss;
    if (is_user_agent_resource) {
        params.SetContentSecurityCheck(network::mojom::CSPDisposition::DO_NOT_CHECK);
    }

    auto* resource
        = To<ImageResource>(fetcher->RequestResource(params, ImageResourceFactory(fetcher->IsSimplifyLoadingTransparentPlaceholderImageEnabled()), nullptr));
...
ImageResource* ImageResource::Create(const ResourceRequest& request, const DOMWrapperWorld* world)
{
...
ImageResource* ImageResource::CreateForTest(const KURL& url)
{
    ...
    request.SetReferrerPolicy(ReferrerUtils::MojoReferrerPolicyResolveDefault(request.GetReferrerPolicy()));
    request.SetPriority(WebURLRequest::Priority::kLow);
    MarkKnownTransparentPlaceholderResourceRequestIfNeeded(request);

    ImageResourceFactory factory(base::FeatureList::IsEnabled(features::kSimplifyLoadingTransparentPlaceholderImage));
    return To<ImageResource>(factory.Create(request, ResourceLoaderOptions(/* world=*/nullptr)));
}

ImageResource::ImageResource(const ResourceRequest& resource_request, const ResourceLoaderOptions& options, ImageResourceContent* content)
    : Resource(resource_request, ResourceType::kImage, options)
    , content_(content)
{
    DCHECK(content_);
    DCHECK(GetContent());
    RESOURCE_LOADING_DVLOG(1) << "MakeGarbageCollected<ImageResource>(ResourceRequest) " << this;
    GetContent()->SetImageResourceInfo(MakeGarbageCollected<ImageResourceInfoImpl>(this));
}
...
void ImageResource::OnMemoryDump(WebMemoryDumpLevelOfDetail level_of_detail, WebProcessMemoryDump* memory_dump) const
{
...
{
    DCHECK((multipart_parser_ && IsLoading()) || !Data() || GetContent()->HasImage());
...
void ImageResource::AllClientsAndObserversRemoved()
{
    ...
    CHECK(is_during_finish_as_error_ || !GetContent()->HasImage() || !ErrorOccurred());
...
void ImageResource::AppendData(absl::variant<SegmentedBuffer, base::span<const char>> data)
{
    ...
    if (multipart_parser_) {
        ...
        if (!is_pending_flushing_) {
            scoped_refptr<base::SingleThreadTaskRunner> task_runner = Loader()->GetLoadingTaskRunner();
            base::TimeTicks now = base::TimeTicks::Now();
            if (last_flush_time_.is_null())
                last_flush_time_ = now;

            DCHECK_LE(last_flush_time_, now);
            base::TimeDelta flush_delay = std::max(base::TimeDelta(), last_flush_time_ - now + kFlushDelay);
            task_runner->PostDelayedTask(FROM_HERE, WTF::BindOnce(&ImageResource::FlushImageIfNeeded, WrapWeakPersistent(this)), flush_delay);
...
void ImageResource::DecodeError(bool all_data_received)
{
    ...
    if (!all_data_received && Loader()) {
        ...
        auto result = GetContent()->UpdateImage(nullptr, GetStatus(), ImageResourceContent::kClearImageAndNotifyObservers, all_data_received, is_multipart);
...
void ImageResource::Finish(base::TimeTicks load_finish_time, base::SingleThreadTaskRunner* task_runner)
{
    if (multipart_parser_) {
        if (!ErrorOccurred())
...
void ImageResource::FinishAsError(const ResourceError& error, base::SingleThreadTaskRunner* task_runner)
{
    if (multipart_parser_)
        multipart_parser_->Cancel();
    ...
    UpdateImage(nullptr, ImageResourceContent::kClearImageAndNotifyObservers, true);
}
...
void ImageResource::ResponseReceived(const ResourceResponse& response)
{
    ...
    if (response.MimeType() == "multipart/x-mixed-replace") {
        Vector<char> boundary = network_utils::ParseMultipartBoundary(response.HttpHeaderField(http_names::kContentType));
        // If there's no boundary, just handle the request normally.
        if (!boundary.empty()) {
            multipart_parser_ = MakeGarbageCollected<MultipartImageResourceParser>(response, boundary, this);
        }
    }
...
void ImageResource::OnePartInMultipartReceived(const ResourceResponse& response)
{
    ...
    if (!GetResponse().IsNull()) {
        CHECK_EQ(GetResponse().WasFetchedViaServiceWorker(), response.WasFetchedViaServiceWorker());
...
bool ImageResource::IsAccessAllowed(ImageResourceInfo::DoesCurrentFrameHaveSingleSecurityOrigin does_current_frame_has_single_security_origin) const
{
    if (does_current_frame_has_single_security_origin != ImageResourceInfo::kHasSingleSecurityOrigin)
        return false;
...
std::pair<ResourcePriority, ResourcePriority> ImageResource::ComputePriorityFromObservers()
{
    return GetContent()->PriorityFromObservers();
}

void ImageResource::UpdateImage(scoped_refptr<SharedBuffer> shared_buffer, ImageResourceContent::UpdateImageOption update_image_option, bool all_data_received)
{
    bool is_multipart = !!multipart_parser_;
    auto result = GetContent()->UpdateImage(std::move(shared_buffer), GetStatus(), update_image_option, all_data_received, is_multipart);
