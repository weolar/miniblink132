...
protected:
    ...
    {
        original_memory_cache_ = ReplaceMemoryCacheForTesting(MakeGarbageCollected<MemoryCache>(blink::scheduler::GetSingleThreadTaskRunnerForTesting()));
    }
    ...
    CSSStyleSheetResource* CreateAndSaveTestStyleSheetResource()
    {
        ...
        CSSStyleSheetResource* css_resource = CSSStyleSheetResource::CreateForTest(css_url, UTF8Encoding());
...
TEST_F(CSSStyleSheetResourceTest, CreateFromCacheRestoresOriginalSheet)
{
    ...
    StyleSheetContents* parsed_stylesheet = css_resource->CreateParsedStyleSheetFromCache(parser_context);
...
TEST_F(CSSStyleSheetResourceSimTest, CachedWithDifferentMQEval)
{
    ...
    SimSubresourceRequest css_resource("https://example.com/frame.css", "text/css", params);
    ...
    Document* frame1_doc = To<HTMLIFrameElement>(GetDocument().getElementById(AtomicString("frame1")))->contentDocument();
    Document* frame2_doc = To<HTMLIFrameElement>(GetDocument().getElementById(AtomicString("frame2")))->contentDocument();
    ...
    const ActiveStyleSheetVector& frame1_sheets = frame1_doc->GetScopedStyleResolver()->GetActiveStyleSheets();
    const ActiveStyleSheetVector& frame2_sheets = frame2_doc->GetScopedStyleResolver()->GetActiveStyleSheets();
    ...
    EXPECT_EQ(frame1_sheets[0].first->Contents(), frame2_sheets[0].first->Contents());
