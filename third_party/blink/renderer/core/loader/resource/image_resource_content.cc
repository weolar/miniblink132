...
class NullImageResourceInfo final : public GarbageCollected<NullImageResourceInfo>, public ImageResourceInfo {
public:
...
private:
    ...
    bool IsAccessAllowed(DoesCurrentFrameHaveSingleSecurityOrigin) const override
    {
        return true;
    }
    ...
    void EmulateLoadStartedForInspector(ResourceFetcher*, const AtomicString& initiator_name) override
    {
    }
...
{
    DEFINE_STATIC_LOCAL(Persistent<NullImageResourceInfo>, null_info, (MakeGarbageCollected<NullImageResourceInfo>()));
    info_ = null_info;
}

ImageResourceContent* ImageResourceContent::CreateLoaded(scoped_refptr<blink::Image> image)
{
    DCHECK(image);
    ImageResourceContent* content = MakeGarbageCollected<ImageResourceContent>(std::move(image));
    content->content_status_ = ResourceStatus::kCached;
    return content;
}

ImageResourceContent* ImageResourceContent::Fetch(FetchParameters& params, ResourceFetcher* fetcher)
{
...
void ImageResourceContent::HandleObserverFinished(ImageResourceObserver* observer)
{
    {
...
void ImageResourceContent::RemoveObserver(ImageResourceObserver* observer)
{
    ...
    bool fully_erased;
    if (it != observers_.end()) {
        fully_erased = observers_.erase(it) && finished_observers_.find(observer) == finished_observers_.end();
    } else {
...
static void PriorityFromObserver(const ImageResourceObserver* observer, ResourcePriority& priority, ResourcePriority& priority_excluding_image_loader)
{
    ...
    if (next_priority.source != ResourcePriority::Source::kImageLoader) {
        ...
        priority_excluding_image_loader.intra_priority_value += next_priority.intra_priority_value;
    }
}

std::pair<ResourcePriority, ResourcePriority> ImageResourceContent::PriorityFromObservers() const
{
    ProhibitAddRemoveObserverInScope prohibit_add_remove_observer_in_scope(this);
...
std::optional<WebURLRequest::Priority> ImageResourceContent::RequestPriority() const
{
...
bool ImageResourceContent::ShouldUpdateImageImmediately() const
{
    ...
    // images.
    return size_available_ == Image::kSizeUnavailable || (image_ && image_->MaybeAnimated());
}
...
gfx::Size ImageResourceContent::IntrinsicSize(RespectImageOrientationEnum should_respect_image_orientation) const
{
    if (!image_)
        return gfx::Size();
    RespectImageOrientationEnum respect_orientation = ForceOrientationIfNecessary(should_respect_image_orientation);
    return image_->Size(respect_orientation);
}

RespectImageOrientationEnum ImageResourceContent::ForceOrientationIfNecessary(RespectImageOrientationEnum default_orientation) const
{
...
void ImageResourceContent::NotifyObservers(NotifyFinishOption notifying_finish_option, CanDeferInvalidation defer)
{
    {
    ...
    }
    {
        ...
        for (ImageResourceObserver* observer : observers_as_vector) {
            if (observers_.Contains(observer)) {
                observer->ImageChanged(this, defer);
                if (notifying_finish_option == kShouldNotifyFinish && observers_.Contains(observer)) {
                    HandleObserverFinished(observer);
                }
            }
        }
...
{
    String content_dpr_value = info_->GetResponse().HttpHeaderField(http_names::kContentDPR);
    ...
    device_pixel_ratio_header_value_ = content_dpr_value.ToFloat(&has_device_pixel_ratio_header_value_);
    if (!has_device_pixel_ratio_header_value_ || device_pixel_ratio_header_value_ <= 0.0) {
...
void ImageResourceContent::UpdateToLoadedContentStatus(ResourceStatus new_status)
{
...
ImageResourceContent::UpdateImageResult ImageResourceContent::UpdateImage(
    scoped_refptr<SharedBuffer> data, ResourceStatus status, UpdateImageOption update_image_option, bool all_data_received, bool is_multipart)
{
    ...
    case kClearAndUpdateImage:
        ...
        if (data) {
            ...
            DCHECK(all_data_received || size_available_ != Image::kSizeAvailableAndLoadingAsynchronously);
        }
        ...
        if (image_) {
            ...
            const HashSet<String>* unsupported_mime_types = info_->GetUnsupportedImageMimeTypes();
            if (unsupported_mime_types && unsupported_mime_types->Contains(image_->MimeType())) {
                return UpdateImageResult::kShouldDecodeError;
            }
        }
        ...
        if (!image_ || (image_->IsNull() && (!IsA<SVGImage>(image_.get()) || size_available_ == Image::kSizeUnavailable))) {
            ClearImage();
            return UpdateImageResult::kShouldDecodeError;
        }
        break;
    ...
    if (all_data_received && size_available_ != Image::kSizeAvailableAndLoadingAsynchronously) {
...
ImageDecoder::CompressionFormat ImageResourceContent::GetCompressionFormat() const
{
    if (!image_)
        return ImageDecoder::kUndefinedFormat;
    return ImageDecoder::GetCompressionFormat(image_->Data(), GetResponse().HttpContentType());
}
...
void ImageResourceContent::DecodedSizeChangedTo(const blink::Image* image, size_t new_size)
{
...
void ImageResourceContent::UpdateImageAnimationPolicy()
{
    ...
    mojom::blink::ImageAnimationPolicy new_policy = mojom::blink::ImageAnimationPolicy::kImageAnimationPolicyAllowed;
    {
        ProhibitAddRemoveObserverInScope prohibit_add_remove_observer_in_scope(this);
        for (const auto& it : finished_observers_) {
...
bool ImageResourceContent::IsAccessAllowed() const
{
    return info_->IsAccessAllowed(
        GetImage()->CurrentFrameHasSingleSecurityOrigin() ? ImageResourceInfo::kHasSingleSecurityOrigin : ImageResourceInfo::kHasMultipleSecurityOrigin);
}

void ImageResourceContent::EmulateLoadStartedForInspector(ResourceFetcher* fetcher, const AtomicString& initiator_name)
{
...
{
    return GetContentStatus() == ResourceStatus::kLoadError || GetContentStatus() == ResourceStatus::kDecodeError;
}
