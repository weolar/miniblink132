...
public:
    ...
    static CSSStyleSheetResource* Fetch(FetchParameters&, ResourceFetcher*, ResourceClient*);
    static CSSStyleSheetResource* CreateForTest(const KURL&, const WTF::TextEncoding&);

    CSSStyleSheetResource(const ResourceRequest&, const ResourceLoaderOptions&, const TextResourceDecoderOptions&);

    ~CSSStyleSheetResource() override;
    void Trace(Visitor*) const override;
    void OnMemoryDump(WebMemoryDumpLevelOfDetail, WebProcessMemoryDump*) const override;

    const String SheetText(const CSSParserContext*, MIMETypeCheck = MIMETypeCheck::kStrict) const;
...
private:
    ...
    public:
        CSSStyleSheetResourceFactory()
            : ResourceFactory(ResourceType::kCSSStyleSheet, TextResourceDecoderOptions::kCSSContent)
        {
        }
...
template <> struct DowncastTraits<CSSStyleSheetResource> {
