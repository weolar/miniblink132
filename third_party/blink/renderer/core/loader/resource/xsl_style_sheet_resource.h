...
public:
    static XSLStyleSheetResource* FetchSynchronously(FetchParameters&, ResourceFetcher*);
    static XSLStyleSheetResource* Fetch(FetchParameters&, ResourceFetcher*, ResourceClient*);

    XSLStyleSheetResource(const ResourceRequest&, const ResourceLoaderOptions&, const TextResourceDecoderOptions&);
...
private:
    ...
    public:
        XSLStyleSheetResourceFactory()
            : ResourceFactory(ResourceType::kXSLStyleSheet, TextResourceDecoderOptions::kXMLContent)
        {
        }
        ...
        {
            return MakeGarbageCollected<XSLStyleSheetResource>(request, options, decoder_options);
        }
    };
...
template <> struct DowncastTraits<XSLStyleSheetResource> {
