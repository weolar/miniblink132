...
XSLStyleSheetResource* XSLStyleSheetResource::FetchSynchronously(FetchParameters& params, ResourceFetcher* fetcher)
{
    ApplyXSLRequestProperties(params);
    params.MakeSynchronous();
    auto* resource = To<XSLStyleSheetResource>(fetcher->RequestResource(params, XSLStyleSheetResourceFactory(), nullptr));
    if (resource->Data())
        resource->sheet_ = resource->DecodedText();
    return resource;
}

XSLStyleSheetResource* XSLStyleSheetResource::Fetch(FetchParameters& params, ResourceFetcher* fetcher, ResourceClient* client)
{
    ApplyXSLRequestProperties(params);
    return To<XSLStyleSheetResource>(fetcher->RequestResource(params, XSLStyleSheetResourceFactory(), client));
}
...
XSLStyleSheetResource::XSLStyleSheetResource(
    ...
    : TextResource(resource_request, ResourceType::kXSLStyleSheet, options, decoder_options)
{
}
