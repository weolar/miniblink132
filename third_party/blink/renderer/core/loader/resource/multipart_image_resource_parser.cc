...
void MultipartImageResourceParser::AppendData(base::span<const char> bytes)
{
    ...
    while ((boundary_position = FindBoundary(data_, &boundary_)) != kNotFound) {
        ...
        if (data_size) {
            client_->MultipartDataReceived(base::span<const uint8_t>((const uint8_t*)data_.data(), data_.size()).first(data_size));
            if (IsCancelled())
                return;
        }
        wtf_size_t boundary_end_position = boundary_position + boundary_.size();
        if (boundary_end_position < data_.size() && '-' == data_[boundary_end_position]) {
    ...
    if (!is_parsing_headers_ && data_.size() > boundary_.size() + 2) {
        auto send_data = base::span<const uint8_t>((const uint8_t*)data_.data(), data_.size()).first(data_.size() - boundary_.size() - 2);
...
void MultipartImageResourceParser::Finish()
{
    ...
    if (!is_parsing_headers_ && !data_.empty()) {
        client_->MultipartDataReceived(base::span<const uint8_t>((const uint8_t*)data_.data(), data_.size()));
    }
    data_.clear();
    saw_last_boundary_ = true;
}

wtf_size_t MultipartImageResourceParser::SkippableLength(const Vector<char>& data, wtf_size_t pos)
{
...
bool MultipartImageResourceParser::ParseHeaders()
{
    ...
    response.SetWasFetchedViaServiceWorker(original_response_.WasFetchedViaServiceWorker());
    ...
    wtf_size_t end = 0;
    if (!ParseMultipartHeadersFromBody(base::span<const uint8_t>((const uint8_t*)data_.data(), data_.size()).subspan(pos), &response, &end)) {
        return false;
    }
...
wtf_size_t MultipartImageResourceParser::FindBoundary(const Vector<char>& data, Vector<char>* boundary)
{
    ...
    if (boundary_position >= 2) {
        if (data[boundary_position - 1] == '-' && data[boundary_position - 2] == '-') {
            boundary_position -= 2;
            Vector<char> v(2, '-');
