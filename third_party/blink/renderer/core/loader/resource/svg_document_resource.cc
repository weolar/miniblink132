...
public:
    SVGDocumentResourceFactory(AgentGroupScheduler& agent_group_scheduler, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    ...
    {
        auto* content = MakeGarbageCollected<SVGResourceDocumentContent>(agent_group_scheduler_, task_runner_);
        return MakeGarbageCollected<SVGDocumentResource>(request, options, decoder_options, content);
    }
...
bool MimeTypeAllowed(const ResourceResponse& response)
{
    ...
    return mime_type == "image/svg+xml" || mime_type == "text/xml" || mime_type == "application/xml" || mime_type == "application/xhtml+xml";
}

} // namespace

SVGDocumentResource* SVGDocumentResource::Fetch(FetchParameters& params, ResourceFetcher* fetcher, AgentGroupScheduler& agent_group_scheduler)
{
    return To<SVGDocumentResource>(
        fetcher->RequestResource(params, SVGDocumentResourceFactory(agent_group_scheduler, agent_group_scheduler.DefaultTaskRunner()), nullptr));
}

SVGDocumentResource::SVGDocumentResource(const ResourceRequest& request, const ResourceLoaderOptions& options,
    const TextResourceDecoderOptions& decoder_options, SVGResourceDocumentContent* content)
    : TextResource(request, ResourceType::kSVGDocument, options, decoder_options)
    , content_(content)
{
}
...
void SVGDocumentResource::Finish(base::TimeTicks load_finish_time, base::SingleThreadTaskRunner* task_runner)
{
    ...
    if (MimeTypeAllowed(response) && HasData()) {
        update_status = content_->UpdateDocument(Data(), response.CurrentRequestUrl());
    }
...
void SVGDocumentResource::FinishAsError(const ResourceError& error, base::SingleThreadTaskRunner* task_runner)
{
