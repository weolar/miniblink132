...
public:
    void SetUp() override
    {
        scoped_feature_list_.InitAndEnableFeature(features::kWebFontsCacheAwareTimeoutAdaption);
...
public:
    void SetUp() override
    {
        scoped_feature_list_.InitWithFeatures({ features::kMemoryCacheStrongReference, features::kResourceFetcherStoresStrongReferences }, {});
        FontResourceTest::SetUp();
    }
...
// loading supports.
TEST_F(FontResourceTest, ResourceFetcherRevalidateDeferedResourceFromTwoInitiators)
{
    ...
    url_test_helpers::RegisterMockedURLLoadWithCustomResponse(url, "", WrappedResourceResponse(response));

    MockFetchContext* context = MakeGarbageCollected<MockFetchContext>();
    auto* properties = MakeGarbageCollected<TestResourceFetcherProperties>();
    auto* fetcher = MakeGarbageCollected<ResourceFetcher>(ResourceFetcherInit(properties->MakeDetachable(), context,
    ...
    FetchParameters fetch_params2 = FetchParameters::CreateForTest(std::move(request2));
    ...
    FetchParameters fetch_params3 = FetchParameters::CreateForTest(std::move(request3));
...
TEST_F(FontResourceTest, RevalidationPolicyMetrics)
{
    ...
    response_preload_font.SetHttpHeaderField(http_names::kCacheControl, AtomicString("max-age=3600"));
    url_test_helpers::RegisterMockedURLLoadWithCustomResponse(url_preload_font, "", WrappedResourceResponse(response_preload_font));

    // Test font preloads are immediately loaded.
    FetchParameters fetch_params_preload = FetchParameters::CreateForTest(ResourceRequest(url_preload_font));
    fetch_params_preload.SetLinkPreload(true);

    Resource* resource = FontResource::Fetch(fetch_params_preload, fetcher, nullptr);
    url_test_helpers::ServeAsynchronousRequests();
    ASSERT_TRUE(resource);
    EXPECT_TRUE(MemoryCache::Get()->Contains(resource));

    Resource* new_resource = FontResource::Fetch(fetch_params_preload, fetcher, nullptr);
    EXPECT_EQ(resource, new_resource);

    // Test histograms.
    histogram_tester.ExpectTotalCount("Blink.MemoryCache.RevalidationPolicy.Preload.Font", 2);
    histogram_tester.ExpectBucketCount(
        "Blink.MemoryCache.RevalidationPolicy.Preload.Font", static_cast<int>(ResourceFetcher::RevalidationPolicyForMetrics::kLoad), 1);
    histogram_tester.ExpectBucketCount(
        "Blink.MemoryCache.RevalidationPolicy.Preload.Font", static_cast<int>(ResourceFetcher::RevalidationPolicyForMetrics::kUse), 1);
    ...
    response_font.SetHttpHeaderField(http_names::kCacheControl, AtomicString("max-age=3600"));
    url_test_helpers::RegisterMockedURLLoadWithCustomResponse(url_font, "", WrappedResourceResponse(response_font));

    // Test deferred and ordinal font loads are correctly counted as deferred.
    FetchParameters fetch_params = FetchParameters::CreateForTest(ResourceRequest(url_font));
    resource = FontResource::Fetch(fetch_params, fetcher, nullptr);
    ASSERT_TRUE(resource);
    histogram_tester.ExpectTotalCount("Blink.MemoryCache.RevalidationPolicy.Font", 1);
    histogram_tester.ExpectBucketCount("Blink.MemoryCache.RevalidationPolicy.Font", static_cast<int>(ResourceFetcher::RevalidationPolicyForMetrics::kDefer), 1);
    fetcher->StartLoad(resource);
    url_test_helpers::ServeAsynchronousRequests();
    histogram_tester.ExpectTotalCount("Blink.MemoryCache.RevalidationPolicy.Font", 2);
    histogram_tester.ExpectBucketCount(
        "Blink.MemoryCache.RevalidationPolicy.Font", static_cast<int>(ResourceFetcher::RevalidationPolicyForMetrics::kPreviouslyDeferredLoad), 1);
    // Load the resource again, deferred resource already loaded shall be counted
    // as kUse.
    resource = FontResource::Fetch(fetch_params, fetcher, nullptr);
    histogram_tester.ExpectTotalCount("Blink.MemoryCache.RevalidationPolicy.Font", 3);
    histogram_tester.ExpectBucketCount("Blink.MemoryCache.RevalidationPolicy.Font", static_cast<int>(ResourceFetcher::RevalidationPolicyForMetrics::kUse), 1);
}
...
TEST_F(CacheAwareFontResourceTest, CacheAwareFontLoading)
{
    ...
    auto* src_uri_value = MakeGarbageCollected<cssvalue::CSSURIValue>(
        CSSUrlData(AtomicString(url.GetString()), url, Referrer(document.Url(), document.GetReferrerPolicy()), OriginClean::kTrue, false /* is_ad_related */));
    auto* src_value = CSSFontFaceSrcValue::Create(src_uri_value, nullptr /* world */);
    ...
    Persistent<MockFontResourceClient> client = MakeGarbageCollected<MockFontResourceClient>();
    FontResource& resource = src_value->Fetch(document.GetExecutionContext(), client);
    ...
    Persistent<MockFontResourceClient> client2 = MakeGarbageCollected<MockFontResourceClient>();
    FontResource& resource2 = src_value->Fetch(document.GetExecutionContext(), client2);
    ...
    Persistent<MockFontResourceClient> client3 = MakeGarbageCollected<MockFontResourceClient>();
    FontResource& resource3 = src_value->Fetch(document.GetExecutionContext(), client3);
...
TEST_F(FontResourceStrongReferenceTest, FollowCacheControl)
{
    ...
    response_font_no_store.SetHttpHeaderField(http_names::kCacheControl, AtomicString("no-cache, no-store"));
    url_test_helpers::RegisterMockedURLLoadWithCustomResponse(url_font_no_store, "", WrappedResourceResponse(response_font_no_store));

    FetchParameters fetch_params_no_store = FetchParameters::CreateForTest(ResourceRequest(url_font_no_store));
    Resource* resource_no_store = FontResource::Fetch(fetch_params_no_store, fetcher, nullptr);
...
using LoadStartCallback = base::OnceCallback<void(mojo::PendingReceiver<network::mojom::URLLoader>, mojo::PendingRemote<network::mojom::URLLoaderClient>)>;

class FakeLoaderFactory final : public ResourceFetcher::LoaderFactory {
public:
    FakeLoaderFactory(scoped_refptr<base::SingleThreadTaskRunner> unfreezable_task_runner, scoped_refptr<base::SequencedTaskRunner> background_task_runner,
        LoadStartCallback load_start_callback)
        : unfreezable_task_runner_(std::move(unfreezable_task_runner))
        , background_resource_fetch_assets_(base::MakeRefCounted<FakeBackgroundResourceFetchAssets>(background_task_runner, std::move(load_start_callback)))
    {
    }
    ...
    {
        return std::make_unique<BackgroundURLLoader>(background_resource_fetch_assets_,
...
private:
    ...
    scoped_refptr<WebBackgroundResourceFetchAssets> background_resource_fetch_assets_;
};

class TestFontResourceClient final : public GarbageCollected<TestFontResourceClient>, public FontResourceClient {
public:
...
{
    std::optional<Vector<char>> font_data = test::ReadFromFile(test::CoreTestDataPath("Ahem.woff2"));
...
public:
    FontResourceBackgroundProcessorTest()
        : url_(String("http://font-test.example.com/foo" + base::NumberToString(url_counter_++)))
    {
        feature_list_.InitWithFeaturesAndParameters({ { features::kBackgroundResourceFetch, { { "background-font-response-processor", "true" } } } }, {});
    }
...
TEST_F(FontResourceBackgroundProcessorTest, Basic)
{
    ...
    scoped_refptr<base::SequencedTaskRunner> background_task_runner = base::ThreadPool::CreateSequencedTaskRunner({ base::TaskPriority::USER_BLOCKING });

    mojo::PendingReceiver<network::mojom::URLLoader> loader_pending_receiver;
    mojo::PendingRemote<network::mojom::URLLoaderClient> loader_client_pending_remote;

    base::RunLoop run_loop_for_request;
    FakeLoaderFactory* fake_loader_factory = MakeGarbageCollected<FakeLoaderFactory>(task_runner, background_task_runner,
    ...
    TestFontResourceClient* resource_client = MakeGarbageCollected<TestFontResourceClient>(run_loop.QuitClosure());

    ResourceRequest request(url_);
    FetchParameters fetch_params = FetchParameters::CreateForTest(std::move(request));
    FontResource* resource = FontResource::Fetch(fetch_params, fetcher, resource_client);
    ...
    mojo::Remote<network::mojom::URLLoaderClient> loader_client_remote(std::move(loader_client_pending_remote));
    loader_client_remote->OnReceiveResponse(CreateTestResponse(), CreateTestFontDataPipe(),
...
TEST_F(FontResourceBackgroundProcessorTest, InvalidFontData)
{
    ...
    ASSERT_TRUE(loader_client_pending_remote);
    mojo::Remote<network::mojom::URLLoaderClient> loader_client_remote(std::move(loader_client_pending_remote));
    loader_client_remote->OnReceiveResponse(CreateTestResponse(), CreateTestTooSmallFontDataPipe(),
        /*cached_metadata=*/std::nullopt);
    loader_client_remote->OnComplete(network::URLLoaderCompletionStatus(net::OK));
