...
class CORE_EXPORT ImageResource final : public Resource, public MultipartImageResourceParser::Client {
public:
    ...
    static ImageResource* Create(const ResourceRequest&, const DOMWrapperWorld* world);
    static ImageResource* CreateForTest(const KURL&);

    ImageResource(const ResourceRequest&, const ResourceLoaderOptions&, ImageResourceContent*);
    ...
    void Finish(base::TimeTicks finish_time, base::SingleThreadTaskRunner*) override;
    void FinishAsError(const ResourceError&, base::SingleThreadTaskRunner*) override;
...
private:
    ...
    bool IsAccessAllowed(ImageResourceInfo::DoesCurrentFrameHaveSingleSecurityOrigin) const;

    std::pair<ResourcePriority, ResourcePriority> ComputePriorityFromObservers() override;

    bool HasClientsOrObservers() const override;

    void UpdateImageAndClearBuffer();
    void UpdateImage(scoped_refptr<SharedBuffer>, ImageResourceContent::UpdateImageOption, bool all_data_received);
    ...
    MultipartParsingState multipart_parsing_state_ = MultipartParsingState::kWaitingForFirstPart;
...
template <> struct DowncastTraits<ImageResource> {
