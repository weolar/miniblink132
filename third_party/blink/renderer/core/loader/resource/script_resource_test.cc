...
TEST(ScriptResourceTest, SuccessfulRevalidation)
{
    ...
    ScriptResource* resource = ScriptResource::CreateForTest(scope.GetIsolate(), url, UTF8Encoding());
...
TEST(ScriptResourceTest, FailedRevalidation)
{
    ...
    V8TestingScope scope;
    const KURL url("https://www.example.com/script.js");
    ScriptResource* resource = ScriptResource::CreateForTest(scope.GetIsolate(), url, Latin1Encoding());
    ResourceResponse response(url);
    response.SetHttpStatusCode(200);
...
TEST(ScriptResourceTest, RedirectDuringRevalidation)
{
    ...
    revalidation_response.SetHttpHeaderField(http_names::kLocation, AtomicString(destination.GetString()));
