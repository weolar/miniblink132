...
class MockClient final : public GarbageCollected<MockClient>, public MultipartImageResourceParser::Client {
public:
...
TEST(MultipartResponseTest, SkippableLength)
{
    ...
    for (const auto& test : line_tests) {
        ...
        EXPECT_EQ(test.expected, MultipartImageResourceParser::SkippableLengthForTest(input, test.position));
    }
}
...
TEST(MultipartResponseTest, FindBoundary)
{
    ...
    for (const auto& test : boundary_tests) {
        ...
        EXPECT_EQ(test.position, MultipartImageResourceParser::FindBoundaryForTest(data, &boundary));
    }
}
...
TEST(MultipartResponseTest, NoStartBoundary)
{
    ...
    MultipartImageResourceParser* parser = MakeGarbageCollected<MultipartImageResourceParser>(response, boundary, client);
    const char kData[] = "Content-type: text/plain\n\n"
...
void VariousChunkSizesTest(base::span<const TestChunk> chunks, size_t responses, int received_data, const char* completed_data)
{
    const char kData[] = "--bound\n" // 0-7
    ...
    auto* parser = MakeGarbageCollected<MultipartImageResourceParser>(response, boundary, client);

    for (const auto& chunk : chunks) {
        ASSERT_LT(chunk.start_position, chunk.end_position);
        parser->AppendData(data.subspan(chunk.start_position, chunk.end_position - chunk.start_position));
        EXPECT_EQ(chunk.expected_responses, client->responses_.size());
        EXPECT_EQ(String(chunk.expected_data), client->data_.size() > 0 ? ToString(client->data_.back()) : String(""));
    }
    // Check final state
    parser->Finish();
...
TEST(MultipartResponseTest, SmallChunk)
{
    ...
    const char kData[] = "--boundContent-type: text/plain\n\n"
...
TEST(MultipartResponseTest, EatLeadingLF)
{
    ...
    ASSERT_EQ(4u, client->responses_.size());
    ASSERT_EQ(4u, client->data_.size());
    EXPECT_EQ(String(), client->responses_[0].HttpHeaderField(http_names::kLowerContentType));
    EXPECT_EQ("", ToString(client->data_[0]));
    EXPECT_EQ(String(), client->responses_[1].HttpHeaderField(http_names::kLowerContentType));
    EXPECT_EQ("\ncontent-type: 1\n\n\n\n", ToString(client->data_[1]));
    EXPECT_EQ(String(), client->responses_[2].HttpHeaderField(http_names::kLowerContentType));
    EXPECT_EQ("content-type: 2\n\n\n\n", ToString(client->data_[2]));
    EXPECT_EQ("3", client->responses_[3].HttpHeaderField(http_names::kLowerContentType));
