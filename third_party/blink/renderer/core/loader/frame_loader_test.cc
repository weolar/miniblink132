...
TEST_F(FrameLoaderSimTest, LoadEventProgressBeforeUnloadCanceled)
{
    ...
    LocalFrame::NotifyUserActivation(frame_b, mojom::UserActivationNotificationType::kTest);

    auto& chrome_client = To<ChromeClientImpl>(WebView().GetPage()->GetChromeClient());
...
class FrameLoaderJavaScriptUrlWebFrameClient : public frame_test_helpers::TestWebFrameClient {
public:
...
TEST_F(FrameLoaderJavaScriptUrlTest, Click)
{
    ...
    GetDocument().GetFrame()->GetTaskRunner(TaskType::kNetworking)->PostTask(FROM_HERE, run_loop.QuitClosure());
    run_loop.Run();
...
TEST_F(FrameLoaderTest, PolicyContainerIsStoredOnCommitNavigation)
{
    ...
    EXPECT_EQ(*mojom::blink::PolicyContainerPolicies::New(network::CrossOriginEmbedderPolicy(network::mojom::CrossOriginEmbedderPolicyValue::kNone),
                  network::mojom::ReferrerPolicy::kAlways, Vector<network::mojom::blink::ContentSecurityPolicyPtr>(),
                  /*anonymous=*/false, network::mojom::WebSandboxFlags::kNone, network::mojom::blink::IPAddressSpace::kUnknown,
