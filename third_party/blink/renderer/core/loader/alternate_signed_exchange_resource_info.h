...
public:
    ...
    public:
        Entry(const KURL& anchor_url, const KURL& alternative_url, const String& header_integrity, const String& variants, const String& variant_key)
        ...
        const KURL& anchor_url() const
        {
            return anchor_url_;
        }
        const KURL& alternative_url() const
        {
            return alternative_url_;
        }
        const String& header_integrity() const
        {
            return header_integrity_;
        }
        const String& variants() const
        {
            return variants_;
        }
        const String& variant_key() const
        {
            return variant_key_;
        }
    ...
    using EntryMap = HashMap<KURL /* anchor_url */, Vector<std::unique_ptr<Entry>>>;

    static std::unique_ptr<AlternateSignedExchangeResourceInfo> CreateIfValid(const String& outer_link_header, const String& inner_link_header);

    explicit AlternateSignedExchangeResourceInfo(EntryMap alternative_resources);
    AlternateSignedExchangeResourceInfo(const AlternateSignedExchangeResourceInfo&) = delete;
    AlternateSignedExchangeResourceInfo& operator=(const AlternateSignedExchangeResourceInfo&) = delete;
    ...
    Entry* FindMatchingEntry(const KURL& url, std::optional<ResourceType> resource_type, const Vector<String>& languages) const;
    Entry* FindMatchingEntry(const KURL& url, network::mojom::RequestDestination request_destination, const Vector<String>& languages) const;

private:
    friend class AlternateSignedExchangeResourceInfoTest;

    Entry* FindMatchingEntry(const KURL& url, const char* accept_header, const Vector<String>& languages) const;
