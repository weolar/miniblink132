...
bool CanReuseFromListOfAvailableImages(const Resource* resource, CrossOriginAttributeValue cross_origin_attribute, const SecurityOrigin* origin)
{
    ...
    if (request.GetCredentialsMode() == network::mojom::CredentialsMode::kSameOrigin && cross_origin_attribute != kCrossOriginAttributeAnonymous) {
        return false;
    }
...
{
    RESOURCE_LOADING_DVLOG(1) << "~ImageLoader " << this << "; has pending load event=" << pending_load_event_.IsActive()
...
void ImageLoader::DispatchDecodeRequestsIfComplete()
{
    ...
    while (it != decode_requests_.end()) {
        ...
        frame->GetChromeClient().RequestDecode(frame, image->PaintImageForCurrentFrame(),
            WTF::BindOnce(&ImageLoader::DecodeRequestFinished, MakeUnwrappingCrossThreadHandle(this), request->request_id()));
...
void ImageLoader::RejectPendingDecodes(UpdateType update_type)
{
    ...
    for (auto it = decode_requests_.begin(); it != decode_requests_.end();) {
        auto& request = *it;
        if (update_type == UpdateType::kAsync && request->state() == DecodeRequest::kPendingMicrotask) {
            ++it;
            continue;
        }
        request->Reject();
...
bool ImageLoader::ImageIsPotentiallyAvailable() const
{
    ...
    bool image_has_loaded = image_content_ && !image_content_->IsLoading() && !image_content_->ErrorOccurred();
    bool image_still_loading = !image_has_loaded && HasPendingActivity() && !HasPendingError() && !element_->ImageSourceURL().empty();
    bool image_has_image = image_content_ && image_content_->HasImage();
    bool image_is_document = element_->GetDocument().IsImageDocument() && image_content_ && !image_content_->ErrorOccurred();
    ...
    return (image_has_loaded && image_has_image) || image_still_loading || image_is_document || is_lazyload;
}
...
void ImageLoader::SetImageWithoutConsideringPendingLoadEvent(ImageResourceContent* new_image_content)
{
...
static void ConfigureRequest(FetchParameters& params, Element& element, const ClientHintsPreferences& client_hints_preferences)
{
    CrossOriginAttributeValue cross_origin = GetCrossOriginAttributeValue(element.FastGetAttribute(html_names::kCrossoriginAttr));
    if (cross_origin != kCrossOriginAttributeNotSet) {
        params.SetCrossOriginAccessControl(element.GetExecutionContext()->GetSecurityOrigin(), cross_origin);
    }

    mojom::blink::FetchPriorityHint fetch_priority_hint = GetFetchPriorityAttributeValue(element.FastGetAttribute(html_names::kFetchpriorityAttr));
    params.SetFetchPriorityHint(fetch_priority_hint);

    auto* html_image_element = DynamicTo<HTMLImageElement>(element);
    if ((client_hints_preferences.ShouldSend(network::mojom::WebClientHintsType::kResourceWidth_DEPRECATED)
            || client_hints_preferences.ShouldSend(network::mojom::WebClientHintsType::kResourceWidth))
        && html_image_element) {
...
inline void ImageLoader::QueuePendingErrorEvent()
{
    ...
    // crbug.com/722500
    pending_error_event_ = PostCancellableTask(*GetElement()->GetDocument().GetTaskRunner(TaskType::kDOMManipulation), FROM_HERE,
        WTF::BindOnce(
            &ImageLoader::DispatchPendingErrorEvent, WrapPersistent(this), std::make_unique<IncrementLoadEventDelayCount>(GetElement()->GetDocument())));
}

inline void ImageLoader::CrossSiteOrCSPViolationOccurred(AtomicString image_source_url)
{
...
inline void ImageLoader::EnqueueImageLoadingMicroTask(UpdateFromElementBehavior update_behavior)
{
    auto task = std::make_unique<Task>(this, update_behavior);
    pending_task_ = task->GetWeakPtr();
    element_->GetDocument().GetAgent().event_loop()->EnqueueMicrotask(WTF::BindOnce(&Task::Run, std::move(task)));
    delay_until_do_update_from_element_ = std::make_unique<IncrementLoadEventDelayCount>(element_->GetDocument());
}
...
void ImageLoader::DoUpdateFromElement(const DOMWrapperWorld* world, UpdateFromElementBehavior update_behavior, UpdateType update_type, bool force_blocking)
{
    // FIXME: According to
    ...
    if (!url.IsNull() && !url.IsEmpty()) {
        ...
        AtomicString referrer_policy_attribute = element_->FastGetAttribute(html_names::kReferrerpolicyAttr);
        if (!referrer_policy_attribute.IsNull()) {
            SecurityPolicy::ReferrerPolicyFromString(referrer_policy_attribute, kSupportReferrerPolicyLegacyKeywords, &referrer_policy);
        }
        resource_request.SetReferrerPolicy(referrer_policy);

        // Correct the RequestContext if necessary.
        if (IsA<HTMLPictureElement>(GetElement()->parentNode()) || !GetElement()->FastGetAttribute(html_names::kSrcsetAttr).IsNull()) {
            resource_request.SetRequestContext(mojom::blink::RequestContextType::IMAGE_SET);
            resource_request.SetRequestDestination(network::mojom::RequestDestination::kImage);
        } else if (IsA<HTMLObjectElement>(GetElement())) {
            resource_request.SetRequestContext(mojom::blink::RequestContextType::OBJECT);
            resource_request.SetRequestDestination(network::mojom::RequestDestination::kObject);
        } else if (IsA<HTMLEmbedElement>(GetElement())) {
            resource_request.SetRequestContext(mojom::blink::RequestContextType::EMBED);
            resource_request.SetRequestDestination(network::mojom::RequestDestination::kEmbed);
        }
        ...
        if (IsA<HTMLImageElement>(GetElement())) {
            if (GetElement()->FastHasAttribute(html_names::kAttributionsrcAttr)
                && frame->GetAttributionSrcLoader()->CanRegister(url, To<HTMLImageElement>(GetElement()),
                    /*request_id=*/std::nullopt)) {
                resource_request.SetAttributionReportingEligibility(network::mojom::AttributionReportingEligibility::kEventSourceOrTrigger);
            }
            bool shared_storage_writable_opted_in = GetElement()->FastHasAttribute(html_names::kSharedstoragewritableAttr)
                && RuntimeEnabledFeatures::SharedStorageAPIM118Enabled(GetElement()->GetExecutionContext())
                && GetElement()->GetExecutionContext()->IsSecureContext() && !SecurityOrigin::Create(url)->IsOpaque();
            resource_request.SetSharedStorageWritableOptedIn(shared_storage_writable_opted_in);
        }

        bool page_is_being_dismissed = document.PageDismissalEventBeingDispatched() != Document::kNoDismissal;
        if (page_is_being_dismissed) {
            resource_request.SetHttpHeaderField(http_names::kCacheControl, AtomicString("max-age=0"));
        ...
        if (update_behavior != kUpdateForcedReload && lazy_image_load_state_ != LazyImageLoadState::kFullImage) {
        ...
        if (lazy_image_load_state_ == LazyImageLoadState::kFullImage && !force_blocking) {
    ...
    if (update_behavior == kUpdateSizeChanged && element_->GetLayoutObject() && element_->GetLayoutObject()->IsImage()
        ...
        if (new_image_content && new_image_content->GetContentStatus() == ResourceStatus::kNotStarted && !is_lazyload)
            NoImageResourceToLoad();
...
void ImageLoader::UpdateFromElement(UpdateFromElementBehavior update_behavior, bool force_blocking)
{
    ...
    if (image_content_for_image_document_) {
        ...
        SetImageWithoutConsideringPendingLoadEvent(image_content_for_image_document_);
        image_content_for_image_document_ = nullptr;
        return;
    }
    ...
    if (ShouldLoadImmediately(ImageSourceToKURL(image_source_url)) && update_behavior != kUpdateFromMicrotask) {
        DoUpdateFromElement(element_->GetExecutionContext()->GetCurrentWorld(), update_behavior, UpdateType::kSync, force_blocking);
        return;
    }
...
KURL ImageLoader::ImageSourceToKURL(AtomicString image_source_url) const
{
    ...
    // an empty string.
    if (!image_source_url.IsNull()) {
        String stripped_image_source_url = StripLeadingAndTrailingHTMLSpaces(image_source_url);
...
bool ImageLoader::ShouldLoadImmediately(const KURL& url) const
{
    ...
    if (!url.IsNull()) {
        Resource* resource = MemoryCache::Get()->ResourceForURL(url, element_->GetDocument().Fetcher()->GetCacheIdentifier(url, /*skip_service_worker=*/false));

        if (resource && !resource->ErrorOccurred()
            && CanReuseFromListOfAvailableImages(resource, GetCrossOriginAttributeValue(element_->FastGetAttribute(html_names::kCrossoriginAttr)),
                element_->GetExecutionContext()->GetSecurityOrigin())) {
            return true;
        }
    }

    return (IsA<HTMLObjectElement>(*element_) || IsA<HTMLEmbedElement>(*element_) || IsA<HTMLVideoElement>(*element_));
}

void ImageLoader::ImageChanged(ImageResourceContent* content, CanDeferInvalidation)
{
    DCHECK_EQ(content, image_content_.Get());
    if (image_complete_ || !content->IsLoading() || delay_until_image_notify_finished_)
        return;
    ...
    delay_until_image_notify_finished_ = std::make_unique<IncrementLoadEventDelayCount>(document);
}

void ImageLoader::ImageNotifyFinished(ImageResourceContent* content)
{
    RESOURCE_LOADING_DVLOG(1) << "ImageLoader::imageNotifyFinished " << this << "; has pending load event=" << pending_load_event_.IsActive();
    ...
    pending_load_event_ = PostCancellableTask(*GetElement()->GetDocument().GetTaskRunner(TaskType::kDOMManipulation), FROM_HERE,
        WTF::BindOnce(
            &ImageLoader::DispatchPendingLoadEvent, WrapPersistent(this), std::make_unique<IncrementLoadEventDelayCount>(GetElement()->GetDocument())));
}
...
LayoutImageResource* ImageLoader::GetLayoutImageResource() const
{
    ...
    if (layout_object->IsImage() && !To<LayoutImage>(layout_object)->IsGeneratedContent())
...
void ImageLoader::UpdateLayoutObject()
{
    ...
    if (image_content_ != cached_image_content && (image_complete_ || !cached_image_content))
...
ResourcePriority ImageLoader::ComputeResourcePriority() const
{
    ...
    static const bool is_image_lcpp_enabled = base::FeatureList::IsEnabled(features::kLCPCriticalPathPredictor)
        && features::kLCPCriticalPathPredictorImageLoadPriorityEnabledForHTMLImageElement.Get();
...
bool ImageLoader::HasPendingEvent() const
{
    ...
    if (image_content_ && !image_complete_ && lazy_image_load_state_ != LazyImageLoadState::kDeferred) {
        return true;
    }

    if (pending_load_event_.IsActive() || pending_error_event_.IsActive() || !decode_requests_.empty()) {
        return true;
    }
...
void ImageLoader::DispatchPendingLoadEvent(std::unique_ptr<IncrementLoadEventDelayCount> count)
{
    if (!image_content_)
        return;
    CHECK(image_complete_);
...
void ImageLoader::DispatchPendingErrorEvent(std::unique_ptr<IncrementLoadEventDelayCount> count)
{
    DispatchErrorEvent();
...
bool ImageLoader::GetImageAnimationPolicy(mojom::blink::ImageAnimationPolicy& policy)
{
...
ScriptPromise<IDLUndefined> ImageLoader::Decode(ScriptState* script_state, ExceptionState& exception_state)
{
    ...
    if (!script_state->ContextIsValid() || !execution_context) {
        exception_state.ThrowDOMException(DOMExceptionCode::kEncodingError, "The source image cannot be decoded.");
        return EmptyPromise();
    }
    ...
    auto* request
        = MakeGarbageCollected<DecodeRequest>(this, MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state, exception_state.GetContext()));
    execution_context->GetAgent()->event_loop()->EnqueueMicrotask(WTF::BindOnce(&DecodeRequest::ProcessForTask, WrapWeakPersistent(request)));
    decode_requests_.push_back(request);
    return request->promise();
}

void ImageLoader::LoadDeferredImage(bool force_blocking, bool update_from_microtask)
{
    if (lazy_image_load_state_ != LazyImageLoadState::kDeferred)
        return;
    ...
    // fully loaded now.
    UpdateFromElement(update_from_microtask ? kUpdateFromMicrotask : kUpdateNormal, force_blocking);
}

void ImageLoader::ElementDidMoveToNewDocument()
{
    if (delay_until_do_update_from_element_) {
        delay_until_do_update_from_element_->DocumentChanged(element_->GetDocument());
    }
    if (delay_until_image_notify_finished_) {
        delay_until_image_notify_finished_->DocumentChanged(element_->GetDocument());
    }
    ClearFailedLoadURL();
    ClearImage();
}
...
ImageLoader::DecodeRequest::DecodeRequest(ImageLoader* loader, ScriptPromiseResolver<IDLUndefined>* resolver)
...
{
    resolver_->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kEncodingError, "The source image cannot be decoded."));
    loader_ = nullptr;
}
