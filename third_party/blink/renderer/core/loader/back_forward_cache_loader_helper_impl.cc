...
BackForwardCacheLoaderHelperImpl::BackForwardCacheLoaderHelperImpl(Delegate& delegate)
    : delegate_(&delegate)
{
}

void BackForwardCacheLoaderHelperImpl::EvictFromBackForwardCache(mojom::blink::RendererEvictionReason reason)
{
    if (!delegate_)
        return;
...
void BackForwardCacheLoaderHelperImpl::DidBufferLoadWhileInBackForwardCache(bool update_process_wide_count, size_t num_bytes)
{
    if (!delegate_)
        return;
    delegate_->DidBufferLoadWhileInBackForwardCache(update_process_wide_count, num_bytes);
}
