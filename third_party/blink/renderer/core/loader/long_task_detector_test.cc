...
protected:
    ...
    base::TimeTicks DummyTaskStartTime()
    {
        return dummy_task_start_time_;
    }
    ...
    void SimulateTask(base::TimeDelta duration)
    {
        platform_->GetMainThreadScheduler()->DefaultTaskRunner()->PostTask(
            FROM_HERE, WTF::BindOnce(&LongTaskDetectorTest::DummyTaskWithDuration, WTF::Unretained(this), duration));
...
{
    TestLongTaskObserver* long_task_observer = MakeGarbageCollected<TestLongTaskObserver>();
    ...
    EXPECT_EQ(long_task_observer->last_long_task_end, long_task_end_when_registered);
