...
scoped_refptr<EncodedFormData> BeaconBlob::GetEncodedFormData() const
{
    ...
    if (data_->HasBackingFile()) {
        entity_body->AppendFile(To<File>(data_)->GetPath(), To<File>(data_)->LastModifiedTime());
    } else {
...
BeaconDOMArrayBufferView::BeaconDOMArrayBufferView(DOMArrayBufferView* data)
    : data_(data)
{
    CHECK(base::CheckedNumeric<wtf_size_t>(data->byteLength()).IsValid()) << "EncodedFormData::Create cannot deal with huge ArrayBuffers.";
}
...
scoped_refptr<EncodedFormData> BeaconDOMArrayBufferView::GetEncodedFormData() const
{
    DCHECK(data_);

    return EncodedFormData::Create(data_->BaseAddress(), base::checked_cast<wtf_size_t>(data_->byteLength()));
}
...
scoped_refptr<EncodedFormData> BeaconDOMArrayBuffer::GetEncodedFormData() const
{
    DCHECK(data_);

    return EncodedFormData::Create(data_->Data(), base::checked_cast<wtf_size_t>(data_->ByteLength()));
}
...
scoped_refptr<EncodedFormData> BeaconURLSearchParams::GetEncodedFormData() const
{
    DCHECK(data_);
...
BeaconFormData::BeaconFormData(FormData* data)
    ...
    , content_type_(String("multipart/form-data; boundary=") + entity_body_->Boundary().data())
{
}
