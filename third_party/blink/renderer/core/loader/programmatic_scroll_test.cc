...
TEST_F(ProgrammaticScrollTest, RestoreScrollPositionAndViewStateWithScale)
{
    ...
    WebViewImpl* web_view = web_view_helper.InitializeAndLoad(base_url_.Utf8() + "long_scroll.html");
    ...
    loader.GetDocumentLoader()->GetInitialScrollState().was_scrolled_by_user = false;
    loader.GetDocumentLoader()->GetHistoryItem()->SetPageScaleFactor(2);
    loader.GetDocumentLoader()->GetHistoryItem()->SetScrollOffset(ScrollOffset(0, 200));
...
TEST_F(ProgrammaticScrollTest, RestoreScrollPositionAndViewStateWithoutScale)
{
    ...
    loader.GetDocumentLoader()->GetHistoryItem()->SetScrollOffset(ScrollOffset(0, 400));
...
TEST_F(ProgrammaticScrollTest, SaveScrollStateClearsAnchor)
{
    ...
    web_view->MainFrameImpl()->SetScrollOffset(gfx::PointF(0, 500));
    loader.GetDocumentLoader()->GetInitialScrollState().was_scrolled_by_user = true;
...
TEST_F(ProgrammaticScrollSimTest, NavigateToHash)
{
    ...
    main_resource.Write("<!DOCTYPE html><link id=link rel=stylesheet href=test.css>");
    css_resource.Start();
