...
class CORE_EXPORT InteractiveDetector : public GarbageCollected<InteractiveDetector>,
...
public:
    ...
    public:
        explicit NetworkActivityChecker(Document* document)
            : document_(document)
        {
        }
    ...
    explicit InteractiveDetector(Document&, std::unique_ptr<NetworkActivityChecker>);
    ...
    WTF::Vector<std::optional<base::TimeDelta>> GetFirstInputDelaysAfterBackForwardCacheRestore() const;
    ...
    void HandleForInputDelay(const Event&, base::TimeTicks event_platform_timestamp, base::TimeTicks processing_start);
    ...
    void SetTaskRunnerForTesting(scoped_refptr<base::SingleThreadTaskRunner> task_runner_for_testing);
    // The caller owns the |clock| which must outlive the InteractiveDetector.
    void SetTickClockForTesting(const base::TickClock* clock);

    void RecordInputEventTimingUMA(base::TimeDelta processing_time, base::TimeDelta time_to_next_paint);

    void DidObserveFirstScrollDelay(base::TimeDelta first_scroll_delay, base::TimeTicks first_scroll_timestamp);
...
private:
    ...
    struct {
        ...
        WTF::Vector<std::optional<base::TimeDelta>> first_input_delays_after_back_forward_cache_restore;
    } page_event_times_;
    ...
    void UpdateNetworkQuietState(double request_count, std::optional<base::TimeTicks> current_time);
    ...
    // window is found.
    base::TimeTicks FindInteractiveCandidate(base::TimeTicks lower_bound, base::TimeTicks current_time);

    // LongTaskObserver implementation
    void OnLongTaskDetected(base::TimeTicks start_time, base::TimeTicks end_time) override;
