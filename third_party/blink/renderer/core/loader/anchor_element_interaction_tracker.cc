...
const base::TimeDelta kMouseAccelerationAndVelocityInterval { base::Milliseconds(50) };
...
ViewportHeuristicConfig GetViewportHeuristicConfig()
{
    ...
    const base::FeatureParam<double> kDistanceFromPointerDownLowerBound { &features::kPreloadingViewportHeuristics, "distance_from_ptr_down_low", -0.3 };
    // 0.0 is the upper bound of the middle 75% of distance_from_ptr_down_ratio
    // values of clicked anchors (i.e. the P87.5 value).
    const base::FeatureParam<double> kDistanceFromPointerDownUpperBound { &features::kPreloadingViewportHeuristics, "distance_from_ptr_down_hi", 0.0 };
    // Note: The default value was selected arbitrarily and hasn't been tuned.
    const base::FeatureParam<double> kLargestAnchorThreshold { &features::kPreloadingViewportHeuristics, "largest_anchor_threshold", 0.5 };
    // Note: The default value was selected arbitrarily and hasn't been tuned.
    const base::FeatureParam<base::TimeDelta> kDelay { &features::kPreloadingViewportHeuristics, "delay", base::Milliseconds(1000) };
    ...
    return { .distance_from_ptr_down_ratio_bounds = std::make_pair(low, high),
...
AnchorElementInteractionTracker::MouseMotionEstimator::MouseMotionEstimator(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    : update_timer_(task_runner, this, &AnchorElementInteractionTracker::MouseMotionEstimator::OnTimer)
    , clock_(base::DefaultTickClock::GetInstance())
{
    CHECK(clock_);
}

void AnchorElementInteractionTracker::MouseMotionEstimator::Trace(Visitor* visitor) const
{
    visitor->Trace(update_timer_);
}

double AnchorElementInteractionTracker::MouseMotionEstimator::GetMouseTangentialAcceleration() const
{
    // Tangential acceleration = (a.v)/|v|
    return DotProduct(acceleration_, velocity_) / std::max(static_cast<double>(velocity_.Length()), eps);
}

inline void AnchorElementInteractionTracker::MouseMotionEstimator::AddDataPoint(base::TimeTicks timestamp, gfx::PointF position)
{
    mouse_position_and_timestamps_.push_front(MousePositionAndTimeStamp { .position = position, .ts = timestamp });
}
inline void AnchorElementInteractionTracker::MouseMotionEstimator::RemoveOldDataPoints(base::TimeTicks now)
{
    while (!mouse_position_and_timestamps_.empty() && (now - mouse_position_and_timestamps_.back().ts) > kMousePosQueueTimeDelta) {
...
void AnchorElementInteractionTracker::MouseMotionEstimator::Update()
{
    ...
    auto replace_zero_with_eps = [](double x) { return x >= 0.0 ? std::max(x, eps) : std::min(x, -eps); };
    ...
    if (mouse_position_and_timestamps_.size() == 2u) {
        ...
        velocity_.InvScale(replace_zero_with_eps((front.ts - back.ts).InSecondsF()));
        return;
    }
    ...
    acceleration_.set_x(kRescaleAcceleration * (a22 * x1 - a12 * x2) / determinant);
    velocity_.set_x(kRescaleVelocity * (-a12 * x1 + a11 * x2) / determinant + acceleration_.x() * (front.ts - back.ts).InSecondsF());

    acceleration_.set_y(kRescaleAcceleration * (a22 * y1 - a12 * y2) / determinant);
    velocity_.set_y(kRescaleVelocity * (-a12 * y1 + a11 * y2) / determinant + acceleration_.y() * (front.ts - back.ts).InSecondsF());
}

void AnchorElementInteractionTracker::MouseMotionEstimator::OnTimer(TimerBase*)
{
...
void AnchorElementInteractionTracker::MouseMotionEstimator::OnMouseMoveEvent(gfx::PointF position)
{
...
void AnchorElementInteractionTracker::MouseMotionEstimator::SetTaskRunnerForTesting(
    scoped_refptr<base::SingleThreadTaskRunner> task_runner, const base::TickClock* clock)
{
    update_timer_.SetTaskRunnerForTesting(task_runner, clock);
    clock_ = clock;
}

AnchorElementInteractionTracker::AnchorElementInteractionTracker(Document& document)
    : mouse_motion_estimator_(MakeGarbageCollected<MouseMotionEstimator>(document.GetTaskRunner(TaskType::kUserInteraction)))
    , interaction_host_(document.GetExecutionContext())
    , hover_timer_(document.GetTaskRunner(TaskType::kUserInteraction), this, &AnchorElementInteractionTracker::HoverTimerFired)
    , clock_(base::DefaultTickClock::GetInstance())
    , document_(&document)
    , viewport_heuristic_timer_(document.GetTaskRunner(TaskType::kUserInteraction), this, &AnchorElementInteractionTracker::ViewportHeuristicTimerFired)
{
    document.GetFrame()->GetBrowserInterfaceBroker().GetInterface(
        interaction_host_.BindNewPipeAndPassReceiver(document.GetExecutionContext()->GetTaskRunner(TaskType::kInternalDefault)));
    if (base::FeatureList::IsEnabled(blink::features::kPreloadingViewportHeuristics)) {
        auto* anchor_metrics_sender = AnchorElementMetricsSender::GetForFrame(GetDocument()->GetFrame());
        auto* anchor_viewport_observer = AnchorElementViewportPositionTracker::MaybeGetOrCreateFor(document);
        ...
        if (anchor_viewport_observer && anchor_metrics_sender && anchor_metrics_sender->AllAnchorsSampledIn()) {
...
{
    static base::FeatureParam<base::TimeDelta> hover_dwell_time { &blink::features::kSpeculationRulesPointerHoverHeuristics, "HoverDwellTime",
        base::Milliseconds(200) };
    return hover_dwell_time.Get();
}

void AnchorElementInteractionTracker::OnMouseMoveEvent(const WebMouseEvent& mouse_event)
{
    mouse_motion_estimator_->OnMouseMoveEvent(mouse_event.PositionInScreen());
}

void AnchorElementInteractionTracker::OnPointerEvent(EventTarget& target, const PointerEvent& pointer_event)
{
    if (!target.ToNode()) {
        return;
    }
    ...
    if (event_type == event_type_names::kPointerdown) {
        ...
        if (auto* viewport_position_tracker = AnchorElementViewportPositionTracker::MaybeGetOrCreateFor(*GetDocument())) {
    ...
    HTMLAnchorElementBase* anchor = FirstAnchorElementIncludingSelf(target.ToNode());
    if (!anchor) {
        return;
    }
    ...
    if (event_type == event_type_names::kPointerover) {
        hover_event_candidates_.insert(url,
            HoverEventCandidate { .is_mouse = pointer_event.pointerType() == pointer_type_names::kMouse,
...
void AnchorElementInteractionTracker::OnClickEvent(HTMLAnchorElementBase& anchor, const MouseEvent& click_event)
{
    if (auto* sender = AnchorElementMetricsSender::GetForFrame(GetDocument()->GetFrame())) {
    ...
    const char* orientation_pattern = screen->width() <= screen_height ? ".Portrait" : ".Landscape";
    double click_y = click_event.screenY();
    int normalized_click_y = base::ClampRound(100.0f * (click_y / screen_height));
    base::UmaHistogramPercentage(base::StrCat({ "Blink.AnchorElementInteractionTracker.ClickLocationY", orientation_pattern }), normalized_click_y);
    ...
    if (last_pointer_down_locations_[1]) {
        ...
        int shifted_normalized_click_distance = base::ClampRound(50.0f * (1 + click_distance / screen_height));
        base::UmaHistogramPercentage(base::StrCat({ "Blink.AnchorElementInteractionTracker"
    ...
    for (const auto& hover_event_candidate : hover_event_candidates_) {
        ...
        if (now >= hover_event_candidate.value.timestamp) {
            ...
            if (hover_event_candidate.value.is_mouse) {
                if (auto* sender = AnchorElementMetricsSender::GetForFrame(GetDocument()->GetFrame())) {
                    sender->MaybeReportAnchorElementPointerDataOnHoverTimerFired(hover_event_candidate.value.anchor_id, pointer_data->Clone());
                }
            }
        ...
        next_fire_time = std::min(next_fire_time, hover_event_candidate.value.timestamp);
    }
...
void AnchorElementInteractionTracker::SetTaskRunnerForTesting(scoped_refptr<base::SingleThreadTaskRunner> task_runner, const base::TickClock* clock)
{
...
HTMLAnchorElementBase* AnchorElementInteractionTracker::FirstAnchorElementIncludingSelf(Node* node)
{
...
KURL AnchorElementInteractionTracker::GetHrefEligibleForPreloading(const HTMLAnchorElementBase& anchor)
{
...
void AnchorElementInteractionTracker::AnchorPositionsUpdated(HeapVector<Member<AnchorPositionUpdate>>& position_updates)
{
    CHECK(base::FeatureList::IsEnabled(blink::features::kPreloadingViewportHeuristics));
    ...
    for (AnchorPositionUpdate* anchor_position : position_updates) {
        ...
        if (anchor_position->distance_from_pointer_down.has_value()
            && (anchor_position->distance_from_pointer_down < low || anchor_position->distance_from_pointer_down > high)) {
            continue;
        }

        if (!largest_anchors[0] || anchor_position->size_in_viewport > largest_anchors[0]->size_in_viewport) {
            largest_anchors[1] = largest_anchors[0];
            largest_anchors[0] = anchor_position;
        } else if (!largest_anchors[1] || anchor_position->size_in_viewport > largest_anchors[1]->size_in_viewport) {
    ...
    if (largest_anchors[1]) {
        const double size_difference = largest_anchors[0]->size_in_viewport - largest_anchors[1]->size_in_viewport;
        // If the largest two anchors are similar in size, we don't preload
        // anything.
        if (size_difference / largest_anchors[1]->size_in_viewport < config.largest_anchor_threshold) {
            return;
        }
    }
...
void AnchorElementInteractionTracker::ViewportHeuristicTimerFired(TimerBase* timer)
{
    CHECK(base::FeatureList::IsEnabled(blink::features::kPreloadingViewportHeuristics));
    ...
    interaction_host_->OnViewportHeuristicTriggered(largest_anchor_element_in_viewport_->Url());
}
