...
void HttpEquiv::Process(
    Document& document, const AtomicString& equiv, const AtomicString& content, bool in_document_head_element, bool is_sync_parser, Element* element)
{
    DCHECK(!equiv.IsNull());
    ...
    if (EqualIgnoringASCIICase(equiv, "default-style")) {
        ...
        document.AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kSecurity, mojom::ConsoleMessageLevel::kError,
        ...
        HTMLMetaElement::ProcessMetaCH(document, content, network::MetaCHType::HttpEquivAcceptCH,
            /*is_doc_preloader=*/false, is_sync_parser);
    } else if (EqualIgnoringASCIICase(equiv, http_names::kDelegateCH)) {
        HTMLMetaElement::ProcessMetaCH(document, content, network::MetaCHType::HttpEquivDelegateCH,
            /*is_doc_preloader=*/false, is_sync_parser);
    } else if (EqualIgnoringASCIICase(equiv, "content-security-policy") || EqualIgnoringASCIICase(equiv, "content-security-policy-report-only")) {
        if (in_document_head_element) {
            ProcessHttpEquivContentSecurityPolicy(document.domWindow(), equiv, content);
...
void HttpEquiv::ProcessHttpEquivContentSecurityPolicy(LocalDOMWindow* window, const AtomicString& equiv, const AtomicString& content)
{
    ...
    if (EqualIgnoringASCIICase(equiv, "content-security-policy")) {
        Vector<network::mojom::blink::ContentSecurityPolicyPtr> parsed
            = ParseContentSecurityPolicies(content, network::mojom::blink::ContentSecurityPolicyType::kEnforce,
                network::mojom::blink::ContentSecurityPolicySource::kMeta, *(window->GetSecurityOrigin()));
...
void HttpEquiv::ProcessHttpEquivOriginTrial(LocalDOMWindow* window, const AtomicString& content)
{
    if (!window)
        return;
    ...
    Vector<String> candidate_scripts = GetScriptUrlsFromCurrentStack(window->GetIsolate(), /*unique_url_count=*/3);
    ...
    if (external_origins.size() > 0) {
        window->GetOriginTrialContext()->AddTokenFromExternalScript(content, external_origins);
        return;
    }
...
void HttpEquiv::ProcessHttpEquivRefresh(LocalDOMWindow* window, const AtomicString& content, Element* element)
{
    if (!window)
        return;
    UseCounter::Count(window, WebFeature::kMetaRefresh);
    if (!window->GetContentSecurityPolicy()->AllowInline(ContentSecurityPolicy::InlineType::kScript, element, "" /* content */, "" /* nonce */, NullURL(),
            OrdinalNumber::First(), ReportingDisposition::kSuppressReporting)) {
        UseCounter::Count(window, WebFeature::kMetaRefreshWhenCSPBlocksInlineScript);
    }

    window->document()->MaybeHandleHttpRefresh(content, Document::kHttpRefreshFromMetaTag);
}

void HttpEquiv::ProcessHttpEquivSetCookie(Document& document, const AtomicString& content, Element* element)
{
    document.AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kSecurity, mojom::ConsoleMessageLevel::kError,
        String::Format("Blocked setting the `%s` cookie from a `<meta>` tag.", content.Utf8().c_str())));
}
