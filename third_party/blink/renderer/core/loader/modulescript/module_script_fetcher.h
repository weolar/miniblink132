...
public:
    ...
    public:
        virtual void NotifyFetchFinishedError(const HeapVector<Member<ConsoleMessage>>& error_messages) = 0;
        virtual void NotifyFetchFinishedSuccess(const ModuleScriptCreationParams&) = 0;
    ...
    virtual void Fetch(FetchParameters&, ModuleType, ResourceFetcher*, ModuleGraphLevel, Client*) = 0;

    void Trace(Visitor*) const override;

protected:
    static bool WasModuleLoadSuccessful(ScriptResource* resource, ModuleType expected_module_type, HeapVector<Member<ConsoleMessage>>* error_messages);
};
