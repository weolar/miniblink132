...
class CORE_EXPORT ModuleScriptLoader final : public GarbageCollected<ModuleScriptLoader>, public ModuleScriptFetcher::Client {
    enum class State {
        kInitial,
...
public:
    ModuleScriptLoader(Modulator*, const ScriptFetchOptions&, ModuleScriptLoaderRegistry*, ModuleScriptLoaderClient*);
    ModuleScriptLoader(const ModuleScriptLoader&) = delete;
    ModuleScriptLoader& operator=(const ModuleScriptLoader&) = delete;
    ~ModuleScriptLoader();

    static void Fetch(const ModuleScriptFetchRequest&, ResourceFetcher* fetch_client_settings_object_fetcher, ModuleGraphLevel,
        Modulator* module_map_settings_object, ModuleScriptCustomFetchType, ModuleScriptLoaderRegistry*, ModuleScriptLoaderClient*);

    // Implements ModuleScriptFetcher::Client.
    void NotifyFetchFinishedSuccess(const ModuleScriptCreationParams&) override;
    void NotifyFetchFinishedError(const HeapVector<Member<ConsoleMessage>>& error_messages) override;
...
private:
    void FetchInternal(const ModuleScriptFetchRequest&, ResourceFetcher* fetch_client_settings_object_fetcher, ModuleGraphLevel, ModuleScriptCustomFetchType);
