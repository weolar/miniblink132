...
struct ModuleScriptFetchTarget {
    ModuleScriptFetchTarget(KURL url, ModuleType module_type, TextPosition position)
        : url(url)
...
ModuleTreeLinker::ModuleTreeLinker(ResourceFetcher* fetch_client_settings_object_fetcher, mojom::blink::RequestContextType context_type,
    network::mojom::RequestDestination destination, Modulator* modulator, ModuleScriptCustomFetchType custom_fetch_type, ModuleTreeLinkerRegistry* registry,
    ModuleTreeClient* client, base::PassKey<ModuleTreeLinkerRegistry>)
...
{
#if DCHECK_IS_ON()
    RESOURCE_LOADING_DVLOG(1) << *this << "::advanceState(" << StateToString(state_) << " -> " << StateToString(new_state) << ")";
#endif
    ...
    case State::kFetchingSelf:
        CHECK_EQ(num_incomplete_fetches_, 0u);
        CHECK(new_state == State::kFetchingDependencies || new_state == State::kFinished);
        break;
    case State::kFetchingDependencies:
        CHECK(new_state == State::kInstantiating || new_state == State::kFinished);
        break;
    case State::kInstantiating:
...
#if DCHECK_IS_ON()
        if (result_) {
            RESOURCE_LOADING_DVLOG(1) << *this << " finished with final result " << *result_;
        } else {
...
void ModuleTreeLinker::FetchRoot(
    const KURL& original_url, ModuleType module_type, const ScriptFetchOptions& options, base::PassKey<ModuleTreeLinkerRegistry>, String referrer)
{
#if DCHECK_IS_ON()
    ...
    modulator_->SetAcquiringImportMapsState(Modulator::AcquiringImportMapsState::kAfterModuleScriptLoad);
    ...
    if (!url.IsValid()) {
        result_ = nullptr;
        modulator_->TaskRunner()->PostTask(FROM_HERE, WTF::BindOnce(&ModuleTreeLinker::AdvanceState, WrapPersistent(this), State::kFinished));
        return;
    }
    ...
    // case.
    ModuleScriptFetchRequest request(url, module_type, context_type_, destination_, options, referrer, TextPosition::MinimumPosition());
    ++num_incomplete_fetches_;

    // <spec label="fetch-a-module-script-tree" step="2">Fetch a single module
    // script given...
    // </spec>
    modulator_->FetchSingle(request, fetch_client_settings_object_fetcher_.Get(), ModuleGraphLevel::kTopLevelModuleFetch, custom_fetch_type_, this);
}

// <specdef
// href="https://html.spec.whatwg.org/C/#fetch-an-inline-module-script-graph">
void ModuleTreeLinker::FetchRootInline(ModuleScript* module_script, base::PassKey<ModuleTreeLinkerRegistry>)
{
    ...
    modulator_->TaskRunner()->PostTask(FROM_HERE, WTF::BindOnce(&ModuleTreeLinker::FetchDescendants, WrapPersistent(this), WrapPersistent(module_script)));
}
...
#if DCHECK_IS_ON()
    if (module_script) {
        RESOURCE_LOADING_DVLOG(1) << *this << "::NotifyModuleLoadFinished() with " << *module_script;
    } else {
        RESOURCE_LOADING_DVLOG(1) << *this << "::NotifyModuleLoadFinished() with nullptr.";
    }
#endif
...
void ModuleTreeLinker::FetchDescendants(const ModuleScript* module_script)
{
    ...
    Vector<ModuleRequest> record_requested_modules = ModuleRecord::ModuleRequests(script_state, record);
    ...
    for (const auto& module_request : module_requests) {
        ...
        ScriptFetchOptions options(module_script->FetchOptions().Nonce(), modulator_->GetIntegrityMetadata(module_request.url),
            modulator_->GetIntegrityMetadataString(module_request.url), module_script->FetchOptions().ParserState(),
            module_script->FetchOptions().CredentialsMode(), module_script->FetchOptions().GetReferrerPolicy(), mojom::blink::FetchPriorityHint::kAuto,
        ...
        ModuleScriptFetchRequest request(module_request.url, module_request.module_type, context_type_, destination_, options,
            module_script->BaseUrl().GetString(), module_request.position);

        // <spec label="IMSGF" step="1">Assert: visited set contains url.</spec>
        DCHECK(visited_set_.Contains(std::make_pair(request.Url(), request.GetExpectedModuleType())));
        ...
        // ...</spec>
        modulator_->FetchSingle(request, fetch_client_settings_object_fetcher_.Get(), ModuleGraphLevel::kDependentModuleFetch, custom_fetch_type_, this);
    }
...
void ModuleTreeLinker::Instantiate()
{
    ...
    if (!found_parse_error_) {
        ...
        ScriptState* script_state = modulator_->GetScriptState();
        UseCounter::Count(ExecutionContext::From(script_state), WebFeature::kInstantiateModuleScript);

        ScriptState::Scope scope(script_state);
        ScriptValue instantiation_error = ModuleRecord::Instantiate(script_state, record, result_->SourceUrl());
...
ScriptValue ModuleTreeLinker::FindFirstParseError(const ModuleScript* module_script, HeapHashSet<Member<const ModuleScript>>* discovered_set) const
{
    ...
    Vector<ModuleRequest> child_specifiers = ModuleRecord::ModuleRequests(modulator_->GetScriptState(), record);
    ...
    for (const auto& module_request : child_specifiers) {
        ...
        KURL child_url = module_script->ResolveModuleSpecifier(module_request.specifier);
        ModuleType child_module_type = modulator_->ModuleTypeFromRequest(module_request);

        // <spec step="5.2">... (None of these will ever fail, as otherwise
        // moduleScript would have been marked as itself having a parse
        // error.)</spec>
        CHECK(child_url.IsValid()) << "ModuleScript::ResolveModuleSpecifier() impl must "
                                      "return a valid url.";
        ...
        const ModuleScript* child_module = modulator_->GetFetchedModuleScript(child_url, child_module_type);
        ...
        ScriptValue child_parse_error = FindFirstParseError(child_module, discovered_set);
...
{
    stream << "ModuleTreeLinker[" << &linker << ", original_url=" << linker.original_url_.GetString() << ", url=" << linker.url_.GetString()
           << ", module_type=" << linker.module_type_ << ", inline=" << linker.root_is_inline_ << "]";
    return stream;
}
#endif
