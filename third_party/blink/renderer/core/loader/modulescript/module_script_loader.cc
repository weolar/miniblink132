...
ModuleScriptLoader::ModuleScriptLoader(
    Modulator* modulator, const ScriptFetchOptions& options, ModuleScriptLoaderRegistry* registry, ModuleScriptLoaderClient* client)
...
#if DCHECK_IS_ON()
    RESOURCE_LOADING_DVLOG(1) << "ModuleLoader[" << url_.GetString() << "]::advanceState(" << StateToString(state_) << " -> " << StateToString(new_state)
                              << ")";
#endif
    state_ = new_state;
...
void ModuleScriptLoader::Fetch(const ModuleScriptFetchRequest& module_request, ResourceFetcher* fetch_client_settings_object_fetcher, ModuleGraphLevel level,
    Modulator* module_map_settings_object, ModuleScriptCustomFetchType custom_fetch_type, ModuleScriptLoaderRegistry* registry,
    ModuleScriptLoaderClient* client)
{
    ModuleScriptLoader* loader = MakeGarbageCollected<ModuleScriptLoader>(module_map_settings_object, module_request.Options(), registry, client);
    registry->AddLoader(loader);
    loader->FetchInternal(module_request, fetch_client_settings_object_fetcher, level, custom_fetch_type);
}

// <specdef
// href="https://html.spec.whatwg.org/C/#fetch-destination-from-module-type">
void SetFetchDestinationFromModuleType(ResourceRequest& resource_request, const ModuleScriptFetchRequest& module_request)
{
    if (!base::FeatureList::IsEnabled(features::kFetchDestinationJsonCssModules)) {
    ...
    case ModuleType::kCSS:
        resource_request.SetRequestContext(mojom::blink::RequestContextType::STYLE);
        resource_request.SetRequestDestination(network::mojom::RequestDestination::kStyle);
        break;
    case ModuleType::kJSON:
        resource_request.SetRequestContext(mojom::blink::RequestContextType::JSON);
        resource_request.SetRequestDestination(network::mojom::RequestDestination::kJson);
        break;
...
void ModuleScriptLoader::FetchInternal(const ModuleScriptFetchRequest& module_request, ResourceFetcher* fetch_client_settings_object_fetcher,
    ModuleGraphLevel level, ModuleScriptCustomFetchType custom_fetch_type)
{
    const FetchClientSettingsObject& fetch_client_settings_object = fetch_client_settings_object_fetcher->GetProperties().GetFetchClientSettingsObject();
    ...
    fetch_params.MutableResourceRequest().SetFetchIntegrity(options_.GetIntegrityAttributeValue());
    ...
    // policy.</spec>
    fetch_params.MutableResourceRequest().SetReferrerPolicy(module_request.Options().GetReferrerPolicy());

    // <spec step="5">... mode is "cors", ...</spec>
    //
    // <spec label="SMSR">... its credentials mode to options's credentials mode,
    // ...</spec>
    fetch_params.SetCrossOriginAccessControl(fetch_client_settings_object.GetSecurityOrigin(), options_.CredentialsMode());
    ...
    if ((module_request.Destination() == network::mojom::RequestDestination::kWorker
            || module_request.Destination() == network::mojom::RequestDestination::kSharedWorker
            || module_request.Destination() == network::mojom::RequestDestination::kServiceWorker)
        && level == ModuleGraphLevel::kTopLevelModuleFetch) {
        // This should be done after SetCrossOriginAccessControl() that sets the
        // mode to kCors.
        fetch_params.MutableResourceRequest().SetMode(network::mojom::RequestMode::kSameOrigin);
    }

    // <spec step="5">... referrer is referrer, ...</spec>
    fetch_params.MutableResourceRequest().SetReferrerString(module_request.ReferrerString());

    // https://wicg.github.io/priority-hints/#script :
    // Step 10. "... request’s priority to option’s fetchpriority ..."
    fetch_params.MutableResourceRequest().SetFetchPriorityHint(options_.FetchPriorityHint());
    ...
    fetch_params.SetRenderBlockingBehavior(module_request.Options().GetRenderBlockingBehavior());

    // <spec step="12.1">Let source text be the result of UTF-8 decoding
    // response's body.</spec>
    fetch_params.SetDecoderOptions(TextResourceDecoderOptions::CreateUTF8Decode());
    ...
    // response.</spec>
    module_fetcher_ = modulator_->CreateModuleScriptFetcher(custom_fetch_type, PassKey());
    module_fetcher_->Fetch(fetch_params, module_request.GetExpectedModuleType(), fetch_client_settings_object_fetcher, level, this);
}

// <specdef href="https://html.spec.whatwg.org/C/#fetch-a-single-module-script">
void ModuleScriptLoader::NotifyFetchFinishedError(const HeapVector<Member<ConsoleMessage>>& error_messages)
{
...
// https://html.spec.whatwg.org/C#fetch-a-single-module-script
void ModuleScriptLoader::NotifyFetchFinishedSuccess(const ModuleScriptCreationParams& params)
{
    // [nospec] Abort the steps if the browsing context is discarded.
    if (!modulator_->HasValidContext()) {
        AdvanceState(State::kFinished);
    ...
    if (params.ResponseReferrerPolicy() != network::mojom::ReferrerPolicy::kDefault) {
        options_.UpdateReferrerPolicyAfterResponseReceived(params.ResponseReferrerPolicy());
    }

    switch (params.GetModuleType()) {
    case ModuleType::kJSON:
        module_script_ = ValueWrapperSyntheticModuleScript::CreateJSONWrapperSyntheticModuleScript(params, modulator_);
        break;
    case ModuleType::kCSS:
        module_script_ = ValueWrapperSyntheticModuleScript::CreateCSSWrapperSyntheticModuleScript(params, modulator_);
        break;
    case ModuleType::kJavaScript:
