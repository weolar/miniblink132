...
public:
    ...
    ModuleTreeLinker(ResourceFetcher* fetch_client_settings_object_fetcher, mojom::blink::RequestContextType context_type,
        network::mojom::RequestDestination destination, Modulator*, ModuleScriptCustomFetchType, ModuleTreeLinkerRegistry*, ModuleTreeClient*,
        base::PassKey<ModuleTreeLinkerRegistry>);
    void FetchRoot(const KURL&, ModuleType, const ScriptFetchOptions&, base::PassKey<ModuleTreeLinkerRegistry>, String referrer);
...
private:
    ...
    // [FFPE]
    ScriptValue FindFirstParseError(const ModuleScript*, HeapHashSet<Member<const ModuleScript>>*) const;
    ...
    friend CORE_EXPORT std::ostream& operator<<(std::ostream&, const ModuleTreeLinker&);
#endif
};
