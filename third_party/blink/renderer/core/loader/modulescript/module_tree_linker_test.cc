...
public:
    explicit SimModuleRequest(KURL url)
        : SimRequestBase(std::move(url), "text/javascript",
...
public:
    ...
    bool HasInstantiated(ModuleScript* module_script)
    {
        ...
        return (module_script->V8Module()->GetStatus() == v8::Module::kInstantiated);
    }
};
...
TEST_F(ModuleTreeLinkerTest, FetchTreeNoDeps)
{
    ...
    GetModulator()->FetchTree(sim_module.GetURL(), ModuleType::kJavaScript, GetDocument().Fetcher(), mojom::blink::RequestContextType::SCRIPT,
        network::mojom::RequestDestination::kScript, ScriptFetchOptions(), ModuleScriptCustomFetchType::kNone, client);

    EXPECT_FALSE(client->WasNotifyFinished()) << "ModuleTreeLinker should always finish asynchronously.";
...
TEST_F(ModuleTreeLinkerTest, FetchTreeInstantiationFailure)
{
    ...
    EXPECT_TRUE(client->GetModuleScript()->HasErrorToRethrow()) << "Expected errored module script but got " << *client->GetModuleScript();
}
