...
public:
    ...
    ModuleScriptFetchRequest(const KURL& url, ModuleType module_type, mojom::blink::RequestContextType context_type,
        network::mojom::RequestDestination destination, const ScriptFetchOptions& options, const String& referrer_string, const TextPosition& referrer_position)
        : url_(url)
    ...
    static ModuleScriptFetchRequest CreateForTest(const KURL& url, ModuleType module_type)
    {
        return ModuleScriptFetchRequest(url, module_type, mojom::blink::RequestContextType::SCRIPT, network::mojom::RequestDestination::kScript,
            ScriptFetchOptions(), Referrer::ClientReferrerString(), TextPosition::MinimumPosition());
    }
    ...
    ModuleType GetExpectedModuleType() const
    {
        return expected_module_type_;
    }
    mojom::blink::RequestContextType ContextType() const
    {
    ...
    const ScriptFetchOptions& Options() const
    {
        return options_;
    }
    ...
    const TextPosition& GetReferrerPosition() const
    {
        return referrer_position_;
    }
