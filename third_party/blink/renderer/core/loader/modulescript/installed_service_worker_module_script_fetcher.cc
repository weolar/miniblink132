...
InstalledServiceWorkerModuleScriptFetcher::InstalledServiceWorkerModuleScriptFetcher(
    WorkerGlobalScope* global_scope, base::PassKey<ModuleScriptLoader> pass_key)
...
void InstalledServiceWorkerModuleScriptFetcher::Fetch(
    FetchParameters& fetch_params, ModuleType expected_module_type, ResourceFetcher*, ModuleGraphLevel level, ModuleScriptFetcher::Client* client)
{
    ...
    std::unique_ptr<InstalledScriptsManager::ScriptData> script_data = installed_scripts_manager->GetScriptData(fetch_params.Url());

    if (!script_data) {
        HeapVector<Member<ConsoleMessage>> error_messages;
        error_messages.push_back(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kJavaScript, mojom::ConsoleMessageLevel::kError,
            "Failed to load the script unexpectedly", fetch_params.Url().GetString(), nullptr, 0));
    ...
    if (level == ModuleGraphLevel::kTopLevelModuleFetch) {
        ...
        if (!script_data->GetReferrerPolicy().IsNull()) {
            SecurityPolicy::ReferrerPolicyFromHeaderValue(
                script_data->GetReferrerPolicy(), kDoNotSupportReferrerPolicyLegacyKeywords, &response_referrer_policy);
        }

        global_scope_->Initialize(response_url, response_referrer_policy,
            ParseContentSecurityPolicyHeaders(script_data->GetContentSecurityPolicyResponseHeaders()), script_data->CreateOriginTrialTokens().get());
    }

    // TODO(sasebree) De-duplicate similar logic that lives in
    // ModuleScriptFetcher::WasModuleLoadSuccessful
    if (expected_module_type_ != ModuleType::kJavaScript || !MIMETypeRegistry::IsSupportedJavaScriptMIMEType(script_data->GetHttpContentType())) {
    ...
    client->NotifyFetchFinishedSuccess(ModuleScriptCreationParams(
        /*source_url=*/fetch_params.Url(), /*base_url=*/fetch_params.Url(), ScriptSourceLocationType::kExternalFile, expected_module_type_,
