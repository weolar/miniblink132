...
class MockThreadableLoaderClient final : public GarbageCollected<MockThreadableLoaderClient>, public ThreadableLoaderClient {
public:
...
void SetUpSuccessURL()
{
    ...
    url_test_helpers::RegisterMockedURLLoad(SuccessURL(), test::CoreTestDataPath(kFileName), "text/html");
}
...
void SetUpRedirectURL()
{
    ...
    network::mojom::LoadTimingInfoPtr timing = network::mojom::LoadTimingInfo::New();
    ...
    // TODO(crbug.com/751425): We should use the mock functionality
    // via |dummy_page_holder_|.
    url_test_helpers::RegisterMockedURLLoadWithCustomResponse(url, test::CoreTestDataPath(kFileName), response);
}
...
public:
        ...
        : dummy_page_holder_(std::make_unique<DummyPageHolder>(gfx::Size(1, 1)))
    {
        ...
        dummy_page_holder_->GetFrame().Loader().CommitNavigation(WebNavigationParams::CreateWithEmptyHTMLForTesting(url), nullptr /* extra_data */);
    ...
    void CreateLoader(ThreadableLoaderClient* client)
    {
        ...
        loader_ = MakeGarbageCollected<ThreadableLoader>(*dummy_page_holder_->GetFrame().DomWindow(), client, resource_loader_options);
    }
...
public:
    ...
    void StartLoader(const KURL& url, network::mojom::RequestMode request_mode = network::mojom::RequestMode::kNoCors)
    {
    ...
    void CancelLoader()
    {
        helper_->CancelLoader();
    }
    void CancelAndClearLoader()
    {
        helper_->CancelAndClearLoader();
    }
    void ClearLoader()
    {
        helper_->ClearLoader();
    ...
    Checkpoint& GetCheckpoint()
    {
        return helper_->GetCheckpoint();
    }
    void CallCheckpoint(int n)
    {
        helper_->CallCheckpoint(n);
    }
    ...
    void CreateLoader()
    {
        helper_->CreateLoader(Client());
    }

    MockThreadableLoaderClient* Client() const
    {
        return client_.Get();
    }
...
TEST_F(ThreadableLoaderTest, CancelAfterStart)
{
    ...
    EXPECT_CALL(GetCheckpoint(), Call(2)).WillOnce(InvokeWithoutArgs(this, &ThreadableLoaderTest::CancelLoader));
...
TEST_F(ThreadableLoaderTest, CancelAndClearAfterStart)
{
    ...
    EXPECT_CALL(GetCheckpoint(), Call(2)).WillOnce(InvokeWithoutArgs(this, &ThreadableLoaderTest::CancelAndClearLoader));
    EXPECT_CALL(*Client(), DidFail(_, Truly(IsCancellation)));
    EXPECT_CALL(GetCheckpoint(), Call(3));
...
TEST_F(ThreadableLoaderTest, CancelInDidReceiveResponse)
{
    ...
    EXPECT_CALL(*Client(), DidReceiveResponse(_, _)).WillOnce(InvokeWithoutArgs(this, &ThreadableLoaderTest::CancelLoader));
    EXPECT_CALL(*Client(), DidFail(_, Truly(IsCancellation)));
...
TEST_F(ThreadableLoaderTest, CancelAndClearInDidReceiveResponse)
{
    ...
    EXPECT_CALL(GetCheckpoint(), Call(2));
    EXPECT_CALL(*Client(), DidReceiveResponse(_, _)).WillOnce(InvokeWithoutArgs(this, &ThreadableLoaderTest::CancelAndClearLoader));
    EXPECT_CALL(*Client(), DidFail(_, Truly(IsCancellation)));
...
TEST_F(ThreadableLoaderTest, CancelInDidReceiveData)
{
    ...
    EXPECT_CALL(*Client(), DidReceiveData(_)).WillOnce(InvokeWithoutArgs(this, &ThreadableLoaderTest::CancelLoader));
    EXPECT_CALL(*Client(), DidFail(_, Truly(IsCancellation)));
...
TEST_F(ThreadableLoaderTest, CancelAndClearInDidReceiveData)
{
    ...
    EXPECT_CALL(GetCheckpoint(), Call(2));
    EXPECT_CALL(*Client(), DidReceiveResponse(_, _));
    EXPECT_CALL(*Client(), DidReceiveData(_)).WillOnce(InvokeWithoutArgs(this, &ThreadableLoaderTest::CancelAndClearLoader));
    EXPECT_CALL(*Client(), DidFail(_, Truly(IsCancellation)));
...
TEST_F(ThreadableLoaderTest, CancelInDidFinishLoading)
{
    ...
    EXPECT_CALL(*Client(), DidFinishLoading(_)).WillOnce(InvokeWithoutArgs(this, &ThreadableLoaderTest::CancelLoader));
...
TEST_F(ThreadableLoaderTest, ClearInDidFinishLoading)
{
    ...
    EXPECT_CALL(*Client(), DidReceiveResponse(_, _));
    EXPECT_CALL(*Client(), DidReceiveData(_));
    EXPECT_CALL(*Client(), DidFinishLoading(_)).WillOnce(InvokeWithoutArgs(this, &ThreadableLoaderTest::ClearLoader));
...
TEST_F(ThreadableLoaderTest, CancelInDidFail)
{
    ...
    EXPECT_CALL(GetCheckpoint(), Call(2));
    EXPECT_CALL(*Client(), DidFail(_, _)).WillOnce(InvokeWithoutArgs(this, &ThreadableLoaderTest::CancelLoader));
...
TEST_F(ThreadableLoaderTest, ClearInDidFail)
{
    ...
    EXPECT_CALL(GetCheckpoint(), Call(2));
    EXPECT_CALL(*Client(), DidFail(_, _)).WillOnce(InvokeWithoutArgs(this, &ThreadableLoaderTest::ClearLoader));
