...
class MockAnchorElementInteractionHost : public mojom::blink::AnchorElementInteractionHost {
public:
    explicit MockAnchorElementInteractionHost(mojo::PendingReceiver<mojom::blink::AnchorElementInteractionHost> pending_receiver)
    {
...
private:
    ...
    void OnPointerHover(const KURL& target, mojom::blink::AnchorElementPointerDataPtr mouse_data) override
    {
        url_received_ = target;
...
protected:
    void SetUp() override
    {
        ...
        MainFrame().GetFrame()->GetBrowserInterfaceBroker().SetBinderForTesting(
            mojom::blink::AnchorElementInteractionHost::Name_, WTF::BindRepeating(&AnchorElementInteractionTest::Bind, WTF::Unretained(this)));
        WebView().MainFrameViewWidget()->Resize(gfx::Size(400, 400));
    }

    void TearDown() override
    {
        MainFrame().GetFrame()->GetBrowserInterfaceBroker().SetBinderForTesting(mojom::blink::AnchorElementInteractionHost::Name_, {});
        hosts_.clear();
        SimTest::TearDown();
    }
    ...
    void Bind(mojo::ScopedMessagePipeHandle message_pipe_handle)
    {
        auto host = std::make_unique<MockAnchorElementInteractionHost>(
            mojo::PendingReceiver<mojom::blink::AnchorElementInteractionHost>(std::move(message_pipe_handle)));
    ...
    void SendMouseDownEvent()
    {
        ...
        WebMouseEvent event(WebInputEvent::Type::kMouseDown, coordinates, coordinates, WebPointerProperties::Button::kLeft, 0, WebInputEvent::kLeftButtonDown,
...
TEST_F(AnchorElementInteractionTest, RightClick)
{
    ...
    gfx::PointF coordinates(100, 100);
    WebMouseEvent event(WebInputEvent::Type::kMouseDown, coordinates, coordinates, WebPointerProperties::Button::kLeft, 0, WebInputEvent::kRightButtonDown,
        WebInputEvent::GetStaticTimeStampForTests());
    GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(event);
    base::RunLoop().RunUntilIdle();
    EXPECT_EQ(1u, hosts_.size());
...
TEST_F(AnchorElementInteractionTest, TouchEvent)
{
    ...
    WebPointerEvent event(WebInputEvent::Type::kPointerDown,
        WebPointerProperties(1, WebPointerProperties::PointerType::kTouch, WebPointerProperties::Button::kLeft, gfx::PointF(100, 100), gfx::PointF(100, 100)),
        1, 1);
    GetDocument().GetFrame()->GetEventHandler().HandlePointerEvent(event, Vector<WebPointerEvent>(), Vector<WebPointerEvent>());
...
TEST_F(AnchorElementInteractionTest, ValidMouseHover)
{
    ...
    task_runner->AdvanceTimeAndRun(AnchorElementInteractionTracker::GetHoverDwellTime());
    base::RunLoop().RunUntilIdle();
...
TEST_F(AnchorElementInteractionTest, ShortMouseHover)
{
    ...
    // Wait for hover logic to process the event
    task_runner->AdvanceTimeAndRun(0.5 * AnchorElementInteractionTracker::GetHoverDwellTime());
    base::RunLoop().RunUntilIdle();
...
TEST_F(AnchorElementInteractionTest, MousePointerEnterAndLeave)
{
    ...
    gfx::PointF coordinates(100, 100);
    WebMouseEvent mouse_enter_event(WebInputEvent::Type::kMouseMove, coordinates, coordinates, WebPointerProperties::Button::kNoButton, 0,
        WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests());
    GetDocument().GetFrame()->GetEventHandler().HandleMouseMoveEvent(mouse_enter_event, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());

    task_runner->AdvanceTimeAndRun(0.5 * AnchorElementInteractionTracker::GetHoverDwellTime());

    WebMouseEvent mouse_leave_event(WebInputEvent::Type::kMouseLeave, coordinates, coordinates, WebPointerProperties::Button::kNoButton, 0,
        WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests());
...
{
    auto task_runner = base::MakeRefCounted<scheduler::FakeTaskRunner>();
    auto* motion_estimator = MakeGarbageCollected<AnchorElementInteractionTracker::MouseMotionEstimator>(task_runner);
    motion_estimator->SetTaskRunnerForTesting(task_runner, task_runner->GetMockTickClock());
    ...
    for (double dt : { 0.0, 1.0, 5.0, 15.0, 30.0, 7.0, 200.0, 50.0, 100.0, 27.0 }) {
        ...
        motion_estimator->OnMouseMoveEvent(gfx::PointF { static_cast<float>(x), static_cast<float>(y) });
        if (i == 1) {
    ...
    EXPECT_NEAR(std::sqrt(2.0), motion_estimator->GetMouseVelocity().Length(), 1e-6);
    EXPECT_NEAR(-std::sqrt(2.0), motion_estimator->GetMouseTangentialAcceleration(), 1e-6);
}

TEST_F(AnchorElementInteractionTest, MouseMotionEstimatorWithVariableAcceleration)
{
    auto task_runner = base::MakeRefCounted<scheduler::FakeTaskRunner>();
    auto* motion_estimator = MakeGarbageCollected<AnchorElementInteractionTracker::MouseMotionEstimator>(task_runner);
    motion_estimator->SetTaskRunnerForTesting(task_runner, task_runner->GetMockTickClock());
...
TEST_F(AnchorElementInteractionTest, MouseVelocitySent)
{
    ...
    for (base::TimeDelta t; t <= AnchorElementInteractionTracker::GetHoverDwellTime(); t += timestep) {
...
class AnchorElementInteractionViewportHeuristicsTest : public AnchorElementInteractionTest {
public:
    AnchorElementInteractionViewportHeuristicsTest()
    {
        feature_list_.InitWithFeaturesAndParameters(
            { { features::kNavigationPredictor, { { "random_anchor_sampling_period", "1" } } }, { features::kNavigationPredictorNewViewportFeatures, {} },
                { features::kPreloadingViewportHeuristics,
                    { { "delay", "100ms" }, { "distance_from_ptr_down_low", "-0.3" }, { "distance_from_ptr_down_hi", "0" },
    ...
    void RunBasicTestFixture(const TestFixtureParams& params)
    {
        ...
        Compositor().BeginFrame();
        DispatchPointerDownAndVerticalScroll(params.pointer_down_location, params.scroll_delta);
...
TEST_F(AnchorElementInteractionViewportHeuristicsTest, BasicTest)
{
    ...
    RunBasicTestFixture({ .main_resource_body = body, .pointer_down_location = gfx::PointF(100, 180), .scroll_delta = -100 });
...
TEST_F(AnchorElementInteractionViewportHeuristicsTest, NotNearPreviousPointerDown)
{
    String body = R"HTML(
    <body style="margin: 0px">
      <div style="height: 200px"></div>
    ...
    RunBasicTestFixture({ .main_resource_body = body, .pointer_down_location = gfx::PointF(100, 350), .scroll_delta = -100 });
...
// other.
TEST_F(AnchorElementInteractionViewportHeuristicsTest, TwoSimilarlySizedAnchors)
{
    String body = R"HTML(
    <body style="margin: 0px">
    ...
    RunBasicTestFixture({ .main_resource_body = body, .pointer_down_location = gfx::PointF(100, 200), .scroll_delta = -25 });
...
// Test scenario with two anchors where one anchor is > 50% larger than the
// other.
TEST_F(AnchorElementInteractionViewportHeuristicsTest, TwoDifferentlySizedAnchors)
{
    String body = R"HTML(
    <body style="margin: 0px">
      <div style="height: 100px"></div>
    ...
    RunBasicTestFixture({ .main_resource_body = body, .pointer_down_location = gfx::PointF(100, 150), .scroll_delta = -25 });
...
TEST_F(AnchorElementInteractionViewportHeuristicsTest, MultipleAnchors)
{
    ...
    RunBasicTestFixture({ .main_resource_body = body, .pointer_down_location = gfx::PointF(100, 220), .scroll_delta = -25 });
...
TEST_F(AnchorElementInteractionViewportHeuristicsTest, PointerDownImmediatelyAfterScroll)
{
...
TEST_F(AnchorElementInteractionViewportHeuristicsTest, PredictorDisabledIfAllAnchorsNotSampledIn)
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeatureWithParameters(features::kNavigationPredictor, { { "random_anchor_sampling_period", "2" } });
