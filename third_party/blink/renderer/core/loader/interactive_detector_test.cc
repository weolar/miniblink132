...
class NetworkActivityCheckerForTest : public InteractiveDetector::NetworkActivityChecker {
public:
...
class InteractiveDetectorTest : public testing::Test, public ScopedMockOverlayScrollbars {
public:
    InteractiveDetectorTest()
    {
        ...
        dummy_page_holder_ = std::make_unique<DummyPageHolder>(gfx::Size(), nullptr, nullptr, base::NullCallback(), tick_clock);

        Document* document = &dummy_page_holder_->GetDocument();
        detector_ = MakeGarbageCollected<InteractiveDetector>(*document, std::make_unique<NetworkActivityCheckerForTest>(document));
...
protected:
    InteractiveDetector* GetDetector()
    {
        return detector_;
    }

    base::TimeTicks GetDummyTaskEndTime()
    {
    ...
    NetworkActivityCheckerForTest* GetNetworkActivityChecker()
    {
        ...
        return static_cast<NetworkActivityCheckerForTest*>(detector_->network_activity_checker_.get());
    }
    ...
    void SimulateFCPDetected(base::TimeTicks fcp_time, base::TimeTicks detection_time)
    {
    ...
    void RunTillTimestamp(base::TimeTicks target_time)
    {
        ...
        platform_->RunForPeriod(std::max(base::TimeDelta(), target_time - current_time));
    }
    ...
    {
        return detector_->interactive_time_;
    }
    ...
    {
        return dummy_page_holder_->GetDocument().GetTaskRunner(TaskType::kUserInteraction);
    }
...
TEST_F(InteractiveDetectorTest, NetworkBusyBlocksTTIEvenWhenMainThreadQuiet)
{
    ...
    SimulateResourceLoadBegin(t0 + base::Seconds(3.5)); // Request 3 start. Network busy.
    SimulateFCPDetected(
        /* fcp_time */ t0 + base::Seconds(3),
        /* detection_time */ t0 + base::Seconds(4));
    SimulateLongTask(t0 + base::Seconds(7),
...
TEST_F(InteractiveDetectorTest, LongEnoughQuietWindowBetweenFCPAndFcpDetect)
{
    ...
    SimulateResourceLoadBegin(t0 + base::Seconds(8.5)); // Request 3 start. Network busy.
    SimulateFCPDetected(
        /* fcp_time */ t0 + base::Seconds(3),
        /* detection_time */ t0 + base::Seconds(10));
...
TEST_F(InteractiveDetectorTest, TaskLongerThan5sBlocksTTI)
{
    ...
    GetTaskRunner()->PostTask(FROM_HERE, WTF::BindOnce(&InteractiveDetectorTest::DummyTaskWithDuration, WTF::Unretained(this), 6.0));
...
TEST_F(InteractiveDetectorTest, LongTaskAfterTTIDoesNothing)
{
    ...
    // Long task 1.
    GetTaskRunner()->PostTask(FROM_HERE, WTF::BindOnce(&InteractiveDetectorTest::DummyTaskWithDuration, WTF::Unretained(this), 0.1));
...
TEST_F(InteractiveDetectorTest, FirstInputDelayForClickOnMobile)
{
    ...
    Event* pointerdown = MakeGarbageCollected<Event>(
        event_type_names::kPointerdown, MessageEvent::Bubbles::kYes, MessageEvent::Cancelable::kYes, MessageEvent::ComposedMode::kComposed, t0);
    ...
    // Pointerup
    Event* pointerup = MakeGarbageCollected<Event>(event_type_names::kPointerup, MessageEvent::Bubbles::kYes, MessageEvent::Cancelable::kYes,
        MessageEvent::ComposedMode::kComposed, t0 + base::Milliseconds(20));
    pointerup->SetTrusted(true);
    detector->HandleForInputDelay(*pointerup, t0 + base::Milliseconds(20), t0 + base::Milliseconds(50));
    EXPECT_TRUE(detector->GetFirstInputDelay().has_value());
    EXPECT_EQ(detector->GetFirstInputDelay().value(), base::Milliseconds(17));
}

TEST_F(InteractiveDetectorTest, FirstInputDelayForClickOnDesktopWithFixEnabled)
{
    ...
    Event* mousedown = MakeGarbageCollected<Event>(
        event_type_names::kMousedown, MessageEvent::Bubbles::kYes, MessageEvent::Cancelable::kYes, MessageEvent::ComposedMode::kComposed, t0);
