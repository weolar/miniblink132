...
private:
    ...
    public:
        explicit DecodedDataPassthroughClient(Client* client)
            : client_(client)
        {
        }
        ...
        {
            client_->DecodedBodyDataReceived(String(base::as_bytes(data)).UpperASCII(), WebEncodingData { .encoding = "utf-8" }, base::SpanOrSize(data));
        }

        void DecodedBodyDataReceived(const WebString& data, const WebEncodingData& encoding_data, base::SpanOrSize<const char> encoded_data) override
        {
            client_->DecodedBodyDataReceived(data, encoding_data, encoded_data);
        }

        void BodyLoadingFinished(base::TimeTicks completion_time, int64_t total_encoded_data_length, int64_t total_encoded_body_length,
            int64_t total_decoded_body_length, const std::optional<WebURLError>& error) override
        {
            client_->BodyLoadingFinished(completion_time, total_encoded_data_length, total_encoded_body_length, total_decoded_body_length, error);
        }
...
public:
    explicit BodyLoaderTestDelegate(std::unique_ptr<StaticDataNavigationBodyLoader> body_loader)
...
struct TestVisitedLink {
    ...
    friend bool operator<(const TestVisitedLink& lhs, const TestVisitedLink& rhs)
    {
        return std::tie(lhs.link_url, lhs.frame_origin, lhs.top_level_site) < std::tie(rhs.link_url, rhs.frame_origin, rhs.top_level_site);
    }
};
...
public:
    ...
    void AddOrUpdateVisitedLinkSalt(const url::Origin& origin, uint64_t salt) override
    {
        salts_[origin] = salt;
    }
    ...
    uint64_t PartitionedVisitedLinkFingerprint(
        std::string_view canonical_link_url, const net::SchemefulSite& top_level_site, const WebSecurityOrigin& frame_origin) override
    {
        ...
        // Then we check whether our mock-hashtable has an entry for the provided
        // visited link.
        const TestVisitedLink link = { GURL(canonical_link_url), top_level_site, url::Origin(frame_origin) };
    ...
    std::optional<uint64_t> GetVisitedLinkSaltForOrigin(const url::Origin& origin)
    {
    ...
    void AddPartitionedVisitedLinkToMockHashtable(const KURL& link_url, const KURL& top_level_url, const KURL& frame_url)
    {
        ...
        const TestVisitedLink link = { GURL(link_url), net::SchemefulSite(GURL(top_level_url)), url::Origin::Create(GURL(frame_url)) };
    ...
    void AddUnpartitionedVisitedLinkToMockHashtable(const KURL& url)
    {
        ...
        unpartitioned_hashtable_.insert({ url::Origin::Create(GURL(url)), mock_fingerprint });
    }
...
class DocumentLoaderTest : public testing::Test, public ::testing::WithParamInterface<TestMode> {
protected:
    void SetUp() override
    {
        switch (GetParam()) {
        case TestMode::kUnpartitionedStorageAndLinks:
            scoped_feature_list_.InitWithFeatures({},
                { net::features::kThirdPartyStoragePartitioning, blink::features::kPartitionVisitedLinkDatabase,
                    blink::features::kPartitionVisitedLinkDatabaseWithSelfLinks });
            break;
        case TestMode::kUnpartitionedStoragePartitionedNoSelfLinks:
            scoped_feature_list_.InitWithFeatures({ blink::features::kPartitionVisitedLinkDatabase },
                { net::features::kThirdPartyStoragePartitioning, blink::features::kPartitionVisitedLinkDatabaseWithSelfLinks });
            break;
        case TestMode::kUnpartitionedStorageParttionedWithSelfLinks:
            scoped_feature_list_.InitWithFeatures({ blink::features::kPartitionVisitedLinkDatabaseWithSelfLinks },
                { net::features::kThirdPartyStoragePartitioning, blink::features::kPartitionVisitedLinkDatabase });
            break;
        case TestMode::kUnpartitionedStoragePartitionedLinksBothEnabled:
            scoped_feature_list_.InitWithFeatures(
                { blink::features::kPartitionVisitedLinkDatabase, blink::features::kPartitionVisitedLinkDatabaseWithSelfLinks },
                { net::features::kThirdPartyStoragePartitioning });
            break;
        case TestMode::kPartitionedStorageUnpartitionedLinks:
            scoped_feature_list_.InitWithFeatures({ net::features::kThirdPartyStoragePartitioning },
                { blink::features::kPartitionVisitedLinkDatabase, blink::features::kPartitionVisitedLinkDatabaseWithSelfLinks });
            break;
        case TestMode::kPartitionedStorageAndLinksNoSelfLinks:
            scoped_feature_list_.InitWithFeatures({ net::features::kThirdPartyStoragePartitioning, blink::features::kPartitionVisitedLinkDatabase },
                { blink::features::kPartitionVisitedLinkDatabaseWithSelfLinks });
            break;
        ...
        url_test_helpers::RegisterMockedURLLoad(url_test_helpers::ToKURL("http://example.com/foo.html"), test::CoreTestDataPath("foo.html"));
        url_test_helpers::RegisterMockedURLLoad(url_test_helpers::ToKURL("http://user:@example.com/foo.html"), test::CoreTestDataPath("foo.html"));
        url_test_helpers::RegisterMockedURLLoad(url_test_helpers::ToKURL("http://:pass@example.com/foo.html"), test::CoreTestDataPath("foo.html"));
        url_test_helpers::RegisterMockedURLLoad(url_test_helpers::ToKURL("http://user:pass@example.com/foo.html"), test::CoreTestDataPath("foo.html"));
        url_test_helpers::RegisterMockedURLLoad(url_test_helpers::ToKURL("https://example.com/foo.html"), test::CoreTestDataPath("foo.html"));
        url_test_helpers::RegisterMockedURLLoad(url_test_helpers::ToKURL("https://example.com:8000/foo.html"), test::CoreTestDataPath("foo.html"));
        url_test_helpers::RegisterMockedURLLoad(url_test_helpers::ToKURL("http://192.168.1.1/foo.html"), test::CoreTestDataPath("foo.html"),
            WebString::FromUTF8("text/html"), URLLoaderMockFactory::GetSingletonInstance(), network::mojom::IPAddressSpace::kPrivate);
        url_test_helpers::RegisterMockedURLLoad(url_test_helpers::ToKURL("https://192.168.1.1/foo.html"), test::CoreTestDataPath("foo.html"),
            WebString::FromUTF8("text/html"), URLLoaderMockFactory::GetSingletonInstance(), network::mojom::IPAddressSpace::kPrivate);
        url_test_helpers::RegisterMockedURLLoad(url_test_helpers::ToKURL("http://somethinglocal/foo.html"), test::CoreTestDataPath("foo.html"),
            WebString::FromUTF8("text/html"), URLLoaderMockFactory::GetSingletonInstance(), network::mojom::IPAddressSpace::kLocal);
    }
    ...
    {
        return GetParam() == kUnpartitionedStoragePartitionedNoSelfLinks || (GetParam() == kUnpartitionedStorageParttionedWithSelfLinks)
            || (GetParam() == kUnpartitionedStoragePartitionedLinksBothEnabled) || (GetParam() == kPartitionedStorageAndLinksNoSelfLinks)
            || (GetParam() == kPartitionedStorageAndLinksWithSelfLinks) || (GetParam() == kPartitionedAllEnabled);
    }
...
INSTANTIATE_TEST_SUITE_P(DocumentLoaderTest, DocumentLoaderTest,
    testing::Values(TestMode::kUnpartitionedStorageAndLinks, TestMode::kUnpartitionedStoragePartitionedNoSelfLinks,
        TestMode::kUnpartitionedStorageParttionedWithSelfLinks, TestMode::kUnpartitionedStoragePartitionedLinksBothEnabled,
        TestMode::kPartitionedStorageUnpartitionedLinks, TestMode::kPartitionedStorageAndLinksNoSelfLinks, TestMode::kPartitionedStorageAndLinksWithSelfLinks,
...
TEST_P(DocumentLoaderTest, SingleChunk)
{
    ...
    public:
        ...
        {
            EXPECT_EQ(34u, data.size()) << "foo.html was not served in a single chunk";
...
TEST_P(DocumentLoaderTest, MultiChunkWithReentrancy)
{
    ...
    class MainFrameClient : public URLLoaderTestDelegate, public frame_test_helpers::TestWebFrameClient {
    public:
        // URLLoaderTestDelegate overrides:
        bool FillNavigationParamsResponse(WebNavigationParams* params) override
        {
        ...
        bool ServedReentrantly() const
        {
            return served_reentrantly_;
        }
    ...
    web_view_helper_.GetWebView()->GetPage()->GetSettings().SetPluginsEnabled(true);

    {
        ScopedLoaderDelegate loader_delegate(&main_frame_client);
        frame_test_helpers::LoadFrameDontWait(MainFrame(), url_test_helpers::ToKURL("https://example.com/foo.html"));
...
{
    WebViewImpl* web_view_impl = web_view_helper_.InitializeAndLoad("about:blank");
    EXPECT_TRUE(To<LocalFrame>(web_view_impl->GetPage()->MainFrame())->Loader().GetDocumentLoader()->IsCommittedButEmpty());
}
...
TEST_F(DocumentLoaderSimTest, FramePolicyIntegrityOnNavigationCommit)
{
    ...
    EXPECT_TRUE(child_window->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kPayment));
}

TEST_P(DocumentLoaderTest, CommitsDeferredOnSameOriginNavigation)
{
    const KURL& requestor_url = KURL(NullURL(), "https://www.example.com/foo.html");
    WebViewImpl* web_view_impl = web_view_helper_.InitializeAndLoad("https://example.com/foo.html");

    const KURL& same_origin_url = KURL(NullURL(), "https://www.example.com/bar.html");
    std::unique_ptr<WebNavigationParams> params = WebNavigationParams::CreateWithEmptyHTMLForTesting(same_origin_url);
    params->requestor_origin = WebSecurityOrigin::Create(WebURL(requestor_url));
    LocalFrame* local_frame = To<LocalFrame>(web_view_impl->GetPage()->MainFrame());
...
TEST_P(DocumentLoaderTest, CommitsDeferredOnDifferentOriginNavigation)
{
    ...
    const KURL& other_origin_url = KURL(NullURL(), "https://www.another.com/bar.html");
    std::unique_ptr<WebNavigationParams> params = WebNavigationParams::CreateWithEmptyHTMLForTesting(other_origin_url);
    params->requestor_origin = WebSecurityOrigin::Create(WebURL(requestor_url));
    LocalFrame* local_frame = To<LocalFrame>(web_view_impl->GetPage()->MainFrame());
    local_frame->Loader().CommitNavigation(std::move(params), nullptr);
...
{
    const KURL& requestor_url = KURL(NullURL(), "https://www.example.com:8000/foo.html");
    WebViewImpl* web_view_impl = web_view_helper_.InitializeAndLoad("https://example.com:8000/foo.html");

    const KURL& different_port_url = KURL(NullURL(), "https://www.example.com:8080/bar.html");
    std::unique_ptr<WebNavigationParams> params = WebNavigationParams::CreateWithEmptyHTMLForTesting(different_port_url);
    params->requestor_origin = WebSecurityOrigin::Create(WebURL(requestor_url));
    LocalFrame* local_frame = To<LocalFrame>(web_view_impl->GetPage()->MainFrame());
    local_frame->Loader().CommitNavigation(std::move(params), nullptr);
...
TEST_P(DocumentLoaderTest, CommitsNotDeferredOnDataURLNavigation)
{
    ...
    std::unique_ptr<WebNavigationParams> params = WebNavigationParams::CreateWithEmptyHTMLForTesting(data_url);
    params->requestor_origin = WebSecurityOrigin::Create(WebURL(requestor_url));
    LocalFrame* local_frame = To<LocalFrame>(web_view_impl->GetPage()->MainFrame());
    local_frame->Loader().CommitNavigation(std::move(params), nullptr);
...
{
    const KURL& requestor_url = KURL(NullURL(), "https://subdomain.example.com/foo.html");
    WebViewImpl* web_view_impl = web_view_helper_.InitializeAndLoad("https://example.com/foo.html");

    const KURL& about_blank_url = KURL(NullURL(), "about:blank");
    std::unique_ptr<WebNavigationParams> params = std::make_unique<WebNavigationParams>();
    ...
    EXPECT_EQ(BlinkStorageKey::CreateFirstParty(SecurityOrigin::Create(requestor_url)), local_frame->DomWindow()->GetStorageKey());
}
...
TEST_P(DocumentLoaderTest, SameOriginNavigation)
{
    ...
    std::unique_ptr<WebNavigationParams> params = WebNavigationParams::CreateWithEmptyHTMLForTesting(same_origin_url);
    params->requestor_origin = WebSecurityOrigin::Create(WebURL(requestor_url));
    params->storage_key = BlinkStorageKey::CreateFirstParty(SecurityOrigin::Create(same_origin_url));
    LocalFrame* local_frame = To<LocalFrame>(web_view_impl->GetPage()->MainFrame());
    local_frame->Loader().CommitNavigation(std::move(params), nullptr);

    EXPECT_EQ(BlinkStorageKey::CreateFirstParty(SecurityOrigin::Create(same_origin_url)), local_frame->DomWindow()->GetStorageKey());

    EXPECT_EQ(local_frame->DomWindow()->GetStorageAccessApiStatus(), net::StorageAccessApiStatus::kNone);

    EXPECT_TRUE(local_frame->Loader().GetDocumentLoader()->LastNavigationHadTrustedInitiator());
}
...
TEST_P(DocumentLoaderTest, SameOriginNavigation_WithStorageAccess)
{
    ...
    EXPECT_EQ(local_frame->DomWindow()->GetStorageAccessApiStatus(), net::StorageAccessApiStatus::kAccessViaAPI);

    EXPECT_TRUE(local_frame->Loader().GetDocumentLoader()->LastNavigationHadTrustedInitiator());

    histogram_tester.ExpectUniqueSample("API.StorageAccess.DocumentLoadedWithStorageAccess", /*sample=*/true,
        /*expected_bucket_count=*/1);
    histogram_tester.ExpectUniqueSample("API.StorageAccess.DocumentInheritedStorageAccess", /*sample=*/true,
        /*expected_bucket_count=*/1);
}
...
TEST_P(DocumentLoaderTest, CrossOriginNavigation)
{
    ...
    std::unique_ptr<WebNavigationParams> params = WebNavigationParams::CreateWithEmptyHTMLForTesting(other_origin_url);
    params->requestor_origin = WebSecurityOrigin::Create(WebURL(requestor_url));
    params->storage_key = BlinkStorageKey::CreateFirstParty(SecurityOrigin::Create(other_origin_url));
    LocalFrame* local_frame = To<LocalFrame>(web_view_impl->GetPage()->MainFrame());
    base::HistogramTester histogram_tester;
    local_frame->Loader().CommitNavigation(std::move(params), nullptr);

    EXPECT_EQ(BlinkStorageKey::CreateFirstParty(SecurityOrigin::Create(other_origin_url)), local_frame->DomWindow()->GetStorageKey());

    EXPECT_FALSE(local_frame->Loader().GetDocumentLoader()->LastNavigationHadTrustedInitiator());

    histogram_tester.ExpectUniqueSample("API.StorageAccess.DocumentLoadedWithStorageAccess", /*sample=*/false,
        /*expected_bucket_count=*/1);
    histogram_tester.ExpectUniqueSample("API.StorageAccess.DocumentInheritedStorageAccess", /*sample=*/false,
        /*expected_bucket_count=*/1);
}
...
TEST_P(DocumentLoaderTest, StorageKeyFromNavigationParams)
{
    ...
    EXPECT_EQ(BlinkStorageKey::CreateWithNonce(SecurityOrigin::Create(other_origin_url), storage_key_to_commit.nonce().value()),
...
TEST_P(DocumentLoaderTest, StorageKeyCrossSiteFromNavigationParams)
{
    ...
    net::SchemefulSite top_level_site = net::SchemefulSite(url::Origin::Create(GURL("https://foo.com")));
    StorageKey storage_key_to_commit = StorageKey::Create(url::Origin::Create(GURL(other_origin_url)), top_level_site, mojom::AncestorChainBit::kCrossSite);
    params->storage_key = storage_key_to_commit;

    LocalFrame* local_frame = To<LocalFrame>(web_view_impl->GetPage()->MainFrame());
    local_frame->Loader().CommitNavigation(std::move(params), nullptr);

    EXPECT_EQ(BlinkStorageKey::Create(SecurityOrigin::Create(other_origin_url), BlinkSchemefulSite(top_level_site), mojom::AncestorChainBit::kCrossSite),
        local_frame->DomWindow()->GetStorageKey());
}
...
TEST_P(DocumentLoaderTest, JavascriptURLKeepsStorageKeyNonce)
{
    ...
    BlinkStorageKey storage_key = BlinkStorageKey::CreateWithNonce(SecurityOrigin::CreateUniqueOpaque(), base::UnguessableToken::Create());

    LocalFrame* frame = To<LocalFrame>(web_view_impl->GetPage()->MainFrame());
    frame->DomWindow()->SetStorageKey(storage_key);

    frame->LoadJavaScriptURL(url_test_helpers::ToKURL("javascript:'<p>hello world</p>'"));

    EXPECT_EQ(storage_key.GetNonce(), frame->DomWindow()->GetStorageKey().GetNonce());
}
...
TEST_P(DocumentLoaderTest, PublicSecureNotCounted)
{
    ...
    EXPECT_FALSE(document->IsUseCounted(WebFeature::kMainFrameNonSecurePrivateAddressSpace));
}
...
TEST_P(DocumentLoaderTest, PublicNonSecureNotCounted)
{
    ...
    // aren't counted for WebFeature::kMainFrameNonSecurePrivateAddressSpace
    WebViewImpl* web_view_impl = web_view_helper_.InitializeAndLoad("http://example.com/foo.html");
    Document* document = To<LocalFrame>(web_view_impl->GetPage()->MainFrame())->GetDocument();
    EXPECT_FALSE(document->IsUseCounted(WebFeature::kMainFrameNonSecurePrivateAddressSpace));
}
...
TEST_P(DocumentLoaderTest, PrivateSecureNotCounted)
{
    ...
    // aren't counted for WebFeature::kMainFrameNonSecurePrivateAddressSpace
    WebViewImpl* web_view_impl = web_view_helper_.InitializeAndLoad("https://192.168.1.1/foo.html");
    Document* document = To<LocalFrame>(web_view_impl->GetPage()->MainFrame())->GetDocument();
    EXPECT_FALSE(document->IsUseCounted(WebFeature::kMainFrameNonSecurePrivateAddressSpace));
}
...
TEST_P(DocumentLoaderTest, PrivateNonSecureIsCounted)
{
    ...
    WebViewImpl* web_view_impl = web_view_helper_.InitializeAndLoad("http://192.168.1.1/foo.html");
    Document* document = To<LocalFrame>(web_view_impl->GetPage()->MainFrame())->GetDocument();
    EXPECT_TRUE(document->IsUseCounted(WebFeature::kMainFrameNonSecurePrivateAddressSpace));
}
...
TEST_P(DocumentLoaderTest, LocalNonSecureIsCounted)
{
    ...
    // are counted for WebFeature::kMainFrameNonSecurePrivateAddressSpace
    WebViewImpl* web_view_impl = web_view_helper_.InitializeAndLoad("http://somethinglocal/foo.html");
    Document* document = To<LocalFrame>(web_view_impl->GetPage()->MainFrame())->GetDocument();
    EXPECT_TRUE(document->IsUseCounted(WebFeature::kMainFrameNonSecurePrivateAddressSpace));
}
...
TEST_F(DocumentLoaderSimTest, PrivateNonSecureChildFrameNotCounted)
{
    ...
    EXPECT_FALSE(child_document->IsUseCounted(WebFeature::kMainFrameNonSecurePrivateAddressSpace));
}
...
TEST_P(DocumentLoaderTest, EmbeddedCredentialsNavigation)
{
    ...
    } test_cases[] = { { "http://example.com/foo.html", false }, { "http://user:@example.com/foo.html", true }, { "http://:pass@example.com/foo.html", true },
        { "http://user:pass@example.com/foo.html", true } };
    for (const auto& test_case : test_cases) {
        WebViewImpl* web_view_impl = web_view_helper_.InitializeAndLoad(test_case.url);
        Document* document = To<LocalFrame>(web_view_impl->GetPage()->MainFrame())->GetDocument();
        EXPECT_EQ(test_case.useCounted, document->IsUseCounted(WebFeature::kTopLevelDocumentWithEmbeddedCredentials));
    }
}
...
TEST_P(DocumentLoaderTest, VisitedLinkSalt)
{
    ...
    std::unique_ptr<WebNavigationParams> params = WebNavigationParams::CreateWithEmptyHTMLForTesting(kUrl);
    ...
    std::optional<uint64_t> result_salt = platform_->GetVisitedLinkSaltForOrigin(url::Origin::Create(GURL(kUrl)));
...
TEST_P(DocumentLoaderTest, PartitionedVisitedLinksMainFrame)
{
    ...
    platform_->AddPartitionedVisitedLinkToMockHashtable(kCrossSiteUrl, kUrl, kUrl);

    // Load a blank slate.
    const WebViewImpl* web_view_impl = web_view_helper_.InitializeAndLoad("about:blank");
    ...
    // Perform the navigation and provide an empty vector for visited link state.
    const LocalFrame* local_frame = To<LocalFrame>(web_view_impl->GetPage()->MainFrame());
    local_frame->Loader().CommitNavigation(std::move(params), nullptr);
    ...
    EInsideLink result = document->GetVisitedLinkState().DetermineLinkState(*visited_link);
    ...
    unvisited_link->setAttribute(html_names::kHrefAttr, AtomicString("https://bar.com"));
