...
constexpr base::TimeDelta kInputDelayTraceEventThreshold = base::Milliseconds(250);
...
constexpr base::TimeDelta kFirstInputDelayTraceEventThreshold = base::Milliseconds(575);
...
const char kHistogramInputTimestamp[] = "PageLoad.InteractiveTiming.InputTimestamp3";
const char kHistogramProcessingTime[] = "PageLoad.InteractiveTiming.ProcessingTime";
const char kHistogramTimeToNextPaint[] = "PageLoad.InteractiveTiming.TimeToNextPaint";
...
{
    InteractiveDetector* detector = Supplement<Document>::From<InteractiveDetector>(document);
    if (!detector) {
        detector = MakeGarbageCollected<InteractiveDetector>(document, std::make_unique<NetworkActivityChecker>(&document));
...
InteractiveDetector::InteractiveDetector(Document& document, std::unique_ptr<NetworkActivityChecker> network_activity_checker)
    : Supplement<Document>(document)
    , ExecutionContextLifecycleObserver(document.GetExecutionContext())
    , clock_(base::DefaultTickClock::GetInstance())
    , network_activity_checker_(std::move(network_activity_checker))
    , time_to_interactive_timer_(document.GetTaskRunner(TaskType::kInternalDefault), this, &InteractiveDetector::TimeToInteractiveTimerFired)
    , initially_hidden_(document.hidden())
{
}

void InteractiveDetector::SetNavigationStartTime(base::TimeTicks navigation_start_time)
{
    ...
    base::TimeTicks initial_timer_fire_time = navigation_start_time + kTimeToInteractiveWindow;
...
WTF::Vector<std::optional<base::TimeDelta>> InteractiveDetector::GetFirstInputDelaysAfterBackForwardCacheRestore() const
{
    return page_event_times_.first_input_delays_after_back_forward_cache_restore;
}

std::optional<base::TimeTicks> InteractiveDetector::GetFirstInputTimestamp() const
{
    return page_event_times_.first_input_timestamp;
}

std::optional<base::TimeTicks> InteractiveDetector::GetFirstScrollTimestamp() const
{
    return page_event_times_.first_scroll_timestamp;
}

std::optional<base::TimeDelta> InteractiveDetector::GetFirstScrollDelay() const
{
    return page_event_times_.frist_scroll_delay;
}

bool InteractiveDetector::PageWasBackgroundedSinceEvent(base::TimeTicks event_time)
{
...
void InteractiveDetector::HandleForInputDelay(const Event& event, base::TimeTicks event_platform_timestamp, base::TimeTicks processing_start)
{
    DCHECK(event.isTrusted());
    DCHECK(event.type() == event_type_names::kPointerdown || event.type() == event_type_names::kPointerup || event.type() == event_type_names::kMousedown
        || event.type() == event_type_names::kMouseup || event.type() == event_type_names::kKeydown || event.type() == event_type_names::kClick);
    ...
    if (!page_event_times_.first_input_delay.has_value()) {
        ...
        if (delay > kFirstInputDelayTraceEventThreshold) {
            ...
            TRACE_EVENT_NESTABLE_ASYNC_BEGIN_WITH_TIMESTAMP0(
                "latency", "Long First Input Delay", TRACE_ID_WITH_SCOPE("Long First Input Delay", g_num_long_input_events), event_timestamp);
            TRACE_EVENT_NESTABLE_ASYNC_END_WITH_TIMESTAMP0(
                "latency", "Long First Input Delay", TRACE_ID_WITH_SCOPE("Long First Input Delay", g_num_long_input_events), event_timestamp + delay);
        ...
        // onwards.
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN_WITH_TIMESTAMP0(
            "latency", "Long Input Delay", TRACE_ID_WITH_SCOPE("Long Input Delay", g_num_long_input_events), event_timestamp);
        TRACE_EVENT_NESTABLE_ASYNC_END_WITH_TIMESTAMP0(
            "latency", "Long Input Delay", TRACE_ID_WITH_SCOPE("Long Input Delay", g_num_long_input_events), event_timestamp + delay);
        // Apply metadata on stack samples.
        base::ApplyMetadataToPastSamples(event_timestamp, event_timestamp + delay, "PageLoad.InteractiveTiming.LongInputDelay", g_num_long_input_events, 1,
    ...
    if (!page_event_times_.first_input_delays_after_back_forward_cache_restore.empty()
        && !page_event_times_.first_input_delays_after_back_forward_cache_restore.back().has_value()) {
        page_event_times_.first_input_delays_after_back_forward_cache_restore.back() = delay;
    }

    UMA_HISTOGRAM_CUSTOM_TIMES(kHistogramInputDelay, delay, base::Milliseconds(1), base::Seconds(60), 50);
    UMA_HISTOGRAM_CUSTOM_TIMES(kHistogramInputTimestamp, event_timestamp - page_event_times_.nav_start, base::Milliseconds(10), base::Minutes(10), 100);
...
void InteractiveDetector::BeginNetworkQuietPeriod(base::TimeTicks current_time)
{
...
void InteractiveDetector::EndNetworkQuietPeriod(base::TimeTicks current_time)
{
    ...
    if (current_time - active_network_quiet_window_start_ >= kTimeToInteractiveWindow) {
        network_quiet_windows_.emplace_back(active_network_quiet_window_start_, current_time);
    }
...
// clock_->NowTicks().
void InteractiveDetector::UpdateNetworkQuietState(double request_count, std::optional<base::TimeTicks> opt_current_time)
{
    if (request_count <= kNetworkQuietMaximumConnections && active_network_quiet_window_start_.is_null()) {
        ...
        base::TimeTicks current_time = opt_current_time ? opt_current_time.value() : clock_->NowTicks();
        BeginNetworkQuietPeriod(current_time);
    } else if (request_count > kNetworkQuietMaximumConnections && !active_network_quiet_window_start_.is_null()) {
        base::TimeTicks current_time = opt_current_time ? opt_current_time.value() : clock_->NowTicks();
        EndNetworkQuietPeriod(current_time);
    }
}

void InteractiveDetector::OnResourceLoadBegin(std::optional<base::TimeTicks> load_begin_time)
{
...
// clock_->NowTicks.
void InteractiveDetector::OnResourceLoadEnd(std::optional<base::TimeTicks> load_finish_time)
{
    if (!GetSupplementable())
        return;
    if (!interactive_time_.is_null())
...
void InteractiveDetector::OnLongTaskDetected(base::TimeTicks start_time, base::TimeTicks end_time)
{
...
void InteractiveDetector::OnFirstContentfulPaint(base::TimeTicks first_contentful_paint)
{
    ...
    if (clock_->NowTicks() - first_contentful_paint >= kTimeToInteractiveWindow) {
        ...
        StartOrPostponeCITimer(page_event_times_.first_contentful_paint + kTimeToInteractiveWindow);
    }
}
...
void InteractiveDetector::OnInvalidatingInputEvent(base::TimeTicks invalidation_time)
{
    ...
    page_event_times_.first_invalidating_input = std::max(invalidation_time, page_event_times_.nav_start);
...
{
    visibility_change_events_.push_back(VisibilityChangeEvent { clock_->NowTicks(), is_hidden });
}
...
void InteractiveDetector::AddCurrentlyActiveNetworkQuietInterval(base::TimeTicks current_time)
{
...
{
    if (!network_quiet_windows_.empty() && network_quiet_windows_.back().Low() == active_network_quiet_window_start_) {
        network_quiet_windows_.pop_back();
    }
}

base::TimeTicks InteractiveDetector::FindInteractiveCandidate(base::TimeTicks lower_bound, base::TimeTicks current_time)
{
    // Network iterator.
    ...
    while (main_quiet_start < current_time && it_net < network_quiet_windows_.end()) {
        base::TimeTicks main_quiet_end = it_lt == long_tasks_.end() ? current_time : it_lt->Low();
        base::TimeTicks next_main_quiet_start = it_lt == long_tasks_.end() ? current_time : it_lt->High();
        ...
        base::TimeTicks overlap_start = std::max({ main_quiet_start, it_net->Low(), lower_bound });
...
void InteractiveDetector::CheckTimeToInteractiveReached()
{
    ...
    const bool ignore_fcp = base::FeatureList::IsEnabled(features::kInteractiveDetectorIgnoreFcp);
    // FCP and DCL have not been detected yet.
    if ((page_event_times_.first_contentful_paint.is_null() && !ignore_fcp) || page_event_times_.dom_content_loaded_end.is_null()) {
        return;
    }

    const base::TimeTicks current_time = clock_->NowTicks();
    if (!ignore_fcp && (current_time - page_event_times_.first_contentful_paint < kTimeToInteractiveWindow)) {
        // Too close to FCP to determine Time to Interactive.
        return;
    }

    AddCurrentlyActiveNetworkQuietInterval(current_time);
    base::TimeTicks interactive_candidate = FindInteractiveCandidate(page_event_times_.first_contentful_paint, current_time);
    ...
    interactive_time_ = std::max({ interactive_candidate, page_event_times_.dom_content_loaded_end });
...
void InteractiveDetector::OnTimeToInteractiveDetected()
{
    ...
    probe::LifecycleEvent(frame, loader, "InteractiveTime", interactive_time_.since_origin().InSecondsF());

    TRACE_EVENT_MARK_WITH_TIMESTAMP2("loading,rail", "InteractiveTime", interactive_time_, "frame", GetFrameIdForTracing(GetSupplementable()->GetFrame()),
        "args", [&](perfetto::TracedValue context) {
            ...
            bool had_user_input_before_interactive
                = !page_event_times_.first_invalidating_input.is_null() && page_event_times_.first_invalidating_input < interactive_time_;

            auto dict = std::move(context).WriteDictionary();
            dict.Add("had_user_input_before_interactive", had_user_input_before_interactive);
            dict.Add("total_blocking_time_ms", ComputeTotalBlockingTime().InMillisecondsF());
        });
...
base::TimeDelta InteractiveDetector::ComputeTotalBlockingTime()
{
    ...
    for (const auto& long_task : long_tasks_) {
        base::TimeTicks clipped_start = std::max(long_task.Low(), page_event_times_.first_contentful_paint);
        base::TimeTicks clipped_end = std::min(long_task.High(), interactive_time_);
        total_blocking_time += std::max(base::TimeDelta(), clipped_end - clipped_start - base::Milliseconds(50));
    }
...
void InteractiveDetector::SetTaskRunnerForTesting(scoped_refptr<base::SingleThreadTaskRunner> task_runner_for_testing)
{
    time_to_interactive_timer_.MoveToNewTaskRunner(task_runner_for_testing);
}

void InteractiveDetector::RecordInputEventTimingUMA(base::TimeDelta processing_time, base::TimeDelta time_to_next_paint)
{
    UmaHistogramCustomTimes(kHistogramProcessingTime, processing_time, base::Milliseconds(1), base::Seconds(60), 50);
    UmaHistogramCustomTimes(kHistogramTimeToNextPaint, time_to_next_paint, base::Milliseconds(1), base::Seconds(60), 50);
}

void InteractiveDetector::DidObserveFirstScrollDelay(base::TimeDelta first_scroll_delay, base::TimeTicks first_scroll_timestamp)
{
...
void InteractiveDetector::OnRestoredFromBackForwardCache()
{
    ...
    page_event_times_.first_input_delays_after_back_forward_cache_restore.push_back(std::nullopt);
}
