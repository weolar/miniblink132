...
LoaderFactoryForWorker::LoaderFactoryForWorker(WorkerOrWorkletGlobalScope& global_scope, scoped_refptr<WebWorkerFetchContext> web_context)
    : global_scope_(global_scope)
    , web_context_(std::move(web_context))
{
}

std::unique_ptr<URLLoader> LoaderFactoryForWorker::CreateURLLoader(const network::ResourceRequest& network_request, const ResourceLoaderOptions& options,
    ...
    bool is_from_origin_dirty_style_sheet)
{
    ...
    WebVector<std::unique_ptr<URLLoaderThrottle>> web_throttles = web_context_->CreateThrottles(network_request);
    ...
    if (options.url_loader_factory) {
        mojo::Remote<network::mojom::blink::URLLoaderFactory> url_loader_factory_remote(std::move(options.url_loader_factory->data));
        url_loader_factory_remote->Clone(url_loader_factory.InitWithNewPipeAndPassReceiver());
    }
    ...
    if (network_request.url.SchemeIs("blob") && !url_loader_factory) {
        global_scope_->GetPublicURLManager().Resolve(KURL(network_request.url), url_loader_factory.InitWithNewPipeAndPassReceiver());
    }

    // KeepAlive is not yet supported in web workers.
    mojo::PendingRemote<mojom::blink::KeepAliveHandle> keep_alive_handle = mojo::NullRemote();

    if (url_loader_factory) {
        return web_context_->WrapURLLoaderFactory(std::move(url_loader_factory))
            ->CreateURLLoader(network_request, freezable_task_runner, unfreezable_task_runner, std::move(keep_alive_handle), back_forward_cache_loader_helper,
                std::move(throttles));
    }
    ...
    if (global_scope_->IsServiceWorkerGlobalScope()) {
        if (network_request.destination == network::mojom::RequestDestination::kServiceWorker
            || network_request.destination == network::mojom::RequestDestination::kScript) {
            // GetScriptLoaderFactory() may return nullptr in tests even for service
            // workers.
            if (web_context_->GetScriptLoaderFactory()) {
                return web_context_->GetScriptLoaderFactory()->CreateURLLoader(network_request, freezable_task_runner, unfreezable_task_runner,
                    std::move(keep_alive_handle), back_forward_cache_loader_helper, std::move(throttles));
            }
        }
        ...
        if (service_worker_race_network_request_token.has_value()) {
            ...
            std::optional<mojo::PendingRemote<network::mojom::blink::URLLoaderFactory>> race_network_request_url_loader_factory
            ...
            if (race_network_request_url_loader_factory) {
                ...
                if (!has_dumped_without_crashing && !network_request.url.SchemeIsHTTPOrHTTPS()) {
                    has_dumped_without_crashing = true;
                    SCOPED_CRASH_KEY_BOOL("SWRace", "loader_factory_has_value", race_network_request_url_loader_factory.has_value());
                    SCOPED_CRASH_KEY_BOOL("SWRace", "is_valid_loader_factory", race_network_request_url_loader_factory->is_valid());
                    SCOPED_CRASH_KEY_BOOL("SWRace", "is_empty_token", token.is_empty());
                    SCOPED_CRASH_KEY_STRING64("SWRace", "token", token.ToString());
                    SCOPED_CRASH_KEY_STRING256("SWRace", "request_url", network_request.url.spec());
                    base::debug::DumpWithoutCrashing();
                }

                return web_context_->WrapURLLoaderFactory(std::move(race_network_request_url_loader_factory.value()))
                    ->CreateURLLoader(network_request, freezable_task_runner, unfreezable_task_runner, std::move(keep_alive_handle),
    ...
    return web_context_->GetURLLoaderFactory()->CreateURLLoader(
        network_request, freezable_task_runner, unfreezable_task_runner, std::move(keep_alive_handle), back_forward_cache_loader_helper, std::move(throttles));
}
