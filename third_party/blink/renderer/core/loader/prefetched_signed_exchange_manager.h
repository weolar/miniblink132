...
class PrefetchedSignedExchangeManager final : public GarbageCollected<PrefetchedSignedExchangeManager> {
public:
    // If threre are no "allowed-alt-sxg" link headers in |inner_link_header|,
    // or |prefetched_signed_exchanges| is empty, returns null.
    static PrefetchedSignedExchangeManager* MaybeCreate(LocalFrame* frame, const String& outer_link_header, const String& inner_link_header,
        WebVector<std::unique_ptr<WebNavigationParams::PrefetchedSignedExchange>> prefetched_signed_exchanges);

    PrefetchedSignedExchangeManager(LocalFrame* frame, std::unique_ptr<AlternateSignedExchangeResourceInfo> alternative_resources,
        HashMap<KURL, std::unique_ptr<WebNavigationParams::PrefetchedSignedExchange>> prefetched_exchanges_map);
    PrefetchedSignedExchangeManager(const PrefetchedSignedExchangeManager&) = delete;
    PrefetchedSignedExchangeManager& operator=(const PrefetchedSignedExchangeManager&) = delete;
    ...
    std::unique_ptr<URLLoader> MaybeCreateURLLoader(
        const network::ResourceRequest& request, base::OnceCallback<Vector<std::unique_ptr<URLLoaderThrottle>>(void)> create_throttles_callback);
...
private:
    ...
    void TriggerLoad();
    std::unique_ptr<URLLoader> CreateDefaultURLLoader(const network::ResourceRequest& request, Vector<std::unique_ptr<URLLoaderThrottle>> throttles);
    std::unique_ptr<URLLoader> CreatePrefetchedSignedExchangeURLLoader(const network::ResourceRequest& request,
    ...
    HashMap<KURL, std::unique_ptr<WebNavigationParams::PrefetchedSignedExchange>> prefetched_exchanges_map_;
    bool started_ = false;
