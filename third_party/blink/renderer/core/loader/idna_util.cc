...
String UnsafeASCIIToIDNA(const StringView& hostname_ascii)
{
    static UIDNA* uidna = [] {
        ...
        UIDNA* value = uidna_openUTS46(UIDNA_CHECK_BIDI | UIDNA_NONTRANSITIONAL_TO_ASCII | UIDNA_NONTRANSITIONAL_TO_UNICODE, &err);
        if (U_FAILURE(err)) {
            value = nullptr;
        }
        return value;
    ...
    Vector<char> output_utf8(static_cast<wtf_size_t>(kMaximumDomainNameLengthForIDNADecoding), '\0');
    ...
    int32_t output_utf8_length
        = uidna_nameToUnicodeUTF8(uidna, hostname.data(), static_cast<int32_t>(hostname.size()), output_utf8.data(), output_utf8.size(), &info, &status);
    if (U_FAILURE(status) || info.errors != 0 || output_utf8_length > kMaximumDomainNameLengthForIDNADecoding) {
        return String();
    }
    //   return String::FromUTF8(
    //     base::span<const uint8_t>((const uint8_t*)output_utf8.data(), output_utf8.size()).first(base::checked_cast<size_t>(output_utf8_length)));
    return String::FromUTF8((const uint8_t*)output_utf8.data(), output_utf8_length);
}
...
String GetConsoleWarningForIDNADeviationCharacters(const KURL& url)
{
    ...
    String elided = url.ElidedString().replace(url.HostStart(), url.HostEnd() - url.HostStart(), host);
    StringBuilder message;
    message.Append("The resource at ");
    message.Append(elided);
    message.Append(" contains IDNA Deviation Characters. The hostname for this URL (");
    message.Append(host);
    message.Append(") might point to a different IP address after "
