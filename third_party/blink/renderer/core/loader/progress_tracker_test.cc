...
public:
    ProgressTrackerTest()
        : response_(KURL("http://example.com"))
        , last_progress_(0.0)
    {
    ...
    void SetUp() override
    {
        FakeLocalFrameHost::Init(web_frame_client_.GetRemoteNavigationAssociatedInterfaces());
    ...
    {
        return GetFrame()->Loader().Progress();
    }

    double LastProgress() const
    {
        return last_progress_;
    }

    const ResourceResponse& ResponseHeaders() const
    {
        return response_;
    }
