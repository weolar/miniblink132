...
constexpr char kFirstCookieRequestHistogram[] = "Blink.Experimental.Cookies.FirstCookieRequest";
...
void CookieJar::SetCookie(const String& value)
{
    ...
    backend_->SetCookieFromString(
        cookie_url, document_->SiteForCookies(), document_->TopFrameOrigin(), document_->GetExecutionContext()->GetStorageAccessApiStatus(), value);
...
String CookieJar::Cookies()
{
    ...
    const bool get_version_shared_memory = !shared_memory_version_client_.has_value();
    ...
    if (IPCNeeded()) {
        bool is_ad_tagged = document_->GetFrame() ? document_->GetFrame()->IsAdFrame() : false;

        if (!backend_->GetCookiesString(cookie_url, document_->SiteForCookies(), document_->TopFrameOrigin(),
                document_->GetExecutionContext()->GetStorageAccessApiStatus(), get_version_shared_memory, is_ad_tagged,
                /*force_disable_third_party_cookies=*/false, &new_version, &new_mapped_region, &value)) {
...
bool CookieJar::CookiesEnabled()
{
    ...
    backend_->CookiesEnabledFor(
        cookie_url, document_->SiteForCookies(), document_->TopFrameOrigin(), document_->GetExecutionContext()->GetStorageAccessApiStatus(), &cookies_enabled);
...
void CookieJar::SetCookieManager(mojo::PendingRemote<network::mojom::blink::RestrictedCookieManager> cookie_manager)
{
    backend_.reset();
    backend_.Bind(std::move(cookie_manager), document_->GetTaskRunner(TaskType::kInternalDefault));
}
...
bool CookieJar::IPCNeeded()
{
    ...
    if (shared_memory_version_client_->SharedVersionIsGreaterThan(last_version_)) {
        return true;
    }
...
void CookieJar::RequestRestrictedCookieManagerIfNeeded()
{
    if (!backend_.is_bound() || !backend_.is_connected()) {
        ...
        document_->GetFrame()->GetBrowserInterfaceBroker().GetInterface(
            backend_.BindNewPipeAndPassReceiver(document_->GetTaskRunner(TaskType::kInternalDefault)));
    }
}

void CookieJar::UpdateCacheAfterGetRequest(const KURL& cookie_url, const String& cookie_string, uint64_t new_version)
{
    std::optional<unsigned> new_hash = WTF::HashInts(WTF::GetHash(cookie_url), cookie_string.IsNull() ? 0 : WTF::GetHash(cookie_string));

    CookieCacheLookupResult result = CookieCacheLookupResult::kCacheMissFirstAccess;

    // An invalid version means no shared memory communication so assume changes
    // happened.
    const bool cookie_is_unchanged = new_version != mojo::shared_memory_version::kInvalidVersion && last_version_ == new_version;

    if (last_cookies_hash_.has_value() && cookie_is_unchanged) {
        if (last_cookies_hash_ == new_hash) {
            result = last_operation_was_set_ ? CookieCacheLookupResult::kCacheHitAfterSet : CookieCacheLookupResult::kCacheHitAfterGet;
        } else {
            result = last_operation_was_set_ ? CookieCacheLookupResult::kCacheMissAfterSet : CookieCacheLookupResult::kCacheMissAfterGet;
        }
    }

    UMA_HISTOGRAM_ENUMERATION("Blink.Experimental.Cookies.CacheLookupResult2", result);
...
void CookieJar::LogFirstCookieRequest(FirstCookieRequest first_cookie_request)
{
    ...
    base::UmaHistogramEnumeration(kFirstCookieRequestHistogram, first_cookie_request);
}
