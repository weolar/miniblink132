...
class CORE_EXPORT WebAssociatedURLLoaderImpl final : public WebAssociatedURLLoader {
    USING_FAST_MALLOC(WebAssociatedURLLoaderImpl);

public:
    WebAssociatedURLLoaderImpl(ExecutionContext*, const WebAssociatedURLLoaderOptions&);
    WebAssociatedURLLoaderImpl(const WebAssociatedURLLoaderImpl&) = delete;
    WebAssociatedURLLoaderImpl& operator=(const WebAssociatedURLLoaderImpl&) = delete;
    ~WebAssociatedURLLoaderImpl() override;

    void LoadAsynchronously(const WebURLRequest&, WebAssociatedURLLoaderClient*) override;
