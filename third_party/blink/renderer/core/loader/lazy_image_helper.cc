...
bool LazyImageHelper::LoadAllImagesAndBlockLoadEvent(Document& document)
{
    if (Document* root_document = GetRootDocumentOrNull(&document)) {
        return root_document->EnsureLazyLoadImageObserver().LoadAllImagesAndBlockLoadEvent(document);
    }
    return false;
}

// static
bool LazyImageHelper::ShouldDeferImageLoad(LocalFrame& frame, HTMLImageElement* html_image)
{
    ...
    LoadingAttributeValue loading_attr = GetLoadingAttributeValue(html_image->FastGetAttribute(html_names::kLoadingAttr));
    if (loading_attr == LoadingAttributeValue::kEager) {
        UseCounter::Count(frame.GetDocument(), WebFeature::kLazyLoadImageLoadingAttributeEager);
        return false;
    }
    ...
    UseCounter::Count(frame.GetDocument(), WebFeature::kLazyLoadImageLoadingAttributeLazy);
    if (frame.GetLazyLoadImageSetting() == LocalFrame::LazyLoadImageSetting::kDisabled) {
        return false;
    }
