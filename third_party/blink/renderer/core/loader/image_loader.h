...
class CORE_EXPORT ImageLoader : public GarbageCollected<ImageLoader>, public ImageResourceObserver {
...
public:
    ...
    void UpdateFromElement(UpdateFromElementBehavior = kUpdateNormal, bool force_blocking = false);
    ...
    {
        return HasPendingEvent() || pending_task_;
    }

    bool HasPendingError() const
    {
        return pending_error_event_.IsActive();
    }

    bool HadError() const
    {
        return !failed_load_url_.empty();
    }
    ...
    // `force_blocking` ensures that the image will block the load event.
    void LoadDeferredImage(bool force_blocking = false, bool update_from_microtask = false);
...
private:
    ...
    // force_blocking ensures that the image will block the load event.
    void DoUpdateFromElement(const DOMWrapperWorld* world, UpdateFromElementBehavior, UpdateType = UpdateType::kAsync, bool force_blocking = false);
    ...
    std::unique_ptr<IncrementLoadEventDelayCount> delay_until_do_update_from_element_;
    ...
    std::unique_ptr<IncrementLoadEventDelayCount> delay_until_image_notify_finished_;
