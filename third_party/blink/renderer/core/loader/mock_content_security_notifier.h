...
class MockContentSecurityNotifier : public mojom::blink::ContentSecurityNotifier {
public:
    ...
    MockContentSecurityNotifier& operator=(const MockContentSecurityNotifier&) = delete;

    mojo::PendingRemote<mojom::blink::ContentSecurityNotifier> BindNewPipeAndPassRemote()
    {
        return receiver_.BindNewPipeAndPassRemote();
    }

    void Bind(mojo::PendingReceiver<mojom::blink::ContentSecurityNotifier> pending_receiver)
    {
    ...
    MOCK_METHOD2(NotifyInsecureContentRan, void(const KURL& origin, const KURL& insecure_url));
