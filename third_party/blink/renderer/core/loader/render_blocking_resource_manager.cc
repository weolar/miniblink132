...
const base::TimeDelta kMaxRenderingDelayForFontPreloads = base::Milliseconds(50);

class ImperativeFontLoadFinishedCallback final : public GarbageCollected<ImperativeFontLoadFinishedCallback>, public FontFace::LoadFontCallback {
public:
...
private:
    void NotifyLoaded(FontFace*) final
    {
        ...
        document_->GetRenderBlockingResourceManager()->RemoveImperativeFontLoading();
    }
...
RenderBlockingResourceManager::RenderBlockingResourceManager(Document& document)
    ...
    , font_preload_max_blocking_timer_(
          document.GetTaskRunner(TaskType::kInternalFrameLifecycleControl), this, &RenderBlockingResourceManager::FontPreloadingTimerFired)
...
void RenderBlockingResourceManager::AddPendingFontPreload(const PendingLinkPreload& link)
{
...
void RenderBlockingResourceManager::AddImperativeFontLoading(FontFace* font_face)
{
    ...
    ImperativeFontLoadFinishedCallback* callback = MakeGarbageCollected<ImperativeFontLoadFinishedCallback>(*document_);
    font_face->AddCallback(callback);
    ++imperative_font_loading_count_;
    EnsureStartFontPreloadMaxBlockingTimer();
}

void RenderBlockingResourceManager::RemovePendingFontPreload(const PendingLinkPreload& link)
{
...
{
    if (font_preload_timer_has_fired_ || font_preload_max_blocking_timer_.IsActive()) {
        return;
    }
    base::TimeDelta timeout = base::FeatureList::IsEnabled(features::kRenderBlockingFonts)
...
void RenderBlockingResourceManager::FontPreloadingTimerFired(TimerBase*)
{
    ...
    base::UmaHistogramBoolean("WebFont.Clients.RenderBlockingFonts.ExpiredFonts", pending_font_preloads_.size() + imperative_font_loading_count_);
...
void RenderBlockingResourceManager::AddPendingParsingElementLink(const AtomicString& id, const HTMLLinkElement* link)
{
    ...
    if (it == element_render_blocking_links_.end()) {
        auto result = element_render_blocking_links_.insert(id, MakeGarbageCollected<HeapHashSet<WeakMember<const HTMLLinkElement>>>());
...
void RenderBlockingResourceManager::RemovePendingParsingElement(const AtomicString& id, Element* element)
{
    if (!RuntimeEnabledFeatures::DocumentRenderBlockingEnabled()) {
        return;
    }
    ...
    element_render_blocking_links_.erase(AtomicString(EncodeWithURLEscapeSequences(id)));
...
void RenderBlockingResourceManager::RemovePendingParsingElementLink(const AtomicString& id, const HTMLLinkElement* link)
{
    if (!RuntimeEnabledFeatures::DocumentRenderBlockingEnabled()) {
        return;
    }
...
void RenderBlockingResourceManager::SetFontPreloadTimeoutForTest(base::TimeDelta timeout)
{
...
bool RenderBlockingResourceManager::AddPendingStylesheet(const Node& owner_node)
{
    if (document_->body())
        return false;
...
bool RenderBlockingResourceManager::RemovePendingStylesheet(const Node& owner_node)
{
...
void RenderBlockingResourceManager::AddPendingScript(const ScriptElementBase& script)
{
    if (document_->body())
        return;
...
void RenderBlockingResourceManager::RemovePendingScript(const ScriptElementBase& script)
{
...
{
    if (base::FeatureList::IsEnabled(features::kRenderBlockingFonts) && !HasNonFontRenderBlockingResources() && HasRenderBlockingFonts()) {
...
{
    document_->RenderBlockingResourceUnblocked();
    if (base::FeatureList::IsEnabled(features::kRenderBlockingFonts) && !HasNonFontRenderBlockingResources() && HasRenderBlockingFonts() && document_->body()) {
        EnsureStartFontPreloadMaxFCPDelayTimer();
    }
}
...
{
    if (font_preload_timer_has_fired_ || font_preload_max_fcp_delay_timer_.IsActive()) {
        return;
    }
    base::TimeDelta max_fcp_delay = base::Milliseconds(features::kMaxFCPDelayMsForRenderBlockingFonts.Get());
