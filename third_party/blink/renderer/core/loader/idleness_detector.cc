...
void IdlenessDetector::DidDropNavigation()
{
    ...
    if (!task_observer_added_ && network_2_quiet_start_time_.is_null() && network_0_quiet_start_time_.is_null()) {
        Start();
    }
}
...
void IdlenessDetector::OnDidLoadResource()
{
    ...
    int request_count = local_frame_->GetDocument()->Fetcher()->ActiveRequestCount();
    ...
    } else if (request_count < 2 && in_network_2_quiet_period_ && network_2_quiet_.is_null()) {
        network_2_quiet_ = timestamp;
        network_2_quiet_start_time_ = timestamp;
    }
...
void IdlenessDetector::WillProcessTask(base::TimeTicks start_time)
{
    ...
    if (in_network_2_quiet_period_ && !network_2_quiet_.is_null() && start_time - network_2_quiet_ > network_quiet_window_) {
        probe::LifecycleEvent(local_frame_, loader, "networkAlmostIdle", network_2_quiet_start_time_.since_origin().InSecondsF());
        DCHECK(local_frame_->GetDocument());
        if (auto* document_resource_coordinator = local_frame_->GetDocument()->GetResourceCoordinator()) {
            document_resource_coordinator->SetNetworkAlmostIdle();
        }
        if (WebServiceWorkerNetworkProvider* service_worker_network_provider = loader->GetServiceWorkerNetworkProvider()) {
            service_worker_network_provider->DispatchNetworkQuiet();
        }
        FirstMeaningfulPaintDetector::From(*local_frame_->GetDocument()).OnNetwork2Quiet();
        in_network_2_quiet_period_ = false;
        network_2_quiet_ = base::TimeTicks();
    }

    if (in_network_0_quiet_period_ && !network_0_quiet_.is_null() && start_time - network_0_quiet_ > network_quiet_window_) {
        probe::LifecycleEvent(local_frame_, loader, "networkIdle", network_0_quiet_start_time_.since_origin().InSecondsF());
        in_network_0_quiet_period_ = false;
        network_0_quiet_ = base::TimeTicks();
    }
...
void IdlenessDetector::DidProcessTask(base::TimeTicks start_time, base::TimeTicks end_time)
{
...
IdlenessDetector::IdlenessDetector(LocalFrame* local_frame, const base::TickClock* clock)
    : local_frame_(local_frame)
    , task_observer_added_(false)
    , clock_(clock)
    , network_quiet_timer_(local_frame->GetTaskRunner(TaskType::kInternalLoading), this, &IdlenessDetector::NetworkQuietTimerFired)
{
    if (local_frame->GetSettings()) {
        network_quiet_window_ = base::Seconds(local_frame->GetSettings()->GetNetworkQuietTimeout());
    }
}
...
void IdlenessDetector::NetworkQuietTimerFired(TimerBase*)
{
    ...
    if ((in_network_0_quiet_period_ && !network_0_quiet_.is_null()) || (in_network_2_quiet_period_ && !network_2_quiet_.is_null())) {
        network_quiet_timer_.StartOneShot(kNetworkQuietWatchdog, FROM_HERE);
    }
}
