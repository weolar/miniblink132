...
BASE_FEATURE(kResourceLoadThrottlingTrial, "ResourceLoadScheduler", base::FEATURE_DISABLED_BY_DEFAULT);
...
constexpr base::FeatureParam<int> kOutstandingLimitForBackgroundMainFrame { &kResourceLoadThrottlingTrial, "bg_limit", 3 };
constexpr base::FeatureParam<int> kOutstandingLimitForBackgroundSubFrame { &kResourceLoadThrottlingTrial, "bg_sub_limit", 2 };

} // namespace

FrameResourceFetcherProperties::FrameResourceFetcherProperties(DocumentLoader& document_loader, Document& document)
    : document_loader_(document_loader)
    , document_(document)
    , fetch_client_settings_object_(MakeGarbageCollected<FetchClientSettingsObjectImpl>(*document.domWindow()))
{
}
...
mojom::ControllerServiceWorkerMode FrameResourceFetcherProperties::GetControllerServiceWorkerMode() const
{
    auto* service_worker_network_provider = document_loader_->GetServiceWorkerNetworkProvider();
...
{
    DCHECK_NE(GetControllerServiceWorkerMode(), blink::mojom::ControllerServiceWorkerMode::kNoController);
    auto* service_worker_network_provider = document_loader_->GetServiceWorkerNetworkProvider();
...
bool FrameResourceFetcherProperties::IsSubframeDeprioritizationEnabled() const
{
    ...
    const WebEffectiveConnectionType max_effective_connection_type_threshold = settings->GetLowPriorityIframesThreshold();
    if (max_effective_connection_type_threshold <= WebEffectiveConnectionType::kTypeOffline) {
        return false;
    }

    const WebEffectiveConnectionType effective_connection_type = GetNetworkStateNotifier().EffectiveType();
...
{
    static const int main_frame_limit = kOutstandingLimitForBackgroundMainFrame.Get();
    static const int sub_frame_limit = kOutstandingLimitForBackgroundSubFrame.Get();
