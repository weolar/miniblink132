...
public:
    ...
    AlternateSignedExchangeResourceInfoTest(const AlternateSignedExchangeResourceInfoTest&) = delete;
    AlternateSignedExchangeResourceInfoTest& operator=(const AlternateSignedExchangeResourceInfoTest&) = delete;
    ~AlternateSignedExchangeResourceInfoTest() override = default;

protected:
    const AlternateSignedExchangeResourceInfo::EntryMap& GetEntries(const AlternateSignedExchangeResourceInfo* info)
    {
...
{
    std::unique_ptr<AlternateSignedExchangeResourceInfo> info = AlternateSignedExchangeResourceInfo::CreateIfValid("", "");
    EXPECT_FALSE(info);
}
...
TEST_F(AlternateSignedExchangeResourceInfoTest, Simple)
{
    ...
    EXPECT_EQ(KURL("https://publisher.example/script.js"), resource->anchor_url());
    EXPECT_EQ(KURL("https://distributor.example/publisher.example/script.js.sxg"), resource->alternative_url());
    EXPECT_EQ("sha256-7KheEN4nyNxE3c4yQZdgCBJthJ2UwgpLSBeSUpII+jg=", resource->header_integrity());
    EXPECT_TRUE(resource->variants().empty());
    EXPECT_TRUE(resource->variant_key().empty());

    EXPECT_EQ(resource.get(), info->FindMatchingEntry(KURL("https://publisher.example/script.js"), std::nullopt, { "en" }));
    EXPECT_EQ(nullptr, info->FindMatchingEntry(KURL("https://publisher.example/image"), std::nullopt, { "en" }));
}
...
TEST_F(AlternateSignedExchangeResourceInfoTest, MultipleResources)
{
    ...
    }
    {
        ...
        EXPECT_EQ(KURL("https://distributor.example/publisher.example/image.sxg"), resource->alternative_url());
        EXPECT_EQ("sha256-q1phjFcR+umcl0zBaEz6E5AGVlnc9yF0zOjDYi5c6aM=", resource->header_integrity());
        EXPECT_TRUE(resource->variants().empty());
        EXPECT_TRUE(resource->variant_key().empty());
        EXPECT_EQ(resource.get(), info->FindMatchingEntry(KURL("https://publisher.example/image"), std::nullopt, { "en" }));
    }
}

TEST_F(AlternateSignedExchangeResourceInfoTest, NoMatchingOuterAlternateLinkHeader)
{
    std::unique_ptr<AlternateSignedExchangeResourceInfo> info = AlternateSignedExchangeResourceInfo::CreateIfValid(
...
TEST_F(AlternateSignedExchangeResourceInfoTest, Variants)
{
    ...
    ASSERT_EQ(2u, it->value.size());
    {
        const auto& resource = it->value.at(0);
        EXPECT_EQ(KURL("https://publisher.example/image"), resource->anchor_url());
        EXPECT_EQ(KURL("https://distributor.example/publisher.example/image_jpeg.sxg"), resource->alternative_url());
        EXPECT_EQ("sha256-q1phjFcR+umcl0zBaEz6E5AGVlnc9yF0zOjDYi5c6aM=", resource->header_integrity());
    ...
    }
    {
        ...
        EXPECT_EQ(KURL("https://publisher.example/image"), resource->anchor_url());
        EXPECT_EQ(KURL("https://distributor.example/publisher.example/image_webp.sxg"), resource->alternative_url());
        EXPECT_EQ("sha256-KRcYU+BZK8Sb2ccJfDPz+uUKXDdB1PVToPugItdzRXY=", resource->header_integrity());
        EXPECT_EQ("accept;image/jpeg;image/webp", resource->variants());
        EXPECT_EQ("image/webp", resource->variant_key());

        EXPECT_EQ(resource.get(), info->FindMatchingEntry(KURL("https://publisher.example/image"), network::mojom::RequestDestination::kImage, { "en" }));
    }
}
