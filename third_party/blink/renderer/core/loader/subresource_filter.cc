...
String GetErrorStringForDisallowedLoad(const KURL& url)
{
    ...
    builder.Append(" on this site because this site tends to show ads that interrupt, "
...
SubresourceFilter::SubresourceFilter(ExecutionContext* execution_context, std::unique_ptr<WebDocumentSubresourceFilter> subresource_filter)
...
bool SubresourceFilter::AllowLoad(const KURL& resource_url, network::mojom::RequestDestination request_destination, ReportingDisposition reporting_disposition)
{
    // TODO(csharrison): Implement a caching layer here which is a HashMap of
    // Pair<url string, context> -> LoadPolicy.
    WebDocumentSubresourceFilter::LoadPolicy load_policy = subresource_filter_->GetLoadPolicy(resource_url, request_destination);

    if (reporting_disposition == ReportingDisposition::kReport) {
        ReportLoad(resource_url, load_policy);
    }

    last_resource_check_result_ = std::make_pair(std::make_pair(resource_url, request_destination), load_policy);

    return load_policy != WebDocumentSubresourceFilter::kDisallow;
}

void SubresourceFilter::ReportLoadAsync(const KURL& resource_url, WebDocumentSubresourceFilter::LoadPolicy load_policy)
{
    ...
    scoped_refptr<base::SingleThreadTaskRunner> task_runner = execution_context_->GetTaskRunner(TaskType::kNetworking);
    DCHECK(task_runner->RunsTasksInCurrentSequence());
    task_runner->PostTask(FROM_HERE, WTF::BindOnce(&SubresourceFilter::ReportLoad, WrapPersistent(this), resource_url, load_policy));
}

bool SubresourceFilter::AllowWebSocketConnection(const KURL& url)
{
    WebDocumentSubresourceFilter::LoadPolicy load_policy = subresource_filter_->GetLoadPolicyForWebSocketConnect(url);
...
{
    WebDocumentSubresourceFilter::LoadPolicy load_policy = subresource_filter_->GetLoadPolicyForWebTransportConnect(url);

    ReportLoadAsync(url, load_policy);
    return load_policy != WebDocumentSubresourceFilter::kDisallow;
}

bool SubresourceFilter::IsAdResource(const KURL& resource_url, network::mojom::RequestDestination request_destination)
{
    WebDocumentSubresourceFilter::LoadPolicy load_policy;
    if (last_resource_check_result_.first == std::make_pair(resource_url, request_destination)) {
        load_policy = last_resource_check_result_.second;
    } else {
        load_policy = subresource_filter_->GetLoadPolicy(resource_url, request_destination);
    }

    return load_policy != WebDocumentSubresourceFilter::kAllow;
}

void SubresourceFilter::ReportLoad(const KURL& resource_url, WebDocumentSubresourceFilter::LoadPolicy load_policy)
{
    switch (load_policy) {
    ...
    case WebDocumentSubresourceFilter::kDisallow:
        ...
        if (subresource_filter_->ShouldLogToConsole()) {
            execution_context_->AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(
                mojom::ConsoleMessageSource::kOther, mojom::ConsoleMessageLevel::kError, GetErrorStringForDisallowedLoad(resource_url)));
        }
        [[fallthrough]];
    case WebDocumentSubresourceFilter::kWouldDisallow:
        ...
        if (auto* window = DynamicTo<LocalDOMWindow>(execution_context_.Get())) {
            if (auto* frame = window->GetFrame()) {
                frame->Loader().GetDocumentLoader()->DidObserveLoadingBehavior(kLoadingBehaviorSubresourceFilterMatch);
            }
        }
        break;
    }
