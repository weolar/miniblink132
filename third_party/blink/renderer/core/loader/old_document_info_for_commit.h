...
struct OldDocumentInfoForCommit : GarbageCollected<OldDocumentInfoForCommit> {
    explicit OldDocumentInfoForCommit(scoped_refptr<SecurityOrigin> new_document_origin);
...
public:
    explicit ScopedOldDocumentInfoForCommitCapturer(OldDocumentInfoForCommit* info)
        : info_(info)
        , previous_capturer_(current_capturer_)
    {
