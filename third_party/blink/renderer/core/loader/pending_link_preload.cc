...
public:
    FinishObserver(PendingLinkPreload* pending_preload, Resource* resource)
        ...
        , resource_(resource)
    {
        resource_->AddFinishObserver(this, pending_preload_->GetLoadingTaskRunner().get());
    }
    ...
    void NotifyFinished() override
    {
        ...
        if (resource_->GetType() == ResourceType::kFont) {
            RenderBlockingMetricsReporter::From(*pending_preload_->document_).PreloadedFontFinishedLoading();
        }
...
void PendingLinkPreload::AddResource(Resource* resource)
{
    ...
    if (resource) {
        if (resource->GetType() == ResourceType::kFont) {
            RenderBlockingMetricsReporter::From(*document_).PreloadedFontStartedLoading();
        }
...
{
    if (RenderBlockingResourceManager* manager = document_->GetRenderBlockingResourceManager()) {
        manager->RemovePendingFontPreload(*this);
    }
}

scoped_refptr<base::SingleThreadTaskRunner> PendingLinkPreload::GetLoadingTaskRunner()
{
