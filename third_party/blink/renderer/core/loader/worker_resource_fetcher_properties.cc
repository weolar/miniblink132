...
WorkerResourceFetcherProperties::WorkerResourceFetcherProperties(
    WorkerOrWorkletGlobalScope& global_scope, const FetchClientSettingsObject& fetch_client_settings_object, scoped_refptr<WebWorkerFetchContext> web_context)
    : global_scope_(global_scope)
    , fetch_client_settings_object_(fetch_client_settings_object)
    , web_context_(std::move(web_context))
    , outstanding_throttled_limit_(global_scope_->GetOutstandingThrottledLimit())
{
    DCHECK(web_context_);
}
...
mojom::ControllerServiceWorkerMode WorkerResourceFetcherProperties::GetControllerServiceWorkerMode() const
{
