...
class CORE_EXPORT SubresourceFilter final : public GarbageCollected<SubresourceFilter> {
public:
    SubresourceFilter(ExecutionContext*, std::unique_ptr<WebDocumentSubresourceFilter>);
    ~SubresourceFilter();

    bool AllowLoad(const KURL& resource_url, network::mojom::RequestDestination, ReportingDisposition);
    ...
    bool IsAdResource(const KURL& resource_url, network::mojom::RequestDestination);

    virtual void Trace(Visitor*) const;

private:
    void ReportLoad(const KURL& resource_url, WebDocumentSubresourceFilter::LoadPolicy);
    void ReportLoadAsync(const KURL& resource_url, WebDocumentSubresourceFilter::LoadPolicy);
    ...
    std::pair<std::pair<KURL, network::mojom::RequestDestination>, WebDocumentSubresourceFilter::LoadPolicy> last_resource_check_result_;
};
