...
public:
    ...
    // test data directory.
    KURL RegisterMockedURL(const String& file_name, const String& mime_type = "image/png")
    {
        return url_test_helpers::RegisterMockedURLLoadFromBase(kIconLoaderBaseUrl, test::CoreTestDataPath(kIconLoaderBaseDir), file_name, mime_type);
    }

    std::pair<SkBitmap, double> LoadIcon(const KURL& url, std::optional<gfx::Size> resize_dimensions = std::nullopt)
    {
        ...
        icon_loader->Start(GetDocument().GetExecutionContext(), resource_request, resize_dimensions,
            WTF::BindOnce(
                &ThreadedIconLoaderTest::DidGetIcon, WTF::Unretained(this), run_loop.QuitClosure(), WTF::Unretained(&icon), WTF::Unretained(&resize_scale)));
...
private:
    void DidGetIcon(base::OnceClosure quit_closure, SkBitmap* out_icon, double* out_resize_scale, SkBitmap icon, double resize_scale)
    {
...
{
    auto result = LoadIcon(RegisterMockedURL(kIconLoaderSVG100x100, "image/svg+xml"));
    const SkBitmap& icon = result.first;
    double resize_scale = result.second;
...
{
    auto result = LoadIcon(RegisterMockedURL(kIconLoaderInvalidIcon, "image/svg+xml"));
    const SkBitmap& icon = result.first;
    double resize_scale = result.second;
