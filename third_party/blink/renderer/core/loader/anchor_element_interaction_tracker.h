...
class BLINK_EXPORT AnchorElementInteractionTracker : public GarbageCollected<AnchorElementInteractionTracker>,
                                                     public AnchorElementViewportPositionTracker::Observer {
public:
    class BLINK_EXPORT MouseMotionEstimator : public GarbageCollected<MouseMotionEstimator> {
    public:
        MouseMotionEstimator() = delete;
        explicit MouseMotionEstimator(scoped_refptr<base::SingleThreadTaskRunner> task_runner);
        ~MouseMotionEstimator() = default;

        void Trace(Visitor* visitor) const;
        void OnTimer(TimerBase*);
        void OnMouseMoveEvent(gfx::PointF position);
        void SetTaskRunnerForTesting(scoped_refptr<base::SingleThreadTaskRunner> task_runner, const base::TickClock* clock);
        bool IsEmpty()
        {
            return mouse_position_and_timestamps_.empty();
        }
        ...
        gfx::Vector2dF GetMouseAcceleration() const
        {
            return acceleration_;
        }
        gfx::Vector2dF GetMouseVelocity() const
        {
            return velocity_;
        }
    ...
    private:
        ...
        HeapTaskRunnerTimer<AnchorElementInteractionTracker::MouseMotionEstimator> update_timer_;
    ...
    void OnClickEvent(HTMLAnchorElementBase& anchor, const MouseEvent& click_event);
...
private:
    ...
    std::array<std::optional<double>, 2> last_pointer_down_locations_ = { std::nullopt, std::nullopt };
    ...
    HeapTaskRunnerTimer<AnchorElementInteractionTracker> viewport_heuristic_timer_;
};
