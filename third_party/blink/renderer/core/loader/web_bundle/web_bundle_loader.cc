...
WebBundleLoader::WebBundleLoader(
    SubresourceWebBundle& subresource_web_bundle, Document& document, const KURL& url, network::mojom::CredentialsMode credentials_mode)
    ...
    , task_runner_(document.GetFrame()->GetTaskRunner(TaskType::kInternalLoading))
    , receivers_(this, document.GetExecutionContext())
{
    ResourceRequest request(url);
    request.SetUseStreamOnResponse(true);
    request.SetRequestContext(mojom::blink::RequestContextType::SUBRESOURCE_WEBBUNDLE);
    ...
    receivers_.Add(web_bundle_handle.InitWithNewPipeAndPassReceiver(), task_runner_);
    request.SetWebBundleTokenParams(ResourceRequestHead::WebBundleTokenParams(url_, web_bundle_token_, std::move(web_bundle_handle)));
...
void WebBundleLoader::Clone(mojo::PendingReceiver<network::mojom::blink::WebBundleHandle> receiver)
{
    receivers_.Add(std::move(receiver), task_runner_);
}

void WebBundleLoader::OnWebBundleError(network::mojom::blink::WebBundleErrorType type, const String& message)
{
    subresource_web_bundle_->OnWebBundleError(url_.ElidedString() + ": " + message);
}
