...
absl::variant<ScriptWebBundle*, ScriptWebBundleError> ScriptWebBundle::CreateOrReuseInline(ScriptElementBase& element, const String& source_text)
{
    Document& document = element.GetDocument();
    auto rule_or_error = ScriptWebBundleRule::ParseJson(source_text, document.BaseURL(), document.GetExecutionContext());
    ...
    if (!resource_fetcher) {
        return ScriptWebBundleError(ScriptWebBundleError::Type::kSystemError, "Missing resource fetcher.");
    }
    SubresourceWebBundleList* active_bundles = resource_fetcher->GetOrCreateSubresourceWebBundleList();
    if (active_bundles->GetMatchingBundle(rule.source_url())) {
        ExecutionContext* context = document.GetExecutionContext();
        if (context) {
            context->AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kOther,
                mojom::blink::ConsoleMessageLevel::kWarning, "A nested bundle is not supported: " + rule.source_url().ElidedString()));
        }
        return ScriptWebBundleError(ScriptWebBundleError::Type::kSystemError, "A nested bundle is not supported.");
    }

    if (SubresourceWebBundle* found = active_bundles->FindSubresourceWebBundleWhichWillBeReleased(rule.source_url(), rule.credentials_mode())) {
...
ScriptWebBundle::ScriptWebBundle(ScriptElementBase& element, Document& element_document, const ScriptWebBundleRule& rule)
    : element_(&element)
    , element_document_(&element_document)
    , rule_(rule)
{
...
bool ScriptWebBundle::CanHandleRequest(const KURL& url) const
{
    ...
    if (!bundle_loader_->GetSecurityOrigin()->IsSameOriginWith(SecurityOrigin::Create(url).get())) {
        OnWebBundleError(url.ElidedString() + " cannot be loaded from WebBundle " + bundle_loader_->url().ElidedString()
...
bool ScriptWebBundle::IsSameOriginBundle() const
{
    ...
    const SecurityOrigin* frame_security_origin = element_document_->GetFrame()->GetSecurityContext()->GetSecurityOrigin();
    auto bundle_origin = SecurityOrigin::Create(rule_.source_url());
    return frame_security_origin && frame_security_origin->IsSameOriginWith(bundle_origin.get());
}
...
void ScriptWebBundle::CreateBundleLoaderAndRegister()
{
    DCHECK(!bundle_loader_);
    DCHECK(element_document_);
    bundle_loader_ = MakeGarbageCollected<WebBundleLoader>(*this, *element_document_, rule_.source_url(), rule_.credentials_mode());
...
void ScriptWebBundle::WillReleaseBundleLoaderAndUnregister()
{
    ...
    if (element_document_) {
        ...
        element_document_->GetAgent().event_loop()->EnqueueMicrotask(WTF::BindOnce(&ReleaseResourceTask::Run, std::move(task)));
    } else {
...
void ScriptWebBundle::ReusedWith(ScriptElementBase& element, ScriptWebBundleRule rule)
{
    ...
    if (bundle_loader_->HasLoaded()) {
        element_document_->GetTaskRunner(TaskType::kDOMManipulation)
            ->PostTask(FROM_HERE, WTF::BindOnce(&ScriptElementBase::DispatchLoadEvent, WrapPersistent(element_.Get())));
    } else if (bundle_loader_->HasFailed()) {
        element_document_->GetTaskRunner(TaskType::kDOMManipulation)
