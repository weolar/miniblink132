...
const char* const kKnownKeys[] = { kSourceKey, kCredentialsKey, kScopesKey, kResourcesKey };
...
absl::variant<ScriptWebBundleRule, ScriptWebBundleError> ScriptWebBundleRule::ParseJson(const String& inline_text, const KURL& base_url, ConsoleLogger* logger)
{
    std::unique_ptr<JSONValue> json = ParseJSON(inline_text);
    if (!json) {
        return ScriptWebBundleError(ScriptWebBundleError::Type::kSyntaxError, "Failed to parse web bundle rule: invalid JSON.");
    }
    std::unique_ptr<JSONObject> json_obj = JSONObject::From(std::move(json));
    if (!json_obj) {
        return ScriptWebBundleError(ScriptWebBundleError::Type::kTypeError, "Failed to parse web bundle rule: not an object.");
    }
    ...
    if (logger) {
        for (wtf_size_t i = 0; i < json_obj->size(); ++i) {
            ...
            if (!base::Contains(kKnownKeys, entry.first)) {
                logger->AddConsoleMessage(mojom::blink::ConsoleMessageSource::kOther, mojom::blink::ConsoleMessageLevel::kWarning,
    ...
    if (scopes && scopes->GetType() != JSONValue::kTypeArray) {
        return ScriptWebBundleError(ScriptWebBundleError::Type::kTypeError, "Failed to parse web bundle rule: \"scopes\" must be an array.");
    }
    JSONValue* resources = json_obj->Get(kResourcesKey);
    if (resources && resources->GetType() != JSONValue::kTypeArray) {
        return ScriptWebBundleError(ScriptWebBundleError::Type::kTypeError, "Failed to parse web bundle rule: \"resources\" must be an array.");
    }

    HashSet<KURL> scope_urls = ParseJSONArrayAsURLs(JSONArray::Cast(scopes), source_url);
    HashSet<KURL> resource_urls = ParseJSONArrayAsURLs(JSONArray::Cast(resources), source_url);

    return ScriptWebBundleRule(source_url, credentials_mode, std::move(scope_urls), std::move(resource_urls));
}

ScriptWebBundleRule::ScriptWebBundleRule(
    const KURL& source_url, network::mojom::CredentialsMode credentials_mode, HashSet<KURL> scope_urls, HashSet<KURL> resource_urls)
