...
private:
    void AddConsoleMessageImpl(mojom::ConsoleMessageSource, mojom::ConsoleMessageLevel, const String& message, bool discard_duplicates,
...
{
    auto result = ScriptWebBundleRule::ParseJson("", KURL("https://example.com/"), nullptr);
    ...
    EXPECT_EQ(error.GetMessage(), "Failed to parse web bundle rule: invalid JSON.");
}
...
TEST(ScriptWebBundleRuleTest, Basic)
{
    ...
    EXPECT_THAT(rule.scope_urls(), testing::UnorderedElementsAre("https://example.com/js"));
    EXPECT_THAT(rule.resource_urls(), testing::UnorderedElementsAre("https://example.com/dir/a.css", "https://example.com/dir/b.css"));
}
...
TEST(ScriptWebBundleRuleTest, ResourcesShouldBeResolvedOnBundleURL)
{
    ...
    EXPECT_THAT(rule.resource_urls(), testing::UnorderedElementsAre("https://example.com/hello/dir/a.css"));
}
...
TEST(ScriptWebBundleRuleTest, ScopesShouldBeResolvedOnBundleURL)
{
    ...
    auto& rule = absl::get<ScriptWebBundleRule>(result);
    EXPECT_EQ(rule.source_url(), "https://example.com/hello/foo.wbn");
    EXPECT_THAT(rule.scope_urls(), testing::UnorderedElementsAre("https://example.com/hello/js"));
}
...
TEST(ScriptWebBundleRuleTest, CredentialsDefaultIsSameOrigin)
{
    ...
    auto& rule = absl::get<ScriptWebBundleRule>(result);
    EXPECT_EQ(rule.source_url(), "https://example.com/foo.wbn");
    EXPECT_EQ(rule.credentials_mode(), network::mojom::CredentialsMode::kSameOrigin);
}
...
TEST(ScriptWebBundleRuleTest, TopLevelIsNotAnObject)
{
    ...
    EXPECT_EQ(error.GetMessage(), "Failed to parse web bundle rule: not an object.");
}
...
{
    const KURL base_url("https://example.com/");
    auto result = ScriptWebBundleRule::ParseJson(R"({"source": 123})", base_url, nullptr);
    ASSERT_TRUE(absl::holds_alternative<ScriptWebBundleError>(result));
    auto& error = absl::get<ScriptWebBundleError>(result);
    EXPECT_EQ(error.GetType(), ScriptWebBundleError::Type::kTypeError);
    EXPECT_EQ(error.GetMessage(),
...
{
    const KURL base_url("https://example.com/");
    auto result = ScriptWebBundleRule::ParseJson(R"({"source": "http://"})", base_url, nullptr);
    ASSERT_TRUE(absl::holds_alternative<ScriptWebBundleError>(result));
    auto& error = absl::get<ScriptWebBundleError>(result);
    EXPECT_EQ(error.GetType(), ScriptWebBundleError::Type::kTypeError);
    EXPECT_EQ(error.GetMessage(),
...
TEST(ScriptWebBundleRuleTest, InvalidScopesType)
{
    ...
    auto& error = absl::get<ScriptWebBundleError>(result);
    EXPECT_EQ(error.GetType(), ScriptWebBundleError::Type::kTypeError);
    EXPECT_EQ(error.GetMessage(), "Failed to parse web bundle rule: \"scopes\" must be an array.");
}
...
TEST(ScriptWebBundleRuleTest, InvalidResourcesType)
{
    ...
    auto& error = absl::get<ScriptWebBundleError>(result);
    EXPECT_EQ(error.GetType(), ScriptWebBundleError::Type::kTypeError);
    EXPECT_EQ(error.GetMessage(), "Failed to parse web bundle rule: \"resources\" must be an array.");
}
...
TEST(ScriptWebBundleRuleTest, UnknownKey)
{
    ...
    EXPECT_TRUE(rule.scope_urls().empty());
    EXPECT_TRUE(rule.resource_urls().empty());
    EXPECT_EQ(logger->Message(), "Invalid top-level key \"unknown\" in WebBundle rule.");
}
