...
class CORE_EXPORT ScriptWebBundle final : public GarbageCollected<ScriptWebBundle>, public SubresourceWebBundle {
public:
    static absl::variant<ScriptWebBundle*, ScriptWebBundleError> CreateOrReuseInline(ScriptElementBase&, const String& inline_text);

    ScriptWebBundle(ScriptElementBase& element, Document& element_document, const ScriptWebBundleRule& rule);
...
template <> struct DowncastTraits<ScriptWebBundle> {
