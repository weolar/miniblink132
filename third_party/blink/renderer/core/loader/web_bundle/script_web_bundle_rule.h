...
public:
    static absl::variant<ScriptWebBundleRule, ScriptWebBundleError> ParseJson(const String& inline_text, const KURL& base_url, ConsoleLogger* logger);

    ScriptWebBundleRule(const KURL& source_url, network::mojom::CredentialsMode credentialsl_mode, HashSet<KURL> scope_urls, HashSet<KURL> resource_urls);

    bool ResourcesOrScopesMatch(const KURL& url) const;
    const KURL& source_url() const
    {
        return source_url_;
    }
    const HashSet<KURL>& scope_urls() const
    {
        return scope_urls_;
    }
    const HashSet<KURL>& resource_urls() const
    {
