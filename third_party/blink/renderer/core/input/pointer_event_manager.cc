...
const char kSkipTouchEventFilterTrialProcessParamName[] = "skip_filtering_process";
...
{
    return registry.HasEventHandlers(EventHandlerRegistry::kPointerEvent) || registry.HasEventHandlers(EventHandlerRegistry::kPointerRawUpdateEvent);
}

const AtomicString& MouseEventNameForPointerEventInputType(const WebInputEvent::Type& event_type)
{
    switch (event_type) {
...
PointerEventManager::PointerEventManager(LocalFrame& frame, MouseEventManager& mouse_event_manager)
    : frame_(frame)
    ...
    , mouse_event_manager_(mouse_event_manager)
{
    ...
    if (RuntimeEnabledFeatures::SkipTouchEventFilterEnabled()
        && base::GetFieldTrialParamValue(kSkipTouchEventFilterTrial, kSkipTouchEventFilterTrialProcessParamName) == "browser_and_renderer") {
        skip_touch_filter_discrete_ = true;
        if (base::GetFieldTrialParamValue(kSkipTouchEventFilterTrial, kSkipTouchEventFilterTrialTypeParamName) == "all") {
...
PointerEventManager::PointerEventBoundaryEventDispatcher::PointerEventBoundaryEventDispatcher(
    PointerEventManager* pointer_event_manager, PointerEvent* pointer_event)
    : BoundaryEventDispatcher(event_type_names::kPointerover, event_type_names::kPointerout, event_type_names::kPointerenter, event_type_names::kPointerleave)
...
void PointerEventManager::PointerEventBoundaryEventDispatcher::Dispatch(
    EventTarget* target, EventTarget* related_target, const AtomicString& type, bool check_for_listener)
{
    pointer_event_manager_->DispatchPointerEvent(
        target, pointer_event_manager_->pointer_event_factory_.CreatePointerBoundaryEvent(pointer_event_, type, related_target), check_for_listener);
}

WebInputEventResult PointerEventManager::DispatchPointerEvent(EventTarget* target, PointerEvent* pointer_event, bool check_for_listener)
{
    if (!target)
    ...
    if (frame_ && frame_->DomWindow()) {
        event_timing = EventTiming::Create(frame_->DomWindow(), *pointer_event, target);
    }

    if (event_type == event_type_names::kPointerdown || event_type == event_type_names::kPointerover || event_type == event_type_names::kPointerout) {
    ...
    if (Node* target_node = target->ToNode()) {
        if (event_type == event_type_names::kPointerdown || event_type == event_type_names::kPointerup) {
    ...
    if (should_filter && !HasPointerEventListener(frame_->GetEventHandlerRegistry()))
        return WebInputEventResult::kNotHandled;

    if (event_type == event_type_names::kPointerdown) {
        auto* html_canvas_element = DynamicTo<HTMLCanvasElement>(target->ToNode());
        if (html_canvas_element && html_canvas_element->NeedsUnbufferedInputEvents()) {
            frame_->GetChromeClient().RequestUnbufferedInputEvents(frame_);
        }
    }

    bool listeners_exist = !check_for_listener || target->HasEventListeners(event_type);
    if (listeners_exist) {
        UseCounter::Count(frame_->GetDocument(), WebFeature::kPointerEventDispatch);
        if (event_type == event_type_names::kPointerdown) {
            UseCounter::Count(frame_->GetDocument(), WebFeature::kPointerEventDispatchPointerDown);
        }
    }

    if (!should_filter || listeners_exist) {
        DCHECK(!dispatching_pointer_id_);
        base::AutoReset<PointerId> dispatch_holder(&dispatching_pointer_id_, pointer_id);
...
Element* PointerEventManager::GetEffectiveTargetForPointerEvent(Element* target, PointerId pointer_id)
{
...
void PointerEventManager::SendMouseAndPointerBoundaryEvents(Element* entered_element, const WebMouseEvent& mouse_event)
{
    ...
    const WebPointerEvent web_pointer_event(WebInputEvent::Type::kPointerMove, mouse_event);
    PointerEvent* dummy_pointer_event
        = pointer_event_factory_.Create(web_pointer_event, Vector<WebPointerEvent>(), Vector<WebPointerEvent>(), frame_->GetDocument()->domWindow());
    ...
    if (dummy_pointer_event->buttons() == 0 && dummy_pointer_event->isPrimary()) {
        prevent_mouse_event_for_pointer_type_[ToPointerTypeIndex(mouse_event.pointer_type)] = false;
    }

    ProcessCaptureAndPositionOfPointerEvent(dummy_pointer_event, entered_element, &mouse_event);
}

void PointerEventManager::SendBoundaryEvents(
    EventTarget* exited_target, bool original_exited_target_removed, EventTarget* entered_target, PointerEvent* pointer_event)
{
    PointerEventBoundaryEventDispatcher boundary_event_dispatcher(this, pointer_event);
    boundary_event_dispatcher.SendBoundaryEvents(exited_target, original_exited_target_removed, entered_target);
}

void PointerEventManager::SetElementUnderPointer(PointerEvent* pointer_event, Element* target)
{
    const PointerId pointer_id = pointer_event->pointerId();

    CHECK(!original_element_under_pointer_removed_.Contains(pointer_id) || RuntimeEnabledFeatures::BoundaryEventDispatchTracksNodeRemovalEnabled());

    Element* exited_target = element_under_pointer_.Contains(pointer_id) ? element_under_pointer_.at(pointer_id) : nullptr;
    bool original_exited_target_removed = original_element_under_pointer_removed_.Contains(pointer_id);
    ...
    SendBoundaryEvents(exited_target, original_exited_target_removed, target, pointer_event);
}
...
void PointerEventManager::NodeWillBeRemoved(Node& node_to_be_removed)
{
    ...
    for (const auto& [pointer_id, element] : element_under_pointer_) {
        if (element && node_to_be_removed.IsShadowIncludingInclusiveAncestorOf(*element)) {
            element_under_pointer_.Set(pointer_id, node_to_be_removed.parentElement());
...
void PointerEventManager::HandlePointerInterruption(const WebPointerEvent& web_pointer_event)
{
    DCHECK(web_pointer_event.GetType() == WebInputEvent::Type::kPointerCausedUaAction);

    HeapVector<Member<PointerEvent>> canceled_pointer_events;
    if (web_pointer_event.pointer_type == WebPointerProperties::PointerType::kMouse) {
        canceled_pointer_events.push_back(
            pointer_event_factory_.CreatePointerCancelEvent(PointerEventFactory::kMouseId, web_pointer_event.TimeStamp(), web_pointer_event.device_id));
    } else {
        ...
        if (!non_hovering_pointers_canceled_) {
            Vector<PointerId> non_hovering_pointer_ids = pointer_event_factory_.GetPointerIdsOfNonHoveringPointers();

            for (PointerId pointer_id : non_hovering_pointer_ids) {
                canceled_pointer_events.push_back(
                    pointer_event_factory_.CreatePointerCancelEvent(pointer_id, web_pointer_event.TimeStamp(), web_pointer_event.device_id));
            }
    ...
    for (auto pointer_event : canceled_pointer_events) {
        ...
        DispatchPointerEvent(GetEffectiveTargetForPointerEvent(target, pointer_event->pointerId()), pointer_event);
...
bool PointerEventManager::ShouldAdjustPointerEvent(const WebPointerEvent& pointer_event) const
{
    return (pointer_event.pointer_type == WebPointerProperties::PointerType::kTouch || ShouldAdjustStylusPointerEvent(pointer_event))
        && pointer_event.GetType() == WebInputEvent::Type::kPointerDown && pointer_event_factory_.IsPrimary(pointer_event);
}

bool PointerEventManager::ShouldAdjustStylusPointerEvent(const WebPointerEvent& pointer_event) const
{
    return base::FeatureList::IsEnabled(blink::features::kStylusPointerAdjustment)
        && (pointer_event.pointer_type == WebPointerProperties::PointerType::kPen || pointer_event.pointer_type == WebPointerProperties::PointerType::kEraser);
}

void PointerEventManager::AdjustPointerEvent(WebPointerEvent& pointer_event)
{
    DCHECK(pointer_event.pointer_type == WebPointerProperties::PointerType::kTouch || pointer_event.pointer_type == WebPointerProperties::PointerType::kPen
...
void PointerEventManager::AdjustPointerEvent(WebPointerEvent& pointer_event, Node*& adjusted_node)
{
    ...
    if (pointer_event.pointer_type == WebPointerProperties::PointerType::kTouch) {
        ...
        float device_scale_factor = chrome_client.GetScreenInfo(*frame_).device_scale_factor;

        float page_scale_factor = frame_->GetPage()->PageScaleFactor();
        adjustment_width = adjustment_height = kStylusWritableAdjustmentSizeDip * (device_scale_factor / page_scale_factor);
    }

    PhysicalSize hit_rect_size = GetHitTestRectForAdjustment(*frame_, PhysicalSize(LayoutUnit(adjustment_width), LayoutUnit(adjustment_height)));

    if (hit_rect_size.IsEmpty())
        return;

    HitTestRequest::HitTestRequestType hit_type
        = HitTestRequest::kTouchEvent | HitTestRequest::kReadOnly | HitTestRequest::kActive | HitTestRequest::kListBased;
    LocalFrame& root_frame = frame_->LocalFrameRoot();
    // TODO(szager): Shouldn't this be PositionInScreen() ?
    PhysicalOffset hit_test_point = PhysicalOffset::FromPointFRound(pointer_event.PositionInWidget());
    hit_test_point -= PhysicalOffset(LayoutUnit(hit_rect_size.width * 0.5f), LayoutUnit(hit_rect_size.height * 0.5f));
    HitTestLocation location(PhysicalRect(hit_test_point, hit_rect_size));
    HitTestResult hit_test_result = root_frame.GetEventHandler().HitTestResultAtLocation(location, hit_type);
    gfx::Point adjusted_point;

    if (pointer_event.pointer_type == WebPointerProperties::PointerType::kTouch) {
        bool adjusted = frame_->GetEventHandler().BestNodeForHitTestResult(
            TouchAdjustmentCandidateType::kClickable, location, hit_test_result, adjusted_point, adjusted_node);

        if (adjusted)
            pointer_event.SetPositionInWidget(adjusted_point.x(), adjusted_point.y());

        frame_->GetEventHandler().CacheTouchAdjustmentResult(pointer_event.unique_touch_event_id, pointer_event.PositionInWidget());
    } else if (pointer_event.pointer_type == WebPointerProperties::PointerType::kPen
        || pointer_event.pointer_type == WebPointerProperties::PointerType::kEraser) {
        ...
        bool adjusted = frame_->GetEventHandler().BestNodeForHitTestResult(
            TouchAdjustmentCandidateType::kStylusWritable, location, hit_test_result, adjusted_point, adjusted_node);
...
bool PointerEventManager::ShouldFilterEvent(PointerEvent* pointer_event)
{
    ...
    // filter.
    if (pointer_event->type() == event_type_names::kPointerdown || pointer_event->type() == event_type_names::kPointerup) {
        return false;
    }
    // If the experiment is "all", do not filter pointermove.
    if (skip_touch_filter_all_ && pointer_event->type() == event_type_names::kPointermove)
        return false;
...
event_handling_util::PointerEventTarget PointerEventManager::ComputePointerEventTarget(const WebPointerEvent& web_pointer_event)
{
    event_handling_util::PointerEventTarget pointer_event_target;

    PointerId pointer_id = pointer_event_factory_.GetPointerEventId(web_pointer_event);
    ...
    if (web_pointer_event.GetType() == WebInputEvent::Type::kPointerDown || !pending_pointer_capture_target_.Contains(pointer_id)) {
        HitTestRequest::HitTestRequestType hit_type = HitTestRequest::kTouchEvent | HitTestRequest::kReadOnly | HitTestRequest::kActive;
        HitTestLocation location(frame_->View()->ConvertFromRootFrame(PhysicalOffset::FromPointFRound(web_pointer_event.PositionInWidget())));
        HitTestResult hit_test_result = frame_->GetEventHandler().HitTestResultAtLocation(location, hit_type);
        ...
        pointer_event_target.target_element = pending_pointer_capture_target_.at(pointer_id);
        pointer_event_target.target_frame = pointer_event_target.target_element->GetDocument().GetFrame();
    }
    return pointer_event_target;
}

WebInputEventResult PointerEventManager::DispatchTouchPointerEvent(const WebPointerEvent& web_pointer_event, const Vector<WebPointerEvent>& coalesced_events,
    const Vector<WebPointerEvent>& predicted_events, const event_handling_util::PointerEventTarget& pointer_event_target)
{
    DCHECK_NE(web_pointer_event.GetType(), WebInputEvent::Type::kPointerCausedUaAction);

    WebInputEventResult result = WebInputEventResult::kHandledSystem;
    if (pointer_event_target.target_element && pointer_event_target.target_frame && !non_hovering_pointers_canceled_) {
        SetLastPointerPositionForFrameBoundary(web_pointer_event, pointer_event_target.target_element);

        PointerEvent* pointer_event = pointer_event_factory_.Create(web_pointer_event, coalesced_events, predicted_events,
            pointer_event_target.target_element ? pointer_event_target.target_element->GetDocument().domWindow() : nullptr);

        if (pointer_event) {
            result = SendTouchPointerEvent(pointer_event_target.target_element, pointer_event, web_pointer_event.hovering);
        } else {
        ...
        if (result != WebInputEventResult::kNotHandled && pointer_event->type() == event_type_names::kPointerdown && pointer_event->isPrimary()) {
            touch_ids_for_canceled_pointerdowns_.push_back(web_pointer_event.unique_touch_event_id);
        }
    }
    return result;
}

WebInputEventResult PointerEventManager::SendTouchPointerEvent(Element* target, PointerEvent* pointer_event, bool hovering)
{
    if (non_hovering_pointers_canceled_)
        return WebInputEventResult::kNotHandled;
    ...
    WebInputEventResult result = DispatchPointerEvent(GetEffectiveTargetForPointerEvent(target, pointer_event->pointerId()), pointer_event);

    if (pointer_event->type() == event_type_names::kPointerup || pointer_event->type() == event_type_names::kPointercancel) {
        ReleasePointerCapture(pointer_event->pointerId());
...
WebInputEventResult PointerEventManager::HandlePointerEvent(
    const WebPointerEvent& event, const Vector<WebPointerEvent>& coalesced_events, const Vector<WebPointerEvent>& predicted_events)
{
    if (event.GetType() == WebInputEvent::Type::kPointerRawUpdate) {
        if (!frame_->GetEventHandlerRegistry().HasEventHandlers(EventHandlerRegistry::kPointerRawUpdateEvent))
        ...
        Node* target;
        Element* pointer_locked_element = PointerLockController::GetPointerLockedElement(frame_);
        if (pointer_locked_element && event.pointer_type == WebPointerProperties::PointerType::kMouse) {
            // The locked element could be in another frame. So we need to delegate
            // sending the event to that frame.
            LocalFrame* target_frame = pointer_locked_element->GetDocument().GetFrame();
            if (!target_frame)
                return WebInputEventResult::kHandledSystem;
            if (target_frame != frame_) {
                target_frame->GetEventHandler().HandlePointerEvent(event, coalesced_events, predicted_events);
                return WebInputEventResult::kHandledSystem;
            }
        ...
        PointerEvent* pointer_event = pointer_event_factory_.Create(event, coalesced_events, predicted_events, frame_->GetDocument()->domWindow());
    ...
    event_handling_util::PointerEventTarget pointer_event_target = ComputePointerEventTarget(pointer_event);

    bool is_pointer_down = event.GetType() == WebInputEvent::Type::kPointerDown;
    if (is_pointer_down && discarded_event_.target != kInvalidDOMNodeId && discarded_event_.target == pointer_event_target.target_element->GetDomNodeId()
        && pointer_event.TimeStamp() - discarded_event_.time < event_handling_util::kDiscardedEventMistakeInterval) {
        pointer_event_target.target_element->GetDocument().CountUse(WebFeature::kInputEventToRecentlyMovedIframeMistakenlyDiscarded);
    }
    bool discard = pointer_event_target.target_frame && event_handling_util::ShouldDiscardEventTargetingFrame(event, *pointer_event_target.target_frame);
    if (discard) {
        if (is_pointer_down) {
            discarded_event_.target = pointer_event_target.target_element->GetDomNodeId();
            discarded_event_.time = pointer_event.TimeStamp();
        }
        PointerEvent* core_pointer_event = pointer_event_factory_.Create(event, coalesced_events, predicted_events,
            pointer_event_target.target_element ? pointer_event_target.target_element->GetDocument().domWindow() : nullptr);
        if (core_pointer_event) {
            // TODO(crbug.com/1141595): We should handle this case further upstream.
            SendTouchPointerEvent(pointer_event_target.target_element,
                pointer_event_factory_.CreatePointerCancelEvent(core_pointer_event->pointerId(), event.TimeStamp(), core_pointer_event->persistentDeviceId()),
                event.hovering);
        }
        ...
        touch_event_manager_->HandleTouchPoint(pointer_cancel_event, coalesced_events, pointer_event_target);
    ...
    if (event.GetType() == WebInputEvent::Type::kPointerUp && !non_hovering_pointers_canceled_ && pointer_event_target.target_frame) {
        LocalFrame::NotifyUserActivation(pointer_event_target.target_frame, mojom::blink::UserActivationNotificationType::kInteraction);
    }
    ...
    if (is_pointer_down) {
        pointerdown_node = touch_event_manager_->GetTouchPointerNode(event, pointer_event_target);
    }

    if (pointerdown_node) {
        TouchAction touch_action = touch_action_util::EffectiveTouchActionAtPointerDown(event, pointerdown_node);
        touch_event_manager_->UpdateTouchAttributeMapsForPointerDown(event, pointerdown_node, touch_action);
    }

    WebInputEventResult result = DispatchTouchPointerEvent(event, coalesced_events, predicted_events, pointer_event_target);

    touch_event_manager_->HandleTouchPoint(event, coalesced_events, pointer_event_target);

    return result;
}

bool PointerEventManager::HandleScrollbarTouchDrag(const WebPointerEvent& event, Scrollbar* scrollbar)
{
    if (!scrollbar || (event.pointer_type != WebPointerProperties::PointerType::kTouch && event.pointer_type != WebPointerProperties::PointerType::kPen)) {
        return false;
    }

    if (event.GetType() == WebInputEvent::Type::kPointerDown) {
        captured_scrollbar_ = scrollbar;
        frame_->GetPage()->GetChromeClient().SetTouchAction(frame_, TouchAction::kNone);
    }
...
bool PointerEventManager::HandleResizerDrag(const WebPointerEvent& event, const event_handling_util::PointerEventTarget& pointer_event_target)
{
    switch (event.GetType()) {
    case WebPointerEvent::Type::kPointerDown: {
        Node* node = pointer_event_target.target_element;
        if (!node || !node->GetLayoutObject() || !node->GetLayoutObject()->EnclosingLayer())
            return false;
        ...
        gfx::Point p = pointer_event_target.target_frame->View()->ConvertFromRootFrame(gfx::ToFlooredPoint(event.PositionInWidget()));
        if (layer->GetScrollableArea()->IsAbsolutePointInResizeControl(p, kResizerForTouch)) {
            resize_scrollable_area_ = layer->GetScrollableArea();
            resize_scrollable_area_->SetInResizeMode(true);
            frame_->GetPage()->GetChromeClient().SetTouchAction(frame_, TouchAction::kNone);
            offset_from_resize_corner_ = resize_scrollable_area_->OffsetFromResizeCorner(p);
            return true;
        }
        break;
    }
    case WebInputEvent::Type::kPointerMove: {
        if (resize_scrollable_area_ && resize_scrollable_area_->Layer() && resize_scrollable_area_->Layer()->GetLayoutBox()
...
WebInputEventResult PointerEventManager::CreateAndDispatchPointerEvent(Element* target, const AtomicString& mouse_event_name, const WebMouseEvent& mouse_event,
    const Vector<WebMouseEvent>& coalesced_events, const Vector<WebMouseEvent>& predicted_events)
{
    ...
    PointerEvent* pointer_event
        = pointer_event_factory_.Create(web_pointer_event, pointer_coalesced_events, pointer_predicted_events, target->GetDocument().domWindow());
...
WebInputEventResult PointerEventManager::DirectDispatchMousePointerEvent(Element* target, const WebMouseEvent& event, const AtomicString& mouse_event_type,
    const Vector<WebMouseEvent>& coalesced_events, const Vector<WebMouseEvent>& predicted_events)
{
    if (!(event.GetModifiers() & WebInputEvent::Modifiers::kRelativeMotionEvent)) {
        // Fetch the last_mouse_position for creating MouseEvent before
        // pointer_event_factory updates it.
        gfx::PointF last_mouse_position = pointer_event_factory_.GetLastPointerPosition(PointerEventFactory::kMouseId, event, event.GetType());

        WebInputEventResult result = CreateAndDispatchPointerEvent(target, mouse_event_type, event, coalesced_events, predicted_events);

        result = event_handling_util::MergeEventResult(
            result, mouse_event_manager_->DispatchMouseEvent(target, mouse_event_type, event, &last_mouse_position, nullptr));
        return result;
    }
    pointer_event_factory_.SetLastPosition(pointer_event_factory_.GetPointerEventId(event), event.PositionInScreen(), event.GetType());

    return WebInputEventResult::kHandledSuppressed;
}

void PointerEventManager::SendEffectivePanActionAtPointer(const WebPointerEvent& event, const Node* node_at_pointer)
{
    if (IsAnyTouchActive())
        return;
    ...
    if (ShouldAdjustStylusPointerEvent(event)) {
        ...
        WebPointerEvent& cloned_pointer_event = static_cast<WebPointerEvent&>(*cloned_event);
    ...
    if (node_at_pointer) {
        effective_touch_action = touch_action_util::EffectiveTouchActionAtPointer(event, node_at_pointer);
    }
    ...
    } else if ((effective_touch_action & TouchAction::kInternalNotWritable) != TouchAction::kInternalNotWritable) {
    ...
    } else if ((effective_touch_action & TouchAction::kInternalPanXScrolls) != TouchAction::kInternalPanXScrolls) {
...
Element* NonDeletedElementTarget(Element* target, PointerEvent* dispatched_pointer_event)
{
    ...
    if (!event_handling_util::IsInDocument(target) && has_event_path) {
        for (const auto& context : dispatched_pointer_event->GetEventPath().NodeEventContexts()) {
...
WebInputEventResult PointerEventManager::SendMousePointerEvent(Element* target, const WebInputEvent::Type event_type, const WebMouseEvent& mouse_event,
    const Vector<WebMouseEvent>& coalesced_events, const Vector<WebMouseEvent>& predicted_events, bool skip_click_dispatch)
{
    DCHECK(event_type == WebInputEvent::Type::kPointerDown || event_type == WebInputEvent::Type::kPointerMove || event_type == WebInputEvent::Type::kPointerUp);
    ...
    gfx::PointF last_mouse_position
        = pointer_event_factory_.GetLastPointerPosition(pointer_event_factory_.GetPointerEventId(mouse_event), mouse_event, event_type);

    bool fake_event = (web_pointer_event.GetModifiers() & WebInputEvent::Modifiers::kRelativeMotionEvent);

    // Fake events should only be move events.
    DCHECK(!fake_event || event_type == WebInputEvent::Type::kPointerMove);

    PointerEvent* pointer_event
        = pointer_event_factory_.Create(web_pointer_event, pointer_coalesced_events, pointer_predicted_events, frame_->GetDocument()->domWindow());
    DCHECK(pointer_event);

    // This is for when the mouse is released outside of the page.
    if (!fake_event && event_type == WebInputEvent::Type::kPointerMove && !pointer_event->buttons()) {
        ReleasePointerCapture(pointer_event->pointerId());
        // Send got/lostpointercapture rightaway if necessary.
        ProcessPendingPointerCapture(pointer_event);

        if (pointer_event->isPrimary()) {
            prevent_mouse_event_for_pointer_type_[ToPointerTypeIndex(web_pointer_event.pointer_type)] = false;
        }
    }
    ...
    // boundary MouseEvent.
    Element* effective_target = ProcessCaptureAndPositionOfPointerEvent(pointer_event, target, &mouse_event);

    // Don't send fake mouse event to the DOM.
    if (fake_event)
        return WebInputEventResult::kHandledSuppressed;

    if ((event_type == WebInputEvent::Type::kPointerDown || event_type == WebInputEvent::Type::kPointerUp)
        && pointer_event->type() == event_type_names::kPointermove
        && frame_->GetEventHandlerRegistry().HasEventHandlers(EventHandlerRegistry::kPointerRawUpdateEvent)) {
        // This is a chorded button move event. We need to also send a
        // pointerrawupdate for it.
        DispatchPointerEvent(effective_target, pointer_event_factory_.CreatePointerRawUpdateEvent(pointer_event));
    }

    WebInputEventResult result = DispatchPointerEvent(effective_target, pointer_event);

    if (result != WebInputEventResult::kNotHandled && pointer_event->type() == event_type_names::kPointerdown && pointer_event->isPrimary()) {
        prevent_mouse_event_for_pointer_type_[ToPointerTypeIndex(mouse_event.pointer_type)] = true;
    }

    bool send_compat_mouse = pointer_event->isPrimary() && !prevent_mouse_event_for_pointer_type_[ToPointerTypeIndex(mouse_event.pointer_type)];
    bool consider_click_dispatch = !skip_click_dispatch && pointer_event->isPrimary() && event_type == WebInputEvent::Type::kPointerUp;
    ...
    if (send_compat_mouse || consider_click_dispatch) {
        mouse_target = RuntimeEnabledFeatures::BoundaryEventDispatchTracksNodeRemovalEnabled() ? mouse_event_manager_->GetElementUnderMouse()
                                                                                               : NonDeletedElementTarget(effective_target, pointer_event);
    }

    // Dispatch compat mouse events.
    if (send_compat_mouse) {
        result = event_handling_util::MergeEventResult(result,
            mouse_event_manager_->DispatchMouseEvent(
                mouse_target, MouseEventNameForPointerEventInputType(event_type), mouse_event, &last_mouse_position, nullptr));
    }
    ...
    if (consider_click_dispatch) {
        ...
        captured_click_target = GetEffectiveTargetForPointerEvent(nullptr, pointer_event->pointerId());
        ...
        if (!RuntimeEnabledFeatures::ClickToCapturedPointerEnabled()) {
            mouse_event_manager_->DispatchMouseClickIfNeeded(
                mouse_target, captured_click_target, mouse_event, pointer_event->pointerId(), pointer_event->pointerType());
        }
    }
    ...
    if (pointer_event->type() == event_type_names::kPointerup || pointer_event->type() == event_type_names::kPointercancel) {
        ...
        if (pointer_event->type() == event_type_names::kPointerup) {
            ...
            if (pointer_capture_target_.find(pointer_event->pointerId()) != pointer_capture_target_.end()) {
                HitTestRequest::HitTestRequestType hit_type = HitTestRequest::kRelease;
                HitTestRequest request(hit_type);
                MouseEventWithHitTestResults mev = event_handling_util::PerformMouseEventHitTest(frame_, request, mouse_event);
                target = mev.InnerElement();
            } else if (RuntimeEnabledFeatures::BoundaryEventDispatchTracksNodeRemovalEnabled()) {
                target = NonDeletedElementTarget(target, pointer_event);
            }

            // Dispatch the click event if applicable, when the flag is enabled.
            if (consider_click_dispatch && RuntimeEnabledFeatures::ClickToCapturedPointerEnabled()) {
                ProcessPendingPointerCapture(pointer_event);
                mouse_event_manager_->DispatchMouseClickIfNeeded(
                    mouse_target, captured_click_target, mouse_event, pointer_event->pointerId(), pointer_event->pointerType());
    ...
    if (mouse_event.GetType() == WebInputEvent::Type::kMouseLeave && mouse_event.pointer_type == WebPointerProperties::PointerType::kPen) {
        pointer_event_factory_.Remove(pointer_event->pointerId());
    }
    return result;
}

bool PointerEventManager::GetPointerCaptureState(PointerId pointer_id, Element** pointer_capture_target, Element** pending_pointer_capture_target)
{
    ...
    Element* pointer_capture_target_temp = (it != pointer_capture_target_.end()) ? it->value : nullptr;
    it = pending_pointer_capture_target_.find(pointer_id);
    Element* pending_pointercapture_target_temp = (it != pending_pointer_capture_target_.end()) ? it->value : nullptr;
...
Element* PointerEventManager::ProcessCaptureAndPositionOfPointerEvent(PointerEvent* pointer_event, Element* hit_test_target, const WebMouseEvent* mouse_event)
{
    ProcessPendingPointerCapture(pointer_event);

    Element* effective_target = GetEffectiveTargetForPointerEvent(hit_test_target, pointer_event->pointerId());
...
void PointerEventManager::ProcessPendingPointerCapture(PointerEvent* pointer_event)
{
    Element* pointer_capture_target = nullptr;
    Element* pending_pointer_capture_target = nullptr;

    const PointerId pointer_id = pointer_event->pointerId();
    const bool is_capture_changed = GetPointerCaptureState(pointer_id, &pointer_capture_target, &pending_pointer_capture_target);
    ...
    if (pointer_capture_target) {
        ...
        DispatchPointerEvent(target, pointer_event_factory_.CreatePointerCaptureEvent(pointer_event, event_type_names::kLostpointercapture));
    }

    if (pending_pointer_capture_target && pending_pointer_capture_target->isConnected()) {
        SetElementUnderPointer(pointer_event, pending_pointer_capture_target);
        DispatchPointerEvent(
            pending_pointer_capture_target, pointer_event_factory_.CreatePointerCaptureEvent(pointer_event, event_type_names::kGotpointercapture));
        if (pending_pointer_capture_target->isConnected()) {
            ...
            // removed.
            DispatchPointerEvent(pending_pointer_capture_target->ownerDocument(),
                pointer_event_factory_.CreatePointerCaptureEvent(pointer_event, event_type_names::kLostpointercapture));
        }
    }
}

void PointerEventManager::RemoveTargetFromPointerCapturingMapping(PointerCapturingMap& map, const Element* target)
{
...
{
    RemoveTargetFromPointerCapturingMapping(pending_pointer_capture_target_, target);
}

bool PointerEventManager::SetPointerCapture(PointerId pointer_id, Element* target, bool explicit_capture)
{
    if (explicit_capture) {
    ...
    if (pointer_event_factory_.IsActiveButtonsState(pointer_id)) {
        if (pointer_id != dispatching_pointer_id_) {
            UseCounter::Count(frame_->GetDocument(), WebFeature::kPointerEventSetCaptureOutsideDispatch);
        }
...
bool PointerEventManager::ReleasePointerCapture(PointerId pointer_id, Element* target)
{
...
bool PointerEventManager::HasPointerCapture(PointerId pointer_id, const Element* target) const
{
...
bool PointerEventManager::IsPointerIdActiveOnFrame(PointerId pointer_id, LocalFrame* frame) const
{
    Element* last_element_receiving_event = element_under_pointer_.Contains(pointer_id) ? element_under_pointer_.at(pointer_id) : nullptr;
    return last_element_receiving_event && last_element_receiving_event->GetDocument().GetFrame() == frame;
}
...
bool PointerEventManager::PrimaryPointerdownCanceled(uint32_t unique_touch_event_id)
{
...
void PointerEventManager::SetLastPointerPositionForFrameBoundary(const WebPointerEvent& web_pointer_event, Element* new_target)
{
    PointerId pointer_id = pointer_event_factory_.GetPointerEventId(web_pointer_event);
    Element* last_target = element_under_pointer_.Contains(pointer_id) ? element_under_pointer_.at(pointer_id) : nullptr;
    if (!new_target) {
        pointer_event_factory_.RemoveLastPosition(pointer_id);
    } else if (!last_target || new_target->GetDocument().GetFrame() != last_target->GetDocument().GetFrame()) {
        pointer_event_factory_.SetLastPosition(pointer_id, web_pointer_event.PositionInScreen(), web_pointer_event.GetType());
    }
}
...
PointerId PointerEventManager::GetPointerIdForTouchGesture(const uint32_t unique_touch_event_id)
{
    return pointer_event_factory_.GetPointerIdForTouchGesture(unique_touch_event_id);
}
