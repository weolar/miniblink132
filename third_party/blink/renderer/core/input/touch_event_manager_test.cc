    ...
    WebPointerEvent CreateTouchPointerEvent(WebInputEvent::Type type)
    {
        WebPointerEvent event(type,
            WebPointerProperties(
                1, WebPointerProperties::PointerType::kTouch, WebPointerProperties::Button::kLeft, gfx::PointF(100, 100), gfx::PointF(100, 100)),
            1, 1);
...
TEST_F(TouchEventManagerTest, LostTouchDueToInnerIframeRemove)
{
    ...
    GetDocument().body()->addEventListener(event_type_names::kTouchstart, callback);

    GetEventHandler().HandlePointerEvent(CreateTouchPointerEvent(WebInputEvent::Type::kPointerDown), Vector<WebPointerEvent>(), Vector<WebPointerEvent>());
    GetEventHandler().DispatchBufferedTouchEvents();

    GetDocument().getElementById(AtomicString("target"))->remove();

    GetEventHandler().HandlePointerEvent(CreateTouchPointerEvent(WebInputEvent::Type::kPointerUp), Vector<WebPointerEvent>(), Vector<WebPointerEvent>());
    GetEventHandler().DispatchBufferedTouchEvents();
...
TEST_F(TouchEventManagerTest, AbosolutePosWithScrollAndZoom)
{
    ...
    auto* input = To<HTMLInputElement>(GetDocument().getElementById(AtomicString("slideElement")));
