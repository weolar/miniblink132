...
CORE_EXPORT HitTestResult HitTestResultInFrame(
    LocalFrame*, const HitTestLocation&, HitTestRequest::HitTestRequestType hit_type = HitTestRequest::kReadOnly | HitTestRequest::kActive);
...
CORE_EXPORT PhysicalOffset ContentPointFromRootFrame(LocalFrame*, const gfx::PointF& point_in_root_frame);

MouseEventWithHitTestResults PerformMouseEventHitTest(LocalFrame*, const HitTestRequest&, const WebMouseEvent&);

LocalFrame* GetTargetSubframe(const MouseEventWithHitTestResults&, bool* is_remote_frame = nullptr);
...
WTF_ALLOW_INIT_WITH_MEM_FUNCTIONS(blink::event_handling_util::PointerEventTarget)
