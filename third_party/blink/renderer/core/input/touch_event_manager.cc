...
bool HasEventHandlerInAncestorPath(Node* touch_node, EventHandlerRegistry::EventHandlerClass handler_class)
{
    Document& document = touch_node->GetDocument();
    const EventTargetSet* event_target_set = document.GetFrame()->GetEventHandlerRegistry().EventHandlerTargets(handler_class);
...
{
    return registry.HasEventHandlers(EventHandlerRegistry::kTouchStartOrMoveEventBlocking)
        || registry.HasEventHandlers(EventHandlerRegistry::kTouchStartOrMoveEventBlockingLowLatency)
        || registry.HasEventHandlers(EventHandlerRegistry::kTouchStartOrMoveEventPassive)
        || registry.HasEventHandlers(EventHandlerRegistry::kTouchEndOrCancelEventBlocking)
        || registry.HasEventHandlers(EventHandlerRegistry::kTouchEndOrCancelEventPassive);
}

const AtomicString& TouchEventNameForPointerEventType(WebInputEvent::Type type)
{
    switch (type) {
...
WebTouchPoint::State TouchPointStateFromPointerEventType(WebInputEvent::Type type, bool stale)
{
    if (stale)
...
WebTouchPoint CreateWebTouchPointFromWebPointerEvent(const WebPointerEvent& web_pointer_event, bool stale)
{
    WebTouchPoint web_touch_point(web_pointer_event);
    web_touch_point.state = TouchPointStateFromPointerEventType(web_pointer_event.GetType(), stale);
...
void SetWebTouchEventAttributesFromWebPointerEvent(WebTouchEvent* web_touch_event, const WebPointerEvent& web_pointer_event)
{
    web_touch_event->dispatch_type = web_pointer_event.dispatch_type;
    web_touch_event->touch_start_or_first_touch_move = web_pointer_event.touch_start_or_first_touch_move;
    web_touch_event->moved_beyond_slop_region = web_pointer_event.moved_beyond_slop_region;
...
Touch* TouchEventManager::CreateDomTouch(const TouchEventManager::TouchPointAttributes* point_attr, bool* known_target)
{
    ...
    *known_target = false;
    ...
    WebPointerEvent transformed_event = point_attr->event_.WebPointerEventInRootFrame();
    float scale_factor = 1.0f / target_frame->LayoutZoomFactor();

    gfx::PointF document_point = gfx::ScalePoint(target_frame->View()->RootFrameToDocument(transformed_event.PositionInWidget()), scale_factor);
    gfx::SizeF adjusted_radius = gfx::ScaleSize(gfx::SizeF(transformed_event.width / 2.f, transformed_event.height / 2.f), scale_factor);

    return MakeGarbageCollected<Touch>(target_frame, touch_node, point_attr->event_.id, transformed_event.PositionInScreen(), document_point, adjusted_radius,
...
WebCoalescedInputEvent TouchEventManager::GenerateWebCoalescedInputEvent()
{
    ...
    const auto& first_touch_pointer_event = touch_attribute_map_.begin()->value->event_;

    SetWebTouchEventAttributesFromWebPointerEvent(event.get(), first_touch_pointer_event);
    SetWebTouchEventAttributesFromWebPointerEvent(&last_coalesced_touch_event_, first_touch_pointer_event);
    ...
    for (const int& touch_point_id : available_ids) {
        ...
        event->touches[event->touches_length++] = CreateWebTouchPointFromWebPointerEvent(touch_pointer_event, touch_point_attribute->stale_);
        if (!touch_point_attribute->stale_) {
            event->SetTimeStamp(std::max(event->TimeStamp(), touch_point_attribute->event_.TimeStamp()));
        }
        ...
        if (touch_event_type == WebInputEvent::Type::kTouchMove) {
            ...
            else if (touch_pointer_event.GetType() == WebInputEvent::Type::kPointerCancel)
        ...
        for (const WebPointerEvent& coalesced_event : touch_point_attribute->coalesced_events_) {
    ...
    std::sort(all_coalesced_events.begin(), all_coalesced_events.end(), timestamp_based_event_comparison);
    ...
    for (const auto& web_pointer_event : all_coalesced_events) {
        if (web_pointer_event.GetType() == WebInputEvent::Type::kPointerDown) {
            ...
            for (unsigned i = 0; i < last_coalesced_touch_event_.touches_length; ++i) {
                if (last_coalesced_touch_event_.touches[i].id == web_pointer_event.id) {
                    last_coalesced_touch_event_.touches[i] = CreateWebTouchPointFromWebPointerEvent(web_pointer_event, false);
                    last_coalesced_touch_event_.SetTimeStamp(web_pointer_event.TimeStamp());
            ...
            if (!found_existing_id) {
                last_coalesced_touch_event_.touches[last_coalesced_touch_event_.touches_length++]
            ...
            base::ranges::sort(base::span(last_coalesced_touch_event_.touches).first(last_coalesced_touch_event_.touches_length), id_based_event_comparison);
            ...
            for (unsigned i = 0; i < last_coalesced_touch_event_.touches_length; ++i) {
                if (last_coalesced_touch_event_.touches[i].id == web_pointer_event.id) {
                    ...
                    for (unsigned j = 0; j < last_coalesced_touch_event_.touches_length; j++) {
                        if (last_coalesced_touch_event_.touches[j].state != WebTouchPoint::State::kStateCancelled
                            && last_coalesced_touch_event_.touches[j].state != WebTouchPoint::State::kStateReleased) {
                            last_coalesced_touch_event_.touches[result_size++] = last_coalesced_touch_event_.touches[j];
                        }
                    }
...
WebInputEventResult TouchEventManager::DispatchTouchEventFromAccumulatdTouchPoints()
{
    ...
    for (const auto& attr : touch_attribute_map_.Values()) {
        ...
        if (attr->event_.GetType() == WebInputEvent::Type::kPointerUp || attr->event_.GetType() == WebInputEvent::Type::kPointerCancel)
    ...
    if (suppressing_touchmoves_within_slop_) {
        ...
        if (touch_point_attribute->event_.GetType() == WebInputEvent::Type::kPointerMove) {
    ...
    using TargetTouchesHeapMap = HeapHashMap<Member<EventTarget>, Member<TouchList>>;
    TargetTouchesHeapMap touches_by_target;

    // Array of touches per state, used to assemble the |changedTouches| list.
    ChangedTouches changed_touches[static_cast<int>(WebInputEvent::Type::kPointerTypeLast) - static_cast<int>(WebInputEvent::Type::kPointerTypeFirst) + 1];
    ...
    for (const int& touch_point_id : available_ids) {
        ...
        TargetTouchesHeapMap::iterator target_touches_iterator = touches_by_target.find(touch_target);
        ...
        if (event_type != WebInputEvent::Type::kPointerUp && event_type != WebInputEvent::Type::kPointerCancel) {
        ...
        if (!touch_point_attribute->stale_ && known_target) {
            size_t event_type_idx = static_cast<int>(event_type) - static_cast<int>(WebInputEvent::Type::kPointerTypeFirst);
    ...
    for (unsigned action = static_cast<int>(WebInputEvent::Type::kPointerTypeFirst); action <= static_cast<int>(WebInputEvent::Type::kPointerTypeLast);
         ++action) {
        size_t action_idx = action - static_cast<int>(WebInputEvent::Type::kPointerTypeFirst);
        if (!changed_touches[action_idx].touches_)
            continue;

        const AtomicString& event_name(TouchEventNameForPointerEventType(static_cast<WebInputEvent::Type>(action)));

        for (const auto& event_target : changed_touches[action_idx].targets_) {
            EventTarget* touch_event_target = event_target;
            TouchEvent* touch_event = TouchEvent::Create(coalesced_event, touches, touches_by_target.at(touch_event_target),
                changed_touches[action_idx].touches_, event_name, touch_event_target->ToNode()->GetDocument().domWindow(), current_touch_action_);

            DispatchEventResult dom_dispatch_result = touch_event_target->DispatchEvent(*touch_event);

            event_result = event_handling_util::MergeEventResult(event_result, event_handling_util::ToWebInputEventResult(dom_dispatch_result));
        }
    }
    ...
    // not consumed.
    if (all_touch_points_pressed && event_result == WebInputEventResult::kNotHandled) {
...
Node* TouchEventManager::GetTouchPointerNode(const WebPointerEvent& event, const event_handling_util::PointerEventTarget& pointer_event_target)
{
    DCHECK(event.GetType() == WebInputEvent::Type::kPointerDown);

    Node* touch_pointer_node = pointer_event_target.target_element;

    if (touch_sequence_document_ && (!touch_pointer_node || &touch_pointer_node->GetDocument() != touch_sequence_document_)) {
        if (!touch_sequence_document_->GetFrame())
            return nullptr;

        HitTestLocation location(PhysicalOffset::FromPointFRound(touch_sequence_document_->GetFrame()->View()->ConvertFromRootFrame(event.PositionInWidget())));
        HitTestRequest::HitTestRequestType hit_type = HitTestRequest::kTouchEvent | HitTestRequest::kReadOnly | HitTestRequest::kActive;
        HitTestResult result = event_handling_util::HitTestResultInFrame(touch_sequence_document_->GetFrame(), location, hit_type);
...
void TouchEventManager::UpdateTouchAttributeMapsForPointerDown(const WebPointerEvent& event, Node* touch_node, TouchAction effective_touch_action)
{
    DCHECK(event.GetType() == WebInputEvent::Type::kPointerDown);
    DCHECK(touch_node);
    ...
    touch_attribute_map_.Set(event.id, MakeGarbageCollected<TouchPointAttributes>(event));
    ...
    should_enforce_vertical_scroll_ = touch_sequence_document_->IsVerticalScrollEnforced();
    if (should_enforce_vertical_scroll_ && HasEventHandlerInAncestorPath(touch_node, EventHandlerRegistry::kTouchStartOrMoveEventBlocking)) {
        delayed_effective_touch_action_ = delayed_effective_touch_action_.value_or(TouchAction::kAuto) & effective_touch_action;
    }
    if (!delayed_effective_touch_action_) {
        frame_->GetPage()->GetChromeClient().SetTouchAction(frame_, effective_touch_action);
    }
...
void TouchEventManager::HandleTouchPoint(
    const WebPointerEvent& event, const Vector<WebPointerEvent>& coalesced_events, const event_handling_util::PointerEventTarget& pointer_event_target)
{
    ...
    DCHECK(frame_->View());
    if (touch_sequence_document_ && (!touch_sequence_document_->GetFrame() || !touch_sequence_document_->GetFrame()->View())) {
    ...
    if (!touch_attribute_map_.Contains(event.id)) {
        touch_attribute_map_.insert(event.id, MakeGarbageCollected<TouchPointAttributes>(event));
    }
...
WebInputEventResult TouchEventManager::FlushEvents()
{
    ...
    // sending the event.
    if (touch_sequence_document_ && touch_sequence_document_->GetPage() && HasTouchHandlers(touch_sequence_document_->GetFrame()->GetEventHandlerRegistry())
    ...
    for (auto& attributes : touch_attribute_map_.Values()) {
        if (attributes->event_.GetType() == WebInputEvent::Type::kPointerUp || attributes->event_.GetType() == WebInputEvent::Type::kPointerCancel) {
...
WebInputEventResult TouchEventManager::EnsureVerticalScrollIsPossible(WebInputEventResult event_result)
{
    bool prevent_defaulted = event_result == WebInputEventResult::kHandledApplication;
    ...
    if (delayed_effective_touch_action_) {
        ...
        frame_->GetPage()->GetChromeClient().SetTouchAction(frame_, delayed_effective_touch_action_.value());
        delayed_effective_touch_action_ = std::nullopt;
    }
