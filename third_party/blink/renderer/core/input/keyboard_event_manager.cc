...
bool MapKeyCodeForScroll(int key_code, WebInputEvent::Modifiers modifiers, mojom::blink::ScrollDirection* scroll_direction,
    ui::ScrollGranularity* scroll_granularity, WebFeature* scroll_use_uma)
{
    if (modifiers & WebInputEvent::kShiftKey || modifiers & WebInputEvent::kMetaKey)
        return false;
    ...
    switch (key_code) {
    case VKEY_LEFT:
        *scroll_direction = mojom::blink::ScrollDirection::kScrollLeftIgnoringWritingMode;
        *scroll_granularity
            = RuntimeEnabledFeatures::PercentBasedScrollingEnabled() ? ui::ScrollGranularity::kScrollByPercentage : ui::ScrollGranularity::kScrollByLine;
        *scroll_use_uma = WebFeature::kScrollByKeyboardArrowKeys;
        break;
    case VKEY_RIGHT:
        *scroll_direction = mojom::blink::ScrollDirection::kScrollRightIgnoringWritingMode;
        *scroll_granularity
            = RuntimeEnabledFeatures::PercentBasedScrollingEnabled() ? ui::ScrollGranularity::kScrollByPercentage : ui::ScrollGranularity::kScrollByLine;
        *scroll_use_uma = WebFeature::kScrollByKeyboardArrowKeys;
        break;
    case VKEY_UP:
        *scroll_direction = mojom::blink::ScrollDirection::kScrollUpIgnoringWritingMode;
        *scroll_granularity
            = RuntimeEnabledFeatures::PercentBasedScrollingEnabled() ? ui::ScrollGranularity::kScrollByPercentage : ui::ScrollGranularity::kScrollByLine;
        *scroll_use_uma = WebFeature::kScrollByKeyboardArrowKeys;
        break;
    case VKEY_DOWN:
        *scroll_direction = mojom::blink::ScrollDirection::kScrollDownIgnoringWritingMode;
        *scroll_granularity
            = RuntimeEnabledFeatures::PercentBasedScrollingEnabled() ? ui::ScrollGranularity::kScrollByPercentage : ui::ScrollGranularity::kScrollByLine;
        *scroll_use_uma = WebFeature::kScrollByKeyboardArrowKeys;
        break;
...
KeyboardEventManager::KeyboardEventManager(LocalFrame& frame, ScrollManager& scroll_manager)
    : frame_(frame)
...
bool KeyboardEventManager::HandleAccessKey(const WebKeyboardEvent& evt)
{
    ...
    if ((evt.GetModifiers() & (WebKeyboardEvent::kKeyModifiers & ~WebInputEvent::kShiftKey)) != kAccessKeyModifiers)
        return false;
    String key = String(evt.unmodified_text.data());
    Element* elem = frame_->GetDocument()->GetElementByAccessKey(key.DeprecatedLower());
    if (!elem)
        return false;
    elem->Focus(
        FocusParams(SelectionBehaviorOnFocus::kReset, mojom::blink::FocusType::kAccessKey, nullptr, FocusOptions::Create(), FocusTrigger::kUserGesture));
    elem->AccessKeyAction(SimulatedClickCreationScope::kFromUserAgent);
    return true;
}

WebInputEventResult KeyboardEventManager::KeyEvent(const WebKeyboardEvent& initial_key_event)
{
    base::AutoReset<bool> is_handling_key_event(&is_handling_key_event_, true);
    ...
    if (scroll_manager_->MiddleClickAutoscrollInProgress()) {
        ...
        // we want to stop.
        if (initial_key_event.GetType() == WebInputEvent::Type::kKeyDown || initial_key_event.GetType() == WebInputEvent::Type::kRawKeyDown)
    ...
    bool is_modifier = ui::KeycodeConverter::IsDomKeyForModifier(static_cast<ui::DomKey>(initial_key_event.dom_key));

    if (!is_modifier && initial_key_event.dom_key != ui::DomKey::ESCAPE
        && (initial_key_event.GetType() == WebInputEvent::Type::kKeyDown || initial_key_event.GetType() == WebInputEvent::Type::kRawKeyDown)) {
        LocalFrame::NotifyUserActivation(
            frame_, mojom::blink::UserActivationNotificationType::kInteraction, RuntimeEnabledFeatures::BrowserVerifiedUserActivationKeyboardEnabled());
    }
    ...
    bool should_send_key_events_to_js = !frame_->GetSettings()->GetDontSendKeyEventsToJavascript();

    if (!should_send_key_events_to_js && frame_->GetDocument()->IsInWebAppScope()) {
        mojom::blink::DisplayMode display_mode = frame_->GetWidgetForLocalRoot()->DisplayMode();
        should_send_key_events_to_js = display_mode == blink::mojom::DisplayMode::kMinimalUi || display_mode == blink::mojom::DisplayMode::kStandalone
            || display_mode == blink::mojom::DisplayMode::kFullscreen || display_mode == blink::mojom::DisplayMode::kBorderless
    ...
    if (!should_send_key_events_to_js) {
        ...
        const int kDomKeysNotCancellabelUnlessInEditor[] = { 0x00400031, 0x00400032, 0x00400033 };
        ...
        for (uint32_t dom_key : kDomKeysNotCancellabelUnlessInEditor) {
            ...
            bool is_editable = IsEditable(*node) || (text_control && !text_control->IsDisabledOrReadOnly())
                || (element && EqualIgnoringASCIICase(element->FastGetAttribute(html_names::kRoleAttr), "textbox"));
    ...
    case WebInputEvent::Type::kKeyUp: {
        KeyboardEvent* event = KeyboardEvent::Create(initial_key_event, frame_->GetDocument()->domWindow(), event_cancellable);
    ...
    case WebInputEvent::Type::kKeyDown: {
        ...
        KeyboardEvent* event = KeyboardEvent::Create(web_event, frame_->GetDocument()->domWindow(), event_cancellable);
        event->SetTarget(node);
        event->SetStopPropagation(!send_key_event);
        ...
        if (initial_key_event.GetType() == WebInputEvent::Type::kKeyDown && HandleAccessKey(initial_key_event)) {
        ...
        if (dispatch_result = node->DispatchEvent(*event); dispatch_result != DispatchEventResult::kNotCanceled) {
            break;
        }
        ...
        if (frame_->GetPage() && frame_ != frame_->GetPage()->GetFocusController().FocusedOrMainFrame()) {
        ...
        // arbitrary cut-off.
        if (initial_key_event.text[0U] >= 0xF700 && initial_key_event.text[0U] <= 0xF7FF) {
            return WebInputEventResult::kNotHandled;
        }
#endif
    ...
    case WebInputEvent::Type::kChar: {
        ...
        KeyboardEvent* event = KeyboardEvent::Create(char_event, frame_->GetDocument()->domWindow(), event_cancellable);
        event->SetTarget(node);
        event->SetStopPropagation(!send_key_event);
...
void KeyboardEventManager::DefaultKeyboardEventHandler(KeyboardEvent* event, Node* possible_focused_node)
{
    if (event->type() == event_type_names::kKeydown) {
        ...
        } else if (event->KeyEvent() && static_cast<int>(event->KeyEvent()->dom_key) == 0x00200310) {
...
void KeyboardEventManager::DefaultSpaceEventHandler(KeyboardEvent* event, Node* possible_focused_node)
{
    ...
    mojom::blink::ScrollDirection direction
        = event->shiftKey() ? mojom::blink::ScrollDirection::kScrollBlockDirectionBackward : mojom::blink::ScrollDirection::kScrollBlockDirectionForward;
    ...
    // crbug.com/410974.
    if (scroll_manager_->LogicalScroll(direction, ui::ScrollGranularity::kScrollByPage, nullptr, possible_focused_node, true)) {
        UseCounter::Count(frame_->GetDocument(), WebFeature::kScrollByKeyboardSpacebarKey);
...
void KeyboardEventManager::DefaultArrowEventHandler(KeyboardEvent* event, Node* possible_focused_node)
{
    ...
    if (RuntimeEnabledFeatures::FocusgroupEnabled(context) && FocusgroupController::HandleArrowKeyboardEvent(event, frame_)) {
        event->SetDefaultHandled();
        return;
    }

    if (IsSpatialNavigationEnabled(frame_) && !frame_->GetDocument()->InDesignMode() && !IsPageUpOrDownKeyEvent(event->keyCode(), event->GetModifiers())) {
        if (page->GetSpatialNavigationController().HandleArrowKeyboardEvent(event)) {
            event->SetDefaultHandled();
            return;
        }
    }
    ...
    if (!MapKeyCodeForScroll(event->keyCode(), event->GetModifiers(), &scroll_direction, &scroll_granularity, &scroll_use_uma))
        return;

    // See KeyboardEventManager::DefaultSpaceEventHandler for the reason for
    // this Clear.
    scrollend_event_target_.Clear();
    if (scroll_manager_->BubblingScroll(scroll_direction, scroll_granularity, nullptr, possible_focused_node, true)) {
...
void KeyboardEventManager::DefaultTabEventHandler(KeyboardEvent* event)
{
    ...
    if (event->ctrlKey() || event->metaKey()) {
        // TODO (liviutinta) remove TRACE after fixing crbug.com/1063548
        TRACE_EVENT_INSTANT1("input", "KeyboardEventManager::DefaultTabEventHandler", TRACE_EVENT_SCOPE_THREAD, "reason_tab_does_not_advance_focus",
            (event->ctrlKey() ? (event->metaKey() ? "Ctrl+MetaKey+Tab" : "Ctrl+Tab") : "MetaKey+Tab"));
        return;
    }
    ...
    if (event->altKey()) {
        // TODO (liviutinta) remove TRACE after fixing crbug.com/1063548
        TRACE_EVENT_INSTANT1("input", "KeyboardEventManager::DefaultTabEventHandler", TRACE_EVENT_SCOPE_THREAD, "reason_tab_does_not_advance_focus", "Alt+Tab");
        return;
    }
#endif
    ...
    if (!page) {
        ...
        TRACE_EVENT_INSTANT1(
            "input", "KeyboardEventManager::DefaultTabEventHandler", TRACE_EVENT_SCOPE_THREAD, "reason_tab_does_not_advance_focus", "Page is null");
        return;
    }
    ...
    mojom::blink::FocusType focus_type = event->shiftKey() ? mojom::blink::FocusType::kBackward : mojom::blink::FocusType::kForward;

    // Tabs can be used in design mode editing.
    if (frame_->GetDocument()->InDesignMode()) {
        // TODO (liviutinta) remove TRACE after fixing crbug.com/1063548
        TRACE_EVENT_INSTANT1(
            "input", "KeyboardEventManager::DefaultTabEventHandler", TRACE_EVENT_SCOPE_THREAD, "reason_tab_does_not_advance_focus", "DesignMode is true");
        return;
    }

    if (page->GetFocusController().AdvanceFocus(focus_type, frame_->GetDocument()->domWindow()->GetInputDeviceCapabilities()->FiresTouchEvents(false))) {
        event->SetDefaultHandled();
    } else {
        // TODO (liviutinta) remove TRACE after fixing crbug.com/1063548
        TRACE_EVENT_INSTANT1("input", "KeyboardEventManager::DefaultTabEventHandler", TRACE_EVENT_SCOPE_THREAD, "reason_tab_does_not_advance_focus",
...
void KeyboardEventManager::DefaultEscapeEventHandler(KeyboardEvent* event)
{
    ...
    if (IsSpatialNavigationEnabled(frame_) && !frame_->GetDocument()->InDesignMode()) {
...
void KeyboardEventManager::SetCurrentCapsLockState(OverrideCapsLockState state)
{
