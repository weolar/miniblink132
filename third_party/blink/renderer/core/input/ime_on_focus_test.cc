...
protected:
    ...
    void RunImeOnFocusTest(
        String file_name, size_t, gfx::Point tap_point = gfx::Point(-1, -1), const AtomicString& focus_element = g_null_atom, String frame = "");
...
void ImeOnFocusTest::SendGestureTap(WebViewImpl* web_view, gfx::Point client_point)
{
    ...
    web_view->MainFrameViewWidget()->HandleInputEvent(WebCoalescedInputEvent(web_gesture_event, ui::LatencyInfo()));
    RunPendingTasks();
}
...
void ImeOnFocusTest::RunImeOnFocusTest(
    String file_name, size_t expected_virtual_keyboard_request_count, gfx::Point tap_point, const AtomicString& focus_element, String frame)
{
    RegisterMockedURLLoadFromBase(WebString(base_url_), test::CoreTestDataPath(), WebString(file_name));
    ...
    document_ = web_view_helper_.GetWebView()->MainFrameImpl()->GetDocument().Unwrap<Document>();
    frame_test_helpers::TestWebFrameWidgetHost& widget_host = web_view_helper_.GetMainFrameWidget()->WidgetHost();
    ...
    if (!frame.empty()) {
        RegisterMockedURLLoadFromBase(WebString(base_url_), test::CoreTestDataPath(), WebString(frame));
        WebLocalFrame* child_frame = web_view->MainFrame()->FirstChild()->ToWebLocalFrame();
    ...
    if (expected_virtual_keyboard_request_count == 0) {
        ...
        // gesture tap.
        EXPECT_LE(expected_virtual_keyboard_request_count, widget_host.VirtualKeyboardRequestCount());
    }
...
{
    RunImeOnFocusTest("ime-on-focus-after-first-gesture.html", 1, gfx::Point(50, 50), AtomicString("input"));
}

TEST_F(ImeOnFocusTest, AfterNavigationWithinPage)
{
    RunImeOnFocusTest("ime-on-focus-after-navigation-within-page.html", 1, gfx::Point(50, 50), AtomicString("input"));
}

TEST_F(ImeOnFocusTest, AfterFrameLoadOnGesture)
{
    RunImeOnFocusTest("ime-on-focus-after-frame-load-on-gesture.html", 1, gfx::Point(50, 50), AtomicString("input"), "frame.html");
}
