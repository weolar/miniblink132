...
HitTestResult HitTestResultInFrame(LocalFrame* frame, const HitTestLocation& location, HitTestRequest::HitTestRequestType hit_type)
{
...
WebInputEventResult MergeEventResult(WebInputEventResult result_a, WebInputEventResult result_b)
{
    ...
    static_assert(static_cast<int>(WebInputEventResult::kNotHandled) == 0, "WebInputEventResult not ordered");
    static_assert(static_cast<int>(WebInputEventResult::kHandledSuppressed) < static_cast<int>(WebInputEventResult::kHandledApplication),
        "WebInputEventResult not ordered");
    static_assert(
        static_cast<int>(WebInputEventResult::kHandledApplication) < static_cast<int>(WebInputEventResult::kHandledSystem), "WebInputEventResult not ordered");
    return static_cast<WebInputEventResult>(max(static_cast<int>(result_a), static_cast<int>(result_b)));
}
...
PhysicalOffset ContentPointFromRootFrame(LocalFrame* frame, const gfx::PointF& point_in_root_frame)
{
    LocalFrameView* view = frame->View();
    // FIXME: Is it really OK to use the wrong coordinates here when view is 0?
    // Historically the code would just crash; this is clearly no worse than that.
    return PhysicalOffset::FromPointFRound(view ? view->ConvertFromRootFrame(point_in_root_frame) : point_in_root_frame);
}

MouseEventWithHitTestResults PerformMouseEventHitTest(LocalFrame* frame, const HitTestRequest& request, const WebMouseEvent& mev)
{
    DCHECK(frame);
    DCHECK(frame->GetDocument());

    return frame->GetDocument()->PerformMouseEventHitTest(request, ContentPointFromRootFrame(frame, mev.PositionInRootFrame()), mev);
}

bool ShouldDiscardEventTargetingFrame(const WebInputEvent& event, const LocalFrame& frame)
{
    ...
    if (frame.IsCrossOriginToOutermostMainFrame()) {
        if (frame.NeedsOcclusionTracking()) {
            should_discard
                = (event.GetModifiers() & WebInputEvent::kTargetFrameMovedRecentlyForIOv2) || !frame.View()->RectInParentIsStableForIOv2(event.TimeStamp());
        } else if (base::FeatureList::IsEnabled(features::kDiscardInputEventsToRecentlyMovedFrames)) {
            should_discard = (event.GetModifiers() & WebInputEvent::kTargetFrameMovedRecently) || !frame.View()->RectInParentIsStable(event.TimeStamp());
        }
    }
    if (should_discard) {
        UseCounter::Count(frame.GetDocument(), WebFeature::kDiscardInputEventToMovingIframe);
    }
...
LocalFrame* GetTargetSubframe(const MouseEventWithHitTestResults& hit_test_result, bool* is_remote_frame)
{
