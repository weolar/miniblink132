...
class CORE_EXPORT TouchEventManager final : public GarbageCollected<TouchEventManager> {
public:
    ...
    void HandleTouchPoint(const WebPointerEvent&, const Vector<WebPointerEvent>&, const event_handling_util::PointerEventTarget&);
    ...
    Node* GetTouchPointerNode(const WebPointerEvent& event, const event_handling_util::PointerEventTarget& pointer_event_target);

    // Updates attributes of the touch point in |touch_points_attributes_| map.
    void UpdateTouchAttributeMapsForPointerDown(const WebPointerEvent&, Node* touch_node, TouchAction effective_touch_action);
...
private:
    ...
    // related attributes.
    class TouchPointAttributes final : public GarbageCollected<TouchPointAttributes> {
    public:
    ...
    using TouchAttributeMap = HeapHashMap<int, Member<TouchPointAttributes>, IntWithZeroKeyHashTraits<int>>;
