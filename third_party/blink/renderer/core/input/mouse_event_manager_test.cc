...
protected:
    ...
    {
        WebKeyboardEvent web_event = { WebInputEvent::Type::kRawKeyDown, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests() };
...
TEST_F(MouseEventManagerTest, DISABLED_MousePressNodeRemoved)
{
    ...
    EXPECT_FLOAT_EQ(GetDocument().getElementById(AtomicString("scroller"))->scrollTop(), 0.0);

    // Click on the target node to set the mouse_press_node_.
    GetEventHandler().HandleMousePressEvent(CreateTestMouseEvent(WebInputEvent::Type::kMouseDown, gfx::PointF(50, 50)));
    ...
    EXPECT_GT(GetDocument().getElementById(AtomicString("scroller"))->scrollTop(), 0.0);
}
...
TEST_F(MouseEventManagerTest, HoverEffectAfterNav)
{
    ...
    GetEventHandler().HandleMouseMoveEvent(
        CreateTestMouseEvent(WebInputEvent::Type::kMouseMove, gfx::PointF(20, 20)), Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
    ...
    LayoutObject* b = GetDocument().getElementById(AtomicString("b"))->GetLayoutObject();
    ...
    Color color = b->Style()->VisitedDependentColor(GetCSSPropertyBackgroundColor());
