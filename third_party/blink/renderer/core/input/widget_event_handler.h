...
protected:
    virtual void HandleMouseMove(
        LocalFrame& main_frame, const WebMouseEvent&, const std::vector<std::unique_ptr<WebInputEvent>>&, const std::vector<std::unique_ptr<WebInputEvent>>&);
    ...
    virtual WebInputEventResult HandleMouseUp(LocalFrame& local_root, const WebMouseEvent&);
    virtual WebInputEventResult HandleMouseWheel(LocalFrame& local_root, const WebMouseWheelEvent&);
    ...
    virtual WebInputEventResult HandlePointerEvent(
        LocalFrame& main_frame, const WebPointerEvent&, const std::vector<std::unique_ptr<WebInputEvent>>&, const std::vector<std::unique_ptr<WebInputEvent>>&);
    virtual ~WidgetEventHandler() = default;

    WebInputEventResult HandleInputEvent(const WebCoalescedInputEvent& coalesced_event, LocalFrame* root);
};
