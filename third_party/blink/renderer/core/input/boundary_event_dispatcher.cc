...
void BuildAncestorChain(EventTarget* target, HeapVector<Member<Node>, 20>* ancestors)
{
...
void BuildAncestorChainsAndFindCommonAncestors(EventTarget* exited_target, EventTarget* entered_target, HeapVector<Member<Node>, 20>* exited_ancestors_out,
    HeapVector<Member<Node>, 20>* entered_ancestors_out, wtf_size_t* exited_ancestors_common_parent_index_out,
    wtf_size_t* entered_ancestors_common_parent_index_out)
{
    ...
    *entered_ancestors_common_parent_index_out = entered_ancestors_out->size();
    while (*exited_ancestors_common_parent_index_out > 0 && *entered_ancestors_common_parent_index_out > 0) {
...
void BoundaryEventDispatcher::SendBoundaryEvents(EventTarget* exited_target, bool original_exited_target_removed, EventTarget* entered_target)
{
    ...
    // Dispatch out event
    if (event_handling_util::IsInDocument(exited_target) && !original_exited_target_removed) {
    ...
    BuildAncestorChainsAndFindCommonAncestors(
        exited_target, entered_target, &exited_ancestors, &entered_ancestors, &exited_ancestors_common_parent_index, &entered_ancestors_common_parent_index);
    ...
    for (wtf_size_t j = 0; j < exited_ancestors_common_parent_index; j++) {
        Dispatch(exited_ancestors[j], entered_target, leave_event_, !exited_node_has_capturing_ancestor);
    }
    ...
    for (wtf_size_t i = entered_ancestors_common_parent_index; i > 0; i--) {
        Dispatch(entered_ancestors[i - 1], exited_target, enter_event_, !entered_node_has_capturing_ancestor);
    }
}
