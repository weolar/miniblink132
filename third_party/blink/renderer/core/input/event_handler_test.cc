...
public:
    ...
    {
        WebMouseEvent mouse_move_event(WebMouseEvent::Type::kMouseMove, gfx::PointF(x, y), gfx::PointF(x, y), WebPointerProperties::Button::kNoButton, 0,
            WebInputEvent::Modifiers::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests());
        GetDocument().GetFrame()->GetEventHandler().HandleMouseMoveEvent(mouse_move_event, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());

        WebView().MainFrameWidget()->SetFocus(true);
        WebView().SetIsActive(true);
    }

    void InjectScrollFromGestureEvents(cc::ElementId element_id, float delta_x, float delta_y)
    {
        WebGestureEvent gesture_scroll_begin { WebInputEvent::Type::kGestureScrollBegin, WebInputEvent::kNoModifiers,
            WebInputEvent::GetStaticTimeStampForTests() };
        gesture_scroll_begin.data.scroll_begin.delta_x_hint = 0;
        gesture_scroll_begin.data.scroll_begin.delta_y_hint = -delta_y;
        gesture_scroll_begin.data.scroll_begin.scrollable_area_element_id = element_id.GetInternalValue();
        WebView().MainFrameWidget()->HandleInputEvent(WebCoalescedInputEvent(gesture_scroll_begin, ui::LatencyInfo()));

        WebGestureEvent gesture_scroll_update { WebInputEvent::Type::kGestureScrollUpdate, WebInputEvent::kNoModifiers,
            WebInputEvent::GetStaticTimeStampForTests() };
        gesture_scroll_update.data.scroll_update.delta_x = delta_x;
        gesture_scroll_update.data.scroll_update.delta_y = -delta_y;
        WebView().MainFrameWidget()->HandleInputEvent(WebCoalescedInputEvent(gesture_scroll_update, ui::LatencyInfo()));

        WebGestureEvent gesture_scroll_end { WebInputEvent::Type::kGestureScrollEnd, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests() };
        WebView().MainFrameWidget()->HandleInputEvent(WebCoalescedInputEvent(gesture_scroll_end, ui::LatencyInfo()));
    }

    void DispatchElementTargetedGestureScroll(const WebGestureEvent& gesture_event)
    {
        GetWebFrameWidget().DispatchThroughCcInputHandler(gesture_event);
    }
};

WebPointerEvent CreateMinimalTouchPointerEvent(WebInputEvent::Type type, gfx::PointF position)
{
    WebPointerEvent event(
        type, WebPointerProperties(1, WebPointerProperties::PointerType::kTouch, WebPointerProperties::Button::kLeft, position, position), 1, 1);
    event.SetFrameScale(1);
    return event;
}

WebGestureEvent CreateMinimalGestureEvent(WebInputEvent::Type type, gfx::PointF position)
{
    WebGestureEvent event(type, WebInputEvent::kNoModifiers, base::TimeTicks::Now(), WebGestureDevice::kTouchscreen);
...
public:
    TapEventBuilder(gfx::PointF position, int tap_count)
        : WebGestureEvent(WebInputEvent::Type::kGestureTap, WebInputEvent::kNoModifiers, base::TimeTicks::Now(), WebGestureDevice::kTouchscreen)
    {
...
public:
    explicit TapDownEventBuilder(gfx::PointF position)
        : WebGestureEvent(WebInputEvent::Type::kGestureTapDown, WebInputEvent::kNoModifiers, base::TimeTicks::Now(), WebGestureDevice::kTouchscreen)
    {
        SetPositionInWidget(position);
        SetPositionInScreen(position);
...
public:
    explicit ShowPressEventBuilder(gfx::PointF position)
        : WebGestureEvent(WebInputEvent::Type::kGestureShowPress, WebInputEvent::kNoModifiers, base::TimeTicks::Now(), WebGestureDevice::kTouchscreen)
    {
        SetPositionInWidget(position);
        SetPositionInScreen(position);
...
public:
    explicit LongPressEventBuilder(gfx::PointF position)
        : WebGestureEvent(WebInputEvent::Type::kGestureLongPress, WebInputEvent::kNoModifiers, base::TimeTicks::Now(), WebGestureDevice::kTouchscreen)
    {
        SetPositionInWidget(position);
        SetPositionInScreen(position);
...
public:
    MousePressEventBuilder(gfx::Point position_param, int click_count_param, WebMouseEvent::Button button_param)
        : WebMouseEvent(WebInputEvent::Type::kMouseDown, WebInputEvent::kNoModifiers, base::TimeTicks::Now())
    {
...
ShadowRoot* EventHandlerTest::SetShadowContent(const char* shadow_content, const char* host)
{
    ShadowRoot* shadow_root = EditingTestBase::CreateShadowRootForElementWithIDAndSetInnerHTML(GetDocument(), host, shadow_content);
    return shadow_root;
}
...
TEST_F(EventHandlerTest, dragSelectionAfterScroll)
{
    ...
    frame_view->LayoutViewport()->SetScrollOffset(ScrollOffset(0, 400), mojom::blink::ScrollType::kProgrammatic);

    WebMouseEvent mouse_down_event(WebInputEvent::Type::kMouseDown, gfx::PointF(0, 0), gfx::PointF(100, 200), WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kLeftButtonDown, WebInputEvent::GetStaticTimeStampForTests());
    GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(mouse_down_event);

    ASSERT_TRUE(GetDocument().GetFrame()->GetEventHandler().GetSelectionController().MouseDownMayStartSelect());

    WebMouseEvent mouse_move_event(WebInputEvent::Type::kMouseMove, gfx::PointF(100, 50), gfx::PointF(200, 250), WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kLeftButtonDown, WebInputEvent::GetStaticTimeStampForTests());
    GetDocument().GetFrame()->GetEventHandler().HandleMouseMoveEvent(mouse_move_event, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
    ...
    WebMouseEvent mouse_up_event(WebMouseEvent::Type::kMouseUp, gfx::PointF(100, 50), gfx::PointF(200, 250), WebPointerProperties::Button::kLeft, 1,
    ...
    ASSERT_FALSE(GetDocument().GetFrame()->GetEventHandler().GetSelectionController().MouseDownMayStartSelect());

    ASSERT_TRUE(Selection().GetSelectionInDOMTree().IsRange());
    Range* range = CreateRange(EphemeralRange(Selection().GetSelectionInDOMTree().Anchor(), Selection().GetSelectionInDOMTree().Focus()));
    ASSERT_TRUE(range);
...
{
    SetHtmlInnerHTML("<style> body { margin: 0px; } .line { display: block; width: 300px; "
                     "height: 30px; } </style>"
...
TEST_F(EventHandlerTest, draggedInlinePositionTest)
{
    SetHtmlInnerHTML("<style>"
                     ...
                     "</div>");
    WebMouseEvent mouse_down_event(WebMouseEvent::Type::kMouseDown, gfx::PointF(262, 29), gfx::PointF(329, 67), WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kLeftButtonDown, WebInputEvent::GetStaticTimeStampForTests());
    GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(mouse_down_event);

    WebMouseEvent mouse_move_event(WebMouseEvent::Type::kMouseMove, gfx::PointF(618, 298), gfx::PointF(685, 436), WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kLeftButtonDown, WebInputEvent::GetStaticTimeStampForTests());
    GetDocument().GetFrame()->GetEventHandler().HandleMouseMoveEvent(mouse_move_event, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());

    EXPECT_EQ(gfx::Point(12, 29), GetDocument().GetFrame()->GetEventHandler().DragDataTransferLocationForTesting());
}
...
TEST_F(EventHandlerTest, draggedSVGImagePositionTest)
{
    SetHtmlInnerHTML("<style>"
                     ...
                     "</svg>"
                     "</div>");
    WebMouseEvent mouse_down_event(WebMouseEvent::Type::kMouseDown, gfx::PointF(145, 144), gfx::PointF(212, 282), WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kLeftButtonDown, WebInputEvent::GetStaticTimeStampForTests());
    GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(mouse_down_event);
    ...
    EXPECT_EQ(gfx::Point(45, 44), GetDocument().GetFrame()->GetEventHandler().DragDataTransferLocationForTesting());
}
...
TEST_F(EventHandlerTest, HitOnNothingDoesNotShowIBeam)
{
    ...
    HitTestResult hit = GetDocument().GetFrame()->GetEventHandler().HitTestResultAtLocation(location);
    EXPECT_FALSE(GetDocument().GetFrame()->GetEventHandler().ShouldShowIBeamForNode(GetDocument().body(), hit));
}
...
TEST_F(EventHandlerTest, HitOnTextShowsIBeam)
{
    ...
    HitTestLocation location(text->GetLayoutObject()->AbsoluteBoundingBoxRect().CenterPoint());
    HitTestResult hit = GetDocument().GetFrame()->GetEventHandler().HitTestResultAtLocation(location);
    EXPECT_TRUE(text->CanStartSelection());
    EXPECT_TRUE(GetDocument().GetFrame()->GetEventHandler().ShouldShowIBeamForNode(text, hit));
}
...
TEST_F(EventHandlerTest, HitOnUserSelectNoneDoesNotShowIBeam)
{
    ...
    EXPECT_FALSE(GetDocument().GetFrame()->GetEventHandler().ShouldShowIBeamForNode(text, hit));
}

TEST_F(EventHandlerTest, ShadowChildCanOverrideUserSelectNone)
{
    SetHtmlInnerHTML("<p style='user-select: none' id='host'></p>");
    ShadowRoot* const shadow_root = SetShadowContent("<span style='user-select: text' id='bla'>blabla</span>", "host");

    Node* const text = shadow_root->getElementById(AtomicString("bla"))->firstChild();
    HitTestLocation location(text->GetLayoutObject()->AbsoluteBoundingBoxRect().CenterPoint());
    HitTestResult hit = GetDocument().GetFrame()->GetEventHandler().HitTestResultAtLocation(location);
    EXPECT_TRUE(text->CanStartSelection());
    EXPECT_TRUE(GetDocument().GetFrame()->GetEventHandler().ShouldShowIBeamForNode(text, hit));
...
{
    SetHtmlInnerHTML("<div style='user-select: none'>"
...
{
    SetHtmlInnerHTML("<div style='user-select: all'>"
...
{
    SetHtmlInnerHTML("<div style='user-select: text'>"
                     "<span style='user-select: none'>blabla</span>"
                     "</div>");
    Node* const text = GetDocument().body()->firstChild()->firstChild()->firstChild();
    HitTestLocation location(text->GetLayoutObject()->AbsoluteBoundingBoxRect().CenterPoint());
...
TEST_F(EventHandlerTest, ShadowChildCanOverrideUserSelectText)
{
    ...
    ShadowRoot* const shadow_root = SetShadowContent("<span style='user-select: none' id='bla'>blabla</span>", "host");
...
TEST_F(EventHandlerTest, ReadOnlyInputDoesNotInheritUserSelect)
{
    SetHtmlInnerHTML("<div style='user-select: none'>"
                     ...
                     "</div>");
    auto* const input = To<HTMLInputElement>(GetDocument().getElementById(AtomicString("sample")));
...
TEST_F(EventHandlerTest, ImagesCannotStartSelection)
{
    ...
    HitTestLocation location(img->GetLayoutObject()->AbsoluteBoundingBoxRect().CenterPoint());
    HitTestResult hit = GetDocument().GetFrame()->GetEventHandler().HitTestResultAtLocation(location);
    EXPECT_FALSE(img->CanStartSelection());
    EXPECT_FALSE(GetDocument().GetFrame()->GetEventHandler().ShouldShowIBeamForNode(img, hit));
}
...
TEST_F(EventHandlerTest, AnchorTextCannotStartSelection)
{
    ...
    HitTestLocation location(link->GetLayoutObject()->AbsoluteBoundingBoxRect().CenterPoint());
    HitTestResult result = GetDocument().GetFrame()->GetEventHandler().HitTestResultAtLocation(location);
    ...
    EXPECT_TRUE(GetDocument().GetFrame()->GetEventHandler().ShouldShowIBeamForNode(text, result));
    EXPECT_EQ(GetDocument().GetFrame()->GetEventHandler().SelectCursor(location, result).value().type(),
...
TEST_F(EventHandlerTest, CursorForVerticalResizableTextArea)
{
    ...
    gfx::Point point = element->GetLayoutObject()->AbsoluteBoundingBoxRect().bottom_right();
    point.Offset(-5, -5);
...
{
    SetHtmlInnerHTML("<textarea style='resize:both;direction:rtl'>both</textarea>");
    Node* const element = GetDocument().body()->firstChild();
    gfx::Point point = element->GetLayoutObject()->AbsoluteBoundingBoxRect().bottom_left();
    point.Offset(5, -5);
    HitTestLocation location(point);
    HitTestResult result = GetDocument().GetFrame()->GetEventHandler().HitTestResultAtLocation(location);
    EXPECT_EQ(GetDocument().GetFrame()->GetEventHandler().SelectCursor(location, result).value().type(),
...
{
    SetHtmlInnerHTML("Test<p style='resize:both;writing-mode:vertical-lr;"
                     "width:30px;height:30px;overflow:hidden;display:inline'>Test "
                     "Test</p>Test");
    Node* const element = GetDocument().body()->firstChild()->nextSibling();
    gfx::Point point = element->GetLayoutObject()->AbsoluteBoundingBoxRect().origin();
    point.Offset(25, 25);
    HitTestLocation location(point);
    HitTestResult result = GetDocument().GetFrame()->GetEventHandler().HitTestResultAtLocation(location);
    EXPECT_EQ(GetDocument().GetFrame()->GetEventHandler().SelectCursor(location, result).value().type(), ui::mojom::blink::CursorType::kSouthEastResize);
}
...
TEST_F(EventHandlerTest, implicitSend)
{
    ...
    WebKeyboardEvent e { WebInputEvent::Type::kRawKeyDown, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests() };
...
{
    SetHtmlInnerHTML("<style>*:hover { color: red; }</style>"
                     "<div>foo</div>");
    GetDocument().GetSettings()->SetScriptEnabled(true);
    Element* script = GetDocument().CreateRawElement(html_names::kScriptTag);
    script->setInnerHTML("document.addEventListener('contextmenu', event => "
                         "event.preventDefault());");
    GetDocument().body()->AppendChild(script);
    GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kTest);
    GetDocument().GetFrame()->Selection().SetSelection(
        SelectionInDOMTree::Builder().Collapse(Position(GetDocument().body(), 0)).Build(), SetSelectionOptions());
    WebMouseEvent mouse_down_event(WebMouseEvent::Type::kMouseDown, gfx::PointF(0, 0), gfx::PointF(100, 200), WebPointerProperties::Button::kRight, 1,
        WebInputEvent::Modifiers::kRightButtonDown, base::TimeTicks::Now());
    EXPECT_EQ(WebInputEventResult::kHandledApplication, GetDocument().GetFrame()->GetEventHandler().SendContextMenuEvent(mouse_down_event));
}
...
TEST_F(EventHandlerTest, SelectionOnDoublePress)
{
    ...
    GetDocument().GetFrame()->GetEventHandler().HandleGestureEvent(single_tap_down_event);
    GetDocument().GetFrame()->GetEventHandler().HandleGestureEvent(single_tap_event);
    GetDocument().GetFrame()->GetEventHandler().HandleGestureEvent(double_tap_down_event);
...
TEST_F(EventHandlerTest, HandleNotShownOnMouseEvents)
{
    ...
    MousePressEventBuilder left_mouse_press_event(gfx::Point(200, 200), 1, WebPointerProperties::Button::kLeft);
    GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(left_mouse_press_event);

    ASSERT_TRUE(Selection().GetSelectionInDOMTree().IsCaret());
    ASSERT_FALSE(Selection().IsHandleVisible());

    MousePressEventBuilder right_mouse_press_event(gfx::Point(200, 200), 1, WebPointerProperties::Button::kRight);
    GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(right_mouse_press_event);

    ASSERT_TRUE(Selection().GetSelectionInDOMTree().IsCaret());
    ASSERT_FALSE(Selection().IsHandleVisible());

    MousePressEventBuilder double_click_mouse_press_event(gfx::Point(200, 200), 2, WebPointerProperties::Button::kLeft);
    GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(double_click_mouse_press_event);

    ASSERT_TRUE(Selection().GetSelectionInDOMTree().IsRange());
    ASSERT_FALSE(Selection().IsHandleVisible());

    MousePressEventBuilder triple_click_mouse_press_event(gfx::Point(200, 200), 3, WebPointerProperties::Button::kLeft);
    GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(triple_click_mouse_press_event);
...
TEST_F(EventHandlerTest, TripleClickUserSelectNoneParagraphWithSelectableChildren)
{
    ...
    MousePressEventBuilder triple_click_mouse_press_event(gfx::Point(10, 10), 3, WebPointerProperties::Button::kLeft);
    GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(triple_click_mouse_press_event);
    ...
    EXPECT_EQ(R"HTML(<div style="user-select:none">
        ...
        SelectionSample::GetSelectionText(*GetDocument().body(), Selection().GetSelectionInDOMTree()));
}

TEST_F(EventHandlerTest, MisspellingContextMenuEvent)
{
    if (GetDocument().GetFrame()->GetEditor().Behavior().ShouldSelectOnContextualMenuClick())
        return;
    ...
    GetDocument().GetFrame()->GetEventHandler().ShowNonLocatedContextMenu(nullptr, kMenuSourceTouchHandle);
...
{
    SetHtmlInnerHTML("<div style='display:contents' contenteditable='true'>TEXT</div>");
...
TEST_F(EventHandlerTest, GestureHandlingForHeldBackTouchPointer)
{
    ...
    WebPointerEvent pointer_down_2 = CreateMinimalTouchPointerEvent(WebInputEvent::Type::kPointerDown, gfx::PointF(10, 10));
    pointer_down_2.unique_touch_event_id = pointer_id_2;
    GetDocument().GetFrame()->GetEventHandler().HandlePointerEvent(pointer_down_2, Vector<WebPointerEvent>(), Vector<WebPointerEvent>());

    WebGestureEvent two_finger_tap = CreateMinimalGestureEvent(WebInputEvent::Type::kGestureTwoFingerTap, gfx::PointF(20, 20));
    two_finger_tap.primary_unique_touch_event_id = pointer_id_1;

    GetDocument().GetFrame()->GetEventHandler().HandleGestureEvent(two_finger_tap);
...
{
    SetHtmlInnerHTML("<style>.box { width: 100px; height: 100px; display: block; }</style>"
                     "<a class='box' href=''>Drag me</a>");

    WebMouseEvent mouse_down_event(WebInputEvent::Type::kMouseDown, gfx::PointF(50, 50), gfx::PointF(50, 50), WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kLeftButtonDown, base::TimeTicks::Now());
    GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(mouse_down_event);

    WebMouseEvent mouse_move_event(WebInputEvent::Type::kMouseMove, gfx::PointF(51, 50), gfx::PointF(51, 50), WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kLeftButtonDown, base::TimeTicks::Now());
    GetDocument().GetFrame()->GetEventHandler().HandleMouseMoveEvent(mouse_move_event, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
    ...
    WebMouseEvent mouse_up_event(WebInputEvent::Type::kMouseUp, gfx::PointF(100, 50), gfx::PointF(200, 250), WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::kNoModifiers, base::TimeTicks::Now());
    GetDocument().GetFrame()->GetEventHandler().DragSourceEndedAt(mouse_up_event, ui::mojom::blink::DragOperation::kNone);
...
TEST_F(EventHandlerTest, FakeMouseMoveNotStartDrag)
{
    ...
    WebMouseEvent fake_mouse_move(WebMouseEvent::Type::kMouseMove, gfx::PointF(618, 298), gfx::PointF(685, 436), WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kLeftButtonDown | WebInputEvent::Modifiers::kRelativeMotionEvent, WebInputEvent::GetStaticTimeStampForTests());
    EXPECT_EQ(WebInputEventResult::kHandledSuppressed,
        GetDocument().GetFrame()->GetEventHandler().HandleMouseMoveEvent(fake_mouse_move, Vector<WebMouseEvent>(), Vector<WebMouseEvent>()));

    EXPECT_EQ(gfx::Point(0, 0), GetDocument().GetFrame()->GetEventHandler().DragDataTransferLocationForTesting());
}
...
public:
    ...
    void UpdateTooltipUnderCursor(LocalFrame&, const String& str, TextDirection) override
    {
    ...
    void UpdateTooltipFromKeyboard(LocalFrame&, const String& str, TextDirection, const gfx::Rect& bounds) override
    {
        last_tooltip_text_ = str;
    ...
    const String& LastToolTipText()
    {
        return last_tooltip_text_;
    }
    const gfx::Rect& LastToolTipBounds()
    {
        return last_tooltip_bounds_;
    }
...
public:
    ...
    const String& LastToolTipText()
    {
        return chrome_client_->LastToolTipText();
    }
    const gfx::Rect& LastToolTipBounds()
    {
...
TEST_F(EventHandlerTooltipTest, mouseLeaveClearsTooltip)
{
    ...
    SetHtmlInnerHTML("<style>.box { width: 100%; height: 100%; }</style>"
                     "<img src='image.png' class='box' title='tooltip'>link</img>");

    EXPECT_EQ(WTF::String(), LastToolTipText());

    WebMouseEvent mouse_move_event(WebInputEvent::Type::kMouseMove, gfx::PointF(51, 50), gfx::PointF(51, 50), WebPointerProperties::Button::kNoButton, 0,
        WebInputEvent::kNoModifiers, base::TimeTicks::Now());
    GetDocument().GetFrame()->GetEventHandler().HandleMouseMoveEvent(mouse_move_event, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());

    EXPECT_EQ("tooltip", LastToolTipText());

    WebMouseEvent mouse_leave_event(WebInputEvent::Type::kMouseLeave, gfx::PointF(0, 0), gfx::PointF(0, 0), WebPointerProperties::Button::kNoButton, 0,
        WebInputEvent::kNoModifiers, base::TimeTicks::Now());
...
#if BUILDFLAG(IS_MAC)
#define MAYBE_FocusSetFromTabUpdatesTooltip DISABLED_FocusSetFromTabUpdatesTooltip
#else
...
// macOS doesn't have keyboard-triggered tooltips.
#if BUILDFLAG(IS_MAC)
#define MAYBE_FocusSetFromAccessKeyUpdatesTooltip DISABLED_FocusSetFromAccessKeyUpdatesTooltip
#else
#define MAYBE_FocusSetFromAccessKeyUpdatesTooltip FocusSetFromAccessKeyUpdatesTooltip
#endif
...
TEST_F(EventHandlerTooltipTest, MAYBE_FocusSetFromAccessKeyUpdatesTooltip)
{
    ...
    WebKeyboardEvent e { WebInputEvent::Type::kRawKeyDown, WebInputEvent::kAltKey, WebInputEvent::GetStaticTimeStampForTests() };
...
// macOS doesn't have keyboard-triggered tooltips.
#if BUILDFLAG(IS_MAC)
#define MAYBE_FocusSetFromMouseDoesntUpdateTooltip DISABLED_FocusSetFromMouseDoesntUpdateTooltip
#else
#define MAYBE_FocusSetFromMouseDoesntUpdateTooltip FocusSetFromMouseDoesntUpdateTooltip
#endif
...
TEST_F(EventHandlerTooltipTest, MAYBE_FocusSetFromMouseDoesntUpdateTooltip)
{
    ...
    Element* element = GetDocument().getElementById(AtomicString("b"));
    gfx::PointF mouse_press_point = gfx::PointF(element->BoundsInWidget().CenterPoint());
    WebMouseEvent mouse_press_event(WebInputEvent::Type::kMouseDown, mouse_press_point, mouse_press_point, WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kLeftButtonDown, base::TimeTicks::Now());
    GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(mouse_press_event);
...
// macOS doesn't have keyboard-triggered tooltips.
#if BUILDFLAG(IS_MAC)
#define MAYBE_FocusSetFromScriptDoesntUpdateTooltip DISABLED_FocusSetFromScriptDoesntUpdateTooltip
#else
#define MAYBE_FocusSetFromScriptDoesntUpdateTooltip FocusSetFromScriptDoesntUpdateTooltip
#endif
...
// macOS doesn't have keyboard-triggered tooltips.
#if BUILDFLAG(IS_MAC)
#define MAYBE_FocusSetScriptInitiatedFromKeypressUpdatesTooltip DISABLED_FocusSetScriptInitiatedFromKeypressUpdatesTooltip
#else
#define MAYBE_FocusSetScriptInitiatedFromKeypressUpdatesTooltip FocusSetScriptInitiatedFromKeypressUpdatesTooltip
#endif
// Moving the focus with a keypress that leads to a script being called
// should trigger a tooltip update.
TEST_F(EventHandlerTooltipTest, MAYBE_FocusSetScriptInitiatedFromKeypressUpdatesTooltip)
{
    GetDocument().GetSettings()->SetScriptEnabled(true);
    SetHtmlInnerHTML(
        R"HTML(
    ...
    // still trigger a tooltip update. The browser-side TooltipController will
    // handle this case.
    WebKeyboardEvent e2 { WebInputEvent::Type::kRawKeyDown, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests() };
...
// macOS doesn't have keyboard-triggered tooltips.
#if BUILDFLAG(IS_MAC)
#define MAYBE_FocusSetFromScriptClearsKeyboardTriggeredTooltip DISABLED_FocusSetFromScriptClearsKeyboardTriggeredTooltip
#else
#define MAYBE_FocusSetFromScriptClearsKeyboardTriggeredTooltip FocusSetFromScriptClearsKeyboardTriggeredTooltip
#endif
...
TEST_F(EventHandlerTooltipTest, MAYBE_FocusSetFromScriptClearsKeyboardTriggeredTooltip)
{
    SetHtmlInnerHTML(
        R"HTML(
        <button id='b1' title='my tooltip 1'>button 1</button>
    ...
    EXPECT_EQ("my tooltip 1", LastToolTipText());
    EXPECT_EQ(GetDocument().getElementById(AtomicString("b1"))->BoundsInWidget(), LastToolTipBounds());
...
TEST_F(EventHandlerTooltipTest, FocusSetFromScriptDoesntClearCursorTriggeredTooltip)
{
    SetHtmlInnerHTML(
        R"HTML(
...
protected:
    void SetUp() override
    {
        chrome_client_ = MakeGarbageCollected<UnbufferedInputEventsTrackingChromeClient>();
        SetupPageWithClients(chrome_client_);
    }
...
{
    GetDocument().GetSettings()->SetScriptEnabled(true);
    SetHtmlInnerHTML("<canvas style='width: 100px; height: 100px' id='first' "
                     "onpointermove='return;'>");

    auto& canvas = To<HTMLCanvasElement>(*GetDocument().getElementById(AtomicString("first")));

    ASSERT_FALSE(chrome_client_->ReceivedRequestForUnbufferedInput());

    WebMouseEvent mouse_press_event(WebInputEvent::Type::kMouseDown, gfx::PointF(51, 50), gfx::PointF(51, 50), WebPointerProperties::Button::kLeft, 0,
        WebInputEvent::kNoModifiers, base::TimeTicks::Now());
    GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(mouse_press_event);
    ASSERT_FALSE(chrome_client_->ReceivedRequestForUnbufferedInput());
...
TEST_F(EventHandlerSimTest, MouseUpOffScrollbarGeneratesScrollEnd)
{
    ...
    // allow hit testing.
    bool scrollbar_theme_allows_hit_test = GetDocument().GetPage()->GetScrollbarTheme().AllowsHitTest();

    const gfx::PointF scrollbar_forward_track(795, 560);
    WebMouseEvent mouse_down(WebInputEvent::Type::kMouseDown, scrollbar_forward_track, scrollbar_forward_track, WebPointerProperties::Button::kLeft, 0,
        WebInputEvent::kNoModifiers, base::TimeTicks::Now());
    ...
    if (scrollbar_theme_allows_hit_test) {
        ...
        EXPECT_EQ(GetWebFrameWidget().GetInjectedScrollEvents()[0]->Event().GetType(), WebInputEvent::Type::kGestureScrollBegin);
        EXPECT_EQ(GetWebFrameWidget().GetInjectedScrollEvents()[1]->Event().GetType(), WebInputEvent::Type::kGestureScrollUpdate);
    } else {
        EXPECT_EQ(GetWebFrameWidget().GetInjectedScrollEvents().size(), 0u);
    }

    const gfx::PointF middle_of_page(100, 100);
    WebMouseEvent mouse_move(WebInputEvent::Type::kMouseMove, middle_of_page, middle_of_page, WebPointerProperties::Button::kLeft, 0,
        WebInputEvent::kNoModifiers, base::TimeTicks::Now());
    GetDocument().GetFrame()->GetEventHandler().HandleMouseMoveEvent(mouse_move, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
    ...
    WebMouseEvent mouse_up(WebInputEvent::Type::kMouseUp, middle_of_page, middle_of_page, WebPointerProperties::Button::kLeft, 0, WebInputEvent::kNoModifiers,
        base::TimeTicks::Now());
    ...
    if (scrollbar_theme_allows_hit_test) {
        ...
        EXPECT_EQ(GetWebFrameWidget().GetInjectedScrollEvents()[2]->Event().GetType(), WebInputEvent::Type::kGestureScrollEnd);
    } else {
        EXPECT_EQ(GetWebFrameWidget().GetInjectedScrollEvents().size(), 0u);
    }
}
...
TEST_F(EventHandlerSimTest, MouseUpOnlyOnScrollbar)
{
    ...
    const gfx::PointF middle_of_page(100, 100);
    WebMouseEvent mouse_down(WebInputEvent::Type::kMouseDown, middle_of_page, middle_of_page, WebPointerProperties::Button::kLeft, 0,
        WebInputEvent::kNoModifiers, base::TimeTicks::Now());
    GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(mouse_down);
    ...
    const gfx::PointF scrollbar_forward_track(795, 560);
    WebMouseEvent mouse_move(WebInputEvent::Type::kMouseMove, scrollbar_forward_track, scrollbar_forward_track, WebPointerProperties::Button::kLeft, 0,
        WebInputEvent::kNoModifiers, base::TimeTicks::Now());
    GetDocument().GetFrame()->GetEventHandler().HandleMouseMoveEvent(mouse_move, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
    ...
    WebMouseEvent mouse_up(WebInputEvent::Type::kMouseUp, scrollbar_forward_track, scrollbar_forward_track, WebPointerProperties::Button::kLeft, 0,
        WebInputEvent::kNoModifiers, base::TimeTicks::Now());
    GetDocument().GetFrame()->GetEventHandler().HandleMouseReleaseEvent(mouse_up);
...
TEST_F(EventHandlerSimTest, RightClickNoGestures)
{
    ...
    const gfx::PointF scrollbar_forward_track(795, 560);
    WebMouseEvent mouse_down(WebInputEvent::Type::kMouseDown, scrollbar_forward_track, scrollbar_forward_track, WebPointerProperties::Button::kRight, 0,
        WebInputEvent::kNoModifiers, base::TimeTicks::Now());
    GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(mouse_down);

    EXPECT_EQ(GetWebFrameWidget().GetInjectedScrollEvents().size(), 0u);

    WebMouseEvent mouse_up(WebInputEvent::Type::kMouseUp, scrollbar_forward_track, scrollbar_forward_track, WebPointerProperties::Button::kRight, 0,
        WebInputEvent::kNoModifiers, base::TimeTicks::Now());
    GetDocument().GetFrame()->GetEventHandler().HandleMouseReleaseEvent(mouse_up);
...
TEST_F(EventHandlerSimTest, MAYBE_GestureTapWithScrollSnaps)
{
    ...
    const Vector<std::unique_ptr<blink::WebCoalescedInputEvent>>& data = GetWebFrameWidget().GetInjectedScrollEvents();
    EXPECT_EQ(data[0]->Event().GetType(), WebInputEvent::Type::kGestureScrollBegin);
    EXPECT_EQ(data[1]->Event().GetType(), WebInputEvent::Type::kGestureScrollUpdate);
    EXPECT_EQ(data[2]->Event().GetType(), WebInputEvent::Type::kGestureScrollEnd);
    const WebGestureEvent& gsb = static_cast<const WebGestureEvent&>(data[0]->Event());
    const WebGestureEvent& gsu = static_cast<const WebGestureEvent&>(data[1]->Event());
    const WebGestureEvent& gse = static_cast<const WebGestureEvent&>(data[2]->Event());
    ...
    Element* scrollable_div = GetDocument().getElementById(AtomicString("container"));
    ScrollableArea* scrollable_area = scrollable_div->GetLayoutBox()->GetScrollableArea();
    ...
    EXPECT_FALSE(scrollable_area->ExistingScrollAnimator()->HasRunningAnimation());
}
...
TEST_F(EventHandlerTest, MouseLeaveResetsUnknownState)
{
    ...
    WebMouseEvent mouse_down_event(WebMouseEvent::Type::kMouseDown, gfx::PointF(262, 29), gfx::PointF(329, 67), WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kLeftButtonDown, WebInputEvent::GetStaticTimeStampForTests());
    GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(mouse_down_event);
    EXPECT_FALSE(GetDocument().GetFrame()->GetEventHandler().IsMousePositionUnknown());

    WebMouseEvent mouse_leave_event(WebMouseEvent::Type::kMouseLeave, gfx::PointF(262, 29), gfx::PointF(329, 67), WebPointerProperties::Button::kNoButton, 1,
        WebInputEvent::Modifiers::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests());
    GetDocument().GetFrame()->GetEventHandler().HandleMouseLeaveEvent(mouse_leave_event);
    EXPECT_TRUE(GetDocument().GetFrame()->GetEventHandler().IsMousePositionUnknown());
}
...
TEST_F(EventHandlerSimTest, MouseLeaveIFrameResets)
{
    ...
    Compositor().BeginFrame();
    WebMouseEvent mouse_move_inside_event(WebMouseEvent::Type::kMouseMove, gfx::PointF(100, 229), gfx::PointF(100, 229),
        WebPointerProperties::Button::kNoButton, 0, WebInputEvent::Modifiers::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests());
    GetDocument().GetFrame()->GetEventHandler().HandleMouseMoveEvent(mouse_move_inside_event, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
    EXPECT_FALSE(GetDocument().GetFrame()->GetEventHandler().IsMousePositionUnknown());
    auto* child_frame = To<HTMLIFrameElement>(GetDocument().getElementById(AtomicString("frame")));
    child_frame->contentDocument()->UpdateStyleAndLayout(DocumentUpdateReason::kTest);
    EXPECT_TRUE(GetDocument().GetFrame()->Tree().FirstChild());
    EXPECT_FALSE(To<LocalFrame>(GetDocument().GetFrame()->Tree().FirstChild())->GetEventHandler().IsMousePositionUnknown());

    WebMouseEvent mouse_move_outside_event(WebMouseEvent::Type::kMouseMove, gfx::PointF(300, 29), gfx::PointF(300, 29), WebPointerProperties::Button::kNoButton,
        0, WebInputEvent::Modifiers::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests());
    GetDocument().GetFrame()->GetEventHandler().HandleMouseMoveEvent(mouse_move_outside_event, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
    EXPECT_FALSE(GetDocument().GetFrame()->GetEventHandler().IsMousePositionUnknown());
    EXPECT_TRUE(GetDocument().GetFrame()->Tree().FirstChild());
    EXPECT_TRUE(To<LocalFrame>(GetDocument().GetFrame()->Tree().FirstChild())->GetEventHandler().IsMousePositionUnknown());
}
...
TEST_F(EventHandlerSimTest, CursorStyleBeforeStartDragging)
{
    ...
    WebMouseEvent mouse_down_event(WebMouseEvent::Type::kMouseDown, gfx::PointF(150, 50), gfx::PointF(150, 50), WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kLeftButtonDown, WebInputEvent::GetStaticTimeStampForTests());
    GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(mouse_down_event);

    WebMouseEvent mouse_move_event(WebMouseEvent::Type::kMouseMove, gfx::PointF(151, 50), gfx::PointF(151, 50), WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kLeftButtonDown, WebInputEvent::GetStaticTimeStampForTests());
    GetDocument().GetFrame()->GetEventHandler().HandleMouseMoveEvent(mouse_move_event, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
    EXPECT_EQ(ui::mojom::blink::CursorType::kHelp, GetDocument().GetFrame()->GetChromeClient().LastSetCursorForTesting().type());
}
...
TEST_F(EventHandlerSimTest, TapActiveInFrame)
{
    ...
    auto* iframe_element = To<HTMLIFrameElement>(GetDocument().getElementById(AtomicString("iframe")));
...
TEST_F(EventHandlerSimTest, TestUpdateHoverAfterCompositorScrollAtBeginFrame)
{
    ...
    WebView().MainFrameWidget()->ApplyViewportChangesForTesting(
        { gfx::Vector2dF(), gfx::Vector2dF(), 1.0f, false, 0, 0, cc::BrowserControlsState::kBoth, true });
...
TEST_F(EventHandlerSimTest, TestUpdateHoverAfterJSScrollAtBeginFrame)
{
    ...
    ScrollableArea* scrollable_area = GetDocument().GetLayoutView()->GetScrollableArea();
    bool finished = false;
    scrollable_area->SetScrollOffset(ScrollOffset(0, 1000), mojom::blink::ScrollType::kProgrammatic, mojom::blink::ScrollBehavior::kSmooth,
        ScrollableArea::ScrollCallback(
            WTF::BindOnce([](bool* finished, ScrollableArea::ScrollCompletionMode) { *finished = true; }, WTF::Unretained(&finished))));
    Compositor().BeginFrame();
    LocalFrameView* frame_view = GetDocument().View();
...
TEST_F(EventHandlerSimTest, LargeCustomCursorIntersectsViewport)
{
    WebView().MainFrameViewWidget()->Resize(gfx::Size(800, 600));
    SimRequest request("https://example.com/test.html", "text/html");
    SimSubresourceRequest cursor_request("https://example.com/100x100.png", "image/png");
    LoadURL("https://example.com/test.html");
    request.Complete(
        R"HTML(
        <!DOCTYPE html>
    ...
    cursor_request.Complete(*test::ReadFromFile(test::CoreTestDataPath("notifications/100x100.png")));
    ...
    struct TestCase {
        ...
        {
            return base::StringPrintf("point: (%s), cursor-scale: %g, device-scale: %g, custom?: %d", point.ToString().c_str(),
                cursor_accessibility_scale_factor, device_scale_factor, custom_expected);
        }
    } test_cases[] = {
    ...
    for (const TestCase& test_case : test_cases) {
        ...
        event_handler.set_cursor_accessibility_scale_factor(test_case.cursor_accessibility_scale_factor);
        WebMouseEvent mouse_move_event(WebMouseEvent::Type::kMouseMove, test_case.point, test_case.point, WebPointerProperties::Button::kNoButton, 0, 0,
            WebInputEvent::GetStaticTimeStampForTests());
        event_handler.HandleMouseMoveEvent(mouse_move_event, {}, {});
        const ui::Cursor& cursor = GetDocument().GetFrame()->GetChromeClient().LastSetCursorForTesting();
        const ui::mojom::blink::CursorType expected_type
            = test_case.custom_expected ? ui::mojom::blink::CursorType::kCustom : ui::mojom::blink::CursorType::kPointer;
...
TEST_F(EventHandlerSimTest, SmallCustomCursorIntersectsViewport)
{
    WebView().MainFrameViewWidget()->Resize(gfx::Size(800, 600));
    SimRequest request("https://example.com/test.html", "text/html");
    SimSubresourceRequest cursor_request("https://example.com/48x48.png", "image/png");
    LoadURL("https://example.com/test.html");
    request.Complete(
        R"HTML(
        <!DOCTYPE html>
    ...
    cursor_request.Complete(*test::ReadFromFile(test::CoreTestDataPath("notifications/48x48.png")));

    Compositor().BeginFrame();

    // Move the cursor so no part of it intersects the viewport.
    {
        WebMouseEvent mouse_move_event(WebMouseEvent::Type::kMouseMove, gfx::PointF(25, 25), gfx::PointF(25, 25), WebPointerProperties::Button::kNoButton, 0, 0,
            WebInputEvent::GetStaticTimeStampForTests());
        GetDocument().GetFrame()->GetEventHandler().HandleMouseMoveEvent(mouse_move_event, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
    ...
    {
        WebMouseEvent mouse_move_event(WebMouseEvent::Type::kMouseMove, gfx::PointF(23, 23), gfx::PointF(23, 23), WebPointerProperties::Button::kNoButton, 0, 0,
            WebInputEvent::GetStaticTimeStampForTests());
        GetDocument().GetFrame()->GetEventHandler().HandleMouseMoveEvent(mouse_move_event, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
...
TEST_F(EventHandlerSimTest, NeverExposeKeyboardEvent)
{
    ...
    WebView().MainFrameImpl()->LocalRootFrameWidget()->SetDisplayMode(blink::mojom::DisplayMode::kFullscreen);
    request.Complete(R"HTML(
    <!DOCTYPE html>
    <style>
    body {
...
TEST_F(EventHandlerSimTest, NotExposeKeyboardEvent)
{
    ...
    GetDocument().SetFocusedElement(input.Unwrap<Element>(), FocusParams(SelectionBehaviorOnFocus::kNone, mojom::blink::FocusType::kNone, nullptr));
...
TEST_F(EventHandlerSimTest, DoNotScrollWithTouchpadIfOverflowIsHidden)
{
    ...
    WebGestureEvent scroll_begin_event(
        WebInputEvent::Type::kGestureScrollBegin, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests(), blink::WebGestureDevice::kTouchpad);
    scroll_begin_event.SetPositionInWidget(gfx::PointF(10, 10));
    scroll_begin_event.SetPositionInScreen(gfx::PointF(10, 10));

    WebGestureEvent scroll_update_event(WebInputEvent::Type::kGestureScrollUpdate, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests(),
    ...
    WebGestureEvent scroll_end_event(
        WebInputEvent::Type::kGestureScrollEnd, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests(), blink::WebGestureDevice::kTouchpad);
    ...
    Compositor().BeginFrame();
    EXPECT_EQ(0, GetDocument().getElementById(AtomicString("outer"))->scrollLeft());
}
...
TEST_F(EventHandlerSimTest, ElementTargetedGestureScroll)
{
    ...
    Element* const scroller = GetDocument().getElementById(AtomicString("scroller"));
    ...
    WebGestureEvent gesture_scroll_begin { WebInputEvent::Type::kGestureScrollBegin, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests(),
        WebGestureDevice::kTouchscreen };
    gesture_scroll_begin.data.scroll_begin.delta_x_hint = 0;
    gesture_scroll_begin.data.scroll_begin.delta_y_hint = -delta_y;
    DispatchElementTargetedGestureScroll(gesture_scroll_begin);

    WebGestureEvent gesture_scroll_update { WebInputEvent::Type::kGestureScrollUpdate, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests(),
        WebGestureDevice::kTouchscreen };
    ...
    // did not).
    ScrollableArea* scrollable_area = scroller->GetLayoutBox()->GetScrollableArea();
    gesture_scroll_begin.data.scroll_begin.scrollable_area_element_id = scrollable_area->GetScrollElementId().GetInternalValue();
...
TEST_F(EventHandlerSimTest, ElementTargetedGestureScrollIFrame)
{
    ...
    auto* const iframe = To<HTMLFrameElementBase>(GetDocument().getElementById(AtomicString("iframe")));
    FrameView* child_frame_view = iframe->GetLayoutEmbeddedContent()->ChildFrameView();
...
TEST_F(EventHandlerSimTest, ElementTargetedGestureScrollViewport)
{
    ...
    const VisualViewport& visual_viewport = GetDocument().GetPage()->GetVisualViewport();
    const ScrollableArea& layout_viewport = *GetDocument().View()->LayoutViewport();
    ...
    gesture_scroll_begin.data.scroll_begin.scrollable_area_element_id = layout_viewport.GetScrollElementId().GetInternalValue();
...
TEST_F(EventHandlerSimTest, SelecteTransformedTextWhenCapturing)
{
    ...
    WebMouseEvent mouse_down_event(WebInputEvent::Type::kMouseDown, gfx::PointF(100, 20), gfx::PointF(0, 0), WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kLeftButtonDown, WebInputEvent::GetStaticTimeStampForTests());
    GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(mouse_down_event);
    ...
    GetDocument().GetFrame()->GetEventHandler().SetPointerCapture(PointerEventFactory::kMouseId, target);

    WebMouseEvent mouse_move_event(WebInputEvent::Type::kMouseMove, gfx::PointF(258, 20), gfx::PointF(0, 0), WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kLeftButtonDown, WebInputEvent::GetStaticTimeStampForTests());
    GetDocument().GetFrame()->GetEventHandler().HandleMouseMoveEvent(mouse_move_event, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());

    WebMouseEvent mouse_up_event(WebMouseEvent::Type::kMouseUp, gfx::PointF(258, 20), gfx::PointF(0, 0), WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests());
    GetDocument().GetFrame()->GetEventHandler().HandleMouseReleaseEvent(mouse_up_event);
...
TEST_F(EventHandlerSimTest, MouseRightButtonDownMoveToIFrame)
{
    ...
    frame_resource.Complete("<!DOCTYPE html>");
    Compositor().BeginFrame();
    WebMouseEvent mouse_down_outside_event(WebMouseEvent::Type::kMouseDown, gfx::PointF(300, 29), gfx::PointF(300, 29), WebPointerProperties::Button::kRight, 0,
        WebInputEvent::Modifiers::kRightButtonDown, WebInputEvent::GetStaticTimeStampForTests());
    WebView().MainFrameWidget()->HandleInputEvent(WebCoalescedInputEvent(mouse_down_outside_event, ui::LatencyInfo()));

    WebMouseEvent mouse_move_outside_event(WebMouseEvent::Type::kMouseMove, gfx::PointF(300, 29), gfx::PointF(300, 29), WebPointerProperties::Button::kRight, 0,
        WebInputEvent::Modifiers::kRightButtonDown, WebInputEvent::GetStaticTimeStampForTests());
    WebView().MainFrameWidget()->HandleInputEvent(WebCoalescedInputEvent(mouse_move_outside_event, ui::LatencyInfo()));

    WebMouseEvent mouse_move_inside_event(WebMouseEvent::Type::kMouseMove, gfx::PointF(100, 229), gfx::PointF(100, 229), WebPointerProperties::Button::kRight,
        0, WebInputEvent::Modifiers::kRightButtonDown, WebInputEvent::GetStaticTimeStampForTests());
    WebView().MainFrameWidget()->HandleInputEvent(WebCoalescedInputEvent(mouse_move_inside_event, ui::LatencyInfo()));
    EXPECT_FALSE(GetDocument().GetFrame()->GetEventHandler().IsMousePositionUnknown());
    EXPECT_FALSE(To<LocalFrame>(GetDocument().GetFrame()->Tree().FirstChild())->GetEventHandler().IsMousePositionUnknown());
}
...
TEST_F(EventHandlerSimTest, PenDraggingOnElementActive)
{
    ...
    Compositor().BeginFrame();
    WebMouseEvent pen_down(WebMouseEvent::Type::kMouseDown, gfx::PointF(100, 100), gfx::PointF(100, 100), WebPointerProperties::Button::kLeft, 0,
        WebInputEvent::Modifiers::kLeftButtonDown, WebInputEvent::GetStaticTimeStampForTests());
    pen_down.pointer_type = blink::WebPointerProperties::PointerType::kPen;
    WebView().MainFrameWidget()->HandleInputEvent(WebCoalescedInputEvent(pen_down, ui::LatencyInfo()));

    WebMouseEvent pen_move(WebMouseEvent::Type::kMouseMove, gfx::PointF(100, 100), gfx::PointF(100, 100), WebPointerProperties::Button::kLeft, 0,
        WebInputEvent::Modifiers::kLeftButtonDown, WebInputEvent::GetStaticTimeStampForTests());
    pen_move.pointer_type = blink::WebPointerProperties::PointerType::kPen;
    // Send first mouse move to update mouse event sates.
    WebView().MainFrameWidget()->HandleInputEvent(WebCoalescedInputEvent(pen_move, ui::LatencyInfo()));

    // Send another mouse move again to update active element to verify mouse
    // event states.
    WebView().MainFrameWidget()->HandleInputEvent(WebCoalescedInputEvent(pen_move, ui::LatencyInfo()));

    EXPECT_EQ(GetDocument().GetActiveElement(), GetDocument().getElementById(AtomicString("target")));
}
...
TEST_F(EventHandlerSimTest, TestNoCrashOnMouseWheelZeroDelta)
{
    ...
    WebMouseWheelEvent wheel_event(
        blink::WebInputEvent::Type::kMouseWheel, blink::WebInputEvent::kNoModifiers, blink::WebInputEvent::GetStaticTimeStampForTests());
    ...
    EXPECT_EQ("received wheel event, deltaX: 0 deltaY: 0", element.InnerHTML().Utf8());
...
TEST_F(EventHandlerSimTest, TestWheelEventsWithDifferentPhases)
{
    ...
    GetDocument().GetFrame()->GetEventHandler().HandleWheelEvent(wheel_event);
    EXPECT_EQ("received wheel event, deltaX: 0 deltaY: 1", element->innerHTML().Utf8());

    wheel_event.delta_y = -2;
    wheel_event.phase = WebMouseWheelEvent::kPhaseChanged;
    element->setInnerHTML("no wheel event");
    GetDocument().GetFrame()->GetEventHandler().HandleWheelEvent(wheel_event);
    EXPECT_EQ("received wheel event, deltaX: 0 deltaY: 2", element->innerHTML().Utf8());

    wheel_event.delta_y = -3;
    wheel_event.phase = WebMouseWheelEvent::kPhaseChanged;
    element->setInnerHTML("no wheel event");
    GetDocument().GetFrame()->GetEventHandler().HandleWheelEvent(wheel_event);
    EXPECT_EQ("received wheel event, deltaX: 0 deltaY: 3", element->innerHTML().Utf8());

    wheel_event.delta_y = -4;
    wheel_event.phase = WebMouseWheelEvent::kPhaseStationary;
    element->setInnerHTML("no wheel event");
    GetDocument().GetFrame()->GetEventHandler().HandleWheelEvent(wheel_event);
    EXPECT_EQ("received wheel event, deltaX: 0 deltaY: 4", element->innerHTML().Utf8());

    wheel_event.delta_y = -5;
    wheel_event.phase = WebMouseWheelEvent::kPhaseChanged;
    element->setInnerHTML("no wheel event");
    GetDocument().GetFrame()->GetEventHandler().HandleWheelEvent(wheel_event);
    EXPECT_EQ("received wheel event, deltaX: 0 deltaY: 5", element->innerHTML().Utf8());
...
TEST_F(EventHandlerSimTest, TestScrollendFiresOnKeyUpAfterScroll)
{
    ...
    GetDocument().getElementById(AtomicString("scroller"))->Focus(FocusOptions::Create());
    // Send first keyDown.
    e.windows_key_code = VKEY_DOWN;
    e.SetType(WebInputEvent::Type::kKeyDown);
    GetDocument().GetFrame()->GetEventHandler().KeyEvent(e);
    ...
    EXPECT_EQ(GetDocument().getElementById(AtomicString("log"))->innerHTML().Utf8(), "");
    ...
    Compositor().BeginFrame();
    EXPECT_EQ(GetDocument().getElementById(AtomicString("log"))->innerHTML().Utf8(), "scrollend");
}
...
TEST_F(EventHandlerSimTest, DiscardEventsToRecentlyMovedIframe)
{
    ...
    feature_list.InitAndEnableFeatureWithParameters(features::kDiscardInputEventsToRecentlyMovedFrames, field_trial_params);

    WebView().MainFrameViewWidget()->Resize(gfx::Size(800, 600));
    SimRequest main_resource("https://example.com/test.html", "text/html");
    SimRequest frame_resource("https://cross-origin.com/iframe.html", "text/html");
    LoadURL("https://example.com/test.html");
    main_resource.Complete(R"HTML(
    <!DOCTYPE html>
    ...
    WebInputEventResult event_result = GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(WebMouseEvent(WebInputEvent::Type::kMouseDown,
        gfx::PointF(100, 50), gfx::PointF(100, 50), WebPointerProperties::Button::kLeft, 1, WebInputEvent::Modifiers::kLeftButtonDown, base::TimeTicks::Now()));
    EXPECT_NE(event_result, WebInputEventResult::kHandledSuppressed);
    event_result = GetDocument().GetFrame()->GetEventHandler().HandleMouseReleaseEvent(WebMouseEvent(WebInputEvent::Type::kMouseUp, gfx::PointF(100, 50),
        gfx::PointF(100, 50), WebPointerProperties::Button::kLeft, 1, WebInputEvent::kNoModifiers, base::TimeTicks::Now()));
    EXPECT_NE(event_result, WebInputEventResult::kHandledSuppressed);

    Element* iframe = GetDocument().getElementById(AtomicString::FromUTF8("iframe"));
    ASSERT_TRUE(iframe);
    ...
    Compositor().BeginFrame();
    event_result = GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(WebMouseEvent(WebInputEvent::Type::kMouseDown, gfx::PointF(170, 90),
        gfx::PointF(170, 90), WebPointerProperties::Button::kLeft, 1, WebInputEvent::Modifiers::kLeftButtonDown, base::TimeTicks::Now()));
    EXPECT_NE(event_result, WebInputEventResult::kHandledSuppressed);
    event_result = GetDocument().GetFrame()->GetEventHandler().HandleMouseReleaseEvent(WebMouseEvent(WebInputEvent::Type::kMouseUp, gfx::PointF(170, 90),
        gfx::PointF(170, 90), WebPointerProperties::Button::kLeft, 1, WebInputEvent::kNoModifiers, base::TimeTicks::Now()));
    EXPECT_NE(event_result, WebInputEventResult::kHandledSuppressed);
    ...
    Compositor().BeginFrame();
    base::TimeTicks event_time = Compositor().LastFrameTime() + base::Milliseconds(400);

    event_result = GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(WebMouseEvent(WebInputEvent::Type::kMouseDown, gfx::PointF(300, 90),
        gfx::PointF(300, 90), WebPointerProperties::Button::kLeft, 1, WebInputEvent::Modifiers::kLeftButtonDown, event_time));
    EXPECT_EQ(event_result, WebInputEventResult::kHandledSuppressed);
    event_result = GetDocument().GetFrame()->GetEventHandler().HandleMouseReleaseEvent(WebMouseEvent(WebInputEvent::Type::kMouseUp, gfx::PointF(300, 90),
        gfx::PointF(300, 90), WebPointerProperties::Button::kLeft, 1, WebInputEvent::kNoModifiers, event_time));
    EXPECT_EQ(event_result, WebInputEventResult::kHandledSuppressed);

    // A second click on the same target within 5 seconds of a discarded click
    // should be recorded as "mistakenly discarded".
    EXPECT_FALSE(
        To<HTMLIFrameElement>(iframe)->contentDocument()->Loader()->GetUseCounter().IsCounted(WebFeature::kInputEventToRecentlyMovedIframeMistakenlyDiscarded));
    GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(WebMouseEvent(WebInputEvent::Type::kMouseDown, gfx::PointF(300, 90), gfx::PointF(300, 90),
        WebPointerProperties::Button::kLeft, 1, WebInputEvent::Modifiers::kLeftButtonDown, event_time));
    EXPECT_TRUE(
        To<HTMLIFrameElement>(iframe)->contentDocument()->Loader()->GetUseCounter().IsCounted(WebFeature::kInputEventToRecentlyMovedIframeMistakenlyDiscarded));
}
...
TEST_F(EventHandlerSimTest, ValidClickPointerIdForUnseenPointerEvent)
{
    ...
    WebElement pointer_id_elem = GetDocument().getElementById(AtomicString("pointer_id"));
