...
{
    if (const ComputedStyle* style = node.GetComputedStyleForElementOrLayoutObject()) {
        return style->EffectiveTouchAction();
    }
    return TouchAction::kAuto;
}

TouchAction EffectiveTouchActionAtPointerDown(const WebPointerEvent& event, const Node* pointerdown_node)
{
    DCHECK(event.GetType() == WebInputEvent::Type::kPointerDown);
    return EffectiveTouchActionAtPointer(event, pointerdown_node);
}

TouchAction EffectiveTouchActionAtPointer(const WebPointerEvent& event, const Node* node_at_pointer)
{
    DCHECK(node_at_pointer);

    TouchAction effective_touch_action = ComputeEffectiveTouchAction(*node_at_pointer);
    ...
    if ((effective_touch_action & TouchAction::kPanX) != TouchAction::kNone) {
        ...
        if (LayoutBox::HasHorizontallyScrollableAncestor(node_at_pointer->GetLayoutObject())) {
    ...
    if ((effective_touch_action & TouchAction::kPan) != TouchAction::kNone
        && ((event.pointer_type != WebPointerProperties::PointerType::kPen && event.pointer_type != WebPointerProperties::PointerType::kEraser)
