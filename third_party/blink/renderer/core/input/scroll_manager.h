...
public:
    ...
    bool LogicalScroll(mojom::blink::ScrollDirection, ui::ScrollGranularity, Node* start_node, Node* mouse_press_node, bool scrolling_via_key = false);
    ...
    bool BubblingScroll(mojom::blink::ScrollDirection, ui::ScrollGranularity, Node* starting_node, Node* mouse_press_node, bool scrolling_via_key = false);
    ...
    // scroll direction.
    static bool CanPropagate(const LayoutBox* layout_box, ScrollPropagationDirection direction);

private:
    void RecomputeScrollChain(const Node& start_node, Deque<DOMNodeId>& scroll_chain, bool is_autoscroll);
