...
class CORE_EXPORT PointerEventManager final : public GarbageCollected<PointerEventManager>, public SynchronousMutationObserver {
public:
    ...
    // in this function.
    WebInputEventResult SendMousePointerEvent(Element* target, const WebInputEvent::Type, const WebMouseEvent&, const Vector<WebMouseEvent>& coalesced_events,
        const Vector<WebMouseEvent>& predicted_events, bool skip_click_dispatch);
    ...
    void SendMouseAndPointerBoundaryEvents(Element* entered_element, const WebMouseEvent&);

    WebInputEventResult DirectDispatchMousePointerEvent(Element* target, const WebMouseEvent&, const AtomicString& event_type,
        const Vector<WebMouseEvent>& coalesced_events, const Vector<WebMouseEvent>& predicted_events);

    void SendEffectivePanActionAtPointer(const WebPointerEvent& event, const Node* node_at_pointer);
...
private:
    ...
    // deleted hash value.
    template <typename T> using PointerIdKeyMap = HeapHashMap<int64_t, T, IntWithZeroKeyHashTraits<int64_t>>;
    ...
    public:
        ...
        PointerEventBoundaryEventDispatcher(const PointerEventBoundaryEventDispatcher&) = delete;
        PointerEventBoundaryEventDispatcher& operator=(const PointerEventBoundaryEventDispatcher&) = delete;

    protected:
        void Dispatch(EventTarget*, EventTarget* related_target, const AtomicString&, bool check_for_listener) override;
    ...
    WebInputEventResult CreateAndDispatchPointerEvent(Element* target, const AtomicString& mouse_event_name, const WebMouseEvent&,
        const Vector<WebMouseEvent>& coalesced_events, const Vector<WebMouseEvent>& predicted_events);

    // Returns PointerEventTarget for a WebTouchPoint, hit-testing as necessary.
    event_handling_util::PointerEventTarget ComputePointerEventTarget(const WebPointerEvent&);

    WebInputEventResult DispatchTouchPointerEvent(const WebPointerEvent&, const Vector<WebPointerEvent>& coalesced_events,
        const Vector<WebPointerEvent>& predicted_events, const event_handling_util::PointerEventTarget&);

    // Returns whether the event is consumed or not.
    WebInputEventResult SendTouchPointerEvent(Element*, PointerEvent*, bool hovering);

    void SendBoundaryEvents(EventTarget* exited_target, bool original_exited_target_removed, EventTarget* entered_target, PointerEvent*);
    ...
    void SetLastPointerPositionForFrameBoundary(const WebPointerEvent& event, Element* target);
    ...
    Element* ProcessCaptureAndPositionOfPointerEvent(PointerEvent*, Element* hit_test_target, const WebMouseEvent* = nullptr);

    void RemoveTargetFromPointerCapturingMapping(PointerCapturingMap&, const Element*);
    Element* GetEffectiveTargetForPointerEvent(Element*, PointerId);
    void RemovePointer(PointerEvent*);
    WebInputEventResult DispatchPointerEvent(EventTarget*, PointerEvent*, bool check_for_listener = false);
    void ReleasePointerCapture(PointerId);
    // Returns true if capture target and pending capture target were different.
    bool GetPointerCaptureState(PointerId pointer_id, Element** pointer_capture_target, Element** pending_pointer_capture_target);
    ...
    bool HandleResizerDrag(const WebPointerEvent&, const event_handling_util::PointerEventTarget&);
    ...
    bool prevent_mouse_event_for_pointer_type_[static_cast<size_t>(WebPointerProperties::PointerType::kMaxValue) + 1];
