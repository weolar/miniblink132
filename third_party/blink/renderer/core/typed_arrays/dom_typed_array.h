...
template <typename T, typename V8TypedArray, bool clamped = false> class DOMTypedArray final : public DOMArrayBufferView {
...
public:
    ...
    static ThisType* Create(DOMArrayBufferBase* buffer, size_t byte_offset, size_t length)
    {
    ...
    {
        DOMArrayBuffer* buffer = DOMArrayBuffer::CreateOrNull(length, sizeof(ValueType));
        return buffer ? Create(buffer, 0, length) : nullptr;
    }

    static ThisType* CreateOrNull(base::span<const ValueType> array)
    {
        DOMArrayBuffer* buffer = DOMArrayBuffer::CreateOrNull(base::as_bytes(array));
        return buffer ? Create(buffer, 0, array.size()) : nullptr;
    }

    static ThisType* CreateUninitializedOrNull(size_t length)
    {
        DOMArrayBuffer* buffer = DOMArrayBuffer::CreateUninitializedOrNull(length, sizeof(ValueType));
        return buffer ? Create(buffer, 0, length) : nullptr;
    }

    DOMTypedArray(DOMArrayBufferBase* dom_array_buffer, size_t byte_offset, size_t length)
    ...
    base::span<ValueType> AsSpan() const
    {
        ...
        return UNSAFE_BUFFERS(base::span(static_cast<ValueType*>(Data()), length()));
    }

    base::span<ValueType> AsSpanMaybeShared() const
    {
        // SAFETY: DataMaybeShared() and length() guarantee the span is valid
        return UNSAFE_BUFFERS(base::span(static_cast<ValueType*>(DataMaybeShared()), length()));
    }

    size_t length() const
    {
        return !IsDetached() ? raw_length_ : 0;
    }
...
private:
    ...
    static bool VerifySubRange(const DOMArrayBufferBase* buffer, size_t byte_offset, size_t num_elements)
    {
        if (!buffer)
            return false;
...
#define DOMTYPEDARRAY_DECLARE_WRAPPERTYPEINFO(val_t, Type, clamped)                                                                                            \
    template <> const WrapperTypeInfo DOMTypedArray<val_t, v8::Type##Array, clamped>::wrapper_type_info_body_;                                                 \
    template <> const WrapperTypeInfo& DOMTypedArray<val_t, v8::Type##Array, clamped>::wrapper_type_info_;
...
#define DOMTYPEDARRAY_DEFINE_GETTYPE(val_t, Type, clamped)                                                                                                     \
    template <> inline DOMArrayBufferView::ViewType DOMTypedArray<val_t, v8::Type##Array, clamped>::GetType() const                                            \
    {                                                                                                                                                          \
...
#define DOMTYPEDARRAY_DECLARE_EXTERN_TEMPLATE(val_t, Type, clamped)                                                                                            \
    extern template class CORE_EXTERN_TEMPLATE_EXPORT DOMTypedArray<val_t, v8::Type##Array, clamped>;
DOMTYPEDARRAY_FOREACH_VIEW_TYPE(DOMTYPEDARRAY_DECLARE_EXTERN_TEMPLATE)
#undef DOMTYPEDARRAY_DECLARE_EXTERN_TEMPLATE

#define DOMTYPEDARRAY_DEFINE_TYPEDEFNAME(val_t, Type, clamped) using DOM##Type##Array = DOMTypedArray<val_t, v8::Type##Array, clamped>;
