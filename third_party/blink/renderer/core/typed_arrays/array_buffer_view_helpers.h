...
template <typename T> class NotShared {
    DISALLOW_NEW();
    static_assert(
        WTF::IsSubclass<typename std::remove_const<T>::type, DOMArrayBufferView>::value, "NotShared<T> must have T as subclass of DOMArrayBufferView");
...
public:
    ...
    NotShared(const NotShared<U>& other)
    ...
    explicit NotShared(T* typed_array)
        : typed_array_(typed_array)
    {
        DCHECK(!typed_array || !typed_array->IsShared());
    }
    template <typename U>
    explicit NotShared(const Member<U>& other)
        : typed_array_(other.Get())
    {
    ...
    template <typename U> NotShared& operator=(const NotShared<U>& other)
    {
    ...
    {
        visitor->Trace(typed_array_);
    }

private:
    T* GetRaw() const
    {
        return typed_array_.Get();
    }
...
//     ...
//   }
template <typename T> class MaybeShared {
    DISALLOW_NEW();
    static_assert(
        WTF::IsSubclass<typename std::remove_const<T>::type, DOMArrayBufferView>::value, "MaybeShared<T> must have T as subclass of DOMArrayBufferView");
...
public:
    ...
    explicit MaybeShared(T* typed_array)
        : typed_array_(typed_array)
    {
    }
    template <typename U>
    explicit MaybeShared(const Member<U>& other)
        : typed_array_(other.Get())
    {
    }

    MaybeShared& operator=(const MaybeShared& other) = default;
    template <typename U> MaybeShared& operator=(const MaybeShared<U>& other)
    {
        typed_array_ = static_cast<T*>(other.Get());
        return *this;
    }
...
template <typename T> struct VectorTraits<blink::NotShared<T>> : VectorTraits<blink::Member<T>> { };

// MaybeShared<T> is essentially Member<T> from the perspective of HeapVector.
template <typename T> struct VectorTraits<blink::MaybeShared<T>> : VectorTraits<blink::Member<T>> { };
