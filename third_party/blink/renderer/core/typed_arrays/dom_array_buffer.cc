...
const WrapperTypeInfo& DOMArrayBuffer::wrapper_type_info_ = DOMArrayBuffer::wrapper_type_info_body_;
...
template <typename Function> void ForArrayBuffersInAllWorlds(v8::Isolate* isolate, const DOMArrayBuffer* object, Function f)
{
    if (!object->has_non_main_world_wrappers() && IsMainThread()) {
        const DOMWrapperWorld& world = DOMWrapperWorld::MainWorld(isolate);
        v8::Local<v8::Object> wrapper;
        if (world.DomDataStore().Get</*entered_context=*/false>(isolate, object).ToLocal(&wrapper)) {
...
bool DOMArrayBuffer::IsDetachable(v8::Isolate* isolate)
{
    ...
    ForArrayBuffersInAllWorlds(isolate, this, [&is_detachable](v8::Local<v8::ArrayBuffer> buffer_handle) { is_detachable &= buffer_handle->IsDetachable(); });
    return is_detachable;
}

void DOMArrayBuffer::SetDetachKey(v8::Isolate* isolate, const StringView& detach_key)
{
    ...
    ForArrayBuffersInAllWorlds(isolate, this, [&v8_detach_key](v8::Local<v8::ArrayBuffer> buffer_handle) { buffer_handle->SetDetachKey(v8_detach_key); });
}

bool DOMArrayBuffer::Transfer(v8::Isolate* isolate, ArrayBufferContents& result, ExceptionState& exception_state)
{
    return Transfer(isolate, v8::Local<v8::Value>(), result, exception_state);
}

bool DOMArrayBuffer::Transfer(v8::Isolate* isolate, v8::Local<v8::Value> detach_key, ArrayBufferContents& result, ExceptionState& exception_state)
{
    ...
    if (!to_transfer->TransferDetachable(isolate, detach_key, result).To(&detach_result)) {
        return false;
    }
    if (!detach_result) {
...
v8::Maybe<bool> DOMArrayBuffer::TransferDetachable(v8::Isolate* isolate, v8::Local<v8::Value> detach_key, ArrayBufferContents& result)
{
    ...
    bool first = true;
    bool failed = false;
    ForArrayBuffersInAllWorlds(isolate, this, [&first, &failed, &detach_key](v8::Local<v8::ArrayBuffer> buffer_handle) {
...
DOMArrayBuffer* DOMArrayBuffer::Create(scoped_refptr<SharedBuffer> shared_buffer)
{
    ArrayBufferContents contents(shared_buffer->size(), 1, ArrayBufferContents::kNotShared, ArrayBufferContents::kDontInitialize,
...
DOMArrayBuffer* DOMArrayBuffer::Create(const Vector<base::span<const char>>& data)
{
    size_t size = 0;
    ...
    ArrayBufferContents contents(
        size, 1, ArrayBufferContents::kNotShared, ArrayBufferContents::kDontInitialize, ArrayBufferContents::AllocationFailureBehavior::kCrash);
    CHECK(contents.IsValid());
...
DOMArrayBuffer* DOMArrayBuffer::CreateOrNull(size_t num_elements, size_t element_byte_size)
{
    ArrayBufferContents contents(num_elements, element_byte_size, ArrayBufferContents::kNotShared, ArrayBufferContents::kZeroInitialize);
    if (!contents.Data()) {
        return nullptr;
    }
    return Create(std::move(contents));
...
DOMArrayBuffer* DOMArrayBuffer::CreateUninitializedOrNull(size_t num_elements, size_t element_byte_size)
{
    ArrayBufferContents contents(num_elements, element_byte_size, ArrayBufferContents::kNotShared, ArrayBufferContents::kDontInitialize);
    if (!contents.Data()) {
        return nullptr;
    }
    return Create(std::move(contents));
...
v8::Local<v8::Value> DOMArrayBuffer::Wrap(ScriptState* script_state)
{
    ...
    v8::Local<v8::ArrayBuffer> wrapper;
    {
        ...
        wrapper
            = backing_store ? v8::ArrayBuffer::New(script_state->GetIsolate(), std::move(backing_store)) : v8::ArrayBuffer::New(script_state->GetIsolate(), 0);
    ...
    return AssociateWithWrapper(script_state->GetIsolate(), wrapper_type_info, wrapper);
}
...
bool DOMArrayBuffer::IsDetached() const
{
    ...
    ForArrayBuffersInAllWorlds(isolate, this, [&detached_count, &nondetached_count](v8::Local<v8::ArrayBuffer> buffer_handle) {
...
v8::Local<v8::Object> DOMArrayBuffer::AssociateWithWrapper(v8::Isolate* isolate, const WrapperTypeInfo* wrapper_type_info, v8::Local<v8::Object> wrapper)
{
    if (!DOMWrapperWorld::Current(isolate).IsMainWorld()) {
        has_non_main_world_wrappers_ = true;
    }
    return ScriptWrappable::AssociateWithWrapper(isolate, wrapper_type_info, wrapper);
}
