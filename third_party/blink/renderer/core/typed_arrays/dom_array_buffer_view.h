...
public:
    ...
    base::span<uint8_t> ByteSpan() const
    {
        ...
        return UNSAFE_BUFFERS(base::span(static_cast<uint8_t*>(BaseAddress()), byteLength()));
    }

    virtual unsigned TypeSize() const = 0;
    bool IsShared() const
    {
        return dom_array_buffer_->IsShared();
    }
    ...
    {
        // SAFETY: `byteLength()` returns the number of bytes at `BaseAddress()`.
        return UNSAFE_BUFFERS(base::span(static_cast<uint8_t*>(BaseAddressMaybeShared()), byteLength()));
    }
    ...
    {
        dom_array_buffer_->Detach();
    }

    bool IsDetached() const
    {
        return dom_array_buffer_->IsDetached();
    }
...
protected:
    ...
    {
        DCHECK(dom_array_buffer_);
        raw_base_address_ = static_cast<char*>(dom_array_buffer_->DataMaybeShared()) + byte_offset;
    }
