...
ContentCaptureTask::TaskDelay::TaskDelay(const base::TimeDelta& task_initial_delay)
...
{
    return base::Milliseconds(task_initial_delay_.InMilliseconds() * (1 << delay_exponent_));
}
...
ContentCaptureTask::ContentCaptureTask(LocalFrame& local_frame_root, TaskSession& task_session)
    : local_frame_root_(&local_frame_root)
    , task_session_(&task_session)
    , delay_task_(local_frame_root_->GetTaskRunner(TaskType::kInternalContentCapture), this, &ContentCaptureTask::Run)
{
    DCHECK(local_frame_root.Client()->GetWebContentCaptureClient());
    task_delay_ = std::make_unique<TaskDelay>(local_frame_root.Client()->GetWebContentCaptureClient()->GetTaskInitialDelay());

    // The histogram is all about time, just disable it if high resolution isn't
    // supported.
    if (base::TimeTicks::IsHighResolution()) {
        histogram_reporter_ = base::MakeRefCounted<ContentCaptureTaskHistogramReporter>();
        task_session_->SetSentNodeCountCallback(
            WTF::BindRepeating(&ContentCaptureTaskHistogramReporter::RecordsSentContentCountPerDocument, histogram_reporter_));
    }
}
...
void ContentCaptureTask::SendContent(TaskSession::DocumentSession& doc_session)
{
...
WebContentCaptureClient* ContentCaptureTask::GetWebContentCaptureClient(const Document& document)
{
...
{
    DCHECK(task_session_);
    while (auto* document_session = task_session_->GetNextUnsentDocumentSession()) {
...
bool ContentCaptureTask::ProcessDocumentSession(TaskSession::DocumentSession& doc_session)
{
    // If no client, we don't need to send it at all.
    auto* content_capture_client = GetWebContentCaptureClient(*doc_session.GetDocument());
    if (!content_capture_client) {
        doc_session.Reset();
        return true;
    }

    while (doc_session.HasUnsentCapturedContent() || doc_session.HasUnsentChangedContent()) {
...
void ContentCaptureTask::Run(TimerBase*)
{
    ...
    if (histogram_reporter_ && (completed || task_state_ == TaskState::kCaptureContent)) {
...
void ContentCaptureTask::ScheduleInternal(ScheduleReason reason)
{
    ...
    TRACE_EVENT_INSTANT1("content_capture", "ScheduleTask", TRACE_EVENT_SCOPE_THREAD, "reason", reason);
    if (histogram_reporter_) {
        histogram_reporter_->OnTaskScheduled(/* record_task_delay = */ reason != ScheduleReason::kRetryTask);
    }
}
...
{
    return delay_task_.IsActive() ? delay_task_.NextFireInterval() : base::TimeDelta();
}
