...
ContentCaptureTaskHistogramReporter::~ContentCaptureTaskHistogramReporter() = default;
...
void ContentCaptureTaskHistogramReporter::OnTaskScheduled(bool record_task_delay)
{
    // Always save the latest schedule time.
    task_scheduled_time_ = record_task_delay ? base::TimeTicks::Now() : base::TimeTicks();
}

void ContentCaptureTaskHistogramReporter::OnTaskRun()
{
    if (!task_scheduled_time_.is_null()) {
        base::UmaHistogramCustomTimes(kTaskDelayInMs, base::TimeTicks::Now() - task_scheduled_time_, base::Milliseconds(1), base::Seconds(128), 100);
    }
...
void ContentCaptureTaskHistogramReporter::OnCaptureContentEnded(size_t captured_content_count)
{
...
void ContentCaptureTaskHistogramReporter::OnSendContentEnded(size_t sent_content_count)
{
    ...
    if (captured_content_change_time_) {
        ...
        base::UmaHistogramCustomTimes(kCaptureContentDelayTime, now - content_change_time, base::Milliseconds(500), base::Seconds(30), 50);
    }
    if (!sent_content_count) {
        return;
    }
    send_content_time_histogram_.CountMicroseconds(now - send_content_start_time_);
}
...
void ContentCaptureTaskHistogramReporter::RecordsSentContentCountPerDocument(int sent_content_count)
{
