...
{
    return base::FeatureList::IsEnabled(features::kContentCaptureConstantStreaming);
}

} // namespace

TaskSession::DocumentSession::DocumentSession(const Document& document, SentNodeCountCallback& callback)
    : document_(&document)
    , callback_(callback)
{
}
...
void TaskSession::DocumentSession::OnContentCaptured(Node& node, const gfx::Rect& visual_rect)
{
...
void TaskSession::SetCapturedContent(const Vector<cc::NodeInfo>& captured_content)
{
...
void TaskSession::GroupCapturedContentByDocument(const Vector<cc::NodeInfo>& captured_content)
{
    ...
    for (const auto& i : captured_content) {
        if (Node* node = DOMNodeIds::NodeForId(i.node_id)) {
            EnsureDocumentSession(node->GetDocument()).OnContentCaptured(*node, i.visual_rect);
        }
    }
...
TaskSession::DocumentSession& TaskSession::EnsureDocumentSession(const Document& doc)
{
...
TaskSession::DocumentSession* TaskSession::GetDocumentSession(const Document& document) const
{
