...
void FindNodeVectorsDiff(const Vector<Persistent<Node>>& a, const Vector<Persistent<Node>>& b, Vector<Persistent<Node>>& a_diff_b)
{
    for (auto& i : a) {
        if (!b.Contains(i))
...
void FindNodeVectorsDiff(
    const Vector<Persistent<Node>>& a, const Vector<Persistent<Node>>& b, Vector<Persistent<Node>>& a_diff_b, Vector<Persistent<Node>>& b_diff_a)
{
    FindNodeVectorsDiff(a, b, a_diff_b);
    FindNodeVectorsDiff(b, a, b_diff_a);
}

void FindNodeVectorsUnion(const Vector<Persistent<Node>>& a, const Vector<Persistent<Node>>& b, HashSet<Persistent<Node>>& a_and_b)
{
    for (auto& n : a) {
        a_and_b.insert(n);
    }
...
void ToNodeIds(const Vector<Persistent<Node>>& nodes, Vector<int64_t>& node_ids)
{
    for (auto& v : nodes) {
...
public:
    ...
    void DidCaptureContent(const WebVector<WebContentHolder>& data, bool first_data) override
    {
        data_ = data;
    ...
    bool FirstData() const
    {
        return first_data_;
    }

    const WebVector<WebContentHolder>& Data() const
    {
        return data_;
    }
    ...
    const Vector<String>& AllText() const
    {
        return all_text_;
    }

    const Vector<String>& CapturedText() const
    {
        return captured_text_;
    }

    const Vector<String>& UpdatedText() const
    {
        return updated_text_;
    }

    const WebVector<int64_t>& RemovedData() const
    {
        return removed_data_;
    }
...
class ContentCaptureTest : public PageTestBase, public ::testing::WithParamInterface<std::vector<base::test::FeatureRef>> {
public:
    ContentCaptureTest()
    ...
    void SetUp() override
    {
        content_capture_client_ = std::make_unique<WebContentCaptureClientTestHelper>();
        local_frame_client_ = MakeGarbageCollected<ContentCaptureLocalFrameClientHelper>(*content_capture_client_);
        SetupPageWithClients(nullptr, local_frame_client_);
        SetHtmlInnerHTML("<!DOCTYPE HTML>"
                         "<p id='p1'>1</p>"
                         "<p id='p2'>2</p>"
                         "<p id='p3'>3</p>"
                         "<p id='p4'>4</p>"
        ...
        GetOrResetContentCaptureManager()->GetContentCaptureTaskForTesting()->SetCapturedContentForTesting(node_ids_);
    ...
    {
        CHECK_LT(step, 4u);
        GetOrResetContentCaptureManager()->GetContentCaptureTaskForTesting()->SetCapturedContentForTesting(scrolling_node_ids_[step]);
    ...
    void CreateTextNodeAndNotifyManager()
    {
        ...
        Vector<cc::NodeInfo> captured_content { cc::NodeInfo(created_node_id_, GetRect(node->GetLayoutObject())) };
        GetOrResetContentCaptureManager()->GetContentCaptureTaskForTesting()->SetCapturedContentForTesting(captured_content);
    }
    ...
    {
        ResetResult();
        FastForwardBy(GetContentCaptureTask()->GetTaskDelayForTesting().GetNextTaskDelay());
    }
    ...
    size_t GetExpectedFirstResultSize()
    {
        return ContentCaptureTask::kBatchSize;
    }
    ...
    const Vector<cc::NodeInfo>& NodeIds() const
    {
        return node_ids_;
    }
    const Vector<Persistent<Node>> Nodes() const
    {
        return nodes_;
    }

    Node& invisible_node() const
    {
        return *invisible_node_;
    }
...
private:
    ...
    void BuildNodesInfo(const Vector<String>& ids, Vector<Persistent<Node>>& nodes, Vector<cc::NodeInfo>& node_ids)
    {
        for (auto id : ids) {
            ...
            GetOrResetContentCaptureManager()->ScheduleTaskIfNeeded(*node);
            node_ids.push_back(cc::NodeInfo(node->GetDomNodeId(), GetRect(layout_object)));
        }
    }
    ...
    {
        BuildNodesInfo(Vector<String> { "p1", "p2", "p3", "p4", "p5", "p6", "p7", "p8" }, nodes_, node_ids_);
    ...
    void InitScrollingTestData()
    {
        ...
        BuildNodesInfo(Vector<String> { "p1", "p2", "p3" }, nodes[0], scrolling_node_ids_[0]);
        BuildNodesInfo(Vector<String> { "p3", "p4", "p5" }, nodes[1], scrolling_node_ids_[1]);
        BuildNodesInfo(Vector<String> { "p6", "p7", "p8" }, nodes[2], scrolling_node_ids_[2]);
        BuildNodesInfo(Vector<String> { "p2", "p3" }, nodes[3], scrolling_node_ids_[3]);
        // Build expected result.
        if (base::FeatureList::IsEnabled(features::kContentCaptureConstantStreaming)) {
            for (int i = 0; i < 4; ++i) {
                Vector<Persistent<Node>> a_diff_b;
                Vector<Persistent<Node>> b_diff_a;
                FindNodeVectorsDiff(nodes[i], i == 0 ? Vector<Persistent<Node>>() : nodes[i - 1], a_diff_b, b_diff_a);
...
INSTANTIATE_TEST_SUITE_P(, ContentCaptureTest,
    testing::Values(std::vector<base::test::FeatureRef> {}, std::vector<base::test::FeatureRef> { features::kContentCaptureConstantStreaming }));

TEST_P(ContentCaptureTest, Basic)
{
    RunContentCaptureTask();
    EXPECT_EQ(ContentCaptureTask::TaskState::kStop, GetContentCaptureTask()->GetTaskStateForTesting());
    EXPECT_FALSE(GetWebContentCaptureClient()->Data().empty());
    EXPECT_EQ(GetExpectedSecondResultSize(), GetWebContentCaptureClient()->Data().size());
}
...
TEST_P(ContentCaptureTest, Scrolling)
{
    for (wtf_size_t step = 0; step < 4; ++step) {
        ...
        RunContentCaptureTask();
        EXPECT_EQ(ContentCaptureTask::TaskState::kStop, GetContentCaptureTask()->GetTaskStateForTesting());
        EXPECT_THAT(GetWebContentCaptureClient()->CapturedText(), testing::UnorderedElementsAreArray(scrolling_expected_captured_nodes()[step]))
            << "at step " << step;
        EXPECT_THAT(GetWebContentCaptureClient()->RemovedData(), testing::UnorderedElementsAreArray(scrolling_expected_removed_nodes()[step]))
            << "at step " << step;
    }
}
...
TEST_P(ContentCaptureTest, PauseAndResume)
{
    ...
    GetContentCaptureTask()->SetTaskStopForTesting(ContentCaptureTask::TaskState::kCaptureContent);
    RunContentCaptureTask();
    ...
    GetContentCaptureTask()->SetTaskStopForTesting(ContentCaptureTask::TaskState::kProcessCurrentSession);
    RunContentCaptureTask();
    EXPECT_FALSE(GetWebContentCaptureClient()->FirstData());
    EXPECT_TRUE(GetWebContentCaptureClient()->Data().empty());
    EXPECT_TRUE(GetWebContentCaptureClient()->RemovedData().empty());
    ...
    GetContentCaptureTask()->SetTaskStopForTesting(ContentCaptureTask::TaskState::kProcessRetryTask);
    RunContentCaptureTask();
    EXPECT_TRUE(GetWebContentCaptureClient()->FirstData());
    EXPECT_FALSE(GetWebContentCaptureClient()->Data().empty());
    EXPECT_TRUE(GetWebContentCaptureClient()->RemovedData().empty());
    EXPECT_EQ(GetExpectedFirstResultSize(), GetWebContentCaptureClient()->Data().size());

    // Run task until it stops, task will not capture content, because there is no
    // content change, so we have 3 NodeHolders.
    GetContentCaptureTask()->SetTaskStopForTesting(ContentCaptureTask::TaskState::kStop);
    RunContentCaptureTask();
    EXPECT_FALSE(GetWebContentCaptureClient()->FirstData());
    EXPECT_FALSE(GetWebContentCaptureClient()->Data().empty());
    EXPECT_TRUE(GetWebContentCaptureClient()->RemovedData().empty());
...
TEST_P(ContentCaptureTest, UnsentNode)
{
    ...
    // change because |invisible_node_| wasn't captured.
    GetOrResetContentCaptureManager()->OnLayoutTextWillBeDestroyed(invisible_node());
    RunContentCaptureTask();
    EXPECT_TRUE(GetWebContentCaptureClient()->Data().empty());
    EXPECT_TRUE(GetWebContentCaptureClient()->UpdatedData().empty());
    EXPECT_TRUE(GetWebContentCaptureClient()->RemovedData().empty());
...
TEST_P(ContentCaptureTest, RemoveNodeBeforeSendingOut)
{
    ...
    EXPECT_EQ(GetExpectedSecondResultSize() - 1, GetWebContentCaptureClient()->Data().size());
    EXPECT_EQ(0u, GetWebContentCaptureClient()->RemovedData().size());
    RunContentCaptureTask();
...
TEST_P(ContentCaptureTest, TaskHistogramReporter)
{
    ...
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kCaptureContentTime, 0u);
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kSendContentTime, 0u);
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kCaptureContentDelayTime, 0u);
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kSentContentCount, 0u);
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kTaskRunsPerCapture, 0u);
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kTaskDelayInMs, 1u);

    // The task stops before sends the captured content out.
    GetContentCaptureTask()->SetTaskStopForTesting(ContentCaptureTask::TaskState::kProcessCurrentSession);
    RunContentCaptureTask();
    // Verify has one CaptureContentTime record.
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kCaptureContentTime, 1u);
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kSendContentTime, 0u);
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kCaptureContentDelayTime, 0u);
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kSentContentCount, 0u);
    ...
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kCaptureContentTime, 1u);
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kSendContentTime, 1u);
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kCaptureContentDelayTime, 1u);
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kSentContentCount, 0u);
    ...
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kCaptureContentTime, 1u);
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kSendContentTime, 2u);
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kCaptureContentDelayTime, 1u);
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kSentContentCount, 0u);
    ...
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kTaskRunsPerCapture, 1u);
    ...
    histograms.ExpectBucketCount(ContentCaptureTaskHistogramReporter::kTaskRunsPerCapture, 4u, 1u);

    // Create a node and run task until it stops.
    CreateTextNodeAndNotifyManager();
    GetContentCaptureTask()->SetTaskStopForTesting(ContentCaptureTask::TaskState::kStop);
    RunNextContentCaptureTask();
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kCaptureContentTime, 2u);
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kSendContentTime, 3u);
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kCaptureContentDelayTime, 2u);
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kSentContentCount, 0u);

    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kTaskRunsPerCapture, 2u);
    // Verify the task ran 1 times for this session because we didn't explicitly
    // stop it.
    histograms.ExpectBucketCount(ContentCaptureTaskHistogramReporter::kTaskRunsPerCapture, 1u, 1u);
    ...
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kSendContentTime, 3u);
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kCaptureContentDelayTime, 2u);
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kSentContentCount, 1u);
    // Verify total content has been sent.
    histograms.ExpectBucketCount(ContentCaptureTaskHistogramReporter::kSentContentCount, 9u, 1u);

    // Verify TaskDelay was recorded again for node change.
    histograms.ExpectTotalCount(ContentCaptureTaskHistogramReporter::kTaskDelayInMs, 2u);
}
...
TEST_P(ContentCaptureTest, RescheduleTask)
{
    ...
    task->Schedule(ContentCaptureTask::ScheduleReason::kNonUserActivatedContentChange);
...
public:
    ...
    void RunContentCaptureTaskUntil(ContentCaptureTask::TaskState state)
    {
        ...
        GetDocument().GetFrame()->LocalFrameRoot().GetOrResetContentCaptureManager()->GetContentCaptureTaskForTesting()->RunTaskForTestingUntil(state);
        // Cancels the scheduled task to simulate that the task is running by
        // scheduler.
        GetOrResetContentCaptureManager()->GetContentCaptureTaskForTesting()->CancelTaskForTesting();
    }

    WebContentCaptureClientTestHelper& Client()
    {
        return client_;
    }
    WebContentCaptureClientTestHelper& ChildClient()
    {
    ...
    void SetCapturedContent(ContentType type)
    {
        if (type == ContentType::kMainFrame) {
            ...
            holders.AppendRange(child_frame_content_.begin(), child_frame_content_.end());
    ...
    void ReplaceMainFrameExpectedText(const String& old_text, const String& new_text)
    {
        std::replace(main_frame_expected_text_.begin(), main_frame_expected_text_.end(), old_text, new_text);
    }

    ContentCaptureManager* GetOrResetContentCaptureManager()
    {
        return DynamicTo<LocalFrame>(LocalFrameRoot().GetFrame())->GetOrResetContentCaptureManager();
    }

    void SimulateUserInputOnMainFrame()
    {
        GetOrResetContentCaptureManager()->NotifyInputEvent(WebInputEvent::Type::kMouseDown, *DynamicTo<LocalFrame>(MainFrame().GetFrame()));
    }

    void SimulateUserInputOnChildFrame()
    {
        GetOrResetContentCaptureManager()->NotifyInputEvent(WebInputEvent::Type::kMouseDown, *child_document_->GetFrame());
    }

    base::TimeDelta GetNextTaskDelay()
    {
        return GetOrResetContentCaptureManager()->GetContentCaptureTaskForTesting()->GetTaskDelayForTesting().GetNextTaskDelay();
    }

    base::TimeDelta GetTaskNextFireInterval()
    {
        return GetOrResetContentCaptureManager()->GetContentCaptureTaskForTesting()->GetTaskNextFireIntervalForTesting();
    }
...
private:
    void SetupPage()
    {
      ...
      <div id='d1'></div>
        ...
        static_cast<WebLocalFrame*>(MainFrame().FindFrameByName("frame"))->SetContentCaptureClient(&child_client_);
        auto* child_frame_element = To<HTMLIFrameElement>(GetDocument().getElementById(AtomicString("frame")));
    ...
    {
        Vector<String> ids = { "p1", "p2", "p3", "p4", "p5", "p6", "p7", "s8", "editable_id" };
        main_frame_expected_text_ = { "Hello World1", "Hello World2", "Hello World3", "Hello World4", "Hello World5", "Hello World6", "Hello World7",
            "Hello World8", kEditableContent };
    ...
    void InitNodeHolders(Vector<cc::NodeInfo>& buffer, const Vector<String>& ids, const Document& document)
    {
        for (auto id : ids) {
            auto* layout_object = document.getElementById(AtomicString(id))->firstChild()->GetLayoutObject();
            auto* layout_text = To<LayoutText>(layout_object);
            EXPECT_TRUE(layout_text->HasNodeId());
            buffer.push_back(cc::NodeInfo(layout_text->EnsureNodeId(), GetRect(layout_object)));
        }
    }
    ...
    void AddNodeToDocument(Document& doc, Vector<cc::NodeInfo>& buffer)
    {
        ...
        EXPECT_TRUE(layout_text->HasNodeId());
        buffer.push_back(cc::NodeInfo(layout_text->EnsureNodeId(), GetRect(node->GetLayoutObject())));
    }

    void InsertNodeContent(Document& doc, const String& id, const String& content, unsigned offset)
    {
        To<Text>(doc.getElementById(AtomicString(id))->firstChild())->insertData(offset, content, IGNORE_EXCEPTION_FOR_TESTING);
        Compositor().BeginFrame();
    }

    void DeleteNodeContent(Document& doc, const String& id, unsigned offset, unsigned length)
    {
        To<Text>(doc.getElementById(AtomicString(id))->firstChild())->deleteData(offset, length, IGNORE_EXCEPTION_FOR_TESTING);
        Compositor().BeginFrame();
    }

    void SetCapturedContent(const Vector<cc::NodeInfo>& captured_content)
    {
        GetDocument().GetFrame()->LocalFrameRoot().GetOrResetContentCaptureManager()->GetContentCaptureTaskForTesting()->SetCapturedContentForTesting(
            captured_content);
    }
...
TEST_F(ContentCaptureSimTest, MultiFrame)
{
    ...
    EXPECT_THAT(Client().AllText(), testing::UnorderedElementsAreArray(MainFrameExpectedText()));
    EXPECT_THAT(ChildClient().AllText(), testing::UnorderedElementsAreArray(ChildFrameExpectedText()));
}
...
TEST_F(ContentCaptureSimTest, AddNodeToMultiFrame)
{
    ...
    RunContentCaptureTaskUntil(ContentCaptureTask::TaskState::kProcessCurrentSession);
...
TEST_F(ContentCaptureSimTest, ChangeNode)
{
    ...
    EXPECT_THAT(Client().UpdatedText(), testing::UnorderedElementsAreArray(expected_text_update));
    ...
    InsertMainFrameEditableContent(insert_text2, insert_text.length() + insert_text1.length());
    ...
    EXPECT_FALSE(Client().FirstData());
    EXPECT_TRUE(ChildClient().Data().empty());
    expected_text_update.push_back(insert_text + insert_text1 + insert_text2 + kEditableContent);
    EXPECT_THAT(Client().UpdatedText(), testing::UnorderedElementsAreArray(expected_text_update));
}
...
TEST_F(ContentCaptureSimTest, ChangeNodeBeforeCapture)
{
    ...
    ReplaceMainFrameExpectedText(kEditableContent, insert_text + insert_text1 + insert_text2 + kEditableContent);
    SetCapturedContent(ContentType::kMainFrame);
    RunContentCaptureTaskUntil(ContentCaptureTask::TaskState::kStop);
    EXPECT_EQ(4u, Client().Data().size());
    EXPECT_FALSE(Client().FirstData());
...
{
    base::TimeDelta expected_delays[] = { base::Milliseconds(500), base::Seconds(1), base::Seconds(2), base::Seconds(4), base::Seconds(8), base::Seconds(16),
