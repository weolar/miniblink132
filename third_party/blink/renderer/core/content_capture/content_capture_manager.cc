...
ContentCaptureManager::UserActivation::UserActivation(const LocalFrame& local_frame)
...
void ContentCaptureManager::ScheduleTaskIfNeeded(const Node& node)
{
    ...
    if (first_node_holder_created_) {
        ScheduleTask(UserActivated(node) ? ContentCaptureTask::ScheduleReason::kUserActivatedContentChange
...
bool ContentCaptureManager::UserActivated(const Node& node) const
{
    if (auto* frame = node.GetDocument().GetFrame()) {
        return latest_user_activation_ && latest_user_activation_->local_frame == frame
            && (base::TimeTicks::Now() - latest_user_activation_->activation_time < kUserActivationExpiryPeriod);
    }
    return false;
}

void ContentCaptureManager::ScheduleTask(ContentCaptureTask::ScheduleReason reason)
{
...
{
    return MakeGarbageCollected<ContentCaptureTask>(*local_frame_root_, *task_session_);
}
...
void ContentCaptureManager::NotifyInputEvent(WebInputEvent::Type type, const LocalFrame& local_frame)
{
