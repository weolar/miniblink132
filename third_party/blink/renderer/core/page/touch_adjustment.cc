...
WTF_ALLOW_MOVE_INIT_AND_COMPARE_WITH_MEM_FUNCTIONS(blink::touch_adjustment::SubtargetGeometry)
...
typedef float (*DistanceFunction)(const gfx::Point&, const gfx::Rect&, const SubtargetGeometry&);
...
bool ProvidesContextMenuItems(Node* node)
{
    ...
    if (node->GetLayoutObject()->CanBeSelectionLeaf()) {
        ...
        // are valid targets.
        if (node->GetLayoutObject()->GetFrame()->GetEditor().Behavior().ShouldSelectOnContextualMenuClick())
            return true;
...
bool NodeRespondsToTapOrMove(Node* node)
{
    ...
    TouchAction effective_touch_action = touch_action_util::ComputeEffectiveTouchAction(*node);
...
static inline void AppendQuadsToSubtargetList(Vector<gfx::QuadF>& quads, Node* node, SubtargetGeometryList& subtargets)
{
...
static inline void AppendContextSubtargetsForNode(Node* node, SubtargetGeometryList& subtargets)
{
    ...
    if (text_layout_object->GetFrame()->GetEditor().Behavior().ShouldSelectOnContextualMenuClick()) {
        // Make subtargets out of every word.
        String text_value = text_node->data();
        TextBreakIterator* word_iterator = WordBreakIterator(text_value, 0, text_value.length());
        ...
        const LayoutTextSelectionStatus& selection_status = frame_selection.ComputeLayoutSelectionStatus(*text_layout_object);
        // If selected, make subtargets out of only the selected part of the text.
        Vector<gfx::QuadF> quads;
        text_layout_object->AbsoluteQuadsForRange(quads, selection_status.start, selection_status.end);
        AppendQuadsToSubtargetList(quads, text_node, subtargets);
    }
}
...
void CompileSubtargetList(const HeapVector<Member<Node>>& intersected_nodes, SubtargetGeometryList& subtargets, NodeFilter node_filter,
    AppendSubtargetsForNode append_subtargets_for_node)
{
    ...
    for (unsigned i = 0; i < intersected_nodes.size(); ++i) {
        ...
        for (Node* visited_node = node; visited_node; visited_node = visited_node->ParentOrShadowHostNode()) {
            ...
            if (node_filter(visited_node)) {
                ...
                for (visited_node = ParentShadowHostOrOwner(visited_node); visited_node; visited_node = ParentShadowHostOrOwner(visited_node)) {
                    HeapHashSet<Member<Node>>::AddResult add_result = ancestors_to_responders_set.insert(visited_node);
...
float ZoomableIntersectionQuotient(const gfx::Point& touch_hotspot, const gfx::Rect& touch_area, const SubtargetGeometry& subtarget)
{
    gfx::Rect rect = subtarget.GetNode()->GetDocument().View()->ConvertToRootFrame(subtarget.BoundingBox());
    ...
    return static_cast<float>(rect.size().Area64()) / static_cast<float>(intersection.size().Area64());
}
...
float HybridDistanceFunction(const gfx::Point& touch_hotspot, const gfx::Rect& touch_rect, const SubtargetGeometry& subtarget)
{
    gfx::RectF rect(subtarget.GetNode()->GetDocument().View()->ConvertToRootFrame(subtarget.BoundingBox()));
    float radius_squared = 0.25f * gfx::Vector2dF(touch_rect.width(), touch_rect.height()).LengthSquared();
    gfx::PointF hotspot_f(touch_hotspot);
    float distance_to_adjust_score = (rect.ClosestPoint(hotspot_f) - hotspot_f).LengthSquared() / radius_squared;

    float max_overlap_width = std::min<float>(touch_rect.width(), rect.width());
    float max_overlap_height = std::min<float>(touch_rect.height(), rect.height());
    float max_overlap_area = std::max<float>(max_overlap_width * max_overlap_height, 1);
...
bool SnapTo(const SubtargetGeometry& geom, const gfx::Point& touch_point, const gfx::Rect& touch_area, gfx::Point& snapped_point)
{
...
bool FindNodeWithLowestDistanceMetric(Node*& adjusted_node, gfx::Point& adjusted_point, const gfx::Point& touch_hotspot, const gfx::Rect& touch_area,
    SubtargetGeometryList& subtargets, DistanceFunction distance_function)
{
    ...
    for (const auto& subtarget : subtargets) {
        ...
        float distance_metric = distance_function(touch_hotspot, touch_area, subtarget);
    ...
    if (adjusted_node && adjusted_node->IsPseudoElement() && !adjusted_node->IsScrollMarkerPseudoElement()) {
...
bool FindBestCandidate(Node*& adjusted_node, gfx::Point& adjusted_point, const gfx::Point& touch_hotspot, const gfx::Rect& touch_area,
    const HeapVector<Member<Node>>& nodes, NodeFilter node_filter, AppendSubtargetsForNode append_subtargets_for_node)
{
    touch_adjustment::SubtargetGeometryList subtargets;
    touch_adjustment::CompileSubtargetList(nodes, subtargets, node_filter, append_subtargets_for_node);
    return touch_adjustment::FindNodeWithLowestDistanceMetric(
        adjusted_node, adjusted_point, touch_hotspot, touch_area, subtargets, touch_adjustment::HybridDistanceFunction);
}

} // namespace touch_adjustment

bool FindBestTouchAdjustmentCandidate(TouchAdjustmentCandidateType candidate_type, Node*& candidate_node, gfx::Point& candidate_point,
    const gfx::Point& touch_hotspot, const gfx::Rect& touch_area, const HeapVector<Member<Node>>& nodes)
{
    ...
    case TouchAdjustmentCandidateType::kClickable:
        ...
        append_subtargets_for_node = touch_adjustment::AppendBasicSubtargetsForNode;
        break;
    case TouchAdjustmentCandidateType::kContextMenu:
        node_filter = touch_adjustment::ProvidesContextMenuItems;
        append_subtargets_for_node = touch_adjustment::AppendContextSubtargetsForNode;
        break;
    ...
    return FindBestCandidate(candidate_node, candidate_point, touch_hotspot, touch_area, nodes, node_filter, append_subtargets_for_node);
}

PhysicalSize GetHitTestRectForAdjustment(LocalFrame& frame, const PhysicalSize& touch_area)
{
    ChromeClient& chrome_client = frame.GetChromeClient();
    float device_scale_factor = chrome_client.GetScreenInfo(frame).device_scale_factor;
    if (frame.GetPage()->InspectorDeviceScaleFactorOverride() != 1) {
        device_scale_factor = 1;
    }

    float page_scale_factor = frame.GetPage()->PageScaleFactor();
    const PhysicalSize max_size_in_dip(touch_adjustment::kMaxAdjustmentSizeDip, touch_adjustment::kMaxAdjustmentSizeDip);

    const PhysicalSize min_size_in_dip(touch_adjustment::kMinAdjustmentSizeDip, touch_adjustment::kMinAdjustmentSizeDip);
    ...
    return touch_area.ShrunkTo(max_size_in_dip * (device_scale_factor / page_scale_factor))
