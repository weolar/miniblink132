...
const AtomicString& FrameTree::GetName() const
{
    ...
    if (experimental_set_nulled_name_) {
        ...
        if (frame) {
            UseCounter::Count(frame->GetDocument(), WebFeature::kCrossOriginMainFrameNulledNameAccessed);
            if (!name_.empty()) {
                UseCounter::Count(frame->GetDocument(), WebFeature::kCrossOriginMainFrameNulledNonEmptyNameAccessed);
            }
        }
    }
    ...
    if (cross_site_cross_browsing_context_group_set_nulled_name_) {
        ...
        if (frame && frame->IsOutermostMainFrame() && !name_.empty()) {
            UseCounter::Count(frame->GetDocument(), WebFeature::kCrossBrowsingContextGroupMainFrameNulledNonEmptyNameAccessed);
        }
    }
    return name_;
}
...
void FrameTree::SetName(const AtomicString& name, ReplicationPolicy replication)
{
...
FrameTree::FindResult FrameTree::FindOrCreateFrameForNavigation(FrameLoadRequest& request, const AtomicString& name) const
{
    // Named frame lookup should always be relative to a local frame.
    DCHECK(IsA<LocalFrame>(this_frame_.Get()));
    LocalFrame* current_frame = To<LocalFrame>(this_frame_.Get());
...
Frame* FrameTree::FindFrameForNavigationInternal(const AtomicString& name, const KURL& url, FrameLoadRequest* request) const
{
    LocalFrame* current_frame = To<LocalFrame>(this_frame_.Get());
    ...
    if (EqualIgnoringASCIICase(name, "_self") || EqualIgnoringASCIICase(name, "_current") || name.empty()) {
    ...
    if (EqualIgnoringASCIICase(name, "_unfencedTop")) {
        ...
        if (current_frame->GetDeprecatedFencedFrameMode() == blink::FencedFrame::DeprecatedFencedFrameMode::kOpaqueAds && request != nullptr) {
    ...
    for (Frame* frame = current_frame; frame; frame = frame->Tree().TraverseNext(current_frame)) {
        if (frame->Tree().GetName() == name && current_frame->CanNavigate(*frame, url)) {
            return frame;
        }
    }
    ...
    for (Frame *top = &current_frame->Tree().Top(), *frame = top; frame; frame = frame->Tree().TraverseNext(top)) {
        ...
        if (frame->Tree().GetName() == name && !frame->Tree().IsDescendantOf(current_frame) && current_frame->CanNavigate(*frame, url)) {
            return frame;
        }
    }
    ...
    for (const Page* other_page : page->RelatedPages()) {
        ...
        for (Frame* frame = other_page->MainFrame(); frame; frame = frame->Tree().TraverseNext(nullptr)) {
            if (frame->Tree().GetName() == name && current_frame->CanNavigate(*frame, url)) {
                return frame;
            }
        }
    ...
    // cluster access.
    DCHECK(!named_frame || current_frame->DomWindow()->GetSecurityOrigin()->IsGrantedCrossAgentClusterAccess());
    return named_frame;
}
...
Frame* FrameTree::TraverseNext(const Frame* stay_within) const
{
    ...
    if (frame) {
        DCHECK(!stay_within || !sibling || sibling->Tree().IsDescendantOf(stay_within));
        return sibling;
    }
...
static void PrintFrames(const blink::Frame* frame, const blink::Frame* targetFrame, int indent)
{
    ...
    PrintIndent(indent);
    printf("  uri=%s\n\n", local_frame && local_frame->GetDocument() ? local_frame->GetDocument()->Url().GetString().Utf8().c_str() : nullptr);
