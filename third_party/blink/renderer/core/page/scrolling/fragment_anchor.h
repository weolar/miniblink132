...
public:
    ...
    static FragmentAnchor* TryCreate(const KURL& url, LocalFrame& frame, bool should_scroll);

    explicit FragmentAnchor(LocalFrame& frame)
        : frame_(frame)
    {
    }
    ...
    virtual bool IsTextFragmentAnchor()
    {
        return false;
    }

    virtual bool IsSelectorFragmentAnchor()
    {
        return false;
    }

    virtual void ScrollElementIntoViewWithOptions(Element* element_to_scroll, ScrollIntoViewOptions* options);
