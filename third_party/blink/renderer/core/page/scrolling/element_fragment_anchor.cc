...
ElementFragmentAnchor* ElementFragmentAnchor::TryCreate(const KURL& url, LocalFrame& frame, bool should_scroll)
{
    ...
    String fragment = RemoveFragmentDirectives(url.FragmentIdentifier().ToString());
    ...
    if (target) {
        target->ActivateDisplayLockIfNeeded(DisplayLockActivationReason::kFragmentNavigation);
    }
...
ElementFragmentAnchor::ElementFragmentAnchor(Node& anchor_node, LocalFrame& frame)
...
void ElementFragmentAnchor::Installed()
{
    ...
    if (needs_focus_) {
        ...
        frame_->GetDocument()->EnqueueAnimationFrameTask(WTF::BindOnce(&ElementFragmentAnchor::ApplyFocusIfNeeded, WrapPersistent(this)));
    }
...
void ElementFragmentAnchor::ApplyFocusIfNeeded()
{
    ...
    DCHECK(frame_->GetDocument()->IsSVGDocument() || !ScriptForbiddenScope::IsScriptForbidden());
    ...
    if (frame_->IsCaretBrowsingEnabled()) {
        ...
        if (pos.IsConnected()) {
            frame_->Selection().SetSelection(SelectionInDOMTree::Builder().Collapse(pos).Build(),
    ...
    if (element && element->IsFocusable()) {
        ...
        frame_->GetDocument()->SetSequentialFocusNavigationStartingPoint(anchor_node_);
