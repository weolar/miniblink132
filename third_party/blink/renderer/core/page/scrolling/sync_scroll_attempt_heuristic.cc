...
SyncScrollAttemptHeuristic::SyncScrollAttemptHeuristic(Frame* frame)
    : frame_(frame)
    , last_instance_(g_sync_scroll_attempt_heuristic)
{
...
SyncScrollAttemptHeuristic::~SyncScrollAttemptHeuristic()
{
    ...
    const bool saw_possible_sync_scrolling_attempt = did_access_scroll_offset_ && (did_set_style_ || did_set_scroll_offset_);
    if (saw_possible_sync_scrolling_attempt && frame_ && frame_->IsOutermostMainFrame() && !frame_->IsDetached()) {
        // This will not cover cases where |frame_| is remote.
        if (LocalFrame* local_frame = DynamicTo<LocalFrame>(frame_)) {
            if (local_frame->View()) {
                if (LocalFrameUkmAggregator* ukm_aggregator = local_frame->View()->GetUkmAggregator()) {
                    ukm_aggregator->RecordCountSample(LocalFrameUkmAggregator::kPossibleSynchronizedScrollCount2, 1);
                }
            }
        }
    }
...
SyncScrollAttemptHeuristic::Scope SyncScrollAttemptHeuristic::GetScrollHandlerScope()
{
    return Scope(g_sync_scroll_attempt_heuristic);
}

SyncScrollAttemptHeuristic::Scope SyncScrollAttemptHeuristic::GetRequestAnimationFrameScope()
{
    ...
    // true.
    return Scope(g_sync_scroll_attempt_heuristic && g_sync_scroll_attempt_heuristic->did_request_animation_frame_);
}

void SyncScrollAttemptHeuristic::DidAccessScrollOffset()
{
    if (g_sync_scroll_attempt_heuristic && g_sync_scroll_attempt_heuristic->is_observing_) [[unlikely]] {
...
void SyncScrollAttemptHeuristic::DidSetScrollOffset()
{
    ...
    // offset.
    if (g_sync_scroll_attempt_heuristic && g_sync_scroll_attempt_heuristic->is_observing_ && g_sync_scroll_attempt_heuristic->did_access_scroll_offset_)
        [[unlikely]] {
