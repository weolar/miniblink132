...
#define EXPECT_WHEEL_BUCKET(index, count)                                                                                                                      \
    do {                                                                                                                                                       \
        ...
        histogram_tester->ExpectBucketCount("Renderer4.MainThreadWheelScrollReason2", index, count);                                                           \
    } while (false)

#define EXPECT_TOUCH_BUCKET(index, count)                                                                                                                      \
    do {                                                                                                                                                       \
        SCOPED_TRACE("EXPECT_TOUCH_BUCKET");                                                                                                                   \
        histogram_tester->ExpectBucketCount("Renderer4.MainThreadGestureScrollReason2", index, count);                                                         \
    } while (false)

#define EXPECT_WHEEL_TOTAL(count)                                                                                                                              \
    do {                                                                                                                                                       \
        SCOPED_TRACE("EXPECT_WHEEL_TOTAL");                                                                                                                    \
        histogram_tester->ExpectTotalCount("Renderer4.MainThreadWheelScrollReason2", count);                                                                   \
    } while (false)

#define EXPECT_TOUCH_TOTAL(count)                                                                                                                              \
    do {                                                                                                                                                       \
        SCOPED_TRACE("EXPECT_TOUCH_TOTAL");                                                                                                                    \
        histogram_tester->ExpectTotalCount("Renderer4.MainThreadGestureScrollReason2", count);                                                                 \
    } while (false)
...
public:
    ScrollBeginEventBuilder(gfx::PointF position, gfx::PointF delta, WebGestureDevice device)
        : WebGestureEvent(WebInputEvent::Type::kGestureScrollBegin, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests(), device)
    {
...
public:
    explicit ScrollUpdateEventBuilder(WebGestureDevice device)
        : WebGestureEvent(WebInputEvent::Type::kGestureScrollUpdate, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests(), device)
    {
...
public:
    explicit ScrollEndEventBuilder(WebGestureDevice device)
        : WebGestureEvent(WebInputEvent::Type::kGestureScrollEnd, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests(), device)
    {
        frame_scale_ = 1;
    }
};
...
void ScrollMetricsTest::Scroll(Element* element, const WebGestureDevice device)
{
    DCHECK(element);
    DCHECK(element->GetBoundingClientRect());
    DOMRect* rect = element->GetBoundingClientRect();
    ScrollBeginEventBuilder scroll_begin(gfx::PointF(rect->left() + rect->width() / 2, rect->top() + rect->height() / 2), gfx::PointF(0.f, -1.f), device);
...
TEST_P(ScrollMetricsTest, TouchAndWheelGeneralTest)
{
    ...
    EXPECT_TOUCH_BUCKET(BucketIndex(cc::MainThreadScrollingReason::kMainThreadScrollHitTestRegion), 1);
    if (!RuntimeEnabledFeatures::RasterInducingScrollEnabled()) {
        EXPECT_TOUCH_BUCKET(BucketIndex(cc::MainThreadScrollingReason::kNotOpaqueForTextAndLCDText), 1);
    }
    EXPECT_TOUCH_BUCKET(cc::MainThreadScrollingReason::kScrollingOnMainForAnyReason, 1);
    EXPECT_TOUCH_TOTAL(RuntimeEnabledFeatures::RasterInducingScrollEnabled() ? 2 : 3);
    ...
    // The below reasons are reported because #box is not composited.
    EXPECT_WHEEL_BUCKET(BucketIndex(cc::MainThreadScrollingReason::kMainThreadScrollHitTestRegion), 1);
    if (!RuntimeEnabledFeatures::RasterInducingScrollEnabled()) {
        EXPECT_WHEEL_BUCKET(BucketIndex(cc::MainThreadScrollingReason::kNotOpaqueForTextAndLCDText), 1);
    }
    EXPECT_WHEEL_BUCKET(cc::MainThreadScrollingReason::kScrollingOnMainForAnyReason, 1);
    EXPECT_WHEEL_TOTAL(RuntimeEnabledFeatures::RasterInducingScrollEnabled() ? 2 : 3);
}
...
TEST_P(ScrollMetricsTest, CompositedScrollableAreaTest)
{
    ...
    box->setAttribute(html_names::kClassAttr, AtomicString("composited transform box"));
    Compositor().BeginFrame();
    Scroll(box, WebGestureDevice::kTouchpad);
...
TEST_P(ScrollMetricsTest, NotScrollableAreaTest)
{
    ...
    box->setAttribute(html_names::kClassAttr, AtomicString("hidden transform box"));
    UpdateAllLifecyclePhases();
    Scroll(box, WebGestureDevice::kTouchpad);
