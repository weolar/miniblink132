...
{
    cc::ScrollSnapType scroll_snap_type = snap_container.Style()->GetScrollSnapType();
...
{
    ScrollableArea* scrollable_area = ScrollableArea::GetForScrolling(&snap_container);
    ...
    snap_container_data.set_targeted_area_id(scrollable_area->GetTargetedSnapAreaId());
    ...
    container_rect.ContractEdges(MinimumValueForLength(container_style->ScrollPaddingTop(), container_rect.Height()),
    ...
    snap_container_data.set_has_horizontal_writing_mode(container_style->IsHorizontalWritingMode());

    if (snap_container_data.scroll_snap_type().strictness == cc::SnapStrictness::kProximity) {
    ...
    const cc::TargetSnapAreaElementIds old_target_ids
        = old_snap_container_data ? old_snap_container_data->GetTargetSnapAreaElementIds() : cc::TargetSnapAreaElementIds();

    for (auto& fragment : snap_container.PhysicalFragments()) {
        if (auto* snap_areas = fragment.SnapAreas()) {
            for (Element* snap_area : *snap_areas) {
                cc::SnapAreaData snap_area_data = CalculateSnapAreaData(*snap_area, snap_container);
                ...
                if (snap_area_data.rect.width() > snap_container_data.rect().width() || snap_area_data.rect.height() > snap_container_data.rect().height()) {
                    snap_container.GetDocument().CountUse(WebFeature::kScrollSnapCoveringSnapArea);
                }
    ...
    if (!old_snap_container_data || *old_snap_container_data != snap_container_data) {
        snap_container.SetNeedsPaintPropertyUpdate();
...
static cc::ScrollSnapAlign GetPhysicalAlignment(
    const ComputedStyle& area_style, const ComputedStyle& container_style, const PhysicalRect& area_rect, const PhysicalRect& container_rect)
{
    ...
    WritingDirectionMode writing_direction = container_style.GetWritingDirection();
    WritingDirectionMode area_writing_direction = area_style.GetWritingDirection();
    ...
    if (writing_direction.IsHorizontal()) {
        adjusted_alignment.alignment_inline = rtl ? AdjustForRtlWritingMode(align.alignment_inline) : align.alignment_inline;
        adjusted_alignment.alignment_block = align.alignment_block;
    } else {
        bool flipped = writing_direction.IsFlippedBlocks();
        adjusted_alignment.alignment_inline = flipped ? AdjustForRtlWritingMode(align.alignment_block) : align.alignment_block;
        adjusted_alignment.alignment_block = rtl ? AdjustForRtlWritingMode(align.alignment_inline) : align.alignment_inline;
    }
    return adjusted_alignment;
}

// static
cc::SnapAreaData SnapCoordinator::CalculateSnapAreaData(Element& snap_area, const LayoutBox& snap_container)
{
    const ComputedStyle* container_style = snap_container.Style();
    ...
    const MapCoordinatesFlags mapping_mode = kTraverseDocumentBoundaries | kIgnoreScrollOffset;
    ...
    if (const LayoutBox* box = snap_area.GetLayoutBox()) {
        ...
        const auto* multicol_box = DynamicTo<LayoutBox>(column_pseudo.UltimateOriginatingElement()->GetLayoutObject());
        DCHECK(multicol_box->IsFragmentationContextRoot());
        quads.push_back(multicol_box->LocalRectToAncestorQuad(column_pseudo.ColumnRect(), &snap_container, mapping_mode));
    }
    ...
    PhysicalBoxStrut area_margin(
        area_style->ScrollMarginTop(), area_style->ScrollMarginRight(), area_style->ScrollMarginBottom(), area_style->ScrollMarginLeft());
    ...
    snap_area_data.scroll_snap_align = GetPhysicalAlignment(*area_style, *container_style, area_rect, container_rect);

    snap_area_data.must_snap = (area_style->ScrollSnapStop() == EScrollSnapStop::kAlways);

    snap_area_data.has_focus_within = snap_area.HasFocusWithin();

    snap_area_data.element_id = CompositorElementIdFromDOMNodeId(snap_area.GetDomNodeId());
