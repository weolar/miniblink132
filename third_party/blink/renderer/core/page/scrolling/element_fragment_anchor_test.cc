...
TEST_F(ElementFragmentAnchorTest, FocusHandlerRunBeforeRaf)
{
    ...
    anchor->click();
    ASSERT_EQ(GetDocument().body(), GetDocument().ActiveElement()) << "Active element changed while rendering is blocked";
    ...
    // point.
    ASSERT_EQ(GetDocument().getElementById(AtomicString("bottom")), GetDocument().ActiveElement());

    // The background color shouldn't yet be updated.
    ASSERT_EQ(GetDocument().body()->GetLayoutObject()->Style()->VisitedDependentColor(GetCSSPropertyBackgroundColor()).NameForLayoutTreeAsText(),
    ...
    // second BeginFrame.
    EXPECT_EQ(GetDocument().body()->GetLayoutObject()->Style()->VisitedDependentColor(GetCSSPropertyBackgroundColor()).NameForLayoutTreeAsText(),
...
{
    SimRequest main_resource("https://example.com/test.html", "text/html");
    SimRequest child_resource("https://example.com/child.html#fragment", "text/html");
    LoadURL("https://example.com/test.html");
    ...
    HTMLFrameOwnerElement* iframe = To<HTMLFrameOwnerElement>(GetDocument().getElementById(AtomicString("child")));
    ScrollableArea* child_viewport = iframe->contentDocument()->View()->LayoutViewport();
    Element* fragment = iframe->contentDocument()->getElementById(AtomicString("fragment"));

    gfx::Rect fragment_rect_in_frame = fragment->GetLayoutObject()->AbsoluteBoundingBoxRect();
    gfx::Rect viewport_rect(child_viewport->VisibleContentRect().size());

    EXPECT_TRUE(viewport_rect.Contains(fragment_rect_in_frame))
        << "Fragment element at [" << fragment_rect_in_frame.ToString() << "] was not scrolled into viewport rect [" << viewport_rect.ToString() << "]";
...
TEST_F(ElementFragmentAnchorTest, AnchorRemovedBeforeBeginFrameCrash)
{
    ...
    ASSERT_TRUE(static_cast<ElementFragmentAnchor*>(GetDocument().View()->GetFragmentAnchor())->anchor_node_.Get());
    ...
    EXPECT_FALSE(static_cast<ElementFragmentAnchor*>(GetDocument().View()->GetFragmentAnchor())->anchor_node_.Get());
...
TEST_F(ElementFragmentAnchorTest, SVGDocumentDoesntCreateFragment)
{
    ...
    auto* img = To<HTMLImageElement>(GetDocument().getElementById(AtomicString("image")));
    auto* svg = To<SVGImage>(img->CachedImage()->GetImage());
    auto* view = DynamicTo<LocalFrameView>(svg->GetPageForTesting()->MainFrame()->View());
