...
bool FillsViewport(const Element& element)
{
    ...
    PhysicalRect rect = layout_box->IsLayoutIFrame() ? layout_box->PhysicalContentBoxRect() : layout_box->PhysicalPaddingBoxRect();
    ...
    gfx::Size controls_hidden_size = gfx::ToCeiledSize(gfx::ScaleSize(top_document.View()->LargeViewportSizeForViewportUnits(), zoom));

    if (bounding_box.size() != icb_size && bounding_box.size() != controls_hidden_size)
        return false;
...
void RootScrollerController::DidResizeFrameView()
{
    ...
    if (auto* frame_owner = DynamicTo<HTMLFrameOwnerElement>(EffectiveRootScroller())) {
        UpdateIFrameGeometryAndLayoutSize(*frame_owner);
    }
}

void RootScrollerController::DidUpdateIFrameFrameView(HTMLFrameOwnerElement& element)
{
...
bool RootScrollerController::RecomputeEffectiveRootScroller()
{
    ...
    if (effective_root_scroller_ == new_effective_root_scroller && effective_root_scroller_->IsEffectiveRootScroller())
        return false;
    ...
    if (LayoutBoxModelObject* new_obj = new_effective_root_scroller->GetLayoutBoxModelObject()) {
        if (new_obj->Layer()) {
            new_effective_root_scroller->GetLayoutBoxModelObject()->Layer()->SetNeedsCompositingInputsUpdate();
        }
    }

    DCHECK(old_effective_root_scroller);
    if (LayoutBoxModelObject* old_obj = old_effective_root_scroller->GetLayoutBoxModelObject()) {
        if (old_obj->Layer()) {
            old_effective_root_scroller->GetLayoutBoxModelObject()->Layer()->SetNeedsCompositingInputsUpdate();
        }
    }
...
bool RootScrollerController::IsValidRootScroller(const Element& element) const
{
    ...
    if (!element.GetLayoutObject()->IsScrollContainer() && !element.IsFrameOwnerElement())
        return false;
...
bool RootScrollerController::IsValidImplicitCandidate(const Element& element) const
{
    if (!element.IsInTreeScope())
        return false;
...
bool RootScrollerController::IsValidImplicit(const Element& element) const
{
    ...
    for (LayoutBox* ancestor = element.GetLayoutObject()->ContainingBlock(); ancestor; ancestor = ancestor->ContainingBlock()) {
        ...
        if (IsA<LayoutView>(ancestor)) {
            ...
            if (ancestor->ShouldClipOverflowAlongEitherAxis() || ancestor->HasMask() || ancestor->HasClip() || ancestor->HasClipPath()) {
                return false;
            }
        }
    }
...
void RootScrollerController::ApplyRootScrollerProperties(Node& node)
{
    ...
    if (IsA<LocalFrame>(frame_owner->ContentFrame())) {
        LocalFrameView* frame_view = DynamicTo<LocalFrameView>(frame_owner->OwnedEmbeddedContentView());
...
void RootScrollerController::UpdateIFrameGeometryAndLayoutSize(HTMLFrameOwnerElement& frame_owner) const
{
    DCHECK(document_->GetFrame());
    DCHECK(document_->GetFrame()->View());

    LocalFrameView* child_view = To<LocalFrameView>(frame_owner.OwnedEmbeddedContentView());
...
template <typename Function> void RootScrollerController::ForAllNonThrottledLocalControllers(const Function& function)
{
    ...
    for (Frame* child = frame->Tree().FirstChild(); child; child = child->Tree().NextSibling()) {
        ...
        if (Document* child_document = child_local_frame->GetDocument()) {
            child_document->GetRootScrollerController().ForAllNonThrottledLocalControllers(function);
        }
    }
...
bool RootScrollerController::PerformRootScrollerSelection()
{
    ...
    bool result = false;
    ForAllNonThrottledLocalControllers([&result](RootScrollerController& controller) { result |= controller.RecomputeEffectiveRootScroller(); });
    return result;
}
