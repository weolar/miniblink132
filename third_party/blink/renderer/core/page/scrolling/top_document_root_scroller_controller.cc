...
{
    if (!node || !node->GetLayoutObject() || !node->GetLayoutObject()->IsBoxModelObject())
        return nullptr;
...
void TopDocumentRootScrollerController::DidResizeViewport()
{
    ...
    auto* layout_object = To<LayoutBoxModelObject>(GlobalRootScroller()->GetLayoutObject());
...
gfx::Size TopDocumentRootScrollerController::RootScrollerVisibleArea() const
{
    ...
    float minimum_page_scale = page_->GetPageScaleConstraintsSet().FinalConstraints().minimum_scale;
    int browser_controls_adjustment = ceilf(page_->GetVisualViewport().BrowserControlsAdjustment() / minimum_page_scale);

    gfx::Size layout_size = TopDocument()->View()->LayoutViewport()->VisibleContentRect(kExcludeScrollbars).size();
    return gfx::Size(layout_size.width(), layout_size.height() + browser_controls_adjustment);
}
...
Node* TopDocumentRootScrollerController::FindGlobalRootScroller()
{
    ...
    Node* root_scroller = &TopDocument()->GetRootScrollerController().EffectiveRootScroller();
    ...
    while (auto* frame_owner = DynamicTo<HTMLFrameOwnerElement>(root_scroller)) {
        ...
        root_scroller = &iframe_document->GetRootScrollerController().EffectiveRootScroller();
    }
...
void TopDocumentRootScrollerController::UpdateGlobalRootScroller(Node* new_global_root_scroller)
{
    ...
    if (new_global_root_scroller == global_root_scroller_ && global_root_scroller_->GetLayoutObject()->IsGlobalRootScroller())
        return;
...
void TopDocumentRootScrollerController::UpdateCachedBits(Node* old_global, Node* new_global)
{
    if (old_global) {
...
void TopDocumentRootScrollerController::DidDisposeScrollableArea(ScrollableArea& area)
{
    if (!TopDocument() || !TopDocument()->View())
        return;
...
void TopDocumentRootScrollerController::Initialize(RootFrameViewport& root_frame_viewport, Document& main_document)
{
    DCHECK(page_);
