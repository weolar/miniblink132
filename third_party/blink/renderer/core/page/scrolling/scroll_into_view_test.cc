...
class ScrollIntoViewTest : public SimTest, public ::testing::WithParamInterface<std::vector<base::test::FeatureRef>> {
public:
    ScrollIntoViewTest()
    {
...
INSTANTIATE_TEST_SUITE_P(, ScrollIntoViewTest,
...
{
    v8::HandleScope HandleScope(WebView().GetPage()->GetAgentGroupScheduler().Isolate());
    WebView().MainFrameViewWidget()->Resize(gfx::Size(800, 600));
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete("<div id='space' style='height: 1000px'></div>"
    ...
    content->scrollIntoView(MakeGarbageCollected<V8UnionBooleanOrScrollIntoViewOptions>(options));
...
TEST_P(ScrollIntoViewTest, ScrollPaddingOnDocumentElWhenDocumentElDefinesViewport)
{
    v8::HandleScope HandleScope(WebView().GetPage()->GetAgentGroupScheduler().Isolate());
    WebView().MainFrameViewWidget()->Resize(gfx::Size(300, 300));
    SimRequest request("https://example.com/test.html", "text/html");
    ...
    // Sanity check that document element is the viewport defining element
    ASSERT_EQ(GetDocument().documentElement(), GetDocument().ViewportDefiningElement());
    ASSERT_EQ(Window().scrollY(), target->OffsetTop() - 10);
}
...
TEST_P(ScrollIntoViewTest, SmoothScroll)
{
    ...
    auto* arg = MakeGarbageCollected<V8UnionBooleanOrScrollIntoViewOptions>(options);
    Compositor().BeginFrame();
    ASSERT_EQ(Window().scrollY(), 0);
    ...
    ASSERT_NEAR(Window().scrollY(), (::features::IsImpulseScrollAnimationEnabled() ? 800 : 299), 1);
...
TEST_P(ScrollIntoViewTest, NestedContainer)
{
    ...
    if (RuntimeEnabledFeatures::MultiSmoothScrollIntoViewEnabled()) {
        ...
        // inner container.
        ASSERT_NEAR(container->scrollTop(), (::features::IsImpulseScrollAnimationEnabled() ? 800 : 299), 1);
    } else {
        ASSERT_EQ(container->scrollTop(), 0);
    }
    ...
    if (RuntimeEnabledFeatures::MultiSmoothScrollIntoViewEnabled()) {
        ...
        ASSERT_EQ(container->scrollTop(), content->OffsetTop() - container->OffsetTop());
    } else {
        ASSERT_EQ(container->scrollTop(), 0);
    }
    ...
    Compositor().BeginFrame(); // Set start_time = now.
    Compositor().BeginFrame(0.2);
    ASSERT_NEAR(container->scrollTop(), (::features::IsImpulseScrollAnimationEnabled() ? 794 : 299), 1);
...
TEST_P(ScrollIntoViewTest, NewScrollIntoViewAbortsCurrentAnimation)
{
    ...
    if (RuntimeEnabledFeatures::MultiSmoothScrollIntoViewEnabled()) {
        ...
        // container1.
        ASSERT_NEAR(container1->scrollTop(), (::features::IsImpulseScrollAnimationEnabled() ? 800 : 299), 1);
    } else {
        ASSERT_EQ(container1->scrollTop(), 0);
    }
    ...
    Compositor().BeginFrame(); // Set start_time = now.
    Compositor().BeginFrame(0.2);
    ASSERT_NEAR(Window().scrollY(), (::features::IsImpulseScrollAnimationEnabled() ? 171 : 61), 1);
    if (RuntimeEnabledFeatures::MultiSmoothScrollIntoViewEnabled()) {
        ...
        ASSERT_GT(container1->scrollTop(), (::features::IsImpulseScrollAnimationEnabled() ? 800 : 299));
    } else {
    ...
    if (RuntimeEnabledFeatures::MultiSmoothScrollIntoViewEnabled()) {
        // If MultiSmoothScrollIntoView is enabled, the frames will also have
        // scrolled inner container.
        ASSERT_EQ(container2->scrollTop(), content2->OffsetTop() - container2->OffsetTop());
    } else {
        ASSERT_EQ(container2->scrollTop(), 0);
    }
    ...
    Compositor().BeginFrame(); // Set start_time = now.
    Compositor().BeginFrame(0.2);
    ASSERT_NEAR(container2->scrollTop(), (::features::IsImpulseScrollAnimationEnabled() ? 952 : 300), 1);
...
TEST_P(ScrollIntoViewTest, NoOpScrollIntoViewContinuesCurrentAnimation)
{
    ...
    Element* container = GetDocument().getElementById(AtomicString("container"));
    Element* content = GetDocument().getElementById(AtomicString("content"));
    Element* visibleElement = GetDocument().getElementById(AtomicString("visibleElement"));
    ...
    Compositor().BeginFrame(); // Set start_time = now.
    Compositor().BeginFrame(0.2);
    ASSERT_NEAR(Window().scrollY(), (::features::IsImpulseScrollAnimationEnabled() ? 250 : 241), 1);
    if (RuntimeEnabledFeatures::MultiSmoothScrollIntoViewEnabled()) {
        ASSERT_NEAR(container->scrollTop(), (::features::IsImpulseScrollAnimationEnabled() ? 800 : 299), 1);
    } else {
        ASSERT_EQ(container->scrollTop(), 0);
    ...
    Compositor().BeginFrame();
    ASSERT_NEAR(Window().scrollY(), (::features::IsImpulseScrollAnimationEnabled() ? 258 : 260), 1);
    if (RuntimeEnabledFeatures::MultiSmoothScrollIntoViewEnabled()) {
        ASSERT_GT(container->scrollTop(), (::features::IsImpulseScrollAnimationEnabled() ? 800 : 299));
    } else {
...
TEST_P(ScrollIntoViewTest, ScrollWindowAbortsCurrentAnimation)
{
    ...
    Compositor().BeginFrame(); // Set start_time = now.
    Compositor().BeginFrame(0.2);
    ASSERT_NEAR(Window().scrollY(), (::features::IsImpulseScrollAnimationEnabled() ? 165 : 58), 1);
...
TEST_P(ScrollIntoViewTest, BlockAndInlineSettings)
{
    ...
    auto* arg1 = MakeGarbageCollected<V8UnionBooleanOrScrollIntoViewOptions>(options);
    content->scrollIntoView(arg1);
    ASSERT_EQ(Window().scrollX(), content->OffsetLeft() + content_width - window_width);
    ASSERT_EQ(Window().scrollY(), content->OffsetTop() + content_height - window_height);

    options->setBlock("start");
    options->setInlinePosition("start");
    auto* arg2 = MakeGarbageCollected<V8UnionBooleanOrScrollIntoViewOptions>(options);
    ...
    auto* arg3 = MakeGarbageCollected<V8UnionBooleanOrScrollIntoViewOptions>(options);
    content->scrollIntoView(arg3);
    ASSERT_EQ(Window().scrollX(), content->OffsetLeft() + (content_width - window_width) / 2);
    ASSERT_EQ(Window().scrollY(), content->OffsetTop() + (content_height - window_height) / 2);

    options->setBlock("end");
    options->setInlinePosition("end");
    auto* arg4 = MakeGarbageCollected<V8UnionBooleanOrScrollIntoViewOptions>(options);
...
TEST_P(ScrollIntoViewTest, SmoothAndInstantInChain)
{
    ...
    Element* container = GetDocument().getElementById(AtomicString("container"));
    Element* inner_container = GetDocument().getElementById(AtomicString("inner_container"));
    Element* content = GetDocument().getElementById(AtomicString("content"));
    ScrollIntoViewOptions* options = ScrollIntoViewOptions::Create();
    options->setBlock("start");
    auto* arg = MakeGarbageCollected<V8UnionBooleanOrScrollIntoViewOptions>(options);
    ...
    // Finish scrolling the container
    Compositor().BeginFrame(1);
    ASSERT_EQ(container->scrollTop(), inner_container->OffsetTop() - container->OffsetTop());
    // Instant scroll of the inner container should have finished.
    ASSERT_EQ(inner_container->scrollTop(), content->OffsetTop() - inner_container->OffsetTop());
}
...
TEST_P(ScrollIntoViewTest, FindDoesNotScrollOverflowHidden)
{
    ...
    MainFrame().GetFindInPage()->FindInternal(kFindIdentifier, WebString::FromUTF8("hello"), *options, false);
    ASSERT_EQ(container->scrollTop(), 0);
}
...
TEST_P(ScrollIntoViewTest, StopAtLayoutViewportForFocusedEditable)
{
    ...
    auto params = scroll_into_view_util::CreateScrollIntoViewParams(
        ScrollAlignment::LeftAlways(), ScrollAlignment::TopAlways(), mojom::blink::ScrollType::kProgrammatic, false, mojom::blink::ScrollBehavior::kInstant);
    ...
    params->for_focused_editable->size = gfx::SizeF(target->AbsoluteBoundingBoxRect().size());
    params->for_focused_editable->can_zoom = false;

    scroll_into_view_util::ScrollRectToVisible(*target, PhysicalRect(target->AbsoluteBoundingBoxRect()), std::move(params));

    ScrollableArea* root_scroller = To<LayoutBox>(root->GetLayoutObject())->GetScrollableArea();
    ScrollableArea* inner_scroller = To<LayoutBox>(inner->GetLayoutObject())->GetScrollableArea();

    // Only the inner scroller should have scrolled. The root_scroller shouldn't
    // scroll because it is the layout viewport.
    ASSERT_EQ(root_scroller, &GetDocument().View()->GetRootFrameViewport()->LayoutViewport());
...
TEST_P(ScrollIntoViewTest, SmoothUserScrollNotAbortedByProgrammaticScrolls)
{
    ...
    // A smooth UserScroll.
    Element* content = GetDocument().getElementById(AtomicString("content"));
    scroll_into_view_util::ScrollRectToVisible(*content->GetLayoutObject(), content->BoundingBoxForScrollIntoView(),
        scroll_into_view_util::CreateScrollIntoViewParams(ScrollAlignment::ToEdgeIfNeeded(), ScrollAlignment::TopAlways(), mojom::blink::ScrollType::kUser,
            false, mojom::blink::ScrollBehavior::kSmooth, true));
...
TEST_P(ScrollIntoViewTest, LongDistanceSmoothScrollFinishedInThreeSeconds)
{
    ...
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete("<div id='space' style='height: 100000px'></div>"
    ...
    Compositor().BeginFrame(); // Set start_time = now.
    Compositor().BeginFrame(0.2);
    ASSERT_NEAR(Window().scrollY(), (::features::IsImpulseScrollAnimationEnabled() ? 79389 : 16971), 1);
...
TEST_P(ScrollIntoViewTest, OriginCrossingUseCounter)
{
    ...
    Element* local_child_frame = GetDocument().getElementById(AtomicString("localChildFrame"));
    Element* xorigin_child_frame = GetDocument().getElementById(AtomicString("xoriginChildFrame"));
    Document* local_child_document = To<HTMLIFrameElement>(local_child_frame)->contentDocument();
    Document* xorigin_child_document = To<HTMLIFrameElement>(xorigin_child_frame)->contentDocument();

    // Same origin frames shouldn't count the scroll into view.
    {
        ASSERT_EQ(GetDocument().View()->GetScrollableArea()->GetScrollOffset(), ScrollOffset(0, 0));

        Element* target = local_child_document->getElementById(AtomicString("target"));
        target->scrollIntoView();

        ASSERT_NE(GetDocument().View()->GetScrollableArea()->GetScrollOffset(), ScrollOffset(0, 0));
        EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kCrossOriginScrollIntoView));
    }

    GetDocument().View()->GetScrollableArea()->SetScrollOffset(ScrollOffset(0, 0), mojom::blink::ScrollType::kProgrammatic);

    // Cross origin frames should record the scroll into view use count.
    {
        ASSERT_EQ(GetDocument().View()->GetScrollableArea()->GetScrollOffset(), ScrollOffset(0, 0));

        Element* target = xorigin_child_document->getElementById(AtomicString("target"));
        target->scrollIntoView();

        ASSERT_NE(GetDocument().View()->GetScrollableArea()->GetScrollOffset(), ScrollOffset(0, 0));
        EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kCrossOriginScrollIntoView));
    }
}
...
TEST_P(ScrollIntoViewTest, FromDisplayNoneIframe)
{
    ...
    Element* child_frame = GetDocument().getElementById(AtomicString("childFrame"));
    ...
    child_frame->setAttribute(html_names::kStyleAttr, AtomicString("display:none"));
    Compositor().BeginFrame();
    ...
    auto params = scroll_into_view_util::CreateScrollIntoViewParams(
        ScrollAlignment::LeftAlways(), ScrollAlignment::TopAlways(), mojom::blink::ScrollType::kProgrammatic, false, mojom::blink::ScrollBehavior::kInstant);
    scroll_into_view_util::ScrollRectToVisible(*child_document->GetLayoutView(), rect, std::move(params));
...
TEST_P(ScrollIntoViewTest, EmptyEditableElementRect)
{
    ...
    EXPECT_EQ(GetDocument().View()->GetScrollableArea()->GetScrollOffset(), ScrollOffset(0, 0));
}
