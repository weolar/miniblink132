...
class RootScrollerTest : public testing::Test, private ScopedImplicitRootScrollerForTest {
public:
    RootScrollerTest()
    ...
    void ExecuteScript(const WebString& code, WebLocalFrame& frame)
    {
        ...
        frame.View()->MainFrameWidget()->UpdateAllLifecyclePhases(DocumentUpdateReason::kTest);
        RunPendingTasks();
    }
    ...
    Page& GetPage() const
    {
        return *GetWebView()->GetPage();
    }

    PaintLayerScrollableArea* GetScrollableArea(const Element& element) const
    {
        return To<LayoutBoxModelObject>(element.GetLayoutObject())->GetScrollableArea();
    }
    ...
    {
        return GetWebView()->MainFrameImpl();
    }
    ...
    WebGestureEvent GenerateTouchGestureEvent(WebInputEvent::Type type, int delta_x = 0, int delta_y = 0)
    {
    ...
    void SetCreateWebFrameWidgetCallback(const frame_test_helpers::CreateTestWebFrameWidgetCallback& create_widget_callback)
    {
        create_widget_callback_ = create_widget_callback;
    }

    bool UsesCompositedScrolling(const PaintLayerScrollableArea* scrollable_area)
    {
        auto* property_trees = MainFrameView()->RootCcLayer()->layer_tree_host()->property_trees();
        auto* scroll_node = property_trees->scroll_tree_mutable().FindNodeFromElementId(scrollable_area->GetScrollElementId());
...
protected:
    ...
    {
        helper_ = std::make_unique<frame_test_helpers::WebViewHelper>(create_widget_callback_);

        helper_->InitializeAndLoad(url.Utf8(), nullptr, nullptr, &ConfigureSettings);
        ...
        helper_->GetMainFrameWidget()->UpdateCompositorViewportRect(gfx::Rect(viewport_size));
...
TEST_F(RootScrollerTest, TestDefaultRootScroller)
{
    ...
    EXPECT_EQ(MainFrame()->GetDocument(), EffectiveRootScroller(MainFrame()->GetDocument()));
}
...
TEST_F(RootScrollerTest, defaultEffectiveRootScrollerIsDocumentNode)
{
    ...
    nodes.push_back(MakeGarbageCollected<V8UnionNodeOrStringOrTrustedScript>(iframe));
...
TEST_F(RootScrollerTest, BrowserControlsAndOverscroll)
{
    ...
    Element* container = MainFrame()->GetDocument()->getElementById(AtomicString("container"));
    ...
    widget->DispatchThroughCcInputHandler(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureScrollBegin));
    {
        ...
        widget->DispatchThroughCcInputHandler(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureScrollUpdate, 0, -GetBrowserControls().TopHeight()));
        layer_tree_host->CompositeForTest(base::TimeTicks::Now(), false, base::OnceClosure());
...

    {
        ...
        widget->DispatchThroughCcInputHandler(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureScrollUpdate, 0, -100));
        layer_tree_host->CompositeForTest(base::TimeTicks::Now(), false, base::OnceClosure());

        EXPECT_FLOAT_EQ(100, container->scrollTop());
        EXPECT_FLOAT_EQ(0, MainFrameView()->LayoutViewport()->GetScrollOffset().y());
    }

    {
        // Scroll 50 pixels past the end. Ensure we report the 50 pixels as
        // overscroll.
        widget->DispatchThroughCcInputHandler(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureScrollUpdate, 0, -440));
        layer_tree_host->CompositeForTest(base::TimeTicks::Now(), false, base::OnceClosure());

        EXPECT_TRUE(widget->last_overscroll()->Equals(mojom::blink::DidOverscrollParams(
            gfx::Vector2dF(0, 50), gfx::Vector2dF(0, 50), gfx::Vector2dF(), gfx::PointF(100, 100), cc::OverscrollBehavior())));
...

    {
        ...
        widget->DispatchThroughCcInputHandler(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureScrollUpdate, 0, -20));
        layer_tree_host->CompositeForTest(base::TimeTicks::Now(), false, base::OnceClosure());

        EXPECT_TRUE(widget->last_overscroll()->Equals(mojom::blink::DidOverscrollParams(
            gfx::Vector2dF(0, 70), gfx::Vector2dF(0, 20), gfx::Vector2dF(), gfx::PointF(100, 100), cc::OverscrollBehavior())));

        EXPECT_FLOAT_EQ(maximum_scroll, container->scrollTop());
        EXPECT_FLOAT_EQ(0, MainFrameView()->LayoutViewport()->GetScrollOffset().y());
    }

    widget->DispatchThroughCcInputHandler(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureScrollEnd));
    layer_tree_host->CompositeForTest(base::TimeTicks::Now(), false, base::OnceClosure());

    {
        ...
        widget->DispatchThroughCcInputHandler(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureScrollUpdate, 0, -30));
        layer_tree_host->CompositeForTest(base::TimeTicks::Now(), false, base::OnceClosure());

        EXPECT_TRUE(widget->last_overscroll()->Equals(mojom::blink::DidOverscrollParams(
            gfx::Vector2dF(0, 30), gfx::Vector2dF(0, 30), gfx::Vector2dF(), gfx::PointF(100, 100), cc::OverscrollBehavior())));
...
TEST_F(RootScrollerTest, TestRootScrollerBecomesInvalid)
{
...

    {
        ...
        ExecuteScript("document.querySelector('#container').style.display = 'inline'");
        UpdateAllLifecyclePhases(MainFrameView());
...
TEST_F(RootScrollerTest, RemoveCurrentRootScroller)
{
    ...
    RootScrollerController& controller = MainFrame()->GetDocument()->GetRootScrollerController();
    Element* container = MainFrame()->GetDocument()->getElementById(AtomicString("container"));
    UpdateAllLifecyclePhases(MainFrameView());
...
TEST_F(RootScrollerTest, AlwaysCreateCompositedScrollingLayers)
{
    ...
    ExecuteScript("document.querySelector('#container').style.width = '98%'");
    ASSERT_EQ(MainFrame()->GetDocument(), EffectiveRootScroller(MainFrame()->GetDocument()));
...
TEST_F(RootScrollerTest, IFrameSwapToRemote)
{
    ...
    Element* iframe = MainFrame()->GetDocument()->getElementById(AtomicString("iframe"));
    ...
    {
        frame_test_helpers::SwapRemoteFrame(MainWebFrame()->FirstChild(), frame_test_helpers::CreateRemote());
        UpdateAllLifecyclePhases(MainFrameView());
        EXPECT_EQ(MainFrame()->GetDocument(), EffectiveRootScroller(MainFrame()->GetDocument()));
...
TEST_F(RootScrollerTest, RemoveRootScrollerFromDom)
{
    ...
    {
        auto* iframe = To<HTMLFrameOwnerElement>(MainFrame()->GetDocument()->getElementById(AtomicString("iframe")));
...
TEST_F(RootScrollerTest, DocumentElementHasNoLayoutObject)
{
    ...
    const TopDocumentRootScrollerController& global_controller = MainFrame()->GetDocument()->GetPage()->GlobalRootScrollerController();
...
TEST_F(RootScrollerTest, UseVisualViewportScrollbarsIframe)
{
    ...
    Element* iframe = MainFrame()->GetDocument()->getElementById(AtomicString("iframe"));
    auto* child_frame = To<LocalFrame>(To<HTMLFrameOwnerElement>(iframe)->ContentFrame());
...
TEST_F(RootScrollerTest, TopControlsAdjustmentAppliedToRootScroller)
{
    ...
    widget->DispatchThroughCcInputHandler(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureScrollUpdate, 0, -3000));
    layer_tree_host->CompositeForTest(base::TimeTicks::Now(), false, base::OnceClosure());
...
TEST_F(RootScrollerTest, RotationAnchoring)
{
    ...
    Element* target = MainFrame()->GetDocument()->getElementById(AtomicString("target"));
    ...
    // viepwort.
    {
        ...
        widget->DispatchThroughCcInputHandler(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureScrollBegin));
        widget->DispatchThroughCcInputHandler(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureScrollUpdate, -scroll_x, -scroll_y));
        widget->DispatchThroughCcInputHandler(GenerateTouchGestureEvent(WebInputEvent::Type::kGestureScrollEnd));
        layer_tree_host->CompositeForTest(base::TimeTicks::Now(), false, base::OnceClosure());
...
TEST_F(RootScrollerTest, ImmediateUpdateOfLayoutViewport)
{
    ...
    RootScrollerController& main_controller = MainFrame()->GetDocument()->GetRootScrollerController();

    auto* iframe_local_frame = To<LocalFrame>(iframe->ContentFrame());
    EXPECT_EQ(iframe, &main_controller.EffectiveRootScroller());
    EXPECT_EQ(iframe_local_frame->View()->LayoutViewport(), &MainFrameView()->GetRootFrameViewport()->LayoutViewport());

    // Remove the <iframe> and make sure the layout viewport reverts to the
    // LocalFrameView without a layout.
    iframe->remove();

    EXPECT_EQ(MainFrameView()->LayoutViewport(), &MainFrameView()->GetRootFrameViewport()->LayoutViewport());
}
...
TEST_F(ImplicitRootScrollerSimTest, NonLifecycleLayoutDoesntCauseReselection)
{
    ...
    Compositor().BeginFrame();
    ASSERT_EQ(container, GetDocument().GetRootScrollerController().EffectiveRootScroller());

    container->style()->setProperty(GetDocument().GetExecutionContext(), "width", "95%", String(), ASSERT_NO_EXCEPTION);
    ...
    EXPECT_EQ(container, GetDocument().GetRootScrollerController().EffectiveRootScroller());

    Compositor().BeginFrame();

    EXPECT_EQ(GetDocument(), GetDocument().GetRootScrollerController().EffectiveRootScroller());
}
...
TEST_F(ImplicitRootScrollerSimTest, UsePaddingBoxForViewportFillingCondition)
{
    ...
    container->setAttribute(html_names::kStyleAttr, AtomicString("border: 1px solid black"));
    Compositor().BeginFrame();
    EXPECT_EQ(&GetDocument(), GetDocument().GetRootScrollerController().EffectiveRootScroller());
}
...
TEST_F(ImplicitRootScrollerSimTest, RootScrollerDoesntAffectVisualViewport)
{
    ...
    GetDocument().GetPage()->GetVisualViewport().SetLocation(gfx::PointF(100, 120));
...
TEST_F(ImplicitRootScrollerSimTest, ImplicitRootScroller)
{
    ...
    ASSERT_EQ(&GetDocument(), GetDocument().GetRootScrollerController().EffectiveRootScroller());
    Element* container = GetDocument().getElementById(AtomicString("container"));
    ...
    Vector<std::tuple<String, String, Node*>> test_cases = { { "overflow", "hidden", &GetDocument() }, { "overflow", "auto", container },
        { "overflow", "scroll", container }, { "overflow", "visible", &GetDocument() },
        // Overflow: hidden in one axis forces the other axis to auto so it should
        // be promoted.
        { "overflow-x", "hidden", container }, { "overflow-x", "auto", container }, { "overflow-x", "scroll", container },
        { "overflow-x", "visible", &GetDocument() }, { "overflow-y", "hidden", container }, { "overflow-y", "auto", container },
        { "overflow-y", "scroll", container }, { "overflow-y", "visible", &GetDocument() } };
    ...
    for (auto test_case : test_cases) {
        ...
        container->style()->setProperty(GetDocument().GetExecutionContext(), style, style_val, String(), ASSERT_NO_EXCEPTION);
        Compositor().BeginFrame();
        ASSERT_EQ(expected_root_scroller, GetDocument().GetRootScrollerController().EffectiveRootScroller())
            << "Failed to set rootScroller after setting " << std::get<0>(test_case) << ": " << std::get<1>(test_case);
        container->style()->setProperty(GetDocument().GetExecutionContext(), std::get<0>(test_case), String(), String(), ASSERT_NO_EXCEPTION);
        Compositor().BeginFrame();
        ASSERT_EQ(&GetDocument(), GetDocument().GetRootScrollerController().EffectiveRootScroller())
            << "Failed to reset rootScroller after setting " << std::get<0>(test_case) << ": " << std::get<1>(test_case);
    }
...
TEST_F(ImplicitRootScrollerSimTest, ImplicitRootScrollerAddOverflow)
{
    ...
    ASSERT_EQ(&GetDocument(), GetDocument().GetRootScrollerController().EffectiveRootScroller()) << "Shouldn't promote 'container' since it has no overflow.";

    Element* spacer = GetDocument().getElementById(AtomicString("spacer"));
    spacer->style()->setProperty(GetDocument().GetExecutionContext(), "height", "2000px", String(), ASSERT_NO_EXCEPTION);
    spacer->style()->setProperty(GetDocument().GetExecutionContext(), "width", "2000px", String(), ASSERT_NO_EXCEPTION);
    Compositor().BeginFrame();
    Element* container = GetDocument().getElementById(AtomicString("container"));
    EXPECT_EQ(container, GetDocument().GetRootScrollerController().EffectiveRootScroller()) << "Adding overflow should cause 'container' to be promoted.";
}
...
TEST_F(ImplicitRootScrollerSimTest, ImplicitRootScrollerDocumentScrollsOverflow)
{
    WebView().MainFrameViewWidget()->Resize(gfx::Size(800, 600));
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    ...
    overflow->style()->setProperty(GetDocument().GetExecutionContext(), "height", "10px", String(), ASSERT_NO_EXCEPTION);
    overflow->style()->setProperty(GetDocument().GetExecutionContext(), "width", "10px", String(), ASSERT_NO_EXCEPTION);
    Compositor().BeginFrame();
    EXPECT_EQ(&GetDocument(), GetDocument().GetRootScrollerController().EffectiveRootScroller())
...
TEST_F(ImplicitRootScrollerSimTest, ImplicitRootScrollerVisibilityCondition)
{
    ...
    container->style()->setProperty(GetDocument().GetExecutionContext(), "opacity", "0.5", String(), ASSERT_NO_EXCEPTION);
    Compositor().BeginFrame();
    EXPECT_EQ(&GetDocument(), GetDocument().GetRootScrollerController().EffectiveRootScroller()) << "Adding opacity to 'container' causes it to be demoted.";

    container->style()->setProperty(GetDocument().GetExecutionContext(), "opacity", "", String(), ASSERT_NO_EXCEPTION);
    Compositor().BeginFrame();
    EXPECT_EQ(container, GetDocument().GetRootScrollerController().EffectiveRootScroller()) << "Removing opacity from 'container' causes it to be promoted.";

    container->style()->setProperty(GetDocument().GetExecutionContext(), "visibility", "hidden", String(), ASSERT_NO_EXCEPTION);
    Compositor().BeginFrame();
    EXPECT_EQ(&GetDocument(), GetDocument().GetRootScrollerController().EffectiveRootScroller()) << "visibility:hidden causes 'container' to be demoted.";

    container->style()->setProperty(GetDocument().GetExecutionContext(), "visibility", "collapse", String(), ASSERT_NO_EXCEPTION);
    Compositor().BeginFrame();
    EXPECT_EQ(&GetDocument(), GetDocument().GetRootScrollerController().EffectiveRootScroller())
        << "visibility:collapse doesn't cause 'container' to be promoted.";

    container->style()->setProperty(GetDocument().GetExecutionContext(), "visibility", "visible", String(), ASSERT_NO_EXCEPTION);
    Compositor().BeginFrame();
    EXPECT_EQ(container, GetDocument().GetRootScrollerController().EffectiveRootScroller()) << "visibility:visible causes promotion";
}
...
TEST_F(ImplicitRootScrollerSimTest, ImplicitRootScrollerIframe)
{
    ...
    container->style()->setProperty(GetDocument().GetExecutionContext(), "height", "95%", String(), ASSERT_NO_EXCEPTION);
    Compositor().BeginFrame();
...
TEST_F(ImplicitRootScrollerSimTest, UseCounterNegative)
{
    ...
    Element* container = GetDocument().getElementById(AtomicString("container"));
    ASSERT_NE(container, GetDocument().GetRootScrollerController().EffectiveRootScroller());

    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kActivatedImplicitRootScroller));

    container->style()->setProperty(GetDocument().GetExecutionContext(), "height", "150%", String(), ASSERT_NO_EXCEPTION);
    Compositor().BeginFrame();
...
TEST_F(ImplicitRootScrollerSimTest, UseCounterPositive)
{
    ...
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kActivatedImplicitRootScroller));
...
TEST_F(ImplicitRootScrollerSimTest, UseCounterPositiveAfterLoad)
{
    ...
    container->style()->setProperty(GetDocument().GetExecutionContext(), "height", "100%", String(), ASSERT_NO_EXCEPTION);
    Compositor().BeginFrame();
...
TEST_F(ImplicitRootScrollerSimTest, LayoutTreeReplaced)
{
    ...
    Element* scroller = GetDocument().getElementById(AtomicString("rootscroller"));
    ASSERT_EQ(scroller, GetDocument().GetRootScrollerController().EffectiveRootScroller());
...
TEST_F(ImplicitRootScrollerSimTest, DontPromoteWhenMultipleAreValid)
{
    ...
    // promoted.
    Element* container2 = GetDocument().getElementById(AtomicString("container2"));
    container2->style()->setProperty(GetDocument().GetExecutionContext(), "height", "95%", String(), ASSERT_NO_EXCEPTION);
    Compositor().BeginFrame();
...
TEST_F(ImplicitRootScrollerSimTest, IframeLoadedWithoutLayout)
{
    ...
    Compositor().BeginFrame();
    ASSERT_EQ(GetDocument(), GetDocument().GetRootScrollerController().EffectiveRootScroller()) << "The iframe isn't yet scrollable.";
    ...
    Compositor().BeginFrame();
    EXPECT_EQ(GetDocument().getElementById(AtomicString("container")), GetDocument().GetRootScrollerController().EffectiveRootScroller())
...
TEST_F(ImplicitRootScrollerSimTest, NavigateToValidRemainsRootScroller)
{
    ...
    Compositor().BeginFrame();
    ASSERT_EQ(GetDocument().getElementById(AtomicString("container")), GetDocument().GetRootScrollerController().EffectiveRootScroller());
    ...
    frame_test_helpers::LoadFrameDontWait(WebView().MainFrameImpl()->FirstChild()->ToWebLocalFrame(), KURL("https://example.com/child-next.html"));
...
TEST_F(ImplicitRootScrollerSimTest, ScrollRestorationIgnoresImplicit)
{
    ...
    GetDocument().View()->GetScrollableArea()->SetPendingHistoryRestoreScrollOffset(view_state, true, mojom::blink::ScrollBehavior::kAuto);
    GetDocument().View()->LayoutViewport()->SetPendingHistoryRestoreScrollOffset(view_state, true, mojom::blink::ScrollBehavior::kAuto);
    GetDocument().View()->ScheduleAnimation();

    Compositor().BeginFrame();
    EXPECT_EQ(ScrollOffset(0, 0), GetDocument().View()->GetScrollableArea()->GetScrollOffset());
    ...
    Compositor().BeginFrame();
    EXPECT_FALSE(GetDocument().GetFrame()->Loader().GetDocumentLoader()->GetHistoryItem()->GetViewState());
}

// Test that a root scroller is considered to fill the viewport at both the URL
// bar shown and URL bar hidden height.
TEST_F(ImplicitRootScrollerSimTest, RootScrollerFillsViewportAtBothURLBarStates)
{
    ...
    // should be demoted.
    container->style()->setProperty(GetDocument().GetExecutionContext(), "height", "601px", String(), ASSERT_NO_EXCEPTION);
    Compositor().BeginFrame();
    EXPECT_EQ(GetDocument(), GetDocument().GetRootScrollerController().EffectiveRootScroller());

    // Reset back to valid and hide the top controls. Zoom to 2x. Ensure we're
    // still considered valid.
    container->style()->setProperty(GetDocument().GetExecutionContext(), "height", "", String(), ASSERT_NO_EXCEPTION);
    Compositor().BeginFrame();
    EXPECT_EQ(container, GetDocument().GetRootScrollerController().EffectiveRootScroller());
...
TEST_F(ImplicitRootScrollerSimTest, ContinuallyReevaluateImplicitPromotion)
{
    ...
    container->style()->setProperty(GetDocument().GetExecutionContext(), "overflow", "auto", String(), ASSERT_NO_EXCEPTION);
    Compositor().BeginFrame();
    EXPECT_EQ(container, GetDocument().GetRootScrollerController().EffectiveRootScroller());

    // The container is now not viewport-filling so it should be demoted.
    container->style()->setProperty(GetDocument().GetExecutionContext(), "transform", "translateX(-50px)", String(), ASSERT_NO_EXCEPTION);
    Compositor().BeginFrame();
    EXPECT_EQ(GetDocument(), GetDocument().GetRootScrollerController().EffectiveRootScroller());

    // The container is viewport-filling again so it should be promoted.
    parent->style()->setProperty(GetDocument().GetExecutionContext(), "transform", "translateX(50px)", String(), ASSERT_NO_EXCEPTION);
    Compositor().BeginFrame();
    EXPECT_EQ(container, GetDocument().GetRootScrollerController().EffectiveRootScroller());

    // No longer scrollable so demote.
    container->style()->setProperty(GetDocument().GetExecutionContext(), "overflow", "hidden", String(), ASSERT_NO_EXCEPTION);
    Compositor().BeginFrame();
    EXPECT_EQ(GetDocument(), GetDocument().GetRootScrollerController().EffectiveRootScroller());
}
...
TEST_F(ImplicitRootScrollerSimTest, IframeScrollingAffectsPromotion)
{
    ...
    auto* container = To<HTMLFrameOwnerElement>(GetDocument().getElementById(AtomicString("container")));
    ...
    inner_html_element->style()->setProperty(To<LocalDOMWindow>(container->contentWindow()), "overflow", "auto", String(), ASSERT_NO_EXCEPTION);
    Compositor().BeginFrame();
    EXPECT_EQ(container, GetDocument().GetRootScrollerController().EffectiveRootScroller());

    // Demote again.
    inner_html_element->style()->setProperty(To<LocalDOMWindow>(container->contentWindow()), "overflow", "hidden", String(), ASSERT_NO_EXCEPTION);
    Compositor().BeginFrame();
    EXPECT_EQ(GetDocument(), GetDocument().GetRootScrollerController().EffectiveRootScroller());
}
...
TEST_F(ImplicitRootScrollerSimTest, BottomFixedAffectedByTopControls)
{
    ...
    Element* container1 = GetDocument().getElementById(AtomicString("container1"));
    Element* container2 = GetDocument().getElementById(AtomicString("container2"));
    ASSERT_EQ(container1, GetDocument().GetRootScrollerController().EffectiveRootScroller()) << "The #container1 iframe must be promoted.";

    Document* child1_document = To<HTMLFrameOwnerElement>(container1)->contentDocument();
    Document* child2_document = To<HTMLFrameOwnerElement>(container2)->contentDocument();
    LayoutObject* fixed_layout = GetDocument().getElementById(AtomicString("fixed"))->GetLayoutObject();
    LayoutObject* fixed_layout1 = child1_document->getElementById(AtomicString("fixed"))->GetLayoutObject();
    LayoutObject* fixed_layout2 = child2_document->getElementById(AtomicString("fixed"))->GetLayoutObject();

    EXPECT_TRUE(fixed_layout->FirstFragment().PaintProperties()->PaintOffsetTranslation()->IsAffectedByOuterViewportBoundsDelta());
    EXPECT_TRUE(fixed_layout1->FirstFragment().PaintProperties()->PaintOffsetTranslation()->IsAffectedByOuterViewportBoundsDelta());
    EXPECT_FALSE(fixed_layout2->FirstFragment().PaintProperties()->PaintOffsetTranslation()->IsAffectedByOuterViewportBoundsDelta());
}
...
TEST_F(ImplicitRootScrollerSimTest, IframeUsesContentBox)
{
    ...
    ASSERT_EQ(iframe, GetDocument().GetRootScrollerController().EffectiveRootScroller()) << "The iframe should start off promoted.";

    // Adding padding should cause the iframe to be demoted.
    {
        iframe->setAttribute(html_names::kStyleAttr, AtomicString("padding-left: 20%"));
        Compositor().BeginFrame();

        EXPECT_NE(iframe, GetDocument().GetRootScrollerController().EffectiveRootScroller()) << "The iframe should be demoted once it has padding.";
    }

    // Replacing padding with a border should also ensure the iframe remains
    // demoted.
    {
        iframe->setAttribute(html_names::kStyleAttr, AtomicString("border: 5px solid black"));
        Compositor().BeginFrame();

        EXPECT_NE(iframe, GetDocument().GetRootScrollerController().EffectiveRootScroller()) << "The iframe should be demoted once it has border.";
    }
    ...
    ASSERT_EQ(iframe, GetDocument().GetRootScrollerController().EffectiveRootScroller()) << "The iframe should once again be promoted when border is removed";
}
...
TEST_F(ImplicitRootScrollerSimTest, OverflowInMainDocumentRestrictsImplicit)
{
    ...
    Element* spacer = GetDocument().getElementById(AtomicString("spacer"));
    spacer->style()->setProperty(GetDocument().GetExecutionContext(), "height", "100%", String(), ASSERT_NO_EXCEPTION);
    Compositor().BeginFrame();
...
TEST_F(ImplicitRootScrollerSimTest, OverflowHiddenDoesntRestrictImplicit)
{
    ...
    html->style()->setProperty(GetDocument().GetExecutionContext(), "overflow", "auto", String(), ASSERT_NO_EXCEPTION);
    Compositor().BeginFrame();

    EXPECT_EQ(GetDocument(), GetDocument().GetRootScrollerController().EffectiveRootScroller())
        << "iframe should now be demoted since main document scrolls overflow.";

    html->style()->setProperty(GetDocument().GetExecutionContext(), "overflow", "visible", String(), ASSERT_NO_EXCEPTION);
    Compositor().BeginFrame();
...
TEST_F(ImplicitRootScrollerSimTest, ClippingAncestorPreventsPromotion)
{
    ...
    Vector<std::tuple<String, String>> test_cases = { { "overflow", "scroll" }, { "overflow", "hidden" }, { "overflow", "auto" }, { "contain", "paint" },
        { "-webkit-mask-image", "linear-gradient(black 25%, transparent 50%)" }, { "clip", "rect(10px, 290px, 190px, 10px" }, { "clip-path", "circle(40%)" } };
    ...
    for (auto test_case : test_cases) {
        ...
        ASSERT_EQ(iframe, GetDocument().GetRootScrollerController().EffectiveRootScroller()) << "iframe should start off promoted.";

        ancestor->style()->setProperty(GetDocument().GetExecutionContext(), style, style_val, String(), ASSERT_NO_EXCEPTION);
        Compositor().BeginFrame();

        EXPECT_EQ(GetDocument(), GetDocument().GetRootScrollerController().EffectiveRootScroller())
            << "iframe should be demoted since ancestor has " << style << ": " << style_val;

        ancestor->style()->setProperty(GetDocument().GetExecutionContext(), style, String(), String(), ASSERT_NO_EXCEPTION);
        Compositor().BeginFrame();
        ASSERT_EQ(iframe, GetDocument().GetRootScrollerController().EffectiveRootScroller())
            << "iframe should be promoted since ancestor removed " << style << ": " << style_val;
    }
}
...
private:
    void HideTopControlsWithMaximalScroll()
    {
        ...
        ASSERT_EQ(1, GetBrowserControls().TopShownRatio());
        ASSERT_EQ(1, GetBrowserControls().BottomShownRatio());
        WebView().MainFrameWidget()->ApplyViewportChangesForTesting({ gfx::Vector2dF(), gfx::Vector2dF(), 1, false, -1, -1, cc::BrowserControlsState::kBoth });
        ASSERT_EQ(0, GetBrowserControls().TopShownRatio());
        ASSERT_EQ(0, GetBrowserControls().BottomShownRatio());

        Node* scroller = GetDocument().GetPage()->GlobalRootScrollerController().GlobalRootScroller();
        ScrollableArea* scrollable_area = To<LayoutBox>(scroller->GetLayoutObject())->GetScrollableArea();
...
TEST_F(RootScrollerHitTest, HitTestInAreaRevealedByURLBarSameLayer)
{
    ...
    // the root scroller.
    ASSERT_EQ(To<LayoutBox>(target->GetLayoutObject())->EnclosingLayer(), To<LayoutBox>(container->GetLayoutObject())->Layer());
...
TEST_F(RootScrollerHitTest, HitTestInAreaRevealedByURLBarDifferentLayer)
{
    ...
    ASSERT_NE(To<LayoutBox>(target->GetLayoutObject())->EnclosingLayer(), To<LayoutBox>(container->GetLayoutObject())->Layer());
