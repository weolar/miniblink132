...
void StickyPositionScrollingConstraints::ComputeStickyOffset(const gfx::PointF& scroll_position)
{
    PhysicalRect sticky_box_rect = scroll_container_relative_sticky_box_rect;
    PhysicalRect containing_block_rect = scroll_container_relative_containing_block_rect;
    PhysicalOffset ancestor_sticky_box_offset = AncestorStickyBoxOffset();
    PhysicalOffset ancestor_containing_block_offset = AncestorContainingBlockOffset();
    ...
    // being called on us.
    sticky_box_rect.Move(ancestor_sticky_box_offset + ancestor_containing_block_offset);
    ...
    if (right_inset) {
        ...
        LayoutUnit available_space = containing_block_rect.X() - sticky_box_rect.X();
    ...
    if (left_inset) {
        ...
        LayoutUnit available_space = containing_block_rect.Right() - sticky_box_rect.Right();
    ...
    if (bottom_inset) {
        ...
        LayoutUnit available_space = containing_block_rect.Y() - sticky_box_rect.Y();
    ...
    if (top_inset) {
        ...
        LayoutUnit available_space = containing_block_rect.Bottom() - sticky_box_rect.Bottom();
    ...
    total_containing_block_sticky_offset_ = ancestor_sticky_box_offset + ancestor_containing_block_offset + sticky_offset_;
}
...
PhysicalOffset StickyPositionScrollingConstraints::AncestorStickyBoxOffset() const
{
    if (!nearest_sticky_layer_shifting_sticky_box)
        return PhysicalOffset();
    auto* constraints = nearest_sticky_layer_shifting_sticky_box->StickyConstraints();
    DCHECK(constraints);
    return constraints->total_sticky_box_sticky_offset_;
}

PhysicalOffset StickyPositionScrollingConstraints::AncestorContainingBlockOffset() const
{
    if (!nearest_sticky_layer_shifting_containing_block)
        return PhysicalOffset();
    auto* constraints = nearest_sticky_layer_shifting_containing_block->StickyConstraints();
    DCHECK(constraints);
