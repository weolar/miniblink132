...
public:
    ...
    public:
        FindResult(Frame* f, bool is_new)
            : frame(f)
            , new_window(is_new)
        {
        }
    ...
    FindResult FindOrCreateFrameForNavigation(FrameLoadRequest&, const AtomicString& name) const;
...
private:
    ...
    Frame* FindFrameForNavigationInternal(const AtomicString& name, const KURL&, FrameLoadRequest* request = nullptr) const;
