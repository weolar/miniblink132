...
void SetSafeAreaEnvVariables(LocalFrame* frame, const gfx::Insets& safe_area)
{
    ...
    vars.SetVariable(UADefinedVariable::kSafeAreaInsetTop, StyleEnvironmentVariables::FormatPx(safe_area.top()));
    vars.SetVariable(UADefinedVariable::kSafeAreaInsetLeft, StyleEnvironmentVariables::FormatPx(safe_area.left()));
    vars.SetVariable(UADefinedVariable::kSafeAreaInsetBottom, StyleEnvironmentVariables::FormatPx(safe_area.bottom()));
    vars.SetVariable(UADefinedVariable::kSafeAreaInsetRight, StyleEnvironmentVariables::FormatPx(safe_area.right()));
}
...
{
    DEFINE_STATIC_LOCAL(Persistent<Page::PageSet>, pages, (MakeGarbageCollected<Page::PageSet>()));
    return *pages;
}
...
Page* Page::CreateNonOrdinary(ChromeClient& chrome_client, AgentGroupScheduler& agent_group_scheduler, const ColorProviderColorMaps* color_provider_colors)
{
    return MakeGarbageCollected<Page>(base::PassKey<Page>(), chrome_client, agent_group_scheduler, BrowsingContextGroupInfo::CreateUnique(),
        color_provider_colors,
        /*partitioned_popin_params=*/nullptr,
        /*is_ordinary=*/false);
}

Page* Page::CreateOrdinary(ChromeClient& chrome_client, Page* opener, AgentGroupScheduler& agent_group_scheduler,
...
{
    Page* page = MakeGarbageCollected<Page>(base::PassKey<Page>(), chrome_client, agent_group_scheduler, browsing_context_group_info, color_provider_colors,
...
Page::Page(base::PassKey<Page>, ChromeClient& chrome_client, AgentGroupScheduler& agent_group_scheduler,
    const BrowsingContextGroupInfo& browsing_context_group_info, const ColorProviderColorMaps* color_provider_colors,
    blink::mojom::PartitionedPopinParamsPtr partitioned_popin_params, bool is_ordinary)
    ...
    , context_menu_controller_(MakeGarbageCollected<ContextMenuController>(this))
    , page_scale_constraints_set_(MakeGarbageCollected<PageScaleConstraintsSet>(this))
    , pointer_lock_controller_(MakeGarbageCollected<PointerLockController>(this))
    ...
    , global_root_scroller_controller_(MakeGarbageCollected<TopDocumentRootScrollerController>(*this))
    ...
    validation_message_client_(MakeGarbageCollected<ValidationMessageClientImpl>(*this))
    ...
    , v8_compile_hints_producer_(MakeGarbageCollected<v8_compile_hints::V8CrowdsourcedCompileHintsProducer>(this))
    , v8_compile_hints_consumer_(MakeGarbageCollected<v8_compile_hints::V8CrowdsourcedCompileHintsConsumer>())
    , browsing_context_group_info_(browsing_context_group_info)
{
    if (partitioned_popin_params) {
        partitioned_popin_opener_properties_ = PartitionedPopinOpenerProperties(
            SecurityOrigin::CreateFromUrlOrigin(partitioned_popin_params->opener_top_frame_origin), partitioned_popin_params->opener_site_for_cookies);
    }
    ...
    DCHECK(!main_frame_);
    if (auto* virtual_time_controller = page_scheduler_->GetVirtualTimeController()) {
        history_navigation_virtual_time_pauser_
            = virtual_time_controller->CreateWebScopedVirtualTimePauser("HistoryNavigation", WebScopedVirtualTimePauser::VirtualTaskDuration::kInstant);
    }
    UpdateColorProviders(color_provider_colors && !color_provider_colors->IsEmpty() ? *color_provider_colors : ColorProviderColorMaps::CreateDefault());
}
...
public:
    explicit CloseTaskHandler(WeakMember<Page> page)
        : page_(page)
    {
    }
...
void Page::CloseSoon()
{
    ...
    if (!close_task_handler_ && MainFrame()) {
        ...
        GetPageScheduler()->GetAgentGroupScheduler().DefaultTaskRunner()->PostTask(
            FROM_HERE, WTF::BindOnce(&Page::CloseTaskHandler::DoDeferredClose, WrapWeakPersistent(close_task_handler_.Get())));
    }
}

ViewportDescription Page::GetViewportDescription() const
{
    return MainFrame() && MainFrame()->IsLocalFrame() && DeprecatedLocalMainFrame()->GetDocument()
        ? DeprecatedLocalMainFrame()->GetDocument()->GetViewportData().GetViewportDescription()
        : ViewportDescription();
}
...
void Page::LinkRelatedPagesIfNeeded()
{
    ...
    // to null).
    if (!opener_ || prev_related_page_ != this || next_related_page_ != this || !opener_->next_related_page_) {
        return;
    }
    // Before: ... -> opener -> next -> ...
...
bool Page::IsPartitionedPopin() const
{
    ...
    CHECK(RuntimeEnabledFeatures::PartitionedPopinsEnabled() || !partitioned_popin_opener_properties_);

    return !!partitioned_popin_opener_properties_;
}

const PartitionedPopinOpenerProperties& Page::GetPartitionedPopinOpenerProperties() const
{
...
SpatialNavigationController& Page::GetSpatialNavigationController()
{
    if (!spatial_navigation_controller_) {
        spatial_navigation_controller_ = MakeGarbageCollected<SpatialNavigationController>(*this);
    }
...
SVGResourceDocumentCache& Page::GetSVGResourceDocumentCache()
{
    if (!svg_resource_document_cache_) {
        svg_resource_document_cache_ = MakeGarbageCollected<SVGResourceDocumentCache>(GetPageScheduler()->GetAgentGroupScheduler().DefaultTaskRunner());
    }
...
{
    *(int*)1 = 1;
//     emulated_forced_colors_provider_ = WebTestSupport::IsRunningWebTest() ? ui::CreateEmulatedForcedColorsColorProviderForTest()
...
bool Page::UpdateColorProviders(const ColorProviderColorMaps& color_provider_colors)
{
    ...
    if (!ui::IsRendererColorMappingEquivalent(light_color_provider_.get(), color_provider_colors.light_colors_map)) {
        light_color_provider_ = ui::CreateColorProviderFromRendererColorMap(color_provider_colors.light_colors_map);
        did_color_provider_update = true;
    }
    if (!ui::IsRendererColorMappingEquivalent(dark_color_provider_.get(), color_provider_colors.dark_colors_map)) {
        dark_color_provider_ = ui::CreateColorProviderFromRendererColorMap(color_provider_colors.dark_colors_map);
        did_color_provider_update = true;
    }
    if (!ui::IsRendererColorMappingEquivalent(forced_colors_color_provider_.get(), color_provider_colors.forced_colors_map)) {
        forced_colors_color_provider_ = WebTestSupport::IsRunningWebTest()
            ? ui::CreateEmulatedForcedColorsColorProviderForTest()
            : ui::CreateColorProviderFromRendererColorMap(color_provider_colors.forced_colors_map);
        did_color_provider_update = true;
    }
...
{
    light_color_provider_ = ui::CreateDefaultColorProviderForBlink(/*dark_mode=*/false);
    dark_color_provider_ = ui::CreateDefaultColorProviderForBlink(/*dark_mode=*/true);
    forced_colors_color_provider_ = ui::CreateEmulatedForcedColorsColorProviderForTest();
}

const ui::ColorProvider* Page::GetColorProviderForPainting(mojom::blink::ColorScheme color_scheme, bool in_forced_colors) const
{
    ...
    return color_scheme == mojom::blink::ColorScheme::kDark ? dark_color_provider_.get() : light_color_provider_.get();
}
...
{
    for (const Page* page : AllPages()) {
        if (auto* svg_image_chrome_client = DynamicTo<IsolatedSVGChromeClient>(page->GetChromeClient())) {
            svg_image_chrome_client->RestoreAnimationIfNeeded();
        }
    }
}

void Page::SetValidationMessageClientForTesting(ValidationMessageClient* client)
{
...
{
    PageScaleConstraints new_defaults = GetPageScaleConstraintsSet().DefaultConstraints();
...
void Page::SetUserAgentPageScaleConstraints(const PageScaleConstraints& new_constraints)
{
...
void Page::AllVisitedStateChanged(bool invalidate_visited_link_hashes)
{
    for (const Page* page : OrdinaryPages()) {
        for (Frame* frame = page->main_frame_; frame; frame = frame->Tree().TraverseNext()) {
            if (auto* main_local_frame = DynamicTo<LocalFrame>(frame))
                main_local_frame->GetDocument()->GetVisitedLinkState().InvalidateStyleForAllLinks(invalidate_visited_link_hashes);
        }
    }
}
...
void Page::VisitedStateChanged(LinkHash link_hash)
{
    for (const Page* page : OrdinaryPages()) {
        for (Frame* frame = page->main_frame_; frame; frame = frame->Tree().TraverseNext()) {
            if (auto* main_local_frame = DynamicTo<LocalFrame>(frame))
                main_local_frame->GetDocument()->GetVisitedLinkState().InvalidateStyleForLink(link_hash);
        }
    }
}

void Page::SetVisibilityState(mojom::blink::PageVisibilityState visibility_state, bool is_initial_state)
{
    ...
    const bool was_visible = lifecycle_state_->visibility == mojom::blink::PageVisibilityState::kVisible;
    const bool is_visible = visibility_state == mojom::blink::PageVisibilityState::kVisible;
    ...
    if (main_frame_) {
        if (lifecycle_state_->visibility == mojom::blink::PageVisibilityState::kVisible) {
        ...
        if (!RuntimeEnabledFeatures::DispatchHiddenVisibilityTransitionsEnabled() || was_visible || is_visible) {
...
{
    return lifecycle_state_->visibility == mojom::blink::PageVisibilityState::kVisible;
}

bool Page::DispatchedPagehideAndStillHidden()
{
    return lifecycle_state_->pagehide_dispatch != mojom::blink::PagehideDispatch::kNotDispatched;
}

bool Page::DispatchedPagehidePersistedAndStillHidden()
{
    return lifecycle_state_->pagehide_dispatch == mojom::blink::PagehideDispatch::kDispatchedPersisted;
}
...
void Page::OnSetPageFrozen(bool frozen)
{
    ...
    for (Frame* frame = main_frame_.Get(); frame; frame = frame->Tree().TraverseNext()) {
        if (auto* local_frame = DynamicTo<LocalFrame>(frame)) {
            local_frame->OnPageLifecycleStateUpdated();
        }
    }
...
void CheckFrameCountConsistency(int expected_frame_count, Frame* frame)
{
    ...
    for (; frame; frame = frame->Tree().TraverseNext()) {
        ...
        if (auto* local_frame = DynamicTo<LocalFrame>(frame)) {
            if (auto* fenced_frames = DocumentFencedFrames::Get(*local_frame->GetDocument())) {
                actual_frame_count += static_cast<int>(fenced_frames->GetFencedFrames().size());
            }
        }
    }
...
void Page::UpdateSafeAreaInsetWithBrowserControls(const BrowserControls& browser_controls, bool force_update)
{
    ...
    gfx::Insets new_safe_area
        = gfx::Insets::TLBR(max_safe_area_insets_.top(), max_safe_area_insets_.left(), max_safe_area_insets_.bottom(), max_safe_area_insets_.right());
    if (max_safe_area_insets_.bottom() > 0) {
        ...
        float dip_scale = chrome_client_->GetScreenInfo(*DeprecatedLocalMainFrame()).device_scale_factor;
        ...
        float safe_area_inset_bottom = std::max(0.f, inset_bottom - control_ratio * bottom_controls_full_height / dip_scale);
...
void Page::SetMaxSafeAreaInsets(LocalFrame* setter, gfx::Insets max_safe_area)
{
    ...
    if (GetSettings().GetDynamicSafeAreaInsetsEnabled() && setter->IsMainFrame()) {
...
void Page::SettingsChanged(ChangeType change_type)
{
    ...
    case ChangeType::kViewportDescription:
        if (MainFrame() && MainFrame()->IsLocalFrame()) {
            DeprecatedLocalMainFrame()->GetDocument()->GetViewportData().UpdateViewportDescription();
    ...
    case ChangeType::kImageLoading:
        for (Frame* frame = MainFrame(); frame; frame = frame->Tree().TraverseNext()) {
            if (auto* local_frame = DynamicTo<LocalFrame>(frame)) {
                ...
                local_frame->GetDocument()->Fetcher()->SetAutoLoadImages(GetSettings().GetLoadsImagesAutomatically());
            }
        }
        break;
    ...
    case ChangeType::kFontFamily:
        for (Frame* frame = MainFrame(); frame; frame = frame->Tree().TraverseNext()) {
            if (auto* local_frame = DynamicTo<LocalFrame>(frame))
                local_frame->GetDocument()->GetStyleEngine().UpdateGenericFontFamilySettings();
        }
        break;
    ...
    case ChangeType::kMediaQuery:
        for (Frame* frame = MainFrame(); frame; frame = frame->Tree().TraverseNext()) {
            if (auto* local_frame = DynamicTo<LocalFrame>(frame)) {
                local_frame->GetDocument()->MediaQueryAffectingValueChanged(MediaValueChange::kOther);
...
{
    HeapVector<Member<PluginsChangedObserver>, 32> observers(plugins_changed_observers_);
...
void Page::DidCommitLoad(LocalFrame* frame)
{
    if (main_frame_ == frame) {
        ...
        GetVisualViewport().SetScrollOffset(
            ScrollOffset(), mojom::blink::ScrollType::kProgrammatic, mojom::blink::ScrollBehavior::kInstant, ScrollableArea::ScrollCallback());
    }
...
bool Page::RequestBeginMainFrameNotExpected(bool new_state)
{
    ...
    chrome_client_->RequestBeginMainFrameNotExpected(*DeprecatedLocalMainFrame(), new_state);
    return true;
}
...
void Page::SetMediaFeatureOverride(const AtomicString& media_feature, const String& value)
{
    ...
    if (media_feature == "prefers-color-scheme" || media_feature == "forced-colors")
...
void Page::SetPreferenceOverride(const AtomicString& media_feature, const String& value)
{
...
void Page::Animate(base::TimeTicks monotonic_frame_begin_time)
{
    ...
    // animations here.
    GetValidationMessageClient().ServiceScriptedAnimations(monotonic_frame_begin_time);
}

void Page::UpdateLifecycle(LocalFrame& root, WebLifecycleUpdate requested_update, DocumentUpdateReason reason)
{
...
void Page::UpdateBrowsingContextGroup(const blink::BrowsingContextGroupInfo& browsing_context_group_info)
{
    if (browsing_context_group_info_ == browsing_context_group_info) {
        return;
    }

    if (base::FeatureList::IsEnabled(features::kPausePagesPerBrowsingContextGroup) && ScopedBrowsingContextGroupPauser::IsActive(*this)) {
        CHECK(paused_);
        SetPaused(false);
    }
...
void Page::SetAttributionSupport(network::mojom::AttributionSupport attribution_support)
{
...
const char InternalSettingsPageSupplementBase::kSupplementName[] = "InternalSettings";
...
// sufficient.
static_assert(kMaxNumberOfFrames < (1 << NodeRareData::kConnectedFrameCountBits), "Frame limit should fit in rare data count");
static_assert(kTenFrames < kMaxNumberOfFrames, "Reduced frame limit for testing should actually be lower");
