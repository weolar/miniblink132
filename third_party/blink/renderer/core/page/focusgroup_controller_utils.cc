...
FocusgroupDirection FocusgroupControllerUtils::FocusgroupDirectionForEvent(KeyboardEvent* event)
{
    DCHECK(event);
...
bool FocusgroupControllerUtils::IsDirectionForward(FocusgroupDirection direction)
{
    return direction == FocusgroupDirection::kForwardInline || direction == FocusgroupDirection::kForwardBlock;
}

bool FocusgroupControllerUtils::IsDirectionBackward(FocusgroupDirection direction)
{
    return direction == FocusgroupDirection::kBackwardInline || direction == FocusgroupDirection::kBackwardBlock;
}

bool FocusgroupControllerUtils::IsDirectionInline(FocusgroupDirection direction)
{
    return direction == FocusgroupDirection::kBackwardInline || direction == FocusgroupDirection::kForwardInline;
}

bool FocusgroupControllerUtils::IsDirectionBlock(FocusgroupDirection direction)
{
    return direction == FocusgroupDirection::kBackwardBlock || direction == FocusgroupDirection::kForwardBlock;
}

bool FocusgroupControllerUtils::IsAxisSupported(FocusgroupFlags flags, FocusgroupDirection direction)
{
    return ((flags & FocusgroupFlags::kInline) && IsDirectionInline(direction)) || ((flags & FocusgroupFlags::kBlock) && IsDirectionBlock(direction));
}

bool FocusgroupControllerUtils::WrapsInDirection(FocusgroupFlags flags, FocusgroupDirection direction)
{
    return ((flags & FocusgroupFlags::kWrapInline) && IsDirectionInline(direction)) || ((flags & FocusgroupFlags::kWrapBlock) && IsDirectionBlock(direction));
}

bool FocusgroupControllerUtils::FocusgroupExtendsInAxis(FocusgroupFlags extending_focusgroup, FocusgroupFlags focusgroup, FocusgroupDirection direction)
{
    if (focusgroup == FocusgroupFlags::kNone || extending_focusgroup == FocusgroupFlags::kNone) {
        return false;
    }

    return extending_focusgroup & FocusgroupFlags::kExtend && (IsAxisSupported(focusgroup, direction) == IsAxisSupported(extending_focusgroup, direction));
}

Element* FocusgroupControllerUtils::FindNearestFocusgroupAncestor(const Element* element, FocusgroupType type)
{
    if (!element)
        return nullptr;
    ...
    for (Element* ancestor = FlatTreeTraversal::ParentElement(*element); ancestor; ancestor = FlatTreeTraversal::ParentElement(*ancestor)) {
        ...
        if (ancestor_flags != FocusgroupFlags::kNone) {
            ...
            case FocusgroupType::kGrid:
                ...
                if (ancestor_flags & FocusgroupFlags::kGrid && IsA<LayoutTable>(ancestor->GetLayoutObject())) {
                    return ancestor;
                }
                break;
...
Element* FocusgroupControllerUtils::NextElement(const Element* current, bool skip_subtree)
{
    DCHECK(current);
    Node* node;
    if (skip_subtree)
...
Element* FocusgroupControllerUtils::LastElementWithin(const Element* current)
{
    ...
    for (; last_node && last_node != current; last_node = FlatTreeTraversal::Previous(*last_node)) {
...
Element* FocusgroupControllerUtils::AdjustElementOutOfUnrelatedFocusgroup(Element* element, Element* stop_ancestor, FocusgroupDirection direction)
{
    DCHECK(element);
    ...
    if (focusgroup_flags == FocusgroupFlags::kNone) {
        Element* focusgroup = FindNearestFocusgroupAncestor(stop_ancestor, FocusgroupType::kLinear);
        DCHECK(focusgroup);
    ...
    for (Element* ancestor = FlatTreeTraversal::ParentElement(*element); ancestor; ancestor = FlatTreeTraversal::ParentElement(*ancestor)) {
        ...
        FocusgroupFlags ancestor_flags = ancestor->GetFocusgroupFlags();
        if (ancestor_flags != FocusgroupFlags::kNone && !FocusgroupExtendsInAxis(ancestor_flags, focusgroup_flags, direction)) {
...
GridFocusgroupStructureInfo* FocusgroupControllerUtils::CreateGridFocusgroupStructureInfoForGridRoot(Element* root)
{
    if (IsA<LayoutTable>(root->GetLayoutObject()) && root->GetFocusgroupFlags() & FocusgroupFlags::kGrid) {
        return MakeGarbageCollected<AutomaticGridFocusgroupStructureInfo>(root->GetLayoutObject());
    } else {
