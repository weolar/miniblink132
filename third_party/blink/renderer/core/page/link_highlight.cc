...
void LinkHighlight::SetTapHighlight(Node* node)
{
    ...
    Color highlight_color = node->GetLayoutObject()->StyleRef().VisitedDependentColor(GetCSSPropertyWebkitTapHighlightColor());
...
{
    return GetPage().MainFrame() && GetPage().MainFrame()->IsLocalFrame() ? GetPage().DeprecatedLocalMainFrame() : nullptr;
}
...
void LinkHighlight::AnimationHostInitialized(cc::AnimationHost& animation_host)
{
    animation_host_ = &animation_host;
    if (Platform::Current()->IsThreadedAnimationEnabled()) {
        timeline_ = cc::AnimationTimeline::Create(cc::AnimationIdProvider::NextTimelineId());
...
void LinkHighlight::UpdateAfterPaint(const PaintArtifactCompositor* paint_artifact_compositor)
{
    if (impl_)
