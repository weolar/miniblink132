...
void ClearFocusInExitedFrames(LocalFrame* old_frame, const LocalFrame* const new_frame)
{
    while (old_frame && new_frame != old_frame) {
        ...
        old_frame->GetDocument()->SetSequentialFocusNavigationStartingPoint(nullptr);
...
bool IsEqualDistanceAndContainsBestCandidate(
    const FocusCandidate& candidate, const FocusCandidate& best_candidate, const double& candidate_distance, const double& best_distance)
{
    return std::fabs(candidate_distance - best_distance) < std::numeric_limits<double>::epsilon()
        && candidate.rect_in_root_frame.Contains(best_candidate.rect_in_root_frame);
}
...
// the current best.
static void ConsiderForBestCandidate(SpatialNavigationDirection direction, const FocusCandidate& current_interest, const FocusCandidate& candidate,
    FocusCandidate* best_candidate, double& best_distance, FocusCandidate* previous_best_candidate, double& previous_best_distance)
{
    DCHECK(candidate.visible_node->IsElementNode());
    DCHECK(candidate.visible_node->GetLayoutObject());

    // Ignore iframes that don't have a src attribute
    if (FrameOwnerElement(candidate) && (!FrameOwnerElement(candidate)->ContentFrame() || candidate.rect_in_root_frame.IsEmpty()))
        return;
    ...
    double distance = ComputeDistanceDataForNode(direction, current_interest, candidate);
    ...
    if (candidate_element->IsDescendantOf(best_candidate_element) && IsSkippableCandidate(best_candidate_element)
        && best_candidate->rect_in_root_frame.Contains(candidate.rect_in_root_frame)) {
    ...
    if ((distance < best_distance || IsEqualDistanceAndContainsBestCandidate(candidate, *best_candidate, distance, best_distance)) && IsUnobscured(candidate)) {
...
bool SpatialNavigationController::HandleArrowKeyboardEvent(KeyboardEvent* event)
{
    DCHECK(page_->GetSettings().GetSpatialNavigationEnabled());
...
bool SpatialNavigationController::HandleEnterKeyboardEvent(KeyboardEvent* event)
{
    DCHECK(page_->GetSettings().GetSpatialNavigationEnabled());
...
bool SpatialNavigationController::HandleImeSubmitKeyboardEvent(KeyboardEvent* event)
{
    DCHECK(page_->GetSettings().GetSpatialNavigationEnabled());
...
bool SpatialNavigationController::HandleEscapeKeyboardEvent(KeyboardEvent* event)
{
    DCHECK(page_->GetSettings().GetSpatialNavigationEnabled());
...
bool SpatialNavigationController::Advance(SpatialNavigationDirection direction)
{
    Node* interest_node = StartingNode();
    if (!interest_node)
        return false;

    interest_node->GetDocument().View()->UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason::kSpatialNavigation);

    Node* container = ScrollableAreaOrDocumentOf(interest_node);

    const PhysicalRect visible_rect = PhysicalRect::EnclosingRect(page_->GetVisualViewport().VisibleRect());
    const PhysicalRect start_box = SearchOrigin(visible_rect, interest_node, direction);

    if (IsScrollableAreaOrDocument(interest_node) && !IsOffscreen(interest_node)) {
...
FocusCandidate SpatialNavigationController::FindNextCandidateInContainer(
    Node& container, const PhysicalRect& starting_rect_in_root_frame, SpatialNavigationDirection direction, Node* interest_child_in_container)
{
    ...
    for (; element; element
         = IsScrollableAreaOrDocument(element) ? ElementTraversal::NextSkippingChildren(*element, &container) : ElementTraversal::Next(*element, &container)) {
        ...
        ConsiderForBestCandidate(direction, current_interest, candidate, &best_candidate, best_distance, &previous_best_candidate, previous_best_distance);
    }
    ...
    Node& container, const PhysicalRect& starting_rect_in_root_frame, SpatialNavigationDirection direction, Node* interest_child_in_container)
{
    ...
    FocusCandidate candidate = FindNextCandidateInContainer(container, starting_rect_in_root_frame, direction, interest_child_in_container);
...
Node* SpatialNavigationController::StartingNode()
{
    ...
    const auto* current_frame = DynamicTo<LocalFrame>(page_->GetFocusController().FocusedOrMainFrame());
    if (!current_frame)
        return nullptr;
...
void SpatialNavigationController::MoveInterestTo(Node* next_node)
{
    ...
    element->Focus(FocusParams(
        SelectionBehaviorOnFocus::kReset, mojom::blink::FocusType::kSpatialNavigation, nullptr, FocusOptions::Create(), FocusTrigger::kUserGesture));
...
void SpatialNavigationController::DispatchMouseMoveAt(Element* element)
{
    ...
    int click_count = 0;
    WebMouseEvent fake_mouse_move_event(WebInputEvent::Type::kMouseMove, event_position, event_position_screen, WebPointerProperties::Button::kNoButton,
        click_count, WebInputEvent::kRelativeMotionEvent, base::TimeTicks::Now());
    ...
    DCHECK(frame);
    frame->GetEventHandler().HandleMouseMoveEvent(TransformWebMouseEvent(frame->View(), fake_mouse_move_event), coalesced_events, predicted_events);
}

bool SpatialNavigationController::IsValidCandidate(const Element* element) const
{
