...
public:
    ...
    void onDrawAnnotation(const SkRect& rect, const char key[], SkData* value) override
    {
        ...
        if (rect.width() == 0 && rect.height() == 0) {
            ...
            Operation operation = { kDrawPoint, SkRect::MakeXYWH(point.x(), point.y(), 0, 0) };
    ...
    MOCK_METHOD3(onDrawPicture, void(const SkPicture*, const SkMatrix*, const SkPaint*));
    MOCK_METHOD5(onDrawImage2, void(const SkImage*, SkScalar, SkScalar, const SkSamplingOptions&, const SkPaint*));
    MOCK_METHOD6(onDrawImageRect2, void(const SkImage*, const SkRect&, const SkRect&, const SkSamplingOptions&, const SkPaint*, SrcRectConstraint));
...
protected:
    ...
    {
        RenderingTest::SetUp();
        print_context_ = MakeGarbageCollected<PrintContext>(GetDocument().GetFrame());
        base::FieldTrialParams auto_flush_params;
        auto_flush_params["max_pinned_image_kb"] = "1";
        print_feature_list_.InitAndEnableFeatureWithParameters(kCanvas2DAutoFlushParams, auto_flush_params);
    }
    ...
    {
        return *print_context_.Get();
    }

    void SetBodyInnerHTML(String body_content)
    {
        GetDocument().body()->setAttribute(html_names::kStyleAttr, AtomicString("margin: 0"));
    ...
    gfx::Rect PrintSinglePage(SkCanvas& canvas, int page_index = 0)
    {
        ...
        GetPrintContext().BeginPrintMode(WebPrintParams(gfx::SizeF(kPageWidth, kPageHeight)));
        ...
        GetPrintContext().OutputLinkedDestinations(context, GetDocument().GetLayoutView()->FirstFragment().ContentsProperties(), page_rect);
    ...
    static String AbsoluteBlockHtmlForLink(int x, int y, int width, int height, String url, String children = String())
    {
        StringBuilder ts;
        ts << "<a style='position: absolute; left: " << x << "px; top: " << y << "px; width: " << width << "px; height: " << height << "px' href='" << url
           << "'>" << (children ? children : url) << "</a>";
    ...
    {
        StringBuilder ts;
        ts << "<a name='" << name << "' style='position: absolute; left: " << x << "px; top: " << y << "px'>" << text_content << "</a>";
        return ts.ReleaseString();
    }
...
TEST_P(PrintContextTest, LinkTarget)
{
    ...
    const Vector<MockPageContextCanvas::Operation>& operations = canvas.RecordedOperations();
...
{
    testing::NiceMock<MockPageContextCanvas> canvas;
    SetBodyInnerHTML("<div style='width: 200px; height: 200px; overflow: scroll;"
                     "            position: relative; will-change: scroll-position'>"
        + AbsoluteBlockHtmlForLink(50, 60, 70, 80, "http://www.google.com") + AbsoluteBlockHtmlForLink(250, 60, 70, 80, "http://www.google.com") + "</div>");
    PrintSinglePage(canvas);
...
TEST_P(PrintContextTest, LinkTargetUnderAnonymousBlockBeforeBlock)
{
    ...
    testing::NiceMock<MockPageContextCanvas> canvas;
    SetBodyInnerHTML("<div style='padding-top: 50px'>" + InlineHtmlForLink("http://www.google.com", "<img style='width: 111; height: 10'>") + "<div> "
        + InlineHtmlForLink("http://www.google1.com", "<img style='width: 122; height: 20'>") + "</div>" + "</div>");
    PrintSinglePage(canvas);
    const Vector<MockPageContextCanvas::Operation>& operations = canvas.RecordedOperations();
...
{
    testing::NiceMock<MockPageContextCanvas> canvas;
    SetBodyInnerHTML("<div style='padding-top: 50px; width:555px;'>"
        + InlineHtmlForLink("http://www.google2.com", "<div style='width:133px; height: 30px'>BLOCK</div>") + "</div>");
    PrintSinglePage(canvas);
    const Vector<MockPageContextCanvas::Operation>& operations = canvas.RecordedOperations();
...
TEST_P(PrintContextTest, LinkTargetUnderInInlines)
{
    ...
    SetBodyInnerHTML("<span><b><i><img style='width: 40px; height: 40px'><br>"
        + InlineHtmlForLink("http://www.google3.com", "<img style='width: 144px; height: 40px'>") + "</i></b></span>");
    PrintSinglePage(canvas);
    const Vector<MockPageContextCanvas::Operation>& operations = canvas.RecordedOperations();
    ASSERT_EQ(2u, operations.size());
    // The 'A' element:
...
{
    testing::NiceMock<MockPageContextCanvas> canvas;
    SetBodyInnerHTML("<span><b><i><br>" + InlineHtmlForLink("http://www.google3.com", "xxx<br>xxx") + "</i></b></span>");
    PrintSinglePage(canvas);
    const Vector<MockPageContextCanvas::Operation>& operations = canvas.RecordedOperations();
...
TEST_P(PrintContextTest, LinkTargetRelativelyPositionedInline)
{
    ...
    SetBodyInnerHTML("<a style='position: relative; top: 50px; left: 50px' "
...
{
    testing::NiceMock<MockPageContextCanvas> canvas;
    SetBodyInnerHTML(+"<span style='position: relative; top: 50px; left: 50px'><b><i><img style='width: 1px; height: 40px'><br>"
        + InlineHtmlForLink("http://www.google3.com", "<img style='width: 155px; height: 50px'>") + "</i></b></span>");
    PrintSinglePage(canvas);
    const Vector<MockPageContextCanvas::Operation>& operations = canvas.RecordedOperations();
    ASSERT_EQ(2u, operations.size());
    // The 'A' element:
...
TEST_P(PrintContextTest, LinkTargetUnderRelativelyPositionedInlineMultipleLines)
{
    testing::NiceMock<MockPageContextCanvas> canvas;
    SetBodyInnerHTML(+"<span style='position: relative; top: 50px; left: 50px'><b><i><img style='width: 1px; height: 40px'><br>"
        + InlineHtmlForLink("http://www.google3.com", "<img style='width: 10px; height: 50px'><br><img style='width: 155px; height: 50px'>")
        + "</i></b></span>");
    PrintSinglePage(canvas);
    const Vector<MockPageContextCanvas::Operation>& operations = canvas.RecordedOperations();
    ASSERT_EQ(4u, operations.size());
...
TEST_P(PrintContextTest, LinkTargetUnderRelativelyPositionedInlineMultipleLinesCulledInline)
{
    testing::NiceMock<MockPageContextCanvas> canvas;
    SetBodyInnerHTML(
        +"<span style='position: relative; top: 50px; left: 50px'><b><i><br>" + InlineHtmlForLink("http://www.google3.com", "xxx<br>xxx") + "</i></b></span>");
    PrintSinglePage(canvas);
    const Vector<MockPageContextCanvas::Operation>& operations = canvas.RecordedOperations();
    ASSERT_EQ(3u, operations.size());
    // In this test, only check that we have rectangles. We cannot reliably test
...
TEST_P(PrintContextTest, LinkedTarget)
{
    ...
    Vector<MockPageContextCanvas::Operation> operations = canvas.RecordedOperations();
    ...
    std::sort(operations.begin() + 4, operations.begin() + 8, [](const MockPageContextCanvas::Operation& a, const MockPageContextCanvas::Operation& b) {
        return std::pair(a.rect.x(), a.rect.y()) < std::pair(b.rect.x(), b.rect.y());
    });
...
TEST_P(PrintContextTest, LinkInFragmentedContainer)
{
    ...
    Vector<MockPageContextCanvas::Operation> operations = first_page_canvas.RecordedOperations();
...
TEST_P(PrintContextTest, LinkedTargetSecondPage)
{
    ...
    const Vector<MockPageContextCanvas::Operation>* operations = &first_canvas.RecordedOperations();
    ASSERT_EQ(1u, operations->size());
    EXPECT_EQ(MockPageContextCanvas::kDrawRect, (*operations)[0].type);
...
TEST_P(PrintContextTest, ScaledVerticalRL1)
{
    ...
    int page_count = PrintContext::NumberOfPages(GetDocument().GetFrame(), gfx::SizeF(500, 500));
...
TEST_P(PrintContextTest, SvgMarkersOnMultiplePages)
{
    ...
    EXPECT_CALL(first_page_canvas, onDrawRect(SkRect::MakeWH(100, 75), _)).Times(1);
    PrintSinglePage(first_page_canvas, 0);

    MockCanvas second_page_canvas;
    EXPECT_CALL(second_page_canvas, didTranslate(2, 0)).Times(1);
    EXPECT_CALL(second_page_canvas, onDrawRect(SkRect::MakeWH(50, 25), _)).Times(1);
...
TEST_P(PrintContextFrameTest, WithSubframe)
{
    ...
    SetChildFrameHTML(AbsoluteBlockHtmlForLink(50, 60, 70, 80, "#fragment") + AbsoluteBlockHtmlForLink(150, 160, 170, 180, "http://www.google.com")
...
TEST_P(PrintContextFrameTest, WithScrolledSubframe)
{
    ...
    SetChildFrameHTML(AbsoluteBlockHtmlForLink(10, 10, 20, 20, "http://invisible.com") + AbsoluteBlockHtmlForLink(50, 60, 70, 80, "http://partly.visible.com")
        + AbsoluteBlockHtmlForLink(150, 160, 170, 180, "http://www.google.com") + AbsoluteBlockHtmlForLink(250, 260, 270, 280, "http://www.google.com#fragment")
...
TEST_P(PrintContextTest, Canvas2DBeforePrint)
{
    ...
    script_element->setTextContent("window.addEventListener('beforeprint', (ev) => {"
...
{
    MockPageContextCanvas canvas;
    SetBodyInnerHTML("<canvas id='c' style='image-rendering: pixelated' "
...
TEST_P(PrintContextTest, Canvas2DAutoFlushingSuppressed)
{
    ...
    script_element->setTextContent("source_canvas = document.createElement('canvas');"
    ...
    EXPECT_CALL(canvas, onDrawRect(_, _)).Times(testing::Exactly(1)).InSequence(s);
    // The drawImage call
...
public:
    void SetUp() override
    {
        accelerated_canvas_scope_ = std::make_unique<ScopedAccelerated2dCanvasForTest>(true);
...
public:
    ...
    {
        accelerated_canvas_scope_ = std::make_unique<ScopedAccelerated2dCanvasForTest>(true);
        std::unique_ptr<viz::TestGLES2Interface> gl_context = std::make_unique<viz::TestGLES2Interface>();
        gl_context->set_gpu_rasterization(true);
        std::unique_ptr<viz::TestContextSupport> context_support = std::make_unique<viz::TestContextSupport>();
        std::unique_ptr<viz::TestRasterInterface> raster_interface = std::make_unique<viz::TestRasterInterface>();
        test_context_provider_ = base::MakeRefCounted<viz::TestContextProvider>(std::move(context_support), std::move(gl_context), std::move(raster_interface),
        ...
        PrintContextTest::SetUp();
        accelerated_compositing_scope_ = std::make_unique<ScopedTestingPlatformSupport<AcceleratedCompositingTestPlatform>>();
...
private:
    scoped_refptr<viz::TestContextProvider> test_context_provider_;
    std::unique_ptr<ScopedAccelerated2dCanvasForTest> accelerated_canvas_scope_;
    std::unique_ptr<ScopedTestingPlatformSupport<AcceleratedCompositingTestPlatform>> accelerated_compositing_scope_;
};
...
TEST_P(PrintContextOOPRCanvasTest, Canvas2DFlushForImageListener)
{
    ...
    EXPECT_CALL(canvas, onDrawRect(_, _)).Times(testing::Exactly(0)).InSequence(s);
...
TEST_P(PrintContextFrameTest, DISABLED_SubframePrintPageLayout)
{
    ...
    ChildDocument().GetFrame()->StartPrinting(WebPrintParams(page_size), maximum_shrink_ratio);
    EXPECT_EQ(parent->OffsetWidth(), 800);
...
TEST_P(PrintContextTest, TransparentRootBackgroundWithShouldPrintBackgroundDisabled)
{
    MockPageContextCanvas canvas;
    SetBodyInnerHTML("");
...
TEST_P(PrintContextTest, TransparentRootBackgroundWithShouldPrintBackgroundEnabled)
{
    MockPageContextCanvas canvas;
    SetBodyInnerHTML("");
