...
public:
    void UpdateTooltipUnderCursor(LocalFrame&, const String& text, TextDirection) override
    {
...
{
    ChromeClientToolTipLogger* logger = MakeGarbageCollected<ChromeClientToolTipLogger>();
    ...
    auto* element = MakeGarbageCollected<HTMLElement>(html_names::kDivTag, holder->GetDocument());
...
TEST_F(ChromeClientTest, UpdateTooltipUnderCursorEmptyString)
{
    ...
    HitTestResult result(HitTestRequest(HitTestRequest::kMove), location);
    auto holder = std::make_unique<DummyPageHolder>(gfx::Size(500, 500));
    auto& input_element = *MakeGarbageCollected<HTMLInputElement>(holder->GetDocument());
