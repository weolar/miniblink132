...
public:
    ...
    {
        EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), focus_node, SpatialNavigationDirection::kUp), BottomOfVisualViewport());
        EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), focus_node, SpatialNavigationDirection::kDown), TopOfVisualViewport());
        EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), focus_node, SpatialNavigationDirection::kLeft), RightSideOfVisualViewport());
        EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), focus_node, SpatialNavigationDirection::kRight), LeftSideOfVisualViewport());
    }

    void AssertNormalizedHeight(Element* e, int line_height, bool will_shrink)
    {
        PhysicalRect search_origin = SearchOrigin(RootViewport(e->GetDocument().GetFrame()), e, SpatialNavigationDirection::kDown);
        ...
        PhysicalRect normalized = ShrinkInlineBoxToLineBox(*e->GetLayoutObject(), uncropped);
        ...
        normalized = ShrinkInlineBoxToLineBox(*e->GetLayoutObject(), uncropped, LineBoxes(*e->GetLayoutObject()));
    ...
    {
        PhysicalRect a_origin = SearchOrigin(RootViewport(a->GetDocument().GetFrame()), a, SpatialNavigationDirection::kDown);
    ...
    {
        PhysicalRect a_origin = SearchOrigin(RootViewport(a->GetDocument().GetFrame()), a, SpatialNavigationDirection::kDown);
        PhysicalRect b_origin = SearchOrigin(RootViewport(b->GetDocument().GetFrame()), b, SpatialNavigationDirection::kDown);
...
TEST_F(SpatialNavigationTest, RootFramesVisualViewport)
{
    ...
    const PhysicalRect roots_visible_doc_rect(root_frame_view->GetScrollableArea()->VisibleContentRect());
    ...
    PhysicalRect viewport_rect_of_root_frame = root_frame_view->DocumentToFrame(roots_visible_doc_rect);
...
TEST_F(SpatialNavigationTest, FindContainerWhenEnclosingContainerIsIframe)
{
    ...
    SetChildFrameHTML("<!DOCTYPE html>"
                      "<a>link</a>");
...
TEST_F(SpatialNavigationTest, FindContainerWhenEnclosingContainerIsScrollableOverflowBox)
{
...
TEST_F(SpatialNavigationTest, StartAtVisibleFocusedElement)
{
    ...
    EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), b, SpatialNavigationDirection::kDown), NodeRectInRootFrame(b));
}
...
TEST_F(SpatialNavigationTest, StartAtVisibleFocusedScroller)
{
    ...
    EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), scroller, SpatialNavigationDirection::kDown), NodeRectInRootFrame(scroller));
}
...
TEST_F(SpatialNavigationTest, StartAtVisibleFocusedIframe)
{
    ...
    EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), iframe, SpatialNavigationDirection::kDown), NodeRectInRootFrame(iframe));
}

TEST_F(SpatialNavigationTest, StartAtTopWhenGoingDownwardsWithoutFocus)
{
    EXPECT_EQ(PhysicalRect(0, -1, 111, 0), SearchOrigin({ 0, 0, 111, 222 }, nullptr, SpatialNavigationDirection::kDown));

    EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), nullptr, SpatialNavigationDirection::kDown), TopOfVisualViewport());
}

TEST_F(SpatialNavigationTest, StartAtBottomWhenGoingUpwardsWithoutFocus)
{
    EXPECT_EQ(PhysicalRect(0, 222 + 1, 111, 0), SearchOrigin({ 0, 0, 111, 222 }, nullptr, SpatialNavigationDirection::kUp));

    EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), nullptr, SpatialNavigationDirection::kUp), BottomOfVisualViewport());
}

TEST_F(SpatialNavigationTest, StartAtLeftSideWhenGoingEastWithoutFocus)
{
    EXPECT_EQ(PhysicalRect(-1, 0, 0, 222), SearchOrigin({ 0, 0, 111, 222 }, nullptr, SpatialNavigationDirection::kRight));

    EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), nullptr, SpatialNavigationDirection::kRight), LeftSideOfVisualViewport());
}

TEST_F(SpatialNavigationTest, StartAtRightSideWhenGoingWestWithoutFocus)
{
    EXPECT_EQ(PhysicalRect(111 + 1, 0, 0, 222), SearchOrigin({ 0, 0, 111, 222 }, nullptr, SpatialNavigationDirection::kLeft));

    EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), nullptr, SpatialNavigationDirection::kLeft), RightSideOfVisualViewport());
}

TEST_F(SpatialNavigationTest, StartAtBottomWhenGoingUpwardsAndFocusIsOffscreen)
{
    SetBodyInnerHTML("<button id='b' style='margin-top: 120%;'>B</button>"); // Outside the
        // visual
        // viewport.
    Element* b = GetDocument().getElementById(AtomicString("b"));
    EXPECT_TRUE(IsOffscreen(b));

    EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), b, SpatialNavigationDirection::kUp), BottomOfVisualViewport());
}
...
TEST_F(SpatialNavigationTest, StartAtContainersEdge)
{
    ...
    Element* b = GetDocument().getElementById(AtomicString("b"));
    const Element* container = GetDocument().getElementById(AtomicString("container"));
    ...
    EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), b, SpatialNavigationDirection::kDown), container_top_edge);
    ...
    EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), b, SpatialNavigationDirection::kUp), container_bottom_edge);
    ...
    EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), b, SpatialNavigationDirection::kRight), container_leftmost_edge);
    ...
    EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), b, SpatialNavigationDirection::kLeft), container_rightmost_edge);
}

TEST_F(SpatialNavigationTest, StartFromDocEdgeWhenFocusIsClippedInOffscreenScroller)
{
    SetBodyInnerHTML("<!DOCTYPE html>"
                     "<style>"
                     "  div {"
    ...
    EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), b, SpatialNavigationDirection::kUp), BottomOfVisualViewport());
    EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), b, SpatialNavigationDirection::kDown), TopOfVisualViewport());
}

TEST_F(SpatialNavigationTest, StartFromDocEdgeWhenFocusIsClippedInNestedOffscreenScroller)
{
    SetBodyInnerHTML("<!DOCTYPE html>"
                     "<style>"
                     "  div {"
...
TEST_F(SpatialNavigationTest, PartiallyVisible)
{
    ...
    EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), b, SpatialNavigationDirection::kUp), Intersection(button_in_root_frame, RootViewport(&GetFrame())));

    // Do some scrolling.
    ScrollableArea* root_scroller = GetDocument().View()->GetScrollableArea();
    root_scroller->SetScrollOffset(ScrollOffset(0, 600), mojom::blink::ScrollType::kProgrammatic);
    ...
    EXPECT_FALSE(IsOffscreen(b)); // <button> is not completely offscreen.
    EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), b, SpatialNavigationDirection::kUp), Intersection(button_after_scroll, RootViewport(&GetFrame())));
}

TEST_F(SpatialNavigationTest, StartFromDocEdgeWhenOffscreenIframeDisplaysFocus)
{
    SetBodyInnerHTML("<!DOCTYPE html>"
                     "<style>"
                     "  iframe {"
...
TEST_F(SpatialNavigationTest, PartiallyVisibleIFrame)
{
    ...
    UpdateAllLifecyclePhasesForTest();
    Element* child_element = ChildDocument().getElementById(AtomicString("child"));
    Node* enclosing_container = ScrollableAreaOrDocumentOf(child_element);
    EXPECT_EQ(enclosing_container, ChildDocument());
    ...
    EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), child_element, SpatialNavigationDirection::kDown),
        OppositeEdge(SpatialNavigationDirection::kDown, Intersection(iframe, RootViewport(&GetFrame()))));

    // When searching upwards we start at activeElement's
    // container's (here: the iframe's) bottommost visible edge.
    EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), child_element, SpatialNavigationDirection::kUp),
        OppositeEdge(SpatialNavigationDirection::kUp, Intersection(iframe, RootViewport(&GetFrame()))));

    // When searching eastwards, "to the right", we start at activeElement's
    // container's (here: the iframe's) leftmost visible edge.
    EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), child_element, SpatialNavigationDirection::kRight),
        OppositeEdge(SpatialNavigationDirection::kRight, Intersection(iframe, RootViewport(&GetFrame()))));

    // When searching westwards, "to the left", we start at activeElement's
    // container's (here: the iframe's) rightmost visible edge.
    EXPECT_EQ(SearchOrigin(RootViewport(&GetFrame()), child_element, SpatialNavigationDirection::kLeft),
        OppositeEdge(SpatialNavigationDirection::kLeft, Intersection(iframe, RootViewport(&GetFrame()))));
}

TEST_F(SpatialNavigationTest, BottomOfPinchedViewport)
{
    PhysicalRect origin = SearchOrigin(RootViewport(&GetFrame()), nullptr, SpatialNavigationDirection::kUp);
    ...
    visual_viewport.SetScale(2);
    visual_viewport.SetLocation(gfx::PointF(200, 200));
    origin = SearchOrigin(RootViewport(&GetFrame()), nullptr, SpatialNavigationDirection::kUp);
    EXPECT_EQ(origin.Height(), 0);
...
TEST_F(SpatialNavigationTest, UseTheFirstFragment)
{
    ...
    // Search downards.
    PhysicalRect origin_down = SearchOrigin(RootViewport(&GetFrame()), a, SpatialNavigationDirection::kDown);
    PhysicalRect origin_fragment = SearchOriginFragment(NodeRectInRootFrame(a), *a->GetLayoutObject(), SpatialNavigationDirection::kDown);
    ...
    // Search upwards.
    PhysicalRect origin_up = SearchOrigin(RootViewport(&GetFrame()), a, SpatialNavigationDirection::kUp);
    PhysicalRect origin_fragment_up = SearchOriginFragment(NodeRectInRootFrame(a), *a->GetLayoutObject(), SpatialNavigationDirection::kUp);
    ...
    PhysicalRect origin_left = SearchOrigin(RootViewport(&GetFrame()), a, SpatialNavigationDirection::kLeft);
    EXPECT_EQ(origin_left, origin_down);

    // Search from the bottom fragment.
    PhysicalRect origin_right = SearchOrigin(RootViewport(&GetFrame()), a, SpatialNavigationDirection::kRight);
...
TEST_F(SpatialNavigationTest, InlineImageLink)
{
    ...
    PhysicalRect search_origin = SearchOrigin(RootViewport(&GetFrame()), a, SpatialNavigationDirection::kDown);
...
TEST_F(SpatialNavigationTest, LineBrokenLink)
{
    ...
    PhysicalRect search_origin = SearchOrigin(RootViewport(a->GetDocument().GetFrame()), a, SpatialNavigationDirection::kDown);
...
TEST_F(SpatialNavigationTest, BoxWithLineHeight)
{
    ...
    PhysicalRect search_origin = SearchOrigin(RootViewport(&GetFrame()), block, SpatialNavigationDirection::kDown);
    PhysicalRect uncropped = NodeRectInRootFrame(block);
    PhysicalRect normalized = ShrinkInlineBoxToLineBox(*block->GetLayoutObject(), uncropped);
    EXPECT_EQ(search_origin, uncropped);
...
TEST_F(SpatialNavigationTest, ReplacedInlineElement)
{
    ...
    PhysicalRect search_origin = SearchOrigin(RootViewport(&GetFrame()), pic, SpatialNavigationDirection::kDown);
    PhysicalRect uncropped = NodeRectInRootFrame(pic);
    PhysicalRect normalized = ShrinkInlineBoxToLineBox(*pic->GetLayoutObject(), uncropped);
    EXPECT_EQ(search_origin, uncropped);
    EXPECT_EQ(normalized, uncropped);
...
{
    PhysicalRect origin = SearchOrigin(RootViewport(&GetFrame()), nullptr, SpatialNavigationDirection::kDown);
    EXPECT_EQ(origin.Height(), 0);
    EXPECT_EQ(origin.Width(), GetFrame().View()->Width());
    EXPECT_EQ(origin.X(), 0);
    ...
    visual_viewport.SetScale(2);
    visual_viewport.SetLocation(gfx::PointF(200, 200));
    origin = SearchOrigin(RootViewport(&GetFrame()), nullptr, SpatialNavigationDirection::kDown);
    EXPECT_EQ(origin.Height(), 0);
    EXPECT_LT(origin.Width(), GetFrame().View()->Width());
    EXPECT_GT(origin.X(), 0);
...
TEST_F(SpatialNavigationTest, HasRemoteFrame)
{
    ...
    Element* iframe = webview->MainFrameImpl()->GetFrame()->GetDocument()->getElementById(AtomicString("iframe"));
    EXPECT_FALSE(HasRemoteFrame(iframe));

    frame_test_helpers::SwapRemoteFrame(webview->MainFrameImpl()->FirstChild(), frame_test_helpers::CreateRemote());
...
TEST_F(SpatialNavigationTest, PressEnterKeyActiveElement)
{
    ...
    WebKeyboardEvent arrow_down { WebInputEvent::Type::kRawKeyDown, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests() };
    ...
    WebKeyboardEvent enter { WebInputEvent::Type::kRawKeyDown, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests() };
