...
public:
    ...
    MimeClassInfo(const String& type, const String& description, PluginInfo& plugin, const Vector<String> extensions);
...
public:
    ...
    PluginInfo(const String& name, const String& filename, const String& desc, Color background_color, bool may_use_mime_handler_view);

    void AddMimeType(MimeClassInfo*);

    const HeapVector<Member<MimeClassInfo>>& Mimes() const
    {
        return mimes_;
    }
    ...
    bool MayUseExternalHandler() const
    {
        return may_use_external_handler_;
    }
...
public:
    ...
    const HeapVector<Member<PluginInfo>>& Plugins() const
    {
        return plugins_;
    }
    const HeapVector<Member<MimeClassInfo>>& Mimes() const
