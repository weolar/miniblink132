...
AutomaticGridFocusgroupStructureInfo::AutomaticGridFocusgroupStructureInfo(LayoutObject* root)
    : table_(root)
{
    DCHECK(Table());
...
Element* AutomaticGridFocusgroupStructureInfo::PreviousCellInRow(const Element* cell_element)
{
    DCHECK(cell_element);
    ...
    return CellAtIndexInRow(cell->AbsoluteColumnIndex() - 1, row_element, NoCellFoundAtIndexBehavior::kFindPreviousCellInRow);
}

Element* AutomaticGridFocusgroupStructureInfo::NextCellInRow(const Element* cell_element)
{
    DCHECK(cell_element);
    auto* cell = DynamicTo<LayoutTableCell>(cell_element->GetLayoutObject());
    if (!cell)
    ...
    return CellAtIndexInRow(cell->AbsoluteColumnIndex() + col_span, row_element, NoCellFoundAtIndexBehavior::kFindNextCellInRow);
}
...
Element* AutomaticGridFocusgroupStructureInfo::FirstCellInRow(Element* row)
{
    ...
    return CellAtIndexInRow(0, row, NoCellFoundAtIndexBehavior::kFindNextCellInRow);
}
...
Element* AutomaticGridFocusgroupStructureInfo::LastCellInRow(Element* row)
{
    ...
    return CellAtIndexInRow(ColumnCount() - 1, row, NoCellFoundAtIndexBehavior::kFindPreviousCellInRow);
}

unsigned AutomaticGridFocusgroupStructureInfo::ColumnIndexForCell(const Element* cell_element)
{
    DCHECK(cell_element);
...
Element* AutomaticGridFocusgroupStructureInfo::PreviousCellInColumn(const Element* cell_element)
{
    DCHECK(cell_element);
    auto* cell = DynamicTo<LayoutTableCell>(cell_element->GetLayoutObject());
    if (!cell) {
    ...
    return CellAtIndexInRow(cell->AbsoluteColumnIndex(), previous_row_element, NoCellFoundAtIndexBehavior::kFindPreviousCellInColumn);
}

Element* AutomaticGridFocusgroupStructureInfo::NextCellInColumn(const Element* cell_element)
{
    DCHECK(cell_element);
    ...
    return CellAtIndexInRow(cell->AbsoluteColumnIndex(), next_row_element, NoCellFoundAtIndexBehavior::kFindNextCellInColumn);
}

Element* AutomaticGridFocusgroupStructureInfo::FirstCellInColumn(unsigned index)
{
    if (index >= ColumnCount())
        return nullptr;

    return CellAtIndexInRow(index, FirstRow(), NoCellFoundAtIndexBehavior::kFindNextCellInColumn);
}

Element* AutomaticGridFocusgroupStructureInfo::LastCellInColumn(unsigned index)
{
    if (index >= ColumnCount())
        return nullptr;

    return CellAtIndexInRow(index, LastRow(), NoCellFoundAtIndexBehavior::kFindPreviousCellInColumn);
}

Element* AutomaticGridFocusgroupStructureInfo::PreviousRow(Element* row_element)
{
    DCHECK(row_element);
...
Element* AutomaticGridFocusgroupStructureInfo::RowForCell(Element* cell_element)
{
    auto* cell = DynamicTo<LayoutTableCell>(cell_element->GetLayoutObject());
    if (!cell) {
        return nullptr;
...
Element* AutomaticGridFocusgroupStructureInfo::CellAtIndexInRow(unsigned index, Element* row_element, NoCellFoundAtIndexBehavior behavior)
{
    auto* row = DynamicTo<LayoutTableRow>(row_element->GetLayoutObject());
    if (!row) {
        return nullptr;
...
LayoutTableRow* AutomaticGridFocusgroupStructureInfo::PreviousRow(LayoutTableRow* current_row)
{
...
LayoutTableRow* AutomaticGridFocusgroupStructureInfo::NextRow(LayoutTableRow* current_row)
{
    auto* current_section = current_row->Section();
...
LayoutTableCell* AutomaticGridFocusgroupStructureInfo::TableCellAtIndexInRowRecursive(
    unsigned index, LayoutTableRow* row, std::optional<unsigned> expected_rowspan)
{
    if (!row)
        return nullptr;
    ...
    if (auto* table_row = DynamicTo<HTMLTableRowElement>(row->GetNode())) {
        ...
        auto* table_cell = DynamicTo<HTMLTableCellElement>(table_row->cells()->item(index));
        if (table_cell) {
    ...
    while (actual_index != index) {
        ...
        cell = TableCellAtIndexInRowRecursive(index, PreviousRow(row), rowspan_to_expect);
