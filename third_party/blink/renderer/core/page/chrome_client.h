...
public:
    ...
    virtual float WindowToViewportScalar(LocalFrame*, const float value) const = 0;
    ...
    // transform.
    virtual gfx::Rect LocalRootToScreenDIPs(const gfx::Rect&, const LocalFrameView*) const = 0;

    void ScheduleAnimation(const LocalFrameView* view)
    {
        ScheduleAnimation(view, base::TimeDelta());
    }
    virtual void ScheduleAnimation(const LocalFrameView*, base::TimeDelta delay) = 0;
    ...
    virtual bool StartDeferringCommits(LocalFrame& main_frame, base::TimeDelta timeout, cc::PaintHoldingReason reason) = 0;
    virtual void StopDeferringCommits(LocalFrame& main_frame, cc::PaintHoldingCommitTrigger) = 0;

    virtual std::unique_ptr<cc::ScopedPauseRendering> PauseRendering(LocalFrame& main_frame) = 0;
    ...
    virtual std::optional<int> GetMaxRenderBufferBounds(LocalFrame& frame) const = 0;
    ...
    virtual void StartDragging(
        LocalFrame*, const WebDragData&, DragOperationsMask, const SkBitmap& drag_image, const gfx::Vector2d& cursor_offset, const gfx::Rect& drag_obj_rect)
        = 0;
    ...
    Page* CreateWindow(LocalFrame*, const FrameLoadRequest&, const AtomicString& frame_name, const WebWindowFeatures&, network::mojom::blink::WebSandboxFlags,
        const SessionStorageNamespaceId&, bool& consumed_user_gesture);
    ...
    virtual void Show(LocalFrame& frame, LocalFrame& opener_frame, NavigationPolicy navigation_policy, bool consumed_user_gesture) = 0;
    ...
    virtual void InjectScrollbarGestureScroll(LocalFrame& local_frame, const gfx::Vector2dF& delta, ui::ScrollGranularity granularity,
    ...
    virtual void FinishScrollFocusedEditableIntoView(const gfx::RectF& caret_rect_in_root_frame, mojom::blink::ScrollIntoViewParamsPtr params)
    {
    }
    ...
    // local main frames.
    virtual void SetOverscrollBehavior(LocalFrame& main_frame, const cc::OverscrollBehavior&) = 0;

    virtual bool ShouldReportDetailedMessageForSourceAndSeverity(LocalFrame&, mojom::blink::ConsoleMessageLevel log_level, const String& source) = 0;
    virtual void AddMessageToConsole(LocalFrame*, mojom::ConsoleMessageSource, mojom::ConsoleMessageLevel, const String& message, unsigned line_number,
        const String& source_id, const String& stack_trace)
        = 0;

    virtual bool CanOpenBeforeUnloadConfirmPanel() = 0;
    bool OpenBeforeUnloadConfirmPanel(const String& message, LocalFrame*, bool is_reload);

    virtual void CloseWindow() = 0;

    bool OpenJavaScriptAlert(LocalFrame*, const String&);
    bool OpenJavaScriptConfirm(LocalFrame*, const String&);
    bool OpenJavaScriptPrompt(LocalFrame*, const String& message, const String& default_value, String& result);
    virtual bool TabsToLinks() = 0;

    virtual const display::ScreenInfo& GetScreenInfo(LocalFrame& frame) const = 0;
    virtual const display::ScreenInfos& GetScreenInfos(LocalFrame& frame) const = 0;
    ...
    // device metics.
    virtual float InputEventsScaleForEmulation() const
    {
        return 1;
    }
    ...
    virtual bool DoubleTapToZoomEnabled() const
    {
        return false;
    }
    ...
    void MouseDidMoveOverElement(LocalFrame&, const HitTestLocation&, const HitTestResult&);
    virtual void UpdateTooltipUnderCursor(LocalFrame&, const String&, TextDirection) = 0;
    ...
    virtual void UpdateTooltipFromKeyboard(LocalFrame&, const String&, TextDirection, const gfx::Rect&) = 0;
    ...
    virtual DateTimeChooser* OpenDateTimeChooser(LocalFrame*, DateTimeChooserClient*, const DateTimeChooserParameters&) = 0;
    ...
    virtual void AttachRootLayer(scoped_refptr<cc::Layer>, LocalFrame* local_root) = 0;

    virtual cc::AnimationHost* GetCompositorAnimationHost(LocalFrame&) const = 0;

    virtual cc::AnimationTimeline* GetScrollAnimationTimeline(LocalFrame&) const = 0;

    virtual void EnterFullscreen(LocalFrame&, const FullscreenOptions*, FullscreenRequestType)
    {
    }
    virtual void ExitFullscreen(LocalFrame&)
    {
    }
    virtual void FullscreenElementChanged(Element* old_element, Element* new_element, const FullscreenOptions* options, FullscreenRequestType)
    {
    }

    virtual void AnimateDoubleTapZoom(const gfx::Point& point, const gfx::Rect& rect)
    {
    }
    ...
    virtual void SetEventListenerProperties(LocalFrame*, cc::EventListenerClass, cc::EventListenerProperties) = 0;
    ...
    virtual void SetPanAction(LocalFrame*, mojom::blink::PanAction pan_action) = 0;
    ...
    virtual void SetBrowserControlsState(float top_height, float bottom_height, bool shrinks_layout)
    {
    }
    virtual void SetBrowserControlsShownRatio(float top_ratio, float bottom_ratio)
    {
    }

    virtual String AcceptLanguages() = 0;

    enum class UIElementType { kAlertDialog = 0, kConfirmDialog = 1, kPromptDialog = 2, kPrintDialog = 3, kPopup = 4 };
    virtual bool ShouldOpenUIElementDuringPageDismissal(LocalFrame&, UIElementType, const String&, Document::PageDismissalType) const
    {
        return false;
    }

    virtual bool IsIsolatedSVGChromeClient() const
    {
        return false;
    }

    virtual gfx::Size MinimumWindowSize() const
    {
        return gfx::Size(100, 100);
    }

    virtual bool IsChromeClientImpl() const
    {
        return false;
    }

    virtual void DidChangeFormRelatedElementDynamically(LocalFrame*, HTMLElement*, WebFormRelatedChangeType)
    {
    }
    ...
    virtual void JavaScriptChangedValue(HTMLFormControlElement&, const String& old_value, bool was_autofilled)
    {
    }
    ...
    virtual gfx::Vector2dF ElasticOverscroll() const
    {
    ...
    virtual void RequestDecode(LocalFrame*, const cc::PaintImage& image, base::OnceCallback<void(bool)> callback)
    {
    ...
    // failure.
    using ReportTimeCallback = WTF::CrossThreadOnceFunction<void(const viz::FrameTimingDetails&)>;
    virtual void NotifyPresentationTime(LocalFrame& frame, ReportTimeCallback callback)
    {
    }
    ...
    // scheduler.
    virtual void RequestBeginMainFrameNotExpected(LocalFrame& frame, bool request) = 0;
    ...
    virtual void DidUpdateTextAutosizerPageInfo(const mojom::blink::TextAutosizerPageInfo&)
    {
    }
    ...
    virtual double UserZoomFactor(LocalFrame* frame) const
    {
        return 1;
    }

    virtual void SetDelegatedInkMetadata(LocalFrame* frame, std::unique_ptr<gfx::DelegatedInkMetadata> metadata)
    {
    }
    ...
    virtual float ZoomFactorForViewportLayout()
    {
        return 1;
    }
...
protected:
    ...
    virtual bool OpenBeforeUnloadConfirmPanelDelegate(LocalFrame*, bool is_reload) = 0;
    ...
    virtual bool OpenJavaScriptPromptDelegate(LocalFrame*, const String& message, const String& default_value, String& result) = 0;
    virtual void PrintDelegate(LocalFrame*) = 0;
    virtual Page* CreateWindowDelegate(LocalFrame*, const FrameLoadRequest&, const AtomicString& frame_name, const WebWindowFeatures&,
        network::mojom::blink::WebSandboxFlags, const SessionStorageNamespaceId&, bool& consumed_user_gesture)
        = 0;

private:
    bool CanOpenUIElementIfDuringPageDismissal(Frame& main_frame, UIElementType, const String& message);
    void UpdateTooltipUnderCursor(LocalFrame&, const HitTestLocation&, const HitTestResult&);
    ...
    FRIEND_TEST_ALL_PREFIXES(ChromeClientTest, UpdateTooltipUnderCursorEmptyString);
};
