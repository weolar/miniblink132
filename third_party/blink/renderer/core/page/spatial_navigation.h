...
double ComputeDistanceDataForNode(SpatialNavigationDirection, const FocusCandidate& current_interest, const FocusCandidate& candidate);
CORE_EXPORT PhysicalRect NodeRectInRootFrame(const Node*);
CORE_EXPORT PhysicalRect OppositeEdge(SpatialNavigationDirection side, const PhysicalRect& box, LayoutUnit thickness = LayoutUnit());
CORE_EXPORT PhysicalRect RootViewport(const LocalFrame*);
PhysicalRect StartEdgeForAreaElement(const HTMLAreaElement&, SpatialNavigationDirection);
HTMLFrameOwnerElement* FrameOwnerElement(const FocusCandidate&);

CORE_EXPORT PhysicalRect ShrinkInlineBoxToLineBox(const LayoutObject& layout_object, PhysicalRect visible_part, int line_boxes = -1);

CORE_EXPORT PhysicalRect SearchOriginFragment(const PhysicalRect& visible_part, const LayoutObject& fragmented, const SpatialNavigationDirection direction);
CORE_EXPORT PhysicalRect SearchOrigin(const PhysicalRect&, Node*, const SpatialNavigationDirection);
