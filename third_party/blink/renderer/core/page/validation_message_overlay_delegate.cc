...
public:
    explicit ValidationMessageChromeClient(ChromeClient& main_chrome_client, LocalFrameView* anchor_view)
    ...
    float WindowToViewportScalar(LocalFrame* local_frame, const float scalar_value) const override
    {
        return main_chrome_client_->WindowToViewportScalar(local_frame, scalar_value);
    }
...
ValidationMessageOverlayDelegate::ValidationMessageOverlayDelegate(
    Page& main_page, const Element& anchor, const String& message, TextDirection message_dir, const String& sub_message, TextDirection sub_message_dir)
...
{
    DCHECK(page_) << "Do not call FrameView() before the first call of CreatePage()";
    return *To<LocalFrame>(page_->MainFrame())->View();
}

void ValidationMessageOverlayDelegate::PaintFrameOverlay(const FrameOverlay& overlay, GraphicsContext& context, const gfx::Size& view_size) const
{
    if (IsHiding() && !page_)
        return;

    if (DrawingRecorder::UseCachedDrawingIfPossible(context, overlay, DisplayItem::kFrameOverlay))
        return;
    DrawingRecorder recorder(context, overlay, DisplayItem::kFrameOverlay, gfx::Rect(view_size));
    context.DrawRecord(FrameView().GetPaintRecord());
}

void ValidationMessageOverlayDelegate::ServiceScriptedAnimations(base::TimeTicks monotonic_frame_begin_time)
{
    page_->Animator().ServiceScriptedAnimations(monotonic_frame_begin_time);
}

void ValidationMessageOverlayDelegate::UpdateFrameViewState(const FrameOverlay& overlay)
{
    ...
    AdjustBubblePosition(intersection.IsEmpty() ? gfx::Rect(gfx::Point(), view_size) : intersection);
...
void ValidationMessageOverlayDelegate::CreatePage(const FrameOverlay& overlay)
{
    ...
    gfx::Size view_size = overlay.Size();
    chrome_client_ = MakeGarbageCollected<ValidationMessageChromeClient>(main_page_->GetChromeClient(), anchor_->GetDocument().View());
    Settings& main_settings = main_page_->GetSettings();
    page_ = Page::CreateNonOrdinary(*chrome_client_, main_page_->GetPageScheduler()->GetAgentGroupScheduler(), &main_page_->GetColorProviderColorMaps());
    page_->GetSettings().SetMinimumFontSize(main_settings.GetMinimumFontSize());
    page_->GetSettings().SetMinimumLogicalFontSize(main_settings.GetMinimumLogicalFontSize());

    auto* frame = MakeGarbageCollected<LocalFrame>(MakeGarbageCollected<EmptyLocalFrameClient>(), *page_, nullptr, nullptr, nullptr,
        FrameInsertType::kInsertInConstructor, LocalFrameToken(), nullptr, nullptr, mojo::NullRemote());
    ...
    page_->GetSettings().SetPreferredColorScheme(
        !in_forced_colors_mode && anchor_layout && anchor_layout->StyleRef().UsedColorScheme() == mojom::blink::ColorScheme::kDark
    ...
    if (WebTestSupport::IsRunningWebTest()) {
        ...
        GetElementById(AtomicString("icon")).SetInlineStyleProperty(CSSPropertyID::kTransition, "none");
    ...
    container.SetInlineStyleProperty(CSSPropertyID::kMinWidth, bubble_size_.width() / zoom_factor, CSSPrimitiveValue::UnitType::kPixels);
    container.setAttribute(html_names::kClassAttr, AtomicString("shown-initially"));
    FrameView().UpdateAllLifecyclePhases(DocumentUpdateReason::kOverlay);
}

void ValidationMessageOverlayDelegate::WriteDocument(SegmentedBuffer& data)
{
    PagePopupClient::AddString("<!DOCTYPE html><head><meta charset='UTF-8'><meta name='color-scheme' "
    ...
    PagePopupClient::AddString(Locale::DefaultLocale().IsRTL() ? "<body dir=rtl>" : "<body dir=ltr>", data);
    PagePopupClient::AddString("<div id=container>"
    ...
    PagePopupClient::AddString(message_dir_ == TextDirection::kLtr ? "<div dir=ltr id=main-message></div>" : "<div dir=rtl id=main-message></div>", data);
    PagePopupClient::AddString(sub_message_dir_ == TextDirection::kLtr ? "<div dir=ltr id=sub-message></div>" : "<div dir=rtl id=sub-message></div>", data);
    PagePopupClient::AddString("</main>"
...
Element& ValidationMessageOverlayDelegate::GetElementById(const AtomicString& id) const
{
    Element* element = To<LocalFrame>(page_->MainFrame())->GetDocument()->getElementById(id);
    DCHECK(element) << "No element with id=" << id << ". Failed to load the document?";
    return *element;
}

void ValidationMessageOverlayDelegate::AdjustBubblePosition(const gfx::Rect& view_rect)
{
    if (IsHiding())
        return;
    ...
    if (IsA<LocalFrame>(anchor_page->MainFrame())) {
        ...
        anchor_->GetDocument().GetFrame()->LocalFrameRoot().ContentLayoutObject()->MapToVisualRectInAncestorSpace(nullptr, rect);
        anchor_rect = ToPixelSnappedRect(rect);
        anchor_rect = anchor_page->GetVisualViewport().RootFrameToViewport(anchor_rect);
    ...
    double bubble_x = anchor_rect.x() + anchor_rect.width() / 2 - bubble_size_.width() / 2;
    ...
    Element& container = GetElementById(AtomicString("container"));
    container.SetInlineStyleProperty(CSSPropertyID::kLeft, bubble_x / zoom_factor, CSSPrimitiveValue::UnitType::kPixels);
    container.SetInlineStyleProperty(CSSPropertyID::kTop, bubble_y / zoom_factor, CSSPrimitiveValue::UnitType::kPixels);
    ...
    double max_arrow_anchor_x = bubble_size_.width() - (kArrowSize + kArrowMargin) * zoom_factor;
    ...
    if (!Locale::DefaultLocale().IsRTL()) {
        double anchor_rect_left = anchor_rect.x() + kOffsetToAnchorRect * zoom_factor;
        ...
        double anchor_rect_right = anchor_rect.right() - kOffsetToAnchorRect * zoom_factor;
    ...
    if (show_bottom_arrow) {
        GetElementById(AtomicString("outer-arrow-bottom")).SetInlineStyleProperty(CSSPropertyID::kLeft, arrow_x, CSSPrimitiveValue::UnitType::kPixels);
        GetElementById(AtomicString("inner-arrow-bottom")).SetInlineStyleProperty(CSSPropertyID::kLeft, arrow_x, CSSPrimitiveValue::UnitType::kPixels);
        container.setAttribute(html_names::kClassAttr, AtomicString("shown-fully bottom-arrow"));
        container.SetInlineStyleProperty(CSSPropertyID::kTransformOrigin, String::Format("%.2f%% bottom", arrow_anchor_percent));
    } else {
        GetElementById(AtomicString("outer-arrow-top")).SetInlineStyleProperty(CSSPropertyID::kLeft, arrow_x, CSSPrimitiveValue::UnitType::kPixels);
        GetElementById(AtomicString("inner-arrow-top")).SetInlineStyleProperty(CSSPropertyID::kLeft, arrow_x, CSSPrimitiveValue::UnitType::kPixels);
        container.setAttribute(html_names::kClassAttr, AtomicString("shown-fully"));
        container.SetInlineStyleProperty(CSSPropertyID::kTransformOrigin, String::Format("%.2f%% top", arrow_anchor_percent));
    }
}
...
void ValidationMessageOverlayDelegate::StartToHide()
{
    ...
    if (!page_)
        return;
    GetElementById(AtomicString("container")).classList().replace(AtomicString("shown-fully"), AtomicString("hiding"), ASSERT_NO_EXCEPTION);
}
