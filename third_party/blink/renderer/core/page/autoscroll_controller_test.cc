...
TEST_F(AutoscrollControllerTest, CrashWhenLayoutStopAnimationBeforeScheduleAnimation)
{
    ...
    WebMouseEvent event(WebInputEvent::Type::kMouseDown, gfx::PointF(5, 5), gfx::PointF(5, 5), WebPointerProperties::Button::kLeft, 0,
    ...
    scrollable->SetInlineStyleProperty(CSSPropertyID::kDisplay, CSSValueID::kNone);
...
TEST_F(AutoscrollControllerTest, ContinueAutoscrollAfterMouseLeaveEvent)
{
    ...
    controller.StartMiddleClickAutoscroll(frame, document_node->parentNode()->GetLayoutBox(), gfx::PointF(), gfx::PointF());

    EXPECT_TRUE(controller.IsAutoscrolling());

    WebMouseEvent mouse_leave_event(WebInputEvent::Type::kMouseLeave, WebInputEvent::kNoModifiers, base::TimeTicks::Now());
...
TEST_F(AutoscrollControllerTest, StopAutoscrollOnResize)
{
    ...
    LocalFrame* frame = GetDocument().GetFrame();
    controller.StartMiddleClickAutoscroll(frame, GetDocument().GetLayoutView(), gfx::PointF(), gfx::PointF());
    ...
    WebMouseEvent mouse_move_event(WebInputEvent::Type::kMouseMove, WebInputEvent::kNoModifiers, base::TimeTicks::Now());

    frame->GetEventHandler().HandleMouseMoveEvent(mouse_move_event, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
    ...
    WebMouseEvent mouse_move_event2(WebInputEvent::Type::kMouseMove, WebInputEvent::kNoModifiers, base::TimeTicks::Now());

    frame->GetEventHandler().HandleMouseMoveEvent(mouse_move_event2, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
...
TEST_F(AutoscrollControllerTest, AutoscrollIsNotPropagated)
{
    ...
    LocalFrame* frame = GetDocument().GetFrame();
    LayoutBox* scrollable = GetDocument().getElementById(AtomicString("scrollable"))->GetLayoutBox();

    controller.StartMiddleClickAutoscroll(frame, scrollable, gfx::PointF(15.0, 15.0), gfx::PointF(15.0, 15.0));
...
TEST_F(AutoscrollControllerTest, TextSelectionAutoScroll)
{
    ...
    WebMouseEvent event(WebInputEvent::Type::kMouseDown, gfx::PointF(10, 10), gfx::PointF(10, 10), WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kLeftButtonDown, base::TimeTicks::Now());
    event.SetFrameScale(1);
    GetDocument().GetFrame()->GetEventHandler().HandleMousePressEvent(event);
    ...
    WebMouseEvent mouse_event(WebInputEvent::Type::kMouseMove, gfx::PointF(50, 150), gfx::PointF(50, 150), WebPointerProperties::Button::kLeft, 1,
        WebInputEvent::Modifiers::kLeftButtonDown, base::TimeTicks::Now());
    GetDocument().GetFrame()->GetEventHandler().HandleMouseMoveEvent(mouse_event, Vector<WebMouseEvent>(), Vector<WebMouseEvent>());
    ScrollableArea* scrollable_area = scrollable->GetLayoutBox()->GetScrollableArea();
    ...
    GetDocument().GetFrame()->GetEventHandler().HandleMouseReleaseEvent(mouse_event);
    ...
    ASSERT_TRUE(GetDocument().GetFrame()->Selection().GetSelectionInDOMTree().IsRange());
    Range* range = CreateRange(EphemeralRange(
        GetDocument().GetFrame()->Selection().GetSelectionInDOMTree().Anchor(), GetDocument().GetFrame()->Selection().GetSelectionInDOMTree().Focus()));
    ASSERT_TRUE(range);
