...
using ControllersVector = HeapVector<std::pair<Member<ScriptedAnimationController>, bool>>;
...
public:
    ...
    static void ServiceScriptedAnimations(base::TimeTicks monotonic_time_now, const ControllersVector& documents_vector);
    void PostAnimate();

    bool IsServicingAnimations() const
    {
        return servicing_animations_;
    }
    ...
    void UpdateAllLifecyclePhases(LocalFrame& root_frame, DocumentUpdateReason reason);
    void UpdateAllLifecyclePhasesExceptPaint(LocalFrame& root_frame, DocumentUpdateReason reason);
    void UpdateLifecycleToLayoutClean(LocalFrame& root_frame, DocumentUpdateReason reason);
