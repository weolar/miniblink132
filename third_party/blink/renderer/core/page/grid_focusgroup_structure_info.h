...
public:
    ...
    virtual Element* CellAtIndexInRow(unsigned index, Element* row, NoCellFoundAtIndexBehavior behavior) = 0;
};
...
// `display: table`.
class CORE_EXPORT AutomaticGridFocusgroupStructureInfo final : public GarbageCollected<AutomaticGridFocusgroupStructureInfo>,
...
public:
    ...
    Element* CellAtIndexInRow(unsigned index, Element* row_element, NoCellFoundAtIndexBehavior behavior) override;

private:
    LayoutTableRow* PreviousRow(LayoutTableRow* current_row);
    LayoutTableRow* NextRow(LayoutTableRow* current_row);

    LayoutTableCell* TableCellAtIndexInRowRecursive(unsigned index, LayoutTableRow* row, std::optional<unsigned> expected_rowspan = std::nullopt);
