...
class ValidationMessageOverlayDelegateTest : public PaintTestConfigurations, public RenderingTest {
#if BUILDFLAG(IS_WIN)
public:
    void SetUp() override
    {
        ...
        blink::WebFontRendering::SetMenuFontMetrics(blink::WebString::FromASCII("Arial"), 12);
    }
#endif
...
TEST_P(ValidationMessageOverlayDelegateTest, OverlayAnimationsShouldNotBeComposited)
{
    ...
    auto delegate
        = std::make_unique<ValidationMessageOverlayDelegate>(GetPage(), *anchor, "Test message", TextDirection::kLtr, "Sub-message", TextDirection::kLtr);
    ValidationMessageOverlayDelegate* delegate_ptr = delegate.get();

    auto* overlay = MakeGarbageCollected<FrameOverlay>(&GetFrame(), std::move(delegate));
    delegate_ptr->CreatePage(*overlay);
    ASSERT_TRUE(GetFrame().View()->UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason::kTest));
    ...
    Document* internal_document = To<LocalFrame>(delegate_ptr->GetPageForTesting()->MainFrame())->GetDocument();
    HeapVector<Member<Animation>> animations = internal_document->GetDocumentAnimations().getAnimations(*internal_document);
...
TEST_P(ValidationMessageOverlayDelegateTest, DelegatesInternalPageShouldHaveAnimationTimesUpdated)
{
    ...
    client->ShowValidationMessage(*anchor, "Test message", TextDirection::kLtr, "Sub-message", TextDirection::kLtr);
    ...
    AnimationClock& internal_clock = delegate->GetPageForTesting()->Animator().Clock();
...
TEST_P(ValidationMessageOverlayDelegateTest, Repaint)
{
    ...
    EXPECT_FALSE(GetDocument().View()->VisualViewportOrOverlayNeedsRepaintForTesting());
    ...
    EXPECT_TRUE(GetDocument().View()->VisualViewportOrOverlayNeedsRepaintForTesting());
