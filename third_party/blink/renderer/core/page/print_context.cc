...
{
    return page.x() <= left && left < page.right() && page.y() <= top && top < page.bottom();
}
...
// static
int PrintContext::PageNumberForElement(Element* element, const gfx::SizeF& page_size_in_pixels)
{
    ...
    LayoutBoxModelObject* box = EnclosingBoxModelObject(element->GetLayoutObject());
    if (!box)
        return -1;

    int top = box->OffsetTop(box->OffsetParent()).ToInt();
    int left = box->OffsetLeft(box->OffsetParent()).ToInt();
    for (wtf_size_t page_number = 0; page_number < print_context->PageCount(); ++page_number) {
...
void PrintContext::CollectLinkedDestinations(Node* node)
{
    ...
    if (url.HasFragmentIdentifier() && EqualIgnoringFragmentIdentifier(url, node->GetDocument().BaseURL())) {
...
void PrintContext::OutputLinkedDestinations(GraphicsContext& context, const PropertyTreeStateOrAlias& property_tree_state, const gfx::Rect& page_rect)
{
    DEFINE_STATIC_DISPLAY_ITEM_CLIENT(client, "PrintedLinkedDestinations");
    ScopedPaintChunkProperties scoped_paint_chunk_properties(
        context.GetPaintController(), property_tree_state, *client, DisplayItem::kPrintedContentDestinationLocations);
    DrawingRecorder line_boundary_recorder(context, *client, DisplayItem::kPrintedContentDestinationLocations);
    ...
    for (const auto& entry : linked_destinations_) {
        ...
        if (page_rect.Contains(anchor_point)) {
            ...
            context.SetURLDestinationLocation(entry.key, anchor_point - page_rect.OffsetFromOrigin());
        }
    }
}

// static
int PrintContext::NumberOfPages(LocalFrame* frame, const gfx::SizeF& page_size_in_pixels)
{
...
{
    return frame_->View() && frame_->GetDocument() && frame_->GetDocument()->GetLayoutView();
}
