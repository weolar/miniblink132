...
static void RecordViewportTypeMetric(ViewportDescription::ViewportUMAType type)
{
    UMA_HISTOGRAM_ENUMERATION("Viewport.MetaTagType", type);
}

float ViewportDescription::ResolveViewportLength(const Length& length, const gfx::SizeF& initial_viewport_size, Direction direction)
{
    if (length.IsAuto())
...
PageScaleConstraints ViewportDescription::Resolve(const gfx::SizeF& initial_viewport_size, const Length& legacy_fallback_width) const
{
    ...
    float result_max_width = ResolveViewportLength(copy_max_width, initial_viewport_size, Direction::kHorizontal);
    float result_min_width = ResolveViewportLength(copy_min_width, initial_viewport_size, Direction::kHorizontal);

    float result_height = kValueAuto;
    float result_max_height = ResolveViewportLength(max_height, initial_viewport_size, Direction::kVertical);
    float result_min_height = ResolveViewportLength(min_height, initial_viewport_size, Direction::kVertical);
    ...
    if (result_min_zoom != ViewportDescription::kValueAuto && result_max_zoom != ViewportDescription::kValueAuto)
    ...
    if (result_zoom != ViewportDescription::kValueAuto)
        result_zoom = CompareIgnoringAuto(result_min_zoom, CompareIgnoringAuto(result_max_zoom, result_zoom, std::min), std::max);

    float extend_zoom = CompareIgnoringAuto(result_zoom, result_max_zoom, std::min);
    ...
    if (extend_zoom == ViewportDescription::kValueAuto) {
        ...
        if (result_min_width == ViewportDescription::kValueExtendToZoom)
            result_min_width = CompareIgnoringAuto(extend_width, result_max_width, std::max);

        if (result_min_height == ViewportDescription::kValueExtendToZoom)
            result_min_height = CompareIgnoringAuto(extend_height, result_max_height, std::max);
    }

    // Resolve initial width from min/max descriptors.
    if (result_min_width != ViewportDescription::kValueAuto || result_max_width != ViewportDescription::kValueAuto)
        result_width = CompareIgnoringAuto(result_min_width, CompareIgnoringAuto(result_max_width, initial_viewport_size.width(), std::min), std::max);

    // Resolve initial height from min/max descriptors.
    if (result_min_height != ViewportDescription::kValueAuto || result_max_height != ViewportDescription::kValueAuto)
        result_height = CompareIgnoringAuto(result_min_height, CompareIgnoringAuto(result_max_height, initial_viewport_size.height(), std::min), std::max);

    // Resolve width value.
    if (result_width == ViewportDescription::kValueAuto) {
        if (result_height == ViewportDescription::kValueAuto || !initial_viewport_size.height()) {
            result_width = initial_viewport_size.width();
        } else {
            result_width = result_height * (initial_viewport_size.width() / initial_viewport_size.height());
        }
    }
    ...
    if (result_height == ViewportDescription::kValueAuto) {
        if (!initial_viewport_size.width()) {
            ...
            result_height = result_width * initial_viewport_size.height() / initial_viewport_size.width();
        }
    }
    ...
    if (result_zoom == ViewportDescription::kValueAuto) {
        ...
        if (result_height != ViewportDescription::kValueAuto && result_height > 0) {
            ...
            result_zoom = std::max<float>(result_zoom, initial_viewport_size.height() / result_height);
        }
...
void ViewportDescription::ReportMobilePageStats(const LocalFrame* main_frame) const
{
    if (!main_frame || !main_frame->GetPage() || !main_frame->View() || !main_frame->GetDocument())
        return;

    if (!main_frame->GetSettings() || !main_frame->GetSettings()->GetViewportEnabled())
        return;
    ...
    if (!IsSpecifiedByAuthor()) {
        RecordViewportTypeMetric(main_frame->GetDocument()->IsMobileDocument() ? ViewportUMAType::kXhtmlMobileProfile : ViewportUMAType::kNoViewportTag);
        return;
    }
