...
DocumentsVector GetAllDocuments(Frame* main_frame)
{
    ...
    for (Frame* frame = main_frame; frame; frame = frame->Tree().TraverseNext()) {
        if (auto* local_frame = DynamicTo<LocalFrame>(frame)) {
            ...
            bool can_throttle = document->View() ? document->View()->CanThrottleRendering() : false;
...
void PageAnimator::ServiceScriptedAnimations(base::TimeTicks monotonic_animation_start_time)
{
    ...
    for (const auto& [document, can_throttle] : documents) {
        ...
        if (monotonic_animation_start_time > animation_time) {
            animation_time
                += time_clamper.ClampTimeResolution(monotonic_animation_start_time - animation_time, document->domWindow()->CrossOriginIsolatedCapability());
        }
    ...
    for (const auto& document : documents) {
        controllers.emplace_back(document.first->GetScriptedAnimationController(), document.second);
    }
...
void PageAnimator::ServiceScriptedAnimations(base::TimeTicks monotonic_time_now, const ControllersVector& controllers)
{
    ...
    for (wtf_size_t i = 0; i < controllers.size(); ++i) {
        ...
        if (!controller->GetExecutionContext() || controller->GetExecutionContext()->IsContextFrozenOrPaused()) {
            continue;
        }
        ...
        controller->SetCurrentFrameTimeMs(window->document()->Timeline().CurrentTimeMilliseconds().value());
        controller->SetCurrentFrameLegacyTimeMs(loader->GetTiming().MonotonicTimeToPseudoWallTime(monotonic_time_now).InMillisecondsF());
        if (can_throttle) {
            continue;
        }
    ...
    const auto run_for_all_active_controllers_with_timing = [&](const auto& function) {
    ...
    if (RuntimeEnabledFeatures::PageRevealEventEnabled()) {
        ...
        auto page_reveal_event_filter = WTF::BindRepeating([](const LocalDOMWindow* window, Event* event) {
            ...
            if (RuntimeEnabledFeatures::ViewTransitionOnNavigationEnabled()) {
                if (auto* supplement = ViewTransitionSupplement::FromIfExists(*window->document())) {
                    DOMViewTransition* view_transition = supplement->ResolveCrossDocumentViewTransition();
        ...
        run_for_all_active_controllers_with_timing([&](wtf_size_t i) {
            ...
            bool pagereveal_dispatched = active_controllers[i]->DispatchEvents(WTF::BindRepeating(page_reveal_event_filter, WrapPersistent(window)));

            if (pagereveal_dispatched) {
                window->SetHasBeenRevealed(true);
                if (RuntimeEnabledFeatures::ViewTransitionOnNavigationEnabled()) {
                    if (ViewTransition* transition = ViewTransitionUtils::GetTransition(*window->document());
    ...
    for (wtf_size_t i = 0; i < controllers.size(); ++i) {
        auto& [controller, can_throttle] = controllers[i];
        controller->DispatchEvents(WTF::BindRepeating([](Event* event) { return event->type() == event_type_names::kResize; }));
        auto end_time = base::TimeTicks::Now();
        if (active_controller_id < active_controllers_ids.size() && i == active_controllers_ids[active_controller_id]) {
    ...
    run_for_all_active_controllers_with_timing([&](wtf_size_t i) {
        ...
        active_controllers[i]->DispatchEvents(WTF::BindRepeating([](Event* event) {
            return event->type() == event_type_names::kScroll || event->type() == event_type_names::kScrollsnapchange
                || event->type() == event_type_names::kScrollsnapchanging || event->type() == event_type_names::kScrollend;
        }));
    });
    ...
    // timestamp
    run_for_all_active_controllers_with_timing([&](wtf_size_t i) { active_controllers[i]->CallMediaQueryListListeners(); });
    ...
    run_for_all_active_controllers_with_timing([&](wtf_size_t i) { active_controllers[i]->DispatchEvents(); });

    // 11. For each fully active Document in docs, run the fullscreen
    // steps for that Document, passing in now as the timestamp.
    run_for_all_active_controllers_with_timing([&](wtf_size_t i) { active_controllers[i]->RunTasks(); });

    // Run the fulfilled HTMLVideoELement.requestVideoFrameCallback() callbacks.
    // See https://wicg.github.io/video-rvfc/.
    run_for_all_active_controllers_with_timing([&](wtf_size_t i) { active_controllers[i]->ExecuteVideoFrameCallbacks(); });
...
void PageAnimator::SetSuppressFrameRequestsWorkaroundFor704763Only(bool suppress_frame_requests)
{
    ...
    DCHECK(!suppress_frame_requests_workaround_for704763_only_ || !suppress_frame_requests);
...
{
    if (servicing_animations_ || updating_layout_and_style_for_painting_ || suppress_frame_requests_workaround_for704763_only_) {
        return;
    }
    page_->GetChromeClient().ScheduleAnimation(frame->View());
}

void PageAnimator::UpdateAllLifecyclePhases(LocalFrame& root_frame, DocumentUpdateReason reason)
{
    LocalFrameView* view = root_frame.View();
    base::AutoReset<bool> servicing(&updating_layout_and_style_for_painting_, true);
    view->UpdateAllLifecyclePhases(reason);
}

void PageAnimator::UpdateAllLifecyclePhasesExceptPaint(LocalFrame& root_frame, DocumentUpdateReason reason)
{
    LocalFrameView* view = root_frame.View();
    base::AutoReset<bool> servicing(&updating_layout_and_style_for_painting_, true);
...
void PageAnimator::UpdateLifecycleToLayoutClean(LocalFrame& root_frame, DocumentUpdateReason reason)
{
    LocalFrameView* view = root_frame.View();
    base::AutoReset<bool> servicing(&updating_layout_and_style_for_painting_, true);
...
HeapVector<Member<Animation>> PageAnimator::GetAnimations(const TreeScope& tree_scope)
{
    HeapVector<Member<Animation>> animations;
    DocumentsVector documents = GetAllDocuments(page_->MainFrame());
    for (auto& [document, can_throttle] : documents) {
        document->GetDocumentAnimations().GetAnimationsTargetingTreeScope(animations, tree_scope);
    }
    return animations;
}
