...
// static
bool FocusgroupController::HandleArrowKeyboardEvent(KeyboardEvent* event, const LocalFrame* frame)
{
    DCHECK(frame);
...
// static
bool FocusgroupController::Advance(Element* initial_element, FocusgroupDirection direction)
{
    // Only allow grid focusgroup navigation when the focus is on a grid
    // focusgroup item.
    Element* grid_root = utils::FindNearestFocusgroupAncestor(initial_element, FocusgroupType::kGrid);
...
// static
bool FocusgroupController::AdvanceForward(Element* initial_element, FocusgroupDirection direction)
{
    DCHECK(initial_element);
    DCHECK(utils::IsDirectionForward(direction));
    DCHECK(utils::IsFocusgroupItem(initial_element));

    Element* nearest_focusgroup = utils::FindNearestFocusgroupAncestor(initial_element, FocusgroupType::kLinear);
    ...
    bool can_only_descend = !utils::IsAxisSupported(nearest_focusgroup->GetFocusgroupFlags(), direction);
    ...
    Element* first_element_after_focusgroup = utils::NextElement(nearest_focusgroup, /* skip_subtree */ true);
    ...
    while (true) {
        ...
        if (current_flags != FocusgroupFlags::kNone) {
            ...
            if (!(current_flags & FocusgroupFlags::kExtend) || !utils::IsAxisSupported(current_flags, direction)) {
                skip_subtree = true;
            } else {
                nearest_focusgroup = current;
                first_element_after_focusgroup = utils::NextElement(nearest_focusgroup, /* skip_subtree */ true);
                descended = true;
            }
        }
        ...
        if (current && current == first_element_after_focusgroup) {
            if (CanExitFocusgroupForward(nearest_focusgroup, current, direction)) {
                nearest_focusgroup = utils::FindNearestFocusgroupAncestor(current, FocusgroupType::kLinear);
                first_element_after_focusgroup = utils::NextElement(nearest_focusgroup, /* skip_subtree */ true);
            } else {
                current = nullptr;
            }
...
// wrap.
bool FocusgroupController::CanExitFocusgroupForward(const Element* exiting_focusgroup, const Element* next_element, FocusgroupDirection direction)
{
    DCHECK(exiting_focusgroup);
    DCHECK(next_element);
    DCHECK(utils::NextElement(exiting_focusgroup, /*skip_subtree */ true) == next_element);

    const Element* next_element_focusgroup = utils::FindNearestFocusgroupAncestor(next_element, FocusgroupType::kLinear);
    if (!next_element_focusgroup)
        return false;

    return CanExitFocusgroupForwardRecursive(
        exiting_focusgroup, next_element, direction, utils::WrapsInDirection(exiting_focusgroup->GetFocusgroupFlags(), direction));
}

// static
bool FocusgroupController::CanExitFocusgroupForwardRecursive(
    const Element* exiting_focusgroup, const Element* next_element, FocusgroupDirection direction, bool check_wrap)
{
    DCHECK(exiting_focusgroup);
    DCHECK(next_element);
    ...
    if (utils::NextElement(exiting_focusgroup, /* skip_subtree */ true) != next_element) {
        return true;
    }

    FocusgroupFlags exiting_focusgroup_flags = exiting_focusgroup->GetFocusgroupFlags();
    DCHECK(exiting_focusgroup_flags != FocusgroupFlags::kNone);

    if (!(exiting_focusgroup_flags & FocusgroupFlags::kExtend))
        return false;

    const Element* parent_focusgroup = utils::FindNearestFocusgroupAncestor(exiting_focusgroup, FocusgroupType::kLinear);
    FocusgroupFlags parent_focusgroup_flags = parent_focusgroup ? parent_focusgroup->GetFocusgroupFlags() : FocusgroupFlags::kNone;
    ...
    return CanExitFocusgroupForwardRecursive(parent_focusgroup, next_element, direction, check_wrap);
}

// static
Element* FocusgroupController::WrapForward(Element* nearest_focusgroup, FocusgroupDirection direction)
{
    ...
    for (Element* focusgroup = nearest_focusgroup; focusgroup; focusgroup = utils::FindNearestFocusgroupAncestor(focusgroup, FocusgroupType::kLinear)) {
...
// static
bool FocusgroupController::AdvanceBackward(Element* initial_element, FocusgroupDirection direction)
{
    DCHECK(initial_element);
    DCHECK(utils::IsDirectionBackward(direction));
    DCHECK(utils::IsFocusgroupItem(initial_element));
    ...
    Element* initial_focusgroup = utils::FindNearestFocusgroupAncestor(initial_element, FocusgroupType::kLinear);
    if (!initial_focusgroup)
        return false;
    bool can_only_ascend = !utils::IsAxisSupported(initial_focusgroup->GetFocusgroupFlags(), direction);
    ...
    while (true) {
        ...
        if (current == parent) {
            ...
            if (wrap_result) {
                current = utils::AdjustElementOutOfUnrelatedFocusgroup(wrap_result, parent, direction);
                ...
                FocusgroupFlags current_flags = current->GetFocusgroupFlags();
                if (current_flags != FocusgroupFlags::kNone && !(current_flags & FocusgroupFlags::kExtend)) {
                    return false;
                }

                // We can't ascend if there is no focusgroup ancestor.
                Element* parent_focusgroup = utils::FindNearestFocusgroupAncestor(current, FocusgroupType::kLinear);
                if (!parent_focusgroup)
                    return false;

                // We can't ascend if the parent focusgroup doesn't support the axis of
                // the arrow key pressed.
                if (!utils::IsAxisSupported(parent_focusgroup->GetFocusgroupFlags(), direction)) {
                    return false;
                }
            ...
            // Case (ii).
            current = utils::AdjustElementOutOfUnrelatedFocusgroup(current, parent, direction);
            parent = FlatTreeTraversal::ParentElement(*current);
        }
...
// static
Element* FocusgroupController::WrapBackward(Element* current, FocusgroupDirection direction)
{
    DCHECK(current);
    DCHECK(utils::IsDirectionBackward(direction));
    ...
    Element* parent_focusgroup = utils::FindNearestFocusgroupAncestor(current, FocusgroupType::kLinear);
    if (current_flags & FocusgroupFlags::kExtend && parent_focusgroup && utils::WrapsInDirection(parent_focusgroup->GetFocusgroupFlags(), direction)) {
        return nullptr;
    }
...
// static
bool FocusgroupController::AdvanceInGrid(Element* initial_element, Element* grid_root, FocusgroupDirection direction)
{
    DCHECK(initial_element);
    DCHECK(grid_root);

    grid_root->GetDocument().View()->UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason::kFocusgroup);
...
// static
Element* FocusgroupController::WrapOrFlowInGrid(Element* element, FocusgroupDirection direction, GridFocusgroupStructureInfo* helper)
{
    DCHECK(element);
    DCHECK(helper->Root());
...
// static
void FocusgroupController::Focus(Element* element, FocusgroupDirection direction)
{
    DCHECK(element);
    element->Focus(FocusParams(SelectionBehaviorOnFocus::kReset,
        utils::IsDirectionForward(direction) ? mojom::blink::FocusType::kForward : mojom::blink::FocusType::kBackward, nullptr));
}
