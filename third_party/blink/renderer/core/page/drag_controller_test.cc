...
public:
    ...
    void StartDragging(LocalFrame*, const WebDragData&, DragOperationsMask, const SkBitmap& drag_image, const gfx::Vector2d& cursor_offset,
...
protected:
    ...
    void PerformDragAndDropFromTextareaToTargetElement(HTMLTextAreaElement* drag_text_area, DataObject* data_object, Element* drop_target)
    {
        const gfx::PointF drag_client_point(drag_text_area->OffsetLeft(), drag_text_area->OffsetTop());
        const gfx::PointF drop_client_point(drop_target->OffsetLeft(), drop_target->OffsetTop());
        ...
        GetFrame().GetPage()->GetDragController().StartDrag(&GetFrame(), GetFrame().GetPage()->GetDragController().GetDragState(), mouse_event,
            gfx::Point(drag_text_area->OffsetLeft(), drag_text_area->OffsetTop()));
        DragData data(data_object, GetFrame().GetPage()->GetVisualViewport().ViewportToRootFrame(drop_client_point), drop_client_point,
            static_cast<DragOperationsMask>(kDragOperationMove), false);
        GetFrame().GetPage()->GetDragController().DragEnteredOrUpdated(&data, GetFrame());
...
{
    SetBodyInnerHTML("<div>Hello world! This tests that the bitmap for drag image is scaled "
                     "by page scale factor</div>");
    GetFrame().GetPage()->GetVisualViewport().SetScale(1);
    GetFrame().Selection().SelectAll();
    UpdateAllLifecyclePhasesForTest();
    const std::unique_ptr<DragImage> image1(DragController::DragImageForSelection(GetFrame(), 0.75f));
    GetFrame().GetPage()->GetVisualViewport().SetScale(2);
    GetFrame().Selection().SelectAll();
    UpdateAllLifecyclePhasesForTest();
    const std::unique_ptr<DragImage> image2(DragController::DragImageForSelection(GetFrame(), 0.75f));
...
TEST_F(DragControllerSimTest, DropURLOnNonNavigatingClearsState)
{
    ...
    EXPECT_TRUE(WebView().GetPage()->GetAutoscrollController().AutoscrollInProgress());

    widget->DragTargetDrop(drag_data, client_point, screen_point, 0, base::DoNothing());
    frame_test_helpers::PumpPendingRequestsForFrameToLoad(WebView().MainFrameImpl());

    // Once we've "performed" the drag (in which nothing happens), the
    // AutoscrollController should have been cleared.
    EXPECT_FALSE(WebView().GetPage()->GetAutoscrollController().AutoscrollInProgress());
}
...
TEST_F(DragControllerSimTest, ThrottledDocumentHandled)
{
    ...
    DragData data(
        object, gfx::PointF(10, 10), gfx::PointF(10, 10), static_cast<DragOperationsMask>(kDragOperationCopy | kDragOperationLink | kDragOperationMove), false);

    WebView().GetPage()->GetDragController().DragEnteredOrUpdated(&data, *GetDocument().GetFrame());

    // Throttle updates, which prevents hit testing from yielding a node.
    WebView().MainFrameImpl()->GetFrameView()->SetLifecycleUpdatesThrottledForTesting();

    WebView().GetPage()->GetDragController().PerformDrag(&data, *GetDocument().GetFrame());
...
TEST_F(DragControllerTest, DragImageForSelectionClipsToViewport)
{
    ...
    // viewport.
    gfx::RectF expected_selection(0, node_margin_top, node_width, viewport_height_css - node_margin_top);
    ...
    gfx::Size expected_image_size = gfx::ToRoundedSize(gfx::ScaleSize(expected_selection.size(), page_scale_factor));
    ...
    expected_image_size = gfx::ToRoundedSize(gfx::ScaleSize(expected_selection.size(), page_scale_factor));
    EXPECT_EQ(expected_image_size, selection_image->Size());
    ...
    scroll_offset = 800;
    frame_view->LayoutViewport()->SetScrollOffset(ScrollOffset(0, scroll_offset), mojom::blink::ScrollType::kProgrammatic);
    expected_selection = gfx::RectF(0, 0, node_width, node_height + node_margin_top - scroll_offset);
    EXPECT_EQ(expected_selection, DragController::ClippedSelection(GetFrame()));
    selection_image = DragController::DragImageForSelection(GetFrame(), 1);
    expected_image_size = gfx::ToRoundedSize(gfx::ScaleSize(expected_selection.size(), page_scale_factor));
    EXPECT_EQ(expected_image_size, selection_image->Size());
...
TEST_F(DragControllerTest, DragImageForSelectionClipsChildFrameToViewport)
{
    ...
    child_frame.View()->LayoutViewport()->SetScrollOffset(ScrollOffset(0, iframe_scroll_offset), mojom::blink::ScrollType::kProgrammatic);
...
TEST_F(DragControllerTest, DragImageForSelectionClipsChildFrameToViewportWithPageScaleFactor)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      * { margin: 0; }
...
TEST_F(DragControllerTest, DragImageOffsetWithPageScaleFactor)
{
    ...
    drag_state.drag_data_transfer_ = DataTransfer::Create(DataTransfer::kDragAndDrop, DataTransferAccessPolicy::kWritable, DataObject::Create());
    GetFrame().GetPage()->GetDragController().StartDrag(&GetFrame(), drag_state, mouse_event, gfx::Point(5, 10));

    gfx::Size expected_image_size = gfx::Size(50 * page_scale_factor, 40 * page_scale_factor);
    ...
    gfx::Vector2d expected_offset(5 * page_scale_factor, (10 - 2) * page_scale_factor);
...
TEST_F(DragControllerTest, DragAndDropUrlFromTextareaToTextarea)
{
    ...
    HTMLTextAreaElement* drag_text_area = DynamicTo<HTMLTextAreaElement>(*(GetDocument().getElementById(AtomicString("drag"))));
    HTMLTextAreaElement* drop_text_area = DynamicTo<HTMLTextAreaElement>(*(GetDocument().getElementById(AtomicString("drop"))));
    ...
    drag_state.drag_data_transfer_ = DataTransfer::Create(DataTransfer::kDragAndDrop, DataTransferAccessPolicy::kWritable, data_object);

    PerformDragAndDropFromTextareaToTargetElement(drag_text_area, data_object, drop_text_area);
...
TEST_F(DragControllerTest, DragAndDropUrlFromTextareaToRichlyEditableDiv)
{
    ...
    PerformDragAndDropFromTextareaToTargetElement(drag_text_area, data_object, drop_div_rich);
    EXPECT_EQ("<a href=\"https://www.example.com/index.html\">index.html</a>", drop_div_rich->innerHTML());
...
TEST_F(DragControllerTest, DragAndDropUrlFromTextareaToPlaintextonlyEditableDiv)
{
    SetBodyInnerHTML(R"HTML(
    <style>
    body,html { height: 1000px; width: 1000px; }
    ...
    PerformDragAndDropFromTextareaToTargetElement(drag_text_area, data_object, drop_div_plain);
    EXPECT_EQ("https://www.example.com/index.html", drop_div_plain->innerHTML());
    EXPECT_EQ("", drag_text_area->Value());
}

TEST_F(DragControllerTest, DragAndDropUrlFromTextareaToRichlyEditableParagraph)
{
    SetBodyInnerHTML(R"HTML(
    <style>
    body,html { height: 1000px; width: 1000px; }
    ...
    HTMLTextAreaElement* drag_text_area = DynamicTo<HTMLTextAreaElement>(*(GetDocument().getElementById(AtomicString("drag"))));
    Element* drop_paragraph_rich = GetDocument().getElementById(AtomicString("drop"));
    WebDragData web_drag_data;
    WebDragData::StringItem item1;
    item1.type = "text/uri-list";
    item1.data = WebString::FromUTF8("https://www.example.com/index.html");
    ...
    PerformDragAndDropFromTextareaToTargetElement(drag_text_area, data_object, drop_paragraph_rich);
    EXPECT_EQ("<a href=\"https://www.example.com/index.html\">index.html</a>", drop_paragraph_rich->innerHTML());
    EXPECT_EQ("", drag_text_area->Value());
}

TEST_F(DragControllerTest, DragAndDropUrlFromTextareaToPlaintextonlyEditableParagraph)
{
    SetBodyInnerHTML(R"HTML(
    <style>
    body,html { height: 1000px; width: 1000px; }
    ...
    HTMLTextAreaElement* drag_text_area = DynamicTo<HTMLTextAreaElement>(*(GetDocument().getElementById(AtomicString("drag"))));
    Element* drop_paragraph_plain = GetDocument().getElementById(AtomicString("drop"));
    WebDragData web_drag_data;
    WebDragData::StringItem item1;
    item1.type = "text/uri-list";
    item1.data = WebString::FromUTF8("https://www.example.com/index.html");
    ...
    PerformDragAndDropFromTextareaToTargetElement(drag_text_area, data_object, drop_paragraph_plain);
    EXPECT_EQ("https://www.example.com/index.html", drop_paragraph_plain->innerHTML());
    EXPECT_EQ("", drag_text_area->Value());
}
