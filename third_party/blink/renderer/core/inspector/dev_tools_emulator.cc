...
class DevToolsEmulator::ScopedGlobalOverrides : public WTF::RefCounted<ScopedGlobalOverrides> {
public:
    static scoped_refptr<ScopedGlobalOverrides> AssureInstalled()
    {
        return g_instance_ ? g_instance_ : base::AdoptRef(new ScopedGlobalOverrides());
    }
...
private:
    ...
    ScopedGlobalOverrides()
        : overlay_scrollbars_enabled_(ScrollbarThemeSettings::OverlayScrollbarsEnabled())
        , orientation_event_enabled_(RuntimeEnabledFeatures::OrientationEventEnabled())
        , mobile_layout_theme_enabled_(RuntimeEnabledFeatures::MobileLayoutThemeEnabled())
    {
    ...
    ~ScopedGlobalOverrides()
    {
        ...
        ScrollbarThemeSettings::SetOverlayScrollbarsEnabled(overlay_scrollbars_enabled_);
        Page::UsesOverlayScrollbarsChanged();
        RuntimeEnabledFeatures::SetOrientationEventEnabled(orientation_event_enabled_);
        RuntimeEnabledFeatures::SetMobileLayoutThemeEnabled(mobile_layout_theme_enabled_);
        Page::PlatformColorsChanged();
    }
...
DevToolsEmulator::ScopedGlobalOverrides* DevToolsEmulator::ScopedGlobalOverrides::g_instance_ = nullptr;

DevToolsEmulator::DevToolsEmulator(WebViewImpl* web_view)
    : web_view_(web_view)
    , device_metrics_enabled_(false)
    , embedder_text_autosizing_enabled_(web_view->GetPage()->GetSettings().GetTextAutosizingEnabled())
    , embedder_device_scale_adjustment_(web_view->GetPage()->GetSettings().GetDeviceScaleAdjustment())
    , embedder_lcd_text_preference_(web_view->GetPage()->GetSettings().GetLCDTextPreference())
    , embedder_viewport_style_(web_view->GetPage()->GetSettings().GetViewportStyle())
    , embedder_plugins_enabled_(web_view->GetPage()->GetSettings().GetPluginsEnabled())
    , embedder_available_pointer_types_(web_view->GetPage()->GetSettings().GetAvailablePointerTypes())
    , embedder_primary_pointer_type_(web_view->GetPage()->GetSettings().GetPrimaryPointerType())
    , embedder_available_hover_types_(web_view->GetPage()->GetSettings().GetAvailableHoverTypes())
    , embedder_primary_hover_type_(web_view->GetPage()->GetSettings().GetPrimaryHoverType())
    , embedder_main_frame_resizes_are_orientation_changes_(web_view->GetPage()->GetSettings().GetMainFrameResizesAreOrientationChanges())
    ...
    , embedder_shrink_viewport_content_(web_view->GetPage()->GetSettings().GetShrinksViewportContentToFit())
    , embedder_viewport_enabled_(web_view->GetPage()->GetSettings().GetViewportEnabled())
    , embedder_viewport_meta_enabled_(web_view->GetPage()->GetSettings().GetViewportMetaEnabled())
    , touch_event_emulation_enabled_(false)
    , double_tap_to_zoom_enabled_(false)
    , original_max_touch_points_(0)
    , embedder_script_enabled_(web_view->GetPage()->GetSettings().GetScriptEnabled())
    , script_execution_disabled_(false)
    , embedder_hide_scrollbars_(web_view->GetPage()->GetSettings().GetHideScrollbars())
    , scrollbars_hidden_(false)
    , embedder_cookie_enabled_(web_view->GetPage()->GetSettings().GetCookieEnabled())
    , document_cookie_disabled_(false)
    , embedder_force_dark_mode_enabled_(web_view->GetPage()->GetSettings().GetForceDarkModeEnabled())
...
void DevToolsEmulator::SetDeviceScaleAdjustment(float device_scale_adjustment)
{
    ...
    if (!emulate_mobile_enabled()) {
        web_view_->GetPage()->GetSettings().SetDeviceScaleAdjustment(device_scale_adjustment);
    }
}
...
void DevToolsEmulator::SetMainFrameResizesAreOrientationChanges(bool value)
{
    ...
    if (!emulate_mobile_enabled()) {
        web_view_->GetPage()->GetSettings().SetMainFrameResizesAreOrientationChanges(value);
    }
}

void DevToolsEmulator::SetDefaultPageScaleLimits(float min_scale, float max_scale)
{
...
void DevToolsEmulator::SetShrinksViewportContentToFit(bool shrink_viewport_content)
{
    embedder_shrink_viewport_content_ = shrink_viewport_content;
    if (!emulate_mobile_enabled()) {
        web_view_->GetPage()->GetSettings().SetShrinksViewportContentToFit(shrink_viewport_content);
    }
}
...
void DevToolsEmulator::SetPrimaryPointerType(mojom::blink::PointerType pointer_type)
{
...
void DevToolsEmulator::SetOutputDeviceUpdateAbilityType(mojom::blink::OutputDeviceUpdateAbilityType type)
{
    embedder_output_device_update_ability_type_ = type;
    web_view_->GetPage()->GetSettings().SetOutputDeviceUpdateAbilityType(type);
}

gfx::Transform DevToolsEmulator::EnableDeviceEmulation(const DeviceEmulationParams& params)
{
    if (device_metrics_enabled_ && emulation_params_.view_size == params.view_size && emulation_params_.screen_type == params.screen_type
        && emulation_params_.device_scale_factor == params.device_scale_factor && emulation_params_.scale == params.scale
        && emulation_params_.viewport_offset == params.viewport_offset && emulation_params_.viewport_scale == params.viewport_scale) {
        return ComputeRootLayerTransform();
    }
    if (emulation_params_.device_scale_factor != params.device_scale_factor || !device_metrics_enabled_)
    ...
    web_view_->GetPage()->GetSettings().SetDeviceScaleAdjustment(
        calculateDeviceScaleAdjustment(params.view_size.width(), params.view_size.height(), params.device_scale_factor));
    ...
    if (web_view_->MainFrameImpl()) {
        if (Document* document = web_view_->MainFrameImpl()->GetFrame()->GetDocument())
...
void DevToolsEmulator::DisableDeviceEmulation()
{
    ...
    MemoryCache::Get()->EvictResources();
    device_metrics_enabled_ = false;
    web_view_->GetPage()->GetSettings().SetDeviceScaleAdjustment(embedder_device_scale_adjustment_);
    DisableMobileEmulation();
...
void DevToolsEmulator::EnableMobileEmulation()
{
    ...
    web_view_->GetPage()->GetSettings().SetViewportStyle(mojom::blink::ViewportStyle::kMobile);
    ...
    web_view_->GetPage()->GetSettings().SetLCDTextPreference(LCDTextPreference::kIgnored);
    web_view_->GetPage()->GetSettings().SetPluginsEnabled(false);
    web_view_->GetPage()->GetSettings().SetMainFrameResizesAreOrientationChanges(true);
    ...
    if (web_view_->MainFrameImpl()) {
        web_view_->MainFrameImpl()->GetFrameView()->UpdateLifecycleToLayoutClean(DocumentUpdateReason::kInspector);
    }
}
...
void DevToolsEmulator::DisableMobileEmulation()
{
    ...
    web_view_->GetPage()->GetSettings().SetViewportEnabled(embedder_viewport_enabled_);
    web_view_->GetPage()->GetSettings().SetViewportMetaEnabled(embedder_viewport_meta_enabled_);
    web_view_->GetPage()->GetVisualViewport().InitializeScrollbars();
    web_view_->GetSettings()->SetShrinksViewportContentToFit(embedder_shrink_viewport_content_);
    web_view_->GetPage()->GetSettings().SetTextAutosizingEnabled(embedder_text_autosizing_enabled_);
    web_view_->GetPage()->GetSettings().SetLCDTextPreference(embedder_lcd_text_preference_);
    web_view_->GetPage()->GetSettings().SetViewportStyle(embedder_viewport_style_);
    web_view_->GetPage()->GetSettings().SetPluginsEnabled(embedder_plugins_enabled_);
    web_view_->GetPage()->GetSettings().SetMainFrameResizesAreOrientationChanges(embedder_main_frame_resizes_are_orientation_changes_);
    web_view_->SetZoomFactorOverride(0);
    web_view_->GetPage()->SetDefaultPageScaleLimits(embedder_min_page_scale_, embedder_max_page_scale_);
...
gfx::Transform DevToolsEmulator::ForceViewport(const gfx::PointF& position, float scale)
{
...
void DevToolsEmulator::ApplyViewportOverride(gfx::Transform* transform)
{
    ...
    gfx::PointF scroll_offset = web_view_->MainFrame()->IsWebLocalFrame() ? web_view_->MainFrame()->ToWebLocalFrame()->GetScrollOffset() : gfx::PointF();
    ...
    transform->Translate(-viewport_override_->position.x() + scroll_x, -viewport_override_->position.y() + scroll_y);
...
void DevToolsEmulator::SetTouchEventEmulationEnabled(bool enabled, int max_touch_points)
{
    if (!touch_event_emulation_enabled_) {
        original_max_touch_points_ = web_view_->GetPage()->GetSettings().GetMaxTouchPoints();
    }
    touch_event_emulation_enabled_ = enabled;
    web_view_->GetPage()->GetSettings().SetForceTouchEventFeatureDetectionForInspector(enabled);
    web_view_->GetPage()->GetSettings().SetMaxTouchPoints(enabled ? max_touch_points : original_max_touch_points_);
    web_view_->GetPage()->GetSettings().SetAvailablePointerTypes(
        enabled ? static_cast<int>(mojom::blink::PointerType::kPointerCoarseType) : embedder_available_pointer_types_);
    web_view_->GetPage()->GetSettings().SetPrimaryPointerType(enabled ? mojom::blink::PointerType::kPointerCoarseType : embedder_primary_pointer_type_);
    web_view_->GetPage()->GetSettings().SetAvailableHoverTypes(
        enabled ? static_cast<int>(mojom::blink::HoverType::kHoverNone) : embedder_available_hover_types_);
    web_view_->GetPage()->GetSettings().SetPrimaryHoverType(enabled ? mojom::blink::HoverType::kHoverNone : embedder_primary_hover_type_);
    WebLocalFrameImpl* frame = web_view_->MainFrameImpl();
    if (enabled && frame)
        frame->GetFrame()->GetEventHandler().ClearMouseEventManager();
}

void DevToolsEmulator::SetScriptExecutionDisabled(bool script_execution_disabled)
{
    script_execution_disabled_ = script_execution_disabled;
    web_view_->GetPage()->GetSettings().SetScriptEnabled(script_execution_disabled_ ? false : embedder_script_enabled_);
}
...
void DevToolsEmulator::SetScrollbarsHidden(bool hidden)
{
    ...
    web_view_->GetPage()->GetSettings().SetHideScrollbars(scrollbars_hidden_ ? true : embedder_hide_scrollbars_);
}
...
void DevToolsEmulator::SetDocumentCookieDisabled(bool disabled)
{
    ...
    web_view_->GetPage()->GetSettings().SetCookieEnabled(document_cookie_disabled_ ? false : embedder_cookie_enabled_);
}
...
void DevToolsEmulator::SetAutoDarkModeOverride(bool enabled)
{
    if (!auto_dark_overriden_) {
        ...
        embedder_force_dark_mode_enabled_ = web_view_->GetPage()->GetSettings().GetForceDarkModeEnabled();
    }
...
{
    if (auto_dark_overriden_) {
        web_view_->GetPage()->GetSettings().SetForceDarkModeEnabled(embedder_force_dark_mode_enabled_);
        auto_dark_overriden_ = false;
    }
}
