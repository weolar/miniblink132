...
public:
    ...
    // - void Clear();
    template <class ValueType> class SimpleField : public Field {
        // Means in practice: const WTF::String& for WTF::String, otherwise same
        // as ValueType.
        using ConstRefType = typename std::conditional<std::is_fundamental<ValueType>::value, ValueType, const ValueType&>::type;
    ...
    private:
        ...
        {
            const mojom::blink::DevToolsSessionState* reattach_state = session_state_->ReattachState();
            if (!reattach_state)
                return;
            auto it = reattach_state->entries.find(prefix_key_);
            if (it != reattach_state->entries.end()) {
                Deserialize(crdtp::span<uint8_t>(it->value->data(), it->value->size()), &value_);
            }
        }
    ...
    template <class ValueType> class MapField : public Field {
        // Means in practice: const WTF::String& for WTF::String, otherwise same
        // as ValueType.
        using ConstRefType = typename std::conditional<std::is_fundamental<ValueType>::value, ValueType, const ValueType&>::type;
    ...
    private:
        ...
        void Decode() override
        {
            ...
            for (const auto& entry : reattach_state->entries) {
                ...
                ValueType v;
                if (Deserialize(crdtp::span<uint8_t>(entry.value->data(), entry.value->size()), &v)) {
