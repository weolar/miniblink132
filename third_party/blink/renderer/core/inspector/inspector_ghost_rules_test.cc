...
class InspectorGhostRuleTest : public testing::Test, public testing::WithParamInterface<const char*> {
protected:
    void SetUp() override;
    ...
    template <typename T> void SerializeChildren(T* container, StringBuilder& sb)
    {
    ...
    void Serialize(CSSRule* rule, StringBuilder& sb)
    {
        ...
        } else if (auto* nested_declarations_rule = DynamicTo<CSSNestedDeclarationsRule>(rule)) {
    ...
    {
        if (auto* nested_declarations_rule = DynamicTo<CSSNestedDeclarationsRule>(rule)) {
            To<CSSStyleRule>(nested_declarations_rule->InnerCSSStyleRule())->style()->removeProperty("--ghost", ASSERT_NO_EXCEPTION);
        } else if (auto* grouping_rule = DynamicTo<CSSGroupingRule>(rule)) {
...
TEST_P(InspectorGhostRuleTest, RefTest)
{
    ...
    CSSStyleSheet* expected_sheet = css_test_helpers::CreateStyleSheet(GetDocument());
    ...
    CSSStyleSheet* actual_sheet = css_test_helpers::CreateStyleSheet(GetDocument());
...
INSTANTIATE_TEST_SUITE_P(All, InspectorGhostRuleTest, testing::ValuesIn(ghost_rules_data));
