...
constexpr PerformanceEntryType kSupportedTypes = PerformanceEntry::EntryType::kLargestContentfulPaint | PerformanceEntry::EntryType::kLayoutShift;

double ToProtocolTime(DOMHighResTimeStamp timeOrigin, DOMHighResTimeStamp time)
{
    return time ? ConvertDOMHighResTimeStampToSeconds(timeOrigin + time) : 0.0;
}

std::unique_ptr<protocol::PerformanceTimeline::LargestContentfulPaint> BuildEventDetails(const LargestContentfulPaint& lcp, DOMHighResTimeStamp timeOrigin)
{
...
std::unique_ptr<protocol::Array<protocol::PerformanceTimeline::LayoutShiftAttribution>> BuildLayoutShiftAttributions(
    const LayoutShift::AttributionList& attributions)
{
    auto result = std::make_unique<protocol::Array<protocol::PerformanceTimeline::LayoutShiftAttribution>>();
...
std::unique_ptr<protocol::PerformanceTimeline::LayoutShift> BuildEventDetails(const LayoutShift& ls, DOMHighResTimeStamp timeOrigin)
{
    auto result = protocol::PerformanceTimeline::LayoutShift::create()
                      .setValue(ls.value())
...
std::unique_ptr<protocol::PerformanceTimeline::TimelineEvent> BuildProtocolEvent(String frame_id, DOMHighResTimeStamp timeOrigin, PerformanceEntry* entry)
{
...
InspectorPerformanceTimelineAgent::InspectorPerformanceTimelineAgent(InspectedFrames* inspected_frames)
    : inspected_frames_(inspected_frames)
    , enabled_types_(&agent_state_, /*default_value=*/false)
{
}

InspectorPerformanceTimelineAgent::~InspectorPerformanceTimelineAgent() = default;
...
void InspectorPerformanceTimelineAgent::PerformanceEntryAdded(ExecutionContext* context, PerformanceEntry* entry)
{
    ...
    GetFrontend()->timelineEventAdded(BuildProtocolEvent(frame_id, performance->timeOrigin(), entry));
}

protocol::Response InspectorPerformanceTimelineAgent::enable(std::unique_ptr<protocol::Array<String>> entry_types)
{
    ...
    for (const auto& type_str : *entry_types) {
        ...
        PerformanceEntryType type_enum = PerformanceEntry::ToEntryTypeEnum(type_atomic);
        if (type_enum == PerformanceEntry::EntryType::kInvalid || (type_enum & kSupportedTypes) != type_enum) {
            return protocol::Response::InvalidParams("Unknown or unsupported entry type");
        }
...
void InspectorPerformanceTimelineAgent::CollectEntries(AtomicString type, EventsVector* events)
{
    for (LocalFrame* frame : *inspected_frames_) {
        ...
        for (Member<PerformanceEntry> entry : performance->getBufferedEntriesByType(type)) {
            events->push_back(BuildProtocolEvent(frame_id, performance->timeOrigin(), entry));
        }
    }
}
