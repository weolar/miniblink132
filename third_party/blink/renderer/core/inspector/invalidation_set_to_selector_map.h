...
class CORE_EXPORT InvalidationSetToSelectorMap final : public GarbageCollected<InvalidationSetToSelectorMap> {
public:
    // A small helper to bundle together a StyleRule plus an index into its
    // selector list.
    class CORE_EXPORT IndexedSelector final : public GarbageCollected<IndexedSelector> {
    public:
    ...
    private:
        friend struct WTF::HashTraits<blink::InvalidationSetToSelectorMap::IndexedSelector>;
    ...
    static void RecordInvalidationSetEntry(const InvalidationSet* invalidation_set, SelectorFeatureType type, const AtomicString& value);
    ...
    // being destroyed.
    static void RemoveEntriesForInvalidationSet(const InvalidationSet* invalidation_set);
    ...
    static const IndexedSelectorList* Lookup(const InvalidationSet* invalidation_set, SelectorFeatureType type, const AtomicString& value);
...
private:
    ...
    using InvalidationSetEntryMap = HeapHashMap<InvalidationSetEntry, Member<IndexedSelectorList>>;
    using InvalidationSetMap = HeapHashMap<const InvalidationSet*, Member<InvalidationSetEntryMap>>;
...
struct HashTraits<blink::InvalidationSetToSelectorMap::IndexedSelector>
    : TwoFieldsHashTraits<blink::InvalidationSetToSelectorMap::IndexedSelector, &blink::InvalidationSetToSelectorMap::IndexedSelector::style_rule_,
          &blink::InvalidationSetToSelectorMap::IndexedSelector::selector_index_> { };

template <>
struct HashTraits<blink::Member<blink::InvalidationSetToSelectorMap::IndexedSelector>>
    ...
    static bool Equal(const blink::Member<IndexedSelector>& a, const blink::Member<IndexedSelector>& b)
    {
