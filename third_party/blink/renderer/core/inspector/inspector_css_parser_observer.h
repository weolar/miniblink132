...
public:
    ...
    InspectorCSSParserObserver(
        const String& parsed_text, Document* document, CSSRuleSourceDataList* result, std::optional<IssueReportingContext> issue_reporting_context = {})
...
private:
    ...
    template <typename CharacterType> inline void SetRuleHeaderEnd(const base::span<const CharacterType>, unsigned);
    const LineEndings* GetLineEndings();
    void ReportPropertyRuleFailure(unsigned start_offset, CSSPropertyID invalid_property);
