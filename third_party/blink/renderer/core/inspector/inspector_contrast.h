...
public:
    ...
    std::vector<ContrastInfo> GetElementsWithContrastIssues(bool report_aaa, size_t max_elements);
...
private:
    ...
    HeapVector<Member<Node>> ElementsFromRect(const PhysicalRect& rect, Document& document);
    bool GetColorsFromRect(PhysicalRect rect, Document& document, Element* top_element, Vector<Color>& colors, float* text_opacity);
