...
InspectorLogAgent::InspectorLogAgent(ConsoleMessageStorage* storage, PerformanceMonitor* performance_monitor, v8_inspector::V8InspectorSession* v8_session)
    : storage_(storage)
...
void InspectorLogAgent::Restore()
{
    ...
    for (const WTF::String& key : violation_thresholds_.Keys()) {
        settings->emplace_back(ViolationSetting::create().setName(key).setThreshold(violation_thresholds_.Get(key)).build());
    }
...
void InspectorLogAgent::ConsoleMessageAdded(ConsoleMessage* message)
{
    ...
    std::unique_ptr<v8_inspector::protocol::Runtime::API::StackTrace> stack_trace = message->Location()->BuildInspectorObject();
    if (stack_trace)
        entry->setStackTrace(std::move(stack_trace));
    if (message->Location()->LineNumber())
        entry->setLineNumber(message->Location()->LineNumber() - 1);
    if (message->GetSource() == ConsoleMessage::Source::kWorker && !message->WorkerId().empty()) {
        entry->setWorkerId(message->WorkerId());
    }
    if (message->GetSource() == ConsoleMessage::Source::kNetwork && !message->RequestIdentifier().IsNull()) {
        entry->setNetworkRequestId(message->RequestIdentifier());
    }

    if (v8_session_ && message->Frame() && !message->Nodes().empty()) {
        ScriptForbiddenScope::AllowUserAgentScript allow_script;
        auto remote_objects = std::make_unique<protocol::Array<v8_inspector::protocol::Runtime::API::RemoteObject>>();
        for (DOMNodeId node_id : message->Nodes()) {
            std::unique_ptr<v8_inspector::protocol::Runtime::API::RemoteObject> remote_object;
            Node* node = DOMNodeIds::NodeForId(node_id);
            if (node) {
                remote_object = ResolveNode(v8_session_, node, "console", protocol::Maybe<int>());
            }
            if (!remote_object) {
                remote_object = NullRemoteObject(v8_session_, message->Frame(), "console");
            }
            if (remote_object) {
...
void InspectorLogAgent::InnerEnable()
{
    ...
    if (storage_->ExpiredCount()) {
        std::unique_ptr<protocol::Log::LogEntry> expired = protocol::Log::LogEntry::create()
                                                               ...
                                                               .setText(String::Number(storage_->ExpiredCount()) + String(" log entries are not shown."))
                                                               .setTimestamp(0)
                                                               .build();
...
protocol::Response InspectorLogAgent::startViolationsReport(std::unique_ptr<protocol::Array<ViolationSetting>> settings)
{
    if (!enabled_.Get())
        return protocol::Response::ServerError("Log is not enabled");
    if (!performance_monitor_) {
        return protocol::Response::ServerError("Violations are not supported for this target");
    }
    ...
    for (const std::unique_ptr<ViolationSetting>& setting : *settings) {
        ...
        if (violation == PerformanceMonitor::kAfterLast)
            continue;
        performance_monitor_->Subscribe(violation, base::Milliseconds(threshold), this);
...
{
    String message_text = String::Format("Forced reflow while executing JavaScript took %" PRId64 "ms", duration.InMilliseconds());
    auto* message
        = MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kViolation, mojom::blink::ConsoleMessageLevel::kVerbose, message_text);
    ConsoleMessageAdded(message);
}

void InspectorLogAgent::ReportGenericViolation(PerformanceMonitor::Violation, const String& text, base::TimeDelta time, SourceLocation* location)
{
    auto* message = MakeGarbageCollected<ConsoleMessage>(
        mojom::blink::ConsoleMessageSource::kViolation, mojom::blink::ConsoleMessageLevel::kVerbose, text, location->Clone());
    ConsoleMessageAdded(message);
}
