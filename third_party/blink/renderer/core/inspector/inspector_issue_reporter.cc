...
void InspectorIssueReporter::DidFailLoading(
    CoreProbeSink* sink, uint64_t identifier, DocumentLoader* loader, const ResourceError& error, const base::UnguessableToken& token)
{
    if (!storage_)
        return;
    std::optional<network::mojom::BlockedByResponseReason> blocked_by_response_reason = error.GetBlockedByResponseReason();
    if (!blocked_by_response_reason)
        return;

    auto issue = AuditsIssue::CreateBlockedByResponseIssue(*blocked_by_response_reason, identifier, loader, error, token);
...
void InspectorIssueReporter::DomContentLoadedEventFired(LocalFrame* frame)
{
    ...
    AuditsIssue::ReportQuirksModeIssue(document->GetExecutionContext(), document->InLimitedQuirksMode(), document->GetDomNodeId(), url.GetString(),
        IdentifiersFactory::FrameId(frame), IdentifiersFactory::LoaderId(document->Loader()));
}
