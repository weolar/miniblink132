...
TEST_F(InspectorStyleResolverTest, DirectlyMatchedRules)
{
    ...
    EXPECT_EQ("#grid { display: grid; gap: 10px; grid-template-columns: 100px 1fr 20%; "
              "}",
        rule.first->cssText());
}
...
TEST_F(InspectorStyleResolverTest, ParentRules)
{
    ...
    Element* grid = GetDocument().getElementById(AtomicString("grid"));
    InspectorStyleResolver resolver(grid, kPseudoIdNone, g_null_atom);
    HeapVector<Member<InspectorCSSMatchedRules>> parent_rules = resolver.ParentRules();
    Element* grid_container = GetDocument().getElementById(AtomicString("grid-container"));
    // Some rules are coming for UA.
...
TEST_F(InspectorStyleResolverTest, HighlightPseudoInheritance)
{
    ...
    HeapVector<Member<InspectorCSSMatchedPseudoElements>> parent_pseudos = resolver.ParentPseudoElementRules();
    ...
    EXPECT_EQ(kPseudoIdHighlight, parent_pseudos.at(0)->pseudo_element_rules.at(0)->pseudo_id);
    EXPECT_EQ(2u, parent_pseudos.at(0)->pseudo_element_rules.at(0)->matched_rules->size());
    EXPECT_EQ("#middle::highlight(foo) { color: red; }", parent_pseudos.at(0)->pseudo_element_rules.at(0)->matched_rules->at(0).first->cssText());
    EXPECT_EQ("#middle::highlight(bar) { color: orange; }", parent_pseudos.at(0)->pseudo_element_rules.at(0)->matched_rules->at(1).first->cssText());
    ...
    EXPECT_EQ(kPseudoIdSelection, parent_pseudos.at(2)->pseudo_element_rules.at(0)->pseudo_id);
    EXPECT_EQ(1u, parent_pseudos.at(2)->pseudo_element_rules.at(0)->matched_rules->size());
    EXPECT_EQ("#outer::selection { color: limegreen; }", parent_pseudos.at(2)->pseudo_element_rules.at(0)->matched_rules->at(0).first->cssText());
