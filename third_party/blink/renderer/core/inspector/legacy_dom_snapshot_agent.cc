...
std::unique_ptr<protocol::DOM::Rect> LegacyBuildRectForPhysicalRect(const PhysicalRect& rect)
{
    return protocol::DOM::Rect::create().setX(rect.X()).setY(rect.Y()).setWidth(rect.Width()).setHeight(rect.Height()).build();
}

} // namespace

struct LegacyDOMSnapshotAgent::VectorStringHashTraits : public WTF::GenericHashTraits<Vector<String>> {
    ...
    {
        new (WTF::NotNullTag::kNotNull, &vec) Vector<String>(WTF::kHashTableDeletedValue);
    }
    ...
    static bool IsEmptyValue(const Vector<String>& vec)
    {
        return vec.empty();
    }
...
LegacyDOMSnapshotAgent::LegacyDOMSnapshotAgent(InspectorDOMDebuggerAgent* dom_debugger_agent, OriginUrlMap* origin_url_map)
...
protocol::Response LegacyDOMSnapshotAgent::GetSnapshot(Document* document, std::unique_ptr<protocol::Array<String>> style_filter,
    protocol::Maybe<bool> include_event_listeners, protocol::Maybe<bool> include_paint_order, protocol::Maybe<bool> include_user_agent_shadow_tree,
...
{
    document->View()->UpdateLifecycleToCompositingInputsClean(DocumentUpdateReason::kInspector);
    // Setup snapshot.
    dom_nodes_ = std::make_unique<protocol::Array<protocol::DOMSnapshot::DOMNode>>();
    layout_tree_nodes_ = std::make_unique<protocol::Array<protocol::DOMSnapshot::LayoutTreeNode>>();
    computed_styles_ = std::make_unique<protocol::Array<protocol::DOMSnapshot::ComputedStyle>>();
    ...
    for (const String& entry : *style_filter) {
        CSSPropertyID property_id = CssPropertyID(document->GetExecutionContext(), entry);
    ...
    // Actual traversal.
    VisitNode(document, include_event_listeners.value_or(false), include_user_agent_shadow_tree.value_or(false));
...
int LegacyDOMSnapshotAgent::VisitNode(Node* node, bool include_event_listeners, bool include_user_agent_shadow_tree)
{
    ...
    std::unique_ptr<protocol::DOMSnapshot::DOMNode> owned_value = protocol::DOMSnapshot::DOMNode::create()
                                                                      ...
                                                                      .build();
    if (origin_url_map_ && origin_url_map_->Contains(owned_value->getBackendNodeId())) {
    ...
    int layoutNodeIndex = VisitLayoutTreeNode(node->GetLayoutObject(), node, index);
    ...
    if (include_event_listeners && node->GetDocument().GetFrame()) {
        ScriptState* script_state = ToScriptStateForMainWorld(node->GetDocument().GetFrame());
        if (script_state->ContextIsValid()) {
            ScriptState::Scope scope(script_state);
            v8::Local<v8::Context> context = script_state->GetContext();
            V8EventListenerInfoList event_information;
            InspectorDOMDebuggerAgent::CollectEventListeners(script_state->GetIsolate(), node, v8::Local<v8::Value>(), node, true, &event_information);
            if (!event_information.empty()) {
                value->setEventListeners(dom_debugger_agent_->BuildObjectsForEventListeners(event_information, context, v8_inspector::StringView()));
            }
        }
    }
    ...
    if (element) {
        ...
        if (auto* frame_owner = DynamicTo<HTMLFrameOwnerElement>(node)) {
            if (LocalFrame* frame = DynamicTo<LocalFrame>(frame_owner->ContentFrame()))
                value->setFrameId(IdentifiersFactory::FrameId(frame));

            if (Document* doc = frame_owner->contentDocument()) {
                value->setContentDocumentIndex(VisitNode(doc, include_event_listeners, include_user_agent_shadow_tree));
            }
        }
        ...
        if (element->IsPseudoElement()) {
            value->setPseudoType(InspectorDOMAgent::ProtocolPseudoElementType(element->GetPseudoIdForStyling()));
        }
        value->setPseudoElementIndexes(VisitPseudoElements(element, index, include_event_listeners, include_user_agent_shadow_tree));
    ...
    if (node->IsInShadowTree()) {
        value->setShadowRootType(InspectorDOMAgent::GetShadowRootType(node->ContainingShadowRoot()));
    }

    if (node->IsContainerNode()) {
        value->setChildNodeIndexes(VisitContainerChildren(node, include_event_listeners, include_user_agent_shadow_tree));
    }
    return index;
}

std::unique_ptr<protocol::Array<int>> LegacyDOMSnapshotAgent::VisitContainerChildren(
    Node* container, bool include_event_listeners, bool include_user_agent_shadow_tree)
{
    auto children = std::make_unique<protocol::Array<int>>();

    if (!blink::dom_traversal_utils::HasChildren(*container, include_user_agent_shadow_tree))
        return nullptr;

    Node* child = blink::dom_traversal_utils::FirstChild(*container, include_user_agent_shadow_tree);
    while (child) {
        children->emplace_back(VisitNode(child, include_event_listeners, include_user_agent_shadow_tree));
        child = blink::dom_traversal_utils::NextSibling(*child, include_user_agent_shadow_tree);
    }

    return children;
}

std::unique_ptr<protocol::Array<int>> LegacyDOMSnapshotAgent::VisitPseudoElements(
    Element* parent, int index, bool include_event_listeners, bool include_user_agent_shadow_tree)
{
    if (!parent->GetPseudoElement(kPseudoIdFirstLetter) && !parent->GetPseudoElement(kPseudoIdCheck) && !parent->GetPseudoElement(kPseudoIdBefore)
        && !parent->GetPseudoElement(kPseudoIdAfter) && !parent->GetPseudoElement(kPseudoIdSelectArrow)) {
        return nullptr;
    }

    auto pseudo_elements = std::make_unique<protocol::Array<int>>();
    for (PseudoId pseudo_id : { kPseudoIdFirstLetter, kPseudoIdCheck, kPseudoIdBefore, kPseudoIdAfter, kPseudoIdSelectArrow }) {
        if (Node* pseudo_node = parent->GetPseudoElement(pseudo_id)) {
            pseudo_elements->emplace_back(VisitNode(pseudo_node, include_event_listeners, include_user_agent_shadow_tree));
        }
    }
    return pseudo_elements;
}

std::unique_ptr<protocol::Array<protocol::DOMSnapshot::NameValue>> LegacyDOMSnapshotAgent::BuildArrayForElementAttributes(Element* element)
{
    AttributeCollection attributes = element->Attributes();
    if (attributes.IsEmpty())
        return nullptr;
    auto attributes_value = std::make_unique<protocol::Array<protocol::DOMSnapshot::NameValue>>();
    for (const auto& attribute : attributes) {
        attributes_value->emplace_back(protocol::DOMSnapshot::NameValue::create().setName(attribute.GetName().ToString()).setValue(attribute.Value()).build());
    }
    return attributes_value;
}

int LegacyDOMSnapshotAgent::VisitLayoutTreeNode(LayoutObject* layout_object, Node* node, int node_index)
{
    if (!layout_object)
        return -1;
    ...
    auto layout_tree_node = protocol::DOMSnapshot::LayoutTreeNode::create()
                                ...
                                .setBoundingBox(LegacyBuildRectForPhysicalRect(InspectorDOMSnapshotAgent::RectInDocument(layout_object)))
                                .build();
    ...
    if (layout_object->IsText()) {
        ...
        if (!text_boxes.empty()) {
            auto inline_text_nodes = std::make_unique<protocol::Array<protocol::DOMSnapshot::InlineTextBox>>();
            for (const auto& text_box : text_boxes) {
                inline_text_nodes->emplace_back(
                    protocol::DOMSnapshot::InlineTextBox::create()
                        ...
                        .setBoundingBox(LegacyBuildRectForPhysicalRect(InspectorDOMSnapshotAgent::TextFragmentRectInDocument(layout_object, text_box)))
                        .build());
            }
...
int LegacyDOMSnapshotAgent::GetStyleIndexForNode(Node* node)
{
    ...
    for (const auto& pair : *css_property_filter_) {
        ...
        if (const CSSValue* css_value
            = CSSProperty::Get(pair.second).CSSValueFromComputedStyle(*computed_style, node->GetLayoutObject(), true, CSSValuePhase::kResolvedValue)) {
    ...
    auto style_properties = std::make_unique<protocol::Array<protocol::DOMSnapshot::NameValue>>();

    for (wtf_size_t i = 0; i < style.size(); i++) {
        if (style[i].empty())
            continue;
        style_properties->emplace_back(protocol::DOMSnapshot::NameValue::create().setName((*css_property_filter_)[i].first).setValue(style[i]).build());
    }

    wtf_size_t index = static_cast<wtf_size_t>(computed_styles_->size());
    computed_styles_->emplace_back(protocol::DOMSnapshot::ComputedStyle::create().setProperties(std::move(style_properties)).build());
