...
InspectorHighlightContrastInfo FetchContrast(Node* node)
{
    ...
    InspectorCSSAgent::GetBackgroundColors(element, &bgcolors, &font_size, &font_weight, &text_opacity);
    if (bgcolors.size() == 1) {
...
Node* HoveredNodeForPoint(LocalFrame* frame, const gfx::Point& point_in_root_frame, bool ignore_pointer_events_none)
{
    ...
    HitTestLocation location(frame->View()->ConvertFromRootFrame(point_in_root_frame));
...
Node* HoveredNodeForEvent(LocalFrame* frame, const WebGestureEvent& event, bool ignore_pointer_events_none)
{
    return HoveredNodeForPoint(frame, gfx::ToRoundedPoint(event.PositionInRootFrame()), ignore_pointer_events_none);
}

Node* HoveredNodeForEvent(LocalFrame* frame, const WebMouseEvent& event, bool ignore_pointer_events_none)
{
    return HoveredNodeForPoint(frame, gfx::ToRoundedPoint(event.PositionInRootFrame()), ignore_pointer_events_none);
}

Node* HoveredNodeForEvent(LocalFrame* frame, const WebPointerEvent& event, bool ignore_pointer_events_none)
{
    WebPointerEvent transformed_point = event.WebPointerEventInRootFrame();
    return HoveredNodeForPoint(frame, gfx::ToRoundedPoint(transformed_point.PositionInWidget()), ignore_pointer_events_none);
}
...
{
    return IsSelfLocked(node) ? NodeContentVisibilityState::kIsLocked : NodeContentVisibilityState::kNone;
}

std::pair<Node*, NodeContentVisibilityState> DetermineContentVisibilityState(Node* node)
{
    DCHECK(node);
    std::pair<Node*, NodeContentVisibilityState> result;
    if (auto* locked_ancestor = DisplayLockUtilities::HighestLockedExclusiveAncestor(*node)) {
...
SearchingForNodeTool::SearchingForNodeTool(
    InspectorOverlayAgent* overlay, OverlayFrontend* frontend, InspectorDOMAgent* dom_agent, bool ua_shadow, const std::vector<uint8_t>& config)
    : InspectTool(overlay, frontend)
    , dom_agent_(dom_agent)
    , ua_shadow_(ua_shadow)
{
    auto parsed_config = protocol::Overlay::HighlightConfig::FromBinary(config.data(), config.size());
    if (parsed_config) {
        highlight_config_ = InspectorOverlayAgent::ToHighlightConfig(parsed_config.get());
    }
}
...
void SearchingForNodeTool::Draw(float scale)
{
    ...
    bool append_element_info = (node->IsElementNode() || node->IsTextNode()) && !omit_tooltip_ && highlight_config_->show_info && node->GetLayoutObject()
        && node->GetDocument().GetFrame();
    DCHECK(overlay_->HasAXContext(node));
    InspectorHighlight highlight(node, *highlight_config_, contrast_info_, append_element_info, false, content_visibility_state_);
    if (event_target_node_) {
        highlight.AppendEventTargetQuads(event_target_node_.Get(), *highlight_config_);
    }
...
bool SearchingForNodeTool::HandleInputEvent(LocalFrameView* frame_view, const WebInputEvent& input_event, bool* swallow_next_mouse_up)
{
    if (input_event.GetType() == WebInputEvent::Type::kGestureScrollBegin || input_event.GetType() == WebInputEvent::Type::kGestureScrollUpdate
    ...
    return InspectTool::HandleInputEvent(frame_view, input_event, swallow_next_mouse_up);
}
...
bool SearchingForNodeTool::HandleMouseMove(const WebMouseEvent& event)
{
    ...
    if (!frame || !frame->View() || !frame->ContentLayoutObject())
        return false;
    Node* node = HoveredNodeForEvent(frame, event, event.GetModifiers() & WebInputEvent::kShiftKey);
    ...
    bool hold_selected_node = (event.GetModifiers() & (WebInputEvent::kAltKey | WebInputEvent::kAltGrKey))
        && (event.GetModifiers() & (WebInputEvent::kControlKey | WebInputEvent::kMetaKey));
    if (!node || hold_selected_node)
        return true;

    std::tie(node, content_visibility_state_) = DetermineContentVisibilityState(node);
    ...
    event_target_node_ = (event.GetModifiers() & WebInputEvent::kShiftKey) ? HoveredNodeForEvent(frame, event, false) : nullptr;
    if (event_target_node_ == hovered_node_)
        event_target_node_ = nullptr;
    omit_tooltip_ = event.GetModifiers() & (WebInputEvent::kControlKey | WebInputEvent::kMetaKey);
...
bool SearchingForNodeTool::HandleMouseDown(const WebMouseEvent& event, bool* swallow_next_mouse_up)
{
    if (hovered_node_) {
...
QuadHighlightTool::QuadHighlightTool(
    InspectorOverlayAgent* overlay, OverlayFrontend* frontend, std::unique_ptr<gfx::QuadF> quad, Color color, Color outline_color)
    : InspectTool(overlay, frontend)
    , quad_(std::move(quad))
    , color_(color)
...
NodeHighlightTool::NodeHighlightTool(InspectorOverlayAgent* overlay, OverlayFrontend* frontend, Member<Node> node, String selector_list,
    std::unique_ptr<InspectorHighlightConfig> highlight_config)
    : InspectTool(overlay, frontend)
    , selector_list_(selector_list)
    , highlight_config_(std::move(highlight_config))
{
    std::tie(node_, content_visibility_state_) = DetermineContentVisibilityState(node);
...
void NodeHighlightTool::DrawNode()
{
    bool append_element_info
        = (node_->IsElementNode() || node_->IsTextNode()) && highlight_config_->show_info && node_->GetLayoutObject() && node_->GetDocument().GetFrame();
    overlay_->EvaluateInOverlay("drawHighlight", GetNodeInspectorHighlightAsJson(append_element_info, false /* append_distance_info */));
}
...
void NodeHighlightTool::DrawMatchingSelector()
{
    ...
    StaticElementList* elements = query_base->QuerySelectorAll(AtomicString(selector_list_), exception_state);
    ...
    for (unsigned i = 0; i < elements->length(); ++i) {
        ...
        if (DisplayLockUtilities::LockedAncestorPreventingPaint(*element))
            continue;
        NodeContentVisibilityState content_visibility_state = DetermineSelfContentVisibilityState(element);
        InspectorHighlight highlight(
            element, *highlight_config_, contrast_info_, false /* append_element_info */, false /* append_distance_info */, content_visibility_state);
        overlay_->EvaluateInOverlay("drawHighlight", highlight.AsProtocolValue());
    }
}
...
std::unique_ptr<protocol::DictionaryValue> NodeHighlightTool::GetNodeInspectorHighlightAsJson(bool append_element_info, bool append_distance_info) const
{
    DCHECK(overlay_->HasAXContext(node_.Get()));
    InspectorHighlight highlight(node_.Get(), *highlight_config_, contrast_info_, append_element_info, append_distance_info, content_visibility_state_);
...
{
    return !grid_node_highlights_.size() && !flex_container_configs_.size() && !scroll_snap_configs_.size() && !container_query_configs_.size()
...
void PersistentTool::Draw(float scale)
{
    for (auto& entry : grid_node_highlights_) {
        std::unique_ptr<protocol::Value> highlight = InspectorGridHighlight(entry.key, *(entry.value));
        if (!highlight)
            continue;
        overlay_->EvaluateInOverlay("drawGridHighlight", std::move(highlight));
    }
    for (auto& entry : flex_container_configs_) {
        std::unique_ptr<protocol::Value> highlight = InspectorFlexContainerHighlight(entry.key, *(entry.value));
        if (!highlight)
            continue;
        overlay_->EvaluateInOverlay("drawFlexContainerHighlight", std::move(highlight));
    }
    for (auto& entry : scroll_snap_configs_) {
        std::unique_ptr<protocol::Value> highlight = InspectorScrollSnapHighlight(entry.key, *(entry.value));
        if (!highlight)
            continue;
        overlay_->EvaluateInOverlay("drawScrollSnapHighlight", std::move(highlight));
    }
    for (auto& entry : container_query_configs_) {
        std::unique_ptr<protocol::Value> highlight = InspectorContainerQueryHighlight(entry.key, *(entry.value));
        if (!highlight)
            continue;
        overlay_->EvaluateInOverlay("drawContainerQueryHighlight", std::move(highlight));
    }
    for (auto& entry : isolated_element_configs_) {
        std::unique_ptr<protocol::Value> highlight = InspectorIsolatedElementHighlight(entry.key, *(entry.value));
        if (!highlight)
            continue;
        overlay_->EvaluateInOverlay("drawIsolatedElementHighlight", std::move(highlight));
    }
}
...
// message.
void PersistentTool::Dispatch(const ScriptValue& message, ExceptionState& exception_state)
{
    Dictionary dict(message);

    String highlight_type = dict.Get<IDLString>("highlightType", exception_state).value_or("");
    int32_t index = dict.Get<IDLLong>("highlightIndex", exception_state).value_or(-1);
    String new_width = dict.Get<IDLString>("newWidth", exception_state).value_or("");
    String new_height = dict.Get<IDLString>("newHeight", exception_state).value_or("");
    String resizer_type = dict.Get<IDLString>("resizerType", exception_state).value_or("");
...
std::unique_ptr<protocol::DictionaryValue> PersistentTool::GetGridInspectorHighlightsAsJson() const
{
...
void SourceOrderTool::Draw(float scale)
{
    ...
    for (Node& child_node : NodeTraversal::ChildrenOf(*node_)) {
        ...
        if (const ComputedStyle* style = element->GetComputedStyle()) {
            if (style->Display() == EDisplay::kNone || style->Visibility() == EVisibility::kHidden) {
                continue;
            }
        }
...
{
    InspectorSourceOrderHighlight highlight(node, source_order_config_->child_outline_color, source_order_position);
    overlay_->EvaluateInOverlay("drawSourceOrder", highlight.AsProtocolValue());
}

void SourceOrderTool::DrawParentNode()
{
    InspectorSourceOrderHighlight highlight(node_.Get(), source_order_config_->parent_outline_color, 0);
    overlay_->EvaluateInOverlay("drawSourceOrder", highlight.AsProtocolValue());
}
...
std::unique_ptr<protocol::DictionaryValue> SourceOrderTool::GetNodeInspectorSourceOrderHighlightAsJson() const
{
    InspectorSourceOrderHighlight highlight(node_.Get(), source_order_config_->parent_outline_color, 0);
    return highlight.AsProtocolValue();
}
...
bool NearbyDistanceTool::HandleMouseDown(const WebMouseEvent& event, bool* swallow_next_mouse_up)
{
    return true;
}
...
void NearbyDistanceTool::Draw(float scale)
{
    ...
    InspectorHighlight highlight(node, InspectorHighlight::DefaultConfig(), InspectorHighlightContrastInfo(), false /* append_element_info */,
...
ScreenshotTool::ScreenshotTool(InspectorOverlayAgent* overlay, OverlayFrontend* frontend)
    : InspectTool(overlay, frontend)
{
...
void ScreenshotTool::Dispatch(const ScriptValue& message, ExceptionState& exception_state)
{
    Dictionary dict(message);
    ...
    if (!x || !y || !width || !height) {
        exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, kInvalidOverlayCommand);
        return;
    }
    ...
    if (LocalFrame* frame = overlay_->GetFrame()) {
        float emulation_scale = overlay_->GetFrame()->GetPage()->GetChromeClient().InputEventsScaleForEmulation();
        ...
        const VisualViewport& visual_viewport = frame->GetPage()->GetVisualViewport();
        p1 = visual_viewport.ViewportToRootFrame(p1);
        p2 = visual_viewport.ViewportToRootFrame(p2);

        scale = frame->GetPage()->PageScaleFactor();
        if (const RootFrameViewport* root_frame_viewport = frame->View()->GetRootFrameViewport()) {
            gfx::Vector2d scroll_offset = gfx::ToFlooredVector2d(root_frame_viewport->LayoutViewport().GetScrollOffset());
    ...
    gfx::Rect rect = UnionRectsEvenIfEmpty(gfx::Rect(p1, gfx::Size()), gfx::Rect(p2, gfx::Size()));
    frontend_->screenshotRequested(
        protocol::Page::Viewport::create().setX(rect.x()).setY(rect.y()).setWidth(rect.width()).setHeight(rect.height()).setScale(scale).build());
}
...
void PausedInDebuggerTool::Dispatch(const ScriptValue& message, ExceptionState& exception_state)
{
    String message_string;
    if (message.ToString(message_string)) {
        auto task_runner = overlay_->GetFrame()->GetTaskRunner(TaskType::kInternalInspector);
        if (message_string == "resume") {
            task_runner->PostTask(FROM_HERE,
                WTF::BindOnce(&v8_inspector::V8InspectorSession::resume, WTF::Unretained(v8_session_),
                    /* setTerminateOnResume */ false));
            return;
        }
        if (message_string == "stepOver") {
            task_runner->PostTask(FROM_HERE, WTF::BindOnce(&v8_inspector::V8InspectorSession::stepOver, WTF::Unretained(v8_session_)));
            return;
        }
    }
    exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, kInvalidOverlayCommand);
...
WindowControlsOverlayTool::WindowControlsOverlayTool(
    InspectorOverlayAgent* overlay, OverlayFrontend* frontend, std::unique_ptr<protocol::DictionaryValue> wco_config)
    : InspectTool(overlay, frontend)
    , wco_config_(std::move(wco_config))
{
}
...
{
    overlay_->EvaluateInOverlay("drawWindowControlsOverlay", wco_config_->clone());
}
