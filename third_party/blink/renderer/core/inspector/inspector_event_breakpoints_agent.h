...
class CORE_EXPORT InspectorEventBreakpointsAgent final : public InspectorBaseAgent<protocol::EventBreakpoints::Metainfo> {
public:
...
private:
    ...
    protocol::Response setInstrumentationBreakpoint(const String& event_name) override;
    protocol::Response removeInstrumentationBreakpoint(const String& event_name) override;

    bool IsEnabled() const;
    std::unique_ptr<protocol::DictionaryValue> MaybeBuildBreakpointData(const String& name);

    void TriggerSyncBreakpoint(const protocol::DictionaryValue& breakpoint_data);
    void ScheduleAsyncBreakpoint(const protocol::DictionaryValue& breakpoint_data);
    void UnscheduleAsyncBreakpoint();

    v8_inspector::V8InspectorSession* const v8_session_;
    InspectorAgentState::BooleanMap event_listener_breakpoints_ { &agent_state_, /*default_value=*/false };
};
