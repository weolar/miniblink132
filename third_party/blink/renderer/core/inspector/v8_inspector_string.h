...
public:
    ...
    static Binary fromCachedData(std::unique_ptr<v8::ScriptCompiler::CachedData> data);

private:
    explicit Binary(scoped_refptr<Impl> impl)
        : impl_(std::move(impl))
    {
    }
...
template <> struct ProtocolTypeTraits<WTF::String> {
...
template <> struct ProtocolTypeTraits<blink::protocol::Binary> {
    static bool Deserialize(DeserializerState* state, blink::protocol::Binary* value);
    static void Serialize(const blink::protocol::Binary& value, std::vector<uint8_t>* bytes);
};

namespace detail {
template <> struct MaybeTypedef<WTF::String> {
    typedef ValueMaybe<WTF::String> type;
};

template <> struct MaybeTypedef<blink::protocol::Binary> {
