...
static std::unique_ptr<protocol::Value> ParseJSON(const String& string)
{
    ...
    if (string.Is8Bit()) {
        crdtp::json::ConvertJSONToCBOR(crdtp::span<uint8_t>(string.Characters8(), string.length()), &cbor);
    } else {
        crdtp::json::ConvertJSONToCBOR(crdtp::span<uint16_t>(reinterpret_cast<const uint16_t*>(string.Characters16()), string.length()), &cbor);
    }
...
TEST(ProtocolParserTest, Reading)
{
    ...
    for (const char* test : { "{\"number\":9.87654321, \"null\":null , \"S\" : \"str\" }",
             "{\n"
             "  \"number\":9.87654321,\n"
             "  \"null\":null,\n"
             "  \"S\":\"str\"\n"
...
{
    const auto kInvalidJson
        = std::to_array<const char*>({ "/* test *", "{\"foo\"", "{\"foo\":", "  [", "\"\\u123g\"", "{\n\"eh:\n}", "////", "*/**/", "/**/", "/*/", "//**/" });
