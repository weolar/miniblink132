...
InspectorEmulationAgent::InspectorEmulationAgent(WebLocalFrameImpl* web_local_frame_impl, VirtualTimeController& virtual_time_controller)
...
void InspectorEmulationAgent::Restore()
{
    ...
    // directly.
    std::vector<uint8_t> save_serialized_ua_metadata_override = serialized_ua_metadata_override_.Get();
    setUserAgentOverride(user_agent_override_.Get(), accept_language_override_.Get(), navigator_platform_override_.Get(),
        protocol::Maybe<protocol::Emulation::UserAgentMetadata>());
    ua_metadata_override_ = blink::UserAgentMetadata::Demarshal(
        std::string(reinterpret_cast<char*>(save_serialized_ua_metadata_override.data()), save_serialized_ua_metadata_override.size()));
    ...
    setTouchEmulationEnabled(touch_event_emulation_enabled_.Get(), max_touch_points_.Get());
    auto features = std::make_unique<protocol::Array<protocol::Emulation::MediaFeature>>();
    for (auto const& name : emulated_media_features_.Keys()) {
        auto const& value = emulated_media_features_.Get(name);
        features->push_back(protocol::Emulation::MediaFeature::create().setName(name).setValue(value).build());
    }
    ...
    auto status_or_rgba = protocol::DOM::RGBA::ReadFrom(default_background_color_override_rgba_.Get());
    ...
    if (virtual_time_policy_.Get() == protocol::Emulation::VirtualTimePolicyEnum::Pause) {
        setVirtualTimePolicy(
            protocol::Emulation::VirtualTimePolicyEnum::Pause, Maybe<double>(), Maybe<int>(), initial_virtual_time_.Get(), &virtual_time_ticks_base_ms);
        return;
    }
    ...
    setVirtualTimePolicy(
        virtual_time_policy_.Get(), budget_remaining, virtual_time_task_starvation_count_.Get(), initial_virtual_time_.Get(), &virtual_time_ticks_base_ms);
}
...
protocol::Response InspectorEmulationAgent::disable()
{
    ...
    setUserAgentOverride(String(), protocol::Maybe<String>(), protocol::Maybe<String>(), protocol::Maybe<protocol::Emulation::UserAgentMetadata>());
    if (!locale_override_.Get().empty())
    ...
    setEmulatedMedia(String(), std::make_unique<protocol::Array<protocol::Emulation::MediaFeature>>());
    if (!emulated_vision_deficiency_.Get().IsNull())
...
protocol::Response InspectorEmulationAgent::setPageScaleFactor(double page_scale_factor)
{
    protocol::Response response = AssertPage();
    if (!response.IsSuccess())
        return response;
...
protocol::Response InspectorEmulationAgent::setDocumentCookieDisabled(bool disabled)
{
    protocol::Response response = AssertPage();
    if (!response.IsSuccess())
        return response;
...
protocol::Response InspectorEmulationAgent::setTouchEmulationEnabled(bool enabled, protocol::Maybe<int> max_touch_points)
{
    protocol::Response response = AssertPage();
    if (!response.IsSuccess())
        return response;
    ...
    if (max_points < 1 || max_points > WebTouchEvent::kTouchesLengthCap) {
        String msg = "Touch points must be between 1 and " + String::Number(static_cast<uint16_t>(WebTouchEvent::kTouchesLengthCap));
...
protocol::Response InspectorEmulationAgent::setEmulatedMedia(Maybe<String> media, Maybe<protocol::Array<protocol::Emulation::MediaFeature>> features)
{
    protocol::Response response = AssertPage();
    if (!response.IsSuccess())
        return response;
    ...
    if (features.has_value()) {
        ...
        auto const& forced_colors_value = emulated_media_features_.Get("forced-colors");
        auto const& prefers_color_scheme_value = emulated_media_features_.Get("prefers-color-scheme");

        if (forced_colors_value == "active") {
            if (!forced_colors_override_) {
                initial_system_forced_colors_state_ = GetWebViewImpl()->GetPage()->GetSettings().GetInForcedColors();
            }
            forced_colors_override_ = true;
            bool is_dark_mode = false;
            if (prefers_color_scheme_value.empty()) {
                is_dark_mode = GetWebViewImpl()->GetPage()->GetSettings().GetPreferredColorScheme() == mojom::blink::PreferredColorScheme::kDark;
            } else {
            ...
            GetWebViewImpl()->GetPage()->DisableEmulatedForcedColors();
            GetWebViewImpl()->GetPage()->GetSettings().SetInForcedColors(initial_system_forced_colors_state_);
        }

        for (const WTF::String& feature : emulated_media_features_.Keys()) {
            auto const& value = emulated_media_features_.Get(feature);
            GetWebViewImpl()->GetPage()->SetMediaFeatureOverride(AtomicString(feature), value);
        }
...
protocol::Response InspectorEmulationAgent::setEmulatedVisionDeficiency(const String& type)
{
    protocol::Response response = AssertPage();
    if (!response.IsSuccess())
        return response;
    ...
    namespace TypeEnum = protocol::Emulation::SetEmulatedVisionDeficiency::TypeEnum;
...
protocol::Response InspectorEmulationAgent::setFocusEmulationEnabled(bool enabled)
{
    protocol::Response response = AssertPage();
    if (!response.IsSuccess())
        return response;
    ...
    GetWebViewImpl()->GetPage()->GetFocusController().SetFocusEmulationEnabled(enabled);
    return response;
}

protocol::Response InspectorEmulationAgent::setAutoDarkModeOverride(Maybe<bool> enabled)
{
    protocol::Response response = AssertPage();
    if (!response.IsSuccess())
        return response;
    if (enabled.has_value()) {
        ...
        GetWebViewImpl()->GetDevToolsEmulator()->SetAutoDarkModeOverride(enabled.value());
    } else {
...
protocol::Response InspectorEmulationAgent::setVirtualTimePolicy(const String& policy, Maybe<double> virtual_time_budget_ms,
    protocol::Maybe<int> max_virtual_time_task_starvation_count, protocol::Maybe<double> initial_virtual_time, double* virtual_time_ticks_base_ms)
{
    VirtualTimeController::VirtualTimePolicy scheduler_policy = VirtualTimeController::VirtualTimePolicy::kPause;
    ...
    } else if (protocol::Emulation::VirtualTimePolicyEnum::PauseIfNetworkFetchesPending == policy) {
        scheduler_policy = VirtualTimeController::VirtualTimePolicy::kDeterministicLoading;
    } else {
        DCHECK_EQ(scheduler_policy, VirtualTimeController::VirtualTimePolicy::kPause);
        if (virtual_time_budget_ms.has_value()) {
            return protocol::Response::InvalidParams("Can only specify budget for non-Pause policy");
        }
        if (max_virtual_time_task_starvation_count.has_value()) {
            return protocol::Response::InvalidParams("Can only specify starvation count for non-Pause policy");
        }
    }
    ...
    virtual_time_task_starvation_count_.Set(max_virtual_time_task_starvation_count.value_or(0));

    InnerEnable();

    // This needs to happen before we apply virtual time.
    base::Time initial_time = initial_virtual_time.has_value() ? base::Time::FromSecondsSinceUnixEpoch(initial_virtual_time.value()) : base::Time();
    virtual_time_base_ticks_ = virtual_time_controller_.EnableVirtualTime(initial_time);
    virtual_time_controller_.SetVirtualTimePolicy(scheduler_policy);
    if (virtual_time_budget_ms.value_or(0) > 0) {
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN1("renderer.scheduler", "VirtualTimeBudget", TRACE_ID_LOCAL(this), "budget", virtual_time_budget_ms.value());
        const base::TimeDelta budget_amount = base::Milliseconds(virtual_time_budget_ms.value());
        virtual_time_controller_.GrantVirtualTimeBudget(
            budget_amount, WTF::BindOnce(&InspectorEmulationAgent::VirtualTimeBudgetExpired, WrapWeakPersistent(this)));
    ...
    if (max_virtual_time_task_starvation_count.value_or(0)) {
        virtual_time_controller_.SetMaxVirtualTimeTaskStarvationCount(max_virtual_time_task_starvation_count.value());
    }

    *virtual_time_ticks_base_ms = virtual_time_base_ticks_.is_null() ? 0 : (virtual_time_base_ticks_ - base::TimeTicks()).InMillisecondsF();

    return protocol::Response::Success();
}

AtomicString InspectorEmulationAgent::OverrideAcceptImageHeader(const HashSet<String>* disabled_image_types)
{
...
void InspectorEmulationAgent::PrepareRequest(DocumentLoader* loader, ResourceRequest& request, ResourceLoaderOptions& options, ResourceType resource_type)
{
    if (!accept_language_override_.Get().empty() && request.HttpHeaderField(http_names::kAcceptLanguage).empty()) {
        request.SetHttpHeaderField(http_names::kAcceptLanguage, AtomicString(network_utils::GenerateAcceptLanguageHeader(accept_language_override_.Get())));
    }
    ...
    if (!options.unsupported_image_mime_types) {
        options.unsupported_image_mime_types = base::MakeRefCounted<base::RefCountedData<HashSet<String>>>();
    }
    ...
    request.SetHTTPAccept(OverrideAcceptImageHeader(&options.unsupported_image_mime_types->data));
...
protocol::Response InspectorEmulationAgent::setNavigatorOverrides(const String& platform)
{
    protocol::Response response = AssertPage();
    if (!response.IsSuccess())
        return response;
    navigator_platform_override_.Set(platform);
    GetWebViewImpl()->GetPage()->GetSettings().SetNavigatorPlatformOverride(platform);
    return response;
}

void InspectorEmulationAgent::VirtualTimeBudgetExpired()
{
    TRACE_EVENT_NESTABLE_ASYNC_END0("renderer.scheduler", "VirtualTimeBudget", TRACE_ID_LOCAL(this));
    ...
    virtual_time_controller_.SetVirtualTimePolicy(VirtualTimeController::VirtualTimePolicy::kPause);
...
protocol::Response InspectorEmulationAgent::setDefaultBackgroundColorOverride(Maybe<protocol::DOM::RGBA> color)
{
    protocol::Response response = AssertPage();
    if (!response.IsSuccess())
        return response;
    ...
    GetWebViewImpl()->SetBaseBackgroundColorOverrideForInspector(Color(rgba->getR(), rgba->getG(), rgba->getB(), alpha).Rgb());
    return protocol::Response::Success();
}

protocol::Response InspectorEmulationAgent::setDeviceMetricsOverride(int width, int height, double device_scale_factor, bool mobile, Maybe<double> scale,
    Maybe<int> screen_width, Maybe<int> screen_height, Maybe<int> position_x, Maybe<int> position_y, Maybe<bool> dont_set_visible_size,
    Maybe<protocol::Emulation::ScreenOrientation>, Maybe<protocol::Page::Viewport>, Maybe<protocol::Emulation::DisplayFeature>,
...
protocol::Response InspectorEmulationAgent::setHardwareConcurrencyOverride(int hardware_concurrency)
{
    if (hardware_concurrency <= 0) {
        return protocol::Response::InvalidParams("HardwareConcurrency must be a positive number");
    }
    InnerEnable();
...
protocol::Response InspectorEmulationAgent::setUserAgentOverride(const String& user_agent, protocol::Maybe<String> accept_language,
    protocol::Maybe<String> platform, protocol::Maybe<protocol::Emulation::UserAgentMetadata> ua_metadata_override)
{
    if (!user_agent.empty() || accept_language.has_value() || platform.has_value()) {
        InnerEnable();
    }
    ...
    if (web_local_frame_) {
        GetWebViewImpl()->GetPage()->GetSettings().SetNavigatorPlatformOverride(navigator_platform_override_.Get());
    }

    if (ua_metadata_override.has_value()) {
        blink::UserAgentMetadata default_ua_metadata = Platform::Current()->UserAgentMetadata();

        if (user_agent.empty()) {
            ua_metadata_override_ = std::nullopt;
            serialized_ua_metadata_override_.Set(std::vector<uint8_t>());
            return protocol::Response::InvalidParams("Can't specify UserAgentMetadata but no UA string");
        }
        protocol::Emulation::UserAgentMetadata& ua_metadata = ua_metadata_override.value();
        ...
        if (ua_metadata.hasBrands()) {
            ...
            ua_metadata_override_->brand_version_list = std::move(default_ua_metadata.brand_version_list);
        }
        ...
        if (ua_metadata.hasFullVersionList()) {
            for (const auto& bv : *ua_metadata.getFullVersionList(nullptr)) {
                ...
                out_bv.brand = bv->getBrand().Ascii();
                out_bv.version = bv->getVersion().Ascii();
                ua_metadata_override_->brand_full_version_list.push_back(std::move(out_bv));
            }
        } else {
            ua_metadata_override_->brand_full_version_list = std::move(default_ua_metadata.brand_full_version_list);
        }

        if (ua_metadata.hasFullVersion()) {
            ua_metadata_override_->full_version = ua_metadata.getFullVersion("").Ascii();
        } else {
            ua_metadata_override_->full_version = std::move(default_ua_metadata.full_version);
        }
        ua_metadata_override_->platform = ua_metadata.getPlatform().Ascii();
        ua_metadata_override_->platform_version = ua_metadata.getPlatformVersion().Ascii();
    ...
    std::string marshalled = blink::UserAgentMetadata::Marshal(ua_metadata_override_).value_or(std::string());
    std::vector<uint8_t> marshalled_as_bytes;
    marshalled_as_bytes.insert(marshalled_as_bytes.end(), marshalled.begin(), marshalled.end());
    serialized_ua_metadata_override_.Set(std::move(marshalled_as_bytes));

    return protocol::Response::Success();
}

protocol::Response InspectorEmulationAgent::setLocaleOverride(protocol::Maybe<String> maybe_locale)
{
    // Only allow resetting overrides set by the same agent.
    if (locale_override_.Get().empty() && LocaleController::instance().has_locale_override()) {
        return protocol::Response::ServerError("Another locale override is already in effect");
    }
...
protocol::Response InspectorEmulationAgent::setTimezoneOverride(const String& timezone_id)
{
    if (timezone_id == TimeZoneController::TimeZoneIdOverride()) {
        ...
        if (!timezone_override_) {
            return TimeZoneController::HasTimeZoneOverride() ? protocol::Response::ServerError("Timezone override is already in effect")
...
void InspectorEmulationAgent::WillCreateDocumentParser(bool& force_sync_parsing)
{
    if (virtual_time_policy_.Get().IsNull())
        return;
...
void InspectorEmulationAgent::ApplyHardwareConcurrencyOverride(unsigned int& hardware_concurrency)
{
    if (int concurrency = hardware_concurrency_override_.Get())
...
void InspectorEmulationAgent::ApplyUserAgentMetadataOverride(std::optional<blink::UserAgentMetadata>* ua_metadata)
{
...
{
    if (!web_local_frame_) {
        return protocol::Response::ServerError("Operation is only supported for pages, not workers");
    }
    return protocol::Response::Success();
}
...
protocol::Response InspectorEmulationAgent::setDisabledImageTypes(std::unique_ptr<protocol::Array<protocol::Emulation::DisabledImageType>> disabled_types)
{
    ...
    for (protocol::Emulation::DisabledImageType type : *disabled_types) {
        if (DisabledImageTypeEnum::Avif == type || DisabledImageTypeEnum::Webp == type) {
