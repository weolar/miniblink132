...
{
    std::unique_ptr<base::ProcessMetrics> process_metrics = GetCurrentProcessMetrics();
    return process_metrics->GetCumulativeCPUUsage().value_or(base::TimeDelta());
}

} // namespace

InspectorPerformanceAgent::InspectorPerformanceAgent(InspectedFrames* inspected_frames)
...
protocol::Response InspectorPerformanceAgent::enable(Maybe<String> optional_time_domain)
{
    String time_domain = optional_time_domain.value_or(TimeDomain::TimeTicks);
    if (enabled_.Get()) {
        if (!HasTimeDomain(time_domain)) {
            return protocol::Response::ServerError("Cannot change time domain while performance metrics collection is "
                                                   "enabled.");
        }
...
namespace {
void AppendMetric(protocol::Array<protocol::Performance::Metric>* container, const String& name, double value)
{
    container->emplace_back(protocol::Performance::Metric::create().setName(name).setValue(value).build());
}
} // namespace

// TODO(crbug.com/1056306): remove this redundant API.
protocol::Response InspectorPerformanceAgent::setTimeDomain(const String& time_domain)
{
    if (enabled_.Get()) {
        return protocol::Response::ServerError("Cannot set time domain while performance metrics collection"
                                               " is enabled.");
    }
...
{
    return use_thread_ticks_.Get() ? GetThreadTimeNow() : base::subtle::TimeTicksNowIgnoringOverride();
}

base::TimeTicks InspectorPerformanceAgent::GetThreadTimeNow()
{
    return base::TimeTicks() + base::Microseconds(base::ThreadTicks::Now().since_origin().InMicroseconds());
}

bool InspectorPerformanceAgent::HasTimeDomain(const String& time_domain)
{
    return use_thread_ticks_.Get() ? time_domain == TimeDomain::ThreadTicks : time_domain == TimeDomain::TimeTicks;
}

protocol::Response InspectorPerformanceAgent::InnerSetTimeDomain(const String& time_domain)
{
    DCHECK(!enabled_.Get());
    ...
    if (time_domain == TimeDomain::ThreadTicks) {
        if (!base::ThreadTicks::IsSupported()) {
            return protocol::Response::ServerError("Thread time is not supported on this platform.");
        }
...
protocol::Response InspectorPerformanceAgent::getMetrics(std::unique_ptr<protocol::Array<protocol::Performance::Metric>>* out_result)
{
    if (!enabled_.Get()) {
        *out_result = std::make_unique<protocol::Array<protocol::Performance::Metric>>();
        return protocol::Response::Success();
    }

    auto result = std::make_unique<protocol::Array<protocol::Performance::Metric>>();

    AppendMetric(result.get(), "Timestamp", base::TimeTicks::Now().since_origin().InSecondsF());

    // Renderer instance counters.
    for (size_t i = 0; i < std::size(kInstanceCounterNames); ++i) {
        AppendMetric(result.get(), kInstanceCounterNames[i], InstanceCounters::CounterValue(static_cast<InstanceCounters::CounterType>(i)));
    }

    // Page performance metrics.
    base::TimeTicks now = GetTimeTicksNow();
    AppendMetric(result.get(), "LayoutCount", static_cast<double>(layout_count_));
    AppendMetric(result.get(), "RecalcStyleCount", static_cast<double>(recalc_style_count_));
    AppendMetric(result.get(), "LayoutDuration", layout_duration_.InSecondsF());
    AppendMetric(result.get(), "RecalcStyleDuration", recalc_style_duration_.InSecondsF());
    ...
    AppendMetric(result.get(), "DevToolsCommandDuration", devtools_command_duration.InSecondsF());
    ...
    AppendMetric(result.get(), "V8CompileDuration", v8compile_duration.InSecondsF());
    ...
    base::TimeDelta known_tasks_duration = script_duration + v8compile_duration + recalc_style_duration_ + layout_duration_ + devtools_command_duration;
    base::TimeDelta other_tasks_duration = task_duration - known_tasks_duration;
    AppendMetric(result.get(), "TaskOtherDuration", other_tasks_duration.InSecondsF());
    ...
    if (document) {
        ...
        AppendMetric(result.get(), "JSHeapUsedSize", heap_statistics.used_heap_size());
        AppendMetric(result.get(), "JSHeapTotalSize", heap_statistics.total_heap_size());

        AppendMetric(result.get(), "FirstMeaningfulPaint", PaintTiming::From(*document).FirstMeaningfulPaint().since_origin().InSecondsF());
        AppendMetric(result.get(), "DomContentLoaded", document->GetTiming().DomContentLoadedEventStart().since_origin().InSecondsF());
        AppendMetric(result.get(), "NavigationStart", document->Loader()->GetTiming().NavigationStart().since_origin().InSecondsF());
    }
...
void InspectorPerformanceAgent::ConsoleTimeStamp(v8::Isolate* isolate, v8::Local<v8::String> label)
{
    if (!enabled_.Get())
        return;
...
void InspectorPerformanceAgent::DidFinishDebuggerTask()
{
    ...
    devtools_command_duration_ += GetTimeTicksNow() - devtools_command_start_ticks_;
    devtools_command_start_ticks_ = base::TimeTicks();
}
...
void InspectorPerformanceAgent::DidProcessTask(base::TimeTicks start_time, base::TimeTicks end_time)
{
    if (task_start_ticks_.is_null())
        return;
