...
protocol::DOM::ScrollOrientation ToScrollOrientation(ScrollSnapshotTimeline::ScrollAxis scroll_axis_enum, bool is_horizontal_writing_mode)
{
    switch (scroll_axis_enum) {
    case ScrollSnapshotTimeline::ScrollAxis::kBlock:
        return is_horizontal_writing_mode ? protocol::DOM::ScrollOrientationEnum::Vertical : protocol::DOM::ScrollOrientationEnum::Horizontal;
    case ScrollSnapshotTimeline::ScrollAxis::kInline:
        return is_horizontal_writing_mode ? protocol::DOM::ScrollOrientationEnum::Horizontal : protocol::DOM::ScrollOrientationEnum::Vertical;
...
double NormalizedDuration(V8UnionCSSNumericValueOrStringOrUnrestrictedDouble* duration)
{
    ...
    if (duration->IsCSSNumericValue()) {
        CSSUnitValue* percentage_unit_value = duration->GetAsCSSNumericValue()->to(CSSPrimitiveValue::UnitType::kPercentage);
...
InspectorAnimationAgent::InspectorAnimationAgent(InspectedFrames* inspected_frames, InspectorCSSAgent* css_agent, v8_inspector::V8InspectorSession* v8_session)
...
String InspectorAnimationAgent::AnimationDisplayName(const Animation& animation)
{
...
protocol::Response InspectorAnimationAgent::enable()
{
    ...
    HeapVector<Member<Animation>> animations = document_animations.getAnimations(document->GetTreeScope());
    for (Animation* animation : animations) {
        const String& animation_id = String::Number(animation->SequenceNumber());
        V8AnimationPlayState::Enum play_state = animation->CalculateAnimationPlayState();
        bool is_play_state_running_or_finished = play_state == V8AnimationPlayState::Enum::kRunning || play_state == V8AnimationPlayState::Enum::kFinished;
        if (!is_play_state_running_or_finished || cleared_animations_.Contains(animation_id) || id_to_animation_.Contains(animation_id)) {
            continue;
        }
...
static std::unique_ptr<protocol::Animation::ViewOrScrollTimeline> BuildObjectForViewOrScrollTimeline(AnimationTimeline* timeline)
{
    ScrollSnapshotTimeline* scroll_snapshot_timeline = DynamicTo<ScrollSnapshotTimeline>(timeline);
    if (scroll_snapshot_timeline) {
        ...
        std::unique_ptr<protocol::Animation::ViewOrScrollTimeline> timeline_object
            = protocol::Animation::ViewOrScrollTimeline::create()
                  ...
                  .setAxis(ToScrollOrientation(scroll_snapshot_timeline->GetAxis(), scroll_container->IsHorizontalWritingMode()))
                  .build();
        std::optional<ScrollSnapshotTimeline::ScrollOffsets> scroll_offsets = scroll_snapshot_timeline->GetResolvedScrollOffsets();
        ...
        ViewTimeline* view_timeline = DynamicTo<ViewTimeline>(scroll_snapshot_timeline);
        if (view_timeline && view_timeline->subject()) {
            timeline_object->setSubjectNodeId(IdentifiersFactory::IntIdForNode(view_timeline->subject()));
        }
...
static std::unique_ptr<protocol::Animation::AnimationEffect> BuildObjectForAnimationEffect(KeyframeEffect* effect)
{
    ...
    std::unique_ptr<protocol::Animation::AnimationEffect> animation_object = protocol::Animation::AnimationEffect::create()
                                                                                 .setDelay(delay)
    ...
    if (effect->EffectTarget()) {
        animation_object->setBackendNodeId(IdentifiersFactory::IntIdForNode(effect->EffectTarget()));
    }
    return animation_object;
}

static std::unique_ptr<protocol::Animation::KeyframeStyle> BuildObjectForStringKeyframe(const StringKeyframe* keyframe, double computed_offset)
{
    ...
    std::unique_ptr<protocol::Animation::KeyframeStyle> keyframe_object
        = protocol::Animation::KeyframeStyle::create().setOffset(offset).setEasing(keyframe->Easing().ToString()).build();
...
static std::unique_ptr<protocol::Animation::KeyframesRule> BuildObjectForAnimationKeyframes(const KeyframeEffect* effect)
{
    ...
    Vector<double> computed_offsets = KeyframeEffectModelBase::GetComputedOffsets(model->GetFrames());
    auto keyframes = std::make_unique<protocol::Array<protocol::Animation::KeyframeStyle>>();
    ...
    for (wtf_size_t i = 0; i < model->GetFrames().size(); i++) {
        ...
        keyframes->emplace_back(BuildObjectForStringKeyframe(string_keyframe, computed_offsets.at(i)));
    }
...
std::unique_ptr<protocol::Animation::Animation> InspectorAnimationAgent::BuildObjectForAnimation(blink::Animation& animation)
{
    ...
    if (animation.effect()) {
        animation_effect_object = BuildObjectForAnimationEffect(To<KeyframeEffect>(animation.effect()));

        if (IsA<CSSTransition>(animation)) {
            animation_type = AnimationType::CSSTransition;
        } else {
            animation_effect_object->setKeyframesRule(BuildObjectForAnimationKeyframes(To<KeyframeEffect>(animation.effect())));
    ...
    std::optional<AnimationTimeDelta> animation_current_time = animation.CurrentTimeInternal();
    ...
    std::unique_ptr<protocol::Animation::Animation> animation_object
        = protocol::Animation::Animation::create()
              ...
              .setPlayState(V8AnimationPlayState(animation.CalculateAnimationPlayState()).AsString())
    ...
    std::unique_ptr<protocol::Animation::ViewOrScrollTimeline> view_or_scroll_timeline = BuildObjectForViewOrScrollTimeline(animation.TimelineInternal());
    if (view_or_scroll_timeline) {
        animation_object->setViewOrScrollTimeline(std::move(view_or_scroll_timeline));
    }
    return animation_object;
}

protocol::Response InspectorAnimationAgent::getPlaybackRate(double* playback_rate)
{
    *playback_rate = ReferenceTimeline().PlaybackRate();
    return protocol::Response::Success();
}

protocol::Response InspectorAnimationAgent::setPlaybackRate(double playback_rate)
{
...
protocol::Response InspectorAnimationAgent::getCurrentTime(const String& id, double* current_time)
{
    ...
    *current_time = Timing::NullValue();
    if (animation->Paused() || !animation->TimelineInternal()->IsActive()) {
        ...
        std::optional<AnimationTimeDelta> animation_start_time = animation->StartTimeInternal();
        if (animation_start_time) {
            std::optional<AnimationTimeDelta> timeline_time = animation->TimelineInternal()->CurrentTime();
            // TODO(crbug.com/916117): Handle NaN values for scroll linked animations.
            if (timeline_time) {
                *current_time = timeline_time.value().InMillisecondsF() - animation_start_time.value().InMillisecondsF();
            }
        }
    }
    return protocol::Response::Success();
...
protocol::Response InspectorAnimationAgent::setPaused(std::unique_ptr<protocol::Array<String>> animation_ids, bool paused)
{
...
protocol::Response InspectorAnimationAgent::seekAnimations(std::unique_ptr<protocol::Array<String>> animation_ids, double current_time)
{
    for (const String& animation_id : *animation_ids) {
        blink::Animation* animation = nullptr;
        protocol::Response response = AssertAnimation(animation_id, animation);
        ...
        animation->SetCurrentTimeInternal(ANIMATION_TIME_DELTA_FROM_MILLISECONDS(current_time));
    }
    return protocol::Response::Success();
}

protocol::Response InspectorAnimationAgent::releaseAnimations(std::unique_ptr<protocol::Array<String>> animation_ids)
{
    for (const String& animation_id : *animation_ids) {
...
protocol::Response InspectorAnimationAgent::setTiming(const String& animation_id, double duration, double delay)
{
    blink::Animation* animation = nullptr;
    protocol::Response response = AssertAnimation(animation_id, animation);
    if (!response.IsSuccess())
    ...
    timing->setDuration(MakeGarbageCollected<V8UnionCSSNumericValueOrStringOrUnrestrictedDouble>(duration));
...
protocol::Response InspectorAnimationAgent::resolveAnimation(
    const String& animation_id, std::unique_ptr<v8_inspector::protocol::Runtime::API::RemoteObject>* result)
{
    blink::Animation* animation = nullptr;
    protocol::Response response = AssertAnimation(animation_id, animation);
    if (!response.IsSuccess())
    ...
    const Element* element = To<KeyframeEffect>(animation->effect())->EffectTarget();
    Document* document = element->ownerDocument();
    LocalFrame* frame = document ? document->GetFrame() : nullptr;
    ScriptState* script_state = ToScriptStateForMainWorld(frame);
    if (!script_state) {
        return protocol::Response::ServerError("Element not associated with a document.");
    }

    ScriptState::Scope scope(script_state);
    static const char kAnimationObjectGroup[] = "animation";
    v8_session_->releaseObjectGroup(ToV8InspectorStringView(kAnimationObjectGroup));
    *result = v8_session_->wrapObject(script_state->GetContext(), ToV8Traits<Animation>::ToV8(script_state, animation),
        ToV8InspectorStringView(kAnimationObjectGroup), false /* generatePreview */);
    if (!*result) {
        return protocol::Response::ServerError("Element not associated with a document.");
    }
    return protocol::Response::Success();
...
String InspectorAnimationAgent::CreateCSSId(blink::Animation& animation)
{
    ...
    HeapVector<Member<CSSStyleDeclaration>> styles = css_agent_->MatchingStyles(element);
    Digestor digestor(kHashAlgorithmSha1);
    digestor.UpdateUtf8(IsA<CSSTransition>(animation) ? AnimationType::CSSTransition : AnimationType::CSSAnimation);
    digestor.UpdateUtf8(animation.id());
    for (const CSSPropertyName& name : css_property_names) {
        CSSStyleDeclaration* style = css_agent_->FindEffectiveDeclaration(name, styles);
        // Ignore inline styles.
        if (!style || !style->ParentStyleSheet() || !style->parentRule() || style->parentRule()->GetType() != CSSRule::kStyleRule)
            continue;
...
void InspectorAnimationAgent::NotifyAnimationUpdated(const String& animation_id)
{
...
bool InspectorAnimationAgent::CompareAndUpdateKeyframesSnapshot(
    KeyframeEffect* keyframe_effect, HeapVector<Member<AnimationKeyframeSnapshot>>* animation_snapshot_keyframes)
{
    ...
    if (model->GetFrames().size() != animation_snapshot_keyframes->size()) {
        ...
        for (wtf_size_t i = 0; i < model->GetFrames().size(); i++) {
            ...
            const auto* string_keyframe = To<StringKeyframe>(keyframe);
            AnimationKeyframeSnapshot* keyframe_snapshot = MakeGarbageCollected<AnimationKeyframeSnapshot>();
    ...
    for (wtf_size_t i = 0; i < animation_snapshot_keyframes->size(); i++) {
        AnimationKeyframeSnapshot* keyframe_snapshot = animation_snapshot_keyframes->at(i);
        const Keyframe* keyframe = model->GetFrames().at(i);
        if (!keyframe->IsStringKeyframe()) {
            continue;
        }
...
bool InspectorAnimationAgent::CompareAndUpdateInternalSnapshot(blink::Animation& animation, AnimationSnapshot* snapshot)
{
    V8AnimationPlayState::Enum new_play_state = animation.PendingInternal() ? V8AnimationPlayState::Enum::kPending : animation.CalculateAnimationPlayState();
    bool should_notify_frontend = false;
    ...
    if (animation.effect()) {
        ComputedEffectTiming* computed_timing = animation.effect()->getComputedTiming();
        if (computed_timing) {
        ...
        if (KeyframeEffect* keyframe_effect = DynamicTo<KeyframeEffect>(animation.effect())) {
            if (CompareAndUpdateKeyframesSnapshot(keyframe_effect, &snapshot->keyframes)) {
                should_notify_frontend = true;
            }
        }
    }

    ScrollSnapshotTimeline* scroll_snapshot_timeline = DynamicTo<ScrollSnapshotTimeline>(animation.TimelineInternal());
    if (scroll_snapshot_timeline) {
        std::optional<ScrollSnapshotTimeline::ScrollOffsets> scroll_offsets = scroll_snapshot_timeline->GetResolvedScrollOffsets();
        if (scroll_offsets.has_value()) {
...
void InspectorAnimationAgent::AnimationUpdated(blink::Animation* animation)
{
    ...
    if (new_play_state != V8AnimationPlayState::Enum::kRunning && new_play_state != V8AnimationPlayState::Enum::kFinished) {
        return;
    }

    bool should_notify_frontend = this->CompareAndUpdateInternalSnapshot(*animation, snapshot);
    if (should_notify_frontend && !notify_animation_updated_tasks_.Contains(animation_id)) {
        ...
        task_runner->PostDelayedTask(FROM_HERE,
            WTF::BindOnce(&InspectorAnimationAgent::NotifyAnimationUpdated, WrapPersistent(weak_factory_.GetWeakCell()), animation_id), base::Milliseconds(50));
    }
}
...
void InspectorAnimationAgent::DidClearDocumentOfWindowObject(LocalFrame* frame)
{
    ...
    frame->GetDocument()->Timeline().SetPlaybackRate(ReferenceTimeline().PlaybackRate());
}

protocol::Response InspectorAnimationAgent::AssertAnimation(const String& id, blink::Animation*& result)
{
    auto it = id_to_animation_.find(id);
    if (it == id_to_animation_.end()) {
        result = nullptr;
        return protocol::Response::ServerError("Could not find animation with given id");
    }
    result = it->value;
    return protocol::Response::Success();
}
...
double InspectorAnimationAgent::NormalizedStartTime(blink::Animation& animation)
{
    ...
    if (start_time->IsDouble()) {
        ...
        auto* document_timeline = DynamicTo<DocumentTimeline>(animation.TimelineInternal());
        if (document_timeline) {
            if (ReferenceTimeline().PlaybackRate() == 0) {
                time_ms += ReferenceTimeline().CurrentTimeMilliseconds().value_or(Timing::NullValue())
                    - document_timeline->CurrentTimeMilliseconds().value_or(Timing::NullValue());
            } else {
                time_ms += (document_timeline->ZeroTime() - ReferenceTimeline().ZeroTime()).InMillisecondsF() * ReferenceTimeline().PlaybackRate();
            }
        }
    ...
    if (start_time->IsCSSNumericValue()) {
        CSSUnitValue* percent_unit_value = start_time->GetAsCSSNumericValue()->to(CSSPrimitiveValue::UnitType::kPercentage);
