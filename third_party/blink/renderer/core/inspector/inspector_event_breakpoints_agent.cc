...
constexpr char kSharedStorageWorkletScriptFirstStatement[] = "sharedStorageWorkletScriptFirstStatement";

} // namespace event_names

using Response = protocol::Response;

InspectorEventBreakpointsAgent::InspectorEventBreakpointsAgent(v8_inspector::V8InspectorSession* v8_session)
...
{
    if (auto data = MaybeBuildBreakpointData(event_names::kCanvasContextCreated)) {
        TriggerSyncBreakpoint(*data);
    }
}

void InspectorEventBreakpointsAgent::DidFireWebGLError(const String& error_name)
{
...
void InspectorEventBreakpointsAgent::DidFireWebGLErrorOrWarning(const String& message)
{
...
void InspectorEventBreakpointsAgent::ScriptExecutionBlockedByCSP(const String& directive_text)
{
...
{
    if (auto data = MaybeBuildBreakpointData(event_names::kScriptFirstStatement)) {
        ScheduleAsyncBreakpoint(*data);
        return;
    }

    if (probe.context && probe.context->IsSharedStorageWorkletGlobalScope()) {
        if (auto data = MaybeBuildBreakpointData(event_names::kSharedStorageWorkletScriptFirstStatement)) {
            ScheduleAsyncBreakpoint(*data);
        }
    }
}
...
{
    if (auto data = MaybeBuildBreakpointData(event_names::kAudioContextSuspended)) {
        TriggerSyncBreakpoint(*data);
    }
}
...
Response InspectorEventBreakpointsAgent::setInstrumentationBreakpoint(const String& event_name)
{
    if (event_name.empty()) {
...
Response InspectorEventBreakpointsAgent::removeInstrumentationBreakpoint(const String& event_name)
{
    if (event_name.empty()) {
        return protocol::Response::InvalidParams("Event name is empty");
    }
...
std::unique_ptr<protocol::DictionaryValue> InspectorEventBreakpointsAgent::MaybeBuildBreakpointData(const String& event_name)
{
    ...
    const String full_event_name = String(kInstrumentationEventCategoryType) + event_name;
...
void InspectorEventBreakpointsAgent::TriggerSyncBreakpoint(const protocol::DictionaryValue& breakpoint_data)
{
    std::vector<uint8_t> json = JsonFromDictionary(breakpoint_data);
    v8_session_->breakProgram(
        ToV8InspectorStringView(v8_inspector::protocol::Debugger::API::Paused::ReasonEnum::EventListener), v8_inspector::StringView(json.data(), json.size()));
}

void InspectorEventBreakpointsAgent::ScheduleAsyncBreakpoint(const protocol::DictionaryValue& breakpoint_data)
{
    std::vector<uint8_t> json = JsonFromDictionary(breakpoint_data);
