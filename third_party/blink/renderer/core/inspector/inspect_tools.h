...
public:
    SearchingForNodeTool(
        InspectorOverlayAgent* overlay, OverlayFrontend* frontend, InspectorDOMAgent* dom_agent, bool ua_shadow, const std::vector<uint8_t>& highlight_config);
...
private:
    bool HandleInputEvent(LocalFrameView* frame_view, const WebInputEvent& input_event, bool* swallow_next_mouse_up) override;
    bool HandleMouseDown(const WebMouseEvent& event, bool* swallow_next_mouse_up) override;
    ...
    NodeContentVisibilityState content_visibility_state_ = NodeContentVisibilityState::kNone;
...
public:
    QuadHighlightTool(InspectorOverlayAgent* overlay, OverlayFrontend* frontend, std::unique_ptr<gfx::QuadF> quad, Color color, Color outline_color);
...
public:
    NodeHighlightTool(InspectorOverlayAgent* overlay, OverlayFrontend* frontend, Member<Node> node, String selector_list,
        std::unique_ptr<InspectorHighlightConfig> highlight_config);
    NodeHighlightTool(const NodeHighlightTool&) = delete;
    NodeHighlightTool& operator=(const NodeHighlightTool&) = delete;

    std::unique_ptr<protocol::DictionaryValue> GetNodeInspectorHighlightAsJson(bool append_element_info, bool append_distance_info) const;
...
public:
    SourceOrderTool(InspectorOverlayAgent* overlay, OverlayFrontend* frontend, Node* node, std::unique_ptr<InspectorSourceOrderConfig> source_order_config);
    SourceOrderTool(const SourceOrderTool&) = delete;
    SourceOrderTool& operator=(const SourceOrderTool&) = delete;
    std::unique_ptr<protocol::DictionaryValue> GetNodeInspectorSourceOrderHighlightAsJson() const;
...
// -----------------------------------------------------------------------------
using GridConfigs = HeapHashMap<WeakMember<Node>, std::unique_ptr<InspectorGridHighlightConfig>>;
using FlexContainerConfigs = HeapHashMap<WeakMember<Node>, std::unique_ptr<InspectorFlexContainerHighlightConfig>>;
using ScrollSnapConfigs = HeapHashMap<WeakMember<Node>, std::unique_ptr<InspectorScrollSnapContainerHighlightConfig>>;
using ContainerQueryConfigs = HeapHashMap<WeakMember<Node>, std::unique_ptr<InspectorContainerQueryContainerHighlightConfig>>;
using IsolatedElementConfigs = HeapHashMap<WeakMember<Element>, std::unique_ptr<InspectorIsolationModeHighlightConfig>>;
...
public:
    ...
    std::unique_ptr<protocol::DictionaryValue> GetGridInspectorHighlightsAsJson() const;
...
public:
    PausedInDebuggerTool(InspectorOverlayAgent* overlay, OverlayFrontend* frontend, v8_inspector::V8InspectorSession* v8_session, const String& message)
...
public:
    WindowControlsOverlayTool(InspectorOverlayAgent* overlay, OverlayFrontend* frontend, std::unique_ptr<protocol::DictionaryValue> wco_config);
    WindowControlsOverlayTool(const WindowControlsOverlayTool&) = delete;
    WindowControlsOverlayTool& operator=(const WindowControlsOverlayTool&) = delete;
