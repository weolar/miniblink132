...
void BlendWithColorsFromGradient(
    cssvalue::CSSGradientValue* gradient, Vector<Color>& colors, bool& found_non_transparent_color, bool& found_opaque_color, const LayoutObject& layout_object)
{
    ...
    if (colors.empty()) {
        ...
        for (auto stop_color : stop_colors) {
            found_non_transparent_color = found_non_transparent_color || !stop_color.IsFullyTransparent();
            colors.push_back(existing_color.Blend(stop_color));
        }
    }
    found_opaque_color = found_opaque_color || gradient->KnownToBeOpaque(document, style);
}

// Gets the colors from an image style, if one exists and it is a gradient.
void AddColorsFromImageStyle(
    const ComputedStyle& style, const LayoutObject& layout_object, Vector<Color>& colors, bool& found_opaque_color, bool& found_non_transparent_color)
{
    ...
    if (auto* gradient = DynamicTo<cssvalue::CSSGradientValue>(image_css)) {
        BlendWithColorsFromGradient(gradient, colors, found_non_transparent_color, found_opaque_color, layout_object);
    }
}
...
InspectorContrast::InspectorContrast(Document* document)
{
    if (!document->IsInMainFrame()) {
        ...
        // for all frames.
        for (HTMLFrameOwnerElement* owner_element = document->LocalOwner(); owner_element; owner_element = owner_element->GetDocument().LocalOwner()) {
            document = &owner_element->GetDocument();
        }
    }
...
{
    TRACE_EVENT0("devtools.contrast", "InspectorContrast::CollectNodesAndBuildRTreeIfNeeded");
    ...
    if (!layout_view->GetFrameView()->UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason::kInspector)) {
        return;
    }

    InspectorDOMAgent::CollectNodes(
        document_, INT_MAX, true, InspectorDOMAgent::IncludeWhitespaceEnum::NONE, WTF::BindRepeating(&NodeIsElementWithLayoutObject), &elements_);
    SortElementsByPaintOrder(elements_, document_);
    rtree_.Build(
        elements_.size(), [this](size_t index) { return ToPixelSnappedRect(GetNodeRect(elements_[static_cast<wtf_size_t>(index)])); },
        [this](size_t index) { return elements_[static_cast<wtf_size_t>(index)]; });

    rtree_built_ = true;
}

std::vector<ContrastInfo> InspectorContrast::GetElementsWithContrastIssues(bool report_aaa, size_t max_elements = 0)
{
    TRACE_EVENT0("devtools.contrast", "InspectorContrast::GetElementsWithContrastIssues");
    ...
    for (Node* node : elements_) {
        ...
        if (info.able_to_compute_contrast && ((info.contrast_ratio < info.threshold_aa) || (info.contrast_ratio < info.threshold_aaa && report_aaa))) {
...
static bool IsLargeFont(const TextInfo& text_info)
{
    ...
    bool is_bold
        = font_weight == "bold" || font_weight == "bolder" || font_weight == "600" || font_weight == "700" || font_weight == "800" || font_weight == "900";
    if (is_bold) {
...
ContrastInfo InspectorContrast::GetContrast(Element* top_element)
{
    ...
    const CSSValue* text_color_value = ComputedStyleUtils::ComputedPropertyValue(CSSProperty::Get(CSSPropertyID::kColor), layout_object->StyleRef());
    ...
    Color text_color = static_cast<const cssvalue::CSSColor*>(text_color_value)->Value();

    text_color.SetAlpha(text_opacity * text_color.Alpha());

    float contrast_ratio = color_utils::GetContrastRatio(bgcolors.at(0).Blend(text_color).toSkColor4f(), bgcolors.at(0).toSkColor4f());
...
TextInfo InspectorContrast::GetTextInfo(Element* element)
{
    ...
    const CSSValue* font_size = computed_style_info->GetPropertyCSSValue(CSSPropertyID::kFontSize);
    if (font_size)
        info.font_size = font_size->CssText();
    const CSSValue* font_weight = computed_style_info->GetPropertyCSSValue(CSSPropertyID::kFontWeight);
    if (font_weight)
        info.font_weight = font_weight->CssText();
    return info;
}

Vector<Color> InspectorContrast::GetBackgroundColors(Element* element, float* text_opacity)
{
    Vector<Color> colors;
    ...
    GetColorsFromRect(content_bounds, text_node->GetDocument(), element, colors, text_opacity);

    return colors;
}

// Get the elements which overlap the given rectangle.
HeapVector<Member<Node>> InspectorContrast::ElementsFromRect(const PhysicalRect& rect, Document& document)
{
    CollectNodesAndBuildRTreeIfNeeded();
    HeapVector<Member<Node>> overlapping_elements;
    rtree_.Search(
        ToPixelSnappedRect(rect), [&overlapping_elements](const Member<Node>& payload, const gfx::Rect& rect) { overlapping_elements.push_back(payload); });
    return overlapping_elements;
}

bool InspectorContrast::GetColorsFromRect(PhysicalRect rect, Document& document, Element* top_element, Vector<Color>& colors, float* text_opacity)
{
    HeapVector<Member<Node>> elements_under_rect = ElementsFromRect(rect, document);
    ...
    *text_opacity = 1.0f;
    ...
    for (const Member<Node>& node : elements_under_rect) {
        ...
        if (IsA<HTMLCanvasElement>(element) || IsA<HTMLEmbedElement>(element) || IsA<HTMLImageElement>(element) || IsA<HTMLObjectElement>(element)
            || IsA<HTMLPictureElement>(element) || element->IsSVGElement() || IsA<HTMLVideoElement>(element)) {
            colors.clear();
            found_opaque_color = false;
            continue;
        }
        ...
        if (style->HasOpacity()) {
            ...
            if (element == top_element || FlatTreeTraversal::IsDescendantOf(*top_element, *element)) {
        ...
        AddColorsFromImageStyle(*style, *layout_object, colors, found_opaque_color, found_non_transparent_color);
...
void InspectorContrast::SortElementsByPaintOrder(HeapVector<Member<Node>>& unsorted_elements, Document* document)
{
    InspectorDOMSnapshotAgent::PaintOrderMap* paint_layer_tree = InspectorDOMSnapshotAgent::BuildPaintLayerTree(document);

    std::stable_sort(unsorted_elements.begin(), unsorted_elements.end(), [&paint_layer_tree = paint_layer_tree](Node* a, Node* b) {
