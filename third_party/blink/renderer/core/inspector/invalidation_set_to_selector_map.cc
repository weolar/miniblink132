...
void InvalidationSetToSelectorMap::IndexedSelector::Trace(Visitor* visitor) const
{
...
// static
void InvalidationSetToSelectorMap::StartOrStopTrackingIfNeeded(StyleEngine& style_engine)
{
...
void InvalidationSetToSelectorMap::BeginSelector(StyleRule* style_rule, unsigned selector_index)
{
    ...
    instance->current_selector_ = MakeGarbageCollected<IndexedSelector>(style_rule, selector_index);
}
...
// static
void InvalidationSetToSelectorMap::RecordInvalidationSetEntry(const InvalidationSet* invalidation_set, SelectorFeatureType type, const AtomicString& value)
{
    InvalidationSetToSelectorMap* instance = GetInstanceReference().Get();
    if (instance == nullptr) {
        return;
    ...
    InvalidationSetEntryMap* entry_map
        = instance->invalidation_set_map_->insert(invalidation_set, MakeGarbageCollected<InvalidationSetEntryMap>()).stored_value->value.Get();
    IndexedSelectorList* indexed_selector_list
        = entry_map->insert(InvalidationSetEntry(type, value), MakeGarbageCollected<IndexedSelectorList>()).stored_value->value.Get();
    indexed_selector_list->insert(instance->current_selector_);
}

// static
void InvalidationSetToSelectorMap::BeginInvalidationSetCombine(const InvalidationSet* target, const InvalidationSet* source)
{
    InvalidationSetToSelectorMap* instance = GetInstanceReference().Get();
    if (instance == nullptr) {
        return;
    ...
    if (instance->invalidation_set_map_->Contains(source)) {
        InvalidationSetEntryMap* target_entry_map
            = instance->invalidation_set_map_->insert(target, MakeGarbageCollected<InvalidationSetEntryMap>()).stored_value->value.Get();
        ...
        for (auto source_selector_list_it : *(source_entry_it->value)) {
            IndexedSelectorList* target_selector_list
                = target_entry_map->insert(source_selector_list_it.key, MakeGarbageCollected<IndexedSelectorList>()).stored_value->value.Get();
...
// static
void InvalidationSetToSelectorMap::RemoveEntriesForInvalidationSet(const InvalidationSet* invalidation_set)
{
...
const InvalidationSetToSelectorMap::IndexedSelectorList* InvalidationSetToSelectorMap::Lookup(
    const InvalidationSet* invalidation_set, SelectorFeatureType type, const AtomicString& value)
{
    const InvalidationSetToSelectorMap* instance = GetInstanceReference().Get();
    if (instance == nullptr) {
        return nullptr;
    ...
    if (entry_it != instance->invalidation_set_map_->end()) {
        auto selector_list_it = entry_it->value->find(InvalidationSetEntry(type, value));
...
// static
Persistent<InvalidationSetToSelectorMap>& InvalidationSetToSelectorMap::GetInstanceReference()
{
