...
private:
    ...
    public:
        explicit FurthestReaching(wtf_size_t size)
            : v_(size)
        {
        }
    ...
    {
        return FindEditPath(Point { 0, 0 }, Point { input_->GetLength1(), input_->GetLength2() });
    }
    ...
    class ResultWriter {
    public:
        explicit ResultWriter(InspectorDiff::Output* output)
            : output_(output)
        {
        }
...
public:
    static void MyersDiff(InspectorDiff::Input* input, InspectorDiff::Output* output)
    {
...
public:
    MappingInput(const Vector<String>& list_a, const Vector<String>& list_b, int start_offset, int end_offset)
        : list_a_(list_a)
        , list_b_(list_b)
    ...
    {
        return list_a_.at(index1 + start_offset_) == list_b_.at(index2 + start_offset_);
    }
...
public:
    MappingOutput(int start_offset, InspectorIndexMap* a_to_b, InspectorIndexMap* b_to_a)
        : a_to_b_(a_to_b)
        , b_to_a_(b_to_a)
        , start_offset_(start_offset)
    {
...
void InspectorDiff::CalculateMatches(InspectorDiff::Input* input, InspectorDiff::Output* result_writer)
{
...
void InspectorDiff::FindLCSMapping(const Vector<String>& list_a, const Vector<String>& list_b, InspectorIndexMap* a_to_b, InspectorIndexMap* b_to_a)
{
    ...
    while (end_offset < list_a.size() - start_offset && end_offset < list_b.size() - start_offset) {
