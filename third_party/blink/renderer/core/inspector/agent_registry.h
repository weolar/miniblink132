...
// Copy-On-Write.
template <class AgentType> class CORE_EXPORT AgentRegistry {
    DISALLOW_NEW();
...
public:
    ...
    //       agent->SomeMethod();
    //     });
    template <typename ForEachCallable> void ForEachAgent(const ForEachCallable& callable) const
    {
        iteration_counter_++;
