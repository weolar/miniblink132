...
// Exposed for tests.
CORE_EXPORT std::unique_ptr<protocol::Preload::RuleSet> BuildProtocolRuleSet(const SpeculationRuleSet& rule_set, const String& loader_id);

} // namespace internal

class CORE_EXPORT InspectorPreloadAgent final : public InspectorBaseAgent<protocol::Preload::Metainfo> {
public:
    ...
    // Probes
    void DidAddSpeculationRuleSet(Document& document, const SpeculationRuleSet& rule_set);
    void DidRemoveSpeculationRuleSet(const SpeculationRuleSet& rule_set);
    void SpeculationCandidatesUpdated(Document& document, const HeapVector<Member<SpeculationCandidate>>& candidates);
