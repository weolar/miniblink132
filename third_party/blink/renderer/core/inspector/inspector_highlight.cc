...
public:
    PathBuilder()
        : path_(protocol::ListValue::create())
    {
    }
    ...
    std::unique_ptr<protocol::ListValue> Release()
    {
...
protected:
    virtual gfx::PointF TranslatePoint(const gfx::PointF& point)
    {
        return point;
    }

private:
    static void AppendPathElement(void* path_builder, const PathElement& path_element)
    {
        static_cast<PathBuilder*>(path_builder)->AppendPathElement(path_element);
    }

    void AppendPathElement(const PathElement&);
    void AppendPathCommandAndPoints(const char* command, base::span<const gfx::PointF> points);

    std::unique_ptr<protocol::ListValue> path_;
};

void PathBuilder::AppendPathCommandAndPoints(const char* command, base::span<const gfx::PointF> points)
{
...
public:
    ShapePathBuilder(LocalFrameView& view, LayoutObject& layout_object, const ShapeOutsideInfo& shape_outside_info)
        : view_(&view)
    ...
    static std::unique_ptr<protocol::ListValue> BuildPath(
        LocalFrameView& view, LayoutObject& layout_object, const ShapeOutsideInfo& shape_outside_info, const Path& path, float scale)
    {
...
protected:
    ...
    {
        PhysicalOffset layout_object_point = PhysicalOffset::FromPointFRound(shape_outside_info_.ShapeToLayoutObjectPoint(point));
        // TODO(pfeldman): Is this kIgnoreTransforms correct?
        return gfx::PointF(view_->FrameToViewport(ToRoundedPoint(layout_object_->LocalToAbsolutePoint(layout_object_point, kIgnoreTransforms))));
    }
...
std::unique_ptr<protocol::Array<double>> BuildArrayForQuad(const gfx::QuadF& quad)
{
    return std::make_unique<std::vector<double>, std::initializer_list<double>>(
        { quad.p1().x(), quad.p1().y(), quad.p2().x(), quad.p2().y(), quad.p3().x(), quad.p3().y(), quad.p4().x(), quad.p4().y() });
}
...
gfx::PointF FramePointToViewport(const LocalFrameView* view, gfx::PointF point_in_frame)
{
    gfx::PointF point_in_root_frame = view->ConvertToRootFrame(point_in_frame);
    return view->GetPage()->GetVisualViewport().RootFrameToViewport(point_in_root_frame);
}
...
{
    return 1.f / frame_view->GetChromeClient()->WindowToViewportScalar(&frame_view->GetFrame(), 1.f);
}
...
const ShapeOutsideInfo* ShapeOutsideInfoForNode(Node* node, Shape::DisplayPaths* paths, gfx::QuadF* bounds)
{
    LayoutObject* layout_object = node->GetLayoutObject();
    if (!layout_object || !layout_object->IsBox() || !To<LayoutBox>(layout_object)->GetShapeOutsideInfo())
        return nullptr;

    LocalFrameView* containing_view = node->GetDocument().View();
    auto* layout_box = To<LayoutBox>(layout_object);
    const ShapeOutsideInfo* shape_outside_info = layout_box->GetShapeOutsideInfo();

    shape_outside_info->ComputedShape().BuildDisplayPaths(*paths);

    PhysicalRect shape_bounds = shape_outside_info->ComputedShapePhysicalBoundingBox();
...
{
    return String::Format("#%02X%02X%02X%02X", color.Red(), color.Green(), color.Blue(), color.AlphaAsInteger());
}
...
void AppendStyleInfo(
    Element* element, protocol::DictionaryValue* element_info, const InspectorHighlightContrastInfo& node_contrast, const ContrastAlgorithm& contrast_algorithm)
{
    std::unique_ptr<protocol::DictionaryValue> computed_style = protocol::DictionaryValue::create();
    CSSComputedStyleDeclaration* style = MakeGarbageCollected<CSSComputedStyleDeclaration>(element, true);
    Vector<CSSPropertyID> properties;

    // For text nodes, we can show color & font properties.
    bool has_text_children = false;
    for (Node* child = element->firstChild(); !has_text_children && child; child = child->nextSibling()) {
    ...
    if (!node_contrast.font_size.empty()) {
        std::unique_ptr<protocol::DictionaryValue> contrast = protocol::DictionaryValue::create();
        contrast->setString("fontSize", node_contrast.font_size);
        contrast->setString("fontWeight", node_contrast.font_weight);
        contrast->setString("backgroundColor", ToHEXA(node_contrast.background_color));
        contrast->setArray("backgroundColorUnclampedRgba", ToRGBAList(node_contrast.background_color));
        contrast->setString("backgroundColorCssText", node_contrast.background_color.SerializeAsCSSColor());
        contrast->setString("contrastAlgorithm", ContrastAlgorithmToString(contrast_algorithm));
...
{
    std::unique_ptr<protocol::DictionaryValue> element_info = protocol::DictionaryValue::create();
    ...
    element_info->setString("tagName", is_xhtml ? real_element->nodeName() : real_element->nodeName().DeprecatedLower());
    ...
    if (pseudo_element) {
        ...
        } else if (pseudo_element->GetPseudoIdForStyling() == kPseudoIdScrollMarkerGroup) {
    ...
    DCHECK(element->GetDocument().Lifecycle().GetState() >= DocumentLifecycle::kLayoutClean);
    ...
    element_info->setBoolean("isKeyboardFocusable", element->IsKeyboardFocusable());
    element_info->setString("accessibleName", element->ComputedNameNoLifecycleUpdate());
    element_info->setString("accessibleRole", element->ComputedRoleNoLifecycleUpdate());
...
{
    std::unique_ptr<protocol::DictionaryValue> text_info = protocol::DictionaryValue::create();
    LayoutObject* layout_object = text_node->GetLayoutObject();
    if (!layout_object || !layout_object->IsText())
        return text_info;
    PhysicalRect bounding_box = To<LayoutText>(layout_object)->VisualOverflowRect();
...
void AppendLineStyleConfig(const std::optional<LineStyle>& line_style, std::unique_ptr<protocol::DictionaryValue>& parent_config, String line_name)
{
    if (!line_style || line_style->IsFullyTransparent()) {
        return;
    }

    std::unique_ptr<protocol::DictionaryValue> config = protocol::DictionaryValue::create();
...
void AppendBoxStyleConfig(const std::optional<BoxStyle>& box_style, std::unique_ptr<protocol::DictionaryValue>& parent_config, String box_name)
{
...
std::unique_ptr<protocol::DictionaryValue> BuildFlexContainerHighlightConfigInfo(const InspectorFlexContainerHighlightConfig& flex_config)
{
    std::unique_ptr<protocol::DictionaryValue> flex_config_info = protocol::DictionaryValue::create();

    AppendLineStyleConfig(flex_config.container_border, flex_config_info, "containerBorder");
    AppendLineStyleConfig(flex_config.line_separator, flex_config_info, "lineSeparator");
    AppendLineStyleConfig(flex_config.item_separator, flex_config_info, "itemSeparator");

    AppendBoxStyleConfig(flex_config.main_distributed_space, flex_config_info, "mainDistributedSpace");
    AppendBoxStyleConfig(flex_config.cross_distributed_space, flex_config_info, "crossDistributedSpace");
    AppendBoxStyleConfig(flex_config.row_gap_space, flex_config_info, "rowGapSpace");
    AppendBoxStyleConfig(flex_config.column_gap_space, flex_config_info, "columnGapSpace");
    AppendLineStyleConfig(flex_config.cross_alignment, flex_config_info, "crossAlignment");

    return flex_config_info;
}

std::unique_ptr<protocol::DictionaryValue> BuildFlexItemHighlightConfigInfo(const InspectorFlexItemHighlightConfig& flex_config)
{
    std::unique_ptr<protocol::DictionaryValue> flex_config_info = protocol::DictionaryValue::create();

    AppendBoxStyleConfig(flex_config.base_size_box, flex_config_info, "baseSizeBox");
    AppendLineStyleConfig(flex_config.base_size_border, flex_config_info, "baseSizeBorder");
    AppendLineStyleConfig(flex_config.flexibility_arrow, flex_config_info, "flexibilityArrow");

    return flex_config_info;
}

std::unique_ptr<protocol::DictionaryValue> BuildGridHighlightConfigInfo(const InspectorGridHighlightConfig& grid_config)
{
    std::unique_ptr<protocol::DictionaryValue> grid_config_info = protocol::DictionaryValue::create();
    ...
    grid_config_info->setBoolean("showGridExtensionLines", grid_config.show_grid_extension_lines);
    grid_config_info->setBoolean("showPositiveLineNumbers", grid_config.show_positive_line_numbers);
    grid_config_info->setBoolean("showNegativeLineNumbers", grid_config.show_negative_line_numbers);
    ...
    if (grid_config.grid_color != Color::kTransparent) {
        grid_config_info->setString("gridBorderColor", grid_config.grid_color.SerializeAsCSSColor());
    }
    if (grid_config.row_line_color != Color::kTransparent) {
        grid_config_info->setString("rowLineColor", grid_config.row_line_color.SerializeAsCSSColor());
    }
    if (grid_config.column_line_color != Color::kTransparent) {
        grid_config_info->setString("columnLineColor", grid_config.column_line_color.SerializeAsCSSColor());
    }
    if (grid_config.row_gap_color != Color::kTransparent) {
        grid_config_info->setString("rowGapColor", grid_config.row_gap_color.SerializeAsCSSColor());
    }
    if (grid_config.column_gap_color != Color::kTransparent) {
        grid_config_info->setString("columnGapColor", grid_config.column_gap_color.SerializeAsCSSColor());
    }
    if (grid_config.row_hatch_color != Color::kTransparent) {
        grid_config_info->setString("rowHatchColor", grid_config.row_hatch_color.SerializeAsCSSColor());
    }
    if (grid_config.column_hatch_color != Color::kTransparent) {
        grid_config_info->setString("columnHatchColor", grid_config.column_hatch_color.SerializeAsCSSColor());
    }
    if (grid_config.area_border_color != Color::kTransparent) {
        grid_config_info->setString("areaBorderColor", grid_config.area_border_color.SerializeAsCSSColor());
    }
    if (grid_config.grid_background_color != Color::kTransparent) {
        grid_config_info->setString("gridBackgroundColor", grid_config.grid_background_color.SerializeAsCSSColor());
    }
    return grid_config_info;
}

std::unique_ptr<protocol::DictionaryValue> BuildContainerQueryContainerHighlightConfigInfo(
    const InspectorContainerQueryContainerHighlightConfig& container_config)
{
    std::unique_ptr<protocol::DictionaryValue> container_config_info = protocol::DictionaryValue::create();

    AppendLineStyleConfig(container_config.container_border, container_config_info, "containerBorder");
    AppendLineStyleConfig(container_config.descendant_border, container_config_info, "descendantBorder");

    return container_config_info;
}

std::unique_ptr<protocol::DictionaryValue> BuildIsolationModeHighlightConfigInfo(const InspectorIsolationModeHighlightConfig& config)
{
    std::unique_ptr<protocol::DictionaryValue> config_info = protocol::DictionaryValue::create();

    config_info->setString("resizerColor", config.resizer_color.SerializeAsCSSColor());
    config_info->setString("resizerHandleColor", config.resizer_handle_color.SerializeAsCSSColor());
...
LayoutUnit TranslateRTLCoordinate(const LayoutObject* layout_object, LayoutUnit position, const Vector<LayoutUnit>& column_positions)
{
...
LayoutUnit GetPositionForTrackAt(const LayoutObject* layout_object, wtf_size_t index, GridTrackSizingDirection direction, const Vector<LayoutUnit>& positions)
{
    if (direction == kForRows)
        return positions.at(index);

    LayoutUnit position = positions.at(index);
    return layout_object->StyleRef().IsLeftToRightDirection() ? position : TranslateRTLCoordinate(layout_object, position, positions);
}

LayoutUnit GetPositionForFirstTrack(const LayoutObject* layout_object, GridTrackSizingDirection direction, const Vector<LayoutUnit>& positions)
{
    return GetPositionForTrackAt(layout_object, 0, direction, positions);
}

LayoutUnit GetPositionForLastTrack(const LayoutObject* layout_object, GridTrackSizingDirection direction, const Vector<LayoutUnit>& positions)
{
    wtf_size_t index = positions.size() - 1;
    return GetPositionForTrackAt(layout_object, index, direction, positions);
}

PhysicalOffset LocalToAbsolutePoint(Node* node, PhysicalOffset local, float scale)
{
    LayoutObject* layout_object = node->GetLayoutObject();
    PhysicalOffset abs_point = layout_object->LocalToAbsolutePoint(local);
    gfx::PointF abs_point_in_viewport = FramePointToViewport(node->GetDocument().View(), gfx::PointF(abs_point.left, abs_point.top));
    PhysicalOffset scaled_abs_point = PhysicalOffset::FromPointFRound(abs_point_in_viewport);
...
std::unique_ptr<protocol::ListValue> BuildPathFromQuad(const blink::LocalFrameView* containing_view, gfx::QuadF quad)
{
    FrameQuadToViewport(containing_view, quad);
    PathBuilder builder;
    builder.AppendPath(QuadToPath(quad), DeviceScaleFromFrameView(containing_view));
    return builder.Release();
}

void BuildSnapAlignment(const cc::ScrollSnapType& snap_type, const cc::SnapAlignment& alignment_block, const cc::SnapAlignment& alignment_inline,
    std::unique_ptr<protocol::DictionaryValue>& result)
{
    if (snap_type.axis == cc::SnapAxis::kBlock || snap_type.axis == cc::SnapAxis::kBoth || snap_type.axis == cc::SnapAxis::kY) {
        result->setString("alignBlock", SnapAlignToString(alignment_block));
    }
    if (snap_type.axis == cc::SnapAxis::kInline || snap_type.axis == cc::SnapAxis::kBoth || snap_type.axis == cc::SnapAxis::kX) {
        result->setString("alignInline", SnapAlignToString(alignment_inline));
    }
}

std::unique_ptr<protocol::DictionaryValue> BuildPosition(PhysicalOffset position)
{
...
std::unique_ptr<protocol::ListValue> BuildGridTrackSizes(Node* node, GridTrackSizingDirection direction, float scale, LayoutUnit gap, LayoutUnit rtl_offset,
    const Vector<LayoutUnit>& positions, const Vector<LayoutUnit>& alt_axis_positions, const Vector<String>* authored_values)
{
    LayoutObject* layout_object = node->GetLayoutObject();
    ...
    LayoutUnit alt_axis_pos = GetPositionForFirstTrack(layout_object, direction == kForRows ? kForColumns : kForRows, alt_axis_positions);
    if (is_rtl && direction == kForRows)
        alt_axis_pos += rtl_offset;

    for (wtf_size_t i = 1; i < track_count; i++) {
        LayoutUnit current_position = GetPositionForTrackAt(layout_object, i, direction, positions);
        LayoutUnit prev_position = GetPositionForTrackAt(layout_object, i - 1, direction, positions);
        ...
        auto adjusted_size = AdjustForAbsoluteZoom::AdjustFloat(width * scale, layout_object->StyleRef());
        PhysicalOffset track_size_pos(main_axis_pos, alt_axis_pos);
        if (direction == kForRows)
            track_size_pos = Transpose(track_size_pos);
        std::unique_ptr<protocol::DictionaryValue> size_info = BuildPosition(LocalToAbsolutePoint(node, track_size_pos, scale));
...
std::unique_ptr<protocol::ListValue> BuildGridPositiveLineNumberPositions(Node* node, const LayoutUnit& grid_gap, GridTrackSizingDirection direction,
    float scale, LayoutUnit rtl_offset, const Vector<LayoutUnit>& positions, const Vector<LayoutUnit>& alt_axis_positions)
{
    auto* grid = To<LayoutGrid>(node->GetLayoutObject());
    bool is_rtl = !grid->StyleRef().IsLeftToRightDirection();

    std::unique_ptr<protocol::ListValue> number_positions = protocol::ListValue::create();

    wtf_size_t track_count = positions.size();
    LayoutUnit alt_axis_pos = GetPositionForFirstTrack(grid, direction == kForRows ? kForColumns : kForRows, alt_axis_positions);

    if (is_rtl && direction == kForRows)
        alt_axis_pos += rtl_offset;

    // Find index of the first explicit Grid Line.
    wtf_size_t first_explicit_index = grid->ExplicitGridStartForDirection(direction);
    ...
    for (wtf_size_t i = first_explicit_index; i < track_count; ++i) {
        ...
        number_positions->pushValue(BuildPosition(LocalToAbsolutePoint(node, number_position, scale)));
    }

    return number_positions;
}

std::unique_ptr<protocol::ListValue> BuildGridNegativeLineNumberPositions(Node* node, const LayoutUnit& grid_gap, GridTrackSizingDirection direction,
    float scale, LayoutUnit rtl_offset, const Vector<LayoutUnit>& positions, const Vector<LayoutUnit>& alt_axis_positions)
{
    auto* grid = To<LayoutGrid>(node->GetLayoutObject());
    bool is_rtl = !grid->StyleRef().IsLeftToRightDirection();
    ...
    wtf_size_t track_count = positions.size();
    LayoutUnit alt_axis_pos = GetPositionForLastTrack(grid, direction == kForRows ? kForColumns : kForRows, alt_axis_positions);
    if (is_rtl && direction == kForRows)
        alt_axis_pos += rtl_offset;
    ...
    size_t explicit_grid_end_track_count = grid->ExplicitGridEndForDirection(direction);

    {
        LayoutUnit first_offset = GetPositionForFirstTrack(grid, direction, positions);
        if (is_rtl && direction == kForColumns)
            first_offset += rtl_offset;

        // Always start negative numbers at the first line.
        std::unique_ptr<protocol::DictionaryValue> pos = protocol::DictionaryValue::create();
    ...
    for (wtf_size_t i = 1; i <= explicit_grid_end_track_count; i++) {
        ...
        if (is_rtl && direction == kForColumns)
            gapOffset *= -1;
        if (grid_gap == 0 || (i == explicit_grid_end_track_count && i == track_count - 1)) {
            gapOffset = LayoutUnit();
        }
        LayoutUnit offset = GetPositionForTrackAt(grid, i, direction, positions);
        if (is_rtl && direction == kForColumns)
...
{
    return layout_object.StyleRef().IsDisplayFlexibleBox() && layout_object.IsFlexibleBox();
}

bool IsLayoutNGFlexItem(const LayoutObject& layout_object)
{
    return !layout_object.GetNode()->IsDocumentNode() && IsLayoutNGFlexibleBox(*layout_object.Parent()) && To<LayoutBox>(layout_object).IsFlexItem();
}

std::unique_ptr<protocol::DictionaryValue> BuildAreaNamePaths(Node* node, float scale, const Vector<LayoutUnit>& rows, const Vector<LayoutUnit>& columns)
{
    const auto* grid = To<LayoutGrid>(node->GetLayoutObject());
    LocalFrameView* containing_view = node->GetDocument().View();
    bool is_rtl = !grid->StyleRef().IsLeftToRightDirection();

    std::unique_ptr<protocol::DictionaryValue> area_paths = protocol::DictionaryValue::create();
    ...
    if (const NamedGridAreaMap* named_area_map = grid->CachedPlacementData().line_resolver.NamedAreasMap()) {
        for (const auto& item : *named_area_map) {
            const GridArea& area = item.value;
            const String& name = item.key;

            const auto start_column = GetPositionForTrackAt(grid, area.columns.StartLine(), kForColumns, columns);
            const auto end_column = GetPositionForTrackAt(grid, area.columns.EndLine(), kForColumns, columns);
            const auto start_row = GetPositionForTrackAt(grid, area.rows.StartLine(), kForRows, rows);
            const auto end_row = GetPositionForTrackAt(grid, area.rows.EndLine(), kForRows, rows);

            // Only subtract the gap size if the end line isn't the last line in the
            // container.
            const auto row_gap_offset = (area.rows.EndLine() == rows.size() - 1) ? LayoutUnit() : row_gap;
            auto column_gap_offset = (area.columns.EndLine() == columns.size() - 1) ? LayoutUnit() : column_gap;
            if (is_rtl) {
                column_gap_offset = -column_gap_offset;
            }

            PhysicalOffset position(start_column, start_row);
            PhysicalSize size(end_column - start_column - column_gap_offset, end_row - start_row - row_gap_offset);
...
std::unique_ptr<protocol::ListValue> BuildGridLineNames(
    Node* node, GridTrackSizingDirection direction, float scale, const Vector<LayoutUnit>& positions, const Vector<LayoutUnit>& alt_axis_positions)
{
    auto* grid = To<LayoutGrid>(node->GetLayoutObject());
    const ComputedStyle& grid_container_style = grid->StyleRef();
    bool is_rtl = direction == kForColumns && !grid_container_style.IsLeftToRightDirection();
    ...
    auto process_grid_lines_map = [&](const NamedGridLinesMap& named_lines_map) {
        for (const auto& item : named_lines_map) {
            ...
            for (const wtf_size_t index : item.value) {
                LayoutUnit track = GetPositionForTrackAt(grid, index, direction, positions);

                LayoutUnit gap_offset = index > 0 && index < positions.size() - 1 ? gap / 2 : LayoutUnit();
                if (is_rtl)
                    gap_offset *= -1;
                ...
                std::unique_ptr<protocol::DictionaryValue> line = BuildPosition(LocalToAbsolutePoint(node, line_name_pos, scale));
    ...
    const NamedGridLinesMap& explicit_lines_map = grid->CachedPlacementData().line_resolver.ExplicitNamedLinesMap(direction);
    process_grid_lines_map(explicit_lines_map);
    const NamedGridLinesMap& implicit_lines_map = grid->CachedPlacementData().line_resolver.ImplicitNamedLinesMap(direction);
...
Vector<String> GetAuthoredGridTrackSizes(const CSSValue* value, size_t auto_repeat_count)
{
    ...
    for (auto list_value : *value_list) {
        ...
        if (auto* repeated_values = DynamicTo<cssvalue::CSSGridIntegerRepeatValue>(list_value.Get())) {
...
{
    return layout_flex->StyleRef().IsHorizontalWritingMode() != layout_flex->StyleRef().ResolvedIsColumnFlexDirection();
}

DevtoolsFlexInfo GetFlexLinesAndItems(LayoutBox* layout_box, bool is_horizontal, bool is_reverse)
{
    if (auto* layout_ng_flex = DynamicTo<LayoutFlexibleBox>(layout_box)) {
        const DevtoolsFlexInfo* flex_info_from_layout = layout_ng_flex->FlexLayoutData();
    ...
    for (const auto& fragment : layout_box->PhysicalFragments()) {
        ...
        for (const auto& child : fragment.Children()) {
            ...
            LayoutUnit baseline = LogicalBoxFragment(layout_box->StyleRef().GetWritingDirection(), *To<PhysicalBoxFragment>(child_fragment))
                                      .FirstBaselineOrSynthesize(layout_box->StyleRef().GetFontBaseline());
            float adjusted_baseline = AdjustForAbsoluteZoom::AdjustFloat(baseline + box->MarginTop(), box->StyleRef());

            PhysicalRect item_rect = PhysicalRect(fragment_offset.left - box->MarginLeft(), fragment_offset.top - box->MarginTop(),
                fragment_size.width + box->MarginWidth(), fragment_size.height + box->MarginHeight());

            LayoutUnit item_start = is_horizontal ? item_rect.X() : item_rect.Y();
            LayoutUnit item_end = is_horizontal ? item_rect.X() + item_rect.Width() : item_rect.Y() + item_rect.Height();

            if (flex_lines.empty() || (is_reverse ? item_end > progression : item_start < progression)) {
                flex_lines.emplace_back();
            }

            flex_lines.back().items.push_back(DevtoolsFlexInfo::Item(item_rect, LayoutUnit(adjusted_baseline)));
...
std::unique_ptr<protocol::DictionaryValue> BuildFlexContainerInfo(
    Element* element, const InspectorFlexContainerHighlightConfig& flex_container_highlight_config, float scale)
{
    CSSComputedStyleDeclaration* style = MakeGarbageCollected<CSSComputedStyleDeclaration>(element, true);
    LocalFrameView* containing_view = element->GetDocument().View();
    LayoutObject* layout_object = element->GetLayoutObject();
    ...
    std::unique_ptr<protocol::DictionaryValue> flex_info = protocol::DictionaryValue::create();
    ...
    DevtoolsFlexInfo flex_lines = GetFlexLinesAndItems(layout_box, is_horizontal, is_reverse);

    // We send a list of flex lines, each containing a list of flex items, with
    // their baselines, to the frontend.
    std::unique_ptr<protocol::ListValue> lines_info = protocol::ListValue::create();
    for (auto line : flex_lines.lines) {
        std::unique_ptr<protocol::ListValue> items_info = protocol::ListValue::create();
        for (auto item_data : line.items) {
            std::unique_ptr<protocol::DictionaryValue> item_info = protocol::DictionaryValue::create();

            gfx::QuadF item_margin_quad = layout_object->LocalRectToAbsoluteQuad(item_data.rect);
    ...
    flex_info->setString("alignItemsStyle", style->GetPropertyCSSValue(CSSPropertyID::kAlignItems)->CssText());
    ...
    const CSSValue* column_gap = style->GetPropertyCSSValue(CSSPropertyID::kColumnGap);
    if (column_gap->IsNumericLiteralValue()) {
        column_gap_value = To<CSSNumericLiteralValue>(column_gap)->DoubleValue();
    }

    flex_info->setDouble("mainGap", is_horizontal ? column_gap_value : row_gap_value);
    flex_info->setDouble("crossGap", is_horizontal ? row_gap_value : column_gap_value);

    flex_info->setValue("flexContainerHighlightConfig", BuildFlexContainerHighlightConfigInfo(flex_container_highlight_config));

    return flex_info;
}

std::unique_ptr<protocol::DictionaryValue> BuildFlexItemInfo(Element* element, const InspectorFlexItemHighlightConfig& flex_item_highlight_config, float scale)
{
    std::unique_ptr<protocol::DictionaryValue> flex_info = protocol::DictionaryValue::create();
    ...
    const Length& size = is_horizontal ? layout_object->StyleRef().Width() : layout_object->StyleRef().Height();
    ...
    if (base_size.IsFixed()) {
        ...
        flex_info->setString("boxSizing", box_sizing == EBoxSizing::kBorderBox ? "border" : "content");

        flex_info->setValue("flexItemHighlightConfig", BuildFlexItemHighlightConfigInfo(flex_item_highlight_config));
    }
...
std::unique_ptr<protocol::DictionaryValue> BuildGridInfo(
    Element* element, const InspectorGridHighlightConfig& grid_highlight_config, float scale, bool isPrimary)
{
    LocalFrameView* containing_view = element->GetDocument().View();
    DCHECK(element->GetLayoutObject());
    auto* grid = To<LayoutGrid>(element->GetLayoutObject());

    std::unique_ptr<protocol::DictionaryValue> grid_info = protocol::DictionaryValue::create();
    ...
    auto column_gap = grid->GridGap(kForColumns) + grid->GridItemOffset(kForColumns);
    ...
    // of the grid.
    LayoutUnit rtl_offset = grid->LogicalWidth() - columns.back() - grid->BorderAndPaddingInlineEnd();

    if (grid_highlight_config.show_track_sizes) {
        StyleResolver& style_resolver = element->GetDocument().GetStyleResolver();

        HeapHashMap<CSSPropertyName, Member<const CSSValue>> cascaded_values = style_resolver.CascadedValuesForElement(element, kPseudoIdNone);

        auto FindCSSValue = [&cascaded_values](CSSPropertyID id) -> const CSSValue* {
            auto it = cascaded_values.find(CSSPropertyName(id));
            return it != cascaded_values.end() ? it->value : nullptr;
        };
        Vector<String> column_authored_values
            = GetAuthoredGridTrackSizes(FindCSSValue(CSSPropertyID::kGridTemplateColumns), grid->AutoRepeatCountForDirection(kForColumns));
        Vector<String> row_authored_values
            = GetAuthoredGridTrackSizes(FindCSSValue(CSSPropertyID::kGridTemplateRows), grid->AutoRepeatCountForDirection(kForRows));

        grid_info->setValue(
            "columnTrackSizes", BuildGridTrackSizes(element, kForColumns, scale, column_gap, rtl_offset, columns, rows, &column_authored_values));
        grid_info->setValue("rowTrackSizes", BuildGridTrackSizes(element, kForRows, scale, row_gap, rtl_offset, rows, columns, &row_authored_values));
    }
    ...
    for (wtf_size_t i = 1; i < rows.size(); ++i) {
        ...
        row_builder.AppendPath(RowQuadToPath(row_quad, i == rows.size() - 1 || row_gap > 0), scale);
        // Row Gaps
        if (i != rows.size() - 1) {
    ...
    for (wtf_size_t i = 1; i < columns.size(); ++i) {
        ...
        LayoutUnit line_left = GetPositionForTrackAt(grid, i - 1, kForColumns, columns);
        if (!is_ltr) {
        ...
        column_builder.AppendPath(ColumnQuadToPath(column_quad, draw_end_line || column_gap > 0), scale);
        // Column Gaps
        if (i != columns.size() - 1) {
            LayoutUnit gap_left = GetPositionForTrackAt(grid, i, kForColumns, columns);
            if (is_ltr)
                gap_left -= column_gap;
            else
                gap_left += rtl_offset;
    ...
    if (grid_highlight_config.show_positive_line_numbers) {
        grid_info->setValue(
            "positiveRowLineNumberPositions", BuildGridPositiveLineNumberPositions(element, row_gap, kForRows, scale, rtl_offset, rows, columns));
        grid_info->setValue(
            "positiveColumnLineNumberPositions", BuildGridPositiveLineNumberPositions(element, column_gap, kForColumns, scale, rtl_offset, columns, rows));
    }

    // Negative Row and column Line positions
    if (grid_highlight_config.show_negative_line_numbers) {
        grid_info->setValue(
            "negativeRowLineNumberPositions", BuildGridNegativeLineNumberPositions(element, row_gap, kForRows, scale, rtl_offset, rows, columns));
        grid_info->setValue(
            "negativeColumnLineNumberPositions", BuildGridNegativeLineNumberPositions(element, column_gap, kForColumns, scale, rtl_offset, columns, rows));
    }

    // Area names
    if (grid_highlight_config.show_area_names) {
        grid_info->setValue("areaNames", BuildAreaNamePaths(element, scale, rows, columns));
    }

    // line names
    if (grid_highlight_config.show_line_names) {
        grid_info->setValue("rowLineNameOffsets", BuildGridLineNames(element, kForRows, scale, rows, columns));
        grid_info->setValue("columnLineNameOffsets", BuildGridLineNames(element, kForColumns, scale, columns, rows));
    }
    ...
    grid_info->setValue("gridHighlightConfig", BuildGridHighlightConfigInfo(grid_highlight_config));

    grid_info->setBoolean("isPrimaryGrid", isPrimary);
    return grid_info;
}

std::unique_ptr<protocol::DictionaryValue> BuildGridInfo(Element* element, const InspectorHighlightConfig& highlight_config, float scale, bool isPrimary)
{
    // Legacy support for highlight_config.css_grid
    if (highlight_config.css_grid != Color::kTransparent) {
        std::unique_ptr<InspectorGridHighlightConfig> grid_config = std::make_unique<InspectorGridHighlightConfig>();
    ...
    return BuildGridInfo(element, *(highlight_config.grid_highlight_config), scale, isPrimary);
}

void CollectQuads(Node* node, bool adjust_for_absolute_zoom, Vector<gfx::QuadF>& out_quads)
{
    LayoutObject* layout_object = node->GetLayoutObject();
    ...
    if (((layout_object && layout_object->IsLayoutInline()) || (!layout_object && node->IsElementNode() && To<Element>(node)->HasDisplayContentsStyle()))
        ...
        for (wtf_size_t i = old_size; i < new_size; i++) {
            ...
            if (adjust_for_absolute_zoom) {
                AdjustForAbsoluteZoom::AdjustQuadMaybeExcludingCSSZoom(out_quads[i], *layout_object);
            }
        }
    }
}

std::unique_ptr<protocol::Array<double>> RectForPhysicalRect(const PhysicalRect& rect)
{
    return std::make_unique<std::vector<double>, std::initializer_list<double>>({ rect.X(), rect.Y(), rect.Width(), rect.Height() });
}
...
PhysicalRect RectInRootFrame(const LayoutObject* layout_object)
{
    ...
    PhysicalRect rect_in_absolute = PhysicalRect::EnclosingRect(layout_object->AbsoluteBoundingBoxRectF());
    return local_frame_view ? local_frame_view->ConvertToRootFrame(rect_in_absolute) : rect_in_absolute;
}

PhysicalRect TextFragmentRectInRootFrame(const LayoutObject* layout_object, const LayoutText::TextBoxInfo& text_box)
{
    PhysicalRect absolute_coords_text_box_rect = layout_object->LocalToAbsoluteRect(text_box.local_rect);
    LocalFrameView* local_frame_view = layout_object->GetFrameView();
    return local_frame_view ? local_frame_view->ConvertToRootFrame(absolute_coords_text_box_rect) : absolute_coords_text_box_rect;
}
...
InspectorFlexContainerHighlightConfig::InspectorFlexContainerHighlightConfig() = default;
...
bool InspectorHighlightBase::BuildNodeQuads(Node* node, gfx::QuadF* content, gfx::QuadF* padding, gfx::QuadF* border, gfx::QuadF* margin)
{
    LayoutObject* layout_object = node->GetLayoutObject();
    if (!layout_object)
        return false;
    ...
    if (!containing_view)
        return false;
    if (!layout_object->IsBox() && !layout_object->IsLayoutInline() && !layout_object->IsText()) {
        return false;
    }
    ...
    if (layout_object->IsText()) {
        ...
        margin_box = PhysicalRect(border_box.X() - layout_box->MarginLeft(), border_box.Y() - layout_box->MarginTop(),
            border_box.Width() + layout_box->MarginWidth(), border_box.Height() + layout_box->MarginHeight());
    } else {
        ...
        padding_box = PhysicalRect(border_box.X() + layout_inline->BorderLeft(), border_box.Y() + layout_inline->BorderTop(),
            border_box.Width() - layout_inline->BorderLeft() - layout_inline->BorderRight(),
            border_box.Height() - layout_inline->BorderTop() - layout_inline->BorderBottom());
        content_box = PhysicalRect(padding_box.X() + layout_inline->PaddingLeft(), padding_box.Y() + layout_inline->PaddingTop(),
            padding_box.Width() - layout_inline->PaddingLeft() - layout_inline->PaddingRight(),
            padding_box.Height() - layout_inline->PaddingTop() - layout_inline->PaddingBottom());
        // Ignore marginTop and marginBottom for inlines.
        margin_box = PhysicalRect(
            border_box.X() - layout_inline->MarginLeft(), border_box.Y(), border_box.Width() + layout_inline->MarginWidth(), border_box.Height());
    }
...
void InspectorHighlightBase::AppendQuad(const gfx::QuadF& quad, const Color& fill_color, const Color& outline_color, const String& name)
{
...
void InspectorHighlightBase::AppendPath(std::unique_ptr<protocol::ListValue> path, const Color& fill_color, const Color& outline_color, const String& name)
{
...
InspectorSourceOrderHighlight::InspectorSourceOrderHighlight(Node* node, Color outline_color, int source_order_position)
...
std::unique_ptr<protocol::DictionaryValue> InspectorSourceOrderHighlight::AsProtocolValue() const
{
    std::unique_ptr<protocol::DictionaryValue> object = protocol::DictionaryValue::create();
...
InspectorHighlight::InspectorHighlight(Node* node, const InspectorHighlightConfig& highlight_config, const InspectorHighlightContrastInfo& node_contrast,
    bool append_element_info, bool append_distance_info, NodeContentVisibilityState content_visibility_state)
    : InspectorHighlightBase(node)
...
{
    DCHECK_GE(node->GetDocument().Lifecycle().GetState(), DocumentLifecycle::kLayoutClean);
    ...
    if (element && element_info_ && highlight_config.show_styles) {
        AppendStyleInfo(element, element_info_.get(), node_contrast, highlight_config.contrast_algorithm);
    }
    ...
    if (element_info_) {
        ...
        element_info_->setBoolean("showAccessibilityInfo", show_accessibility_info_);
    }
...
void InspectorHighlight::AppendDistanceInfo(Node* node)
{
    ...
    node->GetDocument().EnsurePaintLocationDataValidForNode(node, DocumentUpdateReason::kInspector);
    LayoutObject* layout_object = node->GetLayoutObject();
    if (!layout_object)
        return;
    ...
    if (Element* element = DynamicTo<Element>(node)) {
        ...
        for (unsigned i = 0; i < style->length(); ++i) {
            ...
            const CSSValue* value = style->GetPropertyCSSValue(CssPropertyID(element->GetExecutionContext(), name));
            if (!value) {
                continue;
            }
            if (value->IsColorValue()) {
    ...
    PhysicalRect document_rect(node->GetDocument().GetLayoutView()->DocumentRect());
    LocalFrameView* local_frame_view = node->GetDocument().View();
    boxes_->emplace_back(RectForPhysicalRect(local_frame_view->ConvertToRootFrame(document_rect)));
}
...
void InspectorHighlight::VisitAndCollectDistanceInfo(Node* node)
{
    ...
    if (auto* element = DynamicTo<Element>(node)) {
        if (element->GetPseudoId()) {
            ...
            for (PseudoId pseudo_id : { kPseudoIdFirstLetter, kPseudoIdScrollMarkerGroupBefore, kPseudoIdCheck, kPseudoIdBefore, kPseudoIdAfter,
                     kPseudoIdSelectArrow, kPseudoIdScrollMarkerGroupAfter, kPseudoIdScrollMarker, kPseudoIdScrollNextButton, kPseudoIdScrollPrevButton }) {
...
void InspectorHighlight::VisitAndCollectDistanceInfo(PseudoId pseudo_id, LayoutObject* layout_object)
{
...
void InspectorHighlight::AddLayoutBoxToDistanceInfo(LayoutObject* layout_object)
{
    if (layout_object->IsText()) {
        auto* layout_text = To<LayoutText>(layout_object);
        for (const auto& text_box : layout_text->GetTextBoxInfo()) {
            PhysicalRect text_rect(TextFragmentRectInRootFrame(layout_object, text_box));
...
void InspectorHighlight::AppendEventTargetQuads(Node* event_target_node, const InspectorHighlightConfig& highlight_config)
{
...
void InspectorHighlight::AppendPathsForShapeOutside(Node* node, const InspectorHighlightConfig& config)
{
    ...
    const ShapeOutsideInfo* shape_outside_info = ShapeOutsideInfoForNode(node, &paths, &bounds_quad);
    ...
    AppendPath(ShapePathBuilder::BuildPath(*node->GetDocument().View(), *node->GetLayoutObject(), *shape_outside_info, paths.shape, scale_), config.shape,
        Color::kTransparent);
    if (paths.margin_shape.length())
        AppendPath(ShapePathBuilder::BuildPath(*node->GetDocument().View(), *node->GetLayoutObject(), *shape_outside_info, paths.margin_shape, scale_),
            config.shape_margin, Color::kTransparent);
}

void InspectorHighlight::AppendNodeHighlight(Node* node, const InspectorHighlightConfig& highlight_config)
{
    LayoutObject* layout_object = node->GetLayoutObject();
    if (!layout_object)
        return;
    ...
    if (BuildSVGQuads(node, svg_quads)) {
        for (wtf_size_t i = 0; i < svg_quads.size(); ++i) {
            AppendQuad(svg_quads[i], highlight_config.content, highlight_config.content_outline);
        }
        return;
    }
    ...
    if (!BuildNodeQuads(node, &content, &padding, &border, &margin))
        return;
    AppendQuad(content, highlight_config.content, highlight_config.content_outline, "content");
    ...
    if (highlight_config.css_grid != Color::kTransparent || highlight_config.grid_highlight_config) {
        grid_info_ = protocol::ListValue::create();
        if (layout_object->IsLayoutGrid()) {
            grid_info_->pushValue(BuildGridInfo(To<Element>(node), highlight_config, scale_, true));
        }
    }
    ...
    if (highlight_config.flex_container_highlight_config) {
        ...
        if (IsLayoutNGFlexibleBox(*layout_object)) {
            flex_container_info_->pushValue(BuildFlexContainerInfo(To<Element>(node), *(highlight_config.flex_container_highlight_config), scale_));
        }
    }
    ...
    if (highlight_config.flex_item_highlight_config) {
        ...
        if (IsLayoutNGFlexItem(*layout_object)) {
            flex_item_info_->pushValue(BuildFlexItemInfo(To<Element>(node), *(highlight_config.flex_item_highlight_config), scale_));
        }
    }

    if (highlight_config.container_query_container_highlight_config) {
        container_query_container_info_ = protocol::ListValue::create();
        container_query_container_info_->pushValue(
            BuildContainerQueryContainerInfo(node, *(highlight_config.container_query_container_highlight_config), scale_));
    }
}
...
std::unique_ptr<protocol::DictionaryValue> InspectorHighlight::AsProtocolValue() const
{
    ...
    if (model_) {
        std::unique_ptr<protocol::DictionaryValue> distance_info = protocol::DictionaryValue::create();
        distance_info->setArray("boxes", protocol::ValueConversions<std::vector<std::unique_ptr<std::vector<double>>>>::toValue(boxes_.get()));
        distance_info->setArray("content", protocol::ValueConversions<std::vector<double>>::toValue(model_->getContent()));
        distance_info->setArray("padding", protocol::ValueConversions<std::vector<double>>::toValue(model_->getPadding()));
        distance_info->setArray("border", protocol::ValueConversions<std::vector<double>>::toValue(model_->getBorder()));
    ...
    if (container_query_container_info_ && container_query_container_info_->size() > 0) {
        object->setValue("containerQueryInfo", container_query_container_info_->clone());
    }
    return object;
}

// static
bool InspectorHighlight::GetBoxModel(Node* node, std::unique_ptr<protocol::DOM::BoxModel>* model, bool use_absolute_zoom)
{
    node->GetDocument().EnsurePaintLocationDataValidForNode(node, DocumentUpdateReason::kInspector);
    LayoutObject* layout_object = node->GetLayoutObject();
    ...
    if (use_absolute_zoom) {
        AdjustForAbsoluteZoom::AdjustQuadMaybeExcludingCSSZoom(content, *layout_object);
        AdjustForAbsoluteZoom::AdjustQuadMaybeExcludingCSSZoom(padding, *layout_object);
        AdjustForAbsoluteZoom::AdjustQuadMaybeExcludingCSSZoom(border, *layout_object);
        AdjustForAbsoluteZoom::AdjustQuadMaybeExcludingCSSZoom(margin, *layout_object);
    }
    ...
    gfx::Rect bounding_box = view->ConvertToRootFrame(layout_object->AbsoluteBoundingBoxRect());
    ...
    *model = protocol::DOM::BoxModel::create()
                 ...
                 .setWidth(model_object ? AdjustForAbsoluteZoom::AdjustLayoutUnit(model_object->OffsetWidth(), *model_object).Round() : bounding_box.width())
                 .setHeight(model_object ? AdjustForAbsoluteZoom::AdjustLayoutUnit(model_object->OffsetHeight(), *model_object).Round() : bounding_box.height())
                 .build();

    Shape::DisplayPaths paths;
    gfx::QuadF bounds_quad;
    protocol::ErrorSupport errors;
    if (const ShapeOutsideInfo* shape_outside_info = ShapeOutsideInfoForNode(node, &paths, &bounds_quad)) {
        auto shape = ShapePathBuilder::BuildPath(*view, *layout_object, *shape_outside_info, paths.shape, 1.f);
        auto margin_shape = ShapePathBuilder::BuildPath(*view, *layout_object, *shape_outside_info, paths.margin_shape, 1.f);
        (*model)->setShapeOutside(protocol::DOM::ShapeOutsideInfo::create()
                                      .setBounds(BuildArrayForQuad(bounds_quad))
                                      .setShape(protocol::ValueConversions<protocol::Array<protocol::Value>>::fromValue(shape.get(), &errors))
                                      .setMarginShape(protocol::ValueConversions<protocol::Array<protocol::Value>>::fromValue(margin_shape.get(), &errors))
                                      .build());
    }
...
bool InspectorHighlight::BuildSVGQuads(Node* node, Vector<gfx::QuadF>& quads)
{
    ...
    if (!layout_object)
        return false;
    if (!layout_object->GetNode() || !layout_object->GetNode()->IsSVGElement() || layout_object->IsSVGRoot())
        return false;
...
// static
bool InspectorHighlight::GetContentQuads(Node* node, std::unique_ptr<protocol::Array<protocol::Array<double>>>* result)
{
    LocalFrameView* view = node->GetDocument().View();
    if (!view)
        return false;
...
std::unique_ptr<protocol::DictionaryValue> InspectorGridHighlight(Node* node, const InspectorGridHighlightConfig& config)
{
    ...
    std::unique_ptr<protocol::DictionaryValue> grid_info = BuildGridInfo(To<Element>(node), config, scale, true);
    return grid_info;
}

std::unique_ptr<protocol::DictionaryValue> InspectorFlexContainerHighlight(Node* node, const InspectorFlexContainerHighlightConfig& config)
{
    if (DisplayLockUtilities::LockedAncestorPreventingPaint(*node)) {
        // Skip if node is part of display locked tree.
        return nullptr;
...
std::unique_ptr<protocol::DictionaryValue> BuildSnapContainerInfo(Node* node)
{
    ...
    LayoutBox* layout_box = node == node->GetDocument().documentElement() ? node->GetDocument().GetLayoutBoxForScrolling() : node->GetLayoutBox();
    ...
    std::unique_ptr<protocol::DictionaryValue> scroll_snap_info = protocol::DictionaryValue::create();
    ...
    gfx::QuadF snapport_quad = layout_box->LocalToAbsoluteQuad(gfx::QuadF(container_data->rect()));
    scroll_snap_info->setValue("snapport", BuildPathFromQuad(containing_view, snapport_quad));

    auto padding_box = layout_box->PhysicalPaddingBoxRect();
    gfx::QuadF padding_box_quad = layout_box->LocalRectToAbsoluteQuad(padding_box);
    scroll_snap_info->setValue("paddingBox", BuildPathFromQuad(containing_view, padding_box_quad));

    auto snap_type = container_data->scroll_snap_type();
    std::unique_ptr<protocol::ListValue> result_areas = protocol::ListValue::create();
    ...
    std::sort(snap_area_items.begin(), snap_area_items.end(),
        [](const cc::SnapAreaData& a, const cc::SnapAreaData& b) -> bool { return a.rect.origin() < b.rect.origin(); });

    for (const auto& data : snap_area_items) {
        std::unique_ptr<protocol::DictionaryValue> result_area = protocol::DictionaryValue::create();

        gfx::QuadF area_quad = layout_box->LocalToAbsoluteQuad(gfx::QuadF(data.rect));
        result_area->setValue("path", BuildPathFromQuad(containing_view, area_quad));

        Node* area_node = DOMNodeIds::NodeForId(DOMNodeIdFromCompositorElementId(data.element_id));
        DCHECK(area_node);
        if (!area_node)
            continue;

        auto* area_layout_box = area_node->GetLayoutBox();
        gfx::QuadF area_box_quad = area_layout_box->LocalRectToAbsoluteQuad(area_layout_box->PhysicalBorderBoxRect());
        result_area->setValue("borderBox", BuildPathFromQuad(containing_view, area_box_quad));

        BuildSnapAlignment(snap_type, data.scroll_snap_align.alignment_block, data.scroll_snap_align.alignment_inline, result_area);
...
std::unique_ptr<protocol::DictionaryValue> InspectorScrollSnapHighlight(Node* node, const InspectorScrollSnapContainerHighlightConfig& config)
{
    std::unique_ptr<protocol::DictionaryValue> scroll_snap_info = BuildSnapContainerInfo(node);

    if (!scroll_snap_info)
        return nullptr;

    AppendLineStyleConfig(config.snapport_border, scroll_snap_info, "snapportBorder");
    AppendLineStyleConfig(config.snap_area_border, scroll_snap_info, "snapAreaBorder");
    scroll_snap_info->setString("scrollMarginColor", config.scroll_margin_color.SerializeAsCSSColor());
    scroll_snap_info->setString("scrollPaddingColor", config.scroll_padding_color.SerializeAsCSSColor());
...
Vector<gfx::QuadF> GetContainerQueryingDescendantQuads(Element* container)
{
    ...
    for (Element* descendant : InspectorDOMAgent::GetContainerQueryingDescendants(container)) {
...
std::unique_ptr<protocol::DictionaryValue> BuildContainerQueryContainerInfo(
    Node* node, const InspectorContainerQueryContainerHighlightConfig& container_query_container_highlight_config, float scale)
{
    if (!node)
        return nullptr;
    ...
    std::unique_ptr<protocol::DictionaryValue> container_query_container_info = protocol::DictionaryValue::create();
    ...
    FrameQuadToViewport(containing_view, content_quad);
    container_builder.AppendPath(QuadToPath(content_quad), scale);
    container_query_container_info->setValue("containerBorder", container_builder.Release());

    auto* element = DynamicTo<Element>(node);
    bool include_descendants
        = container_query_container_highlight_config.descendant_border && !container_query_container_highlight_config.descendant_border->IsFullyTransparent();
    if (element && include_descendants) {
        ...
        for (auto& descendant_quad : GetContainerQueryingDescendantQuads(element)) {
            std::unique_ptr<protocol::DictionaryValue> descendant_info = protocol::DictionaryValue::create();
            descendant_info->setValue("descendantBorder", BuildPathFromQuad(containing_view, descendant_quad));
            descendants_info->pushValue(std::move(descendant_info));
        }
        container_query_container_info->setArray("queryingDescendants", std::move(descendants_info));
    }

    container_query_container_info->setValue(
        "containerQueryContainerHighlightConfig", BuildContainerQueryContainerHighlightConfigInfo(container_query_container_highlight_config));

    return container_query_container_info;
}

std::unique_ptr<protocol::DictionaryValue> BuildIsolatedElementInfo(Element& element, const InspectorIsolationModeHighlightConfig& config, float scale)
{
    ...
    gfx::QuadF element_box_quad = layout_box->LocalRectToAbsoluteQuad(element_box);
    ...
    PhysicalRect width_resizer_box(layout_box->ContentLeft() + layout_box->ContentWidth(), layout_box->ContentTop(), resizer_size, layout_box->ContentHeight());
    isolated_element_info->setValue("widthResizerBorder", BuildPathFromQuad(containing_view, layout_box->LocalRectToAbsoluteQuad(width_resizer_box)));
    PhysicalRect height_resizer_box(
        layout_box->ContentLeft(), layout_box->ContentTop() + layout_box->ContentHeight(), layout_box->ContentWidth(), resizer_size);
    isolated_element_info->setValue("heightResizerBorder", BuildPathFromQuad(containing_view, layout_box->LocalRectToAbsoluteQuad(height_resizer_box)));

    PhysicalRect bidirection_resizer_box(
        layout_box->ContentLeft() + layout_box->ContentWidth(), layout_box->ContentTop() + layout_box->ContentHeight(), resizer_size, resizer_size);
    isolated_element_info->setValue(
        "bidirectionResizerBorder", BuildPathFromQuad(containing_view, layout_box->LocalRectToAbsoluteQuad(bidirection_resizer_box)));

    CSSComputedStyleDeclaration* style = MakeGarbageCollected<CSSComputedStyleDeclaration>(&element, true);
    const CSSValue* width = style->GetPropertyCSSValue(CSSPropertyID::kWidth);
    if (width && width->IsNumericLiteralValue()) {
        isolated_element_info->setDouble("currentWidth", To<CSSNumericLiteralValue>(width)->DoubleValue());
    }
    const CSSValue* height = style->GetPropertyCSSValue(CSSPropertyID::kHeight);
    if (height && height->IsNumericLiteralValue()) {
        isolated_element_info->setDouble("currentHeight", To<CSSNumericLiteralValue>(height)->DoubleValue());
    }

    isolated_element_info->setValue("isolationModeHighlightConfig", BuildIsolationModeHighlightConfigInfo(config));

    return isolated_element_info;
}

std::unique_ptr<protocol::DictionaryValue> InspectorContainerQueryHighlight(Node* node, const InspectorContainerQueryContainerHighlightConfig& config)
{
    LocalFrameView* frame_view = node->GetDocument().View();
    if (!frame_view)
        return nullptr;
    ...
    std::unique_ptr<protocol::DictionaryValue> container_query_container_info
        = BuildContainerQueryContainerInfo(node, config, DeviceScaleFromFrameView(frame_view));
...
std::unique_ptr<protocol::DictionaryValue> InspectorIsolatedElementHighlight(Element* element, const InspectorIsolationModeHighlightConfig& config)
{
    LocalFrameView* frame_view = element->GetDocument().View();
    if (!frame_view)
        return nullptr;

    std::unique_ptr<protocol::DictionaryValue> isolated_element_info = BuildIsolatedElementInfo(*element, config, DeviceScaleFromFrameView(frame_view));
...
InspectorHighlightConfig InspectorHighlight::DefaultConfig()
{
    ...
    config.grid_highlight_config = std::make_unique<InspectorGridHighlightConfig>(InspectorHighlight::DefaultGridConfig());
    config.flex_container_highlight_config = std::make_unique<InspectorFlexContainerHighlightConfig>(InspectorHighlight::DefaultFlexContainerConfig());
    config.flex_item_highlight_config = std::make_unique<InspectorFlexItemHighlightConfig>(InspectorHighlight::DefaultFlexItemConfig());
    return config;
}
...
// static
InspectorFlexContainerHighlightConfig InspectorHighlight::DefaultFlexContainerConfig()
{
    InspectorFlexContainerHighlightConfig config;
    config.container_border = std::optional<LineStyle>(InspectorHighlight::DefaultLineStyle());
    config.line_separator = std::optional<LineStyle>(InspectorHighlight::DefaultLineStyle());
    config.item_separator = std::optional<LineStyle>(InspectorHighlight::DefaultLineStyle());
    config.main_distributed_space = std::optional<BoxStyle>(InspectorHighlight::DefaultBoxStyle());
    config.cross_distributed_space = std::optional<BoxStyle>(InspectorHighlight::DefaultBoxStyle());
    config.row_gap_space = std::optional<BoxStyle>(InspectorHighlight::DefaultBoxStyle());
    config.column_gap_space = std::optional<BoxStyle>(InspectorHighlight::DefaultBoxStyle());
    config.cross_alignment = std::optional<LineStyle>(InspectorHighlight::DefaultLineStyle());
    return config;
}
...
InspectorFlexItemHighlightConfig InspectorHighlight::DefaultFlexItemConfig()
{
    ...
    config.base_size_box = std::optional<BoxStyle>(InspectorHighlight::DefaultBoxStyle());
    config.base_size_border = std::optional<LineStyle>(InspectorHighlight::DefaultLineStyle());
    config.flexibility_arrow = std::optional<LineStyle>(InspectorHighlight::DefaultLineStyle());
    return config;
}
