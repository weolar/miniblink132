...
protected:
    ...
    CSSRuleSourceDataList Parse(String text)
    {
        ...
        InspectorCSSParserObserver observer(text, &GetDocument(),
...
TEST_F(InspectorCSSParserObserverTest, DeclRangeNormal)
{
    ...
    EXPECT_EQ(" left:1px; right:2px; ", Substring(text, data[0]->rule_body_range));
...
TEST_F(InspectorCSSParserObserverTest, DeclRangeWithChildRule)
{
    ...
    ASSERT_EQ(1u, data.size());
    EXPECT_EQ(" left:1px; right:2px; & {} ", Substring(text, data[0]->rule_body_range));
    EXPECT_EQ(" left:1px; right:2px;", Substring(text, data[0]->rule_declarations_range));
}
...
TEST_F(InspectorCSSParserObserverTest, DeclRangeWithNestedDecl)
{
    ...
    CSSRuleSourceDataList data = Parse(text);
    ASSERT_EQ(1u, data.size());
    EXPECT_EQ(" left:1px; right:2px; & {} --nested:1; ", Substring(text, data[0]->rule_body_range));
    EXPECT_EQ(" left:1px; right:2px;", Substring(text, data[0]->rule_declarations_range));
}
...
TEST_F(InspectorCSSParserObserverTest, NestedDeclarationsNonEmpty)
{
    ...
    EXPECT_EQ("right:2px;", Substring(text, data[0]->child_rules[1]->rule_body_range));
    EXPECT_EQ("top:3px;", Substring(text, data[0]->child_rules[3]->rule_body_range));
}
...
TEST_F(InspectorCSSParserObserverTest, NestedDeclarationsComment)
{
    ...
    ASSERT_EQ(1u, data.size());
    ASSERT_EQ(4u, data[0]->child_rules.size());
    EXPECT_EQ("/* left:1px; */", Substring(text, data[0]->child_rules[1]->rule_body_range));
    EXPECT_EQ("/* right:2px; */", Substring(text, data[0]->child_rules[3]->rule_body_range));
}

TEST_F(InspectorCSSParserObserverTest, NestedDeclarationsInvalid)
{
    String text = ".a { & { } dino-affinity:t-rex; & { } dino-name:--rex-ruthor; }";
    CSSRuleSourceDataList data = Parse(text);
    ASSERT_EQ(1u, data.size());
    ASSERT_EQ(4u, data[0]->child_rules.size());
    EXPECT_EQ("dino-affinity:t-rex;", Substring(text, data[0]->child_rules[1]->rule_body_range));
    EXPECT_EQ("dino-name:--rex-ruthor;", Substring(text, data[0]->child_rules[3]->rule_body_range));
}
...
TEST_F(InspectorCSSParserObserverTest, NestedDeclarationsCommentMedia)
{
    ...
    EXPECT_EQ("/* left:1px; */", Substring(text, media.child_rules[0]->rule_body_range));
}
...
TEST_F(InspectorCSSParserObserverTest, NestedDeclarationsInvalidMedia)
{
    ...
    EXPECT_EQ("dino-affinity:t-rex;", Substring(text, media.child_rules[0]->rule_body_range));
}
...
TEST_F(InspectorCSSParserObserverTest, NestedDeclarationsInvalidPrecedingRule)
{
    ...
    ASSERT_EQ(1u, data.size());
    ASSERT_EQ(2u, data[0]->child_rules.size());
    EXPECT_EQ("span", Substring(text, data[0]->child_rules[0]->rule_header_range));
