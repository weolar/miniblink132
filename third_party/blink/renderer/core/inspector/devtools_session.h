...
public:
    DevToolsSession(DevToolsAgent*, mojo::PendingAssociatedRemote<mojom::blink::DevToolsSessionHost> host_remote,
    ...
    template <typename Agent, typename... Args> Agent* CreateAndAppend(Args&&... args)
    {
...
private:
    ...
    void DispatchProtocolCommandImpl(int call_id, const String& method, base::span<const uint8_t> message);

    // protocol::FrontendChannel implementation.
    void SendProtocolResponse(int call_id, std::unique_ptr<protocol::Serializable> message) override;
    ...
    blink::mojom::blink::DevToolsMessagePtr FinalizeMessage(std::vector<uint8_t> message, std::optional<int> call_id) const;

    template <typename T> bool IsDomainAvailableToUntrustedClient()
    {
        return std::disjunction_v<std::is_same<T, InspectorAuditsAgent>, std::is_same<T, InspectorCSSAgent>, std::is_same<T, InspectorCacheStorageAgent>,
            std::is_same<T, InspectorAccessibilityAgent>, std::is_same<T, InspectorDOMAgent>, std::is_same<T, InspectorDOMDebuggerAgent>,
            std::is_same<T, InspectorDOMSnapshotAgent>, std::is_same<T, InspectorDatabaseAgent>, std::is_same<T, InspectorEmulationAgent>,
            std::is_same<T, InspectorIOAgent>, std::is_same<T, InspectorLogAgent>, std::is_same<T, InspectorNetworkAgent>,
            std::is_same<T, InspectorOverlayAgent>, std::is_same<T, InspectorPageAgent>, std::is_same<T, InspectorPerformanceAgent>,
    ...
    HeapMojoAssociatedReceiver<mojom::blink::DevToolsSession, DevToolsSession> receiver_ { this, nullptr };
    // DevToolsSession is not tied to ExecutionContext
    HeapMojoAssociatedRemote<mojom::blink::DevToolsSessionHost> host_remote_ { nullptr };
