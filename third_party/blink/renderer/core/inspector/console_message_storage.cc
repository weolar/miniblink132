...
void TraceConsoleMessageEvent(ConsoleMessage* message)
{
    ...
    if (message->GetLevel() == ConsoleMessage::Level::kError) {
        TRACE_EVENT_INSTANT2("blink.console", "ConsoleMessage::Error", TRACE_EVENT_SCOPE_THREAD, "source", MessageSourceToString(message->GetSource()),
            "message", MessageTracedValue(message));
    }
}
} // anonymous namespace
...
bool ConsoleMessageStorage::AddConsoleMessage(ExecutionContext* context, ConsoleMessage* message, bool discard_duplicates)
{
