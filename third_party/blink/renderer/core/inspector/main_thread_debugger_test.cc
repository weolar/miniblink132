...
TEST_F(MainThreadDebuggerTest, HitBreakPointDuringLifecycle)
{
    ...
    std::unique_ptr<DocumentLifecycle::PostponeTransitionScope> postponed_transition_scope
        = std::make_unique<DocumentLifecycle::PostponeTransitionScope>(document.Lifecycle());
...
class MainThreadDebuggerMultipleMainFramesTest : public MainThreadDebuggerTest, public testing::WithParamInterface<bool> {
public:
    MainThreadDebuggerMultipleMainFramesTest()
    {
        if (IsDebuggerAllowed()) {
            scoped_feature_list_.InitAndEnableFeature(features::kAllowDevToolsMainThreadDebuggerForMultipleMainFrames);
        } else {
            scoped_feature_list_.InitAndDisableFeature(features::kAllowDevToolsMainThreadDebuggerForMultipleMainFrames);
        }
    }
    ...
    Page& GetSecondPage()
    {
        return second_dummy_page_holder_->GetPage();
    }

    bool IsDebuggerAllowed()
    {
        return GetParam();
    }
...
INSTANTIATE_TEST_SUITE_P(All, MainThreadDebuggerMultipleMainFramesTest, testing::Bool());
...
TEST_P(MainThreadDebuggerMultipleMainFramesTest, Allow)
{
    ...
    auto* debugger = MainThreadDebugger::Instance(GetDocument().GetAgent().isolate());
