...
bool IsServiceWorkerPresent(Document* document)
{
    ...
    WebServiceWorkerNetworkProvider* provider = loader->GetServiceWorkerNetworkProvider();
    if (!provider)
        return false;
...
class InspectorResourceContentLoader::ResourceClient final : public GarbageCollected<InspectorResourceContentLoader::ResourceClient>,
...
InspectorResourceContentLoader::InspectorResourceContentLoader(LocalFrame* inspected_frame)
...
void InspectorResourceContentLoader::Start()
{
    ...
    InspectedFrames* inspected_frames = MakeGarbageCollected<InspectedFrames>(inspected_frame_);
    ...
    for (Document* document : documents) {
        ...
        if (item) {
            resource_request = item->GenerateResourceRequest(mojom::FetchCacheMode::kOnlyIfCached);
        } else {
        ...
        const DOMWrapperWorld* world = document->GetExecutionContext()->GetCurrentWorld();
        ...
        for (CSSStyleSheet* style_sheet : style_sheets) {
            ...
            style_sheet_resource_request.SetRequestContext(mojom::blink::RequestContextType::INTERNAL);
            ResourceLoaderOptions options(world);
            options.initiator_info.name = fetch_initiator_type_names::kInternal;
            FetchParameters params(std::move(style_sheet_resource_request), options);
            ResourceClient* resource_client = MakeGarbageCollected<ResourceClient>(this);
            // Prevent garbage collection by holding a reference to this resource.
            resources_.push_back(CSSStyleSheetResource::Fetch(params, fetcher, resource_client));
        ...
        if (!ShouldSkipFetchingUrl(link)) {
            auto use_credentials = EqualIgnoringASCIICase(link_element->FastGetAttribute(html_names::kCrossoriginAttr), "use-credentials");
            ResourceRequest manifest_request(link);
            manifest_request.SetMode(network::mojom::RequestMode::kCors);
            manifest_request.SetTargetAddressSpace(network::mojom::IPAddressSpace::kUnknown);
            // See https://w3c.github.io/manifest/. Use "include" when use_credentials
            // is true, and "omit" otherwise.
            manifest_request.SetCredentialsMode(use_credentials ? network::mojom::CredentialsMode::kInclude : network::mojom::CredentialsMode::kOmit);
            manifest_request.SetRequestContext(mojom::blink::RequestContextType::MANIFEST);
            ResourceLoaderOptions manifest_options(world);
            manifest_options.initiator_info.name = fetch_initiator_type_names::kInternal;
            FetchParameters manifest_params(std::move(manifest_request), manifest_options);
            ResourceClient* manifest_client = MakeGarbageCollected<ResourceClient>(this);
            resources_.push_back(RawResource::Fetch(manifest_params, fetcher, manifest_client));
...
void InspectorResourceContentLoader::EnsureResourcesContentLoaded(int client_id, base::OnceClosure callback)
{
    if (!started_)
        Start();
    callbacks_.insert(client_id, Callbacks()).stored_value->value.push_back(std::move(callback));
    CheckDone();
}
