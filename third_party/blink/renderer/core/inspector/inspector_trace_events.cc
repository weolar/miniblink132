...
void InspectorParseHtmlBeginData(perfetto::TracedValue context, Document* document, int start_line)
{
...
void GetNavigationTracingData(perfetto::TracedValue context, Document* document)
{
    auto dict = std::move(context).WriteDictionary();
...
void SetCallStack(v8::Isolate* isolate, perfetto::TracedDictionary& dict)
{
    ...
    if (!trace_category_enabled) {
        //trace_category_enabled = TRACE_EVENT_API_GET_CATEGORY_GROUP_ENABLED(TRACE_DISABLED_BY_DEFAULT("devtools.timeline.stack"));
        trace_category_enabled = (const unsigned char*)"devtools.timeline.stack";
    }
    if (!*trace_category_enabled)
        return;
...
void InspectorTraceEvents::WillSendRequest(ExecutionContext* execution_context, DocumentLoader* loader, const KURL& fetch_context_url,
    const ResourceRequest& request, const ResourceResponse& redirect_response, const ResourceLoaderOptions& resource_loader_options, ResourceType resource_type,
    RenderBlockingBehavior render_blocking_behavior, base::TimeTicks timestamp)
{
    LocalFrame* frame = loader ? loader->GetFrame() : nullptr;
    TRACE_EVENT_INSTANT_WITH_TIMESTAMP1(
        "devtools.timeline", "ResourceSendRequest", TRACE_EVENT_SCOPE_THREAD, timestamp, "data", [&](perfetto::TracedValue ctx) {
            inspector_send_request_event::Data(std::move(ctx), execution_context, loader, request.InspectorId(), frame, request, resource_type,
                render_blocking_behavior, resource_loader_options);
        });
}

void InspectorTraceEvents::WillSendNavigationRequest(
    uint64_t identifier, DocumentLoader* loader, const KURL& url, const AtomicString& http_method, EncodedFormData*)
{
    LocalFrame* frame = loader ? loader->GetFrame() : nullptr;
    DEVTOOLS_TIMELINE_TRACE_EVENT_INSTANT("ResourceSendRequest", inspector_send_navigation_request_event::Data, loader, identifier, frame, url, http_method);
}

void InspectorTraceEvents::DidReceiveResourceResponse(uint64_t identifier, DocumentLoader* loader, const ResourceResponse& response, const Resource*)
{
    LocalFrame* frame = loader ? loader->GetFrame() : nullptr;
    DEVTOOLS_TIMELINE_TRACE_EVENT_INSTANT("ResourceReceiveResponse", inspector_receive_response_event::Data, loader, identifier, frame, response);
}

void InspectorTraceEvents::DidReceiveData(uint64_t identifier, DocumentLoader* loader, base::SpanOrSize<const char> encoded_data)
{
    LocalFrame* frame = loader ? loader->GetFrame() : nullptr;
    DEVTOOLS_TIMELINE_TRACE_EVENT_INSTANT("ResourceReceivedData", inspector_receive_data_event::Data, loader, identifier, frame, encoded_data.size());
}

void InspectorTraceEvents::DidFinishLoading(
    uint64_t identifier, DocumentLoader* loader, base::TimeTicks finish_time, int64_t encoded_data_length, int64_t decoded_body_length)
{
    DEVTOOLS_TIMELINE_TRACE_EVENT_INSTANT(
        "ResourceFinish", inspector_resource_finish_event::Data, loader, identifier, finish_time, false, encoded_data_length, decoded_body_length);
}

void InspectorTraceEvents::DidFailLoading(
    CoreProbeSink* sink, uint64_t identifier, DocumentLoader* loader, const ResourceError&, const base::UnguessableToken& devtools_frame_or_worker_token)
{
    DEVTOOLS_TIMELINE_TRACE_EVENT_INSTANT("ResourceFinish", inspector_resource_finish_event::Data, loader, identifier, base::TimeTicks(), true, 0, 0);
}

void InspectorTraceEvents::MarkResourceAsCached(DocumentLoader* loader, uint64_t identifier)
{
    DEVTOOLS_TIMELINE_TRACE_EVENT_INSTANT("ResourceMarkAsCached", inspector_mark_resource_cached_event::Data, loader, identifier);
}
...
{
    TRACE_EVENT_INSTANT1(TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "UpdateCounters", TRACE_EVENT_SCOPE_THREAD, "data",
        [&](perfetto::TracedValue context) { inspector_update_counters_event::Data(std::move(context), probe.context->GetIsolate()); });
}

void InspectorTraceEvents::Will(const probe::ParseHTML& probe)
{
    // FIXME: Pass in current input length.
    TRACE_EVENT_BEGIN1("devtools.timeline", "ParseHTML", "beginData", [&](perfetto::TracedValue context) {
        InspectorParseHtmlBeginData(std::move(context), probe.parser->GetDocument(), probe.parser->LineNumber().ZeroBasedInt());
    });
}
...
void InspectorTraceEvents::Did(const probe::ParseHTML& probe)
{
    TRACE_EVENT_END1("devtools.timeline", "ParseHTML", "endData",
        [&](perfetto::TracedValue context) { InspectorParseHtmlEndData(std::move(context), probe.parser->LineNumber().ZeroBasedInt() - 1); });
    TRACE_EVENT_INSTANT1(TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "UpdateCounters", TRACE_EVENT_SCOPE_THREAD, "data",
        [&](perfetto::TracedValue context) { inspector_update_counters_event::Data(std::move(context), probe.document->GetAgent().isolate()); });
}
...
void InspectorTraceEvents::PaintTiming(Document* document, const char* name, double timestamp)
{
    TRACE_EVENT_MARK_WITH_TIMESTAMP2("loading,rail,devtools.timeline", name, trace_event::ToTraceTimestamp(timestamp), "frame",
        GetFrameIdForTracing(document->GetFrame()), "data", [&](perfetto::TracedValue context) { GetNavigationTracingData(std::move(context), document); });
}

void InspectorTraceEvents::FrameStartedLoading(LocalFrame* frame)
{
    TRACE_EVENT_INSTANT1("devtools.timeline", "FrameStartedLoading", TRACE_EVENT_SCOPE_THREAD, "frame", GetFrameIdForTracing(frame));
}

namespace {

void SetNodeInfo(perfetto::TracedDictionary& dict, Node* node, perfetto::StaticString id_field_name, perfetto::StaticString name_field_name = nullptr)
{
    *(int*)1 = 1;
...
const char* NotStreamedReasonString(ScriptStreamer::NotStreamingReason reason)
{
    ...
    case ScriptStreamer::NotStreamingReason::kBackgroundResponseProcessorWillBeUsed:
...
void FillCommonPart(perfetto::TracedDictionary& dict, ContainerNode& node, const InvalidationSet& invalidation_set, const char* invalidated_selector)
{
    dict.Add("frame", IdentifiersFactory::FrameId(node.GetDocument().GetFrame()));
    SetNodeInfo(dict, &node, "nodeId", "nodeName");
    dict.Add("invalidationSet", DescendantInvalidationSetToIdString(invalidation_set));
...
const char inspector_schedule_style_invalidation_tracking_event::kAttribute[] = "attribute";
const char inspector_schedule_style_invalidation_tracking_event::kClass[] = "class";
const char inspector_schedule_style_invalidation_tracking_event::kId[] = "id";
const char inspector_schedule_style_invalidation_tracking_event::kPseudo[] = "pseudo";
const char inspector_schedule_style_invalidation_tracking_event::kRuleSet[] = "ruleset";
...
const char* FetchPriorityString(mojom::blink::FetchPriorityHint fetch_priority)
{
...
void inspector_schedule_style_invalidation_tracking_event::IdChange(
    perfetto::TracedValue context, Element& element, const InvalidationSet& invalidation_set, const AtomicString& id)
{
    auto dict = std::move(context).WriteDictionary();
...
void inspector_schedule_style_invalidation_tracking_event::ClassChange(
    perfetto::TracedValue context, Element& element, const InvalidationSet& invalidation_set, const AtomicString& class_name)
{
    auto dict = std::move(context).WriteDictionary();
...
void inspector_schedule_style_invalidation_tracking_event::AttributeChange(
    perfetto::TracedValue context, Element& element, const InvalidationSet& invalidation_set, const QualifiedName& attribute_name)
{
    auto dict = std::move(context).WriteDictionary();
...
void inspector_schedule_style_invalidation_tracking_event::PseudoChange(
    perfetto::TracedValue context, Element& element, const InvalidationSet& invalidation_set, CSSSelector::PseudoType pseudo_type)
{
    auto dict = std::move(context).WriteDictionary();
    FillCommonPart(dict, element, invalidation_set, kAttribute);
...
const char inspector_style_invalidator_invalidate_event::kElementHasPendingInvalidationList[] = "Element has pending invalidation list";
const char inspector_style_invalidator_invalidate_event::kInvalidateCustomPseudo[] = "Invalidate custom pseudo element";
const char inspector_style_invalidator_invalidate_event::kInvalidationSetInvalidatesSelf[] = "Invalidation set invalidates self";
const char inspector_style_invalidator_invalidate_event::kInvalidationSetInvalidatesSubtree[] = "Invalidation set invalidates subtree";
const char inspector_style_invalidator_invalidate_event::kInvalidationSetMatchedAttribute[] = "Invalidation set matched attribute";
const char inspector_style_invalidator_invalidate_event::kInvalidationSetMatchedClass[] = "Invalidation set matched class";
const char inspector_style_invalidator_invalidate_event::kInvalidationSetMatchedId[] = "Invalidation set matched id";
const char inspector_style_invalidator_invalidate_event::kInvalidationSetMatchedTagName[] = "Invalidation set matched tagName";
const char inspector_style_invalidator_invalidate_event::kInvalidationSetMatchedPart[] = "Invalidation set matched part";

namespace inspector_style_invalidator_invalidate_event {
void FillCommonPart(perfetto::TracedDictionary& dict, ContainerNode& node, const char* reason)
{
    dict.Add("frame", IdentifiersFactory::FrameId(node.GetDocument().GetFrame()));
    SetNodeInfo(dict, &node, "nodeId", "nodeName");
    dict.Add("reason", reason);
}
void FillSelectors(perfetto::TracedDictionary& dict, const InvalidationSet& invalidation_set, InvalidationSetToSelectorMap::SelectorFeatureType feature_type,
    const AtomicString& feature_value)
{
    const InvalidationSetToSelectorMap::IndexedSelectorList* selectors = InvalidationSetToSelectorMap::Lookup(&invalidation_set, feature_type, feature_value);
...
void inspector_style_invalidator_invalidate_event::Data(perfetto::TracedValue context, Element& element, const char* reason)
{
    auto dict = std::move(context).WriteDictionary();
...
void inspector_style_invalidator_invalidate_event::SelectorPart(
    perfetto::TracedValue context, Element& element, const char* reason, const InvalidationSet& invalidation_set, const AtomicString& selector_part)
{
    auto dict = std::move(context).WriteDictionary();
    FillCommonPart(dict, element, reason);
    InvalidationSetToSelectorMap::SelectorFeatureType feature_type = InvalidationSetToSelectorMap::SelectorFeatureType::kUnknown;
    if (reason == kInvalidationSetMatchedClass) {
        ...
        feature_type = InvalidationSetToSelectorMap::SelectorFeatureType::kAttribute;
    } else if (reason == kInvalidationSetInvalidatesSubtree) {
        feature_type = InvalidationSetToSelectorMap::SelectorFeatureType::kWholeSubtree;
    }
    if (feature_type != InvalidationSetToSelectorMap::SelectorFeatureType::kUnknown) {
...
void inspector_style_invalidator_invalidate_event::InvalidationList(
    perfetto::TracedValue context, ContainerNode& node, const Vector<scoped_refptr<InvalidationSet>>& invalidation_list)
{
    auto dict = std::move(context).WriteDictionary();
...
void inspector_style_recalc_invalidation_tracking_event::Data(
    perfetto::TracedValue context, Node* node, StyleChangeType change_type, const StyleChangeReasonForTracing& reason)
{
    DCHECK(node);
...
void inspector_layout_event::BeginData(perfetto::TracedValue context, LocalFrameView* frame_view)
{
    bool is_partial;
    unsigned needs_layout_objects;
    unsigned total_objects;
    LocalFrame& frame = frame_view->GetFrame();
    frame.View()->CountObjectsNeedingLayout(needs_layout_objects, total_objects, is_partial);
...
static void SetGeneratingNodeInfo(
    perfetto::TracedDictionary& dict, const LayoutObject* layout_object, perfetto::StaticString id_field_name, perfetto::StaticString name_field_name = nullptr)
{
    Node* node = nullptr;
...
static void CreateLayoutRoot(perfetto::TracedValue context, const LayoutObjectWithDepth& layout_root)
{
    auto dict = std::move(context).WriteDictionary();
...
static void SetHeaders(perfetto::TracedValue context, const HTTPHeaderMap& headers)
{
    auto array = std::move(context).WriteArray();
...
void inspector_layout_event::EndData(perfetto::TracedValue context, const HeapVector<LayoutObjectWithDepth>& layout_roots)
{
    auto dict = std::move(context).WriteDictionary();
    {
...
void inspector_layout_invalidation_tracking_event::Data(
    perfetto::TracedValue context, const LayoutObject* layout_object, LayoutInvalidationReasonForTracing reason)
{
...
void inspector_change_resource_priority_event::Data(
    perfetto::TracedValue context, DocumentLoader* loader, uint64_t identifier, const ResourceLoadPriority& load_priority)
{
...
namespace {
String GetRenderBlockingStringFromBehavior(RenderBlockingBehavior render_blocking_behavior)
{
...
void inspector_send_request_event::Data(perfetto::TracedValue context, ExecutionContext* execution_context, DocumentLoader* loader, uint64_t identifier,
    LocalFrame* frame, const ResourceRequest& request, ResourceType resource_type, RenderBlockingBehavior render_blocking_behavior,
    const ResourceLoaderOptions& resource_loader_options)
{
    ...
    dict.Add("fetchPriorityHint", FetchPriorityString(request.GetFetchPriorityHint()));
    SetCallStack(execution_context->GetIsolate(), dict);
    SetInitiator(frame ? frame->GetDocument() : nullptr, resource_loader_options.initiator_info, dict);
}

void inspector_change_render_blocking_behavior_event::Data(perfetto::TracedValue context, DocumentLoader* loader, uint64_t identifier,
    const ResourceRequestHead& request, RenderBlockingBehavior render_blocking_behavior)
{
    String request_id = IdentifiersFactory::RequestId(loader, identifier);
...
void inspector_send_navigation_request_event::Data(
    perfetto::TracedValue context, DocumentLoader* loader, uint64_t identifier, LocalFrame* frame, const KURL& url, const AtomicString& http_method)
{
    auto dict = std::move(context).WriteDictionary();
    ...
    if (priority)
        dict.Add("priority", priority);
    dict.Add("fetchPriorityHint", FetchPriorityString(mojom::blink::FetchPriorityHint::kAuto));
    SetCallStack(frame->DomWindow()->GetIsolate(), dict);
}

namespace {
void RecordTiming(perfetto::TracedValue context, const ResourceLoadTiming& timing)
{
    auto dict = std::move(context).WriteDictionary();
    ...
    dict.Add("dnsStart", timing.CalculateMillisecondDelta(timing.DomainLookupStart()));
    dict.Add("dnsEnd", timing.CalculateMillisecondDelta(timing.DomainLookupEnd()));
    dict.Add("connectStart", timing.CalculateMillisecondDelta(timing.ConnectStart()));
    ...
    dict.Add("workerStart", timing.CalculateMillisecondDelta(timing.WorkerStart()));
    dict.Add("workerReady", timing.CalculateMillisecondDelta(timing.WorkerReady()));
    ...
    dict.Add("receiveHeadersStart", timing.CalculateMillisecondDelta(timing.ReceiveHeadersStart()));
    dict.Add("receiveHeadersEnd", timing.CalculateMillisecondDelta(timing.ReceiveHeadersEnd()));
    dict.Add("pushStart", timing.PushStart().since_origin().InSecondsF());
    dict.Add("pushEnd", timing.PushEnd().since_origin().InSecondsF());
}
} // namespace

void inspector_receive_response_event::Data(
    perfetto::TracedValue context, DocumentLoader* loader, uint64_t identifier, LocalFrame* frame, const ResourceResponse& response)
{
    String request_id = IdentifiersFactory::RequestId(loader, identifier);
    ...
    if (!response.ResponseTime().is_null()) {
        dict.Add("responseTime", response.ResponseTime().InMillisecondsFSinceUnixEpoch());
    }
    ...
    if (response.GetServiceWorkerRouterInfo()) {
        ...
        info.Add("ruleIdMatched", response.GetServiceWorkerRouterInfo()->RuleIdMatched());
        info.Add("matchedSourceType", response.GetServiceWorkerRouterInfo()->MatchedSourceType());
    }
...
void inspector_receive_data_event::Data(
    perfetto::TracedValue context, DocumentLoader* loader, uint64_t identifier, LocalFrame* frame, uint64_t encoded_data_length)
{
    String request_id = IdentifiersFactory::RequestId(loader, identifier);
...
void inspector_resource_finish_event::Data(perfetto::TracedValue context, DocumentLoader* loader, uint64_t identifier, base::TimeTicks finish_time,
    bool did_fail, int64_t encoded_data_length, int64_t decoded_body_length)
{
    String request_id = IdentifiersFactory::RequestId(loader, identifier);
...
void inspector_mark_resource_cached_event::Data(perfetto::TracedValue context, DocumentLoader* loader, uint64_t identifier)
{
    auto dict = std::move(context).WriteDictionary();
    String request_id = IdentifiersFactory::RequestId(loader, identifier);
    dict.Add("requestId", request_id);
...
static void GenericTimerData(perfetto::TracedDictionary& dict, ExecutionContext* context, int timer_id)
{
...
void inspector_timer_install_event::Data(
    perfetto::TracedValue trace_context, ExecutionContext* context, int timer_id, base::TimeDelta timeout, bool single_shot)
{
...
void inspector_timer_remove_event::Data(perfetto::TracedValue trace_context, ExecutionContext* context, int timer_id)
{
    auto dict = std::move(trace_context).WriteDictionary();
    GenericTimerData(dict, context, timer_id);
    SetCallStack(context->GetIsolate(), dict);
...
void inspector_timer_fire_event::Data(perfetto::TracedValue trace_context, ExecutionContext* context, int timer_id)
{
    auto dict = std::move(trace_context).WriteDictionary();
    GenericTimerData(dict, context, timer_id);
}

void inspector_animation_frame_event::Data(perfetto::TracedValue trace_context, ExecutionContext* context, int callback_id)
{
    auto dict = std::move(trace_context).WriteDictionary();
...
void GenericIdleCallbackEvent(perfetto::TracedDictionary& dict, ExecutionContext* context, int id)
{
    dict.Add("id", id);
    if (LocalFrame* frame = FrameForExecutionContext(context))
        dict.Add("frame", IdentifiersFactory::FrameId(frame));
...
void inspector_idle_callback_request_event::Data(perfetto::TracedValue trace_context, ExecutionContext* context, int id, double timeout)
{
    auto dict = std::move(trace_context).WriteDictionary();
...
void inspector_idle_callback_cancel_event::Data(perfetto::TracedValue trace_context, ExecutionContext* context, int id)
{
    auto dict = std::move(trace_context).WriteDictionary();
    GenericIdleCallbackEvent(dict, context, id);
}
...
void inspector_idle_callback_fire_event::Data(
    perfetto::TracedValue trace_context, ExecutionContext* context, int id, double allotted_milliseconds, bool timed_out)
{
    auto dict = std::move(trace_context).WriteDictionary();
    GenericIdleCallbackEvent(dict, context, id);
...
void inspector_parse_author_style_sheet_event::Data(perfetto::TracedValue context, const CSSStyleSheetResource* cached_style_sheet)
{
    auto dict = std::move(context).WriteDictionary();
    dict.Add("styleSheetUrl", cached_style_sheet->Url().GetString());
}

void inspector_xhr_ready_state_change_event::Data(perfetto::TracedValue trace_context, ExecutionContext* context, XMLHttpRequest* request)
{
    auto dict = std::move(trace_context).WriteDictionary();
    dict.Add("url", request->Url().GetString());
...
void inspector_xhr_load_event::Data(perfetto::TracedValue trace_context, ExecutionContext* context, XMLHttpRequest* request)
{
    auto dict = std::move(trace_context).WriteDictionary();
    dict.Add("url", request->Url().GetString());
    if (LocalFrame* frame = FrameForExecutionContext(context))
...
void inspector_paint_event::Data(perfetto::TracedValue context, LocalFrame* frame, const LayoutObject* layout_object, const gfx::QuadF& quad, int layer_id)
{
    auto dict = std::move(context).WriteDictionary();
    dict.Add("frame", IdentifiersFactory::FrameId(frame));
...
void inspector_commit_load_event::Data(perfetto::TracedValue context, LocalFrame* frame)
{
    auto dict = std::move(context).WriteDictionary();
...
void inspector_layerize_event::Data(perfetto::TracedValue context, LocalFrame* frame)
{
    auto dict = std::move(context).WriteDictionary();
    FrameEventData(dict, frame);
    dict.Add("frame", IdentifiersFactory::FrameId(frame));
...
void inspector_mark_load_event::Data(perfetto::TracedValue context, LocalFrame* frame)
{
    auto dict = std::move(context).WriteDictionary();
    FrameEventData(dict, frame);
    dict.Add("frame", IdentifiersFactory::FrameId(frame));
...
void inspector_pre_paint_event::Data(perfetto::TracedValue context, LocalFrame* frame)
{
    auto dict = std::move(context).WriteDictionary();
    FrameEventData(dict, frame);
    dict.Add("frame", IdentifiersFactory::FrameId(frame));
...
void inspector_scroll_layer_event::Data(perfetto::TracedValue context, LayoutObject* layout_object)
{
    auto dict = std::move(context).WriteDictionary();
    dict.Add("frame", IdentifiersFactory::FrameId(layout_object->GetFrame()));
    SetGeneratingNodeInfo(dict, layout_object, "nodeId");
...
namespace {
void FillLocation(perfetto::TracedDictionary& dict, const String& url, const TextPosition& text_position)
{
    dict.Add("url", url);
...
void inspector_evaluate_script_event::Data(
    perfetto::TracedValue context, v8::Isolate* isolate, LocalFrame* frame, const String& url, const TextPosition& text_position)
{
    auto dict = std::move(context).WriteDictionary();
...
void inspector_target_rundown_event::Data(
    perfetto::TracedValue context, ExecutionContext* execution_context, v8::Isolate* isolate, ScriptState* scriptState, int scriptId)
{
    // Target related info
...
void inspector_parse_script_event::Data(perfetto::TracedValue context, uint64_t identifier, const String& url)
{
    String request_id = IdentifiersFactory::RequestId(static_cast<ExecutionContext*>(nullptr), identifier);
    auto dict = std::move(context).WriteDictionary();
    dict.Add("requestId", request_id);
    dict.Add("url", url);
}

void inspector_deserialize_script_event::Data(perfetto::TracedValue context, uint64_t identifier, const String& url)
{
    String request_id = IdentifiersFactory::RequestId(static_cast<ExecutionContext*>(nullptr), identifier);
    auto dict = std::move(context).WriteDictionary();
    dict.Add("requestId", request_id);
...
inspector_compile_script_event::V8ConsumeCacheResult::V8ConsumeCacheResult(int cache_size, bool rejected, bool full)
...
void inspector_compile_script_event::Data(perfetto::TracedValue context, const String& url, const TextPosition& text_position,
    std::optional<V8ConsumeCacheResult> consume_cache_result, bool eager, bool streamed, ScriptStreamer::NotStreamingReason not_streaming_reason)
{
    auto dict = std::move(context).WriteDictionary();
    FillLocation(dict, url, text_position);
    ...
    if (!streamed) {
        dict.Add("notStreamedReason", NotStreamedReasonString(not_streaming_reason));
    }
}

void inspector_produce_script_cache_event::Data(perfetto::TracedValue context, const String& url, const TextPosition& text_position, int cache_size)
{
    auto dict = std::move(context).WriteDictionary();
    FillLocation(dict, url, text_position);
...
void inspector_handle_post_message_event::Data(perfetto::TracedValue context, ExecutionContext* execution_context, const MessageEvent& message_event)
{
    auto dict = std::move(context).WriteDictionary();
    dict.Add("traceId", base::NumberToString(message_event.GetTraceId()));
}

void inspector_schedule_post_message_event::Data(perfetto::TracedValue context, ExecutionContext* execution_context, uint64_t trace_id)
{
    auto dict = std::move(context).WriteDictionary();
...
void inspector_function_call_event::Data(perfetto::TracedValue trace_context, ExecutionContext* context, const v8::Local<v8::Function>& function)
{
    auto dict = std::move(trace_context).WriteDictionary();
    if (LocalFrame* frame = FrameForExecutionContext(context))
        dict.Add("frame", IdentifiersFactory::FrameId(frame));
    ...
    if (!function_name.IsEmpty() && function_name->IsString()) {
        dict.Add("functionName", ToCoreString(context->GetIsolate(), function_name.As<v8::String>()));
    }
    std::unique_ptr<SourceLocation> location = CaptureSourceLocation(context->GetIsolate(), original_function);
...
void inspector_paint_image_event::Data(perfetto::TracedValue context, const LayoutImage& layout_image, const gfx::RectF& src_rect, const gfx::RectF& dest_rect)
{
    auto dict = std::move(context).WriteDictionary();
...
void inspector_paint_image_event::Data(perfetto::TracedValue context, const LayoutObject& owning_layout_object, const StyleImage& style_image)
{
    auto dict = std::move(context).WriteDictionary();
...
void inspector_paint_image_event::Data(
    perfetto::TracedValue context, Node* node, const StyleImage& style_image, const gfx::RectF& src_rect, const gfx::RectF& dest_rect)
{
    auto dict = std::move(context).WriteDictionary();
    if (node)
...
void inspector_paint_image_event::Data(perfetto::TracedValue context, const LayoutObject* owning_layout_object, const ImageResourceContent& image_content)
{
    auto dict = std::move(context).WriteDictionary();
    SetGeneratingNodeInfo(dict, owning_layout_object, "nodeId");
...
void inspector_update_counters_event::Data(perfetto::TracedValue context, v8::Isolate* isolate)
{
    auto dict = std::move(context).WriteDictionary();
    if (IsMainThread()) {
        dict.Add("documents", InstanceCounters::CounterValue(InstanceCounters::kDocumentCounter));
        dict.Add("nodes", InstanceCounters::CounterValue(InstanceCounters::kNodeCounter));
        dict.Add("jsEventListeners", InstanceCounters::CounterValue(InstanceCounters::kJSEventListenerCounter));
    }
    dict.Add("jsHeapSizeUsed", static_cast<double>(UsedHeapSize(isolate)));
}

void inspector_invalidate_layout_event::Data(perfetto::TracedValue context, LocalFrame* frame, DOMNodeId nodeId)
{
    auto dict = std::move(context).WriteDictionary();
    dict.Add("frame", IdentifiersFactory::FrameId(frame));
...
void inspector_recalculate_styles_event::Data(perfetto::TracedValue context, LocalFrame* frame)
{
    auto dict = std::move(context).WriteDictionary();
    dict.Add("frame", IdentifiersFactory::FrameId(frame));
    SetCallStack(frame->DomWindow()->GetIsolate(), dict);
...
void inspector_event_dispatch_event::Data(perfetto::TracedValue context, const Event& event, v8::Isolate* isolate)
{
    auto dict = std::move(context).WriteDictionary();
    dict.Add("type", event.type());
    bool record_input_enabled;
    TRACE_EVENT_CATEGORY_GROUP_ENABLED(TRACE_DISABLED_BY_DEFAULT("devtools.timeline.inputs"), &record_input_enabled);
    if (record_input_enabled) {
        ...
        if (keyboard_event) {
            ...
            dict.Add("timestamp", keyboard_event->PlatformTimeStamp().since_origin().InMicroseconds());
        ...
        if (mouse_event || wheel_event) {
            ...
            dict.Add("timestamp", mouse_event->PlatformTimeStamp().since_origin().InMicroseconds());
...
void inspector_time_stamp_event::Data(perfetto::TracedValue trace_context, ExecutionContext* context, const String& message)
{
    auto dict = std::move(trace_context).WriteDictionary();
...
void inspector_tracing_session_id_for_worker_event::Data(perfetto::TracedValue context, const base::UnguessableToken& worker_devtools_token,
    const base::UnguessableToken& parent_devtools_token, const KURL& url, PlatformThreadId worker_thread_id)
{
    auto dict = std::move(context).WriteDictionary();
...
void inspector_tracing_started_in_frame::Data(perfetto::TracedValue context, const String& session_id, LocalFrame* frame)
{
    auto dict = std::move(context).WriteDictionary();
...
void inspector_set_layer_tree_id::Data(perfetto::TracedValue context, LocalFrame* frame)
{
    auto dict = std::move(context).WriteDictionary();
    dict.Add("frame", IdentifiersFactory::FrameId(frame));
    dict.Add("layerTreeId", frame->GetPage()->GetChromeClient().GetLayerTreeId(*frame));
}

void inspector_animation_event::Data(perfetto::TracedValue context, const Animation& animation)
{
    auto dict = std::move(context).WriteDictionary();
...
void inspector_animation_state_event::Data(perfetto::TracedValue context, const Animation& animation)
{
    auto dict = std::move(context).WriteDictionary();
    dict.Add("state", V8AnimationPlayState(animation.CalculateAnimationPlayState()).AsCStr());
}
...
void inspector_animation_compositor_event::Data(
    perfetto::TracedValue context, CompositorAnimations::FailureReasons failure_reasons, const PropertyHandleSet& unsupported_properties)
{
    auto dict = std::move(context).WriteDictionary();
    dict.Add("compositeFailed", failure_reasons);
    {
        ...
        for (const PropertyHandle& p : unsupported_properties) {
            unsupported_properties_array.Append(p.GetCSSPropertyName().ToAtomicString());
        }
    }
}

void inspector_hit_test_event::EndData(
    perfetto::TracedValue context, const HitTestRequest& request, const HitTestLocation& location, const HitTestResult& result)
{
    auto dict = std::move(context).WriteDictionary();
...
void inspector_async_task::Data(perfetto::TracedValue context, const StringView& name)
{
    auto dict = std::move(context).WriteDictionary();
...
void SchedulerBaseData(perfetto::TracedDictionary& dict, ExecutionContext* context, uint64_t task_id)
{
...
void inspector_scheduler_schedule_event::Data(
    perfetto::TracedValue trace_context, ExecutionContext* execution_context, uint64_t task_id, WebSchedulingPriority priority, std::optional<double> delay)
{
    auto dict = std::move(trace_context).WriteDictionary();
...
void inspector_scheduler_run_event::Data(
    perfetto::TracedValue trace_context, ExecutionContext* execution_context, uint64_t task_id, WebSchedulingPriority priority, std::optional<double> delay)
{
    auto dict = std::move(trace_context).WriteDictionary();
    SchedulerBaseData(dict, execution_context, task_id);
...
void inspector_scheduler_abort_event::Data(perfetto::TracedValue trace_context, ExecutionContext* execution_context, uint64_t task_id)
{
    auto dict = std::move(trace_context).WriteDictionary();
    SchedulerBaseData(dict, execution_context, task_id);
    SetCallStack(execution_context->GetIsolate(), dict);
