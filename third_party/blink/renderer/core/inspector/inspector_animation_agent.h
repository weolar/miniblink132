...
public:
    InspectorAnimationAgent(InspectedFrames*, InspectorCSSAgent*, v8_inspector::V8InspectorSession*);
    ...
    protocol::Response getCurrentTime(const String& id, double* current_time) override;
    protocol::Response setPaused(std::unique_ptr<protocol::Array<String>> animations, bool paused) override;
    protocol::Response setTiming(const String& animation_id, double duration, double delay) override;
    protocol::Response seekAnimations(std::unique_ptr<protocol::Array<String>> animations, double current_time) override;
    protocol::Response releaseAnimations(std::unique_ptr<protocol::Array<String>> animations) override;
    protocol::Response resolveAnimation(const String& animation_id, std::unique_ptr<v8_inspector::protocol::Runtime::API::RemoteObject>*) override;
    ...
    protocol::Response AssertAnimation(const String& id, blink::Animation*& result);

    static String AnimationDisplayName(const Animation& animation);
    void Trace(Visitor*) const override;

private:
    struct AnimationKeyframeSnapshot : public GarbageCollected<AnimationKeyframeSnapshot> {
    ...
    bool CompareAndUpdateInternalSnapshot(blink::Animation& animation, AnimationSnapshot* snapshot);
    bool CompareAndUpdateKeyframesSnapshot(KeyframeEffect* keyframe_effect, HeapVector<Member<AnimationKeyframeSnapshot>>* snapshot_keyframes);
