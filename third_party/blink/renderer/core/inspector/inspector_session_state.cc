...
//
InspectorSessionState::InspectorSessionState(mojom::blink::DevToolsSessionStatePtr reattach)
    : reattach_state_(std::move(reattach))
    , updates_(mojom::blink::DevToolsSessionState::New())
{
}

const mojom::blink::DevToolsSessionState* InspectorSessionState::ReattachState() const
{
    return reattach_state_.get();
}

void InspectorSessionState::EnqueueUpdate(const WTF::String& key, const WebVector<uint8_t>* value)
{
...
/*static*/
void InspectorAgentState::Serialize(const WTF::String& v, WebVector<uint8_t>* out)
{
    auto encode = out->ReleaseVector();
    if (v.Is8Bit()) {
        auto span8 = v.Span8();
        ...
        EncodeFromUTF16(span<uint16_t>(reinterpret_cast<const uint16_t*>(span16.data()), span16.size()), &encode);
    }
    *out = std::move(encode);
}
...
bool InspectorAgentState::Deserialize(span<uint8_t> in, WTF::String* v)
{
    ...
    if (tokenizer.TokenTag() == CBORTokenTag::STRING8) {
        span<uint8_t> tokenizer_span = tokenizer.GetString8();
        base::span<const uint8_t> tokenizer_span_const(tokenizer_span.data(), tokenizer_span.size());
        *v = WTF::String::FromUTF8(tokenizer_span_const);
        return true;
    }
    if (tokenizer.TokenTag() == CBORTokenTag::STRING16) {
        ...
        *v = WTF::String(UNSAFE_BUFFERS(base::span(reinterpret_cast<const UChar*>(data.data()), data.size() / 2)));
        return true;
    }
    return false;
}
...
/*static*/
void InspectorAgentState::Serialize(const std::vector<uint8_t>& v, WebVector<uint8_t>* out)
{
...
/*static*/
bool InspectorAgentState::Deserialize(span<uint8_t> in, std::vector<uint8_t>* v)
{
...
{
    WTF::String prefix_key = domain_name_ + "." + WTF::String::Number(fields_.size()) + "/";
