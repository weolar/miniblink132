...
bool EncodeAsImage(char* body, size_t size, const String& encoding, const double quality, Vector<unsigned char>* output)
{
    const gfx::Size maximum_size = gfx::Size(kMaximumEncodeImageWidthInPixels, kMaximumEncodeImageHeightInPixels);
    SkBitmap bitmap = WebImage::FromData(WebData(body, size), maximum_size);
    if (bitmap.isNull())
        return false;

    SkImageInfo info = SkImageInfo::Make(bitmap.width(), bitmap.height(), kRGBA_8888_SkColorType, kUnpremul_SkAlphaType);
    uint32_t row_bytes = static_cast<uint32_t>(info.minRowBytes());
    Vector<unsigned char> pixel_storage(base::checked_cast<wtf_size_t>(info.computeByteSize(row_bytes)));
    ...
    std::unique_ptr<ImageDataBuffer> image_to_encode = ImageDataBuffer::Create(pixmap);
    if (!image_to_encode)
        return false;
...
std::unique_ptr<protocol::Audits::InspectorIssue> CreateLowTextContrastIssue(const ContrastInfo& info)
{
    ...
    auto low_contrast_details = protocol::Audits::LowTextContrastIssueDetails::create()
...
InspectorAuditsAgent::InspectorAuditsAgent(
    InspectorNetworkAgent* network_agent, InspectorIssueStorage* storage, InspectedFrames* inspected_frames, WebAutofillClient* web_autofill_client)
...
protocol::Response InspectorAuditsAgent::getEncodedResponse(const String& request_id, const String& encoding, Maybe<double> quality, Maybe<bool> size_only,
    Maybe<protocol::Binary>* out_body, int* out_original_size, int* out_encoded_size)
{
    DCHECK(encoding == encoding_enum::Jpeg || encoding == encoding_enum::Png || encoding == encoding_enum::Webp);

    String body;
    bool is_base64_encoded;
    protocol::Response response = network_agent_->GetResponseBody(request_id, &body, &is_base64_encoded);
    if (!response.IsSuccess())
        return response;

    Vector<char> base64_decoded_buffer;
    if (!is_base64_encoded || !Base64Decode(body, base64_decoded_buffer) || base64_decoded_buffer.size() == 0) {
        return protocol::Response::ServerError("Failed to decode original image");
    }

    Vector<unsigned char> encoded_image;
    if (!EncodeAsImage(base64_decoded_buffer.data(), base64_decoded_buffer.size(), encoding, quality.value_or(kDefaultEncodeQuality), &encoded_image)) {
        return protocol::Response::ServerError("Could not encode image with given settings");
    }
...
void InspectorAuditsAgent::CheckContrastForDocument(Document* document, bool report_aaa)
{
    InspectorContrast contrast(document);
    unsigned max_elements = 100;
    for (ContrastInfo info : contrast.GetElementsWithContrastIssues(report_aaa, max_elements)) {
        GetFrontend()->issueAdded(CreateLowTextContrastIssue(info));
    }
    GetFrontend()->flush();
}

protocol::Response InspectorAuditsAgent::checkContrast(protocol::Maybe<bool> report_aaa)
{
    if (!inspected_frames_) {
        return protocol::Response::ServerError("Inspected frames are not available");
    }
...
protocol::Response InspectorAuditsAgent::checkFormsIssues(std::unique_ptr<protocol::Array<protocol::Audits::GenericIssueDetails>>* out_formIssues)
{
    *out_formIssues = std::make_unique<protocol::Array<protocol::Audits::GenericIssueDetails>>();
...
void InspectorAuditsAgent::InspectorIssueAdded(protocol::Audits::InspectorIssue* issue)
{
