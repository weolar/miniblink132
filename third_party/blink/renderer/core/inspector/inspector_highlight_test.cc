...
void AssertValueEqualsJSON(const std::unique_ptr<protocol::Value>& actual_value, const std::string& json_expected)
{
    std::string json_actual;
    auto status_to_json = crdtp::json::ConvertCBORToJSON(crdtp::SpanFrom(actual_value->Serialize()), &json_actual);
...
TEST_F(InspectorHighlightTest, BuildSnapContainerInfoSnapAreas)
{
    ...
    AssertValueEqualsJSON(protocol::ValueConversions<protocol::Value>::fromValue(info.get(), &errors), expected_container);
}
...
TEST_F(InspectorHighlightTest, BuildContainerQueryContainerInfoWithoutDescendants)
{
    GetDocument().body()->setInnerHTML(R"HTML(
    <style>
      #container {
    ...
    auto info = BuildContainerQueryContainerInfo(container, InspectorContainerQueryContainerHighlightConfig(), 1.0f);
    EXPECT_TRUE(info);
...
TEST_F(InspectorHighlightTest, BuildContainerQueryContainerInfoWithDescendants)
{
    GetDocument().body()->setInnerHTML(R"HTML(
    <style>
      #container {
    ...
    auto info = BuildContainerQueryContainerInfo(container, highlight_config, 1.0f);
    EXPECT_TRUE(info);
...
TEST_F(InspectorHighlightTest, BuildIsolatedElementInfo)
{
    ...
    auto info = BuildIsolatedElementInfo(*element, InspectorIsolationModeHighlightConfig(), 1.0f);
    EXPECT_TRUE(info);
    ...
    AssertValueEqualsJSON(protocol::ValueConversions<protocol::Value>::fromValue(info.get(), &errors), expected_isolated_element);
}
...
static std::string GetBackgroundColorFromElementInfo(Element* element)
{
    ...
    protocol::ErrorSupport errors;
    auto actual_value = protocol::ValueConversions<protocol::Value>::fromValue(info.get(), &errors);
...
TEST_F(InspectorHighlightTest, BuildElementInfo_Colors)
{
    ...
    GetDocument().View()->UpdateAllLifecyclePhasesForTest();
    EXPECT_THAT(GetBackgroundColorFromElementInfo(GetDocument().getElementById(AtomicString("lab"))), Eq("lab(100 0 0)"));
    EXPECT_THAT(GetBackgroundColorFromElementInfo(GetDocument().getElementById(AtomicString("color"))), Eq("color(display-p3 0.5 0.5 0.5)"));
    EXPECT_THAT(GetBackgroundColorFromElementInfo(GetDocument().getElementById(AtomicString("hex"))), Eq("#FF00FFFF"));
    EXPECT_THAT(GetBackgroundColorFromElementInfo(GetDocument().getElementById(AtomicString("rgb"))), Eq("#808080FF"));
    EXPECT_THAT(GetBackgroundColorFromElementInfo(GetDocument().getElementById(AtomicString("var"))), Eq("lab(20 -10 -10)"));
}
...
TEST_F(InspectorHighlightTest, GridLineNames)
{
    ...
    EXPECT_TRUE(subgrid);
    auto info = InspectorGridHighlight(subgrid, InspectorHighlight::DefaultGridConfig());
    EXPECT_TRUE(info);

    auto GetLineNames = [](protocol::ListValue* row_or_column_list) {
        Vector<String> ret;
        for (wtf_size_t i = 0; i < row_or_column_list->size(); ++i) {
            protocol::DictionaryValue* current_value = static_cast<protocol::DictionaryValue*>(row_or_column_list->at(i));
    ...
    EXPECT_THAT(GetLineNames(info->getArray("rowLineNameOffsets")), UnorderedElementsAre("d", "d_sub", "e", "e_sub", "f", "f_sub"));
    EXPECT_THAT(GetLineNames(info->getArray("columnLineNameOffsets")), UnorderedElementsAre("a", "a_sub", "b", "b_sub", "c", "c_sub"));
}
...
TEST_F(InspectorHighlightTest, GridAreaNames)
{
    ...
    auto CompareAreaNames = [](protocol::DictionaryValue* area_names, WTF::Vector<WTF::String>& expected_names) -> void {
    ...
    protocol::DictionaryValue* grid_area_names = grid_info->getObject("areaNames");
    ...
    Node* subgrid = GetDocument().getElementById(AtomicString("subgrid"));
    EXPECT_TRUE(subgrid);
    auto subgrid_info = InspectorGridHighlight(subgrid, InspectorHighlight::DefaultGridConfig());
    EXPECT_TRUE(subgrid_info);

    protocol::DictionaryValue* subgrid_area_names = subgrid_info->getObject("areaNames");
    EXPECT_EQ(subgrid_area_names->size(), 6u);

    WTF::Vector<WTF::String> expected_subgrid_area_names = { "a", "b", "c", "d", "e", "f" };
