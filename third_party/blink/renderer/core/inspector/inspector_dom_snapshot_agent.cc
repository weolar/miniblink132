...
std::unique_ptr<protocol::Array<double>> BuildRectForPhysicalRect(const PhysicalRect& rect)
{
    return std::make_unique<std::vector<double>, std::initializer_list<double>>({ rect.X(), rect.Y(), rect.Width(), rect.Height() });
}

std::unique_ptr<protocol::Array<double>> BuildRectForLayout(const int x, const int y, const int width, const int height)
{
    return std::make_unique<std::vector<double>, std::initializer_list<double>>(
        { static_cast<double>(x), static_cast<double>(y), static_cast<double>(width), static_cast<double>(height) });
}

Document* GetEmbeddedDocument(PaintLayer* layer)
{
    // Documents are embedded on their own PaintLayer via a LayoutEmbeddedContent.
    if (auto* embedded = DynamicTo<LayoutEmbeddedContent>(layer->GetLayoutObject())) {
...
{
    return protocol::DOMSnapshot::RareBooleanData::create().setIndex(std::make_unique<protocol::Array<int>>()).build();
}
...
public:
    DOMTreeIterator(Node* root, int root_node_id)
        : current_(root)
        , path_to_current_node_({ root_node_id })
    {
        DCHECK(current_);
    }
    ...
    {
        DCHECK(current_);
        const bool skip_shadow_root = current_->GetShadowRoot() && current_->GetShadowRoot()->IsUserAgent();
        if (Node* first_child = skip_shadow_root ? current_->firstChild() : FlatTreeTraversal::FirstChild(*current_)) {
        ...
        while (current_) {
            const bool in_ua_shadow_tree = current_->ParentElementShadowRoot() && current_->ParentElementShadowRoot()->IsUserAgent();
            if (Node* node = in_ua_shadow_tree ? current_->nextSibling() : FlatTreeTraversal::NextSibling(*current_)) {
                path_to_current_node_.back() = next_node_id;
                current_ = node;
                return;
            }
            current_ = in_ua_shadow_tree ? current_->parentNode() : FlatTreeTraversal::Parent(*current_);
    ...
    {
        return path_to_current_node_.size() > 1 ? *(path_to_current_node_.rbegin() + 1) : -1;
    }
...
// static
PhysicalRect InspectorDOMSnapshotAgent::RectInDocument(const LayoutObject* layout_object)
{
...
// static
PhysicalRect InspectorDOMSnapshotAgent::TextFragmentRectInDocument(const LayoutObject* layout_object, const LayoutText::TextBoxInfo& text_box)
{
    PhysicalRect absolute_coords_text_box_rect = layout_object->LocalToAbsoluteRect(text_box.local_rect);
    LocalFrameView* local_frame_view = layout_object->GetFrameView();
    return local_frame_view ? local_frame_view->FrameToDocument(absolute_coords_text_box_rect) : absolute_coords_text_box_rect;
}

InspectorDOMSnapshotAgent::InspectorDOMSnapshotAgent(InspectedFrames* inspected_frames, InspectorDOMDebuggerAgent* dom_debugger_agent)
...
void InspectorDOMSnapshotAgent::CharacterDataModified(CharacterData* character_data)
{
...
{
    if (!enabled_.Get()) {
        return protocol::Response::ServerError("DOM snapshot agent hasn't been enabled.");
    }
    enabled_.Clear();
...
protocol::Response InspectorDOMSnapshotAgent::getSnapshot(std::unique_ptr<protocol::Array<String>> style_filter, protocol::Maybe<bool> include_event_listeners,
    protocol::Maybe<bool> include_paint_order, protocol::Maybe<bool> include_user_agent_shadow_tree,
    std::unique_ptr<protocol::Array<protocol::DOMSnapshot::DOMNode>>* dom_nodes,
    std::unique_ptr<protocol::Array<protocol::DOMSnapshot::LayoutTreeNode>>* layout_tree_nodes,
    std::unique_ptr<protocol::Array<protocol::DOMSnapshot::ComputedStyle>>* computed_styles)
{
    Document* document = inspected_frames_->Root()->GetDocument();
    if (!document)
        return protocol::Response::ServerError("Document is not available");
    LegacyDOMSnapshotAgent legacySupport(dom_debugger_agent_, origin_url_map_.get());
    return legacySupport.GetSnapshot(document, std::move(style_filter), std::move(include_event_listeners), std::move(include_paint_order),
        std::move(include_user_agent_shadow_tree), dom_nodes, layout_tree_nodes, computed_styles);
}

protocol::Response InspectorDOMSnapshotAgent::captureSnapshot(std::unique_ptr<protocol::Array<String>> computed_styles,
    protocol::Maybe<bool> include_paint_order, protocol::Maybe<bool> include_dom_rects, protocol::Maybe<bool> include_blended_background_colors,
    protocol::Maybe<bool> include_text_color_opacities, std::unique_ptr<protocol::Array<protocol::DOMSnapshot::DocumentSnapshot>>* documents,
    std::unique_ptr<protocol::Array<String>>* strings)
{
    ...
    inspected_frames_->Root()->View()->UpdateLifecycleToCompositingInputsClean(DocumentUpdateReason::kInspector);

    strings_ = std::make_unique<protocol::Array<String>>();
    documents_ = std::make_unique<protocol::Array<protocol::DOMSnapshot::DocumentSnapshot>>();
    ...
    for (String& property_name : *computed_styles) {
        const CSSPropertyID id = UnresolvedCSSPropertyID(main_window, property_name);
    ...
    if (include_paint_order.value_or(false)) {
        paint_order_map_ = InspectorDOMSnapshotAgent::BuildPaintLayerTree(main_window->document());
    }

    include_snapshot_dom_rects_ = include_dom_rects.value_or(false);
    include_blended_background_colors_ = include_blended_background_colors.value_or(false);
...
void InspectorDOMSnapshotAgent::SetRare(protocol::DOMSnapshot::RareIntegerData* data, int index, int value)
{
    data->getIndex()->emplace_back(index);
    data->getValue()->emplace_back(value);
}

void InspectorDOMSnapshotAgent::SetRare(protocol::DOMSnapshot::RareStringData* data, int index, const String& value)
{
    data->getIndex()->emplace_back(index);
    data->getValue()->emplace_back(AddString(value));
}

void InspectorDOMSnapshotAgent::SetRare(protocol::DOMSnapshot::RareBooleanData* data, int index)
{
    data->getIndex()->emplace_back(index);
}
...
void InspectorDOMSnapshotAgent::VisitDocument(Document* document)
{
    ...
    document_ = protocol::DOMSnapshot::DocumentSnapshot::create()
                    .setDocumentURL(AddString(InspectorDOMAgent::DocumentURLString(document)))
                    .setBaseURL(AddString(InspectorDOMAgent::DocumentBaseURLString(document)))
                    ...
                    .setFrameId(AddString(IdentifiersFactory::FrameId(document->GetFrame())))
                    ...
                    .setNodes(protocol::DOMSnapshot::NodeTreeSnapshot::create()
                                  ...
                                  .setAttributes(std::make_unique<protocol::Array<protocol::Array<int>>>())
                                  ...
                                  .build())
                    .setLayout(protocol::DOMSnapshot::LayoutTreeSnapshot::create()
                                   .setNodeIndex(std::make_unique<protocol::Array<int>>())
                                   .setBounds(std::make_unique<protocol::Array<protocol::Array<double>>>())
                                   ...
                                   .build())
                    .setTextBoxes(protocol::DOMSnapshot::TextBoxSnapshot::create()
    ...
    if (paint_order_map_) {
        document_->getLayout()->setPaintOrders(std::make_unique<protocol::Array<int>>());
    }
    if (include_snapshot_dom_rects_) {
        document_->getLayout()->setOffsetRects(std::make_unique<protocol::Array<protocol::Array<double>>>());
        document_->getLayout()->setClientRects(std::make_unique<protocol::Array<protocol::Array<double>>>());
        document_->getLayout()->setScrollRects(std::make_unique<protocol::Array<protocol::Array<double>>>());
    }

    if (include_blended_background_colors_) {
        document_->getLayout()->setBlendedBackgroundColors(std::make_unique<protocol::Array<int>>());
    }
    if (include_text_color_opacities_) {
        document_->getLayout()->setTextColorOpacities(std::make_unique<protocol::Array<double>>());
    }

    auto* node_names = document_->getNodes()->getNodeName(nullptr);
    // Note: node_names->size() changes as the loop runs.
    for (DOMTreeIterator it(document, base::checked_cast<int>(node_names->size())); it.CurrentNode(); it.Advance(base::checked_cast<int>(node_names->size()))) {
...
void InspectorDOMSnapshotAgent::VisitNode(Node* node, int parent_index, InspectorContrast& contrast)
{
    String node_value;
    ...
    nodes->getNodeType(nullptr)->emplace_back(static_cast<int>(node->getNodeType()));
    ...
    if (node->IsInShadowTree()) {
        SetRare(nodes->getShadowRootType(nullptr), index, InspectorDOMAgent::GetShadowRootType(node->ContainingShadowRoot()));
    }
    nodes->getBackendNodeId(nullptr)->emplace_back(IdentifiersFactory::IntIdForNode(node));
    nodes->getAttributes(nullptr)->emplace_back(BuildArrayForElementAttributes(node));
    ...
    if (element) {
        ...
        if (auto* input_element = DynamicTo<HTMLInputElement>(*element)) {
            ...
            if ((input_element->FormControlType() == FormControlType::kInputRadio) || (input_element->FormControlType() == FormControlType::kInputCheckbox)) {
        ...
        if (element->IsPseudoElement()) {
            SetRare(nodes->getPseudoType(nullptr), index, InspectorDOMAgent::ProtocolPseudoElementType(element->GetPseudoIdForStyling()));
        ...
        if (image_element) {
            SetRare(nodes->getCurrentSourceURL(nullptr), index, image_element->currentSrc());
        }
    }
}

void InspectorDOMSnapshotAgent::VisitPseudoElements(Element* parent, int parent_index, InspectorContrast& contrast)
{
    for (PseudoId pseudo_id : { kPseudoIdFirstLetter, kPseudoIdCheck, kPseudoIdBefore, kPseudoIdAfter, kPseudoIdSelectArrow, kPseudoIdMarker }) {
        if (Node* pseudo_node = parent->GetPseudoElement(pseudo_id))
            VisitNode(pseudo_node, parent_index, contrast);
    }
}

std::unique_ptr<protocol::Array<int>> InspectorDOMSnapshotAgent::BuildArrayForElementAttributes(Node* node)
{
...
int InspectorDOMSnapshotAgent::BuildLayoutTreeNode(LayoutObject* layout_object, Node* node, int node_index, InspectorContrast& contrast)
{
    if (!layout_object)
        return -1;
    ...
    int layout_index = static_cast<int>(layout_tree_snapshot->getNodeIndex()->size());
    ...
    layout_tree_snapshot->getBounds()->emplace_back(BuildRectForPhysicalRect(InspectorDOMSnapshotAgent::RectInDocument(layout_object)));

    if (include_snapshot_dom_rects_) {
        protocol::Array<protocol::Array<double>>* offsetRects = layout_tree_snapshot->getOffsetRects(nullptr);
        DCHECK(offsetRects);

        protocol::Array<protocol::Array<double>>* clientRects = layout_tree_snapshot->getClientRects(nullptr);
        DCHECK(clientRects);

        protocol::Array<protocol::Array<double>>* scrollRects = layout_tree_snapshot->getScrollRects(nullptr);
        DCHECK(scrollRects);

        if (auto* element = DynamicTo<Element>(node)) {
            offsetRects->emplace_back(BuildRectForLayout(element->OffsetLeft(), element->OffsetTop(), element->OffsetWidth(), element->OffsetHeight()));

            clientRects->emplace_back(BuildRectForLayout(element->clientLeft(), element->clientTop(), element->clientWidth(), element->clientHeight()));

            scrollRects->emplace_back(BuildRectForLayout(element->scrollLeft(), element->scrollTop(), element->scrollWidth(), element->scrollHeight()));
        } else {
    ...
    if (include_blended_background_colors_ || include_text_color_opacities_) {
        ...
        if (include_blended_background_colors_) {
            if (colors.size()) {
                layout_tree_snapshot->getBlendedBackgroundColors(nullptr)->emplace_back(AddString(colors[0].SerializeAsCSSColor()));
            } else {
                layout_tree_snapshot->getBlendedBackgroundColors(nullptr)->emplace_back(-1);
            }
        }
        if (include_text_color_opacities_) {
            layout_tree_snapshot->getTextColorOpacities(nullptr)->emplace_back(opacity);
        }
    }
    ...
    if (paint_order_map_) {
        ...
        if (paint_order != paint_order_map_->end()) {
            layout_tree_snapshot->getPaintOrders(nullptr)->emplace_back(paint_order->value);
        } else {
    ...
    String text = layout_object->IsText() ? To<LayoutText>(layout_object)->TransformedText() : String();
    ...
    for (const auto& text_box : text_boxes) {
        ...
        text_box_snapshot->getBounds()->emplace_back(BuildRectForPhysicalRect(InspectorDOMSnapshotAgent::TextFragmentRectInDocument(layout_object, text_box)));
...
std::unique_ptr<protocol::Array<int>> InspectorDOMSnapshotAgent::BuildStylesForNode(Node* node)
{
    DCHECK(!node->GetDocument().NeedsLayoutTreeUpdateForNodeIncludingDisplayLocked(*node));
    auto result = std::make_unique<protocol::Array<int>>();
    ...
    for (const auto* property : *css_property_filter_) {
        const CSSValue* value = property->CSSValueFromComputedStyle(*style, layout_object, /* allow_visited_style= */ true, CSSValuePhase::kResolvedValue);
        if (!value) {
...
// static
InspectorDOMSnapshotAgent::PaintOrderMap* InspectorDOMSnapshotAgent::BuildPaintLayerTree(Document* document)
{
...
// static
void InspectorDOMSnapshotAgent::TraversePaintLayerTree(Document* document, PaintOrderMap* paint_order_map)
{
...
// static
void InspectorDOMSnapshotAgent::VisitPaintLayer(PaintLayer* layer, PaintOrderMap* paint_order_map)
{
