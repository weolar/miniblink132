...
protected:
    void SetUp() override
    {
        dummy_page_holder_ = std::make_unique<DummyPageHolder>(gfx::Size(), nullptr, nullptr);
        impl = MediaInspectorContextImpl::From(*dummy_page_holder_->GetFrame().DomWindow());
    }
...
TEST_F(InspectorMediaContextImplTest, KillsPlayersInCorrectOrder)
{
    ...
    impl->NotifyPlayerEvents(alive_player_id, MakeEvents(kMaxCachedPlayerEvents - 32));
