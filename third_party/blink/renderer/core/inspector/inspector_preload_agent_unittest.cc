...
TEST_F(InspectorPreloadAgentTest, OutOfDocumentSpeculationRules)
{
    ...
    auto* source = SpeculationRuleSet::Source::FromRequest(source_text, KURL("https://example.com/speculationrules.js"), 42);
...
TEST_F(InspectorPreloadAgentTest, NoRequestIdIfInvalidId)
{
    ...
    auto* source = SpeculationRuleSet::Source::FromRequest(source_text, KURL("https://example.com/speculationrules.js"), 0);
    auto* rule_set = SpeculationRuleSet::Parse(source, execution_context());
    CHECK(rule_set);
