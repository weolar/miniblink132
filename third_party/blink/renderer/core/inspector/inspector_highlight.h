...
public:
    ...
    {
        return color == Color::kTransparent;
    }
...
public:
    ...
    bool IsFullyTransparent() const
    {
        return fill_color == Color::kTransparent && hatch_color == Color::kTransparent;
    }
...
public:
    ...
    std::unique_ptr<InspectorFlexContainerHighlightConfig> flex_container_highlight_config;
    std::unique_ptr<InspectorFlexItemHighlightConfig> flex_item_highlight_config;
    std::unique_ptr<InspectorContainerQueryContainerHighlightConfig> container_query_container_highlight_config;
};
...
public:
    ...
    void AppendPath(std::unique_ptr<protocol::ListValue> path, const Color& fill_color, const Color& outline_color, const String& name = String());
    void AppendQuad(const gfx::QuadF&, const Color& fill_color, const Color& outline_color = Color::kTransparent, const String& name = String());
    virtual std::unique_ptr<protocol::DictionaryValue> AsProtocolValue() const = 0;

protected:
    static bool BuildNodeQuads(Node*, gfx::QuadF* content, gfx::QuadF* padding, gfx::QuadF* border, gfx::QuadF* margin);
    std::unique_ptr<protocol::ListValue> highlight_paths_;
    float scale_;
};

class CORE_EXPORT InspectorSourceOrderHighlight : public InspectorHighlightBase {
    STACK_ALLOCATED();
...
public:
    InspectorHighlight(Node*, const InspectorHighlightConfig&, const InspectorHighlightContrastInfo&, bool append_element_info, bool append_distance_info,
        NodeContentVisibilityState content_visibility_state);
    explicit InspectorHighlight(float scale);
    ~InspectorHighlight();

    static bool GetBoxModel(Node*, std::unique_ptr<protocol::DOM::BoxModel>*, bool use_absolute_zoom);
    static bool GetContentQuads(Node*, std::unique_ptr<protocol::Array<protocol::Array<double>>>*);
    ...
    void AppendEventTargetQuads(Node* event_target_node, const InspectorHighlightConfig&);
    std::unique_ptr<protocol::DictionaryValue> AsProtocolValue() const override;
...
private:
    ...
    void VisitAndCollectDistanceInfo(PseudoId pseudo_id, LayoutObject* layout_object);
...
std::unique_ptr<protocol::DictionaryValue> CORE_EXPORT InspectorGridHighlight(Node*, const InspectorGridHighlightConfig& config);

std::unique_ptr<protocol::DictionaryValue> CORE_EXPORT BuildSnapContainerInfo(Node* node);

std::unique_ptr<protocol::DictionaryValue> CORE_EXPORT BuildContainerQueryContainerInfo(
    Node* node, const InspectorContainerQueryContainerHighlightConfig& container_query_container_highlight_config, float scale);

std::unique_ptr<protocol::DictionaryValue> CORE_EXPORT BuildIsolatedElementInfo(
    Element& element, const InspectorIsolationModeHighlightConfig& config, float scale);

void CORE_EXPORT AppendStyleInfo(Element* element, protocol::DictionaryValue* element_info, const InspectorHighlightContrastInfo& node_contrast,
    const ContrastAlgorithm& contrast_algorithm);

std::unique_ptr<protocol::DictionaryValue> CORE_EXPORT BuildElementInfo(Element* element);
} // namespace blink
