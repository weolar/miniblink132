...
template <typename Func> void ForEachRule(CSSRule& rule, Func func)
{
...
template <typename Func> void ForEachRule(CSSStyleSheet& sheet, Func func)
{
...
template <typename T> bool HasNestedDeclarationsAtIndex(T& rule, wtf_size_t index)
{
    if (index == kNotFound || index >= rule.length()) {
        return false;
    }
    return rule.ItemInternal(index)->GetType() == CSSRule::kNestedDeclarationsRule;
}

} // namespace

void InspectorGhostRules::PopulateSheet(const ExecutionContext& execution_context, CSSStyleSheet& sheet)
{
    ForEachRule(sheet, [&](auto& rule) {
        ...
        static_assert(std::is_same_v<Type, CSSStyleRule> || std::is_same_v<Type, CSSGroupingRule>);
        ...
        for (wtf_size_t i = rule.length(); i != end_index; --i) {
            if (HasNestedDeclarationsAtIndex(rule, i) || HasNestedDeclarationsAtIndex(rule, i - 1)) {
...
void InspectorGhostRules::DepopulateSheet(CSSStyleSheet& sheet)
{
    ForEachRule(sheet, [&](auto& rule) {
        ...
        for (wtf_size_t i = rule.length() - 1; i != kNotFound; --i) {
            auto* nested_declarations_rule = DynamicTo<CSSNestedDeclarationsRule>(rule.ItemInternal(i));
            if (nested_declarations_rule && inserted_rules_.Contains(nested_declarations_rule)) {
