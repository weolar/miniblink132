...
String StringUtil::fromUTF16LE(const uint16_t* data, size_t length)
{
    ...
    // `length` are safe.
    return String(UNSAFE_BUFFERS(base::span(reinterpret_cast<const UChar*>(data), length)));
}
...
public:
    explicit BinaryBasedOnCachedData(std::unique_ptr<v8::ScriptCompiler::CachedData> data)
        : data_(std::move(data))
    {
    }
...
{
    return impl_ ? Base64Encode(base::span<const uint8_t>(impl_->data(), impl_->size())) : String();
}
...
// static
Binary Binary::fromCachedData(std::unique_ptr<v8::ScriptCompiler::CachedData> data)
{
...
// static
bool ProtocolTypeTraits<WTF::String>::Deserialize(DeserializerState* state, String* value)
{
...
// static
void ProtocolTypeTraits<WTF::String>::Serialize(const String& value, std::vector<uint8_t>* bytes)
{
    if (value.length() == 0) {
    ...
    if (value.Is8Bit()) {
        crdtp::cbor::EncodeFromLatin1(span<uint8_t>(reinterpret_cast<const uint8_t*>(value.Characters8()), value.length()), bytes);
        return;
    }
    crdtp::cbor::EncodeFromUTF16(span<uint16_t>(reinterpret_cast<const uint16_t*>(value.Characters16()), value.length()), bytes);
}

// static
bool ProtocolTypeTraits<blink::protocol::Binary>::Deserialize(DeserializerState* state, blink::protocol::Binary* value)
{
    auto* tokenizer = state->tokenizer();
    if (tokenizer->TokenTag() == crdtp::cbor::CBORTokenTag::BINARY) {
        span<uint8_t> token = tokenizer->GetBinary();
        base::span<const uint8_t> data(token.data(), token.size());
        *value = Binary::fromSpan(data);
        return true;
    }
    if (tokenizer->TokenTag() == crdtp::cbor::CBORTokenTag::STRING8) {
...
// static
void ProtocolTypeTraits<blink::protocol::Binary>::Serialize(const blink::protocol::Binary& value, std::vector<uint8_t>* bytes)
{
