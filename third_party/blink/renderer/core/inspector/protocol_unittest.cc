...
TEST(ProtocolStringTest, HelloWorldBase64Roundtrip)
{
    ...
    Vector<uint8_t> msg;
    msg.Append(reinterpret_cast<const uint8_t*>(kMsg), static_cast<wtf_size_t>(strlen(kMsg)));
    ...
    decoded.Append(decoded_binary.data(), static_cast<wtf_size_t>(decoded_binary.size()));
