...
const char MediaInspectorContextImpl::kSupplementName[] = "MediaInspectorContextImpl";

// static
MediaInspectorContextImpl* MediaInspectorContextImpl::From(ExecutionContext& execution_context)
{
    auto* context = Supplement<ExecutionContext>::From<MediaInspectorContextImpl>(execution_context);
    if (!context) {
        context = MakeGarbageCollected<MediaInspectorContextImpl>(execution_context);
...
template <typename T, typename Iterable> static Vector<T> Iter2Vector(const Iterable& iterable)
{
    Vector<T> result;
...
const MediaPlayer& MediaInspectorContextImpl::MediaPlayerFromId(const WebString& player_id)
{
...
{
    String next_player_id = String::FromUTF8(base::UnguessableToken::Create().ToString());
    players_.insert(next_player_id, MakeGarbageCollected<MediaPlayer>());
    probe::PlayersCreated(GetSupplementable(), { next_player_id });
    if (!GetSupplementable()->GetProbeSink() || !GetSupplementable()->GetProbeSink()->HasInspectorMediaAgents()) {
...
void MediaInspectorContextImpl::RemovePlayer(const WebString& playerId)
{
    ...
    if (player) {
        total_event_count_ -= player->errors.size() + player->events.size() + player->messages.size();
...
void MediaInspectorContextImpl::NotifyPlayerErrors(WebString playerId, const InspectorPlayerErrors& errors)
{
    ...
    Vector<InspectorPlayerError> vector = Iter2Vector<InspectorPlayerError>(errors);
    probe::PlayerErrorsRaised(GetSupplementable(), playerId, vector);
}

void MediaInspectorContextImpl::NotifyPlayerEvents(WebString playerId, const InspectorPlayerEvents& events)
{
    const auto& player = players_.find(playerId);
    if (player != players_.end()) {
    ...
    Vector<InspectorPlayerEvent> vector = Iter2Vector<InspectorPlayerEvent>(events);
    probe::PlayerEventsAdded(GetSupplementable(), playerId, vector);
}

void MediaInspectorContextImpl::SetPlayerProperties(WebString playerId, const InspectorPlayerProperties& props)
{
    const auto& player = players_.find(playerId);
...
void MediaInspectorContextImpl::NotifyPlayerMessages(WebString playerId, const InspectorPlayerMessages& messages)
{
    const auto& player = players_.find(playerId);
    if (player != players_.end()) {
    ...
    Vector<InspectorPlayerMessage> vector = Iter2Vector<InspectorPlayerMessage>(messages);
    probe::PlayerMessagesLogged(GetSupplementable(), playerId, vector);
}

HeapHashMap<String, Member<MediaPlayer>>* MediaInspectorContextImpl::GetPlayersForTesting()
{
    return &players_;
}
