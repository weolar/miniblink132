...
InspectorTaskRunner::InspectorTaskRunner(scoped_refptr<base::SingleThreadTaskRunner> isolate_task_runner)
...
bool InspectorTaskRunner::AppendTask(Task task)
{
    ...
    PostCrossThreadTask(
        *isolate_task_runner_, FROM_HERE, CrossThreadBindOnce(&InspectorTaskRunner::PerformSingleInterruptingTaskDontWait, WrapRefCounted(this)));
    if (isolate_) {
        AddRef();
...
InspectorTaskRunner::Task InspectorTaskRunner::WaitForNextInterruptingTaskOrQuitRequest()
{
    base::AutoLock locker(lock_);
