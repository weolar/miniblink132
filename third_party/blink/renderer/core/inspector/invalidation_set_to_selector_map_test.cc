...
protected:
    ...
    void TearDown() override
    {
        ...
        InvalidationSetToSelectorMap::StartOrStopTrackingIfNeeded(GetDocument().GetStyleEngine());
        CHECK(GetInstance() == nullptr);
    ...
    void StartTracing()
    {
        trace_analyzer::Start(TRACE_DISABLED_BY_DEFAULT("devtools.timeline.invalidationTracking"));
    }
...
TEST_F(InvalidationSetToSelectorMapTest, TrackerLifetime)
{
    ...
    GetElementById("d")->setAttribute(html_names::kStyleAttr, AtomicString("color: red"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_NE(GetInstance(), nullptr);

    StopTracing();
    GetElementById("d")->setAttribute(html_names::kStyleAttr, AtomicString("color: green"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(GetInstance(), nullptr);

    StartTracingWithoutInvalidationTracking();
    GetElementById("d")->setAttribute(html_names::kStyleAttr, AtomicString("color: blue"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(GetInstance(), nullptr);
    StopTracing();
}
...
TEST_F(InvalidationSetToSelectorMapTest, ClassMatch)
{
    ...
    GetElementById("parent")->setAttribute(html_names::kClassAttr, AtomicString("b"));
    UpdateAllLifecyclePhasesForTest();

    auto analyzer = StopTracing();
    trace_analyzer::TraceEventVector events;
    analyzer->FindEvents(trace_analyzer::Query::EventNameIs("StyleInvalidatorInvalidationTracking"), &events);
...
TEST_F(InvalidationSetToSelectorMapTest, SelfInvalidation)
{
    ...
    analyzer->FindEvents(
        trace_analyzer::Query::EventNameIs("ScheduleStyleInvalidationTracking") || trace_analyzer::Query::EventNameIs("StyleInvalidatorInvalidationTracking"),
        &events);
    ASSERT_EQ(events.size(), 4u);
    EXPECT_EQ(events[0]->name, "ScheduleStyleInvalidationTracking");
    EXPECT_EQ(*(events[0]->GetKnownArgAsDict("data").FindString("invalidatedSelectorId")), "class");
    EXPECT_EQ(*(events[0]->GetKnownArgAsDict("data").FindString("changedClass")), "b");
    EXPECT_EQ(events[1]->name, "ScheduleStyleInvalidationTracking");
    EXPECT_EQ(*(events[1]->GetKnownArgAsDict("data").FindString("invalidatedSelectorId")), "class");
    EXPECT_EQ(*(events[1]->GetKnownArgAsDict("data").FindString("changedClass")), "a");
    ...
    EXPECT_EQ(*(events[2]->GetKnownArgAsDict("data").FindString("reason")), "Invalidation set invalidates self");
    EXPECT_EQ(events[3]->name, "StyleInvalidatorInvalidationTracking");
    EXPECT_EQ(*(events[3]->GetKnownArgAsDict("data").FindString("reason")), "Invalidation set invalidates self");
}
...
TEST_F(InvalidationSetToSelectorMapTest, SubtreeInvalidation)
{
    ...
    for (auto event : events) {
        ...
        base::Value::Dict data_dict = event->GetKnownArgAsDict("data");
        std::string* reason = data_dict.FindString("reason");
        if (reason != nullptr && *reason == "Invalidation set invalidates subtree") {
            base::Value::List* selector_list = data_dict.FindList("selectors");
            if (selector_list != nullptr) {
                EXPECT_EQ(selector_list->size(), 1u);
...
TEST_F(InvalidationSetToSelectorMapTest, InvalidationSetRemoval)
{
    ...
    StyleRule* style_rule = To<StyleRule>(css_test_helpers::ParseRule(GetDocument(), ".a .b { color: red; }"));
    ...
    InvalidationSet* invalidation_set = DescendantInvalidationSet::Create().release();
    InvalidationSetToSelectorMap::RecordInvalidationSetEntry(invalidation_set, SelectorFeatureType::kClass, class_name);
    InvalidationSetToSelectorMap::EndSelector();

    const IndexedSelectorList* result = InvalidationSetToSelectorMap::Lookup(invalidation_set, SelectorFeatureType::kClass, class_name);
    EXPECT_TRUE(result->Contains(MakeGarbageCollected<IndexedSelector>(style_rule, 0)));
    ...
    result = InvalidationSetToSelectorMap::Lookup(invalidation_set, SelectorFeatureType::kClass, class_name);
...
TEST_F(InvalidationSetToSelectorMapTest, StartTracingLateWithSiblingAndDescendantRules)
{
    SetBodyInnerHTML(R"HTML(
    <style>
      .a ~ .b { color: red; }
...
TEST_F(InvalidationSetToSelectorMapTest, StartTracingLateWithPendingInsertRule)
{
    SetBodyInnerHTML(R"HTML(
    <style id=target>
      .a .b { color: red; }
    ...
    CSSStyleSheet* sheet = To<HTMLStyleElement>(GetElementById("target"))->sheet();
...
TEST_F(InvalidationSetToSelectorMapTest, HandleRebuildAfterRuleSetChange)
{
    ...
    GetDocument().body()->appendChild(GetDocument().CreateRawElement(html_names::kDivTag));
    UpdateAllLifecyclePhasesForTest();
