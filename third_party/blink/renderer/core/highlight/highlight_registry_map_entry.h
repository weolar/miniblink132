...
struct HighlightRegistryMapEntry final : public GarbageCollected<HighlightRegistryMapEntry> {
    HighlightRegistryMapEntry(const AtomicString& highlight_name, Member<Highlight> highlight)
        : highlight(highlight)
...
struct HighlightRegistryMapEntryNameTranslator {
    ...
    static bool Equal(const HighlightRegistryMapEntry* entry, const AtomicString& name)
    {
        DCHECK(entry);
...
template <> struct HashTraits<blink::Member<blink::HighlightRegistryMapEntry>> : MemberHashTraits<blink::HighlightRegistryMapEntry> {
    ...
    static inline unsigned GetHash(const blink::Member<blink::HighlightRegistryMapEntry>& key)
    {
        DCHECK(key);
        return WTF::GetHash(key->highlight_name);
    }
    static inline bool Equal(const blink::Member<blink::HighlightRegistryMapEntry>& a, const blink::Member<blink::HighlightRegistryMapEntry>& b)
    {
        DCHECK(a && b);
        return HashTraits<AtomicString>::Equal(a->highlight_name, b->highlight_name);
    }
