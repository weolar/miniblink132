...
public:
    Highlight* CreateHighlight(Text* node_start, int start, Text* node_end, int end)
    {
        auto* range = MakeGarbageCollected<Range>(GetDocument(), node_start, start, node_end, end);
...
TEST_F(HighlightRegistryTest, CompareStacking)
{
    ...
    EXPECT_EQ(HighlightRegistry::kOverlayStackingPositionEquivalent,
        registry->CompareOverlayStackingPosition(highlight1_name, highlight1, highlight1_name, highlight1));
    EXPECT_EQ(
        HighlightRegistry::kOverlayStackingPositionBelow, registry->CompareOverlayStackingPosition(highlight1_name, highlight1, highlight2_name, highlight2));
    EXPECT_EQ(
        HighlightRegistry::kOverlayStackingPositionAbove, registry->CompareOverlayStackingPosition(highlight2_name, highlight2, highlight1_name, highlight1));
    highlight1->setPriority(2);
    highlight1->setPriority(1);
    EXPECT_EQ(
        HighlightRegistry::kOverlayStackingPositionAbove, registry->CompareOverlayStackingPosition(highlight1_name, highlight1, highlight2_name, highlight2));
}

TEST_F(HighlightRegistryTest, ValidateMarkers)
{
    GetDocument().body()->setInnerHTML("<p>aaaaaaaaaa</p><p>bbbbbbbbbb</p><p>cccccccccc</p>");
    auto* dom_window = GetDocument().domWindow();
    HighlightRegistry* registry = HighlightRegistry::From(*dom_window);
    ...
    auto* range_aa = MakeGarbageCollected<Range>(GetDocument(), text_a, 0, text_a, 4);
    range_vector_1.push_back(range_aa);
    auto* range_ab = MakeGarbageCollected<Range>(GetDocument(), text_a, 5, text_b, 4);
    ...
    auto* range_bb = MakeGarbageCollected<Range>(GetDocument(), text_b, 5, text_b, 8);
    ...
    auto* range_bc = MakeGarbageCollected<Range>(GetDocument(), text_b, 9, text_c, 4);
    range_vector_3.push_back(range_bc);
    auto* range_cc = MakeGarbageCollected<Range>(GetDocument(), text_c, 5, text_c, 9);
    ...
    DocumentMarkerVector text_a_markers = marker_controller.MarkersFor(*text_a, DocumentMarker::MarkerTypes::CustomHighlight());
    DocumentMarkerVector text_b_markers = marker_controller.MarkersFor(*text_b, DocumentMarker::MarkerTypes::CustomHighlight());
    DocumentMarkerVector text_c_markers = marker_controller.MarkersFor(*text_c, DocumentMarker::MarkerTypes::CustomHighlight());
    ...
    text_a_markers = marker_controller.MarkersFor(*text_a, DocumentMarker::MarkerTypes::CustomHighlight());
    text_b_markers = marker_controller.MarkersFor(*text_b, DocumentMarker::MarkerTypes::CustomHighlight());
    text_c_markers = marker_controller.MarkersFor(*text_c, DocumentMarker::MarkerTypes::CustomHighlight());
