...
Highlight* Highlight::addForBinding(ScriptState*, AbstractRange* range, ExceptionState&)
{
...
bool Highlight::deleteForBinding(ScriptState*, AbstractRange* range, ExceptionState&)
{
...
bool Highlight::hasForBinding(ScriptState*, AbstractRange* range, ExceptionState&) const
{
...
{
    auto map_iterator = containing_highlight_registries_.find(highlight_registry);
    CHECK_NE(map_iterator, containing_highlight_registries_.end(), base::NotFatalUntil::M130);
    DCHECK_GT(map_iterator->value, 0u);
...
Highlight::IterationSource::IterationSource(const Highlight& highlight)
    : index_(0)
{
    highlight_ranges_snapshot_.ReserveInitialCapacity(highlight.highlight_ranges_.size());
    for (const auto& range : highlight.highlight_ranges_) {
        highlight_ranges_snapshot_.push_back(range);
    }
}

bool Highlight::IterationSource::FetchNextItem(ScriptState*, AbstractRange*& value, ExceptionState&)
{
...
HighlightSetIterable::IterationSource* Highlight::CreateIterationSource(ScriptState*, ExceptionState&)
{
