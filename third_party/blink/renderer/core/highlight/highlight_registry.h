...
using HighlightRegistryMap = HeapLinkedHashSet<Member<HighlightRegistryMapEntry>>;
using HighlightRegistryMapIterable = Maplike<HighlightRegistry>;
class HighlightsFromPointOptions;
class LocalFrame;
class Text;

class CORE_EXPORT HighlightRegistry : public ScriptWrappable, public Supplement<LocalDOMWindow>, public HighlightRegistryMapIterable {
...
public:
    ...
    HighlightRegistry* setForBinding(ScriptState*, AtomicString, Member<Highlight>, ExceptionState&);
    ...
    const HighlightRegistryMap& GetHighlights() const
    {
        return highlights_;
    }
    ...
    class IterationSource final : public HighlightRegistryMapIterable::IterationSource {
    public:
        explicit IterationSource(const HighlightRegistry& highlight_registry);

        bool FetchNextItem(ScriptState* script_state, String& key, Highlight*& value, ExceptionState& exception_state) override;
    ...
    HeapVector<Member<Highlight>> highlightsFromPoint(float x, float y, const HighlightsFromPointOptions* options);

private:
    HighlightRegistryMap highlights_;
    Member<LocalFrame> frame_;
    // Only valid after ValidateHighlightMarkers(), used to optimize painting.
    HeapHashMap<WeakMember<const Text>, HashSet<AtomicString>> active_highlights_in_node_;
    ...
    bool GetMapEntry(ScriptState*, const String& key, Highlight*& value, ExceptionState&) override
    {
    ...
    HighlightRegistryMapIterable::IterationSource* CreateIterationSource(ScriptState*, ExceptionState&) override;
};
