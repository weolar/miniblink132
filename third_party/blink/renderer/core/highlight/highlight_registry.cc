...
{
    HighlightRegistry* supplement = Supplement<LocalDOMWindow>::From<HighlightRegistry>(window);
    if (!supplement) {
...
HighlightRegistry* HighlightRegistry::GetHighlightRegistry(const Node* node)
{
    ...
    return node->GetDocument().domWindow()->Supplementable<LocalDOMWindow>::RequireSupplement<HighlightRegistry>();
}
...
void HighlightRegistry::ValidateHighlightMarkers()
{
    ...
    // rebuilding them.
    if (dom_tree_version_for_validate_highlight_markers_ == document->DomTreeVersion()
        && style_version_for_validate_highlight_markers_ == document->StyleVersion() && !force_markers_validation_) {
        return;
    }
    ...
    markers_controller.RemoveMarkersOfTypes(DocumentMarker::MarkerTypes::CustomHighlight());
    ...
    for (const auto& highlight_registry_map_entry : highlights_) {
        ...
        for (const auto& abstract_range : highlight->GetRanges()) {
            if (abstract_range->OwnerDocument() == document && !abstract_range->collapsed()) {
                ...
                markers_controller.AddCustomHighlightMarker(eph_range, highlight_name, highlight);
            }
        }
    }
    ...
    // removed.
    markers_controller.ApplyToMarkersOfType(
        [&nodes_with_overflow, &active = active_highlights_in_node_](const Text& node, DocumentMarker* marker) {
            auto& highlight_marker = To<CustomHighlightMarker>(*marker);
            const auto& iterator = active.find(&node);
            if (iterator == active.end()) {
                active.insert(&node, HashSet<AtomicString>({ highlight_marker.GetHighlightName() }));
            } else {
                iterator->value.insert(highlight_marker.GetHighlightName());
            }
            bool has_visual_overflow = HighlightStyleUtils::CustomHighlightHasVisualOverflow(node, highlight_marker.GetHighlightName());
...
const HashSet<AtomicString>& HighlightRegistry::GetActiveHighlights(const Text& node) const
{
...
void HighlightRegistry::SetForTesting(AtomicString highlight_name, Highlight* highlight)
{
    ...
    highlights_.insert(MakeGarbageCollected<HighlightRegistryMapEntry>(highlight_name, highlight));
    highlight->RegisterIn(this);
    ScheduleRepaint();
}

void HighlightRegistry::RemoveForTesting(AtomicString highlight_name, Highlight* highlight)
{
    auto highlights_iterator = GetMapIterator(highlight_name);
    if (highlights_iterator != highlights_.end()) {
        highlights_iterator->Get()->highlight->DeregisterFrom(this);
...
HighlightRegistry* HighlightRegistry::setForBinding(
    ScriptState* script_state, AtomicString highlight_name, Member<Highlight> highlight, ExceptionState& exception_state)
{
    UseCounter::Count(ExecutionContext::From(script_state), WebFeature::kHighlightAPIRegisterHighlight);
...
bool HighlightRegistry::deleteForBinding(ScriptState*, const AtomicString& highlight_name, ExceptionState&)
{
    auto highlights_iterator = GetMapIterator(highlight_name);
    if (highlights_iterator != highlights_.end()) {
        highlights_iterator->Get()->highlight->DeregisterFrom(this);
...
int8_t HighlightRegistry::CompareOverlayStackingPosition(
    const AtomicString& highlight_name1, const Highlight* highlight1, const AtomicString& highlight_name2, const Highlight* highlight2) const
{
    ...
    return highlight1->priority() > highlight2->priority() ? kOverlayStackingPositionAbove : kOverlayStackingPositionBelow;
}

HighlightRegistry::IterationSource::IterationSource(const HighlightRegistry& highlight_registry)
    : index_(0)
{
    highlights_snapshot_.ReserveInitialCapacity(highlight_registry.highlights_.size());
    for (const auto& highlight_registry_map_entry : highlight_registry.highlights_) {
        highlights_snapshot_.push_back(MakeGarbageCollected<HighlightRegistryMapEntry>(highlight_registry_map_entry));
    }
}

bool HighlightRegistry::IterationSource::FetchNextItem(ScriptState*, String& key, Highlight*& value, ExceptionState&)
{
...
HighlightRegistryMapIterable::IterationSource* HighlightRegistry::CreateIterationSource(ScriptState*, ExceptionState&)
{
    return MakeGarbageCollected<IterationSource>(*this);
}

HeapVector<Member<Highlight>> HighlightRegistry::highlightsFromPoint(float x, float y, const HighlightsFromPointOptions* options)
{
