...
constexpr TextResourceDecoderOptions::ContentType kFetchLaterContentType = TextResourceDecoderOptions::kPlainTextContent;

constexpr net::NetworkTrafficAnnotationTag kFetchLaterTrafficAnnotationTag = net::DefineNetworkTrafficAnnotation("blink_fetch_later_manager",
    R"(
    semantics {
...
{
    return base::GetFieldTrialParamByFeatureAsBool(features::kFetchLaterAPI, "use_background_sync_permission", true);
}
...
{
    return base::GetFieldTrialParamByFeatureAsBool(features::kFetchLaterAPI, "send_on_enter_bfcache", true);
}
...
{
    return base::GetFieldTrialParamByFeatureAsBool(features::kFetchLaterAPI, "use_permissions_policy", false);
}
...
const char* SerializeTrustTokenOperationType(network::mojom::TrustTokenOperationType operation_type)
{
...
void HistogramNetErrorForTrustTokensOperation(network::mojom::blink::TrustTokenOperationType operation_type, int net_error)
{
    base::UmaHistogramSparse(base::StrCat({ "Net.TrustTokens.NetErrorForFetchFailure", ".", SerializeTrustTokenOperationType(operation_type) }), net_error);
}

ResourceLoadPriority ComputeFetchLaterLoadPriority(const FetchParameters& params)
{
    ...
    return AdjustPriorityWithPriorityHintAndRenderBlocking(
        ResourceLoadPriority::kHigh, kFetchLaterResourceType, params.GetResourceRequest().GetFetchPriorityHint(), params.GetRenderBlockingBehavior());
...
public:
    ...
    void RejectBecauseFailed(std::optional<String> devtools_request_id, std::optional<base::UnguessableToken> issue_id, std::optional<String> issue_summary);
    ...
    : resolver_(resolver)
{
    ...
    v8::Local<v8::Value> exception = V8ThrowException::CreateTypeError(isolate, "Failed to fetch");
...
void ResponseResolver::RejectBecauseFailed(
    std::optional<String> devtools_request_id, std::optional<base::UnguessableToken> issue_id, std::optional<String> issue_summary)
{
    CHECK(resolver_);
    auto* script_state = resolver_->GetScriptState();
...
public:
    explicit FetchLoaderBase(ExecutionContext* ec, FetchRequestData* data, ScriptState* script_state, AbortSignal* signal)
        : execution_context_(ec)
        , fetch_request_data_(data)
        , script_state_(script_state)
        , world_(std::move(&script_state->World()))
        , signal_(signal)
        , abort_handle_(signal->AddAlgorithm(WTF::BindOnce(&FetchLoaderBase::Abort, WrapWeakPersistent(this))))
    {
        CHECK(world_);
    }
...
protected:
    ...
    virtual void CreateLoader(ResourceRequest request, const ResourceLoaderOptions& resource_loader_options) = 0;
    virtual void Failed(const String& message, DOMException* dom_exception, std::optional<String> devtools_request_id = std::nullopt,
        std::optional<base::UnguessableToken> issue_id = std::nullopt, std::optional<String> issue_summary = std::nullopt)
        = 0;

    void PerformSchemeFetch(ExceptionState&);
    void PerformNetworkError(const String& message, std::optional<base::UnguessableToken> issue_id = std::nullopt);
    void FileIssueAndPerformNetworkError(RendererCorsIssueCode, int64_t identifier);
    void PerformHTTPFetch(ExceptionState&);
    void PerformDataFetch();
    bool AddConsoleMessage(const String& message, std::optional<base::UnguessableToken> issue_id);

    ExecutionContext* GetExecutionContext()
    {
        return execution_context_.Get();
    }
    void SetExecutionContext(ExecutionContext* ec)
    {
        execution_context_ = ec;
    }
...
class FetchManager::Loader final : public GarbageCollected<FetchManager::Loader>, public FetchLoaderBase, public ThreadableLoaderClient {
public:
    Loader(ExecutionContext*, FetchManager*, ScriptPromiseResolver<Response>*, FetchRequestData*, ScriptState*, AbortSignal*);
    ~Loader() override;
    ...
    bool WillFollowRedirect(uint64_t, const KURL&, const ResourceResponse&) override;
    ...
    class SRIVerifier final : public GarbageCollected<SRIVerifier>, public BytesConsumer::Client {
    public:
        SRIVerifier(BytesConsumer* body, PlaceHolderBytesConsumer* updater, Response* response, FetchManager::Loader* loader, String integrity_metadata,
            const KURL& url, FetchResponseType response_type)
            : body_(body)
            , updater_(updater)
            , response_(response)
            , loader_(loader)
        ...
        void OnStateChange() override
        {
            ...
            if (result == Result::kDone) {
                ...
                if (body_is_null
                    || (response_type_ != FetchResponseType::kBasic && response_type_ != FetchResponseType::kCors
                        && response_type_ != FetchResponseType::kDefault)) {
                    report_info.AddConsoleErrorMessage("Subresource Integrity: The resource '" + url_.ElidedString()
                ...
                if (check_result) {
                    check_result = SubresourceIntegrity::CheckSubresourceIntegrity(
                        integrity_metadata_, SubresourceIntegrityHelper::GetFeatures(loader_->GetExecutionContext()), &buffer_, url_, report_info);
                }
                SubresourceIntegrityHelper::DoReport(*loader_->GetExecutionContext(), report_info);
                if (check_result) {
                    updater_->Update(MakeGarbageCollected<FormDataBytesConsumer>(std::move(buffer_)));
                    loader_->response_resolver_->Resolve(response_);
                    loader_->response_resolver_.Clear();
                    return;
                }
            }
            String error_message = "Unknown error occurred while trying to verify integrity.";
            if (updater_) {
                updater_->Update(BytesConsumer::CreateErrored(BytesConsumer::Error(error_message)));
            }
...
private:
    ...
    void Abort() override;
    void NotifyFinished();
    void CreateLoader(ResourceRequest request, const ResourceLoaderOptions& resource_loader_options) override;
    // If |dom_exception| is provided, throws the specified DOMException instead
    // of the usual "Failed to fetch" TypeError.
    void Failed(const String& message, DOMException* dom_exception, std::optional<String> devtools_request_id = std::nullopt,
        std::optional<base::UnguessableToken> issue_id = std::nullopt, std::optional<String> issue_summary = std::nullopt) override;
...
FetchManager::Loader::Loader(ExecutionContext* execution_context, FetchManager* fetch_manager, ScriptPromiseResolver<Response>* resolver,
    FetchRequestData* fetch_request_data, ScriptState* script_state, AbortSignal* signal)
    : FetchLoaderBase(execution_context, fetch_request_data, script_state, signal)
...
bool FetchManager::Loader::WillFollowRedirect(uint64_t identifier, const KURL& url, const ResourceResponse& response)
{
...
void FetchManager::Loader::DidReceiveResponse(uint64_t, const ResourceResponse& response)
{
    ...
    DCHECK(response.CurrentRequestUrl() == url_list_.back()
        || (response.CurrentRequestUrl().ProtocolIs("https") && url_list_.back().ProtocolIs("http")
            && response.CurrentRequestUrl().Host() == url_list_.back().Host() && response.CurrentRequestUrl().GetPath() == url_list_.back().GetPath()
    ...
    if (response.MimeType() == "application/wasm"
        && (response.CurrentRequestUrl().ProtocolIsInHTTPFamily()
            || CommonSchemeRegistry::IsExtensionScheme(response.CurrentRequestUrl().Protocol().Ascii()))) {
        // We create a ScriptCachedMetadataHandler for WASM modules.
        cached_metadata_handler_ = MakeGarbageCollected<ScriptCachedMetadataHandler>(
            WTF::TextEncoding(), CachedMetadataSender::Create(response, mojom::blink::CodeCacheType::kWebAssembly, GetExecutionContext()->GetSecurityOrigin()));
    }

    place_holder_body_ = MakeGarbageCollected<PlaceHolderBytesConsumer>();
    FetchResponseData* response_data
        = FetchResponseData::CreateWithBuffer(BodyStreamBuffer::Create(GetScriptState(), place_holder_body_, Signal(), cached_metadata_handler_));
    if (!GetExecutionContext() || GetExecutionContext()->IsContextDestroyed() || response.GetType() == FetchResponseType::kError) {
    ...
    DCHECK(!(network_utils::IsRedirectResponseCode(response_http_status_code_) && HasNonEmptyLocationHeader(response_data->HeaderList())
        && GetFetchRequestData()->Redirect() != RedirectMode::kManual));

    if (network_utils::IsRedirectResponseCode(response_http_status_code_) && GetFetchRequestData()->Redirect() == RedirectMode::kManual) {
        response_type = network::mojom::FetchResponseType::kOpaqueRedirect;
    }

    response_data->InitFromResourceResponse(
        GetExecutionContext(), response_type, url_list_, GetFetchRequestData()->Method(), GetFetchRequestData()->Credentials(), response);
    ...
    case FetchResponseType::kCors: {
        HTTPHeaderSet header_names = cors::ExtractCorsExposedHeaderNamesList(GetFetchRequestData()->Credentials(), response);
        tainted_response = response_data->CreateCorsFilteredResponse(header_names);
        break;
    }
    ...
    Response* r = Response::Create(response_resolver_->GetExecutionContext(), tainted_response);
    ...
    if (GetFetchRequestData()->Integrity().empty()) {
        ...
        integrity_verifier_ = MakeGarbageCollected<SRIVerifier>(
            underlying, verified, r, this, GetFetchRequestData()->Integrity(), response.CurrentRequestUrl(), r->GetResponse()->GetType());
    }
}
...
{
    if (GetFetchRequestData()->Integrity().empty() && !response_has_no_store_header_) {
        ...
        place_holder_body_->Update(BufferingBytesConsumer::CreateWithDelay(&body, GetExecutionContext()->GetTaskRunner(TaskType::kNetworking)));
    } else {
...
void FetchManager::Loader::DidFinishLoading(uint64_t)
{
    ...
    if (window && window->GetFrame() && network::IsSuccessfulStatus(response_http_status_code_)) {
        window->GetFrame()->GetPage()->GetChromeClient().AjaxSucceeded(window->GetFrame());
    }
    NotifyFinished();
}

void FetchManager::Loader::DidFail(uint64_t identifier, const ResourceError& error)
{
    if (GetFetchRequestData() && GetFetchRequestData()->TrustTokenParams()) {
        HistogramNetErrorForTrustTokensOperation(GetFetchRequestData()->TrustTokenParams()->operation, error.ErrorCode());
    }

    if (error.TrustTokenOperationError() != network::mojom::blink::TrustTokenOperationStatus::kOk) {
        Failed(String(), TrustTokenErrorToDOMException(error.TrustTokenOperationError()), IdentifiersFactory::SubresourceRequestId(identifier));
        return;
    }
    ...
    if (const auto& cors_error_status = error.CorsErrorStatus()) {
        ...
        if (base::FeatureList::IsEnabled(features::kDevToolsImprovedNetworkError)) {
            issue_summary = cors::GetErrorStringForIssueSummary(*cors_error_status, fetch_initiator_type_names::kFetch);
        }
    }
    Failed(String(), nullptr, IdentifiersFactory::SubresourceRequestId(identifier), issue_id, issue_summary);
}

void FetchManager::Loader::DidFailRedirectCheck(uint64_t identifier)
{
    Failed(String(), nullptr, IdentifiersFactory::SubresourceRequestId(identifier));
}
...
void FetchLoaderBase::Start(ExceptionState& exception_state)
{
    ...
    if (!execution_context_->GetContentSecurityPolicyForWorld(world_.Get())
             ->AllowConnectToSource(fetch_request_data_->Url(), fetch_request_data_->Url(), RedirectStatus::kNoRedirect)) {
        // "A network error."
        PerformNetworkError(
            "Refused to connect to '" + fetch_request_data_->Url().ElidedString() + "' because it violates the document's Content Security Policy.");
        return;
    }
    ...
    if (fetch_request_data_->Origin()->CanReadContent(url)
        || (fetch_request_data_->IsolatedWorldOrigin() && fetch_request_data_->IsolatedWorldOrigin()->CanReadContent(url))
    ...
    if (fetch_request_data_->Mode() == RequestMode::kSameOrigin) {
        ...
        FileIssueAndPerformNetworkError(RendererCorsIssueCode::kDisallowedByMode, CreateUniqueIdentifier());
        return;
    }
    ...
    if (fetch_request_data_->Mode() == RequestMode::kNoCors) {
        ...
        if (fetch_request_data_->Redirect() != RedirectMode::kFollow) {
            ...
            // one.
            FileIssueAndPerformNetworkError(RendererCorsIssueCode::kNoCorsRedirectModeNotFollow, CreateUniqueIdentifier());
            return;
        }
    ...
    if (!SchemeRegistry::ShouldTreatURLSchemeAsSupportingFetchAPI(fetch_request_data_->Url().Protocol())) {
        // This error is so early that there isn't an identifier yet, generate one.
        FileIssueAndPerformNetworkError(RendererCorsIssueCode::kCorsDisabledScheme, CreateUniqueIdentifier());
        return;
    }
...
void FetchLoaderBase::PerformSchemeFetch(ExceptionState& exception_state)
{
    ...
    if (SchemeRegistry::ShouldTreatURLSchemeAsSupportingFetchAPI(fetch_request_data_->Url().Protocol()) || fetch_request_data_->Url().ProtocolIs("blob")) {
...
void FetchLoaderBase::FileIssueAndPerformNetworkError(RendererCorsIssueCode network_error, int64_t identifier)
{
    auto issue_id = base::UnguessableToken::Create();
    switch (network_error) {
    case RendererCorsIssueCode::kCorsDisabledScheme: {
        AuditsIssue::ReportCorsIssue(execution_context_, identifier, network_error, fetch_request_data_->Url().GetString(),
            fetch_request_data_->Origin()->ToString(), fetch_request_data_->Url().Protocol(), issue_id);
        PerformNetworkError("Fetch API cannot load " + fetch_request_data_->Url().GetString() + ". URL scheme \"" + fetch_request_data_->Url().Protocol()
                + "\" is not supported.",
            issue_id);
        break;
    }
    case RendererCorsIssueCode::kDisallowedByMode: {
        AuditsIssue::ReportCorsIssue(execution_context_, identifier, network_error, fetch_request_data_->Url().GetString(),
            fetch_request_data_->Origin()->ToString(), WTF::g_empty_string, issue_id);
        PerformNetworkError("Fetch API cannot load " + fetch_request_data_->Url().GetString()
...
void FetchLoaderBase::PerformNetworkError(const String& message, std::optional<base::UnguessableToken> issue_id)
{
...
void FetchLoaderBase::PerformHTTPFetch(ExceptionState& exception_state)
{
    ...
    request.SetNavigationRedirectChain(fetch_request_data_->NavigationRedirectChain());
    ...
    for (const auto& header : fetch_request_data_->HeaderList()->List()) {
        request.AddHttpHeaderField(AtomicString(header.first), AtomicString(header.second));
    }

    if (fetch_request_data_->Method() != http_names::kGET && fetch_request_data_->Method() != http_names::kHEAD) {
        if (fetch_request_data_->Buffer()) {
            scoped_refptr<EncodedFormData> form_data = fetch_request_data_->Buffer()->DrainAsFormData(exception_state);
            if (form_data) {
                request.SetHttpBody(form_data);
            } else if (RuntimeEnabledFeatures::FetchUploadStreamingEnabled(execution_context_)) {
                UseCounter::Count(execution_context_, WebFeature::kFetchUploadStreaming);
                DCHECK(!fetch_request_data_->Buffer()->IsStreamLocked());
                mojo::PendingRemote<network::mojom::blink::ChunkedDataPipeGetter> pending_remote;
                fetch_request_data_->Buffer()->DrainAsChunkedDataPipeGetter(script_state_, pending_remote.InitWithNewPipeAndPassReceiver(),
                    /*client=*/nullptr);
    ...
    request.SetAttributionReportingEligibility(fetch_request_data_->AttributionReportingEligibility());
    request.SetAttributionReportingSupport(fetch_request_data_->AttributionSupport());
    request.SetSharedStorageWritableOptedIn(fetch_request_data_->SharedStorageWritable());

    request.SetOriginalDestination(fetch_request_data_->OriginalDestination());

    request.SetServiceWorkerRaceNetworkRequestToken(fetch_request_data_->ServiceWorkerRaceNetworkRequestToken());

    request.SetFetchLaterAPI(IsDeferred());

    if (execution_context_->IsSharedWorkerGlobalScope() && DynamicTo<SharedWorkerGlobalScope>(*execution_context_)->DoesRequireCrossSiteRequestForCookies()) {
    ...
    resource_loader_options.initiator_info.name = fetch_initiator_type_names::kFetch;
    resource_loader_options.data_buffering_policy = kDoNotBufferData;
    if (fetch_request_data_->URLLoaderFactory()) {
        mojo::PendingRemote<network::mojom::blink::URLLoaderFactory> factory_clone;
        fetch_request_data_->URLLoaderFactory()->Clone(factory_clone.InitWithNewPipeAndPassReceiver());
        resource_loader_options.url_loader_factory
            = base::MakeRefCounted<base::RefCountedData<mojo::PendingRemote<network::mojom::blink::URLLoaderFactory>>>(std::move(factory_clone));
    }

    if (fetch_request_data_->Keepalive() && !request.IsFetchLaterAPI()) {
        FetchUtils::LogFetchKeepAliveRequestMetric(request.GetRequestContext(), FetchUtils::FetchKeepAliveRequestState::kTotal);
    }
...
void FetchManager::Loader::CreateLoader(ResourceRequest request, const ResourceLoaderOptions& resource_loader_options)
{
    threadable_loader_ = MakeGarbageCollected<ThreadableLoader>(*GetExecutionContext(), this, resource_loader_options);
    threadable_loader_->Start(std::move(request));
}

bool FetchLoaderBase::AddConsoleMessage(const String& message, std::optional<base::UnguessableToken> issue_id)
{
    ...
    if (!message.empty()) {
        ...
        auto* console_message
            = MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kJavaScript, mojom::blink::ConsoleMessageLevel::kError, message);
        if (issue_id) {
...
void FetchManager::Loader::Failed(const String& message, DOMException* dom_exception, std::optional<String> devtools_request_id,
    std::optional<base::UnguessableToken> issue_id, std::optional<String> issue_summary)
{
    ...
    if (response_resolver_) {
        ...
        if (dom_exception) {
            ...
            response_resolver_->RejectBecauseFailed(std::move(devtools_request_id), issue_id, std::move(issue_summary));
...
void FetchManager::Loader::LogIfKeepalive(std::string_view request_state) const
{
    return;
    ...
    base::UmaHistogramMediumTimes(base::StrCat({ "FetchKeepAlive.RequestDuration.", request_state }), duration);
}
...
class FetchLaterManager::DeferredLoader final : public GarbageCollected<FetchLaterManager::DeferredLoader>, public FetchLoaderBase {
public:
    DeferredLoader(ExecutionContext* ec, FetchLaterManager* fetch_later_manager, FetchRequestData* fetch_request_data, ScriptState* script_state,
        AbortSignal* signal, const std::optional<base::TimeDelta>& activate_after)
        ...
        , timer_(ec->GetTaskRunner(FetchLaterManager::kTaskType), this, &DeferredLoader::TimerFired)
        , loader_(ec)
    {
    ...
    {
        return fetch_later_result_.Get();
    }
    ...
    {
        return GetFetchRequestData()->Buffer() && SecurityOrigin::AreSameOrigin(GetFetchRequestData()->Url(), url) ? GetFetchRequestData()->BufferByteLength()
                                                                                                                   : 0;
    }
    ...
    // For testing only:
    void RecreateTimerForTesting(scoped_refptr<base::SingleThreadTaskRunner> task_runner, const base::TickClock* tick_clock)
    {
        timer_.Stop();
...
private:
    enum class InvokeState { DEFERRED, ABORTED, ACTIVATED };
    void SetInvokeState(InvokeState state)
    {
        switch (state) {
        case InvokeState::DEFERRED:
            UseCounter::Count(GetExecutionContext(), WebFeature::kFetchLaterInvokeStateDeferred);
            break;
        case InvokeState::ABORTED:
            UseCounter::Count(GetExecutionContext(), WebFeature::kFetchLaterInvokeStateAborted);
            break;
        case InvokeState::ACTIVATED:
            UseCounter::Count(GetExecutionContext(), WebFeature::kFetchLaterInvokeStateActivated);
            break;
        default:
            NOTREACHED();
        };
    ...
    void CreateLoader(ResourceRequest request, const ResourceLoaderOptions& resource_loader_options) override
    {
        ...
        std::unique_ptr<network::ResourceRequest> network_request = fetch_later_manager_->PrepareNetworkRequest(std::move(request), resource_loader_options);
        if (!network_request) {
            Failed(/*message=*/String(), /*dom_exception=*/nullptr);
            return;
        }
        ...
        factory->CreateFetchLaterLoader(loader_.BindNewEndpointAndPassReceiver(GetExecutionContext()->GetTaskRunner(FetchLaterManager::kTaskType)), request_id,
            url_loader_options, *network_request, net::MutableNetworkTrafficAnnotationTag(kFetchLaterTrafficAnnotationTag));
        CHECK(loader_.is_bound());
        loader_.set_disconnect_handler(WTF::BindOnce(&DeferredLoader::NotifyFinished, WrapWeakPersistent(this)));
...
ScriptPromise<Response> FetchManager::Fetch(ScriptState* script_state, FetchRequestData* request, AbortSignal* signal, ExceptionState& exception_state)
{
    DCHECK(signal);
    if (signal->aborted()) {
        return ScriptPromise<Response>::Reject(script_state, signal->reason(script_state));
    }

    request->SetDestination(network::mojom::RequestDestination::kEmpty);

    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<Response>>(script_state, exception_state.GetContext());
    auto promise = resolver->Promise();

    auto* loader = MakeGarbageCollected<Loader>(GetExecutionContext(), this, resolver, request, script_state, signal);
...
FetchLaterResult* FetchLaterManager::FetchLater(ScriptState* script_state, FetchRequestData* request, AbortSignal* signal,
    std::optional<DOMHighResTimeStamp> activate_after_ms, ExceptionState& exception_state)
{
    ...
    if (signal->aborted()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kAbortError, "The user aborted a fetchLater request.");
        return nullptr;
    }
    ...
    if (activate_after_ms.has_value()) {
        ...
        if (activate_after->is_negative()) {
            exception_state.ThrowRangeError("fetchLater's activateAfter cannot be negative.");
            return nullptr;
        }
    }
    ...
    if (!DomWindow() || GetExecutionContext()->is_in_back_forward_cache()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "fetchLater can only be called from a fully active Document.");
        return nullptr;
    }
    ...
    if (!network::IsUrlPotentiallyTrustworthy(GURL(request->Url()))) {
        exception_state.ThrowSecurityError("fetchLater was passed an insecure URL.");
        return nullptr;
    }
    ...
    if (IsFetchLaterUsePermissionsPolicyEnabled()
        && !GetExecutionContext()->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kDeferredFetch, ReportOptions::kReportOnFailure)) {
    ...
    if (request->Buffer()) {
        ...
        if (request->BufferByteLength() == 0) {
            UseCounter::Count(GetExecutionContext(), WebFeature::kFetchLaterErrorUnknownBodyLength);
            exception_state.ThrowTypeError("fetchLater doesn't support body with unknown length.");
            return nullptr;
        }
    ...
    if (bytes_for_origin > kMaxScheduledDeferredBytesPerOrigin) {
        UseCounter::Count(GetExecutionContext(), WebFeature::kFetchLaterErrorQuotaExceeded);
        exception_state.ThrowDOMException(DOMExceptionCode::kQuotaExceededError, "fetchLater exceeds its quota for the origin.");
        return nullptr;
    }
    ...
    for (const auto& deferred_loader : deferred_loaders_) {
        ...
        bytes_for_origin += deferred_loader->GetDeferredBytesForUrlOrigin(request->Url());
    ...
    // `request`.
    auto* deferred_loader = MakeGarbageCollected<DeferredLoader>(GetExecutionContext(), this, request, script_state, signal, activate_after);
    ...
    , permission_observer_receiver_(this, ec)
{
    ...
    if (IsFetchLaterUseBackgroundSyncPermissionEnabled()) {
        auto* permission_service = DomWindow()->document()->GetPermissionService(ec);
        ...
        permission_service->AddPermissionObserver(std::move(descriptor), background_sync_permission_, std::move(observer));
    }
}
...
void FetchLaterManager::ContextEnteredBackForwardCache()
{
    ...
    if (IsFetchLaterSendOnEnterBackForwardCacheEnabled() || (IsFetchLaterUseBackgroundSyncPermissionEnabled() && !IsBackgroundSyncGranted())) {
        for (auto& deferred_loader : deferred_loaders_) {
            deferred_loader->Process(FetchLaterRendererMetricType::kActivatedOnEnteredBackForwardCache);
            deferred_loader->Dispose();
        }
        deferred_loaders_.clear();
    }
...
void FetchLaterManager::OnDeferredLoaderFinished(DeferredLoader* deferred_loader)
{
...
void FetchLaterManager::OnPermissionStatusChange(mojom::blink::PermissionStatus status)
{
...
void FetchLaterManager::RecreateTimerForTesting(scoped_refptr<base::SingleThreadTaskRunner> task_runner, const base::TickClock* tick_clock)
{
    for (auto& deferred_loader : deferred_loaders_) {
...
// static
ResourceLoadPriority FetchLaterManager::ComputeLoadPriorityForTesting(const FetchParameters& params)
{
    return ComputeFetchLaterLoadPriority(params);
}

std::unique_ptr<network::ResourceRequest> FetchLaterManager::PrepareNetworkRequest(ResourceRequest request, const ResourceLoaderOptions& options) const
{
    ...
    const FetchClientSettingsObject& fetch_client_settings_object = fetcher->GetProperties().GetFetchClientSettingsObject();
    ...
    if (!RuntimeEnabledFeatures::MinimimalResourceRequestPrepBeforeCacheLookupEnabled()) {
        if (PrepareResourceRequest(
                kFetchLaterResourceType, fetch_client_settings_object, params, fetcher->Context(), unused_virtual_time_pauser, resource_request_context, KURL())
            != std::nullopt) {
            return nullptr;
        }
    } else {
        if (PrepareResourceRequestForCacheAccess(
                kFetchLaterResourceType, fetch_client_settings_object, KURL(), resource_request_context, fetcher->Context(), params)
            != std::nullopt) {
            return nullptr;
        }
        UpgradeResourceRequestForLoaderNew(kFetchLaterResourceType, params, fetcher->Context(), resource_request_context, unused_virtual_time_pauser);
    }

    // From `ResourceFetcher::StartLoad()`:
    ScriptForbiddenScope script_forbidden_scope;
    auto network_resource_request = std::make_unique<network::ResourceRequest>();
    PopulateResourceRequest(params.GetResourceRequest(), std::move(params.MutableResourceRequest().MutableBody()), network_resource_request.get());
