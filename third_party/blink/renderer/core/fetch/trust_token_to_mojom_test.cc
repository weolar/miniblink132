...
TEST(TrustTokenToMojomTest, Issuance)
{
    ...
    EXPECT_TRUE(ConvertTrustTokenToMojomAndCheckPermissions(*pt, { .issuance_enabled = true, .redemption_enabled = true }, &e, params.get()));
    EXPECT_EQ(params->operation, network::mojom::blink::TrustTokenOperationType::kIssuance);
}
...
TEST(TrustTokenToMojomTest, IssuanceDenied)
{
    ...
    DummyExceptionStateForTesting e;
    EXPECT_FALSE(ConvertTrustTokenToMojomAndCheckPermissions(*pt, { .issuance_enabled = false, .redemption_enabled = true }, &e, params.get()));
...
TEST(TrustTokenToMojomTest, Redemption)
{
    ...
    DummyExceptionStateForTesting e;
    EXPECT_TRUE(ConvertTrustTokenToMojomAndCheckPermissions(*pt, { .issuance_enabled = true, .redemption_enabled = true }, &e, params.get()));
    EXPECT_EQ(params->operation, network::mojom::blink::TrustTokenOperationType::kRedemption);
}
...
TEST(TrustTokenToMojomTest, RedemptionDenied)
{
    ...
    auto params = network::mojom::blink::TrustTokenParams::New();
    DummyExceptionStateForTesting e;
    EXPECT_FALSE(ConvertTrustTokenToMojomAndCheckPermissions(*pt, { .issuance_enabled = true, .redemption_enabled = false }, &e, params.get()));
    EXPECT_TRUE(e.HadException());
    EXPECT_EQ(e.CodeAs<DOMExceptionCode>(), DOMExceptionCode::kNotAllowedError);
}
