...
MATCHER_P(HasRangeError, expected_message, base::StrCat({ "has ", negation ? "no " : "", "RangeError('", expected_message, "')" }))
{
    ...
    if (exception_state.Message() != expected_message) {
        *result_listener << "unexpected message from RangeError: " << exception_state.Message();
        return false;
    }
    return true;
}

MATCHER_P(HasAbortError, expected_message, base::StrCat({ "has ", negation ? "no " : "", "AbortError('", expected_message, "')" }))
{
    const DummyExceptionStateForTesting& exception_state = arg;
    if (!exception_state.HadException()) {
        *result_listener << "no exception";
    ...
    if (exception_state.CodeAs<DOMExceptionCode>() != DOMExceptionCode::kAbortError) {
        *result_listener << "exception is not AbortError";
        return false;
    }
    if (exception_state.Message() != expected_message) {
        *result_listener << "unexpected message from AbortError: " << exception_state.Message();
        return false;
    }
    return true;
}

MATCHER_P(MatchNetworkResourceRequest, expected, base::StrCat({ "does ", negation ? "not " : "", "match given network::ResourceRequest" }))
{
    ...
    if (src.request_initiator != expected.request_initiator) {
        *result_listener << "mismatched request_initiator: " << *src.request_initiator;
        return false;
    }
    ...
    if (src.priority_incremental != expected.priority_incremental) {
        *result_listener << "mismatched priority_incremental: " << src.priority_incremental;
        return false;
    }
    if (src.cors_preflight_policy != expected.cors_preflight_policy) {
        *result_listener << "mismatched cors_preflight_policy: " << src.cors_preflight_policy;
        return false;
    }
    ...
    if (src.is_fetch_like_api != expected.is_fetch_like_api) {
        *result_listener << "unexpected is_fetch_like_api: " << src.is_fetch_like_api;
        return false;
    }
    if (src.is_fetch_later_api != expected.is_fetch_later_api) {
        *result_listener << "unexpected is_fetch_later_api: " << src.is_fetch_later_api;
        return false;
    }
...
class MockFetchLaterLoaderFactory : public blink::mojom::FetchLaterLoaderFactory {
public:
    MockFetchLaterLoaderFactory() = default;

    mojo::PendingAssociatedRemote<blink::mojom::FetchLaterLoaderFactory> BindNewEndpointAndPassDedicatedRemote()
    {
    ...
    MOCK_METHOD(void, CreateLoader,
        (mojo::PendingAssociatedReceiver<blink::mojom::FetchLaterLoader>, int32_t, uint32_t, const network::ResourceRequest&,
            const net::MutableNetworkTrafficAnnotationTag&),
        (override));
    MOCK_METHOD(void, Clone, (mojo::PendingAssociatedReceiver<blink::mojom::FetchLaterLoaderFactory>), (override));

private:
    mojo::AssociatedReceiver<blink::mojom::FetchLaterLoaderFactory> receiver_ { this };
};
...
public:
    FakeLocalFrameClient()
        : loader_factory_bundle_(base::MakeRefCounted<blink::ChildURLLoaderFactoryBundle>())
    {
    }
...
protected:
    void SetUp() override
    {
        ...
        frame_client_->GetLoaderFactoryBundle()->SetFetchLaterLoaderFactory(factory_.BindNewEndpointAndPassDedicatedRemote());
    }
    ...
    Request* CreateFetchLaterRequest(V8TestingScope& scope, const WTF::String& url, AbortSignal* signal) const
    {
        auto* request_init = RequestInit::Create();
        request_init->setMethod("GET");
        request_init->setSignal(signal);
        auto* request = Request::Create(scope.GetScriptState(), url, request_init, scope.GetExceptionState());

        return request;
    }

    std::unique_ptr<network::ResourceRequest> CreateNetworkResourceRequest(const KURL& url)
    {
        auto request = std::make_unique<network::ResourceRequest>();
        request->url = GURL(url);
        request->request_initiator = SecurityOrigin::Create(KURL(GetSourcePageURL()))->ToUrlOrigin();
        request->referrer = WebStringToGURL(GetSourcePageURL());
        request->referrer_policy = network::ReferrerPolicyForUrlRequest(network::mojom::ReferrerPolicy::kStrictOriginWhenCrossOrigin);
        request->priority = WebURLRequest::ConvertToNetPriority(WebURLRequest::Priority::kHigh);
    ...
    FakeLocalFrameClient* FrameClient()
    {
        return frame_client_;
    }
    ...
    const base::HistogramTester& Histogram() const
    {
        return histogram_;
    }
...
public:
    explicit FetchLaterTestingScope(LocalFrameClient* frame_client)
        : V8TestingScope(DummyPageHolder::CreateAndCommitNavigation(KURL(FetchLaterTest::GetSourcePageURL()),
            /*initial_view_size=*/gfx::Size(),
            /*chrome_client=*/nullptr, frame_client))
    {
    }
};
...
TEST_F(FetchLaterTest, CreateSameOriginFetchLaterRequest)
{
    ...
    url_test_helpers::RegisterMockedURLLoad(KURL(GetSourcePageURL() + target_url), test::CoreTestDataPath("foo.html"), "text/html");
    auto* fetch_later_manager = MakeGarbageCollected<FetchLaterManager>(scope.GetExecutionContext());
    auto* controller = AbortController::Create(scope.GetScriptState());
    auto* request = CreateFetchLaterRequest(scope, target_url, controller->signal());

    EXPECT_CALL(Factory(), CreateLoader(_, _, _, MatchNetworkResourceRequest(*CreateNetworkResourceRequest(KURL(GetSourcePageURL() + target_url))), _))
        .Times(1)
        .RetiresOnSaturation();
    auto* result = fetch_later_manager->FetchLater(
        scope.GetScriptState(), request->PassRequestData(scope.GetScriptState(), exception_state), request->signal(), std::nullopt, exception_state);
...
TEST_F(FetchLaterTest, NegativeActivateAfterThrowRangeError)
{
    ...
    auto* result = fetch_later_manager->FetchLater(scope.GetScriptState(), request->PassRequestData(scope.GetScriptState(), exception_state), request->signal(),
        /*activate_after=*/std::make_optional(-1), exception_state);

    EXPECT_THAT(result, IsNull());
    EXPECT_THAT(exception_state, HasRangeError("fetchLater's activateAfter cannot be negative."));
...
TEST_F(FetchLaterTest, AbortBeforeFetchLater)
{
    ...
    EXPECT_THAT(result, IsNull());
    EXPECT_THAT(exception_state, HasAbortError("The user aborted a fetchLater request."));
    EXPECT_EQ(fetch_later_manager->NumLoadersForTesting(), 0u);
    Histogram().ExpectTotalCount("FetchLater.Renderer.Total", 0);
}
...
TEST_F(FetchLaterTest, AbortAfterFetchLater)
{
    ...
    EXPECT_EQ(fetch_later_manager->NumLoadersForTesting(), 0u);
    Histogram().ExpectTotalCount("FetchLater.Renderer.Total", 1);
    Histogram().ExpectUniqueSample("FetchLater.Renderer.Metrics", 0 /*kAbortedByUser*/, 1);
}
...
TEST_F(FetchLaterTest, ActivateAfter)
{
    ...
    // Sets up a FetchLater request.
    auto* result = fetch_later_manager->FetchLater(scope.GetScriptState(), request->PassRequestData(scope.GetScriptState(), exception_state), request->signal(),
        std::make_optional(activate_after_ms), exception_state);
    EXPECT_THAT(result, Not(IsNull()));
    fetch_later_manager->RecreateTimerForTesting(TaskRunner(), TaskRunner()->GetMockTickClock());
    ...
    EXPECT_EQ(fetch_later_manager->NumLoadersForTesting(), 0u);
    Histogram().ExpectTotalCount("FetchLater.Renderer.Total", 1);
    Histogram().ExpectUniqueSample("FetchLater.Renderer.Metrics", 2 /*kActivatedByTimeout*/, 1);
}
...
TEST_F(FetchLaterTest, ContextDestroyed)
{
    ...
    EXPECT_EQ(fetch_later_manager->NumLoadersForTesting(), 0u);
    Histogram().ExpectTotalCount("FetchLater.Renderer.Total", 1);
    Histogram().ExpectUniqueSample("FetchLater.Renderer.Metrics", 1 /*kContextDestroyed*/, 1);
}
...
TEST_F(FetchLaterTest, ForcedSendingWithBackgroundSyncOff)
{
    ...
    // Sets up a FetchLater request.
    auto* result = fetch_later_manager->FetchLater(scope.GetScriptState(), request->PassRequestData(scope.GetScriptState(), exception_state), request->signal(),
        /*activate_after=*/std::nullopt, exception_state);
    EXPECT_THAT(result, Not(IsNull()));
    ...
    EXPECT_FALSE(exception_state.HadException());
    Histogram().ExpectTotalCount("FetchLater.Renderer.Total", 1);
    Histogram().ExpectUniqueSample("FetchLater.Renderer.Metrics", 3 /*kActivatedOnEnteredBackForwardCache*/, 1);
}
...
TEST(FetchLaterLoadPriorityTest, DefaultHigh)
{
    ...
    ResourceRequest request(KURL(FetchLaterTest::GetSourcePageURL() + "/fetch-later"));
    FetchParameters params(std::move(request), options);

    auto computed_priority = FetchLaterManager::ComputeLoadPriorityForTesting(params);
...
// should be kHigh.
TEST(FetchLaterLoadPriorityTest, WithFetchPriorityHintLowAndRenderBlockingBehaviorBlocking)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    ResourceLoaderOptions options(scope.GetExecutionContext()->GetCurrentWorld());
