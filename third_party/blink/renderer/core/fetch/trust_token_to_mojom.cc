...
PSTFeatures GetPSTFeatures(const ExecutionContext& execution_context)
{
    ...
    features.issuance_enabled = execution_context.IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kPrivateStateTokenIssuance);
    features.redemption_enabled = execution_context.IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kTrustTokenRedemption);
    return features;
}

bool ConvertTrustTokenToMojomAndCheckPermissions(
    const PrivateToken& in, const PSTFeatures& pst_features, ExceptionState* exception_state, network::mojom::blink::TrustTokenParams* out)
{
    ...
    if (in.operation().AsEnum() == OperationType::kTokenRequest) {
        ...
        if (in.refreshPolicy().AsEnum() == RefreshPolicy::kNone) {
            ...
            out->refresh_policy = network::mojom::blink::TrustTokenRefreshPolicy::kRefresh;
        }
    } else {
        ...
        if (in.hasIssuers() && !in.issuers().empty()) {
            for (const String& issuer : in.issuers()) {
                ...
                if (!parsed_url.ProtocolIsInHTTPFamily()) {
                    exception_state->ThrowTypeError("privateToken: operation type 'send-redemption-record' requires "
                                                    "that "
                                                    "the 'issuers' "
            ...
            exception_state->ThrowTypeError("privateToken: operation type 'send-redemption-record' requires that "
...
{
    auto create = [](const String& message, DOMExceptionCode code) { return DOMException::Create(message, DOMException::GetErrorName(code)); };
    ...
    case TrustTokenOperationStatus::kAlreadyExists:
        return create("Redemption operation aborted due to Redemption Record cache hit", DOMExceptionCode::kNoModificationAllowedError);
    case TrustTokenOperationStatus::kOperationSuccessfullyFulfilledLocally:
        return create("Private State Tokens operation satisfied locally, without needing "
                      "to send the request to its initial destination",
            DOMExceptionCode::kNoModificationAllowedError);
    case TrustTokenOperationStatus::kMissingIssuerKeys:
        return create("No keys currently available for PST issuer. Issuer may need to "
                      "register their key commitments.",
            DOMExceptionCode::kInvalidStateError);
    case TrustTokenOperationStatus::kFailedPrecondition:
        return create("Precondition failed during Private State Tokens operation", DOMExceptionCode::kInvalidStateError);
    case TrustTokenOperationStatus::kInvalidArgument:
        return create("Invalid arguments for Private State Tokens operation", DOMExceptionCode::kOperationError);
    case TrustTokenOperationStatus::kResourceExhausted:
        return create("Tokens exhausted for Private State Tokens operation", DOMExceptionCode::kOperationError);
    case TrustTokenOperationStatus::kResourceLimited:
        return create("Quota hit for Private State Tokens operation", DOMExceptionCode::kOperationError);
    case TrustTokenOperationStatus::kUnauthorized:
        return create("Private State Tokens API unavailable due to user settings.", DOMExceptionCode::kOperationError);
    case TrustTokenOperationStatus::kBadResponse:
        return create("Unknown response for Private State Tokens operation", DOMExceptionCode::kOperationError);
    default:
        return create("Error executing Trust Tokens operation", DOMExceptionCode::kOperationError);
    }
}
