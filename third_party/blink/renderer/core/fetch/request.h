...
public:
    using ForServiceWorkerFetchEvent = FetchRequestData::ForServiceWorkerFetchEvent;

    // These "create" function must be called with entering an appropriate
    // V8 context.
    // From Request.idl:
    static Request* Create(ScriptState* script_state, const V8RequestInfo* input, const RequestInit* init, ExceptionState& exception_state);

    static Request* Create(ScriptState*, const String&, ExceptionState&);
    static Request* Create(ScriptState*, const String&, const RequestInit*, ExceptionState&);
    static Request* Create(ScriptState*, Request*, ExceptionState&);
    static Request* Create(ScriptState*, Request*, const RequestInit*, ExceptionState&);
    static Request* Create(ScriptState*, FetchRequestData*, AbortSignal*);
    static Request* Create(ScriptState*, mojom::blink::FetchAPIRequestPtr, ForServiceWorkerFetchEvent);
    ...
    static network::mojom::CredentialsMode V8RequestCredentialsToCredentialsMode(V8RequestCredentials::Enum credentials_mode);
    ...
    uint64_t BodyBufferByteLength() const
    {
        return request_->BufferByteLength();
    }
...
private:
    const FetchRequestData* GetRequest() const
    {
        return request_.Get();
    }
