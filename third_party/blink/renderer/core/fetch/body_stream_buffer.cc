...
class BodyStreamBuffer::LoaderClient final : public GarbageCollected<LoaderClient>, public ExecutionContextLifecycleObserver, public FetchDataLoader::Client {
public:
    LoaderClient(ExecutionContext* execution_context, BodyStreamBuffer* buffer, FetchDataLoader::Client* client)
    ...
    void DidFetchDataLoadedBlobHandle(scoped_refptr<BlobDataHandle> blob_data_handle) override
    {
        buffer_->EndLoading();
    ...
    void DidFetchDataStartedDataPipe(mojo::ScopedDataPipeConsumerHandle data_pipe) override
    {
...
// static
BodyStreamBuffer* BodyStreamBuffer::Create(ScriptState* script_state, BytesConsumer* consumer, AbortSignal* signal,
    ScriptCachedMetadataHandler* cached_metadata_handler, scoped_refptr<BlobDataHandle> side_data_blob)
{
    auto* buffer = MakeGarbageCollected<BodyStreamBuffer>(PassKey(), script_state, consumer, signal, cached_metadata_handler, std::move(side_data_blob));
    buffer->Init();
    return buffer;
}

BodyStreamBuffer::BodyStreamBuffer(PassKey, ScriptState* script_state, BytesConsumer* consumer, AbortSignal* signal,
    ScriptCachedMetadataHandler* cached_metadata_handler, scoped_refptr<BlobDataHandle> side_data_blob)
...
void BodyStreamBuffer::Init()
{
    ...
    if (signal_) {
        if (signal_->aborted()) {
            ...
            stream_buffer_abort_handle_ = signal_->AddAlgorithm(WTF::BindOnce(&BodyStreamBuffer::Abort, WrapWeakPersistent(this)));
        }
    }
    OnStateChange();
}
...
BodyStreamBuffer::BodyStreamBuffer(
    ScriptState* script_state, ReadableStream* stream, ScriptCachedMetadataHandler* cached_metadata_handler, scoped_refptr<BlobDataHandle> side_data_blob)
    : ExecutionContextLifecycleObserver(ExecutionContext::From(script_state))
    , script_state_(script_state)
    , stream_(stream)
    , signal_(nullptr)
...
scoped_refptr<BlobDataHandle> BodyStreamBuffer::DrainAsBlobDataHandle(BytesConsumer::BlobSizePolicy policy, ExceptionState& exception_state)
{
    ...
    scoped_refptr<BlobDataHandle> blob_data_handle = consumer_->DrainAsBlobDataHandle(policy);
    if (blob_data_handle) {
...
scoped_refptr<EncodedFormData> BodyStreamBuffer::DrainAsFormData(ExceptionState& exception_state)
{
    DCHECK(!IsStreamLocked());
    DCHECK(!IsStreamDisturbed());
    if (IsStreamClosed() || IsStreamErrored() || stream_broken_)
...
void BodyStreamBuffer::DrainAsChunkedDataPipeGetter(
    ScriptState* script_state, mojo::PendingReceiver<network::mojom::blink::ChunkedDataPipeGetter> pending_receiver, BytesUploader::Client* client)
{
    DCHECK(!IsStreamLocked());
    auto* consumer = MakeGarbageCollected<ReadableStreamBytesConsumer>(script_state, stream_);
    ExecutionContext* execution_context = ExecutionContext::From(script_state);
    stream_uploader_ = MakeGarbageCollected<BytesUploader>(
        execution_context, consumer, std::move(pending_receiver), execution_context->GetTaskRunner(TaskType::kNetworking), client);
}

void BodyStreamBuffer::StartLoading(FetchDataLoader* loader, FetchDataLoader::Client* client, ExceptionState& exception_state)
{
    DCHECK(!loader_);
    DCHECK(!keep_alive_);

    if (!script_state_->ContextIsValid()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Cannot load body from a frame or worker than has been detached");
        return;
    }
    ...
    if (signal_) {
        ...
        loader_client_abort_handle_ = signal_->AddAlgorithm(WTF::BindOnce(&FetchDataLoader::Client::Abort, WrapWeakPersistent(client)));
    }
    loader_ = loader;
    ...
    if (execution_context) {
        virtual_time_pauser_
            = execution_context->GetScheduler()->CreateWebScopedVirtualTimePauser("ResponseBody", WebScopedVirtualTimePauser::VirtualTaskDuration::kInstant);
        virtual_time_pauser_.PauseVirtualTime();
    }
    loader->Start(handle, MakeGarbageCollected<LoaderClient>(execution_context, this, client));
}

void BodyStreamBuffer::Tee(BodyStreamBuffer** branch1, BodyStreamBuffer** branch2, ExceptionState& exception_state)
{
    DCHECK(!IsStreamLocked());
    DCHECK(!IsStreamDisturbed());
    *branch1 = nullptr;
    *branch2 = nullptr;
    ...
    if (made_from_readable_stream_) {
        if (stream_broken_) {
            ...
            exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Unsafe to tee stream in unknown state");
            return;
        }
        ...
        if (stream_->GetController()->IsByteStreamController()) {
            stream_->ByteStreamTee(script_state_, &stream1, &stream2, exception_state);
        } else {
        ...
        *branch1 = MakeGarbageCollected<BodyStreamBuffer>(script_state_, stream1, cached_metadata_handler, side_data_blob);
        *branch2 = MakeGarbageCollected<BodyStreamBuffer>(script_state_, stream2, cached_metadata_handler, side_data_blob);
        return;
    }
    ...
    BytesConsumerTee(ExecutionContext::From(script_state_), handle, &dest1, &dest2);
    *branch1 = BodyStreamBuffer::Create(script_state_, dest1, signal_, cached_metadata_handler, side_data_blob);
    *branch2 = BodyStreamBuffer::Create(script_state_, dest2, signal_, cached_metadata_handler, side_data_blob);
}

ScriptPromise<IDLUndefined> BodyStreamBuffer::Pull(ReadableByteStreamController* controller, ExceptionState& exception_state)
{
    if (!consumer_) {
...
ScriptPromise<IDLUndefined> BodyStreamBuffer::Cancel(v8::Local<v8::Value> reason)
{
...
{
    if (!consumer_ || !GetExecutionContext() || GetExecutionContext()->IsContextDestroyed()) {
        return;
    }
    ExceptionState exception_state(script_state_->GetIsolate(), v8::ExceptionContext::kUnknown, "", "");
...
void BodyStreamBuffer::Abort()
{
    ...
    auto* byte_controller = To<ReadableByteStreamController>(stream_->GetController());
    v8::Local<v8::Value> dom_exception
        = V8ThrowDOMException::CreateOrEmpty(script_state_->GetIsolate(), DOMExceptionCode::kAbortError, "BodyStreamBuffer was aborted");
    CHECK(!dom_exception.IsEmpty());
    ReadableByteStreamController::Error(script_state_, byte_controller, dom_exception);
    CancelConsumer();
}
...
{
    {
        ScriptState::Scope scope(script_state_);
        auto* byte_controller = To<ReadableByteStreamController>(stream_->GetController());
        ReadableByteStreamController::Error(script_state_, byte_controller, V8ThrowException::CreateTypeError(script_state_->GetIsolate(), "network error"));
    }
    CancelConsumer();
}
...
{
    {
        ...
        ReadableByteStreamController::Error(
            script_state_, byte_controller, V8ThrowException::CreateRangeError(script_state_->GetIsolate(), "Array buffer allocation failed"));
    }
    CancelConsumer();
}
...
void BodyStreamBuffer::ProcessData(ExceptionState& exception_state)
{
    ...
    while (stream_needs_more_) {
        ...
        if (result == BytesConsumer::Result::kOk) {
            if (stream_->GetController()->IsByteStreamController()) {
                auto* byte_controller = To<ReadableByteStreamController>(stream_->GetController());
                if (ReadableStreamBYOBRequest* request = byte_controller->byobRequest()) {
        ...
        case BytesConsumer::Result::kDone:
            if (array || byob_view) {
                ...
                auto* byte_controller = To<ReadableByteStreamController>(stream_->GetController());
                if (byob_view) {
                    ReadableByteStreamController::Respond(script_state_, byte_controller, buffer.size(), PassThroughException(script_state_->GetIsolate()));
                } else {
                    CHECK(array);
                    ReadableByteStreamController::Enqueue(script_state_, byte_controller, NotShared(array), PassThroughException(script_state_->GetIsolate()));
                }
                if (try_catch.HasCaught()) {
                    return;
                }
            ...
            if (!stream_needs_more_) {
                auto* byte_controller = To<ReadableByteStreamController>(stream_->GetController());
                std::optional<double> desired_size = ReadableByteStreamController::GetDesiredSize(byte_controller);
...
BytesConsumer* BodyStreamBuffer::ReleaseHandle(ExceptionState& exception_state)
{
    DCHECK(!IsStreamLocked());
    DCHECK(!IsStreamDisturbed());

    if (stream_broken_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Body stream has suffered a fatal error and cannot be inspected");
        return nullptr;
    }

    if (!GetExecutionContext()) {
        // Avoid crashing if ContextDestroyed() has been called.
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Cannot release body in a window or worker that has been detached");
        return nullptr;
    }
    ...
    if (made_from_readable_stream_) {
        ...
        ScriptState::Scope scope(script_state_);
        return MakeGarbageCollected<ReadableStreamBytesConsumer>(script_state_, stream_);
    }
