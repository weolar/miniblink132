...
class TeeHelper final : public GarbageCollected<TeeHelper>, public BytesConsumer::Client {
public:
    TeeHelper(ExecutionContext* execution_context, BytesConsumer* consumer)
        : src_(consumer)
        , destination1_(MakeGarbageCollected<Destination>(execution_context, this))
        , destination2_(MakeGarbageCollected<Destination>(execution_context, this))
    {
    ...
    BytesConsumer* Destination1() const
    {
        return destination1_.Get();
    }
    BytesConsumer* Destination2() const
    {
        return destination2_.Get();
    }
...
private:
    ...
    public:
        ...
        {
            buffer_.ReserveInitialCapacity(base::checked_cast<wtf_size_t>(data.size()));
            buffer_.AppendSpan(data);
            // Report buffer size to V8 so GC can be triggered appropriately.
            external_memory_accounter_.Increase(v8::Isolate::GetCurrent(), static_cast<int64_t>(buffer_.size()));
        }
        ~Chunk()
        {
            external_memory_accounter_.Decrease(v8::Isolate::GetCurrent(), static_cast<int64_t>(buffer_.size()));
        }
    ...
    public:
        ...
        Result BeginRead(base::span<const char>& buffer) override
        {
            ...
            if (!chunks_.empty()) {
                ...
                buffer = base::span(chunk->data(), chunk->size()).subspan(offset_);
                chunk_in_use_ = chunk;
                return Result::kOk;
            }
        ...
        Result EndRead(size_t read) override
        {
            ...
            if (chunks_.empty() && tee_->GetPublicState() == PublicState::kClosed) {
                ...
                execution_context_->GetTaskRunner(TaskType::kNetworking)->PostTask(FROM_HERE, WTF::BindOnce(&Destination::Close, WrapPersistent(this)));
            }
            return Result::kOk;
        }
        ...
        PublicState GetPublicState() const override
        {
            ...
            // m_isClosed is set.
            return state == PublicState::kClosed ? PublicState::kReadableOrWaiting : state;
        }

        Error GetError() const override
        {
            return tee_->GetError();
        }

        String DebugName() const override
        {
            return "TeeHelper::Destination";
        }
...
void BytesConsumerTee(ExecutionContext* execution_context, BytesConsumer* src, BytesConsumer** dest1, BytesConsumer** dest2)
{
    scoped_refptr<BlobDataHandle> blob_data_handle = src->DrainAsBlobDataHandle(BytesConsumer::BlobSizePolicy::kAllowBlobWithInvalidSize);
    if (blob_data_handle) {
        // Register a client in order to be consistent.
        src->SetClient(MakeGarbageCollected<NoopClient>());
        *dest1 = MakeGarbageCollected<BlobBytesConsumer>(execution_context, blob_data_handle);
        *dest2 = MakeGarbageCollected<BlobBytesConsumer>(execution_context, blob_data_handle);
        return;
    }
    ...
    if (form_data) {
        // Register a client in order to be consistent.
        src->SetClient(MakeGarbageCollected<NoopClient>());
        *dest1 = MakeGarbageCollected<FormDataBytesConsumer>(execution_context, form_data);
        *dest2 = MakeGarbageCollected<FormDataBytesConsumer>(execution_context, form_data);
        return;
    }
