...
ScopedNullExecutionContext MakeExecutionContext(bool has_permission)
{
    ...
    if (has_permission) {
        AllowFeatureEverywhere(mojom::blink::PermissionsPolicyFeature::kAttributionReporting, parsed_policy);
    } else {
        DisallowFeature(mojom::blink::PermissionsPolicyFeature::kAttributionReporting, parsed_policy);
    }
    ...
    execution_context.GetExecutionContext().GetSecurityContext().SetPermissionsPolicy(
        PermissionsPolicy::CreateFromParsedPolicy(parsed_policy, /*base_plicy=*/std::nullopt, origin->ToUrlOrigin()));
...
TEST(AttributionReportingToMojomTest, Convert)
{
    ...
    for (const auto& test_case : kTestCases) {
        ...
        SCOPED_TRACE(base::StringPrintf("event_source_eligible=%d,trigger_eligible=%d", test_case.event_source_eligible, test_case.trigger_eligible));
...

        {
            ...
            EXPECT_EQ(test_case.expected_result,
                ConvertAttributionReportingRequestOptionsToMojom(*options, execution_context.GetExecutionContext(), scope.GetExceptionState()));

            EXPECT_FALSE(scope.GetExceptionState().HadException());
            histograms.ExpectBucketCount("Conversions.AllowedByPermissionPolicy", 1, 1);
        }

        {
            ...
            histograms.ExpectBucketCount("Conversions.AllowedByPermissionPolicy", 0, 1);
        }
    }
}
