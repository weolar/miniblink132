...
TEST(ServiceWorkerResponseTest, FromFetchResponseData)
{
    ...
    Response* response = Response::Create(page->GetFrame().DomWindow(), fetch_response_data);
    DCHECK(response);
    EXPECT_EQ(url, response->url());
}

void CheckResponseStream(ScriptState* script_state, Response* response, bool check_response_body_stream_buffer)
{
    ...
    EXPECT_NE(response->InternalBodyBuffer(), cloned_response->InternalBodyBuffer());
    if (check_response_body_stream_buffer) {
        EXPECT_EQ(response->BodyBuffer(), response->InternalBodyBuffer());
        EXPECT_EQ(cloned_response->BodyBuffer(), cloned_response->InternalBodyBuffer());
    } else {
        EXPECT_FALSE(response->BodyBuffer());
        EXPECT_FALSE(cloned_response->BodyBuffer());
    }
    BytesConsumerTestUtil::MockFetchDataLoaderClient* client1 = MakeGarbageCollected<BytesConsumerTestUtil::MockFetchDataLoaderClient>();
    BytesConsumerTestUtil::MockFetchDataLoaderClient* client2 = MakeGarbageCollected<BytesConsumerTestUtil::MockFetchDataLoaderClient>();
    ...
    response->InternalBodyBuffer()->StartLoading(
        FetchDataLoader::CreateLoaderAsString(TextResourceDecoderOptions::CreateUTF8Decode()), client1, ASSERT_NO_EXCEPTION);
    cloned_response->InternalBodyBuffer()->StartLoading(
        FetchDataLoader::CreateLoaderAsString(TextResourceDecoderOptions::CreateUTF8Decode()), client2, ASSERT_NO_EXCEPTION);
...
BodyStreamBuffer* CreateHelloWorldBuffer(ScriptState* script_state)
{
    ...
    auto* src = MakeGarbageCollected<ReplayingBytesConsumer>(ExecutionContext::From(script_state)->GetTaskRunner(TaskType::kNetworking));
...
TEST(ServiceWorkerResponseTest, BodyStreamBufferCloneDefault)
{
    ...
    FetchResponseData* fetch_response_data = FetchResponseData::CreateWithBuffer(buffer);
    Vector<KURL> url_list;
    url_list.push_back(KURL("http://www.response.com"));
    fetch_response_data->SetURLList(url_list);
    Response* response = Response::Create(scope.GetExecutionContext(), fetch_response_data);
...
TEST(ServiceWorkerResponseTest, BodyStreamBufferCloneError)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    BodyStreamBuffer* buffer = BodyStreamBuffer::Create(scope.GetScriptState(), BytesConsumer::CreateErrored(BytesConsumer::Error()), nullptr,
        /*cached_metadata_handler=*/nullptr);
    FetchResponseData* fetch_response_data = FetchResponseData::CreateWithBuffer(buffer);
    Vector<KURL> url_list;
    url_list.push_back(KURL("http://www.response.com"));
    ...
    Response* response = Response::Create(scope.GetExecutionContext(), fetch_response_data);
    DummyExceptionStateForTesting exception_state;
    Response* cloned_response = response->clone(scope.GetScriptState(), exception_state);
    EXPECT_FALSE(exception_state.HadException());
