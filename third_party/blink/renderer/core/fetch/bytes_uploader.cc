...
BytesUploader::BytesUploader(ExecutionContext* execution_context, BytesConsumer* consumer,
    mojo::PendingReceiver<network::mojom::blink::ChunkedDataPipeGetter> pending_receiver, scoped_refptr<base::SingleThreadTaskRunner> task_runner,
    Client* client)
    : ExecutionContextLifecycleObserver(execution_context)
    , consumer_(consumer)
    , client_(client)
    , receiver_(this, execution_context)
    , upload_pipe_watcher_(FROM_HERE, mojo::SimpleWatcher::ArmingPolicy::MANUAL, task_runner)
{
    DCHECK(consumer_);
    DCHECK_EQ(consumer_->GetPublicState(), BytesConsumer::PublicState::kReadableOrWaiting);
...
void BytesUploader::StartReading(mojo::ScopedDataPipeProducerHandle upload_pipe)
{
    ...
    upload_pipe_watcher_.Watch(upload_pipe_.get(), MOJO_HANDLE_SIGNAL_WRITABLE, WTF::BindRepeating(&BytesUploader::OnPipeWriteable, WrapWeakPersistent(this)));
    consumer_->SetClient(this);
    if (consumer_->GetPublicState() == BytesConsumer::PublicState::kReadableOrWaiting) {
        WriteDataOnPipe();
    }
}
...
{
    DVLOG(3) << this << " OnStateChange(). consumer_->GetPublicState()=" << consumer_->GetPublicState();
    DCHECK(get_size_callback_);
...
{
    DVLOG(3) << this << " WriteDataOnPipe(). consumer_->GetPublicState()=" << consumer_->GetPublicState();
    ...
    while (true) {
        ...
        DVLOG(3) << "  consumer_->BeginRead()=" << consumer_result << ", available=" << buffer.size();
        ...
        const MojoResult mojo_result = upload_pipe_->WriteData(base::as_bytes(buffer), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes);
        DVLOG(3) << "  upload_pipe_->WriteData()=" << mojo_result << ", mojo_written=" << actually_written_bytes;
        ...
        if (!base::CheckAdd(total_size_, actually_written_bytes).AssignIfValid(&total_size_)) {
            CloseOnError();
            return;
        }
