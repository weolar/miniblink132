...
class BlobBytesConsumer::BlobClient : public GarbageCollected<BlobBytesConsumer::BlobClient>, public mojom::blink::BlobReaderClient {
public:
    BlobClient(ExecutionContext* context, DataPipeBytesConsumer::CompletionNotifier* completion_notifier)
    ...
    mojo::PendingRemote<mojom::blink::BlobReaderClient> BindNewPipeAndPassRemote()
    {
...
BlobBytesConsumer::BlobBytesConsumer(ExecutionContext* execution_context, scoped_refptr<BlobDataHandle> blob_data_handle)
...
BytesConsumer::Result BlobBytesConsumer::BeginRead(base::span<const char>& buffer)
{
    if (!nested_consumer_) {
        ...
        options.capacity_num_bytes = blink::BlobUtils::GetDataPipeCapacity(blob_data_handle_->size());
        ...
        nested_consumer_ = MakeGarbageCollected<DataPipeBytesConsumer>(
            execution_context_->GetTaskRunner(TaskType::kNetworking), std::move(consumer_handle), &completion_notifier);
        if (client_)
            nested_consumer_->SetClient(client_);

        // Start reading the blob.
        blob_client_ = MakeGarbageCollected<BlobClient>(execution_context_, completion_notifier);
        blob_data_handle_->ReadAll(std::move(producer_handle), blob_client_->BindNewPipeAndPassRemote());
...
scoped_refptr<BlobDataHandle> BlobBytesConsumer::DrainAsBlobDataHandle(BlobSizePolicy policy)
{
    if (!blob_data_handle_)
        return nullptr;
    if (policy == BlobSizePolicy::kDisallowBlobWithInvalidSize && blob_data_handle_->size() == UINT64_MAX)
        return nullptr;
...
{
    scoped_refptr<BlobDataHandle> handle = DrainAsBlobDataHandle(BlobSizePolicy::kAllowBlobWithInvalidSize);
    if (!handle)
        return nullptr;
...
{
    if (!nested_consumer_) {
        return blob_data_handle_ ? PublicState::kReadableOrWaiting : PublicState::kClosed;
    }
