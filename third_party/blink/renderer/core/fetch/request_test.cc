...
protected:
    ...
    static RequestInit* CreateRequestInit(V8TestingScope& scope, const v8::Local<v8::Value>& body_value)
    {
...
TEST_F(RequestBodyTest, EmptyBody)
{
    ...
    Request* request = Request::Create(scope.GetScriptState(), RequestURL(), scope.GetExceptionState());
...
TEST_F(RequestBodyTest, InitWithBodyString)
{
    ...
    auto* init = CreateRequestInit(scope, ToV8Traits<IDLString>::ToV8(scope.GetScriptState(), body));

    Request* request = Request::Create(scope.GetScriptState(), RequestURL(), init, scope.GetExceptionState());
    ASSERT_FALSE(scope.GetExceptionState().HadException());
    ASSERT_EQ(request->url(), RequestURL());
...
TEST_F(RequestBodyTest, InitWithBodyArrayBuffer)
{
    ...
    auto* init = CreateRequestInit(scope, ToV8Traits<DOMArrayBuffer>::ToV8(scope.GetScriptState(), buffer));
...
TEST_F(RequestBodyTest, InitWithBodyArrayBufferView)
{
    ...
    auto* init = CreateRequestInit(scope, ToV8Traits<DOMArrayBufferView>::ToV8(scope.GetScriptState(), buffer_view));
...
TEST_F(RequestBodyTest, InitWithBodyFormData)
{
    ...
    auto* init = CreateRequestInit(scope, ToV8Traits<FormData>::ToV8(scope.GetScriptState(), form));

    Request* request = Request::Create(scope.GetScriptState(), RequestURL(), init, scope.GetExceptionState());
    ASSERT_FALSE(scope.GetExceptionState().HadException());
    ASSERT_EQ(request->url(), RequestURL());

    EXPECT_EQ(request->BodyBufferByteLength(), form->EncodeMultiPartFormData()->SizeInBytes());
}

TEST_F(RequestBodyTest, InitWithUrlSearchParams)
{
    V8TestingScope scope;
    auto* params = URLSearchParams::Create({ std::make_pair("test-key", "test-value") }, scope.GetExceptionState());
    auto* init = CreateRequestInit(scope, ToV8Traits<URLSearchParams>::ToV8(scope.GetScriptState(), params));

    Request* request = Request::Create(scope.GetScriptState(), RequestURL(), init, scope.GetExceptionState());
    ASSERT_FALSE(scope.GetExceptionState().HadException());
    ASSERT_EQ(request->url(), RequestURL());

    EXPECT_EQ(request->BodyBufferByteLength(), params->ToEncodedFormData()->SizeInBytes());
}
...
TEST_F(RequestBodyTest, InitWithBlob)
{
    ...
    auto* init = CreateRequestInit(scope, ToV8Traits<Blob>::ToV8(scope.GetScriptState(), blob));
...
TEST(ServiceWorkerRequestTest, FromString)
{
    ...
    Request* request = Request::Create(scope.GetScriptState(), url, exception_state);
...
TEST(ServiceWorkerRequestTest, FromRequest)
{
    ...
    KURL url("http://www.example.com/");
    Request* request1 = Request::Create(scope.GetScriptState(), url, exception_state);
    DCHECK(request1);

    Request* request2 = Request::Create(scope.GetScriptState(), request1, exception_state);
    ASSERT_FALSE(exception_state.HadException());
    DCHECK(request2);
...
TEST(ServiceWorkerRequestTest, FromAndToFetchAPIRequest)
{
    ...
    const network::mojom::ReferrerPolicy kReferrerPolicy = network::mojom::ReferrerPolicy::kAlways;
    const network::mojom::RequestDestination kDestination = network::mojom::RequestDestination::kAudio;
    const network::mojom::RequestMode kMode = network::mojom::RequestMode::kNavigate;
    const network::mojom::CredentialsMode kCredentialsMode = network::mojom::CredentialsMode::kInclude;
    const auto kCacheMode = mojom::FetchCacheMode::kValidateCache;
    const network::mojom::RedirectMode kRedirectMode = network::mojom::RedirectMode::kError;
    ...
    fetch_api_request->referrer = mojom::blink::Referrer::New(KURL(NullURL(), referrer), kReferrerPolicy);
    const auto fetch_api_request_headers = fetch_api_request->headers;

    Request* request = Request::Create(scope.GetScriptState(), std::move(fetch_api_request), Request::ForServiceWorkerFetchEvent::kFalse);
    DCHECK(request);
    EXPECT_EQ(url, request->url());
    ...
    for (WTF::HashMap<String, String>::iterator iter = headers_map.begin(); iter != headers_map.end(); ++iter) {
        DummyExceptionStateForTesting exception_state;
        EXPECT_EQ(iter->value, request_headers->get(iter->key, exception_state));
        EXPECT_FALSE(exception_state.HadException());
    }

    mojom::blink::FetchAPIRequestPtr second_fetch_api_request = request->CreateFetchAPIRequest();
    ...
    EXPECT_EQ(network::mojom::ReferrerPolicy::kAlways, second_fetch_api_request->referrer->policy);
...
TEST(ServiceWorkerRequestTest, ToFetchAPIRequestDoesNotStripURLFragment)
{
    ...
    Request* request = Request::Create(scope.GetScriptState(), url_with_fragment, exception_state);
    DCHECK(request);

    mojom::blink::FetchAPIRequestPtr fetch_api_request = request->CreateFetchAPIRequest();
