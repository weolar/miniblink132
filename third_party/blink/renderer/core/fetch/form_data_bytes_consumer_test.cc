...
public:
    SimpleDataPipeGetter(const String& str, mojo::PendingReceiver<network::mojom::blink::DataPipeGetter> receiver)
        : str_(str)
    {
        receivers_.set_disconnect_handler(WTF::BindRepeating(&SimpleDataPipeGetter::OnMojoDisconnect, WTF::Unretained(this)));
    ...
    void Read(mojo::ScopedDataPipeProducerHandle handle, ReadCallback callback) override
    {
...
scoped_refptr<EncodedFormData> ComplexFormData()
{
    ...
    data->AppendFileRange("/foo/bar/baz", 3, 4, base::Time::FromSecondsSinceUnixEpoch(5));
    auto blob_data = std::make_unique<BlobData>();
    blob_data->AppendText("hello", false);
    auto size = blob_data->length();
    scoped_refptr<BlobDataHandle> blob_data_handle = BlobDataHandle::Create(std::move(blob_data), size);
...
scoped_refptr<EncodedFormData> DataPipeFormData()
{
    ...
    // Add data pipe.
    mojo::PendingRemote<network::mojom::blink::DataPipeGetter> data_pipe_getter_remote;
    // Object deletes itself.
    new SimpleDataPipeGetter(String(" hello world"), data_pipe_getter_remote.InitWithNewPipeAndPassReceiver());
    body.AppendDataPipe(std::move(data_pipe_getter_remote));

    // Add another data pipe.
    mojo::PendingRemote<network::mojom::blink::DataPipeGetter> data_pipe_getter_remote2;
    // Object deletes itself.
    new SimpleDataPipeGetter(String(" here's another data pipe "), data_pipe_getter_remote2.InitWithNewPipeAndPassReceiver());
...
class NoopClient final : public GarbageCollected<NoopClient>, public BytesConsumer::Client {
public:
...
public:
    void SetUp() override
    {
        ...
        file_factory_helper_ = std::make_unique<FileBackedBlobFactoryTestHelper>(GetFrame().GetDocument()->GetExecutionContext());
    }

    String DrainAsString(scoped_refptr<EncodedFormData> input_form_data)
    {
        auto* consumer = MakeGarbageCollected<FormDataBytesConsumer>(GetFrame().DomWindow(), input_form_data);
    ...
    scoped_refptr<EncodedFormData> DrainAsFormData(scoped_refptr<EncodedFormData> input_form_data)
    {
        auto* consumer = MakeGarbageCollected<FormDataBytesConsumer>(GetFrame().DomWindow(), input_form_data);
        return consumer->DrainAsFormData();
    }

    scoped_refptr<BlobDataHandle> DrainAsBlobDataHandle(scoped_refptr<EncodedFormData> input_form_data)
    {
        auto* consumer = MakeGarbageCollected<FormDataBytesConsumer>(GetFrame().DomWindow(), input_form_data);
        return consumer->DrainAsBlobDataHandle(BytesConsumer::BlobSizePolicy::kAllowBlobWithInvalidSize);
    }
...
{
    auto result = (MakeGarbageCollected<BytesConsumerTestReader>(MakeGarbageCollected<FormDataBytesConsumer>("hello, world")))->Run();
    EXPECT_EQ(Result::kDone, result.first);
...
TEST_F(FormDataBytesConsumerTest, TwoPhaseReadFromStringNonLatin)
{
    ...
    auto result = (MakeGarbageCollected<BytesConsumerTestReader>(MakeGarbageCollected<FormDataBytesConsumer>(String(kCs))))->Run();
    EXPECT_EQ(Result::kDone, result.first);
...
{
    constexpr unsigned char kData[] = { 0x21, 0xfe, 0x00, 0x00, 0xff, 0xa3, 0x42, 0x30, 0x42, 0x99, 0x88 };
    DOMArrayBuffer* buffer = DOMArrayBuffer::Create(kData);
    auto result = (MakeGarbageCollected<BytesConsumerTestReader>(MakeGarbageCollected<FormDataBytesConsumer>(buffer)))->Run();
...
TEST_F(FormDataBytesConsumerTest, TwoPhaseReadFromArrayBufferView)
{
    ...
    DOMArrayBuffer* buffer = DOMArrayBuffer::Create(kData);
    auto result
        = (MakeGarbageCollected<BytesConsumerTestReader>(MakeGarbageCollected<FormDataBytesConsumer>(DOMUint8Array::Create(buffer, kOffset, kSize))))->Run();
    Vector<char> expected;
...
TEST_F(FormDataBytesConsumerTest, TwoPhaseReadFromSimpleFormData)
{
    ...
    auto result = (MakeGarbageCollected<BytesConsumerTestReader>(MakeGarbageCollected<FormDataBytesConsumer>(GetFrame().DomWindow(), data)))->Run();
    EXPECT_EQ(Result::kDone, result.first);
...
TEST_F(FormDataBytesConsumerTest, TwoPhaseReadFromComplexFormData)
{
    ...
    auto* consumer = MakeGarbageCollected<FormDataBytesConsumer>(GetFrame().DomWindow(), data, underlying);
...
{
    BytesConsumer* consumer = MakeGarbageCollected<FormDataBytesConsumer>("hello, world");
    base::span<const char> buffer;
    ASSERT_EQ(Result::kOk, consumer->BeginRead(buffer));
    ASSERT_EQ(12u, buffer.size());
    EXPECT_EQ("hello, world", String(base::as_bytes(buffer)));
    EXPECT_EQ(BytesConsumer::PublicState::kReadableOrWaiting, consumer->GetPublicState());
...
{
    BytesConsumer* consumer = MakeGarbageCollected<FormDataBytesConsumer>("hello, world");
    scoped_refptr<BlobDataHandle> blob_data_handle = consumer->DrainAsBlobDataHandle();
...
{
    BytesConsumer* consumer = MakeGarbageCollected<FormDataBytesConsumer>(DOMArrayBuffer::Create(base::byte_span_from_cstring("foo")));
    scoped_refptr<BlobDataHandle> blob_data_handle = consumer->DrainAsBlobDataHandle();
    ASSERT_TRUE(blob_data_handle);
...
TEST_F(FormDataBytesConsumerTest, DrainAsBlobDataHandleFromSimpleFormData)
{
    ...
    BytesConsumer* consumer = MakeGarbageCollected<FormDataBytesConsumer>(GetFrame().DomWindow(), input_form_data);
    scoped_refptr<BlobDataHandle> blob_data_handle = consumer->DrainAsBlobDataHandle();
    ASSERT_TRUE(blob_data_handle);

    EXPECT_EQ(String(), blob_data_handle->GetType());
    EXPECT_EQ(input_form_data->FlattenToString().Utf8().length(), blob_data_handle->size());
    EXPECT_FALSE(consumer->DrainAsFormData());
    base::span<const char> buffer;
    EXPECT_EQ(Result::kDone, consumer->BeginRead(buffer));
    EXPECT_EQ(BytesConsumer::PublicState::kClosed, consumer->GetPublicState());
...
{
    auto* underlying = MakeGarbageCollected<MockBytesConsumer>();
    BytesConsumer* consumer = MakeGarbageCollected<FormDataBytesConsumer>(GetFrame().DomWindow(), ComplexFormData(), underlying);
    ...
    EXPECT_CALL(*underlying, GetPublicState()).WillOnce(Return(BytesConsumer::PublicState::kReadableOrWaiting));
...
TEST_F(FormDataBytesConsumerTest, SetClientWithComplexFormData)
{
    ...
    auto* underlying = MakeGarbageCollected<MockBytesConsumer>();
    auto* consumer = MakeGarbageCollected<FormDataBytesConsumer>(GetFrame().DomWindow(), input_form_data, underlying);
    Checkpoint checkpoint;
...
TEST_F(FormDataBytesConsumerTest, DataPipeFormData)
{
    ...
    std::pair<BytesConsumer::Result, Vector<char>> result = reader->Run();
    EXPECT_EQ(Result::kDone, result.first);
    EXPECT_EQ("foo hello world here's another data pipe bar baz", String(result.second));
}
...
TEST_F(FormDataBytesConsumerTest, DataPipeFormData_DrainAsFormData)
{
    scoped_refptr<EncodedFormData> input_form_data = DataPipeFormData();
    auto* consumer = MakeGarbageCollected<FormDataBytesConsumer>(GetFrame().DomWindow(), input_form_data);
    scoped_refptr<EncodedFormData> drained_form_data = consumer->DrainAsFormData();
...
// starting to read.
TEST_F(FormDataBytesConsumerTest, DataPipeFormData_DrainAsFormDataWhileReading)
{
    ...
    std::pair<BytesConsumer::Result, Vector<char>> result = reader->Run();
    EXPECT_EQ(Result::kDone, result.first);
    EXPECT_EQ(" hello world here's another data pipe bar baz", String(result.second));
}
...
void AppendDataPipe(scoped_refptr<EncodedFormData> data, String content)
{
    ...
    // Object deletes itself.
    new SimpleDataPipeGetter(content, data_pipe_getter.InitWithNewPipeAndPassReceiver());
    auto wrapped = base::MakeRefCounted<WrappedDataPipeGetter>(std::move(data_pipe_getter));
...
TEST_F(FormDataBytesConsumerTest, InvalidType2)
{
    ...
    auto* consumer = MakeGarbageCollected<FormDataBytesConsumer>(GetFrame().DomWindow(), data);
    Vector<char> str;
    {
        base::span<const char> buffer;
        EXPECT_EQ(BytesConsumer::Result::kOk, consumer->BeginRead(buffer));
