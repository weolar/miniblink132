...
class CORE_EXPORT FetchResponseData final : public GarbageCollected<FetchResponseData> {
public:
    // "A response can have an associated termination reason which is one of
    // end-user abort, fatal, and timeout."
    enum TerminationReason { kEndUserAbortTermination, kFatalTermination, kTimeoutTermination };
    ...
    FetchResponseData(network::mojom::FetchResponseType, network::mojom::FetchResponseSource, uint16_t, AtomicString);
    ...
    FetchResponseData* CreateCorsFilteredResponse(const HTTPHeaderSet& exposed_headers) const;
    FetchResponseData* CreateOpaqueFilteredResponse() const;
    FetchResponseData* CreateOpaqueRedirectFilteredResponse() const;

    FetchResponseData* InternalResponse()
    {
    ...
    AtomicString StatusMessage() const
    {
    ...
    String CacheStorageCacheName() const
    {
    ...
    const Vector<KURL>& UrlList() const
    {
        return url_list_;
    }
    ...
    void SetAuthChallengeInfo(const std::optional<net::AuthChallengeInfo>& auth_challenge_info);
    ...
    mojom::blink::FetchAPIResponsePtr PopulateFetchAPIResponse(const KURL& request_url);

    // Initialize non-body data from the given |response|.
    void InitFromResourceResponse(ExecutionContext* context, network::mojom::FetchResponseType response_type, const Vector<KURL>& request_url_list,
        const AtomicString& request_method, network::mojom::CredentialsMode request_credentials, const ResourceResponse& response);
