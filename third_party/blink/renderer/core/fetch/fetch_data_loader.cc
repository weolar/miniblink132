...
class FetchDataLoaderAsBlobHandle final : public FetchDataLoader, public FetchDataLoader::Client {
public:
    FetchDataLoaderAsBlobHandle(const String& mime_type, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    ...
    void Start(BytesConsumer* consumer, FetchDataLoader::Client* client) override
    {
        ...
        scoped_refptr<BlobDataHandle> blob_handle = consumer_->DrainAsBlobDataHandle();
        if (blob_handle) {
            ...
            if (blob_handle->GetType() != mime_type_) {
                ...
                client_->DidFetchDataLoadedBlobHandle(BlobDataHandle::Create(std::move(blob_data), blob_size));
            } else {
    ...
    void DidFetchDataStartedDataPipe(mojo::ScopedDataPipeConsumerHandle handle) override
    {
        ...
        BlobDataHandle::GetBlobRegistry()->RegisterFromStream(mime_type_ ? mime_type_ : "", /*content_disposition=*/"",
            /*length_hint=*/0, std::move(handle), mojo::PendingAssociatedRemote<mojom::blink::ProgressClient>(),
            WTF::BindOnce(&FetchDataLoaderAsBlobHandle::FinishedCreatingFromDataPipe, WrapWeakPersistent(this)));
    }
...
private:
    void FinishedCreatingFromDataPipe(const scoped_refptr<BlobDataHandle>& blob_handle)
    {
        if (load_canceled_)
            return;
        if (!blob_handle) {
...
public:
    ...
    {
        return "FetchDataLoaderAsArrayBuffer";
    }
...
private:
    ...
    bool Append(base::span<const char> data)
    {
        DCHECK(!data.empty());
        buffer_->Append(data);
        if (buffer_->size() > static_cast<size_t>(std::numeric_limits<uint32_t>::max())) {
            return false;
        }
        return true;
    }
    ...
    {
        DOMArrayBuffer* result = DOMArrayBuffer::CreateUninitializedOrNull(base::checked_cast<unsigned>(buffer_->size()), 1);
...
class FetchDataLoaderAsFailure final : public FetchDataLoader, public BytesConsumer::Client {
public:
    void Start(BytesConsumer* consumer, FetchDataLoader::Client* client) override
    {
        DCHECK(!client_);
    ...
    String DebugName() const override
    {
        return "FetchDataLoaderAsFailure";
    }
...
public:
    ...
    void Start(BytesConsumer* consumer, FetchDataLoader::Client* client) override
    {
        ...
        multipart_boundary_vector.AppendSpan(base::span(multipart_boundary_utf8.data(), multipart_boundary_utf8.size()));

        client_ = client;
        form_data_ = MakeGarbageCollected<FormData>();
        multipart_parser_ = MakeGarbageCollected<MultipartParser>(std::move(multipart_boundary_vector), this);
        consumer_ = consumer;
        consumer_->SetClient(this);
        OnStateChange();
    }
    ...
    String DebugName() const override
    {
        return "FetchDataLoaderAsFormData";
    }
...
private:
    void PartHeaderFieldsInMultipartReceived(const HTTPHeaderMap& header_fields) override
    {
    ...
    public:
        ...
        {
            const ParsedContentDisposition disposition(header_fields.Get(http_names::kContentDisposition));
            ...
            if (!filename_.IsNull()) {
                ...
                const AtomicString& content_type = header_fields.Get(http_names::kContentType);
                blob_data_->SetContentType(content_type.IsNull() ? AtomicString("text/plain") : content_type);
            } else {
                if (!string_decoder_) {
                    string_decoder_ = std::make_unique<TextResourceDecoder>(TextResourceDecoderOptions::CreateUTF8DecodeWithoutBOM());
                }
        ...
        bool Finish(FormData* form_data)
        {
            if (blob_data_) {
                ...
                auto* file = MakeGarbageCollected<File>(filename_, std::nullopt, BlobDataHandle::Create(std::move(blob_data_), size));
...
public:
    ...
    String DebugName() const override
    {
        return "FetchDataLoaderAsString";
    }
...
class FetchDataLoaderAsDataPipe final : public FetchDataLoader, public BytesConsumer::Client {
    USING_PRE_FINALIZER(FetchDataLoaderAsDataPipe, Dispose);

public:
    explicit FetchDataLoaderAsDataPipe(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
        : data_pipe_watcher_(FROM_HERE, mojo::SimpleWatcher::ArmingPolicy::MANUAL, task_runner)
        , data_pipe_close_watcher_(FROM_HERE, mojo::SimpleWatcher::ArmingPolicy::AUTOMATIC, std::move(task_runner))
    {
    }
    ...
    void Start(BytesConsumer* consumer, FetchDataLoader::Client* client) override
    {
        ...
        mojo::ScopedDataPipeConsumerHandle pipe_consumer = consumer->DrainAsDataPipe();
        if (!pipe_consumer.is_valid()) {
            ...
            MojoResult rv = mojo::CreateDataPipe(&options, out_data_pipe_, pipe_consumer);
            ...
            data_pipe_watcher_.Watch(
                out_data_pipe_.get(), MOJO_HANDLE_SIGNAL_WRITABLE, WTF::BindRepeating(&FetchDataLoaderAsDataPipe::OnWritable, WrapWeakPersistent(this)));
            data_pipe_close_watcher_.Watch(out_data_pipe_.get(), MOJO_HANDLE_SIGNAL_PEER_CLOSED, MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED,
                WTF::BindRepeating(&FetchDataLoaderAsDataPipe::OnPeerClosed, WrapWeakPersistent(this)));
        ...
        while (!should_wait) {
            ...
            if (result == BytesConsumer::Result::kOk) {
                if (buffer.empty()) {
                    ...
                    MojoResult mojo_result = out_data_pipe_->WriteData(base::as_bytes(buffer), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes);
    ...
    String DebugName() const override
    {
        return "FetchDataLoaderAsDataPipe";
    }
...
FetchDataLoader* FetchDataLoader::CreateLoaderAsBlobHandle(const String& mime_type, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    return MakeGarbageCollected<FetchDataLoaderAsBlobHandle>(mime_type, std::move(task_runner));
}
...
FetchDataLoader* FetchDataLoader::CreateLoaderAsString(const TextResourceDecoderOptions& options)
{
    return MakeGarbageCollected<FetchDataLoaderAsString>(options);
}

FetchDataLoader* FetchDataLoader::CreateLoaderAsDataPipe(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    return MakeGarbageCollected<FetchDataLoaderAsDataPipe>(std::move(task_runner));
}
