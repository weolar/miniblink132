...
class BlobBytesConsumerTestClient final : public GarbageCollected<BlobBytesConsumerTestClient>, public BytesConsumer::Client {
public:
    ...
    {
        return "BlobBytesConsumerTestClient";
    }
...
public:
    ...
    scoped_refptr<BlobDataHandle> CreateBlob(const String& body)
    {
        ...
        mojo::MakeSelfOwnedReceiver(std::make_unique<FakeBlob>(kBlobUUID, body, &blob_state_), mojo_blob.InitWithNewPipeAndPassReceiver());
        return BlobDataHandle::Create(kBlobUUID, "", body.length(), std::move(mojo_blob));
    }
...
TEST_F(BlobBytesConsumerTest, TwoPhaseRead)
{
    ...
    BlobBytesConsumer* consumer = MakeGarbageCollected<BlobBytesConsumer>(GetFrame().DomWindow(), blob_data_handle);
    ...
    EXPECT_FALSE(consumer->DrainAsBlobDataHandle(BytesConsumer::BlobSizePolicy::kAllowBlobWithInvalidSize));
...
TEST_F(BlobBytesConsumerTest, CancelBeforeStarting)
{
    ...
    BlobBytesConsumer* consumer = MakeGarbageCollected<BlobBytesConsumer>(GetFrame().DomWindow(), blob_data_handle);
    BlobBytesConsumerTestClient* client = MakeGarbageCollected<BlobBytesConsumerTestClient>();
...
TEST_F(BlobBytesConsumerTest, DrainAsBlobDataHandle)
{
    ...
    scoped_refptr<BlobDataHandle> result = consumer->DrainAsBlobDataHandle(BytesConsumer::BlobSizePolicy::kDisallowBlobWithInvalidSize);
    ASSERT_TRUE(result);
    EXPECT_FALSE(consumer->DrainAsBlobDataHandle(BytesConsumer::BlobSizePolicy::kDisallowBlobWithInvalidSize));
...
TEST_F(BlobBytesConsumerTest, DrainAsBlobDataHandle_2)
{
    scoped_refptr<BlobDataHandle> blob_data_handle
        = BlobDataHandle::Create("uuid", "", std::numeric_limits<uint64_t>::max(), CreateBlob("foo bar")->CloneBlobRemote());
    BlobBytesConsumer* consumer = MakeGarbageCollected<BlobBytesConsumer>(GetFrame().DomWindow(), blob_data_handle);

    EXPECT_EQ(PublicState::kReadableOrWaiting, consumer->GetPublicState());
    EXPECT_FALSE(DidStartLoading());

    scoped_refptr<BlobDataHandle> result = consumer->DrainAsBlobDataHandle(BytesConsumer::BlobSizePolicy::kAllowBlobWithInvalidSize);
    ASSERT_TRUE(result);
    EXPECT_FALSE(consumer->DrainAsBlobDataHandle(BytesConsumer::BlobSizePolicy::kAllowBlobWithInvalidSize));
...
{
    BlobBytesConsumer* consumer = MakeGarbageCollected<BlobBytesConsumer>(GetFrame().DomWindow(), nullptr);
    base::span<const char> buffer;
