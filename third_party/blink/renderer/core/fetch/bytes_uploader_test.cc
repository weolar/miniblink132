...
public:
    ...
    void SetPublicState(PublicState state)
    {
        state_ = state;
    }
...
public:
    ...
    void InitializeBytesUploader(MockBytesConsumer* mock_bytes_consumer, uint32_t capacity = 100u)
    {
        bytes_uploader_ = MakeGarbageCollected<BytesUploader>(nullptr, mock_bytes_consumer, remote_.BindNewPipeAndPassReceiver(),
        ...
        ASSERT_EQ(MOJO_RESULT_OK, mojo::CreateDataPipe(&data_pipe_options, writable_, readable_));
    }
    ...
    mojo::Remote<ChunkedDataPipeGetter>& Remote()
    {
        return remote_;
    }
...
{
    auto* mock_bytes_consumer = MakeGarbageCollected<StrictMock<MockBytesConsumer>>();
    ...
    BytesUploader* bytes_uploader_ = MakeGarbageCollected<BytesUploader>(nullptr, mock_bytes_consumer, pending_remote.InitWithNewPipeAndPassReceiver(),
        blink::scheduler::GetSingleThreadTaskRunnerForTesting(),
        /*client=*/nullptr);
...
TEST_F(BytesUploaderTest, ReadEmpty)
{
    ...
    Checkpoint checkpoint;
    {
        ...
        EXPECT_CALL(*mock_bytes_consumer, BeginRead(_)).WillOnce(Return(BytesConsumer::Result::kDone));
    ...
    MojoResult rv = Readable()->ReadData(MOJO_READ_DATA_FLAG_NONE, base::as_writable_byte_span(buffer), actually_read_bytes);
...
TEST_F(BytesUploaderTest, ReadSmall)
{
    ...
    Checkpoint checkpoint;
    {
        ...
        EXPECT_CALL(checkpoint, Call(2));
        EXPECT_CALL(*mock_bytes_consumer, SetClient(_));
        EXPECT_CALL(*mock_bytes_consumer, BeginRead(_)).WillOnce(Invoke([](base::span<const char>& buffer) {
            buffer = base::span_from_cstring("foobar");
            return BytesConsumer::Result::kOk;
        }));
        EXPECT_CALL(*mock_bytes_consumer, EndRead(6u)).WillOnce(Return(BytesConsumer::Result::kDone));
        EXPECT_CALL(*mock_bytes_consumer, Cancel());
    ...
    std::string buffer(20, '\0');
    size_t actually_read_bytes = 0;
    EXPECT_EQ(MOJO_RESULT_OK, Readable()->ReadData(MOJO_READ_DATA_FLAG_NONE, base::as_writable_byte_span(buffer), actually_read_bytes));
...
TEST_F(BytesUploaderTest, ReadOverPipeCapacity)
{
    ...
    Checkpoint checkpoint;
    {
        ...
        EXPECT_CALL(*mock_bytes_consumer, EndRead(10u)).WillOnce(Return(BytesConsumer::Result::kOk));

        EXPECT_CALL(*mock_bytes_consumer, BeginRead(_)).WillOnce(Invoke([](base::span<const char>& buffer) {
            buffer = base::span_from_cstring("AR");
            return BytesConsumer::Result::kOk;
        }));
        EXPECT_CALL(*mock_bytes_consumer, EndRead(0u)).WillOnce(Return(BytesConsumer::Result::kOk));
        ...
        EXPECT_CALL(*mock_bytes_consumer, EndRead(2u)).WillOnce(Return(BytesConsumer::Result::kDone));
        EXPECT_CALL(*mock_bytes_consumer, Cancel());
    ...
    EXPECT_EQ(MOJO_RESULT_OK, Readable()->ReadData(MOJO_READ_DATA_FLAG_NONE, base::as_writable_byte_span(buffer2), actually_read_bytes));
...
{
    auto* mock_bytes_consumer = MakeGarbageCollected<NiceMock<MockBytesConsumer>>();
    InitializeBytesUploader(mock_bytes_consumer);
