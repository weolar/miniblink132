...
class CORE_EXPORT BodyStreamBuffer final : public UnderlyingByteSourceBase, public ExecutionContextLifecycleObserver, public BytesConsumer::Client {
public:
    ...
    static BodyStreamBuffer* Create(ScriptState*, BytesConsumer* consumer, AbortSignal* signal, ScriptCachedMetadataHandler* cached_metadata_handler,
        scoped_refptr<BlobDataHandle> side_data_blob = nullptr);

    // Create() should be used instead of calling this constructor directly.
    BodyStreamBuffer(PassKey, ScriptState*, BytesConsumer* consumer, AbortSignal* signal, ScriptCachedMetadataHandler* cached_metadata_handler,
        scoped_refptr<BlobDataHandle> side_data_blob);

    BodyStreamBuffer(
        ScriptState*, ReadableStream* stream, ScriptCachedMetadataHandler* cached_metadata_handler, scoped_refptr<BlobDataHandle> side_data_blob = nullptr);
    ...
    scoped_refptr<BlobDataHandle> DrainAsBlobDataHandle(BytesConsumer::BlobSizePolicy, ExceptionState&);
    scoped_refptr<EncodedFormData> DrainAsFormData(ExceptionState&);
    void DrainAsChunkedDataPipeGetter(ScriptState*, mojo::PendingReceiver<network::mojom::blink::ChunkedDataPipeGetter>, BytesUploader::Client* client);
    ...
    void StartLoading(FetchDataLoader*, FetchDataLoader::Client* /* client */, ExceptionState&);
    ...
    bool IsMadeFromReadableStream() const
    {
        return made_from_readable_stream_;
    }
