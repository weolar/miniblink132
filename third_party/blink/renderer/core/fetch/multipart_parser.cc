...
MultipartParser::Matcher::Matcher(base::span<const char> match_data, size_t num_matched_bytes)
...
bool MultipartParser::AppendData(base::span<const char> bytes)
{
    ...
    while (!bytes.empty()) {
        ...
        case State::kParsingPartOctets: {
            ...
            ParseDelimiter(bytes);
            if (!matcher_.IsMatchComplete() && !bytes.empty()) {
                if (matcher_.NumMatchedBytes() >= num_initially_matched_bytes && num_initially_matched_bytes > 0u) {
...
MultipartParser::Matcher MultipartParser::DelimiterMatcher(size_t num_already_matched_bytes) const
{
    return Matcher(base::span<const char>(delimiter_.data(), delimiter_.size()), num_already_matched_bytes);
}
...
void MultipartParser::ParseDataAndDelimiter(base::span<const char>& bytes)
{
    ...
    if (delimiter_begin != bytes.end()) {
        ...
        // bytes.
        auto delimiter_and_rest = bytes.subspan(std::distance(bytes.begin(), delimiter_begin));
        ...
        auto maybe_delimiter_span = bytes.last(std::min(static_cast<size_t>(delimiter_.size() - 1u), bytes.size()));
...
bool MultipartParser::ParseHeaderFields(base::span<const char>& bytes, HTTPHeaderMap* header_fields)
{
    ...
    if (!buffered_header_bytes_.empty()) {
        buffered_header_bytes_.Append(header_bytes.data(), base::checked_cast<wtf_size_t>(header_bytes.size()));
        header_bytes = base::span<const char>(buffered_header_bytes_.data(), buffered_header_bytes_.size());
    }

    wtf_size_t end = 0u;
    if (!ParseMultipartFormHeadersFromBody(base::as_bytes(header_bytes), header_fields, &end)) {
...
void MultipartParser::ParseTransportPadding(base::span<const char>& bytes) const
{
    size_t matched = 0;
    while (matched < bytes.size() && (bytes[matched] == '\t' || bytes[matched] == ' ')) {
        ++matched;
    }
    bytes = bytes.subspan(matched);
}
