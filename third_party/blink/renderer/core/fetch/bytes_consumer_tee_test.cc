...
class BytesConsumerTestClient final : public GarbageCollected<BytesConsumerTestClient>, public BytesConsumer::Client {
public:
    void OnStateChange() override
    {
        ++num_on_state_change_called_;
    }
    String DebugName() const override
    {
        return "BytesConsumerTestClient";
    }
    int NumOnStateChangeCalled() const
    {
        return num_on_state_change_called_;
    }
...
public:
    ...
    scoped_refptr<BlobDataHandle> DrainAsBlobDataHandle(BlobSizePolicy policy) override
    {
        if (state_ != PublicState::kReadableOrWaiting)
            return nullptr;
        DCHECK(blob_handle_);
        if (policy == BlobSizePolicy::kDisallowBlobWithInvalidSize && blob_handle_->size() == UINT64_MAX)
            return nullptr;
...
public:
    ...
    String DebugName() const override
    {
        return "FakeFormDataBytesConsumer";
    }
...
{
    ReplayingBytesConsumer* src = MakeGarbageCollected<ReplayingBytesConsumer>(GetDocument().GetTaskRunner(TaskType::kNetworking));
...
TEST_F(BytesConsumerTeeTest, TwoPhaseRead)
{
    ...
    EXPECT_EQ(BytesConsumer::PublicState::kReadableOrWaiting, dest1->GetPublicState());
    EXPECT_EQ(BytesConsumer::PublicState::kReadableOrWaiting, dest2->GetPublicState());
...
{
    scoped_refptr<BlobDataHandle> blob_data_handle = BlobDataHandle::Create(std::make_unique<BlobData>(), 12345);
    BytesConsumer* src = MakeGarbageCollected<FakeBlobBytesConsumer>(blob_data_handle);

    BytesConsumer* dest1 = nullptr;
    BytesConsumer* dest2 = nullptr;
    BytesConsumerTee(GetFrame().DomWindow(), src, &dest1, &dest2);

    scoped_refptr<BlobDataHandle> dest_blob_data_handle1 = dest1->DrainAsBlobDataHandle(BytesConsumer::BlobSizePolicy::kAllowBlobWithInvalidSize);
    scoped_refptr<BlobDataHandle> dest_blob_data_handle2 = dest2->DrainAsBlobDataHandle(BytesConsumer::BlobSizePolicy::kDisallowBlobWithInvalidSize);
...
{
    scoped_refptr<BlobDataHandle> blob_data_handle = BlobDataHandle::Create(std::make_unique<BlobData>(), std::numeric_limits<uint64_t>::max());
    BytesConsumer* src = MakeGarbageCollected<FakeBlobBytesConsumer>(blob_data_handle);
...
TEST_F(BytesConsumerTeeTest, ConsumerCanBeErroredInTwoPhaseRead)
{
    ...
    BytesConsumer* dest2 = nullptr;
    BytesConsumerTee(GetFrame().DomWindow(), src, &dest1, &dest2);
    BytesConsumerTestClient* client = MakeGarbageCollected<BytesConsumerTestClient>();
    ...
    EXPECT_EQ(Result::kError, (MakeGarbageCollected<BytesConsumerTestReader>(dest2))->Run().first);
    EXPECT_EQ(BytesConsumer::PublicState::kErrored, dest1->GetPublicState());
...
TEST_F(BytesConsumerTeeTest, AsyncNotificationShouldBeDispatchedWhenAllDataIsConsumed)
{
    ...
    EXPECT_EQ(BytesConsumer::PublicState::kReadableOrWaiting, src->GetPublicState());
