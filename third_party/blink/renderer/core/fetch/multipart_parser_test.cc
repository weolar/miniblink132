...
class MockMultipartParserClient final : public GarbageCollected<MockMultipartParserClient>, public MultipartParser::Client {
public:
    struct Part {
        Part() = default;
        explicit Part(const HTTPHeaderMap& header_fields)
            : header_fields(header_fields)
            , data_fully_received(false)
        {
        }
    ...
    wtf_size_t NumberOfParts() const
    {
        return parts_.size();
    }
...
constexpr char kBytes[] = "preamble"
                          "\r\n--boundary\r\n\r\n"
...
TEST(MultipartParserTest, AppendDataInChunks)
{
    ...
    for (const size_t size : sizes) {
        MockMultipartParserClient* client = MakeGarbageCollected<MockMultipartParserClient>();
        MultipartParser* parser = MakeGarbageCollected<MultipartParser>(boundary, client);
        ...
        EXPECT_EQ("application/xhtml+xml", client->GetPart(1).header_fields.Get(http_names::kContentType));
        ...
        EXPECT_EQ("text/html", client->GetPart(2).header_fields.Get(http_names::kContentType));
        ...
        EXPECT_EQ("text/plain; charset=iso-8859-1", client->GetPart(3).header_fields.Get(http_names::kContentType));
...
TEST(MultipartParserTest, NoEndBoundary)
{
    ...
    EXPECT_EQ("application/xhtml+xml", client->GetPart(0).header_fields.Get(http_names::kContentType));
...
TEST(MultipartParserTest, NoStartBoundary)
{
    ...
    EXPECT_FALSE(parser->AppendData(base::span_from_cstring(bytes))); // Close delimiter before delimiter.
...
TEST(MultipartParserTest, NoStartNorEndBoundary)
{
    ...
    EXPECT_TRUE(parser->AppendData(base::span_from_cstring(bytes))); // Valid preamble.
...
TEST(MultipartParserTest, Preamble)
{
    ...
    for (const size_t start : kStarts) {
        ...
        case 11u:
            ...
            EXPECT_TRUE(client->GetPart(0).data_fully_received);
            EXPECT_EQ(1u, client->GetPart(1).header_fields.size());
            EXPECT_EQ("text/html", client->GetPart(1).header_fields.Get(http_names::kContentType));
            EXPECT_EQ("2\r\n--\r\n--bound--\r\n--\r\n2\r\n", String(client->GetPart(1).data));
            EXPECT_TRUE(client->GetPart(1).data_fully_received);
            EXPECT_EQ(1u, client->GetPart(2).header_fields.size());
            EXPECT_EQ("text/plain; charset=iso-8859-1", client->GetPart(2).header_fields.Get(http_names::kContentType));
