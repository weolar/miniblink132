...
class MockClient : public GarbageCollected<MockClient>, public BytesConsumer::Client {
public:
    MockClient() = default;
...
TEST(ReadableStreamBytesConsumerTest, Create)
{
    ...
    Persistent<BytesConsumer> consumer = MakeGarbageCollected<ReadableStreamBytesConsumer>(script_state, stream);
...
TEST(ReadableStreamBytesConsumerTest, ErroredStream)
{
    ...
    auto* underlying_source = MakeGarbageCollected<TestUnderlyingSource>(script_state);
    auto* stream = ReadableStream::CreateWithCountQueueingStrategy(script_state, underlying_source, 0);
    underlying_source->Error(ScriptValue(script_state->GetIsolate(), v8::Undefined(script_state->GetIsolate())));
...
TEST(ReadableStreamBytesConsumerTest, TwoPhaseRead)
{
    ...
    auto* stream = ReadableStream::CreateWithCountQueueingStrategy(script_state, underlying_source, 0);
    {
        ...
        auto* chunk2 = DOMUint8Array::Create(std::to_array<uint8_t>({ 0x43, 0x44, 0x45, 0x46 }));
        auto* chunk3 = DOMUint8Array::Create(std::to_array<uint8_t>({ 0x47, 0x48, 0x49, 0x4a }));
        underlying_source->Enqueue(ScriptValue(script_state->GetIsolate(), ToV8Traits<DOMUint8Array>::ToV8(script_state, chunk1)));
        underlying_source->Enqueue(ScriptValue(script_state->GetIsolate(), ToV8Traits<DOMUint8Array>::ToV8(script_state, chunk2)));
        underlying_source->Enqueue(ScriptValue(script_state->GetIsolate(), ToV8Traits<DOMUint8Array>::ToV8(script_state, chunk3)));
        underlying_source->Close();
    }
...
TEST(ReadableStreamBytesConsumerTest, TwoPhaseReadDetachedDuringRead)
{
    ...
    auto* chunk = DOMUint8Array::Create(std::to_array<uint8_t>({ 0x43, 0x44, 0x45, 0x46 }));
    underlying_source->Enqueue(ScriptValue(script_state->GetIsolate(), ToV8Traits<DOMUint8Array>::ToV8(script_state, chunk)));
    underlying_source->Close();
...
TEST(ReadableStreamBytesConsumerTest, EnqueueUndefined)
{
    ...
    auto* underlying_source = MakeGarbageCollected<TestUnderlyingSource>(script_state);
    auto* stream = ReadableStream::CreateWithCountQueueingStrategy(script_state, underlying_source, 0);
    underlying_source->Enqueue(ScriptValue(script_state->GetIsolate(), v8::Undefined(script_state->GetIsolate())));
    underlying_source->Close();
...
TEST(ReadableStreamBytesConsumerTest, EnqueueNull)
{
    ...
    auto* underlying_source = MakeGarbageCollected<TestUnderlyingSource>(script_state);
    auto* stream = ReadableStream::CreateWithCountQueueingStrategy(script_state, underlying_source, 0);
    underlying_source->Enqueue(ScriptValue(script_state->GetIsolate(), v8::Null(script_state->GetIsolate())));
    underlying_source->Close();
...
TEST(ReadableStreamBytesConsumerTest, EnqueueString)
{
    ...
    auto* underlying_source = MakeGarbageCollected<TestUnderlyingSource>(script_state);
    auto* stream = ReadableStream::CreateWithCountQueueingStrategy(script_state, underlying_source, 0);
    underlying_source->Enqueue(ScriptValue(script_state->GetIsolate(), V8String(script_state->GetIsolate(), "hello")));
    underlying_source->Close();
