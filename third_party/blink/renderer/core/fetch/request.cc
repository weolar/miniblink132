...
V8RequestDestination::Enum DestinationToV8Enum(network::mojom::RequestDestination destination)
{
    switch (destination) {
...
FetchRequestData* CreateCopyOfFetchRequestDataForFetch(ScriptState* script_state, const FetchRequestData* original)
{
    ...
    request->SetOrigin(original->Origin() ? original->Origin() : context->GetSecurityOrigin());
    ...
    if (original->URLLoaderFactory()) {
        ...
        original->URLLoaderFactory()->Clone(factory_clone.InitWithNewPipeAndPassReceiver());
    ...
    request->SetAttributionReportingEligibility(original->AttributionReportingEligibility());
    request->SetAttributionReportingSupport(original->AttributionSupport());
    request->SetServiceWorkerRaceNetworkRequestToken(original->ServiceWorkerRaceNetworkRequestToken());
...
{
    return init->hasMethod() || init->hasHeaders() || init->hasBody() || init->hasReferrer() || init->hasReferrerPolicy() || init->hasMode()
        || init->hasTargetAddressSpace() || init->hasCredentials() || init->hasCache() || init->hasRedirect() || init->hasIntegrity() || init->hasKeepalive()
        || init->hasBrowsingTopics() || init->hasAdAuctionHeaders() || init->hasSharedStorageWritable() || init->hasPriority() || init->hasSignal()
        || init->hasDuplex() || init->hasPrivateToken() || init->hasAttributionReporting();
}

static BodyStreamBuffer* ExtractBody(
    ScriptState* script_state, ExceptionState& exception_state, v8::Local<v8::Value> body, String& content_type, uint64_t& body_byte_length)
{
    DCHECK(!body->IsNull());
    ...
    if (Blob* blob = V8Blob::ToWrappable(isolate, body)) {
        ...
        return_buffer = BodyStreamBuffer::Create(script_state, MakeGarbageCollected<BlobBytesConsumer>(execution_context, blob->GetBlobDataHandle()),
        ...
        DOMArrayBuffer* array_buffer = NativeValueTraits<DOMArrayBuffer>::NativeValue(isolate, body, exception_state);
        if (exception_state.HadException())
            return nullptr;
        if (!base::CheckedNumeric<wtf_size_t>(array_buffer->ByteLength()).IsValid()) {
            exception_state.ThrowRangeError("The provided ArrayBuffer exceeds the maximum supported size");
            return nullptr;
        }
        body_byte_length = array_buffer->ByteLength();
        return_buffer = BodyStreamBuffer::Create(
            script_state, MakeGarbageCollected<FormDataBytesConsumer>(array_buffer), nullptr /* AbortSignal */, /*cached_metadata_handler=*/nullptr);
    } else if (body->IsArrayBufferView()) {
        // Avoid calling into V8 from the following constructor parameters, which
        // is potentially unsafe.
        DOMArrayBufferView* array_buffer_view = NativeValueTraits<MaybeShared<DOMArrayBufferView>>::NativeValue(isolate, body, exception_state).Get();
        if (exception_state.HadException())
            return nullptr;
        if (!base::CheckedNumeric<wtf_size_t>(array_buffer_view->byteLength()).IsValid()) {
            exception_state.ThrowRangeError("The provided ArrayBufferView exceeds the maximum supported size");
            return nullptr;
        }
        body_byte_length = array_buffer_view->byteLength();
        return_buffer = BodyStreamBuffer::Create(
            script_state, MakeGarbageCollected<FormDataBytesConsumer>(array_buffer_view), nullptr /* AbortSignal */, /*cached_metadata_handler=*/nullptr);
        ...
        content_type = AtomicString("multipart/form-data; boundary=") + form_data->Boundary().data();
        body_byte_length = form_data->SizeInBytes();
        return_buffer = BodyStreamBuffer::Create(script_state, MakeGarbageCollected<FormDataBytesConsumer>(execution_context, std::move(form_data)),
            nullptr /* AbortSignal */, /*cached_metadata_handler=*/nullptr);
    } else if (URLSearchParams* url_search_params = V8URLSearchParams::ToWrappable(isolate, body)) {
        scoped_refptr<EncodedFormData> form_data = url_search_params->ToEncodedFormData();
        body_byte_length = form_data->SizeInBytes();
        return_buffer = BodyStreamBuffer::Create(script_state, MakeGarbageCollected<FormDataBytesConsumer>(execution_context, std::move(form_data)),
            nullptr /* AbortSignal */, /*cached_metadata_handler=*/nullptr);
        content_type = "application/x-www-form-urlencoded;charset=UTF-8";
    } else if (ReadableStream* readable_stream = V8ReadableStream::ToWrappable(isolate, body);
               readable_stream && RuntimeEnabledFeatures::FetchUploadStreamingEnabled(execution_context)) {
        ...
        if (readable_stream->IsDisturbed()) {
            exception_state.ThrowTypeError("The provided ReadableStream is disturbed");
            return nullptr;
        }
        ...
        return_buffer = MakeGarbageCollected<BodyStreamBuffer>(script_state, readable_stream, /*cached_metadata_handler=*/nullptr);
    } else {
        String string = NativeValueTraits<IDLUSVString>::NativeValue(isolate, body, exception_state);
        if (exception_state.HadException())
            return nullptr;

        body_byte_length = string.length();
        return_buffer = BodyStreamBuffer::Create(
            script_state, MakeGarbageCollected<FormDataBytesConsumer>(string), nullptr /* AbortSignal */, /*cached_metadata_handler=*/nullptr);
...
Request* Request::CreateRequestWithRequestOrString(
    ScriptState* script_state, Request* input_request, const String& input_string, const RequestInit* init, ExceptionState& exception_state)
{
    ExecutionContext* execution_context = ExecutionContext::From(script_state);
    scoped_refptr<const SecurityOrigin> origin = execution_context->GetSecurityOrigin();
    ...
    FetchRequestData* request = CreateCopyOfFetchRequestDataForFetch(
        script_state, input_request ? input_request->GetRequest() : MakeGarbageCollected<FetchRequestData>(execution_context));
    ...
    if (!input_request) {
        ...
        if (!parsed_url.User().empty() || !parsed_url.Pass().empty()) {
            exception_state.ThrowTypeError("Request cannot be constructed from a URL that includes "
                                           "credentials: "
                + input_string);
            return nullptr;
        }
        ...
        if (parsed_url.ProtocolIs("blob")) {
            ...
            ExecutionContext::From(script_state)->GetPublicURLManager().Resolve(parsed_url, url_loader_factory.InitWithNewPipeAndPassReceiver());
    ...
    if (init->hasReferrer()) {
        ...
        if (init->referrer().empty()) {
            ...
            if (!parsed_referrer.IsValid()) {
                ...
                exception_state.ThrowTypeError("Referrer '" + init->referrer() + "' is not a valid URL.");
                return nullptr;
            }
            if ((parsed_referrer.ProtocolIsAbout() && parsed_referrer.Host().empty() && parsed_referrer.GetPath() == "client")
                || !origin->IsSameOriginWith(SecurityOrigin::Create(parsed_referrer).get())) {
    ...
    if (init->hasReferrerPolicy()) {
        ...
        case V8ReferrerPolicy::Enum::kOriginWhenCrossOrigin:
            referrer_policy = network::mojom::ReferrerPolicy::kOriginWhenCrossOrigin;
            break;
        case V8ReferrerPolicy::Enum::kStrictOriginWhenCrossOrigin:
            referrer_policy = network::mojom::ReferrerPolicy::kStrictOriginWhenCrossOrigin;
            break;
    ...
    if (init->hasMode()) {
        ...
        if (mode == network::mojom::RequestMode::kNavigate) {
            exception_state.ThrowTypeError("Cannot construct a Request with a RequestInit whose mode member is "
                                           "set as 'navigate'.");
            return nullptr;
        }
    ...
    if (init->hasCredentials()) {
        request->SetCredentials(V8RequestCredentialsToCredentialsMode(init->credentials().AsEnum()));
    ...
    if (request->CacheMode() == mojom::blink::FetchCacheMode::kOnlyIfCached && request->Mode() != network::mojom::RequestMode::kSameOrigin) {
        exception_state.ThrowTypeError("'only-if-cached' can be set only with 'same-origin' mode");
        return nullptr;
    }
    ...
    if (init->hasBrowsingTopics()) {
        if (!execution_context->IsSecureContext()) {
            exception_state.ThrowTypeError("browsingTopics: Topics operations are only available in secure "
                                           "contexts.");
            return nullptr;
        }
        ...
        if (init->browsingTopics()) {
            UseCounter::Count(execution_context, mojom::blink::WebFeature::kTopicsAPIFetch);
            UseCounter::Count(execution_context, mojom::blink::WebFeature::kTopicsAPIAll);
        }
    }
    ...
    if (init->hasSharedStorageWritable()) {
        if (!execution_context->IsSecureContext()) {
            exception_state.ThrowTypeError("sharedStorageWritable: sharedStorage operations are only available"
                                           " in secure contexts.");
            return nullptr;
        }
        if (SecurityOrigin::Create(request->Url())->IsOpaque()) {
            exception_state.ThrowTypeError("sharedStorageWritable: sharedStorage operations are not available"
                                           " for opaque origins.");
            return nullptr;
        }
        request->SetSharedStorageWritable(init->sharedStorageWritable());
        if (init->sharedStorageWritable()) {
            UseCounter::Count(execution_context, mojom::blink::WebFeature::kSharedStorageAPI_Fetch_Attribute);
        }
    }
    ...
    if (init->hasMethod()) {
        ...
        if (!IsValidHTTPToken(init->method())) {
            exception_state.ThrowTypeError("'" + init->method() + "' is not a valid HTTP method.");
            return nullptr;
        }
        if (FetchUtils::IsForbiddenMethod(init->method())) {
            exception_state.ThrowTypeError("'" + init->method() + "' HTTP method is unsupported.");
            return nullptr;
        }
        // "Normalize |method|."
    ...
    if (init->hasPrivateToken()) {
        UseCounter::Count(ExecutionContext::From(script_state), mojom::blink::WebFeature::kTrustTokenFetch);

        network::mojom::blink::TrustTokenParams params;
        if (!ConvertTrustTokenToMojomAndCheckPermissions(*init->privateToken(), GetPSTFeatures(*execution_context), &exception_state, &params)) {
        ...
        if (!execution_context->IsSecureContext()) {
            exception_state.ThrowTypeError("trustToken: TrustTokens operations are only available in secure "
                                           "contexts.");
            return nullptr;
        }
    ...
    if (init->hasAttributionReporting()) {
        if (!execution_context->IsSecureContext()) {
            exception_state.ThrowTypeError("attributionReporting: Attribution Reporting operations are only "
                                           "available in secure contexts.");
            return nullptr;
        }

        request->SetAttributionReportingEligibility(
            ConvertAttributionReportingRequestOptionsToMojom(*init->attributionReporting(), *execution_context, exception_state));
    }
    ...
    // from |signals|".
    auto* request_signal = MakeGarbageCollected<AbortSignal>(script_state, signals);
    ...
    if (r->GetRequest()->Mode() == network::mojom::RequestMode::kNoCors) {
        ...
        if (!cors::IsCorsSafelistedMethod(r->GetRequest()->Method())) {
            exception_state.ThrowTypeError("'" + r->GetRequest()->Method() + "' is unsupported in no-cors mode.");
            return nullptr;
        }
    ...
    BodyStreamBuffer* input_body = input_request ? input_request->BodyBuffer() : nullptr;
    uint64_t input_body_byte_length = input_request ? input_request->BodyBufferByteLength() : 0;

    // "If either |init|["body"] exists and is non-null or |inputBody| is
    // non-null, and |request|'s method is `GET` or `HEAD`, throw a TypeError.
    v8::Local<v8::Value> init_body = init->hasBody() ? init->body().V8Value() : v8::Local<v8::Value>();
    if ((!init_body.IsEmpty() && !init_body->IsNull()) || input_body) {
        if (request->Method() == http_names::kGET || request->Method() == http_names::kHEAD) {
            exception_state.ThrowTypeError("Request with GET/HEAD method cannot have body.");
            return nullptr;
        }
    }
    ...
    if (!init_body.IsEmpty() && !init_body->IsNull()) {
        ...
        // From "extract a body":
        // - If the keepalive flag is set, then throw a TypeError.
        if (init->hasKeepalive() && init->keepalive() && V8ReadableStream::HasInstance(script_state->GetIsolate(), init_body)) {
            exception_state.ThrowTypeError("Keepalive request cannot have a ReadableStream body.");
            return nullptr;
        }
        ...
        //  init["body"]."
        String content_type;
        body = ExtractBody(script_state, exception_state, init_body, content_type, body_byte_length);
        ...
        if (!content_type.empty() && !r->getHeaders()->has(http_names::kContentType, exception_state)) {
            r->getHeaders()->append(script_state, http_names::kContentType, content_type, exception_state);
        }
        if (exception_state.HadException())
            return nullptr;
    }
    ...
    if (body && body->IsMadeFromReadableStream()) {
        ...
        if (!init_body.IsEmpty() && !init_body->IsNull() && !init->hasDuplex()) {
            exception_state.ThrowTypeError("The `duplex` member must be specified for a request with a "
                                           "streaming body");
            return nullptr;
        }
        ...
        // throw a TypeError."
        if (request->Mode() != network::mojom::RequestMode::kSameOrigin && request->Mode() != network::mojom::RequestMode::kCors
            && request->Mode() != network::mojom::RequestMode::kCorsWithForcedPreflight) {
            exception_state.ThrowTypeError("If request is made from ReadableStream, mode should be"
                                           "\"same-origin\" or \"cors\"");
            return nullptr;
        }
    ...
    // TypeError."
    if (input_body == body && input_request && (input_request->IsBodyUsed() || input_request->IsBodyLocked())) {
        exception_state.ThrowTypeError("Cannot construct a Request with a Request object that has already "
                                       "been used.");
        return nullptr;
    }
    ...
    if (input_request && input_request->BodyBuffer()) {
        ...
        auto* dummy_stream = BodyStreamBuffer::Create(script_state, BytesConsumer::CreateClosed(), nullptr, /*cached_metadata_handler=*/nullptr);
...
Request* Request::Create(ScriptState* script_state, const V8RequestInfo* input, const RequestInit* init, ExceptionState& exception_state)
{
    DCHECK(input);
...
Request* Request::Create(ScriptState* script_state, const String& input, ExceptionState& exception_state)
{
    return Create(script_state, input, RequestInit::Create(), exception_state);
}

Request* Request::Create(ScriptState* script_state, const String& input, const RequestInit* init, ExceptionState& exception_state)
{
    return CreateRequestWithRequestOrString(script_state, nullptr, input, init, exception_state);
}

Request* Request::Create(ScriptState* script_state, Request* input, ExceptionState& exception_state)
{
    return Create(script_state, input, RequestInit::Create(), exception_state);
}

Request* Request::Create(ScriptState* script_state, Request* input, const RequestInit* init, ExceptionState& exception_state)
{
    return CreateRequestWithRequestOrString(script_state, input, String(), init, exception_state);
}

Request* Request::Create(ScriptState* script_state, FetchRequestData* request, AbortSignal* signal)
{
    return MakeGarbageCollected<Request>(script_state, request, signal);
}

Request* Request::Create(
    ScriptState* script_state, mojom::blink::FetchAPIRequestPtr fetch_api_request, ForServiceWorkerFetchEvent for_service_worker_fetch_event)
{
    FetchRequestData* data = FetchRequestData::Create(script_state, std::move(fetch_api_request), for_service_worker_fetch_event);
    auto* signal = MakeGarbageCollected<AbortSignal>(ExecutionContext::From(script_state));
    return MakeGarbageCollected<Request>(script_state, data, signal);
}

network::mojom::CredentialsMode Request::V8RequestCredentialsToCredentialsMode(V8RequestCredentials::Enum credentials_mode)
{
...
Request::Request(ScriptState* script_state, FetchRequestData* request, Headers* headers, AbortSignal* signal)
...
Request::Request(ScriptState* script_state, FetchRequestData* request, AbortSignal* signal)
    : Request(script_state, request, Headers::Create(request->HeaderList()), signal)
{
...
V8ReferrerPolicy Request::getReferrerPolicy() const
{
    ...
    case network::mojom::ReferrerPolicy::kStrictOriginWhenCrossOrigin:
        return V8ReferrerPolicy(V8ReferrerPolicy::Enum::kStrictOriginWhenCrossOrigin);
    }
    NOTREACHED();
}
...
Request* Request::clone(ScriptState* script_state, ExceptionState& exception_state)
{
...
FetchRequestData* Request::PassRequestData(ScriptState* script_state, ExceptionState& exception_state)
{
    DCHECK(!IsBodyUsed());
...
mojom::blink::FetchAPIRequestPtr Request::CreateFetchAPIRequest() const
{
    ...
    for (const auto& header : headers_->HeaderList()->List()) {
        ...
        if (!result.is_new_entry) {
            result.stored_value->value = result.stored_value->value + ", " + String(value);
        }
    }
    ...
    if (!request_->ReferrerString().empty()) {
        fetch_api_request->referrer = mojom::blink::Referrer::New(KURL(NullURL(), request_->ReferrerString()), request_->GetReferrerPolicy());
