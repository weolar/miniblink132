...
class ReadableStreamBytesConsumer::BytesConsumerReadRequest final : public ReadRequest {
public:
    ...
    void ChunkSteps(ScriptState* script_state, v8::Local<v8::Value> chunk, ExceptionState& exception_state) const override
    {
        ...
        consumer_->OnRead(NativeValueTraits<MaybeShared<DOMUint8Array>>::NativeValue(script_state->GetIsolate(), chunk, exception_state).Get());
...
ReadableStreamBytesConsumer::ReadableStreamBytesConsumer(ScriptState* script_state, ReadableStream* stream)
...
BytesConsumer::Result ReadableStreamBytesConsumer::BeginRead(base::span<const char>& buffer)
{
    ...
    if (pending_buffer_) {
        ...
        buffer = base::as_chars(pending_buffer_->ByteSpan().subspan(pending_offset_));
        return Result::kOk;
    }
    if (!is_reading_) {
        is_reading_ = true;
...
void ReadableStreamBytesConsumer::OnRead(DOMUint8Array* buffer)
{
    ...
    if (is_inside_read_) {
        scoped_refptr<scheduler::EventLoop> event_loop = ExecutionContext::From(script_state_)->GetAgent()->event_loop();
        event_loop->EnqueueMicrotask(WTF::BindOnce(&ReadableStreamBytesConsumer::OnRead, WrapPersistent(this), WrapPersistent(buffer)));
        return;
    }
    is_reading_ = false;
    if (state_ == PublicState::kClosed)
...
void ReadableStreamBytesConsumer::OnReadDone()
{
    ...
    if (is_inside_read_) {
        scoped_refptr<scheduler::EventLoop> event_loop = ExecutionContext::From(script_state_)->GetAgent()->event_loop();
        event_loop->EnqueueMicrotask(WTF::BindOnce(&ReadableStreamBytesConsumer::OnReadDone, WrapPersistent(this)));
        return;
    }
    is_reading_ = false;
    if (state_ == PublicState::kClosed)
...
void ReadableStreamBytesConsumer::OnRejected()
{
    ...
    if (is_inside_read_) {
        scoped_refptr<scheduler::EventLoop> event_loop = ExecutionContext::From(script_state_)->GetAgent()->event_loop();
        event_loop->EnqueueMicrotask(WTF::BindOnce(&ReadableStreamBytesConsumer::OnRejected, WrapPersistent(this)));
        return;
    }
    is_reading_ = false;
    if (state_ == PublicState::kClosed)
