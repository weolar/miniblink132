...
TEST_F(FetchResponseDataTest, HeaderList)
{
    ...
    ASSERT_TRUE(response_data->HeaderList()->Get("cache-control", cache_control_value));
...
TEST_F(FetchResponseDataTest, ToFetchAPIResponseDefaultType)
{
    ...
    mojom::blink::FetchAPIResponsePtr fetch_api_response = internal_response->PopulateFetchAPIResponse(KURL());
    EXPECT_EQ(network::mojom::FetchResponseType::kDefault, fetch_api_response->response_type);
...
{
    FetchResponseData* internal_response = CreateInternalResponse();
    FetchResponseData* basic_response_data = internal_response->CreateBasicFilteredResponse();
    ...
    String cache_control_value;
    ASSERT_TRUE(basic_response_data->HeaderList()->Get("cache-control", cache_control_value));
    EXPECT_EQ("no-cache", cache_control_value);
}
...
TEST_F(FetchResponseDataTest, ToFetchAPIResponseBasicType)
{
    ...
    mojom::blink::FetchAPIResponsePtr fetch_api_response = basic_response_data->PopulateFetchAPIResponse(KURL());
    EXPECT_EQ(network::mojom::FetchResponseType::kBasic, fetch_api_response->response_type);
    CheckHeaders(*fetch_api_response);
}
...
{
    FetchResponseData* internal_response = CreateInternalResponse();
    FetchResponseData* cors_response_data = internal_response->CreateCorsFilteredResponse(HTTPHeaderSet());
    ...
    String cache_control_value;
    ASSERT_TRUE(cors_response_data->HeaderList()->Get("cache-control", cache_control_value));
    EXPECT_EQ("no-cache", cache_control_value);
}
...
{
    FetchResponseData* internal_response = CreateInternalResponse();
    internal_response->HeaderList()->Append("access-control-expose-headers", "set-cookie, bar");

    FetchResponseData* cors_response_data = internal_response->CreateCorsFilteredResponse({ "set-cookie", "bar" });
...
TEST_F(FetchResponseDataTest, CorsFilterWithExplicitHeaderSet)
{
    ...
    FetchResponseData* cors_response_data = internal_response->CreateCorsFilteredResponse(exposed_headers);
...
TEST_F(FetchResponseDataTest, ToFetchAPIResponseCorsType)
{
    ...
    mojom::blink::FetchAPIResponsePtr fetch_api_response = cors_response_data->PopulateFetchAPIResponse(KURL());
    EXPECT_EQ(network::mojom::FetchResponseType::kCors, fetch_api_response->response_type);
    CheckHeaders(*fetch_api_response);
}
...
{
    FetchResponseData* internal_response = CreateInternalResponse();
    FetchResponseData* opaque_response_data = internal_response->CreateOpaqueFilteredResponse();
...
{
    FetchResponseData* internal_response = CreateInternalResponse();
    FetchResponseData* opaque_response_data = internal_response->CreateOpaqueRedirectFilteredResponse();
...
TEST_F(FetchResponseDataTest, ToFetchAPIResponseOpaqueType)
{
    ...
    mojom::blink::FetchAPIResponsePtr fetch_api_response = opaque_response_data->PopulateFetchAPIResponse(KURL());
    EXPECT_EQ(network::mojom::FetchResponseType::kOpaque, fetch_api_response->response_type);
    CheckHeaders(*fetch_api_response);
}
...
{
    FetchResponseData* internal_response = CreateInternalResponse();
    FetchResponseData* opaque_redirect_response_data = internal_response->CreateOpaqueRedirectFilteredResponse();

    mojom::blink::FetchAPIResponsePtr fetch_api_response = opaque_redirect_response_data->PopulateFetchAPIResponse(KURL());
    EXPECT_EQ(network::mojom::FetchResponseType::kOpaqueRedirect, fetch_api_response->response_type);
    CheckHeaders(*fetch_api_response);
}
...
TEST_F(FetchResponseDataTest, ContentSecurityPolicy)
{
    ...
    FetchResponseData* internal_response = CreateInternalResponse();
    internal_response->HeaderList()->Append("content-security-policy", "frame-ancestors 'none'");
    internal_response->HeaderList()->Append("content-security-policy-report-only", "frame-ancestors 'none'");

    mojom::blink::FetchAPIResponsePtr fetch_api_response = internal_response->PopulateFetchAPIResponse(KURL("https://www.example.org"));
    auto& csp = fetch_api_response->parsed_headers->content_security_policy;

    EXPECT_EQ(csp.size(), 2U);
    EXPECT_EQ(csp[0]->header->type, network::mojom::ContentSecurityPolicyType::kEnforce);
    EXPECT_EQ(csp[1]->header->type, network::mojom::ContentSecurityPolicyType::kReport);
}
