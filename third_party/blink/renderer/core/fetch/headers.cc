...
bool Headers::HeadersIterationSource::FetchNextItem(ScriptState* script_state, String& key, String& value, ExceptionState& exception)
{
...
Headers* Headers::Create(ScriptState* script_state, const V8HeadersInit* init, ExceptionState& exception_state)
{
...
void Headers::append(ScriptState* script_state, const String& name, const String& value, ExceptionState& exception_state)
{
    ...
    if (guard_ == kRequestGuard && EqualIgnoringASCIICase(name, "set-cookie")) {
        ...
        UseCounter::Count(execution_context, WebFeature::kFetchSetCookieInRequestGuardedHeaders);
    }
    ...
    //     header name, return."
    if (guard_ == kResponseGuard && FetchUtils::IsForbiddenResponseHeaderName(name)) {
        return;
    }
...
void Headers::remove(ScriptState* script_state, const String& name, ExceptionState& exception_state)
{
    ...
    if (guard_ == kRequestNoCorsGuard && !cors::IsNoCorsSafelistedHeaderName(name) && !cors::IsPrivilegedNoCorsHeaderName(name)) {
        return;
    }
...
void Headers::set(ScriptState* script_state, const String& name, const String& value, ExceptionState& exception_state)
{
    ...
    if (guard_ == kRequestNoCorsGuard && !cors::IsNoCorsSafelistedHeader(name, normalized_value)) {
        return;
    }
    // "6. Otherwise, if guard is |response| and |name| is a forbidden response
    //     header name, return."
...
void Headers::FillWith(ScriptState* script_state, const Headers* object, ExceptionState& exception_state)
{
...
void Headers::FillWith(ScriptState* script_state, const V8HeadersInit* init, ExceptionState& exception_state)
{
    DCHECK_EQ(header_list_->size(), 0U);
    ...
    case V8HeadersInit::ContentType::kByteStringByteStringRecord:
        return FillWith(script_state, init->GetAsByteStringByteStringRecord(), exception_state);
    case V8HeadersInit::ContentType::kByteStringSequenceSequence:
        return FillWith(script_state, init->GetAsByteStringSequenceSequence(), exception_state);
    }

    NOTREACHED();
}

void Headers::FillWith(ScriptState* script_state, const Vector<Vector<String>>& object, ExceptionState& exception_state)
{
...
void Headers::FillWith(ScriptState* script_state, const Vector<std::pair<String, String>>& object, ExceptionState& exception_state)
{
    DCHECK(!header_list_->size());
...
{
    const Vector<String> privileged_no_cors_header_names = cors::PrivilegedNoCorsHeaderNames();
...
PairSyncIterable<Headers>::IterationSource* Headers::CreateIterationSource(ScriptState*, ExceptionState&)
{
