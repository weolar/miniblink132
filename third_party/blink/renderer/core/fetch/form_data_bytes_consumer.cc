...
public:
    ...
    scoped_refptr<BlobDataHandle> DrainAsBlobDataHandle(BlobSizePolicy policy) override
    {
        ...
        blob_data->AppendBytes(base::span<const uint8_t>((const uint8_t*)data.data(), data.size()));
...
public:
    DataAndDataPipeBytesConsumer(ExecutionContext* execution_context, EncodedFormData* form_data)
    ...
    Result BeginRead(base::span<const char>& buffer) override
    {
        ...
        if (iter_->type_ == FormDataElement::kData) {
            ...
            if (!simple_consumer_) {
                scoped_refptr<EncodedFormData> simple_data = EncodedFormData::Create(iter_->data_.data(), iter_->data_.size());
                simple_consumer_ = MakeGarbageCollected<DataOnlyBytesConsumer>(std::move(simple_data));
                if (client_)
        ...
        if (iter_->type_ == FormDataElement::kDataPipe) {
            ...
            if (!data_pipe_consumer_) {
                network::mojom::blink::DataPipeGetter* data_pipe_getter = iter_->data_pipe_getter_->GetDataPipeGetter();

                mojo::ScopedDataPipeProducerHandle pipe_producer_handle;
                mojo::ScopedDataPipeConsumerHandle pipe_consumer_handle;
                MojoResult rv = mojo::CreateDataPipe(nullptr, pipe_producer_handle, pipe_consumer_handle);
                if (rv != MOJO_RESULT_OK) {
                    return Result::kError;
                }

                data_pipe_getter->Read(
                    std::move(pipe_producer_handle), WTF::BindOnce(&DataAndDataPipeBytesConsumer::DataPipeGetterCallback, WrapWeakPersistent(this)));
                DataPipeBytesConsumer::CompletionNotifier* completion_notifier = nullptr;
                data_pipe_consumer_ = MakeGarbageCollected<DataPipeBytesConsumer>(
                    execution_context_->GetTaskRunner(TaskType::kNetworking), std::move(pipe_consumer_handle), &completion_notifier);
    ...
    String DebugName() const override
    {
        return "DataAndDataPipeBytesConsumer";
    }
...
public:
    DataAndEncodedFileOrBlobBytesConsumer(ExecutionContext* execution_context, scoped_refptr<EncodedFormData> form_data, BytesConsumer* consumer_for_testing)
        : form_data_(std::move(form_data))
    {
        // TODO(crbug.com/374124998): we should have this type check.
        ...
        for (const auto& element : form_data_->Elements()) {
            ...
            case FormDataElement::kData:
                blob_data->AppendBytes(base::span<const uint8_t>((const uint8_t*)element.data_.data(), element.data_.size()));
                break;
            case FormDataElement::kEncodedFile: {
                auto file_length = element.file_length_;
                if (file_length < 0) {
                    if (!GetFileSize(element.filename_, *execution_context, file_length)) {
                        form_data_ = nullptr;
                        blob_bytes_consumer_ = BytesConsumer::CreateErrored(Error("Cannot determine a file size"));
                        return;
                    }
                }
                blob_data->AppendBlob(BlobDataHandle::CreateForFile(FileBackedBlobFactoryDispatcher::GetFileBackedBlobFactory(execution_context),
                                          element.filename_, element.file_start_, file_length, element.expected_file_modification_time_,
                                          /*content_type=*/""),
                    0, file_length);
                break;
            }
            case FormDataElement::kEncodedBlob:
                if (element.blob_data_handle_) {
                    blob_data->AppendBlob(element.blob_data_handle_, 0, element.blob_data_handle_->size());
                }
                break;
        ...
        blob_data->SetContentType(AtomicString("multipart/form-data; boundary=") + form_data_->Boundary().data());
        auto size = blob_data->length();
        blob_bytes_consumer_ = MakeGarbageCollected<BlobBytesConsumer>(execution_context, BlobDataHandle::Create(std::move(blob_data), size));
    }
    ...
    scoped_refptr<BlobDataHandle> DrainAsBlobDataHandle(BlobSizePolicy policy) override
    {
        ...
        scoped_refptr<BlobDataHandle> handle = blob_bytes_consumer_->DrainAsBlobDataHandle(policy);
        if (handle)
            form_data_ = nullptr;
        return handle;
    }
    ...
    void ClearClient() override
    {
        blob_bytes_consumer_->ClearClient();
    }
    void Cancel() override
    {
        form_data_ = nullptr;
    ...
    Error GetError() const override
    {
        return blob_bytes_consumer_->GetError();
    }
    String DebugName() const override
    {
...
EncodedFormData::FormDataType GetDeprecatedType(const EncodedFormData* form_data)
{
...
FormDataBytesConsumer::FormDataBytesConsumer(const String& string)
    : impl_(MakeGarbageCollected<DataOnlyBytesConsumer>(EncodedFormData::Create(UTF8Encoding().Encode(string, WTF::kNoUnencodables))))
{
}

FormDataBytesConsumer::FormDataBytesConsumer(DOMArrayBuffer* buffer)
    : FormDataBytesConsumer(buffer->Data(), base::checked_cast<wtf_size_t>(buffer->ByteLength()))
{
}

FormDataBytesConsumer::FormDataBytesConsumer(DOMArrayBufferView* view)
    : FormDataBytesConsumer(view->BaseAddress(), base::checked_cast<wtf_size_t>(view->byteLength()))
{
}

FormDataBytesConsumer::FormDataBytesConsumer(SegmentedBuffer&& buffer)
    : impl_(MakeGarbageCollected<DataOnlyBytesConsumer>(EncodedFormData::Create(std::move(buffer))))
{
}

FormDataBytesConsumer::FormDataBytesConsumer(const void* data, wtf_size_t size)
    : impl_(MakeGarbageCollected<DataOnlyBytesConsumer>(EncodedFormData::Create(data, size)))
{
}

FormDataBytesConsumer::FormDataBytesConsumer(ExecutionContext* execution_context, scoped_refptr<EncodedFormData> form_data)
    : FormDataBytesConsumer(execution_context, std::move(form_data), nullptr)
{
}

FormDataBytesConsumer::FormDataBytesConsumer(ExecutionContext* execution_context, scoped_refptr<EncodedFormData> form_data, BytesConsumer* consumer_for_testing)
    : impl_(GetImpl(execution_context, std::move(form_data), consumer_for_testing))
{
}

// static
BytesConsumer* FormDataBytesConsumer::GetImpl(
    ExecutionContext* execution_context, scoped_refptr<EncodedFormData> form_data, BytesConsumer* consumer_for_testing)
{
    DCHECK(form_data);
    ...
    if (form_data_type == EncodedFormData::FormDataType::kInvalid) {
        ...
        DUMP_WILL_BE_CHECK_NE(EncodedFormData::FormDataType::kInvalid, form_data_type);
    }
    ...
    case EncodedFormData::FormDataType::kDataAndEncodedFileOrBlob:
        return MakeGarbageCollected<DataAndEncodedFileOrBlobBytesConsumer>(execution_context, std::move(form_data), consumer_for_testing);
    case EncodedFormData::FormDataType::kDataAndDataPipe:
        return MakeGarbageCollected<DataAndDataPipeBytesConsumer>(execution_context, form_data.get());
