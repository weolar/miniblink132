...
BytesConsumer::Result PlaceHolderBytesConsumer::BeginRead(base::span<const char>& buffer)
{
    if (!underlying_) {
...
scoped_refptr<BlobDataHandle> PlaceHolderBytesConsumer::DrainAsBlobDataHandle(BlobSizePolicy policy)
{
...
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    return underlying_ ? underlying_->GetPublicState() : is_cancelled_ ? PublicState::kClosed : PublicState::kReadableOrWaiting;
}
