...
class CORE_EXPORT FetchRequestData final : public GarbageCollected<FetchRequestData> {
public:
    enum class ForServiceWorkerFetchEvent { kFalse, kTrue };

    static FetchRequestData* Create(ScriptState*, mojom::blink::FetchAPIRequestPtr, ForServiceWorkerFetchEvent);
    ...
    void SetMethod(AtomicString method)
    {
        method_ = method;
    }
    const AtomicString& Method() const
    {
        return method_;
    }
    ...
    const AtomicString& ReferrerString() const
    {
        return referrer_string_;
    }
    void SetReferrerString(const AtomicString& s)
    {
        referrer_string_ = s;
    }
    ...
    mojom::blink::FetchCacheMode CacheMode() const
    {
        return cache_mode_;
    }
    ...
    network::mojom::RedirectMode Redirect() const
    {
        return redirect_;
    }
    ...
    FetchHeaderList* HeaderList() const
    {
        return header_list_.Get();
    }
    ...
    uint64_t BufferByteLength() const
    {
        return buffer_byte_length_;
    }
    ...
    void SetIntegrity(const String& integrity)
    {
    ...
    void SetPriority(ResourceLoadPriority priority)
    {
        priority_ = priority;
    }
    ...
    void SetAdAuctionHeaders(bool b)
    {
        ad_auction_headers_ = b;
    }

    bool SharedStorageWritable() const
    {
        return shared_storage_writable_;
    }
    ...
    void SetIsHistoryNavigation(bool b)
    {
        is_history_navigation_ = b;
    }
    ...
    void SetURLLoaderFactory(mojo::PendingRemote<network::mojom::blink::URLLoaderFactory> factory)
    {
        url_loader_factory_.Bind(std::move(factory), execution_context_->GetTaskRunner(TaskType::kNetworking));
    }
    const base::UnguessableToken& WindowId() const
    {
        return window_id_;
    }
    void SetWindowId(const base::UnguessableToken& id)
    {
        window_id_ = id;
    }
    ...
    void SetTrustTokenParams(std::optional<network::mojom::blink::TrustTokenParams> trust_token_params)
    {
        trust_token_params_ = std::move(trust_token_params);
    }

    network::mojom::AttributionReportingEligibility AttributionReportingEligibility() const
    {
    ...
    void SetAttributionReportingSupport(network::mojom::AttributionSupport support)
    {
...
private:
    ...
    KURL url_;
    Member<FetchHeaderList> header_list_ = MakeGarbageCollected<FetchHeaderList>();
    // FIXME: Support m_skipServiceWorkerFlag;
    network::mojom::RequestDestination destination_ = network::mojom::RequestDestination::kEmpty;
    ...
    network::mojom::IPAddressSpace target_address_space_ = network::mojom::IPAddressSpace::kUnknown;
    network::mojom::CredentialsMode credentials_ = network::mojom::CredentialsMode::kOmit;
    ...
    // worker.
    mojom::blink::FetchCacheMode cache_mode_ = mojom::blink::FetchCacheMode::kDefault;
    network::mojom::RedirectMode redirect_ = network::mojom::RedirectMode::kFollow;
