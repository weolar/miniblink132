...
protected:
    ...
    ScriptValue Eval(ScriptState* script_state, const char* s)
    {
        ...
        v8::MicrotasksScope microtasks(script_state->GetIsolate(), ToMicrotaskQueue(script_state), v8::MicrotasksScope::kDoNotRunMicrotasks);
        if (!v8::String::NewFromUtf8(script_state->GetIsolate(), s, v8::NewStringType::kNormal).ToLocal(&source)) {
            ADD_FAILURE();
            return ScriptValue();
        }
        if (!v8::Script::Compile(script_state->GetContext(), source).ToLocal(&script)) {
            ADD_FAILURE() << "Compilation fails";
            return ScriptValue();
        }
        return ScriptValue(script_state->GetIsolate(), script->Run(script_state->GetContext()).ToLocalChecked());
    }
    ScriptValue EvalWithPrintingError(ScriptState* script_state, const char* s)
    {
        ...
        if (block.HasCaught()) {
            ADD_FAILURE() << ToCoreString(script_state->GetIsolate(), block.Exception()->ToString(script_state->GetContext()).ToLocalChecked()).Utf8();
            block.ReThrow();
        }
        return r;
    }
...
TEST_F(BodyStreamBufferTest, Tee)
{
    ...
    ReplayingBytesConsumer* src = MakeGarbageCollected<ReplayingBytesConsumer>(scope.GetDocument().GetTaskRunner(TaskType::kNetworking));
    src->Add(Command(Command::kData, "hello, "));
    src->Add(Command(Command::kData, "world"));
    src->Add(Command(Command::kDone));
    BodyStreamBuffer* buffer = BodyStreamBuffer::Create(scope.GetScriptState(), src,
    ...
    checkpoint.Call(0);
    new1->StartLoading(FetchDataLoader::CreateLoaderAsString(TextResourceDecoderOptions::CreateUTF8Decode()), client1, exception_state);
    checkpoint.Call(1);
    test::RunPendingTasks();
    checkpoint.Call(2);

    new2->StartLoading(FetchDataLoader::CreateLoaderAsString(TextResourceDecoderOptions::CreateUTF8Decode()), client2, exception_state);
    checkpoint.Call(3);
    test::RunPendingTasks();
    checkpoint.Call(4);
}
...
TEST_F(BodyStreamBufferTest, TeeFromHandleMadeFromStream)
{
    ...
    auto* underlying_source = MakeGarbageCollected<TestUnderlyingSource>(scope.GetScriptState());
    ...
    auto* stream = ReadableStream::CreateWithCountQueueingStrategy(scope.GetScriptState(), underlying_source, 0);
    ASSERT_TRUE(stream);

    underlying_source->Enqueue(ScriptValue(scope.GetIsolate(), ToV8Traits<DOMUint8Array>::ToV8(scope.GetScriptState(), chunk1)));
    underlying_source->Enqueue(ScriptValue(scope.GetIsolate(), ToV8Traits<DOMUint8Array>::ToV8(scope.GetScriptState(), chunk2)));
    ...
    BodyStreamBuffer* buffer = MakeGarbageCollected<BodyStreamBuffer>(scope.GetScriptState(), stream, /*cached_metadta_handler=*/nullptr);
...
TEST_F(BodyStreamBufferTest, DrainAsBlobDataHandle)
{
    ...
    BodyStreamBuffer* buffer
        = BodyStreamBuffer::Create(scope.GetScriptState(), MakeGarbageCollected<BlobBytesConsumer>(scope.GetExecutionContext(), blob_data_handle),
            /*abort_signal=*/nullptr, /*cached_metadata_handler=*/nullptr, side_data_blob);

    EXPECT_FALSE(buffer->IsStreamLocked());
    EXPECT_FALSE(buffer->IsStreamDisturbed());
    EXPECT_EQ(side_data_blob, buffer->GetSideDataBlobForTest());
    scoped_refptr<BlobDataHandle> output_blob_data_handle
        = buffer->DrainAsBlobDataHandle(BytesConsumer::BlobSizePolicy::kAllowBlobWithInvalidSize, ASSERT_NO_EXCEPTION);
...
TEST_F(BodyStreamBufferTest, DrainAsBlobDataHandleReturnsNull)
{
    ...
    BytesConsumer* src = MakeGarbageCollected<ReplayingBytesConsumer>(scope.GetDocument().GetTaskRunner(TaskType::kNetworking));
    scoped_refptr<BlobDataHandle> side_data_blob = CreateBlob("side data");
    BodyStreamBuffer* buffer = BodyStreamBuffer::Create(scope.GetScriptState(), src,
        /*abort_signal=*/nullptr, /*cached_metadata_handler=*/nullptr, side_data_blob);
    ...
    EXPECT_FALSE(buffer->DrainAsBlobDataHandle(BytesConsumer::BlobSizePolicy::kAllowBlobWithInvalidSize, ASSERT_NO_EXCEPTION));
...
TEST_F(BodyStreamBufferTest, DrainAsBlobFromBufferMadeFromBufferMadeFromStream)
{
    V8TestingScope scope;
    NonThrowableExceptionState exception_state;
    auto* stream = ReadableStream::Create(scope.GetScriptState(), exception_state);
    ASSERT_TRUE(stream);
    BodyStreamBuffer* buffer = MakeGarbageCollected<BodyStreamBuffer>(scope.GetScriptState(), stream, /*cached_metadata_handler=*/nullptr);
...
TEST_F(BodyStreamBufferTest, DrainAsFormData)
{
    ...
    scoped_refptr<EncodedFormData> input_form_data = data->EncodeMultiPartFormData();
    scoped_refptr<BlobDataHandle> side_data_blob = CreateBlob("side data");

    BodyStreamBuffer* buffer
        = BodyStreamBuffer::Create(scope.GetScriptState(), MakeGarbageCollected<FormDataBytesConsumer>(scope.GetExecutionContext(), input_form_data),
            /*abort_signal=*/nullptr, /*cached_metadata_handler=*/nullptr, side_data_blob);

    EXPECT_FALSE(buffer->IsStreamLocked());
    EXPECT_FALSE(buffer->IsStreamDisturbed());
    EXPECT_EQ(side_data_blob, buffer->GetSideDataBlobForTest());
    scoped_refptr<EncodedFormData> output_form_data = buffer->DrainAsFormData(ASSERT_NO_EXCEPTION);

    EXPECT_TRUE(buffer->IsStreamLocked());
    EXPECT_TRUE(buffer->IsStreamDisturbed());
    EXPECT_EQ(nullptr, buffer->GetSideDataBlobForTest());
    EXPECT_EQ(output_form_data->FlattenToString(), input_form_data->FlattenToString());
}
...
TEST_F(BodyStreamBufferTest, LoadBodyStreamBufferAsArrayBuffer)
{
    ...
    InSequence s;
    EXPECT_CALL(checkpoint, Call(1));
    EXPECT_CALL(*client, DidFetchDataLoadedArrayBufferMock(_)).WillOnce(SaveArg<0>(&array_buffer));
    EXPECT_CALL(checkpoint, Call(2));
    ...
    BodyStreamBuffer* buffer = BodyStreamBuffer::Create(scope.GetScriptState(), src,
        /*abort_signal=*/nullptr, /*cached_metadata_handler=*/nullptr, side_data_blob);
    EXPECT_EQ(side_data_blob, buffer->GetSideDataBlobForTest());
    buffer->StartLoading(FetchDataLoader::CreateLoaderAsArrayBuffer(), client, ASSERT_NO_EXCEPTION);
...
public:
    BodyStreamBufferBlobTest()
        ...
        , blob_registry_receiver_(&fake_blob_registry_, blob_registry_remote_.BindNewPipeAndPassReceiver())
    {
...
TEST_F(BodyStreamBufferBlobTest, LoadBodyStreamBufferAsBlob)
{
    ...
    InSequence s;
    EXPECT_CALL(checkpoint, Call(1));
    EXPECT_CALL(*client, DidFetchDataLoadedBlobHandleMock(_)).WillOnce(SaveArg<0>(&blob_data_handle));
    EXPECT_CALL(checkpoint, Call(2));
    ...
    BodyStreamBuffer* buffer = BodyStreamBuffer::Create(scope.GetScriptState(), src,
        /*abort_signal=*/nullptr, /*cached_metadata_handler=*/nullptr, side_data_blob);
    EXPECT_EQ(side_data_blob, buffer->GetSideDataBlobForTest());
    buffer->StartLoading(FetchDataLoader::CreateLoaderAsBlobHandle("text/plain", fake_task_runner_), client, ASSERT_NO_EXCEPTION);
...
TEST_F(BodyStreamBufferTest, LoadBodyStreamBufferAsString)
{
    ...
    BodyStreamBuffer* buffer = BodyStreamBuffer::Create(scope.GetScriptState(), src,
        /*abort_signal=*/nullptr, /*cached_metadata_handler=*/nullptr, side_data_blob);
    EXPECT_EQ(side_data_blob, buffer->GetSideDataBlobForTest());
    buffer->StartLoading(FetchDataLoader::CreateLoaderAsString(TextResourceDecoderOptions::CreateUTF8Decode()), client, ASSERT_NO_EXCEPTION);
...
TEST_F(BodyStreamBufferTest, LoadClosedHandle)
{
    ...
    scoped_refptr<BlobDataHandle> side_data_blob = CreateBlob("side data");
    BodyStreamBuffer* buffer = BodyStreamBuffer::Create(scope.GetScriptState(), BytesConsumer::CreateClosed(),
        /*abort_signal=*/nullptr, /*cached_metadata_handler=*/nullptr, side_data_blob);
...
TEST_F(BodyStreamBufferTest, LoadErroredHandle)
{
    ...
    scoped_refptr<BlobDataHandle> side_data_blob = CreateBlob("side data");
    BodyStreamBuffer* buffer = BodyStreamBuffer::Create(scope.GetScriptState(), BytesConsumer::CreateErrored(BytesConsumer::Error()),
        /*abort_signal=*/nullptr, /*cached_metadata_handler=*/nullptr, side_data_blob);
...
TEST_F(BodyStreamBufferTest, LoaderShouldBeKeptAliveByBodyStreamBuffer)
{
    ...
    src->Add(Command(Command::kData, "hello"));
    src->Add(Command(Command::kDone));
    Persistent<BodyStreamBuffer> buffer = BodyStreamBuffer::Create(scope.GetScriptState(), src, nullptr,
...
{
    V8TestingScope scope;
    ReplayingBytesConsumer* consumer = MakeGarbageCollected<ReplayingBytesConsumer>(scope.GetDocument().GetTaskRunner(TaskType::kNetworking));

    BodyStreamBuffer* buffer = BodyStreamBuffer::Create(scope.GetScriptState(), consumer, nullptr,
        /*cached_metadata_handler=*/nullptr);
    ScriptValue reason(scope.GetIsolate(), V8String(scope.GetIsolate(), "reason"));
...
TEST_F(BodyStreamBufferTest, NestedPull)
{
    ...
    auto result = scope.GetScriptState()->GetContext()->Global()->CreateDataProperty(scope.GetScriptState()->GetContext(),
        V8String(scope.GetIsolate(), "stream"), ToV8Traits<ReadableStream>::ToV8(scope.GetScriptState(), buffer->Stream()));

    ASSERT_TRUE(result.IsJust());
    ASSERT_TRUE(result.FromJust());

    ScriptValue stream = EvalWithPrintingError(scope.GetScriptState(), "reader = stream.getReader();");
...
TEST_F(BodyStreamBufferTest, NullAbortSignalIsNotAborted)
{
    ...
    // This BytesConsumer is not drainable.
    BytesConsumer* src = MakeGarbageCollected<ReplayingBytesConsumer>(scope.GetDocument().GetTaskRunner(TaskType::kNetworking));
    BodyStreamBuffer* buffer = BodyStreamBuffer::Create(scope.GetScriptState(), src, nullptr,
        /*cached_metadata_handler=*/nullptr);
...
TEST_F(BodyStreamBufferTest, AbortSignalMakesAborted)
{
    ...
    BodyStreamBuffer* buffer = BodyStreamBuffer::Create(scope.GetScriptState(), src, controller->signal(),
        /*cached_metadata_handler=*/nullptr);
...
TEST_F(BodyStreamBufferTest, AbortBeforeStartLoadingCallsDataLoaderClientAbort)
{
    V8TestingScope scope;
    Checkpoint checkpoint;
    ...
    EXPECT_CALL(*src, GetPublicState()).WillOnce(Return(BytesConsumer::PublicState::kReadableOrWaiting));
...
TEST_F(BodyStreamBufferTest, AsyncAbortAfterStartLoadingCallsDataLoaderClientAbort)
{
    V8TestingScope scope;
    Checkpoint checkpoint;
    MockFetchDataLoader* loader = MockFetchDataLoader::Create();
...
TEST_F(BodyStreamBufferTest, CachedMetadataHandler)
{
    ...
    {
        BytesConsumer* src = MakeGarbageCollected<ReplayingBytesConsumer>(scope.GetDocument().GetTaskRunner(TaskType::kNetworking));
        auto* handler = MakeGarbageCollected<ScriptCachedMetadataHandler>(WTF::TextEncoding(), nullptr);
...
TEST_F(BodyStreamBufferTest, CachedMetadataHandlerAndTeeForBufferMadeFromStream)
{
    V8TestingScope scope;
    auto* handler = MakeGarbageCollected<ScriptCachedMetadataHandler>(WTF::TextEncoding(), nullptr);
    auto* stream = ReadableStream::Create(scope.GetScriptState(), ASSERT_NO_EXCEPTION);
    auto* buffer = MakeGarbageCollected<BodyStreamBuffer>(scope.GetScriptState(), stream, handler);
...
TEST_F(BodyStreamBufferTest, KeptAliveWhileLoading)
{
    ...
    WeakPersistent<ReplayingBytesConsumer> src;
    {
        ...
        src = MakeGarbageCollected<ReplayingBytesConsumer>(scope.GetDocument().GetTaskRunner(TaskType::kNetworking));
        src->Add(Command(Command::kWait));
        src->Add(Command(Command::kData, "hello"));
