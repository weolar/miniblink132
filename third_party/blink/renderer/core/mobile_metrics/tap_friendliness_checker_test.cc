...
protected:
    ...
    void TapAt(int x, int y)
    {
        ...
        helper_->LocalMainFrame()->GetFrame()->GetEventHandler().HandleGestureEvent(tap_event);
    }
    ...
    void LoadHTML(const std::string& html, float device_scale = 1.0)
    {
        ...
        helper_->GetWebView()->MainFrameWidget()->SetDeviceScaleFactorForTesting(device_scale);
        helper_->Resize(gfx::Size(kDeviceWidth, kDeviceHeight));
        helper_->GetWebView()->GetPage()->SetDefaultPageScaleLimits(kMinimumZoom, kMaximumZoom);
        helper_->GetWebView()->GetPage()->GetSettings().SetTextAutosizingEnabled(true);
        helper_->GetWebView()->GetPage()->GetSettings().SetShrinksViewportContentToFit(true);
        helper_->GetWebView()->GetPage()->GetSettings().SetViewportStyle(mojom::blink::ViewportStyle::kMobile);
        helper_->LoadAhem();
        frame_test_helpers::LoadHTMLString(helper_->GetWebView()->MainFrameImpl(), html, url_test_helpers::ToKURL(kBaseUrl));
...
<html>
  ...
  <body style="font-size: 18px; margin: 0px">
    ...
    auto entries = recorder->GetEntriesByName("MobileFriendliness.TappedBadTargets");
...
<html>
  ...
  <body style="font-size: 18px; margin: 0px">
    ...
    ASSERT_EQ(entries[0]->event_hash, ukm::builders::MobileFriendliness_TappedBadTargets::kEntryNameHash);
...
<html>
  ...
  <body style="font-size: 18px; margin: 0px">
    ...
    ASSERT_EQ(entries[1]->event_hash, ukm::builders::MobileFriendliness_TappedBadTargets::kEntryNameHash);
    ASSERT_TRUE(entries[2]->metrics.empty());
    ASSERT_EQ(entries[2]->event_hash, ukm::builders::MobileFriendliness_TappedBadTargets::kEntryNameHash);
}
...
<html>
  ...
  <body style="font-size: 18px; margin: 0px">
    ...
    auto it = entries[0]->metrics.find(ukm::builders::MobileFriendliness_TappedBadTargets::kTooSmallNameHash);
...
<html>
  ...
  <body style="font-size: 18px; margin: 0px">
    ...
    ASSERT_EQ(entries[0]->event_hash, ukm::builders::MobileFriendliness_TappedBadTargets::kEntryNameHash);
    ASSERT_EQ(entries[0]->metrics.size(), 1u);
    auto it = entries[0]->metrics.find(ukm::builders::MobileFriendliness_TappedBadTargets::kCloseDisplayEdgeNameHash);
    ASSERT_EQ(it->second, 1);
}
...
<html>
  ...
  <body style="font-size: 18px; margin: 0px">
    ...
    auto close_it = entries[0]->metrics.find(ukm::builders::MobileFriendliness_TappedBadTargets::kCloseDisplayEdgeNameHash);
    ASSERT_EQ(close_it->second, 1);
    auto small_it = entries[0]->metrics.find(ukm::builders::MobileFriendliness_TappedBadTargets::kTooSmallNameHash);
