...
bool ShouldRegister(Element* target)
{
    ...
    } else if (auto* element = DynamicTo<HTMLElement>(target); element && element->WillRespondToMouseClickEvents()) {
        return true;
    } else {
...
TapFriendlinessChecker* TapFriendlinessChecker::CreateIfMobile(LocalFrameView& view)
{
    if (!view.GetPage()->GetSettings().GetViewportEnabled() || !view.GetPage()->GetSettings().GetViewportMetaEnabled()) {
        return nullptr;
    }
...
void TapFriendlinessChecker::RegisterTapEvent(Element* target)
{
    ...
    const float zoom_factor = view_->GetPage()->GetVisualViewport().Scale() / view_->GetPage()->GetPageScaleConstraintsSet().FinalConstraints().initial_scale;
    const gfx::Rect dip_rect = view_->FrameToScreen(gfx::ToEnclosingRect(node->GetBoundingClientRectNoLifecycleUpdate()));

    ukm::builders::MobileFriendliness_TappedBadTargets builder(view_->GetFrame().GetDocument()->UkmSourceID());

    if (dip_rect.width() * kOneDipInMm / zoom_factor <= kTooSmallThresholdInMm && dip_rect.height() * kOneDipInMm / zoom_factor <= kTooSmallThresholdInMm) {
        builder.SetTooSmall(true);
    }
    const float display_width = ViewportWidthInDIPS(*view_) * kOneDipInMm / zoom_factor;
    const double center_x = (dip_rect.x() + dip_rect.width()) * kOneDipInMm / zoom_factor / 2.0;
    const double center_y = (dip_rect.y() + dip_rect.height()) * kOneDipInMm / zoom_factor / 2.0;
    if (center_x <= kTooCloseDisplayEdgeThresholdInMm || display_width - kTooCloseDisplayEdgeThresholdInMm <= center_x
