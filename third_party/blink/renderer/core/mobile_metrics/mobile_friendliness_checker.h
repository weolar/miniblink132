...
// of UKM.
class CORE_EXPORT MobileFriendlinessChecker : public GarbageCollected<MobileFriendlinessChecker> {
public:
    ...
    void NotifyPaintEnd();
    void NotifyPaintTextFragment(const PhysicalRect& paint_rect, int font_size, const TransformPaintPropertyNodeOrAlias& current_transform);
    void NotifyPaintReplaced(const PhysicalRect& paint_rect, const TransformPaintPropertyNodeOrAlias& current_transform);
    ...
    public:
        explicit PaintScope(MobileFriendlinessChecker& mfc)
            : mfc_(mfc)
        {
...
private:
    ...
    void UpdateBeyondViewportAreaSizes(const PhysicalRect& paint_rect, const TransformPaintPropertyNodeOrAlias& current_transform);
