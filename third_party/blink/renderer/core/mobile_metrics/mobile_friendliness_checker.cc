...
int32_t BucketWithOffsetAndUnit(int32_t num, int32_t offset, int32_t unit)
{
    ...
    const int32_t bucketed = grid == 0 ? 0
...
static constexpr base::TimeDelta kFirstEvaluationOffsetTime = kEvaluationInterval - kEvaluationDelay;

MobileFriendlinessChecker::MobileFriendlinessChecker(LocalFrameView& frame_view)
    : frame_view_(&frame_view)
    , viewport_scalar_(
          frame_view_->GetFrame().GetWidgetForLocalRoot() ? frame_view_->GetPage()->GetChromeClient().WindowToViewportScalar(&frame_view_->GetFrame(), 1) : 1.0)
...
void MobileFriendlinessChecker::NotifyPaintBegin()
{
    ...
    ignore_beyond_viewport_scope_count_ = frame_view_->LayoutViewport()->MaximumScrollOffset().x() == 0
        && frame_view_->GetPage()->GetVisualViewport().MaximumScrollOffsetAtScale(initial_scale_).x() == 0;
    is_painting_ = true;
    viewport_transform_ = &frame_view_->GetLayoutView()->FirstFragment().ContentsProperties().Transform();
    previous_transform_ = viewport_transform_;
    current_x_offset_ = 0.0;

    const ViewportDescription& viewport = frame_view_->GetFrame().GetDocument()->GetViewportData().GetViewportDescription();
    if (viewport.type == ViewportDescription::Type::kViewportMeta) {
        ...
        if (viewport.max_width.IsFixed()) {
            ...
            viewport_hardcoded_width_ = viewport.max_width.GetFloatValue() / viewport_scalar_;
        }
    ...
    initial_scale_ = frame_view_->GetPage()->GetPageScaleConstraintsSet().FinalConstraints().initial_scale;
...
MobileFriendlinessChecker* MobileFriendlinessChecker::Create(LocalFrameView& frame_view)
{
    ...
    // devtools, svg, etc.
    if (!frame_view.GetFrame().Client()->IsLocalFrameClientImpl() || !frame_view.GetFrame().IsOutermostMainFrame()
        || !frame_view.GetPage()->GetSettings().GetViewportEnabled() || !frame_view.GetPage()->GetSettings().GetViewportMetaEnabled()) {
        return nullptr;
    }
    return MakeGarbageCollected<MobileFriendlinessChecker>(frame_view);
}

MobileFriendlinessChecker* MobileFriendlinessChecker::From(const Document& document)
{
    ...
    MobileFriendlinessChecker* mfc = local_frame->View()->GetMobileFriendlinessChecker();
...
{
    ukm::builders::MobileFriendliness builder(frame_view_->GetFrame().GetDocument()->UkmSourceID());
    ...
    if (viewport_initial_scale_x10_) {
        builder.SetViewportInitialScaleX10(GetBucketedViewportInitialScale(*viewport_initial_scale_x10_));
    }
    if (viewport_hardcoded_width_) {
        builder.SetViewportHardcodedWidth(GetBucketedViewportHardcodedWidth(*viewport_hardcoded_width_));
    }
    builder.SetTextContentOutsideViewportPercentage(area_sizes_.TextContentsOutsideViewportPercentage(
        // Use SizeF when computing the area to avoid integer overflow.
        gfx::SizeF(frame_view_->GetPage()->GetVisualViewport().Size()).GetArea()));
...
int MobileFriendlinessChecker::AreaSizes::TextContentsOutsideViewportPercentage(double viewport_area) const
{
    return std::ceil(content_beyond_viewport_area * 100 / viewport_area);
}

void MobileFriendlinessChecker::UpdateTextAreaSizes(const PhysicalRect& text_rect, int font_size)
{
...
void MobileFriendlinessChecker::UpdateBeyondViewportAreaSizes(const PhysicalRect& paint_rect, const TransformPaintPropertyNodeOrAlias& current_transform)
{
    DCHECK(is_painting_);
    ...
    if (previous_transform_ != &current_transform) {
        auto projection = GeometryMapper::SourceToDestinationProjection(current_transform, *viewport_transform_);
    ...
    float width_beyond_viewport = std::min(std::max(right - viewport_width_, 0.f), width);

    area_sizes_.content_beyond_viewport_area += width_beyond_viewport * paint_rect.Height();
}

void MobileFriendlinessChecker::NotifyPaintTextFragment(
    const PhysicalRect& paint_rect, int font_size, const TransformPaintPropertyNodeOrAlias& current_transform)
{
    DCHECK(frame_view_->GetFrame().Client()->IsLocalFrameClientImpl());
    DCHECK(frame_view_->GetFrame().IsOutermostMainFrame());
...
void MobileFriendlinessChecker::NotifyPaintReplaced(const PhysicalRect& paint_rect, const TransformPaintPropertyNodeOrAlias& current_transform)
{
    DCHECK(frame_view_->GetFrame().Client()->IsLocalFrameClientImpl());
