...
static constexpr char kFullscreenDurationMetricKeyRequestFullscreen[] = "Blink.Element.Fullscreen.DurationUpTo1H.RequestFullscreen";

void FullscreenElementChanged(
    Document& document, Element* old_element, Element* new_element, FullscreenRequestType new_request_type, const FullscreenOptions* new_options)
{
    ...
    if (old_element) {
        ...
        old_element->SetContainsFullScreenElementOnAncestorsCrossingFrameBoundaries(false);
    }
    ...
    if (new_element) {
        ...
        new_element->SetContainsFullScreenElementOnAncestorsCrossingFrameBoundaries(true);
    }

    // Update IsInert() flags.
    const StyleChangeReasonForTracing& reason = StyleChangeReasonForTracing::Create(style_change_reason::kFullscreen);
    ...
    if (LocalFrame* frame = document.GetFrame()) {
        ...
        frame->GetChromeClient().FullscreenElementChanged(old_element, new_element, new_options, new_request_type);
...
public:
    MetaParams() = default;
    MetaParams(FullscreenRequestType request_type, const FullscreenOptions* options, const base::TimeTicks& fullscreen_enter_time)
    ...
    FullscreenRequestType request_type() const
    {
        return request_type_;
    }
    const FullscreenOptions* options() const
    {
        return options_.Get();
    }
...
using ElementMetaParamsMap = HeapHashMap<WeakMember<const Element>, Member<const MetaParams>>;

ElementMetaParamsMap& FullscreenParamsMap()
{
    DEFINE_STATIC_LOCAL(Persistent<ElementMetaParamsMap>, map, (MakeGarbageCollected<ElementMetaParamsMap>()));
    return *map;
}
...
void SetFullscreenFlag(const Element& element, FullscreenRequestType request_type, const FullscreenOptions* options)
{
    FullscreenParamsMap().insert(&element, MakeGarbageCollected<MetaParams>(request_type, options, base::TimeTicks::Now()));
}
...
void GoFullscreen(Element& element, FullscreenRequestType request_type, const FullscreenOptions* options)
{
    ...
    auto* hide_until = HTMLElement::TopLayerElementPopoverAncestor(element, TopLayerElementType::kFullscreen);
    HTMLElement::HideAllPopoversUntil(hide_until, document, HidePopoverFocusBehavior::kNone, HidePopoverTransitionBehavior::kFireEventsAndWaitForTransitions);
    ...
    FullscreenElementChanged(document, old_element, &element, request_type, options);
}
...
void Unfullscreen(Element& element)
{
    ...
    document.ScheduleForTopLayerRemoval(&element, Document::TopLayerReason::kFullscreen);
    ...
    if (old_element != new_element) {
        ...
        FullscreenElementChanged(document, old_element, new_element, new_request_type, new_options);
    }
}
...
bool AllowedToUseFullscreen(const Document& document, ReportOptions report_on_failure)
{
    ...
    // feature.
    return document.GetExecutionContext()->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kFullscreen, report_on_failure);
}
...
bool FullscreenIsSupported(const Document& document)
{
    ...
    if (RuntimeEnabledFeatures::DocumentPictureInPictureAPIEnabled(document.GetExecutionContext()) && frame->LocalFrameRoot().DomWindow()
    ...
    return !document.GetSettings() || document.GetSettings()->GetFullscreenSupported();
}

// https://fullscreen.spec.whatwg.org/#fullscreen-element-ready-check
RequestFullscreenError FullscreenElementReadyCheck(const Element& element, ReportOptions report_on_failure)
{
    ...
    if (auto* html_element = DynamicTo<HTMLElement>(element); html_element && html_element->popoverOpen()) {
...
public:
    ...
    static bool RunningRequestFullscreen()
    {
        return running_request_fullscreen_;
    }
...
void FireEvent(const AtomicString& type, Element* element, Document* document)
{
    ...
    EventTarget* target
        = element->isConnected() && &element->GetDocument() == document ? static_cast<EventTarget*>(element) : static_cast<EventTarget*>(document);
...
const AtomicString& AdjustEventType(const AtomicString& type, FullscreenRequestType request_type)
{
    DCHECK(type == event_type_names::kFullscreenchange || type == event_type_names::kFullscreenerror);

    if (!(request_type & FullscreenRequestType::kPrefixed))
        return type;
    return type == event_type_names::kFullscreenchange ? event_type_names::kWebkitfullscreenchange : event_type_names::kWebkitfullscreenerror;
}

void EnqueueEvent(const AtomicString& type, Element& element, Document& document, FullscreenRequestType request_type)
{
    const AtomicString& adjusted_type = AdjustEventType(type, request_type);
    document.EnqueueAnimationFrameTask(WTF::BindOnce(FireEvent, adjusted_type, WrapWeakPersistent(&element), WrapWeakPersistent(&document)));
}
...
ScriptPromise<IDLUndefined> Fullscreen::RequestFullscreen(
    Element& pending, const FullscreenOptions* options, FullscreenRequestType request_type, ScriptState* script_state, ExceptionState* exception_state)
{
    ...
    if (script_state) {
        ...
        resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state);
    }

    bool for_cross_process_descendant = request_type & FullscreenRequestType::kForCrossProcessDescendant;
    ...
    if (for_cross_process_descendant) {
        ContinueRequestFullscreenAfterConditionsEnforcement(&pending, request_type, options, resolver, error);
    } else {
        EnforceRequestFullscreenConditions(pending, document,
            WTF::BindOnce(&Fullscreen::ContinueRequestFullscreenAfterConditionsEnforcement, WrapPersistent(&pending), request_type, WrapPersistent(options),
...
void Fullscreen::EnforceRequestFullscreenConditions(Element& pending, Document& document, base::OnceCallback<void(RequestFullscreenError)> callback)
{
    ...
    RequestFullscreenError ready_error = FullscreenElementReadyCheck(pending, ReportOptions::kReportOnFailure);
    ...
    if (ScopedAllowFullscreen::FullscreenAllowedReason() == ScopedAllowFullscreen::kXrOverlay
        || ScopedAllowFullscreen::FullscreenAllowedReason() == ScopedAllowFullscreen::kXrSession) {
    ...
    if (ScopedAllowFullscreen::FullscreenAllowedReason() == ScopedAllowFullscreen::kOrientationChange) {
        UseCounter::Count(document, WebFeature::kFullscreenAllowedByOrientationChange);
        std::move(callback).Run(RequestFullscreenError::kNone);
        return;
    }
    ...
    if (Fullscreen* fullscreen = Supplement<LocalDOMWindow>::From<Fullscreen>(*document.domWindow());
        fullscreen && base::TimeTicks::Now() <= fullscreen->block_automatic_fullscreen_until()) {
        std::move(callback).Run(RequestFullscreenError::kPermissionCheckFailed);
        return;
    }

    // Check permission and wait for the async result.
    auto* permission_service = document.GetPermissionService(document.GetExecutionContext());
    ...
    descriptor->extension = mojom::blink::PermissionDescriptorExtension::NewFullscreen(mojom::blink::FullscreenPermissionDescriptor::New(
        /*allow_without_user_gesture=*/true));
    permission_service->HasPermission(std::move(descriptor),
        WTF::BindOnce(
            [](base::OnceCallback<void(RequestFullscreenError)> callback, Document* document, mojom::blink::PermissionStatus status) {
...
void Fullscreen::ContinueRequestFullscreenAfterConditionsEnforcement(Element* pending, FullscreenRequestType request_type, const FullscreenOptions* options,
    ScriptPromiseResolver<IDLUndefined>* resolver, RequestFullscreenError error)
{
    CHECK(pending);
    Document& document = pending->GetDocument();
    if (error != RequestFullscreenError::kNone) {
        ...
        if (error == RequestFullscreenError::kPermissionCheckFailed) {
            String message = ExceptionMessages::FailedToExecute("requestFullscreen", "Element", "API can only be initiated by a user gesture.");
        ...
        ContinueRequestFullscreen(document, *pending, request_type, options, resolver, error);
        return;
    }
    ...
    From(window).pending_requests_.push_back(MakeGarbageCollected<PendingRequest>(pending, request_type, options, resolver));
...
void Fullscreen::DidResolveEnterFullscreenRequest(Document& document, bool granted)
{
    ...
    const RequestFullscreenError error = granted ? RequestFullscreenError::kNone : RequestFullscreenError::kNotGranted;
    for (const Member<PendingRequest>& request : requests) {
        ContinueRequestFullscreen(document, *request->element(), request->type(), request->options(), request->resolver(), error);
    }
}

void Fullscreen::ContinueRequestFullscreen(Document& document, Element& pending, FullscreenRequestType request_type, const FullscreenOptions* options,
    ScriptPromiseResolver<IDLUndefined>* resolver, RequestFullscreenError error)
{
    ...
    if (error != RequestFullscreenError::kNone) {
        ...
        EnqueueEvent(event_type_names::kFullscreenerror, pending, document, request_type);
        ...
        if (resolver && resolver->GetScriptState()->ContextIsValid()) {
            ...
            resolver->Reject(V8ThrowException::CreateTypeError(resolver->GetScriptState()->GetIsolate(), GetErrorString(error)));
        }
        return;
    }
    ...
    for (Frame* frame = pending.GetDocument().GetFrame(); frame; frame = frame->Tree().Parent()) {
    ...
    for (Element* element : fullscreen_elements) {
        ...
        // fullscreen events.
        EnqueueEvent(event_type_names::kFullscreenchange, *element, doc, request_type);
    }
...
ScriptPromise<IDLUndefined> Fullscreen::ExitFullscreen(Document& doc, ScriptState* script_state, ExceptionState* exception_state, bool ua_originated)
{
    ...
    if (!docs.empty() && docs.back() == &top_level_doc && IsSimpleFullscreenDocument(top_level_doc)) {
        resize = true;
    }
    ...
    FullscreenRequestType request_type = element_params ? element_params->request_type() : FullscreenRequestType::kUnprefixed;
    if (element_params) {
        ...
        if (request_type == FullscreenRequestType::kUnprefixed || request_type == FullscreenRequestType::kPrefixed) {
            UMA_HISTOGRAM_LONG_TIMES(kFullscreenDurationMetricKeyRequestFullscreen, base::TimeTicks::Now() - element_params->fullscreen_enter_time());
        }
    }
    ...
    if (resize) {
        ...
        doc.GetAgent().event_loop()->EnqueueMicrotask(
            WTF::BindOnce(ContinueExitFullscreen, WrapPersistent(&doc), WrapPersistent(resolver), false /* resize */));
    }
    return promise;
}
...
void Fullscreen::DidExitFullscreen(Document& document)
{
    ...
    fullscreen.block_automatic_fullscreen_until_ = base::TimeTicks::Now() + base::Seconds(5);
...
void Fullscreen::ContinueExitFullscreen(Document* doc, ScriptPromiseResolver<IDLUndefined>* resolver, bool resize)
{
    if (!doc || !doc->IsActive() || !doc->GetFrame()) {
        if (resolver) {
            ScriptState::Scope scope(resolver->GetScriptState());
            resolver->Reject(V8ThrowException::CreateTypeError(resolver->GetScriptState()->GetIsolate(), "Document is not active"));
        }
        return;
    }
    ...
    for (Frame* descendant = doc->GetFrame()->Tree().FirstChild(); descendant; descendant = descendant->Tree().TraverseNext(doc->GetFrame())) {
    ...
    for (Document* exit_doc : exit_docs) {
        ...
        EnqueueEvent(event_type_names::kFullscreenchange, *exit_element, *exit_doc, request_type);
    ...
    for (Document* descendant_doc : descendant_docs) {
        ...
        EnqueueEvent(event_type_names::kFullscreenchange, *descendant_element, *descendant_doc, request_type);
...
bool Fullscreen::FullscreenEnabled(Document& document, ReportOptions report_on_failure)
{
    ...
    return AllowedToUseFullscreen(document, report_on_failure) && FullscreenIsSupported(document);
}
...
void Fullscreen::DidUpdateSize(Element& element)
{
    ...
    // bit surprising.
    element.SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kFullscreen));
}
...
Fullscreen::PendingRequest::PendingRequest(
    Element* element, FullscreenRequestType type, const FullscreenOptions* options, ScriptPromiseResolver<IDLUndefined>* resolver)
    : element_(element)
    , type_(type)
    , options_(options)
    , resolver_(resolver)
