...
class CORE_EXPORT Fullscreen final : public GarbageCollected<Fullscreen>, public Supplement<LocalDOMWindow>, public ExecutionContextLifecycleObserver {
public:
    ...
    static ScriptPromise<IDLUndefined> RequestFullscreen(
        Element&, const FullscreenOptions*, FullscreenRequestType, ScriptState* state = nullptr, ExceptionState* exception_state = nullptr);

    static void FullyExitFullscreen(Document&, bool ua_originated = false);
    static ScriptPromise<IDLUndefined> ExitFullscreen(
        Document&, ScriptState* state = nullptr, ExceptionState* exception_state = nullptr, bool ua_originated = false);

    static bool FullscreenEnabled(Document&, ReportOptions report_on_failure = ReportOptions::kDoNotReport);
...
private:
    ...
    static void EnforceRequestFullscreenConditions(Element& pending, Document& document, base::OnceCallback<void(RequestFullscreenError)> callback);

    // Run after EnforceRequestFullscreenConditions checks for any `error`.
    static void ContinueRequestFullscreenAfterConditionsEnforcement(Element* pending, FullscreenRequestType request_type, const FullscreenOptions* options,
        ScriptPromiseResolver<IDLUndefined>* resolver, RequestFullscreenError error);

    static void ContinueRequestFullscreen(
        Document&, Element&, FullscreenRequestType, const FullscreenOptions*, ScriptPromiseResolver<IDLUndefined>* resolver, RequestFullscreenError error);

    static void ContinueExitFullscreen(Document*, ScriptPromiseResolver<IDLUndefined>* resolver, bool resize);

    void FullscreenElementChanged(Element* old_element, Element* new_element, FullscreenRequestType new_request_type);
    ...
    public:
        PendingRequest(Element* element, FullscreenRequestType type, const FullscreenOptions* options, ScriptPromiseResolver<IDLUndefined>* resolver);
...
{
    if (HasFullscreenElements() && FullscreenElementFrom(element.GetDocument()) == &element) [[unlikely]] {
        return true;
    }
    return false;
}
