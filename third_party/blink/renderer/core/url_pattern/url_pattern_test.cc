...
TEST(URLPatternTest, CompatibleFromString)
{
    ...
    v8::Local<v8::String> pattern_string = V8String(scope.GetIsolate(), "baz/:quux");
    auto* compatible = V8URLPatternCompatible::Create(scope.GetIsolate(), pattern_string, ASSERT_NO_EXCEPTION);
    auto* url_pattern = URLPattern::From(scope.GetIsolate(), compatible, base_url, ASSERT_NO_EXCEPTION);
...
TEST(URLPatternTest, CompatibleFromStringInvalid)
{
    ...
    EXPECT_FALSE(URLPattern::From(scope.GetIsolate(), compatible, base_url, exception_state));
...
TEST(URLPatternTest, CompatibleFromInit)
{
    ...
    auto* compatible = V8URLPatternCompatible::Create(scope.GetIsolate(), init, ASSERT_NO_EXCEPTION);
    auto* url_pattern = URLPattern::From(scope.GetIsolate(), compatible, base_url, ASSERT_NO_EXCEPTION);
    EXPECT_EQ(url_pattern->protocol(), "https");
    EXPECT_EQ(url_pattern->hostname(), "urlpattern.example");
...
TEST(URLPatternTest, CompatibleFromInitWithBaseURL)
{
    ...
    KURL base_url("https://urlpattern.example/foo/bar");
    V8TestingScope scope(base_url);
    v8::Local<v8::Value> init = Eval(scope, "({search: 'a=42', baseURL: 'https://alt.example/'})");
    ASSERT_TRUE(init->IsObject());
    auto* compatible = V8URLPatternCompatible::Create(scope.GetIsolate(), init, ASSERT_NO_EXCEPTION);
    auto* url_pattern = URLPattern::From(scope.GetIsolate(), compatible, base_url, ASSERT_NO_EXCEPTION);
    EXPECT_EQ(url_pattern->protocol(), "https");
...
TEST(URLPatternTest, CompatibleFromURLPattern)
{
    ...
    KURL base_url("https://urlpattern.example/foo/bar");
    V8TestingScope scope(base_url);
    v8::Local<v8::Value> wrapper = Eval(scope, "new URLPattern({protocol: 'https'})");
    ASSERT_TRUE(V8URLPattern::HasInstance(scope.GetIsolate(), wrapper));
    auto* compatible = V8URLPatternCompatible::Create(scope.GetIsolate(), wrapper, ASSERT_NO_EXCEPTION);
    auto* url_pattern = URLPattern::From(scope.GetIsolate(), compatible, base_url, ASSERT_NO_EXCEPTION);
    EXPECT_EQ(url_pattern, V8URLPattern::ToWrappable(scope.GetIsolate(), wrapper.As<v8::Object>()));
}
