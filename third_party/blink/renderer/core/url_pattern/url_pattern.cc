...
using ComponentSet = base::EnumSet<Component::Type, Component::Type::kProtocol, Component::Type::kHash>;
...
void ApplyInit(const URLPatternInit* init, ValueType type, String& protocol, String& username, String& password, String& hostname, String& port,
    String& pathname, String& search, String& hash, ExceptionState& exception_state)
{
    ...
    if (init->hasBaseURL()) {
        ...
        //                                 +-> username --> password
        protocol = init->hasProtocol() ? String() : EscapeBaseURLString(base_url.Protocol(), type);
        username = (type == ValueType::kPattern || (init->hasProtocol() || init->hasHostname() || init->hasPort() || init->hasUsername()))
            ? String()
            : EscapeBaseURLString(base_url.User(), type);
        password
            = (type == ValueType::kPattern || (init->hasProtocol() || init->hasHostname() || init->hasPort() || init->hasUsername() || init->hasPassword()))
            ? String()
            : EscapeBaseURLString(base_url.Pass(), type);
        hostname = (init->hasProtocol() || init->hasHostname()) ? String() : EscapeBaseURLString(base_url.Host(), type);
        port = (init->hasProtocol() || init->hasHostname() || init->hasPort()) ? String()
            : base_url.Port() > 0                                              ? String::Number(base_url.Port())
                                                                               : g_empty_string;
        pathname
            = (init->hasProtocol() || init->hasHostname() || init->hasPort() || init->hasPathname()) ? String() : EscapeBaseURLString(base_url.GetPath(), type);
        search = (init->hasProtocol() || init->hasHostname() || init->hasPort() || init->hasPathname() || init->hasSearch())
            ? String()
            : EscapeBaseURLString(base_url.Query(), type);
        hash = (init->hasProtocol() || init->hasHostname() || init->hasPort() || init->hasPathname() || init->hasSearch() || init->hasHash()) ? String()
            : base_url.HasFragmentIdentifier() ? EscapeBaseURLString(base_url.FragmentIdentifier(), type)
                                               : g_empty_string;
    }
    ...
    // baseURL values.
    if (init->hasProtocol()) {
        protocol = url_pattern::CanonicalizeProtocol(init->protocol(), type, exception_state);
    ...
    if (init->hasUsername() || init->hasPassword()) {
        ...
        url_pattern::CanonicalizeUsernameAndPassword(init_username, init_password, type, username, password, exception_state);
        if (exception_state.HadException())
            return;
    }
    if (init->hasHostname()) {
        hostname = url_pattern::CanonicalizeHostname(init->hostname(), type, exception_state);
        if (exception_state.HadException())
            return;
    }
    if (init->hasPort()) {
        port = url_pattern::CanonicalizePort(init->port(), type, protocol, exception_state);
        if (exception_state.HadException())
            return;
    }
    if (init->hasPathname()) {
        pathname = init->pathname();
        if (base_url.IsValid() && base_url.IsHierarchical() && !IsAbsolutePathname(pathname, type)) {
        ...
        pathname = url_pattern::CanonicalizePathname(protocol, pathname, type, exception_state);
        if (exception_state.HadException())
            return;
    }
    if (init->hasSearch()) {
        search = url_pattern::CanonicalizeSearch(init->search(), type, exception_state);
        if (exception_state.HadException())
            return;
    }
    if (init->hasHash()) {
...
URLPatternComponentResult* MakeURLPatternComponentResult(ScriptState* script_state, const String& input, const Vector<std::pair<String, String>>& group_values)
{
    ...
    for (const auto& pair : group_values) {
        ...
        v8_group_values.emplace_back(pair.first, ScriptValue(script_state->GetIsolate(), std::move(v8_value)));
    }
...
URLPatternInit* MakeURLPatternInit(const liburlpattern::ConstructorStringParser::Result& result)
{
...
ComponentSet ToURLPatternComponentSet(const liburlpattern::ConstructorStringParser::ComponentSet& present_components)
{
    ComponentSet result;
...
URLPattern* URLPattern::From(v8::Isolate* isolate, const V8URLPatternCompatible* compatible, const KURL& base_url, ExceptionState& exception_state)
{
    ...
    case V8URLPatternCompatible::ContentType::kURLPatternInit: {
        ...
        return Create(isolate, init, /*precomputed_protocol_component=*/nullptr, MakeGarbageCollected<URLPatternOptions>(), exception_state);
    }
    case V8URLPatternCompatible::ContentType::kUSVString:
        return Create(isolate, MakeGarbageCollected<V8URLPatternInput>(compatible->GetAsUSVString()), base_url.GetString(),
            MakeGarbageCollected<URLPatternOptions>(), exception_state);
    }
}

URLPattern* URLPattern::Create(
    v8::Isolate* isolate, const V8URLPatternInput* input, const String& base_url, const URLPatternOptions* options, ExceptionState& exception_state)
{
    if (input->GetContentType() == V8URLPatternInput::ContentType::kURLPatternInit) {
        exception_state.ThrowTypeError("Invalid second argument baseURL '" + base_url
    ...
    liburlpattern::ConstructorStringParser constructor_string_parser(utf8_string.AsStringView());

    Component* protocol_component = nullptr;
    absl::Status status = constructor_string_parser.Parse([=, &protocol_component, &exception_state](std::string_view protocol_string) -> absl::StatusOr<bool> {
        protocol_component = Component::Compile(isolate, String::FromUTF8(protocol_string), Component::Type::kProtocol,
            /*protocol_component=*/nullptr, *options, exception_state);
        if (exception_state.HadException()) {
            return absl::InvalidArgumentError("Failed to compile protocol");
        }
        return protocol_component && protocol_component->ShouldTreatAsStandardURL();
    });
    ...
    if (!status.ok()) {
        exception_state.ThrowTypeError("Invalid input string '" + input_string + "'. It unexpectedly fails to tokenize.");
        return nullptr;
    }
    URLPatternInit* init = MakeURLPatternInit(constructor_string_parser.GetResult());

    if (!base_url && !init->hasProtocol()) {
        exception_state.ThrowTypeError("Relative constructor string '" + input_string + "' must have a base URL passed as the second argument.");
        return nullptr;
    }
    ...
    URLPattern* result = Create(isolate, init, protocol_component, options, exception_state);
    if (result) {
        URLPattern::ComponentSet present = ToURLPatternComponentSet(constructor_string_parser.GetPresentComponents());
        URLPattern::ComponentSet wildcard_with_string_format_change = URLPattern::ComponentSet::All();
        wildcard_with_string_format_change.RemoveAll(present);
        if (present.Has(Component::Type::kUsername)) {
            wildcard_with_string_format_change.RemoveAll({ Component::Type::kProtocol, Component::Type::kHostname, Component::Type::kPort });
        }
        if (present.Has(Component::Type::kPassword)) {
            wildcard_with_string_format_change.RemoveAll(
                { Component::Type::kProtocol, Component::Type::kHostname, Component::Type::kPort, Component::Type::kUsername });
        }
        if (present.Has(Component::Type::kHostname)) {
            // As a special case, don't wildcard the port if the host is present, even
            // with no path.
            wildcard_with_string_format_change.RemoveAll({ Component::Type::kProtocol, Component::Type::kPort });
        }
        if (present.Has(Component::Type::kPort)) {
            wildcard_with_string_format_change.RemoveAll({ Component::Type::kProtocol, Component::Type::kHostname });
        }
        ...
        if (present.Has(Component::Type::kSearch)) {
            wildcard_with_string_format_change.RemoveAll(
                { Component::Type::kProtocol, Component::Type::kHostname, Component::Type::kPort, Component::Type::kPathname });
        }
        if (present.Has(Component::Type::kHash)) {
            wildcard_with_string_format_change.RemoveAll(
                { Component::Type::kProtocol, Component::Type::kHostname, Component::Type::kPort, Component::Type::kPathname, Component::Type::kSearch });
        }
        result->wildcard_with_string_format_change_ = wildcard_with_string_format_change;
    }
    return result;
}

URLPattern* URLPattern::Create(v8::Isolate* isolate, const V8URLPatternInput* input, const String& base_url, ExceptionState& exception_state)
{
    return Create(isolate, input, base_url, MakeGarbageCollected<URLPatternOptions>(), exception_state);
}

URLPattern* URLPattern::Create(v8::Isolate* isolate, const V8URLPatternInput* input, const URLPatternOptions* options, ExceptionState& exception_state)
{
    if (input->IsURLPatternInit()) {
        return URLPattern::Create(isolate, input->GetAsURLPatternInit(),
            /*precomputed_protocol_component=*/nullptr, options, exception_state);
    }
    return Create(isolate, input, /*base_url=*/String(), options, exception_state);
}

URLPattern* URLPattern::Create(v8::Isolate* isolate, const V8URLPatternInput* input, ExceptionState& exception_state)
{
    if (input->IsURLPatternInit()) {
        return URLPattern::Create(isolate, input->GetAsURLPatternInit(),
            /*precomputed_protocol_component=*/nullptr, MakeGarbageCollected<URLPatternOptions>(), exception_state);
    }

    return Create(isolate, input, /*base_url=*/String(), exception_state);
}

URLPattern* URLPattern::Create(v8::Isolate* isolate, const URLPatternInit* init, Component* precomputed_protocol_component, const URLPatternOptions* options,
    ExceptionState& exception_state)
{
    ...
    ApplyInit(init, ValueType::kPattern, protocol, username, password, hostname, port, pathname, search, hash, exception_state);
    if (exception_state.HadException())
        return nullptr;
    ...
    if (!protocol_component) {
        protocol_component = Component::Compile(isolate, protocol, Component::Type::kProtocol,
            /*protocol_component=*/nullptr, *options, exception_state);
    }
    if (exception_state.HadException())
        return nullptr;

    auto* username_component = Component::Compile(isolate, username, Component::Type::kUsername, protocol_component, *options, exception_state);
    if (exception_state.HadException())
        return nullptr;

    auto* password_component = Component::Compile(isolate, password, Component::Type::kPassword, protocol_component, *options, exception_state);
    if (exception_state.HadException())
        return nullptr;

    auto* hostname_component = Component::Compile(isolate, hostname, Component::Type::kHostname, protocol_component, *options, exception_state);
    if (exception_state.HadException())
        return nullptr;

    auto* port_component = Component::Compile(isolate, port, Component::Type::kPort, protocol_component, *options, exception_state);
    if (exception_state.HadException())
        return nullptr;

    auto* pathname_component = Component::Compile(isolate, pathname, Component::Type::kPathname, protocol_component, *options, exception_state);
    if (exception_state.HadException())
        return nullptr;

    auto* search_component = Component::Compile(isolate, search, Component::Type::kSearch, protocol_component, *options, exception_state);
    if (exception_state.HadException())
        return nullptr;

    auto* hash_component = Component::Compile(isolate, hash, Component::Type::kHash, protocol_component, *options, exception_state);
    if (exception_state.HadException())
        return nullptr;
    ...
    URLPattern* result = MakeGarbageCollected<URLPattern>(protocol_component, username_component, password_component, hostname_component, port_component,
        pathname_component, search_component, hash_component, urlpattern_options, base::PassKey<URLPattern>());
    if (init->hasBaseURL()) {
        auto& would_be_wildcard = result->wildcard_with_base_url_change_;
        if (!init->hasUsername() && (init->hasProtocol() || init->hasHostname() || init->hasPort())) {
            would_be_wildcard.Put(Component::Type::kUsername);
        }
        if (!init->hasPassword() && (init->hasProtocol() || init->hasHostname() || init->hasPort() || init->hasUsername())) {
        ...
        if (!init->hasPathname() && (init->hasProtocol() || init->hasHostname() || init->hasPort())) {
            would_be_wildcard.Put(Component::Type::kPathname);
        }
        if (!init->hasSearch() && (init->hasProtocol() || init->hasHostname() || init->hasPort() || init->hasPathname())) {
            would_be_wildcard.Put(Component::Type::kSearch);
        }
        if (!init->hasHash() && (init->hasProtocol() || init->hasHostname() || init->hasPort() || init->hasPathname() || init->hasSearch())) {
...
URLPattern::URLPattern(Component* protocol, Component* username, Component* password, Component* hostname, Component* port, Component* pathname,
    Component* search, Component* hash, Options options, base::PassKey<URLPattern> key)
    : protocol_(protocol)
    , username_(username)
    , password_(password)
    , hostname_(hostname)
...
bool URLPattern::test(ScriptState* script_state, const V8URLPatternInput* input, const String& base_url, ExceptionState& exception_state) const
{
    return Match(script_state, input, base_url, /*result=*/nullptr, exception_state);
}

bool URLPattern::test(ScriptState* script_state, const V8URLPatternInput* input, ExceptionState& exception_state) const
{
    return test(script_state, input, /*base_url=*/String(), exception_state);
}

URLPatternResult* URLPattern::exec(ScriptState* script_state, const V8URLPatternInput* input, const String& base_url, ExceptionState& exception_state) const
{
...
URLPatternResult* URLPattern::exec(ScriptState* script_state, const V8URLPatternInput* input, ExceptionState& exception_state) const
{
...
{
    const url_pattern::Component* components[] = { protocol_, username_, password_, hostname_, port_, pathname_, search_, hash_ };
    return base::ranges::any_of(components, &url_pattern::Component::HasRegExpGroups);
}

// static
int URLPattern::compareComponent(const V8URLPatternComponent& component, const URLPattern* left, const URLPattern* right)
{
    switch (component.AsEnum()) {
    case V8URLPatternComponent::Enum::kProtocol:
        return url_pattern::Component::Compare(*left->protocol_, *right->protocol_);
    case V8URLPatternComponent::Enum::kUsername:
        return url_pattern::Component::Compare(*left->username_, *right->username_);
    case V8URLPatternComponent::Enum::kPassword:
        return url_pattern::Component::Compare(*left->password_, *right->password_);
    case V8URLPatternComponent::Enum::kHostname:
        return url_pattern::Component::Compare(*left->hostname_, *right->hostname_);
    ...
    case V8URLPatternComponent::Enum::kPathname:
        return url_pattern::Component::Compare(*left->pathname_, *right->pathname_);
...
std::optional<SafeUrlPattern> URLPattern::ToSafeUrlPattern(ExceptionState& exception_state) const
{
    const std::pair<const url_pattern::Component*, const char*> components_with_names[] = { { protocol_, "protocol" }, { username_, "username" },
        { password_, "password" }, { hostname_, "hostname" }, { port_, "port" }, { pathname_, "pathname" }, { search_, "search" }, { hash_, "hash" } };
    ...
    for (auto [component, name] : components_with_names) {
        if (component->HasRegExpGroups()) {
            components_with_regexp
                = components_with_regexp + (components_with_regexp.IsNull() ? "" : ", ") + name + " (" + component->GeneratePatternString() + ")";
        }
    }
    if (!components_with_regexp.IsNull()) {
        exception_state.ThrowTypeError("The pattern cannot contain regexp groups, but did in the following "
                                       "components: "
            + components_with_regexp);
        return std::nullopt;
    }
...
String URLPattern::ToString() const
{
    ...
    builder.Append("(");
    Vector<String> components = { protocol(), username(), password(), hostname(), port(), pathname(), search(), hash() };
...
bool URLPattern::Match(
    ScriptState* script_state, const V8URLPatternInput* input, const String& base_url, URLPatternResult* result, ExceptionState& exception_state) const
{
    ...
    case V8URLPatternInput::ContentType::kURLPatternInit: {
        ...
        ApplyInit(
            init, ValueType::kURL, protocol, username, password, hostname, port, pathname, search, hash, PassThroughException(script_state->GetIsolate()));
    ...
    // the input.
    bool matched = protocol_->Match(protocol, protocol_group_list_ref) && username_->Match(username, username_group_list_ref)
        && password_->Match(password, password_group_list_ref) && hostname_->Match(hostname, hostname_group_list_ref) && port_->Match(port, port_group_list_ref)
        && pathname_->Match(pathname, pathname_group_list_ref) && search_->Match(search, search_group_list_ref) && hash_->Match(hash, hash_group_list_ref);
    ...
    result->setProtocol(MakeURLPatternComponentResult(script_state, protocol, protocol_group_list));
    result->setUsername(MakeURLPatternComponentResult(script_state, username, username_group_list));
    result->setPassword(MakeURLPatternComponentResult(script_state, password, password_group_list));
    result->setHostname(MakeURLPatternComponentResult(script_state, hostname, hostname_group_list));
    result->setPort(MakeURLPatternComponentResult(script_state, port, port_group_list));
    result->setPathname(MakeURLPatternComponentResult(script_state, pathname, pathname_group_list));
    result->setSearch(MakeURLPatternComponentResult(script_state, search, search_group_list));
    result->setHash(MakeURLPatternComponentResult(script_state, hash, hash_group_list));
