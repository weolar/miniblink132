...
String CanonicalizeProtocol(const String& input, ValueType type, ExceptionState& exception_state)
{
    ...
    if (stripped.Is8Bit()) {
        ...
        result = url::CanonicalizeScheme(utf8.data(), url::Component(0, utf8.size()), &canon_output, &component);
    } else {
        result = url::CanonicalizeScheme(stripped.Characters16(), url::Component(0, stripped.length()), &canon_output, &component);
    }
...
void CanonicalizeUsernameAndPassword(
    const String& username, const String& password, ValueType type, String& username_out, String& password_out, ExceptionState& exception_state)
{
    if (type == ValueType::kPattern) {
        // Canonicalization for patterns is handled during compilation via
        // encoding callbacks.
    ...
    if (username && password && username.Is8Bit() && password.Is8Bit()) {
        ...
        result = url::CanonicalizeUserInfo(username_utf8.data(), url::Component(0, username_utf8.size()), password_utf8.data(),
            url::Component(0, password_utf8.size()), &canon_output, &username_component, &password_component);
        ...
        result = url::CanonicalizeUserInfo(username16.Characters16(), url::Component(0, username16.length()), password16.Characters16(),
            url::Component(0, password16.length()), &canon_output, &username_component, &password_component);
    }

    if (!result) {
        exception_state.ThrowTypeError("Invalid username '" + username + "' and/or password '" + password + "'.");
        return;
    }
...
String CanonicalizeHostname(const String& input, ValueType type, ExceptionState& exception_state)
{
    if (type == ValueType::kPattern) {
        // Canonicalization for patterns is handled during compilation via
        // encoding callbacks.
...
String CanonicalizePort(const String& input, ValueType type, const String& protocol, ExceptionState& exception_state)
{
    if (type == ValueType::kPattern) {
        // Canonicalization for patterns is handled during compilation via
        // encoding callbacks.
    ...
    url::RawCanonOutputT<char> canon_output;
    url::Component component;
    if (!url::CanonicalizePort(utf8.data(), url::Component(0, utf8.size()), default_port, &canon_output, &component)) {
        exception_state.ThrowTypeError("Invalid port '" + input + "'.");
        return String();
    }

    return component.len == -1 ? g_empty_string : StringFromCanonOutput(canon_output, component);
}

String CanonicalizePathname(const String& protocol, const String& input, ValueType type, ExceptionState& exception_state)
{
    if (type == ValueType::kPattern) {
        // Canonicalization for patterns is handled during compilation via
        // encoding callbacks.
    ...
    if (protocol.empty()) {
        ...
        standard = url::IsStandard(protocol.Characters16(), url::Component(0, protocol.length()));
    }
    ...
    const auto canonicalize_path = [&](const auto* data, int length) {
        if (standard) {
            return url::CanonicalizePartialPath(data, url::Component(0, length), &canon_output, &component);
        }
        url::CanonicalizePathURLPath(data, url::Component(0, length), &canon_output, &component);
        return true;
    };
...
String CanonicalizeSearch(const String& input, ValueType type, ExceptionState& exception_state)
{
    ...
    if (stripped.Is8Bit()) {
        ...
        url::CanonicalizeQuery(stripped.Characters16(), url::Component(0, stripped.length()),
            /*converter=*/nullptr, &canon_output, &component);
    }
...
String CanonicalizeHash(const String& input, ValueType type, ExceptionState& exception_state)
{
    ...
    if (stripped.Is8Bit()) {
        StringUTF8Adaptor utf8(stripped);
        url::CanonicalizeRef(utf8.data(), url::Component(0, utf8.size()), &canon_output, &component);
    } else {
        url::CanonicalizeRef(stripped.Characters16(), url::Component(0, stripped.length()), &canon_output, &component);
    }
