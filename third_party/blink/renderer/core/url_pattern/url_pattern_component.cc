...
// type.
const liburlpattern::Options GetOptions(Component::Type type, Component* protocol_component, const URLPatternOptions& external_options)
{
    ...
    Options value = { .delimiter_list = "", .prefix_list = "", .sensitive = !external_options.ignoreCase(), .strict = true };
...
Component::Component(Type type, liburlpattern::Pattern pattern, ScriptRegexp* regexp, Vector<String> name_list, base::PassKey<Component> key)
    : type_(type)
...
bool Component::Match(StringView input, Vector<std::pair<String, String>>* group_list) const
{
    // If there is a regexp, then we cannot directly match the pattern.
    if (regexp_) {
        Vector<String> value_list;
        if (group_list) {
            value_list.ReserveInitialCapacity(base::checked_cast<wtf_size_t>(name_list_.size()));
        }
        bool result = regexp_->Match(input, /*start_from=*/0, /*match_length=*/nullptr, group_list ? &value_list : nullptr) == 0;
    ...
    std::vector<std::pair<std::string_view, std::optional<std::string_view>>> pattern_group_list;
    // Lossy UTF8 conversion is fine given the input has come through a
    // USVString webidl argument.
    StringUTF8Adaptor utf8(input);
    bool result = pattern_.DirectMatch(utf8.AsStringView(), group_list ? &pattern_group_list : nullptr);
    if (group_list) {
        group_list->ReserveInitialCapacity(base::checked_cast<wtf_size_t>(pattern_group_list.size()));
...
bool Component::ShouldTreatAsStandardURL() const
{
    ...
    should_treat_as_standard_url_ = base::ranges::any_of(url::GetStandardSchemes(), protocol_matches);
    return *should_treat_as_standard_url_;
}
