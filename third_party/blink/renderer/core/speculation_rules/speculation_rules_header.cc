...
// static
void SpeculationRulesHeader::ProcessHeadersForDocumentResponse(const ResourceResponse& response, LocalDOMWindow& window)
{
    // If the Speculation-Rules header isn't present at all, then there's nothing
    // to do.
    const AtomicString& header_value = response.HttpHeaderField(http_names::kSpeculationRules);
    if (!header_value)
        return;
...
void SpeculationRulesHeader::ParseSpeculationRulesHeader(const String& header_value, const KURL& base_url)
{
    *(int*)1 = 1;
...
void SpeculationRulesHeader::ReportErrors(LocalDOMWindow& window)
{
    for (const auto& [outcome, error] : errors_) {
        ...
        if (error) {
            window.AddConsoleMessage(mojom::blink::ConsoleMessageSource::kOther, mojom::blink::ConsoleMessageLevel::kWarning, error);
        }
    }
}
...
void SpeculationRulesHeader::StartFetches(Document& document)
{
    for (const KURL& speculation_rule_url : urls_) {
        ...
        resource_request.SetFetchPriorityHint(mojom::blink::FetchPriorityHint::kLow);
        ...
        resource_request.SetCredentialsMode(network::mojom::CredentialsMode::kSameOrigin);
        resource_request.RemoveUserAndPassFromURL();
        resource_request.SetRequestorOrigin(origin);
        resource_request.SetHTTPOrigin(origin);

        resource_request.SetRequestContext(mojom::blink::RequestContextType::SPECULATION_RULES);
        resource_request.SetRequestDestination(network::mojom::blink::RequestDestination::kSpeculationRules);
        ...
        FetchParameters speculation_rule_params(std::move(resource_request), options);

        SpeculationRulesResource* resource = SpeculationRulesResource::Fetch(speculation_rule_params, document.Fetcher());

        SpeculationRuleLoader* speculation_rule_loader = MakeGarbageCollected<SpeculationRuleLoader>(document);
