...
class CORE_EXPORT DocumentSpeculationRules : public GarbageCollected<DocumentSpeculationRules>, public Supplement<Document> {
public:
    ...
    void HrefAttributeChanged(HTMLAnchorElementBase* link, const AtomicString& old_value, const AtomicString& new_value);
...
private:
    ...
    void AddLinkBasedSpeculationCandidates(HeapVector<Member<SpeculationCandidate>>& candidates);
    ...
    bool IsMicrotaskQueued() const
    {
        return pending_update_state_ == PendingUpdateState::kMicrotaskQueued
            || pending_update_state_ == PendingUpdateState::kMicrotaskQueuedWithForcedStyleUpdate;
    }
    ...
    // added/removed.
    HeapHashMap<Member<HTMLAnchorElementBase>, Member<HeapVector<Member<SpeculationCandidate>>>> matched_links_;
