...
std::ostream& network::mojom::operator<<(std::ostream& a, ::network::mojom::NoVarySearchParseError b)
{
    *(int*)1 = 1;
    return a << (int)(b);
}
...
void AddConsoleMessageForSpeculationRuleSetValidation(
    SpeculationRuleSet& speculation_rule_set, Document& element_document, ScriptElementBase* script_element, SpeculationRulesResource* resource)
{
    ...
    if (speculation_rule_set.HasError()) {
        ...
        if (script_element) {
            error_message = "While parsing speculation rules: " + speculation_rule_set.error_message();
        } else {
            error_message = "While parsing speculation rules fetched from \"" + resource->GetResourceRequest().Url().ElidedString()
                + "\": " + speculation_rule_set.error_message() + "\".";
        }
        auto* console_message
            = MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kOther, mojom::blink::ConsoleMessageLevel::kWarning, error_message);
        if (script_element) {
            console_message->SetNodes(element_document.GetFrame(), { script_element->GetDOMNodeId() });
        }
    ...
    if (speculation_rule_set.HasWarnings()) {
        ...
        if (script_element) {
            warning_message = "While parsing speculation rules: " + speculation_rule_set.warning_messages()[0];
        } else {
            warning_message = "While parsing speculation rules fetched from \"" + resource->GetResourceRequest().Url().ElidedString()
                + "\": " + speculation_rule_set.warning_messages()[0] + "\".";
        }
        auto* console_message
            = MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kOther, mojom::blink::ConsoleMessageLevel::kWarning, warning_message);
        if (script_element) {
            console_message->SetNodes(element_document.GetFrame(), { script_element->GetDOMNodeId() });
        }
        element_document.AddConsoleMessage(console_message);
...
bool IsValidBrowsingContextNameOrKeyword(const String& name_or_keyword)
{
    ...
    if (IsValidContextName(name_or_keyword) || EqualIgnoringASCIICase(name_or_keyword, "_blank") || EqualIgnoringASCIICase(name_or_keyword, "_self")
        || EqualIgnoringASCIICase(name_or_keyword, "_parent") || EqualIgnoringASCIICase(name_or_keyword, "_top")) {
        return true;
    }
    return false;
}
...
SpeculationRule* ParseSpeculationRule(
    JSONObject* input, const KURL& base_url, ExecutionContext* context, bool is_browser_injected, String* out_error, Vector<String>& out_warnings)
{
    ...
    const char* const kKnownKeys[]
        = { "source", "urls", "where", "requires", "target_hint", "referrer_policy", "relative_to", "eagerness", "expects_no_vary_search" };
    for (wtf_size_t i = 0; i < input->size(); ++i) {
        const String& input_key = input->at(i).first;
        if (!base::Contains(kKnownKeys, input_key)) {
            SetParseErrorMessage(out_error, "A rule contains an unknown key: \"" + input_key + "\".");
            return nullptr;
        }
    }
    ...
    if (source_value) {
        if (!source_value->AsString(&source)) {
            SetParseErrorMessage(out_error, "The value of the \"source\" key must be a string.");
            return nullptr;
        }
    } else {
    ...
    if (source != "list" && source != "document") {
        SetParseErrorMessage(out_error, "A rule has an unknown source: \"" + source + "\".");
        return nullptr;
    }
    ...
    if (source == "list") {
        ...
        if (input->Get("where")) {
            SetParseErrorMessage(out_error, "A list rule may not have document rule matchers.");
            return nullptr;
        }
        ...
        if (JSONValue* relative_to = input->Get("relative_to")) {
            ...
            if (!relative_to->AsString(&value) || !base::Contains(kKnownRelativeToValues, value)) {
                SetParseErrorMessage(out_error, "A rule has an unknown \"relative_to\" value.");
                return nullptr;
            }
        ...
        if (!input_urls) {
            SetParseErrorMessage(out_error, "A list rule must have a \"urls\" array.");
            return nullptr;
        }
    ...
    if (source == "document") {
        ...
        if (input->Get("urls")) {
            SetParseErrorMessage(out_error, "A document rule cannot have a \"urls\" key.");
            return nullptr;
        }
        ...
        if (!input->Get("where")) {
            ...
            document_rule_predicate = DocumentRulePredicate::Parse(input->GetJSONObject("where"), base_url, context, IGNORE_EXCEPTION_FOR_TESTING, out_error);
        }
    ...
    SpeculationRule::RequiresAnonymousClientIPWhenCrossOrigin requires_anonymous_client_ip(false);
    if (JSONArray* requirements_array = JSONArray::Cast(requirements)) {
        for (wtf_size_t i = 0; i < requirements_array->size(); ++i) {
            ...
            if (requirement == "anonymous-client-ip-when-cross-origin") {
                requires_anonymous_client_ip = SpeculationRule::RequiresAnonymousClientIPWhenCrossOrigin(true);
            } else {
                SetParseErrorMessage(out_error, "A rule has an unknown requirement: \"" + requirement + "\".");
                return nullptr;
            }
        }
    }
    ...
    if (target_hint_value) {
        ...
        if (!IsValidBrowsingContextNameOrKeyword(target_hint_str)) {
            SetParseErrorMessage(out_error, "A rule has an invalid \"target_hint\": \"" + target_hint_str + "\".");
            return nullptr;
        }
        target_hint = SpeculationRuleSet::SpeculationTargetHintFromString(target_hint_str);
    }
    ...
    if (referrer_policy_value) {
        ...
        if (!referrer_policy_str.empty()) {
            network::mojom::ReferrerPolicy referrer_policy_out = network::mojom::ReferrerPolicy::kDefault;
            if (!SecurityPolicy::ReferrerPolicyFromString(referrer_policy_str, kDoNotSupportReferrerPolicyLegacyKeywords, &referrer_policy_out)) {
                SetParseErrorMessage(out_error, "A rule has an invalid referrer policy: \"" + referrer_policy_str + "\".");
                return nullptr;
            }
            ...
            UseCounter::Count(context, WebFeature::kSpeculationRulesExplicitReferrerPolicy);
        }
    }
    ...
    if (JSONValue* eagerness_value = input->Get("eagerness")) {
        ...
        if (eagerness_str == "eager" || eagerness_str == "immediate") {
            ...
            SetParseErrorMessage(out_error, "Eagerness value: \"" + eagerness_str + "\" is invalid.");
            return nullptr;
        }

        UseCounter::Count(context, WebFeature::kSpeculationRulesExplicitEagerness);
    } else {
        eagerness = source == "list" ? mojom::blink::SpeculationEagerness::kEager : mojom::blink::SpeculationEagerness::kConservative;
    }

    network::mojom::blink::NoVarySearchPtr no_vary_search = nullptr;
    if (JSONValue* no_vary_search_value = input->Get("expects_no_vary_search"); no_vary_search_value) {
        String no_vary_search_str;
        if (!no_vary_search_value->AsString(&no_vary_search_str)) {
            SetParseErrorMessage(out_error, "expects_no_vary_search's value must be a string.");
            return nullptr;
        }
        ...
        if (no_vary_search_hint->is_parse_error()) {
            ...
            if (parse_error != network::mojom::NoVarySearchParseError::kDefaultValue) {
    ...
    if (is_browser_injected) {
        injection_type = mojom::blink::SpeculationInjectionType::kAutoSpeculationRules;
    } else if (auto* world = context->GetCurrentWorld()) {
        if (world->IsMainWorld()) {
            injection_type = mojom::blink::SpeculationInjectionType::kMainWorldScript;
        } else {
            injection_type = mojom::blink::SpeculationInjectionType::kIsolatedWorldScript;
        }
    }

    return MakeGarbageCollected<SpeculationRule>(std::move(urls), document_rule_predicate, requires_anonymous_client_ip, target_hint, referrer_policy,
        eagerness, std::move(no_vary_search), injection_type);
}
...
SpeculationRuleSet::Source::Source(base::PassKey<SpeculationRuleSet::Source>, const String& source_text, Document* document, std::optional<DOMNodeId> node_id,
    std::optional<KURL> base_url, std::optional<uint64_t> request_id, bool ignore_opt_out)
...
SpeculationRuleSet::Source* SpeculationRuleSet::Source::FromInlineScript(const String& source_text, Document& document, DOMNodeId node_id)
{
    return MakeGarbageCollected<Source>(base::PassKey<Source>(), source_text, &document, node_id, std::nullopt, std::nullopt, false);
}

SpeculationRuleSet::Source* SpeculationRuleSet::Source::FromRequest(const String& source_text, const KURL& base_url, uint64_t request_id)
{
    return MakeGarbageCollected<Source>(base::PassKey<Source>(), source_text, nullptr, std::nullopt, base_url, request_id, false);
}

SpeculationRuleSet::Source* SpeculationRuleSet::Source::FromBrowserInjected(
    const String& source_text, const KURL& base_url, BrowserInjectedSpeculationRuleOptOut opt_out)
{
    const bool ignore_opt_out = opt_out == BrowserInjectedSpeculationRuleOptOut::kIgnore;

    return MakeGarbageCollected<Source>(base::PassKey<Source>(), source_text, nullptr, std::nullopt, base_url, std::nullopt, ignore_opt_out);
}
...
bool SpeculationRuleSet::Source::IsFromBrowserInjectedAndRespectsOptOut() const
{
...
const std::optional<uint64_t>& SpeculationRuleSet::Source::GetRequestId() const
{
    return request_id_;
}
...
SpeculationRuleSet::SpeculationRuleSet(base::PassKey<SpeculationRuleSet>, Source* source)
    : inspector_id_(IdentifiersFactory::CreateIdentifier())
    , source_(source)
{
}

void SpeculationRuleSet::SetError(SpeculationRuleSetErrorType error_type, String error_message)
{
...
void SpeculationRuleSet::AddWarnings(base::span<const String> warning_messages)
{
    warning_messages_.AppendSpan(warning_messages);
}

// static
SpeculationRuleSet* SpeculationRuleSet::Parse(Source* source, ExecutionContext* context)
{
    ...
    SpeculationRuleSet* result = MakeGarbageCollected<SpeculationRuleSet>(base::PassKey<SpeculationRuleSet>(), source);
    ...
    if (!parsed) {
        result->SetError(SpeculationRuleSetErrorType::kSourceIsNotJsonObject,
            parse_error.type != JSONParseErrorType::kNoError ? parse_error.message : "Parsed JSON must be an object.");
        return result;
    }
    ...
    if (!parse_error.duplicate_keys.empty()) {
        ...
        if (parse_error.duplicate_keys.size() == 1) {
            ...
            duplicate_key_warning = "An object contained more than one key named " + key.EncodeForDebugging() + ". All but the last are ignored."
                + ((key == "prefetch" || key == "prerender") ? " It is likely that either one of them was intended to be "
                                                               "another action, or that their rules should be merged into a "
                                                               "single array."
                                                             : String());
        } else {
            StringBuilder builder;
            builder.Append("The following keys were duplicated on one or more objects: ");
    ...
    const auto parse_for_action = [&](const char* key, HeapVector<Member<SpeculationRule>>& destination, bool allow_target_hint,
        ...
        if (!array) {
            result->SetError(SpeculationRuleSetErrorType::kInvalidRulesSkipped, "A rule set for a key must be an array: path = [\"" + String(key) + "\"]");
            return;
        }
        ...
        for (wtf_size_t i = 0; i < array->size(); ++i) {
            ...
            if (!input_rule) {
                result->SetError(
                    SpeculationRuleSetErrorType::kInvalidRulesSkipped, "A rule must be an object: path = [\"" + String(key) + "\"][" + String::Number(i) + "]");
                continue;
            }
            ...
            String error_message;
            SpeculationRule* rule = ParseSpeculationRule(input_rule, base_url, context, source->IsFromBrowserInjected(), &error_message, warning_messages);

            // If parse failed for a rule, then ignore it and continue.
            if (!rule) {
                result->SetError(SpeculationRuleSetErrorType::kInvalidRulesSkipped, error_message);
                continue;
            }

            // Rejects if "target_hint" is set but not allowed.
            if (!allow_target_hint && rule->target_browsing_context_name_hint().has_value()) {
                result->SetError(SpeculationRuleSetErrorType::kInvalidRulesSkipped, "\"target_hint\" may not be set for " + String(key) + " rules.");
                continue;
            }

            // Rejects if "anonymous-client-ip-when-cross-origin" is required but
            // not allowed.
            if (!allow_requires_anonymous_client_ip_when_cross_origin && rule->requires_anonymous_client_ip_when_cross_origin()) {
                result->SetError(SpeculationRuleSetErrorType::kInvalidRulesSkipped,
                    "requirement \"anonymous-client-ip-when-cross-origin\" for \"" + String(key) + "\" is not supported.");
                continue;
            }

            // Add the warnings and continue
            result->AddWarnings(base::span<const String>(warning_messages.data(), warning_messages.size()));
...
// static
mojom::blink::SpeculationTargetHint SpeculationRuleSet::SpeculationTargetHintFromString(const StringView& target_hint_str)
{
...
void SpeculationRuleSet::AddConsoleMessageForValidation(ScriptElementBase& script_element)
{
    AddConsoleMessageForSpeculationRuleSetValidation(*this, script_element.GetDocument(), &script_element, nullptr);
}

void SpeculationRuleSet::AddConsoleMessageForValidation(Document& document, SpeculationRulesResource& resource)
{
    AddConsoleMessageForSpeculationRuleSetValidation(*this, document, nullptr, &resource);
}
