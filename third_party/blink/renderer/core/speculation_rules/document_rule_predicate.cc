...
public:
    ...
    {
        return base::ranges::all_of(clauses_, [&](DocumentRulePredicate* clause) { return clause->Matches(el); });
    }
    ...
    Type GetTypeForTesting() const override
    {
        return Type::kAnd;
    }

    HeapVector<Member<DocumentRulePredicate>> GetSubPredicatesForTesting() const override
    {
        return clauses_;
    }
...
public:
    ...
    bool Matches(const HTMLAnchorElementBase& el) const override
    {
        return base::ranges::any_of(clauses_, [&](DocumentRulePredicate* clause) { return clause->Matches(el); });
    }
...
public:
    explicit Negation(DocumentRulePredicate* clause)
        : clause_(clause)
    {
    }
...
public:
    explicit URLPatternPredicate(HeapVector<Member<URLPattern>> patterns, ExecutionContext* execution_context)
    ...
    bool Matches(const HTMLAnchorElementBase& el) const override
    {
        ...
        for (const auto& pattern : patterns_) {
            ...
            if (pattern->test(ToScriptStateForMainWorld(execution_context_), MakeGarbageCollected<V8URLPatternInput>(href), ASSERT_NO_EXCEPTION)) {
                return true;
            }
        }
        return false;
...
public:
    ...
    bool Matches(const HTMLAnchorElementBase& link) const override
    {
        ...
        const Persistent<HeapHashSet<WeakMember<StyleRule>>>& matched_selectors = computed_style->DocumentRulesSelectors();
...
URLPattern* ParseRawPattern(v8::Isolate* isolate, JSONValue* raw_pattern, const KURL& base_url, ExceptionState& exception_state, String* out_error)
{
    ...
    if (String raw_string; raw_pattern->AsString(&raw_string)) {
        ...
        // serializedBaseURL.
        V8URLPatternInput* url_pattern_input = MakeGarbageCollected<V8URLPatternInput>(raw_string);
        return URLPattern::Create(isolate, url_pattern_input, base_url, exception_state);
    }
    ...
    if (JSONObject* pattern_object = JSONObject::Cast(raw_pattern)) {
        ...
        for (wtf_size_t i = 0; i < pattern_object->size(); i++) {
            ...
            if (!entry.second->AsString(&value)) {
                SetParseErrorMessage(out_error, "Values for a URL pattern object must be strings.");
                return nullptr;
            }
            ...
            if (key == "protocol") {
                ...
                SetParseErrorMessage(out_error, String::Format("Invalid key \"%s\" for a URL pattern object found.", key.Latin1().c_str()));
                return nullptr;
            }
        }
        ...
        V8URLPatternInput* url_pattern_input = MakeGarbageCollected<V8URLPatternInput>(init);
...
String GetPredicateType(JSONObject* input, String* out_error)
{
    ...
    constexpr const char* kValidTypes[] = { "and", "or", "not", "href_matches", "selector_matches" };
            ...
            if (!predicate_type.IsNull()) {
                SetParseErrorMessage(out_error,
                    String::Format("Document rule predicate type is ambiguous, "
                        ...
                        predicate_type.Latin1().c_str(), type.Latin1().c_str()));
                return String();
            }
...
DocumentRulePredicate* DocumentRulePredicate::Parse(
    JSONObject* input, const KURL& ruleset_base_url, ExecutionContext* execution_context, ExceptionState& exception_state, String* out_error)
{
    // If input is not a map, then return null.
    if (!input) {
        SetParseErrorMessage(out_error, "Document rule predicate must be an object.");
        return nullptr;
    }
    ...
    if (predicate_type == "and" || predicate_type == "or") {
        ...
        if (input->size() != 1) {
            SetParseErrorMessage(out_error, String::Format("Document rule predicate with \"%s\" key cannot have other keys.", predicate_type.Latin1().c_str()));
            return nullptr;
        }
        ...
        if (!raw_clauses) {
            SetParseErrorMessage(out_error, String::Format("\"%s\" key should have a list value.", predicate_type.Latin1().c_str()));
            return nullptr;
        }
        ...
        for (wtf_size_t i = 0; i < raw_clauses->size(); i++) {
            ...
            DocumentRulePredicate* clause = Parse(raw_clause, ruleset_base_url, execution_context, exception_state, out_error);
    ...
    if (predicate_type == "not") {
        ...
        if (input->size() != 1) {
            SetParseErrorMessage(out_error, "Document rule predicate with \"not\" key cannot have other keys.");
            return nullptr;
        }
    ...
    if (predicate_type == "href_matches") {
        ...
        for (wtf_size_t i = 0; i < input->size(); ++i) {
            ...
            if (key == "href_matches") {
                ...
                String relative_to;
                if (!input->GetString("relative_to", &relative_to) || !base::Contains(kKnownRelativeToValues, relative_to)) {
                    SetParseErrorMessage(
                        out_error, String::Format("Unrecognized \"relative_to\" value: %s.", input->Get("relative_to")->ToJSONString().Latin1().c_str()));
                    return nullptr;
                }
                ...
                SetParseErrorMessage(out_error, String::Format("Unrecognized key found: \"%s\".", key.Latin1().c_str()));
                return nullptr;
            }
        }
        ...
        for (JSONValue* raw_pattern : raw_patterns) {
            URLPattern* pattern = ParseRawPattern(execution_context->GetIsolate(), raw_pattern, base_url, IGNORE_EXCEPTION, out_error);
            // If those steps throw, `pattern` will be null. Ignore the exception and
            // return null.
            if (!pattern) {
                SetParseErrorMessage(
                    out_error, String::Format("URL Pattern for \"href_matches\" could not be parsed: %s.", raw_pattern->ToJSONString().Latin1().c_str()));
                return nullptr;
            }
        ...
        return MakeGarbageCollected<URLPatternPredicate>(std::move(patterns), execution_context);
    }
    ...
    if (predicate_type == "selector_matches" && input->size() == 1) {
        ...
        CSSPropertyValueSet* empty_properties = ImmutableCSSPropertyValueSet::Create(base::span<CSSPropertyValue>(), kUASheetMode);
        CSSParserContext* css_parser_context = MakeGarbageCollected<CSSParserContext>(*execution_context);
        for (auto* raw_selector : raw_selectors) {
            ...
            base::span<CSSSelector> selector_vector = CSSParser::ParseSelector(css_parser_context, CSSNestingType::kNone,
                /*parent_rule_for_nesting=*/nullptr, /*is_within_scope=*/false, nullptr, raw_selector_string, arena);
            if (selector_vector.empty()) {
                SetParseErrorMessage(out_error, String::Format("\"%s\" is not a valid selector.", raw_selector_string.Latin1().c_str()));
                return nullptr;
            }
            StyleRule* selector = StyleRule::Create(selector_vector, empty_properties);
            // Append selector to selectors.
            selectors.push_back(std::move(selector));
        }
        UseCounter::Count(execution_context, WebFeature::kSpeculationRulesSelectorMatches);
...
{
    return MakeGarbageCollected<Conjunction>(HeapVector<Member<DocumentRulePredicate>>());
}

HeapVector<Member<DocumentRulePredicate>> DocumentRulePredicate::GetSubPredicatesForTesting() const
{
    NOTREACHED();
}

HeapVector<Member<URLPattern>> DocumentRulePredicate::GetURLPatternsForTesting() const
{
    NOTREACHED();
}

HeapVector<Member<StyleRule>> DocumentRulePredicate::GetStyleRulesForTesting() const
{
    NOTREACHED();
}
