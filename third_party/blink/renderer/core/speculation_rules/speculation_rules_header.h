...
public:
    ...
    CORE_EXPORT static void ProcessHeadersForDocumentResponse(const ResourceResponse&, LocalDOMWindow&);

private:
    SpeculationRulesHeader();
    ~SpeculationRulesHeader();

    void ParseSpeculationRulesHeader(const String& header_value, const KURL& base_url);
