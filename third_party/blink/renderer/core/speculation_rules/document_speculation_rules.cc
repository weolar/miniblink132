...
bool AcceptableReferrerPolicy(const Referrer& referrer, bool is_initially_same_site)
{
...
String MakeReferrerWarning(mojom::blink::SpeculationAction action, const KURL& url, const Referrer& referrer, bool has_link)
{
    const String action_string = SpeculationActionAsString(action);

    const String suggested_fix = has_link ? "A stricter referrer policy may be set using the matched "
                                            "link's \"referrerpolicy\" attribute, or it may be set "
                                            "specifically for the "
            + action_string
            + " request using the \"referrer_policy\" key in the "
              "speculation rule."
                                          : "A stricter referrer policy may be set for this specific " + action_string
            + " request using the \"referrer_policy\" key in the "
              "speculation rule.";
    constexpr auto kExampleAcceptablePolicy = network::mojom::ReferrerPolicy::kStrictOriginWhenCrossOrigin;

    return "Ignored attempt to " + action_string + " " + url.ElidedString() + " due to unacceptable referrer policy ("
        + SecurityPolicy::ReferrerPolicyAsString(referrer.referrer_policy) + "). " + suggested_fix + " For example, the policy \""
        + SecurityPolicy::ReferrerPolicyAsString(kExampleAcceptablePolicy) + "\" is sufficiently strict.";
}
...
std::optional<Referrer> GetReferrer(const SpeculationRule* rule, const SpeculationRuleSet& rule_set, Document& document, mojom::blink::SpeculationAction action,
    HTMLAnchorElementBase* link, std::optional<KURL> opt_url)
{
    ...
    if (rule->referrer_policy()) {
        ...
        UseCounter::Count(document, WebFeature::kSpeculationRulesUsedLinkReferrerPolicy);
        ...
        bool valid = SecurityPolicy::ReferrerPolicyFromString(
            link->FastGetAttribute(html_names::kReferrerpolicyAttr), kSupportReferrerPolicyLegacyKeywords, &referrer_policy);
        if (valid) {
            UseCounter::Count(document, WebFeature::kSpeculationRulesUsedLinkReferrerPolicy);
        } else {
    ...
    const bool is_initially_same_site = url_origin->IsSameSiteWith(execution_context->GetSecurityOrigin());
    Referrer referrer = SecurityPolicy::GenerateReferrer(referrer_policy, url, outgoing_referrer);

    if (!AcceptableReferrerPolicy(referrer, is_initially_same_site)) {
        auto* console_message = MakeGarbageCollected<ConsoleMessage>(
            mojom::blink::ConsoleMessageSource::kOther, mojom::blink::ConsoleMessageLevel::kWarning, MakeReferrerWarning(action, url, referrer, link));
        ...
        UseCounter::Count(document, WebFeature::kSpeculationRulesRejectedLaxReferrerPolicy);
        return std::nullopt;
    }
...
std::ostream& operator<<(std::ostream& o, const DocumentSpeculationRules::PendingUpdateState& s)
{
    return o << static_cast<unsigned>(s);
}

// static
const char DocumentSpeculationRules::kSupplementName[] = "DocumentSpeculationRules";
...
// static
DocumentSpeculationRules* DocumentSpeculationRules::FromIfExists(Document& document)
{
    ...
    , host_(document.GetExecutionContext())
{
    ...
    lcpp->AddLCPPredictedCallback(WTF::BindOnce(&DocumentSpeculationRules::OnLCPPredicted, WrapPersistent(this)));
}
...
void DocumentSpeculationRules::AddRuleSet(SpeculationRuleSet* rule_set)
{
    ...
    if (rule_set->ShouldReportUMAForError()) {
        ...
        for (const auto& other_rule_set : rule_sets_) {
            if (!other_rule_set->source()->IsFromBrowserInjected()) {
                CountSpeculationRulesLoadOutcome(SpeculationRulesLoadOutcome::kAutoSpeculationRulesOptedOut);
                UseCounter::Count(GetSupplementable(), WebFeature::kAutoSpeculationRulesOptedOut);
                return;
            }
        }
    }
    ...
    if (rule_set->has_document_rule()) {
        UseCounter::Count(GetSupplementable(), WebFeature::kSpeculationRulesDocumentRules);
    ...
    if (!wants_pointer_events_ && rule_set->requires_unfiltered_input()) {
        ...
        if (auto* frame = document.GetFrame()) {
            frame->GetEventHandlerRegistry().DidAddEventHandler(document, EventHandlerRegistry::kPointerEvent);
        }
    }
    ...
    if (rule_set->prefetch_rules().size()) {
        UseCounter::Count(GetSupplementable(),
            rule_set->source()->IsFromBrowserInjected() ? WebFeature::kSpeculationRulesBrowserPrefetchRule : WebFeature::kSpeculationRulesAuthorPrefetchRule);
    }
    if (rule_set->prerender_rules().size()) {
        UseCounter::Count(GetSupplementable(),
            rule_set->source()->IsFromBrowserInjected() ? WebFeature::kSpeculationRulesBrowserPrerenderRule : WebFeature::kSpeculationRulesAuthorPrerenderRule);
    }
...
void DocumentSpeculationRules::RemoveRuleSet(SpeculationRuleSet* rule_set)
{
    ...
    CHECK(it != rule_sets_.end(), base::NotFatalUntil::M130) << "rule set was removed without existing";
    ...
    if (wants_pointer_events_ && rule_set->requires_unfiltered_input() && base::ranges::none_of(rule_sets_, &SpeculationRuleSet::requires_unfiltered_input)) {
        wants_pointer_events_ = false;
        Document& document = *GetSupplementable();
        if (auto* frame = document.GetFrame()) {
            frame->GetEventHandlerRegistry().DidRemoveEventHandler(document, EventHandlerRegistry::kPointerEvent);
        }
    }
...
void DocumentSpeculationRules::AddSpeculationRuleLoader(SpeculationRuleLoader* speculation_rule_loader)
{
    speculation_rule_loaders_.insert(speculation_rule_loader);
}

void DocumentSpeculationRules::RemoveSpeculationRuleLoader(SpeculationRuleLoader* speculation_rule_loader)
{
...
void DocumentSpeculationRules::HrefAttributeChanged(HTMLAnchorElementBase* link, const AtomicString& old_value, const AtomicString& new_value)
{
    if (!initialized_)
        return;
...
void DocumentSpeculationRules::ReferrerPolicyAttributeChanged(HTMLAnchorElementBase* link)
{
    LinkAttributeChanged(link);
}

void DocumentSpeculationRules::RelAttributeChanged(HTMLAnchorElementBase* link)
{
    LinkAttributeChanged(link);
}

void DocumentSpeculationRules::TargetAttributeChanged(HTMLAnchorElementBase* link)
{
    LinkAttributeChanged(link);
}
...
void DocumentSpeculationRules::LinkMatchedSelectorsUpdated(HTMLAnchorElementBase* link)
{
    DCHECK(initialized_);
    InvalidateLink(link);
    QueueUpdateSpeculationCandidates();
...
void DocumentSpeculationRules::LinkGainedOrLostComputedStyle(HTMLAnchorElementBase* link)
{
    if (!initialized_) {
        return;
    }
...
void DocumentSpeculationRules::ChildStyleRecalcBlocked(Element* root)
{
    ...
    while (node) {
        if (node->IsLink() && (node->HasTagName(html_names::kATag) || node->HasTagName(html_names::kAreaTag))) {
        ...
        if (auto* element = DynamicTo<Element>(node); element && elements_blocking_child_style_recalc_.Contains(element)) {
...
void DocumentSpeculationRules::QueueUpdateSpeculationCandidates(bool force_style_update)
{
    const bool microtask_already_queued = IsMicrotaskQueued();

    bool needs_microtask = true;
    if (force_style_update) {
        SetPendingUpdateState(PendingUpdateState::kMicrotaskQueuedWithForcedStyleUpdate);
    ...
    if (needs_microtask && !microtask_already_queued && execution_context) {
        execution_context->GetAgent()->event_loop()->EnqueueMicrotask(
            WTF::BindOnce(&DocumentSpeculationRules::UpdateSpeculationCandidatesMicrotask, WrapWeakPersistent(this)));
    }
}
...
mojom::blink::SpeculationHost* DocumentSpeculationRules::GetHost()
{
    if (!host_.is_bound()) {
        ...
        execution_context->GetBrowserInterfaceBroker().GetInterface(
            host_.BindNewPipeAndPassReceiver(execution_context->GetTaskRunner(TaskType::kInternalDefault)));
    }
    return host_.get();
}
...
void DocumentSpeculationRules::UpdateSpeculationCandidatesMicrotask()
{
    ...
    if (document.NeedsLayoutTreeUpdate()) {
        if (pending_update_state_ == PendingUpdateState::kMicrotaskQueuedWithForcedStyleUpdate) {
...
void DocumentSpeculationRules::UpdateSpeculationCandidates()
{
    ...
    auto push_candidates
        = [&candidates, &document](mojom::blink::SpeculationAction action, SpeculationRuleSet* rule_set, const HeapVector<Member<SpeculationRule>>& rules) {
              for (SpeculationRule* rule : rules) {
                  for (const KURL& url : rule->urls()) {
                      std::optional<Referrer> referrer = GetReferrer(rule, *rule_set, document, action, /*link=*/nullptr, url);
                      if (!referrer)
                          continue;

                      // Ensured by `SpeculationRuleSet`.
                      CHECK(!rule->target_browsing_context_name_hint() || action == mojom::blink::SpeculationAction::kPrerender);
                      CHECK(!rule->requires_anonymous_client_ip_when_cross_origin() || action == mojom::blink::SpeculationAction::kPrefetch);

                      candidates.push_back(
                          MakeGarbageCollected<SpeculationCandidate>(url, action, referrer.value(), rule->requires_anonymous_client_ip_when_cross_origin(),
                              rule->target_browsing_context_name_hint().value_or(mojom::blink::SpeculationTargetHint::kNoHint), rule->eagerness(),
                              rule->no_vary_search_expected().Clone(), rule->injection_type(), rule_set, /*anchor=*/nullptr));
                  }
              }
          };

    for (SpeculationRuleSet* rule_set : rule_sets_) {
        push_candidates(mojom::blink::SpeculationAction::kPrefetch, rule_set, rule_set->prefetch_rules());

        if (RuntimeEnabledFeatures::SpeculationRulesPrefetchWithSubresourcesEnabled(execution_context)) {
            push_candidates(mojom::blink::SpeculationAction::kPrefetchWithSubresources, rule_set, rule_set->prefetch_with_subresources_rules());
        }
        ...
        if (RuntimeEnabledFeatures::Prerender2Enabled(execution_context)) {
            push_candidates(mojom::blink::SpeculationAction::kPrerender, rule_set, rule_set->prerender_rules());
    ...
    auto last = base::ranges::remove_if(candidates, [&](const auto& candidate) {
        ...
        return url.HasFragmentIdentifier() && EqualIgnoringFragmentIdentifier(url, document_url);
    });
    ...
    base::EnumSet<SpeculationEagerness, SpeculationEagerness::kMinValue, SpeculationEagerness::kMaxValue> eagerness_set;
    ...
    if (eagerness_set.Has(SpeculationEagerness::kConservative)) {
        UseCounter::Count(document, WebFeature::kSpeculationRulesEagernessConservative);
    }
    ...
    base::UmaHistogramEnumeration("Preloading.Experimental.UpdateSpeculationCandidatesReason",
        first_update_after_restored_from_bfcache_ ? UpdateSpeculationCandidatesReason::kRestoredFromBFCache : UpdateSpeculationCandidatesReason::kOther);

    first_update_after_restored_from_bfcache_ = false;
}

void DocumentSpeculationRules::AddLinkBasedSpeculationCandidates(HeapVector<Member<SpeculationCandidate>>& candidates)
{
    // Match all the unmatched
    while (!pending_links_.empty()) {
        auto it = pending_links_.begin();
        HTMLAnchorElementBase* link = *it;
        HeapVector<Member<SpeculationCandidate>>* link_candidates = MakeGarbageCollected<HeapVector<Member<SpeculationCandidate>>>();
        Document& document = *GetSupplementable();
        ExecutionContext* execution_context = document.GetExecutionContext();
        CHECK(execution_context);

        const auto push_link_candidates = [&link, &link_candidates, &document, this](mojom::blink::SpeculationAction action, SpeculationRuleSet* rule_set,
            ...
            for (SpeculationRule* rule : speculation_rules) {
                ...
                std::optional<Referrer> referrer = GetReferrer(rule, *rule_set, document, action, link,
                    /*opt_url=*/std::nullopt);
                if (!referrer)
                    continue;

                mojom::blink::SpeculationTargetHint target_hint = mojom::blink::SpeculationTargetHint::kNoHint;
                if (action == mojom::blink::SpeculationAction::kPrerender) {
                    if (rule->target_browsing_context_name_hint()) {
                        ...
                        target_hint = SpeculationRuleSet::SpeculationTargetHintFromString(link->GetEffectiveTarget());
                    }
                }

                SpeculationCandidate* candidate = MakeGarbageCollected<SpeculationCandidate>(link->HrefURL(), action, referrer.value(),
                    rule->requires_anonymous_client_ip_when_cross_origin(), target_hint, rule->eagerness(), rule->no_vary_search_expected().Clone(),
        ...
        for (SpeculationRuleSet* rule_set : rule_sets_) {
            push_link_candidates(mojom::blink::SpeculationAction::kPrefetch, rule_set, rule_set->prefetch_rules());

            if (RuntimeEnabledFeatures::SpeculationRulesPrefetchWithSubresourcesEnabled(execution_context)) {
                push_link_candidates(mojom::blink::SpeculationAction::kPrefetchWithSubresources, rule_set, rule_set->prefetch_with_subresources_rules());
            }

            if (RuntimeEnabledFeatures::Prerender2Enabled(execution_context)) {
                push_link_candidates(mojom::blink::SpeculationAction::kPrerender, rule_set, rule_set->prerender_rules());
            }
        }
...
void DocumentSpeculationRules::InitializeIfNecessary()
{
    if (initialized_)
        return;
    initialized_ = true;
    for (Node& node : ShadowIncludingTreeOrderTraversal::DescendantsOf(*GetSupplementable())) {
        if (!node.IsLink())
            continue;
...
void DocumentSpeculationRules::LinkAttributeChanged(HTMLAnchorElementBase* link)
{
    if (!initialized_) {
        return;
    }
...
void DocumentSpeculationRules::SetPendingUpdateState(PendingUpdateState new_state)
{
#if DCHECK_IS_ON()
    // TODO(jbroman): This could use "using enum" once that's allowed.
    using S = PendingUpdateState;
    DEFINE_STATIC_LOCAL(base::StateTransitions<S>, transitions,
        ({
            // When there is no update, we can only queue an update.
            { S::kNoUpdate, { S::kMicrotaskQueued, S::kMicrotaskQueuedWithForcedStyleUpdate } },
            // When an update is queued, it can complete, get upgraded to forcing
            // style, or need to wait for style (lazily).
            { S::kMicrotaskQueued, { S::kNoUpdate, S::kMicrotaskQueuedWithForcedStyleUpdate, S::kOnNextStyleUpdate } },
            ...
            { S::kOnNextStyleUpdate, { S::kNoUpdate, S::kMicrotaskQueuedWithForcedStyleUpdate } },
