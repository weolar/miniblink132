...
protected:
    void ExpectNoFrameworkSpeculationRules(const AutoSpeculationRulesConfig& config)
    {
        for (auto i = base::to_underlying(mojom::JavaScriptFramework::kMinValue); i <= base::to_underlying(mojom::JavaScriptFramework::kMaxValue); ++i) {
...
TEST_F(AutoSpeculationRulesConfigTest, ValidFrameworkToSpeculationRules)
{
    AutoSpeculationRulesConfig config(R"(
  {
    ...
    EXPECT_TRUE(config.ForFramework(mojom::JavaScriptFramework::kNuxt /* = 0 */).IsNull());
    EXPECT_EQ(config.ForFramework(mojom::JavaScriptFramework::kVuePress /* = 1 */), "speculation_rules_1");
    EXPECT_TRUE(config.ForFramework(mojom::JavaScriptFramework::kSapper /* = 2 */).IsNull());
    EXPECT_EQ(config.ForFramework(mojom::JavaScriptFramework::kGatsby /* = 1 */), "speculation_rules_3");
}
...
{
    static_assert(base::to_underlying(mojom::JavaScriptFramework::kMaxValue) < 999);
    ...
    AutoSpeculationRulesConfig config(R"(
  {
    ...
    EXPECT_EQ(config.ForFramework(mojom::JavaScriptFramework::kVuePress /* = 1 */), "speculation_rules_1");
    EXPECT_TRUE(config.ForFramework(static_cast<mojom::JavaScriptFramework>(999)).IsNull());
}
...
TEST_F(AutoSpeculationRulesConfigTest, ValidUrlMatchPattern)
{
    AutoSpeculationRulesConfig config(R"(
  {
    ...
    EXPECT_THAT(config.ForUrl(KURL("https://example.com/")),
        ElementsAre(std::make_pair("speculation_rules_1", BrowserInjectedSpeculationRuleOptOut::kRespect),
            std::make_pair("speculation_rules_5", BrowserInjectedSpeculationRuleOptOut::kRespect)));
    ...
    EXPECT_THAT(config.ForUrl(KURL("https://other.example.com/path")),
        ElementsAre(std::make_pair("speculation_rules_2", BrowserInjectedSpeculationRuleOptOut::kRespect),
            std::make_pair("speculation_rules_4", BrowserInjectedSpeculationRuleOptOut::kRespect)));

    EXPECT_THAT(config.ForUrl(KURL("https://example.org/")), ElementsAre());

    EXPECT_THAT(config.ForUrl(KURL("https://www.example.org/path")),
        ElementsAre(std::make_pair("speculation_rules_3", BrowserInjectedSpeculationRuleOptOut::kRespect),
            std::make_pair("speculation_rules_4", BrowserInjectedSpeculationRuleOptOut::kRespect)));

    EXPECT_THAT(config.ForUrl(KURL("https://example.co/")), ElementsAre(std::make_pair("speculation_rules_5", BrowserInjectedSpeculationRuleOptOut::kRespect)));

    EXPECT_THAT(
        config.ForUrl(KURL("https://www.example.xyz/")), ElementsAre(std::make_pair("speculation_rules_4", BrowserInjectedSpeculationRuleOptOut::kRespect)));
}
...
TEST_F(AutoSpeculationRulesConfigTest, NonStringUrlMatchPatternSpeculationRules)
{
    AutoSpeculationRulesConfig config(R"(
  {
    "url_match_pattern_to_speculation_rules": {
...
TEST_F(AutoSpeculationRulesConfigTest, NonObjectFrameworkValidUrlMatchPatterns)
{
    AutoSpeculationRulesConfig config(R"(
  {
    "framework_to_speculation_rules": true,
    ...
    ExpectNoFrameworkSpeculationRules(config);
    EXPECT_THAT(
        config.ForUrl(KURL("https://example.com/")), ElementsAre(std::make_pair("speculation_rules_1", BrowserInjectedSpeculationRuleOptOut::kRespect)));
}

TEST_F(AutoSpeculationRulesConfigTest, ValidFrameworkNonObjectUrlMatchPatterns)
{
    AutoSpeculationRulesConfig config(R"(
  {
    "framework_to_speculation_rules": {
...
TEST_F(AutoSpeculationRulesConfigTest, ValidFrameworkValidUrlMatchPatterns)
{
    AutoSpeculationRulesConfig config(R"(
  {
    ...
    EXPECT_EQ(config.ForFramework(mojom::JavaScriptFramework::kVuePress /* = 1 */), "speculation_rules_1");
    EXPECT_THAT(
        config.ForUrl(KURL("https://example.com/")), ElementsAre(std::make_pair("speculation_rules_2", BrowserInjectedSpeculationRuleOptOut::kRespect)));
}
...
TEST_F(AutoSpeculationRulesConfigTest, ValidUrlMatchPatternsIgnoreOptOut)
{
    AutoSpeculationRulesConfig config(R"(
  {
    ...
    EXPECT_THAT(config.ForUrl(KURL("https://example.com/")), ElementsAre(std::make_pair("speculation_rules_2", BrowserInjectedSpeculationRuleOptOut::kIgnore)));
}
