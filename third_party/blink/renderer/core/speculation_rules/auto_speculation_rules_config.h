...
public:
    ...
    Vector<std::pair<String, BrowserInjectedSpeculationRuleOptOut>> ForUrl(const KURL&) const;

private:
    void ParseUrlMatchPatternConfig(const JSONObject* config, const String& json_key_name, BrowserInjectedSpeculationRuleOptOut opt_out);
    ...
    static AutoSpeculationRulesConfig* OverrideInstanceForTesting(AutoSpeculationRulesConfig* new_override);

    Vector<std::pair<mojom::JavaScriptFramework, String>> framework_to_speculation_rules_;
    ...
    // at test time.
    Vector<std::pair<std::string, std::pair<String, BrowserInjectedSpeculationRuleOptOut>>> url_match_pattern_to_speculation_rules_;
};
