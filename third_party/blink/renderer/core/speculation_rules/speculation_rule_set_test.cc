...
public:
    ...
    bool MatchAndExplain(const Member<SpeculationRule>& rule, ::testing::MatchResultListener* listener) const
    {
        return MatchAndExplain(*rule, listener);
    }

    bool MatchAndExplain(const SpeculationRule& rule, ::testing::MatchResultListener* listener) const
    {
        ::testing::StringMatchResultListener inner_listener;
        const bool matches = url_matcher_.MatchAndExplain(rule.urls(), &inner_listener);
...
public:
    explicit URLPatternMatcher(v8::Isolate* isolate, String pattern, const KURL& base_url)
    {
        auto* url_pattern_input = MakeGarbageCollected<V8URLPatternInput>(pattern);
        url_pattern_ = URLPattern::Create(isolate, url_pattern_input, base_url, ASSERT_NO_EXCEPTION);
    }

    bool MatchAndExplain(URLPattern* pattern, ::testing::MatchResultListener* listener) const
    {
        if (!pattern) {
            return false;
        }
        ...
        Component components[] = { Component::kProtocol, Component::kUsername, Component::kPassword, Component::kHostname, Component::kPort,
            Component::kPathname, Component::kSearch, Component::kHash };
        for (auto component : components) {
            if (URLPattern::compareComponent(V8URLPatternComponent(component), url_pattern_, pattern) != 0) {
                return false;
            }
        }
        return true;
...
template <typename... Matchers> auto MatchesListOfURLs(Matchers&&... matchers)
{
    return ::testing::MakePolymorphicMatcher(ListRuleMatcher(ElementsAre(std::forward<Matchers>(matchers)...)));
}

MATCHER(RequiresAnonymousClientIPWhenCrossOrigin,
    negation ? "doesn't require anonymous client IP when cross origin" : "requires anonymous client IP when cross origin")
{
    return arg->requires_anonymous_client_ip_when_cross_origin();
}

MATCHER(SetsReferrerPolicy, std::string(negation ? "doesn't set" : "sets") + " a referrer policy")
{
    return arg->referrer_policy().has_value();
}

MATCHER_P(ReferrerPolicyIs, policy, std::string(negation ? "doesn't have" : "has") + " " + PrintToString(policy) + " as the referrer policy")
{
...
public:
    ...
    SpeculationRuleSet* CreateRuleSet(const String& source_text, const KURL& base_url, ExecutionContext* context)
    {
        return SpeculationRuleSet::Parse(SpeculationRuleSet::Source::FromRequest(source_text, base_url,
                                             /* request_id */ 0),
            context);
    }

    SpeculationRuleSet* CreateSpeculationRuleSetWithTargetHint(const char* target_hint)
    {
    ...
    auto URLPattern(String pattern, const KURL& base_url = KURL("https://example.com/"))
    {
        return ::testing::MakePolymorphicMatcher(URLPatternMatcher(execution_context_->GetIsolate(), pattern, base_url));
    }
...
template <typename... Matchers> auto HasURLs(Matchers&&... urls)
{
    return ::testing::ResultOf(
        "urls",
        [](const auto& candidates) {
            Vector<KURL> urls;
            base::ranges::transform(candidates.begin(), candidates.end(), std::back_inserter(urls), [](const auto& candidate) { return candidate->url; });
            return urls;
        },
...
auto HasEagerness(::testing::Matcher<blink::mojom::SpeculationEagerness> matcher)
{
    return ::testing::Pointee(::testing::Field("eagerness", &mojom::blink::SpeculationCandidate::eagerness, matcher));
}

// Matches a SpeculationCandidatePtr with a KURL.
auto HasURL(::testing::Matcher<KURL> matcher)
{
    return ::testing::Pointee(::testing::Field("url", &mojom::blink::SpeculationCandidate::url, matcher));
}
...
{
    return ::testing::Pointee(::testing::Field("action", &mojom::blink::SpeculationCandidate::action, matcher));
}

// Matches a SpeculationCandidatePtr with a SpeculationTargetHint.
auto HasTargetHint(::testing::Matcher<mojom::blink::SpeculationTargetHint> matcher)
{
    return ::testing::Pointee(::testing::Field("target_hint", &mojom::blink::SpeculationCandidate::target_browsing_context_name_hint, matcher));
}

// Matches a SpeculationCandidatePtr with a ReferrerPolicy.
auto HasReferrerPolicy(::testing::Matcher<network::mojom::ReferrerPolicy> matcher)
{
    return ::testing::Pointee(::testing::Field(
        "referrer", &mojom::blink::SpeculationCandidate::referrer, ::testing::Pointee(::testing::Field("policy", &mojom::blink::Referrer::policy, matcher))));
}

auto HasNoVarySearchHint()
{
    return ::testing::Pointee(::testing::Field("no_vary_search_hint", &mojom::blink::SpeculationCandidate::no_vary_search_hint, ::testing::IsTrue()));
}

auto NVSVariesOnKeyOrder()
{
    return ::testing::AllOf(HasNoVarySearchHint(),
        ::testing::Pointee(::testing::Field("no_vary_search_hint", &mojom::blink::SpeculationCandidate::no_vary_search_hint,
            testing::Pointee(::testing::Field("vary_on_key_order", &network::mojom::blink::NoVarySearch::vary_on_key_order, ::testing::IsTrue())))));
}

template <typename... Matchers> auto NVSHasNoVaryParams(Matchers&&... params)
{
    return ::testing::ResultOf(
        "no_vary_params",
        [](const auto& nvs) {
            if (!nvs->no_vary_search_hint || !nvs->no_vary_search_hint->search_variance || !nvs->no_vary_search_hint->search_variance->is_no_vary_params()) {
...
{
    auto* rule_set = CreateRuleSet("{}", KURL("https://example.com/"), execution_context());
...
{
    EXPECT_EQ(rule_set->error_type(), SpeculationRuleSetErrorType::kSourceIsNotJsonObject);
    EXPECT_THAT(rule_set->prefetch_rules(), ElementsAre());
    EXPECT_THAT(rule_set->prefetch_with_subresources_rules(), ElementsAre());
...
{
    auto* rule_set = CreateRuleSet("[invalid]", KURL("https://example.com"), execution_context());
    ASSERT_TRUE(rule_set);
    AssertParseError(rule_set);
    EXPECT_TRUE(rule_set->error_message().Contains("Syntax error")) << rule_set->error_message();
}

TEST_F(SpeculationRuleSetTest, RejectsNonObject)
{
    auto* rule_set = CreateRuleSet("42", KURL("https://example.com"), execution_context());
    ASSERT_TRUE(rule_set);
    AssertParseError(rule_set);
    EXPECT_TRUE(rule_set->error_message().Contains("must be an object")) << rule_set->error_message();
}

TEST_F(SpeculationRuleSetTest, RejectsComments)
{
    auto* rule_set = CreateRuleSet("{ /* comments! */ }", KURL("https://example.com/"), execution_context());
    ASSERT_TRUE(rule_set);
    AssertParseError(rule_set);
    EXPECT_TRUE(rule_set->error_message().Contains("Syntax error")) << rule_set->error_message();
}
...
TEST_F(SpeculationRuleSetTest, SimplePrefetchRule)
{
    ...
    ASSERT_TRUE(rule_set);
    EXPECT_EQ(rule_set->error_type(), SpeculationRuleSetErrorType::kNoError);
    EXPECT_THAT(rule_set->prefetch_rules(), ElementsAre(MatchesListOfURLs("https://example.com/index2.html")));
    EXPECT_THAT(rule_set->prefetch_with_subresources_rules(), ElementsAre());
    EXPECT_THAT(rule_set->prerender_rules(), ElementsAre());
}
...
TEST_F(SpeculationRuleSetTest, SimplePrerenderRule)
{
    ...
    ASSERT_TRUE(rule_set);
    EXPECT_EQ(rule_set->error_type(), SpeculationRuleSetErrorType::kNoError);
    EXPECT_THAT(rule_set->prerender_rules(), ElementsAre(MatchesListOfURLs("https://example.com/index2.html")));
    EXPECT_THAT(rule_set->prefetch_rules(), ElementsAre());
    EXPECT_THAT(rule_set->prefetch_with_subresources_rules(), ElementsAre());
}
...
TEST_F(SpeculationRuleSetTest, SimplePrefetchWithSubresourcesRule)
{
    ...
    EXPECT_EQ(rule_set->error_type(), SpeculationRuleSetErrorType::kNoError);
    EXPECT_THAT(rule_set->prefetch_rules(), ElementsAre());
    EXPECT_THAT(rule_set->prefetch_with_subresources_rules(), ElementsAre(MatchesListOfURLs("https://example.com/index2.html")));
    EXPECT_THAT(rule_set->prerender_rules(), ElementsAre());
}
...
TEST_F(SpeculationRuleSetTest, ResolvesURLs)
{
    ...
    EXPECT_THAT(rule_set->prefetch_rules(),
        ElementsAre(MatchesListOfURLs("https://example.com/foo/bar", "https://example.com/baz", "https://example.org/", "http://example.net/")));
}
...
TEST_F(SpeculationRuleSetTest, ResolvesURLsWithRelativeTo)
{
    ...
    EXPECT_EQ(rule_set->error_type(), SpeculationRuleSetErrorType::kNoError);
    EXPECT_THAT(rule_set->prefetch_rules(),
        ElementsAre(MatchesListOfURLs("https://document.com/foo/bar", "https://document.com/baz", "https://example.org/", "http://example.net/")));
}
...
TEST_F(SpeculationRuleSetTest, RequiresAnonymousClientIPWhenCrossOrigin)
{
    ...
    EXPECT_EQ(rule_set->error_type(), SpeculationRuleSetErrorType::kNoError);
    EXPECT_THAT(rule_set->prefetch_rules(),
        ElementsAre(AllOf(MatchesListOfURLs("https://example.net/anonymous.html"), RequiresAnonymousClientIPWhenCrossOrigin()),
            AllOf(MatchesListOfURLs("https://example.net/direct.html"), Not(RequiresAnonymousClientIPWhenCrossOrigin()))));
}
...
TEST_F(SpeculationRuleSetTest, IgnoresUnknownOrDifferentlyTypedTopLevelKeys)
{
    auto* rule_set = CreateRuleSet(
        ...
        KURL("https://example.com/"), execution_context());
    ASSERT_TRUE(rule_set);
    EXPECT_EQ(rule_set->error_type(), SpeculationRuleSetErrorType::kInvalidRulesSkipped);
    EXPECT_THAT(rule_set->prefetch_rules(), ElementsAre());
    EXPECT_THAT(rule_set->prefetch_with_subresources_rules(), ElementsAre());
}
...
{
    auto* rule_set = CreateRuleSet(R"({"prefetch": [)"
    ...
    EXPECT_THAT(
        rule_set->prefetch_rules(), ElementsAre(MatchesListOfURLs("https://example.com/no-source.html"), MatchesListOfURLs("https://example.com/valid.html")));
}
...
TEST_F(SpeculationRuleSetTest, RulesWithTargetHint_Blank)
{
    ...
    ASSERT_TRUE(rule_set);
    EXPECT_EQ(rule_set->error_type(), SpeculationRuleSetErrorType::kInvalidRulesSkipped);
    EXPECT_TRUE(rule_set->error_message().Contains("\"target_hint\" may not be set for prefetch")) << rule_set->error_message();
    EXPECT_THAT(rule_set->prefetch_rules(), ElementsAre());
    EXPECT_THAT(rule_set->prefetch_with_subresources_rules(), ElementsAre());
    EXPECT_THAT(rule_set->prerender_rules(), ElementsAre(MatchesListOfURLs("https://example.com/hint.html")));
    EXPECT_EQ(rule_set->prerender_rules()[0]->target_browsing_context_name_hint(), mojom::blink::SpeculationTargetHint::kBlank);
}
...
TEST_F(SpeculationRuleSetTest, RulesWithTargetHint_Self)
{
    ...
    EXPECT_THAT(rule_set->prefetch_with_subresources_rules(), ElementsAre());
    EXPECT_THAT(rule_set->prerender_rules(), ElementsAre(MatchesListOfURLs("https://example.com/hint.html")));
    EXPECT_EQ(rule_set->prerender_rules()[0]->target_browsing_context_name_hint(), mojom::blink::SpeculationTargetHint::kSelf);
}
...
TEST_F(SpeculationRuleSetTest, RulesWithTargetHint_Parent)
{
    ...
    EXPECT_THAT(rule_set->prefetch_with_subresources_rules(), ElementsAre());
    EXPECT_THAT(rule_set->prerender_rules(), ElementsAre(MatchesListOfURLs("https://example.com/hint.html")));
    EXPECT_EQ(rule_set->prerender_rules()[0]->target_browsing_context_name_hint(), mojom::blink::SpeculationTargetHint::kNoHint);
}
...
TEST_F(SpeculationRuleSetTest, RulesWithTargetHint_EmptyString)
{
    ...
    ASSERT_TRUE(rule_set);
    EXPECT_EQ(rule_set->error_type(), SpeculationRuleSetErrorType::kInvalidRulesSkipped);
    EXPECT_TRUE(rule_set->error_message().Contains("invalid \"target_hint\"")) << rule_set->error_message();
    EXPECT_THAT(rule_set->prefetch_rules(), ElementsAre());
    EXPECT_THAT(rule_set->prefetch_with_subresources_rules(), ElementsAre());
    EXPECT_THAT(rule_set->prerender_rules(), ElementsAre());
}
...
// requirement.
TEST_F(SpeculationRuleSetTest, RulesWithRequiresAnonymousClientIpWhenCrossOrigin)
{
    auto* rule_set = CreateRuleSet(R"({
        "prefetch": [{
          "source": "list",
    ...
    EXPECT_THAT(rule_set->prefetch_rules(), ElementsAre(MatchesListOfURLs("https://example.com/requires-proxy.html")));
    EXPECT_TRUE(rule_set->prefetch_rules()[0]->requires_anonymous_client_ip_when_cross_origin());
    EXPECT_THAT(rule_set->prefetch_with_subresources_rules(), ElementsAre());
    EXPECT_THAT(rule_set->prerender_rules(), ElementsAre());
}
...
TEST_F(SpeculationRuleSetTest, ReferrerPolicy)
{
    ...
    EXPECT_EQ(rule_set->error_type(), SpeculationRuleSetErrorType::kNoError);
    EXPECT_THAT(rule_set->prefetch_rules(),
        ElementsAre(AllOf(MatchesListOfURLs("https://example.com/index2.html"), ReferrerPolicyIs(network::mojom::ReferrerPolicy::kStrictOrigin)),
            AllOf(MatchesListOfURLs("https://example.com/index3.html"), Not(SetsReferrerPolicy()))));
}
...
TEST_F(SpeculationRuleSetTest, EmptyReferrerPolicy)
{
    ...
    ASSERT_TRUE(rule_set);
    EXPECT_EQ(rule_set->error_type(), SpeculationRuleSetErrorType::kNoError);
    EXPECT_THAT(rule_set->prefetch_rules(), ElementsAre(AllOf(MatchesListOfURLs("https://example.com/index2.html"), Not(SetsReferrerPolicy()))));
}
...
TEST_F(SpeculationRuleSetTest, PropagatesToDocument)
{
    ...
    HTMLScriptElement* script = MakeGarbageCollected<HTMLScriptElement>(document, CreateElementFlags());
    ...
    EXPECT_THAT(rule_set->prefetch_rules(), ElementsAre(MatchesListOfURLs("https://example.com/foo")));
    EXPECT_THAT(rule_set->prerender_rules(), ElementsAre(MatchesListOfURLs("https://example.com/bar")));
}

HTMLScriptElement* InsertSpeculationRules(Document& document, const String& speculation_script)
{
    HTMLScriptElement* script = MakeGarbageCollected<HTMLScriptElement>(document, CreateElementFlags());
    script->setAttribute(html_names::kTypeAttr, AtomicString("SpEcUlAtIoNrUlEs"));
...
using IncludesStyleUpdate = base::StrongAlias<class IncludesStyleUpdateTag, bool>;
...
template <typename F>
void PropagateRulesToStubSpeculationHost(DummyPageHolder& page_holder, StubSpeculationHost& speculation_host, const F& functor,
    IncludesStyleUpdate includes_style_update = IncludesStyleUpdate { true })
{
    ...
    broker.SetBinderForTesting(mojom::blink::SpeculationHost::Name_, WTF::BindRepeating(&StubSpeculationHost::BindUnsafe, WTF::Unretained(&speculation_host)));

    base::RunLoop run_loop;
    speculation_host.SetDoneClosure(run_loop.QuitClosure());
    {
        auto* script_state = ToScriptStateForMainWorld(&frame);
        v8::MicrotasksScope microtasks_scope(script_state->GetIsolate(), ToMicrotaskQueue(script_state), v8::MicrotasksScope::kRunMicrotasks);
        functor();
...
void PropagateRulesToStubSpeculationHost(DummyPageHolder& page_holder, StubSpeculationHost& speculation_host, const String& speculation_script)
{
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { InsertSpeculationRules(page_holder.GetDocument(), speculation_script); });
}

template <typename F>
testing::AssertionResult NoRulesPropagatedToStubSpeculationHost(DummyPageHolder& page_holder, StubSpeculationHost& speculation_host, const F& functor,
    IncludesStyleUpdate includes_style_update = IncludesStyleUpdate { true })
{
    LocalFrame& frame = page_holder.GetFrame();
    auto& broker = frame.DomWindow()->GetBrowserInterfaceBroker();
    ...
    base::RunLoop run_loop;
    speculation_host.SetDoneClosure(base::BindLambdaForTesting([&done_was_called] { done_was_called = true; }));
    {
        auto* script_state = ToScriptStateForMainWorld(&frame);
        v8::MicrotasksScope microtasks_scope(script_state->GetIsolate(), ToMicrotaskQueue(script_state), v8::MicrotasksScope::kRunMicrotasks);
        functor();
    ...
    broker.SetBinderForTesting(mojom::blink::SpeculationHost::Name_, {});
    return done_was_called ? testing::AssertionFailure() : testing::AssertionSuccess();
}
...
TEST_F(SpeculationRuleSetTest, PropagatesAllRulesToBrowser)
{
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, speculation_script);
...
TEST_F(SpeculationRuleSetTest, PrerenderIgnorePrefetchRules)
{
    ...
    EXPECT_FALSE(base::ranges::any_of(
        candidates, [](const auto& candidate) { return candidate->action == mojom::blink::SpeculationAction::kPrefetchWithSubresources; }));
}
...
TEST_F(SpeculationRuleSetTest, PrefetchIgnorePrerenderRules)
{
    ...
    EXPECT_FALSE(base::ranges::any_of(candidates, [](const auto& candidate) { return candidate->action == mojom::blink::SpeculationAction::kPrerender; }));
}
...
TEST_F(SpeculationRuleSetTest, UseCounter)
{
    ...
    StubSpeculationHost speculation_host;
    page_holder.GetFrame().GetSettings()->SetScriptEnabled(true);
    EXPECT_FALSE(page_holder.GetDocument().IsUseCounted(WebFeature::kSpeculationRules));

    const String speculation_script = R"({"prefetch": [{"source": "list", "urls": ["/foo"]}]})";
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, speculation_script);
    EXPECT_TRUE(page_holder.GetDocument().IsUseCounted(WebFeature::kSpeculationRules));
}
...
TEST_F(SpeculationRuleSetTest, NoVarySearchHintUseCounter)
{
    ...
    StubSpeculationHost speculation_host;
    page_holder.GetFrame().GetSettings()->SetScriptEnabled(true);
    EXPECT_FALSE(page_holder.GetDocument().IsUseCounted(WebFeature::kSpeculationRulesNoVarySearchHint));
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, speculation_script);
    EXPECT_TRUE(page_holder.GetDocument().IsUseCounted(WebFeature::kSpeculationRulesNoVarySearchHint)) << "No-Vary-Search hint functionality is counted";
}
...
TEST_F(SpeculationRuleSetTest, ExcludesFragmentLinks)
{
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, String(R"({"prefetch": [
           {"source": "list", "urls":
              ["https://example.com/", "#foo", "/b#bar"]}]})"));
    EXPECT_THAT(speculation_host.candidates(), HasURLs(KURL("https://example.com"), KURL("https://example.com/b#bar")));

    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&] { page_holder.GetDocument().SetURL(KURL("https://example.com/b")); });
    EXPECT_THAT(speculation_host.candidates(), HasURLs(KURL("https://example.com")));
}
...
TEST_F(SpeculationRuleSetTest, ExcludesFragmentLinksWithBase)
{
    ...
    StubSpeculationHost speculation_host;
    page_holder.GetDocument().SetURL(KURL("https://example.com/"));
    page_holder.GetDocument().head()->setInnerHTML("<base href=\"https://not-example.com/\">");

    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, String(R"({"prefetch": [
           {"source": "list", "urls":
              ["https://example.com/#baz", "#foo", "/b#bar"]}]})"));
    EXPECT_THAT(speculation_host.candidates(), HasURLs(KURL("https://not-example.com/#foo"), KURL("https://not-example.com/b#bar")));

    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&] { page_holder.GetDocument().SetURL(KURL("https://example.com/b")); });
    EXPECT_THAT(
        speculation_host.candidates(), HasURLs(KURL("https://example.com/#baz"), KURL("https://not-example.com/#foo"), KURL("https://not-example.com/b#bar")));
}
...
TEST_F(SpeculationRuleSetTest, AddAndRemoveInSameTask)
{
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() {
        InsertSpeculationRules(page_holder.GetDocument(),
            R"({"prefetch": [
             {"source": "list", "urls": ["https://example.com/foo"]}]})");
        HTMLScriptElement* to_remove = InsertSpeculationRules(page_holder.GetDocument(),
            R"({"prefetch": [
             {"source": "list", "urls": ["https://example.com/bar"]}]})");
        InsertSpeculationRules(page_holder.GetDocument(),
            R"({"prefetch": [
...
TEST_F(SpeculationRuleSetTest, AddAndRemoveAfterReport)
{
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { to_remove->remove(); });
...
TEST_F(SpeculationRuleSetTest, RemoveInMicrotask)
{
    ...
    base::RunLoop run_loop;
    base::MockCallback<base::RepeatingCallback<void(const Vector<mojom::blink::SpeculationCandidatePtr>&)>> mock_callback;
    {
        ::testing::InSequence sequence;
        EXPECT_CALL(mock_callback, Run(::testing::SizeIs(2)));
        EXPECT_CALL(mock_callback, Run(::testing::SizeIs(1)));
        EXPECT_CALL(mock_callback, Run(::testing::SizeIs(2))).WillOnce(::testing::Invoke([&]() { run_loop.Quit(); }));
    }
    ...
    HTMLScriptElement* to_remove = InsertSpeculationRules(page_holder.GetDocument(),
        R"({"prefetch": [
             {"source": "list", "urls": ["https://example.com/bar"]}]})");
    scoped_refptr<scheduler::EventLoop> event_loop = frame.DomWindow()->GetAgent()->event_loop();
...
public:
    void AddMessageToConsole(LocalFrame*, mojom::ConsoleMessageSource, mojom::ConsoleMessageLevel, const String& message, unsigned line_number,
        const String& source_id, const String& stack_trace) override
    {
        messages_.push_back(message);
    }

    const Vector<String>& ConsoleMessages() const
    {
        return messages_;
    }
...
TEST_F(SpeculationRuleSetTest, ConsoleWarning)
{
    ...
    EXPECT_TRUE(base::ranges::any_of(chrome_client->ConsoleMessages(), [](const String& message) { return message.Contains("Syntax error"); }));
}
...
TEST_F(SpeculationRuleSetTest, ConsoleWarningForInvalidRule)
{
    ...
    EXPECT_TRUE(
        base::ranges::any_of(chrome_client->ConsoleMessages(), [](const String& message) { return message.Contains("URLs must be given as strings"); }));
}
...
TEST_F(SpeculationRuleSetTest, ConsoleWarningForSetInnerHTML)
{
    ...
    EXPECT_TRUE(base::ranges::any_of(
        chrome_client->ConsoleMessages(), [](const String& message) { return message.Contains("speculation rule") && message.Contains("will be ignored"); }));
}
...
TEST_F(SpeculationRuleSetTest, ConsoleWarningForChildModification)
{
    ...
    EXPECT_TRUE(base::ranges::any_of(
        chrome_client->ConsoleMessages(), [](const String& message) { return message.Contains("speculation rule") && message.Contains("modified"); }));
}
...
TEST_F(SpeculationRuleSetTest, ConsoleWarningForDuplicateKey)
{
    ...
    EXPECT_TRUE(base::ranges::any_of(chrome_client->ConsoleMessages(),
        [](const String& message) { return message.Contains("speculation rule") && message.Contains("more than one") && message.Contains("prefetch"); }));
}
TEST_F(SpeculationRuleSetTest, DropNotArrayAtRuleSetPosition)
{
    ...
    ASSERT_TRUE(rule_set);
    EXPECT_EQ(rule_set->error_type(), SpeculationRuleSetErrorType::kInvalidRulesSkipped);
    EXPECT_TRUE(rule_set->error_message().Contains("A rule set for a key must be an array: path = [\"prefetch\"]")) << rule_set->error_message();
    EXPECT_THAT(rule_set->prefetch_rules(), ElementsAre());
    EXPECT_THAT(rule_set->prerender_rules(), ElementsAre());
    EXPECT_THAT(rule_set->prefetch_with_subresources_rules(), ElementsAre());
}
...
TEST_F(SpeculationRuleSetTest, DropNotObjectAtRulePosition)
{
    ...
    ASSERT_TRUE(rule_set);
    EXPECT_EQ(rule_set->error_type(), SpeculationRuleSetErrorType::kInvalidRulesSkipped);
    EXPECT_TRUE(rule_set->error_message().Contains("A rule must be an object: path = [\"prefetch\"][0]")) << rule_set->error_message();
    EXPECT_THAT(rule_set->prefetch_rules(), ElementsAre());
    EXPECT_THAT(rule_set->prerender_rules(), ElementsAre());
    EXPECT_THAT(rule_set->prefetch_with_subresources_rules(), ElementsAre());
}

MATCHER_P(MatchesPredicate, matcher, ::testing::DescribeMatcher<DocumentRulePredicate*>(matcher))
{
    if (!arg->predicate()) {
...
template <typename ItemType> class PredicateMatcher {
public:
    using DocumentRulePredicateGetter = HeapVector<Member<ItemType>> (DocumentRulePredicate::*)() const;

    explicit PredicateMatcher(Vector<::testing::Matcher<ItemType*>> matchers, DocumentRulePredicate::Type type, DocumentRulePredicateGetter getter)
    ...
    bool MatchAndExplain(DocumentRulePredicate* predicate, ::testing::MatchResultListener* listener) const
    {
        if (!predicate) {
            return false;
        }
...
auto MakePredicateMatcher(
    Vector<::testing::Matcher<ItemType*>> matchers, DocumentRulePredicate::Type type, typename PredicateMatcher<ItemType>::DocumentRulePredicateGetter getter)
{
    return testing::MakePolymorphicMatcher(PredicateMatcher<ItemType>(std::move(matchers), type, getter));
}

auto MakeConditionMatcher(Vector<::testing::Matcher<DocumentRulePredicate*>> matchers, DocumentRulePredicate::Type type)
{
    return MakePredicateMatcher(std::move(matchers), type, &DocumentRulePredicate::GetSubPredicatesForTesting);
}

auto And(Vector<::testing::Matcher<DocumentRulePredicate*>> matchers = {})
{
    return MakeConditionMatcher(std::move(matchers), DocumentRulePredicate::Type::kAnd);
}

auto Or(Vector<::testing::Matcher<DocumentRulePredicate*>> matchers = {})
{
    return MakeConditionMatcher(std::move(matchers), DocumentRulePredicate::Type::kOr);
}
...
{
    return MakePredicateMatcher(std::move(pattern_matchers), DocumentRulePredicate::Type::kURLPatterns, &DocumentRulePredicate::GetURLPatternsForTesting);
}

auto Selector(Vector<::testing::Matcher<StyleRule*>> style_rule_matchers = {})
{
    return MakePredicateMatcher(std::move(style_rule_matchers), DocumentRulePredicate::Type::kCSSSelectors, &DocumentRulePredicate::GetStyleRulesForTesting);
}
...
public:
    ...
    bool MatchAndExplain(StyleRule* style_rule, ::testing::MatchResultListener* listener) const
    {
        if (!style_rule) {
            return false;
        }
...
public:
    ...
    DocumentRulePredicate* CreatePredicate(String where_text, KURL base_url = KURL("https://example.com/"))
    {
        auto* rule_set = CreateRuleSetWithPredicate(where_text, base_url);
        DCHECK(!rule_set->prefetch_rules().empty()) << "Invalid predicate: " << rule_set->error_message();
        return rule_set->prefetch_rules()[0]->predicate();
    }

    String CreateInvalidPredicate(String where_text)
    {
        auto* rule_set = CreateRuleSetWithPredicate(where_text, KURL("https://example.com"));
        EXPECT_TRUE(!rule_set || rule_set->prefetch_rules().empty()) << "Rule set is valid.";
        return rule_set->error_message();
    }

private:
    SpeculationRuleSet* CreateRuleSetWithPredicate(String where_text, KURL base_url)
    {
        // clang-format on
        auto* rule_set = CreateRuleSet(String::Format(
                                           R"({
              "prefetch": [{
                "source": "document",
                "where": {%s}
...
TEST_F(DocumentRulesTest, ParseAnd)
{
    auto* rule_set = CreateRuleSet(
        ...
        KURL("https://example.com/"), execution_context());
    EXPECT_THAT(rule_set->prefetch_rules(), ElementsAre(MatchesPredicate(And()), MatchesPredicate(And({ And(), And() }))));
}
...
TEST_F(DocumentRulesTest, ParseOr)
{
    auto* rule_set = CreateRuleSet(
        ...
        KURL("https://example.com/"), execution_context());
    EXPECT_THAT(rule_set->prefetch_rules(), ElementsAre(MatchesPredicate(Or()), MatchesPredicate(Or({ And(), Or() }))));
}
...
TEST_F(DocumentRulesTest, ParseNot)
{
    auto* rule_set = CreateRuleSet(
        ...
        KURL("https://example.com/"), execution_context());
    EXPECT_THAT(rule_set->prefetch_rules(), ElementsAre(MatchesPredicate(Neg(And())), MatchesPredicate(Neg(Or({ And(), Or() })))));
}
...
TEST_F(DocumentRulesTest, ParseHref)
{
    auto* rule_set = CreateRuleSet(
        ...
        KURL("https://example.com/"), execution_context());
    EXPECT_THAT(rule_set->prefetch_rules(),
        ElementsAre(MatchesPredicate(Href({ URLPattern("/foo#bar") })), MatchesPredicate(Href({ URLPattern("/foo") })),
            MatchesPredicate(Href({ URLPattern("/buzz"), URLPattern("/fizz"), URLPattern("https://bar.com:*") })),
            MatchesPredicate(Or({ Href({ URLPattern("https://foo.com:*") }), Neg(Href({ URLPattern("http://*:*") })) }))));
}
...
{
    auto* without_base_specified = CreatePredicate(R"("href_matches": {"pathname": "/hello"})", KURL("http://foo.com"));
    EXPECT_THAT(without_base_specified, Href({ URLPattern("http://foo.com/hello") }));
    auto* with_base_specified = CreatePredicate(R"("href_matches": {"pathname": "hello", "baseURL": "http://bar.com"})", KURL("http://foo.com"));
...
TEST_F(DocumentRulesTest, HrefMatchesWithBaseURLAndRelativeTo)
{
    ...
    auto* relative_to_no_effect = CreatePredicate(
        ...
        KURL("http://foo.com"));
    EXPECT_THAT(relative_to_no_effect, Href({ URLPattern("http://buz.com/hello") }));
    ...
    EXPECT_THAT(nested_relative_to, Or({ Href({ URLPattern("http://bar.com/hello") }), Neg(Href({ URLPattern("http://foo.com/world") })) }));
...
TEST_F(DocumentRulesTest, DropInvalidRules)
{
    ...
    EXPECT_THAT(rule_set->prefetch_rules(),
        ElementsAre(MatchesPredicate(And({})),
            MatchesPredicate(And({ Or({ Href({ URLPattern("/hello.html") }), Selector({ StyleRuleWithSelectorText(".valid") }) }),
...
TEST_F(DocumentRulesTest, ConsoleWarningForInvalidRule)
{
    ...
    EXPECT_TRUE(base::ranges::any_of(
        chrome_client->ConsoleMessages(), [](const String& message) { return message.Contains("Document rule predicate type is ambiguous"); }));
}

TEST_F(DocumentRulesTest, DocumentRuleParseErrors)
{
    auto* rule_set1 = CreateRuleSet(R"({"prefetch": [{
    "source": "document", "relative_to": "document"
  }]})",
        KURL("https://example.com"), execution_context());
    EXPECT_THAT(rule_set1->error_message().Utf8(),
        ::testing::HasSubstr("A document rule cannot have \"relative_to\" "
                             "outside the \"where\" clause"));

    auto* rule_set2 = CreateRuleSet(R"({"prefetch": [{
    "source": "document",
    "urls": ["/one",  "/two"]
  }]})",
        KURL("https://example.com"), execution_context());
    EXPECT_THAT(rule_set2->error_message().Utf8(), ::testing::HasSubstr("A document rule cannot have a \"urls\" key"));
}
...
TEST_F(DocumentRulesTest, DocumentRulePredicateParseErrors)
{
    ...
    EXPECT_THAT(parse_error.Utf8(), ::testing::HasSubstr("Document rule predicate type is ambiguous, two types found"));
    ...
    parse_error = CreateInvalidPredicate(R"("not": [{"href_matches": "foo.com"}])");
    EXPECT_THAT(parse_error.Utf8(), ::testing::HasSubstr("Document rule predicate must be an object"));

    parse_error = CreateInvalidPredicate(R"("and": [], "relative_to": "document")");
    EXPECT_THAT(parse_error.Utf8(), ::testing::HasSubstr("Document rule predicate with \"and\" key cannot have other keys."));

    parse_error = CreateInvalidPredicate(R"("or": {})");
    EXPECT_THAT(parse_error.Utf8(), ::testing::HasSubstr("\"or\" key should have a list value"));

    parse_error = CreateInvalidPredicate(R"("href_matches": {"port": 1234})");
    EXPECT_THAT(parse_error.Utf8(), ::testing::HasSubstr("Values for a URL pattern object must be strings"));

    parse_error = CreateInvalidPredicate(R"("href_matches": {"path_name": "foo"})");
    EXPECT_THAT(parse_error.Utf8(),
        ::testing::HasSubstr("Invalid key \"path_name\" for a URL "
                             "pattern object found"));

    parse_error = CreateInvalidPredicate(R"("href_matches": [["bar.com/foo.html"]])");
    EXPECT_THAT(parse_error.Utf8(),
        ::testing::HasSubstr("Value for \"href_matches\" should "
                             "either be a string"));

    parse_error = CreateInvalidPredicate(R"("href_matches": "/home", "relative_to": "window")");
    EXPECT_THAT(parse_error.Utf8(), ::testing::HasSubstr("Unrecognized \"relative_to\" value: \"window\""));

    parse_error = CreateInvalidPredicate(R"("href_matches": "/home", "relativeto": "document")");
    EXPECT_THAT(parse_error.Utf8(), ::testing::HasSubstr("Unrecognized key found: \"relativeto\""));
    ...
    EXPECT_THAT(parse_error.Utf8(), ::testing::HasSubstr("Value for \"selector_matches\" must be a string"));

    parse_error = CreateInvalidPredicate(R"("selector_matches": "##bad_selector")");
    EXPECT_THAT(parse_error.Utf8(), ::testing::HasSubstr("\"##bad_selector\" is not a valid selector"));
}
...
TEST_F(DocumentRulesTest, EvaluateHrefMatches)
{
    ...
    auto* single = CreatePredicate(R"("href_matches": "https://foo.com/bar.html?*")");
    EXPECT_TRUE(single->Matches(*link));

    // Two patterns which don't match.
    auto* double_fail = CreatePredicate(R"("href_matches": ["http://foo.com/*", "https://bar.com/*"])");
    ...
    auto* pass_fail = CreatePredicate(R"("href_matches": ["https://foo.com/bar.html?*", "https://bar.com/*"])");
    EXPECT_TRUE(pass_fail->Matches(*link));
}

HTMLAnchorElement* AddAnchor(ContainerNode& parent, const String& href)
{
    HTMLAnchorElement* link = MakeGarbageCollected<HTMLAnchorElement>(parent.GetDocument());
    link->setHref(href);
...
{
    HTMLAreaElement* area = MakeGarbageCollected<HTMLAreaElement>(parent.GetDocument());
    area->setHref(href);
...
TEST_F(DocumentRulesTest, SpeculationCandidatesReportedAfterInitialization)
{
    ...
    const auto& candidates = speculation_host.candidates();
    EXPECT_THAT(candidates, HasURLs(KURL("https://foo.com/doc.html"), KURL("https://foo.com/doc2.html")));
}

// Tests that speculation candidates based of existing links are reported after
// a document rule is inserted. Test that the speculation candidates include
// No-Vary-Search hint.
TEST_F(DocumentRulesTest, SpeculationCandidatesReportedAfterInitializationWithNVS)
{
    DummyPageHolder page_holder;
    StubSpeculationHost speculation_host;
    Document& document = page_holder.GetDocument();
    ...
    EXPECT_THAT(candidates, ::testing::Each(::testing::AllOf(HasNoVarySearchHint(), NVSVariesOnKeyOrder(), NVSHasNoVaryParams("a"))));
}
...
TEST_F(DocumentRulesTest, SpeculationCandidatesUpdatedAfterLinkModifications)
{
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { link = AddAnchor(*document.body(), "https://foo.com/action.html"); });
    ASSERT_EQ(candidates.size(), 1u);
    EXPECT_EQ(candidates[0]->url, KURL("https://foo.com/action.html"));

    // Update link href to URL that doesn't match.
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { link->setHref("https://bar.com/document.html"); });
    EXPECT_TRUE(candidates.empty());

    // Update link href to URL that matches.
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { link->setHref("https://foo.com/document.html"); });
    ASSERT_EQ(candidates.size(), 1u);
    EXPECT_EQ(candidates[0]->url, KURL("https://foo.com/document.html"));

    // Remove link.
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { link->remove(); });
    EXPECT_TRUE(candidates.empty());
}
...
TEST_F(DocumentRulesTest, SpeculationCandidatesUpdatedAfterRuleSetsChanged)
{
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, speculation_script_1);
    const auto& candidates = speculation_host.candidates();
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { script_el = InsertSpeculationRules(document, speculation_script_2); });
    EXPECT_THAT(candidates, HasURLs(url_1, url_1, url_2, url_2));
    EXPECT_THAT(candidates,
        ::testing::UnorderedElementsAre(HasAction(mojom::SpeculationAction::kPrefetch), HasAction(mojom::SpeculationAction::kPrefetch),
            HasAction(mojom::SpeculationAction::kPrerender), HasAction(mojom::SpeculationAction::kPrerender)));

    // Remove the recently added rule set, the number of candidates should be
    // halved.
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { script_el->remove(); });
    ASSERT_EQ(candidates.size(), 2u);
    EXPECT_THAT(candidates, HasURLs(url_1, url_2));
    EXPECT_THAT(candidates, ::testing::Each(HasAction(mojom::SpeculationAction::kPrefetch)));
}
...
TEST_F(DocumentRulesTest, ListRuleCombinedWithDocumentRule)
{
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, speculation_script);
    const auto& candidates = speculation_host.candidates();
    EXPECT_THAT(candidates, HasURLs(KURL("https://foo.com/bar"), KURL("https://bar.com/foo")));
}
...
TEST_F(DocumentRulesTest, LinkInShadowTreeIncluded)
{
    ...
    Document& document = page_holder.GetDocument();
    ShadowRoot& shadow_root = document.body()->AttachShadowRootForTesting(ShadowRootMode::kOpen);
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { shadow_tree_link->setHref("https://not-foo.com/"); });
    EXPECT_TRUE(candidates.empty());

    HTMLAnchorElement* shadow_tree_link_2 = nullptr;
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { shadow_tree_link_2 = AddAnchor(shadow_root, "https://foo.com/buzz"); });
    ASSERT_EQ(candidates.size(), 1u);
    EXPECT_EQ(candidates[0]->url, KURL("https://foo.com/buzz"));

    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { shadow_tree_link_2->remove(); });
    EXPECT_TRUE(candidates.empty());
}
...
TEST_F(DocumentRulesTest, LinkWithNoHrefAttribute)
{
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { link->setHref("https://foo.com/bar"); });
    ASSERT_EQ(candidates.size(), 1u);
    ASSERT_EQ(candidates[0]->url, "https://foo.com/bar");

    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { link->removeAttribute(html_names::kHrefAttr); });
    ASSERT_TRUE(candidates.empty());
...
TEST_F(DocumentRulesTest, AreaElement)
{
    ...
    StubSpeculationHost speculation_host;
    Document& document = page_holder.GetDocument();
    HTMLAreaElement* area = AddAreaElement(*document.body(), "https://foo.com/action.html");
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { area->setHref("https://bar.com/document.html"); });
    EXPECT_TRUE(candidates.empty());

    // Update area href to URL that matches.
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { area->setHref("https://foo.com/document.html"); });
    ASSERT_EQ(candidates.size(), 1u);
    EXPECT_EQ(candidates[0]->url, KURL("https://foo.com/document.html"));

    // Remove area.
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { area->remove(); });
    EXPECT_TRUE(candidates.empty());
}
...
TEST_F(DocumentRulesTest, DisconnectedLinkInShadowTree)
{
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() {
        div = MakeGarbageCollected<HTMLDivElement>(document);
        ShadowRoot& shadow_root = div->AttachShadowRootForTesting(ShadowRootMode::kOpen);
...
TEST_F(DocumentRulesTest, ReferrerPolicy)
{
    ...
    link_with_referrer->setAttribute(html_names::kReferrerpolicyAttr, AtomicString("same-origin"));
    auto* link_with_rel_no_referrer = AddAnchor(*document.body(), "https://foo.com/def");
    link_with_rel_no_referrer->setAttribute(html_names::kRelAttr, AtomicString("noreferrer"));
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, speculation_script);
    const auto& candidates = speculation_host.candidates();
    EXPECT_THAT(candidates, ::testing::Each(HasReferrerPolicy(network::mojom::ReferrerPolicy::kStrictOrigin)));
}
...
TEST_F(DocumentRulesTest, LinkReferrerPolicy)
{
    ...
    StubSpeculationHost speculation_host;
    Document& document = page_holder.GetDocument();
    page_holder.GetFrame().DomWindow()->SetReferrerPolicy(network::mojom::ReferrerPolicy::kStrictOrigin);

    auto* link_with_referrer = AddAnchor(*document.body(), "https://foo.com/abc");
    link_with_referrer->setAttribute(html_names::kReferrerpolicyAttr, AtomicString("same-origin"));
    auto* link_with_no_referrer = AddAnchor(*document.body(), "https://foo.com/xyz");
    auto* link_with_rel_noreferrer = AddAnchor(*document.body(), "https://foo.com/mno");
    link_with_rel_noreferrer->setAttribute(html_names::kRelAttr, AtomicString("noreferrer"));
    auto* link_with_invalid_referrer = AddAnchor(*document.body(), "https://foo.com/pqr");
    link_with_invalid_referrer->setAttribute(html_names::kReferrerpolicyAttr, AtomicString("invalid"));
    auto* link_with_disallowed_referrer = AddAnchor(*document.body(), "https://foo.com/aaa");
    link_with_disallowed_referrer->setAttribute(html_names::kReferrerpolicyAttr, AtomicString("unsafe-url"));
    ...
    const auto& candidates = speculation_host.candidates();
    EXPECT_THAT(candidates,
        ::testing::UnorderedElementsAre(::testing::AllOf(HasURL(link_with_referrer->HrefURL()), HasReferrerPolicy(network::mojom::ReferrerPolicy::kSameOrigin)),
            ::testing::AllOf(HasURL(link_with_rel_noreferrer->HrefURL()), HasReferrerPolicy(network::mojom::ReferrerPolicy::kNever)),
            ::testing::AllOf(HasURL(link_with_no_referrer->HrefURL()), HasReferrerPolicy(network::mojom::ReferrerPolicy::kStrictOrigin)),
            ::testing::AllOf(HasURL(link_with_invalid_referrer->HrefURL()), HasReferrerPolicy(network::mojom::ReferrerPolicy::kStrictOrigin))));

    // Console message should have been logged for
    // |link_with_disallowed_referrer|.
    const auto& console_message_storage = page_holder.GetPage().GetConsoleMessageStorage();
    EXPECT_EQ(console_message_storage.size(), 1u);
    EXPECT_THAT(console_message_storage.at(0)->Nodes(), testing::Contains(link_with_disallowed_referrer->GetDomNodeId()));
}
...
TEST_F(DocumentRulesTest, ReferrerPolicyAttributeChangeCausesLinkInvalidation)
{
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, speculation_script);
    const auto& candidates = speculation_host.candidates();
    EXPECT_THAT(candidates, ElementsAre(HasReferrerPolicy(network::mojom::ReferrerPolicy::kSameOrigin)));

    PropagateRulesToStubSpeculationHost(
        page_holder, speculation_host, [&]() { link_with_referrer->setAttribute(html_names::kReferrerpolicyAttr, AtomicString("strict-origin")); });
    EXPECT_THAT(candidates, ElementsAre(HasReferrerPolicy(network::mojom::ReferrerPolicy::kStrictOrigin)));
}
...
TEST_F(DocumentRulesTest, RelAttributeChangeCausesLinkInvalidation)
{
    ...
    link->setAttribute(html_names::kReferrerpolicyAttr, AtomicString("same-origin"));
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { link->setAttribute(html_names::kRelAttr, AtomicString("noreferrer")); });
    EXPECT_THAT(candidates, ElementsAre(HasReferrerPolicy(network::mojom::ReferrerPolicy::kNever)));
}
...
TEST_F(DocumentRulesTest, ReferrerMetaChangeShouldInvalidateCandidates)
{
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, speculation_script);
    const auto& candidates = speculation_host.candidates();
    EXPECT_THAT(candidates, ElementsAre(HasReferrerPolicy(network::mojom::ReferrerPolicy::kStrictOriginWhenCrossOrigin)));

    auto* meta = MakeGarbageCollected<HTMLMetaElement>(document, CreateElementFlags());
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { document.head()->appendChild(meta); });
    EXPECT_THAT(candidates, ElementsAre(HasReferrerPolicy(network::mojom::ReferrerPolicy::kStrictOrigin)));

    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { meta->setAttribute(html_names::kContentAttr, AtomicString("same-origin")); });
    EXPECT_THAT(candidates, ElementsAre(HasReferrerPolicy(network::mojom::ReferrerPolicy::kSameOrigin)));
}
...
TEST_F(DocumentRulesTest, BaseURLChanged)
{
    ...
    const auto& candidates = speculation_host.candidates();
    EXPECT_THAT(candidates, HasURLs(KURL("https://foo.com/bar"), KURL("https://foo.com/bart")));

    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { document.SetBaseURLOverride(KURL("https://bar.com")); });
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { speculation_script->remove(); });
...
TEST_F(DocumentRulesTest, TargetHintFromLink)
{
    ...
    EXPECT_THAT(candidates,
        ::testing::UnorderedElementsAre(
            ::testing::AllOf(HasAction(mojom::blink::SpeculationAction::kPrefetch), HasTargetHint(mojom::blink::SpeculationTargetHint::kNoHint)),
            ::testing::AllOf(HasURL(KURL("https://foo.com/bar")), HasAction(mojom::blink::SpeculationAction::kPrerender),
                HasTargetHint(mojom::blink::SpeculationTargetHint::kBlank)),
            ::testing::AllOf(HasURL(KURL("https://fizz.com/buzz")), HasAction(mojom::blink::SpeculationAction::kPrerender),
                HasTargetHint(mojom::blink::SpeculationTargetHint::kSelf)),
            ::testing::AllOf(HasURL(KURL("https://hello.com/world")), HasAction(mojom::blink::SpeculationAction::kPrerender),
...
TEST_F(DocumentRulesTest, TargetHintFromSpeculationRuleOverridesLinkTarget)
{
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, speculation_script);
    const auto& candidates = speculation_host.candidates();
    EXPECT_THAT(candidates, ::testing::ElementsAre(HasTargetHint(mojom::blink::SpeculationTargetHint::kSelf)));
}
...
TEST_F(DocumentRulesTest, TargetHintFromLinkDynamic)
{
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, speculation_script);
    const auto& candidates = speculation_host.candidates();
    EXPECT_THAT(candidates, ::testing::ElementsAre(HasTargetHint(mojom::blink::SpeculationTargetHint::kNoHint)));
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { anchor->setAttribute(html_names::kTargetAttr, AtomicString("_blank")); });
    EXPECT_THAT(candidates, ::testing::ElementsAre(HasTargetHint(mojom::blink::SpeculationTargetHint::kBlank)));
}
...
TEST_F(DocumentRulesTest, ParseSelectorMatches)
{
    ...
    EXPECT_THAT(simple_selector_matches, Selector({ StyleRuleWithSelectorText(".valid") }));

    auto* simple_selector_matches_list = CreatePredicate(R"(
    "selector_matches": [".one", "#two"]
  )");
    EXPECT_THAT(simple_selector_matches_list, Selector({ StyleRuleWithSelectorText(".one"), StyleRuleWithSelectorText("#two") }));

    auto* selector_matches_with_compound_selector = CreatePredicate(R"(
    "selector_matches": ".interesting-section > a"
  )");
    EXPECT_THAT(selector_matches_with_compound_selector, Selector({ StyleRuleWithSelectorText(".interesting-section > a") }));
}
...
TEST_F(DocumentRulesTest, GetStyleRules)
{
    ...
    EXPECT_THAT(predicate,
        And({ Or({ Neg(Selector({ StyleRuleWithSelectorText("span.fizz > a") })), Selector({ StyleRuleWithSelectorText("#bar a") }) }),
            Selector({ StyleRuleWithSelectorText("a.foo") }) }));
    EXPECT_THAT(predicate->GetStyleRules(),
        UnorderedElementsAre(StyleRuleWithSelectorText("span.fizz > a"), StyleRuleWithSelectorText("#bar a"), StyleRuleWithSelectorText("a.foo")));
}
...
TEST_F(DocumentRulesTest, SelectorMatchesAddsCandidates)
{
    ...
    auto* important_section = document.getElementById(AtomicString("important-section"));
    auto* unimportant_section = document.getElementById(AtomicString("unimportant-section"));
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, speculation_script);
    const auto& candidates = speculation_host.candidates();
    EXPECT_THAT(candidates, HasURLs(KURL("https://foo.com/foo"), KURL("https://foo.com/fizz")));
}
...
TEST_F(DocumentRulesTest, SelectorMatchesIsDynamic)
{
    ...
    PropagateRulesToStubSpeculationHost(
        page_holder, speculation_host, [&]() { second_anchor->setAttribute(html_names::kClassAttr, AtomicString("important-link")); });
    EXPECT_THAT(candidates, HasURLs(KURL("https://foo.com/fizz"), KURL("https://foo.com/buzz")));

    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { important_section->SetIdAttribute(AtomicString("random-section")); });
...
TEST_F(DocumentRulesTest, AddingDocumentRulesInvalidatesStyle)
{
    ...
    auto* script_with_irrelevant_selector_matches = InsertSpeculationRules(document, R"(
    {"prefetch": [{
      "source": "document",
...
TEST_F(DocumentRulesTest, IrrelevantDOMChangeShouldNotInvalidateCandidateList)
{
    ...
    ASSERT_TRUE(NoRulesPropagatedToStubSpeculationHost(page_holder, speculation_host, [&]() {
        unimportant_section->SetIdAttribute(AtomicString("random-section"));
        page_holder.GetFrameView().UpdateAllLifecyclePhasesForTest();
    }));
}
...
TEST_F(DocumentRulesTest, SelectorMatchesInsideShadowTree)
{
    ...
    auto* important_section = shadow_root.getElementById(AtomicString("important-section"));
    auto* unimportant_section = shadow_root.getElementById(AtomicString("unimportant-section"));
...
TEST_F(DocumentRulesTest, UpdateQueueingWithSelectorMatches_1)
{
    ...
    ASSERT_TRUE(NoRulesPropagatedToStubSpeculationHost(
        page_holder, speculation_host, [&]() { AddAnchor(*document.body(), "https://bar.com/fizz.html"); }, IncludesStyleUpdate { false }));
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, []() {});
    ...
    // the new ruleset).
    PropagateRulesToStubSpeculationHost(
        page_holder, speculation_host, [&]() { InsertSpeculationRules(document, speculation_script_with_selector_matches); }, IncludesStyleUpdate { false });
    ASSERT_FALSE(document.NeedsLayoutTreeUpdate());
    EXPECT_THAT(candidates, HasURLs(KURL("https://bar.com/fizz.html")));
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, []() {});
    EXPECT_THAT(candidates, HasURLs(KURL("https://bar.com/fizz.html"), KURL("https://foo.com/fizz.html")));
}
...
TEST_F(DocumentRulesTest, UpdateQueueingWithSelectorMatches_2)
{
    ...
    PropagateRulesToStubSpeculationHost(
        ...
        [&]() {
            EXPECT_FALSE(document.NeedsLayoutTreeUpdate());
            auto* referrer_meta = MakeGarbageCollected<HTMLMetaElement>(document, CreateElementFlags());
            referrer_meta->setAttribute(html_names::kNameAttr, AtomicString("referrer"));
            referrer_meta->setAttribute(html_names::kContentAttr, AtomicString("strict-origin"));
...
TEST_F(DocumentRulesTest, UpdateQueueingWithSelectorMatches_3)
{
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { AddAnchor(*important_section, "https://foo.com/bar.html"); });
    EXPECT_THAT(candidates, HasURLs(KURL("https://foo.com/bar.html")));
}
...
TEST_F(DocumentRulesTest, UpdateQueueingWithSelectorMatches_4)
{
    ...
    ASSERT_TRUE(NoRulesPropagatedToStubSpeculationHost(
        page_holder, speculation_host, [&]() { AddAnchor(*important_section, "https://foo.com/bar"); }, IncludesStyleUpdate { false }));
    ASSERT_TRUE(document.NeedsLayoutTreeUpdate());
...
TEST_F(DocumentRulesTest, UpdateQueueingWithSelectorMatches_5)
{
    ...
    ASSERT_TRUE(NoRulesPropagatedToStubSpeculationHost(
        page_holder, speculation_host, [&]() { important_section->SetIdAttribute(AtomicString("unimportant-section")); }, IncludesStyleUpdate { false }));
...
TEST_F(DocumentRulesTest, LinksWithoutComputedStyle)
{
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { InsertSpeculationRules(document, speculation_script); });
    const auto& candidates = speculation_host.candidates();
    EXPECT_THAT(candidates, HasURLs(KURL("https://foo.com/bar")));
    ...
    PropagateRulesToStubSpeculationHost(
        page_holder, speculation_host, [&]() { important_section->SetInlineStyleProperty(CSSPropertyID::kDisplay, CSSValueID::kNone); });
    EXPECT_THAT(candidates, HasURLs());

    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { important_section->RemoveInlineStyleProperty(CSSPropertyID::kDisplay); });
    EXPECT_THAT(candidates, HasURLs(KURL("https://foo.com/bar")));
    ...
    // the candidate list.
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { important_section->AttachShadowRootForTesting(ShadowRootMode::kOpen); });
    EXPECT_THAT(candidates, HasURLs());
}
...
TEST_F(DocumentRulesTest, LinksWithoutComputedStyle_HrefMatches)
{
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { anchor->SetInlineStyleProperty(CSSPropertyID::kDisplay, CSSValueID::kNone); });
    EXPECT_THAT(candidates, HasURLs());
}
...
TEST_F(DocumentRulesTest, LinkInsideDisplayLockedElement)
{
    ...
    PropagateRulesToStubSpeculationHost(
        page_holder, speculation_host, [&]() { important_section->SetInlineStyleProperty(CSSPropertyID::kContentVisibility, CSSValueID::kHidden); });
    EXPECT_THAT(candidates, HasURLs());

    PropagateRulesToStubSpeculationHost(
        page_holder, speculation_host, [&]() { important_section->RemoveInlineStyleProperty(CSSPropertyID::kContentVisibility); });
    EXPECT_THAT(candidates, HasURLs(KURL("https://foo.com/bar")));
}
...
TEST_F(DocumentRulesTest, LinkInsideNestedDisplayLockedElement)
{
    ...
    PropagateRulesToStubSpeculationHost(
        page_holder, speculation_host, [&]() { links->SetInlineStyleProperty(CSSPropertyID::kContentVisibility, CSSValueID::kHidden); });
    EXPECT_THAT(candidates, HasURLs());

    ASSERT_TRUE(NoRulesPropagatedToStubSpeculationHost(page_holder, speculation_host, [&]() {
        important_section->SetInlineStyleProperty(CSSPropertyID::kContentVisibility, CSSValueID::kHidden);
        page_holder.GetFrameView().UpdateAllLifecyclePhasesForTest();
    }));

    ASSERT_TRUE(NoRulesPropagatedToStubSpeculationHost(page_holder, speculation_host, [&]() {
        important_section->RemoveInlineStyleProperty(CSSPropertyID::kContentVisibility);
        page_holder.GetFrameView().UpdateAllLifecyclePhasesForTest();
    }));

    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { links->RemoveInlineStyleProperty(CSSPropertyID::kContentVisibility); });
    EXPECT_THAT(candidates, HasURLs(KURL("https://foo.com/bar")));
    ...
    ASSERT_TRUE(NoRulesPropagatedToStubSpeculationHost(page_holder, speculation_host, [&]() {
        links->SetInlineStyleProperty(CSSPropertyID::kContentVisibility, CSSValueID::kHidden);
        page_holder.GetFrameView().UpdateAllLifecyclePhasesForTest();
    }));
...
TEST_F(DocumentRulesTest, DisplayLockedLink)
{
    ...
    ASSERT_TRUE(NoRulesPropagatedToStubSpeculationHost(
        page_holder, speculation_host, [&]() { anchor->SetInlineStyleProperty(CSSPropertyID::kContentVisibility, CSSValueID::kHidden); }));

    ASSERT_TRUE(
        NoRulesPropagatedToStubSpeculationHost(page_holder, speculation_host, [&]() { anchor->RemoveInlineStyleProperty(CSSPropertyID::kContentVisibility); }));
}
...
TEST_F(DocumentRulesTest, AddLinkToDisplayLockedContainer)
{
    ...
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { anchor = AddAnchor(*important_section, "https://foo.com/bar"); });
    EXPECT_THAT(candidates, HasURLs());

    // Tests removing a display-locked container with links.
    PropagateRulesToStubSpeculationHost(page_holder, speculation_host, [&]() { important_section->remove(); });
    EXPECT_THAT(candidates, HasURLs());
}
...
TEST_F(DocumentRulesTest, DisplayLockedContainerTracking)
{
    ...
    auto* important_section = document.getElementById(AtomicString("important-section"));
    auto* irrelevant_section = document.getElementById(AtomicString("irrelevant-section"));
    ...
    ASSERT_TRUE(NoRulesPropagatedToStubSpeculationHost(page_holder, speculation_host, [&]() {
        ...
        // update.
        important_section->SetInlineStyleProperty(CSSPropertyID::kColor, CSSValueID::kDarkviolet);
        page_holder.GetFrameView().UpdateAllLifecyclePhasesForTest();
    }));

    PropagateRulesToStubSpeculationHost(
        page_holder, speculation_host, [&]() { important_section->SetInlineStyleProperty(CSSPropertyID::kContentVisibility, CSSValueID::kVisible); });
    EXPECT_THAT(candidates, HasURLs(KURL("https://foo.com/fizz.html")));

    ASSERT_TRUE(NoRulesPropagatedToStubSpeculationHost(page_holder, speculation_host, [&]() {
        // Changing style of the display-locked container should not cause an
        // update.
        important_section->SetInlineStyleProperty(CSSPropertyID::kColor, CSSValueID::kDeepskyblue);
        page_holder.GetFrameView().UpdateAllLifecyclePhasesForTest();
    }));

    ASSERT_TRUE(NoRulesPropagatedToStubSpeculationHost(page_holder, speculation_host, [&]() {
        irrelevant_section->SetInlineStyleProperty(CSSPropertyID::kContentVisibility, CSSValueID::kHidden);
        page_holder.GetFrameView().UpdateAllLifecyclePhasesForTest();
    }));

    ASSERT_TRUE(NoRulesPropagatedToStubSpeculationHost(page_holder, speculation_host, [&]() {
        irrelevant_section->RemoveInlineStyleProperty(CSSPropertyID::kContentVisibility);
        page_holder.GetFrameView().UpdateAllLifecyclePhasesForTest();
    }));
}
...
TEST_F(DocumentRulesTest, RemoveForcesStyleUpdate)
{
    ...
    base::MockCallback<base::RepeatingCallback<void(const Vector<mojom::blink::SpeculationCandidatePtr>&)>> mock_callback;
    {
        ::testing::InSequence sequence;
        EXPECT_CALL(mock_callback, Run(::testing::SizeIs(2)));
        EXPECT_CALL(mock_callback, Run(::testing::SizeIs(3))).WillOnce(::testing::Invoke([&]() { run_loop.Quit(); }));
    }
    speculation_host.SetCandidatesUpdatedCallback(mock_callback.Get());
...
TEST_F(DocumentRulesTest, RemoveWhileWaitingForStyle)
{
    ...
    base::RunLoop run_loop;
    ::testing::StrictMock<base::MockCallback<base::RepeatingCallback<void(const Vector<mojom::blink::SpeculationCandidatePtr>&)>>> mock_callback;
    EXPECT_CALL(mock_callback, Run(::testing::SizeIs(1))).WillOnce(::testing::Invoke([&]() { run_loop.Quit(); }));
    speculation_host.SetCandidatesUpdatedCallback(mock_callback.Get());
...
TEST_F(DocumentRulesTest, UniversalSelector)
{
    ...
    page_holder.GetFrame().GetSettings()->SetScriptEnabled(true);
    StubSpeculationHost speculation_host;
    InsertSpeculationRules(page_holder.GetDocument(), R"({"prefetch": [{"source":"document", "where":{"selector_matches":"*"}}]})");
}
...
TEST_F(SpeculationRuleSetTest, Eagerness)
{
    ...
    const auto& candidates = speculation_host.candidates();
    EXPECT_THAT(candidates,
        UnorderedElementsAre(AllOf(HasURL(kUrl1), HasEagerness(blink::mojom::SpeculationEagerness::kConservative)),
            AllOf(HasURL(kUrl2), HasEagerness(blink::mojom::SpeculationEagerness::kEager)),
            AllOf(HasURL(kUrl3), HasEagerness(blink::mojom::SpeculationEagerness::kModerate)),
            AllOf(HasURL(kUrl4), HasEagerness(blink::mojom::SpeculationEagerness::kEager)),
            AllOf(HasURL(kUrl5), HasEagerness(blink::mojom::SpeculationEagerness::kEager)),
            AllOf(HasURL(kUrl6), HasEagerness(blink::mojom::SpeculationEagerness::kConservative)),
            AllOf(HasURL(kUrl7), HasEagerness(blink::mojom::SpeculationEagerness::kEager)),
            AllOf(HasURL(kUrl8), HasEagerness(blink::mojom::SpeculationEagerness::kConservative)),
            AllOf(HasURL(kUrl9), HasEagerness(blink::mojom::SpeculationEagerness::kEager))));
}
...
TEST_F(SpeculationRuleSetTest, ValidNoVarySearchHintValueGeneratesCandidate)
{
    ...
    EXPECT_THAT(candidates, ElementsAre(::testing::AllOf(HasNoVarySearchHint(), NVSVariesOnKeyOrder(), NVSHasNoVaryParams("a"))));
}
...
TEST_F(SpeculationRuleSetTest, ConsoleWarningForNoVarySearchHintNotAString)
{
    ...
    EXPECT_TRUE(base::ranges::any_of(
        chrome_client->ConsoleMessages(), [](const String& message) { return message.Contains("expects_no_vary_search's value must be a string"); }));
}
...
TEST_F(SpeculationRuleSetTest, NoVarySearchHintParseErrorRuleSkipped)
{
    ...
    EXPECT_THAT(rule_set->error_message().Utf8(), ::testing::HasSubstr("expects_no_vary_search's value must be a string"));
}
...
{
    {
      ...
      "prefetch": [{
        ...
        EXPECT_THAT(rule_set->warning_messages()[0].Utf8(), ::testing::HasSubstr("No-Vary-Search hint value is not a dictionary"));
    }

    {
      ...
      "prefetch": [{
        ...
        EXPECT_FALSE(rule_set->HasError());
        ASSERT_TRUE(rule_set->HasWarnings());
        EXPECT_THAT(rule_set->warning_messages()[0].Utf8(), ::testing::HasSubstr("No-Vary-Search hint value contains a \"key-order\" dictionary"));
    }
    {
        auto* rule_set = CreateRuleSet(R"({
      "prefetch": [
    ...
    }
    {
      ...
      "prefetch": [{
        ...
        ASSERT_TRUE(rule_set->HasWarnings());
        EXPECT_THAT(rule_set->warning_messages()[0].Utf8(),
            ::testing::HasSubstr("contains an \"except\" dictionary key"
...
TEST_F(SpeculationRuleSetTest, DocumentReportsSuccessMetric)
{
    ...
    script->setText("{}");
    document.head()->appendChild(script);
    histogram_tester.ExpectUniqueSample("Blink.SpeculationRules.LoadOutcome", SpeculationRulesLoadOutcome::kSuccess, 1);
}
...
TEST_F(SpeculationRuleSetTest, DocumentReportsParseErrorFromScript)
{
    ...
    script->setText("{---}");
    document.head()->appendChild(script);
    histogram_tester.ExpectUniqueSample("Blink.SpeculationRules.LoadOutcome", SpeculationRulesLoadOutcome::kParseErrorInline, 1);
}
...
TEST_F(SpeculationRuleSetTest, DocumentReportsParseErrorFromRequest)
{
    ...
    SpeculationRuleSet* rule_set
        = SpeculationRuleSet::Parse(SpeculationRuleSet::Source::FromRequest("{---}", KURL("https://fake.test/sr.json"), 0), document.GetExecutionContext());
    DocumentSpeculationRules::From(document).AddRuleSet(rule_set);
    histogram_tester.ExpectUniqueSample("Blink.SpeculationRules.LoadOutcome", SpeculationRulesLoadOutcome::kParseErrorFetched, 1);
}
...
TEST_F(SpeculationRuleSetTest, DocumentReportsParseErrorFromBrowserInjection)
{
    ...
    SpeculationRuleSet* rule_set = SpeculationRuleSet::Parse(
        SpeculationRuleSet::Source::FromBrowserInjected("{---}", KURL(), BrowserInjectedSpeculationRuleOptOut::kRespect), document.GetExecutionContext());
    DocumentSpeculationRules::From(document).AddRuleSet(rule_set);
    histogram_tester.ExpectUniqueSample("Blink.SpeculationRules.LoadOutcome", SpeculationRulesLoadOutcome::kParseErrorBrowserInjected, 1);
}
...
TEST_F(SpeculationRuleSetTest, ImplicitSource)
{
    auto* rule_set = CreateRuleSet(
        ...
        KURL("https://example.com/"), execution_context());
    EXPECT_THAT(rule_set->prefetch_rules(), ElementsAre(MatchesPredicate(Href({ URLPattern("/foo") })), MatchesListOfURLs("https://example.com/bar")));
}
