...
class CORE_EXPORT DocumentRulePredicate : public GarbageCollected<DocumentRulePredicate> {
public:
    DocumentRulePredicate() = default;
    virtual ~DocumentRulePredicate() = default;

    static DocumentRulePredicate* Parse(
        JSONObject* input, const KURL& ruleset_base_url, ExecutionContext* context, ExceptionState& exception_state, String* out_error = nullptr);
    ...
    virtual HeapVector<Member<DocumentRulePredicate>> GetSubPredicatesForTesting() const;
