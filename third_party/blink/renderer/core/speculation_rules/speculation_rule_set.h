...
class CORE_EXPORT SpeculationRuleSet final : public GarbageCollected<SpeculationRuleSet> {
public:
    ...
    public:
        ...
        Source(base::PassKey<Source>, const String& source_text, Document*, std::optional<DOMNodeId> node_id, std::optional<KURL> base_url,
            std::optional<uint64_t> request_id, bool ignore_opt_out);

        static Source* FromInlineScript(const String& source_text, Document&, DOMNodeId node_id);
        static Source* FromRequest(const String& source_text, const KURL& base_url, uint64_t request_id);
        static Source* FromBrowserInjected(const String& source_text, const KURL& base_url, BrowserInjectedSpeculationRuleOptOut opt_out);
    ...
    SpeculationRuleSetId InspectorId() const
    {
    ...
    const HeapVector<Member<SpeculationRule>>& prefetch_with_subresources_rules() const
    {
    ...
    bool has_document_rule() const
    {
        return has_document_rule_;
    }
    bool requires_unfiltered_input() const
    {
        return requires_unfiltered_input_;
    }
    ...
    const HeapVector<Member<StyleRule>>& selectors()
    {
        return selectors_;
    }
    ...
    SpeculationRuleSetErrorType error_type() const
    {
        return error_type_;
    }
    ...
    const Vector<String>& warning_messages() const
    {
    ...
    void AddConsoleMessageForValidation(Document& element_document, SpeculationRulesResource& resource);

    static mojom::blink::SpeculationTargetHint SpeculationTargetHintFromString(const StringView& target_hint_str);
...
private:
    ...
    SpeculationRuleSetErrorType error_type_ = SpeculationRuleSetErrorType::kNoError;
