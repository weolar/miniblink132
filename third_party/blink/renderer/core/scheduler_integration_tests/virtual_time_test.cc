...
protected:
    ...
    String ExecuteJavaScript(String script_source)
    {
        ...
        WebView().MainFrame()->ToWebLocalFrame()->RequestExecuteScript(DOMWrapperWorld::kMainWorldId, base::span_from_ref(source),
            mojom::blink::UserActivationOption::kDoNotActivate, mojom::blink::EvaluationTiming::kSynchronous,
            mojom::blink::LoadEventBlockingOption::kDoNotBlock, WTF::BindOnce(&ScriptExecutionCallbackHelper::Completed, base::Unretained(&callback_helper)),
            BackForwardCacheAware::kAllow, mojom::blink::WantResultOption::kWantResult, mojom::blink::PromiseResultOption::kDoNotWait);
    ...
    void RunTasksForPeriod(double delay_ms)
    {
        ...
        scheduler::GetSingleThreadTaskRunnerForTesting()->PostDelayedTask(
            FROM_HERE, WTF::BindOnce(&VirtualTimeTest::StopVirtualTimeAndExitRunLoop, WTF::Unretained(this), loop.QuitClosure()), base::Milliseconds(delay_ms));
...
TEST_F(VirtualTimeTest, MAYBE_SetInterval)
{
    ...
    ExecuteJavaScript("var run_order = [];"
                      "var count = 10;"
...
{
    GetVirtualTimeController()->EnableVirtualTime(base::Time());
    GetVirtualTimeController()->SetVirtualTimePolicy(VirtualTimeController::VirtualTimePolicy::kDeterministicLoading);

    EXPECT_TRUE(GetVirtualTimeController()->VirtualTimeAllowedToAdvance());

    SimRequest main_resource("https://example.com/test.html", "text/html");
    SimSubresourceRequest css_resource("https://example.com/test.css", "text/css");
