...
protected:
    ...
    size_t TouchHandlerRegionSize()
    {
        ...
        return ScrollingContentsCcLayerByScrollElementId(frame_view->RootCcLayer(), frame_view->LayoutViewport()->GetScrollElementId())
            ->touch_action_region()
            .GetAllRegions()
            .GetRegionComplexity();
    }
...
TEST_P(FrameThrottlingTest, ThrottleInvisibleFrames)
{
    ...
    frame_element->setAttribute(kStyleAttr, AtomicString("transform: translateY(480px)"));
    CompositeFrame();
    EXPECT_FALSE(GetDocument().View()->IsHiddenForThrottling());
    EXPECT_TRUE(frame_document->View()->IsHiddenForThrottling());

    // A partially visible child is considered visible.
    frame_element->setAttribute(kStyleAttr, AtomicString("transform: translate(-50px, 0px, 0px)"));
    CompositeFrame();
    EXPECT_FALSE(GetDocument().View()->IsHiddenForThrottling());
    EXPECT_FALSE(frame_document->View()->IsHiddenForThrottling());
}
...
TEST_P(FrameThrottlingTest, HiddenSameOriginFramesAreNotThrottled)
{
    ...
    auto* inner_frame_element = To<HTMLIFrameElement>(frame_document->getElementById(AtomicString("innerFrame")));
...
TEST_P(FrameThrottlingTest, IntersectionObservationOverridesThrottling)
{
    ...
    inner_frame_document->View()->SetIntersectionObservationState(LocalFrameView::kRequired);
    {
        GetDocument().GetFrame()->View()->SetTargetStateForTest(DocumentLifecycle::kPaintClean);
        inner_frame_document->Lifecycle().EnsureStateAtMost(DocumentLifecycle::kVisualUpdatePending);
        EXPECT_FALSE(inner_frame_document->View()->ShouldThrottleRenderingForTest());
        GetDocument().GetFrame()->View()->SetTargetStateForTest(DocumentLifecycle::kUninitialized);
    }
    ...
    inner_view->SetShouldDoFullPaintInvalidation(PaintInvalidationReason::kLayout);
    inner_view->Layer()->SetNeedsRepaint();
    EXPECT_TRUE(inner_frame_document->View()->GetLayoutView()->ShouldDoFullPaintInvalidation());
    ...
    EXPECT_LT(inner_frame_document->Lifecycle().GetState(), DocumentLifecycle::kPaintClean);
    EXPECT_TRUE(inner_view->Layer()->SelfNeedsRepaint());
}
...
TEST_P(FrameThrottlingTest, NestedIntersectionObservationStateUpdated)
{
    ...
    SimRequest main_resource("https://example.com/", "text/html");
    SimRequest frame_resource("https://example.com/iframe.html", "text/html");
    SimRequest child_frame_resource("https://example.com/child-iframe.html", "text/html");

    LoadURL("https://example.com/");
    main_resource.Complete("<iframe id=frame sandbox src=iframe.html></iframe>");
    frame_resource.Complete("<iframe id=child-frame sandbox src=child-iframe.html></iframe>");
    child_frame_resource.Complete("");

    // Move both frames offscreen to make them throttled.
    auto* frame_element = To<HTMLIFrameElement>(GetDocument().getElementById(AtomicString("frame")));
    auto* child_frame_element = To<HTMLIFrameElement>(frame_element->contentDocument()->getElementById(AtomicString("child-frame")));
    frame_element->setAttribute(kStyleAttr, AtomicString("transform: translateY(480px)"));
    ...
    CompositeFrame();
    EXPECT_EQ(LocalFrameView::kNotNeeded, root_view->GetIntersectionObservationStateForTesting());
    EXPECT_EQ(LocalFrameView::kNotNeeded, frame_view->GetIntersectionObservationStateForTesting());
    EXPECT_EQ(LocalFrameView::kNotNeeded, child_view->GetIntersectionObservationStateForTesting());
}
...
// updated.
TEST_P(FrameThrottlingTest, IntersectionObservationStateUpdatedWithThrottledLocalRoot)
{
    ...
    LoadURL("https://example.com/");
    local_root_resource.Complete("<iframe id=frame sandbox src=iframe.html></iframe>");
    ...
    auto* child_frame_document = To<HTMLIFrameElement>(root_frame->GetDocument()->getElementById(AtomicString("frame")))->contentDocument();
    ...
    CompositeFrame();
    EXPECT_EQ(LocalFrameView::kNotNeeded, root_frame_view->GetIntersectionObservationStateForTesting());
    EXPECT_EQ(LocalFrameView::kNotNeeded, child_frame_view->GetIntersectionObservationStateForTesting());
}

TEST_P(FrameThrottlingTest, ThrottlingOverrideOnlyAppliesDuringLifecycleUpdate)
{
    ...
    // the lifecycle.
    frame_document->View()->SetIntersectionObservationState(LocalFrameView::kRequired);
    EXPECT_TRUE(frame_document->View()->ShouldThrottleRenderingForTest());
    {
        GetDocument().GetFrame()->View()->SetTargetStateForTest(DocumentLifecycle::kPaintClean);
        frame_document->Lifecycle().EnsureStateAtMost(DocumentLifecycle::kVisualUpdatePending);
    ...
    CompositeFrame();
    EXPECT_EQ(DocumentLifecycle::kPrePaintClean, frame_document->Lifecycle().GetState());
    EXPECT_TRUE(frame_document->View()->ShouldThrottleRenderingForTest());
}
...
TEST_P(FrameThrottlingTest, ForAllThrottledLocalFrameViews)
{
    ...
    GetDocument().View()->ForAllThrottledLocalFrameViews([&throttled_count](LocalFrameView&) { throttled_count++; });
...
TEST_P(FrameThrottlingTest, HiddenCrossOriginDisplayNoneFramesAreThrottled)
{
    ...
    LoadURL("https://example.com/");
    main_resource.Complete("<iframe id=frame src=iframe.html></iframe>");
    frame_resource.Complete("<iframe id=innerFrame style='display: none; width: 0; height: 0' "
                            "sandbox></iframe>");
...
TEST_P(FrameThrottlingTest, ThrottledLifecycleUpdate)
{
    ...
    EXPECT_EQ(DocumentLifecycle::kPaintClean, frame_document->Lifecycle().GetState());
...
TEST_P(FrameThrottlingTest, ThrottledFrameCompositing)
{
    ...
    container_element->setAttribute(kStyleAttr, AtomicString("will-change: transform; transform: translateY(480px)"));
    CompositeFrame();
...
TEST_P(FrameThrottlingTest, MutatingThrottledFrameDoesNotCauseAnimation)
{
    ...
    frame_element->contentDocument()->documentElement()->setAttribute(kStyleAttr, AtomicString("background: green"));
    EXPECT_FALSE(Compositor().NeedsBeginFrame());
...
TEST_P(FrameThrottlingTest, SynchronousLayoutInThrottledFrame)
{
    ...
    auto* div_element = frame_element->contentDocument()->getElementById(AtomicString("div"));
...
TEST_P(FrameThrottlingTest, UnthrottlingTriggersRepaint)
{
    ...
    auto* frame_element = To<HTMLIFrameElement>(GetDocument().getElementById(AtomicString("frame")));
    frame_element->setAttribute(kStyleAttr, AtomicString("transform: translateY(480px)"));
    EXPECT_FALSE(frame_element->contentDocument()->View()->CanThrottleRendering());
    CompositeFrame();
    EXPECT_TRUE(frame_element->contentDocument()->View()->CanThrottleRendering());
    ...
    WebView().MainFrameImpl()->GetFrameView()->LayoutViewport()->SetScrollOffset(ScrollOffset(0, 480), mojom::blink::ScrollType::kProgrammatic);
...
TEST_P(FrameThrottlingTest, ChangeStyleInThrottledFrame)
{
    ...
    frame_element->contentDocument()->body()->setAttribute(kStyleAttr, AtomicString("background: green"));
...
TEST_P(FrameThrottlingTest, ChangeOriginInThrottledFrame)
{
    ...
    main_resource.Complete("<iframe style='position: absolute; top: 10000px' id=frame "
    ...
    EXPECT_TRUE(frame_element->contentDocument()->View()->CanThrottleRendering());
    EXPECT_TRUE(frame_element->contentDocument()->GetFrame()->IsCrossOriginToNearestMainFrame());
    EXPECT_FALSE(frame_element->contentDocument()->View()->GetLayoutView()->NeedsPaintPropertyUpdate());

    NonThrowableExceptionState exception_state;

    // Security policy requires setting domain on both frames.
    GetDocument().setDomain(String("example.com"), exception_state);
    frame_element->contentDocument()->setDomain(String("example.com"), exception_state);

    EXPECT_FALSE(frame_element->contentDocument()->GetFrame()->IsCrossOriginToNearestMainFrame());
    EXPECT_FALSE(frame_element->contentDocument()->View()->CanThrottleRendering());
    EXPECT_TRUE(frame_element->contentDocument()->View()->GetLayoutView()->NeedsPaintPropertyUpdate());
}
...
TEST_P(FrameThrottlingTest, MainFrameOriginChangeInvalidatesDescendants)
{
    ...
    EXPECT_FALSE(frame_document->View()->GetLayoutView()->NeedsPaintPropertyUpdate());
    ...
    EXPECT_FALSE(frame_document->View()->CanThrottleRendering());
    EXPECT_TRUE(frame_document->View()->GetLayoutView()->NeedsPaintPropertyUpdate());
}
...
TEST_P(FrameThrottlingTest, ThrottledFrameWithFocus)
{
    ...
    LoadURL("https://example.com/");
    main_resource.Complete("<iframe id=frame sandbox=allow-scripts src=iframe.html></iframe>");
    frame_resource.Complete("some text to select\n"
                            "<script>\n"
    ...
    GetDocument().GetPage()->GetFocusController().SetFocusedFrame(frame_element->contentDocument()->GetFrame());
    GetDocument().body()->setAttribute(kStyleAttr, AtomicString("background: green"));
    CompositeFrame();
    EXPECT_FALSE(Compositor().HasSelection());
}
...
TEST_P(FrameThrottlingTest, ScrollingCoordinatorShouldSkipThrottledFrame)
{
    ...
    EXPECT_EQ(DocumentLifecycle::kVisualUpdatePending, frame_element->contentDocument()->Lifecycle().GetState());
    ...
    // thread scrolling.
    EXPECT_FALSE(GetDocument().View()->LayoutViewport()->ShouldScrollOnMainThread());

    // Make the frame visible by changing its transform. This doesn't cause a
    // layout, but should still unthrottle the frame.
    frame_element->setAttribute(kStyleAttr, AtomicString("transform: translateY(0px)"));
    CompositeFrame();
    EXPECT_FALSE(frame_element->contentDocument()->View()->CanThrottleRendering());
    ...
    EXPECT_TRUE(frame_element->contentDocument()->View()->LayoutViewport()->ShouldScrollOnMainThread());
    EXPECT_FALSE(GetDocument().View()->LayoutViewport()->ShouldScrollOnMainThread());
}
...
TEST_P(FrameThrottlingTest, ScrollingCoordinatorShouldSkipThrottledLayer)
{
    ...
    LoadURL("https://example.com/");
    main_resource.Complete("<iframe id=frame sandbox=allow-scripts src=iframe.html></iframe>");
    frame_resource.Complete("<div id=div style='transform: translateZ(0)' ontouchstart='foo()'>touch "
                            "handler</div>");
...
TEST_P(FrameThrottlingTest, ScrollingCoordinatorShouldSkipCompositedThrottledFrame)
{
    SetPreferCompositingToLCDText(true);
    ...
    EXPECT_FALSE(frame_element->contentDocument()->View()->ShouldThrottleRenderingForTest());
    // Handle the pending visual update of the unthrottled frame.
    CompositeFrame();
    EXPECT_EQ(DocumentLifecycle::kPaintClean, frame_element->contentDocument()->Lifecycle().GetState());
}
...
TEST_P(FrameThrottlingTest, DumpThrottledFrame)
{
    ...
    LoadURL("https://example.com/");
    main_resource.Complete("main <iframe id=frame sandbox=allow-scripts src=iframe.html></iframe>");
    frame_resource.Complete("");
    CompositeFrame();
    auto* frame_element = To<HTMLIFrameElement>(GetDocument().getElementById(AtomicString("frame")));
    frame_element->setAttribute(kStyleAttr, AtomicString("transform: translateY(480px)"));
    ...
    ClassicScript::CreateUnspecifiedScript("document.body.innerHTML = 'throttled'")->RunScript(To<LocalDOMWindow>(frame_element->contentWindow()));
    EXPECT_FALSE(Compositor().NeedsBeginFrame());

    // The dumped contents should not include the throttled frame.
    WebString result = TestWebFrameContentDumper::DumpWebViewAsText(&WebView(), 1024);
...
TEST_P(FrameThrottlingTest, ThrottleInnerCompositedLayer)
{
    ...
    LoadURL("https://example.com/");
    main_resource.Complete("<iframe id=frame sandbox src=iframe.html></iframe>");
    frame_resource.Complete("<div id=div style='will-change: transform; background: blue'>DIV</div>");
    ...
    EXPECT_FALSE(To<LayoutBoxModelObject>(frame_element->GetLayoutObject())->Layer()->IsValid());
    ...
    auto* inner_div = frame_element->contentDocument()->getElementById(AtomicString("div"));
    inner_div->setAttribute(kStyleAttr, AtomicString("background: yellow; overflow: hidden"));
...
TEST_P(FrameThrottlingTest, ThrottleSubtreeAtomically)
{
    ...
    EXPECT_TRUE(frame_element->contentDocument()->View()->CanThrottleRendering());
    EXPECT_TRUE(child_frame_element->contentDocument()->View()->CanThrottleRendering());

    // Move the frame back on screen.
    frame_element->setAttribute(kStyleAttr, AtomicString("transform: translateY(0px)"));
    Compositor().BeginFrame();
    EXPECT_FALSE(frame_element->contentDocument()->View()->CanThrottleRendering());
    EXPECT_FALSE(child_frame_element->contentDocument()->View()->CanThrottleRendering());
}
...
TEST_P(FrameThrottlingTest, SkipPaintingLayersInThrottledFrames)
{
    ...
    LoadURL("https://example.com/");
    main_resource.Complete("<iframe id=frame sandbox src=iframe.html></iframe>");
    frame_resource.Complete("<div id=div style='transform: translateZ(0); background: "
                            "red'>layer</div>");
    auto commands = CompositeFrame();
    ...
    frame_document->View()->GetLayoutView()->InvalidatePaintForViewAndDescendants();
...
TEST_P(FrameThrottlingTest, SynchronousLayoutInAnimationFrameCallback)
{
    ...
    SimRequest main_resource("https://example.com/", "text/html");
    SimRequest first_frame_resource("https://thirdparty.com/first.html", "text/html");
    SimRequest second_frame_resource("https://thirdparty.com/second.html", "text/html");
    LoadURL("https://example.com/");
    main_resource.Complete(R"HTML(
    ...
    auto* first_frame_element = To<HTMLIFrameElement>(GetDocument().getElementById(AtomicString("first")));
    first_frame_element->setAttribute(kStyleAttr, AtomicString("transform: translateY(480px)"));
    CompositeFrame();
    EXPECT_TRUE(first_frame_element->contentDocument()->View()->CanThrottleRendering());
    ...
    auto* second_frame_element = To<HTMLIFrameElement>(GetDocument().getElementById(AtomicString("second")));
...
TEST_P(FrameThrottlingTest, AllowOneAnimationFrame)
{
    ...
    LoadURL("https://example.com/");
    main_resource.Complete("<iframe id=frame style=\"position: fixed; top: -10000px\" "
    ...
    v8::Local<v8::Value> result = ClassicScript::CreateUnspecifiedScript("window.didRaf;")
                                      ->RunScriptAndReturnValue(To<LocalDOMWindow>(frame_element->contentWindow()))
...
TEST_P(FrameThrottlingTest, UpdatePaintPropertiesOnUnthrottling)
{
    ...
    frame_element->setAttribute(html_names::kStyleAttr, AtomicString("transform: translateY(1000px)"));
    CompositeFrame();
    EXPECT_TRUE(frame_document->View()->CanThrottleRendering());
    ...
    inner_div->setAttribute(html_names::kStyleAttr, AtomicString("transform: translateY(20px)"));
    EXPECT_FALSE(Compositor().NeedsBeginFrame());
    EXPECT_TRUE(frame_document->View()->CanThrottleRendering());
    UpdateAllLifecyclePhases();
    EXPECT_FALSE(inner_div_object->FirstFragment().PaintProperties());
    ...
    CompositeFrame();
    EXPECT_FALSE(frame_document->View()->CanThrottleRendering());
    EXPECT_EQ(gfx::Vector2dF(0, 20), inner_div->GetLayoutObject()->FirstFragment().PaintProperties()->Transform()->Get2dTranslation());
}
...
TEST_P(FrameThrottlingTest, DisplayNoneNotThrottled)
{
    ...
    LoadURL("https://example.com/");
    main_resource.Complete("<style>iframe { transform: translateY(480px); }</style>"
...
TEST_P(FrameThrottlingTest, LifecycleUpdateAfterUnthrottledCompositingUpdate)
{
    ...
    frame_document->getElementById(AtomicString("div"))->setAttribute(kStyleAttr, AtomicString("will-change: transform"));
...
TEST_P(FrameThrottlingTest, NestedFramesInRemoteFrameHiddenAndShown)
{
    ...
    static_cast<WebFrameWidgetImpl*>(LocalFrameRoot().FrameWidget())->ApplyViewportIntersectionForTesting(intersection.Clone());

    auto* root_frame = LocalFrameRoot().GetFrame();
    auto* frame_document = To<HTMLIFrameElement>(root_frame->GetDocument()->getElementById(AtomicString("frame")))->contentDocument();
    auto* frame_view = frame_document->View();
    auto* child_document = To<HTMLIFrameElement>(frame_document->getElementById(AtomicString("child-frame")))->contentDocument();
    ...
    CompositeFrame();
    EXPECT_EQ(root_frame->RemoteViewportIntersection(), gfx::Rect(0, 0, 100, 100));
    EXPECT_TRUE(root_frame->View()->CanThrottleRenderingForPropagation());
    EXPECT_EQ(root_frame->GetOcclusionState(), mojom::FrameOcclusionState::kPossiblyOccluded);
    EXPECT_TRUE(frame_view->CanThrottleRendering());
    ...
    frame_document->documentElement()->setAttribute(html_names::kStyleAttr, AtomicString("color: blue"));
    ...
    EXPECT_NE(root_frame->GetOcclusionState(), mojom::FrameOcclusionState::kPossiblyOccluded);
    EXPECT_FALSE(frame_view->CanThrottleRendering());
...
TEST_P(FrameThrottlingTest, LifecycleThrottledFrameNeedsRepaint)
{
    ...
    frame_document->body()->setAttribute(kStyleAttr, AtomicString("background: green"));
    // Update life cycle update except paint without throttling, which will do
    // paint invalidation.
    GetDocument().View()->UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason::kTest);
    EXPECT_TRUE(frame_document->GetLayoutView()->Layer()->SelfOrDescendantNeedsRepaint());
    // The NeedsRepaint flag doesn't propagte across frame boundary for now.
    EXPECT_FALSE(GetDocument().GetLayoutView()->Layer()->SelfOrDescendantNeedsRepaint());
    ...
    EXPECT_FALSE(frame_document->GetLayoutView()->Layer()->SelfOrDescendantNeedsRepaint());
    EXPECT_FALSE(GetDocument().GetLayoutView()->Layer()->SelfOrDescendantNeedsRepaint());
}
...
TEST_P(FrameThrottlingTest, ThrottledIframeGetsResizeEvents)
{
    ...
    frame_element->contentWindow()->addEventListener(event_type_names::kResize, listener,
        /*use_capture=*/false);
    ...
    frame_element->contentWindow()->removeEventListener(event_type_names::kResize, listener,
        /*use_capture=*/false);
}
...
TEST_P(FrameThrottlingTest, DescendantTouchActionAndWheelEventHandlers)
{
    ...
    EXPECT_TRUE(child_layout_view->DescendantEffectiveAllowedTouchActionChanged());
    ...
    EXPECT_FALSE(child_layout_view->DescendantEffectiveAllowedTouchActionChanged());
...
public:
    ...
    {
        entries[0]->target()->SetInlineStyleProperty(CSSPropertyID::kWidth, "100px");
    }
};
...
TEST_P(FrameThrottlingTest, ForceUnthrottled)
{
    ...
    CompositeFrame();
    HTMLIFrameElement* frame_element = To<HTMLIFrameElement>(GetDocument().getElementById(AtomicString("frame")));
    LocalFrameView* inner_frame_view = To<LocalFrameView>(frame_element->OwnedEmbeddedContentView());
    EXPECT_TRUE(inner_frame_view->ShouldThrottleRenderingForTest());

    IntersectionObserverInit* intersection_init = IntersectionObserverInit::Create();
    TestIntersectionObserverDelegate* intersection_delegate = MakeGarbageCollected<TestIntersectionObserverDelegate>(*frame_element->contentDocument());
    IntersectionObserver* intersection_observer
        = IntersectionObserver::Create(intersection_init, *intersection_delegate, LocalFrameUkmAggregator::kJavascriptIntersectionObserver);
    intersection_observer->observe(frame_element->contentDocument()->body());

    ResizeObserver::Delegate* resize_delegate = MakeGarbageCollected<TestResizeObserverDelegate>();
    ResizeObserver* resize_observer = ResizeObserver::Create(&Window(), resize_delegate);
...
TEST_P(FrameThrottlingTest, CullRectUpdate)
{
    ...
    EXPECT_TRUE(frame_document->View()->ShouldThrottleRenderingForTest());
    EXPECT_EQ(gfx::Rect(0, 0, 100, 100), frame_object->FirstFragment().GetCullRect().Rect());
    ...
    GetDocument().getElementById(AtomicString("clip"))->setAttribute(kStyleAttr, AtomicString("width: 630px"));
    CompositeFrame();
    EXPECT_EQ(gfx::Rect(0, 0, 630, 100), frame_object->FirstFragment().GetCullRect().Rect());
    EXPECT_TRUE(child_layout_view->Layer()->NeedsCullRectUpdate());

    // Move the frame into the visible viewport.
    GetDocument().getElementById(AtomicString("container"))->setAttribute(kStyleAttr, AtomicString("transform: translate(0)"));
    // Update throttling, which will schedule visual update on unthrottling of the
    // frame.
    CompositeFrame();
    EXPECT_FALSE(frame_document->View()->ShouldThrottleRenderingForTest());
    ...
    EXPECT_FALSE(frame_document->View()->ShouldThrottleRenderingForTest());
    EXPECT_EQ(gfx::Rect(0, 0, 630, 100), frame_object->FirstFragment().GetCullRect().Rect());
    EXPECT_EQ(gfx::Rect(0, 0, 630, 100), child_layout_view->FirstFragment().GetCullRect().Rect());
    EXPECT_FALSE(child_layout_view->Layer()->NeedsCullRectUpdate());
}
...
TEST_P(FrameThrottlingTest, ClearPaintArtifactOnThrottlingLocalRoot)
{
    ...
    SimRequest local_root_resource("https://example.com/", "text/html");
    LoadURL("https://example.com/");
    local_root_resource.Complete("<div style='will-change:transform'>Hello, world!</div>");
    CompositeFrame();
    LocalFrameView* view = LocalFrameRoot().GetFrame()->View();
    Element* div = view->GetFrame().GetDocument()->QuerySelector(AtomicString("div"));
    EXPECT_FALSE(view->GetPaintControllerPersistentDataForTesting().GetPaintArtifact().IsEmpty());
    ...
    // results.
    EXPECT_TRUE(view->GetPaintControllerPersistentDataForTesting().GetPaintArtifact().IsEmpty());
}
