...
TEST_F(FrameSchedulerTest, BackForwardCacheOptOut_FrameNavigated)
{
    ...
    const auto bf_cache_metrics_contains = [&frame_scheduler](SchedulingPolicy::Feature feature) {
        return frame_scheduler->GetActiveFeaturesTrackedForBackForwardCacheMetrics().Contains(feature);
    };
    EXPECT_FALSE(bf_cache_metrics_contains(SchedulingPolicy::Feature::kWebSocket));
    EXPECT_FALSE(bf_cache_metrics_contains(SchedulingPolicy::Feature::kMainResourceHasCacheControlNoStore));

    auto feature_handle = frame_scheduler->RegisterFeature(SchedulingPolicy::Feature::kWebSocket, { SchedulingPolicy::DisableBackForwardCache() });

    EXPECT_TRUE(bf_cache_metrics_contains(SchedulingPolicy::Feature::kWebSocket));
    EXPECT_FALSE(bf_cache_metrics_contains(SchedulingPolicy::Feature::kMainResourceHasCacheControlNoStore));

    frame_scheduler->RegisterStickyFeature(SchedulingPolicy::Feature::kMainResourceHasCacheControlNoStore, { SchedulingPolicy::DisableBackForwardCache() });

    EXPECT_TRUE(bf_cache_metrics_contains(SchedulingPolicy::Feature::kWebSocket));
    EXPECT_TRUE(bf_cache_metrics_contains(SchedulingPolicy::Feature::kMainResourceHasCacheControlNoStore));
...
TEST_F(FrameSchedulerTest, SchedulingFeatureHandlesResetWhenDocumentReplaced)
{
    ...
    auto scheduling_feature_handle = MainFrame().GetFrame()->GetFrameScheduler()->RegisterFeature(SchedulingPolicy::Feature::kWebSocket, {});
...
TEST_F(FrameSchedulerFrameTypeTest, GetFrameType)
{
    ...
    EXPECT_EQ(FrameScheduler::FrameType::kMainFrame, MainFrame().GetFrame()->GetFrameScheduler()->GetFrameType());

    Frame* child = MainFrame().GetFrame()->Tree().FirstChild();
    EXPECT_EQ(FrameScheduler::FrameType::kSubframe, To<LocalFrame>(child)->GetFrameScheduler()->GetFrameType());
}

class FencedFrameFrameSchedulerTest : private ScopedFencedFramesForTest, public testing::WithParamInterface<const char*>, public SimTest {
public:
    FencedFrameFrameSchedulerTest()
...
TEST_F(FencedFrameFrameSchedulerTest, GetFrameType)
{
    ...
    EXPECT_EQ(FrameScheduler::FrameType::kMainFrame, MainFrame().GetFrame()->GetFrameScheduler()->GetFrameType());
    EXPECT_TRUE(MainFrame().GetFrame()->GetFrameScheduler()->IsInEmbeddedFrameTree());
}
