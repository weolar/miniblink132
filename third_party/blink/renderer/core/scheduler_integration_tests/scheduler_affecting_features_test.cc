...
public:
    ...
    FrameScheduler* MainFrameScheduler()
    {
        return MainFrame().Scheduler();
    }
    ...
    Vector<SchedulingPolicy::Feature> GetNonTrivialMainFrameFeatures()
    {
        ...
        for (SchedulingPolicy::Feature feature : MainFrameScheduler()->GetActiveFeaturesTrackedForBackForwardCacheMetrics()) {
            ...
            if (feature == SchedulingPolicy::Feature::kOutstandingNetworkRequestXHR)
                continue;
            if (feature == SchedulingPolicy::Feature::kOutstandingNetworkRequestOthers) {
                continue;
            }
...
TEST_F(SchedulingAffectingFeaturesTest, WebSocketIsTracked)
{
    ...
    EXPECT_THAT(GetNonTrivialMainFrameFeatures(), testing::UnorderedElementsAre());

    main_resource.Complete("<script>"
                           "  var socket = new WebSocket(\"ws://www.example.com/websocket\");"
                           "</script>");

    EXPECT_FALSE(GetPageScheduler()->OptedOutFromAggressiveThrottlingForTest());
    EXPECT_THAT(
        GetNonTrivialMainFrameFeatures(), testing::UnorderedElementsAre(SchedulingPolicy::Feature::kWebSocket, SchedulingPolicy::Feature::kWebSocketSticky));
    test::RunPendingTasks();

    MainFrame().ExecuteScript(WebScriptSource(WebString("socket.close();")));

    EXPECT_FALSE(GetPageScheduler()->OptedOutFromAggressiveThrottlingForTest());
    EXPECT_THAT(GetNonTrivialMainFrameFeatures(), testing::UnorderedElementsAre(SchedulingPolicy::Feature::kWebSocketSticky));
}
...
TEST_F(SchedulingAffectingFeaturesTest, CacheControl_NoStore)
{
    ...
    SimSubresourceRequest image_resource("https://example.com/image.png", "image/png", params);

    LoadURL("https://example.com/");

    main_resource.Complete("<img src=image.png>");

    EXPECT_THAT(GetNonTrivialMainFrameFeatures(), testing::UnorderedElementsAre(SchedulingPolicy::Feature::kMainResourceHasCacheControlNoStore));

    image_resource.Complete();

    EXPECT_THAT(GetNonTrivialMainFrameFeatures(),
        testing::UnorderedElementsAre(
            SchedulingPolicy::Feature::kMainResourceHasCacheControlNoStore, SchedulingPolicy::Feature::kSubresourceHasCacheControlNoStore));
}
...
TEST_F(SchedulingAffectingFeaturesTest, CacheControl_NoCache)
{
    ...
    EXPECT_THAT(GetNonTrivialMainFrameFeatures(), testing::UnorderedElementsAre(SchedulingPolicy::Feature::kMainResourceHasCacheControlNoCache));

    image_resource.Complete();

    EXPECT_THAT(GetNonTrivialMainFrameFeatures(),
        testing::UnorderedElementsAre(
            SchedulingPolicy::Feature::kMainResourceHasCacheControlNoCache, SchedulingPolicy::Feature::kSubresourceHasCacheControlNoCache));
}
...
TEST_F(SchedulingAffectingFeaturesTest, CacheControl_Navigation)
{
    ...
    EXPECT_THAT(GetNonTrivialMainFrameFeatures(),
        testing::UnorderedElementsAre(
            SchedulingPolicy::Feature::kMainResourceHasCacheControlNoCache, SchedulingPolicy::Feature::kMainResourceHasCacheControlNoStore));
...
{
    {
        SimRequest main_resource("https://example.com/", "text/html");
        LoadURL("https://example.com/");
        main_resource.Complete("<object type='application/x-webkit-test-plugin'></object>");

        // |RunUntilIdle| is required as |Complete| doesn't wait for loading plugin.
        base::RunLoop().RunUntilIdle();

        EXPECT_THAT(GetNonTrivialMainFrameFeatures(), testing::UnorderedElementsAre(SchedulingPolicy::Feature::kContainsPlugins));
    }
    {
        SimRequest main_resource("https://example.com/", "text/html");
        LoadURL("https://example.com/");
        main_resource.Complete("<embed type='application/x-webkit-test-plugin'></embed>");
...
{
    {
        ...
        EXPECT_THAT(GetNonTrivialMainFrameFeatures(), testing::Not(testing::Contains(SchedulingPolicy::Feature::kContainsPlugins)));
    }
    {
        SimRequest main_resource("https://example.com/", "text/html");
        LoadURL("https://example.com/");
...
TEST_F(SchedulingAffectingFeaturesTest, WebLocks)
{
    ...
    EXPECT_THAT(GetNonTrivialMainFrameFeatures(), testing::UnorderedElementsAre(SchedulingPolicy::Feature::kWebLocks));
}
