...
public:
    ThrottlingTestBase()
    {
        ...
        platform_->AdvanceClock(platform_->NowTicks().SnappedToNextTick(base::TimeTicks(), base::Minutes(1)) - platform_->NowTicks());
    }
    ...
    Vector<String> FilteredConsoleMessages()
    {
        ...
        result.erase(
            std::remove_if(result.begin(), result.end(), [](const String& element) { return !element.StartsWith(kTestConsoleMessagePrefix); }), result.end());
...
class DisableBackgroundThrottlingIsRespectedTest : public ThrottlingTestBase, private ScopedTimerThrottlingForBackgroundTabsForTest {
public:
...
TEST_F(DisableBackgroundThrottlingIsRespectedTest, DisableBackgroundThrottlingIsRespected)
{
    ...
    main_resource.Complete(String::Format("(<script>"
    ...
    EXPECT_THAT(FilteredConsoleMessages(), ElementsAre(console_message, console_message, console_message, console_message, console_message));
}
...
TEST_F(BackgroundPageThrottlingTest, TimersThrottledInBackgroundPage)
{
    ...
    EXPECT_THAT(FilteredConsoleMessages(), ElementsAre(console_message, console_message, console_message));
}
...
TEST_F(BackgroundPageThrottlingTest, WithoutNesting)
{
    ...
    main_resource.Complete(String::Format("<script>"
                                          ...
                                          "  }, 1000);"
                                          "</script>",
        timeout_0_message.Utf8().c_str(), timeout_minus_1_message.Utf8().c_str(), timeout_5_message.Utf8().c_str()));
    GetDocument().GetPage()->GetPageScheduler()->SetPageVisible(false);

    platform_->RunForPeriod(base::Milliseconds(1001));
    EXPECT_THAT(FilteredConsoleMessages(), ElementsAre(timeout_0_message, timeout_minus_1_message));

    platform_->RunForPeriod(base::Milliseconds(998));
    EXPECT_THAT(FilteredConsoleMessages(), ElementsAre(timeout_0_message, timeout_minus_1_message));

    platform_->RunForPeriod(base::Milliseconds(1));
    EXPECT_THAT(FilteredConsoleMessages(), ElementsAre(timeout_0_message, timeout_minus_1_message, timeout_5_message));
}
...
TEST_F(AbortSignalTimeoutThrottlingTest, ZeroMsTimersNotThrottled)
{
    ...
    platform_->RunForPeriod(base::Milliseconds(1));
    EXPECT_THAT(FilteredConsoleMessages(), Vector<String>(kIterations, console_message()));
}
...
public:
    ...
    {
        scoped_feature_list_.InitWithFeaturesAndParameters({ { features::kIntensiveWakeUpThrottling, {} }, { features::kSetTimeoutWithoutClamp, {} } },
    ...
    void TestNoIntensiveThrottlingOnTitleOrFaviconUpdate(const String& console_message)
    {
...
constexpr char kCommunicationNop[] = "<script>"
...
// update.
constexpr char kCommunicateThroughTitleScript[] = "<script>"
                                                  "  function maybeCommunicateInBackground() {"
                                                  "    document.title += \"A\";"
                                                  "  }"
                                                  "</script>";
...
// update.
constexpr char kCommunicateThroughFavisonScript[] = "<script>"
                                                    "  function maybeCommunicateInBackground() {"
...
constexpr char kLongUnalignedTimerScriptTemplate[] = "<script>"
...
// |communicate_script|.
String BuildRepeatingTimerPage(const char* console_message, const char* communicate_script)
{
    constexpr char kRepeatingTimerPageTemplate[] = "<html>"
                                                   "<head>"
    ...
    return String::Format(kRepeatingTimerPageTemplate, console_message, communicate_script);
}
...
TEST_F(IntensiveWakeUpThrottlingTest, MainFrameTimer_ShortTimeout)
{
    ...
    // applies.
    main_resource.Complete(BuildRepeatingTimerPage(BuildTimerConsoleMessage().Utf8().c_str(), kCommunicationNop));
...
TEST_F(IntensiveWakeUpThrottlingTest, MainFrameTimer_ShortTimeout_TitleUpdate)
{
    ...
    const String console_message = BuildTimerConsoleMessage();
    main_resource.Complete(BuildRepeatingTimerPage(console_message.Utf8().c_str(), kCommunicateThroughTitleScript));
...
TEST_F(IntensiveWakeUpThrottlingTest, MainFrameTimer_ShortTimeout_FaviconUpdate)
{
    SimRequest main_resource("https://example.com/", "text/html");
    LoadURL("https://example.com/");

    const String console_message = BuildTimerConsoleMessage();
    main_resource.Complete(BuildRepeatingTimerPage(console_message.Utf8().c_str(), kCommunicateThroughFavisonScript));
...
TEST_F(IntensiveWakeUpThrottlingTest, SameOriginSubFrameTimer_ShortTimeout)
{
    ...
    subframe_resource.Complete(BuildRepeatingTimerPage(BuildTimerConsoleMessage().Utf8().c_str(), kCommunicationNop));
...
{
    SimRequest main_resource("https://example.com/", "text/html");
    SimRequest subframe_resource("https://cross-origin.example.com/iframe.html", "text/html");
    LoadURL("https://example.com/");
    main_resource.Complete(R"(<iframe src="https://cross-origin.example.com/iframe.html" />)");
    // Run tasks to let the main frame request the iframe resource. It is not
    // possible to complete the iframe resource request before that.
    platform_->RunUntilIdle();
...
TEST_F(IntensiveWakeUpThrottlingTest, MainFrameTimer_LongUnalignedTimeout)
{
    ...
    const String console_message = BuildTimerConsoleMessage();
    main_resource.Complete(String::Format(kLongUnalignedTimerScriptTemplate, console_message.Utf8().c_str()));
...
TEST_F(IntensiveWakeUpThrottlingTest, SameOriginSubFrameTimer_LongUnalignedTimeout)
{
    SimRequest main_resource("https://example.com/", "text/html");
    SimRequest subframe_resource("https://example.com/iframe.html", "text/html");
    LoadURL("https://example.com/");
    ...
    const String console_message = BuildTimerConsoleMessage();
    subframe_resource.Complete(String::Format(kLongUnalignedTimerScriptTemplate, console_message.Utf8().c_str()));
...
TEST_F(IntensiveWakeUpThrottlingTest, CrossOriginSubFrameTimer_LongUnalignedTimeout)
{
    SimRequest main_resource("https://example.com/", "text/html");
    SimRequest subframe_resource("https://cross-origin.example.com/iframe.html", "text/html");
    LoadURL("https://example.com/");
...
TEST_F(IntensiveWakeUpThrottlingTest, MainFrameAndCrossOriginSubFrameTimer_LongUnalignedTimeout)
{
    SimRequest main_resource("https://example.com/", "text/html");
    SimRequest subframe_resource("https://cross-origin.example.com/iframe.html", "text/html");
    LoadURL("https://example.com/");
    ...
    const String console_message = BuildTimerConsoleMessage();
    const String script = String::Format(kLongUnalignedTimerScriptTemplate, console_message.Utf8().c_str());

    main_resource.Complete(script + "<iframe src=\"https://cross-origin.example.com/iframe.html\" />");
    // Run tasks to let the main frame request the iframe resource. It is not
    // possible to complete the iframe resource request before that.
    platform_->RunUntilIdle();
    ...
    // Fast-forward to the next aligned time.
    platform_->RunForPeriod(base::Seconds(18));
    EXPECT_THAT(FilteredConsoleMessages(), ElementsAre(console_message, console_message));
}
