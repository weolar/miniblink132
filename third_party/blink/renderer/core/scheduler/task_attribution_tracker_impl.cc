...
// static
std::unique_ptr<TaskAttributionTracker> TaskAttributionTrackerImpl::Create(v8::Isolate* isolate)
{
...
scheduler::TaskAttributionInfo* TaskAttributionTrackerImpl::RunningTask() const
{
    if (ScriptWrappableTaskState* task_state = ScriptWrappableTaskState::GetCurrent(isolate_)) {
...
TaskAttributionTracker::TaskScope TaskAttributionTrackerImpl::CreateTaskScope(ScriptState* script_state, TaskAttributionInfo* task_state, TaskScopeType type)
{
    return CreateTaskScope(script_state, task_state, type,
        /*abort_source=*/nullptr, /*priority_source=*/nullptr);
}

TaskAttributionTracker::TaskScope TaskAttributionTrackerImpl::CreateTaskScope(ScriptState* script_state, SoftNavigationContext* soft_navigation_context)
{
    next_task_id_ = next_task_id_.NextId();
    auto* task_state = MakeGarbageCollected<TaskAttributionInfoImpl>(next_task_id_, soft_navigation_context);
    return CreateTaskScope(script_state, task_state, TaskScopeType::kSoftNavigation,
        /*abort_source=*/nullptr, /*priority_source=*/nullptr);
}

TaskAttributionTracker::TaskScope TaskAttributionTrackerImpl::CreateTaskScope(
    ScriptState* script_state, TaskAttributionInfo* task_state, TaskScopeType type, AbortSignal* abort_source, DOMTaskSignal* priority_source)
{
    CHECK(script_state);
    CHECK_EQ(script_state->GetIsolate(), isolate_);

    ScriptWrappableTaskState* previous_task_state = ScriptWrappableTaskState::GetCurrent(isolate_);
    WrappableTaskState* previous_unwrapped_task_state = previous_task_state ? previous_task_state->WrappedState() : nullptr;

    WrappableTaskState* running_task_state = nullptr;
    if (abort_source || priority_source) {
        running_task_state = MakeGarbageCollected<WebSchedulingTaskState>(task_state, abort_source, priority_source);
    } else {
    ...
    if (running_task_state != previous_unwrapped_task_state) {
        ScriptWrappableTaskState::SetCurrent(script_state, running_task_state ? MakeGarbageCollected<ScriptWrappableTaskState>(running_task_state) : nullptr);
    }

    TaskAttributionInfo* current = running_task_state ? running_task_state->GetTaskAttributionInfo() : nullptr;
    TaskAttributionInfo* previous = previous_unwrapped_task_state ? previous_unwrapped_task_state->GetTaskAttributionInfo() : nullptr;
    ...
    if (observer_ && type != TaskScopeType::kSoftNavigation && running_task_state) {
...
std::optional<TaskAttributionTracker::TaskScope> TaskAttributionTrackerImpl::MaybeCreateTaskScopeForCallback(
    ScriptState* script_state, TaskAttributionInfo* task_state)
{
    CHECK(script_state);
...
void TaskAttributionTrackerImpl::OnTaskScopeDestroyed(const TaskScope& task_scope)
{
    ScriptWrappableTaskState::SetCurrent(task_scope.script_state_, task_scope.previous_task_state_);
    TRACE_EVENT_END("scheduler");
}

TaskAttributionTracker::ObserverScope TaskAttributionTrackerImpl::RegisterObserver(Observer* observer)
{
    CHECK(observer);
...
void TaskAttributionTrackerImpl::OnObserverScopeDestroyed(const ObserverScope& observer_scope)
{
    observer_ = observer_scope.PreviousObserver();
}

void TaskAttributionTrackerImpl::AddSameDocumentNavigationTask(TaskAttributionInfo* task)
{
...
TaskAttributionInfo* TaskAttributionTrackerImpl::CommitSameDocumentNavigation(TaskAttributionId task_id)
{
