...
IdleDeadline::IdleDeadline(base::TimeTicks deadline, bool cross_origin_isolated_capability, CallbackType callback_type)
    : deadline_(deadline)
...
double IdleDeadline::timeRemaining() const
{
    ...
    if (time_remaining.is_negative() || ThreadScheduler::Current()->ShouldYieldForHighPriorityWork()) {
        return 0;
    }
