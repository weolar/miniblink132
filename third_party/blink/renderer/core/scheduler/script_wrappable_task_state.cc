...
ScriptWrappableTaskState::ScriptWrappableTaskState(WrappableTaskState* task_state)
...
// static
ScriptWrappableTaskState* ScriptWrappableTaskState::GetCurrent(v8::Isolate* isolate)
{
    CHECK(isolate);
    ...
    v8::Local<v8::Value> v8_value = isolate->GetContinuationPreservedEmbedderData();
    if (v8_value->IsNullOrUndefined()) {
        return nullptr;
    }
    // If not empty, the value must be a `ScriptWrappableTaskState`.
    NonThrowableExceptionState exception_state;
    ScriptWrappableTaskState* task_state = NativeValueTraits<ScriptWrappableTaskState>::NativeValue(isolate, v8_value, exception_state);
    DCHECK(task_state);
    return task_state;
}

// static
void ScriptWrappableTaskState::SetCurrent(ScriptState* script_state, ScriptWrappableTaskState* task_state)
{
    DCHECK(script_state);
    ...
    if (!script_state->ContextIsValid() || !task_state) {
        ...
        isolate->SetContinuationPreservedEmbedderData(ToV8Traits<ScriptWrappableTaskState>::ToV8(script_state, task_state));
    }
}
