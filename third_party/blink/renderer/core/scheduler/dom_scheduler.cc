...
namespace {
WebSchedulingPriority WebSchedulingPriorityFromEnum(V8TaskPriority::Enum priority)
{
    switch (priority) {
...
V8TaskPriority::Enum V8TaskEnumFromWebSchedulingPriority(WebSchedulingPriority priority)
{
    switch (priority) {
...
{
    DOMScheduler* scheduler = Supplement<ExecutionContext>::From<DOMScheduler>(context);
    if (!scheduler) {
    ...
    , fixed_priority_task_signals_(kWebSchedulingPriorityCount)
{
    ...
    CreateFixedPriorityTaskQueues(context, WebSchedulingQueueType::kTaskQueue, fixed_priority_task_queues_);
}
...
ScriptPromise<IDLAny> DOMScheduler::postTask(
    ScriptState* script_state, V8SchedulerPostTaskCallback* callback_function, SchedulerPostTaskOptions* options, ExceptionState& exception_state)
{
    if (!GetExecutionContext() || GetExecutionContext()->IsContextDestroyed()) {
        // The bindings layer implicitly converts thrown exceptions in
        // promise-returning functions to promise rejections.
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "Current window is detached");
        return EmptyPromise();
    }

    AbortSignal* signal_option = options->getSignalOr(nullptr);
    if (signal_option && signal_option->aborted()) {
        return ScriptPromise<IDLAny>::Reject(script_state, signal_option->reason(script_state));
    }

    DOMTaskSignal* priority_source = nullptr;
    if (options->hasPriority()) {
        // The priority option overrides the signal for priority.
        priority_source = GetFixedPriorityTaskSignal(script_state, WebSchedulingPriorityFromEnum(options->priority().AsEnum()));
    ...
    if (!priority_source) {
        priority_source = GetFixedPriorityTaskSignal(script_state, kDefaultPriority);
    }

    auto* task_queue = GetTaskQueue(priority_source, WebSchedulingQueueType::kTaskQueue);
    ...
    MakeGarbageCollected<DOMTask>(
        resolver, callback_function, signal_option, priority_source, task_queue, base::Milliseconds(options->delay()), NextIdForTracing());
    return resolver->Promise();
}

ScriptPromise<IDLUndefined> DOMScheduler::yield(ScriptState* script_state, ExceptionState& exception_state)
{
    if (!GetExecutionContext() || GetExecutionContext()->IsContextDestroyed()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotSupportedError, "Current window is detached");
        return EmptyPromise();
    ...
    if (fixed_priority_continuation_queues_.empty()) {
        CreateFixedPriorityTaskQueues(GetExecutionContext(), WebSchedulingQueueType::kContinuationQueue, fixed_priority_continuation_queues_);
    }

    AbortSignal* abort_source = nullptr;
    DOMTaskSignal* priority_source = nullptr;
    if (auto* inherited_state = ScriptWrappableTaskState::GetCurrent(script_state->GetIsolate())) {
    ...
    if (abort_source && abort_source->aborted()) {
        return ScriptPromise<IDLUndefined>::Reject(script_state, abort_source->reason(script_state));
    }
    ...
    auto* task_queue = GetTaskQueue(priority_source, WebSchedulingQueueType::kContinuationQueue);
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLUndefined>>(script_state, exception_state.GetContext());
    MakeGarbageCollected<DOMTaskContinuation>(resolver, abort_source, task_queue, NextIdForTracing());
    return resolver->Promise();
}

scheduler::TaskAttributionIdType DOMScheduler::taskId(ScriptState* script_state)
{
...
void DOMScheduler::setTaskId(ScriptState* script_state, scheduler::TaskAttributionIdType task_id)
{
    ...
    auto* task_state = MakeGarbageCollected<TaskAttributionInfoImpl>(scheduler::TaskAttributionId(task_id),
        /*soft_navigation_context=*/nullptr);
    ScriptWrappableTaskState::SetCurrent(script_state, MakeGarbageCollected<ScriptWrappableTaskState>(task_state));
    ...
    scheduler->ExecuteAfterCurrentTaskForTesting(
        WTF::BindOnce([](ScriptState* script_state) { ScriptWrappableTaskState::SetCurrent(script_state, nullptr); }, WrapPersistent(script_state)),
        ExecuteAfterCurrentTaskRestricted {});
}

void DOMScheduler::CreateFixedPriorityTaskQueues(ExecutionContext* context, WebSchedulingQueueType queue_type, FixedPriorityTaskQueueVector& task_queues)
{
    ...
    for (size_t i = 0; i < kWebSchedulingPriorityCount; i++) {
        ...
        std::unique_ptr<WebSchedulingTaskQueue> task_queue = scheduler->CreateWebSchedulingTaskQueue(queue_type, priority);
        task_queues.push_back(MakeGarbageCollected<DOMTaskQueue>(std::move(task_queue), priority));
    }
}

DOMScheduler::DOMTaskQueue* DOMScheduler::CreateDynamicPriorityTaskQueue(DOMTaskSignal* signal, WebSchedulingQueueType queue_type)
{
    FrameOrWorkerScheduler* scheduler = GetExecutionContext()->GetScheduler();
    CHECK(scheduler);
    WebSchedulingPriority priority = WebSchedulingPriorityFromEnum(signal->priority().AsEnum());
    std::unique_ptr<WebSchedulingTaskQueue> task_queue = scheduler->CreateWebSchedulingTaskQueue(queue_type, priority);
    CHECK(task_queue);
    auto* dom_task_queue = MakeGarbageCollected<DOMTaskQueue>(std::move(task_queue), priority);
    auto* handle = signal->AddPriorityChangeAlgorithm(
        WTF::BindRepeating(&DOMScheduler::OnPriorityChange, WrapWeakPersistent(this), WrapWeakPersistent(signal), WrapWeakPersistent(dom_task_queue)));
    dom_task_queue->SetPriorityChangeHandle(handle);
    return dom_task_queue;
}

DOMTaskSignal* DOMScheduler::GetFixedPriorityTaskSignal(ScriptState* script_state, WebSchedulingPriority priority)
{
    wtf_size_t index = static_cast<wtf_size_t>(priority);
    if (!fixed_priority_task_signals_[index]) {
        auto* signal = DOMTaskSignal::CreateFixedPriorityTaskSignal(script_state, V8TaskEnumFromWebSchedulingPriority(priority));
...
DOMScheduler::DOMTaskQueue* DOMScheduler::GetTaskQueue(DOMTaskSignal* task_signal, WebSchedulingQueueType queue_type)
{
    if (task_signal->HasFixedPriority()) {
        auto priority = WebSchedulingPriorityFromEnum(task_signal->priority().AsEnum());
        return queue_type == WebSchedulingQueueType::kTaskQueue ? fixed_priority_task_queues_[static_cast<wtf_size_t>(priority)]
                                                                : fixed_priority_continuation_queues_[static_cast<wtf_size_t>(priority)];
    } else {
        SignalToTaskQueueMap& queue_map = queue_type == WebSchedulingQueueType::kTaskQueue ? signal_to_task_queue_map_ : signal_to_continuation_queue_map_;
        ...
        auto* dom_task_queue = CreateDynamicPriorityTaskQueue(task_signal, queue_type);
        queue_map.insert(task_signal, dom_task_queue);
        return dom_task_queue;
    }
}

void DOMScheduler::OnPriorityChange(DOMTaskSignal* signal, DOMTaskQueue* task_queue)
{
    if (!GetExecutionContext() || GetExecutionContext()->IsContextDestroyed()) {
        return;
    }
    DCHECK(signal);
    task_queue->SetPriority(WebSchedulingPriorityFromEnum(signal->priority().AsEnum()));
}

DOMScheduler::DOMTaskQueue::DOMTaskQueue(std::unique_ptr<WebSchedulingTaskQueue> task_queue, WebSchedulingPriority priority)
