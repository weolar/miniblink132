...
public:
    ...
    explicit V8IdleTask(V8IdleRequestCallback* callback)
        : callback_(callback)
    {
    ...
    void invoke(IdleDeadline* deadline) override
    {
        ...
        if (auto* tracker = scheduler::TaskAttributionTracker::From(script_state->GetIsolate())) {
            ...
            if (RuntimeEnabledFeatures::SchedulerYieldEnabled(ExecutionContext::From(script_state))) {
                auto* context = ExecutionContext::From(script_state);
                CHECK(context);
                signal = DOMScheduler::scheduler(*context)->GetFixedPriorityTaskSignal(script_state, WebSchedulingPriority::kBackgroundPriority);
            }
            task_attribution_scope
                = tracker->CreateTaskScope(script_state, parent_task_, scheduler::TaskAttributionTracker::TaskScopeType::kRequestIdleCallback,
...
int WindowIdleTasks::requestIdleCallback(LocalDOMWindow& window, V8IdleRequestCallback* callback, const IdleRequestOptions* options)
{
    return ScriptedIdleTaskController::From(window).RegisterCallback(V8IdleTask::Create(callback), options);
}
