...
public:
    static DOMTaskSignal* CreateFixedPriorityTaskSignal(ScriptState*, V8TaskPriority::Enum priority);
    ...
    DOMTaskSignal(ScriptState*, V8TaskPriority::Enum priority, DOMTaskSignal* source_task_signal, const HeapVector<Member<AbortSignal>>& source_abort_signals);
    ~DOMTaskSignal() override;

    // task_signal.idl
    static DOMTaskSignal* any(ScriptState*, HeapVector<Member<AbortSignal>> signals, TaskSignalAnyInit*);
    V8TaskPriority priority();
    DEFINE_ATTRIBUTE_EVENT_LISTENER(prioritychange, kPrioritychange)

    [[nodiscard]] DOMTaskSignal::AlgorithmHandle* AddPriorityChangeAlgorithm(base::RepeatingClosure algorithm);
    void SignalPriorityChange(V8TaskPriority::Enum priority, ExceptionState&);

    bool IsTaskSignal() const override
    {
        return true;
    }
...
private:
    ...
    AbortSignalCompositionManager* GetCompositionManager(AbortSignalCompositionType) override;
...
template <> struct DowncastTraits<DOMTaskSignal> {
