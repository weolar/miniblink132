...
// static
DOMTaskSignal* DOMTaskSignal::CreateFixedPriorityTaskSignal(ScriptState* script_state, V8TaskPriority::Enum priority)
{
    return MakeGarbageCollected<DOMTaskSignal>(script_state, priority, nullptr, HeapVector<Member<AbortSignal>>());
}

DOMTaskSignal::DOMTaskSignal(ExecutionContext* context, V8TaskPriority::Enum priority, SignalType signal_type)
    : AbortSignal(context, signal_type)
    , priority_(priority)
{
    DCHECK_NE(signal_type, AbortSignal::SignalType::kComposite);
    priority_composition_manager_ = MakeGarbageCollected<SourceSignalCompositionManager>(*this, AbortSignalCompositionType::kPriority);
}

DOMTaskSignal::DOMTaskSignal(ScriptState* script_state, V8TaskPriority::Enum priority, DOMTaskSignal* priority_source_signal,
    const HeapVector<Member<AbortSignal>>& abort_source_signals)
    : AbortSignal(script_state, abort_source_signals)
    , priority_(priority)
{
    ...
    priority_composition_manager_ = MakeGarbageCollected<DependentSignalCompositionManager>(*this, AbortSignalCompositionType::kPriority, signals);
}

DOMTaskSignal::~DOMTaskSignal() = default;

DOMTaskSignal* DOMTaskSignal::any(ScriptState* script_state, HeapVector<Member<AbortSignal>> signals, TaskSignalAnyInit* init)
{
    DOMTaskSignal* priority_source = init->priority()->IsTaskSignal() ? init->priority()->GetAsTaskSignal() : nullptr;
    V8TaskPriority priority = priority_source ? priority_source->priority() : init->priority()->GetAsTaskPriority();
    return MakeGarbageCollected<DOMTaskSignal>(script_state, priority.AsEnum(), priority_source, signals);
}
...
DOMTaskSignal::AlgorithmHandle* DOMTaskSignal::AddPriorityChangeAlgorithm(base::RepeatingClosure algorithm)
{
    if (priority_composition_manager_->IsSettled()) {
        return nullptr;
    }
    auto* callback_algorithm = MakeGarbageCollected<RepeatingCallbackAlgorithm>(std::move(algorithm));
...
void DOMTaskSignal::SignalPriorityChange(V8TaskPriority::Enum priority, ExceptionState& exception_state)
{
    if (is_priority_changing_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotAllowedError, "Cannot change priority when a prioritychange event is in progress.");
        return;
    }
    ...
    DispatchEvent(*TaskPriorityChangeEvent::Create(event_type_names::kPrioritychange, init));

    if (auto* source_signal_manager = DynamicTo<SourceSignalCompositionManager>(*priority_composition_manager_.Get())) {
        ...
        for (auto& abort_signal : source_signal_manager->GetDependentSignals()) {
            To<DOMTaskSignal>(abort_signal.Get())->SignalPriorityChange(priority, exception_state);
        }
    }
...
AbortSignalCompositionManager* DOMTaskSignal::GetCompositionManager(AbortSignalCompositionType composition_type)
{
...
void DOMTaskSignal::OnSignalSettled(AbortSignalCompositionType composition_type)
{
    if (composition_type == AbortSignalCompositionType::kPriority) {
...
bool DOMTaskSignal::IsSettledFor(AbortSignalCompositionType composition_type) const
{
    if (composition_type == AbortSignalCompositionType::kPriority) {
        return priority_composition_manager_->IsSettled();
    }
