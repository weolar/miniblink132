...
public:
    ...
    void SetUp() override
    {
        ...
        RenderingTest::SetUp();
        auto* window_performance = DOMWindowPerformance::performance(*GetDocument().domWindow());
    ...
    {
        return ClassicScript::CreateUnspecifiedScript(expr)->RunScriptAndReturnValue(GetDocument().domWindow()).GetSuccessValueOrEmpty();
    }

    Vector<double> ToDoubleArray(v8::Local<v8::Value> value, v8::HandleScope& scope)
    {
        ScriptState::Scope context_scope(ToScriptStateForMainWorld(&GetFrame()));
        NonThrowableExceptionState exception_state;
        return NativeValueTraits<IDLSequence<IDLDouble>>::NativeValue(scope.GetIsolate(), value, exception_state);
    }
    ...
    {
        ClassicScript::CreateUnspecifiedScript(String(script_text))->RunScript(GetDocument().domWindow());
        FastForwardUntilNoTasksRemain();
    }
};

const char* const kSetTimeout0ScriptText = "var last = performance.now();"
                                           "var elapsed;"
...
const char* const kSetTimeoutNestedScriptText = "var last = performance.now();"
                                                "var times = [];"
...
const char* const kSetIntervalScriptText = "var last = performance.now();"
                                           "var times = [];"
