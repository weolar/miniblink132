...
public:
    ...
    TaskScope CreateTaskScope(ScriptState* script_state, TaskAttributionInfo* task_state, TaskScopeType type) override;

    TaskScope CreateTaskScope(ScriptState* script_state, SoftNavigationContext*) override;

    TaskScope CreateTaskScope(
        ScriptState* script_state, TaskAttributionInfo* task_state, TaskScopeType type, AbortSignal* abort_source, DOMTaskSignal* priority_source) override;

    std::optional<TaskScope> MaybeCreateTaskScopeForCallback(ScriptState*, TaskAttributionInfo* task_state) override;
