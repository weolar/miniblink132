...
public:
    ...
    MockIdleDeadlineScheduler& operator=(const MockIdleDeadlineScheduler&) = delete;
...
{
    auto* deadline = MakeGarbageCollected<IdleDeadline>(base::TimeTicks() + base::Seconds(1.25),
        /*cross_origin_isolated_capability=*/false, IdleDeadline::CallbackType::kCalledWhenIdle);
...
{
    auto* deadline = MakeGarbageCollected<IdleDeadline>(base::TimeTicks() + base::Seconds(0.75),
        /*cross_origin_isolated_capability=*/false, IdleDeadline::CallbackType::kCalledWhenIdle);
    deadline->SetTickClockForTesting(test_task_runner_->GetMockTickClock());
    test_task_runner_->FastForwardBy(base::Seconds(1));
