...
void UpdateMaxIdleTasksCrashKey(size_t num_pending_idle_tasks)
{
    ...
    static auto* crash_key = base::debug::AllocateCrashKeyString("max_idle_tasks", base::debug::CrashKeySize::Size32);
    static std::optional<size_t> crash_key_value;

    const size_t num_pending_idle_tasks_rounded_down = (num_pending_idle_tasks / 100) * 100;
    if (!crash_key_value.has_value() || crash_key_value.value() < num_pending_idle_tasks_rounded_down) {
        base::debug::SetCrashKeyString(crash_key, base::NumberToString(num_pending_idle_tasks_rounded_down));
        crash_key_value = num_pending_idle_tasks_rounded_down;
    }
}

void UpdateMaxSchedulerIdleTasksCrashKey(size_t num_pending_scheduler_idle_tasks)
{
    ...
    static auto* crash_key = base::debug::AllocateCrashKeyString("max_scheduler_idle_tasks", base::debug::CrashKeySize::Size32);
    static std::optional<size_t> crash_key_value;

    const size_t num_pending_scheduler_idle_tasks_rounded_down = (num_pending_scheduler_idle_tasks / 100) * 100;
    if (!crash_key_value.has_value() || crash_key_value.value() < num_pending_scheduler_idle_tasks_rounded_down) {
        base::debug::SetCrashKeyString(crash_key, base::NumberToString(num_pending_scheduler_idle_tasks_rounded_down));
...
BASE_FEATURE(kScriptedIdleTaskControllerOOMFix, "ScriptedIdleTaskControllerOOMFix", base::FEATURE_DISABLED_BY_DEFAULT);
...
ScriptedIdleTaskController::DelayedTaskCanceler::DelayedTaskCanceler() = default;
ScriptedIdleTaskController::DelayedTaskCanceler::DelayedTaskCanceler(base::DelayedTaskHandle delayed_task_handle)
    : delayed_task_handle_(std::move(delayed_task_handle))
{
}
ScriptedIdleTaskController::DelayedTaskCanceler::DelayedTaskCanceler(DelayedTaskCanceler&&) = default;
ScriptedIdleTaskController::DelayedTaskCanceler& ScriptedIdleTaskController::DelayedTaskCanceler::operator=(ScriptedIdleTaskController::DelayedTaskCanceler&&)
    = default;
...
const char ScriptedIdleTaskController::kSupplementName[] = "ScriptedIdleTaskController";

// static
ScriptedIdleTaskController& ScriptedIdleTaskController::From(ExecutionContext& context)
{
    ScriptedIdleTaskController* controller = Supplement<ExecutionContext>::From<ScriptedIdleTaskController>(&context);
    if (!controller) {
...
ScriptedIdleTaskController::CallbackId ScriptedIdleTaskController::RegisterCallback(IdleTask* idle_task, const IdleRequestOptions* options)
{
    ...
    idle_task->async_task_context()->Schedule(GetExecutionContext(), "requestIdleCallback");

    PostSchedulerIdleAndTimeoutTasks(id, timeout_millis);
    DEVTOOLS_TIMELINE_TRACE_EVENT_INSTANT("RequestIdleCallback", inspector_idle_callback_request_event::Data, GetExecutionContext(), id, timeout_millis);
    return id;
}

void ScriptedIdleTaskController::PostSchedulerIdleAndTimeoutTasks(CallbackId id, uint32_t timeout_millis)
{
    ...
    if (timeout_millis > 0) {
        auto callback = WTF::BindOnce(&ScriptedIdleTaskController::SchedulerTimeoutTask, WrapWeakPersistent(this), id);
        delayed_task_handle
            = GetExecutionContext()
                  ->GetTaskRunner(TaskType::kIdleTask)
                  ->PostCancelableDelayedTask(base::subtle::PostDelayedTaskPassKey(), FROM_HERE, std::move(callback), base::Milliseconds(timeout_millis));
    ...
    PostSchedulerIdleTask(id, DelayedTaskCanceler(std::move(delayed_task_handle)));
}
...
void ScriptedIdleTaskController::CancelCallback(CallbackId id)
{
    ...
    DEVTOOLS_TIMELINE_TRACE_EVENT_INSTANT("CancelIdleCallback", inspector_idle_callback_cancel_event::Data, GetExecutionContext(), id);
...
void ScriptedIdleTaskController::PostSchedulerIdleTask(CallbackId id, DelayedTaskCanceler canceler)
{
    ++num_pending_scheduler_idle_tasks_;
    UpdateMaxSchedulerIdleTasksCrashKey(num_pending_scheduler_idle_tasks_);

    scheduler_->PostIdleTask(FROM_HERE, WTF::BindOnce(&ScriptedIdleTaskController::SchedulerIdleTask, WrapWeakPersistent(this), id, std::move(canceler)));
}

void ScriptedIdleTaskController::SchedulerIdleTask(CallbackId id, ScriptedIdleTaskController::DelayedTaskCanceler /* canceler */, base::TimeTicks deadline)
{
...
void ScriptedIdleTaskController::SchedulerTimeoutTask(CallbackId id)
{
    ...
    RunIdleTask(id, /*deadline=*/base::TimeTicks::Now(), IdleDeadline::CallbackType::kCalledByTimeout);
}

void ScriptedIdleTaskController::RunIdleTask(CallbackId id, base::TimeTicks deadline, IdleDeadline::CallbackType callback_type)
{
    DCHECK(!paused_);
    ...
    base::TimeDelta allotted_time = std::max(deadline - base::TimeTicks::Now(), base::TimeDelta());

    probe::AsyncTask async_task(GetExecutionContext(), idle_task->async_task_context());
    probe::UserCallback probe(GetExecutionContext(), "requestIdleCallback", AtomicString(), true);

    bool cross_origin_isolated_capability = GetExecutionContext() ? GetExecutionContext()->CrossOriginIsolatedCapability() : false;
    DEVTOOLS_TIMELINE_TRACE_EVENT("FireIdleCallback", inspector_idle_callback_fire_event::Data, GetExecutionContext(), id, allotted_time.InMillisecondsF(),
        callback_type == IdleDeadline::CallbackType::kCalledByTimeout);
    idle_task->invoke(MakeGarbageCollected<IdleDeadline>(deadline, cross_origin_isolated_capability, callback_type));
...
void ScriptedIdleTaskController::ContextLifecycleStateChanged(mojom::FrameLifecycleState state)
{
...
void ScriptedIdleTaskController::ContextUnpaused()
{
    ...
    for (auto& id : idle_tasks_with_expired_timeout_) {
        GetExecutionContext()
            ->GetTaskRunner(TaskType::kIdleTask)
            ->PostTask(FROM_HERE, WTF::BindOnce(&ScriptedIdleTaskController::SchedulerTimeoutTask, WrapWeakPersistent(this), id));
    }
