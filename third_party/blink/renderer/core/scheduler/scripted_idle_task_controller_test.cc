...
class DelayedTaskHandleDelegateFacade : public base::DelayedTaskHandle::Delegate {
public:
    explicit DelayedTaskHandleDelegateFacade(base::DelayedTaskHandle handle, base::OnceClosure on_canceled)
...
public:
    int GetTaskCanceledCount() const
    {
        return task_canceled_count_;
    }

private:
    base::DelayedTaskHandle PostCancelableDelayedTask(
        base::subtle::PostDelayedTaskPassKey pass_key, const base::Location& from_here, base::OnceClosure task, base::TimeDelta delay) override
    {
        auto handle = scheduler::FakeTaskRunner::PostCancelableDelayedTask(pass_key, from_here, std::move(task), delay);
        return base::DelayedTaskHandle(
            std::make_unique<DelayedTaskHandleDelegateFacade>(std::move(handle), base::BindOnce(&TestTaskRunner::OnTaskCanceled, this)));
    }

    void OnTaskCanceled()
    {
        ++task_canceled_count_;
    }
...
public:
    ...
    MockScriptedIdleTaskControllerScheduler(const MockScriptedIdleTaskControllerScheduler&) = delete;
    MockScriptedIdleTaskControllerScheduler& operator=(const MockScriptedIdleTaskControllerScheduler&) = delete;
    ...
    void PostIdleTask(const base::Location&, Thread::IdleTask idle_task) override
    {
        idle_tasks_.push_back(std::move(idle_task));
    }
    void PostDelayedIdleTask(const base::Location&, base::TimeDelta, Thread::IdleTask) override
    {
        NOTIMPLEMENTED();
    }
    ...
    void RunIdleTask()
    {
        TakeIdleTask().Run(base::TimeTicks());
    }
    size_t GetNumIdleTasks() const
    {
    ...
    scoped_refptr<TestTaskRunner> TaskRunner()
    {
        return task_runner_;
    }
...
private:
    ...
    scoped_refptr<TestTaskRunner> task_runner_ = base::MakeRefCounted<TestTaskRunner>();
};

class IdleTaskControllerFrameScheduler : public FrameScheduler {
public:
    explicit IdleTaskControllerFrameScheduler(MockScriptedIdleTaskControllerScheduler* scripted_idle_scheduler)
        : scripted_idle_scheduler_(scripted_idle_scheduler)
        , page_scheduler_(scheduler::CreateDummyPageScheduler(scripted_idle_scheduler->GetIsolate()))
    {
    }
...
class ScriptedIdleTaskControllerTest : public testing::Test, public testing::WithParamInterface<bool> {
public:
    ScriptedIdleTaskControllerTest()
    {
        if (IsOOMFixEnabled()) {
            scoped_feature_list_.InitAndEnableFeature(kScriptedIdleTaskControllerOOMFix);
        } else {
            scoped_feature_list_.InitAndDisableFeature(kScriptedIdleTaskControllerOOMFix);
        }
    }
    ...
    void InitializeScheduler(ShouldYield should_yield)
    {
        ...
        execution_context_.emplace(std::make_unique<IdleTaskControllerFrameScheduler>(&scheduler_.value()));
    }
    ...
    {
        return &ScriptedIdleTaskController::From(execution_context_->GetExecutionContext());
    }

    bool IsOOMFixEnabled()
    {
        return GetParam();
    }
...
TEST_P(ScriptedIdleTaskControllerTest, NoUnnecessaryRepostOnUnpause)
{
    ...
    for (int i = 0; i < 3; ++i) {
        GetController()->ContextLifecycleStateChanged(mojom::FrameLifecycleState::kPaused);
        GetController()->ContextLifecycleStateChanged(mojom::FrameLifecycleState::kRunning);
    }
...
TEST_P(ScriptedIdleTaskControllerTest, SchedulerTimeoutTaskCanceledOnIdleTaskCanceled)
{
    InitializeScheduler(ShouldYield(false));
