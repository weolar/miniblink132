...
TEST_F(DOMSchedulerTest, FixedPriorityTasksDontCreateTaskQueues)
{
    ...
    const char* kScript = "scheduler.postTask(() => {}, {priority: 'user-blocking'});"
...
TEST_F(DOMSchedulerTest, FixedPriorityTasksWithAbortSignalDontCreateTaskQueues)
{
    EXPECT_EQ(GetDynamicPriorityTaskQueueCount(), 0u);

    const char* kScript1 = "const controller = new AbortController();"
...
TEST_F(DOMSchedulerTest, DynamicPriorityTaskQueueCreation)
{
    ...
    const char* kScript1 = "const controller1 = new TaskController();"
    ...
    const char* kScript2 = "const controller2 = new TaskController();"
...
TEST_F(DOMSchedulerTest, DynamicPriorityTaskQueueCreationReuseSignal)
{
    ...
    const char* kScript = "const controller = new TaskController();"
                          "const signal = controller.signal;"
                          "scheduler.postTask(() => {}, {signal});"
                          "scheduler.postTask(() => {}, {signal});"
                          "scheduler.postTask(() => {}, {signal});";
...
TEST_F(DOMSchedulerTest, DynamicPriorityTaskQueueGarbageCollection)
{
    ...
    const char* kScript = "function test() {"
