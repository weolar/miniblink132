...
DOMTask::DOMTask(ScriptPromiseResolver<IDLAny>* resolver, V8SchedulerPostTaskCallback* callback, AbortSignal* abort_source, DOMTaskSignal* priority_source,
    DOMScheduler::DOMTaskQueue* task_queue, base::TimeDelta delay, uint64_t task_id_for_tracing)
    : callback_(callback)
    , resolver_(resolver)
    ...
    , task_id_for_tracing_(task_id_for_tracing)
{
    ...
    if (abort_source_ && abort_source_->CanAbort()) {
        abort_handle_ = abort_source_->AddAlgorithm(WTF::BindOnce(&DOMTask::OnAbort, WrapWeakPersistent(this)));
    }

    task_handle_ = PostDelayedCancellableTask(task_queue_->GetTaskRunner(), FROM_HERE, WTF::BindOnce(&DOMTask::Invoke, WrapPersistent(this)), delay);

    ScriptState* script_state = callback_->CallbackRelevantScriptStateOrReportError("DOMTask", "Create");
    ...
    DEVTOOLS_TIMELINE_TRACE_EVENT_INSTANT("SchedulePostTaskCallback", inspector_scheduler_schedule_event::Data, context, task_id_for_tracing_,
        task_queue_->GetPriority(), delay_.InMillisecondsF());
...
void DOMTask::Invoke()
{
    ...
    ScriptState* script_state = callback_->CallbackRelevantScriptStateOrReportError("DOMTask", "Invoke");
    if (!script_state || !script_state->ContextIsValid()) {
        DCHECK(resolver_->GetExecutionContext() && !resolver_->GetExecutionContext()->IsContextDestroyed());
...
void DOMTask::InvokeInternal(ScriptState* script_state)
{
    ...
    DEVTOOLS_TIMELINE_TRACE_EVENT(
        "RunPostTaskCallback", inspector_scheduler_run_event::Data, context, task_id_for_tracing_, task_queue_->GetPriority(), delay_.InMillisecondsF());
    ...
    if (auto* tracker = scheduler::TaskAttributionTracker::From(script_state->GetIsolate())) {
        task_attribution_scope = tracker->CreateTaskScope(
            script_state, parent_task_, scheduler::TaskAttributionTracker::TaskScopeType::kSchedulerPostTask, abort_source_, priority_source_);
    } else if (RuntimeEnabledFeatures::SchedulerYieldEnabled(ExecutionContext::From(script_state))) {
...
void DOMTask::OnAbort()
{
    ...
    DCHECK(context);
    DEVTOOLS_TIMELINE_TRACE_EVENT("AbortPostTaskCallback", inspector_scheduler_abort_event::Data, context, task_id_for_tracing_);

    // TODO(crbug.com/1293949): Add an error message.
    resolver_->Reject(abort_source_->reason(resolver_script_state).V8ValueFor(resolver_script_state));
}
