...
const char kForcedRendering[] = "Rendering was performed in a subtree hidden by content-visibility.";
const char kForcedRenderingMax[] = "Rendering was performed in a subtree hidden by content-visibility. "
...
void DisplayLockDocumentState::AddDisplayLockContext(DisplayLockContext* context)
{
    display_lock_contexts_.insert(context);
    context->SetShouldUnlockAutoForPrint(printing_);
}

void DisplayLockDocumentState::RemoveDisplayLockContext(DisplayLockContext* context)
{
...
{
    TRACE_COUNTER_ID1(TRACE_DISABLED_BY_DEFAULT("blink.debug.display_lock"), "LockedDisplayLockCount", TRACE_ID_LOCAL(this), locked_display_lock_count_);
...
void DisplayLockDocumentState::RegisterDisplayLockActivationObservation(Element* element)
{
    EnsureIntersectionObserver().observe(element);
}

void DisplayLockDocumentState::UnregisterDisplayLockActivationObservation(Element* element)
{
...
IntersectionObserver& DisplayLockDocumentState::EnsureIntersectionObserver()
{
    if (!intersection_observer_) {
        ...
        intersection_observer_ = IntersectionObserver::Create(*document_,
            WTF::BindRepeating(&DisplayLockDocumentState::ProcessDisplayLockActivationObservation, WrapWeakPersistent(this)),
...
void DisplayLockDocumentState::ProcessDisplayLockActivationObservation(const HeapVector<Member<IntersectionObserverEntry>>& entries)
{
    DCHECK(document_);
    ...
    for (auto& entry : entries) {
        ...
        if (context->HadAnyViewportIntersectionNotifications()) {
            if (entry->isIntersecting()) {
                document_->View()->EnqueueStartOfLifecycleTask(WTF::BindOnce(&DisplayLockContext::NotifyIsIntersectingViewport, WrapWeakPersistent(context)));
            } else {
                document_->View()->EnqueueStartOfLifecycleTask(
                    WTF::BindOnce(&DisplayLockContext::NotifyIsNotIntersectingViewport, WrapWeakPersistent(context)));
            }
    ...
    if (had_asynchronous_notifications) {
        ...
        document_->GetTaskRunner(TaskType::kInternalFrameLifecycleControl)
            ->PostTask(FROM_HERE, WTF::BindOnce(&DisplayLockDocumentState::ScheduleAnimation, WrapWeakPersistent(this)));
    }
}
...
DisplayLockDocumentState::ScopedForceActivatableDisplayLocks DisplayLockDocumentState::GetScopedForceActivatableLocks()
{
...
void DisplayLockDocumentState::ElementAddedToTopLayer(Element* element)
{
    ...
    // do this work later.
    if (document_->IsFlatTreeTraversalForbidden() || document_->GetSlotAssignmentEngine().HasPendingSlotAssignmentRecalc()) {
...
bool DisplayLockDocumentState::MarkAncestorContextsHaveTopLayerElement(Element* element)
{
...
void DisplayLockDocumentState::BeginNodeForcedScope(const Node* node, bool self_was_forced, DisplayLockUtilities::ScopedForcedUpdate::Impl* impl)
{
    forced_node_infos_.push_back(ForcedNodeInfo(node, self_was_forced, impl));
}

void DisplayLockDocumentState::BeginRangeForcedScope(const Range* range, DisplayLockUtilities::ScopedForcedUpdate::Impl* impl)
{
    forced_range_infos_.push_back(ForcedRangeInfo(range, impl));
}

void DisplayLockDocumentState::EndForcedScope(DisplayLockUtilities::ScopedForcedUpdate::Impl* impl)
{
...
void DisplayLockDocumentState::EnsureMinimumForcedPhase(DisplayLockContext::ForcedPhase phase)
{
...
void DisplayLockDocumentState::ForcedNodeInfo::ForceLockIfNeeded(Element* new_locked_element)
{
    auto ancestor_view = self_forced_ ? FlatTreeTraversal::InclusiveAncestorsOf(*node_) : FlatTreeTraversal::AncestorsOf(*node_);
    for (Node& ancestor : ancestor_view) {
        if (new_locked_element == &ancestor) {
            chain_->AddForcedUpdateScopeForContext(new_locked_element->GetDisplayLockContext());
            break;
        }
    }
}

void DisplayLockDocumentState::ForcedRangeInfo::ForceLockIfNeeded(Element* new_locked_element)
{
    ...
    // tree ancestors.
    for (Node* node = range_->FirstNode(); node != range_->PastLastNode(); node = NodeTraversal::Next(*node)) {
    ...
    for (Node* node = range_->FirstNode(); node; node = FlatTreeTraversal::Parent(*node)) {
...
DisplayLockDocumentState::ScopedForceActivatableDisplayLocks::ScopedForceActivatableDisplayLocks(DisplayLockDocumentState* state)
    : state_(state)
{
    if (++state_->activatable_display_locks_forced_ == 1) {
        for (auto context : state_->display_lock_contexts_) {
            if (context->HasElement()) {
                ...
                DCHECK(false) << "The DisplayLockContext's element has been garbage collected or"
...
DisplayLockDocumentState::ScopedForceActivatableDisplayLocks::ScopedForceActivatableDisplayLocks(ScopedForceActivatableDisplayLocks&& other)
    : state_(other.state_)
{
    other.state_ = nullptr;
}

DisplayLockDocumentState::ScopedForceActivatableDisplayLocks& DisplayLockDocumentState::ScopedForceActivatableDisplayLocks::operator=(
...
DisplayLockDocumentState::ScopedForceActivatableDisplayLocks::~ScopedForceActivatableDisplayLocks()
{
    if (!state_)
        return;
...
void DisplayLockDocumentState::IssueForcedRenderWarning(Element* element)
{
    ...
    if (forced_render_warnings_ < kMaxConsoleMessages) {
        ...
        auto level = RuntimeEnabledFeatures::WarnOnContentVisibilityRenderAccessEnabled() ? mojom::blink::ConsoleMessageLevel::kWarning
                                                                                          : mojom::blink::ConsoleMessageLevel::kVerbose;
        element->AddConsoleMessage(
            mojom::blink::ConsoleMessageSource::kJavaScript, level, forced_render_warnings_ == kMaxConsoleMessages ? kForcedRenderingMax : kForcedRendering);
    }
}
