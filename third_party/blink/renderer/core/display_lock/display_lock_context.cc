...
const char* kContainmentNotSatisfied = "Containment requirement is not satisfied.";
const char* kUnsupportedDisplay = "Element has unsupported display type (display: contents).";
} // namespace rejection_names
...
ScrollableArea* GetScrollableArea(Node* node)
{
    ...
    LayoutBoxModelObject* object = DynamicTo<LayoutBoxModelObject>(node->GetLayoutObject());
    if (!object)
        return nullptr;
...
void DisplayLockContext::SetRequestedState(EContentVisibility state)
{
    ...
    case EContentVisibility::kHidden:
        ...
        RequestLock(is_hidden_until_found_ || is_details_slot_ ? static_cast<uint16_t>(DisplayLockActivationReason::kFindInPage) : 0u);
        break;
    }
    ...
    // no-op.
    DCHECK(state_ != EContentVisibility::kAuto || !needs_deferred_not_intersecting_signal_);
    ...
    // object.
    element_->GetDocument().GetDisplayLockDocumentState().ForceLockIfNeeded(element_.Get());
}

const ComputedStyle* DisplayLockContext::AdjustElementStyle(const ComputedStyle* style) const
{
...
void DisplayLockContext::UpdateActivationMask(uint16_t activatable_mask)
{
    ...
    UpdateDocumentBookkeeping(IsLocked(), all_activation_was_blocked, IsLocked(), all_activation_is_blocked);

    activatable_mask_ = activatable_mask;
}

void DisplayLockContext::UpdateDocumentBookkeeping(bool was_locked, bool all_activation_was_blocked, bool is_locked, bool all_activation_is_blocked)
{
    if (!document_)
        return;
    ...
    if (was_locked_and_blocking != is_locked_and_blocking) {
        if (is_locked_and_blocking) {
            document_->GetDisplayLockDocumentState().IncrementDisplayLockBlockingAllActivation();
        } else {
            document_->GetDisplayLockDocumentState().DecrementDisplayLockBlockingAllActivation();
        }
    }
}
...
void DisplayLockContext::UpdateActivationObservationIfNeeded()
{
    ...
    // view.
    bool should_observe = state_ == EContentVisibility::kAuto && ConnectedToView();
    ...
    if (should_observe) {
        document_->GetDisplayLockDocumentState().RegisterDisplayLockActivationObservation(element_);
    } else {
        document_->GetDisplayLockDocumentState().UnregisterDisplayLockActivationObservation(element_);
...
{
    return needs_deferred_not_intersecting_signal_ || render_affecting_state_[static_cast<int>(RenderAffectingState::kAutoStateUnlockedUntilLifecycle)]
        || has_pending_subtree_checks_ || has_pending_clear_has_top_layer_ || has_pending_top_layer_check_ || anchor_positioning_render_state_may_have_changed_;
}
...
void DisplayLockContext::Lock()
{
    DCHECK(!IsLocked());
    is_locked_ = true;
    UpdateDocumentBookkeeping(false, !activatable_mask_, true, !activatable_mask_);
    ...
    if (CanDirtyStyle()) {
        element_->SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kDisplayLock));
...
void DisplayLockContext::CommitForActivation(DisplayLockActivationReason reason)
{
    DCHECK(element_);
    ...
    if (reason == DisplayLockActivationReason::kAccessibility || reason == DisplayLockActivationReason::kFindInPage
        || reason == DisplayLockActivationReason::kFragmentNavigation || reason == DisplayLockActivationReason::kScrollIntoView
...
void DisplayLockContext::SetKeepUnlockedUntilLifecycleCount(int count)
{
    ...
    SetRenderAffectingState(RenderAffectingState::kAutoStateUnlockedUntilLifecycle, true);
    UpdateLifecycleNotificationRegistration();
    ScheduleAnimation();
}
...
void DisplayLockContext::NotifyIsNotIntersectingViewport()
{
    ...
    auto* locked_ancestor = DisplayLockUtilities::LockedAncestorPreventingPaint(*element_);
    if (locked_ancestor) {
...
bool DisplayLockContext::ShouldCommitForActivation(DisplayLockActivationReason reason) const
{
    return IsActivatable(reason) && IsLocked();
}

void DisplayLockContext::UpgradeForcedScope(ForcedPhase old_phase, ForcedPhase new_phase, bool emit_warnings)
{
    ...
    if (IsLocked()) {
        ...
        if (CanDirtyStyle() && !old_forced_info.is_forced(ForcedPhase::kStyleAndLayoutTree) && forced_info_.is_forced(ForcedPhase::kStyleAndLayoutTree)) {
            MarkForStyleRecalcIfNeeded();
        }
        if (!old_forced_info.is_forced(ForcedPhase::kLayout) && forced_info_.is_forced(ForcedPhase::kLayout)) {
            MarkForLayoutIfNeeded();
        }
        if (!old_forced_info.is_forced(ForcedPhase::kPrePaint) && forced_info_.is_forced(ForcedPhase::kPrePaint)) {
            MarkAncestorsForPrePaintIfNeeded();
        }

        if (emit_warnings && document_ && document_->GetAgent().isolate()->InContext() && element_
            && (!IsActivatable(DisplayLockActivationReason::kAny) || RuntimeEnabledFeatures::WarnOnContentVisibilityRenderAccessEnabled())) {
            document_->GetDisplayLockDocumentState().IssueForcedRenderWarning(element_);
        }
    }
}
...
void DisplayLockContext::ScheduleStateChangeEventIfNeeded()
{
    if (state_ == EContentVisibility::kAuto && !state_change_task_pending_) {
        document_->GetExecutionContext()
            ...
            ->PostTask(FROM_HERE, WTF::BindOnce(&DisplayLockContext::DispatchStateChangeEventIfNeeded, WrapPersistent(this)));
...
void DisplayLockContext::DispatchStateChangeEventIfNeeded()
{
    ...
    if (!last_notified_skipped_state_ || *last_notified_skipped_state_ != is_locked_) {
        last_notified_skipped_state_ = is_locked_;
        element_->DispatchEvent(*ContentVisibilityAutoStateChangeEvent::Create(event_type_names::kContentvisibilityautostatechange, is_locked_));
    }
}
...
void DisplayLockContext::Unlock()
{
    ...
    UpdateDocumentBookkeeping(true, !activatable_mask_, false, !activatable_mask_);
    ...
    if (CanDirtyStyle()) {
        ...
        blocked_child_recalc_change_ = blocked_child_recalc_change_.ForceRecalcDescendants();
    }
    ...
    if (IsElementDirtyForStyleRecalc()) {
        ...
        for (auto top_layer_element : document_->TopLayerElements()) {
            if (!top_layer_element->GetComputedStyle()) {
                top_layer_element->SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kDisplayLock));
            }
        }
        return true;
    }
...
bool DisplayLockContext::MarkForLayoutIfNeeded()
{
    if (IsElementDirtyForLayout()) {
        ...
        public:
            ...
            ~ScopedForceLayout()
            {
                context_->forced_info_.end(ForcedPhase::kLayout);
            }
...
bool DisplayLockContext::MarkAncestorsForPrePaintIfNeeded()
{
    ...
    if (IsElementDirtyForPrePaint()) {
        ...
        if (needs_effective_allowed_touch_action_update_ || layout_object->EffectiveAllowedTouchActionChanged()
        ...
        if (needs_blocking_wheel_event_handler_update_ || layout_object->BlockingWheelEventHandlerChanged()
...
bool DisplayLockContext::IsElementDirtyForStyleRecalc() const
{
    ...
    return element_->IsDirtyForStyleRecalc() || element_->ChildNeedsStyleRecalc() || element_->ChildNeedsReattachLayoutTree()
...
{
    if (auto* layout_object = element_->GetLayoutObject()) {
        return layout_object->NeedsLayout() || child_layout_was_blocked_ || HasStashedScrollOffset();
    }
    return false;
}
...
{
    if (auto* layout_object = element_->GetLayoutObject()) {
        return PrePaintTreeWalk::ObjectRequiresPrePaint(*layout_object) || PrePaintTreeWalk::ObjectRequiresTreeBuilderContext(*layout_object)
            || needs_prepaint_subtree_walk_ || needs_effective_allowed_touch_action_update_ || needs_blocking_wheel_event_handler_update_;
    }
    return false;
}
...
void DisplayLockContext::DidMoveToNewDocument(Document& old_document)
{
    ...
    if (is_observed_) {
        old_document.GetDisplayLockDocumentState().UnregisterDisplayLockActivationObservation(element_);
        document_->GetDisplayLockDocumentState().RegisterDisplayLockActivationObservation(element_);
    }
    ...
    if (IsLocked()) {
        ...
        if (!IsActivatable(DisplayLockActivationReason::kAny)) {
            old_document.GetDisplayLockDocumentState().DecrementDisplayLockBlockingAllActivation();
            document_->GetDisplayLockDocumentState().IncrementDisplayLockBlockingAllActivation();
        }
    }
...
void DisplayLockContext::WillStartLifecycleUpdate(const LocalFrameView& view)
{
    ...
    if (keep_unlocked_count_) {
        if (--keep_unlocked_count_) {
            ...
            SetRenderAffectingState(RenderAffectingState::kAutoStateUnlockedUntilLifecycle, false);
            update_registration = true;
        }
    } else {
        DCHECK(!render_affecting_state_[static_cast<int>(RenderAffectingState::kAutoStateUnlockedUntilLifecycle)]);
    }
    ...
    if (has_pending_clear_has_top_layer_) {
        SetRenderAffectingState(RenderAffectingState::kSubtreeHasTopLayerElement, false);
...
void DisplayLockContext::ElementDisconnected()
{
    ...
    if (auto* document_rules = DocumentSpeculationRules::FromIfExists(*document_)) {
...
const char* DisplayLockContext::ShouldForceUnlock() const
{
    ...
    if (const auto* object_element = DynamicTo<HTMLObjectElement>(*element_)) {
        if (!object_element->UseFallbackContent())
            return nullptr;
    } else if (IsA<HTMLImageElement>(*element_) || IsA<HTMLCanvasElement>(*element_) || (element_->IsFormControlElement() && !element_->IsOutputElement())
        || element_->IsMediaElement() || element_->IsFrameOwnerElement() || element_->IsSVGElement()) {
        return nullptr;
    }
    ...
    if ((style->IsDisplayTableType() && style->Display() != EDisplay::kTableCell) || style->Display() == EDisplay::kRubyText
...
void DisplayLockContext::DetermineIfSubtreeHasFocus()
{
    ...
    for (auto* focused = document_->FocusedElement(); focused; focused = FlatTreeTraversal::ParentElement(*focused)) {
        if (focused == element_.Get()) {
            subtree_has_focus = true;
            break;
        }
    }
    SetRenderAffectingState(RenderAffectingState::kSubtreeHasFocus, subtree_has_focus);
}
...
void DisplayLockContext::DetermineIfDescendantIsViewTransitionElement()
{
    ...
    if (ConnectedToView()) {
        document_->GetDisplayLockDocumentState().UpdateViewTransitionElementAncestorLocks();
    }
}

void DisplayLockContext::ResetDescendantIsViewTransitionElement()
{
    SetRenderAffectingState(RenderAffectingState::kDescendantIsViewTransitionElement, false);
}

void DisplayLockContext::SetDescendantIsViewTransitionElement()
{
    SetRenderAffectingState(RenderAffectingState::kDescendantIsViewTransitionElement, true);
}
...
{
    has_pending_clear_has_top_layer_ = false;
    SetRenderAffectingState(RenderAffectingState::kSubtreeHasTopLayerElement, true);
    UpdateLifecycleNotificationRegistration();
}

bool DisplayLockContext::SubtreeHasTopLayerElement() const
{
    return render_affecting_state_[static_cast<int>(RenderAffectingState::kSubtreeHasTopLayerElement)];
}
...
void DisplayLockContext::DetermineIfSubtreeHasSelection()
{
    ...
    auto range = ToEphemeralRangeInFlatTree(document_->GetFrame()->Selection().GetSelectionInDOMTree().ComputeRange());
    ...
    SetRenderAffectingState(RenderAffectingState::kSubtreeHasSelection, subtree_has_selection);
}

void DisplayLockContext::SetRenderAffectingState(RenderAffectingState state, bool new_flag)
{
    ...
    if (state == RenderAffectingState::kSubtreeHasSelection && !new_flag && document_->GetDisplayLockDocumentState().ActivatableDisplayLocksForced()) {
...
{
    auto state = [this](RenderAffectingState state) { return render_affecting_state_[static_cast<int>(state)]; };

    // Check that we're visible if and only if lock has not been requested.
    DCHECK_EQ(state_ == EContentVisibility::kVisible, !state(RenderAffectingState::kLockRequested));
    ...
    bool should_be_locked = state(RenderAffectingState::kLockRequested)
        && (state_ != EContentVisibility::kAuto
            || (!state(RenderAffectingState::kIntersectsViewport) && !state(RenderAffectingState::kSubtreeHasFocus)
                && !state(RenderAffectingState::kSubtreeHasSelection) && !state(RenderAffectingState::kAutoStateUnlockedUntilLifecycle)
                && !state(RenderAffectingState::kAutoUnlockedForPrint) && !state(RenderAffectingState::kSubtreeHasTopLayerElement)
                && !state(RenderAffectingState::kDescendantIsViewTransitionElement) && !state(RenderAffectingState::kDescendantIsAnchorTarget)));
...
bool DisplayLockContext::DescendantIsAnchorTargetFromOutsideDisplayLock()
{
    for (auto* obj = element_->GetLayoutObject(); obj; obj = obj->Container()) {
        if (const auto* ancestor_box = DynamicTo<LayoutBox>(obj)) {
            ...
            for (const PhysicalBoxFragment& fragment : ancestor_box->PhysicalFragments()) {
                ...
                for (const PhysicalFragmentLink& fragment_child : fragment.Children()) {
                    ...
                    if (auto* box = DynamicTo<LayoutBox>(fragment_child->GetMutableLayoutObject())) {
...
String DisplayLockContext::RenderAffectingStateToString() const
{
    ...
    for (int i = 0; i < static_cast<int>(RenderAffectingState::kNumRenderAffectingStates); ++i) {
...
void DisplayLockContext::RestoreScrollOffsetIfStashed()
{
    ...
    if (auto* area = GetScrollableArea(element_)) {
        area->SetScrollOffset(*stashed_scroll_offset_, mojom::blink::ScrollType::kAnchoring);
...
{
    return document_->GetDisplayLockDocumentState().ActivatableDisplayLocksForced();
}
