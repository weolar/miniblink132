...
public:
    ...
    gfx::Rect ActiveMatchRect() const
    {
...
public:
    ...
    {
        if (activatable) {
            element.setAttribute(html_names::kHiddenAttr, AtomicString("until-found"));
        } else {
            element.setAttribute(html_names::kStyleAttr, AtomicString("content-visibility: hidden"));
        }
        UpdateAllLifecyclePhasesForTest();
    }
    ...
    void Find(String search_text, DisplayLockTestFindInPageClient& client, bool new_session = true)
    {
        client.Reset();
    ...
    {
        return context->render_affecting_state_[static_cast<int>(DisplayLockContext::RenderAffectingState::kSubtreeHasSelection)];
    }
    DisplayLockUtilities::ScopedForcedUpdate GetScopedForcedUpdate(const Node* node, DisplayLockContext::ForcedPhase phase, bool include_self = false)
    {
...
TEST_F(DisplayLockContextTest, LockAfterAppendStyleDirtyBits)
{
    ...
    EXPECT_EQ(GetDocument().GetDisplayLockDocumentState().LockedDisplayLockCount(), 1);

    // If the element is dirty, style recalc would handle it in the next recalc.
    element->setAttribute(html_names::kStyleAttr, AtomicString("content-visibility: hidden; color: red;"));
    ...
    EXPECT_EQ(element->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()), Color::FromRGB(255, 0, 0));
    ...
    // Lock the child.
    child->setAttribute(html_names::kStyleAttr, AtomicString("content-visibility: hidden; color: blue;"));
    UpdateAllLifecyclePhasesForTest();
    ...
    EXPECT_EQ(child->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()), Color::FromRGB(0, 0, 255));
...
TEST_F(DisplayLockContextTest, ActivatableLockedElementIsSearchableViaFindInPage)
{
    ResizeAndFocus();
    SetHtmlInnerHTML(R"HTML(
    <style>
    ...
    GetFindInPage()->StopFinding(mojom::StopFindAction::kStopFindActionClearSelection);
...
TEST_F(DisplayLockContextTest, FindInPageContinuesAfterRelock)
{
    ...
    // Clears selections since we're going to use the same query next time.
    GetFindInPage()->StopFinding(mojom::StopFindAction::kStopFindActionKeepSelection);
...
TEST_F(DisplayLockContextTest, ActivatableLockedElementTickmarksAreAtLockedRoots)
{
    ResizeAndFocus();
    SetHtmlInnerHTML(R"HTML(
    <style>
    ...
    std::sort(tick_rects.begin(), tick_rects.end(), [](const gfx::Rect& a, const gfx::Rect& b) { return a.y() < b.y(); });
...
TEST_F(DisplayLockContextTest, FindInPageWhileLockedContentChangesDoesNotCrash)
{
    ResizeAndFocus();
    SetHtmlInnerHTML(R"HTML(
    <style>
...
TEST_F(DisplayLockContextTest, FindInPageWithChangedContent)
{
    ...
    LockElement(*container, true /* activatable */);
    EXPECT_TRUE(container->GetDisplayLockContext()->IsLocked());
    container->setInnerHTML("testing"
                            "<div>testing</div>"
...
TEST_F(DisplayLockContextTest, NestedActivatableLockedElementIsSearchableViaFindInPage)
{
    ResizeAndFocus();
    SetHtmlInnerHTML(R"HTML(
    <body>
    ...
    auto* non_activatable = GetDocument().getElementById(AtomicString("nonActivatable"));
    auto* nested_non_activatable = GetDocument().getElementById(AtomicString("nestedNonActivatable"));
...
TEST_F(DisplayLockContextTest, NestedActivatableLockedElementIsNotUnlockedByFindInPage)
{
    ResizeAndFocus();
    SetHtmlInnerHTML(R"HTML(
    <body>
...
TEST_F(DisplayLockContextTest, CallUpdateStyleAndLayoutAfterChange)
{
    ...
    EXPECT_FALSE(element->GetDisplayLockContext()->ShouldPaintChildren());
    EXPECT_EQ(GetDocument().GetDisplayLockDocumentState().LockedDisplayLockCount(), 1);
    EXPECT_EQ(GetDocument().GetDisplayLockDocumentState().DisplayLockBlockingAllActivationCount(), 1);
...
TEST_F(DisplayLockContextTest, LockedElementAndDescendantsAreNotFocusable)
{
    ...
    ASSERT_TRUE(GetDocument().getElementById(AtomicString("textfield"))->IsKeyboardFocusable());
    ASSERT_TRUE(GetDocument().getElementById(AtomicString("textfield"))->IsFocusable());
    ASSERT_TRUE(GetDocument().getElementById(AtomicString("textfield"))->IsFocusable());
    EXPECT_EQ(GetDocument().GetDisplayLockDocumentState().LockedDisplayLockCount(), 0);
    EXPECT_EQ(GetDocument().GetDisplayLockDocumentState().DisplayLockBlockingAllActivationCount(), 0);
    ...
    EXPECT_FALSE(GetDocument().getElementById(AtomicString("textfield"))->IsKeyboardFocusable());
    EXPECT_FALSE(GetDocument().getElementById(AtomicString("textfield"))->IsFocusable());
    EXPECT_FALSE(GetDocument().getElementById(AtomicString("textfield"))->IsFocusable());
    ...
    EXPECT_EQ(GetDocument().GetDisplayLockDocumentState().LockedDisplayLockCount(), 0);
    EXPECT_EQ(GetDocument().GetDisplayLockDocumentState().DisplayLockBlockingAllActivationCount(), 0);
    EXPECT_TRUE(GetDocument().getElementById(AtomicString("textfield"))->IsKeyboardFocusable());
    EXPECT_TRUE(GetDocument().getElementById(AtomicString("textfield"))->IsFocusable());
    EXPECT_TRUE(GetDocument().getElementById(AtomicString("textfield"))->IsFocusable());

    // Calling explicit focus() should focus the element
    GetDocument().getElementById(AtomicString("textfield"))->Focus();
    EXPECT_EQ(GetDocument().FocusedElement(), GetDocument().getElementById(AtomicString("textfield")));
}
...
TEST_F(DisplayLockContextTest, DisplayLockPreventsActivation)
{
    ...
    ASSERT_FALSE(DisplayLockUtilities::ShouldIgnoreNodeDueToDisplayLock(*host, DisplayLockActivationReason::kAny));
    ASSERT_FALSE(DisplayLockUtilities::ShouldIgnoreNodeDueToDisplayLock(*slotted, DisplayLockActivationReason::kAny));

    ShadowRoot& shadow_root = host->AttachShadowRootForTesting(ShadowRootMode::kOpen);
    shadow_root.setInnerHTML("<div id='container' style='contain:style layout "
                             "paint;'><slot></slot></div>");
    UpdateAllLifecyclePhasesForTest();

    auto* container = shadow_root.getElementById(AtomicString("container"));
    ASSERT_FALSE(DisplayLockUtilities::ShouldIgnoreNodeDueToDisplayLock(*host, DisplayLockActivationReason::kAny));
    ASSERT_FALSE(DisplayLockUtilities::ShouldIgnoreNodeDueToDisplayLock(*container, DisplayLockActivationReason::kAny));
    ASSERT_FALSE(DisplayLockUtilities::ShouldIgnoreNodeDueToDisplayLock(*slotted, DisplayLockActivationReason::kAny));
    ...
    ASSERT_FALSE(DisplayLockUtilities::ShouldIgnoreNodeDueToDisplayLock(*container, DisplayLockActivationReason::kAny));
    ASSERT_TRUE(DisplayLockUtilities::ShouldIgnoreNodeDueToDisplayLock(*slotted, DisplayLockActivationReason::kAny));
    ...
    auto* non_viewport = GetDocument().getElementById(AtomicString("nonviewport-child"));

    EXPECT_EQ(GetDocument().GetDisplayLockDocumentState().LockedDisplayLockCount(), 1);
    EXPECT_EQ(GetDocument().GetDisplayLockDocumentState().DisplayLockBlockingAllActivationCount(), 0);

    ASSERT_FALSE(DisplayLockUtilities::ShouldIgnoreNodeDueToDisplayLock(*non_viewport, DisplayLockActivationReason::kAny));
    ASSERT_FALSE(DisplayLockUtilities::ShouldIgnoreNodeDueToDisplayLock(*non_viewport, DisplayLockActivationReason::kFindInPage));
    ASSERT_TRUE(DisplayLockUtilities::ShouldIgnoreNodeDueToDisplayLock(*non_viewport, DisplayLockActivationReason::kUserFocus));
}

TEST_F(DisplayLockContextTest, LockedElementAndFlatTreeDescendantsAreNotFocusable)
{
    ResizeAndFocus();
    SetHtmlInnerHTML(R"HTML(
    <body>
...
TEST_F(DisplayLockContextTest, LockedCountsWithMultipleLocks)
{
    ...
    EXPECT_EQ(GetDocument().GetDisplayLockDocumentState().LockedDisplayLockCount(), 2);
    EXPECT_EQ(GetDocument().GetDisplayLockDocumentState().DisplayLockBlockingAllActivationCount(), 2);
...
TEST_F(DisplayLockContextTest, ActivatableNotCountedAsBlocking)
{
    ...
    // activation for it.
    EXPECT_TRUE(activatable->EnsureDisplayLockContext().IsActivatable(DisplayLockActivationReason::kAny));

    LockElement(*activatable, true);

    EXPECT_EQ(GetDocument().GetDisplayLockDocumentState().LockedDisplayLockCount(), 1);
    EXPECT_EQ(GetDocument().GetDisplayLockDocumentState().DisplayLockBlockingAllActivationCount(), 0);
    EXPECT_TRUE(activatable->GetDisplayLockContext()->IsActivatable(DisplayLockActivationReason::kAny));

    LockElement(*non_activatable, false);

    EXPECT_EQ(GetDocument().GetDisplayLockDocumentState().LockedDisplayLockCount(), 2);
    EXPECT_EQ(GetDocument().GetDisplayLockDocumentState().DisplayLockBlockingAllActivationCount(), 1);
    EXPECT_FALSE(non_activatable->GetDisplayLockContext()->IsActivatable(DisplayLockActivationReason::kAny));
...
TEST_F(DisplayLockContextTest, ElementInTemplate)
{
    ...
    auto* template_el = To<HTMLTemplateElement>(GetDocument().getElementById(AtomicString("template")));
    ...
    auto* document_child = To<Element>(GetDocument().adoptNode(child, ASSERT_NO_EXCEPTION));
    auto* container = GetDocument().getElementById(AtomicString("container"));
    ...
    container->setAttribute(html_names::kStyleAttr, AtomicString("display: block;"));
    ...
    document_child->setAttribute(html_names::kStyleAttr, AtomicString("content-visibility: hidden; color: red;"));
    UpdateAllLifecyclePhasesForTest();

    EXPECT_FALSE(document_child->NeedsStyleRecalc());

    // Commit will unlock the element and update the style.
    document_child->setAttribute(html_names::kStyleAttr, AtomicString("color: red;"));
    UpdateAllLifecyclePhasesForTest();
    ...
    EXPECT_EQ(document_child->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()), Color::FromRGB(255, 0, 0));
    ...
    EXPECT_EQ(grandchild->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()), Color::FromRGB(0, 0, 255));
}
...
TEST_F(DisplayLockContextTest, AncestorAllowedTouchAction)
{
    ...
    auto* ancestor_element = GetDocument().getElementById(AtomicString("ancestor"));
    auto* handler_element = GetDocument().getElementById(AtomicString("handler"));
    auto* descendant_element = GetDocument().getElementById(AtomicString("descendant"));
    auto* locked_element = GetDocument().getElementById(AtomicString("locked"));
    auto* lockedchild_element = GetDocument().getElementById(AtomicString("lockedchild"));
    ...
    EXPECT_FALSE(descendant_object->DescendantEffectiveAllowedTouchActionChanged());
    EXPECT_FALSE(locked_object->DescendantEffectiveAllowedTouchActionChanged());
    EXPECT_FALSE(lockedchild_object->DescendantEffectiveAllowedTouchActionChanged());
    ...
    EXPECT_TRUE(descendant_object->DescendantEffectiveAllowedTouchActionChanged());
    EXPECT_FALSE(locked_object->DescendantEffectiveAllowedTouchActionChanged());
    EXPECT_FALSE(lockedchild_object->DescendantEffectiveAllowedTouchActionChanged());
...
TEST_F(DisplayLockContextTest, AncestorWheelEventHandler)
{
    ...
    EXPECT_FALSE(lockedchild_object->DescendantBlockingWheelEventHandlerChanged());
...
public:
    ...
    bool DescendantDependentFlagUpdateWasBlocked(DisplayLockContext* context) const
    {
    ...
    {
        auto start_of_lifecycle_tasks = GetDocument().View()->TakeStartOfLifecycleTasksForTest();
...
TEST_F(DisplayLockContextRenderingTest, VisualOverflowCalculateOnChildPaintLayer)
{
    SetHtmlInnerHTML(R"HTML(
    <style>
    ...
    EXPECT_TRUE(DescendantDependentFlagUpdateWasBlocked(lockable->GetDisplayLockContext()));
    EXPECT_TRUE(child_layer->NeedsVisualOverflowRecalc());
...
TEST_F(DisplayLockContextRenderingTest, FloatChildLocked)
{
    ...
    EXPECT_EQ(PhysicalRect(0, 0, 200, 100), lockable_box->ScrollableOverflowRect());
    ...
    EXPECT_EQ(PhysicalRect(0, 0, 200, 50), lockable_box->ScrollableOverflowRect());
    ...
    EXPECT_EQ(PhysicalRect(0, 0, 200, 200), lockable_box->ScrollableOverflowRect());
}

TEST_F(DisplayLockContextRenderingTest, VisualOverflowCalculateOnChildPaintLayerInForcedLock)
{
    SetHtmlInnerHTML(R"HTML(
    <style>
      .hidden { content-visibility: hidden }
    ...
    ASSERT_TRUE(lockable->GetDisplayLockContext());
    {
        auto scope = GetScopedForcedUpdate(lockable, DisplayLockContext::ForcedPhase::kPrePaint, true /* include self */);
    ...
    // processed.
    EXPECT_FALSE(DescendantDependentFlagUpdateWasBlocked(lockable->GetDisplayLockContext()));
    EXPECT_FALSE(child_layer->NeedsVisualOverflowRecalc());
...
TEST_F(DisplayLockContextRenderingTest, SelectionOnAnonymousColumnSpannerDoesNotCrash)
{
    SetHtmlInnerHTML(R"HTML(
    <style>
      #columns {
    ...
    auto* columns_object = GetDocument().getElementById(AtomicString("columns"))->GetLayoutObject();
    LayoutObject* spanner_placeholder_object = nullptr;
    for (auto* candidate = columns_object->SlowFirstChild(); candidate; candidate = candidate->NextSibling()) {
...
TEST_F(DisplayLockContextRenderingTest, ObjectsNeedingLayoutConsidersLocks)
{
    ...
    auto dirty_all = [this]() {
        GetDocument().getElementById(AtomicString("c"))->GetLayoutObject()->SetNeedsLayout("test");
        GetDocument().getElementById(AtomicString("d"))->GetLayoutObject()->SetNeedsLayout("test");
        GetDocument().getElementById(AtomicString("f"))->GetLayoutObject()->SetNeedsLayout("test");
        GetDocument().getElementById(AtomicString("g"))->GetLayoutObject()->SetNeedsLayout("test");
    };
    ...
    dirty_all();
    GetDocument().View()->CountObjectsNeedingLayout(dirty_count, total_count, is_subtree);
    // 7 divs + body + html + layout view
    EXPECT_EQ(dirty_count, 10u);
    EXPECT_EQ(total_count, 10u);

    GetDocument().getElementById(AtomicString("e"))->setAttribute(html_names::kStyleAttr, AtomicString("content-visibility: hidden"));
    UpdateAllLifecyclePhasesForTest();
    ...
    GetDocument().getElementById(AtomicString("a"))->setAttribute(html_names::kStyleAttr, AtomicString("content-visibility: hidden"));
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(DisplayLockContextRenderingTest, PaintDirtyBitsNotPropagatedAcrossBoundary)
{
    SetHtmlInnerHTML(R"HTML(
    <style>
...
TEST_F(DisplayLockContextRenderingTest, NestedLockDoesNotInvalidateOnHideOrShow)
{
    SetHtmlInnerHTML(R"HTML(
    <style>
      .auto { content-visibility: auto; }
    ...
    auto* unrelated_element = GetDocument().getElementById(AtomicString("unrelated"));
...
TEST_F(DisplayLockContextRenderingTest, NestedLockDoesHideWhenItIsOffscreen)
{
    ...
    GetDocument().getElementById(AtomicString("future_spacer"))->setAttribute(html_names::kClassAttr, AtomicString("spacer"));
...
TEST_F(DisplayLockContextRenderingTest, LockedCanvasWithFallbackHasFocusableStyle)
{
    SetHtmlInnerHTML(R"HTML(
    <style>
      .auto { content-visibility: auto; }
...
TEST_F(DisplayLockContextRenderingTest, LayoutRootIsProcessedIfLockedAndForced)
{
    SetHtmlInnerHTML(R"HTML(
    <style>
      .hidden { content-visibility: hidden; }
    ...
    {
        auto scope = GetScopedForcedUpdate(hide, DisplayLockContext::ForcedPhase::kLayout, true /* include self */);
...
TEST_F(DisplayLockContextRenderingTest, UseCounter)
{
    ...
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kContentVisibilityHidden));

    GetDocument().getElementById(AtomicString("e1"))->classList().Add(AtomicString("auto"));
    UpdateAllLifecyclePhasesForTest();

    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kContentVisibilityAuto));
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kContentVisibilityHidden));

    GetDocument().getElementById(AtomicString("e2"))->classList().Add(AtomicString("hidden"));
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(DisplayLockContextRenderingTest, NeedsLayoutTreeUpdateForNodeRespectsForcedLocks)
{
    SetHtmlInnerHTML(R"HTML(
    <style>
      .hidden { content-visibility: hidden; }
    ...
    auto scope = GetScopedForcedUpdate(hide, DisplayLockContext::ForcedPhase::kPrePaint, true /* include self */);
...
TEST_F(DisplayLockContextRenderingTest, AutoReachesStableStateOnContentSmallerThanLockedSize)
{
    SetHtmlInnerHTML(R"HTML(
    <style>
...
TEST_F(DisplayLockContextRenderingTest, AutoReachesStableStateOnContentSmallerThanLockedSizeInLtr)
{
    SetHtmlInnerHTML(R"HTML(
    <style>
...
TEST_F(DisplayLockContextRenderingTest, AutoReachesStableStateOnContentSmallerThanLockedSizeInRtl)
{
    SetHtmlInnerHTML(R"HTML(
    <style>
...
TEST_F(DisplayLockContextRenderingTest, HadIntersectionNotificationsResetsWhenConnected)
{
    SetHtmlInnerHTML(R"HTML(
    <style>
      .auto { content-visibility: auto; }
...
TEST_F(DisplayLockContextTest, CullRectUpdate)
{
    ...
    EXPECT_EQ(gfx::Rect(0, 0, 100, 100), target->FirstFragment().GetCullRect().Rect());

    container->classList().Add(AtomicString("locked"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(gfx::Rect(0, 0, 100, 100), target->FirstFragment().GetCullRect().Rect());

    GetDocument().getElementById(AtomicString("clip"))->setAttribute(html_names::kStyleAttr, AtomicString("width: 200px"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(gfx::Rect(0, 0, 100, 100), target->FirstFragment().GetCullRect().Rect());

    container->classList().Remove(AtomicString("locked"));
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(gfx::Rect(0, 0, 200, 100), target->FirstFragment().GetCullRect().Rect());
}
...
TEST_F(DisplayLockContextTest, ConnectedElementDefersSubtreeChecks)
{
    ...
    range->setStart(GetDocument().getElementById(AtomicString("s1"))->firstChild(), 0);
    range->setEnd(GetDocument().getElementById(AtomicString("s2"))->firstChild(), 5);

    Selection().SetSelection(SelectionInDOMTree::Builder().SetBaseAndExtent(EphemeralRange(range)).Build(), SetSelectionOptions());

    UpdateAllLifecyclePhasesForTest();

    EXPECT_FALSE(HasSelection(context));

    GetDocument().body()->insertBefore(locked, GetDocument().getElementById(AtomicString("s2")));
...
TEST_F(DisplayLockContextTest, ReattachPropagationBlockedByDisplayLock)
{
    ...
    auto scope = GetScopedForcedUpdate(grandchild, DisplayLockContext::ForcedPhase::kStyleAndLayoutTree);
...
TEST_F(DisplayLockContextTest, NoUpdatesInDisplayNone)
{
    ...
    auto* displaylocked = GetDocument().getElementById(AtomicString("displaylocked"));
    auto* child = GetDocument().getElementById(AtomicString("child"));
...
TEST_F(DisplayLockContextTest, ElementActivateDisplayLockIfNeeded)
{
    ...
    EXPECT_TRUE(target->ActivateDisplayLockIfNeeded(DisplayLockActivationReason::kScrollIntoView));
}
...
TEST_F(DisplayLockContextTest, ShouldForceUnlockObjectWithFallbackContent)
{
    ...
    UpdateAllLifecyclePhasesForTest();
    auto* target = To<HTMLPlugInElement>(GetDocument().getElementById(AtomicString("target")));
