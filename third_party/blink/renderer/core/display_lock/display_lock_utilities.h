...
public:
    ...
    public:
        ScopedForcedUpdate(ScopedForcedUpdate&& other)
            : impl_(other.impl_)
        {
            other.impl_ = nullptr;
        }
    ...
    private:
        ...
        friend void Document::UpdateStyleAndLayoutForNode(const Node* node, DocumentUpdateReason reason);
        friend void Document::UpdateStyleAndLayoutForRange(const Range* range, DocumentUpdateReason reason);
        friend void Document::UpdateStyleAndLayoutTreeForElement(const Element* node, DocumentUpdateReason reason);
        friend void Document::UpdateStyleAndLayoutTreeForSubtree(const Element* node, DocumentUpdateReason reason);
        friend void Document::EnsurePaintLocationDataValidForNode(const Node* node, DocumentUpdateReason reason);
        friend VisibleSelection FrameSelection::ComputeVisibleSelectionInDOMTreeDeprecated() const;
        ...
        explicit ScopedForcedUpdate(
            const Node* node, DisplayLockContext::ForcedPhase phase, bool include_self = false, bool only_cv_auto = false, bool emit_warnings = true)
            : impl_(MakeGarbageCollected<Impl>(node, phase, include_self, only_cv_auto, emit_warnings))
        {
        }
        explicit ScopedForcedUpdate(const Range* range, DisplayLockContext::ForcedPhase phase, bool only_cv_auto = false, bool emit_warnings = true)
        ...
        public:
            Impl(const Node* node, DisplayLockContext::ForcedPhase phase, bool include_self = false, bool only_cv_auto = false, bool emit_warnings = true);
            Impl(const Range* range, DisplayLockContext::ForcedPhase phase, bool only_cv_auto = false, bool emit_warnings = true);
    ...
    private:
        ...
        bool IsMemoizationScopeFull()
        {
            ...
            return (nodes_preventing_paint.size() + nodes_preventing_accessibility.size() + unlocked_nodes.size()) >= kTotalMemoizedNodeLimit;
        }
    ...
    static bool ActivateFindInPageMatchRangeIfNeeded(const EphemeralRangeInFlatTree& range);
    static bool NeedsActivationForFindInPage(const EphemeralRangeInFlatTree& range);
    ...
    // activatable-locked).
    static const HeapVector<Member<Element>> ActivatableLockedInclusiveAncestors(const Node& node, DisplayLockActivationReason reason);

    // Ancestor navigation functions.

    // Helpers for ancestor navigation to find locks.
    static const Element* LockedInclusiveAncestorPreventingLayout(const Node& node);
    static const Element* LockedInclusiveAncestorPreventingPaint(const LayoutObject& object);
    static const Element* LockedInclusiveAncestorPreventingPaint(const Node& node);
    ...
    static bool IsDisplayLockedPreventingPaint(const Node* node, bool inclusive_check = false);
    ...
    static Element* LockedInclusiveAncestorPreventingStyleWithinTreeScope(const Node& node);
    ...
    static bool IsInUnlockedOrActivatableSubtree(const Node& node, DisplayLockActivationReason activation_reason = DisplayLockActivationReason::kAny);
    ...
    // |activation_reason|.
    static bool ShouldIgnoreNodeDueToDisplayLock(const Node& node, DisplayLockActivationReason activation_reason)
    {
    ...
    static bool IsInLockedSubtreeCrossingFrames(const Node& node, IncludeSelfOrNot self = kExcludeSelf);
    ...
    static void SelectionChanged(const EphemeralRangeInFlatTree& old_selection, const EphemeralRangeInFlatTree& new_selection);
...
private:
    static bool IsDisplayLockedPreventingPaintUnmemoized(const Node& node, bool inclusive_check);
