...
enum class DisplayLockActivationReason {
    ...
    // Shorthands
    kViewport = static_cast<uint16_t>(kSelection) | static_cast<uint16_t>(kUserFocus) | static_cast<uint16_t>(kViewportIntersection)
        | static_cast<uint16_t>(kAccessibility),
    kAny = static_cast<uint16_t>(kAccessibility) | static_cast<uint16_t>(kFindInPage) | static_cast<uint16_t>(kFragmentNavigation)
        | static_cast<uint16_t>(kScriptFocus) | static_cast<uint16_t>(kScrollIntoView) | static_cast<uint16_t>(kSelection)
        | static_cast<uint16_t>(kSimulatedClick) | static_cast<uint16_t>(kUserFocus) | static_cast<uint16_t>(kViewportIntersection),
};
...
// small-ish.
static_assert(static_cast<uint32_t>(DisplayLockActivationReason::kAny) < std::numeric_limits<uint16_t>::max(), "DisplayLockActivationReason is too large");

class CORE_EXPORT DisplayLockContext final : public GarbageCollected<DisplayLockContext>,
...
public:
    ...
    {
        return !is_locked_ || forced_info_.is_forced(ForcedPhase::kStyleAndLayoutTree)
            || (IsActivatable(DisplayLockActivationReason::kAny) && ActivatableDisplayLocksForced())
            || (IsActivatable(DisplayLockActivationReason::kAccessibility) && document_->ExistingAXObjectCache());
    }
    ...
    {
        blocked_child_recalc_change_ = blocked_child_recalc_change_.ForceReattachLayoutTree();
    }

    void NotifyChildLayoutWasBlocked()
    {
        child_layout_was_blocked_ = true;
    }
    ...
    void SetNeedsPrePaintSubtreeWalk(bool needs_effective_allowed_touch_action_update, bool needs_blocking_wheel_event_handler_update)
    {
        needs_effective_allowed_touch_action_update_ = needs_effective_allowed_touch_action_update;
        needs_blocking_wheel_event_handler_update_ = needs_blocking_wheel_event_handler_update;
    ...
    bool IsAuto() const
    {
        return state_ == EContentVisibility::kAuto;
    }
...
private:
    ...
    void UpgradeForcedScope(ForcedPhase old_phase, ForcedPhase new_phase, bool emit_warnings);

    // Records the locked context counts on the document as well as context that
    // block all activation.
    void UpdateDocumentBookkeeping(bool was_locked, bool all_activation_was_blocked, bool is_locked, bool all_activation_is_blocked);
    ...
    struct UpdateForcedInfo {
        bool is_forced(ForcedPhase phase) const
        {
            ...
            case ForcedPhase::kStyleAndLayoutTree:
                return style_update_forced_ || layout_update_forced_ || prepaint_update_forced_;
    ...
    uint16_t activatable_mask_ = static_cast<uint16_t>(DisplayLockActivationReason::kAny);
    ...
    std::array<bool, static_cast<int>(RenderAffectingState::kNumRenderAffectingStates)> render_affecting_state_ = { false };
