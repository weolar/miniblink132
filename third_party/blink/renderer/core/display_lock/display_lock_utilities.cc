...
DisplayLockUtilities::LockCheckMemoizationScope* DisplayLockUtilities::memoizer_ = nullptr;
...
{
    if (!node.isConnected() || node.GetDocument().GetDisplayLockDocumentState().LockedDisplayLockCount() == 0 || node.IsShadowRoot()) {
        return nullptr;
    }
...
{
    return const_cast<Element*>(NearestLockedInclusiveAncestor(static_cast<const Node&>(node)));
}
...
{
    if (!child || !child->GetDocument().GetFrame() || !child->GetDocument().GetFrame()->OwnerLayoutObject()) {
        return nullptr;
    }
    return child->GetDocument().GetFrame()->OwnerLayoutObject()->GetNode();
}

void PopulateAncestorContexts(Node& node, HeapHashSet<Member<DisplayLockContext>>& contexts)
{
...
template <typename Lambda> Element* LockedAncestorPreventingUpdate(const Node& node, Lambda update_is_prevented)
{
    for (auto* ancestor = NearestLockedExclusiveAncestor(node); ancestor; ancestor = NearestLockedExclusiveAncestor(*ancestor)) {
...
template <typename Lambda> const Element* LockedInclusiveAncestorPreventingUpdate(const Node& node, Lambda update_is_prevented)
{
...
template <typename Lambda> Element* LockedAncestorPreventingUpdate(const LayoutObject& object, Lambda update_is_prevented)
{
...
template <typename Lambda> Element* LockedInclusiveAncestorPreventingUpdate(const LayoutObject& object, Lambda update_is_prevented)
{
...
bool DisplayLockUtilities::ActivateFindInPageMatchRangeIfNeeded(const EphemeralRangeInFlatTree& range)
{
    DCHECK(!range.IsNull());
    DCHECK(!range.IsCollapsed());
    if (!range.GetDocument().GetDisplayLockDocumentState().HasActivatableLocks()) {
        return false;
    }
    ...
    Element* enclosing_block = EnclosingBlock(range.StartPosition(), kCanCrossEditingBoundary);
    ...
    return enclosing_block->ActivateDisplayLockIfNeeded(DisplayLockActivationReason::kFindInPage);
}

bool DisplayLockUtilities::NeedsActivationForFindInPage(const EphemeralRangeInFlatTree& range)
{
    DisplayLockDocumentState& state = range.GetDocument().GetDisplayLockDocumentState();
    ...
    for (Node& ancestor : FlatTreeTraversal::InclusiveAncestorsOf(*enclosing_block)) {
        auto* ancestor_element = DynamicTo<Element>(ancestor);
        if (!ancestor_element) {
            continue;
        }
        if (auto* context = ancestor_element->GetDisplayLockContext()) {
            if (context->ShouldCommitForActivation(DisplayLockActivationReason::kFindInPage)) {
                return true;
            }
        }
    }
...
const HeapVector<Member<Element>> DisplayLockUtilities::ActivatableLockedInclusiveAncestors(const Node& node, DisplayLockActivationReason reason)
{
    HeapVector<Member<Element>> elements_to_activate;
    if (node.GetDocument().GetDisplayLockDocumentState().LockedDisplayLockCount()
        == node.GetDocument().GetDisplayLockDocumentState().DisplayLockBlockingAllActivationCount())
...
DisplayLockUtilities::ScopedForcedUpdate::Impl::Impl(const Range* range, DisplayLockContext::ForcedPhase phase, bool only_cv_auto, bool emit_warnings)
    ...
    , emit_warnings_(emit_warnings)
{
    ...
    range->OwnerDocument().GetDisplayLockDocumentState().BeginRangeForcedScope(range, this);

    if (node_->GetDocument().GetDisplayLockDocumentState().LockedDisplayLockCount() == 0)
        return;
    ...
    // tree ancestors.
    for (Node* node = range->FirstNode(); node != range->PastLastNode(); node = NodeTraversal::Next(*node)) {
        if (node->IsChildOfShadowHost()) {
            ...
            for (Node* ancestor = node; ancestor; ancestor = FlatTreeTraversal::Parent(*ancestor)) {
    ...
    for (Node* node = range->FirstNode(); node; node = FlatTreeTraversal::Parent(*node)) {
        if (Element* element = DynamicTo<Element>(node)) {
            if (DisplayLockContext* context = element->GetDisplayLockContext()) {
                forced_context_set_.insert(context);
            }
...
DisplayLockUtilities::ScopedForcedUpdate::Impl::Impl(
    const Node* node, DisplayLockContext::ForcedPhase phase, bool include_self, bool only_cv_auto, bool emit_warnings)
    : node_(node)
    , phase_(phase)
    , only_cv_auto_(only_cv_auto)
    , emit_warnings_(emit_warnings)
{
    ...
    node->GetDocument().GetDisplayLockDocumentState().BeginNodeForcedScope(node, include_self, this);

    if (node->GetDocument().GetDisplayLockDocumentState().LockedDisplayLockCount() == 0)
        return;
...
void DisplayLockUtilities::ScopedForcedUpdate::Impl::EnsureMinimumForcedPhase(DisplayLockContext::ForcedPhase phase)
{
...
void DisplayLockUtilities::ScopedForcedUpdate::Impl::AddForcedUpdateScopeForContext(DisplayLockContext* context)
{
...
void DisplayLockUtilities::ScopedForcedUpdate::Impl::ForceDisplayLockIfNeeded(DisplayLockContext* context)
{
    if (!only_cv_auto_ || context->IsActivatable(DisplayLockActivationReason::kViewport)) {
        forced_context_set_.insert(context);
        context->NotifyForcedUpdateScopeStarted(phase_, emit_warnings_);
    }
}

Element* DisplayLockUtilities::LockedInclusiveAncestorPreventingStyleWithinTreeScope(const Node& node)
{
    if (!node.isConnected() || node.GetDocument().GetDisplayLockDocumentState().LockedDisplayLockCount() == 0) {
        return nullptr;
    }
    ...
    for (Node& ancestor : NodeTraversal::InclusiveAncestorsOf(node)) {
        ...
        if (!ancestor_element)
            continue;
        if (DisplayLockContext* context = ancestor_element->GetDisplayLockContext()) {
...
const Element* DisplayLockUtilities::LockedInclusiveAncestorPreventingLayout(const Node& node)
{
    return LockedInclusiveAncestorPreventingUpdate(node, [](DisplayLockContext* context) { return !context->ShouldLayoutChildren(); });
}

const Element* DisplayLockUtilities::LockedInclusiveAncestorPreventingPaint(const Node& node)
{
    return LockedInclusiveAncestorPreventingUpdate(node, [](DisplayLockContext* context) { return !context->ShouldPaintChildren(); });
}

const Element* DisplayLockUtilities::LockedInclusiveAncestorPreventingPaint(const LayoutObject& object)
{
    return LockedInclusiveAncestorPreventingUpdate(object, [](DisplayLockContext* context) { return !context->ShouldPaintChildren(); });
}

Element* DisplayLockUtilities::HighestLockedInclusiveAncestor(const Node& node)
{
    if (node.IsShadowRoot())
        return nullptr;
    ...
    if (element && element->GetDisplayLockContext() && element->GetDisplayLockContext()->IsLocked()) {
        return element;
    }
    return nullptr;
}

Element* DisplayLockUtilities::HighestLockedExclusiveAncestor(const Node& node)
{
    if (node.IsShadowRoot())
        return nullptr;
...
bool DisplayLockUtilities::IsInUnlockedOrActivatableSubtree(const Node& node, DisplayLockActivationReason activation_reason)
{
    if (node.GetDocument().GetDisplayLockDocumentState().LockedDisplayLockCount() == 0 || node.IsShadowRoot()) {
        return true;
    }

    if (activation_reason == DisplayLockActivationReason::kAccessibility && memoizer_) {
        return !IsLockedForAccessibility(node);
    }

    for (auto* element = NearestLockedExclusiveAncestor(node); element; element = NearestLockedExclusiveAncestor(*element)) {
    ...
    for (Node& ancestor : FlatTreeTraversal::AncestorsOf(node)) {
        ...
        if (auto* ancestor_element = DynamicTo<Element>(ancestor)) {
            if (auto* context = ancestor_element->GetDisplayLockContext()) {
                ...
                if (ancestor_is_locked && !context->IsActivatable(DisplayLockActivationReason::kAccessibility)) {
...
bool DisplayLockUtilities::IsInLockedSubtreeCrossingFrames(const Node& source_node, IncludeSelfOrNot self)
{
...
{
    if (element && element->GetDocument().GetDisplayLockDocumentState().DisplayLockCount() == 0) {
        return;
    }
...
// static
bool DisplayLockUtilities::NeedsSelectionChangedUpdate(const Document& document)
{
    return document.GetDisplayLockDocumentState().DisplayLockCount() > 0;
}

void DisplayLockUtilities::SelectionChanged(const EphemeralRangeInFlatTree& old_selection, const EphemeralRangeInFlatTree& new_selection)
{
    if ((!old_selection.IsNull() && old_selection.GetDocument().GetDisplayLockDocumentState().DisplayLockCount() == 0)
        || (!new_selection.IsNull() && new_selection.GetDocument().GetDisplayLockDocumentState().DisplayLockCount() == 0)) {
        return;
    }
    ...
    for (DisplayLockContext* context : lost_selection_contexts) {
        if (auto it = gained_selection_contexts.find(context); it != gained_selection_contexts.end()) {
...
Element* DisplayLockUtilities::LockedAncestorPreventingPaint(const LayoutObject& object)
{
    return LockedAncestorPreventingUpdate(object, [](DisplayLockContext* context) { return !context->ShouldPaintChildren(); });
}

Element* DisplayLockUtilities::LockedAncestorPreventingPaint(const Node& node)
{
    return LockedAncestorPreventingUpdate(node, [](DisplayLockContext* context) { return !context->ShouldPaintChildren(); });
}

Element* DisplayLockUtilities::LockedAncestorPreventingPrePaint(const LayoutObject& object)
{
    return LockedAncestorPreventingUpdate(object, [](DisplayLockContext* context) { return !context->ShouldPrePaintChildren(); });
}

Element* DisplayLockUtilities::LockedAncestorPreventingLayout(const LayoutObject& object)
{
    return LockedAncestorPreventingUpdate(object, [](DisplayLockContext* context) { return !context->ShouldLayoutChildren(); });
}

Element* DisplayLockUtilities::LockedAncestorPreventingLayout(const Node& node)
{
    return LockedAncestorPreventingUpdate(node, [](DisplayLockContext* context) { return !context->ShouldLayoutChildren(); });
}

Element* DisplayLockUtilities::LockedAncestorPreventingStyle(const Node& node)
{
    return LockedAncestorPreventingUpdate(node, [](DisplayLockContext* context) { return !context->ShouldStyleChildren(); });
}
...
bool DisplayLockUtilities::IsAutoWithoutLayout(const LayoutObject& object)
{
    ...
    if (!context)
        return false;
    return !context->IsLocked() && context->IsAuto() && !context->HadLifecycleUpdateSinceLastUnlock();
}
...
bool DisplayLockUtilities::RevealHiddenUntilFoundAncestors(const Node& node)
{
    ...
    for (Node& parent : FlatTreeTraversal::InclusiveAncestorsOf(node)) {
        if (HTMLElement* element = DynamicTo<HTMLElement>(parent)) {
            if (EqualIgnoringASCIICase(element->FastGetAttribute(html_names::kHiddenAttr), "until-found")) {
    ...
    for (HTMLElement* element : elements_to_reveal) {
        element->DispatchEvent(*Event::CreateBubble(event_type_names::kBeforematch));
    }
...
bool DisplayLockUtilities::IsDisplayLockedPreventingPaintUnmemoized(const Node& node, bool inclusive_check)
{
    return inclusive_check ? DisplayLockUtilities::LockedInclusiveAncestorPreventingPaint(node) : DisplayLockUtilities::LockedAncestorPreventingPaint(node);
}

bool DisplayLockUtilities::IsDisplayLockedPreventingPaint(const Node* node, bool inclusive_check)
{
    ...
    if (memoizer_) {
        ...
        if (memoized_result) {
            bool final_result = *memoized_result || (inclusive_check && CheckSelf(node));
#if DCHECK_IS_ON()
            bool nonmemoized_result = IsDisplayLockedPreventingPaintUnmemoized(*node, inclusive_check);
            DCHECK_EQ(final_result, nonmemoized_result)
                << "\nMemoized result did not match non-memoized result for " << (inclusive_check ? "inclusive" : "non-inclusive") << " check."
                << "\n* node = " << node << "\n* Inclusive ancestor preventing paint: " << DisplayLockUtilities::LockedInclusiveAncestorPreventingPaint(*node)
                << "\n* Non-inclusive ancestor preventing paint: " << DisplayLockUtilities::LockedAncestorPreventingPaint(*node);
#endif
        ...
        return inclusive_check ? DisplayLockUtilities::LockedInclusiveAncestorPreventingPaint(*node)
...
bool DisplayLockUtilities::IsDisplayLockedPreventingPaint(const LayoutObject* object)
{
