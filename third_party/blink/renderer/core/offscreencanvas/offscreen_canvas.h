...
class OffscreenCanvasRenderingContext2DOrWebGLRenderingContextOrWebGL2RenderingContextOrImageBitmapRenderingContextOrGPUCanvasContext;
...
class CORE_EXPORT OffscreenCanvas final : public EventTarget,
...
public:
    ...
    void SetFilterQualityInResource(cc::PaintFlags::FilterQuality filter_quality) override;
    ...
    ScriptPromise<Blob> convertToBlob(ScriptState* script_state, const ImageEncodeOptions* options, ExceptionState& exception_state);
    ...
    DOMNodeId PlaceholderCanvasId() const
    {
        return placeholder_canvas_id_;
    }
    ...
    CanvasRenderingContext* GetCanvasRenderingContext(
        ExecutionContext*, CanvasRenderingContext::CanvasRenderingAPI, const CanvasContextCreationAttributesCore&);
    ...
    void Commit(scoped_refptr<CanvasResource>&& bitmap_image, const SkIRect& damage_rect) override;
    ...
    ScriptPromise<ImageBitmap> CreateImageBitmap(ScriptState*, std::optional<gfx::Rect>, const ImageBitmapOptions*, ExceptionState&) final;

    // CanvasImageSource implementation
    scoped_refptr<Image> GetSourceImageForCanvas(
        FlushReason, SourceImageStatus*, const gfx::SizeF&, const AlphaDisposition alpha_disposition = kPremultiplyAlpha) final;
    bool WouldTaintOrigin() const final
    {
        return !origin_clean_;
    }
    gfx::SizeF ElementSize(const gfx::SizeF& default_object_size, const RespectImageOrientationEnum) const final
    {
    ...
    public:
        ...
        bool AddOffscreenCanvas(OffscreenCanvas* canvas)
        {
            ...
            if (canvas->GetOrCreateResourceDispatcher()) {
                ...
                if (canvas->GetOrCreateResourceDispatcher()->HasTooManyPendingFrames()) {
                    abort_raf_ = true;
                    return false;
                }
            }
        ...
        ~ScopedInsideWorkerRAF()
        {
            for (auto canvas : canvases_) {
                ...
                if (abort_raf_)
                    continue;
                if (canvas->GetOrCreateResourceDispatcher()) {
                    canvas->GetOrCreateResourceDispatcher()->ReplaceBeginFrameAck(begin_frame_args_);
                }
