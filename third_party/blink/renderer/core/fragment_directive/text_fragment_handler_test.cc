...
public:
    ...
    {
        GetDocument().GetFrame()->Selection().SetSelection(SelectionInDOMTree::Builder().SetBaseAndExtent(start, end).Build(), SetSelectionOptions());
    }
    ...
    void SetLocationHash(Document& document, String hash)
    {
        ...
        document.GetFrame()->DomWindow()->location()->setHash(script_state->GetIsolate(), hash, ASSERT_NO_EXCEPTION);
    }
    ...
    String RequestSelector()
    {
        ...
        String selector;
        auto lambda = [](bool& callback_called, String& selector, const String& generated_selector, shared_highlighting::LinkGenerationError error,
    ...
    Vector<String> ExtractTextFragmentsMatches()
    {
        ...
        auto lambda = [](bool& callback_called, Vector<String>& target_texts, const Vector<String>& fetched_target_texts) {
            target_texts = fetched_target_texts;
            callback_called = true;
        };
        auto callback = WTF::BindOnce(lambda, std::ref(callback_called), std::ref(target_texts));
    ...
    gfx::Rect ExtractFirstTextFragmentsRect()
    {
        ...
        auto lambda = [](bool& callback_called, gfx::Rect& text_fragment_rect, const gfx::Rect& fetched_text_fragment_rect) {
            text_fragment_rect = fetched_text_fragment_rect;
            callback_called = true;
        };
        auto callback = WTF::BindOnce(lambda, std::ref(callback_called), std::ref(text_fragment_rect));
    ...
    void LoadAhem()
    {
        ...
        FontFace* ahem = FontFace::Create(GetDocument().GetExecutionContext(), AtomicString("Ahem"), buffer, FontFaceDescriptors::Create());
...
TEST_F(TextFragmentHandlerTest, ExtractTextFragmentWithWithMultipleTextFragments)
{
    SimRequest request("https://example.com/"
                       "test.html#:~:text=test%20page&text=more%20text",
        "text/html");
...
TEST_F(TextFragmentHandlerTest, ExtractFirstTextFragmentRect)
{
    ...
    gfx::Rect expected_rect = GetDocument().GetFrame()->View()->FrameToViewport(rect);
...
TEST_F(TextFragmentHandlerTest, ExtractFirstTextFragmentRectScroll)
{
    ...
    SimRequest request("https://example.com/test.html#:~:text=test,page", "text/html");
...
TEST_F(TextFragmentHandlerTest, ExtractFirstTextFragmentRectMultipleHighlightWithNoFoundText)
{
    SimRequest request("https://example.com/"
...
TEST_F(TextFragmentHandlerTest, CheckNoPreemptiveGenerationEditable)
{
    ...
    Node* input_text = FlatTreeTraversal::Next(*GetDocument().getElementById(AtomicString("input")))->firstChild();
    const auto& selected_start = Position(input_text, 0);
    const auto& selected_end = Position(input_text, 12);
    ASSERT_EQ("default text", PlainText(EphemeralRange(selected_start, selected_end)));
...
TEST_F(TextFragmentHandlerTest, SecondGenerationCrash)
{
  ...
  <p id='p'>First paragraph text</p>
    ...
    auto callback = WTF::BindOnce([](const TextFragmentSelector& selector, shared_highlighting::LinkGenerationError error) {});
    MakeGarbageCollected<TextFragmentSelectorGenerator>(GetDocument().GetFrame())->SetCallbackForTesting(std::move(callback));
...
TEST_F(TextFragmentHandlerTest, ShouldCreateTextFragmentHandlerAndRemoveHighlightForIframes)
{
...
TEST_F(TextFragmentHandlerTest, ShouldCreateTextFragmentHandlerAndRemoveHighlight)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
    ...
    mojo::Remote<mojom::blink::TextFragmentReceiver> remote;
    GetDocument().GetFrame()->BindTextFragmentReceiver(remote.BindNewPipeAndPassReceiver());
...
TEST_F(TextFragmentHandlerTest, ShouldCreateTextFragmentHandlerAndRequestSelector)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
...
TEST_F(TextFragmentHandlerTest, ShouldRemoveFromMainFrameAndIframeWhenBothHaveHighlights)
{
    ...
    ASSERT_EQ("https://example.com/child.html#:~:text=iframe", child_frame->Loader().GetDocumentLoader()->GetHistoryItem()->Url());

    ASSERT_EQ(1u, GetDocument().Markers().Markers().size());
    ASSERT_EQ("https://example.com/test.html#:~:text=test", main_frame->Loader().GetDocumentLoader()->GetHistoryItem()->Url());
    ...
    // Remove shared highlights from the iframe.
    {
        ...
        EXPECT_EQ(0u, child_frame->GetDocument()->Markers().Markers().size());
        EXPECT_FALSE(child_frame->GetDocument()->View()->GetFragmentAnchor());
        EXPECT_EQ("https://example.com/child.html", child_frame->Loader().GetDocumentLoader()->GetHistoryItem()->Url());
    }
    ...
    // Remove shared highlights from the main frame.
    {
        ...
        EXPECT_EQ(0u, GetDocument().Markers().Markers().size());
        EXPECT_FALSE(GetDocument().View()->GetFragmentAnchor());
        EXPECT_EQ("https://example.com/test.html", main_frame->Loader().GetDocumentLoader()->GetHistoryItem()->Url());
    }
}
...
TEST_F(TextFragmentHandlerTest, IfGeneratorResetShouldRecordCorrectError)
{
    ...
    shared_highlighting::LinkGenerationError expected_error = shared_highlighting::LinkGenerationError::kEmptySelection;
...
TEST_F(TextFragmentHandlerTest, NotGenerated)
{
    ...
    shared_highlighting::LinkGenerationError expected_error = shared_highlighting::LinkGenerationError::kNotGenerated;
    EXPECT_EQ(expected_error, GetTextFragmentHandler().error_);
}
...
TEST_F(TextFragmentHandlerTest, InvalidateOverflowOnRemoval)
{
    ...
    EXPECT_EQ(1u, GetDocument().Markers().Markers().size());
    Text* first_paragraph = To<Text>(GetDocument().getElementById(AtomicString("first"))->firstChild());
