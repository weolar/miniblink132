...
SameBlockWordIterator<Direction>::SameBlockWordIterator(const PositionInFlatTree& position)
...
template <typename Direction> String SameBlockWordIterator<Direction>::TextFromStart() const
{
    String range_text;
    if (start_position_.ComputeContainerNode() != current_node_) {
        ...
        range_text = Direction::RangeText(start_position_, PositionInFlatTree(current_node_, Direction::FirstPosition(current_node_text_)));
    }
    ...
    String current_node_words = Direction::Substring(current_node_text_, Direction::FirstPosition(current_node_text_), current_text_offset_);

    return Direction::Concat(range_text, current_node_words).StripWhiteSpace();
}

template <typename Direction> bool SameBlockWordIterator<Direction>::AdvanceNextWord()
{
    do {
        int pos = Direction::FindNextWordPos(current_node_text_, current_text_offset_);
        unsigned next_word_stripped_length = Direction::Substring(current_node_text_, current_text_offset_, pos).LengthWithStrippedWhiteSpace();
...
template <typename Direction> bool SameBlockWordIterator<Direction>::NextNode()
{
...
template <typename Direction> Node* SameBlockWordIterator<Direction>::NextVisibleTextNodeWithinBlock(Node& start_node)
{
...
template <typename Direction> void SameBlockWordIterator<Direction>::Trace(Visitor* visitor) const
{
