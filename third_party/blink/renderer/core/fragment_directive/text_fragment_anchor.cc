...
bool CheckSecurityRestrictions(LocalFrame& frame)
{
    ...
    if (!frame.Loader().GetDocumentLoader()->ConsumeTextFragmentToken()) {
        TRACE_EVENT_INSTANT("blink", "CheckSecurityRestrictions", "Result", "No Token");
        return false;
    }

    AtomicString content_type = frame.GetDocument()->contentType();
    if (content_type != "text/html" && content_type != "text/plain") {
        TRACE_EVENT_INSTANT("blink", "CheckSecurityRestrictions", "Result", "Invalid ContentType");
        return false;
    }
    ...
    if (frame.IsFencedFrameRoot()) {
        TRACE_EVENT_INSTANT("blink", "CheckSecurityRestrictions", "Result", "Fenced Frame");
        return false;
    }
    ...
    if (!frame.Loader().GetDocumentLoader()->LastNavigationHadTrustedInitiator()) {
        if (frame.Tree().Parent()) {
            TRACE_EVENT_INSTANT("blink", "CheckSecurityRestrictions", "Result", "Cross-Origin Subframe");
            return false;
        }

        if (frame.GetPage()->RelatedPages().size()) {
            TRACE_EVENT_INSTANT("blink", "CheckSecurityRestrictions", "Result", "Non-Empty Browsing Context Group");
            return false;
        }
    }
...
bool TextFragmentAnchor::GenerateNewToken(const DocumentLoader& loader)
{
    ...
    // navigations.
    if (loader.GetNavigationType() != kWebNavigationTypeLinkClicked && loader.GetNavigationType() != kWebNavigationTypeOther) {
        return false;
    }
    ...
    return loader.LastNavigationHadTransientUserActivation() || loader.IsBrowserInitiated();
}

// static
bool TextFragmentAnchor::GenerateNewTokenForSameDocument(
    const DocumentLoader& loader, WebFrameLoadType load_type, mojom::blink::SameDocumentNavigationType same_document_navigation_type)
{
    if ((load_type != WebFrameLoadType::kStandard && load_type != WebFrameLoadType::kReplaceCurrentItem)
        || same_document_navigation_type != mojom::blink::SameDocumentNavigationType::kFragment)
        return false;
    ...
    FragmentDirective& fragment_directive = loader.GetFrame()->GetDocument()->fragmentDirective();
    if (!fragment_directive.LastNavigationHadFragmentDirective() || fragment_directive.GetDirectives<TextDirective>().empty()) {
        return false;
    }
...
// static
TextFragmentAnchor* TextFragmentAnchor::TryCreate(const KURL& url, LocalFrame& frame, bool should_scroll)
{
    DCHECK(RuntimeEnabledFeatures::TextFragmentIdentifiersEnabled(frame.DomWindow()));

    HeapVector<Member<TextDirective>> text_directives = frame.GetDocument()->fragmentDirective().GetDirectives<TextDirective>();
    if (text_directives.empty()) {
        if (frame.GetDocument()->fragmentDirective().LastNavigationHadFragmentDirective()) {
            UseCounter::Count(frame.GetDocument(), WebFeature::kInvalidFragmentDirective);
        }
        return nullptr;
    }

    TRACE_EVENT("blink", "TextFragmentAnchor::TryCreate", "url", url, "should_scroll", should_scroll);

    if (!CheckSecurityRestrictions(frame)) {
        return nullptr;
    } else if (!should_scroll) {
        if (frame.Loader().GetDocumentLoader() && !frame.Loader().GetDocumentLoader()->NavigationScrollAllowed()) {
            ...
            //      history navigations.
            UseCounter::Count(frame.GetDocument(), WebFeature::kTextFragmentBlockedByForceLoadAtTop);
        }
    }

    return MakeGarbageCollected<TextFragmentAnchor>(text_directives, frame, should_scroll);
}

TextFragmentAnchor::TextFragmentAnchor(HeapVector<Member<TextDirective>>& text_directives, LocalFrame& frame, bool should_scroll)
    : SelectorFragmentAnchor(frame, should_scroll)
    , post_load_timer_(frame.GetTaskRunner(TaskType::kInternalFindInPage), this, &TextFragmentAnchor::PostLoadTask)
    , post_load_timeout_timer_(frame.GetTaskRunner(TaskType::kInternalFindInPage), this, &TextFragmentAnchor::PostLoadTask)
    , metrics_(MakeGarbageCollected<TextFragmentAnchorMetrics>(frame_->GetDocument()))
{
    ...
    AnnotationAgentContainerImpl* annotation_container = AnnotationAgentContainerImpl::CreateIfNeeded(*frame_->GetDocument());
    ...
    for (Member<TextDirective>& directive : text_directives) {
        auto* selector = MakeGarbageCollected<TextAnnotationSelector>(directive->GetSelector());
        AnnotationAgentImpl* agent = annotation_container->CreateUnboundAgent(mojom::blink::AnnotationType::kSharedHighlight, *selector);
...
{
    AnnotationAgentContainerImpl* container = Supplement<Document>::From<AnnotationAgentContainerImpl>(frame_->GetDocument());
    CHECK(container);
...
void TextFragmentAnchor::FinalizeAnchor()
{
    ...
    for (DirectiveAnnotationPair& directive_annotation_pair : directive_annotation_pairs_) {
        ...
        const RangeInFlatTree* attached_range = annotation->IsAttached() ? &annotation->GetAttachedRange() : nullptr;
...
void TextFragmentAnchor::UpdateCurrentState()
{
    ...
    for (auto& directive_annotation_pair : directive_annotation_pairs_) {
        ...
        bool found_match = annotation->IsAttachmentPending() || annotation->IsAttached();
        if (!found_match) {
            all_found = false;
            continue;
        }
        ...
        if (!first_match_) {
            ...
            state_ = annotation->IsAttachmentPending() ? kWaitingForDOMMutations : kApplyEffects;
...
void TextFragmentAnchor::ApplyEffectsToFirstMatch()
{
    ...
    frame_->GetDocument()->UpdateStyleAndLayout(DocumentUpdateReason::kFindInPage);
    ...
    frame_->GetDocument()->SetSequentialFocusNavigationStartingPoint(range.StartPosition().NodeAsRangeFirstNode());
}
...
void TextFragmentAnchor::DidFinishSearch()
{
    ...
    if (!did_find_any_matches) {
        ...
        // here.
        element_fragment_anchor_ = ElementFragmentAnchor::TryCreate(frame_->GetDocument()->Url(), *frame_, should_scroll_);
    }
    ...
    frame_->GetDocument()->EnqueueAnimationFrameTask(WTF::BindOnce(&TextFragmentAnchor::FinalizeAnchor, WrapWeakPersistent(this)));
    finalize_pending_ = true;
}

void TextFragmentAnchor::ApplyTargetToCommonAncestor(const EphemeralRangeInFlatTree& range)
{
...
void TextFragmentAnchor::SetTickClockForTesting(const base::TickClock* tick_clock)
{
...
bool TextFragmentAnchor::HasSearchEngineSource()
{
    ...
    return IsKnownSearchEngine(frame_->GetDocument()->Loader()->GetRequestorOrigin()->ToString());
}
