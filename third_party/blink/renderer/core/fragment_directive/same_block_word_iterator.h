...
public:
    ...
    static int FirstPosition(const String& text)
    {
        return 0;
    }
    ...
    {
        return RangeText(position, PositionInFlatTree::LastPositionInNode(*position.ComputeContainerNode()));
    }
    ...
    static String Substring(const String& text, int start_position, int end_position)
    {
        return text.Substring(start_position, end_position - start_position);
    }

    // Returns the text within given positions.
    static String RangeText(const PositionInFlatTree& start, const PositionInFlatTree& end)
    {
...
public:
    ...
    static String NodeTextFromOffset(const PositionInFlatTree position)
    {
        return RangeText(position, PositionInFlatTree::FirstPositionInNode(*position.ComputeContainerNode()));
    }
...
template <typename Direction> class SameBlockWordIterator : public GarbageCollected<SameBlockWordIterator<Direction>> {
public:
...
extern template class CORE_EXTERN_TEMPLATE_EXPORT SameBlockWordIterator<ForwardDirection>;
extern template class CORE_EXTERN_TEMPLATE_EXPORT SameBlockWordIterator<BackwardDirection>;
