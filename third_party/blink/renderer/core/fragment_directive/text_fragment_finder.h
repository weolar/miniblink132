...
class CORE_EXPORT TextFragmentFinder : public GarbageCollected<TextFragmentFinder> {
public:
    class Client {
    public:
    ...
    static PositionInFlatTree NextTextPosition(PositionInFlatTree position, PositionInFlatTree end_position);

    // Returns previous position that is not space or equivalent.
    static PositionInFlatTree PreviousTextPosition(PositionInFlatTree position, PositionInFlatTree max_position);
    ...
    static bool IsInSameUninterruptedBlock(const PositionInFlatTree& start, const PositionInFlatTree& end);

    // Client must outlive the finder.
    TextFragmentFinder(Client& client, const TextFragmentSelector& selector, Document* document, FindBufferRunnerType runner_type);
    ...
    const RangeInFlatTree* FirstMatch() const
    {
        return first_match_.Get();
    }

    const TextFragmentSelector& GetSelector() const
    {
        return selector_;
    }
...
protected:
    ...
    // Used for tracking what should be the next match stage.
    enum SelectorMatchStep { kMatchPrefix, kMatchTextStart, kMatchTextEnd, kMatchSuffix };
...
private:
    ...
    void OnFindMatchInRangeComplete(
        String search_text, RangeInFlatTree* range, bool word_start_bounded, bool word_end_bounded, const EphemeralRangeInFlatTree& match);

    void FindMatchInRange(String search_text, RangeInFlatTree* range, bool word_start_bounded, bool word_end_bounded);
