...
//               │        kFinalized         │◄──────┘
//               └───────────────────────────┘
class CORE_EXPORT TextFragmentAnchor final : public SelectorFragmentAnchor, public AnnotationAgentContainerImpl::Observer {
public:
    ...
    static bool GenerateNewTokenForSameDocument(
        const DocumentLoader&, WebFrameLoadType load_type, mojom::blink::SameDocumentNavigationType same_document_navigation_type);

    static TextFragmentAnchor* TryCreate(const KURL& url, LocalFrame& frame, bool should_scroll);

    TextFragmentAnchor(HeapVector<Member<TextDirective>>& text_directives, LocalFrame& frame, bool should_scroll);
    ...
    bool IsTextFragmentAnchor() override
    {
        return true;
    }
...
private:
    ...
    using DirectiveAnnotationPair = std::pair<Member<TextDirective>, Member<AnnotationAgentImpl>>;
