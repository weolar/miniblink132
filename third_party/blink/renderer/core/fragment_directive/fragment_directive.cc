...
KURL FragmentDirective::ConsumeFragmentDirective(const KURL& url)
{
    ...
    wtf_size_t start_pos = fragment.Find(shared_highlighting::kFragmentsUrlDelimiter);
    ...
    KURL new_url = url;
    String fragment_directive = fragment.Substring(start_pos + shared_highlighting::kFragmentsUrlDelimiterLength);
...
ScriptPromise<SelectorDirective> FragmentDirective::createSelectorDirective(ScriptState* state, const V8UnionRangeOrSelection* arg)
{
    if (ExecutionContext::From(state)->IsContextDestroyed())
        return EmptyPromise();

    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<SelectorDirective>>(state);
    ...
    bool is_content_type_selection = arg->GetContentType() == V8UnionRangeOrSelection::ContentType::kSelection;
    if (is_content_type_selection) {
        DOMSelection* selection = arg->GetAsSelection();
        if (selection->rangeCount() == 0) {
            resolver->RejectWithDOMException(DOMExceptionCode::kNotSupportedError, "Selection must contain a range");
            return promise;
        }

        range = selection->getRangeAt(0, ASSERT_NO_EXCEPTION);
    } else {
        DCHECK_EQ(arg->GetContentType(), V8UnionRangeOrSelection::ContentType::kRange);
        range = arg->GetAsRange();
    }

    if (!range || range->collapsed()) {
        resolver->RejectWithDOMException(DOMExceptionCode::kNotSupportedError, "RangeOrSelector must be non-null and non-collapsed");
        if (is_content_type_selection) {
    ...
    if (range->OwnerDocument() != owner_document_) {
        resolver->RejectWithDOMException(DOMExceptionCode::kWrongDocumentError, "RangeOrSelector must be from this document");
        if (is_content_type_selection) {
            DisposeTemporaryRange(range);
        }
        return promise;
    ...
    if (!frame) {
        resolver->RejectWithDOMException(DOMExceptionCode::kInvalidStateError, "Document must be attached to frame");
        if (is_content_type_selection) {
            DisposeTemporaryRange(range);
        }
        return promise;
    ...
    RangeInFlatTree* range_in_flat_tree = MakeGarbageCollected<RangeInFlatTree>(ephemeral_range.StartPosition(), ephemeral_range.EndPosition());

    auto* generator = MakeGarbageCollected<TextFragmentSelectorGenerator>(frame);
    generator->Generate(*range_in_flat_tree,
        WTF::BindOnce(
            [](ScriptPromiseResolver<SelectorDirective>* resolver, TextFragmentSelectorGenerator* generator, const RangeInFlatTree* range,
                const TextFragmentSelector& selector, shared_highlighting::LinkGenerationError error) {
                if (selector.Type() == TextFragmentSelector::SelectorType::kInvalid) {
                    resolver->RejectWithDOMException(DOMExceptionCode::kOperationError, "Failed to generate selector for the given range");
                    return;
                }
                TextDirective* dom_text_directive = MakeGarbageCollected<TextDirective>(selector);
                dom_text_directive->DidFinishMatching(range);
                resolver->Resolve(dom_text_directive);
            },
            WrapPersistent(resolver), WrapPersistent(generator), WrapPersistent(range_in_flat_tree)));
...
void FragmentDirective::ParseDirectives(const String& fragment_directive)
{
    ...
    fragment_directive.Split("&", /*allow_empty_entries=*/true, directive_strings);
    ...
    for (String& directive_string : directive_strings) {
        ...
        } else if (auto* selector_directive = CssSelectorDirective::TryParse(directive_string)) {
