...
public:
    ...
    void VerifySelector(Position selected_start, Position selected_end, String expected_selector)
    {
        ...
        histogram_tester_.ExpectTotalCount("SharedHighlights.LinkGenerated.Error.Requested", 0);

        histogram_tester_.ExpectTotalCount("SharedHighlights.LinkGenerated", generate_call_count_);
        auto entries = ukm_recorder()->GetEntriesByName(ukm::builders::SharedHighlights_LinkGenerated::kEntryName);
    ...
    void VerifySelectorFails(Position selected_start, Position selected_end, LinkGenerationError error)
    {
        String generated_selector = GenerateSelector(selected_start, selected_end);
        EXPECT_EQ("", generated_selector);

        histogram_tester_.ExpectTotalCount("SharedHighlights.LinkGenerated", generate_call_count_);
        histogram_tester_.ExpectBucketCount("SharedHighlights.LinkGenerated.Error", error, 1);
        auto entries = ukm_recorder()->GetEntriesByName(ukm::builders::SharedHighlights_LinkGenerated::kEntryName);
        ASSERT_EQ(1u, entries.size());
        const ukm::mojom::UkmEntry* entry = entries[0];
        EXPECT_EQ(GetDocument().UkmSourceID(), entry->source_id);
        ukm_recorder()->ExpectEntryMetric(entry, kSuccessUkmMetric, false);
        ukm_recorder()->ExpectEntryMetric(entry, kErrorUkmMetric, static_cast<int64_t>(error));
    }
    ...
    String GenerateSelector(Position selected_start, Position selected_end)
    {
        ...
        String selector;
        auto lambda
            = [](bool& callback_called, String& selector, const TextFragmentSelector& generated_selector, shared_highlighting::LinkGenerationError error) {
                  selector = generated_selector.ToString();
                  callback_called = true;
              };
        auto callback = WTF::BindOnce(lambda, std::ref(callback_called), std::ref(selector));
        CreateGenerator()->Generate(
            *MakeGarbageCollected<RangeInFlatTree>(ToPositionInFlatTree(selected_start), ToPositionInFlatTree(selected_end)), std::move(callback));
    ...
    {
        return MakeGarbageCollected<TextFragmentSelectorGenerator>(GetDocument().GetFrame());
    }
...
TEST_F(TextFragmentSelectorGeneratorTest, EmptySelection)
{
    ...
    VerifySelectorFails(selected_start, selected_end, LinkGenerationError::kEmptySelection);
}
...
TEST_F(TextFragmentSelectorGeneratorTest, ExactTextSelector)
{
    ...
    ASSERT_EQ("First paragraph text that is", PlainText(EphemeralRange(selected_start, selected_end)));

    VerifySelector(selected_start, selected_end, "First%20paragraph%20text%20that%20is");
}
...
TEST_F(TextFragmentSelectorGeneratorTest, ExactTextSelector_Long)
{
    ...
    ASSERT_EQ("first_texts_and_last", PlainText(EphemeralRange(selected_start, selected_end)));
...
TEST_F(TextFragmentSelectorGeneratorTest, ExactTextWithNestedTextNodes)
{
    ...
    const auto& selected_end = Position(first_paragraph->firstChild()->nextSibling()->firstChild(), 6);
    ASSERT_EQ("First paragraph text that is longer", PlainText(EphemeralRange(selected_start, selected_end)));

    VerifySelector(selected_start, selected_end, "First%20paragraph%20text%20that%20is%20longer");
}
...
TEST_F(TextFragmentSelectorGeneratorTest, ExactTextWithExtraSpace)
{
    ...
    Node* second_paragraph = GetDocument().getElementById(AtomicString("second"))->firstChild();
    const auto& selected_start = Position(second_paragraph, 0);
    const auto& selected_end = Position(second_paragraph, 27);
    ASSERT_EQ("Second paragraph text", PlainText(EphemeralRange(selected_start, selected_end)));
...
// required.
TEST_F(TextFragmentSelectorGeneratorTest, ExactTextSelector_TooShortNeedsContext)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
    ...
    ASSERT_EQ("unique snippet", PlainText(EphemeralRange(selected_start, selected_end)));

    VerifySelector(selected_start, selected_end, "paragraph%20prefix%20to-,unique%20snippet,-of%20text.");
}

// Exact selector with context test. Case when only one word for prefix and
// suffix is enough to disambiguate the selection.
TEST_F(TextFragmentSelectorGeneratorTest, ExactTextSelector_WithOneWordContext)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
    ...
    const auto& selected_end = Position(first_paragraph, 28);
    ASSERT_EQ("paragraph text that is", PlainText(EphemeralRange(selected_start, selected_end)));

    VerifySelector(selected_start, selected_end, "First-,paragraph%20text%20that%20is,-longer%20than%2020");
}
...
TEST_F(TextFragmentSelectorGeneratorTest, ExactTextSelector_MultipleWordContext)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
    ...
    ASSERT_EQ("not unique snippet of text", PlainText(EphemeralRange(selected_start, selected_end)));
...
// contain extra space.
TEST_F(TextFragmentSelectorGeneratorTest, ExactTextSelector_MultipleWordContext_ExtraSpace)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
...
TEST_F(TextFragmentSelectorGeneratorTest, ExactTextSelector_SamePrefixSuffix)
{
    ...
    VerifySelectorFails(selected_start, selected_end, LinkGenerationError::kContextExhausted);
}

// Exact selector with context test. Case when available prefix and suffix for
// all the occurrences of selected text are the same for the first 10 words. In
// this case generation should be unsuccessful.
TEST_F(TextFragmentSelectorGeneratorTest, ExactTextSelector_SimilarLongPreffixSuffix)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
    ...
    VerifySelectorFails(selected_start, selected_end, LinkGenerationError::kContextLimitReached);
}
...
TEST_F(TextFragmentSelectorGeneratorTest, ExactTextSelector_NoPrefix)
{
    ...
    VerifySelector(selected_start, selected_end, "Not%20unique%20snippet%20of%20text,-followed%20by%20first");
}
...
TEST_F(TextFragmentSelectorGeneratorTest, ExactTextSelector_NoSuffix)
{
    ...
    Node* first_paragraph = GetDocument().getElementById(AtomicString("second"))->firstChild();
...
TEST_F(TextFragmentSelectorGeneratorTest, ExactTextSelector_PrevNodePrefix)
{
    ...
    ASSERT_EQ("not unique snippet", PlainText(EphemeralRange(selected_start, selected_end)));

    VerifySelector(selected_start, selected_end, "not%20unique%20snippet-,not%20unique%20snippet,-of%20text");
}

// Exact selector with context test. Case when available prefix is the
// preceding block, which is a text node.
TEST_F(TextFragmentSelectorGeneratorTest, ExactTextSelector_PrevTextNodePrefix)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
    ...
    VerifySelector(selected_start, selected_end, "text-,not%20unique%20snippet,-of%20text");
}
...
TEST_F(TextFragmentSelectorGeneratorTest, ExactTextSelector_NexttextNodeSuffix)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
    ...
    VerifySelector(selected_start, selected_end, "First%20paragraph%20with-,not%20unique%20snippet,-text");
}
...
TEST_F(TextFragmentSelectorGeneratorTest, RangeSelector)
{
    ...
    ASSERT_EQ("First paragraph text that is longer than 20 chars\n\nSecond", PlainText(EphemeralRange(selected_start, selected_end)));

    VerifySelector(selected_start, selected_end, "First%20paragraph%20text,Second");
}
...
TEST_F(TextFragmentSelectorGeneratorTest, RangeSelector_SameNode)
{
    ...
    ASSERT_EQ("First paragraph text text text text text text text \
text text text text text text text text text text text text text \
text text text text text text text text text text text text text \
text text text text text text text text text text text text text \
text text text text text text text text text and last text",
    ...
    VerifySelector(selected_start, selected_end, "First%20paragraph%20text,and%20last%20text");
}

// It should be more than 300 characters selected from the same node so that
// ranges are used.
TEST_F(TextFragmentSelectorGeneratorTest, RangeSelector_SameNode_MultipleSelections)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
...
text text text text text text text text text text and last text",
        PlainText(EphemeralRange(selected_start, selected_end)));
    ASSERT_EQ(309u, PlainText(EphemeralRange(selected_start, selected_end)).length());
    ...
    ASSERT_EQ("paragraph text text text text text text text \
text text text text text text text text text text text text text \
text text text text text text text text text text text text text \
text text text text text text text text text text text text text \
text text text text text text text text text text and last text",
        PlainText(EphemeralRange(second_selected_start, second_selected_end)));
    ASSERT_EQ(303u, PlainText(EphemeralRange(second_selected_start, second_selected_end)).length());

    VerifySelector(second_selected_start, second_selected_end, "paragraph%20text%20text,and%20last%20text");
}
...
TEST_F(TextFragmentSelectorGeneratorTest, RangeSelector_RangeNotUnique)
{
    ...
    Node* first_paragraph = GetDocument().getElementById(AtomicString("first"))->firstChild();
    Node* first_text = GetDocument().getElementById(AtomicString("text1"))->firstChild();
    const auto& selected_start = Position(first_paragraph, 6);
    const auto& selected_end = Position(first_text, 4);
    ASSERT_EQ("paragraph\n\ntext", PlainText(EphemeralRange(selected_start, selected_end)));

    VerifySelector(selected_start, selected_end, "First-,paragraph,text,-Second%20paragraph");
}

// When selecting multiple short non block nodes, ensure range is produced
// correctly.
TEST_F(TextFragmentSelectorGeneratorTest, RangeSelector_RangeMultipleNonBlockNodes)
{
...
TEST_F(TextFragmentSelectorGeneratorTest, RangeSelector_RangeNotUnique_NoSuffix)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
    ...
    Node* second_paragraph = GetDocument().getElementById(AtomicString("second"))->firstChild();
    Node* second_text = GetDocument().getElementById(AtomicString("text2"))->firstChild();
...
TEST_F(TextFragmentSelectorGeneratorTest, RangeSelector_LongWord)
{
    ...
    ASSERT_EQ("First second third fourth fifth sixth text_text_text_text_\
...
// overlaping.
TEST_F(TextFragmentSelectorGeneratorTest, RangeSelector_OverlapFailOnFirstAttempt)
{
    ScopedExactTextMaxCharsOverride force_range_generation(10);
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    ...
    ASSERT_EQ("one two three four five", PlainText(EphemeralRange(selected_start, selected_end)));
...
TEST_F(TextFragmentSelectorGeneratorTest, RangeSelector_OverlapNeedsContext)
{
    ...
    ASSERT_EQ("one two three four five six seven eight nine ten", PlainText(EphemeralRange(selected_start, selected_end)));
    VerifySelector(selected_start, selected_end,
...
TEST_F(TextFragmentSelectorGeneratorTest, WordLimit)
{
    ...
    ASSERT_EQ("aragraph text that is long", PlainText(EphemeralRange(selected_start, selected_end)));

    VerifySelector(selected_start, selected_end, "paragraph%20text%20that%20is%20longer");
}
...
TEST_F(TextFragmentSelectorGeneratorTest, WordLimit_SelectionStartsAndEndsAtWordLimit)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
    ...
    ASSERT_EQ(" paragraph text that is longer ", PlainText(EphemeralRange(selected_start, selected_end)));
...
TEST_F(TextFragmentSelectorGeneratorTest, PrevNodeIsSiblingsChild)
{
  ...
  <div><p id='start'>First paragraph</p></div><p id='end'>Second paragraph</p>
    ...
    Node* first_paragraph = GetDocument().getElementById(AtomicString("start"))->firstChild();
...
TEST_F(TextFragmentSelectorGeneratorTest, RangeSelector_SameNode_Interrupted)
{
    ...
    ASSERT_EQ("First\nblock text\nparagraph", PlainText(EphemeralRange(start, end)));
...
TEST_F(TextFragmentSelectorGeneratorTest, SelectionEndsWithNonTextWithNoTextChild)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
  ...
  <div id='div'><p id='start'>First paragraph</p><p id='second'>Second paragraph</p><img id="img">
    ...
    Node* div = GetDocument().getElementById(AtomicString("div"));
    const auto& start = Position(first_paragraph, 0);
    const auto& end = Position(div, 3); // Points to the 3rd child of the div, which is <img>
    ASSERT_EQ("First paragraph\n\nSecond paragraph\n\n", PlainText(EphemeralRange(start, end)));
...
TEST_F(TextFragmentSelectorGeneratorTest, SelectionEndsWithImageDiv)
{
  ...
  <div id='div'><p id='start'>First paragraph</p><p id='second'>Second paragraph</p><div id='div_img'><img id="img"></div>
    ...
    Node* div = GetDocument().getElementById(AtomicString("div"));
    const auto& start = Position(first_paragraph, 0);
    const auto& end = Position(div, 3); // Points to the 3rd child of the div, which is div_img
    ASSERT_EQ("First paragraph\n\nSecond paragraph\n\n", PlainText(EphemeralRange(start, end)));
...
TEST_F(TextFragmentSelectorGeneratorTest, OverlappingRange)
{
    ...
    GetDocument().UpdateStyleAndLayoutTree();
    Node* start_node = GetDocument().getElementById(AtomicString("first"))->firstChild();
    Node* end_node = GetDocument().getElementById(AtomicString("first"))->lastChild();
...
<table id='table'>
  <tbody>
    ...
    GetDocument().UpdateStyleAndLayoutTree();
    Node* first_paragraph = GetDocument().getElementById(AtomicString("row1-col1"))->firstChild();
    Node* second_paragraph = GetDocument().getElementById(AtomicString("row1-col3"))->firstChild();
    const auto& start = Position(first_paragraph, 0);
    const auto& end = Position(second_paragraph, 9);
    ASSERT_EQ("row1 col1\trow1 col2\trow1 col3", PlainText(EphemeralRange(start, end)));
...
TEST_F(TextFragmentSelectorGeneratorTest, EscapeSelectorSpecialChars)
{
  ...
  <div id='div'>First paragraph with hyphen- ampersand& and comma,</div>
    ...
    ASSERT_EQ("First paragraph with hyphen- ampersand& and comma,", PlainText(EphemeralRange(start, end)));

    VerifySelector(start, end, "First%20paragraph%20with%20hyphen%2D%20ampersand%26%20and%20comma%2C");
}
...
TEST_F(TextFragmentSelectorGeneratorTest, Multiline_paragraph)
{
  ...
  first paragraph line<br>second paragraph line
    ...
    ASSERT_EQ("first paragraph line\nsecond paragraph line", PlainText(EphemeralRange(start, end)));

    VerifySelector(start, end, "first%20paragraph%20line%0Asecond%20paragraph%20line");
}
...
TEST_F(TextFragmentSelectorGeneratorTest, Nbsp_before_suffix)
{
  ...
  <p id ='p2'>&nbsp;second paragraph line.</p>
    ...
    VerifySelector(start, end, "first%20paragraph-,line.,-second%20paragraph%20line");
}
...
TEST_F(TextFragmentSelectorGeneratorTest, Nbsp_before_prefix)
{
  ...
  <p id ='p2'>&nbsp;    second paragraph line.</p>
    ...
    VerifySelector(start, end, "first%20paragraph%20line.-,second,-paragraph%20line.");
}
...
TEST_F(TextFragmentSelectorGeneratorTest, ContextAfterMaxRange)
{
  ...
  <p>I mean an elephone who tried to use a_telephone</p>
    ...
    ASSERT_EQ("There was an elephant who tried to use a_telephant\n\nI mean an "
...
TEST_F(TextFragmentSelectorGeneratorTest, GetPreviousTextEndPosition_PrevNode)
{
    ...
    Node* first_paragraph = GetDocument().getElementById(AtomicString("first"))->firstChild();
    const auto& expected_position = ToPositionInFlatTree(Position::LastPositionInNode(*first_paragraph));
    EXPECT_EQ(expected_position, CreateGenerator()->GetPreviousTextEndPosition(start));
}

// Check the case when available prefix is a text node outside of selection
// block.
TEST_F(TextFragmentSelectorGeneratorTest, GetPreviousTextEndPosition_PrevTextNode)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
    ...
    Node* node = GetDocument().getElementById(AtomicString("first"))->previousSibling();
    const auto& expected_position = ToPositionInFlatTree(Position::LastPositionInNode(*node));
    EXPECT_EQ(expected_position, CreateGenerator()->GetPreviousTextEndPosition(start));
}

// Check the case when available prefix is a parent node text content outside of
// selection block.
TEST_F(TextFragmentSelectorGeneratorTest, GetPreviousTextEndPosition_ParentNode)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
...
TEST_F(TextFragmentSelectorGeneratorTest, GetPreviousTextEndPosition_SpacesBeforeSelection)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
    ...
    Node* node = GetDocument().getElementById(AtomicString("first"))->firstChild();
    const auto& expected_position = ToPositionInFlatTree(Position::LastPositionInNode(*node));
    EXPECT_EQ(expected_position, CreateGenerator()->GetPreviousTextEndPosition(start));
}
...
TEST_F(TextFragmentSelectorGeneratorTest, GetPreviousTextEndPosition_InvisibleBeforeSelection)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
    ...
    Node* second_paragraph = GetDocument().getElementById(AtomicString("invisible"))->nextSibling();
    const auto& start = PositionInFlatTree(second_paragraph, 6);
    const auto& end = PositionInFlatTree(second_paragraph, 13);
    ASSERT_EQ("Second", PlainText(EphemeralRangeInFlatTree(start, end)));
...
// block.
TEST_F(TextFragmentSelectorGeneratorTest, GetPreviousTextEndPosition_NoPrevious)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
    ...
    Node* second_paragraph = GetDocument().getElementById(AtomicString("first"))->firstChild();
    const auto& start = PositionInFlatTree(second_paragraph, 0);
...
TEST_F(TextFragmentSelectorGeneratorTest, GetNextTextStartPosition_NextNode)
{
    ...
    ASSERT_EQ("First paragraph text", PlainText(EphemeralRangeInFlatTree(start, end)));

    Node* node = GetDocument().getElementById(AtomicString("second"))->firstChild();
    const auto& expected_position = ToPositionInFlatTree(Position::FirstPositionInNode(*node));
    EXPECT_EQ(expected_position, CreateGenerator()->GetNextTextStartPosition(end));
}

// Check the case when there is a commented block between selection and the
// available suffix.
TEST_F(TextFragmentSelectorGeneratorTest, GetNextTextStartPosition_NextNode_WithComment)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
...
// block.
TEST_F(TextFragmentSelectorGeneratorTest, GetNextTextStartPosition_NextTextNode)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
    ...
    Node* node = GetDocument().getElementById(AtomicString("first"))->nextSibling();
    const auto& expected_position = ToPositionInFlatTree(Position::FirstPositionInNode(*node));
    EXPECT_EQ(expected_position, CreateGenerator()->GetNextTextStartPosition(end));
}
...
TEST_F(TextFragmentSelectorGeneratorTest, GetNextTextStartPosition_SpacesAfterSelection)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
...
TEST_F(TextFragmentSelectorGeneratorTest, GetNextTextStartPosition_InvisibleAfterSelection)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
...
TEST_F(TextFragmentSelectorGeneratorTest, GetPreviousTextEndPosition_ShouldSkipNodesWithNoLayoutObject)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
    ...
    ShadowRoot& shadow1 = GetDocument().getElementById(AtomicString("host1"))->AttachShadowRootForTesting(ShadowRootMode::kOpen);
    ...
    Node* first_paragraph = shadow1.getElementById(AtomicString("first"))->firstChild();
    const auto& start = PositionInFlatTree(first_paragraph, 0);
...
TEST_F(TextFragmentSelectorGeneratorTest, RemoveLayoutObjectAsync)
{
    ...
    String selector;
    bool finished = false;
    auto lambda = [](bool& callback_called, String& selector, const TextFragmentSelector& generated_selector, shared_highlighting::LinkGenerationError error) {
        selector = generated_selector.ToString();
        callback_called = true;
    };
    ...
    generator->Generate(*MakeGarbageCollected<RangeInFlatTree>(ToPositionInFlatTree(selected_start), ToPositionInFlatTree(selected_end)), std::move(callback));
    ...
    EXPECT_EQ(generator->state_, TextFragmentSelectorGenerator::SelectorState::kTestCandidate);
