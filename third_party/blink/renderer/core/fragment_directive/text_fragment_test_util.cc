...
TextFragmentAnchorTestBase::TextFragmentAnchorTestBase(base::test::TaskEnvironment::TimeSource time_source)
    : SimTest(time_source)
{
}
...
void TextFragmentAnchorTestBase::SetUp()
{
    ...
    if (enable_virtual_time_) {
        ...
        WebView().Scheduler()->GetVirtualTimeController()->EnableVirtualTime(base::Time());
    }
...
{
    FragmentAnchor* base_anchor = GetDocument().GetFrame()->View()->GetFragmentAnchor();
    CHECK(base_anchor);
    ...
    if (anchor->iteration_ == TextFragmentAnchor::kPostLoad) {
        ...
        if (enable_virtual_time_) {
            ...
            task_environment().FastForwardBy(TextFragmentAnchor::PostLoadTaskTimeout());
        }
...
void TextFragmentAnchorTestBase::DisableVirtualTimeIfSet()
{
    ...
    auto* virtual_time_controller = WebView().Scheduler()->GetVirtualTimeController();
