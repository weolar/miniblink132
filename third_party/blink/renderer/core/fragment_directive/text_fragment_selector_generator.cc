...
bool IsFirstVisiblePosition(Node* node, unsigned pos_offset)
{
    ...
    return node->getNodeType() == Node::kElementNode || pos_offset == 0
        || PlainText(EphemeralRangeInFlatTree(range_start, range_end)).LengthWithStrippedWhiteSpace() == 0;
}
...
bool IsLastVisiblePosition(Node* node, unsigned pos_offset)
{
    ...
    return node->getNodeType() == Node::kElementNode || pos_offset == node->textContent().length()
        || PlainText(EphemeralRangeInFlatTree(range_start, range_end)).LengthWithStrippedWhiteSpace() == 0;
}
...
template <class Direction> Node* NextNonEmptyVisibleTextNode(Node* start_node)
{
    if (!start_node)
        return nullptr;
    ...
    for (Node* node = start_node; node; node = Direction::Next(*node)) {
        ...
        if (next_node->GetLayoutObject() && PlainText(EphemeralRange::RangeOfContents(*next_node)).LengthWithStrippedWhiteSpace() > 0) {
            return next_node;
        }
        node = next_node;
    }
...
Node* ResolvePositionToNode(const PositionInFlatTree& position)
{
    ...
    if (node->getNodeType() == Node::kElementNode && node->hasChildren() && node->childNodes()->item(offset)) {
...
TextFragmentSelectorGenerator::TextFragmentSelectorGenerator(LocalFrame* main_frame)
    : frame_(main_frame)
{
}

void TextFragmentSelectorGenerator::Generate(const RangeInFlatTree& range, GenerateCallback callback)
{
    DCHECK(callback);
    Reset();
    range_ = MakeGarbageCollected<RangeInFlatTree>(range.StartPosition(), range.EndPosition());
...
{
    base::UmaHistogramEnumeration("SharedHighlights.LinkGenerated.StateAtRequest", state_);
}
...
void TextFragmentSelectorGenerator::DidFindMatch(const RangeInFlatTree& match, bool is_unique)
{
    finder_.Clear();
    ...
    if (is_unique
        && PlainText(match.ToEphemeralRange()).LengthWithStrippedWhiteSpace() == PlainText(range_->ToEphemeralRange()).LengthWithStrippedWhiteSpace()) {
        state_ = kSuccess;
...
void TextFragmentSelectorGenerator::AdjustSelection()
{
    ...
    Node* start_container = ephemeral_range.StartPosition().ComputeContainerNode();
    Node* end_container = ephemeral_range.EndPosition().ComputeContainerNode();
    Node* corrected_start = ResolvePositionToNode(ephemeral_range.StartPosition());
    int corrected_start_offset = (corrected_start->isSameNode(start_container)) ? ephemeral_range.StartPosition().ComputeOffsetInContainerNode() : 0;

    Node* corrected_end = ResolvePositionToNode(ephemeral_range.EndPosition());
    int corrected_end_offset = (corrected_end->isSameNode(end_container)) ? ephemeral_range.EndPosition().ComputeOffsetInContainerNode() : 0;
    ...
    if (IsLastVisiblePosition(corrected_start, corrected_start_offset)) {
        corrected_start = FirstNonEmptyVisibleTextNode(FlatTreeTraversal::NextSkippingChildren(*corrected_start));
        ...
        corrected_start_offset = FindWordStartBoundary(start_text.Span16(), corrected_start_offset);
    }
    ...
    if (IsFirstVisiblePosition(corrected_end, corrected_end_offset)) {
        ...
        corrected_end = BackwardNonEmptyVisibleTextNode(FlatTreeTraversal::Previous(*corrected_end));
        if (corrected_end)
        ...
        if (corrected_end_offset != FindWordStartBoundary(end_text.Span16(), corrected_end_offset)) {
            corrected_end_offset = FindWordEndBoundary(end_text.Span16(), corrected_end_offset);
        }
    }

    if (corrected_start != start_container || static_cast<int>(corrected_start_offset) != ephemeral_range.StartPosition().ComputeOffsetInContainerNode()
        || corrected_end != end_container || static_cast<int>(corrected_end_offset) != ephemeral_range.EndPosition().ComputeOffsetInContainerNode()) {
...
void TextFragmentSelectorGenerator::StartGeneration()
{
    ...
    range_->StartPosition().GetDocument()->UpdateStyleAndLayout(DocumentUpdateReason::kFindInPage);
...
void TextFragmentSelectorGenerator::ResolveSelectorState()
{
    ...
    case kNeedsNewCandidate:
        NOTREACHED();
    case kFailure:
        OnSelectorReady(TextFragmentSelector(TextFragmentSelector::SelectorType::kInvalid));
        break;
    case kSuccess:
...
void TextFragmentSelectorGenerator::RunTextFinder()
{
    ...
    finder_ = MakeGarbageCollected<TextFragmentFinder>(*this, *selector_, frame_->GetDocument(), TextFragmentFinder::FindBufferRunnerType::kAsynchronous);
    finder_->FindMatch();
}

PositionInFlatTree TextFragmentSelectorGenerator::GetPreviousTextEndPosition(const PositionInFlatTree& position)
{
    PositionInFlatTree search_end_position = PositionInFlatTree::FirstPositionInNode(*frame_->GetDocument()->documentElement()->firstChild());
    PositionInFlatTree previous_text_position = TextFragmentFinder::PreviousTextPosition(position, search_end_position);
...
PositionInFlatTree TextFragmentSelectorGenerator::GetNextTextStartPosition(const PositionInFlatTree& position)
{
    PositionInFlatTree search_end_position = PositionInFlatTree::LastPositionInNode(*frame_->GetDocument()->documentElement()->lastChild());
    PositionInFlatTree next_text_position = TextFragmentFinder::NextTextPosition(position, search_end_position);
...
void TextFragmentSelectorGenerator::GenerateExactSelector()
{
    ...
    if (!ephemeral_range.StartPosition().ComputeContainerNode() || !ephemeral_range.EndPosition().ComputeContainerNode()) {
        state_ = kFailure;
        error_ = LinkGenerationError::kEmptySelection;
        return;
    }
    ...
    if (!TextFragmentFinder::IsInSameUninterruptedBlock(ephemeral_range.StartPosition(), ephemeral_range.EndPosition())) {
        step_ = kRange;
        return;
    }
    ...
    selector_ = std::make_unique<TextFragmentSelector>(TextFragmentSelector::SelectorType::kExact, selected_text, "", "", "");
...
void TextFragmentSelectorGenerator::ExtendRangeSelector()
{
    ...
    if (!range_start_iterator_ && !range_end_iterator_) {
        PositionInFlatTree range_start_position = GetNextTextStartPosition(range_->StartPosition());
        PositionInFlatTree range_end_position = GetPreviousTextEndPosition(range_->EndPosition());
        ...
        range_start_iterator_ = MakeGarbageCollected<ForwardSameBlockWordIterator>(range_start_position);
        range_end_iterator_ = MakeGarbageCollected<BackwardSameBlockWordIterator>(range_end_position);

        // Use at least 3 words from both sides for more robust link to text unless
        // the selected text is shorter than 6 words.
        if (TextFragmentFinder::IsInSameUninterruptedBlock(range_start_position, range_end_position)) {
            num_words_to_add = 0;
            auto* range_start_counter = MakeGarbageCollected<ForwardSameBlockWordIterator>(range_start_position);
            ...
            while (num_words_to_add < kMinWordCount * 2 && range_start_counter->AdvanceNextWord()
    ...
    String start = range_start_iterator_ ? range_start_iterator_->TextFromStart() : "";
    ...
    selector_ = std::make_unique<TextFragmentSelector>(TextFragmentSelector::SelectorType::kRange, start, end, "", "");
    state_ = kTestCandidate;
}
...
void TextFragmentSelectorGenerator::ExtendContext()
{
    ...
    if (!suffix_iterator_ && !prefix_iterator_) {
        PositionInFlatTree suffix_start_position = GetNextTextStartPosition(range_->EndPosition());
        PositionInFlatTree prefix_end_position = GetPreviousTextEndPosition(range_->StartPosition());

        if (suffix_start_position.IsNotNull()) {
            suffix_iterator_ = MakeGarbageCollected<ForwardSameBlockWordIterator>(suffix_start_position);
        }

        if (prefix_end_position.IsNotNull()) {
            prefix_iterator_ = MakeGarbageCollected<BackwardSameBlockWordIterator>(prefix_end_position);
        }
    ...
    selector_ = std::make_unique<TextFragmentSelector>(selector_->Type(), selector_->Start(), selector_->End(), prefix, suffix);
...
void TextFragmentSelectorGenerator::RecordAllMetrics(const TextFragmentSelector& selector)
{
    LinkGenerationStatus status
        = selector.Type() == TextFragmentSelector::SelectorType::kInvalid ? LinkGenerationStatus::kFailure : LinkGenerationStatus::kSuccess;
    ...
    if (selector.Type() != TextFragmentSelector::SelectorType::kInvalid) {
        UMA_HISTOGRAM_TIMES("SharedHighlights.LinkGenerated.TimeToGenerate", base::DefaultTickClock::GetInstance()->NowTicks() - generation_start_time_);

        shared_highlighting::LogLinkGeneratedSuccessUkmEvent(recorder, source_id);
    } else {
        UMA_HISTOGRAM_EXACT_LINEAR("SharedHighlights.LinkGenerated.Error.Iterations", iteration_, kMaxIterationCountToRecord);
        UMA_HISTOGRAM_TIMES("SharedHighlights.LinkGenerated.Error.TimeToGenerate", base::DefaultTickClock::GetInstance()->NowTicks() - generation_start_time_);
        ...
        shared_highlighting::LogLinkGeneratedErrorUkmEvent(recorder, source_id, error_);
    }
}

void TextFragmentSelectorGenerator::OnSelectorReady(const TextFragmentSelector& selector)
{
...
// static
void TextFragmentSelectorGenerator::OverrideExactTextMaxCharsForTesting(int value)
{
    if (value < 0)
