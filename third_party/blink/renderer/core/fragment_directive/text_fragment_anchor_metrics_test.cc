...
public:
    TextFragmentAnchorMetricsTest()
        : TextFragmentAnchorTestBase(base::test::TaskEnvironment::TimeSource::MOCK_TIME)
    {
    }
...
{
    SimRequest request("https://example.com/test.html#:~:text=test&text=cat", "text/html");
    ...
    histogram_tester_.ExpectUniqueSample("TextFragmentAnchor.Unknown.MatchRate", 50, 1);

    histogram_tester_.ExpectTotalCount("TextFragmentAnchor.Unknown.AmbiguousMatch", 1);
    histogram_tester_.ExpectUniqueSample("TextFragmentAnchor.Unknown.AmbiguousMatch", 1, 1);

    histogram_tester_.ExpectTotalCount("TextFragmentAnchor.Unknown.TimeToScrollIntoView", 1);

    histogram_tester_.ExpectTotalCount("TextFragmentAnchor.LinkOpenSource", 1);
    histogram_tester_.ExpectUniqueSample("TextFragmentAnchor.LinkOpenSource", 0, 1);
}
...
TEST_F(TextFragmentAnchorMetricsTest, UMAMetricsCollectedSearchEngineReferrer)
{
    ...
    params.requestor_origin = WebSecurityOrigin::CreateFromString(WebString::FromUTF8("https://www.bing.com"));
    SimRequest request("https://example.com/test.html#:~:text=test&text=cat", "text/html", params);
    LoadURL("https://example.com/test.html#:~:text=test&text=cat");
    request.Complete(R"HTML(
    <!DOCTYPE html>
    <style>
    ...
    histogram_tester_.ExpectTotalCount("TextFragmentAnchor.SearchEngine.MatchRate", 1);
    histogram_tester_.ExpectUniqueSample("TextFragmentAnchor.SearchEngine.MatchRate", 50, 1);

    histogram_tester_.ExpectTotalCount("TextFragmentAnchor.SearchEngine.AmbiguousMatch", 1);
    histogram_tester_.ExpectUniqueSample("TextFragmentAnchor.SearchEngine.AmbiguousMatch", 1, 1);

    histogram_tester_.ExpectTotalCount("TextFragmentAnchor.SearchEngine.TimeToScrollIntoView", 1);

    histogram_tester_.ExpectTotalCount("TextFragmentAnchor.LinkOpenSource", 1);
    histogram_tester_.ExpectUniqueSample("TextFragmentAnchor.LinkOpenSource", 1, 1);
}
...
TEST_F(TextFragmentAnchorMetricsTest, NoMatchFoundWithUnknownSource)
{
    ...
    histogram_tester_.ExpectTotalCount("TextFragmentAnchor.Unknown.MatchRate", 1);
    histogram_tester_.ExpectUniqueSample("TextFragmentAnchor.Unknown.MatchRate", 0, 1);

    histogram_tester_.ExpectTotalCount("TextFragmentAnchor.Unknown.AmbiguousMatch", 1);
    histogram_tester_.ExpectUniqueSample("TextFragmentAnchor.Unknown.AmbiguousMatch", 0, 1);

    histogram_tester_.ExpectTotalCount("TextFragmentAnchor.Unknown.TimeToScrollIntoView", 0);
...
TEST_F(TextFragmentAnchorMetricsTest, NoMatchFoundWithSearchEngineSource)
{
    ...
    SimRequest::Params params;
    params.requestor_origin = WebSecurityOrigin::CreateFromString(WebString::FromUTF8("https://www.bing.com"));
    SimRequest request("https://example.com/test.html#:~:text=cat", "text/html", params);
    LoadURL("https://example.com/test.html#:~:text=cat");
    request.Complete(R"HTML(
    <!DOCTYPE html>
    <style>
    ...
    histogram_tester_.ExpectTotalCount("TextFragmentAnchor.SearchEngine.MatchRate", 1);
    histogram_tester_.ExpectUniqueSample("TextFragmentAnchor.SearchEngine.MatchRate", 0, 1);

    histogram_tester_.ExpectTotalCount("TextFragmentAnchor.SearchEngine.AmbiguousMatch", 1);
    histogram_tester_.ExpectUniqueSample("TextFragmentAnchor.SearchEngine.AmbiguousMatch", 0, 1);

    histogram_tester_.ExpectTotalCount("TextFragmentAnchor.SearchEngine.TimeToScrollIntoView", 0);
...
TEST_F(TextFragmentAnchorMetricsTest, NoTextFragmentAnchor)
{
    ...
    histogram_tester_.ExpectTotalCount("TextFragmentAnchor.Unknown.AmbiguousMatch", 0);
...
TEST_F(TextFragmentAnchorMetricsTest, MatchFoundNoScroll)
{
    ...
    histogram_tester_.ExpectTotalCount("TextFragmentAnchor.Unknown.MatchRate", 1);
    histogram_tester_.ExpectUniqueSample("TextFragmentAnchor.Unknown.MatchRate", 100, 1);
...
TEST_F(TextFragmentAnchorMetricsTest, InvalidFragmentDirective)
{
    ...
    Vector<std::pair<String, int>> test_cases = { { "", kUncounted }, { "#element", kUncounted }, { "#doesntExist", kUncounted }, { "#:~:element", kCounted },
        { "#element:~:", kCounted }, { "#foo:~:bar", kCounted }, { "#:~:utext=foo", kCounted }, { "#:~:text=foo", kUncounted },
        { "#:~:text=foo&invalid", kUncounted }, { "#foo:~:text=foo", kUncounted } };
    ...
    for (auto test_case : test_cases) {
      ...
      <p id="element">This is a test page</p>
        ...
        bool is_use_counted = GetDocument().IsUseCounted(WebFeature::kInvalidFragmentDirective);
        if (test_case.second == kCounted) {
            EXPECT_TRUE(is_use_counted) << "Expected invalid directive in case: " << test_case.first;
        } else {
            EXPECT_FALSE(is_use_counted) << "Expected valid directive in case: " << test_case.first;
        }
    }
}

class TextFragmentRelatedMetricTest : public TextFragmentAnchorMetricsTest, public testing::WithParamInterface<bool> {
public:
    TextFragmentRelatedMetricTest()
        : text_fragment_anchors_state_(GetParam())
    {
    }
...
INSTANTIATE_TEST_SUITE_P(All, TextFragmentRelatedMetricTest, testing::Values(false, true));
...
TEST_P(TextFragmentRelatedMetricTest, TextFragmentAPIUseCounter)
{
    ...
    EXPECT_EQ(text_fragments_enabled, GetDocument().IsUseCounted(WebFeature::kV8Document_FragmentDirective_AttributeGetter));
}
...
TEST_P(TextFragmentRelatedMetricTest, TextFragmentActivationDoesNotCountAPI)
{
    ...
    EXPECT_EQ(text_fragments_enabled, GetDocument().IsUseCounted(WebFeature::kTextFragmentAnchor));
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kV8Document_FragmentDirective_AttributeGetter));
}
...
{
    SimRequest request("https://example.com/test.html#:~:text=not%20on%20the%20page", "text/html");
...
{
    SimRequest::Params params;
    params.response_http_headers.insert("Document-Policy", "no-force-load-at-top");
    SimRequest request("https://example.com/test.html", "text/html", params);
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
    <!DOCTYPE html>
...
TEST_F(TextFragmentAnchorMetricsTest, TextFragmentBlockedByForceLoadAtTopUseCounter)
{
    ...
    // there is no text fragment.
    {
      ...
      <p>This is a test page</p>
        ...
        EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kTextFragmentBlockedByForceLoadAtTop));
    }
    ...
    // record TextFragmentBlocked.
    {
        SimRequest::Params params;
        params.response_http_headers.insert("Document-Policy", "force-load-at-top");
        SimRequest request("https://example.com/test2.html#:~:text=foo", "text/html", params);
      ...
      <p>This is a test page</p>
        ...
        ASSERT_TRUE(GetDocument().IsUseCounted(WebFeature::kForceLoadAtTop));
        EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kTextFragmentBlockedByForceLoadAtTop));
    }

    // Ensure that an unblocked text fragment doesn't cause recording the
    // TextFragmentBlocked counter.
    {
        SimRequest request("https://example.com/test3.html#:~:text=foo", "text/html");
...
TEST_F(TextFragmentAnchorMetricsTest, TextFragmentLinkOpenSource_GoogleDomain)
{
    ...
    SimRequest::Params params;
    params.requestor_origin = WebSecurityOrigin::CreateFromString(WebString::FromUTF8("https://www.mail.google.com"));
    SimRequest request("https://example.com/test.html#:~:text=test&text=cat", "text/html", params);
    LoadURL("https://example.com/test.html#:~:text=test&text=cat");
    request.Complete(R"HTML(
    <!DOCTYPE html>
...
{
    {
        SimRequest request("https://example.com/test.html#:~:text=RegularDOM", "text/html");
      ...
      <p>This is RegularDOM</p>
        ...
        EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kTextDirectiveInShadowDOM));
    }

    {
        SimRequest request("https://example.com/shadowtest.html#:~:text=ShadowDOM", "text/html");
      ...
      <script>
        ...
        EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kTextDirectiveInShadowDOM));
    }
}
