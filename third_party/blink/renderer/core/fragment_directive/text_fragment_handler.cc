...
void TextFragmentHandler::BindTextFragmentReceiver(mojo::PendingReceiver<mojom::blink::TextFragmentReceiver> producer)
{
    selector_producer_.reset();
    selector_producer_.Bind(std::move(producer), GetFrame()->GetTaskRunner(blink::TaskType::kInternalDefault));
}
...
{
    DCHECK(shared_highlighting::ShouldOfferLinkToText(GURL(GetFrame()->GetDocument()->Url())));

    response_callback_ = std::move(callback);
    selector_ready_status_ = preemptive_generation_result_.has_value() ? shared_highlighting::LinkGenerationReadyStatus::kRequestedAfterReady
    ...
    if (!GetTextFragmentSelectorGenerator()) {
        ...
        InvokeReplyCallback(TextFragmentSelector(TextFragmentSelector::SelectorType::kInvalid), error_);
        return;
    }
...
void TextFragmentHandler::GetExistingSelectors(GetExistingSelectorsCallback callback)
{
...
bool TextFragmentHandler::IsOverTextFragment(const HitTestResult& result)
{
    ...
    // |kPrePaintClean|.
    DCHECK_GE(result.InnerNodeFrame()->GetDocument()->Lifecycle().GetState(), DocumentLifecycle::kPrePaintClean);

    DocumentMarkerController& marker_controller = result.InnerNodeFrame()->GetDocument()->Markers();
    PositionWithAffinity pos_with_affinity = result.GetPosition();
    const Position marker_position = pos_with_affinity.GetPosition();
    auto markers = marker_controller.MarkersAroundPosition(ToPositionInFlatTree(marker_position), DocumentMarker::MarkerTypes::TextFragment());
    return !markers.empty();
}

void TextFragmentHandler::ExtractTextFragmentsMatches(ExtractTextFragmentsMatchesCallback callback)
{
    Vector<String> text_fragment_matches;

    for (auto& annotation : annotation_agents_) {
        if (annotation->IsAttached()) {
            text_fragment_matches.push_back(PlainText(annotation->GetAttachedRange().ToEphemeralRange()));
        }
    }
...
void TextFragmentHandler::ExtractFirstFragmentRect(ExtractFirstFragmentRectCallback callback)
{
    ...
    for (auto& annotation : annotation_agents_) {
        ...
        PhysicalRect bounding_box(ComputeTextRect(annotation->GetAttachedRange().ToEphemeralRange()));
        rect_in_viewport = GetFrame()->View()->FrameToViewport(ToEnclosingRect(bounding_box));
        break;
    }
...
void TextFragmentHandler::DidFinishSelectorGeneration(const TextFragmentSelector& selector, shared_highlighting::LinkGenerationError error)
{
...
void TextFragmentHandler::StartGeneratingForCurrentSelection()
{
    ...
    VisibleSelectionInFlatTree selection = GetFrame()->Selection().ComputeVisibleSelectionInFlatTree();
    EphemeralRangeInFlatTree selection_range(selection.Start(), selection.End());
    RangeInFlatTree* current_selection_range = MakeGarbageCollected<RangeInFlatTree>(selection_range.StartPosition(), selection_range.EndPosition());
    if (!GetTextFragmentSelectorGenerator()) {
        text_fragment_selector_generator_ = MakeGarbageCollected<TextFragmentSelectorGenerator>(GetFrame());
    }
    GetTextFragmentSelectorGenerator()->Generate(
        *current_selection_range, WTF::BindOnce(&TextFragmentHandler::DidFinishSelectorGeneration, WrapWeakPersistent(this)));
}
...
void TextFragmentHandler::InvokeReplyCallback(const TextFragmentSelector& selector, shared_highlighting::LinkGenerationError error)
{
    DCHECK(response_callback_);
    DCHECK(selector_ready_status_.has_value());

    std::move(response_callback_).Run(selector.ToString(), error, selector_ready_status_.value());
...
bool TextFragmentHandler::ShouldPreemptivelyGenerateFor(LocalFrame* frame)
{
    ...
    return shared_highlighting::SupportsLinkGenerationInIframe(GURL(frame->GetDocument()->Url()));
}
...
void TextFragmentHandler::OpenedContextMenuOverSelection(LocalFrame* frame)
{
    ...
    if (!shared_highlighting::ShouldOfferLinkToText(GURL(frame->GetDocument()->Url()))) {
        return;
    }
...
// static
void TextFragmentHandler::DidCreateTextFragment(AnnotationAgentImpl& agent, Document& owning_document)
{
