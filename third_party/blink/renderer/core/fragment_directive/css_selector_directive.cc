...
{
    if (!directive_string.StartsWith(shared_highlighting::kSelectorDirectiveParameterName)
    ...
    directive_string
        .Substring(shared_highlighting::kSelectorDirectiveParameterNameLength,
            directive_string.length() - shared_highlighting::kSelectorDirectiveParameterNameLength - shared_highlighting::kSelectorDirectiveSuffixLength)
    ...
    for (auto& part : parts) {
        if (part.StartsWith(shared_highlighting::kSelectorDirectiveValuePrefix)) {
            ...
            if (parsed_value)
                return false;
            value = DecodeURLEscapeSequences(part.Substring(shared_highlighting::kSelectorDirectiveValuePrefixLength), DecodeURLMode::kUTF8);
            parsed_value = true;
        } else if (part.StartsWith(shared_highlighting::kSelectorDirectiveTypePrefix)) {
            // ambiguous case, can't have two type= parts
            if (parsed_type)
                return false;
            type = part.Substring(shared_highlighting::kSelectorDirectiveTypePrefixLength);
            parsed_type = true;
        }
    }
...
CssSelectorDirective* CssSelectorDirective::TryParse(const String& directive_string)
{
    String value;
...
{
    return "selector(type=CssSelector,value=" + EncodeWithURLEscapeSequences(value_) + ")";
}
