...
CssSelectorFragmentAnchor* CssSelectorFragmentAnchor::TryCreate(const KURL& url, LocalFrame& frame, bool should_scroll)
{
    DCHECK(RuntimeEnabledFeatures::CSSSelectorFragmentAnchorEnabled());

    Document& doc = *frame.GetDocument();
    HeapVector<Member<CssSelectorDirective>> css_selector_directives = frame.GetDocument()->fragmentDirective().GetDirectives<CssSelectorDirective>();
    ...
    return MakeGarbageCollected<CssSelectorFragmentAnchor>(*anchor_node, frame, should_scroll);
}

CssSelectorFragmentAnchor::CssSelectorFragmentAnchor(Element& anchor_node, LocalFrame& frame, bool should_scroll)
