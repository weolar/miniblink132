...
bool IsWordBounded(EphemeralRangeInFlatTree range, bool start, bool end)
{
    ...
    if (start_position != 0 && start) {
        ...
        wtf_size_t word_start = FindWordStartBoundary(start_text.Span16(), start_position);
    ...
    if (end_position != end_text.length() && end) {
        ...
        wtf_size_t word_end = FindWordEndBoundary(end_text.Span16(), end_position - 1);
...
// static
PositionInFlatTree TextFragmentFinder::NextTextPosition(PositionInFlatTree position, PositionInFlatTree end_position)
{
    const TextIteratorBehavior options = TextIteratorBehavior::Builder().SetEmitsSpaceForNbsp(true).Build();
...
// static
PositionInFlatTree TextFragmentFinder::PreviousTextPosition(PositionInFlatTree position, PositionInFlatTree max_position)
{
    const TextIteratorBehavior options = TextIteratorBehavior::Builder().SetEmitsSpaceForNbsp(true).Build();
    BackwardsCharacterIteratorInFlatTree char_it(EphemeralRangeInFlatTree(max_position, position), options);
...
void TextFragmentFinder::OnFindMatchInRangeComplete(
    String search_text, RangeInFlatTree* search_range, bool word_start_bounded, bool word_end_bounded, const EphemeralRangeInFlatTree& match)
{
    ...
    if (match.IsNull() || IsWordBounded(match, word_start_bounded, word_end_bounded)) {
        switch (step_) {
        case kMatchPrefix:
    ...
    FindMatchInRange(search_text, search_range, word_start_bounded, word_end_bounded);
}

void TextFragmentFinder::FindMatchInRange(String search_text, RangeInFlatTree* search_range, bool word_start_bounded, bool word_end_bounded)
{
    find_buffer_runner_->FindMatchInRange(search_range, search_text, FindOptions().SetCaseInsensitive(true),
        WTF::BindOnce(&TextFragmentFinder::OnFindMatchInRangeComplete, WrapWeakPersistent(this), search_text, WrapWeakPersistent(search_range),
            word_start_bounded, word_end_bounded));
}
...
void TextFragmentFinder::OnPrefixMatchComplete(EphemeralRangeInFlatTree prefix_match)
{
...
void TextFragmentFinder::FindTextStart()
{
    ...
    // details.
    const bool end_at_word_boundary = !selector_.End().empty() || selector_.Suffix().empty();
    if (prefix_match_) {
        search_range_->SetStart(NextTextPosition(prefix_match_->EndPosition(), match_range_->EndPosition()));
...
void TextFragmentFinder::OnTextStartMatchComplete(EphemeralRangeInFlatTree potential_match)
{
    if (prefix_match_) {
        PositionInFlatTree next_position_after_prefix = NextTextPosition(prefix_match_->EndPosition(), match_range_->EndPosition());
        // We found a potential match but it didn't immediately follow the prefix.
        if (!potential_match.IsNull() && potential_match.StartPosition() != next_position_after_prefix) {
            potential_match_.Clear();
    ...
    if (!prefix_match_) {
        match_range_->SetStart(FirstWordBoundaryAfter(potential_match.StartPosition()));
    }
    if (!range_end_search_start_) {
        range_end_search_start_ = MakeGarbageCollected<RelocatablePosition>(ToPositionInDOMTree(potential_match.EndPosition()));
    } else {
        range_end_search_start_->SetPosition(ToPositionInDOMTree(potential_match.EndPosition()));
    }
...
void TextFragmentFinder::FindTextEnd()
{
    ...
    if (!selector_.End().empty()) {
        search_range_->SetStart(ToPositionInFlatTree(range_end_search_start_->GetPosition()));
...
void TextFragmentFinder::OnTextEndMatchComplete(EphemeralRangeInFlatTree text_end_match)
{
...
void TextFragmentFinder::FindSuffix()
{
    ...
    search_range_->SetStart(NextTextPosition(potential_match_->EndPosition(), match_range_->EndPosition()));
    FindMatchInRange(selector_.Suffix(), search_range_,
        /*word_start_bounded=*/false, /*word_end_bounded=*/true);
}

void TextFragmentFinder::OnSuffixMatchComplete(EphemeralRangeInFlatTree suffix_match)
{
    ...
    PositionInFlatTree next_position_after_match = NextTextPosition(potential_match_->EndPosition(), match_range_->EndPosition());
    ...
    range_end_search_start_->SetPosition(ToPositionInDOMTree(potential_match_->EndPosition()));
    GoToStep(kMatchTextEnd);
}
...
// static
bool TextFragmentFinder::IsInSameUninterruptedBlock(const PositionInFlatTree& start, const PositionInFlatTree& end)
{
    Node* start_node = start.ComputeContainerNode();
    Node* end_node = end.ComputeContainerNode();
    if (!start_node || !start_node->GetLayoutObject() || !end_node || !end_node->GetLayoutObject()) {
        return true;
    }
    return FindBuffer::IsInSameUninterruptedBlock(*start_node, *end_node);
}

TextFragmentFinder::TextFragmentFinder(Client& client, const TextFragmentSelector& selector, Document* document, FindBufferRunnerType runner_type)
    : client_(client)
    , selector_(selector)
    , document_(document)
{
...
void TextFragmentFinder::FindMatch()
{
    ...
    auto forced_lock_scope = document_->GetDisplayLockDocumentState().GetScopedForceActivatableLocks();
...
void TextFragmentFinder::FindMatchFromPosition(PositionInFlatTree search_start)
{
    PositionInFlatTree search_end;
    if (document_->documentElement() && document_->documentElement()->lastChild()) {
        search_end = PositionInFlatTree::AfterNode(*document_->documentElement()->lastChild());
    } else {
        search_end = PositionInFlatTree::LastPositionInNode(*document_);
    }
    search_range_ = MakeGarbageCollected<RangeInFlatTree>(search_start, search_end);
    match_range_ = MakeGarbageCollected<RangeInFlatTree>(search_start, search_end);
    potential_match_.Clear();
    prefix_match_.Clear();
    GoToStep(kMatchPrefix);
}
...
void TextFragmentFinder::SetPotentialMatch(EphemeralRangeInFlatTree range)
{
    if (potential_match_) {
        ...
        potential_match_ = MakeGarbageCollected<RangeInFlatTree>(range.StartPosition(), range.EndPosition());
    }
}
...
void TextFragmentFinder::SetPrefixMatch(EphemeralRangeInFlatTree range)
{
    if (prefix_match_) {
        ...
        prefix_match_ = MakeGarbageCollected<RangeInFlatTree>(range.StartPosition(), range.EndPosition());
    }
}

bool TextFragmentFinder::HasValidRanges()
{
    return !((prefix_match_ && (prefix_match_->IsNull() || !prefix_match_->IsConnected()))
        || (potential_match_ && (potential_match_->IsNull() || !potential_match_->IsConnected()))
        || (search_range_ && (search_range_->IsNull() || !search_range_->IsConnected()))
        || (match_range_ && (match_range_->IsNull() || !match_range_->IsConnected())));
}
