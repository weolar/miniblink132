...
public:
    ...
    {
        return GetDocument().View()->GetFragmentAnchor() && GetDocument().View()->GetFragmentAnchor()->IsSelectorFragmentAnchor();
    }

    const CSSValue* GetComputedValue(const CSSPropertyID& property_id, const Element& element)
    {
        return CSSProperty::Get(property_id)
            .CSSValueFromComputedStyle(element.ComputedStyleRef(), nullptr /* layout_object */, false /* allow_visited_style */, CSSValuePhase::kComputedValue);
    }

    bool IsElementOutlined(const Element& element)
    {
        const CSSValue* value = GetComputedValue(CSSPropertyID::kOutlineWidth, element);
...
{
    SimRequest main_request("https://example.com/"
...
{
    SimRequest main_request("https://example.com/test.html"
                            "#:~:selector(type=CssSelector,value=img[src$=\"second.svg\"])"
                            "&selector(type=CssSelector,value=img[src$=\"first.svg\"])",
        "text/html");
    SimRequest first_img_request("https://example.com/first.svg", "image/svg+xml");
    SimRequest second_img_request("https://example.com/second.svg", "image/svg+xml");
...
TEST_F(CssSelectorFragmentAnchorTest, PrioritizeCssSelectorFragmentOverElementFragment)
{
    SimRequest main_request("https://example.com/test.html#element"
                            ":~:selector(type=CssSelector,value=img[src$=\"first.svg\"])",
        "text/html");
    SimRequest image_request("https://example.com/first.svg", "image/svg+xml");

    LoadURL("https://example.com/test.html#element"
            ":~:selector(type=CssSelector,value=img[src$=\"first.svg\"])");
...
TEST_F(CssSelectorFragmentAnchorTest, FragmentStaysAfterUserClicks)
{
    ...
    KURL expected_url = GetDocument().GetFrame()->Loader().GetDocumentLoader()->GetHistoryItem()->Url();
...
TEST_F(CssSelectorFragmentAnchorTest, TargetElementIsNotHighlightedWithElementFragment)
{
...
TEST_F(CssSelectorFragmentAnchorTest, TargetElementIsNotHighlightedWithTextFragment)
{
    SimRequest main_request("https://example.com/test.html#:~:text=some other", "text/html");
...
TEST_F(CssSelectorFragmentAnchorTest, SelectorFragmentTargetOutline)
{
    ...
    auto* anchor = To<HTMLAnchorElement>(GetDocument().getElementById(AtomicString("element")));
    anchor->click();
