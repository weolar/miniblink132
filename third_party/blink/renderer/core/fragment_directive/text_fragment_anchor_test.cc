...
public:
    explicit TextFragmentAnchorTestController(base::test::TaskEnvironment::TimeSource time_source)
    ...
    {
        WebMouseEvent event(WebInputEvent::Type::kMouseDown, gfx::PointF(x, y), gfx::PointF(x, y), WebPointerProperties::Button::kLeft, 0,
            WebInputEvent::Modifiers::kLeftButtonDown, base::TimeTicks::Now());
        event.SetFrameScale(1);
        WebView().MainFrameWidget()->ProcessInputEventSynchronouslyForTesting(WebCoalescedInputEvent(event, ui::LatencyInfo()));
    }

    void SimulateRightClick(int x, int y)
    {
        WebMouseEvent event(WebInputEvent::Type::kMouseDown, gfx::PointF(x, y), gfx::PointF(x, y), WebPointerProperties::Button::kRight, 0,
            WebInputEvent::Modifiers::kLeftButtonDown, base::TimeTicks::Now());
        event.SetFrameScale(1);
        WebView().MainFrameWidget()->ProcessInputEventSynchronouslyForTesting(WebCoalescedInputEvent(event, ui::LatencyInfo()));
    }
    ...
    void LoadAhem()
    {
        std::optional<Vector<char>> data = test::ReadFromFile(test::CoreTestDataPath("Ahem.ttf"));
        ASSERT_TRUE(data);
        auto* buffer = MakeGarbageCollected<V8UnionArrayBufferOrArrayBufferViewOrString>(DOMArrayBuffer::Create(base::as_byte_span(*data)));
        FontFace* ahem = FontFace::Create(GetDocument().GetFrame()->DomWindow(), AtomicString("Ahem"), buffer, FontFaceDescriptors::Create());

        ScriptState* script_state = ToScriptStateForMainWorld(GetDocument().GetFrame());
        DummyExceptionStateForTesting exception_state;
        FontFaceSetDocument::From(GetDocument())->addForBinding(script_state, ahem, exception_state);
    }
...
private:
    void InjectEvent(WebInputEvent::Type type, int x, int y)
    {
        if (WebInputEvent::IsGestureEventType(type)) {
            ...
            WebTouchEvent event(type, WebInputEvent::kNoModifiers, base::TimeTicks::Now());
            event.SetFrameScale(1);

            WebPointerProperties pointer(
                0, WebPointerProperties::PointerType::kTouch, WebPointerProperties::Button::kNoButton, gfx::PointF(x, y), gfx::PointF(x, y));
...
public:
    TextFragmentAnchorTest()
        : TextFragmentAnchorTestController(base::test::TaskEnvironment::TimeSource::MOCK_TIME)
    {
    }
};
...
TEST_F(TextFragmentAnchorTest, BasicSmokeTest)
{
    ...
    EXPECT_TRUE(ViewportRect().Contains(BoundingRectInFrame(p)))
        << "<p> Element wasn't scrolled into view, viewport's scroll offset: " << LayoutViewport()->GetScrollOffset().ToString();
}
...
{
    SimRequest request("https://example.com/test.html#:~:text=unicorn", "text/html");
    ...
    // Force a layout
    GetDocument().body()->setAttribute(html_names::kStyleAttr, AtomicString("height: 1300px"));
    Compositor().BeginFrame();
...
TEST_F(TextFragmentAnchorTest, MultipleMatches)
{
    ...
    EXPECT_TRUE(ViewportRect().Contains(BoundingRectInFrame(first)))
        << "First <p> wasn't scrolled into view, viewport's scroll offset: " << LayoutViewport()->GetScrollOffset().ToString();
...
TEST_F(TextFragmentAnchorTest, NestedBlocks)
{
    ...
    EXPECT_TRUE(ViewportRect().Contains(BoundingRectInFrame(match)))
        << "<p> wasn't scrolled into view, viewport's scroll offset: " << LayoutViewport()->GetScrollOffset().ToString();
}
...
{
    SimRequest request("https://example.com/test.html#:~:text=test&text=more", "text/html");
...
TEST_F(TextFragmentAnchorTest, FirstTextFragmentNotFound)
{
    ...
    EXPECT_TRUE(ViewportRect().Contains(BoundingRectInFrame(second)))
        << "Second <p> wasn't scrolled into view, viewport's scroll offset: " << LayoutViewport()->GetScrollOffset().ToString();
...
{
    SimRequest request("https://example.com/"
...
{
    SimRequest request("https://example.com/test.html#:~:text=This,page", "text/html");
    ...
    EXPECT_EQ(*GetDocument().getElementById(AtomicString("text")), *GetDocument().CssTarget());
    EXPECT_EQ(1u, GetDocument().Markers().Markers().size());

    // Expect marker on "This is a test page".
    auto* text = To<Text>(GetDocument().getElementById(AtomicString("text"))->firstChild());
    DocumentMarkerVector markers = GetDocument().Markers().MarkersFor(*text, DocumentMarker::MarkerTypes::TextFragment());
...
{
    SimRequest request("https://example.com/test.html#:~:text=test,paragraph", "text/html");
    ...
    auto* text1 = To<Text>(GetDocument().getElementById(AtomicString("text1"))->firstChild());
    DocumentMarkerVector markers = GetDocument().Markers().MarkersFor(*text1, DocumentMarker::MarkerTypes::TextFragment());
    ASSERT_EQ(1u, markers.size());
    EXPECT_EQ(10u, markers.at(0)->StartOffset());
    EXPECT_EQ(19u, markers.at(0)->EndOffset());

    // Expect marker on "with another paragraph"
    auto* text2 = To<Text>(GetDocument().getElementById(AtomicString("text2"))->firstChild());
    markers = GetDocument().Markers().MarkersFor(*text2, DocumentMarker::MarkerTypes::TextFragment());
    ASSERT_EQ(1u, markers.size());
    EXPECT_EQ(0u, markers.at(0)->StartOffset());
...
{
    SimRequest request("https://example.com/test.html#:~:text=test,cat", "text/html");
...
{
    SimRequest request("https://example.com/test.html#:~:text=This-,is,test,-page", "text/html");
...
{
    SimRequest request("https://example.com/test.html#:~:text=prefix-,is,test,-page", "text/html");
...
{
    SimRequest request("https://example.com/test.html#:~:text=This-,is,test,-suffix", "text/html");
...
{
    SimRequest request("https://example.com/test.html#:~:text=Header%202-,A,text,-Footer%201", "text/html");
    LoadURL("https://example.com/"
    ...
    EXPECT_EQ(*GetDocument().getElementById(AtomicString("expected")), *GetDocument().CssTarget());
    EXPECT_EQ(1u, GetDocument().Markers().Markers().size());

    // Expect marker on the expected "A string of text".
    auto* text = To<Text>(GetDocument().getElementById(AtomicString("expected"))->firstChild());
    DocumentMarkerVector markers = GetDocument().Markers().MarkersFor(*text, DocumentMarker::MarkerTypes::TextFragment());
    ASSERT_EQ(1u, markers.size());
    EXPECT_EQ(0u, markers.at(0)->StartOffset());
...
{
    SimRequest request("https://example.com/test.html#:~:text=Prefix-,Cats,-Suffix", "text/html");
...
TEST_F(TextFragmentAnchorTest, OneContextTerm)
{
    ...
    EXPECT_EQ(*GetDocument().getElementById(AtomicString("text1")), *GetDocument().CssTarget());
...
class TextFragmentAnchorScrollTest : public TextFragmentAnchorTest, public testing::WithParamInterface<mojom::blink::ScrollType> {
protected:
    bool IsUserScrollType()
    {
        return GetParam() == mojom::blink::ScrollType::kCompositor || GetParam() == mojom::blink::ScrollType::kUser;
    }
};

INSTANTIATE_TEST_SUITE_P(ScrollTypes, TextFragmentAnchorScrollTest,
    testing::Values(mojom::blink::ScrollType::kUser, mojom::blink::ScrollType::kProgrammatic, mojom::blink::ScrollType::kClamping,
        mojom::blink::ScrollType::kCompositor, mojom::blink::ScrollType::kAnchoring, mojom::blink::ScrollType::kSequenced));
...
TEST_P(TextFragmentAnchorScrollTest, ScrollCancelled)
{
    ...
    SimSubresourceRequest img_request("https://example.com/test.png", "image/png");
    LoadURL("https://example.com/test.html#:~:text=test");
    request.Complete(R"HTML(
    <!DOCTYPE html>
    <style>
    ...
    GetDocument().View()->LayoutViewport()->ScrollBy(ScrollOffset(0, 100), scroll_type);
...
TEST_F(TextFragmentAnchorTest, DisabledInIframes)
{
    ...
    SimRequest child_request("https://example.com/child.html#:~:text=test", "text/html");
    ...
    EXPECT_EQ(nullptr, GetDocument().CssTarget());
    EXPECT_EQ(ScrollOffset(), child_frame->View()->GetScrollableArea()->GetScrollOffset());
}
...
TEST_F(TextFragmentAnchorTest, DisabledInWindowOpen)
{
    ...
    LocalDOMWindow* child_window
        = To<LocalDOMWindow>(main_window->open(script_state->GetIsolate(), destination, AtomicString("frame1"), "", ASSERT_NO_EXCEPTION));
...
TEST_F(TextFragmentAnchorTest, DisabledInSamePageNavigation)
{
    ...
    ASSERT_EQ(ScrollOffset(), GetDocument().View()->GetScrollableArea()->GetScrollOffset());

    ScriptState* script_state = ToScriptStateForMainWorld(GetDocument().GetFrame());
    ScriptState::Scope entered_context_scope(script_state);
    GetDocument().GetFrame()->DomWindow()->location()->setHash(script_state->GetIsolate(), ":~:text=test", ASSERT_NO_EXCEPTION);
    RunAsyncMatchingTasks();
...
{
    SimRequest main_request("https://example.com/test.html#:~:text=test", "text/html");
...
{
    SimRequest request("https://example.com/test.html#:~:text=This,test&text=is,page", "text/html");
...
{
    SimRequest request("https://example.com/test.html#:~:text=test%20page", "text/html");
...
{
    SimRequest request("https://example.com/test.html#element:~:text=cats", "text/html");
...
{
    SimRequest request("https://example.com/test.html#:~:text=est-,page", "text/html");
...
{
    SimRequest request("https://example.com/test.html#:~:text=tes,age", "text/html");
    ...
    EXPECT_EQ(p, *GetDocument().CssTarget());
    EXPECT_TRUE(ViewportRect().Contains(BoundingRectInFrame(p)))
        << "Should have scrolled <p> into view but didn't, scroll offset: " << LayoutViewport()->GetScrollOffset().ToString();

    // Expect marker on only "tes age"
    EXPECT_EQ(1u, GetDocument().Markers().Markers().size());
    DocumentMarkerVector markers = GetDocument().Markers().MarkersFor(*To<Text>(p.firstChild()), DocumentMarker::MarkerTypes::TextFragment());
    ASSERT_EQ(1u, markers.size());
    EXPECT_EQ(10u, markers.at(0)->StartOffset());
...
TEST_F(TextFragmentAnchorTest, DismissTextHighlightWithClick)
{
    ...
    KURL url = GetDocument().GetFrame()->Loader().GetDocumentLoader()->GetHistoryItem()->Url();
    EXPECT_EQ("https://example.com/test.html#:~:text=test%20page&text=more%20text", url.GetString());
    EXPECT_EQ(2u, GetDocument().Markers().Markers().size());
    ...
    url = GetDocument().GetFrame()->Loader().GetDocumentLoader()->GetHistoryItem()->Url();
    EXPECT_EQ("https://example.com/test.html#:~:text=test%20page&text=more%20text", url.GetString());
}
...
{
    SimRequest request("https://example.com/test.html#element:~:text=test", "text/html");
...
TEST_F(TextFragmentAnchorTest, IdFragmentWithFragmentDirective)
{
    ...
    EXPECT_EQ(p, *GetDocument().CssTarget());
    EXPECT_TRUE(ViewportRect().Contains(BoundingRectInFrame(p)))
        << "Should have scrolled <div> into view but didn't, scroll offset: " << LayoutViewport()->GetScrollOffset().ToString();
}
...
TEST_F(TextFragmentAnchorTest, TextDirectiveInSvg)
{
    ...
    EXPECT_TRUE(ViewportRect().Contains(BoundingRectInFrame(text)))
        << "<text> Element wasn't scrolled into view, viewport's scroll offset: " << LayoutViewport()->GetScrollOffset().ToString();
...
TEST_F(TextFragmentAnchorTest, DISABLED_HighlightOnReload)
{
    ...
    SimRequest reload_request("https://example.com/test.html#:~:text=test", "text/html");
...
{
    SimRequest request("https://example.com/test.html#:~:text=test&directive&text=more", "text/html");
...
{
    SimRequest request("https://example.com/test.html#:~:text=abcdef", "text/html");
...
{
    SimRequest request("https://example.com/test.html#:~:text=foo-,foo,-bar", "text/html");
...
TEST_F(TextFragmentAnchorTest, IsInSameUninterruptedBlock_NonBlockInterruption)
{
    SimRequest request("https://example.com/test.html", "text/html");
    LoadURL("https://example.com/test.html");
    request.Complete(R"HTML(
    ...
    Node* first_paragraph = GetDocument().getElementById(AtomicString("first"))->firstChild();
    const auto& start = PositionInFlatTree(first_paragraph, 0);
    const auto& end = PositionInFlatTree(first_paragraph->nextSibling()->nextSibling(), 10);
    ASSERT_EQ("First styled text paragraph", PlainText(EphemeralRangeInFlatTree(start, end)));
...
TEST_F(TextFragmentAnchorTest, IsInSameUninterruptedBlock_BlockInterruption)
{
    ...
    const auto& start = PositionInFlatTree(first_paragraph, 0);
    const auto& end = PositionInFlatTree(first_paragraph->nextSibling()->nextSibling(), 10);
    ASSERT_EQ("First\nblock text\nparagraph", PlainText(EphemeralRangeInFlatTree(start, end)));
...
{
    SimRequest request("https://www.test.com/#:~:text=First%20test,page%20three", "text/html");
    ...
    WebView().GetSettings()->SetEditingBehavior(mojom::EditingBehavior::kEditingMacBehavior);
    ...
    gfx::PointF middle_elem_point(((middle_rect->left() + 1)), ((middle_rect->top() + 1)));
    mouse_down_event.SetPositionInWidget(middle_elem_point.x(), middle_elem_point.y());
    mouse_down_event.SetPositionInScreen(middle_elem_point.x(), middle_elem_point.y());
    ...
    gfx::PointF last_elem_point(((last_rect->left() + 1)), ((last_rect->top() + 1)));
    mouse_down_event.SetPositionInWidget(last_elem_point.x(), last_elem_point.y());
    mouse_down_event.SetPositionInScreen(last_elem_point.x(), last_elem_point.y());
    ...
    WebView().MainFrameViewWidget()->HandleInputEvent(WebCoalescedInputEvent(mouse_down_event, ui::LatencyInfo()));
    WebView().MainFrameViewWidget()->HandleInputEvent(WebCoalescedInputEvent(last_mouse_up_event, ui::LatencyInfo()));
...
TEST_F(TextFragmentAnchorTest, ShouldOpenContextMenuOnTap)
{
    ...
    EXPECT_FALSE(GetDocument().GetPage()->GetContextMenuController().ContextMenuNodeForFrame(GetDocument().GetFrame()));

    Range* range = Range::Create(GetDocument());
    range->setStart(GetDocument().getElementById(AtomicString("first")), 0, IGNORE_EXCEPTION_FOR_TESTING);
    range->setEnd(GetDocument().getElementById(AtomicString("first")), 1, IGNORE_EXCEPTION_FOR_TESTING);
    ...
    if (RuntimeEnabledFeatures::TextFragmentTapOpensContextMenuEnabled()) {
        EXPECT_TRUE(GetDocument().GetPage()->GetContextMenuController().ContextMenuNodeForFrame(GetDocument().GetFrame()));
    } else {
        EXPECT_FALSE(GetDocument().GetPage()->GetContextMenuController().ContextMenuNodeForFrame(GetDocument().GetFrame()));
    }
    ...
    range->setStart(GetDocument().getElementById(AtomicString("two")), 0, IGNORE_EXCEPTION_FOR_TESTING);
    range->setEndAfter(GetDocument().getElementById(AtomicString("two")), IGNORE_EXCEPTION_FOR_TESTING);
...
TEST_F(TextFragmentAnchorTest, ShouldNotRequestUnhandledTapNotifierWhenTapOnTextFragment)
{
    LoadAhem();
    SimRequest request("https://example.com/"
                       "test.html#:~:text=this%20is%20a%20test%20page",
    ...
    GetDocument().GetFrame()->GetBrowserInterfaceBroker().SetBinderForTesting(
        mojom::blink::UnhandledTapNotifier::Name_, WTF::BindRepeating(&MockUnhandledTapNotifierImpl::Bind, WTF::Unretained(&mock_notifier)));
...
{
    SimRequest request("https://example.com/test.html#:~:text=test", "text/html");
    SimSubresourceRequest sub_request("https://example.com/null.png", "image/png");
    LoadURL("https://example.com/test.html#:~:text=test");
    request.Complete(R"HTML(
    <!DOCTYPE html>
    <style>
    ...
    auto annotations = container->GetAgentsOfType(mojom::blink::AnnotationType::kSharedHighlight);
...
TEST_F(TextFragmentAnchorPostLoadTest, PostLoadSearchTimesOut)
{
    ...
    // perform the search.
    test::RunDelayedTasks(TextFragmentAnchor::PostLoadTaskTimeout() - TextFragmentAnchor::PostLoadTaskDelay());
    Compositor().BeginFrame();
