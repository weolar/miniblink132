...
public:
    static SnapEvent* Create(const AtomicString& type, Bubbles bubbles, Member<Node>& block_target, Member<Node>& inline_target);
    SnapEvent(const AtomicString& type, Bubbles bubbles, Member<Node>& block_target, Member<Node>& inline_target);

    Node* snapTargetBlock()
    {
        return snap_target_block_.Get();
    }
    Node* snapTargetInline()
    {
        return snap_target_inline_.Get();
    }
