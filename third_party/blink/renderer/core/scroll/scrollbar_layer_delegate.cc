...
public:
    explicit ScopedScrollbarPainter(cc::PaintCanvas& canvas)
        : canvas_(canvas)
    {
    }
...
{
    return scrollbar_.Get() == static_cast<const ScrollbarLayerDelegate&>(other).scrollbar_.Get();
}
...
gfx::Rect ScrollbarLayerDelegate::ShrinkMainThreadedMinimalModeThumbRect(gfx::Rect& rect) const
{
    return scrollbar_->GetTheme().ShrinkMainThreadedMinimalModeThumbRect(*scrollbar_, rect);
}
...
{
    gfx::Rect back_button_rect = scrollbar_->GetTheme().BackButtonRect(*scrollbar_);
...
{
    gfx::Rect forward_button_rect = scrollbar_->GetTheme().ForwardButtonRect(*scrollbar_);
...
void ScrollbarLayerDelegate::PaintThumb(cc::PaintCanvas& canvas, const gfx::Rect& rect)
{
    if (!ShouldPaint()) {
        return;
    }
...
void ScrollbarLayerDelegate::PaintTrackAndButtons(cc::PaintCanvas& canvas, const gfx::Rect& rect)
{
    if (!ShouldPaint()) {
        return;
    }
