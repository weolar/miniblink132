...
struct SequencedScroll final : public GarbageCollected<SequencedScroll> {
    ...
    SequencedScroll(ScrollableArea* area, ScrollOffset offset, mojom::blink::ScrollBehavior behavior)
...
// is called.
class CORE_EXPORT SmoothScrollSequencer final : public GarbageCollected<SmoothScrollSequencer> {
public:
    explicit SmoothScrollSequencer(LocalFrame& owner_frame);
    void SetScrollType(mojom::blink::ScrollType type)
    {
        scroll_type_ = type;
    }

    // Add a scroll offset animation to the back of a queue.
    void QueueAnimation(ScrollableArea*, ScrollOffset, mojom::blink::ScrollBehavior);
