...
TEST_F(ScrollbarThemeOverlayTest, PaintInvalidation)
{
    ...
    NiceMock<MockScrollableArea>* mock_scrollable_area = MakeGarbageCollected<NiceMock<MockScrollableArea>>(ScrollOffset(100, 100));
    ScrollbarThemeOverlay theme(14, 0, 14, 0);

    Scrollbar* vertical_scrollbar = Scrollbar::CreateForTesting(mock_scrollable_area, kVerticalScrollbar, &theme);
    Scrollbar* horizontal_scrollbar = Scrollbar::CreateForTesting(mock_scrollable_area, kHorizontalScrollbar, &theme);
    ON_CALL(*mock_scrollable_area, VerticalScrollbar()).WillByDefault(Return(vertical_scrollbar));
    ON_CALL(*mock_scrollable_area, HorizontalScrollbar()).WillByDefault(Return(horizontal_scrollbar));
    ...
    ASSERT_FALSE(mock_scrollable_area->HorizontalScrollbarNeedsPaintInvalidation());
    ...
    mock_scrollable_area->UpdateScrollOffset(ScrollOffset(0, 5), mojom::blink::ScrollType::kUser);
    ...
    EXPECT_FALSE(mock_scrollable_area->HorizontalScrollbarNeedsPaintInvalidation());

    // Try the horizontal scrollbar.
    mock_scrollable_area->ClearNeedsPaintInvalidationForScrollControls();
    mock_scrollable_area->UpdateScrollOffset(ScrollOffset(5, 5), mojom::blink::ScrollType::kUser);
    horizontal_scrollbar->OffsetDidChange(mojom::blink::ScrollType::kUser);
    vertical_scrollbar->OffsetDidChange(mojom::blink::ScrollType::kUser);
    EXPECT_FALSE(vertical_scrollbar->ThumbNeedsRepaint());
    EXPECT_FALSE(vertical_scrollbar->TrackAndButtonsNeedRepaint());
    ...
    EXPECT_FALSE(horizontal_scrollbar->ThumbNeedsRepaint());
    EXPECT_FALSE(horizontal_scrollbar->TrackAndButtonsNeedRepaint());
    EXPECT_TRUE(mock_scrollable_area->HorizontalScrollbarNeedsPaintInvalidation());
    ...
    vertical_scrollbar->SetPressedPart(kThumbPart, WebInputEvent::Type::kMouseDown);
    EXPECT_TRUE(vertical_scrollbar->ThumbNeedsRepaint());
    EXPECT_TRUE(mock_scrollable_area->VerticalScrollbarNeedsPaintInvalidation());
    ...
    EXPECT_CALL(*mock_scrollable_area, ScrollbarsHiddenIfOverlay()).WillOnce(Return(true));
    ...
    EXPECT_CALL(*mock_scrollable_area, ScrollbarsHiddenIfOverlay()).WillOnce(Return(false));
