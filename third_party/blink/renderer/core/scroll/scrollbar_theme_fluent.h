...
public:
    ...
    gfx::Rect ShrinkMainThreadedMinimalModeThumbRect(const Scrollbar&, const gfx::Rect& rect) const override;
...
protected:
    ...
    void PaintButton(GraphicsContext& context, const Scrollbar& scrollbar, const gfx::Rect& rect, ScrollbarPart part) override;
    WebThemeEngine::ScrollbarThumbExtraParams BuildScrollbarThumbExtraParams(const Scrollbar&) const override;
...
private:
    ...
    int ThumbThickness(float scale_from_dip, EScrollbarWidth scrollbar_width) const;
    // Overlay scrollbar tracks have a invisible length-wise inset to give them a
    // floating appearance.
    gfx::Rect InsetButtonRect(const Scrollbar& scrollbar, gfx::Rect rect, ScrollbarPart part) const;
