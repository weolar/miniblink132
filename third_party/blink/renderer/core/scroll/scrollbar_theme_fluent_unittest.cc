...
public:
    int ButtonLength(const Scrollbar& scrollbar) const
    {
        ...
        return scrollbar.Orientation() == kVerticalScrollbar ? size.height() : size.width();
    }

    // Margin between the thumb and the edge of the scrollbars.
    int ThumbOffset(float scale_from_dip) const
    {
        int scrollbar_thumb_offset = (scrollbar_track_thickness() - scrollbar_thumb_thickness()) / 2;
        return base::ClampRound(scrollbar_thumb_offset * scale_from_dip);
    }

    int scrollbar_thumb_thickness() const
    {
        return scrollbar_thumb_thickness_;
    }
    int scrollbar_track_thickness() const
    {
        return scrollbar_track_thickness_;
    }
    int scrollbar_track_inset() const
    {
        return scrollbar_track_inset_;
    }
...
protected:
    ...
    {
        return theme_->ThumbOffset(ScaleFromDIP());
    }
...
TEST_P(ScrollbarThemeFluentTest, ScrollbarThicknessScalesProperly)
{
    ...
    EXPECT_EQ(theme_->scrollbar_track_thickness(), base::ClampRound(scrollbar_thickness / ScaleFromDIP()));
}
...
{
    Scrollbar* vertical_scrollbar = Scrollbar::CreateForTesting(mock_scrollable_area(), kVerticalScrollbar, &(theme_->GetInstance()));
    int scrollbar_thickness = ScrollbarThickness();
    vertical_scrollbar->SetFrameRect(gfx::Rect(kOffsetFromViewport, 0, scrollbar_thickness, kScrollbarLength));
    ...
    EXPECT_EQ(track_rect, gfx::Rect(kOffsetFromViewport, theme_->ButtonLength(*vertical_scrollbar), scrollbar_thickness, TrackLength(*vertical_scrollbar)));

    const gfx::Rect thumb_rect = theme_->ThumbRect(*vertical_scrollbar);
    EXPECT_EQ(thumb_rect,
        gfx::Rect(kOffsetFromViewport + ThumbOffset(), theme_->ButtonLength(*vertical_scrollbar), ThumbThickness(), theme_->ThumbLength(*vertical_scrollbar)));

    const gfx::Size button_size = theme_->ButtonSize(*vertical_scrollbar);
    EXPECT_EQ(button_size, gfx::Size(scrollbar_thickness, theme_->ButtonLength(*vertical_scrollbar)));
}

// Test that Scrollbar objects are correctly sized with Fluent theme parts.
TEST_P(ScrollbarThemeFluentTest, HorizontalScrollbarPartsSizes)
{
    Scrollbar* horizontal_scrollbar = Scrollbar::CreateForTesting(mock_scrollable_area(), kHorizontalScrollbar, &(theme_->GetInstance()));
    int scrollbar_thickness = ScrollbarThickness();
    horizontal_scrollbar->SetFrameRect(gfx::Rect(0, kOffsetFromViewport, kScrollbarLength, scrollbar_thickness));

    // Check that ThumbOffset() calculation is correct.
    EXPECT_EQ(ThumbThickness() + 2 * ThumbOffset(), scrollbar_thickness);

    const gfx::Rect track_rect = theme_->TrackRect(*horizontal_scrollbar);
    EXPECT_EQ(track_rect, gfx::Rect(theme_->ButtonLength(*horizontal_scrollbar), kOffsetFromViewport, TrackLength(*horizontal_scrollbar), scrollbar_thickness));

    const gfx::Rect thumb_rect = theme_->ThumbRect(*horizontal_scrollbar);
    EXPECT_EQ(thumb_rect,
        gfx::Rect(
            theme_->ButtonLength(*horizontal_scrollbar), kOffsetFromViewport + ThumbOffset(), theme_->ThumbLength(*horizontal_scrollbar), ThumbThickness()));

    const gfx::Size button_size = theme_->ButtonSize(*horizontal_scrollbar);
    EXPECT_EQ(button_size, gfx::Size(theme_->ButtonLength(*horizontal_scrollbar), scrollbar_thickness));
}
...
{
    Scrollbar* scrollbar = Scrollbar::CreateForTesting(mock_scrollable_area(), kVerticalScrollbar, &(theme_->GetInstance()));
    ON_CALL(*mock_scrollable_area(), VerticalScrollbar()).WillByDefault(Return(scrollbar));

    scrollbar->SetFrameRect(gfx::Rect(0, 0, ScrollbarThickness(), kScrollbarLength));
    ...
    mock_scrollable_area()->SetScrollOffset(ScrollOffset(0, 10), mojom::blink::ScrollType::kCompositor);
    ...
    mock_scrollable_area()->SetScrollOffset(ScrollOffset(0, 20), mojom::blink::ScrollType::kCompositor);
    EXPECT_FALSE(scrollbar->TrackAndButtonsNeedRepaint());

    // Verifies that when the thumb position changes back to 0 (min) offset,
    // the background invalidation is not triggered.
    mock_scrollable_area()->SetScrollOffset(ScrollOffset(0, 0), mojom::blink::ScrollType::kCompositor);
    EXPECT_FALSE(scrollbar->TrackAndButtonsNeedRepaint());
}
...
TEST_P(OverlayScrollbarThemeFluentTest, TestVerticalInsetTrackRect)
{
    ...
    Scrollbar* vertical_scrollbar = Scrollbar::CreateForTesting(mock_scrollable_area(), kVerticalScrollbar, &(theme_->GetInstance()));
    vertical_scrollbar->SetFrameRect(gfx::Rect(kOffsetFromViewport, 0, scrollbar_thickness, kScrollbarLength));
    gfx::Rect track_rect(kOffsetFromViewport, 0, scrollbar_thickness, kScrollbarLength);

    // Vertical scrollbars should be inset from the left and right.
    gfx::Rect expected_rect(kOffsetFromViewport + ScrollbarTrackInsetPx(), 0, scrollbar_thickness - 2 * ScrollbarTrackInsetPx(), kScrollbarLength);
    EXPECT_EQ(expected_rect, theme_->InsetTrackRect(*vertical_scrollbar, track_rect));
}
...
TEST_P(OverlayScrollbarThemeFluentTest, TestHorizontalInsetTrackRect)
{
    ...
    Scrollbar* horizontal_scrollbar = Scrollbar::CreateForTesting(mock_scrollable_area(), kHorizontalScrollbar, &(theme_->GetInstance()));
    horizontal_scrollbar->SetFrameRect(gfx::Rect(0, kOffsetFromViewport, kScrollbarLength, scrollbar_thickness));
    gfx::Rect track_rect(0, kOffsetFromViewport, kScrollbarLength, scrollbar_thickness);

    // Horizontal scrollbars should be inset from the top and the bottom.
    gfx::Rect expected_rect(0, kOffsetFromViewport + ScrollbarTrackInsetPx(), kScrollbarLength, scrollbar_thickness - 2 * ScrollbarTrackInsetPx());
    EXPECT_EQ(expected_rect, theme_->InsetTrackRect(*horizontal_scrollbar, track_rect));
}
...
TEST_P(OverlayScrollbarThemeFluentTest, TestVerticalInsetButtonRect)
{
    ...
    gfx::Rect expected_up_rect(inset, inset, scrollbar_thickness - inset * 2, button_length - inset);
    EXPECT_EQ(expected_up_rect, theme_->InsetButtonRect(*vertical_scrollbar, button_rect, kBackButtonStartPart));
    // Down arrow button should be inset from every part except the top.
    gfx::Rect expected_down_rect(inset, 0, scrollbar_thickness - inset * 2, button_length - inset);
    EXPECT_EQ(expected_down_rect, theme_->InsetButtonRect(*vertical_scrollbar, button_rect, kForwardButtonStartPart));
}
...
TEST_P(OverlayScrollbarThemeFluentTest, TestHorizontalInsetButtonRect)
{
    ...
    gfx::Rect expected_left_rect(inset, inset, button_length - inset, scrollbar_thickness - inset * 2);
    EXPECT_EQ(expected_left_rect, theme_->InsetButtonRect(*horizontal_scrollbar, button_rect, kBackButtonStartPart));
    // Right arrow button should be inset from every part except the left.
    gfx::Rect expected_right_rect(0, inset, button_length - inset, scrollbar_thickness - inset * 2);
    EXPECT_EQ(expected_right_rect, theme_->InsetButtonRect(*horizontal_scrollbar, button_rect, kForwardButtonStartPart));
}

INSTANTIATE_TEST_SUITE_P(All, ScrollbarThemeFluentTest, ::testing::Values(1.f, 1.25f, 1.5f, 1.75f, 2.f));
INSTANTIATE_TEST_SUITE_P(All, OverlayScrollbarThemeFluentTest, ::testing::Values(1.f, 1.25f, 1.5f, 1.75f, 2.f));
