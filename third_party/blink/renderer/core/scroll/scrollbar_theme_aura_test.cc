...
public:
    ScrollbarThemeAuraButtonOverride()
        : has_scrollbar_buttons_(true)
    {
    }

    void SetHasScrollbarButtons(bool value)
    {
        has_scrollbar_buttons_ = value;
    }
    ...
    {
        return ScrollbarThickness(scrollbar.ScaleFromDIP(), scrollbar.CSSScrollbarWidth());
    }

    void PaintTrackBackground(GraphicsContext&, const Scrollbar&, const gfx::Rect& rect) override
    {
        last_painted_track_rect = rect;
    }
    void PaintButton(GraphicsContext&, const Scrollbar&, const gfx::Rect& rect, ScrollbarPart part) override
    {
...
protected:
    ...
    {
        MockScrollableArea* scrollable_area = MockScrollableArea::Create(ScrollOffset(0, 1000));
        scrollable_area->SetScaleFromDIP(GetParam());
        return scrollable_area;
    }

    void TestSetFrameRect(Scrollbar& scrollbar, const gfx::Rect& rect, bool thumb_expectation, bool track_and_buttons_expectation)
    {
        scrollbar.SetFrameRect(rect);
        EXPECT_EQ(scrollbar.TrackAndButtonsNeedRepaint(), track_and_buttons_expectation);
    ...
    void TestSetProportion(Scrollbar& scrollbar, int proportion, bool thumb_expectation, bool track_and_buttons_expectation)
    {
...
void SendEvent(Scrollbar* scrollbar, blink::WebInputEvent::Type type, gfx::PointF point)
{
    const blink::WebMouseEvent web_mouse_event(type, point, point, blink::WebPointerProperties::Button::kLeft, 0,
        blink::WebInputEvent::kScrollbarManipulationHandledOnCompositorThread, base::TimeTicks::Now());
    switch (type) {
...
TEST_P(ScrollbarThemeAuraTest, ButtonSizeHorizontal)
{
    ...
    Scrollbar* scrollbar = Scrollbar::CreateForTesting(mock_scrollable_area, kHorizontalScrollbar, &theme);
...
TEST_P(ScrollbarThemeAuraTest, ButtonSizeVertical)
{
    ...
    MockScrollableArea* mock_scrollable_area = CreateMockScrollableArea();
    ScrollbarThemeAuraButtonOverride theme;
    Scrollbar* scrollbar = Scrollbar::CreateForTesting(mock_scrollable_area, kVerticalScrollbar, &theme);
...
TEST_P(ScrollbarThemeAuraTest, ScrollbarPartsInvalidationTest)
{
    ...
    SendEvent(scrollbar, blink::WebInputEvent::Type::kMouseMove, gfx::PointF(10, 20));
    SendEvent(scrollbar, blink::WebInputEvent::Type::kMouseDown, gfx::PointF(10, 20));
    EXPECT_TRUE(scrollbar->ThumbNeedsRepaint());

    // Tests that mouseup on the thumb causes an invalidation.
    scrollbar->ClearThumbNeedsRepaint();
    SendEvent(scrollbar, blink::WebInputEvent::Type::kMouseUp, gfx::PointF(10, 20));
    EXPECT_TRUE(scrollbar->ThumbNeedsRepaint());
    ...
    scrollbar->ClearTrackAndButtonsNeedRepaint();
    SendEvent(scrollbar, blink::WebInputEvent::Type::kMouseMove, gfx::PointF(10, 760));
    SendEvent(scrollbar, blink::WebInputEvent::Type::kMouseDown, gfx::PointF(10, 760));
    EXPECT_TRUE(scrollbar->TrackAndButtonsNeedRepaint());

    // Tests that mouseup on the arrow causes an invalidation.
    scrollbar->ClearTrackAndButtonsNeedRepaint();
    SendEvent(scrollbar, blink::WebInputEvent::Type::kMouseUp, gfx::PointF(10, 760));
    EXPECT_TRUE(scrollbar->TrackAndButtonsNeedRepaint());
...
TEST_P(ScrollbarThemeAuraTest, NinePatchLargerThanMinimalSize)
{
    ...
    EXPECT_EQ(gfx::Rect(0, width + 1, width, width), theme.last_painted_forward_button_rect);
}
...
TEST_P(ScrollbarThemeAuraTest, NinePatchSmallerThanMinimalSize)
{
    ...
    if (int track_height = height - button_size.height() * 2) {
        ...
        EXPECT_EQ(gfx::Rect(0, button_size.height(), width, track_height), theme.last_painted_track_rect);
    }
    EXPECT_EQ(gfx::Rect(0, 0, width, button_size.height()), theme.last_painted_back_button_rect);
    EXPECT_EQ(gfx::Rect(0, height - button_size.height(), width, button_size.height()), theme.last_painted_forward_button_rect);
}
...
TEST_P(ScrollbarThemeAuraTest, NinePatchTrackWithoutButtons)
{
    ...
    EXPECT_EQ(gfx::Size(1, 1), theme.NinePatchTrackAndButtonsCanvasSize(*scrollbar));
    EXPECT_EQ(gfx::Rect(1, 1), theme.NinePatchTrackAndButtonsAperture(*scrollbar));
...
TEST_P(ScrollbarThemeAuraTest, TestPaintInvalidationsWhenNinePatchScaled)
{
    ...
    ScrollbarThemeAuraButtonOverride theme;
    ASSERT_TRUE(theme.UsesNinePatchTrackAndButtonsResource());
    Scrollbar* scrollbar = Scrollbar::CreateForTesting(CreateMockScrollableArea(), kVerticalScrollbar, &theme);
    // Start the test with a scrollbar larger than the canvas size and clean
    // flags.
    scrollbar->SetFrameRect(gfx::Rect(0, 0, scrollbar->Width(), scrollbar->Width() * 5));
    scrollbar->ClearTrackAndButtonsNeedRepaint();
    scrollbar->ClearThumbNeedsRepaint();

    // Test that resizing the scrollbar's length while larger than the canvas
    // doesn't trigger a repaint.
    TestSetFrameRect(*scrollbar, gfx::Rect(0, 0, scrollbar->Width(), scrollbar->Width() * 4),
    ...
    TestSetFrameRect(*scrollbar, gfx::Rect(0, 0, scrollbar->Width() / 2, scrollbar->Height()),
    ...
    // repaint.
    TestSetFrameRect(*scrollbar, gfx::Rect(0, 0, scrollbar->Width(), scrollbar->Width() / 2),
        /*thumb_expectation=*/true, /*track_and_buttons_expectation=*/true);
...
INSTANTIATE_TEST_SUITE_P(All, ScrollbarThemeAuraTest, ::testing::Values(1.f, 1.25f, 1.5f, 1.75f, 2.f));
