...
ProgrammaticScrollAnimator::ProgrammaticScrollAnimator(ScrollableArea* scrollable_area)
...
{
    return is_sequenced_scroll_ ? mojom::blink::ScrollType::kSequenced : mojom::blink::ScrollType::kProgrammatic;
}

void ProgrammaticScrollAnimator::ScrollToOffsetWithoutAnimation(const ScrollOffset& offset, bool is_sequenced_scroll)
{
    CancelAnimation();
    is_sequenced_scroll_ = is_sequenced_scroll;
    ScrollOffsetChanged(offset, GetScrollType());
    is_sequenced_scroll_ = false;
    if (SmoothScrollSequencer* sequencer = GetScrollableArea()->GetSmoothScrollSequencer()) {
        sequencer->RunQueuedAnimations();
    }
}

void ProgrammaticScrollAnimator::AnimateToOffset(const ScrollOffset& offset, bool is_sequenced_scroll, ScrollableArea::ScrollCallback on_finish)
{
    ...
    animation_curve_ = cc::ScrollOffsetAnimationCurveFactory::CreateAnimation(
        CompositorOffsetFromBlinkOffset(target_offset_), cc::ScrollOffsetAnimationCurveFactory::ScrollType::kProgrammatic);
...
void ProgrammaticScrollAnimator::TickAnimation(base::TimeTicks monotonic_time)
{
    ...
    ScrollOffset offset = BlinkOffsetFromCompositorOffset(animation_curve_->GetValue(elapsed_time));
    ScrollOffsetChanged(offset, GetScrollType());
...
void ProgrammaticScrollAnimator::UpdateCompositorAnimations()
{
    ...
    if (compositor_animation_id() && run_state_ != RunState::kRunningOnCompositor) {
        // If the current run state is WaitingToSendToCompositor but we have a
        // non-zero compositor animation id, there's a currently running
        // compositor animation that needs to be removed here before the new
        // animation is added below.
        DCHECK(run_state_ == RunState::kWaitingToCancelOnCompositor || run_state_ == RunState::kWaitingToSendToCompositor);
    ...
    if (run_state_ == RunState::kWaitingToSendToCompositor) {
        ...
        if (!scrollable_area_->ShouldScrollOnMainThread()) {
            ...
            if (!is_sequenced_scroll_ || RuntimeEnabledFeatures::MultiSmoothScrollIntoViewEnabled()) {
        ...
        if (!sent_to_compositor) {
            ...
            animation_curve_->SetInitialValue(CompositorOffsetFromBlinkOffset(scrollable_area_->GetScrollOffset()));
            if (!scrollable_area_->ScheduleAnimation()) {
    ...
    if (run_state_ == RunState::kRunningOnCompositor && scrollable_area_->ShouldScrollOnMainThread()) {
        RemoveAnimation();
        run_state_ = RunState::kRunningOnMainThread;
        animation_curve_->SetInitialValue(CompositorOffsetFromBlinkOffset(scrollable_area_->GetScrollOffset()));
        scrollable_area_->RegisterForAnimation();
...
void ProgrammaticScrollAnimator::NotifyCompositorAnimationFinished(int group_id)
{
    DCHECK_NE(run_state_, RunState::kRunningOnCompositorButNeedsUpdate);
