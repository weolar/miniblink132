...
// parent frame.
bool AllowedToPropagateToParent(const LocalFrame& from_frame, const mojom::blink::ScrollIntoViewParamsPtr& params)
{
    ...
    if (!params->cross_origin_boundaries) {
        ...
        if (parent_frame && !parent_frame->GetSecurityContext()->GetSecurityOrigin()->CanAccess(from_frame.GetSecurityContext()->GetSecurityOrigin())) {
            return false;
        }
    }
...
ALWAYS_INLINE ScrollableArea* GetScrollableAreaForLayoutBox(const LayoutBox& box, const mojom::blink::ScrollIntoViewParamsPtr& params)
{
    if (box.IsScrollContainer() && !box.IsLayoutView()) {
        return box.GetScrollableArea();
    } else if (!box.ContainingBlock()) {
        return params->make_visible_in_visual_viewport ? box.GetFrameView()->GetScrollableArea() : box.GetFrameView()->LayoutViewport();
    }
    return nullptr;
}
...
// the root.
std::optional<LayoutBox*> GetScrollParent(const LayoutBox& box, const mojom::blink::ScrollIntoViewParamsPtr& params)
{
    bool is_fixed_to_frame = box.StyleRef().GetPosition() == EPosition::kFixed && box.Container() == box.View();
    ...
    DCHECK(box.GetFrame()->IsMainFrame() || box.GetFrame()->Parent()->IsRemoteFrame());
    return nullptr;
}
...
ALWAYS_INLINE void AdjustRectAndParamsForParentFrame(
    const LayoutBox& current_box, const LayoutBox* next_box, PhysicalRect& absolute_rect_to_scroll, mojom::blink::ScrollIntoViewParamsPtr& params)
{
    ...
    if (next_box && next_box->View() != current_box.View()) {
        scroll_into_view_util::ConvertParamsToParentFrame(params, gfx::RectF(absolute_rect_to_scroll), *current_box.View(), *next_box->View());

        absolute_rect_to_scroll = current_box.View()->LocalToAncestorRect(absolute_rect_to_scroll, next_box->View(), kTraverseDocumentBoundaries);
    }
}
...
std::optional<PhysicalRect> PerformBubblingScrollIntoView(const LayoutBox& box, const PhysicalRect& absolute_rect,
    mojom::blink::ScrollIntoViewParamsPtr& params, const PhysicalBoxStrut& scroll_margin, bool from_remote_frame)
{
    ...
    bool will_sequence_scrolls = !RuntimeEnabledFeatures::MultiSmoothScrollIntoViewEnabled() && params->is_for_scroll_sequence;

    // TODO(bokan): Temporary, to track cross-origin scroll-into-view prevalence.
    // https://crbug.com/1339003.
    const SecurityOrigin* starting_frame_origin = box.GetFrame()->GetSecurityContext()->GetSecurityOrigin();
    ...
    while (current_box) {
        ...
        if (!current_box->GetFrame()->IsInFencedFrameTree() && params->for_focused_editable && current_box->IsGlobalRootScroller()) {
            break;
        }

        ScrollableArea* area_to_scroll = GetScrollableAreaForLayoutBox(*current_box, params);
        if (area_to_scroll) {
            ScrollOffset scroll_before = area_to_scroll->GetScrollOffset();
            CHECK(!will_sequence_scrolls || area_to_scroll->GetSmoothScrollSequencer());
            wtf_size_t num_scroll_sequences = will_sequence_scrolls ? area_to_scroll->GetSmoothScrollSequencer()->GetCount() : 0ul;

            absolute_rect_to_scroll = area_to_scroll->ScrollIntoView(absolute_rect_to_scroll, active_scroll_margin, params);
            ...
            bool scroll_changed = will_sequence_scrolls ? area_to_scroll->GetSmoothScrollSequencer()->GetCount() != num_scroll_sequences
                                                        : area_to_scroll->GetScrollOffset() != scroll_before;
            if (scroll_changed && !params->for_focused_editable && params->type == mojom::blink::ScrollType::kProgrammatic) {
                const SecurityOrigin* current_frame_origin = current_box->GetFrame()->GetSecurityContext()->GetSecurityOrigin();
                if (!current_frame_origin->CanAccess(starting_frame_origin) || from_remote_frame) {
                    ...
                    UseCounter::Count(current_box->GetFrame()->LocalFrameRoot().GetDocument(), WebFeature::kCrossOriginScrollIntoView);
                }
            }
        }

        bool is_fixed_to_frame = current_box->StyleRef().GetPosition() == EPosition::kFixed && current_box->Container() == current_box->View();

        VisualViewport& visual_viewport = current_box->GetFrame()->GetPage()->GetVisualViewport();
        if (is_fixed_to_frame && params->make_visible_in_visual_viewport) {
            ...
            if (current_box->GetFrame()->IsMainFrame() && visual_viewport.IsActiveViewport()) {
                absolute_rect_to_scroll
                    = current_box->GetFrame()->GetPage()->GetVisualViewport().ScrollIntoView(absolute_rect_to_scroll, active_scroll_margin, params);
                scrolled_to_area = true;
            }
        ...
        std::optional<LayoutBox*> next_box_opt = GetScrollParent(*current_box, params);
        if (!next_box_opt) {
            return std::nullopt;
        }
        ...
        AdjustRectAndParamsForParentFrame(*current_box, next_box, absolute_rect_to_scroll, params);
...
void ScrollRectToVisible(
    const LayoutObject& layout_object, const PhysicalRect& absolute_rect, mojom::blink::ScrollIntoViewParamsPtr params, bool from_remote_frame)
{
    ...
    params->is_for_scroll_sequence |= params->type == mojom::blink::ScrollType::kProgrammatic;
    bool will_sequence_scrolls = !RuntimeEnabledFeatures::MultiSmoothScrollIntoViewEnabled() && params->is_for_scroll_sequence;
    ...
    PhysicalBoxStrut scroll_margin = layout_object.Style() ? layout_object.Style()->ScrollMarginStrut() : PhysicalBoxStrut();
    PhysicalRect absolute_rect_to_scroll = absolute_rect;
    absolute_rect_to_scroll.Expand(scroll_margin);
    std::optional<PhysicalRect> updated_absolute_rect
        = PerformBubblingScrollIntoView(*enclosing_box, absolute_rect_to_scroll, params, scroll_margin, from_remote_frame);
    ...
    if (!local_root.IsOutermostMainFrame()) {
        ...
        if (AllowedToPropagateToParent(local_root, params)) {
            local_root_view->ScrollRectToVisibleInRemoteParent(*updated_absolute_rect, std::move(params));
        }
        ...
        local_root.GetPage()->GetChromeClient().FinishScrollFocusedEditableIntoView(caret_rect_in_root_frame, std::move(params));
    }
}

gfx::RectF FocusedEditableBoundsFromParams(const gfx::RectF& caret_rect, const mojom::blink::ScrollIntoViewParamsPtr& params)
{
    DCHECK(params->for_focused_editable);
    DCHECK(!params->for_focused_editable->size.IsEmpty());

    gfx::PointF editable_location = caret_rect.origin() + params->for_focused_editable->relative_location;
    return gfx::RectF(editable_location, params->for_focused_editable->size);
}

void ConvertParamsToParentFrame(
    mojom::blink::ScrollIntoViewParamsPtr& params, const gfx::RectF& caret_rect_in_src, const LayoutObject& src_frame, const LayoutView& dest_frame)
{
    if (!params->for_focused_editable)
        return;
    ...
    gfx::RectF editable_bounds_in_src = FocusedEditableBoundsFromParams(caret_rect_in_src, params);

    PhysicalRect editable_bounds_in_dest
        = src_frame.LocalToAncestorRect(PhysicalRect::EnclosingRect(editable_bounds_in_src), &dest_frame, kTraverseDocumentBoundaries);

    PhysicalRect caret_rect_in_dest = src_frame.LocalToAncestorRect(PhysicalRect::EnclosingRect(caret_rect_in_src), &dest_frame, kTraverseDocumentBoundaries);

    params->for_focused_editable->relative_location = gfx::Vector2dF(editable_bounds_in_dest.offset - caret_rect_in_dest.offset);
    params->for_focused_editable->size = gfx::SizeF(editable_bounds_in_dest.size);

    DCHECK(!params->for_focused_editable->size.IsEmpty());
}

mojom::blink::ScrollIntoViewParamsPtr CreateScrollIntoViewParams(const mojom::blink::ScrollAlignment& align_x, const mojom::blink::ScrollAlignment& align_y,
    mojom::blink::ScrollType scroll_type, bool make_visible_in_visual_viewport, mojom::blink::ScrollBehavior scroll_behavior, bool is_for_scroll_sequence,
...
mojom::blink::ScrollAlignment ResolveToPhysicalAlignment(
    V8ScrollLogicalPosition::Enum inline_alignment, V8ScrollLogicalPosition::Enum block_alignment, ScrollOrientation axis, const ComputedStyle& computed_style)
{
    bool is_horizontal_writing_mode = computed_style.IsHorizontalWritingMode();
    V8ScrollLogicalPosition::Enum alignment
        = ((axis == kHorizontalScroll && is_horizontal_writing_mode) || (axis == kVerticalScroll && !is_horizontal_writing_mode)) ? inline_alignment
                                                                                                                                  : block_alignment;
    ...
    if (alignment == V8ScrollLogicalPosition::Enum::kStart) {
        PhysicalToLogical<const mojom::blink::ScrollAlignment& (*)()> to_logical(computed_style.GetWritingDirection(), ScrollAlignment::TopAlways,
            ScrollAlignment::RightAlways, ScrollAlignment::BottomAlways, ScrollAlignment::LeftAlways);
        if (axis == kHorizontalScroll) {
            return is_horizontal_writing_mode ? (*to_logical.InlineStart())() : (*to_logical.BlockStart())();
        } else {
            return is_horizontal_writing_mode ? (*to_logical.BlockStart())() : (*to_logical.InlineStart())();
        }
    }
    if (alignment == V8ScrollLogicalPosition::Enum::kEnd) {
        PhysicalToLogical<const mojom::blink::ScrollAlignment& (*)()> to_logical(computed_style.GetWritingDirection(), ScrollAlignment::TopAlways,
            ScrollAlignment::RightAlways, ScrollAlignment::BottomAlways, ScrollAlignment::LeftAlways);
        if (axis == kHorizontalScroll) {
            return is_horizontal_writing_mode ? (*to_logical.InlineEnd())() : (*to_logical.BlockEnd())();
        } else {
            return is_horizontal_writing_mode ? (*to_logical.BlockEnd())() : (*to_logical.InlineEnd())();
        }
    }

    // Default values
    if (is_horizontal_writing_mode) {
        return (axis == kHorizontalScroll) ? ScrollAlignment::ToEdgeIfNeeded() : ScrollAlignment::TopAlways();
    }
    return (axis == kHorizontalScroll) ? ScrollAlignment::LeftAlways() : ScrollAlignment::ToEdgeIfNeeded();
}

V8ScrollLogicalPosition::Enum SnapAlignmentToV8ScrollLogicalPosition(cc::SnapAlignment alignment)
{
    switch (alignment) {
...
mojom::blink::ScrollIntoViewParamsPtr CreateScrollIntoViewParams(const ScrollIntoViewOptions& options, const ComputedStyle& computed_style)
{
    ...
    auto align_x = ResolveToPhysicalAlignment(options.inlinePosition().AsEnum(), options.block().AsEnum(), kHorizontalScroll, computed_style);
    auto align_y = ResolveToPhysicalAlignment(options.inlinePosition().AsEnum(), options.block().AsEnum(), kVerticalScroll, computed_style);

    mojom::blink::ScrollIntoViewParamsPtr params = CreateScrollIntoViewParams(align_x, align_y);
    params->behavior = behavior;
    return params;
}

mojom::blink::ScrollIntoViewParamsPtr CreateScrollIntoViewParams(const ComputedStyle& computed_style)
{
    V8ScrollLogicalPosition::Enum inline_alignment = SnapAlignmentToV8ScrollLogicalPosition(computed_style.GetScrollSnapAlign().alignment_inline);
    V8ScrollLogicalPosition::Enum block_alignment = SnapAlignmentToV8ScrollLogicalPosition(computed_style.GetScrollSnapAlign().alignment_block);
    auto align_x = ResolveToPhysicalAlignment(inline_alignment, block_alignment, kHorizontalScroll, computed_style);
    auto align_y = ResolveToPhysicalAlignment(inline_alignment, block_alignment, kVerticalScroll, computed_style);
...
ScrollOffset GetScrollOffsetToExpose(const ScrollableArea& scroll_area, const PhysicalRect& local_expose_rect, const PhysicalBoxStrut& expose_scroll_margin,
    const mojom::blink::ScrollAlignment& align_x, const mojom::blink::ScrollAlignment& align_y)
{
    ...
    PhysicalRect expose_rect_x(expose_rect_no_margin.X(), non_zero_visible_rect.Y(), expose_rect_no_margin.Width(), non_zero_visible_rect.Height());
    LayoutUnit intersect_width = Intersection(non_zero_visible_rect, expose_rect_x).Width();
    ...
    if (scroll_x == mojom::blink::ScrollAlignment::Behavior::kClosestEdge) {
        ...
        if ((scroll_origin_to_expose_rect.Right() > non_zero_visible_rect.Right() && scroll_origin_to_expose_rect.Width() < non_zero_visible_rect.Width())
            || (scroll_origin_to_expose_rect.Right() < non_zero_visible_rect.Right() && scroll_origin_to_expose_rect.Width() > non_zero_visible_rect.Width())) {
    ...
    PhysicalRect expose_rect_y(non_zero_visible_rect.X(), expose_rect_no_margin.Y(), non_zero_visible_rect.Width(), expose_rect_no_margin.Height());
    LayoutUnit intersect_height = Intersection(non_zero_visible_rect, expose_rect_y).Height();
    ...
    if (scroll_y == mojom::blink::ScrollAlignment::Behavior::kClosestEdge) {
        ...
        if ((scroll_origin_to_expose_rect.Bottom() > non_zero_visible_rect.Bottom() && scroll_origin_to_expose_rect.Height() < non_zero_visible_rect.Height())
            || (scroll_origin_to_expose_rect.Bottom() < non_zero_visible_rect.Bottom()
                && scroll_origin_to_expose_rect.Height() > non_zero_visible_rect.Height())) {
    ...
    non_zero_visible_rect.Move(-PhysicalOffset::FromVector2dFRound(current_scroll_offset));
    ...
    if (scroll_x == mojom::blink::ScrollAlignment::Behavior::kNoScroll) {
        ...
        x = (scroll_origin_to_expose_rect.Right() - non_zero_visible_rect.Right()).ToFloat();
    } else if (scroll_x == mojom::blink::ScrollAlignment::Behavior::kCenter) {
        x = ((scroll_origin_to_expose_rect.X() + scroll_origin_to_expose_rect.Right() - (non_zero_visible_rect.X() + non_zero_visible_rect.Right())) / 2)
                .ToFloat();
    } else {
        x = (scroll_origin_to_expose_rect.X() - non_zero_visible_rect.X()).ToFloat();
    }
    ...
    if (scroll_y == mojom::blink::ScrollAlignment::Behavior::kNoScroll) {
        ...
        y = (scroll_origin_to_expose_rect.Bottom() - non_zero_visible_rect.Bottom()).ToFloat();
    } else if (scroll_y == mojom::blink::ScrollAlignment::Behavior::kCenter) {
        y = ((scroll_origin_to_expose_rect.Y() + scroll_origin_to_expose_rect.Bottom() - (non_zero_visible_rect.Y() + non_zero_visible_rect.Bottom())) / 2)
                .ToFloat();
    } else {
        y = (scroll_origin_to_expose_rect.Y() - non_zero_visible_rect.Y()).ToFloat();
    }

    return ScrollOffset(x, y);
}

mojom::blink::ScrollAlignment PhysicalAlignmentFromSnapAlignStyle(const LayoutBox& box, ScrollOrientation axis)
{
    cc::ScrollSnapAlign snap = box.Style()->GetScrollSnapAlign();
    return ResolveToPhysicalAlignment(
        SnapAlignmentToV8ScrollLogicalPosition(snap.alignment_inline), SnapAlignmentToV8ScrollLogicalPosition(snap.alignment_block), axis, *box.Style());
}
