...
//     }
    ...
    DEFINE_STATIC_LOCAL(ScrollbarThemeOverlayMobile, theme, (ScrollbarStyle().thumb_thickness, ScrollbarStyle().scrollbar_margin));
    return theme;
}

ScrollbarThemeOverlayMobile::ScrollbarThemeOverlayMobile(int thumb_thickness, int scrollbar_margin)
    : ScrollbarThemeOverlay(thumb_thickness, scrollbar_margin, thumb_thickness, scrollbar_margin)
    , default_color_(Color::FromSkColor4f(ScrollbarStyle().color))
{
}

void ScrollbarThemeOverlayMobile::PaintThumb(GraphicsContext& context, const Scrollbar& scrollbar, const gfx::Rect& rect)
{
    ...
    AutoDarkMode auto_dark_mode(PaintAutoDarkMode(box->StyleRef(), DarkModeFilter::ElementRole::kBackground));
    context.FillRect(rect, color, auto_dark_mode);
}

SkColor4f ScrollbarThemeOverlayMobile::ThumbColor(const Scrollbar& scrollbar) const
{
