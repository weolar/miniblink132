...
public:
    ...
    static MockAnimatingScrollableArea* Create(const ScrollOffset& maximum_scroll_offset)
    {
...
public:
    ...
    MOCK_CONST_METHOD3(PartsToInvalidateOnThumbPositionChange, ScrollbarPart(const Scrollbar&, float, float));
};

} // namespace

class ScrollableAreaTest : public testing::Test, public PaintTestConfigurations {
private:
...
TEST_P(ScrollableAreaTest, ScrollAnimatorCurrentPositionShouldBeSync)
{
    ...
    MockScrollableArea* scrollable_area = MockScrollableArea::Create(ScrollOffset(0, 100));
    scrollable_area->SetScrollOffset(ScrollOffset(0, 10000), mojom::blink::ScrollType::kCompositor);
...
TEST_P(ScrollableAreaTest, InvalidatesNonCompositedScrollbarsWhenThumbMoves)
{
    ...
    ScrollbarThemeWithMockInvalidation theme;
    MockScrollableArea* scrollable_area = MockScrollableArea::Create(ScrollOffset(100, 100));
    Scrollbar* horizontal_scrollbar = Scrollbar::CreateForTesting(scrollable_area, kHorizontalScrollbar, &theme);
    Scrollbar* vertical_scrollbar = Scrollbar::CreateForTesting(scrollable_area, kVerticalScrollbar, &theme);
    EXPECT_CALL(*scrollable_area, HorizontalScrollbar()).WillRepeatedly(Return(horizontal_scrollbar));
    EXPECT_CALL(*scrollable_area, VerticalScrollbar()).WillRepeatedly(Return(vertical_scrollbar));

    // Regardless of whether the theme invalidates any parts, non-composited
    // scrollbars have to be repainted if the thumb moves.
    EXPECT_CALL(*scrollable_area, LayerForHorizontalScrollbar()).WillRepeatedly(Return(nullptr));
    EXPECT_CALL(*scrollable_area, LayerForVerticalScrollbar()).WillRepeatedly(Return(nullptr));
    ASSERT_FALSE(scrollable_area->HasLayerForVerticalScrollbar());
    ASSERT_FALSE(scrollable_area->HasLayerForHorizontalScrollbar());
    EXPECT_CALL(theme, ShouldRepaintAllPartsOnInvalidation()).WillRepeatedly(Return(false));
    EXPECT_CALL(theme, PartsToInvalidateOnThumbPositionChange(_, _, _)).WillRepeatedly(Return(kNoPart));

    // A scroll in each direction should only invalidate one scrollbar.
    scrollable_area->SetScrollOffset(ScrollOffset(0, 50), mojom::blink::ScrollType::kProgrammatic);
    ...
    scrollable_area->SetScrollOffset(ScrollOffset(50, 50), mojom::blink::ScrollType::kProgrammatic);
...
{
    ScopedTestingPlatformSupport<TestingPlatformSupportWithMockScheduler> platform;
    MockAnimatingScrollableArea* scrollable_area = MockAnimatingScrollableArea::Create(ScrollOffset(0, 100));
    // Disable animations. Make sure an explicitly smooth programmatic scroll is
    // instantly scrolled.
    {
        EXPECT_CALL(*scrollable_area, ScrollAnimatorEnabled()).WillRepeatedly(Return(false));
        EXPECT_CALL(*scrollable_area, ScheduleAnimation()).Times(0);
        scrollable_area->SetScrollOffset(ScrollOffset(0, 100), mojom::blink::ScrollType::kProgrammatic, mojom::blink::ScrollBehavior::kSmooth);
    ...
    {
        EXPECT_CALL(*scrollable_area, ScrollAnimatorEnabled()).WillRepeatedly(Return(true));
        EXPECT_CALL(*scrollable_area, ScheduleAnimation()).WillOnce(Return(true));
        scrollable_area->SetScrollOffset(ScrollOffset(0, 50), mojom::blink::ScrollType::kProgrammatic, mojom::blink::ScrollBehavior::kSmooth);
        // Offset is unchanged.
        EXPECT_EQ(100, scrollable_area->GetScrollOffset().y());
    }
}
...
TEST_P(ScrollableAreaTest, PopupOverlayScrollbarShouldNotFadeOut)
{
    ...
    MockScrollableArea* scrollable_area = MockScrollableArea::Create(ScrollOffset(0, 100));
    EXPECT_CALL(*scrollable_area, UsesCompositedScrolling()).WillRepeatedly(Return(false));
    scrollable_area->SetIsPopup();

    ScrollbarThemeOverlayMock& theme = (ScrollbarThemeOverlayMock&)scrollable_area->GetPageScrollbarTheme();
...
TEST_P(ScrollableAreaTest, ScrollAnimatorCallbackFiresOnAnimationCancel)
{
    ...
    MockScrollableArea* scrollable_area = MockScrollableArea::Create(ScrollOffset(0, 100));
    EXPECT_CALL(*scrollable_area, ScheduleAnimation()).WillRepeatedly(Return(true));
    bool finished = false;
    scrollable_area->SetScrollOffset(ScrollOffset(0, 10000), mojom::blink::ScrollType::kProgrammatic, mojom::blink::ScrollBehavior::kSmooth,
...
TEST_P(ScrollableAreaTest, ScrollAnimatorCallbackFiresOnInstantScroll)
{
    ...
    EXPECT_CALL(*scrollable_area, ScheduleAnimation()).WillRepeatedly(Return(true));
    bool finished = false;
    scrollable_area->SetScrollOffset(ScrollOffset(0, 10000), mojom::blink::ScrollType::kProgrammatic, mojom::blink::ScrollBehavior::kInstant,
        ScrollableArea::ScrollCallback(
            WTF::BindOnce([](bool* finished, ScrollableArea::ScrollCompletionMode) { *finished = true; }, WTF::Unretained(&finished))));
...
TEST_P(ScrollableAreaTest, ScrollAnimatorCallbackFiresOnAnimationFinish)
{
    ...
    EXPECT_CALL(*scrollable_area, ScheduleAnimation()).WillRepeatedly(Return(true));
    bool finished = false;
    scrollable_area->SetScrollOffset(ScrollOffset(0, 9), mojom::blink::ScrollType::kProgrammatic, mojom::blink::ScrollBehavior::kSmooth,
        ScrollableArea::ScrollCallback(
            WTF::BindOnce([](bool* finished, ScrollableArea::ScrollCompletionMode) { *finished = true; }, WTF::Unretained(&finished))));
    EXPECT_EQ(0.0, scrollable_area->GetScrollAnimator().CurrentOffset().y());
    EXPECT_FALSE(finished);
...
TEST_P(ScrollableAreaTest, ScrollBackToInitialPosition)
{
    ...
    scrollable_area->SetScrollOffset(ScrollOffset(0, 50), mojom::blink::ScrollType::kProgrammatic, mojom::blink::ScrollBehavior::kSmooth,
        ScrollableArea::ScrollCallback(
            WTF::BindOnce([](bool* finished, ScrollableArea::ScrollCompletionMode) { *finished = true; }, WTF::Unretained(&finished))));
    scrollable_area->SetScrollOffset(ScrollOffset(0, 0), mojom::blink::ScrollType::kProgrammatic, mojom::blink::ScrollBehavior::kSmooth);
    scrollable_area->UpdateCompositorScrollAnimations();
    scrollable_area->ServiceScrollAnimations(1);
    scrollable_area->ServiceScrollAnimations(1000000);
...
void VerifyOffsetFromScrollStart(ScrollableArea* scrollable_area, ScrollStartValueType y_type, ScrollStartValueType x_type, const Length& y_length,
    const Length& x_length, const ScrollOffset& offset)
{
    switch (y_type) {
    ...
    case blink::ScrollStartValueType::kCenter: {
        EXPECT_EQ(offset.y(), scrollable_area->MinimumScrollOffset().y() + 0.5 * scrollable_area->ScrollSize(kVerticalScrollbar));
        break;
    }
    ...
    case blink::ScrollStartValueType::kLengthOrPercentage: {
        float expected_offset = scrollable_area->MinimumScrollOffset().y() + FloatValueForLength(y_length, scrollable_area->ScrollSize(kVerticalScrollbar));
    ...
    case blink::ScrollStartValueType::kCenter: {
        EXPECT_EQ(offset.x(), scrollable_area->MinimumScrollOffset().x() + 0.5 * scrollable_area->ScrollSize(kHorizontalScrollbar));
        break;
    }
    case blink::ScrollStartValueType::kEnd:
    case blink::ScrollStartValueType::kRight: {
    ...
    case blink::ScrollStartValueType::kLengthOrPercentage: {
        float expected_offset = scrollable_area->MinimumScrollOffset().x() + FloatValueForLength(x_length, scrollable_area->ScrollSize(kHorizontalScrollbar));
...
void test_scroll_start_combination(
    ScrollableArea* scrollable_area, ScrollStartValueType y_type, ScrollStartValueType x_type, const Length& y_length, const Length& x_length)
{
    ...
    ScrollOffset offset = scrollable_area->ScrollOffsetFromScrollStartData(y_data, x_data);
    VerifyOffsetFromScrollStart(scrollable_area, y_type, x_type, y_length, x_length, offset);
}

TEST_P(ScrollableAreaTest, ScrollOffsetFromScrollStartDataAllCombinations)
{
    const Vector<ScrollStartValueType> scroll_start_values = { ScrollStartValueType::kAuto, ScrollStartValueType::kLengthOrPercentage,
        ScrollStartValueType::kStart, ScrollStartValueType::kCenter, ScrollStartValueType::kEnd, ScrollStartValueType::kTop, ScrollStartValueType::kBottom,
        ScrollStartValueType::kLeft, ScrollStartValueType::kRight };
    const int max_horizontal_scroll_offset = 500;
    const int max_vertical_scroll_offset = 500;
    MockScrollableArea* scrollable_area = MockScrollableArea::Create(ScrollOffset(max_horizontal_scroll_offset, max_vertical_scroll_offset));
    ON_CALL(*scrollable_area, ScrollSize(kHorizontalScrollbar)).WillByDefault(Return(max_horizontal_scroll_offset));
    ON_CALL(*scrollable_area, ScrollSize(kVerticalScrollbar)).WillByDefault(Return(max_vertical_scroll_offset));

    for (auto y_type : scroll_start_values) {
        Length y_length = y_type == ScrollStartValueType::kLengthOrPercentage ? Length(100, Length::Type::kFixed) : Length();
        for (auto x_type : scroll_start_values) {
            Length x_length = x_type == ScrollStartValueType::kLengthOrPercentage ? Length(100, Length::Type::kFixed) : Length();
            test_scroll_start_combination(scrollable_area, y_type, x_type, y_length, x_length);
        }
    }
}
...
TEST_P(ScrollableAreaTest, ScrollOffsetFromScrollStartDataNonZeroMin)
{
    ...
    const int horizontal_scroll_size = max_horizontal_scroll_offset - min_horizontal_scroll_offset;
    const int vertical_scroll_size = max_vertical_scroll_offset - min_vertical_scroll_offset;
    MockScrollableArea* scrollable_area = MockScrollableArea::Create(
        ScrollOffset(max_horizontal_scroll_offset, max_vertical_scroll_offset), ScrollOffset(min_horizontal_scroll_offset, min_vertical_scroll_offset));
    ScrollOffset offset;
    ScrollStartData y_data;
    ScrollStartData x_data;

    ON_CALL(*scrollable_area, ScrollSize(kHorizontalScrollbar)).WillByDefault(Return(horizontal_scroll_size));
    ON_CALL(*scrollable_area, ScrollSize(kVerticalScrollbar)).WillByDefault(Return(vertical_scroll_size));
    ...
    x_data.value_type = ScrollStartValueType::kLengthOrPercentage;
    offset = scrollable_area->ScrollOffsetFromScrollStartData(y_data, x_data);
    EXPECT_EQ(offset.y(), scrollable_area->MinimumScrollOffset().y() + 0.5 * vertical_scroll_size);
    EXPECT_EQ(offset.x(), scrollable_area->MinimumScrollOffset().x() + 0.5 * horizontal_scroll_size);
...
TEST_P(ScrollableAreaTest, FilterIncomingScrollDuringSmoothUserScroll)
{
    ...
    MockScrollableArea* area = MockScrollableArea::Create(ScrollOffset(100, 100), ScrollOffset(0, 0));
    area->set_active_smooth_scroll_type_for_testing(ScrollType::kUser);
    const std::vector<mojom::blink::ScrollType> scroll_types = { ScrollType::kUser, ScrollType::kProgrammatic, ScrollType::kClamping, ScrollType::kCompositor,
        ScrollType::kAnchoring, ScrollType::kSequenced, ScrollType::kScrollStart };
