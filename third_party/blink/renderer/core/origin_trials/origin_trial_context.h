...
struct OriginTrialTokenResult {
    OriginTrialTokenResult(const String& raw_token, OriginTrialTokenStatus status, const std::optional<TrialToken>& parsed_token);
...
class CORE_EXPORT OriginTrialContext final : public GarbageCollected<OriginTrialContext> {
public:
    ...
    static std::unique_ptr<Vector<String>> ParseHeaderValue(const String& header_value);

    static void AddTokensFromHeader(ExecutionContext*, const String& header_value);
    ...
    static std::unique_ptr<Vector<mojom::blink::OriginTrialFeature>> GetInheritedTrialFeatures(ExecutionContext*);
    ...
    static std::unique_ptr<Vector<mojom::blink::OriginTrialFeature>> GetEnabledNavigationFeatures(ExecutionContext*);
    ...
    static void ActivateWorkerInheritedFeatures(ExecutionContext*, const Vector<mojom::blink::OriginTrialFeature>*);
    ...
    static void ActivateNavigationFeaturesFromInitiator(ExecutionContext*, const Vector<mojom::blink::OriginTrialFeature>*);
    ...
    void AddTokenFromExternalScript(const String& token, const Vector<scoped_refptr<SecurityOrigin>>& external_origins);
    void AddTokens(const Vector<String>& tokens);

    void ActivateWorkerInheritedFeatures(const Vector<mojom::blink::OriginTrialFeature>& features);

    void ActivateNavigationFeaturesFromInitiator(const Vector<mojom::blink::OriginTrialFeature>& features);
    ...
    std::unique_ptr<Vector<mojom::blink::OriginTrialFeature>> GetInheritedTrialFeatures() const;

    std::unique_ptr<Vector<mojom::blink::OriginTrialFeature>> GetEnabledNavigationFeatures() const;
    ...
    bool IsNavigationFeatureActivated(const mojom::blink::OriginTrialFeature feature) const;
    ...
    const HashMap<mojom::blink::OriginTrialFeature, Vector<String>> GetFeatureToTokensForTesting() const
    {
...
private:
    ...
    void AddTokenInternal(const String& token, const OriginInfo origin_info, const Vector<OriginInfo>* script_origins);
    ...
    OriginTrialFeaturesEnabled EnableTrialFromName(const String& trial_name, base::Time expiry_time);
    ...
    bool EnableTrialFromToken(const String& token, const OriginInfo origin_info, const Vector<OriginInfo>* script_origins);
    ...
    bool InstallFeatures(const HashSet<mojom::blink::OriginTrialFeature>& features, Document&, ScriptState*);
    ...
    // `trial_results_`.
    void CacheToken(const String& raw_token, const TrialTokenResult&, OriginTrialStatus);
    ...
    WTF::HashMap<mojom::blink::OriginTrialFeature, base::Time> feature_expiry_times_;
    ...
    HashMap<mojom::blink::OriginTrialFeature, Vector</*Raw Token*/ String>> feature_to_tokens_;
};
