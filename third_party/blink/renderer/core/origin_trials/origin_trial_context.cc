...
String ExtractTokenOrQuotedString(const String& header_value, unsigned& pos)
{
    ...
    if (header_value[pos] == '\'' || header_value[pos] == '"') {
        ...
        int start_pos = pos;
        while (pos < len && !IsWhitespace(header_value[pos]) && header_value[pos] != ',')
            pos++;
...
OriginTrialStatus MergeOriginTrialStatus(OriginTrialStatus s1, OriginTrialStatus s2)
{
...
OriginTrialTokenResult::OriginTrialTokenResult(const String& raw_token, OriginTrialTokenStatus status, const std::optional<TrialToken>& parsed_token)
    : raw_token(raw_token)
    , status(status)
...
void OriginTrialContext::SetTrialTokenValidatorForTesting(std::unique_ptr<TrialTokenValidator> validator)
{
    trial_token_validator_ = std::move(validator);
}

// static
std::unique_ptr<Vector<String>> OriginTrialContext::ParseHeaderValue(const String& header_value)
{
...
// static
void OriginTrialContext::AddTokensFromHeader(ExecutionContext* context, const String& header_value)
{
...
// static
void OriginTrialContext::AddTokens(ExecutionContext* context, const Vector<String>* tokens)
{
...
// static
void OriginTrialContext::ActivateWorkerInheritedFeatures(ExecutionContext* context, const Vector<mojom::blink::OriginTrialFeature>* features)
{
    if (!features || features->empty())
        return;
    DCHECK(context && context->GetOriginTrialContext());
    DCHECK(context->IsDedicatedWorkerGlobalScope() || context->IsWorkletGlobalScope());
    context->GetOriginTrialContext()->ActivateWorkerInheritedFeatures(*features);
}

// static
void OriginTrialContext::ActivateNavigationFeaturesFromInitiator(ExecutionContext* context, const Vector<mojom::blink::OriginTrialFeature>* features)
{
    if (!features || features->empty())
        return;
    DCHECK(context && context->GetOriginTrialContext());
    context->GetOriginTrialContext()->ActivateNavigationFeaturesFromInitiator(*features);
}

// static
std::unique_ptr<Vector<String>> OriginTrialContext::GetTokens(ExecutionContext* execution_context)
{
    DCHECK(execution_context);
    const OriginTrialContext* context = execution_context->GetOriginTrialContext();
    ...
    for (const auto& entry : context->trial_results_) {
        ...
        for (const OriginTrialTokenResult& token_result : trial_result.token_results) {
...
// static
std::unique_ptr<Vector<mojom::blink::OriginTrialFeature>> OriginTrialContext::GetInheritedTrialFeatures(ExecutionContext* execution_context)
{
    DCHECK(execution_context);
    const OriginTrialContext* context = execution_context->GetOriginTrialContext();
...
// static
std::unique_ptr<Vector<mojom::blink::OriginTrialFeature>> OriginTrialContext::GetEnabledNavigationFeatures(ExecutionContext* execution_context)
{
    DCHECK(execution_context);
    const OriginTrialContext* context = execution_context->GetOriginTrialContext();
...
std::unique_ptr<Vector<mojom::blink::OriginTrialFeature>> OriginTrialContext::GetInheritedTrialFeatures() const
{
    if (enabled_features_.empty()) {
        return nullptr;
    }
    std::unique_ptr<Vector<mojom::blink::OriginTrialFeature>> result = std::make_unique<Vector<mojom::blink::OriginTrialFeature>>();
...
std::unique_ptr<Vector<mojom::blink::OriginTrialFeature>> OriginTrialContext::GetEnabledNavigationFeatures() const
{
    if (enabled_features_.empty())
        return nullptr;
    std::unique_ptr<Vector<mojom::blink::OriginTrialFeature>> result = std::make_unique<Vector<mojom::blink::OriginTrialFeature>>();
...
void OriginTrialContext::AddTokenFromExternalScript(const String& token, const Vector<scoped_refptr<SecurityOrigin>>& external_origins)
{
    Vector<OriginInfo> script_origins;
    for (const scoped_refptr<SecurityOrigin>& origin : external_origins) {
        OriginInfo origin_info = { .origin = origin, .is_secure = origin->IsPotentiallyTrustworthy() };
        DVLOG(1) << "AddTokenFromExternalScript: " << origin->ToString() << ", secure = " << origin_info.is_secure;
        script_origins.push_back(origin_info);
    }
    AddTokenInternal(token, GetCurrentOriginInfo(), &script_origins);
}

void OriginTrialContext::AddTokenInternal(const String& token, const OriginInfo origin, const Vector<OriginInfo>* script_origins)
{
    if (token.empty())
        return;
...
void OriginTrialContext::ActivateWorkerInheritedFeatures(const Vector<mojom::blink::OriginTrialFeature>& features)
{
...
void OriginTrialContext::ActivateNavigationFeaturesFromInitiator(const Vector<mojom::blink::OriginTrialFeature>& features)
{
    for (const mojom::blink::OriginTrialFeature& feature : features) {
        if (IsCrossNavigationFeature(feature)) {
...
void OriginTrialContext::InitializePendingFeatures()
{
    ...
    bool added_binding_feature = InstallFeatures(enabled_features_, *window->document(), script_state);
    added_binding_feature |= InstallFeatures(navigation_activated_features_, *window->document(), script_state);

    if (added_binding_feature) {
        // Also allow V8 to install conditional features now.
        script_state->GetIsolate()->InstallConditionalFeatures(script_state->GetContext());
    }
}

bool OriginTrialContext::InstallFeatures(const HashSet<mojom::blink::OriginTrialFeature>& features, Document& document, ScriptState* script_state)
{
    ...
    for (mojom::blink::OriginTrialFeature enabled_feature : features) {
        ...
        if (context_->IsWindow() && feature_to_tokens_.Contains(enabled_feature)) {
            ...
            context_->GetRuntimeFeatureStateOverrideContext()->ApplyOriginTrialOverride(enabled_feature, feature_to_tokens_.find(enabled_feature)->value);
        }
...
bool OriginTrialContext::InstallSettingFeature(Document& document, mojom::blink::OriginTrialFeature enabled_feature)
{
...
bool OriginTrialContext::IsFeatureEnabled(mojom::blink::OriginTrialFeature feature) const
{
    return enabled_features_.Contains(feature) || navigation_activated_features_.Contains(feature);
}

base::Time OriginTrialContext::GetFeatureExpiry(mojom::blink::OriginTrialFeature feature)
{
...
bool OriginTrialContext::IsNavigationFeatureActivated(mojom::blink::OriginTrialFeature feature) const
{
    return navigation_activated_features_.Contains(feature);
}

void OriginTrialContext::AddForceEnabledTrials(const Vector<String>& trial_names)
{
    bool is_valid = false;
    for (const auto& trial_name : trial_names) {
        DCHECK(origin_trials::IsTrialValid(trial_name.Utf8()));
        is_valid |= EnableTrialFromName(trial_name, /*expiry_time=*/base::Time::Max()).status == OriginTrialStatus::kEnabled;
    }
...
bool OriginTrialContext::CanEnableTrialFromName(const StringView& trial_name)
{
    if (trial_name == "FledgeBiddingAndAuctionServer") {
        return base::FeatureList::IsEnabled(features::kInterestGroupStorage) && base::FeatureList::IsEnabled(features::kFledgeBiddingAndAuctionServer);
    }
    ...
    if (trial_name == "SpeculationRulesPrefetchFuture") {
        return base::FeatureList::IsEnabled(features::kSpeculationRulesPrefetchFuture);
    }

    if (trial_name == "BackForwardCacheSendNotRestoredReasons") {
        return base::FeatureList::IsEnabled(features::kBackForwardCacheSendNotRestoredReasons);
    }

    if (trial_name == "CompressionDictionaryTransport") {
        return base::FeatureList::IsEnabled(network::features::kCompressionDictionaryTransportBackend);
    }
    ...
    if (trial_name == "AIPromptAPIForExtension") {
        return base::FeatureList::IsEnabled(features::kEnableAIPromptAPIForExtension);
    }
...
OriginTrialFeaturesEnabled OriginTrialContext::EnableTrialFromName(const String& trial_name, base::Time expiry_time)
{
    if (!CanEnableTrialFromName(trial_name)) {
        DVLOG(1) << "EnableTrialFromName: cannot enable trial " << trial_name;
        return { OriginTrialStatus::kTrialNotAllowed, Vector<mojom::blink::OriginTrialFeature>() };
    }

    bool did_enable_feature = false;
    Vector<mojom::blink::OriginTrialFeature> origin_trial_features;
    for (mojom::blink::OriginTrialFeature feature : origin_trials::FeaturesForTrial(trial_name.Utf8())) {
        if (!origin_trials::FeatureEnabledForOS(feature)) {
            DVLOG(1) << "EnableTrialFromName: feature " << static_cast<int>(feature) << " is disabled on current OS.";
            continue;
        }
        ...
        for (mojom::blink::OriginTrialFeature implied_feature : origin_trials::GetImpliedFeatures(feature)) {
    ...
    OriginTrialStatus status = did_enable_feature ? OriginTrialStatus::kEnabled : OriginTrialStatus::kOSNotSupported;
    return { status, std::move(origin_trial_features) };
}

bool OriginTrialContext::EnableTrialFromToken(const String& token, const OriginInfo origin_info)
{
    return EnableTrialFromToken(token, origin_info, nullptr);
}

bool OriginTrialContext::EnableTrialFromToken(const String& token, const OriginInfo origin_info, const Vector<OriginInfo>* script_origins)
{
    DCHECK(!token.empty());
    ...
    if (script_origins) {
        for (const OriginInfo& script_origin_info : *script_origins) {
            script_url_origins.emplace_back(script_origin_info.origin->ToUrlOrigin(), script_origin_info.is_secure);
        }
    }
    base::span<const TrialTokenValidator::OriginInfo> script_url_origins_span(script_url_origins.data(), script_url_origins.size());

    TrialTokenResult token_result = trial_token_validator_->ValidateTokenAndTrialWithOriginInfo(
        token_string.AsStringView(),
        TrialTokenValidator::OriginInfo(origin_info.origin->ToUrlOrigin(), origin_info.is_secure), 
        script_url_origins_span, 
        base::Time::Now());
    DVLOG(1) << "EnableTrialFromToken: token_result = " << token_result.Status() << ", token = " << token;

    if (token_result.Status() == OriginTrialTokenStatus::kSuccess) {
        String trial_name = String::FromUTF8(token_result.ParsedToken()->feature_name());
        OriginTrialFeaturesEnabled result = EnableTrialFromName(trial_name, token_result.ParsedToken()->expiry_time());
        ...
        SendTokenToBrowser(origin_info, *token_result.ParsedToken(), token, script_origins);
    }
...
void OriginTrialContext::CacheToken(const String& raw_token, const TrialTokenResult& token_result, OriginTrialStatus trial_status)
{
    String trial_name = token_result.ParsedToken() && token_result.Status() != OriginTrialTokenStatus::kUnknownTrial
        ? String::FromUTF8(token_result.ParsedToken()->feature_name())
        : kDefaultTrialName;

    // Does nothing if key already exists.
    auto& trial_result = trial_results_
                             .insert(trial_name,
                                 OriginTrialResult {
                                     trial_name,
    ...
    trial_result.status = MergeOriginTrialStatus(trial_result.status, trial_status);
    trial_result.token_results.push_back(OriginTrialTokenResult {
        raw_token, token_result.Status(), token_result.ParsedToken() ? std::make_optional(*token_result.ParsedToken()) : std::nullopt });
}
...
void OriginTrialContext::SendTokenToBrowser(
    const OriginInfo& origin_info, const TrialToken& parsed_token, const String& raw_token, const Vector<OriginInfo>* script_origin_info)
{
    ...
    if (!origin_trials::IsTrialPersistentToNextResponse(parsed_token.feature_name())) {
        return;
    }
    ...
    context_->GetRuntimeFeatureStateOverrideContext()->EnablePersistentTrial(raw_token, std::move(script_origins));
}
