...
public:
    ...
    struct ValidationParams {
        ...
        ValidationParams(std::string_view token_param, const OriginInfo& origin_info, base::span<const OriginInfo> scripts, base::Time time)
            : token(token_param)
            , origin(origin_info)
            , current_time(time)
        {
    ...
    TrialTokenResult ValidateTokenAndTrialWithOriginInfo(
        std::string_view token, const OriginInfo& origin, base::span<const OriginInfo> third_party_origin_info, base::Time current_time) const override
    {
        validation_params_.emplace_back(token, origin, third_party_origin_info, current_time);
        if (response_.status == OriginTrialTokenStatus::kMalformed) {
            ...
            return TrialTokenResult(response_.status,
                TrialToken::CreateTrialTokenForTesting(
                    origin.origin, false, response_.feature, response_.expiry, false, TrialToken::UsageRestriction::kNone, ""));
        }
    }
...
protected:
    ...
    {
        execution_context_->GetOriginTrialContext()->SetTrialTokenValidatorForTesting(std::unique_ptr<MockTokenValidator>(token_validator_));
    }
    ...
    void UpdateSecurityOrigin(const String& origin)
    {
        ...
        scoped_refptr<SecurityOrigin> page_origin = SecurityOrigin::Create(page_url);
    ...
    void AddTokenWithResponse(const std::string& trial_name, OriginTrialTokenStatus validation_status)
    {
        AddTokenWithResponse({ .status = validation_status, .feature = trial_name });
    }

    void AddTokenForThirdPartyOriginsWithResponse(const std::string& trial_name, OriginTrialTokenStatus validation_status, const Vector<String>& script_origins)
    {
        token_validator_->SetResponse({ .status = validation_status, .feature = trial_name });
        ...
        execution_context_->GetOriginTrialContext()->AddTokenFromExternalScript(kTokenPlaceholder, external_origins);
    }

    bool IsFeatureEnabled(mojom::blink::OriginTrialFeature feature)
    {
        return execution_context_->GetOriginTrialContext()->IsFeatureEnabled(feature);
    }

    base::Time GetFeatureExpiry(mojom::blink::OriginTrialFeature feature)
    {
        return execution_context_->GetOriginTrialContext()->GetFeatureExpiry(feature);
    }

    std::unique_ptr<Vector<mojom::blink::OriginTrialFeature>> GetEnabledNavigationFeatures()
    {
        return execution_context_->GetOriginTrialContext()->GetEnabledNavigationFeatures();
    }

    HashMap<mojom::blink::OriginTrialFeature, Vector<String>> GetFeatureToTokens()
    {
        return execution_context_->GetOriginTrialContext()->GetFeatureToTokensForTesting();
    }

    bool ActivateNavigationFeature(mojom::blink::OriginTrialFeature feature)
    {
        execution_context_->GetOriginTrialContext()->ActivateNavigationFeaturesFromInitiator({ feature });
        return execution_context_->GetOriginTrialContext()->IsNavigationFeatureActivated(feature);
    }
...
TEST_F(OriginTrialContextTest, ValidatorGetsCorrectInfo)
{
    ...
    Vector<MockTokenValidator::ValidationParams> validation_params = token_validator_->GetValidationParams();
    ASSERT_EQ(1ul, validation_params.size());
    EXPECT_EQ(url::Origin::Create(GURL(kFrobulateEnabledOrigin)), validation_params[0].origin.origin);
    ...
    ASSERT_LT(base::Time::Now() - validation_params[0].current_time, base::Minutes(5));
    ASSERT_LT(validation_params[0].current_time - base::Time::Now(), base::Minutes(5));
}

// Test that we're passing correct security information to the validator
TEST_F(OriginTrialContextTest, ValidatorGetsCorrectSecurityInfoForInsecureOrigins)
{
    ...
    Vector<MockTokenValidator::ValidationParams> validation_params = token_validator_->GetValidationParams();
    ASSERT_EQ(1ul, validation_params.size());
    EXPECT_EQ(url::Origin::Create(GURL(kFrobulateEnabledOriginInsecure)), validation_params[0].origin.origin);
...
TEST_F(OriginTrialContextTest, ValidatorGetsCorrectSecurityInfoThirdParty)
{
    ...
    AddTokenForThirdPartyOriginsWithResponse(
        kFrobulateThirdPartyTrialName, OriginTrialTokenStatus::kInsecure, { kUnrelatedSecureOrigin, kFrobulateEnabledOriginInsecure });
    ...
    auto unrelated_info = base::ranges::find_if(validation_params[0].third_party_origin_info,
        [](const TrialTokenValidator::OriginInfo& item) { return item.origin.IsSameOriginWith(GURL(kUnrelatedSecureOrigin)); });
    ASSERT_NE(validation_params[0].third_party_origin_info.end(), unrelated_info);
    EXPECT_TRUE(unrelated_info->is_secure);

    auto insecure_origin_info = base::ranges::find_if(validation_params[0].third_party_origin_info,
        [](const TrialTokenValidator::OriginInfo& item) { return item.origin.IsSameOriginWith(GURL(kFrobulateEnabledOriginInsecure)); });
    ASSERT_NE(validation_params[0].third_party_origin_info.end(), insecure_origin_info);
...
TEST_F(OriginTrialContextTest, EnabledNonExistingTrial)
{
    ...
    bool is_non_existing_feature_enabled = IsFeatureEnabled(mojom::blink::OriginTrialFeature::kNonExisting);
...
TEST_F(OriginTrialContextTest, EnabledSecureRegisteredOrigin)
{
    ...
    AddTokenWithResponse(kFrobulateTrialName, OriginTrialTokenStatus::kSuccess);
    bool is_origin_enabled = IsFeatureEnabled(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPI);
...
{
    UpdateSecurityOrigin(kFrobulateEnabledOrigin);
    AddTokenForThirdPartyOriginsWithResponse(kFrobulateThirdPartyTrialName, OriginTrialTokenStatus::kSuccess, { kFrobulateEnabledOrigin });
    bool is_origin_enabled = IsFeatureEnabled(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPIThirdParty);
    EXPECT_TRUE(is_origin_enabled);
}
...
{
    UpdateSecurityOrigin(kFrobulateEnabledOrigin);
    AddTokenWithResponse(kFrobulateTrialName, OriginTrialTokenStatus::kInvalidSignature);
...
TEST_F(OriginTrialContextTest, FeatureNotEnableOnInsecureOrigin)
{
    UpdateSecurityOrigin(kFrobulateEnabledOriginInsecure);
    AddTokenWithResponse(kFrobulateTrialName, OriginTrialTokenStatus::kInsecure);
    EXPECT_FALSE(IsFeatureEnabled(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPI));
}
...
{
    UpdateSecurityOrigin(kFrobulateEnabledOrigin);
    AddTokenForThirdPartyOriginsWithResponse(kFrobulateThirdPartyTrialName, OriginTrialTokenStatus::kInsecure, { kFrobulateEnabledOriginInsecure });
    EXPECT_FALSE(IsFeatureEnabled(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPIThirdParty));
}
...
TEST_F(OriginTrialContextTest, PermissionsPolicy)
{
    ...
    EXPECT_TRUE(context->IsFeatureEnabled(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPI));

    // Make a mock feature name map with "frobulate".
    FeatureNameMap feature_map;
    feature_map.Set("frobulate", mojom::blink::PermissionsPolicyFeature::kFrobulate);
    ...
    scoped_refptr<const SecurityOrigin> security_origin = SecurityOrigin::CreateFromString(kFrobulateEnabledOrigin);

    PolicyParserMessageBuffer logger;
    ParsedPermissionsPolicy result;
    result = PermissionsPolicyParser::ParsePermissionsPolicyForTest("frobulate=*", security_origin, nullptr, logger, feature_map, window);
    EXPECT_TRUE(logger.GetMessages().empty());
    ASSERT_EQ(1u, result.size());
    EXPECT_EQ(mojom::blink::PermissionsPolicyFeature::kFrobulate, result[0].feature);
}

TEST_F(OriginTrialContextTest, GetEnabledNavigationFeatures)
{
    UpdateSecurityOrigin(kFrobulateEnabledOrigin);
    AddTokenWithResponse(kFrobulateNavigationTrialName, OriginTrialTokenStatus::kSuccess);
    EXPECT_TRUE(IsFeatureEnabled(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPINavigation));

    auto enabled_navigation_features = GetEnabledNavigationFeatures();
    ASSERT_NE(nullptr, enabled_navigation_features.get());
    EXPECT_EQ(WTF::Vector<mojom::blink::OriginTrialFeature>({ mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPINavigation }),
        *enabled_navigation_features.get());
}

TEST_F(OriginTrialContextTest, ActivateNavigationFeature)
{
    EXPECT_TRUE(ActivateNavigationFeature(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPINavigation));
    EXPECT_FALSE(ActivateNavigationFeature(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPI));
}
...
TEST_F(OriginTrialContextTest, GetTokenExpiryTimeIgnoresIrrelevantTokens)
{
    ...
    EXPECT_FALSE(IsFeatureEnabled(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPI));
    EXPECT_EQ(base::Time(), GetFeatureExpiry(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPI));

    // A different trial shouldn't affect Frobulate's expiry time.
    AddTokenWithResponse(kFrobulateDeprecationTrialName, OriginTrialTokenStatus::kSuccess);
    EXPECT_TRUE(IsFeatureEnabled(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPIDeprecation));
    EXPECT_EQ(base::Time(), GetFeatureExpiry(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPI));

    // A valid trial should update the expiry time.
    AddTokenWithResponse(kFrobulateTrialName, OriginTrialTokenStatus::kSuccess);
    EXPECT_TRUE(IsFeatureEnabled(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPI));
    EXPECT_EQ(kBaseTokenExpiryTime, GetFeatureExpiry(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPI));
}
...
TEST_F(OriginTrialContextTest, LastExpiryForFeatureIsUsed)
{
    ...
    EXPECT_TRUE(IsFeatureEnabled(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPI));
    EXPECT_EQ(plusone, GetFeatureExpiry(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPI));
    ...
    EXPECT_TRUE(IsFeatureEnabled(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPI));
    EXPECT_EQ(plusthree, GetFeatureExpiry(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPI));
...
TEST_F(OriginTrialContextTest, ImpliedFeatureExpiryTimesAreUpdated)
{
    ...
    EXPECT_TRUE(IsFeatureEnabled(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPI));
    EXPECT_EQ(plusone, GetFeatureExpiry(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPIImplied));
}
...
TEST_F(OriginTrialContextTest, SettingFeatureUpdatesDocumentSettings)
{
    ...
    EXPECT_TRUE(context->IsFeatureEnabled(mojom::blink::OriginTrialFeature::kAutoDarkMode));

    // Expect the AutoDarkMode setting to have been enabled.
    EXPECT_TRUE(page_holder->GetDocument().GetSettings()->GetForceDarkModeEnabled());
...
TEST_F(OriginTrialContextTest, AddedFeaturesAreMappedToTokens)
{
    ...
    UpdateSecurityOrigin(kFrobulateEnabledOrigin);
    AddTokenWithResponse(kFrobulateBrowserReadWriteTrialName, OriginTrialTokenStatus::kSuccess);
    // Ensure that FrobulateBrowserReadWrite is enabled.
    EXPECT_TRUE(IsFeatureEnabled(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPIBrowserReadWrite));
    EXPECT_TRUE(GetFeatureToTokens().Contains(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPIBrowserReadWrite));
    // Ensure that the corresponding token is stored.
    Vector<String> expected_tokens({ kTokenPlaceholder });
    EXPECT_EQ(GetFeatureToTokens().at(mojom::blink::OriginTrialFeature::kOriginTrialsSampleAPIBrowserReadWrite), expected_tokens);
}
...
public:
    ...
    const HashMap<String, OriginTrialResult> GetOriginTrialResultsForDevtools() const
    {
        return execution_context_->GetOriginTrialContext()->GetOriginTrialResultsForDevtools();
    }
    ...
    void ExpectTrialResultContains(const HashMap<String, OriginTrialResult>& trial_results, const String& trial_name, OriginTrialStatus trial_status,
        const Vector<ExpectedOriginTrialTokenResult>& expected_token_results) const
    {
        ...
        EXPECT_EQ(trial_result->value.token_results.size(), expected_token_results.size());
        ...
        for (wtf_size_t i = 0; i < expected_token_results.size(); i++) {
            ...
            EXPECT_EQ(actual_token_result.parsed_token.has_value(), expected_token_result.token_parsable);
...
TEST_F(OriginTrialContextDevtoolsTest, DependentFeatureNotEnabled)
{
    ...
    feature_list_.InitAndDisableFeature(blink::features::kSpeculationRulesPrefetchFuture);

    AddTokenWithResponse("SpeculationRulesPrefetchFuture", OriginTrialTokenStatus::kSuccess);

    EXPECT_FALSE(IsFeatureEnabled(mojom::blink::OriginTrialFeature::kSpeculationRulesPrefetchFuture));
    HashMap<String, OriginTrialResult> origin_trial_results = GetOriginTrialResultsForDevtools();
    EXPECT_EQ(origin_trial_results.size(), 1u);
    ExpectTrialResultContains(origin_trial_results, "SpeculationRulesPrefetchFuture", OriginTrialStatus::kTrialNotAllowed,
...
TEST_F(OriginTrialContextDevtoolsTest, TrialNameNotRecognized)
{
    ...
    AddTokenWithResponse(kUnknownTrialName, OriginTrialTokenStatus::kUnknownTrial);
    ...
    EXPECT_EQ(origin_trial_results.size(), 1u);
    ExpectTrialResultContains(origin_trial_results,
        /* trial_name */ "UNKNOWN", OriginTrialStatus::kValidTokenNotProvided, { { OriginTrialTokenStatus::kUnknownTrial, /* token_parsable */ true } });
}
...
TEST_F(OriginTrialContextDevtoolsTest, NoValidToken)
{
    ...
    EXPECT_EQ(origin_trial_results.size(), 1u);
    ExpectTrialResultContains(origin_trial_results,
        /* trial_name */ kFrobulateTrialName, OriginTrialStatus::kValidTokenNotProvided, { { OriginTrialTokenStatus::kExpired, /* token_parsable */ true } });
...
TEST_F(OriginTrialContextDevtoolsTest, Enabled)
{
    ...
    EXPECT_EQ(origin_trial_results.size(), 1u);
    ExpectTrialResultContains(origin_trial_results,
        /* trial_name */ kFrobulateTrialName, OriginTrialStatus::kEnabled, { { OriginTrialTokenStatus::kSuccess, /* token_parsable */ true } });
...
TEST_F(OriginTrialContextDevtoolsTest, UnparsableToken)
{
    ...
    EXPECT_EQ(origin_trial_results.size(), 1u);
    ExpectTrialResultContains(origin_trial_results,
        /* trial_name */ "UNKNOWN", OriginTrialStatus::kValidTokenNotProvided, { { OriginTrialTokenStatus::kMalformed, /* token_parsable */ false } });
}
...
TEST_F(OriginTrialContextDevtoolsTest, InsecureOrigin)
{
    ...
    EXPECT_EQ(origin_trial_results.size(), 1u);
    ExpectTrialResultContains(origin_trial_results,
        /* trial_name */ kFrobulateTrialName, OriginTrialStatus::kValidTokenNotProvided, { { OriginTrialTokenStatus::kInsecure, /* token_parsable */ true } });
}
