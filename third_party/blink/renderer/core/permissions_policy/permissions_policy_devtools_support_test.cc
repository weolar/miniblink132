...
TEST_F(PermissionsPolicyDevtoolsSupportSimTest, DetectIframeAttributeBlockage)
{
    ...
    std::optional<PermissionsPolicyBlockLocator> locator
        = TracePermissionsPolicyBlockSource(MainFrame().GetFrame()->FirstChild(), mojom::blink::PermissionsPolicyFeature::kFullscreen);

    ASSERT_NE(locator, std::nullopt);
    EXPECT_EQ(locator->frame_id, IdentifiersFactory::FrameId(MainFrame().GetFrame()->FirstChild()));
    EXPECT_EQ(locator->reason, PermissionsPolicyBlockReason::kIframeAttribute);
}

TEST_F(PermissionsPolicyDevtoolsSupportSimTest, DetectNestedIframeAttributeBlockage)
{
    SimRequest main_resource("https://example.com", "text/html");
    ...
    std::optional<PermissionsPolicyBlockLocator> locator
        = TracePermissionsPolicyBlockSource(MainFrame().GetFrame()->FirstChild()->FirstChild(), mojom::blink::PermissionsPolicyFeature::kFullscreen);
...
TEST_F(PermissionsPolicyDevtoolsSupportSimTest, DetectHeaderBlockage)
{
    ...
    std::optional<PermissionsPolicyBlockLocator> locator
        = TracePermissionsPolicyBlockSource(MainFrame().GetFrame(), mojom::blink::PermissionsPolicyFeature::kFullscreen);

    ASSERT_NE(locator, std::nullopt);
    EXPECT_EQ(locator->frame_id, IdentifiersFactory::FrameId(MainFrame().GetFrame()));
...
TEST_F(PermissionsPolicyDevtoolsSupportSimTest, DetectCrossOriginHeaderBlockage)
{
    SimRequest::Params main_params;
    main_params.response_http_headers = {
...
TEST_F(PermissionsPolicyDevtoolsSupportSimTest, DetectCrossOriginDefaultAllowlistBlockage)
{
    SimRequest main_resource("https://example.com", "text/html");
...
TEST_F(PermissionsPolicyDevtoolsSupportSimTest, DetectCrossOriginIframeAttributeBlockage)
{
    SimRequest::Params main_params;
    main_params.response_http_headers = {
        { "Permissions-Policy", "fullscreen=*" },
...
TEST_F(PermissionsPolicyDevtoolsSupportSimTest, DetectNestedCrossOriginNoBlockage)
{
    SimRequest::Params main_params;
    main_params.response_http_headers = {
    ...
    SecurityContext::FeatureStatus status = MainFrame().GetFrame()->GetSecurityContext()->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kFullscreen);
    EXPECT_TRUE(status.enabled);
    EXPECT_FALSE(status.should_report);
    EXPECT_EQ(status.reporting_endpoint, std::nullopt);

    status = MainFrame().GetFrame()->FirstChild()->GetSecurityContext()->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kFullscreen);
    EXPECT_TRUE(status.enabled);
    EXPECT_FALSE(status.should_report);
    EXPECT_EQ(status.reporting_endpoint, std::nullopt);

    status = MainFrame().GetFrame()->FirstChild()->FirstChild()->GetSecurityContext()->IsFeatureEnabled(mojom::blink::PermissionsPolicyFeature::kFullscreen);
    EXPECT_TRUE(status.enabled);
    EXPECT_FALSE(status.should_report);
    EXPECT_EQ(status.reporting_endpoint, std::nullopt);
