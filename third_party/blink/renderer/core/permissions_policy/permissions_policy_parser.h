...
public:
    ...
    struct Node {
        OriginWithPossibleWildcards::NodeType type { OriginWithPossibleWildcards::NodeType::kUnknown };
    ...
    static ParsedPermissionsPolicy ParseHeader(const String& feature_policy_header, const String& permission_policy_header, scoped_refptr<const SecurityOrigin>,
        PolicyParserMessageBuffer& feature_policy_logger, PolicyParserMessageBuffer& permissions_policy_logger, ExecutionContext* = nullptr);
    ...
    static ParsedPermissionsPolicy ParseAttribute(const String& policy, scoped_refptr<const SecurityOrigin> self_origin,
        scoped_refptr<const SecurityOrigin> src_origin, PolicyParserMessageBuffer& logger, ExecutionContext* = nullptr);
    ...
    static ParsedPermissionsPolicy ParsePolicyFromNode(
        Node&, scoped_refptr<const SecurityOrigin>, PolicyParserMessageBuffer& logger, ExecutionContext* = nullptr);

    static ParsedPermissionsPolicy ParseFeaturePolicyForTest(const String& policy, scoped_refptr<const SecurityOrigin> self_origin,
        scoped_refptr<const SecurityOrigin> src_origin, PolicyParserMessageBuffer& logger, const FeatureNameMap& feature_names, ExecutionContext* = nullptr);

    static ParsedPermissionsPolicy ParsePermissionsPolicyForTest(const String& policy, scoped_refptr<const SecurityOrigin> self_origin,
        scoped_refptr<const SecurityOrigin> src_origin, PolicyParserMessageBuffer& logger, const FeatureNameMap& feature_names, ExecutionContext* = nullptr);
};

// Returns true iff any declaration in the policy is for the given feature.
CORE_EXPORT bool IsFeatureDeclared(mojom::blink::PermissionsPolicyFeature, const ParsedPermissionsPolicy&);

// Removes any declaration in the policy for the given feature. Returns true if
// the policy was modified.
CORE_EXPORT bool RemoveFeatureIfPresent(mojom::blink::PermissionsPolicyFeature, ParsedPermissionsPolicy&);
...
// policy was modified.
CORE_EXPORT bool DisallowFeatureIfNotPresent(mojom::blink::PermissionsPolicyFeature, ParsedPermissionsPolicy&);

// If no declaration in the policy exists already for the feature, adds a
// declaration which allows the feature in all origins. Returns true if the
// policy was modified.
CORE_EXPORT bool AllowFeatureEverywhereIfNotPresent(mojom::blink::PermissionsPolicyFeature, ParsedPermissionsPolicy&);
...
CORE_EXPORT void DisallowFeature(mojom::blink::PermissionsPolicyFeature, ParsedPermissionsPolicy&);

// Returns true iff the feature should not be exposed to script.
CORE_EXPORT bool IsFeatureForMeasurementOnly(mojom::blink::PermissionsPolicyFeature);

// Replaces any existing declarations in the policy for the given feature with
// a declaration which allows the feature in all origins.
CORE_EXPORT void AllowFeatureEverywhere(mojom::blink::PermissionsPolicyFeature, ParsedPermissionsPolicy&);

CORE_EXPORT const String GetNameForFeature(mojom::blink::PermissionsPolicyFeature, bool is_isolated_context);
