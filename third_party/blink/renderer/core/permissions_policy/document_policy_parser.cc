...
std::optional<PolicyValue> ItemToPolicyValue(const net::structured_headers::Item& item, mojom::blink::PolicyValueType type)
{
    switch (type) {
    case mojom::blink::PolicyValueType::kBool: {
        if (item.is_boolean()) {
    ...
    case mojom::blink::PolicyValueType::kDecDouble:
        ...
        case net::structured_headers::Item::ItemType::kIntegerType:
            return PolicyValue::CreateDecDouble(static_cast<double>(item.GetInteger()));
...
std::optional<std::string> ItemToString(const net::structured_headers::Item& item)
{
...
void ApplyDefaultEndpoint(DocumentPolicy::ParsedDocumentPolicy& parsed_policy, const std::string& default_endpoint)
{
...
// static
std::optional<DocumentPolicy::ParsedDocumentPolicy> DocumentPolicyParser::Parse(const String& policy_string, PolicyParserMessageBuffer& logger)
{
    if (policy_string.empty())
        return std::make_optional<DocumentPolicy::ParsedDocumentPolicy>({});

    return ParseInternal(policy_string, GetDocumentPolicyNameFeatureMap(), GetDocumentPolicyFeatureInfoMap(), GetAvailableDocumentPolicyFeatures(), logger);
}

// static
std::optional<DocumentPolicy::ParsedDocumentPolicy> DocumentPolicyParser::ParseInternal(const String& policy_string,
