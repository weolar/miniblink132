...
public:
    ...
    // Returns whether or not the given feature is allowed on the origin of the
    // given URL.
    bool allowsFeature(ScriptState* script_state, const String& feature, const String& url) const;
    ...
    // URL.
    Vector<String> getAllowlistForFeature(ScriptState* script_state, const String& url) const;

    // Inform the DOMFeaturePolicy object when the container policy on its frame
    // element has changed.
    virtual void UpdateContainerPolicy(const ParsedPermissionsPolicy& container_policy = {}, scoped_refptr<const SecurityOrigin> src_origin = nullptr)
    {
    }
...
protected:
    ...
    virtual bool IsIFramePolicy() const
    {
        return false;
    }
