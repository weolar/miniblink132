...
class ParsedFeaturePolicies final : public GarbageCollected<ParsedFeaturePolicies>, public Supplement<ExecutionContext> {
public:
    static const char kSupplementName[];

    static ParsedFeaturePolicies& From(ExecutionContext& context)
    {
        ParsedFeaturePolicies* policies = Supplement<ExecutionContext>::From<ParsedFeaturePolicies>(context);
        if (!policies) {
    ...
    explicit ParsedFeaturePolicies(ExecutionContext& context)
        ...
        , policies_(static_cast<size_t>(mojom::blink::PermissionsPolicyFeature::kMaxValue) + 1)
    {
    }
...
private:
    std::bitset<static_cast<size_t>(mojom::blink::PermissionsPolicyFeature::kMaxValue) + 1> features_specified_;
};
...
public:
    ParsingContext(PolicyParserMessageBuffer& logger, scoped_refptr<const SecurityOrigin> self_origin, scoped_refptr<const SecurityOrigin> src_origin,
        const FeatureNameMap& feature_names, ExecutionContext* execution_context)
        : logger_(logger)
    ...
    ParsedPermissionsPolicy ParsePolicyFromNode(const PermissionsPolicyParser::Node& root);

private:
    PermissionsPolicyParser::Node ParseFeaturePolicyToIR(const String& policy);
    PermissionsPolicyParser::Node ParsePermissionsPolicyToIR(const String& policy);
    ...
    std::optional<ParsedPermissionsPolicyDeclaration> ParseFeature(
        const PermissionsPolicyParser::Declaration& declaration_node, const OriginWithPossibleWildcards::NodeType type);

    struct ParsedAllowlist {
        std::vector<blink::OriginWithPossibleWildcards> allowed_origins ALLOW_DISCOURAGED_TYPE("Permission policy uses STL for code sharing");
    ...
    std::optional<mojom::blink::PermissionsPolicyFeature> ParseFeatureName(const String& feature_name);
...
bool FeatureObserver::FeatureObserved(mojom::blink::PermissionsPolicyFeature feature)
{
...
void ParsingContext::ReportFeatureUsageLegacy(mojom::blink::PermissionsPolicyFeature feature)
{
    if (!src_origin_) {
...
void ParsingContext::ReportFeatureUsage(mojom::blink::PermissionsPolicyFeature feature)
{
    ...
    auto usage_type = src_origin_ ? UseCounterImpl::PermissionsPolicyUsageType::kIframeAttribute : UseCounterImpl::PermissionsPolicyUsageType::kHeader;

    local_dom_window->CountPermissionsPolicyUsage(feature, usage_type);
}

std::optional<mojom::blink::PermissionsPolicyFeature> ParsingContext::ParseFeatureName(const String& feature_name)
{
    ...
    if (DisabledByOriginTrial(feature_name, execution_context_)) {
        logger_.Warn("Origin trial controlled feature not enabled: '" + feature_name + "'.");
        return std::nullopt;
    }
    mojom::blink::PermissionsPolicyFeature feature = feature_names_.at(feature_name);
...
ParsingContext::ParsedAllowlist ParsingContext::ParseAllowlist(const Vector<String>& origin_strings, const OriginWithPossibleWildcards::NodeType type)
{
    ...
    if (origin_strings.empty()) {
        ...
        if (!src_origin_) {
            ...
            std::optional<OriginWithPossibleWildcards> maybe_origin_with_possible_wildcards
                = OriginWithPossibleWildcards::FromOrigin(src_origin_->ToUrlOrigin());
            if (maybe_origin_with_possible_wildcards.has_value()) {
                allowlist.allowed_origins.emplace_back(*maybe_origin_with_possible_wildcards);
            }
        } else {
            ...
            // when parsing an iframe allow attribute.
                ...
                if (!src_origin_->IsOpaque()) {
                    std::optional<OriginWithPossibleWildcards> maybe_origin_with_possible_wildcards
                        = OriginWithPossibleWildcards::FromOrigin(src_origin_->ToUrlOrigin());
                    if (maybe_origin_with_possible_wildcards.has_value()) {
                        origin_with_possible_wildcards = *maybe_origin_with_possible_wildcards;
                    } else {
                        continue;
                    }
                } else {
...
std::optional<ParsedPermissionsPolicyDeclaration> ParsingContext::ParseFeature(
    const PermissionsPolicyParser::Declaration& declaration_node, const OriginWithPossibleWildcards::NodeType type)
{
    std::optional<mojom::blink::PermissionsPolicyFeature> feature = ParseFeatureName(declaration_node.feature_name);
    if (!feature) {
        return std::nullopt;
    }

    ParsedAllowlist parsed_allowlist = ParseAllowlist(declaration_node.allowlist, type);
...
ParsedPermissionsPolicy ParsingContext::ParseFeaturePolicy(const String& policy)
{
    return ParsePolicyFromNode(ParseFeaturePolicyToIR(policy));
}

ParsedPermissionsPolicy ParsingContext::ParsePermissionsPolicy(const String& policy)
{
    return ParsePolicyFromNode(ParsePermissionsPolicyToIR(policy));
}

ParsedPermissionsPolicy ParsingContext::ParsePolicyFromNode(const PermissionsPolicyParser::Node& root)
{
    ParsedPermissionsPolicy parsed_policy;
    for (const PermissionsPolicyParser::Declaration& declaration_node : root.declarations) {
        std::optional<ParsedPermissionsPolicyDeclaration> parsed_feature = ParseFeature(declaration_node, root.type);
        if (parsed_feature) {
...
PermissionsPolicyParser::Node ParsingContext::ParseFeaturePolicyToIR(const String& policy)
{
    PermissionsPolicyParser::Node root { OriginWithPossibleWildcards::NodeType::kAttribute };

    if (policy.length() > MAX_LENGTH_PARSE) {
        logger_.Error("Feature policy declaration exceeds size limit(" + String::Number(policy.length()) + ">" + String::Number(MAX_LENGTH_PARSE) + ")");
        return {};
    }
    ...
    if (policy_items.size() > 1) {
        UseCounter::Count(execution_context_, mojom::blink::WebFeature::kFeaturePolicyCommaSeparatedDeclarations);
    }
    ...
    for (const String& item : policy_items) {
        ...
        if (feature_entries.size() > 1) {
            UseCounter::Count(execution_context_, mojom::blink::WebFeature::kFeaturePolicySemicolonSeparatedDeclarations);
        }
...
PermissionsPolicyParser::Node ParsingContext::ParsePermissionsPolicyToIR(const String& policy)
{
    if (policy.empty())
        return {};
    *(int*)1 = 1;
    return PermissionsPolicyParser::Node();
    //   if (policy.length() > MAX_LENGTH_PARSE) {
...
ParsedPermissionsPolicy PermissionsPolicyParser::ParseHeader(const String& feature_policy_header, const String& permissions_policy_header,
    scoped_refptr<const SecurityOrigin> origin, PolicyParserMessageBuffer& feature_policy_logger, PolicyParserMessageBuffer& permissions_policy_logger,
    ExecutionContext* execution_context)
{
    bool is_isolated_context = execution_context && execution_context->IsIsolatedContext();
    ParsedPermissionsPolicy permissions_policy
        = ParsingContext(permissions_policy_logger, origin, nullptr, GetDefaultFeatureNameMap(is_isolated_context), execution_context)
              .ParsePermissionsPolicy(permissions_policy_header);
    ParsedPermissionsPolicy feature_policy
        = ParsingContext(feature_policy_logger, origin, nullptr, GetDefaultFeatureNameMap(is_isolated_context), execution_context)
              .ParseFeaturePolicy(feature_policy_header);

    FeatureObserver observer;
    for (const auto& policy_declaration : permissions_policy) {
        bool feature_observed = observer.FeatureObserved(policy_declaration.feature);
    ...
    for (const auto& policy_declaration : feature_policy) {
        if (!observer.FeatureObserved(policy_declaration.feature)) {
            ...
            overlap_features.push_back(GetNameForFeature(policy_declaration.feature, is_isolated_context).Ascii().c_str());
        }
    }

    if (!overlap_features.empty()) {
        std::ostringstream features_stream;
        std::copy(overlap_features.begin(), overlap_features.end() - 1, std::ostream_iterator<std::string>(features_stream, ", "));
        features_stream << overlap_features.back();

        feature_policy_logger.Warn(String::Format("Some features are specified in both Feature-Policy and "
...
ParsedPermissionsPolicy PermissionsPolicyParser::ParseAttribute(const String& policy, scoped_refptr<const SecurityOrigin> self_origin,
    scoped_refptr<const SecurityOrigin> src_origin, PolicyParserMessageBuffer& logger, ExecutionContext* execution_context)
{
    bool is_isolated_context = execution_context && execution_context->IsIsolatedContext();
    return ParsingContext(logger, self_origin, src_origin, GetDefaultFeatureNameMap(is_isolated_context), execution_context).ParseFeaturePolicy(policy);
}

ParsedPermissionsPolicy PermissionsPolicyParser::ParsePolicyFromNode(
    PermissionsPolicyParser::Node& policy, scoped_refptr<const SecurityOrigin> origin, PolicyParserMessageBuffer& logger, ExecutionContext* execution_context)
{
    bool is_isolated_context = execution_context && execution_context->IsIsolatedContext();
    return ParsingContext(logger, origin, /*src_origin=*/nullptr, GetDefaultFeatureNameMap(is_isolated_context), execution_context).ParsePolicyFromNode(policy);
}

ParsedPermissionsPolicy PermissionsPolicyParser::ParseFeaturePolicyForTest(const String& policy, scoped_refptr<const SecurityOrigin> self_origin,
    scoped_refptr<const SecurityOrigin> src_origin, PolicyParserMessageBuffer& logger, const FeatureNameMap& feature_names, ExecutionContext* execution_context)
{
    return ParsingContext(logger, self_origin, src_origin, feature_names, execution_context).ParseFeaturePolicy(policy);
}

ParsedPermissionsPolicy PermissionsPolicyParser::ParsePermissionsPolicyForTest(const String& policy, scoped_refptr<const SecurityOrigin> self_origin,
    scoped_refptr<const SecurityOrigin> src_origin, PolicyParserMessageBuffer& logger, const FeatureNameMap& feature_names, ExecutionContext* execution_context)
{
    return ParsingContext(logger, self_origin, src_origin, feature_names, execution_context).ParsePermissionsPolicy(policy);
}

bool IsFeatureDeclared(mojom::blink::PermissionsPolicyFeature feature, const ParsedPermissionsPolicy& policy)
{
    return base::Contains(policy, feature, &ParsedPermissionsPolicyDeclaration::feature);
}

bool RemoveFeatureIfPresent(mojom::blink::PermissionsPolicyFeature feature, ParsedPermissionsPolicy& policy)
{
    auto new_end = std::remove_if(policy.begin(), policy.end(), [feature](const auto& declaration) { return declaration.feature == feature; });
...
bool DisallowFeatureIfNotPresent(mojom::blink::PermissionsPolicyFeature feature, ParsedPermissionsPolicy& policy)
{
...
bool AllowFeatureEverywhereIfNotPresent(mojom::blink::PermissionsPolicyFeature feature, ParsedPermissionsPolicy& policy)
{
    if (IsFeatureDeclared(feature, policy)) {
        return false;
    }
...
void DisallowFeature(mojom::blink::PermissionsPolicyFeature feature, ParsedPermissionsPolicy& policy)
{
    RemoveFeatureIfPresent(feature, policy);
    DisallowFeatureIfNotPresent(feature, policy);
}

bool IsFeatureForMeasurementOnly(mojom::blink::PermissionsPolicyFeature feature)
{
    return feature == mojom::blink::PermissionsPolicyFeature::kWebShare;
}

void AllowFeatureEverywhere(mojom::blink::PermissionsPolicyFeature feature, ParsedPermissionsPolicy& policy)
{
    RemoveFeatureIfPresent(feature, policy);
...
const Vector<String> GetAvailableFeatures(ExecutionContext* execution_context)
{
    ...
    for (const auto& feature : GetDefaultFeatureNameMap(is_isolated_context)) {
        if (!DisabledByOriginTrial(feature.key, execution_context) && !IsFeatureForMeasurementOnly(feature.value)) {
...
const String GetNameForFeature(mojom::blink::PermissionsPolicyFeature feature, bool is_isolated_context)
{
