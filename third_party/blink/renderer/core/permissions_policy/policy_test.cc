...
public:
    void SetUp() override
    {
        ...
        auto header = PermissionsPolicyParser::ParseHeader("fullscreen *; payment 'self'; midi 'none'; camera 'self' "
                                                           "https://example.com https://example.net",
            "gyroscope=(self \"https://*.example.com\" \"https://example.net\")", origin.get(), dummy_logger, dummy_logger);
        auto permissions_policy = PermissionsPolicy::CreateFromParentPolicy(nullptr, header, {}, origin->ToUrlOrigin());

        auto& security_context = page_holder_->GetFrame().DomWindow()->GetSecurityContext();
        security_context.SetSecurityOriginForTesting(origin);
        security_context.SetPermissionsPolicy(std::move(permissions_policy));
    }

    DOMFeaturePolicy* GetPolicy() const
    {
        return policy_;
    }
...
public:
    ...
    {
        PolicyTest::SetUp();
        policy_ = MakeGarbageCollected<DOMFeaturePolicy>(page_holder_->GetFrame().DomWindow());
    }
};
...
public:
    void SetUp() override
    {
        ...
        policy_ = MakeGarbageCollected<IFramePolicy>(
            page_holder_->GetFrame().DomWindow(), ParsedPermissionsPolicy(), SecurityOrigin::CreateFromString(kSelfOrigin));
    }
};
...
TEST_F(DOMFeaturePolicyTest, TestAllowsFeature)
{
    ...
    EXPECT_FALSE(GetPolicy()->allowsFeature(nullptr, "camera", "https://badorigin.com"));
    ...
    EXPECT_TRUE(GetPolicy()->allowsFeature(nullptr, "gyroscope", kOriginASubdomain));
    EXPECT_TRUE(GetPolicy()->allowsFeature(nullptr, "gyroscope", kOriginB));
    EXPECT_FALSE(GetPolicy()->allowsFeature(nullptr, "gyroscope", kOriginBSubdomain));
}

TEST_F(DOMFeaturePolicyTest, TestGetAllowList)
{
    EXPECT_THAT(GetPolicy()->getAllowlistForFeature(nullptr, "camera"), UnorderedElementsAre(kSelfOrigin, kOriginA, kOriginB));
    EXPECT_THAT(GetPolicy()->getAllowlistForFeature(nullptr, "payment"), UnorderedElementsAre(kSelfOrigin));
    EXPECT_THAT(GetPolicy()->getAllowlistForFeature(nullptr, "geolocation"), UnorderedElementsAre(kSelfOrigin));
    EXPECT_THAT(GetPolicy()->getAllowlistForFeature(nullptr, "fullscreen"), UnorderedElementsAre("*"));
    EXPECT_TRUE(GetPolicy()->getAllowlistForFeature(nullptr, "badfeature").empty());
    EXPECT_TRUE(GetPolicy()->getAllowlistForFeature(nullptr, "midi").empty());
    EXPECT_THAT(GetPolicy()->getAllowlistForFeature(nullptr, "sync-xhr"), UnorderedElementsAre("*"));
    EXPECT_THAT(GetPolicy()->getAllowlistForFeature(nullptr, "gyroscope"), UnorderedElementsAre(kSelfOrigin, kOriginB, "https://*.example.com"));
}
...
TEST_F(IFramePolicyTest, TestAllowsFeature)
{
    ...
    EXPECT_TRUE(GetPolicy()->allowsFeature(nullptr, "gyroscope"));
    EXPECT_FALSE(GetPolicy()->allowsFeature(nullptr, "gyroscope", kOriginA));
    EXPECT_FALSE(GetPolicy()->allowsFeature(nullptr, "gyroscope", kOriginASubdomain));
...
{
    EXPECT_THAT(GetPolicy()->getAllowlistForFeature(nullptr, "camera"), UnorderedElementsAre(kSelfOrigin));
    EXPECT_THAT(GetPolicy()->getAllowlistForFeature(nullptr, "payment"), UnorderedElementsAre(kSelfOrigin));
    EXPECT_THAT(GetPolicy()->getAllowlistForFeature(nullptr, "geolocation"), UnorderedElementsAre(kSelfOrigin));
    EXPECT_THAT(GetPolicy()->getAllowlistForFeature(nullptr, "fullscreen"), UnorderedElementsAre(kSelfOrigin));
    EXPECT_TRUE(GetPolicy()->getAllowlistForFeature(nullptr, "badfeature").empty());
    EXPECT_TRUE(GetPolicy()->getAllowlistForFeature(nullptr, "midi").empty());
    EXPECT_THAT(GetPolicy()->getAllowlistForFeature(nullptr, "sync-xhr"), UnorderedElementsAre("*"));
    EXPECT_THAT(GetPolicy()->getAllowlistForFeature(nullptr, "gyroscope"), UnorderedElementsAre(kSelfOrigin));
}
...
TEST_F(IFramePolicyTest, TestCrossOriginAllowedFeatures)
{
    ...
    GetPolicy()->UpdateContainerPolicy(ParsedPermissionsPolicy(), SecurityOrigin::CreateFromString(kOriginA));
    Vector<String> allowed_features = GetPolicy()->allowedFeatures(nullptr);
...
{
    PolicyParserMessageBuffer dummy_logger("", true /* discard_message */);
    ParsedPermissionsPolicy container_policy = PermissionsPolicyParser::ParseAttribute("geolocation 'src'; payment 'none'; midi; camera 'src'; gyroscope "
                                                                                       "'src'",
        SecurityOrigin::CreateFromString(kSelfOrigin), SecurityOrigin::CreateFromString(kOriginA), dummy_logger);
    GetPolicy()->UpdateContainerPolicy(container_policy, SecurityOrigin::CreateFromString(kOriginA));
    Vector<String> allowed_features = GetPolicy()->allowedFeatures(nullptr);
...
TEST_F(IFramePolicyTest, TestCombinedPolicyOnOriginASubdomain)
{
    ...
    ParsedPermissionsPolicy container_policy = PermissionsPolicyParser::ParseAttribute("geolocation 'src'; payment 'none'; midi; camera 'src'; gyroscope "
                                                                                       "'src'",
        SecurityOrigin::CreateFromString(kSelfOrigin), SecurityOrigin::CreateFromString(kOriginASubdomain), dummy_logger);
    GetPolicy()->UpdateContainerPolicy(container_policy, SecurityOrigin::CreateFromString(kOriginASubdomain));
    Vector<String> allowed_features = GetPolicy()->allowedFeatures(nullptr);
    // These features are not explicitly allowed.
    EXPECT_FALSE(allowed_features.Contains("fullscreen"));
    EXPECT_FALSE(allowed_features.Contains("payment"));
...
TEST_F(IFramePolicyTest, TestCombinedPolicyOnOriginB)
{
    ...
    ParsedPermissionsPolicy container_policy = PermissionsPolicyParser::ParseAttribute("geolocation 'src'; payment 'none'; midi; camera 'src'; gyroscope "
                                                                                       "'src'",
        SecurityOrigin::CreateFromString(kSelfOrigin), SecurityOrigin::CreateFromString(kOriginB), dummy_logger);
    GetPolicy()->UpdateContainerPolicy(container_policy, SecurityOrigin::CreateFromString(kOriginB));
    Vector<String> allowed_features = GetPolicy()->allowedFeatures(nullptr);
    // These features are not explicitly allowed.
    EXPECT_FALSE(allowed_features.Contains("fullscreen"));
    EXPECT_FALSE(allowed_features.Contains("payment"));
...
TEST_F(IFramePolicyTest, TestCombinedPolicyOnOriginBSubdomain)
{
    ...
    ParsedPermissionsPolicy container_policy = PermissionsPolicyParser::ParseAttribute("geolocation 'src'; payment 'none'; midi; camera 'src'; gyroscope "
                                                                                       "'src'",
        SecurityOrigin::CreateFromString(kSelfOrigin), SecurityOrigin::CreateFromString(kOriginBSubdomain), dummy_logger);
    GetPolicy()->UpdateContainerPolicy(container_policy, SecurityOrigin::CreateFromString(kOriginBSubdomain));
    Vector<String> allowed_features = GetPolicy()->allowedFeatures(nullptr);
    // These features are not explicitly allowed.
    EXPECT_FALSE(allowed_features.Contains("fullscreen"));
    EXPECT_FALSE(allowed_features.Contains("payment"));
