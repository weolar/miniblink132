...
public:
    ...
    static std::optional<DocumentPolicy::ParsedDocumentPolicy> Parse(const String& policy_string, PolicyParserMessageBuffer&);

    // Internal parsing method for testing.
    static std::optional<DocumentPolicy::ParsedDocumentPolicy> ParseInternal(const String& policy_string, const DocumentPolicyNameFeatureMap& name_feature_map,
        const DocumentPolicyFeatureInfoMap& feature_info_map, const DocumentPolicyFeatureSet& available_features, PolicyParserMessageBuffer&);
};
} // namespace blink
