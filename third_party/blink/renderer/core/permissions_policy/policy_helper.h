...
public:
    ...
    PolicyParserMessageBuffer()
        : discard_message_(false)
    {
    }
    explicit PolicyParserMessageBuffer(const StringView& prefix, bool discard_message = false)
        : prefix_(prefix)
        , discard_message_(discard_message)
    {
    }
    ...
    {
        if (!discard_message_) {
            message_buffer_.emplace_back(mojom::blink::ConsoleMessageLevel::kWarning, prefix_ + message);
        }
    }
    ...
    {
        if (!discard_message_) {
            message_buffer_.emplace_back(mojom::blink::ConsoleMessageLevel::kError, prefix_ + message);
        }
    }

    const Vector<Message>& GetMessages()
    {
...
struct FeatureNameMapCacheKey {
    ...
    {
        return IsIsolatedContext == other.IsIsolatedContext && is_deleted_value == other.is_deleted_value && is_empty_value == other.is_empty_value;
    }
    bool operator!=(const FeatureNameMapCacheKey& other) const
    {
        return !(*this == other);
...
// policy object.
CORE_EXPORT const FeatureNameMap GetDefaultFeatureNameMap(bool is_isolated_context);
...
template <> struct HashTraits<blink::FeatureNameMapCacheKey> : SimpleClassHashTraits<blink::FeatureNameMapCacheKey> {
