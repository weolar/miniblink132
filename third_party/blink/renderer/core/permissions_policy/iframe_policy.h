...
public:
    ...
    // a document.
    IFramePolicy(ExecutionContext* parent_context, const ParsedPermissionsPolicy& container_policy, scoped_refptr<const SecurityOrigin> src_origin)
    ...
    void UpdateContainerPolicy(const ParsedPermissionsPolicy& container_policy, scoped_refptr<const SecurityOrigin> src_origin) override
    {
        policy_ = PermissionsPolicy::CreateFromParentPolicy(context_->GetSecurityContext().GetPermissionsPolicy(),
            /*header_policy=*/ {}, container_policy, src_origin->ToUrlOrigin());
    }

protected:
    const PermissionsPolicy* GetPolicy() const override
    {
        return policy_.get();
    }

    bool IsIFramePolicy() const override
    {
        return true;
    }
