...
V8PerformanceTimingConfidenceValue::Enum GetNavigationConfidenceString(mojom::blink::ConfidenceLevel confidence)
{
    return confidence == mojom::blink::ConfidenceLevel::kHigh ? V8PerformanceTimingConfidenceValue::Enum::kHigh
...
PerformanceNavigationTiming::PerformanceNavigationTiming(
    LocalDOMWindow& window, mojom::blink::ResourceTimingInfoPtr resource_timing, base::TimeTicks time_origin)
    : PerformanceResourceTiming(std::move(resource_timing), AtomicString("navigation"), time_origin, window.CrossOriginIsolatedCapability(), &window)
...
void PerformanceNavigationTiming::OnBodyLoadFinished(int64_t encoded_body_size, int64_t decoded_body_size)
{
...
V8NavigationTimingType::Enum PerformanceNavigationTiming::GetNavigationTimingType(WebNavigationType type)
{
    switch (type) {
...
{
    DocumentLoadTiming* timing = GetDocumentLoadTiming();
    if (!AllowRedirectDetails() || !timing || !timing->CanRequestFromPreviousDocument()) {
        return 0;
    }
    return Performance::MonotonicTimeToDOMHighResTimeStamp(TimeOrigin(), timing->UnloadEventStart(), AllowNegativeValues(), CrossOriginIsolatedCapability());
}
...
DOMHighResTimeStamp PerformanceNavigationTiming::unloadEventEnd() const
{
    ...
    return Performance::MonotonicTimeToDOMHighResTimeStamp(TimeOrigin(), timing->UnloadEventEnd(), AllowNegativeValues(), CrossOriginIsolatedCapability());
}
...
DOMHighResTimeStamp PerformanceNavigationTiming::domInteractive() const
{
    ...
    return Performance::MonotonicTimeToDOMHighResTimeStamp(TimeOrigin(), timing->DomInteractive(), AllowNegativeValues(), CrossOriginIsolatedCapability());
}

DOMHighResTimeStamp PerformanceNavigationTiming::domContentLoadedEventStart() const
{
    const DocumentTiming* timing = GetDocumentTiming();
    if (!timing) {
        return 0.0;
    ...
    return Performance::MonotonicTimeToDOMHighResTimeStamp(
        TimeOrigin(), timing->DomContentLoadedEventStart(), AllowNegativeValues(), CrossOriginIsolatedCapability());
}

DOMHighResTimeStamp PerformanceNavigationTiming::domContentLoadedEventEnd() const
{
    const DocumentTiming* timing = GetDocumentTiming();
    if (!timing) {
        return 0.0;
    ...
    return Performance::MonotonicTimeToDOMHighResTimeStamp(
        TimeOrigin(), timing->DomContentLoadedEventEnd(), AllowNegativeValues(), CrossOriginIsolatedCapability());
}
...
DOMHighResTimeStamp PerformanceNavigationTiming::domComplete() const
{
    ...
    return Performance::MonotonicTimeToDOMHighResTimeStamp(TimeOrigin(), timing->DomComplete(), AllowNegativeValues(), CrossOriginIsolatedCapability());
}
...
DOMHighResTimeStamp PerformanceNavigationTiming::loadEventStart() const
{
    ...
    return Performance::MonotonicTimeToDOMHighResTimeStamp(TimeOrigin(), timing->LoadEventStart(), AllowNegativeValues(), CrossOriginIsolatedCapability());
}
...
DOMHighResTimeStamp PerformanceNavigationTiming::loadEventEnd() const
{
    ...
    return Performance::MonotonicTimeToDOMHighResTimeStamp(TimeOrigin(), timing->LoadEventEnd(), AllowNegativeValues(), CrossOriginIsolatedCapability());
}

V8NavigationTimingType PerformanceNavigationTiming::type() const
{
    if (DomWindow()) {
        return V8NavigationTimingType(GetNavigationTimingType(GetDocumentLoader()->GetNavigationType()));
    }
...
DOMHighResTimeStamp PerformanceNavigationTiming::redirectStart() const
{
    ...
    return Performance::MonotonicTimeToDOMHighResTimeStamp(TimeOrigin(), timing->RedirectStart(), AllowNegativeValues(), CrossOriginIsolatedCapability());
}
...
DOMHighResTimeStamp PerformanceNavigationTiming::redirectEnd() const
{
    ...
    return Performance::MonotonicTimeToDOMHighResTimeStamp(TimeOrigin(), timing->RedirectEnd(), AllowNegativeValues(), CrossOriginIsolatedCapability());
}
...
DOMHighResTimeStamp PerformanceNavigationTiming::fetchStart() const
{
    ...
    return Performance::MonotonicTimeToDOMHighResTimeStamp(TimeOrigin(), timing->FetchStart(), AllowNegativeValues(), CrossOriginIsolatedCapability());
}
...
DOMHighResTimeStamp PerformanceNavigationTiming::responseEnd() const
{
    ...
    return Performance::MonotonicTimeToDOMHighResTimeStamp(TimeOrigin(), timing->ResponseEnd(), AllowNegativeValues(), CrossOriginIsolatedCapability());
}
...
{
    if (DomWindow()) {
        blink::UseCounter::Count(DomWindow()->document(), WebFeature::kPerformanceNavigationTimingConfidence);
    }
    ...
    std::optional<RandomizedConfidenceValue> confidence = timing->RandomizedConfidence();
    if (!confidence) {
        return nullptr;
    }

    return MakeGarbageCollected<PerformanceTimingConfidence>(
        confidence->first, V8PerformanceTimingConfidenceValue(GetNavigationConfidenceString(confidence->second)));
}

V8NavigationEntropy PerformanceNavigationTiming::systemEntropy() const
{
    if (DomWindow()) {
        blink::UseCounter::Count(DomWindow()->document(), WebFeature::kPerformanceNavigateSystemEntropy);
    }

    return V8NavigationEntropy(GetSystemEntropy(GetDocumentLoader()));
}

DOMHighResTimeStamp PerformanceNavigationTiming::criticalCHRestart(ScriptState* script_state) const
{
    ExecutionContext::From(script_state)->CountUse(WebFeature::kCriticalCHRestartNavigationTiming);
    DocumentLoadTiming* timing = GetDocumentLoadTiming();
    if (!timing) {
        return 0.0;
    }
    return Performance::MonotonicTimeToDOMHighResTimeStamp(TimeOrigin(), timing->CriticalCHRestart(), AllowNegativeValues(), CrossOriginIsolatedCapability());
}

NotRestoredReasons* PerformanceNavigationTiming::BuildNotRestoredReasons(const mojom::blink::BackForwardCacheNotRestoredReasonsPtr& nrr) const
{
    if (!nrr) {
        return nullptr;
    }
    ...
    for (const auto& reason : nrr->reasons) {
        NotRestoredReasonDetails* detail = MakeGarbageCollected<NotRestoredReasonDetails>(reason->name);
    ...
    NotRestoredReasons* not_restored_reasons = MakeGarbageCollected<NotRestoredReasons>(
        /*src=*/nrr->src,
        /*id=*/nrr->id,
        /*name=*/nrr->name, /*url=*/url,
        /*reasons=*/reasons_to_report, nrr->same_origin_details ? &children : nullptr);
    return not_restored_reasons;
}

void PerformanceNavigationTiming::BuildJSONValue(V8ObjectBuilder& builder) const
{
    ...
    builder.AddNumber("activationStart", PerformanceNavigationTimingActivationStart::activationStart(*this));
    builder.AddNumber("criticalCHRestart", criticalCHRestart(builder.GetScriptState()));

    if (RuntimeEnabledFeatures::BackForwardCacheNotRestoredReasonsEnabled(ExecutionContext::From(builder.GetScriptState()))) {
        ...
        ExecutionContext::From(builder.GetScriptState())->CountUse(WebFeature::kBackForwardCacheNotRestoredReasons);
    }

    if (RuntimeEnabledFeatures::PerformanceNavigateSystemEntropyEnabled(ExecutionContext::From(builder.GetScriptState()))) {
        builder.AddString("systemEntropy", V8NavigationEntropy(GetSystemEntropy(GetDocumentLoader())).AsString());
    }

    if (RuntimeEnabledFeatures::PerformanceNavigationTimingConfidenceEnabled(ExecutionContext::From(builder.GetScriptState()))) {
