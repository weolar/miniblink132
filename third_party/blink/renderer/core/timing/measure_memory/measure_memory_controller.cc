...
MeasureMemoryController::MeasureMemoryController(
    base::PassKey<MeasureMemoryController>, v8::Isolate* isolate, v8::Local<v8::Context> context, ScriptPromiseResolver<MemoryMeasurement>* resolver)
...
void StartMemoryMeasurement(LocalDOMWindow* window, MeasureMemoryController* controller, WebMemoryMeasurement::Mode mode)
{
    Document* document = window->document();
    document->GetResourceCoordinator()->OnWebMemoryMeasurementRequested(
        mode, WTF::BindOnce(&MeasureMemoryController::MeasurementComplete, WrapPersistent(controller)));
}

void StartMemoryMeasurement(WorkerGlobalScope* worker, MeasureMemoryController* controller, WebMemoryMeasurement::Mode mode)
{
    DCHECK(worker->IsSharedWorkerGlobalScope() || worker->IsServiceWorkerGlobalScope());
    WebMemoryAttribution::Scope attribution_scope
        = worker->IsServiceWorkerGlobalScope() ? WebMemoryAttribution::Scope::kServiceWorker : WebMemoryAttribution::Scope::kSharedWorker;
    LocalWebMemoryMeasurer::StartMeasurement(worker->GetIsolate(), mode, controller, attribution_scope, worker->Url().GetString());
}

} // anonymous namespace

ScriptPromise<MemoryMeasurement> MeasureMemoryController::StartMeasurement(ScriptState* script_state, ExceptionState& exception_state)
{
    ExecutionContext* execution_context = ExecutionContext::From(script_state);
    DCHECK(execution_context->CrossOriginIsolatedCapability());
    ApiStatus status = CheckMeasureMemoryAvailability();
    if (status == ApiStatus::kAvailable && execution_context->IsWindow() && !IsAttached(execution_context)) {
    ...
    case ApiStatus::kNotAvailableDueToResourceCoordinator:
        exception_state.ThrowSecurityError("performance.measureUserAgentSpecificMemory is not available.");
        return EmptyPromise();
    case ApiStatus::kNotAvailableDueToDetachedContext:
        exception_state.ThrowSecurityError("performance.measureUserAgentSpecificMemory is not supported"
                                           " in detached iframes.");
        return EmptyPromise();
    ...
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<MemoryMeasurement>>(script_state);
    auto promise = resolver->Promise();

    auto measurement_mode = RuntimeEnabledFeatures::ForceEagerMeasureMemoryEnabled(ExecutionContext::From(script_state)) ? WebMemoryMeasurement::Mode::kEager
                                                                                                                         : WebMemoryMeasurement::Mode::kDefault;

    auto* impl = MakeGarbageCollected<MeasureMemoryController>(base::PassKey<MeasureMemoryController>(), isolate, context, resolver);

    if (execution_context->IsWindow()) {
        StartMemoryMeasurement(To<LocalDOMWindow>(execution_context), impl, measurement_mode);
    } else {
        StartMemoryMeasurement(To<WorkerGlobalScope>(execution_context), impl, measurement_mode);
    }
    return promise;
}
...
MemoryAttributionContainer* ConvertContainer(const WebMemoryAttributionPtr& attribution)
{
...
MemoryAttribution* ConvertAttribution(const WebMemoryAttributionPtr& attribution)
{
...
MemoryBreakdownEntry* ConvertJavaScriptBreakdown(const WebMemoryBreakdownEntryPtr& breakdown_entry)
{
...
MemoryBreakdownEntry* ConvertCanvasBreakdown(const WebMemoryBreakdownEntryPtr& breakdown_entry)
{
    auto* result = MemoryBreakdownEntry::Create();
...
MemoryBreakdownEntry* CreateUnattributedBreakdown(const WebMemoryUsagePtr& memory, const WTF::String& memory_type)
{
    auto* result = MemoryBreakdownEntry::Create();
    DCHECK(memory);
...
MemoryMeasurement* ConvertResult(const WebMemoryMeasurementPtr& measurement)
{
    ...
    breakdown.push_back(CreateUnattributedBreakdown(measurement->shared_memory, kMemoryTypeShared));
    breakdown.push_back(CreateUnattributedBreakdown(measurement->blink_memory, kMemoryTypeDom));
...
uint64_t GetDedicatedWorkerJavaScriptUkm(const WebMemoryMeasurementPtr& measurement)
{
    size_t result = 0;
    for (const auto& entry : measurement->breakdown) {
...
void RecordWebMemoryUkm(ExecutionContext* execution_context, const WebMemoryMeasurementPtr& measurement)
{
    ...
    ukm::builders::PerformanceAPI_Memory(execution_context->UkmSourceID())
        ...
        .SetJavaScript_DedicatedWorker(GetDedicatedWorkerJavaScriptUkm(measurement) / kBytesInKB)
...
void MeasureMemoryController::MeasurementComplete(WebMemoryMeasurementPtr measurement)
{
