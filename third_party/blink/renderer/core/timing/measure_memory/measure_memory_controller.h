...
class MeasureMemoryController final : public GarbageCollected<MeasureMemoryController> {
public:
    // Private constructor guarded by PassKey. Use the StartMeasurement() wrapper
    // to construct the object and start the measurement.
    MeasureMemoryController(base::PassKey<MeasureMemoryController>, v8::Isolate*, v8::Local<v8::Context>, ScriptPromiseResolver<MemoryMeasurement>*);

    ~MeasureMemoryController() = default;

    static ScriptPromise<MemoryMeasurement> StartMeasurement(ScriptState*, ExceptionState&);

    void Trace(Visitor* visitor) const;

    void MeasurementComplete(performance_manager::mojom::blink::WebMemoryMeasurementPtr);
