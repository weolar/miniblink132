...
namespace {
v8::MeasureMemoryExecution ToV8MeasureMemoryExecution(WebMemoryMeasurement::Mode mode)
{
    switch (mode) {
...
// static
void LocalWebMemoryMeasurer::StartMeasurement(v8::Isolate* isolate, WebMemoryMeasurement::Mode mode, MeasureMemoryController* controller,
    WebMemoryAttribution::Scope attribution_scope, WTF::String attribution_url)
{
    // We cannot use std::make_unique here because the constructor is private.
    auto delegate = std::unique_ptr<LocalWebMemoryMeasurer>(new LocalWebMemoryMeasurer(controller, attribution_scope, attribution_url));
    isolate->MeasureMemory(std::move(delegate), ToV8MeasureMemoryExecution(mode));
}

LocalWebMemoryMeasurer::LocalWebMemoryMeasurer(MeasureMemoryController* controller, WebMemoryAttribution::Scope attribution_scope, WTF::String attribution_url)
...
void LocalWebMemoryMeasurer::MeasurementComplete(v8::MeasureMemoryDelegate::Result result)
{
