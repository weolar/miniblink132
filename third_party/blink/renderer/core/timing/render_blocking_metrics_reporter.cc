...
const char RenderBlockingMetricsReporter::kSupplementName[] = "RenderBlockingMetricsReporter";
...
// static
RenderBlockingMetricsReporter& RenderBlockingMetricsReporter::From(Document& document)
{
    RenderBlockingMetricsReporter* supplement = Supplement<Document>::From<RenderBlockingMetricsReporter>(document);
    if (!supplement) {
...
void RenderBlockingMetricsReporter::Report()
{
    ...
    //  rendering?
    base::TimeDelta critical_font_delay = (preloaded_fonts_loaded_timestamp_ > render_blocking_resources_loaded_timestamp_)
        ? preloaded_fonts_loaded_timestamp_ - render_blocking_resources_loaded_timestamp_
        : base::TimeDelta();
    base::UmaHistogramTimes("Renderer.CriticalFonts.CriticalFontDelay", critical_font_delay);
    base::UmaHistogramTimes("Renderer.CriticalFonts.BlockingResourcesLoadTime", render_blocking_resources_loaded_timestamp_);
    base::UmaHistogramTimes("Renderer.CriticalFonts.PreloadedFontsLoadTime", preloaded_fonts_loaded_timestamp_);
}
