...
uint64_t PerformanceTiming::unloadEventStart() const
{
    ...
    if (timing->HasCrossOriginRedirect() || !timing->CanRequestFromPreviousDocument())
        return 0;
...
uint64_t PerformanceTiming::connectStart() const
{
    ...
    // the start.
    if (!timing->DomainLookupEnd().is_null() && timing->DomainLookupEnd() > connect_start) {
...
ScriptValue PerformanceTiming::toJSONForBinding(ScriptState* script_state) const
{
    V8ObjectBuilder result(script_state);
    for (const auto& name_attribute_pair : GetAttributeMapping()) {
        result.AddNumber(name_attribute_pair.key, (this->*(name_attribute_pair.value))());
    }
    return result.GetScriptValue();
}

uint64_t PerformanceTiming::MonotonicTimeToIntegerMilliseconds(base::TimeTicks time) const
{
...
// static
const PerformanceTiming::NameToAttributeMap& PerformanceTiming::GetAttributeMapping()
{
