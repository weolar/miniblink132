...
protected:
    ...
    void SimulateResolvePresentationPromise(uint64_t presentation_index, base::TimeTicks timestamp)
    {
        viz::FrameTimingDetails presentation_details;
        presentation_details.presentation_feedback.timestamp = timestamp;
        performance_->OnPresentationPromiseResolved(presentation_index, presentation_details);
    }
    ...
    {
        SimulatePaint();
        SimulateResolvePresentationPromise(performance_->event_presentation_promise_count_, timestamp);
    }

    void SimulateInteractionId(PerformanceEventTiming* entry)
    {
        ResponsivenessMetrics::EventTimestamps event_timestamps = { entry->GetEventTimingReportingInfo()->creation_time, base::TimeTicks(), base::TimeTicks(),
            entry->GetEventTimingReportingInfo()->fallback_time.has_value() ? entry->GetEventTimingReportingInfo()->fallback_time.value()
    ...
    uint64_t RegisterKeyboardEvent(
        AtomicString type, base::TimeTicks start_time, base::TimeTicks processing_start, base::TimeTicks processing_end, int key_code)
    {
        KeyboardEventInit* init = KeyboardEventInit::Create();
        init->setKeyCode(key_code);
        KeyboardEvent* keyboard_event = MakeGarbageCollected<KeyboardEvent>(type, init);
        performance_->RegisterEventTiming(*keyboard_event, keyboard_event->target(), start_time, processing_start, processing_end);
        return performance_->event_presentation_promise_count_;
    }

    void RegisterPointerEvent(AtomicString type, base::TimeTicks start_time, base::TimeTicks processing_start, base::TimeTicks processing_end,
        PointerId pointer_id, EventTarget* target = nullptr)
    {
        ...
        performance_->RegisterEventTiming(*pointer_event, pointer_event->target(), start_time, processing_start, processing_end);
    }

    PerformanceEventTiming* CreatePerformanceEventTiming(const AtomicString& name, std::optional<int> key_code, std::optional<PointerId> pointer_id,
        base::TimeTicks event_creation_timestamp, base::TimeTicks presentation_timestamp)
    {
        PerformanceEventTiming::EventTimingReportingInfo reporting_info {
            .creation_time = event_creation_timestamp, .presentation_time = presentation_timestamp, .key_code = key_code, .pointer_id = pointer_id
        };

        return PerformanceEventTiming::Create(name, reporting_info, false, nullptr, LocalDOMWindow::From(GetScriptState()));
    }

    HeapVector<Member<PerformanceEventTiming>>* GetWindowPerformanceEventTimingEntries()
    {
    ...
    String SanitizedAttribution(ExecutionContext* context, bool has_multiple_contexts, LocalFrame* observer_frame)
    {
        return WindowPerformance::SanitizedAttribution(context, has_multiple_contexts, observer_frame).first;
    }
    ...
    void ResetPerformance()
    {
        ...
        performance_->SetClocksForTesting(test_task_runner_->GetMockClock(), test_task_runner_->GetMockTickClock());
...
TEST_F(WindowPerformanceTest, SanitizedLongTaskName)
{
    ...
    EXPECT_EQ("multiple-contexts", SanitizedAttribution(GetWindow(), true, GetFrame()));
}
...
TEST_F(WindowPerformanceTest, SanitizedLongTaskName_CrossOrigin)
{
    ...
    // Attribute for same context (and same origin).
    EXPECT_EQ("cross-origin-unreachable", SanitizedAttribution(another_page.GetFrame().DomWindow(), false, GetFrame()));
}
...
TEST(PerformanceLifetimeTest, SurviveContextSwitch)
{
    ...
    page_holder->GetFrame().DomWindow()->GetSecurityContext().SetSecurityOriginForTesting(SecurityOrigin::Create(KURL(url)));

    WindowPerformance* perf = DOMWindowPerformance::performance(*page_holder->GetFrame().DomWindow());
    ...
    std::unique_ptr<WebNavigationParams> params = WebNavigationParams::CreateWithEmptyHTMLForTesting(url);
    ...
    EXPECT_EQ(perf, DOMWindowPerformance::performance(*page_holder->GetFrame().DomWindow()));
...
TEST_F(WindowPerformanceTest, EnsureEntryListOrder)
{
    ...
    auto initial_offset = test_task_runner_->NowTicks().since_origin().InSecondsF();
    test_task_runner_->FastForwardBy(GetTimeOrigin() - base::TimeTicks());

    DummyExceptionStateForTesting exception_state;
    test_task_runner_->FastForwardBy(base::Seconds(2));
    for (int i = 0; i < 8; i++) {
        performance_->mark(GetScriptState(), AtomicString::Number(i), nullptr, exception_state);
    }
    test_task_runner_->FastForwardBy(base::Seconds(2));
    for (int i = 8; i < 17; i++) {
        performance_->mark(GetScriptState(), AtomicString::Number(i), nullptr, exception_state);
    }
    PerformanceEntryVector entries = performance_->getEntriesByType(performance_entry_names::kMark);
...
TEST_F(WindowPerformanceTest, EventTimingEntryBuffering)
{
    ...
    RegisterPointerEvent(event_type_names::kClick, start_time, processing_start, processing_end, 4);
    ...
    EXPECT_EQ(1u, performance_->getBufferedEntriesByType(performance_entry_names::kEvent).size());

    page_holder_->GetFrame().Loader().GetDocumentLoader()->GetTiming().MarkLoadEventStart();
    RegisterPointerEvent(event_type_names::kClick, start_time, processing_start, processing_end, 4);
    SimulatePaintAndResolvePresentationPromise(presentation_time);
    EXPECT_EQ(2u, performance_->getBufferedEntriesByType(performance_entry_names::kEvent).size());
    ...
    RegisterPointerEvent(event_type_names::kClick, start_time, processing_start, processing_end, 4);
    SimulatePaintAndResolvePresentationPromise(presentation_time);
    EXPECT_EQ(3u, performance_->getBufferedEntriesByType(performance_entry_names::kEvent).size());
}
...
TEST_F(WindowPerformanceTest, Expose100MsEvents)
{
    ...
    RegisterPointerEvent(event_type_names::kMousedown, start_time, processing_start, processing_end, 4);

    base::TimeTicks start_time2 = start_time + base::Microseconds(200);
    RegisterPointerEvent(event_type_names::kClick, start_time2, processing_start, processing_end, 4);
    ...
    const auto& entries = performance_->getBufferedEntriesByType(performance_entry_names::kEvent);
...
TEST_F(WindowPerformanceTest, EventTimingDuration)
{
    ...
    RegisterPointerEvent(event_type_names::kClick, start_time, processing_start, processing_end, 4);
    base::TimeTicks short_presentation_time = GetTimeOrigin() + base::Milliseconds(1003);
    SimulatePaintAndResolvePresentationPromise(short_presentation_time);
    EXPECT_EQ(0u, performance_->getBufferedEntriesByType(performance_entry_names::kEvent).size());

    RegisterPointerEvent(event_type_names::kClick, start_time, processing_start, processing_end, 4);
    base::TimeTicks long_presentation_time = GetTimeOrigin() + base::Milliseconds(2000);
...
TEST_F(WindowPerformanceTest, MultipleEventsThenPresent)
{
    ...
    base::TimeTicks presentation_time = GetTimeOrigin() + base::Seconds(num_events);
    SimulatePaintAndResolvePresentationPromise(presentation_time);
    EXPECT_EQ(num_events, performance_->getBufferedEntriesByType(performance_entry_names::kEvent).size());
}

// Test the case where commit finish timestamps are recorded on all pending
// EventTimings.
TEST_F(WindowPerformanceTest, CommitFinishTimeRecordedOnAllPendingEventTimings)
{
    size_t num_events = 3;
    for (size_t i = 0; i < num_events; ++i) {
        base::TimeTicks start_time = GetTimeOrigin() + base::Seconds(i);
    ...
    for (const auto event_data : *event_timing_entries) {
        EXPECT_FALSE(event_data->GetEventTimingReportingInfo()->commit_finish_time.has_value());
    }
    ...
    for (const auto event_data : *event_timing_entries) {
        EXPECT_TRUE(event_data->GetEventTimingReportingInfo()->commit_finish_time.has_value());
        EXPECT_EQ(event_data->GetEventTimingReportingInfo()->commit_finish_time.value(), commit_finish_time);
    }
}
...
TEST_F(WindowPerformanceTest, NewCommitNotOverwritePreviousEventTimings)
{
    ...
    EXPECT_EQ(event_timing_entries->at(0)->GetEventTimingReportingInfo()->commit_finish_time, commit_finish_time);
    ...
    EXPECT_EQ(event_timing_entries->at(0)->GetEventTimingReportingInfo()->commit_finish_time, commit_finish_time);
    EXPECT_NE(event_timing_entries->at(0)->GetEventTimingReportingInfo()->commit_finish_time, commit_finish_time_1);
}
...
TEST_F(WindowPerformanceTest, FirstInput)
{
    struct {
        ...
        bool should_report;
    } inputs[] = { { event_type_names::kClick, true }, { event_type_names::kKeydown, true }, { event_type_names::kKeypress, false },
        { event_type_names::kPointerdown, false }, { event_type_names::kMousedown, true }, { event_type_names::kMouseover, false } };
    for (const auto& input : inputs) {
        // first-input does not have a |duration| threshold so use close values.
        if (input.event_type == event_type_names::kKeydown || input.event_type == event_type_names::kKeypress) {
            RegisterKeyboardEvent(input.event_type, GetTimeOrigin(), GetTimeOrigin() + base::Milliseconds(1), GetTimeOrigin() + base::Milliseconds(2), 4);
        } else {
            RegisterPointerEvent(input.event_type, GetTimeOrigin(), GetTimeOrigin() + base::Milliseconds(1), GetTimeOrigin() + base::Milliseconds(2), 4);
        }
        SimulatePaintAndResolvePresentationPromise(GetTimeOrigin() + base::Milliseconds(3));
        PerformanceEntryVector firstInputs = performance_->getEntriesByType(performance_entry_names::kFirstInput);
...
{
    AtomicString several_events[] = { event_type_names::kMouseover, event_type_names::kMousedown, event_type_names::kPointerup };
    for (const auto& event : several_events) {
        RegisterPointerEvent(event, GetTimeOrigin(), GetTimeOrigin() + base::Milliseconds(1), GetTimeOrigin() + base::Milliseconds(2), 4);
        SimulatePaintAndResolvePresentationPromise(GetTimeOrigin() + base::Milliseconds(3));
    }
    ASSERT_EQ(1u, performance_->getEntriesByType(performance_entry_names::kFirstInput).size());
    EXPECT_EQ(event_type_names::kMousedown, performance_->getEntriesByType(performance_entry_names::kFirstInput)[0]->name());
}
...
TEST_F(WindowPerformanceTest, FirstPointerUp)
{
    ...
    RegisterPointerEvent(event_type_names::kPointerdown, start_time, processing_start, processing_end, 4);
    SimulatePaintAndResolvePresentationPromise(presentation_time);
    EXPECT_EQ(0u, performance_->getEntriesByType(performance_entry_names::kFirstInput).size());
    RegisterPointerEvent(event_type_names::kPointerup, start_time, processing_start, processing_end, 4);
    SimulatePaintAndResolvePresentationPromise(presentation_time);
    EXPECT_EQ(1u, performance_->getEntriesByType(performance_entry_names::kFirstInput).size());
    // The name of the entry should be event_type_names::kPointerdown.
    EXPECT_EQ(1u, performance_->getEntriesByName(event_type_names::kPointerdown, performance_entry_names::kFirstInput).size());
}
...
TEST_F(WindowPerformanceTest, PointerdownOptimizedOut)
{
    ...
    EXPECT_EQ(1u, performance_->getEntriesByName(event_type_names::kMousedown, performance_entry_names::kFirstInput).size());
}
...
TEST_F(WindowPerformanceTest, OneKeyboardInteraction)
{
    ...
    int key_code = 2;
    RegisterKeyboardEvent(event_type_names::kKeydown, keydown_timestamp, processing_start_keydown, processing_end_keydown, key_code);
    ...
    RegisterKeyboardEvent(event_type_names::kKeyup, keyup_timestamp, processing_start_keyup, processing_end_keyup, key_code);
    ...
    // Check UKM recording.
    auto entries = GetUkmRecorder()->GetEntriesByName(ukm::builders::Responsiveness_UserInteraction::kEntryName);
    EXPECT_EQ(1u, entries.size());
    const ukm::mojom::UkmEntry* ukm_entry = entries[0];
    GetUkmRecorder()->ExpectEntryMetric(ukm_entry, ukm::builders::Responsiveness_UserInteraction::kMaxEventDurationName, 7);
    GetUkmRecorder()->ExpectEntryMetric(ukm_entry, ukm::builders::Responsiveness_UserInteraction::kTotalEventDurationName, 10);
    GetUkmRecorder()->ExpectEntryMetric(ukm_entry, ukm::builders::Responsiveness_UserInteraction::kInteractionTypeName, 0);

    // Check UMA recording.
    GetHistogramTester().ExpectTotalCount("Blink.Responsiveness.UserInteraction.MaxEventDuration.AllTypes", 1);
    GetHistogramTester().ExpectTotalCount("Blink.Responsiveness.UserInteraction.MaxEventDuration.Keyboard", 1);
    GetHistogramTester().ExpectTotalCount("Blink.Responsiveness.UserInteraction.MaxEventDuration.TapOrClick", 0);
    GetHistogramTester().ExpectTotalCount("Blink.Responsiveness.UserInteraction.MaxEventDuration.Drag", 0);
}
...
TEST_F(WindowPerformanceTest, HoldingDownAKey)
{
    ...
    // Check UKM recording.
    entries = GetUkmRecorder()->GetEntriesByName(ukm::builders::Responsiveness_UserInteraction::kEntryName);
    ...
    for (std::size_t i = 0; i < entries.size(); ++i) {
        ...
        GetUkmRecorder()->ExpectEntryMetric(entry, ukm::builders::Responsiveness_UserInteraction::kMaxEventDurationName, expected_durations[i].first);
        GetUkmRecorder()->ExpectEntryMetric(entry, ukm::builders::Responsiveness_UserInteraction::kTotalEventDurationName, expected_durations[i].second);
        GetUkmRecorder()->ExpectEntryMetric(entry, ukm::builders::Responsiveness_UserInteraction::kInteractionTypeName, 0);
    }

    // Check UMA recording.
    GetHistogramTester().ExpectTotalCount("Blink.Responsiveness.UserInteraction.MaxEventDuration.AllTypes", 3);
    GetHistogramTester().ExpectTotalCount("Blink.Responsiveness.UserInteraction.MaxEventDuration.Keyboard", 3);
    GetHistogramTester().ExpectTotalCount("Blink.Responsiveness.UserInteraction.MaxEventDuration.TapOrClick", 0);
    GetHistogramTester().ExpectTotalCount("Blink.Responsiveness.UserInteraction.MaxEventDuration.Drag", 0);
}
...
TEST_F(WindowPerformanceTest, PressMultipleKeys)
{
    ...
    RegisterKeyboardEvent(event_type_names::kKeydown, keydown_timestamp, processing_start_keydown, processing_end_keydown, first_key_code);
    SimulatePaintAndResolvePresentationPromise(presentation_time_keydown);
    ...
    RegisterKeyboardEvent(event_type_names::kKeydown, keydown_timestamp, processing_start_keydown, processing_end_keydown, second_key_code);
    SimulatePaintAndResolvePresentationPromise(presentation_time_keydown);
    ...
    base::TimeTicks processing_end_keyup = GetTimeStamp(6);
    base::TimeTicks presentation_time_keyup = GetTimeStamp(13);
    RegisterKeyboardEvent(event_type_names::kKeyup, keyup_timestamp, processing_start_keyup, processing_end_keyup, first_key_code);
    SimulatePaintAndResolvePresentationPromise(presentation_time_keyup);
    ...
    RegisterKeyboardEvent(event_type_names::kKeyup, keyup_timestamp, processing_start_keyup, processing_end_keyup, second_key_code);
    SimulatePaintAndResolvePresentationPromise(presentation_time_keyup);
...
TEST_F(WindowPerformanceTest, KeyupFinishLastButCallbackInvokedFirst)
{
    ...
    const uint64_t presentation_index_keydown
        = RegisterKeyboardEvent(event_type_names::kKeydown, keydown_timestamp, processing_start_keydown, processing_end_keydown, digit_1_key_code);
    ...
    const uint64_t presentation_index_keyup
        = RegisterKeyboardEvent(event_type_names::kKeyup, keyup_timestamp, processing_start_keyup, processing_end_keyup, digit_1_key_code);

    // keyup resolved without a paint, due to no damage.
    SimulateResolvePresentationPromise(presentation_index_keyup, presentation_promise_break_time_keyup);
    SimulateResolvePresentationPromise(presentation_index_keydown, presentation_time_keydown);
    ...
    const ukm::mojom::UkmEntry* ukm_entry = entries[0];
    GetUkmRecorder()->ExpectEntryMetric(ukm_entry, ukm::builders::Responsiveness_UserInteraction::kMaxEventDurationName, 7);
    GetUkmRecorder()->ExpectEntryMetric(ukm_entry, ukm::builders::Responsiveness_UserInteraction::kTotalEventDurationName, 8);
    GetUkmRecorder()->ExpectEntryMetric(ukm_entry, ukm::builders::Responsiveness_UserInteraction::kInteractionTypeName, 0);
...
TEST_F(WindowPerformanceTest, TapOrClick)
{
    ...
    RegisterPointerEvent(event_type_names::kPointerdown, pointerdown_timestamp, processing_start_pointerdown, processing_end_pointerdown, pointer_id);
    ...
    RegisterPointerEvent(event_type_names::kPointerup, pointerup_timestamp, processing_start_pointerup, processing_end_pointerup, pointer_id);
    ...
    RegisterPointerEvent(event_type_names::kClick, click_timestamp, processing_start_click, processing_end_click, pointer_id);
    ...
    const ukm::mojom::UkmEntry* ukm_entry = entries[0];
    GetUkmRecorder()->ExpectEntryMetric(ukm_entry, ukm::builders::Responsiveness_UserInteraction::kMaxEventDurationName, 7);
    GetUkmRecorder()->ExpectEntryMetric(ukm_entry, ukm::builders::Responsiveness_UserInteraction::kTotalEventDurationName, 17);
    GetUkmRecorder()->ExpectEntryMetric(ukm_entry, ukm::builders::Responsiveness_UserInteraction::kInteractionTypeName, 1);

    // Check UMA recording.
    GetHistogramTester().ExpectTotalCount("Blink.Responsiveness.UserInteraction.MaxEventDuration.AllTypes", 1);
    GetHistogramTester().ExpectTotalCount("Blink.Responsiveness.UserInteraction.MaxEventDuration.Keyboard", 0);
    GetHistogramTester().ExpectTotalCount("Blink.Responsiveness.UserInteraction.MaxEventDuration.TapOrClick", 1);
    GetHistogramTester().ExpectTotalCount("Blink.Responsiveness.UserInteraction.MaxEventDuration.Drag", 0);
}
...
TEST_F(WindowPerformanceTest, PageVisibilityChanged)
{
    ...
    performance_->GetPage()->SetVisibilityState(mojom::blink::PageVisibilityState::kHidden, true);
    PageVisibilityChanged(GetTimeStamp(18));

    SimulatePaintAndResolvePresentationPromise(presentation_time_pointerup_and_click);
    ...
    GetUkmRecorder()->ExpectEntryMetric(ukm_entry, ukm::builders::Responsiveness_UserInteraction::kMaxEventDurationName, 5);
    ...
    GetUkmRecorder()->ExpectEntryMetric(ukm_entry, ukm::builders::Responsiveness_UserInteraction::kTotalEventDurationName, 9);
    GetUkmRecorder()->ExpectEntryMetric(ukm_entry, ukm::builders::Responsiveness_UserInteraction::kInteractionTypeName, 1);
...
TEST_F(WindowPerformanceTest, Drag)
{
    ...
    base::TimeTicks presentation_time_pointerdown = GetTimeStamp(5);
    PointerId pointer_id = 4;
    RegisterPointerEvent(event_type_names::kPointerdown, pointerdwon_timestamp, processing_start_pointerdown, processing_end_pointerdown, pointer_id);
    SimulatePaintAndResolvePresentationPromise(presentation_time_pointerdown);
    // Notify drag.
    ...
    GetUkmRecorder()->ExpectEntryMetric(ukm_entry, ukm::builders::Responsiveness_UserInteraction::kMaxEventDurationName, 7);
    GetUkmRecorder()->ExpectEntryMetric(ukm_entry, ukm::builders::Responsiveness_UserInteraction::kTotalEventDurationName, 17);
    GetUkmRecorder()->ExpectEntryMetric(ukm_entry, ukm::builders::Responsiveness_UserInteraction::kInteractionTypeName, 2);

    // Check UMA recording.
    GetHistogramTester().ExpectTotalCount("Blink.Responsiveness.UserInteraction.MaxEventDuration.AllTypes", 1);
    GetHistogramTester().ExpectTotalCount("Blink.Responsiveness.UserInteraction.MaxEventDuration.Keyboard", 0);
    GetHistogramTester().ExpectTotalCount("Blink.Responsiveness.UserInteraction.MaxEventDuration.TapOrClick", 0);
    GetHistogramTester().ExpectTotalCount("Blink.Responsiveness.UserInteraction.MaxEventDuration.Drag", 1);
}
...
TEST_F(WindowPerformanceTest, Scroll)
{
    ...
    RegisterPointerEvent(event_type_names::kPointerdown, pointerdown_timestamp, processing_start_keydown, processing_end_keydown, pointer_id);
    SimulatePaintAndResolvePresentationPromise(presentation_time_keydown);
    // Pointercancel
    base::TimeTicks pointerup_timestamp = GetTimeStamp(3);
    base::TimeTicks processing_start_keyup = GetTimeStamp(5);
    base::TimeTicks processing_end_keyup = GetTimeStamp(6);
    base::TimeTicks presentation_time_keyup = GetTimeStamp(10);
    RegisterPointerEvent(event_type_names::kPointercancel, pointerup_timestamp, processing_start_keyup, processing_end_keyup, pointer_id);
    SimulatePaintAndResolvePresentationPromise(presentation_time_keyup);
    ...
    // Check UMA recording.
    GetHistogramTester().ExpectTotalCount("Blink.Responsiveness.UserInteraction.MaxEventDuration.AllTypes", 0);
    GetHistogramTester().ExpectTotalCount("Blink.Responsiveness.UserInteraction.MaxEventDuration.Keyboard", 0);
    GetHistogramTester().ExpectTotalCount("Blink.Responsiveness.UserInteraction.MaxEventDuration.TapOrClick", 0);
    GetHistogramTester().ExpectTotalCount("Blink.Responsiveness.UserInteraction.MaxEventDuration.Drag", 0);
}
...
TEST_F(WindowPerformanceTest, ArtificialPointerupOrClick)
{
    ...
    EXPECT_EQ(1u, entries.size());
    const ukm::mojom::UkmEntry* ukm_entry = entries[0];
    GetUkmRecorder()->ExpectEntryMetric(ukm_entry, ukm::builders::Responsiveness_UserInteraction::kMaxEventDurationName, 12);
    GetUkmRecorder()->ExpectEntryMetric(ukm_entry, ukm::builders::Responsiveness_UserInteraction::kTotalEventDurationName, 12);
    GetUkmRecorder()->ExpectEntryMetric(ukm_entry, ukm::builders::Responsiveness_UserInteraction::kInteractionTypeName, 1);
...
TEST_F(WindowPerformanceTest, PerformanceMarkTraceEvent)
{
    ...
    performance_->mark(GetScriptState(), AtomicString("test_trace"), nullptr, exception_state);
...
TEST_F(WindowPerformanceTest, ElementTimingTraceEvent)
{
    ...
    performance_->AddElementTiming(AtomicString("image-paint"), "url", gfx::RectF(10, 20, 30, 40), GetTimeStamp(2000), GetTimeStamp(1000),
...
TEST_F(WindowPerformanceTest, EventTimingTraceEvents)
{
    ...
    base::TimeTicks processing_end = processing_start + base::Milliseconds(10);
    RegisterPointerEvent(event_type_names::kPointerdown, start_time, processing_start, processing_end, 4, GetWindow()->document());
    ...
    RegisterPointerEvent(event_type_names::kPointerup, start_time2, processing_start2, processing_end2, 4, GetWindow()->document());
    ...
    RegisterPointerEvent(event_type_names::kClick, start_time3, processing_start3, processing_end3, 4, GetWindow()->document());
    ...
    trace_analyzer::TraceEventVector events;
    Query q = Query::EventNameIs("EventTiming") && Query::EventPhaseIs(TRACE_EVENT_PHASE_NESTABLE_ASYNC_BEGIN);
    analyzer->FindEvents(q, &events);
    ...
    EXPECT_EQ(arg_dict.FindInt("nodeId"), DOMNodeIds::IdForNode(GetWindow()->document()));
    ASSERT_TRUE(pointerdown_begin->has_other_event());
    EXPECT_EQ(base::ClampRound(pointerdown_begin->GetAbsTimeToOtherEvent()), 25000);
...
TEST_F(WindowPerformanceTest, SlowInteractionToNextPaintTraceEvents)
{
    ...
    // Short, untraced keyboard event.
    {
        ...
        RegisterKeyboardEvent(event_type_names::kKeydown, keydown_timestamp, processing_start_keydown, processing_end_keydown, kKeyCode);
        SimulatePaintAndResolvePresentationPromise(presentation_time_keydown);
        ...
        RegisterKeyboardEvent(event_type_names::kKeyup, keyup_timestamp, processing_start_keyup, processing_end_keyup, kKeyCode);
        SimulatePaintAndResolvePresentationPromise(presentation_time_keyup);
    }
    ...
    // Overlapping events.
    {
        ...
        RegisterKeyboardEvent(event_type_names::kKeydown, keydown_timestamp2, processing_start_keydown2, processing_end_keydown2, kKeyCode);
        ...
        RegisterKeyboardEvent(event_type_names::kKeyup, keyup_timestamp2, processing_start_keyup2, processing_end_keyup2, kKeyCode);
    ...
    trace_analyzer::TraceEventVector events;
    Query q = Query::EventNameIs("SlowInteractionToNextPaint") && Query::EventPhaseIs(TRACE_EVENT_PHASE_NESTABLE_ASYNC_BEGIN);
    analyzer->FindEvents(q, &events);
...
TEST_F(WindowPerformanceTest, InteractionID)
{
    ...
    PerformanceEventTiming* keydown_entry = CreatePerformanceEventTiming(event_type_names::kKeydown, 1, std::nullopt, GetTimeStamp(100), GetTimeStamp(120));
    SimulateInteractionId(keydown_entry);
    PerformanceEventTiming* keyup_entry = CreatePerformanceEventTiming(event_type_names::kKeyup, 1, std::nullopt, GetTimeStamp(115), GetTimeStamp(140));
    ...
    PerformanceEventTiming* pointerdown_entry
        = CreatePerformanceEventTiming(event_type_names::kPointerdown, std::nullopt, pointer_id_1, GetTimeStamp(100), GetTimeStamp(120));
    SimulateInteractionId(pointerdown_entry);
    PerformanceEventTiming* pointerup_entry
        = CreatePerformanceEventTiming(event_type_names::kPointerup, std::nullopt, pointer_id_1, GetTimeStamp(130), GetTimeStamp(150));
    SimulateInteractionId(pointerup_entry);
    PerformanceEventTiming* click_entry
        = CreatePerformanceEventTiming(event_type_names::kClick, std::nullopt, pointer_id_1, GetTimeStamp(130), GetTimeStamp(200));
    SimulateInteractionId(click_entry);
    EXPECT_GT(pointerdown_entry->interactionId(), 0u);
    EXPECT_EQ(pointerdown_entry->interactionId(), pointerup_entry->interactionId());
    ...
    pointerdown_entry = CreatePerformanceEventTiming(event_type_names::kPointerdown, std::nullopt, pointer_id_2, GetTimeStamp(150), GetTimeStamp(200));
    SimulateInteractionId(pointerdown_entry);
    performance_->NotifyPotentialDrag(20);
    pointerup_entry = CreatePerformanceEventTiming(event_type_names::kPointerup, std::nullopt, pointer_id_2, GetTimeStamp(200), GetTimeStamp(230));
    SimulateInteractionId(pointerup_entry);
    EXPECT_GT(pointerdown_entry->interactionId(), 0u);
    EXPECT_EQ(pointerdown_entry->interactionId(), pointerup_entry->interactionId());

    // Scroll should not be reported in ukm.
    pointerdown_entry = CreatePerformanceEventTiming(event_type_names::kPointerdown, std::nullopt, pointer_id_2, GetTimeStamp(300), GetTimeStamp(315));
    SimulateInteractionId(pointerdown_entry);
    PerformanceEventTiming* pointercancel_entry
        = CreatePerformanceEventTiming(event_type_names::kPointercancel, std::nullopt, pointer_id_2, GetTimeStamp(310), GetTimeStamp(330));
    ...
    } expected_ukm[] = { { 25, 40, UserInteractionType::kKeyboard }, { 70, 90, UserInteractionType::kTapOrClick }, { 50, 80, UserInteractionType::kDrag } };
    auto entries = GetUkmRecorder()->GetEntriesByName(ukm::builders::Responsiveness_UserInteraction::kEntryName);
    EXPECT_EQ(3u, entries.size());
    for (size_t i = 0; i < entries.size(); ++i) {
        const ukm::mojom::UkmEntry* ukm_entry = entries[i];
        GetUkmRecorder()->ExpectEntryMetric(ukm_entry, ukm::builders::Responsiveness_UserInteraction::kMaxEventDurationName, expected_ukm[i].max_duration);
        GetUkmRecorder()->ExpectEntryMetric(ukm_entry, ukm::builders::Responsiveness_UserInteraction::kTotalEventDurationName, expected_ukm[i].total_duration);
        GetUkmRecorder()->ExpectEntryMetric(
            ukm_entry, ukm::builders::Responsiveness_UserInteraction::kInteractionTypeName, static_cast<int>(expected_ukm[i].type));
    }
}
...
public:
    struct EventForInteraction {
        EventForInteraction(const AtomicString& name, std::optional<int> key_code, std::optional<PointerId> pointer_id,
            base::TimeTicks event_timestamp = base::TimeTicks(), base::TimeTicks presentation_timestamp = base::TimeTicks())
            : name_(name)
            , key_code_(key_code)
    ...
    std::vector<uint32_t> SimulateInteractionIds(const std::vector<EventForInteraction>& events)
    {
        // Store the entries first and record interactionIds at the end.
        HeapVector<Member<PerformanceEventTiming>> entries;
        for (const auto& event : events) {
            PerformanceEventTiming* entry
                = CreatePerformanceEventTiming(event.name_, event.key_code_, event.pointer_id_, event.event_timestamp_, event.presentation_timestamp_);
    ...
    void CheckUKMValues(const std::vector<ExpectedUkmValue>& expected_ukms)
    {
        ...
        for (size_t i = 0; i < entries.size(); ++i) {
            const ukm::mojom::UkmEntry* ukm_entry = entries[i];
            GetUkmRecorder()->ExpectEntryMetric(
                ukm_entry, ukm::builders::Responsiveness_UserInteraction::kMaxEventDurationName, expected_ukms[i].max_duration_);
            GetUkmRecorder()->ExpectEntryMetric(
                ukm_entry, ukm::builders::Responsiveness_UserInteraction::kTotalEventDurationName, expected_ukms[i].total_duration_);
            GetUkmRecorder()->ExpectEntryMetric(
                ukm_entry, ukm::builders::Responsiveness_UserInteraction::kInteractionTypeName, static_cast<int>(expected_ukms[i].interaction_type_));
        }
    }
};
...
TEST_F(InteractionIdTest, InputOutsideComposition)
{
    ...
    std::vector<EventForInteraction> events1 = { { event_type_names::kKeydown, 65, std::nullopt, GetTimeStamp(100), GetTimeStamp(150) },
        { event_type_names::kInput, std::nullopt, std::nullopt, GetTimeStamp(120), GetTimeStamp(220) },
        { event_type_names::kKeyup, 65, std::nullopt, GetTimeStamp(130), GetTimeStamp(150) } };
    ...
    std::vector<EventForInteraction> events2 = { { event_type_names::kKeydown, 53, std::nullopt, GetTimeStamp(200), GetTimeStamp(220) },
        { event_type_names::kInput, std::nullopt, std::nullopt, GetTimeStamp(220), GetTimeStamp(320) },
        { event_type_names::kKeyup, 53, std::nullopt, GetTimeStamp(250), GetTimeStamp(290) } };
    ...
    EXPECT_NE(ids1[0], ids2[0]) << "First and second keydown have different interactionId";

    // Backspace with max duration of 25 and total of 25.
    std::vector<EventForInteraction> events3 = { { event_type_names::kKeydown, 8, std::nullopt, GetTimeStamp(300), GetTimeStamp(320) },
        { event_type_names::kInput, std::nullopt, std::nullopt, GetTimeStamp(300), GetTimeStamp(400) },
        { event_type_names::kKeyup, 8, std::nullopt, GetTimeStamp(300), GetTimeStamp(325) } };
    ...
    EXPECT_NE(ids1[0], ids3[0]) << "First and third keydown have different interactionId";
    EXPECT_NE(ids2[0], ids3[0]) << "Second and third keydown have different interactionId";

    CheckUKMValues({ { 50, 50, UserInteractionType::kKeyboard }, { 40, 60, UserInteractionType::kKeyboard }, { 25, 25, UserInteractionType::kKeyboard } });
}
...
TEST_F(InteractionIdTest, CompositionSingleKeydown)
{
    ...
    std::vector<EventForInteraction> events1 = { { event_type_names::kKeydown, 229, std::nullopt, GetTimeStamp(100), GetTimeStamp(200) },
        { event_type_names::kCompositionstart, std::nullopt, std::nullopt }, { event_type_names::kCompositionupdate, std::nullopt, std::nullopt },
        { event_type_names::kInput, std::nullopt, std::nullopt, GetTimeStamp(120), GetTimeStamp(140) },
        { event_type_names::kKeyup, 65, std::nullopt, GetTimeStamp(120), GetTimeStamp(220) } };
    std::vector<uint32_t> ids1 = SimulateInteractionIds(events1);

    // Insert "b" and finish composition with a duration of 30.
    std::vector<EventForInteraction> events2 = { { event_type_names::kKeydown, 229, std::nullopt, GetTimeStamp(200), GetTimeStamp(300) },
        { event_type_names::kCompositionupdate, std::nullopt, std::nullopt },
        { event_type_names::kInput, std::nullopt, std::nullopt, GetTimeStamp(230), GetTimeStamp(260) },
        { event_type_names::kKeyup, 66, std::nullopt, GetTimeStamp(270), GetTimeStamp(370) },
    ...
    EXPECT_EQ(ids1[0], ids1[3]) << "Keydown and Input have the same interactionIds";
    ...
    EXPECT_EQ(ids2[0], ids2[2]) << "Keydown and Input have the same interactionIds";
    EXPECT_NE(ids1[3], ids2[2]) << "First and second inputs have different interactionIds";

    CheckUKMValues({ { 100, 120, UserInteractionType::kKeyboard }, { 100, 170, UserInteractionType::kKeyboard } });
}
...
TEST_F(InteractionIdTest, CompositionToFinalInput)
{
    ...
    std::vector<EventForInteraction> events1 = { { event_type_names::kKeydown, 229, std::nullopt, GetTimeStamp(100), GetTimeStamp(190) },
        { event_type_names::kCompositionstart, std::nullopt, std::nullopt }, { event_type_names::kCompositionupdate, std::nullopt, std::nullopt },
        { event_type_names::kInput, std::nullopt, std::nullopt, GetTimeStamp(100), GetTimeStamp(125) },
        { event_type_names::kKeyup, 65, std::nullopt, GetTimeStamp(110), GetTimeStamp(190) } };
    std::vector<uint32_t> ids1 = SimulateInteractionIds(events1);
    EXPECT_GT(ids1[3], 0u) << "First input nonzero";

    // Insert "b" with a duration of 35.
    std::vector<EventForInteraction> events2 = { { event_type_names::kKeydown, 229, std::nullopt, GetTimeStamp(200), GetTimeStamp(290) },
        { event_type_names::kCompositionupdate, std::nullopt, std::nullopt },
        { event_type_names::kInput, std::nullopt, std::nullopt, GetTimeStamp(220), GetTimeStamp(255) },
        { event_type_names::kKeyup, 66, std::nullopt, GetTimeStamp(210), GetTimeStamp(290) } };
    std::vector<uint32_t> ids2 = SimulateInteractionIds(events2);
    EXPECT_GT(ids2[2], 0u) << "Second input nonzero";
    EXPECT_NE(ids1[3], ids2[2]) << "First and second input have different interactionIds";

    // Select a composed input and finish, with a duration of 140.
    std::vector<EventForInteraction> events3 = { { event_type_names::kCompositionupdate, std::nullopt, std::nullopt },
        { event_type_names::kInput, std::nullopt, std::nullopt, GetTimeStamp(300), GetTimeStamp(440) },
        { event_type_names::kCompositionend, std::nullopt, std::nullopt } };
    std::vector<uint32_t> ids3 = SimulateInteractionIds(events3);
    EXPECT_EQ(ids3[2], 0u) << "Compositionend has zero interactionId";
    EXPECT_GT(ids3[1], 0u) << "Third input has nonzero interactionId";
    EXPECT_NE(ids1[3], ids3[1]) << "First and third inputs have different interactionIds";
    EXPECT_NE(ids2[2], ids3[1]) << "Second and third inputs have different interactionIds";

    performance_->GetResponsivenessMetrics().FlushAllEventsForTesting();

    CheckUKMValues({ { 90, 90, UserInteractionType::kKeyboard }, { 90, 90, UserInteractionType::kKeyboard }, { 140, 140, UserInteractionType::kKeyboard } });
}
...
TEST_F(InteractionIdTest, CompositionToFinalInputMultipleKeyUps)
{
    ...
    std::vector<EventForInteraction> events1 = { { event_type_names::kKeydown, 229, std::nullopt, GetTimeStamp(0), GetTimeStamp(100) },
        { event_type_names::kCompositionstart, std::nullopt, std::nullopt }, { event_type_names::kCompositionupdate, std::nullopt, std::nullopt },
        { event_type_names::kInput, std::nullopt, std::nullopt, GetTimeStamp(0), GetTimeStamp(66) },
        { event_type_names::kKeyup, 229, std::nullopt, GetTimeStamp(0), GetTimeStamp(100) },
        { event_type_names::kKeyup, 65, std::nullopt, GetTimeStamp(0), GetTimeStamp(100) } };
    std::vector<uint32_t> ids1 = SimulateInteractionIds(events1);

    // Insert "b" with a duration of 51.
    std::vector<EventForInteraction> events2 = { { event_type_names::kKeydown, 229, std::nullopt, GetTimeStamp(200), GetTimeStamp(300) },
        { event_type_names::kCompositionupdate, std::nullopt, std::nullopt },
        { event_type_names::kInput, std::nullopt, std::nullopt, GetTimeStamp(200), GetTimeStamp(251) },
        { event_type_names::kKeyup, 229, std::nullopt, GetTimeStamp(200), GetTimeStamp(300) },
        { event_type_names::kKeyup, 66, std::nullopt, GetTimeStamp(200), GetTimeStamp(300) } };
    std::vector<uint32_t> ids2 = SimulateInteractionIds(events2);

    // Select a composed input and finish, with duration of 85.
    std::vector<EventForInteraction> events3 = { { event_type_names::kCompositionupdate, std::nullopt, std::nullopt },
        { event_type_names::kInput, std::nullopt, std::nullopt, GetTimeStamp(300), GetTimeStamp(385) },
        { event_type_names::kCompositionend, std::nullopt, std::nullopt } };
    std::vector<uint32_t> ids3 = SimulateInteractionIds(events3);
    ...
    EXPECT_NE(ids1[3], ids3[1]) << "First and third inputs have different interactionIds";
    EXPECT_NE(ids2[2], ids3[1]) << "Second and third inputs have different interactionIds";
    CheckUKMValues({ { 100, 100, UserInteractionType::kKeyboard }, { 100, 100, UserInteractionType::kKeyboard }, { 85, 85, UserInteractionType::kKeyboard } });
}
...
TEST_F(InteractionIdTest, SmartSuggestion)
{
    ...
    std::vector<EventForInteraction> events1 = { { event_type_names::kKeydown, 229, std::nullopt, GetTimeStamp(0), GetTimeStamp(16) },
        { event_type_names::kCompositionstart, std::nullopt, std::nullopt }, { event_type_names::kCompositionupdate, std::nullopt, std::nullopt },
        { event_type_names::kInput, std::nullopt, std::nullopt, GetTimeStamp(0), GetTimeStamp(9) },
        { event_type_names::kKeyup, 229, std::nullopt, GetTimeStamp(0), GetTimeStamp(16) } };
    std::vector<uint32_t> ids1 = SimulateInteractionIds(events1);

    // Compose to "At" with a duration of 14.
    std::vector<EventForInteraction> events2 = { { event_type_names::kCompositionupdate, std::nullopt, std::nullopt },
        { event_type_names::kInput, std::nullopt, std::nullopt, GetTimeStamp(100), GetTimeStamp(114) },
        { event_type_names::kCompositionend, std::nullopt, std::nullopt } };
    std::vector<uint32_t> ids2 = SimulateInteractionIds(events2);

    // Add "the". No composition so need to consider the keydown and keyup.
    // Max duration of 43 and total duration of 70
    std::vector<EventForInteraction> events3 = { { event_type_names::kKeydown, 229, std::nullopt, GetTimeStamp(200), GetTimeStamp(243) },
        { event_type_names::kInput, std::nullopt, std::nullopt, GetTimeStamp(200), GetTimeStamp(300) },
        { event_type_names::kKeyup, 229, std::nullopt, GetTimeStamp(235), GetTimeStamp(270) } };
    std::vector<uint32_t> ids3 = SimulateInteractionIds(events3);
    ...
    EXPECT_NE(ids1[3], ids2[1]) << "First and second input have different interactionIds";
    ...
    CheckUKMValues({ { 16, 16, UserInteractionType::kKeyboard }, { 14, 14, UserInteractionType::kKeyboard }, { 43, 70, UserInteractionType::kKeyboard } });
}

TEST_F(InteractionIdTest, TapWithoutClick)
{
    std::vector<EventForInteraction> events = { { event_type_names::kPointerdown, std::nullopt, 1, GetTimeStamp(100), GetTimeStamp(140) },
        { event_type_names::kPointerup, std::nullopt, 1, GetTimeStamp(120), GetTimeStamp(150) } };
    std::vector<uint32_t> ids = SimulateInteractionIds(events);
    EXPECT_GT(ids[0], 0u) << "Nonzero interaction id";
    EXPECT_EQ(ids[0], ids[1]) << "Pointerdown and pointerup have same interaction id";
...
{
    std::vector<EventForInteraction> events = { { event_type_names::kPointerup, std::nullopt, 1, GetTimeStamp(100), GetTimeStamp(140) },
        { event_type_names::kClick, std::nullopt, 1, GetTimeStamp(120), GetTimeStamp(150) } };
    std::vector<uint32_t> ids = SimulateInteractionIds(events);
...
TEST_F(InteractionIdTest, JustClick)
{
    ...
    std::vector<EventForInteraction> events = { { event_type_names::kClick, std::nullopt, 0, GetTimeStamp(120), GetTimeStamp(150) } };
    std::vector<uint32_t> ids = SimulateInteractionIds(events);
    EXPECT_GT(ids[0], 0u) << "Nonzero interaction id";
    // Flush UKM logging mojo request.
    RunPendingTasks();
...
TEST_F(InteractionIdTest, MultiTouch)
{
    ...
    // clicks.
    std::vector<EventForInteraction> events = { { event_type_names::kPointerdown, std::nullopt, 1, GetTimeStamp(100), GetTimeStamp(110) },
        { event_type_names::kPointerdown, std::nullopt, 2, GetTimeStamp(120), GetTimeStamp(140) },
        { event_type_names::kPointerup, std::nullopt, 2, GetTimeStamp(200), GetTimeStamp(230) },
        { event_type_names::kPointerup, std::nullopt, 1, GetTimeStamp(200), GetTimeStamp(250) } };
    std::vector<uint32_t> ids = SimulateInteractionIds(events);
    for (uint32_t id : ids) {
        EXPECT_GT(id, 0u);
    }
    ...
    test::RunDelayedTasks(base::Seconds(1));
    CheckUKMValues({ { 30, 50, UserInteractionType::kTapOrClick }, { 50, 60, UserInteractionType::kTapOrClick } });
}
...
TEST_F(InteractionIdTest, ClickIncorrectPointerId)
{
    ...
    std::vector<EventForInteraction> events = { { event_type_names::kPointerup, std::nullopt, 1, GetTimeStamp(100), GetTimeStamp(130) },
        { event_type_names::kClick, std::nullopt, 0, GetTimeStamp(120), GetTimeStamp(160) } };
    std::vector<uint32_t> ids = SimulateInteractionIds(events);
    EXPECT_EQ(ids[0], 0u) << "Orphan pointerup gets interaction id of zero";
    EXPECT_GT(ids[1], 0u) << "Nonzero interaction id for click";
    // Flush UKM logging mojo request.
