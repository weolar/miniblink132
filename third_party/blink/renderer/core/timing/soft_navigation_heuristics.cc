...
const char kPageLoadInternalSoftNavigationOutcome[] = "PageLoad.Internal.SoftNavigationOutcome";
...
void LogAndTraceDetectedSoftNavigation(LocalFrame* frame, LocalDOMWindow* window, const SoftNavigationContext& context)
{
    ...
    auto* console_message = MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kJavaScript, mojom::blink::ConsoleMessageLevel::kInfo,
        String("A soft navigation has been detected: ") + context.Url());
    window->AddConsoleMessage(console_message);

    TRACE_EVENT_INSTANT("scheduler,devtools.timeline,loading", "SoftNavigationHeuristics_SoftNavigationDetected", context.UserInteractionTimestamp(), "frame",
        GetFrameIdForTracing(frame), "url", context.Url(), "navigationId", window->GetNavigationId());
}

constexpr bool IsInteractionStart(SoftNavigationHeuristics::EventScope::Type type)
{
    return (type == SoftNavigationHeuristics::EventScope::Type::kClick || type == SoftNavigationHeuristics::EventScope::Type::kKeydown
        || type == SoftNavigationHeuristics::EventScope::Type::kNavigate);
}

constexpr bool IsInteractionEnd(SoftNavigationHeuristics::EventScope::Type type)
{
    return (type == SoftNavigationHeuristics::EventScope::Type::kClick || type == SoftNavigationHeuristics::EventScope::Type::kKeyup
        || type == SoftNavigationHeuristics::EventScope::Type::kNavigate);
}

std::optional<SoftNavigationHeuristics::EventScope::Type> EventScopeTypeFromEvent(const Event& event)
{
    if (!event.isTrusted()) {
        return std::nullopt;
    }
    ...
    if (event.IsKeyboardEvent()) {
        ...
        if (target_node && target_node->IsHTMLElement() && DynamicTo<HTMLElement>(target_node)->IsHTMLBodyElement()) {
...
// static
const char SoftNavigationHeuristics::kSupplementName[] = "SoftNavigationHeuristics";
...
SoftNavigationHeuristics* SoftNavigationHeuristics::From(LocalDOMWindow& window)
{
    ...
    SoftNavigationHeuristics* heuristics = Supplement<LocalDOMWindow>::From<SoftNavigationHeuristics>(window);
    if (!heuristics) {
...
void SoftNavigationHeuristics::RecordUmaForNonSoftNavigationInteraction(const SoftNavigationContext& context) const
{
    ...
    if (!paint_conditions_met_ && !context.HasMainModification()) {
        base::UmaHistogramEnumeration(kPageLoadInternalSoftNavigationOutcome, SoftNavigationOutcome::kNoPaintOrDomModification);
    } else if (!paint_conditions_met_) {
        base::UmaHistogramEnumeration(kPageLoadInternalSoftNavigationOutcome, SoftNavigationOutcome::kNoPaint);
    } else if (!context.HasMainModification()) {
        base::UmaHistogramEnumeration(kPageLoadInternalSoftNavigationOutcome, SoftNavigationOutcome::kNoDomModification);
    }
}

void SoftNavigationHeuristics::SetIsTrackingSoftNavigationHeuristicsOnDocument(bool value) const
{
...
SoftNavigationContext* SoftNavigationHeuristics::GetSoftNavigationContextForCurrentTask()
{
    if (potential_soft_navigations_.empty()) {
        return nullptr;
    }
    auto* tracker = scheduler::TaskAttributionTracker::From(GetSupplementable()->GetIsolate());
    ...
    SoftNavigationContext* context = task_state ? task_state->GetSoftNavigationContext() : nullptr;
...
std::optional<scheduler::TaskAttributionId> SoftNavigationHeuristics::AsyncSameDocumentNavigationStarted()
{
    auto* tracker = scheduler::TaskAttributionTracker::From(GetSupplementable()->GetIsolate());
    ...
    SoftNavigationContext* context = task_state ? task_state->GetSoftNavigationContext() : nullptr;
    TRACE_EVENT1("scheduler", "SoftNavigationHeuristics::AsyncSameDocumentNavigationStarted", "has_context", !!context);
    if (context) {
        tracker->AddSameDocumentNavigationTask(task_state);
    }
    return context ? std::optional<scheduler::TaskAttributionId>(task_state->Id()) : std::nullopt;
}

void SoftNavigationHeuristics::SameDocumentNavigationCommitted(const String& url, SoftNavigationContext* context)
{
    TRACE_EVENT2("scheduler", "SoftNavigationHeuristics::SameDocumentNavigationCommitted", "url", url, "has_context", !!context);
    if (context) {
        ...
        base::UmaHistogramEnumeration(kPageLoadInternalSoftNavigationOutcome, SoftNavigationOutcome::kNoSoftNavContextDuringUrlChange);
    }
}
...
bool SoftNavigationHeuristics::ModifiedDOM()
{
    ...
    TRACE_EVENT1("scheduler", "SoftNavigationHeuristics::ModifiedDOM", "has_context", !!context);
    return !!context;
}

void SoftNavigationHeuristics::EmitSoftNavigationEntryIfAllConditionsMet(SoftNavigationContext* context)
{
    ...
    // into this condition.
    if (!context || !context->IsSoftNavigation() || context->UserInteractionTimestamp().is_null() || !frame || !frame->IsOutermostMainFrame()) {
        return;
    }
    ...
    performance->AddSoftNavigationEntry(AtomicString(context->Url()), context->UserInteractionTimestamp());
...
// there.
void SoftNavigationHeuristics::RecordPaint(LocalFrame* frame, uint64_t painted_area, bool is_modified_by_soft_navigation)
{
    if (potential_soft_navigations_.empty()) {
    ...
    TRACE_EVENT_INSTANT("loading", "SoftNavigationHeuristics_RecordPaint", "softnav_painted_area", softnav_painted_area_, "required_paint_area",
        required_paint_area, "url", (last_detected_soft_navigation_ ? last_detected_soft_navigation_->Url() : ""), "is_above_threshold", is_above_threshold);
    ...
    if (is_above_threshold) {
        ...
        EmitSoftNavigationEntryIfAllConditionsMet(last_detected_soft_navigation_.Get());
    }
}

void SoftNavigationHeuristics::ReportSoftNavigationToMetrics(LocalFrame* frame, SoftNavigationContext* context) const
{
    ...
    auto soft_navigation_start_time = loader->GetTiming().MonotonicTimeToPseudoWallTime(context->UserInteractionTimestamp());

    LocalDOMWindow* window = GetSupplementable();

    blink::SoftNavigationMetrics metrics = { soft_navigation_count_, soft_navigation_start_time, window->GetNavigationId().Utf8() };
    ...
    base::UmaHistogramEnumeration(kPageLoadInternalSoftNavigationOutcome, SoftNavigationOutcome::kSoftNavigationDetected);
}
...
void SoftNavigationHeuristics::ResetPaintsIfNeeded()
{
    ...
    LocalDOMWindow* window = GetSupplementable();
    if (RuntimeEnabledFeatures::SoftNavigationHeuristicsEnabled(window)) {
        if (Document* document = window->document(); document && RuntimeEnabledFeatures::SoftNavigationHeuristicsExposeFPAndFCPEnabled(window)) {
...
void SoftNavigationHeuristics::CommitPreviousPaints(LocalFrame* frame)
{
    ...
    if (!did_commit_previous_paints_) {
        ...
        if (RuntimeEnabledFeatures::SoftNavigationHeuristicsExposeFPAndFCPEnabled(window)) {
...
void SoftNavigationHeuristics::Trace(Visitor* visitor) const
{
    ...
    visitor->RegisterWeakCallbackMethod<SoftNavigationHeuristics, &SoftNavigationHeuristics::ProcessCustomWeakness>(this);
}

void SoftNavigationHeuristics::OnCreateTaskScope(scheduler::TaskAttributionInfo& task_state)
{
    ...
    // handler.
    if (task_state.GetSoftNavigationContext() != active_interaction_context_.Get()) {
        return;
    }
    ...
    TRACE_EVENT1("scheduler", "SoftNavigationHeuristics::OnCreateTaskScope", "task_id", task_state.Id().value());
...
void SoftNavigationHeuristics::ProcessCustomWeakness(const LivenessBroker& info)
{
    if (potential_soft_navigations_.empty()) {
        return;
    }
...
SoftNavigationHeuristics::EventScope SoftNavigationHeuristics::CreateEventScope(EventScope::Type type, ScriptState* script_state)
{
    if (!has_active_event_scope_) {
    ...
    if (!tracker) {
        return SoftNavigationHeuristics::EventScope(this,
            ...
            /*task_scope=*/std::nullopt, type, is_nested);
    }
    return SoftNavigationHeuristics::EventScope(
        this, tracker->RegisterObserver(this), tracker->CreateTaskScope(script_state, active_interaction_context_.Get()), type, is_nested);
}

std::optional<SoftNavigationHeuristics::EventScope> SoftNavigationHeuristics::MaybeCreateEventScopeForEvent(const Event& event)
{
...
void SoftNavigationHeuristics::OnSoftNavigationEventScopeDestroyed(const EventScope& event_scope)
{
    ...
    if (active_interaction_context_->UserInteractionTimestamp().is_null()) {
        active_interaction_context_->SetUserInteractionTimestamp(base::TimeTicks::Now());
    }
...
uint64_t SoftNavigationHeuristics::CalculateRequiredPaintArea() const
{
    ...
    uint64_t required_paint_area = (viewport_area * kSoftNavigationPaintAreaPercentage) / 100;
...
SoftNavigationHeuristics::EventScope::EventScope(
    SoftNavigationHeuristics* heuristics, std::optional<ObserverScope> observer_scope, std::optional<TaskScope> task_scope, Type type, bool is_nested)
...
SoftNavigationHeuristics::EventScope& SoftNavigationHeuristics::EventScope::operator=(EventScope&& other)
{
    heuristics_ = std::exchange(other.heuristics_, nullptr);
    observer_scope_ = std::move(other.observer_scope_);
    task_scope_ = std::move(other.task_scope_);
