...
TEST_F(ProfilerGroupTest, StopProfiler)
{
    ...
    Profiler* profiler = profiler_group->CreateProfiler(scope.GetScriptState(), *init_options, base::TimeTicks(), scope.GetExceptionState());
...
TEST_F(ProfilerGroupTest, ClampedSamplingIntervalZero)
{
    ...
    // interval.
    EXPECT_EQ(profiler->sampleInterval(), ProfilerGroup::GetBaseSampleInterval().InMilliseconds());
...
TEST_F(ProfilerGroupTest, ClampedSamplingIntervalNext)
{
    ...
    ProfilerInitOptions* init_options = ProfilerInitOptions::Create();
    init_options->setSampleInterval((ProfilerGroup::GetBaseSampleInterval() + base::Milliseconds(1)).InMilliseconds());
    Profiler* profiler = profiler_group->CreateProfiler(scope.GetScriptState(), *init_options, base::TimeTicks(), scope.GetExceptionState());
    ...
    // interval.
    EXPECT_EQ(profiler->sampleInterval(), (ProfilerGroup::GetBaseSampleInterval() * 2).InMilliseconds());

    // clean up
    profiler->stop(scope.GetScriptState());
}

TEST_F(ProfilerGroupTest, V8ProfileLimitThrowsExceptionWhenMaxConcurrentReached)
{
    V8TestingScope scope;
    ...
    for (auto i = 0; i < kMaxConcurrentProfilerCount; i++) {
        ...
        profilers.push_back(profiler_group->CreateProfiler(scope.GetScriptState(), *init_options, base::TimeTicks(), scope.GetExceptionState()));
        EXPECT_FALSE(scope.GetExceptionState().HadException());
    }
    ...
    for (auto i = kMaxConcurrentProfilerCount; i < kLargeProfilerCount; i++) {
        extra_profiler_group->CreateProfiler(scope.GetScriptState(), *extra_init_options, base::TimeTicks(), scope.GetExceptionState());
        EXPECT_TRUE(scope.GetExceptionState().HadException());
        EXPECT_EQ(scope.GetExceptionState().Message(), "Reached maximum concurrent amount of profilers");
    }
...
TEST_F(ProfilerGroupTest, NegativeSamplingInterval)
{
    ...
    ProfilerInitOptions* init_options = ProfilerInitOptions::Create();
    init_options->setSampleInterval(-10);
    profiler_group->CreateProfiler(scope.GetScriptState(), *init_options, base::TimeTicks(), scope.GetExceptionState());
...
TEST_F(ProfilerGroupTest, OverflowSamplingInterval)
{
    ...
    ProfilerInitOptions* init_options = ProfilerInitOptions::Create();
    init_options->setSampleInterval((double)std::numeric_limits<int>::max() + 1.f);
    profiler_group->CreateProfiler(scope.GetScriptState(), *init_options, base::TimeTicks(), scope.GetExceptionState());
...
{
    class ExpectNoCallFunction : public ThenCallable<ProfilerTrace, ExpectNoCallFunction> {
    public:
        void React(ScriptState*, ProfilerTrace*)
        {
            EXPECT_FALSE(true) << "Promise should not resolve without dispatching a task";
        }
    };
    ...
    auto* profiler = profiler_group->CreateProfiler(scope.GetScriptState(), *init_options, base::TimeTicks(), scope.GetExceptionState());

    profiler->stop(scope.GetScriptState()).React(scope.GetScriptState(), MakeGarbageCollected<ExpectNoCallFunction>());
}
...
TEST_F(ProfilerGroupTest, LeakProfilerWithContext)
{
    Profiler* profiler;
    {
        ...
        init_options->setSampleInterval(0);
        init_options->setMaxBufferSize(0);
        profiler = profiler_group->CreateProfiler(scope.GetScriptState(), *init_options, base::TimeTicks(), scope.GetExceptionState());
