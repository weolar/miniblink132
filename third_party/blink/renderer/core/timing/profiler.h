...
public:
    Profiler(ProfilerGroup* profiler_group, ScriptState* script_state, const String& profiler_id, int target_sample_rate,
        scoped_refptr<const SecurityOrigin> source_origin, base::TimeTicks time_origin)
    ...
    static Profiler* Create(ScriptState*, const ProfilerInitOptions*, ExceptionState&);

    void Trace(Visitor* visitor) const override;

    void DisposeAsync();

    String ProfilerId() const
    {
        return profiler_id_;
    }
    ...
    const SecurityOrigin* SourceOrigin() const
    {
        return source_origin_.get();
    }
