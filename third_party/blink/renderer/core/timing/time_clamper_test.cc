...
namespace blink {
namespace {
const int64_t kIntervalInMicroseconds = TimeClamper::kFineResolutionMicroseconds;
}
...
{
    TimeClamper clamper;
    EXPECT_GE(clamper.ClampTimeResolution(base::TimeDelta(), true).InMicroseconds(), 0.f);
    EXPECT_GE(clamper.ClampTimeResolution(base::Microseconds(TimeClamper::kFineResolutionMicroseconds), true).InMicroseconds(), 0.f);
}

TEST_F(TimeClamperTest, TimeStampsIncreaseByFixedAmount)
{
    TimeClamper clamper;
    int64_t prev = clamper.ClampTimeResolution(base::TimeDelta(), true).InMicroseconds();
    for (int64_t time_microseconds = 0; time_microseconds < kIntervalInMicroseconds * 100; time_microseconds += 1) {
        int64_t clamped_time = clamper.ClampTimeResolution(base::Microseconds(time_microseconds), true).InMicroseconds();
...
TEST_F(TimeClamperTest, ClampingIsDeterministic)
{
    TimeClamper clamper;
    for (int64_t time_microseconds = 0; time_microseconds < kIntervalInMicroseconds * 100; time_microseconds += 1) {
        int64_t t1 = clamper.ClampTimeResolution(base::Microseconds(time_microseconds), true).InMicroseconds();
        int64_t t2 = clamper.ClampTimeResolution(base::Microseconds(time_microseconds), true).InMicroseconds();
        EXPECT_EQ(t1, t2);
    }
}
...
{
    TimeClamper clamper;
    for (int64_t time_microseconds = -kIntervalInMicroseconds * 100; time_microseconds < kIntervalInMicroseconds * 100; time_microseconds += 1) {
        int64_t t1 = clamper.ClampTimeResolution(base::Microseconds(time_microseconds), true).InMicroseconds();
        int64_t t2 = clamper.ClampTimeResolution(base::Microseconds(time_microseconds), true).InMicroseconds();
        EXPECT_EQ(t1, t2);
    }
...
TEST_F(TimeClamperTest, ClampingIsPerInstance)
{
    ...
    while (true) {
        if (std::abs(clamper1.ClampTimeResolution(base::Microseconds(time_microseconds), true).InMicroseconds()
                - clamper2.ClampTimeResolution(base::Microseconds(time_microseconds), true).InMicroseconds())
            >= 1) {
            break;
        }
...
void UniformityTest(int64_t time_microseconds, int interval, bool cross_origin_isolated_capability)
{
    ...
    for (int i = 0; i < kSampleCount; i++) {
        int64_t start = clamper.ClampTimeResolution(base::Microseconds(time_microseconds), cross_origin_isolated_capability).InMicroseconds();
        for (int step = 0; step < kBuckets; step++) {
            time_microseconds += kTimeStep;
            if (std::abs(clamper.ClampTimeResolution(base::Microseconds(time_microseconds), cross_origin_isolated_capability).InMicroseconds() - start) >= 1) {
                histogram[step]++;
                // Skip to the next interval to make sure each measurement is
                // independent.
                time_microseconds = floor(time_microseconds / interval) * interval + interval;
                break;
            }
        }
    }
