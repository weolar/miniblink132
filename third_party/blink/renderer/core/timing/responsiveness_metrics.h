...
class CORE_EXPORT ResponsivenessMetrics : public GarbageCollected<ResponsivenessMetrics> {
public:
    ...
    public:
        InteractionInfo(uint32_t interaction_id, uint32_t interaction_offset, EventTimestamps timestamps)
        ...
        ~InteractionInfo() = default;
        uint32_t GetInteractionId() const
        {
            return interaction_id_;
        }
        uint32_t GetInteractionOffset() const
        {
            return interaction_offset_;
        }
        void SetInteractionIdAndOffset(uint32_t interaction_id, uint32_t interaction_offset)
        {
            interaction_id_ = interaction_id;
            interaction_offset_ = interaction_offset;
        }
        Vector<EventTimestamps> const& GetTimeStamps()
        {
            return timestamps_;
        }
        bool Empty()
    ...
    // on a HeapHashMap.
    class KeyboardEntryAndTimestamps : public GarbageCollected<KeyboardEntryAndTimestamps> {
    public:
        KeyboardEntryAndTimestamps(PerformanceEventTiming* entry, EventTimestamps timestamps)
            : entry_(entry)
            , timestamps_({ timestamps })
        {
        }

        static KeyboardEntryAndTimestamps* Create(PerformanceEventTiming* entry, EventTimestamps timestamps)
        {
            return MakeGarbageCollected<KeyboardEntryAndTimestamps>(entry, timestamps);
        }
    ...
    public:
        PointerEntryAndInfo(PerformanceEventTiming* entry, EventTimestamps timestamps)
            : entry_(entry)
            , timestamps_({ timestamps })
        {
        }

        static PointerEntryAndInfo* Create(PerformanceEventTiming* entry, EventTimestamps timestamps)
        {
        ...
        Vector<EventTimestamps>& GetTimeStamps()
        {
            return timestamps_;
        }
        void SetIsDrag()
    ...
    void StopUkmSamplingForTesting()
    {
        sampling_ = false;
    }
    ...
    bool SetPointerIdAndRecordLatency(PerformanceEventTiming* entry, EventTimestamps event_timestamps);
    ...
    void SetKeyIdAndRecordLatency(PerformanceEventTiming* entry, EventTimestamps event_timestamps);
...
private:
    ...
    void RecordUserInteractionUKM(LocalDOMWindow* window, UserInteractionType interaction_type,
        const WTF::Vector<ResponsivenessMetrics::EventTimestamps>& timestamps, uint32_t interaction_offset);

    void RecordDragTapOrClickUKM(LocalDOMWindow*, PointerEntryAndInfo&);

    void RecordKeyboardUKM(LocalDOMWindow* window, const WTF::Vector<EventTimestamps>& event_timestamps, uint32_t interaction_offset);
    ...
    bool TryHandleKeyboardEventSimulatedClick(PerformanceEventTiming* entry, const std::optional<PointerId>& last_pointer_id);

    Member<WindowPerformance> window_performance_;

    // Map from keyCodes to interaction info (ID, offset, and timestamps).
    HashMap<int, InteractionInfo, IntWithZeroKeyHashTraits<int>> key_code_to_interaction_info_map_;
    ...
    HeapHashMap<PointerId, Member<PointerEntryAndInfo>, IntWithZeroKeyHashTraits<PointerId>> pointer_id_entry_map_;
