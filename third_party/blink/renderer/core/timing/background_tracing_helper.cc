...
std::optional<uint32_t> ConvertToHashInteger(std::string_view chars)
{
    ...
    return WTF::HexCharactersToUInt(base::as_byte_span(chars), WTF::NumberParsingOptions(), nullptr);
}
...
{
    return StringView(mark_name, 0, std::size(kTriggerPrefix) - 1) == kTriggerPrefix;
}

std::string GenerateFullTrigger(std::string_view site, std::string_view mark_name)
{
...
BackgroundTracingHelper::SiteHashSet MakeSiteHashSet()
{
    ...
    if (!base::FeatureList::IsEnabled(features::kBackgroundTracingPerformanceMark)) {
        return {};
    }
    // Get the allow-list from the Finch configuration.
    std::string allow_list = features::kBackgroundTracingPerformanceMark_AllowList.Get();
...
BackgroundTracingHelper::BackgroundTracingHelper(ExecutionContext* context)
{
    ...
    if (origin->IsLocal() || origin->IsOpaque() || origin->IsLocalhost())
        return;
    if (!CommonSchemeRegistry::IsExtensionScheme(origin->Protocol().Ascii()) && origin->Protocol() != url::kHttpScheme
    ...
    uint64_t merged = token.value().GetHighForSerialization() ^ token.value().GetLowForSerialization();
    execution_context_id_ = static_cast<uint32_t>(merged & 0xffffffff) ^ static_cast<uint32_t>((merged >> 32) & 0xffffffff);
}

BackgroundTracingHelper::~BackgroundTracingHelper() = default;

void BackgroundTracingHelper::MaybeEmitBackgroundTracingPerformanceMarkEvent(const PerformanceMark& mark)
{
    if (site_.empty()) {
        return;
    }
    ...
    TRACE_EVENT_INSTANT("blink,latency", "performance.mark", mark.UnsafeTimeForTraces(), event_lambda);

    base::trace_event::EmitNamedTrigger(GenerateFullTrigger(site_, mark_name), mark_and_id.second);
}
...
// static
const BackgroundTracingHelper::SiteHashSet& BackgroundTracingHelper::GetSiteHashSet()
{
    // This needs to be thread-safe because performance.mark is supported by both
    // windows and workers.
    DEFINE_THREAD_SAFE_STATIC_LOCAL(SiteHashSet, site_hash_set_, (MakeSiteHashSet()));
...
std::pair<StringView, std::optional<uint32_t>> BackgroundTracingHelper::SplitMarkNameAndId(StringView mark_name)
{
    ...
    bool result = false;
    int seq_num = WTF::CharactersToInt(suffix, WTF::NumberParsingOptions(), &result);
    if (!result) {
        return std::make_pair(mark_name, std::nullopt);
    }
...
// static
BackgroundTracingHelper::SiteHashSet BackgroundTracingHelper::ParsePerformanceMarkSiteHashes(std::string_view allow_list)
{
    SiteHashSet allow_listed_hashes;
    auto hashes = base::SplitStringPiece(allow_list, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
