...
AtomicString FromPaintTypeToString(PerformancePaintTiming::PaintType type)
{
    ...
    case PerformancePaintTiming::PaintType::kFirstContentfulPaint: {
        DEFINE_STATIC_LOCAL(const AtomicString, kFirstContentfulPaint, ("first-contentful-paint"));
...
PerformancePaintTiming::PerformancePaintTiming(PaintType type, double start_time, DOMWindow* source, bool is_triggered_by_soft_navigation)
    : PerformanceEntry(FromPaintTypeToString(type), start_time, start_time, source, is_triggered_by_soft_navigation)
{
}
