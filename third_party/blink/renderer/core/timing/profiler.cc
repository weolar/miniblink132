...
Profiler* Profiler::Create(ScriptState* script_state, const ProfilerInitOptions* options, ExceptionState& exception_state)
{
    ...
    if (LocalDOMWindow* window = LocalDOMWindow::From(script_state)) {
        can_profile = ProfilerGroup::CanProfile(window, &exception_state, ReportOptions::kReportOnFailure);
    ...
    auto* profiler = profiler_group->CreateProfiler(script_state, *options, performance->GetTimeOriginInternal(), exception_state);
...
{
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<ProfilerTrace>>(script_state);
    ...
    if (!stopped()) {
        ...
        resolver->Reject(MakeGarbageCollected<DOMException>(DOMExceptionCode::kInvalidStateError, "Profiler already stopped."));
    }
