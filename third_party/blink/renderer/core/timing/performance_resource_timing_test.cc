...
protected:
    AtomicString GetNextHopProtocol(const AtomicString& alpn_negotiated_protocol, const AtomicString& connection_info)
    {
        mojom::blink::ResourceTimingInfo info;
        info.allow_timing_details = true;
        PerformanceResourceTiming* timing = MakePerformanceResourceTiming(info.Clone());
        return timing->GetNextHopProtocol(alpn_negotiated_protocol, connection_info);
    }

    AtomicString GetNextHopProtocolWithoutTao(const AtomicString& alpn_negotiated_protocol, const AtomicString& connection_info)
    {
        mojom::blink::ResourceTimingInfo info;
    ...
    PerformanceResourceTiming* MakePerformanceResourceTiming(mojom::blink::ResourceTimingInfoPtr info)
    {
        std::unique_ptr<DummyPageHolder> dummy_page_holder = std::make_unique<DummyPageHolder>();
        return MakeGarbageCollected<PerformanceResourceTiming>(std::move(info), g_empty_atom, base::TimeTicks() + base::Milliseconds(100),
            dummy_page_holder->GetDocument().GetExecutionContext()->CrossOriginIsolatedCapability(), dummy_page_holder->GetDocument().GetExecutionContext());
    }
...
TEST_F(PerformanceResourceTimingTest, TestFallbackToConnectionInfoWhenALPNUnknown)
{
    ...
    EXPECT_EQ(GetNextHopProtocol(alpn_negotiated_protocol, connection_info), connection_info);
}

TEST_F(PerformanceResourceTimingTest, TestFallbackToHTTPInfoWhenALPNAndConnectionInfoUnknown)
{
    V8TestingScope scope;
    Initialize(scope.GetScriptState());
...
TEST_F(PerformanceResourceTimingTest, TestNoChangeWhenContainsQuic)
{
    ...
    EXPECT_EQ(GetNextHopProtocol(alpn_negotiated_protocol, connection_info), alpn_negotiated_protocol);
}
...
TEST_F(PerformanceResourceTimingTest, TestNextHopProtocolIsGuardedByTao)
{
    ...
    AtomicString connection_info("http/1.1");
    AtomicString alpn_negotiated_protocol("RandomProtocol");
    EXPECT_EQ(GetNextHopProtocolWithoutTao(alpn_negotiated_protocol, connection_info), "");
}
...
TEST_F(PerformanceResourceTimingTest, TestRequestStart)
{
    ...
    PerformanceResourceTiming* resource_timing = MakePerformanceResourceTiming(info.Clone());
...
TEST_F(PerformanceResourceTimingTest, TestRequestStartNullSendStart)
{
    ...
    info.timing->connect_timing = network::mojom::blink::LoadTimingInfoConnectTiming::New();

    info.timing->connect_timing->connect_end = base::TimeTicks() + base::Milliseconds(751);
