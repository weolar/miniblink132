...
protected:
    ...
    SoftNavigationHeuristics* CreateSoftNavigationHeuristicsForTest()
    {
        ...
        SoftNavigationHeuristics* heuristics = SoftNavigationHeuristics::From(*window);
    ...
    {
        return LocalDOMWindow::From(GetScriptStateForTest())->document()->IsTrackingSoftNavigationHeuristics();
    }

    static WTF::AtomicString KeyboardEventScopeTypeToEventName(SoftNavigationHeuristics::EventScope::Type type)
    {
        switch (type) {
    ...
    Event* CreateEvent(SoftNavigationHeuristics::EventScope::Type type)
    {
        ...
        case SoftNavigationHeuristics::EventScope::Type::kKeypress:
        case SoftNavigationHeuristics::EventScope::Type::kKeyup:
            event = KeyboardEvent::Create(GetScriptStateForTest(), KeyboardEventScopeTypeToEventName(type), KeyboardEventInit::Create());
            event->SetTarget(MakeGarbageCollected<HTMLBodyElement>(GetDocument()));
            break;
        case SoftNavigationHeuristics::EventScope::Type::kClick:
            event = MouseEvent::Create(GetScriptStateForTest(), event_type_names::kClick, MouseEventInit::Create());
            break;
...
TEST_F(SoftNavigationHeuristicsTest, EarlyReturnOnInvalidPendingInteractionTimestamp)
{
    ...
    auto* event = CreateEvent(SoftNavigationHeuristics::EventScope::Type::kKeypress);
    std::optional<SoftNavigationHeuristics::EventScope> event_scope(test_heuristics->MaybeCreateEventScopeForEvent(*event));
    auto* tracker = scheduler::TaskAttributionTracker::From(GetScriptStateForTest()->GetIsolate());
    ASSERT_TRUE(tracker);
    {
        // Simulate a top-level event dispatch with no context to propagate.
        std::optional<TaskScope> task_scope = tracker->MaybeCreateTaskScopeForCallback(GetScriptStateForTest(), nullptr);
    }
...
TEST_F(SoftNavigationHeuristicsTest, ResetHeuristicOnSetBecameEmpty)
{
    ...
    // Simulate a click.
    {
        ...
        std::optional<SoftNavigationHeuristics::EventScope> event_scope(heuristics->MaybeCreateEventScopeForEvent(*event));

        // Simulate a top-level event dispatch with no context to propagate.
        std::optional<TaskScope> task_scope = tracker->MaybeCreateTaskScopeForCallback(script_state, nullptr);
    ...
    {
        TaskScope task_scope = tracker->CreateTaskScope(script_state, root_task, TaskScopeType::kCallback);
...
TEST_F(SoftNavigationHeuristicsTest, NestedEventScopesAreMerged)
{
    ...
    auto* event = CreateEvent(SoftNavigationHeuristics::EventScope::Type::kClick);
    std::optional<SoftNavigationHeuristics::EventScope> outer_event_scope(heuristics->MaybeCreateEventScopeForEvent(*event));
    auto* tracker = scheduler::TaskAttributionTracker::From(GetScriptStateForTest()->GetIsolate());
    ASSERT_TRUE(tracker);
    ...
    auto* inner_event = CreateEvent(SoftNavigationHeuristics::EventScope::Type::kNavigate);
    std::optional<SoftNavigationHeuristics::EventScope> inner_event_scope(heuristics->MaybeCreateEventScopeForEvent(*inner_event));
...
TEST_F(SoftNavigationHeuristicsTest, EventAfterSoftNavDetection)
{
    ...
    auto* outer_event = CreateEvent(SoftNavigationHeuristics::EventScope::Type::kClick);
    std::optional<SoftNavigationHeuristics::EventScope> outer_event_scope(heuristics->MaybeCreateEventScopeForEvent(*outer_event));
    auto* tracker = scheduler::TaskAttributionTracker::From(script_state->GetIsolate());
    ASSERT_TRUE(tracker);
...
TEST_F(SoftNavigationHeuristicsTest, HeuristicNotResetDuringGCWithActiveContext)
{
    auto* heuristics = CreateSoftNavigationHeuristicsForTest();
    auto* script_state = GetScriptStateForTest();
    auto* tracker = scheduler::TaskAttributionTracker::From(script_state->GetIsolate());
    ...
    // cleared.
    ThreadState::Current()->CollectAllGarbageForTesting(cppgc::EmbedderStackState::kMayContainHeapPointers);
...
TEST_F(SoftNavigationHeuristicsTest, SoftNavigationEmittedOnlyOnce)
{
    ...
    {
        std::optional<TaskScope> task_scope = tracker->MaybeCreateTaskScopeForCallback(script_state, task_state);
...
TEST_F(SoftNavigationHeuristicsTest, AsyncSameDocumentNavigationNoContext)
{
    ...
    std::optional<scheduler::TaskAttributionId> navigation_task_id;
    {
        std::optional<TaskScope> task_scope = tracker->MaybeCreateTaskScopeForCallback(GetScriptStateForTest(),
            /*task_state=*/nullptr);
        navigation_task_id = heuristics->AsyncSameDocumentNavigationStarted();
    }
...
TEST_F(SoftNavigationHeuristicsTest, MaybeCreateEventScopeForEvent)
{
    ...
    for (unsigned type = 0; type <= static_cast<unsigned>(SoftNavigationHeuristics::EventScope::Type::kLast); type++) {
        auto* event = CreateEvent(static_cast<SoftNavigationHeuristics::EventScope::Type>(type));
        auto event_scope = heuristics->MaybeCreateEventScopeForEvent(*event);
        EXPECT_TRUE(event_scope);
    }

    // Untrusted events should be ignored.
    Event* event = CreateEvent(SoftNavigationHeuristics::EventScope::Type::kClick);
