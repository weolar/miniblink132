...
PerformanceEventTiming* PerformanceEventTiming::Create(
    const AtomicString& event_type, EventTimingReportingInfo reporting_info, bool cancelable, Node* target, DOMWindow* source)
{
    // TODO(npm): enable this DCHECK once https://crbug.com/852846 is fixed.
    // DCHECK_LE(start_time, processing_start);
    DCHECK_LE(reporting_info.processing_start_time, reporting_info.processing_end_time);
    CHECK(source);
    return MakeGarbageCollected<PerformanceEventTiming>(event_type, performance_entry_names::kEvent, std::move(reporting_info), cancelable, target, source);
}

// static
PerformanceEventTiming* PerformanceEventTiming::CreateFirstInputTiming(PerformanceEventTiming* entry)
{
    PerformanceEventTiming* first_input = MakeGarbageCollected<PerformanceEventTiming>(
        entry->name(), performance_entry_names::kFirstInput, *entry->GetEventTimingReportingInfo(), entry->cancelable(), entry->target(), entry->source());
    first_input->SetDuration(entry->duration());
    if (entry->HasKnownInteractionID()) {
        first_input->SetInteractionIdAndOffset(entry->interactionId(), entry->interactionOffset());
    }
    return first_input;
}

PerformanceEventTiming::PerformanceEventTiming(
    const AtomicString& event_type, const AtomicString& entry_type, EventTimingReportingInfo reporting_info, bool cancelable, Node* target, DOMWindow* source)
    : PerformanceEntry(event_type,
        DOMWindowPerformance::performance(*source->ToLocalDOMWindow())->MonotonicTimeToDOMHighResTimeStamp(reporting_info.creation_time), 0.0, source)
    , entry_type_(entry_type)
    , processing_start_(
          DOMWindowPerformance::performance(*source->ToLocalDOMWindow())->MonotonicTimeToDOMHighResTimeStamp(reporting_info.processing_start_time))
    , processing_end_(DOMWindowPerformance::performance(*source->ToLocalDOMWindow())->MonotonicTimeToDOMHighResTimeStamp(reporting_info.processing_end_time))
...
{
    return entry_type_ == performance_entry_names::kEvent ? PerformanceEntry::EntryType::kEvent : PerformanceEntry::EntryType::kFirstInput;
}
...
{
    return reporting_info_.presentation_time.has_value() || reporting_info_.fallback_time.has_value();
}
...
void PerformanceEventTiming::SetInteractionIdAndOffset(uint32_t interaction_id, uint32_t interaction_offset)
{
    interaction_id_ = interaction_id;
...
void PerformanceEventTiming::SetPerfettoData(Frame* frame, perfetto::protos::pbzero::EventTiming* event_timing, base::TimeTicks time_origin)
{
    *(int*)1 = 1;
...
// perfetto events.
std::unique_ptr<TracedValue> PerformanceEventTiming::ToTracedValue(Frame* frame) const
{
    auto traced_value = std::make_unique<TracedValue>();
    traced_value->SetString("type", name());
    // Recalculate this as the stored duration value is rounded.
    traced_value->SetDouble("duration", (GetEndTime() - reporting_info_.creation_time).InMillisecondsF());
    ...
    traced_value->SetInteger("interactionOffset", static_cast<int>(interactionOffset()));
    traced_value->SetInteger("nodeId", target_ ? target_->GetDomNodeId() : kInvalidDOMNodeId);
    ...
    base::TimeTicks origin_time = WindowPerformance::GetTimeOrigin(To<LocalDOMWindow>(source()));
    traced_value->SetDouble("timeStamp", (reporting_info_.creation_time - origin_time).InMillisecondsF());
    traced_value->SetDouble("processingStart", (reporting_info_.processing_start_time - origin_time).InMillisecondsF());
    traced_value->SetDouble("processingEnd", (reporting_info_.processing_end_time - origin_time).InMillisecondsF());
    traced_value->SetDouble("enqueuedToMainThreadTime", (reporting_info_.enqueued_to_main_thread_time - origin_time).InMillisecondsF());

    if (reporting_info_.commit_finish_time.has_value()) {
        traced_value->SetDouble("commitFinishTime", (reporting_info_.commit_finish_time.value() - origin_time).InMillisecondsF());
    }
    return traced_value;
}
