...
class CORE_EXPORT SoftNavigationHeuristics : public GarbageCollected<SoftNavigationHeuristics>,
                                             public Supplement<LocalDOMWindow>,
                                             public scheduler::TaskAttributionTracker::Observer {
    USING_PRE_FINALIZER(SoftNavigationHeuristics, Dispose);

public:
    FRIEND_TEST_ALL_PREFIXES(SoftNavigationHeuristicsTest, EarlyReturnOnInvalidPendingInteractionTimestamp);
    ...
    public:
        enum class Type { kKeydown, kKeypress, kKeyup, kClick, kNavigate, kLast = kNavigate };
    ...
    private:
        ...
        EventScope(SoftNavigationHeuristics*, std::optional<ObserverScope>, std::optional<TaskScope>, Type, bool is_nested);
    ...
    // navigation.
    std::optional<scheduler::TaskAttributionId> AsyncSameDocumentNavigationStarted();

    void SameDocumentNavigationCommitted(const String& url, SoftNavigationContext*);
    bool ModifiedDOM();
    uint32_t SoftNavigationCount()
    {
    ...
    void RecordPaint(LocalFrame*, uint64_t painted_area, bool is_modified_by_soft_navigation);
...
private:
    void RecordUmaForNonSoftNavigationInteraction(const SoftNavigationContext&) const;
    ...
    HashSet<UntracedMember<const SoftNavigationContext>> potential_soft_navigations_;
