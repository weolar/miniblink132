...
// static
LayoutShift* LayoutShift::Create(double start_time, double value, bool input_detected, double input_timestamp, AttributionList sources, DOMWindow* source)
{
    return MakeGarbageCollected<LayoutShift>(start_time, value, input_detected, input_timestamp, sources, source);
}

LayoutShift::LayoutShift(double start_time, double value, bool input_detected, double input_timestamp, AttributionList sources, DOMWindow* source)
...
void LayoutShift::BuildJSONValue(V8ObjectBuilder& builder) const
{
    ...
    builder.AddV8Value("sources", ToV8Traits<IDLArray<LayoutShiftAttribution>>::ToV8(builder.GetScriptState(), sources_));
}
