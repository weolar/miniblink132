...
class CORE_EXPORT PerformanceTimingForReporting final : public GarbageCollected<PerformanceTimingForReporting>, public ExecutionContextClient {
public:
    struct BackForwardCacheRestoreTiming {
        uint64_t navigation_start;
        uint64_t first_paint;
        std::array<uint64_t, WebPerformanceMetricsForReporting::kRequestAnimationFramesToRecordAfterBackForwardCacheRestore> request_animation_frames;
        std::optional<base::TimeDelta> first_input_delay;
    };

    using BackForwardCacheRestoreTimings = WTF::Vector<BackForwardCacheRestoreTiming>;
    ...
    LargestContentfulPaintDetailsForReporting LargestContentfulPaintDetailsForMetrics() const;

    LargestContentfulPaintDetailsForReporting SoftNavigationLargestContentfulPaintDetailsForMetrics() const;

    LargestContentfulPaintDetailsForReporting PopulateLargestContentfulPaintDetailsForReporting(const LargestContentfulPaintDetails& timing) const;
    ...
    std::optional<std::tuple<AtomicString, base::TimeDelta>> CustomUserTimingMark() const;
...
private:
    ...
    std::optional<base::TimeDelta> MonotonicTimeToPseudoWallTime(const std::optional<base::TimeTicks>&) const;
