...
public:
    LargestContentfulPaint(double start_time, DOMHighResTimeStamp render_time, uint64_t size, DOMHighResTimeStamp load_time,
        DOMHighResTimeStamp first_animated_frame_time, const AtomicString& id, const String& url, Element* element, DOMWindow* source,
...
template <> struct DowncastTraits<LargestContentfulPaint> {
    static bool AllowFrom(const PerformanceEntry& entry)
    {
        return entry.EntryTypeEnum() == PerformanceEntry::EntryType::kLargestContentfulPaint;
    }
};
