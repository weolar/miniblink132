...
PerformanceMark::PerformanceMark(const AtomicString& name, double start_time, base::TimeTicks unsafe_time_for_traces,
    scoped_refptr<SerializedScriptValue> serialized_detail, ExceptionState& exception_state, DOMWindow* source)
...
// static
PerformanceMark* PerformanceMark::Create(
    ScriptState* script_state, const AtomicString& mark_name, PerformanceMarkOptions* mark_options, ExceptionState& exception_state)
{
    ...
    } else if (auto* scope = DynamicTo<WorkerGlobalScope>(ExecutionContext::From(script_state))) {
    ...
    if (mark_options) {
        if (mark_options->hasStartTime()) {
            ...
            if (start < 0.0) {
                exception_state.ThrowTypeError("'" + mark_name + "' cannot have a negative start time.");
                return nullptr;
            }
            ...
            unsafe_start_for_traces = performance->GetTimeOriginInternal() + base::Milliseconds(start);
        } else {
    ...
    if (!is_worker_global_scope && PerformanceTiming::IsAttributeName(mark_name)) {
    ...
    if (!detail) {
        ...
        serialized_detail
            = SerializedScriptValue::Serialize(script_state->GetIsolate(), (*detail).V8Value(), SerializedScriptValue::SerializeOptions(), exception_state);
    ...
    return MakeGarbageCollected<PerformanceMark>(
        mark_name, start, unsafe_start_for_traces, std::move(serialized_detail), exception_state, LocalDOMWindow::From(script_state));
}
...
mojom::blink::PerformanceMarkOrMeasurePtr PerformanceMark::ToMojoPerformanceMarkOrMeasure()
{
    auto mojo_performance_mark_or_measure = PerformanceEntry::ToMojoPerformanceMarkOrMeasure();
...
ScriptValue PerformanceMark::detail(ScriptState* script_state)
{
    ...
    auto result = deserialized_detail_map_.insert(script_state, TraceWrapperV8Reference<v8::Value>());
    TraceWrapperV8Reference<v8::Value>& relevant_data = result.stored_value->value;
...
// static
const PerformanceMark::UserFeatureNameToWebFeatureMap& PerformanceMark::GetUseCounterMapping()
{
    DEFINE_THREAD_SAFE_STATIC_LOCAL(ThreadSpecific<UserFeatureNameToWebFeatureMap>, map, ());
    if (!map.IsSet()) {
        *map = {
            ...
            { "next-third-parties-YouTubeEmbed", WebFeature::kUserFeatureNextThirdPartiesYouTubeEmbed },
            { "next-third-parties-GoogleMapsEmbed", WebFeature::kUserFeatureNextThirdPartiesGoogleMapsEmbed },
            ...
            { "nuxt-third-parties-YouTubeEmbed", WebFeature::kUserFeatureNuxtThirdPartiesYouTubeEmbed },
            { "nuxt-third-parties-GoogleMaps", WebFeature::kUserFeatureNuxtThirdPartiesGoogleMaps },
        };
    }
    return *map;
}
...
// static
std::optional<mojom::blink::WebFeature> PerformanceMark::GetWebFeatureForUserFeatureName(const String& feature_name)
{
