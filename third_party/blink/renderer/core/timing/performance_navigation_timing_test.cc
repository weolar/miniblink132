...
TEST_F(PerformanceNavigationTimingTest, GetNavigationTimingType)
{
    ...
    V8NavigationTimingType::Enum returned_type = GetNavigationTimingType(kWebNavigationTypeBackForward);
    ...
    GetPage().SetVisibilityState(mojom::blink::PageVisibilityState::kVisible,
        /*is_initial_state=*/false);
    returned_type = GetNavigationTimingType(kWebNavigationTypeFormResubmittedBackForward);
    EXPECT_EQ(returned_type, V8NavigationTimingType::Enum::kBackForward);

    returned_type = GetNavigationTimingType(kWebNavigationTypeFormResubmittedReload);
