...
// static
LayoutShiftAttribution* LayoutShiftAttribution::Create(Node* node, DOMRectReadOnly* previous, DOMRectReadOnly* current)
{
    return MakeGarbageCollected<LayoutShiftAttribution>(node, previous, current);
}

LayoutShiftAttribution::LayoutShiftAttribution(Node* node, DOMRectReadOnly* previous, DOMRectReadOnly* current)
    : node_(node)
    , previous_rect_(previous)
    , current_rect_(current)
{
}
...
ScriptValue LayoutShiftAttribution::toJSONForBinding(ScriptState* script_state) const
{
