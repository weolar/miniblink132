...
PerformanceLongTaskTiming::PerformanceLongTaskTiming(double start_time, int duration, const AtomicString& name, const AtomicString& culprit_type,
    const AtomicString& culprit_src, const AtomicString& culprit_id, const AtomicString& culprit_name, DOMWindow* source)
    : PerformanceEntry(duration, name, start_time, source)
{
    auto* attribution_entry
        = MakeGarbageCollected<TaskAttributionTiming>(performance_entry_names::kUnknown, culprit_type, culprit_src, culprit_id, culprit_name, source);
...
void PerformanceLongTaskTiming::BuildJSONValue(V8ObjectBuilder& builder) const
{
    ...
    builder.AddV8Value("attribution", ToV8Traits<IDLArray<TaskAttributionTiming>>::ToV8(builder.GetScriptState(), attribution_));
}
