...
PerformanceScriptTiming::PerformanceScriptTiming(ScriptTimingInfo* info, base::TimeTicks time_origin, bool cross_origin_isolated_capability, DOMWindow* source)
    : PerformanceEntry((info->EndTime() - info->StartTime()).InMilliseconds(), performance_entry_names::kScript,
        DOMWindowPerformance::performance(*source->ToLocalDOMWindow())->MonotonicTimeToDOMHighResTimeStamp(info->StartTime()), source)
{
    info_ = info;
    ...
    if (!info_->Window() || !source) {
        ...
        window_attribution_ = V8ScriptWindowAttribution::Enum::kOther;
    } else if (info_->Window()->GetFrame()->Tree().IsDescendantOf(source->GetFrame())) {
        window_attribution_ = V8ScriptWindowAttribution::Enum::kDescendant;
    } else if (source->GetFrame()->Tree().IsDescendantOf(info_->Window()->GetFrame())) {
        window_attribution_ = V8ScriptWindowAttribution::Enum::kAncestor;
    } else if (source->GetFrame()->Tree().Top() == info_->Window()->GetFrame()->Top()) {
...
AtomicString PerformanceScriptTiming::invoker() const
{
    ...
    case ScriptTimingInfo::InvokerType::kModuleScript: {
        ...
        if (const DOMWindow* owner_window = source()) {
            ...
            return AtomicString(To<LocalDOMWindow>(owner_window)->BaseURL().GetString());
        }
    ...
    case ScriptTimingInfo::InvokerType::kUserCallback: {
        ...
        if (info_->GetInvokerType() == ScriptTimingInfo::InvokerType::kEventHandler) {
    ...
    case ScriptTimingInfo::InvokerType::kPromiseReject: {
        ...
        if (info_->PropertyLikeName().empty()) {
            return AtomicString(info_->GetInvokerType() == ScriptTimingInfo::InvokerType::kPromiseResolve ? "Promise.resolve" : "Promise.reject");
        }
        ...
        builder.Append(info_->PropertyLikeName());
        builder.Append(".");
        builder.Append(info_->GetInvokerType() == ScriptTimingInfo::InvokerType::kPromiseResolve ? "then" : "catch");
        return builder.ToAtomicString();
    }
    }
}
...
DOMHighResTimeStamp PerformanceScriptTiming::ToMonotonicTime(base::TimeTicks time) const
{
...
DOMHighResTimeStamp PerformanceScriptTiming::forcedStyleAndLayoutDuration() const
{
...
void PerformanceScriptTiming::BuildJSONValue(V8ObjectBuilder& builder) const
{
    ...
    builder.AddNumber("forcedStyleAndLayoutDuration", forcedStyleAndLayoutDuration());
