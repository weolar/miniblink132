...
public:
    ...
    static std::unique_ptr<EventTiming> Create(LocalDOMWindow* window, const Event& event, EventTarget* original_event_target);

    explicit EventTiming(base::TimeTicks processing_start, WindowPerformance* performance, const Event& event, EventTarget* original_event_target);
    ~EventTiming();
    EventTiming(const EventTiming&) = delete;
    EventTiming& operator=(const EventTiming&) = delete;

    static void HandleInputDelay(LocalDOMWindow* window, const Event& event, base::TimeTicks processing_start);
