...
class CORE_EXPORT BackgroundTracingHelper final : public GarbageCollected<BackgroundTracingHelper> {
public:
    ...
    void MaybeEmitBackgroundTracingPerformanceMarkEvent(const PerformanceMark& mark);
    ...
    // WTF primitives.
    static SiteHashSet ParsePerformanceMarkSiteHashes(std::string_view allow_list);
...
protected:
    ...
    // and suffix.
    static std::pair<StringView, std::optional<uint32_t>> SplitMarkNameAndId(StringView mark_name);
