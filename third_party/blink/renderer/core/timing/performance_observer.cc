...
PerformanceObserver* PerformanceObserver::Create(ScriptState* script_state, V8PerformanceObserverCallback* callback)
{
    ...
    if (window) {
        ...
        return MakeGarbageCollected<PerformanceObserver>(context, DOMWindowPerformance::performance(*window), callback);
    }
    if (auto* scope = DynamicTo<WorkerGlobalScope>(context)) {
        UseCounter::Count(context, WebFeature::kPerformanceObserverForWorker);
        return MakeGarbageCollected<PerformanceObserver>(context, WorkerGlobalScopePerformance::performance(*scope), callback);
    }
    V8ThrowException::ThrowTypeError(
        script_state->GetIsolate(), ExceptionMessages::FailedToConstruct("PerformanceObserver", "No 'worker' or 'window' in current context."));
    return nullptr;
}

// static
PerformanceEntryType PerformanceObserver::supportedEntryTypeMask(ScriptState* script_state)
{
    constexpr PerformanceEntryType types_always_supported = PerformanceEntry::kMark | PerformanceEntry::kMeasure | PerformanceEntry::kResource;
    constexpr PerformanceEntryType types_supported_on_window = types_always_supported | PerformanceEntry::kNavigation | PerformanceEntry::kLongTask
        | PerformanceEntry::kPaint | PerformanceEntry::kEvent | PerformanceEntry::kFirstInput | PerformanceEntry::kElement | PerformanceEntry::kLayoutShift
        | PerformanceEntry::kLargestContentfulPaint | PerformanceEntry::kVisibilityState;
    ...
    if (RuntimeEnabledFeatures::SoftNavigationHeuristicsEnabled(execution_context)) {
        mask |= PerformanceEntry::kSoftNavigation;
    }
    if (RuntimeEnabledFeatures::LongAnimationFrameTimingEnabled(execution_context)) {
...
// static
Vector<AtomicString> PerformanceObserver::supportedEntryTypes(ScriptState* script_state)
{
    ...
    if (mask & PerformanceEntry::kBackForwardCacheRestoration) {
        supportedEntryTypes.push_back(performance_entry_names::kBackForwardCacheRestoration);
    }
    ...
    if (mask & PerformanceEntry::kLargestContentfulPaint) {
        supportedEntryTypes.push_back(performance_entry_names::kLargestContentfulPaint);
    }
...
PerformanceObserver::PerformanceObserver(ExecutionContext* execution_context, Performance* performance, V8PerformanceObserverCallback* callback)
...
void PerformanceObserver::observe(ScriptState* script_state, const PerformanceObserverInit* observer_init, ExceptionState& exception_state)
{
    if (!performance_) {
        exception_state.ThrowTypeError("Window/worker may be destroyed? Performance target is invalid.");
        return;
    }
    ...
    if (observer_init->hasEntryTypes()) {
        if (observer_init->hasType()) {
            UseCounter::Count(GetExecutionContext(), WebFeature::kPerformanceObserverTypeError);
            exception_state.ThrowTypeError("An observe() call must not include "
        ...
        for (const auto& entry_type_string : sequence) {
            PerformanceEntry::EntryType entry_type = PerformanceEntry::ToEntryTypeEnum(AtomicString(entry_type_string));
            if (!(supported_types & entry_type)) {
                String message = "The entry type '" + entry_type_string + "' does not exist or isn't supported.";
        ...
        if (observer_init->buffered() || observer_init->hasDurationThreshold()) {
            UseCounter::Count(GetExecutionContext(), WebFeature::kPerformanceObserverEntryTypesAndBuffered);
            String message = "The PerformanceObserver does not support buffered flag with "
        ...
        if (!observer_init->hasType()) {
            UseCounter::Count(GetExecutionContext(), WebFeature::kPerformanceObserverTypeError);
            exception_state.ThrowTypeError("An observe() call must include either "
        ...
        PerformanceEntryType entry_type = PerformanceEntry::ToEntryTypeEnum(entry_type_atomic_string);
        if (!(supported_types & entry_type)) {
            String message = "The entry type '" + observer_init->type() + "' does not exist or isn't supported.";
            if (GetExecutionContext()) {
                GetExecutionContext()->AddConsoleMessage(
                    MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kJavaScript, mojom::ConsoleMessageLevel::kWarning, message));
            }
        ...
        include_soft_navigation_observations_ = observer_init->includeSoftNavigationObservations();
        if (observer_init->buffered()) {
            ...
            performance_entries_.AppendVector(
                performance_->getBufferedEntriesByType(AtomicString(observer_init->type()), include_soft_navigation_observations_));
            std::sort(performance_entries_.begin(), performance_entries_.end(), PerformanceEntry::StartTimeCompareLessThan);
            is_buffered = true;
        }
        if (entry_type == PerformanceEntry::kEvent && observer_init->hasDurationThreshold()) {
    ...
    if (filter_options_ & PerformanceEntry::kLayoutShift) {
        UseCounter::Count(GetExecutionContext(), WebFeature::kLayoutShiftExplicitlyRequested);
    }
    if (filter_options_ & PerformanceEntry::kElement) {
        UseCounter::Count(GetExecutionContext(), WebFeature::kElementTimingExplicitlyRequested);
    }
    if (filter_options_ & PerformanceEntry::kLargestContentfulPaint) {
        UseCounter::Count(GetExecutionContext(), WebFeature::kLargestContentfulPaintExplicitlyRequested);
    }
    ...
    if (filter_options_ & PerformanceEntry::kVisibilityState) {
        UseCounter::Count(GetExecutionContext(), WebFeature::kVisibilityStateObserver);
    }
    if (filter_options_ & PerformanceEntry::kLongAnimationFrame) {
        UseCounter::Count(GetExecutionContext(), WebFeature::kLongAnimationFrameObserver);
    }
    ...
    is_registered_ = true;
    if (is_buffered) {
        UseCounter::Count(GetExecutionContext(), WebFeature::kPerformanceObserverBufferedFlag);
...
void PerformanceObserver::Deliver(std::optional<int> dropped_entries_count)
{
    ...
    PerformanceEntryVector performance_entries;
    performance_entries.swap(performance_entries_);
    PerformanceObserverEntryList* entry_list = MakeGarbageCollected<PerformanceObserverEntryList>(performance_entries);
...
void PerformanceObserver::ContextLifecycleStateChanged(mojom::FrameLifecycleState state)
{
