...
const char kHistogramMaxEventDuration[] = "Blink.Responsiveness.UserInteraction.MaxEventDuration";
...
constexpr char kSlowInteractionToNextPaintTraceEventName[] = "SlowInteractionToNextPaint";

const char kPageLoadInternalEventTimingClickInteractionEvents[] = "PageLoad.Internal.EventTiming.ClickInteractionEvents";
...
void EmitSlowInteractionToNextPaintTraceEvent(const ResponsivenessMetrics::EventTimestamps& event)
{
    uint64_t trace_id = base::trace_event::GetNextGlobalTraceId();
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN_WITH_TIMESTAMP0(
        kSlowInteractionToNextPaintTraceEventCategory, kSlowInteractionToNextPaintTraceEventName, trace_id, event.creation_time);
    TRACE_EVENT_NESTABLE_ASYNC_END_WITH_TIMESTAMP0(
        kSlowInteractionToNextPaintTraceEventCategory, kSlowInteractionToNextPaintTraceEventName, trace_id, event.end_time);
}

// Returns the longest event in `timestamps`.
ResponsivenessMetrics::EventTimestamps LongestEvent(const WTF::Vector<ResponsivenessMetrics::EventTimestamps>& events)
{
    DCHECK(events.size());
    return *std::max_element(
        events.begin(), events.end(), [](const ResponsivenessMetrics::EventTimestamps& left, const ResponsivenessMetrics::EventTimestamps& right) {
    ...
    const WTF::Vector<ResponsivenessMetrics::EventTimestamps>& timestamps)
{
    ...
    base::TimeDelta total_duration = timestamps[0].end_time - timestamps[0].creation_time;
    ...
    for (WTF::wtf_size_t i = 1; i < timestamps.size(); ++i) {
        ...
        if (timestamps[i].creation_time < current_end_time) {
            total_duration -= std::min(current_end_time, timestamps[i].end_time) - timestamps[i].creation_time;
        }
...
std::unique_ptr<TracedValue> UserInteractionTraceData(base::TimeDelta max_duration, base::TimeDelta total_duration, UserInteractionType interaction_type)
{
    auto traced_value = std::make_unique<TracedValue>();
    traced_value->SetInteger("maxDuration", static_cast<int>(max_duration.InMilliseconds()));
    traced_value->SetInteger("totalDuration", static_cast<int>(total_duration.InMilliseconds()));
    traced_value->SetString("interactionType", InteractionTypeToString(interaction_type));
    return traced_value;
}

void LogResponsivenessHistogram(base::TimeDelta max_event_duration, const char* suffix)
{
    base::UmaHistogramCustomTimes(base::StrCat({ kHistogramMaxEventDuration, suffix }), max_event_duration, base::Milliseconds(1), base::Seconds(60), 50);
}

} // namespace

ResponsivenessMetrics::ResponsivenessMetrics(WindowPerformance* window_performance)
    : window_performance_(window_performance)
    , pointer_flush_timer_(window_performance_->task_runner_, this, &ResponsivenessMetrics::FlushPointerTimerFired)
    , contextmenu_flush_timer_(window_performance_->task_runner_, this, &ResponsivenessMetrics::ContextmenuFlushTimerFired)
    , composition_end_flush_timer_(window_performance_->task_runner_, this, &ResponsivenessMetrics::FlushCompositionEndTimerFired)
...
void ResponsivenessMetrics::RecordUserInteractionUKM(
    LocalDOMWindow* window, UserInteractionType interaction_type, const WTF::Vector<EventTimestamps>& timestamps, uint32_t interaction_offset)
{
    if (!window) {
        return;
    }
    ...
    base::TimeTicks max_event_queued_main_thread = longest_event.queued_to_main_thread_time;
    ...
    TRACE_EVENT2("devtools.timeline", "Responsiveness.Renderer.UserInteraction", "data",
        UserInteractionTraceData(max_event_duration, total_event_duration, interaction_type), "frame", GetFrameIdForTracing(window->GetFrame()));

    EmitInteractionToNextPaintTraceEvent(longest_event, interaction_type, total_event_duration);
    // Emit a trace event when "interaction to next paint" is considered "slow"
    // according to RAIL guidelines (web.dev/rail).
    constexpr base::TimeDelta kSlowInteractionToNextPaintThreshold = base::Milliseconds(100);
    ...
    if (source_id != ukm::kInvalidSourceId && (!sampling_ || base::RandInt(kMinValueForSampling, kMaxValueForSampling) <= kUkmSamplingRate)) {
...
void ResponsivenessMetrics::RecordDragTapOrClickUKM(LocalDOMWindow* window, PointerEntryAndInfo& pointer_info)
{
    DCHECK(pointer_info.GetEntry());
    // Early return if all we got was a pointerdown.
    if (pointer_info.GetEntry()->name() == event_type_names::kPointerdown && pointer_info.GetTimeStamps().size() == 1u) {
        return;
    }
    if (pointer_info.GetEntry()->interactionId() == 0u) {
        return;
    }
    RecordUserInteractionUKM(window, pointer_info.IsDrag() ? UserInteractionType::kDrag : UserInteractionType::kTapOrClick, pointer_info.GetTimeStamps(),
...
bool ResponsivenessMetrics::SetPointerIdAndRecordLatency(PerformanceEventTiming* entry, EventTimestamps event_timestamps)
{
    const AtomicString& event_type = entry->name();
    auto pointer_id = entry->GetEventTimingReportingInfo()->pointer_id.value();
    auto* pointer_info = pointer_id_entry_map_.Contains(pointer_id) ? pointer_id_entry_map_.at(pointer_id) : nullptr;
    ...
    if (event_type == event_type_names::kPointercancel && pointer_info) {
        ...
        pointer_id_entry_map_.erase(entry->GetEventTimingReportingInfo()->pointer_id.value());
        ...
        pointer_id_entry_map_.Set(pointer_id, PointerEntryAndInfo::Create(entry, event_timestamps));
        ...
        // active.
        if (pointer_info && !pointer_info->GetEntry()->HasKnownInteractionID() && !entry->HasKnownInteractionID()) {
            UpdateInteractionId();
            entry->SetInteractionIdAndOffset(GetCurrentInteractionId(), GetInteractionCount());
        }

        if (pointer_info && pointer_info->GetEntry()->name() == event_type_names::kPointerdown) {
            ...
            if (entry->HasKnownInteractionID()) {
                pointer_down_entry->SetInteractionIdAndOffset(entry->interactionId(), entry->interactionOffset());
            }
        ...
        base::UmaHistogramEnumeration(kPageLoadInternalEventTimingClickInteractionEvents, ClickInteractionEvents::kClickDetected);
        if (pointer_id == PointerEventFactory::kReservedNonPointerId) {
            base::UmaHistogramEnumeration(kPageLoadInternalEventTimingClickInteractionEvents, ClickInteractionEvents::kKeyboardClick);
        } else if (is_last_pointerup_orphan_) {
            base::UmaHistogramEnumeration(kPageLoadInternalEventTimingClickInteractionEvents, ClickInteractionEvents::kPointerClickWithMissingPointerdownOnly);
            is_last_pointerup_orphan_ = false;
        }
        ...
        if (last_pointer_id_.has_value() && pointer_id != *last_pointer_id_ &&
            ...
            if (pointer_id_entry_map_.Contains(pointer_id)) {
                base::UmaHistogramEnumeration(kPageLoadInternalEventTimingClickInteractionEvents,
                    ClickInteractionEvents::kPointerClickPointerIdDifferFromLastPointerIdAndPointerIdExistInMap);
            } else {
                if (pointer_id_entry_map_.Contains(*last_pointer_id_)) {
                    base::UmaHistogramEnumeration(kPageLoadInternalEventTimingClickInteractionEvents,
                        ClickInteractionEvents::kPointerClickPointerIdDifferFromLastPointerIdAndOnlyLastPointerIdInMap);
                } else {
                    base::UmaHistogramEnumeration(kPageLoadInternalEventTimingClickInteractionEvents,
                        ClickInteractionEvents::kPointerClickPointerIdDifferFromLastPointerIdAndNeitherInMap);
                }
            }
        }

        if (RuntimeEnabledFeaturesBase::EventTimingHandleKeyboardEventSimulatedClickEnabled()) {
        ...
        pointer_info = nullptr;
        if (last_pointer_id_.has_value() && pointer_id_entry_map_.Contains(*last_pointer_id_)) {
        ...
        if (pointer_info) {
            ...
            if (previous_entry->name() == event_type_names::kPointerdown) {
                if (pointer_info->GetTimeStamps().size() > 1u) {
                    base::UmaHistogramEnumeration(
                        kPageLoadInternalEventTimingClickInteractionEvents, ClickInteractionEvents::kPointerClickWithPointerdownAndPointerup);
                } else {
                    base::UmaHistogramEnumeration(
                        kPageLoadInternalEventTimingClickInteractionEvents, ClickInteractionEvents::kPointerClickWithMissingPointerupOnly);
                }
            }
            ...
            if (!previous_entry->HasKnownInteractionID()) {
                UpdateInteractionId();
                previous_entry->SetInteractionIdAndOffset(GetCurrentInteractionId(), GetInteractionCount());
            }
            entry->SetInteractionIdAndOffset(previous_entry->interactionId(), previous_entry->interactionOffset());
            pointer_info->GetTimeStamps().push_back(event_timestamps);
            RecordDragTapOrClickUKM(window, *pointer_info);
            // The pointer id of the pointerdown is no longer needed.
            ...
            UpdateInteractionId();
            entry->SetInteractionIdAndOffset(GetCurrentInteractionId(), GetInteractionCount());
            RecordDragTapOrClickUKM(window, *PointerEntryAndInfo::Create(entry, event_timestamps));
            ...
            if (pointer_id != PointerEventFactory::kReservedNonPointerId) {
                ...
                base::UmaHistogramEnumeration(
                    kPageLoadInternalEventTimingClickInteractionEvents, ClickInteractionEvents::kPointerClickWithMissingPointerdownAndPointerup);
            }
        }
        // Any existing pointerup in the map cannot fire a click.
        FlushPointerup();
...
void ResponsivenessMetrics::RecordKeyboardUKM(LocalDOMWindow* window, const WTF::Vector<EventTimestamps>& event_timestamps, uint32_t interaction_offset)
{
    RecordUserInteractionUKM(window, UserInteractionType::kKeyboard, event_timestamps, interaction_offset);
}
...
// events as interactions.
void ResponsivenessMetrics::SetKeyIdAndRecordLatency(PerformanceEventTiming* entry, EventTimestamps event_timestamps)
{
    last_pointer_id_ = std::nullopt;
    ...
    if (event_type == event_type_names::kKeydown) {
        ...
        entry->SetInteractionIdAndOffset(GetCurrentInteractionId(), GetInteractionCount());
        sequence_based_keyboard_interaction_info_.SetInteractionIdAndOffset(GetCurrentInteractionId(), GetInteractionCount());
        sequence_based_keyboard_interaction_info_.AddTimestamps(event_timestamps);

        if (composition_state_ == kNonComposition) {
            InteractionInfo keydown_entry(GetCurrentInteractionId(), GetInteractionCount(), event_timestamps);
            key_code_to_interaction_info_map_.Set(key_code, std::move(keydown_entry));
        }
        last_keydown_keycode_info_ = KeycodeInfo(key_code, GetCurrentInteractionId(), GetInteractionCount());
        ...
        if (composition_state_ == kNonComposition) {
            ...
            entry->SetInteractionIdAndOffset(key_entry.GetInteractionId(), key_entry.GetInteractionOffset());
            key_entry.AddTimestamps(event_timestamps);
            RecordKeyboardUKM(window_performance_->DomWindow(), key_entry.GetTimeStamps(), key_entry.GetInteractionOffset());
        ...
        if (composition_state_ == kNonComposition && last_keydown_keycode_info_.has_value()
            && key_code_to_interaction_info_map_.find(last_keydown_keycode_info_.value().keycode) != key_code_to_interaction_info_map_.end()) {
            // Set a interaction id generated by previous keydown entry
            entry->SetInteractionIdAndOffset(GetCurrentInteractionId(), GetInteractionCount());
            key_code_to_interaction_info_map_.find(last_keydown_keycode_info_.value().keycode)->value.AddTimestamps(event_timestamps);
        }
...
void ResponsivenessMetrics::FlushKeydown()
{
    for (auto& entry : key_code_to_interaction_info_map_) {
        ...
        RecordKeyboardUKM(window_performance_->DomWindow(), entry.value.GetTimeStamps(), entry.value.GetInteractionOffset());
    }
    key_code_to_interaction_info_map_.clear();
}
...
void ResponsivenessMetrics::SetCurrentInteractionEventQueuedTimestamp(base::TimeTicks queued_time)
{
    current_interaction_event_queued_timestamp_ = queued_time;
}

base::TimeTicks ResponsivenessMetrics::CurrentInteractionEventQueuedTimestamp() const
{
...
void ResponsivenessMetrics::FlushSequenceBasedKeyboardEvents()
{
    ...
    if (!sequence_based_keyboard_interaction_info_.Empty()) {
        RecordKeyboardUKM(window, sequence_based_keyboard_interaction_info_.GetTimeStamps(), sequence_based_keyboard_interaction_info_.GetInteractionOffset());
        sequence_based_keyboard_interaction_info_.Clear();
    }
}
...
{
    return last_keydown_keycode_info_.has_value() && last_keydown_keycode_info_->keycode == key_code;
}
...
void ResponsivenessMetrics::FlushPointerup()
{
    ...
    for (const auto& item : pointer_id_entry_map_) {
        ...
        if (entry->name() == event_type_names::kPointerup || item.value->GetTimeStamps().size() > 1u) {
...
void ResponsivenessMetrics::FlushPointerdownAndPointerup()
{
    ...
    for (const auto& item : pointer_id_entry_map_) {
        PerformanceEventTiming* entry = item.value->GetEntry();
        if (entry->name() == event_type_names::kPointerdown && item.value->GetTimeStamps().size() == 1u) {
            UpdateInteractionId();
            entry->SetInteractionIdAndOffset(GetCurrentInteractionId(), GetInteractionCount());
...
void ResponsivenessMetrics::NotifyPointerdown(PerformanceEventTiming* entry) const
{
...
void ResponsivenessMetrics::KeyboardEntryAndTimestamps::Trace(Visitor* visitor) const
{
...
// them.
bool ResponsivenessMetrics::TryHandleKeyboardEventSimulatedClick(PerformanceEventTiming* entry, const std::optional<PointerId>& pointer_id)
{
    ...
    if (!last_keydown_keycode_info_.has_value()) {
        ...
        blink::UseCounter::Count(window_performance_->GetExecutionContext(), WebFeature::kEventTimingSimulatedClickWithNoKeyboardInteraction);
        return false;
    }

    entry->SetInteractionIdAndOffset(last_keydown_keycode_info_->interactionId, last_keydown_keycode_info_->interactionOffset);
