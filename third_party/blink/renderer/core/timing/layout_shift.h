...
public:
    ...
    typedef HeapVector<Member<LayoutShiftAttribution>, kMaxAttributions> AttributionList;

    static LayoutShift* Create(double start_time, double value, bool input_detected, double input_timestamp, AttributionList sources, DOMWindow* source);

    explicit LayoutShift(double start_time, double value, bool input_detected, double input_timestamp, AttributionList sources, DOMWindow* source);
    ...
    {
        return had_recent_input_;
    }
    double lastInputTime() const
    {
        return most_recent_input_timestamp_;
    }

    const AttributionList& sources() const
    {
        return sources_;
    }
...
template <> struct DowncastTraits<LayoutShift> {
