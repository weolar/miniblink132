...
public:
    explicit MockPerformance(ScriptState* script_state)
        : Performance(base::TimeTicks(), ExecutionContext::From(script_state)->CrossOriginIsolatedCapability(),
...
protected:
    ...
    bool IsRegistered()
    {
        return observer_->is_registered_;
    }
...
TEST_F(PerformanceObserverTest, ObserveWithBufferedFlag)
{
    ...
    auto* entry = LayoutShift::Create(0.0, 1234, true, 5678, LayoutShift::AttributionList(), LocalDOMWindow::From(scope.GetScriptState()));
...
TEST_F(PerformanceObserverTest, Enqueue)
{
    ...
    Persistent<PerformanceEntry> entry = PerformanceMark::Create(scope.GetScriptState(), AtomicString("m"), options, exception_state);
    EXPECT_EQ(0, NumPerformanceEntries());
