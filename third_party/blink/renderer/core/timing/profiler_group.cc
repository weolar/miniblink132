...
static constexpr int kBaseSampleIntervalMs = base::Time::kMinLowResolutionThresholdMs;
#else
...
class ProfilerGroup::ProfilingContextObserver : public GarbageCollected<ProfilingContextObserver>, public ExecutionContextLifecycleObserver {
public:
    ProfilingContextObserver(ProfilerGroup* profiler_group, ExecutionContext* context)
...
bool ProfilerGroup::CanProfile(LocalDOMWindow* local_window, ExceptionState* exception_state, ReportOptions report_options)
{
    DCHECK(local_window);
    if (!local_window->IsFeatureEnabled(mojom::blink::DocumentPolicyFeature::kJSProfiling, report_options)) {
        if (exception_state) {
            exception_state->ThrowDOMException(DOMExceptionCode::kNotAllowedError, "JS profiling is disabled by Document Policy.");
        }
        return false;
    }
...
ProfilerGroup* ProfilerGroup::From(v8::Isolate* isolate)
{
    ...
    auto* profiler_group = reinterpret_cast<ProfilerGroup*>(isolate_data->GetUserData(V8PerIsolateData::UserData::Key::kProfileGroup));
    if (!profiler_group) {
        profiler_group = MakeGarbageCollected<ProfilerGroup>(isolate);
        isolate_data->SetUserData(V8PerIsolateData::UserData::Key::kProfileGroup, profiler_group);
    }
...
void ProfilerGroup::OnProfilingContextAdded(ExecutionContext* context)
{
    ...
    // profiler alive.
    auto* observer = MakeGarbageCollected<ProfilingContextObserver>(this, context);
...
void ProfilerGroup::DispatchSampleBufferFullEvent(String profiler_id)
{
    for (const auto& profiler : profilers_) {
        if (profiler->ProfilerId() == profiler_id) {
            profiler->DispatchEvent(*Event::Create(event_type_names::kSamplebufferfull));
            break;
        }
    }
}

Profiler* ProfilerGroup::CreateProfiler(
    ScriptState* script_state, const ProfilerInitOptions& init_options, base::TimeTicks time_origin, ExceptionState& exception_state)
{
    DCHECK_EQ(script_state->GetIsolate(), isolate_);
    DCHECK(init_options.hasSampleInterval());

    const base::TimeDelta sample_interval = base::Milliseconds(init_options.sampleInterval());
    const int64_t sample_interval_us = sample_interval.InMicroseconds();

    if (sample_interval_us < 0 || sample_interval_us > std::numeric_limits<int>::max()) {
    ...
    v8::CpuProfilingStatus status = cpu_profiler_->StartProfiling(V8String(isolate_, profiler_id),
        v8::CpuProfilingOptions(v8::kLeafNodeLineNumbers, init_options.maxBufferSize(), static_cast<int>(sample_interval_us), script_state->GetContext()),
        std::make_unique<DiscardedSamplesDelegate>(this, profiler_id));

    switch (status) {
    case v8::CpuProfilingStatus::kErrorTooManyProfilers: {
        exception_state.ThrowTypeError("Reached maximum concurrent amount of profilers");
        return nullptr;
    }
    ...
    case v8::CpuProfilingStatus::kStarted: {
        ...
        scoped_refptr<const SecurityOrigin> source_origin(execution_context->GetSecurityOrigin());
        ...
        // interval.
        int effective_sample_interval_ms = static_cast<int>(sample_interval.InMilliseconds());
        if (effective_sample_interval_ms % kBaseSampleIntervalMs != 0 || effective_sample_interval_ms == 0) {
            effective_sample_interval_ms += (kBaseSampleIntervalMs - effective_sample_interval_ms % kBaseSampleIntervalMs);
        }

        auto* profiler = MakeGarbageCollected<Profiler>(this, script_state, profiler_id, effective_sample_interval_ms, source_origin, time_origin);
...
void ProfilerGroup::OnProfilingContextDestroyed(ProfilingContextObserver* observer)
{
...
void ProfilerGroup::InitV8Profiler()
{
    ...
    cpu_profiler_ = v8::CpuProfiler::New(isolate_, v8::kStandardNaming, v8::kEagerLogging);
#if BUILDFLAG(IS_WIN)
...
#endif // BUILDFLAG(IS_WIN)
    cpu_profiler_->SetSamplingInterval(kBaseSampleIntervalMs * base::Time::kMicrosecondsPerMillisecond);
}
...
void ProfilerGroup::StopProfiler(ScriptState* script_state, Profiler* profiler, ScriptPromiseResolver<ProfilerTrace>* resolver)
{
    DCHECK(cpu_profiler_);
    DCHECK(!profiler->stopped());

    v8::Local<v8::String> profiler_id = V8String(isolate_, profiler->ProfilerId());
    auto* profile = cpu_profiler_->StopProfiling(profiler_id);
    auto* trace = ProfilerTraceBuilder::FromProfile(script_state, profile, profiler->SourceOrigin(), profiler->TimeOrigin());
...
void ProfilerGroup::CancelProfilerAsync(ScriptState* script_state, Profiler* profiler)
{
    DCHECK(IsMainThread());
    DCHECK(cpu_profiler_);
    DCHECK(!profiler->stopped());
    ...
    ThreadScheduler::Current()->V8TaskRunner()->PostTask(
        FROM_HERE, WTF::BindOnce(&ProfilerGroup::StopDetachedProfiler, WrapPersistent(this), profiler->ProfilerId()));
}
