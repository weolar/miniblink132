...
public:
    explicit TestPerformance(ScriptState* script_state)
        : Performance(base::TimeTicks() + base::Milliseconds(kTimeOrigin), ExecutionContext::From(script_state)->CrossOriginIsolatedCapability(),
            ExecutionContext::From(script_state)->GetTaskRunner(TaskType::kPerformanceTimeline))
    ...
    base::TimeTicks MsAfterTimeOrigin(uint32_t ms)
    {
        ...
        return loader->GetTiming().ReferenceMonotonicTime() + base::Milliseconds(ms);
    }
...
protected:
    ...
    {
        v8::Local<v8::Function> callback = v8::Function::New(script_state->GetContext(), nullptr).ToLocalChecked();
        base_ = MakeGarbageCollected<TestPerformance>(script_state);
        cb_ = V8PerformanceObserverCallback::Create(callback);
        observer_ = MakeGarbageCollected<PerformanceObserver>(ExecutionContext::From(script_state), base_, cb_);
    }
    ...
    void CheckBackForwardCacheRestoration(PerformanceEntryVector entries)
    {
        ...
        EXPECT_EQ(2, base::ranges::count(entries, "back-forward-cache-restoration", &PerformanceEntry::entryType));

        // Retain only back forward cache restoration entries.
        entries.erase(std::remove_if(
                          entries.begin(), entries.end(), [](const PerformanceEntry* e) -> bool { return e->entryType() != "back-forward-cache-restoration"; }),
            entries.end());

        BackForwardCacheRestoration* b1 = static_cast<BackForwardCacheRestoration*>(entries[0].Get());
        ...
        BackForwardCacheRestoration* b2 = static_cast<BackForwardCacheRestoration*>(entries[1].Get());
...
TEST_F(PerformanceTest, AddLongTaskTiming)
{
    ...
    base_->AddLongTaskTiming(base::TimeTicks() + base::Seconds(1234), base::TimeTicks() + base::Seconds(5678), AtomicString("window"),
        AtomicString("same-origin"), AtomicString("www.foo.com/bar"), g_empty_atom, g_empty_atom);
...
TEST_F(PerformanceTest, BackForwardCacheRestoration)
{
    ...
    EXPECT_TRUE(base_->HasPerformanceObserverFor(PerformanceEntry::kBackForwardCacheRestoration));

    base_->AddBackForwardCacheRestoration(base::TimeTicks() + base::Milliseconds(kEvent1Time), base::TimeTicks() + base::Milliseconds(kEvent1PageshowStart),
        base::TimeTicks() + base::Milliseconds(kEvent1PageshowEnd));

    base_->AddBackForwardCacheRestoration(base::TimeTicks() + base::Milliseconds(kEvent2Time), base::TimeTicks() + base::Milliseconds(kEvent2PageshowStart),
    ...
    entries = base_->getEntriesByType(performance_entry_names::kBackForwardCacheRestoration);
    CheckBackForwardCacheRestoration(entries);
}
...
TEST_F(PerformanceTest, InsertEntryOnEmptyBuffer)
{
    ...
    PerformanceEventTiming::EventTimingReportingInfo info {
        .creation_time = base_->MsAfterTimeOrigin(0), .processing_start_time = base_->MsAfterTimeOrigin(0), .processing_end_time = base_->MsAfterTimeOrigin(0)
    };

    PerformanceEventTiming* test_entry
        = PerformanceEventTiming::Create(AtomicString("event"), info, false, nullptr, LocalDOMWindow::From(scope.GetScriptState()));

    base_->InsertEntryIntoSortedBuffer(test_buffer_, *test_entry, Performance::kDoNotRecordSwaps);
...
TEST_F(PerformanceTest, InsertEntryOnExistingBuffer)
{
    ...
    for (int i = 0; i < 3; i++) {
        double tmp = 1.0;
        PerformanceEventTiming::EventTimingReportingInfo info { .creation_time = base_->MsAfterTimeOrigin(tmp * i),
    ...
    PerformanceEventTiming::EventTimingReportingInfo info {
        .creation_time = base_->MsAfterTimeOrigin(1), .processing_start_time = base_->MsAfterTimeOrigin(0), .processing_end_time = base_->MsAfterTimeOrigin(0)
    };
    PerformanceEventTiming* test_entry
        = PerformanceEventTiming::Create(AtomicString("event"), info, false, nullptr, LocalDOMWindow::From(scope.GetScriptState()));
    ...
    sorted_buffer_.push_back(*test_entry);
    std::sort(sorted_buffer_.begin(), sorted_buffer_.end(), PerformanceEntry::StartTimeCompareLessThan);
...
TEST_F(PerformanceTest, MergePerformanceEntryVectorsTest)
{
    ...
    all_entries = MergePerformanceEntryVectors(all_entries, first_vector, g_null_atom);
    all_entries = MergePerformanceEntryVectors(all_entries, second_vector, g_null_atom);

    std::sort(test_vector.begin(), test_vector.end(), PerformanceEntry::StartTimeCompareLessThan);
