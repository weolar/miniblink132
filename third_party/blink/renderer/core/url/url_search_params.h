...
using URLSearchParamsInit = V8UnionUSVStringOrUSVStringSequenceSequenceOrUSVStringUSVStringRecord;

class CORE_EXPORT URLSearchParams final : public ScriptWrappable, public PairSyncIterable<URLSearchParams> {
    DEFINE_WRAPPERTYPEINFO();

public:
    static URLSearchParams* Create(const URLSearchParamsInit* init, ExceptionState& exception_state);
    static URLSearchParams* Create(const Vector<std::pair<String, String>>&, ExceptionState&);
    static URLSearchParams* Create(const Vector<Vector<String>>&, ExceptionState&);

    static URLSearchParams* Create(const String& query_string, DOMURL* url_object = nullptr)
    {
    ...
    void deleteAllWithNameOrTuple(ExecutionContext* execution_context, const String& name);
    void deleteAllWithNameOrTuple(ExecutionContext* execution_context, const String& name, const String& val);
    ...
    bool has(ExecutionContext* execution_context, const String& name, const String& val) const;
