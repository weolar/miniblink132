...
class URLSearchParamsIterationSource final : public PairSyncIterable<URLSearchParams>::IterationSource {
public:
    ...
    bool FetchNextItem(ScriptState*, String& key, String& value, ExceptionState&) override
    {
...
bool CompareParams(const std::pair<String, String>& a, const std::pair<String, String>& b)
{
...
URLSearchParams* URLSearchParams::Create(const URLSearchParamsInit* init, ExceptionState& exception_state)
{
    DCHECK(init);
    ...
    case URLSearchParamsInit::ContentType::kUSVStringSequenceSequence:
        return URLSearchParams::Create(init->GetAsUSVStringSequenceSequence(), exception_state);
    case URLSearchParamsInit::ContentType::kUSVStringUSVStringRecord:
        return URLSearchParams::Create(init->GetAsUSVStringUSVStringRecord(), exception_state);
    }
    NOTREACHED();
}

URLSearchParams* URLSearchParams::Create(const Vector<Vector<String>>& init, ExceptionState& exception_state)
{
    ...
    for (unsigned i = 0; i < init.size(); ++i) {
        ...
        if (pair.size() != 2) {
            exception_state.ThrowTypeError(ExceptionMessages::FailedToConstruct("URLSearchParams", "Sequence initializer must only contain pair elements"));
            return nullptr;
        }
...
URLSearchParams* URLSearchParams::Create(const Vector<std::pair<String, String>>& init, ExceptionState& exception_state)
{
    URLSearchParams* instance = MakeGarbageCollected<URLSearchParams>(String());
    if (init.empty())
        return instance;
...
static String DecodeString(String input)
{
    ...
    return DecodeURLEscapeSequences(input.Replace('+', ' '), DecodeURLMode::kUTF8);
}
...
void URLSearchParams::SetInputWithoutUpdate(const String& query_string)
{
    ...
    while (start < query_string_length) {
        ...
        if (name_value_end > start) {
            ...
            String name = DecodeString(query_string.Substring(name_start, end_of_name - name_start));
            String value;
            if (end_of_name != name_value_end)
                value = DecodeString(query_string.Substring(end_of_name + 1, name_value_end - end_of_name - 1));
            if (value.IsNull())
                value = "";
...
String URLSearchParams::toString() const
{
    ...
    return String(base::span<const char>(encoded_data.data(), encoded_data.size()));
}
...
void URLSearchParams::AppendWithoutUpdate(const String& name, const String& value)
{
...
void URLSearchParams::deleteAllWithNameOrTuple(ExecutionContext* execution_context, const String& name)
{
    deleteAllWithNameOrTuple(execution_context, name, String());
}

void URLSearchParams::deleteAllWithNameOrTuple(ExecutionContext* execution_context, const String& name, const String& val)
{
    String value = val;
    if (!RuntimeEnabledFeatures::URLSearchParamsHasAndDeleteMultipleArgsEnabled()) {
        value = String();
    }
    ...
    if (indices_to_remove_with_name_value != indices_to_remove_with_name) {
        UseCounter::Count(execution_context, WebFeature::kURLSearchParamsDeleteFnBehaviourDiverged);
    }

    for (auto it = indices_to_remove_with_name_value.rbegin(); it != indices_to_remove_with_name_value.rend(); ++it) {
        params_.EraseAt(*it);
    }
...
bool URLSearchParams::has(ExecutionContext* execution_context, const String& name) const
{
    return has(execution_context, name, String());
}

bool URLSearchParams::has(ExecutionContext* execution_context, const String& name, const String& val) const
{
    String value = val;
    if (!RuntimeEnabledFeatures::URLSearchParamsHasAndDeleteMultipleArgsEnabled()) {
        value = String();
    ...
    if (found_match_using_name_and_value != found_match_using_name) {
        UseCounter::Count(execution_context, WebFeature::kURLSearchParamsHasFnBehaviourDiverged);
    }
...
void URLSearchParams::EncodeAsFormData(Vector<char>& encoded_data) const
{
    for (const auto& param : params_) {
        FormDataEncoder::AddKeyValuePairAsFormData(
            encoded_data, param.first.Utf8(), param.second.Utf8(), EncodedFormData::kFormURLEncoded, FormDataEncoder::kDoNotNormalizeCRLF);
    }
}
...
PairSyncIterable<URLSearchParams>::IterationSource* URLSearchParams::CreateIterationSource(ScriptState*, ExceptionState&)
{
