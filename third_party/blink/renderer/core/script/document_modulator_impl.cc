...
ModuleScriptFetcher* DocumentModulatorImpl::CreateModuleScriptFetcher(ModuleScriptCustomFetchType custom_fetch_type, base::PassKey<ModuleScriptLoader> pass_key)
{
    DCHECK_EQ(ModuleScriptCustomFetchType::kNone, custom_fetch_type);
    return MakeGarbageCollected<DocumentModuleScriptFetcher>(GetExecutionContext(), pass_key);
}
...
void DocumentModulatorImpl::MergeExistingAndNewImportMaps(ImportMap* new_import_map)
{
    if (!RuntimeEnabledFeatures::MultipleImportMapsEnabled()) {
    ...
    import_map_->MergeExistingAndNewImportMaps(new_import_map, scoped_resolved_module_map_, toplevel_resolved_module_set_, *GetExecutionContext());
}

// https://html.spec.whatwg.org/C#add-module-to-resolved-module-set
void DocumentModulatorImpl::AddModuleToResolvedModuleSet(std::optional<AtomicString> referring_script_url, AtomicString specifier)
{
    if (!RuntimeEnabledFeatures::MultipleImportMapsEnabled()) {
        return;
    }
    ...
    Vector<AtomicString> referring_script_prefixes = FindUrlPrefixes(referring_script_url.value());
    for (auto referring_script_prefix : referring_script_prefixes) {
        const auto& current_set_it = scoped_resolved_module_map_.find(referring_script_prefix);
        ...
        if (current_set_it != scoped_resolved_module_map_.end()) {
            ...
            current_set = &(scoped_resolved_module_map_.insert(referring_script_prefix, HashSet<AtomicString>()).stored_value->value);
        }
