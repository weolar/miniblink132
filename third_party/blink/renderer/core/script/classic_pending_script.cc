...
InlineScriptStreamer* GetInlineScriptStreamer(const String& source, Document& document)
{
    ScriptableDocumentParser* scriptable_parser = document.GetScriptableDocumentParser();
...
ClassicPendingScript* ClassicPendingScript::Fetch(const KURL& url, Document& element_document, const ScriptFetchOptions& options,
    CrossOriginAttributeValue cross_origin, const WTF::TextEncoding& encoding, ScriptElementBase* element, FetchParameters::DeferOption defer,
    scheduler::TaskAttributionInfo* parent_task)
{
    ExecutionContext* context = element_document.GetExecutionContext();
    FetchParameters params(options.CreateFetchParameters(url, context->GetSecurityOrigin(), context->GetCurrentWorld(), cross_origin, encoding, defer));

    ClassicPendingScript* pending_script = MakeGarbageCollected<ClassicPendingScript>(element, TextPosition::MinimumPosition(), KURL(), KURL(), String(),
    ...
    // screen.
    pending_script->intervened_ = MaybeDisallowFetchForDocWrittenScript(params, element_document);
    ...
    v8_compile_hints::V8CrowdsourcedCompileHintsProducer* compile_hints_producer = nullptr;
    v8_compile_hints::V8CrowdsourcedCompileHintsConsumer* compile_hints_consumer = nullptr;
    ...
    ScriptResource::Fetch(params, element_document.Fetcher(), pending_script, context->GetIsolate(), ScriptResource::kAllowStreaming, compile_hints_producer,
        compile_hints_consumer, v8_compile_hints::GetMagicCommentMode(element_document.GetExecutionContext()));
    pending_script->CheckState();
    return pending_script;
}

ClassicPendingScript* ClassicPendingScript::CreateInline(ScriptElementBase* element, const TextPosition& starting_position, const KURL& source_url,
    const KURL& base_url, const String& source_text, ScriptSourceLocationType source_location_type, const ScriptFetchOptions& options,
    scheduler::TaskAttributionInfo* parent_task)
{
    ClassicPendingScript* pending_script = MakeGarbageCollected<ClassicPendingScript>(
        element, starting_position, source_url, base_url, source_text, source_location_type, options, /*is_external=*/false, parent_task);
    pending_script->CheckState();
    return pending_script;
}

ClassicPendingScript::ClassicPendingScript(ScriptElementBase* element, const TextPosition& starting_position, const KURL& source_url_for_inline_script,
    const KURL& base_url_for_inline_script, const String& source_text_for_inline_script, ScriptSourceLocationType source_location_type,
    const ScriptFetchOptions& options, bool is_external, scheduler::TaskAttributionInfo* parent_task)
...
void ClassicPendingScript::RecordThirdPartyRequestWithCookieIfNeeded(const ResourceResponse& response) const
{
    ...
    scoped_refptr<const SecurityOrigin> top_frame_origin = element_document->TopFrameOrigin();
    if (!top_frame_origin) {
        return;
    }
    ...
    // Therefore...
    String doc_registrable_domain = execution_context->GetSecurityOrigin()->RegistrableDomain();
    // Ignore third-party frames.
    if (top_frame_origin->RegistrableDomain() != doc_registrable_domain) {
        return;
    }

    scoped_refptr<SecurityOrigin> script_origin = SecurityOrigin::Create(response.ResponseUrl());
    // Ignore first-party requests.
    if (doc_registrable_domain == script_origin->RegistrableDomain()) {
        return;
    }

    execution_context->CountUse(mojom::blink::WebFeature::kUndeferrableThirdPartySubresourceRequestWithCookie);
}
...
bool ClassicPendingScript::IsEligibleForLowPriorityAsyncScriptExecution() const
{
    DCHECK_EQ(GetSchedulingType(), ScriptSchedulingType::kAsync);

    static const bool feature_enabled = base::FeatureList::IsEnabled(features::kLowPriorityAsyncScriptExecution);
    if (!feature_enabled) {
        return false;
    }
    ...
    const bool main_frame_only = features::kLowPriorityAsyncScriptExecutionMainFrameOnlyParam.Get();
    if (main_frame_only && !element_document->IsInOutermostMainFrame()) {
        return false;
    }

    const base::TimeDelta feature_limit = features::kLowPriorityAsyncScriptExecutionFeatureLimitParam.Get();
    ...
    if (top_document.Loader() && top_document.Loader()->IsReloadedOrFormSubmitted()) {
        return false;
    }

    // Check if LCP influencing scripts are to be excluded.
    const bool exclude_lcp_influencers = features::kLowPriorityAsyncScriptExecutionExcludeLcpInfluencersParam.Get();
    ...
    const bool disable_when_lcp_not_in_html = features::kLowPriorityAsyncScriptExecutionDisableWhenLcpNotInHtmlParam.Get();
    ...
    const bool cross_site_only = features::kLowPriorityAsyncScriptExecutionCrossSiteOnlyParam.Get();
    if (cross_site_only && GetResource() && element_document->GetExecutionContext()) {
        scoped_refptr<const SecurityOrigin> url_origin = SecurityOrigin::Create(GetResource()->Url());
        if (url_origin->IsSameSiteWith(element_document->GetExecutionContext()->GetSecurityOrigin())) {
            return false;
        }
    }
    ...
    bool is_ad_resource = GetResource() && GetResource()->GetResourceRequest().IsAdResource();
    ...
    const bool exclude_non_parser_inserted = features::kLowPriorityAsyncScriptExecutionExcludeNonParserInsertedParam.Get();
    if (exclude_non_parser_inserted && !parser_inserted()) {
        return false;
    }

    const bool exclude_scripts_via_document_write = features::kLowPriorityAsyncScriptExecutionExcludeDocumentWriteParam.Get();
    if (exclude_scripts_via_document_write && is_in_document_write()) {
        return false;
    }

    const bool opt_out_low = features::kLowPriorityAsyncScriptExecutionOptOutLowFetchPriorityHintParam.Get();
    const bool opt_out_auto = features::kLowPriorityAsyncScriptExecutionOptOutAutoFetchPriorityHintParam.Get();
    const bool opt_out_high = features::kLowPriorityAsyncScriptExecutionOptOutHighFetchPriorityHintParam.Get();
...
void ClassicPendingScript::NotifyFinished(Resource* resource)
{
    ...
    ExecutionContext* execution_context = OriginalExecutionContext();
    Document* element_document = OriginalElementDocument();
    if (!execution_context || execution_context->IsContextDestroyed() || !element_document || !element_document->IsActive()) {
        AdvanceReadyState(kErrorOccurred);
        return;
    }

    SubresourceIntegrityHelper::DoReport(*execution_context, resource->IntegrityReportInfo());

    bool integrity_failure = false;
    if (!options_.GetIntegrityMetadata().empty() || resource->ForceIntegrityChecks()) {
        integrity_failure = !resource->PassedIntegrityChecks();
    }

    if (intervened_) {
        CrossOriginAttributeValue cross_origin = GetCrossOriginAttributeValue(GetElement()->CrossOriginAttributeValue());
        PossiblyFetchBlockedDocWriteScript(resource, *element_document, options_, cross_origin);
    }
    ...
    const bool mime_type_failure = !AllowedByNosniff::MimeTypeAsScript(
        fetcher->GetUseCounter(), &fetcher->GetConsoleLogger(), resource->GetResponse(), AllowedByNosniff::MimeTypeCheck::kLaxForElement);

    TRACE_EVENT_WITH_FLOW1(TRACE_DISABLED_BY_DEFAULT("v8.compile"), "ClassicPendingScript::NotifyFinished", this, TRACE_EVENT_FLAG_FLOW_OUT, "data",
        [&](perfetto::TracedValue context) { inspector_parse_script_event::Data(std::move(context), resource->InspectorId(), resource->Url().GetString()); });

    // Ordinal ErrorOccurred(), SRI, and MIME check are all considered as network
    // errors in the Fetch spec.
    bool error_occurred = resource->ErrorOccurred() || integrity_failure || mime_type_failure;
    if (error_occurred) {
        AdvanceReadyState(kErrorOccurred);
        return;
    }
    ...
    classic_script_ = ClassicScript::CreateFromResource(To<ScriptResource>(resource), options_);
    ...
    if (classic_script_->CacheConsumer()) {
        ...
        classic_script_->CacheConsumer()->NotifyClientWaiting(this, classic_script_, execution_context->GetTaskRunner(TaskType::kNetworking));
    } else {
...
ClassicScript* ClassicPendingScript::GetSource() const
{
    ...
    if (!is_external_) {
        ...
        Document* element_document = OriginalElementDocument();
        if (source_location_type_ == ScriptSourceLocationType::kInline && element_document && element_document->IsActive()) {
            streamer = GetInlineScriptStreamer(source_text_for_inline_script_, *element_document);
        }

        DCHECK(!GetResource());
        ScriptStreamer::RecordStreamingHistogram(GetSchedulingType(), streamer, ScriptStreamer::NotStreamingReason::kInlineScript);

        return ClassicScript::Create(source_text_for_inline_script_, ClassicScript::StripFragmentIdentifier(source_url_for_inline_script_),
            base_url_for_inline_script_, options_, source_location_type_, SanitizeScriptErrors::kDoNotSanitize, nullptr, StartingPosition(),
            streamer ? ScriptStreamer::NotStreamingReason::kInvalid : ScriptStreamer::NotStreamingReason::kInlineScript, streamer);
    }
    ...
    ScriptStreamer::RecordStreamingHistogram(GetSchedulingType(), classic_script_->Streamer(), classic_script_->NotStreamingReason());

    TRACE_EVENT_WITH_FLOW1(TRACE_DISABLED_BY_DEFAULT("v8.compile"), "ClassicPendingScript::GetSource", this, TRACE_EVENT_FLAG_FLOW_IN, "not_streamed_reason",
...
void ClassicPendingScript::AdvanceReadyState(ReadyState new_ready_state)
{
    ...
    switch (ready_state_) {
    case kWaitingForResource:
        CHECK(new_ready_state == kReady || new_ready_state == kErrorOccurred || new_ready_state == kWaitingForCacheConsumer);
        break;
    case kWaitingForCacheConsumer:
