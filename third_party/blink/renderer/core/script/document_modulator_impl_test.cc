...
public:
    ...
    DocumentModulatorImplTest& operator=(const DocumentModulatorImplTest&) = delete;
    void SetUp() override;
...
TEST_F(DocumentModulatorImplTest, ResolveModuleSpecifier)
{
    ...
    KURL resolved = modulator_->ResolveModuleSpecifier("http:example.com\\pears.mjs", NullURL(), /*failure_reason=*/nullptr);
    ...
    EXPECT_TRUE(modulator_
                    ->ResolveModuleSpecifier("/limes.jsx", base_url,
                        /*failure_reason=*/nullptr)
                    .IsValid());
    EXPECT_TRUE(modulator_->ResolveModuleSpecifier("data:text/javascript,export default 'grapes';", NullURL(), /*failure_reason=*/nullptr).IsValid());
    EXPECT_TRUE(
        modulator_->ResolveModuleSpecifier("blob:https://whatwg.org/d0360e2f-caee-469f-9a2f-87d5b0456f6f", KURL(), /*failure_reason=*/nullptr).IsValid());

    // "The following are valid module specifiers according to the above
    // algorithm, but will invariably cause failures when they are fetched:"
    EXPECT_TRUE(modulator_->ResolveModuleSpecifier("javascript:export default 'artichokes';", NullURL(), /*failure_reason=*/nullptr).IsValid());
    EXPECT_TRUE(modulator_->ResolveModuleSpecifier("data:text/plain,export default 'kale';", NullURL(), /*failure_reason=*/nullptr).IsValid());
    EXPECT_TRUE(modulator_
                    ->ResolveModuleSpecifier("about:legumes", NullURL(),
                        /*failure_reason=*/nullptr)
                    .IsValid());
    EXPECT_TRUE(modulator_
                    ->ResolveModuleSpecifier("wss://example.com/celery", NullURL(),
                        /*failure_reason=*/nullptr)
                    .IsValid());
