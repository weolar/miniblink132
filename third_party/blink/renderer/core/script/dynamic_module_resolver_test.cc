...
constexpr const char* kTestDependencyURLJSON = "https://example.com/dependency.json";
...
public:
    ...
    bool fetch_tree_was_called() const
    {
        return fetch_tree_was_called_;
    }
...
private:
    ...
    ScriptState* GetScriptState() final
    {
        return script_state_.Get();
    }

    ModuleScript* GetFetchedModuleScript(const KURL& url, ModuleType module_type) final
    {
        EXPECT_EQ(TestReferrerURL(), url);
        ModuleScript* module_script = JSModuleScript::CreateForTest(this, v8::Local<v8::Module>(), url);
        return module_script;
    }

    KURL ResolveModuleSpecifier(const String& module_request, const KURL& base_url, String*) final
    {
    ...
    void FetchTree(const KURL& url, ModuleType module_type, ResourceFetcher*, mojom::blink::RequestContextType, network::mojom::RequestDestination,
        const ScriptFetchOptions&, ModuleScriptCustomFetchType custom_fetch_type, ModuleTreeClient* client, String) final
    {
...
class CaptureExportedStringFunction final : public ThenCallable<IDLAny, CaptureExportedStringFunction> {
public:
    ...
    const String& CapturedValue() const
    {
    ...
    void React(ScriptState* script_state, ScriptValue value)
    {
        ...
        v8::Local<v8::Object> module_namespace = value.V8Value()->ToObject(context).ToLocalChecked();
        v8::Local<v8::Value> exported_value = module_namespace->Get(context, V8String(isolate, export_name_)).ToLocalChecked();
        captured_value_ = ToCoreString(isolate, exported_value->ToString(context).ToLocalChecked());
    }
...
class DynamicModuleResolverTestNotReached final : public ThenCallable<IDLAny, DynamicModuleResolverTestNotReached> {
public:
...
TEST_F(DynamicModuleResolverTest, ResolveSuccess)
{
    ...
    auto* modulator = MakeGarbageCollected<DynamicModuleResolverTestModulator>(scope.GetScriptState());
    modulator->SetExpectedFetchTreeURL(TestDependencyURL());

    auto* promise_resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLAny>>(scope.GetScriptState());
    auto promise = promise_resolver->Promise();

    auto* capture = MakeGarbageCollected<CaptureExportedStringFunction>("foo");
    promise.React(scope.GetScriptState(), capture, MakeGarbageCollected<DynamicModuleResolverTestNotReached>());

    auto* resolver = MakeGarbageCollected<DynamicModuleResolver>(modulator);
    ModuleRequest module_request("./dependency.js", TextPosition::MinimumPosition(), Vector<ImportAttribute>());
    resolver->ResolveDynamically(module_request, TestReferrerScriptInfo(), promise_resolver);

    scope.PerformMicrotaskCheckpoint();
    EXPECT_FALSE(capture->WasCalled());

    v8::Local<v8::Module> record = ModuleTestBase::CompileModule(scope.GetScriptState(), "export const foo = 'hello';", TestReferrerURL());
    ModuleScript* module_script = JSModuleScript::CreateForTest(modulator, record, TestDependencyURL());
    EXPECT_TRUE(ModuleRecord::Instantiate(scope.GetScriptState(), record, TestReferrerURL()).IsEmpty());
...
TEST_F(DynamicModuleResolverTest, ResolveJSONModuleSuccess)
{
    ...
    auto* resolver = MakeGarbageCollected<DynamicModuleResolver>(modulator);
    Vector<ImportAttribute> import_attributes { ImportAttribute("type", "json", TextPosition::MinimumPosition()) };
    ModuleRequest module_request("./dependency.json", TextPosition::MinimumPosition(), import_attributes);
    resolver->ResolveDynamically(module_request, TestReferrerScriptInfo(), promise_resolver);
...
TEST_F(DynamicModuleResolverTest, ResolveSpecifierFailure)
{
    ...
    promise.React(scope.GetScriptState(), MakeGarbageCollected<DynamicModuleResolverTestNotReached>(), capture);

    auto* resolver = MakeGarbageCollected<DynamicModuleResolver>(modulator);
    ModuleRequest module_request("invalid-specifier", TextPosition::MinimumPosition(), Vector<ImportAttribute>());
    resolver->ResolveDynamically(module_request, TestReferrerScriptInfo(), promise_resolver);
...
TEST_F(DynamicModuleResolverTest, ResolveModuleTypeFailure)
{
    ...
    auto* resolver = MakeGarbageCollected<DynamicModuleResolver>(modulator);
    Vector<ImportAttribute> import_attributes { ImportAttribute("type", "notARealType", TextPosition::MinimumPosition()) };
    ModuleRequest module_request("./dependency.js", TextPosition::MinimumPosition(), import_attributes);
    resolver->ResolveDynamically(module_request, TestReferrerScriptInfo(), promise_resolver);
...
TEST_F(DynamicModuleResolverTest, ExceptionThrown)
{
    ...
    v8::Local<v8::Module> record = ModuleTestBase::CompileModule(scope.GetScriptState(), "throw Error('bar')", TestReferrerURL());
    ModuleScript* module_script = JSModuleScript::CreateForTest(modulator, record, TestDependencyURL());
    EXPECT_TRUE(ModuleRecord::Instantiate(scope.GetScriptState(), record, TestReferrerURL()).IsEmpty());
    modulator->ResolveTreeFetch(module_script);
...
TEST_F(DynamicModuleResolverTest, ResolveWithNullReferrerScriptSuccess)
{
    ...
    auto* resolver = MakeGarbageCollected<DynamicModuleResolver>(modulator);
    ModuleRequest module_request("./dependency.js", TextPosition::MinimumPosition(), Vector<ImportAttribute>());
    resolver->ResolveDynamically(module_request, ReferrerScriptInfo(), promise_resolver);

    scope.PerformMicrotaskCheckpoint();
    EXPECT_FALSE(capture->WasCalled());

    v8::Local<v8::Module> record = ModuleTestBase::CompileModule(scope.GetScriptState(), "export const foo = 'hello';", TestDependencyURL());
    ModuleScript* module_script = JSModuleScript::CreateForTest(modulator, record, TestDependencyURL());
    EXPECT_TRUE(ModuleRecord::Instantiate(scope.GetScriptState(), record, TestDependencyURL()).IsEmpty());
    modulator->ResolveTreeFetch(module_script);
...
TEST_F(DynamicModuleResolverTest, ResolveWithReferrerScriptInfoBaseURL)
{
    ...
    auto* modulator = MakeGarbageCollected<DynamicModuleResolverTestModulator>(scope.GetScriptState());
    modulator->SetExpectedFetchTreeURL(KURL("https://example.com/correct/dependency.js"));

    auto* promise_resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLAny>>(scope.GetScriptState());
    auto* resolver = MakeGarbageCollected<DynamicModuleResolver>(modulator);
    KURL correct_base_url("https://example.com/correct/baz.js");
    ModuleRequest module_request("./dependency.js", TextPosition::MinimumPosition(), Vector<ImportAttribute>());
    resolver->ResolveDynamically(module_request, ReferrerScriptInfo(correct_base_url, ScriptFetchOptions()), promise_resolver);
