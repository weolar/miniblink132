...
// module map.
class CORE_EXPORT SingleModuleClient : public GarbageCollected<SingleModuleClient>, public NameClient {
public:
...
class CORE_EXPORT ModuleTreeClient : public GarbageCollected<ModuleTreeClient>, public NameClient {
public:
...
class CORE_EXPORT Modulator : public GarbageCollected<Modulator>, public V8PerContextData::Data, public NameClient {
public:
    ...
    virtual void FetchTree(const KURL&, ModuleType, ResourceFetcher* fetch_client_settings_object_fetcher, mojom::blink::RequestContextType context_type,
        network::mojom::RequestDestination destination, const ScriptFetchOptions&, ModuleScriptCustomFetchType, ModuleTreeClient*,
        String referrer = Referrer::ClientReferrerString())
        = 0;
    ...
    virtual void FetchSingle(const ModuleScriptFetchRequest&, ResourceFetcher* fetch_client_settings_object_fetcher, ModuleGraphLevel,
        ModuleScriptCustomFetchType, SingleModuleClient*)
        = 0;

    virtual void FetchDescendantsForInlineScript(ModuleScript*, ResourceFetcher* fetch_client_settings_object_fetcher,
        mojom::blink::RequestContextType context_type, network::mojom::RequestDestination destination, ModuleTreeClient*)
        = 0;
    ...
    virtual KURL ResolveModuleSpecifier(const String& module_request, const KURL& base_url, String* failure_reason) = 0;

    // https://tc39.github.io/proposal-dynamic-import/#sec-hostimportmoduledynamically
    virtual void ResolveDynamically(const ModuleRequest& module_request, const ReferrerScriptInfo&, ScriptPromiseResolver<IDLAny>*) = 0;
    ...
    const ImportMap* GetImportMapForTest() const
    {
        return import_map_.Get();
    }
    ...
    virtual ModuleImportMeta HostGetImportMetaProperties(v8::Local<v8::Module>) const = 0;
    ...
    virtual ModuleType ModuleTypeFromRequest(const ModuleRequest& module_request) const = 0;

    virtual ModuleScriptFetcher* CreateModuleScriptFetcher(ModuleScriptCustomFetchType, base::PassKey<ModuleScriptLoader> pass_key) = 0;
    ...
    virtual void AddModuleToResolvedModuleSet(std::optional<AtomicString>, AtomicString)
    {
...
private:
    ...
    AcquiringImportMapsState acquiring_import_maps_ = AcquiringImportMapsState::kAcquiring;
};
