...
{
    String message = "A parser-blocking, cross site (i.e. different eTLD+1) script, " + url
        + ", is invoked via document.write. The network request for this script "
          ...
          "for more details.";
    document.AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kJavaScript, mojom::ConsoleMessageLevel::kWarning, message));
    DVLOG(1) << message.Utf8();
}

void EmitWarningNotBlocked(const String& url, Document& document)
{
    String message = "The parser-blocking, cross site (i.e. different eTLD+1) script, " + url
...
{
    String message = "Network request for the parser-blocking, cross site (i.e. different "
                     "eTLD+1) script, "
        + url
        + ", invoked via document.write was BLOCKED by the browser due to poor "
          "network connectivity. ";
    document.AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kIntervention, mojom::ConsoleMessageLevel::kError, message));
}

void AddWarningHeader(FetchParameters* params)
{
    params->MutableResourceRequest().AddHttpHeaderField(AtomicString("Intervention"),
...
{
    params->MutableResourceRequest().AddHttpHeaderField(AtomicString("Intervention"), AtomicString("<https://www.chromestatus.com/feature/5718547946799104>"));
}
...
bool ShouldDisallowFetch(Settings* settings, WebConnectionType connection_type, WebEffectiveConnectionType effective_connection)
{
    if (settings->GetDisallowFetchForDocWrittenScriptsInMainFrame())
        return true;
    if (settings->GetDisallowFetchForDocWrittenScriptsInMainFrameOnSlowConnections() && connection_type == kWebConnectionTypeCellular2G)
        return true;
    if (settings->GetDisallowFetchForDocWrittenScriptsInMainFrameIfEffectively2G() && IsConnectionEffectively2G(effective_connection))
        return true;
    return false;
}
...
bool MaybeDisallowFetchForDocWrittenScript(FetchParameters& params, Document& document)
{
    ...
    String request_domain = network_utils::GetDomainAndRegistry(request_host, network_utils::kIncludePrivateRegistries);
    ...
    if (!request_domain.empty() && !document_domain.empty() && request_domain == document_domain) {
        return false;
    }
    ...
    document.Loader()->DidObserveLoadingBehavior(LoadingBehaviorFlag::kLoadingBehaviorDocumentWriteBlock);

    if (!ShouldDisallowFetch(settings, GetNetworkStateNotifier().ConnectionType(), GetNetworkStateNotifier().EffectiveType())) {
        AddWarningHeader(&params);
        return false;
    }
    ...
    params.MutableResourceRequest().SetCacheMode(mojom::FetchCacheMode::kOnlyIfCached);

    return true;
}

void PossiblyFetchBlockedDocWriteScript(
    const Resource* resource, Document& element_document, const ScriptFetchOptions& options, CrossOriginAttributeValue cross_origin)
{
    ...
    FetchParameters params(options.CreateFetchParameters(
        resource->Url(), context->GetSecurityOrigin(), context->GetCurrentWorld(), cross_origin, resource->Encoding(), FetchParameters::kIdleLoad));
    ...
    ScriptResource::Fetch(params, element_document.Fetcher(), nullptr, context->GetIsolate(), ScriptResource::kNoStreaming, kNoCompileHintsProducer,
