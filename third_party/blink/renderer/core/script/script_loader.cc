...
ScriptLoader::ScriptLoader(ScriptElementBase* element, const CreateElementFlags flags)
    : element_(element)
{
...
{
    if (already_started_
        && GetScriptTypeAtPrepare(element_->TypeAttributeValue(), element_->LanguageAttributeValue()) == ScriptTypeAtPrepare::kSpeculationRules) {
        // See https://crbug.com/359355331, where this was requested.
        auto* message = MakeGarbageCollected<ConsoleMessage>(ConsoleMessage::Source::kJavaScript, ConsoleMessage::Level::kWarning,
    ...
    if (!parser_inserted_) {
        PendingScript* pending_script = PrepareScript(ParserBlockingInlineOption::kDeny, TextPosition::MinimumPosition());
...
void ScriptLoader::ChildrenChanged(const ContainerNode::ChildrenChange& change)
{
    if (script_type_ == ScriptTypeAtPrepare::kSpeculationRules
        && (change.type == ContainerNode::ChildrenChangeType::kTextChanged || change.type == ContainerNode::ChildrenChangeType::kNonElementInserted
    ...
    if (change.IsChildInsertion() && !parser_inserted_ && element_->IsConnected()) {
        PendingScript* pending_script = PrepareScript(ParserBlockingInlineOption::kDeny, TextPosition::MinimumPosition());
        DCHECK(!pending_script);
    }
}
...
bool IsValidClassicScriptTypeAndLanguage(const String& type, const String& language)
{
    if (type.IsNull()) {
...
bool IsSameSite(const KURL& url, const Document& element_document)
{
    ...
    return url_origin->IsSameSiteWith(element_document.GetExecutionContext()->GetSecurityOrigin());
}

bool IsDocumentReloadedOrFormSubmitted(const Document& element_document)
{
    Document& top_document = element_document.TopDocument();
    return top_document.Loader() && top_document.Loader()->IsReloadedOrFormSubmitted();
}
...
{
    return base::FeatureList::IsEnabled(features::kForceInOrderScript) && IsEligibleCommon(element_document);
}

// [Intervention, DelayAsyncScriptExecution, crbug.com/1340837]
bool IsEligibleForDelay(const Resource& resource, const Document& element_document, const ScriptElementBase& element)
{
    ...
    const bool main_frame_only = features::kDelayAsyncScriptExecutionMainFrameOnlyParam.Get();
    if (main_frame_only && !element_document.IsInOutermostMainFrame()) {
        return false;
    }

    const base::TimeDelta feature_limit = features::kDelayAsyncScriptExecutionFeatureLimitParam.Get();
    if (!feature_limit.is_zero() && element_document.GetStartTime().Elapsed() > feature_limit) {
        return false;
    }

    bool is_ad_resource = resource.GetResourceRequest().IsAdResource();
    const features::AsyncScriptExperimentalSchedulingTarget target = features::kDelayAsyncScriptExecutionTargetParam.Get();
    switch (target) {
    ...
    const bool opt_out_low = features::kDelayAsyncScriptExecutionOptOutLowFetchPriorityHintParam.Get();
    const bool opt_out_auto = features::kDelayAsyncScriptExecutionOptOutAutoFetchPriorityHintParam.Get();
    const bool opt_out_high = features::kDelayAsyncScriptExecutionOptOutHighFetchPriorityHintParam.Get();
    ...
    const features::DelayAsyncScriptTarget delay_async_script_target = features::kDelayAsyncScriptTargetParam.Get();
    ...
    case features::DelayAsyncScriptTarget::kCrossSiteWithAllowListReportOnly:
        ...
        DEFINE_STATIC_LOCAL(UrlMatcher, url_matcher,
            (UrlMatcher(GetFieldTrialParamByFeatureAsString(features::kDelayAsyncScriptExecution, "delay_async_exec_allow_list", ""))));
        return url_matcher.Match(resource.Url());
    }
}

// [Intervention, LowPriorityScriptLoading, crbug.com/1365763]
bool IsEligibleForLowPriorityScriptLoading(const Document& element_document, const ScriptElementBase& element, const KURL& url)
{
    static const bool enabled = base::FeatureList::IsEnabled(features::kLowPriorityScriptLoading);
    if (!enabled) {
        return false;
    }
    ...
    // Most LCP elements are provided by the main frame, and delaying subframe's
    // resources seems not to improve LCP.
    const bool main_frame_only = features::kLowPriorityScriptLoadingMainFrameOnlyParam.Get();
    if (main_frame_only && !element_document.IsInOutermostMainFrame()) {
        return false;
    }

    const base::TimeDelta feature_limit = features::kLowPriorityScriptLoadingFeatureLimitParam.Get();
    if (!feature_limit.is_zero() && element_document.GetStartTime().Elapsed() > feature_limit) {
        return false;
    }

    const bool cross_site_only = features::kLowPriorityScriptLoadingCrossSiteOnlyParam.Get();
    if (cross_site_only && IsSameSite(url, element_document)) {
        return false;
    }

    DEFINE_STATIC_LOCAL(UrlMatcher, deny_list, (UrlMatcher(features::kLowPriorityScriptLoadingDenyListParam.Get())));
...
bool IsEligibleForSelectiveInOrder(const Resource& resource, const Document& element_document)
{
    ...
    DEFINE_STATIC_LOCAL(UrlMatcher, url_matcher, (UrlMatcher(features::kSelectiveInOrderScriptAllowList.Get())));
    return url_matcher.Match(resource.Url());
}

ScriptRunner::DelayReasons DetermineDelayReasonsToWait(ScriptRunner* script_runner, bool is_eligible_for_delay)
{
    ...
    if (is_eligible_for_delay && script_runner->IsActive(DelayReason::kMilestone)) {
...
ScriptLoader::ScriptTypeAtPrepare ScriptLoader::GetScriptTypeAtPrepare(const String& type, const String& language)
{
...
bool ScriptLoader::BlockForNoModule(ScriptTypeAtPrepare script_type, bool nomodule)
{
...
network::mojom::CredentialsMode ScriptLoader::ModuleScriptCredentialsMode(CrossOriginAttributeValue cross_origin)
{
    switch (cross_origin) {
...
// <specdef href="https://html.spec.whatwg.org/C/#prepare-the-script-element">
PendingScript* ScriptLoader::PrepareScript(ParserBlockingInlineOption parser_blocking_inline_option, const TextPosition& script_start_position)
{
    ...
    // Steps 8-11.
    script_type_ = GetScriptTypeAtPrepare(element_->TypeAttributeValue(), element_->LanguageAttributeValue());
    ...
    TextPosition position = is_in_document_write ? TextPosition::MinimumPosition() : script_start_position;
    ...
    // [CSP]</spec>
    if (!element_->HasSourceAttribute() && !element_->AllowInlineScriptForCSP(element_->GetNonceForElement(), position.line_, source_text)) {
        return nullptr;
    }
    ...
    CrossOriginAttributeValue cross_origin = GetCrossOriginAttributeValue(element_->CrossOriginAttributeValue());
    ...
    network::mojom::CredentialsMode credentials_mode = ModuleScriptCredentialsMode(cross_origin);
    ...
    if (!integrity_attr.empty()) {
        SubresourceIntegrity::IntegrityFeatures integrity_features = SubresourceIntegrityHelper::GetFeatures(element_->GetExecutionContext());
        SubresourceIntegrity::ReportInfo report_info;
        SubresourceIntegrity::ParseIntegrityAttribute(integrity_attr, integrity_features, integrity_metadata, &report_info);
        SubresourceIntegrityHelper::DoReport(*element_->GetExecutionContext(), report_info);
    }
    ...
    if (!referrerpolicy_attr.empty()) {
        SecurityPolicy::ReferrerPolicyFromString(referrerpolicy_attr, kDoNotSupportReferrerPolicyLegacyKeywords, &referrer_policy);
    }
    ...
    mojom::blink::FetchPriorityHint fetch_priority_hint = GetFetchPriorityAttributeValue(fetch_priority_attr);

    // <spec step="28">Let parser metadata be "parser-inserted" if el is
    // ...</spec>
    ParserDisposition parser_state = IsParserInserted() ? kParserInserted : kNotParserInserted;
    ...
    RenderBlockingBehavior render_blocking_behavior = potentially_render_blocking ? RenderBlockingBehavior::kBlocking : RenderBlockingBehavior::kNonBlocking;
    ...
    ScriptFetchOptions options(nonce, integrity_metadata, integrity_attr, parser_state, credentials_mode, referrer_policy, fetch_priority_hint,
        render_blocking_behavior, RejectCoepUnsafeNone(false));
    ...
    ScriptState* script_state = ToScriptStateForMainWorld(context_window->GetFrame());
    ...
    if (element_->HasSourceAttribute()) {
        ...
        if (GetScriptType() == ScriptTypeAtPrepare::kImportMap) {
            element_document.GetTaskRunner(TaskType::kDOMManipulation)
                ->PostTask(FROM_HERE, WTF::BindOnce(&ScriptElementBase::DispatchErrorEvent, WrapPersistent(element_.Get())));
            return nullptr;
        }
        // <spec step="31.2">Let src be the value of el's src attribute.</spec>
        String src = StripLeadingAndTrailingHTMLSpaces(element_->SourceAttributeValue());
        ...
        if (element_->HasAttributionsrcAttribute()
            && context_window->GetFrame()->GetAttributionSrcLoader()->CanRegister(url,
                /*element=*/nullptr,
                /*request_id=*/std::nullopt)) {
            options.SetAttributionReportingEligibility(ScriptFetchOptions::AttributionReportingEligibility::kEligible);
        }

        // <spec step="31.7">If el is potentially render-blocking, then block
        // rendering on el.</spec>
        if (potentially_render_blocking && element_document.GetRenderBlockingResourceManager()) {
            element_document.GetRenderBlockingResourceManager()->AddPendingScript(*element_);
        }
        ...
        case ScriptTypeAtPrepare::kSpeculationRules:
            ...
            element_document.AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kJavaScript,
                mojom::blink::ConsoleMessageLevel::kError, "External speculation rules are not yet supported."));
            return nullptr;

        case ScriptTypeAtPrepare::kWebBundle:
            element_document.AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(
                mojom::blink::ConsoleMessageSource::kJavaScript, mojom::blink::ConsoleMessageLevel::kError, "External webbundle is not yet supported."));
            element_document.GetTaskRunner(TaskType::kDOMManipulation)
                ->PostTask(FROM_HERE, WTF::BindOnce(&ScriptElementBase::DispatchErrorEvent, WrapPersistent(element_.Get())));
            return nullptr;
        ...
        case ScriptTypeAtPrepare::kClassic: {
            ...
            if (!parser_inserted_ || element_->AsyncAttributeValue() || element_->DeferAttributeValue()) {
                if (!IsEligibleForLowPriorityScriptLoading(element_document, *element_, url)) {
            ...
            ClassicPendingScript* pending_script
                = ClassicPendingScript::Fetch(url, element_document, options, cross_origin, encoding, element_, defer, GetRunningTask(script_state));
            prepared_pending_script_ = pending_script;
            Resource* resource = pending_script->GetResource();
            resource_keep_alive_ = resource;
            is_eligible_for_delay = IsEligibleForDelay(*resource, element_document, *element_);
            is_eligible_for_selective_in_order = IsEligibleForSelectiveInOrder(*resource, element_document);
            break;
        }
        case ScriptTypeAtPrepare::kModule: {
            // - "module":
            ...
            if (integrity_attr.IsNull()) {
                ...
                options.SetIntegrityAttributeValue(modulator->GetIntegrityMetadataString(url));
            }
            FetchModuleScriptTree(url, fetch_client_settings_object_fetcher, modulator, options);
        } break;
        }
    }
    ...
    if (!element_->HasSourceAttribute()) {
        ...
        KURL source_url = (!is_in_document_write && parser_inserted_) ? element_document.Url() : KURL();
        ...
        case ScriptTypeAtPrepare::kImportMap: {
            if (!RuntimeEnabledFeatures::MultipleImportMapsEnabled()) {
                ...
                case Modulator::AcquiringImportMapsState::kMultipleImportMaps:
                    element_document.AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kJavaScript,
                        ...
                        acquiring_state == Modulator::AcquiringImportMapsState::kAfterModuleScriptLoad ? "An import map is added after module script load was "
                                                                                                         "triggered."
                ...
                case Modulator::AcquiringImportMapsState::kAcquiring:
                    modulator->SetAcquiringImportMapsState(Modulator::AcquiringImportMapsState::kMultipleImportMaps);
            ...
            PendingImportMap* pending_import_map = PendingImportMap::CreateInline(*element_, source_text, base_url);
        ...
        case ScriptTypeAtPrepare::kWebBundle: {
            ...
            absl::variant<ScriptWebBundle*, ScriptWebBundleError> script_web_bundle_or_error = ScriptWebBundle::CreateOrReuseInline(*element_, source_text);
            if (absl::holds_alternative<ScriptWebBundle*>(script_web_bundle_or_error)) {
                script_web_bundle_ = absl::get<ScriptWebBundle*>(script_web_bundle_or_error);
                DCHECK(script_web_bundle_);
            }
            if (absl::holds_alternative<ScriptWebBundleError>(script_web_bundle_or_error)) {
                ScriptWebBundleError error = absl::get<ScriptWebBundleError>(script_web_bundle_or_error);
                ...
                if (error.GetType() == ScriptWebBundleError::Type::kSystemError) {
                    ...
                    if (script_state->ContextIsValid()) {
                        ...
                        V8ScriptRunner::ReportException(script_state->GetIsolate(), error.ToV8(script_state));
                    }
                }
            }
            return nullptr;
        ...
        case ScriptTypeAtPrepare::kSpeculationRules: {
            auto* source = SpeculationRuleSet::Source::FromInlineScript(source_text, element_document, element_->GetDOMNodeId());
        ...
        case ScriptTypeAtPrepare::kClassic: {
            ...
            ScriptSourceLocationType script_location_type = ScriptSourceLocationType::kInline;
            if (!parser_inserted_) {
                script_location_type = ScriptSourceLocationType::kInlineInsideGeneratedElement;
            } else if (is_in_document_write) {
                script_location_type = ScriptSourceLocationType::kInlineInsideDocumentWrite;
            }

            prepared_pending_script_ = ClassicPendingScript::CreateInline(
                element_, position, source_url, base_url, source_text, script_location_type, options, GetRunningTask(script_state));
        ...
        case ScriptTypeAtPrepare::kModule: {
            ...
            ModuleScriptCreationParams params(source_url, base_url, ScriptSourceLocationType::kInline, ModuleType::kJavaScript,
                ParkableString(source_text.Impl()), nullptr, network::mojom::ReferrerPolicy::kDefault);
            ModuleScript* module_script = JSModuleScript::Create(params, modulator, options, position);
            ...
            if (RuntimeEnabledFeatures::RenderBlockingInlineModuleScriptEnabled() && potentially_render_blocking
            ...
            auto* module_tree_client = MakeGarbageCollected<ModulePendingScriptTreeClient>();
            modulator->FetchDescendantsForInlineScript(module_script, fetch_client_settings_object_fetcher, mojom::blink::RequestContextType::SCRIPT,
                network::mojom::RequestDestination::kScript, module_tree_client);
            prepared_pending_script_
                = MakeGarbageCollected<ModulePendingScript>(element_, module_tree_client, is_external_script_, GetRunningTask(script_state));
            break;
        }
        }
    }
    ...
    ScriptSchedulingType script_scheduling_type = GetScriptSchedulingTypePerSpec(element_document, parser_blocking_inline_option);
    ...
    if (is_eligible_for_selective_in_order) {
        ...
        case ScriptSchedulingType::kParserBlocking:
            UseCounter::Count(context_window->document()->TopDocument(), WebFeature::kSelectiveInOrderScript);
    ...
    if (script_scheduling_type == ScriptSchedulingType::kImmediate && parser_inserted_ && parser_blocking_inline_option == ParserBlockingInlineOption::kAllow
    ...
    case ScriptSchedulingType::kForceInOrder:
...

        {
            ...
            if (is_eligible_for_delay && script_scheduling_type == ScriptSchedulingType::kAsync) {
                const features::DelayAsyncScriptTarget delay_async_script_target = features::kDelayAsyncScriptTargetParam.Get();
                if (delay_async_script_target == features::DelayAsyncScriptTarget::kCrossSiteWithAllowListReportOnly) {
                    is_eligible_for_delay = false;
                }
            }
            // TODO(hiroshige): Here the context document is used as "node document"
            // while Step 14 uses |elementDocument| as "node document". Fix this.
            ScriptRunner* script_runner = context_window->document()->GetScriptRunner();
            script_runner->QueueScriptForExecution(
                TakePendingScript(script_scheduling_type), DetermineDelayReasonsToWait(script_runner, is_eligible_for_delay));
        ...
        if (resource_keep_alive_) {
            resource_keep_alive_->AddFinishObserver(this, element_document.GetTaskRunner(TaskType::kNetworking).get());
        }
    ...
    case ScriptSchedulingType::kParserBlockingInline:
        ...
        if (script_scheduling_type == ScriptSchedulingType::kParserBlockingInline) {
            DCHECK_EQ(parser_blocking_inline_option, ParserBlockingInlineOption::kAllow);
        }
...
ScriptSchedulingType ScriptLoader::GetScriptSchedulingTypePerSpec(Document& element_document, ParserBlockingInlineOption parser_blocking_inline_option) const
{
    ...
    // <spec step="31">If el's type is "classic" and el has a src attribute, or
    // el's type is "module":</spec>
    if ((GetScriptType() == ScriptTypeAtPrepare::kClassic && element_->HasSourceAttribute()) || GetScriptType() == ScriptTypeAtPrepare::kModule) {
        ...
        // "module":</spec>
        if (element_->DeferAttributeValue() || GetScriptType() == ScriptTypeAtPrepare::kModule) {
        ...
        if (parser_inserted_ && parser_blocking_inline_option == ParserBlockingInlineOption::kAllow && !element_document.IsScriptExecutionReady()) {
...
void ScriptLoader::FetchModuleScriptTree(
    const KURL& url, ResourceFetcher* fetch_client_settings_object_fetcher, Modulator* modulator, const ScriptFetchOptions& options)
{
    auto* module_tree_client = MakeGarbageCollected<ModulePendingScriptTreeClient>();
    modulator->FetchTree(url, ModuleType::kJavaScript, fetch_client_settings_object_fetcher, mojom::blink::RequestContextType::SCRIPT,
        network::mojom::RequestDestination::kScript, options, ModuleScriptCustomFetchType::kNone, module_tree_client);
    prepared_pending_script_
        = MakeGarbageCollected<ModulePendingScript>(element_, module_tree_client, is_external_script_, GetRunningTask(modulator->GetScriptState()));
}

PendingScript* ScriptLoader::TakePendingScript(ScriptSchedulingType scheduling_type)
{
    CHECK(prepared_pending_script_);

    // Record usage histograms per script tag.
    if (element_->GetDocument().Url().ProtocolIsInHTTPFamily()) {
        base::UmaHistogramEnumeration("Blink.Script.SchedulingType", scheduling_type);
    }

    // Record usage histograms per page.
    switch (scheduling_type) {
    case ScriptSchedulingType::kDefer:
        UseCounter::Count(element_->GetDocument(), WebFeature::kScriptSchedulingType_Defer);
        break;
    case ScriptSchedulingType::kParserBlocking:
        UseCounter::Count(element_->GetDocument(), WebFeature::kScriptSchedulingType_ParserBlocking);
        break;
    case ScriptSchedulingType::kParserBlockingInline:
        UseCounter::Count(element_->GetDocument(), WebFeature::kScriptSchedulingType_ParserBlockingInline);
        break;
    case ScriptSchedulingType::kInOrder:
        UseCounter::Count(element_->GetDocument(), WebFeature::kScriptSchedulingType_InOrder);
        break;
    case ScriptSchedulingType::kAsync:
        UseCounter::Count(element_->GetDocument(), WebFeature::kScriptSchedulingType_Async);
        break;
    default:
        break;
    }
...
void ScriptLoader::NotifyFinished()
{
    ...
    // memory cache.
    if (resource_keep_alive_ && !resource_keep_alive_->GetResponse().IsSignedExchangeInnerResponse()) {
...
bool ScriptLoader::IsScriptForEventSupported() const
{
    ...
    if (GetScriptType() != ScriptTypeAtPrepare::kClassic || event_attribute.IsNull() || for_attribute.IsNull()) {
        return true;
    }
    ...
    return EqualIgnoringASCIICase(event_attribute, "onload") || EqualIgnoringASCIICase(event_attribute, "onload()");
}
...
String ScriptLoader::GetScriptText() const
{
    ...
    return GetStringForScriptExecution(child_text_content, element_->GetScriptElementType(), element_->GetExecutionContext());
}
...
{
    if (SpeculationRuleSet* rule_set = std::exchange(speculation_rule_set_, nullptr)) {
        ...
        DocumentSpeculationRules::From(element_->GetDocument()).RemoveRuleSet(rule_set);
        return rule_set;
    }
    return nullptr;
}
