...
public:
    ...
    v8::Local<v8::Module> Resolve(const ModuleRequest& module_request, v8::Local<v8::Module> referrer, ExceptionState&) override
    {
        NOTREACHED();
    }
...
private:
    ...
    class TestModuleScriptFetcher final : public GarbageCollected<TestModuleScriptFetcher>, public ModuleScriptFetcher {
    public:
        TestModuleScriptFetcher(ModuleMapTestModulator* modulator, base::PassKey<ModuleScriptLoader> pass_key)
            : ModuleScriptFetcher(pass_key)
            , modulator_(modulator)
        {
        }
        void Fetch(FetchParameters& request, ModuleType module_type, ResourceFetcher*, ModuleGraphLevel, ModuleScriptFetcher::Client* client) override
        {
            CHECK_EQ(request.GetScriptType(), mojom::blink::ScriptType::kModule);
            TestRequest* test_request = MakeGarbageCollected<TestRequest>(request.Url(), client);
            modulator_->test_requests_.push_back(test_request);
        }
        String DebugName() const override
        {
            return "TestModuleScriptFetcher";
        }
        void Trace(Visitor* visitor) const override
        {
    ...
    ModuleScriptFetcher* CreateModuleScriptFetcher(ModuleScriptCustomFetchType, base::PassKey<ModuleScriptLoader> pass_key) override
    {
    ...
    struct TestRequest final : public GarbageCollected<TestRequest> {
        ...
        {
            client_->NotifyFetchFinishedSuccess(ModuleScriptCreationParams(url_, url_, ScriptSourceLocationType::kExternalFile, ModuleType::kJavaScript,
                ParkableString(String("").ReleaseImpl()), nullptr, network::mojom::ReferrerPolicy::kDefault));
        }
...
void ModuleMapTestModulator::ResolveFetches()
{
    for (const auto& test_request : test_requests_) {
        TaskRunner()->PostTask(FROM_HERE, WTF::BindOnce(&TestRequest::NotifyFetchFinished, WrapPersistent(test_request.Get())));
    }
...
public:
    ...
    ModuleMapTestModulator* Modulator()
    {
...
void ModuleMapTest::SetUp()
{
    ...
    modulator_ = MakeGarbageCollected<ModuleMapTestModulator>(ToScriptStateForMainWorld(&GetFrame()));
...
TEST_F(ModuleMapTest, sequentialRequests)
{
    ...
    // First request
    TestSingleModuleClient* client = MakeGarbageCollected<TestSingleModuleClient>();
    Map()->FetchSingleModuleScript(ModuleScriptFetchRequest::CreateForTest(url, ModuleType::kJavaScript), GetDocument().Fetcher(),
        ModuleGraphLevel::kTopLevelModuleFetch, ModuleScriptCustomFetchType::kNone, client);
    Modulator()->ResolveFetches();
    EXPECT_FALSE(client->WasNotifyFinished()) << "fetchSingleModuleScript shouldn't complete synchronously";
    test::RunPendingTasks();

    EXPECT_EQ(Modulator()->GetTestModuleRecordResolver()->RegisterModuleScriptCallCount(), 1);
    EXPECT_TRUE(client->WasNotifyFinished());
    EXPECT_TRUE(client->GetModuleScript());

    // Secondary request
    TestSingleModuleClient* client2 = MakeGarbageCollected<TestSingleModuleClient>();
    Map()->FetchSingleModuleScript(ModuleScriptFetchRequest::CreateForTest(url, ModuleType::kJavaScript), GetDocument().Fetcher(),
        ModuleGraphLevel::kTopLevelModuleFetch, ModuleScriptCustomFetchType::kNone, client2);
    Modulator()->ResolveFetches();
    EXPECT_FALSE(client2->WasNotifyFinished()) << "fetchSingleModuleScript shouldn't complete synchronously";
    test::RunPendingTasks();
