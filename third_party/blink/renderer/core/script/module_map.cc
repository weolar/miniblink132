...
class ModuleMap::Entry final : public GarbageCollected<Entry>, public NameClient, public ModuleScriptLoaderClient {
...
void ModuleMap::Entry::DispatchFinishedNotificationAsync(SingleModuleClient* client)
{
    map_->GetModulator()->TaskRunner()->PostTask(
        FROM_HERE, WTF::BindOnce(&SingleModuleClient::NotifyModuleLoadFinished, WrapPersistent(client), WrapPersistent(module_script_.Get())));
}
...
void ModuleMap::Entry::NotifyNewSingleModuleFinished(ModuleScript* module_script)
{
    CHECK(is_fetching_);
...
void ModuleMap::FetchSingleModuleScript(const ModuleScriptFetchRequest& request, ResourceFetcher* fetch_client_settings_object_fetcher, ModuleGraphLevel level,
    ModuleScriptCustomFetchType custom_fetch_type, SingleModuleClient* client)
{
    ...
    MapImpl::AddResult result = map_.insert(std::make_pair(request.Url(), request.GetExpectedModuleType()), nullptr);
    ...
    if (result.is_new_entry) {
        ...
        ModuleScriptLoader::Fetch(request, fetch_client_settings_object_fetcher, level, modulator_, custom_fetch_type, loader_registry_, entry);
    }
    DCHECK(entry);
...
ModuleScript* ModuleMap::GetFetchedModuleScript(const KURL& url, ModuleType module_type) const
{
