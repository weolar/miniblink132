...
public:
    static ImportMap* Parse(const String& text, const KURL& base_url, ExecutionContext& context, std::optional<ImportMapError>* error_to_rethrow);
    ...
    ImportMap(SpecifierMap&& imports, ScopesMap&& scopes_map, IntegrityMap&& integrity);
    ...
    std::optional<KURL> Resolve(const ParsedSpecifier&, const KURL& base_url, String* debug_message) const;
    ...
    // this call.
    void MergeExistingAndNewImportMaps(ImportMap* new_import_map, const HashMap<AtomicString, HashSet<AtomicString>>& scoped_resolved_module_map,
        const HashSet<AtomicString>& toplevel_resolved_module_set, ConsoleLogger&);
...
private:
    ...
    std::optional<KURL> ResolveImportsMatch(const ParsedSpecifier&, const SpecifierMap&, String* debug_message) const;
    std::optional<MatchResult> MatchPrefix(const ParsedSpecifier&, const SpecifierMap&) const;
    static SpecifierMap SortAndNormalizeSpecifierMap(const JSONObject* imports, const KURL& base_url, ConsoleLogger&);

    KURL ResolveImportsMatchInternal(const String& normalizedSpecifier, const MatchResult&, String* debug_message) const;
