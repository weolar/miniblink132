...
ModuleScriptFetcher* WorkerModulatorImpl::CreateModuleScriptFetcher(ModuleScriptCustomFetchType custom_fetch_type, base::PassKey<ModuleScriptLoader> pass_key)
{
    auto* global_scope = To<WorkerGlobalScope>(GetExecutionContext());
    switch (custom_fetch_type) {
    case ModuleScriptCustomFetchType::kNone:
        return MakeGarbageCollected<DocumentModuleScriptFetcher>(global_scope, pass_key);
    case ModuleScriptCustomFetchType::kWorkerConstructor:
        return MakeGarbageCollected<WorkerModuleScriptFetcher>(global_scope, pass_key);
    case ModuleScriptCustomFetchType::kWorkletAddModule:
        break;
    case ModuleScriptCustomFetchType::kInstalledServiceWorker:
        return MakeGarbageCollected<InstalledServiceWorkerModuleScriptFetcher>(global_scope, pass_key);
    }
    NOTREACHED();
}
...
{
    if (GetExecutionContext()->IsDedicatedWorkerGlobalScope() || GetExecutionContext()->IsSharedWorkerGlobalScope()) {
        return false;
    }
    ...
    *reason = "import() is disallowed on ServiceWorkerGlobalScope by the HTML "
