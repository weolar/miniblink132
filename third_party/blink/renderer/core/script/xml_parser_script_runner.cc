...
void XMLParserScriptRunner::PendingScriptFinished(PendingScript* unused_pending_script)
{
...
void XMLParserScriptRunner::ProcessScriptElement(Document& document, Element* element, TextPosition script_start_position)
{
    DCHECK(element);
    DCHECK(!parser_blocking_script_);
    ...
    // [spec text]
    PendingScript* pending_script = ScriptLoaderFromElement(element)->PrepareScript(ScriptLoader::ParserBlockingInlineOption::kAllow, script_start_position);
    ...
    if (pending_script->GetScriptType() == mojom::blink::ScriptType::kModule) {
        ...
        // module scripts.
        document.AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kJavaScript, mojom::ConsoleMessageLevel::kError,
