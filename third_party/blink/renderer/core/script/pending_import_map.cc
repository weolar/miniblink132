...
PendingImportMap* PendingImportMap::CreateInline(ScriptElementBase& element, const String& import_map_text, const KURL& base_url)
{
    ExecutionContext* context = element.GetExecutionContext();

    std::optional<ImportMapError> error_to_rethrow;
    ImportMap* import_map = ImportMap::Parse(import_map_text, base_url, *context, &error_to_rethrow);
    return MakeGarbageCollected<PendingImportMap>(element, import_map, std::move(error_to_rethrow), *context);
}

PendingImportMap::PendingImportMap(
    ScriptElementBase& element, ImportMap* import_map, std::optional<ImportMapError> error_to_rethrow, const ExecutionContext& original_context)
    : element_(&element)
...
void PendingImportMap::RegisterImportMap()
{
    ...
    Modulator* modulator = Modulator::From(ToScriptStateForMainWorld(To<LocalDOMWindow>(context)->GetFrame()));
    if (!modulator)
        return;
    ...
    if (error_to_rethrow_.has_value()) {
        if (ExecutionContext::From(script_state)->CanExecuteScripts(kAboutToExecuteScript)) {
            ModuleRecord::ReportException(script_state, error_to_rethrow_->ToV8(script_state));
        }
        return;
    }
