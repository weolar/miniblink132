...
void AddIgnoredKeyMessage(ConsoleLogger& logger, const String& key, const String& reason)
{
    logger.AddConsoleMessage(mojom::ConsoleMessageSource::kOther, mojom::ConsoleMessageLevel::kWarning, "Ignored an import map key \"" + key + "\": " + reason);
}

void AddIgnoredValueMessage(ConsoleLogger& logger, const String& key, const String& reason)
{
    logger.AddConsoleMessage(
        mojom::ConsoleMessageSource::kOther, mojom::ConsoleMessageLevel::kWarning, "Ignored an import map value of \"" + key + "\": " + reason);
}

// <specdef
// href="https://html.spec.whatwg.org/C#normalizing-a-specifier-key">
AtomicString NormalizeSpecifierKey(const String& key_string, const KURL& base_url, ConsoleLogger& logger)
{
    ...
    if (key_string.empty()) {
        ...
        AddIgnoredKeyMessage(logger, key_string, "specifier keys cannot be the empty string.");
...
KURL NormalizeValue(const String& key, const String& value_string, const KURL& base_url, ConsoleLogger& logger)
{
    ...
    case ParsedSpecifier::Type::kURL:
        ...
        if (key.EndsWith("/") && !value.GetUrl().GetString().EndsWith("/")) {
            ...
            AddIgnoredValueMessage(logger, key, "Since specifierKey ended in a slash, so must the address: " + value_string);
...
void MergeModuleSpecifierMaps(ImportMap::SpecifierMap& old_map, const ImportMap::SpecifierMap& new_map, ConsoleLogger& logger)
{
    ...
    for (auto specifier : new_map.Keys()) {
        ...
        if (old_map.Contains(specifier)) {
            ...
            auto* message = MakeGarbageCollected<ConsoleMessage>(ConsoleMessage::Source::kJavaScript, ConsoleMessage::Level::kWarning,
                "An import map rule for specifier '" + specifier + "' was removed, as it conflicted with an existing rule.");
...
void SpecifierMapToStringForTesting(StringBuilder& builder, const ImportMap::SpecifierMap& specifier_map)
{
    builder.Append("{");
...
// the console |logger|.
ImportMap* ImportMap::Parse(const String& input, const KURL& base_url, ExecutionContext& context, std::optional<ImportMapError>* error_to_rethrow)
{
    ...
    if (!parsed) {
        *error_to_rethrow = ImportMapError(ImportMapError::Type::kSyntaxError, "Failed to parse import map: invalid JSON");
    ...
    if (!parsed_map) {
        *error_to_rethrow = ImportMapError(ImportMapError::Type::kTypeError, "Failed to parse import map: not an object");
        return MakeGarbageCollected<ImportMap>();
    }
    ...
    if (parsed_map->Get("imports")) {
        ...
        if (!imports) {
            *error_to_rethrow = ImportMapError(ImportMapError::Type::kTypeError,
        ...
        // baseURL.</spec>
        sorted_and_normalized_imports = SortAndNormalizeSpecifierMap(imports, base_url, context);
    }
        ...
        for (wtf_size_t i = 0; i < scopes->size(); ++i) {
            ...
            if (!prefix_url.IsValid()) {
                ...
                context.AddConsoleMessage(
                    mojom::ConsoleMessageSource::kOther, mojom::ConsoleMessageLevel::kWarning, "Ignored scope \"" + entry.first + "\": not parsable as a URL.");
            ...
            if (normalized_scopes_map.find(prefix_url_string) != normalized_scopes_map.end()) {
            ...
            normalized_scopes_map.insert(prefix_url_string, SortAndNormalizeSpecifierMap(specifier_map, base_url, context));
        }
    }
    ...
    if (RuntimeEnabledFeatures::ImportMapIntegrityEnabled() && parsed_map->Get("integrity")) {
        ...
        for (wtf_size_t i = 0; i < integrity->size(); ++i) {
            ...
            ParsedSpecifier parsed_specifier = ParsedSpecifier::Create(entry.first, base_url);
            ...
            if (entry.second->GetType() != JSONValue::ValueType::kTypeString) {
                AddIgnoredValueMessage(context, entry.first, "Integrity value is not a string.");
                continue;
            }
            ...
            if (integrity->GetString(entry.first, &value_string)) {
                ...
                AddIgnoredValueMessage(context, entry.first, "Internal error in GetString().");
            }
        }
    }
    ...
    return MakeGarbageCollected<ImportMap>(std::move(sorted_and_normalized_imports), std::move(normalized_scopes_map), std::move(normalized_integrity_map));
}

// <specdef
// href="https://html.spec.whatwg.org/C#sorting-and-normalizing-a-module-specifier-map">
ImportMap::SpecifierMap ImportMap::SortAndNormalizeSpecifierMap(const JSONObject* imports, const KURL& base_url, ConsoleLogger& logger)
{
    ...
    for (wtf_size_t i = 0; i < imports->size(); ++i) {
        ...
        const AtomicString normalized_specifier_key = NormalizeSpecifierKey(entry.first, base_url, logger);
        ...
        case JSONValue::ValueType::kTypeString: {
            ...
            if (!imports->GetString(entry.first, &value_string)) {
                AddIgnoredValueMessage(logger, entry.first, "Internal error in GetString().");
                normalized.Set(normalized_specifier_key, NullURL());
                break;
            }

            normalized.Set(normalized_specifier_key, NormalizeValue(entry.first, value_string, base_url, logger));
            break;
        }
...
std::optional<ImportMap::MatchResult> ImportMap::MatchPrefix(const ParsedSpecifier& parsed_specifier, const SpecifierMap& specifier_map) const
{
...
ImportMap::ImportMap(SpecifierMap&& imports, ScopesMap&& scopes_map, IntegrityMap&& integrity)
...
std::optional<KURL> ImportMap::Resolve(const ParsedSpecifier& parsed_specifier, const KURL& base_url, String* debug_message) const
{
    ...
    for (const auto& scope : scopes_vector_) {
        ...
        if (scope == base_url.GetString() || (scope.EndsWith("/") && base_url.GetString().StartsWith(scope))) {
            ...
            std::optional<KURL> scope_match = ResolveImportsMatch(parsed_specifier, specifier_map, debug_message);
...
// <specdef href="https://html.spec.whatwg.org/C#resolving-an-imports-match">
std::optional<KURL> ImportMap::ResolveImportsMatch(const ParsedSpecifier& parsed_specifier, const SpecifierMap& specifier_map, String* debug_message) const
{
    DCHECK(debug_message);
    ...
    if (parsed_specifier.GetType() == ParsedSpecifier::Type::kURL && !SchemeRegistry::IsSpecialScheme(parsed_specifier.GetUrl().Protocol())) {
        *debug_message = "Import Map: \"" + key + "\" skips prefix match because of non-special URL scheme";
    ...
    *debug_message = "Import Map: \"" + key + "\" matches with no entries and thus is not mapped.";
    return std::nullopt;
}

// <specdef href="https://html.spec.whatwg.org/C#resolving-an-imports-match">
KURL ImportMap::ResolveImportsMatchInternal(const String& key, const MatchResult& matched, String* debug_message) const
{
    ...
    if (!matched->value.IsValid()) {
        *debug_message = "Import Map: \"" + key + "\" matches with \"" + matched->key + "\" but is blocked by a null value";
        return NullURL();
    }
    ...
    const KURL url = after_prefix.empty() ? matched->value : KURL(matched->value, after_prefix);
    ...
    // failure.</spec>
    if (!url.IsValid()) {
        *debug_message = "Import Map: \"" + key + "\" matches with \"" + matched->key + "\" but is blocked due to relative URL parse failure";
        return NullURL();
    }
    ...
    if (!url.GetString().StartsWith(matched->value.GetString())) {
        *debug_message = "Import Map: \"" + key + "\" matches with \"" + matched->key + "\" but is blocked due to backtracking";
        return NullURL();
    }

    // <spec step="1.2.9">Return url.</spec>
    *debug_message = "Import Map: \"" + key + "\" matches with \"" + matched->key + "\" and is mapped to " + url.ElidedString();
    return url;
}
...
void ImportMap::MergeExistingAndNewImportMaps(ImportMap* new_import_map, const HashMap<AtomicString, HashSet<AtomicString>>& scoped_resolved_module_map,
    const HashSet<AtomicString>& toplevel_resolved_module_set, ConsoleLogger& logger)
{
    ...
    ImportMap::IntegrityMap& new_import_map_integrity = new_import_map->integrity_;
    ...
    for (auto scope : new_import_map_scopes) {
        ...
        if (current_set_it != scoped_resolved_module_map.end()) {
            ...
            for (auto specifier : scope_imports.Keys()) {
                if (current_resolved_set.find(specifier) != current_resolved_set.end()) {
        ...
        if (old_scope_specifier_map_it != scopes_map_.end()) {
            ImportMap::SpecifierMap& old_scope_specifier_map = old_scope_specifier_map_it->value;
...
void ImportMap::InitializeScopesVector()
{
    ...
    std::sort(scopes_vector_.begin(), scopes_vector_.end(), [](const String& a, const String& b) { return CodeUnitCompareLessThan(b, a); });
}
