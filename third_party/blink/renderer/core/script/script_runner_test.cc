...
public:
    ...
    MockPendingScript(ScriptElementBase* element, ScriptSchedulingType scheduling_type)
...
private:
    static MockPendingScript* Create(Document* document, ScriptSchedulingType scheduling_type)
    {
        MockScriptElementBase* element = MockScriptElementBase::Create();
        EXPECT_CALL(*element, GetDocument()).WillRepeatedly(testing::ReturnRef(*document));
        EXPECT_CALL(*element, GetExecutionContext()).WillRepeatedly(testing::Return(document->GetExecutionContext()));
        MockPendingScript* pending_script = MakeGarbageCollected<MockPendingScript>(element, scheduling_type);
...
public:
    ...
    void SetUp() override
    {
        ...
        script_runner_->SetTaskRunnerForTesting(platform_->GetMainThreadScheduler()->DefaultTaskRunner().get());
...
protected:
    ...
    {
        script_runner_->QueueScriptForExecution(pending_script, static_cast<ScriptRunner::DelayReasons>(ScriptRunner::DelayReason::kLoad));
    }
...
TEST_F(ScriptRunnerTest, QueueMultipleScripts_InOrder)
{
    ...
    for (wtf_size_t i = 0; i < pending_scripts.size(); ++i) {
        EXPECT_CALL(*pending_scripts[i], ExecuteScriptBlock()).WillOnce(InvokeWithoutArgs([this, i] { order_.push_back(i + 1); }));
    }
...
TEST_F(ScriptRunnerTest, QueueMixedScripts)
{
    ...
    EXPECT_CALL(*pending_script1, ExecuteScriptBlock()).WillOnce(InvokeWithoutArgs([this] { order_.push_back(1); }));
    EXPECT_CALL(*pending_script2, ExecuteScriptBlock()).WillOnce(InvokeWithoutArgs([this] { order_.push_back(2); }));
    EXPECT_CALL(*pending_script3, ExecuteScriptBlock()).WillOnce(InvokeWithoutArgs([this] { order_.push_back(3); }));
    EXPECT_CALL(*pending_script4, ExecuteScriptBlock()).WillOnce(InvokeWithoutArgs([this] { order_.push_back(4); }));
    EXPECT_CALL(*pending_script5, ExecuteScriptBlock()).WillOnce(InvokeWithoutArgs([this] { order_.push_back(5); }));

    platform_->RunSingleTask();
    document_->domWindow()->SetLifecycleState(mojom::FrameLifecycleState::kPaused);
    document_->domWindow()->SetLifecycleState(mojom::FrameLifecycleState::kRunning);
    platform_->RunUntilIdle();
...
TEST_F(ScriptRunnerTest, QueueReentrantScript_Async)
{
    ...
    EXPECT_CALL(*pending_script1, ExecuteScriptBlock()).WillOnce(InvokeWithoutArgs([pending_script, this] {
        order_.push_back(1);
        NotifyScriptReady(pending_script);
    }));

    pending_script = pending_script3;
    EXPECT_CALL(*pending_script2, ExecuteScriptBlock()).WillOnce(InvokeWithoutArgs([pending_script, this] {
        order_.push_back(2);
        NotifyScriptReady(pending_script);
    }));
...
TEST_F(ScriptRunnerTest, QueueReentrantScript_InOrder)
{
    ...
    EXPECT_CALL(*pending_script1, ExecuteScriptBlock()).WillOnce(InvokeWithoutArgs([pending_script, &pending_script2, this] {
        order_.push_back(1);
        QueueScriptForExecution(pending_script);
        NotifyScriptReady(pending_script2);
    }));
    ...
    pending_script = pending_script3;
    EXPECT_CALL(*pending_script2, ExecuteScriptBlock()).WillOnce(InvokeWithoutArgs([pending_script, &pending_script3, this] {
        order_.push_back(2);
        QueueScriptForExecution(pending_script);
        NotifyScriptReady(pending_script3);
    }));
...
TEST_F(ScriptRunnerTest, QueueReentrantScript_ManyAsyncScripts)
{
    ...
    for (int i = 0; i < 20; i++) {
        ...
        if (i > 0) {
            EXPECT_CALL(*pending_scripts[i], ExecuteScriptBlock()).WillOnce(InvokeWithoutArgs([this, i] { order_.push_back(i); }));
        }
    }
    ...
    EXPECT_CALL(*pending_scripts[0], ExecuteScriptBlock()).WillOnce(InvokeWithoutArgs([&pending_scripts, this] {
        for (int i = 2; i < 20; i++) {
            NotifyScriptReady(pending_scripts[i]);
        }
        order_.push_back(0);
...
{
    Persistent<MockPendingScript> pending_script1 = MockPendingScript::CreateAsync(document_);
    Persistent<MockPendingScript> pending_script2 = MockPendingScript::CreateAsync(document_);
...
TEST_F(ScriptRunnerTest, DelayReasons)
{
    ...
    MockPendingScript* pending_script1 = MockPendingScript::CreateAsync(document_);

    // Script waiting for one additional delay reason.
    MockPendingScript* pending_script2 = MockPendingScript::CreateAsync(document_);

    // Script waiting for two additional delay reason.
    MockPendingScript* pending_script3 = MockPendingScript::CreateAsync(document_);

    // Script waiting for an additional delay reason that is removed before load
    // completion.
    MockPendingScript* pending_script4 = MockPendingScript::CreateAsync(document_);
    ...
    auto* delayer1 = MakeGarbageCollected<ScriptRunnerDelayer>(script_runner_, ScriptRunner::DelayReason::kTest1);
    auto* delayer2 = MakeGarbageCollected<ScriptRunnerDelayer>(script_runner_, ScriptRunner::DelayReason::kTest2);
    delayer1->Activate();
    delayer1->Activate();
    delayer2->Activate();

    script_runner_->QueueScriptForExecution(pending_script1, static_cast<int>(ScriptRunner::DelayReason::kLoad));
    script_runner_->QueueScriptForExecution(
        pending_script2, static_cast<int>(ScriptRunner::DelayReason::kLoad) | static_cast<int>(ScriptRunner::DelayReason::kTest1));
    script_runner_->QueueScriptForExecution(pending_script3,
        static_cast<int>(ScriptRunner::DelayReason::kLoad) | static_cast<int>(ScriptRunner::DelayReason::kTest1)
            | static_cast<int>(ScriptRunner::DelayReason::kTest2));
    script_runner_->QueueScriptForExecution(
        pending_script4, static_cast<int>(ScriptRunner::DelayReason::kLoad) | static_cast<int>(ScriptRunner::DelayReason::kTest1));
...
{
    int counter = 0;
    base::OnceClosure task = WTF::BindOnce([](int* counter) { (*counter)++; }, WTF::Unretained(&counter));

    PostTaskWithLowPriorityUntilTimeoutForTesting(FROM_HERE, std::move(task), base::Seconds(1),
