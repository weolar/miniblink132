...
public:
    DynamicImportTreeClient(const KURL& url, Modulator* modulator, ScriptPromiseResolver<IDLAny>* promise_resolver)
        : url_(url)
...
class ModuleResolutionCallback : public ThenCallable<IDLAny, ModuleResolutionCallback> {
public:
    explicit ModuleResolutionCallback(ScriptPromiseResolver<IDLAny>* promise_resolver)
        : promise_resolver_(promise_resolver)
    {
    }
...
public:
    ModuleResolutionSuccessCallback(ScriptPromiseResolver<IDLAny>* promise_resolver, ModuleScript* module_script)
...
public:
    explicit ModuleResolutionFailureCallback(ScriptPromiseResolver<IDLAny>* promise_resolver)
        : ModuleResolutionCallback(promise_resolver)
    {
    }
...
void DynamicImportTreeClient::NotifyModuleTreeLoadFinished(ModuleScript* module_script)
{
    ...
    if (!module_script) {
        ...
        v8::Local<v8::Value> error = V8ThrowException::CreateTypeError(isolate, "Failed to fetch dynamically imported module: " + url_.GetString());
    ...
    ScriptEvaluationResult result = module_script->RunScriptOnScriptStateAndReturnValue(
    ...
    case ScriptEvaluationResult::ResultType::kSuccess: {
        ...
        result.GetPromise(script_state)
            .React(script_state, MakeGarbageCollected<ModuleResolutionSuccessCallback>(promise_resolver_, module_script),
                MakeGarbageCollected<ModuleResolutionFailureCallback>(promise_resolver_));
        break;
    }
    }
}
...
void DynamicModuleResolver::ResolveDynamically(
    const ModuleRequest& module_request, const ReferrerScriptInfo& referrer_info, ScriptPromiseResolver<IDLAny>* promise_resolver)
{
    DCHECK(modulator_->GetScriptState()->GetIsolate()->InContext()) << "ResolveDynamically should be called from V8 callback, within a valid "
                                                                       "context.";
    ...
    KURL url = modulator_->ResolveModuleSpecifier(module_request.specifier, base_url, /*failure_reason=*/nullptr);
    ...
    if (!url.IsValid() || module_type == ModuleType::kInvalid) {
        ...
        if (!url.IsValid()) {
            error_message = "Failed to resolve module specifier '" + module_request.specifier + "'";
            if (referrer_info.BaseURL().IsAboutBlankURL() && base_url.IsAboutBlankURL()) {
            ...
            error_message = "\"" + module_request.GetModuleTypeString() + "\" is not a valid module type.";
        }

        // <spec step="6.1">Let completion be Completion { [[Type]]: throw,
        // [[Value]]: a new TypeError, [[Target]]: empty }.</spec>
        v8::Isolate* isolate = modulator_->GetScriptState()->GetIsolate();
        v8::Local<v8::Value> error = V8ThrowException::CreateTypeError(isolate, error_message);
    ...
    // Priority Hints.
    //
    ScriptFetchOptions options(referrer_info.Nonce(), modulator_->GetIntegrityMetadata(url), modulator_->GetIntegrityMetadataString(url),
        referrer_info.ParserState(), referrer_info.CredentialsMode(), referrer_info.GetReferrerPolicy(), mojom::blink::FetchPriorityHint::kAuto,
    ...
    // result.</spec>
    auto* tree_client = MakeGarbageCollected<DynamicImportTreeClient>(url, modulator_.Get(), promise_resolver);
    // TODO(kouhei): ExecutionContext::From(modulator_->GetScriptState()) is
    // highly discouraged since it breaks layering. Rewrite this.
    auto* execution_context = ExecutionContext::From(modulator_->GetScriptState());
    modulator_->FetchTree(url, module_type, execution_context->Fetcher(), mojom::blink::RequestContextType::SCRIPT, network::mojom::RequestDestination::kScript,
        options, ModuleScriptCustomFetchType::kNone, tree_client, referrer_info.BaseURL().GetString());
