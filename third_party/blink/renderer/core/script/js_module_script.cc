...
JSModuleScript* JSModuleScript::Create(
    const ModuleScriptCreationParams& original_params, Modulator* modulator, const ScriptFetchOptions& options, const TextPosition& start_position)
{
    ...
    const ModuleScriptCreationParams& params = modulator->IsScriptingDisabled() ? original_params.CopyWithClearedSourceText() : original_params;
    ...
    v8::Local<v8::Module> result = ModuleRecord::Compile(script_state, params, options, start_position, modulator->GetV8CacheOptions(), &produce_cache_data);
    ...
    JSModuleScript* script
        = CreateInternal(params.GetSourceText().length(), modulator, result, params.SourceURL(), params.BaseURL(), options, start_position, produce_cache_data);
    ...
    for (const auto& requested : ModuleRecord::ModuleRequests(script_state, result)) {
        ...
        if (requested.HasInvalidImportAttributeKey(&failure_reason)) {
            ...
            error = V8ThrowException::CreateSyntaxError(isolate, "Invalid attribute key \"" + failure_reason + "\".");

            // <spec step="9.2">Resolve a module specifier given script and
            // requested.[[Specifier]], catching any exceptions.</spec>
        } else if (!script->ResolveModuleSpecifier(requested.specifier, &failure_reason).IsValid()) {
            error = V8ThrowException::CreateTypeError(isolate, "Failed to resolve module specifier \"" + requested.specifier + "\". " + failure_reason);
        ...
        } else if (modulator->ModuleTypeFromRequest(requested) == ModuleType::kInvalid) {
            // <spec step="9.5.1">Let error be a new TypeError exception.</spec>
            error = V8ThrowException::CreateTypeError(isolate, "\"" + requested.GetModuleTypeString() + "\" is not a valid module type.");
        }

        if (!error.IsEmpty()) {
            // <spec step="9.1.2">Set script's parse error to error.</spec>
            script->SetParseErrorAndClearRecord(ScriptValue(isolate, error.ToLocalChecked()));
...
JSModuleScript* JSModuleScript::CreateForTest(Modulator* modulator, v8::Local<v8::Module> record, const KURL& base_url, const ScriptFetchOptions& options)
{
    KURL dummy_source_url;
    return CreateInternal(0, modulator, record, dummy_source_url, base_url, options, TextPosition::MinimumPosition(), nullptr);
}

// <specdef
// href="https://html.spec.whatwg.org/C/#creating-a-javascript-module-script">
JSModuleScript* JSModuleScript::CreateInternal(size_t source_text_length, Modulator* modulator, v8::Local<v8::Module> result, const KURL& source_url,
    const KURL& base_url, const ScriptFetchOptions& options, const TextPosition& start_position, ModuleRecordProduceCacheData* produce_cache_data)
{
    ...
    JSModuleScript* module_script
        = MakeGarbageCollected<JSModuleScript>(modulator, result, source_url, base_url, options, source_text_length, start_position, produce_cache_data);
...
JSModuleScript::JSModuleScript(Modulator* settings_object, v8::Local<v8::Module> record, const KURL& source_url, const KURL& base_url,
    const ScriptFetchOptions& fetch_options, size_t source_text_length, const TextPosition& start_position, ModuleRecordProduceCacheData* produce_cache_data)
...
void JSModuleScript::ProduceCache()
{
    ...
    V8CodeCache::ProduceCache(
        isolate, ExecutionContext::GetCodeCacheHostFromContext(execution_context), produce_cache_data_, source_text_length_, SourceUrl(), StartPosition());
