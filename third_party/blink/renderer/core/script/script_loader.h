...
class CORE_EXPORT ScriptLoader final : public ResourceFinishObserver, public NameClient {
public:
    ...
    enum class ScriptTypeAtPrepare : uint8_t { kClassic, kModule, kImportMap, kSpeculationRules, kWebBundle, kInvalid };

    static ScriptTypeAtPrepare GetScriptTypeAtPrepare(const String& type_attribute_value, const String& language_attribute_value);

    static bool BlockForNoModule(ScriptTypeAtPrepare, bool nomodule);

    static network::mojom::CredentialsMode ModuleScriptCredentialsMode(CrossOriginAttributeValue);
    ...
    [[nodiscard]] PendingScript* PrepareScript(ParserBlockingInlineOption, const TextPosition& script_start_position);

    bool IsParserInserted() const
    {
        return parser_inserted_;
    }
    bool AlreadyStarted() const
    {
        return already_started_;
    }
    bool IsForceAsync() const
    {
        return force_async_;
    }
    ScriptTypeAtPrepare GetScriptType() const
    {
        return script_type_;
    }
...
private:
    ...
    void FetchModuleScriptTree(const KURL&, ResourceFetcher*, Modulator*, const ScriptFetchOptions&);
    // Calculate ScriptSchedulingType per spec (#prepare-the-script-element Steps
    // 31-32), before any intervention applied.
    ScriptSchedulingType GetScriptSchedulingTypePerSpec(Document& element_document, ParserBlockingInlineOption) const;
