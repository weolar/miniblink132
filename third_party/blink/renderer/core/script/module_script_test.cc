...
ClassicScript* CreateClassicScript(const String& source_text, CachedMetadataHandler* cache_handler)
{
    return ClassicScript::Create(
        source_text, KURL(), KURL(), ScriptFetchOptions(), ScriptSourceLocationType::kInternal, SanitizeScriptErrors::kSanitize, cache_handler);
}
...
protected:
    ...
    static JSModuleScript* CreateJSModuleScript(Modulator* modulator, const String& source_text, CachedMetadataHandler* cache_handler)
    {
        ModuleScriptCreationParams params(KURL("https://fox.url/script.js"), KURL("https://fox.url/"), ScriptSourceLocationType::kInline,
            ModuleType::kJavaScript, ParkableString(source_text.Impl()->IsolatedCopy()), cache_handler, network::mojom::ReferrerPolicy::kDefault);
        return JSModuleScript::Create(params, modulator, ScriptFetchOptions());
    }

    static ValueWrapperSyntheticModuleScript* CreateValueWrapperSyntheticModuleScript(Modulator* modulator, v8::Local<v8::Value> local_value)
    {
        return ValueWrapperSyntheticModuleScript::CreateWithDefaultExport(
            local_value, modulator, KURL("https://fox.url/script.js"), KURL("https://fox.url/"), ScriptFetchOptions());
    }
    ...
    static void TestFoo(V8TestingScope& scope)
    {
        ...
        EXPECT_EQ(kScriptRepeatLength, value->NumberValue(scope.GetContext()).ToChecked());

        ClassicScript::CreateUnspecifiedScript("window.foo = undefined;")->RunScript(&scope.GetWindow());
    }

    // Accessors for ModuleScript private members.
    static V8CodeCache::ProduceCacheOptions GetProduceCacheOptions(const JSModuleScript* module_script)
    {
        return module_script->produce_cache_data_->GetProduceCacheOptions();
    }

    static bool HandlerCachedMetadataWasDiscarded(CachedMetadataHandler* cache_handler)
    {
...
TEST_F(ModuleScriptTest, V8CodeCache)
{
    ...
    Modulator* modulator = MakeGarbageCollected<ModuleScriptTestModulator>(scope.GetScriptState());
    ...
    CachedMetadataHandler* cache_handler = MakeGarbageCollected<ScriptCachedMetadataHandler>(UTF8Encoding(), std::move(sender));
    ...
    for (int nth_load = 0; nth_load < 3; ++nth_load) {
        ...
        JSModuleScript* module_script = CreateJSModuleScript(modulator, LargeSourceText(), cache_handler);
        ASSERT_TRUE(module_script);

        // Check that the module script is instantiated/evaluated correctly.
        ASSERT_TRUE(ModuleRecord::Instantiate(scope.GetScriptState(), module_script->V8Module(), module_script->SourceUrl()).IsEmpty());
        ASSERT_EQ(module_script->RunScriptOnScriptStateAndReturnValue(scope.GetScriptState()).GetResultType(), ScriptEvaluationResult::ResultType::kSuccess);
        TestFoo(scope);
        ...
        case 0:
            ...
            EXPECT_EQ(V8CodeCache::ProduceCacheOptions::kSetTimeStamp, GetProduceCacheOptions(module_script));
        ...
        case 1:
            ...
            EXPECT_FALSE(cache_handler->GetCachedMetadata(kCodeTag));
            EXPECT_EQ(V8CodeCache::ProduceCacheOptions::kProduceCodeCache, GetProduceCacheOptions(module_script));
            EXPECT_CALL(*sender_ptr, Send(_, _));
            break;
        ...
        case 2:
            ...
            EXPECT_EQ(V8CodeCache::ProduceCacheOptions::kNoProduceCache, GetProduceCacheOptions(module_script));
            break;
        }
    ...
    CreateClassicScript(LargeSourceText(), cache_handler)->RunScript(&scope.GetWindow());
...
TEST_F(ModuleScriptTest, ValueWrapperSyntheticModuleScript)
{
    ...
    Modulator* modulator = MakeGarbageCollected<ModuleScriptTestModulator>(scope.GetScriptState());
    ValueWrapperSyntheticModuleScript* module_script = CreateValueWrapperSyntheticModuleScript(modulator, local_value);
...
TEST_F(ModuleScriptTest, V8CodeCacheWithHashChecking)
{
    ...
    auto sender = std::make_unique<MockCachedMetadataSender>();
    MockCachedMetadataSender* sender_ptr = sender.get();
    ScriptCachedMetadataHandlerWithHashing* cache_handler = MakeGarbageCollected<ScriptCachedMetadataHandlerWithHashing>(UTF8Encoding(), std::move(sender));
    const uint32_t kTimeStampTag = V8CodeCache::TagForTimeStamp(cache_handler);
    const uint32_t kCodeTag = V8CodeCache::TagForCodeCache(cache_handler);
    ...
    for (int nth_load = 0; nth_load < 6; ++nth_load) {
        ...
        // Compile a module script.
        String source = LargeSourceText((nth_load == 1 || nth_load == 2) ? " " : nullptr);
        cache_handler->ResetForTesting();
        JSModuleScript* module_script = CreateJSModuleScript(modulator, source, cache_handler);
        ASSERT_TRUE(module_script);
