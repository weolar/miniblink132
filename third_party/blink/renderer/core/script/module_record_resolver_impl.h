...
class CORE_EXPORT ModuleRecordResolverImpl final : public ModuleRecordResolver, public ExecutionContextLifecycleObserver {
public:
    explicit ModuleRecordResolverImpl(Modulator* modulator, ExecutionContext* execution_context)
...
private:
    ...
    const ModuleScript* GetModuleScriptFromModuleRecord(v8::Local<v8::Module>) const final;
    ...
    v8::Local<v8::Module> Resolve(const ModuleRequest& module_request, v8::Local<v8::Module> referrer, ExceptionState&) final;
    ...
    HeapHashMap<Member<BoxedV8Module>, Member<const ModuleScript>> record_to_module_script_map_;
