...
WebFrameWidget* FrameWidgetTestHelper::CreateTestWebFrameWidget(base::PassKey<WebLocalFrame> pass_key,
    ...
    bool is_for_scalable_page, content::TestRunner* test_runner)
{
    return MakeGarbageCollected<WebTestWebFrameWidgetImpl>(pass_key, std::move(frame_widget_host), std::move(frame_widget), std::move(widget_host),
        ...
        is_for_scalable_page, test_runner);
}

WebTestWebFrameWidgetImpl::WebTestWebFrameWidgetImpl(base::PassKey<WebLocalFrame> pass_key,
    CrossVariantMojoAssociatedRemote<mojom::blink::FrameWidgetHostInterfaceBase> frame_widget_host,
    CrossVariantMojoAssociatedReceiver<mojom::blink::FrameWidgetInterfaceBase> frame_widget,
    CrossVariantMojoAssociatedRemote<mojom::blink::WidgetHostInterfaceBase> widget_host,
    CrossVariantMojoAssociatedReceiver<mojom::blink::WidgetInterfaceBase> widget, scoped_refptr<base::SingleThreadTaskRunner> task_runner,
    const viz::FrameSinkId& frame_sink_id, bool hidden, bool never_composited, bool is_for_child_local_root, bool is_for_nested_main_frame,
    bool is_for_scalable_page, content::TestRunner* test_runner)
    : WebFrameWidgetImpl(pass_key, std::move(frame_widget_host), std::move(frame_widget), std::move(widget_host), std::move(widget), std::move(task_runner),
...
void WebTestWebFrameWidgetImpl::UpdateAllLifecyclePhasesAndComposite(base::OnceClosure callback)
{
    LayerTreeHost()->RequestSuccessfulPresentationTimeForNextFrame(base::IgnoreArgs<const viz::FrameTimingDetails&>(std::move(callback)));
...
void WebTestWebFrameWidgetImpl::ScheduleAnimationInternal(bool do_raster)
{
    ...
    if (!animation_scheduled_) {
        ...
        frame->GetTaskRunner(TaskType::kInternalTest)
            ->PostDelayedTask(FROM_HERE, WTF::BindOnce(&WebTestWebFrameWidgetImpl::AnimateNow, WrapWeakPersistent(this)), base::Milliseconds(1));
    }
}
...
void WebTestWebFrameWidgetImpl::StartDragging(LocalFrame* source_frame, const WebDragData& data, DragOperationsMask mask, const SkBitmap& drag_image,
    const gfx::Vector2d& cursor_offset, const gfx::Rect& drag_obj_rect)
{
    if (!GetTestRunner()->AutomaticDragDropEnabled()) {
        return WebFrameWidgetImpl::StartDragging(source_frame, data, mask, drag_image, cursor_offset, drag_obj_rect);
    }
...
FrameWidgetTestHelper* WebTestWebFrameWidgetImpl::GetFrameWidgetTestHelperForTesting()
{
    return this;
}
...
void WebTestWebFrameWidgetImpl::Reset()
{
    ...
    if (ForMainFrame()) {
        ...
        SetTextZoomFactor(1);
        LocalRootImpl()->GetFrame()->GetEventHandler().ResetLastMousePositionForWebTest();
    }
}
...
void WebTestWebFrameWidgetImpl::SynchronouslyCompositeAfterTest(base::OnceClosure callback)
{
...
// static
void WebTestWebFrameWidgetImpl::DoComposite(cc::LayerTreeHost* layer_tree_host, bool do_raster, base::OnceClosure callback)
{
    ...
    layer_tree_host->CompositeForTest(base::TimeTicks::Now(), do_raster, std::move(callback));
}

void WebTestWebFrameWidgetImpl::SynchronouslyComposite(base::OnceClosure callback, bool do_raster)
{
    ...
    if (base::FeatureList::IsEnabled(blink::features::kNoForcedFrameUpdatesForWebTests) && LayerTreeHost()->MainFrameUpdatesAreDeferred()) {
        if (callback) {
            std::move(callback).Run();
        }
        return;
    ...
    DoComposite(popup->LayerTreeHostForTesting(), do_raster, std::move(wrapped_callback));
}
...
void WebTestWebFrameWidgetImpl::AnimateNow()
{
    ...
    if (LocalRootImpl()->ViewImpl()->does_composite() && !LayerTreeHost()->IsVisible()) {
...
void WebTestWebFrameWidgetImpl::RequestDecode(const PaintImage& image, base::OnceCallback<void(bool)> callback)
{
