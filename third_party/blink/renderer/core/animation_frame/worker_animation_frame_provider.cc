...
WorkerAnimationFrameProvider::WorkerAnimationFrameProvider(ExecutionContext* context, const BeginFrameProviderParams& begin_frame_provider_params)
    : begin_frame_provider_(MakeGarbageCollected<BeginFrameProvider>(begin_frame_provider_params, this, context))
...
int WorkerAnimationFrameProvider::RegisterCallback(FrameCallback* callback)
{
    ...
    FrameRequestCallbackCollection::CallbackId id = callback_collection_.RegisterFrameCallback(callback);
...
void WorkerAnimationFrameProvider::BeginFrame(const viz::BeginFrameArgs& args)
{
    TRACE_EVENT_WITH_FLOW0(
        "blink", "WorkerAnimationFrameProvider::BeginFrame", TRACE_ID_GLOBAL(args.trace_id), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT);

    context_->GetAgent()->event_loop()->EnqueueMicrotask(WTF::BindOnce(
        [](WeakPersistent<WorkerAnimationFrameProvider> provider, const viz::BeginFrameArgs& args) {
            if (!provider)
                return;
            TRACE_EVENT_WITH_FLOW0("blink", "WorkerAnimationFrameProvider::RequestAnimationFrame", TRACE_ID_GLOBAL(args.trace_id), TRACE_EVENT_FLAG_FLOW_IN);
            {
                ...
                DCHECK(global_scope);
                base::TimeDelta relative_time = args.frame_time.is_null() ? base::TimeDelta() : args.frame_time - global_scope->TimeOrigin();
                double time = Performance::ClampTimeResolution(relative_time, provider->context_->CrossOriginIsolatedCapability());
...
scoped_refptr<base::SingleThreadTaskRunner> WorkerAnimationFrameProvider::GetCompositorTaskRunner()
{
    return context_->GetScheduler()->CompositorTaskRunner();
}

void WorkerAnimationFrameProvider::RegisterOffscreenCanvas(OffscreenCanvas* context)
{
    auto result = offscreen_canvases_.insert(context);
    DCHECK(result.is_new_entry);
}

void WorkerAnimationFrameProvider::DeregisterOffscreenCanvas(OffscreenCanvas* offscreen_canvas)
{
