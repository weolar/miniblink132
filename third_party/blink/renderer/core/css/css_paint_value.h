...
public:
    ...
    CSSPaintValue(CSSCustomIdentValue* name, HeapVector<Member<CSSVariableData>>&&);
    ~CSSPaintValue();
    ...
    scoped_refptr<Image> GetImage(const ImageResourceObserver&, const Document&, const ComputedStyle&, const gfx::SizeF& target_size);

    bool KnownToBeOpaque(const Document&, const ComputedStyle&) const;

    bool Equals(const CSSPaintValue&) const;

    const Vector<CSSPropertyID>* NativeInvalidationProperties(const Document&) const;
    const Vector<AtomicString>* CustomInvalidationProperties(const Document&) const;

    const CSSStyleValueVector* GetParsedInputArgumentsForTesting()
    {
        return parsed_input_arguments_.Get();
    }
    void BuildInputArgumentValuesForTesting(Vector<std::unique_ptr<CrossThreadStyleValue>>& style_value)
    {
        BuildInputArgumentValues(style_value);
    }

    bool IsUsingCustomProperty(const AtomicString& custom_property_name, const Document&) const;

    void CreateGeneratorForTesting(const Document& document)
    {
        EnsureGenerator(document);
    }
    unsigned NumberOfGeneratorsForTesting() const
    {
...
private:
    ...
    public:
        explicit Observer(CSSPaintValue* owner_value)
    ...
    void BuildInputArgumentValues(Vector<std::unique_ptr<CrossThreadStyleValue>>&);
    ...
    HeapHashMap<WeakMember<const Document>, Member<CSSPaintImageGenerator>> generators_;
...
template <> struct DowncastTraits<CSSPaintValue> {
    static bool AllowFrom(const CSSValue& value)
    {
        return value.IsPaintValue();
    }
};
