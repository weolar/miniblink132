...
TEST(StyleSheetContentsTest, InsertMediaRule)
{
    ...
    style_sheet->WrapperInsertRule(CSSParser::ParseRule(context, style_sheet, CSSNestingType::kNone,
                                       /*parent_rule_for_nesting=*/nullptr,
                                       /*is_within_scope=*/false, "@media all { div { color: pink } }"),
        0);
    EXPECT_EQ(1U, style_sheet->RuleCount());
    ...
    style_sheet->WrapperInsertRule(CSSParser::ParseRule(context, style_sheet, CSSNestingType::kNone,
                                       /*parent_rule_for_nesting=*/nullptr,
                                       /*is_within_scope=*/false, "@media all { div { color: green } }"),
        1);
...
TEST(StyleSheetContentsTest, InsertFontFaceRule)
{
    ...
    style_sheet->WrapperInsertRule(CSSParser::ParseRule(context, style_sheet, CSSNestingType::kNone,
                                       /*parent_rule_for_nesting=*/nullptr,
                                       /*is_within_scope=*/false, "@font-face { font-family: a }"),
        0);
    EXPECT_EQ(1U, style_sheet->RuleCount());
    ...
    style_sheet->WrapperInsertRule(CSSParser::ParseRule(context, style_sheet, CSSNestingType::kNone,
                                       /*parent_rule_for_nesting=*/nullptr,
                                       /*is_within_scope=*/false, "@font-face { font-family: b }"),
        1);
    EXPECT_EQ(2U, style_sheet->RuleCount());
    EXPECT_TRUE(style_sheet->HasFontFaceRule());
}

TEST(StyleSheetContentsTest, HasFailedOrCanceledSubresources_StartingStyleCrash)
{
    auto* context = MakeGarbageCollected<CSSParserContext>(kHTMLStandardMode, SecureContextMode::kInsecureContext);
