...
Element* ClosestInclusiveAncestorContainer(Element& element, Element* stay_within = nullptr)
{
    for (auto* container = &element; container && container != stay_within; container = ContainerQueryEvaluator::ParentContainerCandidateElement(*container)) {
...
StyleRecalcContext StyleRecalcContext::FromAncestors(Element& element)
{
    ...
    if (Element* parent = ContainerQueryEvaluator::ParentContainerCandidateElement(element)) {
...
StyleRecalcContext StyleRecalcContext::ForSlottedRules(HTMLSlotElement& slot) const
{
    ...
    if (Element* shadow_container = ClosestInclusiveAncestorContainer(slot, slot.OwnerShadowHost())) {
