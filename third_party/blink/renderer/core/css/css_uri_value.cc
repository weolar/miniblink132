...
{
    if (!resource_) {
        resource_ = MakeGarbageCollected<ExternalSVGResourceDocumentContent>(AbsoluteUrl());
    }
...
const AtomicString& CSSURIValue::NormalizedFragmentIdentifier() const
{
    if (normalized_fragment_identifier_cache_.IsNull()) {
        normalized_fragment_identifier_cache_ = AtomicString(DecodeURLEscapeSequences(FragmentIdentifier(), DecodeURLMode::kUTF8OrIsomorphic));
    }
    ...
    DCHECK_EQ(normalized_fragment_identifier_cache_, AtomicString(DecodeURLEscapeSequences(FragmentIdentifier(), DecodeURLMode::kUTF8OrIsomorphic)));
...
CSSURIValue* CSSURIValue::ComputedCSSValue(const KURL& base_url, const WTF::TextEncoding& charset) const
{
    return MakeGarbageCollected<CSSURIValue>(url_data_.MakeResolved(base_url, charset));
}
