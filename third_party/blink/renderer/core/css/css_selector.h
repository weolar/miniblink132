...
public:
    ...
    explicit CSSSelector(MatchType match_type, const QualifiedName& attribute, AttributeMatchType case_sensitivity);
    explicit CSSSelector(MatchType match_type, const QualifiedName& attribute, AttributeMatchType case_sensitivity, const AtomicString& value);
    ...
    static const AtomicString& UniversalSelectorAtom()
    {
        return g_null_atom;
    }
    ...
    {
        return HasRareData() ? data_.rare_data_->bits_.has_.contains_pseudo_ : false;
    }
    bool ContainsComplexLogicalCombinationsInsideHasPseudoClass() const
    {
        return HasRareData() ? data_.rare_data_->bits_.has_.contains_complex_logical_combinations_ : false;
    }
    bool HasArgumentMatchInShadowTree() const
    {
        return HasRareData() ? data_.rare_data_->bits_.has_.argument_match_in_shadow_tree_ : false;
    }
    ...
    {
        return GetPseudoType() == kPseudoHost || GetPseudoType() == kPseudoHostContext;
    }
    ...
    enum LinkMatchMask { kMatchLink = 1, kMatchVisited = 2, kMatchAll = kMatchLink | kMatchVisited };
    ...
    {
        return bits_.get<HasRareDataField>();
    }

    bool IsForPage() const
    {
        return bits_.get<IsForPageField>();
    }
    void SetForPage()
    {
        bits_.set<IsForPageField>(true);
    }
    ...
    {
        return bits_.get<IsImplicitlyAddedField>();
    }
...
private:
    ...
    using RelationField = BitField::DefineFirstValue<uint32_t, 4>; // RelationType
    using MatchField = RelationField::DefineNextValue<uint32_t, 4>; // MatchType
    using PseudoTypeField = MatchField::DefineNextValue<uint32_t, 8>; // PseudoType
    using IsLastInSelectorListField = PseudoTypeField::DefineNextValue<bool, 1>;
    using IsLastInComplexSelectorField = IsLastInSelectorListField::DefineNextValue<bool, 1>;
    using HasRareDataField = IsLastInComplexSelectorField::DefineNextValue<bool, 1>;
    ...
    using IsCoveredByBucketingField = IsImplicitlyAddedField::DefineNextValue<bool, 1>;
    // Used for attribute selector (with value). Real type is AttributeMatchType.
    using AttributeMatchField = IsCoveredByBucketingField::DefineNextValue<unsigned, 2>;
    using LegacyCaseInsensitiveMatchField = AttributeMatchField::DefineNextValue<bool, 1>;
    // 4 free bits here.
    BitField bits_;
    ...
    template <bool expand_pseudo_parent> bool SerializeSimpleSelector(WTF::StringBuilder& builder) const;

    template <bool expand_pseudo_parent> const CSSSelector* SerializeCompound(WTF::StringBuilder&) const;

    template <bool expand_pseudo_parent> static void SerializeSelectorList(const CSSSelectorList* selector_list, WTF::StringBuilder& builder);

    template <bool expand_pseudo_parent> String SelectorTextInternal() const;
    ...
    struct RareData : public GarbageCollected<RareData> {
        ...
        int NthAValue() const
        {
            return bits_.nth_.a_;
        }
        int NthBValue() const
        {
            return bits_.nth_.b_;
        }
        ...
        Member<CSSSelectorList> selector_list_; // Used :is, :not, :-webkit-any, etc.
        std::unique_ptr<Vector<AtomicString>> ident_list_; // Used for ::part(), :active-view-transition-type().
    ...
    union DataUnion {
        ...
        explicit DataUnion(ConstructEmptyValueTag)
            : value_()
        {
        }
        ...
        explicit DataUnion(const AtomicString& value)
            : value_(value)
        {
        }
        ...
        explicit DataUnion(Member<RareData> rare_data)
...
inline CSSSelector::CSSSelector()
    : bits_(RelationField::encode(kSubSelector) | MatchField::encode(kUnknown) | PseudoTypeField::encode(kPseudoUnknown)
        | IsLastInSelectorListField::encode(false) | IsLastInComplexSelectorField::encode(false) | HasRareDataField::encode(false)
        | IsForPageField::encode(false) | IsImplicitlyAddedField::encode(false) | IsCoveredByBucketingField::encode(false) | AttributeMatchField::encode(0)
...
inline CSSSelector::CSSSelector(const QualifiedName& tag_q_name, bool tag_is_implicit)
    : bits_(RelationField::encode(kSubSelector) | MatchField::encode(kTag) | PseudoTypeField::encode(kPseudoUnknown) | IsLastInSelectorListField::encode(false)
        | IsLastInComplexSelectorField::encode(false) | HasRareDataField::encode(false) | IsForPageField::encode(false)
        | IsImplicitlyAddedField::encode(tag_is_implicit) | IsCoveredByBucketingField::encode(false) | AttributeMatchField::encode(0)
        | LegacyCaseInsensitiveMatchField::encode(false))
    , data_(tag_q_name)
{
}
...
inline CSSSelector::CSSSelector(const StyleRule* parent_rule, bool is_implicit)
    : bits_(RelationField::encode(kSubSelector) | MatchField::encode(kPseudoClass) | PseudoTypeField::encode(kPseudoParent)
        | IsLastInSelectorListField::encode(false) | IsLastInComplexSelectorField::encode(false) | HasRareDataField::encode(false)
        | IsForPageField::encode(false) | IsImplicitlyAddedField::encode(is_implicit) | IsCoveredByBucketingField::encode(false)
        | AttributeMatchField::encode(0) | LegacyCaseInsensitiveMatchField::encode(false))
    , data_(parent_rule)
{
}
...
{
    return GetPseudoType() == kPseudoHover || GetPseudoType() == kPseudoActive || GetPseudoType() == kPseudoFocus || GetPseudoType() == kPseudoDrag
        || GetPseudoType() == kPseudoFocusWithin || GetPseudoType() == kPseudoFocusVisible;
}

inline bool CSSSelector::IsIdClassOrAttributeSelector() const
{
    return IsAttributeSelector() || Match() == CSSSelector::kId || Match() == CSSSelector::kClass;
}
...
namespace WTF {
template <> struct VectorTraits<blink::CSSSelector> : VectorTraitsBase<blink::CSSSelector> {
