...
{
    const CounterStyle* disc = CounterStyleMap::GetUACounterStyleMap()->FindCounterStyleAcrossScopes(keywords::kDisc);
    DCHECK(disc);
    return *disc;
}
...
std::pair<int, int> BoundsToIntegerPair(const CSSValuePair& bounds)
{
    ...
    if (bounds.First().IsIdentifierValue()) {
        DCHECK_EQ(CSSValueID::kInfinite, To<CSSIdentifierValue>(bounds.First()).GetValueID());
    ...
    if (bounds.Second().IsIdentifierValue()) {
        DCHECK_EQ(CSSValueID::kInfinite, To<CSSIdentifierValue>(bounds.Second()).GetValueID());
...
Vector<wtf_size_t> FixedAlgorithm(int value, int first_symbol_value, wtf_size_t num_symbols)
{
    if (value < first_symbol_value || static_cast<unsigned>(value - first_symbol_value) >= num_symbols) {
...
Vector<wtf_size_t> AdditiveAlgorithm(unsigned value, const Vector<unsigned>& weights)
{
    DCHECK(weights.size());
    if (!value) {
        if (weights.back() == 0u) {
    ...
    Vector<wtf_size_t> result;
    for (wtf_size_t index = 0; value && index < weights.size() && weights[index]; ++index) {
...
String CJKIdeoGraphicAlgorithm(unsigned number, const std::array<UChar, 21>& table, CJKStyle cjk_style)
{
    ...
    constexpr unsigned kGroupLength = 9; // 4 digits, 3 digit markers, group marker of size 2.
    ...
    for (unsigned i = 0; i < 4; ++i) {
        ...
        base::span<AbstractCJKChar> group = base::span(buffer).subspan((3 - i) * kGroupLength);
        ...
        if (digit_value) {
            bool drop_one = table[kLang] == kKorean && cjk_style == kInformal && digit_value == 1 && i > 0;
            if (!drop_one) {
        ...
        if (number != 0 || group_value > 9) {
            ...
            bool drop_one = table[kLang] == kKorean && cjk_style == kInformal && digit_value == 1;
        ...
        if (table[kLang] == kChinese && cjk_style == kInformal && group_value < 20) {
        ...
        if (a != kNoChar) {
            if (a != kDigit0 || (table[kLang] == kChinese && last != kDigit0)) {
                ...
                if (new_char != kNoChar) {
                    ...
                    if (table[kLang] == kKorean && (a == kSecondGroupMarker || a == kThirdGroupMarker || a == kFourthGroupMarker)) {
                        characters[length++] = ' ';
                    }
                }
            }
    ...
    if ((table[kLang] == kChinese && last == kDigit0) || characters[length - 1] == ' ') {
        --length;
    }
...
{
    constexpr std::array<UChar, 21> kSimpleChineseInformalTable = { kChinese, 0x4E07, 0x0000, 0x4EBF, 0x0000, 0x4E07, 0x4EBF, 0x5341, 0x767E, 0x5343, 0x96F6,
        0x4E00, 0x4E8C, 0x4E09, 0x56DB, 0x4E94, 0x516D, 0x4E03, 0x516B, 0x4E5D, 0x0000 };
...
{
    constexpr std::array<UChar, 21> kSimpleChineseFormalTable = { kChinese, 0x4E07, 0x0000, 0x4EBF, 0x0000, 0x4E07, 0x4EBF, 0x62FE, 0x4F70, 0x4EDF, 0x96F6,
        0x58F9, 0x8D30, 0x53C1, 0x8086, 0x4F0D, 0x9646, 0x67D2, 0x634C, 0x7396, 0x0000 };
    return CJKIdeoGraphicAlgorithm(value, kSimpleChineseFormalTable, kFormal);
}

String TradChineseInformalAlgorithm(unsigned value)
{
    constexpr std::array<UChar, 21> kTraditionalChineseInformalTable = { kChinese, 0x842C, 0x0000, 0x5104, 0x0000, 0x5146, 0x0000, 0x5341, 0x767E, 0x5343,
        0x96F6, 0x4E00, 0x4E8C, 0x4E09, 0x56DB, 0x4E94, 0x516D, 0x4E03, 0x516B, 0x4E5D, 0x0000 };
    return CJKIdeoGraphicAlgorithm(value, kTraditionalChineseInformalTable, kInformal);
}

String TradChineseFormalAlgorithm(unsigned value)
{
    constexpr std::array<UChar, 21> kTraditionalChineseFormalTable = { kChinese, 0x842C, 0x0000, 0x5104, 0x0000, 0x5146, 0x0000, 0x62FE, 0x4F70, 0x4EDF, 0x96F6,
        0x58F9, 0x8CB3, 0x53C3, 0x8086, 0x4F0D, 0x9678, 0x67D2, 0x634C, 0x7396, 0x0000 };
    return CJKIdeoGraphicAlgorithm(value, kTraditionalChineseFormalTable, kFormal);
}

String KoreanHangulFormalAlgorithm(unsigned value)
{
    constexpr std::array<UChar, 21> kKoreanHangulFormalTable = { kKorean, 0xB9CC, 0x0000, 0xC5B5, 0x0000, 0xC870, 0x0000, 0xC2ED, 0xBC31, 0xCC9C, 0xC601,
        0xC77C, 0xC774, 0xC0BC, 0xC0AC, 0xC624, 0xC721, 0xCE60, 0xD314, 0xAD6C, 0x0000 };
    return CJKIdeoGraphicAlgorithm(value, kKoreanHangulFormalTable, kFormal);
}

String KoreanHanjaInformalAlgorithm(unsigned value)
{
    constexpr std::array<UChar, 21> kKoreanHanjaInformalTable = { kKorean, 0x842C, 0x0000, 0x5104, 0x0000, 0x5146, 0x0000, 0x5341, 0x767E, 0x5343, 0x96F6,
        0x4E00, 0x4E8C, 0x4E09, 0x56DB, 0x4E94, 0x516D, 0x4E03, 0x516B, 0x4E5D, 0x0000 };
...
{
    constexpr std::array<UChar, 21> kKoreanHanjaFormalTable = { kKorean, 0x842C, 0x0000, 0x5104, 0x0000, 0x5146, 0x0000, 0x62FE, 0x767E, 0x4EDF, 0x96F6, 0x58F9,
        0x8CB3, 0x53C3, 0x56DB, 0x4E94, 0x516D, 0x4E03, 0x516B, 0x4E5D, 0x0000 };
...
String HebrewAlgorithmUnder1000(unsigned number)
{
    ...
    if (number == 15 || number == 16) {
        ...
        if (unsigned tens = number / 10) {
            constexpr std::array<UChar, 9> kHebrewTens = { 1497, 1499, 1500, 1502, 1504, 1505, 1506, 1508, 1510 };
...
String HebrewAlgorithm(unsigned number)
{
    ...
    return HebrewAlgorithmUnder1000(number / 1000) + kHebrewPunctuationGereshCharacter + HebrewAlgorithmUnder1000(number % 1000);
}

String ArmenianAlgorithmUnder10000(unsigned number, bool upper, bool add_circumflex)
{
    ...
    if (unsigned thousands = number / 1000) {
        if (thousands == 7) {
            ...
            letters.Append(static_cast<UChar>((0x054C - 1 + lower_offset) + thousands));
            if (add_circumflex) {
                letters.Append(static_cast<UChar>(0x0302));
            }
        }
...
String ArmenianAlgorithm(unsigned number, bool upper)
{
    ...
    return ArmenianAlgorithmUnder10000(number / 10000, upper, true) + ArmenianAlgorithmUnder10000(number % 10000, upper, false);
}
...
String EthiopicNumericAlgorithm(unsigned value)
{
    ...
    constexpr std::array<UChar, 9> units = { 0x1369, 0x136A, 0x136B, 0x136C, 0x136D, 0x136E, 0x136F, 0x1370, 0x1371 };
    // Ethiopic characters for 10, 20, ..., 90
    constexpr std::array<UChar, 9> tens = { 0x1372, 0x1373, 0x1374, 0x1375, 0x1376, 0x1377, 0x1378, 0x1379, 0x137A };
    if (!value) {
        return String();
    }
    if (value < 10u) {
    ...
    for (bool odd_group = false; value; odd_group = !odd_group) {
        ...
        bool remove_digits = !group_value || (group_value == 1 && most_significant_group) || (group_value == 1 && odd_group);
        if (!remove_digits) {
...
{
    DEFINE_STATIC_LOCAL(Persistent<CounterStyle>, decimal, (CounterStyleMap::GetUACounterStyleMap()->FindCounterStyleAcrossScopes(keywords::kDecimal)));
    DCHECK(decimal);
    return *decimal;
}

// static
CounterStyleSystem CounterStyle::ToCounterStyleSystemEnum(const CSSValue* value)
{
    if (!value) {
...
// static
CounterStyleSpeakAs ToCounterStyleSpeakAsEnum(const CSSIdentifierValue& keyword)
{
...
CounterStyle::CounterStyle(const StyleRuleCounterStyle& rule)
    : style_rule_(rule)
    , style_rule_version_(rule.GetVersion())
{
    ...
    if (HasSymbols(system_)) {
        if (system_ == CounterStyleSystem::kAdditive) {
            for (const auto& symbol : To<CSSValueList>(*rule.GetAdditiveSymbols())) {
                ...
                additive_weights_.push_back(To<CSSPrimitiveValue>(pair.First()).GetIntValue());
...
String CounterStyle::GenerateRepresentation(int value) const
{
    ...
    wtf_size_t pad_copies = pad_length_ > initial_length ? pad_length_ - initial_length : 0;
...
String CounterStyle::GenerateInitialRepresentation(int value) const
{
    ...
    unsigned abs_value = value == std::numeric_limits<int>::min() ? static_cast<unsigned>(std::numeric_limits<int>::max()) + 1u : std::abs(value);

    switch (system_) {
    case CounterStyleSystem::kCyclic:
        return IndexesToString(CyclicAlgorithm(value, symbols_.size()));
    case CounterStyleSystem::kFixed:
        return IndexesToString(FixedAlgorithm(value, first_symbol_value_, symbols_.size()));
    case CounterStyleSystem::kNumeric:
...
String CounterStyle::IndexesToString(const Vector<wtf_size_t>& symbol_indexes) const
{
...
void CounterStyle::TraverseAndMarkDirtyIfNeeded(HeapHashSet<Member<CounterStyle>>& visited_counter_styles)
{
    ...
    if (has_inexistent_references_ || style_rule_version_ != style_rule_->GetVersion()) {
        SetIsDirty();
        return;
    }
...
String CounterStyle::GenerateTextAlternative(int value) const
{
    ...
    String text_without_prefix_suffix = GenerateTextAlternativeWithoutPrefixSuffix(value);
...
String CounterStyle::GenerateTextAlternativeWithoutPrefixSuffix(int value) const
{
