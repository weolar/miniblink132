...
MediaQuerySet* MediaQuerySet::Create(const String& media_string, ExecutionContext* execution_context)
{
...
const MediaQuerySet* MediaQuerySet::CopyAndAdd(const String& query_string, ExecutionContext* execution_context) const
{
...
const MediaQuerySet* MediaQuerySet::CopyAndRemove(const String& query_string_to_remove, ExecutionContext* execution_context) const
{
    // To "parse a media query" for a given string means to follow "the parse
    // a media query list" steps and return "null" if more than one media query
    // is returned, or else the returned media query.
...
void MediaList::setMediaText(ExecutionContext* execution_context, const String& value)
{
...
{
    const HeapVector<Member<const MediaQuery>>& queries = Queries()->QueryVector();
...
void MediaList::deleteMedium(ExecutionContext* execution_context, const String& medium, ExceptionState& exception_state)
{
    CSSStyleSheet::RuleMutationScope mutation_scope(parent_rule_);

    const MediaQuerySet* new_media_queries = Queries()->CopyAndRemove(medium, execution_context);
    if (!new_media_queries) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotFoundError, "Failed to delete '" + medium + "'.");
        return;
    }
...
void MediaList::appendMedium(ExecutionContext* execution_context, const String& medium)
{
    CSSStyleSheet::RuleMutationScope mutation_scope(parent_rule_);

    const MediaQuerySet* new_media_queries = Queries()->CopyAndAdd(medium, execution_context);
    if (!new_media_queries) {
        return;
    }
    Owner()->SetMediaQueries(new_media_queries);
...
void MediaList::NotifyMutation()
{
    if (parent_rule_ && parent_rule_->parentStyleSheet()) {
        StyleSheetContents* parent_contents = parent_rule_->parentStyleSheet()->Contents();
