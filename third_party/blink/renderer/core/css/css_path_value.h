...
public:
    ...
    explicit CSSPathValue(scoped_refptr<StylePath>, PathSerializationFormat = kNoTransformation);
    explicit CSSPathValue(SVGPathByteStream, WindRule wind_rule = RULE_NONZERO, PathSerializationFormat = kNoTransformation);

    StylePath* GetStylePath() const
    {
        return style_path_.get();
    }
...
template <> struct DowncastTraits<cssvalue::CSSPathValue> {
