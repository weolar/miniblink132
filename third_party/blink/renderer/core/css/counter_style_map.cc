...
bool CounterStyleShouldOverride(
    Document& document, const TreeScope* tree_scope, const StyleRuleCounterStyle& new_rule, const StyleRuleCounterStyle& existing_rule)
{
    const CascadeLayerMap* cascade_layer_map
        = tree_scope ? tree_scope->GetScopedStyleResolver()->GetCascadeLayerMap() : document.GetStyleEngine().GetUserCascadeLayerMap();
    if (!cascade_layer_map) {
        return true;
    }
    return cascade_layer_map->CompareLayerOrder(existing_rule.GetCascadeLayer(), new_rule.GetCascadeLayer()) <= 0;
}
...
// static
CounterStyleMap* CounterStyleMap::GetAuthorCounterStyleMap(const TreeScope& scope)
{
...
// static
CounterStyleMap* CounterStyleMap::CreateUserCounterStyleMap(Document& document)
{
    return MakeGarbageCollected<CounterStyleMap>(&document, nullptr);
}

// static
CounterStyleMap* CounterStyleMap::CreateAuthorCounterStyleMap(TreeScope& tree_scope)
{
    return MakeGarbageCollected<CounterStyleMap>(&tree_scope.GetDocument(), &tree_scope);
}
...
void CounterStyleMap::AddCounterStyles(const RuleSet& rule_set)
{
    ...
    for (StyleRuleCounterStyle* rule : rule_set.CounterStyleRules()) {
        ...
        if (replaced_iter != counter_styles_.end()) {
            if (!CounterStyleShouldOverride(*owner_document_, tree_scope_, *rule, replaced_iter->value->GetStyleRule())) {
                continue;
            }
        }
...
CounterStyleMap* CounterStyleMap::GetAncestorMap() const
{
    if (tree_scope_) {
        ...
        for (TreeScope* scope = tree_scope_->ParentTreeScope(); scope; scope = scope->ParentTreeScope()) {
...
CounterStyle* CounterStyleMap::FindCounterStyleAcrossScopes(const AtomicString& name) const
{
    if (!owner_document_) {
...
void CounterStyleMap::ResolveExtendsFor(CounterStyle& counter_style)
{
    ...
    } while (extends_chain.back() && extends_chain.back()->HasUnresolvedExtends() && !unresolved_styles.Contains(extends_chain.back()));
    ...
    if (extends_chain.back() && extends_chain.back()->HasUnresolvedExtends()) {
        ...
        DCHECK(owner_document_) << "'extends' cycle detected for predefined counter style " << counter_style.GetName();
    ...
    while (extends_chain.size() > 1u) {
        ...
        if (next) {
            ...
            DCHECK(owner_document_) << "Can't resolve 'extends: " << extends_chain.back()->GetExtendsName() << "' for predefined counter style "
...
void CounterStyleMap::ResolveFallbackFor(CounterStyle& counter_style)
{
    ...
    if (fallback_style) {
        ...
        // otherwise we'll enter an infinite recursion to look for 'decimal'.
        DCHECK(owner_document_) << "Can't resolve fallback " << fallback_name << " for predefined counter style " << counter_style.GetName();
...
void CounterStyleMap::ResolveSpeakAsReferenceFor(CounterStyle& counter_style)
{
    ...
    } while (speak_as_chain.back() && speak_as_chain.back()->HasUnresolvedSpeakAsReference() && !unresolved_styles.Contains(speak_as_chain.back()));
...
void CounterStyleMap::ResolveReferences(HeapHashSet<Member<CounterStyleMap>>& visited_maps)
{
...
void CounterStyleMap::MarkDirtyCounterStyles(HeapHashSet<Member<CounterStyle>>& visited_counter_styles)
{
    for (CounterStyle* counter_style : counter_styles_.Values()) {
    ...
    for (Member<CounterStyle>& counter_style_ref : counter_styles_.Values()) {
        if (counter_style_ref->IsDirty()) {
            CounterStyle* clean_style = MakeGarbageCollected<CounterStyle>(counter_style_ref->GetStyleRule());
...
// static
void CounterStyleMap::MarkAllDirtyCounterStyles(Document& document, const HeapHashSet<Member<TreeScope>>& active_tree_scopes)
{
...
// static
void CounterStyleMap::ResolveAllReferences(Document& document, const HeapHashSet<Member<TreeScope>>& active_tree_scopes)
{
