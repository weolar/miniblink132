...
void StyleRuleFontFeature::UpdateAlias(AtomicString alias, Vector<uint32_t> features)
{
    feature_aliases_.Set(alias, FeatureIndicesWithPriority { std::move(features), std::numeric_limits<uint16_t>::max() });
}
...
FontFeatureValuesStorage::FontFeatureValuesStorage(FontFeatureAliases stylistic, FontFeatureAliases styleset, FontFeatureAliases character_variant,
    FontFeatureAliases swash, FontFeatureAliases ornaments, FontFeatureAliases annotation)
...
Vector<uint32_t> FontFeatureValuesStorage::ResolveStylistic(const AtomicString& alias) const
{
    return ResolveInternal(stylistic_, alias);
}

Vector<uint32_t> FontFeatureValuesStorage::ResolveStyleset(const AtomicString& alias) const
{
    return ResolveInternal(styleset_, alias);
}

Vector<uint32_t> FontFeatureValuesStorage::ResolveCharacterVariant(const AtomicString& alias) const
{
    return ResolveInternal(character_variant_, alias);
}

Vector<uint32_t> FontFeatureValuesStorage::ResolveSwash(const AtomicString& alias) const
{
    return ResolveInternal(swash_, alias);
}

Vector<uint32_t> FontFeatureValuesStorage::ResolveOrnaments(const AtomicString& alias) const
{
    return ResolveInternal(ornaments_, alias);
}
Vector<uint32_t> FontFeatureValuesStorage::ResolveAnnotation(const AtomicString& alias) const
{
...
/* static */
Vector<uint32_t> FontFeatureValuesStorage::ResolveInternal(const FontFeatureAliases& aliases, const AtomicString& alias)
{
...
StyleRuleFontFeatureValues::StyleRuleFontFeatureValues(Vector<AtomicString> families, FontFeatureAliases stylistic, FontFeatureAliases styleset,
    FontFeatureAliases character_variant, FontFeatureAliases swash, FontFeatureAliases ornaments, FontFeatureAliases annotation)
...
void StyleRuleFontFeatureValues::TraceAfterDispatch(blink::Visitor* visitor) const
{
    StyleRuleBase::TraceAfterDispatch(visitor);
