...
TEST_F(StyleRecalcChangeTest, SuppressRecalc)
{
    ...
    EXPECT_FALSE(StyleRecalcChange().SuppressRecalc().ShouldRecalcStyleFor(*element));
    // The flag should be lost when ForChildren is called.
    EXPECT_TRUE(StyleRecalcChange().SuppressRecalc().ForChildren(*element).ShouldRecalcStyleFor(*element));
}
...
TEST_F(StyleRecalcChangeTestCQ, SkipStyleRecalcForContainer)
{
    ...
    EXPECT_EQ(Color::kBlack, affected->ComputedStyleRef().VisitedDependentColor(GetCSSPropertyColor()));
    ...
    ASSERT_TRUE(affected->GetComputedStyle());
    EXPECT_EQ(Color(0xff, 0x00, 0x00), affected->ComputedStyleRef().VisitedDependentColor(GetCSSPropertyColor()));
    ...
    EXPECT_FALSE(GetDocument().View()->NeedsLayout());
    ASSERT_TRUE(affected->GetComputedStyle());
    EXPECT_EQ(Color(0x00, 0x80, 0x00), affected->ComputedStyleRef().VisitedDependentColor(GetCSSPropertyColor()));
}
...
TEST_F(StyleRecalcChangeTestCQ, DontSkipLayoutRoot)
{
    ...
    Element* outer_child = GetDocument().getElementById(AtomicString("outer_child"));
    Element* inner_child = GetDocument().getElementById(AtomicString("inner_child"));
