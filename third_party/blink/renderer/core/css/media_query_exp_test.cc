...
{
    return MediaQueryExpValue(WrapDouble(value, CSSPrimitiveValue::UnitType::kEms));
}

MediaQueryExpValue RemValue(double value)
{
    return MediaQueryExpValue(WrapDouble(value, CSSPrimitiveValue::UnitType::kRems));
}

MediaQueryExpValue DvhValue(double value)
{
    return MediaQueryExpValue(WrapDouble(value, CSSPrimitiveValue::UnitType::kDynamicViewportHeight));
}

MediaQueryExpValue SvhValue(double value)
{
    return MediaQueryExpValue(WrapDouble(value, CSSPrimitiveValue::UnitType::kSmallViewportHeight));
}

MediaQueryExpValue LvhValue(double value)
{
    return MediaQueryExpValue(WrapDouble(value, CSSPrimitiveValue::UnitType::kLargeViewportHeight));
}

MediaQueryExpValue VhValue(double value)
{
    return MediaQueryExpValue(WrapDouble(value, CSSPrimitiveValue::UnitType::kViewportHeight));
}

MediaQueryExpValue CqhValue(double value)
{
    return MediaQueryExpValue(WrapDouble(value, CSSPrimitiveValue::UnitType::kContainerHeight));
}
...
{
    return MediaQueryExpValue(WrapDouble(value, CSSPrimitiveValue::UnitType::kDotsPerPixel));
}
...
MediaQueryExpValue CalcValue(const String& syntax, const String& value)
{
    ...
    const auto* calc_value
        = DynamicTo<CSSPrimitiveValue>(css_test_helpers::ParseValue(*Document::CreateForTest(execution_context.GetExecutionContext()), syntax, value));
    EXPECT_NE(calc_value, nullptr);

    return CssValue(*calc_value);
}

MediaQueryExpValue NumericLiteralValue(double value, CSSPrimitiveValue::UnitType unit)
{
...
{
    return MediaQueryExp::Create(AtomicString(feature), MediaQueryExpBounds(cmp, NoCmp(InvalidValue())));
}

MediaQueryExp RightExp(String feature, MediaQueryExpComparison cmp)
{
    return MediaQueryExp::Create(AtomicString(feature), MediaQueryExpBounds(NoCmp(InvalidValue()), cmp));
}

MediaQueryExp PairExp(String feature, MediaQueryExpComparison left, MediaQueryExpComparison right)
{
    return MediaQueryExp::Create(AtomicString(feature), MediaQueryExpBounds(left, right));
}
...
{
    return MediaQueryExpNode::Nested(MakeGarbageCollected<MediaQueryFeatureExpNode>(expr));
}
...
const MediaQueryExpNode* FunctionNode(const MediaQueryExpNode* child, const AtomicString& name)
{
...
const MediaQueryExpNode* AndNode(const MediaQueryExpNode* left, const MediaQueryExpNode* right)
{
    return MediaQueryExpNode::And(left, right);
}

const MediaQueryExpNode* OrNode(const MediaQueryExpNode* left, const MediaQueryExpNode* right)
{
...
TEST(MediaQueryExpTest, BoundaryEquality)
{
    ...
    EXPECT_EQ(MediaQueryExpBounds(LtCmp(px1), LeCmp(px1)), MediaQueryExpBounds(LtCmp(px1), LeCmp(px1)));

    EXPECT_NE(MediaQueryExpBounds(LtCmp(px1), LeCmp(px1)), MediaQueryExpBounds(GtCmp(px1), LeCmp(px1)));
    EXPECT_NE(MediaQueryExpBounds(LtCmp(px1), LeCmp(px1)), MediaQueryExpBounds(LtCmp(px1), GeCmp(px1)));
    EXPECT_NE(MediaQueryExpBounds(LtCmp(px1), LeCmp(px2)), MediaQueryExpBounds(LtCmp(px1), LeCmp(px1)));
}
...
TEST(MediaQueryExpTest, Serialize)
{
    ...
    EXPECT_EQ("10px < width < 20px", PairExp("width", LtCmp(PxValue(10.0)), LtCmp(PxValue(20.0))).Serialize());
    EXPECT_EQ("20px > width > 10px", PairExp("width", GtCmp(PxValue(20.0)), GtCmp(PxValue(10.0))).Serialize());
    EXPECT_EQ("10px <= width <= 20px", PairExp("width", LeCmp(PxValue(10.0)), LeCmp(PxValue(20.0))).Serialize());
    EXPECT_EQ("20px > width >= 10px", PairExp("width", GtCmp(PxValue(20.0)), GeCmp(PxValue(10.0))).Serialize());
}

TEST(MediaQueryExpTest, SerializeNode)
{
    test::TaskEnvironment task_environment;
    EXPECT_EQ("width < 10px", FeatureNode(RightExp("width", LtCmp(PxValue(10))))->Serialize());

    EXPECT_EQ("(width < 10px)", EnclosedFeatureNode(RightExp("width", LtCmp(PxValue(10))))->Serialize());

    EXPECT_EQ("(width < 10px) and (11px >= thing) and (height = 12px)",
        AndNode(EnclosedFeatureNode(RightExp("width", LtCmp(PxValue(10)))),
            AndNode(EnclosedFeatureNode(LeftExp("thing", GeCmp(PxValue(11)))), EnclosedFeatureNode(RightExp("height", EqCmp(PxValue(12))))))
            ->Serialize());

    // Same as previous, but with 'or' instead:
    EXPECT_EQ("(width < 10px) or (11px >= thing) or (height = 12px)",
        OrNode(EnclosedFeatureNode(RightExp("width", LtCmp(PxValue(10)))),
            OrNode(EnclosedFeatureNode(LeftExp("thing", GeCmp(PxValue(11)))), EnclosedFeatureNode(RightExp("height", EqCmp(PxValue(12))))))
            ->Serialize());

    EXPECT_EQ("not (width < 10px)", NotNode(EnclosedFeatureNode(RightExp("width", LtCmp(PxValue(10)))))->Serialize());

    EXPECT_EQ("((width < 10px))", NestedNode(EnclosedFeatureNode(RightExp("width", LtCmp(PxValue(10)))))->Serialize());
    EXPECT_EQ("(((width < 10px)))", NestedNode(NestedNode(EnclosedFeatureNode(RightExp("width", LtCmp(PxValue(10))))))->Serialize());

    EXPECT_EQ("not ((11px >= thing) and (height = 12px))",
        NotNode(NestedNode(AndNode(EnclosedFeatureNode(LeftExp("thing", GeCmp(PxValue(11)))), EnclosedFeatureNode(RightExp("height", EqCmp(PxValue(12)))))))
            ->Serialize());

    EXPECT_EQ("special(width < 10px)", FunctionNode(FeatureNode(RightExp("width", LtCmp(PxValue(10)))), AtomicString("special"))->Serialize());
    EXPECT_EQ("special((width < 10px))", FunctionNode(EnclosedFeatureNode(RightExp("width", LtCmp(PxValue(10)))), AtomicString("special"))->Serialize());
    EXPECT_EQ("special((11px >= thing) and (height = 12px))",
        FunctionNode(AndNode(EnclosedFeatureNode(LeftExp("thing", GeCmp(PxValue(11)))), EnclosedFeatureNode(RightExp("height", EqCmp(PxValue(12))))),
...
TEST(MediaQueryExpTest, CollectExpressions)
{
    ...
    {
        HeapVector<MediaQueryExp> expressions;
        AndNode(EnclosedFeatureNode(width_lt10), EnclosedFeatureNode(height_lt10))->CollectExpressions(expressions);
    ...
    {
        HeapVector<MediaQueryExp> expressions;
        OrNode(EnclosedFeatureNode(width_lt10), EnclosedFeatureNode(height_lt10))->CollectExpressions(expressions);
        ASSERT_EQ(2u, expressions.size());
        EXPECT_EQ(width_lt10, expressions[0]);
        EXPECT_EQ(height_lt10, expressions[1]);
    }
    ...
    {
        HeapVector<MediaQueryExp> expressions;
        NestedNode(EnclosedFeatureNode(width_lt10))->CollectExpressions(expressions);
        ASSERT_EQ(1u, expressions.size());
        EXPECT_EQ(width_lt10, expressions[0]);
    }
...
TEST(MediaQueryExpTest, UnitFlags)
{
    test::TaskEnvironment task_environment;
    // width < 10px
    EXPECT_EQ(MediaQueryExpValue::UnitFlags::kNone, RightExp("width", LtCmp(PxValue(10.0))).GetUnitFlags());
    // width < 10em
    EXPECT_EQ(MediaQueryExpValue::UnitFlags::kFontRelative, RightExp("width", LtCmp(EmValue(10.0))).GetUnitFlags());
    // width < 10rem
    EXPECT_EQ(MediaQueryExpValue::UnitFlags::kRootFontRelative, RightExp("width", LtCmp(RemValue(10.0))).GetUnitFlags());
    // 10px < width
    EXPECT_EQ(MediaQueryExpValue::UnitFlags::kNone, LeftExp("width", LtCmp(PxValue(10.0))).GetUnitFlags());
    // 10em <  width
    EXPECT_EQ(MediaQueryExpValue::UnitFlags::kFontRelative, LeftExp("width", LtCmp(EmValue(10.0))).GetUnitFlags());
    // 10rem < width
    EXPECT_EQ(MediaQueryExpValue::UnitFlags::kRootFontRelative, LeftExp("width", LtCmp(RemValue(10.0))).GetUnitFlags());
    // 10dvh < width
    EXPECT_EQ(MediaQueryExpValue::UnitFlags::kDynamicViewport, LeftExp("width", LtCmp(DvhValue(10.0))).GetUnitFlags());
    // 10svh < width
    EXPECT_EQ(MediaQueryExpValue::UnitFlags::kStaticViewport, LeftExp("width", LtCmp(SvhValue(10.0))).GetUnitFlags());
    // 10lvh < width
    EXPECT_EQ(MediaQueryExpValue::UnitFlags::kStaticViewport, LeftExp("width", LtCmp(LvhValue(10.0))).GetUnitFlags());
    // 10vh < width
    EXPECT_EQ(MediaQueryExpValue::UnitFlags::kStaticViewport, LeftExp("width", LtCmp(VhValue(10.0))).GetUnitFlags());
    // 10cqh < width
    EXPECT_EQ(MediaQueryExpValue::UnitFlags::kContainer, LeftExp("width", LtCmp(CqhValue(10.0))).GetUnitFlags());

    // width < calc(10em + 10dvh)
    ScopedNullExecutionContext execution_context;
    const auto* calc_value = DynamicTo<CSSPrimitiveValue>(
        css_test_helpers::ParseValue(*Document::CreateForTest(execution_context.GetExecutionContext()), "<length>", "calc(10em + 10dvh)"));
    ASSERT_TRUE(calc_value);
    EXPECT_EQ(static_cast<unsigned>(MediaQueryExpValue::UnitFlags::kFontRelative | MediaQueryExpValue::UnitFlags::kDynamicViewport),
...
TEST(MediaQueryExpTest, ResolutionChecks)
{
    ...
    EXPECT_FALSE(NumericLiteralValue(3, CSSPrimitiveValue::UnitType::kPixels).IsResolution());
}
