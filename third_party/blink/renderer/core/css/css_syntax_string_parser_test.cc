...
TEST_F(CSSSyntaxStringParserTest, ValidDataType)
{
    ...
    EXPECT_EQ(CSSSyntaxType::kLengthPercentage, *ParseSingleType("<length-percentage>"));
    ...
    EXPECT_EQ(CSSSyntaxType::kTransformFunction, *ParseSingleType("<transform-function>"));
    EXPECT_EQ(CSSSyntaxType::kTransformList, *ParseSingleType("<transform-list>"));
...
TEST_F(CSSSyntaxStringParserTest, Multipliers)
{
...

    {
        ...
        EXPECT_EQ(1u, desc->Components().size());
        EXPECT_EQ(CSSSyntaxType::kLength, desc->Components()[0].GetType());
        EXPECT_EQ(CSSSyntaxRepeat::kSpaceSeparated, desc->Components()[0].GetRepeat());
    }

    {
        ...
        EXPECT_EQ(CSSSyntaxRepeat::kCommaSeparated, desc->Components()[0].GetRepeat());
    }
...
{
    {
        ...
        EXPECT_EQ(CSSSyntaxRepeat::kSpaceSeparated, desc->Components()[0].GetRepeat());
        EXPECT_EQ(CSSSyntaxType::kColor, desc->Components()[1].GetType());
        EXPECT_EQ(CSSSyntaxRepeat::kCommaSeparated, desc->Components()[1].GetRepeat());
    }

    {
        ...
        EXPECT_EQ(CSSSyntaxType::kIdent, desc->Components()[2].GetType());
        EXPECT_EQ(CSSSyntaxRepeat::kCommaSeparated, desc->Components()[2].GetRepeat());
    }
}
