...
public:
    ...
    const CSSUrlData& UrlData() const
    {
        return url_data_;
    }
    ...
    CSSURIValue* ComputedCSSValue(const KURL& base_url, const WTF::TextEncoding&) const;
...
template <> struct DowncastTraits<cssvalue::CSSURIValue> {
    static bool AllowFrom(const CSSValue& value)
    {
        return value.IsURIValue();
    }
};
