...
class CORE_EXPORT ContainerQueryEvaluator final : public GarbageCollected<ContainerQueryEvaluator> {
public:
    ...
    static Element* FindContainer(Element* starting_element, const ContainerSelector&, const TreeScope* selector_tree_scope);
    static bool EvalAndAdd(Element* style_container_candidate, const StyleRecalcContext&, const ContainerQuery&, ContainerSelectorCache&, MatchResult&);
    ...
    void SetReferencedByUnit()
    {
        referenced_by_unit_ = true;
    }
    bool DependsOnStyle() const
    {
        return depends_on_style_;
    }
    bool DependsOnStuck() const
    {
        return depends_on_stuck_;
    }
    bool DependsOnSnapped() const
    {
        return depends_on_snapped_;
    }
    bool DependsOnOverflowing() const
    {
        return depends_on_overflowing_;
    }
    bool DependsOnSize() const
    {
        return depends_on_size_;
    }
    bool MayDependOnWritingDirection() const
    {
        return DependsOnSize() || DependsOnStuck() || DependsOnSnapped() || DependsOnOverflowing();
    }
    ...
    void SetPendingSnappedStateFromScrollSnapshot(const SnappedQueryScrollSnapshot&);
...
private:
    ...
    void UpdateContainerStuck(ContainerStuckPhysical stuck_horizontal, ContainerStuckPhysical stuck_vertical);

    // Update the CSSContainerValues with the new stuck state.
    void UpdateContainerSnapped(ContainerSnappedFlags snapped);

    // Update the CSSContainerValues with the new overflowing state.
    void UpdateContainerOverflowing(ContainerOverflowingFlags overflowing_horizontal, ContainerOverflowingFlags overflowing_vertical);

    // Re-evaluate the cached results and clear any results which are affected by
    // the ContainerStuckPhysical changes.
    Change StickyContainerChanged(ContainerStuckPhysical stuck_horizontal, ContainerStuckPhysical stuck_vertical);
    ...
    // Re-evaluate the cached results and clear any results which are affected by
    // the snapped target changes.
    Change OverflowContainerChanged(ContainerOverflowingFlags overflowing_horizontal, ContainerOverflowingFlags overflowing_vertical);
    ...
    // evaluation result.
    bool EvalAndAdd(const ContainerQuery& query, Change change, MatchResult& match_result);
    ...
    ContainerSnappedFlags snapped_ = static_cast<ContainerSnappedFlags>(ContainerSnapped::kNone);
    ContainerSnappedFlags pending_snapped_ = static_cast<ContainerSnappedFlags>(ContainerSnapped::kNone);
    ContainerOverflowingFlags overflowing_horizontal_ = static_cast<ContainerOverflowingFlags>(ContainerOverflowing::kNone);
    ContainerOverflowingFlags overflowing_vertical_ = static_cast<ContainerOverflowingFlags>(ContainerOverflowing::kNone);
