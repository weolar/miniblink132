...
public:
    ...
    inline SelectorChecker(PartNames* part_names, const StyleRequest& style_request, const Mode& mode, const bool& is_ua_rule)
    ...
    public:
        ...
        explicit SelectorCheckingContext(Element* element)
            : element(element)
        {
        }
        explicit SelectorCheckingContext(const ElementResolveContext& element_context)
            : element(&element_context.GetUltimateOriginatingElementOrSelf())
            , pseudo_element(element_context.GetPseudoElement())
            , pseudo_element_ancestors(element_context.GetPseudoElementAncestors())
        {
        }
    ...
    public:
        void SetFlag(MatchFlag flag)
        {
            flags |= static_cast<MatchFlags>(flag);
        }
        ...
        {
            pseudo_ancestor_index = pseudo_ancestor_index == kNotFound ? 0u : pseudo_ancestor_index + 1u;
        }
        ...
        HeapVector<Member<Element>>* has_argument_leftmost_compound_matches { nullptr };
    ...
    public:
        explicit SubResult(MatchResult& parent)
            : parent_(parent)
        {
        ...
        void PropagatePseudoAncestorIndex()
        {
            ...
            if (pseudo_ancestor_index != kNotFound) {
                parent_.pseudo_ancestor_index
                    = parent_.pseudo_ancestor_index == kNotFound ? pseudo_ancestor_index : std::max(pseudo_ancestor_index, parent_.pseudo_ancestor_index);
            }
        }
    ...
    static bool MatchesFocusPseudoClass(const Element&, PseudoId matching_for_pseudo_element);
...
private:
    ...
    // methods below.
    ALWAYS_INLINE bool CheckOne(const SelectorCheckingContext&, MatchResult&) const;

    enum MatchStatus { kSelectorMatches, kSelectorFailsLocally, kSelectorFailsAllSiblings, kSelectorFailsCompletely };
    ...
    MatchStatus MatchForSubSelector(const SelectorCheckingContext&, MatchResult&) const;
    MatchStatus MatchForScopeActivation(const SelectorCheckingContext&, MatchResult&) const;
    MatchStatus MatchForRelation(const SelectorCheckingContext&, MatchResult&) const;
    MatchStatus MatchForPseudoContent(const SelectorCheckingContext&, const Element&, MatchResult&) const;
    MatchStatus MatchForPseudoShadow(const SelectorCheckingContext&, const ContainerNode*, MatchResult&) const;
    ...
    bool MatchesAnyInList(const SelectorCheckingContext& context, const CSSSelector* selector_list, MatchResult& result) const;

    const StyleScopeActivations& EnsureActivations(const SelectorCheckingContext&, const StyleScope&) const;
    const StyleScopeActivations* CalculateActivations(
        Element&, const StyleScope&, const StyleScopeActivations& outer_activations, StyleScopeFrame*, bool match_visited) const;
    bool MatchesWithScope(Element&, const CSSSelector& selector_list, const ContainerNode* scope, bool match_visited, MatchFlags&) const;
    // https://drafts.csswg.org/css-cascade-6/#scoping-limit
    bool ElementIsScopingLimit(const StyleScope&, const StyleScopeActivation&, Element& element, bool match_visited, MatchFlags&) const;
...
public:
    ...
    ALWAYS_INLINE static bool Match(const CSSSelector* selector, const Element* element);

private:
    ALWAYS_INLINE static bool MatchOne(const CSSSelector* selector, const Element* element);
    ALWAYS_INLINE static bool AttributeIsSet(const Element& element, const QualifiedName& attr);
    ALWAYS_INLINE static bool AttributeMatches(const Element& element, const QualifiedName& attr, const AtomicString& value, bool insensitive_match);
    ALWAYS_INLINE static bool AttributeItemHasName(const Attribute& attribute_item, const Element& element, const QualifiedName& name);
};
