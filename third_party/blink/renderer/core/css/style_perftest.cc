...
static WTF::String StripStyleTags(const WTF::String& html)
{
    ...
    wtf_size_t pos = 0;
    for (;;) {
        wtf_size_t style_start = html.FindIgnoringCase("<style", pos); // Allow <style id=" etc.
        ...
        if (style_start + 6 >= html.length() || (html[style_start + 6] != ' ' && html[style_start + 6] != '>')) {
...
static std::unique_ptr<DummyPageHolder> LoadDumpedPage(const base::Value::Dict& dict, base::TimeDelta& parse_time, perf_test::PerfResultReporter* reporter)
{
    const std::string parse_iterations_str = base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII("style-parse-iterations");
    int parse_iterations = parse_iterations_str.empty() ? 1 : stoi(parse_iterations_str);

    const CSSDeferPropertyParsing defer_property_parsing
        = base::CommandLine::ForCurrentProcess()->HasSwitch("style-lazy-parsing") ? CSSDeferPropertyParsing::kYes : CSSDeferPropertyParsing::kNo;

    auto page = std::make_unique<DummyPageHolder>(gfx::Size(800, 600), nullptr, MakeGarbageCollected<NoNetworkLocalFrameClient>());
    ...
    document.documentElement()->setInnerHTML(StripStyleTags(WTF::String(*dict.FindString("html"))), ASSERT_NO_EXCEPTION);
    ...
    for (const base::Value& sheet_json : *dict.FindList("stylesheets")) {
        ...
        auto* sheet = MakeGarbageCollected<StyleSheetContents>(MakeGarbageCollected<CSSParserContext>(document));
...
static StylePerfResult MeasureStyleForDumpedPage(const char* filename, bool parse_only, perf_test::PerfResultReporter* reporter)
{
    ...
    const std::string recalc_iterations_str = base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII("style-recalc-iterations");
    int recalc_iterations = recalc_iterations_str.empty() ? 1 : stoi(recalc_iterations_str);

    const bool measure_computed_style_memory = base::CommandLine::ForCurrentProcess()->HasSwitch("measure-computed-style-memory");
    ...
    size_t orig_gc_allocated_bytes = blink::ProcessHeap::TotalAllocatedObjectSize();
    size_t orig_partition_allocated_bytes = WTF::Partitions::TotalSizeOfCommittedPages();

    std::unique_ptr<DummyPageHolder> page;

    {
        std::optional<Vector<char>> serialized = test::ReadFromFile(test::StylePerfTestDataPath(filename));
        if (!serialized) {
        ...
        std::optional<base::Value> json = base::JSONReader::Read(base::as_string_view(*serialized));
        CHECK(json.has_value());
        page = LoadDumpedPage(json->GetDict(), result.parse_time, reporter);
    }

    page->GetDocument().GetStyleEngine().GetStyleResolver().SetCountComputedStyleBytes(measure_computed_style_memory);
    ...
    if (!parse_only) {
        {
            ...
            for (int i = 0; i < recalc_iterations; ++i) {
                ...
                if (i != recalc_iterations - 1) {
                    page->GetDocument().GetStyleEngine().MarkAllElementsForStyleRecalc(StyleChangeReasonForTracing::Create("test"));
                }
            }
    ...
    size_t partition_allocated_bytes = WTF::Partitions::TotalSizeOfCommittedPages();

    result.gc_allocated_bytes = gc_allocated_bytes - orig_gc_allocated_bytes;
    result.partition_allocated_bytes = partition_allocated_bytes - orig_partition_allocated_bytes;
    if (measure_computed_style_memory) {
        result.computed_style_used_bytes = page->GetDocument().GetStyleEngine().GetStyleResolver().GetComputedStyleBytesUsed();
    }

    return result;
}

static void MeasureAndPrintStyleForDumpedPage(const char* filename, const char* label)
{
    auto reporter = perf_test::PerfResultReporter("BlinkStyle", label);
    const bool parse_only = base::CommandLine::ForCurrentProcess()->HasSwitch("parse-style-only");

    StylePerfResult result = MeasureStyleForDumpedPage(filename, parse_only, &reporter);
    if (result.skipped) {
        char msg[256];
        snprintf(msg, sizeof(msg), "Skipping %s test because %s could not be read", label, filename);
        GTEST_SKIP_(msg);
    }
    ...
    if (result.computed_style_used_bytes > 0) {
        ...
        reporter.AddResult("ComputedStyleUsed", static_cast<size_t>(result.computed_style_used_bytes) / 1024);
        ...
        reporter.AddResult("GCAllocated", static_cast<size_t>(result.gc_allocated_bytes) / 1024);
    }

    reporter.RegisterImportantMetric("PartitionAllocated", "kB");
    reporter.AddResult("PartitionAllocated", static_cast<size_t>(result.partition_allocated_bytes) / 1024);
}
...
TEST(StyleCalcPerfTest, Alexa1000)
{
    ...
    for (int i = 1; i <= 1000; ++i) {
        ...
        StylePerfResult result = MeasureStyleForDumpedPage(filename, parse_only, /*reporter=*/nullptr);
        if (!result.skipped) {
            results.push_back(result);
        }
        if (i % 100 == 0) {
            LOG(INFO) << "Benchmarked " << results.size() << " pages, skipped " << (i - results.size()) << "...";
        }
    ...
    for (double percentile : { 0.5, 0.9, 0.99 }) {
        char label[256];
        size_t pos = std::min<size_t>(lrint(results.size() * percentile), results.size() - 1);

        std::nth_element(results.begin(), results.begin() + pos, results.end(),
            [](const StylePerfResult& a, const StylePerfResult& b) { return a.parse_time < b.parse_time; });
        snprintf(label, sizeof(label), "ParseTime%.0fthPercentile", percentile * 100.0);
        reporter.RegisterImportantMetric(label, "us");
        reporter.AddResult(label, results[pos].parse_time);

        if (!parse_only) {
            std::nth_element(results.begin(), results.begin() + pos, results.end(),
                [](const StylePerfResult& a, const StylePerfResult& b) { return a.initial_style_time < b.initial_style_time; });
            snprintf(label, sizeof(label), "InitialCalcTime%.0fthPercentile", percentile * 100.0);
            reporter.RegisterImportantMetric(label, "us");
            reporter.AddResult(label, results[pos].initial_style_time);

            std::nth_element(results.begin(), results.begin() + pos, results.end(),
                [](const StylePerfResult& a, const StylePerfResult& b) { return a.recalc_style_time < b.recalc_style_time; });
            snprintf(label, sizeof(label), "RecalcTime%.0fthPercentile", percentile * 100.0);
            reporter.RegisterImportantMetric(label, "us");
            reporter.AddResult(label, results[pos].recalc_style_time);
        }

        std::nth_element(results.begin(), results.begin() + pos, results.end(),
            [](const StylePerfResult& a, const StylePerfResult& b) { return a.gc_allocated_bytes < b.gc_allocated_bytes; });
        snprintf(label, sizeof(label), "GCAllocated%.0fthPercentile", percentile * 100.0);
        reporter.RegisterImportantMetric(label, "kB");
        reporter.AddResult(label, static_cast<size_t>(results[pos].gc_allocated_bytes) / 1024);

        std::nth_element(results.begin(), results.begin() + pos, results.end(),
            [](const StylePerfResult& a, const StylePerfResult& b) { return a.partition_allocated_bytes < b.partition_allocated_bytes; });
        snprintf(label, sizeof(label), "PartitionAllocated%.0fthPercentile", percentile * 100.0);
        reporter.RegisterImportantMetric(label, "kB");
        reporter.AddResult(label, static_cast<size_t>(results[pos].partition_allocated_bytes) / 1024);
    }
}
