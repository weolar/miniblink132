...
protected:
    ...
    const HeapHashMap<String, WeakMember<ImageResourceContent>>& FetchedImageMap()
    {
...
TEST_F(StyleImageCacheTest, DuplicateBackgroundImageURLs)
{
    ...
    StyleImage* rule1_image = target->ComputedStyleRef().BackgroundLayers().GetImage();
    EXPECT_TRUE(rule1_image);

    target->setAttribute(html_names::kClassAttr, AtomicString("rule2"));
    UpdateAllLifecyclePhasesForTest();

    StyleImage* rule2_image = target->ComputedStyleRef().BackgroundLayers().GetImage();
...
TEST_F(StyleImageCacheTest, CustomPropertyURL)
{
    ...
    StyleImage* initial_image = target->ComputedStyleRef().BackgroundLayers().GetImage();
    EXPECT_TRUE(initial_image);

    target->setAttribute(html_names::kClassAttr, AtomicString("green"));
    UpdateAllLifecyclePhasesForTest();

    StyleImage* image_after_recalc = target->ComputedStyleRef().BackgroundLayers().GetImage();
...
TEST_F(StyleImageCacheTest, ComputedValueRelativePath)
{
    ...
    EXPECT_EQ(*target1->ComputedStyleRef().BackgroundLayers().GetImage(), *target2->ComputedStyleRef().BackgroundLayers().GetImage());

    const CSSProperty& property = CSSProperty::Get(CSSPropertyID::kBackgroundImage);
    EXPECT_EQ(property.CSSValueFromComputedStyle(target1->ComputedStyleRef(), nullptr, false, CSSValuePhase::kComputedValue)->CssText(),
        "url(\"http://test.com/url.png\")");
    EXPECT_EQ(property.CSSValueFromComputedStyle(target2->ComputedStyleRef(), nullptr, false, CSSValuePhase::kComputedValue)->CssText(),
        "url(\"http://test.com/url.png\")");
}
...
protected:
    void SetUp() override
    {
        auto setting_overrider = [](Settings& settings) { settings.SetLoadsImagesAutomatically(true); };
        PageTestBase::SetupPageWithClients(nullptr, MakeGarbageCollected<StyleImageCacheFrameClientTest>(), setting_overrider);
    }
};
...
TEST_F(StyleImageCacheWithLoadingTest, DuplicateBackgroundImageURLs)
{
    ...
    url_test_helpers::RegisterMockedURLLoad(
        url_test_helpers::ToKURL("http://test.com/background_image.png"), test::CoreTestDataPath(kTestResourceFilename), kTestResourceMimeType);
    Element* target = GetDocument().getElementById(AtomicString("target"));
    ASSERT_TRUE(target);
    ASSERT_FALSE(target->ComputedStyleRef().BackgroundLayers().GetImage());
...
TEST_F(StyleImageCacheWithLoadingTest, LoadFailedBackgroundImageURL)
{
    ...
    const auto image_url = url_test_helpers::ToKURL("http://test.com/background_image.png");
    ...
    UpdateAllLifecyclePhasesForTest();
    url_test_helpers::ServeAsynchronousRequests();
    StyleImage* rule1_image1 = target->ComputedStyleRef().BackgroundLayers().GetImage();
    EXPECT_TRUE(rule1_image1->ErrorOccurred());
    url_test_helpers::RegisterMockedURLUnregister(image_url);
    url_test_helpers::RegisterMockedURLLoad(image_url, test::CoreTestDataPath(kTestResourceFilename), kTestResourceMimeType);
    target->setAttribute(html_names::kClassAttr, AtomicString("rule2"));
    UpdateAllLifecyclePhasesForTest();
    url_test_helpers::ServeAsynchronousRequests();
    StyleImage* rule1_image2 = target->ComputedStyleRef().BackgroundLayers().GetImage();
