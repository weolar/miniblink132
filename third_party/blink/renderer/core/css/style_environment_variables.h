...
class CORE_EXPORT StyleEnvironmentVariables : public GarbageCollected<StyleEnvironmentVariables> {
public:
    ...
    static const AtomicString GetVariableName(UADefinedVariable variable, const FeatureContext* feature_context);
    static const AtomicString GetVariableName(UADefinedTwoDimensionalVariable variable, const FeatureContext* feature_context);
    ...
    // Tokenize |value| and set it. This will invalidate any dependents.
    void SetVariable(UADefinedTwoDimensionalVariable variable, unsigned first_dimension, unsigned second_dimenison, const String& value,
    ...
    // dependents.
    void RemoveVariable(UADefinedTwoDimensionalVariable variable, const FeatureContext* feature_context);

    // Resolve the variable |name| by traversing the tree of
    // |StyleEnvironmentVariables|.
    virtual CSSVariableData* ResolveVariable(const AtomicString& name, WTF::Vector<unsigned> indices);
...
protected:
    ...
    void SetVariable(const AtomicString& name, unsigned first_dimension, unsigned second_dimension, const String& value);
...
private:
    typedef HeapVector<HeapVector<Member<CSSVariableData>>> TwoDimensionVariableValues;
