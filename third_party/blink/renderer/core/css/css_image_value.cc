...
FetchParameters CSSImageValue::PrepareFetch(
    const Document& document, FetchParameters::ImageRequestBehavior image_request_behavior, CrossOriginAttributeValue cross_origin) const
{
    const Referrer& referrer = url_data_.GetReferrer();
    ResourceRequest resource_request(url_data_.ResolveUrl(document));
    resource_request.SetReferrerPolicy(ReferrerUtils::MojoReferrerPolicyResolveDefault(referrer.referrer_policy));
    ...
    options.initiator_info.name = initiator_name_.empty() ? fetch_initiator_type_names::kCSS : initiator_name_;
    ...
    if (image_request_behavior == FetchParameters::ImageRequestBehavior::kDeferImageLoad) {
...
StyleImage* CSSImageValue::CacheImage(const Document& document, FetchParameters::ImageRequestBehavior image_request_behavior,
    CrossOriginAttributeValue cross_origin, const float override_image_resolution)
{
    if (!cached_image_) {
        ...
        FetchParameters params = PrepareFetch(document, image_request_behavior, cross_origin);
        ImageResourceContent* image_content = document.GetStyleEngine().CacheImageContent(params);
        cached_image_ = MakeGarbageCollected<StyleFetchedImage>(image_content, document,
            params.GetImageRequestBehavior() == FetchParameters::ImageRequestBehavior::kDeferImageLoad, url_data_.IsFromOriginCleanStyleSheet(),
            url_data_.IsAdRelated(), params.Url(), override_image_resolution);
    }
...
{
    if (!cached_image_ || !document.Fetcher() || url_data_.ResolvedUrl().IsNull()) {
        return;
    }
    ...
    cached_content->EmulateLoadStartedForInspector(document.Fetcher(), initiator_name_.empty() ? fetch_initiator_type_names::kCSS : initiator_name_);
}

SVGResource* CSSImageValue::EnsureSVGResource() const
{
    if (!svg_resource_) {
        svg_resource_ = MakeGarbageCollected<ExternalSVGResourceImageContent>(cached_image_->CachedImage(), NormalizedFragmentIdentifier());
    }
...
AtomicString CSSImageValue::NormalizedFragmentIdentifier() const
{
    ...
    return AtomicString(DecodeURLEscapeSequences(KURL(url_data_.ResolvedUrl()).FragmentIdentifier(), DecodeURLMode::kUTF8OrIsomorphic));
}
