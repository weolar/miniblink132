...
#define TSAN_TEST(test_case_name, test_name)                                                                                                                   \
    GTEST_TEST_(test_case_name, test_name, ::blink::MultiThreadedTest, testing::internal::GetTypeId<::blink::MultiThreadedTest>())
...
#define TSAN_TEST(test_case_name, test_name)                                                                                                                   \
    GTEST_TEST_(test_case_name, DISABLED_##test_name, ::blink::MultiThreadedTest, testing::internal::GetTypeId<::blink::MultiThreadedTest>())

#define TSAN_TEST_F(test_fixture, test_name) TEST_F(test_fixture, DISABLED_##test_name)
...
public:
    ...
    template <typename FunctionType, typename... Ps> void RunOnThreads(FunctionType function, Ps&&... parameters)
    {
        Vector<std::unique_ptr<blink::NonMainThread>> threads;
        Vector<std::unique_ptr<base::WaitableEvent>> waits;

        for (int i = 0; i < num_threads_; ++i) {
            threads.push_back(blink::NonMainThread::CreateThread(ThreadCreationParams(ThreadType::kTestThread).SetSupportsGC(true)));
            waits.push_back(std::make_unique<base::WaitableEvent>());
        }

        for (int i = 0; i < num_threads_; ++i) {
            base::SingleThreadTaskRunner* task_runner = threads[i]->GetTaskRunner().get();

            for (int j = 0; j < callbacks_per_thread_; ++j) {
                PostCrossThreadTask(*task_runner, FROM_HERE, CrossThreadBindOnce(function, parameters...));
            }

            PostCrossThreadTask(*task_runner, FROM_HERE,
                CrossThreadBindOnce([](blink::Thread* thread, base::WaitableEvent* w) { w->Signal(); }, CrossThreadUnretained(threads[i].get()),
