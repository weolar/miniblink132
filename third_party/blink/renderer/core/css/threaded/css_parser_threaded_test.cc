...
public:
    ...
    {
        const CSSValue* value = CSSParser::ParseSingleValue(prop, text, StrictCSSParserContext(SecureContextMode::kInsecureContext));
        ASSERT_TRUE(value);
        EXPECT_EQ(text, value->CssText());
    }

    static MutableCSSPropertyValueSet* TestValue(CSSPropertyID prop, const String& text)
    {
...
{
    RunOnThreads([]() {
        MutableCSSPropertyValueSet* v = TestValue(CSSPropertyID::kFont, "15px arial");
...
{
    RunOnThreads([]() {
        auto* ctx = MakeGarbageCollected<CSSParserContext>(kCSSFontFaceRuleMode, SecureContextMode::kInsecureContext);
        const CSSValue* v = CSSParser::ParseFontFaceDescriptor(CSSPropertyID::kSrc, "url(myfont.ttf)", ctx);
        ASSERT_TRUE(v);
