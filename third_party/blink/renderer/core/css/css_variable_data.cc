...
void CSSVariableData::ExtractFeatures(const CSSParserToken& token, bool& has_font_units, bool& has_root_font_units, bool& has_line_height_units)
{
...
CSSVariableData* CSSVariableData::Create(const String& original_text, bool is_animation_tainted, bool needs_variable_resolution)
{
    ...
    return Create(original_text, is_animation_tainted, needs_variable_resolution, has_font_units, has_root_font_units, has_line_height_units);
}
...
String CSSVariableData::Serialize() const
{
    ...
    if (length_ > 0 && OriginalText()[length_ - 1] == '\\') {
        ...
        return is_tainted ? RemoveAttrTaintToken(serialized_text.ReleaseString()) : serialized_text.ReleaseString();
    }

    return is_tainted ? RemoveAttrTaintToken(OriginalText()) : OriginalText().ToString();
}
...
CSSVariableData::CSSVariableData(PassKey, StringView original_text, bool is_animation_tainted, bool needs_variable_resolution, bool has_font_units,
    bool has_root_font_units, bool has_line_height_units)
...
{
    if (is_8bit_) {
        base::ranges::copy(original_text.Span8(), reinterpret_cast<LChar*>(this + 1));
    } else {
        base::ranges::copy(original_text.Span16(), reinterpret_cast<UChar*>(this + 1));
    }
}

const CSSValue* CSSVariableData::ParseForSyntax(const CSSSyntaxDefinition& syntax, SecureContextMode secure_context_mode) const
{
    DCHECK(!NeedsVariableResolution());
    // TODO(timloh): This probably needs a proper parser context for
    // relative URL resolution.
    return syntax.Parse(OriginalText(), *StrictCSSParserContext(secure_context_mode), is_animation_tainted_);
}
