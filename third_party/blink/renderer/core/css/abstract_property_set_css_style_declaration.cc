...
void AbstractPropertySetCSSStyleDeclaration::setCSSText(const ExecutionContext* execution_context, const String& text, ExceptionState&)
{
    StyleAttributeMutationScope mutation_scope(this);
    WillMutate();

    const SecureContextMode mode = execution_context ? execution_context->GetSecureContextMode() : SecureContextMode::kInsecureContext;
...
String AbstractPropertySetCSSStyleDeclaration::getPropertyValue(const String& property_name)
{
...
String AbstractPropertySetCSSStyleDeclaration::getPropertyPriority(const String& property_name)
{
    CSSPropertyID property_id = CssPropertyID(GetExecutionContext(), property_name);
    if (!IsValidCSSPropertyID(property_id)) {
        return String();
...
String AbstractPropertySetCSSStyleDeclaration::GetPropertyShorthand(const String& property_name)
{
    CSSPropertyID property_id = CssPropertyID(GetExecutionContext(), property_name);
...
bool AbstractPropertySetCSSStyleDeclaration::IsPropertyImplicit(const String& property_name)
{
    CSSPropertyID property_id = CssPropertyID(GetExecutionContext(), property_name);
...
void AbstractPropertySetCSSStyleDeclaration::setProperty(
    const ExecutionContext* execution_context, const String& property_name, const String& value, const String& priority, ExceptionState& exception_state)
{
    ...
    const SecureContextMode mode = execution_context ? execution_context->GetSecureContextMode() : SecureContextMode::kInsecureContext;
    SetPropertyInternal(property_id, property_name, value, important, mode, exception_state);
}

String AbstractPropertySetCSSStyleDeclaration::removeProperty(const String& property_name, ExceptionState& exception_state)
{
    CSSPropertyID property_id = CssPropertyID(GetExecutionContext(), property_name);
    if (!IsValidCSSPropertyID(property_id)) {
        return String();
    ...
    bool changed = false;
    if (property_id == CSSPropertyID::kVariable) {
        changed = PropertySet().RemoveProperty(AtomicString(property_name), &result);
    } else {
...
const CSSValue* AbstractPropertySetCSSStyleDeclaration::GetPropertyCSSValueInternal(CSSPropertyID property_id)
{
    return PropertySet().GetPropertyCSSValue(property_id);
}

const CSSValue* AbstractPropertySetCSSStyleDeclaration::GetPropertyCSSValueInternal(const AtomicString& custom_property_name)
{
...
String AbstractPropertySetCSSStyleDeclaration::GetPropertyValueInternal(CSSPropertyID property_id)
{
    return PropertySet().GetPropertyValue(property_id);
}

String AbstractPropertySetCSSStyleDeclaration::GetPropertyValueWithHint(const String& property_name, unsigned index)
{
    CSSPropertyID property_id = CssPropertyID(GetExecutionContext(), property_name);
    if (!IsValidCSSPropertyID(property_id)) {
        return String();
    ...
    if (property_id == CSSPropertyID::kVariable) {
        return PropertySet().GetPropertyValueWithHint(AtomicString(property_name), index);
    }
    return PropertySet().GetPropertyValue(property_id);
}

String AbstractPropertySetCSSStyleDeclaration::GetPropertyPriorityWithHint(const String& property_name, unsigned index)
{
    CSSPropertyID property_id = CssPropertyID(GetExecutionContext(), property_name);
    if (!IsValidCSSPropertyID(property_id)) {
        return String();
    ...
    bool important = false;
    if (property_id == CSSPropertyID::kVariable) {
        important = PropertySet().PropertyIsImportantWithHint(AtomicString(property_name), index);
    } else {
        important = PropertySet().PropertyIsImportant(property_id);
    }
    return important ? "important" : "";
...
DISABLE_CFI_PERF
void AbstractPropertySetCSSStyleDeclaration::SetPropertyInternal(CSSPropertyID unresolved_property, const String& custom_property_name, StringView value,
    bool important, SecureContextMode secure_context_mode, ExceptionState&)
{
    StyleAttributeMutationScope mutation_scope(this);
    WillMutate();
    ...
    if (unresolved_property == CSSPropertyID::kVariable) {
        ...
        result = PropertySet().ParseAndSetCustomProperty(atomic_name, value, important, secure_context_mode, ContextStyleSheet(), is_animation_tainted);
    } else {
        result = PropertySet().ParseAndSetProperty(unresolved_property, value, important, secure_context_mode, ContextStyleSheet());
    }
    ...
    if (result == MutableCSSPropertyValueSet::kModifiedExisting && CSSProperty::Get(property_id).SupportsIncrementalStyle()) {
...
bool AbstractPropertySetCSSStyleDeclaration::FastPathSetProperty(CSSPropertyID unresolved_property, double value)
{
    ...
    const CSSValue* css_value = CSSNumericLiteralValue::Create(value, CSSPrimitiveValue::UnitType::kNumber);
    MutableCSSPropertyValueSet::SetResult result = PropertySet().SetLonghandProperty(CSSPropertyValue(CSSPropertyName(property_id), *css_value,
        /*important=*/false));
...
bool AbstractPropertySetCSSStyleDeclaration::CssPropertyMatches(CSSPropertyID property_id, const CSSValue& property_value) const
{
