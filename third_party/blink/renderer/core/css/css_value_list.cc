...
CSSValueList::CSSValueList(ClassType class_type, ValueListSeparator list_separator)
    : CSSValue(class_type)
{
...
CSSValueList::CSSValueList(ValueListSeparator list_separator, HeapVector<Member<const CSSValue>, 4> values)
    : CSSValue(kValueListClass)
...
const CSSValue* CSSValueList::UntaintedCopy() const
{
    ...
    return MakeGarbageCollected<CSSValueList>(static_cast<ValueListSeparator>(value_list_separator_), std::move(untainted_values));
}
