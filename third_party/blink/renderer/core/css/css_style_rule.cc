...
{
    DEFINE_STATIC_LOCAL(Persistent<SelectorTextCache>, cache, (MakeGarbageCollected<SelectorTextCache>()));
    return *cache;
}

CSSStyleRule::CSSStyleRule(StyleRule* style_rule, CSSStyleSheet* parent, wtf_size_t position_hint)
    : CSSRule(parent)
    , style_rule_(style_rule)
    , style_map_(MakeGarbageCollected<DeclaredStylePropertyMap>(this))
    , position_hint_(position_hint)
    , child_rule_cssom_wrappers_(style_rule->ChildRules() ? style_rule->ChildRules()->size() : 0)
{
}
...
CSSStyleDeclaration* CSSStyleRule::style() const
{
    if (!properties_cssom_wrapper_) {
        properties_cssom_wrapper_ = MakeGarbageCollected<StyleRuleCSSStyleDeclaration>(style_rule_->MutableProperties(), const_cast<CSSStyleRule*>(this));
    }
...
void CSSStyleRule::setSelectorText(const ExecutionContext* execution_context, const String& selector_text)
{
    CSSStyleSheet::RuleMutationScope mutation_scope(this);

    const auto* context = MakeGarbageCollected<CSSParserContext>(ParserContext(execution_context->GetSecureContextMode()));
    StyleSheetContents* parent_contents = parentStyleSheet() ? parentStyleSheet()->Contents() : nullptr;
    HeapVector<CSSSelector> arena;
    StyleRule* parent_rule_for_nesting = FindClosestParentStyleRuleOrNull(parentRule());
    CSSNestingType nesting_type = parent_rule_for_nesting ? CSSNestingType::kNesting : CSSNestingType::kNone;
    base::span<CSSSelector> selector_vector
        = CSSParser::ParseSelector(context, nesting_type, parent_rule_for_nesting, /*is_within_scope=*/false, parent_contents, selector_text, arena);
    if (selector_vector.empty()) {
        return;
    }

    StyleRule* new_style_rule = StyleRule::Create(selector_vector, std::move(*style_rule_));
    if (parent_contents) {
        position_hint_ = parent_contents->ReplaceRuleIfExists(style_rule_, new_style_rule, position_hint_);
    }
...
void CSSStyleRule::Reattach(StyleRuleBase* rule)
{
    ...
    for (unsigned i = 0; i < child_rule_cssom_wrappers_.size(); ++i) {
        if (child_rule_cssom_wrappers_[i]) {
            child_rule_cssom_wrappers_[i]->Reattach((*style_rule_->ChildRules())[i].Get());
        }
    }
}
...
CSSRule* CSSStyleRule::Item(unsigned index, bool trigger_use_counters) const
{
    ...
    DCHECK_EQ(child_rule_cssom_wrappers_.size(), style_rule_->ChildRules()->size());
    Member<CSSRule>& rule = child_rule_cssom_wrappers_[index];
    if (!rule) {
        rule = (*style_rule_->ChildRules())[index]->CreateCSSOMWrapper(index, const_cast<CSSStyleRule*>(this), trigger_use_counters);
    }
    return rule.Get();
}
...
CSSRuleList* CSSStyleRule::cssRules() const
{
    if (!rule_list_cssom_wrapper_) {
        rule_list_cssom_wrapper_ = MakeGarbageCollected<LiveCSSRuleList<CSSStyleRule>>(const_cast<CSSStyleRule*>(this));
    }
    return rule_list_cssom_wrapper_.Get();
}

unsigned CSSStyleRule::insertRule(const ExecutionContext* execution_context, const String& rule_string, unsigned index, ExceptionState& exception_state)
{
    ...
    StyleRuleBase* new_rule = ParseRuleForInsert(execution_context, rule_string, index, style_rule_->ChildRules()->size(), *this, exception_state);
...
{
    if (style_rule_->ChildRules() == nullptr || index >= style_rule_->ChildRules()->size()) {
        exception_state.ThrowDOMException(
