...
public:
    ...
    const CSSScopedKeywordValue& PopulateWithTreeScope(const TreeScope* tree_scope) const;

    bool Equals(const CSSScopedKeywordValue& other) const
    {
        return IsScopedValue() == other.IsScopedValue() && tree_scope_ == other.tree_scope_ && value_id_ == other.value_id_;
    }
...
template <> struct DowncastTraits<cssvalue::CSSScopedKeywordValue> {
