...
{
    instance->SetVariable(UADefinedVariable::kSafeAreaInsetTop, kSafeAreaInsetDefault);
    instance->SetVariable(UADefinedVariable::kSafeAreaInsetLeft, kSafeAreaInsetDefault);
    instance->SetVariable(UADefinedVariable::kSafeAreaInsetBottom, kSafeAreaInsetDefault);
    instance->SetVariable(UADefinedVariable::kSafeAreaInsetRight, kSafeAreaInsetDefault);
    instance->SetVariable(UADefinedVariable::kKeyboardInsetTop, kKeyboardInsetDefault);
    instance->SetVariable(UADefinedVariable::kKeyboardInsetLeft, kKeyboardInsetDefault);
    instance->SetVariable(UADefinedVariable::kKeyboardInsetBottom, kKeyboardInsetDefault);
    instance->SetVariable(UADefinedVariable::kKeyboardInsetRight, kKeyboardInsetDefault);
    instance->SetVariable(UADefinedVariable::kKeyboardInsetWidth, kKeyboardInsetDefault);
    instance->SetVariable(UADefinedVariable::kKeyboardInsetHeight, kKeyboardInsetDefault);
}
...
{
    DEFINE_STATIC_LOCAL(Persistent<StyleEnvironmentVariables>, instance, (MakeGarbageCollected<StyleEnvironmentVariables>()));
    return *instance;
}

// static
const AtomicString StyleEnvironmentVariables::GetVariableName(UADefinedVariable variable, const FeatureContext* feature_context)
{
    switch (variable) {
...
const AtomicString StyleEnvironmentVariables::GetVariableName(UADefinedTwoDimensionalVariable variable, const FeatureContext* feature_context)
{
    switch (variable) {
...
void StyleEnvironmentVariables::SetVariable(const AtomicString& name, const String& value)
{
    data_.Set(name, CSSVariableData::Create(value, false /* is_animation_tainted */, false /* needs_variable_resolution */));
    InvalidateVariable(name);
}

void StyleEnvironmentVariables::SetVariable(const AtomicString& name, unsigned first_dimension, unsigned second_dimension, const String& value)
{
    ...
    CSSVariableData* variable_data = CSSVariableData::Create(value, false /* is_animation_tainted */, false /* needs_variable_resolution */);
    ...
    if (second_dimension_size.ValueOrDie() > (*values_to_set)[first_dimension].size()) {
...
void StyleEnvironmentVariables::SetVariable(UADefinedVariable variable, const String& value)
{
    SetVariable(GetVariableName(variable, GetFeatureContext()), value);
}

void StyleEnvironmentVariables::SetVariable(
    UADefinedTwoDimensionalVariable variable, unsigned first_dimension, unsigned second_dimension, const String& value, const FeatureContext* feature_context)
{
    SetVariable(GetVariableName(variable, feature_context), first_dimension, second_dimension, value);
}
...
void StyleEnvironmentVariables::RemoveVariable(UADefinedTwoDimensionalVariable variable, const FeatureContext* feature_context)
{
...
CSSVariableData* StyleEnvironmentVariables::ResolveVariable(const AtomicString& name, WTF::Vector<unsigned> indices)
{
    if (indices.size() == 0u) {
        ...
        if (first_dimension >= result->value.size() || second_dimension >= result->value[first_dimension].size()) {
            return nullptr;
        }
...
void StyleEnvironmentVariables::ParentInvalidatedVariable(const AtomicString& name)
{
    // If we have not overridden the variable then we should invalidate it
    // locally.
    if (!base::Contains(data_, name) && !base::Contains(two_dimension_data_, name)) {
        InvalidateVariable(name);
    }
}
