...
{
    return GetExecutionContext() && (listeners_.size() || HasEventListeners(event_type_names::kChange));
}
...
bool MediaQueryList::MediaFeaturesChanged(HeapVector<Member<MediaQueryListListener>>* listeners_to_notify)
{
    matches_dirty_ = true;
    if (!UpdateMatches()) {
        return false;
...
bool MediaQueryList::matches()
{
    ...
    if (matcher_->GetDocument() && matcher_->GetDocument()->GetFrame()) {
        if (auto* owner = matcher_->GetDocument()->GetFrame()->OwnerLayoutObject()) {
            owner->GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kJavaScript);
        }
    }
    UpdateMatches();
    return matches_;
