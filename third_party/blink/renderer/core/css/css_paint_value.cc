...
CSSPaintValue::CSSPaintValue(CSSCustomIdentValue* name, bool threaded_compositing_enabled)
    : CSSImageGeneratorValue(kPaintClass)
    , name_(name)
    , paint_image_generator_observer_(MakeGarbageCollected<Observer>(this))
    , off_thread_paint_state_((!threaded_compositing_enabled || !RuntimeEnabledFeatures::OffMainThreadCSSPaintEnabled()) ? OffThreadPaintState::kMainThread
...
CSSPaintValue::CSSPaintValue(CSSCustomIdentValue* name, HeapVector<Member<CSSVariableData>>&& variable_data)
    : CSSPaintValue(name)
{
...
const Vector<CSSPropertyID>* CSSPaintValue::NativeInvalidationProperties(const Document& document) const
{
...
const Vector<AtomicString>* CSSPaintValue::CustomInvalidationProperties(const Document& document) const
{
    auto it = generators_.find(&document);
    if (it == generators_.end()) {
        return nullptr;
...
bool CSSPaintValue::IsUsingCustomProperty(const AtomicString& custom_property_name, const Document& document) const
{
    auto it = generators_.find(&document);
    if (it == generators_.end() || !it->value->IsImageGeneratorReady()) {
        return false;
    }
    return it->value->CustomInvalidationProperties().Contains(custom_property_name);
}

CSSPaintImageGenerator& CSSPaintValue::EnsureGenerator(const Document& document)
{
    auto& generator = generators_.insert(&document, nullptr).stored_value->value;
    if (!generator) {
        generator = CSSPaintImageGenerator::Create(GetName(), document, paint_image_generator_observer_);
    }
    return *generator;
}

scoped_refptr<Image> CSSPaintValue::GetImage(
    const ImageResourceObserver& client, const Document& document, const ComputedStyle& style, const gfx::SizeF& target_size)
{
    ...
    if (off_thread_paint_state_ != OffThreadPaintState::kMainThread && !document.Printing()) {
        ...
        const Vector<CSSPropertyID>& native_properties = generator.NativeInvalidationProperties();
        const Vector<AtomicString>& custom_properties = generator.CustomInvalidationProperties();
        float zoom = layout_object.StyleRef().EffectiveZoom();
        CompositorPaintWorkletInput::PropertyKeys input_property_keys;
        auto style_data = PaintWorkletStylePropertyMap::BuildCrossThreadData(
            document, layout_object.UniqueId(), style, native_properties, custom_properties, input_property_keys);
        off_thread_paint_state_ = style_data.has_value() ? OffThreadPaintState::kOffThread : OffThreadPaintState::kMainThread;
        if (off_thread_paint_state_ == OffThreadPaintState::kOffThread) {
            ...
            scoped_refptr<CSSPaintWorkletInput> input = base::MakeRefCounted<CSSPaintWorkletInput>(GetName(), target_size, zoom, generator.WorkletId(),
                std::move(style_data.value()), std::move(cross_thread_input_arguments), std::move(input_property_keys));
...
void CSSPaintValue::BuildInputArgumentValues(Vector<std::unique_ptr<CrossThreadStyleValue>>& cross_thread_input_arguments)
{
    if (!parsed_input_arguments_) {
        return;
    }
    for (const auto& style_value : *parsed_input_arguments_) {
        std::unique_ptr<CrossThreadStyleValue> cross_thread_style = ComputedStyleUtils::CrossThreadStyleValueFromCSSStyleValue(style_value);
...
bool CSSPaintValue::ParseInputArguments(const Document& document)
{
    ...
    if (parsed_input_arguments_ || !RuntimeEnabledFeatures::CSSPaintAPIArgumentsEnabled()) {
        return true;
    }
    ...
    const Vector<CSSSyntaxDefinition>& input_argument_types = it->value->InputArgumentTypes();
    ...
    for (wtf_size_t i = 0; i < argument_variable_data_.size(); ++i) {
        ...
        DCHECK_EQ(SecureContextMode::kSecureContext, document.GetExecutionContext()->GetSecureContextMode());
        DCHECK(!argument_variable_data_[i]->NeedsVariableResolution());
        const CSSValue* parsed_value = argument_variable_data_[i]->ParseForSyntax(input_argument_types[i], SecureContextMode::kSecureContext);
        if (!parsed_value) {
            input_arguments_invalid_ = true;
            parsed_input_arguments_ = nullptr;
            return false;
        }
        parsed_input_arguments_->AppendVector(StyleValueFactory::CssValueToStyleValueVector(*parsed_value));
    }
    return true;
}
...
void CSSPaintValue::PaintImageGeneratorReady()
{
    for (const ImageResourceObserver* client : Clients().Keys()) {
        ...
        const_cast<ImageResourceObserver*>(client)->ImageChanged(static_cast<WrappedImagePtr>(this), ImageResourceObserver::CanDeferInvalidation::kNo);
    }
}

bool CSSPaintValue::KnownToBeOpaque(const Document& document, const ComputedStyle&) const
{
    auto it = generators_.find(&document);
...
{
    return GetName() == other.GetName() && CustomCSSText() == other.CustomCSSText();
}
