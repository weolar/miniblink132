    ...
    if (!pending_task_queued_) {
        if (auto* context = GetExecutionContext()) {
            ...
            context->GetTaskRunner(TaskType::kFontLoading)
                ->PostTask(FROM_HERE, WTF::BindOnce(&FontFaceSet::HandlePendingEventsAndPromises, WrapPersistent(this)));
        }
    }
}
...
void FontFaceSet::FireLoadingEvent()
{
    if (should_fire_loading_event_) {
        ...
        DispatchEvent(*FontFaceSetLoadEvent::CreateForFontFaces(event_type_names::kLoading));
    }
}

V8FontFaceSetLoadStatus FontFaceSet::status() const
{
    return V8FontFaceSetLoadStatus(is_loading_ ? V8FontFaceSetLoadStatus::Enum::kLoading : V8FontFaceSetLoadStatus::Enum::kLoaded);
}

FontFaceSet* FontFaceSet::addForBinding(ScriptState*, FontFace* font_face, ExceptionState&)
{
    DCHECK(font_face);
    if (!InActiveContext()) {
        return this;
    ...
    font_selector->FontFaceInvalidated(FontInvalidationReason::kGeneralInvalidation);
    return this;
}
...
bool FontFaceSet::deleteForBinding(ScriptState*, FontFace* font_face, ExceptionState&)
{
    DCHECK(font_face);
    if (!InActiveContext()) {
        return false;
    }
    HeapLinkedHashSet<Member<FontFace>>::iterator it = non_css_connected_faces_.find(font_face);
    if (it != non_css_connected_faces_.end()) {
        ...
        font_selector->FontFaceInvalidated(FontInvalidationReason::kFontFaceDeleted);
        return true;
    }
    return false;
}

bool FontFaceSet::hasForBinding(ScriptState*, FontFace* font_face, ExceptionState&) const
{
    DCHECK(font_face);
    if (!InActiveContext()) {
        return false;
    }
    return non_css_connected_faces_.Contains(font_face) || IsCSSConnectedFontFace(font_face);
}
...
ScriptPromise<IDLSequence<FontFace>> FontFaceSet::load(ScriptState* script_state, const String& font_string, const String& text)
{
    if (!InActiveContext()) {
    ...
    if (!ResolveFontStyle(font_string, font)) {
        return ScriptPromise<IDLSequence<FontFace>>::RejectWithDOMException(
            script_state, MakeGarbageCollected<DOMException>(DOMExceptionCode::kSyntaxError, "Could not resolve '" + font_string + "' as a font."));
    }
    ...
    for (const FontFamily* f = &font.GetFontDescription().Family(); f; f = f->Next()) {
        ...
        CSSSegmentedFontFace* segmented_font_face = font_face_cache->Get(font.GetFontDescription(), f->FamilyName());
        if (segmented_font_face) {
            segmented_font_face->Match(text, faces);
        }
    }

    auto* resolver = MakeGarbageCollected<LoadFontPromiseResolver>(faces, script_state);
...
bool FontFaceSet::check(const String& font_string, const String& text, ExceptionState& exception_state)
{
    if (!InActiveContext()) {
        return false;
    }
    ...
    Font font;
    if (!ResolveFontStyle(font_string, font)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "Could not resolve '" + font_string + "' as a font.");
        return false;
    }
    ...
    while (index < text.length()) {
        ...
        for (const FontFamily* f = &font.GetFontDescription().Family(); f; f = f->Next()) {
            if (f->FamilyIsGeneric() || font_selector->IsPlatformFamilyMatchAvailable(font.GetFontDescription(), *f)) {
                continue;
            }

            CSSSegmentedFontFace* face = font_face_cache->Get(font.GetFontDescription(), f->FamilyName());
...
void FontFaceSet::FireDoneEvent()
{
    if (is_loading_) {
        ...
        done_event = FontFaceSetLoadEvent::CreateForFontFaces(event_type_names::kLoadingdone, loaded_fonts_);
        loaded_fonts_.clear();
        if (!failed_fonts_.empty()) {
            error_event = FontFaceSetLoadEvent::CreateForFontFaces(event_type_names::kLoadingerror, failed_fonts_);
            failed_fonts_.clear();
        }
        is_loading_ = false;
...
bool FontFaceSet::IterationSource::FetchNextItem(ScriptState*, FontFace*& value, ExceptionState&)
{
...
FontFaceSetIterable::IterationSource* FontFaceSet::CreateIterationSource(ScriptState*, ExceptionState&)
{
    ...
    if (InActiveContext()) {
        const HeapLinkedHashSet<Member<FontFace>>& css_connected_faces = CSSConnectedFontFaceList();
        font_faces.ReserveInitialCapacity(css_connected_faces.size() + non_css_connected_faces_.size());
