...
StyleSheetContents::StyleSheetContents(const CSSParserContext* context, const String& original_url, StyleRuleImport* owner_rule)
...
StyleSheetContents::StyleSheetContents(const StyleSheetContents& o)
    ...
    , pre_import_layer_statement_rules_(o.pre_import_layer_statement_rules_.size())
    ...
    , parser_context_(o.parser_context_)
{
    for (unsigned i = 0; i < pre_import_layer_statement_rules_.size(); ++i) {
        pre_import_layer_statement_rules_[i] = To<StyleRuleLayerStatement>(o.pre_import_layer_statement_rules_[i]->Copy());
    }
    ...
    for (unsigned i = 0; i < namespace_rules_.size(); ++i) {
        namespace_rules_[i] = static_cast<StyleRuleNamespace*>(o.namespace_rules_[i]->Copy());
    }
...
void StyleSheetContents::ParserAppendRule(StyleRuleBase* rule)
{
    if (auto* layer_statement_rule = DynamicTo<StyleRuleLayerStatement>(rule)) {
        if (import_rules_.empty() && namespace_rules_.empty() && child_rules_.empty()) {
...
{
    return pre_import_layer_statement_rules_.size() + import_rules_.size() + namespace_rules_.size() + child_rules_.size();
}
...
static wtf_size_t ReplaceRuleIfExistsInternal(const StyleRuleBase* old_rule, StyleRuleBase* new_rule, HeapVector<Member<StyleRuleBase>>& child_rules)
{
    for (wtf_size_t i = 0; i < child_rules.size(); ++i) {
        ...
        if (auto* style_rule_group = DynamicTo<StyleRuleGroup>(rule)) {
            if (ReplaceRuleIfExistsInternal(old_rule, new_rule, style_rule_group->ChildRules()) != std::numeric_limits<wtf_size_t>::max()) {
                return 0; // Dummy non-failure value.
            }
        } else if (auto* style_rule = DynamicTo<StyleRule>(rule); style_rule && style_rule->ChildRules()) {
            if (ReplaceRuleIfExistsInternal(old_rule, new_rule, *style_rule->ChildRules()) != std::numeric_limits<wtf_size_t>::max()) {
                return 0; // Dummy non-failure value.
            }
        }
    }
...
wtf_size_t StyleSheetContents::ReplaceRuleIfExists(StyleRuleBase* old_rule, StyleRuleBase* new_rule, wtf_size_t position_hint)
{
    if (rule_set_diff_) {
    ...
    if (position_hint < child_rules_.size() && child_rules_[position_hint] == old_rule) {
...
bool StyleSheetContents::WrapperInsertRule(StyleRuleBase* rule, unsigned index)
{
    ...
    if (pre_import_layer_statement_rules_.size() && !import_rules_.size() && !namespace_rules_.size()) {
        child_rules_.PrependVector(pre_import_layer_statement_rules_);
        pre_import_layer_statement_rules_.clear();
    }

    if (index < pre_import_layer_statement_rules_.size() || (index == pre_import_layer_statement_rules_.size() && rule->IsLayerStatementRule())) {
    ...
    if (index < import_rules_.size() || (index == import_rules_.size() && rule->IsImportRule())) {
    ...
    if (index < namespace_rules_.size() || (index == namespace_rules_.size() && rule->IsNamespaceRule())) {
...
void StyleSheetContents::ParseAuthorStyleSheet(const CSSStyleSheetResource* cached_style_sheet)
{
    TRACE_EVENT1("blink,devtools.timeline", "ParseAuthorStyleSheet", "data",
        [&](perfetto::TracedValue context) { inspector_parse_author_style_sheet_event::Data(std::move(context), cached_style_sheet); });

    const ResourceResponse& response = cached_style_sheet->GetResponse();
    CSSStyleSheetResource::MIMETypeCheck mime_type_check = (IsQuirksModeBehavior(parser_context_->Mode()) && response.IsCorsSameOrigin())
        ? CSSStyleSheetResource::MIMETypeCheck::kLax
        : CSSStyleSheetResource::MIMETypeCheck::kStrict;
    String sheet_text = cached_style_sheet->SheetText(parser_context_, mime_type_check);
    ...
    const auto* context = MakeGarbageCollected<CSSParserContext>(ParserContext(), this);
    CSSParser::ParseSheet(context, this, sheet_text, CSSDeferPropertyParsing::kYes);
}

ParseSheetResult StyleSheetContents::ParseString(const String& sheet_text, bool allow_import_rules, CSSDeferPropertyParsing defer_property_parsing)
{
    const auto* context = MakeGarbageCollected<CSSParserContext>(ParserContext(), this);
    return CSSParser::ParseSheet(context, this, sheet_text, defer_property_parsing, allow_import_rules);
}
        ...
        if (Node* owner_node = loading_clients[i]->ownerNode()) {
            if (loading_clients[i]->SheetLoaded()) {
                owner_node->NotifyLoadedSheetAndAllCriticalSubresources(
                    did_load_error_occur_ ? Node::kErrorOccurredLoadingSubresource : Node::kNoErrorLoadingSubresource);
            }
        }
    }
}
...
static bool ChildRulesHaveFailedOrCanceledSubresources(const HeapVector<Member<StyleRuleBase>>& rules)
{
    for (unsigned i = 0; i < rules.size(); ++i) {
        ...
        case StyleRuleBase::kFontFace:
            if (To<StyleRuleFontFace>(rule)->Properties().HasFailedOrCanceledSubresources()) {
                return true;
            }
            break;
        ...
        case StyleRuleBase::kStartingStyle:
            if (ChildRulesHaveFailedOrCanceledSubresources(To<StyleRuleGroup>(rule)->ChildRules())) {
                return true;
            }
            break;
...
void StyleSheetContents::UnregisterClient(CSSStyleSheet* sheet)
{
    ...
    if (!sheet->OwnerDocument() || !loading_clients_.empty() || !completed_clients_.empty()) {
        return;
    }
...
static void RemoveFontFaceRules(HeapHashSet<WeakMember<CSSStyleSheet>>& clients, const StyleRuleFontFace* font_face_rule)
{
    for (const auto& sheet : clients) {
        if (Node* owner_node = sheet->ownerNode()) {
            owner_node->GetDocument().GetStyleEngine().RemoveFontFaceRules(HeapVector<Member<const StyleRuleFontFace>>(1, font_face_rule));
        }
    }
}

void StyleSheetContents::NotifyRemoveFontFaceRule(const StyleRuleFontFace* font_face_rule)
{
    StyleSheetContents* root = RootStyleSheet();
