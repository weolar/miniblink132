...
static bool IsAcceptableCSSStyleSheetParent(const Node& parent_node)
{
    ...
    return parent_node.IsDocumentNode() || IsA<HTMLLinkElement>(parent_node) || IsA<HTMLStyleElement>(parent_node) || IsA<SVGStyleElement>(parent_node)
...
CSSStyleSheet* CSSStyleSheet::Create(Document& document, const CSSStyleSheetInit* options, ExceptionState& exception_state)
{
    return CSSStyleSheet::Create(document, document.BaseURL(), options, exception_state);
}

CSSStyleSheet* CSSStyleSheet::Create(Document& document, const KURL& base_url, const CSSStyleSheetInit* options, ExceptionState& exception_state)
{
    auto* parser_context = MakeGarbageCollected<CSSParserContext>(document, base_url);
...
CSSStyleSheet* CSSStyleSheet::CreateInline(StyleSheetContents* sheet, Node& owner_node, const TextPosition& start_position)
{
    DCHECK(sheet);
    return MakeGarbageCollected<CSSStyleSheet>(sheet, owner_node, true, start_position);
}

CSSStyleSheet* CSSStyleSheet::CreateInline(Node& owner_node, const KURL& base_url, const TextPosition& start_position, const WTF::TextEncoding& encoding)
{
    Document& owner_node_document = owner_node.GetDocument();
    auto* parser_context = MakeGarbageCollected<CSSParserContext>(owner_node_document, owner_node_document.BaseURL(), true /* origin_clean */,
        Referrer(
            ...
            // when fetching.
            Referrer::ClientReferrerString(), network::mojom::ReferrerPolicy::kDefault),
        encoding);
    if (AdTracker::IsAdScriptExecutingInDocument(&owner_node.GetDocument())) {
        parser_context->SetIsAdRelated();
    }
    auto* sheet = MakeGarbageCollected<StyleSheetContents>(parser_context, base_url.GetString());
    return MakeGarbageCollected<CSSStyleSheet>(sheet, owner_node, true, start_position);
}

CSSStyleSheet::CSSStyleSheet(StyleSheetContents* contents, CSSImportRule* owner_rule)
    : contents_(contents)
...
CSSStyleSheet::CSSStyleSheet(StyleSheetContents* contents, Document& document, const CSSStyleSheetInit* options)
    : CSSStyleSheet(contents, nullptr)
{
    ...
    case V8UnionMediaListOrString::ContentType::kString:
        media_queries_ = MediaQuerySet::Create(options->media()->GetAsString(), document.GetExecutionContext());
        break;
    }
...
CSSStyleSheet::CSSStyleSheet(StyleSheetContents* contents, Node& owner_node, bool is_inline_stylesheet, const TextPosition& start_position)
    : contents_(contents)
...
void CSSStyleSheet::WillMutateRules()
{
    ...
    if (!contents_->IsUsedFromTextCache() && !contents_->IsReferencedFromResource()) {
...
CSSStyleSheet::InspectorMutationScope::InspectorMutationScope(CSSStyleSheet* sheet)
    : style_sheet_(sheet)
{
...
unsigned CSSStyleSheet::insertRule(const String& rule_string, unsigned index, ExceptionState& exception_state)
{
    if (!CanAccessRules()) {
        exception_state.ThrowSecurityError("Cannot access StyleSheet to insertRule");
        return 0;
    }
    ...
    if (index > length()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kIndexSizeError,
            "The index provided (" + String::Number(index) + ") is larger than the maximum index (" + String::Number(length()) + ").");
        return 0;
    }

    const auto* context = MakeGarbageCollected<CSSParserContext>(contents_->ParserContext(), this);

    StyleRuleBase* rule = CSSParser::ParseRule(context, contents_.Get(), CSSNestingType::kNone,
        /*parent_rule_for_nesting=*/nullptr,
        /*is_within_scope=*/false, rule_string);

    if (!rule) {
        exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "Failed to parse the rule '" + rule_string + "'.");
        return 0;
    }
    RuleMutationScope mutation_scope(this);
    if (rule->IsImportRule() && IsConstructed()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "Can't insert @import rules into a constructed stylesheet.");
        return 0;
    }
    bool success = contents_->WrapperInsertRule(rule, index);
    if (!success) {
        if (rule->IsNamespaceRule()) {
            exception_state.ThrowDOMException(DOMExceptionCode::kInvalidStateError, "Failed to insert the rule");
        } else {
            exception_state.ThrowDOMException(DOMExceptionCode::kHierarchyRequestError, "Failed to insert the rule.");
        }
        return 0;
    }
    if (!child_rule_cssom_wrappers_.empty()) {
...
void CSSStyleSheet::deleteRule(unsigned index, ExceptionState& exception_state)
{
    ...
    if (index >= length()) {
        if (length()) {
            exception_state.ThrowDOMException(DOMExceptionCode::kIndexSizeError,
                "The index provided (" + String::Number(index) + ") is larger than the maximum index (" + String::Number(length() - 1) + ").");
        } else {
            exception_state.ThrowDOMException(DOMExceptionCode::kIndexSizeError, "Style sheet is empty (length 0).");
        }
        return;
    }
...
int CSSStyleSheet::addRule(const String& selector, const String& style, int index, ExceptionState& exception_state)
{
    StringBuilder text;
...
int CSSStyleSheet::addRule(const String& selector, const String& style, ExceptionState& exception_state)
{
    return addRule(selector, style, length(), exception_state);
}

ScriptPromise<CSSStyleSheet> CSSStyleSheet::replace(ScriptState* script_state, const String& text, ExceptionState& exception_state)
{
    if (!IsConstructed()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kNotAllowedError, "Can't call replace on non-constructed CSSStyleSheets.");
        return EmptyPromise();
    }
...
void CSSStyleSheet::replaceSync(const String& text, ExceptionState& exception_state)
{
    if (!IsConstructed()) {
        return exception_state.ThrowDOMException(DOMExceptionCode::kNotAllowedError, "Can't call replaceSync on non-constructed CSSStyleSheets.");
    }
    SetText(text, CSSImportRules::kIgnoreWithWarning);
    probe::DidReplaceStyleSheetText(OwnerDocument(), this, text);
}
...
void CSSStyleSheet::SetText(const String& text, CSSImportRules import_rules)
{
    ...
    if (contents_->ParseString(text, allow_imports) == ParseSheetResult::kHasUnallowedImportRule && import_rules == CSSImportRules::kIgnoreWithWarning) {
        OwnerDocument()->AddConsoleMessage(
...
bool CSSStyleSheet::CanBeActivated(const String& current_preferrable_name) const
{
    if (disabled()) {
        return false;
    }
    ...
    if (!owner_node_ || owner_node_->getNodeType() == Node::kProcessingInstructionNode || !html_link_element || !html_link_element->IsEnabledViaScript()) {
