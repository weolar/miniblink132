...
bool IsAttrTainted(const CSSParserTokenStream& stream, wtf_size_t start_offset, wtf_size_t end_offset)
{
    return IsAttrTainted(stream.StringRangeAt(start_offset, end_offset - start_offset));
}

bool IsAttrTainted(StringView str)
{
    if (str.Is8Bit() && memchr(str.Characters8(), '\0', str.length()) == nullptr) {
...
String RemoveAttrTaintToken(StringView str)
{
    ...
    while (true) {
        ...
        StringView token_str = tokenizer.StringRangeAt(prev_offset, offset - prev_offset);
