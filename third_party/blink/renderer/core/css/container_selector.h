...
public:
    ...
    ContainerSelector(AtomicString name, PhysicalAxes physical_axes, LogicalAxes logical_axes, bool scroll_state)
        : name_(std::move(name))
        , physical_axes_(physical_axes)
    ...
    {
        return (name_ == o.name_) && (physical_axes_ == o.physical_axes_) && (logical_axes_ == o.logical_axes_) && (has_style_query_ == o.has_style_query_)
            && (has_sticky_query_ == o.has_sticky_query_) && (has_snap_query_ == o.has_snap_query_) && (has_overflow_query_ == o.has_overflow_query_);
    }
    bool operator!=(const ContainerSelector& o) const
    {
        return !(*this == o);
    ...
    {
        return physical_axes_ != kPhysicalAxesNone || logical_axes_ != kLogicalAxesNone;
    }

    bool SelectsStyleContainers() const
    {
        return has_style_query_;
    }
    bool SelectsStickyContainers() const
    {
        return has_sticky_query_;
    }
    bool SelectsSnapContainers() const
    {
        return has_snap_query_;
    }
    bool SelectsOverflowContainers() const
    {
        return has_overflow_query_;
    }
    bool SelectsScrollStateContainers() const
    {
        return SelectsStickyContainers() || SelectsSnapContainers() || SelectsOverflowContainers();
    }
    bool HasUnknownFeature() const
    {
        return has_unknown_feature_;
    }

    PhysicalAxes GetPhysicalAxes() const
    {
        return physical_axes_;
    }
    LogicalAxes GetLogicalAxes() const
    {
        return logical_axes_;
    }
...
class CORE_EXPORT ScopedContainerSelector : public GarbageCollected<ScopedContainerSelector> {
public:
    ScopedContainerSelector(ContainerSelector selector, const TreeScope* tree_scope)
        : selector_(selector)
...
struct ScopedContainerSelectorHashTraits : WTF::MemberHashTraits<ScopedContainerSelector> {
    static unsigned GetHash(const Member<ScopedContainerSelector>& scoped_selector)
    {
        return scoped_selector->GetHash();
    }
    static bool Equal(const Member<ScopedContainerSelector>& a, const Member<ScopedContainerSelector>& b)
    {
        return *a == *b;
    }
...
struct ScopedContainerSelectorHashTranslator {
    ...
    static bool Equal(const Member<ScopedContainerSelector>& a, const ScopedContainerSelector& b)
    {
        return a && *a == b;
    }
};
...
template <> struct HashTraits<blink::ContainerSelector> : SimpleClassHashTraits<blink::ContainerSelector> {
...
using ContainerSelectorCache = HeapHashMap<Member<ScopedContainerSelector>, Member<Element>, ScopedContainerSelectorHashTraits>;
