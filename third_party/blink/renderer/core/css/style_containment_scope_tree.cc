...
StyleContainmentScope* StyleContainmentScopeTree::FindOrCreateEnclosingScopeForElement(const Element& element)
{
    ...
    for (const Element* it = LayoutTreeBuilderTraversal::ParentElement(element); it; it = LayoutTreeBuilderTraversal::ParentElement(*it)) {
...
StyleContainmentScope* StyleContainmentScopeTree::CreateScopeForElement(const Element& element)
{
    auto entry = scopes_.find(&element);
    if (entry != scopes_.end()) {
        return entry->value.Get();
    }
    StyleContainmentScope* scope = MakeGarbageCollected<StyleContainmentScope>(&element, this);
    ...
    for (StyleContainmentScope* child : children) {
        if (child != scope && scope->IsAncestorOf(child->GetElement(), parent->GetElement())) {
...
StyleContainmentScope* FindCommonAncestor(StyleContainmentScope* scope1, StyleContainmentScope* scope2)
{
    if (!scope1) {
        return scope2;
    }
...
void StyleContainmentScopeTree::UpdateOutermostQuotesDirtyScope(StyleContainmentScope* scope)
{
    outermost_quotes_dirty_scope_ = FindCommonAncestor(scope, outermost_quotes_dirty_scope_);
}
...
#if DCHECK_IS_ON()
String StyleContainmentScopeTree::ToString(StyleContainmentScope* style_scope, wtf_size_t depth) const
{
    ...
    if (style_scope->GetElement()) {
        builder.AppendFormat("SCOPE: %s; ", style_scope->GetElement()->DebugName().Ascii().c_str());
        builder.AppendFormat("PARENT: %s", style_scope->Parent()->GetElement() ? style_scope->Parent()->GetElement()->DebugName().Ascii().c_str() : "root");
    } else {
    ...
    for (StyleContainmentScope* child : style_scope->Children()) {
        builder.AppendFormat("CHILD %s; ", child->GetElement()->DebugName().Ascii().c_str());
    }
    builder.Append("\n");
    for (StyleContainmentScope* child : style_scope->Children()) {
