...
CSSValueID CssIdentifierForFontSizeKeyword(int keyword_size)
{
    ...
    return static_cast<CSSValueID>(static_cast<int>(CSSValueID::kXxSmall) + keyword_size - 1);
}

void LogUnimplementedPropertyID(const CSSProperty& property)
{
    if (!DCHECK_IS_ON() || !VLOG_IS_ON(1) || property.PropertyID() == CSSPropertyID::kVariable) {
        return;
    }
    ...
    DVLOG(1) << "Blink does not yet implement getComputedStyle for '" << property.GetPropertyName() << "'.";
}
...
void UseCountIfAnimationDurationZero(Document& document, const ComputedStyle& style, mojom::blink::WebFeature feature)
{
...
const Vector<const CSSProperty*>& CSSComputedStyleDeclaration::ComputableProperties(const ExecutionContext* execution_context)
{
    DEFINE_STATIC_LOCAL(Vector<const CSSProperty*>, properties, ());
    if (properties.empty()) {
        CSSProperty::FilterWebExposedCSSPropertiesIntoVector(execution_context, kCSSComputableProperties, std::size(kCSSComputableProperties), properties);
    }
    return properties;
}

CSSComputedStyleDeclaration::CSSComputedStyleDeclaration(Element* element, bool allow_visited_style, const String& pseudo_element_name)
...
{
    pseudo_element_specifier_ = CSSSelectorParser::ParsePseudoElement(pseudo_element_name, element, pseudo_argument_);
}
...
void CSSComputedStyleDeclaration::setCSSText(const ExecutionContext*, const String&, ExceptionState& exception_state)
{
    exception_state.ThrowDOMException(DOMExceptionCode::kNoModificationAllowedError, "These styles are computed, and therefore read-only.");
}
...
const CSSValue* CSSComputedStyleDeclaration::GetFontSizeCSSValuePreferringKeyword() const
{
    ...
    const ComputedStyle* style = element_->EnsureComputedStyle(pseudo_element_specifier_);
    if (!style) {
        return nullptr;
    }

    if (int keyword_size = style->GetFontDescription().KeywordSize()) {
        return CSSIdentifierValue::Create(CssIdentifierForFontSizeKeyword(keyword_size));
    }
...
const ComputedStyle* CSSComputedStyleDeclaration::ComputeComputedStyle() const
{
    ...
    const ComputedStyle* style
        = styled_element->EnsureComputedStyle((styled_element == element_) ? pseudo_element_specifier_ : kPseudoIdNone, pseudo_argument_);
    if (style && style->IsEnsuredOutsideFlatTree()) {
        UseCounter::Count(element_->GetDocument(), WebFeature::kGetComputedStyleOutsideFlatTree);
    }
    return style;
}

const Vector<AtomicString>* CSSComputedStyleDeclaration::GetVariableNames() const
{
...
Element* CSSComputedStyleDeclaration::StyledElement() const
{
    ...
    if (pseudo_element_specifier_ == kPseudoIdInvalid) {
        CHECK(RuntimeEnabledFeatures::CSSComputedStyleFullPseudoElementParserEnabled());
        return nullptr;
    }

    if (Element* pseudo_element = element_->GetStyledPseudoElement(pseudo_element_specifier_, pseudo_argument_)) {
...
const CSSValue* CSSComputedStyleDeclaration::GetPropertyCSSValue(CSSPropertyID property_id) const
{
...
const CSSValue* CSSComputedStyleDeclaration::GetPropertyCSSValue(const AtomicString& custom_property_name) const
{
    return GetPropertyCSSValue(CSSPropertyName(custom_property_name));
}

HeapHashMap<AtomicString, Member<const CSSValue>> CSSComputedStyleDeclaration::GetVariables() const
{
    ...
    return ComputedStyleCSSValueMapping::GetVariables(*style, StyledElement()->GetDocument().GetPropertyRegistry(), CSSValuePhase::kResolvedValue);
}

void CSSComputedStyleDeclaration::UpdateStyleAndLayoutTreeIfNeeded(const CSSPropertyName* property_name, bool for_all_properties) const
{
    ...
    if (HTMLFrameOwnerElement* owner = document.LocalOwner()) {
        ...
        bool is_for_layout_dependent_property
            = for_all_properties || (property_name && !property_name->IsCustomProperty() && CSSProperty::Get(property_name->Id()).IsLayoutDependentProperty());
        if (is_for_layout_dependent_property) {
            owner->GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kComputedStyle);
    ...
    if (IsTransitionPseudoElement(styled_element->GetPseudoId())) {
        if (auto* view = document.View()) {
            view->UpdateAllLifecyclePhasesExceptPaint(DocumentUpdateReason::kComputedStyle);
        }
        return;
    }

    document.UpdateStyleAndLayoutTreeForElement(styled_element, DocumentUpdateReason::kComputedStyle);
}

void CSSComputedStyleDeclaration::UpdateStyleAndLayoutIfNeeded(const CSSProperty* property, bool for_all_properties) const
{
    if (guaranteed_style_clean_) {
        return;
    }
    ...
    bool is_for_layout_dependent_property
        = for_all_properties || (property && property->IsLayoutDependent(styled_element->GetComputedStyle(), StyledLayoutObject()));

    if (is_for_layout_dependent_property) {
        auto& doc = styled_element->GetDocument();
        doc.UpdateStyleAndLayoutForNode(styled_element, DocumentUpdateReason::kJavaScript);
    }
}

const CSSValue* CSSComputedStyleDeclaration::GetPropertyCSSValue(const CSSPropertyName& property_name) const
{
    Element* styled_element = StyledElement();
    if (!styled_element) {
        return nullptr;
    ...
    if (property_class.PropertyID() == CSSPropertyID::kAnimationDuration) {
        UseCountIfAnimationDurationZero(styled_element->GetDocument(), *style, WebFeature::kCSSGetComputedAnimationDurationZero);
    }
    ...
    if (property_class.PropertyID() == CSSPropertyID::kWebkitFontSmoothing) {
        UseCountIfAnimationDurationZero(styled_element->GetDocument(), *style, WebFeature::kCSSGetComputedWebkitFontSmoothingAnimationDurationZero);
    }

    const CSSValue* value = property_class.CSSValueFromComputedStyle(*style, StyledLayoutObject(), allow_visited_style_, CSSValuePhase::kResolvedValue);
    if (value) {
        return value;
    }
...
String CSSComputedStyleDeclaration::GetPropertyValue(CSSPropertyID property_id) const
{
...
{
    if (!element_ || !element_->InActiveDocument() || (pseudo_element_specifier_ == kPseudoIdInvalid)) {
        return 0;
    }
...
bool CSSComputedStyleDeclaration::CssPropertyMatches(CSSPropertyID property_id, const CSSValue& property_value) const
{
    if (property_id == CSSPropertyID::kFontSize && (property_value.IsPrimitiveValue() || property_value.IsIdentifierValue()) && element_) {
        // This is only used by editing code.
        element_->GetDocument().UpdateStyleAndLayout(DocumentUpdateReason::kEditing);
        const ComputedStyle* style = element_->EnsureComputedStyle(pseudo_element_specifier_);
        if (style && style->GetFontDescription().KeywordSize()) {
            CSSValueID size_value = CssIdentifierForFontSizeKeyword(style->GetFontDescription().KeywordSize());
...
MutableCSSPropertyValueSet* CSSComputedStyleDeclaration::CopyProperties() const
{
    return CopyPropertiesInSet(ComputableProperties(GetExecutionContext()));
}

MutableCSSPropertyValueSet* CSSComputedStyleDeclaration::CopyPropertiesInSet(const Vector<const CSSProperty*>& properties) const
{
...
String CSSComputedStyleDeclaration::getPropertyValue(const String& property_name)
{
    CSSPropertyID property_id = CssPropertyID(GetExecutionContext(), property_name);
...
void CSSComputedStyleDeclaration::setProperty(const ExecutionContext*, const String& name, const String&, const String&, ExceptionState& exception_state)
{
    exception_state.ThrowDOMException(
        DOMExceptionCode::kNoModificationAllowedError, "These styles are computed, and therefore the '" + name + "' property is read-only.");
}

String CSSComputedStyleDeclaration::removeProperty(const String& name, ExceptionState& exception_state)
{
    exception_state.ThrowDOMException(
        DOMExceptionCode::kNoModificationAllowedError, "These styles are computed, and therefore the '" + name + "' property is read-only.");
    return String();
...
const CSSValue* CSSComputedStyleDeclaration::GetPropertyCSSValueInternal(CSSPropertyID property_id)
{
    return GetPropertyCSSValue(property_id);
}

const CSSValue* CSSComputedStyleDeclaration::GetPropertyCSSValueInternal(const AtomicString& custom_property_name)
{
    DCHECK_EQ(CSSPropertyID::kVariable, CssPropertyID(GetExecutionContext(), custom_property_name));
    return GetPropertyCSSValue(custom_property_name);
}

String CSSComputedStyleDeclaration::GetPropertyValueInternal(CSSPropertyID property_id)
{
    return GetPropertyValue(property_id);
}

String CSSComputedStyleDeclaration::GetPropertyValueWithHint(const String& property_name, unsigned index)
{
    NOTREACHED();
}

String CSSComputedStyleDeclaration::GetPropertyPriorityWithHint(const String& property_name, unsigned index)
{
    NOTREACHED();
}

void CSSComputedStyleDeclaration::SetPropertyInternal(CSSPropertyID id, const String&, StringView, bool, SecureContextMode, ExceptionState& exception_state)
{
    exception_state.ThrowDOMException(DOMExceptionCode::kNoModificationAllowedError,
        "These styles are computed, and therefore the '" + CSSUnresolvedProperty::Get(id).GetPropertyNameString() + "' property is read-only.");
}
...
CSSComputedStyleDeclaration::ScopedCleanStyleForAllProperties::ScopedCleanStyleForAllProperties(CSSComputedStyleDeclaration* declaration)
    : declaration_(declaration)
{
    ...
    declaration_->UpdateStyleAndLayoutIfNeeded(nullptr,
        /*for_all_properties=*/true);
    disallow_scope_.emplace(declaration_->StyledElement()->GetDocument().Lifecycle());
    declaration_->guaranteed_style_clean_ = true;
}

CSSComputedStyleDeclaration::ScopedCleanStyleForAllProperties::~ScopedCleanStyleForAllProperties()
{
