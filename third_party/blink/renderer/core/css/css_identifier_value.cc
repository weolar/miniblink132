...
CSSIdentifierValue* CSSIdentifierValue::Create(CSSValueID value_id)
{
    ...
    if (!css_value) {
        css_value = CssValuePool().SetIdentifierCacheValue(value_id, MakeGarbageCollected<CSSIdentifierValue>(value_id));
    }
    return css_value;
}
    ...
    : CSSValue(kIdentifierClass)
{
    ...
    case Length::kStretch:
        value_id_ = RuntimeEnabledFeatures::LayoutStretchEnabled() ? CSSValueID::kStretch : CSSValueID::kWebkitFillAvailable;
        break;
