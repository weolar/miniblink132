...
public:
    explicit CheckPseudoHasArgumentContext(const CSSSelector* selector, bool match_in_shadow_tree);

    inline bool AdjacentDistanceFixed() const
    {
        return adjacent_distance_limit_ != kInfiniteAdjacentDistance;
    }
    inline int AdjacentDistanceLimit() const
    {
        return adjacent_distance_limit_;
    }
    inline bool DepthFixed() const
    {
        return depth_limit_ != kInfiniteDepth;
    }
    inline int DepthLimit() const
    {
        return depth_limit_;
    }
    ...
    {
        DCHECK(!match_in_shadow_tree_ || traversal_scope_ == kShadowRootSubtree || traversal_scope_ == kShadowRootFixedDepthDescendants
    ...
    const CSSSelector* HasArgument() const
    {
    ...
    {
        return depth_limit_ | (adjacent_distance_limit_ << kDepthBits) | (traversal_scope_ << (kDepthBits + kAdjacentBits));
    }
...
private:
    ...
    static_assert(kTraversalScopeMax <= ((1 << kTraversalScopeBits) - 1), "traversal scope size check");
    static_assert((kDepthBits + kAdjacentBits + kTraversalScopeBits) <= sizeof(CheckPseudoHasArgumentTraversalType) * 8, "traversal type size check");
...
public:
    CheckPseudoHasArgumentTraversalIterator(Element&, CheckPseudoHasArgumentContext&);
    void operator++();
    ...
    inline Element* ScopeElement() const
    {
        return has_anchor_element_;
    }
