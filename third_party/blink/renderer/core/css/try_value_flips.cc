...
const CSSPropertyValueSet* TryValueFlips::FlipSet(const TryTacticList& tactic_list) const
{
...
const CSSPropertyValueSet* TryValueFlips::CreateFlipSet(const TryTacticTransform& transform) const
{
    ...
    auto add = [&declarations, transform](CSSPropertyID from, CSSPropertyID to) {
        declarations.push_back(CSSPropertyValue(CSSPropertyName(from), *MakeGarbageCollected<cssvalue::CSSFlipRevertValue>(to, transform)));
    };
    ...
    add(CSSPropertyID::kAlignSelf, transform.FlippedStart() ? CSSPropertyID::kJustifySelf : CSSPropertyID::kAlignSelf);
    add(CSSPropertyID::kJustifySelf, transform.FlippedStart() ? CSSPropertyID::kAlignSelf : CSSPropertyID::kJustifySelf);
    ...
    return ImmutableCSSPropertyValueSet::Create(base::span<const CSSPropertyValue>(declarations.data(), declarations.size()), kHTMLStandardMode);
}

namespace {

LogicalAxis DeterminePropertyAxis(CSSPropertyID property_id, const WritingDirectionMode& writing_direction)
{
    ...
    case CSSPropertyID::kMinWidth:
        return writing_direction.IsHorizontal() ? LogicalAxis::kInline : LogicalAxis::kBlock;
    ...
    case CSSPropertyID::kMinHeight:
        return writing_direction.IsHorizontal() ? LogicalAxis::kBlock : LogicalAxis::kInline;
    default:
        break;
    }
...
std::optional<LogicalAxis> DetermineValueAxis(CSSValueID value_id, const WritingDirectionMode& writing_direction)
{
    switch (value_id) {
...
CSSValueID ConvertLeftRightToLogical(CSSValueID value, const WritingDirectionMode& writing_direction)
{
    if (value == CSSValueID::kLeft) {
        return writing_direction.IsLtr() ? CSSValueID::kSelfStart : CSSValueID::kSelfEnd;
    }
    if (value == CSSValueID::kRight) {
        return writing_direction.IsLtr() ? CSSValueID::kSelfEnd : CSSValueID::kSelfStart;
    }
    return value;
}
...
const CSSValue* TransformSelfAlignment(
    const CSSValue* value, LogicalAxis logical_axis, const TryTacticTransform& transform, const WritingDirectionMode& writing_direction)
{
    ...
    bool flip_side = (logical_axis == LogicalAxis::kInline) ? transform.FlippedInline() : transform.FlippedBlock();

    CSSValueID from = ident ? ident->GetValueID() : To<CSSIdentifierValue>(pair->Second()).GetValueID();
    ...
    to = transform.FlippedStart() ? ConvertLeftRightToLogical(to, writing_direction) : to;
    if (from == to) {
        return value;
    }
    ...
    return MakeGarbageCollected<CSSValuePair>(
        &pair->First(), CSSIdentifierValue::Create(to), pair->KeepIdenticalValues() ? CSSValuePair::kKeepIdenticalValues : CSSValuePair::kDropIdenticalValues);
}

LogicalToPhysical<CSSValueID> TransformPhysical(
    CSSValueID left, CSSValueID right, CSSValueID top, CSSValueID bottom, const TryTacticTransform& transform, const WritingDirectionMode& writing_direction)
{
    ...
    return transform.Transform(
        TryTacticTransform::LogicalSides<CSSValueID> {
            .inline_start = logical.InlineStart(), .inline_end = logical.InlineEnd(), .block_start = logical.BlockStart(), .block_end = logical.BlockEnd() },
        writing_direction);
}

LogicalToPhysical<CSSValueID> TransformXY(CSSValueID x_start, CSSValueID x_end, CSSValueID y_start, CSSValueID y_end, const TryTacticTransform& transform,
    const WritingDirectionMode& writing_direction)
{
    ...
    return TransformPhysical(x_start, x_end, y_start, y_end, transform, writing_direction);
}

TryTacticTransform::LogicalSides<CSSValueID> TransformLogical(
    CSSValueID inline_start, CSSValueID inline_end, CSSValueID block_start, CSSValueID block_end, const TryTacticTransform& transform)
{
    return transform.Transform(TryTacticTransform::LogicalSides<CSSValueID> {
        .inline_start = inline_start, .inline_end = inline_end, .block_start = block_start, .block_end = block_end });
}
...
{
    bool flip_start_end = (logical_axis == LogicalAxis::kInline) ? transform.FlippedInline() : transform.FlippedBlock();

    auto transform_physical = [&transform, &writing_direction] {
        return TransformPhysical(CSSValueID::kLeft, CSSValueID::kRight, CSSValueID::kTop, CSSValueID::kBottom, transform, writing_direction);
    };

    auto transform_physical_span = [&transform, &writing_direction] {
        return TransformPhysical(CSSValueID::kSpanLeft, CSSValueID::kSpanRight, CSSValueID::kSpanTop, CSSValueID::kSpanBottom, transform, writing_direction);
    };

    auto transform_logical = [&transform] {
        return TransformLogical(CSSValueID::kInlineStart, CSSValueID::kInlineEnd, CSSValueID::kBlockStart, CSSValueID::kBlockEnd, transform);
    };

    auto transform_logical_span = [&transform] {
        return TransformLogical(CSSValueID::kSpanInlineStart, CSSValueID::kSpanInlineEnd, CSSValueID::kSpanBlockStart, CSSValueID::kSpanBlockEnd, transform);
    };

    auto transform_logical_self = [&transform] {
        return TransformLogical(CSSValueID::kSelfInlineStart, CSSValueID::kSelfInlineEnd, CSSValueID::kSelfBlockStart, CSSValueID::kSelfBlockEnd, transform);
    };

    auto transform_logical_span_self = [&transform] {
        return TransformLogical(
            CSSValueID::kSpanSelfInlineStart, CSSValueID::kSpanSelfInlineEnd, CSSValueID::kSpanSelfBlockStart, CSSValueID::kSpanSelfBlockEnd, transform);
    };

    auto transform_xy = [&transform, &writing_direction] {
        return TransformXY(CSSValueID::kXStart, CSSValueID::kXEnd, CSSValueID::kYStart, CSSValueID::kYEnd, transform, writing_direction);
    };

    auto transform_xy_span = [&transform, &writing_direction] {
        return TransformXY(CSSValueID::kSpanXStart, CSSValueID::kSpanXEnd, CSSValueID::kSpanYStart, CSSValueID::kSpanYEnd, transform, writing_direction);
    };

    auto transform_xy_self = [&transform, &writing_direction] {
        return TransformXY(CSSValueID::kXSelfStart, CSSValueID::kXSelfEnd, CSSValueID::kYSelfStart, CSSValueID::kYSelfEnd, transform, writing_direction);
    };

    auto transform_xy_span_self = [&transform, &writing_direction] {
        return TransformXY(
            CSSValueID::kSpanXSelfStart, CSSValueID::kSpanXSelfEnd, CSSValueID::kSpanYSelfStart, CSSValueID::kSpanYSelfEnd, transform, writing_direction);
    };
    ...
    case CSSValueID::kSpanSelfStart:
        return flip_start_end ? CSSValueID::kSpanSelfEnd : CSSValueID::kSpanSelfStart;
    case CSSValueID::kSpanSelfEnd:
        return flip_start_end ? CSSValueID::kSpanSelfStart : CSSValueID::kSpanSelfEnd;
...
const CSSValue* TransformPositionArea(const CSSValue* value, const TryTacticTransform& transform, const WritingDirectionMode& writing_direction)
{
    auto* ident = DynamicTo<CSSIdentifierValue>(value);
    auto* pair = DynamicTo<CSSValuePair>(value);
    if (!ident && !pair) {
    ...
    if (ident) {
        ...
        second_value = css_parsing_utils::IsRepeatedPositionAreaValue(first_value) ? first_value : CSSValueID::kSpanAll;
    } else {
        first_value = To<CSSIdentifierValue>(pair->First()).GetValueID();
        second_value = To<CSSIdentifierValue>(pair->Second()).GetValueID();
    }

    std::optional<LogicalAxis> first_axis = DetermineValueAxis(first_value, writing_direction);
    std::optional<LogicalAxis> second_axis = DetermineValueAxis(second_value, writing_direction);
    ...
    if (first_axis.has_value()) {
        second_axis = (first_axis.value() == LogicalAxis::kInline) ? LogicalAxis::kBlock : LogicalAxis::kInline;
    } else if (second_axis.has_value()) {
        first_axis = (second_axis.value() == LogicalAxis::kInline) ? LogicalAxis::kBlock : LogicalAxis::kInline;
    } else {
        first_axis = LogicalAxis::kBlock;
        second_axis = LogicalAxis::kInline;
    }

    CSSValueID first_value_transformed = TransformPositionAreaKeyword(first_value, first_axis.value(), transform, writing_direction);
    CSSValueID second_value_transformed = TransformPositionAreaKeyword(second_value, second_axis.value(), transform, writing_direction);
    ...
    if (first_value == first_value_transformed && second_value == second_value_transformed) {
    ...
    if (first_value_transformed == CSSValueID::kSpanAll && !css_parsing_utils::IsRepeatedPositionAreaValue(second_value_transformed)) {
        return CSSIdentifierValue::Create(second_value_transformed);
    }
    if (second_value_transformed == CSSValueID::kSpanAll && !css_parsing_utils::IsRepeatedPositionAreaValue(first_value_transformed)) {
        return CSSIdentifierValue::Create(first_value_transformed);
    }

    return MakeGarbageCollected<CSSValuePair>(CSSIdentifierValue::Create(first_value_transformed), CSSIdentifierValue::Create(second_value_transformed),
        pair->KeepIdenticalValues() ? CSSValuePair::kKeepIdenticalValues : CSSValuePair::kDropIdenticalValues);
}

} // namespace

const CSSValue* TryValueFlips::FlipValue(
    CSSPropertyID from_property, const CSSValue* value, const TryTacticTransform& transform, const WritingDirectionMode& writing_direction)
{
    if (const auto* math_value = DynamicTo<CSSMathFunctionValue>(value)) {
        LogicalAxis logical_axis = DeterminePropertyAxis(from_property, writing_direction);
        return math_value->TransformAnchors(logical_axis, transform, writing_direction);
    }
    if (from_property == CSSPropertyID::kAlignSelf || from_property == CSSPropertyID::kJustifySelf) {
        LogicalAxis logical_axis = DeterminePropertyAxis(from_property, writing_direction);
        return TransformSelfAlignment(value, logical_axis, transform, writing_direction);
    }
    if (from_property == CSSPropertyID::kPositionArea || from_property == CSSPropertyID::kInsetArea) {
