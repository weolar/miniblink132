...
public:
    ...
    unsigned GetHash(const CSSPropertyName& name) const
    {
        return name.GetHash();
    }
};
...
{
    EXPECT_EQ(CSSPropertyName(AtomicString("--x")), CSSPropertyName(AtomicString("--x")));
    EXPECT_EQ(CSSPropertyName(CSSPropertyID::kColor), CSSPropertyName(CSSPropertyID::kColor));
    EXPECT_NE(CSSPropertyName(AtomicString("--x")), CSSPropertyName(AtomicString("--y")));
    EXPECT_NE(CSSPropertyName(CSSPropertyID::kColor), CSSPropertyName(CSSPropertyID::kBackgroundColor));
}

TEST_F(CSSPropertyNameTest, From)
{
    EXPECT_TRUE(CSSPropertyName::From(GetDocument().GetExecutionContext(), "color"));
    EXPECT_TRUE(CSSPropertyName::From(GetDocument().GetExecutionContext(), "--x"));
    EXPECT_FALSE(CSSPropertyName::From(GetDocument().GetExecutionContext(), "notaproperty"));
    EXPECT_FALSE(CSSPropertyName::From(GetDocument().GetExecutionContext(), "-not-a-property"));

    EXPECT_EQ(*CSSPropertyName::From(GetDocument().GetExecutionContext(), "color"), CSSPropertyName(CSSPropertyID::kColor));
    EXPECT_EQ(*CSSPropertyName::From(GetDocument().GetExecutionContext(), "--x"), CSSPropertyName(AtomicString("--x")));
}
