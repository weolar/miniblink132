...
static AdditionalBytes AdditionalBytesForImmutableCSSPropertyValueSetWithPropertyCount(unsigned count)
{
    return AdditionalBytes(base::bits::AlignUp(sizeof(Member<CSSValue>) * count, alignof(CSSPropertyValueMetadata)) + sizeof(CSSPropertyValueMetadata) * count);
}

ImmutableCSSPropertyValueSet* ImmutableCSSPropertyValueSet::Create(
    base::span<const CSSPropertyValue> properties, CSSParserMode css_parser_mode, bool contains_cursor_hand)
{
    DCHECK_LE(properties.size(), static_cast<unsigned>(kMaxArraySize));
    return MakeGarbageCollected<ImmutableCSSPropertyValueSet>(
        AdditionalBytesForImmutableCSSPropertyValueSetWithPropertyCount(properties.size()), PassKey(), properties, css_parser_mode, contains_cursor_hand);
}

ImmutableCSSPropertyValueSet* CSSPropertyValueSet::ImmutableCopyIfNeeded() const
{
    auto* immutable_property_set = DynamicTo<ImmutableCSSPropertyValueSet>(const_cast<CSSPropertyValueSet*>(this));
    if (immutable_property_set) {
        return immutable_property_set;
    }

    const auto* mutable_this = To<MutableCSSPropertyValueSet>(this);
    return ImmutableCSSPropertyValueSet::Create(base::span(mutable_this->property_vector_.data(), mutable_this->property_vector_.size()), CssParserMode());
}
...
unsigned CSSPropertyValueSet::ComputeHash() const
{
    ...
    static_assert((WTF::HashTraits<unsigned>::EmptyValue() ^ 0x80000000) != WTF::HashTraits<unsigned>::DeletedValue(),
        "We assume below that flipping the top bit will not turn "
        "EmptyValue into DeletedValue or vice versa");
    if (hash == WTF::HashTraits<unsigned>::EmptyValue() || hash == WTF::HashTraits<unsigned>::DeletedValue()) {
        hash ^= 0x80000000;
    }
...
bool CSSPropertyValueSet::ContentsEqual(const CSSPropertyValueSet& other) const
{
    const unsigned num_properties = PropertyCount();
...
MutableCSSPropertyValueSet::MutableCSSPropertyValueSet(base::span<const CSSPropertyValue> properties)
...
ImmutableCSSPropertyValueSet::ImmutableCSSPropertyValueSet(
    PassKey, base::span<const CSSPropertyValue> properties, CSSParserMode css_parser_mode, bool contains_query_hand)
    : CSSPropertyValueSet(css_parser_mode, properties.size(), contains_query_hand)
{
    if (array_size_ > 0) {
        ...
        // expect.
        UNSAFE_BUFFERS(base::span<CSSPropertyValueMetadata> metadata_array(const_cast<CSSPropertyValueMetadata*>(MetadataArrayBase()), array_size_));
        UNSAFE_BUFFERS(base::span<Member<const CSSValue>> value_array(const_cast<Member<const CSSValue>*>(ValueArrayBase()), array_size_));
...
{
    return static_cast<uint16_t>(AtRuleDescriptorIDAsCSSPropertyID(descriptor_id));
}
...
#if DCHECK_IS_ON()
    DCHECK(!result || property_id == CSSPropertyID::kInternalFontSizeDelta || CSSProperty::Get(ResolveCSSPropertyID(property_id)).IsWebExposed());
#endif
    return result;
}

static bool IsPropertyMatch(const CSSPropertyValueMetadata& metadata, uint16_t id, const AtomicString& custom_property_name)
{
    DCHECK_EQ(id, static_cast<uint16_t>(CSSPropertyID::kVariable));
    return metadata.Name() == CSSPropertyName(custom_property_name);
}

static bool IsPropertyMatch(const CSSPropertyValueMetadata& metadata, uint16_t id, AtRuleDescriptorID descriptor_id)
{
    return IsPropertyMatch(metadata, id, AtRuleDescriptorIDAsCSSPropertyID(descriptor_id));
}

template <typename T> int ImmutableCSSPropertyValueSet::FindPropertyIndex(const T& property) const
{
...
template CORE_EXPORT int ImmutableCSSPropertyValueSet::FindPropertyIndex(const CSSPropertyID&) const;
template CORE_EXPORT int ImmutableCSSPropertyValueSet::FindPropertyIndex(const AtomicString&) const;
template CORE_EXPORT int ImmutableCSSPropertyValueSet::FindPropertyIndex(const AtRuleDescriptorID&) const;

void ImmutableCSSPropertyValueSet::TraceAfterDispatch(blink::Visitor* visitor) const
{
    ...
    : CSSPropertyValueSet(other.CssParserMode())
{
    if (auto* other_mutable_property_set = DynamicTo<MutableCSSPropertyValueSet>(other)) {
        ...
        for (unsigned i = 0; i < other.PropertyCount(); ++i) {
            ...
            property_vector_.UncheckedAppend(CSSPropertyValue(property.PropertyMetadata(), property.Value()));
            may_have_logical_properties_ |= kLogicalGroupProperties.Has(property.Id());
        }
    }
}

static String SerializeShorthand(const CSSPropertyValueSet& property_set, CSSPropertyID property_id)
{
...
static String SerializeShorthand(const CSSPropertyValueSet&, const AtomicString& custom_property_name)
{
    // Custom properties are never shorthands.
    return String();
}

static String SerializeShorthand(const CSSPropertyValueSet& property_set, AtRuleDescriptorID atrule_id)
{
    // Descriptor shorthands aren't handled yet.
    return String();
}

template <typename T> String CSSPropertyValueSet::GetPropertyValue(const T& property) const
{
...
template CORE_EXPORT String CSSPropertyValueSet::GetPropertyValue<CSSPropertyID>(const CSSPropertyID&) const;
template CORE_EXPORT String CSSPropertyValueSet::GetPropertyValue<AtRuleDescriptorID>(const AtRuleDescriptorID&) const;
template CORE_EXPORT String CSSPropertyValueSet::GetPropertyValue<AtomicString>(const AtomicString&) const;

String CSSPropertyValueSet::GetPropertyValueWithHint(const AtomicString& property_name, unsigned index) const
{
...
template <typename T> const CSSValue* CSSPropertyValueSet::GetPropertyCSSValue(const T& property) const
{
...
template CORE_EXPORT const CSSValue* CSSPropertyValueSet::GetPropertyCSSValue<CSSPropertyID>(const CSSPropertyID&) const;
template CORE_EXPORT const CSSValue* CSSPropertyValueSet::GetPropertyCSSValue<AtRuleDescriptorID>(const AtRuleDescriptorID&) const;
template CORE_EXPORT const CSSValue* CSSPropertyValueSet::GetPropertyCSSValue<AtomicString>(const AtomicString&) const;

const CSSValue* CSSPropertyValueSet::GetPropertyCSSValueWithHint(const AtomicString& property_name, unsigned index) const
{
...
template <typename T> bool MutableCSSPropertyValueSet::RemoveProperty(const T& property, String* return_text)
{
...
template CORE_EXPORT bool MutableCSSPropertyValueSet::RemoveProperty(const CSSPropertyID&, String*);
template CORE_EXPORT bool MutableCSSPropertyValueSet::RemoveProperty(const AtomicString&, String*);

template <typename T> bool CSSPropertyValueSet::PropertyIsImportant(const T& property) const
{
    int found_property_index = FindPropertyIndex(property);
    if (found_property_index != -1) {
...
template CORE_EXPORT bool CSSPropertyValueSet::PropertyIsImportant<CSSPropertyID>(const CSSPropertyID&) const;
template bool CSSPropertyValueSet::PropertyIsImportant<AtomicString>(const AtomicString&) const;

bool CSSPropertyValueSet::PropertyIsImportantWithHint(const AtomicString& property_name, unsigned index) const
{
    DCHECK_EQ(property_name, PropertyAt(index).Name().ToAtomicString());
...
MutableCSSPropertyValueSet::SetResult MutableCSSPropertyValueSet::ParseAndSetProperty(
    CSSPropertyID unresolved_property, StringView value, bool important, SecureContextMode secure_context_mode, StyleSheetContents* context_style_sheet)
{
    ...
    return CSSParser::ParseValue(this, unresolved_property, value, important, secure_context_mode, context_style_sheet);
}

MutableCSSPropertyValueSet::SetResult MutableCSSPropertyValueSet::ParseAndSetCustomProperty(const AtomicString& custom_property_name, StringView value,
    bool important, SecureContextMode secure_context_mode, StyleSheetContents* context_style_sheet, bool is_animation_tainted)
{
    if (value.empty()) {
        return RemoveProperty(custom_property_name) ? kChangedPropertySet : kUnchanged;
    }
    return CSSParser::ParseValueForCustomProperty(this, custom_property_name, value, important, secure_context_mode, context_style_sheet, is_animation_tainted);
}

void MutableCSSPropertyValueSet::SetProperty(const CSSPropertyName& name, const CSSValue& value, bool important)
{
...
void MutableCSSPropertyValueSet::SetProperty(CSSPropertyID property_id, const CSSValue& value, bool important)
{
    DCHECK_NE(property_id, CSSPropertyID::kVariable);
    DCHECK_NE(property_id, CSSPropertyID::kWhiteSpace);
    StylePropertyShorthand shorthand = shorthandForProperty(property_id);
    if (!shorthand.length()) {
        SetLonghandProperty(CSSPropertyValue(CSSPropertyName(property_id), value, important));
        return;
    }
    ...
    for (const CSSProperty* const longhand : shorthand.properties()) {
        ...
        property_vector_.push_back(CSSPropertyValue(longhand_name, value, important));
    }
    InvalidateHashIfComputed();
}
...
MutableCSSPropertyValueSet::SetResult MutableCSSPropertyValueSet::SetLonghandProperty(CSSPropertyValue property)
{
    ...
    DCHECK_EQ(shorthandForProperty(id).length(), 0u) << CSSProperty::Get(id).GetPropertyNameString() << " is a shorthand";
...
void MutableCSSPropertyValueSet::SetLonghandProperty(CSSPropertyID property_id, const CSSValue& value)
{
    DCHECK_EQ(shorthandForProperty(property_id).length(), 0u) << CSSProperty::Get(property_id).GetPropertyNameString() << " is a shorthand";
...
MutableCSSPropertyValueSet::SetResult MutableCSSPropertyValueSet::SetLonghandProperty(CSSPropertyID property_id, CSSValueID identifier, bool important)
{
...
void MutableCSSPropertyValueSet::ParseDeclarationList(
    const String& style_declaration, SecureContextMode secure_context_mode, StyleSheetContents* context_style_sheet)
{
    ...
    if (context_style_sheet) {
        context = MakeGarbageCollected<CSSParserContext>(context_style_sheet->ParserContext(), context_style_sheet);
        context->SetMode(CssParserMode());
    } else {
        context = MakeGarbageCollected<CSSParserContext>(CssParserMode(), secure_context_mode);
    }

    CSSParser::ParseDeclarationList(context, this, style_declaration);
}

MutableCSSPropertyValueSet::SetResult MutableCSSPropertyValueSet::AddParsedProperties(const HeapVector<CSSPropertyValue, 64>& properties)
{
...
inline bool ContainsId(const base::span<const CSSProperty* const>& set, CSSPropertyID id)
{
...
bool MutableCSSPropertyValueSet::RemovePropertiesInSet(base::span<const CSSProperty* const> set)
{
...
{
    return const_cast<CSSPropertyValue*>(name.IsCustomProperty() ? FindPropertyPointer(name.ToAtomicString()) : FindPropertyPointer(name.Id()));
}
...
MutableCSSPropertyValueSet* CSSPropertyValueSet::CopyPropertiesInSet(const Vector<const CSSProperty*>& properties) const
{
    ...
    return MakeGarbageCollected<MutableCSSPropertyValueSet>(base::span<const CSSPropertyValue>(list.data(), list.size()));
}

CSSStyleDeclaration* MutableCSSPropertyValueSet::EnsureCSSStyleDeclaration(ExecutionContext* execution_context)
{
    // FIXME: get rid of this weirdness of a CSSStyleDeclaration inside of a
    // style property set.
    if (cssom_wrapper_) {
        DCHECK(!static_cast<CSSStyleDeclaration*>(cssom_wrapper_.Get())->parentRule());
        DCHECK(!cssom_wrapper_->ParentElement());
        return cssom_wrapper_.Get();
    }
    cssom_wrapper_ = MakeGarbageCollected<PropertySetCSSStyleDeclaration>(execution_context, *this);
    return cssom_wrapper_.Get();
}

template <typename T> int MutableCSSPropertyValueSet::FindPropertyIndex(const T& property) const
{
...
template CORE_EXPORT int MutableCSSPropertyValueSet::FindPropertyIndex(const CSSPropertyID&) const;
template CORE_EXPORT int MutableCSSPropertyValueSet::FindPropertyIndex(const AtomicString&) const;

template <typename T> const CSSPropertyValue* MutableCSSPropertyValueSet::FindPropertyPointer(const T& property) const
{
    uint16_t id = GetConvertedCSSPropertyID(property);

    auto it = std::find_if(property_vector_.begin(), property_vector_.end(),
        [property, id](const CSSPropertyValue& css_property) -> bool { return IsPropertyMatch(css_property.Metadata(), id, property); });
    return (it == property_vector_.end()) ? nullptr : &*it;
}

void MutableCSSPropertyValueSet::TraceAfterDispatch(blink::Visitor* visitor) const
{
...
unsigned CSSPropertyValueSet::AverageSizeInBytes()
{
    ...
    return sizeof(ImmutableCSSPropertyValueSet) + static_cast<wtf_size_t>(AdditionalBytesForImmutableCSSPropertyValueSetWithPropertyCount(4).value);
}

// See the function above if you need to update this.
struct SameSizeAsCSSPropertyValueSet final : public GarbageCollected<SameSizeAsCSSPropertyValueSet> {
    uint32_t bitfield;
    unsigned hash;
};
