...
{
    return name == html_names::kClassAttr.LocalName() || name == html_names::kIdAttr.LocalName() || name == html_names::kStyleAttr.LocalName();
}

template <class Func> inline void CollectElementIdentifierHashes(const Element& element, Func&& func)
{
...
void CollectDescendantCompoundSelectorIdentifierHashes(
    const CSSSelector* selector, CSSSelector::RelationType relation, const StyleScope* style_scope, Vector<unsigned>& hashes);

inline void CollectDescendantSelectorIdentifierHashes(const CSSSelector& selector, const StyleScope* style_scope, Vector<unsigned>& hashes)
{
    ...
    case CSSSelector::kAttributeHyphen: {
        ...
        auto lower_name = attribute_name.IsLowerASCII() ? attribute_name : attribute_name.LowerASCII();
    ...
    case CSSSelector::kPseudoClass:
        ...
        case CSSSelector::kPseudoParent: {
            ...
            if (selector_list && CSSSelectorList::Next(*selector_list) == nullptr) {
                CollectDescendantCompoundSelectorIdentifierHashes(selector_list, CSSSelector::kDescendant, style_scope, hashes);
            }
            break;
        }
        case CSSSelector::kPseudoScope:
            if (style_scope) {
                ...
                if (selector_list && CSSSelectorList::Next(*selector_list) == nullptr) {
                    CollectDescendantCompoundSelectorIdentifierHashes(selector_list, CSSSelector::kDescendant, style_scope->Parent(), hashes);
                }
            }
            break;
        default:
    ...
    for (const CSSSelector* current = selector; current; current = current->NextSimpleSelector()) {
        ...
        case CSSSelector::kScopeActivation:
            if (!skip_over_subselectors) {
                CollectDescendantSelectorIdentifierHashes(*current, style_scope, hashes);
            }
            break;
...
void SelectorFilter::PushParentStackFrame(Element& parent)
{
    ...
    CollectElementIdentifierHashes(parent, [this](unsigned hash) { ancestor_identifier_filter_.Add(hash); });
}

void SelectorFilter::PopParentStackFrame()
{
    DCHECK(!parent_stack_.empty());
    CollectElementIdentifierHashes(*parent_stack_.back(), [this](unsigned hash) { ancestor_identifier_filter_.Remove(hash); });
...
void SelectorFilter::PushParent(Element& parent)
{
    ...
    } else if (parent_stack_.back() != FlatTreeTraversal::ParentElement(parent) && parent_stack_.back() != parent.ParentOrShadowHostElement()) {
        LOG(DFATAL) << "Parent stack must be consistent; pushed " << parent << " with parent " << parent.ParentOrShadowHostElement() << " and flat-tree parent "
                    << FlatTreeTraversal::ParentElement(parent) << ", but the stack contained " << parent_stack_.back() << ", which is neither";
    }
#endif
...
void SelectorFilter::CollectIdentifierHashes(const CSSSelector& selector, const StyleScope* style_scope, Vector<unsigned>& bloom_hash_backing)
{
    CollectDescendantCompoundSelectorIdentifierHashes(selector.NextSimpleSelector(), selector.Relation(), style_scope, bloom_hash_backing);
}
