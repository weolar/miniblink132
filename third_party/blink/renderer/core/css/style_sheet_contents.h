...
class CORE_EXPORT StyleSheetContents final : public GarbageCollected<StyleSheetContents> {
public:
    static const Document* SingleOwnerDocument(const StyleSheetContents*);

    StyleSheetContents(const CSSParserContext* context, const String& original_url = String(), StyleRuleImport* owner_rule = nullptr);
    ...
    ParseSheetResult ParseString(const String&, bool allow_import_rules = true, CSSDeferPropertyParsing defer_property_parsing = CSSDeferPropertyParsing::kNo);
    ...
    bool HasSingleOwnerDocument() const
    {
        return has_single_owner_document_;
    }
    ...
    void SetHasFontFaceRule()
    {
        has_font_face_rule_ = true;
    }
    bool HasFontFaceRule() const
    {
        return has_font_face_rule_;
    }
    ...
    wtf_size_t ReplaceRuleIfExists(StyleRuleBase* old_rule, StyleRuleBase* new_rule, wtf_size_t position_hint);
    ...
    RuleSetDiff* GetRuleSetDiff() const
    {
        return rule_set_diff_.Get();
    }
    void ClearRuleSetDiff()
    {
        rule_set_diff_.Clear();
    }
    ...
    const HeapVector<Member<StyleRuleLayerStatement>>& PreImportLayerStatementRules() const
    {
    ...
    StyleRuleImport* OwnerRule() const
    {
    ...
    {
        return parser_context_->BaseURL();
    }
    ...
    {
        return parser_context_->IsOriginClean();
    }
    ...
    bool IsUsedFromTextCache() const
    {
        return is_used_from_text_cache_;
    }
    void SetIsUsedFromTextCache()
    {
        is_used_from_text_cache_ = true;
    }
    ...
    {
        return has_media_queries_;
    }

    bool DidLoadErrorOccur() const
    {
        return did_load_error_occur_;
    }
