...
{
    return GetDocument()->GetStyleEngine().StyleSheetsForStyleSheetList(*tree_scope_);
}
...
{
    if (!tree_scope_) {
        return index < style_sheet_vector_.size() ? style_sheet_vector_[index].Get() : nullptr;
    }
...
{
    if (GetDocument()) {
        UseCounter::Count(*GetDocument(), WebFeature::kStyleSheetListAnonymousNamedGetter);
    }
    ...
    if (sheet) {
        UseCounter::Count(*GetDocument(), WebFeature::kStyleSheetListNonNullAnonymousNamedGetter);
    }
    return sheet;
}

bool StyleSheetList::NamedPropertyQuery(const AtomicString& name, ExceptionState&)
{
