...
bool ScrollStateQuerySnapshot::UpdateScrollState()
{
    ...
    ContainerOverflowingFlags overflowing_horizontal = static_cast<ContainerOverflowingFlags>(ContainerOverflowing::kNone);
    ContainerOverflowingFlags overflowing_vertical = static_cast<ContainerOverflowingFlags>(ContainerOverflowing::kNone);

    LayoutBoxModelObject* layout_object = DynamicTo<LayoutBoxModelObject>(container_->GetLayoutObject());
    if (layout_object) {
        ...
        if (PaintLayerScrollableArea* scrollable_area = layout_object->GetScrollableArea()) {
            ...
            if (offset.x() > min_offset.x()) {
                overflowing_horizontal |= static_cast<ContainerOverflowingFlags>(ContainerOverflowing::kStart);
            }
            if (offset.x() < max_offset.x()) {
                overflowing_horizontal |= static_cast<ContainerOverflowingFlags>(ContainerOverflowing::kEnd);
            }
            if (offset.y() > min_offset.y()) {
                overflowing_vertical |= static_cast<ContainerOverflowingFlags>(ContainerOverflowing::kStart);
            }
            if (offset.y() < max_offset.y()) {
                overflowing_vertical |= static_cast<ContainerOverflowingFlags>(ContainerOverflowing::kEnd);
            }
        }
    }
    ...
    if (stuck_horizontal_ != stuck_horizontal || stuck_vertical_ != stuck_vertical || overflowing_horizontal_ != overflowing_horizontal
        ...
        container_->SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kScrollTimeline));
        return true;
    }
    return false;
}
