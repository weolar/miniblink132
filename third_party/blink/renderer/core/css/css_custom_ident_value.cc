...
String CSSCustomIdentValue::CustomCSSText() const
{
    if (IsKnownPropertyID()) {
        return CSSUnresolvedProperty::Get(property_id_).GetPropertyNameAtomicString();
    }
...
const CSSCustomIdentValue& CSSCustomIdentValue::PopulateWithTreeScope(const TreeScope* tree_scope) const
{
    DCHECK(this->needs_tree_scope_population_);
    CSSCustomIdentValue* populated = MakeGarbageCollected<CSSCustomIdentValue>(*this);
