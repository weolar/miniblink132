...
public:
    CSSColorMixValue(const CSSValue* color1, const CSSValue* color2, const CSSPrimitiveValue* p1, const CSSPrimitiveValue* p2,
        const Color::ColorSpace color_interpolation_space, const Color::HueInterpolationMethod hue_interpolation_method)
    ...
    const CSSValue& Color1() const
    {
        return *color1_;
    }
    const CSSValue& Color2() const
    {
        return *color2_;
    }
    const CSSPrimitiveValue* Percentage1() const
    {
        return percentage1_.Get();
    }
    const CSSPrimitiveValue* Percentage2() const
    {
        return percentage2_.Get();
    }
    ...
    Color Mix(const Color& color1, const Color& color2, const CSSLengthResolver& length_resolver) const;

    // https://www.w3.org/TR/css-color-5/#color-mix-percent-norm
    static bool NormalizePercentages(const CSSPrimitiveValue* percentage1, const CSSPrimitiveValue* percentage2, double& mix_amount, double& alpha_multiplier,
        const CSSLengthResolver& length_resolver);
    bool NormalizePercentages(double& mix_amount, double& alpha_multiplier, const CSSLengthResolver& length_resolver) const
    {
        return NormalizePercentages(Percentage1(), Percentage2(), mix_amount, alpha_multiplier, length_resolver);
    }
...
template <> struct DowncastTraits<cssvalue::CSSColorMixValue> {
