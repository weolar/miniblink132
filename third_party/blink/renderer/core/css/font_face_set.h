...
class CORE_EXPORT FontFaceSet : public EventTarget, public ExecutionContextClient, public FontFaceSetIterable, public FontFace::LoadFontCallback {
...
public:
    ...
    ScriptPromise<IDLSequence<FontFace>> load(ScriptState*, const String& font, const String& text);
...
protected:
    ...
    virtual const HeapLinkedHashSet<Member<FontFace>>& CSSConnectedFontFaceList() const = 0;
    ...
    public:
        explicit IterationSource(HeapVector<Member<FontFace>>&& font_faces)
            : index_(0)
            , font_faces_(std::move(font_faces))
        {
        }
        bool FetchNextItem(ScriptState* script_state, FontFace*& value, ExceptionState& exception_state) override;
    ...
    class LoadFontPromiseResolver final : public GarbageCollected<LoadFontPromiseResolver>, public FontFace::LoadFontCallback {
    public:
        LoadFontPromiseResolver(FontFaceArray* faces, ScriptState* script_state)
            : num_loading_(faces->size())
            , error_occured_(false)
            , resolver_(MakeGarbageCollected<ScriptPromiseResolver<IDLSequence<FontFace>>>(script_state))
        {
...
private:
    FontFaceSetIterable::IterationSource* CreateIterationSource(ScriptState*, ExceptionState&) override;
