...
typename RobinHoodMap<Key, Value>::Bucket* RobinHoodMap<Key, Value>::InsertInternal(RobinHoodMap<Key, Value>::Bucket to_insert)
{
...
template <class Key, class Value> typename RobinHoodMap<Key, Value>::Bucket* RobinHoodMap<Key, Value>::Insert(const Key& key)
{
...
template <class Key, class Value> RobinHoodMap<Key, Value> RobinHoodMap<Key, Value>::Grow()
{
    double new_size = num_buckets_ * kGrowthFactor;
    CHECK_LE(new_size + kPossibleBucketsPerKey, std::numeric_limits<unsigned>::max()) << "This should never happen with 24-bit hashes";
...
template <class Key, class Value> typename RobinHoodMap<Key, Value>::Bucket* RobinHoodMap<Key, Value>::InsertWithRehashing(const Key& key)
{
