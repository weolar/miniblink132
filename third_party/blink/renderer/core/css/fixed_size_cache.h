...
template <class Key, class Value, class KeyTraits = HashTraits<Key>, unsigned cache_size = 512> class FixedSizeCache {
    static_assert((cache_size & (cache_size - 1)) == 0, "cache_size should be a power of two");
...
public:
    ...
    {
        return Find(key, KeyTraits::GetHash(key));
    }
    ...
    Value* Find(const Key& key, unsigned hash)
    {
        ...
        if (prefilter_[bucket_set] == prefilter_hash && cache_[bucket_set].first == key) {
            return &cache_[bucket_set].second;
        }
        if (prefilter_[bucket_set + 1] == prefilter_hash && cache_[bucket_set + 1].first == key) {
            using std::swap;
