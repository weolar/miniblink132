...
DocumentStyleSheetCollection::DocumentStyleSheetCollection(TreeScope& tree_scope)
...
void DocumentStyleSheetCollection::CollectStyleSheetsFromCandidates(StyleEngine& engine, DocumentStyleSheetCollector& collector)
{
    ...
    for (Node* n : style_sheet_candidate_nodes_) {
        ...
        if (!candidate.CanBeActivated(GetDocument().GetStyleEngine().PreferredStylesheetSetName())) {
            continue;
        }

        CSSStyleSheet* css_sheet = To<CSSStyleSheet>(sheet);
        collector.AppendActiveStyleSheet(std::make_pair(css_sheet, rule_set_scope.RuleSetForSheet(engine, css_sheet)));
    ...
    for (CSSStyleSheet* sheet : *tree_scope.AdoptedStyleSheets()) {
        if (!sheet || !sheet->CanBeActivated(GetDocument().GetStyleEngine().PreferredStylesheetSetName())) {
            continue;
        }
        DCHECK_EQ(GetDocument(), sheet->ConstructorDocument());
        collector.AppendSheetForList(sheet);
        collector.AppendActiveStyleSheet(std::make_pair(sheet, engine.RuleSetForSheet(*sheet)));
    }
}

void DocumentStyleSheetCollection::CollectStyleSheets(StyleEngine& engine, DocumentStyleSheetCollector& collector)
{
    for (auto& sheet : GetDocument().GetStyleEngine().InjectedAuthorStyleSheets()) {
        collector.AppendActiveStyleSheet(std::make_pair(sheet.second, GetDocument().GetStyleEngine().RuleSetForSheet(*sheet.second)));
    }
    CollectStyleSheetsFromCandidates(engine, collector);
    if (CSSStyleSheet* inspector_sheet = GetDocument().GetStyleEngine().InspectorStyleSheet()) {
        collector.AppendActiveStyleSheet(std::make_pair(inspector_sheet, GetDocument().GetStyleEngine().RuleSetForSheet(*inspector_sheet)));
    }
}
