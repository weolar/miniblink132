...
public:
    ...
    void SetUp() override
    {
        document_ = HTMLDocument::CreateForTest(execution_context_.GetExecutionContext());
    ...
    SelectorPreMatch CollectFeatures(
        const String& selector_text, CSSNestingType nesting_type = CSSNestingType::kNone, StyleRule* parent_rule_for_nesting = nullptr)
    {
        return CollectFeaturesTo(selector_text, rule_feature_set_, nesting_type, parent_rule_for_nesting);
    }

    SelectorPreMatch CollectFeatures(StyleRule* style_rule, const StyleScope* style_scope)
    {
        return CollectFeaturesTo(style_rule, style_scope, rule_feature_set_);
    }

    static SelectorPreMatch CollectFeaturesTo(base::span<CSSSelector> selector_vector, const StyleScope* style_scope, RuleFeatureSet& set)
    {
        if (selector_vector.empty()) {
            return SelectorPreMatch::kNeverMatches;
        }

        auto* style_rule = StyleRule::Create(selector_vector, MakeGarbageCollected<MutableCSSPropertyValueSet>(kHTMLStandardMode));
        return CollectFeaturesTo(style_rule, style_scope, set);
    }

    static SelectorPreMatch CollectFeaturesTo(StyleRule* style_rule, const StyleScope* style_scope, RuleFeatureSet& set)
    {
        SelectorPreMatch result = SelectorPreMatch::kNeverMatches;
        for (const CSSSelector* s = style_rule->FirstSelector(); s; s = CSSSelectorList::Next(*s)) {
            if (set.CollectFeaturesFromSelector(*s, style_scope) == SelectorPreMatch::kMayMatch) {
    ...
    static SelectorPreMatch CollectFeaturesTo(const String& selector_text, RuleFeatureSet& set, CSSNestingType nesting_type, StyleRule* parent_rule_for_nesting)
    {
        HeapVector<CSSSelector> arena;
        base::span<CSSSelector> selector_vector = CSSParser::ParseSelector(StrictCSSParserContext(SecureContextMode::kInsecureContext), nesting_type,
            parent_rule_for_nesting, false /* is_within_scope */, nullptr, selector_text, arena);
        return CollectFeaturesTo(selector_vector, nullptr /* style_scope */, set);
    }

    void ClearFeatures()
    {
        rule_feature_set_.Clear();
    }

    void CollectInvalidationSetsForClass(InvalidationLists& invalidation_lists, const char* class_name) const
    {
        Element* element = Traversal<HTMLElement>::FirstChild(*Traversal<HTMLElement>::FirstChild(*document_->body()));
        rule_feature_set_.GetRuleInvalidationData().CollectInvalidationSetsForClass(invalidation_lists, *element, AtomicString(class_name));
    }

    void CollectInvalidationSetsForId(InvalidationLists& invalidation_lists, const char* id) const
    {
        Element* element = Traversal<HTMLElement>::FirstChild(*Traversal<HTMLElement>::FirstChild(*document_->body()));
        rule_feature_set_.GetRuleInvalidationData().CollectInvalidationSetsForId(invalidation_lists, *element, AtomicString(id));
    }

    void CollectInvalidationSetsForAttribute(InvalidationLists& invalidation_lists, const QualifiedName& attribute_name) const
    {
        Element* element = Traversal<HTMLElement>::FirstChild(*Traversal<HTMLElement>::FirstChild(*document_->body()));
        rule_feature_set_.GetRuleInvalidationData().CollectInvalidationSetsForAttribute(invalidation_lists, *element, attribute_name);
    }

    void CollectInvalidationSetsForPseudoClass(InvalidationLists& invalidation_lists, CSSSelector::PseudoType pseudo) const
    {
        Element* element = Traversal<HTMLElement>::FirstChild(*Traversal<HTMLElement>::FirstChild(*document_->body()));
        rule_feature_set_.GetRuleInvalidationData().CollectInvalidationSetsForPseudoClass(invalidation_lists, *element, pseudo);
    }

    void CollectPartInvalidationSet(InvalidationLists& invalidation_lists) const
    {
        rule_feature_set_.GetRuleInvalidationData().CollectPartInvalidationSet(invalidation_lists);
    }

    void CollectUniversalSiblingInvalidationSet(InvalidationLists& invalidation_lists)
    {
        rule_feature_set_.GetRuleInvalidationData().CollectUniversalSiblingInvalidationSet(invalidation_lists, 1);
    }

    void CollectNthInvalidationSet(InvalidationLists& invalidation_lists)
    {
        rule_feature_set_.GetRuleInvalidationData().CollectNthInvalidationSet(invalidation_lists);
    }

    bool NeedsHasInvalidationForClass(const char* class_name)
    {
        return rule_feature_set_.GetRuleInvalidationData().NeedsHasInvalidationForClass(AtomicString(class_name));
    }
    ...
    template <BackingType type> HashSet<AtomicString> ToHashSet(typename InvalidationSet::Backing<type>::Range range)
    {
    ...
    AssertionResult HasNoInvalidation(InvalidationSetVector& invalidation_sets)
    {
        if (invalidation_sets.size() != 0) {
            return AssertionFailure() << "has " << invalidation_sets.size() << " invalidation set(s), should have 0";
        }
        return AssertionSuccess();
    }

    AssertionResult HasSelfInvalidation(InvalidationSetVector& invalidation_sets)
    {
        if (invalidation_sets.size() != 1) {
            return AssertionFailure() << "has " << invalidation_sets.size() << " invalidation set(s), should have 1";
        }
    ...
    AssertionResult HasNoSelfInvalidation(InvalidationSetVector& invalidation_sets)
    {
    ...
    AssertionResult HasSelfInvalidationSet(InvalidationSetVector& invalidation_sets)
    {
        if (invalidation_sets.size() != 1u) {
            return AssertionFailure() << "has " << invalidation_sets.size() << " invalidation set(s), should have 1";
        }
    ...
    AssertionResult HasNotSelfInvalidationSet(InvalidationSetVector& invalidation_sets)
    {
        if (invalidation_sets.size() != 1u) {
            return AssertionFailure() << "has " << invalidation_sets.size() << " invalidation set(s), should have 1";
        }
    ...
    AssertionResult HasWholeSubtreeInvalidation(InvalidationSetVector& invalidation_sets)
    {
        if (invalidation_sets.size() != 1u) {
            return AssertionFailure() << "has " << invalidation_sets.size() << " invalidation set(s), should have 1";
        }
    ...
    AssertionResult HasClassInvalidation(const char* class_name, InvalidationSetVector& invalidation_sets)
    {
        if (invalidation_sets.size() != 1u) {
            return AssertionFailure() << "has " << invalidation_sets.size() << " invalidation set(s), should have 1";
        }
    ...
    AssertionResult HasClassInvalidation(const char* first_class_name, const char* second_class_name, InvalidationSetVector& invalidation_sets)
    {
        if (invalidation_sets.size() != 1u) {
            return AssertionFailure() << "has " << invalidation_sets.size() << " invalidation set(s), should have 1";
        }
        ...
        if (!classes.Contains(AtomicString(first_class_name))) {
            return AssertionFailure() << "should invalidate class " << first_class_name;
        }
        if (!classes.Contains(AtomicString(second_class_name))) {
            return AssertionFailure() << "should invalidate class " << second_class_name;
        }
        return AssertionSuccess();
    }

    AssertionResult HasClassInvalidation(const AtomicString& first_class_name, const AtomicString& second_class_name, const AtomicString& third_class_name,
        InvalidationSetVector& invalidation_sets)
    {
        ...
        if (!classes.Contains(third_class_name)) {
            return AssertionFailure() << "should invalidate class " << third_class_name;
        }
        return AssertionSuccess();
    }

    AssertionResult HasSiblingClassInvalidation(unsigned max_direct_adjacent_selectors, const char* sibling_name, InvalidationSetVector& invalidation_sets)
    {
        if (invalidation_sets.size() != 1u) {
            return AssertionFailure() << "has " << invalidation_sets.size() << " invalidation set(s), should have 1";
        }
        const auto& sibling_invalidation_set = To<SiblingInvalidationSet>(*invalidation_sets[0]);
        HashSet<AtomicString> classes = ClassSet(sibling_invalidation_set);
        if (classes.size() != 1u) {
            return AssertionFailure() << classes.size() << " should be 1";
        }
        if (!classes.Contains(AtomicString(sibling_name))) {
            return AssertionFailure() << "should invalidate sibling id " << sibling_name;
        }
        if (sibling_invalidation_set.MaxDirectAdjacentSelectors() != max_direct_adjacent_selectors) {
            return AssertionFailure() << sibling_invalidation_set.MaxDirectAdjacentSelectors() << " should be " << max_direct_adjacent_selectors;
        }
        return AssertionSuccess();
    }

    AssertionResult HasSiblingIdInvalidation(unsigned max_direct_adjacent_selectors, const char* sibling_name, InvalidationSetVector& invalidation_sets)
    {
        if (invalidation_sets.size() != 1u) {
            return AssertionFailure() << "has " << invalidation_sets.size() << " invalidation set(s), should have 1";
        }
    ...
    AssertionResult HasSiblingDescendantInvalidation(
        unsigned max_direct_adjacent_selectors, const char* sibling_name, const char* descendant_name, InvalidationSetVector& invalidation_sets)
    {
        if (invalidation_sets.size() != 1u) {
            return AssertionFailure() << "has " << invalidation_sets.size() << " invalidation set(s), should have 1";
        }
        ...
        if (!classes.Contains(AtomicString(sibling_name))) {
            return AssertionFailure() << "classes.Contains(sibling_name) should be true";
        }
        if (sibling_invalidation_set.MaxDirectAdjacentSelectors() != max_direct_adjacent_selectors) {
            return AssertionFailure() << sibling_invalidation_set.MaxDirectAdjacentSelectors() << " should be " << max_direct_adjacent_selectors;
        }

        HashSet<AtomicString> descendant_classes = ClassSet(*sibling_invalidation_set.SiblingDescendants());
        ...
        if (!descendant_classes.Contains(AtomicString(descendant_name))) {
            return AssertionFailure() << "should invalidate descendant class " << descendant_name;
        }
        return AssertionSuccess();
    }

    AssertionResult HasSiblingDescendantInvalidation(
        unsigned max_direct_adjacent_selectors, const char* descendant_name, InvalidationSetVector& invalidation_sets)
    {
        if (invalidation_sets.size() != 1u) {
            return AssertionFailure() << "has " << invalidation_sets.size() << " invalidation set(s), should have 1";
        }
        ...
        if (!descendant_classes.Contains(AtomicString(descendant_name))) {
            return AssertionFailure() << "should descendant invalidate " << descendant_name;
        }
        return AssertionSuccess();
    }

    AssertionResult HasSiblingAndSiblingDescendantInvalidationForLogicalCombinationsInHas(
        const char* sibling_name, const char* sibling_name_for_sibling_descendant, const char* descendant_name, InvalidationSetVector& invalidation_sets)
    {
        if (invalidation_sets.size() != 1u) {
            return AssertionFailure() << "has " << invalidation_sets.size() << " invalidation set(s), should have 1";
        }
        ...
        if (!classes.Contains(AtomicString(sibling_name_for_sibling_descendant))) {
            return AssertionFailure() << "should sibling invalidate " << sibling_name_for_sibling_descendant;
        }
        if (sibling_invalidation_set.MaxDirectAdjacentSelectors() != SiblingInvalidationSet::kDirectAdjacentMax) {
            return AssertionFailure() << sibling_invalidation_set.MaxDirectAdjacentSelectors() << " should be " << SiblingInvalidationSet::kDirectAdjacentMax;
        }
    ...
    AssertionResult HasSiblingNoDescendantInvalidation(InvalidationSetVector& invalidation_sets)
    {
        if (invalidation_sets.size() != 1u) {
            return AssertionFailure() << "has " << invalidation_sets.size() << " invalidation set(s), should have 1";
        }
    ...
    AssertionResult HasSiblingWholeSubtreeInvalidation(InvalidationSetVector& invalidation_sets)
    {
        if (invalidation_sets.size() != 1u) {
            return AssertionFailure() << "has " << invalidation_sets.size() << " invalidation set(s), should have 1";
        }
        ...
        if (!sibling_invalidation_set.SiblingDescendants()->WholeSubtreeInvalid()) {
            return AssertionFailure() << "sibling descendants should invalidate whole subtree";
        }
        return AssertionSuccess();
    }

    AssertionResult HasIdInvalidation(const char* id, InvalidationSetVector& invalidation_sets)
    {
        if (invalidation_sets.size() != 1u) {
            return AssertionFailure() << "has " << invalidation_sets.size() << " invalidation set(s), should have 1";
        }
    ...
    AssertionResult HasIdInvalidation(const char* first_id, const char* second_id, InvalidationSetVector& invalidation_sets)
    {
        if (invalidation_sets.size() != 1u) {
            return AssertionFailure() << "has " << invalidation_sets.size() << " invalidation set(s), should have 1";
        }
    ...
    AssertionResult HasTagNameInvalidation(const char* tag_name, InvalidationSetVector& invalidation_sets)
    {
        if (invalidation_sets.size() != 1u) {
            return AssertionFailure() << "has " << invalidation_sets.size() << " invalidation set(s), should have 1";
        }
    ...
    AssertionResult HasTagNameInvalidation(const char* first_tag_name, const char* second_tag_name, InvalidationSetVector& invalidation_sets)
    {
        if (invalidation_sets.size() != 1u) {
            return AssertionFailure() << "has " << invalidation_sets.size() << " invalidation set(s), should have 1";
        }
    ...
    AssertionResult HasAttributeInvalidation(const char* attribute, InvalidationSetVector& invalidation_sets)
    {
        if (invalidation_sets.size() != 1u) {
            return AssertionFailure() << "has " << invalidation_sets.size() << " invalidation set(s), should have 1";
        }
    ...
    AssertionResult HasPartsInvalidation(InvalidationSetVector& invalidation_sets)
    {
        if (invalidation_sets.size() != 1u) {
            return AssertionFailure() << "has " << invalidation_sets.size() << " invalidation set(s), should have 1";
        }
    ...
    template <typename MapType, typename KeyType> AssertionResult HasRefCountForInvalidationSet(const MapType& map, const KeyType& key, RefCount ref_count)
    {
        ...
        if (ref_count == RefCount::kOne) {
            ...
            if (it->value->IsSiblingInvalidationSet()) {
                const auto& sibling_invalidation_set = To<SiblingInvalidationSet>(*it->value);
                bool sibling_descendants_has_one_ref
                    = !sibling_invalidation_set.SiblingDescendants() || sibling_invalidation_set.SiblingDescendants()->HasOneRef();
                bool descendants_has_one_ref = !sibling_invalidation_set.Descendants() || sibling_invalidation_set.Descendants()->HasOneRef();
                if (!sibling_descendants_has_one_ref) {
                    return AssertionFailure() << "sibling descendants should have a single ref";
                }
    ...
    AssertionResult HasRefCountForClassInvalidationSet(const RuleFeatureSet& rule_feature_set, const char* class_name, RefCount ref_count)
    {
        return HasRefCountForInvalidationSet(rule_feature_set.GetRuleInvalidationData().class_invalidation_sets, AtomicString(class_name), ref_count);
    }

    AssertionResult HasRefCountForAttributeInvalidationSet(const RuleFeatureSet& rule_feature_set, const char* attribute, RefCount ref_count)
    {
        return HasRefCountForInvalidationSet(rule_feature_set.GetRuleInvalidationData().attribute_invalidation_sets, AtomicString(attribute), ref_count);
    }

    AssertionResult HasRefCountForIdInvalidationSet(const RuleFeatureSet& rule_feature_set, const char* id, RefCount ref_count)
    {
        return HasRefCountForInvalidationSet(rule_feature_set.GetRuleInvalidationData().id_invalidation_sets, AtomicString(id), ref_count);
    }

    AssertionResult HasRefCountForPseudoInvalidationSet(const RuleFeatureSet& rule_feature_set, CSSSelector::PseudoType key, RefCount ref_count)
    {
        return HasRefCountForInvalidationSet(rule_feature_set.GetRuleInvalidationData().pseudo_invalidation_sets, key, ref_count);
    }
...
TEST_F(RuleFeatureSetTest, interleavedDescendantSibling4)
{
    ...
    EXPECT_TRUE(HasNoInvalidation(invalidation_lists.descendants));
    EXPECT_TRUE(HasSiblingDescendantInvalidation(1, "n", "p", invalidation_lists.siblings));
}

TEST_F(RuleFeatureSetTest, interleavedDescendantSibling5)
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".l ~ .m + .n .o + .p"));

    InvalidationLists invalidation_lists;
    CollectInvalidationSetsForClass(invalidation_lists, "l");
    EXPECT_TRUE(HasNoInvalidation(invalidation_lists.descendants));
    EXPECT_TRUE(HasSiblingDescendantInvalidation(SiblingInvalidationSet::kDirectAdjacentMax, "n", "p", invalidation_lists.siblings));
}

TEST_F(RuleFeatureSetTest, interleavedDescendantSibling6)
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".k > .l ~ .m + .n .o + .p"));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(":-webkit-any(.q, .r) ~ .s .t"));

    InvalidationLists invalidation_lists;
    CollectInvalidationSetsForClass(invalidation_lists, "q");
    EXPECT_TRUE(HasNoInvalidation(invalidation_lists.descendants));
    EXPECT_TRUE(HasSiblingDescendantInvalidation(SiblingInvalidationSet::kDirectAdjacentMax, "s", "t", invalidation_lists.siblings));
}

TEST_F(RuleFeatureSetTest, any)
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(":-webkit-any(.w, .x)"));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(":-webkit-any(.v, .w):-webkit-any(.x, .y, .z)"));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".a :-webkit-any(#b, #c)"));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".a :-webkit-any(.v, .w):-webkit-any(.x, .y, .z)"));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".a :-webkit-any(span, div)"));

    InvalidationLists invalidation_lists;
    CollectInvalidationSetsForClass(invalidation_lists, "a");
    EXPECT_TRUE(HasTagNameInvalidation("span", "div", invalidation_lists.descendants));
}

TEST_F(RuleFeatureSetTest, siblingAny)
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".v ~ :-webkit-any(.w, .x)"));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".u .v ~ :-webkit-any(.w, .x)"));
...
TEST_F(RuleFeatureSetTest, attribute)
{
    ...
    InvalidationLists invalidation_lists;
    CollectInvalidationSetsForAttribute(invalidation_lists, QualifiedName(g_empty_atom, AtomicString("c"), g_empty_atom));
...
TEST_F(RuleFeatureSetTest, pseudoClass)
{
    ...
    InvalidationLists invalidation_lists;
    CollectInvalidationSetsForPseudoClass(invalidation_lists, CSSSelector::kPseudoValid);
    EXPECT_TRUE(HasSelfInvalidation(invalidation_lists.descendants));
}
...
TEST_F(RuleFeatureSetTest, nonMatchingHost)
{
    ...
    EXPECT_EQ(SelectorPreMatch::kNeverMatches, CollectFeatures(":host:has(.b):hover .a"));
...
{
    EXPECT_EQ(SelectorPreMatch::kNeverMatches, CollectFeatures(".a:host-context(*)"));
    EXPECT_EQ(SelectorPreMatch::kNeverMatches, CollectFeatures("*:host-context(.a)"));
    EXPECT_EQ(SelectorPreMatch::kNeverMatches, CollectFeatures("*:host-context(*) .a"));
    EXPECT_EQ(SelectorPreMatch::kNeverMatches, CollectFeatures("div :host-context(div) .a"));
    EXPECT_EQ(SelectorPreMatch::kNeverMatches, CollectFeatures(":host-context(div):hover .a"));
    EXPECT_EQ(SelectorPreMatch::kNeverMatches, CollectFeatures(":host-context(div):has(.b):hover .a"));
...
TEST_F(RuleFeatureSetTest, emptyIsWhere)
{
    ...
    EXPECT_EQ(SelectorPreMatch::kNeverMatches, CollectFeatures(":where(:nonsense)"));
    EXPECT_EQ(SelectorPreMatch::kNeverMatches, CollectFeatures(".a:is(:nonsense)"));
    EXPECT_EQ(SelectorPreMatch::kNeverMatches, CollectFeatures(".b:where(:nonsense)"));
}
...
TEST_F(RuleFeatureSetTest, universalSiblingInvalidationDirectAdjacentDescendant)
{
    ...
    EXPECT_TRUE(HasSiblingDescendantInvalidation(1, "a", "b", invalidation_lists.siblings));
...
TEST_F(RuleFeatureSetTest, universalSiblingInvalidationIndirectAdjacent)
{
    ...
    EXPECT_TRUE(HasSiblingClassInvalidation(SiblingInvalidationSet::kDirectAdjacentMax, "a", invalidation_lists.siblings));
    EXPECT_TRUE(HasSelfInvalidation(invalidation_lists.siblings));
}
...
TEST_F(RuleFeatureSetTest, universalSiblingInvalidationMultipleIndirectAdjacent)
{
    ...
    EXPECT_TRUE(HasSiblingClassInvalidation(SiblingInvalidationSet::kDirectAdjacentMax, "b", invalidation_lists.siblings));
    EXPECT_TRUE(HasSelfInvalidation(invalidation_lists.siblings));
}
...
TEST_F(RuleFeatureSetTest, universalSiblingInvalidationIndirectAdjacentDescendant)
{
    ...
    EXPECT_TRUE(HasSiblingDescendantInvalidation(SiblingInvalidationSet::kDirectAdjacentMax, "a", "b", invalidation_lists.siblings));
    EXPECT_TRUE(HasNoSelfInvalidation(invalidation_lists.siblings));
}
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures("#x:-webkit-any(.a) + .b"));
...
TEST_F(RuleFeatureSetTest, nthInvalidationDescendant)
{
    ...
    EXPECT_TRUE(HasNoSelfInvalidation(invalidation_lists.siblings));
    EXPECT_TRUE(HasWholeSubtreeInvalidation(invalidation_lists.siblings));
    EXPECT_TRUE(HasSiblingDescendantInvalidation(SiblingInvalidationSet::kDirectAdjacentMax, "a", invalidation_lists.siblings));
}
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(":nth-child(2n) + .a .b"));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(":not(:nth-child(2n))"));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".a:not(:nth-child(2n))"));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".blah:not(:nth-child(2n)) .a"));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(":-webkit-any(#nomatch, :nth-child(2n))"));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".a:-webkit-any(#nomatch, :nth-child(2n))"));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".blah:-webkit-any(#nomatch, :nth-child(2n)) .a"));
...
TEST_F(RuleFeatureSetTest, SelfInvalidationSet)
{
    ...
    invalidation_lists.descendants.clear();
    CollectInvalidationSetsForAttribute(invalidation_lists, QualifiedName(g_empty_atom, AtomicString("d"), g_empty_atom));
    EXPECT_TRUE(HasSelfInvalidation(invalidation_lists.descendants));
    EXPECT_TRUE(HasSelfInvalidationSet(invalidation_lists.descendants));

    invalidation_lists.descendants.clear();
    CollectInvalidationSetsForPseudoClass(invalidation_lists, CSSSelector::kPseudoHover);
    EXPECT_TRUE(HasSelfInvalidation(invalidation_lists.descendants));
    EXPECT_TRUE(HasSelfInvalidationSet(invalidation_lists.descendants));
}
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(":is(.q, .r) ~ .s .t"));
    {
        InvalidationLists invalidation_lists;
        CollectInvalidationSetsForClass(invalidation_lists, "q");
        EXPECT_TRUE(HasNoInvalidation(invalidation_lists.descendants));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".u .v ~ :is(.w, .x)"));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".a :is(.w+.b, .x>#c)"));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".a :is(.w+.b, .e+:is(.c, #d))"));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(":where(.q, .r) ~ .s .t"));
    {
        InvalidationLists invalidation_lists;
        CollectInvalidationSetsForClass(invalidation_lists, "q");
        EXPECT_TRUE(HasNoInvalidation(invalidation_lists.descendants));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".a :where(span, div)"));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".v ~ :where(.w, .x)"));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".u .v ~ :where(.w, .x)"));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".a :where(.w+.b, .x>#c)"));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".a :where(.w+.b, .e+:where(.c, #d))"));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".a .b::part(partname)"));
...
TEST_F(RuleFeatureSetTest, invalidatesNonTerminalHas)
{
    ...
    {
        InvalidationLists invalidation_lists;
        CollectInvalidationSetsForPseudoClass(invalidation_lists, CSSSelector::kPseudoHas);
        EXPECT_EQ(1u, invalidation_lists.descendants.size());
        EXPECT_TRUE(HasClassInvalidation("d", invalidation_lists.descendants));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(":host .a:has(.b) .c"));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(":host:has(.a):has(.b)"));
...
TEST_F(RuleFeatureSetTest, invalidatesMultipleHasAfterHostAtNonSubjectPosition)
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(":host:has(.a):has(.b) .c"));
...
TEST_F(RuleFeatureSetTest, MediaQueryResultFlagsEquality)
{
    ...
    viewport_dependent.MutableMediaQueryResultFlags().is_viewport_dependent = true;
    ...
    font_unit.MutableMediaQueryResultFlags().unit_flags = MediaQueryExpValue::kFontRelative;

    RuleFeatureSet dynamic_viewport_unit;
    dynamic_viewport_unit.MutableMediaQueryResultFlags().unit_flags = MediaQueryExpValue::kDynamicViewport;
...
RefTestData ref_equal_test_data[] = {
    ...
    // :is
    { ":is(.a)", ".a" }, { ":is(.a .b)", ".a .b" }, { ".a :is(.b .c)", ".a .c, .b .c" }, { ".a + :is(.b .c)", ".a + .c, .b .c" },
    { ".a + :is(.b .c)", ".a + .c, .b .c" }, { "div + :is(.b .c)", "div + .c, .b .c" }, { ":is(.a :is(.b + .c))", ".a .c, .b + .c" },
    { ".a + :is(.b) :is(.c)", ".a + .b .c" }, { ":is(#a:nth-child(1))", "#a:nth-child(1)" },
    { ":is(#a:nth-child(1), #b:nth-child(1))", "#a:nth-child(1), #b:nth-child(1)" }, { ":is(#a, #b):nth-child(1)", "#a:nth-child(1), #b:nth-child(1)" },
    { ":is(:nth-child(1))", ":nth-child(1)" }, { ".a :is(.b, .c):nth-child(1)", ".a .b:nth-child(1), .a .c:nth-child(1)" },
    ...
    // selector in the ref.
    { ".a :is(.b, .c:nth-child(1))", ".a .b, .a .c:nth-child(1), .b:nth-child(1)" }, { ":is(.a) .b", ".a .b" }, { ":is(.a, .b) .c", ".a .c, .b .c" },
    { ":is(.a .b, .c .d) .e", ".a .b .e, .c .d .e" }, { ":is(:is(.a .b, .c) :is(.d, .e .f), .g) .h", ".a .b .h, .c .h, .d .h, .e .f .h, .g .h" },
    { ":is(.a, .b) :is(.c, .d)", ".a .c, .a .d, .b .c, .b .d" },
    { ":is(.a .b, .c .d) :is(.e .f, .g .h)", ".a .b .f, .a .b .h, .c .d .f, .c .d .h, .e .f, .g .h" }, { ":is(.a + .b)", ".a + .b" },
    { ":is(.a + .b, .c + .d) .e", ".a + .b .e, .c + .d .e" },
    { ":is(.a ~ .b, .c + .e + .f) :is(.c .d, .e)", ".a ~ .b .d, .a ~ .b .e, .c + .e + .f .d, .c + .e + .f .e, .c .d" }, { ":is(.a) + .b", ".a + .b" },
    { ":is(.a, .b) + .c", ".a + .c, .b + .c" }, { ":is(.a + .b, .c + .d) + .e", ".a + .b + .e, .c + .d + .e" },
    { ":is(.a + .b, .c + .d) + :is(.e + .f, .g + .h)",
        ".a + .b + .f, .a + .b + .h, .c + .d + .f, .c + .d + .h,"
        ".e + .f, .g + .h" },
    { ":is(div)", "div" }, { ":is(div, span)", "div, span" }, { ":is(.a, div)", ".a, div" }, { ":is(.a, :is(div, span))", ".a, div, span" },
    { ":is(.a, span) :is(div, .b)", ".a div, .a .b, span div, span .b" }, { ":is(.a, span) + :is(div, .b)", ".a + div, .a + .b, span + div, span + .b" },
    { ":is(.a, .b)::slotted(.c)", ".a::slotted(.c), .b::slotted(.c)" }, { ".a :is(.b, .c)::slotted(.d)", ".a .b::slotted(.d), .a .c::slotted(.d)" },
    { ".a + :is(.b, .c)::slotted(.d)", ".a + .b::slotted(.d), .a + .c::slotted(.d)" }, { ".a::slotted(:is(.b, .c))", ".a::slotted(.b), .a::slotted(.c)" },
    { ":is(.a, .b)::cue(i)", ".a::cue(i), .b::cue(i)" }, { ".a :is(.b, .c)::cue(i)", ".a .b::cue(i), .a .c::cue(i)" },
    { ".a + :is(.b, .c)::cue(i)", ".a + .b::cue(i), .a + .c::cue(i)" }, { ".a::cue(:is(.b, .c))", ".a::cue(.b), .a::cue(.c)" },
    { ":is(.a, :host + .b, .c) .d", ".a .d, :host + .b .d, .c .d" }, { ":is(.a, :host(.b) .c, .d) div", ".a div, :host(.b) .c div, .d div" },
    { ".a::host(:is(.b, .c))", ".a::host(.b), .a::host(.c)" }, { ".a :is(.b, .c)::part(foo)", ".a .b::part(foo), .a .c::part(foo)" },
    { ":is(.a, .b)::part(foo)", ".a::part(foo), .b::part(foo)" },
    { ":is(.a, .b) :is(.c, .d)::part(foo)",
        ".a .c::part(foo), .a .d ::part(foo),"
        ".b .c::part(foo), .b .d ::part(foo)" },
    { ":is(.a, .b)::first-letter", ".a::first-letter, .b::first-letter" }, { ":is(.a, .b .c)::first-line", ".a::first-line, .b .c::first-line" },
    ...
    { ":is(.a, .b ~ .c > .d)::first-line", ".a::first-line, .b ~ .c > .d::first-line" }, { ":is(.a, :host-context(.b), .c)", ".a, :host-context(.b), .c" },
    { ":is(.a, :host-context(.b), .c) .d", ".a .d, :host-context(.b) .d, .c .d" },
    { ":is(.a, :host-context(.b), .c) + .d", ".a + .d, :host-context(.b) + .d, .c + .d" },
    { ":host-context(.a) :is(.b, .c)", ":host-context(.a) .b, :host-context(.a) .c" }, { ":host-context(:is(.a))", ":host-context(.a)" },
    { ":host-context(:is(.a, .b))", ":host-context(.a), :host-context(.b)" }, { ":is(.a, .b + .c).d", ".a.d, .b + .c.d" },
    { ".a :is(.b .c .d).e", ".a .d.e, .b .c .d.e" }, { ":is(*)", "*" }, { ".a :is(*)", ".a *" }, { ":is(*) .a", "* .a" }, { ".a + :is(*)", ".a + *" },
    { ":is(*) + .a", "* + .a" }, { ".a + :is(.b, *)", ".a + .b, .a + *" }, { ":is(.a, *) + .b", ".a + .b, * + .b" }, { ".a :is(.b, *)", ".a .b, .a *" },
    { ":is(.a, *) .b", ".a .b, * .b" }, { ":is(.a + .b, .c) *", ".a + .b *, .c *" }, { ":is(.a + *, .c) *", ".a + * *, .c *" },
    { ".a + .b + .c:is(*)", ".a + .b + .c" }, { ".a :not(.b)", ".a *, .b" }, { ".a :not(.b, .c)", ".a *, .b, .c" }, { ".a :not(.b, .c .d)", ".a *, .b, .c .d" },
    { ".a :not(.b, .c + .d)", ".a *, .b, .c + .d" }, { ".a + :not(.b, .c + .d)", ".a + *, .b, .c + .d" }, { ":not(.a .b) .c", ".a .c, .b .c" },
    { ":not(.a .b, .c) + .d", "* + .d, .a .b + .d, .c + .d" },
    { ":not(.a .b, .c .d) :not(.e + .f, .g + .h)", ".a .b *, .c .d *, :not(.e + .f), :not(.g + .h)" }, { ":not(.a, .b)", ":not(.a), :not(.b)" },
    { ":not(.a .b, .c)", ":not(.a .b), :not(.c)" }, { ":not(.a :not(.b + .c), :not(div))", ":not(.a :not(.b + .c)), :not(div)" },
    { ":not(:is(.a))", ":not(.a)" }, { ":not(:is(.a, .b))", ":not(.a), :not(.b)" }, { ":not(:is(.a .b))", ":not(.a .b)" },
    { ":not(:is(.a .b, .c + .d))", ":not(.a .b, .c + .d)" }, { ".a :not(:is(.b .c))", ".a :not(.b .c)" }, { ":not(:is(.a)) .b", ":not(.a) .b" },
    { ":not(:is(.a .b, .c)) :not(:is(.d + .e, .f))", ":not(.a .b, .c) :not(.d + .e, .f)" },
    // We don't have any special support for nested :not(): it's treated
    // as a single :not() level in terms of invalidation:
    { ":not(:not(.a))", ":not(.a)" }, { ":not(:not(:not(.a)))", ":not(.a)" }, { ".a :not(:is(:not(.b), .c))", ".a :not(.b), .a :not(.c)" },
    { ":not(:is(:not(.a), .b)) .c", ":not(.a) .c, :not(.b) .c" }, { ".a :is(:hover)", ".a :hover" }, { ":is(:hover) .a", ":hover .a" },
    { "button:is(:hover, :focus)", "button:hover, button:focus" }, { ".a :is(.b, :hover)", ".a .b, .a :hover" },
    { ".a + :is(:hover) + .c", ".a + :hover + .c" }, { ".a + :is(.b, :hover) + .c", ".a + .b + .c, .a + :hover + .c" },
    { ":is(ol, li)::before", "ol::before, li::before" }, { ":is(.a + .b, .c)::before", ".a + .b::before, .c::before" },
    { ":is(ol, li)::-internal-input-suggested", "ol::-internal-input-suggested, li::-internal-input-suggested" }, { ":is([foo], [bar])", "[foo], [bar]" },
    { ".a :is([foo], [bar])", ".a [foo], .a [bar]" }, { ":is([foo], [bar]) .a", "[foo] .a, [bar] .a" },
    { ":is([a], [b]) :is([c], [d])", "[a] [c], [a] [d], [b] [c], [b] [d]" },

    { "", "div" }, { "", "::before" }, { "", ":host" }, { "", "*" }, { "ol", "ul" }, { "::cue(a)", "::cue(b)" }, { "div", "span" },
    // clang-format on
};
...
public:
    ...
    virtual void CollectTo(
        const char*, RuleFeatureSet&, CSSNestingType nesting_type = CSSNestingType::kNone, StyleRule* parent_rule_for_nesting = nullptr) const
        = 0;
...
public:
    void CollectTo(
        const char* text, RuleFeatureSet& set, CSSNestingType nesting_type = CSSNestingType::kNone, StyleRule* parent_rule_for_nesting = nullptr) const override
    {
        CollectFeaturesTo(text, set, nesting_type, parent_rule_for_nesting);
    }
};

class RuleFeatureSetRefEqualTest : public RuleFeatureSetSelectorRefTest, public testing::WithParamInterface<RefTestData> {
public:
    void Compare(const RuleFeatureSet& main, const RuleFeatureSet& ref) const override
    {
        EXPECT_EQ(main, ref);
    }
};

INSTANTIATE_TEST_SUITE_P(RuleFeatureSetTest, RuleFeatureSetRefEqualTest, testing::ValuesIn(ref_equal_test_data));
...
class RuleFeatureSetRefNotEqualTest : public RuleFeatureSetSelectorRefTest, public testing::WithParamInterface<RefTestData> {
public:
    void Compare(const RuleFeatureSet& main, const RuleFeatureSet& ref) const override
    {
        EXPECT_NE(main, ref);
...
INSTANTIATE_TEST_SUITE_P(RuleFeatureSetTest, RuleFeatureSetRefNotEqualTest, testing::ValuesIn(ref_not_equal_test_data));
...
RefTestData ref_scope_equal_test_data[] = {
    ...
    { "@scope (.a, .b) { .c + .d {} }", ".c + .d, :is(.a, .b) .d, :is(.a, .b) {}" },
    { "@scope (.a) { .b ~ :scope {} }", ".b ~ *, .a *, .a {}" },
    { "@scope (.a) { .b + :scope {} }", ".b + *, .a *, .a {}" },

    // Multiple items in selector lists:
    { "@scope (.a, .b) { div {} }", ":is(.a, .b) div, :is(.a, .b):is(div) {}" },
    { "@scope (.a, :is(.b, .c)) { div {} }", ":is(.a, .b, .c) div, :is(.a, .b, .c):is(div) {}" },

    // Using "to" keyword:
    { "@scope (.a, .b) to (.c, .d) { div {} }", ":is(.a, .b, .c, .d) div, :is(.a, .b):is(.c, .d):is(div) {}" },
    ...
    { "@scope (.a, .b) { @scope (.c, :scope .d) { .e {} } }", ":is(.a, .b):is(.c, .d) .e, :is(.a, .b):is(.c, .d):is(.e) {}" },
    ...
    { "@scope (.a, .b) { @scope (.c, & .d) { .e {} } }", ":is(.a, .b, .c, .d) .e, :is(.a, .b), :is(.c, .d) {}" },
    { "@scope (.a) to (&) { .b {} }", ".a .b, .a {}" },
    { "@scope (.a) to (&) { & {} }", ".a .a {}" },

    // Nested @scopes
    { "@scope (.a, .b) { @scope (.c, .d) { .e {} } }", ":is(.a, .b, .c, .d) .e, :is(.a, .b, .c, .d):is(.e) {}" },
    { "@scope (.a, .b) { @scope (.c, .d) { :scope {} } }", ":is(.a, .b, .c, .d) *, :is(.a, .b, .c, .d) {}" },
    { "@scope (.a, .b) { @scope (:scope, .c) { :scope {} } }", ":is(.a, .b, .c) *, :is(.a, .b, .c) {}" },
    { "@scope (.a) to (.b) { @scope (.c) to (.d) { .e {} } }", ":is(.a, .b, .c, .d) .e, :is(.a, .b):is(.c, .d):is(.e) {}" },
};

class RuleFeatureSetScopeRefTest : public RuleFeatureSetRefTest, public testing::WithParamInterface<RefTestData> {
public:
    void CollectTo(
        const char* text, RuleFeatureSet& set, CSSNestingType nesting_type = CSSNestingType::kNone, StyleRule* parent_rule_for_nesting = nullptr) const override
    {
        ...
        while (IsA<StyleRuleScope>(rule)) {
            ...
            const HeapVector<Member<StyleRuleBase>>& child_rules = scope_rule.ChildRules();
...
INSTANTIATE_TEST_SUITE_P(SelectorChecker, RuleFeatureSetScopeRefTest, testing::ValuesIn(ref_scope_equal_test_data));
...
TEST_F(RuleFeatureSetTest, CopyOnWrite)
{
    ...
    EXPECT_TRUE(HasRefCountForAttributeInvalidationSet(local1, "thing", RefCount::kOne));
    EXPECT_TRUE(HasRefCountForPseudoInvalidationSet(local1, CSSSelector::kPseudoHover, RefCount::kOne));
    ...
    MergeInto(local3);
    ClearFeatures();
    EXPECT_TRUE(HasRefCountForAttributeInvalidationSet(local3, "thing", RefCount::kOne));
    EXPECT_TRUE(HasRefCountForPseudoInvalidationSet(local3, CSSSelector::kPseudoHover, RefCount::kOne));
    ...
    EXPECT_TRUE(HasRefCountForAttributeInvalidationSet(global, "thing", RefCount::kMany));
    EXPECT_TRUE(HasRefCountForPseudoInvalidationSet(global, CSSSelector::kPseudoHover, RefCount::kMany));
    ...
    EXPECT_TRUE(HasRefCountForClassInvalidationSet(global, "a", RefCount::kOne));
    EXPECT_TRUE(HasRefCountForIdInvalidationSet(global, "d", RefCount::kOne));
    EXPECT_TRUE(HasRefCountForAttributeInvalidationSet(global, "thing", RefCount::kOne));
    EXPECT_TRUE(HasRefCountForPseudoInvalidationSet(global, CSSSelector::kPseudoHover, RefCount::kOne));
}
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".a:has(~ :is(.b .c ~ .d))"));
...
TEST_F(RuleFeatureSetTest, isPseudoContainingComplexInsideHas7)
{
...

    {
        ...
        CollectInvalidationSetsForClass(invalidation_lists, "b");
        EXPECT_TRUE(HasNoInvalidation(invalidation_lists.descendants));
        EXPECT_TRUE(HasSiblingAndSiblingDescendantInvalidationForLogicalCombinationsInHas("a", "c", "a", invalidation_lists.siblings));
    }
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".a:has(:is(:is(.b, .c) .d))"));
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".a:has(~ :is(.b ~ .c .d ~ .e))"));
...
TEST_F(RuleFeatureSetTest, isPseudoContainingComplexInsideHas14)
{
...

    {
        InvalidationLists invalidation_lists;
        CollectInvalidationSetsForClass(invalidation_lists, "b");
        EXPECT_TRUE(HasSiblingDescendantInvalidation(SiblingInvalidationSet::kDirectAdjacentMax, "a", "d", invalidation_lists.siblings));
        EXPECT_TRUE(HasNoSelfInvalidation(invalidation_lists.siblings));
    }
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".a:has(~ :is(* ~ .b))"));
    {
        InvalidationLists invalidation_lists;
        CollectUniversalSiblingInvalidationSet(invalidation_lists);
...
TEST_F(RuleFeatureSetTest, isPseudoContainingComplexInsideHas16)
{
...

    {
        ...
        EXPECT_TRUE(HasSiblingDescendantInvalidation(SiblingInvalidationSet::kDirectAdjacentMax, "a", "c", invalidation_lists.siblings));
        EXPECT_TRUE(HasNoSelfInvalidation(invalidation_lists.siblings));
    }
...
{
    EXPECT_EQ(SelectorPreMatch::kMayMatch, CollectFeatures(".a:has(~ :is(.b ~ :is(.c ~ .d)))"));
...
TEST_F(RuleFeatureSetTest, NestedSelector)
{
    ...
    HeapVector<CSSSelector> arena;
    base::span<CSSSelector> selector_vector = CSSParser::ParseSelector(StrictCSSParserContext(SecureContextMode::kInsecureContext), CSSNestingType::kNone,
        /*parent_rule_for_nesting=*/nullptr, /*is_within_scope=*/false, nullptr, ".a, .b", arena);
    auto* parent_rule = StyleRule::Create(selector_vector, MakeGarbageCollected<MutableCSSPropertyValueSet>(kHTMLStandardMode));
...
{
    Document* document = Document::CreateForTest(execution_context_.GetExecutionContext());
    StyleRuleBase* parent_rule_base = css_test_helpers::ParseRule(*document, "@scope (.a) { :scope {} }");
    ...
    scope = scope_rule.GetStyleScope().CopyWithParent(scope);
    const HeapVector<Member<StyleRuleBase>>& scope_child_rules = scope_rule.ChildRules();
