...
public:
    ...
    {
        const CSSPrimitiveValue* value = To<CSSPrimitiveValue>(css_test_helpers::ParseValue(GetDocument(), "<length>", text));
        DCHECK(value);
        return value;
    }
...
CSSPrimitiveValue* CreateAddition(UnitValue a, UnitValue b)
{
    return CSSMathFunctionValue::Create(CSSMathExpressionOperation::CreateArithmeticOperation(
        CSSMathExpressionNumericLiteral::Create(Create(a)), CSSMathExpressionNumericLiteral::Create(Create(b)), CSSMathOperator::kAdd));
}

CSSPrimitiveValue* CreateNonNegativeSubtraction(UnitValue a, UnitValue b)
{
    return CSSMathFunctionValue::Create(CSSMathExpressionOperation::CreateArithmeticOperation(CSSMathExpressionNumericLiteral::Create(Create(a)),
                                            CSSMathExpressionNumericLiteral::Create(Create(b)), CSSMathOperator::kSubtract),
...
TEST_F(CSSPrimitiveValueTest, Zooming)
{
    ...
    CSSPrimitiveValue* converted = CSSPrimitiveValue::CreateFromLength(length, conversion_data.Zoom());
...
{
    CSSPrimitiveValue* value = Create({ std::numeric_limits<double>::infinity(), UnitType::kPercentage });
    CSSToLengthConversionData conversion_data;
...
{
    CSSPrimitiveValue* value = Create({ -std::numeric_limits<double>::quiet_NaN(), UnitType::kPercentage });
    CSSToLengthConversionData conversion_data;
    Length length = value->ConvertToLength(conversion_data);
...
{
    CSSPrimitiveValue* value = Create({ std::numeric_limits<double>::quiet_NaN(), UnitType::kPixels });
...
{
    CSSPrimitiveValue* value = Create({ std::numeric_limits<double>::infinity(), UnitType::kPixels });
...
TEST_F(CSSPrimitiveValueTest, ContainerProgressTreeScope)
{
    ...
    const CSSValue* value = css_test_helpers::ParseValue(GetDocument(), "<number>", "container-progress(width of my-container from 0px to 1px)");
    ASSERT_TRUE(value);
    ...
    // Don't crash:
    const CSSValue& scoped_value2 = scoped_value.EnsureScopedValue(&GetDocument());
...
TEST_F(CSSPrimitiveValueTest, CSSPrimitiveValueOperations)
{
    ...
    auto* node_subtract = CSSMathExpressionOperation::CreateArithmeticOperation(node_10_px, node_20_em, CSSMathOperator::kSubtract);
    ...
    EXPECT_EQ(function->Multiply(1, CSSPrimitiveValue::UnitType::kPixels)->Add(10, CSSPrimitiveValue::UnitType::kPixels)->CustomCSSText(),
        "calc(10px + sign(-20em + 10px) * 1px)");
    EXPECT_EQ(function->MultiplyBy(10, CSSPrimitiveValue::UnitType::kNumber)->CustomCSSText(), "calc(10 * sign(-20em + 10px))");
    EXPECT_EQ(
        function->MultiplyBy(1, CSSPrimitiveValue::UnitType::kPixels)->Subtract(*numeric_percentage)->CustomCSSText(), "calc(-10% + 1px * sign(-20em + 10px))");
    EXPECT_EQ(function->Divide(20, CSSPrimitiveValue::UnitType::kNumber)->CustomCSSText(), "calc(sign(-20em + 10px) / 20)");
    EXPECT_EQ(function->Subtract(*function)->CustomCSSText(), "calc(sign(-20em + 10px) - sign(-20em + 10px))");
    EXPECT_EQ(numeric_percentage->SubtractFrom(10, CSSPrimitiveValue::UnitType::kPixels)->CustomCSSText(), "calc(-10% + 10px)");
...
{
    CSSNumericLiteralValue* numeric_percentage = CSSNumericLiteralValue::Create(10, CSSPrimitiveValue::UnitType::kPercentage);
    CSSMathExpressionNode* node_20_px = CSSMathExpressionNumericLiteral::Create(20, CSSPrimitiveValue::UnitType::kPixels);
    CSSMathExpressionNode* node_2_em = CSSMathExpressionNumericLiteral::Create(2, CSSPrimitiveValue::UnitType::kEms);
    CSSMathExpressionNode* node_sub = CSSMathExpressionOperation::CreateArithmeticOperation(node_20_px, node_2_em, CSSMathOperator::kSubtract);
