...
{
    return type.empty() || (element.IsHTMLElement() ? EqualIgnoringASCIICase(type, "text/css") : (type == "text/css"));
}
...
{
    if (created_by_parser && document && document->GetScriptableDocumentParser() && !document->IsInDocumentWrite()) {
        start_position_ = document->GetScriptableDocumentParser()->GetTextPosition();
    }
}
...
void StyleElement::RemovedFrom(Element& element, ContainerNode& insertion_point)
{
    ...
    if (registered_as_candidate_) {
        document.GetStyleEngine().RemoveStyleSheetCandidateNode(element, insertion_point);
        registered_as_candidate_ = false;
    }
...
void StyleElement::ClearSheet(Element& owner_element)
{
    ...
    if (sheet_->IsLoading()) {
        ...
        if (pending_sheet_type_ != PendingSheetType::kNonBlocking) {
            owner_element.GetDocument().GetStyleEngine().RemovePendingBlockingSheet(owner_element, pending_sheet_type_);
        }
        pending_sheet_type_ = PendingSheetType::kNone;
    }
...
StyleElement::ProcessingResult StyleElement::CreateSheet(Element& element, const String& text)
{
    ...
    ContentSecurityPolicy* csp = element.GetExecutionContext() ? element.GetExecutionContext()->GetContentSecurityPolicyForCurrentWorld() : nullptr;

    // CSP is bypassed for style elements in user agent shadow DOM.
    bool passes_content_security_policy_checks = IsInUserAgentShadowDOM(element)
        || (csp && csp->AllowInline(ContentSecurityPolicy::InlineType::kStyle, &element, text, element.nonce(), document.Url(), start_position_.line_));
    ...
    if (IsCSS(element, type) && passes_content_security_policy_checks) {
        ...
        if (!media_string.empty()) {
            media_queries = MediaQuerySet::Create(media_string, element.GetExecutionContext());
        ...
        auto type_and_behavior = ComputePendingSheetTypeAndRenderBlockingBehavior(element, media_query_matches, created_by_parser_);
        pending_sheet_type_ = type_and_behavior.first;
        render_blocking_behavior_ = type_and_behavior.second;

        loading_ = true;
        TextPosition start_position = start_position_ == TextPosition::BelowRangePosition() ? TextPosition::MinimumPosition() : start_position_;
        new_sheet = document.GetStyleEngine().CreateSheet(element, text, start_position, pending_sheet_type_, render_blocking_behavior_);
    ...
    return passes_content_security_policy_checks ? kProcessingSuccessful : kProcessingFatalError;
}
...
bool StyleElement::SheetLoaded(Document& document)
{
    ...
    if (pending_sheet_type_ != PendingSheetType::kNonBlocking) {
        document.GetStyleEngine().RemovePendingBlockingSheet(*sheet_->ownerNode(), pending_sheet_type_);
    }
    document.GetStyleEngine().SetNeedsActiveStyleUpdate(sheet_->ownerNode()->GetTreeScope());
    pending_sheet_type_ = PendingSheetType::kNone;
    return true;
}
...
void StyleElement::BlockingAttributeChanged(Element& element)
{
    ...
    if (const auto* html_element = DynamicTo<HTMLElement>(element); !html_element || html_element->IsPotentiallyRenderBlocking()) {
        return;
    }
    element.GetDocument().GetStyleEngine().RemovePendingBlockingSheet(element, pending_sheet_type_);
