...
Vector<unsigned> CollectIdentifierHashesFromInnerRule(Document& document, String rule_text)
{
    Vector<unsigned> result;
    const auto* outer_rule = DynamicTo<StyleRuleGroup>(css_test_helpers::ParseRule(document, rule_text));
    CHECK(outer_rule);
    CHECK_EQ(1u, outer_rule->ChildRules().size());

    const auto* inner_style_rule = DynamicTo<StyleRule>(outer_rule->ChildRules()[0].Get());
    CHECK(inner_style_rule);
    CHECK(inner_style_rule->FirstSelector());

    const auto* scope_rule = DynamicTo<StyleRuleScope>(outer_rule);
    const StyleScope* style_scope = scope_rule ? &scope_rule->GetStyleScope() : nullptr;

    SelectorFilter::CollectIdentifierHashes(*inner_style_rule->FirstSelector(), style_scope, result);
    return result;
}
