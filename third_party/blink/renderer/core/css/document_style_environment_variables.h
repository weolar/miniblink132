...
class CORE_EXPORT DocumentStyleEnvironmentVariables : public StyleEnvironmentVariables {
public:
    // Create an instance bound to |parent| that will invalidate |document|'s
    // style when a variable is changed.
    DocumentStyleEnvironmentVariables(StyleEnvironmentVariables& parent, Document& document);
    ...
    CSSVariableData* ResolveVariable(const AtomicString& name, WTF::Vector<unsigned> indices, bool record_metrics);

    // Resolve the variable |name| and return the data. This will also cause
    // future changes to this variable to invalidate the associated document's
    // style. UseCounter metrics will be recorded when this function is used.
    CSSVariableData* ResolveVariable(const AtomicString& name, WTF::Vector<unsigned> indices) override;
