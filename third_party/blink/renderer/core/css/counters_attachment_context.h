...
class CORE_EXPORT CountersAttachmentContext {
    ...
    using CounterInheritanceTable = HeapHashMap<AtomicString, Member<CounterStack>>;
...
public:
    ...
    CountersAttachmentContext& operator=(const CountersAttachmentContext&) = delete;
    ...
    Vector<int> GetCounterValues(const LayoutObject&, const AtomicString& counter_name, bool only_last);
...
private:
    ...
    void ProcessCounter(const LayoutObject& layout_object, const AtomicString& counter_name, unsigned counter_type, int value_argument, bool is_page_box);
    ...
    // return true.
    bool ObscurePageCounterIfNeeded(
    ...
    void UnobscurePageCounterIfNeeded(const AtomicString& counter_name, unsigned counter_type, bool is_page_box);

    void CreateCounter(const LayoutObject&, const AtomicString& counter_name, int value);
    void RemoveStaleCounters(const LayoutObject&, const AtomicString& counter_name);
    void RemoveCounterIfAncestorExists(const LayoutObject&, const AtomicString& counter_name);
    void UpdateCounterValue(const LayoutObject&, const AtomicString& counter_name, unsigned counter_type, int counter_value);
