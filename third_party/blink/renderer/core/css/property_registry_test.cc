...
public:
    ...
    const PropertyRegistration* RegisterProperty(const char* name, String syntax = "*", String initial_value = g_null_atom)
    {
        auto* registration = css_test_helpers::CreatePropertyRegistration(name, syntax, MaybeParseInitialValue(syntax, initial_value));
        Registry().RegisterProperty(AtomicString(name), *registration);
        return registration;
    }

    const PropertyRegistration* DeclareProperty(const char* name, String syntax = "*", String initial_value = g_null_atom)
    {
        auto* registration = css_test_helpers::CreatePropertyRegistration(name, syntax, MaybeParseInitialValue(syntax, initial_value));
...
TEST_F(PropertyRegistryTest, MarkReferencedAtProperty)
{
    ...
    css_test_helpers::RegisterProperty(GetDocument(), "--x", "<length>", "1px", false);
...
{
    EXPECT_EQ(0u, RegisterProperty("--px", "<length>", "1px")->GetViewportUnitFlags());
    EXPECT_EQ(static_cast<unsigned>(ViewportUnitFlag::kStatic), RegisterProperty("--vh", "<length>", "1vh")->GetViewportUnitFlags());
    EXPECT_EQ(static_cast<unsigned>(ViewportUnitFlag::kStatic), RegisterProperty("--svh", "<length>", "1svh")->GetViewportUnitFlags());
    EXPECT_EQ(static_cast<unsigned>(ViewportUnitFlag::kStatic), RegisterProperty("--lvh", "<length>", "1lvh")->GetViewportUnitFlags());
    EXPECT_EQ(static_cast<unsigned>(ViewportUnitFlag::kDynamic), RegisterProperty("--dvh", "<length>", "1dvh")->GetViewportUnitFlags());
    EXPECT_EQ(static_cast<unsigned>(ViewportUnitFlag::kStatic) | static_cast<unsigned>(ViewportUnitFlag::kDynamic),
        RegisterProperty("--mixed", "<length>", "calc(1dvh + 1svh)")->GetViewportUnitFlags());
}

TEST_F(PropertyRegistryTest, GetViewportUnitFlagsDeclared)
{
    EXPECT_EQ(0u, DeclareProperty("--px", "<length>", "1px")->GetViewportUnitFlags());
    EXPECT_EQ(static_cast<unsigned>(ViewportUnitFlag::kStatic), DeclareProperty("--vh", "<length>", "1vh")->GetViewportUnitFlags());
    EXPECT_EQ(static_cast<unsigned>(ViewportUnitFlag::kStatic), DeclareProperty("--svh", "<length>", "1svh")->GetViewportUnitFlags());
    EXPECT_EQ(static_cast<unsigned>(ViewportUnitFlag::kStatic), DeclareProperty("--lvh", "<length>", "1lvh")->GetViewportUnitFlags());
    EXPECT_EQ(static_cast<unsigned>(ViewportUnitFlag::kDynamic), DeclareProperty("--dvh", "<length>", "1dvh")->GetViewportUnitFlags());
    EXPECT_EQ(static_cast<unsigned>(ViewportUnitFlag::kStatic) | static_cast<unsigned>(ViewportUnitFlag::kDynamic),
        DeclareProperty("--mixed", "<length>", "calc(1dvh + 1svh)")->GetViewportUnitFlags());
}
...
TEST_F(PropertyRegistryTest, GetViewportUnitFlagsRegistry)
{
    ...
    EXPECT_EQ(static_cast<unsigned>(ViewportUnitFlag::kStatic), Registry().GetViewportUnitFlags());

    DeclareProperty("--dvh", "<length>", "1dvh");
    EXPECT_EQ(static_cast<unsigned>(ViewportUnitFlag::kStatic) | static_cast<unsigned>(ViewportUnitFlag::kDynamic), Registry().GetViewportUnitFlags());
