...
CSSMarginRule::CSSMarginRule(StyleRulePageMargin* margin_rule, CSSStyleSheet* parent)
    : CSSRule(parent)
    , margin_rule_(margin_rule)
{
}
...
CSSStyleDeclaration* CSSMarginRule::style() const
{
    if (!properties_cssom_wrapper_) {
        properties_cssom_wrapper_ = MakeGarbageCollected<StyleRuleCSSStyleDeclaration>(margin_rule_->MutableProperties(), const_cast<CSSMarginRule*>(this));
    }
