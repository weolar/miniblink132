...
bool RemoteFontFaceSource::NeedsInterventionToAlignWithLCPGoal() const
{
    ...
    if (!GetDocument() || !FontFaceSetDocument::From(*GetDocument())->HasReachedLCPLimit()) {
        return false;
    }
    ...
    return !IsLoaded() || (!FinishedFromMemoryCache() && !finished_before_lcp_limit_);
}

RemoteFontFaceSource::DisplayPeriod RemoteFontFaceSource::ComputeFontDisplayAutoPeriod() const
{
    DCHECK_EQ(display_, FontDisplay::kAuto);
...
RemoteFontFaceSource::DisplayPeriod RemoteFontFaceSource::ComputePeriod() const
{
    switch (display_) {
    ...
    case FontDisplay::kOptional: {
        ...
        if (GetDocument()->RenderingHasBegun()) {
            if (FinishedFromMemoryCache() || finished_before_document_rendering_begin_ || !paint_requested_while_pending_) {
                return kSwapPeriod;
            }
            return kFailurePeriod;
        }
...
RemoteFontFaceSource::RemoteFontFaceSource(
    CSSFontFace* css_font_face, FontSelector* font_selector, FontDisplay display, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    : face_(css_font_face)
...
{
    auto* window = DynamicTo<LocalDOMWindow>(font_selector_->GetExecutionContext());
...
void RemoteFontFaceSource::NotifyFinished(Resource* resource)
{
    ...
    if (font->GetStatus() == ResourceStatus::kDecodeError) {
        execution_context->AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(
            mojom::ConsoleMessageSource::kOther, mojom::ConsoleMessageLevel::kWarning, "Failed to decode downloaded font: " + font->Url().ElidedString()));
        if (!font->OtsParsingMessage().empty()) {
            execution_context->AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(
                mojom::ConsoleMessageSource::kOther, mojom::ConsoleMessageLevel::kWarning, "OTS parsing error: " + font->OtsParsingMessage()));
        }
    }
    ...
    if (face_->FontLoaded(this)) {
        ...
        if (custom_font_data_) {
            probe::FontsUpdated(execution_context, face_->GetFontFace(), resource->Url().GetString(), custom_font_data_.Get());
        }
    }
}
...
bool RemoteFontFaceSource::UpdatePeriod()
{
    ...
    if (IsLoading() && period_ != new_period && (period_ == kBlockPeriod || new_period == kBlockPeriod)) {
        ClearTable();
        if (face_->FallbackVisibilityChanged(this)) {
            font_selector_->FontFaceInvalidated(FontInvalidationReason::kGeneralInvalidation);
        }
...
bool RemoteFontFaceSource::ShouldTriggerWebFontsIntervention()
{
    ...
    WebEffectiveConnectionType connection_type = GetNetworkStateNotifier().EffectiveType();

    bool network_is_slow = WebEffectiveConnectionType::kTypeOffline <= connection_type && connection_type <= WebEffectiveConnectionType::kType3G;
...
const SimpleFontData* RemoteFontFaceSource::CreateFontData(
    const FontDescription& font_description, const FontSelectionCapabilities& font_selection_capabilities)
{
    ...
    return MakeGarbageCollected<SimpleFontData>(
        custom_font_data_->GetFontPlatformData(font_description.EffectiveFontSize(), font_description.AdjustedSpecifiedSize(),
            font_description.IsSyntheticBold() && font_description.SyntheticBoldAllowed(),
            font_description.IsSyntheticItalic() && font_description.SyntheticItalicAllowed(), font_description.GetFontSelectionRequest(),
            font_selection_capabilities, font_description.FontOpticalSizing(), font_description.TextRendering(),
            ...
            font_description.Orientation(), font_description.VariationSettings(), font_description.GetFontPalette()),
        MakeGarbageCollected<CustomFontData>());
}

const SimpleFontData* RemoteFontFaceSource::CreateLoadingFallbackFontData(const FontDescription& font_description)
{
    ...
    CSSCustomFontData* css_font_data
        = MakeGarbageCollected<CSSCustomFontData>(this, period_ == kBlockPeriod ? CSSCustomFontData::kInvisibleFallback : CSSCustomFontData::kVisibleFallback);
    return MakeGarbageCollected<SimpleFontData>(&temporary_font->PlatformData(), css_font_data);
}
...
void RemoteFontFaceSource::BeginLoadIfNeeded()
{
    ...
    ExecutionContext* const execution_context = font_selector_->GetExecutionContext();
    if (!execution_context) {
        return;
    }
    ...
    if (font->StillNeedsLoad()) {
        TRACE_EVENT("devtools.timeline", "BeginRemoteFontLoad", "id", font->InspectorId(), "display", face_->GetFontFace()->display());
        if (font->IsLowPriorityLoadingAllowedForRemoteFont()) {
            execution_context->AddConsoleMessage(
                MakeGarbageCollected<ConsoleMessage>(mojom::blink::ConsoleMessageSource::kIntervention, mojom::blink::ConsoleMessageLevel::kInfo,
    ...
    // off the timers.
    font->StartLoadLimitTimersIfNecessary(execution_context->GetTaskRunner(TaskType::kInternalLoading).get());
...
void RemoteFontFaceSource::FontLoadHistograms::MaySetDataSource(DataSource data_source)
{
...
void RemoteFontFaceSource::FontLoadHistograms::RecordLoadTimeHistogram(const FontResource* font, base::TimeDelta delta)
{
