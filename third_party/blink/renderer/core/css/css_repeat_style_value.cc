...
CSSRepeatStyleValue::CSSRepeatStyleValue(const CSSIdentifierValue* x, const CSSIdentifierValue* y)
    : CSSValue(kRepeatStyleClass)
    , x_(x)
    , y_(y)
{
...
{
    return base::ValuesEquivalent(x_, other.x_) && base::ValuesEquivalent(y_, other.y_);
}

bool CSSRepeatStyleValue::IsRepeat() const
{
    return x_->GetValueID() == CSSValueID::kRepeat && y_->GetValueID() == CSSValueID::kRepeat;
}
