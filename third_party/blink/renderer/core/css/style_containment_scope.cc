...
StyleContainmentScope::StyleContainmentScope(const Element* element, StyleContainmentScopeTree* style_containment_tree)
    : element_(element)
    , parent_(nullptr)
...
bool StyleContainmentScope::IsAncestorOf(const Element* element, const Element* stay_within)
{
    for (const Element* it = element; it && it != stay_within; it = LayoutTreeBuilderTraversal::ParentElement(*it)) {
        if (it == GetElement()) {
            return true;
        }
    }
...
// Element*.
const LayoutQuote* StyleContainmentScope::FindQuotePrecedingElement(const Element& element) const
{
    ...
    auto comp = [](const Element& element, const LayoutQuote* quote) {
        return LayoutTreeBuilderTraversal::ComparePreorderTreePosition(element, *quote->GetOwningPseudo()) < 0;
    };
...
int StyleContainmentScope::ComputeInitialQuoteDepth() const
{
    ...
    for (StyleContainmentScope* parent = parent_; parent; parent = parent->Parent()) {
        const LayoutQuote* parent_quote = parent->FindQuotePrecedingElement(*quotes_.front()->GetOwningPseudo());
        if (parent_quote) {
