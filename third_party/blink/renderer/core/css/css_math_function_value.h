...
public:
    ...
    static CSSMathFunctionValue* Create(const CSSMathExpressionNode*, ValueRange = ValueRange::kAll);

    CSSMathFunctionValue(const CSSMathExpressionNode* expression, ValueRange range);
    ...
    scoped_refptr<const CalculationValue> ToCalcValue(const CSSLengthResolver&) const;

    bool MayHaveRelativeUnit() const;

    CalculationResultCategory Category() const
    {
        return expression_->Category();
    }

    bool IsAngle() const
    {
        return Category() == kCalcAngle;
    }
    bool IsLength() const
    {
    ...
    bool IsTime() const
    {
        return Category() == kCalcTime;
    }
    bool IsResolution() const
    {
        return Category() == kCalcResolution;
    }
    ...
    {
        return expression_->HasComparisons();
    }
    ...
    {
        return expression_->HasAnchorFunctions();
    }
    ...
    const CSSMathFunctionValue* TransformAnchors(LogicalAxis, const TryTacticTransform&, const WritingDirectionMode&) const;
...
template <> struct DowncastTraits<CSSMathFunctionValue> {
