...
protected:
    ...
    void SetUp() override
    {
        ...
        GetDocument().GetSettings()->SetPreferredColorScheme(mojom::blink::PreferredColorScheme::kDark);
    }
};
...
TEST_F(ForceDarkTest, ForcedColorScheme)
{
    ...
    auto run_test = [&document = GetDocument()](const TestCase& test_case) {
        ...
        ASSERT_TRUE(style);
        EXPECT_EQ(test_case.expected_dark, style->DarkColorScheme()) << "Element #" << test_case.id;
        EXPECT_EQ(test_case.expected_forced, style->ColorSchemeForced()) << "Element #" << test_case.id;

        const auto* child_style = element->firstElementChild()->GetComputedStyle();
        ASSERT_TRUE(child_style);
        EXPECT_EQ(test_case.expected_dark, child_style->DarkColorScheme()) << "Element #" << test_case.id << " > span";
        EXPECT_EQ(test_case.expected_forced, child_style->ColorSchemeForced()) << "Element #" << test_case.id << " > span";
    };
    ...
    GetDocument().GetSettings()->SetPreferredColorScheme(mojom::blink::PreferredColorScheme::kLight);
...
TEST_F(ForceDarkTest, ForcedColorSchemeInvalidation)
{
    ...
    auto run_test = [&document = GetDocument()](const TestCase& test_case) {
        ...
        EXPECT_EQ(test_case.expected_dark, style->DarkColorScheme()) << "Element #" << test_case.id;
        EXPECT_EQ(test_case.expected_forced, style->ColorSchemeForced()) << "Element #" << test_case.id;
        EXPECT_EQ(test_case.expected_repaint, element->GetLayoutObject()->ShouldDoFullPaintInvalidation()) << "Element #" << test_case.id;
    };
