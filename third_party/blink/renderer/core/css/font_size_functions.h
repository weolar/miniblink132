...
enum ApplyMinimumFontSize { kDoNotApplyMinimumForFontSize, kApplyMinimumForFontSize };
...
public:
    static float GetComputedSizeFromSpecifiedSize(
        const Document*, float zoom_factor, bool is_absolute_size, float specified_size, ApplyMinimumFontSize = kApplyMinimumForFontSize);
    ...
    {
        return (value_id >= CSSValueID::kXxSmall && value_id <= CSSValueID::kXxxLarge) || value_id == CSSValueID::kWebkitXxxLarge;
    }

    static CSSValueID InitialValueID()
    {
        return CSSValueID::kMedium;
    }
    static unsigned InitialKeywordSize()
    {
        return KeywordSize(InitialValueID());
    }
    ...
    static float FontSizeForKeyword(const Document*, unsigned keyword, bool is_monospace);

    // Given a font size in pixel, this function will return legacy font size
    // between 1 and 7.
    static int LegacyFontSize(const Document*, int pixel_font_size, bool is_monospace);
