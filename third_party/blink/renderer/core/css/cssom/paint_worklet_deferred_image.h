...
public:
    static scoped_refptr<PaintWorkletDeferredImage> Create(scoped_refptr<PaintWorkletInput> input, const gfx::SizeF& size)
    {
...
protected:
    ...
    void DrawTile(cc::PaintCanvas*, const gfx::RectF&, const ImageDrawOptions&) override;
    sk_sp<cc::PaintShader> CreateShader(const gfx::RectF& tile_rect, const SkMatrix* pattern_matrix, const gfx::RectF& src_rect, const ImageDrawOptions&) final;

private:
    PaintWorkletDeferredImage(scoped_refptr<PaintWorkletInput> input, const gfx::SizeF& size)
        : GeneratedImage(size)
    {
        image_ = PaintImageBuilder::WithDefault().set_deferred_paint_record(std::move(input)).set_id(PaintImage::GetNextId()).TakePaintImage();
    }
