...
CSSNumericValueType NumericTypeFromUnitMap(const CSSNumericSumValue::UnitMap& units)
{
    CSSNumericValueType type;
    for (const auto& unit_exponent : units) {
        bool error = false;
        type = CSSNumericValueType::Multiply(type, CSSNumericValueType(unit_exponent.value, unit_exponent.key), error);
        DCHECK(!error);
    }
    return type;
}
...
bool CanCreateNumericTypeFromSumValue(const CSSNumericSumValue& sum)
{
    ...
    return base::ranges::all_of(sum.terms, [&first_type](const CSSNumericSumValue::Term& term) {
        bool error = false;
        CSSNumericValueType::Add(first_type, NumericTypeFromUnitMap(term.units), error);
        return !error;
    });
}
...
CSSMathSum* CSSMathSum::Create(const HeapVector<Member<V8CSSNumberish>>& args, ExceptionState& exception_state)
{
    if (args.empty()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "Arguments can't be empty");
        return nullptr;
    }

    CSSMathSum* result = Create(CSSNumberishesToNumericValues(args), exception_state);
    if (!result) {
...
CSSMathSum* CSSMathSum::Create(CSSNumericValueVector values, ExceptionState& exception_state)
{
    bool error = false;
    CSSNumericValueType final_type = CSSMathVariadic::TypeCheck(values, CSSNumericValueType::Add, error);
    CSSMathSum* result = error ? nullptr : MakeGarbageCollected<CSSMathSum>(MakeGarbageCollected<CSSNumericArray>(std::move(values)), final_type);
    if (!result) {
        exception_state.ThrowTypeError("Incompatible types");
    }
...
void CSSMathSum::BuildCSSText(Nested nested, ParenLess paren_less, StringBuilder& result) const
{
    ...
    for (wtf_size_t i = 1; i < values.size(); i++) {
        ...
        if (arg.GetType() == CSSStyleValue::kNegateType) {
            result.Append(" - ");
            static_cast<const CSSMathNegate&>(arg).Value().BuildCSSText(Nested::kYes, ParenLess::kNo, result);
        } else {
            result.Append(" + ");
