...
CSSNumericValue* CSSColorValue::ToNumberOrPercentage(const V8CSSNumberish* input)
{
    CSSNumericValue* value = CSSNumericValue::FromPercentish(input);
    DCHECK(value);
    if (!CSSOMTypes::IsCSSStyleValueNumber(*value) && !CSSOMTypes::IsCSSStyleValuePercentage(*value)) {
        return nullptr;
    }
...
{
    return MakeGarbageCollected<CSSRGB>(CSSNumericValue::FromNumberish(MakeGarbageCollected<V8CSSNumberish>(red)),
        CSSNumericValue::FromNumberish(MakeGarbageCollected<V8CSSNumberish>(green)), CSSNumericValue::FromNumberish(MakeGarbageCollected<V8CSSNumberish>(blue)),
        CSSNumericValue::FromPercentish(MakeGarbageCollected<V8CSSNumberish>(alpha / 255.0)));
}

V8UnionCSSColorValueOrCSSStyleValue* CSSColorValue::parse(const ExecutionContext* execution_context, const String& css_text, ExceptionState& exception_state)
{
    ...
    if (color_type == CSSColorType::kInvalid) {
        exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "Invalid color expression");
        return nullptr;
    }

    const CSSValue* parsed_value = css_parsing_utils::ConsumeColor(stream, *MakeGarbageCollected<CSSParserContext>(*execution_context));
    stream.ConsumeWhitespace();
    ...
    if (parsed_value->IsColorValue()) {
        ...
        case CSSColorType::kRGB:
            return MakeGarbageCollected<V8UnionCSSColorValueOrCSSStyleValue>(
                CreateCSSRGBByNumbers(result->Value().Red(), result->Value().Green(), result->Value().Blue(), result->Value().AlphaAsInteger()));
        case CSSColorType::kHSL:
            return MakeGarbageCollected<V8UnionCSSColorValueOrCSSStyleValue>(MakeGarbageCollected<CSSHSL>(result->Value()));
        case CSSColorType::kHWB:
            return MakeGarbageCollected<V8UnionCSSColorValueOrCSSStyleValue>(MakeGarbageCollected<CSSHWB>(result->Value()));
        default:
            break;
        }
    }

    const CSSValueID value_id = To<CSSIdentifierValue>(parsed_value)->GetValueID();
    ...
    if (const NamedColor* named_color = FindColor(value_name)) {
        ...
        return MakeGarbageCollected<V8UnionCSSColorValueOrCSSStyleValue>(
            CreateCSSRGBByNumbers(color.Red(), color.Green(), color.Blue(), color.AlphaAsInteger()));
    }

    return MakeGarbageCollected<V8UnionCSSColorValueOrCSSStyleValue>(MakeGarbageCollected<CSSKeywordValue>(value_id));
}
