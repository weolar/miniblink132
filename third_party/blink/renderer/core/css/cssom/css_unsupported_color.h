...
public:
    explicit CSSUnsupportedColor(Color color)
        : CSSUnsupportedStyleValue(cssvalue::CSSColor::SerializeAsCSSComponentValue(color))
        , color_value_(color)
    {
    }
    explicit CSSUnsupportedColor(const CSSPropertyName& name, Color color)
        : CSSUnsupportedStyleValue(name, cssvalue::CSSColor::SerializeAsCSSComponentValue(color))
        , color_value_(color)
    {
    }
...
template <> struct DowncastTraits<CSSUnsupportedColor> {
    static bool AllowFrom(const CSSStyleValue& value)
    {
        return value.GetType() == CSSStyleValue::StyleValueType::kUnsupportedColorType;
    }
};
