...
CSSNumericSumValue::UnitMap MultiplyUnitMaps(CSSNumericSumValue::UnitMap a, const CSSNumericSumValue::UnitMap& b)
{
    for (const auto& unit_exponent : b) {
        DCHECK_NE(unit_exponent.value, 0);
        const auto old_value = a.Contains(unit_exponent.key) ? a.at(unit_exponent.key) : 0;
...
CSSMathProduct* CSSMathProduct::Create(const HeapVector<Member<V8CSSNumberish>>& args, ExceptionState& exception_state)
{
    if (args.empty()) {
...
{
    bool error = false;
    CSSNumericValueType final_type = CSSMathVariadic::TypeCheck(values, CSSNumericValueType::Multiply, error);
    return error ? nullptr : MakeGarbageCollected<CSSMathProduct>(MakeGarbageCollected<CSSNumericArray>(std::move(values)), final_type);
}
    ...
    for (const auto& value : NumericValues()) {
        ...
        for (const auto& a : sum.terms) {
            for (const auto& b : child_sum->terms) {
                new_sum.terms.emplace_back(a.value * b.value, MultiplyUnitMaps(a.units, b.units));
            }
        }
...
void CSSMathProduct::BuildCSSText(Nested nested, ParenLess paren_less, StringBuilder& result) const
{
    ...
    for (wtf_size_t i = 1; i < values.size(); i++) {
        ...
        if (arg.GetType() == CSSStyleValue::kInvertType) {
            result.Append(" / ");
            static_cast<const CSSMathInvert&>(arg).Value().BuildCSSText(Nested::kYes, ParenLess::kNo, result);
        } else {
            result.Append(" * ");
