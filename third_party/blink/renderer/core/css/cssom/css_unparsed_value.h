...
public:
    static CSSUnparsedValue* Create(const HeapVector<Member<V8CSSUnparsedSegment>>& tokens)
    {
    ...
    explicit CSSUnparsedValue(const HeapVector<Member<V8CSSUnparsedSegment>>& tokens)
        : tokens_(tokens)
    {
    }
    ...
    {
        return kUnparsedType;
    }

    V8CSSUnparsedSegment* AnonymousIndexedGetter(uint32_t index, ExceptionState& exception_state) const;
    IndexedPropertySetterResult AnonymousIndexedSetter(uint32_t index, V8CSSUnparsedSegment* segment, ExceptionState& exception_state);
...
private:
    ...
    bool AppendUnparsedString(StringBuilder&, HeapHashSet<Member<const CSSUnparsedValue>>& values_on_stack) const;
...
template <> struct DowncastTraits<CSSUnparsedValue> {
