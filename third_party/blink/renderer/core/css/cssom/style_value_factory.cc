...
CSSStyleValue* CreateStyleValueWithoutProperty(const CSSValue& value)
{
    ...
    if (auto* variable_reference_value = DynamicTo<CSSUnparsedDeclarationValue>(value)) {
        return CSSUnparsedValue::FromCSSValue(*variable_reference_value);
    }
    if (auto* custom_prop_declaration = DynamicTo<CSSUnparsedDeclarationValue>(value)) {
...
{
    if (IsA<CSSIdentifierValue>(value) || IsA<CSSCustomIdentValue>(value) || IsA<cssvalue::CSSScopedKeywordValue>(value)) {
        return CSSKeywordValue::FromCSSValue(value);
    }
...
CSSStyleValue* CreateStyleValueWithPropertyInternal(CSSPropertyID property_id, const CSSValue& value)
{
    ...
    case CSSPropertyID::kTextEmphasisColor: {
        ...
        return MakeGarbageCollected<CSSUnsupportedStyleValue>(CSSPropertyName(property_id), value);
    }
...
CSSStyleValue* CreateStyleValueWithProperty(CSSPropertyID property_id, const CSSValue& value)
{
    ...
    CSSStyleValue* style_value = CreateStyleValueWithPropertyInternal(property_id, value);
    if (style_value) {
        return style_value;
    }
    return CreateStyleValue(value);
...
CSSStyleValueVector UnsupportedCSSValue(const CSSPropertyName& name, const CSSValue& value)
{
    CSSStyleValueVector style_value_vector;
    style_value_vector.push_back(MakeGarbageCollected<CSSUnsupportedStyleValue>(name, value));
...
CSSStyleValueVector StyleValueFactory::FromString(
    CSSPropertyID property_id, const AtomicString& custom_property_name, const String& css_text, const CSSParserContext* parser_context)
{
    DCHECK_NE(property_id, CSSPropertyID::kInvalid);
    DCHECK_EQ(property_id == CSSPropertyID::kVariable, !custom_property_name.IsNull());
    ...
    if (property_id != CSSPropertyID::kVariable
        && CSSPropertyParser::ParseValue(
            property_id, /*allow_important_annotation=*/false, stream, parser_context, parsed_properties, StyleRule::RuleType::kStyle)) {
        if (parsed_properties.size() == 1) {
            const auto result = StyleValueFactory::CssValueToStyleValueVector(CSSPropertyName(parsed_properties[0].Id()), *parsed_properties[0].Value());
        ...
        result.push_back(MakeGarbageCollected<CSSUnsupportedStyleValue>(CSSPropertyName(property_id), css_text));
        return result;
    }

    stream.Restore(savepoint);
    bool important_ignored;
    const CSSVariableData* variable_data = CSSVariableParser::ConsumeUnparsedDeclaration(stream, /*allow_important_annotation=*/false,
        /*is_animation_tainted=*/false,
        /*must_contain_variable_reference=*/false,
        /*restricted_value=*/false, /*comma_ends_declaration=*/false, important_ignored, *parser_context);
    if (variable_data) {
        if ((property_id == CSSPropertyID::kVariable && variable_data->OriginalText().length() > 0) || variable_data->NeedsVariableResolution()) {
...
CSSStyleValue* StyleValueFactory::CssValueToStyleValue(const CSSPropertyName& name, const CSSValue& css_value)
{
    DCHECK(!CSSProperty::IsRepeated(name));
    CSSStyleValue* style_value = CreateStyleValueWithProperty(name.Id(), css_value);
    if (!style_value) {
...
CSSStyleValueVector StyleValueFactory::CoerceStyleValuesOrStrings(const CSSProperty& property, const AtomicString& custom_property_name,
    const HeapVector<Member<V8UnionCSSStyleValueOrString>>& values, const ExecutionContext& execution_context)
{
    ...
    for (const auto& value : values) {
        ...
        case V8UnionCSSStyleValueOrString::ContentType::kString: {
            if (!parser_context) {
                parser_context = MakeGarbageCollected<CSSParserContext>(execution_context);
            }

            const auto& subvalues = StyleValueFactory::FromString(property.PropertyID(), custom_property_name, value->GetAsString(), parser_context);
            if (subvalues.empty()) {
                return CSSStyleValueVector();
            }
...
CSSStyleValueVector StyleValueFactory::CssValueToStyleValueVector(const CSSPropertyName& name, const CSSValue& css_value)
{
    CSSStyleValueVector style_value_vector;

    CSSPropertyID property_id = name.Id();
    CSSStyleValue* style_value = CreateStyleValueWithProperty(property_id, css_value);
    if (style_value) {
    ...
    if (!css_value_list ||
        ...
        // properties.
        (property_id != CSSPropertyID::kVariable && !CSSProperty::Get(property_id).IsRepeated()) ||
        ...
        (property_id == CSSPropertyID::kVariable && CSSTransformComponent::FromCSSValue(css_value))) {
...
CSSStyleValueVector StyleValueFactory::CssValueToStyleValueVector(const CSSValue& css_value)
{
    CSSStyleValueVector style_value_vector;
    ...
    if (CSSStyleValue* value = CreateStyleValueWithoutProperty(css_value)) {
        ...
        style_value_vector.push_back(MakeGarbageCollected<CSSUnsupportedStyleValue>(css_value.CssText()));
    }
