...
public:
    explicit CrossThreadColorValue(Color value)
        : value_(value)
    {
    }
    ...
    {
        return StyleValueType::kColorType;
    }
...
template <> struct DowncastTraits<CrossThreadColorValue> {
    static bool AllowFrom(const CrossThreadStyleValue& color_value)
    {
        return color_value.GetType() == CrossThreadStyleValue::StyleValueType::kColorType;
    }
};
