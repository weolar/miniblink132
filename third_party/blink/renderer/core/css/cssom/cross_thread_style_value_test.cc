...
public:
    ...
    void ShutDownThread()
    {
        ...
        PostCrossThreadTask(*thread_->GetTaskRunner(), FROM_HERE,
            CrossThreadBindOnce(&CrossThreadStyleValueTest::ShutDown, CrossThreadUnretained(this), CrossThreadUnretained(&waitable_event)));
        waitable_event.Wait();
    }

    void CheckUnsupportedValue(base::WaitableEvent* waitable_event, std::unique_ptr<CrossThreadUnsupportedValue> value)
    {
        DCHECK(!IsMainThread());
    ...
    void CheckKeywordValue(base::WaitableEvent* waitable_event, std::unique_ptr<CrossThreadKeywordValue> value)
    {
        DCHECK(!IsMainThread());
    ...
    void CheckUnparsedValue(base::WaitableEvent* waitable_event, std::unique_ptr<CrossThreadUnparsedValue> value)
    {
        DCHECK(!IsMainThread());
    ...
    void CheckUnitValue(base::WaitableEvent* waitable_event, std::unique_ptr<CrossThreadUnitValue> value)
    {
        DCHECK(!IsMainThread());
    ...
    void CheckColorValue(base::WaitableEvent* waitable_event, std::unique_ptr<CrossThreadColorValue> value)
    {
        DCHECK(!IsMainThread());
...
{
    std::unique_ptr<CrossThreadUnsupportedValue> value = std::make_unique<CrossThreadUnsupportedValue>("Unsupported");
    DCHECK(value);

    // Use a Thread to emulate worklet thread.
    thread_ = blink::NonMainThread::CreateThread(ThreadCreationParams(ThreadType::kTestThread).SetSupportsGC(true));
    base::WaitableEvent waitable_event;
    PostCrossThreadTask(*thread_->GetTaskRunner(), FROM_HERE,
        CrossThreadBindOnce(
            &CrossThreadStyleValueTest::CheckUnsupportedValue, CrossThreadUnretained(this), CrossThreadUnretained(&waitable_event), std::move(value)));
    waitable_event.Wait();
...
TEST_F(CrossThreadStyleValueTest, CrossThreadUnsupportedValueToCSSStyleValue)
{
    ...
    EXPECT_EQ(style_value->GetType(), CSSStyleValue::StyleValueType::kUnknownType);
    EXPECT_EQ(style_value->CSSText(), "Unsupported");
}

TEST_F(CrossThreadStyleValueTest, PassUnparsedValueCrossThread)
{
    std::unique_ptr<CrossThreadUnparsedValue> value = std::make_unique<CrossThreadUnparsedValue>("Unparsed");
    DCHECK(value);
    ...
    PostCrossThreadTask(*thread_->GetTaskRunner(), FROM_HERE,
        CrossThreadBindOnce(
            &CrossThreadStyleValueTest::CheckUnparsedValue, CrossThreadUnretained(this), CrossThreadUnretained(&waitable_event), std::move(value)));
    waitable_event.Wait();
...
TEST_F(CrossThreadStyleValueTest, CrossThreadUnparsedValueToCSSStyleValue)
{
    ...
    EXPECT_EQ(style_value->GetType(), CSSStyleValue::StyleValueType::kUnparsedType);
    EXPECT_EQ(static_cast<CSSUnparsedValue*>(style_value)->ToUnparsedString(), "Unparsed");
}

TEST_F(CrossThreadStyleValueTest, PassKeywordValueCrossThread)
{
    std::unique_ptr<CrossThreadKeywordValue> value = std::make_unique<CrossThreadKeywordValue>("Keyword");
    DCHECK(value);
    ...
    PostCrossThreadTask(*thread_->GetTaskRunner(), FROM_HERE,
        CrossThreadBindOnce(
            &CrossThreadStyleValueTest::CheckKeywordValue, CrossThreadUnretained(this), CrossThreadUnretained(&waitable_event), std::move(value)));
    waitable_event.Wait();
...
{
    std::unique_ptr<CrossThreadUnitValue> value = std::make_unique<CrossThreadUnitValue>(1, CSSPrimitiveValue::UnitType::kDegrees);
    DCHECK(value);
    ...
    base::WaitableEvent waitable_event;
    PostCrossThreadTask(*thread_->GetTaskRunner(), FROM_HERE,
        CrossThreadBindOnce(&CrossThreadStyleValueTest::CheckUnitValue, CrossThreadUnretained(this), CrossThreadUnretained(&waitable_event), std::move(value)));
    waitable_event.Wait();
...
{
    std::unique_ptr<CrossThreadColorValue> value = std::make_unique<CrossThreadColorValue>(Color(0, 255, 0));
    DCHECK(value);
    ...
    PostCrossThreadTask(*thread_->GetTaskRunner(), FROM_HERE,
        CrossThreadBindOnce(
            &CrossThreadStyleValueTest::CheckColorValue, CrossThreadUnretained(this), CrossThreadUnretained(&waitable_event), std::move(value)));
    waitable_event.Wait();
...
TEST_F(CrossThreadStyleValueTest, CrossThreadColorValueToCSSStyleValue)
{
    ...
    CSSStyleValue* style_value = value->ToCSSStyleValue();
    EXPECT_EQ(style_value->GetType(), CSSStyleValue::StyleValueType::kUnsupportedColorType);
    EXPECT_EQ(static_cast<CSSUnsupportedColor*>(style_value)->Value(), Color(0, 255, 0));
}
...
TEST_F(CrossThreadStyleValueTest, ComparingNullValues)
{
    ...
    std::unique_ptr<CrossThreadStyleValue> keyword_value(new CrossThreadKeywordValue("keyword"));
    std::unique_ptr<CrossThreadStyleValue> unit_value(new CrossThreadUnitValue(1, CSSPrimitiveValue::UnitType::kDegrees));
    std::unique_ptr<CrossThreadStyleValue> unsupported_value(new CrossThreadUnsupportedValue("unsupported"));
...
TEST_F(CrossThreadStyleValueTest, ComparingCrossThreadKeywordValue)
{
    ...
    // so are they.
    std::unique_ptr<CrossThreadStyleValue> keyword_value_1(new CrossThreadKeywordValue("keyword"));
    std::unique_ptr<CrossThreadStyleValue> keyword_value_2(new CrossThreadKeywordValue("keyword"));
    std::unique_ptr<CrossThreadStyleValue> keyword_value_3(new CrossThreadKeywordValue("different"));
...
TEST_F(CrossThreadStyleValueTest, ComparingCrossThreadUnitValue)
{
    ...
    std::unique_ptr<CrossThreadStyleValue> unit_value_1(new CrossThreadUnitValue(1, CSSPrimitiveValue::UnitType::kDegrees));

    // Same value, same unit.
    std::unique_ptr<CrossThreadStyleValue> unit_value_2(new CrossThreadUnitValue(1, CSSPrimitiveValue::UnitType::kDegrees));
    EXPECT_TRUE(base::ValuesEquivalent(unit_value_1, unit_value_2));

    // Same value, different unit.
    std::unique_ptr<CrossThreadStyleValue> unit_value_3(new CrossThreadUnitValue(1, CSSPrimitiveValue::UnitType::kPoints));
    EXPECT_FALSE(base::ValuesEquivalent(unit_value_1, unit_value_3));

    // Different value, same unit.
    std::unique_ptr<CrossThreadStyleValue> unit_value_4(new CrossThreadUnitValue(2, CSSPrimitiveValue::UnitType::kDegrees));
...
TEST_F(CrossThreadStyleValueTest, ComparingCrossThreadColorValue)
{
    ...
    // channels must match.
    std::unique_ptr<CrossThreadStyleValue> color_value_1(new CrossThreadColorValue(Color(0, 0, 0)));
    std::unique_ptr<CrossThreadStyleValue> color_value_2(new CrossThreadColorValue(Color(0, 0, 0)));
    std::unique_ptr<CrossThreadStyleValue> color_value_3(new CrossThreadColorValue(Color(0, 255, 0)));
...
TEST_F(CrossThreadStyleValueTest, ComparingCrossThreadUnsupportedValue)
{
    ...
    // then so are they.
    std::unique_ptr<CrossThreadStyleValue> unsupported_value_1(new CrossThreadUnsupportedValue("value"));
    std::unique_ptr<CrossThreadStyleValue> unsupported_value_2(new CrossThreadUnsupportedValue("value"));
    std::unique_ptr<CrossThreadStyleValue> unsupported_value_3(new CrossThreadUnsupportedValue("different"));

    EXPECT_TRUE(base::ValuesEquivalent(unsupported_value_1, unsupported_value_2));
    EXPECT_FALSE(base::ValuesEquivalent(unsupported_value_1, unsupported_value_3));
}
