...
CSSMathClamp* CSSMathClamp::Create(V8CSSNumberish* lower, V8CSSNumberish* value, V8CSSNumberish* upper, ExceptionState& exception_state)
{
...
CSSMathClamp* CSSMathClamp::Create(CSSNumericValue* lower, CSSNumericValue* value, CSSNumericValue* upper)
{
    bool error = false;
    CSSNumericValueType final_type = CSSMathClamp::TypeCheck(lower, value, upper, CSSNumericValueType::Add, error);
    return error ? nullptr : MakeGarbageCollected<CSSMathClamp>(lower, value, upper, final_type);
}
...
std::optional<CSSNumericSumValue> CSSMathClamp::SumValue() const
{
    ...
    for (const auto& value : { lower_, value_, upper_ }) {
        ...
        if (!child_sum.has_value() || child_sum->terms.size() != 1 || child_sum->terms[0].units != lower->terms[0].units) {
            return std::nullopt;
        }
    }
...
void CSSMathClamp::BuildCSSText(Nested, ParenLess, StringBuilder& result) const
{
    result.Append("clamp(");
    DCHECK(lower_);
