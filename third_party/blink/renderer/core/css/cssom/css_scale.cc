...
bool IsValidScaleCoord(CSSNumericValue* coord)
{
    ...
    if (coord && coord->GetType() != CSSStyleValue::StyleValueType::kUnitType) {
        ...
        return (resolved_type == CSSPrimitiveValue::UnitType::kNumber || resolved_type == CSSPrimitiveValue::UnitType::kInteger);
    }
...
CSSScale* FromScaleXYZ(const CSSFunctionValue& value)
{
    ...
    CSSNumericValue* numeric_value = CSSNumericValue::FromCSSValue(To<CSSPrimitiveValue>(value.Item(0)));
...
CSSScale* CSSScale::Create(const V8CSSNumberish* x, const V8CSSNumberish* y, ExceptionState& exception_state)
{
...
CSSScale* CSSScale::Create(const V8CSSNumberish* x, const V8CSSNumberish* y, const V8CSSNumberish* z, ExceptionState& exception_state)
{
    CSSNumericValue* x_value = CSSNumericValue::FromNumberish(x);
    CSSNumericValue* y_value = CSSNumericValue::FromNumberish(y);
    CSSNumericValue* z_value = CSSNumericValue::FromNumberish(z);

    if (!IsValidScaleCoord(x_value) || !IsValidScaleCoord(y_value) || !IsValidScaleCoord(z_value)) {
...
DOMMatrix* CSSScale::toMatrix(ExceptionState& exception_state) const
{
    ...
    if (!x || !y || !z) {
        exception_state.ThrowTypeError("Cannot create matrix if values are not numbers");
        return nullptr;
    }
...
const CSSFunctionValue* CSSScale::ToCSSValue() const
{
    ...
    CSSFunctionValue* result = MakeGarbageCollected<CSSFunctionValue>(is2D() ? CSSValueID::kScale : CSSValueID::kScale3d);
    result->Append(*x);
    result->Append(*y);
    if (!is2D()) {
...
CSSScale::CSSScale(CSSNumericValue* x, CSSNumericValue* y, CSSNumericValue* z, bool is2D)
