...
public:
    enum class BaseType : unsigned { kLength, kAngle, kTime, kFrequency, kResolution, kFlex, kPercent, kNumBaseTypes };

    static constexpr unsigned kNumBaseTypes = static_cast<unsigned>(BaseType::kNumBaseTypes);

    static String BaseTypeToString(BaseType);

    explicit CSSNumericValueType(CSSPrimitiveValue::UnitType = CSSPrimitiveValue::UnitType::kNumber);
    ...
    static CSSNumericValueType Add(CSSNumericValueType, CSSNumericValueType, bool& error);
    static CSSNumericValueType Multiply(CSSNumericValueType, CSSNumericValueType, bool& error);
    ...
    {
        DCHECK_NE(base_type, BaseType::kPercent);
        return IsOnlyNonZeroEntry(base_type, 1) || IsOnlyNonZeroEntry(BaseType::kPercent, 1);
    }
...
private:
    bool HasNonZeroEntries() const
    {
        return num_non_zero_entries_ > 0;
    }
