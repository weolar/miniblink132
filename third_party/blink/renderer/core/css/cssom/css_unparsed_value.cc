...
V8CSSUnparsedSegment* VariableReferenceValue(const StringView& variable_name, const HeapVector<Member<V8CSSUnparsedSegment>>& tokens)
{
    ...
    CSSStyleVariableReferenceValue* variable_reference = CSSStyleVariableReferenceValue::Create(variable_name.ToString(), unparsed_value);
    return MakeGarbageCollected<V8CSSUnparsedSegment>(variable_reference);
}

HeapVector<Member<V8CSSUnparsedSegment>> ParserTokenStreamToTokens(CSSParserTokenStream& stream)
{
    int nesting_level = 0;
    HeapVector<Member<V8CSSUnparsedSegment>> tokens;
    StringBuilder builder;
    while (stream.Peek().GetType() != kEOFToken) {
        if (stream.Peek().FunctionId() == CSSValueID::kVar || stream.Peek().FunctionId() == CSSValueID::kEnv) {
            if (!builder.empty()) {
                tokens.push_back(MakeGarbageCollected<V8CSSUnparsedSegment>(builder.ReleaseString()));
            }
            ...
            tokens.push_back(VariableReferenceValue(variable_name, ParserTokenStreamToTokens(stream)));
        } else {
...
CSSUnparsedValue* CSSUnparsedValue::FromCSSValue(const CSSUnparsedDeclarationValue& value)
{
    DCHECK(value.VariableDataValue());
    return FromCSSVariableData(*value.VariableDataValue());
}

CSSUnparsedValue* CSSUnparsedValue::FromCSSVariableData(const CSSVariableData& value)
{
    CSSParserTokenStream stream(value.OriginalText());
    return CSSUnparsedValue::Create(ParserTokenStreamToTokens(stream));
}

V8CSSUnparsedSegment* CSSUnparsedValue::AnonymousIndexedGetter(uint32_t index, ExceptionState& exception_state) const
{
...
IndexedPropertySetterResult CSSUnparsedValue::AnonymousIndexedSetter(uint32_t index, V8CSSUnparsedSegment* segment, ExceptionState& exception_state)
{
    if (index < tokens_.size()) {
    ...
    exception_state.ThrowRangeError(ExceptionMessages::IndexOutsideRange<unsigned>(
        "index", index, 0, ExceptionMessages::kInclusiveBound, tokens_.size(), ExceptionMessages::kInclusiveBound));
    return IndexedPropertySetterResult::kIntercepted;
}
...
const CSSValue* CSSUnparsedValue::ToCSSValue() const
{
    ...
    if (stream.AtEnd()) {
        return MakeGarbageCollected<CSSUnparsedDeclarationValue>(MakeGarbageCollected<CSSVariableData>());
    }
    ...
    return MakeGarbageCollected<CSSUnparsedDeclarationValue>(
        CSSVariableData::Create(original_text, false /* is_animation_tainted */, false /* needs_variable_resolution */));
}
...
bool CSSUnparsedValue::AppendUnparsedString(StringBuilder& builder, HeapHashSet<Member<const CSSUnparsedValue>>& values_on_stack) const
{
    ...
    for (unsigned i = 0; i < tokens_.size(); i++) {
        ...
        case V8CSSUnparsedSegment::ContentType::kCSSVariableReferenceValue: {
            ...
            if (reference_value->fallback()) {
                builder.Append(",");
                if (!reference_value->fallback()->AppendUnparsedString(builder, values_on_stack)) {
                    return false; // Cycle.
                }
            }
            builder.Append(")");
