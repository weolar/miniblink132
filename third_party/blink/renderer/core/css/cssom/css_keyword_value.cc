...
CSSKeywordValue* CSSKeywordValue::Create(const String& keyword, ExceptionState& exception_state)
{
    if (keyword.empty()) {
        exception_state.ThrowTypeError("CSSKeywordValue does not support empty strings");
        return nullptr;
    }
...
CSSKeywordValue* CSSKeywordValue::FromCSSValue(const CSSValue& value)
{
    ...
    if (auto* identifier_value = DynamicTo<CSSIdentifierValue>(value)) {
        return MakeGarbageCollected<CSSKeywordValue>(identifier_value->GetValueID());
    }
    ...
    if (auto* scoped_keyword_value = DynamicTo<cssvalue::CSSScopedKeywordValue>(value)) {
        return MakeGarbageCollected<CSSKeywordValue>(scoped_keyword_value->GetValueID());
    }
    NOTREACHED();
}
...
void CSSKeywordValue::setValue(const String& keyword, ExceptionState& exception_state)
{
    if (keyword.empty()) {
        exception_state.ThrowTypeError("CSSKeywordValue does not support empty strings");
        return;
...
const CSSValue* CSSKeywordValue::ToCSSValue() const
{
    ...
    case (CSSValueID::kInvalid):
        return MakeGarbageCollected<CSSCustomIdentValue>(AtomicString(keyword_value_));
    default:
