...
CSSSkewX* CSSSkewX::Create(CSSNumericValue* ax, ExceptionState& exception_state)
{
...
CSSSkewX* CSSSkewX::FromCSSValue(const CSSFunctionValue& value)
{
    ...
    if (value.length() == 1U) {
        return CSSSkewX::Create(CSSNumericValue::FromCSSValue(To<CSSPrimitiveValue>(value.Item(0))));
    }
    NOTREACHED();
}
...
const CSSFunctionValue* CSSSkewX::ToCSSValue() const
{
    ...
    CSSFunctionValue* result = MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kSkewX);
    result->Append(*ax);
    return result;
}
