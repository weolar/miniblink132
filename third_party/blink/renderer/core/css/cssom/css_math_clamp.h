...
public:
    ...
    static CSSMathClamp* Create(V8CSSNumberish* lower, V8CSSNumberish* value, V8CSSNumberish* upper, ExceptionState& exception_state);
    // Blink-internal constructor.
    static CSSMathClamp* Create(CSSNumericValue* lower, CSSNumericValue* value, CSSNumericValue* upper);

    CSSMathClamp(CSSNumericValue* lower, CSSNumericValue* value, CSSNumericValue* upper, const CSSNumericValueType& type)
        : CSSMathValue(type)
        , lower_(lower)
        , value_(value)
        , upper_(upper)
    ...
    CSSNumericValue* lowerValue()
    {
        return lower_.Get();
    }
    CSSNumericValue* valueValue()
    {
        return value_.Get();
    }
    CSSNumericValue* upperValue()
    {
        return upper_.Get();
    }
    ...
    {
        return CSSStyleValue::kClampType;
    }
    ...
    bool Equals(const CSSNumericValue& other) const final
    {
        ...
        return (lower_->Equals(*other_variadic.lower_) && value_->Equals(*other_variadic.value_) && upper_->Equals(*other_variadic.upper_));
    }

    template <class BinaryTypeOperation>
    static CSSNumericValueType TypeCheck(CSSNumericValue* lower, CSSNumericValue* value, CSSNumericValue* upper, BinaryTypeOperation op, bool& error)
    {
        error = false;
