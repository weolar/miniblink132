...
class CORE_EXPORT CrossThreadUnparsedValue final : public CrossThreadStyleValue {
public:
    explicit CrossThreadUnparsedValue(const String& value)
        : value_(value)
    {
    }
...
template <> struct DowncastTraits<CrossThreadUnparsedValue> {
    static bool AllowFrom(const CrossThreadStyleValue& value)
    {
        return value.GetType() == CrossThreadStyleValue::StyleValueType::kUnparsedType;
    }
};
