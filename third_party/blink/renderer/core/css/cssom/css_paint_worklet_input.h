...
public:
    CSSPaintWorkletInput(const String& name, const gfx::SizeF& container_size, float effective_zoom, int worklet_id,
        PaintWorkletStylePropertyMap::CrossThreadData values, Vector<std::unique_ptr<CrossThreadStyleValue>> parsed_input_args,
    ...
    const Vector<std::unique_ptr<CrossThreadStyleValue>>& ParsedInputArguments() const
    {
        return parsed_input_arguments_;
    }

    // These should only be accessed on the PaintWorklet thread.
    String NameCopy() const
    {
        return name_;
    }
...
template <> struct DowncastTraits<CSSPaintWorkletInput> {
    ...
    {
        return worklet_input.GetType() == PaintWorkletInput::PaintWorkletInputType::kCSS;
    }
};
