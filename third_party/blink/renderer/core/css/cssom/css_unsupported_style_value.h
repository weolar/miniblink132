...
template <> struct DowncastTraits<CSSUnsupportedStyleValue> {
    static bool AllowFrom(const CSSStyleValue& value)
    {
        return value.GetType() == CSSStyleValue::StyleValueType::kUnknownType || value.GetType() == CSSStyleValue::StyleValueType::kUnsupportedColorType;
    }
};
