...
class CORE_EXPORT ComputedStylePropertyMap : public StylePropertyMapReadOnlyMainThread {
public:
    explicit ComputedStylePropertyMap(Element* element, const String& pseudo_element = String())
        : pseudo_id_(CSSSelectorParser::ParsePseudoElement(pseudo_element, element, pseudo_argument_))
        , element_(element)
    {
    }
    ...
    static bool ComparePropertyNames(const CSSPropertyName&, const CSSPropertyName&);
