...
CSSSkewY* CSSSkewY::Create(CSSNumericValue* ay, ExceptionState& exception_state)
{
...
CSSSkewY* CSSSkewY::FromCSSValue(const CSSFunctionValue& value)
{
    ...
    if (value.length(), 1U) {
        return CSSSkewY::Create(CSSNumericValue::FromCSSValue(To<CSSPrimitiveValue>(value.Item(0))));
    }
    NOTREACHED();
}
...
const CSSFunctionValue* CSSSkewY::ToCSSValue() const
{
    ...
    CSSFunctionValue* result = MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kSkewY);
    result->Append(*ay);
    return result;
}
