...
const CSSValue* StyleValueToCSSValue(
    const CSSProperty& property, const AtomicString& custom_property_name, const CSSStyleValue& style_value, const ExecutionContext& execution_context)
{
    DCHECK_EQ(property.IDEquals(CSSPropertyID::kVariable), !custom_property_name.IsNull());

    const CSSPropertyID property_id = property.PropertyID();
    if (!CSSOMTypes::PropertyCanTake(property_id, custom_property_name, style_value)) {
        return nullptr;
    }

    if (style_value.GetType() == CSSStyleValue::kUnknownType) {
        return CSSParser::ParseSingleValue(property.PropertyID(), style_value.toString(), MakeGarbageCollected<CSSParserContext>(execution_context));
    }
    ...
    case CSSPropertyID::kAnchorScope: {
        ...
        if (const auto* ident = DynamicTo<CSSIdentifierValue>(style_value.ToCSSValue()); ident && ident->GetValueID() == CSSValueID::kAll) {
            return MakeGarbageCollected<cssvalue::CSSScopedKeywordValue>(ident->GetValueID());
        }
        break;
    }
    ...
    case CSSPropertyID::kBorderStartStartRadius: {
        ...
        if (value->IsPrimitiveValue()) {
            return MakeGarbageCollected<CSSValuePair>(value, value, CSSValuePair::kDropIdenticalValues);
        }
        break;
    }
    case CSSPropertyID::kClipPath: {
        ...
        if (identifier_value && !value->IsCSSWideKeyword() && identifier_value->GetValueID() != CSSValueID::kNone) {
    ...
    case CSSPropertyID::kContainerType: {
        ...
        const auto* value = style_value.ToCSSValue();
        if ((value->IsIdentifierValue() && !value->IsCSSWideKeyword()) || value->IsPrimitiveValue()) {
            CSSValueList* list = CSSValueList::CreateSpaceSeparated();
            list->Append(*style_value.ToCSSValue());
            return list;
        }
    ...
    case CSSPropertyID::kFontVariantNumeric: {
        ...
        // them as a list
        if (const auto* value = DynamicTo<CSSIdentifierValue>(style_value.ToCSSValue())) {
            // 'none' and 'normal' are stored as a single value
            if (value->GetValueID() == CSSValueID::kNone || value->GetValueID() == CSSValueID::kNormal) {
                break;
            }
    ...
    case CSSPropertyID::kPaintOrder: {
        ...
        // wrapped in a list.
        auto* identifier_value = DynamicTo<CSSIdentifierValue>(value);
        if (identifier_value && !value->IsCSSWideKeyword() && identifier_value->GetValueID() != CSSValueID::kNormal) {
            CSSValueList* list = CSSValueList::CreateSpaceSeparated();
            list->Append(*style_value.ToCSSValue());
            return list;
        }
...
const CSSValue* CoerceStyleValueOrString(
    const CSSProperty& property, const AtomicString& custom_property_name, const V8UnionCSSStyleValueOrString* value, const ExecutionContext& execution_context)
{
    ...
    case V8UnionCSSStyleValueOrString::ContentType::kCSSStyleValue:
        return StyleValueToCSSValue(property, custom_property_name, *value->GetAsCSSStyleValue(), execution_context);
    case V8UnionCSSStyleValueOrString::ContentType::kString: {
        const auto& values = StyleValueFactory::FromString(
            property.PropertyID(), custom_property_name, value->GetAsString(), MakeGarbageCollected<CSSParserContext>(execution_context));
        if (values.size() != 1U) {
            return nullptr;
        }
        return StyleValueToCSSValue(property, custom_property_name, *values[0], execution_context);
    }
    }
...
const CSSValue* CoerceStyleValuesOrStrings(const CSSProperty& property, const AtomicString& custom_property_name,
    const HeapVector<Member<V8UnionCSSStyleValueOrString>>& values, const ExecutionContext& execution_context)
{
    DCHECK(property.IsRepeated());
    DCHECK_EQ(property.IDEquals(CSSPropertyID::kVariable), !custom_property_name.IsNull());
    if (values.empty()) {
    ...
    CSSStyleValueVector style_values = StyleValueFactory::CoerceStyleValuesOrStrings(property, custom_property_name, values, execution_context);
    ...
    for (const auto& style_value : style_values) {
        const CSSValue* css_value = StyleValueToCSSValue(property, custom_property_name, *style_value, execution_context);
        if (!css_value) {
            return nullptr;
        }
...
void StylePropertyMap::set(const ExecutionContext* execution_context, const String& property_name,
    const HeapVector<Member<V8UnionCSSStyleValueOrString>>& values, ExceptionState& exception_state)
{
    const CSSPropertyID property_id = CssPropertyID(execution_context, property_name);
    ...
    if (property.IsShorthand()) {
        ...
        case V8UnionCSSStyleValueOrString::ContentType::kCSSStyleValue: {
            ...
            if (CSSOMTypes::PropertyCanTake(property_id, g_null_atom, *style_value)) {
        ...
        if (css_text.empty() || !SetShorthandProperty(property.PropertyID(), css_text, execution_context->GetSecureContextMode())) {
            exception_state.ThrowTypeError("Invalid type for property");
        }
    ...
    const AtomicString& custom_property_name = (property_id == CSSPropertyID::kVariable) ? AtomicString(property_name) : g_null_atom;

    const CSSValue* result = nullptr;
    if (property.IsRepeated()) {
        result = CoerceStyleValuesOrStrings(property, custom_property_name, values, *execution_context);
    } else if (values.size() == 1U) {
        result = CoerceStyleValueOrString(property, custom_property_name, values[0], *execution_context);
    }
...
void StylePropertyMap::append(const ExecutionContext* execution_context, const String& property_name,
    const HeapVector<Member<V8UnionCSSStyleValueOrString>>& values, ExceptionState& exception_state)
{
    if (values.empty()) {
        return;
    ...
    if (property_id == CSSPropertyID::kVariable) {
        exception_state.ThrowTypeError("Appending to custom properties is not supported");
        return;
    }
    ...
    if (const CSSValue* css_value = GetProperty(property_id)) {
        if (css_value->IsUnparsedDeclaration() || css_value->IsPendingSubstitutionValue()) {
            ...
            exception_state.ThrowTypeError("Cannot append to a list containing a variable reference");
            return;
        }
        if (!css_value->IsValueList()) {
            ...
            exception_state.ThrowTypeError("Cannot append to something that is not a list");
            return;
        }
    ...
    const CSSValue* result = CoerceStyleValuesOrStrings(property, g_null_atom, values, *execution_context);
...
void StylePropertyMap::remove(const ExecutionContext* execution_context, const String& property_name, ExceptionState& exception_state)
{
