...
CSSRGB::CSSRGB(CSSNumericValue* r, CSSNumericValue* g, CSSNumericValue* b, CSSNumericValue* alpha)
    : r_(r)
    , g_(g)
    , b_(b)
    , alpha_(alpha)
...
CSSRGB* CSSRGB::Create(
    const V8CSSNumberish* red, const V8CSSNumberish* green, const V8CSSNumberish* blue, const V8CSSNumberish* alpha, ExceptionState& exception_state)
{
    CSSNumericValue* r;
    CSSNumericValue* g;
    CSSNumericValue* b;
    ...
    if (!(r = ToNumberOrPercentage(red)) || !(g = ToNumberOrPercentage(green)) || !(b = ToNumberOrPercentage(blue))) {
        exception_state.ThrowTypeError("Color channel must be interpretable as a number or a percentage.");
        return nullptr;
    }
...
void CSSRGB::setG(const V8CSSNumberish* green, ExceptionState& exception_state)
{
...
void CSSRGB::setAlpha(const V8CSSNumberish* alpha, ExceptionState& exception_state)
{
...
{
    return Color::FromRGBAFloat(ComponentToColorInput(r_), ComponentToColorInput(g_), ComponentToColorInput(b_), ComponentToColorInput(alpha_));
}
