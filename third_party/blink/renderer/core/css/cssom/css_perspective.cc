...
V8CSSPerspectiveValue* HandleInputPerspective(V8CSSPerspectiveValue* value)
{
    ...
    case V8CSSPerspectiveValue::ContentType::kCSSNumericValue: {
        if (!value->GetAsCSSNumericValue()->Type().MatchesBaseType(CSSNumericValueType::BaseType::kLength)) {
            return nullptr;
        }
        break;
    }
    case V8CSSPerspectiveValue::ContentType::kString: {
        CSSKeywordValue* keyword = MakeGarbageCollected<CSSKeywordValue>(value->GetAsString());
    ...
    case V8CSSPerspectiveValue::ContentType::kCSSKeywordValue: {
        if (value->GetAsCSSKeywordValue()->KeywordValueID() != CSSValueID::kNone) {
            return nullptr;
        }
        break;
    }
...
CSSPerspective* CSSPerspective::Create(V8CSSPerspectiveValue* length, ExceptionState& exception_state)
{
    length = HandleInputPerspective(length);
    if (!length) {
        exception_state.ThrowTypeError("Must pass length or none to CSSPerspective");
        return nullptr;
    }
    return MakeGarbageCollected<CSSPerspective>(length);
}

void CSSPerspective::setLength(V8CSSPerspectiveValue* length, ExceptionState& exception_state)
{
    length = HandleInputPerspective(length);
    if (!length) {
        exception_state.ThrowTypeError("Must pass length or none to CSSPerspective");
...
CSSPerspective* CSSPerspective::FromCSSValue(const CSSFunctionValue& value)
{
    ...
    if (arg.IsPrimitiveValue()) {
        length = MakeGarbageCollected<V8CSSPerspectiveValue>(CSSNumericValue::FromCSSValue(To<CSSPrimitiveValue>(arg)));
    } else {
        DCHECK(arg.IsIdentifierValue() && To<CSSIdentifierValue>(arg).GetValueID() == CSSValueID::kNone);
        length = MakeGarbageCollected<V8CSSPerspectiveValue>(CSSKeywordValue::FromCSSValue(arg));
    }
    return MakeGarbageCollected<CSSPerspective>(length);
}
...
CSSPerspective::CSSPerspective(V8CSSPerspectiveValue* length)
    : CSSTransformComponent(false /* is2D */)
    , length_(length)
{
    DCHECK(length);
}
