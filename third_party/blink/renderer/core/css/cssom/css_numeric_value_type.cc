...
CSSNumericValueType::BaseType UnitTypeToBaseType(CSSPrimitiveValue::UnitType unit)
{
...
CSSNumericValueType::CSSNumericValueType(int exponent, CSSPrimitiveValue::UnitType unit)
{
    exponents_.Fill(0, kNumBaseTypes);
    if (unit != CSSPrimitiveValue::UnitType::kNumber) {
...
CSSNumericValueType CSSNumericValueType::NegateExponents(CSSNumericValueType type)
{
    base::ranges::transform(type.exponents_, type.exponents_.begin(), std::negate());
    return type;
}

CSSNumericValueType CSSNumericValueType::Add(CSSNumericValueType type1, CSSNumericValueType type2, bool& error)
{
...
CSSNumericValueType CSSNumericValueType::Multiply(CSSNumericValueType type1, CSSNumericValueType type2, bool& error)
{
    if (type1.HasPercentHint() && type2.HasPercentHint() && type1.PercentHint() != type2.PercentHint()) {
        error = true;
        return type1;
