...
public:
    ...
    bool Equals(const CSSNumericValue& other) const final
    {
        ...
        return base::ranges::equal(NumericValues(), other_variadic.NumericValues(), [](const auto& a, const auto& b) { return a->Equals(*b); });
    }
...
protected:
    ...
    template <class BinaryTypeOperation> static CSSNumericValueType TypeCheck(const CSSNumericValueVector& values, BinaryTypeOperation op, bool& error)
    {
        error = false;
    ...
    CSSMathExpressionNode* ToCalcExporessionNodeForVariadic(CSSMathOperator op) const
    {
        ...
        for (wtf_size_t i = 1; i < NumericValues().size(); i++) {
            CSSMathExpressionNode* next_arg = NumericValues()[i]->ToCalcExpressionNode();
            if (!next_arg) {
                return nullptr;
            }
