...
PrepopulatedComputedStylePropertyMap::PrepopulatedComputedStylePropertyMap(
    const Document& document, const ComputedStyle& style, const Vector<CSSPropertyID>& native_properties, const Vector<AtomicString>& custom_properties)
{
...
void PrepopulatedComputedStylePropertyMap::UpdateStyle(const Document& document, const ComputedStyle& style)
{
...
void PrepopulatedComputedStylePropertyMap::UpdateNativeProperty(const ComputedStyle& style, CSSPropertyID property_id)
{
...
void PrepopulatedComputedStylePropertyMap::UpdateCustomProperty(const Document& document, const ComputedStyle& style, const AtomicString& property_name)
{
    CSSPropertyRef ref(property_name, document);
    const CSSValue* value = ref.GetProperty().CSSValueFromComputedStyle(style, /*layout_object=*/nullptr,
        /*allow_visited_style=*/false, CSSValuePhase::kComputedValue);
    if (!value) {
...
const CSSValue* PrepopulatedComputedStylePropertyMap::GetProperty(CSSPropertyID property_id) const
{
    auto it = native_values_.find(property_id);
    return it != native_values_.end() ? it->value : nullptr;
}

const CSSValue* PrepopulatedComputedStylePropertyMap::GetCustomProperty(const AtomicString& property_name) const
{
    auto it = custom_values_.find(property_name);
    return it != custom_values_.end() ? it->value : nullptr;
}

void PrepopulatedComputedStylePropertyMap::ForEachProperty(IterationFunction visitor)
{
    ...
    std::sort(values.begin(), values.end(), [](const auto& a, const auto& b) { return ComputedStylePropertyMap::ComparePropertyNames(a.first, b.first); });
...
String PrepopulatedComputedStylePropertyMap::SerializationForShorthand(const CSSProperty&) const
{
