...
std::optional<CSSNumericSumValue> CSSMathNegate::SumValue() const
{
    ...
    base::ranges::for_each(maybe_sum->terms, [](auto& term) { term.value *= -1; });
    return maybe_sum;
}

void CSSMathNegate::BuildCSSText(Nested nested, ParenLess paren_less, StringBuilder& result) const
{
...
CSSMathExpressionNode* CSSMathNegate::ToCalcExpressionNode() const
{
    ...
    return CSSMathExpressionOperation::CreateArithmeticOperationSimplified(
        CSSMathExpressionNumericLiteral::Create(-1, CSSPrimitiveValue::UnitType::kNumber), right_side, CSSMathOperator::kMultiply);
}
