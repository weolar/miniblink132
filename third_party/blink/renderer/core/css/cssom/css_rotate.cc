...
{
    return value && value->Type().MatchesBaseType(CSSNumericValueType::BaseType::kAngle);
}

CSSRotate* FromCSSRotate(const CSSFunctionValue& value)
{
    DCHECK_EQ(value.length(), 1UL);
    CSSNumericValue* angle = CSSNumericValue::FromCSSValue(To<CSSPrimitiveValue>(value.Item(0)));
...
CSSRotate* FromCSSRotate3d(const CSSFunctionValue& value)
{
    ...
    CSSNumericValue* angle = CSSNumericValue::FromCSSValue(To<CSSPrimitiveValue>(value.Item(3)));
...
CSSRotate* FromCSSRotateXYZ(const CSSFunctionValue& value)
{
    ...
    case CSSValueID::kRotateX:
        return CSSRotate::Create(CSSUnitValue::Create(1), CSSUnitValue::Create(0), CSSUnitValue::Create(0), angle);
    case CSSValueID::kRotateY:
        return CSSRotate::Create(CSSUnitValue::Create(0), CSSUnitValue::Create(1), CSSUnitValue::Create(0), angle);
    case CSSValueID::kRotateZ:
        return CSSRotate::Create(CSSUnitValue::Create(0), CSSUnitValue::Create(0), CSSUnitValue::Create(1), angle);
    default:
        NOTREACHED();
    }
}
...
CSSRotate* CSSRotate::Create(CSSNumericValue* angle, ExceptionState& exception_state)
{
    if (!IsValidRotateAngle(angle)) {
        exception_state.ThrowTypeError("Must pass an angle to CSSRotate");
        return nullptr;
    }
    return MakeGarbageCollected<CSSRotate>(CSSUnitValue::Create(0), CSSUnitValue::Create(0), CSSUnitValue::Create(1), angle, true /* is2D */);
}

CSSRotate* CSSRotate::Create(const V8CSSNumberish* x, const V8CSSNumberish* y, const V8CSSNumberish* z, CSSNumericValue* angle, ExceptionState& exception_state)
{
    ...
    if (!IsValidRotateCoord(x_value) || !IsValidRotateCoord(y_value) || !IsValidRotateCoord(z_value)) {
    ...
    return MakeGarbageCollected<CSSRotate>(x_value, y_value, z_value, angle, false /* is2D */);
}
...
CSSRotate* CSSRotate::Create(CSSNumericValue* x, CSSNumericValue* y, CSSNumericValue* z, CSSNumericValue* angle)
{
...
void CSSRotate::setAngle(CSSNumericValue* angle, ExceptionState& exception_state)
{
    if (!IsValidRotateAngle(angle)) {
        exception_state.ThrowTypeError("Must pass an angle to CSSRotate");
        return;
...
DOMMatrix* CSSRotate::toMatrix(ExceptionState& exception_state) const
{
    ...
    if (!x || !y || !z) {
        exception_state.ThrowTypeError("Cannot create matrix if units cannot be converted to CSSUnitValue");
        return nullptr;
    }
...
{
    CSSFunctionValue* result = MakeGarbageCollected<CSSFunctionValue>(is2D() ? CSSValueID::kRotate : CSSValueID::kRotate3d);
    if (!is2D()) {
...
CSSRotate::CSSRotate(CSSNumericValue* x, CSSNumericValue* y, CSSNumericValue* z, CSSNumericValue* angle, bool is2D)
