...
TEST(InlineStylePropertyMapTest, PendingSubstitutionValueCrash)
{
    ...
    InlineStylePropertyMap* map = MakeGarbageCollected<InlineStylePropertyMap>(div);
    ...
    for (CSSPropertyID property_id : CSSPropertyIDList()) {
        ...
        for (const CSSProperty* longhand : longhands.properties()) {
            map->get(document->GetExecutionContext(), longhand->GetCSSPropertyName().ToAtomicString(), ASSERT_NO_EXCEPTION);
        }
    }
}
