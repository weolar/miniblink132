...
template <CSSStyleValue::StyleValueType type> void PrependValueForArithmetic(CSSNumericValueVector& vector, CSSNumericValue* value)
{
    DCHECK(value);
...
template <class BinaryOperation> CSSUnitValue* MaybeSimplifyAsUnitValue(const CSSNumericValueVector& values, const BinaryOperation& op)
{
    DCHECK(!values.empty());
    ...
    for (wtf_size_t i = 1; i < values.size(); i++) {
        ...
        if (!unit_value || unit_value->GetInternalUnit() != first_unit_value->GetInternalUnit()) {
            return nullptr;
        }
...
bool CanCombineNodes(const CSSMathExpressionNode& root, const CSSMathExpressionNode& node)
{
    ...
    return CanonicalOperator(To<CSSMathExpressionOperation>(root).OperatorType()) == CanonicalOperator(node_exp.OperatorType());
}

CSSNumericValue* NegateOrInvertIfRequired(CSSMathOperator parent_op, CSSNumericValue* value)
{
    DCHECK(value);
...
CSSNumericValue* CalcToNumericValue(const CSSMathExpressionNode& root)
{
    if (root.IsNumericLiteral()) {
        ...
        auto* value = CSSUnitValue::Create(root.DoubleValue(), unit == CSSPrimitiveValue::UnitType::kInteger ? CSSPrimitiveValue::UnitType::kNumber : unit);
        DCHECK(value);
    ...
    // CSSMathMax.
    if (const auto& node = To<CSSMathExpressionOperation>(root); node.IsMathFunction()) {
    ...
    do {
        ...
        const CSSMathExpressionOperation* binary_op = To<CSSMathExpressionOperation>(cur_node);
        ...
        values.push_back(NegateOrInvertIfRequired(binary_op->OperatorType(), value));
        cur_node = left_node;
    ...
    CSSMathOperator operator_type = To<CSSMathExpressionOperation>(root).OperatorType();
    if (operator_type == CSSMathOperator::kAdd || operator_type == CSSMathOperator::kSubtract) {
        return CSSMathSum::Create(std::move(values));
    }
    return CSSMathProduct::Create(std::move(values));
}

CSSUnitValue* CSSNumericSumValueEntryToUnitValue(const CSSNumericSumValue::Term& term)
{
...
bool CSSNumericValue::IsValidUnit(CSSPrimitiveValue::UnitType unit)
{
    ...
    if (unit == CSSPrimitiveValue::UnitType::kNumber || unit == CSSPrimitiveValue::UnitType::kPercentage || CSSPrimitiveValue::IsLength(unit)
        || CSSPrimitiveValue::IsAngle(unit) || CSSPrimitiveValue::IsTime(unit) || CSSPrimitiveValue::IsFrequency(unit) || CSSPrimitiveValue::IsResolution(unit)
...
// static
CSSNumericValue* CSSNumericValue::parse(const ExecutionContext* execution_context, const String& css_text, ExceptionState& exception_state)
{
    ...
    case kFunctionToken:
        if (stream.Peek().FunctionId() == CSSValueID::kCalc || stream.Peek().FunctionId() == CSSValueID::kWebkitCalc
            || stream.Peek().FunctionId() == CSSValueID::kMin || stream.Peek().FunctionId() == CSSValueID::kMax
            ...
            CSSMathExpressionNode* expression = CSSMathExpressionNode::ParseMathFunction(
                CSSValueID::kCalc, stream, *MakeGarbageCollected<CSSParserContext>(*execution_context), Flags({ AllowPercent }), kCSSAnchorQueryTypesNone);
            if (expression) {
    ...
    exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "Invalid math expression");
    return nullptr;
}
...
{
    if (value->IsDouble()) {
        return CSSUnitValue::Create(value->GetAsDouble(), CSSPrimitiveValue::UnitType::kNumber);
    }
...
{
    if (value->IsDouble()) {
        return CSSUnitValue::Create(value->GetAsDouble() * 100, CSSPrimitiveValue::UnitType::kPercentage);
    }
    return value->GetAsCSSNumericValue();
}

CSSUnitValue* CSSNumericValue::to(const String& unit_string, ExceptionState& exception_state)
{
    CSSPrimitiveValue::UnitType target_unit = UnitFromName(unit_string);
    if (!IsValidUnit(target_unit)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "Invalid unit for conversion");
        return nullptr;
    }
...
CSSMathSum* CSSNumericValue::toSum(const Vector<String>& unit_strings, ExceptionState& exception_state)
{
    ...
    if (unit_strings.size() == 0) {
        std::sort(values.begin(), values.end(),
            [](const auto& a, const auto& b) { return WTF::CodeUnitCompareLessThan(To<CSSUnitValue>(a.Get())->unit(), To<CSSUnitValue>(b.Get())->unit()); });
    ...
    for (const auto& unit_string : unit_strings) {
        ...
        double total_value = std::accumulate(values.begin(), values.end(), 0.0, [target_unit](double cur_sum, auto& value) {
            if (value) {
                auto& unit_value = To<CSSUnitValue>(*value);
                if (const auto* converted_value = unit_value.ConvertTo(target_unit)) {
    ...
    if (base::ranges::any_of(values, [](const auto& v) { return v; })) {
        exception_state.ThrowTypeError("There were leftover terms that were not converted");
        return nullptr;
    }
...
CSSNumericType* CSSNumericValue::type() const
{
    ...
    if (type_.HasPercentHint()) {
        type->setPercentHint(CSSNumericValueType::BaseTypeToString(type_.PercentHint()));
    }
    return type;
}

CSSNumericValue* CSSNumericValue::add(const HeapVector<Member<V8CSSNumberish>>& numberishes, ExceptionState& exception_state)
{
    auto values = CSSNumberishesToNumericValues(numberishes);
    PrependValueForArithmetic<kSumType>(values, this);

    if (CSSUnitValue* unit_value = MaybeSimplifyAsUnitValue(values, std::plus<double>())) {
        return unit_value;
    }
    return CSSMathSum::Create(std::move(values), exception_state);
}

CSSNumericValue* CSSNumericValue::sub(const HeapVector<Member<V8CSSNumberish>>& numberishes, ExceptionState& exception_state)
{
    auto values = CSSNumberishesToNumericValues(numberishes);
...
CSSNumericValue* CSSNumericValue::mul(const HeapVector<Member<V8CSSNumberish>>& numberishes, ExceptionState& exception_state)
{
    auto values = CSSNumberishesToNumericValues(numberishes);
...
CSSNumericValue* CSSNumericValue::div(const HeapVector<Member<V8CSSNumberish>>& numberishes, ExceptionState& exception_state)
{
    auto values = CSSNumberishesToNumericValues(numberishes);
    for (auto& v : values) {
...
CSSNumericValue* CSSNumericValue::min(const HeapVector<Member<V8CSSNumberish>>& numberishes, ExceptionState& exception_state)
{
    auto values = CSSNumberishesToNumericValues(numberishes);
    PrependValueForArithmetic<kMinType>(values, this);

    if (CSSUnitValue *unit_value = MaybeSimplifyAsUnitValue(values, [](double a, double b) { return std::min(a, b); })) {
        return unit_value;
    }
    return CSSMathMin::Create(std::move(values));
}

CSSNumericValue* CSSNumericValue::max(const HeapVector<Member<V8CSSNumberish>>& numberishes, ExceptionState& exception_state)
{
    auto values = CSSNumberishesToNumericValues(numberishes);
    PrependValueForArithmetic<kMaxType>(values, this);

    if (CSSUnitValue *unit_value = MaybeSimplifyAsUnitValue(values, [](double a, double b) { return std::max(a, b); })) {
        return unit_value;
    }
    return CSSMathMax::Create(std::move(values));
}

bool CSSNumericValue::equals(const HeapVector<Member<V8CSSNumberish>>& numberishes)
{
    CSSNumericValueVector values = CSSNumberishesToNumericValues(numberishes);
    return base::ranges::all_of(values, [this](const auto& v) { return this->Equals(*v); });
}
...
CSSNumericValueVector CSSNumberishesToNumericValues(const HeapVector<Member<V8CSSNumberish>>& values)
{
    CSSNumericValueVector result;
