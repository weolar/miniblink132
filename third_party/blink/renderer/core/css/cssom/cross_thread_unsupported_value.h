...
class CORE_EXPORT CrossThreadUnsupportedValue final : public CrossThreadStyleValue {
public:
    explicit CrossThreadUnsupportedValue(const String& value)
        : value_(value)
    {
    }
    CrossThreadUnsupportedValue(const CrossThreadUnsupportedValue&) = delete;
    CrossThreadUnsupportedValue& operator=(const CrossThreadUnsupportedValue&) = delete;
...
template <> struct DowncastTraits<CrossThreadUnsupportedValue> {
    static bool AllowFrom(const CrossThreadStyleValue& unsupported_value)
    {
        return unsupported_value.GetType() == CrossThreadStyleValue::StyleValueType::kUnknownType;
    }
};
