...
CSSMathMin* CSSMathMin::Create(const HeapVector<Member<V8CSSNumberish>>& args, ExceptionState& exception_state)
{
    if (args.empty()) {
...
CSSMathMin* CSSMathMin::Create(CSSNumericValueVector values)
{
    ...
    return error ? nullptr : MakeGarbageCollected<CSSMathMin>(MakeGarbageCollected<CSSNumericArray>(std::move(values)), final_type);
}
...
std::optional<CSSNumericSumValue> CSSMathMin::SumValue() const
{
    ...
    for (const auto& value : NumericValues()) {
        ...
        if (!child_sum.has_value() || child_sum->terms.size() != 1 || child_sum->terms[0].units != cur_min->terms[0].units) {
            return std::nullopt;
        }
...
CSSMathExpressionNode* CSSMathMin::ToCalcExpressionNode() const
{
    ...
    return CSSMathExpressionOperation::CreateComparisonFunction(std::move(operands), CSSMathOperator::kMin);
}
