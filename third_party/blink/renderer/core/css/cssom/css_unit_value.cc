...
CSSPrimitiveValue::UnitType ToCanonicalUnitIfPossible(CSSPrimitiveValue::UnitType unit)
{
...
bool IsValueOutOfRangeForProperty(CSSPropertyID property_id, double value, CSSPrimitiveValue::UnitType unit)
{
    ...
    if (LengthPropertyFunctions::GetValueRange(CSSProperty::Get(property_id)) == Length::ValueRange::kNonNegative && value < 0) {
        return true;
    }
    ...
    case CSSPropertyID::kTabSize:
        return value < 0 || (unit == CSSPrimitiveValue::UnitType::kNumber && round(value) != value);
...
CSSUnitValue* CSSUnitValue::Create(double value, const String& unit_name, ExceptionState& exception_state)
{
...
CSSUnitValue* CSSUnitValue::Create(double value, CSSPrimitiveValue::UnitType unit)
{
...
CSSUnitValue* CSSUnitValue::ConvertTo(CSSPrimitiveValue::UnitType target_unit) const
{
    ...
    if (canonical_unit != ToCanonicalUnit(target_unit) || canonical_unit == CSSPrimitiveValue::UnitType::kUnknown) {
        return nullptr;
    }

    const double scale_factor
        = CSSPrimitiveValue::ConversionToCanonicalUnitsScaleFactor(unit_) / CSSPrimitiveValue::ConversionToCanonicalUnitsScaleFactor(target_unit);
...
std::optional<CSSNumericSumValue> CSSUnitValue::SumValue() const
{
    ...
    sum.terms.emplace_back(value_ * CSSPrimitiveValue::ConversionToCanonicalUnitsScaleFactor(unit_), std::move(unit_map));
    return sum;
}
...
const CSSPrimitiveValue* CSSUnitValue::ToCSSValueWithProperty(CSSPropertyID property_id) const
{
...
{
    return CSSMathExpressionNumericLiteral::Create(CSSNumericLiteralValue::Create(value_, unit_));
}
...
void CSSUnitValue::BuildCSSText(Nested, ParenLess, StringBuilder& result) const
{
