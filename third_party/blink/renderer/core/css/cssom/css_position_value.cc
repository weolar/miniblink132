...
{
    return v && v->Type().MatchesBaseTypePercentage(CSSNumericValueType::BaseType::kLength);
}
...
CSSNumericValue* FromSingleValue(const CSSValue& value)
{
    if (const auto* ident = DynamicTo<CSSIdentifierValue>(value)) {
        ...
        case CSSValueID::kTop:
            return CSSUnitValue::Create(0, CSSPrimitiveValue::UnitType::kPercentage);
        case CSSValueID::kCenter:
            return CSSUnitValue::Create(50, CSSPrimitiveValue::UnitType::kPercentage);
        case CSSValueID::kRight:
        case CSSValueID::kBottom:
            return CSSUnitValue::Create(100, CSSPrimitiveValue::UnitType::kPercentage);
        default:
            NOTREACHED();
        }
    }
    ...
    CSSNumericValue* offset = CSSNumericValue::FromCSSValue(To<CSSPrimitiveValue>(pair.Second()));
    DCHECK(offset);
    ...
    case CSSValueID::kBottom: {
        ...
        args.push_back(CSSUnitValue::Create(100, CSSPrimitiveValue::UnitType::kPercentage));
...
CSSPositionValue* CSSPositionValue::Create(CSSNumericValue* x, CSSNumericValue* y, ExceptionState& exception_state)
{
    if (!IsValidPositionCoord(x)) {
        exception_state.ThrowTypeError("Must pass length or percentage to x in CSSPositionValue");
        return nullptr;
    }
    if (!IsValidPositionCoord(y)) {
        exception_state.ThrowTypeError("Must pass length or percentage to y in CSSPositionValue");
        return nullptr;
    }
    return MakeGarbageCollected<CSSPositionValue>(x, y);
}

CSSPositionValue* CSSPositionValue::Create(CSSNumericValue* x, CSSNumericValue* y)
{
...
void CSSPositionValue::setX(CSSNumericValue* x, ExceptionState& exception_state)
{
    if (!IsValidPositionCoord(x)) {
        exception_state.ThrowTypeError("Must pass length or percentage to x in CSSPositionValue");
        return;
...
void CSSPositionValue::setY(CSSNumericValue* y, ExceptionState& exception_state)
{
    if (!IsValidPositionCoord(y)) {
        exception_state.ThrowTypeError("Must pass length or percentage to y in CSSPositionValue");
        return;
...
const CSSValue* CSSPositionValue::ToCSSValue() const
{
    ...
    return MakeGarbageCollected<CSSValuePair>(x, y, CSSValuePair::kKeepIdenticalValues);
}
