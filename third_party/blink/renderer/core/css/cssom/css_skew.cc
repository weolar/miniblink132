...
CSSSkew* CSSSkew::Create(CSSNumericValue* ax, CSSNumericValue* ay, ExceptionState& exception_state)
{
...
CSSSkew* CSSSkew::FromCSSValue(const CSSFunctionValue& value)
{
    ...
    if (value.length() == 1U) {
        return CSSSkew::Create(CSSNumericValue::FromCSSValue(x_value), CSSUnitValue::Create(0, CSSPrimitiveValue::UnitType::kDegrees));
    } else if (value.length() == 2U) {
        const auto& y_value = To<CSSPrimitiveValue>(value.Item(1));
        return CSSSkew::Create(CSSNumericValue::FromCSSValue(x_value), CSSNumericValue::FromCSSValue(y_value));
    }
    NOTREACHED();
}
...
const CSSFunctionValue* CSSSkew::ToCSSValue() const
{
    ...
    CSSFunctionValue* result = MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kSkew);
    result->Append(*ax);
