...
CSSMathMax* CSSMathMax::Create(const HeapVector<Member<V8CSSNumberish>>& args, ExceptionState& exception_state)
{
    if (args.empty()) {
...
CSSMathMax* CSSMathMax::Create(CSSNumericValueVector values)
{
    ...
    return error ? nullptr : MakeGarbageCollected<CSSMathMax>(MakeGarbageCollected<CSSNumericArray>(std::move(values)), final_type);
}
...
std::optional<CSSNumericSumValue> CSSMathMax::SumValue() const
{
    ...
    for (const auto& value : NumericValues()) {
        ...
        if (!child_sum.has_value() || child_sum->terms.size() != 1 || child_sum->terms[0].units != cur_max->terms[0].units) {
            return std::nullopt;
        }
...
CSSMathExpressionNode* CSSMathMax::ToCalcExpressionNode() const
{
    ...
    return CSSMathExpressionOperation::CreateComparisonFunction(std::move(operands), CSSMathOperator::kMax);
}
