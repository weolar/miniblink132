...
public:
    CSSCursorImageValue(const CSSValue& image_value, bool hot_spot_specified, const gfx::Point& hot_spot);

    bool HotSpotSpecified() const
    {
        return hot_spot_specified_;
    }
    const gfx::Point& HotSpot() const
    {
        return hot_spot_;
    }
    const CSSValue& ImageValue() const
    {
        return *image_value_;
    }
...
template <> struct DowncastTraits<cssvalue::CSSCursorImageValue> {
