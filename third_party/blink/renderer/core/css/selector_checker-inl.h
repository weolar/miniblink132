...
bool EasySelectorChecker::IsEasy(const CSSSelector* selector)
{
    ...
    for (; selector != nullptr; selector = selector->NextSimpleSelector()) {
        if (!selector->IsLastInComplexSelector() && selector->Relation() != CSSSelector::kSubSelector && selector->Relation() != CSSSelector::kDescendant
        ...
        } else if (selector->Relation() == CSSSelector::kChild && has_descendant_selector) {
        ...
        case CSSSelector::kTag: {
            ...
            if (tag_q_name == AnyQName() || tag_q_name.LocalName() == CSSSelector::UniversalSelectorAtom()) {
...
bool EasySelectorChecker::MatchOne(const CSSSelector* selector, const Element* element)
{
    ...
    case CSSSelector::kTag: {
        const QualifiedName& tag_q_name = selector->TagQName();
        if (element->namespaceURI() != tag_q_name.NamespaceURI() && tag_q_name.NamespaceURI() != g_star_atom) {
            // Namespace mismatch.
            return false;
        }
    ...
    case CSSSelector::kAttributeExact: {
        bool case_insensitive = selector->AttributeMatch() == CSSSelector::AttributeMatchType::kCaseInsensitive
            || (selector->LegacyCaseInsensitiveMatch() && IsA<HTMLDocument>(element->GetDocument()));
        return AttributeMatches(*element, selector->Attribute(), selector->Value(), case_insensitive);
    }
    default:
        NOTREACHED();
    }
...
bool EasySelectorChecker::AttributeIsSet(const Element& element, const QualifiedName& attr)
{
...
bool EasySelectorChecker::AttributeMatches(const Element& element, const QualifiedName& attr, const AtomicString& value, bool case_insensitive)
{
    element.SynchronizeAttribute(attr.LocalName());
    AttributeCollection attributes = element.AttributesWithoutUpdate();
    for (const auto& attribute_item : attributes) {
        if (AttributeItemHasName(attribute_item, element, attr)) {
            return attribute_item.Value() == value || (case_insensitive && EqualIgnoringASCIICase(attribute_item.Value(), value));
        }
    }
    return false;
}

bool EasySelectorChecker::AttributeItemHasName(const Attribute& attribute_item, const Element& element, const QualifiedName& name)
{
    ...
    return attribute_item.Matches(name)
        || (!element.IsHTMLElement() && IsA<HTMLDocument>(element.GetDocument()) && attribute_item.MatchesCaseInsensitive(name));
}
