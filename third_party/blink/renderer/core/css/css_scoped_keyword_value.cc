...
const CSSScopedKeywordValue& CSSScopedKeywordValue::PopulateWithTreeScope(const TreeScope* tree_scope) const
{
    DCHECK(needs_tree_scope_population_);
    CSSScopedKeywordValue* populated = MakeGarbageCollected<CSSScopedKeywordValue>(*this);
