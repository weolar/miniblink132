...
public:
    ...
    void CollectSiblingInvalidationSetForClass(InvalidationLists&, Element&, const AtomicString& class_name, unsigned min_direct_adjacent) const;
    void CollectSiblingInvalidationSetForId(InvalidationLists&, Element&, const AtomicString& id, unsigned min_direct_adjacent) const;
    void CollectSiblingInvalidationSetForAttribute(InvalidationLists&, Element&, const QualifiedName& attribute_name, unsigned min_direct_adjacent) const;
    ...
    {
        return not_pseudo_in_has_argument || universal_in_has_argument || !tag_names_in_has_argument.empty() || NeedsHasInvalidationForClassChange()
            || NeedsHasInvalidationForAttributeChange() || NeedsHasInvalidationForIdChange() || NeedsHasInvalidationForPseudoStateChange();
    }
    ...
    {
        return id_invalidation_sets.size() > 0;
    }
    bool InvalidatesParts() const
    {
        return invalidates_parts;
    }
    // Returns true if we have :nth-child(... of S) selectors where S contains a
    // :has() selector.
    bool UsesHasInsideNth() const
    {
        return uses_has_inside_nth;
    }
    bool UsesFirstLineRules() const
    {
        return uses_first_line_rules;
    }
...
private:
    static void ExtractInvalidationSets(InvalidationSet* invalidation_set, DescendantInvalidationSet*& descendants, SiblingInvalidationSet*& siblings);
    ...
    using InvalidationSetMap = HashMap<AtomicString, scoped_refptr<InvalidationSet>>;
    using PseudoTypeInvalidationSetMap = HashMap<CSSSelector::PseudoType, scoped_refptr<InvalidationSet>, IntWithZeroKeyHashTraits<unsigned>>;
    ...
    template <RuleInvalidationDataVisitorType VisitorType> friend class RuleInvalidationDataVisitor;
};
