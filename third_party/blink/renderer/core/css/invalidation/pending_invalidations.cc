...
void PendingInvalidations::ScheduleInvalidationSetsForNode(const InvalidationLists& invalidation_lists, ContainerNode& node)
{
    ...
    if (node.GetStyleChangeType() < kSubtreeStyleChange) {
        for (auto& invalidation_set : invalidation_lists.descendants) {
            ...
            if (invalidation_set->WholeSubtreeInvalid()) {
                ...
                if (subtree_root->IsElementNode()) {
                    TRACE_STYLE_INVALIDATOR_INVALIDATION_SET(To<Element>(*subtree_root), kInvalidationSetInvalidatesSubtree, *invalidation_set);
                }
                subtree_root->SetNeedsStyleRecalc(kSubtreeStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kRelatedStyleRule));
                requires_descendant_invalidation = false;
                break;
            }

            if (invalidation_set->InvalidatesSelf() && node.IsElementNode()) {
                TRACE_STYLE_INVALIDATOR_INVALIDATION_SET(To<Element>(node), kInvalidationSetInvalidatesSelf, *invalidation_set);
                node.SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kRelatedStyleRule));
            }
        ...
        if (requires_descendant_invalidation && node.IsElementNode() && !To<Element>(node).GetComputedStyle()) {
            requires_descendant_invalidation = false;
        }
    }
...
void PendingInvalidations::ScheduleSiblingInvalidationsAsDescendants(const InvalidationLists& invalidation_lists, ContainerNode& scheduling_parent)
{
    ...
    for (auto& invalidation_set : invalidation_lists.siblings) {
        DescendantInvalidationSet* descendants = To<SiblingInvalidationSet>(*invalidation_set).SiblingDescendants();
        bool whole_subtree_invalid = false;
        if (invalidation_set->WholeSubtreeInvalid()) {
            TRACE_STYLE_INVALIDATOR_INVALIDATION_SET(*subtree_root, kInvalidationSetInvalidatesSubtree, *invalidation_set);
            whole_subtree_invalid = true;
        } else if (descendants && descendants->WholeSubtreeInvalid()) {
            TRACE_STYLE_INVALIDATOR_INVALIDATION_SET(*subtree_root, kInvalidationSetInvalidatesSubtree, *descendants);
            whole_subtree_invalid = true;
        }
        ...
        if (invalidation_set->InvalidatesSelf() && !pending_invalidations.Descendants().Contains(invalidation_set)) {
            pending_invalidations.Descendants().push_back(invalidation_set);
        }

        if (descendants && !pending_invalidations.Descendants().Contains(descendants)) {
...
void PendingInvalidations::RescheduleSiblingInvalidationsAsDescendants(Element& element)
{
    ...
    if (pending_invalidations_iterator == pending_invalidation_map_.end() || pending_invalidations_iterator->value.Siblings().empty()) {
        return;
    }
    ...
    for (const auto& invalidation_set : pending_invalidations.Siblings()) {
        ...
        if (DescendantInvalidationSet* descendants = To<SiblingInvalidationSet>(*invalidation_set).SiblingDescendants()) {
...
NodeInvalidationSets& PendingInvalidations::EnsurePendingInvalidations(ContainerNode& node)
{
    ...
    PendingInvalidationMap::AddResult add_result = pending_invalidation_map_.insert(&node, NodeInvalidationSets());
