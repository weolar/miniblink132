...
template <typename KeyType, typename MapType = HashMap<KeyType, scoped_refptr<InvalidationSet>>>
...
bool RuleInvalidationData::operator==(const RuleInvalidationData& other) const
{
    return InvalidationSetMapsEqual<AtomicString>(class_invalidation_sets, other.class_invalidation_sets)
        && base::ValuesEquivalent(names_with_self_invalidation, other.names_with_self_invalidation)
        ...
        && base::ValuesEquivalent(universal_sibling_invalidation_set, other.universal_sibling_invalidation_set)
        && base::ValuesEquivalent(nth_invalidation_set, other.nth_invalidation_set)
        && base::ValuesEquivalent(universal_sibling_invalidation_set, other.universal_sibling_invalidation_set)
        && classes_in_has_argument == other.classes_in_has_argument && attributes_in_has_argument == other.attributes_in_has_argument
        && ids_in_has_argument == other.ids_in_has_argument && tag_names_in_has_argument == other.tag_names_in_has_argument
        && max_direct_adjacent_selectors == other.max_direct_adjacent_selectors && uses_first_line_rules == other.uses_first_line_rules
        && uses_window_inactive_selector == other.uses_window_inactive_selector && universal_in_has_argument == other.universal_in_has_argument
        && not_pseudo_in_has_argument == other.not_pseudo_in_has_argument && pseudos_in_has_argument == other.pseudos_in_has_argument
...
void RuleInvalidationData::CollectInvalidationSetsForClass(InvalidationLists& invalidation_lists, Element& element, const AtomicString& class_name) const
{
    // Implicit self-invalidation sets for all classes (with Bloom filter
    // rejection); see comment on class_invalidation_sets_.
    if (names_with_self_invalidation && names_with_self_invalidation->MayContain(class_name.Hash() * kClassSalt)) {
    ...
    if (descendants) {
        TRACE_SCHEDULE_STYLE_INVALIDATION(element, *descendants, ClassChange, class_name);
        invalidation_lists.descendants.push_back(descendants);
    }

    if (siblings) {
        TRACE_SCHEDULE_STYLE_INVALIDATION(element, *siblings, ClassChange, class_name);
        invalidation_lists.siblings.push_back(siblings);
    }
}

void RuleInvalidationData::CollectSiblingInvalidationSetForClass(
    InvalidationLists& invalidation_lists, Element& element, const AtomicString& class_name, unsigned min_direct_adjacent) const
{
    RuleInvalidationData::InvalidationSetMap::const_iterator it = class_invalidation_sets.find(class_name);
    if (it == class_invalidation_sets.end()) {
        return;
    ...
    TRACE_SCHEDULE_STYLE_INVALIDATION(element, *sibling_set, ClassChange, class_name);
    invalidation_lists.siblings.push_back(sibling_set);
}

void RuleInvalidationData::CollectInvalidationSetsForId(InvalidationLists& invalidation_lists, Element& element, const AtomicString& id) const
{
    if (names_with_self_invalidation && names_with_self_invalidation->MayContain(id.Hash() * kIdSalt)) {
        invalidation_lists.descendants.push_back(InvalidationSet::SelfInvalidationSet());
    }
...
void RuleInvalidationData::CollectSiblingInvalidationSetForId(
    InvalidationLists& invalidation_lists, Element& element, const AtomicString& id, unsigned min_direct_adjacent) const
{
    RuleInvalidationData::InvalidationSetMap::const_iterator it = id_invalidation_sets.find(id);
    if (it == id_invalidation_sets.end()) {
        return;
...
void RuleInvalidationData::CollectInvalidationSetsForAttribute(
    InvalidationLists& invalidation_lists, Element& element, const QualifiedName& attribute_name) const
{
    RuleInvalidationData::InvalidationSetMap::const_iterator it = attribute_invalidation_sets.find(attribute_name.LocalName());
    ...
    if (descendants) {
        TRACE_SCHEDULE_STYLE_INVALIDATION(element, *descendants, AttributeChange, attribute_name);
        invalidation_lists.descendants.push_back(descendants);
    }

    if (siblings) {
        TRACE_SCHEDULE_STYLE_INVALIDATION(element, *siblings, AttributeChange, attribute_name);
        invalidation_lists.siblings.push_back(siblings);
    }
}

void RuleInvalidationData::CollectSiblingInvalidationSetForAttribute(
    InvalidationLists& invalidation_lists, Element& element, const QualifiedName& attribute_name, unsigned min_direct_adjacent) const
{
    RuleInvalidationData::InvalidationSetMap::const_iterator it = attribute_invalidation_sets.find(attribute_name.LocalName());
    if (it == attribute_invalidation_sets.end()) {
        return;
    ...
    TRACE_SCHEDULE_STYLE_INVALIDATION(element, *sibling_set, AttributeChange, attribute_name);
    invalidation_lists.siblings.push_back(sibling_set);
}

void RuleInvalidationData::CollectInvalidationSetsForPseudoClass(InvalidationLists& invalidation_lists, Element& element, CSSSelector::PseudoType pseudo) const
{
...
void RuleInvalidationData::CollectUniversalSiblingInvalidationSet(InvalidationLists& invalidation_lists, unsigned min_direct_adjacent) const
{
    if (universal_sibling_invalidation_set && universal_sibling_invalidation_set->MaxDirectAdjacentSelectors() >= min_direct_adjacent) {
...
{
    return universal_in_has_argument || tag_names_in_has_argument.Contains(tag_name);
}
...
String RuleInvalidationData::ToString() const
{
    ...
    auto add_invalidation_sets = [&entries](const String& base, InvalidationSet* set, unsigned flags, const char* prefix = "", const char* suffix = "") {
        if (!set) {
            return;
        }
        DescendantInvalidationSet* descendants;
    ...
    for (auto& i : pseudo_invalidation_sets) {
        String name = CSSSelector::FormatPseudoTypeForDebugging(static_cast<CSSSelector::PseudoType>(i.key));
...
void RuleInvalidationData::ExtractInvalidationSets(
    InvalidationSet* invalidation_set, DescendantInvalidationSet*& descendants, SiblingInvalidationSet*& siblings)
{
