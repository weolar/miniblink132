...
{
    GetDocument().body()->setInnerHTML("<div id='d'></div><i id='i'></i><span></span>");
    GetDocument().View()->UpdateAllLifecyclePhasesForTest();

    unsigned before_count = GetStyleEngine().StyleForElementCount();

    scoped_refptr<DescendantInvalidationSet> set = DescendantInvalidationSet::Create();
...
TEST_F(PendingInvalidationsTest, DescendantInvalidationOnDisplayNone)
{
    ...
    GetDocument().getElementById(AtomicString("a"))->setAttribute(html_names::kClassAttr, AtomicString("a"));
