...
TEST_F(StyleInvalidatorTest, SkipDisplayNone)
{
    ...
    PendingInvalidations pending;
    {
        ...
        pending.ScheduleInvalidationSetsForNode(lists, *GetDocument().getElementById(AtomicString("root")));
    }
...
TEST_F(StyleInvalidatorTest, SkipDisplayNoneClearPendingNth)
{
    ...
    PendingInvalidations pending;
    {
        ...
        pending.ScheduleInvalidationSetsForNode(lists, *GetDocument().getElementById(AtomicString("none")));
    }
    {
        InvalidationLists lists;
        scoped_refptr<InvalidationSet> set = DescendantInvalidationSet::Create();
        set->AddClass(AtomicString("a"));
        lists.descendants.push_back(set);
        pending.ScheduleInvalidationSetsForNode(lists, *GetDocument().getElementById(AtomicString("descendant")));
    }
    ...
    EXPECT_FALSE(GetDocument().getElementById(AtomicString("none"))->ChildNeedsStyleRecalc());
    EXPECT_TRUE(GetDocument().getElementById(AtomicString("descendant"))->ChildNeedsStyleRecalc());
}
