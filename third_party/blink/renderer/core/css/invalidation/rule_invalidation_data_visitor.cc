...
{
    return pseudo == CSSSelector::kPseudoAny || pseudo == CSSSelector::kPseudoCue || pseudo == CSSSelector::kPseudoHost
        || pseudo == CSSSelector::kPseudoHostContext || pseudo == CSSSelector::kPseudoIs || pseudo == CSSSelector::kPseudoNot
        || pseudo == CSSSelector::kPseudoSlotted || pseudo == CSSSelector::kPseudoWhere || pseudo == CSSSelector::kPseudoParent
        || pseudo == CSSSelector::kPseudoNthChild || pseudo == CSSSelector::kPseudoNthLastChild;
}

bool RequiresSubtreeInvalidation(const CSSSelector& selector)
{
    if (selector.Match() != CSSSelector::kPseudoElement && selector.Match() != CSSSelector::kPseudoClass) {
...
scoped_refptr<InvalidationSet> CopyInvalidationSet(const InvalidationSet& invalidation_set)
{
    if (invalidation_set.IsSiblingInvalidationSet()) {
        scoped_refptr<InvalidationSet> copy = SiblingInvalidationSet::Create(nullptr);
...
bool IsSimpleSelectorValidAfterHost(const CSSSelector* simple_selector)
{
    ...
    // (e.g. ':host:not(:has(.a))')
    return simple_selector->Match() == CSSSelector::kPseudoElement || simple_selector->IsHostPseudoClass()
...
RuleInvalidationDataVisitor<VisitorType>::RuleInvalidationDataVisitor(RuleInvalidationDataType& rule_invalidation_data)
    : rule_invalidation_data_(rule_invalidation_data)
{
}

template <RuleInvalidationDataVisitorType VisitorType>
void RuleInvalidationDataVisitor<VisitorType>::InvalidationSetFeatures::Merge(const InvalidationSetFeatures& other)
{
...
template <RuleInvalidationDataVisitorType VisitorType> bool RuleInvalidationDataVisitor<VisitorType>::InvalidationSetFeatures::HasFeatures() const
{
...
template <RuleInvalidationDataVisitorType VisitorType> bool RuleInvalidationDataVisitor<VisitorType>::InvalidationSetFeatures::HasIdClassOrAttribute() const
{
    return !classes.empty() || !attributes.empty() || !ids.empty();
}

template <RuleInvalidationDataVisitorType VisitorType>
void RuleInvalidationDataVisitor<VisitorType>::InvalidationSetFeatures::NarrowToFeatures(const InvalidationSetFeatures& other)
{
    unsigned size = Size();
...
template <RuleInvalidationDataVisitorType VisitorType>
SelectorPreMatch RuleInvalidationDataVisitor<VisitorType>::CollectFeaturesFromSelector(const CSSSelector& selector, const StyleScope* style_scope)
{
    FeatureMetadata metadata;
    const unsigned max_direct_adjacent_selectors = 0;
    if (CollectMetadataFromSelector(selector, max_direct_adjacent_selectors, metadata) == SelectorPreMatch::kNeverMatches) {
        return SelectorPreMatch::kNeverMatches;
    }
    if constexpr (is_builder()) {
        rule_invalidation_data_.uses_first_line_rules |= metadata.uses_first_line_rules;
        rule_invalidation_data_.uses_window_inactive_selector |= metadata.uses_window_inactive_selector;
        rule_invalidation_data_.max_direct_adjacent_selectors
            = std::max(rule_invalidation_data_.max_direct_adjacent_selectors, metadata.max_direct_adjacent_selectors);
    }
...
template <RuleInvalidationDataVisitorType VisitorType>
SelectorPreMatch RuleInvalidationDataVisitor<VisitorType>::CollectMetadataFromSelector(
    const CSSSelector& selector, unsigned max_direct_adjacent_selectors, FeatureMetadata& metadata)
{
    ...
    for (const CSSSelector* current = &selector; current; current = current->NextSimpleSelector()) {
        ...
        case CSSSelector::kPseudoHas:
            if (found_host_pseudo && !current->IsLastInComplexSelector() && !IsSimpleSelectorValidAfterHost(current->NextSimpleSelector())) {
                return SelectorPreMatch::kNeverMatches;
            }
            break;
        case CSSSelector::kPseudoFirstLine:
        ...
        case CSSSelector::kPseudoHostContext:
            ...
            if (!current->IsLastInComplexSelector() && !IsSimpleSelectorValidAfterHost(current->NextSimpleSelector())) {
                return SelectorPreMatch::kNeverMatches;
            }
        ...
        default:
            for (const CSSSelector* sub_selector = current->SelectorListOrParent(); sub_selector; sub_selector = CSSSelectorList::Next(*sub_selector)) {
                CollectMetadataFromSelector(*sub_selector, max_direct_adjacent_selectors, metadata);
            }
            break;
        }
        ...
        } else if (max_direct_adjacent_selectors && ((relation != CSSSelector::kSubSelector) || current->IsLastInComplexSelector())) {
...
template <RuleInvalidationDataVisitorType VisitorType>
void RuleInvalidationDataVisitor<VisitorType>::UpdateInvalidationSets(const CSSSelector& selector, const StyleScope* style_scope)
{
    STACK_UNINITIALIZED InvalidationSetFeatures features;
    FeatureInvalidationType feature_invalidation_type
        = UpdateInvalidationSetsForComplex(selector, /*in_nth_child=*/false, style_scope, features, kSubject, CSSSelector::kPseudoUnknown);
    if (feature_invalidation_type == FeatureInvalidationType::kRequiresSubtreeInvalidation) {
...
// (e.g. for :not() or :has()).
template <RuleInvalidationDataVisitorType VisitorType>
RuleInvalidationDataVisitor<VisitorType>::FeatureInvalidationType RuleInvalidationDataVisitor<VisitorType>::UpdateInvalidationSetsForComplex(
    const CSSSelector& complex, bool in_nth_child, const StyleScope* style_scope, InvalidationSetFeatures& features, PositionType position,
    CSSSelector::PseudoType pseudo_type)
{
    ...
    // at this point.
    const CSSSelector* last_in_compound = ExtractInvalidationSetFeaturesFromCompound(complex, features, position,
        /* for_logical_combination_in_has */ false, in_nth_child);

    bool was_whole_subtree_invalid = features.invalidation_flags.WholeSubtreeInvalid();
    ...
    // Step 2.
    const CSSSelector* next_compound = last_in_compound ? last_in_compound->NextSimpleSelector() : &complex;

    if (next_compound) {
        if (last_in_compound) {
            UpdateFeaturesFromCombinator(last_in_compound->Relation(), nullptr, features, sibling_features, features,
                /* for_logical_combination_in_has */ false, in_nth_child);
        }

        AddFeaturesToInvalidationSets(*next_compound, in_nth_child, sibling_features, features);
...
template <RuleInvalidationDataVisitorType VisitorType>
void RuleInvalidationDataVisitor<VisitorType>::UpdateFeaturesFromCombinator(CSSSelector::RelationType combinator,
    const CSSSelector* last_compound_in_adjacent_chain, InvalidationSetFeatures& last_compound_in_adjacent_chain_features,
    InvalidationSetFeatures*& sibling_features, InvalidationSetFeatures& descendant_features, bool for_logical_combination_in_has, bool in_nth_child)
{
        ...
        if (!sibling_features) {
            ...
            if (last_compound_in_adjacent_chain) {
                ExtractInvalidationSetFeaturesFromCompound(
                    *last_compound_in_adjacent_chain, last_compound_in_adjacent_chain_features, kAncestor, for_logical_combination_in_has, in_nth_child);
                if (!last_compound_in_adjacent_chain_features.HasFeatures()) {
                    last_compound_in_adjacent_chain_features.invalidation_flags.SetWholeSubtreeInvalid(true);
                }
            }
        }
        if (sibling_features->max_direct_adjacent_selectors == SiblingInvalidationSet::kDirectAdjacentMax) {
            return;
        }
        if (combinator == CSSSelector::kDirectAdjacent) {
            ++sibling_features->max_direct_adjacent_selectors;
        } else {
            sibling_features->max_direct_adjacent_selectors = SiblingInvalidationSet::kDirectAdjacentMax;
        }
        return;
    }
    ...
    if (sibling_features && last_compound_in_adjacent_chain_features.max_direct_adjacent_selectors) {
...
template <RuleInvalidationDataVisitorType VisitorType>
void RuleInvalidationDataVisitor<VisitorType>::UpdateFeaturesFromStyleScope(const StyleScope& style_scope, InvalidationSetFeatures& descendant_features)
{
    auto add_features = [this](const CSSSelector* selector_list, InvalidationSetFeatures& descendant_features) {
        for (const CSSSelector* selector = selector_list; selector; selector = CSSSelectorList::Next(*selector)) {
            InvalidationSetFeatures scope_features;
            ExtractInvalidationSetFeaturesFromCompound(*selector, scope_features, kSubject,
...
template <RuleInvalidationDataVisitorType VisitorType>
void RuleInvalidationDataVisitor<VisitorType>::ExtractInvalidationSetFeaturesFromSimpleSelector(const CSSSelector& selector, InvalidationSetFeatures& features)
{
    features.has_features_for_rule_set_invalidation |= selector.IsIdClassOrAttributeSelector();

    if (selector.Match() == CSSSelector::kTag && selector.TagQName().LocalName() != CSSSelector::UniversalSelectorAtom()) {
...
template <RuleInvalidationDataVisitorType VisitorType>
const CSSSelector* RuleInvalidationDataVisitor<VisitorType>::ExtractInvalidationSetFeaturesFromCompound(
    const CSSSelector& compound, InvalidationSetFeatures& features, PositionType position, bool for_logical_combination_in_has, bool in_nth_child)
{
    ...
    for (;; simple_selector = simple_selector->NextSimpleSelector()) {
        ...
        ExtractInvalidationSetFeaturesFromSimpleSelector(*simple_selector, features);
        ...
        if (InvalidationSetType* invalidation_set
            = InvalidationSetForSimpleSelector(*simple_selector, InvalidationType::kInvalidateDescendants, position, in_nth_child)) {
        ...
        ExtractInvalidationSetFeaturesFromSelectorList(*simple_selector, in_nth_child, features, position);
        ...
        if (simple_selector->GetPseudoType() == CSSSelector::kPseudoHas && !for_logical_combination_in_has) {
            CollectValuesInHasArgument(*simple_selector);
            AddFeaturesToInvalidationSetsForHasPseudoClass(*simple_selector, &compound, nullptr, features, in_nth_child);
        }

        if (!simple_selector->NextSimpleSelector()
            || (simple_selector->Relation() != CSSSelector::kSubSelector && simple_selector->Relation() != CSSSelector::kScopeActivation)) {
...
template <RuleInvalidationDataVisitorType VisitorType>
void RuleInvalidationDataVisitor<VisitorType>::ExtractInvalidationSetFeaturesFromSelectorList(
    const CSSSelector& simple_selector, bool in_nth_child, InvalidationSetFeatures& features, PositionType position)
{
    ...
    for (; sub_selector; sub_selector = CSSSelectorList::Next(*sub_selector)) {
        ...
        if (UpdateInvalidationSetsForComplex(*sub_selector, in_nth_child, nullptr /* style_scope */, complex_features, position, pseudo_type)
...
void RuleInvalidationDataVisitor<VisitorType>::AddFeaturesToInvalidationSets(
    const CSSSelector& selector, bool in_nth_child, InvalidationSetFeatures* sibling_features, InvalidationSetFeatures& descendant_features)
{
    ...
    while (compound) {
        const CSSSelector* last_in_compound = AddFeaturesToInvalidationSetsForCompoundSelector(*compound, in_nth_child, sibling_features, descendant_features);
        DCHECK(last_in_compound);
        UpdateFeaturesFromCombinator(last_in_compound->Relation(), compound, last_compound_in_sibling_chain_features, sibling_features, descendant_features,
            /* for_logical_combination_in_has */ false, in_nth_child);
...
template <RuleInvalidationDataVisitorType VisitorType>
const CSSSelector* RuleInvalidationDataVisitor<VisitorType>::AddFeaturesToInvalidationSetsForCompoundSelector(
    const CSSSelector& compound, bool in_nth_child, InvalidationSetFeatures* sibling_features, InvalidationSetFeatures& descendant_features)
{
    bool compound_has_features_for_rule_set_invalidation = false;
    const CSSSelector* simple_selector = &compound;
    for (; simple_selector; simple_selector = simple_selector->NextSimpleSelector()) {
        base::AutoReset<bool> reset_has_features(&descendant_features.has_features_for_rule_set_invalidation, false);
        AddFeaturesToInvalidationSetsForSimpleSelector(*simple_selector, compound, in_nth_child, sibling_features, descendant_features);
    ...
    if (compound_has_features_for_rule_set_invalidation) {
        ...
        AddFeaturesToUniversalSiblingInvalidationSet(*sibling_features, descendant_features);
    }
...
template <RuleInvalidationDataVisitorType VisitorType>
void RuleInvalidationDataVisitor<VisitorType>::AddFeaturesToInvalidationSetsForSimpleSelector(const CSSSelector& simple_selector, const CSSSelector& compound,
    bool in_nth_child, InvalidationSetFeatures* sibling_features, InvalidationSetFeatures& descendant_features)
{
    ...
    if (pseudo_type == CSSSelector::kPseudoHas) {
        CollectValuesInHasArgument(simple_selector);
        AddFeaturesToInvalidationSetsForHasPseudoClass(simple_selector, &compound, sibling_features, descendant_features, in_nth_child);
    ...
    if (InvalidationSetType* invalidation_set = InvalidationSetForSimpleSelector(
            simple_selector, sibling_features ? InvalidationType::kInvalidateSiblings : InvalidationType::kInvalidateDescendants, kAncestor, in_nth_child)) {
        if (!sibling_features) {
            if (invalidation_set == rule_invalidation_data_.nth_invalidation_set) {
                // TODO(futhark): We can extract the features from the current compound
                // to optimize this.
                SetWholeSubtreeInvalid(invalidation_set);
                AddFeaturesToInvalidationSet(EnsureSiblingDescendantInvalidationSet(To<SiblingInvalidationSet>(invalidation_set)), descendant_features);
                return;
            } else {
        ...
        auto* sibling_invalidation_set = To<SiblingInvalidationSet>(invalidation_set);
        UpdateMaxDirectAdjacentSelectors(sibling_invalidation_set, sibling_features->max_direct_adjacent_selectors);
        ...
        if (sibling_features == &descendant_features) {
            ...
            AddFeaturesToInvalidationSet(EnsureSiblingDescendantInvalidationSet(sibling_invalidation_set), descendant_features);
        }
        return;
    }
    ...
    AddFeaturesToInvalidationSetsForSelectorList(simple_selector, in_nth_child, sibling_features, descendant_features);
}

template <RuleInvalidationDataVisitorType VisitorType>
void RuleInvalidationDataVisitor<VisitorType>::AddFeaturesToInvalidationSetsForSelectorList(
    const CSSSelector& simple_selector, bool in_nth_child, InvalidationSetFeatures* sibling_features, InvalidationSetFeatures& descendant_features)
{
    ...
    bool had_features_for_rule_set_invalidation = descendant_features.has_features_for_rule_set_invalidation;
    bool selector_list_contains_universal
        = simple_selector.GetPseudoType() == CSSSelector::kPseudoNot || simple_selector.GetPseudoType() == CSSSelector::kPseudoHostContext;
    in_nth_child |= simple_selector.GetPseudoType() == CSSSelector::kPseudoNthChild;
    in_nth_child |= simple_selector.GetPseudoType() == CSSSelector::kPseudoNthLastChild;

    for (const CSSSelector* sub_selector = simple_selector.SelectorListOrParent(); sub_selector; sub_selector = CSSSelectorList::Next(*sub_selector)) {
        ...
        AutoRestoreTreeBoundaryCrossingFlag restore_tree_boundary(descendant_features);
        AutoRestoreInsertionPointCrossingFlag restore_insertion_point(descendant_features);
        ...
        AddFeaturesToInvalidationSets(*sub_selector, in_nth_child, sibling_features, descendant_features);
    ...
    descendant_features.has_features_for_rule_set_invalidation = had_features_for_rule_set_invalidation || !selector_list_contains_universal;
}

// See also UpdateFeaturesFromStyleScope.
template <RuleInvalidationDataVisitorType VisitorType>
void RuleInvalidationDataVisitor<VisitorType>::AddFeaturesToInvalidationSetsForStyleScope(
    const StyleScope& style_scope, InvalidationSetFeatures& descendant_features)
{
    auto add_features = [this](const CSSSelector& selector_list, InvalidationSetFeatures& features) {
        for (const CSSSelector* selector = &selector_list; selector; selector = CSSSelectorList::Next(*selector)) {
            AddFeaturesToInvalidationSets(*selector, /*in_nth_child=*/false, nullptr /* sibling_features */, features);
        }
    };
...
template <RuleInvalidationDataVisitorType VisitorType>
void RuleInvalidationDataVisitor<VisitorType>::AddFeaturesToUniversalSiblingInvalidationSet(
    const InvalidationSetFeatures& sibling_features, const InvalidationSetFeatures& descendant_features)
{
    SiblingInvalidationSetType* universal_set = EnsureUniversalSiblingInvalidationSet();
    AddFeaturesToInvalidationSet(universal_set, sibling_features);
    UpdateMaxDirectAdjacentSelectors(universal_set, sibling_features.max_direct_adjacent_selectors);

    if (&sibling_features == &descendant_features) {
        SetInvalidatesSelf(universal_set);
    } else {
        AddFeaturesToInvalidationSet(EnsureSiblingDescendantInvalidationSet(universal_set), descendant_features);
    }
}

template <RuleInvalidationDataVisitorType VisitorType>
void RuleInvalidationDataVisitor<VisitorType>::AddValuesInComplexSelectorInsideIsWhereNot(const CSSSelector* selector_first)
{
    for (const CSSSelector* complex = selector_first; complex; complex = CSSSelectorList::Next(*complex)) {
        DCHECK(complex);
...
template <RuleInvalidationDataVisitorType VisitorType>
bool RuleInvalidationDataVisitor<VisitorType>::AddValueOfSimpleSelectorInHasArgument(const CSSSelector& selector)
{
    if (selector.Match() == CSSSelector::kClass) {
        if constexpr (is_builder()) {
    ...
    if (selector.IsAttributeSelector()) {
        if constexpr (is_builder()) {
            rule_invalidation_data_.attributes_in_has_argument.insert(selector.Attribute().LocalName());
        }
        return true;
    }
    if (selector.Match() == CSSSelector::kId) {
    ...
    if (selector.Match() == CSSSelector::kTag && selector.TagQName().LocalName() != CSSSelector::UniversalSelectorAtom()) {
        if constexpr (is_builder()) {
            rule_invalidation_data_.tag_names_in_has_argument.insert(selector.TagQName().LocalName());
        }
        return true;
    }
    if (selector.Match() == CSSSelector::kPseudoClass) {
        ...
        case CSSSelector::kPseudoWhere:
        case CSSSelector::kPseudoParent:
            AddValuesInComplexSelectorInsideIsWhereNot(selector.SelectorListOrParent());
            break;
        case CSSSelector::kPseudoVisited:
...
template <RuleInvalidationDataVisitorType VisitorType>
void RuleInvalidationDataVisitor<VisitorType>::CollectValuesInHasArgument(const CSSSelector& has_pseudo_class)
{
    ...
    for (const CSSSelector* relative_selector = selector_list->First(); relative_selector; relative_selector = CSSSelectorList::Next(*relative_selector)) {
...
template <RuleInvalidationDataVisitorType VisitorType>
void RuleInvalidationDataVisitor<VisitorType>::AddFeaturesToInvalidationSetsForHasPseudoClass(const CSSSelector& pseudo_has,
    const CSSSelector* compound_containing_has, InvalidationSetFeatures* sibling_features, InvalidationSetFeatures& descendant_features, bool in_nth_child)
{
    ...
    for (const CSSSelector* relative = pseudo_has.SelectorList()->First(); relative; relative = CSSSelectorList::Next(*relative)) {
        for (const CSSSelector* simple = relative; simple->GetPseudoType() != CSSSelector::kPseudoRelativeAnchor; simple = simple->NextSimpleSelector()) {
            ...
            case CSSSelector::kPseudoParent:
                ...
                AddFeaturesToInvalidationSetsForLogicalCombinationInHas(
                    *simple, compound_containing_has, sibling_features, descendant_features, CSSSelector::kSubSelector, kForAllNonRightmostCompounds);
                AddFeaturesToInvalidationSetsForLogicalCombinationInHas(*simple, compound_containing_has, sibling_features, descendant_features,
                    CSSSelector::kSubSelector, kForCompoundImmediatelyFollowsAdjacentRelation);
                break;
            default:
                break;
            }
...
template <RuleInvalidationDataVisitorType VisitorType>
struct RuleInvalidationDataVisitor<VisitorType>::AddFeaturesToInvalidationSetsForLogicalCombinationInHasContext {
    ...
    AddFeaturesToInvalidationSetsForLogicalCombinationInHasContext(const CSSSelector* compound_in_logical_combination,
        const CSSSelector* compound_containing_has, CSSSelector::RelationType previous_combinator,
        AddFeaturesMethodForLogicalCombinationInHas add_features_method)
    {
        ...
        bool is_in_has_argument_checking_scope = previous_combinator == CSSSelector::kSubSelector;
        bool add_features_for_compound_immediately_follows_adjacent_relation = add_features_method == kForCompoundImmediatelyFollowsAdjacentRelation;
        ...
        if (is_in_has_argument_checking_scope) {
            ...
            if (add_features_for_compound_immediately_follows_adjacent_relation && !CSSSelector::IsAdjacentRelation(previous_combinator)) {
                needs_skip_adding_features = true;
            }
...
template <RuleInvalidationDataVisitorType VisitorType>
void RuleInvalidationDataVisitor<VisitorType>::AddFeaturesToInvalidationSetsForLogicalCombinationInHas(const CSSSelector& logical_combination,
    const CSSSelector* compound_containing_has, InvalidationSetFeatures* sibling_features, InvalidationSetFeatures& descendant_features,
    CSSSelector::RelationType previous_combinator, AddFeaturesMethodForLogicalCombinationInHas add_features_method)
{
    DCHECK(compound_containing_has);

    for (const CSSSelector* complex = logical_combination.SelectorListOrParent(); complex; complex = CSSSelectorList::Next(*complex)) {
        base::AutoReset<CSSSelector::RelationType> restore_previous_combinator(&previous_combinator, previous_combinator);
        AutoRestoreMaxDirectAdjacentSelectors restore_max(sibling_features);
        AutoRestoreDescendantFeaturesDepth restore_depth(&descendant_features);
        AutoRestoreTreeBoundaryCrossingFlag restore_tree_boundary(descendant_features);
        AutoRestoreInsertionPointCrossingFlag restore_insertion_point(descendant_features);
        ...
        while (compound_in_logical_combination) {
            AddFeaturesToInvalidationSetsForLogicalCombinationInHasContext context(
                compound_in_logical_combination, compound_containing_has, previous_combinator, add_features_method);

            const CSSSelector* last_in_compound;
            if (context.needs_skip_adding_features) {
                last_in_compound = SkipAddingAndGetLastInCompoundForLogicalCombinationInHas(compound_in_logical_combination, compound_containing_has,
                    inner_sibling_features, descendant_features, previous_combinator, add_features_method);
            } else {
                last_in_compound = AddFeaturesAndGetLastInCompoundForLogicalCombinationInHas(compound_in_logical_combination, compound_containing_has,
                    inner_sibling_features, descendant_features, previous_combinator, add_features_method);
            }
            ...
            if (context.needs_update_features) {
                UpdateFeaturesFromCombinatorForLogicalCombinationInHas(
                    context.use_indirect_adjacent_combinator_for_updating_features ? CSSSelector::kIndirectAdjacent : previous_combinator,
                    context.last_compound_in_adjacent_chain, last_compound_in_adjacent_chain_features, inner_sibling_features, descendant_features);
            }
...
template <RuleInvalidationDataVisitorType VisitorType>
void RuleInvalidationDataVisitor<VisitorType>::UpdateFeaturesFromCombinatorForLogicalCombinationInHas(CSSSelector::RelationType combinator,
    const CSSSelector* last_compound_in_adjacent_chain, InvalidationSetFeatures& last_compound_in_adjacent_chain_features,
    InvalidationSetFeatures*& sibling_features, InvalidationSetFeatures& descendant_features)
{
    ...
    UpdateFeaturesFromCombinator(combinator, last_compound_in_adjacent_chain, last_compound_in_adjacent_chain_features, sibling_features, descendant_features,
        /* for_logical_combination_in_has */ true,
        /*in_nth_child=*/false);
}

template <RuleInvalidationDataVisitorType VisitorType>
const CSSSelector* RuleInvalidationDataVisitor<VisitorType>::SkipAddingAndGetLastInCompoundForLogicalCombinationInHas(
    const CSSSelector* compound_in_logical_combination, const CSSSelector* compound_containing_has, InvalidationSetFeatures* sibling_features,
    InvalidationSetFeatures& descendant_features, CSSSelector::RelationType previous_combinator,
    AddFeaturesMethodForLogicalCombinationInHas add_features_method)
{
    ...
    for (; simple; simple = simple->NextSimpleSelector()) {
        ...
        case CSSSelector::kPseudoParent:
            ...
            AddFeaturesToInvalidationSetsForLogicalCombinationInHas(
                *simple, compound_containing_has, sibling_features, descendant_features, previous_combinator, add_features_method);
            break;
        default:
            break;
        }
        if (simple->Relation() != CSSSelector::kSubSelector && simple->Relation() != CSSSelector::kScopeActivation) {
            break;
        }
    }
    return simple;
...
template <RuleInvalidationDataVisitorType VisitorType>
const CSSSelector* RuleInvalidationDataVisitor<VisitorType>::AddFeaturesAndGetLastInCompoundForLogicalCombinationInHas(
    const CSSSelector* compound_in_logical_combination, const CSSSelector* compound_containing_has, InvalidationSetFeatures* sibling_features,
    InvalidationSetFeatures& descendant_features, CSSSelector::RelationType previous_combinator,
    AddFeaturesMethodForLogicalCombinationInHas add_features_method)
{
    ...
    for (; simple; simple = simple->NextSimpleSelector()) {
        ...
        default:
            AddFeaturesToInvalidationSetsForSimpleSelector(
                *simple, *compound_in_logical_combination, /*in_nth_child=*/false, sibling_features, descendant_features);
            break;
        }
        if (descendant_features.has_features_for_rule_set_invalidation) {
            compound_has_features_for_rule_set_invalidation = true;
...
template <RuleInvalidationDataVisitorType VisitorType>
void RuleInvalidationDataVisitor<VisitorType>::MarkInvalidationSetsWithinNthChild(const CSSSelector& selector, bool in_nth_child)
{
    const CSSSelector* simple_selector = &selector;
    for (; simple_selector; simple_selector = simple_selector->NextSimpleSelector()) {
        if (in_nth_child) {
            if (InvalidationSetType* invalidation_set
                = InvalidationSetForSimpleSelector(*simple_selector, InvalidationType::kInvalidateDescendants, kAncestor, in_nth_child)) {
        ...
        if (simple_selector->SelectorList()) {
            bool sub_in_nth_child = in_nth_child || simple_selector->GetPseudoType() == CSSSelector::kPseudoNthChild
                || simple_selector->GetPseudoType() == CSSSelector::kPseudoNthLastChild;
            MarkInvalidationSetsWithinNthChild(*simple_selector->SelectorList()->First(), sub_in_nth_child);
        }
    }
}

template <RuleInvalidationDataVisitorType VisitorType>
RuleInvalidationDataVisitor<VisitorType>::InvalidationSetType* RuleInvalidationDataVisitor<VisitorType>::InvalidationSetForSimpleSelector(
    const CSSSelector& selector, InvalidationType type, PositionType position, bool in_nth_child)
{
    if (selector.Match() == CSSSelector::kClass) {
        if (type == InvalidationType::kInvalidateDescendants && position == kSubject && !in_nth_child
            && InsertIntoSelfInvalidationBloomFilter(selector.Value(), RuleInvalidationData::kClassSalt)) {
            // Do not insert self-invalidation sets for classes;
            // see comment on class_invalidation_sets_.
            return nullptr;
        }
        return EnsureClassInvalidationSet(selector.Value(), type, position, in_nth_child);
    }
    if (selector.IsAttributeSelector()) {
        return EnsureAttributeInvalidationSet(selector.Attribute().LocalName(), type, position, in_nth_child);
    }
    if (selector.Match() == CSSSelector::kId) {
        if (type == InvalidationType::kInvalidateDescendants && position == kSubject
            && InsertIntoSelfInvalidationBloomFilter(selector.Value(), RuleInvalidationData::kIdSalt)) {
            // Do not insert self-invalidation sets for IDs;
            // see comment on class_invalidation_sets_.
            return nullptr;
        }
        return EnsureIdInvalidationSet(selector.Value(), type, position, in_nth_child);
    }
    if (selector.Match() == CSSSelector::kPseudoClass) {
        switch (selector.GetPseudoType()) {
        case CSSSelector::kPseudoEmpty:
        ...
        case CSSSelector::kPseudoActiveViewTransitionType:
        case CSSSelector::kPseudoHasSlotted:
            return EnsurePseudoInvalidationSet(selector.GetPseudoType(), type, position, in_nth_child);
        case CSSSelector::kPseudoFirstOfType:
        case CSSSelector::kPseudoLastOfType:
        case CSSSelector::kPseudoOnlyOfType:
        case CSSSelector::kPseudoNthChild:
        ...
        case CSSSelector::kPseudoHas:
            return position == kAncestor ? EnsurePseudoInvalidationSet(selector.GetPseudoType(), type, position, in_nth_child) : nullptr;
        case CSSSelector::kPseudoPart:
        default:
            break;
        }
...
template <RuleInvalidationDataVisitorType VisitorType>
RuleInvalidationDataVisitor<VisitorType>::InvalidationSetType* RuleInvalidationDataVisitor<VisitorType>::EnsureClassInvalidationSet(
    const AtomicString& class_name, InvalidationType type, PositionType position, bool in_nth_child)
{
    CHECK(!class_name.empty());
    return EnsureInvalidationSet(rule_invalidation_data_.class_invalidation_sets, class_name, type, position, in_nth_child);
}

template <RuleInvalidationDataVisitorType VisitorType>
RuleInvalidationDataVisitor<VisitorType>::InvalidationSetType* RuleInvalidationDataVisitor<VisitorType>::EnsureAttributeInvalidationSet(
    const AtomicString& attribute_name, InvalidationType type, PositionType position, bool in_nth_child)
{
    CHECK(!attribute_name.empty());
    return EnsureInvalidationSet(rule_invalidation_data_.attribute_invalidation_sets, attribute_name, type, position, in_nth_child);
}

template <RuleInvalidationDataVisitorType VisitorType>
RuleInvalidationDataVisitor<VisitorType>::InvalidationSetType* RuleInvalidationDataVisitor<VisitorType>::EnsureIdInvalidationSet(
    const AtomicString& id, InvalidationType type, PositionType position, bool in_nth_child)
{
    CHECK(!id.empty());
...
template <RuleInvalidationDataVisitorType VisitorType>
RuleInvalidationDataVisitor<VisitorType>::InvalidationSetType* RuleInvalidationDataVisitor<VisitorType>::EnsurePseudoInvalidationSet(
    CSSSelector::PseudoType pseudo_type, InvalidationType type, PositionType position, bool in_nth_child)
{
    CHECK_NE(pseudo_type, CSSSelector::kPseudoUnknown);
    return EnsureInvalidationSet(rule_invalidation_data_.pseudo_invalidation_sets, pseudo_type, type, position, in_nth_child);
}

template <RuleInvalidationDataVisitorType VisitorType>
RuleInvalidationDataVisitor<VisitorType>::InvalidationSetType* RuleInvalidationDataVisitor<VisitorType>::EnsureInvalidationSet(
    InvalidationSetMapType& map, const AtomicString& key, InvalidationType type, PositionType position, bool in_nth_child)
{
    if constexpr (is_builder()) {
        scoped_refptr<InvalidationSet>& invalidation_set = map.insert(key, nullptr).stored_value->value;
...
template <RuleInvalidationDataVisitorType VisitorType>
RuleInvalidationDataVisitor<VisitorType>::InvalidationSetType* RuleInvalidationDataVisitor<VisitorType>::EnsureInvalidationSet(
    PseudoTypeInvalidationSetMapType& map, CSSSelector::PseudoType key, InvalidationType type, PositionType position, bool in_nth_child)
{
    if constexpr (is_builder()) {
        scoped_refptr<InvalidationSet>& invalidation_set = map.insert(key, nullptr).stored_value->value;
        return &EnsureMutableInvalidationSet(type, position, in_nth_child, invalidation_set);
...
template <RuleInvalidationDataVisitorType VisitorType>
RuleInvalidationDataVisitor<VisitorType>::SiblingInvalidationSetType* RuleInvalidationDataVisitor<VisitorType>::EnsureUniversalSiblingInvalidationSet()
{
    if constexpr (is_builder()) {
        if (!rule_invalidation_data_.universal_sibling_invalidation_set) {
            rule_invalidation_data_.universal_sibling_invalidation_set = SiblingInvalidationSet::Create(nullptr);
        }
    }
...
template <RuleInvalidationDataVisitorType VisitorType>
RuleInvalidationDataVisitor<VisitorType>::SiblingInvalidationSetType* RuleInvalidationDataVisitor<VisitorType>::EnsureNthInvalidationSet()
{
    if constexpr (is_builder()) {
        if (!rule_invalidation_data_.nth_invalidation_set) {
            rule_invalidation_data_.nth_invalidation_set = NthSiblingInvalidationSet::Create();
        }
    }
...
// set.
template <RuleInvalidationDataVisitorType VisitorType>
void RuleInvalidationDataVisitor<VisitorType>::AddFeaturesToInvalidationSet(InvalidationSetType* invalidation_set, const InvalidationSetFeatures& features)
{
    ...
    if (features.content_pseudo_crossing || features.invalidation_flags.WholeSubtreeInvalid()) {
        return;
    }
    ...
    for (const auto& id : features.ids) {
        ...
        InvalidationSetToSelectorMap::RecordInvalidationSetEntry(invalidation_set, InvalidationSetToSelectorMap::SelectorFeatureType::kId, id);
    }
    for (const auto& tag_name : features.tag_names) {
        if constexpr (is_builder()) {
            invalidation_set->AddTagName(tag_name);
        }
        InvalidationSetToSelectorMap::RecordInvalidationSetEntry(invalidation_set, InvalidationSetToSelectorMap::SelectorFeatureType::kTagName, tag_name);
    }
    for (const auto& emitted_tag_name : features.emitted_tag_names) {
        ...
        InvalidationSetToSelectorMap::RecordInvalidationSetEntry(
            invalidation_set, InvalidationSetToSelectorMap::SelectorFeatureType::kTagName, emitted_tag_name);
    }
    for (const auto& class_name : features.classes) {
        if constexpr (is_builder()) {
            invalidation_set->AddClass(class_name);
        }
        InvalidationSetToSelectorMap::RecordInvalidationSetEntry(invalidation_set, InvalidationSetToSelectorMap::SelectorFeatureType::kClass, class_name);
    }
    for (const auto& attribute : features.attributes) {
        if constexpr (is_builder()) {
            invalidation_set->AddAttribute(attribute);
        }
        InvalidationSetToSelectorMap::RecordInvalidationSetEntry(invalidation_set, InvalidationSetToSelectorMap::SelectorFeatureType::kAttribute, attribute);
    }
...
template <RuleInvalidationDataVisitorType VisitorType>
void RuleInvalidationDataVisitor<VisitorType>::SetWholeSubtreeInvalid(InvalidationSetType* invalidation_set)
{
    if constexpr (is_builder()) {
        invalidation_set->SetWholeSubtreeInvalid();
    }
    InvalidationSetToSelectorMap::RecordInvalidationSetEntry(invalidation_set, InvalidationSetToSelectorMap::SelectorFeatureType::kWholeSubtree, g_empty_atom);
}

template <RuleInvalidationDataVisitorType VisitorType> void RuleInvalidationDataVisitor<VisitorType>::SetInvalidatesSelf(InvalidationSetType* invalidation_set)
{
    if constexpr (is_builder()) {
...
template <RuleInvalidationDataVisitorType VisitorType> void RuleInvalidationDataVisitor<VisitorType>::SetInvalidatesNth(InvalidationSetType* invalidation_set)
{
    if constexpr (is_builder()) {
...
template <RuleInvalidationDataVisitorType VisitorType>
void RuleInvalidationDataVisitor<VisitorType>::UpdateMaxDirectAdjacentSelectors(SiblingInvalidationSetType* invalidation_set, unsigned value)
{
    if constexpr (is_builder()) {
...
template <RuleInvalidationDataVisitorType VisitorType>
bool RuleInvalidationDataVisitor<VisitorType>::InsertIntoSelfInvalidationBloomFilter(const AtomicString& value, int salt)
{
    if constexpr (is_builder()) {
        if (rule_invalidation_data_.names_with_self_invalidation == nullptr) {
            if (rule_invalidation_data_.num_candidates_for_names_bloom_filter++ < 50) {
                // It's not worth spending 2 kB on the Bloom filter for this
                // style sheet yet, so just insert a regular entry.
                return false;
            } else {
                rule_invalidation_data_.names_with_self_invalidation = std::make_unique<WTF::BloomFilter<14>>();
            }
        }
        rule_invalidation_data_.names_with_self_invalidation->Add(value.Hash() * salt);
        return true;
    } else {
...
template <RuleInvalidationDataVisitorType VisitorType>
RuleInvalidationDataVisitor<VisitorType>::InvalidationSetType* RuleInvalidationDataVisitor<VisitorType>::EnsureSiblingDescendantInvalidationSet(
...
template <RuleInvalidationDataVisitorType VisitorType>
InvalidationSet& RuleInvalidationDataVisitor<VisitorType>::EnsureMutableInvalidationSet(
    InvalidationType type, PositionType position, bool in_nth_child, scoped_refptr<InvalidationSet>& invalidation_set)
{
    if (!invalidation_set) {
    ...
    if (invalidation_set->IsSelfInvalidationSet() && type == InvalidationType::kInvalidateDescendants && position == kSubject && !in_nth_child) {
    ...
    if (invalidation_set->IsSelfInvalidationSet() || !invalidation_set->HasOneRef()) {
    ...
    if (type == InvalidationType::kInvalidateDescendants) {
        ...
        DescendantInvalidationSet& embedded_invalidation_set = To<SiblingInvalidationSet>(*invalidation_set).EnsureDescendants();
        return embedded_invalidation_set;
    } else {
        // descendant → sibling+descendant.
        scoped_refptr<InvalidationSet> descendants = invalidation_set;
        invalidation_set = SiblingInvalidationSet::Create(To<DescendantInvalidationSet>(descendants.get()));
        return *invalidation_set;
    }
}

template class RuleInvalidationDataVisitor<RuleInvalidationDataVisitorType::kBuilder>;
template class RuleInvalidationDataVisitor<RuleInvalidationDataVisitorType::kTracer>;
