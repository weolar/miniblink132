...
void StyleInvalidator::Invalidate(Document& document, Element* root_element)
{
    ...
    if (root_element) {
        ...
        if (!sibling_data.IsEmpty()) {
            for (Element* child = ElementTraversal::NextSibling(*root_element); child; child = ElementTraversal::NextSibling(*child)) {
...
ALWAYS_INLINE bool StyleInvalidator::MatchesCurrentInvalidationSets(Element& element) const
{
    if (invalidation_flags_.InvalidateCustomPseudo() && element.ShadowPseudoId() != g_null_atom) {
        TRACE_STYLE_INVALIDATOR_INVALIDATION_IF_ENABLED(element, kInvalidateCustomPseudo);
        return true;
    }
...
bool StyleInvalidator::SiblingData::MatchCurrentInvalidationSets(Element& element, StyleInvalidator& style_invalidator)
{
    ...
    while (index < invalidation_entries_.size()) {
        ...
        if (const DescendantInvalidationSet* descendants = invalidation_set.SiblingDescendants()) {
            if (descendants->WholeSubtreeInvalid()) {
                TRACE_STYLE_INVALIDATOR_INVALIDATION_SET(element, kInvalidationSetInvalidatesSubtree, *descendants);
                element.SetNeedsStyleRecalc(kSubtreeStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kRelatedStyleRule));
                return true;
            }
...
void StyleInvalidator::PushInvalidationSetsForContainerNode(ContainerNode& node, SiblingData& sibling_data)
{
    ...
    if (pending_invalidations_iterator == pending_invalidation_map_.end()) {
        DUMP_WILL_BE_NOTREACHED() << "We should strictly not have marked an element for "
                                     "invalidation without any pending invalidations.";
        return;
    }
    NodeInvalidationSets& pending_invalidations = pending_invalidations_iterator->value;
    ...
    if (!pending_invalidations.Descendants().empty()) {
        ...
        if (InvalidationTracingFlag::IsEnabled()) [[unlikely]] {
            DEVTOOLS_TIMELINE_TRACE_EVENT_INSTANT_WITH_CATEGORIES(TRACE_DISABLED_BY_DEFAULT("devtools.timeline.invalidationTracking"),
                "StyleInvalidatorInvalidationTracking", inspector_style_invalidator_invalidate_event::InvalidationList, node,
                pending_invalidations.Descendants());
        }
    }
}

ALWAYS_INLINE bool StyleInvalidator::CheckInvalidationSetsAgainstElement(Element& element, SiblingData& sibling_data)
{
    ...
    if (!sibling_data.IsEmpty() && sibling_data.MatchCurrentInvalidationSets(element, *this)) [[unlikely]] {
...
void StyleInvalidator::InvalidateShadowRootChildren(Element& element)
{
    if (ShadowRoot* root = element.GetShadowRoot()) {
        if (!TreeBoundaryCrossing() && !root->ChildNeedsStyleInvalidation() && !root->NeedsStyleInvalidation()) {
            return;
        }
...
void StyleInvalidator::Invalidate(Element& element, SiblingData& sibling_data)
{
    ...
    if (!WholeSubtreeInvalid()) {
        if (element.GetStyleChangeType() == kSubtreeStyleChange) {
            ...
            element.SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kRelatedStyleRule));
        }
    ...
    if ((!WholeSubtreeInvalid() && HasInvalidationSets() && element.GetComputedStyle()) || element.ChildNeedsStyleInvalidation()) {
...
void StyleInvalidator::InvalidateSlotDistributedElements(HTMLSlotElement& slot) const
{
    for (auto& distributed_node : slot.FlattenedAssignedNodes()) {
        ...
        if (MatchesCurrentInvalidationSetsAsSlotted(*element)) {
            distributed_node->SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kRelatedStyleRule));
        }
    }
}
