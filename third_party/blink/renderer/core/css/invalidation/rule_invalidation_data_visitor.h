...
template <RuleInvalidationDataVisitorType VisitorType> class RuleInvalidationDataVisitor {
public:
...
protected:
    ...
    using RuleInvalidationDataType = std::conditional<is_builder(), RuleInvalidationData, const RuleInvalidationData>::type;
    using InvalidationSetType = std::conditional<is_builder(), InvalidationSet, const InvalidationSet>::type;
    using SiblingInvalidationSetType = std::conditional<is_builder(), SiblingInvalidationSet, const SiblingInvalidationSet>::type;
    using InvalidationSetMapType
        = std::conditional<is_builder(), RuleInvalidationData::InvalidationSetMap, const RuleInvalidationData::InvalidationSetMap>::type;
    using PseudoTypeInvalidationSetMapType
        = std::conditional<is_builder(), RuleInvalidationData::PseudoTypeInvalidationSetMap, const RuleInvalidationData::PseudoTypeInvalidationSetMap>::type;
    ...
    SelectorPreMatch CollectMetadataFromSelector(const CSSSelector&, unsigned max_direct_adjacent_selectors, FeatureMetadata&);
    ...
    public:
        explicit AutoRestoreMaxDirectAdjacentSelectors(InvalidationSetFeatures* features)
            : features_(features)
            , original_value_(features ? features->max_direct_adjacent_selectors : 0)
        {
        }
    ...
    public:
        explicit AutoRestoreInsertionPointCrossingFlag(InvalidationSetFeatures& features)
            : features_(features)
            , original_value_(features.invalidation_flags.InsertionPointCrossing())
        {
        }
    ...
    FeatureInvalidationType UpdateInvalidationSetsForComplex(
        const CSSSelector&, bool in_nth_child, const StyleScope*, InvalidationSetFeatures&, PositionType, CSSSelector::PseudoType);

    void UpdateFeaturesFromCombinator(CSSSelector::RelationType, const CSSSelector* last_compound_selector_in_adjacent_chain,
        InvalidationSetFeatures& last_compound_in_adjacent_chain_features, InvalidationSetFeatures*& sibling_features,
        InvalidationSetFeatures& descendant_features, bool for_logical_combination_in_has, bool in_nth_child);
    ...
    const CSSSelector* ExtractInvalidationSetFeaturesFromCompound(
        const CSSSelector&, InvalidationSetFeatures&, PositionType, bool for_logical_combination_in_has, bool in_nth_child);
    void ExtractInvalidationSetFeaturesFromSelectorList(const CSSSelector&, bool in_nth_child, InvalidationSetFeatures&, PositionType);

    void AddFeaturesToInvalidationSets(
        const CSSSelector&, bool in_nth_child, InvalidationSetFeatures* sibling_features, InvalidationSetFeatures& descendant_features);
    const CSSSelector* AddFeaturesToInvalidationSetsForCompoundSelector(
        const CSSSelector&, bool in_nth_child, InvalidationSetFeatures* sibling_features, InvalidationSetFeatures& descendant_features);
    void AddFeaturesToInvalidationSetsForSimpleSelector(const CSSSelector& simple_selector, const CSSSelector& compound, bool in_nth_child,
    ...
    void AddFeaturesToUniversalSiblingInvalidationSet(const InvalidationSetFeatures& sibling_features, const InvalidationSetFeatures& descendant_features);
    ...
    void AddFeaturesToInvalidationSetsForHasPseudoClass(const CSSSelector& has_pseudo_class, const CSSSelector* compound_containing_has,
        InvalidationSetFeatures* sibling_features, InvalidationSetFeatures& descendant_features, bool in_nth_child);
    ...
    enum AddFeaturesMethodForLogicalCombinationInHas { kForAllNonRightmostCompounds, kForCompoundImmediatelyFollowsAdjacentRelation };
    ...
    void AddFeaturesToInvalidationSetsForLogicalCombinationInHas(const CSSSelector& logical_combination, const CSSSelector* compound_containing_has,
        InvalidationSetFeatures* sibling_features, InvalidationSetFeatures& descendant_features, CSSSelector::RelationType previous_combinator,
        AddFeaturesMethodForLogicalCombinationInHas);

    void UpdateFeaturesFromCombinatorForLogicalCombinationInHas(CSSSelector::RelationType combinator,
        const CSSSelector* last_compound_selector_in_adjacent_chain, InvalidationSetFeatures& last_compound_in_adjacent_chain_features,
        InvalidationSetFeatures*& sibling_features, InvalidationSetFeatures& descendant_features);
    const CSSSelector* SkipAddingAndGetLastInCompoundForLogicalCombinationInHas(const CSSSelector* compound_in_logical_combination,
        ...
        CSSSelector::RelationType previous_combinator, AddFeaturesMethodForLogicalCombinationInHas);
    const CSSSelector* AddFeaturesAndGetLastInCompoundForLogicalCombinationInHas(const CSSSelector* compound_in_logical_combination,
        const CSSSelector* compound_containing_has, InvalidationSetFeatures* sibling_features, InvalidationSetFeatures& descendant_features,
        CSSSelector::RelationType previous_combinator, AddFeaturesMethodForLogicalCombinationInHas);
    ...
    InvalidationSetType* InvalidationSetForSimpleSelector(const CSSSelector&, InvalidationType, PositionType, bool in_nth_child);

    InvalidationSetType* EnsureClassInvalidationSet(const AtomicString& class_name, InvalidationType, PositionType, bool in_nth_child);
    InvalidationSetType* EnsureAttributeInvalidationSet(const AtomicString& attribute_name, InvalidationType, PositionType, bool in_nth_child);
    InvalidationSetType* EnsureIdInvalidationSet(const AtomicString& id, InvalidationType, PositionType, bool in_nth_child);
    InvalidationSetType* EnsurePseudoInvalidationSet(CSSSelector::PseudoType, InvalidationType, PositionType, bool in_nth_child);

    InvalidationSetType* EnsureInvalidationSet(InvalidationSetMapType&, const AtomicString& key, InvalidationType, PositionType, bool in_nth_child);
    InvalidationSetType* EnsureInvalidationSet(
        PseudoTypeInvalidationSetMapType&, CSSSelector::PseudoType key, InvalidationType, PositionType, bool in_nth_child);
