...
protected:
    ...
    void AddElementIdentifierHashes(CheckPseudoHasFastRejectFilter& filter, const base::span<const ElementInfo> element_info_list)
    {
        for (const ElementInfo& element_info : element_info_list) {
            NonThrowableExceptionState no_exceptions;
            Element* element = GetDocument().CreateElementForBinding(AtomicString(element_info.tag_name), nullptr, no_exceptions);
            element->setAttribute(html_names::kIdAttr, AtomicString(element_info.id));
            element->setAttribute(html_names::kClassAttr, AtomicString(element_info.class_names));
            element->setAttribute(AtomicString(element_info.attribute_name), AtomicString(element_info.attribute_value));
            filter.AddElementIdentifierHashes(*element);
        }
    }

    bool CheckFastReject(CheckPseudoHasFastRejectFilter& filter, const char* selector_text)
    {
        ...
        CheckPseudoHasArgumentContext context(selector_list->First()->SelectorList()->First(),
...
TEST_F(CheckPseudoHasFastRejectFilterTest, CheckFastReject)
{
    ...
    const ElementInfo element_infos[] = { { /* tag_name */ "div", /* id */ "d1", /* class_names */ "a",
