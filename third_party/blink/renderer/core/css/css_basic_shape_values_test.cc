...
{
    auto* ua_context = MakeGarbageCollected<CSSParserContext>(kHTMLStandardMode, SecureContextMode::kInsecureContext);
    return CSSParser::ParseSingleValue(id, value, ua_context);
}

TEST(CSSBasicShapeValuesTest, PolygonEquals)
{
    const auto* value_default_windrule = ParsePropertyValue(CSSPropertyID::kClipPath, "polygon(0% 0%, 100% 0%, 50% 50%)");
    const auto* value_evenodd_windrule = ParsePropertyValue(CSSPropertyID::kClipPath, "polygon(evenodd, 0% 0%, 100% 0%, 50% 50%)");
    const auto* value_nonzero_windrule = ParsePropertyValue(CSSPropertyID::kClipPath, "polygon(nonzero, 0% 0%, 100% 0%, 50% 50%)");
