...
public:
    ...
    CSSVariableData(PassKey, StringView, bool is_animation_tainted, bool needs_variable_resolution, bool has_font_units, bool has_root_font_units,
    ...
    // substitution.
    static CSSVariableData* Create(StringView original_text, bool is_animation_tainted, bool needs_variable_resolution, bool has_font_units,
        bool has_root_font_units, bool has_line_height_units)
    {
        ...
        return MakeGarbageCollected<CSSVariableData>(AdditionalBytes(original_text.Is8Bit() ? original_text.length() : 2 * original_text.length()), PassKey(),
            original_text, is_animation_tainted, needs_variable_resolution, has_font_units, has_root_font_units, has_line_height_units);
    }

    // This tokenizes the string to determine the has_* data.
    // (The tokens are not used apart from that; only the original string is
    // stored.)
    static CSSVariableData* Create(const String& original_text, bool is_animation_tainted, bool needs_variable_resolution);
    ...
    {
        return needs_variable_resolution_;
    }
    ...
    {
        return has_line_height_units_;
    }

    const CSSValue* ParseForSyntax(const CSSSyntaxDefinition&, SecureContextMode) const;
    ...
    static void ExtractFeatures(const CSSParserToken& token, bool& has_font_units, bool& has_root_font_units, bool& has_line_height_units);
