...
public:
    ...
    CSSQuadValue(CSSValue* top, CSSValue* right, CSSValue* bottom, CSSValue* left, TypeForSerialization serialization_type)
        : CSSValue(kQuadClass)
    ...
    {
        return base::ValuesEquivalent(top_, other.top_) && base::ValuesEquivalent(right_, other.right_) && base::ValuesEquivalent(left_, other.left_)
...
template <> struct DowncastTraits<CSSQuadValue> {
