...
TEST_F(CSSAttrValueTaintingTest, StringValue)
{
    ...
    const CSSValue* value = css_test_helpers::ParseValue(GetDocument(), "<string>", text);
...
{
    String text = String("\"a\", \"b\"%T, \"c\"").Replace("%T", GetCSSAttrTaintToken());
    const CSSValue* value = css_test_helpers::ParseValue(GetDocument(), "<string>#", text);
    ASSERT_NE(value, nullptr);
    EXPECT_EQ(text, value->CssText());
...
{
    String tainted_text = String("\"abc\"%T").Replace("%T", GetCSSAttrTaintToken());
    const CSSValue* tainted_value = css_test_helpers::ParseValue(GetDocument(), "<string>", tainted_text);

    String non_tainted_text = String("\"abc\"");
    const CSSValue* non_tainted_value = css_test_helpers::ParseValue(GetDocument(), "<string>", non_tainted_text);
