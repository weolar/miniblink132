...
LocalFontFaceSource::LocalFontFaceSource(CSSFontFace* css_font_face, FontSelector* font_selector, const String& font_name)
    : face_(css_font_face)
...
bool LocalFontFaceSource::IsLocalFontAvailable(const FontDescription& font_description) const
{
    ...
    bool font_available = FontCache::Get().IsPlatformFontUniqueNameMatchAvailable(font_description, font_name_);
    if (font_available) {
...
const SimpleFontData* LocalFontFaceSource::CreateLoadingFallbackFontData(const FontDescription& font_description)
{
    ...
    CSSCustomFontData* css_font_data = MakeGarbageCollected<CSSCustomFontData>(this, CSSCustomFontData::kVisibleFallback);
    return MakeGarbageCollected<SimpleFontData>(&temporary_font->PlatformData(), css_font_data);
}

const SimpleFontData* LocalFontFaceSource::CreateFontData(const FontDescription& font_description, const FontSelectionCapabilities& font_selection_capabilities)
{
    if (!IsValid()) {
    ...
    probe::LocalFontsEnabled(font_selector_->GetExecutionContext(), &local_fonts_enabled);
    ...
    if (IsValid() && IsLoading()) {
        const SimpleFontData* fallback_font_data = CreateLoadingFallbackFontData(font_description);
        ReportFontLookup(font_description, fallback_font_data, true /* is_loading_fallback */);
    ...
    const SimpleFontData* unique_lookup_result = FontCache::Get().GetFontData(unstyled_description, font_name_, AlternateFontName::kLocalUniqueFace);
    ...
    FontCustomPlatformData* custom_platform_data = FontCustomPlatformData::Create(typeface, 0);
    SimpleFontData* font_data_variations_palette_applied
        = MakeGarbageCollected<SimpleFontData>(custom_platform_data->GetFontPlatformData(font_description.EffectiveFontSize(),
...
void LocalFontFaceSource::BeginLoadIfNeeded()
{
    ...
    unique_name_lookup->PrepareFontUniqueNameLookup(WTF::BindOnce(&LocalFontFaceSource::NotifyFontUniqueNameLookupReady, WrapWeakPersistent(this)));
    face_->DidBeginLoad();
}
...
void LocalFontFaceSource::ReportFontLookup(const FontDescription& font_description, const SimpleFontData* font_data, bool is_loading_fallback)
{
    font_selector_->ReportFontLookupByUniqueNameOnly(font_name_, font_description, font_data, is_loading_fallback);
}
