...
public:
    ...
    ContainerQuery* ParseContainer(String query)
    {
        ...
        auto* style_rule = DynamicTo<StyleRuleContainer>(css_test_helpers::ParseRule(GetDocument(), rule));
        if (!style_rule) {
            return nullptr;
        }
    ...
    {
        ComputedStyleBuilder builder(*GetDocument().GetStyleResolver().InitialStyleForElement());
    ...
    bool Eval(String query, double width, double height, unsigned container_type, PhysicalAxes contained_axes)
    {
        ...
        evaluator->SizeContainerChanged(PhysicalSize(LayoutUnit(width), LayoutUnit(height)), contained_axes);
        return evaluator->Eval(*container_query).value;
    }

    bool Eval(String query, String custom_property_name, String custom_property_value)
    {
        const CSSParserContext* context = StrictCSSParserContext(SecureContextMode::kSecureContext);
        CSSUnparsedDeclarationValue* value = CSSVariableParser::ParseDeclarationValue(custom_property_value, false, *context);
        DCHECK(value);

        ComputedStyleBuilder builder = GetDocument().GetStyleResolver().InitialStyleBuilderForElement();
        builder.SetVariableData(AtomicString(custom_property_name), value->VariableDataValue(), false);
        ContainerElement().SetComputedStyle(builder.TakeStyle());

        auto* evaluator = MakeGarbageCollected<ContainerQueryEvaluator>(ContainerElement());
        evaluator->SizeContainerChanged(PhysicalSize(LayoutUnit(100), LayoutUnit(100)), PhysicalAxes { kPhysicalAxesNone });
    ...
    Change SizeContainerChanged(ContainerQueryEvaluator* evaluator, PhysicalSize size, unsigned container_type, PhysicalAxes axes)
    {
        ComputedStyleBuilder builder(*GetDocument().GetStyleResolver().InitialStyleForElement());
        builder.SetContainerType(container_type);
        ContainerElement().SetComputedStyle(builder.TakeStyle());
    ...
    Change StickyContainerChanged(
        ContainerQueryEvaluator* evaluator, ContainerStuckPhysical stuck_horizontal, ContainerStuckPhysical stuck_vertical, unsigned container_type)
    {
        ComputedStyleBuilder builder(*GetDocument().GetStyleResolver().InitialStyleForElement());
        builder.SetContainerType(container_type);
        ContainerElement().SetComputedStyle(builder.TakeStyle());
    ...
    Change SnapContainerChanged(ContainerQueryEvaluator* evaluator, ContainerSnappedFlags snapped, unsigned container_type)
    {
        ComputedStyleBuilder builder(*GetDocument().GetStyleResolver().InitialStyleForElement());
        builder.SetContainerType(container_type);
        ContainerElement().SetComputedStyle(builder.TakeStyle());
    ...
    bool EvalAndAdd(ContainerQueryEvaluator* evaluator, const ContainerQuery& query, Change change = Change::kNearestContainer)
    {
    ...
    const HeapHashMap<Member<const ContainerQuery>, Result>& GetResults(ContainerQueryEvaluator* evaluator) const
    {
    ...
    void ClearSizeResults(ContainerQueryEvaluator* evaluator, Change change) const
    {
        return evaluator->ClearResults(change, ContainerQueryEvaluator::kSizeContainer);
    }

    void ClearStyleResults(ContainerQueryEvaluator* evaluator, Change change) const
    {
        return evaluator->ClearResults(change, ContainerQueryEvaluator::kStyleContainer);
    }
...
TEST_F(ContainerQueryEvaluatorTest, SizeContainerChanged)
{
    ...
    // produce a Change.
    EXPECT_EQ(Change::kNone, SizeContainerChanged(evaluator, size_100, type_size, horizontal));
    EXPECT_EQ(2u, GetResults(evaluator).size());
    ...
    EXPECT_EQ(Change::kNearestContainer, SizeContainerChanged(evaluator, size_200, type_size, horizontal));
    ...
    // Calling SizeContainerChanged with the values we already have should not
    // produce a Change.
    EXPECT_EQ(Change::kNone, SizeContainerChanged(evaluator, size_200, type_size, horizontal));
    EXPECT_EQ(2u, GetResults(evaluator).size());
    ...
    // they query width.
    EXPECT_EQ(Change::kNearestContainer, SizeContainerChanged(evaluator, size_200, type_size, vertical));
    EXPECT_EQ(0u, GetResults(evaluator).size());
    ...
    EXPECT_EQ(Change::kNearestContainer, SizeContainerChanged(evaluator, size_100, type_size, horizontal));
    EXPECT_EQ(0u, GetResults(evaluator).size());
    ...
    EXPECT_TRUE(EvalAndAdd(evaluator, *container_query_100, Change::kNearestContainer));
    EXPECT_TRUE(EvalAndAdd(evaluator, *container_query_200, Change::kDescendantContainers));
    EXPECT_EQ(2u, GetResults(evaluator).size());

    // Resize to 50px should cause both queries to change their evaluation.
    // `ContainerChanged` should return the biggest `Change`.
    EXPECT_EQ(Change::kDescendantContainers, SizeContainerChanged(evaluator, size_50, type_size, horizontal));
}
...
TEST_F(ContainerQueryEvaluatorTest, StyleContainerChanged)
{
    ...
    ContainerQueryEvaluator* evaluator = CreateEvaluatorForType(type_inline_size);
    EXPECT_EQ(Change::kNone, evaluator->SizeContainerChanged(size_100, horizontal));

    ContainerQuery* foo_bar_query = ParseContainer("style(--foo: bar)");
    ContainerQuery* size_bar_foo_query = ParseContainer("(inline-size = 100px) and style(--bar: foo)");
    ContainerQuery* no_match_query = ParseContainer("(inline-size > 1000px) and style(--no: match)");
    ...
    builder.SetVariableData(AtomicString("--no"), css_test_helpers::CreateVariableData("match"), inherited);
    ...
    builder = ComputedStyleBuilder(*style);
    builder.SetVariableData(AtomicString("--foo"), css_test_helpers::CreateVariableData("bar"), inherited);
    style = builder.TakeStyle();
    container_element.SetComputedStyle(style);
    ...
    eval_and_add_all();
    builder = ComputedStyleBuilder(*style);
    builder.SetVariableData(AtomicString("--bar"), css_test_helpers::CreateVariableData("foo"), inherited);
    style = builder.TakeStyle();
    container_element.SetComputedStyle(style);
    EXPECT_EQ(Change::kNearestContainer, StyleContainerChanged(evaluator));
    EXPECT_EQ(0u, GetResults(evaluator).size());
...
{
    ContainerQuery* container_query_left = ParseContainer("scroll-state(stuck: left)");
    ContainerQuery* container_query_bottom = ParseContainer("scroll-state(stuck: bottom)");
    ASSERT_TRUE(container_query_left);
    ASSERT_TRUE(container_query_bottom);

    ContainerQueryEvaluator* evaluator = CreateEvaluatorForType(type_scroll_state);
    StickyContainerChanged(evaluator, ContainerStuckPhysical::kLeft, ContainerStuckPhysical::kNo, type_scroll_state);
    ...
    // produce a Change.
    EXPECT_EQ(Change::kNone, StickyContainerChanged(evaluator, ContainerStuckPhysical::kLeft, ContainerStuckPhysical::kNo, type_scroll_state));
    EXPECT_EQ(2u, GetResults(evaluator).size());
    ...
    EXPECT_EQ(Change::kNearestContainer, StickyContainerChanged(evaluator, ContainerStuckPhysical::kLeft, ContainerStuckPhysical::kBottom, type_scroll_state));
    EXPECT_EQ(0u, GetResults(evaluator).size());
...
{
    ContainerQuery* container_query_snap_block = ParseContainer("scroll-state(snapped: block)");
    ContainerQuery* container_query_snap_inline = ParseContainer("scroll-state(snapped: inline)");
    ASSERT_TRUE(container_query_snap_block);
    ASSERT_TRUE(container_query_snap_inline);

    ContainerQueryEvaluator* evaluator = CreateEvaluatorForType(type_scroll_state);
    SnapContainerChanged(evaluator, static_cast<ContainerSnappedFlags>(ContainerSnapped::kY), type_scroll_state);
    ...
    // produce a Change.
    EXPECT_EQ(Change::kNone, SnapContainerChanged(evaluator, static_cast<ContainerSnappedFlags>(ContainerSnapped::kY), type_scroll_state));
    EXPECT_EQ(2u, GetResults(evaluator).size());
    ...
    EXPECT_EQ(Change::kNearestContainer,
        SnapContainerChanged(
            evaluator, static_cast<ContainerSnappedFlags>(ContainerSnapped::kX) | static_cast<ContainerSnappedFlags>(ContainerSnapped::kY), type_scroll_state));
    EXPECT_EQ(0u, GetResults(evaluator).size());
...
TEST_F(ContainerQueryEvaluatorTest, ClearResults)
{
    ...
    ContainerQuery* container_query_size_and_style = ParseContainer("(width > 0px) and style(--foo: bar)");
    ...
    EXPECT_EQ(Change::kNearestContainer, GetResults(evaluator).at(container_query_px).change);
    EXPECT_EQ(UnitFlags::kNone, GetResults(evaluator).at(container_query_px).unit_flags);
    ...
    EXPECT_EQ(Change::kDescendantContainers, GetResults(evaluator).at(container_query_em).change);
    EXPECT_EQ(UnitFlags::kFontRelative, GetResults(evaluator).at(container_query_em).unit_flags);
    ...
    EXPECT_EQ(Change::kNearestContainer, GetResults(evaluator).at(container_query_vh).change);
    EXPECT_EQ(UnitFlags::kStaticViewport, GetResults(evaluator).at(container_query_vh).unit_flags);
    EXPECT_EQ(static_cast<unsigned>(UnitFlags::kFontRelative | UnitFlags::kStaticViewport), GetUnitFlags(evaluator));
    ...
    EXPECT_EQ(Change::kDescendantContainers, GetResults(evaluator).at(container_query_cqw).change);
    EXPECT_EQ(UnitFlags::kContainer, GetResults(evaluator).at(container_query_cqw).unit_flags);
    EXPECT_EQ(static_cast<unsigned>(UnitFlags::kFontRelative | UnitFlags::kStaticViewport | UnitFlags::kContainer), GetUnitFlags(evaluator));
    ...
    EXPECT_EQ(UnitFlags::kFontRelative, GetResults(evaluator).at(container_query_em).unit_flags);
    EXPECT_EQ(UnitFlags::kContainer, GetResults(evaluator).at(container_query_cqw).unit_flags);
    EXPECT_EQ(static_cast<unsigned>(UnitFlags::kFontRelative | UnitFlags::kContainer), GetUnitFlags(evaluator));
    ...
    EvalAndAdd(evaluator, *container_query_size_and_style, Change::kNearestContainer);
...
TEST_F(ContainerQueryEvaluatorTest, DependentQueries)
{
    ...
    EXPECT_EQ(Change::kNone, SizeContainerChanged(evaluator, size_150, type_size, horizontal));
    ...
    // Makes no difference for either of (min-width: 200px), (max-width: 300px):
    EXPECT_EQ(Change::kNone, SizeContainerChanged(evaluator, size_300, type_size, horizontal));

    // (max-width: 300px) becomes false:
    EXPECT_EQ(Change::kNearestContainer, SizeContainerChanged(evaluator, size_400, type_size, horizontal));
}
...
TEST_F(ContainerQueryEvaluatorTest, Printing)
{
    ...
    EXPECT_NE(target->ComputedStyleRef().VisitedDependentColor(GetCSSPropertyColor()), Color(0, 128, 0));
    ...
    EXPECT_EQ(target->ComputedStyleRef().VisitedDependentColor(GetCSSPropertyColor()), Color(0, 128, 0));
}
...
TEST_F(ContainerQueryEvaluatorTest, FindContainer)
{
    ...
    EXPECT_EQ(ContainerQueryEvaluator::FindContainer(inner, ParseContainer("style(--foo: bar)")->Selector(), &GetDocument()), inner);
    EXPECT_EQ(ContainerQueryEvaluator::FindContainer(inner, ParseContainer("(width > 100px) and style(--foo: bar)")->Selector(), &GetDocument()), inner_size);
    EXPECT_EQ(ContainerQueryEvaluator::FindContainer(inner, ParseContainer("outer style(--foo: bar)")->Selector(), &GetDocument()), outer);
    EXPECT_EQ(
        ContainerQueryEvaluator::FindContainer(inner, ParseContainer("outer (width > 100px) and style(--foo: bar)")->Selector(), &GetDocument()), outer_size);
}
...
TEST_F(ContainerQueryEvaluatorTest, FindStickyContainer)
{
    ...
    EXPECT_EQ(ContainerQueryEvaluator::FindContainer(inner, ParseContainer("scroll-state(stuck: top) and style(--foo: bar)")->Selector(), &GetDocument()),
        inner_sticky);
    EXPECT_EQ(ContainerQueryEvaluator::FindContainer(inner, ParseContainer("outer scroll-state(stuck: top) and style(--foo: bar)")->Selector(), &GetDocument()),
        outer_sticky);
    EXPECT_EQ(
        ContainerQueryEvaluator::FindContainer(inner, ParseContainer("scroll-state(stuck: top) and (width > 0px)")->Selector(), &GetDocument()), sticky_size);
}
...
TEST_F(ContainerQueryEvaluatorTest, FindSnapContainer)
{
    ...
    EXPECT_EQ(ContainerQueryEvaluator::FindContainer(inner, ParseContainer("scroll-state(snapped: inline) and style(--foo: bar)")->Selector(), &GetDocument()),
        inner_snap);
    EXPECT_EQ(
        ContainerQueryEvaluator::FindContainer(inner, ParseContainer("outer scroll-state(snapped: block) and style(--foo: bar)")->Selector(), &GetDocument()),
        outer_snap);
    EXPECT_EQ(ContainerQueryEvaluator::FindContainer(inner,
...
class UseCountEvalUnknownTest : public ContainerQueryEvaluatorTest, public ::testing::WithParamInterface<EvalUnknownQueries> { };

INSTANTIATE_TEST_SUITE_P(ContainerQueryEvaluatorTest, UseCountEvalUnknownTest, testing::ValuesIn(eval_unknown_queries));
...
TEST_P(UseCountEvalUnknownTest, All)
{
    ...
    EXPECT_EQ(GetDocument().IsUseCounted(WebFeature::kContainerQueryEvalUnknown), param.contains_unknown);
}
