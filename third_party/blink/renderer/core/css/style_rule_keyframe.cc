...
StyleRuleKeyframe::StyleRuleKeyframe(std::unique_ptr<Vector<KeyframeOffset>> keys, CSSPropertyValueSet* properties)
    : StyleRuleBase(kKeyframe)
...
bool StyleRuleKeyframe::SetKeyText(const ExecutionContext* execution_context, const String& key_text)
{
    DCHECK(!key_text.IsNull());

    auto* context = MakeGarbageCollected<CSSParserContext>(*execution_context);

    std::unique_ptr<Vector<KeyframeOffset>> keys = CSSParser::ParseKeyframeKeyList(context, key_text);
    if (!keys || keys->empty()) {
        return false;
    }
