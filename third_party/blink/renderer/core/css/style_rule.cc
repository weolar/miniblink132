...
struct SameSizeAsStyleRuleBase final : public GarbageCollected<SameSizeAsStyleRuleBase> {
    uint8_t field;
};

ASSERT_SIZE(StyleRuleBase, SameSizeAsStyleRuleBase);

CSSRule* StyleRuleBase::CreateCSSOMWrapper(wtf_size_t position_hint, CSSStyleSheet* parent_sheet, bool trigger_use_counters) const
{
    return CreateCSSOMWrapper(position_hint, parent_sheet, nullptr, trigger_use_counters);
}

CSSRule* StyleRuleBase::CreateCSSOMWrapper(wtf_size_t position_hint, CSSRule* parent_rule, bool trigger_use_counters) const
{
...
CSSRule* StyleRuleBase::CreateCSSOMWrapper(wtf_size_t position_hint, CSSStyleSheet* parent_sheet, CSSRule* parent_rule, bool trigger_use_counters) const
{
    CSSRule* rule = nullptr;
    StyleRuleBase* self = const_cast<StyleRuleBase*>(this);
    switch (GetType()) {
    case kStyle:
        rule = MakeGarbageCollected<CSSStyleRule>(To<StyleRule>(self), parent_sheet, position_hint);
        break;
    case kPage:
        if (trigger_use_counters && parent_sheet) {
            UseCounter::Count(parent_sheet->OwnerDocument(), WebFeature::kCSSPageRule);
        }
        rule = MakeGarbageCollected<CSSPageRule>(To<StyleRulePage>(self), parent_sheet);
        break;
    case kPageMargin:
        rule = MakeGarbageCollected<CSSMarginRule>(To<StyleRulePageMargin>(self), parent_sheet);
        break;
    case kProperty:
        rule = MakeGarbageCollected<CSSPropertyRule>(To<StyleRuleProperty>(self), parent_sheet);
        break;
    case kFontFace:
        rule = MakeGarbageCollected<CSSFontFaceRule>(To<StyleRuleFontFace>(self), parent_sheet);
        break;
    case kFontPaletteValues:
        rule = MakeGarbageCollected<CSSFontPaletteValuesRule>(To<StyleRuleFontPaletteValues>(self), parent_sheet);
        break;
    case kFontFeatureValues:
        rule = MakeGarbageCollected<CSSFontFeatureValuesRule>(To<StyleRuleFontFeatureValues>(self), parent_sheet);
        break;
    case kMedia:
        rule = MakeGarbageCollected<CSSMediaRule>(To<StyleRuleMedia>(self), parent_sheet);
        break;
    case kNestedDeclarations:
        rule = MakeGarbageCollected<CSSNestedDeclarationsRule>(To<StyleRuleNestedDeclarations>(self), parent_sheet);
        break;
    case kScope:
        rule = MakeGarbageCollected<CSSScopeRule>(To<StyleRuleScope>(self), parent_sheet);
        break;
    case kSupports:
        rule = MakeGarbageCollected<CSSSupportsRule>(To<StyleRuleSupports>(self), parent_sheet);
        break;
    case kImport:
        rule = MakeGarbageCollected<CSSImportRule>(To<StyleRuleImport>(self), parent_sheet);
        break;
    case kKeyframes:
        rule = MakeGarbageCollected<CSSKeyframesRule>(To<StyleRuleKeyframes>(self), parent_sheet);
        break;
    case kLayerBlock:
        rule = MakeGarbageCollected<CSSLayerBlockRule>(To<StyleRuleLayerBlock>(self), parent_sheet);
        break;
    case kLayerStatement:
        rule = MakeGarbageCollected<CSSLayerStatementRule>(To<StyleRuleLayerStatement>(self), parent_sheet);
        break;
    case kNamespace:
        rule = MakeGarbageCollected<CSSNamespaceRule>(To<StyleRuleNamespace>(self), parent_sheet);
        break;
    case kContainer:
        rule = MakeGarbageCollected<CSSContainerRule>(To<StyleRuleContainer>(self), parent_sheet);
        break;
    case kCounterStyle:
        rule = MakeGarbageCollected<CSSCounterStyleRule>(To<StyleRuleCounterStyle>(self), parent_sheet);
        break;
    case kStartingStyle:
        rule = MakeGarbageCollected<CSSStartingStyleRule>(To<StyleRuleStartingStyle>(self), parent_sheet);
        break;
    case kViewTransition:
        rule = MakeGarbageCollected<CSSViewTransitionRule>(To<StyleRuleViewTransition>(self), parent_sheet);
        break;
    case kPositionTry:
        rule = MakeGarbageCollected<CSSPositionTryRule>(To<StyleRulePositionTry>(self), parent_sheet);
        break;
    case kFontFeature:
    case kKeyframe:
    case kCharset:
...
{
    return sizeof(StyleRule) + sizeof(CSSSelector) + CSSPropertyValueSet::AverageSizeInBytes();
}

StyleRule::StyleRule(base::PassKey<StyleRule>, base::span<CSSSelector> selector_vector, CSSPropertyValueSet* properties)
...
StyleRule::StyleRule(base::PassKey<StyleRule>, base::span<CSSSelector> selector_vector, CSSLazyPropertyParser* lazy_property_parser)
    : StyleRuleBase(kStyle)
...
StyleRule::StyleRule(base::PassKey<StyleRule>, base::span<CSSSelector> selector_vector)
    : StyleRuleBase(kStyle)
{
    CSSSelectorList::AdoptSelectorVector(selector_vector, SelectorArray());
}

StyleRule::StyleRule(base::PassKey<StyleRule>, base::span<CSSSelector> selector_vector, StyleRule&& other)
    : StyleRuleBase(kStyle)
...
void StyleRuleBase::Reparent(StyleRule* new_parent)
{
    switch (GetType()) {
    case kStyle:
        CSSSelectorList::Reparent(To<StyleRule>(this)->SelectorArray(), new_parent);
        break;
    case kScope:
    case kLayerBlock:
    case kContainer:
    ...
    case kSupports:
    case kStartingStyle:
        for (StyleRuleBase* child : DynamicTo<StyleRuleGroup>(this)->ChildRules()) {
            child->Reparent(new_parent);
        }
        break;
    case kPage:
        for (StyleRuleBase* child : DynamicTo<StyleRulePage>(this)->ChildRules()) {
            child->Reparent(new_parent);
        }
        break;
    case kMixin:
    ...
    case kNestedDeclarations:
        ...
        DynamicTo<StyleRuleNestedDeclarations>(this)->ReplaceSelectorList(new_parent->SelectorArray());
        break;
    case kPageMargin:
    case kProperty:
    case kFontFace:
...
StyleRuleProperty::StyleRuleProperty(const String& name, CSSPropertyValueSet* properties)
...
bool StyleRuleProperty::SetNameText(const ExecutionContext* execution_context, const String& name_text)
{
...
StyleRuleScope::StyleRuleScope(const StyleScope& style_scope, HeapVector<Member<StyleRuleBase>> rules)
    : StyleRuleGroup(kScope, std::move(rules))
...
void StyleRuleScope::SetPreludeText(const ExecutionContext* execution_context, String value, CSSNestingType nesting_type, StyleRule* parent_rule_for_nesting,
    bool is_within_scope, StyleSheetContents* style_sheet)
{
    auto* parser_context = MakeGarbageCollected<CSSParserContext>(*execution_context);
    CSSParserTokenStream stream(value);

    style_scope_ = StyleScope::Parse(stream, parser_context, nesting_type, parent_rule_for_nesting, is_within_scope, style_sheet);
    if (!stream.AtEnd()) {
...
StyleRuleGroup::StyleRuleGroup(RuleType type, HeapVector<Member<StyleRuleBase>> rules)
    : StyleRuleBase(type)
    , child_rules_(std::move(rules))
{
}
...
void StyleRuleGroup::WrapperInsertRule(CSSStyleSheet* parent_sheet, unsigned index, StyleRuleBase* rule)
{
...
// static
String StyleRuleBase::LayerNameAsString(const StyleRuleBase::LayerName& name_parts)
{
...
StyleRuleLayerBlock::StyleRuleLayerBlock(LayerName&& name, HeapVector<Member<StyleRuleBase>> rules)
    : StyleRuleGroup(kLayerBlock, std::move(rules))
    , name_(std::move(name))
{
}
...
void StyleRuleLayerStatement::TraceAfterDispatch(blink::Visitor* visitor) const
{
    StyleRuleBase::TraceAfterDispatch(visitor);
}
...
StyleRulePage::StyleRulePage(CSSSelectorList* selector_list, CSSPropertyValueSet* properties, HeapVector<Member<StyleRuleBase>> child_rules)
...
StyleRulePageMargin::StyleRulePageMargin(CSSAtRuleID id, CSSPropertyValueSet* properties)
    : StyleRuleBase(kPageMargin)
    , id_(id)
    , properties_(properties)
{
}
...
StyleRuleCondition::StyleRuleCondition(RuleType type, HeapVector<Member<StyleRuleBase>> rules)
    : StyleRuleGroup(type, std::move(rules))
{
}

StyleRuleCondition::StyleRuleCondition(RuleType type, const String& condition_text, HeapVector<Member<StyleRuleBase>> rules)
    : StyleRuleGroup(type, std::move(rules))
...
StyleRuleMedia::StyleRuleMedia(const MediaQuerySet* media, HeapVector<Member<StyleRuleBase>> rules)
    : StyleRuleCondition(kMedia, std::move(rules))
...
StyleRuleSupports::StyleRuleSupports(const String& condition_text, bool condition_is_supported, HeapVector<Member<StyleRuleBase>> rules)
...
void StyleRuleSupports::SetConditionText(const ExecutionContext* execution_context, String value)
{
    CSSParserTokenStream stream(value);
...
StyleRuleContainer::StyleRuleContainer(ContainerQuery& container_query, HeapVector<Member<StyleRuleBase>> rules)
    ...
    : StyleRuleCondition(container_rule)
{
    ...
    container_query_ = MakeGarbageCollected<ContainerQuery>(*container_rule.container_query_);
}

void StyleRuleContainer::SetConditionText(const ExecutionContext* execution_context, String value)
{
    auto* context = MakeGarbageCollected<CSSParserContext>(*execution_context);
    ...
    if (const MediaQueryExpNode* exp_node = parser.ParseCondition(value)) {
        ...
        container_query_ = MakeGarbageCollected<ContainerQuery>(std::move(selector), exp_node);
    }
}
...
StyleRuleStartingStyle::StyleRuleStartingStyle(HeapVector<Member<StyleRuleBase>> rules)
    : StyleRuleGroup(kStartingStyle, std::move(rules))
{
}

StyleRuleFunction::StyleRuleFunction(
    AtomicString name, Vector<StyleRuleFunction::Parameter> parameters, CSSVariableData* function_body, StyleRuleFunction::Type return_type)
