...
public:
    ContextWithStyleScopeFrame(
        const ElementResolveContext& element_context, const MatchRequest& match_request, StyleRequest* pseudo_style_request, StyleScopeFrame* parent_frame)
        : style_scope_frame(element_context.GetUltimateOriginatingElementOrSelf(), parent_frame)
        , context(element_context)
    {
        context.style_scope_frame = &style_scope_frame.GetParentFrameOrThis(element_context.GetUltimateOriginatingElementOrSelf());
...
struct HashTraits<blink::CumulativeRulePerfKey>
    : TwoFieldsHashTraits<blink::CumulativeRulePerfKey, &blink::CumulativeRulePerfKey::selector, &blink::CumulativeRulePerfKey::style_sheet_id> { };
} // namespace WTF

namespace blink {

template <class CSSRuleCollection> static CSSRule* FindStyleRule(CSSRuleCollection* css_rules, const StyleRule* style_rule);

namespace {

const CSSStyleSheet* FindStyleSheet(const TreeScope* tree_scope_containing_rule, const StyleEngine& style_engine, const StyleRule* rule)
{
    if (tree_scope_containing_rule) {
        for (const auto& [sheet, rule_set] : tree_scope_containing_rule->GetScopedStyleResolver()->GetActiveStyleSheets()) {
...
unsigned AdjustLinkMatchType(EInsideLink inside_link, unsigned link_match_type)
{
...
bool EvaluateAndAddContainerQueries(Element* style_container_candidate, const ContainerQuery& container_query, const StyleRecalcContext& style_recalc_context,
    ContainerSelectorCache& container_selector_cache, MatchResult& result)
{
    for (const ContainerQuery* current = &container_query; current; current = current->Parent()) {
        if (!ContainerQueryEvaluator::EvalAndAdd(style_container_candidate, style_recalc_context, *current, container_selector_cache, result)) {
            return false;
        }
    }
...
public:
    CascadeLayerSeeker(const ContainerNode* scope, Element* vtt_originating_element, bool matching_ua_rules, bool matching_rules_from_no_style_sheet,
        const Document* document, const RuleSet* rule_set)
        : seeker_(rule_set->LayerIntervals())
        , layer_map_(FindLayerMap(scope, vtt_originating_element, matching_ua_rules, matching_rules_from_no_style_sheet, document))
    {
    }
...
private:
    static const CascadeLayerMap* FindLayerMap(
        const ContainerNode* scope, Element* vtt_originating_element, bool matching_ua_rules, bool matching_rules_from_no_style_sheet, const Document* document)
    {
        ...
        if (scope) {
            ...
            return scope->GetTreeScope().GetScopedStyleResolver()->GetCascadeLayerMap();
        }
        if (!document) {
            return nullptr;
        }
...
using SelectorStatisticsRuleMap = HashMap<CumulativeRulePerfKey, CumulativeRulePerfData>;
...
void AggregateRulePerfData(const TreeScope* tree_scope_containing_rule, const StyleEngine& style_engine, const Vector<RulePerfDataPerRequest>& rules_statistics)
{
    ...
    for (const auto& rule_stats : rules_statistics) {
        ...
        const CSSStyleSheet* style_sheet = FindStyleSheet(tree_scope_containing_rule, style_engine, rule->Rule());
        CumulativeRulePerfKey key { rule->Selector().SelectorText(), IdentifiersFactory::IdForCSSStyleSheet(style_sheet) };
        auto it = map.find(key);
        if (it == map.end()) {
            CumulativeRulePerfData data { /*match_attempts*/ 1, (rule_stats.fast_reject) ? 1 : 0, (rule_stats.did_match) ? 1 : 0, rule_stats.elapsed };
            map.insert(key, data);
        } else {
...
ElementRuleCollector::ElementRuleCollector(const ElementResolveContext& context, const StyleRecalcContext& style_recalc_context, const SelectorFilter& filter,
    MatchResult& result, EInsideLink inside_link)
    : context_(context)
    , style_recalc_context_(style_recalc_context)
    , selector_filter_(filter)
    , mode_(SelectorChecker::kResolvingStyle)
    , can_use_fast_reject_(selector_filter_.ParentStackIsConsistent(context.GetElement().IsPseudoElement()
              ? LayoutTreeBuilderTraversal::ParentElement(*To<PseudoElement>(context.GetElement()).UltimateOriginatingElement())
...
void ElementRuleCollector::AddElementStyleProperties(const CSSPropertyValueSet* property_set, CascadeOrigin origin, bool is_cacheable, bool is_inline_style)
{
    if (!property_set) {
        return;
    }
    ...
    result_.AddMatchedProperties(property_set,
        { .link_match_type = static_cast<uint8_t>(AdjustLinkMatchType(inside_link_, link_match_type)), .is_inline_style = is_inline_style, .origin = origin });
    if (!is_cacheable) {
...
void ElementRuleCollector::AddTryStyleProperties()
{
    ...
    auto link_match_type = static_cast<unsigned>(CSSSelector::kMatchAll);
    result_.AddMatchedProperties(property_set,
        { .link_match_type = static_cast<uint8_t>(AdjustLinkMatchType(inside_link_, link_match_type)),
            .valid_property_filter = static_cast<uint8_t>(ValidPropertyFilter::kPositionTry),
            .is_try_style = true,
...
{
    const CSSPropertyValueSet* property_set = style_recalc_context_.try_tactics_set;
    if (!property_set) {
        return;
    }
    auto link_match_type = static_cast<unsigned>(CSSSelector::kMatchAll);
...
static bool RulesApplicableInCurrentTreeScope(const Element* element, const ContainerNode* scoping_node)
{
    // Check if the rules come from a shadow style sheet in the same tree scope.
    DCHECK(element->IsInTreeScope());
    return !scoping_node || element->GetTreeScope() == scoping_node->GetTreeScope();
}

bool SlowMatchWithNoResultFlags(const SelectorChecker& checker, SelectorChecker::SelectorCheckingContext& context, const CSSSelector& selector,
    const RuleData& rule_data, bool suppress_visited, unsigned expected_proximity = std::numeric_limits<unsigned>::max())
{
    SelectorChecker::MatchResult result;
    context.selector = &selector;
    context.match_visited = !suppress_visited && rule_data.LinkMatchType() == CSSSelector::kMatchVisited;
...
bool ElementRuleCollector::CollectMatchingRulesForListInternal(base::span<const RuleData> rules, const MatchRequest& match_request, const RuleSet* rule_set,
    int style_sheet_index, const SelectorChecker& checker, SelectorChecker::SelectorCheckingContext& context, PartRequest* part_request)
{
    bool reject_starting_styles = style_recalc_context_.is_ensuring_style || style_recalc_context_.old_style;

    CascadeLayerSeeker layer_seeker(stop_at_first_match ? nullptr : context.scope, context.vtt_originating_element, matching_ua_rules_,
        matching_rules_from_no_style_sheet_, &context.element->GetDocument(), rule_set);
    Seeker<ContainerQuery> container_query_seeker(rule_set->ContainerQueryIntervals());
    ...
    if (perf_trace_enabled) {
        selector_statistics_collector.ReserveCapacity(static_cast<wtf_size_t>(rules.size()));
    }
    ...
    for (const RuleData& rule_data : rules) {
        ...
        if (can_use_fast_reject_ && selector_filter_.FastRejectSelector(rule_data.DescendantSelectorIdentifierHashes(rule_set->BloomHashBacking()))) {
            fast_rejected++;
            if (perf_trace_enabled) {
        ...
        bool can_use_easy_selector_matching
            = !context.pseudo_element && context.vtt_originating_element == nullptr && !(context.scope && context.scope->OwnerShadowHost() == context.element);

        SelectorChecker::MatchResult result;
        if (can_use_easy_selector_matching && rule_data.IsEntirelyCoveredByBucketing()) {
            ...
            DCHECK(SlowMatchWithNoResultFlags(checker, context, selector, rule_data, suppress_visited_, result.proximity));
#endif
            ...
            context.style_scope = scope_seeker.Seek(rule_data.GetPosition());
            DCHECK(!context.style_scope);
            DCHECK_EQ(easy_match, SlowMatchWithNoResultFlags(checker, context, selector, rule_data, suppress_visited_, result.proximity))
                << "Mismatch for selector " << selector.SelectorText() << " on element " << context.element;
#endif
            if (!easy_match) {
                continue;
            }
            ...
            context.selector = &selector;
            context.match_visited = !suppress_visited_ && rule_data.LinkMatchType() == CSSSelector::kMatchVisited;
            context.style_scope = scope_seeker.Seek(rule_data.GetPosition());
            bool match = checker.Match(context, result);
            ...
            if (context.pseudo_element
                && (result.pseudo_ancestor_index == kNotFound || result.pseudo_ancestor_index < context.pseudo_element_ancestors.size() - 1)) {
                continue;
            }
            if (pseudo_style_request_.pseudo_id != kPseudoIdNone && pseudo_style_request_.pseudo_id != result.dynamic_pseudo) {
                continue;
            }
        }
        ...
        const ContainerQuery* container_query = container_query_seeker.Seek(rule_data.GetPosition());
        if (container_query) {
            ...
            if (pseudo_style_request_.pseudo_id != kPseudoIdNone || result.dynamic_pseudo == kPseudoIdNone) {
                Element* style_container_candidate = style_recalc_context_.style_container;
                if (!style_container_candidate) {
                    if (pseudo_style_request_.pseudo_id == kPseudoIdNone) {
                        style_container_candidate = ContainerQueryEvaluator::ParentContainerCandidateElement(context_.GetElement());
                    } else {
                        style_container_candidate = &context_.GetElement();
                    }
                }
                if (!EvaluateAndAddContainerQueries(style_container_candidate, *container_query, style_recalc_context_, container_selector_cache_, result_)) {
                ...
                for (const ContainerQuery* current = container_query; current; current = current->Parent()) {
                    ...
                    selects_scroll_state |= current->Selector().SelectsScrollStateContainers();
                }
                if (selects_size) {
        ...
        DidMatchRule(&rule_data, layer_order, container_query, result.proximity, result, style_sheet_index);
    }

    if (perf_trace_enabled) {
        DCHECK_EQ(mode_, SelectorChecker::kResolvingStyle);
        selector_statistics_collector.EndCollectionForCurrentRule();
        AggregateRulePerfData(
            current_matching_tree_scope_, context_.GetElement().GetDocument().GetStyleEngine(), selector_statistics_collector.PerRuleStatistics());
    }

    StyleEngine& style_engine = context_.GetElement().GetDocument().GetStyleEngine();
    ...
    INCREMENT_STYLE_STATS_COUNTER(style_engine, rules_fast_rejected, fast_rejected);
    INCREMENT_STYLE_STATS_COUNTER(style_engine, rules_matched, matched);
    return false;
}

template <bool stop_at_first_match>
bool ElementRuleCollector::CollectMatchingRulesForList(base::span<const RuleData> rules, const MatchRequest& match_request, const RuleSet* rule_set,
    int style_sheet_index, const SelectorChecker& checker, SelectorChecker::SelectorCheckingContext& context, PartRequest* part_request)
{
    ...
    if (!*g_selector_stats_tracing_enabled) {
        return CollectMatchingRulesForListInternal<stop_at_first_match, false>(
            rules, match_request, rule_set, style_sheet_index, checker, context, part_request);
    } else {
...
base::span<const Attribute> GetAttributes(const Element& element, bool need_style_synchronized)
{
    if (need_style_synchronized) {
        const AttributeCollection collection = element.Attributes();
        return base::span<const Attribute>(collection.data(), collection.size());
    } else {
        const AttributeCollection collection = element.AttributesWithoutStyleUpdate();
        return base::span<const Attribute>(collection.data(), collection.size());
    }
}
...
void ElementRuleCollector::CollectMatchingRules(const MatchRequest& match_request, PartNames* part_names)
{
    CollectMatchingRulesInternal</*stop_at_first_match=*/false>(match_request, part_names);
}

DISABLE_CFI_PERF
bool ElementRuleCollector::CheckIfAnyRuleMatches(const MatchRequest& match_request)
{
    return CollectMatchingRulesInternal</*stop_at_first_match=*/true>(match_request, /*part_names*/ nullptr);
}
...
bool ElementRuleCollector::CanRejectScope(const StyleScope& style_scope)
{
    ...
    return style_scope_frame && !style_scope_frame->HasSeenImplicitScope(style_scope);
}

template <bool stop_at_first_match>
DISABLE_CFI_PERF bool ElementRuleCollector::CollectMatchingRulesInternal(const MatchRequest& match_request, PartNames* part_names)
{
    DCHECK(!match_request.IsEmpty());

    SelectorChecker checker(part_names, pseudo_style_request_, mode_, matching_ua_rules_);

    ContextWithStyleScopeFrame context(context_, match_request, &pseudo_style_request_, style_recalc_context_.style_scope_frame);
    ...
    if (!pseudo_id.empty()) {
        ...
        for (const auto bundle : match_request.AllRuleSets()) {
            if (CollectMatchingRulesForList<stop_at_first_match>(
                    bundle.rule_set->UAShadowPseudoElementRules(pseudo_id), match_request, bundle.rule_set, bundle.style_sheet_index, checker, context.context)
                && stop_at_first_match) {
                return true;
            }
        }
    ...
    if (element.IsVTTElement()) {
        for (const auto bundle : match_request.AllRuleSets()) {
            if (CollectMatchingRulesForList<stop_at_first_match>(
                    bundle.rule_set->CuePseudoRules(), match_request, bundle.rule_set, bundle.style_sheet_index, checker, context.context)
                && stop_at_first_match) {
                return true;
            }
        }
    ...
    if (!matching_ua_rules_ && !RulesApplicableInCurrentTreeScope(&element, match_request.Scope())) {
        return false;
    }
    ...
    if (element.HasID()) {
        for (const auto bundle : match_request.AllRuleSets()) {
            if (CollectMatchingRulesForList<stop_at_first_match>(bundle.rule_set->IdRules(element.IdForStyleResolution()), match_request, bundle.rule_set,
    ...
    if (element.IsStyledElement() && element.HasClass()) {
        for (const AtomicString& class_name : element.ClassNames()) {
            for (const auto bundle : match_request.AllRuleSets()) {
                if (CollectMatchingRulesForList<stop_at_first_match>(
                        bundle.rule_set->ClassRules(class_name), match_request, bundle.rule_set, bundle.style_sheet_index, checker, context.context)
                    && stop_at_first_match) {
                    return true;
                }
            }
    ...
    if (has_any_attr_rules) {
        ...
        const bool lower_attrs_in_default_ns = !element.IsHTMLElement() && IsA<HTMLDocument>(element.GetDocument());
        ...
        base::span<const Attribute> attributes = GetAttributes(element, need_style_synchronized);
        ...
        for (unsigned attr_idx = 0; attr_idx < attributes.size(); ++attr_idx) {
            ...
            const AtomicString& lower_name
                = (lower_attrs_in_default_ns && attributes[attr_idx].NamespaceURI() == g_null_atom) ? attribute_name.LowerASCII() : attribute_name;
            for (const auto bundle : match_request.AllRuleSets()) {
                if (!bundle.rule_set->HasAnyAttrRules()) {
                    continue;
                }
                base::span<const RuleData> list = bundle.rule_set->AttrRules(lower_name);
                if (list.empty() || bundle.rule_set->CanIgnoreEntireList(list, lower_name, attributes[attr_idx].Value())) {
                    continue;
                }
                if (CollectMatchingRulesForList<stop_at_first_match>(
                        bundle.rule_set->AttrRules(lower_name), match_request, bundle.rule_set, bundle.style_sheet_index, checker, context.context)
                    && stop_at_first_match) {
                    return true;
                }
            }
            ...
            attributes = base::span<const Attribute>(collection.data(), collection.size()); 
        }
    }
    ...
    if (element.IsLink()) {
        for (const auto bundle : match_request.AllRuleSets()) {
            if (CollectMatchingRulesForList<stop_at_first_match>(
                    bundle.rule_set->LinkPseudoClassRules(), match_request, bundle.rule_set, bundle.style_sheet_index, checker, context.context)
                && stop_at_first_match) {
                return true;
            }
        }
    ...
    if (SelectorChecker::MatchesFocusPseudoClass(element, kPseudoIdNone)) {
        for (const auto bundle : match_request.AllRuleSets()) {
            if (CollectMatchingRulesForList<stop_at_first_match>(
                    bundle.rule_set->FocusPseudoClassRules(), match_request, bundle.rule_set, bundle.style_sheet_index, checker, context.context)
                && stop_at_first_match) {
                return true;
            }
        }
    ...
    if (SelectorChecker::MatchesSelectorFragmentAnchorPseudoClass(element)) {
        for (const auto bundle : match_request.AllRuleSets()) {
            if (CollectMatchingRulesForList<stop_at_first_match>(
                    bundle.rule_set->SelectorFragmentAnchorRules(), match_request, bundle.rule_set, bundle.style_sheet_index, checker, context.context)
                && stop_at_first_match) {
                return true;
            }
        }
    ...
    if (SelectorChecker::MatchesFocusVisiblePseudoClass(element)) {
        for (const auto bundle : match_request.AllRuleSets()) {
            if (CollectMatchingRulesForList<stop_at_first_match>(
                    bundle.rule_set->FocusVisiblePseudoClassRules(), match_request, bundle.rule_set, bundle.style_sheet_index, checker, context.context)
                && stop_at_first_match) {
                return true;
            }
        }
    ...
    if (element.GetDocument().documentElement() == element) {
        for (const auto bundle : match_request.AllRuleSets()) {
            if (CollectMatchingRulesForList<stop_at_first_match>(
                    bundle.rule_set->RootElementRules(), match_request, bundle.rule_set, bundle.style_sheet_index, checker, context.context)
                && stop_at_first_match) {
                return true;
            }
        }
    }
    AtomicString element_name = matching_ua_rules_ ? element.localName() : element.LocalNameForSelectorMatching();
    for (const auto bundle : match_request.AllRuleSets()) {
        if (CollectMatchingRulesForList<stop_at_first_match>(
                bundle.rule_set->TagRules(element_name), match_request, bundle.rule_set, bundle.style_sheet_index, checker, context.context)
            && stop_at_first_match) {
            return true;
        }
    }
    for (const auto bundle : match_request.AllRuleSets()) {
        if (CollectMatchingRulesForList<stop_at_first_match>(
                bundle.rule_set->UniversalRules(), match_request, bundle.rule_set, bundle.style_sheet_index, checker, context.context)
            && stop_at_first_match) {
            return true;
        }
    }
...
void ElementRuleCollector::CollectMatchingShadowHostRules(const MatchRequest& match_request)
{
    SelectorChecker checker(nullptr, pseudo_style_request_, mode_, matching_ua_rules_);

    ContextWithStyleScopeFrame context(context_, match_request, &pseudo_style_request_, style_recalc_context_.style_scope_frame);

    for (const auto bundle : match_request.AllRuleSets()) {
        CollectMatchingRulesForList</*stop_at_first_match=*/false>(
            bundle.rule_set->ShadowHostRules(), match_request, bundle.rule_set, bundle.style_sheet_index, checker, context.context);
...
bool ElementRuleCollector::CheckIfAnyShadowHostRuleMatches(const MatchRequest& match_request)
{
    SelectorChecker checker(nullptr, pseudo_style_request_, mode_, matching_ua_rules_);
...
void ElementRuleCollector::CollectMatchingSlottedRules(const MatchRequest& match_request)
{
    SelectorChecker checker(nullptr, pseudo_style_request_, mode_, matching_ua_rules_);
    ContextWithStyleScopeFrame context(context_, match_request, &pseudo_style_request_, style_recalc_context_.style_scope_frame);
    ...
    for (const auto bundle : match_request.AllRuleSets()) {
        CollectMatchingRulesForList</*stop_at_first_match=*/false>(
            bundle.rule_set->SlottedPseudoElementRules(), match_request, bundle.rule_set, bundle.style_sheet_index, checker, context.context);
    }
}

void ElementRuleCollector::CollectMatchingPartPseudoRules(const MatchRequest& match_request, PartNames* part_names, bool for_shadow_pseudo)
{
    ...
    for (const auto bundle : match_request.AllRuleSets()) {
        CollectMatchingRulesForList</*stop_at_first_match=*/false>(
            bundle.rule_set->PartPseudoRules(), match_request, bundle.rule_set, bundle.style_sheet_index, checker, context.context, &request);
    }
}
...
template <class CSSRuleCollection> static CSSRule* FindStyleRule(CSSRuleCollection* css_rules, const StyleRule* style_rule)
{
    ...
    for (unsigned i = 0; i < css_rules->length(); ++i) {
        ...
        if (auto* css_style_rule = DynamicTo<CSSStyleRule>(css_rule)) {
            ...
            if (CSSRule* result = FindStyleRule(css_style_rule->cssRules(), style_rule); result) {
                return result;
            }
        } else if (auto* css_import_rule = DynamicTo<CSSImportRule>(css_rule)) {
            if (CSSRule* result = FindStyleRule(css_import_rule->styleSheet(), style_rule); result) {
                return result;
            }
        } else if (CSSRule* result = FindStyleRule(css_rule->cssRules(), style_rule); result) {
            return result;
        } else if (auto* nested_declarations = DynamicTo<CSSNestedDeclarationsRule>(css_rule)) {
            if (nested_declarations->NestedDeclarationsRule()->InnerStyleRule() == style_rule) {
...
void ElementRuleCollector::AppendCSSOMWrapperForRule(const TreeScope* tree_scope_containing_rule, const RuleData* rule_data, wtf_size_t position)
{
    ...
    if (tree_scope_containing_rule) {
        for (const auto& [parent_style_sheet, rule_set] : tree_scope_containing_rule->GetScopedStyleResolver()->GetActiveStyleSheets()) {
...
void ElementRuleCollector::SortAndTransferMatchedRules(CascadeOrigin origin, bool is_vtt_embedded_style, StyleRuleUsageTracker* tracker)
{
    ...
    if (mode_ == SelectorChecker::kCollectingCSSRules) {
        for (unsigned i = 0; i < matched_rules_.size(); ++i) {
            AppendCSSOMWrapperForRule(current_matching_tree_scope_, matched_rules_[i].GetRuleData(), i);
        }
        return;
    }
    ...
    for (const MatchedRule& matched_rule : matched_rules_) {
        ...
        if (rule_data->IsStartingStyle()) {
            result_.AddFlags(static_cast<MatchFlags>(MatchFlag::kAffectedByStartingStyle));
        }
        result_.AddMatchedProperties(&rule_data->Rule()->Properties(),
            { .link_match_type = static_cast<uint8_t>(AdjustLinkMatchType(inside_link_, rule_data->LinkMatchType())),
                .valid_property_filter = static_cast<uint8_t>(rule_data->GetValidPropertyFilter(matching_ua_rules_)),
...
void ElementRuleCollector::DidMatchRule(const RuleData* rule_data, uint16_t layer_order, const ContainerQuery* container_query, unsigned proximity,
    const SelectorChecker::MatchResult& result, int style_sheet_index)
{
    ...
    if (dynamic_pseudo != kPseudoIdNone && pseudo_style_request_.pseudo_id == kPseudoIdNone) {
        if (mode_ == SelectorChecker::kCollectingCSSRules || mode_ == SelectorChecker::kCollectingStyleRules) {
            return;
        }
        if (dynamic_pseudo > kLastTrackedPublicPseudoId) {
            return;
        }
        if ((dynamic_pseudo == kPseudoIdCheck || dynamic_pseudo == kPseudoIdBefore || dynamic_pseudo == kPseudoIdAfter
        ...
        if (IsHighlightPseudoElement(dynamic_pseudo)) {
            ...
            if (CSSSelector::GetPseudoId(selector.GetPseudoType()) == dynamic_pseudo) {
                ...
                universal = next->IsLastInComplexSelector() && CSSSelector::GetPseudoId(next->GetPseudoType()) == dynamic_pseudo
                    && selector.Match() == CSSSelector::kTag && selector.TagQName().LocalName().IsNull() && selector.TagQName().Prefix() == g_star_atom;
            }
            ...
            if (dynamic_pseudo == kPseudoIdHighlight) {
                ...
                result_.AddCustomHighlightName(AtomicString(result.custom_highlight_name));
            }
        ...
        if (rule_data->Rule()->Properties().ContainsCursorHand()) {
            context_.GetElement().GetDocument().CountUse(WebFeature::kQuirksModeCursorHandApplied);
        }
        matched_rules_.emplace_back(rule_data, layer_order, proximity, style_sheet_index);
    }
}

void ElementRuleCollector::DumpAndClearRulesPerfMap()
{
    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("blink.debug"), "SelectorStats", "selector_stats", [&](perfetto::TracedValue context) {
        perfetto::TracedDictionary dict = std::move(context).WriteDictionary();
        {
            ...
            for (auto& it : GetSelectorStatisticsRuleMap()) {
                ...
                perfetto::TracedDictionary item_dict = std::move(item).WriteDictionary();
...
struct ElementRuleCollector::CompareRules {
    inline bool operator()(const MatchedRule& matched_rule1, const MatchedRule& matched_rule2) const
    {
#ifdef __SIZEOF_INT128__
        // https://github.com/llvm/llvm-project/issues/108418
        __uint128_t key1 = (__uint128_t { matched_rule1.SortKey() } << 64) | matched_rule1.GetPosition();
        __uint128_t key2 = (__uint128_t { matched_rule2.SortKey() } << 64) | matched_rule2.GetPosition();
#else
...
void ElementRuleCollector::AddMatchedRulesToTracker(StyleRuleUsageTracker* tracker) const
{
    for (auto matched_rule : matched_rules_) {
        const StyleRule* rule = matched_rule.GetRuleData()->Rule();
        tracker->Track(FindStyleSheet(current_matching_tree_scope_, context_.GetElement().GetDocument().GetStyleEngine(), rule), rule);
    }
}
