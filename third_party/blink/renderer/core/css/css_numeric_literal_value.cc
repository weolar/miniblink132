...
// static
CSSNumericLiteralValue* CSSNumericLiteralValue::Create(double value, UnitType type)
{
    ...
    case CSSPrimitiveValue::UnitType::kPixels:
        ...
        if (!result) {
            result = pool.SetPixelCacheValue(int_value, MakeGarbageCollected<CSSNumericLiteralValue>(value, type));
        }
        return result;
    case CSSPrimitiveValue::UnitType::kPercentage:
        result = pool.PercentCacheValue(int_value);
        if (!result) {
            result = pool.SetPercentCacheValue(int_value, MakeGarbageCollected<CSSNumericLiteralValue>(value, type));
        }
        return result;
    ...
    case CSSPrimitiveValue::UnitType::kInteger:
        ...
        if (!result) {
            result = pool.SetNumberCacheValue(int_value, MakeGarbageCollected<CSSNumericLiteralValue>(value, CSSPrimitiveValue::UnitType::kInteger));
        }
        return result;
    default:
        return MakeGarbageCollected<CSSNumericLiteralValue>(value, type);
...
unsigned CSSNumericLiteralValue::CustomHash() const
{
    ...
    return WTF::HashInts(static_cast<unsigned>(GetType()), WTF::HashInts(val >> 32, val));
}
...
CSSNumericLiteralValue* CSSNumericLiteralValue::CreateCanonicalUnitValue() const
{
