...
StyleRuleBase* ParseRuleForInsert(const ExecutionContext* execution_context, const String& rule_string, unsigned index, size_t num_child_rules,
    const CSSRule& parent_rule, ExceptionState& exception_state);
...
public:
    ...
    unsigned insertRule(const ExecutionContext*, const String& rule, unsigned index, ExceptionState&);
...
template <> struct DowncastTraits<CSSGroupingRule> {
