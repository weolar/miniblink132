...
public:
    CheckPseudoHasArgumentCompoundIterator(const CSSSelector* compound, Vector<unsigned>& pseudo_has_argument_hashes);

    void operator++();
    bool AtEnd() const
    {
        return !next_compound_;
    }
...
private:
    ...
    inline static bool NeedToCollectAffectedByFlagsFromSubSelector(const CSSSelector* simple_selector)
    {
    ...
    static void CollectAffectedByFlagsFromSimpleSelector(const CSSSelector* simple_selector, AffectedByFlags&);

    const CSSSelector* next_compound_;
    Vector<unsigned>& pseudo_has_argument_hashes_;
    CSSSelector::RelationType relation_to_next_compound_ = CSSSelector::kSubSelector;
    AffectedByFlags current_compound_affected_by_ = kMatchingElementOnly;
};

CheckPseudoHasArgumentCompoundIterator::CheckPseudoHasArgumentCompoundIterator(const CSSSelector* compound, Vector<unsigned>& pseudo_has_argument_hashes)
...
//
// static
void CheckPseudoHasArgumentCompoundIterator::CollectAffectedByFlagsFromSimpleSelector(const CSSSelector* simple_selector, AffectedByFlags& affected_by)
{
    ...
    for (; sub_selector; sub_selector = CSSSelectorList::Next(*sub_selector)) {
        ...
        for (const CSSSelector* selector = sub_selector; selector; selector = selector->NextSimpleSelector()) {
            AffectedByFlags simple_in_sub_affected_by = kMatchingElementOnly;

            CollectAffectedByFlagsFromSimpleSelector(selector, simple_in_sub_affected_by);

            if (simple_in_sub_affected_by & kSiblingsOfMatchingElement) {
                found_child_or_descendant_combinator_in_sub_selector ? affected_by |= kAncestorSiblingsOfMatchingElement
                                                                     : affected_by |= kSiblingsOfMatchingElement;
            }
...
void CheckPseudoHasArgumentCompoundIterator::operator++()
{
    ...
    for (const CSSSelector* simple_selector = next_compound_; simple_selector; simple_selector = simple_selector->NextSimpleSelector()) {
        CheckPseudoHasFastRejectFilter::CollectPseudoHasArgumentHashes(pseudo_has_argument_hashes_, simple_selector);

        CollectAffectedByFlagsFromSimpleSelector(simple_selector, current_compound_affected_by_);
...
CheckPseudoHasArgumentContext::CheckPseudoHasArgumentContext(const CSSSelector* selector, bool match_in_shadow_tree)
    ...
    , match_in_shadow_tree_(match_in_shadow_tree)
{
    ...
    CheckPseudoHasArgumentCompoundIterator iterator(selector, pseudo_has_argument_hashes_);
    ...
    case CSSSelector::kRelativeChild:
        ...
        siblings_affected_by_has_flags_ = SiblingsAffectedByHasFlags::kNoSiblingsAffectedByHasFlags;
        break;
    case CSSSelector::kRelativeIndirectAdjacent:
    case CSSSelector::kRelativeDirectAdjacent:
        if (DepthLimit() == 0) {
            ...
            siblings_affected_by_has_flags_ = SiblingsAffectedByHasFlags::kFlagForSiblingRelationship;
        } else {
            if (AdjacentDistanceFixed()) {
                if (DepthFixed()) {
            ...
            siblings_affected_by_has_flags_ = SiblingsAffectedByHasFlags::kFlagForSiblingDescendantRelationship;
        }
        break;
    default:
        NOTREACHED();
...
CheckPseudoHasArgumentTraversalIterator::CheckPseudoHasArgumentTraversalIterator(Element& has_anchor_element, CheckPseudoHasArgumentContext& context)
    : has_anchor_element_(&has_anchor_element)
    , match_in_shadow_tree_(context.MatchInShadowTree())
    , depth_limit_(context.DepthLimit())
{
    if (match_in_shadow_tree_) {
        if (!has_anchor_element.GetShadowRoot() || context.TraversalScope() == kInvalidShadowRootTraversalScope) {
    ...
    if (!context.AdjacentDistanceFixed()) {
        ...
        Element* last_sibling = ElementTraversal::LastChild(*has_anchor_element_->parentNode());
        ...
        for (; distance < context.AdjacentDistanceLimit() && sibling; distance++, sibling = ElementTraversal::NextSibling(*sibling)) {
            old_sibling = sibling;
        }
        if (sibling) {
...
Element* CheckPseudoHasArgumentTraversalIterator::LastWithin(ContainerNode* container_node)
{
    ...
    for (Element* descendant = ElementTraversal::LastChild(*container_node); descendant; descendant = ElementTraversal::LastChild(*descendant)) {
