...
void StyleTraversalRoot::Update(ContainerNode* common_ancestor, Node* dirty_node)
{
    ...
    if (!common_ancestor) {
        ...
        if (dirty_node->IsDocumentNode() || (root_node_ && dirty_node == document_element)) {
            ...
            DCHECK(!document_element || (!root_node_ && root_type_ == RootType::kSingleRoot));
        }
...
{
    DCHECK(root_node_);
    return root_node_->GetDocument().GetStyleEngine().InDOMRemoval() || root_node_->GetDocument().IsInSlotAssignmentRecalc();
}
#endif
