...
class CORE_EXPORT CSSPropertyValueSet : public GarbageCollected<CSSPropertyValueSet> {
...
public:
    ...
    public:
        ...
        {
            return PropertyMetadata().important_;
        }
        bool IsImplicit() const
        {
            return PropertyMetadata().implicit_;
        }
        ...
        {
            return Name() == other.Name() && IsImportant() == other.IsImportant() && Value() == other.Value();
        }
    ...
    MutableCSSPropertyValueSet* CopyPropertiesInSet(const Vector<const CSSProperty*>&) const;
...
class CORE_EXPORT alignas(std::max(alignof(Member<const CSSValue>), alignof(CSSPropertyValueMetadata))) ImmutableCSSPropertyValueSet
...
public:
    ...
    ImmutableCSSPropertyValueSet(PassKey, base::span<const CSSPropertyValue>, CSSParserMode, bool contains_cursor_hand = false);

    static ImmutableCSSPropertyValueSet* Create(base::span<const CSSPropertyValue>, CSSParserMode, bool contains_cursor_hand = false);
...
inline const Member<const CSSValue>* ImmutableCSSPropertyValueSet::ValueArrayBase() const
{
    static_assert(sizeof(ImmutableCSSPropertyValueSet) % alignof(Member<const CSSValue>) == 0, "ValueArray may be improperly aligned");
    ...
    return UNSAFE_BUFFERS(reinterpret_cast<const Member<const CSSValue>*>(this + 1));
}

inline const CSSPropertyValueMetadata* ImmutableCSSPropertyValueSet::MetadataArrayBase() const
{
    static_assert(sizeof(ImmutableCSSPropertyValueSet) % alignof(CSSPropertyValueMetadata) == 0, "MetadataArray may be improperly aligned");
    // Size of Member<> can be smaller than that of CSSPropertyValueMetadata.
    // Align it up.
    // SAFETY: By funneling all allocation of ImmutableCSSPropertyValueSet through
    // Create(), we guarantee that the array will exist where we expect it.
    CHECK_GT(array_size_, 0u);
    return UNSAFE_BUFFERS(reinterpret_cast<const CSSPropertyValueMetadata*>(
        base::bits::AlignUp(reinterpret_cast<const uint8_t*>(ValueArrayBase() + array_size_), alignof(CSSPropertyValueMetadata))));
}

inline base::span<const Member<const CSSValue>> ImmutableCSSPropertyValueSet::ValueArray() const
{
    if (array_size_ == 0) {
...
inline base::span<const CSSPropertyValueMetadata> ImmutableCSSPropertyValueSet::MetadataArray() const
{
    if (array_size_ == 0) {
...
template <> struct DowncastTraits<ImmutableCSSPropertyValueSet> {
...
public:
    ...
    explicit MutableCSSPropertyValueSet(base::span<const CSSPropertyValue> properties);
    ...
    SetResult ParseAndSetProperty(
        CSSPropertyID unresolved_property, StringView value, bool important, SecureContextMode, StyleSheetContents* context_style_sheet = nullptr);
    ...
    SetResult ParseAndSetCustomProperty(const AtomicString& custom_property_name, StringView value, bool important, SecureContextMode,
        StyleSheetContents* context_style_sheet, bool is_animation_tainted);
    ...
    void Clear();
    void ParseDeclarationList(const String& style_declaration, SecureContextMode, StyleSheetContents* context_style_sheet);

    CSSStyleDeclaration* EnsureCSSStyleDeclaration(ExecutionContext* execution_context);
...
template <> struct DowncastTraits<MutableCSSPropertyValueSet> {
    static bool AllowFrom(const CSSPropertyValueSet& set)
    {
        return set.IsMutable();
    }
...
template <typename T> inline int CSSPropertyValueSet::FindPropertyIndex(const T& property) const
{
    if (auto* mutable_property_set = DynamicTo<MutableCSSPropertyValueSet>(this)) {
