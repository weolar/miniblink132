...
public:
    CSSUrlData(const AtomicString& unresolved_url, const KURL& resolved_url, const Referrer&, OriginClean, bool is_ad_related);
    ...
    CSSUrlData MakeResolved(const KURL& base_url, const WTF::TextEncoding& charset) const;
    ...
    const AtomicString& UnresolvedUrl() const
    {
        return relative_url_;
    }
    const AtomicString& ResolvedUrl() const
    {
    ...
    {
        return is_from_origin_clean_style_sheet_ ? OriginClean::kTrue : OriginClean::kFalse;
    }
