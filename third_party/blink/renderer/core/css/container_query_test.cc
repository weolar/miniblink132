...
public:
    ...
    StyleRuleContainer* ParseAtContainer(String rule_string, UnknownHandling unknown_handling = UnknownHandling::kError)
    {
        auto* rule = DynamicTo<StyleRuleContainer>(css_test_helpers::ParseRule(GetDocument(), rule_string));
    ...
    ContainerQuery* ParseContainerQuery(String query, UnknownHandling unknown_handling = UnknownHandling::kError)
    {
    ...
    std::optional<MediaQueryExpNode::FeatureFlags> FeatureFlagsFrom(String query_string)
    {
        ContainerQuery* query = ParseContainerQuery(query_string, UnknownHandling::kAllow);
        if (!query) {
            return std::nullopt;
        }
    ...
    size_t GetOldStylesCount(String html)
    {
        ...
        size_t old_styles_count = PostStyleUpdateScope::CurrentAnimationData()->old_styles_.size();
...
{
    // Valid:
    EXPECT_EQ("(min-width: 300px)", SerializeCondition(ParseAtContainer("@container (min-width: 300px) {}")));
    EXPECT_EQ("(max-width: 500px)", SerializeCondition(ParseAtContainer("@container (max-width: 500px) {}")));
    EXPECT_EQ("(not (max-width: 500px))", SerializeCondition(ParseAtContainer("@container (not (max-width: 500px)) {}")));
    ...
    EXPECT_EQ("((max-width: 500px) or (max-height: 500px))",
        SerializeCondition(ParseAtContainer("@container ((max-width: 500px) "
                                            "or (max-height: 500px)) {}")));
    EXPECT_EQ("(width < 300px)", SerializeCondition(ParseAtContainer("@container (width < 300px) {}")));

    EXPECT_EQ("somename not (width)", SerializeCondition(ParseAtContainer("@container somename not (width) {}")));

    EXPECT_EQ("(width) and (height)", SerializeCondition(ParseAtContainer("@container (width) and (height) {}")));

    EXPECT_EQ("(width) or (height)", SerializeCondition(ParseAtContainer("@container (width) or (height) {}")));

    EXPECT_EQ("test_name (width) or (height)", SerializeCondition(ParseAtContainer("@container test_name (width) or (height) {}")));

    EXPECT_EQ("test_name ((max-width: 500px) or (max-height: 500px))",
        SerializeCondition(ParseAtContainer("@container test_name ((max-width: 500px) "
                                            "or (max-height: 500px)) {}")));
...
TEST_F(ContainerQueryTest, ValidFeatures)
{
    ...
    EXPECT_TRUE(ParseAtContainer("@container test_name (orientation: portrait) {}"));
...
{
    EXPECT_EQ(MediaQueryExpNode::kFeatureUnknown, FeatureFlagsFrom("(width: 100gil)"));
    EXPECT_EQ(MediaQueryExpNode::kFeatureWidth, FeatureFlagsFrom("(width: 100px)"));
    EXPECT_EQ(MediaQueryExpNode::kFeatureWidth, FeatureFlagsFrom("test_name (width: 100px)"));
    EXPECT_EQ(MediaQueryExpNode::kFeatureHeight, FeatureFlagsFrom("(height < 100px)"));
    EXPECT_EQ(MediaQueryExpNode::kFeatureInlineSize, FeatureFlagsFrom("(100px >= inline-size)"));
    EXPECT_EQ(MediaQueryExpNode::kFeatureBlockSize, FeatureFlagsFrom("(100px = block-size)"));
    EXPECT_EQ(static_cast<MediaQueryExpNode::FeatureFlags>(MediaQueryExpNode::kFeatureWidth | MediaQueryExpNode::kFeatureBlockSize),
        FeatureFlagsFrom("((width) and (100px = block-size))"));
    EXPECT_EQ(static_cast<MediaQueryExpNode::FeatureFlags>(MediaQueryExpNode::kFeatureUnknown | MediaQueryExpNode::kFeatureBlockSize),
        FeatureFlagsFrom("((unknown) and (100px = block-size))"));
    EXPECT_EQ(static_cast<MediaQueryExpNode::FeatureFlags>(
                  MediaQueryExpNode::kFeatureWidth | MediaQueryExpNode::kFeatureHeight | MediaQueryExpNode::kFeatureInlineSize),
        FeatureFlagsFrom("((width) or (height) or (inline-size))"));
    EXPECT_EQ(MediaQueryExpNode::kFeatureWidth, FeatureFlagsFrom("((width: 100px))"));
    EXPECT_EQ(MediaQueryExpNode::kFeatureWidth, FeatureFlagsFrom("(not (width: 100px))"));
}
...
TEST_F(ContainerQueryTest, ImplicitContainerSelector)
{
    ...
    EXPECT_EQ(kContainerTypeInlineSize, inline_size.Type(WritingMode::kHorizontalTb));
    EXPECT_EQ(kContainerTypeInlineSize, inline_size.Type(WritingMode::kVerticalRl));

    ContainerSelector block_size = ContainerSelectorFrom("(block-size: 100px)");
    EXPECT_EQ(kContainerTypeBlockSize, block_size.Type(WritingMode::kHorizontalTb));
    EXPECT_EQ(kContainerTypeBlockSize, block_size.Type(WritingMode::kVerticalRl));

    ContainerSelector width_height = ContainerSelectorFrom("((width: 100px) or (height: 100px))");
    EXPECT_EQ((kContainerTypeInlineSize | kContainerTypeBlockSize), width_height.Type(WritingMode::kHorizontalTb));
    EXPECT_EQ((kContainerTypeInlineSize | kContainerTypeBlockSize), width_height.Type(WritingMode::kVerticalRl));

    ContainerSelector inline_block_size = ContainerSelectorFrom("((inline-size: 100px) or (block-size: 100px))");
    EXPECT_EQ((kContainerTypeInlineSize | kContainerTypeBlockSize), inline_block_size.Type(WritingMode::kHorizontalTb));
    EXPECT_EQ((kContainerTypeInlineSize | kContainerTypeBlockSize), inline_block_size.Type(WritingMode::kVerticalRl));

    ContainerSelector aspect_ratio = ContainerSelectorFrom("(aspect-ratio: 1/2)");
    EXPECT_EQ((kContainerTypeInlineSize | kContainerTypeBlockSize), aspect_ratio.Type(WritingMode::kHorizontalTb));
    EXPECT_EQ((kContainerTypeInlineSize | kContainerTypeBlockSize), aspect_ratio.Type(WritingMode::kVerticalRl));

    ContainerSelector orientation = ContainerSelectorFrom("(orientation: portrait)");
    EXPECT_EQ((kContainerTypeInlineSize | kContainerTypeBlockSize), orientation.Type(WritingMode::kHorizontalTb));
    EXPECT_EQ((kContainerTypeInlineSize | kContainerTypeBlockSize), orientation.Type(WritingMode::kVerticalRl));
}

TEST_F(ContainerQueryTest, ScrollStateContainerSelector)
{
    ContainerSelector stuck_right = ContainerSelectorFrom("scroll-state(stuck: right)");
    EXPECT_EQ(kContainerTypeScrollState, stuck_right.Type(WritingMode::kHorizontalTb));

    ContainerSelector stuck_and_style = ContainerSelectorFrom("scroll-state(stuck: right) and style(--foo: bar)");
    EXPECT_EQ(kContainerTypeScrollState, stuck_and_style.Type(WritingMode::kHorizontalTb));

    ContainerSelector stuck_and_inline_size = ContainerSelectorFrom("scroll-state(stuck: block-end) or (inline-size > 10px)");
    EXPECT_EQ((kContainerTypeScrollState | kContainerTypeInlineSize), stuck_and_inline_size.Type(WritingMode::kHorizontalTb));

    ContainerSelector stuck_and_block_size = ContainerSelectorFrom("scroll-state(stuck: block-end) and (height)");
    EXPECT_EQ((kContainerTypeScrollState | kContainerTypeBlockSize), stuck_and_block_size.Type(WritingMode::kHorizontalTb));
}
...
TEST_F(ContainerQueryTest, RuleParsing)
{
    ...
    auto* rule = DynamicTo<CSSContainerRule>(container->CreateCSSOMWrapper(/*position_hint=*/0, sheet));
    ASSERT_TRUE(rule);
...
TEST_F(ContainerQueryTest, RuleCopy)
{
    ...
    EXPECT_EQ(&GetInnerQuery(container->GetContainerQuery()), &GetInnerQuery(copy->GetContainerQuery()));
}
...
TEST_F(ContainerQueryTest, QueryZoom)
{
    ...
    EXPECT_TRUE(target1->ComputedStyleRef().GetVariableData(AtomicString("--w100")));
    EXPECT_TRUE(target1->ComputedStyleRef().GetVariableData(AtomicString("--h200")));
    EXPECT_FALSE(target1->ComputedStyleRef().GetVariableData(AtomicString("--w200")));
    EXPECT_FALSE(target1->ComputedStyleRef().GetVariableData(AtomicString("--h400")));

    EXPECT_FALSE(target2->ComputedStyleRef().GetVariableData(AtomicString("--w100")));
    EXPECT_FALSE(target2->ComputedStyleRef().GetVariableData(AtomicString("--h200")));
    EXPECT_TRUE(target2->ComputedStyleRef().GetVariableData(AtomicString("--w200")));
    EXPECT_TRUE(target2->ComputedStyleRef().GetVariableData(AtomicString("--h400")));
}
...
TEST_F(ContainerQueryTest, QueryFontRelativeWithZoom)
{
    ...
    EXPECT_TRUE(em_target->ComputedStyleRef().GetVariableData(AtomicString("--em")));
    EXPECT_TRUE(ex_target->ComputedStyleRef().GetVariableData(AtomicString("--ex")));
    EXPECT_TRUE(ch_target->ComputedStyleRef().GetVariableData(AtomicString("--ch")));
}
...
TEST_F(ContainerQueryTest, ContainerUnitsViewportFallback)
{
    ...
    RegisterProperty(GetDocument(), "--fallback-min-cqi-vh", "<length>", "0px", false);
    RegisterProperty(GetDocument(), "--fallback-max-cqi-vh", "<length>", "0px", false);
    ...
    Element* inline_target = GetDocument().getElementById(AtomicString("inline_target"));
    ...
    EXPECT_EQ(ComputedValueString(inline_target, "--cqh"), ComputedValueString(inline_target, "--fallback-h"));
    EXPECT_EQ(ComputedValueString(inline_target, "--cqb"), ComputedValueString(inline_target, "--fallback-h"));
    EXPECT_EQ(ComputedValueString(inline_target, "--cqmin"), ComputedValueString(inline_target, "--fallback-min-cqi-vh"));
    EXPECT_EQ(ComputedValueString(inline_target, "--cqmax"), ComputedValueString(inline_target, "--fallback-max-cqi-vh"));

    Element* size_target = GetDocument().getElementById(AtomicString("size_target"));
...
TEST_F(ContainerQueryTest, OldStyleForTransitions)
{
    ...
    // Simulate a style and layout pass with multiple rounds of style recalc.
    {
        ...
        GetDocument().GetStyleEngine().UpdateStyleAndLayoutTreeForContainer(*container, LogicalSize(120, -1), kLogicalAxesInline);
        EXPECT_EQ("15px", ComputedValueString(target, "height"));
        EXPECT_EQ(0u, GetAnimationsCount(target));

        // Should transition between [10px, 30px]. (Intermediate round).
        GetDocument().GetStyleEngine().UpdateStyleAndLayoutTreeForContainer(*container, LogicalSize(130, -1), kLogicalAxesInline);
...
TEST_F(ContainerQueryTest, AllAnimationAffectingPropertiesInConditional)
{
    ...
    auto generate_html = [](const CSSProperty& property) -> String {
        ...
        builder.Append(String::Format("%s:unset;", property.GetPropertyNameString().Utf8().c_str()));
        builder.Append("  }");
        builder.Append("}");
        builder.Append("</style>");
...
TEST_F(ContainerQueryTest, CQDependentContentVisibilityHidden)
{
    ...
    EXPECT_TRUE(locker->firstElementChild()->GetComputedStyle()) << "The #locker element does not get content-visibility:hidden on the "
...
TEST_F(ContainerQueryTest, TreeScopedReferenceUserOrigin)
{
    ...
    auto* parsed_user_sheet = MakeGarbageCollected<StyleSheetContents>(MakeGarbageCollected<CSSParserContext>(GetDocument()));
    ...
    GetStyleEngine().InjectSheet(user_sheet_key, parsed_user_sheet, WebCssOrigin::kUser);
    ...
    Element* shadow_author_target = shadow_root->getElementById(AtomicString("shadow_author_target"));
    Element* shadow_user_target = shadow_root->getElementById(AtomicString("shadow_user_target"));
