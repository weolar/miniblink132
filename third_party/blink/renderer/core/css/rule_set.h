...
public:
    ...
    RuleData(StyleRule*, unsigned selector_index, unsigned position, const StyleScope*, AddRuleFlags, Vector<unsigned>& bloom_hash_backing);

    unsigned GetPosition() const
    {
        return position_;
    }
    StyleRule* Rule() const
    {
        return rule_.Get();
    }
    ...
    {
        return link_match_type_;
    }
    ValidPropertyFilter GetValidPropertyFilter(bool is_matching_ua_rules = false) const
    {
        return is_matching_ua_rules ? ValidPropertyFilter::kNoFilter : static_cast<ValidPropertyFilter>(valid_property_filter_);
    }

    // Member functions related to the descendant Bloom filter.
    const base::span<const unsigned> DescendantSelectorIdentifierHashes(const Vector<unsigned>& backing) const
    {
        return { backing.data() + bloom_hash_pos_, bloom_hash_size_ };
    }
    void ComputeBloomFilterHashes(const StyleScope* style_scope, Vector<unsigned>& backing);
    void MovedToDifferentRuleSet(const Vector<unsigned>& old_backing, Vector<unsigned>& new_backing, unsigned new_position);
...
public:
    ...
    base::span<const RuleData> Find(const AtomicString& key) const
    {
        ...
        const RobinHoodMap<AtomicString, Extent>::Bucket* bucket = buckets.Find(key);
        if (bucket == nullptr) {
            return {};
        } else {
            return { backing.begin() + bucket->value.start_index, bucket->value.length };
        }
    }
    bool IsEmpty() const
    {
    ...
    struct ConstIterator {
        ...
        WTF::KeyValuePair<AtomicString, base::span<const RuleData>> operator*() const
        {
    ...
    {
        return { buckets.begin(), this };
    }
    ConstIterator end() const
    {
        return { buckets.end(), this };
    }
...
public:
    ...
    void AddRulesFromSheet(StyleSheetContents*, const MediaQueryEvaluator&, CascadeLayer* = nullptr);
    ...
    // AddChildRules().
    void AddStyleRule(StyleRule* style_rule, StyleRule* parent_rule, const MediaQueryEvaluator& medium, AddRuleFlags add_rule_flags, bool within_mixin,
        const ContainerQuery* container_query = nullptr, CascadeLayer* cascade_layer = nullptr, const StyleScope* style_scope = nullptr);
    ...
    void AddFilteredRulesFromOtherSet(const RuleSet& other, const HeapHashSet<Member<StyleRule>>& only_include);
    ...
    bool HasAnyAttrRules() const
    {
        return !attr_rules_.IsEmpty();
    }
    base::span<const RuleData> AttrRules(const AtomicString& key) const
    {
        return attr_rules_.Find(key);
    }
    bool CanIgnoreEntireList(base::span<const RuleData> list, const AtomicString& key, const AtomicString& value) const;
    base::span<const RuleData> TagRules(const AtomicString& key) const
    {
        return tag_rules_.Find(key);
    }
    base::span<const RuleData> UAShadowPseudoElementRules(const AtomicString& key) const
    {
        return ua_shadow_pseudo_element_rules_.Find(key);
    }

    static base::span<const RuleData> HeapVectorRuleDataToSpan(const HeapVector<RuleData>& rd)
    {
        return base::span<const RuleData>(rd.data(), rd.size());
    }

    base::span<const RuleData> LinkPseudoClassRules() const
    {
        return HeapVectorRuleDataToSpan(link_pseudo_class_rules_);
    }
    base::span<const RuleData> CuePseudoRules() const
    {
        return HeapVectorRuleDataToSpan(cue_pseudo_rules_);
    }
    base::span<const RuleData> FocusPseudoClassRules() const
    {
        return HeapVectorRuleDataToSpan(focus_pseudo_class_rules_);
    }
    base::span<const RuleData> FocusVisiblePseudoClassRules() const
    {
        return HeapVectorRuleDataToSpan(focus_visible_pseudo_class_rules_);
    }
    base::span<const RuleData> RootElementRules() const
    {
        return HeapVectorRuleDataToSpan(root_element_rules_);
    }
    base::span<const RuleData> UniversalRules() const
    {
        return HeapVectorRuleDataToSpan(universal_rules_);
    }
    base::span<const RuleData> ShadowHostRules() const
    {
        return HeapVectorRuleDataToSpan(shadow_host_rules_);
    }
    base::span<const RuleData> PartPseudoRules() const
    {
        return HeapVectorRuleDataToSpan(part_pseudo_rules_);
    }
    base::span<const RuleData> SelectorFragmentAnchorRules() const
    {
        return HeapVectorRuleDataToSpan(selector_fragment_anchor_rules_);
    }
    ...
    const HeapVector<Member<StyleRuleFontPaletteValues>>& FontPaletteValuesRules() const
    {
        return font_palette_values_rules_;
    }
    const HeapVector<Member<StyleRuleFontFeatureValues>>& FontFeatureValuesRules() const
    {
        return font_feature_values_rules_;
    }
    const HeapVector<Member<StyleRuleViewTransition>>& ViewTransitionRules() const
    {
    ...
    {
        return HeapVectorRuleDataToSpan(slotted_pseudo_element_rules_);
    }

    bool HasCascadeLayers() const
    {
        return implicit_outer_layer_ != nullptr;
    }
    ...
    bool HasPartPseudoRules() const
    {
        return !part_pseudo_rules_.empty();
    }

    bool HasBucketForStyleAttribute() const
    {
        return has_bucket_for_style_attr_;
    }
    ...
    public:
        Interval(const T* passed_value, unsigned passed_position)
            : value(passed_value)
            , start_position(passed_position)
        {
        }
...
#if DCHECK_IS_ON()
    void Show() const;
    const HeapVector<RuleData>& AllRulesForTest() const
    {
        return all_rules_;
    }
#endif // DCHECK_IS_ON()
...
private:
    ...
    using SubstringMatcherMap = HashMap<AtomicString, std::unique_ptr<base::SubstringSetMatcher>>;
    ...
    bool MatchMediaForAddRules(const MediaQueryEvaluator& evaluator, const MediaQuerySet* media_queries);
    void AddChildRules(StyleRule* parent_rule, const HeapVector<Member<StyleRuleBase>>&, const MediaQueryEvaluator& medium, AddRuleFlags, const ContainerQuery*,
        CascadeLayer*, const StyleScope*, bool within_mixin);
    ...
    template <BucketCoverage bucket_coverage> void FindBestRuleSetAndAdd(CSSSelector&, const RuleData&);

    void AddRule(StyleRule*, unsigned selector_index, AddRuleFlags, const ContainerQuery*, const CascadeLayer*, const StyleScope*);
    ...
    void NewlyAddedFromDifferentRuleSet(const RuleData& old_rule_data, const StyleScope* style_scope, const RuleSet& old_rule_set, RuleData& new_rule_data);

    void SortKeyframesRulesIfNeeded();

    void CompactRules();
    static void CreateSubstringMatchers(RuleMap& attr_map, SubstringMatcherMap& substring_matcher_map);
    ...
    CascadeLayer* GetOrAddSubLayer(CascadeLayer*, const StyleRuleBase::LayerName&);
...
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::RuleSet::Interval<blink::CascadeLayer>)
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::RuleSet::Interval<blink::ContainerQuery>)
WTF_ALLOW_CLEAR_UNUSED_SLOTS_WITH_MEM_FUNCTIONS(blink::RuleSet::Interval<blink::StyleScope>)
