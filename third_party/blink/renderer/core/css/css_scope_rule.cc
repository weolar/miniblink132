...
String CSSScopeRule::PreludeText() const
{
    ...
    const StyleScope& scope = To<StyleRuleScope>(*group_rule_.Get()).GetStyleScope();
...
{
    const StyleScope& scope = To<StyleRuleScope>(*group_rule_.Get()).GetStyleScope();
    return scope.From() ? CSSSelectorList::SelectorsText(scope.From()) : g_null_atom;
}
...
void CSSScopeRule::SetPreludeText(const ExecutionContext* execution_context, String value)
{
    ...
    StyleSheetContents* contents = style_sheet ? style_sheet->Contents() : nullptr;

    GetStyleRuleScope().SetPreludeText(execution_context, value, nesting_type, parent_rule_for_nesting, is_within_scope, contents);
}
