...
public:
    ...
    template <typename T>
    CSSIdentifierValue(T t) // Overriden for special cases in CSSPrimitiveValueMappings.h
        : CSSValue(kIdentifierClass)
        , value_id_(PlatformEnumToCSSValueID(t))
    {
    }
    ...
    {
        return static_cast<unsigned>(value_id_);
    }

    template <typename T> inline T ConvertTo() const
    { // Overridden for special cases in CSSPrimitiveValueMappings.h
        return CssValueIDToPlatformEnum<T>(value_id_);
    }

    bool WasQuirky() const
    {
        return was_quirky_;
    }
...
template <> struct DowncastTraits<CSSIdentifierValue> {
