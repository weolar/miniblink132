...
protected:
    ...
    {
        auto* contents
            = MakeGarbageCollected<StyleSheetContents>(MakeGarbageCollected<CSSParserContext>(kHTMLStandardMode, SecureContextMode::kInsecureContext));
...
TEST_F(ActiveStyleSheetsTest, CompareActiveStyleSheets_NoChange)
{
    ...
    EXPECT_EQ(kNoActiveSheetsChanged, CompareActiveStyleSheets(old_sheets, new_sheets, {}, changed_rule_sets));
    EXPECT_EQ(0u, changed_rule_sets.size());

    CSSStyleSheet* sheet1 = CreateSheet();
    CSSStyleSheet* sheet2 = CreateSheet();

    old_sheets.push_back(std::make_pair(sheet1, &sheet1->Contents()->GetRuleSet()));
    old_sheets.push_back(std::make_pair(sheet2, &sheet2->Contents()->GetRuleSet()));

    new_sheets.push_back(std::make_pair(sheet1, &sheet1->Contents()->GetRuleSet()));
    new_sheets.push_back(std::make_pair(sheet2, &sheet2->Contents()->GetRuleSet()));
...
TEST_F(ActiveStyleSheetsTest, CompareActiveStyleSheets_AppendedToEmpty)
{
    ...
    EXPECT_EQ(kActiveSheetsAppended, CompareActiveStyleSheets(old_sheets, new_sheets, {}, changed_rule_sets));
...
TEST_F(ActiveStyleSheetsTest, CompareActiveStyleSheets_Mutated)
{
    ...
    old_sheets.push_back(std::make_pair(sheet1, &sheet1->Contents()->GetRuleSet()));
    old_sheets.push_back(std::make_pair(sheet2, &sheet2->Contents()->GetRuleSet()));
    old_sheets.push_back(std::make_pair(sheet3, &sheet3->Contents()->GetRuleSet()));

    sheet2->Contents()->ClearRuleSet();
    sheet2->Contents()->EnsureRuleSet(MediaQueryEvaluator(GetDocument().GetFrame()));

    EXPECT_NE(old_sheets[1].second, &sheet2->Contents()->GetRuleSet());

    new_sheets.push_back(std::make_pair(sheet1, &sheet1->Contents()->GetRuleSet()));
    new_sheets.push_back(std::make_pair(sheet2, &sheet2->Contents()->GetRuleSet()));
    new_sheets.push_back(std::make_pair(sheet3, &sheet3->Contents()->GetRuleSet()));

    EXPECT_EQ(kActiveSheetsChanged, CompareActiveStyleSheets(old_sheets, new_sheets, {}, changed_rule_sets));
    EXPECT_EQ(2u, changed_rule_sets.size());
...
TEST_F(ActiveStyleSheetsTest, CompareActiveStyleSheets_RemoveNullRuleSetAndAppend)
{
    ActiveStyleSheetVector old_sheets;
    ActiveStyleSheetVector new_sheets;
    HeapHashSet<Member<RuleSet>> changed_rule_sets;
...
TEST_F(ActiveStyleSheetsTest, CompareActiveStyleSheets_AddRemoveNonMatchingMQ)
{
    ...
    CSSStyleSheet* sheet1 = CreateSheet();
    MediaQuerySet* mq = MediaQueryParser::ParseMediaQuerySet("(min-width: 9000px)", nullptr);
    ...
    EXPECT_EQ(kActiveSheetsChanged, CompareActiveStyleSheets(new_sheets, old_sheets, {}, changed_rule_sets));
    EXPECT_EQ(0u, changed_rule_sets.size());
}
...
TEST_F(ApplyRulesetsTest, AddUniversalRuleToDocument)
{
    ...
    new_style_sheets.push_back(std::make_pair(sheet, &sheet->Contents()->GetRuleSet()));

    GetStyleEngine().ApplyRuleSetChanges(GetDocument(), ActiveStyleSheetVector(), new_style_sheets, {});
...
TEST_F(ApplyRulesetsTest, AddUniversalRuleToShadowTree)
{
    ...
    GetStyleEngine().ApplyRuleSetChanges(shadow_root, ActiveStyleSheetVector(), new_style_sheets, {});
...
TEST_F(ApplyRulesetsTest, AddFontFaceRuleToDocument)
{
    ...
    EXPECT_EQ(kNoStyleChange, GetDocument().documentElement()->GetStyleChangeType());
}
...
TEST_F(ApplyRulesetsTest, RemoveSheetFromShadowTree)
{
    ...
    old_style_sheets.push_back(std::make_pair(css_sheet, &css_sheet->Contents()->GetRuleSet()));
    GetStyleEngine().ApplyRuleSetChanges(shadow_root, old_style_sheets, ActiveStyleSheetVector(), {});
}
