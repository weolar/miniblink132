...
protected:
    ...
    const CSSValue* ComputedValue(Element* element, String property_name)
    {
        ...
        DCHECK(ref.IsValid());
        return ref.GetProperty().CSSValueFromComputedStyle(element->ComputedStyleRef(),
...
void StyleEngineTest::ApplyRuleSetInvalidation(TreeScope& tree_scope, const String& css_text)
{
    auto* sheet = MakeGarbageCollected<StyleSheetContents>(MakeGarbageCollected<CSSParserContext>(kHTMLStandardMode, SecureContextMode::kInsecureContext));
    sheet->ParseString(css_text);
    HeapHashSet<Member<RuleSet>> rule_sets;
    RuleSet& rule_set = sheet->EnsureRuleSet(MediaQueryEvaluator(GetDocument().GetFrame()));
    ...
    GetStyleEngine().ApplyRuleSetInvalidationForTreeScope(
        tree_scope, tree_scope.RootNode(), selector_filter, style_scope_frame, rule_sets, /*changed_rule_flags=*/0);
}

TEST_F(StyleEngineTest, DocumentDirtyAfterInject)
{
    auto* parsed_sheet = MakeGarbageCollected<StyleSheetContents>(MakeGarbageCollected<CSSParserContext>(GetDocument()));
...
TEST_F(StyleEngineTest, AnalyzedInject)
{
    ...
    EXPECT_EQ(Color::FromRGB(255, 0, 0), t1->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    EXPECT_EQ(Color::FromRGB(0, 0, 0), t2->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    EXPECT_EQ(Color::FromRGB(0, 0, 0), t3->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    const unsigned initial_count = GetStyleEngine().StyleForElementCount();

    auto* green_parsed_sheet = MakeGarbageCollected<StyleSheetContents>(MakeGarbageCollected<CSSParserContext>(GetDocument()));
    green_parsed_sheet->ParseString("#t1 { color: green !important }"
                                    "#t2 { color: white !important }"
                                    "#t3 { color: white }");
    StyleSheetKey green_key("green");
    GetStyleEngine().InjectSheet(green_key, green_parsed_sheet, WebCssOrigin::kUser);
    UpdateAllLifecyclePhases();
    ...
    EXPECT_EQ(Color::FromRGB(0, 128, 0), t1->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    EXPECT_EQ(Color::FromRGB(255, 255, 255), t2->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    EXPECT_EQ(Color::FromRGB(0, 0, 0), t3->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    auto* blue_parsed_sheet = MakeGarbageCollected<StyleSheetContents>(MakeGarbageCollected<CSSParserContext>(GetDocument()));
    blue_parsed_sheet->ParseString("#t1 { color: blue !important }"
                                   "#t2 { color: silver }"
                                   "#t3 { color: silver !important }");
    StyleSheetKey blue_key("blue");
    GetStyleEngine().InjectSheet(blue_key, blue_parsed_sheet, WebCssOrigin::kUser);
    UpdateAllLifecyclePhases();
    ...
    EXPECT_EQ(Color::FromRGB(0, 0, 255), t1->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    EXPECT_EQ(Color::FromRGB(255, 255, 255), t2->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    // Important user rules override inline author rules.
    EXPECT_EQ(Color::FromRGB(192, 192, 192), t3->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    ...
    EXPECT_EQ(1u, GetStyleEngine().GetFontSelector()->GetFontFaceCache()->GetNumSegmentedFacesForTesting());
    CSSSegmentedFontFace* font_face
        = GetStyleEngine().GetFontSelector()->GetFontFaceCache()->Get(t4->GetComputedStyle()->GetFontDescription(), AtomicString("Cool Font"));
    EXPECT_TRUE(font_face);
    FontSelectionCapabilities capabilities = font_face->GetFontSelectionCapabilities();
    ASSERT_EQ(capabilities.weight, FontSelectionRange({ kBoldWeightValue, kBoldWeightValue }));
    ASSERT_EQ(capabilities.slope, FontSelectionRange({ kNormalSlopeValue, kNormalSlopeValue }));

    auto* font_face_parsed_sheet = MakeGarbageCollected<StyleSheetContents>(MakeGarbageCollected<CSSParserContext>(GetDocument()));
    font_face_parsed_sheet->ParseString("@font-face {"
                                        " font-family: 'Cool Font';"
                                        " src: url(dummy);"
                                        " font-weight: bold;"
                                        " font-style: italic;"
    ...
    GetStyleEngine().InjectSheet(font_face_key, font_face_parsed_sheet, WebCssOrigin::kUser);
    UpdateAllLifecyclePhases();

    // After injecting a more specific font, now there are two and the
    // bold-italic one is selected.
    EXPECT_EQ(2u, GetStyleEngine().GetFontSelector()->GetFontFaceCache()->GetNumSegmentedFacesForTesting());
    font_face = GetStyleEngine().GetFontSelector()->GetFontFaceCache()->Get(t4->GetComputedStyle()->GetFontDescription(), AtomicString("Cool Font"));
    EXPECT_TRUE(font_face);
    capabilities = font_face->GetFontSelectionCapabilities();
    ASSERT_EQ(capabilities.weight, FontSelectionRange({ kBoldWeightValue, kBoldWeightValue }));
    ASSERT_EQ(capabilities.slope, FontSelectionRange({ kItalicSlopeValue, kItalicSlopeValue }));

    auto* style_element = MakeGarbageCollected<HTMLStyleElement>(GetDocument());
    style_element->setInnerHTML("@font-face {"
                                " font-family: 'Cool Font';"
                                " src: url(dummy);"
                                " font-weight: normal;"
                                " font-style: italic;"
    ...
    // better matching one.
    EXPECT_EQ(3u, GetStyleEngine().GetFontSelector()->GetFontFaceCache()->GetNumSegmentedFacesForTesting());
    font_face = GetStyleEngine().GetFontSelector()->GetFontFaceCache()->Get(t4->GetComputedStyle()->GetFontDescription(), AtomicString("Cool Font"));
    EXPECT_TRUE(font_face);
    capabilities = font_face->GetFontSelectionCapabilities();
    ASSERT_EQ(capabilities.weight, FontSelectionRange({ kBoldWeightValue, kBoldWeightValue }));
    ...
    EXPECT_TRUE(font_face);
    capabilities = font_face->GetFontSelectionCapabilities();
    ASSERT_EQ(capabilities.weight, FontSelectionRange({ kNormalWeightValue, kNormalWeightValue }));
    ASSERT_EQ(capabilities.slope, FontSelectionRange({ kItalicSlopeValue, kItalicSlopeValue }));
    ...
    ASSERT_FALSE(GetStyleEngine().GetStyleResolver().FindKeyframesRule(t5, t5, AtomicString("dummy-animation")).rule);

    auto* keyframes_parsed_sheet = MakeGarbageCollected<StyleSheetContents>(MakeGarbageCollected<CSSParserContext>(GetDocument()));
    keyframes_parsed_sheet->ParseString("@keyframes dummy-animation { from {} }");
    StyleSheetKey keyframes_key("keyframes");
    GetStyleEngine().InjectSheet(keyframes_key, keyframes_parsed_sheet, WebCssOrigin::kUser);
    UpdateAllLifecyclePhases();

    // After injecting the style sheet, a @keyframes rule named dummy-animation
    // is found with one keyframe.
    StyleRuleKeyframes* keyframes = GetStyleEngine().GetStyleResolver().FindKeyframesRule(t5, t5, AtomicString("dummy-animation")).rule;
    ...
    // and to).
    keyframes = GetStyleEngine().GetStyleResolver().FindKeyframesRule(t5, t5, AtomicString("dummy-animation")).rule;
    ASSERT_TRUE(keyframes);
    ...
    EXPECT_EQ(Color::FromRGB(0, 0, 0), t6->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    EXPECT_EQ(Color::FromRGB(255, 255, 255), t7->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    auto* custom_properties_parsed_sheet = MakeGarbageCollected<StyleSheetContents>(MakeGarbageCollected<CSSParserContext>(GetDocument()));
    custom_properties_parsed_sheet->ParseString(":root {"
                                                " --stop-color: red !important;"
                                                " --go-color: green;"
                                                "}");
    StyleSheetKey custom_properties_key("custom_properties");
    GetStyleEngine().InjectSheet(custom_properties_key, custom_properties_parsed_sheet, WebCssOrigin::kUser);
    UpdateAllLifecyclePhases();
    ASSERT_TRUE(t6->GetComputedStyle());
    ASSERT_TRUE(t7->GetComputedStyle());
    EXPECT_EQ(Color::FromRGB(255, 0, 0), t6->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    EXPECT_EQ(Color::FromRGB(255, 255, 255), t7->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    GetStyleEngine().RemoveInjectedSheet(custom_properties_key, WebCssOrigin::kUser);
    UpdateAllLifecyclePhases();
    ASSERT_TRUE(t6->GetComputedStyle());
    ASSERT_TRUE(t7->GetComputedStyle());
    EXPECT_EQ(Color::FromRGB(0, 0, 0), t6->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    ...
    EXPECT_EQ(Color::FromRGB(255, 255, 255), t8->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    auto* media_queries_parsed_sheet = MakeGarbageCollected<StyleSheetContents>(MakeGarbageCollected<CSSParserContext>(GetDocument()));
    media_queries_parsed_sheet->ParseString("@media screen {"
                                            " #t8 {"
                                            "  color: red !important;"
                                            " }"
                                            "}"
    ...
    GetStyleEngine().InjectSheet(media_queries_sheet_key, media_queries_parsed_sheet, WebCssOrigin::kUser);
    UpdateAllLifecyclePhases();
    ASSERT_TRUE(t8->GetComputedStyle());
    EXPECT_EQ(Color::FromRGB(255, 0, 0), t8->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    gfx::SizeF page_size(400, 400);
    GetDocument().GetFrame()->StartPrinting(WebPrintParams(page_size));
    ASSERT_TRUE(t8->GetComputedStyle());
    EXPECT_EQ(Color::FromRGB(0, 0, 0), t8->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    GetDocument().GetFrame()->EndPrinting();
    ASSERT_TRUE(t8->GetComputedStyle());
    EXPECT_EQ(Color::FromRGB(255, 0, 0), t8->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    GetStyleEngine().RemoveInjectedSheet(media_queries_sheet_key, WebCssOrigin::kUser);
    UpdateAllLifecyclePhases();
    ASSERT_TRUE(t8->GetComputedStyle());
    EXPECT_EQ(Color::FromRGB(255, 255, 255), t8->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    ...
    EXPECT_EQ(Color::FromRGB(255, 0, 0), t9->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    EXPECT_EQ(Color::FromRGB(0, 0, 0), t10->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    auto* parsed_author_sheet = MakeGarbageCollected<StyleSheetContents>(MakeGarbageCollected<CSSParserContext>(GetDocument()));
    parsed_author_sheet->ParseString("#t9 {"
                                     " color: green;"
                                     "}"
                                     "#t10 {"
    ...
    GetStyleEngine().InjectSheet(author_sheet_key, parsed_author_sheet, WebCssOrigin::kAuthor);
    UpdateAllLifecyclePhases();
    ASSERT_TRUE(t9->GetComputedStyle());
    ASSERT_TRUE(t10->GetComputedStyle());

    // Specificity works within author origin.
    EXPECT_EQ(Color::FromRGB(0, 128, 0), t9->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    ...
    EXPECT_EQ(Color::FromRGB(255, 255, 255), t11->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    auto* parsed_removable_red_sheet = MakeGarbageCollected<StyleSheetContents>(MakeGarbageCollected<CSSParserContext>(GetDocument()));
    parsed_removable_red_sheet->ParseString("#t11 { color: red !important; }");
    StyleSheetKey removable_red_sheet_key("removable_red_sheet");
    GetStyleEngine().InjectSheet(removable_red_sheet_key, parsed_removable_red_sheet, WebCssOrigin::kUser);
    UpdateAllLifecyclePhases();
    ASSERT_TRUE(t11->GetComputedStyle());

    EXPECT_EQ(Color::FromRGB(255, 0, 0), t11->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    auto* parsed_removable_green_sheet = MakeGarbageCollected<StyleSheetContents>(MakeGarbageCollected<CSSParserContext>(GetDocument()));
    parsed_removable_green_sheet->ParseString("#t11 { color: green !important; }");
    StyleSheetKey removable_green_sheet_key("removable_green_sheet");
    GetStyleEngine().InjectSheet(removable_green_sheet_key, parsed_removable_green_sheet, WebCssOrigin::kUser);
    UpdateAllLifecyclePhases();
    ASSERT_TRUE(t11->GetComputedStyle());

    EXPECT_EQ(Color::FromRGB(0, 128, 0), t11->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    auto* parsed_removable_red_sheet2 = MakeGarbageCollected<StyleSheetContents>(MakeGarbageCollected<CSSParserContext>(GetDocument()));
    parsed_removable_red_sheet2->ParseString("#t11 { color: red !important; }");
    GetStyleEngine().InjectSheet(removable_red_sheet_key, parsed_removable_red_sheet2, WebCssOrigin::kUser);
    UpdateAllLifecyclePhases();
    ASSERT_TRUE(t11->GetComputedStyle());

    EXPECT_EQ(Color::FromRGB(255, 0, 0), t11->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    GetStyleEngine().RemoveInjectedSheet(removable_red_sheet_key, WebCssOrigin::kAuthor);
    UpdateAllLifecyclePhases();
    ASSERT_TRUE(t11->GetComputedStyle());
    ...
    GetStyleEngine().RemoveInjectedSheet(removable_red_sheet_key, WebCssOrigin::kUser);
    UpdateAllLifecyclePhases();
    ASSERT_TRUE(t11->GetComputedStyle());
    ...
    GetStyleEngine().RemoveInjectedSheet(removable_green_sheet_key, WebCssOrigin::kUser);
    UpdateAllLifecyclePhases();
    ASSERT_TRUE(t11->GetComputedStyle());
...
TEST_F(StyleEngineTest, InjectedUserNoAuthorFontFace)
{
    ...
    auto* user_sheet = MakeGarbageCollected<StyleSheetContents>(MakeGarbageCollected<CSSParserContext>(GetDocument()));
    user_sheet->ParseString("@font-face {"
                            "  font-family: 'User';"
                            "  src: url(font.ttf);"
                            "}");
...
{
    GetDocument().body()->setInnerHTML("<section><div id='t1'>Red</div></section>"
...
TEST_F(StyleEngineTest, TextToSheetCache)
{
    ...
    CSSStyleSheet* sheet1 = GetStyleEngine().CreateSheet(*element, sheet_text, min_pos, PendingSheetType::kNonBlocking, RenderBlockingBehavior::kNonBlocking);

    // Check that the first sheet is not using a cached StyleSheetContents.
    EXPECT_FALSE(sheet1->Contents()->IsUsedFromTextCache());

    CSSStyleSheet* sheet2 = GetStyleEngine().CreateSheet(*element, sheet_text, min_pos, PendingSheetType::kNonBlocking, RenderBlockingBehavior::kNonBlocking);
    ...
    sheet1 = GetStyleEngine().CreateSheet(*element, sheet_text, min_pos, PendingSheetType::kNonBlocking, RenderBlockingBehavior::kNonBlocking);
...
TEST_F(StyleEngineTest, RuleSetInvalidationCustomPseudo)
{
    ...
    unsigned before_count = GetStyleEngine().StyleForElementCount();
    ApplyRuleSetInvalidation(GetDocument(), "::-webkit-progress-bar { background: green }");
    UpdateAllLifecyclePhases();
    unsigned after_count = GetStyleEngine().StyleForElementCount();
    EXPECT_EQ(1u, after_count - before_count);
}
...
TEST_F(StyleEngineTest, RuleSetInvalidationHost)
{
    ...
    ShadowRoot& shadow_root = host->AttachShadowRootForTesting(ShadowRootMode::kOpen);

    shadow_root.setInnerHTML("<div></div><div></div><div></div>");
    UpdateAllLifecyclePhases();

    unsigned before_count = GetStyleEngine().StyleForElementCount();
    ApplyRuleSetInvalidation(shadow_root, ":host(#nohost), #nohost { background: green}");
    UpdateAllLifecyclePhases();
    unsigned after_count = GetStyleEngine().StyleForElementCount();
...
{
    GetDocument().body()->setInnerHTML("<div class=match><div id=host></div></div>");
    Element* host = GetDocument().getElementById(AtomicString("host"));
    ASSERT_TRUE(host);
    ...
    unsigned before_count = GetStyleEngine().StyleForElementCount();
    ApplyRuleSetInvalidation(shadow_root, ":host-context(.nomatch) .a { background: green}");
    UpdateAllLifecyclePhases();
    unsigned after_count = GetStyleEngine().StyleForElementCount();
    EXPECT_EQ(0u, after_count - before_count);

    before_count = after_count;
    ApplyRuleSetInvalidation(shadow_root, ":host-context(.match) .a { background: green}");
    UpdateAllLifecyclePhases();
    after_count = GetStyleEngine().StyleForElementCount();
    EXPECT_EQ(1u, after_count - before_count);

    before_count = after_count;
    ApplyRuleSetInvalidation(shadow_root, ":host-context(:hover) { background: green}");
    UpdateAllLifecyclePhases();
    after_count = GetStyleEngine().StyleForElementCount();
    EXPECT_EQ(1u, after_count - before_count);

    before_count = after_count;
    ApplyRuleSetInvalidation(shadow_root, ":host-context(#host) { background: green}");
    UpdateAllLifecyclePhases();
    after_count = GetStyleEngine().StyleForElementCount();
    EXPECT_EQ(1u, after_count - before_count);
}
...
{
    GetDocument().body()->setInnerHTML("<style id='s1' media='(max-width: 1px)'>#t1 { color: green }</style>"
    ...
    ASSERT_TRUE(t1);
    ASSERT_TRUE(t1->GetComputedStyle());
    EXPECT_EQ(Color::FromRGB(0, 0, 0), t1->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    unsigned before_count = GetStyleEngine().StyleForElementCount();

    Element* s1 = GetDocument().getElementById(AtomicString("s1"));
    s1->setAttribute(blink::html_names::kMediaAttr, AtomicString("(max-width: 2000px)"));
    UpdateAllLifecyclePhases();
...
{
    GetDocument().body()->setInnerHTML("<style id='s1' media='(max-width: 1000px)'>#t1 { color: green }</style>"
                                       "<div id='t1'>Green</div><div></div>");
    UpdateAllLifecyclePhases();
...
TEST_F(StyleEngineTest, ModifyStyleRuleMatchedPropertiesCache)
{
    ...
    GetDocument().body()->setInnerHTML("<style id='s1'>#t1 { color: blue }</style>"
                                       "<div id='t1'>Green</div>");
    UpdateAllLifecyclePhases();
    ...
    CSSStyleRule* style_rule = To<CSSStyleRule>(sheet->cssRules(exception_state)->item(0));
    ...
    style_rule->style()->setProperty(GetDocument().GetExecutionContext(), "color", "red", "", ASSERT_NO_EXCEPTION);
    UpdateAllLifecyclePhases();

    ASSERT_TRUE(t1->GetComputedStyle());
    EXPECT_EQ(Color::FromRGB(255, 0, 0), t1->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    style_rule->style()->setProperty(GetDocument().GetExecutionContext(), "color", "green", "", ASSERT_NO_EXCEPTION);
    UpdateAllLifecyclePhases();
...
TEST_F(StyleEngineTest, ScheduleInvalidationAfterSubtreeRecalc)
{
    ...
    auto* s2 = To<HTMLStyleElement>(GetDocument().getElementById(AtomicString("s2")));
    ASSERT_TRUE(s2);
    s2->setDisabled(true);
    ...
    // nothing.
    auto* s1 = To<HTMLStyleElement>(GetDocument().getElementById(AtomicString("s1")));
    ASSERT_TRUE(s1);
    s1->setDisabled(true);
    GetStyleEngine().UpdateActiveStyle();
    EXPECT_FALSE(GetDocument().documentElement()->ChildNeedsStyleRecalc());
...
{
    GetDocument().body()->setInnerHTML("<style>div { color:pink }</style><div id=container></div>");
    UpdateAllLifecyclePhases();
    ...
    container->setInnerHTML("<meta http-equiv='default-style' content='preferred'>");
...
TEST_F(StyleEngineTest, StyleSheetsForStyleSheetList_Document)
{
    ...
    const auto& sheet_list = GetStyleEngine().StyleSheetsForStyleSheetList(GetDocument());
    EXPECT_EQ(1u, sheet_list.size());
    EXPECT_TRUE(GetStyleEngine().NeedsActiveStyleUpdate());

    GetDocument().body()->setInnerHTML("<style>span { color: green }</style><style>div { color: pink }</style>");
    EXPECT_TRUE(GetStyleEngine().NeedsActiveStyleUpdate());

    const auto& second_sheet_list = GetStyleEngine().StyleSheetsForStyleSheetList(GetDocument());
...
TEST_F(StyleEngineTest, StyleSheetsForStyleSheetList_ShadowRoot)
{
    ...
    const auto& sheet_list = GetStyleEngine().StyleSheetsForStyleSheetList(shadow_root);
    EXPECT_EQ(1u, sheet_list.size());
    EXPECT_TRUE(GetStyleEngine().NeedsActiveStyleUpdate());

    shadow_root.setInnerHTML("<style>span { color: green }</style><style>div { color: pink }</style>");
    EXPECT_TRUE(GetStyleEngine().NeedsActiveStyleUpdate());

    const auto& second_sheet_list = GetStyleEngine().StyleSheetsForStyleSheetList(shadow_root);
    EXPECT_EQ(2u, second_sheet_list.size());
    EXPECT_TRUE(GetStyleEngine().NeedsActiveStyleUpdate());
}
...
TEST_F(StyleEngineTest, ViewportDescription)
{
    ...
    Document* document = To<LocalFrame>(web_view_impl->GetPage()->MainFrame())->GetDocument();
    ...
    web_view_impl->MainFrameWidget()->SetDeviceScaleFactorForTesting(device_scale);
    web_view_impl->MainFrameWidget()->UpdateAllLifecyclePhases(DocumentUpdateReason::kTest);
...
TEST_F(StyleEngineTest, MediaQueryAffectingValueChanged_StyleElementMediaNoValue)
{
...
TEST_F(StyleEngineTest, MediaQueryAffectingValueChanged_StyleElementMediaEmpty)
{
    GetDocument().body()->setInnerHTML("<style media=''>div{color:pink}</style>");
    UpdateAllLifecyclePhases();
    GetStyleEngine().MediaQueryAffectingValueChanged(MediaValueChange::kOther);
...
TEST_F(StyleEngineTest, MediaQueryAffectingValueChanged_StyleElementMediaNoValid)
{
    GetDocument().body()->setInnerHTML("<style media=',,'>div{color:pink}</style>");
    UpdateAllLifecyclePhases();
    GetStyleEngine().MediaQueryAffectingValueChanged(MediaValueChange::kOther);
...
{
    GetDocument().body()->setInnerHTML("<style media='all'>div{color:pink}</style>");
    UpdateAllLifecyclePhases();
    GetStyleEngine().MediaQueryAffectingValueChanged(MediaValueChange::kOther);
    EXPECT_TRUE(GetStyleEngine().NeedsActiveStyleUpdate());
}

TEST_F(StyleEngineTest, MediaQueryAffectingValueChanged_StyleElementMediaNotAll)
{
    GetDocument().body()->setInnerHTML("<style media='not all'>div{color:pink}</style>");
    UpdateAllLifecyclePhases();
    GetStyleEngine().MediaQueryAffectingValueChanged(MediaValueChange::kOther);
    EXPECT_TRUE(GetStyleEngine().NeedsActiveStyleUpdate());
}
...
{
    GetDocument().body()->setInnerHTML("<style media='print'>div{color:pink}</style>");
    UpdateAllLifecyclePhases();
    GetStyleEngine().MediaQueryAffectingValueChanged(MediaValueChange::kOther);
    EXPECT_TRUE(GetStyleEngine().NeedsActiveStyleUpdate());
}
...
TEST_F(StyleEngineTest, MediaQueriesChangeDefaultFontSize)
{
    ...
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(255, 0, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    GetDocument().GetSettings()->SetDefaultFontSize(40);
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(0, 128, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
}
...
TEST_F(StyleEngineTest, MediaQueriesChangePrefersContrast)
{
    ...
    ColorSchemeHelper color_scheme_helper(GetDocument());
    color_scheme_helper.SetPreferredContrast(mojom::blink::PreferredContrast::kNoPreference);
    ...
    color_scheme_helper.SetPreferredContrast(mojom::blink::PreferredContrast::kMore);
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(0, 0, 255), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    color_scheme_helper.SetPreferredContrast(mojom::blink::PreferredContrast::kLess);
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(0, 0, 255), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    color_scheme_helper.SetPreferredContrast(mojom::blink::PreferredContrast::kCustom);
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(0, 0, 255), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
}
...
TEST_F(StyleEngineTest, MediaQueriesChangeSpecificPrefersContrast)
{
    ...
    color_scheme_helper.SetPreferredContrast(mojom::blink::PreferredContrast::kLess);
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(255, 165, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    color_scheme_helper.SetPreferredContrast(mojom::blink::PreferredContrast::kCustom);
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(255, 255, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
}
...
TEST_F(StyleEngineTest, MediaQueriesChangePrefersContrastOverride)
{
    ...
    GetDocument().GetPage()->SetMediaFeatureOverride(media_feature_names::kPrefersContrastMediaFeature, "more");

    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(0, 0, 255), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    GetDocument().GetPage()->SetMediaFeatureOverride(media_feature_names::kPrefersContrastMediaFeature, "no-preference");

    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(255, 0, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    GetDocument().GetPage()->SetMediaFeatureOverride(media_feature_names::kPrefersContrastMediaFeature, "less");

    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(255, 165, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    GetDocument().GetPage()->SetMediaFeatureOverride(media_feature_names::kPrefersContrastMediaFeature, "custom");
    ...
    GetDocument().GetPage()->SetPreferenceOverride(media_feature_names::kPrefersContrastMediaFeature, "more");

    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(0, 0, 255), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    GetDocument().GetPage()->SetPreferenceOverride(media_feature_names::kPrefersContrastMediaFeature, "no-preference");

    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(255, 0, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    GetDocument().GetPage()->SetPreferenceOverride(media_feature_names::kPrefersContrastMediaFeature, "less");
    ...
    GetDocument().GetPage()->SetPreferenceOverride(media_feature_names::kPrefersContrastMediaFeature, "invalid");
    ...
    GetDocument().GetPage()->SetPreferenceOverride(media_feature_names::kPrefersContrastMediaFeature, "");
...
TEST_F(StyleEngineTest, MediaQueriesForcedColorsOverride)
{
    ...
    ColorSchemeHelper color_scheme_helper(GetDocument());
    GetDocument().GetPage()->SetMediaFeatureOverride(media_feature_names::kForcedColorsMediaFeature, "active");

    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(0, 128, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    GetDocument().GetPage()->SetMediaFeatureOverride(media_feature_names::kForcedColorsMediaFeature, "none");
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(255, 0, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
...
TEST_F(StyleEngineTest, MediaQueriesColorSchemeOverride)
{
    ColorSchemeHelper color_scheme_helper(GetDocument());
    color_scheme_helper.SetPreferredColorScheme(mojom::blink::PreferredColorScheme::kLight);
    EXPECT_EQ(mojom::blink::PreferredColorScheme::kLight, GetDocument().GetSettings()->GetPreferredColorScheme());
    ...
    GetDocument().GetPage()->SetMediaFeatureOverride(media_feature_names::kPrefersColorSchemeMediaFeature, "dark");
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(0, 128, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    ...
    GetDocument().GetPage()->SetPreferenceOverride(media_feature_names::kPrefersColorSchemeMediaFeature, "dark");
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(0, 128, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    ...
    // Invalid value resets override
    GetDocument().GetPage()->SetPreferenceOverride(media_feature_names::kPrefersColorSchemeMediaFeature, "invalid");
    ...
    // Empty value resets override
    GetDocument().GetPage()->SetPreferenceOverride(media_feature_names::kPrefersColorSchemeMediaFeature, "");
...
TEST_F(StyleEngineTest, MediaQueriesReducedTransparencyOverride)
{
    ...
    GetDocument().GetPage()->SetMediaFeatureOverride(media_feature_names::kPrefersReducedTransparencyMediaFeature, "reduce");
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(0, 128, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    ...
    GetDocument().GetPage()->SetPreferenceOverride(media_feature_names::kPrefersReducedTransparencyMediaFeature, "reduce");
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(0, 128, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    ...
    GetDocument().GetPage()->SetPreferenceOverride(media_feature_names::kPrefersReducedTransparencyMediaFeature, "no-preference");
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(0, 128, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    ...
    // Invalid value resets override
    GetDocument().GetPage()->SetPreferenceOverride(media_feature_names::kPrefersReducedTransparencyMediaFeature, "invalid");
    ...
    // Empty value resets override
    GetDocument().GetPage()->SetPreferenceOverride(media_feature_names::kPrefersReducedTransparencyMediaFeature, "");
...
TEST_F(StyleEngineTest, MediaQueriesReducedDataOverride)
{
    ...
    GetDocument().GetPage()->SetMediaFeatureOverride(media_feature_names::kPrefersReducedDataMediaFeature, "reduce");
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(0, 128, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    ...
    GetDocument().GetPage()->SetPreferenceOverride(media_feature_names::kPrefersReducedDataMediaFeature, "reduce");
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(0, 128, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    ...
    GetDocument().GetPage()->SetPreferenceOverride(media_feature_names::kPrefersReducedDataMediaFeature, "no-preference");
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(0, 128, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    ...
    // Invalid value resets override
    GetDocument().GetPage()->SetPreferenceOverride(media_feature_names::kPrefersReducedDataMediaFeature, "invalid");
    ...
    // Empty value resets override
    GetDocument().GetPage()->SetPreferenceOverride(media_feature_names::kPrefersReducedDataMediaFeature, "");
...
TEST_F(StyleEngineTest, MediaQueriesReducedMotionOverride)
{
    ...
    GetDocument().GetPage()->SetMediaFeatureOverride(media_feature_names::kPrefersReducedMotionMediaFeature, "reduce");
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(0, 128, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    ...
    GetDocument().GetPage()->SetPreferenceOverride(media_feature_names::kPrefersReducedMotionMediaFeature, "reduce");
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(0, 128, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    ...
    GetDocument().GetPage()->SetPreferenceOverride(media_feature_names::kPrefersReducedMotionMediaFeature, "no-preference");
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(0, 128, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    ...
    // Invalid value resets override
    GetDocument().GetPage()->SetPreferenceOverride(media_feature_names::kPrefersReducedMotionMediaFeature, "invalid");
    ...
    // Empty value resets override
    GetDocument().GetPage()->SetPreferenceOverride(media_feature_names::kPrefersReducedMotionMediaFeature, "");
...
TEST_F(StyleEngineTest, MediaQueriesChangeNavigationControls)
{
    ...
    GetDocument().GetSettings()->SetNavigationControls(NavigationControls::kBackButton);
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(0, 128, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
}
...
{
    GetDocument().body()->setInnerHTML("<div id=host></div>");
    auto* host = To<HTMLElement>(GetDocument().getElementById(AtomicString("host")));
    ASSERT_TRUE(host);
...
TEST_F(StyleEngineTest, GetComputedStyleOutsideFlatTreeCrash)
{
    ...
    GetDocument().getElementById(AtomicString("host"))->AttachShadowRootForTesting(ShadowRootMode::kOpen);
    UpdateAllLifecyclePhases();
    GetDocument().body()->EnsureComputedStyle();
    GetDocument().getElementById(AtomicString("non-slotted"))->SetInlineStyleProperty(CSSPropertyID::kColor, "blue");
    UpdateAllLifecyclePhases();
}
...
TEST_F(StyleEngineTest, FirstLetterRemoved)
{
    ...
    FirstLetterPseudoElement* fl1 = To<FirstLetterPseudoElement>(d1->GetPseudoElement(kPseudoIdFirstLetter));
    EXPECT_TRUE(fl1);
    ...
    UpdateAllLifecyclePhases();
    EXPECT_FALSE(To<FirstLetterPseudoElement>(d1->GetPseudoElement(kPseudoIdFirstLetter)));

    FirstLetterPseudoElement* fl2 = To<FirstLetterPseudoElement>(d2->GetPseudoElement(kPseudoIdFirstLetter));
    EXPECT_TRUE(fl2);
    ...
    UpdateAllLifecyclePhases();
    EXPECT_FALSE(To<FirstLetterPseudoElement>(d2->GetPseudoElement(kPseudoIdFirstLetter)));

    FirstLetterPseudoElement* fl3 = To<FirstLetterPseudoElement>(d3->GetPseudoElement(kPseudoIdFirstLetter));
    EXPECT_TRUE(fl3);
    ...
    UpdateAllLifecyclePhases();
    fl3 = To<FirstLetterPseudoElement>(d3->GetPseudoElement(kPseudoIdFirstLetter));
    EXPECT_TRUE(fl3);
    EXPECT_EQ(f3->lastChild()->GetLayoutObject(), fl3->RemainingTextLayoutObject());
}
...
TEST_F(StyleEngineTest, InitialDataCreation)
{
    ...
    css_test_helpers::RegisterProperty(GetDocument(), "--x", "<length>", "10px", false);
    ...
    css_test_helpers::RegisterProperty(GetDocument(), "--y", "<color>", "black", false);
...
TEST_F(StyleEngineTest, CSSSelectorEmptyWhitespaceOnlyFail)
{
    ...
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kCSSSelectorEmptyWhitespaceOnlyFail));
    ...
    auto is_counted = [](Element* element) {
        ...
        return element->GetDocument().IsUseCounted(WebFeature::kCSSSelectorEmptyWhitespaceOnlyFail);
    };
...
TEST_F(StyleEngineTest, EnsureCustomComputedStyle)
{
    ...
    for (Node* node = progress; node; node = FlatTreeTraversal::Next(*node, progress)) {
    ...
    for (Node* node = progress; node; node = FlatTreeTraversal::Next(*node, progress)) {
        if (auto* element = DynamicTo<Element>(node)) {
            ASSERT_TRUE(!element->GetComputedStyle() || !element->ComputedStyleRef().IsEnsuredInDisplayNone() || !element->GetLayoutObject());
        }
    }
}
...
TEST_F(StyleEngineTest, ColorSchemeBaseBackgroundChange)
{
    ...
    GetDocument().documentElement()->SetInlineStyleProperty(CSSPropertyID::kColorScheme, "dark");
    UpdateAllLifecyclePhases();

    EXPECT_EQ(Color(0x12, 0x12, 0x12), GetDocument().View()->BaseBackgroundColor());
    ...
    Color system_background_color = LayoutTheme::GetTheme().SystemColor(
        CSSValueID::kCanvas, color_scheme, GetDocument().GetColorProviderForPainting(color_scheme), GetDocument().IsInWebAppScope());

    EXPECT_EQ(system_background_color, GetDocument().View()->BaseBackgroundColor());
}
...
TEST_F(StyleEngineTest, ColorSchemeOverride)
{
    ...
    GetDocument().documentElement()->SetInlineStyleProperty(CSSPropertyID::kColorScheme, "light dark");
    UpdateAllLifecyclePhases();

    EXPECT_EQ(mojom::blink::ColorScheme::kLight, GetDocument().documentElement()->GetComputedStyle()->UsedColorScheme());

    GetDocument().GetPage()->SetMediaFeatureOverride(media_feature_names::kPrefersColorSchemeMediaFeature, "dark");

    UpdateAllLifecyclePhases();
    EXPECT_EQ(mojom::blink::ColorScheme::kDark, GetDocument().documentElement()->GetComputedStyle()->UsedColorScheme());
    ...
    GetDocument().GetPage()->SetPreferenceOverride(media_feature_names::kPrefersColorSchemeMediaFeature, "light");
    UpdateAllLifecyclePhases();
    EXPECT_EQ(mojom::blink::ColorScheme::kDark, GetDocument().documentElement()->GetComputedStyle()->UsedColorScheme());
}
...
TEST_F(StyleEngineTest, PseudoElementBaseComputedStyle)
{
    ...
    const ComputedStyle* base_computed_style = before->GetComputedStyle()->GetBaseComputedStyle();
    ...
    // different here.
    EXPECT_EQ(base_computed_style, before->GetComputedStyle()->GetBaseComputedStyle());
#endif
}
...
TEST_F(StyleEngineTest, NeedsLayoutTreeRebuild)
{
    ...
    GetDocument().documentElement()->SetInlineStyleProperty(CSSPropertyID::kDisplay, "none");
...
{
    GetDocument().body()->SetInlineStyleProperty(CSSPropertyID::kWritingMode, "vertical-lr");
    ...
    GetDocument().documentElement()->SetInlineStyleProperty(CSSPropertyID::kWritingMode, "horizontal-tb");
...
{
    GetDocument().body()->setInnerHTML(R"HTML(<div id="host"><div id="outer"><div id="inner"><div id="innermost"></div></div></div></div>)HTML");
    ...
    EXPECT_EQ(Color::kTransparent, inner_style->VisitedDependentColor(GetCSSPropertyBackgroundColor()));
    ...
    EXPECT_EQ(Color(0, 128, 0), inner_style->VisitedDependentColor(GetCSSPropertyBackgroundColor()));
    ...
    EXPECT_EQ(Color(0, 128, 0), innermost_style->VisitedDependentColor(GetCSSPropertyColor()));
}
...
TEST_F(StyleEngineTest, MoveSlottedOutsideFlatTree)
{
    ...
    ShadowRoot& shadow_root = host1->AttachShadowRootForTesting(ShadowRootMode::kOpen);
...
TEST_F(StyleEngineTest, InitialColorChange)
{
    ...
    const ComputedStyle* document_element_style = GetDocument().documentElement()->GetComputedStyle();
    ASSERT_TRUE(document_element_style);
    EXPECT_EQ(Color::kBlack, document_element_style->VisitedDependentColor(GetCSSPropertyColor()));

    const ComputedStyle* initial_style = initial->GetComputedStyle();
    ASSERT_TRUE(initial_style);
    EXPECT_EQ(Color::kBlack, initial_style->VisitedDependentColor(GetCSSPropertyColor()));
    ...
    ASSERT_TRUE(document_element_style);
    EXPECT_EQ(Color::kWhite, document_element_style->VisitedDependentColor(GetCSSPropertyColor()));

    initial_style = initial->GetComputedStyle();
    ASSERT_TRUE(initial_style);
    EXPECT_EQ(Color::kWhite, initial_style->VisitedDependentColor(GetCSSPropertyColor()));
}

TEST_F(StyleEngineTest, MediaQueryAffectingValueChanged_InvalidateForChangedSizeQueries)
{
    GetDocument().body()->setInnerHTML(R"HTML(
    <style>
    ...
    EXPECT_EQ(Color::kBlack, div->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
...
TEST_F(StyleEngineTest, MediaQueryAffectingValueChanged_InvalidateForChangedTypeQuery)
{
    GetDocument().body()->setInnerHTML(R"HTML(
    <style>
      @media speech {
...
TEST_F(StyleEngineTest, MediaQueryAffectingValueChanged_InvalidateForChangedReducedMotionQuery)
{
    GetDocument().body()->setInnerHTML(R"HTML(
    <style>
      @media (prefers-reduced-motion: reduce) {
...
{
    GetDocument().body()->setInnerHTML("<style>div { display: unset; }</style><div></div>");
    UpdateAllLifecyclePhases();
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kCSSKeywordRevert));

    GetDocument().body()->setInnerHTML("<style>div { display: revert; }</style><div></div>");
    UpdateAllLifecyclePhases();
...
{
    GetDocument().body()->setInnerHTML("<style>div { --x: unset; }</style><div></div>");
    UpdateAllLifecyclePhases();
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kCSSKeywordRevert));

    GetDocument().body()->setInnerHTML("<style>div { --x: revert; }</style><div></div>");
    UpdateAllLifecyclePhases();
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kCSSKeywordRevert));
}
...
TEST_F(StyleEngineTest, NoRevertUseCountForForcedColors)
{
    ...
    // assumption.
    ASSERT_NE(ComputedValue(ref, "color")->CssText(), ComputedValue(elem, "color")->CssText());
    ...
    color_scheme_helper.SetInForcedColors(GetDocument(),
        /*in_forced_colors=*/true);
    UpdateAllLifecyclePhases();
    EXPECT_EQ(ComputedValue(ref, "color")->CssText(), ComputedValue(elem, "color")->CssText());
...
TEST_F(StyleEngineTest, PrintNoDarkColorScheme)
{
    ...
    EXPECT_EQ(Color::kWhite, root->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    EXPECT_EQ(mojom::blink::ColorScheme::kDark, root->GetComputedStyle()->UsedColorScheme());
    EXPECT_EQ(Color::FromRGB(255, 0, 0), body->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    gfx::SizeF page_size(400, 400);
    GetDocument().GetFrame()->StartPrinting(WebPrintParams(page_size));
    EXPECT_EQ(Color::kBlack, root->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    EXPECT_EQ(mojom::blink::ColorScheme::kLight, root->GetComputedStyle()->UsedColorScheme());
    EXPECT_EQ(Color::FromRGB(0, 128, 0), body->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
...
TEST_F(StyleEngineTest, PrintNoForceDarkMode)
{
    ...
    GetDocument().documentElement()->SetInlineStyleProperty(CSSPropertyID::kBackgroundColor, "white");
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(frame_view->DocumentBackgroundColor(), Color::kWhite);
    EXPECT_EQ(GetDocument().documentElement()->GetComputedStyle()->ForceDark(), false);

    GetDocument().GetSettings()->SetForceDarkModeEnabled(true);
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(frame_view->DocumentBackgroundColor(), Color(18, 18, 18));
    EXPECT_EQ(GetDocument().documentElement()->GetComputedStyle()->ForceDark(), true);
...
TEST_F(StyleEngineTest, PrintScriptingEnabled)
{
    ...
    EXPECT_EQ(true, GetDocument().GetExecutionContext()->CanExecuteScripts(ReasonForCallingCanExecuteScripts::kNotAboutToExecuteScript));
...
TEST_F(StyleEngineTest, MediaQueriesChangeScripting)
{
    ...
    EXPECT_EQ(false, GetDocument().GetExecutionContext()->CanExecuteScripts(ReasonForCallingCanExecuteScripts::kNotAboutToExecuteScript));
...
{
    String properties_to_test[] = { "-internal-forced-background-color", "-internal-forced-border-color", "-internal-forced-color",
        "-internal-forced-outline-color", "-internal-forced-visited-color" };
    for (auto property : properties_to_test) {
        String declaration = property + ":red";
        ASSERT_TRUE(css_test_helpers::ParseDeclarationBlock(declaration, kHTMLStandardMode)->IsEmpty());
        ASSERT_TRUE(!css_test_helpers::ParseDeclarationBlock(declaration, kUASheetMode)->IsEmpty());
    }
}
...
TEST_F(StyleEngineTest, HasViewportUnitFlags)
{
    ...
    for (const auto& data : test_data) {
      ...
      <div id=target></div>
        ...
        EXPECT_EQ(data.has_static, target->GetComputedStyle()->HasStaticViewportUnits());
        EXPECT_EQ(data.has_dynamic, target->GetComputedStyle()->HasDynamicViewportUnits());
...
TEST_F(StyleEngineTest, DynamicViewportUnitInvalidation)
{
  ...
  <div id=target_dvh></div>
    ...
    Element* target_svh = GetDocument().getElementById(AtomicString("target_svh"));
    Element* target_dvh = GetDocument().getElementById(AtomicString("target_dvh"));
...
TEST_F(StyleEngineTest, MediaQueriesChangeDisplayState)
{
    ...
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(/*yellow*/ 255, 255, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyBackgroundColor()));

    WTF::Vector<std::pair<ui::mojom::blink::WindowShowState, Color>> test_cases
        = { { ui::mojom::blink::WindowShowState::kMinimized, Color::FromRGB(/*cyan*/ 0, 255, 255) },
              { ui::mojom::blink::WindowShowState::kMaximized, Color::FromRGB(/*red*/ 255, 0, 0) },
              { ui::mojom::blink::WindowShowState::kFullscreen, Color::FromRGB(/*blue*/ 0, 0, 255) } };

    for (const auto& [show_state, color] : test_cases) {
        GetFrame().GetSettings()->SetWindowShowState(show_state);
        UpdateAllLifecyclePhases();
        EXPECT_EQ(color, GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyBackgroundColor()));
    }
}
...
TEST_F(StyleEngineTest, MediaQueriesChangeResizable)
{
    ...
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(/*cyan*/ 0, 255, 255), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyBackgroundColor()));
}
...
TEST_F(StyleEngineTest, DynamicViewportUnitsInMediaQueryMatcher)
{
    ...
    MediaQuerySet* mq_static = MediaQuerySet::Create("(min-width: 50vh)", GetDocument().GetExecutionContext());
    ...
    MediaQuerySet* mq_dynamic = MediaQuerySet::Create("(min-width: 50dvh)", GetDocument().GetExecutionContext());
...
TEST_F(StyleEngineSimTest, OwnerColorScheme)
{
    ...
    auto* frame_element = To<HTMLIFrameElement>(GetDocument().getElementById(AtomicString("frame")));
    auto* frame_document = frame_element->contentDocument();
    ASSERT_TRUE(frame_document);
    EXPECT_EQ(mojom::blink::ColorScheme::kDark, frame_document->GetStyleEngine().GetOwnerColorScheme());

    frame_element->SetInlineStyleProperty(CSSPropertyID::kColorScheme, "light");

    test::RunPendingTasks();
    Compositor().BeginFrame();
    EXPECT_EQ(mojom::blink::ColorScheme::kLight, frame_document->GetStyleEngine().GetOwnerColorScheme());
}
...
TEST_F(StyleEngineSimTest, OwnerColorSchemeBaseBackground)
{
    ...
    SimRequest light_frame_resource("https://example.com/light.html", "text/html");
    ...
    auto* dark_document = To<HTMLIFrameElement>(GetDocument().getElementById(AtomicString("dark-frame")))->contentDocument();
    auto* light_document = To<HTMLIFrameElement>(GetDocument().getElementById(AtomicString("light-frame")))->contentDocument();
    ASSERT_TRUE(dark_document);
    ASSERT_TRUE(light_document);

    EXPECT_TRUE(dark_document->View()->ShouldPaintBaseBackgroundColor());
    EXPECT_EQ(Color(0x12, 0x12, 0x12), dark_document->View()->BaseBackgroundColor());
    EXPECT_FALSE(light_document->View()->ShouldPaintBaseBackgroundColor());

    GetDocument().documentElement()->setAttribute(blink::html_names::kClassAttr, AtomicString("dark"));
...
{
    SimRequest main_resource("https://example.com", "text/html");
    SimSubresourceRequest css_resource("https://example.com/slow.css", "text/css");
...
TEST_F(StyleEngineSimTest, IFramePreferredColorScheme)
{
    ...
    auto* frame_document = frame_element->contentDocument();
    ASSERT_TRUE(frame_document);
    EXPECT_EQ(mojom::blink::PreferredColorScheme::kLight, GetDocument().GetStyleEngine().GetPreferredColorScheme());
    EXPECT_EQ(mojom::blink::PreferredColorScheme::kLight, frame_document->GetStyleEngine().GetPreferredColorScheme());

    color_scheme_helper.SetPreferredColorScheme(mojom::blink::PreferredColorScheme::kDark);
    test::RunPendingTasks();
    Compositor().BeginFrame();

    EXPECT_EQ(mojom::blink::PreferredColorScheme::kDark, GetDocument().GetStyleEngine().GetPreferredColorScheme());
    EXPECT_EQ(mojom::blink::PreferredColorScheme::kDark, frame_document->GetStyleEngine().GetPreferredColorScheme());
}
...
TEST_F(StyleEngineContainerQueryTest, UpdateStyleAndLayoutTreeForContainer)
{
    ...
    GetStyleEngine().UpdateStyleAndLayoutTreeForContainer(*container1, LogicalSize(200, 100), kLogicalAxesBoth);

    // The first span.affected child and #container2
    EXPECT_EQ(2u, GetStyleEngine().StyleForElementCount() - start_count);

    start_count = GetStyleEngine().StyleForElementCount();
    GetStyleEngine().UpdateStyleAndLayoutTreeForContainer(*container2, LogicalSize(200, 100), kLogicalAxesBoth);
...
TEST_F(StyleEngineContainerQueryTest, ContainerQueriesContainmentNotApplying)
{
    ...
    GetStyleEngine().UpdateStyleAndLayoutTreeForContainer(*container, LogicalSize(200, 100), kLogicalAxesBoth);
...
TEST_F(StyleEngineContainerQueryTest, UpdateStyleAndLayoutTreeWithoutLayoutDependency)
{
    ...
    EXPECT_TRUE(GetDocument().View()->NeedsLayout()) << "No layout if style does not depend on layout";
}

TEST_F(StyleEngineContainerQueryTest, UpdateStyleAndLayoutTreeWithLayoutDependency)
{
    GetDocument().documentElement()->setInnerHTML(R"HTML(
    <style>
      #container {
    ...
    GetDocument().UpdateStyleAndLayoutTree();
    EXPECT_FALSE(GetDocument().View()->NeedsLayout()) << "Layout should happen as part of UpdateStyleAndLayoutTree";
...
TEST_F(StyleEngineContainerQueryTest, UpdateStyleAndLayoutTreeWithUpgradeInDisplayNone)
{
    GetDocument().documentElement()->setInnerHTML(R"HTML(
    <style>
      #container {
    ...
    container->classList().Add(AtomicString("toggle"));
    EXPECT_EQ(Document::StyleAndLayoutTreeUpdate::kAnalyzed, GetDocument().CalculateStyleAndLayoutTreeUpdate());
    ...
    // information.
    GetDocument().UpdateStyleAndLayoutTreeForElement(a, DocumentUpdateReason::kTest);
    EXPECT_FALSE(GetStyleEngine().StyleAffectedByLayout());
    EXPECT_FALSE(GetDocument().View()->NeedsLayout());
    EXPECT_FALSE(GetDocument().NeedsLayoutTreeUpdateForNode(*a));
}
...
TEST_F(StyleEngineTest, UpdateStyleAndLayoutTreeForElementWithAnchorQuery)
{
    ...
    GetDocument().UpdateStyleAndLayoutTreeForElement(inner, DocumentUpdateReason::kTest);
    EXPECT_FALSE(GetDocument().View()->NeedsLayout()) << "Layout should happen as part of UpdateStyleAndLayoutTreeForElement";
...
TEST_F(StyleEngineTest, AudioUAStyleNameSpace)
{
    ...
    Element* html_audio = GetDocument().getElementById(AtomicString("html-audio"));
    Element* audio = GetDocument().createElementNS(AtomicString("http://dummyns"), AtomicString("audio"), ASSERT_NO_EXCEPTION);
...
TEST_F(StyleEngineTest, CascadeLayersInOriginsAndTreeScopes)
{
    ...
    const CascadeLayer& user_outer_layer = user_sheet->GetRuleSet().CascadeLayers();
    EXPECT_EQ("", user_outer_layer.GetName());
    EXPECT_EQ(CascadeLayerMap::kImplicitOuterLayerOrder, user_layer_map->GetLayerOrder(user_outer_layer));
    ...
    auto* document_layer_map = GetDocument().GetScopedStyleResolver()->GetCascadeLayerMap();
    ASSERT_TRUE(document_layer_map);

    const CascadeLayer& document_outer_layer
        = To<HTMLStyleElement>(GetDocument().QuerySelector(AtomicString("style")))->sheet()->Contents()->GetRuleSet().CascadeLayers();
    EXPECT_EQ("", document_outer_layer.GetName());
    EXPECT_EQ(CascadeLayerMap::kImplicitOuterLayerOrder, document_layer_map->GetLayerOrder(document_outer_layer));

    const CascadeLayer& document_bar = *document_outer_layer.GetDirectSubLayers()[0];
    EXPECT_EQ("bar", document_bar.GetName());
    EXPECT_EQ(0u, document_layer_map->GetLayerOrder(document_bar));

    const CascadeLayer& document_foo = *document_outer_layer.GetDirectSubLayers()[1];
    ...
    ShadowRoot* shadow = GetDocument().getElementById(AtomicString("host"))->GetShadowRoot();
    auto* shadow_layer_map = shadow->GetScopedStyleResolver()->GetCascadeLayerMap();
    ASSERT_TRUE(shadow_layer_map);

    const CascadeLayer& shadow_outer_layer
        = To<HTMLStyleElement>(shadow->QuerySelector(AtomicString("style")))->sheet()->Contents()->GetRuleSet().CascadeLayers();
    EXPECT_EQ("", shadow_outer_layer.GetName());
    EXPECT_EQ(CascadeLayerMap::kImplicitOuterLayerOrder, shadow_layer_map->GetLayerOrder(shadow_outer_layer));
...
TEST_F(StyleEngineTest, CascadeLayersFromMultipleSheets)
{
    ...
    auto* layer_map = GetDocument().GetScopedStyleResolver()->GetCascadeLayerMap();
    ASSERT_TRUE(layer_map);

    const CascadeLayer& sheet1_outer_layer
        = To<HTMLStyleElement>(GetDocument().getElementById(AtomicString("sheet1")))->sheet()->Contents()->GetRuleSet().CascadeLayers();
    EXPECT_EQ("", sheet1_outer_layer.GetName());
    EXPECT_EQ(CascadeLayerMap::kImplicitOuterLayerOrder, layer_map->GetLayerOrder(sheet1_outer_layer));
    ...
    const CascadeLayer& sheet2_outer_layer
        = To<HTMLStyleElement>(GetDocument().getElementById(AtomicString("sheet2")))->sheet()->Contents()->GetRuleSet().CascadeLayers();
    EXPECT_EQ("", sheet2_outer_layer.GetName());
    EXPECT_EQ(CascadeLayerMap::kImplicitOuterLayerOrder, layer_map->GetLayerOrder(sheet2_outer_layer));
...
TEST_F(StyleEngineTest, NonSlottedStyleDirty)
{
    ...
    EXPECT_EQ(Color::FromRGB(255, 0, 0), old_style->VisitedDependentColor(GetCSSPropertyColor()));
    EXPECT_EQ(Color::FromRGB(0, 128, 0), new_style->VisitedDependentColor(GetCSSPropertyColor()));
}
...
TEST_F(StyleEngineTest, CascadeLayerUseCount)
{
    ...
    {
        ASSERT_FALSE(IsUseCounted(WebFeature::kCSSCascadeLayers));
        GetDocument().body()->setInnerHTML("<style>@import url(foo.css) layer(foo);</style>");
        EXPECT_TRUE(IsUseCounted(WebFeature::kCSSCascadeLayers));
        ClearUseCounter(WebFeature::kCSSCascadeLayers);
    }
}
...
TEST_F(StyleEngineTest, UserKeyframesOverrideWithCascadeLayers)
{
    ...
    GetDocument().body()->setInnerHTML("<div id=target style='height: 100px'></div>");
...
{
    SimRequest main_resource("https://example.com", "text/html");
    SimSubresourceRequest ahem_resource("https://example.com/ahem.woff2", "font/woff2");
    ...
    StyleSheetKey key("user");
    GetDocument().GetStyleEngine().InjectSheet(key, user_sheet, WebCssOrigin::kUser);

    Compositor().BeginFrame();

    ahem_resource.Complete(*test::ReadFromFile(test::CoreTestDataPath("Ahem.woff2")));
...
TEST_F(StyleEngineTest, CascadeLayerActiveStyleSheetVectorNullRuleSetCrash)
{
    ...
    GetDocument().documentElement()->setInnerHTML("<style media=invalid></style>"
                                                  "<style>@layer {}</style>");
...
TEST_F(StyleEngineTest, ScrollbarStyleNoExcessiveCaching)
{
    ...
    PaintLayerScrollableArea* area = container->GetLayoutBox()->GetScrollableArea();
    ...
    UpdateAllLifecyclePhases();
    EXPECT_FALSE(container->GetComputedStyle()->GetPseudoElementStyleCache());
    EXPECT_EQ("rgb(255, 0, 0)", custom_scrollbar->GetPart(kThumbPart)->Style()->BackgroundColor().GetColor().SerializeAsCSSColor());

    scrollbar->SetHoveredPart(kNoPart);
    UpdateAllLifecyclePhases();
    EXPECT_FALSE(container->GetComputedStyle()->GetPseudoElementStyleCache());
    EXPECT_EQ("rgb(0, 128, 0)", custom_scrollbar->GetPart(kThumbPart)->Style()->BackgroundColor().GetColor().SerializeAsCSSColor());
}
...
TEST_F(StyleEngineTest, HasPseudoClassInvalidationSkipIrrelevantClassChange)
{
    ...
    unsigned start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById(AtomicString("div4"))->setAttribute(html_names::kClassAttr, AtomicString("c"));
    UpdateAllLifecyclePhases();
    unsigned element_count = GetStyleEngine().StyleForElementCount() - start_count;
    ASSERT_EQ(0U, element_count);

    start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById(AtomicString("div4"))->setAttribute(html_names::kClassAttr, AtomicString("b"));
    UpdateAllLifecyclePhases();
...
TEST_F(StyleEngineTest, HasPseudoClassInvalidationSkipIrrelevantIdChange)
{
    ...
    unsigned start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById(AtomicString("div4"))->setAttribute(html_names::kIdAttr, AtomicString("c"));
    UpdateAllLifecyclePhases();
    unsigned element_count = GetStyleEngine().StyleForElementCount() - start_count;
    ASSERT_EQ(0U, element_count);

    start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById(AtomicString("c"))->setAttribute(html_names::kIdAttr, AtomicString("b"));
    UpdateAllLifecyclePhases();
    element_count = GetStyleEngine().StyleForElementCount() - start_count;
    ASSERT_EQ(1U, element_count);
}

TEST_F(StyleEngineTest, HasPseudoClassInvalidationSkipIrrelevantAttributeChange)
{
    GetDocument().body()->setInnerHTML(R"HTML(
    <style>.a:has([b]) { background-color: lime; }</style>
    <div id=div1>
    ...
    unsigned start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById(AtomicString("div4"))->setAttribute(QualifiedName(g_empty_atom, AtomicString("c"), g_empty_atom), AtomicString("C"));
    UpdateAllLifecyclePhases();
    unsigned element_count = GetStyleEngine().StyleForElementCount() - start_count;
    ASSERT_EQ(0U, element_count);

    start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById(AtomicString("div4"))->setAttribute(QualifiedName(g_empty_atom, AtomicString("b"), g_empty_atom), AtomicString("B"));
    UpdateAllLifecyclePhases();
    element_count = GetStyleEngine().StyleForElementCount() - start_count;
    ASSERT_EQ(1U, element_count);
}

TEST_F(StyleEngineTest, HasPseudoClassInvalidationSkipIrrelevantInsertionRemoval)
{
    GetDocument().body()->setInnerHTML(R"HTML(
    <style>.a:has(.b) { background-color: lime; }</style>
    <div id=div1>
    ...
    start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById(AtomicString("div3"))->RemoveChild(GetDocument().getElementById(AtomicString("div5")));
    UpdateAllLifecyclePhases();
    element_count = GetStyleEngine().StyleForElementCount() - start_count;
    ASSERT_EQ(0U, element_count);

    start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById(AtomicString("div4"))->RemoveChild(GetDocument().getElementById(AtomicString("div6")));
    UpdateAllLifecyclePhases();
    element_count = GetStyleEngine().StyleForElementCount() - start_count;
    ASSERT_EQ(1U, element_count);
}
...
TEST_F(StyleEngineTest, HasPseudoClassInvalidationUniversalInArgument)
{
    ...
    start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById(AtomicString("div2"))->RemoveChild(GetDocument().getElementById(AtomicString("div3")));
    UpdateAllLifecyclePhases();
    element_count = GetStyleEngine().StyleForElementCount() - start_count;
    ASSERT_EQ(1U, element_count);
}

TEST_F(StyleEngineTest, HasPseudoClassInvalidationInsertionRemovalWithPseudoInHas)
{
    GetDocument().body()->setInnerHTML(R"HTML(
    <style>
    ...
    start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById(AtomicString("div2"))->RemoveChild(GetDocument().getElementById(AtomicString("div4")));
    UpdateAllLifecyclePhases();
    element_count = GetStyleEngine().StyleForElementCount() - start_count;
    ASSERT_EQ(1U, element_count);
...
TEST_F(StyleEngineTest, HasPseudoClassInvalidationLinkInHas)
{
    ...
    start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById(AtomicString("div1"))->RemoveChild(GetDocument().getElementById(AtomicString("anchor1")));
    UpdateAllLifecyclePhases();
    element_count = GetStyleEngine().StyleForElementCount() - start_count;
    ASSERT_EQ(1U, element_count);
}
...
TEST_F(StyleEngineTest, HasPseudoClassInvalidationCheckFiltering)
{
    ...
    //    - .a:has(.b)
    unsigned start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById(AtomicString("child"))->setAttribute(html_names::kClassAttr, AtomicString("d"));
    UpdateAllLifecyclePhases();
    unsigned element_count = GetStyleEngine().StyleForElementCount() - start_count;
    EXPECT_EQ(4U, element_count);

    GetDocument().getElementById(AtomicString("child"))->setAttribute(html_names::kClassAttr, g_empty_atom);
    UpdateAllLifecyclePhases();
    ...
    //    - .e:has(.f.h) .j
    start_count = GetStyleEngine().StyleForElementCount();
    GetDocument().getElementById(AtomicString("child"))->setAttribute(html_names::kClassAttr, AtomicString("b"));
    UpdateAllLifecyclePhases();
    element_count = GetStyleEngine().StyleForElementCount() - start_count;
    EXPECT_EQ(4U, element_count);
}
...
{
    css_test_helpers::RegisterProperty(GetDocument(), "--int16-max", "<integer>", String::Format("%i", std::numeric_limits<int16_t>::max()), false);
    ...
    EXPECT_EQ(std::numeric_limits<int16_t>::max(), child1->GetComputedStyle()->MathDepth());
    EXPECT_EQ(std::numeric_limits<int16_t>::max(), child2->GetComputedStyle()->MathDepth());
    EXPECT_EQ(std::numeric_limits<int16_t>::max(), child3->GetComputedStyle()->MathDepth());
}
...
TEST_F(StyleEngineTest, RemovedBodyToHTMLPropagation)
{
    ...
    Element* root = GetDocument().documentElement();
    ASSERT_TRUE(root);
    EXPECT_TRUE(root->ComputedStyleRef().IsHorizontalWritingMode()) << "body to html propagation does not affect computed value";
    EXPECT_FALSE(root->GetLayoutObject()->StyleRef().IsHorizontalWritingMode()) << "body to html propagation affects used value";
    ...
    UpdateAllLifecyclePhases();
    EXPECT_TRUE(root->ComputedStyleRef().IsHorizontalWritingMode()) << "body to html propagation does not affect computed value";
    EXPECT_TRUE(root->GetLayoutObject()->StyleRef().IsHorizontalWritingMode()) << "No propagation from removed body";
}
...
TEST_F(StyleEngineSimTest, ResizeWithBlockingSheetTransition)
{
    ...
    EXPECT_EQ(trans->ComputedStyleRef().VisitedDependentColor(GetCSSPropertyColor()), Color::FromRGB(0, 128, 0));
}
...
TEST_F(StyleEngineSimTest, FocusWithBlockingSheetTransition)
{
    ...
    GetDocument().GetPage()->GetFocusController().SetFocusedFrame(GetDocument().GetFrame());
...
TEST_F(StyleEngineSimTest, ShouldInvalidateSubjectPseudoHasAfterChildrenParsingFinished)
{
    SimRequest main_resource("https://example.com/", "text/html");
    ...
    EXPECT_TRUE(first);
    EXPECT_EQ(Color::FromRGB(0, 0, 0), first->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    Element* second = GetDocument().getElementById(AtomicString("second"));
    EXPECT_TRUE(second);
    EXPECT_EQ(Color::FromRGB(255, 0, 0), second->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    ...
    EXPECT_TRUE(third);
    EXPECT_EQ(Color::FromRGB(255, 0, 0), third->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
    ...
    second = GetDocument().getElementById(AtomicString("second"));
    EXPECT_TRUE(second);
    EXPECT_EQ(Color::FromRGB(0, 0, 0), second->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    third = GetDocument().getElementById(AtomicString("third"));
    EXPECT_TRUE(third);
    EXPECT_EQ(Color::FromRGB(0, 0, 0), third->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));

    Element* fourth = GetDocument().getElementById(AtomicString("fourth"));
    EXPECT_TRUE(fourth);
    EXPECT_EQ(Color::FromRGB(0, 0, 0), fourth->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
}
...
TEST_F(StyleEngineTest, StyleElementTypeAttrChange)
{
    ...
    UpdateAllLifecyclePhases();
    EXPECT_EQ(Color::FromRGB(0, 0, 0), GetDocument().body()->GetComputedStyle()->VisitedDependentColor(GetCSSPropertyColor()));
...
TEST_F(StyleEngineTest, AnimationShorthandFlags)
{
    String css = "animation: foo 1s";
    {
        ...
        const CSSPropertyValueSet* set = css_test_helpers::ParseDeclarationBlock(css);
        ASSERT_TRUE(set);
...
TEST_F(StyleEngineTest, InitialStyle_Recalc)
{
    ...
    EXPECT_EQ(GetStyleEngine().StyleForElementCount() - before_count, 1u) << "The style recalc should not do a separate @starting-style pass since "
                                                                             "the element already has a style";
    EXPECT_EQ(target->ComputedStyleRef().VisitedDependentColor(GetCSSPropertyBackgroundColor()), green)
        << "Make sure @starting-style rules do not apply for the second pass";
    EXPECT_EQ(target->ComputedStyleRef().VisitedDependentColor(GetCSSPropertyColor()), lime) << "Check that the color changed to lime";
}
...
TEST_F(StyleEngineTest, InitialStyle_FromDisplayNone)
{
    ...
    EXPECT_EQ(GetStyleEngine().StyleForElementCount() - before_count, 2u) << "The style recalc needs to do two passes because the element was "
...
TEST_F(StyleEngineTest, InitialStyleCount_EnsureComputedStyle)
{
    ...
    ASSERT_FALSE(target->GetComputedStyle()) << "Initially no ComputedStyle on display:none element";

    const ComputedStyle* none_style = target->EnsureComputedStyle();
    ASSERT_TRUE(none_style);

    EXPECT_EQ(GetStyleEngine().StyleForElementCount() - before_count, 1u) << "No @starting-style pass for EnsureComputedStyle";
...
TEST_F(StyleEngineTest, EnsureAppRegionTriggersRelayout)
{
    ...
    auto it = std::find_if(regions.begin(), regions.end(), [](blink::DraggableRegionValue s) { return s.draggable; });
    ...
    it = std::find_if(regions.begin(), regions.end(), [](blink::DraggableRegionValue s) { return s.draggable; });
...
TEST_F(StyleEngineTest, ForcedColorsLightDark)
{
    ...
    const ComputedStyle& light_dark = GetDocument().getElementById(AtomicString("light-dark"))->ComputedStyleRef();
    const ComputedStyle& reference = GetDocument().getElementById(AtomicString("reference"))->ComputedStyleRef();

    EXPECT_EQ(light_dark.VisitedDependentColor(GetCSSPropertyColor()), reference.VisitedDependentColor(GetCSSPropertyColor()));
}
