...
CSSNestedDeclarationsRule::CSSNestedDeclarationsRule(StyleRuleNestedDeclarations* nested_declarations_rule, CSSStyleSheet* parent)
    : CSSRule(parent)
    , nested_declarations_rule_(nested_declarations_rule)
{
}
...
CSSStyleDeclaration* CSSNestedDeclarationsRule::style() const
{
    if (!properties_cssom_wrapper_) {
        properties_cssom_wrapper_
            = MakeGarbageCollected<StyleRuleCSSStyleDeclaration>(nested_declarations_rule_->MutableProperties(), const_cast<CSSNestedDeclarationsRule*>(this));
    }
...
void CSSNestedDeclarationsRule::Reattach(StyleRuleBase* rule)
{
    ...
    if (properties_cssom_wrapper_) {
        properties_cssom_wrapper_->Reattach(nested_declarations_rule_->MutableProperties());
    }
    if (style_rule_cssom_wrapper_) {
        style_rule_cssom_wrapper_->Reattach(nested_declarations_rule_->InnerStyleRule());
    }
}
...
CSSRule* CSSNestedDeclarationsRule::InnerCSSStyleRule() const
{
    if (!style_rule_cssom_wrapper_) {
        style_rule_cssom_wrapper_ = nested_declarations_rule_->InnerStyleRule()->CreateCSSOMWrapper(
            /* position_hint */ std::numeric_limits<wtf_size_t>::max(), parentRule());
    }
