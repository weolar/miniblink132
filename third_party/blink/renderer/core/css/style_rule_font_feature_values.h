...
template <> struct DowncastTraits<StyleRuleFontFeature> {
...
public:
    FontFeatureValuesStorage(FontFeatureAliases stylistic, FontFeatureAliases styleset, FontFeatureAliases character_variant, FontFeatureAliases swash,
        FontFeatureAliases ornaments, FontFeatureAliases annotation);
...
private:
    ...
    static Vector<uint32_t> ResolveInternal(const FontFeatureAliases&, const AtomicString&);
...
public:
    StyleRuleFontFeatureValues(Vector<AtomicString> families, FontFeatureAliases stylistic, FontFeatureAliases styleset, FontFeatureAliases character_variant,
    ...
    const Vector<AtomicString>& GetFamilies() const
    {
        return families_;
    }
    ...
    const FontFeatureValuesStorage& Storage()
    {
        return feature_values_storage_;
    }
    ...
    FontFeatureAliases* GetSwash()
    {
        return &feature_values_storage_.swash_;
    }
...
template <> struct DowncastTraits<StyleRuleFontFeatureValues> {
    static bool AllowFrom(const StyleRuleBase& rule)
    {
