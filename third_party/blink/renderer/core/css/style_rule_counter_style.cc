...
StyleRuleCounterStyle::StyleRuleCounterStyle(const AtomicString& name, CSSPropertyValueSet* properties)
    ...
    , additive_symbols_(properties->GetPropertyCSSValue(CSSPropertyID::kAdditiveSymbols))
    , speak_as_(properties->GetPropertyCSSValue(CSSPropertyID::kSpeakAs))
{
    DCHECK(properties);
}

StyleRuleCounterStyle::StyleRuleCounterStyle(const StyleRuleCounterStyle&) = default;

StyleRuleCounterStyle::~StyleRuleCounterStyle() = default;

bool StyleRuleCounterStyle::HasValidSymbols() const
{
    CounterStyleSystem system = CounterStyle::ToCounterStyleSystemEnum(GetSystem());
...
Member<const CSSValue>& StyleRuleCounterStyle::GetDescriptorReference(AtRuleDescriptorID descriptor_id)
{
...
bool StyleRuleCounterStyle::NewValueInvalidOrEqual(AtRuleDescriptorID descriptor_id, const CSSValue* new_value)
{
    Member<const CSSValue>& original_value = GetDescriptorReference(descriptor_id);
    ...
    case AtRuleDescriptorID::System:
        ...
        return CounterStyle::ToCounterStyleSystemEnum(system_) == CounterStyle::ToCounterStyleSystemEnum(new_value);
    case AtRuleDescriptorID::Symbols:
    case AtRuleDescriptorID::AdditiveSymbols: {
        // If the returned value would cause the @counter-style rule to become
        // invalid, do nothing and abort these steps.
        base::AutoReset<Member<const CSSValue>> auto_reset(&original_value, new_value);
...
void StyleRuleCounterStyle::SetDescriptorValue(AtRuleDescriptorID descriptor_id, const CSSValue* new_value)
{
