...
public:
    CSSPaintValueTest()
        ...
        , ScopedOffMainThreadCSSPaintForTest(GetParam() & kOffMainThreadCSSPaint)
    {
    }
...
INSTANTIATE_TEST_SUITE_P(All, CSSPaintValueTest, Values(0, kCSSPaintAPIArguments, kOffMainThreadCSSPaint, kCSSPaintAPIArguments | kOffMainThreadCSSPaint));
...
TEST_P(CSSPaintValueTest, DelayPaintUntilGeneratorReady)
{
    ...
    auto* ident = MakeGarbageCollected<CSSCustomIdentValue>(AtomicString("testpainter"));
    ...
    EXPECT_FALSE(paint_value->GetImage(*target, GetDocument(), style, target_size));
    ...
    if (!RuntimeEnabledFeatures::OffMainThreadCSSPaintEnabled()) {
        EXPECT_CALL(*mock_generator, Paint(_, _, _)).WillRepeatedly(Return(PaintGeneratedImage::Create(PaintRecord(), target_size)));
    }

    EXPECT_TRUE(paint_value->GetImage(*target, GetDocument(), style, target_size));
}
...
TEST_P(CSSPaintValueTest, GetImageCalledOnMultipleDocuments)
{
    ...
    paint_value->GetImage(*target, new_page_holder->GetDocument(), style, target_size);
...
TEST_P(CSSPaintValueTest, PrintingMustFallbackToMainThread)
{
    ...
    EXPECT_CALL(*mock_generator, Paint(_, _, _)).Times(1).WillOnce(Return(PaintGeneratedImage::Create(PaintRecord(), target_size)));

    ASSERT_TRUE(paint_value->GetImage(*target, GetDocument(), style, target_size));
...
TEST_P(CSSPaintValueTest, DoNotPaintForLink)
{
    ...
    auto* ident = MakeGarbageCollected<CSSCustomIdentValue>(AtomicString("linkpainter"));
    CSSPaintValue* paint_value = MakeGarbageCollected<CSSPaintValue>(ident, true);
    EXPECT_FALSE(paint_value->GetImage(*target, GetDocument(), style, gfx::SizeF(100, 100)));
}
