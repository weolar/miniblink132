...
{
    for (const Element* parent = LayoutTreeBuilderTraversal::ParentElement(descendant); parent; parent = LayoutTreeBuilderTraversal::ParentElement(*parent)) {
        if (parent == ancestor) {
            return true;
        }
    }
...
std::optional<std::pair<unsigned, int>> DetermineCounterTypeAndValue(const LayoutObject& layout_object, const CounterDirectives& directives)
{
    ...
    if (directives.IsDefined()) {
        ...
        type_mask |= directives.IsIncrement() ? static_cast<unsigned>(CountersAttachmentContext::Type::kIncrementType) : 0u;
        type_mask |= directives.IsReset() ? static_cast<unsigned>(CountersAttachmentContext::Type::kResetType) : 0;
        type_mask |= directives.IsSet() ? static_cast<unsigned>(CountersAttachmentContext::Type::kSetType) : 0;
...
{
    return counter_type & static_cast<unsigned>(CountersAttachmentContext::Type::kResetType);
}

inline bool IsSetOrReset(unsigned counter_type)
{
    return counter_type & static_cast<unsigned>(CountersAttachmentContext::Type::kResetType)
        || counter_type & static_cast<unsigned>(CountersAttachmentContext::Type::kSetType);
}

int CalculateCounterValue(unsigned counter_type, int counter_value, int counter_current_value)
{
    if (IsSetOrReset(counter_type)) {
        return counter_value;
    }
    return base::CheckAdd(counter_current_value, counter_value).ValueOrDefault(counter_current_value);
}
...
CountersAttachmentContext::CountersAttachmentContext()
    : counter_inheritance_table_(MakeGarbageCollected<CounterInheritanceTable>())
{
}

bool CountersAttachmentContext::ElementGeneratesListItemCounter(const Element& element)
{
    return IsA<HTMLOListElement>(element) || IsA<HTMLUListElement>(element) || IsA<HTMLLIElement>(element) || IsA<HTMLMenuElement>(element)
...
CountersAttachmentContext CountersAttachmentContext::DeepClone() const
{
    ...
    clone.counter_inheritance_table_ = MakeGarbageCollected<CounterInheritanceTable>(*counter_inheritance_table_);
...
void CountersAttachmentContext::EnterObject(const LayoutObject& layout_object, bool is_page_box)
{
    ...
    if (counter_directives) {
        for (auto& [counter_name, directives] : *counter_directives) {
            std::optional<std::pair<unsigned, int>> type_and_value = DetermineCounterTypeAndValue(layout_object, directives);
            if (!type_and_value.has_value()) {
                continue;
            }
            auto [counter_type, value_argument] = type_and_value.value();
            ProcessCounter(layout_object, counter_name, counter_type, value_argument, is_page_box);
        }
    }
    ...
    if (const auto* element = DynamicTo<Element>(layout_object.GetNode())) {
        if (ElementGeneratesListItemCounter(*element) && (!counter_directives || counter_directives->find(list_item_) == counter_directives->end())) {
    ...
    if (is_page_box) {
        ...
        if (!counter_directives || counter_directives->find(page_str) == counter_directives->end()) {
            ProcessCounter(layout_object, page_str, static_cast<unsigned>(Type::kIncrementType),
...
void CountersAttachmentContext::LeaveObject(const LayoutObject& layout_object, bool is_page_box)
{
    if (!attachment_root_is_document_element_) {
        return;
    }
...
void CountersAttachmentContext::MaybeCreateListItemCounter(const Element& element)
{
    ...
    if (ListItemOrdinal* ordinal = ListItemOrdinal::Get(element)) {
        ...
        unsigned type_mask = static_cast<unsigned>(CountersAttachmentContext::Type::kIncrementType);
        UpdateCounterValue(*layout_object, list_item_, type_mask, value);
        return;
    }
    if (auto* olist = DynamicTo<HTMLOListElement>(element)) {
        int value = base::ClampAdd(olist->StartConsideringItemCount(), olist->IsReversed() ? 1 : -1);
        CreateCounter(*layout_object, list_item_, value);
        return;
    }
    if (IsA<HTMLUListElement>(element) || IsA<HTMLMenuElement>(element) || IsA<HTMLDirectoryElement>(element)) {
...
// functions.
Vector<int> CountersAttachmentContext::GetCounterValues(const LayoutObject& layout_object, const AtomicString& counter_name, bool only_last)
{
...
void CountersAttachmentContext::ProcessCounter(
    const LayoutObject& layout_object, const AtomicString& counter_name, unsigned counter_type, int value_argument, bool is_page_box)
{
    ...
    if (!layout_object.GetNode() && ObscurePageCounterIfNeeded(layout_object, counter_name, counter_type, value_argument, is_page_box)) {
        return;
    }
    ...
    const LayoutObject& layout_object, const AtomicString& counter_name, unsigned counter_type, int value_argument, bool is_page_box)
{
    ...
    if (counter_stack_it == counter_inheritance_table_->end() || (is_page_box && !IsReset(counter_type))) {
        return false;
    }
    CounterStack& counter_stack = *counter_stack_it->value;
    if (!counter_stack.empty() && counter_stack.back()) {
        int counter_value = CalculateCounterValue(counter_type, value_argument, counter_stack.back()->value);
        auto* new_entry = MakeGarbageCollected<CounterEntry>(layout_object, counter_value);
...
void CountersAttachmentContext::UnobscurePageCounterIfNeeded(const AtomicString& counter_name, unsigned counter_type, bool is_page_box)
{
    auto counter_stack_it = counter_inheritance_table_->find(counter_name);
    if (counter_stack_it == counter_inheritance_table_->end() || (is_page_box && !IsReset(counter_type))) {
        return;
    ...
    while (!counter_stack.empty()) {
        ...
        DCHECK(!counter_stack.back() || !counter_stack.back()->layout_object->GetNode());
...
void CountersAttachmentContext::CreateCounter(const LayoutObject& layout_object, const AtomicString& counter_name, int value)
{
    auto* new_entry = MakeGarbageCollected<CounterEntry>(layout_object, value);
    auto counter_stack_it = counter_inheritance_table_->find(counter_name);
    if (counter_stack_it == counter_inheritance_table_->end()) {
        CounterStack* counter_stack = MakeGarbageCollected<CounterStack>(1u, new_entry);
    ...
    if (const auto* element = DynamicTo<Element>(layout_object.GetNode())) {
        ...
        // element.
        if (!counter_stack.empty() && counter_stack.back()) {
            const auto* current = To<Element>(counter_stack.back()->layout_object->GetNode());
            DCHECK(current);
            if (LayoutTreeBuilderTraversal::ParentElement(*current) == LayoutTreeBuilderTraversal::ParentElement(*element)) {
                counter_stack.pop_back();
            }
        }
    }
...
void CountersAttachmentContext::RemoveStaleCounters(const LayoutObject& layout_object, const AtomicString& counter_name)
{
    ...
    while (!counter_stack.empty()) {
        ...
        if (const auto* last_element = DynamicTo<Element>(last_object.GetNode())) {
            const Element* parent = LayoutTreeBuilderTraversal::ParentElement(*last_element);
...
void CountersAttachmentContext::RemoveCounterIfAncestorExists(const LayoutObject& layout_object, const AtomicString& counter_name)
{
    auto counter_stack_it = counter_inheritance_table_->find(counter_name);
    if (counter_stack_it == counter_inheritance_table_->end()) {
        return;
...
void CountersAttachmentContext::UpdateCounterValue(
    const LayoutObject& layout_object, const AtomicString& counter_name, unsigned counter_type, int counter_value)
{
    int default_counter_value = CalculateCounterValue(counter_type, counter_value, 0);
    auto counter_stack_it = counter_inheritance_table_->find(counter_name);
    ...
    current.value = CalculateCounterValue(counter_type, counter_value, current.value);
}
