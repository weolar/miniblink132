...
template <class Key, class Value> struct RobinHoodMap {
    static_assert(std::is_same_v<Key, AtomicString>, "We currently only support AtomicString as key.");
...
public:
    ...
    class iterator {
    public:
        iterator(Bucket* pos, const Bucket* end)
            : pos_(pos)
            , end_(end)
        {
