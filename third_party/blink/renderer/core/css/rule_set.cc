...
template <class T> static void AddRuleToIntervals(const T* value, unsigned position, HeapVector<RuleSet::Interval<T>>& intervals);

static void UnmarkAsCoveredByBucketing(CSSSelector& selector);

static inline ValidPropertyFilter DetermineValidPropertyFilter(const AddRuleFlags add_rule_flags, const CSSSelector& selector)
{
    for (const CSSSelector* component = &selector; component; component = component->NextSimpleSelector()) {
        ...
        case CSSSelector::kPseudoHighlight:
            if (UsesHighlightPseudoInheritance(component->GetPseudoId(component->GetPseudoType()))) {
...
{
    for (const CSSSelector* complex = selector_list; complex; complex = CSSSelectorList::Next(*complex)) {
...
{
    return style_scope && (SelectorListHasLinkOrVisited(style_scope->From()) || SelectorListHasLinkOrVisited(style_scope->To()));
}

static unsigned DetermineLinkMatchType(const AddRuleFlags add_rule_flags, const CSSSelector& selector, const StyleScope* style_scope)
{
    if (selector.HasLinkOrVisited() || StyleScopeHasLinkOrVisited(style_scope)) {
        return (add_rule_flags & kRuleIsVisitedDependent) ? CSSSelector::kMatchVisited : CSSSelector::kMatchLink;
    }
    return CSSSelector::kMatchAll;
}

RuleData::RuleData(StyleRule* rule, unsigned selector_index, unsigned position, const StyleScope* style_scope, AddRuleFlags add_rule_flags,
    Vector<unsigned>& bloom_hash_backing)
    : rule_(rule)
    , selector_index_(selector_index)
    , position_(position)
    , specificity_(Selector().Specificity())
    , link_match_type_(DetermineLinkMatchType(add_rule_flags, Selector(), style_scope))
    , valid_property_filter_(static_cast<std::underlying_type_t<ValidPropertyFilter>>(DetermineValidPropertyFilter(add_rule_flags, Selector())))
    , is_entirely_covered_by_bucketing_(false)
    , // Will be computed in ComputeEntirelyCoveredByBucketing().
    is_easy_(false)
    , // Ditto.
...
void RuleData::ComputeEntirelyCoveredByBucketing()
{
    ...
    for (const CSSSelector* selector = &Selector(); selector; selector = selector->NextSimpleSelector()) {
...
{
    for (CSSSelector* selector = &MutableSelector(); selector; selector = selector->NextSimpleSelector()) {
...
void RuleData::ComputeBloomFilterHashes(const StyleScope* style_scope, Vector<unsigned>& bloom_hash_backing)
{
    ...
    SelectorFilter::CollectIdentifierHashes(Selector(), style_scope, bloom_hash_backing);

    // The clamp here is purely for safety; a real rule would never have
    // as many as 255 descendant selectors.
    bloom_hash_size_ = std::min<uint32_t>(bloom_hash_backing.size() - bloom_hash_pos_, 255);
    ...
    if (bloom_hash_size_ > 0 && bloom_hash_pos_ >= bloom_hash_size_
        && std::equal(bloom_hash_backing.begin() + bloom_hash_pos_ - bloom_hash_size_, bloom_hash_backing.begin() + bloom_hash_pos_,
...
void RuleData::MovedToDifferentRuleSet(const Vector<unsigned>& old_backing, Vector<unsigned>& new_backing, unsigned new_position)
{
    unsigned new_pos = new_backing.size();
    new_backing.insert(new_backing.size(), old_backing.data() + bloom_hash_pos_, bloom_hash_size_);
    bloom_hash_pos_ = new_pos;
    position_ = new_position;
}

void RuleSet::AddToRuleSet(const AtomicString& key, RuleMap& map, const RuleData& rule_data)
{
    if (map.IsCompacted()) {
...
void RuleSet::AddToRuleSet(HeapVector<RuleData>& rules, const RuleData& rule_data)
{
...
static void ExtractSelectorValues(const CSSSelector* selector, AtomicString& id, AtomicString& class_name, AtomicString& attr_name, AtomicString& attr_value,
    bool& is_exact_attr, AtomicString& custom_pseudo_element_name, AtomicString& tag_name, AtomicString& part_name, AtomicString& picker_name,
    CSSSelector::PseudoType& pseudo_type)
{
    ...
    case CSSSelector::kPagePseudoClass:
        ...
        case CSSSelector::kPseudoFocus:
            if (pseudo_type == CSSSelector::kPseudoScrollMarker || pseudo_type == CSSSelector::kPseudoScrollNextButton
        ...
        case CSSSelector::kPseudoWhere: {
            ...
            if (selector_list->HasOneSelector()) {
                ExtractSelectorValues(selector_list->First(), id, class_name, attr_name, attr_value, is_exact_attr, custom_pseudo_element_name, tag_name,
...
static const CSSSelector* ExtractBestSelectorValues(const CSSSelector& component, AtomicString& id, AtomicString& class_name, AtomicString& attr_name,
    AtomicString& attr_value, bool& is_exact_attr, AtomicString& custom_pseudo_element_name, AtomicString& tag_name, AtomicString& part_name,
    AtomicString& picker_name, CSSSelector::PseudoType& pseudo_type)
{
    const CSSSelector* it = &component;
    for (; it && it->Relation() == CSSSelector::kSubSelector; it = it->NextSimpleSelector()) {
        ExtractSelectorValues(
            it, id, class_name, attr_name, attr_value, is_exact_attr, custom_pseudo_element_name, tag_name, part_name, picker_name, pseudo_type);
    }
    if (it) {
        ExtractSelectorValues(
            it, id, class_name, attr_name, attr_value, is_exact_attr, custom_pseudo_element_name, tag_name, part_name, picker_name, pseudo_type);
...
template <class Func> static void MarkAsCoveredByBucketing(CSSSelector& selector, Func&& should_mark_func)
{
    for (CSSSelector* s = &selector;; ++s) { // Termination condition within loop.
        ...
        if (s->IsLastInComplexSelector() || s->Relation() != CSSSelector::kSubSelector) {
            break;
        }
    }
}
...
template <RuleSet::BucketCoverage bucket_coverage> void RuleSet::FindBestRuleSetAndAdd(CSSSelector& component, const RuleData& rule_data)
{
    AtomicString id;
    ...
    const CSSSelector* it = ExtractBestSelectorValues(
        component, id, class_name, attr_name, attr_value, is_exact_attr, custom_pseudo_element_name, tag_name, part_name, picker_name, pseudo_type);

    // Prefer rule sets in order of most likely to apply infrequently.
    if (!id.empty()) {
        if (bucket_coverage == BucketCoverage::kCompute) {
            MarkAsCoveredByBucketing(component, [&id](const CSSSelector& selector) { return selector.Match() == CSSSelector::kId && selector.Value() == id; });
        }
    ...
    if (!class_name.empty()) {
        if (bucket_coverage == BucketCoverage::kCompute) {
            MarkAsCoveredByBucketing(
                component, [&class_name](const CSSSelector& selector) { return selector.Match() == CSSSelector::kClass && selector.Value() == class_name; });
        }
    ...
    auto get_ua_shadow_pseudo = [&]() -> const AtomicString& {
        if (picker_name == "select") {
            ...
            return shadow_element_utils::StringForUAShadowPseudoId(CSSSelector::GetPseudoId(pseudo_type));
        }
        return g_null_atom;
    };
    ...
    if (RuntimeEnabledFeatures::CSSCascadeCorrectScopeEnabled()) {
        ...
        if (part_name.empty() && ua_shadow_pseudo == g_null_atom && it
            && (it->Relation() == CSSSelector::RelationType::kUAShadow || it->Relation() == CSSSelector::RelationType::kShadowPart)) {
            const CSSSelector* previous = it->NextSimpleSelector();
            if (previous->Match() == CSSSelector::kPseudoElement) {
                ExtractSelectorValues(
                    previous, id, class_name, attr_name, attr_value, is_exact_attr, custom_pseudo_element_name, tag_name, part_name, picker_name, pseudo_type);
    ...
    if (!part_name.empty() || (it && it->FollowsPart() && !RuntimeEnabledFeatures::CSSCascadeCorrectScopeEnabled())) {
    ...
    if (!custom_pseudo_element_name.empty()) {
        ...
        AddToRuleSet(custom_pseudo_element_name, ua_shadow_pseudo_element_rules_, rule_data);
    ...
    if (ua_shadow_pseudo != g_null_atom) {
        ...
        if (it->FollowsSlotted()) {
            ...
            AddToRuleSet(ua_shadow_pseudo, ua_shadow_pseudo_element_rules_, rule_data);
        }
        return;
    }
        ...
        if (bucket_coverage == BucketCoverage::kCompute) {
            MarkAsCoveredByBucketing(component, [](const CSSSelector& selector) {
                ...
                return selector.Match() == CSSSelector::kPseudoClass
                    && (selector.GetPseudoType() == CSSSelector::kPseudoAnyLink || selector.GetPseudoType() == CSSSelector::kPseudoWebkitAnyLink);
            });
        }
    ...
    case CSSSelector::kPseudoFocus:
        if (bucket_coverage == BucketCoverage::kCompute) {
            MarkAsCoveredByBucketing(component, [](const CSSSelector& selector) {
                return selector.Match() == CSSSelector::kPseudoClass && selector.GetPseudoType() == CSSSelector::kPseudoFocus;
            });
        }
    ...
    case CSSSelector::kPseudoFocusVisible:
        if (bucket_coverage == BucketCoverage::kCompute) {
            MarkAsCoveredByBucketing(component, [](const CSSSelector& selector) {
                return selector.Match() == CSSSelector::kPseudoClass && selector.GetPseudoType() == CSSSelector::kPseudoFocusVisible;
            });
        }
    ...
    case CSSSelector::kPseudoRoot:
        if (bucket_coverage == BucketCoverage::kCompute) {
            MarkAsCoveredByBucketing(component, [](const CSSSelector& selector) {
                return selector.Match() == CSSSelector::kPseudoClass && selector.GetPseudoType() == CSSSelector::kPseudoRoot;
            });
        }
    ...
    if (!tag_name.empty()) {
        ...
        if (bucket_coverage == BucketCoverage::kCompute) {
            MarkAsCoveredByBucketing(component, [&tag_name](const CSSSelector& selector) {
                return selector.Match() == CSSSelector::kTag && selector.TagQName().LocalName() == tag_name
    ...
    // rules.
    MarkAsCoveredByBucketing(component, [](const CSSSelector& selector) { return selector.Match() == CSSSelector::kTag && selector.TagQName() == AnyQName(); });
    AddToRuleSet(universal_rules_, rule_data);
}

void RuleSet::AddRule(StyleRule* rule, unsigned selector_index, AddRuleFlags add_rule_flags, const ContainerQuery* container_query,
    const CascadeLayer* cascade_layer, const StyleScope* style_scope)
{
    ...
    RuleData rule_data(rule, selector_index, rule_count_, style_scope, add_rule_flags, bloom_hash_backing_);
    ++rule_count_;
    {
        InvalidationSetToSelectorMap::SelectorScope selector_scope(rule, selector_index);
        if (features_.CollectFeaturesFromSelector(rule_data.Selector(), style_scope) == SelectorPreMatch::kNeverMatches) {
            return;
        }
    }

    FindBestRuleSetAndAdd<BucketCoverage::kCompute>(rule_data.MutableSelector(), rule_data);
    ...
    if (rule_data.LinkMatchType() == CSSSelector::kMatchLink) {
        ...
        RuleData visited_dependent(
            rule, rule_data.SelectorIndex(), rule_data.GetPosition(), style_scope, add_rule_flags | kRuleIsVisitedDependent, bloom_hash_backing_);
        ...
        FindBestRuleSetAndAdd<BucketCoverage::kIgnore>(visited_dependent.MutableSelector(), visited_dependent);
    }

    AddRuleToLayerIntervals(cascade_layer, rule_data.GetPosition());
    AddRuleToIntervals(container_query, rule_data.GetPosition(), container_query_intervals_);
    AddRuleToIntervals(style_scope, rule_data.GetPosition(), scope_intervals_);
}

void RuleSet::AddRuleToLayerIntervals(const CascadeLayer* cascade_layer, unsigned position)
{
...
template <class T> static void AddRuleToIntervals(const T* value, unsigned position, HeapVector<RuleSet::Interval<T>>& intervals)
{
    const T* last_value = intervals.empty() ? nullptr : intervals.back().value.Get();
    if (value == last_value) {
        return;
    }
...
void RuleSet::AddChildRules(StyleRule* parent_rule, const HeapVector<Member<StyleRuleBase>>& rules, const MediaQueryEvaluator& medium,
    AddRuleFlags add_rule_flags, const ContainerQuery* container_query, CascadeLayer* cascade_layer, const StyleScope* style_scope, bool within_mixin)
{
    for (StyleRuleBase* rule : rules) {
        if (auto* style_rule = DynamicTo<StyleRule>(rule)) {
            AddStyleRule(style_rule, parent_rule, medium, add_rule_flags, within_mixin, container_query, cascade_layer, style_scope);
            ...
            if (MatchMediaForAddRules(medium, media_rule->MediaQueries())) {
                AddChildRules(parent_rule, media_rule->ChildRules(), medium, add_rule_flags, container_query, cascade_layer, style_scope, within_mixin);
            }
        ...
        } else if (auto* font_palette_values_rule = DynamicTo<StyleRuleFontPaletteValues>(rule)) {
            // TODO(https://crbug.com/1170794): Handle cascade layers for
            // @font-palette-values.
            AddFontPaletteValuesRule(font_palette_values_rule);
        } else if (auto* font_feature_values_rule = DynamicTo<StyleRuleFontFeatureValues>(rule)) {
        ...
        } else if (auto* counter_style_rule = DynamicTo<StyleRuleCounterStyle>(rule)) {
            counter_style_rule->SetCascadeLayer(cascade_layer);
            AddCounterStyleRule(counter_style_rule);
        } else if (auto* view_transition_rule = DynamicTo<StyleRuleViewTransition>(rule)) {
            view_transition_rule->SetCascadeLayer(cascade_layer);
            AddViewTransitionRule(view_transition_rule);
        } else if (auto* position_try_rule = DynamicTo<StyleRulePositionTry>(rule)) {
            ...
            if (supports_rule->ConditionIsSupported()) {
                AddChildRules(parent_rule, supports_rule->ChildRules(), medium, add_rule_flags, container_query, cascade_layer, style_scope, within_mixin);
            }
        } else if (auto* container_rule = DynamicTo<StyleRuleContainer>(rule)) {
            const ContainerQuery* inner_container_query = &container_rule->GetContainerQuery();
            if (container_query) {
                inner_container_query = inner_container_query->CopyWithParent(container_query);
            }
            AddChildRules(parent_rule, container_rule->ChildRules(), medium, add_rule_flags, inner_container_query, cascade_layer, style_scope, within_mixin);
        } else if (auto* layer_block_rule = DynamicTo<StyleRuleLayerBlock>(rule)) {
            CascadeLayer* sub_layer = GetOrAddSubLayer(cascade_layer, layer_block_rule->GetName());
            AddChildRules(parent_rule, layer_block_rule->ChildRules(), medium, add_rule_flags, container_query, sub_layer, style_scope, within_mixin);
        } else if (auto* layer_statement_rule = DynamicTo<StyleRuleLayerStatement>(rule)) {
            ...
            AddChildRules(parent_rule, scope_rule->ChildRules(), medium, add_rule_flags, container_query, cascade_layer, inner_style_scope, within_mixin);
        } else if (auto* starting_style_rule = DynamicTo<StyleRuleStartingStyle>(rule)) {
            AddChildRules(parent_rule, starting_style_rule->ChildRules(), medium, add_rule_flags | kRuleIsStartingStyle, container_query, cascade_layer,
            ...
            if (it != mixins_.end() && it->value->FakeParentRule().ChildRules()) {
                AddChildRules(parent_rule, *it->value->FakeParentRule().ChildRules(), medium, add_rule_flags, container_query, cascade_layer, style_scope,
                    /*within_mixin=*/true);
            }
        } else if (auto* nested_declarations = DynamicTo<StyleRuleNestedDeclarations>(rule)) {
            AddStyleRule(nested_declarations->InnerStyleRule(), parent_rule, medium, add_rule_flags, within_mixin, container_query, cascade_layer, style_scope);
        }
    }
}

bool RuleSet::MatchMediaForAddRules(const MediaQueryEvaluator& evaluator, const MediaQuerySet* media_queries)
{
    if (!media_queries) {
        return true;
    }
    bool match_media = evaluator.Eval(*media_queries, &features_.MutableMediaQueryResultFlags());
    media_query_set_results_.push_back(MediaQuerySetResult(*media_queries, match_media));
    return match_media;
}

void RuleSet::AddRulesFromSheet(StyleSheetContents* sheet, const MediaQueryEvaluator& medium, CascadeLayer* cascade_layer)
{
    ...
    AddChildRules(/*parent_rule=*/nullptr, sheet->ChildRules(), medium, kRuleHasNoSpecialState, nullptr /* container_query */, cascade_layer, nullptr,
        /*within_mixin=*/false);
}
...
{
    do {
        if (selector->Match() == CSSSelector::kPseudoClass && selector->GetPseudoType() == CSSSelector::kPseudoParent) {
            return selector->ParentRule();
        }
        if (selector->SelectorList() && selector->SelectorList()->First()) {
            const StyleRule* parent = FindParentIfUsed(selector->SelectorList()->First());
            if (parent != nullptr) {
                return parent;
            }
        }
...
// this rule.
static bool IncludeRule(const StyleRule* style_rule, const HeapHashSet<Member<StyleRule>>& only_include)
{
...
void RuleSet::NewlyAddedFromDifferentRuleSet(const RuleData& old_rule_data, const StyleScope* style_scope, const RuleSet& old_rule_set, RuleData& new_rule_data)
{
    new_rule_data.MovedToDifferentRuleSet(old_rule_set.bloom_hash_backing_, bloom_hash_backing_, rule_count_);
...
void RuleSet::AddFilteredRulesFromOtherBucket(
    const RuleSet& other, const HeapVector<RuleData>& src, const HeapHashSet<Member<StyleRule>>& only_include, HeapVector<RuleData>* dst)
{
    ...
    for (const RuleData& rule_data : src) {
        if (IncludeRule(rule_data.Rule(), only_include)) {
            ...
            NewlyAddedFromDifferentRuleSet(rule_data, scope_seeker.Seek(rule_data.GetPosition()), other, dst->back());
        }
    }
}

void RuleSet::AddFilteredRulesFromOtherSet(const RuleSet& other, const HeapHashSet<Member<StyleRule>>& only_include)
{
    if (other.rule_count_ > 0) {
        id_rules_.AddFilteredRulesFromOtherSet(other.id_rules_, only_include, other, *this);
        class_rules_.AddFilteredRulesFromOtherSet(other.class_rules_, only_include, other, *this);
        attr_rules_.AddFilteredRulesFromOtherSet(other.attr_rules_, only_include, other, *this);
        // NOTE: attr_substring_matchers_ will be rebuilt in CompactRules().
        tag_rules_.AddFilteredRulesFromOtherSet(other.tag_rules_, only_include, other, *this);
        ua_shadow_pseudo_element_rules_.AddFilteredRulesFromOtherSet(other.ua_shadow_pseudo_element_rules_, only_include, other, *this);
        AddFilteredRulesFromOtherBucket(other, other.link_pseudo_class_rules_, only_include, &link_pseudo_class_rules_);
        AddFilteredRulesFromOtherBucket(other, other.cue_pseudo_rules_, only_include, &cue_pseudo_rules_);
        AddFilteredRulesFromOtherBucket(other, other.focus_pseudo_class_rules_, only_include, &focus_pseudo_class_rules_);
        AddFilteredRulesFromOtherBucket(other, other.focus_visible_pseudo_class_rules_, only_include, &focus_visible_pseudo_class_rules_);
        AddFilteredRulesFromOtherBucket(other, other.universal_rules_, only_include, &universal_rules_);
        AddFilteredRulesFromOtherBucket(other, other.shadow_host_rules_, only_include, &shadow_host_rules_);
        AddFilteredRulesFromOtherBucket(other, other.part_pseudo_rules_, only_include, &part_pseudo_rules_);
        AddFilteredRulesFromOtherBucket(other, other.slotted_pseudo_element_rules_, only_include, &slotted_pseudo_element_rules_);
        AddFilteredRulesFromOtherBucket(other, other.selector_fragment_anchor_rules_, only_include, &selector_fragment_anchor_rules_);
        AddFilteredRulesFromOtherBucket(other, other.root_element_rules_, only_include, &root_element_rules_);
...
void RuleSet::AddStyleRule(StyleRule* style_rule, StyleRule* parent_rule, const MediaQueryEvaluator& medium, AddRuleFlags add_rule_flags, bool within_mixin,
    const ContainerQuery* container_query, CascadeLayer* cascade_layer, const StyleScope* style_scope)
{
    if (within_mixin) {
    ...
    for (const CSSSelector* selector = style_rule->FirstSelector(); selector; selector = CSSSelectorList::Next(*selector)) {
        ...
        AddRule(style_rule, selector_index, add_rule_flags, container_query, cascade_layer, style_scope);
    }

    // Nested rules are taken to be added immediately after their parent rule.
    if (style_rule->ChildRules() != nullptr) {
        AddChildRules(style_rule, *style_rule->ChildRules(), medium, add_rule_flags, container_query, cascade_layer, style_scope, within_mixin);
    }
}

CascadeLayer* RuleSet::GetOrAddSubLayer(CascadeLayer* cascade_layer, const StyleRuleBase::LayerName& name)
{
    if (!cascade_layer) {
        cascade_layer = EnsureImplicitOuterLayer();
    }
...
void RuleMap::Compact()
{
    ...
    auto counts = base::HeapArray<unsigned>::WithSize(num_buckets); // Zero-initialized.
...
void RuleMap::Uncompact()
{
    ...
    for (auto& [key, value] : buckets) {
        ...
        value.length = static_cast<unsigned>(GetBucketNumberFromExtent(value).size());
    }
    compacted = false;
}
...
void RuleMap::AddFilteredRulesFromOtherSet(
    const RuleMap& other, const HeapHashSet<Member<StyleRule>>& only_include, const RuleSet& old_rule_set, RuleSet& new_rule_set)
{
    if (compacted) {
        Uncompact();
    }
    if (other.compacted) {
        for (const auto& [key, extent] : other.buckets) {
            ...
            for (const RuleData& rule_data : other.GetRulesFromExtent(extent)) {
                if (IncludeRule(rule_data.Rule(), only_include)) {
                    Add(key, rule_data);
                    new_rule_set.NewlyAddedFromDifferentRuleSet(rule_data, scope_seeker.Seek(rule_data.GetPosition()), old_rule_set, backing.back());
                }
            }
        }
    } else {
...
bool RuleSet::CanIgnoreEntireList(base::span<const RuleData> list, const AtomicString& key, const AtomicString& value) const
{
...
void RuleSet::CreateSubstringMatchers(RuleMap& attr_map, RuleSet::SubstringMatcherMap& substring_matcher_map)
{
    for (const auto& [/*AtomicString*/ attr,
        ...
        for (const RuleData& rule : ruleset) {
            ...
            bool is_exact_attr;
            CSSSelector::PseudoType pseudo_type = CSSSelector::kPseudoUnknown;
            ExtractBestSelectorValues(rule.Selector(), id, class_name, attr_name, attr_value, is_exact_attr, custom_pseudo_element_name, tag_name, part_name,
            ...
            bool already_exists = any_of(
                patterns.begin(), patterns.end(), [&pattern](const MatcherStringPattern& existing_pattern) { return existing_pattern.pattern() == pattern; });
            if (!already_exists) {
...
template <class RuleList> bool IsRuleListSorted(const RuleList& rules)
{
...
bool RuleSet::DidMediaQueryResultsChange(const MediaQueryEvaluator& evaluator) const
{
...
{
    if (!layer_intervals_.size() || layer_intervals_[0].start_position > rule.GetPosition()) {
...
template <class T> void RuleSet::Interval<T>::Trace(Visitor* visitor) const
{
    visitor->Trace(value);
}
