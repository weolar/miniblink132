...
const PropertyRegistration* PropertyRegistration::From(const ExecutionContext* execution_context, const AtomicString& property_name)
{
...
PropertyRegistration::PropertyRegistration(
    const AtomicString& name, const CSSSyntaxDefinition& syntax, bool inherits, const CSSValue* initial, StyleRuleProperty* property_rule)
    : syntax_(syntax)
    , inherits_(inherits)
    , initial_(initial)
    , property_rule_(property_rule)
    , interpolation_types_(CSSInterpolationTypesMap::CreateInterpolationTypesForCSSSyntax(name, syntax, *this))
    , referenced_(false)
{
}
...
{
    unsigned flags = 0;
    if (const auto* primitive_value = DynamicTo<CSSPrimitiveValue>(initial_.Get())) {
...
static bool ComputationallyIndependent(const CSSValue& value)
{
    ...
    if (auto* variable_reference_value = DynamicTo<CSSUnparsedDeclarationValue>(value)) {
        return !variable_reference_value->VariableDataValue()->NeedsVariableResolution();
    }
...
std::optional<CSSSyntaxDefinition> PropertyRegistration::ConvertSyntax(const CSSValue* syntax_value)
{
    // https://drafts.css-houdini.org/css-properties-values-api-1/#the-syntax-descriptor
    if (!syntax_value) {
        return {};
    }
    return CSSSyntaxStringParser(To<CSSStringValue>(*syntax_value).Value()).Parse();
}

std::optional<bool> PropertyRegistration::ConvertInherits(const CSSValue* inherits_value)
{
...
std::optional<const CSSValue*> PropertyRegistration::ConvertInitial(
    const CSSValue* initial_value, const CSSSyntaxDefinition& syntax, const CSSParserContext& parser_context)
{
    ...
    CSSVariableData* initial_variable_data = To<CSSUnparsedDeclarationValue>(*initial_value).VariableDataValue();
    ...
    if (initial_variable_data) {
        ...
        initial = syntax.Parse(initial_variable_data->OriginalText(), parser_context, is_animation_tainted);
        if (!initial) {
            return {};
        }
...
PropertyRegistration* PropertyRegistration::MaybeCreateForDeclaredProperty(Document& document, const AtomicString& name, StyleRuleProperty& rule)
{
    ...
    std::optional<const CSSValue*> initial = ConvertInitial(rule.GetInitialValue(), *syntax, *parser_context);
    if (!initial.has_value()) {
        return nullptr;
    }

    return MakeGarbageCollected<PropertyRegistration>(name, *syntax, *inherits, *initial, &rule);
}

void PropertyRegistration::registerProperty(ExecutionContext* execution_context, const PropertyDefinition* property_definition, ExceptionState& exception_state)
{
    ...
    if (!CSSVariableParser::IsValidVariableName(name)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "Custom property names must start with '--'.");
        return;
    }
    ...
    if (registry.IsInRegisteredPropertySet(atomic_name)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kInvalidModificationError, "The name provided has already been registered.");
        return;
    }

    std::optional<CSSSyntaxDefinition> syntax_definition = CSSSyntaxStringParser(property_definition->syntax()).Parse();
    if (!syntax_definition) {
        exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "The syntax provided is not a valid custom property syntax.");
        return;
    }
    ...
    if (property_definition->hasInitialValue()) {
        ...
        initial = syntax_definition->Parse(property_definition->initialValue(), *parser_context, is_animation_tainted);
        if (!initial) {
            exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "The initial value provided does not parse for the given syntax.");
            return;
        }
        if (!ComputationallyIndependent(*initial)) {
            exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "The initial value provided is not computationally independent.");
            return;
        }
    } else {
        if (!syntax_definition->IsUniversal()) {
            exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "An initial value must be provided if the syntax is not '*'");
            return;
        }
    }
    registry.RegisterProperty(
        atomic_name, *MakeGarbageCollected<PropertyRegistration>(atomic_name, *syntax_definition, property_definition->inherits(), initial));
