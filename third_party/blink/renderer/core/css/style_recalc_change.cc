...
{
    return RecalcChildren() || RecalcContainerQueryDependent() || element.ChildNeedsStyleRecalc();
}

bool StyleRecalcChange::TraversePseudoElements(const Element& element) const
{
    return UpdatePseudoElements() || RecalcContainerQueryDependent() || element.ChildNeedsStyleRecalc();
}

bool StyleRecalcChange::TraverseChild(const Node& node) const
{
    return ShouldRecalcStyleFor(node) || node.ChildNeedsStyleRecalc() || node.GetForceReattachLayoutTree() || RecalcContainerQueryDependent()
...
bool StyleRecalcChange::RecalcContainerQueryDependent(const Node& node) const
{
    ...
    return !old_style
        || (RecalcSizeContainerQueryDependent()
            && (old_style->DependsOnSizeContainerQueries() || old_style->HighlightPseudoElementStylesDependOnContainerUnits()))
        || (RecalcStyleContainerQueryDependent() && old_style->DependsOnStyleContainerQueries())
        || (RecalcScrollStateContainerQueryDependent() && old_style->DependsOnScrollStateContainerQueries());
}
...
bool StyleRecalcChange::ShouldUpdatePseudoElement(const PseudoElement& pseudo_element) const
{
    ...
    return (RecalcSizeContainerQueryDependent() && style.DependsOnSizeContainerQueries())
        || (RecalcStyleContainerQueryDependent() && style.DependsOnStyleContainerQueries());
}
...
StyleRecalcChange::Flags StyleRecalcChange::FlagsForChildren(const Element& element) const
{
    if (!flags_) {
        return 0;
    }
    ...
    if ((result & (kRecalcSizeContainerFlags | kSuppressRecalc)) == kRecalcSizeContainer) {
        if (!RuntimeEnabledFeatures::CSSFlatTreeContainerEnabled() && IsShadowHost(element)) {
...
bool StyleRecalcChange::IndependentInherit(const ComputedStyle& old_style) const
{
    ...
    // element.
    return propagate_ == kIndependentInherit && (!RecalcSizeContainerQueryDependent() || !old_style.DependsOnSizeContainerQueries())
        && (!RecalcStyleContainerQueryDependent() || !old_style.DependsOnStyleContainerQueries());
}
