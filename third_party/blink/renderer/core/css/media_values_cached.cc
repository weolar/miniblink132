    ...
    : MediaValuesCached::MediaValuesCachedData()
{
    ...
    if (frame && frame->View()) {
        ...
        dynamic_viewport_height = MediaValues::CalculateDynamicViewportHeight(frame);
        ...
        color_bits_per_component = MediaValues::CalculateColorBitsPerComponent(frame);
        monochrome_bits_per_component = MediaValues::CalculateMonochromeBitsPerComponent(frame);
        primary_pointer_type = MediaValues::CalculatePrimaryPointerType(frame);
        available_pointer_types = MediaValues::CalculateAvailablePointerTypes(frame);
        primary_hover_type = MediaValues::CalculatePrimaryHoverType(frame);
        output_device_update_ability_type = MediaValues::CalculateOutputDeviceUpdateAbilityType(frame);
        ...
        prefers_reduced_transparency = MediaValues::CalculatePrefersReducedTransparency(frame);
        forced_colors = MediaValues::CalculateForcedColors(frame);
        navigation_controls = MediaValues::CalculateNavigationControls(frame);
        horizontal_viewport_segments = MediaValues::CalculateHorizontalViewportSegments(frame);
        vertical_viewport_segments = MediaValues::CalculateVerticalViewportSegments(frame);
...
mojom::blink::OutputDeviceUpdateAbilityType MediaValuesCached::OutputDeviceUpdateAbilityType() const
{
...
mojom::blink::PreferredColorScheme MediaValuesCached::GetPreferredColorScheme() const
{
    return data_.preferred_color_scheme;
}

mojom::blink::PreferredContrast MediaValuesCached::GetPreferredContrast() const
{
