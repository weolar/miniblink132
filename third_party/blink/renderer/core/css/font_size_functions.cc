...
const FontSizeTableType kQuirksFontSizeTable { {
    { 9, 9, 9, 9, 11, 14, 18, 28 }, { 9, 9, 9, 10, 12, 15, 20, 31 }, { 9, 9, 9, 11, 13, 17, 22, 34 }, { 9, 9, 10, 12, 14, 18, 24, 37 },
    { 9, 9, 10, 13, 16, 20, 26, 40 }, // fixed font default (13)
    { 9, 9, 11, 14, 17, 21, 28, 42 }, { 9, 10, 12, 15, 17, 23, 30, 45 }, { 9, 10, 13, 16, 18, 24, 32, 48 } // proportional font default (16)
} };
// HTML       1      2      3      4      5      6      7
...
const FontSizeTableType kStrictFontSizeTable { {
    { 9, 9, 9, 9, 11, 14, 18, 27 }, { 9, 9, 9, 10, 12, 15, 20, 30 }, { 9, 9, 10, 11, 13, 17, 22, 33 }, { 9, 9, 10, 12, 14, 18, 24, 36 },
    { 9, 10, 12, 13, 16, 20, 26, 39 }, // fixed font default (13)
    { 9, 10, 12, 14, 17, 21, 28, 42 }, { 9, 10, 13, 15, 18, 23, 30, 45 }, { 9, 10, 13, 16, 18, 24, 32, 48 } // proportional font default (16)
} };
// HTML       1      2      3      4      5      6      7
// CSS  xxs   xs     s      m      l     xl     xxl
//                          |
...
int inline RowFromMediumFontSizeInRange(const Settings* settings, bool quirks_mode, bool is_monospace, int& medium_size)
{
    medium_size = settings ? (is_monospace ? settings->GetDefaultFixedFontSize() : settings->GetDefaultFontSize()) : kDefaultMediumFontSize;
...
template <typename T> int FindNearestLegacyFontSize(int pixel_font_size, const std::array<T, kTotalKeywords>& table, int multiplier)
{
...
float AspectValue(const SimpleFontData& font_data, FontSizeAdjust::Metric metric, float computed_size)
{
    ...
    case FontSizeAdjust::Metric::kIcWidth:
        if (const std::optional<float>& size = font_data.IdeographicAdvanceWidth()) {
...
float FontSizeFunctions::GetComputedSizeFromSpecifiedSize(
    const Document* document, float zoom_factor, bool is_absolute_size, float specified_size, ApplyMinimumFontSize apply_minimum_font_size)
{
...
float FontSizeFunctions::FontSizeForKeyword(const Document* document, unsigned keyword, bool is_monospace)
{
    ...
    int medium_size = 0;
    int row = RowFromMediumFontSizeInRange(settings, quirks_mode, is_monospace, medium_size);
    if (row >= 0) {
        int col = (keyword - 1);
        return quirks_mode ? kQuirksFontSizeTable[row][col] : kStrictFontSizeTable[row][col];
    }

    // Value is outside the range of the table. Apply the scale factor instead.
    float min_logical_size = settings ? std::max(settings->GetMinimumLogicalFontSize(), 1) : 1;
    return std::max(kFontSizeFactors[keyword - 1] * medium_size, min_logical_size);
}

int FontSizeFunctions::LegacyFontSize(const Document* document, int pixel_font_size, bool is_monospace)
{
    ...
    int row = RowFromMediumFontSizeInRange(settings, quirks_mode, is_monospace, medium_size);
    if (row >= 0) {
        return FindNearestLegacyFontSize<int>(pixel_font_size, quirks_mode ? kQuirksFontSizeTable[row] : kStrictFontSizeTable[row], 1);
    }

    return FindNearestLegacyFontSize<float>(pixel_font_size, kFontSizeFactors, medium_size);
}

std::optional<float> FontSizeFunctions::FontAspectValue(const SimpleFontData* font_data, FontSizeAdjust::Metric metric, float computed_size)
{
...
std::optional<float> FontSizeFunctions::MetricsMultiplierAdjustedFontSize(const SimpleFontData* font_data, const FontDescription& font_description)
{
    DCHECK(font_data);
    const float computed_size = font_description.ComputedSize();
    const FontSizeAdjust size_adjust = font_description.SizeAdjust();
    if (!computed_size || size_adjust.Value() == FontSizeAdjust::kFontSizeAdjustNone) {
        return std::nullopt;
    }
