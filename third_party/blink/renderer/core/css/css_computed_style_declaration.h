...
public:
    static const Vector<const CSSProperty*>& ComputableProperties(const ExecutionContext*);
    ...
    explicit CSSComputedStyleDeclaration(Element*, bool allow_visited_style = false, const String& = String());
    ...
    const CSSValue* GetPropertyCSSValue(const AtomicString& custom_property_name) const;
...
private:
    ...
    void UpdateStyleAndLayoutTreeIfNeeded(const CSSPropertyName*, bool for_all_properties) const;
    void UpdateStyleAndLayoutIfNeeded(const CSSProperty*, bool for_all_properties) const;
    ...
    void setProperty(const ExecutionContext*, const String& property_name, const String& value, const String& priority, ExceptionState&) override;
    ...
    void setCSSText(const ExecutionContext*, const String&, ExceptionState&) override;
    const CSSValue* GetPropertyCSSValueInternal(CSSPropertyID) override;
    const CSSValue* GetPropertyCSSValueInternal(const AtomicString& custom_property_name) override;
    String GetPropertyValueInternal(CSSPropertyID) override;
    String GetPropertyValueWithHint(const String& property_name, unsigned index) override;
    String GetPropertyPriorityWithHint(const String& property_name, unsigned index) override;
    void SetPropertyInternal(CSSPropertyID, const String& custom_property_name, StringView value, bool important, SecureContextMode, ExceptionState&) override;
