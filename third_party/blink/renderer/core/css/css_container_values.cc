...
CSSContainerValues::CSSContainerValues(Document& document, Element& container, std::optional<double> width, std::optional<double> height,
    ContainerStuckPhysical stuck_horizontal, ContainerStuckPhysical stuck_vertical, ContainerSnappedFlags snapped,
    ...
    , font_sizes_(CSSToLengthConversionData::FontSizes(container.ComputedStyleRef().GetFontSizeStyle(), document.documentElement()->GetComputedStyle()))
    , line_height_size_(
          CSSToLengthConversionData::LineHeightSize(container.ComputedStyleRef().GetFontSizeStyle(), document.documentElement()->GetComputedStyle()))
    , font_style_(container.GetComputedStyle())
    , root_font_style_(document.documentElement()->GetComputedStyle())
    , container_sizes_(ContainerQueryEvaluator::ParentContainerCandidateElement(container))
{
}
...
ContainerStuckLogical PhysicalToLogicalLtrHorizontalTb(ContainerStuckPhysical physical)
{
    switch (physical) {
...
{
    ContainerStuckPhysical physical = writing_direction_.IsHorizontal() ? StuckHorizontal() : StuckVertical();
...
{
    ContainerStuckPhysical physical = writing_direction_.IsHorizontal() ? StuckVertical() : StuckHorizontal();
    ContainerStuckLogical logical = PhysicalToLogicalLtrHorizontalTb(physical);
...
{
    ContainerOverflowingFlags overflowing_inline = writing_direction_.IsHorizontal() ? OverflowingHorizontal() : OverflowingVertical();
    return writing_direction_.IsRtl() ? Flip(overflowing_inline) : overflowing_inline;
}

ContainerOverflowingFlags CSSContainerValues::OverflowingBlock() const
{
    ContainerOverflowingFlags overflowing_block = writing_direction_.IsHorizontal() ? OverflowingVertical() : OverflowingHorizontal();
    return writing_direction_.IsFlippedBlocks() ? Flip(overflowing_block) : overflowing_block;
}
