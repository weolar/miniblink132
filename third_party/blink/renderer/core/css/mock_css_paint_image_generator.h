...
public:
    MockCSSPaintImageGenerator()
    {
        ...
        ON_CALL(*this, NativeInvalidationProperties()).WillByDefault(ReturnRef(native_properties_));
        ON_CALL(*this, CustomInvalidationProperties()).WillByDefault(ReturnRef(custom_properties_));
        ON_CALL(*this, InputArgumentTypes()).WillByDefault(ReturnRef(input_argument_types_));
    }

    MOCK_METHOD3(Paint, scoped_refptr<Image>(const ImageResourceObserver&, const gfx::SizeF& container_size, const CSSStyleValueVector*));
