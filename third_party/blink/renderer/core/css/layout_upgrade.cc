...
bool DocumentLayoutUpgrade::ShouldUpgrade()
{
    ...
    return style_engine.SkippedContainerRecalc() || style_engine.StyleAffectedByLayout();
}

bool ParentLayoutUpgrade::ShouldUpgrade()
{
    StyleEngine& style_engine = document_.GetStyleEngine();
    return style_engine.HasViewportDependentMediaQueries() || style_engine.HasViewportDependentPropertyRegistrations()
...
bool ElementLayoutUpgrade::ShouldUpgrade()
{
    ...
    bool maybe_affected_by_layout = style_engine.StyleMaybeAffectedByLayout(element_);
    ...
    for (const Element* ancestor = &element_; ancestor; ancestor = LayoutTreeBuilderTraversal::ParentElement(*ancestor)) {
