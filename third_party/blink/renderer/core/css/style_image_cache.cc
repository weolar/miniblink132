...
ImageResourceContent* StyleImageCache::CacheImageContent(ResourceFetcher* fetcher, FetchParameters& params)
{
    CHECK(!params.Url().IsNull());

    const KURL url_without_fragment = MemoryCache::RemoveFragmentIdentifierIfNeeded(params.Url());
    auto& image_content = fetched_image_map_.insert(url_without_fragment.GetString(), nullptr).stored_value->value;
