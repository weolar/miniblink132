...
protected:
    String LayersToString() const
    {
        return root_layer_->ToStringForTesting();
    }
...
{
    CascadeLayer* one = root_layer_->GetOrAddSubLayer(LayerName({ AtomicString("one") }));
    one->GetOrAddSubLayer(LayerName({ AtomicString("two") }));
    root_layer_->GetOrAddSubLayer(LayerName({ AtomicString("three"), AtomicString("four") }));
...
TEST_F(CascadeLayerTest, RepeatedGetOrAdd)
{
    ...
    root_layer_->GetOrAddSubLayer(LayerName({ AtomicString("one"), AtomicString("two") }));
    root_layer_->GetOrAddSubLayer(LayerName({ AtomicString("three") }));

    root_layer_->GetOrAddSubLayer(LayerName({ AtomicString("one") }))->GetOrAddSubLayer(LayerName({ AtomicString("two") }));
    root_layer_->GetOrAddSubLayer(LayerName({ AtomicString("three") }));
...
TEST_F(CascadeLayerTest, RepeatedGetOrAddAnonymous)
{
    ...
    root_layer_->GetOrAddSubLayer(LayerName({ AtomicString("one"), g_empty_atom, AtomicString("two") }));
...
TEST_F(CascadeLayerTest, LayerOrderNotInsertionOrder)
{
    ...
    root_layer_->GetOrAddSubLayer(LayerName({ AtomicString("one"), AtomicString("three") }));
