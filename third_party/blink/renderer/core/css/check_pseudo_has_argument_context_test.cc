...
protected:
    ...
    const int kAdjacentMax = CheckPseudoHasArgumentContext::kInfiniteAdjacentDistance;

    void TestArgumentContext(const String& selector_text, CSSSelector::RelationType expected_leftmost_relation, int expected_adjacent_distance_limit,
        int expected_depth_limit, CheckPseudoHasArgumentTraversalScope expected_traversal_scope, bool match_in_shadow_tree = false) const
    {
        CSSSelectorList* selector_list = css_test_helpers::ParseSelectorList(selector_text);
        CheckPseudoHasArgumentContext context(selector_list->First()->SelectorList()->First(), match_in_shadow_tree);

        EXPECT_EQ(expected_leftmost_relation, context.LeftmostRelation()) << "Failed : " << selector_text;
        EXPECT_EQ(expected_adjacent_distance_limit, context.AdjacentDistanceLimit()) << "Failed : " << selector_text;
        EXPECT_EQ(expected_depth_limit, context.DepthLimit()) << "Failed : " << selector_text;
        EXPECT_EQ(expected_traversal_scope, context.TraversalScope()) << "Failed : " << selector_text;
        EXPECT_EQ(match_in_shadow_tree, context.MatchInShadowTree()) << "Failed : " << selector_text;
    }
    ...
    void TestTraversalIteratorForEmptyRange(
        Document* document, const char* has_anchor_element_id, const char* selector_text, bool match_in_shadow_tree = false) const
    {
        Element* has_anchor_element = document->getElementById(AtomicString(has_anchor_element_id));
        if (!has_anchor_element) {
            ADD_FAILURE() << "Failed : test iterator on #" << has_anchor_element_id << " (Cannot find element)";
            return;
        }

        unsigned i = 0;
        CSSSelectorList* selector_list = css_test_helpers::ParseSelectorList(selector_text);
        CheckPseudoHasArgumentContext argument_context(selector_list->First()->SelectorList()->First(), match_in_shadow_tree);
        for (CheckPseudoHasArgumentTraversalIterator iterator(*has_anchor_element, argument_context); !iterator.AtEnd(); ++iterator, ++i) {
            AtomicString current_element_id = iterator.CurrentElement() ? iterator.CurrentElement()->GetIdAttribute() : g_null_atom;
            int current_depth = iterator.CurrentDepth();
            ADD_FAILURE() << "Iteration failed : exceeded expected iteration"
                          << " (selector: " << selector_text << ", has_anchor_element: #" << has_anchor_element_id << ", index: " << i
                          << ", current_element: " << current_element_id << ", current_depth: " << current_depth << ")";
        }
    }

    CheckPseudoHasArgumentTraversalType GetTraversalType(const char* selector_text, bool match_in_shadow_tree = false) const
    {
        CSSSelectorList* selector_list = css_test_helpers::ParseSelectorList(selector_text);
    ...
    void TestTraversalIteratorSteps(Document* document, const char* has_anchor_element_id, const char* selector_text,
        const ExpectedTraversalStep (&expected_traversal_steps)[length], bool match_in_shadow_tree = false) const
    {
        Element* has_anchor_element = document->getElementById(AtomicString(has_anchor_element_id));
        if (!has_anchor_element) {
            ADD_FAILURE() << "Failed : test iterator on #" << has_anchor_element_id << " (Cannot find element)";
        ...
        for (CheckPseudoHasArgumentTraversalIterator iterator(*has_anchor_element, argument_context); !iterator.AtEnd(); ++iterator, ++i) {
            ...
            EXPECT_EQ(expected_traversal_steps[i].element_id, current_element_id)
                << " (selector: " << selector_text << ", has_anchor_element: #" << has_anchor_element_id << ", index: " << i
                << ", expected: " << expected_traversal_steps[i].element_id << ", actual: " << current_element_id << ")";
            EXPECT_EQ(expected_traversal_steps[i].depth, current_depth)
                << " (selector: " << selector_text << ", has_anchor_element: #" << has_anchor_element_id << ", index: " << i
                << ", expected: " << expected_traversal_steps[i].depth << ", actual: " << current_depth << ")";
        }

        for (; i < length; i++) {
            ADD_FAILURE() << "Iteration failed : expected but not traversed"
                          << " (selector: " << selector_text << ", has_anchor_element: #" << has_anchor_element_id << ", index: " << i
                          << ", expected_element: " << expected_traversal_steps[i].element_id << ")";
            EXPECT_NE(document->getElementById(AtomicString(expected_traversal_steps[i].element_id)), nullptr);
        }
    }
};
...
TEST_F(CheckPseudoHasArgumentContextTest, TestArgumentMatchContext)
{
    ...
    TestArgumentContext(":has(~ .a + .b > .c ~ .d .e)", CSSSelector::kRelativeIndirectAdjacent,
        /* expected_adjacent_distance_limit */ kAdjacentMax,
        /* expected_depth_limit */ kDepthMax, kAllNextSiblingSubtrees);
    ...
    TestArgumentContext(":has(~ .a + .b)", CSSSelector::kRelativeIndirectAdjacent,
        /* expected_adjacent_distance_limit */ kAdjacentMax,
        /* expected_depth_limit */ 0, kAllNextSiblings);
    TestArgumentContext(":has(~ .a + .b ~ .c)", CSSSelector::kRelativeIndirectAdjacent,
        /* expected_adjacent_distance_limit */ kAdjacentMax,
        /* expected_depth_limit */ 0, kAllNextSiblings);
    TestArgumentContext(":has(~ .a > .b)", CSSSelector::kRelativeIndirectAdjacent,
        /* expected_adjacent_distance_limit */ kAdjacentMax,
    ...
    TestArgumentContext(":has(+ .a ~ .b .c)", CSSSelector::kRelativeDirectAdjacent,
        /* expected_adjacent_distance_limit */ kAdjacentMax,
        /* expected_depth_limit */ kDepthMax, kAllNextSiblingSubtrees);
    TestArgumentContext(":has(+ .a ~ .b > .c + .d .e)", CSSSelector::kRelativeDirectAdjacent,
        /* expected_adjacent_distance_limit */ kAdjacentMax,
        /* expected_depth_limit */ kDepthMax, kAllNextSiblingSubtrees);
    ...
    TestArgumentContext(":has(+ .a + .b .c)", CSSSelector::kRelativeDirectAdjacent,
        ...
        /* expected_depth_limit */ kDepthMax, kOneNextSiblingSubtree);
    TestArgumentContext(":has(+ .a > .b + .c .d)", CSSSelector::kRelativeDirectAdjacent,
        /* expected_adjacent_distance_limit */ 1,
        /* expected_depth_limit */ kDepthMax, kOneNextSiblingSubtree);
    TestArgumentContext(":has(+ .a + .b > .c .d)", CSSSelector::kRelativeDirectAdjacent,
        /* expected_adjacent_distance_limit */ 2,
    ...
    TestArgumentContext(":has(~ .a)", CSSSelector::kRelativeIndirectAdjacent,
        /* expected_adjacent_distance_limit */ kAdjacentMax,
        /* expected_depth_limit */ 0, kInvalidShadowRootTraversalScope,
        /* match_in_shadow_tree */ true);
    TestArgumentContext(":has(+ .a .b)", CSSSelector::kRelativeDirectAdjacent,
        /* expected_adjacent_distance_limit */ 1,
        /* expected_depth_limit */ kDepthMax, kInvalidShadowRootTraversalScope,
        /* match_in_shadow_tree */ true);
    TestArgumentContext(":has(~ .a .b)", CSSSelector::kRelativeIndirectAdjacent,
        /* expected_adjacent_distance_limit */ kAdjacentMax,
        /* expected_depth_limit */ kDepthMax, kInvalidShadowRootTraversalScope,
    ...
    TestArgumentContext(":has(> .a)", CSSSelector::kRelativeChild,
        /* expected_adjacent_distance_limit */ 0,
        /* expected_depth_limit */ 1, kShadowRootFixedDepthDescendants,
        /* match_in_shadow_tree */ true);
    TestArgumentContext(":has(+ .a > .b)", CSSSelector::kRelativeDirectAdjacent,
        /* expected_adjacent_distance_limit */ 1,
        /* expected_depth_limit */ 1, kInvalidShadowRootTraversalScope,
        /* match_in_shadow_tree */ true);
    TestArgumentContext(":has(~ .a > .b)", CSSSelector::kRelativeIndirectAdjacent,
        /* expected_adjacent_distance_limit */ kAdjacentMax,
        /* expected_depth_limit */ 1, kInvalidShadowRootTraversalScope,
...
TEST_F(CheckPseudoHasArgumentContextTest, TestTraversalType)
{
    ...
    EXPECT_EQ(GetTraversalType(":has(+ .a + .b > .c ~ .d > .e ~ .f)"), traversal_type);
...
TEST_F(CheckPseudoHasArgumentContextTest, TestTraversalIteratorCase1)
{
    ...
    TestTraversalIteratorSteps(document, "div1", ":has(.a)",
        { { "div13", /* depth */ 1 }, { "div123", /* depth */ 2 }, { "div1223", /* depth */ 3 }, { "div1222", /* depth */ 3 }, { "div1221", /* depth */ 3 },
            { "div122", /* depth */ 2 }, { "div121", /* depth */ 2 }, { "div12", /* depth */ 1 }, { "div111", /* depth */ 2 }, { "div11", /* depth */ 1 } });

    TestTraversalIteratorSteps(document, "div12", ":has(.a)",
        { { "div123", /* depth */ 1 }, { "div1223", /* depth */ 2 }, { "div1222", /* depth */ 2 }, { "div1221", /* depth */ 2 }, { "div122", /* depth */ 1 },
            { "div121", /* depth */ 1 } });

    TestTraversalIteratorSteps(document, "div122", ":has(.a)", { { "div1223", /* depth */ 1 }, { "div1222", /* depth */ 1 }, { "div1221", /* depth */ 1 } });

    TestTraversalIteratorSteps(document, "div11", ":has(.a)", { { "div111", /* depth */ 1 } });
...
TEST_F(CheckPseudoHasArgumentContextTest, TestTraversalIteratorCase2)
{
    ...
    TestTraversalIteratorSteps(document, "div1", ":has(~ .a)", { { "div4", /* depth */ 0 }, { "div3", /* depth */ 0 }, { "div2", /* depth */ 0 } });

    TestTraversalIteratorSteps(document, "div3", ":has(~ .a)", { { "div4", /* depth */ 0 } });
...
TEST_F(CheckPseudoHasArgumentContextTest, TestTraversalIteratorCase3)
{
    ...
    TestTraversalIteratorSteps(document, "div1", ":has(+ .a + .b .c)",
        { { "div3411", /* depth */ 3 }, { "div341", /* depth */ 2 }, { "div34", /* depth */ 1 }, { "div33", /* depth */ 1 }, { "div321", /* depth */ 2 },
            { "div32", /* depth */ 1 }, { "div311", /* depth */ 2 }, { "div31", /* depth */ 1 }, { "div3", /* depth */ 0 }, { "div2", /* depth */ 0 } });

    TestTraversalIteratorSteps(document, "div2", ":has(+ .a + .b .c)", { { "div41", /* depth */ 1 }, { "div4", /* depth */ 0 }, { "div3", /* depth */ 0 } });

    TestTraversalIteratorSteps(document, "div3", ":has(+ .a + .b .c)", { { "div4", /* depth */ 0 } });

    TestTraversalIteratorSteps(document, "div31", ":has(+ .a + .b .c)", { { "div33", /* depth */ 0 }, { "div32", /* depth */ 0 } });

    TestTraversalIteratorSteps(document, "div32", ":has(+ .a + .b .c)",
        { { "div3411", /* depth */ 2 }, { "div341", /* depth */ 1 }, { "div34", /* depth */ 0 }, { "div33", /* depth */ 0 } });
...
TEST_F(CheckPseudoHasArgumentContextTest, TestTraversalIteratorCase4)
{
    ...
    TestTraversalIteratorSteps(document, "div2", ":has(~ .a .b)",
        { { "div5", /* depth */ 0 }, { "div41", /* depth */ 1 }, { "div4", /* depth */ 0 }, { "div3411", /* depth */ 3 }, { "div341", /* depth */ 2 },
            { "div34", /* depth */ 1 }, { "div33", /* depth */ 1 }, { "div321", /* depth */ 2 }, { "div32", /* depth */ 1 }, { "div311", /* depth */ 2 },
            { "div31", /* depth */ 1 }, { "div3", /* depth */ 0 } });

    TestTraversalIteratorSteps(document, "div4", ":has(~ .a .b)", { { "div5", /* depth */ 0 } });
...
TEST_F(CheckPseudoHasArgumentContextTest, TestTraversalIteratorCase5)
{
    ...
    TestTraversalIteratorSteps(document, "div1", ":has(+ .a + .b)", { { "div3", /* depth */ 0 }, { "div2", /* depth */ 0 } });

    TestTraversalIteratorSteps(document, "div2", ":has(+ .a + .b)", { { "div4", /* depth */ 0 }, { "div3", /* depth */ 0 } });
...
TEST_F(CheckPseudoHasArgumentContextTest, TestTraversalIteratorCase6)
{
    ...
    TestTraversalIteratorSteps(document, "div1", ":has(> .a > .b)",
        { { "div13", /* depth */ 1 }, { "div123", /* depth */ 2 }, { "div122", /* depth */ 2 }, { "div121", /* depth */ 2 }, { "div12", /* depth */ 1 },
            { "div111", /* depth */ 2 }, { "div11", /* depth */ 1 } });
    ...
    TestTraversalIteratorSteps(
        document, "div122", ":has(> .a > .b)", { { "div1223", /* depth */ 1 }, { "div1222", /* depth */ 1 }, { "div1221", /* depth */ 1 } });

    TestTraversalIteratorSteps(document, "div11", ":has(> .a > .b)", { { "div111", /* depth */ 1 } });
...
TEST_F(CheckPseudoHasArgumentContextTest, TestTraversalIteratorCase7)
{
    ...
    TestTraversalIteratorSteps(document, "div1", ":has(+ .a + .b > .c > .d)",
        { { "div341", /* depth */ 2 }, { "div34", /* depth */ 1 }, { "div33", /* depth */ 1 }, { "div321", /* depth */ 2 }, { "div32", /* depth */ 1 },
            { "div311", /* depth */ 2 }, { "div31", /* depth */ 1 }, { "div3", /* depth */ 0 }, { "div2", /* depth */ 0 } });

    TestTraversalIteratorSteps(
        document, "div2", ":has(+ .a + .b > .c > .d)", { { "div41", /* depth */ 1 }, { "div4", /* depth */ 0 }, { "div3", /* depth */ 0 } });

    TestTraversalIteratorSteps(document, "div3", ":has(+ .a + .b > .c > .d)", { { "div4", /* depth */ 0 } });

    TestTraversalIteratorSteps(document, "div31", ":has(+ .a + .b > .c > .d)", { { "div33", /* depth */ 0 }, { "div32", /* depth */ 0 } });

    TestTraversalIteratorSteps(document, "div32", ":has(+ .a + .b > .c > .d)",
        { { "div3411", /* depth */ 2 }, { "div341", /* depth */ 1 }, { "div34", /* depth */ 0 }, { "div33", /* depth */ 0 } });

    TestTraversalIteratorForEmptyRange(document, "div4", ":has(+ .a + .b > .c > .d)");
}
...
TEST_F(CheckPseudoHasArgumentContextTest, TestTraversalIteratorCase8)
{
    ...
    TestTraversalIteratorSteps(document, "div2", ":has(~ .a > .b > .c)",
        { { "div5", /* depth */ 0 }, { "div41", /* depth */ 1 }, { "div4", /* depth */ 0 }, { "div341", /* depth */ 2 }, { "div34", /* depth */ 1 },
            { "div33", /* depth */ 1 }, { "div321", /* depth */ 2 }, { "div32", /* depth */ 1 }, { "div311", /* depth */ 2 }, { "div31", /* depth */ 1 },
            { "div3", /* depth */ 0 } });

    TestTraversalIteratorSteps(document, "div31", ":has(~ .a > .b > .c)",
        { { "div3411", /* depth */ 2 }, { "div341", /* depth */ 1 }, { "div34", /* depth */ 0 }, { "div33", /* depth */ 0 }, { "div321", /* depth */ 1 },
            { "div32", /* depth */ 0 } });

    TestTraversalIteratorSteps(document, "div4", ":has(~ .a > .b > .c)", { { "div5", /* depth */ 0 } });
