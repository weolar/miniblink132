...
CascadeLayerMap::CascadeLayerMap(const ActiveStyleSheetVector& sheets)
{
    ...
    for (const auto& sheet : sheets) {
        ...
        if (rule_set && rule_set->HasCascadeLayers()) {
            canonical_root_layer->Merge(rule_set->CascadeLayers(), canonical_layer_map);
        }
    }
...
int CascadeLayerMap::CompareLayerOrder(const CascadeLayer* lhs, const CascadeLayer* rhs) const
{
