...
public:
    ...
    {
        return type == UnitType::kPercentage || type == UnitType::kEms || type == UnitType::kExs || type == UnitType::kRems || type == UnitType::kChs
            || type == UnitType::kIcs || type == UnitType::kLhs || type == UnitType::kRexs || type == UnitType::kRchs || type == UnitType::kRics
            || type == UnitType::kRlhs || type == UnitType::kCaps || type == UnitType::kRcaps || IsViewportPercentageLength(type)
    ...
    static bool IsFlex(UnitType unit)
    {
        return unit == UnitType::kFlex;
    }
    bool IsFlex() const;
    ...
    template <typename T> T ComputeLength(const CSSLengthResolver&) const;
    ...
    template <typename T> inline T ConvertTo(const CSSLengthResolver&) const; // Defined in CSSPrimitiveValueMappings.h
...
template <> struct DowncastTraits<CSSPrimitiveValue> {
...
template <> int CSSPrimitiveValue::ComputeLength(const CSSLengthResolver&) const;

template <> Length CSSPrimitiveValue::ComputeLength(const CSSLengthResolver&) const;

template <> unsigned CSSPrimitiveValue::ComputeLength(const CSSLengthResolver&) const;

template <> int16_t CSSPrimitiveValue::ComputeLength(const CSSLengthResolver&) const;

template <> CORE_EXPORT float CSSPrimitiveValue::ComputeLength(const CSSLengthResolver&) const;

template <> CORE_EXPORT double CSSPrimitiveValue::ComputeLength(const CSSLengthResolver&) const;
} // namespace blink
