...
std::optional<ColorSpaceGamut> ConvertColorGamut(const MediaQueryExpValue& value)
{
    if (!value.IsValid()) {
        return std::nullopt;
    }
...
std::optional<ForcedColors> ConvertForcedColors(const MediaQueryExpValue& value)
{
    if (!value.IsValid()) {
        return std::nullopt;
    }
...
std::optional<mojom::blink::PreferredColorScheme> MediaFeatureOverrides::ConvertPreferredColorScheme(const MediaQueryExpValue& value)
{
    if (!value.IsValid()) {
        return std::nullopt;
    }
...
std::optional<mojom::blink::PreferredContrast> MediaFeatureOverrides::ConvertPreferredContrast(const MediaQueryExpValue& value)
{
    if (!value.IsValid()) {
        return std::nullopt;
    }
...
std::optional<bool> MediaFeatureOverrides::ConvertPrefersReducedMotion(const MediaQueryExpValue& value)
{
    if (!value.IsValid()) {
        return std::nullopt;
    }
...
std::optional<bool> MediaFeatureOverrides::ConvertPrefersReducedData(const MediaQueryExpValue& value)
{
    if (!value.IsValid()) {
        return std::nullopt;
    }
...
std::optional<bool> MediaFeatureOverrides::ConvertPrefersReducedTransparency(const MediaQueryExpValue& value)
{
    if (!value.IsValid()) {
        return std::nullopt;
    }
...
MediaQueryExpValue MediaFeatureOverrides::ParseMediaQueryValue(const AtomicString& feature, const String& value_string, const Document* document)
{
    ...
    const CSSParserContext* fake_context = MakeGarbageCollected<CSSParserContext>(kHTMLStandardMode, SecureContextMode::kInsecureContext, document);
    ...
    // removed.
    MediaQueryExpBounds bounds = MediaQueryExp::Create(feature, stream, *fake_context).Bounds();
    DCHECK(!bounds.left.IsValid());
    return bounds.right.value;
}

void MediaFeatureOverrides::SetOverride(const AtomicString& feature, const String& value_string, const Document* document)
{
    MediaQueryExpValue value = ParseMediaQueryValue(feature, value_string, document);
    ...
    } else if (feature == media_feature_names::kPrefersReducedMotionMediaFeature) {
    ...
    } else if (feature == media_feature_names::kPrefersReducedTransparencyMediaFeature) {
