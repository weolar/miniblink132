...
bool PageRuleCollector::IsLeftPage(const ComputedStyle* root_element_style, uint32_t page_index) const
{
...
PageRuleCollector::PageRuleCollector(
    const ComputedStyle* root_element_style, CSSAtRuleID at_rule_id, uint32_t page_index, const AtomicString& page_name, MatchResult& match_result)
    ...
    , result_(match_result)
{
    DCHECK(at_rule_id_ == CSSAtRuleID::kCSSAtRulePage
        || (at_rule_id_ >= CSSAtRuleID::kCSSAtRuleTopLeftCorner && at_rule_id_ <= CSSAtRuleID::kCSSAtRuleRightBottom));
}

void PageRuleCollector::MatchPageRules(RuleSet* rules, CascadeOrigin origin, TreeScope* tree_scope, const CascadeLayerMap* layer_map)
{
    if (!rules) {
        return;
    }
    ...
    std::stable_sort(matched_page_rules.begin(), matched_page_rules.end(), [layer_map](const StyleRulePage* r1, const StyleRulePage* r2) {
        if (r1->GetCascadeLayer() != r2->GetCascadeLayer()) {
            DCHECK(layer_map);
            return layer_map->CompareLayerOrder(r1->GetCascadeLayer(), r2->GetCascadeLayer()) < 0;
        }
    ...
    if (RuntimeEnabledFeatures::PageMarginBoxesEnabled()) {
        ...
        options.valid_property_filter = static_cast<uint8_t>(ValidPropertyFilter::kPageContext);
    } else {
        ...
        options.valid_property_filter = static_cast<uint8_t>(ValidPropertyFilter::kLimitedPageContext);
    }
...
static bool CheckPageSelectorComponents(const CSSSelector* selector, bool is_left_page, bool is_first_page, const AtomicString& page_name)
{
    for (const CSSSelector* component = selector; component; component = component->NextSimpleSelector()) {
        ...
        if ((pseudo_type == CSSSelector::kPseudoLeftPage && !is_left_page) || (pseudo_type == CSSSelector::kPseudoRightPage && is_left_page)
...
void PageRuleCollector::MatchPageRulesForList(HeapVector<Member<StyleRulePage>>& matched_rules, const HeapVector<Member<StyleRulePage>>& rules)
{
    for (unsigned i = 0; i < rules.size(); ++i) {
        StyleRulePage* rule = rules[i];

        if (!CheckPageSelectorComponents(rule->Selector(), is_left_page_, is_first_page_, page_name_)) {
            continue;
        }
