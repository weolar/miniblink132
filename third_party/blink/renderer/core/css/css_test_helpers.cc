...
{
    RuleSet& rule_set = style_sheet_->Contents()->EnsureRuleSet(MediaQueryEvaluator(document_->GetFrame()));
...
{
    return CSSStyleSheet::CreateInline(document, NullURL(), TextPosition::MinimumPosition(), UTF8Encoding());
}
...
RuleSet* CreateRuleSet(Document& document, String text)
{
    ...
    auto* media_query_evaluator = MakeGarbageCollected<MediaQueryEvaluator>(document.GetFrame());
...
PropertyRegistration* CreatePropertyRegistration(const String& name, String syntax, const CSSValue* initial_value, bool is_inherited)
{
    ...
    return MakeGarbageCollected<PropertyRegistration>(AtomicString(name), *syntax_definition, is_inherited, initial_value);
}

PropertyRegistration* CreateLengthRegistration(const String& name, int px)
{
    const CSSValue* initial = CSSNumericLiteralValue::Create(px, CSSPrimitiveValue::UnitType::kPixels);
    return CreatePropertyRegistration(name, "<length>", initial, false /* is_inherited */);
}

void RegisterProperty(Document& document, const String& name, const String& syntax, const std::optional<String>& initial_value, bool is_inherited)
{
    DummyExceptionStateForTesting exception_state;
    RegisterProperty(document, name, syntax, initial_value, is_inherited, exception_state);
    ...
    ExceptionState& exception_state)
{
    ...
    PropertyRegistration::registerProperty(document.GetExecutionContext(), property_definition, exception_state);
}

void DeclareProperty(Document& document, const String& name, const String& syntax, const std::optional<String>& initial_value, bool is_inherited)
{
    ...
    auto* rule = DynamicTo<StyleRuleProperty>(ParseRule(document, builder.ToString()));
    if (!rule) {
        return;
    }
    auto* registration = PropertyRegistration::MaybeCreateForDeclaredProperty(document, AtomicString(name), *rule);
    if (!registration) {
        return;
    }
    document.EnsurePropertyRegistry().DeclareProperty(AtomicString(name), *registration);
...
const CSSValue* ParseLonghand(Document& document, const CSSProperty& property, const String& value)
{
...
const CSSPropertyValueSet* ParseDeclarationBlock(const String& block_text, CSSParserMode mode)
{
    auto* set = MakeGarbageCollected<MutableCSSPropertyValueSet>(mode);
    set->ParseDeclarationList(block_text, SecureContextMode::kSecureContext, nullptr);
    return set;
}

StyleRuleBase* ParseRule(Document& document, String text)
{
    auto* sheet = CSSStyleSheet::CreateInline(document, NullURL(), TextPosition::MinimumPosition(), UTF8Encoding());
    const auto* context = MakeGarbageCollected<CSSParserContext>(document);
...
CSSSelectorList* ParseSelectorList(const String& string, CSSNestingType nesting_type, const StyleRule* parent_rule_for_nesting, bool is_within_scope)
{
    ...
    base::span<CSSSelector> vector = CSSSelectorParser::ParseSelector(stream, context, nesting_type, parent_rule_for_nesting, is_within_scope,
