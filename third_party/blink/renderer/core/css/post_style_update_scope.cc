...
PostStyleUpdateScope::AnimationData* PostStyleUpdateScope::CurrentAnimationData()
{
...
PostStyleUpdateScope::~PostStyleUpdateScope()
{
    ...
    DCHECK(animation_data_.elements_with_pending_updates_.empty()) << "Missing Apply (animations)";
    DCHECK(pseudo_data_.pending_backdrops_.empty()) << "Missing Apply (::backdrop)";
}
...
{
    StyleEngine::InApplyAnimationUpdateScope in_apply_animation_update_scope(document_.GetStyleEngine());
    ...
    DCHECK(animation_data_.elements_with_pending_updates_.empty()) << "MaybeApplyPendingUpdate must not set further pending updates";
}

void PostStyleUpdateScope::AnimationData::SetPendingUpdate(Element& element, const CSSAnimationUpdate& update)
{
    element.EnsureElementAnimations().CssAnimations().SetPendingUpdate(update);
    elements_with_pending_updates_.insert(&element);
}

void PostStyleUpdateScope::AnimationData::StoreOldStyleIfNeeded(Element& element)
{
    old_styles_.insert(&element, ComputedStyle::NullifyEnsured(element.GetComputedStyle()));
}

const ComputedStyle* PostStyleUpdateScope::AnimationData::GetOldStyle(const Element& element) const
{
...
void PostStyleUpdateScope::PseudoData::AddPendingBackdrop(Element& originating_element)
{
...
{
    if (PostStyleUpdateScope::AnimationData* data = PostStyleUpdateScope::CurrentAnimationData()) {
