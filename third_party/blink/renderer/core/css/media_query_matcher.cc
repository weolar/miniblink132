...
MediaQueryList* MediaQueryMatcher::MatchMedia(const String& query)
{
    ...
    // entirely clear.
    if (document_->IsActive() && document_->IsLoadCompleted() && document_->HaveRenderBlockingStylesheetsLoaded() && !document_->View()->DidFirstLayout()
        && !document_->LoadEventStarted() && !document_->IsInMainFrame()) {
        ...
        DCHECK(base::FeatureList::IsEnabled(blink::features::kAvoidForcedLayoutOnInitialEmptyDocumentInSubframe));
        document_->UpdateStyleAndLayout(DocumentUpdateReason::kUnknown);
    }

    MediaQuerySet* media = MediaQuerySet::Create(query, document_->GetExecutionContext());
    return MakeGarbageCollected<MediaQueryList>(document_->GetExecutionContext(), this, media);
}
...
{
    if (media_query_result_flags_.unit_flags & MediaQueryExpValue::UnitFlags::kDynamicViewport) {
        ViewportChanged();
    }
}
