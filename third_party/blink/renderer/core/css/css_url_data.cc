...
CSSUrlData::CSSUrlData(const AtomicString& unresolved_url, const KURL& resolved_url, const Referrer& referrer, OriginClean origin_clean, bool is_ad_related)
...
CSSUrlData::CSSUrlData(const AtomicString& resolved_url)
    : CSSUrlData(resolved_url, KURL(resolved_url), Referrer(), OriginClean::kTrue,
        /*is_ad_related=*/false)
{
}
...
CSSUrlData CSSUrlData::MakeAbsolute() const
{
    ...
    return CSSUrlData(absolute_url_, KURL(absolute_url_), Referrer(), GetOriginClean(), is_ad_related_);
}

CSSUrlData CSSUrlData::MakeResolved(const KURL& base_url, const WTF::TextEncoding& charset) const
{
    if (relative_url_.empty()) {
        return *this;
    }
    const KURL resolved_url = charset.IsValid() ? KURL(base_url, relative_url_, charset) : KURL(base_url, relative_url_);
    if (is_local_) {
        return CSSUrlData(relative_url_, resolved_url, Referrer(), GetOriginClean(), is_ad_related_);
    }
    return CSSUrlData(AtomicString(resolved_url.GetString()), resolved_url, Referrer(), GetOriginClean(), is_ad_related_);
}

CSSUrlData CSSUrlData::MakeWithoutReferrer() const
{
    return CSSUrlData(relative_url_, KURL(absolute_url_), Referrer(), GetOriginClean(), is_ad_related_);
}
