...
void CheckPseudoHasFastRejectFilter::AddElementIdentifierHashes(const Element& element)
{
    DCHECK(filter_.get());
    ...
    for (const auto& attribute_item : attributes) {
        ...
        auto lower = attribute_name.IsLowerASCII() ? attribute_name : attribute_name.LowerASCII();
        filter_->Add(GetAttributeHash(lower));
    }
}

bool CheckPseudoHasFastRejectFilter::FastReject(const Vector<unsigned>& pseudo_has_argument_hashes) const
{
    DCHECK(filter_.get());
...
// static
void CheckPseudoHasFastRejectFilter::CollectPseudoHasArgumentHashes(Vector<unsigned>& pseudo_has_argument_hashes, const CSSSelector* simple_selector)
{
    ...
    case CSSSelector::kClass:
        ...
        pseudo_has_argument_hashes.push_back(GetClassHash(simple_selector->Value()));
        break;
    case CSSSelector::kTag:
        if (simple_selector->TagQName().LocalName() != CSSSelector::UniversalSelectorAtom()) {
            pseudo_has_argument_hashes.push_back(GetTagHash(simple_selector->TagQName().LocalName()));
        }
        break;
