...
StyleRuleImport::StyleRuleImport(
    const String& href, LayerName&& layer, bool supported, String supports_string, const MediaQuerySet* media, OriginClean origin_clean)
...
void StyleRuleImport::NotifyFinished(Resource* resource)
{
    ...
    // sheet.
    const CSSParserContext* parent_context = StrictCSSParserContext(SecureContextMode::kInsecureContext);
    ...
    if (parent_style_sheet_) {
        ...
        if (resource->LoadFailedOrCanceled() && document) {
            AuditsIssue::ReportStylesheetLoadingRequestFailedIssue(document, resource->Url(), resource->LastResourceRequest().GetDevToolsId(),
                parent_style_sheet_->BaseURL(), resource->Options().initiator_info.position.line_, resource->Options().initiator_info.position.column_,
    ...
    // as an ad.
    CSSParserContext* context = MakeGarbageCollected<CSSParserContext>(parent_context, cached_style_sheet->GetResponse().ResponseUrl(),
        cached_style_sheet->GetResponse().IsCorsSameOrigin(),
        Referrer(cached_style_sheet->GetResponse().ResponseUrl(), cached_style_sheet->GetReferrerPolicy()), cached_style_sheet->Encoding(), document);
    if (cached_style_sheet->GetResourceRequest().IsAdResource()) {
        context->SetIsAdRelated();
    }

    style_sheet_ = MakeGarbageCollected<StyleSheetContents>(context, cached_style_sheet->Url(), this);
...
void StyleRuleImport::RequestStyleSheet()
{
    ...
    for (StyleSheetContents* sheet = parent_style_sheet_; sheet; sheet = sheet->ParentStyleSheet()) {
        if (EqualIgnoringFragmentIdentifier(abs_url, sheet->BaseURL())
            || EqualIgnoringFragmentIdentifier(abs_url, document->CompleteURL(sheet->OriginalURL()))) {
            return;
        }
        root_sheet = sheet;
    }
    ...
    if (loading_) {
        ...
        if (parent_style_sheet_ && parent_style_sheet_->LoadCompleted() && root_sheet == parent_style_sheet_) {
