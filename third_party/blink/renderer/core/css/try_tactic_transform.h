...
public:
    ...
    template <typename T> struct LogicalSides {
        T inline_start;
        T inline_end;
        T block_start;
        T block_end;
    ...
    template <typename T> LogicalSides<T> Transform(LogicalSides<T> sides) const
    {
    ...
    template <typename T> LogicalToPhysical<T> Transform(LogicalSides<T> sides, WritingDirectionMode writing_direction) const
    {
        LogicalSides<T> transformed_sides = Transform(sides);
        return LogicalToPhysical<T>(
            writing_direction, transformed_sides.inline_start, transformed_sides.inline_end, transformed_sides.block_start, transformed_sides.block_end);
    }
    ...
    bool FlippedBlock() const
    {
        return bits_ & kBlock;
    }
    bool FlippedInline() const
    {
        return bits_ & kInline;
    }
    bool FlippedStart() const
    {
        return bits_ & kStart;
    }
    ...
    unsigned CacheIndex() const
    {
        return bits_;
    }

private:
    explicit TryTacticTransform(unsigned bits)
        : bits_(bits)
    {
    }
    ...
    void FlipBlock()
    {
        bits_ = bits_ ^ (FlippedStart() ? kInline : kBlock);
    }
    void FlipInline()
    {
        bits_ = bits_ ^ (FlippedStart() ? kBlock : kInline);
    }
    void FlipStart()
    {
        bits_ = bits_ ^ kStart;
