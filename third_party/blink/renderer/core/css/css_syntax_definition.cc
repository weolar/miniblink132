...
std::optional<CSSSyntaxType> ConsumeTypeName(CSSParserTokenStream& stream)
{
    ...
    if (RuntimeEnabledFeatures::CSSAtPropertyStringSyntaxEnabled() && stream.Peek().Value() == "string") {
        stream.Consume();
...
std::optional<std::tuple<CSSSyntaxType, String>> ConsumeSyntaxSingleComponent(CSSParserTokenStream& stream)
{
    if (stream.Peek().GetType() == kDelimiterToken && stream.Peek().Delimiter() == '<') {
    ...
    if (css_parsing_utils::IsCSSWideKeyword(peek.Value()) || css_parsing_utils::IsDefaultKeyword(peek.Value())) {
        return std::nullopt;
    }
    return std::make_tuple(CSSSyntaxType::kIdent, stream.Consume().Value().ToString());
}

std::optional<CSSSyntaxComponent> ConsumeSyntaxComponent(CSSParserTokenStream& stream)
{
    stream.EnsureLookAhead();
    CSSParserSavePoint save_point(stream);

    std::optional<std::tuple<CSSSyntaxType, String>> css_syntax_type_ident = ConsumeSyntaxSingleComponent(stream);
    ...
    if (syntax_type == CSSSyntaxType::kTransformList && repeat != CSSSyntaxRepeat::kNone) {
...
const CSSValue* ConsumeSingleTypeInternal(const CSSSyntaxComponent& syntax, CSSParserTokenStream& stream, const CSSParserContext& context)
{
    switch (syntax.GetType()) {
    case CSSSyntaxType::kIdent:
        if (stream.Peek().GetType() == kIdentToken && stream.Peek().Value() == syntax.GetString()) {
            stream.ConsumeIncludingWhitespace();
            return MakeGarbageCollected<CSSCustomIdentValue>(AtomicString(syntax.GetString()));
        }
        return nullptr;
    ...
    case CSSSyntaxType::kLengthPercentage: {
        ...
        return css_parsing_utils::ConsumeLengthOrPercent(
            stream, context, CSSPrimitiveValue::ValueRange::kAll, css_parsing_utils::UnitlessQuirk::kForbid, kCSSAnchorQueryTypesAll);
    }
    ...
    case CSSSyntaxType::kAngle:
        return css_parsing_utils::ConsumeAngle(stream, context, std::optional<WebFeature>());
...
const CSSValue* ConsumeSingleType(const CSSSyntaxComponent& syntax, CSSParserTokenStream& stream, const CSSParserContext& context)
{
    ...
    if (value) {
        ...
        if (IsAttrTainted(stream.StringRangeAt(offset_before, /* length */ offset_after - offset_before))) {
            value = TaintedCopyIfNeeded(value);
        }
    }
    return value;
}
const CSSValue* ConsumeSyntaxComponent(const CSSSyntaxComponent& syntax, CSSParserTokenStream& stream, const CSSParserContext& context)
{
...
std::optional<CSSSyntaxDefinition> CSSSyntaxDefinition::Consume(CSSParserTokenStream& stream)
{
    if (stream.Peek().GetType() == kDelimiterToken && stream.Peek().Delimiter() == '*') {
        stream.ConsumeIncludingWhitespace();
    ...
    CSSParserSavePoint save_point(stream);
    do {
        std::optional<CSSSyntaxComponent> syntax_component = ConsumeSyntaxComponent(stream);
...
const CSSValue* CSSSyntaxDefinition::Parse(StringView text, const CSSParserContext& context, bool is_animation_tainted) const
{
    if (IsUniversal()) {
    ...
    for (const CSSSyntaxComponent& component : syntax_components_) {
        ...
        stream.ConsumeWhitespace();
        if (const CSSValue* result = ConsumeSyntaxComponent(component, stream, context)) {
            return result;
        }
    }
    return nullptr;
...
CSSSyntaxDefinition CSSSyntaxDefinition::IsolatedCopy() const
{
    ...
    for (const auto& syntax_component : syntax_components_) {
        syntax_components_copy.push_back(CSSSyntaxComponent(syntax_component.GetType(), syntax_component.GetString(), syntax_component.GetRepeat()));
    }
...
CSSSyntaxDefinition CSSSyntaxDefinition::CreateUniversal()
{
    ...
    components.push_back(CSSSyntaxComponent(CSSSyntaxType::kTokenStream, g_empty_string, CSSSyntaxRepeat::kNone));
