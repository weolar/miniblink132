...
bool FontFaceSetWorker::ResolveFontStyle(const String& font_string, Font& font)
{
    ...
    FontDescription description = FontStyleResolver::ComputeFont(*parsed_style, GetWorker()->GetFontSelector());
...
{
    FontFaceSetWorker* fonts = Supplement<WorkerGlobalScope>::From<FontFaceSetWorker>(worker);
    if (!fonts) {
