...
public:
    ...
    MatchedRule(const RuleData* rule_data, uint16_t layer_order, unsigned proximity, unsigned style_sheet_index)
        : rule_data_(rule_data)
        , sort_key_(
              (static_cast<uint64_t>(layer_order) << 48) | (static_cast<uint64_t>(GetRuleData()->Specificity()) << 16) | (65535 - ClampTo<uint16_t>(proximity)))
        , position_((static_cast<uint64_t>(style_sheet_index) << kBitsForPositionInRuleData) + rule_data->GetPosition())
    {
    }

private:
    const RuleData* GetRuleData() const
    {
        return rule_data_;
    }
    uint16_t LayerOrder() const
    {
        return sort_key_ >> 48;
    }
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(ElementRuleCollectorTest, RuleNotStartingWithAmpersand);
...
public:
    ElementRuleCollector(const ElementResolveContext&, const StyleRecalcContext&, const SelectorFilter&, MatchResult&, EInsideLink);
    ...
    void SetMatchingRulesFromNoStyleSheet(bool matching_rules_from_no_style_sheet)
    {
    ...
    void CollectMatchingPartPseudoRules(const MatchRequest&, PartNames*, bool for_shadow_pseudo);
    void SortAndTransferMatchedRules(CascadeOrigin origin, bool is_vtt_embedded_style, StyleRuleUsageTracker* tracker);
    ...
    void AddElementStyleProperties(const CSSPropertyValueSet*, CascadeOrigin, bool is_cacheable = true, bool is_inline_style = false);
    ...
    PseudoId GetPseudoId() const
    {
        return pseudo_style_request_.pseudo_id;
    }
    ...
    public:
        SlottedRulesScope(ElementRuleCollector& collector, HTMLSlotElement& slot)
            : context_(&collector.style_recalc_context_, collector.style_recalc_context_.ForSlottedRules(slot))
        {
        }
    ...
    public:
        PartRulesScope(ElementRuleCollector& collector, Element& host)
            : context_(&collector.style_recalc_context_, collector.style_recalc_context_.ForPartRules(host))
        {
        }
...
private:
    ...
    template <bool stop_at_first_match> bool CollectMatchingRulesInternal(const MatchRequest&, PartNames* part_names);

    template <bool stop_at_first_match, bool perf_trace_enabled>
    bool CollectMatchingRulesForListInternal(base::span<const RuleData>, const MatchRequest&, const RuleSet*, int, const SelectorChecker&,
        SelectorChecker::SelectorCheckingContext&, PartRequest* = nullptr);

    template <bool stop_at_first_match>
    bool CollectMatchingRulesForList(base::span<const RuleData>, const MatchRequest&, const RuleSet*, int, const SelectorChecker&,
        SelectorChecker::SelectorCheckingContext&, PartRequest* = nullptr);

    bool Match(SelectorChecker&, const SelectorChecker::SelectorCheckingContext&, MatchResult&);
    void DidMatchRule(
        const RuleData*, uint16_t layer_order, const ContainerQuery*, unsigned proximity, const SelectorChecker::MatchResult&, int style_sheet_index);

    void AppendCSSOMWrapperForRule(const TreeScope* tree_scope_containing_rule, const RuleData*, wtf_size_t);
...
private:
    ...
    bool matching_rules_from_no_style_sheet_ = false; // Document rules and watched selectors.
