...
class CORE_EXPORT StyleEngine final : public GarbageCollected<StyleEngine>, public FontSelectorClient, public NameClient {
public:
    ...
    public:
        explicit AttachScrollMarkersScope(StyleEngine& engine)
            : in_scroll_markers_attachment_scope_(&engine.in_scroll_markers_attachment_, true)
        {
        }
    ...
    public:
        explicit AllowMarkForReattachFromRebuildLayoutTreeScope(StyleEngine& engine)
            : allow_marking_(&engine.allow_mark_for_reattach_from_rebuild_layout_tree_, true)
        {
        }
    ...
    public:
        explicit SkipStyleRecalcScope(StyleEngine& engine)
            : allow_skip_(&engine.allow_skip_style_recalc_, engine.AllowSkipStyleRecalcForScope())
        {
        }
    ...
    const HeapVector<std::pair<StyleSheetKey, Member<CSSStyleSheet>>>& InjectedAuthorStyleSheets() const
    {
    ...
    void SetOwnerColorScheme(mojom::blink::ColorScheme color_scheme, mojom::blink::PreferredColorScheme preferred_color_scheme);
    mojom::blink::ColorScheme GetOwnerColorScheme() const
    {
        return owner_color_scheme_;
    }
    mojom::blink::PreferredColorScheme ResolveColorSchemeForEmbedding(const ComputedStyle* embedder_style) const;
    void ViewportStyleSettingChanged();

    void InjectSheet(const StyleSheetKey&, StyleSheetContents*, WebCssOrigin = WebCssOrigin::kAuthor);
    void RemoveInjectedSheet(const StyleSheetKey&, WebCssOrigin = WebCssOrigin::kAuthor);
    ...
    void RemovePendingBlockingSheet(Node& style_sheet_candidate_node, PendingSheetType type);
    ...
    {
        return GetRuleFeatureSet().GetRuleInvalidationData().MaxDirectAdjacentSelectors();
    }
    ...
    {
        return GetRuleFeatureSet().GetRuleInvalidationData().UsesWindowInactiveSelector();
    }

    // Set when we recalc the style of any element that depends on layout.
    void SetStyleAffectedByLayout()
    {
        style_affected_by_layout_ = true;
    }
    bool StyleAffectedByLayout()
    {
        return style_affected_by_layout_;
    }

    bool StyleMaybeAffectedByLayout(const Element&);

    bool SkippedContainerRecalc() const
    {
        return skipped_container_recalc_ != 0;
    }
    void IncrementSkippedContainerRecalc()
    {
        ++skipped_container_recalc_;
    }
    void DecrementSkippedContainerRecalc()
    {
        --skipped_container_recalc_;
    }

    bool UsesLineHeightUnits() const
    {
        return uses_line_height_units_;
    }
    ...
    bool UsesGlyphRelativeUnits() const
    {
        return uses_glyph_relative_units_;
    }
    ...
    bool UpdateRootFontRelativeUnits(const ComputedStyle* old_root_style, const ComputedStyle* new_root_style);

    void ResetCSSFeatureFlags(const RuleFeatureSet&);

    bool CountersChanged() const
    {
        return counters_changed_;
    }
    void MarkCountersDirty()
    {
        counters_changed_ = true;
    }
    void MarkCountersClean()
    {
    ...
    Font ComputeFont(Element& element, const ComputedStyle& font_style, const CSSPropertyValueSet& font_properties);
    ...
    bool HasViewportDependentMediaQueries()
    {
        DCHECK(global_rule_set_);
        UpdateActiveStyle();
        return global_rule_set_->GetRuleFeatureSet().HasViewportDependentMediaQueries();
    }
    ...
    bool InApplyAnimationUpdate() const
    {
        return in_apply_animation_update_;
    }
    ...
    bool InEnsureComputedStyle() const
    {
        return in_ensure_computed_style_;
    }
    ...
    bool SkipStyleRecalcAllowed() const
    {
        return allow_skip_style_recalc_;
    }

    CSSFontSelector* GetFontSelector()
    {
        return font_selector_.Get();
    }
    ...
    CSSStyleSheet* CreateSheet(
        Element&, const String& text, WTF::TextPosition start_position, PendingSheetType type, RenderBlockingBehavior render_blocking_behavior);
    ...
    void ClassChangedForElement(const SpaceSplitString& changed_classes, Element&);
    void ClassChangedForElement(const SpaceSplitString& old_classes, const SpaceSplitString& new_classes, Element&);
    void AttributeChangedForElement(const QualifiedName& attribute_name, Element&);
    void IdChangedForElement(const AtomicString& old_id, const AtomicString& new_id, Element&);
    void PseudoStateChangedForElement(CSSSelector::PseudoType, Element&, bool invalidate_descendants_or_siblings, bool invalidate_ancestors_or_siblings);
    void PartChangedForElement(Element&);
    void ExportpartsChangedForElement(Element&);

    void ScheduleSiblingInvalidationsForElement(Element&, ContainerNode& scheduling_parent, unsigned min_direct_adjacent);
    void ScheduleInvalidationsForInsertedSibling(Element* before_element, Element& inserted_element);
    void ScheduleInvalidationsForRemovedSibling(Element* before_element, Element& removed_element, Element& after_element);
    void ScheduleNthPseudoInvalidations(ContainerNode&);
    void ApplyRuleSetInvalidationForTreeScope(TreeScope&, ContainerNode&, SelectorFilter&, StyleScopeFrame&, const HeapHashSet<Member<RuleSet>>&,
        unsigned changed_rule_flags, InvalidationScope = kInvalidateCurrentScope);
    void ApplyRuleSetInvalidationForSubtree(TreeScope&, Element&, SelectorFilter&, StyleScopeFrame& parent_style_scope_frame,
        const HeapHashSet<Member<RuleSet>>&, unsigned changed_rule_flags, InvalidationScope, bool invalidate_slotted, bool invalidate_part);
    void ScheduleCustomElementInvalidations(HashSet<AtomicString> tag_names);
    void ScheduleInvalidationsForHasPseudoAffectedByInsertionOrRemoval(ContainerNode* parent, Node* node_before_change, Element& changed_element, bool removal);
    ...
    unsigned StyleForElementCount() const
    {
        return style_for_element_count_;
    }
    void IncStyleForElementCount()
    {
        style_for_element_count_++;
    }

    StyleResolverStats* Stats()
    {
        return style_resolver_stats_.get();
    }
    void SetStatsEnabled(bool);

    void ApplyRuleSetChanges(TreeScope&, const ActiveStyleSheetVector& old_style_sheets, const ActiveStyleSheetVector& new_style_sheets,
        const HeapVector<Member<RuleSetDiff>>& diffs);
    void ApplyUserRuleSetChanges(const ActiveStyleSheetVector& old_style_sheets, const ActiveStyleSheetVector& new_style_sheets);

    void VisionDeficiencyChanged();
    void ApplyVisionDeficiencyStyle(ComputedStyleBuilder& layout_view_style_builder);
    ...
    bool IsViewportStyleDirty() const
    {
        return viewport_style_dirty_;
    }
    ...
    void MarkPositionTryStylesDirty(const HeapHashSet<Member<RuleSet>>& changed_rule_sets);
    ...
    StyleRuleKeyframes* KeyframeStylesForAnimation(const AtomicString& animation_name);

    StyleRuleFontPaletteValues* FontPaletteValuesForNameAndFamily(AtomicString palette_name, AtomicString font_family);

    CounterStyleMap* GetUserCounterStyleMap()
    {
        return user_counter_style_map_.Get();
    }
    const CounterStyle& FindCounterStyleAcrossScopes(const AtomicString&, const TreeScope*) const;
    ...
    {
        return style_recalc_root_.GetRootNode();
    }
    ...
    void UpdateStyleAndLayoutTreeForContainer(Element& container, const LogicalSize&, LogicalAxes contained_axes);
    ...
    void UpdateStyleForOutOfFlow(Element& element, const CSSPropertyValueSet* try_set, const TryTacticList&, AnchorEvaluator*);
    ...
    bool InRebuildLayoutTree() const
    {
        return in_layout_tree_rebuild_;
    }
    bool InDOMRemoval() const
    {
        return in_dom_removal_;
    }
    bool InDetachLayoutTree() const
    {
        return in_detach_scope_;
    }
...
#if DCHECK_IS_ON()
        DCHECK(!in_dom_removal_ || in_detach_scope_) << "A DOMRemovalScope must use a DetachLayoutTreeScope to handle "
    ...
    ColorSchemeFlags GetPageColorSchemes() const
    {
    ...
    ForcedColors GetForcedColors() const
    {
        return forced_colors_;
    }
    void UpdateColorSchemeBackground(bool color_scheme_changed = false);
    Color ForcedBackgroundColor() const
    {
        return forced_background_color_;
    }
    ...
    {
        return style_image_cache_.CacheImageContent(GetDocument().Fetcher(), params);
    }

    void AddCachedFillOrClipPathURIValue(const AtomicString& string, const CSSValue& value);
    ...
    {
        DCHECK(viewport_size_ == CSSToLengthConversionData::ViewportSize(GetDocument().GetLayoutView()));
...
private:
    void UpdateCounters(const Element& element, CountersAttachmentContext& context);
    void UpdateLayoutCounters(const LayoutObject& layout_object, CountersAttachmentContext& context);
    ...
    {
        return tree_scopes_removed_ || document_scope_dirty_ || dirty_tree_scopes_.size() || user_style_dirty_;
    }
    ...
    void RevisitStyleRulesForInspector(const RuleFeatureSet& features, const HeapVector<Member<StyleRuleBase>>& rules);

    typedef HeapHashSet<Member<TreeScope>> UnorderedTreeScopeSet;

    bool MediaQueryAffectingValueChanged(const ActiveStyleSheetVector&, MediaValueChange);
    void MediaQueryAffectingValueChanged(TreeScope&, MediaValueChange);
    void MediaQueryAffectingValueChanged(UnorderedTreeScopeSet&, MediaValueChange);
    void MediaQueryAffectingValueChanged(HeapHashSet<Member<TextTrack>>&, MediaValueChange);
    ...
    CSSStyleSheet* ParseSheet(Element&, const String& text, WTF::TextPosition start_position, RenderBlockingBehavior render_blocking_behavior);
    ...
    void UpdateActiveStyleSheetsInShadow(TreeScope*, UnorderedTreeScopeSet& tree_scopes_removed);

    bool ShouldSkipInvalidationFor(const Element&) const;
    bool IsSubtreeAndSiblingsStyleDirty(const Element&) const;
    void ApplyRuleSetInvalidationForElement(const TreeScope& tree_scope, Element& element, SelectorFilter& selector_filter, StyleScopeFrame& style_scope_frame,
        const HeapHashSet<Member<RuleSet>>& rule_sets, unsigned changed_rule_flags, bool is_shadow_host);
    void InvalidateSlottedElements(HTMLSlotElement&, const StyleChangeReasonForTracing&);
    ...
    bool ClearFontFaceCacheAndAddUserFonts(const ActiveStyleSheetVector& user_sheets);
    ...
    void AddPropertyRulesFromSheets(AtRuleCascadeMap&, const ActiveStyleSheetVector&, bool is_user_style);
    void AddFontPaletteValuesRulesFromSheets(const ActiveStyleSheetVector& sheets);
    ...
    bool UserKeyframeStyleShouldOverride(const StyleRuleKeyframes* new_rule, const StyleRuleKeyframes* existing_rule) const;
    ...
    void RecalcPositionTryStyleForPseudoElement(PseudoElement& pseudo_element, const StyleRecalcChange, const StyleRecalcContext&);
    ...
    void RebuildLayoutTreeForTraversalRootAncestors(Element* parent, Element* container_parent);
    ...
    inline void InvalidateElementAffectedByHas(Element&, bool for_element_affected_by_pseudo_in_has);
    class PseudoHasInvalidationTraversalContext;
    inline void InvalidateAncestorsOrSiblingsAffectedByHas(const PseudoHasInvalidationTraversalContext&);
    // Invalidate changed element affected by logical combinations in :has()
    inline void InvalidateChangedElementAffectedByLogicalCombinationsInHas(Element& changed_element, bool for_element_affected_by_pseudo_in_has);
    ...
    HeapVector<std::pair<StyleSheetKey, Member<CSSStyleSheet>>> injected_user_style_sheets_;
    HeapVector<std::pair<StyleSheetKey, Member<CSSStyleSheet>>> injected_author_style_sheets_;

    ActiveStyleSheetVector active_user_style_sheets_;

    using KeyframesRuleMap = HeapHashMap<AtomicString, Member<StyleRuleKeyframes>>;
    ...
    using FontPaletteValuesRuleMap = HeapHashMap<std::pair<AtomicString, String>, Member<StyleRuleFontPaletteValues>>;
    ...
    ColorSchemeFlags page_color_schemes_ = static_cast<ColorSchemeFlags>(ColorSchemeFlag::kNormal);
    ...
    // for more info.
    HeapHashMap<AtomicString, WeakMember<const CSSValue>> fill_or_clip_path_uri_value_cache_;
