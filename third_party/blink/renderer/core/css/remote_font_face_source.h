...
public:
    enum Phase : uint8_t { kNoLimitExceeded, kShortLimitExceeded, kLongLimitExceeded };

    RemoteFontFaceSource(CSSFontFace*, FontSelector*, FontDisplay, scoped_refptr<base::SingleThreadTaskRunner>);
    ...
    {
        return "RemoteFontFaceSource";
    }

    bool IsInBlockPeriod() const override
    {
        return period_ == kBlockPeriod;
    }
    bool IsInFailurePeriod() const override
    {
        return period_ == kFailurePeriod;
    }
    ...
    // For UMA reporting
    bool HadBlankText() override
    {
        return histograms_.HadBlankText();
    }
...
private:
    ...
    enum DisplayPeriod : uint8_t { kBlockPeriod, kSwapPeriod, kFailurePeriod, kNotApplicablePeriod };
    ...
    public:
        ...
        enum DataSource { kFromUnknown, kFromDataURL, kFromMemoryCache, kFromDiskCache, kFromNetwork };
        ...
        bool HadBlankText()
        {
            return !blank_paint_time_.is_null();
        }
