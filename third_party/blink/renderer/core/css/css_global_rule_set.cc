...
void CSSGlobalRuleSet::InitWatchedSelectorsRuleSet(Document& document)
{
    ...
    const HeapVector<Member<StyleRule>>& watched_selectors = watch->WatchedCallbackSelectors();
    ...
    for (unsigned i = 0; i < watched_selectors.size(); ++i) {
        watched_selectors_rule_set_->AddStyleRule(watched_selectors[i], /*parent_rule=*/nullptr, *medium, kRuleHasNoSpecialState, /*within_mixin=*/false);
    }
}
...
void CSSGlobalRuleSet::UpdateDocumentRulesSelectorsRuleSet(Document& document)
{
    ...
    const HeapVector<Member<StyleRule>>& document_rules_selectors = DocumentSpeculationRules::From(document).selectors();
    ...
    for (StyleRule* selector : document_rules_selectors) {
        document_rules_selectors_rule_set_->AddStyleRule(selector, /*parent_rule=*/nullptr, *medium, kRuleHasNoSpecialState,
            /*within_mixin=*/false);
    }
}
