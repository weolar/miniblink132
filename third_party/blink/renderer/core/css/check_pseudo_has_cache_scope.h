...
constexpr CheckPseudoHasResult kCheckPseudoHasResultAllDescendantsOrNextSiblingsChecked = 1 << 2;
constexpr CheckPseudoHasResult kCheckPseudoHasResultSomeChildrenChecked = 1 << 3;
...
using ElementCheckPseudoHasResultMap = HeapHashMap<Member<const Element>, CheckPseudoHasResult>;
using CheckPseudoHasResultCache = HeapHashMap<String, Member<ElementCheckPseudoHasResultMap>>;
...
using ElementCheckPseudoHasFastRejectFilterMap = HeapHashMap<Member<const Element>, std::unique_ptr<CheckPseudoHasFastRejectFilter>>;
...
public:
    ...
    public:
        ...
        void SetAllTraversedElementsAsChecked(Element* last_traversed_element, int last_traversed_depth);

        CheckPseudoHasResult GetResult(Element*) const;

        bool AlreadyChecked(Element*) const;

        CheckPseudoHasFastRejectFilter& EnsureFastRejectFilter(Element*, bool& is_new_entry);

        inline bool CacheAllowed() const
        {
            return cache_allowed_;
        }

    private:
        friend class CheckPseudoHasCacheScopeContextTest;

        CheckPseudoHasResult SetResultAndGetOld(Element*, CheckPseudoHasResult result);

        void SetTraversedElementAsChecked(Element* traversed_element, Element* parent);
...
private:
    static ElementCheckPseudoHasResultMap& GetResultMap(const Document*, const CSSSelector*);
    static ElementCheckPseudoHasFastRejectFilterMap& GetFastRejectFilterMap(const Document*, CheckPseudoHasArgumentTraversalType);

    CheckPseudoHasResultCache& GetResultCache()
    {
