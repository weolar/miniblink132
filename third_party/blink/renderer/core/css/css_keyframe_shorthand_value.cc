...
#if DCHECK_IS_ON()
namespace {
bool ShorthandMatches(CSSPropertyID expected_shorthand, CSSPropertyID longhand)
{
...
CSSKeyframeShorthandValue::CSSKeyframeShorthandValue(CSSPropertyID shorthand, ImmutableCSSPropertyValueSet* properties)
...
String CSSKeyframeShorthandValue::CustomCSSText() const
{
    ...
    for (unsigned i = 0; i < properties_->PropertyCount(); i++) {
        DCHECK(ShorthandMatches(shorthand_, properties_->PropertyAt(i).Id())) << "These are not the longhands you're looking for.";
    }
#endif
...
void CSSKeyframeShorthandValue::TraceAfterDispatch(blink::Visitor* visitor) const
{
