...
TEST(CSSSelector, Representations)
{
    ...
    const char* css_rules = "summary::-webkit-details-marker { }"
                            "* {}"
                            "div {}"
                            "#id {}"
                            ".class {}"
...
TEST(CSSSelector, Specificity_Is)
{
    ...
    EXPECT_EQ(Specificity(":is(.c + .c + .c, .b + .c:not(span), .b + .c + .e)"), Specificity(".c + .c + .c"));
}
...
TEST(CSSSelector, Specificity_Where)
{
    ...
    EXPECT_EQ(Specificity(":where(.c + .c + .c, .b + .c:not(span), .b + .c + .e)"), Specificity("*"));
}
...
TEST(CSSSelector, Specificity_HostContext)
{
    ...
    EXPECT_EQ(Specificity(":host-context(div#a.b)"), Specificity(".host-context div#a.b"));
}
...
TEST(CSSSelector, Specificity_Not)
{
    ...
    EXPECT_EQ(Specificity(".a :not(.e+.f, .g>.b, .h#i)"), Specificity(".a :is(.e+.f, .g>.b, .h#i)"));
    EXPECT_EQ(Specificity(":not(.c + .c + .c, .b + .c:not(span), .b + .c + .e)"), Specificity(":is(.c + .c + .c, .b + .c:not(span), .b + .c + .e)"));
}
...
TEST(CSSSelector, Specificity_Has)
{
    ...
    EXPECT_EQ(Specificity("div > :has(div, div:where(span:where(.b ~ .c)))"), Specificity("div > div"));
    EXPECT_EQ(Specificity(":has(.c + .c + .c, .b + .c:not(span), .b + .c + .e)"), Specificity(".c + .c + .c"));
}
...
TEST(CSSSelector, CueDefaultNamespace)
{
    ...
    const CSSSelector& cue_selector = sheet.GetRuleSet().CuePseudoRules()[0].Selector();
...
{
    test::TaskEnvironment task_environment;
    CSSSelector selector[2] = { CSSSelector(), CSSSelector(AtomicString("hover"), /* is_implicit */ false) };
    selector[0].SetTrue();
...
{
    test::TaskEnvironment task_environment;
    CSSSelector selector[2] = { CSSSelector(AtomicString("hover"), /* is_implicit */ false), CSSSelector() };
    selector[0].SetRelation(CSSSelector::kSubSelector);
    selector[1].SetTrue();
    selector[1].SetLastInComplexSelector(true);
    EXPECT_EQ(":hover", selector[0].SelectorText());
...
TEST(CSSSelector, ExplicitScopeSpecificity)
{
    ...
    selector[1].SetLastInComplexSelector(true);
    EXPECT_EQ(":scope > div", selector[0].SelectorText());
    EXPECT_EQ(CSSSelector::kTagSpecificity | CSSSelector::kClassLikeSpecificity, selector[0].Specificity());
}
...
TEST(CSSSelector, CheckSelectorTextExpandingPseudoParent)
{
    ...
    css_test_helpers::TestStyleSheet sheet;
    sheet.AddCSSRules(".a { .b { .c, &.c, .c:has(&) {} } }"
                      ".d .e { .f:has(> &) {} }");
    ...
    EXPECT_EQ(".c:has(:is(:is(.a) .b))", selector->SelectorTextExpandingPseudoParent());
    ...
    EXPECT_EQ(".f:has(> :is(.d .e))", selector->SelectorTextExpandingPseudoParent());
}
...
TEST(CSSSelector, CheckHasArgumentMatchInShadowTreeFlag)
{
    ...
    selector = &rules[1].Selector();
    EXPECT_EQ(":host:has(.a):host(.b):has(.c):host-context(.d):has(.e) :has(.f)", selector->SelectorText());
    EXPECT_EQ(selector->GetPseudoType(), CSSSelector::kPseudoHas);
    EXPECT_FALSE(selector->HasArgumentMatchInShadowTree());
    selector = selector->NextSimpleSelector();
    selector = selector->NextSimpleSelector();
