...
bool NameMatches(const ComputedStyle& style, const ContainerSelector& container_selector, const TreeScope* selector_tree_scope)
{
    ...
    if (const ScopedCSSNameList* container_name = style.ContainerName()) {
        const HeapVector<Member<const ScopedCSSName>>& names = container_name->GetNames();
        for (auto scoped_name : names) {
            if (scoped_name->GetName() == name) {
                ...
                // rule's tree scope.
                for (const TreeScope* match_scope = selector_tree_scope; match_scope; match_scope = match_scope->ParentTreeScope()) {
...
bool TypeMatches(const ComputedStyle& style, const ContainerSelector& container_selector)
{
...
bool Matches(const ComputedStyle& style, const ContainerSelector& container_selector, const TreeScope* selector_tree_scope)
{
    return TypeMatches(style, container_selector) && NameMatches(style, container_selector, selector_tree_scope);
}

Element* CachedContainer(Element* starting_element, const ContainerSelector& container_selector, const TreeScope* selector_tree_scope,
    ContainerSelectorCache& container_selector_cache)
{
    auto it = container_selector_cache.Find<ScopedContainerSelectorHashTranslator>(ScopedContainerSelector(container_selector, selector_tree_scope));
    if (it != container_selector_cache.end()) {
        return it->value.Get();
    }
    Element* container = ContainerQueryEvaluator::FindContainer(starting_element, container_selector, selector_tree_scope);
    container_selector_cache.insert(MakeGarbageCollected<ScopedContainerSelector>(container_selector, selector_tree_scope), container);
    return container;
}

PaintLayerScrollableArea* FindScrollContainerScrollableArea(const Element& container)
{
    if (const LayoutObject* layout_object = container.GetLayoutObject()) {
        if (const LayoutBox* snap_container = layout_object->ContainingScrollContainer()) {
...
{
    if (PaintLayerScrollableArea* scrollable_area = FindScrollContainerScrollableArea(container)) {
        if (SnappedQueryScrollSnapshot* snapshot = scrollable_area->GetSnappedQueryScrollSnapshot()) {
            ContainerSnappedFlags snapped = static_cast<ContainerSnappedFlags>(ContainerSnapped::kNone);
    ...
    auto* query_values = MakeGarbageCollected<CSSContainerValues>(container.GetDocument(), container, std::nullopt, std::nullopt, ContainerStuckPhysical::kNo,
        ContainerStuckPhysical::kNo, snapped_, static_cast<ContainerOverflowingFlags>(ContainerOverflowing::kNone),
        static_cast<ContainerOverflowingFlags>(ContainerOverflowing::kNone));
    media_query_evaluator_ = MakeGarbageCollected<MediaQueryEvaluator>(query_values);
}

// static
Element* ContainerQueryEvaluator::ParentContainerCandidateElement(Element& element)
{
...
// static
Element* ContainerQueryEvaluator::FindContainer(Element* starting_element, const ContainerSelector& container_selector, const TreeScope* selector_tree_scope)
{
    // TODO(crbug.com/1213888): Cache results.
    for (Element* element = starting_element; element; element = ParentContainerCandidateElement(*element)) {
...
bool ContainerQueryEvaluator::EvalAndAdd(Element* style_container_candidate, const StyleRecalcContext& context, const ContainerQuery& query,
    ContainerSelectorCache& container_selector_cache, MatchResult& match_result)
{
    ...
    Element* starting_element = selects_size ? context.container : style_container_candidate;
    if (Element* container = CachedContainer(starting_element, query.Selector(), match_result.CurrentTreeScope(), container_selector_cache)) {
        Change change = starting_element == container ? Change::kNearestContainer : Change::kDescendantContainers;
        return container->EnsureContainerQueryEvaluator().EvalAndAdd(query, change, match_result);
    }
    return false;
}
...
ContainerQueryEvaluator::Result ContainerQueryEvaluator::Eval(const ContainerQuery& container_query) const
{
    CHECK(media_query_evaluator_);
    ...
    bool value = (media_query_evaluator_->Eval(*container_query.query_, &result_flags) == KleeneValue::kTrue);
...
bool ContainerQueryEvaluator::EvalAndAdd(const ContainerQuery& query, Change change, MatchResult& match_result)
{
    HeapHashMap<Member<const ContainerQuery>, Result>::AddResult entry = results_.insert(&query, Result());
    ...
    bool use_cached = (result.unit_flags
                          & (MediaQueryExpValue::UnitFlags::kRootFontRelative | MediaQueryExpValue::UnitFlags::kDynamicViewport
                              | MediaQueryExpValue::UnitFlags::kStaticViewport | MediaQueryExpValue::UnitFlags::kContainer))
        == 0;
    ...
    if (result.unit_flags & (MediaQueryExpValue::UnitFlags::kStaticViewport | MediaQueryExpValue::UnitFlags::kContainer)) {
    ...
    if (!depends_on_stuck_) {
        ...
        if (depends_on_stuck_ && !scroll_state_snapshot_) {
            ...
            scroll_state_snapshot_ = MakeGarbageCollected<ScrollStateQuerySnapshot>(*container_element);
        }
    }
    if (!depends_on_snapped_) {
        depends_on_snapped_ = query.Selector().SelectsSnapContainers();
        if (depends_on_snapped_) {
            if (PaintLayerScrollableArea* scrollable_area = FindScrollContainerScrollableArea(*ContainerElement())) {
...
ContainerQueryEvaluator::Change ContainerQueryEvaluator::SizeContainerChanged(PhysicalSize size, PhysicalAxes contained_axes)
{
...
void ContainerQueryEvaluator::SetPendingSnappedStateFromScrollSnapshot(const SnappedQueryScrollSnapshot& snapshot)
{
    Element* container = ContainerElement();
    pending_snapped_ = static_cast<ContainerSnappedFlags>(ContainerSnapped::kNone);
    if (snapshot.GetSnappedTargetX() == container) {
        pending_snapped_ |= static_cast<ContainerSnappedFlags>(ContainerSnapped::kX);
    }
    if (snapshot.GetSnappedTargetY() == container) {
        pending_snapped_ |= static_cast<ContainerSnappedFlags>(ContainerSnapped::kY);
    }
    ...
    if (pending_snapped_ != snapped_) {
        ...
        container->SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kScrollTimeline));
    }
}
...
ContainerQueryEvaluator::Change ContainerQueryEvaluator::ApplyScrollState()
{
    ...
    if (scroll_state_snapshot_) {
        change = StickyContainerChanged(scroll_state_snapshot_->StuckHorizontal(), scroll_state_snapshot_->StuckVertical());
        Change overflow_change = OverflowContainerChanged(scroll_state_snapshot_->OverflowingHorizontal(), scroll_state_snapshot_->OverflowingVertical());
...
ContainerQueryEvaluator::Change ContainerQueryEvaluator::StickyContainerChanged(ContainerStuckPhysical stuck_horizontal, ContainerStuckPhysical stuck_vertical)
{
    if (stuck_horizontal_ == stuck_horizontal && stuck_vertical_ == stuck_vertical) {
        return Change::kNone;
    }
...
ContainerQueryEvaluator::Change ContainerQueryEvaluator::SnapContainerChanged(ContainerSnappedFlags snapped)
{
    if (snapped_ == snapped) {
        return Change::kNone;
    }
...
ContainerQueryEvaluator::Change ContainerQueryEvaluator::OverflowContainerChanged(
    ContainerOverflowingFlags overflowing_horizontal, ContainerOverflowingFlags overflowing_vertical)
{
    if (overflowing_horizontal_ == overflowing_horizontal && overflowing_vertical_ == overflowing_vertical) {
        return Change::kNone;
    }
...
ContainerQueryEvaluator::Change ContainerQueryEvaluator::StyleAffectingSizeChanged()
{
    Change change = ComputeSizeChange();
    if (change != Change::kNone) {
        ClearResults(change, kSizeContainer);
...
ContainerQueryEvaluator::Change ContainerQueryEvaluator::StyleAffectingScrollStateChanged()
{
...
void ContainerQueryEvaluator::UpdateContainerValues()
{
    ...
    auto* query_values = MakeGarbageCollected<CSSContainerValues>(container->GetDocument(), *container, existing_values.Width(), existing_values.Height(),
        existing_values.StuckHorizontal(), existing_values.StuckVertical(), existing_values.SnappedFlags(), existing_values.OverflowingHorizontal(),
...
void ContainerQueryEvaluator::UpdateContainerSize(PhysicalSize size, PhysicalAxes contained_axes)
{
    size_ = size;
    ...
    PhysicalAxes supported_axes = ContainerTypeAxes(container->ComputedStyleRef()) & contained_axes;

    if ((supported_axes & PhysicalAxes(kPhysicalAxesHorizontal)) != PhysicalAxes(kPhysicalAxesNone)) {
        width = size.width.ToDouble();
    }

    if ((supported_axes & PhysicalAxes(kPhysicalAxesVertical)) != PhysicalAxes(kPhysicalAxesNone)) {
        height = size.height.ToDouble();
    }

    auto* query_values = MakeGarbageCollected<CSSContainerValues>(container->GetDocument(), *container, width, height, existing_values.StuckHorizontal(),
        existing_values.StuckVertical(), existing_values.SnappedFlags(), existing_values.OverflowingHorizontal(), existing_values.OverflowingVertical());
    media_query_evaluator_ = MakeGarbageCollected<MediaQueryEvaluator>(query_values);
}

void ContainerQueryEvaluator::UpdateContainerStuck(ContainerStuckPhysical stuck_horizontal, ContainerStuckPhysical stuck_vertical)
{
    ...
    auto* query_values = MakeGarbageCollected<CSSContainerValues>(container->GetDocument(), *container, existing_values.Width(), existing_values.Height(),
        stuck_horizontal, stuck_vertical, existing_values.SnappedFlags(), existing_values.OverflowingHorizontal(), existing_values.OverflowingVertical());
    media_query_evaluator_ = MakeGarbageCollected<MediaQueryEvaluator>(query_values);
}

void ContainerQueryEvaluator::UpdateContainerSnapped(ContainerSnappedFlags snapped)
{
    snapped_ = snapped;
    ...
    auto* query_values = MakeGarbageCollected<CSSContainerValues>(container->GetDocument(), *container, existing_values.Width(), existing_values.Height(),
        existing_values.StuckHorizontal(), existing_values.StuckVertical(), snapped, existing_values.OverflowingHorizontal(),
        existing_values.OverflowingVertical());
    media_query_evaluator_ = MakeGarbageCollected<MediaQueryEvaluator>(query_values);
}

void ContainerQueryEvaluator::UpdateContainerOverflowing(ContainerOverflowingFlags overflowing_horizontal, ContainerOverflowingFlags overflowing_vertical)
{
    ...
    auto* query_values = MakeGarbageCollected<CSSContainerValues>(container->GetDocument(), *container, existing_values.Width(), existing_values.Height(),
        existing_values.StuckHorizontal(), existing_values.StuckVertical(), existing_values.Snapped(), overflowing_horizontal, overflowing_vertical);
    media_query_evaluator_ = MakeGarbageCollected<MediaQueryEvaluator>(query_values);
}

void ContainerQueryEvaluator::ClearResults(Change change, ContainerType container_type)
{
    if (change == Change::kNone) {
        return;
    }
    ...
    for (const auto& pair : results_) {
        if (pair.value.change <= change
            && ((container_type == kSizeContainer && pair.key->Selector().SelectsSizeContainers())
                || (container_type == kStickyContainer && pair.key->Selector().SelectsStickyContainers())
                || (container_type == kSnapContainer && pair.key->Selector().SelectsSnapContainers())
                || (container_type == kOverflowContainer && pair.key->Selector().SelectsOverflowContainers())
                || (container_type == kStyleContainer && pair.key->Selector().SelectsStyleContainers()))) {
            continue;
        }
...
ContainerQueryEvaluator::Change ContainerQueryEvaluator::ComputeSizeChange() const
{
    Change change = Change::kNone;
...
ContainerQueryEvaluator::Change ContainerQueryEvaluator::ComputeStyleChange() const
{
    Change change = Change::kNone;
...
ContainerQueryEvaluator::Change ContainerQueryEvaluator::ComputeStickyChange() const
{
    Change change = Change::kNone;
...
ContainerQueryEvaluator::Change ContainerQueryEvaluator::ComputeSnapChange() const
{
    Change change = Change::kNone;
...
ContainerQueryEvaluator::Change ContainerQueryEvaluator::ComputeOverflowChange() const
{
    Change change = Change::kNone;
...
void ContainerQueryEvaluator::UpdateContainerValuesFromUnitChanges(StyleRecalcChange change)
{
    CHECK(media_query_evaluator_);
...
StyleRecalcChange ContainerQueryEvaluator::ApplyScrollStateAndStyleChanges(
    const StyleRecalcChange& child_change, const ComputedStyle& old_style, const ComputedStyle& new_style, bool style_changed)
{
    StyleRecalcChange recalc_change = child_change;
    if (RuntimeEnabledFeatures::CSSStickyContainerQueriesEnabled() || RuntimeEnabledFeatures::CSSSnapContainerQueriesEnabled()
        ...
        case ContainerQueryEvaluator::Change::kDescendantContainers:
            recalc_change = recalc_change.ForceRecalcDescendantScrollStateContainers();
            break;
        }
    }
    ...
    bool invalidate_for_font = (unit_flags_ & MediaQueryExpValue::kFontRelative) && old_style.GetFont() != new_style.GetFont();
    ...
    // change.
    bool invalidate_for_writing_direction = MayDependOnWritingDirection() && old_style.GetWritingDirection() != new_style.GetWritingDirection();
    ...
    if (!base::ValuesEquivalent(old_style.InheritedVariables(), new_style.InheritedVariables())
        || !base::ValuesEquivalent(old_style.NonInheritedVariables(), new_style.NonInheritedVariables())) {
