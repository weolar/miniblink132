...
const CSSNumericLiteralValue& WrapDouble(double value, CSSPrimitiveValue::UnitType unit_type = CSSPrimitiveValue::UnitType::kNumber)
{
...
{
    return MediaQueryExpValue(WrapDouble(value, CSSPrimitiveValue::UnitType::kPixels));
}
...
void TestMQEvaluator(base::span<MediaQueryEvaluatorTestCase> test_cases, const MediaQueryEvaluator* media_query_evaluator, CSSParserMode mode)
{
    ...
    for (const MediaQueryEvaluatorTestCase& test_case : test_cases) {
        if (String(test_case.input).empty()) {
            ...
            query_set = MediaQueryParser::ParseMediaQuerySetInMode(stream, mode, nullptr);
        }
        EXPECT_EQ(test_case.output, media_query_evaluator->Eval(*query_set)) << "Query: " << test_case.input;
    }
}

void TestMQEvaluator(base::span<MediaQueryEvaluatorTestCase> test_cases, const MediaQueryEvaluator* media_query_evaluator)
{
...
TEST(MediaQueryEvaluatorTest, Cached)
{
    ...
    data.output_device_update_ability_type = mojom::blink::OutputDeviceUpdateAbilityType::kFastType;
    ...
    {
        data.output_device_update_ability_type = mojom::blink::OutputDeviceUpdateAbilityType::kSlowType;
        auto* media_values = MakeGarbageCollected<MediaValuesCached>(data);
        MediaQueryEvaluator* media_query_evaluator = MakeGarbageCollected<MediaQueryEvaluator>(media_values);
    ...
    // Overflow values with printing.
    {
        ...
        auto* media_values = MakeGarbageCollected<MediaValuesCached>(data);
        MediaQueryEvaluator* media_query_evaluator = MakeGarbageCollected<MediaQueryEvaluator>(media_values);
        TestMQEvaluator(g_overflow_with_print_device_test_cases, media_query_evaluator);
        data.media_type = media_type_names::kScreen;
    }
    ...
    // Overflow values with scrolling.
    {
        auto* media_values = MakeGarbageCollected<MediaValuesCached>(data);
        MediaQueryEvaluator* media_query_evaluator = MakeGarbageCollected<MediaQueryEvaluator>(media_values);
        TestMQEvaluator(g_overflow_with_scrollable_device_test_cases, media_query_evaluator);
    }
}
...
TEST(MediaQueryEvaluatorTest, Dynamic)
{
    ...
    MediaQueryEvaluator* media_query_evaluator = MakeGarbageCollected<MediaQueryEvaluator>(&page_holder->GetFrame());
    TestMQEvaluator(g_viewport_test_cases, media_query_evaluator);
    TestMQEvaluator(g_overflow_with_scrollable_device_test_cases, media_query_evaluator);
    TestMQEvaluator(g_update_with_fast_device_test_cases, media_query_evaluator);
    page_holder->GetFrame().GetSettings()->SetOutputDeviceUpdateAbilityType(mojom::blink::OutputDeviceUpdateAbilityType::kSlowType);
    TestMQEvaluator(g_update_with_slow_device_test_cases, media_query_evaluator);
...
TEST(MediaQueryEvaluatorTest, DynamicNoView)
{
    ...
    MediaQueryEvaluator* media_query_evaluator = MakeGarbageCollected<MediaQueryEvaluator>(frame);
...
TEST(MediaQueryEvaluatorTest, CachedFloatViewportNonFloatFriendly)
{
    ...
    MediaQueryEvaluator* media_query_evaluator = MakeGarbageCollected<MediaQueryEvaluator>(media_values);
    TestMQEvaluator(g_float_non_friendly_viewport_test_cases, media_query_evaluator);
}
...
TEST(MediaQueryEvaluatorTest, CachedPrefersReducedTransparency)
{
    ...
    // Prefers-reduced-transparency - no-preference.
    {
        ...
        MediaValues* media_values = MakeGarbageCollected<MediaValuesCached>(data);
        MediaQueryEvaluator* media_query_evaluator = MakeGarbageCollected<MediaQueryEvaluator>(media_values);
        TestMQEvaluator(g_prefersreducedtransparency_nopreference_cases, media_query_evaluator);
    }
    ...
    // Prefers-reduced-transparency - reduce.
    {
        ...
        MediaValues* media_values = MakeGarbageCollected<MediaValuesCached>(data);
        MediaQueryEvaluator* media_query_evaluator = MakeGarbageCollected<MediaQueryEvaluator>(media_values);
        TestMQEvaluator(g_prefersreducedtransparency_reduce_cases, media_query_evaluator);
    }
}
...
TEST(MediaQueryEvaluatorTest, CachedViewportSegments)
{
    ...
    MediaValuesCached::MediaValuesCachedData data;
    {
        ...
        MediaQueryEvaluator* media_query_evaluator = MakeGarbageCollected<MediaQueryEvaluator>(media_values);
        TestMQEvaluator(g_single_horizontal_viewport_segment_cases, media_query_evaluator);
    }

    {
        ...
        MediaValues* media_values = MakeGarbageCollected<MediaValuesCached>(data);
        MediaQueryEvaluator* media_query_evaluator = MakeGarbageCollected<MediaQueryEvaluator>(media_values);
        TestMQEvaluator(g_double_horizontal_viewport_segment_cases, media_query_evaluator);
    }

    {
        ...
        MediaQueryEvaluator* media_query_evaluator = MakeGarbageCollected<MediaQueryEvaluator>(media_values);
        TestMQEvaluator(g_single_vertical_viewport_segment_cases, media_query_evaluator);
    }

    {
        ...
        MediaValues* media_values = MakeGarbageCollected<MediaValuesCached>(data);
        MediaQueryEvaluator* media_query_evaluator = MakeGarbageCollected<MediaQueryEvaluator>(media_values);
        TestMQEvaluator(g_double_vertical_viewport_segment_cases, media_query_evaluator);
    }
}
...
TEST(MediaQueryEvaluatorTest, CachedDynamicRange)
{
    ...
    // Test with color spaces supporting standard dynamic range
    {
        ...
        TestMQEvaluator(g_video_dynamic_range_standard_cases, media_query_evaluator);

        // Test again with the feature disabled
        ScopedCSSVideoDynamicRangeMediaQueriesForTest const disable_video_feature { false };
        TestMQEvaluator(g_video_dynamic_range_feature_disabled_cases, media_query_evaluator);
    }
    {
        data.device_supports_hdr = gfx::DisplayColorSpaces(gfx::ColorSpace::CreateDisplayP3D65()).SupportsHDR();
        MediaValues* media_values = MakeGarbageCollected<MediaValuesCached>(data);
        MediaQueryEvaluator* media_query_evaluator = MakeGarbageCollected<MediaQueryEvaluator>(media_values);
        TestMQEvaluator(g_dynamic_range_standard_cases, media_query_evaluator);
        TestMQEvaluator(g_video_dynamic_range_standard_cases, media_query_evaluator);
    ...
    {
        data.device_supports_hdr = gfx::DisplayColorSpaces(gfx::ColorSpace::CreateExtendedSRGB()).SupportsHDR();
        MediaValues* media_values = MakeGarbageCollected<MediaValuesCached>(data);
        MediaQueryEvaluator* media_query_evaluator = MakeGarbageCollected<MediaQueryEvaluator>(media_values);
    ...
    {
        data.device_supports_hdr = gfx::DisplayColorSpaces(gfx::ColorSpace::CreateSRGBLinear()).SupportsHDR();
        MediaValues* media_values = MakeGarbageCollected<MediaValuesCached>(data);
        MediaQueryEvaluator* media_query_evaluator = MakeGarbageCollected<MediaQueryEvaluator>(media_values);
        TestMQEvaluator(g_dynamic_range_high_cases, media_query_evaluator);
        TestMQEvaluator(g_video_dynamic_range_high_cases, media_query_evaluator);
    ...
    {
        data.device_supports_hdr = gfx::DisplayColorSpaces(gfx::ColorSpace::CreateHDR10()).SupportsHDR();
        MediaValues* media_values = MakeGarbageCollected<MediaValuesCached>(data);
        MediaQueryEvaluator* media_query_evaluator = MakeGarbageCollected<MediaQueryEvaluator>(media_values);
        TestMQEvaluator(g_dynamic_range_high_cases, media_query_evaluator);
        TestMQEvaluator(g_video_dynamic_range_high_cases, media_query_evaluator);
    ...
    {
        data.device_supports_hdr = gfx::DisplayColorSpaces(gfx::ColorSpace::CreateHLG()).SupportsHDR();
        MediaValues* media_values = MakeGarbageCollected<MediaValuesCached>(data);
        MediaQueryEvaluator* media_query_evaluator = MakeGarbageCollected<MediaQueryEvaluator>(media_values);
        TestMQEvaluator(g_dynamic_range_high_cases, media_query_evaluator);
        TestMQEvaluator(g_video_dynamic_range_high_cases, media_query_evaluator);
...
TEST(MediaQueryEvaluatorTest, RangedValues)
{
    ...
    // (width < 600px)
    EXPECT_TRUE(eval(MediaQueryExp::Create(AtomicString("width"), MediaQueryExpBounds(MediaQueryExpComparison(PxValue(600), MediaQueryOperator::kLt)))));

    // (width < 501px)
    EXPECT_TRUE(eval(MediaQueryExp::Create(AtomicString("width"), MediaQueryExpBounds(MediaQueryExpComparison(PxValue(501), MediaQueryOperator::kLt)))));

    // (width < 500px)
    EXPECT_FALSE(eval(MediaQueryExp::Create(AtomicString("width"), MediaQueryExpBounds(MediaQueryExpComparison(PxValue(500), MediaQueryOperator::kLt)))));

    // (width > 500px)
    EXPECT_FALSE(eval(MediaQueryExp::Create(AtomicString("width"), MediaQueryExpBounds(MediaQueryExpComparison(PxValue(500), MediaQueryOperator::kGt)))));

    // (width < 501px)
    EXPECT_TRUE(eval(MediaQueryExp::Create(AtomicString("width"), MediaQueryExpBounds(MediaQueryExpComparison(PxValue(501), MediaQueryOperator::kLt)))));

    // (width <= 500px)
    EXPECT_TRUE(eval(MediaQueryExp::Create(AtomicString("width"), MediaQueryExpBounds(MediaQueryExpComparison(PxValue(500), MediaQueryOperator::kLe)))));

    // (400px < width)
    EXPECT_TRUE(eval(MediaQueryExp::Create(
        AtomicString("width"), MediaQueryExpBounds(MediaQueryExpComparison(PxValue(400), MediaQueryOperator::kLt), MediaQueryExpComparison()))));

    // (600px < width)
    EXPECT_FALSE(eval(MediaQueryExp::Create(
        AtomicString("width"), MediaQueryExpBounds(MediaQueryExpComparison(PxValue(600), MediaQueryOperator::kLt), MediaQueryExpComparison()))));

    // (400px > width)
    EXPECT_FALSE(eval(MediaQueryExp::Create(
        AtomicString("width"), MediaQueryExpBounds(MediaQueryExpComparison(PxValue(400), MediaQueryOperator::kGt), MediaQueryExpComparison()))));

    // (600px > width)
    EXPECT_TRUE(eval(MediaQueryExp::Create(
        AtomicString("width"), MediaQueryExpBounds(MediaQueryExpComparison(PxValue(600), MediaQueryOperator::kGt), MediaQueryExpComparison()))));

    // (400px <= width)
    EXPECT_TRUE(eval(MediaQueryExp::Create(
        AtomicString("width"), MediaQueryExpBounds(MediaQueryExpComparison(PxValue(400), MediaQueryOperator::kLe), MediaQueryExpComparison()))));

    // (600px <= width)
    EXPECT_FALSE(eval(MediaQueryExp::Create(
        AtomicString("width"), MediaQueryExpBounds(MediaQueryExpComparison(PxValue(600), MediaQueryOperator::kLe), MediaQueryExpComparison()))));

    // (400px >= width)
    EXPECT_FALSE(eval(MediaQueryExp::Create(
        AtomicString("width"), MediaQueryExpBounds(MediaQueryExpComparison(PxValue(400), MediaQueryOperator::kGe), MediaQueryExpComparison()))));

    // (600px >= width)
    EXPECT_TRUE(eval(MediaQueryExp::Create(
        AtomicString("width"), MediaQueryExpBounds(MediaQueryExpComparison(PxValue(600), MediaQueryOperator::kGe), MediaQueryExpComparison()))));

    // (width = 500px)
    EXPECT_TRUE(eval(MediaQueryExp::Create(AtomicString("width"), MediaQueryExpBounds(MediaQueryExpComparison(PxValue(500), MediaQueryOperator::kEq)))));

    // (width = 400px)
    EXPECT_FALSE(eval(MediaQueryExp::Create(AtomicString("width"), MediaQueryExpBounds(MediaQueryExpComparison(PxValue(400), MediaQueryOperator::kEq)))));

    // (500px = width)
    EXPECT_TRUE(eval(MediaQueryExp::Create(
        AtomicString("width"), MediaQueryExpBounds(MediaQueryExpComparison(PxValue(500), MediaQueryOperator::kEq), MediaQueryExpComparison()))));

    // (400px = width)
    EXPECT_FALSE(eval(MediaQueryExp::Create(
        AtomicString("width"), MediaQueryExpBounds(MediaQueryExpComparison(PxValue(400), MediaQueryOperator::kEq), MediaQueryExpComparison()))));

    // (400px < width < 600px)
    EXPECT_TRUE(eval(MediaQueryExp::Create(AtomicString("width"),
        MediaQueryExpBounds(MediaQueryExpComparison(PxValue(400), MediaQueryOperator::kLt), MediaQueryExpComparison(PxValue(600), MediaQueryOperator::kLt)))));

    // (550px < width < 600px)
    EXPECT_FALSE(eval(MediaQueryExp::Create(AtomicString("width"),
        MediaQueryExpBounds(MediaQueryExpComparison(PxValue(550), MediaQueryOperator::kLt), MediaQueryExpComparison(PxValue(600), MediaQueryOperator::kLt)))));

    // (400px < width < 450px)
    EXPECT_FALSE(eval(MediaQueryExp::Create(AtomicString("width"),
        MediaQueryExpBounds(MediaQueryExpComparison(PxValue(400), MediaQueryOperator::kLt), MediaQueryExpComparison(PxValue(450), MediaQueryOperator::kLt)))));

    // (aspect-ratio = 2/1)
    EXPECT_TRUE(
        eval(MediaQueryExp::Create(AtomicString("aspect-ratio"), MediaQueryExpBounds(MediaQueryExpComparison(RatioValue(2, 1), MediaQueryOperator::kEq)))));

    // (aspect-ratio = 3/1)
    EXPECT_FALSE(
        eval(MediaQueryExp::Create(AtomicString("aspect-ratio"), MediaQueryExpBounds(MediaQueryExpComparison(RatioValue(3, 1), MediaQueryOperator::kEq)))));

    // (aspect-ratio < 1/1)
    EXPECT_FALSE(
        eval(MediaQueryExp::Create(AtomicString("aspect-ratio"), MediaQueryExpBounds(MediaQueryExpComparison(RatioValue(1, 1), MediaQueryOperator::kLt)))));

    // (aspect-ratio < 3/1)
    EXPECT_TRUE(
        eval(MediaQueryExp::Create(AtomicString("aspect-ratio"), MediaQueryExpBounds(MediaQueryExpComparison(RatioValue(3, 1), MediaQueryOperator::kLt)))));

    // (aspect-ratio > 1/1)
    EXPECT_TRUE(
        eval(MediaQueryExp::Create(AtomicString("aspect-ratio"), MediaQueryExpBounds(MediaQueryExpComparison(RatioValue(1, 1), MediaQueryOperator::kGt)))));

    // (aspect-ratio > 3/1)
    EXPECT_FALSE(
        eval(MediaQueryExp::Create(AtomicString("aspect-ratio"), MediaQueryExpBounds(MediaQueryExpComparison(RatioValue(3, 1), MediaQueryOperator::kGt)))));
}
...
TEST(MediaQueryEvaluatorTest, ExpNode)
{
    ...
    auto* width_lt_400 = MakeGarbageCollected<MediaQueryFeatureExpNode>(
        MediaQueryExp::Create(AtomicString("width"), MediaQueryExpBounds(MediaQueryExpComparison(PxValue(400), MediaQueryOperator::kLt))));
    auto* width_lt_600 = MakeGarbageCollected<MediaQueryFeatureExpNode>(
        MediaQueryExp::Create(AtomicString("width"), MediaQueryExpBounds(MediaQueryExpComparison(PxValue(600), MediaQueryOperator::kLt))));
    auto* width_lt_800 = MakeGarbageCollected<MediaQueryFeatureExpNode>(
        MediaQueryExp::Create(AtomicString("width"), MediaQueryExpBounds(MediaQueryExpComparison(PxValue(800), MediaQueryOperator::kLt))));
    ...
    EXPECT_EQ(KleeneValue::kTrue, media_query_evaluator->Eval(*MakeGarbageCollected<MediaQueryNestedExpNode>(width_lt_600)));
    EXPECT_EQ(KleeneValue::kFalse, media_query_evaluator->Eval(*MakeGarbageCollected<MediaQueryNestedExpNode>(width_lt_400)));

    EXPECT_EQ(KleeneValue::kFalse, media_query_evaluator->Eval(*MakeGarbageCollected<MediaQueryNotExpNode>(width_lt_600)));
    EXPECT_EQ(KleeneValue::kTrue, media_query_evaluator->Eval(*MakeGarbageCollected<MediaQueryNotExpNode>(width_lt_400)));

    EXPECT_EQ(KleeneValue::kTrue, media_query_evaluator->Eval(*MakeGarbageCollected<MediaQueryAndExpNode>(width_lt_600, width_lt_800)));
    EXPECT_EQ(KleeneValue::kFalse, media_query_evaluator->Eval(*MakeGarbageCollected<MediaQueryAndExpNode>(width_lt_600, width_lt_400)));

    EXPECT_EQ(KleeneValue::kTrue, media_query_evaluator->Eval(*MakeGarbageCollected<MediaQueryOrExpNode>(width_lt_600, width_lt_400)));
    EXPECT_EQ(KleeneValue::kFalse,
        media_query_evaluator->Eval(*MakeGarbageCollected<MediaQueryOrExpNode>(width_lt_400, MakeGarbageCollected<MediaQueryNotExpNode>(width_lt_800))));
}
...
TEST(MediaQueryEvaluatorTest, DependentResults)
{
    ...
    auto* device_width_lt_600 = MakeGarbageCollected<MediaQueryFeatureExpNode>(
        MediaQueryExp::Create(AtomicString("device-width"), MediaQueryExpBounds(MediaQueryExpComparison(PxValue(600), MediaQueryOperator::kLt))));

    // Neither viewport- nor device-dependent:
    auto* color = MakeGarbageCollected<MediaQueryFeatureExpNode>(
        MediaQueryExp::Create(AtomicString("color"), MediaQueryExpBounds(MediaQueryExpComparison(MediaQueryExpValue()))));
    ...
    // "((device-width < 600px))" should be device-dependent.
    {
        ...
        media_query_evaluator->Eval(*MakeGarbageCollected<MediaQueryNestedExpNode>(device_width_lt_600), &result_flags);
    ...
    // "not (device-width < 600px)" should be device-dependent.
    {
        ...
        media_query_evaluator->Eval(*MakeGarbageCollected<MediaQueryNotExpNode>(device_width_lt_600), &result_flags);
    ...
    // device-dependent.
    {
        ...
        media_query_evaluator->Eval(*MakeGarbageCollected<MediaQueryAndExpNode>(width_lt_400, device_width_lt_600), &result_flags);
    ...
    // the second condition irrelevant.
    {
        ...
        media_query_evaluator->Eval(
            *MakeGarbageCollected<MediaQueryAndExpNode>(MakeGarbageCollected<MediaQueryNotExpNode>(width_lt_400), device_width_lt_600), &result_flags);
    ...
    // the second condition irrelevant.
    {
        ...
        media_query_evaluator->Eval(*MakeGarbageCollected<MediaQueryOrExpNode>(width_lt_400, device_width_lt_600), &result_flags);
    ...
    //  and device-dependent.
    {
        ...
        media_query_evaluator->Eval(
            *MakeGarbageCollected<MediaQueryOrExpNode>(MakeGarbageCollected<MediaQueryNotExpNode>(width_lt_400), device_width_lt_600), &result_flags);
...
TEST(MediaQueryEvaluatorTest, CSSMediaQueries4)
{
    ...
    MediaQueryEvaluatorTestCase test_cases[] = {
        ...
        { "(width >= 500px) and ((499px > width) or (not (width = 500px)))", false },
    };
...
TEST(MediaQueryEvaluatorTest, GeneralEnclosed)
{
    ...
    for (const MediaQueryEvaluatorTestCase& test : tests) {
        ...
        MediaQuerySet* query_set = MediaQueryParser::ParseMediaQuerySet(input, nullptr);
...
public:
    MediaQueryEvaluatorIdentifiabilityTest()
        : counts_ { .response_for_is_active = true, .response_for_is_anything_blocked = false, .response_for_is_allowed = true }
    {
        IdentifiabilityStudySettings::SetGlobalProvider(std::make_unique<CountingSettingsProvider>(&counts_));
    }
...
TEST_F(MediaQueryEvaluatorIdentifiabilityTest, MediaFeatureIdentifiableSurfacePrefersReducedMotion)
{
    ...
    EXPECT_TRUE(GetDocument().WasMediaFeatureEvaluated(static_cast<int>(IdentifiableSurface::MediaFeatureName::kPrefersReducedMotion)));
    ...
    EXPECT_EQ(entry.metrics.begin()->surface,
        IdentifiableSurface::FromTypeAndToken(
            IdentifiableSurface::Type::kMediaFeature, IdentifiableToken(IdentifiableSurface::MediaFeatureName::kPrefersReducedMotion)));
    EXPECT_EQ(entry.metrics.begin()->value, IdentifiableToken(false));
}

TEST_F(MediaQueryEvaluatorIdentifiabilityTest, MediaFeatureIdentifiableSurfacePrefersReducedTransparency)
{
    GetDocument().body()->setInnerHTML(R"HTML(
    <style>
    ...
    UpdateAllLifecyclePhases();
    EXPECT_TRUE(GetDocument().WasMediaFeatureEvaluated(static_cast<int>(IdentifiableSurface::MediaFeatureName::kPrefersReducedTransparency)));
    EXPECT_EQ(collector()->entries().size(), 1u);
    ...
    EXPECT_EQ(entry.metrics.begin()->surface,
        IdentifiableSurface::FromTypeAndToken(
            IdentifiableSurface::Type::kMediaFeature, IdentifiableToken(IdentifiableSurface::MediaFeatureName::kPrefersReducedTransparency)));
    EXPECT_EQ(entry.metrics.begin()->value, IdentifiableToken(false));
}

TEST_F(MediaQueryEvaluatorIdentifiabilityTest, MediaFeatureIdentifiableSurfaceOrientation)
{
    GetDocument().body()->setInnerHTML(R"HTML(
    <style>
    ...
    UpdateAllLifecyclePhases();
    EXPECT_TRUE(GetDocument().WasMediaFeatureEvaluated(static_cast<int>(IdentifiableSurface::MediaFeatureName::kOrientation)));
    ...
    EXPECT_EQ(entry.metrics.begin()->surface,
        IdentifiableSurface::FromTypeAndToken(
            IdentifiableSurface::Type::kMediaFeature, IdentifiableToken(IdentifiableSurface::MediaFeatureName::kOrientation)));
    EXPECT_EQ(entry.metrics.begin()->value, IdentifiableToken(CSSValueID::kLandscape));
}

TEST_F(MediaQueryEvaluatorIdentifiabilityTest, MediaFeatureIdentifiableSurfaceCollectOnce)
{
    GetDocument().body()->setInnerHTML(R"HTML(
    <style>
      @media (orientation: landscape) {
...
TEST_F(MediaQueryEvaluatorIdentifiabilityTest, MediaFeatureIdentifiableSurfaceDisplayMode)
{
    GetDocument().body()->setInnerHTML(R"HTML(
    <style>
    ...
    UpdateAllLifecyclePhases();
    EXPECT_TRUE(GetDocument().WasMediaFeatureEvaluated(static_cast<int>(IdentifiableSurface::MediaFeatureName::kDisplayMode)));
    EXPECT_EQ(collector()->entries().size(), 1u);
    ...
    EXPECT_EQ(entry.metrics.begin()->surface,
        IdentifiableSurface::FromTypeAndToken(
            IdentifiableSurface::Type::kMediaFeature, IdentifiableToken(IdentifiableSurface::MediaFeatureName::kDisplayMode)));
    EXPECT_EQ(entry.metrics.begin()->value, IdentifiableToken(blink::mojom::DisplayMode::kBrowser));
}

TEST_F(MediaQueryEvaluatorIdentifiabilityTest, MediaFeatureIdentifiableSurfaceDisplayState)
{
    GetDocument().body()->setInnerHTML(R"HTML(
    <style>
    ...
    UpdateAllLifecyclePhases();
    EXPECT_TRUE(GetDocument().WasMediaFeatureEvaluated(static_cast<int>(IdentifiableSurface::MediaFeatureName::kDisplayState)));
    EXPECT_EQ(collector()->entries().size(), 1u);
    ...
    EXPECT_EQ(entry.metrics.begin()->surface,
        IdentifiableSurface::FromTypeAndToken(
            IdentifiableSurface::Type::kMediaFeature, IdentifiableToken(IdentifiableSurface::MediaFeatureName::kDisplayState)));
    EXPECT_EQ(entry.metrics.begin()->value, IdentifiableToken(ui::mojom::blink::WindowShowState::kDefault));
}

TEST_F(MediaQueryEvaluatorIdentifiabilityTest, MediaFeatureIdentifiableSurfaceResizable)
{
    GetDocument().body()->setInnerHTML(R"HTML(
    <style>
    ...
    UpdateAllLifecyclePhases();
    EXPECT_TRUE(GetDocument().WasMediaFeatureEvaluated(static_cast<int>(IdentifiableSurface::MediaFeatureName::kResizable)));
    EXPECT_EQ(collector()->entries().size(), 1u);

    auto& entry = collector()->entries().front();
    EXPECT_EQ(entry.metrics.size(), 1u);
    EXPECT_EQ(entry.metrics.begin()->surface,
        IdentifiableSurface::FromTypeAndToken(IdentifiableSurface::Type::kMediaFeature, IdentifiableToken(IdentifiableSurface::MediaFeatureName::kResizable)));
    EXPECT_EQ(entry.metrics.begin()->value, IdentifiableToken(true));
}

TEST_F(MediaQueryEvaluatorIdentifiabilityTest, MediaFeatureIdentifiableSurfaceForcedColorsHover)
{
    GetDocument().body()->setInnerHTML(R"HTML(
    <style>
    ...
    UpdateAllLifecyclePhases();
    EXPECT_TRUE(GetDocument().WasMediaFeatureEvaluated(static_cast<int>(IdentifiableSurface::MediaFeatureName::kForcedColors)));
    EXPECT_TRUE(GetDocument().WasMediaFeatureEvaluated(static_cast<int>(IdentifiableSurface::MediaFeatureName::kHover)));
    ...
    EXPECT_EQ(entry_forced_colors.metrics.begin()->surface,
        IdentifiableSurface::FromTypeAndToken(
            IdentifiableSurface::Type::kMediaFeature, IdentifiableToken(IdentifiableSurface::MediaFeatureName::kForcedColors)));
    EXPECT_EQ(entry_forced_colors.metrics.begin()->value, IdentifiableToken(ForcedColors::kNone));

    auto& entry_hover = collector()->entries().back();
    EXPECT_EQ(entry_hover.metrics.size(), 1u);
    EXPECT_EQ(entry_hover.metrics.begin()->surface,
        IdentifiableSurface::FromTypeAndToken(IdentifiableSurface::Type::kMediaFeature, IdentifiableToken(IdentifiableSurface::MediaFeatureName::kHover)));
    EXPECT_EQ(entry_hover.metrics.begin()->value, IdentifiableToken(mojom::blink::HoverType::kHoverNone));
}

TEST_F(MediaQueryEvaluatorIdentifiabilityTest, MediaFeatureIdentifiableSurfaceAspectRatioNormalized)
{
    GetDocument().body()->setInnerHTML(R"HTML(
    <style>
    ...
    UpdateAllLifecyclePhases();
    EXPECT_TRUE(GetDocument().WasMediaFeatureEvaluated(static_cast<int>(IdentifiableSurface::MediaFeatureName::kAspectRatioNormalized)));
    EXPECT_EQ(collector()->entries().size(), 1u);
    ...
    EXPECT_EQ(entry.metrics.begin()->surface,
        IdentifiableSurface::FromTypeAndToken(
            IdentifiableSurface::Type::kMediaFeature, IdentifiableToken(IdentifiableSurface::MediaFeatureName::kAspectRatioNormalized)));
}

TEST_F(MediaQueryEvaluatorIdentifiabilityTest, MediaFeatureIdentifiableSurfaceResolution)
{
    GetDocument().body()->setInnerHTML(R"HTML(
    <style>
    ...
    EXPECT_EQ(entry.metrics.size(), 1u);
    EXPECT_EQ(entry.metrics.begin()->surface,
        IdentifiableSurface::FromTypeAndToken(IdentifiableSurface::Type::kMediaFeature, IdentifiableToken(IdentifiableSurface::MediaFeatureName::kResolution)));
}

TEST_F(MediaQueryEvaluatorIdentifiabilityTest, MediaFeatureIdentifiableSurfaceInvertedColors)
{
    GetDocument().body()->setInnerHTML(R"HTML(
    <style>
    ...
    UpdateAllLifecyclePhases();
    EXPECT_TRUE(GetDocument().WasMediaFeatureEvaluated(static_cast<int>(IdentifiableSurface::MediaFeatureName::kInvertedColors)));
    EXPECT_EQ(collector()->entries().size(), 1u);
    ...
    EXPECT_EQ(entry.metrics.begin()->surface,
        IdentifiableSurface::FromTypeAndToken(
            IdentifiableSurface::Type::kMediaFeature, IdentifiableToken(IdentifiableSurface::MediaFeatureName::kInvertedColors)));
    EXPECT_EQ(entry.metrics.begin()->value, IdentifiableToken(false));
}

TEST_F(MediaQueryEvaluatorIdentifiabilityTest, MediaFeatureIdentifiableSurfaceScripting)
{
    GetDocument().body()->setInnerHTML(R"HTML(
    <style>
    ...
    EXPECT_EQ(entry.metrics.size(), 1u);
    EXPECT_EQ(entry.metrics.begin()->surface,
        IdentifiableSurface::FromTypeAndToken(IdentifiableSurface::Type::kMediaFeature, IdentifiableToken(IdentifiableSurface::MediaFeatureName::kScripting)));
