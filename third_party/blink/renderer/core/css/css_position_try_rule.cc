...
StyleRulePositionTry::StyleRulePositionTry(const AtomicString& name, CSSPropertyValueSet* properties)
...
CSSPositionTryRule::CSSPositionTryRule(StyleRulePositionTry* position_try_rule, CSSStyleSheet* parent)
    : CSSRule(parent)
    , position_try_rule_(position_try_rule)
{
}
...
CSSStyleDeclaration* CSSPositionTryRule::style() const
{
    if (!properties_cssom_wrapper_) {
        properties_cssom_wrapper_
            = MakeGarbageCollected<CSSPositionTryDescriptors>(position_try_rule_->MutableProperties(), const_cast<CSSPositionTryRule*>(this));
    }
