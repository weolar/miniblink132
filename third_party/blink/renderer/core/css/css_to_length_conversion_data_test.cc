...
public:
    ...
    CSSToLengthConversionData ConversionData(DataOptions options)
    {
        ...
        if (options.css_zoom.has_value()) {
            root->SetInlineStyleProperty(CSSPropertyID::kZoom, String::Format("%f", *options.css_zoom));
        }
        ...
        GetDocument().body()->SetInlineStyleProperty(CSSPropertyID::kLineHeight, "10");
        UpdateAllLifecyclePhasesForTest();

        return CSSToLengthConversionData(div->ComputedStyleRef(), GetDocument().body()->GetComputedStyle(), GetDocument().documentElement()->GetComputedStyle(),
            CSSToLengthConversionData::ViewportSize(GetDocument().GetLayoutView()), CSSToLengthConversionData::ContainerSizes(),
            CSSToLengthConversionData::AnchorData(options.anchor_evaluator,
                /* position_anchor */ nullptr,
                /* position_area_offsets */ std::nullopt),
            options.data_zoom.value_or(div->GetComputedStyle()->EffectiveZoom()), options.flags ? *options.flags : ignored_flags_);
    }
    ...
    // specific property.
    Length ConvertLength(const CSSToLengthConversionData& data, String value, CSSPropertyID property_id = CSSPropertyID::kLeft)
    {
        const CSSValue* result = css_test_helpers::ParseLonghand(GetDocument(), CSSProperty::Get(property_id), value);
        CHECK(result);
    ...
    float ConvertPx(const CSSToLengthConversionData& data, String value, CSSPropertyID property_id = CSSPropertyID::kLeft)
    {
    ...
    void SetLineHeightSize(Element& element, CSSToLengthConversionData& data)
    {
        data.SetLineHeightSize(CSSToLengthConversionData::LineHeightSize(
            element.ComputedStyleRef().GetFontSizeStyle(), element.GetDocument().documentElement()->GetComputedStyle()));
    }
...
TEST_F(CSSToLengthConversionDataTest, DifferentZoom)
{
    ...
    CSSToLengthConversionData data = ConversionData({ .css_zoom = 1.0f, .data_zoom = 2.0f });
    EXPECT_FLOAT_EQ(2.0f, ConvertPx(data, "1px"));
    EXPECT_FLOAT_EQ(40.0f, ConvertPx(data, "1em"));
    EXPECT_FLOAT_EQ(32.0f, ConvertPx(data, "1ex"));
    EXPECT_FLOAT_EQ(40.0f, ConvertPx(data, "1ch"));
...
{
    CSSToLengthConversionData data = ConversionData({ .css_zoom = 2.0f }).Unzoomed();
    EXPECT_FLOAT_EQ(1.0f, ConvertPx(data, "1px"));
    EXPECT_FLOAT_EQ(20.0f, ConvertPx(data, "1em"));
    EXPECT_FLOAT_EQ(16.0f, ConvertPx(data, "1ex"));
    EXPECT_FLOAT_EQ(20.0f, ConvertPx(data, "1ch"));
...
TEST_F(CSSToLengthConversionDataTest, ConversionWithoutPrimaryFont)
{
    ...
    CSSToLengthConversionData::LineHeightSize line_height_size(Length::Fixed(16.0f), &font, /* font_zoom */ 1.0f);
...
TEST_F(CSSToLengthConversionDataTest, AnchorFunction)
{
    ...
    CSSToLengthConversionData data = ConversionData({ .anchor_evaluator = &anchor_evaluator });
...
TEST_F(CSSToLengthConversionDataTest, AnchorFunctionFallback)
{
    ...
    EXPECT_FLOAT_EQ(52.0f, ConvertPx(data, "anchor(--a left, calc(42px + 10px))", right));
    EXPECT_FLOAT_EQ(10.0f, ConvertPx(data, "anchor(--a left, min(42px, 10px))", right));
}
...
TEST_F(CSSToLengthConversionDataTest, AnchorSizeFunction)
{
    ...
    EXPECT_FLOAT_EQ(2.0f, ConvertPx(data, "calc(anchor-size(--a width) / 30)", width));
}
...
TEST_F(CSSToLengthConversionDataTest, AnchorSizeFunctionFallback)
{
    ...
    EXPECT_FLOAT_EQ(42.0f, ConvertPx(data, "anchor-size(--a width, 42px)", width));
    EXPECT_FLOAT_EQ(52.0f, ConvertPx(data, "anchor-size(--a width, calc(42px + 10px))", width));
    EXPECT_FLOAT_EQ(10.0f, ConvertPx(data, "anchor-size(--a width, min(42px, 10px))", width));
}
...
TEST_F(CSSToLengthConversionDataTest, AnchorWithinOtherFunction)
{
    ...
    EXPECT_FLOAT_EQ(42.0f, ConvertPx(data, "calc(anchor(--a left, 10px) + 32px)", right));
    EXPECT_EQ(ConvertLength(data, "calc(10px + 42%)", right), ConvertLength(data, "calc(anchor(--a left, 10px) + 42%)", right));
    EXPECT_EQ(ConvertLength(data, "calc(0px + 42%)", right), ConvertLength(data, "calc(anchor(--a left, 0px) + 42%)", right));
    EXPECT_EQ(ConvertLength(data, "min(10px, 42%)", right), ConvertLength(data, "min(anchor(--a left, 10px), 42%)", right));
    EXPECT_EQ(ConvertLength(data, "min(10px, 42%)", right), ConvertLength(data, "min(anchor(--a left, 10px), 42%)", right));
    EXPECT_FLOAT_EQ(10.0f, ConvertLength(data, "min(anchor(--a left, 10px), 42px)", right).Pixels());
    ...
    EXPECT_EQ("min(10%, 42%)",
        CSSPrimitiveValue::CreateFromLength(ConvertLength(data, "min(anchor(--a left, 10%), 42%)", right),
            /* zoom */ 1.0f)
            ->CssText());
}
...
TEST_F(CSSToLengthConversionDataTest, AnchorFunctionPercentageFallback)
{
    ...
    EXPECT_EQ("42%",
        CSSPrimitiveValue::CreateFromLength(ConvertLength(data, "anchor(--a left, 42%)", right),
            /* zoom */ 1.0f)
            ->CssText());
    EXPECT_EQ("52%",
        CSSPrimitiveValue::CreateFromLength(ConvertLength(data, "anchor(--a left, calc(42% + 10%))", right),
            /* zoom */ 1.0f)
            ->CssText());
    EXPECT_EQ("10%",
        CSSPrimitiveValue::CreateFromLength(ConvertLength(data, "anchor(--a left, min(42%, 10%))", right),
            /* zoom */ 1.0f)
            ->CssText());
}

TEST_F(CSSToLengthConversionDataTest, AnchorFunctionPercentageFallbackNotTaken)
{
    TestAnchorEvaluator anchor_evaluator(/* result */ LayoutUnit(60.0));
    CSSToLengthConversionData data = ConversionData({ .anchor_evaluator = &anchor_evaluator });
...
{
    CSSToLengthConversionData data = ConversionData({ .anchor_evaluator = nullptr });
...
TEST_F(CSSToLengthConversionDataTest, AnchorFunctionLengthPercentageFallback)
{
    ...
    EXPECT_EQ(ConvertLength(data, "calc(10px + 42%)", right), ConvertLength(data, "anchor(--a left, calc(10px + 42%))", right));
    EXPECT_EQ(ConvertLength(data, "min(10px, 42%)", right), ConvertLength(data, "anchor(--a left, min(10px, 42%))", right));
}
...
TEST_F(CSSToLengthConversionDataTest, ContainerUnitsWithContainerName)
{
    ...
    container->SetInlineStyleProperty(CSSPropertyID::kContainerName, "root_container");
    ...
    CSSToLengthConversionData::Flags flags = 0;
    CSSToLengthConversionData length_resolver(child->ComputedStyleRef(), GetDocument().body()->GetComputedStyle(),
        GetDocument().documentElement()->GetComputedStyle(), CSSToLengthConversionData::ViewportSize(GetDocument().GetLayoutView()),
        CSSToLengthConversionData::ContainerSizes(child),
        CSSToLengthConversionData::AnchorData(nullptr,
            /* position_anchor */ nullptr,
            /* position_area_offsets */ std::nullopt),
        child->GetComputedStyle()->EffectiveZoom(), flags);

    ScopedCSSName* name = MakeGarbageCollected<ScopedCSSName>(AtomicString("root_container"), nullptr);
