...
CascadeLayer* CascadeLayer::GetOrAddSubLayer(const StyleRuleBase::LayerName& name)
{
...
void CascadeLayer::ToStringInternal(StringBuilder& result, const String& prefix) const
{
    for (const auto& sub_layer : direct_sub_layers_) {
        String name = sub_layer->name_.length() ? sub_layer->name_ : String("(anonymous)");
        if (result.length()) {
            result.Append(",");
        }
