...
public:
    ...
    virtual std::optional<LayoutUnit> Evaluate(const AnchorQuery&, const ScopedCSSName* position_anchor, const std::optional<PositionAreaOffsets>&) = 0;

    // Take the computed position-area and position-anchor and compute the
    // physical offsets to inset the containing block with.
    virtual std::optional<PositionAreaOffsets> ComputePositionAreaOffsetsForLayout(const ScopedCSSName* position_anchor, PositionArea position_area) = 0;

    // Take the computed position-area and position-anchor from the builder and
    // compute the physical offset for anchor-center
    virtual std::optional<PhysicalOffset> ComputeAnchorCenterOffsets(const ComputedStyleBuilder&) = 0;
