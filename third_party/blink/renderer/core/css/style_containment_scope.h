...
class StyleContainmentScope final : public GarbageCollected<StyleContainmentScope> {
public:
    StyleContainmentScope(const Element* element, StyleContainmentScopeTree* style_containment_tree);
    ...
    StyleContainmentScope* Parent()
    {
        return parent_.Get();
    }
    void SetParent(StyleContainmentScope* parent)
    {
        parent_ = parent;
    }
    const HeapVector<Member<LayoutQuote>>& Quotes() const
    {
        return quotes_;
    }
