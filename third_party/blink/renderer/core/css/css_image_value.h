...
public:
    CSSImageValue(CSSUrlData url_data, StyleImage* image = nullptr);
    ~CSSImageValue();

    bool IsCachePending() const
    {
    ...
    FetchParameters PrepareFetch(const Document&, FetchParameters::ImageRequestBehavior, CrossOriginAttributeValue) const;
    StyleImage* CacheImage(const Document&, FetchParameters::ImageRequestBehavior, CrossOriginAttributeValue = kCrossOriginAttributeNotSet,
        const float override_image_resolution = 0.0f);

    const String& RelativeUrl() const
    {
        return url_data_.UnresolvedUrl();
    }
    ...
    {
        return MakeGarbageCollected<CSSImageValue>(url_data_.MakeAbsolute(), cached_image_.Get());
    }
    CSSImageValue* ComputedCSSValueMaybeLocal() const;

    CSSImageValue* Clone() const
    {
        return MakeGarbageCollected<CSSImageValue>(url_data_.MakeWithoutReferrer(), cached_image_.Get());
    }

    void SetInitiator(const AtomicString& name)
    {
        initiator_name_ = name;
    }
...
template <> struct DowncastTraits<CSSImageValue> {
