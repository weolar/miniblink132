...
inline uint32_t EncodeInterpolationPosition(CSSPropertyID id, uint8_t index, bool is_presentation_attribute)
{
    static_assert(kIntLastCSSProperty < std::numeric_limits<uint16_t>::max(), "Enough bits for CSSPropertyID");
    DCHECK_NE(id, CSSPropertyID::kInvalid);
    DCHECK_LE(id, kLastCSSProperty);
    return (static_cast<uint32_t>(!is_presentation_attribute) << 24) | (static_cast<uint32_t>(index & 0xFF) << 16) | (static_cast<uint32_t>(id) & 0xFFFF);
}
