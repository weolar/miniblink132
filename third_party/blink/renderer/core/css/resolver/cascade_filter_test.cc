...
{
    EXPECT_EQ(CascadeFilter().Add(CSSProperty::kInherited, true), CascadeFilter(CSSProperty::kInherited, true));
    EXPECT_EQ(CascadeFilter().Add(CSSProperty::kInherited, false), CascadeFilter(CSSProperty::kInherited, false));
}

TEST(CascadeFilterTest, Equals)
{
    EXPECT_EQ(CascadeFilter(CSSProperty::kInherited, true), CascadeFilter(CSSProperty::kInherited, true));
    EXPECT_EQ(CascadeFilter(CSSProperty::kInherited, false), CascadeFilter(CSSProperty::kInherited, false));
}

TEST(CascadeFilterTest, NotEqualsMask)
{
    EXPECT_NE(CascadeFilter(CSSProperty::kInherited, true), CascadeFilter(CSSProperty::kInherited, false));
    EXPECT_NE(CascadeFilter(CSSProperty::kInherited, false), CascadeFilter(CSSProperty::kVisited, false));
    EXPECT_NE(CascadeFilter(CSSProperty::kInherited, false), CascadeFilter(CSSProperty::kInherited, false).Add(CSSProperty::kVisited, false));
...
{
    auto filter = CascadeFilter().Add(CSSProperty::kVisited, true).Add(CSSProperty::kInherited, true);
    EXPECT_FALSE(filter.Rejects(GetCSSPropertyBackgroundColor()));
    EXPECT_TRUE(filter.Rejects(GetCSSPropertyColor()));
    EXPECT_FALSE(filter.Rejects(GetCSSPropertyDisplay()));
    EXPECT_FALSE(filter.Rejects(GetCSSPropertyFloat()));
...
{
    auto filter = CascadeFilter().Add(CSSProperty::kVisited, true).Add(CSSProperty::kInherited, false);
    EXPECT_TRUE(filter.Rejects(GetCSSPropertyBackgroundColor()));
    EXPECT_FALSE(filter.Rejects(GetCSSPropertyColor()));
    EXPECT_TRUE(filter.Rejects(GetCSSPropertyDisplay()));
    EXPECT_TRUE(filter.Rejects(GetCSSPropertyFloat()));
