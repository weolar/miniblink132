...
protected:
    const ComputedStyle* StyleForId(const char* id, StyleRecalcContext style_recalc_context = {})
    {
        Element* element = GetElementById(id);
        style_recalc_context.old_style = element->GetComputedStyle();
        const auto* style = GetStyleEngine().GetStyleResolver().ResolveStyle(element, style_recalc_context);
        DCHECK(style);
        return style;
    }
    ...
    String ComputedValue(String name, const ComputedStyle& style)
    {
        ...
        DCHECK(ref.IsValid());
        return ref.GetProperty().CSSValueFromComputedStyle(style, nullptr, false, CSSValuePhase::kComputedValue)->CssText();
    }

    void MatchAllRules(StyleResolverState& state, ElementRuleCollector& collector)
    {
        GetDocument().GetStyleEngine().GetStyleResolver().MatchAllRules(state, collector, false /* include_smil_properties */);
    }
    ...
    const Length& GetTop(const ComputedStyle& style) const
    {
        return style.Top();
    }
    ...
    {
        ScopedCSSName* scoped_name = MakeGarbageCollected<ScopedCSSName>(try_name, &GetDocument());
        StyleRulePositionTry* rule = GetStyleEngine().GetPositionTryRule(*scoped_name);
        CHECK(rule);
        GetStyleEngine().UpdateStyleForOutOfFlow(element, /* try_set */ &rule->Properties(), kNoTryTactics,
...
TEST_F(StyleResolverTest, StyleForTextInDisplayNone)
{
    ...
    EXPECT_TRUE(GetDocument().body()->GetComputedStyle()->IsEnsuredInDisplayNone());
    EXPECT_FALSE(GetStyleEngine().GetStyleResolver().StyleForText(To<Text>(GetDocument().body()->firstChild())));
}
...
TEST_F(StyleResolverTest, AnimationBaseComputedStyle)
{
    ...
    const ComputedStyle* parent_style = GetDocument().documentElement()->GetComputedStyle();
    ...
    EXPECT_EQ(10, resolver.ResolveStyle(div, recalc_context, style_request)->FontSize());
    ASSERT_TRUE(style1->GetBaseComputedStyle());
    EXPECT_EQ(20, style1->GetBaseComputedStyle()->FontSize());
...
TEST_F(StyleResolverTest, HasEmUnits)
{
    ...
    GetDocument().documentElement()->setInnerHTML("<div id=div style='width:1em'>Test</div>");
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(StyleResolverTest, BaseReusableIfFontRelativeUnitsAbsent)
{
    ...
    auto* effect = CreateSimpleKeyframeEffectForTest(div, CSSPropertyID::kFontSize, "50px", "100px");
...
TEST_F(StyleResolverTest, AnimationNotMaskedByImportant)
{
    ...
    auto* effect = CreateSimpleKeyframeEffectForTest(div, CSSPropertyID::kWidth, "50px", "100px");
    GetDocument().Timeline().Play(effect);
    UpdateAllLifecyclePhasesForTest();
    ...
    EXPECT_FALSE(CSSAnimations::IsAnimatingStandardProperties(div->GetElementAnimations(), bitset, KeyframeEffect::kDefaultPriority));
...
TEST_F(StyleResolverTest, AnimationNotMaskedWithoutElementAnimations)
{
    EXPECT_FALSE(CSSAnimations::IsAnimatingStandardProperties(
        /* ElementAnimations */ nullptr, std::make_unique<CSSBitset>().get(), KeyframeEffect::kDefaultPriority));
}
...
TEST_F(StyleResolverTest, AnimationNotMaskedWithoutBitset)
{
    ...
    EXPECT_FALSE(CSSAnimations::IsAnimatingStandardProperties(div->GetElementAnimations(), /* CSSBitset */ nullptr, KeyframeEffect::kDefaultPriority));
}
...
TEST_F(StyleResolverTest, AnimationMaskedByImportant)
{
    ...
    auto* effect = CreateSimpleKeyframeEffectForTest(div, CSSPropertyID::kHeight, "50px", "100px");
    GetDocument().Timeline().Play(effect);
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(StyleResolverTest, TransitionRetargetRelativeFontSizeOnParentlessElement)
{
    GetDocument().documentElement()->setInnerHTML(R"HTML(
    <style>
      html {
    ...
    transition->setCurrentTime(MakeGarbageCollected<V8CSSNumberish>(50), ASSERT_NO_EXCEPTION);
    transition->pause();
    UpdateAllLifecyclePhasesForTest();
    const String before_reversal_font_size = ComputedValue("font-size", *StyleForId("target"));
    ...
    element_animations = element->GetElementAnimations();
    EXPECT_TRUE(element_animations);
    Animation* reverse_transition = (*element_animations->Animations().begin()).key;
    EXPECT_TRUE(reverse_transition);
    EXPECT_EQ(before_reversal_font_size, ComputedValue("font-size", *StyleForId("target")));
}

class StyleResolverFontRelativeUnitTest : public testing::WithParamInterface<const char*>, public StyleResolverTest { };

TEST_P(StyleResolverFontRelativeUnitTest, BaseNotReusableIfFontRelativeUnitPresent)
{
    GetDocument().documentElement()->setInnerHTML(String::Format("<div id=div style='width:1%s'>Test</div>", GetParam()));
    UpdateAllLifecyclePhasesForTest();
...
TEST_P(StyleResolverFontRelativeUnitTest, BaseReusableIfNoFontAffectingAnimation)
{
    GetDocument().documentElement()->setInnerHTML(String::Format("<div id=div style='width:1%s'>Test</div>", GetParam()));
    UpdateAllLifecyclePhasesForTest();
...
INSTANTIATE_TEST_SUITE_P(All, StyleResolverFontRelativeUnitTest, testing::Values("em", "rem", "ex", "ch"));

namespace {

const CSSImageValue& GetBackgroundImageValue(const ComputedStyle& style)
{
    const CSSValue* computed_value = ComputedStyleUtils::ComputedPropertyValue(GetCSSPropertyBackgroundImage(), style);
...
TEST_F(StyleResolverTest, BackgroundImageFetch)
{
    ...
    auto* none_image_set = GetDocument().getElementById(AtomicString("none-image-set"));
    auto* hidden = GetDocument().getElementById(AtomicString("hidden"));
    auto* inside_hidden = GetDocument().getElementById(AtomicString("inside-hidden"));
    auto* contents = GetDocument().getElementById(AtomicString("contents"));
    auto* inside_contents = GetDocument().getElementById(AtomicString("inside-contents"));
    ...
    auto* first_line_span = GetDocument().getElementById(AtomicString("first-line-span"));
    auto* first_line_none = GetDocument().getElementById(AtomicString("first-line-none"));
    ...
    auto* first_line_span_style = first_line_span->EnsureComputedStyle(kPseudoIdFirstLine);
    auto* first_line_none_style = first_line_none->EnsureComputedStyle(kPseudoIdFirstLine);

    ASSERT_TRUE(before_style);
    EXPECT_TRUE(GetBackgroundImageValue(*before_style).IsCachePending()) << "No fetch for non-generated ::before";
    ASSERT_TRUE(first_line_style);
    EXPECT_FALSE(GetBackgroundImageValue(*first_line_style).IsCachePending()) << "Fetched by layout of ::first-line";
    ASSERT_TRUE(first_line_span_style);
    EXPECT_TRUE(GetBackgroundImageValue(*first_line_span_style).IsCachePending()) << "No fetch for inline with ::first-line";
    ASSERT_TRUE(first_line_none_style);
    EXPECT_TRUE(GetBackgroundImageValue(*first_line_none_style).IsCachePending()) << "No fetch for display:none with ::first-line";
    EXPECT_TRUE(GetBackgroundImageValue(none).IsCachePending()) << "No fetch for display:none";
    EXPECT_TRUE(GetBackgroundImageValue(inside_none).IsCachePending()) << "No fetch inside display:none";
    EXPECT_TRUE(GetBackgroundImageSetValue(none_image_set).IsCachePending(1.0f)) << "No fetch for display:none";
    EXPECT_FALSE(GetBackgroundImageValue(hidden).IsCachePending()) << "Fetch for visibility:hidden";
    EXPECT_FALSE(GetBackgroundImageValue(inside_hidden).IsCachePending()) << "Fetch for inherited visibility:hidden";
    EXPECT_FALSE(GetBackgroundImageValue(contents).IsCachePending()) << "Fetch for display:contents";
    EXPECT_FALSE(GetBackgroundImageValue(inside_contents).IsCachePending()) << "Fetch for image inherited from display:contents";
    EXPECT_TRUE(GetBackgroundImageValue(non_slotted).IsCachePending()) << "No fetch for element outside the flat tree";
    ...
    EXPECT_FALSE(GetBackgroundImageValue(frameset1).IsCachePending()) << "Fetch for display:none frameset";
    EXPECT_FALSE(GetBackgroundImageValue(frameset2).IsCachePending()) << "Fetch for display:none frameset - cached";
}
...
TEST_F(StyleResolverTest, FetchForAtPage)
{
    ...
    UpdateAllLifecyclePhasesForTest();
    const ComputedStyle* page_style = GetDocument().GetStyleResolver().StyleForPage(0, g_empty_atom);
    ASSERT_TRUE(page_style);
    const CSSValue* computed_value = ComputedStyleUtils::ComputedPropertyValue(GetCSSPropertyBackgroundImage(), *page_style);
...
TEST_F(StyleResolverTest, NoFetchForAtPage)
{
    ...
    const ComputedStyle* page_style = GetDocument().GetStyleResolver().StyleForPage(0, g_empty_atom);
    ASSERT_TRUE(page_style);
    const CSSValue* computed_value = ComputedStyleUtils::ComputedPropertyValue(GetCSSPropertyListStyleImage(), *page_style);
...
TEST_F(StyleResolverTest, PageComputedStyle)
{
    ...
    UpdateAllLifecyclePhasesForTest();
    const ComputedStyle* style = GetDocument().GetStyleResolver().StyleForPage(0, g_empty_atom);
    ASSERT_TRUE(style);
    ...
    String font_family = ComputedStyleUtils::ValueForFontFamily(style->GetFontDescription().Family())->CssText();
    EXPECT_EQ(font_family, R"(cursive, fantasy, monospace, sans-serif, serif, UnquotedFont, "QuotedFont\",")");
...
TEST_F(StyleResolverTest, NoFetchForHighlightPseudoElements)
{
    ...
    const ComputedStyle* target_text_style
        = GetDocument().GetStyleResolver().ResolveStyle(GetDocument().body(), StyleRecalcContext(), target_text_style_request);
    ...
    const ComputedStyle* selection_style
        = GetDocument().GetStyleResolver().ResolveStyle(GetDocument().body(), StyleRecalcContext(), selection_style_style_request);
    ...
    for (const auto* pseudo_style : { target_text_style, selection_style }) {
        ...
        EXPECT_EQ(Color(0, 128, 0), pseudo_style->VisitedDependentColor(GetCSSPropertyColor()));

        // Check that the background-image does not apply.
        const CSSValue* computed_value = ComputedStyleUtils::ComputedPropertyValue(GetCSSPropertyBackgroundImage(), *pseudo_style);
...
TEST_F(StyleResolverTest, CSSMarkerPseudoElement)
{
    ...
    for (unsigned i = 0; i < lis->length(); ++i) {
        ...
        PseudoElement* before = li->QuerySelector(AtomicString("b"))->GetPseudoElement(kPseudoIdBefore);
        ...
        EXPECT_EQ(marker->GetComputedStyle()->GetUnicodeBidi(), UnicodeBidi::kIsolate);
        EXPECT_EQ(nested_marker->GetComputedStyle()->GetUnicodeBidi(), UnicodeBidi::kIsolate);
    }
...
TEST_F(StyleResolverTest, EnsureComputedStyleSlotFallback)
{
    ...
    ShadowRoot& shadow_root = GetDocument().getElementById(AtomicString("host"))->AttachShadowRootForTesting(ShadowRootMode::kOpen);
    ...
    EXPECT_EQ(Color::kBlack, fallback_style->VisitedDependentColor(GetCSSPropertyColor()));
}
...
TEST_F(StyleResolverTest, ComputeValueStandardProperty)
{
    ...
    MutableCSSPropertyValueSet::SetResult result = set->ParseAndSetProperty(property_id, "var(--color)", false, SecureContextMode::kInsecureContext,
    ...
    const CSSValue* computed_value = StyleResolver::ComputeValue(target, CSSPropertyName(property_id), *parsed_value);
...
const CSSValue* ParseCustomProperty(Document& document, const CustomProperty& property, const String& value)
{
...
TEST_F(StyleResolverTest, ComputeValueCustomProperty)
{
    ...
    const CSSValue* parsed_value = ParseCustomProperty(GetDocument(), CustomProperty(custom_property_name, GetDocument()), "blue");
    ASSERT_TRUE(parsed_value);
    const CSSValue* computed_value = StyleResolver::ComputeValue(target, CSSPropertyName(custom_property_name), *parsed_value);
    ASSERT_TRUE(computed_value);
...
TEST_F(StyleResolverTest, TreeScopedReferences)
{
    ...
    ShadowRoot& inner_root = inner_host->AttachShadowRootForTesting(ShadowRootMode::kOpen);
...

    {
        ...
        ElementRuleCollector collector(state.ElementContext(), StyleRecalcContext(), filter, match_result, EInsideLink::kNotInsideLink);
        GetDocument().GetStyleEngine().GetStyleResolver().MatchAllRules(state, collector, false /* include_smil_properties */);
        ...
        // :host { font-family: myfont }
        EXPECT_EQ(match_result.ScopeFromTreeOrder(properties[2].data_.tree_order), root.GetTreeScope());
        EXPECT_EQ(properties[2].data_.origin, CascadeOrigin::kAuthor);

        // #host { animation-name: anim }
        EXPECT_EQ(properties[3].data_.origin, CascadeOrigin::kAuthor);
        EXPECT_EQ(match_result.ScopeFromTreeOrder(properties[3].data_.tree_order), host->GetTreeScope());
    }

    {
        ...
        EXPECT_EQ(match_result.ScopeFromTreeOrder(properties[0].data_.tree_order), inner_root.GetTreeScope());

        // ::slotted(span) { animation-name: anim-slotted }
        EXPECT_EQ(properties[1].data_.origin, CascadeOrigin::kAuthor);
        EXPECT_EQ(match_result.ScopeFromTreeOrder(properties[1].data_.tree_order), root.GetTreeScope());
    }
}
...
TEST_F(StyleResolverTest, InheritStyleImagesFromDisplayContents)
{
    ...
    EXPECT_FALSE(style->BackgroundLayers().GetImage()->IsPendingImage()) << "background-image is fetched";

    ASSERT_TRUE(style->BorderImageSource());
    EXPECT_FALSE(style->BorderImageSource()->IsPendingImage()) << "border-image-source is fetched";

    ASSERT_TRUE(style->Cursors());
    ASSERT_TRUE(style->Cursors()->size());
    ASSERT_TRUE(style->Cursors()->at(0).GetImage());
    EXPECT_FALSE(style->Cursors()->at(0).GetImage()->IsPendingImage()) << "cursor is fetched";

    ASSERT_TRUE(style->ListStyleImage());
    EXPECT_FALSE(style->ListStyleImage()->IsPendingImage()) << "list-style-image is fetched";

    ASSERT_TRUE(style->ShapeOutside());
    ASSERT_TRUE(style->ShapeOutside()->GetImage());
    EXPECT_FALSE(style->ShapeOutside()->GetImage()->IsPendingImage()) << "shape-outside is fetched";

    ASSERT_TRUE(style->BoxReflect());
    ASSERT_TRUE(style->BoxReflect()->Mask().GetImage());
    EXPECT_FALSE(style->BoxReflect()->Mask().GetImage()->IsPendingImage()) << "-webkit-box-reflect is fetched";

    ASSERT_TRUE(style->MaskBoxImageSource());
    EXPECT_FALSE(style->MaskBoxImageSource()->IsPendingImage()) << "-webkit-mask-box-image-source";

    ASSERT_TRUE(style->MaskLayers().GetImage());
    EXPECT_FALSE(style->MaskLayers().GetImage()->IsPendingImage()) << "-webkit-mask-image is fetched";
}

TEST_F(StyleResolverTest, TextShadowInHighlightPseudoNotCounted1)
{
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kTextShadowInHighlightPseudo));
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kTextShadowNotNoneInHighlightPseudo));
    ...
    const ComputedStyle* selection_style = GetDocument().GetStyleResolver().ResolveStyle(target, StyleRecalcContext(), pseudo_style_request);
...
TEST_F(StyleResolverTest, TextShadowInHighlightPseudotNone)
{
    ...
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kTextShadowInHighlightPseudo));
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kTextShadowNotNoneInHighlightPseudo));
}
...
TEST_F(StyleResolverTest, TextShadowInHighlightPseudoNotNone1)
{
    ...
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kTextShadowInHighlightPseudo));
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kTextShadowNotNoneInHighlightPseudo));
}
...
TEST_F(StyleResolverTest, NoCascadeLayers)
{
    ...
    StyleResolverState state(GetDocument(), *GetDocument().getElementById(AtomicString("a")));
    SelectorFilter filter;
    MatchResult match_result;
    ElementRuleCollector collector(state.ElementContext(), StyleRecalcContext(), filter, match_result, EInsideLink::kNotInsideLink);
    ...
    const uint16_t kImplicitOuterLayerOrder = ClampTo<uint16_t>(CascadeLayerMap::kImplicitOuterLayerOrder);
    ...
    // div { unicode-bidi: isolate; }
    EXPECT_TRUE(properties[1].properties->HasProperty(CSSPropertyID::kUnicodeBidi));
...
TEST_F(StyleResolverTest, CascadeLayersInDifferentSheets)
{
    ...
    EXPECT_TRUE(properties[4].properties->HasProperty(CSSPropertyID::kFontFamily));
...
TEST_F(StyleResolverTest, CascadeLayersInDifferentTreeScopes)
{
    ...
    StyleResolverState state(GetDocument(), *GetDocument().getElementById(AtomicString("host")));
    SelectorFilter filter;
    MatchResult match_result;
    ElementRuleCollector collector(state.ElementContext(), StyleRecalcContext(), filter, match_result, EInsideLink::kNotInsideLink);
    MatchAllRules(state, collector);
    ...
    EXPECT_EQ(0u, properties[2].data_.layer_order);
    EXPECT_EQ(properties[2].data_.origin, CascadeOrigin::kAuthor);
    EXPECT_EQ(match_result.ScopeFromTreeOrder(properties[2].data_.tree_order), GetDocument().getElementById(AtomicString("host"))->GetShadowRoot());

    // @layer foo { #host { color: green } }
    EXPECT_TRUE(properties[3].properties->HasProperty(CSSPropertyID::kColor));
    EXPECT_EQ(0u, properties[3].data_.layer_order);
    EXPECT_EQ(match_result.ScopeFromTreeOrder(properties[3].data_.tree_order), &GetDocument());
}
...
TEST_F(StyleResolverTest, CascadeLayersAfterModifyingAnotherSheet)
{
    ...
    GetDocument().getElementById(AtomicString("addrule"))->appendChild(GetDocument().createTextNode("target { font-size: 10px; }"));

    UpdateAllLifecyclePhasesForTest();

    ASSERT_TRUE(GetDocument().GetScopedStyleResolver()->GetCascadeLayerMap());

    StyleResolverState state(GetDocument(), *GetDocument().QuerySelector(AtomicString("target")));
    SelectorFilter filter;
    MatchResult match_result;
    ElementRuleCollector collector(state.ElementContext(), StyleRecalcContext(), filter, match_result, EInsideLink::kNotInsideLink);
    MatchAllRules(state, collector);
...
TEST_F(StyleResolverTest, CascadeLayersAddLayersWithImportantDeclarations)
{
    ...
    GetDocument()
        ...
        ->appendChild(GetDocument().createTextNode("@layer { target { font-size: 20px !important; } }"
    ...
    EXPECT_TRUE(properties[0].properties->PropertyIsImportant(CSSPropertyID::kFontSize));
    EXPECT_EQ("20px", properties[0].properties->GetPropertyValue(CSSPropertyID::kFontSize));
    EXPECT_EQ(0u, properties[0].data_.layer_order);
    EXPECT_EQ(properties[0].data_.origin, CascadeOrigin::kAuthor);

    // @layer { target { font-size: 10px !important } }
    EXPECT_TRUE(properties[1].properties->HasProperty(CSSPropertyID::kFontSize));
    EXPECT_TRUE(properties[1].properties->PropertyIsImportant(CSSPropertyID::kFontSize));
    EXPECT_EQ("10px", properties[1].properties->GetPropertyValue(CSSPropertyID::kFontSize));
...
{
    GetDocument().documentElement()->setAttribute(html_names::kStyleAttr, AtomicString("contain:size; display:inline-table; content:url(img);"));
    GetDocument().body()->SetInlineStyleProperty(CSSPropertyID::kBackgroundColor, "red");

    // Should not trigger DCHECK
    UpdateAllLifecyclePhasesForTest();

    EXPECT_EQ(Color::kTransparent, GetDocument().GetLayoutView()->StyleRef().VisitedDependentColor(GetCSSPropertyBackgroundColor()));
}
...
TEST_F(StyleResolverTest, IsInertWithAttributeAndDialog)
{
    ...
    auto* dialog = To<HTMLDialogElement>(document.QuerySelector(AtomicString("dialog")));
...
TEST_F(StyleResolverTest, IsInertWithDialogs)
{
    ...
    StaticElementList* dialogs = document.QuerySelectorAll(AtomicString("dialog"));
    Element* html = document.documentElement();
    Element* body = document.body();
...
{
    LocalFrame::NotifyUserActivation(document.GetFrame(), mojom::UserActivationNotificationType::kTest);
...
TEST_F(StyleResolverTestCQ, StyleRulesForElementContainerQuery)
{
    ...
    auto* rule_list = resolver.StyleRulesForElement(target, StyleResolver::kAuthorCSSRules);
    ASSERT_TRUE(rule_list);
    ASSERT_EQ(rule_list->size(), 1u) << "The empty #target rule in the container query should be collected";
    EXPECT_TRUE(rule_list->at(0)->Properties().IsEmpty()) << "Check that it is in fact the empty rule";
}
...
TEST_F(StyleResolverTest, LegacyOverlapPerspectiveOrigin_Single)
{
    ...
    EXPECT_FALSE(IsUseCounted(WebFeature::kCSSLegacyPerspectiveOrigin)) << "Not counted when only perspective-origin is used";
}
...
TEST_F(StyleResolverTest, LegacyOverlapPerspectiveOrigin_Order)
{
    ...
    EXPECT_FALSE(IsUseCounted(WebFeature::kCSSLegacyPerspectiveOrigin)) << "Not counted when perspective-origin is last";
}
...
TEST_F(StyleResolverTest, LegacyOverlapPerspectiveOrigin_Values)
{
    ...
    EXPECT_FALSE(IsUseCounted(WebFeature::kCSSLegacyPerspectiveOrigin)) << "Not counted when values are the same";
}
...
TEST_F(StyleResolverTest, LegacyOverlapPerspectiveOrigin_Last)
{
    ...
    EXPECT_TRUE(IsUseCounted(WebFeature::kCSSLegacyPerspectiveOrigin)) << "Counted when -webkit-perspective-* is last with different values";
}
...
TEST_F(StyleResolverTest, LegacyOverlapTransformOrigin_Single)
{
    ...
    EXPECT_FALSE(IsUseCounted(WebFeature::kCSSLegacyTransformOrigin)) << "Not counted when only transform-origin is used";
}
...
TEST_F(StyleResolverTest, LegacyOverlapTransformOrigin_Order)
{
    ...
    EXPECT_FALSE(IsUseCounted(WebFeature::kCSSLegacyTransformOrigin)) << "Not counted when transform-origin is last";
}
...
TEST_F(StyleResolverTest, LegacyOverlapTransformOrigin_Values)
{
    ...
    EXPECT_FALSE(IsUseCounted(WebFeature::kCSSLegacyTransformOrigin)) << "Not counted when values are the same";
}
...
TEST_F(StyleResolverTest, LegacyOverlapTransformOrigin_Last)
{
    ...
    EXPECT_TRUE(IsUseCounted(WebFeature::kCSSLegacyTransformOrigin)) << "Counted when -webkit-transform-origin-* is last with different "
                                                                        "values";
}
...
TEST_F(StyleResolverTest, LegacyOverlapBorderImage_Single)
{
    ...
    EXPECT_FALSE(IsUseCounted(WebFeature::kCSSLegacyBorderImage)) << "Not counted when only border-image is used";
}
...
TEST_F(StyleResolverTest, LegacyOverlapBorderImage_Order)
{
    ...
    EXPECT_FALSE(IsUseCounted(WebFeature::kCSSLegacyBorderImage)) << "Not counted when border-image is last";
}
...
TEST_F(StyleResolverTest, LegacyOverlapBorderImage_Values)
{
    ...
    EXPECT_FALSE(IsUseCounted(WebFeature::kCSSLegacyBorderImage)) << "Not counted when values are the same";
}
...
TEST_F(StyleResolverTest, LegacyOverlapBorderImage_Last_Source)
{
    ...
    EXPECT_TRUE(IsUseCounted(WebFeature::kCSSLegacyBorderImage)) << "Counted when border-image-source differs";
}
...
TEST_F(StyleResolverTest, LegacyOverlapBorderImage_Last_Slice)
{
    ...
    EXPECT_TRUE(IsUseCounted(WebFeature::kCSSLegacyBorderImage)) << "Counted when border-image-slice differs";
}
...
TEST_F(StyleResolverTest, LegacyOverlapBorderImage_Last_SliceFill)
{
    ...
    EXPECT_TRUE(IsUseCounted(WebFeature::kCSSLegacyBorderImage)) << "Counted when the fill keyword of border-image-slice differs";
}
...
TEST_F(StyleResolverTest, LegacyOverlapBorderImage_SliceFillImplicit)
{
    ...
    EXPECT_TRUE(IsUseCounted(WebFeature::kCSSLegacyBorderImage)) << "Counted when fill-less values are the same";
}
...
TEST_F(StyleResolverTest, LegacyOverlapBorderImage_Last_Outset)
{
    ...
    EXPECT_TRUE(IsUseCounted(WebFeature::kCSSLegacyBorderImage)) << "Counted when border-image-outset differs";
}
...
TEST_F(StyleResolverTest, LegacyOverlapBorderImage_Last_Repeat)
{
    ...
    EXPECT_TRUE(IsUseCounted(WebFeature::kCSSLegacyBorderImage)) << "Counted when border-image-repeat differs";
}
...
TEST_F(StyleResolverTest, LegacyOverlapBorderImageWidth_Single)
{
    ...
    EXPECT_FALSE(IsUseCounted(WebFeature::kCSSLegacyBorderImageWidth)) << "Not counted when only border is used";
}
...
TEST_F(StyleResolverTest, LegacyOverlapBorderImageWidth_Order)
{
    ...
    EXPECT_FALSE(IsUseCounted(WebFeature::kCSSLegacyBorderImageWidth)) << "Not counted when border is last";
}
...
TEST_F(StyleResolverTest, LegacyOverlapBorderImageWidth_Values)
{
    ...
    EXPECT_FALSE(IsUseCounted(WebFeature::kCSSLegacyBorderImageWidth)) << "Not counted when values are the same";
}
...
TEST_F(StyleResolverTest, LegacyOverlapBorderImageWidth_Last_Border)
{
    ...
    // always wins.
    EXPECT_FALSE(IsUseCounted(WebFeature::kCSSLegacyBorderImageWidth)) << "Not even counted when -webkit-border-image is last";
}
...
TEST_F(StyleResolverTest, LegacyOverlapBorderImageWidth_Last_Style)
{
    ...
    EXPECT_TRUE(IsUseCounted(WebFeature::kCSSLegacyBorderImageWidth)) << "Counted when -webkit-border-image is last and there's no "
                                                                         "border-width";
}
...
TEST_F(StyleResolverTest, TrySet_Basic)
{
    ...
    const CSSPropertyValueSet* try_set = css_test_helpers::ParseDeclarationBlock(R"CSS(
      left: 20px;
      right: 30px;
  )CSS");
    ASSERT_TRUE(try_set);

    const ComputedStyle* try_style = StyleForId("div", StyleRecalcContext { .try_set = try_set, .is_interleaved_oof = true });
...
TEST_F(StyleResolverTest, TryTacticsSet_Flip)
{
    ...
    auto* try_tactics_set = MakeGarbageCollected<MutableCSSPropertyValueSet>(kHTMLStandardMode);
    try_tactics_set->SetProperty(CSSPropertyID::kLeft, *MakeGarbageCollected<cssvalue::CSSFlipRevertValue>(CSSPropertyID::kRight, TryTacticTransform()));
    try_tactics_set->SetProperty(CSSPropertyID::kRight, *MakeGarbageCollected<cssvalue::CSSFlipRevertValue>(CSSPropertyID::kLeft, TryTacticTransform()));
    ASSERT_TRUE(try_tactics_set);

    const ComputedStyle* try_style
        = StyleForId("div", StyleRecalcContext { .try_set = try_set, .try_tactics_set = try_tactics_set, .is_interleaved_oof = true });
    ASSERT_TRUE(try_style);
    EXPECT_EQ("200px", ComputedValue("left", *try_style));
    EXPECT_EQ("100px", ComputedValue("right", *try_style));
}

TEST_F(StyleResolverTest, PseudoElementWithAnimationAndOriginatingElementStyleChange)
{
    SetBodyInnerHTML(R"HTML(
      <style>
        div {
...
TEST_F(StyleResolverTest, ScopedAnchorName)
{
    ...
    EXPECT_EQ(*MakeGarbageCollected<ScopedCSSName>(AtomicString("--outer"), &GetDocument()), *outer_anchor->ComputedStyleRef().AnchorName()->GetNames()[0]);
    EXPECT_EQ(*MakeGarbageCollected<ScopedCSSName>(AtomicString("--host"), shadow), *host->ComputedStyleRef().AnchorName()->GetNames()[0]);
    EXPECT_EQ(*MakeGarbageCollected<ScopedCSSName>(AtomicString("--part"), &GetDocument()), *part->ComputedStyleRef().AnchorName()->GetNames()[0]);
    EXPECT_EQ(*MakeGarbageCollected<ScopedCSSName>(AtomicString("--inner"), shadow), *inner_anchor->ComputedStyleRef().AnchorName()->GetNames()[0]);
}
...
TEST_F(StyleResolverTest, ScopedPositionAnchor)
{
    ...
    EXPECT_EQ(*MakeGarbageCollected<ScopedCSSName>(AtomicString("--outer"), &GetDocument()), *outer_anchor->ComputedStyleRef().PositionAnchor());
    EXPECT_EQ(*MakeGarbageCollected<ScopedCSSName>(AtomicString("--host"), shadow), *host->ComputedStyleRef().PositionAnchor());
    EXPECT_EQ(*MakeGarbageCollected<ScopedCSSName>(AtomicString("--part"), &GetDocument()), *part->ComputedStyleRef().PositionAnchor());
    EXPECT_EQ(*MakeGarbageCollected<ScopedCSSName>(AtomicString("--inner"), shadow), *inner_anchor->ComputedStyleRef().PositionAnchor());
}
...
TEST_F(StyleResolverTest, CssRulesForElementIncludeStartingStyle)
{
    ...
    EXPECT_NE(GetStyleEngine().GetStyleResolver().CssRulesForElement(target), nullptr);
...
TEST_F(StyleResolverTest, PseudoCSSRulesForElementIncludeStartingStyle)
{
    ...
    RuleIndexList* pseudo_rules = GetStyleEngine().GetStyleResolver().PseudoCSSRulesForElement(target, kPseudoIdBefore, g_null_atom);
    ...
    pseudo_rules = GetStyleEngine().GetStyleResolver().PseudoCSSRulesForElement(target, kPseudoIdBefore, g_null_atom);
    ASSERT_NE(pseudo_rules, nullptr);
    EXPECT_EQ(pseudo_rules->size(), 2u);
    EXPECT_EQ(pseudo_rules->at(0).first->cssText(), "#target::before { color: red; }");
    EXPECT_EQ(pseudo_rules->at(1).first->cssText(), "#target::before { content: \"X\"; color: green; }");
}

TEST_F(StyleResolverTest, ResizeAutoInUANotCounted)
{
    SetBodyInnerHTML(R"HTML(<textarea></textarea>)HTML");
    EXPECT_FALSE(IsUseCounted(WebFeature::kCSSResizeAuto)) << "resize:auto UA rule for textarea should not be counted";
}
...
TEST_F(StyleResolverTest, ResizeAutoCounted)
{
    ...
    EXPECT_TRUE(IsUseCounted(WebFeature::kCSSResizeAuto)) << "Author style resize:auto applied to div should be counted";
}
...
TEST_F(StyleResolverTest, TextSizeAdjustUseCounter)
{
    ...
    Element* target = GetDocument().getElementById(AtomicString("target"));
    target->setAttribute(html_names::kClassAttr, AtomicString("text-size-adjust-100"));
    UpdateAllLifecyclePhasesForTest();

    EXPECT_TRUE(IsUseCounted(WebFeature::kTextSizeAdjustNotAuto));
    EXPECT_FALSE(IsUseCounted(WebFeature::kTextSizeAdjustPercentNot100));

    target->setAttribute(html_names::kClassAttr, AtomicString("text-size-adjust-101"));
    UpdateAllLifecyclePhasesForTest();
