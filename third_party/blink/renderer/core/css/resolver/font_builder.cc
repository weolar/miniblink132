...
FontFamily FontBuilder::StandardFontFamily() const
{
    ...
    return FontFamily(standard_font_family, FontFamily::InferredTypeFor(standard_font_family));
}
...
AtomicString FontBuilder::GenericFontFamilyName(FontDescription::GenericFamilyType generic_family) const
{
...
float FontBuilder::FontSizeForKeyword(unsigned keyword, bool is_monospace) const
{
    return FontSizeFunctions::FontSizeForKeyword(document_, keyword, is_monospace);
}

void FontBuilder::SetFamilyDescription(const FontDescription::FamilyDescription& family_description)
{
...
void FontBuilder::SetFontSynthesisWeight(FontDescription::FontSynthesisWeight font_synthesis_weight)
{
...
void FontBuilder::SetFontSynthesisStyle(FontDescription::FontSynthesisStyle font_synthesis_style)
{
...
void FontBuilder::SetFontSynthesisSmallCaps(FontDescription::FontSynthesisSmallCaps font_synthesis_small_caps)
{
...
void FontBuilder::SetFontVariantAlternates(scoped_refptr<const FontVariantAlternates> variant_alternates)
{
...
void FontBuilder::SetFeatureSettings(scoped_refptr<const FontFeatureSettings> settings)
{
    Set(PropertySetFlag::kFeatureSettings);
    font_description_.SetFeatureSettings(std::move(settings));
}

void FontBuilder::SetVariationSettings(scoped_refptr<const FontVariationSettings> settings)
{
    Set(PropertySetFlag::kVariationSettings);
    font_description_.SetVariationSettings(std::move(settings));
}

void FontBuilder::SetFamilyDescription(FontDescription& font_description, const FontDescription::FamilyDescription& family_description)
{
    Set(PropertySetFlag::kFamily);

    bool is_initial = family_description.generic_family == FontDescription::kStandardFamily && family_description.family.FamilyName().empty();

    font_description.SetGenericFamily(family_description.generic_family);
    font_description.SetFamily(is_initial ? StandardFontFamily() : family_description.family);
}

void FontBuilder::SetSize(FontDescription& font_description, const FontDescription::Size& size)
{
...
void FontBuilder::SetVariantPosition(FontDescription::FontVariantPosition variant_position)
{
...
float FontBuilder::GetComputedSizeFromSpecifiedSize(const FontDescription& font_description, const ComputedStyleBuilder& builder, float specified_size)
{
    DCHECK(document_);
    ...
    return FontSizeFunctions::GetComputedSizeFromSpecifiedSize(document_, zoom_factor, font_description.IsAbsoluteSize(), specified_size);
}

void FontBuilder::CheckForGenericFamilyChange(const FontDescription& parent_description, FontDescription& new_description)
{
    DCHECK(document_);
    ...
    if (new_description.KeywordSize()) {
        size = FontSizeForKeyword(new_description.KeywordSize(), new_description.IsMonospace());
    } else {
        Settings* settings = document_->GetSettings();
        float fixed_scale_factor = (settings && settings->GetDefaultFixedFontSize() && settings->GetDefaultFontSize())
            ? static_cast<float>(settings->GetDefaultFixedFontSize()) / settings->GetDefaultFontSize()
            : 1;
        size = parent_description.IsMonospace() ? new_description.SpecifiedSize() / fixed_scale_factor : new_description.SpecifiedSize() * fixed_scale_factor;
    }

    new_description.SetSpecifiedSize(size);
}

void FontBuilder::UpdateSpecifiedSize(FontDescription& font_description, const FontDescription& parent_description)
{
    float specified_size = font_description.SpecifiedSize();

    if (!specified_size && font_description.KeywordSize()) {
        specified_size = FontSizeForKeyword(font_description.KeywordSize(), font_description.IsMonospace());
    }
    font_description.SetSpecifiedSize(specified_size);
...
void FontBuilder::UpdateAdjustedSize(FontDescription& font_description, FontSelector* font_selector)
{
    ...
    if (size_adjust.IsFromFont() && size_adjust.Value() == FontSizeAdjust::kFontSizeAdjustNone) {
        std::optional<float> aspect_value = FontSizeFunctions::FontAspectValue(font_data, size_adjust.GetMetric(), font_description.ComputedSize());
        font_description.SetSizeAdjust(FontSizeAdjust(aspect_value.has_value() ? aspect_value.value() : FontSizeAdjust::kFontSizeAdjustNone,
            size_adjust.GetMetric(), FontSizeAdjust::ValueType::kFromFont));
    }

    if (auto adjusted_size = FontSizeFunctions::MetricsMultiplierAdjustedFontSize(font_data, font_description)) {
        font_description.SetAdjustedSize(adjusted_size.value());
    }
}

void FontBuilder::UpdateComputedSize(FontDescription& font_description, const ComputedStyleBuilder& builder)
{
    float computed_size = GetComputedSizeFromSpecifiedSize(font_description, builder, font_description.SpecifiedSize());
    computed_size = TextAutosizer::ComputeAutosizedFontSize(computed_size, builder.TextAutosizingMultiplier(), builder.EffectiveZoom());
    font_description.SetComputedSize(computed_size);
}

bool FontBuilder::UpdateFontDescription(FontDescription& description, FontOrientation font_orientation)
{
    bool modified = false;
    if (IsSet(PropertySetFlag::kFamily)) {
        if (description.GenericFamily() != font_description_.GenericFamily() || description.Family() != font_description_.Family()) {
            modified = true;
    ...
    if (IsSet(PropertySetFlag::kSize)) {
        if (description.KeywordSize() != font_description_.KeywordSize() || description.SpecifiedSize() != font_description_.SpecifiedSize()
    ...
    if (IsSet(PropertySetFlag::kVariantEastAsian)) {
        if (description.VariantEastAsian() != font_description_.VariantEastAsian()) {
            modified = true;
            description.SetVariantEastAsian(font_description_.VariantEastAsian());
        }
    }
    if (IsSet(PropertySetFlag::kVariantLigatures)) {
        if (description.GetVariantLigatures() != font_description_.GetVariantLigatures()) {
            modified = true;
    ...
    if (IsSet(PropertySetFlag::kVariationSettings)) {
        if (description.VariationSettings() != font_description_.VariationSettings()) {
            modified = true;
            description.SetVariationSettings(font_description_.VariationSettings());
        }
    }
    if (IsSet(PropertySetFlag::kFontSynthesisWeight)) {
        if (description.GetFontSynthesisWeight() != font_description_.GetFontSynthesisWeight()) {
            modified = true;
            description.SetFontSynthesisWeight(font_description_.GetFontSynthesisWeight());
        }
    }
    if (IsSet(PropertySetFlag::kFontSynthesisStyle)) {
        if (description.GetFontSynthesisStyle() != font_description_.GetFontSynthesisStyle()) {
            modified = true;
            description.SetFontSynthesisStyle(font_description_.GetFontSynthesisStyle());
        }
    }
    if (IsSet(PropertySetFlag::kFontSynthesisSmallCaps)) {
        if (description.GetFontSynthesisSmallCaps() != font_description_.GetFontSynthesisSmallCaps()) {
            modified = true;
            description.SetFontSynthesisSmallCaps(font_description_.GetFontSynthesisSmallCaps());
        }
    }
    ...
    if (IsSet(PropertySetFlag::kTextSpacingTrim)) {
        if (description.GetTextSpacingTrim() != font_description_.GetTextSpacingTrim()) {
            modified = true;
            description.SetTextSpacingTrim(font_description_.GetTextSpacingTrim());
        }
    }
    if (IsSet(PropertySetFlag::kFontOpticalSizing)) {
        if (description.FontOpticalSizing() != font_description_.FontOpticalSizing()) {
            modified = true;
    ...
    if (IsSet(PropertySetFlag::kFontVariantAlternates)) {
        if (description.GetFontVariantAlternates() != font_description_.GetFontVariantAlternates()) {
            modified = true;
            description.SetFontVariantAlternates(font_description_.GetFontVariantAlternates());
        }
    }
    ...
    if (IsSet(PropertySetFlag::kTextOrientation) || IsSet(PropertySetFlag::kWritingMode)) {
    ...
    if (!modified && !IsSet(PropertySetFlag::kEffectiveZoom) && !IsSet(PropertySetFlag::kTextSizeAdjust)) {
        return false;
    }

    float size = description.SpecifiedSize();
    if (!size && description.KeywordSize()) {
        size = FontSizeForKeyword(description.KeywordSize(), description.IsMonospace());
    }
...
FontSelector* FontBuilder::FontSelectorFromTreeScope(const TreeScope* tree_scope)
{
    ...
    // tree.
    DCHECK(!tree_scope || tree_scope->GetDocument() == document_ || tree_scope->GetDocument().IsSVGDocument());
...
FontSelector* FontBuilder::ComputeFontSelector(const ComputedStyleBuilder& builder)
{
    if (IsSet(PropertySetFlag::kFamily)) {
...
void FontBuilder::CreateFont(ComputedStyleBuilder& builder, const ComputedStyle* parent_style)
{
    DCHECK(document_);
    ...
    const FontDescription& parent_description = parent_style ? parent_style->GetFontDescription() : builder.GetFontDescription();
...
void FontBuilder::CreateInitialFont(ComputedStyleBuilder& builder)
{
    ...
    SetFamilyDescription(font_description, FontBuilder::InitialFamilyDescription());
    SetSize(font_description, FontDescription::Size(FontSizeFunctions::InitialKeywordSize(), 0.0f, false));
