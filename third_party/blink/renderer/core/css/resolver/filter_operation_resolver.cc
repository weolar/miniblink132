...
static void CountFilterUse(FilterOperation::OperationType operation_type, const Document& document)
{
...
double FilterOperationResolver::ResolveNumericArgumentForFunction(const CSSFunctionValue& filter, const CSSLengthResolver& length_resolver)
{
...
FilterOperations FilterOperationResolver::CreateFilterOperations(StyleResolverState& state, const CSSValue& in_value, CSSPropertyID property_id)
{
    ...
    for (auto& curr_value : To<CSSValueList>(in_value)) {
        if (const auto* url_value = DynamicTo<cssvalue::CSSURIValue>(curr_value.Get())) {
            CountFilterUse(FilterOperation::OperationType::kReference, state.GetDocument());

            SVGResource* resource = state.GetElementStyleResources().GetSVGResourceFromValue(property_id, *url_value);
            operations.Operations().push_back(MakeGarbageCollected<ReferenceFilterOperation>(url_value->ValueForSerialization(), resource));
            continue;
        }

        const auto* filter_value = To<CSSFunctionValue>(curr_value.Get());
        FilterOperation::OperationType operation_type = FilterOperationForType(filter_value->FunctionType());
        ...
        case CSSValueID::kHueRotate: {
            operations.Operations().push_back(
                MakeGarbageCollected<BasicColorMatrixFilterOperation>(ResolveNumericArgumentForFunction(*filter_value, conversion_data), operation_type));
            break;
        }
        case CSSValueID::kInvert:
        case CSSValueID::kBrightness:
        ...
        case CSSValueID::kOpacity: {
            operations.Operations().push_back(
                MakeGarbageCollected<BasicComponentTransferFilterOperation>(ResolveNumericArgumentForFunction(*filter_value, conversion_data), operation_type));
            break;
        }
        case CSSValueID::kBlur: {
            Length std_deviation = Length::Fixed(0);
            if (filter_value->length() >= 1) {
                const CSSPrimitiveValue* first_value = DynamicTo<CSSPrimitiveValue>(filter_value->Item(0));
                std_deviation = first_value->ConvertToLength(conversion_data);
            }
            operations.Operations().push_back(MakeGarbageCollected<BlurFilterOperation>(std_deviation));
            break;
        }
        case CSSValueID::kDropShadow: {
            ShadowData shadow = StyleBuilderConverter::ConvertShadow(conversion_data, &state, filter_value->Item(0));
            operations.Operations().push_back(MakeGarbageCollected<DropShadowFilterOperation>(shadow));
            break;
        }
        default:
            NOTREACHED();
...
FilterOperations FilterOperationResolver::CreateOffscreenFilterOperations(const CSSValue& in_value, const Font& font)
{
    FilterOperations operations;
    ...
    float zoom = 1.0f;
    CSSToLengthConversionData::FontSizes font_sizes(kOffScreenCanvasEmFontSize, kOffScreenCanvasRemFontSize, &font, zoom);
    ...
    CSSToLengthConversionData conversion_data(
        WritingMode::kHorizontalTb, font_sizes, line_height_size, viewport_size, container_sizes, anchor_data, 1 /* zoom */, ignored_flags);
    ...
    for (auto& curr_value : To<CSSValueList>(in_value)) {
        ...
        case CSSValueID::kDropShadow: {
            ShadowData shadow = StyleBuilderConverter::ConvertShadow(conversion_data, nullptr, filter_value->Item(0));
            operations.Operations().push_back(MakeGarbageCollected<DropShadowFilterOperation>(shadow));
            break;
        }
        default:
