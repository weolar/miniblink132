...
public:
    ...
    const ComputedStyle* ResolveStyle(Element*, const StyleRecalcContext&, const StyleRequest& = StyleRequest());
    ...
    ComputedStyleBuilder CreateComputedStyleBuilderInheritingFrom(const ComputedStyle& parent_style) const;
    ...
    static CompositorKeyframeValue* CreateCompositorKeyframeValueSnapshot(
        Element&, const ComputedStyle& base_style, const ComputedStyle* parent_style, const PropertyHandle&, const CSSValue*, double offset);
    ...
    const ComputedStyle* StyleForPage(uint32_t page_index, const AtomicString& page_name, float page_fitting_scale = 1.0, bool ignore_author_style = false);
    ...
    void StyleForPageMargins(const ComputedStyle& page_style, uint32_t page_index, const AtomicString& page_name, PageMarginsStyle*);
    ...
    // property.
    const ComputedStyle* StyleForInitialLetterText(const ComputedStyle& initial_letter_box_style, const ComputedStyle& paragraph_style);
    ...
    ComputedStyleBuilder CreateAnonymousStyleBuilderWithDisplay(const ComputedStyle& parent_style, EDisplay);
    const ComputedStyle* CreateAnonymousStyleWithDisplay(const ComputedStyle& parent_style, EDisplay display);

    // Create ComputedStyle for anonymous wrappers between text boxes and
    // display:contents elements.
    const ComputedStyle* CreateInheritedDisplayContentsStyleIfNeeded(const ComputedStyle& parent_style, const ComputedStyle& layout_parent_style);
    ...
    SelectorFilter& GetSelectorFilter()
    {
        return selector_filter_;
    }
    ...
    FindKeyframesRuleResult FindKeyframesRule(const Element*, const Element* animating_element, const AtomicString& animation_name);
    ...
    RuleIndexList* CssRulesForElement(Element*, unsigned rules_to_include = kAllCSSRules);
    RuleIndexList* PseudoCSSRulesForElement(Element*, PseudoId, const AtomicString& view_transition_name, unsigned rules_to_include = kAllCSSRules);
    ...
    HeapHashMap<CSSPropertyName, Member<const CSSValue>> CascadedValuesForElement(Element*, PseudoId);

    Element* FindContainerForElement(Element*, const ContainerSelector&, const TreeScope* selector_tree_scope);
    ...
    bool WasViewportResized() const
    {
        return was_viewport_resized_;
    }
    ...
    static const CSSValue* ComputeValue(Element* element, const CSSPropertyName&, const CSSValue&);
    ...
    static const CSSValue* ResolveValue(Element& element, const ComputedStyle& style, const CSSPropertyName&, const CSSValue&);
    ...
    FilterOperations ComputeFilterOperations(Element*, const Font&, const CSSValue&);

    const ComputedStyle* StyleForInterpolations(Element& element, ActiveInterpolationsMap& animations);
    ...
    StyleRulePositionTry* ResolvePositionTryRule(const TreeScope* tree_scope, AtomicString position_try_name);
...
private:
    ...
    void InitStyle(
        Element& element, const StyleRequest&, const ComputedStyle& source_for_noninherited, const ComputedStyle* parent_style, StyleResolverState& state);

    void ApplyBaseStyle(Element* element, const StyleRecalcContext&, const StyleRequest&, StyleResolverState& state, StyleCascade& cascade);
    void ApplyBaseStyleNoCache(Element* element, const StyleRecalcContext&, const StyleRequest&, StyleResolverState& state, StyleCascade& cascade);
    void ApplyInterpolations(StyleResolverState& state, StyleCascade& cascade, ActiveInterpolationsMap& interpolations);

    void AddMatchedRulesToTracker(const ElementRuleCollector&);

    void CollectPseudoRulesForElement(const Element&, ElementRuleCollector&, PseudoId, const AtomicString& view_transition_name, unsigned rules_to_include);
    ...
    void MatchPseudoPartRules(const Element&, ElementRuleCollector&, bool for_shadow_pseudo = false);
    ...
    void MatchAllRules(StyleResolverState&, ElementRuleCollector&, bool include_smil_properties);
    ...
    public:
        ...
        CacheSuccess(MatchedPropertiesCache::Key key, const CachedMatchedProperties::Entry* cached_matched_properties)
            : key(key)
        ...
        bool IsHit() const
        {
            return cached_matched_properties;
        }
    };

    CacheSuccess ApplyMatchedCache(StyleResolverState&, const StyleRequest&, const MatchResult&);
    void MaybeAddToMatchedPropertiesCache(StyleResolverState&, const MatchedPropertiesCache::Key&);
    ...
    StyleRuleList* CollectMatchingRulesFromUnconnectedRuleSet(StyleResolverState&, RuleSet*, ContainerNode* scope);
    ...
    template <typename Functor> void ForEachUARulesForElement(const Element& element, ElementRuleCollector* collector, Functor& func) const;
