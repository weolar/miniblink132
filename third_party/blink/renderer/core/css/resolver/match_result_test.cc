...
protected:
    ...
    const TreeScope& TreeScopeAt(const MatchResult& result, wtf_size_t index)
    {
        ...
        return result.ScopeFromTreeOrder(result.GetMatchedProperties()[index].data_.tree_order);
    }
...
{
    PageTestBase::SetUp();
    property_sets = MakeGarbageCollected<HeapVector<Member<MutableCSSPropertyValueSet>, 8>>();
    for (unsigned i = 0; i < 8; i++) {
        property_sets->push_back(MakeGarbageCollected<MutableCSSPropertyValueSet>(kHTMLQuirksMode));
    }
}
...
{
    MatchResult result;
    result.AddMatchedProperties(PropertySet(0), { .origin = CascadeOrigin::kUserAgent });
    result.AddMatchedProperties(PropertySet(1), { .origin = CascadeOrigin::kUserAgent });
...
{
    MatchResult result;
    result.AddMatchedProperties(PropertySet(0), { .origin = CascadeOrigin::kAuthor });
    result.AddMatchedProperties(PropertySet(1), { .origin = CascadeOrigin::kAuthor });
...
TEST_F(MatchResultTest, CascadeOriginAll)
{
    ...
    result.AddMatchedProperties(PropertySet(3), { .origin = CascadeOrigin::kAuthor });
    result.AddMatchedProperties(PropertySet(4), { .origin = CascadeOrigin::kAuthor });
    result.AddMatchedProperties(PropertySet(5), { .origin = CascadeOrigin::kAuthor });
...
TEST_F(MatchResultTest, CascadeOriginTreeScopes)
{
    ...
    result.AddMatchedProperties(PropertySet(1), { .origin = CascadeOrigin::kUser });
    result.BeginAddingAuthorRulesForTreeScope(GetDocument());
    result.AddMatchedProperties(PropertySet(2), { .origin = CascadeOrigin::kAuthor });
    result.BeginAddingAuthorRulesForTreeScope(GetDocument());
    result.AddMatchedProperties(PropertySet(3), { .origin = CascadeOrigin::kAuthor });
    result.AddMatchedProperties(PropertySet(4), { .origin = CascadeOrigin::kAuthor });
    result.BeginAddingAuthorRulesForTreeScope(GetDocument());
    result.AddMatchedProperties(PropertySet(5), { .origin = CascadeOrigin::kAuthor });
    result.AddMatchedProperties(PropertySet(6), { .origin = CascadeOrigin::kAuthor });
    result.AddMatchedProperties(PropertySet(7), { .origin = CascadeOrigin::kAuthor });
