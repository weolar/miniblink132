...
TEST_F(StyleBuilderTest, WritingModeChangeDirtiesFont)
{
    ...
    for (const CSSProperty* property : properties) {
        for (const CSSValue* value : values) {
            ...
            StyleResolverState state(GetDocument(), *GetDocument().body(), nullptr /* StyleRecalcContext */, StyleRequest(&parent_style));
            state.SetStyle(GetDocument().GetStyleResolver().InitialStyle());

            // This test assumes that initial 'writing-mode' is not 'vertical-lr'.
            ASSERT_NE(WritingMode::kVerticalLr, state.StyleBuilder().GetWritingMode());
...
TEST_F(StyleBuilderTest, TextOrientationChangeDirtiesFont)
{
    ...
    for (const CSSProperty* property : properties) {
        for (const CSSValue* value : values) {
            ...
            ASSERT_NE(ETextOrientation::kUpright, state.StyleBuilder().GetTextOrientation());
...
TEST_F(StyleBuilderTest, HasExplicitInheritance)
{
    ...
    StyleBuilder::ApplyProperty(GetCSSPropertyBackgroundColor(), state, inherited);
...
TEST_F(StyleBuilderTest, GridTemplateAreasApplyOrder)
{
    ...
    const CSSProperty& grid_template_columns = GetCSSPropertyGridTemplateColumns();

    const CSSValue* grid_template_areas_value = css_test_helpers::ParseLonghand(GetDocument(), grid_template_areas, "'foo' 'bar' 'baz' 'faz'");
    const CSSValue* grid_template_columns_value = css_test_helpers::ParseLonghand(GetDocument(), grid_template_columns, "50px 50px");
    const CSSValue* grid_template_rows_value = css_test_helpers::ParseLonghand(GetDocument(), grid_template_rows, "50px 50px");
    ...
    StyleBuilder::ApplyProperty(grid_template_areas, state, *grid_template_areas_value);
    StyleBuilder::ApplyProperty(grid_template_columns, state, *grid_template_columns_value);
    StyleBuilder::ApplyProperty(grid_template_rows, state, *grid_template_rows_value);
    ...
    ASSERT_TRUE(style1);
    ASSERT_TRUE(style2);
    EXPECT_EQ(*style1, *style2) << "Application order of grid properties does not affect result";
}
