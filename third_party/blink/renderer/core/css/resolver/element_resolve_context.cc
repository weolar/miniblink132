...
ElementResolveContext::PseudoElementAncestors ElementResolveContext::BuildPseudoElementAncestors(Element* element)
{
...
EInsideLink GetLinkStateForElement(Element& element)
{
    ...
    probe::ForcePseudoState(&element, CSSSelector::kPseudoVisited, &force_visited);
    if (force_visited) {
    ...
    return element.GetDocument().GetVisitedLinkState().DetermineLinkState(element);
}
} // namespace

ElementResolveContext::ElementResolveContext(Element& element)
    : element_(&element)
    , ultimate_originating_element_(element_->IsPseudoElement() ? To<PseudoElement>(element_)->UltimateOriginatingElement() : element_)
