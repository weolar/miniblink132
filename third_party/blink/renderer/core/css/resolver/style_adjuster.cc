...
TouchAction AdjustTouchActionForElement(TouchAction touch_action, const ComputedStyleBuilder& builder, const ComputedStyle& parent_style, Element* element)
{
    ...
    if (element == element->GetDocument().FirstBodyElement()) {
        ...
        if (parent_style.IsOverflowVisibleAlongBothAxes()) {
            if (!parent_style.ShouldApplyAnyContainment(*document_element) && !builder.ShouldApplyAnyContainment(*element)) {
                scrolls_overflow = false;
            }
        }
    }
    bool is_child_document = element == document_element && element->GetDocument().LocalOwner();
    if (scrolls_overflow || is_child_document) {
        return touch_action | TouchAction::kPan | TouchAction::kInternalPanXScrolls | TouchAction::kInternalNotWritable;
    }
    return touch_action;
}
...
void StyleAdjuster::AdjustStyleForSvgElement(const SVGElement& element, ComputedStyleBuilder& builder, const ComputedStyle& layout_parent_style)
{
    if (builder.Display() != EDisplay::kNone) {
        ...
        builder.SetTextDecorationStyle(ETextDecorationStyle::kSolid); // crbug.com/1246719
    ...
    if (builder.Display() == EDisplay::kContents
        && (is_svg_root || (!IsA<SVGSVGElement>(element) && !IsA<SVGGElement>(element) && !IsA<SVGUseElement>(element) && !IsA<SVGTSpanElement>(element)))) {
    ...
    if ((IsA<SVGForeignObjectElement>(element) || IsA<SVGTextElement>(element)) && builder.IsDisplayInlineType()) {
    ...
    } else if (baseline == EDominantBaseline::kNoChange || baseline == EDominantBaseline::kResetSize) {
...
static bool StopPropagateTextDecorations(const ComputedStyleBuilder& builder, const Element* element)
{
    return builder.IsDisplayReplacedType() || IsAtMediaUAShadowBoundary(element) || builder.IsFloating() || builder.HasOutOfFlowPosition()
        || IsOutermostSVGElement(element) || builder.Display() == EDisplay::kRubyText;
}

static bool LayoutParentStyleForcesZIndexToCreateStackingContext(const ComputedStyle& layout_parent_style)
{
    return layout_parent_style.IsDisplayFlexibleOrGridBox();
}

void StyleAdjuster::AdjustStyleForEditing(ComputedStyleBuilder& builder, Element* element)
{
...
void StyleAdjuster::AdjustStyleForTextCombine(ComputedStyleBuilder& builder)
{
    ...
    const auto size = LengthSize(Length::Fixed(line_height), Length::Fixed(one_em));
...
#if DCHECK_IS_ON()
    DCHECK_EQ(builder.GetFont().GetFontDescription().Orientation(), FontOrientation::kHorizontal);
...
static void AdjustStyleForFirstLetter(ComputedStyleBuilder& builder)
{
    ...
    builder.SetDisplay(builder.IsFloating() ? EDisplay::kBlock : EDisplay::kInline);
}

static void AdjustStyleForMarker(ComputedStyleBuilder& builder, const ComputedStyle& parent_style, const Element* parent_element)
{
    ...
    if (parent_style.MarkerShouldBeInside(*parent_element, builder.GetDisplayStyle())) {
        Document& document = parent_element->GetDocument();
        auto margins = ListMarker::InlineMarginsForInside(document, builder, parent_style);
        LogicalToPhysicalSetter setter(builder.GetWritingDirection(), builder, &ComputedStyleBuilder::SetMarginTop, &ComputedStyleBuilder::SetMarginRight,
            &ComputedStyleBuilder::SetMarginBottom, &ComputedStyleBuilder::SetMarginLeft);
...
static void AdjustStyleForHTMLElement(ComputedStyleBuilder& builder, HTMLElement& element)
{
    ...
    if (IsA<HTMLTableElement>(element)) {
        ...
        // back to the default.
        if (builder.GetTextAlign() == ETextAlign::kWebkitLeft || builder.GetTextAlign() == ETextAlign::kWebkitCenter
    ...
    if (IsA<HTMLFencedFrameElement>(element)) {
        ...
        builder.SetEffectiveZoom(element.GetDocument().GetStyleResolver().InitialZoom());
    }

    if (IsA<HTMLLegendElement>(element) && builder.Display() != EDisplay::kContents) {
    ...
    if (IsA<HTMLTextAreaElement>(element)) {
        ...
        builder.SetOverflowX(builder.OverflowX() == EOverflow::kVisible ? EOverflow::kAuto : builder.OverflowX());
        builder.SetOverflowY(builder.OverflowY() == EOverflow::kVisible ? EOverflow::kAuto : builder.OverflowY());
        if (builder.Display() == EDisplay::kContents) {
            builder.SetDisplay(EDisplay::kNone);
        }
        return;
    ...
    if (auto* html_plugin_element = DynamicTo<HTMLPlugInElement>(element)) {
        builder.SetRequiresAcceleratedCompositingForExternalReasons(html_plugin_element->ShouldAccelerate());
        if (builder.Display() == EDisplay::kContents) {
            builder.SetDisplay(EDisplay::kNone);
        }
        return;
    ...
    if (builder.Display() == EDisplay::kContents) {
        ...
        if (IsA<HTMLBRElement>(element) || IsA<HTMLWBRElement>(element) || IsA<HTMLMeterElement>(element) || IsA<HTMLProgressElement>(element)
            || IsA<HTMLCanvasElement>(element) || IsA<HTMLMediaElement>(element) || IsA<HTMLInputElement>(element) || IsA<HTMLTextAreaElement>(element)
            || IsA<HTMLSelectElement>(element)) {
            builder.SetDisplay(EDisplay::kNone);
        }
    }

    if (IsA<HTMLBodyElement>(element) && element.GetDocument().FirstBodyElement() != element) {
        builder.SetIsSecondaryBodyElement();
    }
}

void StyleAdjuster::AdjustOverflow(ComputedStyleBuilder& builder, Element* element)
{
    DCHECK(builder.OverflowX() != EOverflow::kVisible || builder.OverflowY() != EOverflow::kVisible);

    bool overflow_is_clip_or_visible = IsOverflowClipOrVisible(builder.OverflowY()) && IsOverflowClipOrVisible(builder.OverflowX());
    ...
    if (element && !element->IsPseudoElement() && (builder.OverflowX() == EOverflow::kClip || builder.OverflowY() == EOverflow::kClip)) {
        UseCounter::Count(element->GetDocument(), WebFeature::kOverflowClipAlongEitherAxis);
    }
...
static bool IsCanvasPlacedElement(const Element* element)
{
    if (RuntimeEnabledFeatures::CanvasPlaceElementEnabled() && element) {
        ...
        if (const auto* canvas = DynamicTo<HTMLCanvasElement>(element->parentElement())) {
...
static void AdjustStyleForDisplay(ComputedStyleBuilder& builder, const ComputedStyle& layout_parent_style, const Element* element, Document* document)
{
    bool is_canvas_placed_element = IsCanvasPlacedElement(element);

    if ((layout_parent_style.BlockifiesChildren() && !HostIsInputFile(element)) || is_canvas_placed_element) {
        ...
        if (layout_parent_style.IsDisplayFlexibleOrGridBox() || layout_parent_style.IsDisplayMathType() || is_canvas_placed_element) {
    ...
    if (layout_parent_style.InlinifiesChildren() && !builder.HasOutOfFlowPosition() && !(element && IsA<HTMLFieldSetElement>(element->parentNode()))) {
        if (builder.IsFloating()) {
            builder.SetFloating(EFloat::kNone);
            if (document) {
                document->AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(ConsoleMessage::Source::kRendering, ConsoleMessage::Level::kInfo,
    ...
    if (builder.Display() == EDisplay::kInline && builder.StyleType() == kPseudoIdNone && builder.GetWritingMode() != layout_parent_style.GetWritingMode()) {
        builder.SetDisplay(EDisplay::kInlineBlock);
    }
    ...
    if (builder.Display() == EDisplay::kTableColumn || builder.Display() == EDisplay::kTableColumnGroup || builder.Display() == EDisplay::kTableFooterGroup
        || builder.Display() == EDisplay::kTableHeaderGroup || builder.Display() == EDisplay::kTableRow || builder.Display() == EDisplay::kTableRowGroup) {
    ...
    if (RuntimeEnabledFeatures::CSSLineClampWebkitBoxBlockificationEnabled() && builder.BoxOrient() == EBoxOrient::kVertical
        && (builder.WebkitLineClamp() != 0 || builder.StandardLineClamp() != 0 || builder.HasAutoStandardLineClamp())) {
...
bool StyleAdjuster::IsEditableElement(Element* element, const ComputedStyleBuilder& builder)
{
...
bool StyleAdjuster::IsPasswordFieldWithUnrevealedPassword(Element* element)
{
    ...
    if (auto* input = DynamicTo<HTMLInputElement>(element)) {
        return input->FormControlType() == FormControlType::kInputPassword && !input->ShouldRevealPassword();
    }
    return false;
}

void StyleAdjuster::AdjustEffectiveTouchAction(ComputedStyleBuilder& builder, const ComputedStyle& parent_style, Element* element, bool is_svg_root)
{
    ...
    bool is_replaced_canvas
        = IsA<HTMLCanvasElement>(element) && element->GetExecutionContext() && element->GetExecutionContext()->CanExecuteScripts(kNotAboutToExecuteScript);
    bool is_non_replaced_inline_elements
        = builder.IsDisplayInlineType() && !(builder.IsDisplayReplacedType() || is_svg_root || IsA<HTMLImageElement>(element) || is_replaced_canvas);
    bool is_table_row_or_column = builder.IsDisplayTableRowOrColumnType();
    bool is_layout_object_needed = element->LayoutObjectIsNeeded(builder.GetDisplayStyle());
    ...
    if (!is_non_replaced_inline_elements && !is_table_row_or_column && is_layout_object_needed) {
    ...
    if (is_child_document && element->GetDocument().GetFrame()) {
        inherited_action &= TouchAction::kPan | TouchAction::kInternalPanXScrolls | TouchAction::kInternalNotWritable
    ...
    // iframe element.
    inherited_action = AdjustTouchActionForElement(inherited_action, builder, parent_style, element);
    ...
    if (::features::IsSwipeToMoveCursorEnabled() && IsEditableElement(element, builder)) {
        element_touch_action &= ~TouchAction::kInternalPanXScrolls;
    }
    ...
    if (RuntimeEnabledFeatures::StylusHandwritingEnabled() && (element_touch_action & TouchAction::kPan) == TouchAction::kPan
        && IsEditableElement(element, builder) && !IsPasswordFieldWithUnrevealedPassword(element)) {
        element_touch_action &= ~TouchAction::kInternalNotWritable;
    }

    // Apply the adjusted parent effective touch actions.
    builder.SetEffectiveTouchAction((element_touch_action & inherited_action) | enforced_by_policy);
    ...
    if (auto* frame_owner = DynamicTo<HTMLFrameOwnerElement>(element)) {
        ...
        if (content_frame) {
            content_frame->SetInheritedEffectiveTouchAction(builder.EffectiveTouchAction());
        }
    }
}

static void AdjustStyleForInert(ComputedStyleBuilder& builder, Element* element)
{
    if (!element) {
        return;
    }
...
void StyleAdjuster::AdjustForForcedColorsMode(ComputedStyleBuilder& builder, Document& document)
{
    ...
    builder.SetScrollbarColor(ComputedStyleInitialValues::InitialScrollbarColor());
    ...
    if (document.GetStyleEngine().GetPreferredColorScheme() == mojom::blink::PreferredColorScheme::kDark) {
    ...
    if (builder.InternalForcedBackgroundColor().IsSystemColor()) {
        builder.SetInternalForcedBackgroundColor(builder.InternalForcedBackgroundColor().ResolveSystemColor(color_scheme, color_provider, is_in_web_app_scope));
    }
    if (builder.InternalForcedColor().IsSystemColor()) {
        builder.SetInternalForcedColor(builder.InternalForcedColor().ResolveSystemColor(color_scheme, color_provider, is_in_web_app_scope));
    }
    if (builder.InternalForcedVisitedColor().IsSystemColor()) {
        builder.SetInternalForcedVisitedColor(builder.InternalForcedVisitedColor().ResolveSystemColor(color_scheme, color_provider, is_in_web_app_scope));
    }
}
...
void StyleAdjuster::AdjustForSVGTextElement(ComputedStyleBuilder& builder)
{
    ...
    builder.SetColumnWidthInternal(ComputedStyleInitialValues::InitialColumnWidth());
    builder.SetColumnRuleStyle(ComputedStyleInitialValues::InitialColumnRuleStyle());
    builder.SetColumnRuleWidthInternal(ComputedStyleInitialValues::InitialColumnRuleWidth());
    builder.SetColumnRuleColor(ComputedStyleInitialValues::InitialColumnRuleColor());
    builder.SetInternalVisitedColumnRuleColor(ComputedStyleInitialValues::InitialInternalVisitedColumnRuleColor());
    builder.SetColumnCountInternal(ComputedStyleInitialValues::InitialColumnCount());
    builder.SetHasAutoColumnCountInternal(ComputedStyleInitialValues::InitialHasAutoColumnCount());
    builder.SetHasAutoColumnWidthInternal(ComputedStyleInitialValues::InitialHasAutoColumnWidth());
...
void StyleAdjuster::AdjustComputedStyle(StyleResolverState& state, Element* element)
{
    ...
    if (html_element && (builder.Display() != EDisplay::kNone || element->LayoutObjectIsNeeded(builder.GetDisplayStyle()))) {
        AdjustStyleForHTMLElement(builder, *html_element);
    }

    if (builder.Display() != EDisplay::kNone) {
        bool is_document_element = element && element->GetDocument().documentElement() == element;
        ...
        if ((builder.Overlay() == EOverlay::kAuto && !is_document_element) || builder.StyleType() == kPseudoIdBackdrop) {
            if (!builder.HasOutOfFlowPosition()) {
                builder.SetPosition(EPosition::kAbsolute);
            }
            if (builder.Display() == EDisplay::kContents) {
        ...
        if (is_document_element || (builder.Display() != EDisplay::kContents && (builder.HasOutOfFlowPosition() || builder.IsFloating()))) {
            builder.SetDisplay(EquivalentBlockDisplay(builder.Display()));
        }
        ...
        if (!IsA<MathMLElement>(element) && builder.IsDisplayMathType()) {
            builder.SetDisplay(builder.Display() == EDisplay::kBlockMath ? EDisplay::kBlock : EDisplay::kInline);
        }
        ...
        AdjustStyleForDisplay(builder, layout_parent_style, element, element ? &element->GetDocument() : nullptr);
        ...
        if (layout_parent_style.IsDisplayLayoutCustomBox()) {
            builder.SetDisplayLayoutCustomParentName(layout_parent_style.DisplayLayoutCustomName());
        }
    ...
    if (!builder.HasAutoZIndex()) {
        if (builder.GetPosition() == EPosition::kStatic && !LayoutParentStyleForcesZIndexToCreateStackingContext(layout_parent_style)) {
    ...
    if (element == state.GetDocument().documentElement() || (element && IsA<SVGForeignObjectElement>(*element)) || builder.Overlay() == EOverlay::kAuto
        || builder.StyleType() == kPseudoIdBackdrop || builder.StyleType() == kPseudoIdViewTransition || IsCanvasPlacedElement(element)) {
        builder.SetForcesStackingContext(true);
    }
    ...
    if (StopPropagateTextDecorations(builder, element) || state.IsForHighlight()) {
        builder.SetBaseTextDecorationData(nullptr);
    } else {
        builder.SetBaseTextDecorationData(layout_parent_style.AppliedTextDecorationData());
    }
    ...
    if (state.UsesHighlightPseudoInheritance() && state.OriginatingElementStyle()) {
        ...
        if (builder.InternalVisitedColorIsCurrentColor()) {
            builder.SetInternalVisitedColor(originating_style->InternalVisitedColor());
        }
    }
    ...
    if (parent_style.JustifyItems().PositionType() == ItemPositionType::kLegacy && builder.JustifyItems().GetPosition() == ItemPosition::kLegacy) {
        builder.SetJustifyItems(parent_style.JustifyItems());
    }

    AdjustEffectiveTouchAction(builder, parent_style, element, IsOutermostSVGElement(element));

    bool is_media_control = element && element->ShadowPseudoId().StartsWith("-webkit-media-controls");
    ...
    if (element && builder.TextOverflow() == ETextOverflow::kEllipsis) {
        ...
        if (pseudo_id == shadow_element_names::kPseudoInputPlaceholder || pseudo_id == shadow_element_names::kPseudoInternalInputSuggested) {
            TextControlElement* text_control = ToTextControl(element->OwnerShadowHost());
            DCHECK(text_control);
    ...
    if (state.GetStyledElement() && ViewTransitionUtils::IsViewTransitionElementExcludingRootFromSupplement(*state.GetStyledElement())) {
        builder.SetElementIsViewTransitionParticipant();
    }

    if (RuntimeEnabledFeatures::CSSContentVisibilityImpliesContainIntrinsicSizeAutoEnabled() && builder.ContentVisibility() == EContentVisibility::kAuto) {
