...
public:
    explicit MatchedPropertiesCacheTestKey(String block_text, unsigned hash, const TreeScope& tree_scope)
        : key_(ParseBlock(block_text, tree_scope), hash)
    {
    }

    const MatchedPropertiesCache::Key& InnerKey() const
    {
        return key_;
    }

private:
    const MatchResult& ParseBlock(String block_text, const TreeScope& tree_scope)
    {
...
public:
    ...
    void Add(const TestKey& key, const ComputedStyle& style, const ComputedStyle& parent_style)
    {
        cache_.Add(key.InnerKey(), &style, &parent_style);
    }

    const CachedMatchedProperties::Entry* Find(
        const TestKey& key, const ComputedStyle& style, const ComputedStyle& parent_style, const StyleRecalcContext* style_recalc_context = nullptr)
    {
        StyleResolverState state(document_, *document_.body(), style_recalc_context, StyleRequest(&parent_style));
...
TEST_F(MatchedPropertiesCacheTest, VariableDependency)
{
    ...
    auto parent_builder_a = CreateStyleBuilder();
    auto parent_builder_b = CreateStyleBuilder();
    parent_builder_a.SetVariableData(AtomicString("--x"), CreateVariableData("1px"), true);
    parent_builder_b.SetVariableData(AtomicString("--x"), CreateVariableData("2px"), true);
    const auto* parent_a = parent_builder_a.TakeStyle();
    const auto* parent_b = parent_builder_b.TakeStyle();
