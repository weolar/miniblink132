...
static unsigned ComputeMatchedPropertiesHash(const MatchResult& result)
{
    ...
    DCHECK(!std::any_of(hashes.begin(), hashes.end(), [](const MatchedPropertiesHash& hash) { return hash.hash == WTF::HashTraits<unsigned>::DeletedValue(); }))
        << "This should have been checked in AddMatchedProperties()";
    // as_byte_span
    unsigned hash = StringHasher::HashMemory(base::span<const uint8_t>((const uint8_t*)hashes.data(), hashes.size() * sizeof(MatchedPropertiesHash)));
...
CachedMatchedProperties::CachedMatchedProperties(
    const ComputedStyle* style, const ComputedStyle* parent_style, const MatchedPropertiesVector& properties, unsigned clock)
    : entries({ Entry { style, parent_style, clock } })
{
    ...
    for (const auto& new_matched_properties : properties) {
        matched_properties.emplace_back(new_matched_properties.properties, new_matched_properties.data_);
    }
}
...
MatchedPropertiesCache::Key::Key(const MatchResult& result)
    : Key(result, result.IsCacheable() ? ComputeMatchedPropertiesHash(result) : HashTraits<unsigned>::EmptyValue())
{
}
...
const CachedMatchedProperties::Entry* MatchedPropertiesCache::Find(const Key& key, const StyleResolverState& style_resolver_state)
{
    ...
    for (CachedMatchedProperties::Entry& entry : cache_item->entries) {
        if (IsAtShadowBoundary(&style_resolver_state.GetElement())
            && entry.parent_computed_style->UserModify() != ComputedStyleInitialValues::InitialUserModify()) {
        ...
        if ((entry.parent_computed_style->IsEnsuredInDisplayNone() || entry.computed_style->IsEnsuredOutsideFlatTree())
            && !style_resolver_state.ParentStyle()->IsEnsuredInDisplayNone() && !style_resolver_state.IsOutsideFlatTree()) {
        ...
        if (style_resolver_state.ParentStyle()->InheritedDataShared(*entry.parent_computed_style)) {
...
bool CachedMatchedProperties::CorrespondsTo(const MatchedPropertiesVector& lookup_properties) const
{
    ...
    for (; lookup_it != lookup_properties.end(); std::advance(lookup_it, 1), std::advance(cached_it, 1)) {
        CSSPropertyValueSet* cached_properties = cached_it->first.Get();
        DCHECK(!lookup_it->properties->ModifiedSinceHashing()) << "This should have been checked in AddMatchedProperties()";
...
void CachedMatchedProperties::RefreshKey(const MatchedPropertiesVector& lookup_properties)
{
...
void MatchedPropertiesCache::Add(const Key& key, const ComputedStyle* style, const ComputedStyle* parent_style)
{
    Member<CachedMatchedProperties>& cache_item = cache_.insert(key.hash_, nullptr).stored_value->value;

    if (!cache_item) {
        cache_item = MakeGarbageCollected<CachedMatchedProperties>(style, parent_style, key.result_.GetMatchedProperties(), clock_++);
    } else {
...
{
    EraseEntriesIf([](const CachedMatchedProperties::Entry& entry) { return entry.computed_style->HasViewportUnits(); });
}

bool MatchedPropertiesCache::IsStyleCacheable(const ComputedStyleBuilder& builder)
{
    ...
    if (builder.HasPseudoElementStyle(kPseudoIdHighlight) || builder.StyleType() == kPseudoIdHighlight) {
        return false;
    }
    return true;
}
...
bool MatchedPropertiesCache::IsCacheable(const StyleResolverState& state)
{
    ...
    if (IsAtShadowBoundary(&state.GetElement()) && state.StyleBuilder().UserModify() != parent_style.UserModify()) {
        return false;
    }
...
void MatchedPropertiesCache::Trace(Visitor* visitor) const
{
    ...
    visitor->RegisterWeakCallbackMethod<MatchedPropertiesCache, &MatchedPropertiesCache::CleanMatchedPropertiesCache>(this);
}

static inline bool ShouldRemoveMPCEntry(CachedMatchedProperties& value, const LivenessBroker& info)
{
    for (const auto& [properties, metadata] : value.matched_properties) {
        if (!info.IsHeapObjectAlive(properties) || properties->ModifiedSinceHashing()) {
            return true;
        }
    }
    return false;
...
template <class Predicate> void MatchedPropertiesCache::EraseEntriesIf(Predicate&& pred)
{
    cache_.erase_if([inner_pred { std::forward<Predicate>(pred) }, this](const auto& entry_pair) {
        if (!entry_pair.value) {
            return false;
        }
        HeapVector<CachedMatchedProperties::Entry, 4>& entries = entry_pair.value->entries;
...
void MatchedPropertiesCache::CleanMatchedPropertiesCache(const LivenessBroker& info)
{
    ...
    if (live_entries.size() > kPruneCacheTarget) {
        ...
        std::nth_element(live_entries.begin(), UNSAFE_BUFFERS(live_entries.begin() + cutoff_idx), live_entries.end());
        unsigned min_last_used = live_entries[cutoff_idx];

        EraseEntriesIf([min_last_used](const CachedMatchedProperties::Entry& entry) { return entry.last_used <= min_last_used; });
    }
}

std::ostream& operator<<(std::ostream& stream, MatchedPropertiesCache::Key& key)
{
    stream << "Key{";
    for (const MatchedProperties& matched_properties : key.result_.GetMatchedProperties()) {
