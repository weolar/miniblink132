...
static_assert(std::is_trivially_destructible<CascadeMap::CascadePriorityList>::value, "Destructor is never called on CascadePriorityList objects created here");

static_assert(!VectorTraits<CascadeMap::CascadePriorityList::Node>::kNeedsDestruction, "Backing vector should not need destruction");
...
const CascadePriority* CascadeMap::Find(const CSSPropertyName& name) const
{
    ...
    return native_properties_.Bits().Has(name.Id()) ? &native_properties_.Buffer()[index].Top(backing_vector_) : nullptr;
}
...
const CascadePriority* CascadeMap::Find(const CSSPropertyName& name, CascadeOrigin origin) const
{
    auto find_origin = [this](const CascadeMap::CascadePriorityList& list, CascadeOrigin origin) -> const CascadePriority* {
        for (auto iter = list.Begin(backing_vector_); iter != list.End(backing_vector_); ++iter) {
    ...
    if (name.IsCustomProperty()) {
        ...
        return find_origin(custom_properties_.find(name.ToAtomicString())->value, origin);
    }
...
const CascadePriority* CascadeMap::FindRevertLayer(const CSSPropertyName& name, uint64_t revert_from) const
{
    auto find_revert_layer = [this](const CascadeMap::CascadePriorityList& list, uint64_t revert_from) -> const CascadePriority* {
        for (auto iter = list.Begin(backing_vector_); iter != list.End(backing_vector_); ++iter) {
    ...
    if (name.IsCustomProperty()) {
        DCHECK(custom_properties_.Contains(name.ToAtomicString()));
        return find_revert_layer(custom_properties_.find(name.ToAtomicString())->value, revert_from);
    }
...
void CascadeMap::Add(const AtomicString& custom_property_name, CascadePriority priority)
{
    auto result = custom_properties_.insert(custom_property_name, CascadePriorityList());
