...
bool AddTo(CascadeMap& map, const CSSPropertyName& name, CascadePriority priority)
{
...
TEST(CascadeMapTest, HighPriorityBits)
{
    ...
    EXPECT_EQ(map.HighPriorityBits(), 1ull << static_cast<uint64_t>(CSSPropertyID::kFontSize));

    map.Add(CSSPropertyID::kColor, CascadePriority(CascadeOrigin::kAuthor));
    map.Add(CSSPropertyID::kFontSize, CascadePriority(CascadeOrigin::kAuthor));
    EXPECT_EQ(map.HighPriorityBits(), (1ull << static_cast<uint64_t>(CSSPropertyID::kFontSize)) | (1ull << static_cast<uint64_t>(CSSPropertyID::kColor)));
}
