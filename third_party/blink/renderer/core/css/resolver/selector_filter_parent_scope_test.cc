...
TEST_F(SelectorFilterParentScopeTest, ParentScope)
{
    ...
    GetDocument().body()->setAttribute(html_names::kClassAttr, AtomicString("match"));
    ...
    SelectorFilterParentScope html_scope(*GetDocument().documentElement());
    {
        ...
        SelectorFilterParentScope::EnsureParentStackIsPushed();
        {
            ...
            base::span<CSSSelector> selector_vector
                = CSSParser::ParseSelector(MakeGarbageCollected<CSSParserContext>(kHTMLStandardMode, SecureContextMode::kInsecureContext),
                    CSSNestingType::kNone, /*parent_rule_for_nesting=*/nullptr,
                    /*is_within_scope=*/false, nullptr, "html *, body *, .match *, #myId *", arena);
            CSSSelectorList* selectors = CSSSelectorList::AdoptSelectorVector(selector_vector);
            ...
            for (const CSSSelector* selector = selectors->First(); selector; selector = CSSSelectorList::Next(*selector)) {
                ...
                filter.CollectIdentifierHashes(*selector, /* style_scope */ nullptr, selector_hashes);
...
TEST_F(SelectorFilterParentScopeTest, RootScope)
{
    ...
    SelectorFilterRootScope span_scope(GetDocument().getElementById(AtomicString("y")));
    SelectorFilterParentScope::EnsureParentStackIsPushed();

    HeapVector<CSSSelector> arena;
    base::span<CSSSelector> selector_vector
        = CSSParser::ParseSelector(MakeGarbageCollected<CSSParserContext>(kHTMLStandardMode, SecureContextMode::kInsecureContext), CSSNestingType::kNone,
            /*parent_rule_for_nesting=*/nullptr,
            /*is_within_scope=*/false, nullptr, "html *, body *, div *, span *, .x *, #y *", arena);
    CSSSelectorList* selectors = CSSSelectorList::AdoptSelectorVector(selector_vector);
...
{
    GetDocument().body()->setInnerHTML(R"HTML(<div ATTR><svg VIewBox></svg></div>)HTML");
    ...
    base::span<CSSSelector> selector_vector
        = CSSParser::ParseSelector(MakeGarbageCollected<CSSParserContext>(kHTMLStandardMode, SecureContextMode::kInsecureContext), CSSNestingType::kNone,
            /*parent_rule_for_nesting=*/nullptr,
            /*is_within_scope=*/false, nullptr, "[Attr] *, [attr] *, [viewbox] *, [VIEWBOX] *", arena);
    CSSSelectorList* selectors = CSSSelectorList::AdoptSelectorVector(selector_vector);
