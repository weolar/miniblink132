...
public:
    ...
    StyleImageLoader(Document& document, ComputedStyleBuilder& builder, const PreCachedContainerSizes& pre_cached_container_sizes, float device_scale_factor)
        : document_(document)
        , builder_(builder)
    ...
    StyleImage* Load(CSSValue&, FetchParameters::ImageRequestBehavior = FetchParameters::ImageRequestBehavior::kNone,
        CrossOriginAttributeValue = kCrossOriginAttributeNotSet, const float override_image_resolution = 0.0f);

private:
    StyleImage* CrossfadeArgument(CSSValue&, CrossOriginAttributeValue);
    StyleImage* ResolveImageSet(CSSImageSetValue& image_set_value, FetchParameters::ImageRequestBehavior, CrossOriginAttributeValue);
...
StyleImage* StyleImageLoader::Load(CSSValue& value, FetchParameters::ImageRequestBehavior image_request_behavior, CrossOriginAttributeValue cross_origin,
    const float override_image_resolution)
{
    if (auto* image_value = DynamicTo<CSSImageValue>(value)) {
        return image_value->CacheImage(document_, image_request_behavior, cross_origin, override_image_resolution);
    }

    if (auto* paint_value = DynamicTo<CSSPaintValue>(value)) {
        auto* image = MakeGarbageCollected<StyleGeneratedImage>(*paint_value, ContainerSizes());
    ...
    if (auto* crossfade_value = DynamicTo<cssvalue::CSSCrossfadeValue>(value)) {
        ...
        for (const auto& [image, percentage] : crossfade_value->GetImagesAndPercentages()) {
            style_images.push_back(CrossfadeArgument(*image, cross_origin));
        }
        return MakeGarbageCollected<StyleCrossfadeImage>(*crossfade_value, std::move(style_images));
    }

    if (auto* image_gradient_value = DynamicTo<cssvalue::CSSGradientValue>(value)) {
        const ContainerSizes& container_sizes = image_gradient_value->IsUsingContainerRelativeUnits() ? pre_cached_container_sizes_.Get() : ContainerSizes();
        return MakeGarbageCollected<StyleGeneratedImage>(*image_gradient_value, container_sizes);
    }

    if (auto* image_set_value = DynamicTo<CSSImageSetValue>(value)) {
        StyleImage* style_image = ResolveImageSet(*image_set_value, image_request_behavior, cross_origin);
        return image_set_value->CacheImage(style_image, device_scale_factor_, style_image ? style_image->IsOriginClean() : true);
    }

    NOTREACHED();
}

StyleImage* StyleImageLoader::CrossfadeArgument(CSSValue& value, CrossOriginAttributeValue cross_origin)
{
    ...
    return Load(value, FetchParameters::ImageRequestBehavior::kNone, cross_origin);
}

StyleImage* StyleImageLoader::ResolveImageSet(
    CSSImageSetValue& image_set_value, FetchParameters::ImageRequestBehavior image_request_behavior, CrossOriginAttributeValue cross_origin)
{
    const CSSImageSetOptionValue* option = image_set_value.GetBestOption(device_scale_factor_);
    if (!option) {
        return nullptr;
    }
    CSSValue& image_value = option->GetImage();
    // Artificially reject types that are not "supported".
    if (!IsA<CSSImageValue>(image_value) && !IsA<cssvalue::CSSGradientValue>(image_value)) {
        return nullptr;
    }
    return Load(image_value, image_request_behavior, cross_origin, option->ComputedResolution());
}

} // namespace

const PreCachedContainerSizes::ContainerSizes& PreCachedContainerSizes::Get() const
{
    if (!cache_) {
        if (conversion_data_) {
...
ElementStyleResources::ElementStyleResources(Element& element, float device_scale_factor)
    : element_(element)
    , device_scale_factor_(device_scale_factor)
{
}
...
StyleImage* ElementStyleResources::CachedStyleImage(const CSSValue& value) const
{
    ...
    if (auto* gradient_value = DynamicTo<cssvalue::CSSGradientValue>(value)) {
        using ContainerSizes = CSSToLengthConversionData::ContainerSizes;
        const ContainerSizes& container_sizes = gradient_value->IsUsingContainerRelativeUnits() ? pre_cached_container_sizes_.Get() : ContainerSizes();
        return MakeGarbageCollected<StyleGeneratedImage>(*gradient_value, container_sizes);
    }
...
StyleImage* ElementStyleResources::GetStyleImage(CSSPropertyID property, const CSSValue& value)
{
    if (auto* identifier_value = DynamicTo<CSSIdentifierValue>(value)) {
        DCHECK_EQ(identifier_value->GetValueID(), CSSValueID::kNone);
        return nullptr;
...
static bool AllowExternalResources(CSSPropertyID property)
{
    if (RuntimeEnabledFeatures::SvgExternalResourcesEnabled()) {
        if (property == CSSPropertyID::kClipPath || property == CSSPropertyID::kFill || property == CSSPropertyID::kMarkerEnd
            || property == CSSPropertyID::kMarkerMid || property == CSSPropertyID::kMarkerStart || property == CSSPropertyID::kStroke) {
            return true;
        }
    }
    return property == CSSPropertyID::kBackdropFilter || property == CSSPropertyID::kFilter;
}

SVGResource* ElementStyleResources::GetSVGResourceFromValue(CSSPropertyID property, const cssvalue::CSSURIValue& value)
{
    if (value.IsLocal(element_.GetDocument())) {
        SVGTreeScopeResources& tree_scope_resources = element_.OriginatingTreeScope().EnsureSVGTreeScopedResources();
        return tree_scope_resources.ResourceForId(value.NormalizedFragmentIdentifier());
    }
...
static void LoadResourcesForFilter(FilterOperations::FilterOperationVector& filter_operations, Document& document)
{
    for (const auto& filter_operation : filter_operations) {
        auto* reference_operation = DynamicTo<ReferenceFilterOperation>(filter_operation.Get());
...
static SVGResource* GetSingleSVGResource(CSSPropertyID property, ComputedStyleBuilder& builder)
{
    CHECK(RuntimeEnabledFeatures::SvgExternalResourcesEnabled());
    switch (property) {
    case CSSPropertyID::kClipPath: {
        auto* reference_clip = DynamicTo<ReferenceClipPathOperation>(builder.MutableClipPath());
...
void ElementStyleResources::LoadPendingSVGResources(ComputedStyleBuilder& builder)
{
    Document& document = element_.GetDocument();
    for (CSSPropertyID property : pending_svg_resource_properties_) {
        switch (property) {
        case CSSPropertyID::kBackdropFilter:
            LoadResourcesForFilter(builder.MutableBackdropFilterOperations(), document);
            break;
...
StyleImage* ElementStyleResources::LoadMaskSource(CSSValue& pending_value)
{
    ...
    if (image_value->IsLocal(element_.GetDocument())) {
        SVGTreeScopeResources& tree_scope_resources = element_.OriginatingTreeScope().EnsureSVGTreeScopedResources();
        SVGResource* resource = tree_scope_resources.ResourceForId(image_value->NormalizedFragmentIdentifier());
        return MakeGarbageCollected<StyleMaskSourceImage>(resource, image_value);
    }
    StyleImage* image = image_value->CacheImage(element_.GetDocument(), FetchParameters::ImageRequestBehavior::kNone, kCrossOriginAttributeAnonymous);
    return MakeGarbageCollected<StyleMaskSourceImage>(To<StyleFetchedImage>(image), image_value->EnsureSVGResource(), image_value);
}
...
void ElementStyleResources::LoadPendingImages(ComputedStyleBuilder& builder)
{
    ...
    StyleImageLoader loader(element_.GetDocument(), builder, pre_cached_container_sizes_, device_scale_factor_);
    for (CSSPropertyID property : pending_image_properties_) {
        switch (property) {
        case CSSPropertyID::kBackgroundImage: {
            for (FillLayer* background_layer = &builder.AccessBackgroundLayers(); background_layer; background_layer = background_layer->Next()) {
                if (auto* pending_value = PendingCssValue(background_layer->GetImage())) {
                    FetchParameters::ImageRequestBehavior image_request_behavior = FetchParameters::ImageRequestBehavior::kNone;
                    StyleImage* new_image = loader.Load(*pending_value, image_request_behavior);
        ...
        case CSSPropertyID::kContent: {
            for (ContentData* content_data = const_cast<ContentData*>(builder.GetContentData()); content_data; content_data = content_data->Next()) {
                if (auto* image_content = DynamicTo<ImageContentData>(*content_data)) {
                    if (auto* pending_value = PendingCssValue(image_content->GetImage())) {
        ...
        case CSSPropertyID::kBorderImageSource: {
            if (auto* pending_value = PendingCssValue(builder.BorderImage().GetImage())) {
        ...
        case CSSPropertyID::kWebkitBoxReflect: {
            if (StyleReflection* reflection = builder.BoxReflect()) {
                ...
                if (auto* pending_value = PendingCssValue(mask_image.GetImage())) {
                    ...
                    reflection->SetMask(NinePieceImage(loaded_image, mask_image.ImageSlices(), mask_image.Fill(), mask_image.BorderSlices(),
                        mask_image.Outset(), mask_image.HorizontalRule(), mask_image.VerticalRule()));
                }
            }
            break;
        }
        ...
        case CSSPropertyID::kMaskImage: {
            for (FillLayer* mask_layer = &builder.AccessMaskLayers(); mask_layer; mask_layer = mask_layer->Next()) {
                if (auto* pending_value = PendingCssValue(mask_layer->GetImage())) {
                    StyleImage* image = LoadMaskSource(*pending_value);
                    if (!image) {
                        image = loader.Load(*pending_value, FetchParameters::ImageRequestBehavior::kNone, kCrossOriginAttributeAnonymous);
                    }
        ...
        case CSSPropertyID::kShapeOutside:
            if (ShapeValue* shape_value = builder.ShapeOutside()) {
                if (auto* pending_value = PendingCssValue(shape_value->GetImage())) {
                    shape_value->SetImage(loader.Load(*pending_value, FetchParameters::ImageRequestBehavior::kNone, kCrossOriginAttributeAnonymous));
                }
            }
            break;
        default:
...
void ElementStyleResources::LoadPendingResources(ComputedStyleBuilder& builder)
{
    LoadPendingImages(builder);
    LoadPendingSVGResources(builder);
}

void ElementStyleResources::UpdateLengthConversionData(const CSSToLengthConversionData* conversion_data)
{
