...
class CORE_EXPORT CachedMatchedProperties final : public GarbageCollected<CachedMatchedProperties> {
public:
    ...
    Vector<std::pair<UntracedMember<CSSPropertyValueSet>, MatchedProperties::Data>> matched_properties;

    struct Entry {
        DISALLOW_NEW();

    public:
        Entry(const ComputedStyle* computed_style_arg, const ComputedStyle* parent_computed_style_arg, unsigned last_used_arg)
    ...
    CachedMatchedProperties(const ComputedStyle* style, const ComputedStyle* parent_style, const MatchedPropertiesVector&, unsigned clock);
...
public:
    ...
    public:
        ...
        {
            return result_.IsCacheable();
        }

    private:
        friend class MatchedPropertiesCache;
        friend class MatchedPropertiesCacheTestKey;
        friend std::ostream& operator<<(std::ostream&, MatchedPropertiesCache::Key&);
    ...
    const CachedMatchedProperties::Entry* Find(const Key&, const StyleResolverState&);
...
private:
    ...
    template <class Predicate> void EraseEntriesIf(Predicate&& pred);
...
template <> struct VectorTraits<blink::CachedMatchedProperties::Entry> : VectorTraitsBase<blink::CachedMatchedProperties::Entry> {
