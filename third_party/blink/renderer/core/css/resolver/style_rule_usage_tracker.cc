...
bool StyleRuleUsageTracker::InsertToUsedRulesMap(const CSSStyleSheet* parent_sheet, const StyleRule* rule)
{
    HeapHashSet<Member<const StyleRule>>* set
        = used_rules_.insert(parent_sheet, MakeGarbageCollected<HeapHashSet<Member<const StyleRule>>>()).stored_value->value;
    return set->insert(rule).is_new_entry;
}

void StyleRuleUsageTracker::Track(const CSSStyleSheet* parent_sheet, const StyleRule* rule)
{
    if (!parent_sheet) {
        return;
    }
    ...
    if (it != used_rules_delta_.end()) {
        ...
        used_rules_delta_.insert(parent_sheet, MakeGarbageCollected<HeapVector<Member<const StyleRule>>>()).stored_value->value->push_back(rule);
    }
}
