...
public:
    ...
    CascadePriority* FindKnownToExist(const CSSPropertyID id)
    {
        ...
        return &native_properties_.Buffer()[static_cast<size_t>(id)].Top(backing_vector_);
    }
    ...
    const CascadePriority* FindRevertLayer(const CSSPropertyName&, uint64_t) const;
    ...
    {
        return inline_style_lost_;
    }
    const CSSBitset& NativeBitset() const
    {
        return native_properties_.Bits();
    }
    ...
    public:
        ...
        inline CascadePriorityList(BackingVector& backing_vector, CascadePriority priority)
    ...
    private:
        ...
        CSSBitset bits_;
        alignas(CascadePriorityList) char properties_[kNumCSSProperties * sizeof(CascadePriorityList)];
    };

    using CustomMap = HashMap<AtomicString, CascadePriorityList>;

    const CustomMap& GetCustomMap() const
    {
...
inline CascadeMap::CascadePriorityList::Iterator::Iterator(const BackingVector* backing_vector, const Node* node)
    : backing_vector_(backing_vector)
    , backing_node_(node)
{
}

inline const CascadePriority& CascadeMap::CascadePriorityList::Iterator::operator*() const
{
...
inline CascadeMap::CascadePriorityList::Iterator& CascadeMap::CascadePriorityList::Iterator::operator++()
{
...
inline bool CascadeMap::CascadePriorityList::Iterator::operator!=(const Iterator& other) const
{
...
inline CascadeMap::CascadePriorityList::Iterator CascadeMap::CascadePriorityList ::Begin(const BackingVector& backing_vector) const
{
    if (!head_index_) {
...
inline CascadeMap::CascadePriorityList::Iterator CascadeMap::CascadePriorityList::End(const BackingVector& backing_vector) const
{
    return Iterator(&backing_vector, nullptr);
}

inline const CascadePriority& CascadeMap::CascadePriorityList::Top(const BackingVector& backing_vector) const
{
    DCHECK(!IsEmpty());
    return *Begin(backing_vector);
}

inline CascadePriority& CascadeMap::CascadePriorityList::Top(BackingVector& backing_vector)
{
    DCHECK(!IsEmpty());
    return const_cast<CascadePriority&>(*Begin(backing_vector));
}

inline void CascadeMap::CascadePriorityList::Push(BackingVector& backing_vector, CascadePriority priority)
{
