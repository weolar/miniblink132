...
public:
    ...
    {
        DCHECK(previous_);
        DCHECK(previous_->scope_type_ == ScopeType::kRoot || (previous_->parent_ && &previous_->parent_->GetDocument() == &parent.GetDocument()));
    }
...
class CORE_EXPORT SelectorFilterRootScope final : private SelectorFilterParentScope {
    STACK_ALLOCATED();
...
inline SelectorFilterParentScope::SelectorFilterParentScope(Element* parent, ScopeType scope_type)
    : parent_(parent)
    , scope_type_(scope_type)
    , previous_(current_scope_)
    , resolver_(nullptr)
{
    DCHECK(scope_type != ScopeType::kRoot || !parent || !previous_ || !previous_->parent_ || &parent_->GetDocument() != &previous_->parent_->GetDocument());
    if (parent) {
