...
public:
    ...
    HeapVector<Member<ExpansionResult>> ExpansionAt(const MatchResult& result, wtf_size_t i)
    {
        HeapVector<Member<ExpansionResult>> ret;
        ExpandCascade(
            result.GetMatchedProperties()[i], GetDocument(), i,
            [this, &ret](CascadePriority cascade_priority, const AtomicString& name) {
                ExpansionResult* er = MakeGarbageCollected<ExpansionResult>(CustomProperty(name, GetDocument()));
                er->priority = cascade_priority;
                ret.push_back(er);
            },
            [&ret](CascadePriority cascade_priority, CSSPropertyID id) {
                ExpansionResult* er = MakeGarbageCollected<ExpansionResult>(CSSProperty::Get(id));
                er->priority = cascade_priority;
                ret.push_back(er);
            });
        return ret;
    ...
    Vector<CSSPropertyID> VisitedPropertiesInExpansion(const MatchedProperties& matched_properties, wtf_size_t i)
    {
        Vector<CSSPropertyID> visited;

        ExpandCascade(
            matched_properties, GetDocument(), i,
            [](CascadePriority cascade_priority [[maybe_unused]], const AtomicString& name [[maybe_unused]]) {
                // Do nothing.
            },
            [&visited](CascadePriority cascade_priority [[maybe_unused]], CSSPropertyID id) {
...
{
    MatchResult result;
    result.AddMatchedProperties(ParseDeclarationBlock("cursor:help;top:1px"), { .origin = CascadeOrigin::kUserAgent });
...
{
    MatchResult result;
    result.AddMatchedProperties(ParseDeclarationBlock("cursor:help"), { .origin = CascadeOrigin::kUser });
    result.AddMatchedProperties(ParseDeclarationBlock("float:left"), { .origin = CascadeOrigin::kUser });
...
TEST_F(CascadeExpansionTest, AuthorRules)
{
    ...
    result.AddMatchedProperties(ParseDeclarationBlock("cursor:help;top:1px"), { .origin = CascadeOrigin::kAuthor });
    result.AddMatchedProperties(ParseDeclarationBlock("float:left"), { .origin = CascadeOrigin::kAuthor });
...
{
    MatchResult result;
    result.AddMatchedProperties(ParseDeclarationBlock("font-size:2px"), { .origin = CascadeOrigin::kUserAgent });
    result.AddMatchedProperties(ParseDeclarationBlock("cursor:help;top:1px"), { .origin = CascadeOrigin::kUser });
    result.BeginAddingAuthorRulesForTreeScope(GetDocument());
    result.AddMatchedProperties(ParseDeclarationBlock("left:1px"), { .origin = CascadeOrigin::kAuthor });
    result.AddMatchedProperties(ParseDeclarationBlock("float:left"), { .origin = CascadeOrigin::kAuthor });
    result.BeginAddingAuthorRulesForTreeScope(GetDocument());
    result.AddMatchedProperties(ParseDeclarationBlock("bottom:2px"), { .origin = CascadeOrigin::kAuthor });
...
TEST_F(CascadeExpansionTest, Name)
{
    MatchResult result;
    result.BeginAddingAuthorRulesForTreeScope(GetDocument());
    result.AddMatchedProperties(ParseDeclarationBlock("--x:1px;--y:2px"), { .origin = CascadeOrigin::kAuthor });
    result.AddMatchedProperties(ParseDeclarationBlock("float:left"), { .origin = CascadeOrigin::kAuthor });
...

    {
        auto e = ExpansionAt(result, 0);
        ASSERT_EQ(2u, e.size());
        EXPECT_EQ(CSSPropertyName(AtomicString("--x")), e[0]->ref.GetProperty().GetCSSPropertyName());
        EXPECT_EQ(CSSPropertyID::kVariable, e[0]->ref.GetProperty().PropertyID());
        EXPECT_EQ(CSSPropertyName(AtomicString("--y")), e[1]->ref.GetProperty().GetCSSPropertyName());
...

    {
        auto e = ExpansionAt(result, 1);
        ASSERT_EQ(1u, e.size());
        EXPECT_EQ(CSSPropertyName(CSSPropertyID::kFloat), e[0]->ref.GetProperty().GetCSSPropertyName());
        EXPECT_EQ(CSSPropertyID::kFloat, e[0]->ref.GetProperty().PropertyID());
    }
}
...
TEST_F(CascadeExpansionTest, LinkOmitted)
{
    ...
    auto e = ExpansionAt(result, 0);
    ASSERT_EQ(1u, e.size());
    EXPECT_EQ(CSSPropertyID::kInternalVisitedColor, e[0]->ref.GetProperty().PropertyID());
}
...
TEST_F(CascadeExpansionTest, InternalVisited)
{
    MatchResult result;
    result.BeginAddingAuthorRulesForTreeScope(GetDocument());
    result.AddMatchedProperties(ParseDeclarationBlock("color:red"), { .origin = CascadeOrigin::kAuthor });
    ...
    EXPECT_EQ(CSSPropertyID::kInternalVisitedColor, e[1]->ref.GetProperty().PropertyID());
}
...
TEST_F(CascadeExpansionTest, InternalVisitedWithTrailer)
{
    MatchResult result;
    result.BeginAddingAuthorRulesForTreeScope(GetDocument());
    result.AddMatchedProperties(ParseDeclarationBlock("color:red;left:1px"), { .origin = CascadeOrigin::kAuthor });
...
TEST_F(CascadeExpansionTest, All)
{
    MatchResult result;
    result.BeginAddingAuthorRulesForTreeScope(GetDocument());
    result.AddMatchedProperties(ParseDeclarationBlock("all:unset"), { .origin = CascadeOrigin::kAuthor });
...
TEST_F(CascadeExpansionTest, InlineAll)
{
    MatchResult result;
    result.BeginAddingAuthorRulesForTreeScope(GetDocument());
    result.AddMatchedProperties(ParseDeclarationBlock("left:1px;all:unset;right:1px"), { .origin = CascadeOrigin::kAuthor });
...
TEST_F(CascadeExpansionTest, FilterFirstLetter)
{
    MatchResult result;
    result.BeginAddingAuthorRulesForTreeScope(GetDocument());
    result.AddMatchedProperties(ParseDeclarationBlock("object-fit:unset;font-size:1px"),
        {
            .valid_property_filter = static_cast<uint8_t>(ValidPropertyFilter::kFirstLetter),
            .origin = CascadeOrigin::kAuthor,
        });
...
TEST_F(CascadeExpansionTest, FilterFirstLine)
{
    MatchResult result;
    result.BeginAddingAuthorRulesForTreeScope(GetDocument());
    result.AddMatchedProperties(ParseDeclarationBlock("display:none;font-size:1px"),
        {
            .valid_property_filter = static_cast<uint8_t>(ValidPropertyFilter::kFirstLine),
            .origin = CascadeOrigin::kAuthor,
        });
...
TEST_F(CascadeExpansionTest, FilterCue)
{
    ...
    result.AddMatchedProperties(ParseDeclarationBlock("object-fit:unset;font-size:1px"),
        {
            .valid_property_filter = static_cast<uint8_t>(ValidPropertyFilter::kCue),
            .origin = CascadeOrigin::kAuthor,
        });
...
TEST_F(CascadeExpansionTest, FilterMarker)
{
    ...
    result.AddMatchedProperties(ParseDeclarationBlock("object-fit:unset;font-size:1px"),
        {
            .valid_property_filter = static_cast<uint8_t>(ValidPropertyFilter::kMarker),
            .origin = CascadeOrigin::kAuthor,
        });
...
TEST_F(CascadeExpansionTest, FilterHighlightLegacy)
{
    MatchResult result;
    result.BeginAddingAuthorRulesForTreeScope(GetDocument());
    result.AddMatchedProperties(ParseDeclarationBlock("display:block;background-color:lime;forced-color-adjust:none"),
        {
            .valid_property_filter = static_cast<uint8_t>(ValidPropertyFilter::kHighlightLegacy),
            .origin = CascadeOrigin::kAuthor,
        });

    auto e = ExpansionAt(result, 0);
    ASSERT_EQ(3u, e.size());
    EXPECT_EQ(CSSPropertyID::kBackgroundColor, e[0]->ref.GetProperty().PropertyID());
    EXPECT_EQ(CSSPropertyID::kInternalVisitedBackgroundColor, e[1]->ref.GetProperty().PropertyID());
    EXPECT_EQ(CSSPropertyID::kForcedColorAdjust, e[2]->ref.GetProperty().PropertyID());
}
...
TEST_F(CascadeExpansionTest, FilterHighlight)
{
    ...
    result.AddMatchedProperties(ParseDeclarationBlock("display:block;background-color:lime;forced-color-adjust:none"),
        {
            .valid_property_filter = static_cast<uint8_t>(ValidPropertyFilter::kHighlight),
            .origin = CascadeOrigin::kAuthor,
        });
...
TEST_F(CascadeExpansionTest, FilterPositionFallback)
{
    MatchResult result;
    result.BeginAddingAuthorRulesForTreeScope(GetDocument());
    result.AddMatchedProperties(ParseDeclarationBlock("display:inline;position:static;left:auto"),
        {
...
TEST_F(CascadeExpansionTest, Importance)
{
    MatchResult result;
    result.BeginAddingAuthorRulesForTreeScope(GetDocument());
    result.AddMatchedProperties(ParseDeclarationBlock("cursor:help;display:block !important"), { .origin = CascadeOrigin::kAuthor });
...
TEST_F(CascadeExpansionTest, AllImportance)
{
    MatchResult result;
    result.BeginAddingAuthorRulesForTreeScope(GetDocument());
    result.AddMatchedProperties(ParseDeclarationBlock("all:unset !important"), { .origin = CascadeOrigin::kAuthor });
...
TEST_F(CascadeExpansionTest, AllVisitedOnly)
{
    ...
    result.AddMatchedProperties(ParseDeclarationBlock("all:unset"),
        {
            .link_match_type = CSSSelector::kMatchVisited,
            .valid_property_filter = static_cast<uint8_t>(ValidPropertyFilter::kNoFilter),
            .origin = CascadeOrigin::kAuthor,
        });

    ASSERT_EQ(1u, result.GetMatchedProperties().size());

    Vector<CSSPropertyID> visited = VisitedPropertiesInExpansion(result.GetMatchedProperties()[0], 0);

    for (CSSPropertyID id : kVisitedPropertySamples) {
        EXPECT_TRUE(visited.Contains(id)) << CSSProperty::Get(id).GetPropertyNameString() << " should be in the expansion";
    }
}
...
TEST_F(CascadeExpansionTest, Position)
{
    MatchResult result;
    result.BeginAddingAuthorRulesForTreeScope(GetDocument());
    result.AddMatchedProperties(ParseDeclarationBlock("left:1px;top:1px"), { .origin = CascadeOrigin::kAuthor });
    result.AddMatchedProperties(ParseDeclarationBlock("bottom:1px;right:1px"), { .origin = CascadeOrigin::kAuthor });
...
TEST_F(CascadeExpansionTest, MatchedDeclarationsLimit)
{
    ...
    for (wtf_size_t i = 0; i < max + 2; ++i) {
        declarations[i] = CSSPropertyValue(CSSPropertyName(CSSPropertyID::kColor), *cssvalue::CSSUnsetValue::Create());
    }

    MatchResult result;
    result.BeginAddingAuthorRulesForTreeScope(GetDocument());
    result.AddMatchedProperties(
        ImmutableCSSPropertyValueSet::Create(base::span(declarations).first(max + 1), kHTMLStandardMode), { .origin = CascadeOrigin::kAuthor });
    result.AddMatchedProperties(
        ImmutableCSSPropertyValueSet::Create(base::span(declarations).first(max + 2), kHTMLStandardMode), { .origin = CascadeOrigin::kAuthor });
