...
public:
    ...
    static constexpr uint64_t kOriginImportanceMask = 0xF << kOriginImportanceOffset; // of high_bits_
    static constexpr uint64_t kTreeOrderMask = 0xFFFF; // of high_bits_
    static constexpr uint64_t kLayerOrderMask = static_cast<uint64_t>(0xFFFF) << kLayerOrderOffset; // of low_bits_
    static constexpr uint64_t kPositionMask = static_cast<uint64_t>(0xFFFFFFFF) << kPositionOffset; // of low_bits_
    ...
    CascadePriority(CascadeOrigin origin, bool important)
        : CascadePriority(origin, important,
            /* tree_order */ 0,
            /* is_inline_style */ false,
            /* is_try_style */ false,
            /* is_try_tactics_style */ false,
    ...
    CascadePriority(CascadeOrigin origin, bool important, uint16_t tree_order)
        : CascadePriority(origin, important, tree_order,
            /* is_inline_style */ false,
            /* is_try_style */ false,
            /* is_try_tactics_style */ false,
            /* layer_order */ 0,
    ...
    CascadePriority(CascadeOrigin origin, bool important, uint16_t tree_order, bool is_inline_style, bool is_try_style, bool is_try_tactics_style,
        uint16_t layer_order, uint32_t position)
        : CascadePriority(static_cast<uint64_t>(position) << kPositionOffset | EncodeLayerOrder(layer_order, important) << kLayerOrderOffset
                | static_cast<uint64_t>(is_inline_style) << kIsInlineStyleOffset | static_cast<uint64_t>(is_try_style) << kIsTryStyleOffset
                | static_cast<uint64_t>(is_try_tactics_style) << kIsTryTacticsStyleOffset,
            EncodeTreeOrder(tree_order, important) | EncodeOriginImportance(origin, important) << kOriginImportanceOffset)
    {
    }
    ...
    {
        return (high_bits_ >> kImportantBit) & 1;
    }
    CascadeOrigin GetOrigin() const
    {
        uint64_t important_xor = (((~high_bits_ >> kImportantBit) & 1) - 1) & kOriginImportanceMask;
        return static_cast<CascadeOrigin>((high_bits_ ^ important_xor) >> kOriginImportanceOffset);
    }
    bool HasOrigin() const
    {
        return GetOrigin() != CascadeOrigin::kNone;
    }
    ...
    {
        return low_bits_ & kGenerationMask;
    }
    bool IsInlineStyle() const
    {
        return (low_bits_ >> kIsInlineStyleOffset) & 1;
    }

    // https://drafts.csswg.org/css-anchor-position-1/#fallback-rule
    bool IsTryStyle() const
    {
        return (low_bits_ >> kIsTryStyleOffset) & 1;
    }
    ...
    uint64_t ForLayerComparison() const
    {
        ...
        uint64_t bits = (low_bits_ >> 32) | (static_cast<uint64_t>(high_bits_) << 32);
