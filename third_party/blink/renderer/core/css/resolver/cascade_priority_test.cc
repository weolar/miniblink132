...
{
    return CascadePriority(o.origin, o.important, o.tree_order, o.is_inline_style, o.is_try_style, o.is_try_tactics_style, o.layer_order, o.position);
}

CascadePriority AuthorPriority(uint16_t tree_order, uint32_t position)
{
    return Priority({ .origin = CascadeOrigin::kAuthor, .tree_order = tree_order, .position = position });
}

CascadePriority ImportantAuthorPriority(uint16_t tree_order, uint32_t position)
{
    return Priority({ .origin = CascadeOrigin::kAuthor, .important = true, .tree_order = tree_order, .position = position });
}
...
{
    std::vector<CascadePriority> priorities = { Priority({ .origin = CascadeOrigin::kTransition }), Priority({ .origin = CascadeOrigin::kAnimation }),
        Priority({ .origin = CascadeOrigin::kAuthor }), Priority({ .origin = CascadeOrigin::kUser }), Priority({ .origin = CascadeOrigin::kUserAgent }),
...
{
    std::vector<CascadePriority> priorities = { Priority({ .origin = CascadeOrigin::kTransition, .important = false }),
        Priority({ .origin = CascadeOrigin::kUserAgent, .important = true }), Priority({ .origin = CascadeOrigin::kUser, .important = true }),
        Priority({ .origin = CascadeOrigin::kAuthor, .important = true }), Priority({ .origin = CascadeOrigin::kAnimation, .important = false }),
        Priority({ .origin = CascadeOrigin::kAuthor, .important = false }), Priority({ .origin = CascadeOrigin::kUser, .important = false }),
        Priority({ .origin = CascadeOrigin::kUserAgent, .important = false }), Priority({ .origin = CascadeOrigin::kNone, .important = false }) };
...
TEST(CascadePriorityTest, IsImportant)
{
    ...
    EXPECT_FALSE(Priority({ .origin = Origin::kAuthor, .important = false, .tree_order = 1024, .layer_order = 2048, .position = 4096 }).IsImportant());

    EXPECT_TRUE(Priority({ .origin = Origin::kUserAgent, .important = true }).IsImportant());
    EXPECT_TRUE(Priority({ .origin = Origin::kUser, .important = true }).IsImportant());
    EXPECT_TRUE(Priority({ .origin = Origin::kAuthor, .important = true }).IsImportant());
    EXPECT_TRUE(Priority({ .origin = Origin::kAnimation, .important = true }).IsImportant());
    EXPECT_TRUE(Priority({ .origin = Origin::kTransition, .important = true }).IsImportant());
    EXPECT_TRUE(Priority({ .origin = Origin::kAuthor, .important = true, .tree_order = 1024, .layer_order = 2048, .position = 4096 }).IsImportant());
}

static std::vector<CascadeOrigin> all_origins
    = { CascadeOrigin::kUserAgent, CascadeOrigin::kUser, CascadeOrigin::kAuthor, CascadeOrigin::kTransition, CascadeOrigin::kAnimation };

TEST(CascadePriorityTest, GetOrigin)
{
    for (CascadeOrigin origin : all_origins) {
        EXPECT_EQ(Priority({ .origin = origin, .important = false }).GetOrigin(), origin);
    }
    ...
    for (CascadeOrigin origin : all_origins) {
        ...
        EXPECT_EQ(Priority({ .origin = origin, .important = true }).GetOrigin(), origin);
    }
}
...
{
    EXPECT_GE(Priority({ .important = true, .layer_order = 0 }), Priority({ .important = true, .layer_order = 1 }));
    EXPECT_GE(Priority({ .important = true, .layer_order = 6 }), Priority({ .important = true, .layer_order = 7 }));
    EXPECT_GE(Priority({ .important = true, .layer_order = 4 }), Priority({ .important = true, .layer_order = 4 }));
    EXPECT_FALSE(Priority({ .important = true, .layer_order = 8 }) >= Priority({ .important = true, .layer_order = 1 }));
}
...
TEST(CascadePriorityTest, LayerOrderDifferentOrigin)
{
    ...
    CascadeOrigin transition = CascadeOrigin::kTransition;
    EXPECT_GE(Priority({ .origin = transition, .layer_order = 1 }), Priority({ .layer_order = 42 }));
    EXPECT_GE(Priority({ .origin = transition, .layer_order = 1 }), Priority({ .layer_order = 1 }));
}
...
TEST(CascadePriorityTest, InlineStyle)
{
    ...
    EXPECT_GE(Priority({ .tree_order = 1, .is_inline_style = true }), Priority({ .is_inline_style = false }));
    EXPECT_LT(Priority({ .tree_order = 1, .is_inline_style = true }), Priority({ .tree_order = 2 }));
    ...
    EXPECT_GE(Priority({ .important = true, .is_inline_style = true }), Priority({ .important = true, .position = 1 }));
    EXPECT_GE(Priority({ .important = true, .is_inline_style = true }), Priority({ .important = true, .layer_order = 1 }));
    EXPECT_LT(Priority({ .important = true, .tree_order = 1, .is_inline_style = true }), Priority({ .important = true }));
    EXPECT_GE(Priority({ .important = true, .tree_order = 1, .is_inline_style = true }), Priority({ .important = true, .tree_order = 2 }));
    EXPECT_LT(Priority({ .important = true, .is_inline_style = true }), Priority({ .origin = user, .important = true }));
    EXPECT_GE(Priority({ .important = true, .is_inline_style = true }), Priority({ .is_inline_style = false }));
}

TEST(CascadePriorityTest, TryStyle)
{
    EXPECT_GE(Priority({ .is_try_style = true }), Priority({}));
    EXPECT_GE(Priority({ .is_try_style = true }), Priority({ .is_inline_style = true }));
    EXPECT_GE(Priority({ .is_try_style = true }), Priority({ .layer_order = static_cast<uint16_t>(EncodeLayerOrder(1u, /* important */ false)) }));
    EXPECT_GE(Priority({ .is_try_style = true }), Priority({ .position = 1000 }));

    EXPECT_LT(Priority({ .is_try_style = true }), Priority({ .important = true }));
    EXPECT_LT(Priority({ .is_try_style = true }), Priority({ .origin = CascadeOrigin::kAnimation }));
    EXPECT_LT(Priority({ .is_try_style = true }), Priority({ .origin = CascadeOrigin::kTransition }));

    // Try styles generate a separate layer.
    EXPECT_NE(Priority({ .is_try_style = true }).ForLayerComparison(), Priority({}).ForLayerComparison());
}

TEST(CascadePriorityTest, TryTacticsStyle)
{
    // Should be stronger than try-style.
    EXPECT_GE(Priority({ .is_try_tactics_style = true }), Priority({ .is_try_style = true }));

    // Should be stronger than inline styles.
    EXPECT_GE(Priority({ .is_try_tactics_style = true }), Priority({ .is_inline_style = true }));

    // Should be stronger than author cascade layers.
    EXPECT_GE(Priority({ .is_try_tactics_style = true }), Priority({ .layer_order = 1000 }));

    // Should be weaker than important in the same origin
    EXPECT_LT(Priority({ .is_try_tactics_style = true }), Priority({ .important = true }));

    // Should be weaker than a stronger origin.
    EXPECT_LT(Priority({ .is_try_tactics_style = true }), Priority({ .origin = CascadeOrigin::kTransition }));

    // Try-tactics styles generate a separate layer.
    EXPECT_NE(Priority({ .is_try_tactics_style = true }).ForLayerComparison(), Priority({}).ForLayerComparison());
    // Also a separate layer vs. the try styles.
    EXPECT_NE(Priority({ .is_try_tactics_style = true }).ForLayerComparison(), Priority({ .is_try_style = true }).ForLayerComparison());
}
...
TEST(CascadePriorityTest, ForLayerComparison)
{
    ...
    EXPECT_EQ(Priority({ .layer_order = 1, .position = 2 }).ForLayerComparison(), Priority({ .layer_order = 1, .position = 8 }).ForLayerComparison());
    EXPECT_EQ(Priority({ .important = true, .tree_order = 1, .layer_order = 1, .position = 4 }).ForLayerComparison(),
        Priority({ .important = true, .tree_order = 1, .layer_order = 1, .position = 8 }).ForLayerComparison());
    EXPECT_EQ(Priority({ .important = true, .tree_order = 1, .layer_order = 1, .position = 16 }).ForLayerComparison(),
        Priority({ .tree_order = 1, .layer_order = 1, .position = 32 }).ForLayerComparison());
    EXPECT_EQ(Priority({ .important = true, .tree_order = 1, .is_inline_style = true, .position = 16 }).ForLayerComparison(),
        Priority({ .tree_order = 1, .is_inline_style = true, .position = 32 }).ForLayerComparison());

    EXPECT_LT(Priority({ .origin = user, .position = 1 }).ForLayerComparison(), Priority({ .origin = user, .layer_order = 1 }).ForLayerComparison());
    EXPECT_LT(Priority({ .origin = user, .position = 1 }).ForLayerComparison(), Priority({ .origin = user, .is_inline_style = true }).ForLayerComparison());
    EXPECT_LT(Priority({ .origin = user, .position = 1 }).ForLayerComparison(), Priority({ .origin = user, .tree_order = 1 }).ForLayerComparison());
    EXPECT_LT(Priority({ .origin = user, .position = 1 }).ForLayerComparison(), Priority({}).ForLayerComparison());
    EXPECT_LT(Priority({ .origin = user, .important = true, .position = 1 }).ForLayerComparison(),
    ...
    EXPECT_LT(Priority({ .origin = user, .important = true, .position = 1 }).ForLayerComparison(), Priority({}).ForLayerComparison());
    EXPECT_LT(Priority({ .origin = user, .position = 1 }).ForLayerComparison(),
        Priority({ .origin = user, .important = true, .layer_order = 1 }).ForLayerComparison());
    EXPECT_LT(Priority({ .origin = user, .position = 1 }).ForLayerComparison(),
        Priority({ .origin = user, .important = true, .is_inline_style = true }).ForLayerComparison());
    EXPECT_LT(Priority({ .origin = user, .position = 1 }).ForLayerComparison(),
        Priority({ .origin = user, .important = true, .tree_order = 1 }).ForLayerComparison());
    EXPECT_LT(Priority({ .origin = user, .position = 1 }).ForLayerComparison(), Priority({ .important = true }).ForLayerComparison());
    EXPECT_LT(Priority({ .origin = user, .important = true, .position = 1 }).ForLayerComparison(),
        Priority({ .origin = user, .important = true, .layer_order = 1 }).ForLayerComparison());
    EXPECT_LT(Priority({ .origin = user, .important = true, .position = 1 }).ForLayerComparison(),
        Priority({ .origin = user, .important = true, .is_inline_style = true }).ForLayerComparison());
    EXPECT_LT(Priority({ .origin = user, .important = true, .position = 1 }).ForLayerComparison(),
        Priority({ .origin = user, .important = true, .tree_order = 1 }).ForLayerComparison());
    EXPECT_LT(Priority({ .origin = user, .important = true, .position = 1 }).ForLayerComparison(), Priority({ .important = true }).ForLayerComparison());
}
