...
public:
    StyleCascade(StyleResolverState& state)
        : state_(state)
    {
    }
    ...
    bool InlineStyleLost() const
    {
        return map_.InlineStyleLost();
    }
    ...
    const CSSValue* Resolve(const CSSPropertyName&, const CSSValue&, CascadeOrigin, CascadeResolver&);
    ...
    static const CSSValue* Resolve(StyleResolverState&, const CSSPropertyName&, const CSSValue&);
...
private:
    ...
    void ApplyInterpolationMap(const ActiveInterpolationsMap&, CascadeOrigin, size_t index, CascadeResolver&);
    void ApplyInterpolation(const CSSProperty&, CascadePriority, const ActiveInterpolations&, CascadeResolver&);
    ...
    void LookupAndApplyValue(const CSSProperty&, CascadePriority*, CascadeResolver&);
    void LookupAndApplyDeclaration(const CSSProperty&, CascadePriority*, CascadeResolver&);
    void LookupAndApplyInterpolation(const CSSProperty&, CascadePriority*, CascadeResolver&);
    ...
    public:
        ...
        String OriginalText()
        {
            return original_text_.ToString();
        }

        bool Append(StringView str, wtf_size_t byte_limit = std::numeric_limits<wtf_size_t>::max());
        bool Append(CSSVariableData* data, wtf_size_t byte_limit = std::numeric_limits<wtf_size_t>::max());
        bool Append(const CSSValue* data, wtf_size_t byte_limit = std::numeric_limits<wtf_size_t>::max());
    ...
    const CSSValue* Resolve(const CSSProperty&, const CSSValue&, CascadePriority, CascadeOrigin&, CascadeResolver&);
    const CSSValue* ResolveSubstitutions(const CSSProperty&, const CSSValue&, CascadeResolver&);
    const CSSValue* ResolveCustomProperty(const CSSProperty&, const CSSUnparsedDeclarationValue&, CascadeResolver&);
    const CSSValue* ResolveVariableReference(const CSSProperty&, const CSSUnparsedDeclarationValue&, CascadeResolver&);
    const CSSValue* ResolvePendingSubstitution(const CSSProperty&, const CSSPendingSubstitutionValue&, CascadeResolver&);
    const CSSValue* ResolveRevert(const CSSProperty&, const CSSValue&, CascadeOrigin&, CascadeResolver&);
    const CSSValue* ResolveRevertLayer(const CSSProperty&, CascadePriority, CascadeOrigin&, CascadeResolver&);
    const CSSValue* ResolveFlipRevert(const CSSProperty&, const CSSFlipRevertValue&, CascadePriority, CascadeOrigin&, CascadeResolver&);
    const CSSValue* ResolveMathFunction(const CSSProperty&, const CSSMathFunctionValue&, CascadePriority);

    CSSVariableData* ResolveVariableData(CSSVariableData*, const CSSParserContext&, CascadeResolver&);
    ...
    bool ResolveTokensInto(
        CSSParserTokenStream&, CascadeResolver&, const CSSParserContext&, const FunctionContext&, CSSParserTokenType stop_type, TokenSequence&);
    bool ResolveVarInto(CSSParserTokenStream&, CascadeResolver&, const CSSParserContext&, TokenSequence&);
    bool ResolveEnvInto(CSSParserTokenStream&, CascadeResolver&, const CSSParserContext&, TokenSequence&);
    bool ResolveArgInto(CSSParserTokenStream&, CascadeResolver&, const CSSParserContext&, const FunctionContext&, TokenSequence&);
    bool ResolveAttrInto(CSSParserTokenStream&, CascadeResolver&, const CSSParserContext&, TokenSequence&);
    bool ResolveAppearanceAutoBaseSelectInto(CSSParserTokenStream&, CascadeResolver&, const CSSParserContext&, TokenSequence&);
    ...
    // definition.
    bool ResolveFunctionInto(StringView function_name, CSSParserTokenStream& stream, CascadeResolver& resolver, const CSSParserContext& context,
        const FunctionContext& function_context, TokenSequence& out);

    const CSSValue* ResolveFunctionExpression(StringView expr, const StyleRuleFunction::Type& type, CascadeResolver& resolver, const CSSParserContext& context,
        const FunctionContext& function_context);

    CSSVariableData* GetVariableData(const CustomProperty&) const;
    CSSVariableData* GetEnvironmentVariable(const AtomicString&, WTF::Vector<unsigned>) const;
    ...
    void MarkIsReferenced(const CSSProperty& referencer, const CustomProperty& referenced);
