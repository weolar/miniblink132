...
static Length ConvertToFloatLength(const CSSPrimitiveValue& primitive_value, const CSSToLengthConversionData& conversion_data)
{
    ...
    for (auto& value : value_list) {
        ...
        for (const CSSValue* item : *transform_value) {
            ...
            if (primitive_value.IsCalculated()) {
                ...
                CSSPrimitiveValue::UnitType unit_type = To<CSSNumericLiteralValue>(primitive_value).GetType();
...
TransformOperation* CreateTransformOperation(const CSSFunctionValue& transform_value, const CSSToLengthConversionData& conversion_data)
{
    TransformOperation::OperationType transform_type = GetTransformOperationType(transform_value.FunctionType());
    ...
    case TransformOperation::kScaleY: {
        ...
        return MakeGarbageCollected<ScaleTransformOperation>(sx, sy, 1.0, transform_type);
    }
    ...
    case TransformOperation::kScale3D: {
        ...
        if (transform_type == TransformOperation::kScaleZ) {
            ...
            sx = first_value.ComputeNumber(conversion_data);
            sy = To<CSSPrimitiveValue>(transform_value.Item(1)).ComputeNumber(conversion_data);
            sz = To<CSSPrimitiveValue>(transform_value.Item(2)).ComputeNumber(conversion_data);
        }
        return MakeGarbageCollected<ScaleTransformOperation>(sx, sy, sz, transform_type);
    }
    ...
    case TransformOperation::kTranslateY: {
        ...
        return MakeGarbageCollected<TranslateTransformOperation>(tx, ty, 0, transform_type);
    }
    ...
    case TransformOperation::kTranslate3D: {
        ...
        if (transform_type == TransformOperation::kTranslateZ) {
            ...
            tx = ConvertToFloatLength(first_value, conversion_data);
            ty = ConvertToFloatLength(To<CSSPrimitiveValue>(transform_value.Item(1)), conversion_data);
            tz = To<CSSPrimitiveValue>(transform_value.Item(2)).ComputeLength<double>(conversion_data);
        }

        return MakeGarbageCollected<TranslateTransformOperation>(tx, ty, tz, transform_type);
    }
    ...
    case TransformOperation::kRotate: {
        ...
        if (transform_value.length() == 1) {
            ...
            double z = transform_type == TransformOperation::kRotateZ || transform_type == TransformOperation::kRotate;
            return MakeGarbageCollected<RotateTransformOperation>(x, y, z, angle, transform_type);
        } else {
            // For SVG 'transform' attributes we generate 3-argument rotate()
            // functions.
            DCHECK_EQ(transform_value.length(), 3u);
            const auto& second_value = To<CSSPrimitiveValue>(transform_value.Item(1));
            const CSSPrimitiveValue& third_value = To<CSSPrimitiveValue>(transform_value.Item(2));
            return MakeGarbageCollected<RotateAroundOriginTransformOperation>(
                angle, second_value.ComputeLength<double>(conversion_data), third_value.ComputeLength<double>(conversion_data));
        }
    }
    ...
    case TransformOperation::kSkewY: {
        ...
        return MakeGarbageCollected<SkewTransformOperation>(angle_x, angle_y, transform_type);
    }
    case TransformOperation::kMatrix: {
        double a = To<CSSPrimitiveValue>(transform_value.Item(0)).ComputeNumber(conversion_data);
        double b = To<CSSPrimitiveValue>(transform_value.Item(1)).ComputeNumber(conversion_data);
        double c = To<CSSPrimitiveValue>(transform_value.Item(2)).ComputeNumber(conversion_data);
        double d = To<CSSPrimitiveValue>(transform_value.Item(3)).ComputeNumber(conversion_data);
        double e = conversion_data.Zoom() * To<CSSPrimitiveValue>(transform_value.Item(4)).ComputeNumber(conversion_data);
        double f = conversion_data.Zoom() * To<CSSPrimitiveValue>(transform_value.Item(5)).ComputeNumber(conversion_data);
        return MakeGarbageCollected<MatrixTransformOperation>(a, b, c, d, e, f);
    }
    case TransformOperation::kMatrix3D: {
        auto matrix = gfx::Transform::ColMajor(To<CSSPrimitiveValue>(transform_value.Item(0)).ComputeNumber(conversion_data),
            To<CSSPrimitiveValue>(transform_value.Item(1)).ComputeNumber(conversion_data),
            To<CSSPrimitiveValue>(transform_value.Item(2)).ComputeNumber(conversion_data),
            To<CSSPrimitiveValue>(transform_value.Item(3)).ComputeNumber(conversion_data),
            To<CSSPrimitiveValue>(transform_value.Item(4)).ComputeNumber(conversion_data),
            To<CSSPrimitiveValue>(transform_value.Item(5)).ComputeNumber(conversion_data),
            To<CSSPrimitiveValue>(transform_value.Item(6)).ComputeNumber(conversion_data),
            To<CSSPrimitiveValue>(transform_value.Item(7)).ComputeNumber(conversion_data),
            To<CSSPrimitiveValue>(transform_value.Item(8)).ComputeNumber(conversion_data),
            To<CSSPrimitiveValue>(transform_value.Item(9)).ComputeNumber(conversion_data),
            To<CSSPrimitiveValue>(transform_value.Item(10)).ComputeNumber(conversion_data),
            To<CSSPrimitiveValue>(transform_value.Item(11)).ComputeNumber(conversion_data),
            To<CSSPrimitiveValue>(transform_value.Item(12)).ComputeNumber(conversion_data),
            To<CSSPrimitiveValue>(transform_value.Item(13)).ComputeNumber(conversion_data),
            To<CSSPrimitiveValue>(transform_value.Item(14)).ComputeNumber(conversion_data),
            To<CSSPrimitiveValue>(transform_value.Item(15)).ComputeNumber(conversion_data));
    ...
    case TransformOperation::kPerspective: {
        ...
        if (primitive_value) {
            ...
            DCHECK_EQ(To<CSSIdentifierValue>(first_value).GetValueID(), CSSValueID::kNone);
...
TransformOperations TransformBuilder::CreateTransformOperations(const CSSValue& in_value, const CSSToLengthConversionData& conversion_data)
{
    TransformOperations operations;
    if (auto* in_value_function = DynamicTo<CSSFunctionValue>(in_value)) {
        operations.Operations().push_back(CreateTransformOperation(*in_value_function, conversion_data));
    } else if (auto* in_value_list = DynamicTo<CSSValueList>(in_value)) {
        for (auto& value : *in_value_list) {
            const auto* transform_value = To<CSSFunctionValue>(value.Get());
            operations.Operations().push_back(CreateTransformOperation(*transform_value, conversion_data));
        }
    } else {
