...
public:
    ...
    Element* GetPseudoElement() const
    {
    ...
    const Element* LayoutParentElement() const
    {
        return layout_parent_;
    }
    const ComputedStyle* RootElementStyle() const
    {
        return root_element_style_;
    }
    ...
    {
        return LayoutParentElement() ? LayoutParentElement()->GetComputedStyle() : nullptr;
    }
    EInsideLink ElementLinkState() const
    {
        return element_link_state_;
    }

    // Some view-transition pseudos can be nested as much as 5 elements.
    static constexpr wtf_size_t kMaxPseudoElementsNesting = 5u;
    using PseudoElementAncestors = std::array<Element*, kMaxPseudoElementsNesting>;
    base::span<Element* const> GetPseudoElementAncestors() const
    {
        return base::span(pseudo_element_ancestors_).subspan(pseudo_element_ancestors_size_, PseudoElementAncestorsSize());
    }
