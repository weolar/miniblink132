...
public:
    ...
    bool InCycle() const
    {
        return resolver_.InCycle();
    }
    bool DetectCycle(const CSSProperty& property)
    {
        return resolver_.DetectCycle(property);
    }
    wtf_size_t CycleStart() const
    {
        return resolver_.cycle_start_;
    }
    wtf_size_t CycleEnd() const
    {
        return resolver_.cycle_end_;
    }
    uint8_t GetGeneration()
    {
        return resolver_.generation_;
    }
    CascadeResolver& InnerResolver()
    {
        return resolver_;
    }
...
public:
    explicit TestCascade(Document& document, Element* target = nullptr, const StyleRecalcContext* style_recalc_context = nullptr)
        : state_(document, target ? *target : *document.body(), style_recalc_context)
        , cascade_(InitState(state_, nullptr))
    {
    }

    TestCascade(Document& document, const ComputedStyle* parent_style, Element* target = nullptr)
    ...
    {
        CSSParserMode mode = options.origin == CascadeOrigin::kUserAgent ? kUASheetMode : kHTMLStandardMode;
    ...
    {
        DCHECK_LE(options.origin, CascadeOrigin::kAuthor) << "Animations not supported";
        DCHECK_LE(current_origin_, options.origin) << "Please add declarations in order";
        EnsureAtLeast(options.origin);
        cascade_.MutableMatchResult().AddMatchedProperties(set,
            {
    ...
    const CSSValue* Resolve(const CSSProperty& property, const CSSValue& value, CascadeOrigin& origin)
    {
        TestCascadeResolver resolver;
        return cascade_.Resolve(property, value, CascadePriority(origin), origin, resolver.InnerResolver());
    }

    static const CSSValue* StaticResolve(StyleResolverState& state, String name, String value)
    {
        const CSSPropertyValueSet* set = ParseDeclarationBlock(name + ":" + value, kHTMLStandardMode);
        DCHECK(set);
    ...
    String ComputedValue(String name) const
    {
        ...
        const CSSValue* value = ref.GetProperty().CSSValueFromComputedStyle(*style, layout_object, allow_visited_style, value_phase);
        return value ? value->CssText() : g_null_atom;
    }

    CascadePriority GetPriority(String name)
    {
        return GetPriority(*CSSPropertyName::From(GetDocument().GetExecutionContext(), name));
    }
    ...
    CascadeOrigin GetOrigin(String name)
    {
        return GetPriority(name).GetOrigin();
    }

    void AddInterpolations()
    {
        state_.StyleBuilder().SetBaseData(StyleBaseData::Create(state_.StyleBuilder().CloneStyle(), nullptr));
        ...
        cascade_.AddInterpolations(&update.ActiveInterpolationsForAnimations(), CascadeOrigin::kAnimation);
        cascade_.AddInterpolations(&update.ActiveInterpolationsForTransitions(), CascadeOrigin::kTransition);
    }
    ...
    {
        return cascade_.InlineStyleLost();
    }

    HeapHashMap<CSSPropertyName, Member<const CSSValue>> GetCascadedValues() const
    {
...
private:
    ...
    static StyleResolverState& InitState(StyleResolverState& state, const ComputedStyle* parent_style)
    {
    ...
    void FinishOrigin()
    {
        ...
        case CascadeOrigin::kAuthorPresentationalHint:
            cascade_.MutableMatchResult().BeginAddingAuthorRulesForTreeScope(GetDocument());
    ...
    void CalculateInterpolationUpdate()
    {
        CSSAnimations::CalculateTransitionUpdate(
            state_.AnimationUpdate(), state_.GetElement(), state_.StyleBuilder(), state_.OldStyle(), true /* can_trigger_animations */);
        CSSAnimations::CalculateAnimationUpdate(state_.AnimationUpdate(), state_.GetElement(), state_.GetElement(), state_.StyleBuilder(), state_.ParentStyle(),
...
public:
    TestCascadeAutoLock(const CSSProperty& property, TestCascadeResolver& resolver)
...
public:
    CSSStyleSheet* CreateSheet(const String& css_text)
    {
        ...
        CSSStyleSheet* sheet = CSSStyleSheet::Create(GetDocument(), init, exception_state);
        sheet->replaceSync(css_text, exception_state);
        sheet->Contents()->EnsureRuleSet(MediaQueryEvaluator(GetDocument().GetFrame()));
        return sheet;
    }
    ...
    void AppendSheet(const String& css_text)
    {
        ...
        ScopedStyleResolver& scoped_resolver = tree_scope.EnsureScopedStyleResolver();
        ActiveStyleSheetVector active_sheets;
        active_sheets.push_back(std::make_pair(sheet, &sheet->Contents()->GetRuleSet()));
        scoped_resolver.AppendActiveStyleSheets(0, active_sheets);
        GetDocument().GetStyleEngine().GetDocumentStyleSheetCollection().AppendActiveStyleSheet(active_sheets[0]);
    }
    ...
    const MutableCSSPropertyValueSet* AnimationTaintedSet(const char* name, String value)
    {
        ...
        set->ParseAndSetCustomProperty(AtomicString(name), value,
            /* important */ false, SecureContextMode::kSecureContext,
            /* context_style_sheet */ nullptr,
            /* is_animation_tainted */ true);
        return set;
    }

    const CSSPropertyValueSet* FlipRevertSet(String from_property, String to_property)
    {
        auto* set = MakeGarbageCollected<MutableCSSPropertyValueSet>(kHTMLStandardMode);
        set->SetProperty(
            PropertyName(from_property).Id(), *MakeGarbageCollected<cssvalue::CSSFlipRevertValue>(PropertyName(to_property).Id(), TryTacticTransform()));
        return set;
    }
    ...
    public:
        ...
        ~AutoEnv()
        {
            EnsureEnvironmentVariables().RemoveVariable(name_);
        }
    ...
    String CssTextAt(const HeapHashMap<CSSPropertyName, Member<const CSSValue>>& map, String name)
    {
...
TEST_F(StyleCascadeTest, ApplyGenerations)
{
    ...
    cascade.State().StyleBuilder().SetVariableData(AtomicString("--x"), nullptr, true);
...
TEST_F(StyleCascadeTest, RevertCausesTransition)
{
    ...
    cascade2.Add("transition: width 1000s steps(2, start)", CascadeOrigin::kAuthor);
    cascade2.Apply();
...
TEST_F(StyleCascadeTest, TransitionCausesInterpolationValue)
{
    ...
    EXPECT_EQ(CascadeOrigin::kTransition, cascade2.GetPriority("--x").GetOrigin());
}
...
TEST_F(StyleCascadeTest, TransitionsWinOverAuthorImportant)
{
    ...
    EXPECT_EQ(CascadeOrigin::kTransition, cascade2.GetPriority("width").GetOrigin());
    EXPECT_EQ(CascadeOrigin::kTransition, cascade2.GetPriority("height").GetOrigin());
}
...
TEST_F(StyleCascadeTest, AnimatedVisitedImportantOverride)
{
    ...
    cascade.Add(ParseDeclarationBlock("background-color:red !important"), { .link_match_type = CSSSelector::kMatchVisited });
    cascade.Add("animation-name:test");
    cascade.Add("animation-duration:10s");
    cascade.Add("animation-timing-function:linear");
    cascade.Add("animation-delay:-5s");
    ...
    EXPECT_EQ(Color(255, 0, 0), style->VisitedDependentColor(GetCSSPropertyBackgroundColor()));

    builder = ComputedStyleBuilder(*style);
    builder.SetInsideLink(EInsideLink::kNotInsideLink);
    style = builder.TakeStyle();
    EXPECT_EQ(Color(150, 150, 150), style->VisitedDependentColor(GetCSSPropertyBackgroundColor()));
}
...
TEST_F(StyleCascadeTest, AnimatedVisitedHighPrio)
{
    ...
    builder.SetInsideLink(EInsideLink::kInsideVisitedLink);
    style = builder.TakeStyle();
    EXPECT_EQ(Color(150, 150, 150), style->VisitedDependentColor(GetCSSPropertyColor()));
...
TEST_F(StyleCascadeTest, ZoomExplicitDefault)
{
    ...
    CascadePriority* priority = cascade.FindPriority(CSSPropertyName(CSSPropertyID::kLineHeight));
...
TEST_F(StyleCascadeTest, RubyPositionSurrogateCanCascadeAsOriginal)
{
    ...
    for (const auto& test : tests) {
        TestCascade cascade(GetDocument());
        auto* set = MakeGarbageCollected<MutableCSSPropertyValueSet>(kHTMLStandardMode);
        set->SetProperty(CSSPropertyID::kWebkitRubyPosition, *CSSIdentifierValue::Create(test.specified));
        cascade.Add(set);
        cascade.Apply();
        EXPECT_EQ(test.unprefixed_expected, cascade.ComputedValue("ruby-position"));
        EXPECT_EQ(test.webkit_expected, cascade.ComputedValue("-webkit-ruby-position"));
    }
}
...
TEST_F(StyleCascadeTest, TextOrientationLegacyKeyword)
{
    ...
    EXPECT_EQ("vertical-right", cascade.ComputedValue("-webkit-text-orientation"));
}
...
TEST_F(StyleCascadeTest, WebkitBorderImageCascadeOrder)
{
    ...
    TestCascade cascade(GetDocument());
    cascade.Add("-webkit-border-image", gradient1 + " round 40 / 10px / 20px", Origin::kAuthor);
...
TEST_F(StyleCascadeTest, InternalVisitedColorLonghand)
{
    TestCascade cascade(GetDocument());
    cascade.Add("color:green");
    cascade.Add("color:red", { .origin = CascadeOrigin::kAuthor, .link_match_type = CSSSelector::kMatchVisited });
...
TEST_F(StyleCascadeTest, VarInInternalVisitedColorShorthand)
{
    TestCascade cascade(GetDocument());
    cascade.Add("--x:red");
    cascade.Add("outline:medium solid var(--x)", { .link_match_type = CSSSelector::kMatchVisited });
    cascade.Add("outline-color:green", { .link_match_type = CSSSelector::kMatchLink });
    ...
    Color red(255, 0, 0);
    const css_longhand::OutlineColor& outline_color = GetCSSPropertyOutlineColor();
...
{
    TestCascade cascade(GetDocument());
    cascade.Add("border-image:linear-gradient(green, red) 1 / 2 / 3 round", Origin::kAuthor);
    cascade.Add("-webkit-border-image:linear-gradient(green, red) 4 / 5 / 6 round", Origin::kAuthor);
    cascade.Apply(CascadeFilter(CSSProperty::kLegacyOverlapping, true));
    EXPECT_EQ("linear-gradient(rgb(0, 128, 0), rgb(255, 0, 0)) 1 / 2 / 3 round", cascade.ComputedValue("-webkit-border-image"));
}
...
{
    Vector<String> properties = { "background-attachment", "background-clip", "background-image", "background-origin", "background-position-x",
...
{
    Vector<String> properties = { "border-top-color", "border-right-color", "border-bottom-color", "border-left-color", "border-top-style",
        ...
        "border-top-left-radius", "border-top-right-radius", "border-bottom-left-radius", "border-bottom-right-radius", "border-image-source",
...
TEST_F(StyleCascadeTest, GetImportantSetMany)
{
    ...
    ASSERT_TRUE(cascade.GetImportantSet());
    EXPECT_EQ(CSSBitset({ CSSPropertyID::kWidth, CSSPropertyID::kHeight, CSSPropertyID::kTop }), *cascade.GetImportantSet());
}
...
TEST_F(StyleCascadeTest, RootColorNotModifiedByEmptyCascade)
{
    ...
    builder.SetInsideLink(EInsideLink::kInsideVisitedLink);
    style = builder.TakeStyle();
    EXPECT_EQ(Color(255, 0, 0), style->VisitedDependentColor(GetCSSPropertyColor()));
...
TEST_F(StyleCascadeTest, MaxVariableBytes)
{
    ...
    cascade.Add("--at-limit-reference-fallback", "var(--unknown,var(--at-limit))");
    cascade.Add("--above-limit-reference-fallback", "var(--unknown,var(--above-limit))");
    cascade.Apply();
    ...
    EXPECT_EQ(g_null_atom, cascade.ComputedValue("--above-limit-reference-fallback"));
}
...
TEST_F(StyleCascadeTest, StaticResolveNoVar)
{
    ...
    EXPECT_EQ("thing", CssText(TestCascade::StaticResolve(cascade.State(), "--x", "thing")));
    EXPECT_EQ("red", CssText(TestCascade::StaticResolve(cascade.State(), "color", "red")));
    EXPECT_EQ("10px", CssText(TestCascade::StaticResolve(cascade.State(), "width", "10px")));
    EXPECT_EQ("10em", CssText(TestCascade::StaticResolve(cascade.State(), "width", "10em")));
    EXPECT_EQ("calc(1% + 1em)", CssText(TestCascade::StaticResolve(cascade.State(), "width", "calc(1% + 1em)")));
}
...
TEST_F(StyleCascadeTest, StaticResolveVar)
{
    ...
    EXPECT_EQ("foo", CssText(TestCascade::StaticResolve(cascade.State(), "--y", "var(--x)")));
    EXPECT_EQ("foo bar", CssText(TestCascade::StaticResolve(cascade.State(), "--y", "var(--x) bar")));
    EXPECT_EQ("bar", CssText(TestCascade::StaticResolve(cascade.State(), "--y", "var(--unknown,bar)")));
    EXPECT_EQ("unset", CssText(TestCascade::StaticResolve(cascade.State(), "--y", "var(--unknown)")));
}
...
TEST_F(StyleCascadeTest, StaticResolveRegisteredVar)
{
    ...
    EXPECT_EQ("100px", CssText(TestCascade::StaticResolve(cascade.State(), "--y", "var(--x)")));
    EXPECT_EQ("100px", CssText(TestCascade::StaticResolve(cascade.State(), "--z", "var(--x)")));

    EXPECT_EQ("50px", CssText(TestCascade::StaticResolve(cascade.State(), "--y", "var(--unknown, 50px)")));
    EXPECT_EQ("50px", CssText(TestCascade::StaticResolve(cascade.State(), "--z", "var(--unknown, 50px)")));

    EXPECT_EQ("unset", CssText(TestCascade::StaticResolve(cascade.State(), "--y", "var(--unknown)")));
    EXPECT_EQ("unset", CssText(TestCascade::StaticResolve(cascade.State(), "--z", "var(--unknown)")));

    // StyleCacade::Resolve does not actually compute values, just eliminate
    // var() references.
    EXPECT_EQ("calc(5em + 100px)", CssText(TestCascade::StaticResolve(cascade.State(), "--y", "calc(5em + var(--x))")));
    EXPECT_EQ("calc(5em + 100px)", CssText(TestCascade::StaticResolve(cascade.State(), "--z", "calc(5em + var(--x))")));
}
...
TEST_F(StyleCascadeTest, RevertOrigin)
{
    ...
    CascadeOrigin origin = CascadeOrigin::kAuthor;
    const CSSValue* resolved_value = cascade.Resolve(GetCSSPropertyWidth(), *revert_value, origin);
    ASSERT_TRUE(resolved_value);
    EXPECT_EQ(CascadeOrigin::kUser, origin);
    EXPECT_EQ("2px", resolved_value->CssText());

    origin = CascadeOrigin::kAuthor;
    resolved_value = cascade.Resolve(GetCSSPropertyHeight(), *revert_value, origin);
    ASSERT_TRUE(resolved_value);
    EXPECT_EQ(CascadeOrigin::kUserAgent, origin);
    EXPECT_EQ("1px", resolved_value->CssText());

    origin = CascadeOrigin::kAuthor;
    resolved_value = cascade.Resolve(GetCSSPropertyDisplay(), *revert_value, origin);
    ASSERT_TRUE(resolved_value);
    EXPECT_EQ(CascadeOrigin::kUserAgent, origin);
    EXPECT_EQ("block", resolved_value->CssText());

    origin = CascadeOrigin::kAuthor;
    resolved_value = cascade.Resolve(GetCSSPropertyMarginLeft(), *revert_value, origin);
    ASSERT_TRUE(resolved_value);
...
TEST_F(StyleCascadeTest, InlineStyleWonCascade)
{
    TestCascade cascade(GetDocument());
    cascade.Add("top:1px", CascadeOrigin::kUserAgent);
    cascade.Add("top:2px", { .origin = CascadeOrigin::kAuthor, .is_inline_style = true });
    cascade.Apply();
...
public:
    std::optional<LayoutUnit> Evaluate(const AnchorQuery&, const ScopedCSSName* position_anchor, const std::optional<PositionAreaOffsets>&) override
    {
    ...
    std::optional<PositionAreaOffsets> ComputePositionAreaOffsetsForLayout(const ScopedCSSName*, PositionArea) override
    {
        return PositionAreaOffsets();
    }
    std::optional<PhysicalOffset> ComputeAnchorCenterOffsets(const ComputedStyleBuilder&) override
    {
        return std::nullopt;
    }
};
...
TEST_F(StyleCascadeTest, FlipToAnchorInvalid)
{
    ...
    TestCascade cascade(GetDocument(), /* element */ GetDocument().body(), &style_recalc_context);
    cascade.Add("position:absolute");
