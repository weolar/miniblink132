...
void MatchResult::AddMatchedProperties(const CSSPropertyValueSet* properties, const MatchedProperties::Data& data)
{
    MatchedProperties::Data new_data = data;
    new_data.tree_order = current_tree_order_;
    matched_properties_.emplace_back(const_cast<CSSPropertyValueSet*>(properties), new_data);
...
void MatchResult::BeginAddingAuthorRulesForTreeScope(const TreeScope& tree_scope)
{
    current_tree_order_ = ClampTo<decltype(current_tree_order_)>(tree_scopes_.size());
