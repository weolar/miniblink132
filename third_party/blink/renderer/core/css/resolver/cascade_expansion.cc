...
CascadeFilter AddValidPropertiesFilter(CascadeFilter filter, const MatchedProperties& matched_properties)
{
    switch (static_cast<ValidPropertyFilter>(matched_properties.data_.valid_property_filter)) {
...
CascadeFilter AddLinkFilter(CascadeFilter filter, const MatchedProperties& matched_properties)
{
...
CORE_EXPORT CascadeFilter CreateExpansionFilter(const MatchedProperties& matched_properties)
{
    return AddLinkFilter(AddValidPropertiesFilter(CascadeFilter(), matched_properties), matched_properties);
}
...
CORE_EXPORT bool IsInAllExpansion(CSSPropertyID id)
{
    ...
    return !property.IsShorthand() && (property.IsAffectedByAll() || (unvisited && unvisited->IsAffectedByAll()));
}
