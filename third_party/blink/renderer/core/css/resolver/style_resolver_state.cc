...
Element* ComputeStyledElement(const StyleRequest& style_request, Element& element)
{
    ...
    if (style_request.IsPseudoStyleRequest()) {
        styled_element = styled_element->GetStyledPseudoElement(style_request.pseudo_id, style_request.pseudo_argument);
    }
...
StyleResolverState::StyleResolverState(Document& document, Element& element, const StyleRecalcContext* style_recalc_context, const StyleRequest& style_request)
    ...
    , old_style_(style_recalc_context ? style_recalc_context->old_style : nullptr)
    , pseudo_request_type_(style_request.type)
    , font_builder_(&document)
    , styled_element_(ComputeStyledElement(style_request, element))
    , element_style_resources_(GetStyledElement() ? *GetStyledElement() : GetElement(), document.DevicePixelRatio())
    , element_type_(style_request.IsPseudoStyleRequest() || element.IsPseudoElement() ? ElementType::kPseudoElement : ElementType::kElement)
    , container_unit_context_(style_recalc_context ? style_recalc_context->container : ContainerQueryEvaluator::ParentContainerCandidateElement(element))
    , anchor_evaluator_(style_recalc_context ? style_recalc_context->anchor_evaluator : nullptr)
    ...
    , uses_highlight_pseudo_inheritance_(::blink::UsesHighlightPseudoInheritance(style_request.pseudo_id))
    , is_outside_flat_tree_(style_recalc_context ? style_recalc_context->is_outside_flat_tree : false)
...
{
    if (had_no_matched_properties_ && pseudo_request_type_ == StyleRequest::kForRenderer) {
        return nullptr;
    }
...
{
    css_to_length_conversion_data_ = CSSToLengthConversionData(*style_builder_, ParentStyle(), RootElementStyle(),
        GetDocument().GetStyleEngine().GetViewportSize(), CSSToLengthConversionData::ContainerSizes(container_unit_context_),
        CSSToLengthConversionData::AnchorData(anchor_evaluator_, StyleBuilder().PositionAnchor(), StyleBuilder().PositionAreaOffsets()),
...
CSSToLengthConversionData StyleResolverState::UnzoomedLengthConversionData(const FontSizeStyle& font_size_style)
{
    ...
    CSSToLengthConversionData::FontSizes font_sizes(font_size_style, root_font_style);
    CSSToLengthConversionData::LineHeightSize line_height_size(
        ParentStyle() ? ParentStyle()->GetFontSizeStyle() : style_builder_->GetFontSizeStyle(), root_font_style);
    CSSToLengthConversionData::ViewportSize viewport_size(GetDocument().GetLayoutView());
    CSSToLengthConversionData::ContainerSizes container_sizes(container_unit_context_);
    CSSToLengthConversionData::AnchorData anchor_data(anchor_evaluator_, StyleBuilder().PositionAnchor(), StyleBuilder().PositionAreaOffsets());
    return CSSToLengthConversionData(
        StyleBuilder().GetWritingMode(), font_sizes, line_height_size, viewport_size, container_sizes, anchor_data, 1, length_conversion_flags_);
}
...
void StyleResolverState::SetLayoutParentStyle(const ComputedStyle* parent_style)
{
...
{
    if (pseudo_request_type_ == StyleRequest::kForComputedStyle || (ParentStyle() && ParentStyle()->IsEnsuredInDisplayNone())
        || StyleBuilder().IsEnsuredOutsideFlatTree()) {
        return;
    }
    if (StyleBuilder().Display() == EDisplay::kNone && !GetElement().LayoutObjectIsNeeded(style_builder_->GetDisplayStyle())) {
        ...
        if (!animating_element || !CSSAnimations::IsAnimatingDisplayProperty(animating_element->GetElementAnimations())) {
            return;
        }
    }

    if (StyleBuilder().StyleType() == kPseudoIdSearchText || StyleBuilder().StyleType() == kPseudoIdTargetText) {
...
{
    float parent_effective_zoom = ParentStyle() ? ParentStyle()->EffectiveZoom() : ComputedStyleInitialValues::InitialZoom();
...
void StyleResolverState::SetTextSizeAdjust(TextSizeAdjust new_text_size_adjust)
{
...
void StyleResolverState::SetPositionAnchor(ScopedCSSName* position_anchor)
{
    if (StyleBuilder().PositionAnchor() != position_anchor) {
        ...
        css_to_length_conversion_data_.SetAnchorData(
            CSSToLengthConversionData::AnchorData(anchor_evaluator_, position_anchor, StyleBuilder().PositionAreaOffsets()));
    }
}

void StyleResolverState::SetPositionAreaOffsets(const std::optional<PositionAreaOffsets>& position_area_offsets)
{
    if (StyleBuilder().PositionAreaOffsets() != position_area_offsets) {
        StyleBuilder().SetPositionAreaOffsets(position_area_offsets);
        css_to_length_conversion_data_.SetAnchorData(
            CSSToLengthConversionData::AnchorData(anchor_evaluator_, StyleBuilder().PositionAnchor(), position_area_offsets));
    }
}
...
void StyleResolverState::UpdateFont()
{
    ...
    SetConversionFontSizes(CSSToLengthConversionData::FontSizes(style_builder_->GetFontSizeStyle(), RootElementStyle()));
...
void StyleResolverState::UpdateLineHeight()
{
    css_to_length_conversion_data_.SetLineHeightSize(
        CSSToLengthConversionData::LineHeightSize(style_builder_->GetFontSizeStyle(), GetDocument().documentElement()->GetComputedStyle()));
}

bool StyleResolverState::CanAffectAnimations() const
{
    return conditionally_affects_animations_ || StyleBuilder().CanAffectAnimations();
}
