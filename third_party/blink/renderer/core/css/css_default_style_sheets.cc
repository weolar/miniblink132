...
String MaybeRemoveCSSImportant(String string)
{
    ...
    return string.EndsWith(kImportantSuffix) ? string.Substring(0, string.length() - kImportantSuffix.length()) : string;
}
} // namespace
...
{
    DEFINE_STATIC_LOCAL(Persistent<CSSDefaultStyleSheets>, css_default_style_sheets, (MakeGarbageCollected<CSSDefaultStyleSheets>()));
    return *css_default_style_sheets;
}

static const MediaQueryEvaluator& PrintEval()
{
    DEFINE_STATIC_LOCAL(const Persistent<MediaQueryEvaluator>, static_print_eval, (MakeGarbageCollected<MediaQueryEvaluator>("print")));
...
static const MediaQueryEvaluator& ForcedColorsEval()
{
    ...
    DEFINE_STATIC_LOCAL(Persistent<MediaQueryEvaluator>, forced_colors_eval, (MakeGarbageCollected<MediaQueryEvaluator>("ua-forced-colors")));
...
StyleSheetContents* CSSDefaultStyleSheets::ParseUASheet(const String& str)
{
    ...
    auto* sheet = MakeGarbageCollected<StyleSheetContents>(MakeGarbageCollected<CSSParserContext>(kUASheetMode, SecureContextMode::kInsecureContext));
...
{
    DEFINE_STATIC_LOCAL(const Persistent<MediaQueryEvaluator>, static_screen_eval, (MakeGarbageCollected<MediaQueryEvaluator>("screen")));
...
{
    // Strict-mode rules.
    String default_rules = UncompressResourceAsASCIIString(IDR_UASTYLE_HTML_CSS) + LayoutTheme::GetTheme().ExtraDefaultStyleSheet();
...
void CSSDefaultStyleSheets::VerifyUniversalRuleCount()
{
    ...
    if (fullscreen_style_sheet_) {
        ...
        DCHECK(default_fullscreen_style_->UniversalRules().size() == 7u || default_fullscreen_style_->UniversalRules().size() == 8u);
    }
...
RuleSet* CSSDefaultStyleSheets::DefaultViewSourceStyle()
{
    if (!default_view_source_style_) {
        ...
        StyleSheetContents* stylesheet = ParseUASheet(UncompressResourceAsASCIIString(IDR_UASTYLE_VIEW_SOURCE_CSS));
...
RuleSet* CSSDefaultStyleSheets::DefaultJSONDocumentStyle()
{
    if (!default_json_document_style_) {
        StyleSheetContents* stylesheet = ParseUASheet(UncompressResourceAsASCIIString(IDR_UASTYLE_JSON_DOCUMENT_CSS));
...
static void AddTextTrackCSSProperties(StringBuilder* builder, CSSPropertyID propertyId, String value)
{
...
void CSSDefaultStyleSheets::AddRulesToDefaultStyleSheets(StyleSheetContents* rules, NamespaceType type)
{
    switch (type) {
    ...
    if (default_forced_color_style_) {
        ...
        case NamespaceType::kMediaControls:
            if (!default_forced_colors_media_controls_style_) {
                default_forced_colors_media_controls_style_ = MakeGarbageCollected<RuleSet>();
            }
            default_forced_colors_media_controls_style_->AddRulesFromSheet(rules, ForcedColorsEval());
            break;
        default:
...
bool CSSDefaultStyleSheets::EnsureDefaultStyleSheetsForElement(const Element& element)
{
    ...
    if (element.IsSVGElement() && !svg_style_sheet_) {
        svg_style_sheet_ = ParseUASheet(UncompressResourceAsASCIIString(IDR_UASTYLE_SVG_CSS));
        AddRulesToDefaultStyleSheets(svg_style_sheet_, NamespaceType::kSVG);
        changed_default_style = true;
    }

    // FIXME: We should assert that the sheet only styles MathML elements.
    if (element.namespaceURI() == mathml_names::kNamespaceURI && !mathml_style_sheet_) {
        mathml_style_sheet_ = ParseUASheet(UncompressResourceAsASCIIString(IDR_UASTYLE_MATHML_CSS));
        AddRulesToDefaultStyleSheets(mathml_style_sheet_, NamespaceType::kMathML);
        changed_default_style = true;
    }

    if (!media_controls_style_sheet_ && HasMediaControlsStyleSheetLoader() && (IsA<HTMLVideoElement>(element) || IsA<HTMLAudioElement>(element))) {
        // FIXME: We should assert that this sheet only contains rules for <video>
        // and <audio>.
        media_controls_style_sheet_ = ParseUASheet(media_controls_style_sheet_loader_->GetUAStyleSheet());
    ...
    if (!text_track_style_sheet_ && IsA<HTMLVideoElement>(element)) {
        ...
        if (settings) {
            ...
            if (CSSParser::ParseColor(color, MaybeRemoveCSSImportant(settings->GetTextTrackWindowColor()),
                    /*strict=*/true)
                && color.Alpha() > 0) {
                AddTextTrackCSSProperties(&builder, CSSPropertyID::kBackgroundColor, settings->GetTextTrackWindowColor());
                AddTextTrackCSSProperties(&builder, CSSPropertyID::kBorderRadius, settings->GetTextTrackWindowRadius());
            } else {
                AddTextTrackCSSProperties(&builder, CSSPropertyID::kBackgroundColor, settings->GetTextTrackBackgroundColor());
            }
            AddTextTrackCSSProperties(&builder, CSSPropertyID::kFontFamily, settings->GetTextTrackFontFamily());
            AddTextTrackCSSProperties(&builder, CSSPropertyID::kFontStyle, settings->GetTextTrackFontStyle());
            AddTextTrackCSSProperties(&builder, CSSPropertyID::kFontVariant, settings->GetTextTrackFontVariant());
            AddTextTrackCSSProperties(&builder, CSSPropertyID::kColor, settings->GetTextTrackTextColor());
            AddTextTrackCSSProperties(&builder, CSSPropertyID::kTextShadow, settings->GetTextTrackTextShadow());
            AddTextTrackCSSProperties(&builder, CSSPropertyID::kFontSize, settings->GetTextTrackTextSize());
            builder.Append(" } ");
            text_track_style_sheet_ = ParseUASheet(builder.ReleaseString());
            AddRulesToDefaultStyleSheets(text_track_style_sheet_, NamespaceType::kMediaControls);
            changed_default_style = true;
        }
    }

    if (!customizable_select_style_sheet_ && IsA<HTMLSelectElement>(element) && RuntimeEnabledFeatures::CustomizableSelectEnabled()) {
        // TODO(crbug.com/1511354): Merge customizable_select.css into html.css and
        // remove this code.
        customizable_select_style_sheet_ = ParseUASheet(UncompressResourceAsASCIIString(IDR_UASTYLE_CUSTOMIZABLE_SELECT_CSS));
        AddRulesToDefaultStyleSheets(customizable_select_style_sheet_, NamespaceType::kHTML);
        changed_default_style = true;
    }

    DCHECK(!default_html_style_->Features().GetRuleInvalidationData().HasIdsInSelectors());
    return changed_default_style;
}

bool CSSDefaultStyleSheets::EnsureDefaultStyleSheetsForPseudoElement(PseudoId pseudo_id)
{
    switch (pseudo_id) {
    case kPseudoIdMarker: {
        ...
        marker_style_sheet_ = ParseUASheet(UncompressResourceAsASCIIString(IDR_UASTYLE_MARKER_CSS));
        if (!default_pseudo_element_style_) {
            default_pseudo_element_style_ = MakeGarbageCollected<RuleSet>();
        }
        default_pseudo_element_style_->AddRulesFromSheet(MarkerStyleSheet(), ScreenEval());
        return true;
    }
    default:
        return false;
...
void CSSDefaultStyleSheets::SetMediaControlsStyleSheetLoader(std::unique_ptr<UAStyleSheetLoader> loader)
{
    media_controls_style_sheet_loader_.swap(loader);
}

void CSSDefaultStyleSheets::EnsureDefaultStyleSheetForFullscreen(const Element& element)
{
    if (fullscreen_style_sheet_) {
        DCHECK(!default_fullscreen_style_->DidMediaQueryResultsChange(MediaQueryEvaluator(element.GetDocument().GetFrame())));
        return;
    }
    ...
    default_fullscreen_style_->AddRulesFromSheet(fullscreen_style_sheet_, MediaQueryEvaluator(element.GetDocument().GetFrame()));
    VerifyUniversalRuleCount();
}

void CSSDefaultStyleSheets::RebuildFullscreenRuleSetIfMediaQueriesChanged(const Element& element)
{
    if (!fullscreen_style_sheet_) {
        return;
    }

    if (!default_fullscreen_style_->DidMediaQueryResultsChange(MediaQueryEvaluator(element.GetDocument().GetFrame()))) {
        return;
    }
...
bool CSSDefaultStyleSheets::EnsureDefaultStyleSheetForForcedColors()
{
    ...
    if (RuntimeEnabledFeatures::ForcedColorsEnabled()) {
        forced_colors_rules = forced_colors_rules + UncompressResourceAsASCIIString(IDR_UASTYLE_THEME_FORCED_COLORS_CSS);
        if (RuntimeEnabledFeatures::CustomizableSelectEnabled()) {
            forced_colors_rules = forced_colors_rules + UncompressResourceAsASCIIString(IDR_UASTYLE_CUSTOMIZABLE_SELECT_FORCED_COLORS_CSS);
        }
    }
    ...
    default_forced_color_style_->AddRulesFromSheet(DefaultStyleSheet(), ForcedColorsEval());
    default_forced_color_style_->AddRulesFromSheet(ForcedColorsStyleSheet(), ForcedColorsEval());
    if (svg_style_sheet_) {
        default_forced_color_style_->AddRulesFromSheet(SvgStyleSheet(), ForcedColorsEval());
    }

    if (media_controls_style_sheet_) {
        CHECK(!default_forced_colors_media_controls_style_);
        default_forced_colors_media_controls_style_ = MakeGarbageCollected<RuleSet>();
        default_forced_colors_media_controls_style_->AddRulesFromSheet(MediaControlsStyleSheet(), ForcedColorsEval());
    }

    return true;
}

void CSSDefaultStyleSheets::CollectFeaturesTo(const Document& document, RuleFeatureSet& features)
{
    if (DefaultHtmlStyle()) {
