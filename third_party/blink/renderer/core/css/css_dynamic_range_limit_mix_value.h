...
public:
    CSSDynamicRangeLimitMixValue(HeapVector<Member<const CSSValue>>&& limits, HeapVector<Member<const CSSPrimitiveValue>>&& percentages)
    ...
    const HeapVector<Member<const CSSValue>>& Limits() const
    {
        return limits_;
    }
...
template <> struct DowncastTraits<cssvalue::CSSDynamicRangeLimitMixValue> {
