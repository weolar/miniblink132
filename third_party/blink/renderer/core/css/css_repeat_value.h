...
public:
    explicit CSSRepeatValue(const CSSPrimitiveValue* repetitions, const CSSValueList& values)
        : CSSValue(kRepeatClass)
...
template <> struct DowncastTraits<cssvalue::CSSRepeatValue> {
    static bool AllowFrom(const CSSValue& value)
    {
        return value.IsRepeatValue();
    }
};
