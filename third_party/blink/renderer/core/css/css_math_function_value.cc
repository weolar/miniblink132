...
CSSMathFunctionValue::CSSMathFunctionValue(const CSSMathExpressionNode* expression, CSSPrimitiveValue::ValueRange range)
...
// static
CSSMathFunctionValue* CSSMathFunctionValue::Create(const CSSMathExpressionNode* expression, CSSPrimitiveValue::ValueRange range)
{
    if (!expression) {
        return nullptr;
    }
...
// static
CSSMathFunctionValue* CSSMathFunctionValue::Create(const Length& length, float zoom)
{
    DCHECK(length.IsCalculated());
    auto calc = length.GetCalculationValue().Zoom(1.0 / zoom);
    return Create(CSSMathExpressionNode::Create(*calc), CSSPrimitiveValue::ValueRangeForLengthValueRange(calc->GetValueRange()));
}
...
double CSSMathFunctionValue::ComputeDotsPerPixel(const CSSLengthResolver& length_resolver) const
{
...
scoped_refptr<const CalculationValue> CSSMathFunctionValue::ToCalcValue(const CSSLengthResolver& length_resolver) const
{
    DCHECK_NE(value_range_in_target_context_, CSSPrimitiveValue::ValueRange::kInteger);
    DCHECK_NE(value_range_in_target_context_, CSSPrimitiveValue::ValueRange::kNonNegativeInteger);
    DCHECK_NE(value_range_in_target_context_, CSSPrimitiveValue::ValueRange::kPositiveInteger);
    return expression_->ToCalcValue(
        length_resolver, CSSPrimitiveValue::ConversionToLengthValueRange(PermittedValueRange()), AllowsNegativePercentageReference());
}

const CSSValue& CSSMathFunctionValue::PopulateWithTreeScope(const TreeScope* tree_scope) const
{
    return *MakeGarbageCollected<CSSMathFunctionValue>(&expression_->PopulateWithTreeScope(tree_scope), value_range_in_target_context_);
}
...
{
    const CSSMathExpressionNode* transformed = expression_->TransformAnchors(logical_axis, transform, writing_direction);
    if (transformed != expression_) {
        return MakeGarbageCollected<CSSMathFunctionValue>(transformed, value_range_in_target_context_);
    }
    return this;
}
