...
CheckPseudoHasCacheScope::CheckPseudoHasCacheScope(Document* document, bool within_selector_checking)
    : document_(document)
    , within_selector_checking_(within_selector_checking)
{
    DCHECK(document_);
...
// static
ElementCheckPseudoHasResultMap& CheckPseudoHasCacheScope::GetResultMap(const Document* document, const CSSSelector* selector)
{
    ...
    auto entry = document->GetCheckPseudoHasCacheScope()->GetResultCache().insert(selector_text, nullptr);
    if (entry.is_new_entry) {
        entry.stored_value->value = MakeGarbageCollected<ElementCheckPseudoHasResultMap>();
    }
...
// static
ElementCheckPseudoHasFastRejectFilterMap& CheckPseudoHasCacheScope::GetFastRejectFilterMap(
    const Document* document, CheckPseudoHasArgumentTraversalType traversal_type)
{
    DCHECK(document);
    DCHECK(document->GetCheckPseudoHasCacheScope());

    auto entry = document->GetCheckPseudoHasCacheScope()->GetFastRejectFilterCache().insert(traversal_type, nullptr);
    if (entry.is_new_entry) {
        entry.stored_value->value = MakeGarbageCollected<ElementCheckPseudoHasFastRejectFilterMap>();
    }
    DCHECK(entry.stored_value->value);
    return *entry.stored_value->value;
}

CheckPseudoHasCacheScope::Context::Context(const Document* document, const CheckPseudoHasArgumentContext& argument_context)
    : argument_context_(argument_context)
{
    ...
    case CheckPseudoHasArgumentTraversalScope::kAllNextSiblings:
        cache_allowed_ = true;
        result_map_ = &CheckPseudoHasCacheScope::GetResultMap(document, argument_context.HasArgument());
        fast_reject_filter_map_ = &CheckPseudoHasCacheScope::GetFastRejectFilterMap(document, argument_context.TraversalType());
        break;
    default:
...
CheckPseudoHasResult CheckPseudoHasCacheScope::Context::SetMatchedAndGetOldResult(Element* element)
{
    return SetResultAndGetOld(element, kCheckPseudoHasResultChecked | kCheckPseudoHasResultMatched);
}
...
CheckPseudoHasResult CheckPseudoHasCacheScope::Context::SetResultAndGetOld(Element* element, CheckPseudoHasResult result)
{
    ...
    DCHECK_NE(cache_result.stored_value->value & (kCheckPseudoHasResultMatched | kCheckPseudoHasResultChecked), kCheckPseudoHasResultMatched);

    // kCheckPseudoHasResultAllDescendantsOrNextSiblingsChecked must set with
    // kCheckPseudoHasResultChecked
    DCHECK_NE(cache_result.stored_value->value & (kCheckPseudoHasResultAllDescendantsOrNextSiblingsChecked | kCheckPseudoHasResultChecked),
        kCheckPseudoHasResultAllDescendantsOrNextSiblingsChecked);

    return old_result;
}

void CheckPseudoHasCacheScope::Context::SetTraversedElementAsChecked(Element* traversed_element, Element* parent)
{
    DCHECK(traversed_element);
    DCHECK(parent);
    DCHECK_EQ(traversed_element->parentElement(), parent);
    SetResultAndGetOld(traversed_element, kCheckPseudoHasResultChecked | kCheckPseudoHasResultAllDescendantsOrNextSiblingsChecked);
    SetResultAndGetOld(parent, kCheckPseudoHasResultSomeChildrenChecked);
}

void CheckPseudoHasCacheScope::Context::SetAllTraversedElementsAsChecked(Element* last_traversed_element, int last_traversed_depth)
{
    DCHECK(last_traversed_element);
    switch (argument_context_.TraversalScope()) {
    case CheckPseudoHasArgumentTraversalScope::kAllNextSiblingSubtrees:
        if (last_traversed_depth == 1 && !ElementTraversal::PreviousSibling(*last_traversed_element)) {
    ...
    case CheckPseudoHasArgumentTraversalScope::kOneNextSiblingSubtree: {
        ...
        if (argument_context_.TraversalScope() == CheckPseudoHasArgumentTraversalScope::kAllNextSiblingSubtrees && element) {
            SetTraversedElementAsChecked(element, parent);
        }
    } break;
    case CheckPseudoHasArgumentTraversalScope::kAllNextSiblings:
        ...
        SetTraversedElementAsChecked(last_traversed_element, last_traversed_element->parentElement());
        break;
    default:
        break;
    }
...
CheckPseudoHasResult CheckPseudoHasCacheScope::Context::GetResult(Element* element) const
{
    DCHECK(cache_allowed_);
    DCHECK(result_map_);
    auto iterator = result_map_->find(element);
    return iterator == result_map_->end() ? kCheckPseudoHasResultNotCached : iterator->value;
}

bool CheckPseudoHasCacheScope::Context::HasSiblingsWithAllDescendantsOrNextSiblingsChecked(Element* element) const
{
    for (Element* sibling = ElementTraversal::PreviousSibling(*element); sibling; sibling = ElementTraversal::PreviousSibling(*sibling)) {
        ...
        if (sibling_result & kCheckPseudoHasResultAllDescendantsOrNextSiblingsChecked) {
            return true;
        }
    }
    return false;
...
bool CheckPseudoHasCacheScope::Context::HasAncestorsWithAllDescendantsOrNextSiblingsChecked(Element* element) const
{
    for (Element* parent = element->parentElement(); parent; element = parent, parent = element->parentElement()) {
        CheckPseudoHasResult parent_result = GetResult(parent);
        if (parent_result == kCheckPseudoHasResultNotCached) {
            continue;
        }
        if (parent_result & kCheckPseudoHasResultAllDescendantsOrNextSiblingsChecked) {
            return true;
        }
...
CheckPseudoHasFastRejectFilter& CheckPseudoHasCacheScope::Context::EnsureFastRejectFilter(Element* element, bool& is_new_entry)
{
    DCHECK(element);
    DCHECK(cache_allowed_);
    ...
    case CheckPseudoHasArgumentTraversalScope::kOneNextSiblingSubtree:
        for (Element* parent = element->parentElement(); parent; parent = parent->parentElement()) {
            ...
            for (int i = argument_context_.AdjacentDistanceLimit() - 1; sibling && i >= 0; sibling = ElementTraversal::PreviousSibling(*sibling), --i) {
            }
            if (!sibling) {
                continue;
            }
    ...
    case CheckPseudoHasArgumentTraversalScope::kAllNextSiblingSubtrees:
        for (Element* parent = element->parentElement(); parent; parent = parent->parentElement()) {
            for (Element* sibling = ElementTraversal::PreviousSibling(*parent); sibling; sibling = ElementTraversal::PreviousSibling(*sibling)) {
                auto iterator = fast_reject_filter_map_->find(sibling);
                if (iterator == fast_reject_filter_map_->end()) {
                    continue;
                }
    ...
    if (entry.is_new_entry) {
        entry.stored_value->value = std::make_unique<CheckPseudoHasFastRejectFilter>();
        is_new_entry = true;
    }
    DCHECK(entry.stored_value->value);
...
size_t CheckPseudoHasCacheScope::Context::GetBloomFilterAllocationCountForTesting() const
{
    if (!cache_allowed_) {
        return 0;
    }
