...
public:
    ...
    void ObserveErroneousAtRule(unsigned start_offset, CSSAtRuleID id, const Vector<CSSPropertyID, 2>& invalid_properties) override
    {
    }
    ...
    {
        return target_nesting_level_ == kEverything || target_nesting_level_ == current_nesting_level_;
    }
...
public:
    TestCSSParserImpl()
        : impl_(MakeGarbageCollected<CSSParserContext>(kHTMLStandardMode, SecureContextMode::kInsecureContext))
    {
    }

    StyleRule* ConsumeStyleRule(
        CSSParserTokenStream& stream, CSSNestingType nesting_type, StyleRule* parent_rule_for_nesting, bool nested, bool& invalid_rule_error)
    {
        return impl_.ConsumeStyleRule(stream, nesting_type, parent_rule_for_nesting,
            /* is_within_scope */ false, nested, invalid_rule_error);
    }
...
TEST(CSSParserImplTest, AtSupportsOffsets)
{
    ...
    CSSParserImpl::ParseStyleSheetForInspector(sheet_text, context, style_sheet, test_css_parser_observer);
    EXPECT_EQ(style_sheet->ChildRules().size(), 1u);
    EXPECT_EQ(test_css_parser_observer.rule_type_, StyleRule::RuleType::kSupports);
...
TEST(CSSParserImplTest, AtKeyframesOffsets)
{
    ...
    CSSParserImpl::ParseStyleSheetForInspector(sheet_text, context, style_sheet, test_css_parser_observer);
    EXPECT_EQ(style_sheet->ChildRules().size(), 1u);
    EXPECT_EQ(test_css_parser_observer.rule_type_, StyleRule::RuleType::kKeyframes);
    EXPECT_EQ(test_css_parser_observer.rule_header_start_, 11u);
...
TEST(CSSParserImplTest, AtPageMarginOffsets)
{
    ...
    CSSParserImpl::ParseStyleSheetForInspector(sheet_text, context, style_sheet, test_css_parser_observer);
    EXPECT_EQ(style_sheet->ChildRules().size(), 1u);
    EXPECT_EQ(test_css_parser_observer.rule_type_, StyleRule::RuleType::kPageMargin);
...
TEST(CSSParserImplTest, AtPropertyOffsets)
{
    ...
    CSSParserImpl::ParseStyleSheetForInspector(sheet_text, context, style_sheet, test_css_parser_observer);
    EXPECT_EQ(style_sheet->ChildRules().size(), 1u);
    EXPECT_EQ(test_css_parser_observer.rule_type_, StyleRule::RuleType::kProperty);
    EXPECT_EQ(test_css_parser_observer.rule_header_start_, 10u);
...
TEST(CSSParserImplTest, AtCounterStyleOffsets)
{
    ...
    CSSParserImpl::ParseStyleSheetForInspector(sheet_text, context, style_sheet, test_css_parser_observer);
    EXPECT_EQ(style_sheet->ChildRules().size(), 1u);
    EXPECT_EQ(test_css_parser_observer.rule_type_, StyleRule::RuleType::kCounterStyle);
...
TEST(CSSParserImplTest, AtContainerOffsets)
{
    ...
    CSSParserImpl::ParseStyleSheetForInspector(sheet_text, context, style_sheet, test_css_parser_observer);
    EXPECT_EQ(style_sheet->ChildRules().size(), 1u);
    EXPECT_EQ(test_css_parser_observer.rule_type_, StyleRule::RuleType::kContainer);
    EXPECT_EQ(test_css_parser_observer.rule_header_start_, 11u);
...
{
    test::TaskEnvironment task_environment;
    String sheet_text = ".element { color: green; &.other { color: red; margin-left: 10px; }}";
...
{
    test::TaskEnvironment task_environment;
    String sheet_text = ".element { color: green; .outer, .outer2 { color: red; }}";
...
TEST(CSSParserImplTest, NestedRulesInsideMediaQueries)
{
    ...
    const StyleRuleNestedDeclarations* child0 = DynamicTo<StyleRuleNestedDeclarations>(media_query->ChildRules()[0].Get());
...
TEST(CSSParserImplTest, NestedRulesInsideMediaQueries_CSSNestedDeclarationsDisabled)
{
...
TEST(CSSParserImplTest, ObserveNestedLayer)
{
    ...
    EXPECT_EQ(test_css_parser_observer.rule_type_, StyleRule::RuleType::kLayerBlock);
    EXPECT_EQ(test_css_parser_observer.rule_header_start_, 49u);
...
TEST(CSSParserImplTest, ConsumeUnparsedDeclarationRemovesImportantAnnotationIfPresent)
{
    test::TaskEnvironment task_environment;
    struct TestCase {
        String input;
    ...
    for (auto current_case : test_cases) {
        ...
        bool is_important;
        CSSVariableData* data = CSSVariableParser::ConsumeUnparsedDeclaration(stream, /*allow_important_annotation=*/true,
            /*is_animation_tainted=*/false,
            /*must_contain_variable_reference=*/false,
            /*restricted_value=*/true, /*comma_ends_declaration=*/false, is_important, *context);
...
TEST(CSSParserImplTest, InvalidLayerRules)
{
    ...
    Document* document = Document::CreateForTest(execution_context.GetExecutionContext());
...
TEST(CSSParserImplTest, ValidLayerStatementRule)
{
    ...
    {
        String rule = "@layer foo;";
        auto* parsed = DynamicTo<StyleRuleLayerStatement>(ParseRule(*document, rule));
        ASSERT_TRUE(parsed);
...
TEST(CSSParserImplTest, NestedLayerRules)
{
    ...
    // Statement rule as a child rule.
    {
        ...
        auto* barbaz = DynamicTo<StyleRuleLayerStatement>(foo->ChildRules()[0].Get());
        ASSERT_TRUE(barbaz);
    ...
    // Nested in an unnamed layer.
    {
        ...
        auto* foo = DynamicTo<StyleRuleLayerStatement>(parent->ChildRules()[0].Get());
        ASSERT_TRUE(foo);
...
TEST(CSSParserImplTest, ImportRulesWithSupports)
{
    ...
    {
        String rule = "@import url(foo.css) layer(bar.baz) supports(display: block);";
        auto* parsed = DynamicTo<StyleRuleImport>(ParseRule(*document, rule));
        ASSERT_TRUE(parsed);
    ...
    {
        String rule = "@import url(foo.css)   supports((display: block) and (color: green));";
        auto* parsed = DynamicTo<StyleRuleImport>(ParseRule(*document, rule));
        ASSERT_TRUE(parsed);
        EXPECT_TRUE(parsed->IsSupported());
    }
    ...
    {
        String rule = "@import url(foo.css) supports((foo: bar) and (color: green));";
        auto* parsed = DynamicTo<StyleRuleImport>(ParseRule(*document, rule));
        ASSERT_TRUE(parsed);
...
TEST(CSSParserImplTest, FontPaletteValuesBasicRuleParsing)
{
    ...
    auto* parsed = DynamicTo<StyleRuleFontPaletteValues>(ParseRule(*document, rule));
    ASSERT_TRUE(parsed);
    ASSERT_EQ("--myTestPalette", parsed->GetName());
    ASSERT_EQ("testFamily", parsed->GetFontFamily()->CssText());
    ASSERT_EQ(0, DynamicTo<CSSPrimitiveValue>(parsed->GetBasePalette())->ComputeInteger(CSSToLengthConversionData()));
...
TEST(CSSParserImplTest, FontFeatureValuesRuleParsing)
{
    ...
    auto* parsed = DynamicTo<StyleRuleFontFeatureValues>(ParseRule(*document, rule));
    ASSERT_TRUE(parsed);
    ...
    ASSERT_EQ(parsed->GetStyleset()->at(AtomicString("curly")).indices, Vector<uint32_t>({ 4, 3, 2, 1 }));
}
...
TEST(CSSParserImplTest, FontFeatureValuesOffsets)
{
    ...
    CSSParserImpl::ParseStyleSheetForInspector(sheet_text, context, style_sheet, test_css_parser_observer);
    EXPECT_EQ(style_sheet->ChildRules().size(), 1u);
    EXPECT_EQ(test_css_parser_observer.rule_type_, StyleRule::RuleType::kFontFeatureValues);
...
TEST(CSSParserImplTest, CSSFunction)
{
    ...
    const StyleRuleFunction* rule = DynamicTo<StyleRuleFunction>(sheet->ChildRules()[0].Get());
    EXPECT_TRUE(rule);
...
static String RoundTripProperty(Document& document, String property_text)
{
    ...
    StyleRule* style_rule = To<StyleRule>(css_test_helpers::ParseRule(document, rule_text));
    if (!style_rule) {
        return "";
    }
...
TEST(CSSParserImplTest, AllPropertiesCanParseImportant)
{
    ...
    ScopedNullExecutionContext execution_context;
    Document* document = Document::CreateForTest(execution_context.GetExecutionContext());
    const ComputedStyle& initial_style = *ComputedStyle::GetInitialStyleSingleton();
    ...
    for (CSSPropertyID property_id : CSSPropertyIDList()) {
        ...
        const CSSValue* computed_value = property.CSSValueFromComputedStyle(initial_style,
        ...
        // is fixed.
        String property_text = property.GetPropertyNameString() + ": " + computed_value->CssText() + ";";
        ...
        property_text = property.GetPropertyNameString() + ": " + computed_value->CssText() + " !important;";
...
TEST(CSSParserImplTest, ParseSupportsBlinkFeature)
{
    ...
    auto* context = MakeGarbageCollected<CSSParserContext>(kUASheetMode, SecureContextMode::kInsecureContext);
    auto* sheet = MakeGarbageCollected<StyleSheetContents>(context);
    CSSParserImpl::ParseStyleSheet(sheet_text, context, sheet);
    ASSERT_EQ(sheet->ChildRules().size(), 1u);
    ...
    ASSERT_EQ(String("div"), To<StyleRule>(child_rules[0].Get())->SelectorsText());
    ASSERT_EQ(String("span"), To<StyleRule>(child_rules[1].Get())->SelectorsText());
}
...
TEST(CSSParserImplTest, CustomPropertyAmbiguityTopLevel)
{
    ...
    const StyleRule* rule = parser.ConsumeStyleRule(stream, CSSNestingType::kNone, /* parent_rule_for_nesting */ nullptr,
...
TEST(CSSParserImplTest, CustomPropertyAmbiguityNested)
{
    ...
    TestCSSParserImpl parser;
    const StyleRule* rule = parser.ConsumeStyleRule(stream, CSSNestingType::kNesting, /* parent_rule_for_nesting */ nullptr,
