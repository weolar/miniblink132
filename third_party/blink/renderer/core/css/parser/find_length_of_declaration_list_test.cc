...
class FindLengthOfDeclarationListTest : public testing::TestWithParam<FindLengthInstructionSet> {
protected:
    void SetUp() override
    {
#ifdef __SSE2__
        if (GetParam() == FindLengthInstructionSet::AVX2 && !base::CPU::GetInstanceNoAllocation().has_avx2()) {
...
#ifdef __SSE2__
INSTANTIATE_TEST_SUITE_P(WithAndWithoutAVX2, FindLengthOfDeclarationListTest, testing::Values(FindLengthInstructionSet::BASE, FindLengthInstructionSet::AVX2));
#else
INSTANTIATE_TEST_SUITE_P(WithBaseOnly, FindLengthOfDeclarationListTest, testing::Values(FindLengthInstructionSet::BASE));
#endif
...
TEST_P(FindLengthOfDeclarationListTest, NonASCII)
{
    ...
    EXPECT_EQ(10u, FindLengthOfDeclarationList(String::FromUTF8("--foo: bar}                   ❤️")));
