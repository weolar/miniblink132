...
const char* valid_variable_reference_value[] = {
    ...
    // {} as the whole value:
    "{ var(--x) }", "{ A var(--x) }", "{ var(--x) A }", "{ var(--x) A", "{ var(--x)", "{ var(--x) []",

    // {} inside another block:
    "var(--x) [{}]", "[{}] var(--x)", "foo({}) var(--x)", "var(--x) foo({})",
    // clang-format on
};
...
TEST_P(ValidVariableReferenceTest, ConsumeUnparsedDeclaration)
{
    ...
    bool important;
    EXPECT_TRUE(CSSVariableParser::ConsumeUnparsedDeclaration(stream, /*allow_important_annotation=*/false,
        /*is_animation_tainted=*/false, /*must_contain_variable_reference=*/true,
        /*restricted_value=*/true, /*comma_ends_declaration=*/false, important, *context));
}
...
TEST_P(ValidVariableReferenceTest, ParseUniversalSyntaxValue)
{
    SCOPED_TRACE(GetParam());
    auto* context = MakeGarbageCollected<CSSParserContext>(kHTMLStandardMode, SecureContextMode::kInsecureContext);
    EXPECT_NE(nullptr, CSSVariableParser::ParseUniversalSyntaxValue(GetParam(), *context, /* is_animation_tainted */ false));
}
...
TEST_P(InvalidVariableReferenceTest, ConsumeUnparsedDeclaration)
{
    ...
    auto* context = MakeGarbageCollected<CSSParserContext>(kHTMLStandardMode, SecureContextMode::kInsecureContext);
    bool important;
    EXPECT_FALSE(CSSVariableParser::ConsumeUnparsedDeclaration(stream, /*allow_important_annotation=*/false,
        /*is_animation_tainted=*/false, /*must_contain_variable_reference=*/true,
        /*restricted_value=*/true, /*comma_ends_declaration=*/false, important, *context));
}
...
TEST_P(CustomPropertyDeclarationTest, ParseDeclarationValue)
{
    SCOPED_TRACE(GetParam());
    auto* context = MakeGarbageCollected<CSSParserContext>(kHTMLStandardMode, SecureContextMode::kInsecureContext);
    EXPECT_NE(nullptr, CSSVariableParser::ParseDeclarationValue(GetParam(), /* is_animation_tainted */ false, *context));
}

class ValidAttrTest : public testing::Test, public testing::WithParamInterface<const char*> { };

INSTANTIATE_TEST_SUITE_P(All, ValidAttrTest, testing::ValuesIn(valid_attr_values));
...
class InvalidAttrTest : public testing::Test, public testing::WithParamInterface<const char*> { };

INSTANTIATE_TEST_SUITE_P(All, InvalidAttrTest, testing::ValuesIn(invalid_attr_values));
...
class ValidAppearanceAutoBaseSelectTest : public testing::Test, public testing::WithParamInterface<const char*> { };

INSTANTIATE_TEST_SUITE_P(All, ValidAppearanceAutoBaseSelectTest, testing::ValuesIn(valid_appearance_auto_base_select_values));
...
class InvalidAppearanceAutoBaseSelectTest : public testing::Test, public testing::WithParamInterface<const char*> { };

INSTANTIATE_TEST_SUITE_P(All, InvalidAppearanceAutoBaseSelectTest, testing::ValuesIn(invalid_appearance_auto_base_select_values));
