...
CSSPropertyParser::CSSPropertyParser(CSSParserTokenStream& stream, const CSSParserContext* context, HeapVector<CSSPropertyValue, 64>* parsed_properties)
    : stream_(stream)
    , context_(context)
...
bool CSSPropertyParser::ParseValue(CSSPropertyID unresolved_property, bool allow_important_annotation, CSSParserTokenStream& stream,
    const CSSParserContext* context, HeapVector<CSSPropertyValue, 64>& parsed_properties, StyleRule::RuleType rule_type)
{
...
const CSSValue* CSSPropertyParser::ParseSingleValue(CSSPropertyID property, CSSParserTokenStream& stream, const CSSParserContext* context)
{
    DCHECK(context);
...
StringView StripInitialWhitespace(StringView value)
{
    ...
    while (initial_whitespace_len < value.length() && IsHTMLSpace(value[initial_whitespace_len])) {
        ++initial_whitespace_len;
    }
    return StringView(value, initial_whitespace_len);
}

bool CSSPropertyParser::ParseValueStart(CSSPropertyID unresolved_property, bool allow_important_annotation, StyleRule::RuleType rule_type)
{
    ...
    if (is_shorthand) {
        const auto local_context = CSSParserLocalContext().WithAliasParsing(IsPropertyAlias(unresolved_property)).WithCurrentShorthand(property_id);
        ...
        if (To<Shorthand>(property).ParseShorthand(
            ...
            if (stream_.AtEnd()) {
                if (important) {
                    for (wtf_size_t property_idx = parsed_properties_size; property_idx < parsed_properties_->size(); ++property_idx) {
        ...
        if (const CSSValue* parsed_value = ParseLonghand(unresolved_property, CSSPropertyID::kInvalid, *context_, stream_)) {
            bool important = css_parsing_utils::MaybeConsumeImportant(stream_, allow_important_annotation);
            if (stream_.AtEnd()) {
                AddProperty(property_id, CSSPropertyID::kInvalid, *parsed_value, important, IsImplicitProperty::kNotImplicit, *parsed_properties_);
                return true;
            }
        }
    }
    ...
    CSSVariableData* variable_data = CSSVariableParser::ConsumeUnparsedDeclaration(stream_,
    ...
    auto* variable = MakeGarbageCollected<CSSUnparsedDeclarationValue>(variable_data, context_);
    if (is_shorthand) {
        const cssvalue::CSSPendingSubstitutionValue& pending_value = *MakeGarbageCollected<cssvalue::CSSPendingSubstitutionValue>(property_id, variable);
        css_parsing_utils::AddExpandedPropertyForValue(property_id, pending_value, important, *parsed_properties_);
    } else {
        AddProperty(property_id, CSSPropertyID::kInvalid, *variable, important, IsImplicitProperty::kNotImplicit, *parsed_properties_);
    }
    return true;
}

static inline bool IsExposedInMode(const ExecutionContext* execution_context, const CSSUnresolvedProperty& property, CSSParserMode mode)
{
...
static inline bool QuasiLowercaseIntoBuffer(const UChar* src, unsigned length, char* dst)
{
...
static inline bool QuasiLowercaseIntoBuffer(const LChar* src, unsigned length, char* dst)
{
    unsigned i;
...
static CSSPropertyID ExposedProperty(CSSPropertyID property_id, const ExecutionContext* execution_context, CSSParserMode mode)
{
    ...
    if (alternative_id != CSSPropertyID::kInvalid) {
        if (CSSPropertyID exposed_id = ExposedProperty(alternative_id, execution_context, mode); exposed_id != CSSPropertyID::kInvalid) {
            return exposed_id;
        }
    }
...
static CSSPropertyID UnresolvedCSSPropertyID(const ExecutionContext* execution_context, const CharacterType* property_name, unsigned length, CSSParserMode mode)
{
    if (length == 0) {
    ...
    if (kKnownExposedProperties.Has(property_id)) {
        DCHECK_EQ(property_id, ExposedProperty(property_id, execution_context, mode));
        return property_id;
    }
...
CSSPropertyID UnresolvedCSSPropertyID(const ExecutionContext* execution_context, StringView string, CSSParserMode mode)
{
    return WTF::VisitCharacters(string, [&](auto chars) { return UnresolvedCSSPropertyID(execution_context, chars.data(), chars.size(), mode); });
}

template <typename CharacterType> static CSSValueID CssValueKeywordID(const CharacterType* value_keyword, unsigned length)
{
...
const CSSValue* CSSPropertyParser::ConsumeCSSWideKeyword(CSSParserTokenStream& stream, bool allow_important_annotation, bool& important)
{
...
{
    bool important;
    const CSSValue* value = ConsumeCSSWideKeyword(stream_, allow_important_annotation, important);
    if (!value) {
        return false;
    }
    ...
    if (!shorthand.length()) {
        ...
        AddProperty(property, CSSPropertyID::kInvalid, *value, important, IsImplicitProperty::kNotImplicit, *parsed_properties_);
    } else {
...
bool CSSPropertyParser::ParseFontFaceDescriptor(CSSPropertyID resolved_property)
{
    ...
    CSSValue* parsed_value = AtRuleDescriptorParser::ParseFontFaceDescriptor(id, stream_, *context_);
    if (!parsed_value) {
        return false;
    }

    AddProperty(resolved_property, CSSPropertyID::kInvalid /* current_shorthand */, *parsed_value, false /* important */, IsImplicitProperty::kNotImplicit,
        *parsed_properties_);
    return true;
}
