...
public:
    String ParseQuery(String string)
    {
        ...
        const MediaQueryExpNode* node = ContainerQueryParser(*context).ParseCondition(string);
        if (!node) {
            return g_null_atom;
        }
        if (node->HasUnknown()) {
    ...
    String ParseFeatureQuery(String feature_query)
    {
        ...
        const MediaQueryExpNode* node = ContainerQueryParser(*context).ConsumeFeatureQuery(stream, TestFeatureSet());
