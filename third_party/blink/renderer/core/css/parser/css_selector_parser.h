...
public:
    ...
    static base::span<CSSSelector> ParseSelector(CSSParserTokenStream&, const CSSParserContext*, CSSNestingType, const StyleRule* parent_rule_for_nesting,
        bool is_within_scope, bool semicolon_aborts_nested_selector, StyleSheetContents*, HeapVector<CSSSelector>&);
    static base::span<CSSSelector> ConsumeSelector(CSSParserTokenStream&, const CSSParserContext*, CSSNestingType, const StyleRule* parent_rule_for_nesting,
        bool is_within_scope, bool semicolon_aborts_nested_selector, StyleSheetContents*, CSSParserObserver*, HeapVector<CSSSelector>&,
    ...
    static bool SupportsComplexSelector(CSSParserTokenStream&, const CSSParserContext*);

    static CSSSelector::PseudoType ParsePseudoType(const AtomicString&, bool has_arguments, const Document*);

    static PseudoId ParsePseudoElement(const String&, const Node*, AtomicString& argument);
    ...
    static base::span<CSSSelector> ParseScopeBoundary(CSSParserTokenStream&, const CSSParserContext*, CSSNestingType, const StyleRule* parent_rule_for_nesting,
        bool is_within_scope, StyleSheetContents*, HeapVector<CSSSelector>&);

private:
    CSSSelectorParser(const CSSParserContext*, const StyleRule* parent_rule_for_nesting, bool is_within_scope, bool semicolon_aborts_nested_selector,
        StyleSheetContents*, HeapVector<CSSSelector>&);
    ...
    // :scope instead of &.
    base::span<CSSSelector> ConsumeComplexSelectorList(CSSParserTokenStream& stream, CSSNestingType);
    base::span<CSSSelector> ConsumeComplexSelectorList(CSSParserTokenStream& stream, CSSParserObserver* observer, CSSNestingType);
    ...
    std::optional<base::span<CSSSelector>> ConsumeForgivingComplexSelectorList(CSSParserTokenStream&, CSSNestingType);
    ...
    base::span<CSSSelector> ConsumeNestedRelativeSelector(CSSParserTokenStream& stream, CSSNestingType);
    base::span<CSSSelector> ConsumeRelativeSelector(CSSParserTokenStream&);
    base::span<CSSSelector> ConsumeComplexSelector(CSSParserTokenStream& stream, CSSNestingType, bool first_in_complex_selector_list);
    ...
    bool ConsumePartialComplexSelector(
        CSSParserTokenStream&, CSSSelector::RelationType& /* current combinator */, unsigned /* previous compound flags */, CSSNestingType);

    bool ConsumeName(CSSParserTokenStream&, AtomicString& name, AtomicString& namespace_prefix);
    ...
    base::span<CSSSelector> ConsumeCompoundSelector(CSSParserTokenStream&, CSSNestingType);
    ...
    void PrependTypeSelectorIfNeeded(
        const AtomicString& namespace_prefix, bool has_element_name, const AtomicString& element_name, wtf_size_t start_index_of_compound_selector);
    void SplitCompoundAtImplicitShadowCrossingCombinator(base::span<CSSSelector> compound_selector);
    void RecordUsageAndDeprecations(base::span<CSSSelector>, bool* has_visited_style = nullptr);
    static bool ContainsUnknownWebkitPseudoElements(base::span<CSSSelector> selectors);

    void SetInSupportsParsing()
    {
        in_supports_parsing_ = true;
    }
    ...
    CSSSelector::PseudoType restricting_pseudo_element_ = CSSSelector::kPseudoUnknown;
    ...
    public:
        explicit DisallowPseudoElementsScope(CSSSelectorParser* parser)
            : parser_(parser)
            , was_disallowed_(parser_->disallow_pseudo_elements_)
        {
...
static inline bool AbortsNestedSelectorParsing(CSSParserTokenType token_type, bool semicolon_aborts_nested_selector, CSSNestingType nesting_type)
{
    return semicolon_aborts_nested_selector && token_type == kSemicolonToken && nesting_type != CSSNestingType::kNone;
}
