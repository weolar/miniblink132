...
CSSSupportsParser::Result CSSSupportsParser::ConsumeSupportsCondition(CSSParserTokenStream& stream, CSSParserImpl& parser)
{
...
bool CSSSupportsParser::ConsumeSupportsSelectorFn(CSSParserTokenStream& stream)
{
    ...
    if (CSSSelectorParser::SupportsComplexSelector(stream, parser_.GetContext()) && guard.Release()) {
        stream.ConsumeWhitespace();
        return true;
    }
    return false;
...
bool CSSSupportsParser::ConsumeFontFormatFn(CSSParserTokenStream& stream)
{
    ...
    CSSIdentifierValue* consumed_value = css_parsing_utils::ConsumeFontFormatIdent(stream);

    if (consumed_value && css_parsing_utils::IsSupportedKeywordFormat(consumed_value->GetValueID()) && guard.Release()) {
        stream.ConsumeWhitespace();
        return true;
    }
...
bool CSSSupportsParser::ConsumeFontTechFn(CSSParserTokenStream& stream)
{
    ...
    CSSIdentifierValue* consumed_value = css_parsing_utils::ConsumeFontTechIdent(stream);

    if (consumed_value && css_parsing_utils::IsSupportedKeywordTech(consumed_value->GetValueID()) && guard.Release()) {
        stream.ConsumeWhitespace();
        return true;
    }
...
bool CSSSupportsParser::ConsumeAtRuleFn(CSSParserTokenStream& stream)
{
    ...
    bool ok = AtRuleDescriptorParser::ParseDescriptorValue(rule_type, descriptor_id, stream, *parser_.GetContext(), parsed_descriptors);
...
bool CSSSupportsParser::ConsumeSupportsDecl(CSSParserTokenStream& stream)
{
    ...
    if (stream.Peek().GetType() == kIdentToken && parser_.ConsumeSupportsDeclaration(stream) && guard.Release()) {
        stream.ConsumeWhitespace();
        return true;
    }
    return false;
...
bool CSSSupportsParser::ConsumeBlinkFeatureFn(CSSParserTokenStream& stream)
{
    ...
    if (stream.Peek().GetType() == kIdentToken) {
        ...
        if (RuntimeEnabledFeatures::IsFeatureEnabledFromString(feature_name.Value().Utf8()) && guard.Release()) {
            stream.ConsumeWhitespace();
            return true;
        }
    }
