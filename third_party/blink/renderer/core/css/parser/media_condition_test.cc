...
TEST(MediaConditionParserTest, Basic)
{
    ...
    for (const MediaConditionTestCase& test_case : test_cases) {
        ...
        MediaQuerySet* media_condition_query_set = MediaQueryParser::ParseMediaCondition(stream, nullptr);
        String query_text = stream.AtEnd() ? media_condition_query_set->MediaText() : "not all";
