...
public:
    explicit CSSParserImpl(const CSSParserContext*, StyleSheetContents* = nullptr);
    ...
    struct RangeOffset {
        ...
        RangeOffset(wtf_size_t start, wtf_size_t end)
            : start(start)
            , end(end)
        {
            DCHECK(start <= end);
        ...
        // an observer).
        static RangeOffset Ignore()
        {
            return { 0, 0 };
        }
    };

    static MutableCSSPropertyValueSet::SetResult ParseValue(MutableCSSPropertyValueSet*, CSSPropertyID, StringView, bool important, const CSSParserContext*);
    static MutableCSSPropertyValueSet::SetResult ParseVariableValue(
        MutableCSSPropertyValueSet*, const AtomicString& property_name, StringView, bool important, const CSSParserContext*, bool is_animation_tainted);
    static ImmutableCSSPropertyValueSet* ParseInlineStyleDeclaration(const String&, Element*);
    static ImmutableCSSPropertyValueSet* ParseInlineStyleDeclaration(const String&, CSSParserMode, SecureContextMode, const Document*);
    ...
    static bool ParseDeclarationList(MutableCSSPropertyValueSet*, const String&, const CSSParserContext*);
    ...
    static StyleRuleBase* ParseNestedDeclarationsRule(
        const CSSParserContext*, CSSNestingType, StyleRule* parent_rule_for_nesting, bool is_within_scope, StringView);
    static StyleRuleBase* ParseRule(const String&, const CSSParserContext*, CSSNestingType, StyleRule* parent_rule_for_nesting, bool is_within_scope,
        StyleSheetContents*, AllowedRulesType);
    static ParseSheetResult ParseStyleSheet(
        const String&, const CSSParserContext*, StyleSheetContents*, CSSDeferPropertyParsing = CSSDeferPropertyParsing::kNo, bool allow_import_rules = true);
    static CSSSelectorList* ParsePageSelector(CSSParserTokenStream&, StyleSheetContents*, const CSSParserContext& context);

    static std::unique_ptr<Vector<KeyframeOffset>> ParseKeyframeKeyList(const CSSParserContext*, const String&);
    static String ParseCustomPropertyName(StringView name_text);

    bool ConsumeEndOfPreludeForAtRuleWithoutBlock(CSSParserTokenStream& stream, CSSAtRuleID id);
    bool ConsumeEndOfPreludeForAtRuleWithBlock(CSSParserTokenStream& stream, CSSAtRuleID id);
    ...
    const CSSParserContext* GetContext() const
    {
        return context_;
    }
    ...
    static CSSPropertyValueSet* ParseDeclarationListForLazyStyle(const String&, wtf_size_t offset, const CSSParserContext*);
...
private:
    ...
    template <typename T>
    bool ConsumeRuleList(CSSParserTokenStream&, RuleListType, CSSNestingType, StyleRule* parent_rule_for_nesting, bool is_within_scope, T callback);

    // These functions update the stream they're given
    StyleRuleBase* ConsumeAtRule(CSSParserTokenStream&, AllowedRulesType, CSSNestingType, StyleRule* parent_rule_for_nesting, bool is_within_scope);
    StyleRuleBase* ConsumeAtRuleContents(
        CSSAtRuleID id, CSSParserTokenStream& stream, AllowedRulesType allowed_rules, CSSNestingType, StyleRule* parent_rule_for_nesting, bool is_within_scope);
    StyleRuleBase* ConsumeQualifiedRule(CSSParserTokenStream&, AllowedRulesType, CSSNestingType, StyleRule* parent_rule_for_nesting, bool is_within_scope);

    StyleRulePageMargin* ConsumePageMarginRule(CSSAtRuleID rule_id, CSSParserTokenStream& stream);
    StyleRuleCharset* ConsumeCharsetRule(CSSParserTokenStream&);
    StyleRuleImport* ConsumeImportRule(const AtomicString& prelude_uri, CSSParserTokenStream&);
    StyleRuleNamespace* ConsumeNamespaceRule(CSSParserTokenStream&);
    StyleRuleMedia* ConsumeMediaRule(CSSParserTokenStream& stream, CSSNestingType, StyleRule* parent_rule_for_nesting, bool is_within_scope);
    StyleRuleSupports* ConsumeSupportsRule(CSSParserTokenStream& stream, CSSNestingType, StyleRule* parent_rule_for_nesting, bool is_within_scope);
    StyleRuleStartingStyle* ConsumeStartingStyleRule(CSSParserTokenStream& stream, CSSNestingType, StyleRule* parent_rule_for_nesting, bool is_within_scope);
    StyleRuleFontFace* ConsumeFontFaceRule(CSSParserTokenStream&);
    StyleRuleFontPaletteValues* ConsumeFontPaletteValuesRule(CSSParserTokenStream&);
    StyleRuleFontFeatureValues* ConsumeFontFeatureValuesRule(CSSParserTokenStream&);
    StyleRuleFontFeature* ConsumeFontFeatureRule(CSSAtRuleID, CSSParserTokenStream&);
    StyleRuleKeyframes* ConsumeKeyframesRule(bool webkit_prefixed, CSSParserTokenStream&);
    ...
    StyleRuleBase* ConsumeScopeRule(CSSParserTokenStream&, CSSNestingType, StyleRule* parent_rule_for_nesting, bool is_within_scope);
    StyleRuleViewTransition* ConsumeViewTransitionRule(CSSParserTokenStream& stream);
    StyleRuleContainer* ConsumeContainerRule(CSSParserTokenStream& stream, CSSNestingType, StyleRule* parent_rule_for_nesting, bool is_within_scope);
    StyleRuleBase* ConsumeLayerRule(CSSParserTokenStream&, CSSNestingType, StyleRule* parent_rule_for_nesting, bool is_within_scope);
    ...
    std::optional<Vector<StyleRuleFunction::Parameter>> ConsumeFunctionParameters(CSSParserTokenStream& stream);
    ...
    StyleRule* ConsumeStyleRule(
        CSSParserTokenStream&, CSSNestingType, StyleRule* parent_rule_for_nesting, bool is_within_scope, bool nested, bool& invalid_rule_error);
    StyleRule* ConsumeStyleRuleContents(base::span<CSSSelector> selector_vector, CSSParserTokenStream& stream, bool is_within_scope, bool has_visited_pseudo);

    void ConsumeBlockContents(CSSParserTokenStream&, StyleRule::RuleType, CSSNestingType, StyleRule* parent_rule_for_nesting, bool is_within_scope,
        wtf_size_t nested_declarations_start_index, HeapVector<Member<StyleRuleBase>, 4>* child_rules, bool has_visited_pseudo = false);

    void ConsumeRuleListOrNestedDeclarationList(CSSParserTokenStream&, bool is_nested_group_rule, CSSNestingType, StyleRule* parent_rule_for_nesting,
        bool is_within_scope, HeapVector<Member<StyleRuleBase>, 4>* child_rules);

    // If id is std::nullopt, we're parsing a qualified style rule;
    // otherwise, we're parsing an at-rule.
    StyleRuleBase* ConsumeNestedRule(std::optional<CSSAtRuleID> id, StyleRule::RuleType parent_rule_type, CSSParserTokenStream& stream, CSSNestingType,
        StyleRule* parent_rule_for_nesting, bool is_within_scope, bool& invalid_rule_error);

    // Returns true if a declaration was parsed and added to parsed_properties_,
    // and false otherwise.
    bool ConsumeDeclaration(CSSParserTokenStream&, StyleRule::RuleType, bool has_visited_pseudo = false);
    void ConsumeDeclarationValue(CSSParserTokenStream&, CSSPropertyID, bool is_in_declaration_list, StyleRule::RuleType);
    bool ConsumeVariableValue(CSSParserTokenStream& stream, const AtomicString& property_name, bool allow_important_annotation, bool is_animation_tainted);

    static std::unique_ptr<Vector<KeyframeOffset>> ConsumeKeyframeKeyList(const CSSParserContext*, CSSParserTokenStream&);
    ...
    // is created instead.
    StyleRule* CreateImplicitNestedRule(CSSNestingType, StyleRule* parent_rule_for_nesting);
    ...
    void EmitNestedDeclarationsRuleIfNeeded(
        CSSNestingType, StyleRule* parent_rule_for_nesting, wtf_size_t start_index, HeapVector<Member<StyleRuleBase>, 4>& child_rules);
