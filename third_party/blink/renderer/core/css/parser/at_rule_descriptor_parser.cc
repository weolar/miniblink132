...
CSSValue* ConsumeFontVariantList(CSSParserTokenStream& stream)
{
    ...
    do {
        ...
        CSSIdentifierValue* font_variant = css_parsing_utils::ConsumeFontVariantCSS21(stream);
        if (font_variant) {
...
{
    return css_parsing_utils::ConsumeIdent<CSSValueID::kAuto, CSSValueID::kBlock, CSSValueID::kSwap, CSSValueID::kFallback, CSSValueID::kOptional>(stream);
}
...
CSSValueList* ConsumeFontFaceUnicodeRange(CSSParserTokenStream& stream)
{
    ...
    do {
        ...
        values->Append(*MakeGarbageCollected<cssvalue::CSSUnicodeRangeValue>(start, end));
    } while (css_parsing_utils::ConsumeCommaIncludingWhitespace(stream));
...
{
    return css_parsing_utils::IsSupportedKeywordFormat(css_parsing_utils::FontFormatToId(font_format)) || EqualIgnoringASCIICase(font_format, "woff-variations")
        || EqualIgnoringASCIICase(font_format, "truetype-variations") || EqualIgnoringASCIICase(font_format, "opentype-variations")
...
CSSValue* ConsumeFontFaceSrcURI(CSSParserTokenStream& stream, const CSSParserContext& context)
{
    cssvalue::CSSURIValue* src_value = css_parsing_utils::ConsumeUrl(stream, context);
    if (!src_value) {
        return nullptr;
    }
    auto* uri_value = CSSFontFaceSrcValue::Create(src_value, context.JavascriptWorld());
    ...
    if (stream.Peek().FunctionId() == CSSValueID::kFormat) {
        {
            ...
            if (peek_type == kIdentToken) {
                CSSIdentifierValue* font_format = css_parsing_utils::ConsumeFontFormatIdent(stream);
                if (!font_format) {
                    return nullptr;
                }
    ...
    if (stream.Peek().FunctionId() == CSSValueID::kTech) {
        {
            ...
            do {
                CSSIdentifierValue* technology_value = css_parsing_utils::ConsumeFontTechIdent(stream);
                if (!technology_value) {
                    return nullptr;
                }
                if (!stream.AtEnd() && stream.Peek().GetType() != CSSParserTokenType::kCommaToken) {
                ...
                if (css_parsing_utils::IsSupportedKeywordTech(technology_value->GetValueID())) {
                    uri_value->AppendTechnology(ValueIDToTechnology(technology_value->GetValueID()));
                } else {
                    return nullptr;
                }
            } while (css_parsing_utils::ConsumeCommaIncludingWhitespace(stream));
...
CSSValue* ConsumeFontFaceSrcLocal(CSSParserTokenStream& stream, const CSSParserContext& context)
{
    CSSParserTokenStream::BlockGuard guard(stream);
    stream.ConsumeWhitespace();
...
CSSValue* ConsumeFontFaceSrcSkipToComma(
    CSSValue* parse_function(CSSParserTokenStream&, const CSSParserContext&), CSSParserTokenStream& stream, const CSSParserContext& context)
{
...
CSSValueList* ConsumeFontFaceSrc(CSSParserTokenStream& stream, const CSSParserContext& context)
{
    CSSValueList* values = CSSValueList::CreateCommaSeparated();
    ...
    do {
        ...
        if (token.FunctionId() == CSSValueID::kLocal) {
            parsed_value = ConsumeFontFaceSrcSkipToComma(ConsumeFontFaceSrcLocal, stream, context);
        } else {
            parsed_value = ConsumeFontFaceSrcSkipToComma(ConsumeFontFaceSrcURI, stream, context);
        }
        if (parsed_value) {
...
CSSValue* ConsumeDescriptor(StyleRule::RuleType rule_type, AtRuleDescriptorID id, CSSParserTokenStream& stream, const CSSParserContext& context)
{
...
CSSValue* ConsumeFontMetricOverride(CSSParserTokenStream& stream, const CSSParserContext& context)
{
    if (CSSIdentifierValue* normal = css_parsing_utils::ConsumeIdent<CSSValueID::kNormal>(stream)) {
        return normal;
    }
...
CSSValue* AtRuleDescriptorParser::ParseFontFaceDescriptor(AtRuleDescriptorID id, CSSParserTokenStream& stream, const CSSParserContext& context)
{
    CSSValue* parsed_value = nullptr;
    stream.ConsumeWhitespace();
    switch (id) {
...
CSSValue* AtRuleDescriptorParser::ParseFontFaceDescriptor(AtRuleDescriptorID id, StringView string, const CSSParserContext& context)
{
    CSSParserTokenStream stream(string);
    return ParseFontFaceDescriptor(id, stream, context);
}

CSSValue* AtRuleDescriptorParser::ParseFontFaceDeclaration(CSSParserTokenStream& stream, const CSSParserContext& context)
{
...
CSSValue* AtRuleDescriptorParser::ParseAtPropertyDescriptor(AtRuleDescriptorID id, CSSParserTokenStream& stream, const CSSParserContext& context)
{
    CSSValue* parsed_value = nullptr;
    switch (id) {
    ...
    case AtRuleDescriptorID::InitialValue: {
        ...
        CSSVariableData* variable_data = CSSVariableParser::ConsumeUnparsedDeclaration(stream, /*allow_important_annotation=*/false,
            /*is_animation_tainted=*/false,
            /*must_contain_variable_reference=*/false,
            /*restricted_value=*/false, /*comma_ends_declaration=*/false, important_ignored, context);
        if (variable_data) {
...
CSSValue* AtRuleDescriptorParser::ParseAtViewTransitionDescriptor(AtRuleDescriptorID id, CSSParserTokenStream& stream, const CSSParserContext& context)
{
    CSSValue* parsed_value = nullptr;
    switch (id) {
    ...
    case AtRuleDescriptorID::Types: {
        ...
        while (!stream.AtEnd()) {
            ...
            CSSCustomIdentValue* ident = css_parsing_utils::ConsumeCustomIdent(stream, context);
...
bool AtRuleDescriptorParser::ParseDescriptorValue(StyleRule::RuleType rule_type, AtRuleDescriptorID id, CSSParserTokenStream& stream,
    const CSSParserContext& context, HeapVector<CSSPropertyValue, 64>& parsed_descriptors)
{
