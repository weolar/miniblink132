...
TEST_F(AtRuleDescriptorParserTest, NoUseCountUACounterStyle)
{
    ...
    EXPECT_FALSE(GetDocument().IsUseCounted(mojom::WebFeature::kCSSAtRuleCounterStyle));
    ...
    EXPECT_FALSE(GetDocument().IsPropertyCounted(CSSPropertyID::kAdditiveSymbols));
...
TEST_F(AtRuleDescriptorParserTest, UseCountCounterStyleDescriptors)
{
    ...
    EXPECT_TRUE(GetDocument().IsUseCounted(mojom::WebFeature::kCSSAtRuleCounterStyle));
