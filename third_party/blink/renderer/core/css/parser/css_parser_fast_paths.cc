...
static unsigned ParsePositiveDouble(const LChar* string, const LChar* end, double& value);

static bool ParseDoubleWithPrefix(const LChar* string, const LChar* end, double& value);
...
static inline bool IsSimpleLengthPropertyID(CSSPropertyID property_id, bool& accepts_negative_numbers)
{
    ...
    static CSSBitset accept_negative { { CSSPropertyID::kBottom, CSSPropertyID::kCx, CSSPropertyID::kCy, CSSPropertyID::kLeft, CSSPropertyID::kMarginBottom,
        CSSPropertyID::kMarginLeft, CSSPropertyID::kMarginRight, CSSPropertyID::kMarginTop, CSSPropertyID::kOffsetDistance, CSSPropertyID::kRight,
        CSSPropertyID::kTop, CSSPropertyID::kMarginBlockEnd, CSSPropertyID::kMarginBlockStart, CSSPropertyID::kMarginInlineEnd,
        CSSPropertyID::kMarginInlineStart, CSSPropertyID::kX, CSSPropertyID::kY } };
...
ALWAYS_INLINE static bool ParseSimpleLength(const LChar* characters, unsigned length, CSSPrimitiveValue::UnitType& unit, double& number)
{
...
static CSSValue* ParseSimpleLengthValue(CSSPropertyID property_id, StringView string, CSSParserMode css_parser_mode)
{
    DCHECK(!string.empty());
    ...
    const bool parsed_simple_length = ParseSimpleLength(string.Characters8(), string.length(), unit, number);
...
ALWAYS_INLINE static unsigned ParseSimpleAngle(const LChar* characters, unsigned length, CSSPrimitiveValue::UnitType& unit, double& number)
{
    int number_length;
    ...
    } else if (length >= 4 && (characters[0] | 0x20) == 'g' && (characters[1] | 0x20) == 'r' && (characters[2] | 0x20) == 'a'
        && (characters[3] | 0x20) == 'd') {
        unit = CSSPrimitiveValue::UnitType::kGradians;
        return number_length + 4;
    } else if (length >= 3 && (characters[0] | 0x20) == 'r' && (characters[1] | 0x20) == 'a' && (characters[2] | 0x20) == 'd') {
        unit = CSSPrimitiveValue::UnitType::kRadians;
        return number_length + 3;
    } else if (length >= 4 && (characters[0] | 0x20) == 't' && (characters[1] | 0x20) == 'u' && (characters[2] | 0x20) == 'r'
        && (characters[3] | 0x20) == 'n') {
...
static unsigned FindLengthOfValidDouble(const LChar* string, const LChar* end)
{
    ...
    if (length >= 16) {
        ...
        uint64_t is_decimal_bits = vget_lane_u64(vreinterpret_u64_u8(vshrn_n_u16(vreinterpretq_u16_s8(is_decimal_mask), 4)), 0);
...
static bool ContainsCharAtPos(const LChar* string, const LChar* end, int pos, char ch, bool also_accept_whitespace)
{
    DCHECK_GE(pos, 0);
...
ALWAYS_INLINE static unsigned ParsePositiveDouble(const LChar* string, const LChar* end, double& value)
{
...
ALWAYS_INLINE static bool ParseFloatWithMaxValue(const LChar*& string, const LChar* end, int max_value, double& value, bool& negative)
{
    value = 0.0;
...
static bool SkipToTerminator(const LChar*& string, const LChar* end, const char terminator, TerminatorStatus& terminator_status)
{
    const LChar* current = string;
...
static bool ParseColorNumberOrPercentage(
    const LChar*& string, const LChar* end, const char terminator, TerminatorStatus& terminator_status, CSSPrimitiveValue::UnitType& expect, int& value)
{
    const LChar* current = string;
    double local_value;
    bool negative = false;
...
// 0.0..1.0.
ALWAYS_INLINE static bool ParsePercentage(const LChar*& string, const LChar* end, const char terminator, TerminatorStatus& terminator_status, double& value)
{
    const LChar* current = string;
    bool negative = false;
...
ALWAYS_INLINE static bool ParseAlphaValue(const LChar*& string, const LChar* end, const char terminator, int& value)
{
    while (string != end && IsHTMLSpace(*string)) {
        string++;
    }
    ...
    if (dbl_length == 0
        || !ContainsCharAtPos(string, end, dbl_length, terminator,
            /*also_accept_whitespace=*/false)) {
        return false;
    }
...
template <int N> static inline bool MatchesLiteral(const LChar* a, const char (&b)[N])
{
    return memcmp(a, b, N - 1) == 0;
}

template <int N> static inline bool MatchesLiteral(const UChar* a, const char (&b)[N])
{
    for (int i = 0; i < N - 1; ++i) {
        if (a[i] != b[i]) {
            return false;
...
static inline bool MatchesCaseInsensitiveLiteral4(const LChar* a, const char (&b)[5])
{
    uint32_t av, bv;
...
static inline bool MatchesCaseInsensitiveLiteral2(const LChar* a, const char (&b)[3])
{
    uint16_t av, bv;
    memcpy(&av, a, sizeof(av));
    memcpy(&bv, b, sizeof(bv));
...
static bool FastParseColorInternal(Color& color, const LChar* characters, unsigned length, bool quirks_mode)
{
...
static ParseColorResult ParseColor(CSSPropertyID property_id, StringView string, CSSParserMode parser_mode, Color& out_color, CSSValueID& out_color_keyword)
{
    DCHECK(!string.empty());
    DCHECK(IsColorPropertyID(property_id));
    CSSValueID value_id = CssValueKeywordID(string);
    if ((value_id == CSSValueID::kAccentcolor || value_id == CSSValueID::kAccentcolortext) && !RuntimeEnabledFeatures::CSSAccentColorKeywordEnabled()) {
    ...
    const bool parsed = FastParseColorInternal(out_color, string.Characters8(), string.length(), quirks_mode);
...
ParseColorResult CSSParserFastPaths::ParseColor(const String& string, CSSParserMode parser_mode, Color& color)
{
    ...
    CSSValueID color_id;
    return blink::ParseColor(CSSPropertyID::kColor, string, parser_mode, color, color_id);
}

bool CSSParserFastPaths::IsValidKeywordPropertyAndValue(CSSPropertyID property_id, CSSValueID value_id, CSSParserMode parser_mode)
{
    ...
    case CSSPropertyID::kAlignmentBaseline:
        return value_id == CSSValueID::kAuto || value_id == CSSValueID::kAlphabetic || value_id == CSSValueID::kBaseline || value_id == CSSValueID::kMiddle
            || value_id == CSSValueID::kHanging || (value_id >= CSSValueID::kBeforeEdge && value_id <= CSSValueID::kMathematical);
    ...
    case CSSPropertyID::kClear:
        return value_id == CSSValueID::kNone || value_id == CSSValueID::kLeft || value_id == CSSValueID::kRight || value_id == CSSValueID::kBoth
            || value_id == CSSValueID::kInlineStart || value_id == CSSValueID::kInlineEnd;
    ...
    case CSSPropertyID::kColorRendering:
        return value_id == CSSValueID::kAuto || value_id == CSSValueID::kOptimizespeed || value_id == CSSValueID::kOptimizequality;
    case CSSPropertyID::kDirection:
        return value_id == CSSValueID::kLtr || value_id == CSSValueID::kRtl;
    case CSSPropertyID::kDominantBaseline:
        return value_id == CSSValueID::kAuto || value_id == CSSValueID::kAlphabetic || value_id == CSSValueID::kMiddle || value_id == CSSValueID::kHanging
    ...
    case CSSPropertyID::kFloat:
        return value_id == CSSValueID::kLeft || value_id == CSSValueID::kRight || value_id == CSSValueID::kInlineStart || value_id == CSSValueID::kInlineEnd
    ...
    case CSSPropertyID::kForcedColorAdjust:
        return value_id == CSSValueID::kNone || value_id == CSSValueID::kAuto
            || (value_id == CSSValueID::kPreserveParentColor
                && (RuntimeEnabledFeatures::ForcedColorsPreserveParentColorEnabled() || parser_mode == kUASheetMode));
    case CSSPropertyID::kImageRendering:
        return value_id == CSSValueID::kAuto || value_id == CSSValueID::kWebkitOptimizeContrast || value_id == CSSValueID::kPixelated;
    ...
    case CSSPropertyID::kObjectFit:
        return value_id == CSSValueID::kFill || value_id == CSSValueID::kContain || value_id == CSSValueID::kCover || value_id == CSSValueID::kNone
            || value_id == CSSValueID::kScaleDown;
    case CSSPropertyID::kOutlineStyle:
        return value_id == CSSValueID::kAuto || value_id == CSSValueID::kNone || (value_id >= CSSValueID::kInset && value_id <= CSSValueID::kDouble);
    ...
    case CSSPropertyID::kOverflowY:
        return value_id == CSSValueID::kVisible || value_id == CSSValueID::kHidden || value_id == CSSValueID::kScroll || value_id == CSSValueID::kAuto
            || value_id == CSSValueID::kOverlay || value_id == CSSValueID::kClip;
    case CSSPropertyID::kBreakAfter:
    case CSSPropertyID::kBreakBefore:
        return value_id == CSSValueID::kAuto || value_id == CSSValueID::kAvoid || value_id == CSSValueID::kAvoidPage || value_id == CSSValueID::kPage
            || value_id == CSSValueID::kLeft || value_id == CSSValueID::kRight || value_id == CSSValueID::kRecto || value_id == CSSValueID::kVerso
            || value_id == CSSValueID::kAvoidColumn || value_id == CSSValueID::kColumn;
    case CSSPropertyID::kBreakInside:
        return value_id == CSSValueID::kAuto || value_id == CSSValueID::kAvoid || value_id == CSSValueID::kAvoidPage || value_id == CSSValueID::kAvoidColumn;
    case CSSPropertyID::kPageOrientation:
        return value_id == CSSValueID::kUpright || value_id == CSSValueID::kRotateLeft || value_id == CSSValueID::kRotateRight;
    case CSSPropertyID::kPointerEvents:
        return value_id == CSSValueID::kVisible || value_id == CSSValueID::kNone || value_id == CSSValueID::kAll || value_id == CSSValueID::kAuto
            || (value_id >= CSSValueID::kVisiblepainted && value_id <= CSSValueID::kBoundingBox);
    case CSSPropertyID::kPosition:
        return value_id == CSSValueID::kStatic || value_id == CSSValueID::kRelative || value_id == CSSValueID::kAbsolute || value_id == CSSValueID::kFixed
            || value_id == CSSValueID::kSticky;
    case CSSPropertyID::kPositionTryOrder:
        return value_id == CSSValueID::kNormal || value_id == CSSValueID::kMostWidth || value_id == CSSValueID::kMostHeight
            || value_id == CSSValueID::kMostBlockSize || value_id == CSSValueID::kMostInlineSize;
    case CSSPropertyID::kReadingFlow:
        DCHECK(RuntimeEnabledFeatures::CSSReadingFlowEnabled());
        return value_id == CSSValueID::kNormal || value_id == CSSValueID::kFlexVisual || value_id == CSSValueID::kFlexFlow || value_id == CSSValueID::kGridRows
            || value_id == CSSValueID::kGridColumns || value_id == CSSValueID::kGridOrder;
    case CSSPropertyID::kResize:
        return value_id == CSSValueID::kNone || value_id == CSSValueID::kBoth || value_id == CSSValueID::kHorizontal || value_id == CSSValueID::kVertical
            || value_id == CSSValueID::kBlock || value_id == CSSValueID::kInline || value_id == CSSValueID::kInternalTextareaAuto
            || (RuntimeEnabledFeatures::CSSResizeAutoEnabled() && value_id == CSSValueID::kAuto);
    ...
    case CSSPropertyID::kShapeRendering:
        return value_id == CSSValueID::kAuto || value_id == CSSValueID::kOptimizespeed || value_id == CSSValueID::kCrispedges
            || value_id == CSSValueID::kGeometricprecision;
    case CSSPropertyID::kSpeak:
        return value_id == CSSValueID::kNone || value_id == CSSValueID::kNormal || value_id == CSSValueID::kSpellOut || value_id == CSSValueID::kDigits
            || value_id == CSSValueID::kLiteralPunctuation || value_id == CSSValueID::kNoPunctuation;
    ...
    case CSSPropertyID::kTextAlign:
        return (value_id >= CSSValueID::kWebkitAuto && value_id <= CSSValueID::kInternalCenter) || value_id == CSSValueID::kStart
            || value_id == CSSValueID::kEnd;
    case CSSPropertyID::kTextAlignLast:
        return (value_id >= CSSValueID::kLeft && value_id <= CSSValueID::kJustify) || value_id == CSSValueID::kStart || value_id == CSSValueID::kEnd
    ...
    case CSSPropertyID::kTextDecorationStyle:
        return value_id == CSSValueID::kSolid || value_id == CSSValueID::kDouble || value_id == CSSValueID::kDotted || value_id == CSSValueID::kDashed
            || value_id == CSSValueID::kWavy;
    ...
    case CSSPropertyID::kWebkitTextOrientation:
        return value_id == CSSValueID::kSideways || value_id == CSSValueID::kSidewaysRight || value_id == CSSValueID::kVerticalRight
    ...
    case CSSPropertyID::kTextRendering:
        return value_id == CSSValueID::kAuto || value_id == CSSValueID::kOptimizespeed || value_id == CSSValueID::kOptimizelegibility
            || value_id == CSSValueID::kGeometricprecision;
    case CSSPropertyID::kTextTransform:
        return (value_id >= CSSValueID::kCapitalize && value_id <= CSSValueID::kMathAuto) || value_id == CSSValueID::kNone;
    case CSSPropertyID::kUnicodeBidi:
        return value_id == CSSValueID::kNormal || value_id == CSSValueID::kEmbed || value_id == CSSValueID::kBidiOverride
            || value_id == CSSValueID::kWebkitIsolate || value_id == CSSValueID::kWebkitIsolateOverride || value_id == CSSValueID::kWebkitPlaintext
            || value_id == CSSValueID::kIsolate || value_id == CSSValueID::kIsolateOverride || value_id == CSSValueID::kPlaintext;
    ...
    case CSSPropertyID::kAppearance:
        return (value_id == CSSValueID::kCheckbox || value_id == CSSValueID::kRadio || value_id == CSSValueID::kButton || value_id == CSSValueID::kListbox
                   || value_id == CSSValueID::kInternalMediaControl || value_id == CSSValueID::kMenulist || value_id == CSSValueID::kMenulistButton
                   || value_id == CSSValueID::kMeter || value_id == CSSValueID::kProgressBar || value_id == CSSValueID::kSearchfield
                   || value_id == CSSValueID::kTextfield || value_id == CSSValueID::kTextarea)
            || (RuntimeEnabledFeatures::CustomizableSelectEnabled() && value_id == CSSValueID::kBaseSelect)
            || (RuntimeEnabledFeatures::NonStandardAppearanceValueSliderVerticalEnabled() && value_id == CSSValueID::kSliderVertical)
    ...
    case CSSPropertyID::kMixBlendMode:
        return value_id == CSSValueID::kNormal || value_id == CSSValueID::kMultiply || value_id == CSSValueID::kScreen || value_id == CSSValueID::kOverlay
            || value_id == CSSValueID::kDarken || value_id == CSSValueID::kLighten || value_id == CSSValueID::kColorDodge || value_id == CSSValueID::kColorBurn
            || value_id == CSSValueID::kHardLight || value_id == CSSValueID::kSoftLight || value_id == CSSValueID::kDifference
            || value_id == CSSValueID::kExclusion || value_id == CSSValueID::kHue || value_id == CSSValueID::kSaturation || value_id == CSSValueID::kColor
            || value_id == CSSValueID::kLuminosity || value_id == CSSValueID::kPlusLighter;
    case CSSPropertyID::kWebkitBoxAlign:
        return value_id == CSSValueID::kStretch || value_id == CSSValueID::kStart || value_id == CSSValueID::kEnd || value_id == CSSValueID::kCenter
    ...
    case CSSPropertyID::kWebkitBoxOrient:
        return value_id == CSSValueID::kHorizontal || value_id == CSSValueID::kVertical || value_id == CSSValueID::kInlineAxis
            || value_id == CSSValueID::kBlockAxis;
    case CSSPropertyID::kWebkitBoxPack:
        return value_id == CSSValueID::kStart || value_id == CSSValueID::kEnd || value_id == CSSValueID::kCenter || value_id == CSSValueID::kJustify;
    ...
    case CSSPropertyID::kAlignContent:
        ...
        // 'justify-self'.
        return value_id == CSSValueID::kFlexStart || value_id == CSSValueID::kFlexEnd || value_id == CSSValueID::kCenter
            || value_id == CSSValueID::kSpaceBetween || value_id == CSSValueID::kSpaceAround || value_id == CSSValueID::kStretch;
    case CSSPropertyID::kAlignItems:
        ...
        return value_id == CSSValueID::kFlexStart || value_id == CSSValueID::kFlexEnd || value_id == CSSValueID::kCenter || value_id == CSSValueID::kBaseline
            || value_id == CSSValueID::kStretch;
    case CSSPropertyID::kAlignSelf:
        // FIXME: Per CSS alignment, this property should accept the same
        // arguments as 'justify-self' so we should share its parsing code.
        return value_id == CSSValueID::kAuto || value_id == CSSValueID::kFlexStart || value_id == CSSValueID::kFlexEnd || value_id == CSSValueID::kCenter
            || value_id == CSSValueID::kBaseline || value_id == CSSValueID::kStretch;
    case CSSPropertyID::kFlexDirection:
        return value_id == CSSValueID::kRow || value_id == CSSValueID::kRowReverse || value_id == CSSValueID::kColumn || value_id == CSSValueID::kColumnReverse;
    ...
    case CSSPropertyID::kHyphens:
        ...
        return value_id == CSSValueID::kAuto || value_id == CSSValueID::kNone || value_id == CSSValueID::kManual;
#else
    ...
    case CSSPropertyID::kJustifyContent:
        ...
        // 'justify-self'.
        return value_id == CSSValueID::kFlexStart || value_id == CSSValueID::kFlexEnd || value_id == CSSValueID::kCenter
            || value_id == CSSValueID::kSpaceBetween || value_id == CSSValueID::kSpaceAround;
    ...
    case CSSPropertyID::kFontVariantEmoji:
        ...
        return value_id == CSSValueID::kNormal || value_id == CSSValueID::kText || value_id == CSSValueID::kEmoji || value_id == CSSValueID::kUnicode;
    case CSSPropertyID::kLineBreak:
        return value_id == CSSValueID::kAuto || value_id == CSSValueID::kLoose || value_id == CSSValueID::kNormal || value_id == CSSValueID::kStrict
    ...
    case CSSPropertyID::kRubyAlign:
        return value_id == CSSValueID::kSpaceAround || value_id == CSSValueID::kStart || value_id == CSSValueID::kCenter
    ...
    case CSSPropertyID::kTextSpacingTrim:
        return value_id == CSSValueID::kNormal || value_id == CSSValueID::kTrimStart || value_id == CSSValueID::kSpaceAll
    ...
    case CSSPropertyID::kWebkitTextSecurity:
        return value_id == CSSValueID::kDisc || value_id == CSSValueID::kCircle || value_id == CSSValueID::kSquare || value_id == CSSValueID::kNone;
    ...
    case CSSPropertyID::kTextWrapStyle:
        return value_id == CSSValueID::kAuto || value_id == CSSValueID::kBalance || value_id == CSSValueID::kPretty || value_id == CSSValueID::kStable;
    case CSSPropertyID::kTransformBox:
        return value_id == CSSValueID::kContentBox || value_id == CSSValueID::kBorderBox || value_id == CSSValueID::kStrokeBox
            || value_id == CSSValueID::kFillBox || value_id == CSSValueID::kViewBox;
    ...
    case CSSPropertyID::kWebkitUserModify:
        return value_id == CSSValueID::kReadOnly || value_id == CSSValueID::kReadWrite || value_id == CSSValueID::kReadWritePlaintextOnly;
    case CSSPropertyID::kUserSelect:
        if (!RuntimeEnabledFeatures::CSSUserSelectContainEnabled()) {
            return value_id == CSSValueID::kAuto || value_id == CSSValueID::kNone || value_id == CSSValueID::kText || value_id == CSSValueID::kAll;
        }
        return value_id == CSSValueID::kAuto || value_id == CSSValueID::kNone || value_id == CSSValueID::kText || value_id == CSSValueID::kAll
    ...
    case CSSPropertyID::kWritingMode:
        if (RuntimeEnabledFeatures::SidewaysWritingModesEnabled()) {
            if (value_id == CSSValueID::kSidewaysRl || value_id == CSSValueID::kSidewaysLr) {
                return true;
            }
        }
        return value_id == CSSValueID::kHorizontalTb || value_id == CSSValueID::kVerticalRl || value_id == CSSValueID::kVerticalLr
            || value_id == CSSValueID::kLrTb || value_id == CSSValueID::kRlTb || value_id == CSSValueID::kTbRl || value_id == CSSValueID::kLr
            || value_id == CSSValueID::kRl || value_id == CSSValueID::kTb;
    case CSSPropertyID::kWhiteSpaceCollapse:
        return value_id == CSSValueID::kCollapse || value_id == CSSValueID::kPreserve || value_id == CSSValueID::kPreserveBreaks
            || value_id == CSSValueID::kBreakSpaces;
    case CSSPropertyID::kWordBreak:
        return value_id == CSSValueID::kNormal || value_id == CSSValueID::kBreakAll || value_id == CSSValueID::kKeepAll || value_id == CSSValueID::kBreakWord
    ...
    case CSSPropertyID::kTextBoxTrim:
        ...
        return value_id == CSSValueID::kNone || value_id == CSSValueID::kTrimStart || value_id == CSSValueID::kTrimEnd || value_id == CSSValueID::kTrimBoth;
...
static inline CSSValue* ParseCSSWideKeywordValue(const LChar* ptr, unsigned length)
{
...
static CSSValue* ParseKeywordValue(CSSPropertyID property_id, StringView string, const CSSParserContext* context)
{
    DCHECK(!string.empty());

    CSSValue* css_wide_keyword = ParseCSSWideKeywordValue(string.Characters8(), string.length());
...
static bool ParseTransformTranslateArguments(const LChar*& pos, const LChar* end, unsigned expected_count, CSSFunctionValue* transform_value)
{
    while (expected_count) {
        wtf_size_t delimiter = WTF::Find(pos, static_cast<wtf_size_t>(end - pos), expected_count == 1 ? ')' : ',');
...
static bool ParseTransformRotateArgument(const LChar*& pos, const LChar* end, CSSFunctionValue* transform_value)
{
...
static bool ParseTransformNumberArguments(const LChar*& pos, const LChar* end, unsigned expected_count, CSSFunctionValue* transform_value)
{
    while (expected_count) {
        wtf_size_t delimiter = WTF::Find(pos, static_cast<wtf_size_t>(end - pos), expected_count == 1 ? ')' : ',');
        if (delimiter == kNotFound) {
...
static CSSFunctionValue* ParseSimpleTransformValue(const LChar*& pos, const LChar* end)
{
    ...
    if (is_translate) {
        ...
        pos += argument_start;
        CSSFunctionValue* transform_value = MakeGarbageCollected<CSSFunctionValue>(transform_type);
    ...
    if (is_matrix3d) {
        pos += 9;
        CSSFunctionValue* transform_value = MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kMatrix3d);
    ...
    if (is_scale3d) {
        pos += 8;
        CSSFunctionValue* transform_value = MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kScale3d);
    ...
    if (is_rotate) {
        ...
        CSSFunctionValue* transform_value = MakeGarbageCollected<CSSFunctionValue>(rotate_value_id);
...
static CSSValue* ParseSimpleTransform(CSSPropertyID property_id, StringView string)
{
    DCHECK(!string.empty());
...
CSSValue* CSSParserFastPaths::MaybeParseValue(CSSPropertyID property_id, StringView string, const CSSParserContext* context)
{
    if (!string.Is8Bit()) {
    ...
    if (CSSValue* length = ParseSimpleLengthValue(property_id, string, context->Mode())) {
        return length;
    }
    if (IsColorPropertyID(property_id)) {
        Color color;
        CSSValueID color_id;
        switch (blink::ParseColor(property_id, string, context->Mode(), color, color_id)) {
