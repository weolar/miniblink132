...
class CORE_EXPORT CSSParserContext final : public GarbageCollected<CSSParserContext> {
public:
    ...
    explicit CSSParserContext(const CSSParserContext* other, const Document* use_counter_document = nullptr);
    ...
    CSSParserContext(const CSSParserContext* other, const KURL& base_url_override, bool origin_clean, const Referrer& referrer,
        const WTF::TextEncoding& charset_override, const Document* use_counter_document);
    CSSParserContext(CSSParserMode, SecureContextMode, const Document* use_counter_document = nullptr);
    explicit CSSParserContext(const Document&);
    CSSParserContext(const Document&, const KURL& base_url_override);
    CSSParserContext(const Document&, const KURL& base_url_override, bool origin_clean, const Referrer& referrer,
        const WTF::TextEncoding& charset = WTF::TextEncoding(), ResourceFetchRestriction resource_fetch_restriction = ResourceFetchRestriction::kNone);

    // This is used for workers, where we don't have a document.
    CSSParserContext(const ExecutionContext& context);

    CSSParserContext(const KURL& base_url, bool origin_clean, const WTF::TextEncoding& charset, CSSParserMode, const Referrer& referrer, bool is_html_document,
        SecureContextMode, const DOMWrapperWorld* world, const Document* use_counter_document, ResourceFetchRestriction resource_fetch_restriction);
    ...
    // override this field.
    void SetMode(CSSParserMode mode)
    {
        mode_ = mode;
    }
