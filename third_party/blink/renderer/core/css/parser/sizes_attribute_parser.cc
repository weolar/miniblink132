...
SizesAttributeParser::SizesAttributeParser(MediaValues* media_values, const String& attribute, ExecutionContext* execution_context, const HTMLImageElement* img)
...
bool SizesAttributeParser::Parse(CSSParserTokenStream& stream)
{
    while (!stream.AtEnd()) {
        ...
        MediaQuerySet* media_condition = MediaQueryParser::ParseMediaCondition(stream, execution_context_);
...
bool SizesAttributeParser::CalculateLengthInPixels(CSSParserTokenStream& stream, float& result)
{
    ...
    if (type == kDimensionToken) {
        ...
        if ((media_values_->ComputeLength(start_token.NumericValue(), start_token.GetUnitType(), length)) && (length >= 0)) {
...
bool SizesAttributeParser::MediaConditionMatches(const MediaQuerySet& media_condition)
{
    ...
    MediaQueryEvaluator* media_query_evaluator = MakeGarbageCollected<MediaQueryEvaluator>(media_values_);
