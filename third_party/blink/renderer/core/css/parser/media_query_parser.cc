...
public:
    ...
    {
        if (feature == media_feature_names::kInlineSizeMediaFeature || feature == media_feature_names::kMinInlineSizeMediaFeature
            || feature == media_feature_names::kMaxInlineSizeMediaFeature || feature == media_feature_names::kBlockSizeMediaFeature
            || feature == media_feature_names::kMinBlockSizeMediaFeature || feature == media_feature_names::kMaxBlockSizeMediaFeature
            || feature == media_feature_names::kStuckMediaFeature || feature == media_feature_names::kSnappedMediaFeature
            || feature == media_feature_names::kOverflowingMediaFeature || CSSVariableParser::IsValidVariableName(feature)) {
            return false;
        }
        return true;
    }
    bool IsAllowedWithoutValue(const AtomicString& feature, const ExecutionContext* execution_context) const override
    {
        // Media features that are prefixed by min/max cannot be used without a
        // value.
        return feature == media_feature_names::kMonochromeMediaFeature || feature == media_feature_names::kColorMediaFeature
            || feature == media_feature_names::kColorIndexMediaFeature || feature == media_feature_names::kGridMediaFeature
            || feature == media_feature_names::kHeightMediaFeature || feature == media_feature_names::kWidthMediaFeature
            || feature == media_feature_names::kBlockSizeMediaFeature || feature == media_feature_names::kInlineSizeMediaFeature
            || feature == media_feature_names::kDeviceHeightMediaFeature || feature == media_feature_names::kDeviceWidthMediaFeature
            || feature == media_feature_names::kOrientationMediaFeature || feature == media_feature_names::kAspectRatioMediaFeature
            || feature == media_feature_names::kDeviceAspectRatioMediaFeature || feature == media_feature_names::kHoverMediaFeature
            || feature == media_feature_names::kAnyHoverMediaFeature || feature == media_feature_names::kTransform3dMediaFeature
            || feature == media_feature_names::kPointerMediaFeature || feature == media_feature_names::kAnyPointerMediaFeature
            || feature == media_feature_names::kDevicePixelRatioMediaFeature || feature == media_feature_names::kResolutionMediaFeature
            || feature == media_feature_names::kDisplayModeMediaFeature || feature == media_feature_names::kScanMediaFeature
            || feature == media_feature_names::kColorGamutMediaFeature || feature == media_feature_names::kPrefersColorSchemeMediaFeature
            || feature == media_feature_names::kPrefersContrastMediaFeature || feature == media_feature_names::kPrefersReducedMotionMediaFeature
            || feature == media_feature_names::kOverflowInlineMediaFeature || feature == media_feature_names::kOverflowBlockMediaFeature
            || feature == media_feature_names::kUpdateMediaFeature
            || (feature == media_feature_names::kPrefersReducedDataMediaFeature && RuntimeEnabledFeatures::PrefersReducedDataEnabled())
            || feature == media_feature_names::kPrefersReducedTransparencyMediaFeature
            || (feature == media_feature_names::kForcedColorsMediaFeature && RuntimeEnabledFeatures::ForcedColorsEnabled())
            || (feature == media_feature_names::kNavigationControlsMediaFeature && RuntimeEnabledFeatures::MediaQueryNavigationControlsEnabled())
            || (feature == media_feature_names::kOriginTrialTestMediaFeature && RuntimeEnabledFeatures::OriginTrialsSampleAPIEnabled(execution_context))
            || (feature == media_feature_names::kHorizontalViewportSegmentsMediaFeature && RuntimeEnabledFeatures::ViewportSegmentsEnabled(execution_context))
            || (feature == media_feature_names::kVerticalViewportSegmentsMediaFeature && RuntimeEnabledFeatures::ViewportSegmentsEnabled(execution_context))
            || (feature == media_feature_names::kDevicePostureMediaFeature && RuntimeEnabledFeatures::DevicePostureEnabled(execution_context))
            || (feature == media_feature_names::kInvertedColorsMediaFeature && RuntimeEnabledFeatures::InvertedColorsEnabled())
            || CSSVariableParser::IsValidVariableName(feature) || feature == media_feature_names::kScriptingMediaFeature
            || (RuntimeEnabledFeatures::DesktopPWAsAdditionalWindowingControlsEnabled() && feature == media_feature_names::kDisplayStateMediaFeature)
            || (RuntimeEnabledFeatures::DesktopPWAsAdditionalWindowingControlsEnabled() && feature == media_feature_names::kResizableMediaFeature);
    }
    ...
    bool SupportsRange() const override
    {
        return true;
    }
};
...
MediaQuerySet* MediaQueryParser::ParseMediaQuerySet(StringView query_string, ExecutionContext* execution_context)
{
    CSSParserTokenStream stream(query_string);
    return ParseMediaQuerySet(stream, execution_context);
}

MediaQuerySet* MediaQueryParser::ParseMediaQuerySet(CSSParserTokenStream& stream, ExecutionContext* execution_context)
{
    return MediaQueryParser(kMediaQuerySetParser, kHTMLStandardMode, execution_context).ParseImpl(stream);
}

MediaQuerySet* MediaQueryParser::ParseMediaQuerySetInMode(CSSParserTokenStream& stream, CSSParserMode mode, ExecutionContext* execution_context)
{
    return MediaQueryParser(kMediaQuerySetParser, mode, execution_context).ParseImpl(stream);
}

MediaQuerySet* MediaQueryParser::ParseMediaCondition(CSSParserTokenStream& stream, ExecutionContext* execution_context)
{
    return MediaQueryParser(kMediaConditionParser, kHTMLStandardMode, execution_context).ParseImpl(stream);
}

MediaQueryParser::MediaQueryParser(ParserType parser_type, CSSParserMode mode, ExecutionContext* execution_context, SyntaxLevel syntax_level)
    : parser_type_(parser_type)
    , mode_(mode)
    , execution_context_(execution_context)
    , syntax_level_(syntax_level)
    , fake_context_(*MakeGarbageCollected<CSSParserContext>(kHTMLStandardMode, SecureContextMode::kInsecureContext,
          DynamicTo<LocalDOMWindow>(execution_context) ? DynamicTo<LocalDOMWindow>(execution_context)->document() : nullptr))
{
}
...
bool IsRestrictorOrLogicalOperator(const CSSParserToken& token)
{
    ...
    return EqualIgnoringASCIICase(token.Value(), "not") || EqualIgnoringASCIICase(token.Value(), "and") || EqualIgnoringASCIICase(token.Value(), "or")
        || EqualIgnoringASCIICase(token.Value(), "only") || EqualIgnoringASCIICase(token.Value(), "layer");
}
...
MediaQueryOperator MediaQueryParser::ConsumeComparison(CSSParserTokenStream& stream)
{
    ...
    if (first.GetType() != kDelimiterToken || !IsComparisonDelimiter(first.Delimiter())) {
...
const MediaQueryExpNode* MediaQueryParser::ConsumeFeature(CSSParserTokenStream& stream, const FeatureSet& feature_set)
{
...

    {
        ...
        // <mf-boolean> = <mf-name>
        if (!feature_name.IsNull() && stream.AtEnd() && feature_set.IsAllowedWithoutValue(feature_name, execution_context_)) {
            return MakeGarbageCollected<MediaQueryFeatureExpNode>(MediaQueryExp::Create(feature_name, MediaQueryExpBounds()));
        }
...

    {
        ...
        if (!feature_name.IsNull() && !stream.AtEnd()) {
            ...
            if (op != MediaQueryOperator::kNone) {
                auto value = MediaQueryExpValue::Consume(feature_name, stream, fake_context_);
                if (value && stream.AtEnd()) {
                    auto left = MediaQueryExpComparison();
                    auto right = MediaQueryExpComparison(*value, op);

                    UseCountRangeSyntax();
                    return MakeGarbageCollected<MediaQueryFeatureExpNode>(MediaQueryExp::Create(feature_name, MediaQueryExpBounds(left, right)));
                }
            }
        }
        stream.Restore(start);
    ...
    stream.Restore(start);
    auto value1 = MediaQueryExpValue::Consume(feature_name, stream, fake_context_);
    if (!value1) {
        return nullptr;
    }
    ...
    auto value2 = MediaQueryExpValue::Consume(feature_name, stream, fake_context_);
    if (!value2) {
        return nullptr;
    }
    ...
    return MakeGarbageCollected<MediaQueryFeatureExpNode>(
        MediaQueryExp::Create(feature_name, MediaQueryExpBounds(MediaQueryExpComparison(*value1, op1), MediaQueryExpComparison(*value2, op2))));
}

const MediaQueryExpNode* MediaQueryParser::ConsumeCondition(CSSParserTokenStream& stream, ConditionMode mode)
{
    // <media-not>
    if (ConsumeIfIdent(stream, "not")) {
...
const MediaQueryExpNode* MediaQueryParser::ConsumeInParens(CSSParserTokenStream& stream)
{
    if (stream.Peek().GetType() == kLeftParenthesisToken) {
...

        {
            ...
            stream.ConsumeWhitespace();
            // ( <media-feature> )
            const MediaQueryExpNode* feature = ConsumeFeature(stream, MediaQueryFeatureSet());
...
const MediaQueryExpNode* MediaQueryParser::ConsumeGeneralEnclosed(CSSParserTokenStream& stream)
{
    ...
    stream.ConsumeWhitespace();
    return MakeGarbageCollected<MediaQueryUnknownExpNode>(general_enclosed.ToString());
}

MediaQuerySet* MediaQueryParser::ConsumeSingleCondition(CSSParserTokenStream& stream)
{
    ...
    if (!node) {
        ...
        queries.push_back(MakeGarbageCollected<MediaQuery>(MediaQuery::RestrictorType::kNone, media_type_names::kAll, node));
    }
...
MediaQuery* MediaQueryParser::ConsumeQuery(CSSParserTokenStream& stream)
{
    ...
    if (!type.IsNull()) {
        ...
        if (const MediaQueryExpNode* node = ConsumeCondition(stream, ConditionMode::kWithoutOr)) {
    ...
    if (const MediaQueryExpNode* node = ConsumeCondition(stream)) {
        return MakeGarbageCollected<MediaQuery>(MediaQuery::RestrictorType::kNone, media_type_names::kAll, node);
    }
    return nullptr;
}
...
MediaQuerySet* MediaQueryParser::ParseImpl(CSSParserTokenStream& stream)
{
    ...
    do {
        ...
        bool ok = query && (stream.AtEnd() || stream.Peek().GetType() == kCommaToken);
