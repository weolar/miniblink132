...
public:
    ...
    template <bool characterPredicate(UChar)> unsigned SkipWhilePredicate(unsigned offset)
    {
        if (string_.Is8Bit()) {
            ...
            while ((offset_ + offset) < string_length_ && characterPredicate(characters16[offset_ + offset])) {
                ++offset;
            }
        }
        return offset;
    ...
    {
        return std::min(offset_, string_length_);
    }
