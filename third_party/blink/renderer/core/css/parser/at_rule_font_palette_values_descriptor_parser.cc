...
CSSValue* ConsumeFontFamily(CSSParserTokenStream& stream, const CSSParserContext& context)
{
    return css_parsing_utils::ConsumeNonGenericFamilyNameList(stream);
}

CSSValue* ConsumeBasePalette(CSSParserTokenStream& stream, const CSSParserContext& context)
{
    if (CSSValue* ident = css_parsing_utils::ConsumeIdent<CSSValueID::kLight, CSSValueID::kDark>(stream)) {
        return ident;
    }
...
CSSValue* ConsumeColorOverride(CSSParserTokenStream& stream, const CSSParserContext& context)
{
    CSSValueList* list = CSSValueList::CreateCommaSeparated();
    do {
        CSSValue* color_index = css_parsing_utils::ConsumeInteger(stream, context, 0);
        if (!color_index) {
            return nullptr;
        }
        ...
        list->Append(*MakeGarbageCollected<CSSValuePair>(color_index, color, CSSValuePair::kKeepIdenticalValues));
...
CSSValue* AtRuleDescriptorParser::ParseAtFontPaletteValuesDescriptor(AtRuleDescriptorID id, CSSParserTokenStream& stream, const CSSParserContext& context)
{
