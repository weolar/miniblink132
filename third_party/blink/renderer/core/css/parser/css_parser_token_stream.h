...
template <typename...> bool IsTokenTypeOneOf(CSSParserTokenType t)
{
    return false;
}

template <CSSParserTokenType Head, CSSParserTokenType... Tail> bool IsTokenTypeOneOf(CSSParserTokenType t)
{
...
public:
    ...
    public:
        explicit BlockGuard(CSSParserTokenStream& stream)
            : stream_(stream)
            , boundaries_(stream.boundaries_)
        {
    ...
    public:
        explicit EnableUnicodeRanges(CSSParserTokenStream& stream, bool unicode_ranges_allowed)
            : stream_(stream)
            , old_unicode_ranges_allowed_(stream.tokenizer_.unicode_ranges_allowed_)
        {
    ...
    inline bool HasLookAhead() const
    {
        return has_look_ahead_;
    }
    ...
    const CSSParserToken& UncheckedConsume()
    {
        ...
        DCHECK(peeked_at_next_) << "You blindly called Consume() without checking the token first, "
    ...
    template <CSSParserTokenType... Types> void SkipUntilPeekedTypeIs()
    {
        EnsureLookAhead();
    ...
    {
        return tokenizer_.TokenCount();
    }

private:
    template <CSSParserTokenType... EndTypes> ALWAYS_INLINE bool TokenMarksEnd(const CSSParserToken& token)
    {
    ...
    void PopBlockStack()
    {
        tokenizer_.block_stack_.pop_back();
    }
