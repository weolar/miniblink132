...
StringView CSSTokenizer::StringRangeAt(wtf_size_t start, wtf_size_t length) const
{
...
template <bool SkipComments> CSSParserToken CSSTokenizer::NextToken()
{
    do {
        ...
        case '<':
            if (input_.PeekWithoutReplacement(0) == '!' && input_.PeekWithoutReplacement(1) == '-' && input_.PeekWithoutReplacement(2) == '-') {
                input_.Advance(3);
