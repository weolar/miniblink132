...
static void VerifyCSSCalc(String text, double value, bool valid, unsigned font_size, unsigned viewport_width, unsigned viewport_height)
{
    const CSSValue* css_value = CSSParser::ParseSingleValue(CSSPropertyID::kLeft, text, StrictCSSParserContext(SecureContextMode::kInsecureContext));
    ...
    Font font;
    CSSToLengthConversionData::FontSizes font_sizes(font_size, font_size, &font, 1);
    CSSToLengthConversionData::LineHeightSize line_height_size;
    CSSToLengthConversionData::ViewportSize viewport_size(viewport_width, viewport_height);
    CSSToLengthConversionData::ContainerSizes container_sizes;
    CSSToLengthConversionData::AnchorData anchor_data;
    CSSToLengthConversionData::Flags ignored_flags = 0;
    CSSToLengthConversionData conversion_data(
        WritingMode::kHorizontalTb, font_sizes, line_height_size, viewport_size, container_sizes, anchor_data, 1.0, ignored_flags);
...
TEST(SizesMathFunctionParserTest, Basic)
{
    ...
    for (const SizesCalcTestCase& test_case : test_cases) {
        ...
        VerifyCSSCalc(test_case.input, test_case.output, test_case.valid, data.em_size, data.viewport_width, data.viewport_height);
    }
}
...
TEST(SizesMathFunctionParserTest, CleansUpWhitespace)
{
    ...
    SizesMathFunctionParser calc_parser(stream, MakeGarbageCollected<MediaValuesCached>());
