...
public:
    ...
    StringView Value() const
    {
        if (value_is_inline_) {
            ...
            return StringView(reinterpret_cast<const LChar*>(value_data_char_inline_), value_length_);
        }
        if (value_is_8bit_) {
            return StringView(reinterpret_cast<const LChar*>(value_data_char_raw_), value_length_);
        }
        return StringView(reinterpret_cast<const UChar*>(value_data_char_raw_), value_length_);
    }

    bool IsEOF() const
    {
        return type_ == static_cast<unsigned>(kEOFToken);
    }
    ...
    {
        return static_cast<BlockType>(block_type_);
    }
    ...
    CSSPropertyID ParseAsUnresolvedCSSPropertyID(const ExecutionContext* execution_context, CSSParserMode mode = kHTMLStandardMode) const;
