...
static void RecordUsageAndDeprecationsOneSelector(const CSSSelector* selector, const CSSParserContext* context, bool* has_visited_pseudo);
...
{
    return selector.GetPseudoType() == CSSSelector::kPseudoHost || selector.GetPseudoType() == CSSSelector::kPseudoHostContext;
}
...
// static
base::span<CSSSelector> CSSSelectorParser::ParseSelector(CSSParserTokenStream& stream, const CSSParserContext* context, CSSNestingType nesting_type,
    const StyleRule* parent_rule_for_nesting, bool is_within_scope, bool semicolon_aborts_nested_selector, StyleSheetContents* style_sheet,
    HeapVector<CSSSelector>& arena)
{
    CSSSelectorParser parser(context, parent_rule_for_nesting, is_within_scope, semicolon_aborts_nested_selector, style_sheet, arena);
    stream.ConsumeWhitespace();
    base::span<CSSSelector> result = parser.ConsumeComplexSelectorList(stream, nesting_type);
    if (!stream.AtEnd()) {
        return {};
    }
...
// static
base::span<CSSSelector> CSSSelectorParser::ConsumeSelector(CSSParserTokenStream& stream, const CSSParserContext* context, CSSNestingType nesting_type,
    const StyleRule* parent_rule_for_nesting, bool is_within_scope, bool semicolon_aborts_nested_selector, StyleSheetContents* style_sheet,
    CSSParserObserver* observer, HeapVector<CSSSelector>& arena, bool* has_visited_style)
{
    CSSSelectorParser parser(context, parent_rule_for_nesting, is_within_scope, semicolon_aborts_nested_selector, style_sheet, arena);
    stream.ConsumeWhitespace();
    base::span<CSSSelector> result = parser.ConsumeComplexSelectorList(stream, observer, nesting_type);
    parser.RecordUsageAndDeprecations(result, has_visited_style);
    return result;
}

// static
base::span<CSSSelector> CSSSelectorParser::ParseScopeBoundary(CSSParserTokenStream& stream, const CSSParserContext* context, CSSNestingType nesting_type,
    const StyleRule* parent_rule_for_nesting, bool is_within_scope, StyleSheetContents* style_sheet, HeapVector<CSSSelector>& arena)
{
    CSSSelectorParser parser(context, parent_rule_for_nesting, is_within_scope,
        /*semicolon_aborts_nested_selector=*/false, style_sheet, arena);
...
// static
bool CSSSelectorParser::SupportsComplexSelector(CSSParserTokenStream& stream, const CSSParserContext* context)
{
    stream.ConsumeWhitespace();
    HeapVector<CSSSelector> arena;
    ...
    base::span<CSSSelector> selectors = parser.ConsumeComplexSelector(stream, CSSNestingType::kNone,
...
CSSSelectorParser::CSSSelectorParser(const CSSParserContext* context, const StyleRule* parent_rule_for_nesting, bool is_within_scope,
    bool semicolon_aborts_nested_selector, StyleSheetContents* style_sheet, HeapVector<CSSSelector>& output)
    : context_(context)
...
base::span<CSSSelector> CSSSelectorParser::ConsumeComplexSelectorList(CSSParserTokenStream& stream, CSSNestingType nesting_type)
{
...
base::span<CSSSelector> CSSSelectorParser::ConsumeComplexSelectorList(CSSParserTokenStream& stream, CSSParserObserver* observer, CSSNestingType nesting_type)
{
    ResetVectorAfterScope reset_vector(output_);
    ...
    while (true) {
        ...
        if (ConsumeComplexSelector(stream, nesting_type, first_in_complex_selector_list).empty() || failed_parsing_ || !AtEndOfComplexSelector(stream)) {
...
CSSSelectorList* CSSSelectorParser::ConsumeCompoundSelectorList(CSSParserTokenStream& stream)
{
    ResetVectorAfterScope reset_vector(output_);

    base::span<CSSSelector> selector = ConsumeCompoundSelector(stream, CSSNestingType::kNone);
    stream.ConsumeWhitespace();
    if (selector.empty()) {
        return nullptr;
    }
...
CSSSelectorList* CSSSelectorParser::ConsumeNestedSelectorList(CSSParserTokenStream& stream)
{
    ...
    ResetVectorAfterScope reset_vector(output_);
    base::span<CSSSelector> result = ConsumeComplexSelectorList(stream, CSSNestingType::kNone);
    if (result.empty()) {
        return {};
    } else {
        CSSSelectorList* selector_list = CSSSelectorList::AdoptSelectorVector(result);
        return selector_list;
    }
}

CSSSelectorList* CSSSelectorParser::ConsumeForgivingNestedSelectorList(CSSParserTokenStream& stream)
{
    if (inside_compound_pseudo_) {
        return ConsumeForgivingCompoundSelectorList(stream);
    }
    ResetVectorAfterScope reset_vector(output_);
    std::optional<base::span<CSSSelector>> forgiving_list = ConsumeForgivingComplexSelectorList(stream, CSSNestingType::kNone);
...
std::optional<base::span<CSSSelector>> CSSSelectorParser::ConsumeForgivingComplexSelectorList(CSSParserTokenStream& stream, CSSNestingType nesting_type)
{
    if (in_supports_parsing_) {
        base::span<CSSSelector> selectors = ConsumeComplexSelectorList(stream, nesting_type);
        if (selectors.empty()) {
            return std::nullopt;
        } else {
            return selectors;
    ...
    while (!stream.AtEnd()) {
        ...
        base::span<CSSSelector> selector = ConsumeComplexSelector(stream, nesting_type, first_in_complex_selector_list);
...
static CSSNestingType ConsumeUntilCommaAndFindNestingType(CSSParserTokenStream& stream)
{
    ...
    while (!stream.AtEnd()) {
        ...
        if (previous_token.GetType() == kColonToken && token.GetType() == kIdentToken && EqualIgnoringASCIICase(token.Value(), "scope")) {
...
void CSSSelectorParser::AddPlaceholderSelectorIfNeeded(CSSParserTokenStream& stream)
{
    ...
    if (nesting_type != CSSNestingType::kNone) {
        ...
        placeholder_selector.SetUnparsedPlaceholder(nesting_type, stream.StringRangeAt(start, end - start).ToAtomicString());
        placeholder_selector.SetLastInComplexSelector(true);
        output_.push_back(placeholder_selector);
    }
}

CSSSelectorList* CSSSelectorParser::ConsumeForgivingCompoundSelectorList(CSSParserTokenStream& stream)
{
    if (in_supports_parsing_) {
...
CSSSelectorList* CSSSelectorParser::ConsumeForgivingRelativeSelectorList(CSSParserTokenStream& stream)
{
    if (in_supports_parsing_) {
    ...
    if (inside_compound_pseudo_ || restricting_pseudo_element_ != CSSSelector::kPseudoUnknown || reset_vector.AddedElements().empty()) {
...
CSSSelectorList* CSSSelectorParser::ConsumeRelativeSelectorList(CSSParserTokenStream& stream)
{
    ResetVectorAfterScope reset_vector(output_);
...
unsigned ExtractCompoundFlags(const base::span<CSSSelector> compound_selector, CSSParserMode parser_mode)
{
...
base::span<CSSSelector> CSSSelectorParser::ConsumeRelativeSelector(CSSParserTokenStream& stream)
{
    ResetVectorAfterScope reset_vector(output_);
...
static CSSSelector CreateImplicitAnchor(CSSNestingType nesting_type, const StyleRule* parent_rule_for_nesting)
{
...
static std::optional<CSSSelector> MaybeCreateImplicitDescendantAnchor(
    CSSNestingType nesting_type, const StyleRule* parent_rule_for_nesting, const CSSSelector* selector)
{
    switch (nesting_type) {
...
base::span<CSSSelector> CSSSelectorParser::ConsumeNestedRelativeSelector(CSSParserTokenStream& stream, CSSNestingType nesting_type)
{
...
base::span<CSSSelector> CSSSelectorParser::ConsumeComplexSelector(
    CSSParserTokenStream& stream, CSSNestingType nesting_type, bool first_in_complex_selector_list)
{
    ...
    ResetVectorAfterScope reset_vector(output_);
    base::span<CSSSelector> compound_selector = ConsumeCompoundSelector(stream, nesting_type);
    ...
    if (nesting_type != CSSNestingType::kNone) {
        ...
        if (std::optional<CSSSelector> anchor
            = MaybeCreateImplicitDescendantAnchor(nesting_type, parent_rule_for_nesting_, reset_vector.AddedElements().data())) {
...
bool CSSSelectorParser::ConsumePartialComplexSelector(
    CSSParserTokenStream& stream, CSSSelector::RelationType& combinator, unsigned previous_compound_flags, CSSNestingType nesting_type)
{
    do {
        base::span<CSSSelector> compound_selector = ConsumeCompoundSelector(stream, nesting_type);
        if (compound_selector.empty()) {
...
// static
CSSSelector::PseudoType CSSSelectorParser::ParsePseudoType(const AtomicString& name, bool has_arguments, const Document* document)
{
    ...
    if (name.StartsWith("--")) {
        ...
        if (ExecutionContext* context = document ? document->GetExecutionContext() : nullptr) {
            Deprecation::CountDeprecation(context, WebFeature::kCSSCustomStateDeprecatedSyntax);
        }
        if (RuntimeEnabledFeatures::CSSCustomStateDeprecatedSyntaxEnabled()) {
            if (document) {
                ...
                document->AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kDeprecation, mojom::ConsoleMessageLevel::kError,
                    "Custom state pseudo classes are changing from \":--" + custom_name + "\" to \":state(" + custom_name
                        + ")\" soon. See more"
            ...
            document->AddConsoleMessage(MakeGarbageCollected<ConsoleMessage>(mojom::ConsoleMessageSource::kDeprecation, mojom::ConsoleMessageLevel::kError,
                "Custom state pseudo classes have been changed from \":--" + custom_name + "\" to \":state(" + custom_name
                    + ")\". See more here: "
...
namespace {
PseudoId ParsePseudoElementLegacy(const String& selector_string, const Node* parent)
{
    ...
    if (stream.Peek().GetType() == kIdentToken) {
        ...
        PseudoId pseudo_id = CSSSelector::GetPseudoId(CSSSelectorParser::ParsePseudoType(selector_name_token.Value().ToAtomicString(),
    ...
    if (stream.Peek().GetType() == kFunctionToken) {
        ...
        if (!PseudoElementHasArguments(pseudo_id) || !PseudoElement::IsWebExposed(pseudo_id, parent)) {
            return kPseudoIdNone;
        }
...
// static
PseudoId CSSSelectorParser::ParsePseudoElement(const String& selector_string, const Node* parent, AtomicString& argument)
{
    if (!RuntimeEnabledFeatures::CSSComputedStyleFullPseudoElementParserEnabled()) {
    ...
    // allow the legacy behavior of single-colon / no-colon.
    {
        ...
        if (selector_name_token.GetType() == kIdentToken) {
            ...
            CSSSelector::PseudoType pseudo_type = ParsePseudoType(selector_name_token.Value().ToAtomicString(),
                /*has_arguments=*/false, parent ? &parent->GetDocument() : nullptr);

            PseudoId pseudo_id = CSSSelector::GetPseudoId(pseudo_type);
            if (pseudo_id == kPseudoIdCheck || pseudo_id == kPseudoIdBefore || pseudo_id == kPseudoIdAfter || pseudo_id == kPseudoIdSelectArrow
            ...
            if ((pseudo_type == CSSSelector::PseudoType::kPseudoWebKitCustomElement || pseudo_type == CSSSelector::PseudoType::kPseudoBlinkInternalElement
                    || (!RuntimeEnabledFeatures::PseudoElementsCorrectInGetComputedStyleEnabled()
                        && (pseudo_type == CSSSelector::PseudoType::kPseudoCue || pseudo_type == CSSSelector::PseudoType::kPseudoPlaceholder
...
bool IsPseudoClassValidAfterPseudoElement(CSSSelector::PseudoType pseudo_class, CSSSelector::PseudoType compound_pseudo_element)
{
    ...
    case CSSSelector::kPseudoScrollPrevButton:
        ...
        return pseudo_class == CSSSelector::kPseudoFocus || pseudo_class == CSSSelector::kPseudoChecked;
    default:
        return false;
    }
}

bool IsSimpleSelectorValidAfterPseudoElement(const CSSSelector& simple_selector, CSSSelector::PseudoType compound_pseudo_element)
{
    switch (compound_pseudo_element) {
    case CSSSelector::kPseudoColumn:
        return simple_selector.GetPseudoType() == CSSSelector::kPseudoScrollMarker;
    ...
    case CSSSelector::kPseudoCheck:
        if (simple_selector.GetPseudoType() == CSSSelector::kPseudoMarker && RuntimeEnabledFeatures::CSSMarkerNestedPseudoElementEnabled()) {
            return true;
        }
        break;
    ...
    if ((compound_pseudo_element == CSSSelector::kPseudoPart || CSSSelector::IsElementBackedPseudoElement(compound_pseudo_element))
...
static bool SimpleSelectorRequiresScopeActivation(const CSSSelector& selector)
{
    ...
    return selector.GetPseudoType() == CSSSelector::kPseudoScope || selector.GetPseudoType() == CSSSelector::kPseudoParent;
}
...
base::span<CSSSelector> CSSSelectorParser::ConsumeCompoundSelector(CSSParserTokenStream& stream, CSSNestingType nesting_type)
{
    ResetVectorAfterScope reset_vector(output_);
    wtf_size_t start_pos = output_.size();
    base::AutoReset<CSSSelector::PseudoType> reset_restricting(&restricting_pseudo_element_, restricting_pseudo_element_);
    base::AutoReset<bool> reset_found_host_in_compound(&found_host_in_compound_, false);
    ...
    base::AutoReset<bool> ignore_namespace(
        &ignore_default_namespace_, ignore_default_namespace_ || (resist_default_namespace_ && !has_q_name && AtEndIgnoringWhitespace(stream)));
...
bool CSSSelectorParser::ConsumeSimpleSelector(CSSParserTokenStream& stream)
{
    ...
    // video::-webkit-media-text-track-region-container.scrolling
    if (!ok || (context_->Mode() != kUASheetMode && !IsSimpleSelectorValidAfterPseudoElement(output_.back(), restricting_pseudo_element_))) {
        failed_parsing_ = true;
        return false;
    }
    return true;
...
bool CSSSelectorParser::ConsumeAttribute(CSSParserTokenStream& stream)
{
    ...
    QualifiedName qualified_name = namespace_prefix.IsNull() ? QualifiedName(attribute_name) : QualifiedName(namespace_prefix, attribute_name, namespace_uri);
    ...
    CSSSelector selector(match_type, qualified_name, case_sensitivity, attribute_value.Value().ToAtomicString());
    output_.push_back(std::move(selector));
    context_->Count(WebFeature::kHasIDClassTagAttribute);
    return true;
}
...
bool CSSSelectorParser::ConsumePseudo(CSSParserTokenStream& stream)
{
    ...
    selector.UpdatePseudoType(token.Value().ToAtomicString(), *context_, has_arguments, context_->Mode());
    ...
    case CSSSelector::kPseudoIs: {
        ...
        base::AutoReset<bool> is_inside_logical_combination_in_has_argument(&is_inside_logical_combination_in_has_argument_, is_inside_has_argument_);

        CSSSelectorList* selector_list = ConsumeForgivingNestedSelectorList(stream);
    ...
    case CSSSelector::kPseudoCue: {
        ...
        base::AutoReset<bool> ignore_namespace(&ignore_default_namespace_, ignore_default_namespace_ || selector.GetPseudoType() == CSSSelector::kPseudoCue);
    ...
    case CSSSelector::kPseudoHas: {
        ...
        base::AutoReset<bool> is_inside_has_argument(&is_inside_has_argument_, true);
        base::AutoReset<bool> found_pseudo_in_has_argument(&found_pseudo_in_has_argument_, false);
        base::AutoReset<bool> found_complex_logical_combinations_in_has_argument(&found_complex_logical_combinations_in_has_argument_, false);
    ...
    case CSSSelector::kPseudoState: {
        CHECK(selector.GetPseudoType() != CSSSelector::kPseudoState || RuntimeEnabledFeatures::CSSCustomStateNewSyntaxEnabled());
    ...
    case CSSSelector::kPseudoViewTransitionOld:
    case CSSSelector::kPseudoViewTransitionNew: {
        std::unique_ptr<Vector<AtomicString>> name_and_classes = std::make_unique<Vector<AtomicString>>();
    ...
    case CSSSelector::kPseudoSlotted: {
        ...
        {
            ResetVectorAfterScope reset_vector(output_);
            base::span<CSSSelector> inner_selector = ConsumeCompoundSelector(stream, CSSNestingType::kNone);
            stream.ConsumeWhitespace();
    ...
    case CSSSelector::kPseudoNthLastOfType: {
        ...
        if (selector.GetPseudoType() != CSSSelector::kPseudoNthChild && selector.GetPseudoType() != CSSSelector::kPseudoNthLastChild) {
            return false;
        }
...
CSSSelector::AttributeMatchType CSSSelectorParser::ConsumeAttributeFlags(CSSParserTokenStream& stream)
{
    ...
    } else if (EqualIgnoringASCIICase(flag.Value(), "s") && RuntimeEnabledFeatures::CSSCaseSensitiveSelectorEnabled()) {
...
CSSSelectorList* CSSSelectorParser::ConsumeNthChildOfSelectors(CSSParserTokenStream& stream)
{
    ...
    ResetVectorAfterScope reset_vector(output_);
    base::span<CSSSelector> selectors = ConsumeComplexSelectorList(stream, CSSNestingType::kNone);
    if (selectors.empty()) {
        return nullptr;
    }
...
void CSSSelectorParser::PrependTypeSelectorIfNeeded(
    const AtomicString& namespace_prefix, bool has_q_name, const AtomicString& element_name, wtf_size_t start_index_of_compound_selector)
{
    const CSSSelector& compound_selector = output_[start_index_of_compound_selector];

    if (!has_q_name && DefaultNamespace() == g_star_atom && !NeedsImplicitShadowCombinatorForMatching(compound_selector)) {
        return;
    }

    AtomicString determined_element_name = !has_q_name ? CSSSelector::UniversalSelectorAtom() : element_name;
    AtomicString namespace_uri = DetermineNamespace(namespace_prefix);
    if (namespace_uri.IsNull()) {
        failed_parsing_ = true;
        return;
    ...
    if (tag != AnyQName() || is_host_pseudo || NeedsImplicitShadowCombinatorForMatching(compound_selector)) {
        const bool is_implicit = determined_prefix == g_null_atom && determined_element_name == CSSSelector::UniversalSelectorAtom() && !is_host_pseudo;

        output_.insert(start_index_of_compound_selector, CSSSelector(tag, is_implicit));
    }
}
...
void CSSSelectorParser::SplitCompoundAtImplicitShadowCrossingCombinator(base::span<CSSSelector> selectors)
{
...
struct PseudoElementFeatureMapEntry {
    ...
    PseudoElementFeatureMapEntry(const char (&key)[key_length], WebFeature feature)
        : key(key)
...
WebFeature FeatureForWebKitCustomPseudoElement(const AtomicString& name)
{
    static const PseudoElementFeatureMapEntry feature_table[] = {
        ...
        { "-internal-media-controls-overlay-cast-button", WebFeature::kCSSSelectorInternalMediaControlsOverlayCastButton },
        { "-webkit-calendar-picker-indicator", WebFeature::kCSSSelectorWebkitCalendarPickerIndicator },
        { "-webkit-clear-button", WebFeature::kCSSSelectorWebkitClearButton },
        { "-webkit-color-swatch", WebFeature::kCSSSelectorWebkitColorSwatch },
        { "-webkit-color-swatch-wrapper", WebFeature::kCSSSelectorWebkitColorSwatchWrapper },
        { "-webkit-date-and-time-value", WebFeature::kCSSSelectorWebkitDateAndTimeValue },
        { "-webkit-datetime-edit", WebFeature::kCSSSelectorWebkitDatetimeEdit },
        { "-webkit-datetime-edit-ampm-field", WebFeature::kCSSSelectorWebkitDatetimeEditAmpmField },
        { "-webkit-datetime-edit-day-field", WebFeature::kCSSSelectorWebkitDatetimeEditDayField },
        { "-webkit-datetime-edit-fields-wrapper", WebFeature::kCSSSelectorWebkitDatetimeEditFieldsWrapper },
        { "-webkit-datetime-edit-hour-field", WebFeature::kCSSSelectorWebkitDatetimeEditHourField },
        { "-webkit-datetime-edit-millisecond-field", WebFeature::kCSSSelectorWebkitDatetimeEditMillisecondField },
        { "-webkit-datetime-edit-minute-field", WebFeature::kCSSSelectorWebkitDatetimeEditMinuteField },
        { "-webkit-datetime-edit-month-field", WebFeature::kCSSSelectorWebkitDatetimeEditMonthField },
        { "-webkit-datetime-edit-second-field", WebFeature::kCSSSelectorWebkitDatetimeEditSecondField },
        { "-webkit-datetime-edit-text", WebFeature::kCSSSelectorWebkitDatetimeEditText },
        { "-webkit-datetime-edit-week-field", WebFeature::kCSSSelectorWebkitDatetimeEditWeekField },
        { "-webkit-datetime-edit-year-field", WebFeature::kCSSSelectorWebkitDatetimeEditYearField },
        { "-webkit-file-upload-button", WebFeature::kCSSSelectorWebkitFileUploadButton },
        { "-webkit-inner-spin-button", WebFeature::kCSSSelectorWebkitInnerSpinButton },
        ...
        { "-webkit-media-controls-current-time-display", WebFeature::kCSSSelectorWebkitMediaControlsCurrentTimeDisplay },
        { "-webkit-media-controls-enclosure", WebFeature::kCSSSelectorWebkitMediaControlsEnclosure },
        { "-webkit-media-controls-fullscreen-button", WebFeature::kCSSSelectorWebkitMediaControlsFullscreenButton },
        { "-webkit-media-controls-mute-button", WebFeature::kCSSSelectorWebkitMediaControlsMuteButton },
        { "-webkit-media-controls-overlay-enclosure", WebFeature::kCSSSelectorWebkitMediaControlsOverlayEnclosure },
        { "-webkit-media-controls-overlay-play-button", WebFeature::kCSSSelectorWebkitMediaControlsOverlayPlayButton },
        { "-webkit-media-controls-panel", WebFeature::kCSSSelectorWebkitMediaControlsPanel },
        { "-webkit-media-controls-play-button", WebFeature::kCSSSelectorWebkitMediaControlsPlayButton },
        { "-webkit-media-controls-timeline", WebFeature::kCSSSelectorWebkitMediaControlsTimeline },
        // Note: This feature is no longer implemented in Blink.
        { "-webkit-media-controls-timeline-container", WebFeature::kCSSSelectorWebkitMediaControlsTimelineContainer },
        { "-webkit-media-controls-time-remaining-display", WebFeature::kCSSSelectorWebkitMediaControlsTimeRemainingDisplay },
        { "-webkit-media-controls-toggle-closed-captions-button", WebFeature::kCSSSelectorWebkitMediaControlsToggleClosedCaptionsButton },
        { "-webkit-media-controls-volume-slider", WebFeature::kCSSSelectorWebkitMediaControlsVolumeSlider },
        { "-webkit-media-slider-container", WebFeature::kCSSSelectorWebkitMediaSliderContainer },
        { "-webkit-media-slider-thumb", WebFeature::kCSSSelectorWebkitMediaSliderThumb },
        { "-webkit-media-text-track-container", WebFeature::kCSSSelectorWebkitMediaTextTrackContainer },
        { "-webkit-media-text-track-display", WebFeature::kCSSSelectorWebkitMediaTextTrackDisplay },
        { "-webkit-media-text-track-region", WebFeature::kCSSSelectorWebkitMediaTextTrackRegion },
        { "-webkit-media-text-track-region-container", WebFeature::kCSSSelectorWebkitMediaTextTrackRegionContainer },
        { "-webkit-meter-bar", WebFeature::kCSSSelectorWebkitMeterBar },
        { "-webkit-meter-even-less-good-value", WebFeature::kCSSSelectorWebkitMeterEvenLessGoodValue },
        { "-webkit-meter-inner-element", WebFeature::kCSSSelectorWebkitMeterInnerElement },
        { "-webkit-meter-optimum-value", WebFeature::kCSSSelectorWebkitMeterOptimumValue },
        { "-webkit-meter-suboptimum-value", WebFeature::kCSSSelectorWebkitMeterSuboptimumValue },
        { "-webkit-progress-bar", WebFeature::kCSSSelectorWebkitProgressBar },
        { "-webkit-progress-inner-element", WebFeature::kCSSSelectorWebkitProgressInnerElement },
        { "-webkit-progress-value", WebFeature::kCSSSelectorWebkitProgressValue },
        { "-webkit-search-cancel-button", WebFeature::kCSSSelectorWebkitSearchCancelButton },
        { "-webkit-slider-container", WebFeature::kCSSSelectorWebkitSliderContainer },
        { "-webkit-slider-runnable-track", WebFeature::kCSSSelectorWebkitSliderRunnableTrack },
        { "-webkit-slider-thumb", WebFeature::kCSSSelectorWebkitSliderThumb },
        { "-webkit-textfield-decoration-container", WebFeature::kCSSSelectorWebkitTextfieldDecorationContainer },
    };
...
static void RecordUsageAndDeprecationsOneSelector(const CSSSelector* selector, const CSSParserContext* context, bool* has_visited_pseudo)
{
    ...
    if (selector->SelectorList()) {
        for (const CSSSelector* current = selector->SelectorList()->First(); current; current = current->NextSimpleSelector()) {
            RecordUsageAndDeprecationsOneSelector(current, context, has_visited_pseudo);
        }
    }
}

void CSSSelectorParser::RecordUsageAndDeprecations(const base::span<CSSSelector> selector_vector, bool* has_visited_pseudo)
{
    ...
    for (const CSSSelector& current : selector_vector) {
        RecordUsageAndDeprecationsOneSelector(&current, context_, has_visited_pseudo);
    }
}

bool CSSSelectorParser::ContainsUnknownWebkitPseudoElements(base::span<CSSSelector> selectors)
{
