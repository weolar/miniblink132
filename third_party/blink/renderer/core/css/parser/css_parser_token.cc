...
CSSParserToken::CSSParserToken(CSSParserTokenType type, double numeric_value, NumericValueType numeric_value_type, NumericSign sign)
    : type_(type)
    , block_type_(kNotBlock)
    ...
    , value_is_inline_(false)
{
    ...
    numeric_value_ = ClampTo<double>(numeric_value, -std::numeric_limits<float>::max(), std::numeric_limits<float>::max());
}
...
{
    DCHECK(type_ == kNumberToken || type_ == kPercentageToken || type_ == kDimensionToken);
...
CSSPropertyID CSSParserToken::ParseAsUnresolvedCSSPropertyID(const ExecutionContext* execution_context, CSSParserMode mode) const
{
...
bool CSSParserToken::HasStringBacking() const
{
    ...
    return token_type == kIdentToken || token_type == kFunctionToken || token_type == kAtKeywordToken || token_type == kHashToken || token_type == kUrlToken
        || token_type == kDimensionToken || token_type == kStringToken;
}
...
bool CSSParserToken::ValueDataCharRawEqual(const CSSParserToken& other) const
{
    ...
    if (ValueDataCharRaw() == other.ValueDataCharRaw() && value_is_8bit_ == other.value_is_8bit_) {
        return true;
    }

    if (value_is_8bit_) {
        return other.value_is_8bit_ ? Equal(static_cast<const LChar*>(ValueDataCharRaw()), static_cast<const LChar*>(other.ValueDataCharRaw()), value_length_)
                                    : Equal(static_cast<const LChar*>(ValueDataCharRaw()), static_cast<const UChar*>(other.ValueDataCharRaw()), value_length_);
    } else {
        return other.value_is_8bit_ ? Equal(static_cast<const UChar*>(ValueDataCharRaw()), static_cast<const LChar*>(other.ValueDataCharRaw()), value_length_)
                                    : Equal(static_cast<const UChar*>(ValueDataCharRaw()), static_cast<const UChar*>(other.ValueDataCharRaw()), value_length_);
    }
}
...
bool CSSParserToken::operator==(const CSSParserToken& other) const
{
    ...
    case kPercentageToken:
        return numeric_sign_ == other.numeric_sign_ && numeric_value_ == other.numeric_value_ && numeric_value_type_ == other.numeric_value_type_;
...
void CSSParserToken::Serialize(StringBuilder& builder) const
{
    ...
    case kHashToken:
        builder.Append('#');
        SerializeIdentifier(Value().ToString(), builder, (GetHashTokenType() == kHashTokenUnrestricted));
        break;
    case kUrlToken:
        builder.Append("url(");
        SerializeIdentifier(Value().ToString(), builder);
...
bool NeedsInsertedComment(const CSSParserToken& a, const CSSParserToken& b)
{
    ...
    if (at == kIdentToken || at == kAtKeywordToken || at == kHashToken || at == kDimensionToken || at == kNumberToken
        ...
        return bt == kIdentToken || bt == kFunctionToken || bt == kUrlToken || bt == kBadUrlToken || bt == kNumberToken || bt == kPercentageToken
            || bt == kDimensionToken || bt == kCDCToken || (bt == kDelimiterToken && b.Delimiter() == '-');
    }
