...
ShadowTreeStyleSheetCollection::ShadowTreeStyleSheetCollection(ShadowRoot& shadow_root)
...
void ShadowTreeStyleSheetCollection::CollectStyleSheets(StyleEngine& engine, StyleSheetCollection& collection)
{
    ...
    for (Node* n : style_sheet_candidate_nodes_) {
        ...
        if (candidate.CanBeActivated(g_null_atom)) {
            ...
            collection.AppendActiveStyleSheet(std::make_pair(css_sheet, rule_set_scope.RuleSetForSheet(engine, css_sheet)));
        }
    }
    ...
    for (CSSStyleSheet* sheet : *tree_scope.AdoptedStyleSheets()) {
        ...
        collection.AppendActiveStyleSheet(std::make_pair(sheet, engine.RuleSetForSheet(*sheet)));
    }
}
