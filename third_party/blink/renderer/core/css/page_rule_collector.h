...
public:
    PageRuleCollector(const ComputedStyle* root_element_style, CSSAtRuleID at_rule_id, uint32_t page_index, const AtomicString& page_name, MatchResult&);

    // TreeScope is required for CascadeOrigin::kAuthor,
    // and ignored for other origins.
    void MatchPageRules(RuleSet* rules, CascadeOrigin, TreeScope*, const CascadeLayerMap* layer_map);
...
private:
    bool IsLeftPage(const ComputedStyle* root_element_style, uint32_t page_index) const;
    bool IsRightPage(const ComputedStyle* root_element_style, uint32_t page_index) const
    {
        return !IsLeftPage(root_element_style, page_index);
    }
    bool IsFirstPage(uint32_t page_index) const;

    void MatchPageRulesForList(HeapVector<Member<StyleRulePage>>& matched_rules, const HeapVector<Member<StyleRulePage>>& rules);
