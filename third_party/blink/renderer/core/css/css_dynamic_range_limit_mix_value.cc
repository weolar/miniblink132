...
bool CSSDynamicRangeLimitMixValue::Equals(const CSSDynamicRangeLimitMixValue& other) const
{
    ...
    for (size_t i = 0; i < limits_.size(); ++i) {
        if (!base::ValuesEquivalent(limits_[i], other.limits_[i]) || !base::ValuesEquivalent(percentages_[i], other.percentages_[i])) {
            return false;
        }
    }
    return true;
...
void CSSDynamicRangeLimitMixValue::TraceAfterDispatch(blink::Visitor* visitor) const
{
