...
StyleScope* StyleScope::Parse(CSSParserTokenStream& stream, const CSSParserContext* context, CSSNestingType nesting_type, StyleRule* parent_rule_for_nesting,
    bool is_within_scope, StyleSheetContents* style_sheet)
{
    ...
    if (stream.Peek().GetType() == kLeftParenthesisToken) {
        ...
        stream.ConsumeWhitespace();
        from = CSSSelectorParser::ParseScopeBoundary(stream, context, nesting_type, parent_rule_for_nesting, is_within_scope, style_sheet, arena);
        if (from.empty()) {
            return nullptr;
        }
    }
    ...
    if (!from.empty()) {
        auto* properties = ImmutableCSSPropertyValueSet::Create(base::span<CSSPropertyValue>(), CSSParserMode::kHTMLStandardMode);
    ...
    CSSSelectorList* to_list = !to.empty() ? CSSSelectorList::AdoptSelectorVector(to) : nullptr;
