...
public:
    ...
    const CounterStyle& GetCounterStyle(const TreeScope& scope, const char* name)
    {
        return *CounterStyleMap::GetAuthorCounterStyleMap(scope)->counter_styles_.at(AtomicString(name));
    }
};
...
TEST_F(CounterStyleMapTest, ExtendsParentScopeStyle)
{
    ...
    shadow.setInnerHTML("<style>@counter-style bar { system: extends foo; }</style>");
    UpdateAllLifecyclePhasesForTest();
...
TEST_F(CounterStyleMapTest, SpeakAsKeywords)
{
    ...
    const CounterStyle& implicit_auto = GetCounterStyle(GetDocument(), "implicit-auto");
    EXPECT_EQ(CounterStyleSpeakAs::kAuto, implicit_auto.GetSpeakAs());

    const CounterStyle& explicit_auto = GetCounterStyle(GetDocument(), "explicit-auto");
...
TEST_F(CounterStyleMapTest, SpeakAsReference)
{
    ...
    const CounterStyle& valid_author_ref = GetCounterStyle(GetDocument(), "valid-author-ref");
    ...
    const CounterStyle& valid_ua_ref = GetCounterStyle(GetDocument(), "valid-ua-ref");
