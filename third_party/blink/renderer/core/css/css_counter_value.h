...
public:
    ...
    {
        return list_style_->GetTreeScope();
    }
    ...
    bool Equals(const CSSCounterValue& other) const
    {
        return Identifier() == other.Identifier() && ListStyle() == other.ListStyle() && Separator() == other.Separator()
            && IsScopedValue() == other.IsScopedValue() && GetTreeScope() == other.GetTreeScope();
    }
...
template <> struct DowncastTraits<cssvalue::CSSCounterValue> {
