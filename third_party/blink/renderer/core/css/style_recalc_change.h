...
private:
    ...
    static const Flags kRecalcSizeContainerFlags = kRecalcSizeContainer | kRecalcDescendantSizeContainers;

    static const Flags kRecalcStyleContainerFlags = kRecalcStyleContainerChildren | kRecalcStyleContainerDescendants;

    static const Flags kRecalcScrollStateContainerFlags = kRecalcScrollStateContainer | kRecalcDescendantScrollStateContainers;

    static const Flags kRecalcContainerFlags = kRecalcSizeContainerFlags | kRecalcStyleContainerFlags | kRecalcScrollStateContainerFlags;
...
public:
    ...
    explicit StyleRecalcChange(Propagate propagate)
    ...
    {
        return { RecalcDescendants() ? kRecalcDescendants : kNo, FlagsForChildren(element) };
    }
    ...
    {
        return { propagate_, static_cast<Flags>(flags_ | kRecalcDescendantSizeContainers) };
    }
    StyleRecalcChange ForceRecalcStyleContainerChildren() const
    {
        return { propagate_, static_cast<Flags>(flags_ | kRecalcStyleContainerChildren) };
    }
    StyleRecalcChange ForceRecalcStyleContainerDescendants() const
    {
        return { propagate_, static_cast<Flags>(flags_ | kRecalcStyleContainerDescendants) };
    }
    StyleRecalcChange ForceRecalcScrollStateContainer() const
    {
        return { propagate_, static_cast<Flags>(flags_ | kRecalcScrollStateContainer) };
    }
    StyleRecalcChange ForceRecalcDescendantScrollStateContainers() const
    {
        return { propagate_, static_cast<Flags>(flags_ | kRecalcDescendantScrollStateContainers) };
    }
    ...
    {
        return { std::max(propagate_, other.propagate_), static_cast<Flags>(flags_ | other.flags_) };
    }

    bool ReattachLayoutTree() const
    {
        return flags_ & kReattach;
    }
    bool MarkReattachLayoutTree() const
    {
        // Never mark the query container (kSuppressRecalc) for reattachment.
        return (flags_ & (kMarkReattach | kReattach | kSuppressRecalc)) == (kMarkReattach | kReattach);
    }
    bool RecalcChildren() const
    {
        return propagate_ > kUpdatePseudoElements;
    }
    bool RecalcDescendants() const
    {
        return propagate_ == kRecalcDescendants;
    }
    bool RecalcContainerQueryDependent(const Node&) const;
    bool UpdatePseudoElements() const
    {
        return propagate_ != kNo;
    }
    ...
    bool RemUnitsMaybeChanged() const
    {
        return RecalcDescendants();
    }
