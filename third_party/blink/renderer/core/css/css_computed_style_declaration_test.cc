...
TEST_F(CSSComputedStyleDeclarationTest, CleanAncestorsNoRecalc)
{
    ...
    GetElementById("dirty")->setAttribute(html_names::kStyleAttr, AtomicString("color:pink"));
    EXPECT_TRUE(GetDocument().NeedsLayoutTreeUpdate());

    Element* target = GetDocument().getElementById(AtomicString("target"));
    auto* computed = MakeGarbageCollected<CSSComputedStyleDeclaration>(target);

    EXPECT_EQ("rgb(0, 128, 0)", computed->GetPropertyValue(CSSPropertyID::kColor));
    EXPECT_TRUE(GetDocument().NeedsLayoutTreeUpdate());
}
...
TEST_F(CSSComputedStyleDeclarationTest, AdjacentInvalidation)
{
    ...
    EXPECT_EQ("rgb(255, 0, 0)", computed->GetPropertyValue(CSSPropertyID::kColor));
...
TEST_F(CSSComputedStyleDeclarationTest, NoCrashWhenCallingGetPropertyCSSValueWithVariable)
{
    UpdateAllLifecyclePhasesForTest();
    Element* target = GetDocument().body();
    auto* computed = MakeGarbageCollected<CSSComputedStyleDeclaration>(target);
    ASSERT_TRUE(computed);
    const CSSValue* result = computed->GetPropertyCSSValue(CSSPropertyID::kVariable);
    EXPECT_FALSE(result);
    // Don't crash.
}
...
TEST_F(CSSComputedStyleDeclarationTest, UseCountDurationZero)
{
    ...
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kCSSGetComputedAnimationDurationZero));
    EXPECT_TRUE(style->GetPropertyCSSValue(CSSPropertyID::kWebkitFontSmoothing));
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kCSSGetComputedWebkitFontSmoothingAnimationDurationZero));
    ...
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kCSSGetComputedAnimationDurationZero));
    EXPECT_TRUE(style->GetPropertyCSSValue(CSSPropertyID::kAnimationDuration));
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kCSSGetComputedAnimationDurationZero));

    // Font smoothing count should remain uncounted until we retrieve the computed
    // value.
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kCSSGetComputedWebkitFontSmoothingAnimationDurationZero));
    EXPECT_TRUE(style->GetPropertyCSSValue(CSSPropertyID::kWebkitFontSmoothing));
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kCSSGetComputedWebkitFontSmoothingAnimationDurationZero));
}
