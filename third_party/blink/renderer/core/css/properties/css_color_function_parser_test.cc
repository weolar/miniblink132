...
TEST(ColorFunctionParserTest, RelativeColorWithKeywordBase_LateResolveEnabled)
{
    ...
    const CSSValue* result = parser.ConsumeFunctionalSyntaxColor(stream, *context);
    EXPECT_TRUE(result->IsRelativeColorValue());
    const cssvalue::CSSRelativeColorValue* color = To<cssvalue::CSSRelativeColorValue>(result);
...
TEST(ColorFunctionParserTest, RelativeColorWithKeywordBase_LateResolveDisabled)
{
    ...
    EXPECT_EQ(color->Value(), Color::FromColorSpace(Color::ColorSpace::kSRGB, 1, 0, 0));
}
...
{
    ScopedCSSRelativeColorSupportsCurrentcolorForTest scoped_feature_for_test(false);
...
{
    ScopedCSSRelativeColorSupportsCurrentcolorForTest scoped_feature_for_test(true);
    ...
    const CSSValue& origin = color->OriginColor();
    EXPECT_TRUE(origin.IsIdentifierValue());
    EXPECT_EQ(To<CSSIdentifierValue>(origin).GetValueID(), CSSValueID::kCurrentcolor);
...
TEST(ColorFunctionParserTest, RelativeColorWithCurrentcolorBase_CalcAlpha)
{
    ...
    const String test_case = "rgb(from currentcolor 1 calc(g) b / calc(alpha / 2))";
    CSSParserTokenStream stream(test_case);
...
TEST(ColorFunctionParserTest, RelativeColorWithColorMixWithCurrentColorBase)
{
    ...
    const String test_case = "rgb(from color-mix(in srgb, currentColor 50%, green) r g b)";
    CSSParserTokenStream stream(test_case);
    ...
    const cssvalue::CSSColorMixValue& origin_color = To<cssvalue::CSSColorMixValue>(origin);
    EXPECT_TRUE(origin_color.Color1().IsIdentifierValue());
    EXPECT_EQ(To<CSSIdentifierValue>(origin_color.Color1()).GetValueID(), CSSValueID::kCurrentcolor);
    EXPECT_TRUE(origin_color.Color2().IsIdentifierValue());
    EXPECT_EQ(To<CSSIdentifierValue>(origin_color.Color2()).GetValueID(), CSSValueID::kGreen);
}
