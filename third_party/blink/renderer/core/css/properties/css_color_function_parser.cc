...
std::optional<Color> TryResolveAtParseTime(const CSSValue& value)
{
    ...
    if (auto* identifier_value = DynamicTo<CSSIdentifierValue>(value)) {
        ...
        if ((value_id >= CSSValueID::kAqua && value_id <= CSSValueID::kYellow) || (value_id >= CSSValueID::kAliceblue && value_id <= CSSValueID::kYellowgreen)
            ...
            // color provider.
            return StyleColor::ColorFromKeyword(value_id, mojom::blink::ColorScheme::kLight, nullptr,
    ...
    if (auto* color_mix_value = DynamicTo<cssvalue::CSSColorMixValue>(value)) {
        ...
        if ((!color_mix_value->Percentage1() || color_mix_value->Percentage1()->IsNumericLiteralValue())
            && (!color_mix_value->Percentage2() || color_mix_value->Percentage2()->IsNumericLiteralValue())) {
            return color_mix_value->Mix(*color1, *color2, CSSToLengthConversionData());
        }
    }
    if (auto* relative_color_value = DynamicTo<cssvalue::CSSRelativeColorValue>(value)) {
        auto origin_color = TryResolveAtParseTime(relative_color_value->OriginColor());
        if (!origin_color) {
            return std::nullopt;
        }
        StyleColor::UnresolvedRelativeColor* unresolved_relative_color
            = MakeGarbageCollected<StyleColor::UnresolvedRelativeColor>(StyleColor(origin_color.value()), relative_color_value->ColorInterpolationSpace(),
...
CSSValue* ConsumeRelativeColorChannel(CSSParserTokenStream& stream, const CSSParserContext& context, const CSSColorChannelMap& color_channel_map,
    CalculationResultCategorySet expected_categories, const double percentage_base = 0)
{
    ...
    if (token.GetType() == kFunctionToken) {
        ...
        CSSMathFunctionValue* calc_value = CSSMathFunctionValue::Create(
            CSSMathExpressionNode::ParseMathFunction(token.FunctionId(), stream, context, Flags({ AllowPercent }), kCSSAnchorQueryTypesNone, color_channel_map),
...
bool IsInGamutRec2020(Color color)
{
    ...
    return -kEpsilon <= color.Param0() && color.Param0() <= 1.f + kEpsilon && -kEpsilon <= color.Param1() && color.Param1() <= 1.f + kEpsilon
        && -kEpsilon <= color.Param2() && color.Param2() <= 1.f + kEpsilon;
}

} // namespace

bool ColorFunctionParser::ConsumeColorSpaceAndOriginColor(CSSParserTokenStream& stream, CSSValueID function_id, const CSSParserContext& context)
{
    // [from <color>]?
    ...
    if (function_id == CSSValueID::kColor) {
        ...
        color_space_ = ColorSpaceFromColorSpaceArgument(stream.ConsumeIncludingWhitespace().Id());
    ...
    if (unresolved_origin_color_) {
        ...
        if (origin_color_.has_value() && !RuntimeEnabledFeatures::CSSRelativeColorLateResolveAlwaysEnabled()) {
            ...
            if (!origin_color_.has_value() && !RuntimeEnabledFeatures::CSSRelativeColorSupportsCurrentcolorEnabled()) {
                return false;
            }
...
bool ColorFunctionParser::ConsumeChannel(CSSParserTokenStream& stream, const CSSParserContext& context, int i)
{
    ...
    if (ColorChannelIsHue(color_space_, i)) {
        if ((unresolved_channels_[i] = css_parsing_utils::ConsumeAngle(stream, context, std::nullopt))) {
            channel_types_[i] = ChannelType::kNumber;
        } else if ((unresolved_channels_[i] = css_parsing_utils::ConsumeNumber(stream, context, CSSPrimitiveValue::ValueRange::kAll))) {
            channel_types_[i] = ChannelType::kNumber;
        } else if (IsRelativeColor()) {
            if ((unresolved_channels_[i] = ConsumeRelativeColorChannel(stream, context, color_channel_map_, { kCalcNumber, kCalcAngle }))) {
    ...
    if ((unresolved_channels_[i] = css_parsing_utils::ConsumeNumber(stream, context, CSSPrimitiveValue::ValueRange::kAll))) {
        channel_types_[i] = ChannelType::kNumber;
        return true;
    }

    if ((unresolved_channels_[i] = css_parsing_utils::ConsumePercent(stream, context, CSSPrimitiveValue::ValueRange::kAll))) {
    ...
    if (IsRelativeColor()) {
        channel_types_[i] = ChannelType::kRelative;
        if ((unresolved_channels_[i]
                = ConsumeRelativeColorChannel(stream, context, color_channel_map_, { kCalcNumber, kCalcPercent }, function_metadata_->channel_percentage[i]))) {
            return true;
        }
    }
...
bool ColorFunctionParser::ConsumeAlpha(CSSParserTokenStream& stream, const CSSParserContext& context)
{
    if ((unresolved_alpha_ = css_parsing_utils::ConsumeNumber(stream, context, CSSPrimitiveValue::ValueRange::kAll))) {
        alpha_channel_type_ = ChannelType::kNumber;
        return true;
    }

    if ((unresolved_alpha_ = css_parsing_utils::ConsumePercent(stream, context, CSSPrimitiveValue::ValueRange::kAll))) {
    ...
    if (IsRelativeColor() && (unresolved_alpha_ = ConsumeRelativeColorChannel(stream, context, color_channel_map_, { kCalcNumber, kCalcPercent }, 1.0))) {
...
double ColorFunctionParser::ResolveColorChannel(
    const CSSValue* value, ChannelType channel_type, double percentage_base, const CSSColorChannelMap& color_channel_map)
{
    if (const CSSPrimitiveValue* primitive_value = DynamicTo<CSSPrimitiveValue>(value)) {
        switch (channel_type) {
    ...
    return ResolveRelativeChannelValue(value, channel_type, percentage_base, color_channel_map);
}

double ColorFunctionParser::ResolveAlpha(const CSSValue* value, ChannelType channel_type, const CSSColorChannelMap& color_channel_map)
{
    if (const CSSPrimitiveValue* primitive_value = DynamicTo<CSSPrimitiveValue>(value)) {
        switch (channel_type) {
        case ChannelType::kNumber:
    ...
    return ResolveRelativeChannelValue(value, channel_type, /*percentage_base=*/1.0, color_channel_map);
}

double ColorFunctionParser::ResolveRelativeChannelValue(
    const CSSValue* value, ChannelType channel_type, double percentage_base, const CSSColorChannelMap& color_channel_map)
{
    if (const CSSIdentifierValue* identifier_value = DynamicTo<CSSIdentifierValue>(value)) {
        // This is for just single variable swaps without calc(). e.g. The "l" in
        // "lab(from cyan l 0.5 0.5)".
    ...
    if (const CSSMathFunctionValue* calc_value = DynamicTo<CSSMathFunctionValue>(value)) {
...
CSSValue* ColorFunctionParser::ConsumeFunctionalSyntaxColor(CSSParserTokenStream& stream, const CSSParserContext& context)
{
    ...
    bool has_alpha = false;
    {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (i < 2) {
                const bool matched_comma = css_parsing_utils::ConsumeCommaIncludingWhitespace(stream);
                if (is_legacy_syntax_) {
    ...
    if (!IsRelativeColor() || (origin_color_.has_value() && !RuntimeEnabledFeatures::CSSRelativeColorLateResolveAlwaysEnabled())) {
        // Resolve channel values.
        for (int i = 0; i < 3; i++) {
            if (channel_types_[i] != ChannelType::kNone) {
                channels_[i] = ResolveColorChannel(unresolved_channels_[i], channel_types_[i], function_metadata_->channel_percentage[i], color_channel_map_);
        ...
        if (has_alpha) {
            if (alpha_channel_type_ != ChannelType::kNone) {
                alpha_ = ResolveAlpha(unresolved_alpha_, alpha_channel_type_, color_channel_map_);
            } else {
                alpha_.reset();
            }
        } else if (IsRelativeColor()) {
        ...
        resolved_color = Color::FromColorSpace(color_space_, channels_[0], channels_[1], channels_[2], alpha_);
    ...
    if (resolved_color.has_value()) {
        ...
        return MakeGarbageCollected<cssvalue::CSSRelativeColorValue>(
            *unresolved_origin_color_, color_space_, *unresolved_channels_[0], *unresolved_channels_[1], *unresolved_channels_[2], unresolved_alpha_);
    }
}
