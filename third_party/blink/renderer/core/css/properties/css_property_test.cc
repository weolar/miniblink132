...
public:
    ...
    {
        return (required_mode_ == GetMode()) ? std::optional<LayoutUnit>(1) : std::optional<LayoutUnit>();
    }
    ...
    std::optional<PhysicalOffset> ComputeAnchorCenterOffsets(const ComputedStyleBuilder& builder) override
    {
        return std::nullopt;
    }
...
public:
    ...
    {
        const CSSPropertyValueSet* set = css_test_helpers::ParseDeclarationBlock(name + ":" + value);
        DCHECK(set);
    ...
    String ComputedValue(String property_str, String value_str, StyleRecalcContext style_recalc_context)
    {
        CSSPropertyRef ref(property_str, GetDocument());
        CHECK(ref.IsValid());
        const CSSProperty& property = ref.GetProperty();

        const CSSValue* value = css_test_helpers::ParseLonghand(GetDocument(), property, value_str);
        CHECK(value);
        ...
        StyleResolverState state(GetDocument(), *GetDocument().body(), &style_recalc_context);
        ...
        const CSSValue* computed_value = property.CSSValueFromComputedStyle(*style,
...
TEST_F(CSSPropertyTest, VisitedPropertiesAreNotWebExposed)
{
    for (CSSPropertyID property_id : CSSPropertyIDList()) {
        ...
        EXPECT_TRUE(!property.IsVisited() || !property.IsWebExposed(GetDocument().GetExecutionContext()));
    }
}
...
TEST_F(CSSPropertyTest, GetUnvisitedPropertyFromVisited)
{
    for (CSSPropertyID property_id : CSSPropertyIDList()) {
        const CSSProperty& property = CSSProperty::Get(property_id);
        EXPECT_EQ(property.IsVisited(), static_cast<bool>(property.GetUnvisitedProperty()));
    }
}

TEST_F(CSSPropertyTest, InternalFontSizeDeltaNotWebExposed)
{
    ASSERT_FALSE(CSSProperty::Get(CSSPropertyID::kInternalFontSizeDelta).IsWebExposed());
}

TEST_F(CSSPropertyTest, VisitedPropertiesCanParseValues)
{
    const ComputedStyle& initial_style = GetDocument().GetStyleResolver().InitialStyle();
    ...
    for (CSSPropertyID property_id : CSSPropertyIDList()) {
        ...
        const CSSValue* initial_value
            = property.CSSValueFromComputedStyle(initial_style, nullptr /* layout_object */, false /* allow_visited_style */, CSSValuePhase::kComputedValue);
        ASSERT_TRUE(initial_value);

        // Parse the initial value using both the regular property, and the
        // accompanying 'visited' property.
        const CSSValue* parsed_regular_value = css_test_helpers::ParseLonghand(GetDocument(), property, initial_value->CssText());
        const CSSValue* parsed_visited_value = css_test_helpers::ParseLonghand(GetDocument(), *visited, initial_value->CssText());

        // The properties should have identical parsing behavior.
        EXPECT_TRUE(base::ValuesEquivalent(parsed_regular_value, parsed_visited_value));
...
TEST_F(CSSPropertyTest, Surrogates)
{
    ...
    const WritingDirectionMode kHorizontalLtr = { WritingMode::kHorizontalTb, TextDirection::kLtr };
    EXPECT_EQ(&GetCSSPropertyWidth(), inline_size.SurrogateFor(kHorizontalLtr));
    EXPECT_EQ(&GetCSSPropertyHeight(), inline_size.SurrogateFor({ WritingMode::kVerticalRl, TextDirection::kLtr }));
    EXPECT_EQ(&GetCSSPropertyWritingMode(), writing_mode.SurrogateFor(kHorizontalLtr));
    EXPECT_FALSE(GetCSSPropertyWidth().SurrogateFor(kHorizontalLtr));
}

TEST_F(CSSPropertyTest, PairsWithIdenticalValues)
{
    const CSSValue* border_radius = css_test_helpers::ParseLonghand(GetDocument(), GetCSSPropertyBorderTopLeftRadius(), "1% 1%");
    const CSSValue* perspective_origin = css_test_helpers::ParseLonghand(GetDocument(), GetCSSPropertyPerspectiveOrigin(), "1% 1%");
...
TEST_F(CSSPropertyTest, AlternativePropertyData)
{
    for (CSSPropertyID property_id : CSSPropertyIDList()) {
        ...
        // in this test.
        if (CSSPropertyID alternative_id = property.GetAlternative(); alternative_id != CSSPropertyID::kInvalid && !IsPropertyAlias(alternative_id)) {
            ...
            // property.
            EXPECT_EQ(property.GetPropertyNameAtomicString(), alternative.GetPropertyNameAtomicString());
            EXPECT_EQ(property.GetPropertyNameString(), alternative.GetPropertyNameString());
            EXPECT_EQ(std::strcmp(property.GetPropertyName(), alternative.GetPropertyName()), 0);
            EXPECT_EQ(std::strcmp(property.GetJSPropertyName(), alternative.GetJSPropertyName()), 0);
...
TEST_F(CSSPropertyTest, AlternativePropertyExposure)
{
    for (CSSPropertyID property_id : CSSPropertyIDList()) {
        ...
        if (CSSPropertyID alternative_id = ResolveCSSPropertyID(property.GetAlternative()); alternative_id != CSSPropertyID::kInvalid) {
            SCOPED_TRACE(property.GetPropertyName());
...
TEST_F(CSSPropertyTest, AlternativePropertyCycle)
{
    for (CSSPropertyID property_id : CSSPropertyIDList()) {
        ...
        CSSBitset seen_properties;
        for (CSSPropertyID current_id = property_id; current_id != CSSPropertyID::kInvalid;
             // TODO(pdr): Remove this call to `ResolveCSSPropertyID` by properly
             // handling aliases in this test.
             current_id = ResolveCSSPropertyID(CSSProperty::Get(current_id).GetAlternative())) {
...
TEST_F(CSSPropertyTest, AnchorModeTop)
{
    ...
    EXPECT_EQ("0px", ComputedValue("min-width", "anchor-size(width, 0px)", context));
    EXPECT_EQ("0px", ComputedValue("min-height", "anchor-size(width, 0px)", context));
    EXPECT_EQ("0px", ComputedValue("max-width", "anchor-size(width, 0px)", context));
    EXPECT_EQ("0px", ComputedValue("max-height", "anchor-size(width, 0px)", context));
}
...
TEST_F(CSSPropertyTest, AnchorModeWidth)
{
    ...
    EXPECT_EQ("0px", ComputedValue("height", "anchor-size(width, 0px)", context));
    EXPECT_EQ("1px", ComputedValue("min-width", "anchor-size(width, 0px)", context));
    EXPECT_EQ("0px", ComputedValue("min-height", "anchor-size(width, 0px)", context));
    EXPECT_EQ("1px", ComputedValue("max-width", "anchor-size(width, 0px)", context));
    EXPECT_EQ("0px", ComputedValue("max-height", "anchor-size(width, 0px)", context));
}
...
TEST_F(CSSPropertyTest, AnchorModeHeight)
{
    ...
    EXPECT_EQ("0px", ComputedValue("min-width", "anchor-size(width, 0px)", context));
    EXPECT_EQ("1px", ComputedValue("min-height", "anchor-size(width, 0px)", context));
    EXPECT_EQ("0px", ComputedValue("max-width", "anchor-size(width, 0px)", context));
    EXPECT_EQ("1px", ComputedValue("max-height", "anchor-size(width, 0px)", context));
}
