...
template <size_t kBits> class CORE_EXPORT CSSBitsetBase {
public:
    ...
    CSSBitsetBase(const CSSBitsetBase<kBits>& o)
    {
        *this = o;
    }
    ...
    inline void Reset()
    {
        std::memset(chunks_.data(), 0, sizeof(chunks_));
    }
    ...
    public:
        ...
        Iterator(const uint64_t* chunks, FirstNonHighPriorityTag)
            : chunks_(chunks)
            , chunk_(chunks_[0] & ~HighPriorityBitMask())
        {
            ++*this; // Go to the first set bit.
        }
    ...
    Iterator end() const
    {
        return Iterator(chunks_.data(), kChunks, kBits);
    }
...
private:
    ...
    template <int N> static constexpr std::array<uint64_t, kChunks> CreateChunks(const CSSPropertyID (&list)[N])
    {
