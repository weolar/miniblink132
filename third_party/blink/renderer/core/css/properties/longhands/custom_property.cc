...
CustomProperty::CustomProperty(AtomicString name, const Document& document)
    : CustomProperty(PropertyRegistration::From(document.GetExecutionContext(), name))
{
...
CustomProperty::CustomProperty(const AtomicString& name, const PropertyRegistry* registry)
    : CustomProperty(name, registry ? registry->Registration(name) : nullptr)
{
}

CustomProperty::CustomProperty(const AtomicString& name, const PropertyRegistration* registration)
...
void CustomProperty::ApplyInherit(StyleResolverState& state) const
{
    ...
    CSSVariableData* parent_data = state.ParentStyle()->GetVariableData(name_, is_inherited_property);
...
void CustomProperty::ApplyValue(StyleResolverState& state, const CSSValue& value, ValueMode value_mode) const
{
    ...
    const CSSParserContext* context = declaration ? declaration->ParserContext() : nullptr;
    ...
    if (!context) {
        ...
        context = StrictCSSParserContext(state.GetDocument().GetExecutionContext()->GetSecureContextMode());
    }
    ...
    if (!registered_value) {
        DCHECK(declaration);
        CSSVariableData& data = *declaration->VariableDataValue();
        registered_value = Parse(data.OriginalText(), *context, CSSParserLocalContext());
    }
    ...
    registered_value = &StyleBuilderConverter::ConvertRegisteredPropertyValue(state, *registered_value, context);
    CSSVariableData* data = StyleBuilderConverter::ConvertRegisteredPropertyVariableData(*registered_value, is_animation_tainted);
    builder.SetVariableData(name_, data, is_inherited_property);
    builder.SetVariableValue(name_, registered_value->UntaintedCopy(), is_inherited_property);
}
    ...
    const ComputedStyle& style, const LayoutObject*, bool allow_visited_style, CSSValuePhase value_phase) const
{
    ...
    return MakeGarbageCollected<CSSUnparsedDeclarationValue>(data, /* parser_context */ nullptr);
}

const CSSValue* CustomProperty::ParseUntyped(StringView text, const CSSParserContext& context, const CSSParserLocalContext& local_context) const
{
    return CSSVariableParser::ParseDeclarationValue(text, local_context.IsAnimationTainted(), context);
}

const CSSValue* CustomProperty::Parse(StringView text, const CSSParserContext& context, const CSSParserLocalContext& local_context) const
{
    if (!registration_) {
        return ParseUntyped(text, context, local_context);
    }
    return registration_->Syntax().Parse(text, context, local_context.IsAnimationTainted());
}
