...
public:
    ...
    {
        GetDocument().body()->setInnerHTML("<div id='target' style='" + value + "'></div>");
    ...
    const CSSValue* GetComputedValue(const CustomProperty& property)
    {
        return property.CSSValueFromComputedStyle(
            GetComputedStyle(), nullptr /* layout_object */, false /* allow_visited_style */, CSSValuePhase::kComputedValue);
    }

    const CSSValue* ParseValue(const CustomProperty& property, const String& value, const CSSParserLocalContext& local_context)
    {
...
TEST_F(CustomPropertyTest, ComputedCSSValueInherited)
{
    ...
    EXPECT_EQ(100, primitive_value->ComputeLength<double>(CSSToLengthConversionData()));
}
...
{
    CustomProperty property(AtomicString("--x"), GetDocument());
    const CSSValue* value = ParseValue(property, "100px", CSSParserLocalContext());
...
{
    CustomProperty property(AtomicString("--x"), GetDocument());
    const CSSValue* value1 = ParseValue(property, "100px", CSSParserLocalContext().WithAnimationTainted(true));
    const CSSValue* value2 = ParseValue(property, "100px", CSSParserLocalContext().WithAnimationTainted(false));

    EXPECT_TRUE(To<CSSUnparsedDeclarationValue>(value1)->VariableDataValue()->IsAnimationTainted());
    EXPECT_FALSE(To<CSSUnparsedDeclarationValue>(value2)->VariableDataValue()->IsAnimationTainted());
}
...
TEST_F(CustomPropertyTest, ParseSingleValueTyped)
{
    RegisterProperty(GetDocument(), "--x", "<length>", "0px", false);
    CustomProperty property(AtomicString("--x"), GetDocument());
    const CSSValue* value1 = ParseValue(property, "100px", CSSParserLocalContext());
    EXPECT_TRUE(value1->IsPrimitiveValue());
    EXPECT_EQ(100, To<CSSPrimitiveValue>(value1)->ComputeLength<double>(CSSToLengthConversionData()));

    const CSSValue* value2 = ParseValue(property, "maroon", CSSParserLocalContext());
    EXPECT_FALSE(value2);
}
...
{
    CustomProperty property(AtomicString("--x"), GetDocument());
    EXPECT_EQ(CSSPropertyName(AtomicString("--x")), property.GetCSSPropertyName());
}
...
TEST_F(CustomPropertyTest, ParseAnchorQueriesAsLength)
{
    ...
    EXPECT_FALSE(ParseValue(property, "anchor(--foo top)", CSSParserLocalContext()));
    EXPECT_FALSE(ParseValue(property, "anchor-size(--foo width)", CSSParserLocalContext()));
}
...
TEST_F(CustomPropertyTest, ParseAnchorQueriesAsLengthPercentage)
{
    ...
    {
        const CSSValue* value = ParseValue(property, "anchor(--foo top)", CSSParserLocalContext());
        ASSERT_TRUE(value);
        EXPECT_EQ("anchor(--foo top)", value->CssText());
    }

    {
        const CSSValue* value = ParseValue(property, "anchor-size(--foo width)", CSSParserLocalContext());
        ASSERT_TRUE(value);
...

    {
        ...
        // computed-value time.
        const CSSValue* value = ParseValue(property, "calc(anchor(--foo top) + anchor-size(--foo width))", CSSParserLocalContext());
        ASSERT_TRUE(value);
        EXPECT_EQ("calc(anchor(--foo top) + anchor-size(--foo width))", value->CssText());
    }
}
...
TEST_F(CustomPropertyTest, ValueMode)
{
    ...
    auto* declaration = MakeGarbageCollected<CSSUnparsedDeclarationValue>(data, /* parser_context */ nullptr);
    ...
    // ValueMode::kNormal
    {
        ...
        EXPECT_FALSE(style->GetVariableData(AtomicString("--x"))->IsAnimationTainted());
    }
    ...
    // ValueMode::kAnimated
    {
        ...
        const ComputedStyle* style = state.TakeStyle();
        ASSERT_TRUE(style->GetVariableData(AtomicString("--x")));
        EXPECT_TRUE(style->GetVariableData(AtomicString("--x"))->IsAnimationTainted());
    }
}
