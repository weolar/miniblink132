...
public:
    ...
    const CSSValue* Parse(StringView, const CSSParserContext&, const CSSParserLocalContext&) const;
...
private:
    CustomProperty(const AtomicString& name, const PropertyRegistration* registration);
    explicit CustomProperty(const PropertyRegistration* registration);

    const CSSValue* ParseUntyped(StringView, const CSSParserContext&, const CSSParserLocalContext&) const;

    AtomicString name_;
    Member<const PropertyRegistration> registration_;
};

template <> struct DowncastTraits<CustomProperty> {
