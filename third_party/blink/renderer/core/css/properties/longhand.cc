...
void Longhand::ApplyParentValue(StyleResolverState& state) const
{
    ...
    // effective zoom.
    const CSSValue* parent_computed_value = ComputedStyleUtils::ComputedPropertyValue(*this, *state.ParentStyle());
...
{
    if (state.ParentStyle()->EffectiveZoom() != state.StyleBuilder().EffectiveZoom()) {
