...
public:
    ...
    CSSValue* ConsumeFunctionalSyntaxColor(CSSParserTokenStream& stream, const CSSParserContext& context);

private:
    enum class ChannelType { kNone, kPercentage, kNumber, kRelative };
    bool ConsumeColorSpaceAndOriginColor(CSSParserTokenStream& stream, CSSValueID function_id, const CSSParserContext& context);
    bool ConsumeChannel(CSSParserTokenStream& stream, const CSSParserContext& context, int index);
    bool ConsumeAlpha(CSSParserTokenStream& stream, const CSSParserContext& context);
    void MakePerColorSpaceAdjustments();

    static double ResolveColorChannel(const CSSValue* value, ChannelType channel_type, double percentage_base, const CSSColorChannelMap& color_channel_map);
    static double ResolveAlpha(const CSSValue* value, ChannelType channel_type, const CSSColorChannelMap& color_channel_map);
