...
CSSValue* ConvertFontPaletteToCSSValue(const blink::FontPalette* palette)
{
    ...
    case blink::FontPalette::kCustomPalette:
        return MakeGarbageCollected<CSSCustomIdentValue>(palette->GetPaletteValuesName());
    case blink::FontPalette::kInterpolablePalette: {
        ...
        // ]#{2})
        CSSFunctionValue* result = MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kPaletteMix);

        CSSValueList* color_space_css_value_list = CSSValueList::CreateSpaceSeparated();
        color_space_css_value_list->Append(*MakeGarbageCollected<CSSCustomIdentValue>(AtomicString("in")));
        if (palette->GetHueInterpolationMethod().has_value()) {
            color_space_css_value_list->Append(*MakeGarbageCollected<CSSCustomIdentValue>(
                AtomicString(Color::SerializeInterpolationSpace(palette->GetColorInterpolationSpace(), *palette->GetHueInterpolationMethod()))));
        } else {
            color_space_css_value_list->Append(
                *MakeGarbageCollected<CSSCustomIdentValue>(AtomicString(Color::SerializeInterpolationSpace(palette->GetColorInterpolationSpace()))));
        }
        ...
        CSSValueList* start_palette_with_percentage = CSSValueList::CreateSpaceSeparated();
        ...
        if (start_percentage_ + end_percentage_ != kFinalStatePercentage || start_percentage_ != kMiddleStatePercentage) {
            CSSValue* param = CSSNumericLiteralValue::Create(start_percentage_, CSSPrimitiveValue::UnitType::kPercentage);
            start_palette_with_percentage->Append(*param);
        }
        result->Append(*start_palette_with_percentage);

        CSSValueList* end_palette_with_percentage = CSSValueList::CreateSpaceSeparated();
        ...
        if (start_percentage_ + end_percentage_ != kFinalStatePercentage) {
            CSSValue* param = CSSNumericLiteralValue::Create(end_percentage_, CSSPrimitiveValue::UnitType::kPercentage);
...
CSSValue* ComputedStyleUtils::ZoomAdjustedPixelValueForLength(const Length& length, const ComputedStyle& style)
{
    if (length.IsFixed()) {
...
CSSValue* ComputedStyleUtils::ValueForPosition(const LengthPoint& position, const ComputedStyle& style)
{
    ...
    return MakeGarbageCollected<CSSValuePair>(
        ZoomAdjustedPixelValueForLength(position.X(), style), ZoomAdjustedPixelValueForLength(position.Y(), style), CSSValuePair::kKeepIdenticalValues);
}

CSSValue* ComputedStyleUtils::ValueForOffset(const ComputedStyle& style, const LayoutObject* layout_object, bool allow_visited_style, CSSValuePhase value_phase)
{
    ...
    if (!position_identifier_value || position_identifier_value->GetValueID() == CSSValueID::kAuto) {
        list->Append(*position);
    } else {
        DCHECK_EQ(position_identifier_value->GetValueID(), CSSValueID::kNormal);
    }

    static const CSSProperty* longhands[3] = { &GetCSSPropertyOffsetPath(), &GetCSSPropertyOffsetDistance(), &GetCSSPropertyOffsetRotate() };
...
const CSSValue* ComputedStyleUtils::ValueForColor(
    const StyleColor& style_color, const ComputedStyle& style, const Color* override_current_color, CSSValuePhase value_phase)
{
    const Color current_color = override_current_color ? *override_current_color : style.GetCurrentColor();
    return cssvalue::CSSColor::Create(style_color.Resolve(current_color, style.UsedColorScheme()));
}

const CSSValue* ComputedStyleUtils::CurrentColorOrValidColor(const ComputedStyle& style, const StyleColor& color, CSSValuePhase value_phase)
{
    return ValueForColor(color, style, nullptr, value_phase);
}

const blink::Color ComputedStyleUtils::BorderSideColor(
    const ComputedStyle& style, const StyleColor& color, EBorderStyle border_style, bool visited_link, bool* is_current_color)
{
    Color current_color;
    if (visited_link) {
        current_color = style.GetInternalVisitedCurrentColor();
    } else if (border_style == EBorderStyle::kInset || border_style == EBorderStyle::kOutset || border_style == EBorderStyle::kRidge
    ...
    return color.Resolve(current_color, style.UsedColorScheme(), is_current_color);
}

const CSSValue* ComputedStyleUtils::BackgroundImageOrMaskImage(
    const ComputedStyle& style, bool allow_visited_style, const FillLayer& fill_layer, CSSValuePhase value_phase)
{
    CSSValueList* list = CSSValueList::CreateCommaSeparated();
    const FillLayer* curr_layer = &fill_layer;
    for (; curr_layer; curr_layer = curr_layer->Next()) {
        if (curr_layer->GetImage()) {
            list->Append(*curr_layer->GetImage()->ComputedCSSValue(style, allow_visited_style, value_phase));
        } else {
...
const CSSValue* ComputedStyleUtils::ValueForFillSize(const FillSize& fill_size, const ComputedStyle& style)
{
    ...
    return MakeGarbageCollected<CSSValuePair>(ZoomAdjustedPixelValueForLength(fill_size.size.Width(), style),
        ZoomAdjustedPixelValueForLength(fill_size.size.Height(), style), CSSValuePair::kKeepIdenticalValues);
}

const CSSValue* ComputedStyleUtils::BackgroundImageOrMaskSize(const ComputedStyle& style, const FillLayer& fill_layer)
{
    CSSValueList* list = CSSValueList::CreateCommaSeparated();
    const FillLayer* curr_layer = &fill_layer;
    for (; curr_layer; curr_layer = curr_layer->Next()) {
...
const CSSValueList* ComputedStyleUtils::CreatePositionListForLayer(const CSSProperty& property, const FillLayer& layer, const ComputedStyle& style)
{
    CSSValueList* position_list = CSSValueList::CreateSpaceSeparated();
    if (layer.IsBackgroundXOriginSet()) {
        DCHECK(property.IDEquals(CSSPropertyID::kBackgroundPosition) || property.IDEquals(CSSPropertyID::kMaskPosition));
        position_list->Append(*CSSIdentifierValue::Create(layer.BackgroundXOrigin()));
    }
    position_list->Append(*ZoomAdjustedPixelValueForLength(layer.PositionX(), style));
    if (layer.IsBackgroundYOriginSet()) {
        DCHECK(property.IDEquals(CSSPropertyID::kBackgroundPosition) || property.IDEquals(CSSPropertyID::kMaskPosition));
        position_list->Append(*CSSIdentifierValue::Create(layer.BackgroundYOrigin()));
    }
    position_list->Append(*ZoomAdjustedPixelValueForLength(layer.PositionY(), style));
    return position_list;
}

const CSSValue* ComputedStyleUtils::ValueForFillRepeat(const FillLayer* curr_layer)
{
    const auto& fill_repeat = curr_layer->Repeat();

    return MakeGarbageCollected<CSSRepeatStyleValue>(CSSIdentifierValue::Create(fill_repeat.x), CSSIdentifierValue::Create(fill_repeat.y));
}
...
const CSSValueList* ComputedStyleUtils::ValuesForBackgroundShorthand(
    const ComputedStyle& style, const LayoutObject* layout_object, bool allow_visited_style, CSSValuePhase value_phase)
{
    ...
    for (; curr_layer; curr_layer = curr_layer->Next()) {
        ...
        if (!curr_layer->Next()) { // color only for final layer
            const CSSValue* value = GetCSSPropertyBackgroundColor().CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
            DCHECK(value);
            before_slash->Append(*value);
        }
        before_slash->Append(curr_layer->GetImage() ? *curr_layer->GetImage()->ComputedCSSValue(style, allow_visited_style, value_phase)
        ...
        before_slash->Append(*CreatePositionListForLayer(GetCSSPropertyBackgroundPosition(), *curr_layer, style));
...
void AppendValuesForMaskClipAndOrigin(CSSValueList* result_list, EFillBox origin, EFillBox clip)
{
    if (origin == clip) {
...
const CSSValueList* ComputedStyleUtils::ValuesForMaskShorthand(
    const StylePropertyShorthand&, const ComputedStyle& style, const LayoutObject* layout_object, bool allow_visited_style, CSSValuePhase value_phase)
{
    ...
    for (; layer; layer = layer->Next()) {
        ...
        // <mask-reference>
        if (layer->GetImage()) {
            list->Append(*layer->GetImage()->ComputedCSSValue(style, allow_visited_style, value_phase));
        }
        // <position> [ / <bg-size> ]?
        if (layer->PositionX() != FillLayer::InitialFillPositionX(EFillLayerType::kMask)
            || layer->PositionY() != FillLayer::InitialFillPositionY(EFillLayerType::kMask)
            ...
            position_size_list->Append(*CreatePositionListForLayer(GetCSSPropertyMaskPosition(), *layer, style));
        ...
        // <repeat-style>
        if (layer->Repeat() != FillLayer::InitialFillRepeat(EFillLayerType::kMask)) {
        ...
        // <compositing-operator>
        if (layer->CompositingOperator() != FillLayer::InitialFillCompositingOperator(EFillLayerType::kMask)) {
            list->Append(*CSSIdentifierValue::Create(layer->CompositingOperator()));
        }
        // <masking-mode>
        if (layer->MaskMode() != FillLayer::InitialFillMaskMode(EFillLayerType::kMask)) {
...
const CSSValue* ComputedStyleUtils::BackgroundPositionOrMaskPosition(
    const CSSProperty& resolved_property, const ComputedStyle& style, const FillLayer* curr_layer)
{
    CSSValueList* list = CSSValueList::CreateCommaSeparated();
    for (; curr_layer; curr_layer = curr_layer->Next()) {
        list->Append(*CreatePositionListForLayer(resolved_property, *curr_layer, style));
    }
    return list;
}

const CSSValue* ComputedStyleUtils::BackgroundPositionXOrWebkitMaskPositionX(const ComputedStyle& style, const FillLayer* curr_layer)
{
    CSSValueList* list = CSSValueList::CreateCommaSeparated();
    for (; curr_layer; curr_layer = curr_layer->Next()) {
        ...
        if (curr_layer->BackgroundXOrigin() == BackgroundEdgeOrigin::kRight) {
            ...
            // parser accepts it.
            list->Append(*ZoomAdjustedPixelValueForLength(from_edge.SubtractFromOneHundredPercent(), style));
        } else {
...
const CSSValue* ComputedStyleUtils::BackgroundPositionYOrWebkitMaskPositionY(const ComputedStyle& style, const FillLayer* curr_layer)
{
    CSSValueList* list = CSSValueList::CreateCommaSeparated();
    for (; curr_layer; curr_layer = curr_layer->Next()) {
...
static CSSNumericLiteralValue* ValueForImageSlice(const Length& slice)
{
    ...
    return CSSNumericLiteralValue::Create(slice.Value(), slice.IsPercent() ? CSSPrimitiveValue::UnitType::kPercentage : CSSPrimitiveValue::UnitType::kNumber);
}

cssvalue::CSSBorderImageSliceValue* ComputedStyleUtils::ValueForNinePieceImageSlice(const NinePieceImage& image)
{
    ...
    return MakeGarbageCollected<cssvalue::CSSBorderImageSliceValue>(
        MakeGarbageCollected<CSSQuadValue>(top, right, bottom, left, CSSQuadValue::kSerializeAsQuad), image.Fill());
}

CSSValue* ValueForBorderImageLength(const BorderImageLength& border_image_length, const ComputedStyle& style)
{
    if (border_image_length.IsNumber()) {
        return CSSNumericLiteralValue::Create(border_image_length.Number(), CSSPrimitiveValue::UnitType::kNumber);
    }
    return CSSValue::Create(border_image_length.length(), style.EffectiveZoom());
}

CSSQuadValue* ComputedStyleUtils::ValueForNinePieceImageQuad(const BorderImageLengthBox& box, const ComputedStyle& style)
{
    // Create the slices.
    ...
    return MakeGarbageCollected<CSSQuadValue>(top, right, bottom, left, CSSQuadValue::kSerializeAsQuad);
}
...
CSSValue* ComputedStyleUtils::ValueForNinePieceImageRepeat(const NinePieceImage& image)
{
    CSSIdentifierValue* horizontal_repeat = nullptr;
    CSSIdentifierValue* vertical_repeat = nullptr;

    horizontal_repeat = CSSIdentifierValue::Create(ValueForRepeatRule(image.HorizontalRule()));
    if (image.HorizontalRule() == image.VerticalRule()) {
        vertical_repeat = horizontal_repeat;
    } else {
        vertical_repeat = CSSIdentifierValue::Create(ValueForRepeatRule(image.VerticalRule()));
    }
    return MakeGarbageCollected<CSSValuePair>(horizontal_repeat, vertical_repeat, CSSValuePair::kDropIdenticalValues);
}

CSSValue* ComputedStyleUtils::ValueForNinePieceImage(
    const NinePieceImage& image, const ComputedStyle& style, bool allow_visited_style, CSSValuePhase value_phase)
{
    if (!image.HasImage()) {
    ...
    if (image.GetImage()) {
        image_value = image.GetImage()->ComputedCSSValue(style, allow_visited_style, value_phase);
    }

    // Create the image slice.
    cssvalue::CSSBorderImageSliceValue* image_slices = ValueForNinePieceImageSlice(image);

    // Create the border area slices.
    CSSValue* border_slices = ValueForNinePieceImageQuad(image.BorderSlices(), style);
    ...
    return CreateBorderImageValue(image_value, image_slices, border_slices, outset, repeat);
}

CSSValue* ComputedStyleUtils::ValueForReflection(
    const StyleReflection* reflection, const ComputedStyle& style, bool allow_visited_style, CSSValuePhase value_phase)
{
    if (!reflection) {
        return CSSIdentifierValue::Create(CSSValueID::kNone);
    }
    ...
    auto* offset = CSSPrimitiveValue::CreateFromLength(reflection->Offset(), style.EffectiveZoom());
    return MakeGarbageCollected<cssvalue::CSSReflectValue>(
        direction, offset, ValueForNinePieceImage(reflection->Mask(), style, allow_visited_style, value_phase));
}

CSSValue* ComputedStyleUtils::MinWidthOrMinHeightAuto(const ComputedStyle& style)
{
...
CSSValue* ComputedStyleUtils::ValueForPositionOffset(const ComputedStyle& style, const CSSProperty& property, const LayoutObject* layout_object)
{
    ...
    if (box && box->IsOutOfFlowPositioned()) {
        ...
        const PhysicalBoxStrut& insets = box->OutOfFlowInsetsForGetComputedStyle().ConvertToPhysical(box->ContainingBlock()->StyleRef().GetWritingDirection());
        LayoutUnit inset;
        switch (property.PropertyID()) {
        case CSSPropertyID::kLeft:
            inset = insets.left;
    ...
    if ((offset.IsPercent() || offset.IsCalculated()) && box && box->IsPositioned()) {
        ...
        if (box->IsStickyPositioned()) {
            ...
            bool use_inline_size = is_horizontal_property == scroll_container->IsHorizontalWritingMode();
            containing_block_size = use_inline_size ? scroll_container->ContentLogicalWidth() : scroll_container->ContentLogicalHeight();
            UseCounter::Count(box->GetDocument(), WebFeature::kPercentOrCalcStickyUsedOffset);
        } else {
            DCHECK(box->IsRelPositioned());
            containing_block_size = is_horizontal_property == box->ContainingBlock()->IsHorizontalWritingMode()
                ? box->ContainingBlockLogicalWidthForContent()
                : box->ContainingBlockLogicalHeightForRelPositioned();
            UseCounter::Count(box->GetDocument(), WebFeature::kPercentOrCalcRelativeUsedOffset);
        }

        return ZoomAdjustedPixelValue(ValueForLength(offset, containing_block_size), style);
    }

    if (offset.IsAuto() && layout_object && layout_object->IsRelPositioned()) {
        UseCounter::Count(layout_object->GetDocument(), WebFeature::kAutoRelativeUsedOffset);
        ...
        // auto.
        if (opposite.IsAuto()) {
            return CSSNumericLiteralValue::Create(0, CSSPrimitiveValue::UnitType::kPixels);
        }

        if (opposite.IsPercent() || opposite.IsCalculated()) {
            if (box) {
                LayoutUnit containing_block_size = is_horizontal_property == layout_object->ContainingBlock()->IsHorizontalWritingMode()
                    ? box->ContainingBlockLogicalWidthForContent()
                    : box->ContainingBlockLogicalHeightForRelPositioned();
                return ZoomAdjustedPixelValue(-FloatValueForLength(opposite, containing_block_size), style);
            }
...
CSSValue* ComputedStyleUtils::ValueForItemPositionWithOverflowAlignment(const StyleSelfAlignmentData& data)
{
    if (data.PositionType() == ItemPositionType::kLegacy) {
        ...
        DCHECK(data.GetPosition() == ItemPosition::kLeft || data.GetPosition() == ItemPosition::kRight || data.GetPosition() == ItemPosition::kCenter)
            << "Unexpected position: " << (unsigned)data.GetPosition();
        DCHECK_EQ(data.Overflow(), OverflowAlignment::kDefault);
        return MakeGarbageCollected<CSSValuePair>(
            CSSIdentifierValue::Create(CSSValueID::kLegacy), CSSIdentifierValue::Create(data.GetPosition()), CSSValuePair::kDropIdenticalValues);
    }
    ...
    if (data.GetPosition() == ItemPosition::kBaseline) {
        ...
        return MakeGarbageCollected<CSSValuePair>(
            CSSIdentifierValue::Create(CSSValueID::kLast), CSSIdentifierValue::Create(CSSValueID::kBaseline), CSSValuePair::kDropIdenticalValues);
    } else {
        auto* position
            = data.GetPosition() == ItemPosition::kLegacy ? CSSIdentifierValue::Create(CSSValueID::kNormal) : CSSIdentifierValue::Create(data.GetPosition());
        if (data.GetPosition() >= ItemPosition::kCenter && data.Overflow() != OverflowAlignment::kDefault) {
            return MakeGarbageCollected<CSSValuePair>(CSSIdentifierValue::Create(data.Overflow()), position, CSSValuePair::kDropIdenticalValues);
        }
        return position;
    }
}

cssvalue::CSSContentDistributionValue* ComputedStyleUtils::ValueForContentPositionAndDistributionWithOverflowAlignment(const StyleContentAlignmentData& data)
{
    ...
    default:
        ...
        if ((data.GetPosition() >= ContentPosition::kCenter || data.Distribution() != ContentDistributionType::kDefault)
    ...
    return MakeGarbageCollected<cssvalue::CSSContentDistributionValue>(distribution, position, overflow);
}
...
CSSValue* ComputedStyleUtils::ComputedValueForLineHeight(const ComputedStyle& style)
{
    const Length& length = style.LineHeight();
    if (length.IsAuto()) {
        return CSSIdentifierValue::Create(CSSValueID::kNormal);
    ...
    if (length.IsPercent()) {
        return CSSNumericLiteralValue::Create(length.GetFloatValue() / 100.0, CSSPrimitiveValue::UnitType::kNumber);
    } else {
        return ZoomAdjustedPixelValue(FloatValueForLength(length, style.GetFontDescription().ComputedSize()), style);
    }
}
...
CSSValueList* ComputedStyleUtils::ValueForFontFamily(const FontFamily& font_family)
{
    CSSValueList* list = CSSValueList::CreateCommaSeparated();
...
CSSValueList* ComputedStyleUtils::ValueForFontFamily(const ComputedStyle& style)
{
    return ComputedStyleUtils::ValueForFontFamily(style.GetFontDescription().Family());
}

CSSPrimitiveValue* ComputedStyleUtils::ValueForFontSize(const ComputedStyle& style)
{
    return ZoomAdjustedPixelValue(style.GetFontDescription().ComputedSize(), style);
}

CSSValue* ComputedStyleUtils::ValueForFontSizeAdjust(const ComputedStyle& style)
{
    if (!style.HasFontSizeAdjust() || style.FontSizeAdjust().Value() == FontSizeAdjust::kFontSizeAdjustNone) {
        return CSSIdentifierValue::Create(CSSValueID::kNone);
    }
    ...
    if (font_size_adjust.GetMetric() == FontSizeAdjust::Metric::kExHeight) {
        return CSSNumericLiteralValue::Create(style.FontSizeAdjust().Value(), CSSPrimitiveValue::UnitType::kNumber);
    }

    return MakeGarbageCollected<CSSValuePair>(CSSIdentifierValue::Create(font_size_adjust.GetMetric()),
        CSSNumericLiteralValue::Create(style.FontSizeAdjust().Value(), CSSPrimitiveValue::UnitType::kNumber), CSSValuePair::kKeepIdenticalValues);
}

CSSPrimitiveValue* ComputedStyleUtils::ValueForFontStretch(const ComputedStyle& style)
{
    return CSSNumericLiteralValue::Create(style.GetFontDescription().Stretch(), CSSPrimitiveValue::UnitType::kPercentage);
}
...
CSSValue* ComputedStyleUtils::ValueForFontStyle(const ComputedStyle& style)
{
    ...
    oblique_values->Append(*CSSNumericLiteralValue::Create(angle, CSSPrimitiveValue::UnitType::kDegrees));
    return MakeGarbageCollected<cssvalue::CSSFontStyleRangeValue>(*CSSIdentifierValue::Create(CSSValueID::kOblique), *oblique_values);
}

CSSNumericLiteralValue* ComputedStyleUtils::ValueForFontWeight(const ComputedStyle& style)
{
    return CSSNumericLiteralValue::Create(style.GetFontDescription().Weight(), CSSPrimitiveValue::UnitType::kNumber);
}

CSSIdentifierValue* ComputedStyleUtils::ValueForFontVariantCaps(const ComputedStyle& style)
{
    FontDescription::FontVariantCaps variant_caps = style.GetFontDescription().VariantCaps();
    switch (variant_caps) {
...
CSSValue* ComputedStyleUtils::ValueForFontVariantLigatures(const ComputedStyle& style)
{
    FontDescription::LigaturesState common_ligatures_state = style.GetFontDescription().CommonLigaturesState();
    FontDescription::LigaturesState discretionary_ligatures_state = style.GetFontDescription().DiscretionaryLigaturesState();
    FontDescription::LigaturesState historical_ligatures_state = style.GetFontDescription().HistoricalLigaturesState();
    FontDescription::LigaturesState contextual_ligatures_state = style.GetFontDescription().ContextualLigaturesState();
    if (common_ligatures_state == FontDescription::kNormalLigaturesState && discretionary_ligatures_state == FontDescription::kNormalLigaturesState
        && historical_ligatures_state == FontDescription::kNormalLigaturesState && contextual_ligatures_state == FontDescription::kNormalLigaturesState) {
        return CSSIdentifierValue::Create(CSSValueID::kNormal);
    }

    if (common_ligatures_state == FontDescription::kDisabledLigaturesState && discretionary_ligatures_state == FontDescription::kDisabledLigaturesState
        && historical_ligatures_state == FontDescription::kDisabledLigaturesState && contextual_ligatures_state == FontDescription::kDisabledLigaturesState) {
        return CSSIdentifierValue::Create(CSSValueID::kNone);
    }
    ...
    if (common_ligatures_state != FontDescription::kNormalLigaturesState) {
        value_list->Append(*CSSIdentifierValue::Create(
            common_ligatures_state == FontDescription::kDisabledLigaturesState ? CSSValueID::kNoCommonLigatures : CSSValueID::kCommonLigatures));
    }
    if (discretionary_ligatures_state != FontDescription::kNormalLigaturesState) {
        value_list->Append(
            *CSSIdentifierValue::Create(discretionary_ligatures_state == FontDescription::kDisabledLigaturesState ? CSSValueID::kNoDiscretionaryLigatures
                                                                                                                  : CSSValueID::kDiscretionaryLigatures));
    }
    if (historical_ligatures_state != FontDescription::kNormalLigaturesState) {
        value_list->Append(*CSSIdentifierValue::Create(
            historical_ligatures_state == FontDescription::kDisabledLigaturesState ? CSSValueID::kNoHistoricalLigatures : CSSValueID::kHistoricalLigatures));
    }
    if (contextual_ligatures_state != FontDescription::kNormalLigaturesState) {
        value_list->Append(*CSSIdentifierValue::Create(
            contextual_ligatures_state == FontDescription::kDisabledLigaturesState ? CSSValueID::kNoContextual : CSSValueID::kContextual));
    }
    return value_list;
}

CSSValue* ComputedStyleUtils::ValueForFontVariantNumeric(const ComputedStyle& style)
{
    FontVariantNumeric variant_numeric = style.GetFontDescription().VariantNumeric();
    ...
    if (variant_numeric.NumericFigureValue() != FontVariantNumeric::kNormalFigure) {
        value_list->Append(*CSSIdentifierValue::Create(
            variant_numeric.NumericFigureValue() == FontVariantNumeric::kLiningNums ? CSSValueID::kLiningNums : CSSValueID::kOldstyleNums));
    }
    if (variant_numeric.NumericSpacingValue() != FontVariantNumeric::kNormalSpacing) {
        value_list->Append(*CSSIdentifierValue::Create(
            variant_numeric.NumericSpacingValue() == FontVariantNumeric::kProportionalNums ? CSSValueID::kProportionalNums : CSSValueID::kTabularNums));
    }
    if (variant_numeric.NumericFractionValue() != FontVariantNumeric::kNormalFraction) {
        value_list->Append(*CSSIdentifierValue::Create(
            variant_numeric.NumericFractionValue() == FontVariantNumeric::kDiagonalFractions ? CSSValueID::kDiagonalFractions : CSSValueID::kStackedFractions));
    }
...
CSSValue* ComputedStyleUtils::ValueForFontVariantAlternates(const ComputedStyle& style)
{
    const FontVariantAlternates* variant_alternates = style.GetFontDescription().GetFontVariantAlternates();
    ...
    if (const AtomicString* opt_stylistic = variant_alternates->Stylistic()) {
        value_list->Append(*MakeGarbageCollected<cssvalue::CSSAlternateValue>(
            *MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kStylistic), *make_single_ident_list(*opt_stylistic)));
    }
    if (variant_alternates->HistoricalForms()) {
        value_list->Append(*CSSIdentifierValue::Create(CSSValueID::kHistoricalForms));
    }
    if (const AtomicString* opt_swash = variant_alternates->Swash()) {
        value_list->Append(*MakeGarbageCollected<cssvalue::CSSAlternateValue>(
            *MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kSwash), *make_single_ident_list(*opt_swash)));
    }
    if (const AtomicString* opt_ornaments = variant_alternates->Ornaments()) {
        value_list->Append(*MakeGarbageCollected<cssvalue::CSSAlternateValue>(
            *MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kOrnaments), *make_single_ident_list(*opt_ornaments)));
    }
    if (const AtomicString* opt_annotation = variant_alternates->Annotation()) {
        value_list->Append(*MakeGarbageCollected<cssvalue::CSSAlternateValue>(
            *MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kAnnotation), *make_single_ident_list(*opt_annotation)));
    }
    ...
    if (!variant_alternates->Styleset().empty()) {
        ...
        value_list->Append(*MakeGarbageCollected<cssvalue::CSSAlternateValue>(*MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kStyleset), *aliases_list));
    }
    if (!variant_alternates->CharacterVariant().empty()) {
        ...
        value_list->Append(
            *MakeGarbageCollected<cssvalue::CSSAlternateValue>(*MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kCharacterVariant), *aliases_list));
    }
...
CSSIdentifierValue* ComputedStyleUtils::ValueForFontVariantPosition(const ComputedStyle& style)
{
    FontDescription::FontVariantPosition variant_position = style.GetFontDescription().VariantPosition();
...
CSSIdentifierValue* ComputedStyleUtils::ValueForFontKerning(const ComputedStyle& style)
{
...
CSSIdentifierValue* ComputedStyleUtils::ValueForFontOpticalSizing(const ComputedStyle& style)
{
...
CSSValue* ComputedStyleUtils::ValueForFontVariantEastAsian(const ComputedStyle& style)
{
    FontVariantEastAsian east_asian = style.GetFontDescription().VariantEastAsian();
    ...
    case FontVariantEastAsian::kProportionalWidth:
        value_list->Append(*CSSIdentifierValue::Create(CSSValueID::kProportionalWidth));
        break;
    default:
        NOTREACHED();
    }
...
CSSValue* ComputedStyleUtils::ValueForFontFeatureSettings(const ComputedStyle& style)
{
    const blink::FontFeatureSettings* feature_settings = style.GetFontDescription().FeatureSettings();
    ...
    for (wtf_size_t i = 0; i < feature_settings->size(); ++i) {
        ...
        auto* feature_value = MakeGarbageCollected<cssvalue::CSSFontFeatureValue>(feature.TagString(), feature.Value());
        list->Append(*feature_value);
    }
    return list;
}

CSSValue* ComputedStyleUtils::ValueForFontVariationSettings(const ComputedStyle& style)
{
    const blink::FontVariationSettings* variation_settings = style.GetFontDescription().VariationSettings();
    ...
    for (wtf_size_t i = 0; i < variation_settings->size(); ++i) {
        ...
        cssvalue::CSSFontVariationValue* variation_value
            = MakeGarbageCollected<cssvalue::CSSFontVariationValue>(variation_axis.TagString(), variation_axis.Value());
...
{
    const blink::FontPalette* palette = style.GetFontDescription().GetFontPalette();
...
CSSValue* ComputedStyleUtils::ValueForFont(const ComputedStyle& style)
{
    ...
    // resolved.
    if (!base::ValuesEquivalent(ligatures_value, static_cast<CSSValue*>(CSSIdentifierValue::Create(CSSValueID::kNormal)))
        || !base::ValuesEquivalent(numeric_value, static_cast<CSSValue*>(CSSIdentifierValue::Create(CSSValueID::kNormal)))
        || !base::ValuesEquivalent(east_asian_value, static_cast<CSSValue*>(CSSIdentifierValue::Create(CSSValueID::kNormal)))
        || !base::ValuesEquivalent(feature_settings, static_cast<CSSValue*>(CSSIdentifierValue::Create(CSSValueID::kNormal)))
        || !base::ValuesEquivalent(variation_settings, static_cast<CSSValue*>(CSSIdentifierValue::Create(CSSValueID::kNormal)))
        || !base::ValuesEquivalent(variant_alternative, static_cast<CSSValue*>(CSSIdentifierValue::Create(CSSValueID::kNormal)))) {
        return nullptr;
    }
    ...
    if (kerning != FontDescription::kAutoKerning || optical_sizing != kAutoOpticalSizing
        || (RuntimeEnabledFeatures::CSSFontSizeAdjustEnabled() && style.GetFontDescription().HasSizeAdjust())
        || variant_position != FontDescription::kNormalVariantPosition
        || (RuntimeEnabledFeatures::FontVariantEmojiEnabled() && variant_emoji != kNormalVariantEmoji)) {
        return nullptr;
    }
    ...
    if (caps_value->GetValueID() != CSSValueID::kNormal && caps_value->GetValueID() != CSSValueID::kSmallCaps) {
        return nullptr;
    }
...

    {
        ...
        if (identifier_line_height && identifier_line_height->GetValueID() == CSSValueID::kNormal) {
...
CSSValue* ComputedStyleUtils::SpecifiedValueForGridTrackSize(const GridTrackSize& track_size, const ComputedStyle& style)
{
    switch (track_size.GetType()) {
    case kLengthTrackSizing:
        return ComputedStyleUtils::ZoomAdjustedPixelValueForLength(track_size.MinTrackBreadth(), style);
    case kMinMaxTrackSizing: {
        if (track_size.MinTrackBreadth().IsAuto() && track_size.MaxTrackBreadth().IsFlex()) {
            return ComputedStyleUtils::ZoomAdjustedPixelValueForLength(track_size.MaxTrackBreadth(), style);
        }

        auto* min_max_track_breadths = MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kMinmax);
        min_max_track_breadths->Append(*ComputedStyleUtils::ZoomAdjustedPixelValueForLength(track_size.MinTrackBreadth(), style));
        min_max_track_breadths->Append(*ComputedStyleUtils::ZoomAdjustedPixelValueForLength(track_size.MaxTrackBreadth(), style));
        return min_max_track_breadths;
    }
    case kFitContentTrackSizing: {
        auto* fit_content_track_breadth = MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kFitContent);
        fit_content_track_breadth->Append(*ComputedStyleUtils::ZoomAdjustedPixelValueForLength(track_size.FitContentTrackBreadth(), style));
...
public:
    OrderedNamedLinesCollector(const OrderedNamedGridLines& ordered_named_grid_lines, const OrderedNamedGridLines& ordered_named_auto_repeat_grid_lines,
        bool is_subgridded_track, bool is_layout_grid)
        : ordered_named_grid_lines_(ordered_named_grid_lines)
        , ordered_named_auto_repeat_grid_lines_(ordered_named_auto_repeat_grid_lines)
    ...
    bool IsEmpty() const
    {
        return ordered_named_grid_lines_.empty() && ordered_named_auto_repeat_grid_lines_.empty();
    }
    bool IsSubgriddedAxis() const
    {
        return is_subgridded_axis_;
    }
    wtf_size_t InsertionPoint() const
    {
    ...
    {
        return is_layout_grid_ && IsSubgriddedAxis() && HasAutoRepeatNamedLinesSpecified() && auto_repeat_total_tracks_ == 0;
    }
    virtual void CollectLineNamesForIndex(cssvalue::CSSBracketedValueList&, wtf_size_t index, NamedLinesType type, bool is_in_repeat) const;

protected:
    void AppendLines(cssvalue::CSSBracketedValueList&, wtf_size_t index, NamedLinesType, bool is_in_repeat = false) const;
...
public:
    OrderedNamedLinesCollectorInGridLayout(const OrderedNamedGridLines& ordered_named_grid_lines,
        const OrderedNamedGridLines& ordered_named_auto_repeat_grid_lines, wtf_size_t insertion_point, wtf_size_t auto_repeat_total_tracks,
        wtf_size_t auto_repeat_track_list_length, bool is_subgridded_track)
        : OrderedNamedLinesCollector(ordered_named_grid_lines, ordered_named_auto_repeat_grid_lines, is_subgridded_track,
            /* is_layout_grid */ true)
    {
    ...
    void CollectLineNamesForIndex(cssvalue::CSSBracketedValueList&, wtf_size_t index, NamedLinesType type, bool is_in_repeat) const override;
};

void OrderedNamedLinesCollector::AppendLines(cssvalue::CSSBracketedValueList& line_names_value, wtf_size_t index, NamedLinesType type, bool is_in_repeat) const
{
    const bool is_auto = type == NamedLinesType::kAutoRepeatNamedLines;
    auto iter = is_auto ? ordered_named_auto_repeat_grid_lines_.find(index) : ordered_named_grid_lines_.find(index);
    auto end_iter = is_auto ? ordered_named_auto_repeat_grid_lines_.end() : ordered_named_grid_lines_.end();
    if (iter == end_iter) {
        return;
    }
    ...
    for (auto named_grid_line : iter->value) {
        ...
        const bool is_not_in_repeat = !is_in_repeat && !named_grid_line.is_in_repeat;
        const bool is_valid_repeat_line = is_in_repeat && named_grid_line.is_first_repeat;
        if (is_layout_grid_ || is_not_in_repeat || is_valid_repeat_line) {
            line_names_value.Append(*MakeGarbageCollected<CSSCustomIdentValue>(AtomicString(named_grid_line.line_name)));
        }
    }
}

void OrderedNamedLinesCollector::CollectLineNamesForIndex(
    cssvalue::CSSBracketedValueList& line_names_value, wtf_size_t i, NamedLinesType type, bool is_in_repeat) const
{
    ...
    cssvalue::CSSBracketedValueList& line_names_value, wtf_size_t i, NamedLinesType type, bool is_in_repeat) const
{
    ...
    if (i > insertion_point_ + auto_repeat_total_tracks_) {
        AppendLines(line_names_value, i - (auto_repeat_total_tracks_ - 1), NamedLinesType::kNamedLines);
        return;
    }
    ...
    if (i == insertion_point_ + auto_repeat_total_tracks_) {
        AppendLines(line_names_value, auto_repeat_track_list_length_, NamedLinesType::kAutoRepeatNamedLines);
        AppendLines(line_names_value, insertion_point_ + 1, NamedLinesType::kNamedLines);
        return;
    }

    // Handle tracks in the auto repeat range.
    wtf_size_t auto_repeat_index_in_first_repetition = (i - insertion_point_) % auto_repeat_track_list_length_;
    if (!auto_repeat_index_in_first_repetition && i > insertion_point_) {
        AppendLines(line_names_value, auto_repeat_track_list_length_, NamedLinesType::kAutoRepeatNamedLines);
    }
    AppendLines(line_names_value, auto_repeat_index_in_first_repetition, NamedLinesType::kAutoRepeatNamedLines);
}

void AddValuesForNamedGridLinesAtIndex(OrderedNamedLinesCollector& collector, wtf_size_t i, CSSValueList& list, NamedLinesType type, bool is_in_repeat = false)
{
    if (collector.IsSubgriddedAxis()) {
        // Skip collapsed lines at the auto repeat insertion point.
        if (i == collector.InsertionPoint() && collector.HasCollapsedAutoRepeatNamedLines()) {
            return;
        }
...
CSSValue* ComputedStyleUtils::ValueForGridAutoTrackList(GridTrackSizingDirection track_direction, const LayoutObject* layout_object, const ComputedStyle& style)
{
    CSSValueList* list = CSSValueList::CreateSpaceSeparated();
    const NGGridTrackList& auto_track_list = track_direction == kForColumns ? style.GridAutoColumns() : style.GridAutoRows();
    if (auto_track_list.RepeaterCount() == 1) {
        for (wtf_size_t i = 0; i < auto_track_list.RepeatSize(0); ++i) {
            list->Append(*SpecifiedValueForGridTrackSize(auto_track_list.RepeatTrackSize(0, i), style));
        }
    }
    return list;
}

void PopulateGridTrackListUsedValues(CSSValueList* list, OrderedNamedLinesCollector& collector, const Vector<LayoutUnit, 1>& tracks, const ComputedStyle& style,
    wtf_size_t start, wtf_size_t end, int offset, bool discard_line_names)
{
    DCHECK_LE(start, end);
    ...
    for (wtf_size_t i = start; i < end; ++i) {
        if (!discard_line_names && (offset >= 0 || i >= static_cast<wtf_size_t>(-offset))) {
            AddValuesForNamedGridLinesAtIndex(collector, i + offset, *list, NamedLinesType::kNamedLines);
        }
    ...
    if (!collector.IsSubgriddedAxis() && (!discard_line_names && (offset >= 0 || end >= static_cast<wtf_size_t>(-offset)))) {
        AddValuesForNamedGridLinesAtIndex(collector, end + offset, *list, NamedLinesType::kNamedLines);
    }
}

void PopulateNonRepeater(CSSValueList* list, OrderedNamedLinesCollector& collector, const blink::NGGridTrackList& track_list, wtf_size_t repeater_index,
    wtf_size_t track_index, const ComputedStyle& style)
{
    DCHECK_EQ(track_list.RepeatType(repeater_index), NGGridTrackRepeater::RepeatType::kNoRepeat);

    AddValuesForNamedGridLinesAtIndex(collector, track_index, *list, NamedLinesType::kNamedLines);
    // Subgrid definitions do not include track sizes.
    if (!track_list.IsSubgriddedAxis()) {
        list->Append(*ComputedStyleUtils::SpecifiedValueForGridTrackSize(track_list.RepeatTrackSize(repeater_index, 0), style));
    }
}

void PopulateAutoRepeater(
    CSSValueList* list, OrderedNamedLinesCollector& collector, const blink::NGGridTrackList& track_list, wtf_size_t repeater_index, const ComputedStyle& style)
{
    blink::NGGridTrackRepeater::RepeatType repeat_type = track_list.RepeatType(repeater_index);
    DCHECK(repeat_type == NGGridTrackRepeater::RepeatType::kAutoFill || repeat_type == NGGridTrackRepeater::RepeatType::kAutoFit);

    const bool is_subgrid = track_list.IsSubgriddedAxis();
    CSSValueList* repeated_values;
    wtf_size_t repeat_size = is_subgrid ? track_list.LineNameIndicesCount(repeater_index) : track_list.RepeatSize(repeater_index);

    repeated_values = MakeGarbageCollected<cssvalue::CSSGridAutoRepeatValue>(
        repeat_type == NGGridTrackRepeater::RepeatType::kAutoFill ? CSSValueID::kAutoFill : CSSValueID::kAutoFit);
    ...
    for (wtf_size_t i = 0; i < repeat_size; ++i) {
        AddValuesForNamedGridLinesAtIndex(collector, i, *repeated_values, NamedLinesType::kAutoRepeatNamedLines);

        // Subgrids do not support track sizes.
        if (!is_subgrid) {
            const GridTrackSize& track_size = track_list.RepeatTrackSize(repeater_index, i);
            repeated_values->Append(*ComputedStyleUtils::SpecifiedValueForGridTrackSize(track_size, style));
        }
    }

    // Add any additional auto repeat line names after size definitions.
    for (wtf_size_t i = repeat_size; i < collector.AutoRepeatNamedLinesCount(); ++i) {
        AddValuesForNamedGridLinesAtIndex(collector, i, *repeated_values, NamedLinesType::kAutoRepeatNamedLines);
    }
    // Subgrids allow for empty line definitions.
    if (is_subgrid && repeat_size == 0) {
        repeated_values->Append(*MakeGarbageCollected<cssvalue::CSSBracketedValueList>());
    }
...
wtf_size_t PopulateIntegerRepeater(CSSValueList* list, OrderedNamedLinesCollector& collector, const blink::NGGridTrackList& track_list,
    wtf_size_t repeater_index, wtf_size_t track_index, const ComputedStyle& style)
{
    const bool is_subgrid = track_list.IsSubgriddedAxis();
    CSSValueList* repeated_values;
    ...
    repeated_values = MakeGarbageCollected<cssvalue::CSSGridIntegerRepeatValue>(number_of_repetitions);

    // Line names for integer repeats get expanded and interspersed with
    // non-repeaters in the track list.
    for (wtf_size_t i = 0; i < repeat_size; ++i) {
        AddValuesForNamedGridLinesAtIndex(collector, track_index + i, *repeated_values, NamedLinesType::kNamedLines, /* is_in_repeat */ true);
    ...
    if (!is_subgrid) {
        AddValuesForNamedGridLinesAtIndex(collector, track_index + repeat_size, *repeated_values, NamedLinesType::kNamedLines, /* is_in_repeat */ true);
...
void PopulateGridTrackListComputedValues(
    CSSValueList* list, OrderedNamedLinesCollector& collector, const blink::NGGridTrackList& track_list, const ComputedStyle& style)
{
    ...
    for (wtf_size_t i = 0; i < track_list.RepeaterCount(); ++i) {
        ...
        case NGGridTrackRepeater::RepeatType::kInteger:
            ...
            // non-repeaters.
            track_index += PopulateIntegerRepeater(list, collector, track_list, i, track_index, style);
            break;
...
CSSValue* ComputedStyleUtils::ValueForGridTrackList(
    GridTrackSizingDirection direction, const LayoutObject* layout_object, const ComputedStyle& style, bool force_computed_value)
{
    const bool is_for_columns = direction == kForColumns;
    const ComputedGridTrackList& computed_grid_track_list = is_for_columns ? style.GridTemplateColumns() : style.GridTemplateRows();
    const auto* grid = DynamicTo<LayoutGrid>(layout_object);

    // Handle the 'none' case.
    bool is_track_list_empty = !computed_grid_track_list.track_list.RepeaterCount();
    if (grid && is_track_list_empty) {
        ...
        const Vector<LayoutUnit>& positions = is_for_columns ? grid->ColumnPositions() : grid->RowPositions();
        is_track_list_empty = positions.size() == 1;
    }

    const bool is_subgrid_specified = computed_grid_track_list.IsSubgriddedAxis();
    const bool is_subgrid_valid = (grid && grid->HasCachedPlacementData()) ? grid->CachedPlacementData().SubgridSpanSize(direction) != kNotFound : false;
    ...
    if (is_subgrid || (is_subgrid_specified && !grid)) {
        list->Append(*MakeGarbageCollected<CSSIdentifierValue>(CSSValueID::kSubgrid));
    } else if (!is_subgrid_specified && is_track_list_empty) {
        return CSSIdentifierValue::Create(CSSValueID::kNone);
    }

    wtf_size_t auto_repeat_insertion_point = computed_grid_track_list.auto_repeat_insertion_point;
    ...
    if (grid && !force_computed_value) {
        ...
        wtf_size_t auto_repeat_track_list_length = ng_track_list.AutoRepeatTrackCount();
        ...
        if (is_subgrid) {
            ...
            end_index = grid->ExplicitGridEndForDirection(direction) - grid->ExplicitGridStartForDirection(direction) + 1;
        }
        ...
        OrderedNamedLinesCollectorInGridLayout collector(computed_grid_track_list.ordered_named_grid_lines,
            computed_grid_track_list.auto_repeat_ordered_named_grid_lines, auto_repeat_insertion_point, grid->AutoRepeatCountForDirection(direction),
        ...
        int offset = -base::checked_cast<int>(grid->ExplicitGridStartForDirection(direction));
        ...
        const bool discard_line_names = grid && is_subgrid_specified && !is_subgrid_valid;
        PopulateGridTrackListUsedValues(list, collector, track_sizes, style, start_index, end_index, offset, discard_line_names);
        return list;
    }

    OrderedNamedLinesCollector collector(
        computed_grid_track_list.ordered_named_grid_lines, computed_grid_track_list.auto_repeat_ordered_named_grid_lines, is_subgrid_specified, !!grid);
    PopulateGridTrackListComputedValues(list, collector, ng_track_list, style);
    return list;
}

CSSValue* ComputedStyleUtils::ValueForGridPosition(const GridPosition& position)
{
    if (position.IsAuto()) {
        return CSSIdentifierValue::Create(CSSValueID::kAuto);
    }
    ...
    if (position.IsSpan()) {
        ...
        if (position.SpanPosition() != 1 || !has_named_line) {
            list->Append(*CSSNumericLiteralValue::Create(position.SpanPosition(), CSSPrimitiveValue::UnitType::kNumber));
        }
    } else {
        list->Append(*CSSNumericLiteralValue::Create(position.IntegerPosition(), CSSPrimitiveValue::UnitType::kNumber));
    }

    if (has_named_line) {
        list->Append(*MakeGarbageCollected<CSSCustomIdentValue>(position.NamedGridLine()));
    }
    return list;
}

CSSValue* ComputedStyleUtils::ValueForMasonrySlack(const std::optional<Length>& slack_length, const ComputedStyle& style)
{
    return slack_length ? ZoomAdjustedPixelValueForLength(*slack_length, style) : CSSIdentifierValue::Create(CSSValueID::kNormal);
}

CSSValue* ComputedStyleUtils::ValueForMasonryTrackList(const LayoutObject* layout_object, const ComputedStyle& style)
{
    ...
    OrderedNamedLinesCollector collector(computed_track_list.ordered_named_grid_lines, computed_track_list.auto_repeat_ordered_named_grid_lines,
        computed_track_list.IsSubgriddedAxis(), /*is_layout_grid=*/false);

    PopulateGridTrackListComputedValues(list, collector, computed_track_list.track_list, style);
    return list;
}
...
static bool IsSVGObjectWithWidthAndHeight(const LayoutObject& layout_object)
{
    ...
    return layout_object.IsSVGImage() || layout_object.IsSVGForeignObject() || (layout_object.IsSVGShape() && IsA<SVGRectElement>(layout_object.GetNode()));
}

gfx::SizeF ComputedStyleUtils::UsedBoxSize(const LayoutObject& layout_object)
{
    if (layout_object.IsSVGChild() && IsSVGObjectWithWidthAndHeight(layout_object)) {
    ...
    return gfx::SizeF(box.StyleRef().BoxSizing() == EBoxSizing::kBorderBox ? box.PhysicalBorderBoxRect().size : box.ComputedCSSContentBoxRect().size);
}

CSSValue* ComputedStyleUtils::RenderTextDecorationFlagsToCSSValue(TextDecorationLine text_decoration)
{
...
CSSValue* ComputedStyleUtils::ValueForTextDecorationStyle(ETextDecorationStyle text_decoration_style)
{
...
CSSValue* ComputedStyleUtils::ValueForTextDecorationSkipInk(ETextDecorationSkipInk text_decoration_skip_ink)
{
...
CSSValue* ComputedStyleUtils::TouchActionFlagsToCSSValue(TouchAction touch_action)
{
    CSSValueList* list = CSSValueList::CreateSpaceSeparated();
...
CSSValue* ComputedStyleUtils::ValueForWillChange(
    const Vector<CSSPropertyID>& will_change_properties, bool will_change_contents, bool will_change_scroll_position)
{
    CSSValueList* list = CSSValueList::CreateCommaSeparated();
    ...
    for (wtf_size_t i = 0; i < will_change_properties.size(); ++i) {
        list->Append(*MakeGarbageCollected<CSSCustomIdentValue>(will_change_properties[i]));
    }
    if (!list->length()) {
        list->Append(*CSSIdentifierValue::Create(CSSValueID::kAuto));
    }
...
CSSValue* CreateAnimationValueList(const Vector<T, C>& values, Func item_func, Args&&... args)
{
    CSSValueList* list = CSSValueList::CreateCommaSeparated();
...
CSSValue* ComputedStyleUtils::ValueForAnimationDelay(const Timing::Delay& delay)
{
    return CSSNumericLiteralValue::Create(delay.AsTimeValue().InSecondsF(), CSSPrimitiveValue::UnitType::kSeconds);
}

CSSValue* ComputedStyleUtils::ValueForAnimationDelayList(const CSSTimingData* timing_data)
{
    return CreateAnimationValueList(
        timing_data ? timing_data->DelayStartList() : Vector<Timing::Delay, 1> { CSSTimingData::InitialDelayStart() }, &ValueForAnimationDelay);
}

CSSValue* ComputedStyleUtils::ValueForAnimationDirection(Timing::PlaybackDirection direction)
{
    switch (direction) {
...
CSSValue* ComputedStyleUtils::ValueForAnimationDirectionList(const CSSAnimationData* animation_data)
{
    return CreateAnimationValueList(
        animation_data ? animation_data->DirectionList() : Vector<Timing::PlaybackDirection> { CSSAnimationData::InitialDirection() },
        &ValueForAnimationDirection);
}

CSSValue* ComputedStyleUtils::ValueForAnimationDuration(const std::optional<double>& duration, bool resolve_auto_to_zero)
{
    std::optional<double> resolved_duration = (!duration.has_value() && resolve_auto_to_zero) ? 0 : duration;
    if (!resolved_duration.has_value()) {
        return CSSIdentifierValue::Create(CSSValueID::kAuto);
    }
    return CSSNumericLiteralValue::Create(resolved_duration.value(), CSSPrimitiveValue::UnitType::kSeconds);
}

CSSValue* ComputedStyleUtils::ValueForAnimationDurationList(const CSSAnimationData* animation_data, CSSValuePhase phase)
{
    ...
    bool resolve_auto_to_zero = (phase == CSSValuePhase::kResolvedValue) && (!animation_data || animation_data->HasSingleInitialTimeline());
    return CreateAnimationValueList(animation_data ? animation_data->DurationList() : Vector<std::optional<double>, 1> { CSSAnimationData::InitialDuration() },
        ValueForAnimationDuration, resolve_auto_to_zero);
}

CSSValue* ComputedStyleUtils::ValueForAnimationDurationList(const CSSTransitionData* transition_data)
{
    return CreateAnimationValueList(
        transition_data ? transition_data->DurationList() : Vector<std::optional<double>, 1> { CSSTransitionData::InitialDuration() },
        ValueForAnimationDuration,
        /* resolve_auto_to_zero */ false);
}

CSSValue* ComputedStyleUtils::ValueForAnimationFillMode(Timing::FillMode fill_mode)
{
    switch (fill_mode) {
...
CSSValue* ComputedStyleUtils::ValueForAnimationFillModeList(const CSSAnimationData* animation_data)
{
    return CreateAnimationValueList(
        animation_data ? animation_data->FillModeList() : Vector<Timing::FillMode> { CSSAnimationData::InitialFillMode() }, &ValueForAnimationFillMode);
}

CSSValue* ComputedStyleUtils::ValueForAnimationIterationCount(double iteration_count)
{
    if (iteration_count == std::numeric_limits<double>::infinity()) {
        return CSSIdentifierValue::Create(CSSValueID::kInfinite);
    }
    return CSSNumericLiteralValue::Create(iteration_count, CSSPrimitiveValue::UnitType::kNumber);
}

CSSValue* ComputedStyleUtils::ValueForAnimationIterationCountList(const CSSAnimationData* animation_data)
{
    return CreateAnimationValueList(
        animation_data ? animation_data->IterationCountList() : Vector<double> { CSSAnimationData::InitialIterationCount() }, &ValueForAnimationIterationCount);
}

CSSValue* ComputedStyleUtils::ValueForAnimationPlayState(EAnimPlayState play_state)
{
...
CSSValue* ComputedStyleUtils::ValueForAnimationPlayStateList(const CSSAnimationData* animation_data)
{
    return CreateAnimationValueList(
        animation_data ? animation_data->PlayStateList() : Vector<EAnimPlayState> { CSSAnimationData::InitialPlayState() }, &ValueForAnimationPlayState);
}

namespace {

CSSValue* ValueForAnimationRange(const std::optional<TimelineOffset>& offset, const ComputedStyle& style, const Length& default_offset)
{
    if (!offset.has_value()) {
    ...
    if (offset->offset != default_offset) {
        list->Append(*ComputedStyleUtils::ZoomAdjustedPixelValueForLength(offset->offset, style));
    }
    return list;
}
...
CSSValue* ComputedStyleUtils::ValueForAnimationRangeStart(const std::optional<TimelineOffset>& offset, const ComputedStyle& style)
{
    return ValueForAnimationRange(offset, style, Length::Percent(0.0));
}

CSSValue* ComputedStyleUtils::ValueForAnimationRangeStartList(const CSSAnimationData* animation_data, const ComputedStyle& style)
{
    return CreateAnimationValueList(
        animation_data ? animation_data->RangeStartList() : Vector<std::optional<TimelineOffset>> { CSSAnimationData::InitialRangeStart() },
        &ValueForAnimationRangeStart, style);
}

CSSValue* ComputedStyleUtils::ValueForAnimationRangeEnd(const std::optional<TimelineOffset>& offset, const ComputedStyle& style)
{
    return ValueForAnimationRange(offset, style, Length::Percent(100.0));
}

CSSValue* ComputedStyleUtils::ValueForAnimationRangeEndList(const CSSAnimationData* animation_data, const ComputedStyle& style)
{
    return CreateAnimationValueList(
        animation_data ? animation_data->RangeEndList() : Vector<std::optional<TimelineOffset>> { CSSAnimationData::InitialRangeEnd() },
        &ValueForAnimationRangeEnd, style);
}

CSSValue* ComputedStyleUtils::ValueForAnimationTimingFunction(const scoped_refptr<TimingFunction>& timing_function)
{
    switch (timing_function->GetType()) {
    case TimingFunction::Type::CUBIC_BEZIER: {
        const auto* bezier_timing_function = To<CubicBezierTimingFunction>(timing_function.get());
        if (bezier_timing_function->GetEaseType() != CubicBezierTimingFunction::EaseType::CUSTOM) {
            CSSValueID value_id = CSSValueID::kInvalid;
        ...
        return MakeGarbageCollected<cssvalue::CSSCubicBezierTimingFunctionValue>(
            bezier_timing_function->X1(), bezier_timing_function->Y1(), bezier_timing_function->X2(), bezier_timing_function->Y2());
    }

    case TimingFunction::Type::STEPS: {
        const auto* steps_timing_function = To<StepsTimingFunction>(timing_function.get());
        StepsTimingFunction::StepPosition position = steps_timing_function->GetStepPosition();
        ...
        return MakeGarbageCollected<cssvalue::CSSStepsTimingFunctionValue>(steps, position);
    }

    default:
        const auto* linear_timing_function = To<LinearTimingFunction>(timing_function.get());
        if (linear_timing_function->IsTrivial()) {
            return CSSIdentifierValue::Create(CSSValueID::kLinear);
        }
        return MakeGarbageCollected<cssvalue::CSSLinearTimingFunctionValue>(linear_timing_function->Points());
    }
}

CSSValue* ComputedStyleUtils::ValueForAnimationTimingFunctionList(const CSSTimingData* timing_data)
{
    return CreateAnimationValueList(
        timing_data ? timing_data->TimingFunctionList() : Vector<scoped_refptr<TimingFunction>, 1> { CSSAnimationData::InitialTimingFunction() },
        &ValueForAnimationTimingFunction);
}

CSSValue* ComputedStyleUtils::ValueForAnimationTimeline(const StyleTimeline& timeline)
{
    if (timeline.IsKeyword()) {
        DCHECK(timeline.GetKeyword() == CSSValueID::kAuto || timeline.GetKeyword() == CSSValueID::kNone);
    ...
    if (timeline.IsName()) {
        ...
        if (css_parsing_utils::IsCSSWideKeyword(name) || EqualIgnoringASCIICase(name, "auto") || EqualIgnoringASCIICase(name, "none")) {
    ...
    if (timeline.IsView()) {
        ...
        CSSValue* axis = view_data.HasDefaultAxis() ? nullptr : CSSIdentifierValue::Create(view_data.GetAxis());
        auto* inset = view_data.HasDefaultInset() ? nullptr
                                                  : MakeGarbageCollected<CSSValuePair>(CSSValue::Create(view_data.GetInset().GetStart(), 1),
                                                      CSSValue::Create(view_data.GetInset().GetEnd(), 1), CSSValuePair::kDropIdenticalValues);
    ...
    CSSValue* scroller = scroll_data.HasDefaultScroller() ? nullptr : CSSIdentifierValue::Create(scroll_data.GetScroller());
    CSSValue* axis = scroll_data.HasDefaultAxis() ? nullptr : CSSIdentifierValue::Create(scroll_data.GetAxis());

    return MakeGarbageCollected<cssvalue::CSSScrollValue>(scroller, axis);
}

CSSValue* ComputedStyleUtils::ValueForAnimationTimelineList(const CSSAnimationData* animation_data)
{
    return CreateAnimationValueList(
        animation_data ? animation_data->TimelineList() : Vector<StyleTimeline> { CSSAnimationData::InitialTimeline() }, &ValueForAnimationTimeline);
}

CSSValue* ComputedStyleUtils::ValueForTimelineInset(const TimelineInset& inset, const ComputedStyle& style)
{
    return MakeGarbageCollected<CSSValuePair>(ComputedStyleUtils::ZoomAdjustedPixelValueForLength(inset.GetStart(), style),
        ComputedStyleUtils::ZoomAdjustedPixelValueForLength(inset.GetEnd(), style), CSSValuePair::kDropIdenticalValues);
}

CSSValue* ComputedStyleUtils::SingleValueForTimelineShorthand(
    const ScopedCSSName* name, TimelineAxis axis, std::optional<TimelineInset> inset, const ComputedStyle& style)
{
    CSSValueList* list = CSSValueList::CreateSpaceSeparated();
...
CSSValueList* ComputedStyleUtils::ValuesForBorderRadiusCorner(const LengthSize& radius, const ComputedStyle& style)
{
    CSSValueList* list = CSSValueList::CreateSpaceSeparated();
    if (radius.Width().IsPercent()) {
        list->Append(*CSSNumericLiteralValue::Create(radius.Width().Percent(), CSSPrimitiveValue::UnitType::kPercentage));
    } else {
        list->Append(*ZoomAdjustedPixelValueForLength(radius.Width(), style));
    }
    if (radius.Height().IsPercent()) {
        list->Append(*CSSNumericLiteralValue::Create(radius.Height().Percent(), CSSPrimitiveValue::UnitType::kPercentage));
    } else {
...
CSSValue* ComputedStyleUtils::ValueForBorderRadiusCorner(const LengthSize& radius, const ComputedStyle& style)
{
    return MakeGarbageCollected<CSSValuePair>(
        ZoomAdjustedPixelValueForLength(radius.Width(), style), ZoomAdjustedPixelValueForLength(radius.Height(), style), CSSValuePair::kDropIdenticalValues);
}

CSSFunctionValue* ComputedStyleUtils::ValueForTransform(const gfx::Transform& matrix, float zoom, bool force_matrix3d)
{
    if (matrix.Is2dTransform() && !force_matrix3d) {
        ...
        for (double value : { unzoomed.A(), unzoomed.B(), unzoomed.C(), unzoomed.D(), unzoomed.E(), unzoomed.F() }) {
            result->Append(*CSSNumericLiteralValue::Create(value, CSSPrimitiveValue::UnitType::kNumber));
        }
        return result;
    } else {
        CSSFunctionValue* result = MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kMatrix3d);
        // CSS matrix values are returned in column-major order.
        auto unzoomed = matrix;
        unzoomed.Zoom(1.f / zoom);
        for (int i = 0; i < 16; i++) {
            result->Append(*CSSNumericLiteralValue::Create(unzoomed.ColMajorData(i), CSSPrimitiveValue::UnitType::kNumber));
        }
        return result;
    }
}

CSSValueID ComputedStyleUtils::CSSValueIDForScaleOperation(const TransformOperation::OperationType type)
{
    switch (type) {
...
CSSValueID ComputedStyleUtils::CSSValueIDForTranslateOperation(const TransformOperation::OperationType type)
{
    switch (type) {
...
CSSValueID ComputedStyleUtils::CSSValueIDForRotateOperation(const TransformOperation::OperationType type)
{
    switch (type) {
...
CSSFunctionValue* ComputedStyleUtils::ValueForTransformOperation(const TransformOperation& operation, float zoom, gfx::SizeF box_size)
{
    ...
    case TransformOperation::kScale3D: {
        ...
        if (id == CSSValueID::kScaleX || id == CSSValueID::kScale || id == CSSValueID::kScale3d) {
            result->Append(*CSSNumericLiteralValue::Create(scale.X(), CSSPrimitiveValue::UnitType::kNumber));
        }
        if (id == CSSValueID::kScaleY || (id == CSSValueID::kScale && scale.Y() != scale.X()) || id == CSSValueID::kScale3d) {
            result->Append(*CSSNumericLiteralValue::Create(scale.Y(), CSSPrimitiveValue::UnitType::kNumber));
        }
        if (id == CSSValueID::kScale3d || id == CSSValueID::kScaleZ) {
            result->Append(*CSSNumericLiteralValue::Create(scale.Z(), CSSPrimitiveValue::UnitType::kNumber));
        }
        return result;
    }
    case TransformOperation::kTranslateX:
    ...
    case TransformOperation::kTranslate3D: {
        ...
        CSSFunctionValue* result = MakeGarbageCollected<CSSFunctionValue>(id);
        if (id == CSSValueID::kTranslateX || id == CSSValueID::kTranslate || id == CSSValueID::kTranslate3d) {
            result->Append(*CSSPrimitiveValue::CreateFromLength(translate.X(), zoom));
        }
        if (id == CSSValueID::kTranslateY || (id == CSSValueID::kTranslate && (translate.Y().Value() != 0.f)) || id == CSSValueID::kTranslate3d) {
        ...
        if (id == CSSValueID::kTranslate3d || id == CSSValueID::kTranslateZ) {
            ...
            result->Append(*CSSNumericLiteralValue::Create(translate.Z() / zoom, CSSPrimitiveValue::UnitType::kPixels));
        }
        return result;
    }
    case TransformOperation::kRotateX:
    ...
    case TransformOperation::kRotate: {
        ...
        if (id == CSSValueID::kRotate3d) {
            result->Append(*CSSNumericLiteralValue::Create(rotate.X(), CSSPrimitiveValue::UnitType::kNumber));
            result->Append(*CSSNumericLiteralValue::Create(rotate.Y(), CSSPrimitiveValue::UnitType::kNumber));
            result->Append(*CSSNumericLiteralValue::Create(rotate.Z(), CSSPrimitiveValue::UnitType::kNumber));
        }
        result->Append(*CSSNumericLiteralValue::Create(rotate.Angle(), CSSPrimitiveValue::UnitType::kDegrees));
        return result;
    }
    ...
    case TransformOperation::kSkewX: {
        ...
        result->Append(*CSSNumericLiteralValue::Create(skew.AngleX(), CSSPrimitiveValue::UnitType::kDegrees));
        return result;
    }
    case TransformOperation::kSkewY: {
        const auto& skew = To<SkewTransformOperation>(operation);
        auto* result = MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kSkewY);
        result->Append(*CSSNumericLiteralValue::Create(skew.AngleY(), CSSPrimitiveValue::UnitType::kDegrees));
        return result;
    }
    ...
    case TransformOperation::kPerspective: {
        ...
        auto* result = MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kPerspective);
        if (perspective.Perspective()) {
            result->Append(*CSSNumericLiteralValue::Create(*perspective.Perspective() / zoom, CSSPrimitiveValue::UnitType::kPixels));
        } else {
            result->Append(*CSSIdentifierValue::Create(CSSValueID::kNone));
        }
        return result;
...
CSSValue* ComputedStyleUtils::ValueForTransformList(const TransformOperations& transform_list, float zoom, gfx::SizeF box_size)
{
...
CSSValue* ComputedStyleUtils::ValueForTransformFunction(const TransformOperations& transform_list)
{
    CHECK_EQ(transform_list.Operations().size(), 1u);
    return ValueForTransformOperation(*transform_list.Operations()[0], 1, gfx::SizeF());
}

gfx::RectF ComputedStyleUtils::ReferenceBoxForTransform(const LayoutObject& layout_object)
{
...
CSSValue* ComputedStyleUtils::ComputedTransformList(const ComputedStyle& style, const LayoutObject* layout_object)
{
    ...
    return ValueForTransformList(style.Transform(), style.EffectiveZoom(), box_size);
}

CSSValue* ComputedStyleUtils::ResolvedTransform(const LayoutObject* layout_object, const ComputedStyle& style)
{
    ...
    const auto* layout_box = layout_object->IsSVGChild() ? nullptr : DynamicTo<LayoutBox>(*layout_object);

    gfx::Transform transform;
    style.ApplyTransform(transform, layout_box, reference_box, ComputedStyle::kIncludeTransformOperations, ComputedStyle::kExcludeTransformOrigin,
        ComputedStyle::kExcludeMotionPath, ComputedStyle::kExcludeIndependentTransformProperties);
...
CSSValue* ComputedStyleUtils::CreateTransitionPropertyValue(const CSSTransitionData::TransitionProperty& property)
{
    ...
    DCHECK_EQ(property.property_type, CSSTransitionData::kTransitionKnownProperty);
    return MakeGarbageCollected<CSSCustomIdentValue>(CSSUnresolvedProperty::Get(property.unresolved_property).GetPropertyNameAtomicString());
}

CSSValue* ComputedStyleUtils::CreateTransitionBehaviorValue(const CSSTransitionData::TransitionBehavior& type)
{
    switch (type) {
...
CSSValue* ComputedStyleUtils::ValueForTransitionProperty(const CSSTransitionData* transition_data)
{
    CSSValueList* list = CSSValueList::CreateCommaSeparated();
    if (transition_data) {
        for (wtf_size_t i = 0; i < transition_data->PropertyList().size(); ++i) {
            list->Append(*CreateTransitionPropertyValue(transition_data->PropertyList()[i]));
        }
    } else {
...
CSSValue* ComputedStyleUtils::ValueForTransitionBehavior(const CSSTransitionData* transition_data)
{
    CSSValueList* list = CSSValueList::CreateCommaSeparated();
    if (transition_data) {
...
CSSValue* ComputedStyleUtils::ValueForContentData(const ComputedStyle& style, bool allow_visited_style, CSSValuePhase value_phase)
{
    ...
    for (const ContentData* content_data = style.GetContentData(); content_data; content_data = content_data->Next()) {
        if (content_data->IsCounter()) {
            const CounterContentData& counter = To<CounterContentData>(*content_data);
            auto* identifier = MakeGarbageCollected<CSSCustomIdentValue>(counter.Identifier());
            auto* separator = MakeGarbageCollected<CSSStringValue>(counter.Separator());
            auto* list_style = MakeGarbageCollected<CSSCustomIdentValue>(counter.ListStyle());
            list->Append(*MakeGarbageCollected<cssvalue::CSSCounterValue>(identifier, list_style, separator));
        } else if (content_data->IsImage()) {
            const StyleImage* image = To<ImageContentData>(content_data)->GetImage();
            DCHECK(image);
            list->Append(*image->ComputedCSSValue(style, allow_visited_style, value_phase));
        } else if (content_data->IsText()) {
            list->Append(*MakeGarbageCollected<CSSStringValue>(To<TextContentData>(content_data)->GetText()));
            ...
            alt_text = MakeGarbageCollected<CSSStringValue>(To<AltTextContentData>(content_data)->ConcatenateAltText());
            break;
        } else {
            NOTREACHED();
        }
...
CSSValue* ComputedStyleUtils::ValueForCounterDirectives(const ComputedStyle& style, CountersAttachmentContext::Type type)
{
    ...
    for (const auto& item : *map) {
        ...
        list->Append(*MakeGarbageCollected<CSSValuePair>(MakeGarbageCollected<CSSCustomIdentValue>(item.key),
            CSSNumericLiteralValue::Create((double)number, CSSPrimitiveValue::UnitType::kInteger), CSSValuePair::IdenticalValuesPolicy::kDropIdenticalValues));
    }
...
CSSValue* ComputedStyleUtils::ValueForShape(const ComputedStyle& style, bool allow_visited_style, ShapeValue* shape_value, CSSValuePhase value_phase)
{
    if (!shape_value) {
        return CSSIdentifierValue::Create(CSSValueID::kNone);
    }
    ...
    if (shape_value->GetType() == ShapeValue::kImage) {
        if (shape_value->GetImage()) {
            return shape_value->GetImage()->ComputedCSSValue(style, allow_visited_style, value_phase);
        }
        return CSSIdentifierValue::Create(CSSValueID::kNone);
    }
...
CSSValueList* ComputedStyleUtils::ValueForBorderRadiusShorthand(const ComputedStyle& style)
{
    CSSValueList* list = CSSValueList::CreateSlashSeparated();

    bool show_horizontal_bottom_left = style.BorderTopRightRadius().Width() != style.BorderBottomLeftRadius().Width();
    bool show_horizontal_bottom_right = show_horizontal_bottom_left || (style.BorderBottomRightRadius().Width() != style.BorderTopLeftRadius().Width());
    bool show_horizontal_top_right = show_horizontal_bottom_right || (style.BorderTopRightRadius().Width() != style.BorderTopLeftRadius().Width());

    bool show_vertical_bottom_left = style.BorderTopRightRadius().Height() != style.BorderBottomLeftRadius().Height();
    bool show_vertical_bottom_right = show_vertical_bottom_left || (style.BorderBottomRightRadius().Height() != style.BorderTopLeftRadius().Height());
    bool show_vertical_top_right = show_vertical_bottom_right || (style.BorderTopRightRadius().Height() != style.BorderTopLeftRadius().Height());

    CSSValueList* top_left_radius = ValuesForBorderRadiusCorner(style.BorderTopLeftRadius(), style);
    CSSValueList* top_right_radius = ValuesForBorderRadiusCorner(style.BorderTopRightRadius(), style);
    CSSValueList* bottom_right_radius = ValuesForBorderRadiusCorner(style.BorderBottomRightRadius(), style);
    CSSValueList* bottom_left_radius = ValuesForBorderRadiusCorner(style.BorderBottomLeftRadius(), style);
...
CSSValue* ComputedStyleUtils::StrokeDashArrayToCSSValueList(const SVGDashArray& dashes, const ComputedStyle& style)
{
...
const CSSValue* ComputedStyleUtils::ValueForSVGPaint(const SVGPaint& paint, const ComputedStyle& style)
{
    switch (paint.type) {
    case SVGPaintType::kColor:
        return CurrentColorOrValidColor(style, paint.GetColor(), CSSValuePhase::kComputedValue);
    case SVGPaintType::kNone:
        return CSSIdentifierValue::Create(CSSValueID::kNone);
    case SVGPaintType::kUriNone:
    case SVGPaintType::kUriColor: {
        CSSValueList* values = CSSValueList::CreateSpaceSeparated();
        values->Append(*MakeGarbageCollected<cssvalue::CSSURIValue>(CSSUrlData(paint.GetUrl())));
        values->Append(paint.type == SVGPaintType::kUriNone ? *CSSIdentifierValue::Create(CSSValueID::kNone)
                                                            : *CurrentColorOrValidColor(style, paint.GetColor(), CSSValuePhase::kComputedValue));
        return values;
    }
    case SVGPaintType::kUri:
        return MakeGarbageCollected<cssvalue::CSSURIValue>(CSSUrlData(paint.GetUrl()));
...
CSSValue* ComputedStyleUtils::ValueForSVGResource(const StyleSVGResource* resource)
{
    if (resource) {
        return MakeGarbageCollected<cssvalue::CSSURIValue>(CSSUrlData(resource->Url()));
    }
    return CSSIdentifierValue::Create(CSSValueID::kNone);
}
...
template <typename T> const CSSValue* GetGapDecorationPropertyValue(const T& value, const ComputedStyle& style, CSSValuePhase value_phase);

template <> const CSSValue* GetGapDecorationPropertyValue(const StyleColor& value, const ComputedStyle& style, CSSValuePhase value_phase)
{
    return ComputedStyleUtils::CurrentColorOrValidColor(style, value, value_phase);
}

template <typename T> void PopulateNonRepeaterGapData(CSSValueList* list, const GapData<T>& gap_data, const ComputedStyle& style, CSSValuePhase value_phase)
{
    const CSSValue* value = GetGapDecorationPropertyValue(gap_data.GetValue(), style, value_phase);
    list->Append(*value);
}

template <typename T> void PopulateRepeaterGapData(CSSValueList* list, const GapData<T>& gap_data, const ComputedStyle& style, CSSValuePhase value_phase)
{
    CSSPrimitiveValue* repetitions = nullptr;

    if (!gap_data.GetValueRepeater()->IsAutoRepeater()) {
        repetitions = CSSNumericLiteralValue::Create(gap_data.GetValueRepeater()->RepeatCount(), CSSPrimitiveValue::UnitType::kNumber);
    }
    ...
    for (const auto& value : gap_data.GetValueRepeater()->RepeatedValues()) {
        const CSSValue* css_value = GetGapDecorationPropertyValue(value, style, value_phase);
        repeated_values->Append(*css_value);
    }

    CSSValue* repeater_value = MakeGarbageCollected<cssvalue::CSSRepeatValue>(repetitions, *repeated_values);

    list->Append(*repeater_value);
}

template <typename T>
const CSSValue* ValueForGapDecorationPropertyDataList(const GapDataList<T>& gap_color_list, const ComputedStyle& style, CSSValuePhase value_phase)
{
    ...
    if (!RuntimeEnabledFeatures::CSSGapDecorationEnabled()) {
        return GetGapDecorationPropertyValue(gap_color_list.GetLegacyValue(), style, value_phase);
    }
...
const CSSValue* ComputedStyleUtils::ValueForGapDecorationColorDataList(
    const GapDataList<StyleColor>& gap_color_list, const ComputedStyle& style, CSSValuePhase value_phase)
{
    return ValueForGapDecorationPropertyDataList(gap_color_list, style, value_phase);
}

CSSValue* ComputedStyleUtils::ValueForShadowData(const ShadowData& shadow, const ComputedStyle& style, bool use_spread, CSSValuePhase value_phase)
{
    ...
    CSSPrimitiveValue* spread = use_spread ? ZoomAdjustedPixelValue(shadow.Spread(), style) : nullptr;
    CSSIdentifierValue* shadow_style = shadow.Style() == ShadowStyle::kNormal ? nullptr : CSSIdentifierValue::Create(CSSValueID::kInset);
    const CSSValue* color = CurrentColorOrValidColor(style, shadow.GetColor(), value_phase);
    return MakeGarbageCollected<CSSShadowValue>(x, y, blur, spread, shadow_style, color);
}

CSSValue* ComputedStyleUtils::ValueForShadowList(const ShadowList* shadow_list, const ComputedStyle& style, bool use_spread, CSSValuePhase value_phase)
{
    if (!shadow_list) {
        return CSSIdentifierValue::Create(CSSValueID::kNone);
    }
    ...
    for (wtf_size_t i = 0; i < shadow_count; ++i) {
        list->Append(*ValueForShadowData(shadow_list->Shadows()[i], style, use_spread, value_phase));
    }
    return list;
}

CSSValue* ComputedStyleUtils::ValueForFilter(const ComputedStyle& style, const FilterOperations& filter_operations)
{
    ...
    for (const auto& operation : filter_operations.Operations()) {
        ...
        case FilterOperation::OperationType::kReference:
            list->Append(*MakeGarbageCollected<cssvalue::CSSURIValue>(CSSUrlData(AtomicString(To<ReferenceFilterOperation>(filter_operation)->Url()))));
            continue;
        case FilterOperation::OperationType::kGrayscale:
            filter_value = MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kGrayscale);
            filter_value->Append(
                *CSSNumericLiteralValue::Create(To<BasicColorMatrixFilterOperation>(filter_operation)->Amount(), CSSPrimitiveValue::UnitType::kNumber));
            break;
        case FilterOperation::OperationType::kSepia:
            filter_value = MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kSepia);
            filter_value->Append(
                *CSSNumericLiteralValue::Create(To<BasicColorMatrixFilterOperation>(filter_operation)->Amount(), CSSPrimitiveValue::UnitType::kNumber));
            break;
        case FilterOperation::OperationType::kSaturate:
            filter_value = MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kSaturate);
            filter_value->Append(
                *CSSNumericLiteralValue::Create(To<BasicColorMatrixFilterOperation>(filter_operation)->Amount(), CSSPrimitiveValue::UnitType::kNumber));
            break;
        case FilterOperation::OperationType::kHueRotate:
            filter_value = MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kHueRotate);
            filter_value->Append(
                *CSSNumericLiteralValue::Create(To<BasicColorMatrixFilterOperation>(filter_operation)->Amount(), CSSPrimitiveValue::UnitType::kDegrees));
            break;
        case FilterOperation::OperationType::kInvert:
            filter_value = MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kInvert);
            filter_value->Append(
                *CSSNumericLiteralValue::Create(To<BasicComponentTransferFilterOperation>(filter_operation)->Amount(), CSSPrimitiveValue::UnitType::kNumber));
            break;
        case FilterOperation::OperationType::kOpacity:
            filter_value = MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kOpacity);
            filter_value->Append(
                *CSSNumericLiteralValue::Create(To<BasicComponentTransferFilterOperation>(filter_operation)->Amount(), CSSPrimitiveValue::UnitType::kNumber));
            break;
        case FilterOperation::OperationType::kBrightness:
            filter_value = MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kBrightness);
            filter_value->Append(
                *CSSNumericLiteralValue::Create(To<BasicComponentTransferFilterOperation>(filter_operation)->Amount(), CSSPrimitiveValue::UnitType::kNumber));
            break;
        case FilterOperation::OperationType::kContrast:
            filter_value = MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kContrast);
            filter_value->Append(
                *CSSNumericLiteralValue::Create(To<BasicComponentTransferFilterOperation>(filter_operation)->Amount(), CSSPrimitiveValue::UnitType::kNumber));
            break;
        case FilterOperation::OperationType::kBlur:
            filter_value = MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kBlur);
            filter_value->Append(*ZoomAdjustedPixelValue(To<BlurFilterOperation>(filter_operation)->StdDeviation().Value(), style));
            break;
        case FilterOperation::OperationType::kDropShadow: {
            const auto& drop_shadow_operation = To<DropShadowFilterOperation>(*filter_operation);
            filter_value = MakeGarbageCollected<CSSFunctionValue>(CSSValueID::kDropShadow);
            // We want our computed style to look like that of a text shadow (has
            // neither spread nor inset style).
            filter_value->Append(*ValueForShadowData(drop_shadow_operation.Shadow(), style, false, CSSValuePhase::kComputedValue));
            break;
        }
        default:
            NOTREACHED();
...
CSSValue* ComputedStyleUtils::ValueForScrollSnapType(const cc::ScrollSnapType& type, const ComputedStyle& style)
{
    if (!type.is_none) {
        ...
        return MakeGarbageCollected<CSSValuePair>(
            CSSIdentifierValue::Create(type.axis), CSSIdentifierValue::Create(type.strictness), CSSValuePair::kDropIdenticalValues);
    }
    return CSSIdentifierValue::Create(CSSValueID::kNone);
}

CSSValue* ComputedStyleUtils::ValueForScrollSnapAlign(const cc::ScrollSnapAlign& align, const ComputedStyle& style)
{
    return MakeGarbageCollected<CSSValuePair>(
        CSSIdentifierValue::Create(align.alignment_block), CSSIdentifierValue::Create(align.alignment_inline), CSSValuePair::kDropIdenticalValues);
}
...
CSSValue* ComputedStyleUtils::ValueForPageBreakBetween(EBreakBetween break_value)
{
    switch (break_value) {
...
// property.
CSSValue* ComputedStyleUtils::ValueForWebkitColumnBreakBetween(EBreakBetween break_value)
{
    switch (break_value) {
    case EBreakBetween::kAvoidPage:
...
CSSValue* ComputedStyleUtils::ValueForPageBreakInside(EBreakInside break_value)
{
    switch (break_value) {
...
CSSValue* ComputedStyleUtils::ValueForWebkitColumnBreakInside(EBreakInside break_value)
{
    switch (break_value) {
    case EBreakInside::kAvoidPage:
        return nullptr;
...
bool ComputedStyleUtils::WidthOrHeightShouldReturnUsedValue(const LayoutObject* object)
{
...
CSSValueList* ComputedStyleUtils::ValuesForShorthandProperty(
    const StylePropertyShorthand& shorthand, const ComputedStyle& style, const LayoutObject* layout_object, bool allow_visited_style, CSSValuePhase value_phase)
{
    CSSValueList* list = CSSValueList::CreateSpaceSeparated();
...
CSSValuePair* ComputedStyleUtils::ValuesForGapShorthand(
    const StylePropertyShorthand& shorthand, const ComputedStyle& style, const LayoutObject* layout_object, bool allow_visited_style, CSSValuePhase value_phase)
{
    const CSSValue* row_gap_value = shorthand.properties()[0]->CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
    const CSSValue* column_gap_value = shorthand.properties()[1]->CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);

    return MakeGarbageCollected<CSSValuePair>(row_gap_value, column_gap_value, CSSValuePair::kDropIdenticalValues);
}
    ...
    const StylePropertyShorthand& shorthand, const ComputedStyle& style, const LayoutObject* layout_object, bool allow_visited_style, CSSValuePhase value_phase)
{
    ...
    for (; last_index > 1; --last_index) {
        const CSSValue* value = shorthand.properties()[last_index - 1]->CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
        if ((!IsA<CSSIdentifierValue>(value) || (To<CSSIdentifierValue>(value)->GetValueID() != CSSValueID::kNone))) {
            break;
        }
    }
    ...
    for (unsigned i = 0; i < last_index; ++i) {
        const CSSValue* value = shorthand.properties()[i]->CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
        DCHECK(value);
        list->Append(*value);
    }
    return list;
    ...
    const StylePropertyShorthand& shorthand, const ComputedStyle& style, const LayoutObject* layout_object, bool allow_visited_style, CSSValuePhase value_phase)
{
    ...
    const CSSValue* grid_row_start = shorthand.properties()[0]->CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
    const CSSValue* grid_column_start = shorthand.properties()[1]->CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
    const CSSValue* grid_row_end = shorthand.properties()[2]->CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
    const CSSValue* grid_column_end = shorthand.properties()[3]->CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
    ...
    const StylePropertyShorthand& shorthand, const ComputedStyle& style, const LayoutObject* layout_object, bool allow_visited_style, CSSValuePhase value_phase)
{
    ...
    const CSSValue* line_start = shorthand.properties()[0]->CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
    const CSSValue* line_end = shorthand.properties()[1]->CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
    CSSValueList* list = CSSValueList::CreateSlashSeparated();
    ...
    const StylePropertyShorthand& shorthand, const ComputedStyle& style, const LayoutObject* layout_object, bool allow_visited_style, CSSValuePhase value_phase)
{
    ...
    const CSSValue* template_rows_computed = ValueForGridTrackList(kForRows, layout_object, style,
        /* force_computed_values */ true);
    const CSSValue* template_columns_computed = ValueForGridTrackList(kForColumns, layout_object, style,
        /* force_computed_values */ true);

    const CSSValue* template_row_values = shorthand.properties()[0]->CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
    const CSSValue* template_column_values = shorthand.properties()[1]->CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
    const CSSValue* template_area_values = shorthand.properties()[2]->CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
    ...
    return CSSOMUtils::ComputedValueForGridTemplateShorthand(
        CSSOMUtils::IsEmptyValueList(template_rows_computed) ? template_row_values : template_rows_computed,
        CSSOMUtils::IsEmptyValueList(template_columns_computed) ? template_column_values : template_columns_computed, template_area_values);
}
    ...
    const StylePropertyShorthand& shorthand, const ComputedStyle& style, const LayoutObject* layout_object, bool allow_visited_style, CSSValuePhase value_phase)
{
    ...
    const CSSValue* top_value = shorthand.properties()[0]->CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
    const CSSValue* right_value = shorthand.properties()[1]->CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
    const CSSValue* bottom_value = shorthand.properties()[2]->CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
    const CSSValue* left_value = shorthand.properties()[3]->CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
    ...
    bool show_bottom = !base::ValuesEquivalent(top_value, bottom_value) || show_left;
    bool show_right = !base::ValuesEquivalent(top_value, right_value) || show_bottom;
...
CSSValuePair* ComputedStyleUtils::ValuesForInlineBlockShorthand(
    const StylePropertyShorthand& shorthand, const ComputedStyle& style, const LayoutObject* layout_object, bool allow_visited_style, CSSValuePhase value_phase)
{
    const CSSValue* start_value = shorthand.properties()[0]->CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
    const CSSValue* end_value = shorthand.properties()[1]->CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
    // Both properties must be specified.
    if (!start_value || !end_value) {
        return nullptr;
    }

    auto* pair = MakeGarbageCollected<CSSValuePair>(start_value, end_value, CSSValuePair::kDropIdenticalValues);
    return pair;
}
...
CSSValuePair* ComputedStyleUtils::ValuesForPlaceShorthand(
    const StylePropertyShorthand& shorthand, const ComputedStyle& style, const LayoutObject* layout_object, bool allow_visited_style, CSSValuePhase value_phase)
{
    const CSSValue* align_value = shorthand.properties()[0]->CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
    const CSSValue* justify_value = shorthand.properties()[1]->CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);

    return MakeGarbageCollected<CSSValuePair>(align_value, justify_value, CSSValuePair::kDropIdenticalValues);
}
...
static CSSValue* ExpandNoneLigaturesValue()
{
    ...
    list->Append(*CSSIdentifierValue::Create(CSSValueID::kNoDiscretionaryLigatures));
...
CSSValue* ComputedStyleUtils::ValuesForFontVariantProperty(
    const ComputedStyle& style, const LayoutObject* layout_object, bool allow_visited_style, CSSValuePhase value_phase)
{
    enum VariantShorthandCases { kAllNormal, kNoneLigatures, kConcatenateNonNormal, kEmptyString };
    ...
    for (const CSSProperty* const longhand : shorthand.properties()) {
        ...
        auto* identifier_value = DynamicTo<CSSIdentifierValue>(value);
        if (shorthand_case == kAllNormal && identifier_value && identifier_value->GetValueID() == CSSValueID::kNone
            && longhand->IDEquals(CSSPropertyID::kFontVariantLigatures)) {
            shorthand_case = kNoneLigatures;
        } else if (!(identifier_value && identifier_value->GetValueID() == CSSValueID::kNormal)) {
            shorthand_case = shorthand_case == kNoneLigatures ? kEmptyString : kConcatenateNonNormal;
            break;
        }
    }
    ...
    const ComputedStyle& style, const LayoutObject* layout_object, bool allow_visited_style, CSSValuePhase value_phase)
{
    ...
    for (const CSSProperty* const longhand : shorthand.properties()) {
        const CSSValue* value = longhand->CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
        auto* identifier_value = DynamicTo<CSSIdentifierValue>(value);
        if (longhand->IDEquals(CSSPropertyID::kFontSynthesisWeight) && identifier_value->GetValueID() == CSSValueID::kAuto) {
            shorthand_case = kConcatenateAuto;
        } else if (longhand->IDEquals(CSSPropertyID::kFontSynthesisStyle) && identifier_value->GetValueID() == CSSValueID::kAuto) {
            shorthand_case = kConcatenateAuto;
        } else if (longhand->IDEquals(CSSPropertyID::kFontSynthesisSmallCaps) && identifier_value->GetValueID() == CSSValueID::kAuto) {
            shorthand_case = kConcatenateAuto;
        }
    }
    ...
    const ComputedStyle& style, const LayoutObject* layout_object, bool allow_visited_style, CSSValuePhase value_phase)
{
    ...
    CHECK_EQ(containerShorthand().properties()[0], &GetCSSPropertyContainerName());
    CHECK_EQ(containerShorthand().properties()[1], &GetCSSPropertyContainerType());

    CSSValueList* list = CSSValueList::CreateSlashSeparated();

    const CSSValue* name = GetCSSPropertyContainerName().CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
    const CSSValue* type = GetCSSPropertyContainerType().CSSValueFromComputedStyle(style, layout_object, allow_visited_style, value_phase);
    ...
    if (const auto* ident_value = DynamicTo<CSSIdentifierValue>(*type); !ident_value || ident_value->GetValueID() != CSSValueID::kNormal) {
        list->Append(*type);
    }
...
CSSValue* ComputedStyleUtils::ValueForGapLength(const std::optional<Length>& gap_length, const ComputedStyle& style)
{
    if (!gap_length) {
        return CSSIdentifierValue::Create(CSSValueID::kNormal);
    }
...
CSSValue* ComputedStyleUtils::ValueForStyleNameOrKeyword(const StyleNameOrKeyword& value)
{
    if (value.IsKeyword()) {
...
CSSValue* ComputedStyleUtils::ValueForCustomIdentOrNone(const AtomicString& ident)
{
    if (ident.empty()) {
        return CSSIdentifierValue::Create(CSSValueID::kNone);
    }
...
CSSValue* ComputedStyleUtils::ValueForCustomIdentOrNone(const ScopedCSSName* name)
{
    return ValueForCustomIdentOrNone(name ? name->GetName() : g_null_atom);
}

const CSSValue* ComputedStyleUtils::ValueForStyleAutoColor(const ComputedStyle& style, const StyleAutoColor& color, CSSValuePhase value_phase)
{
    if (color.IsAutoColor()) {
        return cssvalue::CSSColor::Create(StyleColor::CurrentColor().Resolve(style.GetCurrentColor(), style.UsedColorScheme()));
    }
    return ComputedStyleUtils::CurrentColorOrValidColor(style, color.ToStyleColor(), value_phase);
}

CSSValue* ComputedStyleUtils::ValueForIntrinsicLength(const ComputedStyle& style, const StyleIntrinsicLength& intrinsic_length)
{
...
CSSValue* ComputedStyleUtils::ValueForScrollStart(const ComputedStyle& style, const ScrollStartData& data)
{
    if (data.value_type == ScrollStartValueType::kLengthOrPercentage) {
        return ComputedStyleUtils::ZoomAdjustedPixelValueForLength(data.value, style);
    }
...
CSSIdentifierValue* PositionAreaSpanToCSSIdentifierValue(PositionAreaRegion span_start, PositionAreaRegion span_end)
{
    if (span_start == span_end) {
        return CSSIdentifierValue::Create(span_start);
    }
    CHECK(span_start == PositionAreaRegion::kCenter || span_end == PositionAreaRegion::kCenter);
    PositionAreaRegion span_towards = span_start == PositionAreaRegion::kCenter ? span_end : span_start;
...
CSSValue* ComputedStyleUtils::ValueForPositionArea(const blink::PositionArea& area)
{
    if (area.FirstStart() == PositionAreaRegion::kNone) {
        return CSSIdentifierValue::Create(CSSValueID::kNone);
    }
    CSSIdentifierValue* first_value = PositionAreaSpanToCSSIdentifierValue(area.FirstStart(), area.FirstEnd());
    CSSIdentifierValue* second_value = PositionAreaSpanToCSSIdentifierValue(area.SecondStart(), area.SecondEnd());
...
std::unique_ptr<CrossThreadStyleValue> ComputedStyleUtils::CrossThreadStyleValueFromCSSStyleValue(CSSStyleValue* style_value)
{
    ...
    case CSSStyleValue::StyleValueType::kKeywordType:
        return std::make_unique<CrossThreadKeywordValue>(To<CSSKeywordValue>(style_value)->value());
    case CSSStyleValue::StyleValueType::kUnitType:
        return std::make_unique<CrossThreadUnitValue>(To<CSSUnitValue>(style_value)->value(), To<CSSUnitValue>(style_value)->GetInternalUnit());
    case CSSStyleValue::StyleValueType::kUnsupportedColorType:
        return std::make_unique<CrossThreadColorValue>(To<CSSUnsupportedColor>(style_value)->Value());
    case CSSStyleValue::StyleValueType::kUnparsedType:
        return std::make_unique<CrossThreadUnparsedValue>(To<CSSUnparsedValue>(style_value)->ToUnparsedString());
    default:
        return std::make_unique<CrossThreadUnsupportedValue>(style_value->toString());
    }
}

const CSSValue* ComputedStyleUtils::ComputedPropertyValue(const CSSProperty& property, const ComputedStyle& style, const LayoutObject* layout_object)
{
    return property.CSSValueFromComputedStyle(style, layout_object, false, CSSValuePhase::kComputedValue);
}

CSSValue* ComputedStyleUtils::ValueForPositionTryFallbacks(const PositionTryFallbacks& fallbacks)
{
