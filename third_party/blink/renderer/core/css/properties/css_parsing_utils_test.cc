...
{
    return MakeGarbageCollected<CSSParserContext>(mode, SecureContextMode::kInsecureContext);
}
...
TEST(CSSParsingUtilsTest, BasicShapeUseCount)
{
    ...
    document.documentElement()->setInnerHTML("<style>span { shape-outside: circle(); }</style>");
...
{
    CSSParserTokenStream stream(target);
    return ConsumeAngle(stream, *MakeContext(), std::nullopt, min, max)->ComputeDegrees();
}
...
TEST(CSSParsingUtilsTest, DashedIdent)
{
    ...
    for (auto& expectation : expectations) {
        ...
        EXPECT_EQ(css_parsing_utils::IsDashedIdent(stream.Peek()), expectation.is_dashed_indent);
    }
}
...
TEST(CSSParsingUtilsTest, ConsumeAbsoluteColor)
{
    ...
    for (auto& expectation : expectations) {
        EXPECT_EQ(ConsumeColorForTest(expectation.css_text, css_parsing_utils::ConsumeColor), expectation.consume_color_expectation);
        EXPECT_EQ(ConsumeColorForTest(expectation.css_text, css_parsing_utils::ConsumeAbsoluteColor), expectation.consume_absolute_color_expectation);
    }
}
...
TEST(CSSParsingUtilsTest, InternalColorsOnlyAllowedInUaMode)
{
    ...
    public:
        ...
        { "blue", CSSIdentifierValue::Create(CSSValueID::kBlue), CSSIdentifierValue::Create(CSSValueID::kBlue) },
        { "-internal-spelling-error-color", CSSIdentifierValue::Create(CSSValueID::kInternalSpellingErrorColor), nullptr },
        { "-internal-grammar-error-color", CSSIdentifierValue::Create(CSSValueID::kInternalGrammarErrorColor), nullptr },
        { "-internal-search-color", CSSIdentifierValue::Create(CSSValueID::kInternalSearchColor), nullptr },
        { "-internal-search-text-color", CSSIdentifierValue::Create(CSSValueID::kInternalSearchTextColor), nullptr },
        { "-internal-current-search-color", CSSIdentifierValue::Create(CSSValueID::kInternalCurrentSearchColor), nullptr },
        { "-internal-current-search-text-color", CSSIdentifierValue::Create(CSSValueID::kInternalCurrentSearchTextColor), nullptr },
    };
    for (auto& expectation : expectations) {
        EXPECT_EQ(ConsumeColorForTest(expectation.css_text, kHTMLStandardMode), expectation.other_expectation);
        EXPECT_EQ(ConsumeColorForTest(expectation.css_text, kHTMLQuirksMode), expectation.other_expectation);
        EXPECT_EQ(ConsumeColorForTest(expectation.css_text, kUASheetMode), expectation.ua_expectation);
    }
}
...
{
    auto ConsumePositionTryFallbackForTest = [](String css_text, CSSParserMode mode) {
        CSSParserTokenStream stream(css_text);
        return css_parsing_utils::ConsumeSinglePositionTryFallback(stream, *MakeContext(mode));
    };
    ...
    for (auto& expectation : expectations) {
        EXPECT_EQ(ConsumePositionTryFallbackForTest(expectation.css_text, kHTMLStandardMode) != nullptr, expectation.allow_other);
        EXPECT_EQ(ConsumePositionTryFallbackForTest(expectation.css_text, kHTMLQuirksMode) != nullptr, expectation.allow_other);
        EXPECT_EQ(ConsumePositionTryFallbackForTest(expectation.css_text, kUASheetMode) != nullptr, expectation.allow_ua);
    }
}
...
TEST(CSSParsingUtilsTest, ConsumePositionTryFallbacksInUAMode)
{
    ...
    CSSParserTokenStream stream(css_text);
    CSSValue* value = css_parsing_utils::ConsumePositionTryFallbacks(stream, *MakeContext(kUASheetMode));
    ASSERT_TRUE(value);
