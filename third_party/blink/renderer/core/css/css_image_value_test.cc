...
TEST_F(CSSImageValueTest, BlockPotentiallyDanglingMarkup)
{
    ...
    ImageResourceContent* content1 = t1->ComputedStyleRef().BackgroundLayers().GetImage()->CachedImage();
    ASSERT_TRUE(content1);
    EXPECT_TRUE(content1->ErrorOccurred());

    auto* t2 = GetDocument().getElementById(AtomicString("t2"));
    ImageResourceContent* content2 = t2->ComputedStyleRef().BackgroundLayers().GetImage()->CachedImage();
