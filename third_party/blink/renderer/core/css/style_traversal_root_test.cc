...
public:
...
#if DCHECK_IS_ON()
        for (const Element* element = node->parentElement(); element; element = element->parentElement()) {
    ...
    {
        return root_type_ == RootType::kSingleRoot;
    }
    bool IsCommonRoot() const
    {
        return root_type_ == RootType::kCommonRoot;
    }
...
protected:
    ...
    Element* DivElement(ElementIndex index)
    {
