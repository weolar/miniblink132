...
protected:
    ...
    {
        return GetDocument().GetFrame()->GetPage()->GetSettings().GetGenericFontFamilySettings();
    }
...
TEST_F(CSSFontFamilyWebKitPrefixTest, CSSFontFamilyWebKitPrefixTest_WebKitBodyFontBuilder)
{
    ASSERT_FALSE(GetDocument().IsUseCounted(WebFeature::kFontBuilderCSSFontFamilyWebKitPrefixBody));
    ...
    GetGenericGenericFontFamilySettings().UpdateStandard(AtomicString("MyStandardFont"));
    LoadPageWithFontFamilyValue("initial");
    ASSERT_FALSE(GetDocument().IsUseCounted(WebFeature::kFontBuilderCSSFontFamilyWebKitPrefixBody));
    LoadPageWithFontFamilyValue("-webkit-body, serif");
    ASSERT_TRUE(GetDocument().IsUseCounted(WebFeature::kFontBuilderCSSFontFamilyWebKitPrefixBody));
}

TEST_F(CSSFontFamilyWebKitPrefixTest, CSSFontFamilyWebKitPrefixTest_WebKitBodyFontSelector)
{
    ASSERT_FALSE(GetDocument().IsUseCounted(WebFeature::kFontSelectorCSSFontFamilyWebKitPrefixBody));

    // If empty standard font is specified, counter is never triggered.
    GetGenericGenericFontFamilySettings().UpdateStandard(g_empty_atom);

    for (String font_family_value : { "initial", "-webkit-body", "-webkit-body, serif", "serif, -webkit-body" }) {
    ...
    ASSERT_FALSE(GetDocument().IsUseCounted(WebFeature::kFontSelectorCSSFontFamilyWebKitPrefixBody));
    LoadPageWithFontFamilyValue("serif, -webkit-body");
    ASSERT_TRUE(GetDocument().IsUseCounted(WebFeature::kFontSelectorCSSFontFamilyWebKitPrefixBody));
}

TEST_F(CSSFontFamilyWebKitPrefixTest, CSSFontFamilyWebKitPrefixTest_BlinkMacSystemFont)
{
