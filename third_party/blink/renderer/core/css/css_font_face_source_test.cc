...
public:
    const SimpleFontData* CreateFontData(const FontDescription&, const FontSelectionCapabilities&) override
    {
        return MakeGarbageCollected<SimpleFontData>(MakeGarbageCollected<FontPlatformData>(skia::DefaultTypeface(), /* name */ std::string(),
            /* text_size */ 0, /* synthetic_bold */ false,
            /* synthetic_italic */ false, TextRenderingMode::kAutoTextRendering, ResolvedFontFeatures {}));
    }
...
unsigned SimulateHashCalculation(float size)
{
    ...
    return font_description.CacheKey(FontFaceCreationParams(), is_unique_match).GetHash();
}
} // namespace

TEST(CSSFontFaceSourceTest, HashCollision)
{
    DummyFontFaceSource* font_face_source = MakeGarbageCollected<DummyFontFaceSource>();
    ...
    EXPECT_EQ(SimulateHashCalculation(kEqualHashesFirst), SimulateHashCalculation(kEqualHashesSecond));
    EXPECT_NE(font_face_source->GetFontDataForSize(kEqualHashesFirst), font_face_source->GetFontDataForSize(kEqualHashesSecond));
}
...
TEST(CSSFontFaceSourceTest, UnboundedGrowth)
{
    ...
    for (float wght = 700; wght < 705; wght += 1 / 6.f) {
        for (float wdth = 100; wdth < 125; wdth += 1 / 4.f) {
            ...
            font_face_source->GetFontData(font_description_variable, normal_capabilities);
        }
    }
}
