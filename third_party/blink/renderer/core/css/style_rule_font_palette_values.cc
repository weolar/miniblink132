...
StyleRuleFontPaletteValues::StyleRuleFontPaletteValues(const AtomicString& name, CSSPropertyValueSet* properties)
...
StyleRuleFontPaletteValues::StyleRuleFontPaletteValues(const StyleRuleFontPaletteValues&) = default;
...
FontPalette::BasePaletteValue StyleRuleFontPaletteValues::GetBasePaletteIndex() const
{
    constexpr FontPalette::BasePaletteValue kNoBasePaletteValue = { FontPalette::kNoBasePalette, 0 };
    const CSSValue* base_palette = GetBasePalette();
    if (!base_palette) {
        return kNoBasePaletteValue;
    }

    if (auto* base_palette_identifier = DynamicTo<CSSIdentifierValue>(*base_palette)) {
        switch (base_palette_identifier->GetValueID()) {
        case CSSValueID::kLight:
            return FontPalette::BasePaletteValue({ FontPalette::kLightBasePalette, 0 });
        case CSSValueID::kDark:
            return FontPalette::BasePaletteValue({ FontPalette::kDarkBasePalette, 0 });
        default:
            NOTREACHED();
        }
    }

    const CSSPrimitiveValue& palette_primitive = To<CSSPrimitiveValue>(*base_palette);
    return FontPalette::BasePaletteValue({ FontPalette::kIndexBasePalette, palette_primitive.GetIntValue() });
}

Vector<FontPalette::FontPaletteOverride> StyleRuleFontPaletteValues::GetOverrideColorsAsVector() const
{
    ...
    auto ConvertToColor = [](const CSSValuePair& override_pair) -> Color {
        if (override_pair.Second().IsIdentifierValue()) {
            const CSSIdentifierValue& color_identifier = To<CSSIdentifierValue>(override_pair.Second());
            ...
            return StyleColor::ColorFromKeyword(color_identifier.GetValueID(), mojom::blink::ColorScheme::kLight,
                /*color_provider=*/nullptr, /*is_in_web_app_scope=*/false);
        }
        const cssvalue::CSSColor& css_color = To<cssvalue::CSSColor>(override_pair.Second());
    ...
    for (auto& item : overrides_list) {
        ...
        const CSSPrimitiveValue& palette_index = To<CSSPrimitiveValue>(override_pair.First());
        DCHECK(palette_index.IsInteger());

        const Color override_color = ConvertToColor(override_pair);

        FontPalette::FontPaletteOverride palette_override { palette_index.GetValue<uint16_t>(), override_color };
...
void StyleRuleFontPaletteValues::TraceAfterDispatch(blink::Visitor* visitor) const
{
