...
public:
    ...
    CSSValuePair(const CSSValue* first, const CSSValue* second, IdenticalValuesPolicy identical_values_policy)
    ...
    {
        return WTF::HashInts(identical_values_policy_, WTF::HashInts(first_->Hash(), second_->Hash()));
    }

    void TraceAfterDispatch(blink::Visitor*) const;

protected:
    CSSValuePair(ClassType class_type, const CSSValue* first, const CSSValue* second)
        : CSSValue(class_type)
        , first_(first)
        , second_(second)
...
template <> struct DowncastTraits<CSSValuePair> {
