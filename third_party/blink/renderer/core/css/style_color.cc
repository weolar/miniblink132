...
Color ResolveColorOperand(const StyleColor::ColorOrUnresolvedColorFunction& color, UnderlyingColorType type, const Color& current_color)
{
    switch (type) {
...
CSSValue* ConvertColorOperandToCSSValue(const StyleColor::ColorOrUnresolvedColorFunction& color_or_function, UnderlyingColorType type)
{
    switch (type) {
    case UnderlyingColorType::kColor:
...
CORE_EXPORT bool StyleColor::UnresolvedColorFunction::operator==(const UnresolvedColorFunction& other) const
{
    ...
    case StyleColor::UnresolvedColorFunction::Type::kRelativeColor:
        return *To<UnresolvedRelativeColor>(this) == To<UnresolvedRelativeColor>(other);
    }

    NOTREACHED();
}

StyleColor::UnresolvedColorMix::UnresolvedColorMix(Color::ColorSpace color_interpolation_space, Color::HueInterpolationMethod hue_interpolation_method,
    const StyleColor& c1, const StyleColor& c2, double percentage, double alpha_multiplier)
...
Color StyleColor::UnresolvedColorMix::Resolve(const Color& current_color) const
{
    ...
    return Color::FromColorMix(color_interpolation_space_, hue_interpolation_method_, c1, c2, percentage_, alpha_multiplier_);
}

CSSValue* StyleColor::UnresolvedColorMix::ToCSSValue() const
{
    const CSSPrimitiveValue* percent1 = CSSNumericLiteralValue::Create(100 * (1.0 - percentage_) * alpha_multiplier_, CSSPrimitiveValue::UnitType::kPercentage);
    const CSSPrimitiveValue* percent2 = CSSNumericLiteralValue::Create(100 * percentage_ * alpha_multiplier_, CSSPrimitiveValue::UnitType::kPercentage);

    return MakeGarbageCollected<cssvalue::CSSColorMixValue>(ConvertColorOperandToCSSValue(color1_, color1_type_),
        ConvertColorOperandToCSSValue(color2_, color2_type_), percent1, percent2, color_interpolation_space_, hue_interpolation_method_);
}

StyleColor::UnresolvedRelativeColor::UnresolvedRelativeColor(const StyleColor& origin_color, Color::ColorSpace color_interpolation_space,
    const CSSValue& channel0, const CSSValue& channel1, const CSSValue& channel2, const CSSValue* alpha)
    ...
    , color_interpolation_space_(color_interpolation_space)
{
    auto to_channel = [](const CSSValue& value) -> scoped_refptr<const CalculationValue> {
        if (const CSSNumericLiteralValue* numeric = DynamicTo<CSSNumericLiteralValue>(value)) {
            if (numeric->IsPercentage()) {
                return CalculationValue::Create(PixelsAndPercent(0., numeric->DoubleValue(), false, true), Length::ValueRange::kAll);
            } else {
                // It's not actually a "pixels" value, but treating it as one simplifies
                // storage and resolution.
                return CalculationValue::Create(PixelsAndPercent(numeric->DoubleValue()), Length::ValueRange::kAll);
            }
        } else if (const CSSIdentifierValue* identifier = DynamicTo<CSSIdentifierValue>(value)) {
            if (identifier->GetValueID() == CSSValueID::kNone) {
                return nullptr;
            }
            scoped_refptr<CalculationExpressionNode> expression
                = base::MakeRefCounted<CalculationExpressionColorChannelKeywordNode>(CSSValueIDToColorChannelKeyword(identifier->GetValueID()));
            return CalculationValue::CreateSimplified(std::move(expression), Length::ValueRange::kAll);
        } else if (const CSSMathFunctionValue* function = DynamicTo<CSSMathFunctionValue>(value)) {
    ...
    if (alpha != nullptr) {
        ...
        scoped_refptr<CalculationExpressionNode> expression = base::MakeRefCounted<CalculationExpressionColorChannelKeywordNode>(ColorChannelKeyword::kAlpha);
        alpha_ = CalculationValue::CreateSimplified(std::move(expression), Length::ValueRange::kAll);
    }
}
...
{
    auto to_css_value = [](const scoped_refptr<const CalculationValue>& channel) -> const CSSValue* {
        if (channel == nullptr) {
            return CSSIdentifierValue::Create(CSSValueID::kNone);
        }
        if (!channel->IsExpression()) {
            if (channel->HasExplicitPercent()) {
                return CSSNumericLiteralValue::Create(channel->Percent(), CSSPrimitiveValue::UnitType::kPercentage);
            } else {
                return CSSNumericLiteralValue::Create(channel->Pixels(), CSSPrimitiveValue::UnitType::kNumber);
            }
        }
        scoped_refptr<const CalculationExpressionNode> expression = channel->GetOrCreateExpression();
        if (expression->IsColorChannelKeyword()) {
            return CSSIdentifierValue::Create(ColorChannelKeywordToCSSValueID(To<CalculationExpressionColorChannelKeywordNode>(expression.get())->Value()));
        } else {
    ...
    return MakeGarbageCollected<cssvalue::CSSRelativeColorValue>(
        *ConvertColorOperandToCSSValue(origin_color_, origin_color_type_), color_interpolation_space_, *channel0, *channel1, *channel2, alpha);
}

Color StyleColor::UnresolvedRelativeColor::Resolve(const Color& current_color) const
{
    Color resolved_origin = ResolveColorOperand(origin_color_, origin_color_type_, current_color);
    resolved_origin.ConvertToColorSpace(color_interpolation_space_);

    const ColorFunction::Metadata& function_metadata = ColorFunction::MetadataForColorSpace(color_interpolation_space_);

    std::vector<std::pair<ColorChannelKeyword, float>> keyword_values
        = { { { CSSValueIDToColorChannelKeyword(function_metadata.channel_name[0]), resolved_origin.Param0() },
            { CSSValueIDToColorChannelKeyword(function_metadata.channel_name[1]), resolved_origin.Param1() },
            { CSSValueIDToColorChannelKeyword(function_metadata.channel_name[2]), resolved_origin.Param2() },
    ...
    const bool is_hxx_color_space = (color_interpolation_space_ == Color::ColorSpace::kHSL) || (color_interpolation_space_ == Color::ColorSpace::kHWB);
    const bool is_lch_color_space = (color_interpolation_space_ == Color::ColorSpace::kLch) || (color_interpolation_space_ == Color::ColorSpace::kOklch);
    ...
    evaluation_input.color_channel_keyword_values = base::flat_map(std::move(keyword_values));

    auto to_channel_value = [&evaluation_input](const CalculationValue* calculation_value, double channel_percentage) -> std::optional<float> {
        ...
        const float max_value = (std::isnan(channel_percentage)) ? std::numeric_limits<float>::max() : channel_percentage;
    ...
    std::array<std::optional<float>, 3> params = { to_channel_value(channel0_.get(), function_metadata.channel_percentage[0]),
        to_channel_value(channel1_.get(), function_metadata.channel_percentage[1]),
        to_channel_value(channel2_.get(), function_metadata.channel_percentage[2]) };
    ...
    auto wrap_hue_channel = [](std::optional<float>& param) {
        if (param.has_value()) {
            ...
            param.value() = fmod(fmod(static_cast<double>(param.value()), 360.0) + 360.0, 360.0);
        }
    };
    ...
    Color result = Color::FromColorSpace(color_interpolation_space_, params[0], params[1], params[2], param_alpha);
    if (Color::IsLegacyColorSpace(result.GetColorSpace()) && !RuntimeEnabledFeatures::CSSRelativeColorPreserveNoneEnabled()) {
        result.ConvertToColorSpace(Color::ColorSpace::kSRGB);
    }
    return result;
}

bool StyleColor::UnresolvedRelativeColor::operator==(const UnresolvedRelativeColor& other) const
{
    if (origin_color_type_ != other.origin_color_type_ || color_interpolation_space_ != other.color_interpolation_space_
        || alpha_was_specified_ != other.alpha_was_specified_ || !base::ValuesEquivalent(channel0_, other.channel0_)
        || !base::ValuesEquivalent(channel1_, other.channel1_) || !base::ValuesEquivalent(channel2_, other.channel2_)
        || !base::ValuesEquivalent(alpha_, other.alpha_)) {
        return false;
    }

    return ColorOrUnresolvedColorFunction::Equals(origin_color_, other.origin_color_, origin_color_type_);
}
...
Color StyleColor::Resolve(const Color& current_color, mojom::blink::ColorScheme color_scheme, bool* is_current_color) const
{
    if (IsUnresolvedColorFunction()) {
        Color result = color_or_unresolved_color_function_.unresolved_color_function->Resolve(current_color);
...
Color StyleColor::ResolveWithAlpha(Color current_color, mojom::blink::ColorScheme color_scheme, int alpha, bool* is_current_color) const
{
...
StyleColor StyleColor::ResolveSystemColor(mojom::blink::ColorScheme color_scheme, const ui::ColorProvider* color_provider, bool is_in_web_app_scope) const
{
    CHECK(IsSystemColor());
    Color color = ColorFromKeyword(color_keyword_, color_scheme, color_provider, is_in_web_app_scope);
...
Color StyleColor::ColorFromKeyword(
    CSSValueID keyword, mojom::blink::ColorScheme color_scheme, const ui::ColorProvider* color_provider, bool is_in_web_app_scope)
{
...
bool StyleColor::IsColorKeyword(CSSValueID id)
{
    ...
    //
    return (id >= CSSValueID::kAqua && id <= CSSValueID::kInternalCurrentSearchTextColor) || (id >= CSSValueID::kAliceblue && id <= CSSValueID::kYellowgreen)
        || id == CSSValueID::kMenu;
}
...
CORE_EXPORT std::ostream& operator<<(std::ostream& stream, const StyleColor& color)
{
...
CORE_EXPORT std::ostream& operator<<(std::ostream& stream, const StyleColor::UnresolvedColorFunction& unresolved_color_function)
{
