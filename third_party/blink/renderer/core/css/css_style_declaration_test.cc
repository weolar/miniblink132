...
TEST(CSSStyleDeclarationTest, ParsingRevertWithFeatureEnabled)
{
    ...
    style->SetPropertyInternal(CSSPropertyID::kLeft, "left", "revert", false, SecureContextMode::kSecureContext, exception_state);
    style->SetPropertyInternal(CSSPropertyID::kVariable, "--y", " revert", false, SecureContextMode::kSecureContext, exception_state);
...
TEST(CSSStyleDeclarationTest, ExposureCacheLeak)
{
    ...
    auto* property_value_set = MakeGarbageCollected<MutableCSSPropertyValueSet>(CSSParserMode::kHTMLStandardMode);
    auto* style = MakeGarbageCollected<PropertySetCSSStyleDeclaration>(v8_testing_scope.GetExecutionContext(), *property_value_set);
    ...
    const AtomicString origin_trial_test_property("originTrialTestProperty");
    {
        ...
        EXPECT_TRUE(style->NamedPropertyQuery(origin_trial_test_property, exception_state));
        EXPECT_EQ(NamedPropertySetterResult::kIntercepted, style->AnonymousNamedSetter(script_state, origin_trial_test_property, normal));
        EXPECT_EQ("normal", style->AnonymousNamedGetter(origin_trial_test_property));
    }

    {
        ...
        EXPECT_FALSE(style->NamedPropertyQuery(origin_trial_test_property, exception_state));
        EXPECT_EQ(NamedPropertySetterResult::kDidNotIntercept, style->AnonymousNamedSetter(script_state, origin_trial_test_property, normal));
        EXPECT_EQ(g_null_atom, style->AnonymousNamedGetter(origin_trial_test_property));
    }
}
