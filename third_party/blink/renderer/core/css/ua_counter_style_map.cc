...
bool IsPredefinedSymbolMarkerName(const AtomicString& name)
{
    static const AtomicString* predefined_symbol_markers[]
        = { &keywords::kDisc, &keywords::kSquare, &keywords::kCircle, &keywords::kDisclosureOpen, &keywords::kDisclosureClosed };
...
{
    CounterStyleMap* map = MakeGarbageCollected<CounterStyleMap>(nullptr, nullptr);
...
{
    DEFINE_STATIC_LOCAL(Persistent<CounterStyleMap>, ua_counter_style_map, (CreateUACounterStyleMap()));
...
CounterStyle& CounterStyleMap::CreateUACounterStyle(const AtomicString& name)
{
    ...
    const StyleSheetContents* sheet = CSSDefaultStyleSheets::ParseUASheet(rule_text);
    ...
    DCHECK(counter_style) << "Predefined counter style " << name << " has invalid symbols";
