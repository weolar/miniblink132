...
public:
    ...
    CSSAxisValue(const CSSPrimitiveValue* x, const CSSPrimitiveValue* y, const CSSPrimitiveValue* z);

    WTF::String CustomCSSText() const;

    Axis ComputeAxis(const CSSLengthResolver&) const;
    CSSValueID AxisName() const
    {
        return axis_name_;
    }
...
template <> struct DowncastTraits<cssvalue::CSSAxisValue> {
    static bool AllowFrom(const CSSValue& value)
    {
        return value.IsAxisValue();
    }
};
