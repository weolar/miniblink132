...
class CORE_EXPORT ClipPathPaintImageGenerator : public NativePaintImageGenerator {
public:
    static ClipPathPaintImageGenerator* Create(LocalFrame& local_root);

    ~ClipPathPaintImageGenerator() override = default;

    using ClipPathPaintImageGeneratorCreateFunction = ClipPathPaintImageGenerator*(LocalFrame&);
    static void Init(ClipPathPaintImageGeneratorCreateFunction* create_function);

    static gfx::RectF GetAnimationBoundingRect();

    virtual scoped_refptr<Image> Paint(float zoom, const gfx::RectF& reference_box, const gfx::SizeF& clip_area_size, const Node&) = 0;
};
