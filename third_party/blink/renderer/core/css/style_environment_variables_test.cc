...
public:
    ...
    void InitializeTestPageWithVariableNamed(LocalFrame& frame, const String& name)
    {
    ...
    void InitializeTestPageWithVariableNamed(LocalFrame& frame, const UADefinedVariable name)
    {
        InitializeTestPageWithVariableNamed(frame, StyleEnvironmentVariables::GetVariableName(name, /*feature_context=*/nullptr));
    }

    void SimulateNavigation()
    {
        const KURL& url = KURL(NullURL(), "https://www.example.com");
        GetDocument().GetFrame()->Loader().CommitNavigation(WebNavigationParams::CreateWithEmptyHTMLForTesting(url), nullptr /* extra_data */);
    ...
    String GetRootVariableValue(UADefinedVariable name)
    {
        CSSVariableData* data
            = StyleEnvironmentVariables::GetRootInstance().ResolveVariable(StyleEnvironmentVariables::GetVariableName(name, /*feature_context=*/nullptr), {});
    ...
    {
        StyleEnvironmentVariables::GetRootInstance().SetVariable(AtomicString(name), value);
    }

    void RemoveVariableOnRoot(const char* name)
    {
        StyleEnvironmentVariables::GetRootInstance().RemoveVariable(AtomicString(name));
    }
    ...
    void SetTwoDimensionalVariableOnRoot(UADefinedTwoDimensionalVariable variable, unsigned first_dimension, unsigned second_dimension, const String& value)
    {
        StyleEnvironmentVariables::GetRootInstance().SetVariable(variable, first_dimension, second_dimension, value, nullptr);
    }
};
...
TEST_F(StyleEnvironmentVariablesTest, DocumentVariable_AfterLoad)
{
    ...
    EXPECT_EQ(kTestColorRed, target->ComputedStyleRef().VisitedDependentColor(GetCSSPropertyBackgroundColor()));
}
...
TEST_F(StyleEnvironmentVariablesTest, DocumentVariable_Override_RemoveDocument)
{
    ...
    // variable.
    Element* target = GetDocument().getElementById(AtomicString("target"));
    EXPECT_EQ(kAltTestColor, target->ComputedStyleRef().VisitedDependentColor(GetCSSPropertyBackgroundColor()));
...
TEST_F(StyleEnvironmentVariablesTest, DocumentVariable_Remove)
{
    ...
    EXPECT_NE(kTestColorRed, target->ComputedStyleRef().VisitedDependentColor(GetCSSPropertyBackgroundColor()));
}
...
TEST_F(StyleEnvironmentVariablesTest, NavigateToClear)
{
    ...
    Element* target = GetDocument().getElementById(AtomicString("target"));
    EXPECT_EQ(kNoColor, target->ComputedStyleRef().VisitedDependentColor(GetCSSPropertyBackgroundColor()));
}
...
{
    EXPECT_EQ(kSafeAreaInsetExpectedDefault, GetRootVariableValue(UADefinedVariable::kSafeAreaInsetTop));
    EXPECT_EQ(kSafeAreaInsetExpectedDefault, GetRootVariableValue(UADefinedVariable::kSafeAreaInsetLeft));
    EXPECT_EQ(kSafeAreaInsetExpectedDefault, GetRootVariableValue(UADefinedVariable::kSafeAreaInsetBottom));
    EXPECT_EQ(kSafeAreaInsetExpectedDefault, GetRootVariableValue(UADefinedVariable::kSafeAreaInsetRight));

    EXPECT_EQ(nullptr, StyleEnvironmentVariables::GetRootInstance().ResolveVariable(AtomicString("test"), {}));
}
...
TEST_F(StyleEnvironmentVariablesTest, RecordUseCounter_IgnoreMediaControls)
{
    ...
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kCSSEnvironmentVariable_SafeAreaInsetTop));
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kCSSEnvironmentVariable_SafeAreaInsetLeft));
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kCSSEnvironmentVariable_SafeAreaInsetBottom));
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kCSSEnvironmentVariable_SafeAreaInsetRight));
}
...
{
    InitializeTestPageWithVariableNamed(GetFrame(), UADefinedVariable::kSafeAreaInsetBottom);

    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kCSSEnvironmentVariable));
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kCSSEnvironmentVariable_SafeAreaInsetBottom));
}

// TODO(https://crbug.com/1430288) remove after data collected (end of '23)
TEST_F(StyleEnvironmentVariablesTest, RecordUseCounter_ViewportFitCoverOrSafeAreaInsetBottom)
{
    InitializeWithHTML(GetFrame(), "");
    EXPECT_FALSE(GetDocument().IsUseCounted(WebFeature::kViewportFitCoverOrSafeAreaInsetBottom));
    InitializeTestPageWithVariableNamed(GetFrame(), UADefinedVariable::kSafeAreaInsetBottom);
...
{
    InitializeTestPageWithVariableNamed(GetFrame(), UADefinedVariable::kSafeAreaInsetLeft);

    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kCSSEnvironmentVariable));
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kCSSEnvironmentVariable_SafeAreaInsetLeft));
}

TEST_F(StyleEnvironmentVariablesTest, RecordUseCounter_SafeAreaInsetRight)
{
    InitializeTestPageWithVariableNamed(GetFrame(), UADefinedVariable::kSafeAreaInsetRight);

    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kCSSEnvironmentVariable));
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kCSSEnvironmentVariable_SafeAreaInsetRight));
}

TEST_F(StyleEnvironmentVariablesTest, RecordUseCounter_SafeAreaInsetTop)
{
    InitializeTestPageWithVariableNamed(GetFrame(), UADefinedVariable::kSafeAreaInsetTop);

    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kCSSEnvironmentVariable));
    EXPECT_TRUE(GetDocument().IsUseCounted(WebFeature::kCSSEnvironmentVariable_SafeAreaInsetTop));
}
...
TEST_F(StyleEnvironmentVariablesTest, KeyboardInset_AfterLoad)
{
    ...
    // by default.
    CSSVariableData* data
        = StyleEnvironmentVariables::GetRootInstance().ResolveVariable(StyleEnvironmentVariables::GetVariableName(UADefinedVariable::kKeyboardInsetTop,
                                                                           /*feature_context=*/nullptr),
            {});
    EXPECT_TRUE(data);
    data = StyleEnvironmentVariables::GetRootInstance().ResolveVariable(
        StyleEnvironmentVariables::GetVariableName(UADefinedVariable::kKeyboardInsetLeft, /*feature_context=*/nullptr), {});
    EXPECT_TRUE(data);
    data = StyleEnvironmentVariables::GetRootInstance().ResolveVariable(StyleEnvironmentVariables::GetVariableName(UADefinedVariable::kKeyboardInsetBottom,
                                                                            /*feature_context=*/nullptr),
        {});
    EXPECT_TRUE(data);
    data = StyleEnvironmentVariables::GetRootInstance().ResolveVariable(
        StyleEnvironmentVariables::GetVariableName(UADefinedVariable::kKeyboardInsetRight, /*feature_context=*/nullptr), {});
    EXPECT_TRUE(data);
    data = StyleEnvironmentVariables::GetRootInstance().ResolveVariable(
        StyleEnvironmentVariables::GetVariableName(UADefinedVariable::kKeyboardInsetWidth, /*feature_context=*/nullptr), {});
    EXPECT_TRUE(data);
    data = StyleEnvironmentVariables::GetRootInstance().ResolveVariable(StyleEnvironmentVariables::GetVariableName(UADefinedVariable::kKeyboardInsetHeight,
                                                                            /*feature_context=*/nullptr),
        {});
    EXPECT_TRUE(data);
}
...
TEST_F(StyleEnvironmentVariablesTest, TwoDimensionalVariables_BasicResolve)
{
    ...
    SetTwoDimensionalVariableOnRoot(UADefinedTwoDimensionalVariable::kViewportSegmentTop, 1, 0, "red");
...
TEST_F(StyleEnvironmentVariablesTest, TwoDimensionalVariables_UpdateValue)
{
    ...
    SetTwoDimensionalVariableOnRoot(UADefinedTwoDimensionalVariable::kViewportSegmentTop, 1, 0, "blue");
    EXPECT_TRUE(GetDocument().NeedsLayoutTreeUpdate());
    UpdateAllLifecyclePhasesForTest();
    EXPECT_EQ(kAltTestColor, target->ComputedStyleRef().VisitedDependentColor(GetCSSPropertyBackgroundColor()));
}

TEST_F(StyleEnvironmentVariablesTest, TwoDimensionalVariables_UndefinedFallsBack)
{
    ScopedViewportSegmentsForTest scoped_feature(true);
    String env_contents("viewport-segment-width 10 20, env(viewport-segment-width 0 0, blue)");
    InitializeTestPageWithVariableNamed(GetFrame(), env_contents);
    SetTwoDimensionalVariableOnRoot(UADefinedTwoDimensionalVariable::kViewportSegmentWidth, 1, 1, "red");
...
TEST_F(StyleEnvironmentVariablesTest, TwoDimensionalVariables_IncorrectDimensionsFallsBack)
{
    ScopedViewportSegmentsForTest scoped_feature(true);
    String env_contents("viewport-segment-width 0 0 0 0, blue");
    InitializeTestPageWithVariableNamed(GetFrame(), env_contents);
    SetTwoDimensionalVariableOnRoot(UADefinedTwoDimensionalVariable::kViewportSegmentWidth, 0, 0, "red");
...
TEST_F(StyleEnvironmentVariablesTest, TwoDimensionalVariables_NormalVariableWithDimensionFallsBack)
{
    ScopedViewportSegmentsForTest scoped_feature(true);
...
TEST_F(StyleEnvironmentVariablesTest, TwoDimensionalVariables_NegativeIndicesInvalid)
{
    ScopedViewportSegmentsForTest scoped_feature(true);
    String env_contents("viewport-segment-top -1 -1, blue");
    InitializeTestPageWithVariableNamed(GetFrame(), env_contents);
    SetTwoDimensionalVariableOnRoot(UADefinedTwoDimensionalVariable::kViewportSegmentTop, 0, 0, "red");
    SetTwoDimensionalVariableOnRoot(UADefinedTwoDimensionalVariable::kViewportSegmentTop, 1, 1, "red");
...
TEST_F(StyleEnvironmentVariablesTest, TwoDimensionalVariables_NonCommaAfterIndexInvalid)
{
    ScopedViewportSegmentsForTest scoped_feature(true);
    String env_contents("viewport-segment-left 1 1 ident");
    InitializeTestPageWithVariableNamed(GetFrame(), env_contents);
    SetTwoDimensionalVariableOnRoot(UADefinedTwoDimensionalVariable::kViewportSegmentLeft, 1, 1, "red");
...
TEST_F(StyleEnvironmentVariablesTest, TwoDimensionalVariables_NonIntegerIndicesInvalid)
{
    ScopedViewportSegmentsForTest scoped_feature(true);
...
TEST_F(StyleEnvironmentVariablesTest, TwoDimensionalVariables_NoIndicesFallsBack)
{
    ScopedViewportSegmentsForTest scoped_feature(true);
...
TEST_F(StyleEnvironmentVariablesTest, TwoDimensionalVariables_Removal)
{
    ...
    InitializeTestPageWithVariableNamed(GetFrame(), env_contents);
    SetTwoDimensionalVariableOnRoot(UADefinedTwoDimensionalVariable::kViewportSegmentHeight, 0, 0, "red");
...
TEST_F(StyleEnvironmentVariablesTest, TitlebarArea_AfterLoad)
{
    ...
    CSSVariableData* data = vars.ResolveVariable(StyleEnvironmentVariables::GetVariableName(UADefinedVariable::kTitlebarAreaX,
                                                     /*feature_context=*/nullptr),
        {});
    EXPECT_TRUE(data);
    EXPECT_EQ(data->Serialize(), "0px");
    data = vars.ResolveVariable(StyleEnvironmentVariables::GetVariableName(UADefinedVariable::kTitlebarAreaY, /*feature_context=*/nullptr), {});
    EXPECT_TRUE(data);
    EXPECT_EQ(data->Serialize(), "0px");
    data = vars.ResolveVariable(StyleEnvironmentVariables::GetVariableName(UADefinedVariable::kTitlebarAreaWidth,
                                    /*feature_context=*/nullptr),
        {});
    EXPECT_TRUE(data);
    EXPECT_EQ(data->Serialize(), "100px");
    data = vars.ResolveVariable(StyleEnvironmentVariables::GetVariableName(UADefinedVariable::kTitlebarAreaHeight, /*feature_context=*/nullptr), {});
    EXPECT_TRUE(data);
