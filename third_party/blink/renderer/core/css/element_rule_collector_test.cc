...
{
    auto* style_rule = DynamicTo<StyleRule>(css_test_helpers::ParseRule(document, text));
    if (style_rule == nullptr) {
        return nullptr;
    }
    RuleSet* rule_set = MakeGarbageCollected<RuleSet>();
    MediaQueryEvaluator* medium = MakeGarbageCollected<MediaQueryEvaluator>(document.GetFrame());
    rule_set->AddStyleRule(style_rule, /*parent_rule=*/nullptr, *medium, kRuleHasNoSpecialState, /*within_mixin=*/false);
    return rule_set;
}
...
public:
    ...
    std::optional<unsigned> Match(Element* element, const String& selector, const ContainerNode* scope = nullptr)
    {
        ElementResolveContext context(*element);
        SelectorFilter filter;
        MatchResult result;
        ElementRuleCollector collector(context, StyleRecalcContext(), filter, result, InsideLink(element));
    ...
    RuleIndexList* GetMatchedCSSRuleList(Element* element, RuleSet* rule_set, const CSSStyleSheet* sheet)
    {
        ElementResolveContext context(*element);
        SelectorFilter filter;
        MatchResult result;
...
TEST_F(ElementRuleCollectorTest, LinkMatchType)
{
    ...
    Element* unvisited_span = GetDocument().getElementById(AtomicString("unvisited_span"));
    Element* visited_span = GetDocument().getElementById(AtomicString("visited_span"));
    ASSERT_TRUE(foo);
    ASSERT_TRUE(bar);
    ASSERT_TRUE(visited);
    ASSERT_TRUE(link);
...
TEST_F(ElementRuleCollectorTest, LinkMatchTypeHostContext)
{
    ...
    Element* visited_host = GetDocument().getElementById(AtomicString("visited_host"));
    Element* unvisited_host = GetDocument().getElementById(AtomicString("unvisited_host"));
    ASSERT_TRUE(visited_host);
    ASSERT_TRUE(unvisited_host);

    ShadowRoot& visited_root = visited_host->AttachShadowRootForTesting(ShadowRootMode::kOpen);
    ShadowRoot& unvisited_root = unvisited_host->AttachShadowRootForTesting(ShadowRootMode::kOpen);
    ...
    Element* unvisited_style = unvisited_root.getElementById(AtomicString("style"));
...

    {
        ...
        EXPECT_EQ(Match(element, ":host-context(:visited) div", scope), kMatchVisited);
        EXPECT_EQ(Match(element, ":host-context(:is(:visited, :link)) div", scope), kMatchAll);

        // :host-context(:not(:visited/link)) matches the host itself.
        EXPECT_EQ(Match(element, ":host-context(:not(:visited)) div", scope), kMatchAll);
        EXPECT_EQ(Match(element, ":host-context(:not(:link)) div", scope), kMatchAll);
    }
...
{
    String markup = "<html xmlns='http://www.w3.org/1999/xhtml'><body class='foo'>"
                    "<none xmlns=''/>"
    ...
    GetFrame().ForceSynchronousDocumentInstall(AtomicString("text/xml"), std::move(data));
    ...
    auto run = [&](Element& element, String selector, std::optional<AtomicString> defaultNamespace) {
        auto* parser_context = MakeGarbageCollected<CSSParserContext>(kHTMLStandardMode, SecureContextMode::kInsecureContext);
        auto* sheet = MakeGarbageCollected<StyleSheetContents>(parser_context);
        sheet->ParserAddNamespace(AtomicString("bar"), AtomicString("http://example.org/bar"));
        if (defaultNamespace) {
            sheet->ParserAddNamespace(g_null_atom, *defaultNamespace);
        }
        MediaQueryEvaluator* medium = MakeGarbageCollected<MediaQueryEvaluator>(GetDocument().GetFrame());
        RuleSet& rules = sheet->EnsureRuleSet(*medium);
        auto* rule = To<StyleRule>(CSSParser::ParseRule(sheet->ParserContext(), sheet, CSSNestingType::kNone,
            /*parent_rule_for_nesting=*/nullptr, /*is_within_scope=*/false, selector + " { color: green }"));
        rules.AddStyleRule(rule, /*parent_rule=*/nullptr, *medium, kRuleHasNoSpecialState, /*within_mixin=*/false);

        MatchResult result;
        ElementResolveContext context { element };
        ElementRuleCollector collector(context, StyleRecalcContext(), SelectorFilter(), result, EInsideLink::kNotInsideLink);
