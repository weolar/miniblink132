...
template <unsigned length> void RunTests(ContainerNode& scope, const QueryTest (&test_cases)[length])
{
    for (const auto& test_case : test_cases) {
        const char* selector = test_case.selector;
        SCOPED_TRACE(testing::Message() << (test_case.query_all ? "querySelectorAll('" : "querySelector('") << selector << "')");
        if (test_case.query_all) {
            StaticElementList* match_all = scope.QuerySelectorAll(AtomicString(selector));
        ...
        EXPECT_EQ(test_case.stats.slow_traversing_shadow_tree_scan, stats.slow_traversing_shadow_tree_scan);
#endif
    }
}
} // namespace
...
TEST(SelectorQueryTest, NotMatchingPseudoElement)
{
    ...
    document->documentElement()->setInnerHTML("<body><style>span::before { content: 'X' }</style><span></span></body>");
    ...
    selector_vector = CSSParser::ParseSelector(MakeGarbageCollected<CSSParserContext>(*document, NullURL(), true /* origin_clean */, Referrer()),
...
TEST(SelectorQueryTest, LastOfTypeNotFinishedParsing)
{
    ...
    auto* html = MakeGarbageCollected<HTMLHtmlElement>(*document);
    document->AppendChild(html);
    document->documentElement()->setInnerHTML("<body><p></p><p id=last></p></body>", ASSERT_NO_EXCEPTION);
...
TEST(SelectorQueryTest, FastPathScoped)
{
    ...
    ShadowRoot& shadowRoot = scope->AttachShadowRootForTesting(ShadowRootMode::kOpen);
...
TEST(SelectorQueryTest, DisconnectedTreeScope)
{
    ...
    ShadowRoot& shadowRoot = host->AttachShadowRootForTesting(ShadowRootMode::kOpen);
...
TEST(SelectorQueryTest, QueryHasPseudoClass)
{
    ...
    {
        StaticElementList* result = scope->QuerySelectorAll(AtomicString(":has(> .a ~ .b)"));
        ASSERT_EQ(4U, result->length());
        EXPECT_EQ(result->item(0)->GetIdAttribute(), "div4");
        EXPECT_TRUE(result->item(0)->ClassNames().Contains(AtomicString("subject1")));
        EXPECT_EQ(result->item(1)->GetIdAttribute(), "div7");
        EXPECT_TRUE(result->item(1)->ClassNames().Contains(AtomicString("subject1")));
        EXPECT_EQ(result->item(2)->GetIdAttribute(), "div16");
        EXPECT_TRUE(result->item(2)->ClassNames().Contains(AtomicString("subject1")));
        EXPECT_EQ(result->item(3)->GetIdAttribute(), "div19");
        EXPECT_TRUE(result->item(3)->ClassNames().Contains(AtomicString("subject1")));
    }

    {
        StaticElementList* result = scope->QuerySelectorAll(AtomicString(":has(+ .a > .b .c)"));
        ASSERT_EQ(2U, result->length());
        EXPECT_EQ(result->item(0)->GetIdAttribute(), "div5");
        EXPECT_TRUE(result->item(0)->ClassNames().Contains(AtomicString("subject2")));
        EXPECT_EQ(result->item(1)->GetIdAttribute(), "div17");
        EXPECT_TRUE(result->item(1)->ClassNames().Contains(AtomicString("subject2")));
    }

    {
        ...
        EXPECT_TRUE(result->item(0)->ClassNames().Contains(AtomicString("subject3")));
        EXPECT_EQ(result->item(1)->GetIdAttribute(), "div4");
        EXPECT_TRUE(result->item(1)->ClassNames().Contains(AtomicString("subject3")));
        EXPECT_EQ(result->item(2)->GetIdAttribute(), "div16");
        EXPECT_TRUE(result->item(2)->ClassNames().Contains(AtomicString("subject3")));
    }

    {
        ...
        ASSERT_EQ(3U, result->length());
        EXPECT_EQ(result->item(0)->GetIdAttribute(), "div4");
        EXPECT_TRUE(result->item(0)->ClassNames().Contains(AtomicString("subject4")));
        EXPECT_EQ(result->item(1)->GetIdAttribute(), "div7");
        EXPECT_TRUE(result->item(1)->ClassNames().Contains(AtomicString("subject4")));
        EXPECT_EQ(result->item(2)->GetIdAttribute(), "div19");
        EXPECT_TRUE(result->item(2)->ClassNames().Contains(AtomicString("subject4")));
    }

    {
        StaticElementList* result = scope->QuerySelectorAll(AtomicString(":has(~ .a ~ .b .d)"));
        ASSERT_EQ(3U, result->length());
        EXPECT_EQ(result->item(0)->GetIdAttribute(), "div5");
        EXPECT_TRUE(result->item(0)->ClassNames().Contains(AtomicString("subject5")));
        EXPECT_EQ(result->item(1)->GetIdAttribute(), "div17");
        EXPECT_TRUE(result->item(1)->ClassNames().Contains(AtomicString("subject5")));
        EXPECT_EQ(result->item(2)->GetIdAttribute(), "div20");
        EXPECT_TRUE(result->item(2)->ClassNames().Contains(AtomicString("subject5")));
    }

    {
        ...
        ASSERT_EQ(2U, result->length());
        EXPECT_EQ(result->item(0)->GetIdAttribute(), "div5");
        EXPECT_TRUE(result->item(0)->ClassNames().Contains(AtomicString("subject6")));
        EXPECT_EQ(result->item(1)->GetIdAttribute(), "div20");
        EXPECT_TRUE(result->item(1)->ClassNames().Contains(AtomicString("subject6")));
    }
}
