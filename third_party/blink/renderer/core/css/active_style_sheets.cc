...
ActiveSheetsChange CompareActiveStyleSheets(const ActiveStyleSheetVector& old_style_sheets, const ActiveStyleSheetVector& new_style_sheets,
    const HeapVector<Member<RuleSetDiff>>& diffs, HeapHashSet<Member<RuleSet>>& changed_rule_sets)
{
    ...
    for (; index < min_count && new_style_sheets[index].first == old_style_sheets[index].first; index++) {
        ...
        if (new_style_sheets[index].second && old_style_sheets[index].second) {
            for (const RuleSetDiff* diff : diffs) {
                if (diff->Matches(old_style_sheets[index].second, new_style_sheets[index].second)) {
    ...
    merged_sorted.reserve(old_style_sheet_count + new_style_sheet_count - 2 * index);
    merged_sorted.AppendSpan(base::span(old_style_sheets.data(), old_style_sheets.size()).subspan(index));
    merged_sorted.AppendSpan(base::span(new_style_sheets.data(), new_style_sheets.size()).subspan(index));
    ...
    while (merged_iterator != merged_end) {
        ...
        if (merged_iterator == merged_end || (*merged_iterator).first != sheet1.first) {
...
bool HasMediaQueries(const ActiveStyleSheetVector& active_style_sheets)
{
    for (const auto& active_sheet : active_style_sheets) {
        if (const MediaQuerySet* media_queries = active_sheet.first->MediaQueries()) {
...
bool HasDynamicViewportDependentMediaQueries(const ActiveStyleSheetVector& active_style_sheets)
{
    for (const auto& active_sheet : active_style_sheets) {
        ...
        if (contents->GetRuleSet().Features().HasDynamicViewportDependentMediaQueries()) {
            return true;
        }
    }
    return false;
...
bool AffectedByMediaValueChange(const ActiveStyleSheetVector& active_sheets, MediaValueChange change)
{
