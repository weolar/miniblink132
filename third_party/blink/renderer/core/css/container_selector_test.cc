...
protected:
    ContainerQuery* ParseContainerQuery(Document& document, String query)
    {
        ...
        auto* style_rule = DynamicTo<StyleRuleContainer>(css_test_helpers::ParseRule(document, rule));
        if (!style_rule) {
            return nullptr;
        }
...
TEST_F(ContainerSelectorTest, ContainerSelectorHashing)
{
    ...
    ContainerQuery* query2 = ParseContainerQuery(*document, "style(--foo: bar) and scroll-state(snapped)");
    ASSERT_TRUE(query1);
    ASSERT_TRUE(query2);
    EXPECT_NE(query1->Selector().GetHash(), query2->Selector().GetHash()) << "The query selectors should not generate the same hash since they "
