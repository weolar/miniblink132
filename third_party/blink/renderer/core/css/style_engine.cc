...
const Vector<AtomicString> ConvertFontFamilyToVector(const CSSValue* value)
{
    ...
    for (wtf_size_t i = 0; i < length; i++) {
        const CSSFontFamilyValue* family_value = DynamicTo<CSSFontFamilyValue>(family_list->Item(i));
        if (!family_value) {
            return Vector<AtomicString>();
        }
...
StyleEngine::StyleEngine(Document& document)
    : document_(&document)
    , style_containment_scope_tree_(MakeGarbageCollected<StyleContainmentScopeTree>())
    , document_style_sheet_collection_(MakeGarbageCollected<DocumentStyleSheetCollection>(document))
...
TreeScopeStyleSheetCollection& StyleEngine::EnsureStyleSheetCollectionFor(TreeScope& tree_scope)
{
    if (tree_scope == document_) {
        return GetDocumentStyleSheetCollection();
    }

    StyleSheetCollectionMap::AddResult result = style_sheet_collection_map_.insert(&tree_scope, nullptr);
    if (result.is_new_entry) {
        result.stored_value->value = MakeGarbageCollected<ShadowTreeStyleSheetCollection>(To<ShadowRoot>(tree_scope));
    }
    return *result.stored_value->value.Get();
}

TreeScopeStyleSheetCollection* StyleEngine::StyleSheetCollectionFor(TreeScope& tree_scope)
{
    if (tree_scope == document_) {
        return &GetDocumentStyleSheetCollection();
    }

    StyleSheetCollectionMap::iterator it = style_sheet_collection_map_.find(&tree_scope);
...
const HeapVector<Member<StyleSheet>>& StyleEngine::StyleSheetsForStyleSheetList(TreeScope& tree_scope)
{
    DCHECK(document_);
    TreeScopeStyleSheetCollection& collection = EnsureStyleSheetCollectionFor(tree_scope);
...
void StyleEngine::InjectSheet(const StyleSheetKey& key, StyleSheetContents* sheet, WebCssOrigin origin)
{
    HeapVector<std::pair<StyleSheetKey, Member<CSSStyleSheet>>>& injected_style_sheets
        = origin == WebCssOrigin::kUser ? injected_user_style_sheets_ : injected_author_style_sheets_;
    injected_style_sheets.push_back(std::make_pair(key, MakeGarbageCollected<CSSStyleSheet>(sheet, *document_)));
...
void StyleEngine::RemoveInjectedSheet(const StyleSheetKey& key, WebCssOrigin origin)
{
    HeapVector<std::pair<StyleSheetKey, Member<CSSStyleSheet>>>& injected_style_sheets
        = origin == WebCssOrigin::kUser ? injected_user_style_sheets_ : injected_author_style_sheets_;
    // Remove the last sheet that matches.
    const auto& it = base::ranges::find(base::Reversed(injected_style_sheets), key, &std::pair<StyleSheetKey, Member<CSSStyleSheet>>::first);
...
CSSStyleSheet& StyleEngine::EnsureInspectorStyleSheet()
{
    ...
    auto* contents = MakeGarbageCollected<StyleSheetContents>(MakeGarbageCollected<CSSParserContext>(*document_));
    inspector_style_sheet_ = MakeGarbageCollected<CSSStyleSheet>(contents, *document_);
    MarkDocumentDirty();
...
{
    DCHECK(type == PendingSheetType::kBlocking || type == PendingSheetType::kDynamicRenderBlocking);

    auto* manager = GetDocument().GetRenderBlockingResourceManager();
    bool is_render_blocking = manager && manager->AddPendingStylesheet(style_sheet_candidate_node);
    ...
    if (!is_render_blocking) {
        ...
        if (GetDocument().body()) {
            GetDocument().CountUse(WebFeature::kPendingStylesheetAddedAfterBodyStarted);
        }
...
void StyleEngine::RemovePendingBlockingSheet(Node& style_sheet_candidate_node, PendingSheetType type)
{
    DCHECK(type == PendingSheetType::kBlocking || type == PendingSheetType::kDynamicRenderBlocking);
    ...
    auto* manager = GetDocument().GetRenderBlockingResourceManager();
    bool is_render_blocking = manager && manager->RemovePendingStylesheet(style_sheet_candidate_node);
...
void StyleEngine::RemoveStyleSheetCandidateNode(Node& node, ContainerNode& insertion_point)
{
    DCHECK(!IsXSLStyleSheet(node));
    ...
    static_assert(std::is_base_of<TreeScope, ShadowRoot>::value, "The ShadowRoot must be subclass of TreeScope.");
    TreeScope& tree_scope = shadow_root ? static_cast<TreeScope&>(*shadow_root) : GetDocument();
    TreeScopeStyleSheetCollection* collection = StyleSheetCollectionFor(tree_scope);
...
void StyleEngine::AdoptedStyleSheetAdded(TreeScope& tree_scope, CSSStyleSheet* sheet)
{
...
void StyleEngine::AdoptedStyleSheetRemoved(TreeScope& tree_scope, CSSStyleSheet* sheet)
{
    if (GetDocument().IsDetached()) {
        return;
    }
...
void StyleEngine::WatchedSelectorsChanged()
{
    ...
    MarkAllElementsForStyleRecalc(StyleChangeReasonForTracing::Create(style_change_reason::kDeclarativeContent));
}

void StyleEngine::DocumentRulesSelectorsChanged()
{
    DCHECK(global_rule_set_);
    Member<RuleSet> old_rule_set = global_rule_set_->DocumentRulesSelectorsRuleSet();
    global_rule_set_->UpdateDocumentRulesSelectorsRuleSet(GetDocument());
    Member<RuleSet> new_rule_set = global_rule_set_->DocumentRulesSelectorsRuleSet();
...
void StyleEngine::MediaQueryAffectingValueChanged(UnorderedTreeScopeSet& tree_scopes, MediaValueChange change)
{
...
Element* StyleEngine::EnsureVTTOriginatingElement()
{
    if (!vtt_originating_element_) {
        vtt_originating_element_ = MakeGarbageCollected<Element>(QualifiedName(g_null_atom, g_empty_atom, g_empty_atom), document_);
    }
    return vtt_originating_element_.Get();
}

void StyleEngine::MediaQueryAffectingValueChanged(HeapHashSet<Member<TextTrack>>& text_tracks, MediaValueChange change)
{
    ...
    for (auto text_track : text_tracks) {
        ...
        if (style_needs_recalc && text_track->Owner()) {
            ...
            // changes.
            text_track->Owner()->SetNeedsStyleRecalc(kSubtreeStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kShadow));
        }
    }
}
...
void StyleEngine::UpdateActiveStyleSheetsInShadow(TreeScope* tree_scope, UnorderedTreeScopeSet& tree_scopes_removed)
{
    DCHECK_NE(tree_scope, document_);
    auto* collection = To<ShadowTreeStyleSheetCollection>(StyleSheetCollectionFor(*tree_scope));
    DCHECK(collection);
    collection->UpdateActiveStyleSheets(*this);
    if (!collection->HasStyleSheetCandidateNodes() && !tree_scope->HasAdoptedStyleSheets()) {
...
void StyleEngine::UpdateCounterStyles()
{
    ...
    CounterStyleMap::MarkAllDirtyCounterStyles(GetDocument(), active_tree_scopes_);
    CounterStyleMap::ResolveAllReferences(GetDocument(), active_tree_scopes_);
    counter_styles_need_update_ = false;
}

void StyleEngine::MarkPositionTryStylesDirty(const HeapHashSet<Member<RuleSet>>& changed_rule_sets)
{
...
{
    return (viewport_resolver_ && viewport_resolver_->NeedsUpdate()) || NeedsActiveStyleSheetUpdate() || (global_rule_set_ && global_rule_set_->IsDirty());
}
...
const ActiveStyleSheetVector StyleEngine::ActiveStyleSheetsForInspector()
{
    ...
    active_style_sheets.AppendVector(GetDocumentStyleSheetCollection().ActiveStyleSheets());
    for (TreeScope* tree_scope : active_tree_scopes_) {
        if (TreeScopeStyleSheetCollection* collection = style_sheet_collection_map_.at(tree_scope)) {
...
void StyleEngine::UpdateLayoutCounters(const LayoutObject& layout_object, CountersAttachmentContext& context)
{
    // Check out the parameter list ^^^
    for (LayoutObject* child = layout_object.NextInPreOrder(&layout_object); child; child = child->NextInPreOrder(&layout_object)) {
        if (auto* layout_counter = DynamicTo<LayoutCounter>(child)) {
            Vector<int> counter_values = context.GetCounterValues(layout_object, layout_counter->Identifier(), layout_counter->Separator().IsNull());
            layout_counter->UpdateCounter(std::move(counter_values));
        }
    }
}

void StyleEngine::UpdateCounters(const Element& element, CountersAttachmentContext& context)
{
    ...
    if (layout_object) {
        ...
        } else if (auto* inline_list_item = DynamicTo<LayoutInlineListItem>(layout_object)) {
            if (!inline_list_item->Ordinal().UseExplicitValue()) {
                inline_list_item->Ordinal().MarkDirty();
                inline_list_item->OrdinalValueChanged();
            }
        }
        if (element.GetComputedStyle() && !element.GetComputedStyle()->ContentBehavesAsNormal()) {
            UpdateLayoutCounters(*layout_object, context);
        }
    }
    for (Node* child = LayoutTreeBuilderTraversal::FirstChild(element); child; child = LayoutTreeBuilderTraversal::NextSibling(*child)) {
...
Font StyleEngine::ComputeFont(Element& element, const ComputedStyle& font_style, const CSSPropertyValueSet& font_properties)
{
    UpdateActiveStyle();
...
RuleSet* StyleEngine::RuleSetScope::RuleSetForSheet(StyleEngine& engine, CSSStyleSheet* css_sheet)
{
    RuleSet* rule_set = engine.RuleSetForSheet(*css_sheet);
    if (rule_set && rule_set->HasCascadeLayers() && !css_sheet->Contents()->HasSingleOwnerNode() && !layer_rule_sets_.insert(rule_set).is_new_entry) {
...
void StyleEngine::ClearResolvers()
{
    ...
    if (resolver_) {
        TRACE_EVENT1("blink", "StyleEngine::clearResolver", "frame", GetFrameIdForTracing(GetDocument().GetFrame()));
        resolver_->Dispose();
        resolver_.Clear();
    }
}
...
bool StyleEngine::ClearFontFaceCacheAndAddUserFonts(const ActiveStyleSheetVector& user_sheets)
{
    bool fonts_changed = false;

    if (font_selector_ && font_selector_->GetFontFaceCache()->ClearCSSConnected()) {
        fonts_changed = true;
        if (resolver_) {
...
void StyleEngine::RemoveFontFaceRules(const HeapVector<Member<const StyleRuleFontFace>>& font_face_rules)
{
    if (!font_selector_) {
        return;
    }
...
CSSStyleSheet* StyleEngine::CreateSheet(
    Element& element, const String& text, TextPosition start_position, PendingSheetType type, RenderBlockingBehavior render_blocking_behavior)
{
    ...
    if (result.is_new_entry || !contents || !contents->IsCacheableForStyleElement()) {
        result.stored_value->value = nullptr;
        style_sheet = ParseSheet(element, text, start_position, render_blocking_behavior);
        ...
        style_sheet = CSSStyleSheet::CreateInline(contents, element, start_position);
    }
...
CSSStyleSheet* StyleEngine::ParseSheet(Element& element, const String& text, TextPosition start_position, RenderBlockingBehavior render_blocking_behavior)
{
    CSSStyleSheet* style_sheet = nullptr;
    style_sheet = CSSStyleSheet::CreateInline(element, NullURL(), start_position, GetDocument().Encoding());
...
void StyleEngine::CollectUserStyleFeaturesTo(RuleFeatureSet& features) const
{
    for (unsigned i = 0; i < active_user_style_sheets_.size(); ++i) {
        ...
        features.MutableMediaQueryResultFlags().Add(sheet->GetMediaQueryResultFlags());
...
{
    HeapHashSet<Member<const StyleSheetContents>> visited_shared_style_sheet_contents;
    if (GetDocument().GetScopedStyleResolver()) {
        GetDocument().GetScopedStyleResolver()->CollectFeaturesTo(features, visited_shared_style_sheet_contents);
    }
    for (TreeScope* tree_scope : active_tree_scopes_) {
        if (ScopedStyleResolver* resolver = tree_scope->GetScopedStyleResolver()) {
            resolver->CollectFeaturesTo(features, visited_shared_style_sheet_contents);
        }
    }
}
...
template <typename Func> void MarkElementsForRecalc(TreeScope& tree_scope, const StyleChangeReasonForTracing& reason, Func predicate)
{
    for (Element* element = ElementTraversal::FirstWithin(tree_scope.RootNode()); element; element = ElementTraversal::NextIncludingPseudo(*element)) {
...
void StyleEngine::InvalidateViewportUnitStylesIfNeeded()
{
    ...
    if (initial_data_ && (initial_data_->GetViewportUnitFlags() & dirty_flags)) {
        ...
        MarkAllElementsForStyleRecalc(StyleChangeReasonForTracing::Create(style_change_reason::kViewportUnits));
        return;
    }

    const auto& reason = StyleChangeReasonForTracing::Create(style_change_reason::kViewportUnits);
    MarkElementsForRecalc(GetDocument(), reason, [dirty_flags](const ComputedStyle& style) {
        return (style.ViewportUnitFlags() & dirty_flags) || style.HighlightPseudoElementStylesDependOnViewportUnits();
    });
}
...
void StyleEngine::FontsNeedUpdate(FontSelector*, FontInvalidationReason)
{
    ...
    probe::FontsUpdated(document_->GetExecutionContext(), nullptr, String(), nullptr);
}
...
void StyleEngine::PlatformColorsChanged()
{
    ...
    MarkAllElementsForStyleRecalc(StyleChangeReasonForTracing::Create(style_change_reason::kPlatformColorChange));
...
{
    DCHECK(element.GetDocument() == &GetDocument()) << "Only schedule invalidations using the StyleEngine of the Document "
    ...
    if (!global_rule_set_) {
        ...
        NOTREACHED() << "global_rule_set_ should only be null for inactive documents.";
    }
...
{
    return element.AncestorsOrAncestorSiblingsAffectedByHas() || element.GetSiblingsAffectedByHasFlags() || element.AffectedByLogicalCombinationsInHas();
}

bool InsertionOrRemovalPossiblyAffectHasStateOfAncestorsOrAncestorSiblings(Element* parent)
{
    ...
    return parent
        && (parent->AncestorsOrAncestorSiblingsAffectedByHas()
            || parent->HasSiblingsAffectedByHasFlags(SiblingsAffectedByHasFlags::kFlagForSiblingDescendantRelationship));
}

bool InsertionOrRemovalPossiblyAffectHasStateOfPreviousSiblings(Element* previous_sibling)
{
...
void StyleEngine::InvalidateElementAffectedByHas(Element& element, bool for_element_affected_by_pseudo_in_has)
{
    ...
    if (element.AffectedBySubjectHas()) {
        ...
        element.SetNeedsStyleRecalc(StyleChangeType::kLocalStyleChange, StyleChangeReasonForTracing::Create(blink::style_change_reason::kAffectedByHas));
    ...
    if (element.AffectedByNonSubjectHas()) {
        ...
        GetRuleFeatureSet().GetRuleInvalidationData().CollectInvalidationSetsForPseudoClass(invalidation_lists, element, CSSSelector::kPseudoHas);
...
public:
    ...
    static PseudoHasInvalidationTraversalContext ForAttributeOrPseudoStateChange(Element& changed_element)
    {
        bool traverse_ancestors = changed_element.AncestorsOrAncestorSiblingsAffectedByHas();
        ...
        Element* previous_sibling = changed_element.GetSiblingsAffectedByHasFlags() ? ElementTraversal::PreviousSibling(changed_element) : nullptr;
        if (previous_sibling) {
            first_element = previous_sibling;
            is_first_element_shadow_host = false;
        }

        return PseudoHasInvalidationTraversalContext(first_element, is_first_element_shadow_host, traverse_ancestors);
    }

    // Create :has() invalidation traversal context for element or subtree
    // insertion.
    static PseudoHasInvalidationTraversalContext ForInsertion(Element* parent_or_shadow_host, bool insert_shadow_root_child, Element* previous_sibling)
    {
        ...
        if (first_element) {
            traverse_ancestors = first_element->AncestorsOrAncestorSiblingsAffectedByHas();
    ...
    static PseudoHasInvalidationTraversalContext ForRemoval(
        Element* parent_or_shadow_host, bool remove_shadow_root_child, Element* previous_sibling, Element& removed_element)
    {
        Element* first_element = nullptr;
        bool is_first_element_shadow_host = false;

        bool traverse_ancestors = removed_element.AncestorsOrAncestorSiblingsAffectedByHas();
        if (traverse_ancestors) {
    ...
    // a parent.
    static PseudoHasInvalidationTraversalContext ForAllChildrenRemoved(Element& parent)
    {
        return PseudoHasInvalidationTraversalContext(&parent, /* is_first_element_shadow_host */ false, parent.AncestorsOrAncestorSiblingsAffectedByHas());
    }

private:
    PseudoHasInvalidationTraversalContext(Element* first_element, bool is_first_element_shadow_host, bool traverse_to_parent_of_first_element)
        : first_element_(first_element)
        , is_first_element_shadow_host_(is_first_element_shadow_host)
        , traverse_to_parent_of_first_element_(traverse_to_parent_of_first_element)
    {
    }
...
void StyleEngine::InvalidateAncestorsOrSiblingsAffectedByHas(const PseudoHasInvalidationTraversalContext& traversal_context)
{
    ...
    bool for_element_affected_by_pseudo_in_has = traversal_context.ForElementAffectedByPseudoInHas();
    ...
    while (element) {
        ...
        InvalidateElementAffectedByHas(*element, for_element_affected_by_pseudo_in_has);
    ...
    if (shadow_host) {
        InvalidateElementAffectedByHas(*shadow_host, for_element_affected_by_pseudo_in_has);
    }
}

void StyleEngine::InvalidateChangedElementAffectedByLogicalCombinationsInHas(Element& changed_element, bool for_element_affected_by_pseudo_in_has)
{
    if (!changed_element.AffectedByLogicalCombinationsInHas()) {
        return;
    }
    InvalidateElementAffectedByHas(changed_element, for_element_affected_by_pseudo_in_has);
}

void StyleEngine::ClassChangedForElement(const SpaceSplitString& changed_classes, Element& element)
{
    if (ShouldSkipInvalidationFor(element)) {
        return;
    }

    const RuleInvalidationData& rule_invalidation_data = GetRuleFeatureSet().GetRuleInvalidationData();

    if (rule_invalidation_data.NeedsHasInvalidationForClassChange() && PossiblyAffectingHasState(element)) {
        for (const AtomicString& changed_class : changed_classes) {
            if (rule_invalidation_data.NeedsHasInvalidationForClass(changed_class)) {
                InvalidateChangedElementAffectedByLogicalCombinationsInHas(element, /* for_element_affected_by_pseudo_in_has */ false);
                InvalidateAncestorsOrSiblingsAffectedByHas(PseudoHasInvalidationTraversalContext::ForAttributeOrPseudoStateChange(element));
                break;
            }
        }
    }
    ...
    InvalidationLists invalidation_lists;
    for (const AtomicString& changed_class : changed_classes) {
        rule_invalidation_data.CollectInvalidationSetsForClass(invalidation_lists, element, changed_class);
    }
    pending_invalidations_.ScheduleInvalidationSetsForNode(invalidation_lists, element);
}

void StyleEngine::ClassChangedForElement(const SpaceSplitString& old_classes, const SpaceSplitString& new_classes, Element& element)
{
    if (ShouldSkipInvalidationFor(element)) {
        return;
    }
    ...
    bool possibly_affecting_has_state = rule_invalidation_data.NeedsHasInvalidationForClassChange() && PossiblyAffectingHasState(element);
    ...
    for (const AtomicString& new_class : new_classes) {
        ...
        if (!found) {
            if (needs_schedule_invalidation) [[likely]] {
                rule_invalidation_data.CollectInvalidationSetsForClass(invalidation_lists, element, new_class);
            }
    ...
    for (unsigned i = 0; i < old_classes.size(); ++i) {
        ...
        // Class was removed.
        if (needs_schedule_invalidation) [[likely]] {
            rule_invalidation_data.CollectInvalidationSetsForClass(invalidation_lists, element, old_classes[i]);
        }
        if (possibly_affecting_has_state) [[unlikely]] {
...
bool HasAttributeDependentGeneratedContent(const Element& element)
{
    ...
    return HasAttrFunc(element.GetPseudoElement(kPseudoIdCheck)) || HasAttrFunc(element.GetPseudoElement(kPseudoIdBefore))
        || HasAttrFunc(element.GetPseudoElement(kPseudoIdAfter)) || HasAttrFunc(element.GetPseudoElement(kPseudoIdSelectArrow))
...
void StyleEngine::AttributeChangedForElement(const QualifiedName& attribute_name, Element& element)
{
    if (ShouldSkipInvalidationFor(element)) {
        return;
    }
    ...
    if (rule_invalidation_data.NeedsHasInvalidationForAttributeChange() && PossiblyAffectingHasState(element)) {
        if (rule_invalidation_data.NeedsHasInvalidationForAttribute(attribute_name)) {
            InvalidateChangedElementAffectedByLogicalCombinationsInHas(element, /* for_element_affected_by_pseudo_in_has */ false);
            InvalidateAncestorsOrSiblingsAffectedByHas(PseudoHasInvalidationTraversalContext::ForAttributeOrPseudoStateChange(element));
        }
    }
    ...
    InvalidationLists invalidation_lists;
    rule_invalidation_data.CollectInvalidationSetsForAttribute(invalidation_lists, element, attribute_name);
    pending_invalidations_.ScheduleInvalidationSetsForNode(invalidation_lists, element);

    if (!element.NeedsStyleRecalc()) {
        bool attr_dependent
            = RuntimeEnabledFeatures::CSSAdvancedAttrFunctionEnabled() ? HasAttributeDependentStyle(element) : HasAttributeDependentGeneratedContent(element);
        if (attr_dependent) {
            element.SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::FromAttribute(attribute_name));
        }
    }
}

void StyleEngine::IdChangedForElement(const AtomicString& old_id, const AtomicString& new_id, Element& element)
{
    if (ShouldSkipInvalidationFor(element)) {
        return;
    }
    ...
    if (rule_invalidation_data.NeedsHasInvalidationForIdChange() && PossiblyAffectingHasState(element)) {
        if ((!old_id.empty() && rule_invalidation_data.NeedsHasInvalidationForId(old_id))
            || (!new_id.empty() && rule_invalidation_data.NeedsHasInvalidationForId(new_id))) {
            InvalidateChangedElementAffectedByLogicalCombinationsInHas(element, /* for_element_affected_by_pseudo_in_has */ false);
            InvalidateAncestorsOrSiblingsAffectedByHas(PseudoHasInvalidationTraversalContext::ForAttributeOrPseudoStateChange(element));
        }
    }
    ...
    InvalidationLists invalidation_lists;
    if (!old_id.empty()) {
        rule_invalidation_data.CollectInvalidationSetsForId(invalidation_lists, element, old_id);
    }
    if (!new_id.empty()) {
        rule_invalidation_data.CollectInvalidationSetsForId(invalidation_lists, element, new_id);
    }
    pending_invalidations_.ScheduleInvalidationSetsForNode(invalidation_lists, element);
}

void StyleEngine::PseudoStateChangedForElement(
    CSSSelector::PseudoType pseudo_type, Element& element, bool invalidate_descendants_or_siblings, bool invalidate_ancestors_or_siblings)
{
    DCHECK(invalidate_descendants_or_siblings || invalidate_ancestors_or_siblings);
    ...
    if (invalidate_ancestors_or_siblings && rule_invalidation_data.NeedsHasInvalidationForPseudoStateChange() && PossiblyAffectingHasState(element)) {
        if (rule_invalidation_data.NeedsHasInvalidationForPseudoClass(pseudo_type)) {
            InvalidateChangedElementAffectedByLogicalCombinationsInHas(element, /* for_element_affected_by_pseudo_in_has */ true);
            InvalidateAncestorsOrSiblingsAffectedByHas(
                PseudoHasInvalidationTraversalContext::ForAttributeOrPseudoStateChange(element).SetForElementAffectedByPseudoInHas());
        }
    }

    if (!invalidate_descendants_or_siblings || IsSubtreeAndSiblingsStyleDirty(element)) {
        return;
    }

    InvalidationLists invalidation_lists;
    rule_invalidation_data.CollectInvalidationSetsForPseudoClass(invalidation_lists, element, pseudo_type);
    pending_invalidations_.ScheduleInvalidationSetsForNode(invalidation_lists, element);
}
...
void StyleEngine::PartChangedForElement(Element& element)
{
    ...
    element.SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::FromAttribute(html_names::kPartAttr));
}
...
void StyleEngine::ExportpartsChangedForElement(Element& element)
{
    ...
    InvalidationLists invalidation_lists;
    GetRuleFeatureSet().GetRuleInvalidationData().CollectPartInvalidationSet(invalidation_lists);
    pending_invalidations_.ScheduleInvalidationSetsForNode(invalidation_lists, element);
}

void StyleEngine::ScheduleSiblingInvalidationsForElement(Element& element, ContainerNode& scheduling_parent, unsigned min_direct_adjacent)
{
    ...
    if (element.HasID()) {
        rule_invalidation_data.CollectSiblingInvalidationSetForId(invalidation_lists, element, element.IdForStyleResolution(), min_direct_adjacent);
    }

    if (element.HasClass()) {
        const SpaceSplitString& class_names = element.ClassNames();
        for (const AtomicString& class_name : class_names) {
            rule_invalidation_data.CollectSiblingInvalidationSetForClass(invalidation_lists, element, class_name, min_direct_adjacent);
        }
    }

    for (const Attribute& attribute : element.Attributes()) {
        rule_invalidation_data.CollectSiblingInvalidationSetForAttribute(invalidation_lists, element, attribute.GetName(), min_direct_adjacent);
    }

    rule_invalidation_data.CollectUniversalSiblingInvalidationSet(invalidation_lists, min_direct_adjacent);
...
void StyleEngine::ScheduleInvalidationsForInsertedSibling(Element* before_element, Element& inserted_element)
{
    unsigned affected_siblings
        = inserted_element.parentNode()->ChildrenAffectedByIndirectAdjacentRules() ? SiblingInvalidationSet::kDirectAdjacentMax : MaxDirectAdjacentSelectors();
    ...
    for (unsigned i = 1; before_element && i <= affected_siblings; i++, before_element = ElementTraversal::PreviousSibling(*before_element)) {
        ScheduleSiblingInvalidationsForElement(*before_element, *scheduling_parent, i);
    }
}

void StyleEngine::ScheduleInvalidationsForRemovedSibling(Element* before_element, Element& removed_element, Element& after_element)
{
    unsigned affected_siblings
        = after_element.parentNode()->ChildrenAffectedByIndirectAdjacentRules() ? SiblingInvalidationSet::kDirectAdjacentMax : MaxDirectAdjacentSelectors();
...
{
    InvalidationLists invalidation_lists;
    GetRuleFeatureSet().GetRuleInvalidationData().CollectNthInvalidationSet(invalidation_lists);
...
{
    return result.HasFlag(MatchFlag::kAffectedByDrag) || result.HasFlag(MatchFlag::kAffectedByFocusWithin) || result.HasFlag(MatchFlag::kAffectedByHover)
        || result.HasFlag(MatchFlag::kAffectedByActive);
}

static bool AnyRuleCausesInvalidation(const MatchRequest& match_request, ElementRuleCollector& collector, bool is_shadow_host)
{
    if (collector.CheckIfAnyRuleMatches(match_request) || FlagsCauseInvalidation(collector.MatchedResult())) {
        return true;
    }
    if (is_shadow_host) {
        if (collector.CheckIfAnyShadowHostRuleMatches(match_request) || FlagsCauseInvalidation(collector.MatchedResult())) {
            return true;
        }
    }
    return false;
...
bool CanRejectRuleSet(ElementRuleCollector& collector, const RuleSet& rule_set)
{
...
void StyleEngine::ApplyRuleSetInvalidationForElement(const TreeScope& tree_scope, Element& element, SelectorFilter& selector_filter,
    StyleScopeFrame& style_scope_frame, const HeapHashSet<Member<RuleSet>>& rule_sets, unsigned changed_rule_flags, bool is_shadow_host)
{
    if ((changed_rule_flags & kFunctionRules) && element.GetComputedStyle() && element.GetComputedStyle()->AffectedByCSSFunction()) {
        ...
        element.SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kFunctionRuleChange));
        return;
    }
    ElementResolveContext element_resolve_context(element);
    MatchResult match_result;
    EInsideLink inside_link = EInsideLink::kNotInsideLink; // Only used for MatchedProperties, so does
        // not matter for us.
    StyleRecalcContext style_recalc_context = StyleRecalcContext::FromAncestors(element);
    style_recalc_context.style_scope_frame = &style_scope_frame;
    ElementRuleCollector collector(element_resolve_context, style_recalc_context, selector_filter, match_result, inside_link);
    ...
    if (!match_request.IsEmpty() && !matched_any) {
        matched_any = AnyRuleCausesInvalidation(match_request, collector, is_shadow_host);
    }
    if (matched_any) {
        element.SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kStyleRuleChange));
    }
}

void StyleEngine::ScheduleCustomElementInvalidations(HashSet<AtomicString> tag_names)
{
    scoped_refptr<DescendantInvalidationSet> invalidation_set = DescendantInvalidationSet::Create();
    ...
    pending_invalidations_.ScheduleInvalidationSetsForNode(invalidation_lists, *document_);
}

void StyleEngine::ScheduleInvalidationsForHasPseudoAffectedByInsertionOrRemoval(
    ContainerNode* parent, Node* node_before_change, Element& changed_element, bool removal)
{
    ...
    if (!GetRuleFeatureSet().GetRuleInvalidationData().NeedsHasInvalidationForInsertionOrRemoval()) {
        return;
    }
    ...
    if (removal) {
        ScheduleInvalidationsForHasPseudoAffectedByRemoval(parent_or_shadow_host, previous_sibling, changed_element, insert_or_remove_shadow_root_child);
    } else {
        ScheduleInvalidationsForHasPseudoAffectedByInsertion(parent_or_shadow_host, previous_sibling, changed_element, insert_or_remove_shadow_root_child);
    }
}

void StyleEngine::ScheduleInvalidationsForHasPseudoAffectedByInsertion(
    Element* parent_or_shadow_host, Element* previous_sibling, Element& inserted_element, bool insert_shadow_root_child)
{
    bool possibly_affecting_has_state = false;
    bool descendants_possibly_affecting_has_state = false;

    if (InsertionOrRemovalPossiblyAffectHasStateOfPreviousSiblings(previous_sibling)) {
        inserted_element.SetSiblingsAffectedByHasFlags(previous_sibling->GetSiblingsAffectedByHasFlags());
        possibly_affecting_has_state = true;
        descendants_possibly_affecting_has_state
            = inserted_element.HasSiblingsAffectedByHasFlags(SiblingsAffectedByHasFlags::kFlagForSiblingDescendantRelationship);
    }
    if (InsertionOrRemovalPossiblyAffectHasStateOfAncestorsOrAncestorSiblings(parent_or_shadow_host)) {
    ...
    bool needs_has_invalidation_for_inserted_subtree = parent_or_shadow_host->ChildrenAffectedByDirectAdjacentRules();

    if (!needs_has_invalidation_for_inserted_subtree && rule_invalidation_data.NeedsHasInvalidationForInsertedOrRemovedElement(inserted_element)) {
    ...
    if (descendants_possibly_affecting_has_state) {
        ...
        for (Element& element : ElementTraversal::DescendantsOf(inserted_element)) {
            ...
            if (!needs_has_invalidation_for_inserted_subtree && rule_invalidation_data.NeedsHasInvalidationForInsertedOrRemovedElement(element)) {
                needs_has_invalidation_for_inserted_subtree = true;
            }
        }
    }
    ...
    if (needs_has_invalidation_for_inserted_subtree) {
        InvalidateAncestorsOrSiblingsAffectedByHas(
            PseudoHasInvalidationTraversalContext::ForInsertion(parent_or_shadow_host, insert_shadow_root_child, previous_sibling));
        return;
    }
...
void StyleEngine::ScheduleInvalidationsForHasPseudoAffectedByRemoval(
    Element* parent_or_shadow_host, Element* previous_sibling, Element& removed_element, bool remove_shadow_root_child)
{
    if (!InsertionOrRemovalPossiblyAffectHasStateOfAncestorsOrAncestorSiblings(parent_or_shadow_host)
        && !InsertionOrRemovalPossiblyAffectHasStateOfPreviousSiblings(previous_sibling)) {
    ...
    if (parent_or_shadow_host->ChildrenAffectedByDirectAdjacentRules()) {
        InvalidateAncestorsOrSiblingsAffectedByHas(
            PseudoHasInvalidationTraversalContext::ForRemoval(parent_or_shadow_host, remove_shadow_root_child, previous_sibling, removed_element));
        return;
    }

    const RuleInvalidationData& rule_invalidation_data = GetRuleFeatureSet().GetRuleInvalidationData();

    for (Element& element : ElementTraversal::InclusiveDescendantsOf(removed_element)) {
        if (rule_invalidation_data.NeedsHasInvalidationForInsertedOrRemovedElement(element)) {
            InvalidateAncestorsOrSiblingsAffectedByHas(
                PseudoHasInvalidationTraversalContext::ForRemoval(parent_or_shadow_host, remove_shadow_root_child, previous_sibling, removed_element));
            return;
        }
...
void StyleEngine::ScheduleInvalidationsForHasPseudoWhenAllChildrenRemoved(Element& parent)
{
    ...
    if (!InsertionOrRemovalPossiblyAffectHasStateOfAncestorsOrAncestorSiblings(&parent)) {
        // Removed children will not affect :has() state
        return;
    }

    // Always invalidate elements possibly affected by the removed children.
    InvalidateAncestorsOrSiblingsAffectedByHas(PseudoHasInvalidationTraversalContext::ForAllChildrenRemoved(parent));
}

void StyleEngine::InvalidateStyle()
{
    StyleInvalidator style_invalidator(pending_invalidations_.GetPendingInvalidationMap());
    style_invalidator.Invalidate(GetDocument(), style_invalidation_root_.RootElement());
    style_invalidation_root_.Clear();
}

void StyleEngine::InvalidateSlottedElements(HTMLSlotElement& slot, const StyleChangeReasonForTracing& reason)
{
...
void StyleEngine::ApplyRuleSetInvalidationForTreeScope(TreeScope& tree_scope, ContainerNode& node, SelectorFilter& selector_filter,
    StyleScopeFrame& style_scope_frame, const HeapHashSet<Member<RuleSet>>& rule_sets, unsigned changed_rule_flags, InvalidationScope invalidation_scope)
{
    TRACE_EVENT0("blink,blink_style", "StyleEngine::scheduleInvalidationsForRuleSets");
    ...
    if (auto* shadow_root = DynamicTo<ShadowRoot>(&node)) {
        ...
        ApplyRuleSetInvalidationForElement(tree_scope, host, selector_filter, style_scope_frame, rule_sets, changed_rule_flags,
            /*is_shadow_host=*/true);
    ...
    if (invalidation_scope != kInvalidateAllScopes) {
        for (auto rule_set : rule_sets) {
            if (rule_set->HasUAShadowPseudoElementRules() || rule_set->HasPartPseudoRules()) {
    ...
    for (Element& child : ElementTraversal::ChildrenOf(node)) {
        ApplyRuleSetInvalidationForSubtree(tree_scope, child, selector_filter,
            /* parent_style_scope_frame */ style_scope_frame, rule_sets, changed_rule_flags, invalidation_scope, invalidate_slotted, invalidate_part);
    }
}

void StyleEngine::ApplyRuleSetInvalidationForSubtree(TreeScope& tree_scope, Element& element, SelectorFilter& selector_filter,
    StyleScopeFrame& parent_style_scope_frame, const HeapHashSet<Member<RuleSet>>& rule_sets, unsigned changed_rule_flags, InvalidationScope invalidation_scope,
    bool invalidate_slotted, bool invalidate_part)
{
    ...
    if (invalidate_part && element.hasAttribute(html_names::kPartAttr)) {
        ...
        ApplyRuleSetInvalidationForElement(tree_scope, element, selector_filter, style_scope_frame, rule_sets, changed_rule_flags,
            /*is_shadow_host=*/false);
    }

    auto* html_slot_element = DynamicTo<HTMLSlotElement>(element);
    if (html_slot_element && invalidate_slotted) {
        InvalidateSlottedElements(*html_slot_element, StyleChangeReasonForTracing::Create(style_change_reason::kStyleRuleChange));
    }

    if (invalidation_scope == kInvalidateAllScopes) {
        if (ShadowRoot* shadow_root = element.GetShadowRoot()) {
            selector_filter.PushParent(element);
            ApplyRuleSetInvalidationForTreeScope(tree_scope, shadow_root->RootNode(), selector_filter, style_scope_frame, rule_sets, kInvalidateAllScopes);
    ...
    // anyway.
    const bool traverse_children = (element.GetStyleChangeType() < kSubtreeStyleChange && element.GetComputedStyle());
...
void StyleEngine::EnsureUAStyleForFullscreen(const Element& element)
{
    ...
    CSSDefaultStyleSheets::Instance().EnsureDefaultStyleSheetForFullscreen(element);
    global_rule_set_->MarkDirty();
    UpdateActiveStyle();
}
...
void StyleEngine::EnsureUAStyleForPseudoElement(PseudoId pseudo_id)
{
    ...
    if (CSSDefaultStyleSheets::Instance().EnsureDefaultStyleSheetsForPseudoElement(pseudo_id)) {
        global_rule_set_->MarkDirty();
        UpdateActiveStyle();
    }
}
...
{
    DCHECK(global_rule_set_);
    if (CSSDefaultStyleSheets::Instance().EnsureDefaultStyleSheetForForcedColors()) {
        global_rule_set_->MarkDirty();
        if (GetDocument().IsActive()) {
            UpdateActiveStyle();
        }
...
RuleSet* StyleEngine::DefaultViewTransitionStyle() const
{
    ...
    return &css_style_sheet->Contents()->EnsureRuleSet(CSSDefaultStyleSheets::ScreenEval());
}
...
void StyleEngine::UpdateViewTransitionOptIn()
{
    ...
    if (view_transition_rule_) {
        ...
        if (const CSSValue* value = view_transition_rule_->GetNavigation()) {
            cross_document_enabled = To<CSSIdentifierValue>(value)->GetValueID() == CSSValueID::kAuto;
        }
    }

    ViewTransitionSupplement::From(GetDocument())->OnViewTransitionsStyleUpdated(cross_document_enabled, types);
}

bool StyleEngine::HasRulesForId(const AtomicString& id) const
{
    DCHECK(global_rule_set_);
    return global_rule_set_->GetRuleFeatureSet().GetRuleInvalidationData().HasSelectorForId(id);
}
...
void StyleEngine::InitialStyleChanged()
{
    ...
    MarkAllElementsForStyleRecalc(StyleChangeReasonForTracing::Create(style_change_reason::kSettings));
}
...
void StyleEngine::ViewportStyleSettingChanged()
{
    ...
    if (ScopedStyleResolver* resolver = GetDocument().GetScopedStyleResolver()) {
        ...
        MarkAllElementsForStyleRecalc(StyleChangeReasonForTracing::Create(style_change_reason::kActiveStylesheetsUpdate));
    }
}

void StyleEngine::InvalidateForRuleSetChanges(
    TreeScope& tree_scope, const HeapHashSet<Member<RuleSet>>& changed_rule_sets, unsigned changed_rule_flags, InvalidationScope invalidation_scope)
{
    ...
    Element& invalidation_root = ScopedStyleResolver::InvalidationRootForTreeScope(tree_scope);
    ...
    StyleScopeFrame style_scope_frame(IsA<ShadowRoot>(tree_scope) ? To<ShadowRoot>(tree_scope).host() : *tree_scope.GetDocument().documentElement());

    NthIndexCache nth_index_cache(tree_scope.GetDocument());
    ApplyRuleSetInvalidationForTreeScope(
        tree_scope, tree_scope.RootNode(), selector_filter, style_scope_frame, changed_rule_sets, changed_rule_flags, invalidation_scope);
}
...
public:
    explicit AtRuleCascadeMap(Document& document)
        : document_(document)
    {
    }
...
void StyleEngine::ApplyUserRuleSetChanges(const ActiveStyleSheetVector& old_style_sheets, const ActiveStyleSheetVector& new_style_sheets)
{
    DCHECK(global_rule_set_);
    HeapHashSet<Member<RuleSet>> changed_rule_sets;

    ActiveSheetsChange change = CompareActiveStyleSheets(old_style_sheets, new_style_sheets, /*diffs=*/ {}, changed_rule_sets);
    ...
    if (changed_rule_flags & kLayerRules) {
        ...
        user_cascade_layer_map_ = MakeGarbageCollected<CascadeLayerMap>(new_style_sheets);
    ...
    if (changed_rule_flags & kFontFaceRules) {
        if (ScopedStyleResolver* scoped_resolver = GetDocument().GetScopedStyleResolver()) {
            ...
            bool has_rebuilt_font_face_cache = ClearFontFaceCacheAndAddUserFonts(new_style_sheets);
            if (has_rebuilt_font_face_cache) {
                GetFontSelector()->FontFaceInvalidated(FontInvalidationReason::kGeneralInvalidation);
            }
        }
    }
    ...
    if (changed_rule_flags & (kPropertyRules | kFontPaletteValuesRules | kFontFeatureValuesRules)) {
        if (changed_rule_flags & kPropertyRules) {
            ClearPropertyRules();
            AtRuleCascadeMap cascade_map(GetDocument());
            AddPropertyRulesFromSheets(cascade_map, new_style_sheets, true /* is_user_style */);
        }
...
void StyleEngine::ApplyRuleSetChanges(TreeScope& tree_scope, const ActiveStyleSheetVector& old_style_sheets, const ActiveStyleSheetVector& new_style_sheets,
    const HeapVector<Member<RuleSetDiff>>& diffs)
{
    ...
    bool rebuild_font_face_cache = change == kActiveSheetsChanged && (changed_rule_flags & kFontFaceRules) && tree_scope.RootNode().IsDocumentNode();
    ...
    if (rebuild_cascade_layer_map) {
        tree_scope.EnsureScopedStyleResolver().RebuildCascadeLayerMap(new_style_sheets);
    }
    ...
    if ((changed_rule_flags & kPropertyRules) || rebuild_at_property_registry) {
        ...
        if (tree_scope.RootNode().IsDocumentNode()) {
            ClearPropertyRules();
            AtRuleCascadeMap cascade_map(GetDocument());
            AddPropertyRulesFromSheets(cascade_map, active_user_style_sheets_, true /* is_user_style */);
            AddPropertyRulesFromSheets(cascade_map, new_style_sheets, false /* is_user_style */);
        }
    }

    if ((changed_rule_flags & kFontPaletteValuesRules) || rebuild_at_font_palette_values_map) {
    ...
    if (tree_scope.RootNode().IsDocumentNode()) {
        ...
        if (rebuild_font_face_cache) {
            has_rebuilt_font_face_cache = ClearFontFaceCacheAndAddUserFonts(active_user_style_sheets_);
        }
        if ((changed_rule_flags & kFontFaceRules) || (changed_rule_flags & kFontPaletteValuesRules) || (changed_rule_flags & kFontFeatureValuesRules)
    ...
    if (!new_style_sheets.empty()) {
        tree_scope.EnsureScopedStyleResolver().AppendActiveStyleSheets(append_start_index, new_style_sheets);
    }

    InvalidateForRuleSetChanges(tree_scope, changed_rule_sets, changed_rule_flags, kInvalidateCurrentScope);
}
...
void StyleEngine::LoadVisionDeficiencyFilter()
{
    ...
    if (vision_deficiency_ == VisionDeficiency::kNoVisionDeficiency) {
        ...
        cssvalue::CSSURIValue* css_uri_value = MakeGarbageCollected<cssvalue::CSSURIValue>(CSSUrlData(url));
        ...
        vision_deficiency_filter_ = MakeGarbageCollected<ReferenceFilterOperation>(url, svg_resource);
    }
}
...
void StyleEngine::ApplyVisionDeficiencyStyle(ComputedStyleBuilder& layout_view_style_builder)
{
...
const MediaQueryEvaluator& StyleEngine::EnsureMediaQueryEvaluator()
{
    if (!media_query_evaluator_) {
        if (GetDocument().GetFrame()) {
            media_query_evaluator_ = MakeGarbageCollected<MediaQueryEvaluator>(GetDocument().GetFrame());
        } else {
...
bool StyleEngine::StyleMaybeAffectedByLayout(const Element& element)
{
    ...
    return StyleAffectedByLayout() || ComputedStyle::IsNullOrEnsured(element.GetComputedStyle());
}

bool StyleEngine::UpdateRootFontRelativeUnits(const ComputedStyle* old_root_style, const ComputedStyle* new_root_style)
{
    if (!new_root_style || !UsesRootFontRelativeUnits()) {
        return false;
    }
    bool rem_changed = !old_root_style || old_root_style->SpecifiedFontSize() != new_root_style->SpecifiedFontSize();
    bool root_font_glyphs_changed = !old_root_style || (UsesGlyphRelativeUnits() && old_root_style->GetFont() != new_root_style->GetFont());
    bool root_line_height_changed = !old_root_style || (UsesLineHeightUnits() && old_root_style->LineHeight() != new_root_style->LineHeight());
    bool root_font_changed = rem_changed || root_font_glyphs_changed || root_line_height_changed;
...
void StyleEngine::PropertyRegistryChanged()
{
    ...
    MarkAllElementsForStyleRecalc(StyleChangeReasonForTracing::Create(style_change_reason::kPropertyRegistration));
    if (resolver_) {
        resolver_->InvalidateMatchedPropertiesCache();
    }
    InvalidateInitialData();
...
void StyleEngine::InvalidateEnvDependentStylesIfNeeded()
{
    ...
    is_env_dirty_ = false;
    const auto& reason = StyleChangeReasonForTracing::Create(style_change_reason::kEnvironmentVariableChanged);
    MarkElementsForRecalc(GetDocument(), reason, [](const ComputedStyle& style) { return style.HasEnv(); });
}

void StyleEngine::NodeWillBeRemoved(Node& node)
{
    if (auto* element = DynamicTo<Element>(node)) {
        if (const ComputedStyle* style = element->GetComputedStyle(); style && style->GetCounterDirectives()) {
            MarkCountersDirty();
        }
        if (element->GetComputedStyle() && element->ComputedStyleRef().ContainsStyle()) {
            MarkCountersDirty();
        }
        ...
        pending_invalidations_.RescheduleSiblingInvalidationsAsDescendants(*element);
    }
}
...
void StyleEngine::CollectMatchingUserRules(ElementRuleCollector& collector) const
{
...
void StyleEngine::AddPropertyRulesFromSheets(AtRuleCascadeMap& cascade_map, const ActiveStyleSheetVector& sheets, bool is_user_style)
{
...
void StyleEngine::AddFontPaletteValuesRulesFromSheets(const ActiveStyleSheetVector& sheets)
{
    for (const ActiveStyleSheet& active_sheet : sheets) {
        if (RuleSet* rule_set = active_sheet.second) {
...
bool StyleEngine::AddUserFontFaceRules(const RuleSet& rule_set)
{
    ...
    const HeapVector<Member<StyleRuleFontFace>> font_face_rules = rule_set.FontFaceRules();
    for (auto& font_face_rule : font_face_rules) {
        if (FontFace* font_face = FontFace::Create(document_, font_face_rule, true /* is_user_style */)) {
...
{
    const HeapVector<Member<StyleRuleKeyframes>> keyframes_rules = rule_set.KeyframesRules();
...
void StyleEngine::AddUserKeyframeStyle(StyleRuleKeyframes* rule)
{
    ...
    if (it == keyframes_rule_map_.end() || UserKeyframeStyleShouldOverride(rule, it->value)) {
        keyframes_rule_map_.Set(animation_name, rule);
    }
}

bool StyleEngine::UserKeyframeStyleShouldOverride(const StyleRuleKeyframes* new_rule, const StyleRuleKeyframes* existing_rule) const
{
    if (new_rule->IsVendorPrefixed() != existing_rule->IsVendorPrefixed()) {
        return existing_rule->IsVendorPrefixed();
    }
    return !user_cascade_layer_map_ || user_cascade_layer_map_->CompareLayerOrder(existing_rule->GetCascadeLayer(), new_rule->GetCascadeLayer()) <= 0;
}
...
void StyleEngine::AddViewTransitionRules(const ActiveStyleSheetVector& sheets)
{
    ...
    for (const ActiveStyleSheet& active_sheet : sheets) {
        ...
        const CascadeLayerMap* layer_map = document_->GetScopedStyleResolver() ? document_->GetScopedStyleResolver()->GetCascadeLayerMap() : nullptr;
        for (auto& rule : rule_set->ViewTransitionRules()) {
            if (!view_transition_rule_ || !layer_map || layer_map->CompareLayerOrder(view_transition_rule_->GetCascadeLayer(), rule->GetCascadeLayer()) <= 0) {
...
{
    const HeapVector<Member<StyleRuleFontPaletteValues>> font_palette_values_rules = rule_set.FontPaletteValuesRules();
    for (auto& rule : font_palette_values_rules) {
        ...
        for (auto& family : ConvertFontFamilyToVector(rule->GetFontFamily())) {
            font_palette_values_rule_map_.Set(std::make_pair(rule->GetName(), String(family).FoldCase()), rule);
        }
    }
}

void StyleEngine::AddPropertyRules(AtRuleCascadeMap& cascade_map, const RuleSet& rule_set, bool is_user_style)
{
    const HeapVector<Member<StyleRuleProperty>> property_rules = rule_set.PropertyRules();
    for (unsigned i = 0; i < property_rules.size(); ++i) {
        StyleRuleProperty* rule = property_rules[i];
        AtomicString name(rule->GetName());

        PropertyRegistration* registration = PropertyRegistration::MaybeCreateForDeclaredProperty(GetDocument(), name, *rule);
        if (!registration) {
            continue;
        }

        auto priority = cascade_map.GetPriority(is_user_style, rule->GetCascadeLayer());
...
StyleRuleKeyframes* StyleEngine::KeyframeStylesForAnimation(const AtomicString& animation_name)
{
...
StyleRuleFontPaletteValues* StyleEngine::FontPaletteValuesForNameAndFamily(AtomicString palette_name, AtomicString family_name)
{
    if (font_palette_values_rule_map_.empty() || palette_name.empty()) {
        return nullptr;
    }

    auto it = font_palette_values_rule_map_.find(std::make_pair(palette_name, String(family_name).FoldCase()));
...
{
    if (!environment_variables_) {
        environment_variables_ = MakeGarbageCollected<DocumentStyleEnvironmentVariables>(StyleEnvironmentVariables::GetRootInstance(), *document_);
    }
...
StyleInitialData* StyleEngine::MaybeCreateAndGetInitialData()
{
    if (!initial_data_) {
        if (const PropertyRegistry* registry = document_->GetPropertyRegistry()) {
            if (!registry->IsEmpty()) {
                initial_data_ = MakeGarbageCollected<StyleInitialData>(GetDocument(), *registry);
            }
        }
    }
...
bool StyleEngine::RecalcHighlightStylesForContainer(Element& container)
{
    ...
    bool depends_on_container_queries = style.HighlightData().DependsOnSizeContainerQueries() || style.HighlightsDependOnSizeContainerQueries();
    if (!style.HasAnyHighlightPseudoElementStyles() || !style.HasNonUaHighlightPseudoStyles() || !depends_on_container_queries) {
        return false;
    }
    ...
    if (const ComputedStyle* new_style = container.RecalcHighlightStyles(recalc_context, nullptr /* old_style */, style, container.ParentComputedStyle());
        new_style != &style) {
        container.SetComputedStyle(new_style);
        container.GetLayoutObject()->SetStyle(new_style, LayoutObject::ApplyStyleChanges::kNo);
    }
...
#if DCHECK_IS_ON()
namespace {
bool ContainerStyleChangesAllowed(Element& container, const ComputedStyle* old_element_style, const ComputedStyle* old_layout_style)
{
    ...
    const ComputedStyle* new_layout_style = container.GetLayoutObject() ? container.GetLayoutObject()->Style() : nullptr;
    ...
    if (new_element_style != old_element_style) {
        Vector<ComputedStyleBase::DebugDiff> diff = old_element_style->DebugDiffFields(*new_element_style);
        // Allow highlight styles to change, but only highlight styles.
        if (diff.size() > 1 || (diff.size() == 1 && diff[0].field != ComputedStyleBase::DebugField::highlight_data_)) {
            return false;
        }
    }
    ...
    Vector<ComputedStyleBase::DebugDiff> diff = old_layout_style->DebugDiffFields(*new_layout_style);
    // Allow highlight styles to change, but only highlight styles.
    return diff.size() == 0 || (diff.size() == 1 && diff[0].field == ComputedStyleBase::DebugField::highlight_data_);
}
} // namespace
#endif // DCHECK_IS_ON()

void StyleEngine::RecalcStyleForContainer(Element& container, StyleRecalcChange change)
{
    ...
    const ComputedStyle* old_layout_style = container.GetLayoutObject() ? container.GetLayoutObject()->Style() : nullptr;
#endif // DCHECK_IS_ON()
...
#if DCHECK_IS_ON()
    DCHECK(ContainerStyleChangesAllowed(container, old_element_style, old_layout_style));
#endif // DCHECK_IS_ON()
}
...
void StyleEngine::UpdateStyleForNonEligibleContainer(Element& container)
{
    ...
    ContainerQueryEvaluator& evaluator = container.EnsureContainerQueryEvaluator();
    ContainerQueryEvaluator::Change query_change = evaluator.SizeContainerChanged(PhysicalSize(), kPhysicalAxesNone);
    switch (query_change) {
    case ContainerQueryEvaluator::Change::kNone:
        DCHECK(cq_data->SkippedStyleRecalc());
        break;
    case ContainerQueryEvaluator::Change::kNearestContainer:
        if (RuntimeEnabledFeatures::CSSFlatTreeContainerEnabled() || !IsShadowHost(container)) {
...
void StyleEngine::UpdateStyleAndLayoutTreeForContainer(Element& container, const LogicalSize& logical_size, LogicalAxes contained_axes)
{
    ...
    PhysicalSize physical_size = AdjustForAbsoluteZoom::AdjustPhysicalSize(ToPhysicalSize(logical_size, writing_mode), style);
    PhysicalAxes physical_axes = ToPhysicalAxes(contained_axes, writing_mode);

    StyleRecalcChange change;

    ContainerQueryEvaluator::Change query_change = container.EnsureContainerQueryEvaluator().SizeContainerChanged(physical_size, physical_axes);
    ...
    if (query_change != ContainerQueryEvaluator::Change::kNone) {
        ...
        if (style.HasPseudoElementStyle(kPseudoIdFirstLine) && style.FirstLineDependsOnSizeContainerQueries()) {
    ...
    if (container.NeedsReattachLayoutTree()) {
        ...
        if (layout_tree_rebuild_root_.GetRootNode()->IsDocumentNode()) {
            ...
            DCHECK(FlatTreeTraversal::ContainsIncludingPseudoElement(container, *layout_tree_rebuild_root_.GetRootNode()));
        }
    ...
    GetDocument().GetLayoutView()->UpdateCountersAfterStyleChange(container.GetLayoutObject());
    GetDocument().InvalidatePendingSVGResources();
}

void StyleEngine::UpdateStyleForOutOfFlow(
    Element& element, const CSSPropertyValueSet* try_set, const TryTacticList& tactic_list, AnchorEvaluator* anchor_evaluator)
{
    const CSSPropertyValueSet* try_tactics_set = try_value_flips_.FlipSet(tactic_list);
    ...
    if (auto* pseudo_element = DynamicTo<PseudoElement>(element)) {
        RecalcPositionTryStyleForPseudoElement(*pseudo_element, change, style_recalc_context);
    } else {
...
StyleRulePositionTry* StyleEngine::GetPositionTryRule(const ScopedCSSName& scoped_name)
{
    const TreeScope* tree_scope = scoped_name.GetTreeScope();
    if (!tree_scope) {
        tree_scope = &GetDocument();
    }
    return GetStyleResolver().ResolvePositionTryRule(tree_scope, scoped_name.GetName());
}
...
void StyleEngine::RecalcStyle(StyleRecalcChange change, const StyleRecalcContext& style_recalc_context)
{
    ...
    CheckPseudoHasCacheScope check_pseudo_has_cache_scope(&GetDocument(), /*within_selector_checking=*/false);
...
void StyleEngine::RecalcPositionTryStyleForPseudoElement(
    PseudoElement& pseudo_element, const StyleRecalcChange style_recalc_change, const StyleRecalcContext& style_recalc_context)
{
    ScriptForbiddenScope forbid_script;
    SkipStyleRecalcScope skip_scope(*this);
    CheckPseudoHasCacheScope check_pseudo_has_cache_scope(&GetDocument(), /*within-selector_checking=*/false);
    SelectorFilterRootScope filter_scope(FlatTreeTraversal::ParentElement(*pseudo_element.UltimateOriginatingElement()));
...
void StyleEngine::RecalcTransitionPseudoStyle()
{
    ...
    document_->documentElement()->RecalcTransitionPseudoTreeStyle(view_transition_names_);
}

void StyleEngine::RecalcStyle()
{
    RecalcStyle({}, StyleRecalcContext::FromAncestors(style_recalc_root_.RootElement()));
...
void StyleEngine::RebuildLayoutTreeForTraversalRootAncestors(Element* parent, Element* container_parent)
{
    bool is_container_ancestor = false;

    for (auto* ancestor = parent; ancestor; ancestor = ancestor->GetReattachParent()) {
...
void StyleEngine::RebuildLayoutTree(Element* size_container)
{
    bool propagate_to_root = false;
    {
        ...
        Element* container_parent = size_container ? size_container->GetReattachParent() : nullptr;
        RebuildLayoutTreeForTraversalRootAncestors(root_element.GetReattachParent(), container_parent);
        if (size_container == nullptr) {
            document_->documentElement()->RebuildTransitionPseudoLayoutTree(view_transition_names_);
        }
        layout_tree_rebuild_root_.Clear();
        propagate_to_root = IsA<HTMLHtmlElement>(root_element) || IsA<HTMLBodyElement>(root_element);
    }
...
void StyleEngine::ReattachContainerSubtree(Element& container)
{
    ...
    RebuildLayoutTreeForTraversalRootAncestors(&container, container.GetReattachParent());
    layout_tree_rebuild_root_.Clear();
}
...
void StyleEngine::ViewportDefiningElementDidChange()
{
    ...
    if (layout_object && layout_object->IsLayoutBlock()) {
        ...
        layout_object->SetStyle(ComputedStyleBuilder(*layout_object->Style()).TakeStyle());
    }
}
...
void StyleEngine::FirstBodyElementChanged(HTMLBodyElement* body)
{
    ...
    dirty_element->SetNeedsStyleRecalc(kLocalStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kViewportDefiningElement));
}
...
void StyleEngine::UpdateLayoutTreeRebuildRoot(ContainerNode* ancestor, Node* dirty_node)
{
    DCHECK(!InDOMRemoval());
    if (!GetDocument().IsActive()) {
        return;
...
{
    return IsA<ShadowRoot>(node) ? node.ParentOrShadowHostElement() : LayoutTreeBuilderTraversal::Parent(node);
}
...
StyleEngine::AncestorAnalysis StyleEngine::AnalyzeInclusiveAncestor(const Node& node)
{
...
StyleEngine::AncestorAnalysis StyleEngine::AnalyzeExclusiveAncestor(const Node& node)
{
...
StyleEngine::AncestorAnalysis StyleEngine::AnalyzeAncestors(const Node& node)
{
    ...
    for (const Node* ancestor = LayoutTreeBuilderTraversal::Parent(node); ancestor; ancestor = LayoutTreeBuilderTraversal::Parent(*ancestor)) {
...
{
    return (page_color_schemes_ & static_cast<ColorSchemeFlags>(ColorSchemeFlag::kDark))
        && (!(page_color_schemes_ & static_cast<ColorSchemeFlags>(ColorSchemeFlag::kLight))
...
void StyleEngine::UpdateColorScheme()
{
    ...
    forced_colors_ = settings->GetInForcedColors() ? ForcedColors::kActive : ForcedColors::kNone;

    mojom::blink::PreferredColorScheme old_preferred_color_scheme = preferred_color_scheme_;
    ...
    // overrides?
    if (const MediaFeatureOverrides* overrides = GetDocument().GetPage()->GetMediaFeatureOverrides()) {
        if (std::optional<ForcedColors> forced_color_override = overrides->GetForcedColors()) {
            forced_colors_ = forced_color_override.value();
        }
        if (std::optional<mojom::blink::PreferredColorScheme> preferred_color_scheme_override = overrides->GetPreferredColorScheme()) {
            preferred_color_scheme_ = preferred_color_scheme_override.value();
            media_feature_override_color_scheme = true;
        }
    }

    const PreferenceOverrides* preference_overrides = GetDocument().GetPage()->GetPreferenceOverrides();
    if (preference_overrides && !media_feature_override_color_scheme) {
        std::optional<mojom::blink::PreferredColorScheme> preferred_color_scheme_override = preference_overrides->GetPreferredColorScheme();
...
void StyleEngine::UpdateColorSchemeMetrics()
{
    ...
    if (settings->GetPreferredColorScheme() == mojom::blink::PreferredColorScheme::kDark) {
        UseCounter::Count(GetDocument(), WebFeature::kPreferredColorSchemeDarkSetting);
    }
    ...
    if (page_color_schemes_ & static_cast<ColorSchemeFlags>(ColorSchemeFlag::kDark)) {
        UseCounter::Count(GetDocument(), WebFeature::kColorSchemeDarkSupportedOnRoot);
    }
}
...
void StyleEngine::SetPageColorSchemes(const CSSValue* color_scheme)
{
    ...
    if (auto* value_list = DynamicTo<CSSValueList>(color_scheme)) {
        page_color_schemes_ = StyleBuilderConverter::ExtractColorSchemes(GetDocument(), *value_list, nullptr /* color_schemes */);
    } else {
        page_color_schemes_ = static_cast<ColorSchemeFlags>(ColorSchemeFlag::kNormal);
    }
    DCHECK(GetDocument().documentElement());
    ...
    GetDocument().documentElement()->SetNeedsStyleRecalc(kSubtreeStyleChange, StyleChangeReasonForTracing::Create(style_change_reason::kPlatformColorChange));
    UpdateColorScheme();
    UpdateColorSchemeBackground();
}
...
void StyleEngine::UpdateColorSchemeBackground(bool color_scheme_changed)
{
    ...
    LocalFrameView::UseColorAdjustBackground use_color_adjust_background = LocalFrameView::UseColorAdjustBackground::kNo;

    if (forced_colors_ != ForcedColors::kNone) {
        if (GetDocument().IsInMainFrame()) {
            use_color_adjust_background = LocalFrameView::UseColorAdjustBackground::kIfBaseNotTransparent;
        }
    } else {
        ...
        mojom::blink::ColorScheme root_color_scheme = mojom::blink::ColorScheme::kLight;
        ...
        color_scheme_background_ = root_color_scheme == mojom::blink::ColorScheme::kLight ? Color::kWhite : Color(0x12, 0x12, 0x12);
        if (GetDocument().IsInMainFrame()) {
            ...
            use_color_adjust_background = LocalFrameView::UseColorAdjustBackground::kYes;
        }
    }

    view->SetUseColorAdjustBackground(use_color_adjust_background, color_scheme_changed);
}

void StyleEngine::SetOwnerColorScheme(mojom::blink::ColorScheme color_scheme, mojom::blink::PreferredColorScheme preferred_color_scheme)
{
...
mojom::blink::PreferredColorScheme StyleEngine::ResolveColorSchemeForEmbedding(const ComputedStyle* embedder_style) const
{
    ...
    return embedder_style && embedder_style->UsedColorScheme() == mojom::blink::ColorScheme::kDark ? mojom::blink::PreferredColorScheme::kDark
...
void StyleEngine::UpdateForcedBackgroundColor()
{
    ...
    forced_background_color_ = LayoutTheme::GetTheme().SystemColor(CSSValueID::kCanvas, color_scheme,
        GetDocument().GetPage()->GetColorProviderForPainting(color_scheme, forced_colors_ != ForcedColors::kNone), GetDocument().IsInWebAppScope());
}
...
void StyleEngine::MarkAllElementsForStyleRecalc(const StyleChangeReasonForTracing& reason)
{
    if (Element* root = GetDocument().documentElement()) {
...
void StyleEngine::UpdateViewportStyle()
{
    ...
    if (ComputedStyle::ComputeDifference(viewport_style, GetDocument().GetLayoutView()->Style()) != ComputedStyle::Difference::kEqual) {
...
{
    return NeedsActiveStyleUpdate() || IsViewportStyleDirty() || viewport_unit_dirty_flags_ || is_env_dirty_;
}
...
CounterStyleMap& StyleEngine::EnsureUserCounterStyleMap()
{
    if (!user_counter_style_map_) {
        user_counter_style_map_ = CounterStyleMap::CreateUserCounterStyleMap(GetDocument());
    }
    return *user_counter_style_map_;
}

const CounterStyle& StyleEngine::FindCounterStyleAcrossScopes(const AtomicString& name, const TreeScope* scope) const
{
    CounterStyleMap* target_map = nullptr;
    while (scope) {
        if (CounterStyleMap* map = CounterStyleMap::GetAuthorCounterStyleMap(*scope)) {
            target_map = map;
            break;
        }
...
{
    return element.firstChild() || IsShadowHost(element) || element.IsActiveSlot();
}
...
void StyleEngine::MarkForLayoutTreeChangesAfterDetach()
{
    ...
    if (auto* layout_object_element = DynamicTo<Element>(layout_object->GetNode())) {
...
void StyleEngine::AddCachedFillOrClipPathURIValue(const AtomicString& string, const CSSValue& value)
{
    fill_or_clip_path_uri_value_cache_.insert(string, &value);
}

const CSSValue* StyleEngine::GetCachedFillOrClipPathURIValue(const AtomicString& string)
{
...
{
    viewport_size_ = CSSToLengthConversionData::ViewportSize(GetDocument().GetLayoutView());
}
...
bool UpdateLastSuccessfulPositionFallback(Element& element)
{
    if (OutOfFlowData* out_of_flow_data = element.GetOutOfFlowData()) {
        LayoutObject* layout_object = element.GetLayoutObject();
        if (out_of_flow_data->ApplyPendingSuccessfulPositionFallback(layout_object) && layout_object) {
            layout_object->SetNeedsLayoutAndFullPaintInvalidation(layout_invalidation_reason::kAnchorPositioning);
            return true;
        }
    }
    return false;
...
bool InvalidatePositionTryNames(Element* root, const HashSet<AtomicString>& try_names)
{
    ...
    while (current) {
        if (auto* element = DynamicTo<Element>(current)) {
            ...
            if (ComputedStyle::NullifyEnsured(element->GetComputedStyle()) == nullptr) {
                current = LayoutTreeBuilderTraversal::NextSkippingChildren(*element, root);
                continue;
            }
        }
...
bool StyleEngine::UpdateLastSuccessfulPositionFallbacks()
{
    ...
    if (!dirty_position_try_names_.empty()) {
        ...
        if (InvalidatePositionTryNames(GetDocument().documentElement(), dirty_position_try_names_)) {
            invalidated = true;
        }
...
void StyleEngine::RevisitActiveStyleSheetsForInspector()
{
    ...
    const ActiveStyleSheetVector& active_style_sheets = GetDocumentStyleSheetCollection().ActiveStyleSheets();
    for (const ActiveStyleSheet& sheet : active_style_sheets) {
        ...
        if (contents->HasRuleSet()) {
            RevisitStyleRulesForInspector(contents->GetRuleSet().Features(), contents->ChildRules());
        }
    }
}

void StyleEngine::RevisitStyleRulesForInspector(const RuleFeatureSet& features, const HeapVector<Member<StyleRuleBase>>& rules)
{
    for (StyleRuleBase* rule : rules) {
        if (StyleRule* style_rule = DynamicTo<StyleRule>(rule)) {
            for (const CSSSelector* selector = style_rule->FirstSelector(); selector; selector = CSSSelectorList::Next(*selector)) {
                InvalidationSetToSelectorMap::SelectorScope selector_scope(style_rule, style_rule->SelectorIndex(*selector));
                features.RevisitSelectorForInspector(*selector);
            }
        } else if (StyleRuleGroup* style_rule_group = DynamicTo<StyleRuleGroup>(rule)) {
