...
public:
    using ResultCallback = base::OnceCallback<void(mojom::blink::PerIsolateV8MemoryUsagePtr)>;
...
private:
    ...
    {
        DCHECK(IsMainThread());
        mojom::blink::PerIsolateV8MemoryUsagePtr isolate_memory_usage = mojom::blink::PerIsolateV8MemoryUsage::New();
        ...
        for (size_t i = 0; i < result.contexts.size(); ++i) {
            ...
            if (DOMWrapperWorld::World(isolate, context).GetWorldId() != DOMWrapperWorld::kMainWorldId) {
            ...
            context_memory_usage->token = frame->DomWindow()->GetExecutionContextToken();
...
v8::MeasureMemoryExecution ToV8MeasureMemoryExecution(V8DetailedMemoryReporterImpl::Mode mode)
{
    switch (mode) {
...
public:
    using GetV8MemoryUsageCallback = mojom::blink::V8DetailedMemoryReporter::GetV8MemoryUsageCallback;
    ...
    void StartMeasurements(V8DetailedMemoryReporterImpl::Mode mode)
    {
        ...
        if (!isolate_) {
            ...
            auto delegate = std::make_unique<FrameAssociatedMeasurementDelegate>(
                WTF::BindOnce(&V8ProcessMemoryReporter::MainV8MeasurementComplete, scoped_refptr<V8ProcessMemoryReporter>(this)));

            isolate_->MeasureMemory(std::move(delegate), ToV8MeasureMemoryExecution(mode));
        }
        // 2. Start measurement of all worker isolates.
        V8WorkerMemoryReporter::GetMemoryUsage(
            WTF::BindOnce(&V8ProcessMemoryReporter::WorkerMeasurementComplete, scoped_refptr<V8ProcessMemoryReporter>(this)), ToV8MeasureMemoryExecution(mode));
    }

private:
    void MainV8MeasurementComplete(mojom::blink::PerIsolateV8MemoryUsagePtr isolate_memory_usage)
    {
        ...
        ThreadState::Current()->CollectNodeAndCssStatistics(WTF::BindOnce(
            &V8ProcessMemoryReporter::MainBlinkMeasurementComplete, scoped_refptr<V8ProcessMemoryReporter>(this), std::move(isolate_memory_usage)));
    }

    void MainBlinkMeasurementComplete(mojom::blink::PerIsolateV8MemoryUsagePtr isolate_memory_usage, size_t node_bytes, size_t css_bytes)
    {
        isolate_memory_usage->blink_bytes_used = node_bytes + css_bytes;
        MeasureCanvasMemory(std::move(isolate_memory_usage));
    }

    void MeasureCanvasMemory(mojom::blink::PerIsolateV8MemoryUsagePtr isolate_memory_usage)
    {
        // We do not use HashMap here because there is no designated deleted value
        // of ExecutionContextToken.
        std::unordered_map<ExecutionContextToken, uint64_t, ExecutionContextToken::Hasher> per_context_bytes;
    ...
    void MainMeasurementComplete(mojom::blink::PerIsolateV8MemoryUsagePtr isolate_memory_usage)
    {
...
// static
void V8DetailedMemoryReporterImpl::Bind(mojo::PendingReceiver<mojom::blink::V8DetailedMemoryReporter> receiver)
{
...
void V8DetailedMemoryReporterImpl::GetV8MemoryUsage(V8DetailedMemoryReporterImpl::Mode mode, GetV8MemoryUsageCallback callback)
{
    auto v8_process_memory_reporter = base::MakeRefCounted<V8ProcessMemoryReporter>(std::move(callback));
