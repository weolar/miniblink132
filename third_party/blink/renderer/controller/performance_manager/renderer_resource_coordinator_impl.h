...
class CONTROLLER_EXPORT RendererResourceCoordinatorImpl final : public RendererResourceCoordinator {
    USING_FAST_MALLOC(RendererResourceCoordinatorImpl);

public:
    RendererResourceCoordinatorImpl(const RendererResourceCoordinatorImpl&) = delete;
    RendererResourceCoordinatorImpl& operator=(const RendererResourceCoordinatorImpl&) = delete;
    ...
    void OnScriptStateCreated(ScriptState* script_state, ExecutionContext* execution_context) final;
    void OnScriptStateDetached(ScriptState* script_state) final;
    void OnScriptStateDestroyed(ScriptState* script_state) final;
    void OnBeforeContentFrameAttached(const Frame& frame, const HTMLFrameOwnerElement& owner) final;
    void OnBeforeContentFrameDetached(const Frame& frame, const HTMLFrameOwnerElement& owner) final;

private:
    friend class RendererResourceCoordinatorImplTest;

    explicit RendererResourceCoordinatorImpl(mojo::PendingRemote<performance_manager::mojom::blink::ProcessCoordinationUnit> remote);

    // Used for dispatching script state events which can arrive on any thread
    // but need to be sent outbound from the main thread.
    void DispatchOnV8ContextCreated(performance_manager::mojom::blink::V8ContextDescriptionPtr v8_desc,
    ...
    void OnSharedPerformanceScenarioRegions(base::ReadOnlySharedMemoryRegion global_region, base::ReadOnlySharedMemoryRegion process_region);

    mojo::Remote<performance_manager::mojom::blink::ProcessCoordinationUnit> service_;
    ...
    std::optional<blink::performance_scenarios::ScopedReadOnlyScenarioMemory> process_performance_scenario_memory_;
    std::optional<blink::performance_scenarios::ScopedReadOnlyScenarioMemory> global_performance_scenario_memory_;
};
