...
class V8WorkerMemoryReporterTestWithDedicatedWorker : public DedicatedWorkerTest {
public:
    V8WorkerMemoryReporterTestWithDedicatedWorker()
        : DedicatedWorkerTest(base::test::TaskEnvironment::TimeSource::MOCK_TIME)
    {
    }
};

class V8WorkerMemoryReporterTestWithMockPlatform : public V8WorkerMemoryReporterTestWithDedicatedWorker {
public:
    void SetUp() override
    {
        EnablePlatform();
...
bool operator==(const V8WorkerMemoryReporter::WorkerMemoryUsage& lhs, const V8WorkerMemoryReporter::WorkerMemoryUsage& rhs)
{
    return lhs.token == rhs.token && lhs.bytes == rhs.bytes;
}

bool operator==(const V8WorkerMemoryReporter::Result& lhs, const V8WorkerMemoryReporter::Result& rhs)
{
...
public:
    ...
    MemoryUsageChecker(size_t worker_count, size_t bytes_per_worker_lower_bound, CallbackAction callback_action)
...
TEST_F(V8WorkerMemoryReporterTest, OnMeasurementSuccess)
{
    ...
    V8WorkerMemoryReporter reporter(WTF::BindOnce(&MockCallback::Callback, WTF::Unretained(&mock_callback)));
    reporter.SetWorkerCount(6);
    Result result = { Vector<WorkerMemoryUsage>(
        { WorkerMemoryUsage { WorkerToken(DedicatedWorkerToken()), 1 }, WorkerMemoryUsage { WorkerToken(DedicatedWorkerToken()), 2 },
            WorkerMemoryUsage { WorkerToken(SharedWorkerToken()), 3 }, WorkerMemoryUsage { WorkerToken(SharedWorkerToken()), 4 },
            WorkerMemoryUsage { WorkerToken(ServiceWorkerToken()), 4 }, WorkerMemoryUsage { WorkerToken(ServiceWorkerToken()), 5 } }) };
...
TEST_F(V8WorkerMemoryReporterTest, OnMeasurementFailure)
{
    ...
    EXPECT_CALL(mock_callback, Callback(result)).Times(1);
    reporter.OnMeasurementSuccess(std::make_unique<WorkerMemoryUsage>(result.workers[0]));
    reporter.OnMeasurementFailure();
    reporter.OnMeasurementSuccess(std::make_unique<WorkerMemoryUsage>(result.workers[1]));
}
...
TEST_F(V8WorkerMemoryReporterTest, OnTimeout)
{
    ...
    reporter.OnMeasurementSuccess(std::make_unique<WorkerMemoryUsage>(result.workers[1]));
    reporter.OnTimeout();
    reporter.OnMeasurementSuccess(std::make_unique<WorkerMemoryUsage>(WorkerMemoryUsage { WorkerToken(SharedWorkerToken()), 2 }));
    reporter.OnMeasurementFailure();
}
...
TEST_F(V8WorkerMemoryReporterTestWithDedicatedWorker, GetMemoryUsage)
{
    ...
    MemoryUsageChecker checker(1, kBytesPerArrayElement * kArrayLength, MemoryUsageChecker::CallbackAction::kExitRunLoop);
    V8WorkerMemoryReporter::GetMemoryUsage(WTF::BindOnce(&MemoryUsageChecker::Callback, WTF::Unretained(&checker)), v8::MeasureMemoryExecution::kEager);
    checker.Run();
...
TEST_F(V8WorkerMemoryReporterTestWithMockPlatform, GetMemoryUsageTimeout)
{
    ...
    V8WorkerMemoryReporter::GetMemoryUsage(WTF::BindOnce(&MemoryUsageChecker::Callback, WTF::Unretained(&checker)), v8::MeasureMemoryExecution::kEager);
    FastForwardBy(base::Seconds(V8WorkerMemoryReporter::kTimeout.InSeconds() + 1));
    EXPECT_TRUE(checker.IsCalled());
}
