...
struct CrossThreadCopier<blink::V8WorkerMemoryReporter::WorkerMemoryUsage>
    : public CrossThreadCopierPassThrough<blink::V8WorkerMemoryReporter::WorkerMemoryUsage> {
...
public:
    WorkerMeasurementDelegate(
        base::WeakPtr<V8WorkerMemoryReporter> worker_memory_reporter, WorkerThread* worker_thread, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
...
private:
    void NotifyMeasurementSuccess(std::unique_ptr<V8WorkerMemoryReporter::WorkerMemoryUsage> memory_usage);
...
void WorkerMeasurementDelegate::MeasurementComplete(v8::MeasureMemoryDelegate::Result result)
{
    DCHECK(worker_thread_->IsCurrentThread());
    ...
    auto memory_usage = std::make_unique<V8WorkerMemoryReporter::WorkerMemoryUsage>();
    memory_usage->token = worker_global_scope->GetWorkerToken();
    memory_usage->bytes = bytes;
    if (worker_global_scope->IsUrlValid() && worker_global_scope->Url().GetString().length() < kMaxReportedUrlLength) {
...
void WorkerMeasurementDelegate::NotifyMeasurementFailure()
{
    DCHECK(worker_thread_->IsCurrentThread());
    DCHECK(!did_notify_);
    V8WorkerMemoryReporter::NotifyMeasurementFailure(worker_thread_, task_runner_, worker_memory_reporter_);
    did_notify_ = true;
}

void WorkerMeasurementDelegate::NotifyMeasurementSuccess(std::unique_ptr<V8WorkerMemoryReporter::WorkerMemoryUsage> memory_usage)
{
    DCHECK(worker_thread_->IsCurrentThread());
    DCHECK(!did_notify_);
    V8WorkerMemoryReporter::NotifyMeasurementSuccess(worker_thread_, task_runner_, worker_memory_reporter_, std::move(memory_usage));
    did_notify_ = true;
}
...
// static
void V8WorkerMemoryReporter::GetMemoryUsage(ResultCallback callback, v8::MeasureMemoryExecution mode)
{
    DCHECK(IsMainThread());
    // The private constructor prevents us from using std::make_unique here.
    std::unique_ptr<V8WorkerMemoryReporter> worker_memory_reporter(new V8WorkerMemoryReporter(std::move(callback)));
    auto main_thread_task_runner = Thread::MainThread()->GetTaskRunner(MainThreadTaskRunnerRestricted());
    ...
    unsigned worker_count = WorkerThread::CallOnAllWorkerThreads(
        &V8WorkerMemoryReporter::StartMeasurement, TaskType::kInternalDefault, main_thread_task_runner, worker_memory_reporter->GetWeakPtr(), mode);
    if (worker_count == 0) {
        main_thread_task_runner->PostTask(FROM_HERE, WTF::BindOnce(&V8WorkerMemoryReporter::InvokeCallback, std::move(worker_memory_reporter)));
        return;
    }
    worker_memory_reporter->SetWorkerCount(worker_count);
    // Transfer the ownership of the instance to the timeout task.
    main_thread_task_runner->PostDelayedTask(FROM_HERE, WTF::BindOnce(&V8WorkerMemoryReporter::OnTimeout, std::move(worker_memory_reporter)), kTimeout);
}

// static
void V8WorkerMemoryReporter::StartMeasurement(WorkerThread* worker_thread, scoped_refptr<base::SingleThreadTaskRunner> task_runner,
    base::WeakPtr<V8WorkerMemoryReporter> worker_memory_reporter, v8::MeasureMemoryExecution measurement_mode)
{
    ...
    if (global_scope->IsWorkerGlobalScope()) {
        auto delegate = std::make_unique<WorkerMeasurementDelegate>(std::move(worker_memory_reporter), worker_thread, std::move(task_runner));
        ...
        NotifyMeasurementFailure(worker_thread, std::move(task_runner), worker_memory_reporter);
    }
}

// static
void V8WorkerMemoryReporter::NotifyMeasurementSuccess(WorkerThread* worker_thread, scoped_refptr<base::SingleThreadTaskRunner> task_runner,
    base::WeakPtr<V8WorkerMemoryReporter> worker_memory_reporter, std::unique_ptr<WorkerMemoryUsage> memory_usage)
{
    DCHECK(worker_thread->IsCurrentThread());
    PostCrossThreadTask(
        *task_runner, FROM_HERE, CrossThreadBindOnce(&V8WorkerMemoryReporter::OnMeasurementSuccess, worker_memory_reporter, std::move(memory_usage)));
}

// static
void V8WorkerMemoryReporter::NotifyMeasurementFailure(
    WorkerThread* worker_thread, scoped_refptr<base::SingleThreadTaskRunner> task_runner, base::WeakPtr<V8WorkerMemoryReporter> worker_memory_reporter)
{
    DCHECK(worker_thread->IsCurrentThread());
    PostCrossThreadTask(*task_runner, FROM_HERE, CrossThreadBindOnce(&V8WorkerMemoryReporter::OnMeasurementFailure, worker_memory_reporter));
}
...
void V8WorkerMemoryReporter::OnMeasurementSuccess(std::unique_ptr<WorkerMemoryUsage> memory_usage)
{
    DCHECK(IsMainThread());
    if (state_ == State::kDone)
        return;
