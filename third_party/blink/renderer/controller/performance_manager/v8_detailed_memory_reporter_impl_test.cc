...
public:
    ...
    MemoryUsageChecker(size_t expected_isolate_count, size_t expected_context_count)
...
public:
    ...
    void Callback(mojom::blink::PerProcessV8MemoryUsagePtr result)
    {
        ...
        for (const auto& isolate : result->isolates) {
            for (const auto& entry : isolate->contexts) {
                EXPECT_LE(canvas_width_ * canvas_height_ * kMinBytesPerPixel, entry->bytes_used);
                ++actual_context_count;
            }
        }
...
TEST_F(V8DetailedMemoryReporterImplTest, GetV8MemoryUsage)
{
    ...
    reporter.GetV8MemoryUsage(V8DetailedMemoryReporterImpl::Mode::EAGER, WTF::BindOnce(&MemoryUsageChecker::Callback, WTF::Unretained(&checker)));
...
TEST_F(V8DetailedMemoryReporterImplTest, CanvasMemoryUsage)
{
    ...
    Window().GetTaskRunner(TaskType::kNetworking)->PostTask(FROM_HERE, base::BindLambdaForTesting([&main_resource] {
        main_resource.Complete(R"HTML(
      <script>
        window.onload = function () {
    ...
    reporter.GetV8MemoryUsage(V8DetailedMemoryReporterImpl::Mode::EAGER, WTF::BindOnce(&CanvasMemoryUsageChecker::Callback, WTF::Unretained(&checker)));
    checker.Run();
    EXPECT_TRUE(checker.IsCalled());
}
