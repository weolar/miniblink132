...
public:
    explicit MockProcessCoordinationUnit(mojo::PendingReceiver<ProcessCoordinationUnit> receiver)
    ...
    MOCK_METHOD(void, OnV8ContextCreated, (V8ContextDescriptionPtr description, IframeAttributionDataPtr attribution), (override));
    MOCK_METHOD(void, OnV8ContextDetached, (const blink::V8ContextToken& token), (override));
    MOCK_METHOD(void, OnV8ContextDestroyed, (const blink::V8ContextToken& token), (override));
    MOCK_METHOD(void, OnRemoteIframeAttached,
        (const blink::LocalFrameToken& parent_frame_token, const blink::RemoteFrameToken& remote_frame_token, IframeAttributionDataPtr attribution),
        (override));
    MOCK_METHOD(
    ...
    MOCK_METHOD(void, RequestSharedPerformanceScenarioRegions, (uint64_t, RequestSharedPerformanceScenarioRegionsCallback), (override));
...
using StrictMockProcessCoordinationUnit = ::testing::StrictMock<MockProcessCoordinationUnit>;
using NiceMockProcessCoordinationUnit = ::testing::NiceMock<MockProcessCoordinationUnit>;

MATCHER_P(MatchV8ContextDescription, execution_context_token, "V8ContextDescription::execution_context_token matches")
{
    return arg->execution_context_token == blink::ExecutionContextToken(execution_context_token);
}

MATCHER_P2(MatchAndSaveV8ContextDescription, execution_context_token, output_token, "V8ContextDescription::execution_context_token matches")
{
    DCHECK(output_token);
...
protected:
    ...
    void InitializeMockProcessCoordinationUnit(base::ReadOnlySharedMemoryRegion global_region = base::ReadOnlySharedMemoryRegion(),
        base::ReadOnlySharedMemoryRegion process_region = base::ReadOnlySharedMemoryRegion())
    {
        ...
        mock_process_coordination_unit_ = std::make_unique<MockType>(pending_remote.InitWithNewPipeAndPassReceiver());
        ...
        EXPECT_CALL(*mock_process_coordination_unit_, RequestSharedPerformanceScenarioRegions(_, _))
            .WillOnce([&](uint64_t, ProcessCoordinationUnit::RequestSharedPerformanceScenarioRegionsCallback callback) {
                std::move(callback).Run(std::move(global_region), std::move(process_region));
        ...
        resource_coordinator_ = base::WrapUnique(new RendererResourceCoordinatorImpl(std::move(pending_remote)));
...
TEST_F(RendererResourceCoordinatorImplTest, IframeNotifications)
{
    ...
    auto iframe_attribution_matcher = Pointee(AllOf(Field(&IframeAttributionData::id, "iframe-id"), Field(&IframeAttributionData::src, WTF::String())));
    ...
    EXPECT_CALL(*mock_process_coordination_unit_, OnV8ContextCreated(MatchV8ContextDescription(main_frame->GetLocalFrameToken()), _));
    // This load must include some non-empty script to force context creation.
    frame_test_helpers::LoadHTMLString(main_frame,
        "<!DOCTYPE html>"
    ...
    EXPECT_CALL(*mock_process_coordination_unit_,
        OnRemoteIframeAttached(main_frame->GetLocalFrameToken(), remote_frame->GetRemoteFrameToken(), iframe_attribution_matcher));
    ...
    {
        InSequence seq;
        EXPECT_CALL(*mock_process_coordination_unit_, OnRemoteIframeDetached(main_frame->GetLocalFrameToken(), remote_frame->GetRemoteFrameToken()));
        EXPECT_CALL(*mock_process_coordination_unit_,
            OnRemoteIframeAttached(main_frame->GetLocalFrameToken(), new_remote_frame->GetRemoteFrameToken(), iframe_attribution_matcher));
    }
    frame_test_helpers::SwapRemoteFrame(main_frame->FirstChild(), new_remote_frame);
    mock_process_coordination_unit_->VerifyExpectations();
    ...
    {
        InSequence seq;
        EXPECT_CALL(*mock_process_coordination_unit_, OnRemoteIframeDetached(main_frame->GetLocalFrameToken(), new_remote_frame->GetRemoteFrameToken()));
        EXPECT_CALL(*mock_process_coordination_unit_,
            OnV8ContextCreated(MatchAndSaveV8ContextDescription(local_frame->GetLocalFrameToken(), &current_v8_context_token), iframe_attribution_matcher));
    }
    ...
    {
        InSequence seq;
        EXPECT_CALL(*mock_process_coordination_unit_, OnV8ContextDetached(current_v8_context_token));
        EXPECT_CALL(*mock_process_coordination_unit_,
            OnV8ContextCreated(MatchAndSaveV8ContextDescription(new_local_frame->GetLocalFrameToken(), &current_v8_context_token), iframe_attribution_matcher));
    }
    // Committing a navigation in the provisional frame swaps it in.
...
TEST_F(RendererResourceCoordinatorImplTest, NonIframeNotifications)
{
    ...
    // notification.
    WebRemoteFrameImpl* remote_frame = frame_test_helpers::CreateRemote();
    EXPECT_CALL(*mock_process_coordination_unit_, OnRemoteIframeAttached(_, _, _)).Times(0);
    frame_test_helpers::SwapRemoteFrame(main_frame->FirstChild(), remote_frame);
    mock_process_coordination_unit_->VerifyExpectations();
}
...
TEST_F(RendererResourceCoordinatorImplTest, NoScenarioRegion)
{
    ...
    EXPECT_FALSE(ScopedReadOnlyScenarioMemory::GetMappingForTesting(Scope::kGlobal));
    EXPECT_FALSE(ScopedReadOnlyScenarioMemory::GetMappingForTesting(Scope::kCurrentProcess));
}
...
TEST_F(RendererResourceCoordinatorImplTest, GlobalScenarioRegion)
{
    ...
    EXPECT_TRUE(ScopedReadOnlyScenarioMemory::GetMappingForTesting(Scope::kGlobal));
    EXPECT_FALSE(ScopedReadOnlyScenarioMemory::GetMappingForTesting(Scope::kCurrentProcess));
}
...
TEST_F(RendererResourceCoordinatorImplTest, ProcessScenarioRegion)
{
    ...
    EXPECT_FALSE(ScopedReadOnlyScenarioMemory::GetMappingForTesting(Scope::kGlobal));
    EXPECT_TRUE(ScopedReadOnlyScenarioMemory::GetMappingForTesting(Scope::kCurrentProcess));
}
