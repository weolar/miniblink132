...
public:
    ...
    static void GetMemoryUsage(ResultCallback callback, v8::MeasureMemoryExecution mode);

    // These functions are called by WorkerMeasurementDelegate on a worker thread.
    static void NotifyMeasurementSuccess(WorkerThread*, scoped_refptr<base::SingleThreadTaskRunner> task_runner, base::WeakPtr<V8WorkerMemoryReporter>,
        std::unique_ptr<WorkerMemoryUsage> memory_usage);
    static void NotifyMeasurementFailure(WorkerThread*, scoped_refptr<base::SingleThreadTaskRunner> task_runner, base::WeakPtr<V8WorkerMemoryReporter>);
...
private:
    ...
    static void StartMeasurement(
        WorkerThread*, scoped_refptr<base::SingleThreadTaskRunner> task_runner, base::WeakPtr<V8WorkerMemoryReporter>, v8::MeasureMemoryExecution);
    ...
    FRIEND_TEST_ALL_PREFIXES(V8WorkerMemoryReporterTestWithMockPlatform, GetMemoryUsageTimeout);
};
