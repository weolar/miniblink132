...
class CONTROLLER_EXPORT PrivateMemoryFootprintProvider : public MemoryUsageMonitor::Observer {
    USING_FAST_MALLOC(PrivateMemoryFootprintProvider);

public:
    // Initializes the shared instance. Has no effect if called more than once.
    static void Initialize(scoped_refptr<base::SingleThreadTaskRunner> task_runner);

private:
    explicit PrivateMemoryFootprintProvider(scoped_refptr<base::SingleThreadTaskRunner> task_runner);

    PrivateMemoryFootprintProvider(scoped_refptr<base::SingleThreadTaskRunner> task_runner_for_testing, const base::TickClock* clock);
