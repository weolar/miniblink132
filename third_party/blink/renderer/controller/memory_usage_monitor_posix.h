...
class CONTROLLER_EXPORT MemoryUsageMonitorPosix : public MemoryUsageMonitor
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
    ,
...
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
    static void Bind(mojo::PendingReceiver<mojom::blink::MemoryUsageMonitorLinux> receiver);
#endif

private:
    FRIEND_TEST_ALL_PREFIXES(MemoryUsageMonitorPosixTest, CalculateProcessFootprint);

    friend class CrashMemoryMetricsReporterImpl;
    void GetProcessMemoryUsage(MemoryUsage&) override;
    static bool CalculateProcessMemoryFootprint(
