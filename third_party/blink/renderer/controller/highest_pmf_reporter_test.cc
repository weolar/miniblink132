...
public:
    MockHighestPmfReporter(scoped_refptr<base::TestMockTimeTaskRunner> task_runner_for_testing, const base::TickClock* clock)
...
public:
    MockMemoryUsageMonitor(scoped_refptr<base::TestMockTimeTaskRunner> task_runner_for_testing, const base::TickClock* clock)
...
private:
    ...
    {
        return Page::CreateNonOrdinary(*MakeGarbageCollected<EmptyChromeClient>(), *agent_group_scheduler_,
...
public:
    ...
    void SetUp() override
    {
        ...
        memory_usage_monitor_ = std::make_unique<MockMemoryUsageMonitor>(test_task_runner_, test_task_runner_->GetMockTickClock());
        MemoryUsageMonitor::SetInstanceForTesting(memory_usage_monitor_.get());
        reporter_ = std::make_unique<MockHighestPmfReporter>(test_task_runner_, test_task_runner_->GetMockTickClock());
        PageTestBase::SetUp();
    }
