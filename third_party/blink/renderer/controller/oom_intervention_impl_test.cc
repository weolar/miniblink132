...
public:
    MockOomInterventionHost(mojo::PendingReceiver<mojom::blink::OomInterventionHost> receiver)
...
public:
    ...
    MemoryUsage GetCurrentMemoryUsage() override
    {
        return mock_memory_usage_;
    }

    // MemoryUsageMonitor will report the current memory usage as this value.
    void SetMockMemoryUsage(MemoryUsage usage)
    {
        mock_memory_usage_ = usage;
    }
...
public:
    ...
    void RunDetection(bool renderer_pause_enabled, bool navigate_ads_enabled, bool purge_v8_memory_enabled)
    {
        mojo::PendingRemote<mojom::blink::OomInterventionHost> remote_host;
        MockOomInterventionHost mock_host(remote_host.InitWithNewPipeAndPassReceiver());
        ...
        intervention_->StartDetection(std::move(remote_host), std::move(args), renderer_pause_enabled, navigate_ads_enabled, purge_v8_memory_enabled);
...
TEST_F(OomInterventionImplTest, StopWatchingAfterDetection)
{
    ...
    EXPECT_FALSE(intervention_->mock_memory_usage_monitor()->HasObserver(intervention_.get()));
}
...
TEST_F(OomInterventionImplTest, ContinueWatchingWithoutDetection)
{
    ...
    EXPECT_TRUE(intervention_->mock_memory_usage_monitor()->HasObserver(intervention_.get()));
}
...
TEST_F(OomInterventionImplTest, V1DetectionAdsNavigation)
{
    ...
    web_view->MainFrameImpl()->GetFrame()->GetDocument()->body()->setInnerHTML("<iframe name='ad' src='data:text/html,'></iframe><iframe "
                                                                               "name='non-ad' src='data:text/html,'>");

    WebFrame* ad_iframe = web_view_helper_.LocalMainFrame()->FindFrameByName(WebString::FromUTF8("ad"));
    WebFrame* non_ad_iframe = web_view_helper_.LocalMainFrame()->FindFrameByName(WebString::FromUTF8("non-ad"));

    frame_test_helpers::PumpPendingRequestsForFrameToLoad(ad_iframe->ToWebLocalFrame());
    frame_test_helpers::PumpPendingRequestsForFrameToLoad(non_ad_iframe->ToWebLocalFrame());
    ...
    auto* local_non_adframe = To<LocalFrame>(WebFrame::ToCoreFrame(*non_ad_iframe));
    ...
    EXPECT_EQ(local_non_adframe->GetDocument()->Url().GetString(), "data:text/html,");
