...
// static
UserLevelMemoryPressureSignalGenerator* UserLevelMemoryPressureSignalGenerator::Instance()
{
    DCHECK(g_instance);
    return g_instance;
}
...
// static
void UserLevelMemoryPressureSignalGenerator::Initialize(Platform* platform, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    DEFINE_STATIC_LOCAL(
        UserLevelMemoryPressureSignalGenerator, generator, (std::move(task_runner), platform->InertAndMinimumIntervalOfUserLevelMemoryPressureSignal()));
    (void)generator;
}

UserLevelMemoryPressureSignalGenerator::UserLevelMemoryPressureSignalGenerator(
    scoped_refptr<base::SingleThreadTaskRunner> task_runner, std::pair<base::TimeDelta, base::TimeDelta> inert_and_minimum_interval)
    : UserLevelMemoryPressureSignalGenerator(std::move(task_runner), inert_and_minimum_interval.first, inert_and_minimum_interval.second,
        base::DefaultTickClock::GetInstance(), ThreadScheduler::Current()->ToMainThreadScheduler())
{
}

UserLevelMemoryPressureSignalGenerator::UserLevelMemoryPressureSignalGenerator(scoped_refptr<base::SingleThreadTaskRunner> task_runner,
    base::TimeDelta inert_interval, base::TimeDelta minimum_interval, const base::TickClock* clock, MainThreadScheduler* main_thread_scheduler)
...
void UserLevelMemoryPressureSignalGenerator::OnRAILModeChanged(RAILMode rail_mode)
{
    ...
    if (!is_loading_) {
        ...
        if (has_pending_request_) {
            task_runner_->PostDelayedTask(
                FROM_HERE, WTF::BindOnce(&UserLevelMemoryPressureSignalGenerator::OnTimerFired, WTF::UnretainedWrapper(this)), inert_interval_);
        }
    }
}
...
void UserLevelMemoryPressureSignalGenerator::RequestMemoryPressureSignal()
{
    ...
    if (!inert_interval_.is_negative()) {
        ...
        base::TimeDelta elapsed = !last_loaded_.has_value() ? inert_interval_ : (now - last_loaded_.value());
        ...
        if (elapsed < inert_interval_) {
            ...
            if (!has_pending_request_) {
                task_runner_->PostDelayedTask(
                    FROM_HERE, WTF::BindOnce(&UserLevelMemoryPressureSignalGenerator::OnTimerFired, WTF::UnretainedWrapper(this)), inert_interval_ - elapsed);
            }
            has_pending_request_ = true;
            return;
        }
...
void UserLevelMemoryPressureSignalGenerator::Generate(base::TimeTicks now)
{
    ...
    if (!last_generated_.has_value() || (now - last_generated_.value()) >= minimum_interval_) {
...
void RequestUserLevelMemoryPressureSignal()
{
    ...
    if (UserLevelMemoryPressureSignalGenerator* generator = UserLevelMemoryPressureSignalGenerator::Instance()) {
