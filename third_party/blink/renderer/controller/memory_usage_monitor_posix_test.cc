...
TEST(MemoryUsageMonitorPosixTest, CalculateProcessFootprint)
{
    ...
    uint64_t expected_private_footprint_kb = (40 - 25) * getpagesize() / 1024 + expected_swap_kb;
    ...
    base::File statm_file(statm_path, base::File::FLAG_OPEN | base::File::FLAG_READ);
    ...
    base::File status_file(status_path, base::File::FLAG_OPEN | base::File::FLAG_READ);

    monitor.SetProcFiles(std::move(statm_file), std::move(status_file));

    MemoryUsage usage = monitor.GetCurrentMemoryUsage();
    EXPECT_EQ(expected_private_footprint_kb, static_cast<uint64_t>(usage.private_footprint_bytes / 1024));
    EXPECT_EQ(expected_swap_kb, static_cast<uint64_t>(usage.swap_bytes / 1024));
    EXPECT_EQ(expected_vm_size_kb, static_cast<uint64_t>(usage.vm_size_bytes / 1024));
    EXPECT_EQ(expected_peak_resident_kb, static_cast<uint64_t>(usage.peak_resident_bytes / 1024));
}
