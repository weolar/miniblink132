...
{
    MainThreadScheduler* scheduler = Thread::MainThread()->Scheduler()->ToMainThreadScheduler();
    DCHECK(scheduler);
    timer_.SetTaskRunner(scheduler->NonWakingTaskRunner());
}

MemoryUsageMonitor::MemoryUsageMonitor(scoped_refptr<base::SingleThreadTaskRunner> task_runner_for_testing, const base::TickClock* clock_for_testing)
...
void MemoryUsageMonitor::StartMonitoringIfNeeded()
{
    if (timer_.IsRunning())
        return;
    timer_.Start(FROM_HERE, kPingInterval, WTF::BindRepeating(&MemoryUsageMonitor::TimerFired, WTF::Unretained(this)));
}
...
void MemoryUsageMonitor::GetV8MemoryUsage(MemoryUsage& usage)
{
    ...
    Thread::MainThread()->Scheduler()->ToMainThreadScheduler()->ForEachMainThreadIsolate(WTF::BindRepeating(
        [](MemoryUsage& usage, v8::Isolate* isolate) {
            ...
            usage.v8_bytes += heap_statistics.total_heap_size() + heap_statistics.malloced_memory();
        },
        std::ref(usage)));
}
