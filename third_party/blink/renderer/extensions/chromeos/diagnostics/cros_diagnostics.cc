...
CrosDiagnostics& CrosDiagnostics::From(ExecutionContext& execution_context)
{
    ...
    auto* supplement = Supplement<ExecutionContext>::From<CrosDiagnostics>(execution_context);
    if (!supplement) {
...
mojom::blink::CrosDiagnostics* CrosDiagnostics::GetCrosDiagnosticsOrNull()
{
    ...
    if (!cros_diagnostics_remote_.is_bound()) {
        auto receiver = cros_diagnostics_remote_.BindNewPipeAndPassReceiver(execution_context->GetTaskRunner(TaskType::kMiscPlatformAPI));
...
ScriptPromise<CrosCpuInfo> CrosDiagnostics::getCpuInfo(ScriptState* script_state)
{
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<CrosCpuInfo>>(script_state);
    auto* cros_diagnostics = GetCrosDiagnosticsOrNull();

    if (cros_diagnostics) {
        cros_diagnostics->GetCpuInfo(WTF::BindOnce(&CrosDiagnostics::OnGetCpuInfoResponse, WrapPersistent(this), WrapPersistent(resolver)));
    }

    return resolver->Promise();
}

void CrosDiagnostics::OnGetCpuInfoResponse(ScriptPromiseResolver<CrosCpuInfo>* resolver, mojom::blink::GetCpuInfoResultPtr result)
{
    if (result->is_error()) {
        ...
        case mojom::blink::GetCpuInfoError::kCpuTelemetryInfoUnavailable:
            resolver->Reject("TelemetryProbeService returned an error when retrieving CPU "
                             "telemetry info.");
            return;
        }
        NOTREACHED();
    ...
    for (const auto& logical_cpu : result->get_cpu_info()->logical_cpus) {
        ...
        logical_cpu_info_blink->setMaxClockSpeedKhz(logical_cpu->max_clock_speed_khz);
        logical_cpu_info_blink->setScalingCurrentFrequencyKhz(logical_cpu->scaling_current_frequency_khz);
        logical_cpu_info_blink->setScalingMaxFrequencyKhz(logical_cpu->scaling_max_frequency_khz);
...
ScriptPromise<IDLSequence<CrosNetworkInterface>> CrosDiagnostics::getNetworkInterfaces(ScriptState* script_state)
{
    auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLSequence<CrosNetworkInterface>>>(script_state);
    auto* cros_diagnostics = GetCrosDiagnosticsOrNull();

    if (cros_diagnostics) {
        cros_diagnostics->GetNetworkInterfaces(WTF::BindOnce(&CrosDiagnostics::OnGetNetworkInterfacesResponse, WrapPersistent(this), WrapPersistent(resolver)));
    }
...
void CrosDiagnostics::OnGetNetworkInterfacesResponse(
    ScriptPromiseResolver<IDLSequence<CrosNetworkInterface>>* resolver, mojom::blink::GetNetworkInterfacesResultPtr result)
{
    if (result->is_error()) {
        switch (result->get_error()) {
        case mojom::blink::GetNetworkInterfacesError::kNetworkInterfaceLookupFailed:
    ...
    for (const auto& interface : result->get_network_interfaces()) {
        auto* network_interface_blink = MakeGarbageCollected<CrosNetworkInterface>();
