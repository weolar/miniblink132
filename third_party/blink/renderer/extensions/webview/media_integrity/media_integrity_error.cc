...
V8MediaIntegrityErrorName::Enum MojomToV8Enum(mojom::blink::WebViewMediaIntegrityErrorCode error)
{
    switch (error) {
    ...
    case mojom::blink::WebViewMediaIntegrityErrorCode::kApiDisabledByApplication:
...
// static
MediaIntegrityError* MediaIntegrityError::Create(String message, const MediaIntegrityErrorOptions* options)
{
    return MakeGarbageCollected<MediaIntegrityError>(std::move(message), options->mediaIntegrityErrorName());
}

// static
v8::Local<v8::Value> MediaIntegrityError::CreateForName(v8::Isolate* isolate, V8MediaIntegrityErrorName::Enum name)
{
    MediaIntegrityError* error = MakeGarbageCollected<MediaIntegrityError>(GetErrorMessageForName(name), V8MediaIntegrityErrorName(name));
    return blink::V8ThrowDOMException::AttachStackProperty(isolate, error);
}

// static
v8::Local<v8::Value> MediaIntegrityError::CreateFromMojomEnum(v8::Isolate* isolate, mojom::blink::WebViewMediaIntegrityErrorCode error_code)
{
...
MediaIntegrityError::MediaIntegrityError(String message, V8MediaIntegrityErrorName name)
