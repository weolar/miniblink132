...
class EXTENSIONS_WEBVIEW_EXPORT MediaIntegrityTokenProvider : public ScriptWrappable {
    DEFINE_WRAPPERTYPEINFO();

public:
    MediaIntegrityTokenProvider(ExecutionContext* execution_context, mojo::PendingRemote<mojom::blink::WebViewMediaIntegrityProvider> provider_pending_remote,
        uint64_t cloud_project_number);

    uint64_t cloudProjectNumber()
    {
        return cloud_project_number_;
    }

    ScriptPromise<IDLString> requestToken(ScriptState* script_state, const String& opt_content_binding, ExceptionState& exception_state);
...
private:
    ...
    void OnRequestTokenResponse(
        ScriptState* script_state, ScriptPromiseResolver<IDLString>* resolver, mojom::blink::WebViewMediaIntegrityTokenResponsePtr response);
