...
public:
    ...
    explicit V8ScriptValueSerializerForModules(ScriptState* script_state, const SerializedScriptValue::SerializeOptions& options)
...
private:
    ...
    bool WriteFileSystemHandle(SerializationTag tag, FileSystemHandle* file_system_handle);
    ...
    bool WriteDecoderBuffer(scoped_refptr<media::DecoderBuffer> data, bool for_audio);
    bool WriteMediaStreamTrack(MediaStreamTrack* track, ScriptWrappable::TypeDispatcher& dispatcher, ExceptionState& exception_state);
    bool WriteCropTarget(CropTarget*);
    bool WriteRestrictionTarget(RestrictionTarget*);
    bool WriteMediaSourceHandle(MediaSourceHandleImpl* handle, ExceptionState& exception_state);
