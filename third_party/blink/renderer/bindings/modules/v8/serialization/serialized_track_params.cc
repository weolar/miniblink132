...
SerializedContentHintType SerializeContentHint(WebMediaStreamTrack::ContentHintType type)
{
    switch (type) {
...
SerializedTrackImplSubtype SerializeTrackImplSubtype(ScriptWrappable::TypeDispatcher& dispatcher)
{
    ...
    auto* wrapper_type_info = dispatcher.DowncastTo<MediaStreamTrack>()->GetWrapperTypeInfo();
    LOG(FATAL) << "SerializeTrackImplSubtype is missing a case for " << wrapper_type_info->interface_name;
}

WebMediaStreamTrack::ContentHintType DeserializeContentHint(SerializedContentHintType type)
{
    switch (type) {
...
const WrapperTypeInfo* DeserializeTrackImplSubtype(SerializedTrackImplSubtype type)
{
    switch (type) {
