    ...
    v8::Isolate* isolate, v8::Local<v8::Value> object, wtf_size_t object_index, Transferables& transferables, ExceptionState& exception_state)
{
    ...
    if (V8ScriptValueSerializer::ExtractTransferable(isolate, object, object_index, transferables, exception_state)) {
        return true;
    }
...
//                 DOMExceptionCode::kDataCloneError, "VideoFrame at index " + String::Number(object_index) + " is a duplicate of an earlier VideoFrame.");
//             return false;
//         }
...
//             exception_state.ThrowDOMException(
//                 DOMExceptionCode::kDataCloneError, "AudioData at index " + String::Number(object_index) + " is a duplicate of an earlier AudioData.");
//             return false;
//         }
...
//     }
// 
//     if (RTCDataChannel* channel = V8RTCDataChannel::ToWrappable(isolate, object)) {
//         if (RuntimeEnabledFeatures::TransferableRTCDataChannelEnabled(CurrentExecutionContext(isolate))) {
...
//                     "RTCDataChannel at index " + String::Number(object_index) + " is a duplicate of an earlier RTCDataChannel.");
//                 return false;
//             }
// 
...
//     }
// 
//     if (MediaStreamTrack* track = V8MediaStreamTrack::ToWrappable(isolate, object)) {
//         if (RuntimeEnabledFeatures::MediaStreamTrackTransferEnabled(CurrentExecutionContext(isolate))) {
//             if (transferables.media_stream_tracks.Contains(track)) {
//                 exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError,
//                     "MediaStreamTrack at index " + String::Number(object_index) + " is a duplicate of an earlier MediaStreamTrack.");
//                 return false;
//             }
...
//     }
// 
//     if (MediaSourceHandleImpl* media_source_handle = V8MediaSourceHandle::ToWrappable(isolate, object)) {
...
//             exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError,
//                 "MediaSourceHandle at index " + String::Number(object_index) + " is a duplicate of an earlier MediaSourceHandle.");
//             return false;
//         }
//         if (media_source_handle->is_detached()) {
//             exception_state.ThrowDOMException(
//                 DOMExceptionCode::kDataCloneError, "MediaSourceHandle at index " + String::Number(object_index) + " is detached and cannot be transferred.");
//             return false;
//         }
...
bool V8ScriptValueSerializerForModules::WriteDOMObject(ScriptWrappable* wrappable, ExceptionState& exception_state)
{
    // Give the core/ implementation a chance to try first.
...
//         }
//         if (IsForStorage()) {
//             exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "An RTCDataChannel cannot be serialized for storage.");
//             return false;
//         }
...
//     }
//     if (auto* restriction_target = dispatcher.ToMostDerived<RestrictionTarget>()) {
//         if (!RuntimeEnabledFeatures::ElementCaptureEnabled(ExecutionContext::From(GetScriptState()))) {
//             return false;
//         }
//         if (IsForStorage()) {
//             exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "A RestrictionTarget cannot be serialized for storage.");
//             return false;
//         }
...
uint32_t KeyUsagesForWireFormat(WebCryptoKeyUsageMask usages, bool extractable)
{
...
bool V8ScriptValueSerializerForModules::WriteCryptoKey(const WebCryptoKey& key, ExceptionState& exception_state)
{
    ...
    case kWebCryptoKeyAlgorithmParamsTypeRsaHashed: {
        ...
        if (params.PublicExponent().size() > std::numeric_limits<uint32_t>::max()) {
            exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "A CryptoKey object could not be cloned.");
            return false;
        }
        WriteUint32(static_cast<uint32_t>(params.PublicExponent().size()));
        WriteRawBytes(params.PublicExponent().data(), params.PublicExponent().size());
        WriteUint32(AlgorithmIdForWireFormat(params.GetHash().Id()));
        break;
    }
    ...
    case kWebCryptoKeyAlgorithmParamsTypeNone:
        ...
        case kWebCryptoAlgorithmIdEd25519:
        case kWebCryptoAlgorithmIdX25519: {
            CryptoKeySubTag tag = algorithm.Id() == kWebCryptoAlgorithmIdEd25519 ? kEd25519KeyTag : kX25519KeyTag;
            WriteOneByte(tag);
            WriteUint32(AlgorithmIdForWireFormat(algorithm.Id()));
            WriteUint32(AsymmetricKeyTypeForWireFormat(key.GetType()));
            break;
    ...
    if (!Platform::Current()->Crypto()->SerializeKeyForClone(key, key_data) || key_data.size() > std::numeric_limits<uint32_t>::max()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "A CryptoKey object could not be cloned.");
        return false;
    }
...
bool V8ScriptValueSerializerForModules::WriteFileSystemHandle(SerializationTag tag, FileSystemHandle* file_system_handle)
{
    mojo::PendingRemote<mojom::blink::FileSystemAccessTransferToken> token = file_system_handle->Transfer();
...
// bool V8ScriptValueSerializerForModules::WriteRTCEncodedAudioFrame(RTCEncodedAudioFrame* audio_frame)
// {
//     auto* attachment = GetSerializedScriptValue()->GetOrCreateAttachment<RTCEncodedAudioFramesAttachment>();
...
// }
// 
// bool V8ScriptValueSerializerForModules::WriteRTCEncodedVideoFrame(RTCEncodedVideoFrame* video_frame)
// {
//     auto* attachment = GetSerializedScriptValue()->GetOrCreateAttachment<RTCEncodedVideoFramesAttachment>();
...
// }
// 
// bool V8ScriptValueSerializerForModules::WriteVideoFrameHandle(scoped_refptr<VideoFrameHandle> handle)
// {
//     auto* attachment = GetSerializedScriptValue()->GetOrCreateAttachment<VideoFrameAttachment>();
...
// }
// 
// bool V8ScriptValueSerializerForModules::WriteMediaAudioBuffer(scoped_refptr<media::AudioBuffer> audio_data)
// {
//     auto* attachment = GetSerializedScriptValue()->GetOrCreateAttachment<AudioDataAttachment>();
...
// }
// 
// bool V8ScriptValueSerializerForModules::WriteDecoderBuffer(scoped_refptr<media::DecoderBuffer> data, bool for_audio)
// {
//     auto* attachment = GetSerializedScriptValue()->GetOrCreateAttachment<DecoderBufferAttachment>();
//     auto& buffers = attachment->Buffers();
//     buffers.push_back(std::move(data));
//     const uint32_t index = static_cast<uint32_t>(buffers.size() - 1);
// 
//     WriteAndRequireInterfaceTag(for_audio ? kEncodedAudioChunkTag : kEncodedVideoChunkTag);
//     WriteUint32(index);
// 
//     return true;
// }
...
//     MediaStreamTrack* track, ScriptWrappable::TypeDispatcher& dispatcher, ExceptionState& exception_state)
// {
//     String message;
//     if (!track->TransferAllowed(message)) {
//         exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, message);
//         return false;
//     }
...
//         NOTREACHED() << "device type is " << device->type << " but track impl subtype is " << static_cast<uint32_t>(track_impl_subtype);
...
//         MediaStreamVideoSource* const native_source = MediaStreamVideoSource::GetVideoSource(source);
...
// }
// 
// bool V8ScriptValueSerializerForModules::WriteRTCDataChannel(RTCDataChannel* channel)
// {
//     if (!RuntimeEnabledFeatures::TransferableRTCDataChannelEnabled()) {
//         return false;
//     }
// 
//     auto* attachment = GetSerializedScriptValue()->GetOrCreateAttachment<RTCDataChannelAttachment>();
...
// }
// 
// bool V8ScriptValueSerializerForModules::WriteCropTarget(CropTarget* crop_target)
// {
//     CHECK(crop_target);
...
// }
// 
// bool V8ScriptValueSerializerForModules::WriteRestrictionTarget(RestrictionTarget* restriction_target)
// {
...
// }
// 
// bool V8ScriptValueSerializerForModules::WriteMediaSourceHandle(MediaSourceHandleImpl* handle, ExceptionState& exception_state)
// {
//     if (handle->is_serialized()) {
//         exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "MediaSourceHandle is already serialized.");
//         return false;
//     }
// 
//     if (handle->is_used()) {
...
//     auto* attachment = GetSerializedScriptValue()->GetOrCreateAttachment<MediaSourceHandleAttachment>();
// 
...
// 
//     scoped_refptr<HandleAttachmentProvider> media_source_attachment_provider = handle->TakeAttachmentProvider();
...
//         .attachment_provider = std::move(media_source_attachment_provider), .internal_blob_url = handle->GetInternalBlobURL() });
