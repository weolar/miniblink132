...
ScriptWrappable* V8ScriptValueDeserializerForModules::ReadDOMObject(SerializationTag tag, ExceptionState& exception_state)
{
    // Give the core/ implementation a chance to try first.
    // If it didn't recognize the kind of wrapper, try the modules types.
    if (ScriptWrappable* wrappable = V8ScriptValueDeserializer::ReadDOMObject(tag, exception_state))
        return wrappable;
...
//         String name;
//         String root_url;
//         if (!ReadUint32(&raw_type) || raw_type > static_cast<int32_t>(mojom::blink::FileSystemType::kMaxValue) || !ReadUTF8String(&name)
//             || !ReadUTF8String(&root_url))
//             return nullptr;
//         return MakeGarbageCollected<DOMFileSystem>(
//             ExecutionContext::From(GetScriptState()), name, static_cast<mojom::blink::FileSystemType>(raw_type), KURL(root_url));
    }
...
//         if (!ReadUTF8String(&pem_private_key) || !ReadUTF8String(&pem_certificate))
//             return nullptr;
...
//             return nullptr;
//         rtc::scoped_refptr<rtc::RTCCertificate> certificate = certificate_generator->FromPEM(pem_private_key, pem_certificate);
//         if (!certificate)
//             return nullptr;
...
bool AsymmetricKeyTypeFromWireFormat(uint32_t raw_key_type, WebCryptoKeyType* key_type)
{
...
bool NamedCurveFromWireFormat(uint32_t raw_named_curve, WebCryptoNamedCurve* named_curve)
{
...
bool KeyUsagesFromWireFormat(uint32_t raw_usages, WebCryptoKeyUsageMask* usages, bool* extractable)
{
    // Reminder to update this when adding new key usages.
    static_assert(kEndOfWebCryptoKeyUsage == (1 << 7) + 1, "update required when adding new key usages");
    const uint32_t kAllPossibleUsages
        = kExtractableUsage | kEncryptUsage | kDecryptUsage | kSignUsage | kVerifyUsage | kDeriveKeyUsage | kWrapKeyUsage | kUnwrapKeyUsage | kDeriveBitsUsage;
...
CryptoKey* V8ScriptValueDeserializerForModules::ReadCryptoKey()
{
    ...
    case kAesKeyTag: {
        ...
        if (!ReadUint32(&raw_id) || !AlgorithmIdFromWireFormat(raw_id, &id) || !ReadUint32(&length_bytes)
    ...
    case kHmacKeyTag: {
        ...
        if (!ReadUint32(&length_bytes) || length_bytes > std::numeric_limits<unsigned>::max() / 8 || !ReadUint32(&raw_hash)
    ...
    case kRsaHashedKeyTag: {
        ...
        uint32_t raw_hash;
        WebCryptoAlgorithmId hash;
        if (!ReadUint32(&raw_id) || !AlgorithmIdFromWireFormat(raw_id, &id) || !ReadUint32(&raw_key_type)
            || !AsymmetricKeyTypeFromWireFormat(raw_key_type, &key_type) || !ReadUint32(&modulus_length_bits) || !ReadUint32(&public_exponent_size)
            || !ReadRawBytes(public_exponent_size, &public_exponent_bytes) || !ReadUint32(&raw_hash) || !AlgorithmIdFromWireFormat(raw_hash, &hash))
            return nullptr;
        algorithm = WebCryptoKeyAlgorithm::CreateRsaHashed(
            id, modulus_length_bits, reinterpret_cast<const unsigned char*>(public_exponent_bytes), public_exponent_size, hash);
        break;
    }
    case kEcKeyTag: {
        uint32_t raw_id;
        ...
        if (!ReadUint32(&raw_id) || !AlgorithmIdFromWireFormat(raw_id, &id) || !ReadUint32(&raw_key_type)
            || !AsymmetricKeyTypeFromWireFormat(raw_key_type, &key_type) || !ReadUint32(&raw_named_curve)
    ...
    case kX25519KeyTag: {
        ...
        if (!ReadUint32(&raw_id) || !AlgorithmIdFromWireFormat(raw_id, &id) || !ReadUint32(&raw_key_type)
            ...
            return nullptr;
        algorithm = raw_key_byte == kEd25519KeyTag ? WebCryptoKeyAlgorithm::CreateEd25519(id) : WebCryptoKeyAlgorithm::CreateX25519(id);
        break;
    }
    ...
    bool extractable;
    if (!ReadUint32(&raw_usages) || !KeyUsagesFromWireFormat(raw_usages, &usages, &extractable))
        return nullptr;

    // Read key data.
    uint32_t key_data_length;
    const void* key_data;
    if (!ReadUint32(&key_data_length) || !ReadRawBytes(key_data_length, &key_data))
        return nullptr;

    WebCryptoKey key = WebCryptoKey::CreateNull();
    if (!Platform::Current()->Crypto()->DeserializeKeyForClone(
            algorithm, key_type, extractable, usages, reinterpret_cast<const unsigned char*>(key_data), key_data_length, key))
        return nullptr;

    return MakeGarbageCollected<CryptoKey>(key);
}

FileSystemHandle* V8ScriptValueDeserializerForModules::ReadFileSystemHandle(SerializationTag tag)
{
    *(int*)1 = 1;
    return nullptr;
...
//     SerializedScriptValue::FileSystemAccessTokensArray& tokens_array = GetSerializedScriptValue()->FileSystemAccessTokens();
...
//     mojo::Remote<mojom::blink::FileSystemAccessTransferToken> token(std::move(tokens_array[token_index]));
//     if (!token) {
//         return nullptr;
//     }
// 
...
//     // FileSystemHandle.
//     ExecutionContext* execution_context = ExecutionContext::From(GetScriptState());
//     mojo::Remote<mojom::blink::FileSystemAccessManager> file_system_access_manager;
//     execution_context->GetBrowserInterfaceBroker().GetInterface(file_system_access_manager.BindNewPipeAndPassReceiver());
// 
...
// 
//         file_system_access_manager->GetFileHandleFromToken(token.Unbind(), file_handle.InitWithNewPipeAndPassReceiver());
// 
//         return MakeGarbageCollected<FileSystemFileHandle>(execution_context, name, std::move(file_handle));
//     }
//     case kFileSystemDirectoryHandleTag: {
//         mojo::PendingRemote<mojom::blink::FileSystemAccessDirectoryHandle> directory_handle;
// 
//         file_system_access_manager->GetDirectoryHandleFromToken(token.Unbind(), directory_handle.InitWithNewPipeAndPassReceiver());
// 
//         return MakeGarbageCollected<FileSystemDirectoryHandle>(execution_context, name, std::move(directory_handle));
//     }
//     default: {
//         NOTREACHED();
//     }
...
// {
//     if (!RuntimeEnabledFeatures::TransferableRTCDataChannelEnabled(ExecutionContext::From(GetScriptState()))) {
//         return nullptr;
//     }
// 
//     uint32_t index;
...
//     }
// 
//     const auto* attachment = GetSerializedScriptValue()->GetAttachmentIfExists<RTCDataChannelAttachment>();
//     if (!attachment) {
//         return nullptr;
//     }
// 
//     using NativeDataChannelVector = Vector<rtc::scoped_refptr<webrtc::DataChannelInterface>>;
// 
...
// 
//     return MakeGarbageCollected<RTCDataChannel>(ExecutionContext::From(GetScriptState()), std::move(channels[index]));
// }
// 
// RTCEncodedAudioFrame* V8ScriptValueDeserializerForModules::ReadRTCEncodedAudioFrame()
// {
//     uint32_t index;
//     if (!ReadUint32(&index))
//         return nullptr;
// 
//     const auto* attachment = GetSerializedScriptValue()->GetAttachmentIfExists<RTCEncodedAudioFramesAttachment>();
//     if (!attachment)
//         return nullptr;
// 
...
// }
// 
// RTCEncodedVideoFrame* V8ScriptValueDeserializerForModules::ReadRTCEncodedVideoFrame()
// {
//     uint32_t index;
//     if (!ReadUint32(&index))
//         return nullptr;
// 
//     const auto* attachment = GetSerializedScriptValue()->GetAttachmentIfExists<RTCEncodedVideoFramesAttachment>();
//     if (!attachment)
//         return nullptr;
// 
...
//         return nullptr;
// 
//     const auto* attachment = GetSerializedScriptValue()->GetAttachmentIfExists<AudioDataAttachment>();
//     if (!attachment)
//         return nullptr;
// 
...
//         return nullptr;
// 
//     const auto* attachment = GetSerializedScriptValue()->GetAttachmentIfExists<VideoFrameAttachment>();
//     if (!attachment)
//         return nullptr;
// 
...
// }
// 
// EncodedAudioChunk* V8ScriptValueDeserializerForModules::ReadEncodedAudioChunk()
// {
//     uint32_t index;
//     if (!ReadUint32(&index))
//         return nullptr;
// 
//     const auto* attachment = GetSerializedScriptValue()->GetAttachmentIfExists<DecoderBufferAttachment>();
//     if (!attachment)
//         return nullptr;
// 
...
// }
// 
// EncodedVideoChunk* V8ScriptValueDeserializerForModules::ReadEncodedVideoChunk()
// {
//     uint32_t index;
//     if (!ReadUint32(&index))
//         return nullptr;
...
// 
//     if (!ReadUint32Enum(&track_impl_subtype) || !ReadUnguessableToken(&session_id) || !ReadUnguessableToken(&transfer_id) || !ReadUTF8String(&kind)
//         || (kind != "audio" && kind != "video") || !ReadUTF8String(&id) || !ReadUTF8String(&label) || !ReadOneByte(&enabled) || enabled > 1
//         || !ReadOneByte(&muted) || muted > 1 || !ReadUint32Enum(&contentHint) || !ReadUint32Enum(&readyState)) {
//         return nullptr;
//     }
// 
...
//     }
// 
//     return MediaStreamTrack::FromTransferredState(GetScriptState(),
//         MediaStreamTrack::TransferredValues { .track_impl_subtype = DeserializeTrackImplSubtype(track_impl_subtype),
...
// {
//     if (!RuntimeEnabledFeatures::RegionCaptureEnabled(ExecutionContext::From(GetScriptState()))) {
//         return nullptr;
//     }
// 
//     String crop_id;
...
// }
// 
// RestrictionTarget* V8ScriptValueDeserializerForModules::ReadRestrictionTarget()
// {
//     if (!RuntimeEnabledFeatures::ElementCaptureEnabled(ExecutionContext::From(GetScriptState()))) {
//         return nullptr;
//     }
// 
...
// }
// 
// MediaSourceHandleImpl* V8ScriptValueDeserializerForModules::ReadMediaSourceHandle()
// {
//     uint32_t index;
//     if (!ReadUint32(&index))
//         return nullptr;
// 
//     const auto* attachment = GetSerializedScriptValue()->GetAttachmentIfExists<MediaSourceHandleAttachment>();
//     if (!attachment)
//         return nullptr;
// 
...
//     return MakeGarbageCollected<MediaSourceHandleImpl>(std::move(handle_internals.attachment_provider), std::move(handle_internals.internal_blob_url));
// }

// static
bool V8ScriptValueDeserializerForModules::ExecutionContextExposesInterface(ExecutionContext* execution_context, SerializationTag interface_tag)
{
    ...
    if (V8ScriptValueDeserializer::ExecutionContextExposesInterface(execution_context, interface_tag)) {
        return true;
    }
