...
protected:
    void SetUp() override
    {
        ...
        DCHECK(AtomicStringTable::Instance().WeakFindForTesting("test-attribute").IsNull());
        DCHECK(AtomicStringTable::Instance().WeakFindForTesting("test-value").IsNull());
    }
...
TEST_F(V8ElementTest, SetAttributeOperationCallback)
{
    ...
    EXPECT_FALSE(AtomicStringTable::Instance().WeakFindForTesting("test-attribute").IsNull());
    EXPECT_FALSE(AtomicStringTable::Instance().WeakFindForTesting("test-value").IsNull());
...
TEST_F(V8ElementTest, GetAttributeOperationCallback_NonExisting)
{
    ...
    EXPECT_FALSE(AtomicStringTable::Instance().WeakFindForTesting("test-attribute").IsNull());
    EXPECT_TRUE(AtomicStringTable::Instance().WeakFindForTesting("test-value").IsNull());
