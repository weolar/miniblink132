...
V8TestingScope::V8TestingScope(std::unique_ptr<DummyPageHolder> holder)
    ...
    , microtasks_scope_(GetIsolate(), ToMicrotaskQueue(GetScriptState()), v8::MicrotasksScope::kDoNotRunMicrotasks)
{
...
{
    GetContext()->GetMicrotaskQueue()->PerformCheckpoint(GetContext()->GetIsolate());
}
