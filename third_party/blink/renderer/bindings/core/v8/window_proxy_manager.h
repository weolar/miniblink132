...
class CORE_EXPORT WindowProxyManager : public GarbageCollected<WindowProxyManager> {
public:
    ...
    public:
        explicit GlobalProxyVector(v8::Isolate* isolate)
            : proxies(isolate)
        {
        }
...
template <typename FrameType, typename ProxyType> class WindowProxyManagerImplHelper : public WindowProxyManager {
private:
...
protected:
    WindowProxyManagerImplHelper(v8::Isolate* isolate, Frame& frame, FrameType frame_type)
        : WindowProxyManager(isolate, frame, frame_type)
    {
    }
};

class LocalWindowProxyManager : public WindowProxyManagerImplHelper<LocalFrame, LocalWindowProxy> {
public:
    explicit LocalWindowProxyManager(v8::Isolate* isolate, LocalFrame& frame)
        : WindowProxyManagerImplHelper<LocalFrame, LocalWindowProxy>(isolate, frame, FrameType::kLocal)
    {
    }
...
class RemoteWindowProxyManager : public WindowProxyManagerImplHelper<RemoteFrame, RemoteWindowProxy> {
public:
    explicit RemoteWindowProxyManager(v8::Isolate* isolate, RemoteFrame& frame)
        : WindowProxyManagerImplHelper<RemoteFrame, RemoteWindowProxy>(isolate, frame, FrameType::kRemote)
    {
    }
};
