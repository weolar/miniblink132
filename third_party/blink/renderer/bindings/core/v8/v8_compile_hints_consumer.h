...
class CORE_EXPORT V8CrowdsourcedCompileHintsConsumer : public GarbageCollected<V8CrowdsourcedCompileHintsConsumer> {
public:
    V8CrowdsourcedCompileHintsConsumer() = default;

    V8CrowdsourcedCompileHintsConsumer(const V8CrowdsourcedCompileHintsConsumer&) = delete;
    V8CrowdsourcedCompileHintsConsumer& operator=(const V8CrowdsourcedCompileHintsConsumer&) = delete;

    // Set the compile hints data based on raw memory containing int64_t:s.
    void SetData(const int64_t* memory, size_t int64_count);

    static bool CompileHintCallback(int position, void* raw_data_and_script_name_hash);
    ...
    public:
        DataAndScriptNameHash(const scoped_refptr<Data>& data, uint32_t script_name_hash)
            : data_(data)
            , script_name_hash_(script_name_hash)
        {
        }
    ...
    std::unique_ptr<DataAndScriptNameHash> GetDataWithScriptNameHash(uint32_t script_name_hash);
