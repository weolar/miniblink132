...
public:
    ...
    ScriptPromiseTester(ScriptState* script_state, ScriptPromise<IDLType> script_promise)
        ...
        , value_object_(MakeGarbageCollected<ScriptValueObject>())
    {
        ...
        script_promise.React(script_state, MakeGarbageCollected<ThenFunction<IDLType>>(weak_factory_.GetWeakPtr(), State::kFulfilled),
            MakeGarbageCollected<ThenFunction<IDLAny>>(weak_factory_.GetWeakPtr(), State::kRejected));
    }
...
private:
    ...
    template <typename IDLType> class ThenFunction : public ThenCallable<IDLType, ThenFunction<IDLType>> {
    public:
        ...
        void React(ScriptState* script_state, BlinkType value)
        {
            ...
            owner_->value_object_->Value() = ScriptValue(script_state->GetIsolate(), ToV8Traits<IDLType>::ToV8(script_state, value));
        }
        ...
        void React(ScriptState* script_state)
        {
            ...
            CHECK_EQ(owner_->state_, State::kNotSettled);
            owner_->state_ = target_state_;
            owner_->value_object_->Value() = ScriptValue(script_state->GetIsolate(), v8::Undefined(script_state->GetIsolate()));
        }
