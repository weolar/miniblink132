...
const char kTargetHTML[] = "<!DOCTYPE html>"
                           "<script>"
                           "  (window.opener || window.top).postMessage('yay', '*');"
                           "</script>";
const char kSameOriginDomainTargetHTML[] = "<!DOCTYPE html>"
                                           "<script>"
...
class BindingSecurityCounterTest : public SimTest, public testing::WithParamInterface<const char*> {
public:
    enum class OriginDisposition { CrossOrigin, SameOrigin, SameOriginDomain };

    BindingSecurityCounterTest() = default;

    void LoadWindowAndAccessProperty(OriginDisposition which_origin, const String& property)
    {
        ...
        const String& document = String::Format("<!DOCTYPE html>"
                                                "<script>"
                                                "  %s"
                                                ...
                                                "  var w = window.open('%s');"
                                                "</script>",
            which_origin == OriginDisposition::SameOriginDomain ? "document.domain = 'example.com';" : "", property.Utf8().c_str(), target_url);
    ...
    void LoadFrameAndAccessProperty(OriginDisposition which_origin, const String& property)
    {
        const char* target_url;
        const char* target_html;
        switch (which_origin) {
...
INSTANTIATE_TEST_SUITE_P(WindowProperties, BindingSecurityCounterTest,
    testing::Values("window", "self", "location", "close", "closed", "focus", "blur", "frames", "length", "top", "opener", "parent", "postMessage"));

TEST_P(BindingSecurityCounterTest, CrossOriginWindow)
{
    LoadWindowAndAccessProperty(OriginDisposition::CrossOrigin, GetParam());
    EXPECT_TRUE(GetDocument().Loader()->GetUseCounter().IsCounted(WebFeature::kCrossOriginPropertyAccess));
    EXPECT_TRUE(GetDocument().Loader()->GetUseCounter().IsCounted(WebFeature::kCrossOriginPropertyAccessFromOpener));
    EXPECT_FALSE(GetDocument().Loader()->GetUseCounter().IsCounted(WebFeature::kDocumentDomainEnabledCrossOriginAccess));
}

TEST_P(BindingSecurityCounterTest, SameOriginWindow)
{
    LoadWindowAndAccessProperty(OriginDisposition::SameOrigin, GetParam());
    EXPECT_FALSE(GetDocument().Loader()->GetUseCounter().IsCounted(WebFeature::kCrossOriginPropertyAccess));
    EXPECT_FALSE(GetDocument().Loader()->GetUseCounter().IsCounted(WebFeature::kCrossOriginPropertyAccessFromOpener));
    EXPECT_FALSE(GetDocument().Loader()->GetUseCounter().IsCounted(WebFeature::kDocumentDomainEnabledCrossOriginAccess));
}
...
TEST_P(BindingSecurityCounterTest, SameOriginDomainWindow)
{
    ...
    EXPECT_FALSE(GetDocument().Loader()->GetUseCounter().IsCounted(WebFeature::kCrossOriginPropertyAccess));
    EXPECT_FALSE(GetDocument().Loader()->GetUseCounter().IsCounted(WebFeature::kCrossOriginPropertyAccessFromOpener));
    EXPECT_TRUE(GetDocument().Loader()->GetUseCounter().IsCounted(WebFeature::kDocumentDomainEnabledCrossOriginAccess));
}
