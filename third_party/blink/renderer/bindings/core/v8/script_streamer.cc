...
template <> struct CrossThreadCopier<mojo_base::BigBuffer> {
...
v8::ScriptType ScriptTypeForStreamingTask(ScriptResource* script_resource)
{
    ...
    case mojom::blink::ScriptType::kClassic: {
        ...
        if (script_resource->IsUnusedPreload()) {
            if (script_resource->Url().GetPath().ToString().EndsWithIgnoringCase(".mjs")) {
                return v8::ScriptType::kModule;
            }
        }
...
public:
    ...
    size_t GetMoreData(const uint8_t** src) override
    {
        ...
        while (true) {
            ...
            MojoResult result = data_pipe_->BeginReadData(MOJO_READ_DATA_FLAG_NONE, buffer);
            ...
            case MOJO_RESULT_OK: {
                ...
                if (src) {
                    auto copy_for_script_stream = base::HeapArray<uint8_t>::CopiedFrom(buffer);
                ...
                if (absl::holds_alternative<ScriptDecoder*>(script_decoder_)) {
                    absl::get<ScriptDecoder*>(script_decoder_)->DidReceiveData(std::move(copy_for_decoder));
                } else {
                    CHECK(absl::holds_alternative<ScriptDecoderWithClient*>(script_decoder_));
            ...
            case MOJO_RESULT_SHOULD_WAIT: {
                {
                    TRACE_EVENT_END0("v8,devtools.timeline," TRACE_DISABLED_BY_DEFAULT("v8.compile"), "v8.parseOnBackgroundParsing");
                    TRACE_EVENT_BEGIN0("v8,devtools.timeline," TRACE_DISABLED_BY_DEFAULT("v8.compile"), "v8.parseOnBackgroundWaiting");
                    ...
                    TRACE_EVENT_END0("v8,devtools.timeline," TRACE_DISABLED_BY_DEFAULT("v8.compile"), "v8.parseOnBackgroundWaiting");
                    TRACE_EVENT_BEGIN0("v8,devtools.timeline," TRACE_DISABLED_BY_DEFAULT("v8.compile"), "v8.parseOnBackgroundParsing");
                }
    ...
    void TakeDataAndPipeOnMainThread(
        ScriptResource* resource, ResourceScriptStreamer* streamer, mojo::ScopedDataPipeConsumerHandle data_pipe, ScriptDecoderWithClient* script_decoder)
    {
        DCHECK(IsMainThread());
        CHECK(data_pipe);
        CHECK(!ready_to_run_.IsSet());
        ...
        if (length > 0) {
            ...
            bool success = resource_buffer->GetBytes(base::span<uint8_t>(initial_data_.data(), initial_data_.size()));
            CHECK(success);
        }
    ...
    void TakeDataAndPipeOnBackgroundThread(mojo::ScopedDataPipeConsumerHandle data_pipe, ScriptDecoder* script_decoder)
    {
        CHECK(data_pipe);
        CHECK(!ready_to_run_.IsSet());
        CHECK(!cancelled_.IsSet());
...
private:
    ...
    ResourceScriptStreamer::LoadingState load_state_ = ResourceScriptStreamer::LoadingState::kLoading;
...
std::tuple<ScriptStreamer*, ScriptStreamer::NotStreamingReason> ScriptStreamer::TakeFrom(
    ScriptResource* script_resource, mojom::blink::ScriptType expected_type)
{
    const v8::ScriptType expected_script_type = expected_type == mojom::blink::ScriptType::kClassic ? v8::ScriptType::kClassic : v8::ScriptType::kModule;
    ...
    if (streamer) {
        if (streamer->IsStreamingSuppressed()) {
            ...
            CHECK(not_streamed_reason == NotStreamingReason::kInvalid || not_streamed_reason == NotStreamingReason::kBackgroundResponseProcessorWillBeUsed);
...
void RecordStartedStreamingHistogram(ScriptSchedulingType type, bool did_use_streamer)
{
    StreamedBoolean streamed = did_use_streamer ? StreamedBoolean::kStreamed : StreamedBoolean::kNotStreamed;
    switch (type) {
    case ScriptSchedulingType::kParserBlocking: {
        UMA_HISTOGRAM_ENUMERATION("WebCore.Scripts.ParsingBlocking.StartedStreaming", streamed);
        break;
    }
    case ScriptSchedulingType::kDefer: {
        UMA_HISTOGRAM_ENUMERATION("WebCore.Scripts.Deferred.StartedStreaming", streamed);
        break;
    }
    case ScriptSchedulingType::kAsync: {
        UMA_HISTOGRAM_ENUMERATION("WebCore.Scripts.Async.StartedStreaming", streamed);
        break;
    }
    default: {
        UMA_HISTOGRAM_ENUMERATION("WebCore.Scripts.Other.StartedStreaming", streamed);
        break;
    }
    }
}

void RecordNotStreamingReasonHistogram(ScriptSchedulingType type, ScriptStreamer::NotStreamingReason reason)
{
    switch (type) {
    case ScriptSchedulingType::kParserBlocking: {
        UMA_HISTOGRAM_ENUMERATION("WebCore.Scripts.ParsingBlocking.NotStreamingReason", reason);
        break;
    }
    case ScriptSchedulingType::kDefer: {
        UMA_HISTOGRAM_ENUMERATION("WebCore.Scripts.Deferred.NotStreamingReason", reason);
        break;
    }
    case ScriptSchedulingType::kAsync: {
        UMA_HISTOGRAM_ENUMERATION("WebCore.Scripts.Async.NotStreamingReason", reason);
        break;
    }
    default: {
        UMA_HISTOGRAM_ENUMERATION("WebCore.Scripts.Other.NotStreamingReason", reason);
        break;
    }
    }
}
...
void ScriptStreamer::RecordStreamingHistogram(ScriptSchedulingType type, bool can_use_streamer, ScriptStreamer::NotStreamingReason reason)
{
...
bool ScriptStreamer::ConvertEncoding(const AtomicString& encoding_name, v8::ScriptCompiler::StreamedSource::Encoding* encoding)
{
    // Here's a list of encodings we can use for streaming. These are
    // the canonical names.
    if (encoding_name == "windows-1252" || encoding_name == "ISO-8859-1" || encoding_name == "US-ASCII") {
...
v8_compile_hints::V8LocalCompileHintsConsumer* ResourceScriptStreamer::GetV8LocalCompileHintsConsumerForTest() const
{
...
void ResourceScriptStreamer::StreamingCompleteOnBackgroundThread(LoadingState state)
{
    DCHECK(!IsMainThread());

    // notifyFinished might already be called, or it might be called in the
    // future (if the parsing finishes earlier because of a parse error).
    script_decoder_->FinishDecode(CrossThreadBindOnce(&ResourceScriptStreamer::StreamingComplete, WrapCrossThreadPersistent(this), state));
...
void ResourceScriptStreamer::RunScriptStreamingTask(
    std::unique_ptr<v8::ScriptCompiler::ScriptStreamingTask> task, ResourceScriptStreamer* streamer, SourceStream* stream)
{
    // TODO(leszeks): Add flow event data again
    TRACE_EVENT_BEGIN1("v8,devtools.timeline," TRACE_DISABLED_BY_DEFAULT("v8.compile"), "v8.parseOnBackground", "data", [&](perfetto::TracedValue context) {
        inspector_parse_script_event::Data(std::move(context), streamer->ScriptResourceIdentifier(), streamer->ScriptURLString());
    });
    ...
    TRACE_EVENT_END0("v8,devtools.timeline," TRACE_DISABLED_BY_DEFAULT("v8.compile"), "v8.parseOnBackground");

    // TODO(crbug.com/1021571); Remove this once the last event stops being
    // dropped.
    TRACE_EVENT_END0("v8,devtools.timeline," TRACE_DISABLED_BY_DEFAULT("v8.compile"), "v8.parseOnBackground2");
}
...
bool ResourceScriptStreamer::TryStartStreamingTask()
{
    ...
    if (script_type_ == v8::ScriptType::kModule && !MIMETypeRegistry::IsSupportedJavaScriptMIMEType(script_resource_->GetResponse().HttpContentType())) {
    ...
    // streaming.
    if (!script_resource_->ResourceBuffer() || script_resource_->ResourceBuffer()->size() < kMaximumLengthOfBOM) {
        CHECK(!IsLoaded());
        return false;
    }

    {
        ...
        if (!script_resource_->ResourceBuffer()->GetBytes(base::as_writable_byte_span(maybe_bom))) {
            NOTREACHED();
        }

        std::unique_ptr<TextResourceDecoder> decoder(std::make_unique<TextResourceDecoder>(
            TextResourceDecoderOptions(TextResourceDecoderOptions::kPlainTextContent, WTF::TextEncoding(script_resource_->Encoding()))));
        decoder->CheckForBOM(base::span<const char>(maybe_bom.data(), maybe_bom.size()));
    ...
    if (V8CodeCache::HasCodeCache(script_resource_->CacheHandler(), CachedMetadataHandler::kAllowUnchecked)) {
    ...
    source_ = std::make_unique<v8::ScriptCompiler::StreamedSource>(std::move(stream_ptr), encoding_);

    const bool has_hot_timestamp = V8CodeCache::HasHotTimestamp(script_resource_->CacheHandler());
    compile_hints_ = v8_compile_hints::CompileHintsForStreaming::Builder(script_resource_->GetV8CrowdsourcedCompileHintsProducer(),
        script_resource_->GetV8CrowdsourcedCompileHintsConsumer(), script_resource_->Url(), script_resource_->GetV8CompileHintsMagicCommentMode())
                         .Build((V8CodeCache::HasCompileHints(script_resource_->CacheHandler(), CachedMetadataHandler::kAllowUnchecked) && has_hot_timestamp)
                                 ? V8CodeCache::GetCachedMetadataForCompileHints(script_resource_->CacheHandler(), CachedMetadataHandler::kAllowUnchecked)
                                 : nullptr,
                             has_hot_timestamp);
    ...
    std::unique_ptr<v8::ScriptCompiler::ScriptStreamingTask> script_streaming_task = base::WrapUnique(v8::ScriptCompiler::StartStreaming(isolate, source_.get(),
        script_type_, compile_hints_->compile_options(), compile_hints_->GetCompileHintCallback(), compile_hints_->GetCompileHintCallbackData()));
    ...
    TRACE_EVENT_WITH_FLOW1(TRACE_DISABLED_BY_DEFAULT("v8.compile"), "v8.streamingCompile.start", this, TRACE_EVENT_FLAG_FLOW_OUT, "data",
        [&](perfetto::TracedValue context) { inspector_parse_script_event::Data(std::move(context), ScriptResourceIdentifier(), ScriptURLString()); });

    stream_->TakeDataAndPipeOnMainThread(script_resource_, this, std::move(data_pipe_), script_decoder_.get());
    ...
    worker_pool::PostTask(FROM_HERE, { base::TaskPriority::USER_BLOCKING, base::MayBlock() },
        CrossThreadBindOnce(RunScriptStreamingTask, std::move(script_streaming_task), WrapCrossThreadPersistent(this), WTF::CrossThreadUnretained(stream_)));
...
ResourceScriptStreamer::ResourceScriptStreamer(ScriptResource* script_resource, mojo::ScopedDataPipeConsumerHandle data_pipe,
    ResponseBodyLoaderClient* response_body_loader_client, std::unique_ptr<TextResourceDecoder> decoder,
    scoped_refptr<base::SingleThreadTaskRunner> loading_task_runner)
    : script_resource_(script_resource)
    , response_body_loader_client_(response_body_loader_client)
    , script_decoder_(ScriptDecoderWithClient::Create(response_body_loader_client, std::move(decoder), loading_task_runner))
    , data_pipe_(std::move(data_pipe))
...
{
    watcher_ = std::make_unique<mojo::SimpleWatcher>(FROM_HERE, mojo::SimpleWatcher::ArmingPolicy::MANUAL, loading_task_runner);

    watcher_->Watch(data_pipe_.get(), MOJO_HANDLE_SIGNAL_READABLE, MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED,
        WTF::BindRepeating(&ResourceScriptStreamer::OnDataPipeReadable, WrapWeakPersistent(this)));
...
void ResourceScriptStreamer::OnDataPipeReadable(MojoResult result, const mojo::HandleSignalsState& state)
{
    if (IsClientDetached())
        return;
    ...
    case MOJO_RESULT_CANCELLED:
        ...
        // streaming task.
        watcher_.reset();
        LoadCompleteWithoutStreaming(LoadingState::kCancelled, NotStreamingReason::kLoadingCancelled);
        return;
    ...
    case MOJO_RESULT_FAILED_PRECONDITION:
        ...
        CHECK(IsStreamingSuppressed() || !script_resource_->ResourceBuffer() || script_resource_->ResourceBuffer()->size() < kMaximumLengthOfBOM);
        watcher_.reset();
        // Pass kScriptTooSmall for the !IsStreamingSuppressed() case, it won't
        // override an existing streaming reason.
        LoadCompleteWithoutStreaming(LoadingState::kLoaded, NotStreamingReason::kScriptTooSmall);
        return;
    ...
    default:
        // Some other error occurred.
        watcher_.reset();
        LoadCompleteWithoutStreaming(LoadingState::kFailed, NotStreamingReason::kErrorOccurred);
        return;
    }
...
{
    TRACE_EVENT_WITH_FLOW2(TRACE_DISABLED_BY_DEFAULT("v8.compile"), "v8.streamingCompile.complete", this, TRACE_EVENT_FLAG_FLOW_IN, "streaming_suppressed",
...
void ResourceScriptStreamer::LoadCompleteWithoutStreaming(LoadingState state, NotStreamingReason no_streaming_reason)
{
    ...
    script_decoder_->FinishDecode(CrossThreadBindOnce(&ResourceScriptStreamer::SendClientLoadFinishedCallback, WrapCrossThreadPersistent(this)));
}
...
void ResourceScriptStreamer::AdvanceLoadingState(LoadingState new_state)
{
    switch (loading_state_) {
    case LoadingState::kLoading:
        CHECK(new_state == LoadingState::kLoaded || new_state == LoadingState::kFailed || new_state == LoadingState::kCancelled);
        break;
    case LoadingState::kLoaded:
    case LoadingState::kFailed:
    case LoadingState::kCancelled:
...
void ResourceScriptStreamer::CheckState() const
{
    ...
    case LoadingState::kLoading:
        ...
        // rid of it.
        CHECK(CanStartStreaming() || IsStreamingStarted() || IsStreamingSuppressed());
        break;
    case LoadingState::kLoaded:
    case LoadingState::kFailed:
    case LoadingState::kCancelled:
...
class InlineSourceStream final : public v8::ScriptCompiler::ExternalSourceStream {
public:
    explicit InlineSourceStream(const String& text)
        : text_(text)
    {
    }
...
BackgroundInlineScriptStreamer::BackgroundInlineScriptStreamer(v8::Isolate* isolate, const String& text, v8::ScriptCompiler::CompileOptions compile_options)
{
    auto stream = std::make_unique<InlineSourceStream>(text);
    source_ = std::make_unique<v8::ScriptCompiler::StreamedSource>(
        std::move(stream), text.Is8Bit() ? v8::ScriptCompiler::StreamedSource::ONE_BYTE : v8::ScriptCompiler::StreamedSource::TWO_BYTE);

    // We don't generate code caches for inline scripts, so we never pass the
    // kFollowCompileHintsMagicComment compile option.
    CHECK((compile_options & v8::ScriptCompiler::kFollowCompileHintsMagicComment) == 0);
    task_ = base::WrapUnique(v8::ScriptCompiler::StartStreaming(isolate, source_.get(), v8::ScriptType::kClassic, compile_options));
}
...
v8::ScriptCompiler::StreamedSource* BackgroundInlineScriptStreamer::Source(v8::ScriptType expected_type)
{
    ...
    static const base::FeatureParam<base::TimeDelta> kWaitTimeoutParam { &features::kPrecompileInlineScripts, "inline-script-timeout", base::Milliseconds(0) };
    ...
    base::UmaHistogramBoolean("WebCore.Scripts.InlineStreamerTimedOut", !signaled);
    if (!signaled)
        return nullptr;
    return source_.get();
}
...
// static
InlineScriptStreamer* InlineScriptStreamer::From(scoped_refptr<BackgroundInlineScriptStreamer> streamer)
{
...
std::unique_ptr<v8::ScriptCompiler::ConsumeCodeCacheTask> MaybeCreateConsumeCodeCacheTask(
    std::optional<mojo_base::BigBuffer>& big_buffer, const String& encoding, v8::Isolate* isolate, bool& has_code_cache, v8::ScriptType script_type)
{
    ...
    scoped_refptr<CachedMetadata> metadata = CachedMetadata::CreateFromSerializedData(*big_buffer);
    if (!metadata) {
        return nullptr;
    }
    ...
    if (V8CodeCache::HasCodeCache(*metadata, encoding)) {
        ...
        if (features::kBackgroundCodeCacheDecoderStart.Get()) {
            task.reset(v8::ScriptCompiler::StartConsumingCodeCacheOnBackground(isolate, V8CodeCache::CreateCachedData(metadata)));
        }
    }
    absl::variant<Vector<uint8_t>, mojo_base::BigBuffer> drained_data = std::move(*metadata).DrainSerializedData();
...
std::unique_ptr<v8_compile_hints::CompileHintsForStreaming> BuildCompileHintsForStreaming(
    v8_compile_hints::CompileHintsForStreaming::Builder& builder, std::optional<mojo_base::BigBuffer>& big_buffer, const String& encoding)
{
    ...
    scoped_refptr<CachedMetadata> metadata = big_buffer ? CachedMetadata::CreateFromSerializedData(*big_buffer) : nullptr;

    V8CodeCache::RecordCacheGetStatistics(metadata.get(), encoding);
    std::unique_ptr<v8_compile_hints::CompileHintsForStreaming> result = std::move(builder).Build(
        (metadata && V8CodeCache::HasHotCompileHints(*metadata, encoding)) ? metadata : nullptr, metadata && V8CodeCache::HasHotTimestamp(*metadata, encoding));
    if (metadata) {
        absl::variant<Vector<uint8_t>, mojo_base::BigBuffer> drained_data = std::move(*metadata).DrainSerializedData();
        CHECK(absl::holds_alternative<mojo_base::BigBuffer>(drained_data));
        big_buffer = std::move(absl::get<mojo_base::BigBuffer>(drained_data));
...
BackgroundResourceScriptStreamer::Result::Result(
    String decoded_data, std::unique_ptr<ParkableStringImpl::SecureDigest> digest, std::unique_ptr<v8::ScriptCompiler::StreamedSource> streamed_source)
...
BackgroundResourceScriptStreamer::Result::Result(String decoded_data, std::unique_ptr<ParkableStringImpl::SecureDigest> digest,
    std::unique_ptr<v8::ScriptCompiler::ConsumeCodeCacheTask> consume_code_cache_task)
    : decoded_data(std::move(decoded_data))
    , digest(std::move(digest))
...
class BackgroundResourceScriptStreamer::BackgroundProcessor final : public BackgroundResponseProcessor {
public:
    BackgroundProcessor(v8::ScriptType script_type, const String script_url_string, uint64_t script_resource_identifier, v8::Isolate* isolate,
        WTF::TextEncoding encoding, std::unique_ptr<v8_compile_hints::CompileHintsForStreaming::Builder> compile_hints_builder,
    ...
    bool MaybeStartProcessingResponse(network::mojom::URLResponseHeadPtr& head, mojo::ScopedDataPipeConsumerHandle& body,
        std::optional<mojo_base::BigBuffer>& cached_metadata_buffer, scoped_refptr<base::SequencedTaskRunner> background_task_runner, Client* client) override;

    v8::ScriptType script_type() const
    {
        return script_type_;
    }
...
private:
    static void RunScriptStreamingTask(const String script_url_string, uint64_t script_resource_identifier,
        std::unique_ptr<v8::ScriptCompiler::ScriptStreamingTask> script_streaming_task, std::unique_ptr<v8::ScriptCompiler::StreamedSource> streamed_source,
        SourceStream* source_stream_ptr, ScriptDecoderPtr script_decoder, std::unique_ptr<v8_compile_hints::CompileHintsForStreaming> compile_hints,
        base::WeakPtr<BackgroundProcessor> background_processor_weak_ptr);
    static void RunConsumingCodeCacheTask(const String script_url_string, uint64_t script_resource_identifier,
        std::unique_ptr<v8::ScriptCompiler::ConsumeCodeCacheTask> consume_code_cache_task, scoped_refptr<base::SequencedTaskRunner> background_task_runner,
        mojo_base::BigBuffer cached_metadata, base::WeakPtr<BackgroundProcessor> background_processor_weak_ptr, const uint64_t trace_id);

    void SetState(BackgroundProcessorState state);

    void OnDataPipeReadable(MojoResult ready_result, const mojo::HandleSignalsState& ready_state);
    bool TryStartStreamingTask(MojoResult result, const mojo::HandleSignalsState& state);

    void OnFinishStreaming(std::unique_ptr<v8::ScriptCompiler::StreamedSource> streamed_source, ScriptDecoderPtr script_decoder, ScriptDecoder::Result result);

    void OnFinishCodeCacheConsumer(std::unique_ptr<v8::ScriptCompiler::ConsumeCodeCacheTask> consume_code_cache_task, mojo_base::BigBuffer cached_metadata);
    ...
    // For CompileHints
    std::unique_ptr<v8_compile_hints::CompileHintsForStreaming::Builder> compile_hints_builder_;
    ...
    std::unique_ptr<v8::ScriptCompiler::ConsumeCodeCacheTask> consume_code_cache_task_;
    ...
    BackgroundProcessorState state_ = BackgroundProcessorState::kWaitingForResponse;

    SEQUENCE_CHECKER(background_sequence_checker_);
    base::WeakPtrFactory<BackgroundProcessor> weak_factory_ { this };
};

class BackgroundResourceScriptStreamer::BackgroundProcessorFactory final : public BackgroundResponseProcessorFactory {
public:
    BackgroundProcessorFactory(ScriptResource* script_resource, CrossThreadWeakHandle<BackgroundResourceScriptStreamer> streamer_handle)
        : script_type_(ScriptTypeForStreamingTask(script_resource))
        , script_url_string_(script_resource->Url().GetString())
        , script_resource_identifier_(script_resource->InspectorId())
        , isolate_(script_resource->GetIsolateOrNull())
        , encoding_(script_resource->Encoding())
        , compile_hints_builder_(std::make_unique<v8_compile_hints::CompileHintsForStreaming::Builder>(script_resource->GetV8CrowdsourcedCompileHintsProducer(),
              script_resource->GetV8CrowdsourcedCompileHintsConsumer(), script_resource->Url(), script_resource->GetV8CompileHintsMagicCommentMode()))
        , streamer_handle_(std::move(streamer_handle))
    {
    }
    BackgroundProcessorFactory(const BackgroundProcessorFactory&) = delete;
    BackgroundProcessorFactory& operator=(const BackgroundProcessorFactory&) = delete;
    ...
    std::unique_ptr<BackgroundResponseProcessor> Create() && override
    {
        return std::make_unique<BackgroundProcessor>(
            script_type_, script_url_string_, script_resource_identifier_, isolate_, encoding_, std::move(compile_hints_builder_), std::move(streamer_handle_));
    }
...
BackgroundResourceScriptStreamer::BackgroundProcessor::BackgroundProcessor(v8::ScriptType script_type, const String script_url_string,
    uint64_t script_resource_identifier, v8::Isolate* isolate, WTF::TextEncoding encoding,
    std::unique_ptr<v8_compile_hints::CompileHintsForStreaming::Builder> compile_hints_builder,
    CrossThreadWeakHandle<BackgroundResourceScriptStreamer> streamer_handle)
...
void BackgroundResourceScriptStreamer::BackgroundProcessor::SetState(BackgroundProcessorState state)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(background_sequence_checker_);
#if DCHECK_IS_ON()
    using S = BackgroundProcessorState;
    DEFINE_THREAD_SAFE_STATIC_LOCAL(base::StateTransitions<S>, transitions,
        ({
...
bool BackgroundResourceScriptStreamer::BackgroundProcessor::MaybeStartProcessingResponse(network::mojom::URLResponseHeadPtr& head,
    mojo::ScopedDataPipeConsumerHandle& body, std::optional<mojo_base::BigBuffer>& cached_metadata,
    scoped_refptr<base::SequencedTaskRunner> background_task_runner, Client* client)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(background_sequence_checker_);
    CHECK(background_task_runner->RunsTasksInCurrentSequence());
    TRACE_EVENT1(
        "v8,devtools.timeline," TRACE_DISABLED_BY_DEFAULT("v8.compile"), "BackgroundProcessor::MaybeStartProcessingResponse", "url", script_url_string_.Utf8());
    CHECK(body);
    CHECK(head);
    ...
    if (script_type_ == v8::ScriptType::kModule) {
        ...
        if (!head->headers->GetMimeType(&mime_type) || !MIMETypeRegistry::IsSupportedJavaScriptMIMEType(String(mime_type))) {
    ...
    if (auto consume_code_cache_task = MaybeCreateConsumeCodeCacheTask(cached_metadata_, encoding_.GetName(), isolate_, has_code_cache, script_type())) {
        const uint64_t trace_id = static_cast<uint64_t>(reinterpret_cast<uintptr_t>(this));
        TRACE_EVENT_WITH_FLOW1("v8," TRACE_DISABLED_BY_DEFAULT("v8.compile"), "v8.deserializeOnBackground.start", TRACE_ID_LOCAL(trace_id),
            TRACE_EVENT_FLAG_FLOW_OUT, "data", [&](perfetto::TracedValue context) {
                inspector_deserialize_script_event::Data(std::move(context), script_resource_identifier_, script_url_string_);
            });
        CHECK(features::kBackgroundCodeCacheDecoderStart.Get());
        V8CodeCache::RecordCacheGetStatistics(V8CodeCache::GetMetadataType::kCodeCache);
        SetState(BackgroundProcessorState::kWaitingForConsumeCodeCacheResultAndDecodedScript);
        data_pipe_script_decoder_ = DataPipeScriptDecoder::Create(
            std::make_unique<TextResourceDecoder>(TextResourceDecoderOptions(TextResourceDecoderOptions::kPlainTextContent, encoding_)),
            background_task_runner_, CrossThreadBindOnce(&BackgroundProcessor::OnFinishScriptDecode, weak_factory_.GetWeakPtr()));
        ...
        worker_pool::PostTask(FROM_HERE, { base::TaskPriority::USER_BLOCKING, base::MayBlock() },
            CrossThreadBindOnce(&BackgroundProcessor::RunConsumingCodeCacheTask, script_url_string_, script_resource_identifier_,
                std::move(consume_code_cache_task), background_task_runner_, std::move(*cached_metadata_), weak_factory_.GetWeakPtr(), trace_id));
        return true;
    }
    ...
    compile_hints_ = BuildCompileHintsForStreaming(*compile_hints_builder_, cached_metadata_, encoding_.GetName());
    CHECK(compile_hints_);

    watcher_ = std::make_unique<mojo::SimpleWatcher>(FROM_HERE, mojo::SimpleWatcher::ArmingPolicy::MANUAL);
    watcher_->Watch(body_.get(), MOJO_HANDLE_SIGNAL_NEW_DATA_READABLE, MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED,
        WTF::BindRepeating(&BackgroundProcessor::OnDataPipeReadable, weak_factory_.GetWeakPtr()));
    MojoResult ready_result;
    mojo::HandleSignalsState ready_state;
    MojoResult rv = watcher_->Arm(&ready_result, &ready_state);
    if (rv == MOJO_RESULT_OK) {
...
void BackgroundResourceScriptStreamer::BackgroundProcessor::OnDataPipeReadable(MojoResult ready_result, const mojo::HandleSignalsState& ready_state)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(background_sequence_checker_);
    TRACE_EVENT0("v8,devtools.timeline," TRACE_DISABLED_BY_DEFAULT("v8.compile"), "BackgroundProcessor::OnDataPipeReadable");
    CHECK_EQ(state_, BackgroundProcessorState::kWaitingForDataPipeReadable);
    if (TryStartStreamingTask(ready_result, ready_state)) {
        CHECK_EQ(state_, BackgroundProcessorState::kWaitingForParseResult);
        return;
    ...
    client_->DidFinishBackgroundResponseProcessor(std::move(head_), std::move(body_), std::move(cached_metadata_));
}

bool BackgroundResourceScriptStreamer::BackgroundProcessor::TryStartStreamingTask(MojoResult result, const mojo::HandleSignalsState& state)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(background_sequence_checker_);
    TRACE_EVENT0("v8,devtools.timeline," TRACE_DISABLED_BY_DEFAULT("v8.compile"), "BackgroundProcessor::TryStartStreamingTask");
    ...
    MojoResult begin_read_result = body_->BeginReadData(MOJO_READ_DATA_FLAG_NONE, data);
    CHECK_EQ(begin_read_result, MOJO_RESULT_OK);
    ...
    MojoResult end_read_result = body_->EndReadData(0);
    CHECK_EQ(end_read_result, MOJO_RESULT_OK);
    v8::ScriptCompiler::StreamedSource::Encoding script_source_encoding = v8::ScriptCompiler::StreamedSource::UTF8;
    if (!ScriptStreamer::ConvertEncoding(decoder->Encoding().GetName(), &script_source_encoding)) {
        SuppressStreaming(NotStreamingReason::kEncodingNotSupportedBackground);
        return false;
    }

    ScriptDecoderPtr script_decoder = ScriptDecoder::Create(
        std::make_unique<TextResourceDecoder>(TextResourceDecoderOptions(TextResourceDecoderOptions::kPlainTextContent, encoding_)), background_task_runner_);
    auto source_stream = std::make_unique<SourceStream>();
    source_stream_ptr_ = source_stream.get();
    source_stream->TakeDataAndPipeOnBackgroundThread(std::move(body_), script_decoder.get());
    std::unique_ptr<v8::ScriptCompiler::StreamedSource> streamed_source
        = std::make_unique<v8::ScriptCompiler::StreamedSource>(std::move(source_stream), script_source_encoding);

    CHECK(compile_hints_);
    std::unique_ptr<v8::ScriptCompiler::ScriptStreamingTask> script_streaming_task
        = base::WrapUnique(v8::ScriptCompiler::StartStreaming(isolate_, streamed_source.get(), script_type_, compile_hints_->compile_options(),
            compile_hints_->GetCompileHintCallback(), compile_hints_->GetCompileHintCallbackData()));
    if (!script_streaming_task) {
    ...
    worker_pool::PostTask(FROM_HERE, { base::TaskPriority::USER_BLOCKING, base::MayBlock() },
        CrossThreadBindOnce(&BackgroundProcessor::RunScriptStreamingTask, script_url_string_, script_resource_identifier_, std::move(script_streaming_task),
            std::move(streamed_source), CrossThreadUnretained(source_stream_ptr_), std::move(script_decoder), std::move(compile_hints_),
            weak_factory_.GetWeakPtr()));
    return true;
}

// static
void BackgroundResourceScriptStreamer::BackgroundProcessor::RunScriptStreamingTask(const String script_url_string, uint64_t script_resource_identifier,
    std::unique_ptr<v8::ScriptCompiler::ScriptStreamingTask> script_streaming_task, std::unique_ptr<v8::ScriptCompiler::StreamedSource> streamed_source,
    SourceStream* source_stream_ptr, ScriptDecoderPtr script_decoder, std::unique_ptr<v8_compile_hints::CompileHintsForStreaming> compile_hints,
    base::WeakPtr<BackgroundProcessor> background_processor_weak_ptr)
{
    TRACE_EVENT1(
        "v8,devtools.timeline," TRACE_DISABLED_BY_DEFAULT("v8.compile"), "BackgroundProcessor::RunScriptStreamingTask", "url", script_url_string.Utf8());
    TRACE_EVENT_BEGIN1("v8,devtools.timeline," TRACE_DISABLED_BY_DEFAULT("v8.compile"), "v8.parseOnBackground", "data",
        [&](perfetto::TracedValue context) { inspector_parse_script_event::Data(std::move(context), script_resource_identifier, script_url_string); });
    TRACE_EVENT_BEGIN0("v8,devtools.timeline," TRACE_DISABLED_BY_DEFAULT("v8.compile"), "v8.parseOnBackgroundParsing");
    ...
    decoder->FinishDecode(CrossThreadBindOnce(
        &BackgroundProcessor::OnFinishStreaming, std::move(background_processor_weak_ptr), std::move(streamed_source), std::move(script_decoder)));
}

void BackgroundResourceScriptStreamer::BackgroundProcessor::OnFinishStreaming(
    std::unique_ptr<v8::ScriptCompiler::StreamedSource> streamed_source, ScriptDecoderPtr script_decoder, ScriptDecoder::Result result)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(background_sequence_checker_);
    TRACE_EVENT1("v8,devtools.timeline," TRACE_DISABLED_BY_DEFAULT("v8.compile"), "BackgroundProcessor::OnFinishStreaming", "url", script_url_string_.Utf8());
    source_stream_ptr_ = nullptr;
    CHECK_EQ(state_, BackgroundProcessorState::kWaitingForParseResult);
    SetState(BackgroundProcessorState::kFinished);
    client_->PostTaskToMainThread(
        CrossThreadBindOnce(&BackgroundResourceScriptStreamer::OnResult, MakeUnwrappingCrossThreadWeakHandle(std::move(streamer_handle_)),
            std::make_unique<Result>(std::move(result.decoded_data), std::move(result.digest), std::move(streamed_source)), suppressed_reason_));
    client_->DidFinishBackgroundResponseProcessor(std::move(head_), std::move(result.raw_data), std::move(cached_metadata_));
}

// static
void BackgroundResourceScriptStreamer::BackgroundProcessor::RunConsumingCodeCacheTask(const String script_url_string, uint64_t script_resource_identifier,
    std::unique_ptr<v8::ScriptCompiler::ConsumeCodeCacheTask> consume_code_cache_task, scoped_refptr<base::SequencedTaskRunner> background_task_runner,
    mojo_base::BigBuffer cached_metadata, base::WeakPtr<BackgroundProcessor> background_processor_weak_ptr, const uint64_t trace_id)
{
    TRACE_EVENT_WITH_FLOW1("v8,devtools.timeline," TRACE_DISABLED_BY_DEFAULT("v8.compile"), "v8.deserializeOnBackground", TRACE_ID_LOCAL(trace_id),
        TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "data",
        [&](perfetto::TracedValue context) { inspector_deserialize_script_event::Data(std::move(context), script_resource_identifier, script_url_string); });
    // Run the cache consumption task.
    consume_code_cache_task->Run();
    PostCrossThreadTask(*background_task_runner, FROM_HERE,
        CrossThreadBindOnce(&BackgroundProcessor::OnFinishCodeCacheConsumer, std::move(background_processor_weak_ptr), std::move(consume_code_cache_task),
            std::move(cached_metadata)));
}

void BackgroundResourceScriptStreamer::BackgroundProcessor::OnFinishCodeCacheConsumer(
    std::unique_ptr<v8::ScriptCompiler::ConsumeCodeCacheTask> consume_code_cache_task, mojo_base::BigBuffer cached_metadata)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(background_sequence_checker_);
    ...
    CHECK_EQ(state_, BackgroundProcessorState::kWaitingForConsumeCodeCacheResultAndDecodedScript);
    CHECK(features::kBackgroundCodeCacheDecoderStart.Get());
    SetState(BackgroundProcessorState::kWaitingForDecodedScript);
}

void BackgroundResourceScriptStreamer::BackgroundProcessor::OnFinishScriptDecode(ScriptDecoder::Result result)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(background_sequence_checker_);
...
void BackgroundResourceScriptStreamer::BackgroundProcessor::OnFinishCodeCacheConsumerScriptDecode()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(background_sequence_checker_);
    TRACE_EVENT1("v8,devtools.timeline," TRACE_DISABLED_BY_DEFAULT("v8.compile"), "BackgroundProcessor::OnFinishCodeCacheConsumerScriptDecode", "url",
    ...
    client_->PostTaskToMainThread(
        CrossThreadBindOnce(&BackgroundResourceScriptStreamer::OnResult, MakeUnwrappingCrossThreadWeakHandle(std::move(streamer_handle_)),
            std::make_unique<Result>(std::move(decoder_result_->decoded_data), std::move(decoder_result_->digest), std::move(consume_code_cache_task_)),
            NotStreamingReason::kHasCodeCacheBackground));
    client_->DidFinishBackgroundResponseProcessor(std::move(head_), std::move(decoder_result_->raw_data), std::move(cached_metadata_));
}

bool BackgroundResourceScriptStreamer::BackgroundProcessor::IsStreamingSuppressed()
{
    return suppressed_reason_ != NotStreamingReason::kInvalid;
}

void BackgroundResourceScriptStreamer::BackgroundProcessor::SuppressStreaming(NotStreamingReason reason)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(background_sequence_checker_);
    CHECK_EQ(suppressed_reason_, NotStreamingReason::kInvalid);
    CHECK_NE(reason, NotStreamingReason::kInvalid);
...
BackgroundResourceScriptStreamer::BackgroundResourceScriptStreamer(ScriptResource* script_resource)
    : script_resource_(script_resource)
    , script_type_(ScriptTypeForStreamingTask(script_resource))
{
}
...
v8::ScriptCompiler::StreamedSource* BackgroundResourceScriptStreamer::Source(v8::ScriptType expected_type)
{
    CHECK(result_);
    CHECK(!IsStreamingSuppressed());
...
std::unique_ptr<BackgroundResponseProcessorFactory> BackgroundResourceScriptStreamer::CreateBackgroundResponseProcessorFactory()
{
    return std::make_unique<BackgroundProcessorFactory>(script_resource_, MakeCrossThreadWeakHandle(this));
}

ParkableString BackgroundResourceScriptStreamer::TakeDecodedData()
{
    CHECK(result_);
    CHECK(suppressed_reason_ == NotStreamingReason::kInvalid || suppressed_reason_ == NotStreamingReason::kHasCodeCacheBackground);
    return ParkableString(result_->decoded_data.Impl(), std::move(result_->digest));
}

std::unique_ptr<v8::ScriptCompiler::ConsumeCodeCacheTask> BackgroundResourceScriptStreamer::TakeConsumeCodeCacheTask()
{
    CHECK(result_);
...
void BackgroundResourceScriptStreamer::OnResult(std::unique_ptr<Result> result, NotStreamingReason suppressed_reason)
{
    result_ = std::move(result);
    suppressed_reason_ = suppressed_reason;
    CHECK_EQ(!!result_,
        suppressed_reason_ == NotStreamingReason::kInvalid
            || (features::kBackgroundCodeCacheDecoderStart.Get() && suppressed_reason_ == NotStreamingReason::kHasCodeCacheBackground));
}
