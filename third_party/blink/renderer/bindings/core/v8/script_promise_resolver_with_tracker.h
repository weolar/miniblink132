...
class CORE_EXPORT ScriptPromiseResolverWithTracker : public GarbageCollected<ScriptPromiseResolverWithTracker<ResultEnumType, IDLResolvedType>>,
...
public:
    ...
    // max_latency_bucket).
    ScriptPromiseResolverWithTracker(ScriptState* script_state, std::string metric_name_prefix, base::TimeDelta timeout_interval,
        base::TimeDelta min_latency_bucket = base::Milliseconds(1), base::TimeDelta max_latency_bucket = base::Seconds(10), size_t n_buckets = 50)
        ...
        , n_buckets_(n_buckets)
    {
        ...
        resolver_ = MakeGarbageCollected<ScriptPromiseResolver<IDLResolvedType>>(script_state);
        if (timeout_interval.is_positive()) {
            ...
            execution_context->GetTaskRunner(TaskType::kInternalDefault)
                ->PostDelayedTask(FROM_HERE, WTF::BindOnce(&ScriptPromiseResolverWithTracker::RecordResult, WrapPersistent(this), ResultEnumType::kTimedOut),
                    timeout_interval);
        }
    }

    ScriptPromiseResolverWithTracker(const ScriptPromiseResolverWithTracker&) = delete;
    ScriptPromiseResolverWithTracker& operator=(const ScriptPromiseResolverWithTracker&) = delete;
    ~ScriptPromiseResolverWithTracker() override = default;

    template <typename T> void Resolve(T value, ResultEnumType result = ResultEnumType::kOk)
    {
        RecordResultAndLatency(result);
        resolver_->Resolve(value);
    }

    template <typename IDLRejectType, typename T> void Reject(T value, ResultEnumType result)
    {
        RecordResultAndLatency(result);
    ...
    void RecordAndThrowDOMException(ExceptionState& exception_state, DOMExceptionCode exception_code, const String& message, ResultEnumType result)
    {
        RecordResultAndLatency(result);
    ...
    void RecordAndThrowTypeError(ExceptionState& exception_state, const String& message, ResultEnumType result)
    {
        RecordResultAndLatency(result);
    ...
    void Resolve()
    {
        Resolve(ToV8UndefinedGenerator());
    }
    ...
    void RecordResult(ResultEnumType result)
    {
        ...
        base::UmaHistogramEnumeration(metric_name_prefix_ + "." + result_suffix_, result);
    }
    ...
    void RecordLatency()
    {
        ...
        base::UmaHistogramCustomTimes(metric_name_prefix_ + ".Latency", elapsed, min_latency_bucket_, max_latency_bucket_, n_buckets_);
    }
