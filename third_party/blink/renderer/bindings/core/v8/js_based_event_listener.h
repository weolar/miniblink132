...
public:
    ...
    bool IsJSBasedEventListener() const override
    {
        return true;
    }
    virtual bool IsJSEventListener() const
    {
        return false;
    }
    virtual bool IsJSEventHandler() const
    {
        return false;
    }
...
protected:
    ...
    // returns nullptr.
    virtual ScriptState* GetScriptStateOrReportError(const char* operation) const = 0;
...
private:
    ...
    virtual void InvokeInternal(EventTarget&, Event&, v8::Local<v8::Value> js_event) = 0;
};

template <> struct DowncastTraits<JSBasedEventListener> {
