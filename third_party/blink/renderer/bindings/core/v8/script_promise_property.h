...
class ScriptPromiseProperty final : public GarbageCollected<ScriptPromiseProperty<IDLResolvedType, IDLRejectedType>> {
public:
    enum State {
        kPending,
        kResolved,
    ...
    ScriptPromise<IDLResolvedType> Promise(DOMWrapperWorld& world)
    {
        ...
        for (auto& promise : promises_) {
            if (promise.second == script_state) {
                return ScriptPromise<IDLResolvedType>::FromV8Promise(script_state->GetIsolate(), promise.first.Get(script_state->GetIsolate()));
            }
        }

        auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLResolvedType>>(script_state);
        ...
        promises_.emplace_back(TraceWrapperV8Reference<v8::Promise>(script_state->GetIsolate(), promise), script_state);
        return ScriptPromise<IDLResolvedType>::FromV8Promise(script_state->GetIsolate(), promise);
    }

    template <typename PassResolvedType> void Resolve(PassResolvedType value)
    {
    ...
    template <typename PassRejectedType> void Reject(PassRejectedType value)
    {
        CHECK(!ScriptForbiddenScope::IsScriptForbidden());
...
private:
    using MemberResolvedType = AddMemberIfNeeded<typename IDLTypeToBlinkImplType<IDLResolvedType>::type>;
    using MemberRejectedType = AddMemberIfNeeded<typename IDLTypeToBlinkImplType<IDLRejectedType>::type>;

    template <typename T> static T DefaultPromiseResultValue()
    {
        return {};
    }
    ...
    HeapVector<std::pair<TraceWrapperV8Reference<v8::Promise>, Member<ScriptState>>> promises_;
