...
struct ImportAttribute {
    ...
    ImportAttribute(const String& key, const String& value, const TextPosition& position)
        : key(key)
        , value(value)
        , position(position)
    {
...
struct CORE_EXPORT ModuleRequest {
    ...
    ModuleRequest(const String& specifier, const TextPosition& position, const Vector<ImportAttribute>& import_attributes)
        : specifier(specifier)
        , position(position)
