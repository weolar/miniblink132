...
ScriptCustomElementDefinition::ScriptCustomElementDefinition(const ScriptCustomElementDefinitionData& data, const CustomElementDescriptor& descriptor)
    : CustomElementDefinition(*data.registry_, descriptor, std::move(data.observed_attributes_), data.disabled_features_,
        data.is_form_associated_ ? FormAssociationFlag::kYes : FormAssociationFlag::kNo)
...
HTMLElement* ScriptCustomElementDefinition::CreateAutonomousCustomElementSync(Document& document, const QualifiedName& tag_name)
{
    ...
    // 6.1.3. through 6.1.9.
    CheckConstructorResult(element, document, tag_name, PassThroughException(isolate));
    if (try_catch.HasCaught()) {
        // 6.1."If any of these subsubsteps threw an exception".1
        // Report the exception.
        V8ScriptRunner::ReportException(isolate, try_catch.Exception());
...
bool ScriptCustomElementDefinition::RunConstructor(Element& element)
{
    ...
    if (DisableShadow() && element.GetShadowRoot()) {
        v8::Local<v8::Value> exception = V8ThrowDOMException::CreateOrEmpty(script_state_->GetIsolate(), DOMExceptionCode::kNotSupportedError,
    ...
    if (result != &element) {
        ...
        v8::Local<v8::Value> exception = V8ThrowException::CreateTypeError(script_state_->GetIsolate(), message);
        if (!exception.IsEmpty())
            V8ScriptRunner::ReportException(isolate, exception);
        return false;
    }
...
void ScriptCustomElementDefinition::RunAdoptedCallback(Element& element, Document& old_owner, Document& new_owner)
{
    ...
    Element& element, const QualifiedName& name, const AtomicString& old_value, const AtomicString& new_value)
{
    ...
    attribute_changed_callback_->InvokeAndReportException(&element, name.LocalName(), old_value, new_value, name.NamespaceURI());
}

void ScriptCustomElementDefinition::RunFormAssociatedCallback(Element& element, HTMLFormElement* nullable_form)
{
...
void ScriptCustomElementDefinition::RunFormDisabledCallback(Element& element, bool is_disabled)
{
...
void ScriptCustomElementDefinition::RunFormStateRestoreCallback(Element& element, const V8ControlValue* value, const String& mode)
{
    if (!form_state_restore_callback_)
        return;
    form_state_restore_callback_->InvokeAndReportException(&element, value, V8FormStateRestoreMode::Create(mode).value());
}
