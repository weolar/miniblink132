...
class TestResolveFunction : public ThenCallable<IDLString, TestResolveFunction> {
public:
    explicit TestResolveFunction(String* value)
        : value_(value)
    {
    }
...
public:
    explicit TestRejectFunction(String* value)
        : value_(value)
    {
    }
    ...
    void React(ScriptState* script_state, ScriptValue value)
    {
        ...
        *value_ = ToCoreString(script_state->GetIsolate(), value.V8Value()->ToString(script_state->GetContext()).ToLocalChecked());
    }
...
public:
    ...
    void PerformMicrotaskCheckpoint()
    {
        ...
        GetScriptState()->GetContext()->GetMicrotaskQueue()->PerformCheckpoint(GetIsolate());
    }
};
...
TEST_F(ScriptPromiseResolverBaseTest, resolve)
{
    ...
    {
        ScriptState::Scope scope(GetScriptState());
        resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLString>>(GetScriptState());
    ...
    {
        ScriptState::Scope scope(GetScriptState());
        promise.React(GetScriptState(), MakeGarbageCollected<TestResolveFunction>(&on_fulfilled), MakeGarbageCollected<TestRejectFunction>(&on_rejected));
    }
...
TEST_F(ScriptPromiseResolverBaseTest, OverrideScriptStateToCurrentContext)
{
    ...
    ScriptPromise<IDLString> promise;
    {
        ...
        resolver = MakeGarbageCollected<ScriptPromiseResolver<IDLString>>(main_script_state);
        promise = resolver->Promise();
    }
    ...
    {
        ScriptState::Scope scope(main_script_state);
        promise.React(main_script_state, MakeGarbageCollected<TestResolveFunction>(&on_fulfilled), MakeGarbageCollected<TestRejectFunction>(&on_rejected));
    }
