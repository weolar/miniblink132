...
struct ProfilerNodeFrameHashTraits : HashTraits<const v8::CpuProfileNode*> {
    static bool Equal(const v8::CpuProfileNode* a, const v8::CpuProfileNode* b)
    {
        return a->GetFunctionName() == b->GetFunctionName() && a->GetScriptResourceName() == b->GetScriptResourceName()
            && a->GetLineNumber() == b->GetLineNumber() && a->GetColumnNumber() == b->GetColumnNumber();
    }

    static unsigned GetHash(const v8::CpuProfileNode* node)
    {
        return WTF::GetHash(node->GetFunctionNameStr()) ^ WTF::GetHash(node->GetScriptResourceNameStr()) ^ WTF::GetHash(node->GetLineNumber())
...
class CORE_EXPORT ProfilerTraceBuilder final : public GarbageCollected<ProfilerTraceBuilder> {
public:
    static ProfilerTrace* FromProfile(ScriptState*, const v8::CpuProfile* profile, const SecurityOrigin* allowed_origin, base::TimeTicks time_origin);

    explicit ProfilerTraceBuilder(ScriptState*, const SecurityOrigin* allowed_origin, base::TimeTicks time_origin);
...
private:
    ...
    // and coalescing.
    void AddSample(const v8::CpuProfileNode* node, base::TimeTicks timestamp, const v8::StateTag state, const v8::EmbedderStateTag embedder_state);
    ...
    inline std::optional<V8ProfilerMarker> BlinkStateToMarker(const v8::EmbedderStateTag state_tag, const v8::StateTag fallback_state)
    {
    ...
    HashMap<const v8::CpuProfileNode*, wtf_size_t, ProfilerNodeStackHashTraits> node_to_stack_map_;
    HashMap<const v8::CpuProfileNode*, wtf_size_t, ProfilerNodeFrameHashTraits> node_to_frame_map_;
