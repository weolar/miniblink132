...
// too.
class ShadowRealmLifetimeController : public GarbageCollected<ShadowRealmLifetimeController>, public ContextLifecycleObserver {
public:
    explicit ShadowRealmLifetimeController(
        ExecutionContext* initiator_execution_context, ShadowRealmGlobalScope* shadow_realm_global_scope, ScriptState* shadow_realm_script_state)
        : is_initiator_worker_or_worklet_(initiator_execution_context->IsWorkerOrWorkletGlobalScope())
...
v8::MaybeLocal<v8::Context> OnCreateShadowRealmV8Context(v8::Local<v8::Context> initiator_context)
{
    ExecutionContext* initiator_execution_context = ExecutionContext::From(initiator_context);
    DCHECK(initiator_execution_context);
    v8::Isolate* isolate = initiator_context->GetIsolate();
    DOMWrapperWorld* world = DOMWrapperWorld::Create(isolate, DOMWrapperWorld::WorldType::kShadowRealm);
    CHECK(world); // Not yet run out of the world id.

    // Create a new ShadowRealmGlobalScope.
    ShadowRealmGlobalScope* shadow_realm_global_scope = MakeGarbageCollected<ShadowRealmGlobalScope>(initiator_execution_context);
    const WrapperTypeInfo* wrapper_type_info = shadow_realm_global_scope->GetWrapperTypeInfo();

    // Create a new v8::Context.
    v8::ExtensionConfiguration* extension_configuration = nullptr;
    v8::Local<v8::ObjectTemplate> global_template = wrapper_type_info->GetV8ClassTemplate(isolate, *world).As<v8::FunctionTemplate>()->InstanceTemplate();
    v8::Local<v8::Object> global_proxy; // Will request a new global proxy.
    v8::Local<v8::Context> context = v8::Context::New(isolate, extension_configuration, global_template, global_proxy, v8::DeserializeInternalFieldsCallback(),
        initiator_execution_context->GetMicrotaskQueue());
    context->UseDefaultSecurityToken();

    // Associate the Blink object with the v8::Context.
    ScriptState* script_state = ScriptState::Create(context, world, shadow_realm_global_scope);
    ...
    V8DOMWrapper::SetNativeInfo(isolate, global_object, shadow_realm_global_scope);

    // Install context-dependent properties.
    std::ignore = script_state->PerContextData()->ConstructorForType(wrapper_type_info);

    // Make the initiator execution context the owner of the
    // ShadowRealmGlobalScope and the ScriptState.
    MakeGarbageCollected<ShadowRealmLifetimeController>(initiator_execution_context, shadow_realm_global_scope, script_state);
