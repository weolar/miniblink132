...
public:
    ...
    V8ObjectBuilder& AddNumberOrNull(const StringView& name, std::optional<double> value);
    ...
    V8ObjectBuilder& AddStringOrNull(const StringView& name, const StringView& value);
    V8ObjectBuilder& AddV8Value(const StringView& name, v8::Local<v8::Value>);

    template <typename T>
        requires std::derived_from<T, bindings::DictionaryBase> || std::derived_from<T, ScriptWrappable>
    ...
    template <typename T, typename Container> V8ObjectBuilder& AddVector(const StringView& name, const Container& value)
    {
    ...
    v8::Local<v8::Object> V8Value() const
    {
        return object_;
    }
