...
public:
    ...
    ReferrerScriptInfo(const KURL& base_url, network::mojom::CredentialsMode credentials_mode, const String& nonce, ParserDisposition parser_state,
    ...
    ReferrerScriptInfo(const KURL& base_url, const ScriptFetchOptions& options)
        : ReferrerScriptInfo(base_url, options.CredentialsMode(), options.Nonce(), options.ParserState(), options.GetReferrerPolicy())
    {
    }

    static ReferrerScriptInfo FromV8HostDefinedOptions(v8::Local<v8::Context>, v8::Local<v8::Data>, const KURL& script_origin_resource_name);
    v8::Local<v8::Data> ToV8HostDefinedOptions(v8::Isolate*, const KURL& script_origin_resource_name) const;
    ...
    ParserDisposition ParserState() const
    {
...
private:
    ...
    const network::mojom::CredentialsMode credentials_mode_ = network::mojom::CredentialsMode::kSameOrigin;
    ...
    // https://html.spec.whatwg.org/C/#default-classic-script-fetch-options
    const network::mojom::ReferrerPolicy referrer_policy_ = network::mojom::ReferrerPolicy::kDefault;
};
