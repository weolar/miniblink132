...
ScriptPromiseResolverBase::ScriptPromiseResolverBase(ScriptState* script_state, const ExceptionContext& exception_context)
    : resolver_(script_state->GetIsolate(), v8::Promise::Resolver::New(script_state->GetContext()).ToLocalChecked())
    , state_(kPending)
...
void ScriptPromiseResolverBase::Dispose()
{
    ...
    const bool is_properly_detached = state_ == kDone || !is_promise_called_ || !GetScriptState()->ContextIsValid() || !GetExecutionContext()
...
//         static crash_reporter::CrashKeyString<1024> trace_key("scriptpromiseresolver-trace");
//         crash_reporter::SetCrashKeyStringToStackTrace(&trace_key, create_stack_trace_);
        DCHECK(false) << "ScriptPromiseResolverBase was not properly detached; created at\n" << create_stack_trace_.ToString();
    }
}
#endif
...
void ScriptPromiseResolverBase::RejectWithDOMException(DOMExceptionCode exception_code, const String& message)
{
    ScriptState::Scope scope(script_state_.Get());
    v8::Isolate* isolate = script_state_->GetIsolate();
    auto exception = V8ThrowDOMException::CreateOrDie(isolate, exception_code, message);
    ApplyContextToException(script_state_, exception, exception_context_);
    Reject(exception);
}

void ScriptPromiseResolverBase::RejectWithSecurityError(const String& sanitized_message, const String& unsanitized_message)
{
    ScriptState::Scope scope(script_state_.Get());
    v8::Isolate* isolate = script_state_->GetIsolate();
    auto exception = V8ThrowDOMException::CreateOrDie(isolate, DOMExceptionCode::kSecurityError, sanitized_message, unsanitized_message);
    ApplyContextToException(script_state_, exception, exception_context_);
    Reject(exception);
}
...
{
    return ExceptionMessages::AddContextToMessage(context.GetType(), context.GetClassName(), context.GetPropertyName(), message);
}

void ScriptPromiseResolverBase::RejectWithTypeError(const String& message)
{
    ScriptState::Scope scope(script_state_.Get());
    Reject(V8ThrowException::CreateTypeError(script_state_->GetIsolate(), AddContext(exception_context_, message)));
}

void ScriptPromiseResolverBase::RejectWithRangeError(const String& message)
{
    ScriptState::Scope scope(script_state_.Get());
    Reject(V8ThrowException::CreateRangeError(script_state_->GetIsolate(), AddContext(exception_context_, message)));
}

void ScriptPromiseResolverBase::RejectWithWasmCompileError(const String& message)
{
    ScriptState::Scope scope(script_state_.Get());
    Reject(V8ThrowException::CreateWasmCompileError(script_state_->GetIsolate(), AddContext(exception_context_, message)));
}
...
void ScriptPromiseResolverBase::ResolveOrRejectImmediately()
{
    ...
    probe::WillHandlePromise(
        GetExecutionContext(), script_state_, state_ == kResolving, exception_context_.GetClassName(), exception_context_.GetPropertyName(), script_url_);

    v8::MicrotasksScope microtasks_scope(script_state_->GetIsolate(), ToMicrotaskQueue(script_state_), v8::MicrotasksScope::kDoNotRunMicrotasks);
    auto resolver = resolver_.Get(script_state_->GetIsolate());
    if (state_ == kResolving) {
        std::ignore = resolver->Resolve(script_state_->GetContext(), value_.Get(script_state_->GetIsolate()));
    } else {
        DCHECK_EQ(state_, kRejecting);
        std::ignore = resolver->Reject(script_state_->GetContext(), value_.Get(script_state_->GetIsolate()));
    }
...
void ScriptPromiseResolverBase::ScheduleResolveOrReject()
{
    GetExecutionContext()
        ...
        ->PostTask(FROM_HERE, WTF::BindOnce(&ScriptPromiseResolverBase::ResolveOrRejectDeferred, WrapPersistent(this)));
}
