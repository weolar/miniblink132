...
// static
JSEventHandler* JSEventHandler::CreateOrNull(v8::Local<v8::Value> value, HandlerType type)
{
    if (!value->IsObject())
        return nullptr;

    return MakeGarbageCollected<JSEventHandler>(V8EventHandlerNonNull::Create(value.As<v8::Object>()), type);
}
...
void JSEventHandler::SetCompiledHandler(ScriptState* incumbent_script_state, v8::Local<v8::Function> listener)
{
    ...
    v8::Context::BackupIncumbentScope backup_incumbent_scope(incumbent_script_state->GetContext());
    event_handler_ = V8EventHandlerNonNull::Create(listener);
}

// https://html.spec.whatwg.org/C/#the-event-handler-processing-algorithm
void JSEventHandler::InvokeInternal(EventTarget& event_target, Event& event, v8::Local<v8::Value> js_event)
{
    ...
    v8::Local<v8::Value> listener_value = GetListenerObject(*event.currentTarget());
    ...
    const bool special_error_event_handling
        = IsA<ErrorEvent>(event) && event.type() == event_type_names::kError && event.currentTarget()->IsWindowOrWorkerGlobalScope();
    ...
    if (special_error_event_handling) {
        ...
        if (error_attribute.IsEmpty() || error_event->target()->InterfaceName() == event_target_names::kWorker) {
            error_attribute = ScriptValue::CreateNull(isolate);
        }
        arguments = { ScriptValue(isolate, ToV8Traits<IDLString>::ToV8(script_state_of_listener, error_event->message())),
            ScriptValue(isolate, ToV8Traits<IDLString>::ToV8(script_state_of_listener, error_event->filename())),
            ScriptValue(isolate, ToV8Traits<IDLUnsignedLong>::ToV8(script_state_of_listener, error_event->lineno())),
            ScriptValue(isolate, ToV8Traits<IDLUnsignedLong>::ToV8(script_state_of_listener, error_event->colno())), error_attribute };
    } else {
        arguments.push_back(ScriptValue(isolate, js_event));
    }

    if (!event_handler_->IsRunnableOrThrowException(event.ShouldDispatchEvenWhenExecutionContextIsPaused() ? V8EventHandlerNonNull::IgnorePause::kIgnore
                                                                                                           : V8EventHandlerNonNull::IgnorePause::kDontIgnore)) {
        return;
    }
    ScriptValue result;
    if (!event_handler_->InvokeWithoutRunnabilityCheck(event.currentTarget(), arguments).To(&result) || isolate->IsExecutionTerminating())
        return;
    ...
    if (IsOnBeforeUnloadEventHandler()) {
        event_handler_->EvaluateAsPartOfCallback(WTF::BindOnce(
            [](v8::Local<v8::Value>& v8_return_value, String& result_for_beforeunload, ScriptState* script_state) {
                v8::Isolate* isolate = script_state->GetIsolate();
                v8::TryCatch try_catch(isolate);
                String result = NativeValueTraits<IDLNullable<IDLString>>::NativeValue(isolate, v8_return_value, PassThroughException(isolate));
    ...
    const bool is_beforeunload_event = before_unload_event && event.type() == event_type_names::kBeforeunload;
    if (is_beforeunload_event) {
        ...
        if (special_error_event_handling && v8_return_value->IsBoolean() && v8_return_value.As<v8::Boolean>()->Value())
            event.preventDefault();
        else if (!special_error_event_handling && v8_return_value->IsBoolean() && !v8_return_value.As<v8::Boolean>()->Value())
            event.preventDefault();
    }
}
