...
ScriptCacheConsumer::ScriptCacheConsumer(
    v8::Isolate* isolate, scoped_refptr<CachedMetadata> cached_metadata, const String& script_url_string, uint64_t script_resource_identifier)
    : isolate_(isolate)
...
{
    consume_task_.reset(v8::ScriptCompiler::StartConsumingCodeCache(isolate_, V8CodeCache::CreateCachedData(cached_metadata_)));

    if (consume_task_) {
        TRACE_EVENT_WITH_FLOW1("v8," TRACE_DISABLED_BY_DEFAULT("v8.compile"), "v8.deserializeOnBackground.start", TRACE_ID_LOCAL(this),
        ...
        worker_pool::PostTask(FROM_HERE, WTF::CrossThreadBindOnce(&ScriptCacheConsumer::RunTaskOffThread, WrapCrossThreadWeakPersistent(this)));
    } else {
...
ScriptCacheConsumer::ScriptCacheConsumer(v8::Isolate* isolate, scoped_refptr<CachedMetadata> cached_metadata,
    std::unique_ptr<v8::ScriptCompiler::ConsumeCodeCacheTask> completed_consume_task, const String& script_url_string, uint64_t script_resource_identifier)
    : isolate_(isolate)
...
ScriptCacheConsumer::State ScriptCacheConsumer::AdvanceState(State new_state_bit)
{
    // We should only be setting a single state bit at a time.
    DCHECK(new_state_bit == State::kConsumeFinished || new_state_bit == State::kClientReady || new_state_bit == State::kMergeDoneOrNotNeededBit
...
void ScriptCacheConsumer::RunTaskOffThread()
{
    ...
    TRACE_EVENT_WITH_FLOW1("v8,devtools.timeline," TRACE_DISABLED_BY_DEFAULT("v8.compile"), "v8.deserializeOnBackground", TRACE_ID_LOCAL(this),
        TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "data",
        [&](perfetto::TracedValue context) { inspector_deserialize_script_event::Data(std::move(context), script_resource_identifier_, script_url_string_); });
...
void ScriptCacheConsumer::PostFinishCallbackTask()
{
    ...
    PostCrossThreadTask(
        *finish_callback_task_runner_, FROM_HERE, WTF::CrossThreadBindOnce(&ScriptCacheConsumer::CallFinishCallback, WrapCrossThreadWeakPersistent(this)));
}
...
void ScriptCacheConsumer::RunMergeTaskOffThread()
{
    ...
    TRACE_EVENT_WITH_FLOW1("v8,devtools.timeline," TRACE_DISABLED_BY_DEFAULT("v8.compile"), "v8.deserializeOnBackground.mergeWithExistingScript",
        TRACE_ID_LOCAL(this), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "data",
        [&](perfetto::TracedValue context) { inspector_deserialize_script_event::Data(std::move(context), script_resource_identifier_, script_url_string_); });
...
void ScriptCacheConsumer::NotifyClientWaiting(
    ScriptCacheConsumerClient* client, ClassicScript* classic_script, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    DCHECK(WTF::IsMainThread());
...

    {
        ...
        if (consume_task_) {
            consume_task_->SourceTextAvailable(isolate_, V8String(isolate_, source_text), origin);
        }
    }
    ...
    if (new_state == State::kFinishedAndReady) {
        ...
        if (consume_task_ && consume_task_->ShouldMergeWithExistingScript()) {
            TRACE_EVENT_WITH_FLOW1("v8," TRACE_DISABLED_BY_DEFAULT("v8.compile"), "v8.deserializeOnBackground.startMergeWithExistingScript",
                TRACE_ID_LOCAL(this), TRACE_EVENT_FLAG_FLOW_OUT, "data", [&](perfetto::TracedValue context) {
                    inspector_deserialize_script_event::Data(std::move(context), script_resource_identifier_, script_url_string_);
                });

            worker_pool::PostTask(FROM_HERE, WTF::CrossThreadBindOnce(&ScriptCacheConsumer::RunMergeTaskOffThread, WrapCrossThreadWeakPersistent(this)));
        } else {
            AdvanceState(State::kMergeDoneOrNotNeededBit);
            CallFinishCallback();
        }
...
void ScriptCacheConsumer::CallFinishCallback()
{
    ...
    TRACE_EVENT_WITH_FLOW1("v8," TRACE_DISABLED_BY_DEFAULT("v8.compile"), finish_trace_name_, TRACE_ID_LOCAL(this), TRACE_EVENT_FLAG_FLOW_IN, "data",
        [&](perfetto::TracedValue context) { inspector_deserialize_script_event::Data(std::move(context), script_resource_identifier_, script_url_string_); });
