...
JSEventHandlerForContentAttribute* JSEventHandlerForContentAttribute::Create(
    ExecutionContext* context, const QualifiedName& name, const AtomicString& value, HandlerType type)
{
    ...
    return MakeGarbageCollected<JSEventHandlerForContentAttribute>(context, name, value, type);
}
...
JSEventHandlerForContentAttribute::JSEventHandlerForContentAttribute(
    ...
    , position_(To<LocalDOMWindow>(context)->GetScriptController().EventHandlerPosition())
    , isolate_(context->GetIsolate())
{
}

v8::Local<v8::Value> JSEventHandlerForContentAttribute::GetListenerObject(EventTarget& event_target)
{
...
v8::Local<v8::Value> JSEventHandlerForContentAttribute::GetCompiledHandler(EventTarget& event_target)
{
    // Do not compile the same code twice.
    DCHECK(!did_compile_);
    did_compile_ = true;

    ExecutionContext* execution_context_of_event_target = event_target.GetExecutionContext();
    if (!execution_context_of_event_target)
        return v8::Null(GetIsolate());

    v8::Local<v8::Context> v8_context_of_event_target = ToV8Context(execution_context_of_event_target, GetWorld());
    if (v8_context_of_event_target.IsEmpty())
        return v8::Null(GetIsolate());

    ScriptState* script_state_of_event_target = ScriptState::From(GetIsolate(), v8_context_of_event_target);
    ...
    if (!document->AllowInlineEventHandler(element, this, source_url_, position_.line_))
        return v8::Null(GetIsolate());
    ...
    if (auto* html_element = DynamicTo<HTMLElement>(element)) {
        ...
        // object.
        if (html_element->IsHTMLBodyElement() || html_element->IsHTMLFrameSetElement()) {
    ...
    if (IsOnErrorEventHandler() && window) {
        ...
        parameter_list[parameter_list_size++] = V8String(isolate, element && element->IsSVGElement() ? "evt" : "event");
    ...
    if (element) {
        scopes[scopes_size++] = ToV8Traits<Document>::ToV8(script_state_of_event_target, document).As<v8::Object>();
    }
    if (form_owner) {
        scopes[scopes_size++] = ToV8Traits<HTMLFormElement>::ToV8(script_state_of_event_target, form_owner).As<v8::Object>();
    }
    if (element) {
        scopes[scopes_size++] = ToV8Traits<Element>::ToV8(script_state_of_event_target, element).As<v8::Object>();
    }
    DCHECK_LE(scopes_size, std::size(scopes));

    v8::ScriptOrigin origin(V8String(isolate, source_url_), position_.line_.ZeroBasedInt(), position_.column_.ZeroBasedInt(),
    ...
    v8::Local<v8::Function> compiled_function;
    {
        ...
        v8::MaybeLocal<v8::Function> maybe_result
            = v8::ScriptCompiler::CompileFunction(v8_context_of_event_target, &source, parameter_list_size, parameter_list, scopes_size, scopes);
...
std::unique_ptr<SourceLocation> JSEventHandlerForContentAttribute::GetSourceLocation(EventTarget& target)
{
    auto source_location = JSEventHandler::GetSourceLocation(target);
    if (source_location)
        return source_location;
    // Fallback to uncompiled source info.
    return std::make_unique<SourceLocation>(source_url_, String(), position_.line_.ZeroBasedInt(), position_.column_.ZeroBasedInt(), nullptr);
}
