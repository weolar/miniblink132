...
public:
    ...
    public:
        Result(SegmentedBuffer raw_data, String decoded_data, std::unique_ptr<ParkableStringImpl::SecureDigest> digest);
        ~Result() = default;
    ...
    static ScriptDecoderPtr Create(std::unique_ptr<TextResourceDecoder> decoder, scoped_refptr<base::SequencedTaskRunner> client_task_runner);
...
private:
    ...
    ScriptDecoder(std::unique_ptr<TextResourceDecoder> decoder, scoped_refptr<base::SequencedTaskRunner> client_task_runner);
...
using DataPipeScriptDecoderPtr = std::unique_ptr<DataPipeScriptDecoder, DataPipeScriptDecoderDeleter>;
...
// feature is enabled.
class CORE_EXPORT DataPipeScriptDecoder final : public mojo::DataPipeDrainer::Client {
public:
    using OnDecodeFinishedCallback = CrossThreadOnceFunction<void(ScriptDecoder::Result)>;

    // Creates a DataPipeScriptDecoder.
    static DataPipeScriptDecoderPtr Create(std::unique_ptr<TextResourceDecoder> decoder, scoped_refptr<base::SequencedTaskRunner> client_task_runner,
...
private:
    ...
    DataPipeScriptDecoder(std::unique_ptr<TextResourceDecoder> decoder, scoped_refptr<base::SequencedTaskRunner> client_task_runner,
        OnDecodeFinishedCallback on_decode_finished_callback);
    void Delete() const;
...
using ScriptDecoderWithClientPtr = std::unique_ptr<ScriptDecoderWithClient, ScriptDecoderWithClientDeleter>;
...
public:
    ...
    static ScriptDecoderWithClientPtr Create(ResponseBodyLoaderClient* response_body_loader_client, std::unique_ptr<TextResourceDecoder> decoder,
...
private:
    ...
    ScriptDecoderWithClient(ResponseBodyLoaderClient* response_body_loader_client, std::unique_ptr<TextResourceDecoder> decoder,
        scoped_refptr<base::SequencedTaskRunner> client_task_runner);
