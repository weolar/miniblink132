...
implemented).
*/
class V8CrowdsourcedCompileHintsProducer : public GarbageCollected<V8CrowdsourcedCompileHintsProducer> {
public:
    explicit V8CrowdsourcedCompileHintsProducer(Page* page);

    V8CrowdsourcedCompileHintsProducer(const V8CrowdsourcedCompileHintsProducer&) = delete;
    V8CrowdsourcedCompileHintsProducer& operator=(const V8CrowdsourcedCompileHintsProducer&) = delete;

    // Notifies V8CrowdsourcedCompileHintsProducer of the existence of `script`
    void RecordScript(Frame* frame, ExecutionContext* execution_context, const v8::Local<v8::Script> script, ScriptState* script_state);
...
private:
    ...
    HeapVector<v8::TracedReference<v8::CompileHintsCollector>> compile_hints_collectors_;
