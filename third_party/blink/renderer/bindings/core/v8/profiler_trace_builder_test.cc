...
TEST(ProfilerTraceBuilderTest, AddVMStateMarker)
{
    ...
    ProfilerTraceBuilder* builder = MakeGarbageCollected<ProfilerTraceBuilder>(script_state, nullptr, base::TimeTicks::Now());

    base::TimeTicks sample_ticks = base::TimeTicks::Now();
    builder->AddSample(nullptr, sample_ticks, v8::StateTag::GC, v8::EmbedderStateTag::EMPTY);
...
TEST(ProfilerTraceBuilderTest, AddEmbedderStateMarker)
{
    ...
    base::TimeTicks sample_ticks = base::TimeTicks::Now();
    builder->AddSample(nullptr, sample_ticks, v8::StateTag::IDLE, static_cast<v8::EmbedderStateTag>(BlinkState::LAYOUT));
    builder->AddSample(nullptr, sample_ticks, v8::StateTag::IDLE, static_cast<v8::EmbedderStateTag>(BlinkState::STYLE));
    builder->AddSample(nullptr, sample_ticks, v8::StateTag::IDLE, static_cast<v8::EmbedderStateTag>(BlinkState::PAINT));
    auto* profiler_trace = builder->GetTrace();
    const auto& samples = profiler_trace->samples();
