...
class AsyncIterationSourceBase::CallableCommon : public ThenCallable<IDLAny, CallableCommon, IDLAny> {
public:
...
class AsyncIterationSourceBase::RunNextStepsCallable final : public AsyncIterationSourceBase::CallableCommon {
public:
    ...
    {
        return ScriptValue(script_state->GetIsolate(), iteration_source_->RunNextSteps(script_state).V8Promise());
    }
};

class AsyncIterationSourceBase::RunFulfillStepsCallable final : public AsyncIterationSourceBase::CallableCommon {
public:
    ...
    ScriptValue React(ScriptState* script_state, ScriptValue iter_result_object_or_undefined) override
    {
        return iteration_source_->RunFulfillSteps(script_state, iter_result_object_or_undefined);
    }
};

class AsyncIterationSourceBase::RunRejectStepsCallable final : public AsyncIterationSourceBase::CallableCommon {
public:
...
class AsyncIterationSourceBase::RunReturnStepsCallable final : public AsyncIterationSourceBase::CallableCommon {
public:
    explicit RunReturnStepsCallable(AsyncIterationSourceBase* iteration_source, ScriptValue value)
        : AsyncIterationSourceBase::CallableCommon(iteration_source)
    ...
    ScriptValue React(ScriptState* script_state, ScriptValue) override
    {
        return ScriptValue(script_state->GetIsolate(), iteration_source_->RunReturnSteps(script_state, value_).V8Promise());
    }
...
class AsyncIterationSourceBase::RunReturnFulfillStepsCallable final : public AsyncIterationSourceBase::CallableCommon {
public:
    explicit RunReturnFulfillStepsCallable(AsyncIterationSourceBase* iteration_source, ScriptValue value)
        : AsyncIterationSourceBase::CallableCommon(iteration_source)
        , value_(std::move(value))
    {
    }
...
AsyncIterationSourceBase::AsyncIterationSourceBase(ScriptState* script_state, Kind kind)
    : AsyncIteratorBase::IterationSourceBase(kind)
    , script_state_(script_state)
    , on_settled_function_(MakeGarbageCollected<RunNextStepsCallable>(this))
    , on_fulfilled_function_(MakeGarbageCollected<RunFulfillStepsCallable>(this))
    , on_rejected_function_(MakeGarbageCollected<RunRejectStepsCallable>(this))
{
}

v8::Local<v8::Promise> AsyncIterationSourceBase::Next(ScriptState* script_state, ExceptionState& exception_state)
{
    if (!ongoing_promise_.IsEmpty()) {
        ...
        ongoing_promise_ = ongoing_promise_.Then(script_state, on_settled_function_.Get(), on_settled_function_.Get());
    } else {
        // step 11. Otherwise:
...
v8::Local<v8::Promise> AsyncIterationSourceBase::Return(ScriptState* script_state, v8::Local<v8::Value> value, ExceptionState& exception_state)
{
    ScriptPromise<IDLAny> return_steps_promise;
    if (!ongoing_promise_.IsEmpty()) {
        // step 10. If ongoingPromise is not null, then:
        // step 10.2. Let onSettled be CreateBuiltinFunction(returnSteps, << >>).
        auto* on_settled = MakeGarbageCollected<RunReturnStepsCallable>(this, ScriptValue(script_state->GetIsolate(), value));
        // step 10.3. Perform PerformPromiseThen(ongoingPromise, onSettled,
        //     onSettled, afterOngoingPromiseCapability).
        // step 11.4. Set object's ongoing promise to
        //     afterOngoingPromiseCapability.[[Promise]].
        ongoing_promise_ = ongoing_promise_.Then(script_state, on_settled, on_settled);
    } else {
        // step 11. Otherwise:
        // step 11.1. Set object's ongoing promise to the result of
        //     running returnSteps.
        ongoing_promise_ = RunReturnSteps(script_state, ScriptValue(script_state->GetIsolate(), value));
    }
    // step 13. Let onFulfilled be CreateBuiltinFunction(fulfillSteps, << >>).
    auto* on_fulfilled = MakeGarbageCollected<RunReturnFulfillStepsCallable>(this, ScriptValue(script_state->GetIsolate(), value));
...
ScriptPromise<IDLAny> AsyncIterationSourceBase::RunNextSteps(ScriptState* script_state)
{
    if (is_finished_) {
        ...
        return ToResolvedPromise<IDLAny>(script_state, ESCreateIterResultObject(script_state, true, v8::Undefined(script_state->GetIsolate())));
    }
    ...
    pending_promise_resolver_ = MakeGarbageCollected<ScriptPromiseResolver<IDLAny>>(script_state);
    auto promise = pending_promise_resolver_->Promise();
    GetNextIterationResult();
    return promise.Then(script_state, on_fulfilled_function_.Get(), on_rejected_function_.Get());
}

// step 8.5. Let fulfillSteps be the following steps, given next:
ScriptValue AsyncIterationSourceBase::RunFulfillSteps(ScriptState* script_state, ScriptValue iter_result_object_or_undefined)
{
    ...
    DCHECK(iter_result_object_or_undefined.IsObject() || iter_result_object_or_undefined.IsUndefined());
    ...
    if (iter_result_object_or_undefined.IsUndefined()) {
        ...
        return ScriptValue(script_state->GetIsolate(), ESCreateIterResultObject(script_state, true, v8::Undefined(script_state->GetIsolate())));
    }
...
ScriptValue AsyncIterationSourceBase::RunRejectSteps(ScriptState* script_state, ScriptValue reason)
{
    ...
    V8ThrowException::ThrowException(script_state->GetIsolate(), reason.V8Value());
    return {};
}

// step 8. Let returnSteps be the following steps:
ScriptPromise<IDLAny> AsyncIterationSourceBase::RunReturnSteps(ScriptState* script_state, ScriptValue value)
{
    if (is_finished_) {
        // step 8.2. If object's is finished is true, then:
        ...
        return ToResolvedPromise<IDLAny>(script_state, ESCreateIterResultObject(script_state, true, value.V8Value()));
    }
...
ScriptValue AsyncIterationSourceBase::RunReturnFulfillSteps(ScriptState* script_state, ScriptValue value)
{
    // step 13.1. Return CreateIterResultObject(value, true).
    return ScriptValue(script_state->GetIsolate(), ESCreateIterResultObject(script_state, true, value.V8Value()));
}
