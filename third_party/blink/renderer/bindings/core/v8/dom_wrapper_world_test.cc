...
std::pair<Persistent<HeapVector<Member<DOMWrapperWorld>>>, int32_t> CollectInitialWorlds(v8::Isolate* isolate)
{
    auto* initial_worlds = MakeGarbageCollected<HeapVector<Member<DOMWrapperWorld>>>();
    ...
    for (const auto& world : *initial_worlds) {
        if (world->IsIsolatedWorld()) {
            used_isolated_world_id = std::max(used_isolated_world_id, world->GetWorldId());
        }
    }
...
TEST(DOMWrapperWorldTest, IsolatedWorlds)
{
    ...
    const auto [initial_worlds, used_isolated_world_id] = CollectInitialWorlds(isolate);
    ASSERT_TRUE(DOMWrapperWorld::IsIsolatedWorldId(used_isolated_world_id + 1));

    const auto* isolated_world1 = DOMWrapperWorld::EnsureIsolatedWorld(isolate, used_isolated_world_id + 1);
    const auto* isolated_world2 = DOMWrapperWorld::EnsureIsolatedWorld(isolate, used_isolated_world_id + 2);
...
TEST(DOMWrapperWorldTest, ExplicitDispose)
{
    ...
    auto* worker_world1 = DOMWrapperWorld::Create(isolate, DOMWrapperWorld::WorldType::kWorkerOrWorklet);
    auto* worker_world2 = DOMWrapperWorld::Create(isolate, DOMWrapperWorld::WorldType::kWorkerOrWorklet);
    auto* worker_world3 = DOMWrapperWorld::Create(isolate, DOMWrapperWorld::WorldType::kWorkerOrWorklet);
    ...
    EXPECT_TRUE(worker_world_ids.insert(worker_world1->GetWorldId()).is_new_entry);
    EXPECT_TRUE(worker_world_ids.insert(worker_world2->GetWorldId()).is_new_entry);
    EXPECT_TRUE(worker_world_ids.insert(worker_world3->GetWorldId()).is_new_entry);
    EXPECT_TRUE(DOMWrapperWorld::NonMainWorldsExistInMainThread());
...
void WorkerThreadFunc(WorkerBackingThread* thread, scoped_refptr<base::SingleThreadTaskRunner> main_thread_task_runner, CrossThreadOnceClosure quit_closure)
{
    thread->InitializeOnBackingThread(WorkerBackingThreadStartupData::CreateDefault());
    ...
    PostCrossThreadTask(*main_thread_task_runner, FROM_HERE, CrossThreadBindOnce(std::move(quit_closure)));
}
...
TEST(DOMWrapperWorldTest, NonMainThreadWorlds)
{
    ...
    std::unique_ptr<WorkerBackingThread> thread
        = std::make_unique<WorkerBackingThread>(ThreadCreationParams(ThreadType::kTestThread).SetThreadNameForTest("DOMWrapperWorld test thread"));
    scoped_refptr<base::SingleThreadTaskRunner> main_thread_task_runner = blink::scheduler::GetSingleThreadTaskRunnerForTesting();
    PostCrossThreadTask(*thread->BackingThread().GetTaskRunner(), FROM_HERE,
        CrossThreadBindOnce(
            &WorkerThreadFunc, CrossThreadUnretained(thread.get()), std::move(main_thread_task_runner), CrossThreadOnceClosure(loop.QuitClosure())));
    loop.Run();
