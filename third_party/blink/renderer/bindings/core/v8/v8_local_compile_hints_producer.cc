...
V8LocalCompileHintsProducer::V8LocalCompileHintsProducer(LocalFrame* frame)
    : frame_(frame)
{
    should_generate_data_ = frame->IsMainFrame() && base::FeatureList::IsEnabled(features::kLocalCompileHints);
}

void V8LocalCompileHintsProducer::RecordScript(ExecutionContext* execution_context, const v8::Local<v8::Script> script, ClassicScript* classic_script)
{
    ...
    compile_hints_collectors_.emplace_back(isolate, script->GetCompileHintsCollector());
...
void V8LocalCompileHintsProducer::GenerateData(bool final_data)
{
    ...
    for (wtf_size_t i = 0; i < cache_handlers_.size(); ++i) {
        ...
        if (V8CodeCache::HasCodeCache(cache_handler, CachedMetadataHandler::kAllowUnchecked)) {
            ...
            base::UmaHistogramBoolean(kLocalCompileHintsObsoletedByCodeCacheHistogram, true);
            return;
        }
        base::UmaHistogramBoolean(kLocalCompileHintsObsoletedByCodeCacheHistogram, false);

        V8CodeCache::RecordCacheSetStatistics(
            final_data ? V8CodeCache::SetMetadataType::kLocalCompileHintsAtInteractive : V8CodeCache::SetMetadataType::kLocalCompileHintsAtFMP);

        v8::Local<v8::CompileHintsCollector> compile_hints_collector = compile_hints_collectors_.at(i).Get(isolate);
        std::vector<int> compile_hints = compile_hints_collector->GetCompileHints(isolate);

        uint64_t timestamp = V8CodeCache::GetTimestamp();
        std::unique_ptr<v8::ScriptCompiler::CachedData> data(CreateCompileHintsCachedDataForScript(compile_hints, timestamp));

        cache_handler->ClearCachedMetadata(code_cache_host, CachedMetadataHandler::kClearLocally);
        cache_handler->SetCachedMetadata(code_cache_host, V8CodeCache::TagForCompileHints(cache_handler), data->data, data->length);
    }
    if (final_data) {
        cache_handlers_.clear();
        compile_hints_collectors_.clear();
        base::UmaHistogramEnumeration(kLocalCompileHintsGeneratedHistogram, LocalCompileHintsGenerated::kFinal);

    } else {
        base::UmaHistogramEnumeration(kLocalCompileHintsGeneratedHistogram, LocalCompileHintsGenerated::kNonFinal);
    }
}

v8::ScriptCompiler::CachedData* V8LocalCompileHintsProducer::CreateCompileHintsCachedDataForScript(std::vector<int>& compile_hints, uint64_t prefix)
{
    ...
    return new v8::ScriptCompiler::CachedData(std::move(data).leak().data(), static_cast<int>(data_size), v8::ScriptCompiler::CachedData::BufferOwned);
}
