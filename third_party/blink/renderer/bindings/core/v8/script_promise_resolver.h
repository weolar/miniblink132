...
template <typename IDLResolvedType> class ScriptPromiseResolver;
...
class CORE_EXPORT ScriptPromiseResolverBase : public GarbageCollected<ScriptPromiseResolverBase> {
#if DCHECK_IS_ON()
...
public:
    ...
    ScriptPromiseResolverBase& operator=(const ScriptPromiseResolverBase&) = delete;
    ...
    template <typename IDLType, typename BlinkType> void Reject(BlinkType value)
    {
    ...
    void Reject()
    {
        Reject<IDLUndefined>(ToV8UndefinedGenerator());
    }
    ...
    void RejectWithDOMException(DOMExceptionCode exception_code, const String& message);
    // Reject with DOMException with SECURITY_ERR.
    void RejectWithSecurityError(const String& sanitized_message, const String& unsanitized_message);
    ...
    template <typename IDLResolvedType> ScriptPromiseResolver<IDLResolvedType>* DowncastTo()
    {
#if DCHECK_IS_ON()
        DCHECK_EQ(runtime_type_id_, GetTypeId<ScriptPromiseResolver<IDLResolvedType>>());
#endif
...
private:
    template <typename IDLResolvedType> friend class ScriptPromiseResolver;
    ...
    template <typename IDLType, typename BlinkType> void ResolveOrReject(BlinkType value)
    {
        ...
        // resolveOrReject shouldn't be called inside a ScriptForbiddenScope.
        {
            ...
            v8::MicrotasksScope microtasks_scope(isolate, ToMicrotaskQueue(script_state_.Get()), v8::MicrotasksScope::kDoNotRunMicrotasks);
            value_.Reset(isolate, ToV8Traits<IDLType>::ToV8(script_state_, std::move(value)));
        }
        NotifyResolveOrReject();
    }

    template <ResolutionState new_state> bool PrepareToResolveOrReject()
    {
...
template <typename IDLResolvedType> class ScriptPromiseResolver final : public ScriptPromiseResolverBase {
public:
    explicit ScriptPromiseResolver(ScriptState* script_state)
        : ScriptPromiseResolver(script_state, ExceptionContext(v8::ExceptionContext::kUnknown, nullptr, nullptr))
    {
    }

    ScriptPromiseResolver(ScriptState* script_state, const ExceptionContext& context)
    ...
    // this function.
    template <typename BlinkType> void Resolve(BlinkType value)
    {
    ...
    template <typename BlinkType> void ResolveOverridingToCurrentContext(BlinkType value)
    {
    ...
    void Resolve(BlinkType value)
    {
        ...
        ResolveOrReject<IDLResolvedType, IDLResolvedType*>(MakeGarbageCollected<IDLResolvedType>(value));
    }
    ...
    void Resolve()
    {
        ...
        ResolveOrReject<IDLUndefined, ToV8UndefinedGenerator>(ToV8UndefinedGenerator());
    }
    ...
    {
        return ScriptPromise<IDLResolvedType>(script_state_->GetIsolate(), V8Promise());
    }
    ...
    base::OnceCallback<void(Args...)> WrapCallbackInScriptScope(base::OnceCallback<void(ScriptPromiseResolver<IDLResolvedType>*, Args...)> callback)
    {
        return WTF::BindOnce(
            [](ScriptPromiseResolver<IDLResolvedType>* resolver, base::OnceCallback<void(ScriptPromiseResolver<IDLResolvedType>*, Args...)> callback,
                Args... args) {
