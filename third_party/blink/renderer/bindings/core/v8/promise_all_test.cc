...
struct ResolveUndefined final : public ThenCallable<IDLUndefined, ResolveUndefined> {
public:
...
struct ResolveStrings final : public ThenCallable<IDLSequence<IDLString>, ResolveStrings> {
public:
...
struct ResolveDocuments final : public ThenCallable<IDLSequence<Document>, ResolveDocuments> {
public:
...
TEST(PromiseAllTest, Reject)
{
    ...
    HeapVector<ScriptPromise<IDLUndefined>> promises;
    promises.push_back(ToResolvedUndefinedPromise(script_state));
    promises.push_back(ScriptPromise<IDLUndefined>::Reject(script_state, V8String(scope.GetIsolate(), "world")));
...
TEST(PromiseAllTest, RejectTypeMismatch)
{
    ...
    promises.push_back(ScriptPromise<Document>::FromV8Value(script_state, ToV8Traits<LocalDOMWindow>::ToV8(script_state, &scope.GetWindow())));
    ...
    EXPECT_FALSE(reject->rejected_value.IsEmpty());
    EXPECT_EQ("TypeError: Failed to convert value to 'Document'.", ToString(scope.GetContext(), reject->rejected_value));
}
