...
void InstallFunctionHolderTemplate(v8::Isolate*, const DOMWrapperWorld&, v8::Local<v8::Template> interface_template)
{
}
...
public:
    static v8::Local<v8::Function> Create(ScriptState* script_state, ScriptFunction* function)
    {
        CHECK(function);
        ...
        return v8::Function::New(script_state->GetContext(), CallCallback, holder->Wrap(script_state), function->Length(), v8::ConstructorBehavior::kThrow)
            .ToLocalChecked();
    }

    static void CallCallback(const v8::FunctionCallbackInfo<v8::Value>& args)
    {
        RUNTIME_CALL_TIMER_SCOPE_DISABLED_BY_DEFAULT(args.GetIsolate(), "Blink_CallCallback");
    ...
    explicit FunctionHolder(ScriptFunction* function)
        : function_(function)
    {
    }
...
const WrapperTypeInfo& FunctionHolder::wrapper_type_info_ = function_holder_info;
...
void ScriptFunction::CallRaw(ScriptState* script_state, const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ScriptValue result = Call(script_state, ScriptValue(script_state->GetIsolate(), args[0]));
    bindings::V8SetReturnValue(args, result);
}

v8::Local<v8::Function> ScriptFunction::ToV8Function(ScriptState* script_state)
{
