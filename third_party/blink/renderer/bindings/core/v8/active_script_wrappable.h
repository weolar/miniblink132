...
template <typename T> class ActiveScriptWrappable : public ActiveScriptWrappableBase {
public:
...
protected:
    ...
    {
        return IsContextDestroyedForActiveScriptWrappable(static_cast<const T*>(this)->GetExecutionContext());
    }
};
...
// ASW behavior.
template <typename T> class LazyActiveScriptWrappable : public ActiveScriptWrappableBase {
public:
    LazyActiveScriptWrappable(const LazyActiveScriptWrappable&) = delete;
    LazyActiveScriptWrappable& operator=(const LazyActiveScriptWrappable&) = delete;
...
CORE_EXPORT bool IsContextDestroyedForActiveScriptWrappable(const ExecutionContext* execution_context);

} // namespace blink

namespace cppgc {

template <typename T, typename Unused> struct PostConstructionCallbackTrait;
template <typename T> struct PostConstructionCallbackTrait<T, std::void_t<decltype(std::declval<T>().ActiveScriptWrappableBaseConstructed())>> {
