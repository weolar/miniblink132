...
private:
    ...
    static v8::Local<v8::Value> ToWorldSafeValue(
        ScriptState* target_script_state, const TraceWrapperV8Reference<v8::Value>& v8_reference, const DOMWrapperWorld& v8_reference_world);
    ...
    // across worlds.
    static void MaybeCheckCreationContextWorld(const DOMWrapperWorld& world, v8::Local<v8::Value> value);

    template <typename V8Type> friend class WorldSafeV8Reference;
};
...
// context.
template <typename V8Type> class WorldSafeV8Reference final {
    DISALLOW_NEW();
...
public:
    ...
    WorldSafeV8Reference(v8::Isolate* isolate, v8::Local<V8Type> value)
    {
        ...
        if (isolate->InContext()) {
            ...
            WorldSafeV8ReferenceInternal::MaybeCheckCreationContextWorld(*world_.Get(), value);
        } else if (value->IsObject()) {
            ScriptState* script_state = ScriptState::ForRelevantRealm(isolate, value.template As<v8::Object>());
    ...
    {
        CHECK(world_);
        return WorldSafeV8ReferenceInternal::ToWorldSafeValue(target_script_state, v8_reference_, *world_.Get()).template As<V8Type>();
    }
    ...
    void Set(v8::Isolate* isolate, v8::Local<V8Type> new_value)
    {
        ...
        WorldSafeV8ReferenceInternal::MaybeCheckCreationContextWorld(new_world, new_value);
    ...
    WorldSafeV8Reference& operator=(const WorldSafeV8Reference<V8Type>& other) = default;
...
template <typename V8Type> struct VectorTraits<blink::WorldSafeV8Reference<V8Type>> : VectorTraitsBase<blink::WorldSafeV8Reference<V8Type>> {
    STATIC_ONLY(VectorTraits);

    static constexpr bool kCanInitializeWithMemset = VectorTraits<blink::TraceWrapperV8Reference<V8Type>>::kCanInitializeWithMemset
        && VectorTraits<scoped_refptr<const blink::DOMWrapperWorld>>::kCanInitializeWithMemset;
    static constexpr bool kCanClearUnusedSlotsWithMemset = VectorTraits<blink::TraceWrapperV8Reference<V8Type>>::kCanClearUnusedSlotsWithMemset
        && VectorTraits<scoped_refptr<const blink::DOMWrapperWorld>>::kCanClearUnusedSlotsWithMemset;
    static constexpr bool kCanCopyWithMemcpy = VectorTraits<blink::TraceWrapperV8Reference<V8Type>>::kCanCopyWithMemcpy
        && VectorTraits<scoped_refptr<const blink::DOMWrapperWorld>>::kCanCopyWithMemcpy;
    static constexpr bool kCanMoveWithMemcpy = VectorTraits<blink::TraceWrapperV8Reference<V8Type>>::kCanMoveWithMemcpy
        && VectorTraits<scoped_refptr<const blink::DOMWrapperWorld>>::kCanMoveWithMemcpy;

    static constexpr bool kCanTraceConcurrently = VectorTraits<blink::TraceWrapperV8Reference<V8Type>>::kCanTraceConcurrently;
};
