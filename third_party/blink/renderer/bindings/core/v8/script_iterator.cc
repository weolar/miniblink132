...
class AsyncIteratorCloseFulfillFunction final : public ThenCallable<IDLAny, AsyncIteratorCloseFulfillFunction, IDLAny> {
public:
    explicit AsyncIteratorCloseFulfillFunction(const ExceptionContext& exception_context)
    {
    ...
    ScriptValue React(ScriptState* script_state, ScriptValue value)
    {
        ...
        if (!value.V8Value()->IsObject()) {
            V8ThrowException::ThrowTypeError(script_state->GetIsolate(), "Expected return() to resolve to an Object.");
            return ScriptValue();
        }
    ...
    {
        ThenCallable<IDLAny, AsyncIteratorCloseFulfillFunction, IDLAny>::Trace(visitor);
    }
};
...
// static
ScriptIterator ScriptIterator::FromIterable(v8::Isolate* isolate, v8::Local<v8::Object> iterable, ExceptionState& exception_state, Kind kind)
{
    ...
    if (kind == Kind::kAsync) {
        ...
        if (!iterable->Get(current_context, v8::Symbol::GetAsyncIterator(isolate)).ToLocal(&method)) {
        ...
        if (!iterable->Get(current_context, v8::Symbol::GetIterator(isolate)).ToLocal(&method)) {
            DCHECK(rethrow_scope.HasCaught());
            return ScriptIterator();
        }
    ...
    if (!V8ScriptRunner::CallFunction(method.As<v8::Function>(), ToExecutionContext(current_context), iterable, 0, nullptr, isolate).ToLocal(&iterator)) {
        DCHECK(rethrow_scope.HasCaught());
        return ScriptIterator();
    }
    ...
    if (!iterator.As<v8::Object>()->Get(current_context, V8AtomicString(isolate, "next")).ToLocal(&next_method)) {
        return ScriptIterator();
    }
...
ScriptIterator::ScriptIterator(v8::Isolate* isolate, v8::Local<v8::Object> iterator, v8::Local<v8::Value> next_method, Kind kind)
    : isolate_(isolate)
...
bool ScriptIterator::Next(ExecutionContext* execution_context, ExceptionState& exception_state)
{
    DCHECK(!IsNull());
    ...
    if (!V8ScriptRunner::CallFunction(next_method.As<v8::Function>(), execution_context, iterator_.Get(script_state), 0, nullptr, isolate_)
             .ToLocal(&next_return_value)) {
        done_ = true;
        return false;
    }
    if (!next_return_value->IsObject()) {
        exception_state.ThrowTypeError("Expected iterator.next() to return an Object.");
        done_ = true;
        return false;
    }
    v8::Local<v8::Object> next_return_value_object = next_return_value.As<v8::Object>();
    ...
    if (kind_ == Kind::kAsync) {
        ...
        v8::MaybeLocal<v8::Value> maybe_value = next_return_value_object->Get(context, value_key_);
        value_ = WorldSafeV8Reference(isolate_, maybe_value.FromMaybe(v8::Local<v8::Value>()));
...
ScriptValue ScriptIterator::CloseSync(ScriptState* script_state, ExceptionState& exception_state, v8::Local<v8::Value> reason)
{
    ...
    if (!iterator->Get(current_context, V8AtomicString(isolate_, "return")).ToLocal(&return_method)) {
        DCHECK(rethrow_scope.HasCaught());
    ...
    if (!V8ScriptRunner::CallFunction(
            return_method.As<v8::Function>(), ExecutionContext::From(script_state), iterator, reason.IsEmpty() ? 0 : 1, &reason, isolate_)
...
ScriptPromise<IDLAny> ScriptIterator::CloseAsync(ScriptState* script_state, const ExceptionContext& exception_context, v8::Local<v8::Value> reason)
{
    ...
    if (!iterator->Get(current_context, V8AtomicString(isolate_, "return")).ToLocal(&return_method)) {
        ...
        ScriptPromise<IDLAny> rejected_promise = ScriptPromise<IDLAny>::Reject(script_state, try_catch.Exception());
        try_catch.Reset();
    ...
    // 3. If IsCallable(func) is false, throw a TypeError exception.
    if (!return_method->IsFunction()) {
        V8ThrowException::CreateTypeError(script_state->GetIsolate(), "return() function must be callable");
        ScriptPromise<IDLAny> rejected_promise = ScriptPromise<IDLAny>::Reject(script_state, try_catch.Exception());
        try_catch.Reset();
        return rejected_promise;
    }
    ...
    ScriptPromise<IDLAny> return_promise = ToResolvedPromise<IDLAny>(script_state, return_result);
    ...
    // documentation).
    auto* on_fulfilled = MakeGarbageCollected<AsyncIteratorCloseFulfillFunction>(exception_context);
