...
template <typename T> class V8CTypeTraits;

template <> class V8CTypeTraits<IDLBoolean> {
public:
    static constexpr v8::CTypeInfo kCTypeInfo = v8::CTypeInfoBuilder<bool>().Build();
};

template <typename T, bindings::IDLFloatingPointNumberConvMode mode> class V8CTypeTraits<IDLFloatingPointNumberTypeBase<T, mode>> {
...
public:
    static constexpr v8::CTypeInfo kCTypeInfo = v8::CTypeInfoBuilder<T, GetCTypeInfoFlags()>().Build();
};

template <typename T, bindings::IDLIntegerConvMode mode> class V8CTypeTraits<IDLIntegerTypeBase<T, mode>> {
    static constexpr v8::CTypeInfo::Flags GetCTypeInfoFlags()
    {
        switch (mode) {
