...
class TestResourceClient final : public GarbageCollected<TestResourceClient>, public ResourceClient {
public:
...
class NoopLoaderFactory final : public ResourceFetcher::LoaderFactory {
    std::unique_ptr<URLLoader> CreateURLLoader(const network::ResourceRequest& request, const ResourceLoaderOptions& options,
        ...
        BackForwardCacheLoaderHelper*, const std::optional<base::UnguessableToken>& service_worker_race_network_request_token,
    ...
    public:
        explicit NoopURLLoader(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
            : task_runner_(std::move(task_runner))
        {
        }
        ~NoopURLLoader() override = default;
        void LoadSynchronously(std::unique_ptr<network::ResourceRequest> request, scoped_refptr<const SecurityOrigin> top_frame_origin, bool download_to_blob,
            bool no_mime_sniffing, base::TimeDelta timeout_interval, URLLoaderClient*, WebURLResponse&, std::optional<WebURLError>&,
            scoped_refptr<SharedBuffer>&, int64_t& encoded_data_length, uint64_t& encoded_body_length, scoped_refptr<BlobDataHandle>& downloaded_blob,
            std::unique_ptr<blink::ResourceLoadInfoNotifierWrapper> resource_load_info_notifier_wrapper) override
        {
            NOTREACHED();
        }
        void LoadAsynchronously(std::unique_ptr<network::ResourceRequest> request, scoped_refptr<const SecurityOrigin> top_frame_origin, bool no_mime_sniffing,
            std::unique_ptr<blink::ResourceLoadInfoNotifierWrapper> resource_load_info_notifier_wrapper, CodeCacheHost* code_cache_host,
...
void AppendDataToDataPipe(std::string_view data, mojo::ScopedDataPipeProducerHandle& producer_handle)
{
...
public:
    ScriptStreamingTest()
        : url_(String("http://streaming-test.example.com/foo" + base::NumberToString(url_counter_++)))
    {
    }
    ...
    void Init(v8::Isolate* isolate, bool use_response_http_scheme = true)
    {
        ...
        scoped_refptr<base::SingleThreadTaskRunner> task_runner = scheduler::GetSingleThreadTaskRunnerForTesting();
        auto* fetcher = MakeGarbageCollected<ResourceFetcher>(ResourceFetcherInit(properties->MakeDetachable(), context, task_runner, task_runner,
            MakeGarbageCollected<NoopLoaderFactory>(), MakeGarbageCollected<MockContextLifecycleNotifier>(), nullptr /* back_forward_cache_loader_helper */));

        EXPECT_EQ(mojo::CreateDataPipe(kDataPipeSize, producer_handle_, consumer_handle_), MOJO_RESULT_OK);

        ResourceRequest request(url_);
        request.SetRequestContext(mojom::blink::RequestContextType::SCRIPT);

        resource_client_ = MakeGarbageCollected<TestResourceClient>(run_loop_.QuitClosure());
        FetchParameters params = FetchParameters::CreateForTest(std::move(request));
        constexpr v8_compile_hints::V8CrowdsourcedCompileHintsProducer* kNoCompileHintsProducer = nullptr;
        constexpr v8_compile_hints::V8CrowdsourcedCompileHintsConsumer* kNoCompileHintsConsumer = nullptr;
        resource_ = ScriptResource::Fetch(params, fetcher, resource_client_, isolate, ScriptResource::kAllowStreaming, kNoCompileHintsProducer,
        ...
        resource_->Loader()->DidReceiveResponse(WrappedResourceResponse(response), std::move(consumer_handle_),
...
protected:
    ...
    void Cancel()
    {
        resource_->Loader()->Cancel();
    }

    void RunUntilResourceLoaded()
    {
        run_loop_.Run();
    }
...
TEST_F(ScriptStreamingTest, CompilingStreamedScript)
{
    ...
    std::tie(compile_options, produce_cache_options, no_cache_reason) = V8CodeCache::GetCompileOptions(mojom::blink::V8CacheOptions::kDefault, *classic_script);
    EXPECT_TRUE(V8ScriptRunner::CompileScript(
        scope.GetScriptState(), *classic_script, classic_script->CreateScriptOrigin(scope.GetIsolate()), compile_options, no_cache_reason)
                    .ToLocal(&script));
...
TEST_F(ScriptStreamingTest, SuppressingStreaming)
{
    ...
    cache_handler->SetCachedMetadata(/*code_cache_host*/ nullptr, V8CodeCache::TagForCodeCache(cache_handler), reinterpret_cast<const uint8_t*>("X"), 1);
...
TEST_F(ScriptStreamingTest, ConsumeLocalCompileHints)
{
    ...
    features.InitWithFeatureStates({ { features::kLocalCompileHints, true }, { features::kProduceCompileHints2, false } });
    ...
    std::unique_ptr<v8::ScriptCompiler::CachedData> cached_data(
        v8_compile_hints::V8LocalCompileHintsProducer::CreateCompileHintsCachedDataForScript(compile_hints, timestamp));

    cache_handler->SetCachedMetadata(
        /*code_cache_host*/ nullptr, V8CodeCache::TagForCompileHints(cache_handler), cached_data->data, cached_data->length);

    // Checks for debugging failures in this test.
    EXPECT_TRUE(V8CodeCache::HasCompileHints(cache_handler, CachedMetadataHandler::kAllowUnchecked));
    ...
    ScriptStreamer* script_streamer = std::get<0>(ScriptStreamer::TakeFrom(resource_, mojom::blink::ScriptType::kClassic));
    ResourceScriptStreamer* resource_script_streamer = reinterpret_cast<ResourceScriptStreamer*>(script_streamer);
    EXPECT_TRUE(resource_script_streamer);

    v8_compile_hints::V8LocalCompileHintsConsumer* local_compile_hints_consumer = resource_script_streamer->GetV8LocalCompileHintsConsumerForTest();
...
TEST_F(ScriptStreamingTest, EncodingChanges)
{
    ...
    AppendData("function foo() { var foob\xec\x92\x81r = 13; return foob\xec\x92\x81r; "
               "} foo();");
...
TEST_F(ScriptStreamingTest, EncodingFromBOM)
{
    ...
    // for \uc481.
    AppendData("\xef\xbb\xbf function foo() { var foob\xec\x92\x81r = 13; return "
...
TEST_F(ScriptStreamingTest, ResourceSetRevalidatingRequest)
{
    ...
    // "kRevalidate".
    EXPECT_FALSE(resource_->HasStreamer());
    EXPECT_EQ(resource_->NoStreamerReason(), ScriptStreamer::NotStreamingReason::kRevalidate);
}

class InlineScriptStreamingTest : public ScriptStreamingTest,
                                  public ::testing::WithParamInterface<std::pair<bool /* 16 bit source */, v8::ScriptCompiler::CompileOptions>> { };
...
TEST_P(InlineScriptStreamingTest, InlineScript)
{
    ...
    if (GetParam().first)
        source.Ensure16Bit();
    auto streamer = base::MakeRefCounted<BackgroundInlineScriptStreamer>(scope.GetIsolate(), source, GetParam().second);
    worker_pool::PostTask(FROM_HERE, {}, CrossThreadBindOnce(&BackgroundInlineScriptStreamer::Run, streamer));

    ClassicScript* classic_script
        = ClassicScript::Create(source, KURL(), KURL(), ScriptFetchOptions(), ScriptSourceLocationType::kUnknown, SanitizeScriptErrors::kSanitize, nullptr,
            TextPosition::MinimumPosition(), ScriptStreamer::NotStreamingReason::kInvalid, InlineScriptStreamer::From(streamer));

    DummyPageHolder holder;
    ScriptEvaluationResult result
        = classic_script->RunScriptAndReturnValue(holder.GetFrame().DomWindow(), ExecuteScriptPolicy::kExecuteScriptWhenScriptsDisabled);
    EXPECT_EQ(result.GetResultType(), ScriptEvaluationResult::ResultType::kSuccess);
    EXPECT_EQ(5, result.GetSuccessValue()->Int32Value(scope.GetContext()).FromJust());
}
...
INSTANTIATE_TEST_SUITE_P(All, InlineScriptStreamingTest,
    testing::ValuesIn({ std::make_pair(true, v8::ScriptCompiler::CompileOptions::kNoCompileOptions),
        std::make_pair(false, v8::ScriptCompiler::CompileOptions::kNoCompileOptions), std::make_pair(true, v8::ScriptCompiler::CompileOptions::kEagerCompile),
        std::make_pair(false, v8::ScriptCompiler::CompileOptions::kEagerCompile) }));
...
// \xef\xbb\xbf is the UTF-8 byte order mark. \xec\x92\x81 are the raw bytes
// for \uc481.
const char kScriptWithBOM[] = "\xef\xbb\xbf function foo() { var foob\xec\x92\x81r = 13; return "
                              "foob\xec\x92\x81r; } foo();";
...
public:
    ...
    std::unique_ptr<URLLoader> CreateURLLoader(const network::ResourceRequest& request, const ResourceLoaderOptions& options,
        ...
        bool is_from_origin_dirty_style_sheet) override
    {
        return std::make_unique<DummyURLLoader>(this, std::move(freezable_task_runner));
    }
    CodeCacheHost* GetCodeCacheHost() override
    {
        return nullptr;
    ...
    std::unique_ptr<BackgroundResponseProcessorFactory> TakeBackgroundResponseProcessorFactory()
    {
...
private:
    ...
    public:
        explicit DummyURLLoader(DummyLoaderFactory* factory, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
            : factory_(factory)
            , task_runner_(std::move(task_runner))
        {
        }
        ...
        bool CanHandleResponseOnBackground() override
        {
            return true;
        }
        void SetBackgroundResponseProcessorFactory(std::unique_ptr<BackgroundResponseProcessorFactory> background_response_processor_factory) override
        {
            factory_->background_response_processor_factory_ = std::move(background_response_processor_factory);
        }
        scoped_refptr<base::SingleThreadTaskRunner> GetTaskRunnerForBodyLoader() override
        {
            return task_runner_;
    ...
    std::unique_ptr<BackgroundResponseProcessorFactory> background_response_processor_factory_;
};

class DummyBackgroundResponseProcessorClient : public BackgroundResponseProcessor::Client {
public:
    DummyBackgroundResponseProcessorClient()
        : main_thread_task_runner_(scheduler::GetSingleThreadTaskRunnerForTesting())
    {
    }
    ...
    void DidFinishBackgroundResponseProcessor(
        network::mojom::URLResponseHeadPtr head, BackgroundResponseProcessor::BodyVariant body, std::optional<mojo_base::BigBuffer> cached_metadata) override
    {
        head_ = std::move(head);
        body_ = std::move(body);
    ...
    void CheckResultOfFinishCallback(base::span<const char> expected_body, std::optional<base::span<const uint8_t>> expected_cached_metadata)
    {
        EXPECT_TRUE(head_);
        if (absl::holds_alternative<SegmentedBuffer>(body_)) {
            ...
            mojo::ScopedDataPipeConsumerHandle& handle = absl::get<mojo::ScopedDataPipeConsumerHandle>(body_);
            std::string text;
        ...
        if (expected_cached_metadata) {
            EXPECT_THAT(*cached_metadata_, testing::ElementsAreArray(*expected_cached_metadata));
        }
    }
...
public:
    ...
    bool IsServedFromCacheStorage() override
    {
        return false;
    }
};
...
mojo_base::BigBuffer CreateDummyCodeCacheData()
{
    ScriptCachedMetadataHandler* cache_handler
        = MakeGarbageCollected<ScriptCachedMetadataHandler>(UTF8Encoding(), std::make_unique<DummyCachedMetadataSender>());
    uint32_t data_type_id = V8CodeCache::TagForCodeCache(cache_handler);
    cache_handler->SetCachedMetadata(
        /*code_cache_host=*/nullptr, data_type_id, reinterpret_cast<const uint8_t*>("X"), 1);
    scoped_refptr<CachedMetadata> cached_metadata = cache_handler->GetCachedMetadata(data_type_id);
    mojo_base::BigBuffer cached_metadata_buffer = mojo_base::BigBuffer(cached_metadata->SerializedData());
...
mojo_base::BigBuffer CreateDummyTimeStampData()
{
    ...
    cache_handler->SetCachedMetadata(
        /*code_cache_host=*/nullptr, data_type_id, reinterpret_cast<uint8_t*>(&now_ms), sizeof(now_ms));
    scoped_refptr<CachedMetadata> cached_metadata = cache_handler->GetCachedMetadata(data_type_id);
    mojo_base::BigBuffer cached_metadata_buffer = mojo_base::BigBuffer(cached_metadata->SerializedData());
    return cached_metadata_buffer;
}

network::mojom::URLResponseHeadPtr CreateURLResponseHead(const std::string& content_type = "text/javascript")
{
    auto head = network::mojom::URLResponseHead::New();
    head->headers = base::MakeRefCounted<net::HttpResponseHeaders>(
        net::HttpUtil::AssembleRawHeaders(base::StrCat({ "HTTP/1.1 200 OK\n", "Content-Type: ", content_type, "\n\n" })));
    return head;
}
...
public:
    explicit BackgroundResourceScriptStreamerTest(bool enable_background_code_cache_decode_start = false)
        : url_(String("http://streaming-test.example.com/foo" + base::NumberToString(url_counter_++)))
    {
        feature_list_.InitWithFeaturesAndParameters(
            { { features::kBackgroundResourceFetch,
                { { "background-script-response-processor", "true" },
                    { "background-code-cache-decoder-start", enable_background_code_cache_decode_start ? "true" : "false" } } } },
            {});
    }
    ...
    {
        RunInBackgroundThred(base::BindLambdaForTesting([&]() { background_response_processor_.reset(); }));
    }

protected:
    void Init(v8::Isolate* isolate, bool is_module_script = false, std::optional<WTF::TextEncoding> charset = std::nullopt,
        v8_compile_hints::V8CrowdsourcedCompileHintsConsumer* v8_compile_hints_consumer = nullptr)
    {
        auto* properties = MakeGarbageCollected<TestResourceFetcherProperties>();
        FetchContext* context = MakeGarbageCollected<MockFetchContext>();
        scoped_refptr<base::SingleThreadTaskRunner> main_thread_task_runner = scheduler::GetSingleThreadTaskRunnerForTesting();
        DummyLoaderFactory* dummy_loader_factory = MakeGarbageCollected<DummyLoaderFactory>();
        auto* fetcher
            = MakeGarbageCollected<ResourceFetcher>(ResourceFetcherInit(properties->MakeDetachable(), context, main_thread_task_runner, main_thread_task_runner,
                dummy_loader_factory, MakeGarbageCollected<MockContextLifecycleNotifier>(), nullptr /* back_forward_cache_loader_helper */));
        ...
        background_resource_fetch_task_runner_ = base::ThreadPool::CreateSequencedTaskRunner({ base::TaskPriority::USER_BLOCKING });

        RunInBackgroundThred(base::BindLambdaForTesting([&]() {
            std::unique_ptr<BackgroundResponseProcessorFactory> factory = dummy_loader_factory->TakeBackgroundResponseProcessorFactory();
...
protected:
    ...
    {
        base::RunLoop loop;
        background_resource_fetch_task_runner_->PostTask(FROM_HERE, base::BindLambdaForTesting([&]() {
    ...
    void CheckNotStreamingReason(
        ScriptStreamer::NotStreamingReason expected_not_streamed_reason, mojom::blink::ScriptType script_type = mojom::blink::ScriptType::kClassic)
    {
        ScriptStreamer* streamer;
        ScriptStreamer::NotStreamingReason not_streamed_reason;
        std::tie(streamer, not_streamed_reason) = ScriptStreamer::TakeFrom(resource_, script_type);
        EXPECT_EQ(expected_not_streamed_reason, not_streamed_reason);
        EXPECT_EQ(nullptr, streamer);
    }

    void CheckScriptStreamer(mojom::blink::ScriptType script_type = mojom::blink::ScriptType::kClassic)
    {
        ScriptStreamer* streamer;
        ScriptStreamer::NotStreamingReason not_streamed_reason;
        std::tie(streamer, not_streamed_reason) = ScriptStreamer::TakeFrom(resource_, script_type);
        EXPECT_EQ(ScriptStreamer::NotStreamingReason::kInvalid, not_streamed_reason);
    ...
    scoped_refptr<base::SequencedTaskRunner> background_resource_fetch_task_runner_;
...
TEST_F(BackgroundResourceScriptStreamerTest, UnsupportedModuleMimeType)
{
    ...
    RunInBackgroundThred(base::BindLambdaForTesting([&]() {
        ...
        network::mojom::URLResponseHeadPtr head = CreateURLResponseHead("text/plain");
        std::optional<mojo_base::BigBuffer> cached_metadata;
        EXPECT_FALSE(background_response_processor_->MaybeStartProcessingResponse(
            head, consumer_handle_, cached_metadata, background_resource_fetch_task_runner_, &background_response_processor_client_));
        EXPECT_TRUE(head);
    ...
    Finish();
    RunUntilResourceLoaded();
    CheckNotStreamingReason(ScriptStreamer::NotStreamingReason::kNonJavascriptModuleBackground, mojom::blink::ScriptType::kModule);
}
...
TEST_F(BackgroundResourceScriptStreamerTest, HasCodeCache)
{
    ...
    const std::vector<uint8_t> code_cache_data_copy(code_cache_data.data(), code_cache_data.data() + code_cache_data.size());
    RunInBackgroundThred(base::BindLambdaForTesting([&]() {
        ...
        std::optional<mojo_base::BigBuffer> cached_metadata = std::move(code_cache_data);
        EXPECT_FALSE(background_response_processor_->MaybeStartProcessingResponse(
            head, consumer_handle_, cached_metadata, background_resource_fetch_task_runner_, &background_response_processor_client_));
        EXPECT_TRUE(head);
        EXPECT_TRUE(consumer_handle_);
        ASSERT_TRUE(cached_metadata);
        EXPECT_THAT(*cached_metadata, testing::ElementsAreArray(code_cache_data_copy));
    }));
    Finish();
    RunUntilResourceLoaded();
    // When there is a code cache, we should not stream the script.
    CheckNotStreamingReason(ScriptStreamer::NotStreamingReason::kHasCodeCacheBackground);
}

class BackgroundResourceScriptStreamerCodeCacheDecodeStartTest : public BackgroundResourceScriptStreamerTest {
public:
    ...
    ~BackgroundResourceScriptStreamerCodeCacheDecodeStartTest() override = default;
};
...
TEST_F(BackgroundResourceScriptStreamerCodeCacheDecodeStartTest, HasCodeCache)
{
    ...
    background_response_processor_client_.CheckResultOfFinishCallback(
        /*expected_body=*/base::make_span(kLargeEnoughScript, sizeof(kLargeEnoughScript) - 1),
...
TEST_F(BackgroundResourceScriptStreamerTest, HasTimeStampData)
{
    ...
    const std::vector<uint8_t> time_stamp_data_copy(time_stamp_data.data(), time_stamp_data.data() + time_stamp_data.size());
    RunInBackgroundThred(base::BindLambdaForTesting([&]() {
        network::mojom::URLResponseHeadPtr head = CreateURLResponseHead();
        // Set a dummy time stamp data.
        std::optional<mojo_base::BigBuffer> cached_metadata = std::move(time_stamp_data);
        EXPECT_TRUE(background_response_processor_->MaybeStartProcessingResponse(
            head, consumer_handle_, cached_metadata, background_resource_fetch_task_runner_, &background_response_processor_client_));
        EXPECT_FALSE(head);
        EXPECT_FALSE(consumer_handle_);
        ASSERT_TRUE(cached_metadata);
        EXPECT_EQ(cached_metadata->storage_type(), mojo_base::BigBuffer::StorageType::kInvalidBuffer);
    }));
    AppendData(kLargeEnoughScript);
    producer_handle_.reset();
    background_response_processor_client_.WaitUntilFinished();
...
TEST_F(BackgroundResourceScriptStreamerTest, InvalidCachedMetadata)
{
    ...
    RunInBackgroundThred(base::BindLambdaForTesting([&]() {
        ...
        std::optional<mojo_base::BigBuffer> cached_metadata = mojo_base::BigBuffer(base::make_span(kInvalidCachedMetadata));
        EXPECT_TRUE(background_response_processor_->MaybeStartProcessingResponse(
            head, consumer_handle_, cached_metadata, background_resource_fetch_task_runner_, &background_response_processor_client_));
        EXPECT_FALSE(head);
        EXPECT_FALSE(consumer_handle_);
...
TEST_F(BackgroundResourceScriptStreamerTest, SmallScript)
{
    ...
    background_response_processor_client_.WaitUntilFinished();
    background_response_processor_client_.CheckResultOfFinishCallback(
        /*expected_body=*/base::make_span(kTooSmallScript, sizeof(kTooSmallScript) - 1),
        /*expected_cached_metadata=*/std::nullopt);
    Finish();
    RunUntilResourceLoaded();
    // When the script is too small, we should not stream the script.
    CheckNotStreamingReason(ScriptStreamer::NotStreamingReason::kScriptTooSmallBackground);
}
...
TEST_F(BackgroundResourceScriptStreamerTest, EncodingNotSupported)
{
    ...
    Init(scope.GetIsolate(), /*is_module_script=*/false, WTF::TextEncoding("EUC-JP"));
    RunInBackgroundThred(base::BindLambdaForTesting([&]() {
        network::mojom::URLResponseHeadPtr head = CreateURLResponseHead();
        std::optional<mojo_base::BigBuffer> cached_metadata;
        EXPECT_TRUE(background_response_processor_->MaybeStartProcessingResponse(
    ...
    CheckNotStreamingReason(ScriptStreamer::NotStreamingReason::kEncodingNotSupportedBackground);
}
...
TEST_F(BackgroundResourceScriptStreamerTest, EncodingFromBOM)
{
    ...
    background_response_processor_client_.WaitUntilFinished();
    background_response_processor_client_.CheckResultOfFinishCallback(
        /*expected_body=*/base::make_span(kScriptWithBOM, sizeof(kScriptWithBOM) - 1),
        /*expected_cached_metadata=*/std::nullopt);
    Finish();
    RunUntilResourceLoaded();
    // ScriptStreamer must have been created.
...
TEST_F(BackgroundResourceScriptStreamerTest, ScriptTypeMismatch)
{
    ...
    CheckNotStreamingReason(ScriptStreamer::NotStreamingReason::kErrorScriptTypeMismatch, mojom::blink::ScriptType::kClassic);
}
...
TEST_F(BackgroundResourceScriptStreamerTest, CompilingStreamedScriptWithParseError)
{
    V8TestingScope scope;
    Init(scope.GetIsolate());
    RunInBackgroundThred(base::BindLambdaForTesting([&]() {
    ...
    const char kInvalidScript[] = "This is an invalid script which cause a parse error";
    AppendData(kInvalidScript);
    producer_handle_.reset();
    background_response_processor_client_.WaitUntilFinished();
    background_response_processor_client_.CheckResultOfFinishCallback(
        /*expected_body=*/base::make_span(kInvalidScript, sizeof(kInvalidScript) - 1),
        /*expected_cached_metadata=*/std::nullopt);
    Finish();
    RunUntilResourceLoaded();
...
TEST_F(BackgroundResourceScriptStreamerTest, DataPipeReadableAfterProcessorIsDeleted)
{
    V8TestingScope scope;
    Init(scope.GetIsolate());
    RunInBackgroundThred(base::BindLambdaForTesting([&]() {
...
TEST_F(BackgroundResourceScriptStreamerTest, DeletingBackgroundProcessorWhileParsingShouldNotCrash)
{
    V8TestingScope scope;
    v8_compile_hints::V8CrowdsourcedCompileHintsConsumer* v8_compile_hints_consumer
        = MakeGarbageCollected<v8_compile_hints::V8CrowdsourcedCompileHintsConsumer>();
    ...
    Init(scope.GetIsolate(), /*is_module_script=*/false, /*charset=*/std::nullopt, v8_compile_hints_consumer);
    RunInBackgroundThred(base::BindLambdaForTesting([&]() {
        network::mojom::URLResponseHeadPtr head = CreateURLResponseHead();
        std::optional<mojo_base::BigBuffer> cached_metadata;
        EXPECT_TRUE(background_response_processor_->MaybeStartProcessingResponse(
    ...
    std::string comment_line = base::StrCat({ std::string(kDataPipeSize - 1, '/'), "\n" });
    ...
    RunInBackgroundThred(base::BindLambdaForTesting([&]() {
        ...
        const std::string function_line = base::StrCat({ kFunctionScript, std::string(kDataPipeSize - kFunctionScript.size(), '/') });
        MojoResult result = producer_handle_->WriteAllData(base::as_byte_span(function_line));
        EXPECT_EQ(result, MOJO_RESULT_OK);
