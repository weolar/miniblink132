...
public:
    ...
    ByteSpanWithInlineStorage(const ByteSpanWithInlineStorage& r)
    {
        *this = r;
    }
    ...
    void Assign(base::span<const uint8_t> span)
    {
        span_ = span;
    }
    void Assign(v8::MemorySpan</*const*/ uint8_t> span)
    {
        span_ = base::span<const uint8_t>(span.data(), span.size());
    }
    ...
    operator base::span<const uint8_t>() const&
    {
        return span_;
    }
...
template <typename T> base::span<const uint8_t> GetArrayData(v8::Local<T> array)
{
    return base::make_span(reinterpret_cast<const uint8_t*>(array->Data()), array->ByteLength());
}

CORE_EXPORT base::span<const uint8_t> GetViewData(
    v8::Local<v8::ArrayBufferView> view, base::span<uint8_t, ByteSpanWithInlineStorage::kInlineStorageSize> inline_storage);

template <typename T> class SpanWithInlineStorage {
    STACK_ALLOCATED();
...
public:
    ...
    // NOLINTNEXTLINE(google-explicit-constructor)
    operator base::span<const T>() const&
    {
        return as_span();
    }
    ...
    const base::span<const T> as_span() const
    {
        ...
        return base::make_span(reinterpret_cast<const T*>(bytes.data()), bytes.size() / sizeof(T));
    }
...
template <typename T> class SpanOrVector {
    STACK_ALLOCATED();
...
public:
    ...
    void Assign(Vector<T> vec)
    {
        ...
        span_.Assign(base::make_span(reinterpret_cast<const uint8_t*>(vector_.data()), vector_.size() * sizeof(T)));
    }
    v8::MemorySpan<uint8_t> GetInlineStorage()
    {
...
template <typename T> struct TypedArrayElementTraits { };

#define DEFINE_TYPED_ARRAY_ELEMENT_TRAITS(type, func, idl_type)                                                                                                \
    template <> struct TypedArrayElementTraits<type> {                                                                                                         \
...
DEFINE_TYPED_ARRAY_ELEMENT_TRAITS(uint64_t, IsBigUint64Array, IDLUnsignedLongLong);
DEFINE_TYPED_ARRAY_ELEMENT_TRAITS(float, IsFloat32Array, IDLUnrestrictedFloat);
DEFINE_TYPED_ARRAY_ELEMENT_TRAITS(double, IsFloat64Array, IDLUnrestrictedDouble);

template <> struct TypedArrayElementTraits<uint8_t> {
    static bool IsViewOfType(v8::Local<v8::Value> value)
    {
...
constexpr PassAsSpanMarkerBase::Flags operator|(PassAsSpanMarkerBase::Flags a, PassAsSpanMarkerBase::Flags b)
{
    return static_cast<PassAsSpanMarkerBase::Flags>(static_cast<int>(a) | static_cast<int>(b));
}

template <PassAsSpanMarkerBase::Flags flags = PassAsSpanMarkerBase::Flags::kNone, typename T = void> struct PassAsSpan : public PassAsSpanMarkerBase {
    ...
    using ElementType = T;
    using ReturnType = std::conditional_t<allow_sequence, bindings::internal::SpanOrVector<T>,
        std::conditional_t<is_typed, bindings::internal::SpanWithInlineStorage<T>, bindings::internal::ByteSpanWithInlineStorage>>;
};
