...
class CORE_EXPORT ScriptCacheConsumer final : public GarbageCollected<ScriptCacheConsumer> {
public:
    ...
    ScriptCacheConsumer(v8::Isolate* isolate, scoped_refptr<CachedMetadata> cached_metadata,
    ...
    void NotifyClientWaiting(ScriptCacheConsumerClient* client, ClassicScript* classic_script, scoped_refptr<base::SingleThreadTaskRunner> task_runner);
    ...
    v8::ScriptCompiler::ConsumeCodeCacheTask* TakeV8ConsumeTask(CachedMetadata* cached_metadata)
    {
...
private:
    ...
    static_assert((kConsumeFinished & kClientReady) == 0, "kConsumeFinished and kClientReady have to be independent bits");
    static_assert((kConsumeFinished | kClientReady) == kFinishedAndReady, "kFinishedAndReady has to mean kConsumeFinished and kClientReady");
