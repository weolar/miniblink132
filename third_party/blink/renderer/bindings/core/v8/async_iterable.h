...
// value type.
class CORE_EXPORT AsyncIterationSourceBase : public AsyncIteratorBase::IterationSourceBase {
public:
    ~AsyncIterationSourceBase() override = default;

    // https://webidl.spec.whatwg.org/#es-asynchronous-iterator-prototype-object
    v8::Local<v8::Promise> Next(ScriptState* script_state, ExceptionState& exception_state) final;
    v8::Local<v8::Promise> Return(ScriptState* script_state, v8::Local<v8::Value> value, ExceptionState& exception_state) final;
...
private:
    ...
    ScriptValue RunFulfillSteps(ScriptState* script_state, ScriptValue iter_result_object_or_undefined);
    ScriptValue RunRejectSteps(ScriptState* script_state, ScriptValue reason);

    ScriptPromise<IDLAny> RunReturnSteps(ScriptState* script_state, ScriptValue value);
    ScriptValue RunReturnFulfillSteps(ScriptState* script_state, ScriptValue value);
    ...
    template <typename IDLKeyType, typename IDLValueType, typename KeyType, typename ValueType> friend class PairAsyncIterationSource;
    template <typename IDLValueType, typename ValueType> friend class ValueAsyncIterationSource;
};

template <typename IDLKeyType, typename IDLValueType, typename KeyType, typename ValueType> class PairAsyncIterationSource : public AsyncIterationSourceBase {
protected:
    ...
    // promise.
    v8::Local<v8::Value> MakeIterationResult(const KeyType& key, const ValueType& value) const
    {
        ScriptState* script_state = GetScriptState();
        switch (GetKind()) {
        case Kind::kKey: {
            v8::Local<v8::Value> v8_key = ToV8Traits<IDLKeyType>::ToV8(script_state, key);
            return ESCreateIterResultObject(script_state, false, v8_key);
        }
        case Kind::kValue: {
            v8::Local<v8::Value> v8_value = ToV8Traits<IDLValueType>::ToV8(script_state, value);
...
template <typename IDLValueType, typename ValueType> class ValueAsyncIterationSource : public AsyncIterationSourceBase {
protected:
...
template <typename IDLInterface, typename... InitArgs> class PairAsyncIterable {
public:
    ...
    static_assert(sizeof(AsyncIteratorType), // Read the following for a compile error.
    ...
    using IterationSource = bindings::PairAsyncIterationSource<IDLKeyType, IDLValueType, KeyType, ValueType>;
    ...
    AsyncIteratorType* keysForBinding(ScriptState* script_state, ArgsAndExceptionState&&... args_and_exception_state)
    {
        const auto kind = IterationSource::Kind::kKey;
        IterationSource* source = CreateIterationSource(script_state, kind, std::forward<ArgsAndExceptionState>(args_and_exception_state)...);
        if (!source) {
            return nullptr;
        }
    ...
    template <typename... ArgsAndExceptionState>
    AsyncIteratorType* valuesForBinding(ScriptState* script_state, ArgsAndExceptionState&&... args_and_exception_state)
    {
    ...
    template <typename... ArgsAndExceptionState>
    AsyncIteratorType* entriesForBinding(ScriptState* script_state, ArgsAndExceptionState&&... args_and_exception_state)
    {
...
private:
    virtual IterationSource* CreateIterationSource(ScriptState* script_state, IterationSource::Kind kind, InitArgs... args, ExceptionState& exception_state)
        = 0;
};

template <typename IDLInterface, typename... InitArgs> class ValueAsyncIterable {
public:
    using AsyncIteratorType = AsyncIterator<IDLInterface>;
    // Check whether the v8_async_iterator_foo_bar.h is appropriately included.
    // Make sizeof require the complete definition of the class.
    ...
    using IDLValueType = typename AsyncIteratorType::IDLValueType;
    using ValueType = typename AsyncIteratorType::ValueType;
    using IterationSource = bindings::ValueAsyncIterationSource<IDLValueType, ValueType>;
