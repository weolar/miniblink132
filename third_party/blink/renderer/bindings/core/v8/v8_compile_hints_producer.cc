...
std::atomic<bool> V8CrowdsourcedCompileHintsProducer::data_generated_for_this_process_ = false;
...
bool RandomlySelectedToGenerateData()
{
    ...
    // on more platforms.
#if BUILDFLAG(IS_WIN)
    bool compile_hints_enabled = base::FeatureList::IsEnabled(features::kProduceCompileHints2);
    ...
    double data_production_level = features::kProduceCompileHintsDataProductionLevel.Get();
...
bool ShouldThisProcessGenerateData()
{
    ...
    static bool randomly_selected_to_generate_data = RandomlySelectedToGenerateData();
    return randomly_selected_to_generate_data;
}

} // namespace

V8CrowdsourcedCompileHintsProducer::V8CrowdsourcedCompileHintsProducer(Page* page)
    : page_(page)
{
...
void V8CrowdsourcedCompileHintsProducer::RecordScript(
    Frame* frame, ExecutionContext* execution_context, const v8::Local<v8::Script> script, ScriptState* script_state)
{
    ...
    uint32_t script_name_hash = ScriptNameHash(script->GetResourceName(), context, isolate);
...
void V8CrowdsourcedCompileHintsProducer::ScheduleDataDeletionTask(ExecutionContext* execution_context)
{
    constexpr int kDeletionDelaySeconds = 30;
    auto delay = base::Seconds(kDeletionDelaySeconds);

    execution_context->GetTaskRunner(TaskType::kIdleTask)->PostDelayedTask(FROM_HERE, WTF::BindOnce(&ClearDataTask, WrapWeakPersistent(this)), delay);
}
