...
public:
    explicit TestModuleRecordResolver(v8::Isolate* isolate)
        : isolate_(isolate)
    {
    }
    ~TestModuleRecordResolver() override = default;

    size_t ResolveCount() const
    {
        return specifiers_.size();
    }
    const Vector<String>& Specifiers() const
    {
        return specifiers_;
    }
    void PrepareMockResolveResult(v8::Local<v8::Module> module)
    {
        module_records_.push_back(MakeGarbageCollected<BoxedV8Module>(isolate_, module));
    }
...
private:
    ...
    const ModuleScript* GetModuleScriptFromModuleRecord(v8::Local<v8::Module>) const override
    {
        NOTREACHED();
    }

    v8::Local<v8::Module> Resolve(const ModuleRequest& module_request, v8::Local<v8::Module> module, ExceptionState&) override
    {
...
ModuleRecordTestModulator::ModuleRecordTestModulator(ScriptState* script_state)
    ...
    , resolver_(MakeGarbageCollected<TestModuleRecordResolver>(script_state->GetIsolate()))
{
...
TEST_F(ModuleRecordTest, compileSuccess)
{
    ...
    v8::Local<v8::Module> module = ModuleTestBase::CompileModule(scope.GetScriptState(), "export const a = 42;", js_url);
...
TEST_F(ModuleRecordTest, compileFail)
{
    ...
    const KURL js_url("https://example.com/foo.js");
    v8::Local<v8::Module> module = ModuleTestBase::CompileModule(scope.GetScriptState(), "123 = 456", js_url);
...
TEST_F(ModuleRecordTest, moduleRequests)
{
    V8TestingScope scope;
    const KURL js_url("https://example.com/foo.js");
    v8::Local<v8::Module> module = ModuleTestBase::CompileModule(scope.GetScriptState(), "import 'a'; import 'b'; export const c = 'c';", js_url);
    ASSERT_FALSE(module.IsEmpty());
...
TEST_F(ModuleRecordTest, moduleRequestsWithImportAttributes)
{
    ...
    const KURL js_url("https://example.com/foo.js");
    v8::Local<v8::Module> module = ModuleTestBase::CompileModule(scope.GetScriptState(),
        "import 'a' with { };"
...
TEST_F(ModuleRecordTest, instantiateNoDeps)
{
    ...
    auto* modulator = MakeGarbageCollected<ModuleRecordTestModulator>(scope.GetScriptState());
    auto* resolver = modulator->GetTestModuleRecordResolver();

    const KURL js_url("https://example.com/foo.js");
    v8::Local<v8::Module> module = ModuleTestBase::CompileModule(scope.GetScriptState(), "export const a = 42;", js_url);
    ASSERT_FALSE(module.IsEmpty());
    ScriptValue exception = ModuleRecord::Instantiate(scope.GetScriptState(), module, js_url);
...
TEST_F(ModuleRecordTest, instantiateWithDeps)
{
    ...
    v8::Local<v8::Module> module_a = ModuleTestBase::CompileModule(scope.GetScriptState(), "export const a = 'a';", js_url_a);
    ASSERT_FALSE(module_a.IsEmpty());
    resolver->PrepareMockResolveResult(module_a);

    const KURL js_url_b("https://example.com/b.js");
    v8::Local<v8::Module> module_b = ModuleTestBase::CompileModule(scope.GetScriptState(), "export const b = 'b';", js_url_b);
    ASSERT_FALSE(module_b.IsEmpty());
    resolver->PrepareMockResolveResult(module_b);

    const KURL js_url_c("https://example.com/c.js");
    v8::Local<v8::Module> module = ModuleTestBase::CompileModule(scope.GetScriptState(), "import 'a'; import 'b'; export const c = 123;", js_url_c);
    ASSERT_FALSE(module.IsEmpty());
    ScriptValue exception = ModuleRecord::Instantiate(scope.GetScriptState(), module, js_url_c);
    ASSERT_TRUE(exception.IsEmpty());
...
TEST_F(ModuleRecordTest, EvaluationErrorIsRemembered)
{
    ...
    v8::Local<v8::Module> module_failure = ModuleTestBase::CompileModule(scope.GetScriptState(), "nonexistent_function()", js_url_f);
    ASSERT_FALSE(module_failure.IsEmpty());
    ASSERT_TRUE(ModuleRecord::Instantiate(state, module_failure, js_url_f).IsEmpty());
    ScriptEvaluationResult evaluation_result1
        = JSModuleScript::CreateForTest(modulator, module_failure, js_url_f)->RunScriptOnScriptStateAndReturnValue(scope.GetScriptState());

    resolver->PrepareMockResolveResult(module_failure);

    const KURL js_url_c("https://example.com/c.js");
    v8::Local<v8::Module> module = ModuleTestBase::CompileModule(scope.GetScriptState(), "import 'failure'; export const c = 123;", js_url_c);
    ASSERT_FALSE(module.IsEmpty());
    ASSERT_TRUE(ModuleRecord::Instantiate(state, module, js_url_c).IsEmpty());
    ScriptEvaluationResult evaluation_result2
        = JSModuleScript::CreateForTest(modulator, module, js_url_c)->RunScriptOnScriptStateAndReturnValue(scope.GetScriptState());

    v8::Local<v8::Value> exception1 = GetException(state, std::move(evaluation_result1));
    v8::Local<v8::Value> exception2 = GetException(state, std::move(evaluation_result2));
...
TEST_F(ModuleRecordTest, Evaluate)
{
    ...
    const KURL js_url("https://example.com/foo.js");
    v8::Local<v8::Module> module = ModuleTestBase::CompileModule(scope.GetScriptState(), "export const a = 42; window.foo = 'bar';", js_url);
    ASSERT_FALSE(module.IsEmpty());
    ScriptValue exception = ModuleRecord::Instantiate(scope.GetScriptState(), module, js_url);
    ASSERT_TRUE(exception.IsEmpty());

    EXPECT_EQ(JSModuleScript::CreateForTest(modulator, module, js_url)->RunScriptOnScriptStateAndReturnValue(scope.GetScriptState()).GetResultType(),
        ScriptEvaluationResult::ResultType::kSuccess);
    v8::Local<v8::Value> value = ClassicScript::CreateUnspecifiedScript("window.foo")->RunScriptAndReturnValue(&scope.GetWindow()).GetSuccessValueOrEmpty();
    ASSERT_TRUE(value->IsString());
    EXPECT_EQ("bar", ToCoreString(scope.GetIsolate(), v8::Local<v8::String>::Cast(value)));

    v8::Local<v8::Object> module_namespace = v8::Local<v8::Object>::Cast(ModuleRecord::V8Namespace(module));
    EXPECT_FALSE(module_namespace.IsEmpty());
    v8::Local<v8::Value> exported_value = module_namespace->Get(scope.GetContext(), V8String(scope.GetIsolate(), "a")).ToLocalChecked();
...
TEST_F(ModuleRecordTest, EvaluateCaptureError)
{
    ...
    const KURL js_url("https://example.com/foo.js");
    v8::Local<v8::Module> module = ModuleTestBase::CompileModule(scope.GetScriptState(), "throw 'bar';", js_url);
    ASSERT_FALSE(module.IsEmpty());
    ScriptValue instantiation_exception = ModuleRecord::Instantiate(scope.GetScriptState(), module, js_url);
    ASSERT_TRUE(instantiation_exception.IsEmpty());

    ScriptEvaluationResult result = JSModuleScript::CreateForTest(modulator, module, js_url)->RunScriptOnScriptStateAndReturnValue(scope.GetScriptState());

    v8::Local<v8::Value> exception = GetException(scope.GetScriptState(), std::move(result));
    ASSERT_TRUE(exception->IsString());
    EXPECT_EQ("bar", ToCoreString(scope.GetIsolate(), exception.As<v8::String>()));
}
