...
scoped_refptr<SerializedScriptValue> PostMessageHelper::SerializeMessageByMove(
    v8::Isolate* isolate, const ScriptValue& message, const StructuredSerializeOptions* options, Transferables& transferables, ExceptionState& exception_state)
{
    if (options->hasTransfer() && !options->transfer().empty()) {
        if (!SerializedScriptValue::ExtractTransferables(isolate, options->transfer(), transferables, exception_state)) {
            return nullptr;
        }
    }
    ...
    scoped_refptr<SerializedScriptValue> serialized_message = SerializedScriptValue::Serialize(isolate, message.V8Value(), serialize_options, exception_state);
    ...
    v8::Isolate* isolate, const ScriptValue& message, const StructuredSerializeOptions* options, Transferables& transferables, ExceptionState& exception_state)
{
    ...
    ArrayBufferArray transferable_array_buffers = SerializedScriptValue::ExtractNonSharedArrayBuffers(transferables);
    ...
    SerializedScriptValue::TransferArrayBufferContents(isolate, transferable_array_buffers, exception_state);
    if (exception_state.HadException())
        return nullptr;
    // Neuter the original image bitmaps on the sender context.
    SerializedScriptValue::TransferImageBitmapContents(isolate, transferable_image_bitmaps, exception_state);
    if (exception_state.HadException())
        return nullptr;
...
mojom::blink::UserActivationSnapshotPtr PostMessageHelper::CreateUserActivationSnapshot(ExecutionContext* execution_context, const PostMessageOptions* options)
{
    if (!options->includeUserActivation())
        return nullptr;
    if (auto* dom_window = DynamicTo<LocalDOMWindow>(execution_context)) {
        if (LocalFrame* frame = dom_window->GetFrame()) {
            return mojom::blink::UserActivationSnapshot::New(frame->HasStickyUserActivation(), LocalFrame::HasTransientUserActivation(frame));
        }
    }
    return nullptr;
}
...
scoped_refptr<const SecurityOrigin> PostMessageHelper::GetTargetOrigin(
    const WindowPostMessageOptions* options, const ExecutionContext& context, ExceptionState& exception_state)
{
    ...
    if (target_origin == "*")
        return nullptr;
    scoped_refptr<const SecurityOrigin> target = SecurityOrigin::CreateFromString(target_origin);
    ...
    if (target->IsOpaque()) {
        exception_state.ThrowDOMException(DOMExceptionCode::kSyntaxError, "Invalid target origin '" + target_origin + "' in a call to 'postMessage'.");
        return nullptr;
    }
    return target;
}
