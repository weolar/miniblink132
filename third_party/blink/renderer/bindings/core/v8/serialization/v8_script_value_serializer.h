...
class CORE_EXPORT V8ScriptValueSerializer : public v8::ValueSerializer::Delegate {
    STACK_ALLOCATED();
...
public:
    ...
    static bool ExtractTransferable(v8::Isolate*, v8::Local<v8::Value>, wtf_size_t object_index, Transferables&, ExceptionState&);
    ...
    scoped_refptr<SerializedScriptValue> Serialize(v8::Local<v8::Value>, ExceptionState&);
...
protected:
    ...
    template <typename E> void WriteUint32Enum(E value)
    {
        static_assert(
    ...
    const Transferables* GetTransferables() const
    {
        return transferables_;
    }

    TrailerWriter& GetTrailerWriter()
    {
        return trailer_writer_;
    }
...
private:
    ...
    v8::Maybe<uint32_t> GetSharedArrayBufferId(v8::Isolate*, v8::Local<v8::SharedArrayBuffer>) override;

    v8::Maybe<uint32_t> GetWasmModuleTransferId(v8::Isolate*, v8::Local<v8::WasmModuleObject>) override;
    ...
    // requested.
    void* ReallocateBufferMemory(void* old_buffer, size_t, size_t* actual_size) override;
    void FreeBufferMemory(void* buffer) override;

    bool AdoptSharedValueConveyor(v8::Isolate* isolate, v8::SharedValueConveyor&& conveyor) override;
