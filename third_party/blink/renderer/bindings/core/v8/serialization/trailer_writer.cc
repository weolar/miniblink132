...
Vector<uint8_t> TrailerWriter::MakeTrailerData() const
{
    ...
    if (wtf_size_t num_exposed = requires_exposed_interfaces_.size(); num_exposed) {
        ...
        writer.Write(base::span<const uint8_t>((const uint8_t*)requires_exposed_interfaces_.data(), requires_exposed_interfaces_.size()));
