...
// returned.
size_t ReadVersionEnvelope(SerializedScriptValue* serialized_script_value, uint32_t* out_version)
{
...
MessagePort* CreateEntangledPort(ScriptState* script_state, const MessagePortChannel& channel)
{
    MessagePort* const port = MakeGarbageCollected<MessagePort>(*ExecutionContext::From(script_state));
...
V8ScriptValueDeserializer::V8ScriptValueDeserializer(ScriptState* script_state, UnpackedSerializedScriptValue* unpacked_value, const Options& options)
    : V8ScriptValueDeserializer(script_state, unpacked_value, unpacked_value->Value(), options)
{
}

V8ScriptValueDeserializer::V8ScriptValueDeserializer(ScriptState* script_state, scoped_refptr<SerializedScriptValue> value, const Options& options)
    : V8ScriptValueDeserializer(script_state, nullptr, std::move(value), options)
{
    DCHECK(!serialized_script_value_->HasPackedContents()) << "If the provided SerializedScriptValue could contain packed contents "
...
V8ScriptValueDeserializer::V8ScriptValueDeserializer(
    ScriptState* script_state, UnpackedSerializedScriptValue* unpacked_value, scoped_refptr<SerializedScriptValue> value, const Options& options)
    : script_state_(script_state)
    , unpacked_value_(unpacked_value)
    , serialized_script_value_(value)
    , deserializer_(script_state_->GetIsolate(), serialized_script_value_->Data(), serialized_script_value_->DataLengthInBytes(), this)
...
v8::Local<v8::Value> V8ScriptValueDeserializer::Deserialize()
{
    ...
    size_t version_envelope_size = ReadVersionEnvelope(serialized_script_value_.get(), &version_);
...
void V8ScriptValueDeserializer::Transfer()
{
    ...
    for (unsigned i = 0; i < array_buffers.size(); i++) {
        ...
        v8::Local<v8::Value> wrapper = ToV8Traits<DOMArrayBufferBase>::ToV8(script_state_, array_buffer);
        if (array_buffer->IsShared()) {
            ...
            deserializer_.TransferSharedArrayBuffer(i, v8::Local<v8::SharedArrayBuffer>::Cast(wrapper));
        } else {
            DCHECK(wrapper->IsArrayBuffer());
            deserializer_.TransferArrayBuffer(i, v8::Local<v8::ArrayBuffer>::Cast(wrapper));
        }
    }
}

bool V8ScriptValueDeserializer::ReadUnguessableToken(base::UnguessableToken* token_out)
{
    uint64_t high;
    uint64_t low;
    if (!ReadUint64(&high) || !ReadUint64(&low))
        return false;
    std::optional<base::UnguessableToken> token = base::UnguessableToken::Deserialize(high, low);
    if (!token.has_value()) {
        return false;
    }
...
bool V8ScriptValueDeserializer::ReadUTF8String(String* string)
{
    ...
    if (!ReadUint32(&utf8_length) || !ReadRawBytes(utf8_length, &utf8_data))
        return false;
    *string = String::FromUTF8(reinterpret_cast<const LChar*>(utf8_data), utf8_length);
...
ScriptWrappable* V8ScriptValueDeserializer::ReadDOMObject(SerializationTag tag, ExceptionState& exception_state)
{
    if (!ExecutionContextExposesInterface(ExecutionContext::From(GetScriptState()), tag)) {
        return nullptr;
    }
    switch (tag) {
    case kBlobTag: {
    ...
    case kImageBitmapTag: {
        SerializedPredefinedColorSpace predefined_color_space = SerializedPredefinedColorSpace::kSRGB;
        Vector<double> sk_color_space;
        SerializedPixelFormat canvas_pixel_format = SerializedPixelFormat::kNative8_LegacyObsolete;
        SerializedOpacityMode canvas_opacity_mode = SerializedOpacityMode::kOpaque;
        SerializedImageOrientation image_orientation = SerializedImageOrientation::kTopLeft;
        uint32_t origin_clean = 0, is_premultiplied = 0, width = 0, height = 0, byte_length = 0;
        ...
        if (Version() >= 18) {
            ...
            do {
                ...
                case ImageSerializationTag::kPredefinedColorSpaceTag:
                    if (!ReadUint32Enum<SerializedPredefinedColorSpace>(&predefined_color_space)) {
                        return nullptr;
                    }
                    break;
                ...
                case ImageSerializationTag::kImageOrientationTag:
                    if (!ReadUint32Enum<SerializedImageOrientation>(&image_orientation))
                        return nullptr;
                    break;
        ...
        } else if (!ReadUint32(&origin_clean) || origin_clean > 1 || !ReadUint32(&is_premultiplied) || is_premultiplied > 1) {
            return nullptr;
        }
        if (!ReadUint32(&width) || !ReadUint32(&height) || !ReadUint32(&byte_length) || !ReadRawBytes(byte_length, &pixels))
            return nullptr;
        SerializedImageBitmapSettings settings(
            predefined_color_space, sk_color_space, canvas_pixel_format, canvas_opacity_mode, is_premultiplied, image_orientation);
        SkImageInfo info = settings.GetSkImageInfo(width, height);
        base::CheckedNumeric<uint32_t> computed_byte_length = info.computeMinByteSize();
        if (!computed_byte_length.IsValid() || computed_byte_length.ValueOrDie() != byte_length)
            return nullptr;
        if (!origin_clean) {
        ...
        return MakeGarbageCollected<ImageBitmap>(pixmap, origin_clean, settings.GetImageOrientation());
    }
    ...
    case kImageDataTag: {
        SerializedPredefinedColorSpace predefined_color_space = SerializedPredefinedColorSpace::kSRGB;
        SerializedImageDataStorageFormat image_data_storage_format = SerializedImageDataStorageFormat::kUint8Clamped;
        ...
        if (Version() >= 18) {
            ...
            do {
                ...
                case ImageSerializationTag::kPredefinedColorSpaceTag:
                    ...
                    break;
                case ImageSerializationTag::kImageDataStorageFormatTag:
                    if (!ReadUint32Enum<SerializedImageDataStorageFormat>(&image_data_storage_format))
                        return nullptr;
                    break;
                case ImageSerializationTag::kCanvasPixelFormatTag:
                case ImageSerializationTag::kOriginCleanTag:
        ...
        if (!ReadUint32(&width) || !ReadUint32(&height) || !ReadUint64(&byte_length_64) || !base::MakeCheckedNum(byte_length_64).AssignIfValid(&byte_length)
            || !ReadRawBytes(byte_length, &pixels)) {
            return nullptr;
        }

        SerializedImageDataSettings settings(predefined_color_space, image_data_storage_format);
        ImageData* image_data
            = ImageData::ValidateAndCreate(width, height, std::nullopt, settings.GetImageDataSettings(), ImageData::ValidateAndCreateParams(), exception_state);
        if (!image_data)
            return nullptr;
    ...
    case kDOMPointTag: {
        ...
        if (!ReadDouble(&x) || !ReadDouble(&y) || !ReadDouble(&z) || !ReadDouble(&w))
            return nullptr;
    ...
    case kDOMRectTag: {
        ...
        if (!ReadDouble(&x) || !ReadDouble(&y) || !ReadDouble(&width) || !ReadDouble(&height))
            return nullptr;
    ...
    case kDOMQuadTag: {
        ...
        return DOMQuad::Create(point_inits[0], point_inits[1], point_inits[2], point_inits[3]);
    }
    ...
    case kMessagePortTag: {
        uint32_t index = 0;
        if (!ReadUint32(&index) || !transferred_message_ports_ || index >= transferred_message_ports_->size())
            return nullptr;
        return (*transferred_message_ports_)[index].Get();
    }
    case kMojoHandleTag: {
        uint32_t index = 0;
        if (!RuntimeEnabledFeatures::MojoJSEnabled() || !ReadUint32(&index) || index >= serialized_script_value_->MojoHandles().size()) {
            return nullptr;
        }
        return MakeGarbageCollected<MojoHandle>(std::move(serialized_script_value_->MojoHandles()[index]));
    }
    case kOffscreenCanvasTransferTag: {
        uint32_t width = 0, height = 0, canvas_id = 0, client_id = 0, sink_id = 0, filter_quality = 0;
        if (!ReadUint32(&width) || !ReadUint32(&height) || !ReadUint32(&canvas_id) || !ReadUint32(&client_id) || !ReadUint32(&sink_id)
            || !ReadUint32(&filter_quality))
            return nullptr;
        OffscreenCanvas* canvas = OffscreenCanvas::Create(GetScriptState(), width, height);
    ...
    case kReadableStreamTransferTag: {
        ...
        return ReadableStream::Deserialize(
            script_state_, CreateEntangledPort(GetScriptState(), streams_[index].channel), std::move(streams_[index].readable_optimizer), exception_state);
    }
    case kWritableStreamTransferTag: {
        ...
        return WritableStream::Deserialize(
            script_state_, CreateEntangledPort(GetScriptState(), streams_[index].channel), std::move(streams_[index].writable_optimizer), exception_state);
    }
    case kTransformStreamTransferTag: {
        uint32_t index = 0;
        if (!ReadUint32(&index) || index == std::numeric_limits<decltype(index)>::max() || index + 1 >= streams_.size()) {
            return nullptr;
        }
        MessagePort* const port_for_readable = CreateEntangledPort(GetScriptState(), streams_[index].channel);
        MessagePort* const port_for_writable = CreateEntangledPort(GetScriptState(), streams_[index + 1].channel);

        // https://streams.spec.whatwg.org/#ts-transfer
        // 1. Let readableRecord be !
        //    StructuredDeserializeWithTransfer(dataHolder.[[readable]], the
        //    current Realm).
        ReadableStream* readable = ReadableStream::Deserialize(script_state_, port_for_readable,
        ...
        //    current Realm).
        WritableStream* writable = WritableStream::Deserialize(script_state_, port_for_writable,
            /*optimizer=*/nullptr, exception_state);
        if (!writable)
            return nullptr;
    ...
    case kDOMExceptionTag: {
        ...
        if (!ReadUTF8String(&name) || !ReadUTF8String(&message) || !ReadUTF8String(&stack_unused)) {
            return nullptr;
        }
    ...
    case kFencedFrameConfigTag: {
        ...
        uint32_t has_shared_storage_context, has_container_size, container_width, container_height, has_content_size, content_width, content_height,
            freeze_initial_size;
        KURL url;
        ...
        if (!ReadUTF8String(&url_string) || !ReadUint32Enum<FencedFrameConfig::AttributeVisibility>(&url_visibility) || !ReadUint32(&freeze_initial_size)
        ...
        if (has_shared_storage_context && !ReadUTF8String(&shared_storage_context)) {
            return nullptr;
        }
        ...
        return FencedFrameConfig::Create(url, shared_storage_context, urn_uuid, container_size, content_size, url_visibility, freeze_initial_size);
    }
    default:
        break;
    }
...
File* V8ScriptValueDeserializer::ReadFile()
{
    ...
    if (!ReadUTF8String(&path) || (Version() >= 4 && !ReadUTF8String(&name)) || (Version() >= 4 && !ReadUTF8String(&relative_path)) || !ReadUTF8String(&uuid)
        || !ReadUTF8String(&type) || (Version() >= 4 && !ReadUint32(&has_snapshot)))
        return nullptr;
    if (has_snapshot) {
    ...
    if (Version() >= 7 && !ReadUint32(&is_user_visible))
        return nullptr;
    const File::UserVisibility user_visibility = is_user_visible ? File::kIsUserVisible : File::kIsNotUserVisible;
    auto blob_handle = GetBlobDataHandle(uuid);
    if (!blob_handle)
        return nullptr;
    std::optional<base::Time> last_modified;
    if (has_snapshot && std::isfinite(last_modified_ms)) {
        last_modified = base::Time::FromMillisecondsSinceUnixEpoch(last_modified_ms);
    }
    return File::CreateFromSerialization(path, name, relative_path, user_visibility, has_snapshot, size, last_modified, std::move(blob_handle));
}
...
File* V8ScriptValueDeserializer::ReadFileIndex()
{
    ...
    if (!blob_handle)
        return nullptr;
    return File::CreateFromIndexedSerialization(info.FileName(), info.size(), info.LastModified(), blob_handle);
}
...
scoped_refptr<BlobDataHandle> V8ScriptValueDeserializer::GetBlobDataHandle(const String& uuid)
{
...
v8::MaybeLocal<v8::Object> V8ScriptValueDeserializer::ReadHostObject(v8::Isolate* isolate)
{
    ...
    ExceptionState exception_state(isolate, v8::ExceptionContext::kUnknown, nullptr, nullptr);
    ...
    if (!wrappable) {
        exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "Unable to deserialize cloned data.");
        return v8::MaybeLocal<v8::Object>();
    }
    v8::Local<v8::Value> wrapper = ToV8Traits<ScriptWrappable>::ToV8(script_state_, wrappable);
    DCHECK(wrapper->IsObject());
    return wrapper.As<v8::Object>();
}

v8::MaybeLocal<v8::WasmModuleObject> V8ScriptValueDeserializer::GetWasmModuleFromId(v8::Isolate* isolate, uint32_t id)
{
    if (id < serialized_script_value_->WasmModules().size()) {
        return v8::WasmModuleObject::FromCompiledModule(isolate, serialized_script_value_->WasmModules()[id]);
    }
    CHECK(serialized_script_value_->WasmModules().empty());
    return v8::MaybeLocal<v8::WasmModuleObject>();
}

v8::MaybeLocal<v8::SharedArrayBuffer> V8ScriptValueDeserializer::GetSharedArrayBufferFromId(v8::Isolate* isolate, uint32_t id)
{
    auto& shared_array_buffers_contents = serialized_script_value_->SharedArrayBuffersContents();
    if (id < shared_array_buffers_contents.size()) {
        ...
        v8::Local<v8::Value> wrapper = ToV8Traits<DOMSharedArrayBuffer>::ToV8(script_state_, shared_array_buffer);
        DCHECK(wrapper->IsSharedArrayBuffer());
        return v8::Local<v8::SharedArrayBuffer>::Cast(wrapper);
    }
    ExceptionState exception_state(isolate, v8::ExceptionContext::kUnknown, nullptr, nullptr);
    exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "Unable to deserialize SharedArrayBuffer.");
...
const v8::SharedValueConveyor* V8ScriptValueDeserializer::GetSharedValueConveyor(v8::Isolate* isolate)
{
    if (auto* conveyor = serialized_script_value_->MaybeGetSharedValueConveyor()) {
        return conveyor;
    }
    ExceptionState exception_state(isolate, v8::ExceptionContext::kUnknown, nullptr, nullptr);
    exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "Unable to deserialize shared JS value.");
    return nullptr;
}

// static
bool V8ScriptValueDeserializer::ExecutionContextExposesInterface(ExecutionContext* execution_context, SerializationTag interface_tag)
{
