...
SerializedPredefinedColorSpace SerializeColorSpace(PredefinedColorSpace color_space)
{
    switch (color_space) {
...
PredefinedColorSpace DeserializeColorSpace(SerializedPredefinedColorSpace serialized_color_space)
{
...
SerializedImageDataSettings::SerializedImageDataSettings(PredefinedColorSpace color_space, ImageDataStorageFormat storage_format)
...
SerializedImageDataSettings::SerializedImageDataSettings(SerializedPredefinedColorSpace color_space, SerializedImageDataStorageFormat storage_format)
...
SerializedImageBitmapSettings::SerializedImageBitmapSettings(SkImageInfo info, ImageOrientationEnum image_orientation)
    : sk_color_space_(kSerializedParametricColorSpaceLength)
{
    auto color_space = info.colorSpace() ? info.refColorSpace() : SkColorSpace::MakeSRGB();
...
SerializedImageBitmapSettings::SerializedImageBitmapSettings(SerializedPredefinedColorSpace color_space, const Vector<double>& sk_color_space,
    SerializedPixelFormat pixel_format, SerializedOpacityMode opacity_mode, uint32_t is_premultiplied, SerializedImageOrientation image_orientation)
    : color_space_(color_space)
...
SkImageInfo SerializedImageBitmapSettings::GetSkImageInfo(uint32_t width, uint32_t height) const
{
    sk_sp<SkColorSpace> sk_color_space = PredefinedColorSpaceToSkColorSpace(DeserializeColorSpace(color_space_));
    ...
    return SkImageInfo::Make(width, height, sk_color_type, sk_alpha_type, std::move(sk_color_space));
}

ImageOrientationEnum SerializedImageBitmapSettings::GetImageOrientation() const
{
