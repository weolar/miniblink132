...
TEST(TrailerWriterTest, ExposedInterfaces)
{
    ...
    EXPECT_THAT(base::make_span(trailer).first(5u), ElementsAre(0xA0, 0x00, 0x00, 0x00, 0x02));
    EXPECT_THAT(base::make_span(trailer).subspan(5, 2), UnorderedElementsAre(kImageBitmapTag, kCryptoKeyTag));
}
