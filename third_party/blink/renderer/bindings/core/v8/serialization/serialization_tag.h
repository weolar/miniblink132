...
enum SerializationTag : uint8_t {
    ...
    kFileListTag = 'l', // length:uint32_t, files:RawFile[length] -> FileList (ref)
    kFileListIndexTag = 'L', // length:uint32_t, files:int32_t[length] -> FileList (ref)
    ...
    // data:byte[pixelDataLength]
    // -> ImageBitmap (ref)
    kImageBitmapTransferTag = 'G', // index:uint32_t -> ImageBitmap. For ImageBitmap transfer
    ...
    kMediaStreamTrack = 's', // trackImplSubtype:Uint32Enum, session_id.high:uint64_t,
    ...
    kTrailerOffsetTag = 0xFE, // offset:uint64_t (fixed width, network order) from buffer start
