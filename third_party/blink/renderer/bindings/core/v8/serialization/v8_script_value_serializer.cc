...
bool V8ScriptValueSerializer::ExtractTransferable(
    v8::Isolate* isolate, v8::Local<v8::Value> object, wtf_size_t object_index, Transferables& transferables, ExceptionState& exception_state)
{
    ...
    if (MessagePort* port = V8MessagePort::ToWrappable(isolate, object)) {
        ...
        if (transferables.message_ports.Contains(port)) {
            exception_state.ThrowDOMException(
                DOMExceptionCode::kDataCloneError, "Message port at index " + String::Number(object_index) + " is a duplicate of an earlier port.");
            return false;
        }
...
//             exception_state.ThrowDOMException(
//                 DOMExceptionCode::kDataCloneError, "Mojo handle at index " + String::Number(object_index) + " is a duplicate of an earlier handle.");
//             return false;
//         }
...
//     }
    if (object->IsArrayBuffer()) {
        DOMArrayBuffer* array_buffer = NativeValueTraits<IDLAllowResizable<DOMArrayBuffer>>::NativeValue(isolate, object, exception_state);
        if (exception_state.HadException())
            return false;
        if (transferables.array_buffers.Contains(array_buffer)) {
            exception_state.ThrowDOMException(
                DOMExceptionCode::kDataCloneError, "ArrayBuffer at index " + String::Number(object_index) + " is a duplicate of an earlier ArrayBuffer.");
            return false;
        }
    ...
    if (object->IsSharedArrayBuffer()) {
        DOMSharedArrayBuffer* shared_array_buffer = NativeValueTraits<IDLAllowResizable<DOMSharedArrayBuffer>>::NativeValue(isolate, object, exception_state);
        if (exception_state.HadException())
            return false;
        if (transferables.array_buffers.Contains(shared_array_buffer)) {
            exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError,
                "SharedArrayBuffer at index " + String::Number(object_index) + " is a duplicate of an earlier SharedArrayBuffer.");
            return false;
        }
    ...
    if (ImageBitmap* image_bitmap = V8ImageBitmap::ToWrappable(isolate, object)) {
        if (transferables.image_bitmaps.Contains(image_bitmap)) {
            exception_state.ThrowDOMException(
                DOMExceptionCode::kDataCloneError, "ImageBitmap at index " + String::Number(object_index) + " is a duplicate of an earlier ImageBitmap.");
            return false;
        }
        transferables.image_bitmaps.push_back(image_bitmap);
        return true;
    }
    if (OffscreenCanvas* offscreen_canvas = V8OffscreenCanvas::ToWrappable(isolate, object)) {
        if (transferables.offscreen_canvases.Contains(offscreen_canvas)) {
            exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError,
                "OffscreenCanvas at index " + String::Number(object_index) + " is a duplicate of an earlier OffscreenCanvas.");
            return false;
        }
    ...
    if (ReadableStream* stream = V8ReadableStream::ToWrappable(isolate, object)) {
        if (transferables.readable_streams.Contains(stream)) {
            exception_state.ThrowDOMException(
                DOMExceptionCode::kDataCloneError, "ReadableStream at index " + String::Number(object_index) + " is a duplicate of an earlier ReadableStream.");
            return false;
        }
    ...
    if (WritableStream* stream = V8WritableStream::ToWrappable(isolate, object)) {
        if (transferables.writable_streams.Contains(stream)) {
            exception_state.ThrowDOMException(
                DOMExceptionCode::kDataCloneError, "WritableStream at index " + String::Number(object_index) + " is a duplicate of an earlier WritableStream.");
            return false;
        }
        transferables.writable_streams.push_back(stream);
        return true;
    }
    if (TransformStream* stream = V8TransformStream::ToWrappable(isolate, object)) {
        if (transferables.transform_streams.Contains(stream)) {
            exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError,
                "TransformStream at index " + String::Number(object_index) + " is a duplicate of an earlier TransformStream.");
            return false;
        }
...
V8ScriptValueSerializer::V8ScriptValueSerializer(ScriptState* script_state, const Options& options)
...
scoped_refptr<SerializedScriptValue> V8ScriptValueSerializer::Serialize(v8::Local<v8::Value> value, ExceptionState& exception_state)
{
#if DCHECK_IS_ON()
    ...
    static_assert(SerializedScriptValue::kWireFormatVersion < 0x80, "the following calculation depends on the encoded length of the version");
    static_assert(SerializedScriptValue::kWireFormatVersion == 21, "Only version 21 is supported.");
    static constexpr size_t kTrailerOffsetPosition = 1 /* version tag */ + 1 /* version */ + 1 /* trailer offset tag */;
    ...
    bool wrote_value;
    if (!serializer_.WriteValue(script_state_->GetContext(), value).To(&wrote_value)) {
    ...
    if (shared_array_buffers_.size()) {
        ...
        if (!execution_context->CheckSharedArrayBufferTransferAllowedAndReport()) {
            exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "SharedArrayBuffer transfer requires self.crossOriginIsolated.");
            return nullptr;
        }
    }
    ...
    auto buffer =
        ...
        // Deleter.
        UNSAFE_BUFFERS(SerializedScriptValue::DataBufferPtr::FromOwningPointer(buffer_ptr, buffer_size));
    if (!trailer.empty()) {
        buffer.as_span().subspan<kTrailerOffsetPosition, sizeof(uint64_t)>().copy_from(base::U64ToBigEndian(buffer.size() - trailer.size()));
        buffer.as_span().subspan<kTrailerOffsetPosition + sizeof(uint64_t), sizeof(uint32_t)>().copy_from(base::U32ToBigEndian(trailer.size()));
    }
...
void V8ScriptValueSerializer::PrepareTransfer(ExceptionState& exception_state)
{
    ...
    for (uint32_t i = 0; i < transferables_->array_buffers.size(); i++) {
        ...
        if (!array_buffer->IsShared()) {
            v8::Local<v8::Value> wrapper = ToV8Traits<DOMArrayBuffer>::ToV8(script_state_, static_cast<DOMArrayBuffer*>(array_buffer));
            serializer_.TransferArrayBuffer(i, v8::Local<v8::ArrayBuffer>::Cast(wrapper));
        } else {
            exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "SharedArrayBuffer can not be in transfer list.");
            return;
        }
    }
}

void V8ScriptValueSerializer::FinalizeTransfer(ExceptionState& exception_state)
{
    ...
    if (!array_buffers.empty()) {
        serialized_script_value_->TransferArrayBuffers(isolate, array_buffers, exception_state);
        if (exception_state.HadException())
            return;
    }

    if (transferables_) {
        serialized_script_value_->TransferImageBitmaps(isolate, transferables_->image_bitmaps, exception_state);
        if (exception_state.HadException())
            return;

        serialized_script_value_->TransferOffscreenCanvas(isolate, transferables_->offscreen_canvases, exception_state);
        if (exception_state.HadException())
            return;
        ...
        // WriteDOMObject().
        serialized_script_value_->TransferReadableStreams(script_state_, transferables_->readable_streams, exception_state);
        if (exception_state.HadException())
            return;
        serialized_script_value_->TransferWritableStreams(script_state_, transferables_->writable_streams, exception_state);
        if (exception_state.HadException())
            return;
        serialized_script_value_->TransferTransformStreams(script_state_, transferables_->transform_streams, exception_state);
        if (exception_state.HadException())
            return;
...
void V8ScriptValueSerializer::WriteUnguessableToken(const base::UnguessableToken& token)
{
...
bool V8ScriptValueSerializer::WriteDOMObject(ScriptWrappable* wrappable, ExceptionState& exception_state)
{
    ...
    if (auto* blob = dispatcher.ToMostDerived<Blob>()) {
        if (blob_info_array_) {
            ...
            blob_info_array_->emplace_back(blob->GetBlobDataHandle(), blob->type(), blob->size());
            WriteAndRequireInterfaceTag(kBlobIndexTag);
            WriteUint32(static_cast<uint32_t>(index));
        } else {
            serialized_script_value_->BlobDataHandles().Set(blob->Uuid(), blob->GetBlobDataHandle());
    ...
    if (auto* file_list = dispatcher.ToMostDerived<FileList>()) {
        ...
        WriteAndRequireInterfaceTag(blob_info_array_ ? kFileListIndexTag : kFileListTag);
        WriteUint32(length);
    ...
    if (auto* image_bitmap = dispatcher.ToMostDerived<ImageBitmap>()) {
        if (image_bitmap->IsNeutered()) {
            exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "An ImageBitmap is detached and could not be cloned.");
            return false;
        }
        ...
        if (index != kNotFound) {
            if (image_bitmap->OriginClean()) {
                execution_context->CountUse(mojom::WebFeature::kOriginCleanImageBitmapTransfer);
            } else {
                exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "Non-origin-clean ImageBitmap cannot be transferred.");
                return false;
            }
        ...
        if (image_bitmap->OriginClean()) {
            execution_context->CountUse(mojom::WebFeature::kOriginCleanImageBitmapSerialization);
        } else {
            exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "Non-origin-clean ImageBitmap cannot be cloned.");
            return false;
        }
        WriteAndRequireInterfaceTag(kImageBitmapTag);
        SkImageInfo info = image_bitmap->GetBitmapSkImageInfo();
        SerializedImageBitmapSettings bitmap_settings(info, image_bitmap->ImageOrientation());
        WriteUint32Enum(ImageSerializationTag::kParametricColorSpaceTag);
        DCHECK_EQ(bitmap_settings.GetSerializedSkColorSpace().size(), kSerializedParametricColorSpaceLength);
        ...
        gfx::Size bitmap_size = image_bitmap->BitmapImage()->SizeWithConfig(size_config);
        WriteUint32(bitmap_size.width());
        WriteUint32(bitmap_size.height());
        Vector<uint8_t> pixels = image_bitmap->CopyBitmapData(info, /*apply_orientation=*/false);
        // Check if we succeeded to copy the bitmap data.
        if (!bitmap_size.IsEmpty() && pixels.size() == 0) {
            exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "An ImageBitmap could not be read successfully.");
            return false;
        }
    ...
    if (auto* image_data = dispatcher.ToMostDerived<ImageData>()) {
        ...
        SerializedImageDataSettings settings(image_data->GetPredefinedColorSpace(), image_data->GetImageDataStorageFormat());
    ...
    if (auto* quad = dispatcher.ToMostDerived<DOMQuad>()) {
        ...
        for (const DOMPoint* point : { quad->p1(), quad->p2(), quad->p3(), quad->p4() }) {
            WriteDouble(point->x());
            WriteDouble(point->y());
            WriteDouble(point->z());
            WriteDouble(point->w());
    ...
    if (auto* message_port = dispatcher.ToMostDerived<MessagePort>()) {
        ...
        if (index == kNotFound) {
            exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "A MessagePort could not be cloned because it was not transferred.");
            return false;
        }
        DCHECK_LE(index, std::numeric_limits<uint32_t>::max());
...
//         if (index == kNotFound) {
//             exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "A MojoHandle could not be cloned because it was not transferred.");
//             return false;
//         }
//         DCHECK_LE(index, std::numeric_limits<uint32_t>::max());
//         serialized_script_value_->MojoHandles().push_back(mojo_handle->TakeHandle());
...
//     }
    if (auto* canvas = dispatcher.ToMostDerived<OffscreenCanvas>()) {
        ...
        WriteUint32(canvas->FilterQuality() == cc::PaintFlags::FilterQuality::kNone ? 0 : 1);
        return true;
    }
    if (auto* stream = dispatcher.ToMostDerived<ReadableStream>()) {
        ...
        if (stream->IsLocked()) {
            exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "A ReadableStream could not be cloned because it was locked");
            return false;
        }
    ...
    if (auto* stream = dispatcher.ToMostDerived<WritableStream>()) {
        ...
        if (stream->locked()) {
            exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "A WritableStream could not be cloned because it was locked");
            return false;
        }
        ...
        WriteUint32(static_cast<uint32_t>(index + transferables_->readable_streams.size()));
        return true;
    }
    if (auto* stream = dispatcher.ToMostDerived<TransformStream>()) {
        ...
        if (stream->Readable()->locked() || stream->Writable()->locked()) {
            exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "A TransformStream could not be cloned because it was locked");
            return false;
        }
        ...
        WriteUint32(static_cast<uint32_t>(index * 2 + transferables_->readable_streams.size() + transferables_->writable_streams.size()));
        return true;
    }
    ...
    if (auto* config = dispatcher.ToMostDerived<FencedFrameConfig>()) {
        if (for_storage_) {
            exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "A FencedFrameConfig cannot be serialized for storage.");
            return false;
        }

        WriteAndRequireInterfaceTag(kFencedFrameConfigTag);

        WriteUTF8String(config->GetValueIgnoringVisibility<FencedFrameConfig::Attribute::kURL>().GetString());
        WriteUint32(static_cast<uint32_t>(config->GetAttributeVisibility<FencedFrameConfig::Attribute::kURL>(PassKey())));
...
bool V8ScriptValueSerializer::WriteFile(File* file, ExceptionState& exception_state)
{
    if (blob_info_array_) {
        size_t index = blob_info_array_->size();
        DCHECK_LE(index, std::numeric_limits<uint32_t>::max());
        blob_info_array_->emplace_back(file->GetBlobDataHandle(), file->name(), file->type(), file->LastModifiedTimeForSerialization(), file->size());
        WriteUint32(static_cast<uint32_t>(index));
    } else {
        serialized_script_value_->BlobDataHandles().Set(file->Uuid(), file->GetBlobDataHandle());
        ...
        std::optional<base::Time> last_modified = file->LastModifiedTimeForSerialization();
        WriteDouble(last_modified ? last_modified->InMillisecondsFSinceUnixEpochIgnoringNull() : std::numeric_limits<double>::quiet_NaN());
        WriteUint32(file->GetUserVisibility() == File::kIsUserVisible ? 1 : 0);
    }
    return true;
}

void V8ScriptValueSerializer::ThrowDataCloneError(v8::Local<v8::String> v8_message)
{
    V8ThrowDOMException::Throw(
        script_state_->GetIsolate(), DOMExceptionCode::kDataCloneError, ToBlinkString<String>(script_state_->GetIsolate(), v8_message, kDoNotExternalize));
}

v8::Maybe<bool> V8ScriptValueSerializer::IsHostObject(v8::Isolate* isolate, v8::Local<v8::Object> object)
{
...
v8::Maybe<bool> V8ScriptValueSerializer::WriteHostObject(v8::Isolate* isolate, v8::Local<v8::Object> object)
{
    ...
    if (!V8DOMWrapper::IsWrapper(isolate, object)) {
        exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "An object could not be cloned.");
    ...
    if (!exception_state.HadException()) {
        ...
        exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, interface + " object could not be cloned.");
    }
    return v8::Nothing<bool>();
}
...
DOMSharedArrayBuffer* ToSharedArrayBuffer(v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exception_state)
{
    if (!value->IsSharedArrayBuffer()) [[unlikely]] {
        exception_state.ThrowTypeError(ExceptionMessages::FailedToConvertJSValue("SharedArrayBuffer"));
        return nullptr;
    }
...
v8::Maybe<uint32_t> V8ScriptValueSerializer::GetSharedArrayBufferId(v8::Isolate* isolate, v8::Local<v8::SharedArrayBuffer> v8_shared_array_buffer)
{
    DCHECK_EQ(isolate, script_state_->GetIsolate());
    ...
    if (for_storage_) {
        exception_state.ThrowDOMException(DOMExceptionCode::kDataCloneError, "A SharedArrayBuffer can not be serialized for storage.");
    ...
    DOMSharedArrayBuffer* shared_array_buffer = ToSharedArrayBuffer(isolate, v8_shared_array_buffer, exception_state);
    if (exception_state.HadException())
        return v8::Nothing<uint32_t>();
...
v8::Maybe<uint32_t> V8ScriptValueSerializer::GetWasmModuleTransferId(v8::Isolate* isolate, v8::Local<v8::WasmModuleObject> module)
{
    if (for_storage_) {
        V8ThrowDOMException::Throw(isolate, DOMExceptionCode::kDataCloneError, "A WebAssembly.Module can not be serialized for storage.");
        return v8::Nothing<uint32_t>();
    }
    ...
    case Options::kTransfer: {
        ...
        serialized_script_value_->WasmModules().push_back(module->GetCompiledModule());
        uint32_t size = static_cast<uint32_t>(serialized_script_value_->WasmModules().size());
        DCHECK_GE(size, 1u);
...
void* V8ScriptValueSerializer::ReallocateBufferMemory(void* old_buffer, size_t size, size_t* actual_size)
{
    *actual_size = WTF::Partitions::BufferPotentialCapacity(size);
    return WTF::Partitions::BufferTryRealloc(old_buffer, *actual_size, "SerializedScriptValue buffer");
}
...
bool V8ScriptValueSerializer::AdoptSharedValueConveyor(v8::Isolate* isolate, v8::SharedValueConveyor&& conveyor)
{
    auto* execution_context = ExecutionContext::From(script_state_);
    if (for_storage_ || !execution_context->SharedArrayBufferTransferAllowed()) {
        V8ThrowDOMException::Throw(isolate, DOMExceptionCode::kDataCloneError,
            for_storage_ ? "A shared JS value cannot be serialized for storage." : "Shared JS value conveyance requires self.crossOriginIsolated.");
        return false;
    }
