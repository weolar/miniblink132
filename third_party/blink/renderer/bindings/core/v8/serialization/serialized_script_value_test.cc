...
TEST(SerializedScriptValueTest, WireFormatRoundTrip)
{
    ...
    scoped_refptr<SerializedScriptValue> sourceSerializedScriptValue
        = SerializedScriptValue::Serialize(scope.GetIsolate(), v8OriginalTrue, SerializedScriptValue::SerializeOptions(), ASSERT_NO_EXCEPTION);

    base::span<const uint8_t> wire_data = sourceSerializedScriptValue->GetWireData();

    scoped_refptr<SerializedScriptValue> serializedScriptValue = SerializedScriptValue::Create(wire_data);
    v8::Local<v8::Value> deserialized = serializedScriptValue->Deserialize(scope.GetIsolate());
...
TEST(SerializedScriptValueTest, WireFormatVersion17NoByteSwapping)
{
    ...
    scoped_refptr<SerializedScriptValue> serializedScriptValue = SerializedScriptValue::Create(data);
    v8::Local<v8::Value> deserialized = serializedScriptValue->Deserialize(scope.GetIsolate());
    EXPECT_TRUE(deserialized->IsTrue());
}
...
TEST(SerializedScriptValueTest, WireFormatVersion16ByteSwapping)
{
    ...
    scoped_refptr<SerializedScriptValue> serializedScriptValue = SerializedScriptValue::Create(base::as_bytes(base::make_span(data)));
    v8::Local<v8::Value> deserialized = serializedScriptValue->Deserialize(scope.GetIsolate());
    EXPECT_TRUE(deserialized->IsTrue());
}
...
TEST(SerializedScriptValueTest, WireFormatVersion0ImageData)
{
    ...
    scoped_refptr<SerializedScriptValue> serializedScriptValue = SerializedScriptValue::Create(base::as_bytes(base::make_span(data)));
    v8::Local<v8::Value> deserialized = serializedScriptValue->Deserialize(isolate);
...
TEST(SerializedScriptValueTest, UserSelectedFile)
{
    ...
    auto* original_file = MakeGarbageCollected<File>(scope.GetExecutionContext(), file_path);
    ...
    v8::Local<v8::Value> v8_original_file = ToV8Traits<File>::ToV8(scope.GetScriptState(), original_file);
    scoped_refptr<SerializedScriptValue> serialized_script_value
        = SerializedScriptValue::Serialize(scope.GetIsolate(), v8_original_file, SerializedScriptValue::SerializeOptions(), ASSERT_NO_EXCEPTION);
    v8::Local<v8::Value> v8_file = serialized_script_value->Deserialize(scope.GetIsolate());
...
TEST(SerializedScriptValueTest, FileConstructorFile)
{
    ...
    auto* original_file = MakeGarbageCollected<File>("hello.txt", base::Time::FromMillisecondsSinceUnixEpoch(12345678.0), blob_data_handle);
