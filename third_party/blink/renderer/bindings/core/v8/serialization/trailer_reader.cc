...
base::expected<bool, TrailerReader::Error> TrailerReader::SkipToTrailer()
{
    ...
    if (auto offset_raw = iterator_.Span<uint8_t, sizeof(uint64_t)>(); offset_raw.has_value()) {
    ...
    if (auto size_raw = iterator_.Span<uint8_t, sizeof(uint32_t)>(); size_raw.has_value()) {
    ...
    if (trailer_offset < iterator_.position() || base::ClampAdd(trailer_offset, trailer_size) > iterator_.total_size()) {
        return invalid_header();
    }
...
base::expected<void, TrailerReader::Error> TrailerReader::Read()
{
    while (const uint8_t* tag = iterator_.Object<uint8_t>()) {
        ...
        base::ranges::transform(exposed_raw, required_exposed_interfaces_.begin(), [](uint8_t raw) { return static_cast<SerializationTag>(raw); });
    }
    return {};
}
