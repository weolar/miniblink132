...
int LLVMFuzzerInitialize(int* argc, char*** argv)
{
    ...
    static BlinkFuzzerTestSupport fuzzer_support = BlinkFuzzerTestSupport(*argc, *argv);
...
int LLVMFuzzerTestOneInput(const uint8_t* data, size_t data_size)
{
    ...
    // Truncate the input.
    auto data_span = base::make_span(data, base::saturated_cast<wtf_size_t>(data_size));
    ...
    if (hash & kFuzzMessagePorts) {
        ...
        std::generate(message_ports->begin(), message_ports->end(), [&]() {
            auto* port = MakeGarbageCollected<MessagePort>(*page_holder->GetFrame().DomWindow());
    ...
    // Deserialize.
    scoped_refptr<SerializedScriptValue> serialized_script_value = SerializedScriptValue::Create(data_span);
    serialized_script_value->Deserialize(isolate, options);
    CHECK(!try_catch.HasCaught()) << "deserialize() should return null rather than throwing an exception.";
