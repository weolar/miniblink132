...
v8::Local<v8::Value> RoundTrip(v8::Local<v8::Value> value, V8TestingScope& scope, ExceptionState& exception_state, Transferables* transferables = nullptr,
    WebBlobInfoArray* blob_info = nullptr)
{
    ...
    if (transferables) {
        channels = MessagePort::DisentanglePorts(scope.GetExecutionContext(), transferables->message_ports, exception_state);
    ...
    scoped_refptr<SerializedScriptValue> serialized_script_value = serializer.Serialize(value, exception_state);
    ...
    MessagePortArray* transferred_message_ports = MessagePort::EntanglePorts(*scope.GetExecutionContext(), std::move(channels));

    UnpackedSerializedScriptValue* unpacked = SerializedScriptValue::Unpack(std::move(serialized_script_value));
    ...
    V8ScriptValueDeserializer deserializer(script_state, unpacked, deserialize_options);
    return deserializer.Deserialize();
}

v8::Local<v8::Value> Eval(const String& source, V8TestingScope& scope)
{
    return ClassicScript::CreateUnspecifiedScript(source)->RunScriptAndReturnValue(&scope.GetWindow()).GetSuccessValueOrEmpty();
}

String ToJSON(v8::Local<v8::Object> object, const V8TestingScope& scope)
{
    return ToBlinkString<String>(scope.GetIsolate(), v8::JSON::Stringify(scope.GetContext(), object).ToLocalChecked(), kDoNotExternalize);
}
} // namespace

scoped_refptr<SerializedScriptValue> SerializedValue(const Vector<uint8_t>& bytes)
{
    return SerializedScriptValue::Create(bytes);
}

// Checks for a DOM exception, including a rethrown one.
testing::AssertionResult HadDOMExceptionInCoreTest(const StringView& name, ScriptState* script_state, v8::TryCatch& try_catch)
{
...
TEST(V8ScriptValueSerializerTest, RoundTripJSONLikeValue)
{
    ...
    v8::Local<v8::Value> result = RoundTrip(object, scope, scope.GetExceptionState());
...
TEST(V8ScriptValueSerializerTest, ThrowsDataCloneError)
{
    ...
    ASSERT_FALSE(V8ScriptValueSerializer(script_state).Serialize(symbol, PassThroughException(scope.GetIsolate())));
    ASSERT_TRUE(HadDOMExceptionInCoreTest("DataCloneError", script_state, try_catch));
    DOMException* dom_exception = V8DOMException::ToWrappable(scope.GetIsolate(), try_catch.Exception());
...
TEST(V8ScriptValueSerializerTest, RethrowsScriptError)
{
    ...
    DCHECK(object->IsObject());
    ASSERT_FALSE(V8ScriptValueSerializer(script_state).Serialize(object, PassThroughException(scope.GetIsolate())));
...
TEST(V8ScriptValueSerializerTest, DeserializationErrorReturnsNull)
{
    ...
    V8TestingScope scope;
    ScriptState* script_state = scope.GetScriptState();
    scoped_refptr<SerializedScriptValue> invalid = SerializedScriptValue::Create("invalid data");
    v8::Local<v8::Value> result = V8ScriptValueDeserializer(script_state, invalid).Deserialize();
...
TEST(V8ScriptValueSerializerTest, DetachHappensAfterSerialization)
{
    ...
    RoundTrip(object, scope, PassThroughException(scope.GetIsolate()), &transferables);
    ASSERT_TRUE(try_catch.HasCaught());
    EXPECT_FALSE(HadDOMExceptionInCoreTest("DataCloneError", scope.GetScriptState(), try_catch));
...
TEST(V8ScriptValueSerializerTest, RoundTripDOMPoint)
{
    ...
    v8::Local<v8::Value> wrapper = ToV8Traits<DOMPoint>::ToV8(scope.GetScriptState(), point);
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, scope.GetExceptionState());
...
TEST(V8ScriptValueSerializerTest, DecodeDOMPoint)
{
    ...
    V8TestingScope scope;
    ScriptState* script_state = scope.GetScriptState();
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x11, 0xff, 0x0d, 0x5c, 'Q', 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf0, 0x3f, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x08, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x40 });
...
TEST(V8ScriptValueSerializerTest, RoundTripDOMPointReadOnly)
{
    ...
    v8::Local<v8::Value> wrapper = ToV8Traits<DOMPointReadOnly>::ToV8(scope.GetScriptState(), point);
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, scope.GetExceptionState());
    EXPECT_FALSE(V8DOMPoint::HasInstance(scope.GetIsolate(), result));
    DOMPointReadOnly* new_point = V8DOMPointReadOnly::ToWrappable(scope.GetIsolate(), result);
    ASSERT_NE(new_point, nullptr);
    EXPECT_NE(point, new_point);
    EXPECT_EQ(point->x(), new_point->x());
    EXPECT_EQ(point->y(), new_point->y());
...
TEST(V8ScriptValueSerializerTest, DecodeDOMPointReadOnly)
{
    ...
    V8TestingScope scope;
    ScriptState* script_state = scope.GetScriptState();
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x11, 0xff, 0x0d, 0x5c, 'W', 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf0, 0x3f, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x08, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x40 });
    v8::Local<v8::Value> result = V8ScriptValueDeserializer(script_state, input).Deserialize();
    DOMPointReadOnly* point = V8DOMPointReadOnly::ToWrappable(scope.GetIsolate(), result);
    ASSERT_NE(point, nullptr);
    EXPECT_EQ(1, point->x());
    EXPECT_EQ(2, point->y());
    EXPECT_EQ(3, point->z());
...
TEST(V8ScriptValueSerializerTest, RoundTripDOMRect)
{
    ...
    v8::Local<v8::Value> wrapper = ToV8Traits<DOMRect>::ToV8(scope.GetScriptState(), rect);
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, scope.GetExceptionState());
...
TEST(V8ScriptValueSerializerTest, DecodeDOMRect)
{
    ...
    V8TestingScope scope;
    ScriptState* script_state = scope.GetScriptState();
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x11, 0xff, 0x0d, 0x5c, 'E', 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf0, 0x3f, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x08, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x40 });
    v8::Local<v8::Value> result = V8ScriptValueDeserializer(script_state, input).Deserialize();
...
TEST(V8ScriptValueSerializerTest, RoundTripDOMRectReadOnly)
{
    ...
    v8::Local<v8::Value> wrapper = ToV8Traits<DOMRectReadOnly>::ToV8(scope.GetScriptState(), rect);
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, scope.GetExceptionState());
    EXPECT_FALSE(V8DOMRect::HasInstance(scope.GetIsolate(), result));
    DOMRectReadOnly* new_rect = V8DOMRectReadOnly::ToWrappable(scope.GetIsolate(), result);
    ASSERT_NE(new_rect, nullptr);
    EXPECT_NE(rect, new_rect);
    EXPECT_EQ(rect->x(), new_rect->x());
    EXPECT_EQ(rect->y(), new_rect->y());
...
TEST(V8ScriptValueSerializerTest, DecodeDOMRectReadOnly)
{
    ...
    V8TestingScope scope;
    ScriptState* script_state = scope.GetScriptState();
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x11, 0xff, 0x0d, 0x5c, 'R', 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf0, 0x3f, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x08, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x40 });
    v8::Local<v8::Value> result = V8ScriptValueDeserializer(script_state, input).Deserialize();
    DOMRectReadOnly* rect = V8DOMRectReadOnly::ToWrappable(scope.GetIsolate(), result);
    ASSERT_NE(rect, nullptr);
    EXPECT_EQ(1, rect->x());
    EXPECT_EQ(2, rect->y());
    EXPECT_EQ(3, rect->width());
...
TEST(V8ScriptValueSerializerTest, RoundTripDOMQuad)
{
    ...
    v8::Local<v8::Value> wrapper = ToV8Traits<DOMQuad>::ToV8(scope.GetScriptState(), quad);
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, scope.GetExceptionState());
...
TEST(V8ScriptValueSerializerTest, DecodeDOMQuad)
{
    ...
    V8TestingScope scope;
    ScriptState* script_state = scope.GetScriptState();
    scoped_refptr<SerializedScriptValue> input = SerializedValue({  });
    v8::Local<v8::Value> result = V8ScriptValueDeserializer(script_state, input).Deserialize();
...
TEST(V8ScriptValueSerializerTest, RoundTripDOMMatrix2D)
{
    ...
    v8::Local<v8::Value> wrapper = ToV8Traits<DOMMatrix>::ToV8(scope.GetScriptState(), matrix);
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, scope.GetExceptionState());
...
TEST(V8ScriptValueSerializerTest, RoundTripDOMMatrixReadOnly2D)
{
    ...
    init->setE(5.0);
    init->setF(6.0);
    DOMMatrixReadOnly* matrix = DOMMatrixReadOnly::fromMatrix(init, scope.GetExceptionState());
    EXPECT_TRUE(matrix->is2D());
    v8::Local<v8::Value> wrapper = ToV8Traits<DOMMatrixReadOnly>::ToV8(scope.GetScriptState(), matrix);
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, scope.GetExceptionState());
    EXPECT_FALSE(V8DOMMatrix::HasInstance(scope.GetIsolate(), result));
    DOMMatrixReadOnly* new_matrix = V8DOMMatrixReadOnly::ToWrappable(scope.GetIsolate(), result);
    ASSERT_NE(new_matrix, nullptr);
    EXPECT_NE(matrix, new_matrix);
    EXPECT_TRUE(new_matrix->is2D());
    EXPECT_EQ(matrix->a(), new_matrix->a());
...
TEST(V8ScriptValueSerializerTest, DecodeDOMMatrixReadOnly2D)
{
    ...
    v8::Local<v8::Value> result = V8ScriptValueDeserializer(script_state, input).Deserialize();
    DOMMatrixReadOnly* matrix = V8DOMMatrixReadOnly::ToWrappable(scope.GetIsolate(), result);
    ASSERT_NE(matrix, nullptr);
    EXPECT_TRUE(matrix->is2D());
    EXPECT_EQ(1.0, matrix->a());
    EXPECT_EQ(2.0, matrix->b());
...
TEST(V8ScriptValueSerializerTest, RoundTripImageData)
{
    ...
    V8TestingScope scope;
    ImageData* image_data = ImageData::ValidateAndCreate(2, 1, std::nullopt, nullptr, ImageData::ValidateAndCreateParams(), ASSERT_NO_EXCEPTION);
    SkPixmap pm = image_data->GetSkPixmap();
    pm.writable_addr32(0, 0)[0] = 200u;
    pm.writable_addr32(1, 0)[0] = 100u;
    v8::Local<v8::Value> wrapper = ToV8Traits<ImageData>::ToV8(scope.GetScriptState(), image_data);
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, scope.GetExceptionState());
    ImageData* new_image_data = V8ImageData::ToWrappable(scope.GetIsolate(), result);
...
TEST(V8ScriptValueSerializerTest, RoundTripImageDataWithColorSpaceInfo)
{
    ...
    ImageData* image_data = ImageData::ValidateAndCreate(2, 1, std::nullopt, image_data_settings, ImageData::ValidateAndCreateParams(), ASSERT_NO_EXCEPTION);
    SkPixmap pm = image_data->GetSkPixmap();
...
TEST(V8ScriptValueSerializerTest, DecodeImageDataV9)
{
    ...
    scoped_refptr<SerializedScriptValue> input
        = SerializedValue({ 0xff, 0x09, 0x3f, 0x00, 0x23, 0x02, 0x01, 0x08, 0xc8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 });
    v8::Local<v8::Value> result = V8ScriptValueDeserializer(script_state, input).Deserialize();
    ImageData* new_image_data = V8ImageData::ToWrappable(scope.GetIsolate(), result);
    ASSERT_NE(new_image_data, nullptr);
...
TEST(V8ScriptValueSerializerTest, DecodeImageDataV16)
{
    ...
    ScriptState* script_state = scope.GetScriptState();
    scoped_refptr<SerializedScriptValue> input
        = SerializedValue({ 0xff, 0x10, 0xff, 0x0c, 0x23, 0x02, 0x01, 0x08, 0xc8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 });
    v8::Local<v8::Value> result = V8ScriptValueDeserializer(script_state, input).Deserialize();
    ImageData* new_image_data = V8ImageData::ToWrappable(scope.GetIsolate(), result);
    ASSERT_NE(new_image_data, nullptr);
    EXPECT_EQ(gfx::Size(2, 1), new_image_data->Size());
...
TEST(V8ScriptValueSerializerTest, DecodeImageDataV18)
{
    ...
    ScriptState* script_state = scope.GetScriptState();
    scoped_refptr<SerializedScriptValue> input
        = SerializedValue({ 0xff, 0x12, 0xff, 0x0d, 0x5c, 0x23, 0x01, 0x03, 0x03, 0x02, 0x00, 0x02, 0x01, 0x20, 0xc8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
TEST(V8ScriptValueSerializerTest, InvalidImageDataDecodeV18)
{
    ...
    ScriptState* script_state = scope.GetScriptState();
    {
        ...
        scoped_refptr<SerializedScriptValue> input
            = SerializedValue({ 0xff, 0x12, 0xff, 0x0d, 0x5c, 0x23, 0x02, 0x00, 0x00, 0x01, 0x01, 0x04, 0x00, 0x00, 0x00, 0x00 });
        EXPECT_TRUE(V8ScriptValueDeserializer(script_state, input).Deserialize()->IsNull());
    }
}

MessagePort* MakeMessagePort(ExecutionContext* execution_context, ::MojoHandle* unowned_handle_out = nullptr)
{
...
TEST(V8ScriptValueSerializerTest, RoundTripMessagePort)
{
    ...
    MessagePort* port = MakeMessagePort(scope.GetExecutionContext(), &unowned_handle);
    v8::Local<v8::Value> wrapper = ToV8Traits<MessagePort>::ToV8(scope.GetScriptState(), port);
    Transferables transferables;
    transferables.message_ports.push_back(port);

    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, scope.GetExceptionState(), &transferables);
    MessagePort* new_port = V8MessagePort::ToWrappable(scope.GetIsolate(), result);
...
TEST(V8ScriptValueSerializerTest, NeuteredMessagePortThrowsDataCloneError)
{
    ...
    RoundTrip(wrapper, scope, PassThroughException(scope.GetIsolate()), &transferables);
    ASSERT_TRUE(HadDOMExceptionInCoreTest("DataCloneError", scope.GetScriptState(), try_catch));
}
...
TEST(V8ScriptValueSerializerTest, OutOfRangeMessagePortIndex)
{
    ...
    V8TestingScope scope;
    ScriptState* script_state = scope.GetScriptState();
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x09, 0x3f, 0x00, 0x4d, 0x01 });
...
TEST(V8ScriptValueSerializerTest, RoundTripMojoHandle)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    ContextFeatureSettings::From(scope.GetExecutionContext(), ContextFeatureSettings::CreationMode::kCreateIfNotExists)->EnableMojoJS(true);

    mojo::MessagePipe pipe;
    auto* handle = MakeGarbageCollected<MojoHandle>(mojo::ScopedHandle::From(std::move(pipe.handle0)));
    v8::Local<v8::Value> wrapper = ToV8Traits<MojoHandle>::ToV8(scope.GetScriptState(), handle);
    Transferables transferables;
    transferables.mojo_handles.push_back(handle);

    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, scope.GetExceptionState(), &transferables);
    MojoHandle* new_handle = V8MojoHandle::ToWrappable(scope.GetIsolate(), result);
...
TEST(V8ScriptValueSerializerTest, RoundTripImageBitmap)
{
    ...
    sk_sp<SkSurface> surface = SkSurfaces::Raster(SkImageInfo::MakeN32Premul(10, 7));
    surface->getCanvas()->clear(SK_ColorRED);
    auto* image_bitmap = MakeGarbageCollected<ImageBitmap>(UnacceleratedStaticBitmapImage::Create(surface->makeImageSnapshot()));
    ASSERT_TRUE(image_bitmap->BitmapImage());

    // Serialize and deserialize it.
    v8::Local<v8::Value> wrapper = ToV8Traits<ImageBitmap>::ToV8(scope.GetScriptState(), image_bitmap);
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, scope.GetExceptionState());
    ImageBitmap* new_image_bitmap = V8ImageBitmap::ToWrappable(scope.GetIsolate(), result);
    ...
    uint8_t pixel[4] = {};
    ASSERT_TRUE(new_image_bitmap->BitmapImage()->PaintImageForCurrentFrame().readPixels(
        SkImageInfo::Make(1, 1, kRGBA_8888_SkColorType, kPremul_SkAlphaType), &pixel, 4, 3, 3));
...
TEST(V8ScriptValueSerializerTest, ImageBitmapEXIFImageOrientation)
{
    ...
    for (uint8_t i = static_cast<uint8_t>(ImageOrientationEnum::kOriginTopLeft); i <= static_cast<uint8_t>(ImageOrientationEnum::kMaxValue); i++) {
        ImageOrientationEnum orientation = static_cast<ImageOrientationEnum>(i);
        sk_sp<SkSurface> surface = SkSurfaces::Raster(SkImageInfo::MakeN32Premul(kImageWidth, kImageHeight));
        auto static_image = UnacceleratedStaticBitmapImage::Create(surface->makeImageSnapshot());
...
TEST(V8ScriptValueSerializerTest, RoundTripImageBitmapWithColorSpaceInfo)
{
    ...
    SkImageInfo info = SkImageInfo::Make(10, 7, kRGBA_F16_SkColorType, kPremul_SkAlphaType, p3);
    ...
    uint8_t pixel[8] = {};
    ASSERT_TRUE(new_image_bitmap->BitmapImage()->PaintImageForCurrentFrame().readPixels(info.makeWH(1, 1), &pixel, 8, 3, 3));
...
// other platform writes BGRA.
    ...
    scoped_refptr<SerializedScriptValue> input
        = SerializedValue({ 0xff, 0x09, 0x3f, 0x00, 0x67, 0x01, 0x01, 0x02, 0x01, 0x08, 0xff, 0x00, 0x00, 0xff, 0x00, 0xff, 0x00, 0xff });
#else
    scoped_refptr<SerializedScriptValue> input
        = SerializedValue({ 0xff, 0x09, 0x3f, 0x00, 0x67, 0x01, 0x01, 0x02, 0x01, 0x08, 0x00, 0x00, 0xff, 0xff, 0x00, 0xff, 0x00, 0xff });
#endif
    ...
    uint8_t pixels[8] = {};
    ASSERT_TRUE(new_image_bitmap->BitmapImage()->PaintImageForCurrentFrame().readPixels(
        SkImageInfo::Make(2, 1, kRGBA_8888_SkColorType, kPremul_SkAlphaType), &pixels, 8, 0, 0));
    ASSERT_THAT(pixels, testing::ElementsAre(255, 0, 0, 255, 0, 255, 0, 255));
    // Check that orientation is top left (default).
    ASSERT_EQ(new_image_bitmap->ImageOrientation(), ImageOrientationEnum::kOriginTopLeft);
}
...
TEST(V8ScriptValueSerializerTest, DecodeImageBitmapV18)
{
    ...
    V8TestingScope scope;
    ScriptState* script_state = scope.GetScriptState();
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x12, 0xff, 0x0d, 0x5c, 0x67, 0x01, 0x03, 0x02, 0x01, 0x04, 0x01, 0x05, 0x01, 0x00,
        0x02, 0x01, 0x10, 0x94, 0x3a, 0x3f, 0x28, 0x5f, 0x24, 0x00, 0x3c, 0x94, 0x3a, 0x3f, 0x28, 0x5f, 0x24, 0x00, 0x3c });
    sk_sp<SkColorSpace> p3 = SkColorSpace::MakeRGB(SkNamedTransferFn::kSRGB, SkNamedGamut::kDisplayP3);
    ...
    uint8_t pixel[8] = {};
    SkImageInfo info = SkImageInfo::Make(1, 1, kRGBA_F16_SkColorType, kPremul_SkAlphaType, p3);
    ASSERT_TRUE(new_image_bitmap->BitmapImage()->PaintImageForCurrentFrame().readPixels(info, &pixel, 8, 1, 0));
    // The reference values are the hex representation of red in P3 (as stored
    // in half floats by Skia).
    ASSERT_THAT(pixel, testing::ElementsAre(0x94, 0x3A, 0x3F, 0x28, 0x5F, 0x24, 0x0, 0x3C));
    // Check that orientation is top left (default).
    ASSERT_EQ(new_image_bitmap->ImageOrientation(), ImageOrientationEnum::kOriginTopLeft);
}
...
TEST(V8ScriptValueSerializerTest, DecodeImageBitmapV20WithoutImageOrientation)
{
    ...
    scoped_refptr<SerializedScriptValue> input = SerializedValue({
        ...
        // srgb colorspace
        ,
    ...
    sk_sp<SkColorSpace> srgb = SkColorSpace::MakeRGB(SkNamedTransferFn::kSRGB, SkNamedGamut::kSRGB);
    EXPECT_TRUE(SkColorSpace::Equals(srgb.get(), bitmap_info.colorSpace()));
    // Check that orientation is bottom left.
    ASSERT_EQ(new_image_bitmap->ImageOrientation(), ImageOrientationEnum::kOriginTopLeft);
    // Check pixel value
    SkImageInfo info = SkImageInfo::Make(1, 1, kRGBA_8888_SkColorType, kPremul_SkAlphaType, srgb);
    uint8_t pixel[4] = {};
    ASSERT_TRUE(new_image_bitmap->BitmapImage()->PaintImageForCurrentFrame().readPixels(info, &pixel, 4, 0, 0));
...
TEST(V8ScriptValueSerializerTest, DecodeImageBitmapV20WithImageOrientation)
{
    ...
    EXPECT_TRUE(SkColorSpace::Equals(srgb.get(), bitmap_info.colorSpace()));
    // Check that orientation is bottom left.
    ASSERT_EQ(new_image_bitmap->ImageOrientation(), ImageOrientationEnum::kOriginBottomLeft);
    // Check pixel value
    SkImageInfo info = SkImageInfo::Make(1, 1, kRGBA_8888_SkColorType, kPremul_SkAlphaType, srgb);
    uint8_t pixel[4] = {};
    ASSERT_TRUE(new_image_bitmap->BitmapImage()->PaintImageForCurrentFrame().readPixels(info, &pixel, 4, 0, 0));
...
TEST(V8ScriptValueSerializerTest, InvalidImageBitmapDecode)
{
    ...
    ScriptState* script_state = scope.GetScriptState();
    {
        ...
        scoped_refptr<SerializedScriptValue> input
            = SerializedValue({ 0xff, 0x09, 0x3f, 0x00, 0x67, 0x01, 0x01, 0x02, 0x01, 0x09, 0x00, 0x00, 0xff, 0xff, 0x00, 0xff, 0x00, 0xff, 0x00, 0x00 });
        EXPECT_TRUE(V8ScriptValueDeserializer(script_state, input).Deserialize()->IsNull());
    }
    {
        // Too few bytes declared in pixel data.
        scoped_refptr<SerializedScriptValue> input
            = SerializedValue({ 0xff, 0x09, 0x3f, 0x00, 0x67, 0x01, 0x01, 0x02, 0x01, 0x07, 0x00, 0x00, 0xff, 0xff, 0x00, 0xff, 0x00, 0xff });
        EXPECT_TRUE(V8ScriptValueDeserializer(script_state, input).Deserialize()->IsNull());
    }
    {
        // Nonsense for origin clean data.
        scoped_refptr<SerializedScriptValue> input
            = SerializedValue({ 0xff, 0x09, 0x3f, 0x00, 0x67, 0x02, 0x01, 0x02, 0x01, 0x08, 0x00, 0x00, 0xff, 0xff, 0x00, 0xff, 0x00, 0xff });
        EXPECT_TRUE(V8ScriptValueDeserializer(script_state, input).Deserialize()->IsNull());
    }
    {
        // Nonsense for premultiplied bit.
        scoped_refptr<SerializedScriptValue> input
            = SerializedValue({ 0xff, 0x09, 0x3f, 0x00, 0x67, 0x01, 0x02, 0x02, 0x01, 0x08, 0x00, 0x00, 0xff, 0xff, 0x00, 0xff, 0x00, 0xff });
        EXPECT_TRUE(V8ScriptValueDeserializer(script_state, input).Deserialize()->IsNull());
    }
}
...
TEST(V8ScriptValueSerializerTest, InvalidImageBitmapDecodeV18)
{
    ...
    {
        // Too many bytes declared in pixel data.
        scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x12, 0xff, 0x0d, 0x5c, 0x67, 0x01, 0x03, 0x02, 0x03, 0x04, 0x01, 0x05, 0x01, 0x00,
            0x02, 0x01, 0x11, 0x94, 0x3a, 0x3f, 0x28, 0x5f, 0x24, 0x00, 0x3c, 0x94, 0x3a, 0x3f, 0x28, 0x5f, 0x24, 0x00, 0x3c, 0x00, 0x00 });
        EXPECT_TRUE(V8ScriptValueDeserializer(script_state, input).Deserialize()->IsNull());
    }
    {
        // Too few bytes declared in pixel data.
    ...
    }
    {
        ...
        scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x12, 0xff, 0x0d, 0x5c, 0x67, 0x01, 0x05, 0x02, 0x03, 0x04, 0x01, 0x05, 0x01, 0x00,
            0x02, 0x01, 0x10, 0x94, 0x3a, 0x3f, 0x28, 0x5f, 0x24, 0x00, 0x3c, 0x94, 0x3a, 0x3f, 0x28, 0x5f, 0x24, 0x00, 0x3c });
        EXPECT_TRUE(V8ScriptValueDeserializer(script_state, input).Deserialize()->IsNull());
    }
    {
        // Nonsense for pixel format data.
        scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x12, 0xff, 0x0d, 0x5c, 0x67, 0x01, 0x03, 0x02, 0x04, 0x04, 0x01, 0x05, 0x01, 0x00,
            0x02, 0x01, 0x10, 0x94, 0x3a, 0x3f, 0x28, 0x5f, 0x24, 0x00, 0x3c, 0x94, 0x3a, 0x3f, 0x28, 0x5f, 0x24, 0x00, 0x3c });
        EXPECT_TRUE(V8ScriptValueDeserializer(script_state, input).Deserialize()->IsNull());
    }
    {
        // Nonsense for origin clean data.
        scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x12, 0xff, 0x0d, 0x5c, 0x67, 0x01, 0x03, 0x02, 0x03, 0x04, 0x02, 0x05, 0x01, 0x00,
            0x02, 0x01, 0x10, 0x94, 0x3a, 0x3f, 0x28, 0x5f, 0x24, 0x00, 0x3c, 0x94, 0x3a, 0x3f, 0x28, 0x5f, 0x24, 0x00, 0x3c });
        EXPECT_TRUE(V8ScriptValueDeserializer(script_state, input).Deserialize()->IsNull());
    }
    {
        // Nonsense for premultiplied bit.
        scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x12, 0xff, 0x0d, 0x5c, 0x67, 0x01, 0x03, 0x02, 0x03, 0x04, 0x01, 0x05, 0x02, 0x00,
            0x02, 0x01, 0x10, 0x94, 0x3a, 0x3f, 0x28, 0x5f, 0x24, 0x00, 0x3c, 0x94, 0x3a, 0x3f, 0x28, 0x5f, 0x24, 0x00, 0x3c });
        EXPECT_TRUE(V8ScriptValueDeserializer(script_state, input).Deserialize()->IsNull());
    }
    {
        ...
        scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x12, 0xff, 0x0d, 0x5c, 0x67, 0x01, 0x03, 0x02, 0x03, 0x04, 0x01, 0x05, 0x01, 0x00,
            0x03, 0x01, 0x10, 0x94, 0x3a, 0x3f, 0x28, 0x5f, 0x24, 0x00, 0x3c, 0x94, 0x3a, 0x3f, 0x28, 0x5f, 0x24, 0x00, 0x3c });
        EXPECT_TRUE(V8ScriptValueDeserializer(script_state, input).Deserialize()->IsNull());
    }
    {
        // Nonsense image serialization tag (kImageDataStorageFormatTag).
        scoped_refptr<SerializedScriptValue> input
            = SerializedValue({ 0xff, 0x12, 0xff, 0x0d, 0x5c, 0x67, 0x03, 0x00, 0x00, 0x01, 0x01, 0x04, 0x00, 0x00, 0x00, 0x00 });
        EXPECT_TRUE(V8ScriptValueDeserializer(script_state, input).Deserialize()->IsNull());
    }
}
...
TEST(V8ScriptValueSerializerTest, TransferImageBitmap)
{
    ...
    auto* image_bitmap = MakeGarbageCollected<ImageBitmap>(UnacceleratedStaticBitmapImage::Create(image));
    ASSERT_TRUE(image_bitmap->BitmapImage());
    ...
    // Check that the pixel at (3, 3) is red.
    uint8_t pixel[4] = {};
    sk_sp<SkImage> new_image = new_image_bitmap->BitmapImage()->PaintImageForCurrentFrame().GetSwSkImage();
    ASSERT_TRUE(new_image->readPixels(SkImageInfo::Make(1, 1, kRGBA_8888_SkColorType, kPremul_SkAlphaType), &pixel, 4, 3, 3));
    ASSERT_THAT(pixel, testing::ElementsAre(255, 0, 0, 255));
...
TEST(V8ScriptValueSerializerTest, TransferOffscreenCanvas)
{
    ...
    V8TestingScope scope;
    OffscreenCanvas* canvas = OffscreenCanvas::Create(scope.GetScriptState(), 10, 7);
    canvas->SetPlaceholderCanvasId(519);
    v8::Local<v8::Value> wrapper = ToV8Traits<OffscreenCanvas>::ToV8(scope.GetScriptState(), canvas);
    Transferables transferables;
    transferables.offscreen_canvases.push_back(canvas);
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, scope.GetExceptionState(), &transferables);
    OffscreenCanvas* new_canvas = V8OffscreenCanvas::ToWrappable(scope.GetIsolate(), result);
...
TEST(V8ScriptValueSerializerTest, RoundTripBlob)
{
    ...
    Blob* blob = Blob::Create(base::as_bytes(base::span_with_nul_from_cstring(kHelloWorld)), "text/plain");
    String uuid = blob->Uuid();
    EXPECT_FALSE(uuid.empty());
    v8::Local<v8::Value> wrapper = ToV8Traits<Blob>::ToV8(scope.GetScriptState(), blob);
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, scope.GetExceptionState());
...
TEST(V8ScriptValueSerializerTest, DecodeBlobWithoutHandles)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x09, 0x3f, 0x00, 0x62, 0x24, 0x64, 0x38, 0x37, 0x35, 0x64, 0x66, 0x63, 0x32, 0x2d,
        0x34, 0x35, 0x30, 0x35, 0x2d, 0x34, 0x36, 0x31, 0x62, 0x2d, 0x39, 0x38, 0x66, 0x65, 0x2d, 0x30, 0x63, 0x66, 0x36, 0x63, 0x63, 0x35, 0x65, 0x61, 0x66,
        0x34, 0x34, 0x0a, 0x74, 0x65, 0x78, 0x74, 0x2f, 0x70, 0x6c, 0x61, 0x69, 0x6e, 0x0c });
...
TEST(V8ScriptValueSerializerTest, RoundTripBlobIndex)
{
    ...
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, scope.GetExceptionState(), nullptr, &blob_info_array);
...
TEST(V8ScriptValueSerializerTest, DecodeBlobIndex)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x09, 0x3f, 0x00, 0x69, 0x00 });
    WebBlobInfoArray blob_info_array;
    blob_info_array.emplace_back(WebBlobInfo::BlobForTesting("d875dfc2-4505-461b-98fe-0cf6cc5eaf44", "text/plain", 12));
    V8ScriptValueDeserializer::Options options;
    options.blob_info = &blob_info_array;
    V8ScriptValueDeserializer deserializer(scope.GetScriptState(), input, options);
    v8::Local<v8::Value> result = deserializer.Deserialize();
    Blob* new_blob = V8Blob::ToWrappable(scope.GetIsolate(), result);
    ASSERT_NE(new_blob, nullptr);
...
TEST(V8ScriptValueSerializerTest, DecodeBlobIndexOutOfRange)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x09, 0x3f, 0x00, 0x69, 0x01 });
    {
...
TEST(V8ScriptValueSerializerTest, RoundTripFileNative)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    FileBackedBlobFactoryTestHelper file_factory_helper(scope.GetExecutionContext());
    auto* file = MakeGarbageCollected<File>(scope.GetExecutionContext(), "/native/path");
    file_factory_helper.FlushForTesting();
    v8::Local<v8::Value> wrapper = ToV8Traits<File>::ToV8(scope.GetScriptState(), file);
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, scope.GetExceptionState());
...
TEST(V8ScriptValueSerializerTest, RoundTripFileBackedByBlob)
{
    ...
    auto* file = MakeGarbageCollected<File>("/native/path", kModificationTime, blob_data_handle);
    v8::Local<v8::Value> wrapper = ToV8Traits<File>::ToV8(scope.GetScriptState(), file);
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, scope.GetExceptionState());
    File* new_file = V8File::ToWrappable(scope.GetIsolate(), result);
    ASSERT_NE(new_file, nullptr);
...
TEST(V8ScriptValueSerializerTest, RoundTripFileNativeSnapshot)
{
    ...
    File* file = File::CreateForFileSystemFile(context, "name", metadata, File::kIsUserVisible);
    v8::Local<v8::Value> wrapper = ToV8Traits<File>::ToV8(scope.GetScriptState(), file);
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, scope.GetExceptionState());
    File* new_file = V8File::ToWrappable(scope.GetIsolate(), result);
    ASSERT_NE(new_file, nullptr);
...
TEST(V8ScriptValueSerializerTest, RoundTripFileNonNativeSnapshot)
{
    ...
    FileMetadata metadata;
    metadata.length = 0;
    File* file = File::CreateForFileSystemFile(url, metadata, File::kIsUserVisible, BlobDataHandle::Create());
    v8::Local<v8::Value> wrapper = ToV8Traits<File>::ToV8(scope.GetScriptState(), file);
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, scope.GetExceptionState());
    File* new_file = V8File::ToWrappable(scope.GetIsolate(), result);
    ASSERT_NE(new_file, nullptr);
...
public:
    TimeIntervalChecker()
        : start_time_(NowInMilliseconds())
    {
    }

    bool WasAliveAt(int64_t time_in_milliseconds)
    {
        return start_time_ <= time_in_milliseconds && time_in_milliseconds <= NowInMilliseconds();
    }
...
TEST(V8ScriptValueSerializerTest, DecodeFileWithoutHandles)
{
    ...
    V8TestingScope scope;
    scoped_refptr<SerializedScriptValue> input = SerializedValue(
        { 0xff, 0x08, 0x3f, 0x00, 0x66, 0x04, 'p', 'a', 't', 'h', 0x04, 'n', 'a', 'm', 'e', 0x03, 'r', 'e', 'l', 0x24, 'f', '4', 'a', '6', 'e', 'd', 'd', '5',
            '-', '6', '5', 'a', 'd', '-', '4', 'd', 'c', '3', '-', 'b', '6', '7', 'c', '-', 'a', '7', '7', '9', 'c', '0', '2', 'f', '0', 'f', 'a', '3', 0x0a,
            't', 'e', 'x', 't', '/', 'p', 'l', 'a', 'i', 'n', 0x01, 0x80, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xd0, 0xbf, 0x01, 0x00 });
    v8::Local<v8::Value> result = V8ScriptValueDeserializer(scope.GetScriptState(), input).Deserialize();
    EXPECT_TRUE(result->IsNull());
}
...
TEST(V8ScriptValueSerializerTest, DecodeFileIndex)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x09, 0x3f, 0x00, 0x65, 0x00 });
    WebBlobInfoArray blob_info_array;
    blob_info_array.emplace_back(WebBlobInfo::FileForTesting("d875dfc2-4505-461b-98fe-0cf6cc5eaf44", "path", "text/plain"));
    V8ScriptValueDeserializer::Options options;
    options.blob_info = &blob_info_array;
    V8ScriptValueDeserializer deserializer(scope.GetScriptState(), input, options);
    v8::Local<v8::Value> result = deserializer.Deserialize();
...
TEST(V8ScriptValueSerializerTest, DecodeFileIndexOutOfRange)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x09, 0x3f, 0x00, 0x65, 0x01 });
    {
        V8ScriptValueDeserializer deserializer(scope.GetScriptState(), input);
        ASSERT_TRUE(deserializer.Deserialize()->IsNull());
    }
...
TEST(V8ScriptValueSerializerTest, RoundTripFileList)
{
    ...
    file_list->Append(MakeGarbageCollected<File>(scope.GetExecutionContext(), "/native/path"));
    file_list->Append(MakeGarbageCollected<File>(scope.GetExecutionContext(), "/native/path2"));
    file_factory_helper.FlushForTesting();
    v8::Local<v8::Value> wrapper = ToV8Traits<FileList>::ToV8(scope.GetScriptState(), file_list);
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, scope.GetExceptionState());
...
TEST(V8ScriptValueSerializerTest, DecodeEmptyFileList)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x09, 0x3f, 0x00, 0x6c, 0x00 });
    v8::Local<v8::Value> result = V8ScriptValueDeserializer(scope.GetScriptState(), input).Deserialize();
    FileList* new_file_list = V8FileList::ToWrappable(scope.GetIsolate(), result);
    ASSERT_NE(new_file_list, nullptr);
...
TEST(V8ScriptValueSerializerTest, DecodeFileListWithInvalidLength)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x09, 0x3f, 0x00, 0x6c, 0x01 });
    v8::Local<v8::Value> result = V8ScriptValueDeserializer(scope.GetScriptState(), input).Deserialize();
    EXPECT_TRUE(result->IsNull());
}
...
TEST(V8ScriptValueSerializerTest, DecodeFileListWithoutHandles)
{
    ...
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x08, 0x3f, 0x00, 0x6c, 0x01, 0x04, 'p', 'a', 't', 'h', 0x04, 'n', 'a', 'm', 'e', 0x03,
        'r', 'e', 'l', 0x24, 'f', '4', 'a', '6', 'e', 'd', 'd', '5', '-', '6', '5', 'a', 'd', '-', '4', 'd', 'c', '3', '-', 'b', '6', '7', 'c', '-', 'a', '7',
        '7', '9', 'c', '0', '2', 'f', '0', 'f', 'a', '3', 0x0a, 't', 'e', 'x', 't', '/', 'p', 'l', 'a', 'i', 'n', 0x00, 0x00 });
    v8::Local<v8::Value> result = V8ScriptValueDeserializer(scope.GetScriptState(), input).Deserialize();
    EXPECT_TRUE(result->IsNull());
}
...
TEST(V8ScriptValueSerializerTest, DecodeEmptyFileListIndex)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x09, 0x3f, 0x00, 0x4c, 0x00 });
    WebBlobInfoArray blob_info_array;
    V8ScriptValueDeserializer::Options options;
    options.blob_info = &blob_info_array;
    V8ScriptValueDeserializer deserializer(scope.GetScriptState(), input, options);
...
TEST(V8ScriptValueSerializerTest, DecodeFileListIndexWithInvalidLength)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x09, 0x3f, 0x00, 0x4c, 0x02 });
    WebBlobInfoArray blob_info_array;
    V8ScriptValueDeserializer::Options options;
    options.blob_info = &blob_info_array;
    V8ScriptValueDeserializer deserializer(scope.GetScriptState(), input, options);
...
TEST(V8ScriptValueSerializerTest, DecodeFileListIndex)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x09, 0x3f, 0x00, 0x4c, 0x01, 0x00, 0x00 });
    WebBlobInfoArray blob_info_array;
    blob_info_array.emplace_back(WebBlobInfo::FileForTesting("d875dfc2-4505-461b-98fe-0cf6cc5eaf44", "name", "text/plain"));
    V8ScriptValueDeserializer::Options options;
    options.blob_info = &blob_info_array;
    V8ScriptValueDeserializer deserializer(scope.GetScriptState(), input, options);
    v8::Local<v8::Value> result = deserializer.Deserialize();
...
TEST(V8ScriptValueSerializerTest, DecodeHardcodedNullValue)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    EXPECT_TRUE(V8ScriptValueDeserializer(scope.GetScriptState(), SerializedScriptValue::NullValue()).Deserialize()->IsNull());
}
...
TEST(V8ScriptValueSerializerTest, DecodeWithInefficientVersionEnvelope)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x90, 0x00, 0xff, 0x09, 0x54 });
    EXPECT_TRUE(V8ScriptValueDeserializer(scope.GetScriptState(), std::move(input)).Deserialize()->IsTrue());
}
...
TEST(V8ScriptValueSerializerTest, RoundTripReadableStream)
{
    ...
    v8::Local<v8::Value> wrapper = ToV8Traits<ReadableStream>::ToV8(scope.GetScriptState(), rs);
    HeapVector<ScriptValue> transferable_array = { ScriptValue(isolate, wrapper) };
    Transferables transferables;
    ASSERT_TRUE(SerializedScriptValue::ExtractTransferables(isolate, transferable_array, transferables, ASSERT_NO_EXCEPTION));
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, ASSERT_NO_EXCEPTION, &transferables);
...
TEST(V8ScriptValueSerializerTest, TransformStreamIntegerOverflow)
{
    ...
    v8::Local<v8::Value> wrapper = ToV8Traits<TransformStream>::ToV8(scope.GetScriptState(), ts);
    HeapVector<ScriptValue> transferable_array = { ScriptValue(isolate, wrapper) };
    Transferables transferables;
    ASSERT_TRUE(SerializedScriptValue::ExtractTransferables(isolate, transferable_array, transferables, ASSERT_NO_EXCEPTION));

    // Extract message ports and disentangle them.
    Vector<MessagePortChannel> channels = MessagePort::DisentanglePorts(scope.GetExecutionContext(), transferables.message_ports, ASSERT_NO_EXCEPTION);

    V8ScriptValueSerializer::Options serialize_options;
    serialize_options.transferables = &transferables;
    V8ScriptValueSerializer serializer(script_state, serialize_options);
    scoped_refptr<SerializedScriptValue> serialized_script_value = serializer.Serialize(wrapper, ASSERT_NO_EXCEPTION);
    ...
    uint8_t serialized_value[] = { 0xff, 0x14, 0xff, 0x0d, 0x5c, 0x6d, 0xff, 0xff, 0xff, 0xff, 0x0f };

    auto corrupted_serialized_script_value = SerializedScriptValue::Create(serialized_value);
    corrupted_serialized_script_value->GetStreams() = std::move(serialized_script_value->GetStreams());

    // Entangle the message ports.
    MessagePortArray* transferred_message_ports = MessagePort::EntanglePorts(*scope.GetExecutionContext(), std::move(channels));

    UnpackedSerializedScriptValue* unpacked = SerializedScriptValue::Unpack(std::move(corrupted_serialized_script_value));
    V8ScriptValueDeserializer::Options deserialize_options;
    deserialize_options.message_ports = transferred_message_ports;
    V8ScriptValueDeserializer deserializer(script_state, unpacked, deserialize_options);
...
TEST(V8ScriptValueSerializerTest, RoundTripDOMException)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    DOMException* exception = DOMException::Create("message", "InvalidStateError");
    v8::Local<v8::Value> wrapper = ToV8Traits<DOMException>::ToV8(scope.GetScriptState(), exception);
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, scope.GetExceptionState());
    DOMException* new_exception = V8DOMException::ToWrappable(scope.GetIsolate(), result);
...
TEST(V8ScriptValueSerializerTest, DecodeDOMExceptionWithInvalidNameString)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x13, 0xff, 0x0d, 0x5c, 0x78, 0x01, 0xff, 0x00, 0x00 });
    v8::Local<v8::Value> result = V8ScriptValueDeserializer(scope.GetScriptState(), input).Deserialize();
    EXPECT_TRUE(result->IsNull());
}
...
TEST(V8ScriptValueSerializerTest, NoSharedValueConveyor)
{
    test::TaskEnvironment task_environment;
    V8TestingScope scope;
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x14, 0xff, 0x0f, 'p', 0x00 });
    v8::Local<v8::Value> result = V8ScriptValueDeserializer(scope.GetScriptState(), input).Deserialize();
    EXPECT_TRUE(result->IsNull());
}
...
TEST(V8ScriptValueSerializerTest, CanDeserializeIn_OldValues)
{
    ...
    // CanDeserializeIn.
    V8TestingScope scope;
    scoped_refptr<SerializedScriptValue> input = SerializedValue({ 0xff, 0x09, 'T', 0x00 });
...
TEST(V8ScriptValueSerializerTest, RoundTripFencedFrameConfig)
{
    ...
    V8TestingScope scope;
    FencedFrameConfig* config = FencedFrameConfig::Create(KURL("https://example.com"), "some shared storage context",
        KURL("urn:uuid:37665e6f-f3fd-4393-8429-719d02843a54"), gfx::Size(64, 48), gfx::Size(32, 16), FencedFrameConfig::AttributeVisibility::kOpaque, true);
    v8::Local<v8::Value> wrapper = ToV8Traits<FencedFrameConfig>::ToV8(scope.GetScriptState(), config);
    v8::Local<v8::Value> result = RoundTrip(wrapper, scope, scope.GetExceptionState());
    FencedFrameConfig* new_config = V8FencedFrameConfig::ToWrappable(scope.GetIsolate(), result);
    ASSERT_NE(new_config, nullptr);
    EXPECT_NE(config, new_config);
    EXPECT_EQ(config->url_, new_config->url_);
    EXPECT_EQ(config->shared_storage_context_, new_config->shared_storage_context_);
    ...
    EXPECT_EQ(config->url_attribute_visibility_, new_config->url_attribute_visibility_);
    EXPECT_EQ(config->deprecated_should_freeze_initial_size_, new_config->deprecated_should_freeze_initial_size_);
}
...
TEST(V8ScriptValueSerializerTest, CoexistWithGin)
{
    ...
    scoped_refptr<SerializedScriptValue> serialized_script_value
        = V8ScriptValueSerializer(scope.GetScriptState()).Serialize(wrapper, PassThroughException(scope.GetIsolate()));
