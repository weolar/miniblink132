...
public:
    ...
    SerializedImageDataSettings(SerializedPredefinedColorSpace, SerializedImageDataStorageFormat);
...
private:
    SerializedPredefinedColorSpace color_space_ = SerializedPredefinedColorSpace::kSRGB;
    SerializedImageDataStorageFormat storage_format_ = SerializedImageDataStorageFormat::kUint8Clamped;
};
...
public:
    ...
    SerializedImageBitmapSettings(SerializedPredefinedColorSpace, const Vector<double>& sk_color_space, SerializedPixelFormat, SerializedOpacityMode,
        uint32_t is_premultiplied, SerializedImageOrientation);

    SkImageInfo GetSkImageInfo(uint32_t width, uint32_t height) const;
    ImageOrientationEnum GetImageOrientation() const;

    const Vector<double>& GetSerializedSkColorSpace()
    {
    ...
    uint32_t IsPremultiplied() const
    {
...
private:
    ...
    SerializedImageOrientation image_orientation_ = SerializedImageOrientation::kTopLeft;
};
