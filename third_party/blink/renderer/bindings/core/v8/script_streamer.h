...
public:
    ...
    virtual v8::ScriptCompiler::StreamedSource* Source(v8::ScriptType expected_type) = 0;
    ...
    static void RecordStreamingHistogram(ScriptSchedulingType type, bool can_use_streamer, ScriptStreamer::NotStreamingReason);

    // Returns false if we cannot stream the given encoding.
    static bool ConvertEncoding(const AtomicString& encoding_name, v8::ScriptCompiler::StreamedSource::Encoding*);
    ...
    static std::tuple<ScriptStreamer*, NotStreamingReason> TakeFrom(ScriptResource* resource, mojom::blink::ScriptType expected_type);
};
...
public:
    ResourceScriptStreamer(ScriptResource* resource, mojo::ScopedDataPipeConsumerHandle data_pipe, ResponseBodyLoaderClient* response_body_loader_client,
        std::unique_ptr<TextResourceDecoder> decoder, scoped_refptr<base::SingleThreadTaskRunner> loading_task_runner);
    ...
    v8::ScriptCompiler::StreamedSource* Source(v8::ScriptType expected_type) override
    {
    ...
    const String& ScriptURLString() const
    {
        return script_url_string_;
    }
    ...
    v8_compile_hints::V8LocalCompileHintsConsumer* GetV8LocalCompileHintsConsumerForTest() const;
...
private:
    ...
    static void RunScriptStreamingTask(std::unique_ptr<v8::ScriptCompiler::ScriptStreamingTask> task, ResourceScriptStreamer* streamer, SourceStream* stream);

    void OnDataPipeReadable(MojoResult result, const mojo::HandleSignalsState& state);
    ...
    void LoadCompleteWithoutStreaming(LoadingState loading_state, NotStreamingReason no_streaming_reason);
...
class CORE_EXPORT BackgroundInlineScriptStreamer final : public WTF::ThreadSafeRefCounted<BackgroundInlineScriptStreamer> {
public:
    BackgroundInlineScriptStreamer(v8::Isolate* isolate, const String& text, v8::ScriptCompiler::CompileOptions compile_options);

    void Run();
    bool IsStarted() const
    {
        return started_.IsSet();
    }
    void Cancel()
    {
        cancelled_.Set();
...
public:
    static InlineScriptStreamer* From(scoped_refptr<BackgroundInlineScriptStreamer> streamer);

    explicit InlineScriptStreamer(scoped_refptr<BackgroundInlineScriptStreamer> streamer)
...
public:
    ...
    public:
        ...
        Result(String decoded_data, std::unique_ptr<ParkableStringImpl::SecureDigest> digest,
    ...
    BackgroundResourceScriptStreamer(const BackgroundResourceScriptStreamer&) = delete;
    BackgroundResourceScriptStreamer& operator=(const BackgroundResourceScriptStreamer&) = delete;
    ...
    std::unique_ptr<BackgroundResponseProcessorFactory> CreateBackgroundResponseProcessorFactory();
    ...
    std::unique_ptr<v8::ScriptCompiler::ConsumeCodeCacheTask> TakeConsumeCodeCacheTask();

private:
    class BackgroundProcessor;
    class BackgroundProcessorFactory;

    void OnResult(std::unique_ptr<Result> result, NotStreamingReason suppressed_reason);
