...
V8LocalCompileHintsConsumer::V8LocalCompileHintsConsumer(CachedMetadata* cached_metadata)
{
    ...
    if (!reader.Skip(kLocalCompileHintsPrefixSize) || reader.remaining() % sizeof(int32_t) != 0) {
        rejected_ = true;
        return;
    }
...
{
    auto* v8_local_compile_hints_consumer = reinterpret_cast<V8LocalCompileHintsConsumer*>(data);
    return v8_local_compile_hints_consumer->GetCompileHint(pos);
}

bool V8LocalCompileHintsConsumer::GetCompileHint(int pos)
{
    while (current_index_ < compile_hints_.size() && compile_hints_[current_index_] < pos) {
        ++current_index_;
    }
    if (current_index_ >= compile_hints_.size() || compile_hints_[current_index_] > pos) {
        return false;
    }
