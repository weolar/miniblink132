...
template <typename T> struct ImplTypeFor {
    using type = T;
};
...
template <typename T> struct NativeValueTraitsBase {
    ...
    static constexpr bool has_null_value = std::is_pointer_v<ImplType> || requires(ImplType value) { value.IsNull(); };
    ...
    static constexpr bool supports_scriptwrappable_specific_fast_array_iteration = false;

    template <typename... ExtraArgs>
    static decltype(auto) ArgumentValue(
        v8::Isolate* isolate, int argument_index, v8::Local<v8::Value> value, ExceptionState& exception_state, ExtraArgs... extra_args)
    {
        return NativeValueTraits<std::remove_pointer_t<T>>::NativeValue(isolate, value, exception_state, std::forward<ExtraArgs>(extra_args)...);
    }
};
