...
void WindowProxyManager::ReleaseGlobalProxies(GlobalProxyVector& global_proxies)
{
    ...
    for (auto& entry : isolated_worlds_) {
        ...
        global_proxies.proxies.push_back(WindowProxyMaybeUninitialized(entry.value->World())->ReleaseGlobalProxy());
    }
}

void WindowProxyManager::SetGlobalProxies(const GlobalProxyVector& global_proxies)
{
    DCHECK_EQ(global_proxies.worlds.size(), global_proxies.proxies.size());
    const wtf_size_t size = global_proxies.worlds.size();
    for (wtf_size_t i = 0; i < size; ++i) {
        WindowProxyMaybeUninitialized(*global_proxies.worlds[i])->SetGlobalProxy(global_proxies.proxies[i]);
    }
    ...
    for (wtf_size_t i = 0; i < size; ++i) {
        WindowProxyMaybeUninitialized(*global_proxies.worlds[i])->InitializeIfNeeded();
    }
}
...
WindowProxyManager::WindowProxyManager(v8::Isolate* isolate, Frame& frame, FrameType frame_type)
    : isolate_(isolate)
    , frame_(&frame)
...
WindowProxy* WindowProxyManager::CreateWindowProxy(DOMWrapperWorld& world)
{
    ...
    case FrameType::kLocal:
        ...
        // RemoteFrame.
        return MakeGarbageCollected<LocalWindowProxy>(isolate_, *static_cast<LocalFrame*>(frame_.Get()), &world);
    case FrameType::kRemote:
        return MakeGarbageCollected<RemoteWindowProxy>(isolate_, *static_cast<RemoteFrame*>(frame_.Get()), &world);
    }
    NOTREACHED();
}

WindowProxy* WindowProxyManager::WindowProxyMaybeUninitialized(DOMWrapperWorld& world)
{
...
void LocalWindowProxyManager::UpdateSecurityOrigin(const SecurityOrigin* security_origin)
{
    To<LocalWindowProxy>(window_proxy_.Get())->UpdateSecurityOrigin(security_origin);

    for (auto& entry : isolated_worlds_) {
        auto* isolated_window_proxy = To<LocalWindowProxy>(entry.value.Get());
        scoped_refptr<SecurityOrigin> isolated_security_origin = isolated_window_proxy->World().IsolatedWorldSecurityOrigin(security_origin->AgentClusterId());
        isolated_window_proxy->UpdateSecurityOrigin(isolated_security_origin.get());
    }
}

void LocalWindowProxyManager::SetAbortScriptExecution(v8::Context::AbortScriptExecutionCallback callback)
{
    v8::HandleScope handle_scope(GetIsolate());

    static_cast<LocalWindowProxy*>(window_proxy_.Get())->SetAbortScriptExecution(callback);
