...
protected:
    ...
    {
        v8::Local<v8::String> source = v8::String::NewFromUtf8(script_state->GetIsolate(), s, v8::NewStringType::kNormal).ToLocalChecked();
        v8::Local<v8::Script> script = v8::Script::Compile(script_state->GetContext(), source).ToLocalChecked();
        v8::Local<v8::Value> value = script->Run(script_state->GetContext()).ToLocalChecked();
...
{
    V8TestingScope scope;
    Dictionary dictionary = CreateDictionary(scope.GetScriptState(), "({foo: 3})");
...
{
    V8TestingScope scope;
    Dictionary dictionary = CreateDictionary(scope.GetScriptState(), "({foo: undefined})");
...
{
    V8TestingScope scope;
    Dictionary dictionary = CreateDictionary(scope.GetScriptState(), "({get foo() { return 'xy'; }})");
...
{
    V8TestingScope scope;
    Dictionary dictionary = CreateDictionary(scope.GetScriptState(), "({get foo() { throw Error(2); }})");
...
{
    V8TestingScope scope;
    Dictionary dictionary = CreateDictionary(scope.GetScriptState(), "({foo: 4})");
...
{
    V8TestingScope scope;
    Dictionary dictionary = CreateDictionary(scope.GetScriptState(), "({foo: { toString() { return 'hello'; } } })");
...
{
    V8TestingScope scope;
    Dictionary dictionary = CreateDictionary(scope.GetScriptState(), "({get foo() { return { toString() { throw Error(88); } };} })");
...
{
    V8TestingScope scope;
    Dictionary dictionary = CreateDictionary(scope.GetScriptState(), "({foo: NaN})");
