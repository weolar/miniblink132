...
void V8HTMLConstructor::HtmlConstructor(
    const v8::FunctionCallbackInfo<v8::Value>& info, const WrapperTypeInfo& wrapper_type_info, const HTMLElementType element_interface_name)
{
    ...
    v8::Local<v8::Function> active_function_object = script_state->PerContextData()->ConstructorForType(&wrapper_type_info);
    ...
    if (RuntimeEnabledFeatures::ScopedCustomElementRegistryEnabled()) {
        ...
        CustomElementConstructionStack* construction_stack = GetCustomElementConstructionStack(window, constructor);
        if (construction_stack && construction_stack->size()) {
            definition = construction_stack->back().definition;
        }
    }
    if (!definition) {
        definition = window->customElements()->DefinitionForConstructor(constructor);
    }
    if (!definition) {
        V8ThrowException::ThrowTypeError(isolate, "Illegal constructor");
        return;
    ...
    if (local_name == name) {
        ...
        if (HtmlElementTypeForTag(local_name, window->document()) != element_interface_name) {
            V8ThrowException::ThrowTypeError(isolate,
    ...
    if (!new_target.As<v8::Object>()->Get(script_state->GetContext(), prototype_string).ToLocal(&prototype)) {
        return;
    }

    // 7. If Type(prototype) is not Object, then: ...
    if (!prototype->IsObject()) {
        ScriptState* new_target_script_state = ScriptState::ForRelevantRealm(isolate, new_target.As<v8::Object>());
        if (V8PerContextData* per_context_data = new_target_script_state->PerContextData()) {
    ...
    if (!construction_stack || construction_stack->empty()) {
        ...
        if (element) {
            ...
            // element.
            V8ThrowException::ThrowTypeError(isolate, "This instance is already constructed");
            return;
        }
    }
    const WrapperTypeInfo* wrapper_type = element->GetWrapperTypeInfo();
    v8::Local<v8::Object> wrapper = V8DOMWrapper::AssociateObjectWithWrapper(isolate, element, wrapper_type, info.This());
    ...
    // example.
    v8::Maybe<bool> maybe_result = wrapper->SetPrototype(script_state->GetContext(), prototype.As<v8::Object>());
    bool success;
    ...
    if (!success) {
        ...
        V8ThrowException::ThrowTypeError(isolate, "Unable to call SetPrototype on this element");
        return;
    }
}
} // namespace blink
