...
class IsolatedWorldCSPDelegate final : public GarbageCollected<IsolatedWorldCSPDelegate>, public ContentSecurityPolicyDelegate {

public:
    IsolatedWorldCSPDelegate(LocalDOMWindow& window, scoped_refptr<SecurityOrigin> security_origin, int32_t world_id, CSPType type)
        : window_(&window)
    ...
    {
        window_->GetScriptController().DisableEvalForIsolatedWorld(world_id_, error_message);
    }

    void SetWasmEvalErrorMessage(const String& error_message) override
    {
        window_->GetScriptController().SetWasmEvalErrorMessageForIsolatedWorld(world_id_, error_message);
    }
...
void IsolatedWorldCSP::SetContentSecurityPolicy(int32_t world_id, const String& policy, scoped_refptr<SecurityOrigin> self_origin)
{
    DCHECK(IsMainThread());
...
ContentSecurityPolicy* IsolatedWorldCSP::CreateIsolatedWorldCSP(LocalDOMWindow& window, int32_t world_id)
{
    DCHECK(IsMainThread());
    DCHECK(DOMWrapperWorld::IsIsolatedWorldId(world_id));
    ...
    IsolatedWorldCSPDelegate* delegate
        = MakeGarbageCollected<IsolatedWorldCSPDelegate>(window, self_origin, world_id, policy.empty() ? CSPType::kEmpty : CSPType::kNonEmpty);
    csp->BindToDelegate(*delegate);
    csp->AddPolicies(ParseContentSecurityPolicies(
        policy, network::mojom::blink::ContentSecurityPolicyType::kEnforce, network::mojom::blink::ContentSecurityPolicySource::kHTTP, *(self_origin)));
