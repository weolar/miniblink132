...
{
    if (IsMainThread()) {
        InstanceCounters::IncrementCounter(InstanceCounters::kJSEventListenerCounter);
    }
}
...
{
    if (IsMainThread()) {
        InstanceCounters::DecrementCounter(InstanceCounters::kJSEventListenerCounter);
    }
}

bool JSBasedEventListener::BelongsToTheCurrentWorld(ExecutionContext* execution_context) const
{
    v8::Isolate* isolate = GetIsolate();
    if (!isolate->GetCurrentContext().IsEmpty() && &GetWorld() == &DOMWrapperWorld::Current(isolate))
        return true;
    ...
    if (!isolate->InContext() && execution_context && IsA<LocalDOMWindow>(execution_context)) {
...
void JSBasedEventListener::Invoke(ExecutionContext* execution_context_of_event_target, Event* event)
{
    ...
    // listener's world.
    ScriptState* script_state_of_event_target = ToScriptState(execution_context_of_event_target, GetWorld());
    if (!script_state_of_event_target) {
        return;
    }
    DCHECK_EQ(script_state_of_event_target->World().GetWorldId(), GetWorld().GetWorldId());
    ...
    if (!BindingSecurity::ShouldAllowAccessToV8Context(script_state_of_listener, script_state_of_event_target)) {
        LocalDOMWindow* target_window = DynamicTo<LocalDOMWindow>(execution_context_of_event_target);
        if (window && target_window) {
            window->PrintErrorMessage(target_window->CrossDomainAccessErrorMessage(window, DOMWindow::CrossDocumentAccessPolicy::kDisallowed));
        }
        return;
    }

    v8::Local<v8::Value> js_event = ToV8Traits<Event>::ToV8(script_state_of_event_target, event);
...
std::unique_ptr<SourceLocation> JSBasedEventListener::GetSourceLocation(EventTarget& target)
{
    v8::HandleScope handle_scope(GetIsolate());
    v8::Local<v8::Value> effective_function = GetEffectiveFunction(target);
    if (effective_function->IsFunction()) {
        return CaptureSourceLocation(GetIsolate(), effective_function.As<v8::Function>());
    }
    return nullptr;
}
