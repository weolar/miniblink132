...
enum class TestEnum { kOk = 0, kFailedWithReason = 1, kTimedOut = 2, kMaxValue = kTimedOut };
...
public:
    ...
    ScriptPromiseResolverWithTracker<TestEnum, IDLString>* CreateResultTracker(
        String& on_fulfilled, String& on_rejected, base::TimeDelta timeout_delay = base::Minutes(1))
    {
        ScriptState::Scope scope(GetScriptState());
        auto* result_tracker
            = MakeGarbageCollected<ScriptPromiseResolverWithTracker<TestEnum, IDLString>>(GetScriptState(), metric_name_prefix_, timeout_delay);

        result_tracker->Promise().React(
            GetScriptState(), MakeGarbageCollected<TestResolveFunction>(&on_fulfilled), MakeGarbageCollected<TestRejectFunction>(&on_rejected));
    ...
    void CheckResultHistogram(int expected_count, const std::string& result_string = "Result")
    {
        histogram_tester_.ExpectTotalCount(base::StrCat({ metric_name_prefix_, ".", result_string }), expected_count);
    }

    void CheckLatencyHistogram(int expected_count)
    {
        histogram_tester_.ExpectTotalCount(metric_name_prefix_ + ".Latency", expected_count);
    }
...
TEST_F(ScriptPromiseResolverWithTrackerTest, timeout)
{
    ...
    auto* result_tracker = CreateResultTracker(on_fulfilled, on_rejected, timeout_delay);
