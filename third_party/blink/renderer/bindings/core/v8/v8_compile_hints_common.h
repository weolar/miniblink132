...
enum class MagicCommentMode { kNever = 0, kWhenProducingCodeCache = 1, kAlways = 2 };

inline MagicCommentMode GetMagicCommentMode(ExecutionContext* execution_context)
{
    if (RuntimeEnabledFeatures::JavaScriptCompileHintsMagicAlwaysRuntimeEnabled(execution_context)) {
        return MagicCommentMode::kAlways;
    }
    if (RuntimeEnabledFeatures::JavaScriptCompileHintsMagicRuntimeEnabled(execution_context)) {
...
uint32_t ScriptNameHash(v8::Local<v8::Value> name_value, v8::Local<v8::Context> context, v8::Isolate* isolate);
