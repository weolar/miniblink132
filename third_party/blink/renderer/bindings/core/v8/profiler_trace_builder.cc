...
ProfilerTrace* ProfilerTraceBuilder::FromProfile(
    ScriptState* script_state, const v8::CpuProfile* profile, const SecurityOrigin* allowed_origin, base::TimeTicks time_origin)
{
    TRACE_EVENT0("blink", "ProfilerTraceBuilder::FromProfile");
    ProfilerTraceBuilder* builder = MakeGarbageCollected<ProfilerTraceBuilder>(script_state, allowed_origin, time_origin);
    if (profile) {
        for (int i = 0; i < profile->GetSamplesCount(); i++) {
            const auto* node = profile->GetSample(i);
            auto timestamp = base::TimeTicks() + base::Microseconds(profile->GetSampleTimestamp(i));
...
ProfilerTraceBuilder::ProfilerTraceBuilder(ScriptState* script_state, const SecurityOrigin* allowed_origin, base::TimeTicks time_origin)
...
void ProfilerTraceBuilder::AddSample(
    const v8::CpuProfileNode* node, base::TimeTicks timestamp, const v8::StateTag state, const v8::EmbedderStateTag embedder_state)
{
    ...
    // origin isolated".
    auto relative_timestamp = Performance::MonotonicTimeToDOMHighResTimeStamp(time_origin_, timestamp, /*allow_negative_value=*/true,
    ...
    if (std::optional<blink::V8ProfilerMarker> marker = BlinkStateToMarker(embedder_state, state)) {
...
std::optional<wtf_size_t> ProfilerTraceBuilder::GetOrInsertStackId(const v8::CpuProfileNode* node)
{
    if (!node)
    ...
    if (std::optional<int> parent_stack_id = GetOrInsertStackId(node->GetParent())) {
...
wtf_size_t ProfilerTraceBuilder::GetOrInsertFrameId(const v8::CpuProfileNode* node)
{
    ...
    if (*node->GetScriptResourceNameStr() != '\0') {
        wtf_size_t resource_id = GetOrInsertResourceId(node->GetScriptResourceNameStr());
...
wtf_size_t ProfilerTraceBuilder::GetOrInsertResourceId(const char* resource_name)
{
...
bool ProfilerTraceBuilder::ShouldIncludeStackFrame(const v8::CpuProfileNode* node)
{
    DCHECK(node);

    // Omit V8 metadata frames.
    const v8::CpuProfileNode::SourceType source_type = node->GetSourceType();
    if (source_type != v8::CpuProfileNode::kScript && source_type != v8::CpuProfileNode::kBuiltin && source_type != v8::CpuProfileNode::kCallback) {
        return false;
    }
    ...
    if (source_type != v8::CpuProfileNode::kScript) {
        while (resource_node && resource_node->GetScriptId() == v8::UnboundScript::kNoScriptId) {
    ...
    bool allowed = resource_node->IsScriptSharedCrossOrigin() || origin->IsSameOriginWith(allowed_origin_);
