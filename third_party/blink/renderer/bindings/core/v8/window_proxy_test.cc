...
public:
    explicit DidClearWindowObjectCounter(int& counter)
        : counter_(counter)
    {
    }
...
public:
    ...
    {
        return std::make_unique<DidClearWindowObjectCounter>(did_clear_window_object_count_);
    }
...
TEST_F(WindowProxyTest, NotInitializedIfNoScript)
{
    ...
    main_resource.Complete(R"HTML(<!DOCTYPE html><html><body></body></html>)HTML");
...
TEST_F(WindowProxyTest, NamedItem)
{
    SimRequest main_resource("https://example.com/index.html", "text/html");
    LoadURL("https://example.com/index.html");
    main_resource.Complete(R"HTML(<!DOCTYPE html><html><body><iframe name="x"></iframe></body></html>)HTML");
...
TEST_F(WindowProxyTest, IsolatedWorldReinitializedAfterNavigation)
{
    ...
    v8::Local<v8::Value> window_top
        = MainFrame().ExecuteScriptInIsolatedWorldAndReturnValue(kIsolatedWorldId, WebScriptSource("window"), BackForwardCacheAware::kAllow);
    ...
    v8::Local<v8::Value> saved_child_window
        = MainFrame().ExecuteScriptInIsolatedWorldAndReturnValue(kIsolatedWorldId, WebScriptSource("saved = window[0]"), BackForwardCacheAware::kAllow);
    ASSERT_TRUE(saved_child_window->IsObject());

    frame_test_helpers::LoadFrame(MainFrame().FirstChild()->ToWebLocalFrame(), "data:text/html,<body><p>Hello</p></body>");
    ASSERT_TRUE(MainFrame().FirstChild());
    ...
    v8::Local<v8::Value> top_via_saved
        = MainFrame().ExecuteScriptInIsolatedWorldAndReturnValue(kIsolatedWorldId, WebScriptSource("saved.top"), BackForwardCacheAware::kAllow);
