...
template <typename IDLType, typename ResolverType = std::conditional_t<std::is_same_v<IDLUndefined, IDLType>, IDLUndefined, IDLSequence<IDLType>>>
class CORE_EXPORT PromiseAll final : public GarbageCollected<PromiseAll<IDLType>> {
public:
    // PromiseAll behaves similarly to JS Promise.all() - resolving
    // when all of the given promises have fulfilled, or any rejects.
    static ScriptPromise<ResolverType> Create(ScriptState* script_state, const HeapVector<ScriptPromise<IDLType>>& promises)
    {
        if (promises.empty()) {
        ...
        auto* resolver = MakeGarbageCollected<ScriptPromiseResolver<ResolverType>>(script_state);
        MakeGarbageCollected<PromiseAll<IDLType>>(script_state, promises, resolver);
        return resolver->Promise();
    }

    PromiseAll(ScriptState* script_state, const HeapVector<ScriptPromise<IDLType>>& promises, ScriptPromiseResolver<ResolverType>* resolver)
        : number_of_pending_promises_(promises.size())
        , values_(promises.size())
        , resolver_(resolver)
    {
        for (wtf_size_t i = 0; i < number_of_pending_promises_; ++i) {
            promises[i].React(script_state, MakeGarbageCollected<Resolve>(i, this), MakeGarbageCollected<Reject>(this));
        }
    }
...
private:
    using BlinkType
        = std::conditional_t<WTF::IsGarbageCollectedType<IDLType>::value, std::add_pointer_t<IDLType>, typename IDLTypeToBlinkImplType<IDLType>::type>;
    using VectorType = HeapVector<AddMemberIfNeeded<typename IDLTypeToBlinkImplType<IDLType>::type>>;

    class Resolve final : public ThenCallable<IDLType, Resolve> {
    public:
        Resolve(wtf_size_t index, PromiseAll* all)
            : index_(index)
            , all_(all)
        {
        }
    ...
    public:
        explicit Reject(PromiseAll* all)
            : all_(all)
        {
        }
