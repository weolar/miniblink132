...
v8::Local<v8::Value> JSEventListener::GetEffectiveFunction(EventTarget& target)
{
    ...
    if (v8_listener->IsObject()) {
        ...
        // getter throws.
        if (v8_listener.As<v8::Object>()->Get(isolate->GetCurrentContext(), V8AtomicString(isolate, "handleEvent")).ToLocal(&property)
            && property->IsFunction()) {
...
void JSEventListener::InvokeInternal(EventTarget&, Event& event, v8::Local<v8::Value> js_event)
{
    ...
    if (!event_listener_->IsRunnableOrThrowException(
            event.ShouldDispatchEvenWhenExecutionContextIsPaused() ? V8EventListener::IgnorePause::kIgnore : V8EventListener::IgnorePause::kDontIgnore)) {
        return;
    }
    [[maybe_unused]] v8::Maybe<void> maybe_result = event_listener_->InvokeWithoutRunnabilityCheck(event.currentTarget(), &event);
}
