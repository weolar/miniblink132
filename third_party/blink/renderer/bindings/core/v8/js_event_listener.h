...
public:
    ...
    bool Matches(const EventListener& other) const override
    {
        ...
        return other_listener && event_listener_->HasTheSameCallbackObject(*other_listener->event_listener_);
    }
    ...
    bool IsJSEventListener() const override
    {
        return true;
    }
...
protected:
    ...
    {
        return event_listener_->CallbackRelevantScriptStateOrReportError("EventListener", operation);
    }
...
template <> struct DowncastTraits<JSEventListener> {
