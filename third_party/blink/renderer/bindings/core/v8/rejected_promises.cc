...
public:
    Message(ScriptState* script_state, v8::Local<v8::Promise> promise, v8::Local<v8::Value> exception, const String& error_message,
        std::unique_ptr<SourceLocation> location, SanitizeScriptErrors sanitize_script_errors)
    ...
    bool IsCollected()
    {
        return collected_ || !script_state_->ContextIsValid();
    }
    ...
    void Report()
    {
        ...
        v8::Local<v8::Promise> promise = promise_.NewLocal(script_state_->GetIsolate());
        v8::Local<v8::Value> reason = exception_.NewLocal(script_state_->GetIsolate());
        if (promise.IsEmpty()) {
            return;
        }
        DCHECK(!HasHandler());
        ...
        if (target && sanitize_script_errors_ == SanitizeScriptErrors::kDoNotSanitize) {
            PromiseRejectionEventInit* init = PromiseRejectionEventInit::Create();
            init->setPromise(ScriptPromise<IDLAny>::FromV8Promise(script_state_->GetIsolate(), promise));
            init->setReason(ScriptValue(script_state_->GetIsolate(), reason));
            init->setCancelable(true);
            PromiseRejectionEvent* event = PromiseRejectionEvent::Create(script_state_, event_type_names::kUnhandledrejection, init);
            // Log to console if event was not canceled.
            should_log_to_console_ = target->DispatchEvent(*event) == DispatchEventResult::kNotCanceled;
        }

        if (should_log_to_console_) {
            ThreadDebugger* debugger = ThreadDebugger::From(script_state_->GetIsolate());
            if (debugger) {
                promise_rejection_id_ = debugger->PromiseRejected(script_state_->GetContext(), error_message_, reason, std::move(location_));
            }
        }
    ...
    void Revoke()
    {
        ...
        if (target && sanitize_script_errors_ == SanitizeScriptErrors::kDoNotSanitize) {
            ...
            init->setPromise(ScriptPromise<IDLAny>::FromV8Promise(script_state_->GetIsolate(), promise));
            init->setReason(ScriptValue(script_state_->GetIsolate(), reason));
            PromiseRejectionEvent* event = PromiseRejectionEvent::Create(script_state_, event_type_names::kRejectionhandled, init);
        ...
        if (should_log_to_console_ && promise_rejection_id_) {
            ThreadDebugger* debugger = ThreadDebugger::From(script_state_->GetIsolate());
            if (debugger) {
                debugger->PromiseRejectionRevoked(script_state_->GetContext(), promise_rejection_id_);
            }
        }
    }
...
void RejectedPromises::RejectedWithNoHandler(ScriptState* script_state, v8::PromiseRejectMessage data, const String& error_message,
    std::unique_ptr<SourceLocation> location, SanitizeScriptErrors sanitize_script_errors)
{
    queue_.push_back(std::make_unique<Message>(script_state, data.GetPromise(), data.GetValue(), error_message, std::move(location), sanitize_script_errors));
}
    ...
    for (wtf_size_t i = 0; i < reported_as_errors_.size(); ++i) {
        ...
        if (!message->IsCollected() && message->HasPromise(data.GetPromise())) {
            ...
            context->GetTaskRunner(TaskType::kDOMManipulation)
                ->PostTask(FROM_HERE, WTF::BindOnce(&RejectedPromises::RevokeNow, scoped_refptr<RejectedPromises>(this), std::move(message)));
...
void RejectedPromises::ProcessQueue()
{
    ...
    for (auto& kv : queues) {
        kv.key->GetTaskRunner(blink::TaskType::kDOMManipulation)
            ->PostTask(FROM_HERE, WTF::BindOnce(&RejectedPromises::ProcessQueueNow, scoped_refptr<RejectedPromises>(this), std::move(kv.value)));
    }
}
...
void RejectedPromises::ProcessQueueNow(MessageQueue queue)
{
    ...
    auto new_end = std::remove_if(reported_as_errors_.begin(), reported_as_errors_.end(), [](const auto& message) { return message->IsCollected(); });
    reported_as_errors_.Shrink(static_cast<wtf_size_t>(new_end - reported_as_errors_.begin()));
    ...
    for (auto& message : queue) {
        ...
        if (!message->HasHandler()) {
            ...
            if (reported_as_errors_.size() > kMaxReportedHandlersPendingResolution) {
                reported_as_errors_.EraseAt(0, kMaxReportedHandlersPendingResolution / 10);
            }
        }
    }
}
