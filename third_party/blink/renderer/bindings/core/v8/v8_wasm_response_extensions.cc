...
void SendCachedData(
    String response_url, base::Time response_time, String cache_storage_cache_name, ExecutionContext* execution_context, Vector<uint8_t> serialized_module)
{
    if (!execution_context)
        return;
    scoped_refptr<CachedMetadata> cached_metadata = CachedMetadata::CreateFromSerializedData(std::move(serialized_module));

    CodeCacheHost* code_cache_host = ExecutionContext::GetCodeCacheHostFromContext(execution_context);
    CachedMetadataSender::SendToCodeCacheHost(
        code_cache_host, mojom::blink::CodeCacheType::kWebAssembly, response_url, response_time, cache_storage_cache_name, cached_metadata->SerializedData());
}

class WasmCodeCachingCallback {
public:
    WasmCodeCachingCallback(const String& response_url, const base::Time& response_time, const String& cache_storage_cache_name,
        scoped_refptr<base::SingleThreadTaskRunner> task_runner, ExecutionContext* execution_context)
    ...
    void OnMoreFunctionsCanBeSerialized(v8::CompiledWasmModule compiled_module)
    {
        ...
        TRACE_EVENT_INSTANT1(TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "v8.wasm.compiledModule", TRACE_EVENT_SCOPE_THREAD, "url", response_url_.Utf8());
        ...
        TRACE_EVENT_INSTANT1(
            TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "v8.wasm.cachedModule", TRACE_EVENT_SCOPE_THREAD, "producedCacheSize", serialized_module.size);

        v8::MemorySpan<const uint8_t> wire_bytes = compiled_module.GetWireBytesRef();
        DigestValue wire_bytes_digest;
        {
            TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "v8.wasm.compileDigestForCreate");
            if (!ComputeDigest(kHashAlgorithmSha256, base::span<const uint8_t>(wire_bytes.data(), wire_bytes.size()), wire_bytes_digest)) {
                return;
            }
        ...
        Vector<uint8_t> serialized_data
            = CachedMetadata::GetSerializedDataHeader(kWasmModuleTag, kWireBytesDigestSize + base::checked_cast<wtf_size_t>(serialized_module.size));
        serialized_data.AppendSpan(base::span(wire_bytes_digest));
        serialized_data.Append(reinterpret_cast<const uint8_t*>(serialized_module.buffer.get()), base::checked_cast<wtf_size_t>(serialized_module.size));
        ...
        execution_context_task_runner_->PostTask(FROM_HERE,
            ConvertToBaseOnceCallback(WTF::CrossThreadBindOnce(
                &SendCachedData, response_url_, response_time_, cache_storage_cache_name_, execution_context_, std::move(serialized_data))));
    }
...
class FetchDataLoaderForWasmStreaming final : public FetchDataLoader, public BytesConsumer::Client {
public:
    FetchDataLoaderForWasmStreaming(const String& url, std::shared_ptr<v8::WasmStreaming> streaming, ScriptState* script_state,
        ScriptCachedMetadataHandler* cache_handler, std::shared_ptr<WasmCodeCachingCallback> code_caching_callback)
        : url_(url)
    ...
    v8::WasmStreaming* streaming() const
    {
    ...
    {
        TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "v8.wasm.compileConsume");
        ...
        while (streaming_) {
            ...
            if (result == BytesConsumer::Result::kOk) {
                ...
                if (!buffer.empty()) {
                    ...
                    if (code_cache_state_ == CodeCacheState::kUseCodeCache) {
                        TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "v8.wasm.compileDigestForConsume");
            ...
            case BytesConsumer::Result::kDone: {
                TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "v8.wasm.compileConsumeDone");
                {
            ...
            case BytesConsumer::Result::kError:
                DCHECK_EQ(BytesConsumer::PublicState::kErrored, consumer_->GetPublicState());
    ...
    {
        return "FetchDataLoaderForWasmModule";
    }
    ...
    void AbortFromClient()
    {
        ...
        auto* exception = MakeGarbageCollected<DOMException>(DOMExceptionCode::kAbortError);
        ScriptState::Scope scope(script_state_);
        ...
        // resolveOrReject shouldn't be called inside a ScriptForbiddenScope.
        {
            ...
            v8::Local<v8::Value> v8_exception = ToV8Traits<DOMException>::ToV8(script_state_, exception);
...
private:
    ...
    void AbortCompilation(String reason)
    {
        ...
        if (script_state_->ContextIsValid()) {
            ScriptState::Scope scope(script_state_);
            streaming_->Abort(V8ThrowException::CreateTypeError(script_state_->GetIsolate(), "WebAssembly compilation aborted: " + reason));
        } else {
    ...
    CodeCacheState MaybeConsumeCodeCache()
    {
        ...
        if (!cache_handler_) {
            base::UmaHistogramEnumeration("V8.WasmCodeCaching", WasmCodeCaching::kNoCacheHandler);
        ...
        scoped_refptr<CachedMetadata> cached_module = cache_handler_->GetCachedMetadata(kWasmModuleTag);
        if (!cached_module) {
            base::UmaHistogramEnumeration("V8.WasmCodeCaching", WasmCodeCaching::kMiss);
            return CodeCacheState::kNoCodeCache;
        }
        base::span<const uint8_t> metadata_with_digest = cached_module->Data();

        TRACE_EVENT_INSTANT2(TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "v8.wasm.moduleCacheHit", TRACE_EVENT_SCOPE_THREAD, "url", url_.Utf8(),
        ...
        if (metadata_with_digest.size() >= kWireBytesDigestSize) {
            ...
            is_valid = streaming_->SetCompiledModuleBytes(metadata.data(), metadata.size());
        }

        if (!is_valid) {
            TRACE_EVENT_INSTANT0(TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "v8.wasm.moduleCacheInvalid", TRACE_EVENT_SCOPE_THREAD);
            base::UmaHistogramEnumeration("V8.WasmCodeCaching", WasmCodeCaching::kInvalidCacheEntry);
            ...
            cache_handler_->ClearCachedMetadata(
                /*code_cache_host*/ nullptr, CachedMetadataHandler::kClearPersistentStorage);
            return CodeCacheState::kNoCodeCache;
        }
    ...
    bool HasValidCodeCache()
    {
        ...
        if (digestor_.has_failed() ||
            base::span<const uint8_t>(wire_bytes_digest.data(), wire_bytes_digest.size()) != metadata_with_digest.first(kWireBytesDigestSize)) {
            TRACE_EVENT_INSTANT0(TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "v8.wasm.moduleCacheInvalidDigest", TRACE_EVENT_SCOPE_THREAD);
            cache_handler_->ClearCachedMetadata(
                /*code_cache_host*/ nullptr, CachedMetadataHandler::kClearPersistentStorage);
            return false;
        }
...
class WasmDataLoaderClient final : public GarbageCollected<WasmDataLoaderClient>, public FetchDataLoader::Client {
public:
...
void PropagateExceptionToWasmStreaming(ScriptState* script_state, v8::Local<v8::Value> exception, std::shared_ptr<v8::WasmStreaming>& streaming)
{
    ApplyContextToException(script_state, exception, ExceptionContext(v8::ExceptionContext::kOperation, "WebAssembly", "compile"));
    streaming->Abort(exception);
    streaming.reset();
}

scoped_refptr<base::SingleThreadTaskRunner> GetContextTaskRunner(ExecutionContext& execution_context)
{
    if (execution_context.IsWorkerGlobalScope()) {
        WorkerOrWorkletGlobalScope& global_scope = To<WorkerOrWorkletGlobalScope>(execution_context);
        return global_scope.GetThread()->GetWorkerBackingThread().BackingThread().GetTaskRunner();
    }

    if (execution_context.IsWindow()) {
        return DynamicTo<LocalDOMWindow>(execution_context)->GetTaskRunner(TaskType::kInternalNavigationAssociated);
    }

    DCHECK(execution_context.IsWorkletGlobalScope());
    WorkletGlobalScope& worklet_global_scope = To<WorkletGlobalScope>(execution_context);
    if (worklet_global_scope.IsMainThreadWorkletGlobalScope()) {
        return worklet_global_scope.GetFrame()->GetTaskRunner(TaskType::kInternalNavigationAssociated);
    }

    return worklet_global_scope.GetThread()->GetWorkerBackingThread().BackingThread().GetTaskRunner();
}

void StreamFromResponseCallback(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    TRACE_EVENT_INSTANT0(TRACE_DISABLED_BY_DEFAULT("devtools.timeline"), "v8.wasm.streamFromResponseCallback", TRACE_EVENT_SCOPE_THREAD);
    ...
    if (!response) {
        base::UmaHistogramEnumeration("V8.WasmStreamingInputType", WasmStreamingInputType::kNoResponse);
        auto exception = V8ThrowException::CreateTypeError(args.GetIsolate(),
    ...
    if (!response->ok()) {
        base::UmaHistogramEnumeration("V8.WasmStreamingInputType", WasmStreamingInputType::kResponseNotOK);
        auto exception = V8ThrowException::CreateTypeError(args.GetIsolate(), "HTTP status code is not ok");
        PropagateExceptionToWasmStreaming(script_state, exception, streaming);
        return;
    }
    ...
    if (!EqualIgnoringASCIICase(response->ContentType(), "application/wasm")) {
        base::UmaHistogramEnumeration("V8.WasmStreamingInputType", WasmStreamingInputType::kWrongMimeType);
        auto exception = V8ThrowException::CreateTypeError(args.GetIsolate(), "Incorrect response MIME type. Expected 'application/wasm'.");
        PropagateExceptionToWasmStreaming(script_state, exception, streaming);
        return;
    }

    if (response->IsBodyLocked() || response->IsBodyUsed()) {
        base::UmaHistogramEnumeration("V8.WasmStreamingInputType", WasmStreamingInputType::kReponseLocked);
        auto exception = V8ThrowException::CreateTypeError(args.GetIsolate(), "Cannot compile WebAssembly.Module from an already read Response");
        PropagateExceptionToWasmStreaming(script_state, exception, streaming);
        return;
    }

    if (!response->BodyBuffer()) {
        base::UmaHistogramEnumeration("V8.WasmStreamingInputType", WasmStreamingInputType::kReponseEmpty);
        // Since the status is 2xx (ok), this must be status 204 (No Content),
        // status 205 (Reset Content) or a malformed status 200 (OK).
        auto exception = V8ThrowException::CreateWasmCompileError(args.GetIsolate(), "Empty WebAssembly module");
        PropagateExceptionToWasmStreaming(script_state, exception, streaming);
        return;
    }
    ...
    if (const KURL* kurl = response->GetResponse()->Url()) {
        ...
        protocol_type = protocol == "http"   ? WasmStreamingInputType::kValidHttp
            ...
            : protocol == "chrome-extension" ? WasmStreamingInputType::kValidChromeExtension
    ...
    if (cache_handler) {
        ...
        code_caching_callback = std::make_shared<WasmCodeCachingCallback>(url, response->GetResponse()->InternalResponse()->ResponseTime(),
            response->GetResponse()->InternalResponse()->CacheStorageCacheName(), GetContextTaskRunner(*execution_context), execution_context);
        streaming->SetMoreFunctionsCanBeSerializedCallback([code_caching_callback](v8::CompiledWasmModule compiled_module) {
            code_caching_callback->OnMoreFunctionsCanBeSerialized(std::move(compiled_module));
        });
    }

    FetchDataLoaderForWasmStreaming* loader
        = MakeGarbageCollected<FetchDataLoaderForWasmStreaming>(url, std::move(streaming), script_state, cache_handler, code_caching_callback);
    response->BodyBuffer()->StartLoading(loader, MakeGarbageCollected<WasmDataLoaderClient>(loader), PassThroughException(args.GetIsolate()));
}
