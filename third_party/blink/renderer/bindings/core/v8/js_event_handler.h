...
public:
    ...
    static v8::Local<v8::Value> AsV8Value(v8::Isolate* isolate, EventTarget* event_target, EventListener* listener)
    {
    ...
    explicit JSEventHandler(V8EventHandlerNonNull* event_handler, HandlerType type)
    ...
    bool IsEventHandler() const final
    {
        return true;
    }
    ...
    bool IsJSEventHandler() const override
    {
        return true;
    }
...
protected:
    ...
    ScriptState* GetScriptStateOrReportError(const char* operation) const override
    {
        return event_handler_->CallbackRelevantScriptStateOrReportError("EventHandler", operation);
    }
    ...
    void SetCompiledHandler(ScriptState* incumbent_script_state, v8::Local<v8::Function> listener);

    bool HasCompiledHandler() const
    {
...
private:
    ...
    void InvokeInternal(EventTarget&, Event&, v8::Local<v8::Value> js_event) override;

    Member<V8EventHandlerNonNull> event_handler_;
    const HandlerType type_;
};

template <> struct DowncastTraits<JSEventHandler> {
    static bool AllowFrom(const EventListener& event_listener)
    {
        if (const JSBasedEventListener* js_based_event_listener = DynamicTo<JSBasedEventListener>(event_listener)) {
