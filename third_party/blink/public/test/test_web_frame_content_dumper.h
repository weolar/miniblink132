...
public:
    ...
    enum LayoutAsTextControl { kLayoutAsTextNormal = 0, kLayoutAsTextDebug = 1 << 0, kLayoutAsTextPrinting = 1 << 1, kLayoutAsTextWithLineTrees = 1 << 2 };
    using LayoutAsTextControls = uint8_t;

    // Returns a text representation of the render tree. This method is used to
    // support web tests.
    static WebString DumpLayoutTreeAsText(WebLocalFrame*, LayoutAsTextControls to_show = kLayoutAsTextNormal);
};
