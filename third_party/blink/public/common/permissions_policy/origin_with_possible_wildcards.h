...
public:
    ...
    OriginWithPossibleWildcards& operator=(const OriginWithPossibleWildcards& rhs);
    ~OriginWithPossibleWildcards();
    ...
    static std::optional<OriginWithPossibleWildcards> FromOrigin(const url::Origin& origin);
    ...
    // The returned value will be empty if the origin is opaque or Parse fails.
    static std::optional<OriginWithPossibleWildcards> FromOriginAndWildcardsForTest(const url::Origin& origin, bool has_subdomain_wildcard);
    ...
    static std::optional<OriginWithPossibleWildcards> Parse(const std::string& allowlist_entry, const NodeType type);
...
private:
    friend struct mojo::StructTraits<blink::mojom::OriginWithPossibleWildcardsDataView, blink::OriginWithPossibleWildcards>;
    BLINK_COMMON_EXPORT
    friend bool operator==(const OriginWithPossibleWildcards& lhs, const OriginWithPossibleWildcards& rhs);
    BLINK_COMMON_EXPORT
    friend bool operator!=(const OriginWithPossibleWildcards& lhs, const OriginWithPossibleWildcards& rhs);
    BLINK_COMMON_EXPORT
    friend bool operator<(const OriginWithPossibleWildcards& lhs, const OriginWithPossibleWildcards& rhs);
