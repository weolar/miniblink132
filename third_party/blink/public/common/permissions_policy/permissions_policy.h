...
public:
    ...
    public:
        ...
        static Allowlist FromDeclaration(const ParsedPermissionsPolicyDeclaration& parsed_declaration);
        ...
        void SetAllowedOrigins(const std::vector<OriginWithPossibleWildcards>& allowed_origins)
        {
    ...
    static std::unique_ptr<PermissionsPolicy> CreateFromParentPolicy(const PermissionsPolicy* parent_policy, const ParsedPermissionsPolicy& header_policy,
        const ParsedPermissionsPolicy& container_policy, const url::Origin& origin);

    static std::unique_ptr<PermissionsPolicy> CopyStateFrom(const PermissionsPolicy*);
    ...
    static std::unique_ptr<PermissionsPolicy> CreateFlexibleForFencedFrame(const PermissionsPolicy* parent_policy, const ParsedPermissionsPolicy& header_policy,
        const ParsedPermissionsPolicy& container_policy, const url::Origin& subframe_origin);
    ...
    static std::unique_ptr<PermissionsPolicy> CreateFixedForFencedFrame(const url::Origin& origin, const ParsedPermissionsPolicy& header_policy,
    ...
    // specific origin.
    bool IsFeatureEnabledForOrigin(mojom::PermissionsPolicyFeature feature, const url::Origin& origin) const;
    ...
    const Allowlist GetAllowlistForDevTools(mojom::PermissionsPolicyFeature feature) const;
    ...
    // calculation method.
    const Allowlist GetAllowlistForFeature(mojom::PermissionsPolicyFeature feature) const;
    ...
    std::optional<const Allowlist> GetAllowlistForFeatureIfExists(mojom::PermissionsPolicyFeature feature) const;

    std::optional<std::string> GetEndpointForFeature(mojom::PermissionsPolicyFeature feature) const;
    ...
    // modification.
    std::unique_ptr<PermissionsPolicy> WithClientHints(const ParsedPermissionsPolicy& parsed_header) const;

    const url::Origin& GetOriginForTest() const
    {
        return origin_;
    }
    const std::map<mojom::PermissionsPolicyFeature, Allowlist>& allowlists() const
    {
        return allowlists_;
    }
    ...
    bool IsFeatureEnabledByInheritedPolicy(mojom::PermissionsPolicyFeature feature) const;
...
private:
    ...
    static AllowlistsAndReportingEndpoints CreateAllowlistsAndReportingEndpoints(const ParsedPermissionsPolicy& parsed_header);
    ...
    static AllowlistsAndReportingEndpoints CombinePolicies(const ParsedPermissionsPolicy& base_policy, const ParsedPermissionsPolicy& second_policy);

    PermissionsPolicy(url::Origin origin, AllowlistsAndReportingEndpoints allow_lists_and_reporting_endpoints, PermissionsPolicyFeatureState inherited_policies,
        const PermissionsPolicyFeatureList& feature_list);
    static std::unique_ptr<PermissionsPolicy> CreateFromParentPolicy(const PermissionsPolicy* parent_policy, const ParsedPermissionsPolicy& header_policy,
        const ParsedPermissionsPolicy& container_policy, const url::Origin& origin, const PermissionsPolicyFeatureList& features);

    static std::unique_ptr<PermissionsPolicy> CreateFromParsedPolicy(const ParsedPermissionsPolicy& parsed_policy,
    ...
    static std::unique_ptr<PermissionsPolicy> CreateFlexibleForFencedFrame(const PermissionsPolicy* parent_policy, const ParsedPermissionsPolicy& header_policy,
        const ParsedPermissionsPolicy& container_policy, const url::Origin& subframe_origin, const PermissionsPolicyFeatureList& features);
    ...
    bool IsFeatureEnabledForSubresourceRequestAssumingOptIn(mojom::PermissionsPolicyFeature feature, const url::Origin& origin) const;

    // Returns the inherited policy of the given feature.
    static bool InheritedValueForFeature(const url::Origin& origin, const PermissionsPolicy* parent_policy,
    ...
    // inherited policy.
    bool GetFeatureValueForOrigin(mojom::PermissionsPolicyFeature feature, const url::Origin& origin) const;
    ...
    const std::map<mojom::PermissionsPolicyFeature, std::string> reporting_endpoints_;
