...
public:
    ...
    using FeatureEndpointMap = base::flat_map<mojom::DocumentPolicyFeature, std::string>;
    ...
    static std::unique_ptr<DocumentPolicy> CreateWithHeaderPolicy(const ParsedDocumentPolicy& header_policy);
    ...
    bool IsFeatureEnabled(mojom::DocumentPolicyFeature feature, const PolicyValue& threshold_value) const;
    ...
    const std::optional<std::string> GetFeatureEndpoint(mojom::DocumentPolicyFeature feature) const;
    ...
    static bool IsPolicyCompatible(const DocumentPolicyFeatureState& required_policy, const DocumentPolicyFeatureState& incoming_policy);
    ...
    static std::optional<std::string> Serialize(const DocumentPolicyFeatureState& policy);

    static std::optional<std::string> SerializeInternal(const DocumentPolicyFeatureState& policy, const DocumentPolicyFeatureInfoMap&);
    ...
    static DocumentPolicyFeatureState MergeFeatureState(const DocumentPolicyFeatureState& base_policy, const DocumentPolicyFeatureState& override_policy);

private:
    friend class DocumentPolicyTest;

    DocumentPolicy(const DocumentPolicyFeatureState& header_policy, const FeatureEndpointMap& endpoint_map, const DocumentPolicyFeatureState& defaults);
    ...
    std::array<PolicyValue, static_cast<size_t>(mojom::DocumentPolicyFeature::kMaxValue) + 1> internal_feature_state_;

    FeatureEndpointMap endpoint_map_;
};

bool inline operator==(const DocumentPolicy::ParsedDocumentPolicy& lhs, const DocumentPolicy::ParsedDocumentPolicy& rhs)
{
    return std::tie(lhs.feature_state, lhs.endpoint_map) == std::tie(rhs.feature_state, rhs.endpoint_map);
}
