...
public:
    ...
    enum class Buttons : unsigned { kNoButton = 0, kLeft = 1 << 0, kRight = 1 << 1, kMiddle = 1 << 2, kBack = 1 << 3, kForward = 1 << 4, kEraser = 1 << 5 };

    explicit WebPointerProperties(PointerId id_param, PointerType pointer_type_param = PointerType::kUnknown, Button button_param = Button::kNoButton,
        gfx::PointF position_in_widget = gfx::PointF(), gfx::PointF position_in_screen = gfx::PointF(), int movement_x = 0, int movement_y = 0)
        : id(id_param)
        , button(button_param)
