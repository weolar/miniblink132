...
public:
    ...
    AssociatedInterfaceRegistry& operator=(const AssociatedInterfaceRegistry&) = delete;
    ~AssociatedInterfaceRegistry();
    ...
    // taken.
    bool TryBindInterface(const std::string& name, mojo::ScopedInterfaceEndpointHandle* handle);

    template <typename Interface> using ReceiverBinder = base::RepeatingCallback<void(mojo::PendingAssociatedReceiver<Interface>)>;

    template <typename Interface> void AddInterface(const std::common_type_t<ReceiverBinder<Interface>&> binder)
    {
        AddInterface(Interface::Name_, base::BindRepeating(&BindInterfaceReceiver<Interface>, binder));
    }
...
private:
    template <typename Interface> static void BindInterfaceReceiver(const ReceiverBinder<Interface>& binder, mojo::ScopedInterfaceEndpointHandle handle)
    {
