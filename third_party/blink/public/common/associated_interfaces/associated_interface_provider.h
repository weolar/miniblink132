...
public:
    ...
    explicit AssociatedInterfaceProvider(
        mojo::PendingAssociatedRemote<mojom::AssociatedInterfaceProvider> proxy, scoped_refptr<base::SingleThreadTaskRunner> task_runner = nullptr);
    ...
    // |task_runner| must belong to the same thread. It will be used to dispatch
    // all callbacks and connection error notification.
    explicit AssociatedInterfaceProvider(scoped_refptr<base::SingleThreadTaskRunner> task_runner);

    AssociatedInterfaceProvider(const AssociatedInterfaceProvider&) = delete;
    AssociatedInterfaceProvider& operator=(const AssociatedInterfaceProvider&) = delete;
    ...
    template <typename Interface> void GetInterface(mojo::PendingAssociatedReceiver<Interface> receiver)
    {
        GetInterface(Interface::Name_, receiver.PassHandle());
    }

    template <typename Interface> void GetInterface(mojo::AssociatedRemote<Interface>* remote)
    {
    ...
    // override.
    void OverrideBinderForTesting(const std::string& name, const base::RepeatingCallback<void(mojo::ScopedInterfaceEndpointHandle)>& binder);
