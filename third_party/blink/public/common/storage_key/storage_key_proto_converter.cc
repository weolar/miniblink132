...
url::Origin MakeOrigin(const storage_key_proto::StorageKey::Origin& origin_proto)
{
    std::string scheme;
    ...
    return url::Origin::CreateFromNormalizedTuple(scheme, host, origin_proto.port());
}

blink::mojom::AncestorChainBit MakeAncestorChainBit(
    const storage_key_proto::StorageKey::AncestorChainBit& bit_proto, const url::Origin& origin, const net::SchemefulSite& top_level_site)
{
...
blink::StorageKey Convert(const storage_key_proto::StorageKey& storage_key)
{
    ...
    case StorageKeyType::kUnguessableToken:
        return blink::StorageKey::CreateWithNonce(origin, base::UnguessableToken::Create());
    case StorageKey::OneOfCase::kTopLevelSite:
        net::SchemefulSite top_level_site = net::SchemefulSite(MakeOrigin(storage_key.top_level_site().origin()));
        blink::mojom::AncestorChainBit ancestor_chain_bit = MakeAncestorChainBit(storage_key.ancestor_chain_bit(), origin, top_level_site);
        return blink::StorageKey::Create(origin, top_level_site, ancestor_chain_bit);
    }
}
