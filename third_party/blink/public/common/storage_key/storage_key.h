...
public:
    ...
    static StorageKey CreateWithNonce(const url::Origin& origin, const base::UnguessableToken& nonce);
    ...
    static StorageKey Create(const url::Origin& origin, const net::SchemefulSite& top_level_site, blink::mojom::AncestorChainBit ancestor_chain_bit,
        bool third_party_partitioning_allowed = IsThirdPartyStoragePartitioningEnabled());

    // (1E) Construct for the provided isolation_info.
    // TODO(crbug.com/1346450): This does not account for extension URLs.
    static StorageKey CreateFromOriginAndIsolationInfo(const url::Origin& origin, const net::IsolationInfo& isolation_info);
    ...
    StorageKey CopyWithForceEnabledThirdPartyStoragePartitioning() const
    {
        ...
        storage_key.top_level_site_ = storage_key.top_level_site_if_third_party_enabled_;
        storage_key.ancestor_chain_bit_ = storage_key.ancestor_chain_bit_if_third_party_enabled_;
    ...
    // TODO(crbug.com/1159586): Remove when no longer needed.
    static bool FromWire(const url::Origin& origin, const net::SchemefulSite& top_level_site, const net::SchemefulSite& top_level_site_if_third_party_enabled,
    ...
    static std::optional<StorageKey> DeserializeForLocalStorage(std::string_view in);
    ...
    const net::SchemefulSite& top_level_site() const
    {
    ...
    bool MatchesRegistrableDomainForTrustedStorageDeletion(std::string_view domain) const;
...
private:
    ...
    StorageKey(const url::Origin& origin, const net::SchemefulSite& top_level_site, const base::UnguessableToken* nonce,
    ...
    blink::mojom::AncestorChainBit ancestor_chain_bit_ { blink::mojom::AncestorChainBit::kCrossSite };

    // Stores the value `ancestor_chain_bit_` would have had if
    // `kThirdPartyStoragePartitioning` were enabled. This isn't used in
    // serialization or comparison.
    // TODO(crbug.com/1159586): Remove when no longer needed.
    blink::mojom::AncestorChainBit ancestor_chain_bit_if_third_party_enabled_ = ancestor_chain_bit_;
};
