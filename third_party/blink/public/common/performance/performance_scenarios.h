...
using RefCountedScenarioMapping = base::RefCountedData<base::StructuredSharedMemory<ScenarioState>::ReadOnlyMapping>;
...
template <typename T> class SharedAtomicRef {
public:
    SharedAtomicRef(scoped_refptr<RefCountedScenarioMapping> mapping, const std::atomic<T>& wrapped_atomic)
        : mapping_(std::move(mapping))
        , wrapped_atomic_(wrapped_atomic)
    {
    }
...
public:
    ...
    ScopedReadOnlyScenarioMemory(Scope scope, base::ReadOnlySharedMemoryRegion region);
    ~ScopedReadOnlyScenarioMemory();

    ScopedReadOnlyScenarioMemory(const ScopedReadOnlyScenarioMemory&) = delete;
    ScopedReadOnlyScenarioMemory& operator=(const ScopedReadOnlyScenarioMemory&) = delete;

    // Returns a pointer to the mapping registered for `scope`, if any.
    static scoped_refptr<RefCountedScenarioMapping> GetMappingForTesting(Scope scope);
...
BLINK_COMMON_EXPORT SharedAtomicRef<LoadingScenario> GetLoadingScenario(Scope scope);

// Returns a reference to the input scenario for `scope`.
BLINK_COMMON_EXPORT SharedAtomicRef<InputScenario> GetInputScenario(Scope scope);
