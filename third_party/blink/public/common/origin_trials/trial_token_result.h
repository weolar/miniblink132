...
public:
    ...
    // cannot be nullptr.
    TrialTokenResult(OriginTrialTokenStatus status, std::unique_ptr<TrialToken> parsed_token);
    ~TrialTokenResult() = default;

    OriginTrialTokenStatus Status() const
    {
        return status_;
    }

    const TrialToken* ParsedToken() const
    {
        return parsed_token_.get();
    }

    void SetStatus(OriginTrialTokenStatus status)
    {
        status_ = status;
    }
