...
public:
    ...
    // parsed.
    static std::unique_ptr<TrialToken> From(std::string_view token_text, const OriginTrialPublicKey& public_key, OriginTrialTokenStatus* out_status);
    ...
    // token is not valid.
    OriginTrialTokenStatus IsValid(const url::Origin& origin, const base::Time& now) const;
    ...
    UsageRestriction usage_restriction() const
    {
        return usage_restriction_;
    }

    static std::unique_ptr<TrialToken> CreateTrialTokenForTesting(const url::Origin& origin, bool match_subdomains, const std::string& feature_name,
...
protected:
    ...
    static OriginTrialTokenStatus Extract(std::string_view token_text, const OriginTrialPublicKey& public_key, std::string* out_token_payload,
    ...
    static std::unique_ptr<TrialToken> Parse(const std::string& token_payload, const uint8_t version);
    ...
    static bool ValidateSignature(std::string_view signature_text, const std::string& data, const OriginTrialPublicKey& public_key);

private:
    TrialToken(const url::Origin& origin, bool match_subdomains, const std::string& feature_name, base::Time expiry_time, bool is_third_party,
