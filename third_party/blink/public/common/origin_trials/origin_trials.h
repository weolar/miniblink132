...
BLINK_COMMON_EXPORT bool IsTrialEnabledForInsecureContext(std::string_view trial_name);

// Return true if |trial_name| can be enabled from third party origins.
BLINK_COMMON_EXPORT bool IsTrialEnabledForThirdPartyOrigins(std::string_view trial_name);

// Return true if |trial_name| can be enabled for read access by the browser
// process.
BLINK_COMMON_EXPORT bool IsTrialEnabledForBrowserProcessReadAccess(std::string_view trial_name);

// Returns true if |trial_name| should be enabled until the next response
// from the same origin is received.
BLINK_COMMON_EXPORT bool IsTrialPersistentToNextResponse(std::string_view trial_name);
...
BLINK_COMMON_EXPORT OriginTrialType GetTrialType(blink::mojom::OriginTrialFeature feature);
...
// function).
BLINK_COMMON_EXPORT base::span<const blink::mojom::OriginTrialFeature> FeaturesForTrial(std::string_view trial_name);

// Return the list of features which will also be enabled if the given
// |feature| is enabled.
BLINK_COMMON_EXPORT base::span<const blink::mojom::OriginTrialFeature> GetImpliedFeatures(blink::mojom::OriginTrialFeature feature);

// Returns true if |feature| is enabled on the current platform.
BLINK_COMMON_EXPORT bool FeatureEnabledForOS(blink::mojom::OriginTrialFeature feature);

// Returns true if |feature| can be enabled across navigations.
BLINK_COMMON_EXPORT bool FeatureEnabledForNavigation(blink::mojom::OriginTrialFeature feature);

// Returns true if |feature| has an expiry grace period.
BLINK_COMMON_EXPORT bool FeatureHasExpiryGracePeriod(blink::mojom::OriginTrialFeature feature);
