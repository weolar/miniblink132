...
using ClientHintToPolicyFeatureMap = base::flat_map<network::mojom::WebClientHintsType, mojom::PermissionsPolicyFeature>;

using PolicyFeatureToClientHintMap = base::flat_map<mojom::PermissionsPolicyFeature, std::set<network::mojom::WebClientHintsType>>;
...
BLINK_COMMON_EXPORT const ClientHintToPolicyFeatureMap& GetClientHintToPolicyFeatureMap();

// Mapping from Permissions-Policy to the corresponding WebClientHintsType(s)
// (e.g. kClientHintsDPR => {kDpr, kDpr_DEPRECATED}).
BLINK_COMMON_EXPORT const PolicyFeatureToClientHintMap& GetPolicyFeatureToClientHintMap();
...
BLINK_COMMON_EXPORT void FindClientHintsToRemove(const PermissionsPolicy* permissions_policy, const GURL& url, std::vector<std::string>* removed_headers);
