...
template <typename T> struct IsBaseTokenType : std::false_type { };

template <typename T> struct IsBaseTokenType<base::TokenType<T>> : std::true_type { };

template <typename T>
concept IsBaseToken = IsBaseTokenType<T>::value;

template <typename... Types> bool AreAllUnique;
template <> inline constexpr bool AreAllUnique<> = true;
template <typename T, typename... Ts> inline constexpr bool AreAllUnique<T, Ts...> = (!std::is_same_v<T, Ts> && ...) && AreAllUnique<Ts...>;
