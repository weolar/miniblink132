...
template <typename... Tokens>
    requires(sizeof...(Tokens) > 1 && sizeof...(Tokens) <= std::numeric_limits<uint32_t>::max() && (internal::IsBaseToken<Tokens> && ...)
...
public:
    ...
    explicit MultiToken(const MultiToken<Ts...>& multi_token)
        : MultiToken(multi_token.Visit([](const auto& token) { return MultiToken(token); }))
    {
    }
    ...
    {
        return *this = multi_token.Visit([](const auto& token) { return MultiToken(token); });
    }
    ...
    constexpr friend std::weak_ordering operator<=>(const MultiToken& lhs, const MultiToken& rhs)
    {
    ...
    constexpr friend bool operator==(const MultiToken& lhs, const MultiToken& rhs)
    {
    ...
    {
        return Visit([](const auto& token) -> const base::UnguessableToken& { return token.value(); });
    }

    // 0-based index of the currently held token's type, based on its position in
    // `Tokens...`.
    Tag variant_index() const
    {
