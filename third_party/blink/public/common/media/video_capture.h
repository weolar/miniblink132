...
using VideoCaptureDeliverFrameCB = base::RepeatingCallback<void(scoped_refptr<media::VideoFrame> video_frame, base::TimeTicks estimated_capture_time)>;

// Callback for delivering dropped frame notifications.
// Used to implement the MediaStreamTrack Statistics API.
using VideoCaptureNotifyFrameDroppedCB = base::RepeatingCallback<void(media::VideoCaptureFrameDropReason)>;

// Callback for informing when new sub-capture-target-versions are applied.
using VideoCaptureSubCaptureTargetVersionCB = base::RepeatingCallback<void(uint32_t sub_capture_target_version)>;

using VideoCaptureDeviceFormatsCB = base::OnceCallback<void(const media::VideoCaptureFormats&)>;
...
using VideoCaptureStateUpdateCB = base::RepeatingCallback<void(VideoCaptureState)>;
