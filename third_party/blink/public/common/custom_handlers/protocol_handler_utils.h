...
bool BLINK_COMMON_EXPORT IsValidCustomHandlerScheme(
    std::string_view scheme, ProtocolHandlerSecurityLevel security_level, bool* has_custom_scheme_prefix = nullptr);
...
URLSyntaxErrorCode BLINK_COMMON_EXPORT IsValidCustomHandlerURLSyntax(const GURL& full_url, const std::string_view& user_url);
...
// HTML specification:
// https://html.spec.whatwg.org/multipage/system-state.html#normalize-protocol-handler-parameters
bool BLINK_COMMON_EXPORT IsAllowedCustomHandlerURL(const GURL& url, ProtocolHandlerSecurityLevel security_level);
