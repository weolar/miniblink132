...
class BLINK_COMMON_EXPORT PendingURLLoaderFactoryBundle : public network::PendingSharedURLLoaderFactory {
public:
    ...
    using SchemeMap = std::map<std::string, mojo::PendingRemote<network::mojom::URLLoaderFactory>>;
    ...
    using OriginMap = std::map<url::Origin, mojo::PendingRemote<network::mojom::URLLoaderFactory>>;

    PendingURLLoaderFactoryBundle();
    PendingURLLoaderFactoryBundle(mojo::PendingRemote<network::mojom::URLLoaderFactory> pending_default_factory, SchemeMap scheme_specific_pending_factories,
        OriginMap isolated_world_pending_factories, bool bypass_redirect_checks);
    PendingURLLoaderFactoryBundle(const PendingURLLoaderFactoryBundle&) = delete;
    PendingURLLoaderFactoryBundle& operator=(const PendingURLLoaderFactoryBundle&) = delete;
...
protected:
    ...
    mojo::PendingRemote<network::mojom::URLLoaderFactory> pending_default_factory_;
...
class BLINK_COMMON_EXPORT URLLoaderFactoryBundle : public network::SharedURLLoaderFactory {
public:
    URLLoaderFactoryBundle();

    explicit URLLoaderFactoryBundle(std::unique_ptr<PendingURLLoaderFactoryBundle> pending_factories);
    ...
    bool HasBoundDefaultFactory() const
    {
        return default_factory_.is_bound();
    }
...
protected:
    ...
    network::mojom::URLLoaderFactory* GetFactory(const network::ResourceRequest& request);

    template <typename TKey>
    static std::map<TKey, mojo::PendingRemote<network::mojom::URLLoaderFactory>> CloneRemoteMapToPendingRemoteMap(
        const std::map<TKey, mojo::Remote<network::mojom::URLLoaderFactory>>& input)
    {
    ...
    using SchemeMap = std::map<std::string, mojo::Remote<network::mojom::URLLoaderFactory>>;
    ...
    using OriginMap = std::map<url::Origin, mojo::Remote<network::mojom::URLLoaderFactory>>;
