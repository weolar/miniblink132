...
template <typename T> class SiteSpecificTimeDelta final {
public:
...
private:
    explicit SiteSpecificTimeDelta(base::TimeDelta delta)
        : delta_(delta)
    {
    }
...
template <typename T> std::ostream& operator<<(std::ostream& os, SiteSpecificTimeDelta<T> delta)
{
...
template <typename T> class SiteSpecificTimeTicks final {
public:
    ...
    base::TimeTicks ToTimeTicks() const
    {
        return time_ticks_;
    }
    bool is_null() const
    {
        return time_ticks_.is_null();
    }
    ...
    {
        return SiteSpecificTimeDelta<T>::FromTimeDelta(time_ticks_ - rhs.time_ticks_);
    }
...
// For logging use only.
template <typename T> std::ostream& operator<<(std::ostream& os, SiteSpecificTimeTicks<T> time_ticks)
{
