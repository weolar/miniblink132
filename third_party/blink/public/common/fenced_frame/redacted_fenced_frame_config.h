...
// `std::nullopt`.)
template <class T> struct BLINK_COMMON_EXPORT RedactedFencedFrameProperty {
public:
    RedactedFencedFrameProperty()
        : potentially_opaque_value(std::nullopt)
    {
    }
    explicit RedactedFencedFrameProperty(const std::optional<T>& potentially_opaque_value)
...
struct BLINK_COMMON_EXPORT RedactedFencedFrameConfig {
    ...
    const std::optional<GURL>& urn_uuid() const
    {
        return urn_uuid_;
    }
    ...
    const std::optional<RedactedFencedFrameProperty<gfx::Size>>& container_size() const
    {
        return container_size_;
    }
    const std::optional<RedactedFencedFrameProperty<gfx::Size>>& content_size() const
    {
        return content_size_;
    }
    const std::optional<RedactedFencedFrameProperty<bool>>& deprecated_should_freeze_initial_size() const
    {
        return deprecated_should_freeze_initial_size_;
    }
    const std::optional<RedactedFencedFrameProperty<AdAuctionData>>& ad_auction_data() const
    {
        return ad_auction_data_;
    }
    const std::optional<RedactedFencedFrameProperty<std::vector<RedactedFencedFrameConfig>>>& nested_configs() const
    {
        return nested_configs_;
    }
    const std::optional<RedactedFencedFrameProperty<SharedStorageBudgetMetadata>>& shared_storage_budget_metadata() const
    {
        return shared_storage_budget_metadata_;
    }
    const DeprecatedFencedFrameMode& mode() const
    {
        return mode_;
    }
    const std::vector<blink::mojom::PermissionsPolicyFeature>& effective_enabled_permissions() const
    {
...
private:
    ...
    friend struct mojo::StructTraits<blink::mojom::FencedFrameConfigDataView, blink::FencedFrame::RedactedFencedFrameConfig>;
    ...
    std::optional<RedactedFencedFrameProperty<bool>> deprecated_should_freeze_initial_size_;
    std::optional<RedactedFencedFrameProperty<AdAuctionData>> ad_auction_data_;
    std::optional<RedactedFencedFrameProperty<std::vector<RedactedFencedFrameConfig>>> nested_configs_;
    std::optional<RedactedFencedFrameProperty<SharedStorageBudgetMetadata>> shared_storage_budget_metadata_;

    // TODO(crbug.com/1347953): Not yet used.
    DeprecatedFencedFrameMode mode_ = DeprecatedFencedFrameMode::kDefault;

    std::vector<blink::mojom::PermissionsPolicyFeature> effective_enabled_permissions_;
...
struct BLINK_COMMON_EXPORT RedactedFencedFrameProperties {
    ...
    const std::optional<RedactedFencedFrameProperty<std::vector<std::pair<GURL, RedactedFencedFrameConfig>>>>& nested_urn_config_pairs() const
    {
    ...
    {
        return is_cross_origin_content_;
    }
...
private:
    ...
    friend struct mojo::StructTraits<blink::mojom::FencedFramePropertiesDataView, blink::FencedFrame::RedactedFencedFrameProperties>;
    ...
    std::optional<RedactedFencedFrameProperty<bool>> deprecated_should_freeze_initial_size_;
    std::optional<RedactedFencedFrameProperty<AdAuctionData>> ad_auction_data_;
    std::optional<RedactedFencedFrameProperty<std::vector<std::pair<GURL, RedactedFencedFrameConfig>>>> nested_urn_config_pairs_;
    std::optional<RedactedFencedFrameProperty<SharedStorageBudgetMetadata>> shared_storage_budget_metadata_;
