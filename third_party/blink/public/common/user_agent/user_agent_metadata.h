...
struct BLINK_COMMON_EXPORT UserAgentBrandVersion {
    ...
    UserAgentBrandVersion(const std::string& ua_brand, const std::string& ua_version);
...
private:
    ...
    // in one place.
    const std::string SerializeBrandVersionList(const blink::UserAgentBrandList& ua_brand_version_list);
...
public:
    ...
    static std::optional<UserAgentMetadata> Demarshal(const std::optional<std::string>& encoded);
    static std::optional<std::string> Marshal(const std::optional<UserAgentMetadata>& ua_metadata);
...
bool BLINK_COMMON_EXPORT operator==(const UserAgentMetadata& a, const UserAgentMetadata& b);

bool BLINK_COMMON_EXPORT operator==(const UserAgentOverride& a, const UserAgentOverride& b);
