...
struct BLINK_COMMON_EXPORT ExplodedFrameState {
    ...
    blink::mojom::ScrollRestorationType scroll_restoration_type = blink::mojom::ScrollRestorationType::kAuto;
...
BLINK_COMMON_EXPORT bool DecodePageState(const std::string& encoded, ExplodedPageState* exploded);
...
BLINK_COMMON_EXPORT int DecodePageStateForTesting(const std::string& encoded, ExplodedPageState* exploded);
BLINK_COMMON_EXPORT void EncodePageState(const ExplodedPageState& exploded, std::string* encoded);
BLINK_COMMON_EXPORT void LegacyEncodePageStateForTesting(const ExplodedPageState& exploded, int version, std::string* encoded);

#if BUILDFLAG(IS_ANDROID)
BLINK_COMMON_EXPORT bool DecodePageStateWithDeviceScaleFactorForTesting(const std::string& encoded, float device_scale_factor, ExplodedPageState* exploded);
...
BLINK_COMMON_EXPORT scoped_refptr<network::ResourceRequestBody> DecodeResourceRequestBody(base::span<const uint8_t> data);

// Encodes |resource_request_body| into |encoded|.
BLINK_COMMON_EXPORT std::string EncodeResourceRequestBody(const network::ResourceRequestBody& resource_request_body);
#endif
