...
struct BLINK_COMMON_EXPORT WebMediaDeviceInfo {
    ...
    WebMediaDeviceInfo(const std::string& device_id, const std::string& label, const std::string& group_id,
        const media::VideoCaptureControlSupport& video_control_support = media::VideoCaptureControlSupport(),
        blink::mojom::FacingMode video_facing = blink::mojom::FacingMode::kNone, std::optional<media::CameraAvailability> availability = std::nullopt);
    explicit WebMediaDeviceInfo(const media::VideoCaptureDeviceDescriptor& descriptor);
    ~WebMediaDeviceInfo();
    ...
    {
        return !availability || *availability == media::CameraAvailability::kAvailable;
    }
...
BLINK_COMMON_EXPORT bool operator==(const WebMediaDeviceInfo& first, const WebMediaDeviceInfo& second);

inline bool IsValidMediaDeviceType(mojom::MediaDeviceType type)
{
    return static_cast<size_t>(type) >= 0 && static_cast<size_t>(type) < static_cast<size_t>(mojom::MediaDeviceType::kNumMediaDeviceTypes);
}
