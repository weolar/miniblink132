...
BLINK_COMMON_EXPORT bool IsVideoScreenCaptureMediaType(mojom::MediaStreamType type);
BLINK_COMMON_EXPORT bool IsDesktopCaptureMediaType(mojom::MediaStreamType type);
BLINK_COMMON_EXPORT bool IsVideoDesktopCaptureMediaType(mojom::MediaStreamType type);
...
struct BLINK_COMMON_EXPORT MediaStreamDevice {
    MediaStreamDevice();
    MediaStreamDevice(mojom::MediaStreamType type, const std::string& id, const std::string& name);
    MediaStreamDevice(mojom::MediaStreamType type, const std::string& id, const std::string& name, int64_t display_id);
    MediaStreamDevice(mojom::MediaStreamType type, const std::string& id, const std::string& name, const media::VideoCaptureControlSupport& control_support,
        media::VideoFacingMode facing, const std::optional<std::string>& group_id = std::nullopt);
    MediaStreamDevice(mojom::MediaStreamType type, const std::string& id, const std::string& name, int sample_rate,
    ...
    {
        session_id_ = session_id.is_empty() ? std::optional<base::UnguessableToken>() : session_id;
    }
    ...
    media::AudioParameters input = media::AudioParameters::UnavailableDeviceParams();
...
BLINK_COMMON_EXPORT MediaStreamDevices ToMediaStreamDevicesList(const mojom::StreamDevicesSet& stream_devices_set);

BLINK_COMMON_EXPORT size_t CountDevices(const mojom::StreamDevices& devices);
BLINK_COMMON_EXPORT bool IsMediaStreamDeviceTransferrable(const MediaStreamDevice& device);
