...
template <> struct BLINK_COMMON_EXPORT StructTraits<blink::mojom::TaskAttributionId::DataView, blink::scheduler::TaskAttributionId> {
    static uint32_t value(const blink::scheduler::TaskAttributionId& input)
    {
        return input.value();
    }

    static bool Read(blink::mojom::TaskAttributionId::DataView data, blink::scheduler::TaskAttributionId* out);
};
