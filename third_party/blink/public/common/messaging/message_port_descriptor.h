...
public:
    ...
    MessagePortDescriptor& operator=(const MessagePortDescriptor& message_port) = delete;
    MessagePortDescriptor& operator=(MessagePortDescriptor&& message_port);

    ~MessagePortDescriptor();

    // Simple accessors.
    const mojo::ScopedMessagePipeHandle& handle() const
    {
        return handle_;
    }
    ...
    // need to!
    void InitializeFromSerializedValues(mojo::ScopedMessagePipeHandle handle, const base::UnguessableToken& id, uint64_t sequence_number);
    ...
    mojo::ScopedMessagePipeHandle TakeHandleToEntangle(ExecutionContext* execution_context);
...
public:
    ...
    const MessagePortDescriptor& port0() const
    {
        return port0_;
    }
    const MessagePortDescriptor& port1() const
    {
        return port1_;
    }

    MessagePortDescriptor TakePort0()
    {
        return std::move(port0_);
    }
    MessagePortDescriptor TakePort1()
    {
        return std::move(port1_);
    }
...
public:
    ...
    virtual void NotifyMessagePortPairCreated(const base::UnguessableToken& port0_id, const base::UnguessableToken& port1_id) = 0;
    ...
    virtual void NotifyMessagePortAttached(const base::UnguessableToken& port_id, uint64_t sequence_number, ExecutionContext* execution_context) = 0;

    // Notifies the instrumentation that a handle has been attached to an
    // embedder.
    virtual void NotifyMessagePortAttachedToEmbedder(const base::UnguessableToken& port_id, uint64_t sequence_number) = 0;

    // Notifies the instrumentation that a handle has been detached from an
    // execution context.
    virtual void NotifyMessagePortDetached(const base::UnguessableToken& port_id, uint64_t sequence_number) = 0;

    // Notifies the instrumentation that a handle has been destroyed.
    virtual void NotifyMessagePortDestroyed(const base::UnguessableToken& port_id, uint64_t sequence_number) = 0;
};
