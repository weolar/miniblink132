...
public:
    ...
    virtual std::optional<base::span<const uint8_t>> GetAsSpanIfPossible() const = 0;
    ...
    static std::unique_ptr<WebMessageArrayBufferPayload> CreateFromBigBuffer(mojo_base::BigBuffer buffer, std::optional<size_t> max_byte_length);

    // Create a new WebMessageArrayBufferPayload from vector for testing.
    static std::unique_ptr<WebMessageArrayBufferPayload> CreateForTesting(std::vector<uint8_t> data);
};

// Represent WebMessage payload type between browser and renderer process.
using WebMessagePayload = absl::variant<std::u16string, std::unique_ptr<WebMessageArrayBufferPayload>>;
...
BLINK_COMMON_EXPORT TransferableMessage EncodeWebMessagePayload(const WebMessagePayload& payload);

BLINK_COMMON_EXPORT std::optional<WebMessagePayload> DecodeToWebMessagePayload(TransferableMessage message);
