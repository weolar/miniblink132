...
public:
    ...
    ContentToVisibleTimeReporter& operator=(const ContentToVisibleTimeReporter&) = delete;
    ~ContentToVisibleTimeReporter();
    ...
    SuccessfulPresentationTimeCallback TabWasShown(bool has_saved_frames, mojom::RecordContentToVisibleTimeRequestPtr start_state);
    ...
    SuccessfulPresentationTimeCallback GetCallbackForNextFrameAfterUnfold(base::TimeTicks begin_timestamp);
...
private:
    ...
    void RecordHistogramsAndTraceEventsWithFrameTimingDetails(TabSwitchResult tab_switch_result, bool show_reason_tab_switching,
        bool show_reason_bfcache_restore, const viz::FrameTimingDetails& frame_timing_details);

    // Saves the given `state` and `has_saved_frames`, and invalidates all
    // existing callbacks that might reference the old state.
    void OverwriteTabSwitchStartState(mojom::RecordContentToVisibleTimeRequestPtr state, bool has_saved_frames);
