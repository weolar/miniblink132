...
struct BLINK_COMMON_EXPORT ColorProviderColorMaps {
    ...
    bool IsEmpty() const
    {
        return light_colors_map.empty() || dark_colors_map.empty() || forced_colors_map.empty();
    }

    bool operator==(const ColorProviderColorMaps& other) const
    {
        return light_colors_map == other.light_colors_map && dark_colors_map == other.dark_colors_map && forced_colors_map == other.forced_colors_map;
    }
};
