...
class COMPONENT_EXPORT(PRIVACY_BUDGET_TEST_SUPPORT) ScopedIdentifiabilityTestSampleCollector : public IdentifiabilitySampleCollector {
public:
    ScopedIdentifiabilityTestSampleCollector();
    ~ScopedIdentifiabilityTestSampleCollector() override;

    // IdentifiabilitySampleCollector
    void Record(ukm::UkmRecorder* recorder, ukm::SourceId source, std::vector<IdentifiableSample> metrics) override;
