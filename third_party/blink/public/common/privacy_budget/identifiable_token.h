...
public:
    ...
    constexpr IdentifiableToken()
        : value_(kIdentifiabilityDigestOfNoBytes)
    {
    }
    ...
    constexpr IdentifiableToken(T in) // NOLINT(google-explicit-constructor)
        : value_(base::IsValueInRangeForNumericType<TokenType, U>(in) ? in : internal::DigestOfObjectRepresentation<U>(in))
    {
    }
    ...
    constexpr IdentifiableToken(T in) // NOLINT(google-explicit-constructor)
        : value_(internal::DigestOfObjectRepresentation<double>(static_cast<double>(in)))
    {
    }
    ...
    //   }
    explicit IdentifiableToken(std::string_view s)
        : IdentifiableToken(ByteSpan((const uint8_t*)s.data(), s.size()))
    {
        // The cart is before the horse, but it's a static_assert<>.
        //     static_assert(
        //       std::is_same<ByteSpan,
    ...
    template <typename T, size_t Extent, typename U = std::remove_cvref_t<T>,
        typename std::enable_if_t<std::is_arithmetic<U>::value && !std::is_same<ByteSpan::element_type, T>::value>* = nullptr>
    // NOLINTNEXTLINE(google-explicit-constructor)
    ...
    template <typename T, size_t Extent, typename std::enable_if_t<!std::is_arithmetic<T>::value && !std::is_same<ByteSpan::element_type, T>::value>* = nullptr>
    // NOLINTNEXTLINE(google-explicit-constructor)
    IdentifiableToken(base::span<T, Extent> span)
    {
        ...
        for (const auto& element : span) {
            ...
            cur_digest = IdentifiabilityDigestOfBytes(base::as_bytes(base::make_span(digests)));
        }
        value_ = cur_digest;
    }
    ...
    template <typename T1, typename T2, typename... Trest> constexpr IdentifiableToken(T1 first, T2 second, Trest... rest)
    {
        TokenType samples[] = { IdentifiableToken(first).value_, IdentifiableToken(second).value_, (IdentifiableToken(rest).value_)... };
    ...
    // interfaces.
    int64_t ToUkmMetricValue() const
    {
        return value_;
    }
