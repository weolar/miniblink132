...
public:
    ...
    static_assert(static_cast<uint64_t>(ReservedSurfaceMetrics::kMaxValue)
            < std::min(ukm::builders::Identifiability::kGeneratorVersion_926NameHash, ukm::builders::Identifiability::kStudyGeneration_626NameHash),
    ...
    IdentifiableSurface()
        : IdentifiableSurface(kInvalidHash)
    {
    }
    ...
    // token.
    static constexpr IdentifiableSurface FromTypeAndToken(Type type, IdentifiableToken token)
    {
    ...
    constexpr uint64_t ToUkmMetricHash() const
    {
        return metric_hash_;
    }
    ...
    {
        return metric_hash_ != kInvalidHash;
    }

    friend constexpr auto operator<=>(const IdentifiableSurface& lhs, const IdentifiableSurface& rhs) = default;
    friend constexpr bool operator==(const IdentifiableSurface& lhs, const IdentifiableSurface& rhs) = default;
...
private:
    ...
    static constexpr uint64_t KeyFromSurfaceTypeAndInput(Type type, uint64_t input)
    {
    ...
    static constexpr std::tuple<Type, uint64_t> SurfaceTypeAndInputFromMetricKey(uint64_t metric)
    {
        return std::make_tuple(static_cast<Type>(metric & kTypeMask), metric >> kTypeBits);
    }
