...
template <typename T> using has_unique_object_representations = std::is_arithmetic<T>;
...
constexpr int64_t DigestOfObjectRepresentation(T in)
    requires(std::same_as<T, std::remove_cvref_t<T>> && std::is_trivially_copyable_v<T> && has_unique_object_representations<T>::value
        && sizeof(T) <= sizeof(int64_t))
{
    if constexpr (std::is_integral<T>::value && (std::is_signed<T>::value || sizeof(T) < sizeof(int64_t))) {
        ...
        base::span(bytes).template first<sizeof(T)>().copy_from(base::byte_span_from_ref(in));
