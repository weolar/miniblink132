...
public:
    ...
    IdentifiableTokenBuilder& AddAtomic(std::string_view string)
    {
        ...
        return AddAtomic(base::span<uint8_t>((uint8_t*)string.data(), string.size()));
    }
    ...
    template <typename T,
        typename std::enable_if_t<std::is_same<T, std::remove_cvref_t<T>>::value && internal::has_unique_object_representations<T>::value
