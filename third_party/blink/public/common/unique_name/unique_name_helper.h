...
public:
    ...
    public:
        ...
        virtual std::vector<std::string> CollectAncestorNames(BeginPoint begin_point, bool (*should_stop)(std::string_view)) const = 0;
    ...
    void set_propagated_name(const std::string& name)
    {
        unique_name_ = name;
    }
    ...
    std::string GenerateNameForNewChildFrame(const std::string& name, bool is_created_by_script) const;
    ...
    // tree.
    static std::string UpdateLegacyNameFromV24(std::string legacy_name, std::vector<Replacement>* replacements);

    static std::string CalculateLegacyNameForTesting(const FrameAdapter* frame, const std::string& name);
