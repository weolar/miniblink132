...
bool BLINK_COMMON_EXPORT ServiceWorkerScopeOrScriptUrlContainsDisallowedCharacter(const GURL& scope, const GURL& script_url, std::string* error_message);

// Returns true if `scope` matches `url`.
bool BLINK_COMMON_EXPORT ServiceWorkerScopeMatches(const GURL& scope, const GURL& url);
...
public:
    ...
    ServiceWorkerLongestScopeMatcher(const ServiceWorkerLongestScopeMatcher&) = delete;
    ServiceWorkerLongestScopeMatcher& operator=(const ServiceWorkerLongestScopeMatcher&) = delete;
