...
public:
    ...
    static void SaveResponseInfo(const blink::mojom::FetchAPIResponse& response, network::mojom::URLResponseHead* out_head);
    ...
    static int ReadBlobResponseBody(mojo::Remote<blink::mojom::Blob>* blob, uint64_t blob_size, base::OnceCallback<void(int net_error)> on_blob_read_complete,
        mojo::ScopedDataPipeConsumerHandle* handle_out);

    static bool IsMainRequestDestination(network::mojom::RequestDestination destination);

    static const char* FetchResponseSourceToSuffix(network::mojom::FetchResponseSource source);
};
