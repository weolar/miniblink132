...
struct BLINK_COMMON_EXPORT FramePolicy {
    FramePolicy();
    FramePolicy(network::mojom::WebSandboxFlags sandbox_flags, const ParsedPermissionsPolicy& container_policy,
...
bool BLINK_COMMON_EXPORT operator==(const FramePolicy& lhs, const FramePolicy& rhs);
bool BLINK_COMMON_EXPORT operator!=(const FramePolicy& lhs, const FramePolicy& rhs);
