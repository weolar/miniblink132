...
public:
    ...
    WebPackageRequestMatcher(const net::HttpRequestHeaders& request_headers, const std::string& accept_langs);
    ...
    std::vector<std::string>::const_iterator FindBestMatchingVariantKey(const std::string& variants, const std::vector<std::string>& variant_keys_list) const;

    // Returns the index of best matching variant key within the all possible
    // key for |variants|, in lexicographic (row-major) order.
    std::optional<size_t> FindBestMatchingIndex(const std::string& variants) const;

private:
    net::HttpRequestHeaders request_headers_;

    static bool MatchRequest(const net::HttpRequestHeaders& request_headers, const HeaderMap& response_headers);
    ...
    static std::vector<std::string>::const_iterator FindBestMatchingVariantKey(
        const net::HttpRequestHeaders& request_headers, const std::string& variants, const std::vector<std::string>& variant_key_list);

    static std::optional<size_t> FindBestMatchingIndex(const net::HttpRequestHeaders& request_headers, const std::string& variants);
    ...
    FRIEND_TEST_ALL_PREFIXES(WebPackageRequestMatcherTest, FindBestMatchingVariantKey);
