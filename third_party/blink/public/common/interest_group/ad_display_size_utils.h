...
BLINK_COMMON_EXPORT std::string ConvertAdSizeUnitToString(const blink::AdSize::LengthUnit& unit);

// Converts a valid ad size back to a string.
BLINK_COMMON_EXPORT std::string ConvertAdSizeToString(const blink::AdSize& ad_size);

// Converts a valid ad dimension back to a string.
BLINK_COMMON_EXPORT std::string ConvertAdDimensionToString(double value, AdSize::LengthUnit units);
...
// - " 25sw "
// - "100"
BLINK_COMMON_EXPORT std::tuple<double, blink::AdSize::LengthUnit> ParseAdSizeString(std::string_view input);
