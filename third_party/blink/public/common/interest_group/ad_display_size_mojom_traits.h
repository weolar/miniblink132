...
template <> struct BLINK_COMMON_EXPORT StructTraits<blink::mojom::AdSizeDataView, blink::AdSize> {
    static double width(const blink::AdSize& size)
    {
        return size.width;
    }
    ...
    static double height(const blink::AdSize& size)
    {
        return size.height;
    }
...
template <> struct BLINK_COMMON_EXPORT StructTraits<blink::mojom::AdDescriptorDataView, blink::AdDescriptor> {
    static GURL url(const blink::AdDescriptor& ad_descriptor)
    {
        return ad_descriptor.url;
    }

    static std::optional<blink::AdSize> size(const blink::AdDescriptor& ad_descriptor)
    {
        return ad_descriptor.size;
    }

    static bool Read(blink::mojom::AdDescriptorDataView data, blink::AdDescriptor* ad_descriptor);
};
