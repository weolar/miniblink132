...
public:
    ...
    void SetCurrencyCodeForTesting(const std::string& in)
    {
        currency_code_ = in;
    }

    friend BLINK_COMMON_EXPORT bool operator==(const AdCurrency&, const AdCurrency&);
...
BLINK_COMMON_EXPORT bool VerifyAdCurrencyCode(const std::optional<AdCurrency>& expected, const std::optional<AdCurrency>& actual);
...
BLINK_COMMON_EXPORT std::string PrintableAdCurrency(const std::optional<AdCurrency>& currency);
