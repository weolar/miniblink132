...
template <> struct BLINK_COMMON_EXPORT StructTraits<blink::mojom::SHA256HashValueDataView, net::SHA256HashValue> {
    static const std::string data(const net::SHA256HashValue& value)
    {
        return std::string(reinterpret_cast<const char*>(value.data), sizeof(value.data));
    }

    static bool Read(blink::mojom::SHA256HashValueDataView input, net::SHA256HashValue* out);
};
