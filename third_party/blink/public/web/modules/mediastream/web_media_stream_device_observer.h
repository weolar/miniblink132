...
public:
    ...
    using OnDeviceStoppedCb = base::RepeatingCallback<void(const MediaStreamDevice& device)>;
    using OnDeviceChangedCb = base::RepeatingCallback<void(const MediaStreamDevice& old_device, const MediaStreamDevice& new_device)>;
    using OnDeviceRequestStateChangeCb = base::RepeatingCallback<void(const MediaStreamDevice& device, const mojom::MediaStreamStateChange new_state)>;
    using OnDeviceCaptureConfigurationChangeCb = base::RepeatingCallback<void(const MediaStreamDevice& device)>;
    using OnDeviceCaptureHandleChangeCb = base::RepeatingCallback<void(const MediaStreamDevice& device)>;
#if !BUILDFLAG(IS_ANDROID) && !BUILDFLAG(IS_IOS)
    using OnZoomLevelChangeCb = base::RepeatingCallback<void(const MediaStreamDevice& device, int zoom_level)>;
#endif
    ...
    struct StreamCallbacks {
        ...
        OnDeviceCaptureConfigurationChangeCb on_device_capture_configuration_change_cb;
    ...
    void AddStreams(const WebString& label, const mojom::blink::StreamDevicesSet& stream_devices_set, const StreamCallbacks& stream_callbacks);
