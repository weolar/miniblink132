...
class BLINK_MODULES_EXPORT MediaStreamVideoSource : public WebPlatformMediaStreamSource {
public:
    enum {
    ...
    using SourceStartCallback = base::OnceCallback<void(WebPlatformMediaStreamSource* source, mojom::MediaStreamRequestResult result)>;

    explicit MediaStreamVideoSource(scoped_refptr<base::SingleThreadTaskRunner> main_task_runner);
    ...
    void AddTrack(MediaStreamVideoTrack* track, const VideoTrackAdapterSettings& track_adapter_settings, const VideoCaptureDeliverFrameCB& frame_callback,
        const VideoCaptureNotifyFrameDroppedCB& dropped_callback, const EncodedVideoFrameCB& encoded_frame_callback,
        const VideoCaptureSubCaptureTargetVersionCB& sub_capture_target_version_callback, const VideoTrackSettingsCallback& settings_callback,
        const VideoTrackFormatCallback& format_callback, ConstraintsOnceCallback callback);
    ...
    void Restart(const media::VideoCaptureFormat& new_format, RestartCallback callback);
    ...
    virtual void ApplySubCaptureTarget(media::mojom::SubCaptureTargetType type, const base::Token& sub_capture_target, uint32_t sub_capture_target_version,
    ...
    bool IsStoppedForRestart() const
    {
        return state_ == STOPPED_FOR_RESTART;
    }
...
protected:
    ...
    virtual void StartSourceImpl(VideoCaptureDeliverFrameCB frame_callback, EncodedVideoFrameCB encoded_frame_callback,
    ...
    enum State { NEW, STARTING, STOPPING_FOR_RESTART, STOPPED_FOR_RESTART, RESTARTING, STARTED, ENDED };
    State state() const
    {
        return state_;
    }
...
private:
    ...
    void UpdateTrackSettings(MediaStreamVideoTrack* track, const VideoTrackAdapterSettings& adapter_settings);
