...
protected:
    ...
    // different thread.
    void ConnectToTrack(const WebMediaStreamTrack& track, const VideoCaptureDeliverFrameCB& callback, IsSecure is_secure, UsesAlpha uses_alpha);
    void ConnectEncodedToTrack(const WebMediaStreamTrack& track, const EncodedVideoFrameCB& callback);
