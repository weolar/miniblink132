...
namespace blink {
using CreateSurfaceLayerBridgeCB = base::OnceCallback<std::unique_ptr<WebSurfaceLayerBridge>(WebSurfaceLayerBridgeObserver*, cc::UpdateSubmissionStateCB)>;
...
template <typename TimerFiredClass> class TaskRunnerTimer;
class TimerBase;
class WebLocalFrame;
...
class BLINK_MODULES_EXPORT WebMediaPlayerMS : public WebMediaStreamObserver,
...
public:
    ...
    WebMediaPlayerMS(WebLocalFrame* frame, WebMediaPlayerClient* client, WebMediaPlayerDelegate* delegate, std::unique_ptr<media::MediaLog> media_log,
        scoped_refptr<base::SingleThreadTaskRunner> main_render_task_runner, scoped_refptr<base::SequencedTaskRunner> video_task_runner,
        scoped_refptr<base::SingleThreadTaskRunner> compositor_task_runner, scoped_refptr<base::SequencedTaskRunner> media_task_runner,
        scoped_refptr<base::TaskRunner> worker_task_runner, media::GpuVideoAcceleratorFactories* gpu_factories, const WebString& sink_id,
        CreateSurfaceLayerBridgeCB create_bridge_callback, std::unique_ptr<WebVideoFrameSubmitter> submitter_, bool use_surface_layer);
    ...
    void OnFirstFrameReceived(media::VideoTransformation video_transform, bool is_opaque);
...
private:
    ...
    void SetGpuMemoryBufferVideoForTesting(media::GpuMemoryBufferVideoFramePool* gpu_memory_buffer_pool);
    void SetMediaStreamRendererFactoryForTesting(std::unique_ptr<MediaStreamRendererFactory>);
    ...
    std::unique_ptr<TaskRunnerTimer<WebMediaPlayerMS>> stop_force_begin_frames_timer_;
