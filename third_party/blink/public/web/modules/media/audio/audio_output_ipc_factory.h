...
public:
    explicit AudioOutputIPCFactory(scoped_refptr<base::SingleThreadTaskRunner> io_task_runner);
    ...
    void RegisterRemoteFactory(const blink::LocalFrameToken& frame_token, const blink::BrowserInterfaceBrokerProxy& interface_broker);
    ...
    std::unique_ptr<media::AudioOutputIPC> CreateAudioOutputIPC(const blink::LocalFrameToken& frame_token) const;
