...
public:
    ...
    // painful...
    static WebLocalFrame* CreateMainFrame(WebView*, WebLocalFrameClient*, blink::InterfaceRegistry*,
        CrossVariantMojoRemote<mojom::BrowserInterfaceBrokerInterfaceBase>, const LocalFrameToken& frame_token, const DocumentToken& document_token,
        std::unique_ptr<blink::WebPolicyContainer> policy_container, WebFrame* opener = nullptr, const WebString& name = WebString(),
        network::mojom::WebSandboxFlags = network::mojom::WebSandboxFlags::kNone, const WebURL& base_url = WebURL());
    ...
    // frame.
    static WebLocalFrame* CreateProvisional(WebLocalFrameClient*, InterfaceRegistry*, CrossVariantMojoRemote<mojom::BrowserInterfaceBrokerInterfaceBase>,
        const LocalFrameToken& frame_token, WebFrame* previous_web_frame, const FramePolicy&, const WebString& name, WebView* web_view);
    ...
    virtual WebLocalFrame* CreateLocalChild(mojom::TreeScopeType, WebLocalFrameClient*, InterfaceRegistry*, const LocalFrameToken& frame_token) = 0;
    ...
    virtual void SetNotRestoredReasons(const mojom::BackForwardCacheNotRestoredReasonsPtr&) = 0;

    // Sets LCP Critical Path Detector hint for the current frame that was
    // available at the navigation commit timing.
    virtual void SetLCPPHint(const mojom::LCPCriticalPathPredictorNavigationTimeHintPtr&) = 0;

    // Tests whether the policy-controlled feature is enabled in this frame.
    virtual bool IsFeatureEnabled(const mojom::PermissionsPolicyFeature&) const = 0;
    ...
    WebFrameWidget* InitializeFrameWidget(CrossVariantMojoAssociatedRemote<mojom::FrameWidgetHostInterfaceBase> frame_widget_host,
        CrossVariantMojoAssociatedReceiver<mojom::FrameWidgetInterfaceBase> frame_widget,
        CrossVariantMojoAssociatedRemote<mojom::WidgetHostInterfaceBase> widget_host, CrossVariantMojoAssociatedReceiver<mojom::WidgetInterfaceBase> widget,
        const viz::FrameSinkId& frame_sink_id, bool is_for_nested_main_frame = false, bool is_for_scalable_page = true, bool hidden = false);
    ...
    virtual const std::optional<base::UnguessableToken>& GetEmbeddingToken() const = 0;
    ...
    virtual void DownloadURL(const WebURLRequest& request, network::mojom::RedirectMode cross_origin_redirect_behavior,
    ...
    virtual void ExecuteScriptInIsolatedWorld(int32_t world_id, const WebScriptSource&, BackForwardCacheAware) = 0;

    // `world_id` must be > kMainDOMWorldId and < kEmbedderWorldIdLimit (a
    // high number used internally).
    [[nodiscard]] virtual v8::Local<v8::Value> ExecuteScriptInIsolatedWorldAndReturnValue(int32_t world_id, const WebScriptSource&, BackForwardCacheAware) = 0;
    ...
    virtual v8::Local<v8::Value> ExecuteScriptAndReturnValue(const WebScriptSource&) = 0;

    // Call the function with the given receiver and arguments
    virtual v8::MaybeLocal<v8::Value> ExecuteMethodAndReturnValue(v8::Local<v8::Function>, v8::Local<v8::Value>, int argc, v8::Local<v8::Value> argv[]) = 0;

    // Call the function with the given receiver and arguments, bypassing
    // canExecute().
    virtual v8::MaybeLocal<v8::Value> CallFunctionEvenIfScriptDisabled(v8::Local<v8::Function>, v8::Local<v8::Value>, int argc, v8::Local<v8::Value> argv[])
        = 0;
    ...
    virtual int32_t GetScriptContextWorldId(v8::Local<v8::Context> script_context) const = 0;
    virtual v8::Local<v8::Context> GetScriptContextFromWorldId(v8::Isolate* isolate, int world_id) const = 0;
    ...
    virtual void RequestExecuteV8Function(
        v8::Local<v8::Context>, v8::Local<v8::Function>, v8::Local<v8::Value> receiver, int argc, v8::Local<v8::Value> argv[], WebScriptExecutionCallback)
        = 0;
    ...
    virtual void RequestExecuteScript(int32_t world_id, base::span<const WebScriptSource> sources, mojom::UserActivationOption, mojom::EvaluationTiming,
        mojom::LoadEventBlockingOption, WebScriptExecutionCallback, BackForwardCacheAware, mojom::WantResultOption, mojom::PromiseResultOption)
        = 0;
    ...
    void AddMessageToConsole(const WebConsoleMessage& message, bool discard_duplicates = false)
    {
    ...
    void AddGenericIssue(mojom::GenericIssueErrorType error_type, int violating_node_id, const WebString& violating_node_attribute)
    {
        AddGenericIssueImpl(error_type, violating_node_id, violating_node_attribute);
    }

    void AddGenericIssue(mojom::GenericIssueErrorType error_type, int violating_node_id)
    {
    ...
    virtual WebString Prompt(const WebString& message, const WebString& default_value) = 0;
    ...
    // the console.
    virtual void GenerateInterventionReport(const WebString& message_id, const WebString& message) = 0;
    ...
    virtual bool FirstRectForCharacterRange(uint32_t location, uint32_t length, gfx::Rect&) const = 0;
    ...
    virtual bool SelectionTextDirection(base::i18n::TextDirection& start, base::i18n::TextDirection& end) const = 0;
    ...
    virtual void SetTextDirectionForTesting(base::i18n::TextDirection direction) = 0;
    ...
    virtual void TextSelectionChanged(const WebString& selection_text, uint32_t offset, const gfx::Range& range) = 0;
    ...
    virtual void SelectRange(const WebRange&, HandleVisibilityBehavior, mojom::SelectionMenuBehavior, SelectionSetFocusBehavior) = 0;
    ...
    virtual void MoveRangeSelection(const gfx::Point& base, const gfx::Point& extent, WebFrame::TextGranularity = kCharacterGranularity) = 0;
    virtual void MoveCaretSelection(const gfx::Point&) = 0;

    virtual bool SetEditableSelectionOffsets(int start, int end) = 0;
    virtual bool AddImeTextSpansToExistingText(const WebVector<ui::ImeTextSpan>& ime_text_spans, unsigned text_start, unsigned text_end) = 0;
    virtual bool ClearImeTextSpansByType(ui::ImeTextSpan::Type type, unsigned text_start, unsigned text_end) = 0;
    virtual bool SetCompositionFromExistingText(int composition_start, int composition_end, const WebVector<ui::ImeTextSpan>& ime_text_spans) = 0;
    virtual void ExtendSelectionAndDelete(int before, int after) = 0;
    virtual void ExtendSelectionAndReplace(int before, int after, const WebString& replacement_text) = 0;
    ...
    virtual bool FindForTesting(
        int identifier, const WebString& search_text, bool match_case, bool forward, bool new_session, bool force, bool wrap_within_frame, bool async)
        = 0;
    ...
    virtual void SetTickmarks(const WebElement& target, const WebVector<gfx::Rect>& tickmarks) = 0;
    ...
    virtual void ShowContextMenuFromExternal(
        const UntrustworthyContextMenuParams& params, CrossVariantMojoAssociatedRemote<blink::mojom::ContextMenuClientInterfaceBase> context_menu_client)
        = 0;
    ...
    virtual scoped_refptr<base::SingleThreadTaskRunner> GetTaskRunner(TaskType) = 0;
    ...
    virtual std::unique_ptr<WebAssociatedURLLoader> CreateAssociatedURLLoader(const WebAssociatedURLLoaderOptions&) = 0;
    ...
    virtual void DispatchBeforePrintEvent(base::WeakPtr<WebPrintClient> print_client) = 0;
    ...
    virtual uint32_t PrintBegin(const WebPrintParams& print_params, const WebNode& constrain_to_node) = 0;
    ...
    virtual bool CapturePaintPreview(const gfx::Rect& bounds, cc::PaintCanvas* canvas, bool include_linked_destinations, bool skip_accelerated_content) = 0;
    ...
    virtual WebPerformanceMetricsForReporting PerformanceMetricsForReporting() const = 0;
    virtual WebPerformanceMetricsForNestedContexts PerformanceMetricsForNestedContexts() const = 0;
    ...
    virtual void NotifyUserActivation(mojom::UserActivationNotificationType notification_type) = 0;
    ...
    virtual bool ConsumeTransientUserActivation(UserActivationUpdateSource update_source = UserActivationUpdateSource::kRenderer) = 0;
    ...
    virtual gfx::Size SpoolSizeInPixelsForTesting(const WebVector<uint32_t>& pages) = 0;
    ...
    virtual void PrintPagesForTesting(cc::PaintCanvas*, const gfx::Size& spool_size_in_pixels, const WebVector<uint32_t>* pages = nullptr) = 0;
    ...
    // key.
    virtual void SetLocalStorageArea(CrossVariantMojoRemote<mojom::StorageAreaInterfaceBase> local_storage_area) = 0;

    // Sets a session storage area which can be used for this frame. This storage
    // area is ignored if a cached storage area already exists for the storage
    // key and namespace.
    virtual void SetSessionStorageArea(CrossVariantMojoRemote<mojom::StorageAreaInterfaceBase> session_storage_area) = 0;
    ...
    // callback.
    virtual void AddHitTestOnTouchStartCallback(base::RepeatingCallback<void(const blink::WebHitTestResult&)> callback) = 0;
    ...
    virtual bool AllowStorageAccessSyncAndNotify(WebContentSettingsClient::StorageType storage_type) = 0;

protected:
    explicit WebLocalFrame(mojom::TreeScopeType scope, const LocalFrameToken& frame_token)
    ...
    virtual void AddMessageToConsoleImpl(const WebConsoleMessage&, bool discard_duplicates) = 0;
    virtual void AddInspectorIssueImpl(blink::mojom::InspectorIssueCode code) = 0;
    virtual void AddGenericIssueImpl(blink::mojom::GenericIssueErrorType error_type, int violating_node_id) = 0;
    virtual void AddGenericIssueImpl(blink::mojom::GenericIssueErrorType error_type, int violating_node_id, const WebString& violating_node_attribute) = 0;
    virtual void CreateFrameWidgetInternal(base::PassKey<WebLocalFrame> pass_key,
        CrossVariantMojoAssociatedRemote<mojom::FrameWidgetHostInterfaceBase> frame_widget_host,
        CrossVariantMojoAssociatedReceiver<mojom::FrameWidgetInterfaceBase> frame_widget,
        CrossVariantMojoAssociatedRemote<mojom::WidgetHostInterfaceBase> widget_host, CrossVariantMojoAssociatedReceiver<mojom::WidgetInterfaceBase> widget,
        const viz::FrameSinkId& frame_sink_id, bool is_for_nested_main_frame, bool is_for_scalable_page, bool hidden)
        = 0;
};
