...
struct BLINK_EXPORT WebNavigationInfo {
    ...
    mojom::RequestContextFrameType frame_type = mojom::RequestContextFrameType::kNone;
    ...
    blink::mojom::TriggeringEventInfo triggering_event_info = blink::mojom::TriggeringEventInfo::kUnknown;
    ...
    network::mojom::CSPDisposition should_check_main_world_content_security_policy = network::mojom::CSPDisposition::CHECK;
    ...
    CrossVariantMojoRemote<mojom::NavigationStateKeepAliveHandleInterfaceBase> initiator_navigation_state_keep_alive_handle;

    // The initiator frame's LocalDOMWindow's Storage Access API status.
    net::StorageAccessApiStatus storage_access_api_status = net::StorageAccessApiStatus::kNone;
...
struct BLINK_EXPORT WebNavigationParams {
    ...
    explicit WebNavigationParams(
        const blink::DocumentToken& document_token, const base::UnguessableToken& devtools_navigation_token, const base::Uuid& base_auction_nonce);
    ...
    // - unit tests.
    static std::unique_ptr<WebNavigationParams> CreateFromInfo(const WebNavigationInfo&);

    // Shortcut for loading html with "text/html" mime type and "UTF8" encoding.
    static std::unique_ptr<WebNavigationParams> CreateWithEmptyHTMLForTesting(const WebURL& base_url);
    static std::unique_ptr<WebNavigationParams> CreateWithHTMLStringForTesting(base::span<const char> html, const WebURL& base_url);
    ...
    static void FillStaticResponse(WebNavigationParams*, WebString mime_type, WebString text_encoding, base::span<const char> data);
#if INSIDE_BLINK
    static void FillStaticResponse(WebNavigationParams*, WebString mime_type, WebString text_encoding, SharedBuffer* data);
#endif
    ...
    // document.
    std::unique_ptr<blink::WebServiceWorkerNetworkProvider> service_worker_network_provider;
    ...
    struct BLINK_EXPORT PrefetchedSignedExchange {
        ...
        PrefetchedSignedExchange(const WebURL& outer_url, const WebString& header_integrity, const WebURL& inner_url, const WebURLResponse& inner_response,
            CrossVariantMojoRemote<network::mojom::URLLoaderFactoryInterfaceBase> loader_factory);
        ~PrefetchedSignedExchange();
    ...
    WebVector<std::unique_ptr<PrefetchedSignedExchange>> prefetched_signed_exchanges;
    ...
    // renderer side.
    network::mojom::NavigationDeliveryType navigation_delivery_type = network::mojom::NavigationDeliveryType::kDefault;
    ...
    net::StorageAccessApiStatus load_with_storage_access = net::StorageAccessApiStatus::kNone;
    ...
    std::optional<BrowsingContextGroupInfo> browsing_context_group_info = std::nullopt;
    ...
    std::optional<base::flat_map<mojom::PermissionName, mojom::PermissionStatus>> initial_permission_statuses;
};
