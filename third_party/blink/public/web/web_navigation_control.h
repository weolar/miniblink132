...
public:
    ...
    virtual void CommitNavigation(std::unique_ptr<WebNavigationParams> navigation_params, std::unique_ptr<WebDocumentLoader::ExtraData> extra_data) = 0;
    ...
    virtual mojom::CommitResult CommitSameDocumentNavigation(const WebURL&, WebFrameLoadType, const WebHistoryItem&, bool is_client_redirect,
    ...
    virtual void MaybeStartOutermostMainFrameNavigation(const WebVector<WebURL>& urls) const = 0;
    ...
    virtual void Load(const WebURLRequest& request, WebFrameLoadType frame_load_type, const WebHistoryItem& item
    /*WebHistoryLoadType webHistoryLoadType*/) = 0; // weolar

protected:
    explicit WebNavigationControl(mojom::TreeScopeType scope, const LocalFrameToken& frame_token)
