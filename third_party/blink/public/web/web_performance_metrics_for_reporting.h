...
struct LargestContentfulPaintDetailsForReporting {
    ...
    blink::LargestContentfulPaintType type = blink::LargestContentfulPaintType::kNone;
...
public:
    ...
    static constexpr int kRequestAnimationFramesToRecordAfterBackForwardCacheRestore = 3;

    struct BackForwardCacheRestoreTiming {
        double navigation_start = 0;
        double first_paint = 0;
        std::array<double, kRequestAnimationFramesToRecordAfterBackForwardCacheRestore> request_animation_frames = {};
        std::optional<base::TimeDelta> first_input_delay;
    };

    using BackForwardCacheRestoreTimings = WebVector<BackForwardCacheRestoreTiming>;
    ...
    WebPerformanceMetricsForReporting& operator=(const WebPerformanceMetricsForReporting& p)
    {
        Assign(p);
        return *this;
    }
    ...
    LargestContentfulPaintDetailsForReporting LargestContentfulDetailsForMetrics() const;
    LargestContentfulPaintDetailsForReporting SoftNavigationLargestContentfulDetailsForMetrics() const;
    ...
    std::optional<std::tuple<std::string, base::TimeDelta>> CustomUserTimingMark() const;
