...
class BLINK_PLATFORM_EXPORT VideoFrameCompositor : public media::VideoRendererSink, public cc::VideoFrameProvider {
public:
    // Used to report back the time when the new frame has been processed.
    using OnNewProcessedFrameCB = base::OnceCallback<void(base::TimeTicks, bool is_frame_readable)>;
    ...
    VideoFrameCompositor(const scoped_refptr<base::SingleThreadTaskRunner>& task_runner, std::unique_ptr<WebVideoFrameSubmitter> submitter);
    ...
    virtual void EnableSubmission(const viz::SurfaceId& id, media::VideoTransformation rotation, bool force_submit);
    ...
    void Stop() override;
    void PaintSingleFrame(scoped_refptr<media::VideoFrame> frame, bool repaint_duplicate_frame = false) override;
    ...
    virtual std::unique_ptr<WebMediaPlayer::VideoFramePresentationMetadata> GetLastPresentedFrameMetadata();
    ...
    void set_submitter_for_test(std::unique_ptr<WebVideoFrameSubmitter> submitter)
    {
...
private:
    ...
    bool ProcessNewFrame(scoped_refptr<media::VideoFrame> frame, base::TimeTicks presentation_time, bool repaint_duplicate_frame);

    void SetCurrentFrame_Locked(scoped_refptr<media::VideoFrame> frame, base::TimeTicks expected_display_time);
    ...
    void BackgroundRender(media::VideoRendererSink::RenderCallback::RenderingMode mode = media::VideoRendererSink::RenderCallback::RenderingMode::kBackground);
    ...
    bool CallRender(base::TimeTicks deadline_min, base::TimeTicks deadline_max, media::VideoRendererSink::RenderCallback::RenderingMode mode);
    ...
    raw_ptr<media::VideoRendererSink::RenderCallback> callback_ GUARDED_BY(callback_lock_) = nullptr;
    ...
    base::TimeDelta last_interval_ GUARDED_BY(callback_lock_) = base::Seconds(1.0 / 60);

    // AutoOpenCloseEvent for begin/end events.
    std::unique_ptr<base::trace_event::AutoOpenCloseEvent<kTracingCategory>> auto_open_close_;
