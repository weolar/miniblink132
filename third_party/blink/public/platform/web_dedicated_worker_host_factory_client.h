...
public:
    using CreateWorkerHostCallback
        = base::OnceCallback<void(const network::CrossOriginEmbedderPolicy&, CrossVariantMojoRemote<mojom::BackForwardCacheControllerHostInterfaceBase>)>;
    ...
    virtual void CreateWorkerHostDeprecated(
        const DedicatedWorkerToken& dedicated_worker_token, const blink::WebURL& script_url, const WebSecurityOrigin& origin, CreateWorkerHostCallback callback)
        = 0;
    // For PlzDedicatedWorker.
    virtual void CreateWorkerHost(const DedicatedWorkerToken& dedicated_worker_token, const blink::WebURL& script_url,
        network::mojom::CredentialsMode credentials_mode, const blink::WebFetchClientSettingsObject& fetch_client_settings_object,
        CrossVariantMojoRemote<mojom::BlobURLTokenInterfaceBase> blob_url_token, net::StorageAccessApiStatus storage_access_api_status)
        = 0;

    // Clones the given WebWorkerFetchContext for nested workers.
    virtual scoped_refptr<WebWorkerFetchContext> CloneWorkerFetchContext(WebWorkerFetchContext*, scoped_refptr<base::SingleThreadTaskRunner>) = 0;
};
