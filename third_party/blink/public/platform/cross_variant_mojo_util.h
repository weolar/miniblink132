...
template <typename Interface> class CrossVariantMojoReceiver {
public:
    CrossVariantMojoReceiver() = default;
    ~CrossVariantMojoReceiver() = default;

    CrossVariantMojoReceiver(CrossVariantMojoReceiver&&) noexcept = default;
    CrossVariantMojoReceiver& operator=(CrossVariantMojoReceiver&&) noexcept = default;

    CrossVariantMojoReceiver(const CrossVariantMojoReceiver&) = delete;
    CrossVariantMojoReceiver& operator=(const CrossVariantMojoReceiver&) = default;

    template <typename VariantInterface, typename CrossVariantBase = typename VariantInterface::Base_,
        std::enable_if_t<std::is_same<CrossVariantBase, Interface>::value>* = nullptr>
...
template <typename Interface> class CrossVariantMojoRemote {
public:
    CrossVariantMojoRemote() = default;
    ~CrossVariantMojoRemote() = default;

    CrossVariantMojoRemote(CrossVariantMojoRemote&&) noexcept = default;
    CrossVariantMojoRemote& operator=(CrossVariantMojoRemote&&) noexcept = default;
    ...
    CrossVariantMojoRemote(mojo::PendingRemote<VariantInterface> remote)
        : version_(remote.version())
        , pipe_(remote.PassPipe())
    {
    }
...
template <typename Interface> class CrossVariantMojoAssociatedReceiver {
public:
    CrossVariantMojoAssociatedReceiver() = default;
    ~CrossVariantMojoAssociatedReceiver() = default;

    CrossVariantMojoAssociatedReceiver(CrossVariantMojoAssociatedReceiver&&) = default;
    CrossVariantMojoAssociatedReceiver& operator=(CrossVariantMojoAssociatedReceiver&&) = default;

    CrossVariantMojoAssociatedReceiver(const CrossVariantMojoAssociatedReceiver&) = delete;
    CrossVariantMojoAssociatedReceiver& operator=(const CrossVariantMojoAssociatedReceiver&) = default;

    template <typename VariantInterface, typename CrossVariantBase = typename VariantInterface::Base_,
        std::enable_if_t<std::is_same<CrossVariantBase, Interface>::value>* = nullptr>
    CrossVariantMojoAssociatedReceiver(mojo::PendingAssociatedReceiver<VariantInterface> receiver)
...
private:
    friend struct mojo::PendingAssociatedReceiverConverter<CrossVariantMojoAssociatedReceiver>;

    mojo::ScopedInterfaceEndpointHandle handle_;
};

template <typename Interface> class CrossVariantMojoAssociatedRemote {
public:
    CrossVariantMojoAssociatedRemote() = default;
    ~CrossVariantMojoAssociatedRemote() = default;

    CrossVariantMojoAssociatedRemote(CrossVariantMojoAssociatedRemote&&) = default;
    CrossVariantMojoAssociatedRemote& operator=(CrossVariantMojoAssociatedRemote&&) = default;

    CrossVariantMojoAssociatedRemote(const CrossVariantMojoAssociatedRemote&) = delete;
    CrossVariantMojoAssociatedRemote& operator=(const CrossVariantMojoAssociatedRemote&) = default;

    template <typename VariantInterface, typename CrossVariantBase = typename VariantInterface::Base_,
        std::enable_if_t<std::is_same<CrossVariantBase, Interface>::value>* = nullptr>
    CrossVariantMojoAssociatedRemote(mojo::PendingAssociatedRemote<VariantInterface> remote)
        : version_(remote.version())
        , handle_(remote.PassHandle())
    {
    }
...
private:
    friend struct mojo::PendingAssociatedRemoteConverter<CrossVariantMojoAssociatedRemote>;
...
template <typename VariantBase> auto ToCrossVariantMojoType(mojo::PendingReceiver<VariantBase>&& in)
{
    return blink::CrossVariantMojoReceiver<typename VariantBase::Base_>(std::move(in));
}

template <typename VariantBase> auto ToCrossVariantMojoType(mojo::PendingRemote<VariantBase>&& in)
{
    return blink::CrossVariantMojoRemote<typename VariantBase::Base_>(std::move(in));
}
...
template <typename VariantBase> auto ToCrossVariantAssociatedMojoType(mojo::PendingAssociatedReceiver<VariantBase>&& in)
{
    return blink::CrossVariantMojoAssociatedReceiver<typename VariantBase::Base_>(std::move(in));
}

template <typename VariantBase> auto ToCrossVariantAssociatedMojoType(mojo::PendingAssociatedRemote<VariantBase>&& in)
{
    return blink::CrossVariantMojoAssociatedRemote<typename VariantBase::Base_>(std::move(in));
}
...
template <typename CrossVariantBase> struct PendingReceiverConverter<blink::CrossVariantMojoReceiver<CrossVariantBase>> {
    template <typename VariantBase> static PendingReceiver<VariantBase> To(blink::CrossVariantMojoReceiver<CrossVariantBase>&& in)
    {
        return in.pipe_.is_valid() ? PendingReceiver<VariantBase>(std::move(in.pipe_)) : PendingReceiver<VariantBase>();
    }
};

template <typename CrossVariantBase> struct PendingRemoteConverter<blink::CrossVariantMojoRemote<CrossVariantBase>> {
    template <typename VariantBase> static PendingRemote<VariantBase> To(blink::CrossVariantMojoRemote<CrossVariantBase>&& in)
    {
        return in.pipe_.is_valid() ? PendingRemote<VariantBase>(std::move(in.pipe_), in.version_) : PendingRemote<VariantBase>();
    }
};

template <typename CrossVariantBase> struct PendingAssociatedReceiverConverter<blink::CrossVariantMojoAssociatedReceiver<CrossVariantBase>> {
    template <typename VariantBase> static PendingAssociatedReceiver<VariantBase> To(blink::CrossVariantMojoAssociatedReceiver<CrossVariantBase>&& in)
    {
        return in.handle_.is_valid() ? PendingAssociatedReceiver<VariantBase>(std::move(in.handle_)) : PendingAssociatedReceiver<VariantBase>();
    }
};

template <typename CrossVariantBase> struct PendingAssociatedRemoteConverter<blink::CrossVariantMojoAssociatedRemote<CrossVariantBase>> {
    template <typename VariantBase> static PendingAssociatedRemote<VariantBase> To(blink::CrossVariantMojoAssociatedRemote<CrossVariantBase>&& in)
    {
        return in.handle_.is_valid() ? PendingAssociatedRemote<VariantBase>(std::move(in.handle_), in.version_) : PendingAssociatedRemote<VariantBase>();
    }
};
