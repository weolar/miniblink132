...
public:
    ...
    BrowserInterfaceBrokerProxy& operator=(const BrowserInterfaceBrokerProxy&) = delete;

    virtual ~BrowserInterfaceBrokerProxy();

    virtual CrossVariantMojoReceiver<mojom::BrowserInterfaceBrokerInterfaceBase> Reset(scoped_refptr<base::SingleThreadTaskRunner> task_runner) = 0;
    ...
    void GetInterface(const std::string& name, mojo::ScopedMessagePipeHandle pipe) const
    {
    ...
    // named interface.
    virtual bool SetBinderForTesting(const std::string& name, base::RepeatingCallback<void(mojo::ScopedMessagePipeHandle)> binder) const = 0;
};

// Returns an instance of BrowserInterfaceBrokerProxy that is safe to use but is
// not connected to anything.
BLINK_PLATFORM_EXPORT BrowserInterfaceBrokerProxy& GetEmptyBrowserInterfaceBroker();
