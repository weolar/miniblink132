...
public:
    ...
    float XHeight() const;
    void DrawText(cc::PaintCanvas*, const WebTextRun&, const gfx::PointF& left_baseline, SkColor) const;
    int CalculateWidth(const WebTextRun&) const;
    int OffsetForPosition(const WebTextRun&, float position) const;
    gfx::RectF SelectionRectForText(const WebTextRun&, const gfx::PointF& left_baseline, int height, int from = 0, int to = -1) const;
