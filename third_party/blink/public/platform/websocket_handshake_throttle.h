...
public:
    ...
    // destroyed.
    using OnCompletion = base::OnceCallback<void(const std::optional<WebString>& error)>;
    ...
    // is null.
    virtual void ThrottleHandshake(const WebURL&, const WebSecurityOrigin& creator_origin, const WebSecurityOrigin& isolated_world_origin, OnCompletion) = 0;
};
