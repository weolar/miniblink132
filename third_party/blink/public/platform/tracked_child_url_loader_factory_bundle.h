...
class BLINK_PLATFORM_EXPORT TrackedChildPendingURLLoaderFactoryBundle : public ChildPendingURLLoaderFactoryBundle {
public:
    using HostPtrAndTaskRunner = std::pair<base::WeakPtr<HostChildURLLoaderFactoryBundle>, scoped_refptr<base::SequencedTaskRunner>>;

    TrackedChildPendingURLLoaderFactoryBundle();
    TrackedChildPendingURLLoaderFactoryBundle(mojo::PendingRemote<network::mojom::URLLoaderFactory> pending_default_factory,
    ...
    TrackedChildPendingURLLoaderFactoryBundle(const TrackedChildPendingURLLoaderFactoryBundle&) = delete;
    TrackedChildPendingURLLoaderFactoryBundle& operator=(const TrackedChildPendingURLLoaderFactoryBundle&) = delete;
...
// Service crash.
class BLINK_PLATFORM_EXPORT TrackedChildURLLoaderFactoryBundle final : public ChildURLLoaderFactoryBundle {
public:
    using HostPtrAndTaskRunner = std::pair<base::WeakPtr<HostChildURLLoaderFactoryBundle>, scoped_refptr<base::SequencedTaskRunner>>;

    // Posts a task to the host bundle on main thread to start tracking |this|.
    explicit TrackedChildURLLoaderFactoryBundle(std::unique_ptr<TrackedChildPendingURLLoaderFactoryBundle> pending_factories);
    TrackedChildURLLoaderFactoryBundle(const TrackedChildURLLoaderFactoryBundle&) = delete;
    TrackedChildURLLoaderFactoryBundle& operator=(const TrackedChildURLLoaderFactoryBundle&) = delete;
...
private:
    ...
    void OnUpdate(std::unique_ptr<network::PendingSharedURLLoaderFactory> pending_factories);

    // |WeakPtr| and |TaskRunner| of the host bundle. Can be copied and passed
    // across sequences.
    std::unique_ptr<HostPtrAndTaskRunner> main_thread_host_bundle_;
    base::WeakPtrFactory<TrackedChildURLLoaderFactoryBundle> weak_ptr_factory_ { this };
};
...
// tracking logic.
class BLINK_PLATFORM_EXPORT HostChildURLLoaderFactoryBundle final : public ChildURLLoaderFactoryBundle {
public:
    HostChildURLLoaderFactoryBundle(const HostChildURLLoaderFactoryBundle&) = delete;
    HostChildURLLoaderFactoryBundle& operator=(const HostChildURLLoaderFactoryBundle&) = delete;
    using ObserverPtrAndTaskRunner = std::pair<base::WeakPtr<TrackedChildURLLoaderFactoryBundle>, scoped_refptr<base::SequencedTaskRunner>>;
    using ObserverList = std::unordered_map<ObserverKey, std::unique_ptr<ObserverPtrAndTaskRunner>>;

    explicit HostChildURLLoaderFactoryBundle(scoped_refptr<base::SequencedTaskRunner> task_runner);
    ...
    void UpdateThisAndAllClones(std::unique_ptr<blink::PendingURLLoaderFactoryBundle> pending_factories);
...
private:
    ...
    void AddObserver(ObserverKey observer_key, std::unique_ptr<ObserverPtrAndTaskRunner> observer_info);
    ...
    void NotifyUpdateOnMainOrWorkerThread(ObserverPtrAndTaskRunner* observer_bundle, std::unique_ptr<network::PendingSharedURLLoaderFactory> pending_factories);
