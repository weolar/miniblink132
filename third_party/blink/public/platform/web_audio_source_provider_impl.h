...
class BLINK_PLATFORM_EXPORT WebAudioSourceProviderImpl : public WebAudioSourceProvider, public media::SwitchableAudioRendererSink {
public:
    using CopyAudioCB = base::RepeatingCallback<void(std::unique_ptr<media::AudioBus>, uint32_t frames_delayed, int sample_rate)>;
    ...
    WebAudioSourceProviderImpl& operator=(const WebAudioSourceProviderImpl&) = delete;

    // WebAudioSourceProvider implementation.
    void SetClient(WebAudioSourceProviderClient* client) override;
    void ProvideInput(const WebVector<float*>& audio_data, int number_of_frames) override;

    // RestartableAudioRendererSink implementation.
    void Initialize(const media::AudioParameters& params, RenderCallback* renderer) override;
    void Start() override;
    void Stop() override;
    void Play() override;
    void Pause() override;
...
private:
    ...
    scoped_refptr<media::SwitchableAudioRendererSink> sink_ GUARDED_BY(sink_lock_);
