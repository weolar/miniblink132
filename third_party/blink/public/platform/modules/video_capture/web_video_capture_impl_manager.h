...
public:
    ...
    WebVideoCaptureImplManager& operator=(const WebVideoCaptureImplManager&) = delete;
    ...
    // resources.
    base::OnceClosure UseDevice(const media::VideoCaptureSessionId& id, const BrowserInterfaceBrokerProxy& browser_interface_broker);
    ...
    void GetDeviceSupportedFormats(const media::VideoCaptureSessionId& id, VideoCaptureDeviceFormatsCB callback);

    // Get supported formats currently in use for the given session ID.
    // |callback| will be called on the IO thread.
    void GetDeviceFormatsInUse(const media::VideoCaptureSessionId& id, VideoCaptureDeviceFormatsCB callback);
    ...
    VideoCaptureFeedbackCB GetFeedbackCallback(const media::VideoCaptureSessionId& id) const;
...
private:
    ...
    static void ProcessFeedback(VideoCaptureFeedbackCB callback_to_io_thread, const media::VideoCaptureFeedback& feedback);

    void ProcessFeedbackInternal(const media::VideoCaptureSessionId& id, const media::VideoCaptureFeedback& feedback);
