...
class BLINK_PLATFORM_EXPORT WebMediaStreamAudioSink : public WebMediaStreamSink {
public:
    // Adds a MediaStreamAudioSink to the audio track to receive audio data from
    // the track.
    // Called on the main render thread.
    static void AddToAudioTrack(WebMediaStreamAudioSink* sink, const WebMediaStreamTrack& track);

    // Removes a MediaStreamAudioSink from the audio track to stop receiving
    // audio data from the track.
    // Called on the main render thread.
    static void RemoveFromAudioTrack(WebMediaStreamAudioSink* sink, const WebMediaStreamTrack& track);

    // Returns the format of the audio track.
    // Called on the main render thread.
    static media::AudioParameters GetFormatFromAudioTrack(const WebMediaStreamTrack& track);
    ...
    virtual void OnData(const media::AudioBus& audio_bus, base::TimeTicks estimated_capture_time) = 0;
    ...
    void OnData(const media::AudioBus& audio_bus, base::TimeTicks estimated_capture_time, const media::AudioGlitchInfo& glitch_info)
    {
    ...
    // unknown.
    virtual int NumPreferredChannels()
    {
        return -1;
    }
