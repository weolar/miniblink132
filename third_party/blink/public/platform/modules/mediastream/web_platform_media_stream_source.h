...
public:
    using SourceStoppedCallback = base::OnceCallback<void(const WebMediaStreamSource& source)>;

    using ConstraintsRepeatingCallback
        = base::RepeatingCallback<void(WebPlatformMediaStreamSource* source, mojom::MediaStreamRequestResult result, const WebString& result_name)>;
    using ConstraintsOnceCallback
        = base::OnceCallback<void(WebPlatformMediaStreamSource* source, mojom::MediaStreamRequestResult result, const WebString& result_name)>;

    // Source constraints key for
    // https://dev.w3.org/2011/webrtc/editor/getusermedia.html.
    static const char kSourceId[];

    explicit WebPlatformMediaStreamSource(scoped_refptr<base::SingleThreadTaskRunner>);
    WebPlatformMediaStreamSource(const WebPlatformMediaStreamSource&) = delete;
    WebPlatformMediaStreamSource& operator=(const WebPlatformMediaStreamSource&) = delete;
...
private:
    ...
    WebPrivatePtrForGC<MediaStreamSource, WebPrivatePtrDestruction::kSameThread, WebPrivatePtrStrength::kWeak> owner_;
