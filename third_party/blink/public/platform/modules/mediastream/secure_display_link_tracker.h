...
template <typename T> class SecureDisplayLinkTracker {
public:
    ...
    bool is_capturing_secure() const
    {
        return insecure_links_.empty();
    }
...
template <typename T> void SecureDisplayLinkTracker<T>::Add(T* link, bool is_link_secure)
{
...
template <typename T> void SecureDisplayLinkTracker<T>::Remove(T* link)
{
...
template <typename T> void SecureDisplayLinkTracker<T>::Update(T* link, bool is_link_secure)
{
    auto it = insecure_links_.Find(link);
    if (it != kNotFound) {
        if (is_link_secure)
