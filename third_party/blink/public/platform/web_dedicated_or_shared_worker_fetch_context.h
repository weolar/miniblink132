...
class BLINK_PLATFORM_EXPORT WebDedicatedOrSharedWorkerFetchContext : public WebWorkerFetchContext {
public:
    ...
    // chrome-extension://).
    static scoped_refptr<WebDedicatedOrSharedWorkerFetchContext> Create(WebServiceWorkerProviderContext* provider_context,
        ...
        const WebVector<WebString>& cors_exempt_header_list, mojo::PendingRemote<mojom::ResourceLoadInfoNotifier> pending_resource_load_info_notifier);

    // Clones this fetch context for a nested worker.
    // For non-PlzDedicatedWorker. This will be removed once PlzDedicatedWorker is
    // enabled by default.
    virtual scoped_refptr<WebDedicatedOrSharedWorkerFetchContext> CloneForNestedWorkerDeprecated(scoped_refptr<base::SingleThreadTaskRunner> task_runner) = 0;
    ...
    virtual scoped_refptr<WebDedicatedOrSharedWorkerFetchContext> CloneForNestedWorker(WebServiceWorkerProviderContext* service_worker_provider_context,
        std::unique_ptr<network::PendingSharedURLLoaderFactory> pending_loader_factory,
        std::unique_ptr<network::PendingSharedURLLoaderFactory> pending_fallback_factory,
        CrossVariantMojoReceiver<mojom::SubresourceLoaderUpdaterInterfaceBase> pending_subresource_loader_updater,
    ...
    virtual void set_site_for_cookies(const net::SiteForCookies& site_for_cookies) = 0;
    virtual void set_top_frame_origin(const blink::WebSecurityOrigin& top_frame_origin) = 0;
