...
public:
    explicit ResourceLoadInfoNotifierWrapper(base::WeakPtr<WeakWrapperResourceLoadInfoNotifier> weak_wrapper_resource_load_info_notifier);
...
#endif
    void NotifyResourceLoadInitiated(int64_t request_id, const GURL& request_url, const std::string& http_method, const GURL& referrer,
        network::mojom::RequestDestination request_destination, net::RequestPriority request_priority, bool is_ad_resource);
    void NotifyResourceRedirectReceived(const net::RedirectInfo& redirect_info, network::mojom::URLResponseHeadPtr redirect_response);
    void NotifyResourceResponseReceived(network::mojom::URLResponseHeadPtr response_head);
    void NotifyResourceTransferSizeUpdated(int32_t transfer_size_diff);
    void NotifyResourceLoadCompleted(const network::URLLoaderCompletionStatus& status);
...
private:
    ...
    base::WeakPtr<WeakWrapperResourceLoadInfoNotifier> weak_wrapper_resource_load_info_notifier_;
