...
public:
    ...
    virtual std::vector<std::string> run(base::span<const std::string> available_values, std::optional<std::string> request_header_value) = 0;
...
protected:
    ...
    // with weight of 0.
    std::vector<WeightedValue> ParseRequestHeaderValue(const std::optional<std::string>& request_header_value)
    {
        ...
        while (values.GetNext()) {
            net::HttpUtil::NameValuePairsIterator name_value_pairs(values.value(), /*delimiter=*/';',
                net::HttpUtil::NameValuePairsIterator::Values::NOT_REQUIRED, net::HttpUtil::NameValuePairsIterator::Quotes::STRICT_QUOTES);
            ...
            while (name_value_pairs.GetNext()) {
                if (base::EqualsCaseInsensitiveASCII(name_value_pairs.name(), "q")) {
                    ...
                    base::StrAppend(&item.value, { ";", name_value_pairs.name(), "=", name_value_pairs.value() });
                }
            }
            if (item.weight != 0.0)
...
class ContentTypeNegotiation final : public ContentNegotiationAlgorithm {
    std::vector<std::string> run(base::span<const std::string> available_values, std::optional<std::string> request_header_value) override
    {
        ...
        std::vector<WeightedValue> preferred_types = ParseRequestHeaderValue(request_header_value);
...
class AcceptEncodingNegotiation final : public ContentNegotiationAlgorithm {
    std::vector<std::string> run(base::span<const std::string> available_values, std::optional<std::string> request_header_value) override
    {
        ...
        std::vector<WeightedValue> preferred_codings = ParseRequestHeaderValue(request_header_value);
        ...
        for (const WeightedValue& preferred_coding : preferred_codings) {
            ...
            for (const std::string& available : available_values) {
                if (base::EqualsCaseInsensitiveASCII(preferred_coding.value, available)) {
                    preferred_available.push_back(available);
                    break;
                }
            }
...
public:
    std::vector<std::string> run(base::span<const std::string> available_values, std::optional<std::string> request_header_value) override
    {
        ...
        std::vector<WeightedValue> preferred_langs = ParseRequestHeaderValue(request_header_value);
        ...
        for (const WeightedValue& preferred_lang : preferred_langs) {
            ...
            AppendMatchedLanguages(available_values, preferred_lang.value, &preferred_available);
        }
...
private:
    ...
    void AppendMatchedLanguages(base::span<const std::string> available_values, const std::string& preferred_lang, std::vector<std::string>* output)
    {
        if (preferred_lang == "*") {
        ...
        for (const std::string& available : available_values) {
            if (base::EqualsCaseInsensitiveASCII(preferred_lang, available) || base::StartsWith(available, prefix, base::CompareCase::INSENSITIVE_ASCII)) {
...
std::unique_ptr<ContentNegotiationAlgorithm> GetContentNegotiationAlgorithm(const std::string& field_name)
{
...
std::optional<std::vector<std::pair<std::string, std::vector<std::string>>>> ParseVariants(const std::string_view& str)
{
    ...
    std::optional<net::structured_headers::ListOfLists> parsed = net::structured_headers::ParseListOfLists(str);
    if (!parsed)
        return std::nullopt;
...
std::optional<std::vector<std::vector<std::string>>> ParseVariantKey(const std::string_view& str, size_t num_variant_axes)
{
    // Compatibility note: Draft 4 of Variants
    // (https://tools.ietf.org/id/draft-ietf-httpbis-variants-04.html#variant-key)
...
std::optional<size_t> GetPossibleKeysIndex(const std::vector<std::vector<std::string>>& sorted_variants, const std::vector<std::string>& variant_key)
{
    ...
    for (size_t i = 0; i < sorted_variants.size(); ++i) {
        ...
        index = index * sorted_variants[i].size() + (found - sorted_variants[i].begin());
    }
    return index;
}
...
WebPackageRequestMatcher::WebPackageRequestMatcher(const net::HttpRequestHeaders& request_headers, const std::string& accept_langs)
    : request_headers_(request_headers)
{
    request_headers_.SetHeaderIfMissing(
        net::HttpRequestHeaders::kAcceptLanguage, net::HttpUtil::GenerateAcceptLanguageHeader(net::HttpUtil::ExpandLanguageList(accept_langs)));
    ...
    request_headers_.SetHeader(net::HttpRequestHeaders::kAcceptEncoding, "mi-sha256-03");
}

bool WebPackageRequestMatcher::MatchRequest(const HeaderMap& response_headers) const
{
    return MatchRequest(request_headers_, response_headers);
}

std::vector<std::string>::const_iterator WebPackageRequestMatcher::FindBestMatchingVariantKey(
    const std::string& variants, const std::vector<std::string>& variant_key_list) const
{
    return FindBestMatchingVariantKey(request_headers_, variants, variant_key_list);
}

std::optional<size_t> WebPackageRequestMatcher::FindBestMatchingIndex(const std::string& variants) const
{
...
std::vector<std::vector<std::string>> WebPackageRequestMatcher::CacheBehavior(
    const std::vector<std::pair<std::string, std::vector<std::string>>>& variants, const net::HttpRequestHeaders& request_headers)
{
    ...
    for (const auto& variant_axis : variants) {
        ...
        std::unique_ptr<ContentNegotiationAlgorithm> negotiation_algorithm = GetContentNegotiationAlgorithm(field_name);
        if (negotiation_algorithm) {
            ...
            std::optional<std::string> request_value = request_headers.GetHeader(field_name);
            ...
            std::vector<std::string> sorted_values = negotiation_algorithm->run(variant_axis.second, request_value);
...
bool WebPackageRequestMatcher::MatchRequest(const net::HttpRequestHeaders& request_headers, const HeaderMap& response_headers)
{
    auto variants_found = response_headers.find(blink::kSignedExchangeVariantsHeader);
    auto variant_key_found = response_headers.find(blink::kSignedExchangeVariantKeyHeader);
    ...
    if (variants_found == response_headers.end() && variant_key_found == response_headers.end()) {
        return true;
    }
    ...
    // [spec text]
    std::vector<std::vector<std::string>> sorted_variants = CacheBehavior(*parsed_variants, request_headers);
    ...
    auto parsed_variant_key = ParseVariantKey(variant_key_found->second, parsed_variants->size());
...
// static
std::vector<std::string>::const_iterator WebPackageRequestMatcher::FindBestMatchingVariantKey(
    const net::HttpRequestHeaders& request_headers, const std::string& variants, const std::vector<std::string>& variant_keys_list)
{
    ...
    for (const auto& key : sorted_variants) {
        if (!base::CheckMul(possible_keys_count, key.size()).AssignIfValid(&possible_keys_count)) {
            return variant_keys_list.end();
        }
    }
    ...
    for (auto variant_keys_list_it = variant_keys_list.begin(); variant_keys_list_it < variant_keys_list.end(); ++variant_keys_list_it) {
        auto parsed_variant_keys = ParseVariantKey(*variant_keys_list_it, parsed_variants->size());
...
// static
std::optional<size_t> WebPackageRequestMatcher::FindBestMatchingIndex(const net::HttpRequestHeaders& request_headers, const std::string& variants)
{
    auto parsed_variants = ParseVariants(variants);
    if (!parsed_variants)
        return std::nullopt;
    ...
    for (const auto& variant_axis : *parsed_variants) {
        ...
        if (!base::CheckMul(best_match_index, variant_axis.second.size()).AssignIfValid(&best_match_index)) {
            return std::nullopt;
        }
        if (!base::CheckAdd(best_match_index, best_value_index).AssignIfValid(&best_match_index)) {
            return std::nullopt;
        }
    }
