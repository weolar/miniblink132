...
constexpr auto kSupportedImageTypes = base::MakeFixedFlatSet<std::string_view>({
    "image/jpeg", "image/pjpeg", "image/jpg", "image/webp", "image/png", "image/apng", "image/gif", "image/bmp",
...
constexpr auto kSupportedJavascriptTypes = base::MakeFixedFlatSet<std::string_view>({
...
constexpr auto kUnsupportedTextTypes = base::MakeFixedFlatSet<std::string_view>({
    "text/calendar", "text/x-calendar", "text/x-vcalendar", "text/vcalendar", "text/vcard", "text/x-vcard", "text/directory", "text/ldif", "text/qif",
    "text/x-qif", "text/x-csv", "text/x-vcf", "text/rtf", "text/comma-separated-values", "text/csv", "text/tab-separated-values", "text/tsv",
...
constexpr auto kSupportedNonImageTypes = base::MakeFixedFlatSet<std::string_view>({
    "image/svg+xml", // SVG is text-based XML, even though it has an image/
    // type
    "application/xml", "application/atom+xml", "application/rss+xml", "application/xhtml+xml", "application/json",
...
bool IsSupportedNonImageMimeType(std::string_view mime_type)
{
    ...
    return kSupportedNonImageTypes.contains(mime_lower) || kSupportedJavascriptTypes.contains(mime_lower) ||
#if !BUILDFLAG(IS_IOS)
        media::IsSupportedMediaMimeType(mime_lower) ||
#endif
        (mime_lower.starts_with("text/") && !kUnsupportedTextTypes.contains(mime_lower))
        || (mime_lower.starts_with("application/") && net::MatchesMimeType("application/*+json", mime_lower));
}
...
{
    return net::MatchesMimeType("application/json", mime_type) || net::MatchesMimeType("text/json", mime_type)
...
{
    return net::MatchesMimeType("text/xml", mime_type) || net::MatchesMimeType("application/xml", mime_type)
...
{
    return (base::StartsWith(mime_type, "image/", base::CompareCase::INSENSITIVE_ASCII) && IsSupportedImageMimeType(mime_type))
