...
bool IsDefaultManifest(const mojom::Manifest& manifest, const GURL& document_url)
{
...
bool IsDefaultManifest(const mojom::ManifestPtr& manifest, const GURL& document_url)
{
...
{
    if (display == blink::mojom::DisplayMode::kBrowser || display == blink::mojom::DisplayMode::kMinimalUi || display == blink::mojom::DisplayMode::kStandalone
...
mojom::CaptureLinks CaptureLinksFromString(const std::string& capture_links)
{
    ...
    if (base::EqualsCaseInsensitiveASCII(capture_links, "existing-client-navigate"))
