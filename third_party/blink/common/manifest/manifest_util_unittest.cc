...
TEST(ManifestUtilTest, DisplayModeConversions)
{
    struct ReversibleConversion {
        ...
        { blink::mojom::DisplayMode::kWindowControlsOverlay, "window-controls-overlay" },
    ...
    for (const ReversibleConversion& conversion : reversible_conversions) {
        EXPECT_EQ(conversion.display_mode, DisplayModeFromString(conversion.lowercase_display_mode_string));
        EXPECT_EQ(conversion.lowercase_display_mode_string, DisplayModeToString(conversion.display_mode));
    }

    // DisplayModeFromString() should work with non-lowercase strings.
    EXPECT_EQ(blink::mojom::DisplayMode::kFullscreen, DisplayModeFromString("Fullscreen"));

    // DisplayModeFromString() should return
    // DisplayMode::kUndefined if the string isn't known.
    EXPECT_EQ(blink::mojom::DisplayMode::kUndefined, DisplayModeFromString("random"));
}
...
TEST(ManifestUtilTest, WebScreenOrientationLockTypeConversions)
{
    struct ReversibleConversion {
        ...
        { device::mojom::ScreenOrientationLockType::PORTRAIT_PRIMARY, "portrait-primary" },
        { device::mojom::ScreenOrientationLockType::PORTRAIT_SECONDARY, "portrait-secondary" },
        { device::mojom::ScreenOrientationLockType::LANDSCAPE_PRIMARY, "landscape-primary" },
        { device::mojom::ScreenOrientationLockType::LANDSCAPE_SECONDARY, "landscape-secondary" },
    ...
    for (const ReversibleConversion& conversion : reversible_conversions) {
        EXPECT_EQ(conversion.orientation, WebScreenOrientationLockTypeFromString(conversion.lowercase_orientation_string));
        EXPECT_EQ(conversion.lowercase_orientation_string, WebScreenOrientationLockTypeToString(conversion.orientation));
    }

    // WebScreenOrientationLockTypeFromString() should work with non-lowercase
    // strings.
    EXPECT_EQ(device::mojom::ScreenOrientationLockType::NATURAL, WebScreenOrientationLockTypeFromString("Natural"));

    // WebScreenOrientationLockTypeFromString() should return
    // blink::WebScreenOrientationLockDefault if the string isn't known.
    EXPECT_EQ(device::mojom::ScreenOrientationLockType::DEFAULT, WebScreenOrientationLockTypeFromString("random"));
}
...
TEST(ManifestUtilTest, CaptureLinksFromString)
{
    ...
    EXPECT_EQ(blink::mojom::CaptureLinks::kNewClient, CaptureLinksFromString("new-client"));
    EXPECT_EQ(blink::mojom::CaptureLinks::kExistingClientNavigate, CaptureLinksFromString("existing-client-navigate"));
    ...
    // string isn't known.
    EXPECT_EQ(blink::mojom::CaptureLinks::kUndefined, CaptureLinksFromString("unknown-value"));
}
...
TEST(ManifestUtilTest, LaunchHandlerClientModeFromString)
{
    ...
    EXPECT_EQ(ClientMode::kNavigateExisting, ClientModeFromString("navigate-existing"));
