...
InterProcessTimeTicksConverter::InterProcessTimeTicksConverter(
    LocalTimeTicks local_lower_bound, LocalTimeTicks local_upper_bound, RemoteTimeTicks remote_lower_bound, RemoteTimeTicks remote_upper_bound)
    ...
    , remote_upper_bound_(remote_upper_bound)
{
    ...
    if (remote_range.ToTimeDelta() <= local_range_.ToTimeDelta()) {
        ...
        base::TimeDelta diff = local_range_.ToTimeDelta() - remote_range.ToTimeDelta();

        local_base_time_ = local_lower_bound + LocalTimeDelta::FromTimeDelta(diff / 2);
    ...
    range_conversion_rate_ = local_range_.ToTimeDelta() / remote_range.ToTimeDelta();
    local_base_time_ = local_lower_bound;
}

LocalTimeTicks InterProcessTimeTicksConverter::ToLocalTimeTicks(RemoteTimeTicks remote_time_ticks) const
{
...
LocalTimeDelta InterProcessTimeTicksConverter::ToLocalTimeDelta(RemoteTimeDelta remote_delta) const
{
    ...
    return std::min(local_range_, LocalTimeDelta::FromTimeDelta(remote_delta.ToTimeDelta() * range_conversion_rate_));
}
