...
public:
    ...
    void Start(std::string data, base::OnceClosure done_callback)
    {
        ...
        watcher_.Watch(handle_.get(), MOJO_HANDLE_SIGNAL_WRITABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED,
            base::BindRepeating(&MojoDataPipeSender::OnWritable, base::Unretained(this)));
    }
    ...
    void OnWritable(MojoResult)
    {
        ...
        MojoResult result = handle_->WriteData(bytes, MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes);
        switch (result) {
        case MOJO_RESULT_OK:
            break;
...
public:
    ...
    void Resume() override
    {
        ...
        destination_loader_client()->OnReceiveResponse(std::move(updated_response_head_), std::move(body_), std::nullopt);
    }
        ...
        mojo::PendingReceiver<network::mojom::URLLoaderClient>* original_client_receiver, mojo::ScopedDataPipeConsumerHandle* body) override
    {
        ...
        ASSERT_TRUE(
            mojo::FusePipes(std::move(new_client_receiver), mojo::PendingRemote<network::mojom::URLLoaderClient>(destination_loader_client_.CreateRemote())));
        pending_receiver_ = original_loader->InitWithNewPipeAndPassReceiver();

        *original_client_receiver = source_loader_client_remote_.BindNewPipeAndPassReceiver();
        ...
        EXPECT_EQ(MOJO_RESULT_OK, mojo::CreateDataPipe(nullptr, source_body_handle_, consumer));
    ...
    {
        source_loader_client_remote()->OnComplete(network::URLLoaderCompletionStatus());
    ...
    uint32_t ReadResponseBody(size_t size)
    {
        ...
        MojoResult result = destination_loader_client_.response_body().ReadData(MOJO_READ_DATA_FLAG_NONE, buffer, size);
        switch (result) {
        case MOJO_RESULT_OK:
            return size;
        case MOJO_RESULT_FAILED_PRECONDITION:
...
{
    auto throttle = std::make_unique<MimeSniffingThrottle>(task_environment_.GetMainThreadTaskRunner());
    auto delegate = std::make_unique<MockDelegate>();
    throttle->set_delegate(delegate.get());

    auto response_head = network::mojom::URLResponseHead::New();
    bool defer = false;
    throttle->WillProcessResponse(GURL("https://example.com"), response_head.get(), &defer);
    EXPECT_TRUE(defer);
...
TEST_F(MimeSniffingThrottleTest, NoMimeTypeWithNotSniffableScheme)
{
    ...
    auto response_head = network::mojom::URLResponseHead::New();
    bool defer = false;
    throttle->WillProcessResponse(GURL("wss://example.com"), response_head.get(), &defer);
    EXPECT_FALSE(defer);
    EXPECT_FALSE(delegate->is_intercepted());
}
...
TEST_F(MimeSniffingThrottleTest, NoBody)
{
    ...
    delegate->source_loader_client_remote()->OnComplete(network::URLLoaderCompletionStatus(net::ERR_FAILED));
    ...
    EXPECT_EQ("text/plain", delegate->destination_loader_client()->response_head()->mime_type);
}
...
TEST_F(MimeSniffingThrottleTest, EmptyBody)
{
    ...
    delegate->source_loader_client_remote()->OnComplete(network::URLLoaderCompletionStatus());
    delegate->destination_loader_client()->RunUntilComplete();
...
TEST_F(MimeSniffingThrottleTest, Body_Docx)
{
    ...
    // The mime type should be updated.
    EXPECT_TRUE(delegate->is_resumed());
    EXPECT_EQ("application/msword", delegate->destination_loader_client()->response_head()->mime_type);
}
...
TEST_F(MimeSniffingThrottleTest, Body_PNG)
{
    ...
    // The mime type should be updated.
    EXPECT_TRUE(delegate->is_resumed());
    EXPECT_EQ("image/png", delegate->destination_loader_client()->response_head()->mime_type);
}
...
TEST_F(MimeSniffingThrottleTest, Body_LongPlainText)
{
    ...
    EXPECT_FALSE(delegate->destination_loader_client()->has_received_completion());
