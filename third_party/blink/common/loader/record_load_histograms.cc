...
void RecordLoadHistograms(const url::Origin& origin, network::mojom::RequestDestination destination, int net_error)
{
    ...
    if (destination == network::mojom::RequestDestination::kDocument) {
        ...
        if (GURL::SchemeIsCryptographic(origin.scheme()) && origin.host() == "www.google.com") {
            base::UmaHistogramSparse("Net.ErrorCodesForHTTPSGoogleMainFrame3", -net_error);
        }
    } else {
