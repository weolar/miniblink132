...
net::HttpResponseHeaders* net::URLRequest::response_headers(void) const
{
    *(int*)1 = 1;
    return nullptr;
}
...
{
    static base::NoDestructor<base::RepeatingCallback<OriginTrialPolicy*()>> policy(base::BindRepeating([]() -> OriginTrialPolicy* { return nullptr; }));
    return *policy;
}
...
OriginTrialTokenStatus IsTokenValid(
    const TrialToken& trial_token, const url::Origin& origin, base::span<const url::Origin> third_party_origins, base::Time current_time)
{
...
bool IsTokenExpired(std::string_view trial_name, const base::Time token_expiry_time, const base::Time current_time)
{
    // Check token expiry.
    ...
    if (token_expired) {
        if (origin_trials::IsTrialValid(trial_name)) {
            ...
            for (mojom::OriginTrialFeature feature : origin_trials::FeaturesForTrial(trial_name)) {
...
OriginTrialTokenStatus ValidateTokenEnabled(const OriginTrialPolicy& policy, std::string_view trial_name, const base::Time token_expiry_time,
    const TrialToken::UsageRestriction usage_restriction, std::string_view token_signature, const base::Time current_time)
{
    ...
    if (usage_restriction == TrialToken::UsageRestriction::kSubset && policy.IsFeatureDisabledForUser(trial_name)) {
...
TrialTokenValidator::OriginInfo::OriginInfo(const url::Origin& wrapped_origin, bool origin_is_secure)
    : origin(wrapped_origin)
    , is_secure(origin_is_secure)
{
}
...
void TrialTokenValidator::SetOriginTrialPolicyGetter(base::RepeatingCallback<OriginTrialPolicy*()> policy_getter)
{
...
{
    SetOriginTrialPolicyGetter(base::BindRepeating([]() -> OriginTrialPolicy* { return nullptr; }));
}

TrialTokenResult TrialTokenValidator::ValidateTokenAndTrial(std::string_view token, const url::Origin& origin, base::Time current_time) const
{
    return ValidateTokenAndTrialWithOriginInfo(token, OriginInfo(origin), base::span<const OriginInfo> {}, current_time);
}

TrialTokenResult TrialTokenValidator::ValidateTokenAndTrial(
    std::string_view token, const url::Origin& origin, base::span<const url::Origin> third_party_origins, base::Time current_time) const
{
    ...
    base::span<const OriginInfo> third_party_origin_info_span(third_party_origin_info.data(), third_party_origin_info.size());
    return ValidateTokenAndTrialWithOriginInfo(token, OriginInfo(origin), third_party_origin_info_span, current_time);
}

TrialTokenResult TrialTokenValidator::ValidateTokenAndTrialWithOriginInfo(
    std::string_view token, const OriginInfo& origin, base::span<const OriginInfo> third_party_origin_info, base::Time current_time) const
{
    ...
    base::span<const url::Origin> third_party_origins_span(third_party_origins.data(), third_party_origins.size());
    TrialTokenResult token_result = ValidateToken(token, origin.origin, third_party_origins_span, current_time);
    ...
    if (parsed_token.is_third_party() && !origin_trials::IsTrialEnabledForThirdPartyOrigins(parsed_token.feature_name())) {
    ...
    // skipped.
    if (origin_trials::IsTrialEnabledForInsecureContext(parsed_token.feature_name())) {
        return token_result;
    }
    ...
    if (parsed_token.is_third_party()) {
        ...
        if (third_party_origin_info.size() == 1) {
            ...
            for (const OriginInfo& script_origin_info : third_party_origin_info) {
                ...
                if (parsed_token.IsValid(script_origin_info.origin, current_time) == OriginTrialTokenStatus::kSuccess) {
...
TrialTokenResult TrialTokenValidator::ValidateToken(std::string_view token, const url::Origin& origin, base::Time current_time) const
{
    return ValidateToken(token, origin, base::span<const url::Origin> {}, current_time);
}
    ...
    std::string_view token, const url::Origin& origin, base::span<const url::Origin> third_party_origins, base::Time current_time) const
{
    ...
    status = IsTokenValid(*trial_token, origin, third_party_origins, current_time);

    if (status == OriginTrialTokenStatus::kSuccess || status == OriginTrialTokenStatus::kExpired) {
        // Since manual completion trials have a grace period, we need to check
        // expired tokens in addition to valid tokens.
        status = ValidateTokenEnabled(
            *policy, trial_token->feature_name(), trial_token->expiry_time(), trial_token->usage_restriction(), trial_token->signature(), current_time);
    }
    return TrialTokenResult(status, std::move(trial_token));
}

bool TrialTokenValidator::RevalidateTokenAndTrial(std::string_view trial_name, const base::Time token_expiry_time,
    const TrialToken::UsageRestriction usage_restriction, std::string_view token_signature, const base::Time current_time) const
{
    OriginTrialPolicy* policy = PolicyGetter().Run();
    ...
    OriginTrialTokenStatus status = ValidateTokenEnabled(*policy, trial_name, token_expiry_time, usage_restriction, token_signature, current_time);
    return status == OriginTrialTokenStatus::kSuccess;
}

std::vector<mojom::OriginTrialFeature> TrialTokenValidator::FeaturesEnabledByTrial(std::string_view trial_name)
{
    std::vector<mojom::OriginTrialFeature> enabled_features;
    base::span<const mojom::OriginTrialFeature> features = origin_trials::FeaturesForTrial(trial_name);
    for (const mojom::OriginTrialFeature feature : features) {
        if (origin_trials::FeatureEnabledForOS(feature)) {
            ...
            for (const mojom::OriginTrialFeature implied_feature : origin_trials::GetImpliedFeatures(feature)) {
...
bool TrialTokenValidator::TrialEnablesFeaturesForOS(std::string_view trial_name)
{
    return !FeaturesEnabledByTrial(trial_name).empty();
}

bool TrialTokenValidator::RequestEnablesFeature(const net::URLRequest* request, std::string_view feature_name, base::Time current_time) const
{
...
bool TrialTokenValidator::RequestEnablesFeature(
    const GURL& request_url, const net::HttpResponseHeaders* response_headers, std::string_view feature_name, base::Time current_time) const
{
    return IsTrialPossibleOnOrigin(request_url) && ResponseBearsValidTokenForFeature(request_url, *response_headers, feature_name, current_time);
}

bool TrialTokenValidator::RequestEnablesDeprecatedFeature(
    const GURL& request_url, const net::HttpResponseHeaders* response_headers, std::string_view feature_name, base::Time current_time) const
{
    return IsDeprecationTrialPossible() && ResponseBearsValidTokenForFeature(request_url, *response_headers, feature_name, current_time);
}
    ...
    const GURL& request_url, const net::HttpResponseHeaders& response_headers, std::string_view feature_name, base::Time current_time) const
{
    ...
    while (response_headers.EnumerateHeader(&iter, "Origin-Trial", &token)) {
        TrialTokenResult result = ValidateTokenAndTrial(token, origin, current_time);
...
std::unique_ptr<TrialTokenValidator::FeatureToTokensMap> TrialTokenValidator::GetValidTokensFromHeaders(
    const url::Origin& origin, const net::HttpResponseHeaders* headers, base::Time current_time) const
{
    std::unique_ptr<FeatureToTokensMap> tokens(std::make_unique<FeatureToTokensMap>());
...
std::unique_ptr<TrialTokenValidator::FeatureToTokensMap> TrialTokenValidator::GetValidTokens(
    const url::Origin& origin, const FeatureToTokensMap& tokens, base::Time current_time) const
{
    std::unique_ptr<FeatureToTokensMap> out_tokens(std::make_unique<FeatureToTokensMap>());
    if (!IsTrialPossibleOnOrigin(origin.GetURL()))
        return out_tokens;
...
{
    OriginTrialPolicy* policy = PolicyGetter().Run();
    return policy && policy->IsOriginTrialsSupported() && policy->IsOriginSecure(url);
}
