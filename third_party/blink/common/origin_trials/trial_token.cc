...
// static
std::unique_ptr<TrialToken> TrialToken::From(std::string_view token_text, const OriginTrialPublicKey& public_key, OriginTrialTokenStatus* out_status)
{
    DCHECK(out_status);
    std::string token_payload;
    std::string token_signature;
    uint8_t token_version;
    *out_status = Extract(token_text, public_key, &token_payload, &token_signature, &token_version);
    ...
    if (token) {
        ...
        *out_status = OriginTrialTokenStatus::kSuccess;
        DVLOG(2) << "Well-formed origin trial token found for feature " << token->feature_name();
    } else {
...
OriginTrialTokenStatus TrialToken::IsValid(const url::Origin& origin, const base::Time& now) const
{
...
// static
OriginTrialTokenStatus TrialToken::Extract(std::string_view token_text, const OriginTrialPublicKey& public_key, std::string* out_token_payload,
    std::string* out_token_signature, uint8_t* out_token_version)
{
    if (token_text.empty()) {
    ...
    uint32_t payload_length = base::U32FromBigEndian(token_bytes.subspan(kPayloadLengthOffset).first<4>());
    ...
    std::string_view version_piece(base::as_string_view(token_bytes.subspan(kVersionOffset, kVersionSize)));
    std::string_view signature(base::as_string_view(token_bytes.subspan(kSignatureOffset, kSignatureSize)));
    std::string_view payload_piece(base::as_string_view(token_bytes.subspan(kPayloadLengthOffset, kPayloadLengthSize + payload_length)));
...
// static
std::unique_ptr<TrialToken> TrialToken::Parse(const std::string& token_payload, const uint8_t version)
{
    ...
    return base::WrapUnique(
        new TrialToken(origin, is_subdomain, *feature_name, base::Time::FromSecondsSinceUnixEpoch(expiry_timestamp), is_third_party, usage));
}
...
bool TrialToken::ValidateOrigin(const url::Origin& origin) const
{
    ...
    if (is_third_party_ && origin_trials::IsTrialPersistentToNextResponse(feature_name_)) {
        return true;
    }
...
// static
bool TrialToken::ValidateSignature(std::string_view signature, const std::string& data, const OriginTrialPublicKey& public_key)
{
...
//     int result = ED25519_verify(reinterpret_cast<const uint8_t*>(data.data()), data.length(), reinterpret_cast<const uint8_t*>(signature.data()), public_key.data());
//     return (result != 0);
    *(int*)1 = 1;
    return false;
}

TrialToken::TrialToken(const url::Origin& origin, bool match_subdomains, const std::string& feature_name, base::Time expiry_time, bool is_third_party,
...
// static
std::unique_ptr<TrialToken> TrialToken::CreateTrialTokenForTesting(const url::Origin& origin, bool match_subdomains, const std::string& feature_name,
    base::Time expiry_time, bool is_third_party, UsageRestriction usage_restriction, const std::string& signature)
{
    std::unique_ptr<TrialToken> token
        = base::WrapUnique(new TrialToken(origin, match_subdomains, feature_name, expiry_time, is_third_party, usage_restriction));
