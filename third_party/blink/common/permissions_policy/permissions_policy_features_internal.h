...
BLINK_COMMON_EXPORT const std::unordered_set<std::string> UnloadDeprecationAllowedHosts();

// Returns `true` if `hosts` is empty or contains `host`.
// Exported for testing.
BLINK_COMMON_EXPORT bool UnloadDeprecationAllowedForHost(const std::string& host, const std::unordered_set<std::string>& hosts);
...
BLINK_COMMON_EXPORT bool UnloadDeprecationAllowedForOrigin(const url::Origin& origin);
