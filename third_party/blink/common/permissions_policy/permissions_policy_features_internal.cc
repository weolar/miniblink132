...
{
    auto hosts = base::SplitString(features::kDeprecateUnloadAllowlist.Get(), ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
// buckets.
bool IsIncludedInGradualRollout(const std::string& host, int percent, int bucket)
{
    if (percent == 100) {
        return true;
    }
    ...
    const uint8_t hash[2] = { static_cast<uint8_t>(base::PersistentHash(host)), static_cast<uint8_t>(bucket) };
    const int hash_bucket = base::PersistentHash(hash) % 100;
    return hash_bucket < percent;
}

bool UnloadDeprecationAllowedForHost(const std::string& host, const HostSet& hosts)
{
    if (hosts.empty()) {
        return true;
    }
...
bool UnloadDeprecationAllowedForOrigin(const url::Origin& origin)
{
    ...
    if (base::FeatureList::IsEnabled(features::kDeprecateUnloadByAllowList)) {
        static const base::NoDestructor<HostSet> hosts(UnloadDeprecationAllowedHosts());
        if (!UnloadDeprecationAllowedForHost(shp.host(), *hosts)) {
            return false;
        }
    }

    return IsIncludedInGradualRollout(shp.host(), features::kDeprecateUnloadPercent.Get(), features::kDeprecateUnloadBucket.Get());
}
