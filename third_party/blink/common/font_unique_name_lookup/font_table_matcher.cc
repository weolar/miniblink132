...
FontTableMatcher::FontTableMatcher(const base::ReadOnlySharedMemoryMapping& mapping)
{
...
// static
base::ReadOnlySharedMemoryMapping FontTableMatcher::MemoryMappingFromFontUniqueNameTable(const FontUniqueNameTable& font_unique_name_table)
{
    size_t serialization_size = font_unique_name_table.ByteSizeLong();
    CHECK(serialization_size);
    base::MappedReadOnlyRegion mapped_region = base::ReadOnlySharedMemoryRegion::Create(serialization_size);
...
std::optional<FontTableMatcher::MatchResult> FontTableMatcher::MatchName(const std::string& name_request) const
{
    ...
    auto find_result = std::lower_bound(
        name_map.begin(), name_map.end(), folded_name_request, [](const blink::FontUniqueNameTable_UniqueNameToFontMapping& a, const std::string& b) {
    ...
    if (find_result == name_map.end() || find_result->font_name() != folded_name_request
    ...
    if (found_font.file_path().empty())
        return {};
    return std::optional<MatchResult>({ found_font.file_path(), found_font.ttc_index() });
}
...
bool FontTableMatcher::FontListIsDisjointFrom(const FontTableMatcher& other) const
{
    ...
    std::set_intersection(paths_self.begin(), paths_self.end(), paths_other.begin(), paths_other.end(), std::back_inserter(intersection_result));
    return intersection_result.empty();
}

void FontTableMatcher::SortUniqueNameTableForSearch(FontUniqueNameTable* font_table)
{
    std::sort(font_table->mutable_name_map()->begin(), font_table->mutable_name_map()->end(),
        [](const auto& a, const auto& b) { return a.font_name() < b.font_name(); });
}
