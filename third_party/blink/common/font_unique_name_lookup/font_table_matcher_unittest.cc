...
void PopulateFontUniqueNameEntry(
    blink::FontUniqueNameTable* font_unique_name_table, const std::string& path, int32_t ttc_index, const std::set<std::string>& names)
{
...
protected:
    void SetUp() override
    {
        ...
        font_unique_name_table.set_stored_for_platform_version_identifier(kDummyAndroidBuildFingerPrint);
        PopulateFontUniqueNameEntry(&font_unique_name_table, kTestFilePath1, 0,
            { "FONT NAME UPPERCASE", "எழுத்துரு பெயர்", "字體名稱", "FONT-NAME-UPPERCASE", "எழுத்துரு-பெயர்", "字體名稱" });
        base::ReadOnlySharedMemoryMapping mapping = FontTableMatcher::MemoryMappingFromFontUniqueNameTable(std::move(font_unique_name_table));
...
TEST_F(FontTableMatcherTest, CaseInsensitiveMatchingBothNames)
{
    ...
    std::optional<FontTableMatcher::MatchResult> result = matcher_->MatchName("font name uppercase");
...
TEST_F(FontTableMatcherTest, MatchTamilChinese)
{
    ...
    for (std::string font_name : { "எழுத்துரு பெயர்", "எழுத்துரு-பெயர்", "字體名稱" }) {
        std::optional<FontTableMatcher::MatchResult> result = matcher_->MatchName(font_name);
        ASSERT_TRUE(result.has_value());
        ASSERT_EQ(result->font_path, kTestFilePath1);
        ASSERT_EQ(result->ttc_index, 0u);

        std::optional<FontTableMatcher::MatchResult> result_for_substring = matcher_->MatchName(font_name.substr(0, font_name.size() - 2u));
...
{
    ASSERT_EQ(matcher_->AvailableFonts(), 1u);
    std::optional<FontTableMatcher::MatchResult> result = matcher_->MatchName("font name");
