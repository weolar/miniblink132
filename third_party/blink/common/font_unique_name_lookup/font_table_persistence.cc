...
bool LoadFromFile(base::FilePath file_path, base::MappedReadOnlyRegion* name_table_region)
{
    ...
    *name_table_region = base::MappedReadOnlyRegion();
    std::vector<char> file_contents;
    {
        ...
        base::File table_cache_file(file_path, base::File::FLAG_OPEN | base::File::Flags::FLAG_READ);
        ...
        if (UNSAFE_TODO(table_cache_file.Read(0, file_contents.data(), file_contents.size())) <= 0) {
            return false;
        }
    }

    base::Pickle pickle = base::Pickle::WithUnownedBuffer(base::as_byte_span(file_contents));
    ...
    std::optional<base::span<const uint8_t>> read_result = pickle_iterator.ReadData();
...
bool PersistToFile(const base::MappedReadOnlyRegion& name_table_region, base::FilePath file_path)
{
    DCHECK(name_table_region.mapping.IsValid());
    DCHECK(name_table_region.mapping.size());
    DCHECK(!file_path.empty());

    base::File table_cache_file(file_path, base::File::FLAG_CREATE_ALWAYS | base::File::Flags::FLAG_WRITE);
    ...
    DCHECK(pickle.size());
    {
        ...
        if (UNSAFE_TODO(table_cache_file.Write(0, pickle.data_as_char(), pickle.size())) == -1) {
