...
TEST(IdentifiabilityMetricBuilderTest, Set)
{
    ...
    const auto kSurface = IdentifiableSurface::FromTypeAndToken(IdentifiableSurface::Type::kWebFeature, kInputHash);
...
TEST(IdentifiabilityMetricBuilderTest, BuilderOverload)
{
    ...
    constexpr int64_t kValue = 3;
    constexpr int64_t kInputHash = 2;
    constexpr auto kSurface = IdentifiableSurface::FromTypeAndToken(IdentifiableSurface::Type::kWebFeature, kInputHash);

    const auto kSource = ukm::SourceIdObj::New();
    IdentifiabilityMetricBuilder(kSource).Add(kSurface, kValue).Record(&recorder);

    ASSERT_EQ(1u, collector.entries().size());
    test::ScopedIdentifiabilityTestSampleCollector::Entry expected_entry = collector.entries().front();
    ...
    IdentifiabilityMetricBuilder(kUkmSource).Add(kSurface, kValue).Record(&recorder);
    ASSERT_EQ(1u, collector.entries().size());
    test::ScopedIdentifiabilityTestSampleCollector::Entry entry = collector.entries().front();
...
TEST(IdentifiabilityMetricBuilderTest, SetWebfeature)
{
    ...
    constexpr int64_t kValue = 3;
    constexpr int64_t kTestInput = static_cast<int64_t>(mojom::WebFeature::kEventSourceDocument);

    IdentifiabilityMetricBuilder builder(ukm::SourceIdObj {});
    builder.AddWebFeature(mojom::WebFeature::kEventSourceDocument, kValue).Record(&recorder);
    ASSERT_EQ(1u, collector.entries().size());
    auto entry = collector.entries().front();
    collector.ClearEntries();
    ...
    IdentifiabilityMetricBuilder(ukm::SourceIdObj {})
        .Add(IdentifiableSurface::FromTypeAndToken(IdentifiableSurface::Type::kWebFeature, kTestInput), kValue)
        .Record(&recorder);
    ASSERT_EQ(1u, collector.entries().size());
...
[[maybe_unused]] bool HasSingleEntryWithValue(const test::ScopedIdentifiabilityTestSampleCollector& collector, int64_t value)
{
    if (collector.entries().size() != 1u) {
        SCOPED_TRACE(base::StringPrintf("Expected unique entry. Found %zu entries.", collector.entries().size()));
        return false;
    }
    if (collector.entries().front().metrics.size() != 1u) {
        SCOPED_TRACE(base::StringPrintf("Expected unique metric. Found %zu entries.", collector.entries().front().metrics.size()));
        return false;
    }
    return collector.entries().front().metrics.front().value.ToUkmMetricValue() == value;
}

MATCHER_P(FirstMetricIs, entry, base::StringPrintf("entry is %s0x%" PRIx64, negation ? "not " : "", entry))
{
...
constexpr IdentifiableSurface kTestSurface = IdentifiableSurface::FromTypeAndToken(IdentifiableSurface::Type::kReservedInternal, 0);
...
TEST(IdentifiabilityMetricBuilderTest, SetChar)
{
    test::ScopedIdentifiabilityTestSampleCollector collector;
    test::TestUkmRecorder recorder;
    IdentifiabilityMetricBuilder(ukm::SourceIdObj {}).Add(kTestSurface, 'A').Record(&recorder);
...
TEST(IdentifiabilityMetricBuilderTest, SetCharArray)
{
    ...
    IdentifiabilityMetricBuilder(ukm::SourceIdObj {}).Add(kTestSurface, sample).Record(&recorder);
...
TEST(IdentifiabilityMetricBuilderTest, SetStringPiece)
{
    ...
    IdentifiabilityMetricBuilder(ukm::SourceIdObj {}).Add(kTestSurface, IdentifiableToken(std::string_view(kAbcd))).Record(&recorder);
    EXPECT_THAT(collector, FirstMetricIs(kExpectedHashOfAbcd));
}
...
TEST(IdentifiabilityMetricBuilderTest, SetInt)
{
    test::ScopedIdentifiabilityTestSampleCollector collector;
    test::TestUkmRecorder recorder;
    IdentifiabilityMetricBuilder(ukm::SourceIdObj {}).Add(kTestSurface, -5).Record(&recorder);
...
TEST(IdentifiabilityMetricBuilderTest, SetIntRef)
{
    ...
    int x = -5;
    int& xref = x;
    IdentifiabilityMetricBuilder(ukm::SourceIdObj {}).Add(kTestSurface, xref).Record(&recorder);
    EXPECT_THAT(collector, FirstMetricIs(INT64_C(-5)));
}
...
TEST(IdentifiabilityMetricBuilderTest, SetUnsigned)
{
    test::ScopedIdentifiabilityTestSampleCollector collector;
    test::TestUkmRecorder recorder;
    IdentifiabilityMetricBuilder(ukm::SourceIdObj {}).Add(kTestSurface, 5u).Record(&recorder);
    EXPECT_THAT(collector, FirstMetricIs(INT64_C(5)));
}
...
TEST(IdentifiabilityMetricBuilderTest, SetUint64)
{
    test::ScopedIdentifiabilityTestSampleCollector collector;
    test::TestUkmRecorder recorder;
    IdentifiabilityMetricBuilder(ukm::SourceIdObj {}).Add(kTestSurface, UINT64_C(5)).Record(&recorder);
    EXPECT_THAT(collector, FirstMetricIs(INT64_C(5)));
}
...
TEST(IdentifiabilityMetricBuilderTest, SetBigUnsignedInt)
{
    ...
    IdentifiabilityMetricBuilder(ukm::SourceIdObj {}).Add(kTestSurface, std::numeric_limits<uint64_t>::max()).Record(&recorder);
...
TEST(IdentifiabilityMetricBuilderTest, SetFloat)
{
    test::ScopedIdentifiabilityTestSampleCollector collector;
    test::TestUkmRecorder recorder;
    IdentifiabilityMetricBuilder(ukm::SourceIdObj {}).Add(kTestSurface, 1.5f).Record(&recorder);
...
TEST(IdentifiabilityMetricBuilderTest, SetDouble)
{
    test::ScopedIdentifiabilityTestSampleCollector collector;
    test::TestUkmRecorder recorder;
    IdentifiabilityMetricBuilder(ukm::SourceIdObj {}).Add(kTestSurface, 1.5l).Record(&recorder);
    EXPECT_THAT(collector, FirstMetricIs(kExpectedHashOfOnePointFive));
}
...
TEST(IdentifiabilityMetricBuilderTest, SetEnum)
{
    test::ScopedIdentifiabilityTestSampleCollector collector;
    test::TestUkmRecorder recorder;
    IdentifiabilityMetricBuilder(ukm::SourceIdObj {}).Add(kTestSurface, Never::kUp).Record(&recorder);
...
TEST(IdentifiabilityMetricBuilderTest, SetParameterPack)
{
    test::ScopedIdentifiabilityTestSampleCollector collector;
    test::TestUkmRecorder recorder;
    IdentifiabilityMetricBuilder(ukm::SourceIdObj {}).Add(kTestSurface, IdentifiableToken(1, 2, 3.0, 4, 'a')).Record(&recorder);
