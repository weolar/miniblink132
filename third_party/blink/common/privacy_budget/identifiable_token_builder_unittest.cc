...
TEST(IdentifiableTokenBuilderTest, ConstructVsAdd)
{
    ...
    IdentifiableTokenBuilder construct_token(base::as_bytes(base::make_span(kOneByte)));
...
TEST(IdentifiableTokenBuilderTest, AddValue)
{
    ...
    EXPECT_EQ(kExpectedToken, IdentifiableTokenBuilder().AddValue(INT8_C(1)).GetToken());
    EXPECT_EQ(kExpectedToken, IdentifiableTokenBuilder().AddValue(UINT8_C(1)).GetToken());
    EXPECT_EQ(kExpectedToken, IdentifiableTokenBuilder().AddValue(INT16_C(1)).GetToken());
    EXPECT_EQ(kExpectedToken, IdentifiableTokenBuilder().AddValue(UINT16_C(1)).GetToken());
    EXPECT_EQ(kExpectedToken, IdentifiableTokenBuilder().AddValue(INT32_C(1)).GetToken());
    EXPECT_EQ(kExpectedToken, IdentifiableTokenBuilder().AddValue(UINT32_C(1)).GetToken());
    EXPECT_EQ(kExpectedToken, IdentifiableTokenBuilder().AddValue(INT64_C(1)).GetToken());
    EXPECT_EQ(kExpectedToken, IdentifiableTokenBuilder().AddValue(UINT64_C(1)).GetToken());
}

TEST(IdentifiableTokenBuilderTest, AddAtomic_AlwaysConstant)
{
    const uint8_t kS1[] = { 1, 2, 3, 4, 5 };
    EXPECT_EQ(IdentifiableToken(INT64_C(0xfaeb0b8e769729b9)), IdentifiableTokenBuilder().AddAtomic(base::make_span(kS1)).GetToken());
}
...
TEST(IdentifiableTokenBuilderTest, AddAtomic_PadSuffix)
{
    ...
    const uint8_t kS1_padded[] = { 5, 0, 0, 0, 0, 0, 0, 0, // Little endian 5
        1, 2, 3, 4, 5, 0, 0, 0 };
    EXPECT_EQ(
        IdentifiableTokenBuilder().AddAtomic(base::make_span(kS1)).GetToken(), IdentifiableTokenBuilder().AddBytes(base::make_span(kS1_padded)).GetToken());
}
...
TEST(IdentifiableTokenBuilderTest, AddAtomic_PadPrefix)
{
    ...
    const uint8_t kS2_padded[] = { 1, 2, 0, 0, 0, 0, 0, 0, 3, 0, 0, 0, 0, 0, 0, 0, // Little endian 3
        3, 4, 5, 0, 0, 0, 0, 0 };
    EXPECT_EQ(IdentifiableTokenBuilder().AddBytes(base::make_span(kS2_pre)).AddAtomic(base::make_span(kS2)).GetToken(),
        IdentifiableTokenBuilder().AddBytes(base::make_span(kS2_padded)).GetToken());
}
...
TEST(IdentifiableTokenBuilderTest, LotsOfRandomPartitions)
{
    ...
    partitioned_sample.AddBytes(base::make_span(&data[low], kLargeBufferSize - low));
