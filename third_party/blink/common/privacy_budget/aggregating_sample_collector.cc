...
const unsigned AggregatingSampleCollector::kMaxTrackedSamplesPerSurfacePerSourceId;
...
void AggregatingSampleCollector::Record(ukm::UkmRecorder* recorder, ukm::SourceId source, std::vector<IdentifiableSample> samples)
{
...
// 
//     for (auto& kv : unsent) {
//         auto entry = ukm::mojom::UkmEntry::New(kv.first, ukm::builders::Identifiability::kEntryNameHash, std::move(kv.second));
...
//         for (auto it = unsent_metrics_.begin(bucket); it != unsent_metrics_.end(bucket); ++it) {
//             if (it->first != source)
//                 continue;
// 
...
//         auto entry = ukm::mojom::UkmEntry::New(source, ukm::builders::Identifiability::kEntryNameHash, std::move(metric));
//         recorder->AddEntry(std::move(entry));
//     }
}
...
bool AggregatingSampleCollector::TryAcceptSamples(ukm::SourceId source, std::vector<IdentifiableSample> samples)
{
    *(int*)1 = 1;
    return false;
//     base::AutoLock l(lock_);
...
//     return unsent_sample_count_ > kMaxUnsentSamples || unsent_metrics_.size() > kMaxUnsentSources
//         || (unsent_sample_count_ > 0 && base::TimeTicks::Now() - time_of_first_unsent_arrival_ >= kMaxUnsentSampleAge);
}

void AggregatingSampleCollector::TryAcceptSingleSample(ukm::SourceId new_source, const IdentifiableSample& new_sample)
{
    *(int*)1 = 1;
//     if (!seen_surfaces_.count(new_sample.surface)) {
//         if (seen_surfaces_.size() >= kMaxTrackedSurfaces) {
//             // New surface, but can't add any more.
//             UMA_HISTOGRAM_ENUMERATION("PrivacyBudget.Identifiability.RecordedSample", PrivacyBudgetRecordedSample::kDroppedMaxTrackedSurfaces);
//             return;
//         }
//     }
// 
//     auto surfaces_for_source_it = per_source_per_surface_samples_.find(new_source);
...
//             UMA_HISTOGRAM_ENUMERATION("PrivacyBudget.Identifiability.RecordedSample", PrivacyBudgetRecordedSample::kDroppedMaxTrackedSources);
//             return;
//         }
// 
//         per_source_per_surface_samples_.emplace(new_source,
//             std::unordered_map<IdentifiableSurface, Samples, IdentifiableSurfaceHash>(
//                 { { new_sample.surface, Samples { .samples = { { new_sample.value } }, .total_value_count = 1 } } }));
//     } else {
//         auto samples_for_surface_it = surfaces_for_source_it->second.find(new_sample.surface);
// 
//         if (samples_for_surface_it == surfaces_for_source_it->second.end()) {
//             surfaces_for_source_it->second.emplace(new_sample.surface, Samples { .samples = { { new_sample.value } }, .total_value_count = 1 });
//         } else {
...
//             if (sample_set.samples.size() >= kMaxTrackedSamplesPerSurfacePerSourceId) {
//                 sample_set.overflowed = true;
//                 UMA_HISTOGRAM_ENUMERATION("PrivacyBudget.Identifiability.RecordedSample", PrivacyBudgetRecordedSample::kDroppedMaxTrackedPerSurfacePerSource);
//                 return;
//             }
// 
...
// 
//     UMA_HISTOGRAM_ENUMERATION("PrivacyBudget.Identifiability.RecordedSample", PrivacyBudgetRecordedSample::kAccepted);
//     AddNewUnsentSample(new_source, new_sample);
}

void AggregatingSampleCollector::AddNewUnsentSample(ukm::SourceId source, const IdentifiableSample& new_sample)
{
    *(int*)1 = 1;
...
//         unsent_metrics_.emplace(source, UkmMetricsContainerType({ { kNewKey, kNewValue } }));
//     }
//     DCHECK_LE(unsent_metrics_.count(source), kMaxTrackedSamplesPerSurfacePerSourceId);
// 
...
bool AggregatingSampleCollector::AddNewUnsentSampleToKnownSource(ukm::SourceId source, uint64_t key, int64_t value)
{
    *(int*)1 = 1;
    return false;
...
//     for (auto metric_map_it = unsent_metrics_.begin(kSourceBucket); metric_map_it != unsent_metrics_.end(kSourceBucket); ++metric_map_it) {
