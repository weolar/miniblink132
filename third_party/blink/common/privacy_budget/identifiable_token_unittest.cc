...
struct ImplicitConverter {
    ...
    ImplicitConverter(IdentifiableToken sample)
        : sample(sample)
    {
    }
...
TEST(IdentifiableTokenTest, SampleBool)
{
    ...
    EXPECT_EQ(IdentifiableToken(expected_value), ImplicitConverter(source_value).sample);
}
...
{
    auto source_value = static_cast<uint64_t>(std::numeric_limits<int64_t>::max()) + 1;
...
{
    EXPECT_EQ(IdentifiableToken(INT64_C(0xf75a3b8a1499428d)), IdentifiableToken("abcd"));
    // No implicit converter for const char[].
}

TEST(IdentifiableTokenTest, SampleStdString)
{
    EXPECT_EQ(IdentifiableToken(INT64_C(0xf75a3b8a1499428d)), IdentifiableToken(std::string("abcd")));
...
{
    EXPECT_EQ(IdentifiableToken(INT64_C(0x5848123245be627a)), IdentifiableToken(1, 2, 3, 4, 5));
    // No implicit converter for tuples.
}

TEST(IdentifiableTokenTest, SampleHeterogenousTuple)
{
    EXPECT_EQ(IdentifiableToken(INT64_C(0x672cf4c107b5b22)), IdentifiableToken(1, 2, 3.0, 4, 'a'));
    // No implicit converter for tuples.
}
