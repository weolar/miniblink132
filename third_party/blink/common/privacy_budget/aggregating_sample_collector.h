...
// being recorded.
class BLINK_COMMON_EXPORT_PRIVATE AggregatingSampleCollector : public IdentifiabilitySampleCollector {
public:
    ...
    void Record(ukm::UkmRecorder* recorder, ukm::SourceId source, std::vector<IdentifiableSample> metrics) override LOCKS_EXCLUDED(lock_);
    ...
    void FlushSource(ukm::UkmRecorder* recorder, ukm::SourceId source) override LOCKS_EXCLUDED(lock_);
...
private:
    ...
    bool TryAcceptSamples(ukm::SourceId source, std::vector<IdentifiableSample> samples) LOCKS_EXCLUDED(lock_);
    ...
    void TryAcceptSingleSample(ukm::SourceId source, const IdentifiableSample& sample) EXCLUSIVE_LOCKS_REQUIRED(lock_);

    // Populates `unsent_metrics_` based on a single `IdentifiableSample`.
    void AddNewUnsentSample(ukm::SourceId source, const IdentifiableSample& sample) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    bool AddNewUnsentSampleToKnownSource(ukm::SourceId source, uint64_t key, int64_t value) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    base::flat_map<ukm::SourceId, std::unordered_map<IdentifiableSurface, Samples, IdentifiableSurfaceHash>> per_source_per_surface_samples_ GUARDED_BY(lock_);

    // Seen surfaces across all sources.
    std::unordered_set<IdentifiableSurface, IdentifiableSurfaceHash> seen_surfaces_ GUARDED_BY(lock_);
    ...
    std::unordered_multimap<ukm::SourceId, UkmMetricsContainerType> unsent_metrics_ GUARDED_BY(lock_);
