...
TEST(IdentifiabilityStudySettingsTest, DisabledProvider)
{
    ...
    IdentifiabilityStudySettings settings(std::make_unique<CountingSettingsProvider>(&counts));
    ...
    EXPECT_EQ(1, counts.count_of_is_active);
    EXPECT_FALSE(settings.ShouldSampleType(IdentifiableSurface::Type::kCanvasReadback));
...
TEST(IdentifiabilityStudySettingsTest, MetaExperimentActive)
{
    ...
    EXPECT_TRUE(settings.ShouldSampleType(IdentifiableSurface::Type::kWebFeature));
...
TEST(IdentifiabilityStudySettingsTest, MetaExperimentActiveWithBlockedTypeOrSurface)
{
    CallCounts counts {
...
TEST(IdentifiabilityStudySettingsTest, IsSurfaceOrTypeBlocked)
{
    ...
    EXPECT_TRUE(settings.IsActive());
    EXPECT_FALSE(settings.ShouldSampleSurface(IdentifiableSurface()));
    EXPECT_FALSE(settings.ShouldSampleType(IdentifiableSurface::Type::kWebFeature));
...
TEST(IdentifiabilityStudySettingsTest, DefaultSettings)
{
    ...
    EXPECT_FALSE(default_settings->ShouldSampleType(IdentifiableSurface::Type::kWebFeature));
}
...
TEST(IdentifiabilityStudySettingsTest, StaticSetProvider)
{
    ...
    IdentifiabilityStudySettings::SetGlobalProvider(std::make_unique<CountingSettingsProvider>(&counts));
