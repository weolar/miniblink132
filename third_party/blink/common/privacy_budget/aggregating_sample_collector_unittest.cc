...
constexpr IdentifiableSurface kTestSurface1 = IdentifiableSurface::FromMetricHash(1 << 8);
constexpr IdentifiableSurface kTestSurface2 = IdentifiableSurface::FromMetricHash(2 << 8);
...
public:
    ...
    bool IsSurfaceAllowed(IdentifiableSurface) const override
    {
        return true;
    }
    bool IsTypeAllowed(IdentifiableSurface::Type) const override
    {
        return true;
    }
};
...
public:
    ...
    {
        IdentifiabilityStudySettings::SetGlobalProvider(std::make_unique<TestSettingsProvider>());
    }
...
TEST_F(AggregatingSampleCollectorTest, MergesDuplicates)
{
    ...
    collector()->Flush(recorder());
    const auto entries = recorder()->GetEntriesByHash(ukm::builders::Identifiability::kEntryNameHash);

    // We end up with two entries, one per source.
    EXPECT_THAT(entries,
        UnorderedElementsAre(Pointee(*ukm::mojom::UkmEntry::New(kTestSource1, ukm::builders::Identifiability::kEntryNameHash,
                                 base::flat_map<uint64_t, int64_t> { { kTestSurface1.ToUkmMetricHash(), kTestValue1.ToUkmMetricValue() } })),
            Pointee(*ukm::mojom::UkmEntry::New(kTestSource2, ukm::builders::Identifiability::kEntryNameHash,
                base::flat_map<uint64_t, int64_t> { { kTestSurface1.ToUkmMetricHash(), kTestValue1.ToUkmMetricValue() } }))));

    histogram_tester.ExpectBucketCount("PrivacyBudget.Identifiability.RecordedSample", PrivacyBudgetRecordedSample::kAccepted, 2);
}
...
TEST_F(AggregatingSampleCollectorTest, TooManySurfaces)
{
    ...
    for (; i < AggregatingSampleCollector::kMaxTrackedSurfaces; ++i) {
        collector()->Record(recorder(), kTestSource1, { { IdentifiableSurface::FromMetricHash(i << 8), 1 } });
    }
    collector()->Flush(recorder());
    ...
    histogram_tester.ExpectBucketCount(
        "PrivacyBudget.Identifiability.RecordedSample", PrivacyBudgetRecordedSample::kAccepted, AggregatingSampleCollector::kMaxTrackedSurfaces);
    histogram_tester.ExpectBucketCount("PrivacyBudget.Identifiability.RecordedSample", PrivacyBudgetRecordedSample::kDroppedMaxTrackedSurfaces, 1);
}
...
TEST_F(AggregatingSampleCollectorTest, TooManySources)
{
    ...
    histogram_tester.ExpectBucketCount(
        "PrivacyBudget.Identifiability.RecordedSample", PrivacyBudgetRecordedSample::kAccepted, AggregatingSampleCollector::kMaxTrackedSources + 1);
    histogram_tester.ExpectBucketCount("PrivacyBudget.Identifiability.RecordedSample", PrivacyBudgetRecordedSample::kDroppedMaxTrackedSources, 1);
}
...
TEST_F(AggregatingSampleCollectorTest, TooManySamplesPerSurface)
{
    ...
    for (; i < AggregatingSampleCollector::kMaxTrackedSamplesPerSurfacePerSourceId; ++i) {
        collector()->Record(recorder(), kTestSource1, { { kTestSurface1, i } });
    }
    collector()->Flush(recorder());
    EXPECT_EQ(AggregatingSampleCollector::kMaxTrackedSamplesPerSurfacePerSourceId, recorder()->entries_count());
    EXPECT_EQ(1u, recorder()->entries()[0]->metrics.size());
    recorder()->Purge();
    EXPECT_EQ(0u, recorder()->entries_count());
    ...
    histogram_tester.ExpectBucketCount("PrivacyBudget.Identifiability.RecordedSample", PrivacyBudgetRecordedSample::kAccepted,
        AggregatingSampleCollector::kMaxTrackedSamplesPerSurfacePerSourceId + 2);
    histogram_tester.ExpectBucketCount("PrivacyBudget.Identifiability.RecordedSample", PrivacyBudgetRecordedSample::kDroppedMaxTrackedPerSurfacePerSource, 1);
}

TEST_F(AggregatingSampleCollectorTest, TooManyUnsentMetrics)
{
    // The test is inconclusive if this condition doesn't hold.
    ASSERT_LT(AggregatingSampleCollector::kMaxUnsentSamples, AggregatingSampleCollector::kMaxTrackedSurfaces);
...
{
    // The test is inconclusive if this condition doesn't hold.
    ASSERT_LT(AggregatingSampleCollector::kMaxUnsentSources, AggregatingSampleCollector::kMaxTrackedSurfaces);
    ASSERT_LT(AggregatingSampleCollector::kMaxUnsentSources, AggregatingSampleCollector::kMaxUnsentSamples);

    // Stop one short of the limit.
    unsigned i = 0;
    for (; i < AggregatingSampleCollector::kMaxUnsentSources; ++i) {
        collector()->Record(recorder(), ukm::AssignNewSourceId(), { { IdentifiableSurface::FromMetricHash(i << 8), 1 } });
    }
    EXPECT_EQ(0u, recorder()->entries_count());
...
TEST_F(AggregatingSampleCollectorTest, UnsentMetricsAreTooOld)
{
    ...
    task_environment().FastForwardBy(AggregatingSampleCollector::kMaxUnsentSampleAge);
