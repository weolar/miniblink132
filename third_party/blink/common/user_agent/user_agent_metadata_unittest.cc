...
TEST(UserAgentMetaDataTest, Boundary)
{
    ...
    EXPECT_EQ(std::nullopt, UserAgentMetadata::Demarshal(std::string("nonsense")));
}

TEST(UserAgentMetaDataTest, Basic)
{
    blink::UserAgentMetadata to_encode = MakeToEncode();
    EXPECT_EQ(to_encode, UserAgentMetadata::Demarshal(UserAgentMetadata::Marshal(to_encode)));
}
...
TEST(UserAgentMetaDataTest, MojoTraits)
{
    ...
    mojo::test::SerializeAndDeserialize<mojom::UserAgentMetadata>(to_encode, copied);
