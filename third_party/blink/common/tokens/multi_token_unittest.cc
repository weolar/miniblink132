...
TEST(MultiTokenTest, MultiTokenWorks)
{
    ...
    // Test hasher.
    EXPECT_EQ(FooBarToken::Hasher()(token2), base::UnguessableTokenHash()(token2.value()));
...
{
    struct Visitor {
        std::string_view operator()(const FooToken& token)
        {
            return "FooToken";
        }
        std::string_view operator()(const BarToken& token)
        {
            return "BarToken";
        }
        std::string_view operator()(const BazToken& token)
        {
            return "BazToken";
        }
    };
