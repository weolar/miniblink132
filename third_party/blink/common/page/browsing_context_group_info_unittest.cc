...
{
    base::UnguessableToken browsing_context_group_token = base::UnguessableToken::Create();
    base::UnguessableToken coop_related_group_token = base::UnguessableToken::Create();
    BrowsingContextGroupInfo bcg_info(browsing_context_group_token, coop_related_group_token);

    EXPECT_FALSE(bcg_info.browsing_context_group_token.is_empty());
    EXPECT_FALSE(bcg_info.coop_related_group_token.is_empty());

    EXPECT_EQ(bcg_info.browsing_context_group_token, browsing_context_group_token);
...
TEST(BrowsingContextGroupInfoTest, CreateUnique)
{
    ...
    EXPECT_FALSE(bcg_info.browsing_context_group_token.is_empty());
    EXPECT_FALSE(bcg_info.coop_related_group_token.is_empty());
    EXPECT_NE(bcg_info.coop_related_group_token, bcg_info.browsing_context_group_token);
}
...
TEST(BrowsingContextGroupInfoTest, ComparisonOperator)
{
    ...
    BrowsingContextGroupInfo bcg_info = BrowsingContextGroupInfo::CreateUnique();
    BrowsingContextGroupInfo other_bcg_info = BrowsingContextGroupInfo::CreateUnique();
    ...
    // the comparison fail.
    bcg_info_clone.browsing_context_group_token = base::UnguessableToken::Create();
    EXPECT_NE(bcg_info, bcg_info_clone);

    // Verify that having different coop_related_group_tokens is enough to have
    // the comparison fail.
    bcg_info_clone.browsing_context_group_token = bcg_info.browsing_context_group_token;
