...
TEST(SerializeAuctionConfigTest, SerializeComponents)
{
    ...
    config.non_shared_params.component_auctions.push_back(CreateBasicAuctionConfig(GURL("https://example.org/foo.js")));
    config.non_shared_params.component_auctions.push_back(CreateBasicAuctionConfig(GURL("https://example.com/bar.js")));
...
TEST(SerializeAuctionConfigTest, FullConfig)
{
    ...
    config.non_shared_params.auction_nonce = base::Uuid::ParseLowercase("626e6419-1872-48ac-877d-c4c096f28284");
...
{
    AuctionConfig config = CreateBasicAuctionConfig();
    config.non_shared_params.seller_signals = AuctionConfig::MaybePromiseJson::FromPromise();
...
TEST(SerializeAuctionConfigTest, ServerResponse)
{
    ...
    config.server_response->request_id = base::Uuid::ParseLowercase("626e6419-1872-48ac-877d-c4c096f28284");
    base::Value::Dict serialized = SerializeAuctionConfigForDevtools(config);
...
TEST(SerializeInterestGroupTest, Basic)
{
    ...
    EXPECT_THAT(SerializeInterestGroupForDevtools(ig), base::test::IsJson(kExpected));
}
