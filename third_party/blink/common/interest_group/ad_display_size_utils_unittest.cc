...
void RunTest(const std::string& input, double expected_val, blink::AdSize::LengthUnit expected_unit)
{
...
{
    EXPECT_EQ("px", ConvertAdSizeUnitToString(blink::AdSize::LengthUnit::kPixels));
    EXPECT_EQ("sw", ConvertAdSizeUnitToString(blink::AdSize::LengthUnit::kScreenWidth));
    EXPECT_EQ("sh", ConvertAdSizeUnitToString(blink::AdSize::LengthUnit::kScreenHeight));
    EXPECT_TRUE(ConvertAdSizeUnitToString(blink::AdSize::LengthUnit::kInvalid).empty());
}

TEST(AdDisplaySizeUtilsTest, ConvertAdSizeToString)
{
    // clang-format on
    const AdSize kAdSize1(10, AdSize::LengthUnit::kPixels, 15, AdSize::LengthUnit::kPixels);
    EXPECT_EQ(ConvertAdSizeToString(kAdSize1), "10px,15px");

    const AdSize kAdSize2(0.5, AdSize::LengthUnit::kScreenWidth, 0.2, AdSize::LengthUnit::kScreenHeight);
    EXPECT_EQ(ConvertAdSizeToString(kAdSize2), "0.5sw,0.2sh");

    const AdSize kAdSize3(0.2, AdSize::LengthUnit::kScreenHeight, 0.5, AdSize::LengthUnit::kScreenWidth);
    EXPECT_EQ(ConvertAdSizeToString(kAdSize3), "0.2sh,0.5sw");

    const AdSize kAdSize4(10.5, AdSize::LengthUnit::kPixels, 11, AdSize::LengthUnit::kScreenWidth);
...
{
    EXPECT_EQ(ConvertAdDimensionToString(10, AdSize::LengthUnit::kPixels), "10px");
    EXPECT_EQ(ConvertAdDimensionToString(0.5, AdSize::LengthUnit::kScreenWidth), "0.5sw");
    EXPECT_EQ(ConvertAdDimensionToString(0.5, AdSize::LengthUnit::kScreenHeight), "0.5sh");
}
...
{
    AdSize ad_size(10.0, AdSize::LengthUnit::kPixels, 5.0, AdSize::LengthUnit::kScreenWidth);
    EXPECT_TRUE(IsValidAdSize(ad_size));
}

TEST(AdDisplaySizeUtilsTest, AdSizeInvalidUnits)
{
    AdSize ad_size(10.0, AdSize::LengthUnit::kInvalid, 5.0, AdSize::LengthUnit::kScreenWidth);
    EXPECT_FALSE(IsValidAdSize(ad_size));
}

TEST(AdDisplaySizeUtilsTest, AdSizeZeroValue)
{
    AdSize ad_size(0.0, AdSize::LengthUnit::kPixels, 5.0, AdSize::LengthUnit::kScreenWidth);
    EXPECT_FALSE(IsValidAdSize(ad_size));
}

TEST(AdDisplaySizeUtilsTest, AdSizeNegativeValue)
{
    AdSize ad_size(-1.0, AdSize::LengthUnit::kPixels, 5.0, AdSize::LengthUnit::kScreenWidth);
    EXPECT_FALSE(IsValidAdSize(ad_size));
}

TEST(AdDisplaySizeUtilsTest, AdSizeInfiniteValue)
{
    AdSize ad_size(std::numeric_limits<double>::infinity(), AdSize::LengthUnit::kPixels, 5.0, AdSize::LengthUnit::kScreenWidth);
    EXPECT_FALSE(IsValidAdSize(ad_size));
}
