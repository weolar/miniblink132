...
constexpr char kKAnonKeyForAdNameReportingSelectedBuyerAndSellerIdPrefix[] = "SelectedBuyerAndSellerReportId\n";
...
// bidding signals URL.
bool IsUrlAllowedForTrustedBiddingSignals(const GURL& url, const InterestGroup& group)
{
    return IsUrlAllowedForRenderUrls(url) && !url.has_ref() && !url.has_query();
}

size_t EstimateFlatMapSize(const base::flat_map<std::string, double>& flat_map)
{
    size_t result = 0;
...
void AppendReportingIdForSelectedReportingKeyKAnonKey(base::optional_ref<const std::string> reporting_id, std::string& k_anon_key)
{
    ...
    std::array<uint8_t, 4u> size_in_bytes = base::U32ToBigEndian(reporting_id->size());
    base::StrAppend(&k_anon_key, { "\n", std::string_view("\x01", 1), base::as_string_view(base::as_chars(base::make_span(size_in_bytes))), *reporting_id });
}

std::string InternalPlainTextKAnonKeyForAdNameReporting(const url::Origin& interest_group_owner, const std::string& interest_group_name,
    const GURL& interest_group_bidding_url, const std::string& ad_render_url, base::optional_ref<const std::string> buyer_reporting_id,
    base::optional_ref<const std::string> buyer_and_seller_reporting_id, base::optional_ref<const std::string> selected_buyer_and_seller_reporting_id)
{
    std::string middle = base::StrCat({ interest_group_owner.GetURL().spec(), "\n", interest_group_bidding_url.spec(), "\n", ad_render_url });
    ...
    if (selected_buyer_and_seller_reporting_id.has_value()) {
        ...
        std::string k_anon_key = base::StrCat({ kKAnonKeyForAdNameReportingSelectedBuyerAndSellerIdPrefix, middle });
        AppendReportingIdForSelectedReportingKeyKAnonKey(selected_buyer_and_seller_reporting_id, k_anon_key);
        AppendReportingIdForSelectedReportingKeyKAnonKey(buyer_and_seller_reporting_id, k_anon_key);
        AppendReportingIdForSelectedReportingKeyKAnonKey(buyer_reporting_id, k_anon_key);
        return k_anon_key;
    }

    if (buyer_and_seller_reporting_id.has_value()) {
        return base::StrCat({ kKAnonKeyForAdNameReportingBuyerAndSellerIdPrefix, middle, "\n", *buyer_and_seller_reporting_id });
    }
...
InterestGroup::Ad::Ad(base::PassKey<content::InterestGroupStorage>, std::string&& render_url)
    : render_url_(std::move(render_url))
{
}
InterestGroup::Ad::Ad(base::PassKey<content::InterestGroupStorage>, const std::string& render_url)
    : render_url_(render_url)
{
}
InterestGroup::Ad::Ad(GURL render_gurl, std::optional<std::string> metadata, std::optional<std::string> size_group,
    std::optional<std::string> buyer_reporting_id, std::optional<std::string> buyer_and_seller_reporting_id,
    std::optional<std::vector<std::string>> selectable_buyer_and_seller_reporting_ids, std::optional<std::string> ad_render_id,
    ...
    , selectable_buyer_and_seller_reporting_ids(std::move(selectable_buyer_and_seller_reporting_ids))
...
{
    return std::tie(render_url_, size_group, metadata, buyer_reporting_id, buyer_and_seller_reporting_id, selectable_buyer_and_seller_reporting_ids,
               ad_render_id, allowed_reporting_origins)
        == std::tie(other.render_url_, other.size_group, other.metadata, other.buyer_reporting_id, other.buyer_and_seller_reporting_id,
            other.selectable_buyer_and_seller_reporting_ids, other.ad_render_id, other.allowed_reporting_origins);
}
...
bool InterestGroup::IsValid() const
{
    ...
    if (priority_signals_overrides) {
        for (const auto& [unused_signal_name, value] : *priority_signals_overrides) {
            if (!std::isfinite(value)) {
                return false;
            }
        }
    ...
    if (execution_mode != blink::mojom::InterestGroup::ExecutionMode::kCompatibilityMode
        && execution_mode != blink::mojom::InterestGroup::ExecutionMode::kGroupedByOriginMode
        && execution_mode != blink::mojom::InterestGroup::ExecutionMode::kFrozenContext) {
        return false;
    }
    ...
    if (bidding_wasm_helper_url && !IsUrlAllowed(*bidding_wasm_helper_url, *this)) {
        return false;
    }
    ...
    if (trusted_bidding_signals_url) {
        if (!IsUrlAllowedForTrustedBiddingSignals(*trusted_bidding_signals_url, *this)) {
            return false;
        }
    }

    if (trusted_bidding_signals_slot_size_mode != blink::mojom::InterestGroup::TrustedBiddingSignalsSlotSizeMode::kNone
        && trusted_bidding_signals_slot_size_mode != blink::mojom::InterestGroup::TrustedBiddingSignalsSlotSizeMode::kSlotSize
        && trusted_bidding_signals_slot_size_mode != blink::mojom::InterestGroup::TrustedBiddingSignalsSlotSizeMode::kAllSlotsRequestedSizes) {
        return false;
    }
    ...
    if (ads) {
        for (const auto& ad : ads.value()) {
            ...
            if (ad.size_group) {
                if (ad.size_group->empty() || !size_groups || !size_groups->contains(ad.size_group.value())) {
                    return false;
                }
            }
            if (ad.ad_render_id) {
            ...
            if (ad.allowed_reporting_origins) {
                if (ad.allowed_reporting_origins->size() > blink::mojom::kMaxAllowedReportingOrigins) {
                    return false;
                }
    ...
    if (ad_components) {
        for (const auto& ad : ad_components.value()) {
            ...
            if (ad.buyer_reporting_id || ad.buyer_and_seller_reporting_id || ad.selectable_buyer_and_seller_reporting_ids || ad.allowed_reporting_origins) {
                return false;
            }
        }
    }
...
//     }
    ...
    if (aggregation_coordinator_origin && aggregation_coordinator_origin->scheme() != url::kHttpsScheme) {
        return false;
    }
...
size_t InterestGroup::EstimateSize() const
{
    ...
    if (seller_capabilities) {
        for (const auto& [seller_origin, flags] : *seller_capabilities) {
            size += seller_origin.Serialize().size() + sizeof(decltype(flags)::EnumType);
        }
    }
...
std::string_view InterestGroup::TrustedBiddingSignalsSlotSizeModeToString(TrustedBiddingSignalsSlotSizeMode slot_size_mode)
{
...
std::string DEPRECATED_KAnonKeyForAdBid(const url::Origin& owner, const GURL& bidding_url, const std::string& ad_url_from_gurl_spec)
{
    return base::StrCat({ kKAnonKeyForAdBidPrefix, owner.GetURL().spec(), "\n", bidding_url.spec(), "\n", ad_url_from_gurl_spec });
}

std::string DEPRECATED_KAnonKeyForAdBid(const InterestGroup& group, const std::string& ad_url_from_gurl_spec)
{
    DCHECK(group.ads);
    DCHECK(base::Contains(*group.ads, ad_url_from_gurl_spec, [](const blink::InterestGroup::Ad& ad) { return ad.render_url(); }))
        << "No such ad: " << ad_url_from_gurl_spec;
    DCHECK(group.bidding_url);
    return DEPRECATED_KAnonKeyForAdBid(group.owner, group.bidding_url.value_or(GURL()), ad_url_from_gurl_spec);
}

std::string HashedKAnonKeyForAdBid(const url::Origin& owner, const GURL& bidding_url, const std::string& ad_url_from_gurl_spec)
{
    *(int*)1 = 1;
    return std::string();
    //return crypto::SHA256HashString(DEPRECATED_KAnonKeyForAdBid(owner, bidding_url, ad_url_from_gurl_spec));
}

std::string HashedKAnonKeyForAdBid(const InterestGroup& group, const std::string& ad_url_from_gurl_spec)
{
    *(int*)1 = 1;
    return std::string();
    //return crypto::SHA256HashString(DEPRECATED_KAnonKeyForAdBid(group, ad_url_from_gurl_spec));
}

std::string HashedKAnonKeyForAdBid(const InterestGroup& group, const blink::AdDescriptor& ad_descriptor)
{
    return HashedKAnonKeyForAdBid(group, ad_descriptor.url.spec());
}

std::string DEPRECATED_KAnonKeyForAdComponentBid(const std::string& ad_url_from_gurl_spec)
{
    return base::StrCat({ kKAnonKeyForAdComponentBidPrefix, ad_url_from_gurl_spec });
}

std::string HashedKAnonKeyForAdComponentBid(const std::string& ad_url_from_gurl_spec)
{
    *(int*)1 = 1;
    return std::string();
    //return crypto::SHA256HashString(DEPRECATED_KAnonKeyForAdComponentBid(ad_url_from_gurl_spec));
}
...
std::string HashedKAnonKeyForAdComponentBid(const GURL& ad_url)
{
    *(int*)1 = 1;
    return std::string();
    //return crypto::SHA256HashString(DEPRECATED_KAnonKeyForAdComponentBid(ad_url.spec()));
}

std::string HashedKAnonKeyForAdComponentBid(const blink::AdDescriptor& ad_descriptor)
{
...
std::string DEPRECATED_KAnonKeyForAdNameReporting(
    const blink::InterestGroup& group, const blink::InterestGroup::Ad& ad, base::optional_ref<const std::string> selected_buyer_and_seller_reporting_id)
{
    DCHECK(group.ads);
    DCHECK(base::Contains(*group.ads, ad)) << "No such ad: " << ad.render_url();
    DCHECK(group.bidding_url);
    return InternalPlainTextKAnonKeyForAdNameReporting(group.owner, group.name, group.bidding_url.value_or(GURL()), ad.render_url(), ad.buyer_reporting_id,
        ad.buyer_and_seller_reporting_id, selected_buyer_and_seller_reporting_id);
}
    ...
    const blink::InterestGroup& group, const blink::InterestGroup::Ad& ad, base::optional_ref<const std::string> selected_buyer_and_seller_reporting_id)
{
    *(int*)1 = 1;
    return std::string();
    //return crypto::SHA256HashString(DEPRECATED_KAnonKeyForAdNameReporting(group, ad, selected_buyer_and_seller_reporting_id));
}

std::string HashedKAnonKeyForAdNameReportingWithoutInterestGroup(const url::Origin& interest_group_owner, const std::string& interest_group_name,
    const GURL& interest_group_bidding_url, const std::string& ad_render_url, base::optional_ref<const std::string> buyer_reporting_id,
    base::optional_ref<const std::string> buyer_and_seller_reporting_id, base::optional_ref<const std::string> selected_buyer_and_seller_reporting_id)
{
    *(int*)1 = 1;
    return std::string();
//     return crypto::SHA256HashString(InternalPlainTextKAnonKeyForAdNameReporting(interest_group_owner, interest_group_name, interest_group_bidding_url,
//         ad_render_url, buyer_reporting_id, buyer_and_seller_reporting_id, selected_buyer_and_seller_reporting_id));
}
} // namespace blink
