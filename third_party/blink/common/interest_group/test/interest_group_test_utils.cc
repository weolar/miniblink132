...
#define IG_COMPARE_VEC(actual, expected, func) IgCompareVecInternal(#actual, #expected, actual, expected, expect_equals, found_unequal, func)

#define IG_COMPARE_MAP(actual, expected, func) IgCompareMapInternal(#actual, #expected, actual, expected, expect_equals, found_unequal, func)
...
void IgCompareVecInternal(std::string_view a_name, std::string_view b_name, const std::vector<T>& actual, const std::vector<T>& expected,
    const bool expect_equals, bool& found_unequal, Func f)
{
...
template <typename T, typename Func>
void IgCompareVecInternal(std::string_view a_name, std::string_view b_name, const std::optional<std::vector<T>>& actual,
    const std::optional<std::vector<T>>& expected, const bool expect_equals, bool& found_unequal, Func f)
{
    SCOPED_TRACE(base::StrCat({ a_name, " and ", b_name }));
    IG_COMPARE(actual.has_value(), expected.has_value());
    if (actual && expected) {
        IgCompareVecInternal(a_name, b_name, *actual, *expected, expect_equals, found_unequal, f);
    }
}
...
void IgCompareMapInternal(std::string_view a_name, std::string_view b_name, const base::flat_map<K, V>& actual, const base::flat_map<K, V>& expected,
    const bool expect_equals, bool& found_unequal, Func f)
{
    SCOPED_TRACE(base::StrCat({ a_name, " and ", b_name }));
    IG_COMPARE(actual.size(), expected.size());
    if (actual.size() == expected.size()) {
        ...
        size_t i = 0;
        for (auto a_it = actual.begin(), b_it = expected.begin(); a_it != actual.end(); a_it++, b_it++, i++) {
            SCOPED_TRACE(i);
...
template <typename K, typename V, typename Func>
void IgCompareMapInternal(std::string_view a_name, std::string_view b_name, const std::optional<base::flat_map<K, V>>& actual,
    const std::optional<base::flat_map<K, V>>& expected, const bool expect_equals, bool& found_unequal, Func f)
{
    SCOPED_TRACE(base::StrCat({ a_name, " and ", b_name }));
    IG_COMPARE(actual.has_value(), expected.has_value());
    if (actual && expected) {
        IgCompareMapInternal(a_name, b_name, *actual, *expected, expect_equals, found_unequal, f);
    }
}
...
// messages.
void InterestGroupCompare(const blink::InterestGroup& actual, const blink::InterestGroup& expected, bool expect_equals)
{
    ...
    IG_COMPARE(actual.enable_bidding_signals_prioritization, expected.enable_bidding_signals_prioritization);
    auto compare_doubles = [&](double actual, double expected) { IG_COMPARE(actual, expected); };
    IG_COMPARE_MAP(actual.priority_vector, expected.priority_vector, compare_doubles);
    IG_COMPARE_MAP(actual.priority_signals_overrides, expected.priority_signals_overrides, compare_doubles);
    auto compare_seller_capabilities = [&](SellerCapabilitiesType actual, SellerCapabilitiesType expected) { IG_COMPARE(actual, expected); };
    IG_COMPARE_MAP(actual.seller_capabilities, expected.seller_capabilities, compare_seller_capabilities);
    IG_COMPARE(actual.all_sellers_capabilities, expected.all_sellers_capabilities);
    ...
    IG_COMPARE(actual.trusted_bidding_signals_url, expected.trusted_bidding_signals_url);
    auto compare_strings = [&](const std::string& actual, const std::string& expected) { IG_COMPARE(actual, expected); };
    IG_COMPARE_VEC(actual.trusted_bidding_signals_keys, expected.trusted_bidding_signals_keys, compare_strings);
    IG_COMPARE(actual.trusted_bidding_signals_slot_size_mode, expected.trusted_bidding_signals_slot_size_mode);
    IG_COMPARE(actual.max_trusted_bidding_signals_url_length, expected.max_trusted_bidding_signals_url_length);
    IG_COMPARE(actual.trusted_bidding_signals_coordinator, expected.trusted_bidding_signals_coordinator);
    IG_COMPARE(actual.user_bidding_signals, expected.user_bidding_signals);
    auto compare_ads = [&](const blink::InterestGroup::Ad& actual, const blink::InterestGroup::Ad& expected) {
        ...
        IG_COMPARE(actual.buyer_and_seller_reporting_id, expected.buyer_and_seller_reporting_id);
        IG_COMPARE_VEC(actual.selectable_buyer_and_seller_reporting_ids, expected.selectable_buyer_and_seller_reporting_ids, compare_strings);
        IG_COMPARE(actual.ad_render_id, expected.ad_render_id);

        auto compare_origins = [&](const url::Origin& actual, const url::Origin& expected) { IG_COMPARE(actual, expected); };
        IG_COMPARE_VEC(actual.allowed_reporting_origins, expected.allowed_reporting_origins, compare_origins);
    };
    IG_COMPARE_VEC(actual.ads, expected.ads, compare_ads);
    IG_COMPARE_VEC(actual.ad_components, expected.ad_components, compare_ads);
    auto compare_ad_sizes = [&](const blink::AdSize& actual, const blink::AdSize& expected) {
    ...
    auto compare_vectors_of_strings
        = [&](const std::vector<std::string>& actual, const std::vector<std::string>& expected) { IG_COMPARE_VEC(actual, expected, compare_strings); };
    IG_COMPARE_MAP(actual.size_groups, expected.size_groups, compare_vectors_of_strings);
    IG_COMPARE(actual.auction_server_request_flags, expected.auction_server_request_flags);
    IG_COMPARE(actual.additional_bid_key, expected.additional_bid_key);
    IG_COMPARE(actual.aggregation_coordinator_origin, expected.aggregation_coordinator_origin);
...
void IgExpectEqualsForTesting(const blink::InterestGroup& actual, const blink::InterestGroup& expected)
{
    InterestGroupCompare(actual, expected, /*expect_equals=*/true);
}

void IgExpectNotEqualsForTesting(const blink::InterestGroup& actual, const blink::InterestGroup& not_expected)
{
