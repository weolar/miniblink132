...
void SerializeAndDeserializeAndCompare(InterestGroup& interest_group)
{
    ...
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<blink::mojom::InterestGroup>(interest_group, interest_group_clone));
...
void SerializeAndDeserializeExpectFailure(InterestGroup& interest_group, std::string_view tag = "")
{
    IgExpectNotEqualsForTesting(/*actual=*/interest_group,
        /*not_expected=*/CreateInterestGroup());
    ASSERT_FALSE(testing::Test::HasFailure());
    ...
    InterestGroup interest_group_clone;
    EXPECT_FALSE(mojo::test::SerializeAndDeserialize<blink::mojom::InterestGroup>(interest_group, interest_group_clone)) << tag;
}
...
TEST(InterestGroupMojomTraitsTest, SerializeAndDeserializeEnableBiddingSignalsPrioritization)
{
    InterestGroup interest_group = CreateInterestGroup();
...
TEST(InterestGroupMojomTraitsTest, SerializeAndDeserializePriorityVector)
{
    ...
    interest_group.priority_vector = { { { "signals1", 1 }, { "signals2", 3 }, { "signals3", -5 } } };
    SerializeAndDeserializeAndCompare(interest_group);
}

TEST(InterestGroupMojomTraitsTest, SerializeAndDeserializePrioritySignalsOverride)
{
    InterestGroup interest_group = CreateInterestGroup();
    ...
    interest_group.priority_signals_overrides = { { { "signals1", 1 }, { "signals2", 3 }, { "signals3", -5 } } };
    SerializeAndDeserializeAndCompare(interest_group);
}
...
TEST(InterestGroupMojomTraitsTest, SerializeAndDeserializeNonFinite)
{
    ...
    for (double test_case : test_cases) {
        ...
        SerializeAndDeserializeExpectFailure(interest_group_bad_priority, "priority");
        ...
        SerializeAndDeserializeExpectFailure(interest_group_bad_priority_vector, "priority_vector");

        InterestGroup blink_interest_group_bad_priority_signals_overrides = CreateInterestGroup();
        blink_interest_group_bad_priority_signals_overrides.priority_signals_overrides = { { "foo", test_case } };
        SerializeAndDeserializeExpectFailure(blink_interest_group_bad_priority_signals_overrides, "priority_signals_overrides");
    }
}
...
TEST(InterestGroupMojomTraitsTest, SerializeAndDeserializeSellerCapabilities)
{
    ...
    interest_group.seller_capabilities = { { { url::Origin::Create(GURL(kOrigin1)), {} } } };
    SerializeAndDeserializeAndCompare(interest_group);

    interest_group.seller_capabilities = { { { url::Origin::Create(GURL(kOrigin1)), {} }, { url::Origin::Create(GURL(kOrigin2)), {} } } };
    SerializeAndDeserializeAndCompare(interest_group);
}

TEST(InterestGroupMojomTraitsTest, SerializeAndDeserializeAllSellersCapabilities)
{
    InterestGroup interest_group = CreateInterestGroup();

    interest_group.all_sellers_capabilities.Put(SellerCapabilities::kInterestGroupCounts);
    SerializeAndDeserializeAndCompare(interest_group);

    interest_group.all_sellers_capabilities.Put(SellerCapabilities::kLatencyStats);
    SerializeAndDeserializeAndCompare(interest_group);
...
TEST(InterestGroupMojomTraitsTest, SerializeAndDeserializeTrustedBiddingSignalsUrl)
{
    InterestGroup interest_group = CreateInterestGroup();
...
TEST(InterestGroupMojomTraitsTest, SerializeAndDeserializeCrossOriginTrustedBiddingSignalsUrl)
{
    InterestGroup interest_group = CreateInterestGroup();
    interest_group.trusted_bidding_signals_url = GURL("https://cross-origin.test/");
    SerializeAndDeserializeAndCompare(interest_group);
}

TEST(InterestGroupMojomTraitsTest, SerializeAndDeserializeTrustedBiddingSignalsKeys)
{
    InterestGroup interest_group = CreateInterestGroup();
...
TEST(InterestGroupMojomTraitsTest, SerializeAndDeserializeTrustedBiddingSignalsSlotSizeMode)
{
    InterestGroup interest_group = CreateInterestGroup();
    interest_group.trusted_bidding_signals_slot_size_mode = InterestGroup::TrustedBiddingSignalsSlotSizeMode::kSlotSize;
    SerializeAndDeserializeAndCompare(interest_group);
    interest_group.trusted_bidding_signals_slot_size_mode = InterestGroup::TrustedBiddingSignalsSlotSizeMode::kAllSlotsRequestedSizes;
    SerializeAndDeserializeAndCompare(interest_group);
}

TEST(InterestGroupMojomTraitsTest, SerializeAndDeserializeMaxTrustedBiddingSignalsURLLength)
{
    InterestGroup interest_group = CreateInterestGroup();
...
TEST(InterestGroupMojomTraitsTest, SerializeAndDeserializeTrustedBiddingSignalsCoordinator)
{
    InterestGroup interest_group = CreateInterestGroup();
    interest_group.trusted_bidding_signals_coordinator = url::Origin::Create(GURL("https://example.test"));
    SerializeAndDeserializeAndCompare(interest_group);
}

TEST(InterestGroupMojomTraitsTest, SerializeAndDeserializeInvalidTrustedBiddingSignalsCoordinator)
{
    InterestGroup interest_group = CreateInterestGroup();
    interest_group.trusted_bidding_signals_coordinator = url::Origin::Create(GURL("http://example.test"));
    SerializeAndDeserializeExpectFailure(interest_group, "trustedBiddingSignalsCoordinator");
}
...
TEST(InterestGroupMojomTraitsTest, SerializeAndDeserializeAdsWithReportingIds)
{
    ...
    (*interest_group.ads)[0].selectable_buyer_and_seller_reporting_ids = { "selectable_id1", "selectable_id2" };
    interest_group.ads->emplace_back(GURL(kUrl2),
        /*metadata=*/"[]",
        /*size_group=*/std::nullopt);
    (*interest_group.ads)[1].buyer_reporting_id = "buyer_id_2";
    (*interest_group.ads)[1].buyer_and_seller_reporting_id = "both_id_2";
    (*interest_group.ads)[1].selectable_buyer_and_seller_reporting_ids = { "selectable_id3", "selectable_id4" };
...
TEST(InterestGroupMojomTraitsTest, AdComponentsWithBuyerAndSellerReportingIdInvalid)
{
    InterestGroup interest_group = CreateInterestGroup();
    interest_group.ad_components.emplace();
    interest_group.ad_components->emplace_back(GURL(kUrl1),
        /*metadata=*/std::nullopt,
        /*size_group=*/std::nullopt);
    (*interest_group.ad_components)[0].buyer_and_seller_reporting_id = "both_id_1";
    EXPECT_FALSE(interest_group.IsValid());
}

TEST(InterestGroupMojomTraitsTest, AdComponentsWithSelectableReportingIdInvalid)
{
    InterestGroup interest_group = CreateInterestGroup();
    interest_group.ad_components.emplace();
    interest_group.ad_components->emplace_back(GURL(kUrl1),
        /*metadata=*/std::nullopt,
        /*size_group=*/std::nullopt);
    (*interest_group.ad_components)[0].selectable_buyer_and_seller_reporting_ids = { "selectable_id1", "selectable_id2" };
    EXPECT_FALSE(interest_group.IsValid());
}
...
TEST(InterestGroupMojomTraitsTest, SerializeAndDeserializeAdsWithSizeGroups)
{
    ...
    interest_group.ad_sizes->emplace("size_1", blink::AdSize(300, blink::AdSize::LengthUnit::kPixels, 150, blink::AdSize::LengthUnit::kPixels));
    interest_group.ad_sizes->emplace("size_2", blink::AdSize(640, blink::AdSize::LengthUnit::kPixels, 480, blink::AdSize::LengthUnit::kPixels));
...
TEST(InterestGroupMojomTraitsTest, SerializeAndDeserializeAdsWithAllowedReportingOrigins)
{
    InterestGroup interest_group = CreateInterestGroup();
    interest_group.ads.emplace();
    std::vector<url::Origin> allowed_reporting_origins_1 = { url::Origin::Create(GURL(kOrigin1)) };
    std::vector<url::Origin> allowed_reporting_origins_2 = { url::Origin::Create(GURL(kOrigin2)) };
    interest_group.ads->emplace_back(GURL(kUrl1),
        /*metadata=*/std::nullopt,
        /*size_group=*/std::nullopt,
        /*buyer_reporting_id=*/std::nullopt,
...
TEST(InterestGroupMojomTraitsTest, SerializeAndDeserializeAdComponentsWithSize)
{
    InterestGroup interest_group = CreateInterestGroup();
    // All three of the following mappings must be valid in order for the
...
TEST(InterestGroupMojomTraitsTest, SerializeAndDeserializeAdComponentsWithAdRenderId)
{
    InterestGroup interest_group = CreateInterestGroup();
    interest_group.ad_components.emplace();
    interest_group.ad_components->emplace_back(GURL(kUrl1),
...
TEST(InterestGroupMojomTraitsTest, SerializeAndDeserializeAuctionServerRequestFlags)
{
    InterestGroup interest_group = CreateInterestGroup();

    interest_group.auction_server_request_flags = { blink::AuctionServerRequestFlagsEnum::kIncludeFullAds };
    SerializeAndDeserializeAndCompare(interest_group);

    interest_group.auction_server_request_flags = { blink::AuctionServerRequestFlagsEnum::kOmitAds };
    SerializeAndDeserializeAndCompare(interest_group);

    interest_group.auction_server_request_flags = { blink::AuctionServerRequestFlagsEnum::kOmitUserBiddingSignals };
    SerializeAndDeserializeAndCompare(interest_group);

    interest_group.auction_server_request_flags = { blink::AuctionServerRequestFlagsEnum::kOmitAds, blink::AuctionServerRequestFlagsEnum::kIncludeFullAds,
...
{
    constexpr blink::InterestGroup::AdditionalBidKey kAdditionalBidKey = { 0x7d, 0x4d, 0x0e, 0x7f, 0x61, 0x53, 0xa6, 0x9b, 0x62, 0x42, 0xb5, 0x22, 0xab, 0xbe,
        0xe6, 0x85, 0xfd, 0xa4, 0x42, 0x0f, 0x88, 0x34, 0xb1, 0x08, 0xc3, 0xbd, 0xae, 0x36, 0x9e, 0xf5, 0x49, 0xfa };
    InterestGroup interest_group = CreateInterestGroup();
    interest_group.additional_bid_key = kAdditionalBidKey;
    SerializeAndDeserializeAndCompare(interest_group);
}

TEST(InterestGroupMojomTraitsTest, SerializeAndDeserializeAggregationCoordinatorOrigin)
{
    InterestGroup interest_group = CreateInterestGroup();
    interest_group.aggregation_coordinator_origin = url::Origin::Create(GURL("https://example.com"));
    SerializeAndDeserializeAndCompare(interest_group);
}
