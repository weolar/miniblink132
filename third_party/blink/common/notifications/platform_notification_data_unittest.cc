...
TEST(PlatformNotificationDataTest, AssignmentOperator)
{
    ...
    notification_data.vibration_pattern.assign(std::begin(vibration_pattern), std::end(vibration_pattern));
    ...
    EXPECT_EQ(assigned_notification_data.vibration_pattern, notification_data.vibration_pattern);
    ...
    EXPECT_EQ(assigned_notification_data.require_interaction, notification_data.require_interaction);
    EXPECT_EQ(assigned_notification_data.data, notification_data.data);
    ASSERT_EQ(notification_data.actions.size(), assigned_notification_data.actions.size());
    for (size_t i = 0; i < notification_data.actions.size(); ++i) {
        SCOPED_TRACE(base::StringPrintf("Action index: %zd", i));
        EXPECT_EQ(notification_data.actions[i]->type, assigned_notification_data.actions[i]->type);
        EXPECT_EQ(notification_data.actions[i]->action, assigned_notification_data.actions[i]->action);
        EXPECT_EQ(notification_data.actions[i]->title, assigned_notification_data.actions[i]->title);
        EXPECT_EQ(notification_data.actions[i]->icon, assigned_notification_data.actions[i]->icon);
        EXPECT_EQ(notification_data.actions[i]->placeholder, assigned_notification_data.actions[i]->placeholder);
    }
    EXPECT_EQ(assigned_notification_data.show_trigger_timestamp, notification_data.show_trigger_timestamp);
