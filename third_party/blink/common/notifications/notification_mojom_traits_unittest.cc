...
{
    return lhs.width() == rhs.width() && lhs.height() == rhs.height() && lhs.getColor(0, 0) == rhs.getColor(0, 0);
}
...
TEST(NotificationStructTraitsTest, NotificationDataRoundtrip)
{
    ...
    notification_data.vibration_pattern.assign(vibration_pattern, vibration_pattern + std::size(vibration_pattern));

    notification_data.timestamp = base::Time::FromMillisecondsSinceUnixEpoch(1513966159000.);
    ...
    notification_data.actions[0]->type = blink::mojom::NotificationActionType::BUTTON;
    ...
    notification_data.actions[1]->type = blink::mojom::NotificationActionType::TEXT;
    ...
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<blink::mojom::NotificationData>(notification_data, roundtrip_notification_data));
    ...
    EXPECT_EQ(roundtrip_notification_data.vibration_pattern, notification_data.vibration_pattern);
    ...
    EXPECT_EQ(roundtrip_notification_data.require_interaction, notification_data.require_interaction);
    EXPECT_EQ(roundtrip_notification_data.data, notification_data.data);
    ASSERT_EQ(notification_data.actions.size(), roundtrip_notification_data.actions.size());
    for (size_t i = 0; i < notification_data.actions.size(); ++i) {
        SCOPED_TRACE(base::StringPrintf("Action index: %zd", i));
        EXPECT_EQ(notification_data.actions[i]->type, roundtrip_notification_data.actions[i]->type);
        EXPECT_EQ(notification_data.actions[i]->action, roundtrip_notification_data.actions[i]->action);
        EXPECT_EQ(notification_data.actions[i]->title, roundtrip_notification_data.actions[i]->title);
        EXPECT_EQ(notification_data.actions[i]->icon, roundtrip_notification_data.actions[i]->icon);
        EXPECT_EQ(notification_data.actions[i]->placeholder, roundtrip_notification_data.actions[i]->placeholder);
    }
    EXPECT_EQ(roundtrip_notification_data.show_trigger_timestamp, notification_data.show_trigger_timestamp);
...
TEST(NotificationStructTraitsTest, ValidVibrationPattern)
{
    ...
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<blink::mojom::NotificationData>(notification_data, platform_notification_data));
}
...
TEST(NotificationStructTraitsTest, TooManyVibrations)
{
    ...
    ASSERT_FALSE(mojo::test::SerializeAndDeserialize<blink::mojom::NotificationData>(notification_data, platform_notification_data));
}
...
TEST(NotificationStructTraitsTest, NotificationResourcesRoundtrip)
{
    ...
    resources.action_icons[0] = gfx::test::CreateBitmap(/*size=*/10, SK_ColorLTGRAY);
    resources.action_icons[1] = gfx::test::CreateBitmap(/*size=*/11, SK_ColorDKGRAY);

    NotificationResources roundtrip_resources;

    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<blink::mojom::NotificationResources>(resources, roundtrip_resources));

    ASSERT_FALSE(roundtrip_resources.image.empty());
    EXPECT_TRUE(ImagesShareDimensionsAndColor(resources.image, roundtrip_resources.image));

    ASSERT_FALSE(roundtrip_resources.notification_icon.empty());
    EXPECT_TRUE(ImagesShareDimensionsAndColor(resources.notification_icon, roundtrip_resources.notification_icon));

    ASSERT_FALSE(roundtrip_resources.badge.empty());
    EXPECT_TRUE(ImagesShareDimensionsAndColor(resources.badge, roundtrip_resources.badge));

    ASSERT_EQ(resources.action_icons.size(), roundtrip_resources.action_icons.size());
    ...
    for (size_t i = 0; i < roundtrip_resources.action_icons.size(); ++i) {
        ...
        EXPECT_TRUE(ImagesShareDimensionsAndColor(resources.action_icons[i], roundtrip_resources.action_icons[i]));
    }
}
