...
bool StructTraits<blink::mojom::FrameAdEvidenceDataView, blink::FrameAdEvidence>::Read(blink::mojom::FrameAdEvidenceDataView data, blink::FrameAdEvidence* out)
{
    *out = blink::FrameAdEvidence(data.parent_is_ad());
    ...
    if (!data.ReadMostRestrictiveFilterListResult(&most_restrictive_filter_list_result))
        return false;
    ...
    if (!data.ReadLatestFilterListResult(&latest_filter_list_result))
        return false;
    if (most_restrictive_filter_list_result != blink::MoreRestrictiveFilterListEvidence(latest_filter_list_result, most_restrictive_filter_list_result)) {
        return false;
    }
