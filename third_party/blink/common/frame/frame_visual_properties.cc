...
FrameVisualProperties::FrameVisualProperties(const FrameVisualProperties& other) = default;

FrameVisualProperties::~FrameVisualProperties() = default;

FrameVisualProperties& FrameVisualProperties::operator=(const FrameVisualProperties& other) = default;

double FrameVisualProperties::MaxChildFrameScreenRectMovement()
{
    if (!max_child_frame_screen_rect_movement.has_value()) {
        max_child_frame_screen_rect_movement.emplace(base::GetFieldTrialParamByFeatureAsDouble(
            features::kDiscardInputEventsToRecentlyMovedFrames, "distance_factor", std::numeric_limits<double>::max()));
    }
...
int FrameVisualProperties::MinScreenRectStableTimeMs()
{
    if (!min_screen_rect_stable_time_ms.has_value()) {
        min_screen_rect_stable_time_ms.emplace(base::GetFieldTrialParamByFeatureAsInt(features::kDiscardInputEventsToRecentlyMovedFrames, "time_ms", 0));
    }
