...
void ToFilePathVector(const std::vector<std::optional<std::u16string>>& input, std::vector<base::FilePath>* output)
{
    output->clear();
...
void RecursivelyRemoveReferrer(ExplodedFrameState* state)
{
    ...
    for (std::vector<ExplodedFrameState>::iterator it = state->children.begin(); it != state->children.end(); ++it) {
...
PageState PageState::CreateForTesting(
    const GURL& url, bool body_contains_password_data, const char* optional_body_data, const base::FilePath* optional_body_file_path)
{
    ExplodedPageState state;
    ...
    if (optional_body_data || optional_body_file_path) {
        if (optional_body_data) {
            ...
            state.top.http_body.request_body->AppendBytes(body_data.data(), body_data.size());
        }
        if (optional_body_file_path) {
            state.top.http_body.request_body = new network::ResourceRequestBody();
            state.top.http_body.request_body->AppendFileRange(*optional_body_file_path, 0, std::numeric_limits<uint64_t>::max(), base::Time());
            state.referenced_files.emplace_back(optional_body_file_path->AsUTF16Unsafe());
        }
...
// static
PageState PageState::CreateForTestingWithSequenceNumbers(const GURL& url, int64_t item_sequence_number, int64_t document_sequence_number)
{
