...
base::FilePath GetFilePath()
{
    ...
    return base::MakeAbsoluteFilePath(path.Append(FILE_PATH_LITERAL("third_party/blink/common/page_state/test_data")));
}

//-----------------------------------------------------------------------------

template <typename T> void ExpectEquality(const T& expected, const T& actual)
{
    EXPECT_EQ(expected, actual);
}

template <typename T> void ExpectEquality(const std::vector<T>& expected, const std::vector<T>& actual)
{
...
template <> void ExpectEquality(const network::DataElement& expected, const network::DataElement& actual)
{
    ASSERT_EQ(expected.type(), actual.type());
    if (expected.type() == network::DataElement::Tag::kBytes) {
        EXPECT_EQ(expected.As<network::DataElementBytes>().bytes(), actual.As<network::DataElementBytes>().bytes());
        return;
    }
    ...
    EXPECT_EQ(expected_file.expected_modification_time(), actual_file.expected_modification_time());
}

template <> void ExpectEquality(const ExplodedHttpBody& expected, const ExplodedHttpBody& actual)
{
    ...
    if (!expected.request_body || !actual.request_body) {
        ...
        EXPECT_EQ(expected.request_body->identifier(), actual.request_body->identifier());
        ExpectEquality(*expected.request_body->elements(), *actual.request_body->elements());
    }
}

template <> void ExpectEquality(const ExplodedFrameState& expected, const ExplodedFrameState& actual)
{
    ...
    EXPECT_EQ(expected.initiator_base_url_string, actual.initiator_base_url_string);
    ...
    EXPECT_EQ(expected.visual_viewport_scroll_offset, actual.visual_viewport_scroll_offset);
    ...
    EXPECT_EQ(expected.protect_url_in_navigation_api, actual.protect_url_in_navigation_api);
    ExpectEquality(expected.http_body, actual.http_body);
    ExpectEquality(expected.children, actual.children);
}

void ExpectEquality(const ExplodedPageState& expected, const ExplodedPageState& actual)
{
...
public:
    void PopulateFrameState(ExplodedFrameState* frame_state)
    {
        ...
        frame_state->scroll_restoration_type = mojom::ScrollRestorationType::kManual;
        ...
        frame_state->initiator_origin = url::Origin::Create(GURL("https://initiator.example.com"));
        ...
        frame_state->initiator_base_url_string = base::UTF8ToUTF16(frame_state->initiator_origin->GetURL().spec());
    }

    void PopulateHttpBody(ExplodedHttpBody* http_body, std::vector<std::optional<std::u16string>>* referenced_files)
    {
        ...
        http_body->request_body->AppendFileRange(base::FilePath(path), 100, 1024, base::Time::FromSecondsSinceUnixEpoch(9999.0));

        referenced_files->emplace_back(path.AsUTF16Unsafe());
    }

    void PopulateFrameStateForBackwardsCompatTest(ExplodedFrameState* frame_state, bool is_child, int version)
    {
        if (version < 28) {
        ...
        frame_state->document_state.push_back(u"\n\r?% WebKit serialized form state version 8 \n\r=&");
        ...
        if (version >= 33) {
            frame_state->initiator_base_url_string = base::UTF8ToUTF16(GURL("https://initiator.example.com").spec());
        }
        ...
        if (!is_child) {
            ...
            frame_state->http_body.request_body->AppendBytes(test_body.data(), test_body.size());

            frame_state->http_body.request_body->AppendFileRange(
                base::FilePath(FILE_PATH_LITERAL("file.txt")), 0, std::numeric_limits<uint64_t>::max(), base::Time::FromSecondsSinceUnixEpoch(0.0));

            std::string test_body2("data the second");
            frame_state->http_body.request_body->AppendBytes(test_body2.data(), test_body2.size());
    ...
    void PopulatePageStateForBackwardsCompatTest(ExplodedPageState* page_state, int version)
    {
        page_state->referenced_files.push_back(u"file.txt");
        PopulateFrameStateForBackwardsCompatTest(&page_state->top, false, version);
    }

    void ReadBackwardsCompatPageState(const std::string& suffix, int version, ExplodedPageState* page_state)
    {
        base::FilePath path = GetFilePath();
        path = path.AppendASCII(base::StringPrintf("serialized_%s.dat", suffix.c_str()));
        ...
        // encoded PageState.
        EXPECT_TRUE(base::Base64Decode(trimmed_file_contents, &saved_encoded_state));
        ...
        EXPECT_TRUE(DecodePageStateWithDeviceScaleFactorForTesting(saved_encoded_state, kPresetDeviceScaleFactor, page_state));
#else
        EXPECT_EQ(version, DecodePageStateForTesting(saved_encoded_state, page_state));
#endif
    }
...
TEST_F(PageStateSerializationTest, InitiatorOriginAssign)
{
    ...
    a.initiator_origin = url::Origin::Create(GURL("https://initiator.example.com"));
    b = a;
    ExpectEquality(a, b);
...
TEST_F(PageStateSerializationTest, LegacyEncodePageStateFrozen)
{
    ...
    std::string file_contents;
    ASSERT_TRUE(base::ReadFileToString(path, &file_contents)) << "File not found: " << path.value();

    std::string trimmed_file_contents;
    EXPECT_TRUE(base::RemoveChars(file_contents, "\n", &trimmed_file_contents));

    std::string expected_encoded_state;
    EXPECT_TRUE(base::Base64Decode(trimmed_file_contents, &expected_encoded_state));
...
TEST_F(PageStateSerializationTest, ScrollAnchorSelectorLengthLimited)
{
    ...
    input.top.scroll_anchor_selector = std::u16string(kMaxScrollAnchorSelectorLength + 1, u'a');
...
{
    ExplodedPageState state;
    state.top.document_state.push_back(u"\n\r?% WebKit serialized form state version 8 \n\r=&");
...
TEST_F(PageStateSerializationTest, BackwardsCompat_VisualViewportScrollOffset)
{
    ...
    ExplodedPageState saved_state;
    ReadBackwardsCompatPageState("visual_viewport_scroll_offset", 26, &saved_state);
    ExpectEquality(state, saved_state);
}
