...
void AppendDataToRequestBody(const scoped_refptr<network::ResourceRequestBody>& request_body, const char* data, size_t data_length)
{
    request_body->AppendBytes(data, data_length);
}

void AppendFileRangeToRequestBody(const scoped_refptr<network::ResourceRequestBody>& request_body, const std::optional<std::u16string>& file_path,
    int file_start, int file_length, base::Time file_modification_time)
{
    request_body->AppendFileRange(file_path ? base::FilePath::FromUTF16Unsafe(*file_path) : base::FilePath(), static_cast<uint64_t>(file_start),
        static_cast<uint64_t>(file_length), file_modification_time);
}

//----------------------------------------------------------------------------

void AppendReferencedFilesFromHttpBody(const std::vector<network::DataElement>& elements, std::vector<std::optional<std::u16string>>* referenced_files)
{
    for (size_t i = 0; i < elements.size(); ++i) {
        if (elements[i].type() == network::DataElement::Tag::kFile) {
            referenced_files->emplace_back(elements[i].As<network::DataElementFile>().path().AsUTF16Unsafe());
        }
    }
}

bool AppendReferencedFilesFromDocumentState(
    const std::vector<std::optional<std::u16string>>& document_state, std::vector<std::optional<std::u16string>>* referenced_files)
{
    ...
    size_t item_count;
    if (!document_state[index] || !base::StringToSizeT(*document_state[index++], &item_count))
        return false;
    ...
    while (item_count--) {
        ...
        size_t value_size;
        if (!document_state[index] || !base::StringToSizeT(*document_state[index++], &value_size))
            return false;
...
bool RecursivelyAppendReferencedFiles(const ExplodedFrameState& frame_state, std::vector<std::optional<std::u16string>>* referenced_files)
{
    if (frame_state.http_body.request_body) {
        AppendReferencedFilesFromHttpBody(*frame_state.http_body.request_body->elements(), referenced_files);
    }

    if (!AppendReferencedFilesFromDocumentState(frame_state.document_state, referenced_files))
        return false;

    for (size_t i = 0; i < frame_state.children.size(); ++i) {
        if (!RecursivelyAppendReferencedFiles(frame_state.children[i], referenced_files))
            return false;
    }
...
double ReadReal(SerializeObject* obj)
{
    ...
    if (data && data->size() == sizeof(double)) {
        double value;
        base::byte_span_from_ref(value).copy_from(data.value().first<sizeof(double)>());
        return value;
    }
...
void WriteString(const std::u16string& str, SerializeObject* obj)
{
    ...
    obj->pickle.WriteData((const char*)(str.c_str()), str.size() * sizeof(char16_t));
}

// If str is a null optional, this simply pickles a length of -1. Otherwise,
// delegates to the std::u16string overload.
void WriteString(const std::optional<std::u16string>& str, SerializeObject* obj)
{
    if (!str) {
...
template <typename T> void WriteAndValidateVectorSize(const std::vector<T>& v, SerializeObject* obj)
{
...
void WriteStringVector(const std::vector<std::optional<std::u16string>>& data, SerializeObject* obj)
{
...
void ReadStringVector(SerializeObject* obj, std::vector<std::optional<std::u16string>>* result)
{
    size_t num_elements = ReadAndValidateVectorSize(obj, sizeof(std::optional<std::u16string>));
...
void WriteResourceRequestBody(const network::ResourceRequestBody& request_body, SerializeObject* obj)
{
    ...
    for (const auto& element : *request_body.elements()) {
        ...
        case network::DataElement::Tag::kBytes: {
            ...
            WriteData(base::span<const uint8_t>(bytes.data(), bytes.size()), obj);
            break;
        }
        case network::DataElement::Tag::kFile: {
            ...
            WriteReal(file.expected_modification_time().InSecondsFSinceUnixEpoch(), obj);
            break;
        }
        default:
            NOTREACHED();
...
void ReadResourceRequestBody(SerializeObject* obj, const scoped_refptr<network::ResourceRequestBody>& request_body)
{
    int num_elements = ReadInteger(obj);
    for (int i = 0; i < num_elements; ++i) {
        HTTPBodyElementType type = static_cast<HTTPBodyElementType>(ReadInteger(obj));
        if (type == HTTPBodyElementType::kTypeData) {
            std::optional<base::span<const uint8_t>> data = ReadData(obj);
            if (data) {
                AppendDataToRequestBody(request_body, reinterpret_cast<const char*>(data->data()), data->size());
            }
            ...
            AppendFileRangeToRequestBody(request_body, file_path, file_start, file_length, base::Time::FromSecondsSinceUnixEpoch(file_modification_time));
...
void ReadLegacyFrameState(SerializeObject* obj, bool is_top, std::vector<UniqueNameHelper::Replacement>* unique_name_replacements, ExplodedFrameState* state)
{
    ...
    if (obj->version < 25 && state->target) {
        state->target = base::UTF8ToUTF16(UniqueNameHelper::UpdateLegacyNameFromV24(base::UTF16ToUTF8(*state->target), unique_name_replacements));
    }
    if (obj->version < 15) {
    ...
    if (obj->version >= 18) {
        state->referrer_policy
            = mojo::ConvertIntToMojoEnum<network::mojom::ReferrerPolicy>(ReadInteger(obj)).value_or(network::mojom::ReferrerPolicy::kDefault);
    }
    ...
    if (obj->version >= 22) {
        state->scroll_restoration_type = static_cast<mojom::ScrollRestorationType>(ReadInteger(obj));
    }
    ...
    if (obj->version == 11) {
        ...
        if (state->page_scale_factor) {
            ...
            if (!device_scale_factor) {
                device_scale_factor = display::Screen::GetScreen()->GetPrimaryDisplay().device_scale_factor();
            }
            state->scroll_offset = gfx::Point(state->scroll_offset.x() / state->page_scale_factor, state->scroll_offset.y() / state->page_scale_factor);
    ...
    for (size_t i = 0; i < num_children; ++i) {
        ReadLegacyFrameState(obj, false, unique_name_replacements, &state->children[i]);
    }
}
...
void WriteLegacyFrameState(const ExplodedFrameState& state, SerializeObject* obj, bool is_top)
{
...
void WriteLegacyPageState(const ExplodedPageState& state, SerializeObject* obj)
{
...
void WriteResourceRequestBody(const network::ResourceRequestBody& request_body, mojom::RequestBody* mojo_body)
{
    for (const auto& element : *request_body.elements()) {
        ...
        case network::DataElement::Tag::kFile: {
            ...
            mojom::FilePtr file = mojom::File::New(
                element_file.path().AsUTF16Unsafe(), element_file.offset(), element_file.length(), element_file.expected_modification_time());
...
void ReadResourceRequestBody(mojom::RequestBody* mojo_body, const scoped_refptr<network::ResourceRequestBody>& request_body)
{
    for (const auto& element : mojo_body->elements) {
        mojom::Element::Tag tag = element->which();
        switch (tag) {
        case mojom::Element::Tag::kBytes:
            AppendDataToRequestBody(request_body, reinterpret_cast<const char*>(element->get_bytes().data()), element->get_bytes().size());
            break;
        case mojom::Element::Tag::kFile: {
            mojom::File* file = element->get_file().get();
            AppendFileRangeToRequestBody(request_body, file->path, file->offset, file->length, file->modification_time);
            break;
        }
...
void WriteHttpBody(const ExplodedHttpBody& http_body, mojom::HttpBody* mojo_body)
{
    if (http_body.request_body) {
        ...
        WriteResourceRequestBody(*http_body.request_body, mojo_body->request_body.get());
    }
}
...
void ReadHttpBody(mojom::HttpBody* mojo_body, ExplodedHttpBody* http_body)
{
    ...
    if (mojo_body->request_body) {
        http_body->request_body = base::MakeRefCounted<network::ResourceRequestBody>();
        ReadResourceRequestBody(mojo_body->request_body.get(), http_body->request_body);
    }
}
...
// or not.
void WriteMojoFrameState(const ExplodedFrameState& state, mojom::FrameState* frame)
{
    ...
    frame->scroll_restoration_type = static_cast<mojom::ScrollRestorationType>(state.scroll_restoration_type);

    if (state.did_save_scroll_or_scale_state) {
        frame->view_state = mojom::ViewState::New();
        frame->view_state->scroll_offset = state.scroll_offset;
        frame->view_state->visual_viewport_scroll_offset = state.visual_viewport_scroll_offset;
        ...
        if (state.scroll_anchor_selector && state.scroll_anchor_selector->length() < kMaxScrollAnchorSelectorLength) {
...
void ReadMojoFrameState(mojom::FrameState* frame, ExplodedFrameState* state)
{
    ...
    if (frame->initiator_origin.has_value()) {
        state->initiator_origin = url::Origin::Create(GURL(frame->initiator_origin.value()));
    }
    ...
    state->scroll_restoration_type = static_cast<mojom::ScrollRestorationType>(frame->scroll_restoration_type);

    if (frame->view_state) {
        state->did_save_scroll_or_scale_state = true;
        state->visual_viewport_scroll_offset = frame->view_state->visual_viewport_scroll_offset;
    ...
    if (frame->view_state) {
        ...
        state->scroll_anchor_offset = frame->view_state->scroll_anchor_offset.value_or(gfx::PointF());
...
void ReadMojoPageState(SerializeObject* obj, ExplodedPageState* state)
{
    ...
    obj->parse_error = !(mojom::PageState::Deserialize(data->data(), data->size(), &page));
    if (obj->parse_error) {
        return;
    }
    ...
    state->referenced_files.erase(std::unique(state->referenced_files.begin(), state->referenced_files.end()), state->referenced_files.end());
}
...
void WriteMojoPageState(const ExplodedPageState& state, SerializeObject* obj)
{
    ...
    obj->pickle.WriteData((const char*)page_bytes.data(), page_bytes.size());
}
...
int DecodePageStateInternal(const std::string& encoded, ExplodedPageState* exploded)
{
...
int DecodePageStateForTesting(const std::string& encoded, ExplodedPageState* exploded)
{
...
void LegacyEncodePageStateForTesting(const ExplodedPageState& exploded, int version, std::string* encoded)
{
    SerializeObject obj;
    obj.version = version;
...
#if BUILDFLAG(IS_ANDROID)
bool DecodePageStateWithDeviceScaleFactorForTesting(const std::string& encoded, float device_scale_factor, ExplodedPageState* exploded)
{
...
scoped_refptr<network::ResourceRequestBody> DecodeResourceRequestBody(base::span<const uint8_t> data)
{
    scoped_refptr<network::ResourceRequestBody> result = new network::ResourceRequestBody();
...
std::string EncodeResourceRequestBody(const network::ResourceRequestBody& resource_request_body)
{
    SerializeObject obj;
    obj.version = 25;
