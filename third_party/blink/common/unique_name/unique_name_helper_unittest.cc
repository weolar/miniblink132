...
public:
    ...
    // |parent_->children_|.
    explicit TestFrameAdapter(TestFrameAdapter* parent, int virtual_index_in_parent, const std::string& requested_name)
        : parent_(parent)
    ...
    {
        return virtual_index_in_parent_;
    }

    int GetChildCount() const override
    {
        ADD_FAILURE() << "GetChildCount() should not be triggered by unit test code!";
        return 0;
    }

    std::vector<std::string> CollectAncestorNames(BeginPoint begin_point, bool (*should_stop)(std::string_view)) const override
    {
    ...
    {
        EXPECT_EQ(GetUniqueName(), base::UTF16ToUTF8(frame_state.target.value_or(std::u16string())));
...
private:
    ...
    void CalculateLegacyName(const std::string& requested_name)
    {
        ...
        if (!IsMainFrame()) {
            ...
            legacy_name_ = UniqueNameHelper::CalculateLegacyNameForTesting(this, requested_name);
        }
    }
...
TEST(UniqueNameHelper, Basic)
{
    ...
    EXPECT_EQ("<!--framePath //<!--frame2-->--><!--framePosition-2/0-->", frame_2a.GetUniqueName());
    EXPECT_EQ("<!--framePath //<!--frame2-->--><!--framePosition-2/0-->", frame_2a.GetLegacyName());
    ...
    EXPECT_EQ("<!--framePath //<!--frame2-->--><!--framePosition-2/0/<!--frame5-->-->", frame_2a_5.GetUniqueName());
    EXPECT_EQ("<!--framePath //<!--frame2-->--><!--framePosition-2/0/<!--frame5-->-->", frame_2a_5.GetLegacyName());
    ...
    EXPECT_EQ("<!--framePath //<!--frame2-->--><!--framePosition-2/1-->", frame_2b.GetUniqueName());
    EXPECT_EQ("<!--framePath //<!--frame2-->--><!--framePosition-2/1-->", frame_2b.GetLegacyName());
...
TEST(UniqueNameHelper, BasicGeneratedFramePath)
{
    ...
    EXPECT_EQ("<!--framePath //<!--frame2-->/named grandchild/<!--frame5-->-->", frame_2_3_5.GetUniqueName());
    EXPECT_EQ("<!--framePath //<!--frame2-->/named grandchild/<!--frame5-->-->", frame_2_3_5.GetLegacyName());
...
TEST(UniqueNameHelper, GeneratedFramePathHashing)
{
    ...
    EXPECT_EQ(
        "<!--framePath //<!--frame0-->/" + too_long_name + "/" + too_long_name3 + "/" + too_long_name4 + "/<!--frame0-->-->", frame_0_1_1_0_0.GetLegacyName());
