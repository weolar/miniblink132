...
public:
    explicit PendingChildFrameAdapter(FrameAdapter* parent)
        : parent_(parent)
    {
    }
...
{
    return name.starts_with(kFramePathPrefix) && name.ends_with("-->") && (kFramePathPrefixLength + kFramePathSuffixLength) < name.size();
}

std::string GenerateCandidate(const FrameAdapter* frame)
{
    std::string new_name(kFramePathPrefix);
    std::vector<std::string> ancestor_names = frame->CollectAncestorNames(FrameAdapter::BeginPoint::kParentFrame, &IsNameWithFramePath);
    ...
    if (!ancestor_names.empty() && IsNameWithFramePath(ancestor_names[0])) {
        ancestor_names[0] = ancestor_names[0].substr(kFramePathPrefixLength, ancestor_names[0].size() - kFramePathPrefixLength - kFramePathSuffixLength);
    }
    new_name += base::JoinString(base::span<const std::string>(ancestor_names.data(), ancestor_names.size()), "/");
...
std::string GenerateFramePosition(const FrameAdapter* frame)
{
    ...
    std::vector<int> positions = frame->GetFramePosition(FrameAdapter::BeginPoint::kParentFrame);
...
std::string AppendUniqueSuffix(const FrameAdapter* frame, const std::string& prefix, const std::string& likely_unique_suffix)
{
...
std::string CalculateNameInternal(const FrameAdapter* frame, std::string_view name)
{
...
UniqueNameHelper::Replacement::Replacement(std::string old_name, std::string new_name)
...
std::string UniqueNameHelper::GenerateNameForNewChildFrame(const std::string& name, bool is_created_by_script) const
{
...
//
std::string UniqueNameHelper::UpdateLegacyNameFromV24(std::string legacy_name, std::vector<Replacement>* replacements)
{
    if (IsNameWithFramePath(legacy_name)) {
        ...
        for (const auto& replacement : *replacements) {
            ...
            if (next_index == std::string::npos)
                continue;
            legacy_name.replace(next_index, replacement.old_name.size(), replacement.new_name);
        }
        return legacy_name;
    }
...
std::string UniqueNameHelper::CalculateLegacyNameForTesting(const FrameAdapter* frame, const std::string& name)
{
    return CalculateNameInternal(frame, name);
}
...
std::string UniqueNameHelper::ExtractStableNameForTesting(std::string_view unique_name)
{
