...
void TestRoundTrip(const blink::ServiceWorkerRouterRules& in)
{
    ...
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::ServiceWorkerRouterRules>(in, result));
    EXPECT_EQ(in, result);
}
...
TEST(ServiceWorkerRouterRulesTest, SimpleRoundTrip)
{
    blink::ServiceWorkerRouterRules rules;
    {
        blink::ServiceWorkerRouterRule rule;
        {
            ...
            {
                auto parse_result = liburlpattern::Parse("/test/*", [](std::string_view input) { return std::string(input); });
            ...
            {
                running_status.status = blink::ServiceWorkerRouterRunningStatusCondition::RunningStatusEnum::kRunning;
            }
            blink::ServiceWorkerRouterOrCondition or_condition;
            {
                or_condition.conditions = std::vector(3, blink::ServiceWorkerRouterCondition::WithRequest({}));
            }
            blink::ServiceWorkerRouterNotCondition not_condition;
            {
                not_condition.condition = std::make_unique<blink::ServiceWorkerRouterCondition>(blink::ServiceWorkerRouterCondition::WithRequest({}));
            }
            rule.condition = { url_pattern, request, running_status, or_condition, not_condition };
        }
        {
