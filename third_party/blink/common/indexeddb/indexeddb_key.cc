...
IndexedDBKey::IndexedDBKey(mojom::IDBKeyType type)
    ...
    , size_estimate_(kOverheadSize)
{
    DCHECK(type == mojom::IDBKeyType::None || type == mojom::IDBKeyType::Invalid || type == mojom::IDBKeyType::Min);
}
...
IndexedDBKey::IndexedDBKey(std::string binary)
    ...
    , size_estimate_(kOverheadSize + (binary_.length() * sizeof(std::string::value_type)))
{
}
...
IndexedDBKey::IndexedDBKey(std::u16string string)
    ...
    , size_estimate_(kOverheadSize + (string_.length() * sizeof(std::u16string::value_type)))
{
}
