...
{
    static base::NoDestructor<scoped_refptr<RefCountedScenarioMapping>> current_process_mapping;
    static base::NoDestructor<scoped_refptr<RefCountedScenarioMapping>> global_mapping;
    switch (scope) {
...
const ScenarioState& GetScenarioStateFromMapping(const RefCountedScenarioMapping* mapping)
{
...
ScopedReadOnlyScenarioMemory::ScopedReadOnlyScenarioMemory(Scope scope, base::ReadOnlySharedMemoryRegion region)
    : scope_(scope)
{
    using SharedScenarioState = base::StructuredSharedMemory<ScenarioState>;
    std::optional<SharedScenarioState::ReadOnlyMapping> mapping = SharedScenarioState::MapReadOnlyRegion(std::move(region));
    if (mapping.has_value()) {
        MappingPtrForScope(scope_) = base::MakeRefCounted<RefCountedScenarioMapping>(std::move(mapping.value()));
    }
}
...
// static
scoped_refptr<RefCountedScenarioMapping> ScopedReadOnlyScenarioMemory::GetMappingForTesting(Scope scope)
{
...
SharedAtomicRef<LoadingScenario> GetLoadingScenario(Scope scope)
{
    ...
    return SharedAtomicRef<LoadingScenario>(mapping, GetScenarioStateFromMapping(mapping.get()).loading);
}

SharedAtomicRef<InputScenario> GetInputScenario(Scope scope)
{
    scoped_refptr<RefCountedScenarioMapping> mapping = MappingPtrForScope(scope);
    return SharedAtomicRef<InputScenario>(mapping, GetScenarioStateFromMapping(mapping.get()).input);
}
