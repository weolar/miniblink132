...
public:
    ...
    MOCK_METHOD1(FinalizeWatchTime, void(const std::vector<media::WatchTimeKey>&));
    ...
    MOCK_METHOD1(UpdateSecondaryProperties, void(media::mojom::SecondaryPlaybackPropertiesPtr));
};
...
protected:
    ...
    std::unique_ptr<WatchTimeComponent<T>> CreateComponent(
        T initial_value, std::vector<media::WatchTimeKey> keys_to_finalize, typename WatchTimeComponent<T>::ValueToKeyCB value_to_key_cb)
    {
        return std::make_unique<WatchTimeComponent<T>>(initial_value, std::move(keys_to_finalize), std::move(value_to_key_cb),
            base::BindRepeating(&WatchTimeComponentTest::GetMediaTime, base::Unretained(this)), &recorder_);
    }
...
{
    auto test_component = CreateComponent<bool>(false, { kTestKey }, WatchTimeComponent<bool>::ValueToKeyCB());
    ...
    EXPECT_CALL(recorder_, RecordWatchTime(kTestKey, kFinalWatchTime - kStartTime));
    ...
    EXPECT_CALL(recorder_, RecordWatchTime(kTestKey, kNewWatchTime - kFinalWatchTime));
...
{
    auto test_component = CreateComponent<bool>(true, { kTestKey }, WatchTimeComponent<bool>::ValueToKeyCB());
    EXPECT_TRUE(test_component->current_value_for_testing());
    EXPECT_FALSE(test_component->NeedsFinalize());
    EXPECT_EQ(test_component->end_timestamp(), media::kNoTimestamp);
...
{
    const std::vector<media::WatchTimeKey> finalize_keys = { media::WatchTimeKey::kAudioVideoDisplayInline, media::WatchTimeKey::kAudioVideoDisplayFullscreen,
        media::WatchTimeKey::kAudioVideoDisplayPictureInPicture };
    auto test_component = CreateComponent<DisplayType>(DisplayType::kFullscreen, finalize_keys, base::BindRepeating([](DisplayType display_type) {
        switch (display_type) {
    ...
    EXPECT_EQ(test_component->current_value_for_testing(), DisplayType::kFullscreen);
    EXPECT_FALSE(test_component->NeedsFinalize());
    EXPECT_EQ(test_component->end_timestamp(), media::kNoTimestamp);
    ...
    EXPECT_CALL(recorder_, RecordWatchTime(media::WatchTimeKey::kAudioVideoDisplayFullscreen, kWatchTime1 - kStartTime));
    ...
    EXPECT_CALL(recorder_, RecordWatchTime(media::WatchTimeKey::kAudioVideoDisplayInline, kWatchTime2 - kStartTime));
    test_component->RecordWatchTime(kWatchTime2);
    ...
    EXPECT_CALL(recorder_, RecordWatchTime(media::WatchTimeKey::kAudioVideoDisplayPictureInPicture, kWatchTime3 - kStartTime));
    test_component->RecordWatchTime(kWatchTime3);
    EXPECT_EQ(test_component->current_value_for_testing(), DisplayType::kPictureInPicture);
    EXPECT_FALSE(test_component->NeedsFinalize());
    EXPECT_EQ(test_component->end_timestamp(), media::kNoTimestamp);
