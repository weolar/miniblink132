...
static bool IsOnBatteryPower()
{
    ...
    return power_monitor->GetBatteryPowerStatus() == base::PowerStateObserver::BatteryPowerStatus::kBatteryPower;
}
...
template <typename T> PropertyAction HandlePropertyChange(T new_value, bool is_timer_running, WatchTimeComponent<T>* component)
{
    if (!component)
    ...
    return component->NeedsFinalize() ? PropertyAction::kFinalizeRequired : PropertyAction::kNoActionRequired;
}

WatchTimeReporter::WatchTimeReporter(media::mojom::PlaybackPropertiesPtr properties, const gfx::Size& natural_size, GetMediaTimeCB get_media_time_cb,
    GetPipelineStatsCB get_pipeline_stats_cb, media::mojom::MediaMetricsProvider* provider, scoped_refptr<base::SequencedTaskRunner> task_runner,
    const base::TickClock* tick_clock)
    : WatchTimeReporter(std::move(properties), false /* is_background */, false /* is_muted */, natural_size, std::move(get_media_time_cb),
        std::move(get_pipeline_stats_cb), provider, task_runner, tick_clock)
{
}

WatchTimeReporter::WatchTimeReporter(media::mojom::PlaybackPropertiesPtr properties, bool is_background, bool is_muted, const gfx::Size& natural_size,
    GetMediaTimeCB get_media_time_cb, GetPipelineStatsCB get_pipeline_stats_cb, media::mojom::MediaMetricsProvider* provider,
    scoped_refptr<base::SequencedTaskRunner> task_runner, const base::TickClock* tick_clock)
    ...
    , natural_size_(natural_size)
{
    ...
    provider->AcquireWatchTimeRecorder(properties_->Clone(), recorder_.BindNewPipeAndPassReceiver());
    ...
    background_reporter_.reset(new WatchTimeReporter(std::move(prop_copy), true /* is_background */, false /* is_muted */, natural_size_, get_media_time_cb_,
        get_pipeline_stats_cb_, provider, task_runner, tick_clock));
    ...
    muted_reporter_.reset(new WatchTimeReporter(std::move(prop_copy), false /* is_background */, true /* is_muted */, natural_size_, get_media_time_cb_,
        get_pipeline_stats_cb_, provider, task_runner, tick_clock));
}
...
void WatchTimeReporter::OnUnderflow()
{
    ...
    pending_underflow_events_.push_back({ false, get_media_time_cb_.Run(), media::kNoTimestamp });
}
...
void WatchTimeReporter::UpdateSecondaryProperties(media::mojom::SecondaryPlaybackPropertiesPtr secondary_properties)
{
    ...
    if (background_reporter_) {
        background_reporter_->UpdateSecondaryProperties(secondary_properties.Clone());
    }
    if (muted_reporter_)
...
void WatchTimeReporter::OnBatteryPowerStatusChange(base::PowerStateObserver::BatteryPowerStatus battery_power_status)
{
    bool battery_power = (battery_power_status == base::PowerStateObserver::BatteryPowerStatus::kBatteryPower);
    if (HandlePropertyChange<bool>(battery_power, reporting_timer_.IsRunning(), power_component_.get()) == PropertyAction::kFinalizeRequired) {
...
void WatchTimeReporter::OnNativeControlsChanged(bool has_native_controls)
{
    ...
    if (HandlePropertyChange<bool>(has_native_controls, reporting_timer_.IsRunning(), controls_component_.get()) == PropertyAction::kFinalizeRequired) {
        RestartTimerForHysteresis();
    }
}
...
void WatchTimeReporter::OnDisplayTypeChanged(DisplayType display_type)
{
    ...
    if (HandlePropertyChange<DisplayType>(display_type, reporting_timer_.IsRunning(), display_type_component_.get()) == PropertyAction::kFinalizeRequired) {
        RestartTimerForHysteresis();
    }
}
...
bool WatchTimeReporter::ShouldReportWatchTime() const
{
    ...
    return properties_->has_video ? (natural_size_.height() >= kMinimumVideoSize.height() && natural_size_.width() >= kMinimumVideoSize.width())
...
bool WatchTimeReporter::ShouldReportingTimerRun() const
{
    ...
    return ShouldReportWatchTime() && is_playing_ && volume_ && is_visible_ && !in_shutdown_ && !is_seeking_ && has_valid_start_timestamp_;
}

void WatchTimeReporter::MaybeStartReportingTimer(base::TimeDelta start_timestamp)
{
    ...
    reporting_timer_.Start(FROM_HERE, reporting_interval_, this, &WatchTimeReporter::UpdateWatchTime);
}

void WatchTimeReporter::MaybeFinalizeWatchTime(FinalizeTime finalize_time)
{
    if (HandlePropertyChange<bool>(ShouldReportingTimerRun(), reporting_timer_.IsRunning(), base_component_.get()) == PropertyAction::kNoActionRequired) {
        return;
    }
...
void WatchTimeReporter::RecordWatchTime()
{
    ...
    const base::TimeDelta current_timestamp = base_component_->NeedsFinalize() ? base_component_->end_timestamp() : get_media_time_cb_.Run();
    ...
    if (!pending_underflow_events_.empty()) {
        ...
        for (auto& ufe : pending_underflow_events_) {
            ...
            if (ufe.duration != media::kNoTimestamp && ufe.duration <= kMaximumRebufferDuration) {
                ++total_completed_underflow_count_;
                total_underflow_duration_ += ufe.duration;
            }
        }

        std::erase_if(pending_underflow_events_, [](const UnderflowEvent& ufe) { return ufe.reported && ufe.duration != media::kNoTimestamp; });
        ...
        if (last_completed_underflow_count != total_completed_underflow_count_) {
            recorder_->UpdateUnderflowDuration(total_completed_underflow_count_, total_underflow_duration_);
        }
    }
    ...
    if (properties_->has_video) {
        ...
        // Only send updates.
        if (last_stats_.video_frames_decoded != stats.video_frames_decoded || last_stats_.video_frames_dropped != stats.video_frames_dropped) {
            recorder_->UpdateVideoDecodeStats(stats.video_frames_decoded, stats.video_frames_dropped);
            last_stats_ = stats;
        }
    }
...
#define NORMAL_KEY(key)                                                                                                                                        \
    ((properties_->has_video && properties_->has_audio)                                                                                                        \
            ? (is_background_ ? media::WatchTimeKey::kAudioVideoBackground##key                                                                                \
                              : (is_muted_ ? media::WatchTimeKey::kAudioVideoMuted##key : media::WatchTimeKey::kAudioVideo##key))                              \
            : properties_->has_video ? (is_background_ ? media::WatchTimeKey::kVideoBackground##key : media::WatchTimeKey::kVideo##key)                        \
                                     : (is_background_ ? media::WatchTimeKey::kAudioBackground##key : media::WatchTimeKey::kAudio##key))

std::unique_ptr<WatchTimeComponent<bool>> WatchTimeReporter::CreateBaseComponent()
{
    std::vector<media::WatchTimeKey> keys_to_finalize;
    keys_to_finalize.emplace_back(NORMAL_KEY(All));

    if (properties_->has_video && properties_->has_audio && !is_background_ && !is_muted_
        && properties_->renderer_type == media::RendererType::kMediaFoundation) {
        keys_to_finalize.emplace_back(media::WatchTimeKey::kAudioVideoMediaFoundationAll);
        if (properties_->is_eme) {
            keys_to_finalize.emplace_back(media::WatchTimeKey::kAudioVideoMediaFoundationEme);
        }
    }
    ...
    return std::make_unique<WatchTimeComponent<bool>>(
        false, std::move(keys_to_finalize), WatchTimeComponent<bool>::ValueToKeyCB(), get_media_time_cb_, recorder_.get());
}

std::unique_ptr<WatchTimeComponent<bool>> WatchTimeReporter::CreatePowerComponent()
{
    std::vector<media::WatchTimeKey> keys_to_finalize { NORMAL_KEY(Battery), NORMAL_KEY(Ac) };

    return std::make_unique<WatchTimeComponent<bool>>(IsOnBatteryPower(), std::move(keys_to_finalize),
        base::BindRepeating(&WatchTimeReporter::GetPowerKey, base::Unretained(this)), get_media_time_cb_, recorder_.get());
}
...
#define FOREGROUND_KEY(key)                                                                                                                                    \
    ((properties_->has_video && properties_->has_audio) ? (is_muted_ ? media::WatchTimeKey::kAudioVideoMuted##key : media::WatchTimeKey::kAudioVideo##key)     \
            : properties_->has_audio                    ? media::WatchTimeKey::kAudio##key                                                                     \
                                                        : media::WatchTimeKey::kVideo##key)

std::unique_ptr<WatchTimeComponent<bool>> WatchTimeReporter::CreateControlsComponent()
{
    DCHECK(!is_background_);

    std::vector<media::WatchTimeKey> keys_to_finalize { FOREGROUND_KEY(NativeControlsOn), FOREGROUND_KEY(NativeControlsOff) };

    return std::make_unique<WatchTimeComponent<bool>>(false, std::move(keys_to_finalize),
        base::BindRepeating(&WatchTimeReporter::GetControlsKey, base::Unretained(this)), get_media_time_cb_, recorder_.get());
}

media::WatchTimeKey WatchTimeReporter::GetControlsKey(bool has_native_controls)
{
    return has_native_controls ? FOREGROUND_KEY(NativeControlsOn) : FOREGROUND_KEY(NativeControlsOff);
}

#undef FOREGROUND_KEY

#define DISPLAY_TYPE_KEY(key)                                                                                                                                  \
    (properties_->has_audio ? (is_muted_ ? media::WatchTimeKey::kAudioVideoMuted##key : media::WatchTimeKey::kAudioVideo##key)                                 \
                            : media::WatchTimeKey::kVideo##key)

std::unique_ptr<WatchTimeComponent<DisplayType>> WatchTimeReporter::CreateDisplayTypeComponent()
{
    DCHECK(properties_->has_video);
    DCHECK(!is_background_);

    std::vector<media::WatchTimeKey> keys_to_finalize { DISPLAY_TYPE_KEY(DisplayInline), DISPLAY_TYPE_KEY(DisplayFullscreen),
        DISPLAY_TYPE_KEY(DisplayPictureInPicture) };

    return std::make_unique<WatchTimeComponent<DisplayType>>(DisplayType::kInline, std::move(keys_to_finalize),
        base::BindRepeating(&WatchTimeReporter::GetDisplayTypeKey, base::Unretained(this)), get_media_time_cb_, recorder_.get());
}

media::WatchTimeKey WatchTimeReporter::GetDisplayTypeKey(DisplayType display_type)
{
    switch (display_type) {
