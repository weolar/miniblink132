...
protected:
    const net::SchemefulSite GetOpaqueSite(uint64_t high, uint64_t low, std::string_view url_string)
    {
        return net::SchemefulSite(url::Origin(url::Origin::Nonce(base::UnguessableToken::CreateForTesting(high, low)), url::SchemeHostPort(GURL(url_string))));
    }

    std::vector<StorageKey> StorageKeysForCookiePartitionKeyTest(const base::UnguessableToken& nonce)
    {
        return std::vector<StorageKey> { /*Check storage key from string*/
            { StorageKey::CreateFromStringForTesting("https://www.example.com") },
            /*kCrossSite*/
            { StorageKey::Create(
                url::Origin::Create(GURL("https://www.foo.com")), net::SchemefulSite(GURL("https://www.bar.com")), mojom::AncestorChainBit::kCrossSite) },
            /*kSameSite keys check*/
            { StorageKey::Create(
                url::Origin::Create(GURL("https://www.foo.com")), net::SchemefulSite(GURL("https://www.foo.com")), mojom::AncestorChainBit::kSameSite) },
            /*First party check*/
            { StorageKey::CreateFirstParty(url::Origin::Create(GURL("https://www.foo.com"))) },
            /*Nonced*/
            { StorageKey::CreateWithNonce(url::Origin::Create(GURL("https://www.example.com")), nonce) }
        };
    }
};
...
TEST_F(StorageKeyTest, ConstructionValidity)
{
    ...
    url::Origin invalid_origin = url::Origin::Create(GURL("I'm not a valid URL."));
...
TEST_F(StorageKeyTest, EquivalenceUnpartitioned)
{
    ...
    scoped_feature_list.InitAndDisableFeature(net::features::kThirdPartyStoragePartitioning);
    ...
    const struct {
        ...
        { StorageKey::CreateFirstParty(origin1), StorageKey::CreateFirstParty(origin1), true },
        { StorageKey::CreateFirstParty(origin2), StorageKey::CreateFirstParty(origin2), true },
        { StorageKey::CreateFirstParty(origin3), StorageKey::CreateFirstParty(origin3), true },
        { StorageKey::CreateFirstParty(origin4), StorageKey::CreateFirstParty(origin4), true },
        // StorageKeys made from the same origin and nonce are equivalent.
        { StorageKey::CreateWithNonce(origin1, nonce1), StorageKey::CreateWithNonce(origin1, nonce1), true },
        { StorageKey::CreateWithNonce(origin1, nonce2), StorageKey::CreateWithNonce(origin1, nonce2), true },
        { StorageKey::CreateWithNonce(origin2, nonce1), StorageKey::CreateWithNonce(origin2, nonce1), true },
        // StorageKeys made from different origins are not equivalent.
        { StorageKey::CreateFirstParty(origin1), StorageKey::CreateFirstParty(origin2), false },
        { StorageKey::CreateFirstParty(origin3), StorageKey::CreateFirstParty(origin4), false },
        { StorageKey::CreateWithNonce(origin1, nonce1), StorageKey::CreateWithNonce(origin2, nonce1), false },
        // StorageKeys made from different nonces are not equivalent.
        { StorageKey::CreateWithNonce(origin1, nonce1), StorageKey::CreateWithNonce(origin1, nonce2), false },
        // StorageKeys made from different origins and nonce are not equivalent.
        { StorageKey::CreateWithNonce(origin1, nonce1), StorageKey::CreateWithNonce(origin2, nonce2), false },
        // When storage partitioning is disabled, the top-level site isn't taken
        // into account for equivalence.
        { StorageKey::Create(origin1, net::SchemefulSite(origin2), blink::mojom::AncestorChainBit::kCrossSite), StorageKey::CreateFirstParty(origin1), true },
        { StorageKey::Create(origin2, net::SchemefulSite(origin1), blink::mojom::AncestorChainBit::kCrossSite), StorageKey::CreateFirstParty(origin2), true },
    };
    for (const auto& test_case : kTestCases) {
        ASSERT_EQ(test_case.storage_key1 == test_case.storage_key2, test_case.expected_equivalent);
    }
}
...
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeature(net::features::kThirdPartyStoragePartitioning);
    ...
    StorageKey TwoArgKey_origin1_origin1 = StorageKey::Create(origin1, site1, blink::mojom::AncestorChainBit::kSameSite);
    StorageKey TwoArgKey_origin2_origin2 = StorageKey::Create(origin2, site2, blink::mojom::AncestorChainBit::kSameSite);
    ...
    StorageKey TwoArgKey1_origin1_origin2 = StorageKey::Create(origin1, site2, blink::mojom::AncestorChainBit::kCrossSite);
    StorageKey TwoArgKey2_origin1_origin2 = StorageKey::Create(origin1, site2, blink::mojom::AncestorChainBit::kCrossSite);
    StorageKey TwoArgKey_origin2_origin1 = StorageKey::Create(origin2, site1, blink::mojom::AncestorChainBit::kCrossSite);
...
TEST_F(StorageKeyTest, SerializeFirstParty)
{
    for (const bool toggle : { false, true }) {
        ...
        scope_feature_list.InitWithFeatureState(net::features::kThirdPartyStoragePartitioning, toggle);
        ...
        for (const auto& test : kTestCases) {
            ...
            const StorageKey key = StorageKey::CreateFromStringForTesting(test.origin_str);
...
{
    base::test::ScopedFeatureList scope_feature_list;
    scope_feature_list.InitAndEnableFeature(net::features::kThirdPartyStoragePartitioning);
    ...
    const struct {
        ...
        // 3p context cases
        { "https://example.com/", SiteTest, mojom::AncestorChainBit::kCrossSite, "https://example.com/^0https://test.example" },
        { "https://sub.test.example/", SiteExample, mojom::AncestorChainBit::kCrossSite, "https://sub.test.example/^0https://example.com" },
        { "https://example.com/", SiteExample, mojom::AncestorChainBit::kCrossSite, "https://example.com/^31" },
    };

    for (const auto& test : kTestCases) {
        SCOPED_TRACE(test.origin);
        const url::Origin origin = url::Origin::Create(GURL(test.origin));
        StorageKey key = StorageKey::Create(origin, test.top_level_site, test.ancestor_chain_bit);
        EXPECT_EQ(test.expected_serialization, key.Serialize());
        EXPECT_EQ(test.expected_serialization, key.SerializeForLocalStorage());
    }
}
...
TEST_F(StorageKeyTest, SerializeNonce)
{
    struct {
        ...
        { { "https://example.com/", base::UnguessableToken::CreateForTesting(12345ULL, 67890ULL) }, "https://example.com/^112345^267890" },
        { { "https://test.example", base::UnguessableToken::CreateForTesting(22222ULL, 99999ULL) }, "https://test.example/^122222^299999" },
        { { "https://sub.test.example/", base::UnguessableToken::CreateForTesting(9876ULL, 54321ULL) }, "https://sub.test.example/^19876^254321" },
        { { "https://other.example/", base::UnguessableToken::CreateForTesting(3735928559ULL, 110521ULL) }, "https://other.example/^13735928559^2110521" },
    };

    for (const auto& test : kTestCases) {
        SCOPED_TRACE(test.origin_and_nonce.first);
        const url::Origin origin = url::Origin::Create(GURL(test.origin_and_nonce.first));
...
TEST_F(StorageKeyTest, Deserialize)
{
    ...
    for (const auto& test_case : kTestCases) {
        ...
        std::optional<StorageKey> key = StorageKey::Deserialize(test_case.serialized_string);
...
{
    const char* kTestCases[] = { "https://example.com", "https://sub.test.example", "https://example.com:90", "file://", "file://example.fileshare.com" };
    ...
    for (const char* test : kTestCases) {
        ...
        std::optional<StorageKey> key_deserialized = StorageKey::Deserialize(key_string);
        std::optional<StorageKey> key_deserialized_from_local_storage = StorageKey::DeserializeForLocalStorage(key_string_for_local_storage);
        ...
        if (origin.scheme() != "file") {
            ...
            EXPECT_EQ(StorageKey::CreateFromStringForTesting("file://"), *key_deserialized);
            EXPECT_EQ(StorageKey::CreateFromStringForTesting("file://"), *key_deserialized_from_local_storage);
        }
    }
}
...
TEST_F(StorageKeyTest, SerializeDeserializePartitioned)
{
    ...
    for (const auto& test : kTestCases) {
        ...
        StorageKey key
            = StorageKey::Create(origin, site, net::SchemefulSite(origin) == site ? mojom::AncestorChainBit::kSameSite : mojom::AncestorChainBit::kCrossSite);
        std::string key_string = key.Serialize();
        std::string key_string_for_local_storage = key.SerializeForLocalStorage();
        std::optional<StorageKey> key_deserialized = StorageKey::Deserialize(key_string);
        std::optional<StorageKey> key_deserialized_from_local_storage = StorageKey::DeserializeForLocalStorage(key_string_for_local_storage);
        ...
        if (origin.scheme() != "file") {
            ...
            // file origins are all collapsed to file:// by serialization.
            EXPECT_EQ(StorageKey::Create(url::Origin::Create(GURL("file://")), net::SchemefulSite(GURL("file://")), mojom::AncestorChainBit::kSameSite),
                *key_deserialized);
            EXPECT_EQ(StorageKey::Create(url::Origin::Create(GURL("file://")), net::SchemefulSite(GURL("file://")), mojom::AncestorChainBit::kSameSite),
...
TEST_F(StorageKeyTest, SerializeDeserializeNonce)
{
    struct {
        ...
        { "https://example.com/", base::UnguessableToken::CreateForTesting(12345ULL, 67890ULL) },
        { "https://test.example", base::UnguessableToken::CreateForTesting(22222ULL, 99999ULL) },
        { "https://sub.test.example/", base::UnguessableToken::CreateForTesting(9876ULL, 54321ULL) },
        { "https://other.example/", base::UnguessableToken::CreateForTesting(3735928559ULL, 110521ULL) },
...
TEST_F(StorageKeyTest, SerializeDeserializeOpaqueTopLevelSite)
{
    for (const bool toggle : { false, true }) {
        ...
        const struct {
            ...
            {
                url::Origin::Create(GURL("https://sub.example.com/")),
                GetOpaqueSite(3735928559ULL, 110521ULL, "https://sub.notexample.com/"),
                "https://sub.example.com/",
        ...
        for (const auto& test : kTestCases) {
            ...
            StorageKey key = StorageKey::Create(test.origin, test.top_level_site, blink::mojom::AncestorChainBit::kCrossSite);
            if (toggle) {
                EXPECT_EQ(test.expected_serialization_with_partitioning, key.Serialize());
                EXPECT_EQ(key, StorageKey::Deserialize(test.expected_serialization_with_partitioning));
            } else {
                EXPECT_EQ(test.expected_serialization_without_partitioning, key.Serialize());
                EXPECT_EQ(key, StorageKey::Deserialize(test.expected_serialization_without_partitioning));
            }
        }
    }
}
...
TEST_F(StorageKeyTest, DeserializeNonces)
{
    for (const bool toggle : { false, true }) {
        ...
        struct {
            ...
            },
            {
                ...
                blink::StorageKey::Create(
                    url::Origin::Create(GURL("https://example.com/")), GetOpaqueSite(1ULL, 0ULL, ""), mojom::AncestorChainBit::kCrossSite),
                false,
            },
            {
            ...
            },
            {
                ...
                blink::StorageKey::Create(
                    url::Origin::Create(GURL("https://example.com/")), GetOpaqueSite(0ULL, 1ULL, ""), mojom::AncestorChainBit::kCrossSite),
                false,
            },
            {
            ...
            },
            {
                ...
                blink::StorageKey::Create(
                    url::Origin::Create(GURL("https://example.com/")), GetOpaqueSite(1ULL, 1ULL, ""), mojom::AncestorChainBit::kCrossSite),
                false,
            },
            {
            ...
            },
            {
                ...
                blink::StorageKey::CreateWithNonce(url::Origin::Create(GURL("https://example.com/")), base::UnguessableToken::CreateForTesting(1ULL, 0ULL)),
                true,
            },
            {
            ...
            },
            {
                ...
                blink::StorageKey::CreateWithNonce(url::Origin::Create(GURL("https://example.com/")), base::UnguessableToken::CreateForTesting(0ULL, 1ULL)),
                true,
            },
            {
            ...
            },
            {
                ...
                blink::StorageKey::CreateWithNonce(url::Origin::Create(GURL("https://example.com/")), base::UnguessableToken::CreateForTesting(1ULL, 1ULL)),
                true,
            },
            {
        ...
        for (const auto& test : kTestCases) {
            ...
            std::optional<blink::StorageKey> maybe_storage_key = StorageKey::Deserialize(test.serialization);
            EXPECT_EQ((test.has_value_if_partitioning_is_disabled || toggle) && test.expected_key, (bool)maybe_storage_key);
    ...
    for (const bool toggle : { false, true }) {
        ...
        struct {
            ...
            // An origin can be serialized with a CrossSite bit.
            {
                ...
                StorageKey::Create(
                    url::Origin::Create(GURL("https://example.com")), net::SchemefulSite(GURL("https://example.com")), mojom::AncestorChainBit::kCrossSite),
            },
            ...
            // A mismatched origin and top_level_site can have no bit.
            {
                ...
                StorageKey::Create(
                    url::Origin::Create(GURL("https://example.com")), net::SchemefulSite(GURL("https://notexample.com")), mojom::AncestorChainBit::kCrossSite),
            },
        };
...
TEST_F(StorageKeyTest, TopLevelSiteGetterWithPartitioningDisabled)
{
    ...
    StorageKey key_origin1_site1 = StorageKey::Create(origin1, site1, mojom::AncestorChainBit::kSameSite);
    StorageKey key_origin1_site2 = StorageKey::Create(origin1, site2, mojom::AncestorChainBit::kCrossSite);
...
TEST_F(StorageKeyTest, TopLevelSiteGetterWithPartitioningEnabled)
{
    ...
    const StorageKey key_origin1 = StorageKey::CreateFirstParty(origin1);
    StorageKey key_origin1_site1 = StorageKey::Create(origin1, net::SchemefulSite(origin1), mojom::AncestorChainBit::kSameSite);
    StorageKey key_origin1_site2 = StorageKey::Create(origin1, net::SchemefulSite(origin2), mojom::AncestorChainBit::kCrossSite);
...
TEST_F(StorageKeyTest, AncestorChainBitGetterWithPartitioningDisabled)
{
    ...
    std::optional<StorageKey> key_cross_site = StorageKey::Deserialize(cross_site_string);
...
TEST_F(StorageKeyTest, AncestorChainBitGetterWithPartitioningEnabled)
{
    ...
    EXPECT_EQ(mojom::AncestorChainBit::kCrossSite, key_cross_site->ancestor_chain_bit());
}
...
TEST_F(StorageKeyTest, IsThirdPartyContext)
{
    ...
    const url::Origin kInsecureOrigin = url::Origin::Create(GURL("http://www.foo.com"));
    const url::Origin kSubdomainOrigin = url::Origin::Create(GURL("https://bar.foo.com"));
    const url::Origin kDifferentSite = url::Origin::Create(GURL("https://www.bar.com"));
    ...
    } test_cases[] = { { kOrigin, kOrigin, false }, { kOrigin, kInsecureOrigin, true }, { kOrigin, kSubdomainOrigin, false }, { kOrigin, kDifferentSite, true },
        { kOrigin, kOrigin, true, true } };
    for (const auto& test_case : test_cases) {
        if (test_case.has_nonce) {
            StorageKey key = StorageKey::CreateWithNonce(test_case.origin, base::UnguessableToken::Create());
            EXPECT_EQ(test_case.expected, key.IsThirdPartyContext());
            EXPECT_NE(key.IsThirdPartyContext(), key.IsFirstPartyContext());
            continue;
        }
        mojom::AncestorChainBit ancestor_chain_bit = net::SchemefulSite(test_case.origin) == net::SchemefulSite(test_case.top_level_origin)
            ? mojom::AncestorChainBit::kSameSite
            : mojom::AncestorChainBit::kCrossSite;
        StorageKey key = StorageKey::Create(test_case.origin, net::SchemefulSite(test_case.top_level_origin), ancestor_chain_bit);
        EXPECT_EQ(test_case.expected, key.IsThirdPartyContext());
        EXPECT_NE(key.IsThirdPartyContext(), key.IsFirstPartyContext());
        // IsThirdPartyContext should not depend on the order of the arguments.
        key = StorageKey::Create(test_case.top_level_origin, net::SchemefulSite(test_case.origin), ancestor_chain_bit);
        EXPECT_EQ(test_case.expected, key.IsThirdPartyContext());
        EXPECT_NE(key.IsThirdPartyContext(), key.IsFirstPartyContext());
    }
    // Explicitly testing the A->B->A case AncestorChainBit is preventing:
    // Same origin and top-level site but cross-site ancestor
    StorageKey cross_key = StorageKey::Create(kOrigin, net::SchemefulSite(kOrigin), mojom::AncestorChainBit::kCrossSite);
...
TEST_F(StorageKeyTest, ToNetSiteForCookies)
{
    ...
    for (const auto& test_case : test_cases) {
        ...
        if (test_case.has_nonce) {
            got_site = StorageKey::CreateWithNonce(test_case.origin, base::UnguessableToken::Create()).ToNetSiteForCookies().site();
        } else {
            net::SchemefulSite top_level_site = net::SchemefulSite(test_case.top_level_origin);

            mojom::AncestorChainBit ancestor_chain_bit
                = top_level_site == net::SchemefulSite(test_case.origin) ? mojom::AncestorChainBit::kSameSite : mojom::AncestorChainBit::kCrossSite;

            got_site = StorageKey::Create(test_case.origin, top_level_site, ancestor_chain_bit).ToNetSiteForCookies().site();
        }
...
TEST_F(StorageKeyTest, ToPartialNetIsolationInfo)
{
    ...
    const auto kOtherOrigin = url::Origin::Create(GURL("https://subdomain.bar.com"));
    const auto nonce = base::UnguessableToken::Create();

    { // Same-site storage key
        const auto storage_key = StorageKey::Create(kOrigin, net::SchemefulSite(kOrigin), mojom::AncestorChainBit::kSameSite);

        storage_key.ToPartialNetIsolationInfo().IsEqualForTesting(
            net::IsolationInfo::Create(net::IsolationInfo::RequestType::kOther, kOrigin, kOrigin, net::SiteForCookies::FromOrigin(kOrigin)));
    }

    { // Cross-site storage key
        const auto storage_key = StorageKey::Create(kOrigin, net::SchemefulSite(kOtherOrigin), mojom::AncestorChainBit::kCrossSite);

        storage_key.ToPartialNetIsolationInfo().IsEqualForTesting(net::IsolationInfo::Create(
            net::IsolationInfo::RequestType::kOther, net::SchemefulSite(kOrigin).GetInternalOriginForTesting(), kOtherOrigin, net::SiteForCookies()));
    }
    ...
    { // Nonced key
        ...
        storage_key.ToPartialNetIsolationInfo().IsEqualForTesting(net::IsolationInfo::Create(
            net::IsolationInfo::RequestType::kOther, net::SchemefulSite(kOrigin).GetInternalOriginForTesting(), kOrigin, net::SiteForCookies(), nonce));
    }
}
...
TEST_F(StorageKeyTest, CopyWithForceEnabledThirdPartyStoragePartitioning)
{
    ...
    for (const bool toggle : { false, true }) {
        ...
        const StorageKey storage_key = StorageKey::Create(kOrigin, net::SchemefulSite(kOtherOrigin), mojom::AncestorChainBit::kCrossSite);
        EXPECT_EQ(storage_key.IsThirdPartyContext(), toggle);
        EXPECT_EQ(storage_key.top_level_site(), net::SchemefulSite(toggle ? kOtherOrigin : kOrigin));
        EXPECT_EQ(storage_key.ancestor_chain_bit(), toggle ? mojom::AncestorChainBit::kCrossSite : mojom::AncestorChainBit::kSameSite);

        const StorageKey storage_key_with_3psp = storage_key.CopyWithForceEnabledThirdPartyStoragePartitioning();
        EXPECT_TRUE(storage_key_with_3psp.IsThirdPartyContext());
        EXPECT_EQ(storage_key_with_3psp.top_level_site(), net::SchemefulSite(kOtherOrigin));
        EXPECT_EQ(storage_key_with_3psp.ancestor_chain_bit(), mojom::AncestorChainBit::kCrossSite);
    }
}
...
TEST_F(StorageKeyTest, ToCookiePartitionKeyAncestorChainBitDisabled)
{
    ...
    std::vector<StorageKey> storage_keys = StorageKeysForCookiePartitionKeyTest(nonce);
    ...
    { // Ancestor Chain Bit disabled in Partitioned Cookies.
        base::test::ScopedFeatureList scope_feature_list;
        scope_feature_list.InitWithFeatures({ net::features::kThirdPartyStoragePartitioning }, { net::features::kAncestorChainBitEnabledInPartitionedCookies });

        std::vector<std::optional<net::CookiePartitionKey>> expected_cpk {
            { net::CookiePartitionKey::FromURLForTesting(GURL("https://www.example.com")) },
            { net::CookiePartitionKey::FromURLForTesting(GURL("https://subdomain.bar.com")) },
            { net::CookiePartitionKey::FromURLForTesting(GURL("https://www.foo.com"), net::CookiePartitionKey::AncestorChainBit::kSameSite) },
            { net::CookiePartitionKey::FromURLForTesting(GURL("https://www.foo.com"), net::CookiePartitionKey::AncestorChainBit::kSameSite) },
            { net::CookiePartitionKey::FromURLForTesting(GURL("https://www.example.com"), net::CookiePartitionKey::AncestorChainBit::kCrossSite, nonce) },
        };

        std::vector<std::optional<net::CookiePartitionKey>> got;
        base::ranges::transform(
            storage_keys, std::back_inserter(got), [](const StorageKey& key) -> std::optional<net::CookiePartitionKey> { return key.ToCookiePartitionKey(); });
...
TEST_F(StorageKeyTest, ToCookiePartitionKeyAncestorChainEnabled)
{
    ...
    { // Ancestor Chain Bit enabled in Partitioned Cookies.
        ...
        std::vector<std::optional<net::CookiePartitionKey>> expected_cpk {
            { net::CookiePartitionKey::FromURLForTesting(GURL("https://www.example.com"), net::CookiePartitionKey::AncestorChainBit::kSameSite) },
            { net::CookiePartitionKey::FromURLForTesting(GURL("https://subdomain.bar.com")) },
            { net::CookiePartitionKey::FromURLForTesting(GURL("https://www.foo.com"), net::CookiePartitionKey::AncestorChainBit::kSameSite) },
            { net::CookiePartitionKey::FromURLForTesting(GURL("https://www.foo.com"), net::CookiePartitionKey::AncestorChainBit::kSameSite) },
            { net::CookiePartitionKey::FromURLForTesting(GURL("https://www.example.com"), net::CookiePartitionKey::AncestorChainBit::kCrossSite, nonce) },
...
TEST_F(StorageKeyTest, NonceRequiresMatchingOriginSiteAndCrossSite)
{
    ...
    for (const bool toggle : { false, true }) {
        ...
        StorageKey key = StorageKey::CreateWithNonce(origin, nonce);
        EXPECT_EQ(key.ancestor_chain_bit(), blink::mojom::AncestorChainBit::kCrossSite);
...
TEST_F(StorageKeyTest, OpaqueTopLevelSiteRequiresCrossSite)
{
    ...
    for (const bool toggle : { false, true }) {
        ...
        std::ignore = StorageKey::Create(origin, site, mojom::AncestorChainBit::kSameSite);
        std::ignore = StorageKey::Create(origin, site, mojom::AncestorChainBit::kCrossSite);

        // An opaque site with CrossSite works.
        std::ignore = StorageKey::Create(origin, opaque_site, mojom::AncestorChainBit::kCrossSite);

        // An opaque site with SameSite fails.
        EXPECT_DCHECK_DEATH(StorageKey::Create(origin, opaque_site, mojom::AncestorChainBit::kSameSite));
    }
}
...
TEST_F(StorageKeyTest, ShouldSkipKeyDueToPartitioning)
{
    ...
    for (const bool toggle : { false, true }) {
        ...
        for (const auto& test_case : kTestCases) {
            if (toggle) {
                EXPECT_FALSE(StorageKey::ShouldSkipKeyDueToPartitioning(test_case.serialized_key));
            } else {
                EXPECT_EQ(StorageKey::ShouldSkipKeyDueToPartitioning(test_case.serialized_key), test_case.expected_skip);
            }
        }
    }
}
...
TEST_F(StorageKeyTest, OriginAndSiteMismatchRequiresCrossSite)
{
    ...
    for (const bool toggle : { false, true }) {
        ...
        EXPECT_DCHECK_DEATH(StorageKey::Create(origin, other_site, mojom::AncestorChainBit::kSameSite));
        EXPECT_DCHECK_DEATH(StorageKey::Create(opaque_origin, other_site, mojom::AncestorChainBit::kSameSite));

        // A mismatched origin and site must be CrossSite.
        std::ignore = StorageKey::Create(origin, other_site, mojom::AncestorChainBit::kCrossSite);
    }
}
...
{
    const url::Origin origin = url::Origin::Create(GURL("https://foo.com"));
    const url::Origin other_origin = url::Origin::Create(GURL("https://notfoo.com"));
    const url::Origin opaque_origin;
    const net::SchemefulSite site(origin);
    ...
    const struct {
        ...
        {
            blink::StorageKey::Create(origin, site, mojom::AncestorChainBit::kSameSite),
            origin,
            std::nullopt,
        },
        // Change in first-party key updated with new origin.
        {
            blink::StorageKey::Create(origin, site, mojom::AncestorChainBit::kSameSite),
            other_origin,
            blink::StorageKey::Create(other_origin, site, mojom::AncestorChainBit::kCrossSite),
        },
        // No change in third-party same-site key updated with same origin.
        {
            blink::StorageKey::Create(origin, site, mojom::AncestorChainBit::kCrossSite),
            origin,
            std::nullopt,
        },
        ...
        {
            blink::StorageKey::Create(origin, other_site, mojom::AncestorChainBit::kCrossSite),
            origin,
            std::nullopt,
        },
        // Change in third-party key updated with new origin.
        {
            blink::StorageKey::Create(origin, other_site, mojom::AncestorChainBit::kCrossSite),
            other_origin,
            blink::StorageKey::Create(other_origin, other_site, mojom::AncestorChainBit::kCrossSite),
        },
        // No change in opaque tls key updated with same origin.
        {
            blink::StorageKey::Create(origin, opaque_site, mojom::AncestorChainBit::kCrossSite),
            origin,
            std::nullopt,
        },
        // Change in opaque tls key updated with new origin.
        {
            blink::StorageKey::Create(origin, opaque_site, mojom::AncestorChainBit::kCrossSite),
            other_origin,
            blink::StorageKey::Create(other_origin, opaque_site, mojom::AncestorChainBit::kCrossSite),
        },
        ...
        // Change in opaque top_level_site key updated with opaque origin.
        {
            blink::StorageKey::Create(origin, opaque_site, mojom::AncestorChainBit::kCrossSite),
            opaque_origin,
            blink::StorageKey::Create(opaque_origin, opaque_site, mojom::AncestorChainBit::kCrossSite),
        },
    };

    for (const auto& test_case : kTestCases) {
        if (test_case.expected_key == std::nullopt) {
            EXPECT_EQ(test_case.original_key, test_case.original_key.WithOrigin(test_case.new_origin));
        } else {
            ASSERT_NE(test_case.expected_key, test_case.original_key);
            EXPECT_EQ(test_case.expected_key, test_case.original_key.WithOrigin(test_case.new_origin));
        }
    }
}
...
TEST_F(StorageKeyTest, FromWireReturnValue)
{
    ...
    const struct TestCase {
        ...
        // Passing cases:
        { o1, site1, site1, std::nullopt, AncestorChainBit::kSameSite, AncestorChainBit::kSameSite, true },
        { o1, site1, site1, nonce1, AncestorChainBit::kCrossSite, AncestorChainBit::kCrossSite, true },
        { o1, site1, site2, std::nullopt, AncestorChainBit::kSameSite, AncestorChainBit::kCrossSite, true },
        { o1, site1, site1, std::nullopt, AncestorChainBit::kSameSite, AncestorChainBit::kCrossSite, true },
        { o1, site1, site1, nonce1, AncestorChainBit::kCrossSite, AncestorChainBit::kCrossSite, true },
        { opaque, site1, site1, std::nullopt, AncestorChainBit::kCrossSite, AncestorChainBit::kCrossSite, true },
        { o1, site1, opaque_site, std::nullopt, AncestorChainBit::kSameSite, AncestorChainBit::kCrossSite, true },
        { o1, opaque_site, opaque_site, std::nullopt, AncestorChainBit::kCrossSite, AncestorChainBit::kCrossSite, true },
        { opaque, opaque_site, opaque_site, std::nullopt, AncestorChainBit::kCrossSite, AncestorChainBit::kCrossSite, true },
        // Failing cases:
        // If a 3p key is indicated, the *if_third_party_enabled pieces should
        // match their counterparts.
        { o1, site2, site3, std::nullopt, AncestorChainBit::kSameSite, AncestorChainBit::kSameSite, false },
        { o1, site1, site1, std::nullopt, AncestorChainBit::kCrossSite, AncestorChainBit::kSameSite, false },
        // If the top_level_site* is cross-site to the origin, the
        // ancestor_chain_bit* must indicate cross-site.
        { o1, site2, site2, std::nullopt, AncestorChainBit::kSameSite, AncestorChainBit::kCrossSite, false },
        { o1, site1, site2, std::nullopt, AncestorChainBit::kSameSite, AncestorChainBit::kSameSite, false },
        { o1, site2, site2, std::nullopt, AncestorChainBit::kSameSite, AncestorChainBit::kSameSite, false },
        // If there is a nonce, all other values must indicate same-site to
        // origin.
        { o1, site2, site2, nonce1, AncestorChainBit::kSameSite, AncestorChainBit::kSameSite, false },
        { o1, site1, site1, nonce1, AncestorChainBit::kSameSite, AncestorChainBit::kSameSite, false },
        { o1, site1, site1, nonce1, AncestorChainBit::kSameSite, AncestorChainBit::kCrossSite, false },
        // If the top_level_site* is opaque, the ancestor_chain_bit* must be
        // cross-site.
        { o1, site1, opaque_site, std::nullopt, AncestorChainBit::kCrossSite, AncestorChainBit::kSameSite, false },
        { o1, opaque_site, opaque_site, std::nullopt, AncestorChainBit::kSameSite, AncestorChainBit::kSameSite, false },
        // If the origin is opaque, the ancestor_chain_bit* must be cross-site.
        { opaque, opaque_site, opaque_site, std::nullopt, AncestorChainBit::kSameSite, AncestorChainBit::kSameSite, false },
        { opaque, opaque_site, opaque_site, std::nullopt, AncestorChainBit::kCrossSite, AncestorChainBit::kSameSite, false },
        { opaque, opaque_site, opaque_site, std::nullopt, AncestorChainBit::kSameSite, AncestorChainBit::kCrossSite, false },
    };
    ...
    for (const auto& test_case : test_cases) {
        ...
        EXPECT_EQ(test_case.result,
            StorageKey::FromWire(test_case.origin, test_case.top_level_site, test_case.top_level_site_if_third_party_enabled, test_case.nonce,
                test_case.ancestor_chain_bit, test_case.ancestor_chain_bit_if_third_party_enabled, result_key));
        if (!test_case.result) {
...
TEST_F(StorageKeyTest, CreateFromOriginAndIsolationInfo)
{
    ...
    for (const bool toggle : { false, true }) {
        ...
        const struct {
            ...
            {
                origin,
                net::IsolationInfo::Create(net::IsolationInfo::RequestType::kMainFrame, origin, origin, net::SiteForCookies::FromOrigin(origin),
                    /*nonce=*/std::nullopt),
                blink::StorageKey::Create(origin, site, mojom::AncestorChainBit::kSameSite),
            },
            ...
            {
                other_origin,
                net::IsolationInfo::Create(net::IsolationInfo::RequestType::kMainFrame, other_origin, origin, net::SiteForCookies(),
                    /*nonce=*/std::nullopt),
                blink::StorageKey::Create(other_origin, other_site, mojom::AncestorChainBit::kCrossSite),
            },
            // Third party context.
            ...
            {
                origin,
                net::IsolationInfo::Create(net::IsolationInfo::RequestType::kMainFrame, opaque_origin, opaque_origin,
                    net::SiteForCookies::FromOrigin(opaque_origin),
                    /*nonce=*/std::nullopt),
                blink::StorageKey::Create(origin, opaque_site, mojom::AncestorChainBit::kCrossSite),
            },
            // Nonce context.
            {
                ...
                net::IsolationInfo::Create(
                    net::IsolationInfo::RequestType::kMainFrame, other_origin, other_origin, net::SiteForCookies::FromOrigin(other_origin), nonce),
                blink::StorageKey::CreateWithNonce(origin, nonce),
            },
            // Opaque context.
            {
                ...
                net::IsolationInfo::Create(net::IsolationInfo::RequestType::kMainFrame, origin, origin, net::SiteForCookies::FromOrigin(origin),
                    /*nonce=*/std::nullopt),
                blink::StorageKey::Create(opaque_origin, site, mojom::AncestorChainBit::kCrossSite),
            },
        };

        for (const auto& test_case : kTestCases) {
            if (test_case.expected_key == std::nullopt) {
                EXPECT_DCHECK_DEATH(StorageKey::CreateFromOriginAndIsolationInfo(test_case.new_origin, test_case.isolation_info));
            } else {
                EXPECT_EQ(test_case.expected_key, StorageKey::CreateFromOriginAndIsolationInfo(test_case.new_origin, test_case.isolation_info));
            }
        }
    }
}
...
TEST_F(StorageKeyTest, DeserializeForLocalStorageFirstParty)
{
    for (const bool toggle : { false, true }) {
        ...
        EXPECT_TRUE(StorageKey::DeserializeForLocalStorage("https://example.com").has_value());

        // This should deserialize as it lacks a trailing slash.
        EXPECT_FALSE(StorageKey::DeserializeForLocalStorage("https://example.com/").has_value());
    }
}

TEST_F(StorageKeyTest, SerializeDeserializeWithAndWithoutThirdPartyStoragePartitioning)
{
    struct {
    ...
    for (const bool toggle : { false, true }) {
        ...
        scope_feature_list.InitWithFeatureState(net::features::kThirdPartyStoragePartitioning, toggle);
        for (const auto& test_case : kTestCases) {
            const std::optional<blink::StorageKey> maybe_storage_key = StorageKey::Deserialize(test_case.serialized_key);
            EXPECT_EQ(test_case.has_value_if_partitioning_is_disabled || toggle, (bool)maybe_storage_key);
            if (maybe_storage_key) {
