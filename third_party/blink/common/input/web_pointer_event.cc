...
WebInputEvent::Type PointerEventTypeForTouchPointState(WebTouchPoint::State state)
{
    switch (state) {
...
WebPointerEvent::WebPointerEvent(const WebTouchEvent& touch_event, const WebTouchPoint& touch_point)
    : WebInputEvent(PointerEventTypeForTouchPointState(touch_point.state), touch_event.GetModifiers(), touch_event.TimeStamp())
    ,
    ...
    , height(touch_point.radius_y * 2.f)
{
    ...
    button = (GetType() == WebInputEvent::Type::kPointerDown || GetType() == WebInputEvent::Type::kPointerUp) ? WebPointerProperties::Button::kLeft
...
WebPointerEvent::WebPointerEvent(WebInputEvent::Type type, const WebMouseEvent& mouse_event)
...
bool WebPointerEvent::CanCoalesce(const WebInputEvent& event) const
{
    if (!IsPointerEventType(event.GetType()))
        return false;
    const WebPointerEvent& pointer_event = static_cast<const WebPointerEvent&>(event);
    return (GetType() == WebInputEvent::Type::kPointerMove || GetType() == WebInputEvent::Type::kPointerRawUpdate) && GetType() == event.GetType()
        && GetModifiers() == event.GetModifiers() && id == pointer_event.id && pointer_type == pointer_event.pointer_type;
}
...
WebPointerEvent WebPointerEvent::CreatePointerCausesUaActionEvent(WebPointerProperties::PointerType type, base::TimeTicks time_stamp)
{
...
WebPointerEvent WebPointerEvent::WebPointerEventInRootFrame() const
{
    ...
    transformed_event.position_in_widget_ = gfx::ScalePoint(transformed_event.PositionInWidget(), 1 / frame_scale_) + frame_translate_;
