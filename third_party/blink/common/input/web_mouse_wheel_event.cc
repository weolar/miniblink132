...
bool WebMouseWheelEvent::CanCoalesce(const WebInputEvent& event) const
{
    ...
    return GetModifiers() == mouse_wheel_event.GetModifiers() && delta_units == mouse_wheel_event.delta_units && HaveConsistentPhase(mouse_wheel_event);
}

bool WebMouseWheelEvent::HaveConsistentPhase(const WebMouseWheelEvent& event) const
{
    ...
    if (has_synthetic_phase) {
        ...
        return (phase == WebMouseWheelEvent::kPhaseBegan && event.phase == WebMouseWheelEvent::kPhaseChanged);
    }
    return false;
}
...
void WebMouseWheelEvent::Coalesce(const WebInputEvent& event)
{
    ...
    float unaccelerated_x
        = GetUnacceleratedDelta(delta_x, acceleration_ratio_x) + GetUnacceleratedDelta(mouse_wheel_event.delta_x, mouse_wheel_event.acceleration_ratio_x);
    float unaccelerated_y
        = GetUnacceleratedDelta(delta_y, acceleration_ratio_y) + GetUnacceleratedDelta(mouse_wheel_event.delta_y, mouse_wheel_event.acceleration_ratio_y);
    ...
    *this = mouse_wheel_event;
    ...
    dispatch_type = MergeDispatchTypes(old_dispatch_type, mouse_wheel_event.dispatch_type);
    if (mouse_wheel_event.has_synthetic_phase && mouse_wheel_event.phase != old_phase) {
...
// static
WebMouseWheelEvent::EventAction WebMouseWheelEvent::GetPlatformSpecificDefaultEventAction(const WebMouseWheelEvent& event)
{
    ...
    if (event.delta_units != ui::ScrollGranularity::kScrollByPrecisePixel && (event.GetModifiers() & WebInputEvent::kControlKey)) {
...
#endif
    if (event.rails_mode == WebInputEvent::kRailsModeHorizontal || (event.delta_x != 0 && event.delta_y == 0)) {
        return blink::WebMouseWheelEvent::EventAction::kScrollHorizontal;
    }

    if (event.rails_mode == WebInputEvent::kRailsModeVertical || (event.delta_x == 0 && event.delta_y != 0)) {
