...
bool WebTouchEvent::CanCoalesce(const WebInputEvent& event) const
{
    ...
    if (GetType() != touch_event.GetType() || GetType() != WebInputEvent::Type::kTouchMove || GetModifiers() != touch_event.GetModifiers()
        || touches_length != touch_event.touches_length || touches_length > kTouchesLengthCap)
        return false;

    static_assert(WebTouchEvent::kTouchesLengthCap <= sizeof(int32_t) * 8U, "suboptimal kTouchesLengthCap size");
    // Ensure that we have a 1-to-1 mapping of pointer ids between touches.
    std::bitset<WebTouchEvent::kTouchesLengthCap> unmatched_event_touches((1 << touches_length) - 1);
    for (unsigned i = 0; i < touch_event.touches_length; ++i) {
        ...
        if (!unmatched_event_touches[event_touch_index])
            return false;
        if (touches[event_touch_index].pointer_type != touch_event.touches[i].pointer_type)
            return false;
...
void WebTouchEvent::Coalesce(const WebInputEvent& event)
{
    ...
    *this = touch_event;
    ...
    dispatch_type = MergeDispatchTypes(old_event.dispatch_type, touch_event.dispatch_type);
...
WebTouchPoint WebTouchEvent::TouchPointInRootFrame(unsigned point) const
{
    ...
    transformed_point.SetPositionInWidget(gfx::ScalePoint(transformed_point.PositionInWidget(), 1 / frame_scale_) + frame_translate_);
    return transformed_point;
}

bool WebTouchEvent::IsTouchSequenceStart() const
{
    DCHECK(touches_length || GetType() == WebInputEvent::Type::kTouchScrollStarted);
...
{
    if (GetType() != WebInputEvent::Type::kTouchEnd && GetType() != WebInputEvent::Type::kTouchCancel) {
        return false;
    }
    if (!touches_length) {
        return true;
    ...
    for (size_t i = 0; i < touches_length; ++i) {
        if (touches[i].state != WebTouchPoint::State::kStateReleased && touches[i].state != WebTouchPoint::State::kStateCancelled) {
            return false;
        }
    }
    return true;
