...
TEST(WebInputEventTest, TouchEventCoalescing)
{
    ...
    coalesced_event.touches[0].pointer_type = WebPointerProperties::PointerType::kPen;
...
{
    WebMouseWheelEvent coalesced_event(WebInputEvent::Type::kMouseWheel, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests());
    coalesced_event.delta_x = 1;
    coalesced_event.delta_y = 1;

    WebMouseWheelEvent event_to_be_coalesced(WebInputEvent::Type::kMouseWheel, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests());
...
{
    WebGestureEvent coalesced_event(WebInputEvent::Type::kGestureScrollUpdate, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests());
    coalesced_event.data.scroll_update.delta_x = 1;
    coalesced_event.data.scroll_update.delta_y = 1;

    WebGestureEvent event_to_be_coalesced(WebInputEvent::Type::kGestureScrollUpdate, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests());
...
TEST(WebInputEventTest, GesturePinchUpdateCoalescing)
{
    ...
    WebGestureEvent coalesced_event(
        WebInputEvent::Type::kGesturePinchUpdate, WebInputEvent::kNoModifiers, WebInputEvent::GetStaticTimeStampForTests(), WebGestureDevice::kTouchpad);
    ...
    EXPECT_FALSE(WebGestureEvent::IsCompatibleScrollorPinch(event_to_be_coalesced, coalesced_event));
    ...
    coalesced_event.data.pinch_update.scale = 1.1f;
    ASSERT_TRUE(WebGestureEvent::IsCompatibleScrollorPinch(event_to_be_coalesced, coalesced_event));

    std::unique_ptr<WebGestureEvent> logical_scroll, logical_pinch;
    std::tie(logical_scroll, logical_pinch) = WebGestureEvent::CoalesceScrollAndPinch(nullptr, coalesced_event, event_to_be_coalesced);
    ASSERT_NE(nullptr, logical_scroll);
    ASSERT_NE(nullptr, logical_pinch);
    ASSERT_EQ(WebInputEvent::Type::kGestureScrollUpdate, logical_scroll->GetType());
