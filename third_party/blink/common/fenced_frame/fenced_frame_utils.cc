...
bool IsValidFencedFrameURL(const GURL& url)
{
    if (!url.is_valid())
        return false;
    return (url.SchemeIs(url::kHttpsScheme) || url.IsAboutBlank() || IsHttpLocalhost(url))
...
bool IsValidUrnUuidURL(const GURL& url)
{
    ...
    return base::StartsWith(spec, kURNUUIDprefix, base::CompareCase::INSENSITIVE_ASCII)
        && base::Uuid::ParseCaseInsensitive(std::string_view(spec).substr(std::strlen(kURNUUIDprefix))).is_valid();
}

void RecordFencedFrameCreationOutcome(const FencedFrameCreationOutcome outcome)
{
    base::UmaHistogramEnumeration(kFencedFrameCreationOrNavigationOutcomeHistogram, outcome);
}

void RecordOpaqueFencedFrameSizeCoercion(bool did_coerce)
{
    base::UmaHistogramBoolean(kIsOpaqueFencedFrameSizeCoercedHistogram, did_coerce);
}
...
void RecordFencedFrameUnsandboxedFlags(network::mojom::WebSandboxFlags flags)
{
    ...
    for (int32_t i = 1; i <= static_cast<int32_t>(WebSandboxFlags::kMaxValue); i = i << 1) {
        ...
        if ((flags & kFencedFrameMandatoryUnsandboxedFlags & current_mask) != WebSandboxFlags::kNone) {
            base::UmaHistogramSparse(kFencedFrameMandatoryUnsandboxedFlagsSandboxed, i);
        }
    }
}
...
{
    base::UmaHistogramBoolean(kFencedFrameFailedSandboxLoadInTopLevelFrame, is_main_frame);
}
