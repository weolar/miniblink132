...
MessagePortDescriptor::InstrumentationDelegate* g_instrumentation_delegate = nullptr;
...
// static
void MessagePortDescriptor::SetInstrumentationDelegate(InstrumentationDelegate* delegate)
{
...
MessagePortDescriptor::MessagePortDescriptor(MessagePortDescriptor&& message_port)
    : handle_(std::move(message_port.handle_))
    , id_(std::exchange(message_port.id_, base::UnguessableToken::Null()))
    , sequence_number_(std::exchange(message_port.sequence_number_, kInvalidSequenceNumber))
{
}

MessagePortDescriptor& MessagePortDescriptor::operator=(MessagePortDescriptor&& message_port)
{
    Reset();
...
void MessagePortDescriptor::InitializeFromSerializedValues(mojo::ScopedMessagePipeHandle handle, const base::UnguessableToken& id, uint64_t sequence_number)
{
#if DCHECK_IS_ON()
    EnsureValidSerializationState();
    serialization_state_ = {};
...
mojo::ScopedMessagePipeHandle MessagePortDescriptor::TakeHandleForSerialization()
{
#if DCHECK_IS_ON()
...
#if DCHECK_IS_ON()
    DCHECK(!id_.is_empty());
    DCHECK(serialization_state_.took_handle_for_serialization_ || handle_.is_valid()); // Ensures not entangled.
...
mojo::ScopedMessagePipeHandle MessagePortDescriptor::TakeHandleToEntangle(ExecutionContext* execution_context)
{
    EnsureNotSerialized();
    DCHECK(handle_.is_valid());
...
mojo::ScopedMessagePipeHandle MessagePortDescriptor::TakeHandleToEntangleWithEmbedder()
{
    EnsureNotSerialized();
    DCHECK(handle_.is_valid());
...
void MessagePortDescriptor::GiveDisentangledHandle(mojo::ScopedMessagePipeHandle handle)
{
    EnsureNotSerialized();
...
MessagePortDescriptor::MessagePortDescriptor(mojo::ScopedMessagePipeHandle handle)
    : handle_(std::move(handle))
...
void MessagePortDescriptor::NotifyAttached(ExecutionContext* execution_context)
{
    EnsureNotSerialized();
    DCHECK(!id_.is_empty());
    if (g_instrumentation_delegate) {
        g_instrumentation_delegate->NotifyMessagePortAttached(id_, sequence_number_++, execution_context);
    }
}
...
void MessagePortDescriptor::NotifyAttachedToEmbedder()
{
    ...
    DCHECK(!id_.is_empty());
    if (g_instrumentation_delegate) {
        g_instrumentation_delegate->NotifyMessagePortAttachedToEmbedder(id_, sequence_number_++);
    }
}
...
void MessagePortDescriptor::NotifyDetached()
{
    ...
    DCHECK(!id_.is_empty());
    if (g_instrumentation_delegate) {
        g_instrumentation_delegate->NotifyMessagePortDetached(id_, sequence_number_++);
    }
}
...
void MessagePortDescriptor::NotifyDestroyed()
{
    ...
    DCHECK(!id_.is_empty());
    if (g_instrumentation_delegate) {
        g_instrumentation_delegate->NotifyMessagePortDestroyed(id_, sequence_number_++);
    }
}
...
{
#if DCHECK_IS_ON()
    DCHECK(!serialization_state_.took_handle_for_serialization_ && !serialization_state_.took_id_for_serialization_
...
void MessagePortDescriptor::EnsureValidSerializationState() const
{
    ...
    DCHECK((serialization_state_.took_handle_for_serialization_ == serialization_state_.took_id_for_serialization_)
        && (serialization_state_.took_handle_for_serialization_ == serialization_state_.took_sequence_number_for_serialization_));
#endif
}
...
MessagePortDescriptorPair::MessagePortDescriptorPair()
{
    ...
    // each other.
    if (g_instrumentation_delegate) {
        g_instrumentation_delegate->NotifyMessagePortPairCreated(port0_.id(), port1_.id());
    }
}
