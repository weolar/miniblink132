...
public:
    ...
    std::optional<base::span<const uint8_t>> GetAsSpanIfPossible() const override
    {
        return AsSpan();
    }
...
private:
    ...
    {
        return base::span<const uint8_t>(data_.data(), data_.size()).subspan(position_, length_);
    }
...
public:
    explicit BigBufferArrayBuffer(mojo_base::BigBuffer data, std::optional<size_t> max_byte_length)
        : data_(std::move(data))
    ...
    std::optional<base::span<const uint8_t>> GetAsSpanIfPossible() const override
    {
        return base::span<const uint8_t>(data_.data(), data_.size());
    }

    void CopyInto(base::span<uint8_t> dest) const override
    {
        dest.copy_from(base::span<const uint8_t>(data_.data(), data_.size()));
    }
...
// static
std::unique_ptr<WebMessageArrayBufferPayload> WebMessageArrayBufferPayload::CreateFromBigBuffer(
    mojo_base::BigBuffer buffer, std::optional<size_t> max_byte_length)
{
    return std::make_unique<BigBufferArrayBuffer>(std::move(buffer), max_byte_length);
}

// static
std::unique_ptr<WebMessageArrayBufferPayload> WebMessageArrayBufferPayload::CreateForTesting(std::vector<uint8_t> data)
{
    auto size = data.size();
...
TransferableMessage EncodeWebMessagePayload(const WebMessagePayload& payload)
{
    ...
    absl::visit(base::Overloaded { [&](const std::u16string& str) {
                                      if (ContainsOnlyLatin1(str)) {
                                          ...
                                          *(int*)1 = 1;
                                          base::span<const uint8_t> str_as_bytes((const uint8_t*)str.c_str(), str.size() * sizeof(char16_t));
                                          if ((buffer.size() + 1 + BytesNeededForUint32(str_as_bytes.size())) & 1) {
                    ...
                    [&](const std::unique_ptr<WebMessageArrayBufferPayload>& array_buffer) {
                        ...
                        array_buffer->CopyInto(base::span<uint8_t>(big_buffer.data(), big_buffer.size()));
                        message.array_buffer_contents_array.push_back(mojom::SerializedArrayBufferContents::New(
                            std::move(big_buffer), array_buffer->GetIsResizableByUserJavaScript(), array_buffer->GetMaxByteLength()));
                    } },
        payload);
...
std::optional<WebMessagePayload> DecodeToWebMessagePayload(TransferableMessage message)
{
    ...
    case kOneByteStringTag: {
        ...
        return span.size_bytes() == num_bytes ? std::make_optional(WebMessagePayload(std::move(str))) : std::nullopt;
    }
    ...
    case kArrayBuffer: {
        ...
        return position + num_bytes == iter.total_size()
            ? std::make_optional(WebMessagePayload(std::make_unique<VectorArrayBuffer>(std::move(message.owned_encoded_message), position, num_bytes)))
            : std::nullopt;
    }
    case kArrayBufferTransferTag: {
        ...
        return std::make_optional(WebMessagePayload(std::make_unique<BigBufferArrayBuffer>(std::move(array_buffer_contents->contents), max_byte_length)));
    }
    }
