...
CloneableMessage CloneableMessage::ShallowClone() const
{
    ...
    for (const auto& blob : blobs) {
        mojom::SerializedBlobPtr& source_serialized_blob = const_cast<mojom::SerializedBlobPtr&>(blob);

        mojo::Remote<mojom::Blob> source_blob(std::move(source_serialized_blob->blob));
        ...
        clone.blobs.push_back(mojom::SerializedBlob::New(
            source_serialized_blob->uuid, source_serialized_blob->content_type, source_serialized_blob->size, std::move(cloned_blob)));
    ...
    std::vector<mojo::PendingRemote<mojom::FileSystemAccessTransferToken>>& source_tokens
        = const_cast<std::vector<mojo::PendingRemote<mojom::FileSystemAccessTransferToken>>&>(file_system_access_tokens);

    for (auto& token : source_tokens) {
        mojo::Remote<mojom::FileSystemAccessTransferToken> source_token(std::move(token));
