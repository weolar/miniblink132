...
WebMessagePort::Message& WebMessagePort::Message::operator=(Message&&) = default;
WebMessagePort::Message::~Message() = default;
...
WebMessagePort::Message::Message(const std::u16string& data, std::vector<WebMessagePort> ports)
    : data(data)
    , ports(std::move(ports))
{
}

WebMessagePort::Message::Message(const std::u16string& data, WebMessagePort port)
    : data(data)
{
    ports.emplace_back(std::move(port));
}
...
std::pair<WebMessagePort, WebMessagePort> WebMessagePort::CreatePair()
{
    ...
    return std::make_pair(WebMessagePort(port_pair.TakePort0()), WebMessagePort(port_pair.TakePort1()));
}
...
void WebMessagePort::SetReceiver(MessageReceiver* receiver, scoped_refptr<base::SequencedTaskRunner> runner)
{
    DCHECK(receiver);
    DCHECK(runner.get());
    ...
    receiver_ = receiver;
    connector_ = std::make_unique<mojo::Connector>(port_.TakeHandleToEntangleWithEmbedder(), mojo::Connector::SINGLE_THREADED_SEND, std::move(runner));
    connector_->set_incoming_receiver(this);
    connector_->set_connection_error_handler(base::BindOnce(&WebMessagePort::OnPipeError, base::Unretained(this)));
}
...
{
    return connector_ && connector_->is_valid() && !is_closed_ && !is_errored_ && receiver_;
}
...
bool WebMessagePort::PostMessage(Message&& message)
{
    ...
    blink::TransferableMessage transferable_message = blink::EncodeWebMessagePayload(std::move(message.data));
    transferable_message.ports = blink::MessagePortChannel::CreateFromHandles(std::move(ports));
    ...
    // accepted.
    mojo::Message mojo_message = blink::mojom::TransferableMessage::SerializeAsMessage(&transferable_message);
...
bool WebMessagePort::Accept(mojo::Message* mojo_message)
{
    ...
    if (!blink::mojom::TransferableMessage::DeserializeFromMessage(std::move(*mojo_message), &transferable_message)) {
        return false;
    }
    auto ports = std::move(transferable_message.ports);
    // Decode the string portion of the message.
    Message message;
    std::optional<WebMessagePayload> optional_payload = blink::DecodeToWebMessagePayload(std::move(transferable_message));
    if (!optional_payload)
        return false;
