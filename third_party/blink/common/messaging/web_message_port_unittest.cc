...
TEST(WebMessagePortTest, EndToEnd)
{
    ...
    std::u16string message(u"foo");
    {
        ...
        EXPECT_CALL(receiver1, OnMessage(_)).WillOnce(Invoke([&message, &run_loop](Message&& received_message) -> bool {
    ...
    {
        base::RunLoop run_loop;
        EXPECT_CALL(receiver0, OnMessage(_)).WillOnce(Invoke([&message, &run_loop](Message&& received_message) -> bool {
            EXPECT_EQ(message, received_message.data);
    ...
    {
        base::RunLoop run_loop;
        EXPECT_CALL(receiver1, OnPipeError()).WillOnce(Invoke([&run_loop]() { run_loop.Quit(); }));
        port0.Close();
        EXPECT_FALSE(port0.IsValid());
        EXPECT_FALSE(port0.is_errored());
        EXPECT_TRUE(port0.is_closed());
