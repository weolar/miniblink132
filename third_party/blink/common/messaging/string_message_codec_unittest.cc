...
WebMessagePayload DecodeWithV8(const TransferableMessage& message)
{
    ...
    v8::Isolate* isolate = v8::Isolate::New(params);
    {
        ...
        v8::ValueDeserializer deserializer(isolate, message.encoded_message.data(), message.encoded_message.size());
        ...
        if (message.array_buffer_contents_array.size() == 1) {
            ...
            mojo_base::BigBuffer& big_buffer = message.array_buffer_contents_array[0]->contents;
            v8::Local<v8::ArrayBuffer> message_as_array_buffer = v8::ArrayBuffer::New(isolate, big_buffer.size());
            memcpy(message_as_array_buffer->GetBackingStore()->Data(), big_buffer.data(), big_buffer.size());
            deserializer.TransferArrayBuffer(0, message_as_array_buffer);
        }
        EXPECT_TRUE(deserializer.ReadHeader(context).ToChecked());

        v8::Local<v8::Value> value = deserializer.ReadValue(context).ToLocalChecked();
        if (value->IsString()) {
            v8::Local<v8::String> js_str = value->ToString(context).ToLocalChecked();
            std::u16string str;
            str.resize(js_str->Length());
            js_str->Write(isolate, reinterpret_cast<uint16_t*>(&str[0]), 0, str.size());
            result = str;
        }
        if (value->IsArrayBuffer()) {
            ...
            memcpy(array_buffer_contents.data(), js_array_buffer->Data(), js_array_buffer->ByteLength());
            result = WebMessageArrayBufferPayload::CreateForTesting(std::move(array_buffer_contents));
        }
    }
    isolate->Dispose();
...
TransferableMessage EncodeWithV8(const WebMessagePayload& message, const bool transferable = false)
{
    ...
    v8::Isolate* isolate = v8::Isolate::New(params);
    {
        ...
        absl::visit(base::Overloaded { [&](const std::u16string& str) {
                                          v8::Local<v8::String> message_as_value = v8::String::NewFromTwoByte(
                                              isolate, reinterpret_cast<const uint16_t*>(str.data()), v8::NewStringType::kNormal, str.size())
                                                                                       .ToLocalChecked();
                                          EXPECT_TRUE(serializer.WriteValue(context, message_as_value).ToChecked());
                                      },
                        [&](const std::unique_ptr<WebMessageArrayBufferPayload>& array_buffer) {
                            // Create a new JS ArrayBuffer, then transfer into serializer.
                            v8::Local<v8::ArrayBuffer> message_as_array_buffer = v8::ArrayBuffer::New(isolate, array_buffer->GetLength());
                            array_buffer->CopyInto(
                                base::make_span(reinterpret_cast<uint8_t*>(message_as_array_buffer->Data()), message_as_array_buffer->ByteLength()));
                            if (transferable) {
                                ...
                                transferable_message.array_buffer_contents_array.push_back(
                                    mojom::SerializedArrayBufferContents::New(std::move(big_buffer), is_resizable_by_user_js, max_byte_length));
                            }
                            EXPECT_TRUE(serializer.WriteValue(context, message_as_array_buffer).ToChecked());
                        } },
            message);
    ...
    transferable_message.encoded_message = transferable_message.owned_encoded_message;
    return transferable_message;
}

void CheckStringEQ(const std::optional<WebMessagePayload>& optional_payload, const std::u16string& str)
{
...
void CheckVectorEQ(const std::optional<WebMessagePayload>& optional_payload, const std::vector<uint8_t>& buffer)
{
    EXPECT_TRUE(optional_payload);
    auto& payload = optional_payload.value();
    EXPECT_TRUE(absl::holds_alternative<std::unique_ptr<WebMessageArrayBufferPayload>>(payload));
    auto& array_buffer = absl::get<std::unique_ptr<WebMessageArrayBufferPayload>>(payload);
...
TEST(StringMessageCodecTest, SelfTest_ASCII)
{
    ...
    CheckStringEQ(DecodeToWebMessagePayload(EncodeWebMessagePayload(WebMessagePayload(message))), message);
}
...
TEST(StringMessageCodecTest, SelfTest_ArrayBuffer)
{
    ...
    CheckVectorEQ(DecodeToWebMessagePayload(EncodeWebMessagePayload(WebMessageArrayBufferPayload::CreateForTesting(message))), message);
}

TEST(StringMessageCodecTest, SelfToV8Test_ASCII)
{
    std::u16string message = u"hello";
    CheckStringEQ(DecodeWithV8(EncodeWebMessagePayload(WebMessagePayload(message))), message);
}
...
{
    std::vector<uint8_t> message(200, 0xFF);
    CheckVectorEQ(DecodeWithV8(EncodeWebMessagePayload(WebMessageArrayBufferPayload::CreateForTesting(message))), message);
}
...
{
    std::vector<uint8_t> message(200, 0xFF);
    CheckVectorEQ(DecodeToWebMessagePayload(EncodeWithV8(WebMessageArrayBufferPayload::CreateForTesting(message))), message);
}

TEST(StringMessageCodecTest, V8ToSelfTest_ArrayBuffer_transferrable)
{
    std::vector<uint8_t> message(200, 0xFF);
    CheckVectorEQ(DecodeToWebMessagePayload(EncodeWithV8(WebMessageArrayBufferPayload::CreateForTesting(message), true)), message);
}
...
TEST(StringMessageCodecTest, Overflow)
{
    ...
    EXPECT_FALSE(DecodeToWebMessagePayload(TransferableMessageFromRawData(kOverflowOneByteData)));

    const std::vector<uint8_t> kOverflowTwoByteData { 'c', 0xff, 0xff, 0xff, 0x7f };
    EXPECT_FALSE(DecodeToWebMessagePayload(TransferableMessageFromRawData(kOverflowTwoByteData)));
}

TEST(StringMessageCodecTest, InvalidDecode)
{
    auto decode_from_raw = [](std::vector<uint8_t> data) { return DecodeToWebMessagePayload(TransferableMessageFromRawData(std::move(data))); };

    EXPECT_FALSE(decode_from_raw({})) << "no data";
    EXPECT_FALSE(decode_from_raw({ 0xff, 0x01 })) << "only one version";
    EXPECT_FALSE(decode_from_raw({ 0xff, 0x80 })) << "end of buffer during first version";
    EXPECT_FALSE(decode_from_raw({ 0xff, 0x01, 0xff, 0x01 })) << "only two versions";
    EXPECT_FALSE(decode_from_raw({ 0xff, 0x10, 0xff, 0x80 })) << "end of buffer during second version";
    EXPECT_FALSE(decode_from_raw({ 0xff, 0x15, 0xfe, 0xff, 0x01, '"', 0x01, 'a' })) << "end of buffer during trailer offset";
    EXPECT_FALSE(decode_from_raw({ 0xff, 0x15, 0x7f, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0x10, '"', 0x01, 'a' }))
        << "unrecognized trailer offset tag";

    // Confirm that aside from the specific errors above, this encoding is
    // generally correct.
    auto valid_payload
        = decode_from_raw({ 0xff, 0x15, 0xfe, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0x10, '"', 0x01, 'a' });
