...
void Release(mojo::PendingRemote<blink::mojom::ImageReleaseCallback> pending_remote, const gpu::SyncToken& sync_token)
{
    mojo::Remote<blink::mojom::ImageReleaseCallback> remote(std::move(pending_remote));
...
StructTraits<blink::mojom::AcceleratedStaticBitmapImage::DataView, blink::AcceleratedImageInfo>::release_callback(blink::AcceleratedImageInfo& input)
{
    mojo::PendingRemote<blink::mojom::ImageReleaseCallback> callback;
    MakeSelfOwnedReceiver(std::make_unique<ReleaseCallbackImpl>(std::move(input.release_callback)), callback.InitWithNewPipeAndPassReceiver());
    return callback;
}

bool StructTraits<blink::mojom::SharedImageUsageSet::DataView, gpu::SharedImageUsageSet>::Read(
    blink::mojom::SharedImageUsageSet::DataView data, gpu::SharedImageUsageSet* out)
{
    *out = gpu::SharedImageUsageSet(data.usage());
    return true;
}

bool StructTraits<blink::mojom::AcceleratedStaticBitmapImage::DataView, blink::AcceleratedImageInfo>::Read(
    blink::mojom::AcceleratedStaticBitmapImage::DataView data, blink::AcceleratedImageInfo* out)
{
    if (!data.ReadMailboxHolder(&out->mailbox_holder) || !data.ReadImageInfo(&out->image_info)) {
        return false;
    }
    ...
    auto callback = data.TakeReleaseCallback<mojo::PendingRemote<blink::mojom::ImageReleaseCallback>>();
    if (!callback) {
        return false;
    }
