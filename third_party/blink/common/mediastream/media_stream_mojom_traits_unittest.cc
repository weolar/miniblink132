...
TEST(MediaStreamMojomTraitsTest, TrackControlsSerialization_DeviceCaptureStreamTypes)
{
    for (const auto& device_stream_type : { blink::mojom::MediaStreamType::DEVICE_AUDIO_CAPTURE, blink::mojom::MediaStreamType::DEVICE_VIDEO_CAPTURE }) {
        ...
        EXPECT_TRUE(mojo::test::SerializeAndDeserialize<blink::mojom::TrackControls>(input, output));
        ...
        // Too short
        {
            ...
            failing_input.device_ids.push_back(base::ToLowerASCII(base::HexEncode(base::RandBytesAsVector(31))));
            EXPECT_FALSE(mojo::test::SerializeAndDeserialize<blink::mojom::TrackControls>(failing_input, output));
        }

        // Too long
        {
            auto failing_input = input;
            failing_input.device_ids.push_back(base::ToLowerASCII(base::HexEncode(base::RandBytesAsVector(33))));
            EXPECT_FALSE(mojo::test::SerializeAndDeserialize<blink::mojom::TrackControls>(failing_input, output));
        }
        ...
        {
            auto failing_input = input;
            auto id = base::ToLowerASCII(base::HexEncode(base::RandBytesAsVector(31)));
            id += "&*";
        ...
        {
            auto failing_input = input;
            failing_input.device_ids.push_back(base::HexEncode(base::RandBytesAsVector(32)));
            EXPECT_FALSE(mojo::test::SerializeAndDeserialize<blink::mojom::TrackControls>(failing_input, output));
        }
        ...
        // Too many ids
        {
            ...
            EXPECT_TRUE(mojo::test::SerializeAndDeserialize<blink::mojom::TrackControls>(big_input, output));
            EXPECT_EQ(output.device_ids, big_input.device_ids);

            big_input.device_ids.push_back(GetRandomDeviceId());
            EXPECT_FALSE(mojo::test::SerializeAndDeserialize<blink::mojom::TrackControls>(big_input, output));
        }
    }
}
