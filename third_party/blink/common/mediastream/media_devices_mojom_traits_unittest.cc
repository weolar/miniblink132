...
TEST(MediaDevicesMojomTraitsTest, Serialization)
{
    ...
    const media::VideoCaptureControlSupport video_control_support = { .pan = true, .tilt = true, .zoom = true };
    const blink::mojom::FacingMode video_facing = blink::mojom::FacingMode::kEnvironment;
    const media::CameraAvailability availability = media::CameraAvailability::kUnavailableExclusivelyUsedByOtherApplication;
    blink::WebMediaDeviceInfo input(device_id, label, group_id, video_control_support, video_facing, availability);
    blink::WebMediaDeviceInfo output;
    EXPECT_TRUE(mojo::test::SerializeAndDeserialize<blink::mojom::MediaDeviceInfo>(input, output));
