...
template <typename ArrayType> static bool IsOnStack(const ArrayType& a)
{
...
TEST(FixedArrayTest, MoveCtor)
{
    ...
    absl::FixedArray<std::unique_ptr<int>, 10> alloced_copy = std::move(allocated);
    for (int i = 0; i < 15; ++i)
...
TEST(FixedArrayTest, AtThrows)
{
    ...
    EXPECT_EQ(a.at(2), 3);
    ABSL_BASE_INTERNAL_EXPECT_FAIL(a.at(3), std::out_of_range, "failed bounds check");
}
...
template <int stack_elements> static void TestArray(int n)
{
    SCOPED_TRACE(n);
    ...
    EXPECT_EQ(ConstructionTester::constructions, ConstructionTester::destructions);
}

template <int elements_per_inner_array, int inline_elements> static void TestArrayOfArrays(int n)
{
    SCOPED_TRACE(n);
    ...
    ConstructionTester::destructions = 0;
    {
        ...
        auto array_ptr = absl::make_unique<absl::FixedArray<InnerArray, inline_elements>>(n);
        auto& array = *array_ptr;

        ASSERT_EQ(array.size(), n);
        ASSERT_EQ(array.memsize(), sizeof(ConstructionTester) * elements_per_inner_array * n);
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < elements_per_inner_array; j++) {
                ...
                ASSERT_EQ((array.data()[i])[j].get(), (i + 1) * elements_per_inner_array + j);
            }
        }
    } // Close scope containing 'array'.
...
TEST(IteratorConstructorTest, NonInline)
{
    ...
    absl::FixedArray<int, ABSL_ARRAYSIZE(kInput) - 1> const fixed(kInput, kInput + ABSL_ARRAYSIZE(kInput));
...
{
    int const kInput[] = { 2, 3, 5, 7, 11, 13, 17 };
    absl::FixedArray<int, ABSL_ARRAYSIZE(kInput)> const fixed(kInput, kInput + ABSL_ARRAYSIZE(kInput));
    ASSERT_EQ(ABSL_ARRAYSIZE(kInput), fixed.size());
    for (size_t i = 0; i < ABSL_ARRAYSIZE(kInput); ++i) {
        ASSERT_EQ(kInput[i], fixed[i]);
    }
...
{
    char const* kInput[] = { "red", "orange", "yellow", "green", "blue", "indigo", "violet" };
...
TEST(FixedArrayTest, TooBigInlinedSpace)
{
    ...
    static_assert(sizeof(absl::FixedArray<TooBig, 0>) == sizeof(Data), "0-sized absl::FixedArray should have same size as Data.");
    static_assert(alignof(absl::FixedArray<TooBig, 0>) == alignof(Data), "0-sized absl::FixedArray should have same alignment as Data.");
    static_assert(sizeof(absl::FixedArray<TooBig>) == sizeof(Data), "default-sized absl::FixedArray should have same size as Data");
    static_assert(alignof(absl::FixedArray<TooBig>) == alignof(Data), "default-sized absl::FixedArray should have same alignment as Data.");
}
...
TEST(FixedArrayTest, ReverseIteratorInlined)
{
    ...
    int counter = 5;
    for (absl::FixedArray<int>::reverse_iterator iter = a.rbegin(); iter != a.rend(); ++iter) {
        counter--;
    ...
    counter = 5;
    for (absl::FixedArray<int>::const_reverse_iterator iter = a.rbegin(); iter != a.rend(); ++iter) {
        counter--;
        EXPECT_EQ(counter, *iter);
    }
    EXPECT_EQ(counter, 0);
