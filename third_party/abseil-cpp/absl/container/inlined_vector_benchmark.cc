...
int GetNonShortStringOptimizationSize()
{
    ...
    ABSL_RAW_LOG(FATAL, "Failed to find a string larger than the short string optimization");
    return -1;
}
...
void BM_InlinedVectorFillString(benchmark::State& state)
{
    ...
    std::string strings[4] = { std::string(no_sso, 'A'), std::string(no_sso, 'B'), std::string(no_sso, 'C'), std::string(no_sso, 'D') };
...
BENCHMARK(BM_InlinedVectorAssignments)->Arg(0)->Arg(1)->Arg(2)->Arg(3)->Arg(4)->Arg(20);
...
struct LargeCopyableMovableSwappable {
    ...
    LargeCopyableMovableSwappable(const LargeCopyableMovableSwappable& o) = default;
    ...
    LargeCopyableMovableSwappable& operator=(LargeCopyableMovableSwappable&& o) = default;

    friend void swap(LargeCopyableMovableSwappable& a, LargeCopyableMovableSwappable& b)
    {
        using std::swap;
        swap(a.d, b.d);
    }
...
template <typename ElementType> void BM_SwapElements(benchmark::State& state)
{
    const int len = state.range(0);
...
BENCHMARK_TEMPLATE(BM_SwapElements, LargeCopyableMovableSwappable)->Range(0, 1024);
...
template <typename VecType> void BM_Sizeof(benchmark::State& state)
{
    int size = 0;
    for (auto _ : state) {
        VecType vec;
...
template <typename T> using InlVec = absl::InlinedVector<T, kInlinedCapacity>;
...
public:
    ABSL_ATTRIBUTE_NOINLINE NontrivialType()
        : val_()
    {
    ...
    ABSL_ATTRIBUTE_NOINLINE NontrivialType& operator=(const NontrivialType& other)
    {
        val_ = other.val_;
        benchmark::DoNotOptimize(*this);
        return *this;
...
template <typename T, typename PrepareVecFn, typename TestVecFn> void BatchedBenchmark(benchmark::State& state, PrepareVecFn prepare_vec, TestVecFn test_vec)
{
...
template <typename T, size_t ToSize> void BM_ConstructFromSize(benchmark::State& state)
{
    using VecT = InlVec<T>;
    auto size = ToSize;
    BatchedBenchmark<T>(
...
template <typename T, size_t ToSize> void BM_ConstructFromSizeRef(benchmark::State& state)
{
    using VecT = InlVec<T>;
    auto size = ToSize;
    auto ref = T();
...
template <typename T, size_t ToSize> void BM_ConstructFromRange(benchmark::State& state)
{
    using VecT = InlVec<T>;
...
template <typename T, size_t ToSize> void BM_ConstructFromCopy(benchmark::State& state)
{
    using VecT = InlVec<T>;
...
template <typename T, size_t ToSize> void BM_ConstructFromMove(benchmark::State& state)
{
    using VecT = InlVec<T>;
...
template <typename T, size_t FromSize, size_t ToSize> void BM_AssignSizeRef(benchmark::State& state)
{
    auto size = ToSize;
    auto ref = T();
    BatchedBenchmark<T>(
...
template <typename T, size_t FromSize, size_t ToSize> void BM_AssignRange(benchmark::State& state)
{
    std::array<T, ToSize> arr {};
    BatchedBenchmark<T>(
        state,
...
template <typename T, size_t FromSize, size_t ToSize> void BM_AssignFromCopy(benchmark::State& state)
{
...
template <typename T, size_t FromSize, size_t ToSize> void BM_AssignFromMove(benchmark::State& state)
{
    using VecT = InlVec<T>;
    std::array<VecT, kBatchSize> vector_batch {};
    BatchedBenchmark<T>(
...
template <typename T, size_t FromSize, size_t ToSize> void BM_ResizeSize(benchmark::State& state)
{
    BatchedBenchmark<T>(
        state,
        /* prepare_vec = */
...
template <typename T, size_t FromSize, size_t ToSize> void BM_ResizeSizeRef(benchmark::State& state)
{
    auto t = T();
    BatchedBenchmark<T>(
        state,
...
template <typename T, size_t FromSize, size_t ToSize> void BM_InsertSizeRef(benchmark::State& state)
{
    auto t = T();
    BatchedBenchmark<T>(
        state,
...
template <typename T, size_t FromSize, size_t ToSize> void BM_InsertRange(benchmark::State& state)
{
    InlVec<T> other_vec(ToSize);
    BatchedBenchmark<T>(
        state,
...
template <typename T, size_t FromSize> void BM_EmplaceBack(benchmark::State& state)
{
    BatchedBenchmark<T>(
        state,
        /* prepare_vec = */
...
template <typename T, size_t FromSize> void BM_PopBack(benchmark::State& state)
{
    BatchedBenchmark<T>(
        state,
        /* prepare_vec = */
...
template <typename T, size_t FromSize> void BM_EraseOne(benchmark::State& state)
{
    BatchedBenchmark<T>(
        state,
        /* prepare_vec = */
...
template <typename T, size_t FromSize> void BM_EraseRange(benchmark::State& state)
{
    BatchedBenchmark<T>(
        state,
        /* prepare_vec = */
...
template <typename T, size_t FromSize> void BM_Clear(benchmark::State& state)
{
    BatchedBenchmark<T>(
        state,
        /* prepare_vec = */ [](InlVec<T>* vec, size_t) { vec->resize(FromSize); },
...
template <typename T, size_t FromSize, size_t ToCapacity> void BM_Reserve(benchmark::State& state)
{
    BatchedBenchmark<T>(
        state,
        /* prepare_vec = */
...
template <typename T, size_t FromCapacity, size_t ToCapacity> void BM_ShrinkToFit(benchmark::State& state)
{
    BatchedBenchmark<T>(
        state,
        /* prepare_vec = */
...
template <typename T, size_t FromSize, size_t ToSize> void BM_Swap(benchmark::State& state)
{
    using VecT = InlVec<T>;
    std::array<VecT, kBatchSize> vector_batch {};
    BatchedBenchmark<T>(
