...
using MapTypes = ::testing::Types<absl::node_hash_map<int, int, StatefulTestingHash, StatefulTestingEqual, Alloc<std::pair<const int, int>>>,
    absl::node_hash_map<std::string, std::string, StatefulTestingHash, StatefulTestingEqual, Alloc<std::pair<const std::string, std::string>>>>;
...
TEST(NodeHashMap, Emplace)
{
    ...
    m.emplace(std::piecewise_construct, std::forward_as_tuple("a"), std::forward_as_tuple(t));
    ASSERT_EQ(0, t.num_moves());
    ASSERT_EQ(7, t.num_copies());
...
struct NonMovableKeyHash {
    ...
    size_t operator()(const NonMovableKey& k) const
    {
        return k.i;
    }
...
struct NonMovableKeyEq {
    ...
    bool operator()(const NonMovableKey& a, int b) const
    {
        return a.i == b;
    }
};
...
{
    absl::node_hash_map<NonMovableKey, int, NonMovableKeyHash, NonMovableKeyEq> set1, set2;
    ...
    set2.emplace(std::piecewise_construct, std::make_tuple(7), std::make_tuple(-70));
    set2.emplace(std::piecewise_construct, std::make_tuple(19), std::make_tuple(-190));

    auto Elem = [](int key, int value) { return Pair(Field(&NonMovableKey::i, key), value); };
    ...
    EXPECT_THAT(set1, UnorderedElementsAre(Elem(7, -7), Elem(17, -17), Elem(19, -190)));
...
TEST(NodeHashMap, CForEach)
{
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        }
        {
            SCOPED_TRACE("const object iteration");
            std::vector<std::pair<int, int>> v;
            absl::container_internal::c_for_each_fast(node_hash_map<int, int>(m), [&v](std::pair<const int, int>& p) { v.push_back(p); });
            EXPECT_THAT(v, UnorderedElementsAreArray(expected));
        }
        m[i] = i;
