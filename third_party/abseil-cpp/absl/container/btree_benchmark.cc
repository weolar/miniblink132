...
template <typename V> std::vector<V> GenerateValues(int n)
{
...
template <typename T> void BM_InsertImpl(benchmark::State& state, bool sorted)
{
...
template <typename T> void BM_Insert(benchmark::State& state)
{
    BM_InsertImpl<T>(state, false);
}

template <typename T> void BM_InsertSorted(benchmark::State& state)
{
...
template <typename T> void BM_InsertSmall(benchmark::State& state)
{
    using V = typename remove_pair_const<typename T::value_type>::type;
...
template <typename T> void BM_LookupImpl(benchmark::State& state, bool sorted)
{
    using V = typename remove_pair_const<typename T::value_type>::type;
    typename KeyOfValue<typename T::key_type, V>::type key_of_value;
...
template <typename T> void BM_Lookup(benchmark::State& state)
{
...
// yields a full tree.
template <typename T> void BM_FullLookup(benchmark::State& state)
{
    BM_LookupImpl<T>(state, true);
}

// Benchmark erasing values from a container.
template <typename T> void BM_Erase(benchmark::State& state)
{
    using V = typename remove_pair_const<typename T::value_type>::type;
    typename KeyOfValue<typename T::key_type, V>::type key_of_value;
    std::vector<V> values = GenerateValues<V>(kBenchmarkValues);
...
template <typename T> void BM_EraseRange(benchmark::State& state)
{
    using V = typename remove_pair_const<typename T::value_type>::type;
    typename KeyOfValue<typename T::key_type, V>::type key_of_value;
    std::vector<V> values = GenerateValues<V>(kBenchmarkValues);
...
template <typename T> void BM_EraseIf(benchmark::State& state)
{
    using V = typename remove_pair_const<typename T::value_type>::type;
    std::vector<V> values = GenerateValues<V>(kBenchmarkValues);
...
template <typename T> void BM_QueueAddRem(benchmark::State& state)
{
    using V = typename remove_pair_const<typename T::value_type>::type;
    typename KeyOfValue<typename T::key_type, V>::type key_of_value;
...
template <typename T> void BM_MixedAddRem(benchmark::State& state)
{
    using V = typename remove_pair_const<typename T::value_type>::type;
    typename KeyOfValue<typename T::key_type, V>::type key_of_value;
...
template <typename T> void BM_Fifo(benchmark::State& state)
{
    using V = typename remove_pair_const<typename T::value_type>::type;
...
template <typename T> void BM_FwdIter(benchmark::State& state)
{
    using V = typename remove_pair_const<typename T::value_type>::type;
...
template <typename T> void BM_RangeConstructionImpl(benchmark::State& state, bool sorted)
{
    using V = typename remove_pair_const<typename T::value_type>::type;
...
template <typename T> void BM_InsertRangeRandom(benchmark::State& state)
{
    BM_RangeConstructionImpl<T>(state, false);
}

template <typename T> void BM_InsertRangeSorted(benchmark::State& state)
{
...
#define STL_UNORDERED_TYPES(value)                                                                                                                             \
    ...
    using stl_unordered_multimap_##value = std::unordered_multimap<value, intptr_t>
...
#define STL_UNORDERED_TYPES_CUSTOM_HASH(value, hash)                                                                                                           \
    ...
    using stl_unordered_multimap_##value = std::unordered_multimap<value, intptr_t, hash>
...
#define BTREE_TYPES(value)                                                                                                                                     \
    using btree_256_set_##value = btree_set<value, std::less<value>, std::allocator<value>>;                                                                   \
    using btree_256_map_##value = btree_map<value, intptr_t, std::less<value>, std::allocator<std::pair<const value, intptr_t>>>;                              \
    using btree_256_multiset_##value = btree_multiset<value, std::less<value>, std::allocator<value>>;                                                         \
    using btree_256_multimap_##value = btree_multimap<value, intptr_t, std::less<value>, std::allocator<std::pair<const value, intptr_t>>>
...
#define MY_BENCHMARK4(type, func)                                                                                                                              \
    void BM_##type##_##func(benchmark::State& state)                                                                                                           \
    {                                                                                                                                                          \
        BM_##func<type>(state);                                                                                                                                \
    }                                                                                                                                                          \
...
template <int Size, int Copies> struct BigType {
    BigType()
        : BigType(0)
    {
    }
    explicit BigType(int x)
    {
        std::iota(values.begin(), values.end(), x);
    }
    ...
    bool operator<(const BigType& other) const
    {
        return std::lexicographical_compare(
            values.begin(), values.begin() + std::min(Size, Copies), other.values.begin(), other.values.begin() + std::min(Size, Copies));
    }
    bool operator==(const BigType& other) const
    {
        return std::equal(values.begin(), values.begin() + std::min(Size, Copies), other.values.begin());
    }

    // Support absl::Hash.
    template <typename State> friend State AbslHashValue(State h, const BigType& b)
    {
        for (int i = 0; i < Size && i < Copies; ++i)
...
#define BIG_TYPE_BENCHMARKS(SIZE, COPIES)                                                                                                                      \
    ...
    using stl_map_size##SIZE##copies##COPIES = std::map<BigType<SIZE, COPIES>, intptr_t>;                                                                      \
    using stl_multiset_size##SIZE##copies##COPIES = std::multiset<BigType<SIZE, COPIES>>;                                                                      \
    using stl_multimap_size##SIZE##copies##COPIES = std::multimap<BigType<SIZE, COPIES>, intptr_t>;                                                            \
    using stl_unordered_set_size##SIZE##copies##COPIES = std::unordered_set<BigType<SIZE, COPIES>, absl::Hash<BigType<SIZE, COPIES>>>;                         \
    using stl_unordered_map_size##SIZE##copies##COPIES = std::unordered_map<BigType<SIZE, COPIES>, intptr_t, absl::Hash<BigType<SIZE, COPIES>>>;               \
    using flat_hash_set_size##SIZE##copies##COPIES = flat_hash_set<BigType<SIZE, COPIES>>;                                                                     \
    using flat_hash_map_size##SIZE##copies##COPIES = flat_hash_map<BigType<SIZE, COPIES>, intptr_t>;                                                           \
    using stl_unordered_multiset_size##SIZE##copies##COPIES = std::unordered_multiset<BigType<SIZE, COPIES>, absl::Hash<BigType<SIZE, COPIES>>>;               \
    using stl_unordered_multimap_size##SIZE##copies##COPIES = std::unordered_multimap<BigType<SIZE, COPIES>, intptr_t, absl::Hash<BigType<SIZE, COPIES>>>;     \
    using btree_256_set_size##SIZE##copies##COPIES = btree_set<BigType<SIZE, COPIES>>;                                                                         \
    using btree_256_map_size##SIZE##copies##COPIES = btree_map<BigType<SIZE, COPIES>, intptr_t>;                                                               \
    using btree_256_multiset_size##SIZE##copies##COPIES = btree_multiset<BigType<SIZE, COPIES>>;                                                               \
    using btree_256_multimap_size##SIZE##copies##COPIES = btree_multimap<BigType<SIZE, COPIES>, intptr_t>;                                                     \
...
template <int Size> struct BigTypePtr {
    BigTypePtr()
        : BigTypePtr(0)
    {
    }
    ...
    bool operator<(const BigTypePtr& other) const
    {
...
template <int Size> double ContainerInfo(const btree_set<BigTypePtr<Size>>& b)
{
    const double bytes_used = b.bytes_used() + b.size() * sizeof(BigType<Size, Size>);
    const double bytes_per_value = bytes_used / b.size();
    BtreeContainerInfoLog(b, bytes_used, bytes_per_value);
    return bytes_per_value;
}
template <int Size> double ContainerInfo(const btree_map<int, BigTypePtr<Size>>& b)
{
    const double bytes_used = b.bytes_used() + b.size() * sizeof(BigType<Size, Size>);
    const double bytes_per_value = bytes_used / b.size();
    BtreeContainerInfoLog(b, bytes_used, bytes_per_value);
...
#define BIG_TYPE_PTR_BENCHMARKS(SIZE)                                                                                                                          \
    ...
    using stl_map_size##SIZE##copies##SIZE##ptr = std::map<int, BigType<SIZE, SIZE>>;                                                                          \
    using stl_unordered_set_size##SIZE##copies##SIZE##ptr = std::unordered_set<BigType<SIZE, SIZE>, absl::Hash<BigType<SIZE, SIZE>>>;                          \
    using stl_unordered_map_size##SIZE##copies##SIZE##ptr = std::unordered_map<int, BigType<SIZE, SIZE>>;                                                      \
    using flat_hash_set_size##SIZE##copies##SIZE##ptr = flat_hash_set<BigType<SIZE, SIZE>>;                                                                    \
    using flat_hash_map_size##SIZE##copies##SIZE##ptr = flat_hash_map<int, BigTypePtr<SIZE>>;                                                                  \
    using btree_256_set_size##SIZE##copies##SIZE##ptr = btree_set<BigTypePtr<SIZE>>;                                                                           \
    using btree_256_map_size##SIZE##copies##SIZE##ptr = btree_map<int, BigTypePtr<SIZE>>;                                                                      \
...
void BM_BtreeSet_IteratorSubtraction(benchmark::State& state)
{
    ...
    while (state.KeepRunningBatch(distance)) {
        ...
        benchmark::DoNotOptimize(set.find(static_cast<int>(end)) - set.find(static_cast<int>(begin)));
