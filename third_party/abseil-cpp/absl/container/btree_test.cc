...
template <typename T, typename U> void CheckPairEquals(const T& x, const U& y)
{
    ABSL_INTERNAL_CHECK(x == y, "Values are unequal.");
}

template <typename T, typename U, typename V, typename W> void CheckPairEquals(const std::pair<T, U>& x, const std::pair<V, W>& y)
{
...
template <typename TreeType, typename CheckerType> class base_checker {
public:
...
public:
    ...
    template <typename IterType, typename CheckerIterType> IterType iter_check(IterType tree_iter, CheckerIterType checker_iter) const
    {
        if (tree_iter == tree_.end()) {
            ABSL_INTERNAL_CHECK(checker_iter == checker_.end(), "Checker iterator not at end.");
        } else {
            CheckPairEquals(*tree_iter, *checker_iter);
        }
        return tree_iter;
    }
    template <typename IterType, typename CheckerIterType> IterType riter_check(IterType tree_iter, CheckerIterType checker_iter) const
    {
        if (tree_iter == tree_.rend()) {
            ABSL_INTERNAL_CHECK(checker_iter == checker_.rend(), "Checker iterator not at rend.");
        } else {
            CheckPairEquals(*tree_iter, *checker_iter);
        }
        return tree_iter;
    ...
    {
        std::pair<typename CheckerType::iterator, typename CheckerType::iterator> checker_res = checker_.equal_range(key);
    ...
    {
        std::pair<typename CheckerType::const_iterator, typename CheckerType::const_iterator> checker_res = checker_.equal_range(key);
    ...
    void erase(iterator begin, iterator end)
    {
        ...
        auto checker_end = end == tree_.end() ? checker_.end() : checker_.lower_bound(end.key());
        if (end != tree_.end()) {
        ...
        EXPECT_EQ(std::distance(checker_.begin(), checker_ret), std::distance(tree_.begin(), tree_ret));
        EXPECT_EQ(tree_.size(), checker_.size());
...
template <typename TreeType, typename CheckerType> class unique_checker : public base_checker<TreeType, CheckerType> {
...
public:
    ...
    unique_checker(const unique_checker& other)
        : super_type(other)
    {
    }
    ...
    std::pair<iterator, bool> insert(const value_type& v)
    {
        ...
        std::pair<typename CheckerType::iterator, bool> checker_res = this->checker_.insert(v);
...
// {multiset,multimap}.
template <typename TreeType, typename CheckerType> class multi_checker : public base_checker<TreeType, CheckerType> {
    using super_type = base_checker<TreeType, CheckerType>;
...
public:
    ...
    multi_checker(const multi_checker& other)
        : super_type(other)
    {
    }
    template <class InputIterator>
...
template <typename T, typename V> void DoTest(const char* name, T* b, const std::vector<V>& values)
{
...
template <typename T> void ConstTest()
{
...
template <typename T, typename C> void BtreeTest()
{
    ConstTest<T>();

    using V = typename remove_pair_const<typename T::value_type>::type;
    const std::vector<V> random_values
        = GenerateValuesWithSeed<V>(absl::GetFlag(FLAGS_test_values), 4 * absl::GetFlag(FLAGS_test_values), GTEST_FLAG_GET(random_seed));
...
template <typename T, typename C> void BtreeMultiTest()
{
    ConstTest<T>();
    ...
    std::fill(identical_values.begin(), identical_values.end(), Generator<V>(2)(2));
    DoTest("identical: ", &container, identical_values);
}

template <typename T> void BtreeMapTest()
{
    using value_type = typename T::value_type;
...
template <typename T> void BtreeMultiMapTest()
{
    using mapped_type = typename T::mapped_type;
    mapped_type m = Generator<mapped_type>(0)(0);
    (void)m;
...
template <typename K, int N = 256> void SetTest()
{
    EXPECT_EQ(sizeof(absl::btree_set<K>), 2 * sizeof(void*) + sizeof(typename absl::btree_set<K>::size_type));
    using BtreeSet = absl::btree_set<K>;
    BtreeTest<BtreeSet, std::set<K>>();
}

template <typename K, int N = 256> void MapTest()
{
    EXPECT_EQ(sizeof(absl::btree_map<K, K>), 2 * sizeof(void*) + sizeof(typename absl::btree_map<K, K>::size_type));
...
template <typename K, int N = 256> void MultiSetTest()
{
    EXPECT_EQ(sizeof(absl::btree_multiset<K>), 2 * sizeof(void*) + sizeof(typename absl::btree_multiset<K>::size_type));
    using BtreeMSet = absl::btree_multiset<K>;
    BtreeMultiTest<BtreeMSet, std::multiset<K>>();
}

template <typename K, int N = 256> void MultiMapTest()
{
    EXPECT_EQ(sizeof(absl::btree_multimap<K, K>), 2 * sizeof(void*) + sizeof(typename absl::btree_multimap<K, K>::size_type));
...
TEST(Btree, multiset_cord)
{
    MultiSetTest<absl::Cord>();
}
...
TEST(Btree, multimap_cord)
{
    MultiMapTest<absl::Cord>();
}
...
template <typename T> bool CanEraseWithEmptyBrace(T t, decltype(t.erase({}))*)
{
    return true;
}

template <typename T> bool CanEraseWithEmptyBrace(T, ...)
{
    return false;
}

template <typename T> void TestHeterogeneous(T table)
{
...
TEST(Btree, HeterogeneousLookup)
{
    ...
    TestHeterogeneous(btree_map<std::string, int, CompareIntToString> { { "1", 1 }, { "3", 3 }, { "5", 5 } });
    TestHeterogeneous(btree_multiset<std::string, CompareIntToString> { "1", "3", "5" });
    TestHeterogeneous(btree_multimap<std::string, int, CompareIntToString> { { "1", 1 }, { "3", 3 }, { "5", 5 } });

    // Only maps have .at()
    btree_map<std::string, int, CompareIntToString> map { { "", -1 }, { "1", 1 }, { "3", 3 }, { "5", 5 } };
...
TEST(Btree, NoHeterogeneousLookupWithoutAlias)
{
    ...
    using StringMultiSet = absl::btree_multiset<std::string, NonTransparentCompare>;
    StringMultiSet ms;
    ms.insert("hello");
    ms.insert("world");
    ms.insert("world");
...
struct SubstringLess {
    ...
    bool operator()(const std::string& a, const std::string& b) const
    {
        return absl::string_view(a).substr(0, n) < absl::string_view(b).substr(0, n);
    }
    int n;
};
...
public:
    ...
    template <typename ValueType> constexpr static size_t GetTargetNodeSize(size_t target_values_per_node)
    {
        return btree_node<set_params<ValueType, std::less<ValueType>, std::allocator<ValueType>,
    ...
    template <typename Btree> constexpr static size_t GetNumSlotsPerNode()
    {
        return btree_node<typename Btree::params_type>::kNodeSlots;
    }

    template <typename Btree> constexpr static size_t GetMaxFieldType()
    {
        return std::numeric_limits<typename btree_node<typename Btree::params_type>::field_type>::max();
    }

    template <typename Btree> constexpr static bool UsesLinearNodeSearch()
    {
        return btree_node<typename Btree::params_type>::use_linear_search::value;
    }

    template <typename Btree> constexpr static bool FieldTypeEqualsSlotType()
    {
        return std::is_same<typename btree_node<typename Btree::params_type>::field_type, typename btree_node<typename Btree::params_type>::slot_type>::value;
    }
};
...
public:
    ...
    template <typename K, typename C> static bool IsLinear()
    {
...
template <typename Compare, typename Key> void AssertKeyCompareStringAdapted()
{
    using Adapted = typename key_compare_adapter<Compare, Key>::type;
    static_assert(std::is_same<Adapted, StringBtreeDefaultLess>::value || std::is_same<Adapted, StringBtreeDefaultGreater>::value,
        "key_compare_adapter should have string-adapted this comparator.");
}
template <typename Compare, typename Key> void AssertKeyCompareNotStringAdapted()
{
    using Adapted = typename key_compare_adapter<Compare, Key>::type;
    static_assert(!std::is_same<Adapted, StringBtreeDefaultLess>::value && !std::is_same<Adapted, StringBtreeDefaultGreater>::value,
...
TEST(Btree, KeyCompareAdapter)
{
    ...
    AssertKeyCompareStringAdapted<std::less<absl::string_view>, absl::string_view>();
    AssertKeyCompareStringAdapted<std::greater<absl::string_view>, absl::string_view>();
...
template <typename Cmp> struct CheckedCompareOptedOutCmp : Cmp, BtreeTestOnlyCheckedCompareOptOutBase {
    using Cmp::Cmp;
    CheckedCompareOptedOutCmp()
    {
    }
    CheckedCompareOptedOutCmp(Cmp cmp)
        : Cmp(std::move(cmp))
    {
    } // NOLINT
};
...
class SizedBtreeSet : public btree_set_container<
                          btree<set_params<Key, CheckedCompareOptedOutCmp<Cmp>, std::allocator<Key>, BtreeNodePeer::GetTargetNodeSize<Key>(TargetValuesPerNode),
                              /*Multi=*/false>>> {
...
void ExpectOperationCounts(const int expected_moves, const int expected_comparisons, const std::vector<int>& values, InstanceTracker* tracker, Set* set)
{
...
{
    if (kAsan)
        GTEST_SKIP() << "We do extra operations in ASan mode.";
    ...
    std::vector<int> values = GenerateValuesWithSeed<int>(10000, 1 << 22, /*seed=*/23);
...
struct MovableOnlyInstanceThreeWayCompare {
    absl::weak_ordering operator()(const MovableOnlyInstance& a, const MovableOnlyInstance& b) const
    {
        return a.compare(b);
    }
};
...
TEST(Btree, MovesComparisonsCopiesSwapsTrackingThreeWayCompare)
{
    ...
    InstanceTracker tracker;
    // Note: this is minimum number of values per node.
    SizedBtreeSet<MovableOnlyInstance, /*TargetValuesPerNode=*/4, MovableOnlyInstanceThreeWayCompare> set4;
    // Note: this is the default number of values per node for a set of int32s
    // (with 64-bit pointers).
    SizedBtreeSet<MovableOnlyInstance, /*TargetValuesPerNode=*/61, MovableOnlyInstanceThreeWayCompare> set61;
    SizedBtreeSet<MovableOnlyInstance, /*TargetValuesPerNode=*/100, MovableOnlyInstanceThreeWayCompare> set100;
...
TEST(Btree, MapValueCompProtected)
{
    ...
    struct value_comp_child : public value_compare {
        explicit value_comp_child(key_compare kc)
            : value_compare(kc)
        {
        }
        int GetId() const
...
template <typename Set> void TestExtractWithTrackingForSet()
{
    InstanceTracker tracker;
    {
        Set s;
...
template <typename Map> void TestExtractWithTrackingForMap()
{
    InstanceTracker tracker;
    {
        Map m;
        ...
        const size_t kSize = 1000;
        while (m.size() < kSize) {
            m.insert({ CopyableMovableInstance(m.size()), MovableOnlyInstance(m.size()) });
        }
        for (int i = 0; i < kSize; ++i) {
            // Extract with key
...
TEST(Btree, ExtractTracking)
{
    ...
    TestExtractWithTrackingForMap<absl::btree_map<CopyableMovableInstance, MovableOnlyInstance>>();
    TestExtractWithTrackingForMap<absl::btree_multimap<CopyableMovableInstance, MovableOnlyInstance>>();
}
...
TEST(Btree, ExtractAndInsertNodeHandleMap)
{
    ...
    absl::btree_map<int, int>::insert_return_type res = other.insert(std::move(nh));
...
TEST(Btree, ExtractDoesntCauseExtraMoves)
{
    ...
    std::array<std::function<void(Set&)>, 3> extracters = { [](Set& s) { auto node = s.extract(s.begin()); },
        [](Set& s) { auto ret = s.extract_and_get_next(s.begin()); }, [](Set& s) { auto node = s.extract(MovableOnlyInstance(0)); } };
...
TEST(Btree, InsertHintNodeHandle)
{
    ...
    absl::btree_multiset<InsertMultiHintData, InsertMultiHintDataKeyCompare> src = { { 1, 2 }, { 3, 4 }, { 3, 5 } };
    auto nh = src.extract(src.lower_bound(3));
    EXPECT_EQ(nh.value(), (InsertMultiHintData { 3, 4 }));
    absl::btree_multiset<InsertMultiHintData, InsertMultiHintDataKeyCompare> other = { { 3, 1 }, { 3, 2 }, { 3, 3 } };
    auto it = other.insert(--other.end(), std::move(nh));
    EXPECT_THAT(other, ElementsAre(InsertMultiHintData { 3, 1 }, InsertMultiHintData { 3, 2 }, InsertMultiHintData { 3, 4 }, InsertMultiHintData { 3, 3 }));
    ...
    EXPECT_THAT(other,
        ElementsAre(InsertMultiHintData { 3, 5 }, InsertMultiHintData { 3, 1 }, InsertMultiHintData { 3, 2 }, InsertMultiHintData { 3, 4 },
...
TEST(Btree, MergeIntoMultiMapsWithDifferentComparators)
{
    ...
    absl::btree_multimap<int, int, std::greater<int>> src2 = { { 5, 5 }, { 4, 1 }, { 4, 4 }, { 3, 2 } };
...
struct KeyCompareToWeakOrdering {
    template <typename T> absl::weak_ordering operator()(const T& a, const T& b) const
    {
        return a < b ? absl::weak_ordering::less : a == b ? absl::weak_ordering::equivalent : absl::weak_ordering::greater;
    }
};

struct KeyCompareToStrongOrdering {
    template <typename T> absl::strong_ordering operator()(const T& a, const T& b) const
    {
        return a < b ? absl::strong_ordering::less : a == b ? absl::strong_ordering::equal : absl::strong_ordering::greater;
    }
};
...
TEST(Btree, TryEmplaceBasicTest)
{
    ...
    EXPECT_THAT(m, ElementsAreArray(std::vector<std::pair<int, std::string>> { { 1, "one" }, { 2, "two" }, { 42, "aaa" } }));
}
...
TEST(Btree, TryEmplaceWithBadHint)
{
    ...
    EXPECT_EQ(it, ++m.begin());
    EXPECT_THAT(m, ElementsAreArray(std::vector<std::pair<int, int>> { { 1, 1 }, { 2, 2 }, { 9, 9 } }));

    // Bad hint, too large this time:
    it = m.try_emplace(++(++m.begin()), 0, 0);
    EXPECT_EQ(it, m.begin());
    EXPECT_THAT(m, ElementsAreArray(std::vector<std::pair<int, int>> { { 0, 0 }, { 1, 1 }, { 2, 2 }, { 9, 9 } }));
}
...
template <typename Alloc> using BtreeSetAlloc = absl::btree_set<int, std::less<int>, Alloc>;
...
TEST(Btree, EraseIf)
{
    ...
    }
    {
        ...
        EXPECT_EQ(erase_if(m, [](std::pair<const int, int> kv) { return kv.first > 3; }), 2);
        EXPECT_THAT(m, ElementsAre(Pair(1, 1), Pair(3, 3)));
    }
    {
        absl::btree_multimap<int, int> m = { { 1, 1 }, { 3, 3 }, { 3, 6 }, { 6, 6 }, { 6, 7 }, { 100, 6 } };
        EXPECT_EQ(erase_if(m, [](std::pair<const int, int> kv) { return kv.second == 6; }), 3);
...
TEST(Btree, InsertOrAssign)
{
    ...
    EXPECT_THAT(m, ElementsAre(Pair(-1, 1), Pair(0, 1), Pair(1, 1), Pair(3, 200), Pair(4, 4)));
}
...
struct ConstructorCounted {
    explicit ConstructorCounted(int i)
        : i(i)
    {
        ++constructor_calls;
    }
...
struct ConstructorCountedCompare {
    bool operator()(int a, const ConstructorCounted& b) const
    {
        return a < b.i;
    }
    bool operator()(const ConstructorCounted& a, int b) const
    {
        return a.i < b;
    }
    bool operator()(const ConstructorCounted& a, const ConstructorCounted& b) const
    {
        return a.i < b.i;
    }
    using is_transparent = void;
...
TEST(Btree, SetRangeConstructorAndInsertExplicitConvComparableLimitConstruction)
{
    const int i[] = { 0, 1, 1 };
    ConstructorCounted::constructor_calls = 0;

    absl::btree_set<ConstructorCounted, ConstructorCountedCompare> set { std::begin(i), std::end(i) };
...
TEST(Btree, SetRangeConstructorAndInsertSupportExplicitConversionNonComparable)
{
    const int i[] = { 0, 1 };
...
TEST(Btree, MapRangeConstructorAndInsertSupportExplicitConversionComparable)
{
    ...
    absl::btree_map<std::string, int> name_map1 { std::begin(names), std::end(names) };
...
TEST(Btree, MapRangeConstructorAndInsertExplicitConvComparableLimitConstruction)
{
    const std::pair<int, int> i[] = { { 0, 1 }, { 1, 2 }, { 1, 3 } };
    ConstructorCounted::constructor_calls = 0;

    absl::btree_map<ConstructorCounted, int, ConstructorCountedCompare> map { std::begin(i), std::end(i) };
...
TEST(Btree, MapRangeConstructorAndInsertSupportExplicitConversionNonComparable)
{
    const std::pair<int, int> i[] = { { 0, 1 }, { 1, 2 } };

    absl::btree_map<std::vector<void*>, int> m1 { std::begin(i), std::end(i) };
    EXPECT_THAT(m1, ElementsAre(Pair(IsEmpty(), 1), Pair(ElementsAre(IsNull()), 2)));

    absl::btree_map<std::vector<void*>, int> m2;
    m2.insert(std::begin(i), std::end(i));
    EXPECT_THAT(m2, ElementsAre(Pair(IsEmpty(), 1), Pair(ElementsAre(IsNull()), 2)));
}
...
struct MultiKeyComp {
    ...
    bool operator()(const int a, const MultiKey b) const
    {
        return a < b.i1;
    }
    bool operator()(const MultiKey a, const int b) const
    {
        return a.i1 < b;
    }
};
...
template <typename Compare> class BtreeMultiKeyTest : public ::testing::Test { };
...
{
    const absl::btree_set<MultiKey, TypeParam> set = { { 1, 1 }, { 2, 1 }, { 2, 2 }, { 3, 1 } };
...
{
    using Set = absl::btree_set<int>;
    EXPECT_TRUE((std::is_same<typename Set::iterator::reference, const int&>::value));
    EXPECT_TRUE((std::is_same<typename Set::iterator::pointer, const int*>::value));

    using MSet = absl::btree_multiset<int>;
    EXPECT_TRUE((std::is_same<typename MSet::iterator::reference, const int&>::value));
    EXPECT_TRUE((std::is_same<typename MSet::iterator::pointer, const int*>::value));
}
...
template <typename Compare> struct TransparentPassThroughComp {
    using is_transparent = void;
...
TEST(Btree, SupportsTransparentComparatorThatDoesNotImplementAllVisibleOperators)
{
...
TEST(Btree, InvalidComparatorsCaught)
{
    ...
    }
    {
        ...
        EXPECT_DEATH(set.insert({ 0, 1, 2 }), R"regex(\!lhs_comp_rhs \|\| !comp\(\)\(rhs, lhs\))regex");
    }
    {
    ...
    ClockTime c = { 6, 0 };
    {
        struct NonTransitiveTimeCmp {
            ...
            {
                if (lhs.hour.has_value() && rhs.hour.has_value() && *lhs.hour != *rhs.hour) {
    ...
    }
    {
        struct ThreeWayNonTransitiveTimeCmp {
            ...
            {
                if (lhs.hour.has_value() && rhs.hour.has_value() && *lhs.hour != *rhs.hour) {
                    return *lhs.hour < *rhs.hour ? absl::weak_ordering::less : absl::weak_ordering::greater;
                }
...
TEST(Btree, InvalidIteratorUse)
{
    ...
    constexpr const char* kInvalidMemoryDeathMessage = "use-after-free|invalidated iterator";
...
class OnlyConstructibleByAllocator {
    explicit OnlyConstructibleByAllocator(int i)
        : i_(i)
    {
    }
...
public:
    ...
    OnlyConstructibleByAllocator& operator=(const OnlyConstructibleByAllocator& other)
    {
        i_ = other.i_;
        return *this;
    }
...
private:
    template <typename T> friend class OnlyConstructibleAllocator;

    int i_;
};

template <typename T = OnlyConstructibleByAllocator> class OnlyConstructibleAllocator : public std::allocator<T> {
public:
    OnlyConstructibleAllocator() = default;
    template <class U> explicit OnlyConstructibleAllocator(const OnlyConstructibleAllocator<U>&)
    {
    }
    ...
    template <typename Pair> void construct(Pair* p, const int i)
    {
...
struct OnlyConstructibleByAllocatorComp {
    using is_transparent = void;
    bool operator()(OnlyConstructibleByAllocator a, OnlyConstructibleByAllocator b) const
    {
        return a.Get() < b.Get();
    }
...
TEST(Btree, OnlyConstructibleByAllocatorType)
{
    ...
    {
        absl::btree_set<OnlyConstructibleByAllocator, OnlyConstructibleByAllocatorComp, OnlyConstructibleAllocator<>> set;
        set.emplace(1);
    ...
    {
        absl::btree_multiset<OnlyConstructibleByAllocator, OnlyConstructibleByAllocatorComp, OnlyConstructibleAllocator<>> set;
        set.emplace(1);
        set.emplace_hint(set.end(), 2);
    ...
    {
        absl::btree_map<OnlyConstructibleByAllocator, int, OnlyConstructibleByAllocatorComp, OnlyConstructibleAllocator<>> map;
        map.emplace(1);
        map.emplace_hint(map.end(), 2);
        map.insert(arr.begin(), arr.end());
        EXPECT_THAT(map, ElementsAre(Pair(1, 1), Pair(2, 2), Pair(3, 3), Pair(4, 4)));
    }
    {
        absl::btree_multimap<OnlyConstructibleByAllocator, int, OnlyConstructibleByAllocatorComp, OnlyConstructibleAllocator<>> map;
        map.emplace(1);
        map.emplace_hint(map.end(), 2);
        // TODO(ezb): fix insert_multi to allow this to compile.
        // map.insert(arr.begin(), arr.end());
...
class NotAssignable {
public:
    explicit NotAssignable(int i)
        : i_(i)
    {
    }
    NotAssignable(const NotAssignable& other)
        : i_(other.i_)
    {
    }
...
TEST(Btree, NotAssignableType)
{
    ...
    }
    {
        ...
        EXPECT_THAT(map, ElementsAre(Pair(1, 1), Pair(2, 2), Pair(2, 3), Pair(3, 3)));
        map.erase(map.begin());
...
template <typename T> class ArenaLikeAllocator : public std::allocator<T> {
public:
    ...
    explicit ArenaLikeAllocator(ArenaLike* arena) noexcept
        : arena_(arena)
    {
    }
...
TEST(Btree, IteratorSubtraction)
{
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        ASSERT_EQ(end - begin, set.find(end) - set.find(begin)) << begin << " " << end;
    }
}
...
TEST(Btree, InvalidIteratorComparison)
{
    ...
    constexpr const char* kValueInitDeathMessage = "Comparing default-constructed iterator with .*non-default-constructed "
                                                   "iterator";
    ...
    constexpr const char* kDifferentContainerDeathMessage = "Comparing iterators from different containers";
    iter1 = set1.begin();
    iter2 = set2.begin();
...
TEST(Btree, InvalidPointerUse)
{
    ...
    for (int i = 2; i < slots_per_node - 1; ++i)
        set.insert(i);
    ptr = &*set.begin();
...
template <typename Set> void TestBasicFunctionality(Set set)
{
    using value_type = typename Set::value_type;
    for (int i = 0; i < 100; ++i) {
        set.insert(value_type(i));
    }
    for (int i = 50; i < 100; ++i) {
        set.erase(value_type(i));
    }
    auto it = set.begin();
...
template <size_t align> struct alignas(align) OveralignedKey {
    explicit OveralignedKey(int i)
        : key(i)
    {
    }
    bool operator<(const OveralignedKey& other) const
    {
...
TEST(Btree, OveralignedKey)
{
    ...
    TestBasicFunctionality(SizedBtreeSet<OveralignedKey<16>, /*TargetValuesPerNode=*/8>());
    TestBasicFunctionality(SizedBtreeSet<OveralignedKey<16>, /*TargetValuesPerNode=*/9>());
}
