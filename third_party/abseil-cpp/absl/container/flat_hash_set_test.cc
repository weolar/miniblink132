...
template <class T> using Set = absl::flat_hash_set<T, StatefulTestingHash, StatefulTestingEqual, Alloc<T>>;
    ...
    for (int i = 0; i < 100; ++i) {
        {
            ...
            absl::container_internal::c_for_each_fast(s, [&v](const ValueType& p) { v.push_back(p); });
        ...
        }
        {
            ...
            absl::container_internal::c_for_each_fast(cs, [&v](const ValueType& p) { v.push_back(p); });
            ASSERT_THAT(v, UnorderedElementsAreArray(expected));
        }
        {
            SCOPED_TRACE("temporary object iteration");
            std::vector<ValueType> v;
            absl::container_internal::c_for_each_fast(flat_hash_set<ValueType>(s), [&v](const ValueType& p) { v.push_back(p); });
            ASSERT_THAT(v, UnorderedElementsAreArray(expected));
        }
        s.emplace(i, i);
...
public:
    explicit PoisonSoo(int64_t d)
        : data_(d)
    {
        SanitizerPoisonObject(&data_);
    }
    PoisonSoo(const PoisonSoo& that)
        : PoisonSoo(*that)
    {
    }
    ~PoisonSoo()
    ...
    template <typename H> friend H AbslHashValue(H h, const PoisonSoo& pi)
    {
        return H::combine(std::move(h), *pi);
    }
    bool operator==(const PoisonSoo& rhs) const
    {
        return **this == *rhs;
    }
};
...
{
    EXPECT_TRUE((decltype(FlatHashSetPolicy<int>::destroy<std::allocator<int>>(nullptr, nullptr))()));
    EXPECT_FALSE((decltype(FlatHashSetPolicy<int>::destroy<CountingAllocator<int>>(nullptr, nullptr))()));
    EXPECT_FALSE((decltype(FlatHashSetPolicy<std::unique_ptr<int>>::destroy<std::allocator<int>>(nullptr, nullptr))()));
}
