...
template <class UnordMap> class ModifiersTest : public ::testing::Test { };
...
template <class V> using IfNotVoid = typename std::enable_if<!std::is_void<V>::value, V>::type;
...
struct EraseFirst {
    template <class Map> auto operator()(Map* m, int) const -> IfNotVoid<decltype(m->erase(m->begin()))>
    {
        return m->erase(m->begin());
    }
    template <class Map> typename Map::iterator operator()(Map* m, ...) const
    {
        auto it = m->begin();
        m->erase(it++);
        return it;
...
REGISTER_TYPED_TEST_SUITE_P(ModifiersTest, Clear, Insert, InsertHint, InsertRange, InsertWithinCapacity, InsertRangeWithinCapacity, InsertOrAssign,
    InsertOrAssignHint, Emplace, EmplaceHint, TryEmplace, TryEmplaceHint, Erase, EraseRange, EraseKey, Swap);
...
template <class UnordMap> class UniquePtrModifiersTest : public ::testing::Test {
protected:
