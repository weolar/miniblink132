...
struct RawHashSetTestOnlyAccess {
    template <typename C> static auto GetSlots(const C& c) -> decltype(c.slots_)
    {
        return c.slots_;
    }
};
...
struct IntPolicy {
    ...
    static void construct(void*, int64_t* slot, int64_t v)
    {
        *slot = v;
    }
    ...
    static int64_t& element(slot_type* slot)
    {
        return *slot;
    }

    template <class F> static auto apply(F&& f, int64_t x) -> decltype(std::forward<F>(f)(x, x))
    {
...
struct IntTable : raw_hash_set<IntPolicy, container_internal::hash_default_hash<int64_t>, std::equal_to<int64_t>, std::allocator<int64_t>> {
...
struct string_generator {
    template <class RNG> std::string operator()(RNG& rng) const
    {
        std::string res;
        res.resize(size);
...
void BM_CacheInSteadyState(benchmark::State& state)
{
    ...
    while (state.KeepRunning()) {
        ...
        // Some cache misses.
        for (int i = 0; i != 10; ++i)
            ::benchmark::DoNotOptimize(t.find(gen(rng)));
...
template <typename Benchmark> void CacheInSteadyStateArgs(Benchmark* bm)
{
    // The default.
    ...
    for (size_t i = 0; i != kNumPoints; ++i)
        bm->Arg(std::ceil(capacity * (max_load_factor + i * max_load_factor / kNumPoints) / 2));
}
...
template <typename CtrlIter> void Iota(CtrlIter begin, CtrlIter end, int value)
{
...
void BM_DropDeletes(benchmark::State& state)
{
    ...
    std::vector<ctrl_t> pattern
        = { ctrl_t::kEmpty, static_cast<ctrl_t>(2), ctrl_t::kDeleted, static_cast<ctrl_t>(2), ctrl_t::kEmpty, static_cast<ctrl_t>(1), ctrl_t::kDeleted };
...
void BM_EraseIf(benchmark::State& state)
{
    ...
    for (auto& k : keys) {
        ...
        for (int64_t i = 0; i < num_elements; i++) {
            ...
            k.push_back(absl::Uniform<int64_t>(rng, 0, std::numeric_limits<int64_t>::max()));
        ...
        threshold.push_back(static_cast<int64_t>(num_erased) < num_elements ? k[num_erased] : std::numeric_limits<int64_t>::max());
    }

    while (state.KeepRunningBatch(static_cast<int64_t>(kRepetitions) * std::max(num_elements, int64_t { 1 }))) {
...
bool CodegenAbslRawHashSetStringFindNeEnd(absl::container_internal::StringTable* table, const std::string& key)
{
    return table->find(key) != table->end();
}
...
int odr
    = (::benchmark::DoNotOptimize(std::make_tuple(&CodegenAbslRawHashSetInt64Find, &CodegenAbslRawHashSetInt64FindNeEnd, &CodegenAbslRawHashSetStringFindNeEnd,
           &CodegenAbslRawHashSetInt64Insert, &CodegenAbslRawHashSetInt64Contains, &CodegenAbslRawHashSetInt64Iterate)),
        1);
