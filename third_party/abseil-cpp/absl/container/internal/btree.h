...
template <typename Compare, typename T, typename U> using compare_result_t = absl::result_of_t<const Compare(const T&, const U&)>;

// A helper class that indicates if the Compare parameter is a key-compare-to
// comparator.
template <typename Compare, typename T> using btree_is_key_compare_to = std::is_convertible<compare_result_t<Compare, T, T>, absl::weak_ordering>;
...
template <typename Compare, bool is_class = std::is_class<Compare>::value> struct checked_compare_base : Compare {
...
template <typename Compare> struct checked_compare_base<Compare, false> {
    explicit checked_compare_base(Compare c)
        : compare(std::move(c))
    {
    }
...
// is valid.
template <typename Compare, typename Key> struct key_compare_adapter {
    ...
    private:
        ...
        template <typename T> bool is_self_equivalent(const T&) const
        {
            return true;
        }
    ...
    using type = absl::conditional_t<std::is_base_of<BtreeTestOnlyCheckedCompareOptOutBase, Compare>::value, Compare, checked_compare>;
};

template <> struct key_compare_adapter<std::less<std::string>, std::string> {
    using type = StringBtreeDefaultLess;
};

template <> struct key_compare_adapter<std::greater<std::string>, std::string> {
    using type = StringBtreeDefaultGreater;
};

template <> struct key_compare_adapter<std::less<absl::string_view>, absl::string_view> {
    using type = StringBtreeDefaultLess;
};

template <> struct key_compare_adapter<std::greater<absl::string_view>, absl::string_view> {
    using type = StringBtreeDefaultGreater;
};

template <> struct key_compare_adapter<std::less<absl::Cord>, absl::Cord> {
    using type = StringBtreeDefaultLess;
};

template <> struct key_compare_adapter<std::greater<absl::Cord>, absl::Cord> {
    using type = StringBtreeDefaultGreater;
};
...
template <typename T, typename = void> struct has_linear_node_search_preference : std::false_type { };
template <typename T, typename = void> struct prefers_linear_node_search : std::false_type { };
template <typename T> struct has_linear_node_search_preference<T, absl::void_t<typename T::absl_btree_prefer_linear_node_search>> : std::true_type { };
template <typename T>
struct prefers_linear_node_search<T, absl::void_t<typename T::absl_btree_prefer_linear_node_search>> : T::absl_btree_prefer_linear_node_search { };

template <typename Compare, typename Key> constexpr bool compare_has_valid_result_type()
{
    using compare_result_type = compare_result_t<Compare, Key, Key>;
    return std::is_same<compare_result_type, bool>::value || std::is_convertible<compare_result_type, absl::weak_ordering>::value;
}

template <typename original_key_compare, typename value_type> class map_value_compare {
...
public:
    auto operator()(const value_type& lhs, const value_type& rhs) const -> decltype(comp(lhs.first, rhs.first))
    {
...
struct common_params : common_policy_traits<SlotPolicy> {
    ...
    static constexpr bool kIsKeyCompareStringAdapted
        = std::is_same<key_compare, StringBtreeDefaultLess>::value || std::is_same<key_compare, StringBtreeDefaultGreater>::value;
    static constexpr bool kIsKeyCompareTransparent = IsTransparent<original_key_compare>::value || kIsKeyCompareStringAdapted;
    ...
    template <typename LookupKey> constexpr static bool can_have_multiple_equivalent_keys()
    {
        return IsMulti || (IsTransparent<key_compare>::value && !std::is_same<LookupKey, Key>::value && !kIsKeyCompareStringAdapted);
    }
    ...
    using node_count_type = absl::conditional_t<(kNodeSlotSpace / sizeof(slot_type) > (std::numeric_limits<uint8_t>::max)()), uint16_t, uint8_t>; // NOLINT
};
...
template <typename Compare> struct upper_bound_adapter {
    ...
    template <typename K1, typename K2> bool operator()(const K1& a, const K2& b) const
    {
...
public:
    ...
    using use_linear_search = std::integral_constant<bool,
        has_linear_node_search_preference<original_key_compare>::value ? prefers_linear_node_search<original_key_compare>::value
            : has_linear_node_search_preference<key_type>::value       ? prefers_linear_node_search<key_type>::value
                                                                       : std::is_arithmetic<key_type>::value
                && (std::is_same<std::less<key_type>, original_key_compare>::value || std::is_same<std::greater<key_type>, original_key_compare>::value)>;
...
private:
    ...
    constexpr static size_type NodeTargetSlots(const size_type begin, const size_type end)
    {
        return begin == end                                                        ? begin
            : SizeWithNSlots((begin + end) / 2 + 1) > params_type::kTargetNodeSize ? NodeTargetSlots(begin, (begin + end) / 2)
                                                                                   : NodeTargetSlots((begin + end) / 2 + 1, end);
    }

    constexpr static size_type kTargetNodeSize = params_type::kTargetNodeSize;
    constexpr static size_type kNodeTargetSlots = NodeTargetSlots(0, kTargetNodeSize);
    ...
    constexpr static size_type kNodeSlots = kNodeTargetSlots >= kMinNodeSlots ? kNodeTargetSlots : kMinNodeSlots;
    ...
    constexpr static leaf_layout_type LeafLayout(const size_type slot_count = kNodeSlots)
    {
    ...
    template <size_type N> inline typename layout_type::template ElementType<N>* GetField()
    {
        // We assert that we don't read from values that aren't there.
        assert(N < 4 || is_internal());
        return InternalLayout().template Pointer<N>(reinterpret_cast<char*>(this));
    }
    template <size_type N> inline const typename layout_type::template ElementType<N>* GetField() const
    {
        assert(N < 4 || is_internal());
...
public:
    ...
    template <typename K> SearchResult<size_type, is_key_compare_to::value> lower_bound(const K& k, const key_compare& comp) const
    {
        return use_linear_search::value ? linear_search(k, comp) : binary_search(k, comp);
    }
    // Returns the position of the first value whose key is greater than k.
    template <typename K> size_type upper_bound(const K& k, const key_compare& comp) const
    {
    ...
    SearchResult<size_type, btree_is_key_compare_to<Compare, key_type>::value> linear_search(const K& k, const Compare& comp) const
    {
        return linear_search_impl(k, start(), finish(), comp, btree_is_key_compare_to<Compare, key_type>());
    }

    template <typename K, typename Compare>
    SearchResult<size_type, btree_is_key_compare_to<Compare, key_type>::value> binary_search(const K& k, const Compare& comp) const
    {
    ...
    template <typename K, typename Compare>
    SearchResult<size_type, false> linear_search_impl(const K& k, size_type s, const size_type e, const Compare& comp, std::false_type /* IsCompareTo */) const
    {
        while (s < e) {
            if (!comp(key(s), k)) {
                break;
    ...
    template <typename K, typename Compare>
    SearchResult<size_type, true> linear_search_impl(const K& k, size_type s, const size_type e, const Compare& comp, std::true_type /* IsCompareTo */) const
    {
        while (s < e) {
    ...
    template <typename K, typename Compare>
    SearchResult<size_type, false> binary_search_impl(const K& k, size_type s, size_type e, const Compare& comp, std::false_type /* IsCompareTo */) const
    {
        while (s != e) {
    ...
    SearchResult<size_type, true> binary_search_impl(const K& k, size_type s, size_type e, const CompareTo& comp, std::true_type /* IsCompareTo */) const
    {
    ...
    template <typename Compare> bool is_ordered_correctly(field_type i, const Compare& comp) const
    {
        if (std::is_base_of<BtreeTestOnlyCheckedCompareOptOutBase, Compare>::value || params_type::kIsKeyCompareStringAdapted) {
            return true;
        }

        const auto compare = [&](field_type a, field_type b) {
            const absl::weak_ordering cmp = compare_internal::do_three_way_comparison(comp, key(a), key(b));
            return cmp < 0 ? -1 : cmp > 0 ? 1 : 0;
        };
        int cmp = -1;
        constexpr bool kCanHaveEquivKeys = params_type::template can_have_multiple_equivalent_keys<key_type>();
    ...
    template <typename... Args> void emplace_value(field_type i, allocator_type* alloc, Args&&... args);
    ...
    void init_leaf(field_type position, field_type max_count, btree_node* parent)
    {
        ...
        absl::container_internal::SanitizerPoisonMemoryRegion(start_slot(), max_count * sizeof(slot_type));
    }
    void init_internal(field_type position, btree_node* parent)
    {
        ...
        absl::container_internal::SanitizerPoisonMemoryRegion(&mutable_child(start()), (kNodeSlots + 1) * sizeof(btree_node*));
    }
...
private:
    template <typename... Args> void value_init(const field_type i, allocator_type* alloc, Args&&... args)
    {
        next_generation();
    ...
    {
        next_generation();
        for (slot_type *src = src_node->slot(src_i), *end = src + n, *dest = slot(dest_i); src != end; ++src, ++dest) {
    ...
    {
        next_generation();
        for (slot_type *src = src_node->slot(src_i + n), *end = src - n, *dest = slot(dest_i + n); src != end; --src, --dest) {
    ...
    template <typename N, typename R, typename P> friend class btree_iterator;
    friend class BtreeNodePeer;
    friend struct btree_access;
};

template <typename Node> bool AreNodesFromSameContainer(const Node* node_a, const Node* node_b)
{
...
public:
    ...
    template <typename Node> void update_generation(const Node* node)
    {
        if (node != nullptr)
    ...
    template <typename Node> void assert_valid_generation(const Node* node) const
    {
...
template <typename Node, typename Reference, typename Pointer> class btree_iterator : private btree_iterator_generation_info {
    ...
    using iterator = absl::conditional_t<is_map_container::value, btree_iterator<normal_node, normal_reference, normal_pointer>,
...
public:
    ...
    template <typename N, typename R, typename P,
        absl::enable_if_t<std::is_same<btree_iterator<N, R, P>, iterator>::value && std::is_same<btree_iterator, const_iterator>::value, int> = 0>
...
private:
    ...
    template <typename TreeType, typename CheckerType> friend class base_checker;
    friend struct btree_access;
    ...
    template <typename N, typename R, typename P,
        absl::enable_if_t<std::is_same<btree_iterator<N, R, P>, const_iterator>::value && std::is_same<btree_iterator, iterator>::value, int> = 0>
    ...
    {
        ABSL_HARDENING_ASSERT(((node_ == nullptr && other.node_ == nullptr) || (node_ != nullptr && other.node_ != nullptr))
...
private:
    ...
    template <typename Btree> void copy_or_move_values_in_order(Btree& other);
...
public:
    ...
    template <typename K> std::pair<iterator, bool> lower_bound_equal(const K& key) const;
    ...
    template <typename K, typename... Args> std::pair<iterator, bool> insert_unique(const K& key, Args&&... args);
    ...
    // Requirement: if `key` already exists in the btree, does not consume `args`.
    // Requirement: `key` is never referenced after consuming `args`.
    template <typename K, typename... Args> std::pair<iterator, bool> insert_hint_unique(iterator position, const K& key, Args&&... args);

    // Insert a range of values into the btree.
    // Note: the first overload avoids constructing a value_type if the key
    // already exists in the btree.
    template <typename InputIterator,
        typename = decltype(std::declval<const key_compare&>()(params_type::key(*std::declval<InputIterator>()), std::declval<const key_type&>()))>
    void insert_iterator_unique(InputIterator b, InputIterator e, int);
    // We need the second overload for cases in which we need to construct a
    // value_type in order to compare it with the keys already in the btree.
    template <typename InputIterator> void insert_iterator_unique(InputIterator b, InputIterator e, char);
    ...
    template <typename K1, typename K2> bool compare_keys(const K1& a, const K2& b) const
    {
...
private:
    ...
    {
        return reinterpret_cast<node_type*>(absl::container_internal::Allocate<node_type::Alignment()>(mutable_allocator(), size));
    }
    ...
    template <typename K> SearchResult<iterator, is_key_compare_to::value> internal_lower_bound(const K& key) const;
...
////
// btree_node methods
template <typename P> template <typename... Args> inline void btree_node<P>::emplace_value(const field_type i, allocator_type* alloc, Args&&... args)
{
    assert(i >= start());
    assert(i <= finish());
...
template <typename P> inline void btree_node<P>::remove_values(const field_type i, const field_type to_erase, allocator_type* alloc)
{
...
template <typename P> void btree_node<P>::rebalance_right_to_left(field_type to_move, btree_node* right, allocator_type* alloc)
{
...
template <typename P> void btree_node<P>::rebalance_left_to_right(field_type to_move, btree_node* right, allocator_type* alloc)
{
    assert(parent() == right->parent());
    assert(position() + 1 == right->position());
...
template <typename P> void btree_node<P>::split(const int insert_position, btree_node* dest, allocator_type* alloc)
{
    ...
    if (is_internal()) {
        for (field_type i = dest->start(), j = finish() + 1; i <= dest->finish(); ++i, ++j) {
...
template <typename P> void btree_node<P>::merge(btree_node* src, allocator_type* alloc)
{
    ...
    if (is_internal()) {
        // Move the child pointers from the right to the left node.
        for (field_type i = src->start(), j = finish() + 1; i <= src->finish(); ++i, ++j) {
...
template <typename P> void btree_node<P>::clear_and_delete(btree_node* node, allocator_type* alloc)
{
    if (node->is_leaf()) {
...
template <typename N, typename R, typename P> auto btree_iterator<N, R, P>::distance_slow(const_iterator other) const -> difference_type
{
...
////
// btree methods
template <typename P> template <typename Btree> void btree<P>::copy_or_move_values_in_order(Btree& other)
{
...
template <typename P> constexpr bool btree<P>::static_assert_validation()
{
    ...
    static_assert(kNodeSlots < (1 << (8 * sizeof(typename node_type::field_type))), "target node size too large");

    // Verify that key_compare returns an absl::{weak,strong}_ordering or bool.
    static_assert(compare_has_valid_result_type<key_compare, key_type>(),
...
template <typename P> template <typename K> auto btree<P>::lower_bound_equal(const K& key) const -> std::pair<iterator, bool>
{
    const SearchResult<iterator, is_key_compare_to::value> res = internal_lower_bound(key);
    const iterator lower = iterator(internal_end(res.value));
    const bool equal = res.HasMatch() ? res.IsEq() : lower != end() && !compare_keys(key, lower.key());
    return { lower, equal };
}

template <typename P> template <typename K> auto btree<P>::equal_range(const K& key) -> std::pair<iterator, iterator>
{
...
template <typename P> template <typename K, typename... Args> auto btree<P>::insert_unique(const K& key, Args&&... args) -> std::pair<iterator, bool>
{
    if (empty()) {
...
inline auto btree<P>::insert_hint_unique(iterator position, const K& key, Args&&... args) -> std::pair<iterator, bool>
{
    if (!empty()) {
...
template <typename P> template <typename InputIterator, typename> void btree<P>::insert_iterator_unique(InputIterator b, InputIterator e, int)
{
    for (; b != e; ++b) {
...
template <typename P> template <typename InputIterator> void btree<P>::insert_iterator_unique(InputIterator b, InputIterator e, char)
{
    for (; b != e; ++b) {
        // Use a node handle to manage a temp slot.
        auto node_handle = CommonAccess::Construct<node_handle_type>(get_allocator(), *b);
        slot_type* slot = CommonAccess::GetSlot(node_handle);
        insert_hint_unique(end(), params_type::key(slot), slot);
    }
}

template <typename P> template <typename ValueType> auto btree<P>::insert_multi(const key_type& key, ValueType&& v) -> iterator
{
    if (empty()) {
        mutable_root() = mutable_rightmost() = new_leaf_root_node(1);
    }
...
template <typename P> template <typename ValueType> auto btree<P>::insert_hint_multi(iterator position, ValueType&& v) -> iterator
{
    if (!empty()) {
...
template <typename P> template <typename InputIterator> void btree<P>::insert_iterator_multi(InputIterator b, InputIterator e)
{
    for (; b != e; ++b) {
...
template <typename P> auto btree<P>::operator=(const btree& other) -> btree&
{
    if (this != &other) {
        clear();

        *mutable_key_comp() = other.key_comp();
        if (absl::allocator_traits<allocator_type>::propagate_on_container_copy_assignment::value) {
...
template <typename P> auto btree<P>::operator=(btree&& other) noexcept -> btree&
{
    if (this != &other) {
        clear();

        using std::swap;
        if (absl::allocator_traits<allocator_type>::propagate_on_container_move_assignment::value) {
...
template <typename P> auto btree<P>::erase(iterator iter) -> iterator
{
    ...
    if (internal_delete) {
        ...
        internal_iter.node_->transfer(static_cast<size_type>(internal_iter.position_), static_cast<size_type>(iter.position_), iter.node_, mutable_allocator());
    } else {
        ...
        const field_type transfer_from = static_cast<field_type>(iter.position_ + 1);
...
template <typename P> auto btree<P>::erase_range(iterator begin, iterator end) -> std::pair<size_type, iterator>
{
    ...
    if (begin.node_ == end.node_) {
        ...
        begin.node_->remove_values(static_cast<field_type>(begin.position_), static_cast<field_type>(end.position_ - begin.position_), mutable_allocator());
        size_ -= count;
    ...
    while (size_ > target_size) {
        if (begin.node_->is_leaf()) {
            ...
            const size_type remaining_in_node = static_cast<size_type>(begin.node_->finish() - begin.position_);
            const field_type to_erase = static_cast<field_type>((std::min)(remaining_to_erase, remaining_in_node));
...
template <typename P> void btree<P>::swap(btree& other)
{
    using std::swap;
    if (absl::allocator_traits<allocator_type>::propagate_on_container_swap::value) {
        // Note: `rightmost_` also contains the allocator and the key comparator.
        swap(rightmost_, other.rightmost_);
    } else {
    ...
    if (node != root()) {
        if (node->position() > parent->start()) {
            ...
            if (left->count() < kNodeSlots) {
                ...
                field_type to_move = (kNodeSlots - left->count()) / (1 + (static_cast<field_type>(insert_position) < kNodeSlots));
                to_move = (std::max)(field_type { 1 }, to_move);

                if (static_cast<field_type>(insert_position) - to_move >= node->start() || left->count() + to_move < kNodeSlots) {
                    left->rebalance_right_to_left(to_move, node, mutable_allocator());

                    assert(node->max_count() - node->count() == to_move);
                    insert_position = static_cast<int>(static_cast<field_type>(insert_position) - to_move);
        ...
        if (node->position() < parent->finish()) {
            ...
            if (right->count() < kNodeSlots) {
                ...
                field_type to_move = (kNodeSlots - right->count()) / (1 + (insert_position > node->start()));
                to_move = (std::max)(field_type { 1 }, to_move);

                if (static_cast<field_type>(insert_position) <= node->finish() - to_move || right->count() + to_move < kNodeSlots) {
...
template <typename P> bool btree<P>::try_merge_or_rebalance(iterator* iter)
{
    ...
    if (iter->node_->position() < parent->finish()) {
        ...
        if (right->count() > kMinNodeValues && (iter->node_->count() == 0 || iter->position_ > iter->node_->start())) {
            field_type to_move = (right->count() - iter->node_->count()) / 2;
            to_move = (std::min)(to_move, static_cast<field_type>(right->count() - 1));
    ...
    if (iter->node_->position() > parent->start()) {
        ...
        node_type* left = parent->child(iter->node_->position() - 1);
        if (left->count() > kMinNodeValues && (iter->node_->count() == 0 || iter->position_ < iter->node_->finish())) {
...
template <typename P> template <typename... Args> inline auto btree<P>::internal_emplace(iterator iter, Args&&... args) -> iterator
{
    ...
    const auto transfer_and_delete = [&](node_type* old_node, node_type* new_node) {
    ...
    if (iter.node_->count() == max_count) {
        ...
        if (max_count < kNodeSlots) {
            ...
            replace_leaf_root_node(static_cast<field_type>((std::min)(static_cast<int>(kNodeSlots), 2 * max_count)));
            replaced_node = true;
        } else {
    ...
    (void)replaced_node;
#if defined(ABSL_HAVE_ADDRESS_SANITIZER) || defined(ABSL_HAVE_HWADDRESS_SANITIZER)
    if (!replaced_node) {
        assert(iter.node_->is_leaf());
    ...
    assert(iter.node_->is_ordered_correctly(static_cast<field_type>(iter.position_), original_key_compare(key_comp()))
...
template <typename P> template <typename K> inline auto btree<P>::internal_locate(const K& key) const -> SearchResult<iterator, is_key_compare_to::value>
{
    iterator iter(const_cast<node_type*>(root()));
    for (;;) {
        SearchResult<size_type, is_key_compare_to::value> res = iter.node_->lower_bound(key, key_comp());
...
template <typename P> template <typename K> auto btree<P>::internal_lower_bound(const K& key) const -> SearchResult<iterator, is_key_compare_to::value>
{
    if (!params_type::template can_have_multiple_equivalent_keys<K>()) {
...
struct btree_access {
    template <typename BtreeContainer, typename Pred> static auto erase_if(BtreeContainer& container, Pred pred) -> typename BtreeContainer::size_type
    {
