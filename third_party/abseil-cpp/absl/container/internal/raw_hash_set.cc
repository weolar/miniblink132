...
alignas(16) ABSL_CONST_INIT ABSL_DLL const ctrl_t kEmptyGroup[32] = { ZeroCtrlT(), ZeroCtrlT(), ZeroCtrlT(), ZeroCtrlT(), ZeroCtrlT(), ZeroCtrlT(), ZeroCtrlT(),
    ZeroCtrlT(), ZeroCtrlT(), ZeroCtrlT(), ZeroCtrlT(), ZeroCtrlT(), ZeroCtrlT(), ZeroCtrlT(), ZeroCtrlT(), ZeroCtrlT(), ctrl_t::kSentinel, ctrl_t::kEmpty,
    ctrl_t::kEmpty, ctrl_t::kEmpty, ctrl_t::kEmpty, ctrl_t::kEmpty, ctrl_t::kEmpty, ctrl_t::kEmpty, ctrl_t::kEmpty, ctrl_t::kEmpty, ctrl_t::kEmpty,
    ctrl_t::kEmpty, ctrl_t::kEmpty, ctrl_t::kEmpty, ctrl_t::kEmpty, ctrl_t::kEmpty };
...
ABSL_CONST_INIT ABSL_DLL const ctrl_t kSooControl[17]
    = { ZeroCtrlT(), ctrl_t::kSentinel, ZeroCtrlT(), ctrl_t::kEmpty, ctrl_t::kEmpty, ctrl_t::kEmpty, ctrl_t::kEmpty, ctrl_t::kEmpty, ctrl_t::kEmpty,
          ctrl_t::kEmpty, ctrl_t::kEmpty, ctrl_t::kEmpty, ctrl_t::kEmpty, ctrl_t::kEmpty, ctrl_t::kEmpty, ctrl_t::kEmpty, ctrl_t::kEmpty };
static_assert(NumControlBytes(SooCapacity()) <= 17, "kSooControl capacity too small");
...
bool ShouldRehashForBugDetection(const ctrl_t* ctrl, size_t capacity)
{
    ...
    return probe(ctrl, capacity, absl::HashOf(RandomSeed())).offset() < RehashProbabilityConstant();
}
...
GenerationType* EmptyGeneration()
{
    if (SwisstableGenerationsEnabled()) {
        ...
        return const_cast<GenerationType*>(&kEmptyGenerations[RandomSeed() % kNumEmptyGenerations]);
    }
    return nullptr;
}
...
bool CommonFieldsGenerationInfoEnabled::should_rehash_for_bug_detection_on_move(const ctrl_t* ctrl, size_t capacity) const
{
    return ShouldRehashForBugDetection(ctrl, capacity);
}
...
void DropDeletesWithoutResize(CommonFields& common, const PolicyFunctions& policy)
{
    void* set = &common;
    ...
    for (size_t i = 0; i != capacity; ++i, slot_ptr = NextSlot(slot_ptr, slot_size)) {
        ...
        const auto probe_index = [probe_offset, capacity](size_t pos) { return ((pos - probe_offset) & capacity) / Group::kWidth; };
...
static bool WasNeverFull(CommonFields& c, size_t index)
{
    ...
    return empty_before && empty_after && static_cast<size_t>(empty_after.TrailingZeros()) + empty_before.LeadingZeros() < Group::kWidth;
}
...
void HashSetResizeHelper::GrowIntoSingleGroupShuffleControlBytes(ctrl_t* __restrict new_ctrl, size_t new_capacity) const
{
    ...
    copied_bytes = absl::little_endian::Load64(old_ctrl() + half_old_capacity + 1);
    ...
    // bandwidth.
    constexpr uint64_t kEmptyXorSentinel = static_cast<uint8_t>(ctrl_t::kEmpty) ^ static_cast<uint8_t>(ctrl_t::kSentinel);
    const uint64_t mask_convert_old_sentinel_to_empty = kEmptyXorSentinel << (half_old_capacity * 8);
    ...
    // position            /      S
    std::memset(new_ctrl + old_capacity_ + 1, static_cast<int8_t>(ctrl_t::kEmpty), Group::kWidth);
    ...
    // position                   S       /
    std::memset(new_ctrl + NumControlBytes(new_capacity) - kHalfWidth, static_cast<int8_t>(ctrl_t::kEmpty), kHalfWidth);
    ...
    // position                   S        /
    std::memset(new_ctrl + new_capacity + old_capacity_ + 2, static_cast<int8_t>(ctrl_t::kEmpty), kQuarterWidth);

    // Finally, we set the new sentinel byte.
    new_ctrl[new_capacity] = ctrl_t::kSentinel;
}

void HashSetResizeHelper::InitControlBytesAfterSoo(ctrl_t* new_ctrl, ctrl_t h2, size_t new_capacity)
{
    assert(is_single_group(new_capacity));
    std::memset(new_ctrl, static_cast<int8_t>(ctrl_t::kEmpty), NumControlBytes(new_capacity));
    assert(HashSetResizeHelper::SooSlotIndex() == 1);
...
void HashSetResizeHelper::GrowIntoSingleGroupShuffleTransferableSlots(void* new_slots, size_t slot_size) const
{
    assert(old_capacity_ > 0);
    const size_t half_old_capacity = old_capacity_ / 2;

    SanitizerUnpoisonMemoryRegion(old_slots(), slot_size * old_capacity_);
    std::memcpy(new_slots, SlotAddress(old_slots(), half_old_capacity + 1, slot_size), slot_size * half_old_capacity);
    std::memcpy(SlotAddress(new_slots, half_old_capacity + 1, slot_size), old_slots(), slot_size * (half_old_capacity + 1));
}

void HashSetResizeHelper::GrowSizeIntoSingleGroupTransferable(CommonFields& c, size_t slot_size)
{
...
void HashSetResizeHelper::TransferSlotAfterSoo(CommonFields& c, size_t slot_size)
{
    assert(was_soo_);
    assert(had_soo_slot_);
    assert(is_single_group(c.capacity()));
    std::memcpy(SlotAddress(c.slot_array(), SooSlotIndex(), slot_size), old_soo_data(), slot_size);
    PoisonSingleGroupEmptySlots(c, slot_size);
}
...
FindInfo FindInsertPositionWithGrowthOrRehash(CommonFields& common, size_t hash, const PolicyFunctions& policy)
{
...
size_t PrepareInsertNonSoo(CommonFields& common, size_t hash, FindInfo target, const PolicyFunctions& policy)
{
    ...
    const bool use_target_hint =
        ...
        !SwisstableGenerationsEnabled() && common.growth_info().HasNoDeletedAndGrowthLeft();
    if (ABSL_PREDICT_FALSE(!use_target_hint)) {
        ...
        if (ABSL_PREDICT_TRUE(common.growth_info().HasNoGrowthLeftAndNoDeleted())) {
            ...
            target = HashSetResizeHelper::FindFirstNonFullAfterResize(common, old_capacity, hash);
        } else {
            ...
            if (rehash_for_bug_detection) {
                ...
                const size_t cap = common.capacity();
                policy.resize(common, common.growth_left() > 0 ? cap : NextCapacity(cap), HashtablezInfoHandle {});
            }
