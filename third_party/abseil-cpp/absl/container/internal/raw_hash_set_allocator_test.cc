...
template <class T, int Spec = kPropagateOnCopy | kPropagateOnMove | kPropagateOnSwap> class CheckedAlloc {
public:
    template <class, int> friend class CheckedAlloc;
    ...
    friend void swap(CheckedAlloc& a, CheckedAlloc& b)
    {
        a.swap(b);
    }
...
struct Policy {
    ...
    template <class F> static auto apply(F&& f, int32_t v) -> decltype(std::forward<F>(f)(v, v))
    {
        return std::forward<F>(f)(v, v);
    }

    template <class F> static auto apply(F&& f, const slot_type& v) -> decltype(std::forward<F>(f)(v.val(), v))
    {
        return std::forward<F>(f)(v.val(), v);
    }

    template <class F> static auto apply(F&& f, slot_type&& v) -> decltype(std::forward<F>(f)(v.val(), std::move(v)))
    {
...
template <int Spec> struct PropagateTest : public ::testing::Test {
...
using PropagateOnAll = PropagateTest<kPropagateOnCopy | kPropagateOnMove | kPropagateOnSwap>;
...
template <class T> class PAlloc {
    template <class> friend class PAlloc;
...
public:
    ...
    explicit PAlloc(size_t id) noexcept
        : id_(id)
    {
    }
    ...
    template <class U>
    PAlloc(const PAlloc<U>& that) noexcept
        : id_(that.id_)
    {
    } // NOLINT
    ...
    constexpr PAlloc select_on_container_copy_construction() const
    {
        return {};
    }
