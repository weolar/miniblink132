...
template <class Derived> struct WithId {
    WithId()
        : id_(next_id<Derived>())
    {
    }
...
private:
    ...
    template <class T> static size_t next_id()
    {
...
struct NonStandardLayout {
    ...
    template <typename H> friend H AbslHashValue(H h, const NonStandardLayout& v)
    {
...
template <class T = int> struct Alloc : std::allocator<T> {
...
template <class Map> auto items(const Map& m) -> std::vector<std::pair<typename Map::key_type, typename Map::mapped_type>>
{
    using std::get;
...
template <class Set> auto keys(const Set& s) -> std::vector<typename std::decay<typename Set::key_type>::type>
{
