...
template <class Container, class = void> struct IsMap : std::false_type { };

template <class Map> struct IsMap<Map, absl::void_t<typename Map::mapped_type>> : std::true_type { };
...
template <class T, class E = void> struct Generator;

template <class T> struct Generator<T, typename std::enable_if<std::is_integral<T>::value>::type> {
    T operator()() const
    {
...
template <> struct Generator<Enum> {
    Enum operator()() const
    {
...
template <> struct Generator<EnumClass> {
...
template <> struct Generator<absl::string_view> {
    absl::string_view operator()() const;
};

template <> struct Generator<NonStandardLayout> {
...
template <class K, class V> struct Generator<std::pair<K, V>> {
...
template <class... Ts> struct Generator<std::tuple<Ts...>> {
...
template <class T> struct Generator<std::unique_ptr<T>> {
...
struct Generator<U, absl::void_t<decltype(std::declval<U&>().key()), decltype(std::declval<U&>().value())>>
    : Generator<std::pair<typename std::decay<decltype(std::declval<U&>().key())>::type, typename std::decay<decltype(std::declval<U&>().value())>::type>> { };
...
template <class T, size_t kMaxValues = 64, class E = void> struct UniqueGenerator {
    Generator<T, E> gen;
    std::vector<T> values;
