...
// Does `Ts...` contain `T`?
template <class T, class... Ts> using Contains = absl::disjunction<std::is_same<T, Ts>...>;
...
using IsLegalElementType = std::integral_constant<bool,
    !std::is_reference<T>::value && !std::is_volatile<T>::value && !std::is_reference<typename Type<T>::type>::value
        && !std::is_volatile<typename Type<T>::type>::value && adl_barrier::IsPow2(AlignOf<T>::value)>;

template <class Elements, class StaticSizeSeq, class RuntimeSizeSeq, class SizeSeq, class OffsetSeq> class LayoutImpl;
...
class LayoutImpl<std::tuple<Elements...>, absl::index_sequence<StaticSizeSeq...>, absl::index_sequence<RuntimeSizeSeq...>, absl::index_sequence<SizeSeq...>,
...
private:
    ...
    static constexpr std::array<size_t, sizeof...(StaticSizeSeq)> kStaticSizes = { StaticSizeSeq... };
...
public:
    ...
    template <size_t N, EnableIf<N != 0> = 0> constexpr size_t Offset() const
    {
        ...
        return adl_barrier::Align(Offset<N - 1>() + SizeOf<ElementType<N - 1>>::value * Size<N - 1>(), ElementAlignment<N>::value);
    }
    ...
    //
    // Requires: `N < NumSizes`.
    template <size_t N, EnableIf<(N < NumStaticSizes)> = 0> constexpr size_t Size() const
    {
    ...
    //
    // Requires: `p` is aligned to `Alignment()`.
    template <class Char> auto Pointers(Char* p) const
    {
    ...
    template <class Char> auto Slices(ABSL_ATTRIBUTE_UNUSED Char* p) const
    {
...
template <class... Elements, size_t... StaticSizeSeq, size_t... RuntimeSizeSeq, size_t... SizeSeq, size_t... OffsetSeq>
constexpr std::array<size_t, sizeof...(StaticSizeSeq)> LayoutImpl<std::tuple<Elements...>, absl::index_sequence<StaticSizeSeq...>,
    absl::index_sequence<RuntimeSizeSeq...>, absl::index_sequence<SizeSeq...>, absl::index_sequence<OffsetSeq...>>::kStaticSizes;

template <class StaticSizeSeq, size_t NumRuntimeSizes, class... Ts>
using LayoutType = LayoutImpl<std::tuple<Ts...>, StaticSizeSeq, absl::make_index_sequence<NumRuntimeSizes>,
    absl::make_index_sequence<NumRuntimeSizes + StaticSizeSeq::size()>,
    absl::make_index_sequence<adl_barrier::Min(sizeof...(Ts), NumRuntimeSizes + StaticSizeSeq::size() + 1)>>;
...
private:
    using Super = LayoutType<StaticSizeSeq, sizeof...(Ts) - adl_barrier::Min(sizeof...(Ts), StaticSizeSeq::size()), Ts...>;

public:
    // The result type of `Partial()` with `NumSizes` arguments.
    template <size_t NumSizes> using PartialType = internal_layout::LayoutType<StaticSizeSeq, NumSizes, Ts...>;
    ...
    template <class... Sizes> static constexpr PartialType<sizeof...(Sizes)> Partial(Sizes&&... sizes)
    {
        ...
        return PartialType<sizeof...(Sizes)>(static_cast<size_t>(std::forward<Sizes>(sizes))...);
    }
...
template <class... Ts> class Layout : public internal_layout::LayoutWithStaticSizes<absl::make_index_sequence<0>, Ts...> {
private:
    using Super = internal_layout::LayoutWithStaticSizes<absl::make_index_sequence<0>, Ts...>;
...
public:
    ...
    template <class StaticSizeSeq> using WithStaticSizeSequence = internal_layout::LayoutWithStaticSizes<StaticSizeSeq, Ts...>;

    template <size_t... StaticSizes> using WithStaticSizes = WithStaticSizeSequence<std::index_sequence<StaticSizes...>>;
