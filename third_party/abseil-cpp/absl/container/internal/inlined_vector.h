...
template <typename A> using IsMoveAssignOk = std::is_move_assignable<ValueType<A>>;
template <typename A> using IsSwapOk = absl::type_traits_internal::IsSwappable<ValueType<A>>;

template <typename A, bool IsTriviallyDestructible = absl::is_trivially_destructible<ValueType<A>>::value> struct DestroyAdapter;
...
template <typename A, bool IsOverAligned = (alignof(ValueType<A>) > ABSL_INTERNAL_DEFAULT_NEW_ALIGNMENT)> struct MallocAdapter {
...
template <typename A> class DefaultValueAdapter {
public:
...
template <typename A> class ConstructionTransaction {
public:
    ...
    template <typename ValueAdapter> void Construct(Pointer<A> data, ValueAdapter& values, SizeType<A> size)
    {
...
public:
    ...
    using MoveAssignmentPolicy = absl::conditional_t<
        ...
        absl::conjunction<absl::is_trivially_move_assignable<ValueType<A>>, absl::is_trivially_destructible<ValueType<A>>,
    ...
    using SwapInlinedElementsPolicy = absl::conditional_t<
        ...
        absl::conjunction<absl::is_trivially_relocatable<ValueType<A>>, std::is_same<A, std::allocator<ValueType<A>>>>::value, MemcpyPolicy,
    ...
    ~Storage()
    {
        ...
        // maybe not even that).
        if (absl::is_trivially_destructible<ValueType<A>>::value && std::is_same<A, std::allocator<ValueType<A>>>::value) {
...
#pragma GCC diagnostic pop
    ...
    {
        return GetIsAllocated() ? StorageView<A> { GetAllocatedData(), GetSize(), GetAllocatedCapacity() }
    ...
    template <typename ValueAdapter> void Initialize(ValueAdapter values, SizeType<A> new_size);
    ...
    void MemcpyFrom(const Storage& other_storage)
    {
        ...
        {
            using V = ValueType<A>;
            ABSL_HARDENING_ASSERT(other_storage.GetIsAllocated()
                || (std::is_same<A, std::allocator<V>>::value
                    && (
                        // First case above
                        absl::is_trivially_relocatable<V>::value ||
                        // Second case above
                        (absl::is_trivially_move_assignable<V>::value && absl::is_trivially_destructible<V>::value) ||
                        // Third case above
                        (absl::is_trivially_copy_constructible<V>::value || absl::is_trivially_copy_assignable<V>::value))));
        }
...
private:
    ...
    struct Inlined {
        alignas(ValueType<A>) char inlined_data[sizeof(ValueType<A>[kOptimalInlinedSize])];
    };
    ...
    template <typename NotMemcpyPolicy> void SwapInlinedElements(NotMemcpyPolicy, Storage* other);
...
template <typename T, size_t N, typename A> void Storage<T, N, A>::InitFrom(const Storage& other)
{
    ...
    if (absl::is_trivially_copy_constructible<ValueType<A>>::value && std::is_same<A, std::allocator<ValueType<A>>>::value) {
...
auto Storage<T, N, A>::Insert(ConstIterator<A> pos, ValueAdapter values, SizeType<A> insert_count) -> Iterator<A>
{
    ...
    auto insert_index = static_cast<SizeType<A>>(std::distance(ConstIterator<A>(storage_view.data), pos));
...
template <typename T, size_t N, typename A> auto Storage<T, N, A>::Erase(ConstIterator<A> from, ConstIterator<A> to) -> Iterator<A>
{
    ...
    auto erase_index = static_cast<SizeType<A>>(std::distance(ConstIterator<A>(storage_view.data), from));
    ...
    if (absl::is_trivially_relocatable<ValueType<A>>::value && std::is_nothrow_destructible<ValueType<A>>::value
        && std::is_same<A, std::allocator<ValueType<A>>>::value) {
        DestroyAdapter<A>::DestroyElements(GetAllocator(), storage_view.data + erase_index, erase_size);
        std::memmove(reinterpret_cast<char*>(storage_view.data + erase_index), reinterpret_cast<const char*>(storage_view.data + erase_end_index),
...
template <typename T, size_t N, typename A> auto Storage<T, N, A>::Swap(Storage* other_storage_ptr) -> void
{
    ...
    if (GetIsAllocated() && other_storage_ptr->GetIsAllocated()) {
        ...
        StorageView<A> allocated_storage_view { allocated_ptr->GetAllocatedData(), allocated_ptr->GetSize(), allocated_ptr->GetAllocatedCapacity() };

        IteratorValueAdapter<A, MoveIterator<A>> move_values(MoveIterator<A>(inlined_ptr->GetInlinedData()));
        ...
        ABSL_INTERNAL_CATCH_ANY
        {
            allocated_ptr->SetAllocation(Allocation<A> { allocated_storage_view.data, allocated_storage_view.capacity });
            ABSL_INTERNAL_RETHROW;
        }
...
template <typename T, size_t N, typename A> void Storage<T, N, A>::SwapN(ElementwiseSwapPolicy, Storage* other, SizeType<A> n)
{
    std::swap_ranges(GetInlinedData(), GetInlinedData() + n, other->GetInlinedData());
}

template <typename T, size_t N, typename A> void Storage<T, N, A>::SwapN(ElementwiseConstructPolicy, Storage* other, SizeType<A> n)
{
...
template <typename T, size_t N, typename A> void Storage<T, N, A>::SwapInlinedElements(MemcpyPolicy, Storage* other)
{
    Data tmp = data_;
    data_ = other->data_;
    other->data_ = tmp;
...
void Storage<T, N, A>::SwapInlinedElements(NotMemcpyPolicy policy, Storage* other)
{
    ...
    IteratorValueAdapter<A, MoveIterator<A>> move_values(MoveIterator<A>(large_ptr->GetInlinedData() + small_size));
