...
class HashtablezInfoHandlePeer {
public:
    static HashtablezInfo* GetInfo(HashtablezInfoHandle*)
    {
        return nullptr;
    }
};
...
std::vector<size_t> GetSizes(HashtablezSampler* s)
{
    ...
    s->Iterate([&](const HashtablezInfo& info) { res.push_back(info.size.load(std::memory_order_acquire)); });
    return res;
}
...
HashtablezInfo* Register(HashtablezSampler* s, size_t size)
{
    ...
    auto* info = s->Register(test_stride, test_element_size, /*key_size=*/test_key_size,
...
TEST(HashtablezSamplerTest, Sample)
{
    ...
    for (int i = 0; i < 1000000; ++i) {
        ...
        if (0.005 < sample_rate && sample_rate < 0.015)
            break;
    }
...
TEST(HashtablezSamplerTest, MultiThreaded)
{
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        pool.Schedule([&sampler, &stop, sampling_stride, elt_size, key_size, value_size]() {
            ...
            while (!stop.HasBeenNotified()) {
                ...
                case 2: {
                    ...
                    sampler.Iterate([&](const HashtablezInfo& info) { oldest = std::max(oldest, absl::Now() - info.create_time); });
