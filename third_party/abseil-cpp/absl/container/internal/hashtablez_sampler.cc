...
ABSL_PER_THREAD_TLS_KEYWORD absl::profiling_internal::ExponentialBiased g_exponential_biased_generator;
#endif
...
void HashtablezInfo::PrepareForSampling(
    int64_t stride, size_t inline_element_size_value, size_t key_size_value, size_t value_size_value, uint16_t soo_capacity_value)
{
...
static bool ShouldForceSampling()
{
    ...
    if (ABSL_PREDICT_TRUE(state == kDontForce))
        return false;

    if (state == kUninitialized) {
        state = ABSL_INTERNAL_C_SYMBOL(AbslContainerInternalSampleEverything)() ? kForce : kDontForce;
...
HashtablezInfo* SampleSlow(SamplingState& next_sample, size_t inline_element_size, size_t key_size, size_t value_size, uint16_t soo_capacity)
{
    if (ABSL_PREDICT_FALSE(ShouldForceSampling())) {
        ...
        HashtablezInfo* result = GlobalHashtablezSampler().Register(old_stride, inline_element_size, key_size, value_size, soo_capacity);
        return result;
    }
    ...
    const int64_t next_stride = g_exponential_biased_generator.GetStride(g_hashtablez_sample_parameter.load(std::memory_order_relaxed));
    ...
    if (!g_hashtablez_enabled.load(std::memory_order_relaxed))
        return nullptr;

    // We will only be negative on our first count, so we should just retry in
    // that case.
    if (first) {
        if (ABSL_PREDICT_TRUE(--next_sample.next_sample > 0))
            return nullptr;
        return SampleSlow(next_sample, inline_element_size, key_size, value_size, soo_capacity);
    }

    return GlobalHashtablezSampler().Register(old_stride, inline_element_size, key_size, value_size, soo_capacity);
#endif
}
...
void RecordRehashSlow(HashtablezInfo* info, size_t total_probe_length)
{
    ...
    info->num_rehashes.store(1 + info->num_rehashes.load(std::memory_order_relaxed), std::memory_order_relaxed);
}

void RecordReservationSlow(HashtablezInfo* info, size_t target_capacity)
{
    info->max_reserve.store((std::max)(info->max_reserve.load(std::memory_order_relaxed), target_capacity), std::memory_order_relaxed);
}
...
void RecordEraseSlow(HashtablezInfo* info)
{
    ...
    // There is only one concurrent writer, so `load` then `store` is sufficient
    // instead of using `fetch_add`.
    info->num_erases.store(1 + info->num_erases.load(std::memory_order_relaxed), std::memory_order_relaxed);
}
