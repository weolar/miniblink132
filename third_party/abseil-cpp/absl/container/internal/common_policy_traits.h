...
template <class Policy, class = void> struct common_policy_traits {
    ...
    template <class Alloc> static auto destroy(Alloc* alloc, slot_type* slot)
    {
    ...
    template <class P = Policy> static auto element(absl::remove_const_t<slot_type>* slot) -> decltype(P::element(slot))
    {
    ...
    template <class Alloc> static constexpr bool destroy_is_trivial()
    {
...
private:
    ...
    static std::true_type transfer_impl(Alloc*, slot_type* new_slot, slot_type* old_slot, Rank1)
    {
        ...
        std::memcpy(static_cast<void*>(std::launder(const_cast<std::remove_const_t<value_type>*>(&element(new_slot)))),
            static_cast<const void*>(&element(old_slot)), sizeof(value_type));
        return {};
    }
#endif

    template <class Alloc> static void transfer_impl(Alloc* alloc, slot_type* new_slot, slot_type* old_slot, Rank0)
    {
