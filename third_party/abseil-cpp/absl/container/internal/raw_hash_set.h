...
#define ABSL_SWISSTABLE_ASSERT(CONDITION) assert((CONDITION) && "Try enabling sanitizers.")
#endif
...
template <typename AllocType> void CopyAlloc(AllocType& lhs, AllocType& rhs, std::true_type /* propagate_alloc */)
{
    lhs = rhs;
}
template <typename AllocType> void CopyAlloc(AllocType&, AllocType&, std::false_type /* propagate_alloc */)
{
}
...
struct IsDecomposable<absl::void_t<decltype(Policy::apply(RequireUsableKey<typename Policy::key_type, Hash, Eq>(), std::declval<Ts>()...))>, Policy, Hash, Eq,
    Ts...> : std::true_type { };

// TODO(alkis): Switch to std::is_nothrow_swappable when gcc/clang supports it.
template <class T> constexpr bool IsNoThrowSwappable(std::true_type = {} /* is_swappable */)
{
    using std::swap;
    return noexcept(swap(std::declval<T&>(), std::declval<T&>()));
}
template <class T> constexpr bool IsNoThrowSwappable(std::false_type /* is_swappable */)
{
    return false;
}

template <typename T> uint32_t TrailingZeros(T x)
{
    ABSL_ASSUME(x != 0);
...
template <class T, int SignificantBits, int Shift = 0> class NonIterableBitMask {
public:
    ...
    uint32_t LeadingZeros() const
    {
        ...
        return static_cast<uint32_t>(countl_zero(static_cast<T>(mask_ << extra_bits))) >> Shift;
    }
...
struct GroupSse2Impl {
    ...
    BitMask<uint16_t, kWidth> Match(h2_t hash) const
    {
        ...
        return BitMask<uint16_t, kWidth>(static_cast<uint16_t>(_mm_movemask_epi8(_mm_cmpeq_epi8(match, ctrl))));
    }
    ...
    NonIterableBitMask<uint16_t, kWidth> MaskEmpty() const
    {
        ...
        return NonIterableBitMask<uint16_t, kWidth>(static_cast<uint16_t>(_mm_movemask_epi8(_mm_sign_epi8(ctrl, ctrl))));
#else
        auto match = _mm_set1_epi8(static_cast<char>(ctrl_t::kEmpty));
        return NonIterableBitMask<uint16_t, kWidth>(static_cast<uint16_t>(_mm_movemask_epi8(_mm_cmpeq_epi8(match, ctrl))));
#endif
    }
    ...
    {
        return BitMask<uint16_t, kWidth>(static_cast<uint16_t>(_mm_movemask_epi8(ctrl) ^ 0xffff));
    }
    ...
    {
        return BitMask<uint16_t, kWidth>(static_cast<uint16_t>(_mm_movemask_epi8(ctrl)));
    }
    ...
    NonIterableBitMask<uint16_t, kWidth> MaskEmptyOrDeleted() const
    {
        ...
        return NonIterableBitMask<uint16_t, kWidth>(static_cast<uint16_t>(_mm_movemask_epi8(_mm_cmpgt_epi8_fixed(special, ctrl))));
    }
...
struct GroupAArch64Impl {
    ...
    auto Match(h2_t hash) const
    {
        ...
        return BitMask<uint64_t, kWidth, /*Shift=*/3,
            /*NullifyBitsOnIteration=*/true>(vget_lane_u64(vreinterpret_u64_u8(mask), 0));
    }

    NonIterableBitMask<uint64_t, kWidth, 3> MaskEmpty() const
    {
        uint64_t mask = vget_lane_u64(vreinterpret_u64_u8(vceq_s8(vdup_n_s8(static_cast<int8_t>(ctrl_t::kEmpty)), vreinterpret_s8_u8(ctrl))), 0);
    ...
    auto MaskFull() const
    {
        uint64_t mask = vget_lane_u64(vreinterpret_u64_u8(vcge_s8(vreinterpret_s8_u8(ctrl), vdup_n_s8(static_cast<int8_t>(0)))), 0);
        return BitMask<uint64_t, kWidth, /*Shift=*/3,
    ...
    auto MaskNonFull() const
    {
        uint64_t mask = vget_lane_u64(vreinterpret_u64_u8(vclt_s8(vreinterpret_s8_u8(ctrl), vdup_n_s8(static_cast<int8_t>(0)))), 0);
        return BitMask<uint64_t, kWidth, /*Shift=*/3,
            /*NullifyBitsOnIteration=*/true>(mask);
    }
    ...
    {
        uint64_t mask = vget_lane_u64(vreinterpret_u64_u8(vcgt_s8(vdup_n_s8(static_cast<int8_t>(ctrl_t::kSentinel)), vreinterpret_s8_u8(ctrl))), 0);
        return NonIterableBitMask<uint64_t, kWidth, 3>(mask);
    }

    uint32_t CountLeadingEmptyOrDeleted() const
    {
        uint64_t mask = vget_lane_u64(vreinterpret_u64_u8(vcle_s8(vdup_n_s8(static_cast<int8_t>(ctrl_t::kSentinel)), vreinterpret_s8_u8(ctrl))), 0);
...
class CommonFieldsGenerationInfoEnabled {
    ...
    static constexpr size_t kReservedGrowthJustRanOut = (std::numeric_limits<size_t>::max)();
...
public:
    ...
    CommonFieldsGenerationInfoDisabled(CommonFieldsGenerationInfoDisabled&&) = default;
...
public:
    explicit RawHashSetLayout(size_t capacity, size_t slot_align, bool has_infoz)
        ...
        , slot_offset_((generation_offset_ + NumGenerationBytes() + slot_align - 1) & (~slot_align + 1))
    {
...
#define ABSL_SWISSTABLE_IGNORE_UNINITIALIZED(x)                                                                                                                \
    _Pragma("GCC diagnostic push") _Pragma("GCC diagnostic ignored \"-Wmaybe-uninitialized\"") _Pragma("GCC diagnostic ignored \"-Wuninitialized\"") x;        \
    _Pragma("GCC diagnostic pop")
#define ABSL_SWISSTABLE_IGNORE_UNINITIALIZED_RETURN(x) ABSL_SWISSTABLE_IGNORE_UNINITIALIZED(return x)
#else
...
public:
    ...
    template <bool kSooEnabled> static CommonFields CreateDefault()
    {
    ...
    void* backing_array_start() const
    {
        ...
        ABSL_SWISSTABLE_ASSERT(reinterpret_cast<uintptr_t>(control()) % alignof(size_t) == 0);
    ...
    GrowthInfo& growth_info()
    {
        ...
        ABSL_SWISSTABLE_ASSERT(reinterpret_cast<uintptr_t>(gl_ptr) % alignof(GrowthInfo) == 0);
        return *gl_ptr;
    }
    ...
    {
        return CommonFieldsGenerationInfo::should_rehash_for_bug_detection_on_move(control(), capacity());
    }
    ...
    {
        return RawHashSetLayout(capacity(), slot_align, has_infoz()).alloc_size(slot_size);
    }

    // Move fields other than heap_or_soo_.
    void move_non_heap_or_soo_fields(CommonFields& that)
    {
        static_cast<CommonFieldsGenerationInfo&>(*this) = std::move(static_cast<CommonFieldsGenerationInfo&>(that));
    ...
    {
        return static_cast<size_t>(std::count(control(), control() + capacity(), ctrl_t::kDeleted));
    }

    // Helper to enable sanitizer mode validation to protect against reentrant
    // calls during element constructor/destructor.
    template <typename F> void RunWithReentrancyGuard(F f)
    {
#ifdef NDEBUG
        f();
        return;
...
template <class InputIter> size_t SelectBucketCountForIterRange(InputIter first, InputIter last, size_t bucket_count)
{
    if (bucket_count != 0) {
        return bucket_count;
    }
    using InputIterCategory = typename std::iterator_traits<InputIter>::iterator_category;
    if (std::is_base_of<std::random_access_iterator_tag, InputIterCategory>::value) {
        return GrowthToLowerboundCapacity(static_cast<size_t>(std::distance(first, last)));
    }
    return 0;
}
...
{
#if defined(ABSL_SWISSTABLE_ENABLE_GENERATIONS) || ABSL_OPTION_HARDENED == 1 || !defined(NDEBUG)
    return true;
#else
    return false;
#endif
...
inline void AssertIsFull(const ctrl_t* ctrl, GenerationType generation, const GenerationType* generation_ptr, const char* operation)
{
    ...
    if (SwisstableGenerationsEnabled()) {
        ...
        if (ABSL_PREDICT_FALSE(!IsFull(*ctrl))) {
            ABSL_RAW_LOG(FATAL, "%s called on invalid iterator. The element was likely erased.", operation);
        }
    } else {
        if (ABSL_PREDICT_FALSE(!IsFull(*ctrl))) {
            ABSL_RAW_LOG(FATAL,
...
inline void AssertIsValidForComparison(const ctrl_t* ctrl, GenerationType generation, const GenerationType* generation_ptr)
{
    if (!SwisstableDebugEnabled())
        return;
    const bool ctrl_is_valid_for_comparison = ctrl == nullptr || ctrl == EmptyGroup() || IsFull(*ctrl);
    if (SwisstableGenerationsEnabled()) {
        if (ABSL_PREDICT_FALSE(generation != *generation_ptr)) {
            ABSL_RAW_LOG(FATAL,
        ...
        if (ABSL_PREDICT_FALSE(!ctrl_is_valid_for_comparison)) {
            ABSL_RAW_LOG(FATAL, "Invalid iterator comparison. The element was likely erased.");
        }
    } else {
        ABSL_HARDENING_ASSERT(ctrl_is_valid_for_comparison
...
// Note: we take slots by reference so that it's not UB if they're uninitialized
// as long as we don't read them (when ctrl is null).
inline void AssertSameContainer(const ctrl_t* ctrl_a, const ctrl_t* ctrl_b, const void* const& slot_a, const void* const& slot_b,
    const GenerationType* generation_ptr_a, const GenerationType* generation_ptr_b)
{
    if (!SwisstableDebugEnabled())
        return;
    // `SwisstableDebugEnabled()` is also true for release builds with hardening
    ...
    if (SwisstableGenerationsEnabled()) {
        ...
        ABSL_HARDENING_ASSERT_SLOW(AreItersFromSameContainer(ctrl_a, ctrl_b, slot_a, slot_b)
...
template <typename = void> inline FindInfo find_first_non_full(const CommonFields& common, size_t hash)
{
    ...
    if (IsEmptyOrDeleted(ctrl[seq.offset()]) && !ShouldInsertBackwards(common.capacity(), hash, ctrl)) {
    ...
    while (true) {
        ...
        if (mask) {
            return { seq.offset(GetInsertionOffset(mask, common.capacity(), hash, ctrl)), seq.index() };
        }
        seq.next();
...
{
    common.growth_info().InitGrowthLeftNoDeleted(CapacityToGrowth(common.capacity()) - common.size());
}
...
template <class SlotType, class Callback> ABSL_ATTRIBUTE_ALWAYS_INLINE inline void IterateOverFullSlots(const CommonFields& c, SlotType* slot, Callback cb)
{
    ...
    while (remaining != 0) {
        ...
        ctrl += Group::kWidth;
        slot += Group::kWidth;
        ABSL_SWISSTABLE_ASSERT((remaining == 0 || *(ctrl - 1) != ctrl_t::kSentinel) && "hash table was modified unexpectedly");
    }
...
template <typename CharAlloc> constexpr bool ShouldSampleHashtablezInfo()
{
...
HashtablezInfoHandle SampleHashtablezInfo(
    size_t sizeof_slot, size_t sizeof_key, size_t sizeof_value, size_t old_capacity, bool was_soo, HashtablezInfoHandle forced_infoz, CommonFields& c)
{
...
public:
    ...
    ABSL_ATTRIBUTE_NOINLINE bool InitializeSlots(CommonFields& c, Alloc alloc, ctrl_t soo_slot_h2, size_t key_size, size_t value_size)
    {
        ...
        HashtablezInfoHandle infoz = ShouldSampleHashtablezInfo<Alloc>()
            ? SampleHashtablezInfo<SooEnabled>(SizeOfSlot, key_size, value_size, old_capacity_, was_soo_, forced_infoz_, c)
            : HashtablezInfoHandle {};

        const bool has_infoz = infoz.IsSampled();
        RawHashSetLayout layout(c.capacity(), AlignOfSlot, has_infoz);
        char* mem = static_cast<char*>(Allocate<BackingArrayAlignment(AlignOfSlot)>(&alloc, layout.alloc_size(SizeOfSlot)));
        const GenerationType old_generation = c.generation();
        c.set_generation_ptr(reinterpret_cast<GenerationType*>(mem + layout.generation_offset()));
        c.set_generation(NextGeneration(old_generation));
        c.set_control(reinterpret_cast<ctrl_t*>(mem + layout.control_offset()));
        c.set_slots(mem + layout.slot_offset());
        ResetGrowthLeft(c);

        const bool grow_single_group = IsGrowingIntoSingleGroupApplicable(old_capacity_, layout.capacity());
    ...
    template <class PolicyTraits, class Alloc> void GrowSizeIntoSingleGroup(CommonFields& c, Alloc& alloc_ref)
    {
        ABSL_SWISSTABLE_ASSERT(old_capacity_ < Group::kWidth / 2);
        ABSL_SWISSTABLE_ASSERT(IsGrowingIntoSingleGroupApplicable(old_capacity_, c.capacity()));
        ...
        for (size_t i = 0; i < old_capacity_; ++i) {
            if (IsFull(old_ctrl()[i])) {
                ...
                PolicyTraits::transfer(&alloc_ref, new_slots + new_i, old_slots_ptr + i);
            }
        }
    ...
    template <size_t AlignOfSlot, class CharAlloc> void DeallocateOld(CharAlloc alloc_ref, size_t slot_size)
    {
        SanitizerUnpoisonMemoryRegion(old_slots(), slot_size * old_capacity_);
        auto layout = RawHashSetLayout(old_capacity_, AlignOfSlot, had_infoz_);
        Deallocate<BackingArrayAlignment(AlignOfSlot)>(&alloc_ref, old_ctrl() - layout.control_offset(), layout.alloc_size(slot_size));
    }
...
// same slot alignment.
template <size_t AlignOfSlot> ABSL_ATTRIBUTE_NOINLINE void DeallocateStandard(CommonFields& common, const PolicyFunctions& policy)
{
    ...
    Deallocate<BackingArrayAlignment(AlignOfSlot)>(&alloc, common.backing_array_start(), common.alloc_size(policy.slot_size, AlignOfSlot));
}

// For trivially relocatable types we use memcpy directly. This allows us to
// share the same function body for raw_hash_set instantiations that have the
// same slot size as long as they are relocatable.
template <size_t SizeOfSlot> ABSL_ATTRIBUTE_NOINLINE void TransferRelocatable(void*, void* dst, void* src)
{
...
private:
    ...
    {
        return PolicyTraits::soo_enabled() && sizeof(slot_type) <= sizeof(HeapOrSoo) && alignof(slot_type) <= alignof(HeapOrSoo);
    }
    ...
    // anyway).
    using CharAlloc = typename absl::allocator_traits<Alloc>::template rebind_alloc<char>;
...
public:
    ...
    private:
        ...
        void skip_empty_or_deleted()
        {
            while (IsEmptyOrDeleted(*ctrl_)) {
                uint32_t shift = GroupFullEmptyOrDeleted { ctrl_ }.CountLeadingEmptyOrDeleted();
                ctrl_ += shift;
                slot_ += shift;
            }
        }
    ...
    class const_iterator {
        friend class raw_hash_set;
        template <class Container, typename Enabler> friend struct absl::container_internal::hashtable_debug_internal::HashtableDebugAccess;
    ...
    raw_hash_set(InputIter first, InputIter last, size_t bucket_count = 0, const hasher& hash = hasher(), const key_equal& eq = key_equal(),
    ...
    raw_hash_set(std::initializer_list<T> init, size_t bucket_count = 0, const hasher& hash = hasher(), const key_equal& eq = key_equal(),
        const allocator_type& alloc = allocator_type())
        : raw_hash_set(init.begin(), init.end(), bucket_count, hash, eq, alloc)
    {
    }

    raw_hash_set(std::initializer_list<init_type> init, size_t bucket_count = 0, const hasher& hash = hasher(), const key_equal& eq = key_equal(),
        const allocator_type& alloc = allocator_type())
        : raw_hash_set(init.begin(), init.end(), bucket_count, hash, eq, alloc)
    {
    }
        ...
        : raw_hash_set(GrowthToLowerboundCapacity(that.size()), that.hash_ref(), that.eq_ref(), a)
    {
        ...
        //     `capacity + 1` (2^N).
        size_t offset = cap;
        const size_t shift = is_single_group(cap) ? (PerTableSalt(control()) | 1) : 0;
        IterateOverFullSlots(that.common(), that.slot_array(), [&](const ctrl_t* that_ctrl, slot_type* that_slot) ABSL_ATTRIBUTE_ALWAYS_INLINE {
            if (shift == 0) {
                // Big tables case. Position must be searched via probing.
                // The table is guaranteed to be empty, so we can do faster than
                // a full `insert`.
                const size_t hash = PolicyTraits::apply(HashElement { hash_ref() }, PolicyTraits::element(that_slot));
            ...
            ABSL_SWISSTABLE_ASSERT( // We rely that hash is not changed for small
                // tables.
                H2(PolicyTraits::apply(HashElement { hash_ref() }, PolicyTraits::element(that_slot))) == h2
    ...
    ABSL_ATTRIBUTE_NOINLINE raw_hash_set(raw_hash_set&& that) noexcept(std::is_nothrow_copy_constructible<hasher>::value&&
            std::is_nothrow_copy_constructible<key_equal>::value&& std::is_nothrow_copy_constructible<allocator_type>::value)
        : // Hash, equality and allocator are copied instead of moved because
        // `that` must be left valid. If Hash is std::function<Key>, moving it
        // would create a nullptr functor that cannot be called.
        // Note: we avoid using exchange for better generated code.
        settings_(PolicyTraits::transfer_uses_memcpy() || !that.is_full_soo() ? std::move(that.common()) : CommonFields { full_soo_tag_t {} }, that.hash_ref(),
            that.eq_ref(), that.alloc_ref())
    {
    ...
    raw_hash_set& operator=(const raw_hash_set& that)
    {
        ...
        if (ABSL_PREDICT_FALSE(this == &that))
            return *this;
        constexpr bool propagate_alloc = AllocTraits::propagate_on_container_copy_assignment::value;
    ...
    raw_hash_set& operator=(raw_hash_set&& that) noexcept(absl::allocator_traits<allocator_type>::is_always_equal::value&&
    ...
    size_t capacity() const
    {
        ...
        ABSL_ATTRIBUTE_UNUSED static constexpr size_t kDefaultCapacity = DefaultCapacity();
    ...
    {
        if (SwisstableGenerationsEnabled() && capacity() >= InvalidCapacity::kMovedFrom) {
    ...
    std::pair<iterator, bool> insert(const T& value) ABSL_ATTRIBUTE_LIFETIME_BOUND
    {
    ...
    //   s.insert({"abc", 42});
    std::pair<iterator, bool> insert(init_type&& value) ABSL_ATTRIBUTE_LIFETIME_BOUND
    {
    ...
    std::pair<iterator, bool> emplace(Args&&... args) ABSL_ATTRIBUTE_LIFETIME_BOUND
    {
    ...
    template <class... Args> iterator emplace_hint(const_iterator, Args&&... args) ABSL_ATTRIBUTE_LIFETIME_BOUND
    {
    ...
    template <class K = key_type, class F> iterator lazy_emplace(const key_arg<K>& key, F&& f) ABSL_ATTRIBUTE_LIFETIME_BOUND
    {
    ...
    { // NOLINT
        ...
        const auto insert_slot = [this](slot_type* src_slot) {
            return PolicyTraits::apply(InsertSlot<false> { *this, std::move(*src_slot) }, PolicyTraits::element(src_slot)).second;
        };
    ...
    {
        const size_t max_size_before_growth = is_soo() ? SooCapacity() : size() + growth_left();
    ...
    template <class K = key_type> iterator find(const key_arg<K>& key, size_t hash) ABSL_ATTRIBUTE_LIFETIME_BOUND
    {
        AssertOnFind(key);
        if (is_soo())
            return find_soo(key);
    ...
    template <class K = key_type> iterator find(const key_arg<K>& key) ABSL_ATTRIBUTE_LIFETIME_BOUND
    {
        AssertOnFind(key);
        if (is_soo())
            return find_soo(key);
    ...
    template <class K = key_type> const_iterator find(const key_arg<K>& key, size_t hash) const ABSL_ATTRIBUTE_LIFETIME_BOUND
    {
        return const_cast<raw_hash_set*>(this)->find(key, hash);
    }
    template <class K = key_type> const_iterator find(const key_arg<K>& key) const ABSL_ATTRIBUTE_LIFETIME_BOUND
    {
    ...
    template <class K = key_type> std::pair<iterator, iterator> equal_range(const key_arg<K>& key) ABSL_ATTRIBUTE_LIFETIME_BOUND
    {
        auto it = find(key);
        if (it != end())
    ...
    template <class K = key_type> std::pair<const_iterator, const_iterator> equal_range(const key_arg<K>& key) const ABSL_ATTRIBUTE_LIFETIME_BOUND
    {
        auto it = find(key);
        if (it != end())
            return { it, std::next(it) };
    ...
    friend bool operator==(const raw_hash_set& a, const raw_hash_set& b)
    {
        ...
        for (const value_type& elem : *outer) {
            ...
            static constexpr bool kKeyEqIsValueEq = std::is_same<key_type, value_type>::value && std::is_same<key_equal, hash_default_eq<key_type>>::value;
    ...
    template <typename H> friend typename std::enable_if<H::template is_hashable<value_type>::value, H>::type AbslHashValue(H h, const raw_hash_set& s)
    {
...
private:
    ...
    template <typename... Args> inline void construct(slot_type* slot, Args&&... args)
    {
        common().RunWithReentrancyGuard([&] { PolicyTraits::construct(&alloc_ref(), slot, std::forward<Args>(args)...); });
    }
    inline void destroy(slot_type* slot)
    {
        common().RunWithReentrancyGuard([&] { PolicyTraits::destroy(&alloc_ref(), slot); });
    }
    inline void transfer(slot_type* to, slot_type* from)
    {
        common().RunWithReentrancyGuard([&] { PolicyTraits::transfer(&alloc_ref(), to, from); });
    }

    // TODO(b/289225379): consider having a helper class that has the impls for
    // SOO functionality.
    template <class K = key_type> iterator find_soo(const key_arg<K>& key)
    {
        ABSL_SWISSTABLE_ASSERT(is_soo());
        return empty() || !PolicyTraits::apply(EqualElement<K> { key, eq_ref() }, PolicyTraits::element(soo_slot())) ? end() : soo_iterator();
    }

    template <class K = key_type> iterator find_non_soo(const key_arg<K>& key, size_t hash)
    {
        ABSL_SWISSTABLE_ASSERT(!is_soo());
        auto seq = probe(common(), hash);
        ...
        while (true) {
            ...
            for (uint32_t i : g.Match(H2(hash))) {
                if (ABSL_PREDICT_TRUE(PolicyTraits::apply(EqualElement<K> { key, eq_ref() }, PolicyTraits::element(slot_array() + seq.offset(i)))))
    ...
    inline void destroy_slots()
    {
        ...
        if (PolicyTraits::template destroy_is_trivial<Alloc>())
            return;
        IterateOverFullSlots(common(), slot_array(), [&](const ctrl_t*, slot_type* slot) ABSL_ATTRIBUTE_ALWAYS_INLINE { this->destroy(slot); });
    }
    ...
    ABSL_ATTRIBUTE_NOINLINE static void resize_impl(CommonFields& common, size_t new_capacity, HashtablezInfoHandle forced_infoz)
    {
        ...
        const ctrl_t soo_slot_h2 = had_soo_slot ? static_cast<ctrl_t>(H2(set->hash_of(set->soo_slot()))) : ctrl_t::kEmpty;
    ...
    static void move_common(bool rhs_is_full_soo, allocator_type& rhs_alloc, CommonFields& lhs, CommonFields&& rhs)
    {
        if (PolicyTraits::transfer_uses_memcpy() || !rhs_is_full_soo) {
            ...
            rhs.RunWithReentrancyGuard(
                [&] { lhs.RunWithReentrancyGuard([&] { PolicyTraits::transfer(&rhs_alloc, to_slot(lhs.soo_data()), to_slot(rhs.soo_data())); }); });
        }
    }
    ...
    void annotate_for_bug_detection_on_move(ABSL_ATTRIBUTE_UNUSED raw_hash_set& that)
    {
        ...
        if (!SwisstableGenerationsEnabled() || capacity() == DefaultCapacity() || capacity() > kAboveMaxValidCapacity) {
            return;
        }
    ...
    template <bool propagate_alloc> raw_hash_set& assign_impl(raw_hash_set&& that)
    {
    ...
    template <class K> std::pair<iterator, bool> find_or_prepare_insert_soo(const K& key)
    {
        if (empty()) {
            const HashtablezInfoHandle infoz = try_sample_soo();
            if (infoz.IsSampled()) {
        ...
        const size_t index = PrepareInsertAfterSoo(hash_ref()(key), sizeof(slot_type), common());
        return { iterator_at(index), true };
    }

    template <class K> std::pair<iterator, bool> find_or_prepare_insert_non_soo(const K& key)
    {
        ABSL_SWISSTABLE_ASSERT(!is_soo());
        prefetch_heap_block();
        ...
        while (true) {
            ...
            if (ABSL_PREDICT_TRUE(mask_empty)) {
                size_t target = seq.offset(GetInsertionOffset(mask_empty, capacity(), hash, control()));
...
protected:
    ...
    template <class K> void AssertOnFind(ABSL_ATTRIBUTE_UNUSED const K& key)
    {
    ...
    void AssertNotDebugCapacity() const
    {
        ...
        if (SwisstableGenerationsEnabled() && ABSL_PREDICT_FALSE(capacity() >= InvalidCapacity::kMovedFrom)) {
    ...
    template <class K> void AssertHashEqConsistent(const K& key)
    {
#ifdef NDEBUG
        return;
#endif
        // If the hash/eq functors are known to be consistent, then skip validation.
        if (std::is_same<hasher, absl::container_internal::StringHash>::value && std::is_same<key_equal, absl::container_internal::StringEq>::value) {
            return;
        }
        if (std::is_scalar<key_type>::value && std::is_same<hasher, absl::Hash<key_type>>::value && std::is_same<key_equal, std::equal_to<key_type>>::value) {
            return;
        }
        if (empty())
            return;
        ...
        const auto assert_consistent = [&](const ctrl_t*, slot_type* slot) {
            ...
            const bool is_key_equal = PolicyTraits::apply(EqualElement<K> { key, eq_ref() }, element);
            if (!is_key_equal)
                return;

            const size_t hash_of_slot = PolicyTraits::apply(HashElement { hash_ref() }, element);
            ABSL_ATTRIBUTE_UNUSED const bool is_hash_equal = hash_of_arg == hash_of_slot;
    ...
    template <class K> std::pair<iterator, bool> find_or_prepare_insert(const K& key)
    {
        AssertOnFind(key);
        if (is_soo())
    ...
    template <class... Args> void emplace_at(iterator iter, Args&&... args)
    {
...
private:
    ...
    absl::container_internal::CompressedTuple<CommonFields, hasher, key_equal, allocator_type> settings_ { CommonFields::CreateDefault<SooEnabled()>(),
        hasher {}, key_equal {}, allocator_type {} };
};

// Friend access for free functions in raw_hash_set.h.
struct HashtableFreeFunctionsAccess {
    template <class Predicate, typename Set> static typename Set::size_type EraseIf(Predicate& pred, Set* c)
    {
        if (c->empty()) {
            return 0;
        }
        ...
        IterateOverFullSlots(c->common(), c->slot_array(), [&](const ctrl_t* ctrl, auto* slot) {
            if (pred(Set::PolicyTraits::element(slot))) {
                c->destroy(slot);
                EraseMetaOnly(c->common(), static_cast<size_t>(ctrl - c->control()), sizeof(*slot));
                ++num_deleted;
            }
        });
    ...
    template <class Callback, typename Set> static void ForEach(Callback& cb, Set* c)
    {
        if (c->empty()) {
            return;
        }
        ...
        IterateOverFullSlots(c->common(), c->slot_array(), [&cb](const ctrl_t*, auto* slot) {
...
template <typename P, typename H, typename E, typename A, typename Callback> void ForEach(Callback& cb, raw_hash_set<P, H, E, A>* c)
{
    return HashtableFreeFunctionsAccess::ForEach(cb, c);
}
template <typename P, typename H, typename E, typename A, typename Callback> void ForEach(Callback& cb, const raw_hash_set<P, H, E, A>* c)
{
    return HashtableFreeFunctionsAccess::ForEach(cb, c);
}

namespace hashtable_debug_internal {
template <typename Set> struct HashtableDebugAccess<Set, absl::void_t<typename Set::raw_hash_set>> {
    ...
    static size_t GetNumProbes(const Set& set, const typename Set::key_type& key)
    {
        ...
        while (true) {
            ...
            for (uint32_t i : g.Match(container_internal::H2(hash))) {
                if (Traits::apply(
                        typename Set::template EqualElement<typename Set::key_type> { key, set.eq_ref() }, Traits::element(set.slot_array() + seq.offset(i))))
                    return num_probes;
                ++num_probes;
            }
            if (g.MaskEmpty())
    ...
    static size_t AllocatedByteSize(const Set& c)
    {
        ...
        if (capacity == 0)
            return 0;
        size_t m = c.is_soo() ? 0 : c.common().alloc_size(sizeof(Slot), alignof(Slot));
