...
template <typename T> class InstanceTest : public ::testing::Test { };
...
// Append 0..len-1 to *v
template <typename Container> static void Fill(Container* v, size_t len, int offset = 0)
{
...
TEST(IntVec, AtThrows)
{
    IntVec v = { 1, 2, 3 };
    EXPECT_EQ(v.at(2), 3);
    ABSL_BASE_INTERNAL_EXPECT_FAIL(v.at(3), std::out_of_range, "failed bounds check");
}
...
TEST(IntVec, ReverseIterator)
{
    for (size_t len = 0; len < 20; len++) {
        ...
        counter = len;
        for (IntVec::const_reverse_iterator iter = v.rbegin(); iter != v.rend(); ++iter) {
            counter--;
            EXPECT_EQ(static_cast<int>(counter), *iter);
        }
        EXPECT_EQ(counter, 0u);
        ...
        counter = len;
        for (IntVec::const_reverse_iterator iter = v.crbegin(); iter != v.crend(); ++iter) {
            counter--;
            EXPECT_EQ(static_cast<int>(counter), *iter);
        }
        EXPECT_EQ(counter, 0u);
...
TEST(InlinedVectorTest, Noexcept)
{
    ...
    EXPECT_TRUE((std::is_nothrow_move_constructible<absl::InlinedVector<MoveOnly, 2>>::value));
    ...
    EXPECT_EQ(absl::default_allocator_is_nothrow::value, (std::is_nothrow_move_constructible<absl::InlinedVector<MoveCanThrow, 2>>::value));
}
    ...
    for (size_t len = 0; len < 20; len++) {
        for (ptrdiff_t pos = 0; pos <= static_cast<ptrdiff_t>(len); pos++) {
            ...
            }
            {
                ...
                IntVec::iterator it = v.insert(v.cbegin() + pos, std::istream_iterator<int>(input), std::istream_iterator<int>());
                EXPECT_THAT(v, ElementsAreArray(std_v));
                EXPECT_EQ(it, v.cbegin() + pos);
            }
            {
...
TEST(AliasingTest, InsertWithCount)
{
    for (size_t i = 1; i < 20; ++i) {
        ...
        for (size_t n = 0; n < 5; ++n) {
            ...
            auto b = vec.begin();
            EXPECT_TRUE(std::all_of(b, b + n, [&vec](const NotTriviallyDestructible& x) { return x == vec.back(); }));

            auto m_idx = vec.size() / 2;
            vec.insert(vec.begin() + m_idx, n, vec.back());
            auto m = vec.begin() + m_idx;
            EXPECT_TRUE(std::all_of(m, m + n, [&vec](const NotTriviallyDestructible& x) { return x == vec.back(); }));
            ...
            EXPECT_TRUE(std::all_of(e, e + n, [&val](const NotTriviallyDestructible& x) { return x == val; }));
        }
    }
}
...
TEST(OverheadTest, Storage)
{
    ...
    EXPECT_EQ((2 * expected_overhead), sizeof(absl::InlinedVector<T, 1>) - sizeof(T[1]));
    EXPECT_EQ(expected_overhead, sizeof(absl::InlinedVector<T, 2>) - sizeof(T[2]));
    EXPECT_EQ(expected_overhead, sizeof(absl::InlinedVector<T, 3>) - sizeof(T[3]));
    EXPECT_EQ(expected_overhead, sizeof(absl::InlinedVector<T, 4>) - sizeof(T[4]));
    EXPECT_EQ(expected_overhead, sizeof(absl::InlinedVector<T, 5>) - sizeof(T[5]));
    EXPECT_EQ(expected_overhead, sizeof(absl::InlinedVector<T, 6>) - sizeof(T[6]));
    EXPECT_EQ(expected_overhead, sizeof(absl::InlinedVector<T, 7>) - sizeof(T[7]));
    EXPECT_EQ(expected_overhead, sizeof(absl::InlinedVector<T, 8>) - sizeof(T[8]));
}
    ...
    for (size_t l1 = 0; l1 < 20; l1++) {
        ...
        for (size_t l2 = 0; l2 < 20; l2++) {
            ...
            if (a.size() > inlined_capacity && b.size() > inlined_capacity) {
                ...
                EXPECT_EQ((tracker.moves() ? tracker.moves() : tracker.copies()), static_cast<int>(max_len - min_len));
            } else {
                ...
                EXPECT_EQ((tracker.moves() ? tracker.moves() : tracker.copies()), static_cast<int>(min_len));
            }
    ...
    for (size_t len = 0; len < 20; len++) {
        ...
        if (len > 0) {
            ...
            if (!v.empty()) {
                ...
                EXPECT_EQ(tracker.copies() + tracker.moves(), static_cast<int>(len) - 2);
            }
        }
...
TEST(CountElemAssign, SimpleTypeWithInlineBacking)
{
    ...
    for (size_t original_size = 0; original_size <= 5; ++original_size) {
        ...
        absl::InlinedVector<int, 2> v(original_contents.begin(), original_contents.end());
        v.assign(2, 123);
...
TYPED_TEST_P(InstanceTest, CountElemAssignInlineBacking)
{
    ...
    for (size_t original_size = 0; original_size <= 5; ++original_size) {
        ...
        absl::InlinedVector<Instance, 2> v(original_contents.begin(), original_contents.end());
...
template <typename Instance> void InstanceCountElemAssignWithAllocationTest()
{
    for (size_t original_size = 0; original_size <= 5; ++original_size) {
        SCOPED_TRACE(original_size);
        // Original contents are [12345, 12345, ...]
...
template <typename Instance, typename SourceContainer, int inlined_capacity> void InstanceRangedConstructorTestForContainer()
{
    InstanceTracker tracker;
    SourceContainer source_v = { Instance(0), Instance(1) };
    tracker.ResetCopiesMovesSwaps();
    absl::InlinedVector<Instance, inlined_capacity> v(source_v.begin(), source_v.end());
    EXPECT_EQ(2u, v.size());
...
template <typename Instance, int inlined_capacity> void InstanceRangedConstructorTestWithCapacity()
{
    ...
    // TODO(bsamwel): Test with an input iterator source.
    {
        ...
        InstanceRangedConstructorTestForContainer<Instance, std::list<Instance>, inlined_capacity>();
        {
            SCOPED_TRACE("const std::list");
            InstanceRangedConstructorTestForContainer<Instance, const std::list<Instance>, inlined_capacity>();
        }
        {
            SCOPED_TRACE("std::vector");
            InstanceRangedConstructorTestForContainer<Instance, std::vector<Instance>, inlined_capacity>();
        }
        {
            SCOPED_TRACE("const std::vector");
            InstanceRangedConstructorTestForContainer<Instance, const std::vector<Instance>, inlined_capacity>();
        }
    }
}
...
TEST(RangedAssign, SimpleType)
{
    ...
    for (size_t original_size = 0; original_size <= 5; ++original_size) {
        ...
        for (size_t target_size = 0; target_size <= 5; ++target_size) {
            ...
            absl::InlinedVector<int, 3> v(original_contents.begin(), original_contents.end());
...
template <typename Instance> static bool InstanceValuesEqual(const Instance& lhs, const Instance& rhs)
{
...
// SourceContainer as the source container type.
template <typename Instance, typename SourceContainer> void InstanceRangedAssignTestForContainer()
{
    // Test for all combinations of original sizes (empty and non-empty inline,
    // and out of line) and target sizes.
    for (size_t original_size = 0; original_size <= 5; ++original_size) {
        ...
        for (size_t target_size = 0; target_size <= 5; ++target_size) {
            ...
            SourceContainer new_contents(new_contents_in.begin(), new_contents_in.end());

            absl::InlinedVector<Instance, 3> v(original_contents.begin(), original_contents.end());
            v.assign(new_contents.begin(), new_contents.end());
            ...
            EXPECT_TRUE(std::equal(v.begin(), v.end(), new_contents.begin(), InstanceValuesEqual<Instance>));
        }
    }
}
...
{
    EXPECT_THAT((absl::InlinedVector<int, 4> { 4, 5, 6 }), AllOf(SizeIs(3u), CapacityIs(4u), ElementsAre(4, 5, 6)));
}

TEST(InitializerListConstructor, SimpleTypeWithReallocationRequired)
{
    EXPECT_THAT((absl::InlinedVector<int, 2> { 4, 5, 6 }), AllOf(SizeIs(3u), CapacityIs(Gt(2u)), ElementsAre(4, 5, 6)));
}
...
TEST(InitializerListConstructor, DisparateTypesInList)
{
    ...
    EXPECT_THAT((absl::InlinedVector<std::string, 2> { "foo", std::string("bar") }), ElementsAre("foo", "bar"));
}

TEST(InitializerListConstructor, ComplexTypeWithInlineBacking)
{
    const size_t inlined_capacity = absl::InlinedVector<CopyableMovableInstance, 1>().capacity();
    EXPECT_THAT((absl::InlinedVector<CopyableMovableInstance, 1> { CopyableMovableInstance(0) }),
        AllOf(SizeIs(1u), CapacityIs(inlined_capacity), ElementsAre(ValueIs(0))));
}

TEST(InitializerListConstructor, ComplexTypeWithReallocationRequired)
{
    EXPECT_THAT((absl::InlinedVector<CopyableMovableInstance, 1> { CopyableMovableInstance(0), CopyableMovableInstance(1) }),
        AllOf(SizeIs(2u), CapacityIs(Gt(1u)), ElementsAre(ValueIs(0), ValueIs(1))));
}
...
TYPED_TEST_P(InstanceTest, InitializerListAssign)
{
    ...
    for (size_t original_size = 0; original_size <= 4; ++original_size) {
        ...
        EXPECT_THAT(v, AllOf(SizeIs(3u), ElementsAre(ValueIs(3), ValueIs(4), ValueIs(5))));
        EXPECT_LE(3u, v.capacity());
    }
}

REGISTER_TYPED_TEST_SUITE_P(InstanceTest, Swap, CountConstructorsDestructors, CountConstructorsDestructorsOnCopyConstruction,
    CountConstructorsDestructorsOnMoveConstruction, CountConstructorsDestructorsOnAssignment, CountConstructorsDestructorsOnMoveAssignment,
    CountElemAssignInlineBacking, RangedConstructor, RangedAssign, InitializerListAssign);
...
TEST(AllocatorSupportTest, SwapBothAllocated)
{
    ...
    int64_t allocated2 = 0;
    {
        ...
        EXPECT_THAT(allocated1, Eq(static_cast<int64_t>(v1.capacity() * sizeof(int))));
        EXPECT_THAT(allocated2, Eq(static_cast<int64_t>(v2.capacity() * sizeof(int))));
        v1.swap(v2);
        EXPECT_THAT(v1, ElementsAreArray(ia2));
        EXPECT_THAT(v2, ElementsAreArray(ia1));
        EXPECT_THAT(allocated1, Eq(static_cast<int64_t>(v2.capacity() * sizeof(int))));
        EXPECT_THAT(allocated2, Eq(static_cast<int64_t>(v1.capacity() * sizeof(int))));
    }
...
class MoveConstructibleOnlyInstance : public absl::test_internal::BaseCountedInstance {
public:
    ...
    MoveConstructibleOnlyInstance& operator=(MoveConstructibleOnlyInstance&& other) = delete;
};
...
template <size_t N> using CharVec = absl::InlinedVector<char, N>;
...
template <typename T> struct MySpan {
    T* data;
    size_t size;
};
...
TEST(StorageTest, InlinedCapacityAutoIncrease)
{
    ...
    EXPECT_EQ((absl::InlinedVector<int, 1>().capacity()), sizeof(MySpan<int>) / sizeof(int));
}
