...
TEST(CRC32C, RFC3720)
{
    ...
    EXPECT_EQ(absl::ComputeCrc32c(absl::string_view(data, sizeof(data))), absl::crc32c_t { 0x8a9136aa });

    // 32 bytes of ones.
    memset(data, 0xff, sizeof(data));
    EXPECT_EQ(absl::ComputeCrc32c(absl::string_view(data, sizeof(data))), absl::crc32c_t { 0x62a8ab43 });

    // 32 incrementing bytes.
    for (int i = 0; i < 32; ++i)
        data[i] = static_cast<char>(i);
    EXPECT_EQ(absl::ComputeCrc32c(absl::string_view(data, sizeof(data))), absl::crc32c_t { 0x46dd794e });

    // 32 decrementing bytes.
    for (int i = 0; i < 32; ++i)
        data[i] = static_cast<char>(31 - i);
    EXPECT_EQ(absl::ComputeCrc32c(absl::string_view(data, sizeof(data))), absl::crc32c_t { 0x113fdb5c });
    ...
    EXPECT_EQ(absl::ComputeCrc32c(absl::string_view(reinterpret_cast<const char*>(cmd), sizeof(cmd))), absl::crc32c_t { 0xd9963a56 });
}
...
TEST(CRC32C, Extend)
{
    ...
    EXPECT_EQ(absl::ExtendCrc32c(absl::crc32c_t { base }, extension), absl::crc32c_t { 0xD2F65090 }); // CRC32C of "Hello WorldExtension String"
}
...
TEST(CRC32C, UnextendByZeroes)
{
    ...
    for (const size_t size : kSizes) {
        ...
        EXPECT_EQ(crc_before, absl::crc_internal::UnextendCrc32cByZeroes(crc_after, size));
    }
}
...
TEST(CRC32C, Memcpy)
{
    ...
    for (size_t bytes : kBytesSize) {
        ...
        absl::crc32c_t memcpy_crc = absl::MemcpyCrc32c(&(target_buffer[0]), sample_string.data(), bytes);
