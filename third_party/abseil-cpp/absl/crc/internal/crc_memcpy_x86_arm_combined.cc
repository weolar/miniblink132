...
#if defined(ABSL_INTERNAL_HAVE_X86_64_ACCELERATED_CRC_MEMCPY_ENGINE) || defined(ABSL_INTERNAL_HAVE_ARM_ACCELERATED_CRC_MEMCPY_ENGINE)
...
inline crc32c_t ShortCrcCopy(char* dst, const char* src, std::size_t length, crc32c_t crc)
{
...
ABSL_ATTRIBUTE_NO_SANITIZE_UNDEFINED inline void LargeTailCopy(crc32c_t* crcs, char** dst, const char** src, size_t region_size, size_t copy_rounds)
{
    ...
    while (copy_rounds > 0) {
        for (size_t i = 0; i < vec_regions; i++) {
            ...
            crcs[region]
                = crc32c_t { static_cast<uint32_t>(CRC32_u64(static_cast<uint32_t>(crcs[region]), static_cast<uint64_t>(V128_Extract64<0>(data[i])))) };
            crcs[region]
                = crc32c_t { static_cast<uint32_t>(CRC32_u64(static_cast<uint32_t>(crcs[region]), static_cast<uint64_t>(V128_Extract64<1>(data[i])))) };
        }
        ...
        for (size_t i = 0; i < int_regions; i++) {
            ...
            for (size_t j = 0; j < kIntLoadsPerVec; j++) {
                ...
                crcs[region] = crc32c_t { CRC32_u64(static_cast<uint32_t>(crcs[region]), int_data[data_index]) };
...
template <size_t vec_regions, size_t int_regions> class AcceleratedCrcMemcpyEngine : public CrcMemcpyEngine {
public:
...
// Disable ubsan for benign unaligned access. See b/254108538.
template <size_t vec_regions, size_t int_regions>
ABSL_ATTRIBUTE_NO_SANITIZE_UNDEFINED crc32c_t AcceleratedCrcMemcpyEngine<vec_regions, int_regions>::Compute(
    void* __restrict dst, const void* __restrict src, std::size_t length, crc32c_t initial_crc) const
{
    ...
    if (length < kCrcSmallSize) {
        crc32c_t crc = ExtendCrc32c(initial_crc, absl::string_view(src_bytes, length));
    ...
    while (copy_rounds > kBlocksPerCacheLine) {
        ...
        for (size_t i = 0; i < kBlocksPerCacheLine; i++) {
            ...
            for (size_t j = 0; j < vec_regions; j++) {
                ...
                crcs[region]
                    = crc32c_t { static_cast<uint32_t>(CRC32_u64(static_cast<uint32_t>(crcs[region]), static_cast<uint64_t>(V128_Extract64<0>(vec_data[j])))) };
                crcs[region]
                    = crc32c_t { static_cast<uint32_t>(CRC32_u64(static_cast<uint32_t>(crcs[region]), static_cast<uint64_t>(V128_Extract64<1>(vec_data[j])))) };
    ...
    LargeTailCopy<vec_regions, int_regions>(crcs, &dst_bytes, &src_bytes, region_size, copy_rounds);
    ...
    LargeTailCopy<0, 1>(&crcs[kRegions - 1], &dst_bytes, &src_bytes, 0, tail_blocks);

    // Final tail copy for under 16 bytes.
    crcs[kRegions - 1] = ShortCrcCopy(dst_bytes, src_bytes, tail_size - tail_blocks * kBlockSize, crcs[kRegions - 1]);
...
std::unique_ptr<CrcMemcpyEngine> CrcMemcpy::GetTestEngine(int vector, int integer)
{
