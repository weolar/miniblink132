...
void CRC32::InitTables()
{
    ...
    const uint32_t kCrc32cUnextendPoly = ReverseBits(static_cast<uint32_t>((kCrc32cPoly << 1) ^ 1));
...
void CRC32::Extend(uint32_t* crc, const void* bytes, size_t length) const
{
    ...
    if (static_cast<size_t>(e - p) >= kSwathSize) {
        ...
        const auto step_swath = [this](uint32_t crc_in, const std::uint8_t* ptr) {
            return absl::little_endian::Load32(ptr) ^ this->table_[3][crc_in & 0xff] ^ this->table_[2][(crc_in >> 8) & 0xff]
                ^ this->table_[1][(crc_in >> 16) & 0xff] ^ this->table_[0][crc_in >> 24];
        };
...
void CRC32::ExtendByZeroesImpl(uint32_t* crc, size_t length, const uint32_t zeroes_table[256], const uint32_t poly_table[256])
{
    if (length != 0) {
        uint32_t l = *crc;
        ...
        for (int i = 0; length != 0; i += ZEROES_BASE - 1, length >>= ZEROES_BASE_LG) {
            ...
            if (c != 0) { // if they are not zero,
                ...
                for (int x = 0; x < 32; x += 8) {
                    ...
                    result ^= mtab[l & 3] ^ (mtab[(l >> 2) & 3] << 2) ^ (mtab[(l >> 4) & 3] << 4) ^ (mtab[(l >> 6) & 3] << 6);
                    l >>= 8;
...
void CRC32::Scramble(uint32_t* crc) const
{
    ...
    *crc = RotateRight<uint32_t>(static_cast<unsigned int>(*crc + kScrambleLo), 32, scramble_rotate) & MaskOfLength<uint32_t>(32);
}

void CRC32::Unscramble(uint32_t* crc) const
{
    constexpr int scramble_rotate = (32 / 2) + 1;
    uint64_t rotated = RotateRight<uint32_t>(static_cast<unsigned int>(*crc), 32, 32 - scramble_rotate);
