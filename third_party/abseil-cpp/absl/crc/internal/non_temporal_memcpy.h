...
#if ABSL_HAVE_CPP_ATTRIBUTE(gnu::target) && !defined(_MSC_VER) && (defined(__x86_64__) || defined(__i386__))
...
#endif
inline void*
non_temporal_store_memcpy_avx(void* __restrict dst, const void* __restrict src, size_t len)
{
...
#if ((defined(__AVX__) || defined(ABSL_INTERNAL_CAN_FORCE_AVX)) && defined(__SSE3__)) || (defined(_MSC_VER) && defined(__AVX__))
    uint8_t* d = reinterpret_cast<uint8_t*>(dst);
    const uint8_t* s = reinterpret_cast<const uint8_t*>(src);
