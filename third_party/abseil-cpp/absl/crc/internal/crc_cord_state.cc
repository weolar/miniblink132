...
crc32c_t CrcCordState::Checksum() const
{
    ...
    return absl::RemoveCrc32cPrefix(rep().removed_prefix.crc, rep().prefix_crc.back().crc, rep().prefix_crc.back().length - rep().removed_prefix.length);
}

CrcCordState::PrefixCrc CrcCordState::NormalizedPrefixCrcAtNthChunk(size_t n) const
{
    assert(n < NumChunks());
    if (IsNormalized()) {
    ...
    return PrefixCrc(length, absl::RemoveCrc32cPrefix(rep().removed_prefix.crc, rep().prefix_crc[n].crc, length));
}
...
void CrcCordState::Normalize()
{
    ...
    for (auto& prefix_crc : r->prefix_crc) {
        ...
        prefix_crc.crc = absl::RemoveCrc32cPrefix(r->removed_prefix.crc, prefix_crc.crc, remaining);
