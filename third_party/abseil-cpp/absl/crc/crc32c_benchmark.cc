...
BENCHMARK(BM_Extend)->Arg(0)->Arg(1)->Arg(100)->Arg(10000)->Arg(500000)->Arg(100 * 1000 * 1000);
...
void BM_ExtendCacheMiss(benchmark::State& state)
{
    ...
    for (auto s : state) {
        for (int i = 0; i < total; i += len * 2) {
            benchmark::DoNotOptimize(base);
            benchmark::DoNotOptimize(extension);
            absl::crc32c_t crc = absl::ExtendCrc32c(base, absl::string_view(&extension[i], len));
            benchmark::DoNotOptimize(crc);
        }
    }
...
BENCHMARK(BM_ExtendByZeroes)->RangeMultiplier(10)->Range(1, 1000000)->RangeMultiplier(32)->Range(1, 1 << 20);
...
void BM_UnextendByZeroes(benchmark::State& state)
{
    ...
    for (auto s : state) {
        benchmark::DoNotOptimize(base);
        absl::crc32c_t crc = absl::crc_internal::UnextendCrc32cByZeroes(base, num_zeroes);
        benchmark::DoNotOptimize(crc);
    }
}
BENCHMARK(BM_UnextendByZeroes)->RangeMultiplier(10)->Range(1, 1000000)->RangeMultiplier(32)->Range(1, 1 << 20);
...
BENCHMARK(BM_Concat)->RangeMultiplier(10)->Range(1, 1000000)->RangeMultiplier(32)->Range(1, 1 << 20);
...
void BM_Memcpy(benchmark::State& state)
{
    ...
    for (auto s : state) {
        ...
        absl::crc32c_t crc = absl::MemcpyCrc32c(dest.get(), source.data(), source.size());
        benchmark::DoNotOptimize(crc);
...
void BM_RemoveSuffix(benchmark::State& state)
{
    ...
    std::string suffix = full_string.substr(full_string_len - suffix_len, full_string_len);
    ...
    for (auto s : state) {
        ...
        absl::crc32c_t crc = absl::RemoveCrc32cSuffix(full_string_crc, suffix_crc, suffix_len);
        benchmark::DoNotOptimize(crc);
    }
}
