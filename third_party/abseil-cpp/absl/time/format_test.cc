...
void TestFormatSpecifier(absl::Time t, absl::TimeZone tz, const std::string& fmt, const std::string& ans)
{
    ...
    EXPECT_EQ("xxx " + ans + " yyy", absl::FormatTime("xxx " + fmt + " yyy", t, tz));
}
...
TEST(FormatTime, LocaleSpecific)
{
    ...
    const std::string s = absl::FormatTime("%c", absl::FromTimeT(0), absl::UTCTimeZone());
...
TEST(FormatTime, ExtendedSeconds)
{
    ...
    EXPECT_EQ("1969-12-31 23:59:59.999999", absl::FormatTime("%Y-%m-%d %H:%M:%E*S", t, tz));

    // Here is a "%E*S" case we got wrong for a while.  While the first
    // instant below is correctly rendered as "...:07.333304", the second
    // one used to appear as "...:07.33330499999999999".
    t = absl::FromUnixMicros(1395024427333304);
    EXPECT_EQ("2014-03-17 02:47:07.333304", absl::FormatTime("%Y-%m-%d %H:%M:%E*S", t, tz));
    t += absl::Microseconds(1);
    EXPECT_EQ("2014-03-17 02:47:07.333305", absl::FormatTime("%Y-%m-%d %H:%M:%E*S", t, tz));
}
...
{ // locale specific
    ...
    EXPECT_EQ("Mon, 28 Jun 0077 09:08:07 +0000", absl::FormatTime(absl::RFC1123_full, t, tz));
    EXPECT_EQ("28 Jun 0077 09:08:07 +0000", absl::FormatTime(absl::RFC1123_no_wday, t, tz));
}
...
TEST(FormatTime, InfiniteTime)
{
    ...
    EXPECT_EQ("infinite-future", absl::FormatTime("%H:%M blah", absl::InfiniteFuture(), tz));
    EXPECT_EQ("infinite-past", absl::FormatTime("%H:%M blah", absl::InfinitePast(), tz));
}
...
TEST(ParseTime, NullErrorString)
{
    ...
    EXPECT_FALSE(absl::ParseTime("%H out of range", "42 out of range", &t, nullptr));
}

TEST(ParseTime, WithTimeZone)
{
    const absl::TimeZone tz = absl::time_internal::LoadTimeZone("America/Los_Angeles");
    absl::Time t;
    std::string e;

    // We can parse a string without a UTC offset if we supply a timezone.
    EXPECT_TRUE(absl::ParseTime("%Y-%m-%d %H:%M:%S", "2013-06-28 19:08:09", tz, &t, &e)) << e;
    auto ci = tz.At(t);
    EXPECT_EQ(absl::CivilSecond(2013, 6, 28, 19, 8, 9), ci.cs);
    EXPECT_EQ(absl::ZeroDuration(), ci.subsecond);

    // But the timezone is ignored when a UTC offset is present.
    EXPECT_TRUE(absl::ParseTime("%Y-%m-%d %H:%M:%S %z", "2013-06-28 19:08:09 +0800", tz, &t, &e)) << e;
...
TEST(ParseTime, ExtendedSeconds)
{
    ...
    EXPECT_EQ(absl::UnixEpoch() + absl::Nanoseconds(214748364) + absl::Nanoseconds(1) / 2, t);
    t = absl::UnixEpoch();
    ...
    t = absl::UnixEpoch();
    EXPECT_TRUE(absl::ParseTime("%E*S", "0.214748364801234567890123456789012345678901234567890123456789", &t, &err)) << err;
    EXPECT_EQ(absl::UnixEpoch() + absl::Nanoseconds(214748364) + absl::Nanoseconds(3) / 4, t);
}
...
TEST(ParseTime, InfiniteTime)
{
    ...
    absl::TimeZone tz = absl::UTCTimeZone();
    EXPECT_TRUE(absl::ParseTime("infinite-future %H:%M", "infinite-future 03:04", &t, &err));
    EXPECT_NE(absl::InfiniteFuture(), t);
    EXPECT_EQ(3, tz.At(t).cs.hour());
    EXPECT_EQ(4, tz.At(t).cs.minute());

    // "infinite-past" as literal string
    EXPECT_TRUE(absl::ParseTime("infinite-past %H:%M", "infinite-past 03:04", &t, &err));
...
TEST(ParseTime, FailsOnUnrepresentableTime)
{
    ...
    absl::Time t;
    EXPECT_FALSE(absl::ParseTime("%Y-%m-%d", "-292277022657-01-27", utc, &t, nullptr));
    EXPECT_TRUE(absl::ParseTime("%Y-%m-%d", "-292277022657-01-28", utc, &t, nullptr));
    EXPECT_TRUE(absl::ParseTime("%Y-%m-%d", "292277026596-12-04", utc, &t, nullptr));
    EXPECT_FALSE(absl::ParseTime("%Y-%m-%d", "292277026596-12-05", utc, &t, nullptr));
}
...
{
    const absl::TimeZone lax = absl::time_internal::LoadTimeZone("America/Los_Angeles");
    const absl::Time in = absl::FromCivil(absl::CivilSecond(1977, 6, 28, 9, 8, 7), lax);
    ...
    // RFC3339, which renders subseconds.
    {
        ...
        EXPECT_TRUE(absl::ParseTime(absl::RFC3339_full, s, &out, &err)) << s << ": " << err;
    ...
    // RFC1123, which only does whole seconds.
    {
        ...
        EXPECT_TRUE(absl::ParseTime(absl::RFC1123_full, s, &out, &err)) << s << ": " << err;
...
{
    const absl::TimeZone tz = absl::UTCTimeZone();
    const absl::Time in = absl::FromUnixSeconds(std::numeric_limits<int64_t>::max());
    std::string err;
...
{
    const absl::TimeZone tz = absl::UTCTimeZone();
    const absl::Time in = absl::FromUnixSeconds(std::numeric_limits<int64_t>::min());
    std::string err;
