...
int64_t IDivSlowPath(bool satq, const Duration num, const Duration den, Duration* rem)
{
...
ABSL_ATTRIBUTE_ALWAYS_INLINE inline int64_t IDivDurationImpl(bool satq, const Duration num, const Duration den, Duration* rem)
{
    int64_t q = 0;
...
Duration& Duration::operator+=(Duration rhs)
{
    ...
    rep_lo_ += rhs.rep_lo_;
    if (rhs.rep_hi_.Get() < 0 ? rep_hi_.Get() > orig_rep_hi : rep_hi_.Get() < orig_rep_hi) {
...
double FDivDuration(Duration num, Duration den)
{
    ...
    if (time_internal::IsInfiniteDuration(num) || den == ZeroDuration()) {
        return (num < ZeroDuration()) == (den < ZeroDuration()) ? std::numeric_limits<double>::infinity() : -std::numeric_limits<double>::infinity();
    }
    if (time_internal::IsInfiniteDuration(den))
        return 0.0;
...
bool ParseDuration(absl::string_view dur_sv, Duration* d)
{
    ...
    while (start != end) {
        ...
        int64_t frac_scale;
        Duration unit;
        if (!ConsumeDurationNumber(&start, end, &int_part, &frac_part, &frac_scale) || !ConsumeDurationUnit(&start, end, &unit)) {
            return false;
        }
        if (int_part != 0)
