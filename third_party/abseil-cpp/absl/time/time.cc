...
bool FindTransition(const cctz::time_zone& tz,
    bool (cctz::time_zone::*find_transition)(const cctz::time_point<cctz::seconds>& tp, cctz::time_zone::civil_transition* trans) const, Time t,
...
{
    if (*this == absl::InfiniteFuture())
        return InfiniteFutureBreakdown();
    if (*this == absl::InfinitePast())
...
{
    return absl::FDivDuration(time_internal::ToUnixDuration(t), absl::Milliseconds(1));
}
...
absl::TimeConversion ConvertDateTime(int64_t year, int mon, int day, int hour, int min, int sec, TimeZone tz)
{
    ...
    tc.normalized = false;
    if (year != cs.year() || mon != cs.month() || day != cs.day() || hour != cs.hour() || min != cs.minute() || sec != cs.second()) {
        tc.normalized = true;
    }
    return tc;
}
...
absl::Time FromTM(const struct tm& tm, absl::TimeZone tz)
{
    ...
    const auto ti = tz.At(CivilSecond(tm_year + 1900, tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec));
