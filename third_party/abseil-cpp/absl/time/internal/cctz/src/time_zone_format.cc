...
std::string format(const std::string& format, const time_point<seconds>& tp, const detail::femtoseconds& fs, const time_zone& tz)
{
    ...
    while (cur != end) { // while something is unexamined
        ...
        if (*cur == 'T') {
            ...
            result.append(bp, static_cast<std::size_t>(ep - bp));
            pending = cur += 2;
        } else if (*cur == '*' && cur + 1 != end && (*(cur + 1) == 'S' || *(cur + 1) == 'f')) {
            // Formats %E*S or %E*F.
            if (cur - 2 != pending) {
                FormatTM(&result, std::string(pending, cur - 2), tm);
            }
            ...
            if (const char* np = ParseInt(cur, 0, 0, 1024, &n)) {
                if (*np == 'S' || *np == 'f') {
                    ...
                    if (n > 0) {
                        if (n > kDigits10_64)
                            n = kDigits10_64;
                        bp = Format64(bp, n, (n > 15) ? fs.count() * kExp10[n - 15] : fs.count() / kExp10[15 - n]);
                        if (*np == 'S')
                            *--bp = '.';
                    }
                    if (*np == 'S')
...
bool parse(const std::string& format, const std::string& input, const time_zone& tz, time_point<seconds>* sec, detail::femtoseconds* fs, std::string* err)
{
    ...
    while (data != nullptr && *fmt != '\0') {
        ...
        case 'Z': // ignored; zone abbreviations are ambiguous
            ...
            continue;
        case 's':
            data = ParseInt(data, 0, std::numeric_limits<std::int_fast64_t>::min(), std::numeric_limits<std::int_fast64_t>::max(), &percent_s);
            if (data != nullptr)
                saw_percent_s = true;
            continue;
        case ':':
    ...
    if ((offset < 0 && cs > civil_second::max() + offset) || (offset > 0 && cs < civil_second::min() + offset)) {
        if (err != nullptr)
            *err = "Out-of-range field";
        return false;
    }
