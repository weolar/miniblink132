...
void BM_Time_ToCivil_CCTZ(benchmark::State& state)
{
    ...
    std::chrono::system_clock::time_point tp = std::chrono::system_clock::from_time_t(1384569027);
    std::chrono::system_clock::time_point tp2 = std::chrono::system_clock::from_time_t(1418962578);
    while (state.KeepRunning()) {
        std::swap(tp, tp2);
...
void BM_Time_FromCivil_CCTZ(benchmark::State& state)
{
    ...
    while (state.KeepRunning()) {
        if ((i++ & 1) == 0) {
            benchmark::DoNotOptimize(cctz::convert(cctz::civil_second(2014, 12, 18, 20, 16, 18), tz));
        } else {
            benchmark::DoNotOptimize(cctz::convert(cctz::civil_second(2013, 11, 15, 18, 30, 27), tz));
        }
    }
}
...
void BM_Time_FromCivilDay0_CCTZ(benchmark::State& state)
{
    ...
    while (state.KeepRunning()) {
        if ((i++ & 1) == 0) {
            benchmark::DoNotOptimize(cctz::convert(cctz::civil_second(2014, 12, 0, 20, 16, 18), tz));
        } else {
            benchmark::DoNotOptimize(cctz::convert(cctz::civil_second(2013, 11, 0, 18, 30, 27), tz));
        }
    }
}
...
void BM_Format_FormatTime(benchmark::State& state)
{
    ...
    state.SetLabel(fmt);
    const cctz::time_zone tz = TestTimeZone();
    const std::chrono::system_clock::time_point tp = cctz::convert(cctz::civil_second(1977, 6, 28, 9, 8, 7), tz) + std::chrono::microseconds(1);
    while (state.KeepRunning()) {
...
void BM_Format_ParseTime(benchmark::State& state)
{
    ...
    state.SetLabel(fmt);
    const cctz::time_zone tz = TestTimeZone();
    std::chrono::system_clock::time_point tp = cctz::convert(cctz::civil_second(1977, 6, 28, 9, 8, 7), tz) + std::chrono::microseconds(1);
