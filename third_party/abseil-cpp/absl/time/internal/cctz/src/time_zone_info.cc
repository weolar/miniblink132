...
{
    if (posix.dst_start.date.fmt != PosixTransition::N)
        return false;
    if (posix.dst_start.date.n.day != 0)
        return false;
    if (posix.dst_start.time.offset != 0)
        return false;

    if (posix.dst_end.date.fmt != PosixTransition::J)
        return false;
    if (posix.dst_end.date.j.day != kDaysPerYear[0])
        return false;
    const auto offset = posix.std_offset - posix.dst_offset;
    if (posix.dst_end.time.offset + offset != kSecsPerDay)
        return false;
...
std::int_fast64_t TransOffset(bool leap_year, int jan1_weekday, const PosixTransition& pt)
{
...
inline time_zone::civil_lookup MakeSkipped(const Transition& tr, const civil_second& cs)
{
    time_zone::civil_lookup cl;
...
inline time_zone::civil_lookup MakeRepeated(const Transition& tr, const civil_second& cs)
{
    time_zone::civil_lookup cl;
...
{
    return civil_second(cs.year() + shift, cs.month(), cs.day(), cs.hour(), cs.minute(), cs.second());
}

} // namespace

// Find/make a transition type with these attributes.
bool TimeZoneInfo::GetTransitionType(std::int_fast32_t utc_offset, bool is_dst, const std::string& abbr, std::uint_least8_t* index)
{
    ...
    for (; type_index != transition_types_.size(); ++type_index) {
        ...
        if (tt_abbr == abbr)
            abbr_index = tt.abbr_index;
        if (tt.utc_offset == utc_offset && tt.is_dst == is_dst) {
            if (abbr_index == tt.abbr_index)
                break; // reuse
        }
    }
...
bool TimeZoneInfo::EquivTransitions(std::uint_fast8_t tt1_index, std::uint_fast8_t tt2_index) const
{
...
bool TimeZoneInfo::ExtendTransitions()
{
    ...
    if (future_spec_.empty())
        return true; // last transition prevails
    ...
    for (const year_t limit = last_year_ + 401;; ++last_year_) {
        ...
        if (last_time < tb->unix_time) {
            if (last_time < ta->unix_time)
                transitions_.push_back(*ta);
...
protected:
    explicit FileZoneInfoSource(FilePtr fp, std::size_t len = std::numeric_limits<std::size_t>::max())
        : fp_(std::move(fp))
        , len_(len)
    {
    }
...
private:
    explicit AndroidZoneInfoSource(FilePtr fp, std::size_t len, std::string version)
...
std::unique_ptr<ZoneInfoSource> AndroidZoneInfoSource::Open(const std::string& name)
{
    ...
    for (const char* tzdata : { "/apex/com.android.tzdata/etc/tz/tzdata", "/data/misc/zoneinfo/current/tzdata", "/system/usr/share/zoneinfo/tzdata" }) {
        ...
        const std::size_t index_size = static_cast<std::size_t>(data_offset - index_offset);
        ...
        for (std::size_t i = 0; i != zonecnt; ++i) {
            ...
            if (strcmp(name.c_str() + pos, ebuf) == 0) {
                if (fseek(fp.get(), static_cast<long>(start), SEEK_SET) != 0)
                    break;
                return std::unique_ptr<ZoneInfoSource>(new AndroidZoneInfoSource(std::move(fp), static_cast<std::size_t>(length), vers));
            }
        }
    }
...
private:
    explicit FuchsiaZoneInfoSource(FilePtr fp, std::string version)
        : FileZoneInfoSource(std::move(fp))
        , version_(std::move(version))
    {
    }
    std::string version_;
...
std::unique_ptr<ZoneInfoSource> FuchsiaZoneInfoSource::Open(const std::string& name)
{
    ...
    for (const std::string prefix : prefixes) {
        ...
        return std::unique_ptr<ZoneInfoSource>(new FuchsiaZoneInfoSource(std::move(fp), std::move(version)));
    }
...
bool TimeZoneInfo::Load(ZoneInfoSource* zip)
{
    ...
    for (std::size_t i = 0; i != hdr.typecnt; ++i) {
        transition_types_[i].utc_offset = static_cast<std::int_least32_t>(Decode32(bp));
        if (transition_types_[i].utc_offset >= kSecsPerDay || transition_types_[i].utc_offset <= -kSecsPerDay)
            return false;
        bp += 4;
...
bool TimeZoneInfo::Load(const std::string& name)
{
    ...
    auto zip = cctz_extension::zone_info_source_factory(name, [](const std::string& n) -> std::unique_ptr<ZoneInfoSource> {
        if (auto z = FileZoneInfoSource::Open(n))
            return z;
        if (auto z = AndroidZoneInfoSource::Open(n))
            return z;
        if (auto z = FuchsiaZoneInfoSource::Open(n))
            return z;
        return nullptr;
    });
...
std::unique_ptr<TimeZoneInfo> TimeZoneInfo::Make(const std::string& name)
{
    auto tz = std::unique_ptr<TimeZoneInfo>(new TimeZoneInfo);
    if (!tz->Load(name))
        tz.reset(); // fallback to UTC
    return tz;
}

// BreakTime() translation for a particular transition type.
time_zone::absolute_lookup TimeZoneInfo::LocalTime(std::int_fast64_t unix_time, const TransitionType& tt) const
{
    // A civil time in "+offset" looks like (time+offset) in UTC.
    // Note: We perform two additions in the civil_second domain to
    // sidestep the chance of overflow in (unix_time + tt.utc_offset).
    return { (civil_second() + unix_time) + tt.utc_offset, tt.utc_offset, tt.is_dst, &abbreviations_[tt.abbr_index] };
}
...
time_zone::absolute_lookup TimeZoneInfo::BreakTime(const time_point<seconds>& tp) const
{
    ...
    if (unix_time >= transitions_[timecnt - 1].unix_time) {
        ...
        if (extended_) {
            const std::int_fast64_t diff = unix_time - transitions_[timecnt - 1].unix_time;
    ...
    const Transition* tr = std::upper_bound(begin, begin + timecnt, target, Transition::ByUnixTime());
    local_time_hint_.store(static_cast<std::size_t>(tr - begin), std::memory_order_relaxed);
...
time_zone::civil_lookup TimeZoneInfo::MakeTime(const civil_second& cs) const
{
    ...
    if (cs < begin->civil_sec) {
        ...
        if (tr == nullptr) {
            ...
            time_local_hint_.store(static_cast<std::size_t>(tr - begin), std::memory_order_relaxed);
        }
    }
    ...
    if (tr == begin) {
        if (tr->prev_civil_sec >= cs) {
            ...
            if (cs < tt.civil_min)
                return MakeUnique(time_point<seconds>::min());
    ...
    if (tr == end) {
        if (cs > (--tr)->prev_civil_sec) {
            ...
            const TransitionType& tt(transition_types_[tr->type_index]);
            if (cs > tt.civil_max)
                return MakeUnique(time_point<seconds>::max());
...
bool TimeZoneInfo::NextTransition(const time_point<seconds>& tp, time_zone::civil_transition* trans) const
{
    ...
    std::int_fast64_t unix_time = ToUnixSeconds(tp);
    const Transition target = { unix_time, 0, civil_second(), civil_second() };
    const Transition* tr = std::upper_bound(begin, end, target, Transition::ByUnixTime());
    for (; tr != end; ++tr) { // skip no-op transitions
        std::uint_fast8_t prev_type_index = (tr == begin) ? default_transition_type_ : tr[-1].type_index;
        if (!EquivTransitions(prev_type_index, tr[0].type_index))
            break;
    }
...
bool TimeZoneInfo::PrevTransition(const time_point<seconds>& tp, time_zone::civil_transition* trans) const
{
    ...
    const Transition target = { unix_time, 0, civil_second(), civil_second() };
    const Transition* tr = std::lower_bound(begin, end, target, Transition::ByUnixTime());
    for (; tr != begin; --tr) { // skip no-op transitions
        std::uint_fast8_t prev_type_index = (tr - 1 == begin) ? default_transition_type_ : tr[-2].type_index;
        if (!EquivTransitions(prev_type_index, tr[-1].type_index))
            break;
    }
