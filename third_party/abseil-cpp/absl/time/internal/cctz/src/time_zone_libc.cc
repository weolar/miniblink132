...
#elif defined(__native_client__) || defined(__myriad2__) || defined(__EMSCRIPTEN__)
...
#else
template <typename T> auto tm_gmtoff(const T& tm) -> decltype(tm.tm_gmtoff)
{
    return tm.tm_gmtoff;
}
template <typename T> auto tm_gmtoff(const T& tm) -> decltype(tm.__tm_gmtoff)
{
    return tm.__tm_gmtoff;
}
#endif // tm_gmtoff
...
template <typename T> auto tm_zone(const T& tm) -> decltype(tm.__tm_zone)
{
    return tm.__tm_zone;
}
#endif // tm_zone
...
bool make_time(const civil_second& cs, int is_dst, std::time_t* t, std::tm* tm)
{
    ...
    *t = std::mktime(tm);
    if (*t == std::time_t { -1 }) {
        ...
        if (tmp == nullptr || tmp->tm_year != tm->tm_year || tmp->tm_mon != tm->tm_mon || tmp->tm_mday != tm->tm_mday || tmp->tm_hour != tm->tm_hour
            || tmp->tm_min != tm->tm_min || tmp->tm_sec != tm->tm_sec) {
...
time_zone::civil_lookup TimeZoneLibC::MakeTime(const civil_second& cs) const
{
    if (!local_) {
        ...
        static const civil_second min_tp_cs = civil_second() + ToUnixSeconds(time_point<seconds>::min());
        static const civil_second max_tp_cs = civil_second() + ToUnixSeconds(time_point<seconds>::max());
    ...
    const time_point<seconds> tp = (cs < civil_second()) ? time_point<seconds>::min() : time_point<seconds>::max();
    return { time_zone::civil_lookup::UNIQUE, tp, tp, tp };
}

bool TimeZoneLibC::NextTransition(const time_point<seconds>&, time_zone::civil_transition*) const
{
    return false;
}

bool TimeZoneLibC::PrevTransition(const time_point<seconds>&, time_zone::civil_transition*) const
{
    return false;
}
