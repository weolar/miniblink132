...
void BM_Format_FormatTime(benchmark::State& state)
{
    ...
    const absl::Time t = absl::FromCivil(absl::CivilSecond(1977, 6, 28, 9, 8, 7), lax) + absl::Nanoseconds(1);
...
void BM_Format_ParseTime(benchmark::State& state)
{
    ...
    state.SetLabel(fmt);
    const absl::TimeZone lax = absl::time_internal::LoadTimeZone("America/Los_Angeles");
    absl::Time t = absl::FromCivil(absl::CivilSecond(1977, 6, 28, 9, 8, 7), lax) + absl::Nanoseconds(1);
