...
#define GET_CURRENT_TIME_NANOS_CYCLECLOCK_NOW() ::absl::time_internal::UnscaledCycleClockWrapperForGetCurrentTime::Now()
#endif
...
static_assert(((kMinNSBetweenSamples << (kScale + 1)) >> (kScale + 1)) == kMinNSBetweenSamples, "cannot represent kMaxBetweenSamplesNSScaled");
...
struct ABSL_CACHELINE_ALIGNED TimeState {
    ...
    absl::base_internal::SpinLock lock { absl::kConstInit, base_internal::SCHEDULE_KERNEL_ONLY };
};
...
static int64_t GetCurrentTimeNanosFromKernel(uint64_t last_cycleclock, uint64_t* cycleclock) ABSL_EXCLUSIVE_LOCKS_REQUIRED(time_state.lock)
{
    ...
    do {
        ...
        if (elapsed_cycles >= local_approx_syscall_time_in_cycles && ++loops == 20) { // clock changed frequencies?  Back off.
            ...
            time_state.approx_syscall_time_in_cycles.store(local_approx_syscall_time_in_cycles, std::memory_order_relaxed);
        }
    ...
    } else if (time_state.kernel_time_seen_smaller.fetch_add(1, std::memory_order_relaxed) >= 3) {
        // smaller delays several times in a row; reduce approximation by 12.5%
        const uint64_t new_approximation = local_approx_syscall_time_in_cycles - (local_approx_syscall_time_in_cycles >> 3);
        time_state.approx_syscall_time_in_cycles.store(new_approximation, std::memory_order_relaxed);
        time_state.kernel_time_seen_smaller.store(0, std::memory_order_relaxed);
    }
...
static void ReadTimeSampleAtomic(const struct TimeSampleAtomic* atomic, struct TimeSample* sample)
{
...
int64_t GetCurrentTimeNanos()
{
    ...
    min_cycles_per_sample = time_state.last_sample.min_cycles_per_sample.load(std::memory_order_relaxed);
    ...
    if (seq_read0 == seq_read1 && (seq_read0 & 1) == 0 && (delta_cycles = now_cycles - base_cycles) < min_cycles_per_sample) {
        return static_cast<int64_t>(base_ns + ((delta_cycles * nsscaled_per_cycle) >> kScale));
    }
...
static uint64_t UpdateLastSample(uint64_t now_cycles, uint64_t now_ns, uint64_t delta_cycles, const struct TimeSample* sample) ABSL_ATTRIBUTE_COLD;
...
static int64_t GetCurrentTimeNanosSlowPath() ABSL_LOCKS_EXCLUDED(time_state.lock)
{
    ...
    uint64_t now_cycles;
    uint64_t now_ns = static_cast<uint64_t>(GetCurrentTimeNanosFromKernel(time_state.last_now_cycles, &now_cycles));
...
static uint64_t UpdateLastSample(uint64_t now_cycles, uint64_t now_ns, uint64_t delta_cycles, const struct TimeSample* sample)
    ABSL_EXCLUSIVE_LOCKS_REQUIRED(time_state.lock)
{
    ...
    if (sample->raw_ns == 0 || // no recent sample, or clock went backwards
        ...
        if (new_nsscaled_per_cycle != 0 && diff_ns < 100 * 1000 * 1000 && -diff_ns < 100 * 1000 * 1000) {
            ...
            time_state.last_sample.nsscaled_per_cycle.store(new_nsscaled_per_cycle, std::memory_order_relaxed);
            uint64_t new_min_cycles_per_sample = SafeDivideAndScale(kMinNSBetweenSamples, new_nsscaled_per_cycle);
            time_state.last_sample.min_cycles_per_sample.store(new_min_cycles_per_sample, std::memory_order_relaxed);
