...
template <typename Type1, typename Type2> constexpr bool IsSame()
{
    return (std::is_same<Type1, Type2>::value);
}

struct IdentityFactory {
    template <typename Callback> static Callback AsCallback(Callback callback)
    {
...
struct FunctorClassFactory {
    template <typename Callback> static FunctorClass AsCallback(Callback callback)
    {
...
struct StdFunctionFactory {
    template <typename Callback> static std::function<void()> AsCallback(Callback callback)
    {
...
template <typename> struct CleanupTest : public ::testing::Test { };
...
TYPED_TEST(CleanupTest, FactoryAndCTADProduceSameType)
{
    ...
    {
        auto factory_cleanup = absl::MakeCleanup(FunctorClassFactory::AsCallback([] {}));
    ...
    {
        auto factory_cleanup = absl::MakeCleanup(StdFunctionFactory::AsCallback([] {}));
...
TYPED_TEST(CleanupTest, BasicUsage)
{
    ...
    {
        auto cleanup = absl::MakeCleanup(TypeParam::AsCallback([&called] { called = true; }));
...
TYPED_TEST(CleanupTest, Move)
{
    ...
    {
        auto moved_from_cleanup = absl::MakeCleanup(TypeParam::AsCallback([&called] { called = true; }));
        EXPECT_FALSE(called); // Constructor shouldn't invoke the callback
