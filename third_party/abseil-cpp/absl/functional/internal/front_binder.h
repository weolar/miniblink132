...
template <class R, class Tuple, size_t... Idx, class... Args> R Apply(Tuple&& bound, absl::index_sequence<Idx...>, Args&&... free)
{
    return base_internal::invoke(std::forward<Tuple>(bound).template get<Idx>()..., std::forward<Args>(free)...);
}

template <class F, class... BoundArgs> class FrontBinder {
...
public:
    ...
    template <class... FreeArgs, class R = base_internal::invoke_result_t<F&, BoundArgs&..., FreeArgs&&...>> R operator()(FreeArgs&&... free_args) &
    {
...
template <class F, class... BoundArgs> using bind_front_t = FrontBinder<decay_t<F>, absl::decay_t<BoundArgs>...>;
