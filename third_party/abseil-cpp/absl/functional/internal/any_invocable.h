...
template <class T> struct IsAnyInvocable : std::false_type { };

template <class Sig> struct IsAnyInvocable<AnyInvocable<Sig>> : std::true_type { };
//
////////////////////////////////////////////////////////////////////////////////
...
template <class T>
using IsStoredLocally = std::integral_constant<bool,
    sizeof(T) <= kStorageSize && alignof(T) <= kAlignment && kAlignment % alignof(T) == 0 && std::is_nothrow_move_constructible<T>::value>;
...
template <class ReturnType, class F, class... P, typename = absl::enable_if_t<std::is_void<ReturnType>::value>> void InvokeR(F&& f, P&&... args)
{
    absl::base_internal::invoke(std::forward<F>(f), std::forward<P>(args)...);
}

template <class ReturnType, class F, class... P, absl::enable_if_t<!std::is_void<ReturnType>::value, int> = 0> ReturnType InvokeR(F&& f, P&&... args)
{
...
template <typename T> T ForwardImpl(std::true_type);

template <typename T> T&& ForwardImpl(std::false_type);
...
template <class T> struct ForwardedParameter {
    using type = decltype((ForwardImpl<T>)(std::integral_constant<bool, std::is_scalar<T>::value>()));
};

template <class T> using ForwardedParameterType = typename ForwardedParameter<T>::type;
//
////////////////////////////////////////////////////////////////////////////////
...
template <class T> T& ObjectInLocalStorage(TypeErasedState* const state)
{
...
using ManagerType = void(FunctionToCall /*operation*/, TypeErasedState* /*from*/, TypeErasedState* /*to*/) ABSL_INTERNAL_NOEXCEPT_SPEC(true);
...
template <class T> void LocalManagerNontrivial(FunctionToCall operation, TypeErasedState* const from, TypeErasedState* const to) noexcept
{
...
ReturnType LocalInvoker(TypeErasedState* const state, ForwardedParameterType<P>... args) noexcept(SigIsNoexcept)
{
    ...
    static_assert(IsStoredLocally<RawT>::value, "Target object must be in local storage in order to be invoked from it.");
...
template <class T> struct IsInPlaceType : std::false_type { };

template <class T> struct IsInPlaceType<absl::in_place_type_t<T>> : std::true_type { };
//
////////////////////////////////////////////////////////////////////////////////
...
template <class QualDecayedTRef> struct TypedConversionConstruct { };
...
template <class Sig> class Impl { }; // Note: This is partially-specialized later.
...
template <bool SigIsNoexcept, class ReturnType, class... P> class CoreImpl;

constexpr bool IsCompatibleConversion(void*, void*)
{
    return false;
}
template <bool NoExceptSrc, bool NoExceptDest, class... T> constexpr bool IsCompatibleConversion(CoreImpl<NoExceptSrc, T...>*, CoreImpl<NoExceptDest, T...>*)
{
...
public:
    ...
    // type to be stored.
    template <class QualDecayedTRef, class F> explicit CoreImpl(TypedConversionConstruct<QualDecayedTRef>, F&& f)
    {
        using DecayedT = RemoveCVRef<QualDecayedTRef>;

        constexpr TargetType kTargetType = (std::is_pointer<DecayedT>::value || std::is_member_pointer<DecayedT>::value) ? TargetType::kPointer
    ...
    // type to be stored.
    template <class QualTRef, class... Args> explicit CoreImpl(absl::in_place_type_t<QualTRef>, Args&&... args)
    {
    ...
    template <TargetType target_type, class QualDecayedTRef, class F, absl::enable_if_t<target_type == TargetType::kPointer, int> = 0> void Initialize(F&& f)
    {
...
#pragma GCC diagnostic pop
    ...
    template <TargetType target_type, class QualDecayedTRef, class F, typename = absl::enable_if_t<target_type == TargetType::kOther>> void Initialize(F&& f)
    {
        InitializeStorage<QualDecayedTRef>(std::forward<F>(f));
    }

    // Use local (inline) storage for applicable target object types.
    template <class QualTRef, class... Args, typename = absl::enable_if_t<IsStoredLocally<RemoveCVRef<QualTRef>>::value>> void InitializeStorage(Args&&... args)
    {
        using RawT = RemoveCVRef<QualTRef>;
    ...
    template <class QualTRef, class... Args, absl::enable_if_t<!IsStoredLocally<RemoveCVRef<QualTRef>>::value, int> = 0> void InitializeStorage(Args&&... args)
    {
    ...
    template <class T, typename = absl::enable_if_t<std::is_trivially_copyable<T>::value>> void InitializeLocalManager()
    {
        manager_ = LocalManagerTrivial;
    }

    template <class T, absl::enable_if_t<!std::is_trivially_copyable<T>::value, int> = 0> void InitializeLocalManager()
    {
    ...
    template <class T, class... Args, typename = absl::enable_if_t<HasTrivialRemoteStorage<T>::value>> void InitializeRemoteManager(Args&&... args)
    {
    ...
    template <class T, class... Args, absl::enable_if_t<!HasTrivialRemoteStorage<T>::value, int> = 0> void InitializeRemoteManager(Args&&... args)
    {
    ...
    template <typename Other> struct IsCompatibleAnyInvocable {
        static constexpr bool value = false;
    };

    template <typename Sig> struct IsCompatibleAnyInvocable<AnyInvocable<Sig>> {
        static constexpr bool value = (IsCompatibleConversion)(static_cast<typename AnyInvocable<Sig>::CoreImpl*>(nullptr), static_cast<CoreImpl*>(nullptr));
    };
...
// the underlying type.
template <class T> struct UnwrapStdReferenceWrapperImpl {
    using type = T;
};

template <class T> struct UnwrapStdReferenceWrapperImpl<std::reference_wrapper<T>> {
    using type = T&;
};

template <class T> using UnwrapStdReferenceWrapper = typename UnwrapStdReferenceWrapperImpl<T>::type;
//
////////////////////////////////////////////////////////////////////////////////
...
template <class... T> using TrueAlias = std::integral_constant<bool, sizeof(absl::void_t<T...>*) != 0>;
...
using CanConvert = TrueAlias<absl::enable_if_t<!IsInPlaceType<RemoveCVRef<F>>::value>, absl::enable_if_t<Impl<Sig>::template CallIsValid<F>::value>,
    absl::enable_if_t<Impl<Sig>::template CallIsNoexceptIfSigIsNoexcept<F>::value>, absl::enable_if_t<std::is_constructible<absl::decay_t<F>, F>::value>>;

/*SFINAE constraints for the std::in_place constructors.*/
template <class Sig, class F, class... Args>
using CanEmplace = TrueAlias<absl::enable_if_t<Impl<Sig>::template CallIsValid<F>::value>,
    absl::enable_if_t<Impl<Sig>::template CallIsNoexceptIfSigIsNoexcept<F>::value>, absl::enable_if_t<std::is_constructible<absl::decay_t<F>, Args...>::value>>;

/*SFINAE constraints for the conversion-assign operator.*/
template <class Sig, class F, class = absl::enable_if_t<!std::is_same<RemoveCVRef<F>, AnyInvocable<Sig>>::value>>
using CanAssign = TrueAlias<absl::enable_if_t<Impl<Sig>::template CallIsValid<F>::value>,
    absl::enable_if_t<Impl<Sig>::template CallIsNoexceptIfSigIsNoexcept<F>::value>, absl::enable_if_t<std::is_constructible<absl::decay_t<F>, F>::value>>;

/*SFINAE constraints for the reference-wrapper conversion-assign operator.*/
template <class Sig, class F>
using CanAssignReferenceWrapper = TrueAlias<absl::enable_if_t<Impl<Sig>::template CallIsValid<std::reference_wrapper<F>>::value>,
    absl::enable_if_t<Impl<Sig>::template CallIsNoexceptIfSigIsNoexcept<std::reference_wrapper<F>>::value>>;
...
#define ABSL_INTERNAL_ANY_INVOCABLE_NOEXCEPT_CONSTRAINT_true(inv_quals)                                                                                        \
    absl::enable_if_t<absl::disjunction<std::is_nothrow_invocable_r<ReturnType, UnwrapStdReferenceWrapper<absl::decay_t<F>> inv_quals, P...>,                  \
        std::conjunction<std::is_nothrow_invocable<UnwrapStdReferenceWrapper<absl::decay_t<F>> inv_quals, P...>,                                               \
            std::is_same<ReturnType, absl::base_internal::invoke_result_t<UnwrapStdReferenceWrapper<absl::decay_t<F>> inv_quals, P...>>>>::value>
...
#define ABSL_INTERNAL_ANY_INVOCABLE_IMPL_(cv, ref, inv_quals, noex)                                                                                            \
    template <class ReturnType, class... P> class Impl<ReturnType(P...) cv ref ABSL_INTERNAL_NOEXCEPT_SPEC(noex)> : public CoreImpl<noex, ReturnType, P...> {  \
    public:                                                                                                                                                    \
        ...
        template <class F>                                                                                                                                     \
        using CallIsValid = TrueAlias<absl::enable_if_t<absl::disjunction<absl::base_internal::is_invocable_r<ReturnType, absl::decay_t<F> inv_quals, P...>,   \
            std::is_same<ReturnType, absl::base_internal::invoke_result_t<absl::decay_t<F> inv_quals, P...>>>::value>>;                                        \
                                                                                                                                                               \
        /*SFINAE constraint to check if F is nothrow-invocable when necessary*/                                                                                \
        template <class F> using CallIsNoexceptIfSigIsNoexcept = TrueAlias<ABSL_INTERNAL_ANY_INVOCABLE_NOEXCEPT_CONSTRAINT(inv_quals, noex)>;                  \
                                                                                                                                                               \
        ...
        explicit Impl(ConversionConstruct, F&& f)                                                                                                              \
            : Core(TypedConversionConstruct<typename std::decay<F>::type inv_quals>(), std::forward<F>(f))                                                     \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
                                                                                                                                                               \
        ...
        explicit Impl(absl::in_place_type_t<T>, Args&&... args)                                                                                                \
            : Core(absl::in_place_type<absl::decay_t<T> inv_quals>, std::forward<Args>(args)...)                                                               \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
                                                                                                                                                               \
        /*Raises a fatal error when the AnyInvocable is invoked after a move*/                                                                                 \
        static ReturnType InvokedAfterMove(TypeErasedState*, ForwardedParameterType<P>...) noexcept(noex)                                                      \
        {                                                                                                                                                      \
        ...
        {                                                                                                                                                      \
            ...
            if (!std::is_const<QualifiedTestType>::value && std::is_rvalue_reference<QualifiedTestType>::value) {                                              \
                ABSL_ASSERT([this]() {                                                                                                                         \
        ...
        {                                                                                                                                                      \
            ...
            return this->ExtractInvoker()(const_cast<TypeErasedState*>(&this->state_), static_cast<ForwardedParameterType<P>>(args)...);                       \
        }                                                                                                                                                      \
    }
