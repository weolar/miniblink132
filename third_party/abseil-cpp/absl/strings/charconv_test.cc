...
void TestDoubleParse(absl::string_view str, double expected_number)
{
    ...
    absl::from_chars_result result = absl::from_chars(str.data(), str.data() + str.length(), actual_number);
...
TEST(FromChars, NearRoundingCasesExplicit)
{
    ...
    EXPECT_EQ(ToDouble("20505426358836677347.e-221"), ldexp(4524032052079546, -722));
    EXPECT_EQ(ToDouble("836168422905420598437.e-234"), ldexp(5070963299887562, -760));
    EXPECT_EQ(ToDouble("4891559871276714924261.e222"), ldexp(6452687840519111, 757));
    ...
    EXPECT_EQ(ToDouble("25188282901709339043.e-252"), ldexp(5635662608542340, -825));
    EXPECT_EQ(ToDouble("308984926168550152811.e-052"), ldexp(5644774693823803, -157));
    EXPECT_EQ(ToDouble("6372891218502368041059.e064"), ldexp(4616868614322430, 233));
...
void TestHalfwayValue(const std::string& mantissa, int exponent, FloatType expected_low, FloatType expected_high, FloatType expected_half)
{
    ...
    absl::from_chars(high_rep.data(), high_rep.data() + high_rep.size(), actual_high);
    EXPECT_EQ(expected_high, actual_high);

    std::string halfway_rep = absl::StrCat(mantissa, "e", exponent);
    FloatType actual_half = 0;
    absl::from_chars(halfway_rep.data(), halfway_rep.data() + halfway_rep.size(), actual_half);
...
TEST(FromChars, FloatRounding)
{
    ...
    // penultimate_normal and last_normal.  Round-to-even rounds down.
    TestHalfwayValue("3.40282336497324057985868971510891282432", 38, penultimate_normal, last_normal, penultimate_normal);
}
...
TEST(FromChars, ReturnValuePtr)
{
    ...
    result = absl::from_chars(overflow.data(), overflow.data() + overflow.size(), d);
    EXPECT_EQ(result.ec, std::errc::result_out_of_range);
...
TEST(FromChars, TestVersusStrtod)
{
    for (int mantissa = 1000000; mantissa <= 9999999; mantissa += 501) {
        for (int exponent = -300; exponent < 300; ++exponent) {
            ...
            double absl_value = 0;
            absl::from_chars(candidate.data(), candidate.data() + candidate.size(), absl_value);
...
template <typename Float> bool Identical(Float a, Float b)
{
...
{
    for (std::string n_char_sequence : { "", "1", "2", "3", "fff", "FFF", "200000", "400000", "4000000000000", "8000000000000", "abc123",
             "legal_but_unexpected", "99999999999999999999999", "_" }) {
        std::string input = absl::StrCat("nan(", n_char_sequence, ")");
        SCOPED_TRACE(input);
        double from_chars_double;
        absl::from_chars(input.data(), input.data() + input.size(), from_chars_double);
        ...
        absl::from_chars(negative_input.data(), negative_input.data() + negative_input.size(), negative_from_chars_double);
...
TEST(FromChars, NaNFloats)
{
    for (std::string n_char_sequence : { "", "1", "2", "3", "fff", "FFF", "200000", "400000", "4000000000000", "8000000000000", "abc123",
        ...
        absl::from_chars(input.data(), input.data() + input.size(), from_chars_float);
        ...
        absl::from_chars(negative_input.data(), negative_input.data() + negative_input.size(), negative_from_chars_float);
...
void TestOverflowAndUnderflow(
    const std::function<std::string(int)>& input_generator, const std::function<Float(int)>& expected_generator, int lower_bound, int upper_bound)
{
    // test legal values near lower_bound
    int index, step;
    for (index = lower_bound, step = 1; index < upper_bound; index += step, step = NextStep(step)) {
    ...
    for (index = upper_bound, step = 1; index > lower_bound; index -= step, step = NextStep(step)) {
        std::string input = input_generator(index);
        SCOPED_TRACE(input);
        Float expected = expected_generator(index);
        Float actual;
    ...
    for (index = lower_bound - 1, step = 1; index > -1000000; index -= step, step = NextStep(step)) {
        std::string input = input_generator(index);
        SCOPED_TRACE(input);
        Float actual;
        auto result = absl::from_chars(input.data(), input.data() + input.size(), actual);
    ...
    for (index = upper_bound + 1, step = 1; index < 1000000; index += step, step = NextStep(step)) {
        std::string input = input_generator(index);
        SCOPED_TRACE(input);
        Float actual;
        auto result = absl::from_chars(input.data(), input.data() + input.size(), actual);
