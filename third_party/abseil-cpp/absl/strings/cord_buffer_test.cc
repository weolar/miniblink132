...
public:
    static cord_internal::CordRep* ConsumeValue(CordBuffer& buffer, absl::string_view& short_value)
    {
...
INSTANTIATE_TEST_SUITE_P(MediumSize, CordBufferTest, testing::Values(1, kInlinedSize - 1, kInlinedSize, kInlinedSize + 1, kDefaultLimit - 1, kDefaultLimit));
...
TEST(CordBufferTest, CreateSsoWithDefaultLimit)
{
    ...
    EXPECT_THAT(CordBufferTestPeer::ConsumeValue(buffer, short_value), Eq(nullptr));
...
TEST_P(CordBufferTest, ConsumeValue)
{
    ...
    absl::string_view short_value;
    if (cord_internal::CordRep* rep = CordBufferTestPeer::ConsumeValue(buffer, short_value)) {
...
{
    const size_t requested = GetParam();
    CordBuffer buffer = CordBuffer::CreateWithCustomLimit(kMaxFlatSize, requested);
    EXPECT_THAT(buffer.capacity(), Ge(requested));
    EXPECT_THAT(buffer.capacity(), Le(MaxCapacityFor(kMaxFlatSize, requested)));
    EXPECT_THAT(buffer.length(), Eq(0));
...
TEST(CordLargeBufferTest, CreateAtOrBelowDefaultLimit)
{
    ...
    EXPECT_THAT(buffer.capacity(), Le(MaxCapacityFor(kMaxFlatSize, kDefaultLimit)));
