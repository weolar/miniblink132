...
#define ABSL_EXPECT_DEATH_IF_SUPPORTED(statement, regex) EXPECT_DEATH_IF_SUPPORTED(statement, ".*")
#else
...
static_assert(!absl::type_traits_internal::IsOwner<absl::string_view>::value && absl::type_traits_internal::IsView<absl::string_view>::value,
    "string_view is a view, not an owner");

static_assert(absl::type_traits_internal::IsLifetimeBoundAssignment<absl::string_view, std::string>::value, "lifetimebound assignment not detected");
...
template <typename T> struct is_type {
    template <typename U> static bool same(U)
    {
        return false;
    }
    static bool same(T)
...
TEST(StringViewTest, STL2)
{
    ...
    EXPECT_EQ(a.find(c.data(), absl::string_view::npos, 0), absl::string_view::npos);
    EXPECT_EQ(b.find(c.data(), absl::string_view::npos, 0), absl::string_view::npos);
    // empty string nonsense
...
TEST(StringViewTest, STL2FindFirst)
{
    ...
    EXPECT_EQ(a.find_first_not_of(d, absl::string_view::npos), absl::string_view::npos);
    EXPECT_EQ(a.find_first_not_of(e, absl::string_view::npos), absl::string_view::npos);
...
#else
    ABSL_EXPECT_DEATH_IF_SUPPORTED((void)a.substr(99, 2), "absl::string_view::substr");
#endif
}
...
TEST(StringViewTest, FindConformance)
{
    ...
    for (const auto& s : specs) {
        ...
        for (size_t i = 0; i <= sp.size(); ++i) {
            ...
            EXPECT_EQ(sp.find_first_of(s.needle, pos), st.find_first_of(s.needle, pos));
            EXPECT_EQ(sp.find_first_not_of(s.needle, pos), st.find_first_not_of(s.needle, pos));
            EXPECT_EQ(sp.find_last_of(s.needle, pos), st.find_last_of(s.needle, pos));
            EXPECT_EQ(sp.find_last_not_of(s.needle, pos), st.find_last_not_of(s.needle, pos));
        }
    }
}
...
#if !defined(ABSL_USES_STD_STRING_VIEW) || (!(defined(_GLIBCXX_RELEASE) && _GLIBCXX_RELEASE >= 9) && !defined(_LIBCPP_VERSION) && !defined(_MSC_VER))
...
#if ABSL_HAVE_BUILTIN(__builtin_strlen) || (defined(__GNUC__) && !defined(__clang__))
#define ABSL_HAVE_CONSTEXPR_STRING_VIEW_FROM_CSTR 1
...
#if ABSL_HAVE_BUILTIN(__builtin_memcmp) || (defined(__GNUC__) && !defined(__clang__))
#define ABSL_HAVE_CONSTEXPR_STRING_VIEW_COMPARISON 1
...
#if !defined(__clang__) || 3 < __clang_major__ || (3 == __clang_major__ && 4 < __clang_minor__)
    ...
    static_assert(const_begin + const_size == const_end, "pointer arithmetic check");
    static_assert(const_begin + const_length == const_end, "pointer arithmetic check");
#ifndef _MSC_VER
...
{
    EXPECT_TRUE((std::is_nothrow_constructible<absl::string_view, const std::string&>::value));
    EXPECT_TRUE((std::is_nothrow_constructible<absl::string_view, const std::string&>::value));
...
{
    ABSL_EXPECT_DEATH_IF_SUPPORTED(absl::string_view("xyz", static_cast<size_t>(-1)), "len <= kMaxSize");
}
...
public:
    ...
    template <typename T> std::string Pad(const T& s, int width, char fill = 0)
    {
