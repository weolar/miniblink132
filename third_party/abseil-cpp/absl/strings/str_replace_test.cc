...
TEST(StrReplaceAll, ManyReplacementsInMap)
{
    ...
    std::string s = absl::StrReplaceAll("$who bought $count #Noun. Thanks $who!", replacements);
...
TEST(StrReplaceAll, ReplacementsInPlace)
{
    ...
    int count;
    count = absl::StrReplaceAll({ { "$count", absl::StrCat(5) }, { "$who", "Bob" }, { "#Noun", "Apples" } }, &s);
    EXPECT_EQ(count, 4);
    EXPECT_EQ("Bob bought 5 Apples. Thanks Bob!", s);
}
...
struct Cont {
    Cont() = default;
    explicit Cont(absl::string_view src)
        : data(src)
    {
    }
...
template <int index> absl::string_view get(const Cont& c)
{
...
TEST(StrReplaceAll, VariableNumber)
{
    ...
    {
        std::list<std::pair<absl::string_view, absl::string_view>> replacements = { { "a", "x" }, { "b", "y" }, { "c", "z" } };
...
TEST(StrReplaceAll, Inplace)
{
    ...
    // Replace once in the middle.
    s = "Abc!";
    reps = absl::StrReplaceAll({ { "a", "x" }, { "ab", "xy" }, { "b", "y" }, { "bc", "yz" }, { "c", "z" } }, &s);
    EXPECT_EQ(reps, 1);
    EXPECT_EQ(s, "Ayz!");
