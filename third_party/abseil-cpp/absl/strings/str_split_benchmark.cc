...
void BM_Split2StringViewByAnyChar(benchmark::State& state)
{
    ...
    for (auto _ : state) {
        std::vector<absl::string_view> result = absl::StrSplit(test, absl::ByAnyChar(kDelimiters));
        benchmark::DoNotOptimize(result);
    }
}
...
void BM_SplitStringToUnorderedSet(benchmark::State& state)
{
    ...
    for (auto _ : state) {
        std::unordered_set<std::string> result = absl::StrSplit(test, ':', absl::SkipEmpty());
        benchmark::DoNotOptimize(result);
    }
}
...
void BM_SplitStringToUnorderedMap(benchmark::State& state)
{
    ...
    for (auto _ : state) {
        std::unordered_map<std::string, std::string> result = absl::StrSplit(test, ':', absl::SkipEmpty());
        benchmark::DoNotOptimize(result);
    }
}
...
template <typename DelimiterFactory> void BM_SplitStringWithOneChar(benchmark::State& state)
{
    const auto delimiter = DelimiterFactory()();
    std::vector<absl::string_view> pieces;
    size_t v = 0;
    for (auto _ : state) {
        pieces = absl::StrSplit("The quick brown fox jumps over the lazy dog", delimiter);
        v += pieces.size();
    }
...
template <typename DelimiterFactory> void BM_SplitStringWithOneCharNoVector(benchmark::State& state)
{
    const auto delimiter = DelimiterFactory()();
    size_t v = 0;
    for (auto _ : state) {
        auto splitter = absl::StrSplit("The quick brown fox jumps over the lazy dog", delimiter);
