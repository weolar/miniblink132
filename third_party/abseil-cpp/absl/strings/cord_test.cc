...
static void AddExternalMemory(absl::string_view s, absl::Cord* dst)
{
    ...
    dst->Append(absl::MakeCordFromExternal(*str, [str](absl::string_view data) { DeleteExternalString(data, str); }));
}
...
static void AddNewStringBlock(const std::string& str, absl::Cord* dst)
{
    ...
    dst->Append(absl::MakeCordFromExternal(absl::string_view(data, str.size()), [](absl::string_view s) { delete[] s.data(); }));
}
...
INSTANTIATE_TEST_SUITE_P(WithParam, CordTest, testing::Bool(), CordTest::ToString);
...
TEST_P(CordTest, GigabyteCordFromExternal)
{
    ...
    absl::Cord from = absl::MakeCordFromExternal(absl::string_view(data, length), [](absl::string_view sv) { delete[] sv.data(); });
...
static absl::Cord MakeExternalCord(int size)
{
    ...
    absl::Cord cord;
    cord.Append(absl::MakeCordFromExternal(absl::string_view(buffer, size), [](absl::string_view s) { delete[] s.data(); }));
    return cord;
}
...
TEST_P(CordTest, Assignment)
{
    ...
    std::vector<std::pair<absl::string_view, absl::string_view>> test_string_pairs = { { "hi there", "foo" }, { "loooooong coooooord", "short cord" },
        { "short cord", "loooooong coooooord" }, { "loooooong coooooord1", "loooooong coooooord2" } };
    for (std::pair<absl::string_view, absl::string_view> test_strings : test_string_pairs) {
    ...
    }
    {
        ...
        absl::Cord* my_small_alias = my_unique_true_boolean ? &my_small_cord : &my_big_cord;
        absl::Cord* my_big_alias = !my_unique_true_boolean ? &my_small_cord : &my_big_cord;
...
TEST_P(CordTest, Contains)
{
    ...
    auto fragmented_haystack = absl::MakeFragmentedCord({ "this", " ", "is", " ", "a", " ", "fragmented", " ", "cord" });
    ...
    EXPECT_FALSE(flat_haystack.Contains(absl::MakeFragmentedCord({ "is", " ", "an" })));
    EXPECT_FALSE(fragmented_haystack.Contains(absl::MakeFragmentedCord({ "is", " ", "an" })));
    ...
    EXPECT_TRUE(flat_haystack.Contains(absl::MakeFragmentedCord({ "is", " ", "a" })));
    EXPECT_TRUE(fragmented_haystack.Contains(absl::MakeFragmentedCord({ "is", " ", "a" })));
}
...
TEST_P(CordTest, Find)
{
    ...
    EXPECT_EQ(fragmented_haystack.Find(absl::Cord("")), fragmented_haystack.char_begin());
    ...
    EXPECT_EQ(empty_haystack.Find(fragmented_haystack), empty_haystack.char_end());
    ...
    EXPECT_EQ(fragmented_haystack.Find(absl::Cord("z")), fragmented_haystack.char_end());
    ...
    EXPECT_EQ(fragmented_haystack.Find(absl::Cord("is an")), fragmented_haystack.char_end());
    EXPECT_EQ(flat_haystack.Find(absl::MakeFragmentedCord({ "is", " ", "an" })), flat_haystack.char_end());
    EXPECT_EQ(fragmented_haystack.Find(absl::MakeFragmentedCord({ "is", " ", "an" })), fragmented_haystack.char_end());

    EXPECT_EQ(flat_haystack.Find("is a"), std::next(flat_haystack.char_begin(), 5));
    EXPECT_EQ(fragmented_haystack.Find("is a"), std::next(fragmented_haystack.char_begin(), 5));
    EXPECT_EQ(flat_haystack.Find(absl::Cord("is a")), std::next(flat_haystack.char_begin(), 5));
    EXPECT_EQ(fragmented_haystack.Find(absl::Cord("is a")), std::next(fragmented_haystack.char_begin(), 5));
    EXPECT_EQ(flat_haystack.Find(absl::MakeFragmentedCord({ "is", " ", "a" })), std::next(flat_haystack.char_begin(), 5));
    EXPECT_EQ(fragmented_haystack.Find(absl::MakeFragmentedCord({ "is", " ", "a" })), std::next(fragmented_haystack.char_begin(), 5));
}
...
TEST_P(CordTest, Subcord)
{
    ...
    for (size_t pos : positions) {
        ...
        for (size_t end_pos : positions) {
            ...
            ASSERT_EQ(absl::string_view(s).substr(pos, end_pos - pos), std::string(sa)) << a;
...
static void VerifyCopyToString(const absl::Cord& cord)
{
    ...
    if (cord.size() <= kInitialLength) {
        EXPECT_EQ(has_initial_contents.data(), address_before_copy) << "CopyCordToString allocated new string storage; "
...
TEST_P(CordTest, CopyToString)
{
    ...
    VerifyCopyToString(MaybeHardened(absl::MakeFragmentedCord({ "fragmented ", "cord ", "to ", "test ", "copying ", "to ", "a ", "string." })));
}
...
static void VerifyAppendCordToString(const absl::Cord& cord)
{
    ...
    std::string expected_after_append = absl::StrCat(kInitialContents, std::string(cord));
    ...
    EXPECT_EQ(has_reserved_capacity.data(), address_before_copy) << "AppendCordToString allocated new string storage; "
...
TEST_P(CordTest, AppendToString)
{
    ...
    VerifyAppendCordToString(MaybeHardened(absl::MakeFragmentedCord({ "fragmented ", "cord ", "to ", "test ", "appending ", "to ", "a ", "string." })));
}
...
public:
    ...
    size_t limit() const
    {
        return is_default() ? absl::CordBuffer::kDefaultLimit : absl::CordBuffer::kCustomLimit;
    }

    size_t maximum_payload() const
    {
        return is_default() ? absl::CordBuffer::MaximumPayload() : absl::CordBuffer::MaximumPayload(limit());
    }

    absl::CordBuffer GetAppendBuffer(absl::Cord& cord, size_t capacity, size_t min_capacity = 16)
    {
        return is_default() ? cord.GetAppendBuffer(capacity, min_capacity) : cord.GetCustomAppendBuffer(limit(), capacity, min_capacity);
    }
};

INSTANTIATE_TEST_SUITE_P(WithParam, CordAppendBufferTest, testing::Bool(), CordAppendBufferTest::ToString);
...
TEST_P(CordTest, TryFlatCommonlyAssumedInvariants)
{
    ...
    absl::string_view fragments[] = { "A fragmented test", " cord", " to test subcords", " of ", "a", " cord for",
        " each chunk "
        "returned by the ",
        "iterator" };
...
static void VerifyFlatten(absl::Cord c)
{
    ...
    // wasn't copied.
    if (already_flat_and_non_empty) {
        EXPECT_EQ(old_flat.data(), new_flat.data()) << "Allocated new memory even though the Cord was already flat.";
    }
...
TEST_P(CordTest, Flatten)
{
    ...
    VerifyFlatten(MaybeHardened(absl::Cord("larger than small buffer optimization")));
    VerifyFlatten(MaybeHardened(absl::MakeFragmentedCord({ "small ", "fragmented ", "cord" })));
...
{
    absl::Cord cord = absl::MakeCordFromExternal("foo bar baz", [](absl::string_view s) { DoNothing(s, nullptr); });
...
absl::Cord CordWithZedBlock(size_t size)
{
    ...
    absl::Cord cord = absl::MakeCordFromExternal(absl::string_view(data, size), [](absl::string_view s) { delete[] s.data(); });
    return cord;
}
...
absl::Cord SpliceCord(const absl::Cord& blob, int64_t offset, const absl::Cord& block)
{
    CHECK_GE(offset, 0);
...
void VerifyComparison(const CordCompareTestCase& test_case)
{
    ...
    EXPECT_EQ(expected, test_case.lhs_cord.Compare(test_case.rhs_cord)) << "LHS=" << lhs_string << "; RHS=" << rhs_string;
    EXPECT_EQ(expected, test_case.lhs_cord.Compare(rhs_string)) << "LHS=" << lhs_string << "; RHS=" << rhs_string;
    EXPECT_EQ(-expected, test_case.rhs_cord.Compare(test_case.lhs_cord)) << "LHS=" << rhs_string << "; RHS=" << lhs_string;
    EXPECT_EQ(-expected, test_case.rhs_cord.Compare(lhs_string)) << "LHS=" << rhs_string << "; RHS=" << lhs_string;
}
...
TEST_P(CordTest, Compare)
{
    ...
    std::vector<CordCompareTestCase> test_cases = { {
        ...
        { concat, "aaaaaaaaaaaaaaaaBBBBBBBBBBBBBBBBccccccccccccccccDDDDDDDDDDDDDDDD", use_crc },
        { concat, "aaaaaaaaaaaaaaaaBBBBBBBBBBBBBBBBcccccccccccccccxDDDDDDDDDDDDDDDD", use_crc },
        { concat, "aaaaaaaaaaaaaaaaBBBBBBBBBBBBBBBBacccccccccccccccDDDDDDDDDDDDDDDD", use_crc },
        { concat, "aaaaaaaaaaaaaaaaBBBBBBBBBBBBBBBBccccccccccccccccDDDDDDDDDDDDDDD", use_crc },
        { concat, "aaaaaaaaaaaaaaaaBBBBBBBBBBBBBBBBccccccccccccccccDDDDDDDDDDDDDDDDe", use_crc },
...
static void TestCompare(const absl::Cord& c, const absl::Cord& d, RandomEngine* rng)
{
...
TEST_P(CordTest, CompareComparisonIsUnsigned)
{
    ...
    TestCompare(absl::Cord(std::string(GetUniformRandomUpTo(&rng, 100), x)), absl::Cord(std::string(GetUniformRandomUpTo(&rng, 100), x ^ 0x80)), &rng);
}
...
TEST_P(CordTest, CompareRandomComparisons)
{
    ...
    absl::Cord a[] = { MakeExternalCord(n), absl::Cord("ant"), absl::Cord("elephant"), absl::Cord("giraffe"),
        absl::Cord(std::string(GetUniformRandomUpTo(&rng, 100), GetUniformRandomUpTo(&rng, 100))), absl::Cord(""), absl::Cord("x"), absl::Cord("A"),
    ...
    for (int i = 0; i < kIters; i++) {
        ...
        MaybeHarden(c);
        MaybeHarden(d);
        TestCompare(coin_flip(rng) ? c : absl::Cord(std::string(c)), coin_flip(rng) ? d : absl::Cord(std::string(d)), &rng);
    }
}

template <typename T1, typename T2> void CompareOperators()
{
    const T1 a("a");
    const T2 b("b");
...
TEST_P(CordTest, ConstructFromExternalCompareContents)
{
    ...
    for (int length = 1; length <= 2048; length *= 2) {
        ...
        auto cord = absl::MakeCordFromExternal(*external, [external](absl::string_view sv) {
...
TEST_P(CordTest, ConstructFromExternalFunctionPointerReleaser)
{
    ...
    static bool invoked;
    auto* releaser = static_cast<void (*)(absl::string_view)>([](absl::string_view sv) {
        EXPECT_EQ(data, sv);
        invoked = true;
    });
    invoked = false;
...
{
    bool invoked = false;
    (void)MaybeHardened(absl::MakeCordFromExternal("dummy", [&invoked]() { invoked = true; }));
    EXPECT_TRUE(invoked);
}
...
{
    bool invoked = false;
    (void)MaybeHardened(absl::MakeCordFromExternal("dummy", [&invoked](absl::string_view) { invoked = true; }));
    EXPECT_TRUE(invoked);
}
...
TEST_P(CordTest, ExternalMemoryBasicUsage)
{
    ...
    for (const char* str : strings) {
        ...
        MaybeHarden(dst);
        dst.Append("(suffix)");
        EXPECT_EQ((std::string("(prefix)") + str + std::string("(suffix)")), std::string(dst));
    }
}
...
TEST_P(CordTest, ExternalMemoryRemovePrefixSuffix)
{
    ...
    for (int offset = 0; offset <= s.size(); offset++) {
        for (int length = 0; length <= s.size() - offset; length++) {
            ...
            EXPECT_EQ(s.substr(offset, length), std::string(result)) << offset << " " << length;
        }
    }
}
...
constexpr auto kTotalMorePrecise = absl::CordMemoryAccounting::kTotalMorePrecise;
...
TEST(CordTest, CordMemoryUsageExternalMemory)
{
    ...
    const size_t expected = sizeof(absl::Cord) + 1000 + sizeof(CordRepExternal) + sizeof(intptr_t);
...
TEST(CordTest, CordMemoryUsageFlat)
{
    ...
    const size_t flat_size = absl::CordTestPeer::Tree(cord)->flat()->AllocatedSize();
    EXPECT_EQ(cord.EstimatedMemoryUsage(), sizeof(absl::Cord) + flat_size);
    EXPECT_EQ(cord.EstimatedMemoryUsage(kFairShare), sizeof(absl::Cord) + flat_size);
    EXPECT_EQ(cord.EstimatedMemoryUsage(kTotalMorePrecise), sizeof(absl::Cord) + flat_size);
}

TEST(CordTest, CordMemoryUsageSubStringSharedFlat)
{
    absl::Cord flat = MakeCord(2000, 'a');
    const size_t flat_size = absl::CordTestPeer::Tree(flat)->flat()->AllocatedSize();
    absl::Cord cord = flat.Subcord(500, 1000);
    EXPECT_EQ(cord.EstimatedMemoryUsage(), sizeof(absl::Cord) + sizeof(CordRepSubstring) + flat_size);
    EXPECT_EQ(cord.EstimatedMemoryUsage(kTotalMorePrecise), sizeof(absl::Cord) + sizeof(CordRepSubstring) + flat_size);
    EXPECT_EQ(cord.EstimatedMemoryUsage(kFairShare), sizeof(absl::Cord) + sizeof(CordRepSubstring) + flat_size / 2);
}
...
TEST(CordTest, CordMemoryUsageFlatShared)
{
    ...
    EXPECT_EQ(cord.EstimatedMemoryUsage(), sizeof(absl::Cord) + flat_size);
    EXPECT_EQ(cord.EstimatedMemoryUsage(kTotalMorePrecise), sizeof(absl::Cord) + flat_size);
    EXPECT_EQ(cord.EstimatedMemoryUsage(kFairShare), sizeof(absl::Cord) + flat_size / 2);
}
...
TEST(CordTest, CordMemoryUsageFlatHardenedAndShared)
{
    ...
    EXPECT_EQ(cord.EstimatedMemoryUsage(), sizeof(absl::Cord) + sizeof(CordRepCrc) + flat_size);
    EXPECT_EQ(cord.EstimatedMemoryUsage(kFairShare), sizeof(absl::Cord) + sizeof(CordRepCrc) + flat_size / 2);

    absl::Cord cord2(cord);
    EXPECT_EQ(cord2.EstimatedMemoryUsage(), sizeof(absl::Cord) + sizeof(CordRepCrc) + flat_size);
    EXPECT_EQ(cord2.EstimatedMemoryUsage(kTotalMorePrecise), sizeof(absl::Cord) + sizeof(CordRepCrc) + flat_size);
    EXPECT_EQ(cord2.EstimatedMemoryUsage(kFairShare), sizeof(absl::Cord) + (sizeof(CordRepCrc) + flat_size / 2) / 2);
}
...
TEST(CordTest, CordMemoryUsageBTree)
{
    ...
    absl::Cord flats1[4] = { MakeCord(1000, 'a'), MakeCord(1100, 'a'), MakeCord(1200, 'a'), MakeCord(1300, 'a') };
    ...
    EXPECT_EQ(cord1.EstimatedMemoryUsage(kTotalMorePrecise), sizeof(absl::Cord) + rep1_size);
    EXPECT_EQ(cord1.EstimatedMemoryUsage(kFairShare), sizeof(absl::Cord) + rep1_shared_size);

    absl::Cord cord2;
    size_t flats2_size = 0;
    absl::Cord flats2[4] = { MakeCord(600, 'a'), MakeCord(700, 'a'), MakeCord(800, 'a'), MakeCord(900, 'a') };
    ...
    EXPECT_EQ(cord2.EstimatedMemoryUsage(kTotalMorePrecise), sizeof(absl::Cord) + rep2_size);
    EXPECT_EQ(cord2.EstimatedMemoryUsage(kFairShare), sizeof(absl::Cord) + rep2_size);

    absl::Cord cord(cord1);
    cord.Append(std::move(cord2));

    EXPECT_EQ(cord.EstimatedMemoryUsage(), sizeof(absl::Cord) + sizeof(CordRepBtree) + rep1_size + rep2_size);
    EXPECT_EQ(cord.EstimatedMemoryUsage(kTotalMorePrecise), sizeof(absl::Cord) + sizeof(CordRepBtree) + rep1_size + rep2_size);
    EXPECT_EQ(cord.EstimatedMemoryUsage(kFairShare), sizeof(absl::Cord) + sizeof(CordRepBtree) + rep1_shared_size / 2 + rep2_size);
}
...
TEST(CordTest, TestHashFragmentation)
{
    ...
    EXPECT_TRUE(absl::VerifyTypeImplementsAbslHashCorrectly({
        ...
        absl::MakeFragmentedCord({ std::string(2000, 'c'), std::string(2000, 'c') }),
...
TEST_P(CordTest, CordMemoryUsageTotalMorePreciseMode)
{
    ...
    const size_t flat_internal_usage = flat.EstimatedMemoryUsage() - sizeof(absl::Cord);

    // `fragmented` holds a Cord and a CordRepBtree. That tree points to two
    // copies of flat's internals, which we expect to dedup:
    EXPECT_EQ(fragmented.EstimatedMemoryUsage(kTotalMorePrecise), sizeof(absl::Cord) + sizeof(CordRepBtree) + flat_internal_usage);

    // This is a case where kTotal produces an overestimate:
    EXPECT_EQ(fragmented.EstimatedMemoryUsage(), sizeof(absl::Cord) + sizeof(CordRepBtree) + 2 * flat_internal_usage);
}
...
TEST_P(CordTest, CordMemoryUsageTotalMorePreciseModeWithSubstring)
{
    ...
    EXPECT_EQ(
        fragmented.EstimatedMemoryUsage(kTotalMorePrecise), sizeof(absl::Cord) + sizeof(CordRepBtree) + 2 * sizeof(CordRepSubstring) + flat_internal_usage);

    // This is a case where kTotal produces an overestimate:
    EXPECT_EQ(fragmented.EstimatedMemoryUsage(), sizeof(absl::Cord) + sizeof(CordRepBtree) + 2 * sizeof(CordRepSubstring) + 2 * flat_internal_usage);
}
} // namespace
...
static absl::Cord MakeHuge(absl::string_view prefix)
{
    ...
    if (sizeof(size_t) > 4) {
        ...
        cord.Append(absl::MakeCordFromExternal(absl::string_view(prefix.data(), size), [](absl::string_view s) { DoNothing(s, nullptr); }));
    } else {
        ...
        const size_t s2 = 600;
        cord.Append(absl::MakeCordFromExternal(absl::string_view(prefix.data(), s1), [](absl::string_view s) { DoNothing(s, nullptr); }));
        cord.Append(absl::MakeCordFromExternal(absl::string_view("", s2), [](absl::string_view s) { DoNothing(s, nullptr); }));
    }
    return cord;
}
...
TEST_P(CordTest, HugeCord)
{
    ...
    const size_t acceptable_delta = 100 + (UseCrc() ? sizeof(absl::cord_internal::CordRepCrc) : 0);
...
{
    absl::Cord fragmented = absl::MakeFragmentedCord({ "A ", "fragmented ", "Cord" });
...
TEST_P(CordTest, CordChunkIteratorTraits)
{
    ...
    static_assert(std::is_same<std::iterator_traits<absl::Cord::ChunkIterator>::iterator_category, std::input_iterator_tag>::value, "");
    static_assert(std::is_same<std::iterator_traits<absl::Cord::ChunkIterator>::value_type, absl::string_view>::value, "");
    static_assert(std::is_same<std::iterator_traits<absl::Cord::ChunkIterator>::difference_type, ptrdiff_t>::value, "");
    static_assert(std::is_same<std::iterator_traits<absl::Cord::ChunkIterator>::pointer, const absl::string_view*>::value, "");
    static_assert(std::is_same<std::iterator_traits<absl::Cord::ChunkIterator>::reference, absl::string_view>::value, "");
}
...
static void VerifyChunkIterator(const absl::Cord& cord, size_t expected_chunks)
{
    ...
    while (pre_iter != cord.chunk_end() && post_iter != cord.chunk_end()) {
        ...
        for (absl::Cord::ChunkIterator it = range.begin(); it != range.end(); ++it) {
...
TEST_P(CordTest, CordChunkIteratorOperations)
{
    ...
    VerifyChunkIterator(MaybeHardened(absl::MakeFragmentedCord({ "a ", "small ", "fragmented ", "cord ", "for ", "testing ", "chunk ", "iterations." })), 8);
...
TEST_P(CordTest, AdvanceAndReadOnDataEdge)
{
    ...
    for (bool as_flat : { true, false }) {
        ...
        absl::Cord cord = as_flat ? absl::Cord(data) : absl::MakeCordFromExternal(data, [](absl::string_view) {});
...
TEST_P(CordTest, CharIteratorTraits)
{
    ...
    static_assert(std::is_same<std::iterator_traits<absl::Cord::CharIterator>::iterator_category, std::input_iterator_tag>::value, "");
    static_assert(std::is_same<std::iterator_traits<absl::Cord::CharIterator>::value_type, char>::value, "");
    static_assert(std::is_same<std::iterator_traits<absl::Cord::CharIterator>::difference_type, ptrdiff_t>::value, "");
    static_assert(std::is_same<std::iterator_traits<absl::Cord::CharIterator>::pointer, const char*>::value, "");
    static_assert(std::is_same<std::iterator_traits<absl::Cord::CharIterator>::reference, const char&>::value, "");
}
...
static void VerifyCharIterator(const absl::Cord& cord)
{
    ...
    while (pre_iter != cord.char_end() && post_iter != cord.char_end()) {
        ...
        advance_iter = pre_iter;
        EXPECT_EQ(absl::Cord::AdvanceAndRead(&advance_iter, cord.size() - i), cord.Subcord(i, cord.size() - i));
        EXPECT_EQ(range.end(), advance_iter);
...
TEST_P(CordTest, CharIteratorOperations)
{
    ...
    VerifyCharIterator(MaybeHardened(absl::MakeFragmentedCord({ "a ", "small ", "fragmented ", "cord ", "for ", "testing ", "character ", "iteration." })));
...
TEST_P(CordTest, CharIteratorAdvanceAndRead)
{
    ...
    for (size_t chunk_size : { kChunkSize1, kChunkSize2, kChunkSize3, kChunkSize4 }) {
        absl::Cord::CharIterator it = cord.char_begin();
        size_t offset = 0;
...
{
    absl::Cord c = absl::MakeFragmentedCord({ "A ", "small ", "fragmented ", "Cord", "." });
    MaybeHarden(c);
...
TEST_P(CordTest, ForEachChunk)
{
    for (int num_elements : { 1, 10, 200 }) {
        ...
        absl::CordTestPeer::ForEachChunk(c, [&iterated_chunks](absl::string_view sv) { iterated_chunks.emplace_back(sv); });
...
TEST_P(CordTest, SmallBufferAssignFromOwnData)
{
    ...
    for (size_t pos = 0; pos < contents.size(); ++pos) {
        for (size_t count = contents.size() - pos; count > 0; --count) {
            ...
            EXPECT_EQ(c, contents.substr(pos, count)) << "pos = " << pos << "; count = " << count;
        }
    }
}
...
template <typename Str> void TestAfterExit(Str)
{
...
struct LongView {
    constexpr absl::string_view operator()() const
    {
        return absl::string_view("String that does not fit SSO.", SimpleStrlen("String that does not fit SSO."));
    }
};
...
public:
    constexpr PopulatedCordFactory(absl::string_view name, absl::Cord (*generator)())
        : name_(name)
...
class CordMutator {
public:
    constexpr CordMutator(absl::string_view name, void (*mutate)(absl::Cord&), void (*undo)(absl::Cord&) = nullptr)
        : name_(name)
        , mutate_(mutate)
        , undo_(undo)
    {
    ...
    void Mutate(absl::Cord& cord) const
    {
        mutate_(cord);
    }
    bool CanUndo() const
    {
        return undo_ != nullptr;
    }
    void Undo(absl::Cord& cord) const
    {
        undo_(cord);
    }
...
TEST_P(CordTest, ExpectedChecksum)
{
    for (const PopulatedCordFactory& factory : cord_factories) {
        ...
        for (bool shared : { false, true }) {
            ...
            auto make_instance = [=] { return shared ? shared_cord_source : factory.Generate(); };
...
TEST(CordThreeWayComparisonTest, CompareCords)
{
    ...
    EXPECT_EQ(absl::Cord("aaaa") <=> absl::Cord("aaab"), std::strong_ordering::less);
    EXPECT_EQ(absl::Cord("baaa") <=> absl::Cord("a"), std::strong_ordering::greater);
#endif
}
...
TEST(CordThreeWayComparisonTest, CompareCordsAndStringViews)
{
    ...
    GTEST_SKIP() << "C++20 three-way <=> comparison not supported";
#else
    EXPECT_EQ(absl::string_view("a") <=> absl::Cord("a"), std::strong_ordering::equal);
    EXPECT_EQ(absl::Cord("a") <=> absl::string_view("b"), std::strong_ordering::less);
    EXPECT_EQ(absl::string_view("b") <=> absl::Cord("a"), std::strong_ordering::greater);
#endif
}
