...
TEST(AsciiIsFoo, All)
{
    ...
    for (int i = 0; i < 256; i++) {
        const auto c = static_cast<unsigned char>(i);
        if (absl::ascii_isprint(c) && !absl::ascii_isspace(c) && !absl::ascii_isalnum(c)) {
    ...
    for (int i = 0; i < 256; i++) {
        const auto c = static_cast<unsigned char>(i);
        if (absl::ascii_isdigit(c) || (i >= 'A' && i <= 'F') || (i >= 'a' && i <= 'f')) {
...
TEST(AsciiStrTo, Lower)
{
    ...
    std::transform(mutable_buf, mutable_buf + strlen(mutable_buf), mutable_buf, absl::ascii_tolower);
...
TEST(AsciiStrTo, Upper)
{
    ...
    char mutable_buf[] = "Mutable";
    std::transform(mutable_buf, mutable_buf + strlen(mutable_buf), mutable_buf, absl::ascii_toupper);
    EXPECT_STREQ("MUTABLE", mutable_buf);
}

TEST(StripLeadingAsciiWhitespace, FromStringView)
{
    EXPECT_EQ(absl::string_view {}, absl::StripLeadingAsciiWhitespace(absl::string_view {}));
...
{
    EXPECT_EQ(absl::string_view {}, absl::StripTrailingAsciiWhitespace(absl::string_view {}));
...
{
    EXPECT_EQ(absl::string_view {}, absl::StripAsciiWhitespace(absl::string_view {}));
    EXPECT_EQ("foo", absl::StripAsciiWhitespace({ "foo" }));
    EXPECT_EQ("foo", absl::StripAsciiWhitespace({ "\t  \n\f\r\n\vfoo\t  \n\f\r\n\v" }));
    EXPECT_EQ("foo foo", absl::StripAsciiWhitespace({ "\t  \n\f\r\n\vfoo foo\t  \n\f\r\n\v" }));
    EXPECT_EQ(absl::string_view {}, absl::StripAsciiWhitespace({ "\t  \n\f\r\v\n\t  \n\f\r\v\n" }));
}
...
{
    const char* inputs[] = { "No extra space", "  Leading whitespace", "Trailing whitespace  ", "  Leading and trailing  ", " Whitespace \t  in\v   middle  ",
        "'Eeeeep!  \n Newlines!\n", "nospaces", "", "\n\t a\t\n\nb \t\n" };
