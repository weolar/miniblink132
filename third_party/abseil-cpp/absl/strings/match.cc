...
absl::string_view FindLongestCommonPrefix(absl::string_view a, absl::string_view b)
{
    ...
    if (ABSL_PREDICT_FALSE(limit < 8)) {
        while (ABSL_PREDICT_TRUE(count + 2 <= limit)) {
            uint16_t xor_bytes = absl::little_endian::Load16(pa + count) ^ absl::little_endian::Load16(pb + count);
            if (ABSL_PREDICT_FALSE(xor_bytes != 0)) {
                if (ABSL_PREDICT_TRUE((xor_bytes & 0xff) == 0))
                    ++count;
        ...
        if (ABSL_PREDICT_TRUE(count != limit)) {
            if (ABSL_PREDICT_TRUE(pa[count] == pb[count]))
                ++count;
        }
        return absl::string_view(pa, count);
    }
    ...
    do {
        uint64_t xor_bytes = absl::little_endian::Load64(pa + count) ^ absl::little_endian::Load64(pb + count);
        if (ABSL_PREDICT_FALSE(xor_bytes != 0)) {
