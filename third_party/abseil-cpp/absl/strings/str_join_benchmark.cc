...
BENCHMARK(BM_JoinStreamable)->ArgPair(0, 0)->ArgPair(16, 1)->ArgPair(256, 1)->ArgPair(16, 16)->ArgPair(256, 16)->ArgPair(16, 256)->ArgPair(256, 256);

void BM_JoinTuple(benchmark::State& state)
{
    for (auto _ : state) {
        std::string s = absl::StrJoin(std::make_tuple(123456789, 987654321, 24680, 13579), "/");
        benchmark::DoNotOptimize(s);
    }
}
