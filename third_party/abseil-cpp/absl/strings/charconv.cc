...
template <> struct FloatTraits<double> {
    ...
    static double Make(mantissa_t mantissa, int exponent, bool sign)
    {
        ...
        if (mantissa > kMantissaMask) {
            ...
            // representation.
            dbl += static_cast<uint64_t>(exponent + 1023 + kTargetMantissaBits - 1) << 52;
...
template <> struct FloatTraits<float> {
    ...
    static float Make(mantissa_t mantissa, int exponent, bool sign)
    {
        ...
        if (mantissa > kMantissaMask) {
            ...
            // representation.
            flt += static_cast<uint32_t>(exponent + 127 + kTargetMantissaBits - 1) << 23;
            mantissa &= kMantissaMask;
        } else {
            // subnormal value
            assert(exponent == kMinNormalExponent);
...
template <typename FloatType> bool HandleEdgeCase(const strings_internal::ParsedFloat& input, bool negative, absl::Nonnull<FloatType*> value)
{
...
#if (defined(__GNUC__) && !defined(__clang__)) || (defined(__clang__) && __clang_major__ < 7)
...
void EncodeResult(const CalculatedFloat& calculated, bool negative, absl::Nonnull<absl::from_chars_result*> result, absl::Nonnull<FloatType*> value)
{
    ...
    *value = FloatTraits<FloatType>::Make(static_cast<typename FloatTraits<FloatType>::mantissa_t>(calculated.mantissa), calculated.exponent, negative);
}
...
uint64_t ShiftRightAndRound(uint128 value, int shift, bool input_exact, absl::Nonnull<bool*> output_exact)
{
    if (shift <= 0) {
...
template <typename FloatType> CalculatedFloat CalculatedFloatFromRawValues(uint64_t mantissa, int exponent)
{
...
template <typename FloatType>
bool EiselLemire(const strings_internal::ParsedFloat& input, bool negative, absl::Nonnull<FloatType*> value, absl::Nonnull<std::errc*> ec)
{
    ...
    static_assert(FloatTraits<FloatType>::kEiselLemireMinInclusiveExp10 >= kPower10TableMinInclusive,
        "(exp10-kPower10TableMinInclusive) in kPower10MantissaHighTable bounds");
    static_assert(FloatTraits<FloatType>::kEiselLemireMaxExclusiveExp10 <= kPower10TableMaxExclusive,
        "(exp10-kPower10TableMinInclusive) in kPower10MantissaHighTable bounds");
    ...
    if (((Uint128High64(x) & high64_mask) == high64_mask) && (man > (std::numeric_limits<uint64_t>::max() - Uint128Low64(x)))) {
        ...
        if (((Uint128High64(x) & high64_mask) == high64_mask) && ((Uint128Low64(x) + 1) == 0)
    ...
    if (FloatTraits<FloatType>::kTargetBits == 64) {
        *value = FloatTraits<FloatType>::Make((ret_man & 0x000FFFFFFFFFFFFFu) | 0x0010000000000000u, static_cast<int>(ret_exp2) - 1023 - 52, negative);
        return true;
    } else if (FloatTraits<FloatType>::kTargetBits == 32) {
        *value = FloatTraits<FloatType>::Make((static_cast<uint32_t>(ret_man) & 0x007FFFFFu) | 0x00800000u, static_cast<int>(ret_exp2) - 127 - 23, negative);
        return true;
    }
#else
    if (FloatTraits<FloatType>::kTargetBits == 64) {
...
template <typename FloatType>
from_chars_result FromCharsImpl(absl::Nonnull<const char*> first, absl::Nonnull<const char*> last, FloatType& value, chars_format fmt_flags)
{
...
from_chars_result from_chars(absl::Nonnull<const char*> first, absl::Nonnull<const char*> last, double& value, chars_format fmt)
{
    return FromCharsImpl(first, last, value, fmt);
}

from_chars_result from_chars(absl::Nonnull<const char*> first, absl::Nonnull<const char*> last, float& value, chars_format fmt)
{
    return FromCharsImpl(first, last, value, fmt);
}
