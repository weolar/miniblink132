...
template <typename Function> void AsciiBenchmark(benchmark::State& state, Function f)
{
...
template <StdAsciiFunction f> void BM_Ascii(benchmark::State& state)
{
    AsciiBenchmark(state, f);
}

using AbslAsciiIsFunction = bool (*)(unsigned char);
template <AbslAsciiIsFunction f> void BM_Ascii(benchmark::State& state)
{
    AsciiBenchmark(state, f);
}

using AbslAsciiToFunction = char (*)(unsigned char);
template <AbslAsciiToFunction f> void BM_Ascii(benchmark::State& state)
{
    AsciiBenchmark(state, f);
}

inline char Noop(unsigned char b)
{
...
BENCHMARK(BM_StrToLower)->DenseRange(0, 32)->RangeMultiplier(2)->Range(64, 1 << 26);
...
BENCHMARK(BM_StrToUpper)->DenseRange(0, 32)->RangeMultiplier(2)->Range(64, 1 << 26);
...
BENCHMARK(BM_StrToUpperFromRvalref)->DenseRange(0, 32)->RangeMultiplier(2)->Range(64, 1 << 26);
...
BENCHMARK(BM_StrToLowerFromRvalref)->DenseRange(0, 32)->RangeMultiplier(2)->Range(64, 1 << 26);
