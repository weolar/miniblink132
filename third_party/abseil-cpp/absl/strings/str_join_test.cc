...
TEST(StrJoin, CustomFormatter)
{
    ...
    {
        std::string joined = absl::StrJoin(v, "", [](std::string* out, const std::string& in) { absl::StrAppend(out, "(", in, ")"); });
...
struct TestingParenFormatter {
    template <typename T> void operator()(std::string* s, const T& t)
    {
        absl::StrAppend(s, "(", t, ")");
    }
};
...
TEST(PairFormatter, FormatterAPI)
{
...

    {
        ...
        auto f = absl::PairFormatter(TestingParenFormatter(), "=", TestingParenFormatter());
        std::string s;
        f(&s, std::make_pair("a", "b"));
        f(&s, std::make_pair(1, 2));
        EXPECT_EQ("(a)=(b)(1)=(2)", s);
...
{
    {
        ...
        const absl::strings_internal::DereferenceFormatterImpl<absl::strings_internal::AlphaNumFormatterImpl> f;
        int x = 1, y = 2, z = 3;
        std::string s;
        f(&s, &x);
        f(&s, &y);
...

    {
        ...
        absl::strings_internal::DereferenceFormatterImpl<absl::strings_internal::DefaultFormatter<std::string>::Type> f;
    ...
    {
        absl::strings_internal::DereferenceFormatterImpl<absl::strings_internal::AlphaNumFormatterImpl> f;
...
TEST(StrJoin, PublicAPIOverloads)
{
    ...
    // Iterators + formatter
    EXPECT_EQ("a-b-c", absl::StrJoin(v.begin(), v.end(), "-", absl::AlphaNumFormatter()));
    // Range + formatter
...
TEST(StrJoin, StringViewInitializerList)
{
    ...
    class NoCopy {
    public:
        explicit NoCopy(absl::string_view view)
            : view_(view)
        {
        }
    ...
    };
    {
        ...
        EXPECT_EQ("a-b-c", absl::StrJoin({ NoCopy("a"), NoCopy("b"), NoCopy("c") }, "-"));
    }
    {
        // Tests initializer_list of string_views preferred over initializer_list<T>
        // for T that is implicitly convertible to string_view
        TestingParenFormatter f;
        EXPECT_EQ("(a)-(b)-(c)", absl::StrJoin({ NoCopy("a"), NoCopy("b"), NoCopy("c") }, "-", f));
    }
}
...
TEST(StrJoin, Tuple)
{
    ...
    // Faster! Faster!!
    EXPECT_EQ("10-hello-3.14", absl::StrJoin(std::make_tuple(x, std::cref(y), z), "-"));
    ...
    EXPECT_EQ("0x0000000a-hell-3.", absl::StrJoin(std::make_tuple(x, y, z), "-", TestFormatter()));
    EXPECT_EQ("0x0000000a-hell-3.", absl::StrJoin(std::make_tuple(x, std::cref(y), z), "-", TestFormatter()));
    EXPECT_EQ("0x0000000a-hell-3.", absl::StrJoin(std::make_tuple(&x, &y, &z), "-", absl::DereferenceFormatter(TestFormatter())));
    EXPECT_EQ("0x0000000a-hell-3.",
        absl::StrJoin(std::make_tuple(absl::make_unique<int>(x), absl::make_unique<std::string>(y), absl::make_unique<double>(z)), "-",
            absl::DereferenceFormatter(TestFormatter())));
    EXPECT_EQ("0x0000000a-hell-3.", absl::StrJoin(std::make_tuple(absl::make_unique<int>(x), &y, &z), "-", absl::DereferenceFormatter(TestFormatter())));
}
...
class TestValue {
public:
    TestValue(const char* data, size_t size)
        : data_(data)
        , size_(size)
    {
    }
...
// `string_view`.
template <typename ValueT> class TestIterator {
public:
...
template <typename ValueT> class TestIteratorRange {
public:
    ...
    const TestIterator<ValueT>& begin() const
    {
        return begin_;
    }
    const TestIterator<ValueT>& end() const
    {
        return end_;
    }
...
TEST(StrJoin, TestIteratorRequirementsNoFormatter)
{
    ...
    EXPECT_EQ("a-b-c", absl::StrJoin(TestIteratorRange<absl::string_view>(a), "-"));
}

TEST(StrJoin, TestIteratorRequirementsCustomFormatter)
{
    const std::vector<absl::string_view> a = { "a", "b", "c" };
    EXPECT_EQ("a-b-c", absl::StrJoin(TestIteratorRange<TestValue>(a), "-", [](std::string* out, const TestValue& value) {
        absl::StrAppend(out, absl::string_view(value.data(), value.size()));
    }));
}
