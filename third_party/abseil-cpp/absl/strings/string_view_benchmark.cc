...
void BM_StringViewFromString(benchmark::State& state)
{
    ...
    struct SV {
        SV() = default;
        explicit SV(const std::string& s)
            : sv(s)
        {
        }
        absl::string_view sv;
...
bool NonInlinedEq(absl::string_view a, absl::string_view b)
{
    return a == b;
}
...
ABSL_ATTRIBUTE_NOINLINE
void DoEqualityComparisons(benchmark::State& state, absl::string_view a, absl::string_view b)
{
    for (auto _ : state) {
...
BENCHMARK(BM_EqualSame)->DenseRange(0, 10)->Arg(20)->Arg(40)->Arg(70)->Arg(110)->Range(160, 4096);
...
ABSL_ATTRIBUTE_NOINLINE
void DoConstantSizeInlinedEqualityComparisons(benchmark::State& state, absl::string_view a)
{
    for (auto _ : state) {
...
ABSL_ATTRIBUTE_NOINLINE
void DoConstantSizeNonInlinedEqualityComparisons(benchmark::State& state, absl::string_view a)
{
    for (auto _ : state) {
...
template <typename Map, int WordsPerKey> void StringViewMapBenchmark(benchmark::State& state)
{
    ...
    const char* words[] = { "the", "quick", "brown", "fox", "jumped", "over", "the", "lazy", "dog", "and", "found", "a", "large", "mushroom", "and", "a",
        "couple", "crickets", "eating", "pie" };
    ...
    std::vector<int> hot_indices(all_indices.begin(), all_indices.begin() + num_hot);
    ...
    ABSL_RAW_CHECK(num_cold * kNumLookupsOfColdKeys + num_hot * kNumLookupsOfHotKeys == indices.size(), "");
