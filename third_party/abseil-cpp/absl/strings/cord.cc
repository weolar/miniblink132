...
static void DumpNode(absl::Nonnull<CordRep*> nonnull_rep, bool include_data, absl::Nonnull<std::ostream*> os, int indent = 0);
static bool VerifyNode(absl::Nonnull<CordRep*> root, absl::Nonnull<CordRep*> start_node);

static inline absl::Nullable<CordRep*> VerifyTree(absl::Nullable<CordRep*> node)
{
...
static absl::Nonnull<CordRepFlat*> CreateFlat(absl::Nonnull<const char*> data, size_t length, size_t alloc_hint)
{
...
static absl::Nonnull<CordRep*> NewBtree(absl::Nonnull<const char*> data, size_t length, size_t alloc_hint)
{
...
// The returned node has a refcount of 1.
static absl::Nullable<CordRep*> NewTree(absl::Nullable<const char*> data, size_t length, size_t alloc_hint)
{
    if (length == 0)
        return nullptr;
...
static absl::Nonnull<CordRep*> CordRepFromString(std::string&& src)
{
    ...
    auto* rep = static_cast<::absl::cord_internal::CordRepExternalImpl<StringReleaser>*>(
        absl::cord_internal::NewExternalRep(original_data, StringReleaser { std::move(src) }));
...
inline void Cord::InlineRep::set_data(absl::Nonnull<const char*> data, size_t n)
{
...
{
    cord_internal::SmallMemmove(data_.as_chars(), data_.as_chars() + n, inline_size() - n);
    reduce_size(n);
}
...
{
    return rep->IsBtree() ? rep->btree() : CordRepBtree::Create(cord_internal::RemoveCrcNode(rep));
}

void Cord::InlineRep::AppendTreeToInlined(absl::Nonnull<CordRep*> tree, MethodIdentifier method)
{
    assert(!is_tree());
    if (!data_.is_empty()) {
...
void Cord::InlineRep::AppendTreeToTree(absl::Nonnull<CordRep*> tree, MethodIdentifier method)
{
    assert(is_tree());
...
void Cord::InlineRep::AppendTree(absl::Nonnull<CordRep*> tree, MethodIdentifier method)
{
    assert(tree != nullptr);
...
void Cord::InlineRep::PrependTreeToInlined(absl::Nonnull<CordRep*> tree, MethodIdentifier method)
{
    assert(!is_tree());
    if (!data_.is_empty()) {
        CordRepFlat* flat = MakeFlatWithExtraCapacity(0);
...
void Cord::InlineRep::PrependTreeToTree(absl::Nonnull<CordRep*> tree, MethodIdentifier method)
{
    assert(is_tree());
    const CordzUpdateScope scope(data_.cordz_info(), method);
...
void Cord::InlineRep::PrependTree(absl::Nonnull<CordRep*> tree, MethodIdentifier method)
{
    assert(tree != nullptr);
    assert(tree->length != 0);
    assert(!tree->IsCrc());
...
static inline bool PrepareAppendRegion(
    absl::Nonnull<CordRep*> root, absl::Nonnull<absl::Nullable<char*>*> region, absl::Nonnull<size_t*> size, size_t max_length)
{
...
void Cord::InlineRep::AppendArray(absl::string_view src, MethodIdentifier method)
{
    if (src.empty())
...
template <typename C> inline void Cord::AppendImpl(C&& src)
{
...
static CordRep::ExtractResult ExtractAppendBuffer(absl::Nonnull<CordRep*> rep, size_t min_capacity)
{
    switch (rep->tag) {
...
template <typename T, Cord::EnableIfString<T>> inline void Cord::Prepend(T&& src)
{
    if (src.size() <= kMaxBytesToCopy) {
...
{
    ABSL_INTERNAL_CHECK(n <= size(), absl::StrCat("Requested prefix size ", n, " exceeds Cord's size ", size()));
    contents_.MaybeRemoveEmptyCrcNode();
    CordRep* tree = contents_.tree();
    if (tree == nullptr) {
...
{
    ABSL_INTERNAL_CHECK(n <= size(), absl::StrCat("Requested suffix size ", n, " exceeds Cord's size ", size()));
    contents_.MaybeRemoveEmptyCrcNode();
    CordRep* tree = contents_.tree();
    if (tree == nullptr) {
...
Cord Cord::Subcord(size_t pos, size_t new_size) const
{
    ...
    sub_cord.contents_.EmplaceTree(tree, contents_.data_, CordzUpdateTracker::kSubCord);
    return sub_cord;
}
...
int CompareChunks(absl::Nonnull<absl::string_view*> lhs, absl::Nonnull<absl::string_view*> rhs, absl::Nonnull<size_t*> size_to_compare)
{
...
template <typename ResultType> ResultType ComputeCompareResult(int memcmp_res)
{
    return ClampResult(memcmp_res);
}
template <> bool ComputeCompareResult<bool>(int memcmp_res)
{
    return memcmp_res == 0;
}
...
void Cord::SetExpectedChecksum(uint32_t crc)
{
    ...
    state.mutable_rep()->prefix_crc.push_back(crc_internal::CrcCordState::PrefixCrc(size(), absl::crc32c_t { crc }));
    SetCrcCordState(std::move(state));
}

absl::Nullable<const crc_internal::CrcCordState*> Cord::MaybeGetCrcCordState() const
{
...
absl::optional<uint32_t> Cord::ExpectedChecksum() const
{
    ...
    return static_cast<uint32_t>(contents_.tree()->crc()->crc_cord_state.Checksum());
}

inline int Cord::CompareSlowPath(absl::string_view rhs, size_t compared_size, size_t size_to_compare) const
{
    auto advance = [](absl::Nonnull<Cord::ChunkIterator*> it, absl::Nonnull<absl::string_view*> chunk) {
        if (!chunk->empty())
            return true;
        ++*it;
    ...
    absl::string_view lhs_chunk = (lhs_it.bytes_remaining_ != 0) ? *lhs_it : absl::string_view();
...
inline int Cord::CompareSlowPath(const Cord& rhs, size_t compared_size, size_t size_to_compare) const
{
    auto advance = [](absl::Nonnull<Cord::ChunkIterator*> it, absl::Nonnull<absl::string_view*> chunk) {
        if (!chunk->empty())
            return true;
    ...
    absl::string_view lhs_chunk = (lhs_it.bytes_remaining_ != 0) ? *lhs_it : absl::string_view();
    absl::string_view rhs_chunk = (rhs_it.bytes_remaining_ != 0) ? *rhs_it : absl::string_view();
    assert(compared_size <= lhs_chunk.size());
...
template <typename ResultType, typename RHS> ResultType GenericCompare(const Cord& lhs, const RHS& rhs, size_t size_to_compare)
{
    ...
    return ComputeCompareResult<ResultType>(lhs.CompareSlowPath(rhs, compared_size, size_to_compare));
}
...
template <typename RHS> inline int SharedCompareImpl(const Cord& lhs, const RHS& rhs)
{
...
void AppendCordToString(const Cord& src, absl::Nonnull<std::string*> dst)
{
    ...
    absl::strings_internal::STLStringResizeUninitializedAmortized(dst, new_dst_size);
...
{
    ABSL_HARDENING_ASSERT(bytes_remaining_ >= n && "Attempted to iterate past `end()`");
    Cord subcord;
    ...
    CordRep* payload = current_leaf_->IsSubstring() ? current_leaf_->substring()->child : current_leaf_;
    const char* data = payload->IsExternal() ? payload->external()->base : payload->flat()->Data();
...
bool IsSubstringInCordAt(absl::Cord::CharIterator position, absl::string_view needle)
{
...
absl::Cord::CharIterator absl::Cord::FindImpl(CharIterator it, absl::string_view needle) const
{
...
// REQUIRES: remaining `absl::Cord` starting at `position` is greater than or
// equal to `needle.size()`.
bool IsSubcordInCordAt(absl::Cord::CharIterator position, const absl::Cord& needle)
{
...
absl::string_view Cord::FlattenSlowPath()
{
    ...
    if (total_size <= kMaxFlatLength) {
        ...
        new_rep = absl::cord_internal::NewExternalRep(
            absl::string_view(new_buffer, total_size), [](absl::string_view s) { std::allocator<char>().deallocate(const_cast<char*>(s.data()), s.size()); });
    }
...
/* static */ bool Cord::GetFlatAux(absl::Nonnull<CordRep*> rep, absl::Nonnull<absl::string_view*> fragment)
{
    assert(rep != nullptr);
    if (rep->length == 0) {
    ...
    if (rep->IsFlat()) {
        ...
        *fragment = absl::string_view(rep->external()->base, rep->length);
        ...
        if (child->IsFlat()) {
            *fragment = absl::string_view(child->flat()->Data() + rep->substring()->start, rep->length);
            return true;
        } else if (child->IsExternal()) {
            *fragment = absl::string_view(child->external()->base + rep->substring()->start, rep->length);
            return true;
        } else if (child->IsBtree()) {
            return child->btree()->IsFlat(rep->substring()->start, rep->length, fragment);
        }
    }
    return false;
}

/* static */ void Cord::ForEachChunkAux(absl::Nonnull<absl::cord_internal::CordRep*> rep, absl::FunctionRef<void(absl::string_view)> callback)
{
    assert(rep != nullptr);
    if (rep->length == 0)
        return;
...
static void DumpNode(absl::Nonnull<CordRep*> nonnull_rep, bool include_data, absl::Nonnull<std::ostream*> os, int indent)
{
        ...
        if (rep == nullptr) {
            ...
            *os << "SUBSTRING @ " << rep->substring()->start << "\n";
            ...
            if (rep->IsExternal()) {
                *os << "EXTERNAL [";
                if (include_data)
                    *os << absl::CEscape(absl::string_view(rep->external()->base, rep->length));
                *os << "]\n";
            } else if (rep->IsFlat()) {
                *os << "FLAT cap=" << rep->flat()->Capacity() << " [";
                if (include_data)
                    *os << absl::CEscape(absl::string_view(rep->flat()->Data(), rep->length));
                *os << "]\n";
            } else {
...
static std::string ReportError(absl::Nonnull<CordRep*> root, absl::Nonnull<CordRep*> node)
{
...
static bool VerifyNode(absl::Nonnull<CordRep*> root, absl::Nonnull<CordRep*> start_node)
{
    ...
    do {
        ...
        if (node->IsFlat()) {
            ABSL_INTERNAL_CHECK(node->length <= node->flat()->Capacity(), ReportError(root, node));
        } else if (node->IsExternal()) {
            ABSL_INTERNAL_CHECK(node->external()->base != nullptr, ReportError(root, node));
        } else if (node->IsSubstring()) {
            ABSL_INTERNAL_CHECK(node->substring()->start < node->substring()->child->length, ReportError(root, node));
            ABSL_INTERNAL_CHECK(node->substring()->start + node->length <= node->substring()->child->length, ReportError(root, node));
        } else if (node->IsCrc()) {
            ABSL_INTERNAL_CHECK(node->crc()->child != nullptr || node->crc()->length == 0, ReportError(root, node));
            if (node->crc()->child != nullptr) {
                ABSL_INTERNAL_CHECK(node->crc()->length == node->crc()->child->length, ReportError(root, node));
