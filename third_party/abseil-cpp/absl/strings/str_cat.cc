...
inline absl::Nonnull<char*> Append(absl::Nonnull<char*> out, const AlphaNum& x)
{
...
inline void STLStringAppendUninitializedAmortized(std::string* dest, size_t to_append)
{
    strings_internal::AppendUninitializedTraits<std::string>::Append(dest, to_append);
}
} // namespace
...
std::string StrCat(const AlphaNum& a, const AlphaNum& b)
{
    ...
    absl::strings_internal::STLStringResizeUninitialized(&result, static_cast<size_t>(result_size));
...
std::string StrCat(const AlphaNum& a, const AlphaNum& b, const AlphaNum& c)
{
    ...
    ABSL_INTERNAL_CHECK(result_size <= kMaxSize, "size_t overflow");
    strings_internal::STLStringResizeUninitialized(&result, static_cast<size_t>(result_size));
    char* const begin = &result[0];
    char* out = begin;
    out = Append(out, a);
    out = Append(out, b);
...
std::string StrCat(const AlphaNum& a, const AlphaNum& b, const AlphaNum& c, const AlphaNum& d)
{
    ...
    const uint64_t result_size
        = static_cast<uint64_t>(a.size()) + static_cast<uint64_t>(b.size()) + static_cast<uint64_t>(c.size()) + static_cast<uint64_t>(d.size());
    ABSL_INTERNAL_CHECK(result_size <= kMaxSize, "size_t overflow");
    strings_internal::STLStringResizeUninitialized(&result, static_cast<size_t>(result_size));
    char* const begin = &result[0];
    char* out = begin;
...
std::string CatPieces(std::initializer_list<absl::string_view> pieces)
{
    ...
    strings_internal::STLStringResizeUninitialized(&result, static_cast<size_t>(total_size));
...
void StrAppend(absl::Nonnull<std::string*> dest, const AlphaNum& a, const AlphaNum& b, const AlphaNum& c, const AlphaNum& d)
{
    ...
    std::string::size_type old_size = dest->size();
    STLStringAppendUninitializedAmortized(dest, a.size() + b.size() + c.size() + d.size());
    char* const begin = &(*dest)[0];
    char* out = begin + old_size;
    out = Append(out, a);
    out = Append(out, b);
