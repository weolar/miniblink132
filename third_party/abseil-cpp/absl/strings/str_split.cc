...
template <typename FindPolicy> absl::string_view GenericFind(absl::string_view text, absl::string_view delimiter, size_t pos, FindPolicy find_policy)
{
    ...
    if (found_pos != absl::string_view::npos) {
        found = absl::string_view(text.data() + found_pos, find_policy.Length(delimiter));
    }
    return found;
}
...
struct LiteralPolicy {
    static size_t Find(absl::string_view text, absl::string_view delimiter, size_t pos)
    {
...
absl::string_view ByAsciiWhitespace::Find(absl::string_view text, size_t pos) const
{
