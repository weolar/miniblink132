...
struct TestParam {
    ...
    {
        return absl::StrCat(refcount_is_one ? "Private" : "Shared", with_capacity ? "" : "_NoCapacity",
            (input_share_mode == kPrivate)      ? ""
                : (input_share_mode == kShared) ? "_SharedInput"
...
CordRepExternal* MakeExternal(absl::string_view s)
{
    struct Rep : public CordRepExternal {
        std::string s;
        explicit Rep(absl::string_view s)
            : s(s)
        {
            this->tag = EXTERNAL;
            this->base = s.data();
            this->length = s.length();
            this->releaser_invoker = [](CordRepExternal* self) { delete static_cast<Rep*>(self); };
        }
    };
    return new Rep(s);
}
...
constexpr const char* kFoxFlats[] = { "The ", "quick ", "brown ", "fox ", "jumps ", "over ", "the ", "lazy ", "dog" };
...
public:
    ...
    template <typename CordRepType> CordRepType* NeedsUnref(CordRepType* rep)
    {
        assert(rep);
...
public:
    ...
    bool Shared() const
    {
        return !GetParam().refcount_is_one;
    }
    bool InputShared() const
    {
        return GetParam().input_share_mode == kShared;
    }
    ...
    template <typename CordRepType> CordRepType* RefIfShared(CordRepType* rep)
    {
        return Shared() ? Ref(rep) : rep;
    }

    template <typename CordRepType> CordRepType* RefIfInputShared(CordRepType* rep)
    {
        return InputShared() ? Ref(rep) : rep;
    }

    template <typename CordRepType> CordRepType* RefIfInputSharedIndirect(CordRepType* rep)
    {
...
TEST_P(CordRingCreateFromTreeTest, CreateFromSubstringOfLargeExternal)
{
    ...
    auto str = not_a_string_view(external->base, 1 << 20).remove_prefix(1 << 19).remove_suffix(6);
    auto* child = RefIfInputSharedIndirect(MakeSubstring(1 << 19, (1 << 19) - 6, external));
    CordRepRing* result = NeedsUnref(CordRepRing::Create(child));
    ASSERT_THAT(result, IsValidRingBuffer());
...
TEST_P(CordRingBuildTest, AppendPrependStringMixWithExtra)
{
    ...
    EXPECT_THAT(result, EqIfPrivateAndCapacity(GetParam(), ring));
    if (GetParam().refcount_is_one) {
        EXPECT_THAT(ToFlats(result), ElementsAre("The quick brown fox ", "jumps over the lazy dog"));
    } else {
        EXPECT_THAT(ToFlats(result), ElementsAre("The quick brown fox ", "jumps ", "over the lazy dog"));
    }
}
...
TEST_P(CordRingSubTest, RemovePrefixFromLargeExternal)
{
    ...
    EXPECT_THAT(
        ToRawFlats(result), ElementsAre(not_a_string_view(external1->base, 1 << 20).remove_prefix(1 << 16), not_a_string_view(external2->base, 1 << 20)));
}
...
TEST_P(CordRingBuildInputTest, AppendRingWithFlatOffset)
{
    ...
    EXPECT_THAT(result, EqIfPrivateAndCapacity(GetParam(), ring));
    EXPECT_THAT(result, NeIfShared(GetParam(), ring));
    EXPECT_THAT(ToFlats(result), ElementsAre("Head", "brown ", "fox ", "jumps ", "over ", "the ", "lazy ", "dog"));
}
...
TEST_P(CordRingBuildInputTest, AppendRingWithBrokenOffset)
{
    ...
    EXPECT_THAT(result, EqIfPrivateAndCapacity(GetParam(), ring));
    EXPECT_THAT(result, NeIfShared(GetParam(), ring));
    EXPECT_THAT(ToFlats(result), ElementsAre("Head", "umps ", "over ", "the ", "lazy ", "dog"));
}
...
TEST_P(CordRingBuildInputTest, AppendRingWithFlatLength)
{
    ...
    EXPECT_THAT(result, EqIfPrivateAndCapacity(GetParam(), ring));
    EXPECT_THAT(result, NeIfShared(GetParam(), ring));
    EXPECT_THAT(ToFlats(result), ElementsAre("Head", "The ", "quick ", "brown ", "fox ", "jumps ", "over ", "the "));
}
...
TEST_P(CordRingBuildTest, AppendRingWithBrokenFlatLength)
{
    ...
    EXPECT_THAT(result, EqIfPrivateAndCapacity(GetParam(), ring));
    EXPECT_THAT(result, NeIfShared(GetParam(), ring));
    EXPECT_THAT(ToFlats(result), ElementsAre("Head", "The ", "quick ", "brown ", "fox ", "jumps ", "ov"));
}
...
TEST_P(CordRingBuildTest, AppendRingMiddlePiece)
{
    ...
    EXPECT_THAT(result, EqIfPrivateAndCapacity(GetParam(), ring));
    EXPECT_THAT(result, NeIfShared(GetParam(), ring));
    EXPECT_THAT(ToFlats(result), ElementsAre("Head", "ck ", "brown ", "fox ", "jum"));
}
...
TEST_P(CordRingBuildInputTest, PrependRingWithFlatOffset)
{
    ...
    EXPECT_THAT(result, EqIfPrivateAndCapacity(GetParam(), ring));
    EXPECT_THAT(result, NeIfShared(GetParam(), ring));
    EXPECT_THAT(ToFlats(result), ElementsAre("brown ", "fox ", "jumps ", "over ", "the ", "lazy ", "dog", "Tail"));
}
...
TEST_P(CordRingBuildInputTest, PrependRingWithBrokenOffset)
{
    ...
    EXPECT_THAT(result, EqIfPrivateAndCapacity(GetParam(), ring));
    EXPECT_THAT(result, NeIfShared(GetParam(), ring));
    EXPECT_THAT(ToFlats(result), ElementsAre("umps ", "over ", "the ", "lazy ", "dog", "Tail"));
}
...
TEST_P(CordRingBuildInputTest, PrependRingWithFlatLength)
{
    ...
    EXPECT_THAT(result, EqIfPrivateAndCapacity(GetParam(), ring));
    EXPECT_THAT(result, NeIfShared(GetParam(), ring));
    EXPECT_THAT(ToFlats(result), ElementsAre("The ", "quick ", "brown ", "fox ", "jumps ", "over ", "the ", "Tail"));
}
...
TEST_P(CordRingBuildInputTest, PrependRingWithBrokenFlatLength)
{
    ...
    EXPECT_THAT(result, EqIfPrivateAndCapacity(GetParam(), ring));
    EXPECT_THAT(result, NeIfShared(GetParam(), ring));
    EXPECT_THAT(ToFlats(result), ElementsAre("The ", "quick ", "brown ", "fox ", "jumps ", "ov", "Tail"));
}
...
TEST_P(CordRingBuildInputTest, PrependRingMiddlePiece)
{
    ...
    CordRepRing* ring = CreateWithCapacity(MakeFlat("Tail"), flats.size());
    CordRep* child = RefIfInputShared(FromFlats(flats, composition));
    CordRep* stripped = RefIfInputSharedIndirect(MakeSubstring(7, child->length - 27, child));
    CordRepRing* result = NeedsUnref(CordRepRing::Prepend(ring, stripped));
    ASSERT_THAT(result, IsValidRingBuffer());
    EXPECT_THAT(result, EqIfPrivateAndCapacity(GetParam(), ring));
    EXPECT_THAT(result, NeIfShared(GetParam(), ring));
    EXPECT_THAT(ToFlats(result), ElementsAre("ck ", "brown ", "fox ", "jum", "Tail"));
}
...
TEST_F(CordRingTest, Find)
{
    constexpr const char* flats[]
        = { "abcdefghij", "klmnopqrst", "uvwxyz", "ABCDEFGHIJ", "KLMNOPQRST", "UVWXYZ", "1234567890", "~!@#$%^&*()_", "+-=", "[]\\{}|;':", ",/<>?", "." };
    auto composition = RandomComposition();
    SCOPED_TRACE(ToString(composition));
