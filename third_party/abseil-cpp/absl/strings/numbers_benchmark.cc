...
template <typename T> void BM_FastIntToBuffer(benchmark::State& state)
{
...
void BM_safe_strto32_string(benchmark::State& state)
{
    ...
    int32_t value = 0;
    for (auto _ : state) {
        benchmark::DoNotOptimize(absl::numbers_internal::safe_strto32_base(str, &value, base));
    }
...
void BM_safe_strto64_string(benchmark::State& state)
{
    ...
    int64_t value = 0;
    for (auto _ : state) {
        benchmark::DoNotOptimize(absl::numbers_internal::safe_strto64_base(str, &value, base));
    }
    ABSL_RAW_CHECK(value == RepeatedSevens(digits, base), "");
}
...
void BM_safe_strtou32_string(benchmark::State& state)
{
    ...
    uint32_t value = 0;
    for (auto _ : state) {
        benchmark::DoNotOptimize(absl::numbers_internal::safe_strtou32_base(str, &value, base));
    }
    ABSL_RAW_CHECK(value == RepeatedSevens(digits, base), "");
}
...
void BM_safe_strtou64_string(benchmark::State& state)
{
    ...
    uint64_t value = 0;
    for (auto _ : state) {
        benchmark::DoNotOptimize(absl::numbers_internal::safe_strtou64_base(str, &value, base));
    }
    ABSL_RAW_CHECK(value == RepeatedSevens(digits, base), "");
}
...
template <typename StringType> StringType GetStringAs(const std::string& s)
{
    return static_cast<StringType>(s);
}
template <> const char* GetStringAs<const char*>(const std::string& s)
{
    return s.c_str();
}

template <typename StringType> std::vector<StringType> GetStringsAs(const std::vector<std::string>& strings)
{
...
template <typename T> void BM_SimpleAtof(benchmark::State& state)
{
    const int num_strings = state.range(0);
    const int num_digits = state.range(1);
    std::vector<std::string> backing_strings = MakeFloatStrings(num_strings, num_digits);
...
BENCHMARK_TEMPLATE(BM_SimpleAtof, absl::string_view)->ArgPair(10, 1)->ArgPair(10, 2)->ArgPair(10, 4)->ArgPair(10, 8);
BENCHMARK_TEMPLATE(BM_SimpleAtof, const char*)->ArgPair(10, 1)->ArgPair(10, 2)->ArgPair(10, 4)->ArgPair(10, 8);
BENCHMARK_TEMPLATE(BM_SimpleAtof, std::string)->ArgPair(10, 1)->ArgPair(10, 2)->ArgPair(10, 4)->ArgPair(10, 8);

template <typename T> void BM_SimpleAtod(benchmark::State& state)
{
    const int num_strings = state.range(0);
    const int num_digits = state.range(1);
    std::vector<std::string> backing_strings = MakeFloatStrings(num_strings, num_digits);
...
BENCHMARK_TEMPLATE(BM_SimpleAtod, absl::string_view)->ArgPair(10, 1)->ArgPair(10, 2)->ArgPair(10, 4)->ArgPair(10, 8);
BENCHMARK_TEMPLATE(BM_SimpleAtod, const char*)->ArgPair(10, 1)->ArgPair(10, 2)->ArgPair(10, 4)->ArgPair(10, 8);
BENCHMARK_TEMPLATE(BM_SimpleAtod, std::string)->ArgPair(10, 1)->ArgPair(10, 2)->ArgPair(10, 4)->ArgPair(10, 8);
