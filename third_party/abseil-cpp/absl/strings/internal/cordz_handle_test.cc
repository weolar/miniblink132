...
struct CordzHandleDeleteTracker : public CordzHandle {
    bool* deleted;
    explicit CordzHandleDeleteTracker(bool* deleted)
        : deleted(deleted)
    {
    }
    ~CordzHandleDeleteTracker() override
    {
        *deleted = true;
    }
};
...
TEST(CordzHandleTest, MultiSnapshot)
{
    ...
    EXPECT_THAT(DeleteQueue(), ElementsAre(handle[2], snapshot[2], handle[1], snapshot[1], handle[0], snapshot[0]));
    EXPECT_THAT(deleted, ElementsAre(false, false, false));

    delete snapshot[1];
    EXPECT_THAT(DeleteQueue(), ElementsAre(handle[2], snapshot[2], handle[1], handle[0], snapshot[0]));
    EXPECT_THAT(deleted, ElementsAre(false, false, false));
...
TEST(CordzHandleTest, DiagnosticsGetSafeToInspectDeletedHandles)
{
    ...
    EXPECT_THAT(snapshot1->DiagnosticsGetSafeToInspectDeletedHandles(), IsEmpty());
    ...
    EXPECT_THAT(snapshot1->DiagnosticsGetSafeToInspectDeletedHandles(), ElementsAre(handle));
    EXPECT_THAT(snapshot2->DiagnosticsGetSafeToInspectDeletedHandles(), IsEmpty());
            ...
            pool.Schedule([&stop, &handles, &found_safe_to_inspect]() {
                ...
                while (!stop.HasBeenNotified()) {
                    ...
                    if (old_handle != nullptr) {
                        std::vector<const CordzHandle*> safe_to_inspect = old_handle->DiagnosticsGetSafeToInspectDeletedHandles();
