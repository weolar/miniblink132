...
void DumpAll(const CordRep* rep, bool include_contents, std::ostream& stream, size_t depth = 0)
{
    ...
    auto maybe_dump_data = [&stream, include_contents](const CordRep* r) {
        if (include_contents) {
            ...
            stream << ", data = \"" << EdgeData(r).substr(0, kMaxDataLength) << (r->length > kMaxDataLength ? "\"..." : "\"");
        }
        stream << '\n';
    };
...
template <EdgeType edge_type> inline absl::string_view Consume(absl::string_view s, size_t n)
{
...
template <EdgeType edge_type> struct StackOperations {
    ...
    inline CordRepBtree* Propagate(CordRepBtree* tree, int depth, size_t length, OpResult result)
    {
...
{
    cord_btree_exhaustive_validation.store(do_exaustive_validation, std::memory_order_relaxed);
}
...
void CordRepBtree::Dump(const CordRep* rep, absl::string_view label, std::ostream& stream)
{
...
#define NODE_CHECK_EQ(x, y)                                                                                                                                    \
    if ((x) != (y)) {                                                                                                                                          \
        ABSL_RAW_LOG(ERROR, "CordRepBtree::CheckValid() FAILED: %s != %s (%s vs %s)", #x, #y, absl::StrCat(x).c_str(), absl::StrCat(y).c_str());               \
        return false;                                                                                                                                          \
    }
...
template <EdgeType edge_type> inline OpResult CordRepBtree::AddEdge(bool owned, CordRep* edge, size_t delta)
{
...
template <EdgeType edge_type> CordRepBtree* CordRepBtree::AddCordRep(CordRepBtree* tree, CordRep* rep)
{
...
template <> CordRepBtree* CordRepBtree::NewLeaf<kBack>(absl::string_view data, size_t extra)
{
...
template <> CordRepBtree* CordRepBtree::NewLeaf<kFront>(absl::string_view data, size_t extra)
{
    CordRepBtree* leaf = CordRepBtree::New(0);
    size_t length = 0;
