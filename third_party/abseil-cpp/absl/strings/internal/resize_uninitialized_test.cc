...
struct resizable_string {
    ...
    resizable_string& erase(size_t = 0, size_t = 0)
    {
        return *this;
    }
};
...
struct default_init_string {
    ...
    void __resize_default_init(size_t)
    {
        resize_default_init_call_count += 1;
    }
    void __append_default_init(size_t)
    {
        append_default_init_call_count += 1;
    }
    void reserve(size_t)
    {
    }
    default_init_string& erase(size_t = 0, size_t = 0)
    {
        return *this;
    }
};
...
TEST(ResizeUninit, WithAndWithout)
{
    ...
    append_default_init_call_count = 0;
    {
        ...
        EXPECT_FALSE(absl::strings_internal::STLStringSupportsNontrashingResize(&rs));
        EXPECT_EQ(resize_call_count, 0);
        EXPECT_EQ(append_call_count, 0);
        EXPECT_EQ(resize_default_init_call_count, 0);
        EXPECT_EQ(append_default_init_call_count, 0);
    ...
    append_default_init_call_count = 0;
    {
        ...
        EXPECT_EQ(resize_default_init_call_count, 0);
        EXPECT_EQ(append_default_init_call_count, 0);
        EXPECT_TRUE(absl::strings_internal::STLStringSupportsNontrashingResize(&rus));
        EXPECT_EQ(resize_call_count, 0);
        EXPECT_EQ(append_call_count, 0);
        EXPECT_EQ(resize_default_init_call_count, 0);
        EXPECT_EQ(append_default_init_call_count, 0);
