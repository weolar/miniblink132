...
MATCHER_P(EqFlatHolding, data, "Equals flat holding data")
{
    ...
    if (actual != data) {
        *result_listener << "Expected flat holding \"" << data << "\", got flat holding \"" << actual << "\"";
        return false;
    }
    return true;
}
...
MATCHER_P(IsNode, height, absl::StrCat("Is a valid node of height ", height))
{
    ...
    if (!CordRepBtree::IsValid(arg->btree())) {
        CordRepBtree::Dump(arg->btree(), "Expected valid NODE, got:", false, *result_listener->stream());
        return false;
    }
    if (arg->btree()->height() != height) {
        *result_listener << "Expected NODE of height " << height << ", got " << arg->btree()->height();
        return false;
    }
    return true;
}

MATCHER_P2(IsSubstring, start, length, absl::StrCat("Is a substring(start = ", start, ", length = ", length, ")"))
{
    if (arg == nullptr) {
    ...
    if (arg->tag != SUBSTRING) {
        *result_listener << "Expected SUBSTRING, got " << static_cast<int>(arg->tag);
        return false;
    }
    const CordRepSubstring* const substr = arg->substring();
    if (substr->start != start || substr->length != length) {
        *result_listener << "Expected substring(" << start << ", " << length << "), got substring(" << substr->start << ", " << substr->length << ")";
        return false;
    }
    return true;
}
...
MATCHER_P2(EqExtractResult, tree, rep, "Equals ExtractResult")
{
    if (arg.tree != tree || arg.extracted != rep) {
        *result_listener << "Expected {" << static_cast<const void*>(tree) << ", " << static_cast<const void*>(rep) << "}, got {" << arg.tree << ", "
                         << arg.extracted << "}";
        return false;
    }
    return true;
...
public:
    ...
    {
        return forward_ ? data_.substr(0, consumed_) : data_.substr(data_.size() - consumed_);
    }
...
CordRepBtree* BtreeAdd(CordRepBtree* node, bool append, absl::string_view data)
{
    return append ? CordRepBtree::Append(node, data) : CordRepBtree::Prepend(node, data);
}
...
CordRepBtree* MakeTree(size_t size, bool append = true)
{
    ...
    for (size_t i = 1; i < size; ++i) {
        tree = append ? CordRepBtree::Append(tree, MakeHexFlat(i)) : CordRepBtree::Prepend(tree, MakeHexFlat(i));
    }
    return tree;
}
...
CordRepBtree* CreateTree(absl::Span<CordRep* const> reps)
{
    ...
    while (++it != reps.end())
        tree = CordRepBtree::Append(tree, *it);
    return tree;
}
...
CordRepBtree* CreateTreeReverse(absl::string_view data, size_t chunk_size)
{
    ...
    while (++rit != flats.rend())
        tree = CordRepBtree::Prepend(tree, *rit);
    return tree;
}
...
INSTANTIATE_TEST_SUITE_P(WithParam, CordRepBtreeTest, testing::Bool(), CordRepBtreeTest::ToString);
...
INSTANTIATE_TEST_SUITE_P(WithHeights, CordRepBtreeHeightTest, testing::Range(0, CordRepBtree::kMaxHeight), CordRepBtreeHeightTest::ToString);

using TwoBools = testing::tuple<bool, bool>;

class CordRepBtreeDualTest : public testing::TestWithParam<TwoBools> {
public:
    bool first_shared() const
    {
        return std::get<0>(GetParam());
    }
    bool second_shared() const
    {
...
INSTANTIATE_TEST_SUITE_P(WithParam, CordRepBtreeDualTest, testing::Combine(testing::Bool(), testing::Bool()), CordRepBtreeDualTest::ToString);

TEST(CordRepBtreeTest, SizeIsMultipleOf64)
{
    // Only enforce for fully 64-bit platforms.
    if (sizeof(size_t) == 8 && sizeof(void*) == 8) {
        EXPECT_THAT(sizeof(CordRepBtree) % 64, Eq(0u)) << "Should be multiple of 64";
    }
}
...
TEST(CordRepBtreeTest, EdgeData)
{
    ...
    EXPECT_THAT(EdgeData(substr2).data(), TypedEq<const void*>(external->base + 1));
...
TEST_P(CordRepBtreeDualTest, MergeLeafsNotExceedingCapacity)
{
    for (bool use_append : { false, true }) {
        ...
        CordRepBtree* tree = use_append ? CordRepBtree::Append(left, right) : CordRepBtree::Prepend(right, left);
...
TEST(CordRepBtreeTest, MergeFuzzTest)
{
    ...
    for (int i = 0; i < 10000; ++i) {
        ...
        GetLeafEdges(left, flats);
        if (dice_throw(rnd) == 1) {
            std::uniform_int_distribution<size_t> dist(0, static_cast<size_t>(left->height()));
        ...
        GetLeafEdges(right, flats);
        if (dice_throw(rnd) == 1) {
            std::uniform_int_distribution<size_t> dist(0, static_cast<size_t>(right->height()));
...
TEST_P(CordRepBtreeTest, AppendDataToTree)
{
    ...
    EXPECT_THAT(result->Edges(), ElementsAre(leaf0, Conditional(shared(), Ne(leaf1), Eq(leaf1))));
...
TEST_P(CordRepBtreeTest, PrependDataToTree)
{
    ...
    EXPECT_THAT(result, Conditional(shared(), Ne(tree), Eq(tree)));
    EXPECT_THAT(result->Edges(), ElementsAre(Conditional(shared(), Ne(leaf0), Eq(leaf0)), leaf1));
...
TEST(CordRepBtreeTest, Dump)
{
    ...
    for (int api = 0; api <= 3; ++api) {
        ...
        EXPECT_THAT(str, AllOf(HasSubstr("Node(1)"), HasSubstr("Leaf"), HasSubstr("Private"), HasSubstr("Shared")));

        // Contains length and start offset of all data edges
        EXPECT_THAT(str,
            AllOf(HasSubstr("len = 11"), HasSubstr("len = 14"), HasSubstr("len = 6"), HasSubstr("len = 7"), HasSubstr("start = 1"), HasSubstr("start = 2")));

        // Contains address of all data edges
        EXPECT_THAT(str,
            AllOf(HasSubstr(absl::StrCat("0x", absl::Hex(flat))), HasSubstr(absl::StrCat("0x", absl::Hex(external))),
                HasSubstr(absl::StrCat("0x", absl::Hex(substr_flat))), HasSubstr(absl::StrCat("0x", absl::Hex(substr_external)))));
        ...
        if (api != 3) {
            ...
            EXPECT_THAT(str,
                Not(AnyOf((HasSubstr("data = \"Hello world\""), HasSubstr("data = \"Hello external\""), HasSubstr("data = \"ello w\""),
                    HasSubstr("data = \"llo ext\"")))));
        } else {
            // Contains contents
            EXPECT_THAT(str,
                AllOf((HasSubstr("data = \"Hello world\""), HasSubstr("data = \"Hello external\""), HasSubstr("data = \"ello w\""),
                    HasSubstr("data = \"llo ext\""))));
        }
    }
...
TEST(CordRepBtreeTest, AssertValid)
{
    ...
    EXPECT_DEBUG_DEATH(EXPECT_THAT(CordRepBtree::AssertValid(nulltree), Eq(nulltree)), ".*");
    EXPECT_DEBUG_DEATH(EXPECT_THAT(CordRepBtree::AssertValid(cnulltree), Eq(cnulltree)), ".*");
...
TEST(CordRepBtreeTest, CheckAssertValidShallowVsDeep)
{
    ...
    auto cleanup = absl::MakeCleanup([exhaustive_validation] { SetCordBtreeExhaustiveValidation(exhaustive_validation); });
