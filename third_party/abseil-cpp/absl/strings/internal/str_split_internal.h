...
template <typename T> struct HasMappedType<T, absl::void_t<typename T::mapped_type>> : std::true_type { };

// HasValueType<T>::value is true iff there exists a type T::value_type.
template <typename T, typename = void> struct HasValueType : std::false_type { };
template <typename T> struct HasValueType<T, absl::void_t<typename T::value_type>> : std::true_type { };

// HasConstIterator<T>::value is true iff there exists a type T::const_iterator.
template <typename T, typename = void> struct HasConstIterator : std::false_type { };
template <typename T> struct HasConstIterator<T, absl::void_t<typename T::const_iterator>> : std::true_type { };
...
template <typename T> struct HasEmplace<T, absl::void_t<decltype(std::declval<T>().emplace())>> : std::true_type { };
...
template <typename C, bool has_value_type, bool has_mapped_type> struct SplitterIsConvertibleToImpl : std::false_type { };

template <typename C> struct SplitterIsConvertibleToImpl<C, true, false> : std::is_constructible<typename C::value_type, absl::string_view> { };

template <typename C>
struct SplitterIsConvertibleToImpl<C, true, true>
    : absl::conjunction<std::is_constructible<typename C::key_type, absl::string_view>, std::is_constructible<typename C::mapped_type, absl::string_view>> { };
...
template <typename StringType, typename Container, typename = void> struct ShouldUseLifetimeBound : std::false_type { };

template <typename StringType, typename Container>
struct ShouldUseLifetimeBound<StringType, Container,
    std::enable_if_t<std::is_same<StringType, std::string>::value && std::is_same<typename Container::value_type, absl::string_view>::value>> : std::true_type {
};
...
using ShouldUseLifetimeBoundForPair = std::integral_constant<bool,
    std::is_same<StringType, std::string>::value && (std::is_same<First, absl::string_view>::value || std::is_same<Second, absl::string_view>::value)>;
...
private:
    template <typename First, typename Second> std::pair<First, Second> ConvertToPair() const
    {
    ...
    template <typename A> struct ConvertToContainer<std::vector<absl::string_view, A>, absl::string_view, false> {
    ...
    // std::string moves.
    template <typename A> struct ConvertToContainer<std::vector<std::string, A>, std::string, false> {
    ...
    // value.
    template <typename Container, typename First, typename Second> struct ConvertToContainer<Container, std::pair<const First, Second>, true> {
        ...
        template <typename M> static absl::enable_if_t<HasEmplace<M>::value, iterator> InsertOrEmplace(M* m, absl::string_view key)
        {
