...
#if defined(ABSL_HAVE_CONSTANT_EVALUATED) && (defined(ABSL_HAVE_ADDRESS_SANITIZER) || defined(ABSL_HAVE_MEMORY_SANITIZER))
#define ABSL_INTERNAL_CORD_HAVE_SANITIZER 1
#endif

#define ABSL_CORD_INTERNAL_NO_SANITIZE ABSL_ATTRIBUTE_NO_SANITIZE_ADDRESS ABSL_ATTRIBUTE_NO_SANITIZE_MEMORY
...
template <bool nullify_tail = false> inline void SmallMemmove(char* dst, const char* src, size_t n)
{
    if (n >= 8) {
        assert(n <= 15);
        uint64_t buf1;
...
template <typename Releaser, typename = ::absl::base_internal::invoke_result_t<Releaser>> void InvokeReleaser(Rank0, Releaser&& releaser, absl::string_view)
{
    ::absl::base_internal::invoke(std::forward<Releaser>(releaser));
}

// We use CompressedTuple so that we can benefit from EBCO.
template <typename Releaser> struct CordRepExternalImpl : public CordRepExternal, public ::absl::container_internal::CompressedTuple<Releaser> {
...
template <typename Str> ABSL_CONST_INIT CordRepExternal ConstInitExternalStorage<Str>::value(Str::value);
...
public:
    ...
    static bool is_either_profiled(const InlineData& data1, const InlineData& data2)
    {
        ...
        return (data1.rep_.cordz_info() | data2.rep_.cordz_info()) != kNullCordzInfo;
    }
    ...
    {
        assert(is_tree());
        intptr_t info = static_cast<intptr_t>(absl::little_endian::ToHost64(static_cast<uint64_t>(rep_.cordz_info())));
        assert(info & 1);
    ...
    void set_cordz_info(CordzInfo* cordz_info)
    {
        ...
        rep_.set_cordz_info(static_cast<cordz_info_t>(absl::little_endian::FromHost64(info)));
    }
...
private:
    struct Rep {
        ...
        explicit constexpr Rep(absl::string_view chars)
            : data { static_cast<char>((chars.size() << 1)), GetOrNull(chars, 0), GetOrNull(chars, 1), GetOrNull(chars, 2), GetOrNull(chars, 3),
                GetOrNull(chars, 4), GetOrNull(chars, 5), GetOrNull(chars, 6), GetOrNull(chars, 7), GetOrNull(chars, 8), GetOrNull(chars, 9),
                GetOrNull(chars, 10), GetOrNull(chars, 11), GetOrNull(chars, 12), GetOrNull(chars, 13), GetOrNull(chars, 14) }
        {
        }
