...
public:
    ...
    void AnalyzeCordRep(const CordRep* rep)
    {
        ...
        statistics_.estimated_fair_share_memory_usage += static_cast<size_t>(memory_usage_.fair_share);
    }
...
void CordzInfo::TrackCord(InlineData& cord, MethodIdentifier method, int64_t sampling_stride)
{
    assert(cord.is_tree());
    assert(!cord.is_profiled());
    CordzInfo* cordz_info = new CordzInfo(cord.as_tree(), nullptr, method, sampling_stride);
    cord.set_cordz_info(cordz_info);
    cordz_info->Track();
}

void CordzInfo::TrackCord(InlineData& cord, const InlineData& src, MethodIdentifier method)
{
    assert(cord.is_tree());
    assert(src.is_tree());
    ...
    if (cordz_info != nullptr)
        cordz_info->Untrack();

    // Start new cord sample
    cordz_info = new CordzInfo(cord.as_tree(), src.cordz_info(), method, src.cordz_info()->sampling_stride());
    cord.set_cordz_info(cordz_info);
    cordz_info->Track();
}

void CordzInfo::MaybeTrackCordImpl(InlineData& cord, const InlineData& src, MethodIdentifier method)
{
    if (src.is_profiled()) {
...
{
    if (src == nullptr)
        return MethodIdentifier::kUnknown;
    return src->parent_method_ != MethodIdentifier::kUnknown ? src->parent_method_ : src->method_;
}
...
CordzInfo::CordzInfo(CordRep* rep, const CordzInfo* src, MethodIdentifier method, int64_t sampling_stride)
    : rep_(rep)
    , stack_depth_(static_cast<size_t>(absl::GetStackTrace(stack_,
...
void CordzInfo::Lock(MethodIdentifier method) ABSL_EXCLUSIVE_LOCK_FUNCTION(mutex_)
{
    mutex_.Lock();
    update_tracker_.LossyAdd(method);
    assert(rep_);
