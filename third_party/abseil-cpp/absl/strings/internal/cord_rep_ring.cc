...
std::ostream& operator<<(std::ostream& s, const CordRepRing& rep)
{
    ...
    s << "  CordRepRing(" << &rep << ", length = " << rep.length << ", head = " << rep.head_ << ", tail = " << rep.tail_ << ", cap = " << rep.capacity_
      << ", rc = " << rep.refcount.Get() << ", begin_pos_ = " << static_cast<ptrdiff_t>(rep.begin_pos_) << ") {\n";
    CordRepRing::index_type head = rep.head();
    do {
        CordRep* child = rep.entry_child(head);
        s << " entry[" << head << "] length = " << rep.entry_length(head) << ", child " << child << ", clen = " << child->length
          << ", tag = " << static_cast<int>(child->tag) << ", rc = " << child->refcount.Get() << ", offset = " << rep.entry_data_offset(head)
...
public:
    Filler(CordRepRing* rep, index_type pos)
        : rep_(rep)
        , head_(pos)
        , pos_(pos)
    {
...
bool CordRepRing::IsValid(std::ostream& output) const
{
    ...
    if (head_ >= capacity_ || tail_ >= capacity_) {
        output << "head " << head_ << " and/or tail " << tail_ << "exceed capacity " << capacity_;
        return false;
    }
    ...
    if (pos_length != length) {
        output << "length " << length << " does not match positional length " << pos_length << " from begin_pos " << begin_pos_ << " and entry[" << back
               << "].end_pos " << entry_end_pos(back);
        return false;
    }
    ...
    do {
        ...
        if (offset >= child->length || entry_length > child->length - offset) {
            output << "entry[" << head << "] has offset " << offset << " and entry length " << entry_length << " which are outside of the child's length of "
                   << child->length;
            return false;
        }
...
void CordRepRing::SetCapacityForTesting(size_t capacity)
{
    ...
    memmove(Layout::Partial(capacity).Pointer<1>(data_) + head(), Layout::Partial(capacity_).Pointer<1>(data_) + head(),
        entries() * sizeof(Layout::ElementType<1>));
    memmove(Layout::Partial(capacity, capacity).Pointer<2>(data_) + head(), Layout::Partial(capacity_, capacity_).Pointer<2>(data_) + head(),
...
template <bool ref> void CordRepRing::Fill(const CordRepRing* src, index_type head, index_type tail)
{
...
CordRepRing* CordRepRing::Copy(CordRepRing* rep, index_type head, index_type tail, size_t extra)
{
...
CordRepRing* CordRepRing::CreateSlow(CordRep* child, size_t extra)
{
    ...
    Consume(child, [&](CordRep* child_arg, size_t offset, size_t len) {
        if (IsFlatOrExternal(child_arg)) {
            rep = rep ? AppendLeaf(rep, child_arg, offset, len) : CreateFromLeaf(child_arg, offset, len, extra);
        } else if (rep) {
...
template <CordRepRing::AddMode mode> CordRepRing* CordRepRing::AddRing(CordRepRing* rep, CordRepRing* ring, size_t offset, size_t len)
{
    ...
    const pos_type delta_length = (append ? rep->begin_pos_ + rep->length : rep->begin_pos_ - len) - ring->entry_begin_pos(head.index) - head.offset;
    ...
    if (ring->refcount.IsOne()) {
        ...
        ring->ForEach(head.index, tail.index,
            [&](index_type ix) { filler.Add(ring->entry_child(ix), ring->entry_data_offset(ix), ring->entry_end_pos(ix) + delta_length); });

        // Unref entries we did not copy over, and delete source.
        if (head.index != ring->head_)
            UnrefEntries(ring, ring->head_, head.index);
        if (tail.index != ring->tail_)
            UnrefEntries(ring, tail.index, ring->tail_);
        CordRepRing::Delete(ring);
    } else {
        ring->ForEach(head.index, tail.index, [&](index_type ix) {
            CordRep* child = ring->entry_child(ix);
            filler.Add(child, ring->entry_data_offset(ix), ring->entry_end_pos(ix) + delta_length);
            CordRep::Ref(child);
        });
...
CordRepRing* CordRepRing::AppendLeaf(CordRepRing* rep, CordRep* child, size_t offset, size_t len)
{
    rep = Mutable(rep, 1);
...
CordRepRing* CordRepRing::PrependLeaf(CordRepRing* rep, CordRep* child, size_t offset, size_t len)
{
    rep = Mutable(rep, 1);
...
CordRepRing* CordRepRing::SubRing(CordRepRing* rep, size_t offset, size_t len, size_t extra)
{
    ...
    if (rep->refcount.IsOne() && extra <= (rep->capacity() - new_entries)) {
        ...
        if (head.index != rep->head_)
            UnrefEntries(rep, rep->head_, head.index);
        if (tail.index != rep->tail_)
            UnrefEntries(rep, tail.index, rep->tail_);
