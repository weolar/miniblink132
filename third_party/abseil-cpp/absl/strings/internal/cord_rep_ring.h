...
public:
    ...
    template <typename F> void ForEach(index_type head, index_type tail, F&& f) const
    {
...
private:
    ...
    template <bool wrap> index_type FindBinary(index_type head, index_type tail, size_t offset) const;
    ...
    template <bool ref> void Fill(const CordRepRing* src, index_type head, index_type tail);
    ...
    template <AddMode mode> static CordRepRing* AddRing(CordRepRing* rep, CordRepRing* ring, size_t offset, size_t len);
...
{
    return sizeof(CordRepRing) - sizeof(data_) + Layout(capacity, capacity, capacity).AllocSize();
}
