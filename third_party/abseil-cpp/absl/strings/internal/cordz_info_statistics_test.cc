...
{
    return static_cast<CordRepExternal*>(NewExternalRep(absl::string_view("", length), [](absl::string_view) {}));
}
...
struct RefHelper {
    ...
    template <typename T> T* NeedsUnref(T* rep)
    {
        refs.push_back(rep);
        return rep;
    }
    ...
    template <typename T> T* Ref(T* rep, size_t n = 1)
    {
        while (n--) {
...
template <typename T> size_t SizeOf(const T* rep)
{
    return sizeof(T);
}

template <> size_t SizeOf(const CordRepFlat* rep)
{
    return rep->AllocatedSize();
}

template <> size_t SizeOf(const CordRepExternal* rep)
{
    // See cord.cc
...
#define STATS_MATCHER_EXPECT_EQ(member)                                                                                                                        \
    if (stats.member != arg.member) {                                                                                                                          \
        *result_listener << "\n    stats." << #member << ": actual = " << arg.member << ", expected " << stats.member;                                         \
        ok = false;                                                                                                                                            \
    }
...
TEST(CordzInfoStatisticsTest, SharedSubstring)
{
    ...
    expected.estimated_fair_share_memory_usage = SizeOf(substring) / 2 + SizeOf(flat) / 6;
    expected.node_count = 2;
    expected.node_counts.flat = 1;
    expected.node_counts.flat_512 = 1;
    expected.node_counts.substring = 1;
...
TEST(CordzInfoStatisticsTest, BtreeLeaf)
{
    ...
    expected.estimated_memory_usage = SizeOf(tree) + SizeOf(flat1) + SizeOf(flat2) + SizeOf(substr) + flat3_size + SizeOf(external);
    expected.estimated_fair_share_memory_usage = expected.estimated_memory_usage;
                        ...
                        if (coin_toss(gen) != 0) {
                            if (cord.is_tree()) {
                                ...
                                if (coin_toss(gen) != 0) {
                                    ...
                                    CordzUpdateScope scope(cord.cordz_info(), CordzUpdateTracker::kUnknown);
