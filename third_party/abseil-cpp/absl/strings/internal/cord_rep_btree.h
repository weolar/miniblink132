...
public:
    ...
    template <EdgeType edge_type> inline OpResult AddEdge(bool owned, CordRep* edge, size_t delta);
    ...
    template <EdgeType edge_type> OpResult SetEdge(bool owned, CordRep* edge, size_t delta);
...
private:
    ...
    template <EdgeType edge_type> static CordRepBtree* NewLeaf(absl::string_view data, size_t extra);
    ...
    // suffix of the input.
    // See the `Append/Prepend` function for the meaning and purpose of `extra`.
    template <EdgeType edge_type> absl::string_view AddData(absl::string_view data, size_t extra);
    ...
    template <EdgeType edge_type> static CordRepBtree* AddCordRep(CordRepBtree* tree, CordRep* rep);

    // Adds `data` to the specified tree, returning the modified tree.
    // See the `Append/Prepend` function for the meaning and purpose of `extra`.
    template <EdgeType edge_type> static CordRepBtree* AddData(CordRepBtree* tree, absl::string_view data, size_t extra = 0);
    ...
    template <EdgeType edge_type> static CordRepBtree* Merge(CordRepBtree* dst, CordRepBtree* src);
...
template <> inline void CordRepBtree::Add<CordRepBtree::kBack>(absl::Span<CordRep* const> edges)
{
    AlignBegin();
    size_t new_end = end();
    for (CordRep* edge : edges)
...
template <> inline void CordRepBtree::Add<CordRepBtree::kFront>(absl::Span<CordRep* const> edges)
{
    AlignEnd();
...
template <CordRepBtree::EdgeType edge_type> inline void CordRepBtree::SetEdge(CordRep* edge)
{
