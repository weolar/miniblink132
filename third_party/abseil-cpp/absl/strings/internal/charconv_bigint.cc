...
/* static */ BigUnsigned<max_words> BigUnsigned<max_words>::FiveToTheNth(int n)
{
    ...
    while (n >= kLargePowerOfFiveStep) {
        ...
        if (first_pass) {
            ...
            std::copy_n(LargePowerOfFiveData(big_power), LargePowerOfFiveSize(big_power), answer.words_);
...
template <int max_words> void BigUnsigned<max_words>::MultiplyStep(int original_size, const uint32_t* other_words, int other_size, int step)
{
