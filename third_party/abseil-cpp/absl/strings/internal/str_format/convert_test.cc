...
template <typename T> std::string LengthModFor()
{
    static_assert(AlwaysFalse<T>::value, "Unsupported type");
    return "";
}
template <> std::string LengthModFor<char>()
{
    return "hh";
}
template <> std::string LengthModFor<signed char>()
{
    return "hh";
}
template <> std::string LengthModFor<unsigned char>()
{
    return "hh";
}
template <> std::string LengthModFor<short>()
{ // NOLINT
    return "h";
}
template <> std::string LengthModFor<unsigned short>()
{ // NOLINT
    return "h";
}
template <> std::string LengthModFor<int>()
{
    return "";
}
template <> std::string LengthModFor<unsigned>()
{
    return "";
}
template <> std::string LengthModFor<long>()
{ // NOLINT
    return "l";
}
template <> std::string LengthModFor<unsigned long>()
{ // NOLINT
    return "l";
}
template <> std::string LengthModFor<long long>()
{ // NOLINT
    return "ll";
}
template <> std::string LengthModFor<unsigned long long>()
{ // NOLINT
    return "ll";
}
...
// `wchar_t`.
using IntegralTypeForWCharT = std::conditional_t<std::is_signed<wchar_t>::value,
    ...
    // opposite signedness.
    std::make_signed_t<std::make_unsigned_t<wchar_t>>, std::make_unsigned_t<std::make_signed_t<wchar_t>>>;

// Given an integral type `T`, returns a type of the same rank and signedness
// that is guaranteed to not be `wchar_t`.
template <typename T> using MatchingIntegralType = std::conditional_t<std::is_same<T, wchar_t>::value, IntegralTypeForWCharT, T>;
...
std::string EscCharImpl(int v)
{
    ...
    int n = absl::ascii_isprint(static_cast<unsigned char>(v))
        ...
        : snprintf(buf, sizeof(buf), "'\\x%.*x'", CHAR_BIT / 4, static_cast<unsigned>(static_cast<std::make_unsigned_t<char>>(v)));
    assert(n > 0 && static_cast<size_t>(n) < sizeof(buf));
    return std::string(buf, static_cast<size_t>(n));
}

std::string Esc(char v)
{
    return EscCharImpl(v);
}
std::string Esc(signed char v)
{
    return EscCharImpl(v);
}
std::string Esc(unsigned char v)
{
    return EscCharImpl(v);
}
...
{
    char buf[64];
    int n = std::iswprint(static_cast<wint_t>(v)) ? snprintf(buf, sizeof(buf), "L'%lc'", static_cast<wint_t>(v))
                                                  : snprintf(buf, sizeof(buf), "L'\\x%.*llx'", static_cast<int>(sizeof(wchar_t) * CHAR_BIT / 4),
                                                      static_cast<unsigned long long>(static_cast<std::make_unsigned_t<wchar_t>>(v)));
    assert(n > 0 && static_cast<size_t>(n) < sizeof(buf));
    return std::string(buf, static_cast<size_t>(n));
}

template <typename T> std::string Esc(const T& v)
{
...
NativePrintfTraits VerifyNativeImplementationImpl()
{
    ...
    result.hex_float_has_glibc_rounding = StartsWith(StrPrint("%.2a", d0079), "0x1.00") && StartsWith(StrPrint("%.2a", d0179), "0x1.01")
        && StartsWith(StrPrint("%.2a", d0080), "0x1.00") && StartsWith(StrPrint("%.2a", d0180), "0x1.02") && StartsWith(StrPrint("%.2a", d0081), "0x1.01")
    ...
    result.hex_float_uses_minimal_precision_when_not_specified = (StrPrint("%a", d0079) == "0x1.0079p+16");
    ...
    result.hex_float_optimizes_leading_digit_bit_count = StartsWith(StrPrint("%a", d_15), "0x1.8") && StartsWith(StrPrint("%La", ld_15), "0xc");
...
template <typename T> void TestStringConvert(const T& str)
{
...
TEST_F(FormatConvertTest, Pointer)
{
    ...
    const FormatArgImpl args_array[] = {
        ...
        FormatArgImpl(vwcp),
        FormatArgImpl(vcnil),
        FormatArgImpl(vwcnil),
    };
    auto args = absl::MakeConstSpan(args_array);

    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%p"), args), MatchesPointerString(&x));
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%20p"), args), MatchesPointerString(&x));
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%.1p"), args), MatchesPointerString(&x));
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%.20p"), args), MatchesPointerString(&x));
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%30.20p"), args), MatchesPointerString(&x));
    ...
    // const int*
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%1$p"), args), MatchesPointerString(xp));
    // const char*
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%2$p"), args), MatchesPointerString(cp));
    // const wchar_t*
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%3$p"), args), MatchesPointerString(wcp));
    // null const int*
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%4$p"), args), MatchesPointerString(nullptr));
    // null const char*
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%5$p"), args), MatchesPointerString(nullptr));
    // null const wchar_t*
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%6$p"), args), MatchesPointerString(nullptr));
    // nonconst char*
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%7$p"), args), MatchesPointerString(mcp));
    // nonconst wchar_t*
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%8$p"), args), MatchesPointerString(mwcp));
    // function pointer
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%9$p"), args), MatchesPointerString(reinterpret_cast<const void*>(fp)));
    // null function pointer
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%10$p"), args), MatchesPointerString(nullptr));
    // volatile char*
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%11$p"), args), MatchesPointerString(reinterpret_cast<volatile const void*>(vcp)));
    // volatile wchar_t*
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%12$p"), args), MatchesPointerString(reinterpret_cast<volatile const void*>(vwcp)));
    // null volatile char*
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%13$p"), args), MatchesPointerString(nullptr));
    // null volatile wchar_t*
    EXPECT_THAT(FormatPack(UntypedFormatSpecImpl("%14$p"), args), MatchesPointerString(nullptr));
}
...
template <typename T> class TypedFormatConvertTest : public FormatConvertTest { };
            ...
            for (size_t fsi = 0; fsi < flag_sets.size(); ++fsi) {
                ...
                for (size_t wi = 0; wi < ArraySize(kWid); ++wi) {
                    ...
                    for (size_t pi = 0; pi < ArraySize(kPrec); ++pi) {
                        ...
                        const bool is_unsigned_to_signed = !std::is_signed<T>::value && is_signed_conv;
                        // Don't consider sign-related flags '+' and ' ' when doing
                        // unsigned to signed conversions.
                        if (is_unsigned_to_signed && flag_set.find_first_of("+ ") != std::string::npos) {
                            continue;
                        }
                        ...
                        if (is_unsigned_to_signed) {
                            ...
                            UnsignedT uval = static_cast<std::remove_volatile_t<UnsignedT>>(val);
                            old_fmt += LengthModFor<MatchingIntegralType<std::remove_cv_t<decltype(uval)>>>();
                            old_fmt += "u";
                            old_result = StrPrint(old_fmt.c_str(), uval);
                        } else {
                            old_fmt += LengthModFor<MatchingIntegralType<std::remove_cv_t<decltype(val)>>>();
                            old_fmt += conv_char;
...
template <typename T> absl::optional<std::string> StrPrintChar(T c)
{
    return StrPrint("%c", static_cast<int>(c));
}
template <> absl::optional<std::string> StrPrintChar(wchar_t c)
{
...
template <typename T> typename std::remove_volatile<T>::type GetMaxForConversion()
{
    return static_cast<typename std::remove_volatile<T>::type>(std::numeric_limits<int>::max());
}

template <> wchar_t GetMaxForConversion<wchar_t>()
{
    ...
    return (sizeof(wchar_t) * CHAR_BIT <= 16) ? wchar_t { 0xffff } : static_cast<wchar_t>(0x10ffff);
}
...
TYPED_TEST_P(TypedFormatConvertTest, Char)
{
    ...
    using ArgType = std::conditional_t<std::is_same<T, wchar_t>::value, wchar_t, int>;
    static const T kMin = static_cast<remove_volatile_t>(std::numeric_limits<ArgType>::min());
    static const T kMax = GetMaxForConversion<T>();
    vals.insert(vals.end(), { static_cast<remove_volatile_t>(kMin + 1), kMin, static_cast<remove_volatile_t>(kMax - 1), kMax });

    static const auto kMaxWCharT = static_cast<remove_volatile_t>(GetMaxForConversion<wchar_t>());
    for (const T c : vals) {
        SCOPED_TRACE(Esc(c));
        ...
        const T wc = std::max(remove_volatile_t { 0 }, std::min(static_cast<remove_volatile_t>(c), kMaxWCharT));
        SCOPED_TRACE(Esc(wc));
        ...
        if (result.has_value()) {
            EXPECT_EQ(result.value(), FormatPack(wide_format, absl::MakeSpan(wide_args)));
        }
    }
}
...
typedef ::testing::Types<int, unsigned, volatile int, short, // NOLINT
    ...
    AllIntTypes;
INSTANTIATE_TYPED_TEST_SUITE_P(TypedFormatConvertTestWithAllIntTypes, TypedFormatConvertTest, AllIntTypes);
TEST_F(FormatConvertTest, VectorBool)
{
    ...
    EXPECT_EQ("1,0,1,0",
        FormatPack(UntypedFormatSpecImpl("%d,%d,%d,%d"),
            absl::Span<const FormatArgImpl>({ FormatArgImpl(v[0]), FormatArgImpl(v[1]), FormatArgImpl(cv[0]), FormatArgImpl(cv[1]) })));
}
...
TEST_F(FormatConvertTest, UnicodeWideString)
{
    ...
    EXPECT_EQ(output, FormatPack(UntypedFormatSpecImpl("%ls"), absl::MakeSpan(args)));
}
...
TEST_F(FormatConvertTest, Int128)
{
    ...
    const FormatArgImpl args[] = { FormatArgImpl(positive), FormatArgImpl(negative), FormatArgImpl(max), FormatArgImpl(min) };
...
template <typename Floating> void TestWithMultipleFormatsHelper(Floating tested_float)
{
    ...
    const char* const kFormats[]
        = { "%", "%.3", "%8.5", "%500", "%.5000", "%.60", "%.30", "%03", "%+", "% ", "%-10", "%#15.3", "%#.0", "%.0", "%1$*2$", "%1$.*2$" };
    ...
    for (const char* fmt : kFormats) {
        for (char f : { 'f', 'F', //
            ...
            if (fmt == absl::string_view("%.5000") && f != 'f' && f != 'F' && f != 'a' && f != 'A') {
                // This particular test takes way too long with snprintf.
                // Disable for the case we are not implementing natively.
                continue;
            }

            if ((f == 'a' || f == 'A') && !native_traits.hex_float_has_glibc_rounding) {
                continue;
            }

            if (!native_traits.hex_float_prefers_denormal_repr && (f == 'a' || f == 'A') && std::fpclassify(tested_float) == FP_SUBNORMAL) {
                continue;
            }
            int i = -10;
            ...
            ASSERT_EQ(string_printf_result, str_format_result)
                << fmt_str << " " << StrPrint("%.18g", tested_float) << " " << StrPrint("%a", tested_float) << " " << StrPrint("%.50f", tested_float);
        }
    }
}
...
{
    std::vector<float> floats = { 0.0f, -0.0f, .9999999f, 9999999.f, std::numeric_limits<float>::max(), -std::numeric_limits<float>::max(),
        std::numeric_limits<float>::min(), -std::numeric_limits<float>::min(), std::numeric_limits<float>::lowest(), -std::numeric_limits<float>::lowest(),
        std::numeric_limits<float>::epsilon(), std::numeric_limits<float>::epsilon() + 1.0f, std::numeric_limits<float>::infinity(),
    ...
    for (float base :
        { 1.f, 12.f, 123.f, 1234.f, 12345.f, 123456.f, 1234567.f, 12345678.f, 123456789.f, 1234567890.f, 12345678901.f, 12345678.f, 12345678.f }) {
...
{
    std::vector<double> doubles = { 0.0, -0.0, .99999999999999, 99999999999999., std::numeric_limits<double>::max(), -std::numeric_limits<double>::max(),
        std::numeric_limits<double>::min(), -std::numeric_limits<double>::min(), std::numeric_limits<double>::lowest(), -std::numeric_limits<double>::lowest(),
        std::numeric_limits<double>::epsilon(), std::numeric_limits<double>::epsilon() + 1, std::numeric_limits<double>::infinity(),
    ...
    for (double base : { 1., 12., 123., 1234., 12345., 123456., 1234567., 12345678., 123456789., 1234567890., 12345678901., 123456789012., 1234567890123. }) {
        for (int exp = -123; exp <= 123; ++exp) {
            for (int sign : { 1, -1 }) {
...
TEST_F(FormatConvertTest, DoubleRound)
{
    ...
    // Nine handling
    EXPECT_EQ(format("%.35f", exact_value), "0.00000000000005684341886080801486969");
    EXPECT_EQ(format("%.36f", exact_value), "0.000000000000056843418860808014869690");
    // Round down the last nine.
    EXPECT_EQ(format("%.37f", exact_value), "0.0000000000000568434188608080148696899");
    ...
    // Round to even (down)
    EXPECT_EQ(format("%.43f", exact_value), "0.0000000000000568434188608080148696899414062");
    // Exact
    EXPECT_EQ(format("%.44f", exact_value), "0.00000000000005684341886080801486968994140625");
    // Round to even (up), let make the last digits 75 instead of 25
    EXPECT_EQ(format("%.43f", exact_value + std::pow(2, -43)), "0.0000000000001705302565824240446090698242188");
    // Exact, just to check.
    EXPECT_EQ(format("%.44f", exact_value + std::pow(2, -43)), "0.00000000000017053025658242404460906982421875");
    ...
    // Round up at a 5xx.
    EXPECT_EQ(format("%.37f", small_exact_value), "0.0000000000000000000000000000000000008");
    // Round down at a <5
    EXPECT_EQ(format("%.38f", small_exact_value), "0.00000000000000000000000000000000000075");
    // Round up at a >5
    EXPECT_EQ(format("%.41f", small_exact_value), "0.00000000000000000000000000000000000075232");
    // Nine handling
    EXPECT_EQ(format("%.55f", small_exact_value), "0.0000000000000000000000000000000000007523163845262640051");
    EXPECT_EQ(format("%.56f", small_exact_value), "0.00000000000000000000000000000000000075231638452626400510");
    EXPECT_EQ(format("%.57f", small_exact_value), "0.000000000000000000000000000000000000752316384526264005100");
    EXPECT_EQ(format("%.58f", small_exact_value), "0.0000000000000000000000000000000000007523163845262640051000");
    // Round down the last nine
    EXPECT_EQ(format("%.59f", small_exact_value), "0.00000000000000000000000000000000000075231638452626400509999");
    // Round up the last nine
...
TEST_F(FormatConvertTest, LongDoubleRoundA)
{
    ...
    const auto format = [&](const char* fmt, long double d) -> std::string& {
        ...
        FormatArgImpl args[1] = { FormatArgImpl(d) };
        AppendPack(&s, UntypedFormatSpecImpl(fmt), absl::MakeSpan(args));
        if (native_traits.hex_float_has_glibc_rounding && native_traits.hex_float_optimizes_leading_digit_bit_count) {
            EXPECT_EQ(StrPrint(fmt, d), s);
        }
        return s;
    };
...
template <typename... T> bool FormatWithNullSink(absl::string_view fmt, const T&... a)
{
    NullSink sink;
...
{
    const NativePrintfTraits& native_traits = VerifyNativeImplementation();
    const char* const kFormats[] = { "%", "%.3", "%8.5", "%9", "%.5000", "%.60", "%+", "% ", "%-10" };

    std::vector<long double> doubles
        = { 0.0, -0.0, std::numeric_limits<long double>::max(), -std::numeric_limits<long double>::max(), std::numeric_limits<long double>::min(),
              -std::numeric_limits<long double>::min(), std::numeric_limits<long double>::infinity(), -std::numeric_limits<long double>::infinity() };

    for (long double base :
        { 1.L, 12.L, 123.L, 1234.L, 12345.L, 123456.L, 1234567.L, 12345678.L, 123456789.L, 1234567890.L, 12345678901.L, 123456789012.L, 1234567890123.L,
        ...
        for (char f : { 'f', 'F', //
            ...
            for (auto d : doubles) {
                ...
                ASSERT_EQ(StrPrint(fmt_str.c_str(), d), result)
                    << fmt_str << " " << StrPrint("%.18Lg", d) << " " << StrPrint("%La", d) << " " << StrPrint("%.1080Lf", d);
            }
        }
    }
}
...
TEST_F(FormatConvertTest, IntAsDouble)
{
    ...
    const int ia[] = { 1, 2, 3, 123, -1, -2, -3, -123, 0, kMax - 1, kMax, kMin + 1, kMin };
    for (const int fx : ia) {
        SCOPED_TRACE(fx);
...
template <typename T> bool FormatFails(const char* test_format, T value)
{
    ...
    EXPECT_EQ(FormatPack(format, absl::MakeSpan(args)), "") << "format=" << test_format << " value=" << value;
