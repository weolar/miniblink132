...
// the carry.
// Requires: `0 <= carry <= 9`
template <typename Int> inline char MultiplyBy10WithCarry(Int* v, char carry)
{
    using BiggerInt = absl::conditional_t<sizeof(Int) == 4, uint64_t, uint128>;
    BiggerInt tmp = 10 * static_cast<BiggerInt>(*v) + static_cast<BiggerInt>(carry);
...
public:
    ...
    static void RunConversion(uint128 v, int exp, absl::FunctionRef<void(BinaryToDecimal)> f)
    {
        ...
        static_assert(StackArray::kMaxCapacity >= ChunksNeeded(std::numeric_limits<MaxFloatType>::max_exponent), "");
    ...
    {
        return (decimal_end_ - decimal_start_) * kDigitsPerChunk + CurrentDigits().size();
    }
    ...
    bool AdvanceDigits()
    {
        if (decimal_start_ >= decimal_end_)
            return false;
...
private:
    BinaryToDecimal(absl::Span<uint32_t> data, uint128 v, int exp)
        : data_(data)
    {
...
public:
    ...
    static void RunConversion(uint128 v, int exp, absl::FunctionRef<void(FractionalDigitGenerator)> f)
    {
        ...
        StackArray::RunWithCapacity(
            static_cast<size_t>((Limits::digits + exp + 31) / 32), [=](absl::Span<uint32_t> input) { f(FractionalDigitGenerator(input, v, exp)); });
    }

    // Returns true if there are any more non-zero digits left.
    bool HasMoreDigits() const
    {
        return next_digit_ != 0 || after_chunk_index_;
    }
    ...
    {
        return next_digit_ == 5 && !after_chunk_index_;
    }
...
private:
    ...
    FractionalDigitGenerator(absl::Span<uint32_t> data, uint128 v, int exp)
        : after_chunk_index_(static_cast<size_t>(exp / 32 + 1))
        , data_(data)
    {
        const int offset = exp % 32;
...
char* PrintFractionalDigitsFast(uint64_t v, char* start, int exp, size_t precision)
{
    char* p = start;
    v <<= (64 - exp);
    while (precision > 0) {
...
// Performs rounding if necessary to fit within `precision`.
// Returns the pointer to one after the last character written.
char* PrintFractionalDigitsFast(uint128 v, char* start, int exp, size_t precision)
{
    char* p = start;
    v <<= (128 - exp);
    auto high = static_cast<uint64_t>(v >> 64);
...
{
    if (state.conv.width() < 0 || static_cast<size_t>(state.conv.width()) <= total_size) {
        return { 0, 0, 0 };
    }
...
void FinalPrint(const FormatState& state, absl::string_view data, size_t padding_offset, size_t trailing_zeros, absl::string_view data_postfix)
{
    ...
    auto padding = ExtraWidthToPadding((state.sign_char != '\0' ? 1 : 0) + data.size() + data_postfix.size() + trailing_zeros, state);
...
template <typename Int> void FormatFFast(Int v, int exp, const FormatState& state)
{
    ...
    if (exp >= 0) {
        ...
        integral_digits_start = total_bits <= 64 ? PrintIntegralDigitsFromRightFast(static_cast<uint64_t>(v) << exp, integral_digits_end)
                                                 : PrintIntegralDigitsFromRightFast(static_cast<uint128>(v) << exp, integral_digits_end);
    } else {
        exp = -exp;

        integral_digits_start = PrintIntegralDigitsFromRightFast(exp < input_bits ? v >> exp : 0, integral_digits_end);
        // PrintFractionalDigits may pull a carried 1 all the way up through the
        // integral portion.
        integral_digits_start[-1] = '0';

        fractional_digits_end = exp <= 64 ? PrintFractionalDigitsFast(v, fractional_digits_start, exp, state.precision)
                                          : PrintFractionalDigitsFast(static_cast<uint128>(v), fractional_digits_start, exp, state.precision);
        // There was a carry, so include the first digit too.
        if (integral_digits_start[-1] != '0')
            --integral_digits_start;
    }

    size_t size = static_cast<size_t>(fractional_digits_end - integral_digits_start);
    ...
    FinalPrint(state, absl::string_view(integral_digits_start, size),
        /*padding_offset=*/0, state.precision - static_cast<size_t>(fractional_digits_end - fractional_digits_start),
        /*data_postfix=*/"");
}
...
void FormatFPositiveExpSlow(uint128 v, int exp, const FormatState& state)
{
    BinaryToDecimal::RunConversion(v, exp, [&](BinaryToDecimal btd) {
        const size_t total_digits = btd.TotalDigits() + (state.ShouldPrintDot() ? state.precision + 1 : 0);

        const auto padding = ExtraWidthToPadding(total_digits + (state.sign_char != '\0' ? 1 : 0), state);
...
void FormatFNegativeExpSlow(uint128 v, int exp, const FormatState& state)
{
    ...
    auto padding = ExtraWidthToPadding(total_digits + (state.sign_char ? 1 : 0), state);
    padding.zeros += 1;
    state.sink->Append(padding.left_spaces, ' ');
    if (state.sign_char != '\0')
        state.sink->Append(1, state.sign_char);
    ...
    FractionalDigitGenerator::RunConversion(v, exp, [&](FractionalDigitGenerator digit_gen) {
            ...
            if (digits.num_nines + 1 < digits_to_go) {
                ...
                if (digits.num_nines + 1 > digits_to_go) {
                    ...
                    if (digit_gen.IsGreaterThanHalf()) {
                        ...
                        // Round to even
                        round_up = digits.num_nines != 0 || digits.digit_before_nine % 2 == 1;
                    }
                }
...
template <typename Int> void FormatF(Int mantissa, int exp, const FormatState& state)
{
    if (exp >= 0) {
        const int total_bits = static_cast<int>(sizeof(Int) * 8) - LeadingZeros(mantissa) + exp;
...
// bits 4-7.
template <typename Int> uint8_t GetNibble(Int n, size_t nibble_index)
{
...
template <typename Int> bool IncrementNibble(size_t nibble_index, Int* n)
{
    ...
    *n += ((nibble_index >= kNumNibbles) ? 0 : (Int { 1 } << static_cast<int>(nibble_index * 4)));
    Int after = *n >> kShift;
    return (before && !after) || (nibble_index >= kNumNibbles);
}

// Return a mask with 1's in the given nibble and all lower nibbles.
template <typename Int> Int MaskUpToNibbleInclusive(size_t nibble_index)
{
    constexpr size_t kNumNibbles = sizeof(Int) * 8 / 4;
    static const Int ones = ~Int { 0 };
    ++nibble_index;
    return ones >> static_cast<int>(4 * (std::max(kNumNibbles, nibble_index) - nibble_index));
}

// Return a mask with 1's below the given nibble.
template <typename Int> Int MaskUpToNibbleExclusive(size_t nibble_index)
{
    return nibble_index == 0 ? 0 : MaskUpToNibbleInclusive<Int>(nibble_index - 1);
}

template <typename Int> Int MoveToNibble(uint8_t nibble, size_t nibble_index)
{
...
template <typename Float> constexpr size_t HexFloatLeadingDigitSizeInBits()
{
    return std::numeric_limits<Float>::digits % 4 > 0 ? static_cast<size_t>(std::numeric_limits<Float>::digits % 4) : size_t { 4 };
}
...
template <typename Int> bool HexFloatNeedsRoundUp(Int mantissa, size_t final_nibble_displayed, uint8_t leading)
{
    ...
    Int mantissa_up_to_rounding_nibble_inclusive = mantissa & MaskUpToNibbleInclusive<Int>(rounding_nibble_idx);
    ...
    uint8_t round_if_odd = (final_nibble_displayed == kTotalNibbles) ? leading : GetNibble(mantissa, final_nibble_displayed);
...
template <typename Int> void FormatARound(bool precision_specified, const FormatState& state, uint8_t* leading, Int* mantissa, int* exp)
{
    constexpr size_t kTotalNibbles = sizeof(Int) * 8 / 4;
    // Index of the last nibble that we could display given precision.
    size_t final_nibble_displayed = precision_specified ? (std::max(kTotalNibbles, state.precision) - state.precision) : 0;
...
template <typename Int> void FormatANormalize(const HexFloatTypeParams float_traits, uint8_t* leading, Int* mantissa, int* exp)
{
    ...
    // fractional part.
    *leading = static_cast<uint8_t>(*mantissa >> static_cast<int>(kIntBits - kLeadDigitBitsCount));
    *exp -= (*mantissa != 0) ? static_cast<int>(kLeadDigitBitsCount) : *exp;
    *mantissa <<= static_cast<int>(kLeadDigitBitsCount);
}

template <typename Int> void FormatA(const HexFloatTypeParams float_traits, Int mantissa, int exp, bool uppercase, const FormatState& state)
{
    // Int properties.
    constexpr size_t kIntBits = sizeof(Int) * 8;
    constexpr size_t kTotalNibbles = sizeof(Int) * 8 / 4;
    ...
    constexpr size_t kBufSizeForHexFloatRepr = 2 // 0x
    ...
    *digits_iter++ = digits[leading];
    ...
    if ((precision_specified && state.precision > 0) || (!precision_specified && mantissa > 0) || state.conv.has_alt_flag()) {
        *digits_iter++ = '.';
    }
    ...
    auto digits_result = string_view(digits_buffer, static_cast<size_t>(digits_iter - digits_buffer));

    // =============== Exponent ==================
    constexpr size_t kBufSizeForExpDecRepr = numbers_internal::kFastToBufferSize // required for FastIntToBuffer
        + 1 // 'p' or 'P'
        + 1; // '+' or '-'
...
template <typename Float> bool FallbackToSnprintf(const Float v, const FormatConversionSpecImpl& conv, FormatSinkImpl* sink)
{
...
template <typename Float> bool ConvertNonNumericFloats(char sign_char, Float v, const FormatConversionSpecImpl& conv, FormatSinkImpl* sink)
{
    char text[4], *ptr = text;
    if (sign_char != '\0')
        *ptr++ = sign_char;
    if (std::isnan(v)) {
        ptr = std::copy_n(FormatConversionCharIsUpper(conv.conversion_char()) ? "NAN" : "nan", 3, ptr);
    } else if (std::isinf(v)) {
        ptr = std::copy_n(FormatConversionCharIsUpper(conv.conversion_char()) ? "INF" : "inf", 3, ptr);
    } else {
        return false;
    }

    return sink->PutPaddedString(string_view(text, static_cast<size_t>(ptr - text)), conv.width(), -1, conv.has_left_flag());
}

// Round up the last digit of the value.
// It will carry over and potentially overflow. 'exp' will be adjusted in that
// case.
template <FormatStyle mode> void RoundUp(Buffer* buffer, int* exp)
{
    char* p = &buffer->back();
...
template <typename Float, typename Int> constexpr bool CanFitMantissa()
{
    return
#if defined(__clang__) && (__clang_major__ < 9) && !defined(__SSE3__)
        // Workaround for clang bug: https://bugs.llvm.org/show_bug.cgi?id=38289
        // Casting from long double to uint64_t is miscompiled and drops bits.
        (!std::is_same<Float, long double>::value || !std::is_same<Int, uint64_t>::value) &&
#endif
        std::numeric_limits<Float>::digits <= std::numeric_limits<Int>::digits;
}

template <typename Float> struct Decomposed {
    using MantissaType = absl::conditional_t<std::is_same<long double, Float>::value, uint128, uint64_t>;
...
template <typename Float> Decomposed<Float> Decompose(Float v)
{
    int exp;
...
template <FormatStyle mode, typename Int> size_t PrintIntegralDigits(Int digits, Buffer* out)
{
    size_t printed = 0;
    if (digits) {
        for (; digits; digits /= 10)
            out->push_front(digits % 10 + '0');
        printed = out->size();
        if (mode == FormatStyle::Precision) {
...
// Precision mode.
template <typename Int, typename Float, FormatStyle mode> bool FloatToBufferImpl(Int int_mantissa, int exp, size_t precision, Buffer* out, int* exp_out)
{
        ...
        if (mode == FormatStyle::Precision) {
            *exp_out = static_cast<int>(digits_printed - 1);
            if (digits_to_zero_pad < digits_printed - 1) {
                RemoveExtraPrecision(digits_printed - 1 - digits_to_zero_pad, false, out, exp_out);
                return true;
            }
            digits_to_zero_pad -= digits_printed - 1;
        }
        for (; digits_to_zero_pad-- > 0;)
            out->push_back('0');
        return true;
    }
        ...
        if (digits_printed == 0) {
            ...
            *exp_out = static_cast<int>(digits_printed - 1);
            if (fractional_count < digits_printed - 1) {
                ...
                RemoveExtraPrecision(digits_printed - 1 - fractional_count, int_mantissa != 0, out, exp_out);
                return true;
            }
    ...
    if (next_digit > 5 || (next_digit == 5 && (int_mantissa || out->last_digit() % 2 == 1))) {
...
template <FormatStyle mode, typename Float> bool FloatToBuffer(Decomposed<Float> decomposed, size_t precision, Buffer* out, int* exp)
{
    ...
    if (CanFitMantissa<Float, std::uint64_t>()
        && FloatToBufferImpl<std::uint64_t, Float, mode>(static_cast<std::uint64_t>(decomposed.mantissa), decomposed.exponent, precision, out, exp))
        return true;

#if defined(ABSL_HAVE_INTRINSIC_INT128)
    // If that is not enough, try with __uint128_t.
    return CanFitMantissa<Float, __uint128_t>()
        && FloatToBufferImpl<__uint128_t, Float, mode>(static_cast<__uint128_t>(decomposed.mantissa), decomposed.exponent, precision, out, exp);
#endif
    return false;
}
...
void WriteBufferToSink(char sign_char, absl::string_view str, const FormatConversionSpecImpl& conv, FormatSinkImpl* sink)
{
    ...
    if (conv.width() >= 0) {
        ...
        const size_t existing_chars = str.size() + static_cast<size_t>(sign_char != 0);
...
template <typename Float> bool FloatToSink(const Float v, const FormatConversionSpecImpl& conv, FormatSinkImpl* sink)
{
    ...
    size_t precision = conv.precision() < 0 ? 6 : static_cast<size_t>(conv.precision());
    ...
    } else if (c == FormatConversionCharInternal::a || c == FormatConversionCharInternal::A) {
    ...
    WriteBufferToSink(sign_char, absl::string_view(buffer.begin, static_cast<size_t>(buffer.end - buffer.begin)), conv, sink);
