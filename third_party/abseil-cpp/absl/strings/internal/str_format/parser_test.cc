...
public:
    std::pair<string_view, string_view> Consume(string_view src)
    {
        ...
        const char* p = ConsumeUnboundConversion(src.data(), src.data() + src.size(), &o, &next);
        if (!p)
            return { {}, src };
        return { string_view(src.data(), p - src.data()), string_view(p, src.data() + src.size() - p) };
    }
    ...
    bool Run(const char* fmt, bool force_positional = false)
    {
        ...
        o = UnboundConversion(); // refresh
        return ConsumeUnboundConversion(fmt, fmt + strlen(fmt), &o, &next) == fmt + strlen(fmt);
    }
    UnboundConversion o;
};
...
TEST_F(ConsumeUnboundConversionTest, ConsumeSpecification)
{
    ...
    const Expectation kExpect[] = {
        { __LINE__, "", "", "" }, { __LINE__, "b", "", "b" }, // 'b' is invalid
...
TEST_F(ConsumeUnboundConversionTest, Flags)
{
    ...
    for (int rev = 0; rev < 2; ++rev) {
        for (int i = 0; i < 1 << kNumFlags; ++i) {
            ...
            EXPECT_EQ(fmt.find('-') == std::string::npos, !FlagsContains(o.flags, Flags::kLeft));
            EXPECT_EQ(fmt.find('+') == std::string::npos, !FlagsContains(o.flags, Flags::kShowPos));
            EXPECT_EQ(fmt.find(' ') == std::string::npos, !FlagsContains(o.flags, Flags::kSignCol));
            EXPECT_EQ(fmt.find('#') == std::string::npos, !FlagsContains(o.flags, Flags::kAlt));
            EXPECT_EQ(fmt.find('0') == std::string::npos, !FlagsContains(o.flags, Flags::kZero));
        }
    }
}
...
TEST_F(ParsedFormatTest, ValueSemantics)
{
    ...
    p1 = ParsedFormatBase("hello%s", true, { FormatConversionCharSetInternal::s }); // move assign
...
TEST_F(ParsedFormatTest, Parsing)
{
    ...
    const ExpectParse kExpect[] = {
        { "", {}, "" }, { "ab", {}, "[ab]" }, { "a%d", { FormatConversionCharSetInternal::d }, "[a]{d:1$d}" },
        { "a%+d", { FormatConversionCharSetInternal::d }, "[a]{+d:1$d}" }, { "a% d", { FormatConversionCharSetInternal::d }, "[a]{ d:1$d}" },
        { "a%b %d", {}, "[a]!" }, // stop after error
    };
    for (const auto& e : kExpect) {
        SCOPED_TRACE(e.in);
        EXPECT_EQ(e.out, SummarizeParsedFormat(ParsedFormatBase(e.in, false, e.conv_set)));
    }
}
