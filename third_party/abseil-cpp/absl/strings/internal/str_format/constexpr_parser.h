...
constexpr bool CheckFastPathSetting(const UnboundConversion& conv)
{
    ...
    if (width_precision_needed && conv.flags == Flags::kBasic) {
        ...
        fprintf(stderr,
            ...
            "width=%d precision=%d\n",
            conv.flags == Flags::kBasic ? 1 : 0, FlagsContains(conv.flags, Flags::kLeft) ? 1 : 0, FlagsContains(conv.flags, Flags::kShowPos) ? 1 : 0,
            FlagsContains(conv.flags, Flags::kSignCol) ? 1 : 0, FlagsContains(conv.flags, Flags::kAlt) ? 1 : 0, FlagsContains(conv.flags, Flags::kZero) ? 1 : 0,
...
template <bool is_positional> constexpr const char* ConsumeConversion(const char* pos, const char* const end, UnboundConversion* conv, int* next_arg)
{
