...
public:
    ...
    template <typename T> void PrintAsOct(T v)
    {
    ...
    template <typename T> void PrintAsDec(T v)
    {
        ...
        start_ = storage_;
        size_ = static_cast<size_t>(numbers_internal::FastIntToBuffer(v, storage_) - storage_);
    }
...
string_view BaseIndicator(const IntDigits& as_digits, const FormatConversionSpecImpl conv)
{
    // always show 0x for %p.
    bool alt = conv.has_alt_flag() || conv.conversion_char() == FormatConversionCharInternal::p;
    bool hex = (conv.conversion_char() == FormatConversionCharInternal::x || conv.conversion_char() == FormatConversionCharInternal::X
        || conv.conversion_char() == FormatConversionCharInternal::p);
    // From the POSIX description of '#' flag:
    //   "For x or X conversion specifiers, a non-zero result shall have
    //   0x (or 0X) prefixed to it."
    if (alt && hex && !as_digits.without_neg_or_zero().empty()) {
        return conv.conversion_char() == FormatConversionCharInternal::X ? "0X" : "0x";
    }
    return {};
}
...
{
    if (conv.conversion_char() == FormatConversionCharInternal::d || conv.conversion_char() == FormatConversionCharInternal::i) {
        if (neg)
            return "-";
...
bool ConvertCharImpl(char v, const FormatConversionSpecImpl conv, FormatSinkImpl* sink)
{
    size_t fill = 0;
    if (conv.width() >= 0)
...
bool ConvertIntImplInnerSlow(const IntDigits& as_digits, const FormatConversionSpecImpl conv, FormatSinkImpl* sink)
{
    ...
    size_t precision = precision_specified ? static_cast<size_t>(conv.precision()) : size_t { 1 };

    if (conv.has_alt_flag() && conv.conversion_char() == FormatConversionCharInternal::o) {
...
template <typename T> bool ConvertFloatArg(T v, FormatConversionSpecImpl conv, FormatSinkImpl* sink)
{
    if (conv.conversion_char() == FormatConversionCharInternal::v) {
        conv.set_conversion_char(FormatConversionCharInternal::g);
    }

    return FormatConversionCharIsFloat(conv.conversion_char()) && ConvertFloatImpl(v, conv, sink);
}
...
inline bool ConvertStringArg(string_view v, const FormatConversionSpecImpl conv, FormatSinkImpl* sink)
{
    ...
    return sink->PutPaddedString(v, conv.width(), conv.precision(), conv.has_left_flag());
}
...
inline size_t WideToUtf8(wchar_t wc, char* buf, ShiftState& s)
{
    ...
    if (v < 0x80) {
        ...
        *buf = static_cast<char>(0x80 | static_cast<uint8_t>((high_bits & 0x3) << 4) | static_cast<uint8_t>((v >> 2) & 0xf));
        return 2;
    } else if (v < 0xe000 && s.saw_high_surrogate) {
        *buf++ = static_cast<char>(0x80 | static_cast<uint8_t>(s.bits << 4) | static_cast<uint8_t>((v >> 6) & 0xf));
        *buf = static_cast<char>(0x80 | (v & 0x3f));
...
inline bool ConvertStringArg(const wchar_t* v, size_t len, const FormatConversionSpecImpl conv, FormatSinkImpl* sink)
{
...
bool ConvertWCharTImpl(wchar_t v, const FormatConversionSpecImpl conv, FormatSinkImpl* sink)
{
    char mb[4];
    ShiftState s;
    const size_t chars_written = WideToUtf8(v, mb, s);
    return chars_written != static_cast<size_t>(-1) && !s.saw_high_surrogate && ConvertStringArg(string_view(mb, chars_written), conv, sink);
}
...
template <typename T> bool ConvertIntArg(T v, FormatConversionSpecImpl conv, FormatSinkImpl* sink)
{
    ...
    case static_cast<uint8_t>(FormatConversionCharInternal::c):
        return (std::is_same<T, wchar_t>::value || (conv.length_mod() == LengthMod::l)) ? ConvertWCharTImpl(static_cast<wchar_t>(v), conv, sink)
...
template bool ConvertIntArg<char>(char v, FormatConversionSpecImpl conv, FormatSinkImpl* sink);
template bool ConvertIntArg<signed char>(signed char v, FormatConversionSpecImpl conv, FormatSinkImpl* sink);
template bool ConvertIntArg<unsigned char>(unsigned char v, FormatConversionSpecImpl conv, FormatSinkImpl* sink);
template bool ConvertIntArg<wchar_t>(wchar_t v, FormatConversionSpecImpl conv, FormatSinkImpl* sink);
template bool ConvertIntArg<short>(short v, // NOLINT
    FormatConversionSpecImpl conv, FormatSinkImpl* sink);
template bool ConvertIntArg<unsigned short>(unsigned short v, // NOLINT
    FormatConversionSpecImpl conv, FormatSinkImpl* sink);
template bool ConvertIntArg<int>(int v, FormatConversionSpecImpl conv, FormatSinkImpl* sink);
template bool ConvertIntArg<unsigned int>(unsigned int v, FormatConversionSpecImpl conv, FormatSinkImpl* sink);
...
StringConvertResult FormatConvertImpl(const std::wstring& v, const FormatConversionSpecImpl conv, FormatSinkImpl* sink)
{
...
#if defined(ABSL_HAVE_STD_STRING_VIEW)
StringConvertResult FormatConvertImpl(std::wstring_view v, const FormatConversionSpecImpl conv, FormatSinkImpl* sink)
{
    return { ConvertStringArg(v.data(), v.size(), conv, sink) };
}
#endif

StringPtrConvertResult FormatConvertImpl(const char* v, const FormatConversionSpecImpl conv, FormatSinkImpl* sink)
{
...
StringPtrConvertResult FormatConvertImpl(const wchar_t* v, const FormatConversionSpecImpl conv, FormatSinkImpl* sink)
{
    if (conv.conversion_char() == FormatConversionCharInternal::p) {
        return { FormatConvertImpl(VoidPtr(v), conv, sink).value };
    }
...
StringPtrConvertResult FormatConvertImpl(std::nullptr_t, const FormatConversionSpecImpl conv, FormatSinkImpl* sink)
{
...
// ==================== Chars ====================
CharConvertResult FormatConvertImpl(char v, const FormatConversionSpecImpl conv, FormatSinkImpl* sink)
{
    return { ConvertIntArg(v, conv, sink) };
}
CharConvertResult FormatConvertImpl(wchar_t v, const FormatConversionSpecImpl conv, FormatSinkImpl* sink)
{
    return { ConvertIntArg(v, conv, sink) };
}
