...
inline cord_internal::CordRepSubstring* MakeSubstring(size_t start, size_t len, cord_internal::CordRep* rep)
{
...
inline cord_internal::CordRepExternal* MakeExternal(absl::string_view s)
{
    struct Rep : public cord_internal::CordRepExternal {
            ...
            : s(sv)
        {
            ...
            this->releaser_invoker = [](cord_internal::CordRepExternal* self) { delete static_cast<Rep*>(self); };
        }
    };
    return new Rep(s);
}
...
inline std::vector<cord_internal::CordRep*> CreateFlatsFromString(absl::string_view data, size_t chunk_size)
{
    assert(chunk_size > 0);
...
inline cord_internal::CordRepBtree* CordRepBtreeFromFlats(absl::Span<cord_internal::CordRep* const> flats)
{
    assert(!flats.empty());
...
template <typename Fn> inline void CordVisitReps(cord_internal::CordRep* rep, Fn&& fn)
{
    fn(rep);
...
template <typename Predicate> inline std::vector<cord_internal::CordRep*> CordCollectRepsIf(Predicate&& predicate, cord_internal::CordRep* rep)
{
...
inline std::vector<cord_internal::CordRep*> CordCollectReps(cord_internal::CordRep* rep)
{
    std::vector<cord_internal::CordRep*> reps;
...
public:
    ...
    template <typename CordRepType> CordRepType* RefIf(bool condition, CordRepType* rep)
    {
        if (condition)
            unrefs_.push_back(CordRep::Ref(rep));
        return rep;
