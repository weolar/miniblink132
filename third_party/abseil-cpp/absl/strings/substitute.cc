    ...
    absl::Nonnull<std::string*> output, absl::string_view format, absl::Nullable<const absl::string_view*> args_array, size_t num_args)
{
    ...
    ABSL_INTERNAL_CHECK(size <= std::numeric_limits<size_t>::max() - original_size, "size_t overflow");
...
Arg::Arg(absl::Nullable<const void*> value)
{
    ...
    if (value == nullptr) {
        ...
        *--ptr = 'x';
        *--ptr = '0';
        piece_ = absl::string_view(ptr, static_cast<size_t>(scratch_ + sizeof(scratch_) - ptr));
    }
}
