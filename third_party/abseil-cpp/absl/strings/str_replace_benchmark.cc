    ...
    if (big_string == nullptr) {
        ...
        for (std::string phrase : { "the quick brown fox jumped over the lazy dogs", "pack my box with the five dozen liquor jugs" }) {
            for (int i = 0; i < 10 * 1000; ++i) {
                ...
                memcpy(&(*big_string)[r % (big_string->size() - phrase.size())], phrase.data(), phrase.size());
            }
        }
        ...
        for (size_t pos = 0; (pos = after_replacing_the->find("the", pos)) != std::string::npos;) {
        ...
        for (size_t pos = 0;;) {
            ...
            if (next_pos > after_replacing_many->size())
                break;
            after_replacing_many->replace(next_pos, strlen(needle_string), replacement_string);
...
void BM_StrReplaceAllOneReplacement(benchmark::State& state)
{
    ...
    for (auto _ : state) {
        ...
        ABSL_RAW_CHECK(dest == *after_replacing_the, "not benchmarking intended behavior");
    }
}
...
void BM_StrReplaceAll(benchmark::State& state)
{
    ...
    std::string src = *big_string;
    for (auto _ : state) {
        std::string dest = absl::StrReplaceAll(
            src, { { "the", "box" }, { "brown", "quick" }, { "jumped", "liquored" }, { "dozen", "brown" }, { "lazy", "pack" }, { "liquor", "shakes" } });
        ABSL_RAW_CHECK(dest == *after_replacing_many, "not benchmarking intended behavior");
    }
}
