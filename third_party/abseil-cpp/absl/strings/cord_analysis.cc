...
template <Mode mode> struct CordRepRef {
    // Instantiates a CordRepRef instance.
    explicit CordRepRef(absl::Nonnull<const CordRep*> r)
        : rep(r)
    {
    }
...
template <Mode mode> struct RawUsage {
    size_t total = 0;
...
// by the same Cord.
template <> struct RawUsage<Mode::kTotalMorePrecise> {
    size_t total = 0;
...
template <typename refcount_t> double MaybeDiv(double d, refcount_t refcount)
{
...
template <> struct CordRepRef<Mode::kFairShare> {
...
template <> struct RawUsage<Mode::kFairShare> {
    double total = 0;
...
template <Mode mode> void AnalyzeDataEdge(CordRepRef<mode> rep, RawUsage<mode>& raw_usage)
{
    ...
    const size_t size = rep.rep->tag >= FLAT ? rep.rep->flat()->AllocatedSize() : rep.rep->length + sizeof(CordRepExternalImpl<intptr_t>);
    raw_usage.Add(size, rep);
}

// Computes the memory size of the provided Btree tree.
template <Mode mode> void AnalyzeBtree(CordRepRef<mode> rep, RawUsage<mode>& raw_usage)
{
...
template <Mode mode> size_t GetEstimatedUsage(absl::Nonnull<const CordRep*> rep)
{
