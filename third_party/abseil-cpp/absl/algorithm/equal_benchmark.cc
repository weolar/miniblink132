...
// padding).
struct EightBits {
    explicit EightBits(int /* unused */)
        : data(0)
    {
    }
    bool operator==(const EightBits& rhs) const
    {
...
template <typename T> void BM_absl_equal_benchmark(benchmark::State& state)
{
...
template <typename T> void BM_std_equal_benchmark(benchmark::State& state)
{
    std::vector<T> xs(state.range(0), T(0));
    std::vector<T> ys = xs;
    while (state.KeepRunning()) {
...
template <typename T> void BM_memcmp_benchmark(benchmark::State& state)
{
    std::vector<T> xs(state.range(0), T(0));
    std::vector<T> ys = xs;
    while (state.KeepRunning()) {
        const bool same = std::memcmp(xs.data(), ys.data(), xs.size() * sizeof(T)) == 0;
        benchmark::DoNotOptimize(same);
    }
}
...
template <typename T> void BM_absl_equal_self_benchmark(benchmark::State& state)
{
    std::vector<T> xs(state.range(0), T(0));
    while (state.KeepRunning()) {
...
BENCHMARK_TEMPLATE(BM_absl_equal_benchmark, uint8_t)->Range(kMinBenchmarkSize, kMaxBenchmarkSize);
...
BENCHMARK_TEMPLATE(BM_absl_equal_self_benchmark, uint8_t)->Range(kMinBenchmarkSize, kMaxBenchmarkSize);
...
BENCHMARK_TEMPLATE(BM_absl_equal_self_benchmark, uint16_t)->Range(kMinBenchmarkSize, kMaxBenchmarkSize);
...
BENCHMARK_TEMPLATE(BM_absl_equal_self_benchmark, uint32_t)->Range(kMinBenchmarkSize, kMaxBenchmarkSize);
...
BENCHMARK_TEMPLATE(BM_absl_equal_self_benchmark, uint64_t)->Range(kMinBenchmarkSize, kMaxBenchmarkSize);
...
BENCHMARK_TEMPLATE(BM_absl_equal_self_benchmark, EightBits)->Range(kMinBenchmarkSize, kMaxBenchmarkSize);
