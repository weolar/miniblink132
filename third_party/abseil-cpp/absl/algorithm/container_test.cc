...
bool BinPredicate(int v1, int v2)
{
    return v1 < v2;
}
...
{
    EXPECT_EQ(container_.size(), static_cast<size_t>(absl::c_distance(container_)));
    ...
    EXPECT_EQ(ABSL_ARRAYSIZE(array_), static_cast<size_t>(absl::c_distance(array_)));

    // Works with a temporary argument.
    EXPECT_EQ(vector_.size(), static_cast<size_t>(absl::c_distance(std::vector<int>(vector_))));
}
...
TEST_F(NonMutatingTest, ForEach)
{
    ...
    AccumulateCalls c2 = absl::c_for_each(std::unordered_set<int>(container_), AccumulateCalls());
...
TEST_F(NonMutatingTest, AdjacentFind)
{
    absl::c_adjacent_find(sequence_);
}
...
{
    EXPECT_EQ(1, absl::c_count(container_, 3));
}
...
TEST_F(NonMutatingTest, Mismatch)
{
    ...
    {
        struct NoNotEquals {
            constexpr bool operator==(NoNotEquals) const
            {
                return true;
            }
...
TEST_F(NonMutatingTest, SearchN)
{
    absl::c_search_n(sequence_, 3, 1);
}
...
{
    std::pair<std::list<int>::iterator, std::list<int>::iterator> p = absl::c_equal_range(sequence_, 2);
...
TEST_F(NonMutatingTest, EqualRangeWithPredicate)
{
    std::vector<int> v(vector_);
    std::sort(v.begin(), v.end(), std::greater<int>());
    std::pair<std::vector<int>::iterator, std::vector<int>::iterator> p = absl::c_equal_range(v, 2, std::greater<int>());
...
TEST_F(NonMutatingTest, BinarySearchWithPredicate)
{
    ...
    EXPECT_TRUE(absl::c_binary_search(std::vector<int>(v), 2, std::greater<int>()));
}
...
{
    std::list<int>::iterator i = absl::c_min_element(sequence_, std::greater<int>());
    ASSERT_TRUE(i != sequence_.end());
    EXPECT_EQ(*i, 3);
}
...
{
    std::list<int>::iterator i = absl::c_max_element(sequence_, std::greater<int>());
    ASSERT_TRUE(i != sequence_.end());
    EXPECT_EQ(*i, 1);
}
...
{
    EXPECT_FALSE(absl::c_lexicographical_compare(sequence_, sequence_, std::greater<int>()));
    ...
    EXPECT_TRUE(absl::c_lexicographical_compare(v, sequence_, std::greater<int>()));
    EXPECT_TRUE(absl::c_lexicographical_compare(std::vector<int>(v), std::list<int>(sequence_), std::greater<int>()));
}
...
{
    EXPECT_EQ(absl::c_accumulate(sequence_, 4, std::multiplies<int>()), 1 * 2 * 3 * 4);
}
...
{
    int lvalue = 4;
    EXPECT_EQ(absl::c_accumulate(sequence_, lvalue, std::multiplies<int>()), 1 * 2 * 3 * 4);
}

TEST_F(NonMutatingTest, InnerProduct)
{
    EXPECT_EQ(absl::c_inner_product(sequence_, vector_, 1000), 1000 + 1 * 1 + 2 * 2 + 3 * 3);
}

TEST_F(NonMutatingTest, InnerProductWithBinaryOps)
{
    EXPECT_EQ(absl::c_inner_product(sequence_, vector_, 10, std::multiplies<int>(), std::plus<int>()), 10 * (1 + 1) * (2 + 2) * (3 + 3));
}

TEST_F(NonMutatingTest, InnerProductLvalueInit)
{
    int lvalue = 1000;
    EXPECT_EQ(absl::c_inner_product(sequence_, vector_, lvalue), 1000 + 1 * 1 + 2 * 2 + 3 * 3);
}

TEST_F(NonMutatingTest, InnerProductWithBinaryOpsLvalueInit)
{
    int lvalue = 10;
    EXPECT_EQ(absl::c_inner_product(sequence_, vector_, lvalue, std::multiplies<int>(), std::plus<int>()), 10 * (1 + 1) * (2 + 2) * (3 + 3));
}
...
{
    auto last = absl::c_adjacent_difference(list_, std::back_inserter(output_), std::multiplies<int>());
    *last = 1000;
    std::vector<int> expected { 1, 2 * 1, 3 * 2, 1000 };
    EXPECT_EQ(output_, expected);
}
...
{
    auto last = absl::c_partial_sum(list_, std::back_inserter(output_), std::multiplies<int>());
    *last = 1000;
    std::vector<int> expected { 1, 1 * 2, 1 * 2 * 3, 1000 };
    EXPECT_EQ(output_, expected);
}
...
{
    std::pair<std::vector<int>::const_iterator, std::vector<int>::const_iterator> p = absl::c_minmax_element(vector_, std::less<int>());
...
{
    std::pair<std::vector<int>::const_iterator, std::vector<int>::const_iterator> p = absl::c_minmax_element(vector_, std::greater<int>());
...
{
    std::pair<std::vector<int>::const_iterator, std::vector<int>::const_iterator> p = absl::c_minmax_element(vector_);
    EXPECT_TRUE(p.first == vector_.begin());
    EXPECT_TRUE(p.second == vector_.begin() + 2);
}
...
{
    EXPECT_TRUE(absl::c_is_partitioned(std::vector<int> { 1, 3, 5, 2, 4, 6 }, IsOdd));
    EXPECT_FALSE(absl::c_is_partitioned(std::vector<int> { 1, 2, 3, 4, 5, 6 }, IsOdd));
    EXPECT_FALSE(absl::c_is_partitioned(std::vector<int> { 2, 4, 6, 1, 3, 5 }, IsOdd));
}
...
TEST(MutatingTest, Partition)
{
    ...
    EXPECT_THAT(actual, Truly([](const std::vector<int>& c) { return absl::c_is_partitioned(c, IsOdd); }));
}
...
TEST(MutatingTest, PartitionCopy)
{
    ...
    auto ends = absl::c_partition_copy(initial, back_inserter(odds), back_inserter(evens), IsOdd);
    *ends.first = 7;
    *ends.second = 6;
...
TEST(MutatingTest, CopyIf)
{
    ...
    absl::c_copy_if(input, std::back_inserter(output), [](int i) { return i != 2; });
...
TEST(MutatingTest, Move)
{
    ...
    EXPECT_THAT(dest, ElementsAre(Pointee(1), Pointee(2), Pointee(3), Pointee(4), Pointee(5)));
}
...
TEST(MutatingTest, MoveBackward)
{
    ...
    EXPECT_THAT(actual, ElementsAre(IsNull(), IsNull(), Pointee(1), Pointee(2), Pointee(3)));
}
...
TEST(MutatingTest, MoveWithRvalue)
{
    ...
    EXPECT_THAT(dest, ElementsAre(Pointee(1), Pointee(2), Pointee(3), Pointee(1), Pointee(2), Pointee(3)));
}
...
TEST(MutatingTest, StableSort)
{
    ...
    EXPECT_THAT(test_vector, ElementsAre(IsElement(1, 1), IsElement(1, 0), IsElement(2, 1), IsElement(2, 0), IsElement(2, 2)));
}

TEST(MutatingTest, StableSortWithPredicate)
{
    std::vector<Element> test_vector = { { 1, 1 }, { 2, 1 }, { 2, 0 }, { 1, 0 }, { 2, 2 } };
    absl::c_stable_sort(test_vector, [](const Element& e1, const Element& e2) { return e2 < e1; });
    EXPECT_THAT(test_vector, ElementsAre(IsElement(2, 1), IsElement(2, 0), IsElement(2, 2), IsElement(1, 1), IsElement(1, 0)));
}
...
{
    std::vector<int> actual;
    absl::c_remove_copy_if(std::vector<int> { 1, 2, 3 }, back_inserter(actual), IsOdd);
...
{
    std::vector<int> actual;
    absl::c_unique_copy(std::vector<int> { 1, 2, 2, 2, 3, 3, 2 }, back_inserter(actual));
...
{
    std::vector<int> actual;
    absl::c_unique_copy(std::vector<int> { 1, 2, 3, -1, -2, -3, 1 }, back_inserter(actual), [](int x, int y) { return (x < 0) == (y < 0); });
...
TEST(MutatingTest, RotateCopy)
{
    ...
    std::vector<int> actual;
    auto end = absl::c_rotate_copy(initial, initial.begin() + 2, back_inserter(actual));
    *end = 5;
    EXPECT_THAT(actual, ElementsAre(3, 4, 1, 2, 5));
}

template <typename T> T RandomlySeededPrng()
{
...
{
    std::vector<int> actual;
    absl::c_sample(std::vector<int> { 1, 2, 3, 4, 5 }, std::back_inserter(actual), 3, RandomlySeededPrng<std::mt19937_64>());
...
{
    std::vector<int> actual;
    absl::c_merge(std::vector<int> { 1, 3, 5 }, std::vector<int> { 2, 4 }, back_inserter(actual));
    EXPECT_THAT(actual, ElementsAre(1, 2, 3, 4, 5));
}
...
{
    std::vector<int> actual;
    absl::c_merge(std::vector<int> { 5, 3, 1 }, std::vector<int> { 4, 2 }, back_inserter(actual), std::greater<int>());
...
{
    std::vector<int> actual;
    absl::c_set_union(a_reversed_, b_reversed_, back_inserter(actual), std::greater<int>());
...
{
    std::vector<int> actual;
    absl::c_set_intersection(a_reversed_, b_reversed_, back_inserter(actual), std::greater<int>());
...
{
    std::vector<int> actual;
    absl::c_set_difference(a_reversed_, b_reversed_, back_inserter(actual), std::greater<int>());
    EXPECT_THAT(actual, ElementsAre(2));
}
...
{
    std::vector<int> actual;
    absl::c_set_symmetric_difference(a_reversed_, b_reversed_, back_inserter(actual), std::greater<int>());
...
{
    static constexpr std::array<int, 3> kArray = { 1, 2, 3 };
    constexpr auto kMinMaxPair = absl::c_minmax_element(kArray, std::greater<int>());
...
{
    static constexpr std::array<int, 3> kArray = { 1, 2, 3 };
    static_assert(absl::c_find_if(kArray, [](int x) { return x > 2; }) == kArray.begin() + 2);
    static_assert(absl::c_find_if(kArray, [](int x) { return x > 5; }) == kArray.end());
}

TEST(ConstexprTest, FindIfNot)
{
    static constexpr std::array<int, 3> kArray = { 1, 2, 3 };
    static_assert(absl::c_find_if_not(kArray, [](int x) { return x > 1; }) == kArray.begin());
    static_assert(absl::c_find_if_not(kArray, [](int x) { return x > 0; }) == kArray.end());
}
...
{
    static constexpr std::array<int, 3> kArray = { 1, 2, 3 };
    static_assert(absl::c_adjacent_find(kArray, std::less<int>()) == kArray.begin());
}
...
TEST(ConstexprTest, Mismatch)
{
    ...
    static_assert(absl::c_mismatch(kArray1, kArray2) == std::pair { kArray1.end(), kArray2.end() });
    static_assert(absl::c_mismatch(kArray1, kArray3) == std::pair { kArray1.begin(), kArray3.begin() });
}
...
TEST(ConstexprTest, MismatchWithPredicate)
{
    ...
    static constexpr std::array<int, 3> kArray2 = { 1, 2, 3 };
    static constexpr std::array<int, 3> kArray3 = { 2, 3, 4 };
    static_assert(absl::c_mismatch(kArray1, kArray2, std::not_equal_to<int>()) == std::pair { kArray1.begin(), kArray2.begin() });
    static_assert(absl::c_mismatch(kArray1, kArray3, std::not_equal_to<int>()) == std::pair { kArray1.end(), kArray3.end() });
}
...
TEST(ConstexprTest, IsPermutationWithPredicate)
{
    ...
    static_assert(!absl::c_is_permutation(kArray1, kArray3, std::equal_to<int>()));
}
...
TEST(ConstexprTest, SearchWithPredicate)
{
    ...
    static constexpr std::array<int, 3> kArray2 = { 1, 2, 3 };
    static constexpr std::array<int, 3> kArray3 = { 2, 3, 4 };
    static_assert(absl::c_search(kArray1, kArray2, std::not_equal_to<int>()) == kArray1.end());
    static_assert(absl::c_search(kArray1, kArray3, std::not_equal_to<int>()) == kArray1.begin());
}
...
TEST(ConstexprTest, ContainsSubrangeWithPredicate)
{
    ...
    static constexpr std::array<int, 3> kArray2 = { 1, 2, 3 };
    static constexpr std::array<int, 3> kArray3 = { 2, 3, 4 };
    static_assert(!absl::c_contains_subrange(kArray1, kArray2, std::not_equal_to<>()));
    static_assert(absl::c_contains_subrange(kArray1, kArray3, std::not_equal_to<>()));
}
...
{
    static constexpr std::array<int, 4> kArray = { 1, 2, 2, 3 };
    static_assert(absl::c_search_n(kArray, 1, 1, std::not_equal_to<int>()) == kArray.begin() + 1);
    static_assert(absl::c_search_n(kArray, 2, 2, std::not_equal_to<int>()) == kArray.end());
    static_assert(absl::c_search_n(kArray, 1, 4, std::not_equal_to<int>()) == kArray.begin());
}
