...
template <template <typename> class H, typename T> void RunBenchmark(benchmark::State& state, T value)
{
    H<T> h;
    for (auto _ : state) {
...
template <typename T> using AbslHash = absl::Hash<T>;

class TypeErasedInterface {
public:
    virtual ~TypeErasedInterface() = default;

    template <typename H> friend H AbslHashValue(H state, const TypeErasedInterface& wrapper)
    {
...
template <typename T> struct TypeErasedAbslHash {
...
template <typename T> std::vector<T> Vector(size_t count)
{
    std::vector<T> result;
...
template <typename T> struct FastUnorderedSet {
    ...
    template <typename H> friend H AbslHashValue(H h, const FastUnorderedSet& fus)
    {
        return H::combine(H::combine_unordered(std::move(h), fus.values.begin(), fus.values.end()), fus.values.size());
    }
};

template <typename T> absl::flat_hash_set<T> FlatHashSet(size_t count)
{
...
#define MAKE_BENCHMARK(hash, name, ...)                                                                                                                        \
    ...
    bool absl_hash_test_odr_use##hash##name = (benchmark::DoNotOptimize(&Codegen##hash##name), false)
...
template <class T> struct PodRand {
...
template <size_t N> struct StringRand {
