...
template <typename T> class TypeErasedValue {
public:
    ...
    template <typename H> friend H AbslHashValue(H hash_state, const TypeErasedValue& v)
    {
...
template <typename T> class TypeErasedContainer : public TypeErasedValue<T> {
public:
...
// detect it.
template <typename T> using is_hashable = std::is_default_constructible<absl::Hash<T>>;
