...
bool AbslParseFlag(absl::string_view src, OptionalInt* flag, std::string* error)
{
    int val;
    if (src.empty())
        flag->reset();
...
bool AbslParseFlag(absl::string_view src, OptionalString* flag, std::string* error)
{
    std::string val;
    if (src.empty())
        flag->reset();
...
#if defined(_MSC_VER)
#define REPLICATE(A, T, name) REPLICATE_7(A, T, name, 0) REPLICATE_7(A, T, name, 1)
...
#define FLAG_PTR_ARR(T) static constexpr absl::Flag<T>* FlagPtrs_##T[] = { REPLICATE(FLAG_PTR, T, T##_flag) };
...
template <> struct Accumulator<String> {
    using type = size_t;
};
template <> struct Accumulator<VectorOfStrings> {
    using type = size_t;
};
template <> struct Accumulator<OptionalInt> {
    using type = bool;
};
template <> struct Accumulator<OptionalString> {
    using type = bool;
};
template <> struct Accumulator<UDT> {
    using type = bool;
};

template <typename T> void Accumulate(typename Accumulator<T>::type& a, const T& f)
{
    a += f;
}
void Accumulate(bool& a, bool f)
{
    a = a || f;
}
void Accumulate(size_t& a, const std::string& f)
{
    a += f.size();
}
void Accumulate(size_t& a, const std::vector<std::string>& f)
{
    a += f.size();
}
void Accumulate(bool& a, const OptionalInt& f)
{
    a |= f.has_value();
}
void Accumulate(bool& a, const OptionalString& f)
{
    a |= f.has_value();
}
...
#define InvokeGetFlag(T)                                                                                                                                       \
    T AbslInvokeGetFlag##T()                                                                                                                                   \
    {                                                                                                                                                          \
        return absl::GetFlag(SINGLE_FLAG(T));                                                                                                                  \
    }                                                                                                                                                          \
