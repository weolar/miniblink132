...
void FlagRegistry::RegisterFlag(CommandLineFlag& flag, const char* filename)
{
    if (filename != nullptr && flag.Filename() != GetUsageConfig().normalize_filename(filename)) {
        flags_internal::ReportUsageError(absl::StrCat("Inconsistency between flag object and registration for flag '", flag.Name(),
    ...
    if (ins.second == false) { // means the name was already in the map
        ...
        if (flag.IsRetired() != old_flag.IsRetired()) {
            ...
            flags_internal::ReportUsageError(absl::StrCat("Retired flag '", flag.Name(), "' was defined normally in file '",
                                                 (flag.IsRetired() ? old_flag.Filename() : flag.Filename()), "'."),
                true);
        } else if (flags_internal::PrivateHandleAccessor::TypeId(flag) != flags_internal::PrivateHandleAccessor::TypeId(old_flag)) {
            ...
            flags_internal::ReportUsageError(
                absl::StrCat("Flag '", flag.Name(), "' was defined more than once (in files '", old_flag.Filename(), "' and '", flag.Filename(), "')."), true);
        } else {
            flags_internal::ReportUsageError(
                absl::StrCat("Something is wrong with flag '", flag.Name(), "' in file '", flag.Filename(), "'. One possibility: file '", flag.Filename(),
...
FlagSaver::FlagSaver()
    : impl_(new flags_internal::FlagSaverImpl)
{
