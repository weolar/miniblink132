...
ABSL_FLAG(std::vector<std::string>, flagfile, {}, "comma-separated list of files to load flags from").OnUpdate([]() {
    if (absl::GetFlag(FLAGS_flagfile).empty())
        return;
...
// etc.
bool ReadFlagfiles(const std::vector<std::string>& flagfiles, std::vector<ArgsList>& input_args)
{
    bool success = true;
...
std::tuple<bool, absl::string_view> DeduceFlagValue(
    const CommandLineFlag& flag, absl::string_view value, bool is_negative, bool is_empty_value, ArgsList* curr_list)
{
    ...
    if (flag.IsOfType<bool>()) {
        if (value.empty()) {
            ...
            // "--nobool_flag=Y" case
            flags_internal::ReportUsageError(absl::StrCat("Negative form with assignment is not valid for the "
                                                          "boolean flag '",
                                                 flag.Name(), "'"),
                true);
            return std::make_tuple(false, "");
        ...
        if (!value.empty() && value[0] == '-' && flag.IsOfType<std::string>()) {
            ...
            if (maybe_flag_name.empty() || std::get<0>(LocateFlag(maybe_flag_name)) != nullptr) {
...
bool CanIgnoreUndefinedFlag(absl::string_view flag_name)
{
    ...
    if (absl::ConsumePrefix(&flag_name, "no") && std::find(undefok.begin(), undefok.end(), flag_name) != undefok.end()) {
        return true;
    }
...
void ReportUnrecognizedFlags(const std::vector<UnrecognizedFlag>& unrecognized_flags, bool report_as_fatal_error)
{
    for (const auto& unrecognized : unrecognized_flags) {
        ...
        if (unrecognized.source == UnrecognizedFlag::kFromArgv) {
            misspelling_hints = flags_internal::GetMisspellingHints(unrecognized.flag_name);
        }

        if (misspelling_hints.empty()) {
            flags_internal::ReportUsageError(absl::StrCat("Unknown command line flag '", unrecognized.flag_name, "'"), report_as_fatal_error);
        } else {
            flags_internal::ReportUsageError(
                absl::StrCat("Unknown command line flag '", unrecognized.flag_name, "'. Did you mean: ", absl::StrJoin(misspelling_hints, ", "), " ?"),
...
bool WasPresentOnCommandLine(absl::string_view flag_name)
{
    ...
    return std::binary_search(specified_flags->begin(), specified_flags->end(), flag_name, SpecifiedFlagsCompare {});
}

// --------------------------------------------------------------------

struct BestHints {
    explicit BestHints(uint8_t _max)
        : best_distance(_max + 1)
    {
    }
    bool AddHint(absl::string_view hint, uint8_t distance)
    {
        if (hints.size() >= kMaxHints)
            return false;
...
std::vector<std::string> GetMisspellingHints(const absl::string_view flag)
{
    ...
    flags_internal::ForEachFlag([&](const CommandLineFlag& f) {
        if (best_hints.hints.size() >= kMaxHints)
            return;
        uint8_t distance = strings_internal::CappedDamerauLevenshteinDistance(flag, f.Name(), best_hints.best_distance);
        ...
        if (f.IsOfType<bool>()) {
            ...
            distance = strings_internal::CappedDamerauLevenshteinDistance(flag, negated_flag, best_hints.best_distance);
    ...
    absl::c_for_each(undefok, [&](const absl::string_view f) {
        if (best_hints.hints.size() >= kMaxHints)
            return;
        uint8_t distance = strings_internal::CappedDamerauLevenshteinDistance(flag, f, best_hints.best_distance);
...
std::vector<char*> ParseCommandLineImpl(
    int argc, char* argv[], UsageFlagsAction usage_flag_action, OnUndefinedFlag undef_flag_action, std::ostream& error_help_output)
{
    std::vector<char*> positional_args;
    std::vector<UnrecognizedFlag> unrecognized_flags;

    auto help_mode = flags_internal::ParseAbseilFlagsOnlyImpl(argc, argv, positional_args, unrecognized_flags, usage_flag_action);

    if (undef_flag_action != OnUndefinedFlag::kIgnoreUndefined) {
        flags_internal::ReportUnrecognizedFlags(unrecognized_flags, (undef_flag_action == OnUndefinedFlag::kAbortIfUndefined));

        if (undef_flag_action == OnUndefinedFlag::kAbortIfUndefined) {
            if (!unrecognized_flags.empty()) {
                flags_internal::HandleUsageFlags(error_help_output, ProgramUsageMessage());
                std::exit(1);
            }
        }
    }
    ...
    int argc, char* argv[], std::vector<char*>& positional_args, std::vector<UnrecognizedFlag>& unrecognized_flags, UsageFlagsAction usage_flag_action)
{
    ...
    while (!input_args.empty()) {
        ...
        if (!absl::ConsumePrefix(&arg, "-") || arg.empty()) {
            ABSL_INTERNAL_CHECK(arg_from_argv, "Flagfile cannot contain positional argument");
        ...
        std::tie(flag_name, value, is_empty_value) = flags_internal::SplitNameAndValue(arg);
        ...
        if (flag == nullptr) {
            ...
            unrecognized_flags.emplace_back(arg_from_argv ? UnrecognizedFlag::kFromArgv : UnrecognizedFlag::kFromFlagfile, flag_name);
            continue;
        }

        // Deduce flag's value (from this or next argument).
        bool value_success = true;
        std::tie(value_success, value) = flags_internal::DeduceFlagValue(*flag, value, is_negative, is_empty_value, &curr_list);
        ...
        // to retired flag.
        std::string error;
        if (!flags_internal::PrivateHandleAccessor::ParseFrom(*flag, value, flags_internal::SET_FLAGS_VALUE, flags_internal::kCommandLine, error)) {
            if (flag->IsRetired())
                continue;
    ...
    if (!input_args.empty()) {
        for (size_t arg_index = input_args.back().FrontIndex(); arg_index < static_cast<size_t>(argc); ++arg_index) {
    ...
    std::sort(specified_flags->begin(), specified_flags->end(), flags_internal::SpecifiedFlagsCompare {});
    ...
    return usage_flag_action == UsageFlagsAction::kHandleUsage ? flags_internal::HandleUsageFlags(std::cout, ProgramUsageMessage()) : HelpMode::kNone;
}
...
{
    auto help_mode = flags_internal::ParseAbseilFlagsOnlyImpl(argc, argv, positional_args, unrecognized_flags, flags_internal::UsageFlagsAction::kHandleUsage);
...
{
    return flags_internal::ParseCommandLineImpl(argc, argv, flags_internal::UsageFlagsAction::kHandleUsage, flags_internal::OnUndefinedFlag::kAbortIfUndefined);
}
