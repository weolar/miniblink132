...
class ConcurrentSequenceLockTest : public testing::TestWithParam<std::tuple<int, int>> {
public:
    ...
    for (int i = 0; i < num_threads_; i++) {
        threads.emplace_back([&]() {
            ...
            while (!stop.load(std::memory_order_relaxed)) {
                if (seq_lock.TryRead(local_buf.data(), protected_buf.data(), buf_bytes_)) {
                    bool good = true;
...
TEST_P(SequenceLockTest, SingleThreaded)
{
    ...
    absl::FixedArray<std::atomic<uint64_t>> protected_buf(flags::AlignUp(size, sizeof(uint64_t)) / sizeof(uint64_t));
