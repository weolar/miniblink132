...
#define DONT_VALIDATE(T, _)                                                                                                                                    \
    if (flag_type_id == base_internal::FastTypeId<T>())                                                                                                        \
        return false;
...
class MutexRelock {
public:
    explicit MutexRelock(absl::Mutex& mu)
        : mu_(mu)
    {
        mu_.Unlock();
    }
...
private:
    ...
    {
        if (!flag_impl_.RestoreState(*this))
            return;

        ABSL_INTERNAL_LOG(INFO, absl::StrCat("Restore saved value of ", flag_impl_.Name(), " to: ", flag_impl_.CurrentValue()));
    }
...
void FlagImpl::Init()
{
    ...
    case FlagValueStorageKind::kOneWordAtomic: {
        ...
        if (ValueStorageKind() == FlagValueStorageKind::kValueAndInitBit) {
            ...
            std::memcpy(buf.data() + Sizeof(op_), &initialized, sizeof(initialized));
        }
        // Type can contain valid uninitialized bits, e.g. padding.
        ABSL_ANNOTATE_MEMORY_IS_INITIALIZED(buf.data(), buf.size());
        OneWordValue().store(absl::bit_cast<int64_t>(buf), std::memory_order_release);
        break;
    }
    ...
    case FlagValueStorageKind::kHeapAllocated:
        ...
        PtrStorage().store(MaskedPointer(ptr_value.Ptr(), true), std::memory_order_release);
        break;
    }
...
{
    absl::call_once(const_cast<FlagImpl*>(this)->init_control_, &FlagImpl::Init, const_cast<FlagImpl*>(this));

    // data_guard_ is initialized inside Init.
    return reinterpret_cast<absl::Mutex*>(&data_guard_);
}

void FlagImpl::AssertValidType(FlagFastTypeId rhs_type_id, const std::type_info* (*gen_rtti)()) const
{
    ...
    if (ABSL_PREDICT_TRUE(lhs_type_id == rhs_type_id))
        return;
    ...
    ABSL_INTERNAL_LOG(FATAL, absl::StrCat("Flag '", Name(), "' is defined as one type and declared as another"));
}
...
{
    return HelpSourceKind() == FlagHelpKind::kLiteral ? help_.literal : help_.gen_func();
}
...
std::string FlagImpl::CurrentValue() const
{
    ...
    case FlagValueStorageKind::kValueAndInitBit:
    case FlagValueStorageKind::kOneWordAtomic: {
        const auto one_word_val = absl::bit_cast<std::array<char, sizeof(int64_t)>>(OneWordValue().load(std::memory_order_acquire));
        return flags_internal::Unparse(op_, one_word_val.data());
    }
    case FlagValueStorageKind::kSequenceLocked: {
        std::unique_ptr<void, DynValueDeleter> cloned(flags_internal::Alloc(op_), DynValueDeleter { op_ });
        ReadSequenceLockedData(cloned.get());
        return flags_internal::Unparse(op_, cloned.get());
    }
    case FlagValueStorageKind::kHeapAllocated: {
        absl::MutexLock l(guard);
        return flags_internal::Unparse(op_, PtrStorage().load(std::memory_order_acquire).Ptr());
    }
    }
...
std::unique_ptr<FlagStateInterface> FlagImpl::SaveState()
{
    ...
    case FlagValueStorageKind::kValueAndInitBit:
    case FlagValueStorageKind::kOneWordAtomic: {
        return absl::make_unique<FlagState>(*this, OneWordValue().load(std::memory_order_acquire), modified, on_command_line, ModificationCount());
    }
    case FlagValueStorageKind::kSequenceLocked: {
        ...
        return absl::make_unique<FlagState>(*this, cloned, modified, on_command_line, ModificationCount());
    }
    case FlagValueStorageKind::kHeapAllocated: {
        return absl::make_unique<FlagState>(
            *this, flags_internal::Clone(op_, PtrStorage().load(std::memory_order_acquire).Ptr()), modified, on_command_line, ModificationCount());
    }
    }
    return nullptr;
}
...
template <typename StorageT> StorageT* FlagImpl::OffsetValue() const
{
...
std::unique_ptr<void, DynValueDeleter> FlagImpl::TryParse(absl::string_view value, std::string& err) const
{
    ...
    if (!flags_internal::Parse(op_, value, tentative_value.get(), &parse_err)) {
        ...
        err = absl::StrCat("Illegal value '", value, "' specified for flag '", Name(), "'", err_sep, parse_err);
        return nullptr;
    }
...
bool FlagImpl::ReadOneBool() const
{
    ...
    auto* guard = DataGuard(); // Make sure flag initialized
    (void)guard;
    return absl::bit_cast<FlagValueAndInitBit<bool>>(OneWordValue().load(std::memory_order_acquire)).value;
}
...
void FlagImpl::Write(const void* src)
{
    ...
    if (ShouldValidateFlagValue(flags_internal::FastTypeId(op_))) {
        std::unique_ptr<void, DynValueDeleter> obj { flags_internal::Clone(op_, src), DynValueDeleter { op_ } };
        std::string ignored_error;
        std::string src_as_str = flags_internal::Unparse(op_, src);
        if (!flags_internal::Parse(op_, src_as_str, obj.get(), &ignored_error)) {
            ABSL_INTERNAL_LOG(ERROR, absl::StrCat("Attempt to set flag '", Name(), "' to invalid value ", src_as_str));
        }
    }
...
bool FlagImpl::ParseFrom(absl::string_view value, FlagSettingMode set_mode, ValueSource source, std::string& err)
{
    absl::MutexLock l(DataGuard());
...
void FlagImpl::CheckDefaultValueParsingRoundtrip() const
{
    ...
    if (!flags_internal::Parse(op_, v, dst.get(), &error)) {
        ABSL_INTERNAL_LOG(
            FATAL, absl::StrCat("Flag ", Name(), " (from ", Filename(), "): string form of default value '", v, "' could not be parsed; error=", error));
    }
