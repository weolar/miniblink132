...
template <size_t N> struct FixedCharArray {
    ...
    template <size_t... I> static constexpr FixedCharArray<N> FromLiteralString(absl::string_view str, absl::index_sequence<I...>)
    {
        return (void)str, FixedCharArray<N>({ { str[I]..., '\0' } });
    }
};

template <typename Gen, size_t N = Gen::Value().size()> constexpr FixedCharArray<N + 1> HelpStringAsArray(int)
{
...
template <typename T> constexpr T InitDefaultValue(T t)
{
    return t;
}

template <typename T> constexpr T InitDefaultValue(EmptyBraces)
{
    return T {};
}
...
template <typename ValueT, typename GenT> constexpr FlagDefaultArg DefaultArg(char)
{
...
template <typename T> using FlagUseOneWordStorage = std::integral_constant<bool, std::is_trivially_copyable<T>::value && (sizeof(T) <= 8)>;

template <class T> using FlagUseSequenceLockStorage = std::integral_constant<bool, std::is_trivially_copyable<T>::value && (sizeof(T) > 8)>;
...
// type.
template <typename T> static constexpr FlagValueStorageKind StorageKind()
{
    return FlagUseValueAndInitBitStorage<T>::value ? FlagValueStorageKind::kValueAndInitBit
        : FlagUseOneWordStorage<T>::value          ? FlagValueStorageKind::kOneWordAtomic
...
template <typename T> struct alignas(8) FlagValueAndInitBit {
    T value;
...
// implementation.
template <typename T, FlagValueStorageKind Kind = flags_internal::StorageKind<T>()> struct FlagValue;
...
template <typename T> struct FlagValue<T, FlagValueStorageKind::kValueAndInitBit> : FlagOneWordValue {
    constexpr FlagValue()
        : FlagOneWordValue(0)
    {
    }
...
template <typename T> struct FlagValue<T, FlagValueStorageKind::kHeapAllocated> : FlagMaskedPointerValue {
    ...
    alignas(MaskedPointer::RequiredAlignment()) alignas(T) char buffer[sizeof(T)] {};
};
...
private:
    template <typename T> friend class Flag;
    friend class FlagState;

    // Ensures that `data_guard_` is initialized and returns it.
    absl::Mutex* DataGuard() const ABSL_LOCK_RETURNED(reinterpret_cast<absl::Mutex*>(data_guard_));
    // Returns heap allocated value of type T initialized with default value.
    std::unique_ptr<void, DynValueDeleter> MakeInitValue() const ABSL_EXCLUSIVE_LOCKS_REQUIRED(*DataGuard());
    ...
    template <typename StorageT> StorageT* OffsetValue() const;
    ...
    void StoreValue(const void* src, ValueSource source) ABSL_EXCLUSIVE_LOCKS_REQUIRED(*DataGuard());
...
private:
    template <typename, bool> friend class FlagRegistrar;
...
class FlagImplPeer {
public:
    template <typename T, typename FlagType> static T InvokeGet(const FlagType& flag)
    {
        return flag.Get();
    }
    template <typename FlagType, typename T> static void InvokeSet(FlagType& flag, const T& v)
    {
        flag.Set(v);
    }
    template <typename FlagType> static const CommandLineFlag& InvokeReflect(const FlagType& f)
    {
        return f.Reflect();
    }
};
...
template <typename T> void* FlagOps(FlagOp op, const void* v1, void* v2, void* v3)
{
    ...
    case FlagOp::kDelete: {
        ...
        p->~T();
        Allocator alloc;
        std::allocator_traits<Allocator>::deallocate(alloc, reinterpret_cast<AlignedSpace*>(p), 1);
        return nullptr;
    }
    case FlagOp::kCopy:
    ...
    case FlagOp::kUnparse:
        *static_cast<std::string*>(v2) = absl::UnparseFlag<T>(*static_cast<const T*>(v1));
        return nullptr;
