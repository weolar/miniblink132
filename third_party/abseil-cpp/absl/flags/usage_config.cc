...
bool ContainsHelpshortFlags(absl::string_view filename)
{
    ...
    if (!absl::ConsumePrefix(&suffix, program_name_ref))
        return false;
    return absl::StartsWith(suffix, ".") || absl::StartsWith(suffix, "-main.") || absl::StartsWith(suffix, "_main.");
}
...
ABSL_CONST_INIT FlagsUsageConfig* custom_usage_config ABSL_GUARDED_BY(custom_usage_config_guard) = nullptr;
...
void SetFlagsUsageConfig(FlagsUsageConfig usage_config)
{
    ...
    if (!usage_config.contains_helpshort_flags)
        usage_config.contains_helpshort_flags = flags_internal::ContainsHelpshortFlags;
    ...
    if (!usage_config.contains_helppackage_flags)
        usage_config.contains_helppackage_flags = flags_internal::ContainsHelppackageFlags;
