...
template <typename T, typename H = typename std::conditional<std::numeric_limits<T>::is_signed, int64_t, uint64_t>::type>
...
template <typename T> void BM_DivideClass128UniformDivisor(benchmark::State& state)
{
...
template <typename T> void BM_RemainderClass128UniformDivisor(benchmark::State& state)
{
    auto values = GetRandomClass128SampleUniformDivisor<T>();
    while (state.KeepRunningBatch(values.size())) {
        for (const auto& pair : values) {
...
template <typename T> void BM_DivideClass128SmallDivisor(benchmark::State& state)
{
...
template <typename T> void BM_RemainderClass128SmallDivisor(benchmark::State& state)
{
    auto values = GetRandomClass128SampleSmallDivisor<T>();
    while (state.KeepRunningBatch(values.size())) {
        for (const auto& pair : values) {
...
std::vector<std::pair<absl::uint128, absl::uint128>> GetRandomClass128Sample()
{
    ...
    for (size_t i = 0; i < kSampleSize; ++i) {
        values.emplace_back(
            absl::MakeUint128(uniform_uint64(random), uniform_uint64(random)), absl::MakeUint128(uniform_uint64(random), uniform_uint64(random)));
    }
    return values;
}
...
template <typename T, typename H = typename std::conditional<std::is_same<T, __int128>::value, int64_t, uint64_t>::type> class UniformIntDistribution128 {
public:
...
template <typename T, typename H = typename std::conditional<std::is_same<T, __int128>::value, int64_t, uint64_t>::type>
std::vector<std::pair<T, T>> GetRandomIntrinsic128SampleUniformDivisor()
{
    ...
    for (size_t i = 0; i < kSampleSize; ++i) {
        ...
        values.emplace_back(std::max(a, b), std::max(static_cast<T>(2), std::min(a, b)));
    }
    return values;
}

template <typename T> void BM_DivideIntrinsic128UniformDivisor(benchmark::State& state)
{
...
template <typename T> void BM_RemainderIntrinsic128UniformDivisor(benchmark::State& state)
{
    auto values = GetRandomIntrinsic128SampleUniformDivisor<T>();
    while (state.KeepRunningBatch(values.size())) {
        for (const auto& pair : values) {
...
template <typename T> void BM_DivideIntrinsic128SmallDivisor(benchmark::State& state)
{
...
template <typename T> void BM_RemainderIntrinsic128SmallDivisor(benchmark::State& state)
{
    auto values = GetRandomIntrinsic128SampleSmallDivisor<T>();
    while (state.KeepRunningBatch(values.size())) {
        for (const auto& pair : values) {
...
std::vector<std::pair<unsigned __int128, unsigned __int128>> GetRandomIntrinsic128Sample()
{
