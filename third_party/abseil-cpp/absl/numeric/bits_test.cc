...
template <typename IntT> class IntegerTypesTest : public ::testing::Test { };

using OneByteIntegerTypes = ::testing::Types<unsigned char, uint8_t>;
...
TEST(Rotate, Left)
{
    ...
    static_assert(rotl(uint64_t { 0x12345678ABCDEF01ULL }, 0) == uint64_t { 0x12345678ABCDEF01ULL }, "");
    ...
    EXPECT_EQ(rotl(uint64_t { 0x12345678ABCDEF01ULL }, 0), uint64_t { 0x12345678ABCDEF01ULL });
    ...
    EXPECT_EQ(rotl(uint64_t { 0x12345678ABCDEF01ULL }, 64), uint64_t { 0x12345678ABCDEF01ULL });
    ...
    EXPECT_EQ(rotl(uint64_t { 0x12345678ABCDEF01ULL }, 4), uint64_t { 0x2345678ABCDEF011ULL });

    EXPECT_EQ(rotl(uint8_t { 0x12 }, -4), uint8_t { 0x21 });
    EXPECT_EQ(rotl(uint16_t { 0x1234 }, -4), uint16_t { 0x4123 });
    EXPECT_EQ(rotl(uint32_t { 0x12345678UL }, -4), uint32_t { 0x81234567UL });
    EXPECT_EQ(rotl(uint64_t { 0x12345678ABCDEF01ULL }, -4), uint64_t { 0x112345678ABCDEF0ULL });
}
...
TEST(Rotate, Right)
{
    ...
    static_assert(rotr(uint64_t { 0x12345678ABCDEF01ULL }, 0) == uint64_t { 0x12345678ABCDEF01ULL }, "");
    ...
    EXPECT_EQ(rotr(uint64_t { 0x12345678ABCDEF01ULL }, 0), uint64_t { 0x12345678ABCDEF01ULL });
    ...
    EXPECT_EQ(rotr(uint64_t { 0x12345678ABCDEF01ULL }, 64), uint64_t { 0x12345678ABCDEF01ULL });
    ...
    EXPECT_EQ(rotr(uint64_t { 0x12345678ABCDEF01ULL }, 4), uint64_t { 0x112345678ABCDEF0ULL });

    EXPECT_EQ(rotr(uint8_t { 0x12 }, -4), uint8_t { 0x21 });
    EXPECT_EQ(rotr(uint16_t { 0x1234 }, -4), uint16_t { 0x2341 });
    EXPECT_EQ(rotr(uint32_t { 0x12345678UL }, -4), uint32_t { 0x23456781UL });
    EXPECT_EQ(rotr(uint64_t { 0x12345678ABCDEF01ULL }, -4), uint64_t { 0x2345678ABCDEF011ULL });
}
...
TEST(Rotate, Symmetry)
{
    ...
    for (int i = 0; i < kTrials; ++i) {
        uint8_t value = absl::Uniform(rng, std::numeric_limits<uint8_t>::min(), std::numeric_limits<uint8_t>::max());
        int shift = absl::Uniform(rng, -2 * std::numeric_limits<uint8_t>::digits, 2 * std::numeric_limits<uint8_t>::digits);

        EXPECT_EQ(rotl(value, shift), rotr(value, -shift));
    }

    for (int i = 0; i < kTrials; ++i) {
        uint16_t value = absl::Uniform(rng, std::numeric_limits<uint16_t>::min(), std::numeric_limits<uint16_t>::max());
        int shift = absl::Uniform(rng, -2 * std::numeric_limits<uint16_t>::digits, 2 * std::numeric_limits<uint16_t>::digits);

        EXPECT_EQ(rotl(value, shift), rotr(value, -shift));
    }

    for (int i = 0; i < kTrials; ++i) {
        uint32_t value = absl::Uniform(rng, std::numeric_limits<uint32_t>::min(), std::numeric_limits<uint32_t>::max());
        int shift = absl::Uniform(rng, -2 * std::numeric_limits<uint32_t>::digits, 2 * std::numeric_limits<uint32_t>::digits);

        EXPECT_EQ(rotl(value, shift), rotr(value, -shift));
    }

    for (int i = 0; i < kTrials; ++i) {
        uint64_t value = absl::Uniform(rng, std::numeric_limits<uint64_t>::min(), std::numeric_limits<uint64_t>::max());
        int shift = absl::Uniform(rng, -2 * std::numeric_limits<uint64_t>::digits, 2 * std::numeric_limits<uint64_t>::digits);
...
TEST(Counting, Popcount)
{
    ...
    for (int i = 0; i < 8; i++) {
        ...
        EXPECT_EQ(popcount(static_cast<uint8_t>(static_cast<uint8_t>(-1) ^ (uint8_t { 1 } << i))), 7);
    }

    for (int i = 0; i < 16; i++) {
        EXPECT_EQ(popcount(static_cast<uint16_t>(uint16_t { 1 } << i)), 1);
        EXPECT_EQ(popcount(static_cast<uint16_t>(static_cast<uint16_t>(-1) ^ (uint16_t { 1 } << i))), 15);
    }
...
template <typename T> struct PopcountInput {
    T value = 0;
    int expected = 0;
};

template <typename T> PopcountInput<T> GeneratePopcountInput(absl::BitGen& gen)
{
    PopcountInput<T> ret;
...
TEST(IntegralPowersOfTwo, SingleBit)
{
    ...
    EXPECT_TRUE(has_single_bit(static_cast<uint8_t>(std::numeric_limits<uint8_t>::max() / 2 + 1)));
    EXPECT_TRUE(has_single_bit(static_cast<uint16_t>(std::numeric_limits<uint16_t>::max() / 2 + 1)));
    EXPECT_TRUE(has_single_bit(static_cast<uint32_t>(std::numeric_limits<uint32_t>::max() / 2 + 1)));
    EXPECT_TRUE(has_single_bit(static_cast<uint64_t>(std::numeric_limits<uint64_t>::max() / 2 + 1)));
}

template <typename T, T arg, T = bit_ceil(arg)> bool IsBitCeilConstantExpression(int)
{
    return true;
}
template <typename T, T arg> bool IsBitCeilConstantExpression(char)
{
    return false;
}
...
TEST(IntegralPowersOfTwo, Ceiling)
{
    ...
    static_assert(bit_ceil(uint64_t { 0x8000000000000000 }) == uint64_t { 0x8000000000000000 }, "");
    ...
    EXPECT_FALSE((IsBitCeilConstantExpression<uint32_t, uint32_t { 0x80000001 }>(0)));
    EXPECT_FALSE((IsBitCeilConstantExpression<uint32_t, uint32_t { 0xffffffff }>(0)));

    EXPECT_TRUE((IsBitCeilConstantExpression<uint64_t, uint64_t { 0x0 }>(0)));
    EXPECT_TRUE((IsBitCeilConstantExpression<uint64_t, uint64_t { 0x8000000000000000 }>(0)));
    EXPECT_FALSE((IsBitCeilConstantExpression<uint64_t, uint64_t { 0x8000000000000001 }>(0)));
    EXPECT_FALSE((IsBitCeilConstantExpression<uint64_t, uint64_t { 0xffffffffffffffff }>(0)));
#endif
...
#if defined(__GNUC__)
static_assert(ABSL_INTERNAL_HAS_CONSTEXPR_POPCOUNT, "popcount should be constexpr");
