...
template <typename T> class Uint128IntegerTraitsTest : public ::testing::Test { };
typedef ::testing::Types<bool, char, signed char, unsigned char, char16_t, char32_t, wchar_t,
...
template <typename T> class Uint128FloatTraitsTest : public ::testing::Test { };
...
{
    static_assert(std::is_constructible<absl::uint128, TypeParam>::value, "absl::uint128 must be constructible from TypeParam");
    static_assert(std::is_assignable<absl::uint128&, TypeParam>::value, "absl::uint128 must be assignable from TypeParam");
    static_assert(!std::is_assignable<TypeParam&, absl::uint128>::value, "TypeParam must not be assignable from absl::uint128");
}
...
{
    static_assert(std::is_constructible<absl::uint128, TypeParam>::value, "absl::uint128 must be constructible from TypeParam");
    static_assert(!std::is_assignable<absl::uint128&, TypeParam>::value, "absl::uint128 must not be assignable from TypeParam");
    static_assert(!std::is_assignable<TypeParam&, absl::uint128>::value, "TypeParam must not be assignable from absl::uint128");
}
...
{
    static_assert(std::is_constructible<absl::uint128, __int128>::value, "absl::uint128 must be constructible from __int128");
    static_assert(std::is_assignable<absl::uint128&, __int128>::value, "absl::uint128 must be assignable from __int128");
    static_assert(!std::is_assignable<__int128&, absl::uint128>::value, "__int128 must not be assignable from absl::uint128");

    static_assert(std::is_constructible<absl::uint128, unsigned __int128>::value, "absl::uint128 must be constructible from unsigned __int128");
    static_assert(std::is_assignable<absl::uint128&, unsigned __int128>::value, "absl::uint128 must be assignable from unsigned __int128");
    static_assert(!std::is_assignable<unsigned __int128&, absl::uint128>::value, "unsigned __int128 must not be assignable from absl::uint128");
}
...
TEST(Uint128, AllTests)
{
    ...
    absl::uint128 low_high = absl::MakeUint128(0, std::numeric_limits<uint64_t>::max());
    EXPECT_LT(one, two);
    EXPECT_GT(two, one);
    EXPECT_LT(one, big);
    EXPECT_LT(one, big);
    ...
    EXPECT_EQ(absl::Uint128Low64((absl::uint128(1) << 64) - 1), std::numeric_limits<uint64_t>::max());
    EXPECT_TRUE(!!one);
...
TEST(Uint128, ConversionTests)
{
...
#ifdef ABSL_HAVE_INTRINSIC_INT128
    unsigned __int128 intrinsic = (static_cast<unsigned __int128>(0x3a5b76c209de76f6) << 64) + 0x1f25e1d63a2b46c5;
    absl::uint128 custom = absl::MakeUint128(0x3a5b76c209de76f6, 0x1f25e1d63a2b46c5);
    ...
    absl::uint128 from_precise_ints = absl::MakeUint128(0x530e, 0xda74000000000000);
    ...
    absl::uint128 highest_precision_in_long_double = ~absl::uint128 {} >> (128 - std::numeric_limits<long double>::digits);
    EXPECT_EQ(highest_precision_in_long_double, static_cast<absl::uint128>(static_cast<long double>(highest_precision_in_long_double)));
    // Apply a mask just to make sure all the bits are the right place.
    const absl::uint128 arbitrary_mask = absl::MakeUint128(0xa29f622677ded751, 0xf8ca66add076f468);
    EXPECT_EQ(highest_precision_in_long_double & arbitrary_mask,
        static_cast<absl::uint128>(static_cast<long double>(highest_precision_in_long_double & arbitrary_mask)));
...
TEST(Uint128, DivideAndMod)
{
    ...
    b = a / 2 + 1;
    absl::uint128 expected_r = absl::MakeUint128(0x29876d3a0e38ea61, 0xdf92cba98c83ffff);
    // Sanity checks.
...
TEST(Uint128, DivideAndModRandomInputs)
{
    ...
    for (int i = 0; i < kNumIters; ++i) {
        const absl::uint128 a = absl::MakeUint128(uniform_uint64(random), uniform_uint64(random));
        const absl::uint128 b = absl::MakeUint128(uniform_uint64(random), uniform_uint64(random));
        if (b == 0) {
...
TEST(Uint128, NumericLimitsTest)
{
    ...
    EXPECT_EQ(static_cast<int>(128 * std::log10(2)), std::numeric_limits<absl::uint128>::digits10);
...
constexpr absl::uint128 kPi = (absl::uint128(0x3243f6a8885a308d) << 64) | absl::uint128(0x313198a2e0370734);
...
TEST(Uint128, Hash)
{
    ...
    const auto values = std::make_tuple(
        ...
        // 64 bit limits
        absl::uint128 { std::numeric_limits<int64_t>::max() }, absl::uint128 { std::numeric_limits<uint64_t>::max() } + 0,
        absl::uint128 { std::numeric_limits<uint64_t>::max() } + 1, absl::uint128 { std::numeric_limits<uint64_t>::max() } + 2,
        // Keeping high same
        absl::uint128 { 1 } << 62, absl::uint128 { 1 } << 63,
        // Keeping low same
        absl::uint128 { 1 } << 64, absl::uint128 { 1 } << 65,
        // 128 bit limits
        std::numeric_limits<absl::uint128>::max(), std::numeric_limits<absl::uint128>::max() - 1, std::numeric_limits<absl::uint128>::min() + 1,
        ...
        ,
        Ext128 { 0 }, Ext128 { 1 }, ~Ext128 {}, Ext128 { std::numeric_limits<int64_t>::max() }, Ext128 { std::numeric_limits<uint64_t>::max() } + 0,
        Ext128 { std::numeric_limits<uint64_t>::max() } + 1, Ext128 { std::numeric_limits<uint64_t>::max() } + 2, Ext128 { 1 } << 62, Ext128 { 1 } << 63,
        Ext128 { 1 } << 64, Ext128 { 1 } << 65, std::numeric_limits<Ext128>::max(), std::numeric_limits<Ext128>::max() - 1,
        std::numeric_limits<Ext128>::min() + 1, std::numeric_limits<Ext128>::min(), static_cast<Ext128>(kPi)
#endif
    );
...
TEST(Int128Uint128, ConversionTest)
{
    absl::int128 nonnegative_signed_values[]
        = { 0, 1, 0xffeeddccbbaa9988, absl::MakeInt128(0x7766554433221100, 0), absl::MakeInt128(0x1234567890abcdef, 0xfedcba0987654321), absl::Int128Max() };
    ...
    absl::int128 negative_values[]
        = { -1, -0x1234567890abcdef, absl::MakeInt128(-0x5544332211ffeedd, 0), -absl::MakeInt128(0x76543210fedcba98, 0xabcdef0123456789) };
...
template <typename T> class Int128IntegerTraitsTest : public ::testing::Test { };

TYPED_TEST_SUITE(Int128IntegerTraitsTest, IntegerTypes);

TYPED_TEST(Int128IntegerTraitsTest, ConstructAssignTest)
{
    static_assert(std::is_constructible<absl::int128, TypeParam>::value, "absl::int128 must be constructible from TypeParam");
    static_assert(std::is_assignable<absl::int128&, TypeParam>::value, "absl::int128 must be assignable from TypeParam");
    static_assert(!std::is_assignable<TypeParam&, absl::int128>::value, "TypeParam must not be assignable from absl::int128");
}

template <typename T> class Int128FloatTraitsTest : public ::testing::Test { };
...
{
    static_assert(std::is_constructible<absl::int128, TypeParam>::value, "absl::int128 must be constructible from TypeParam");
    static_assert(!std::is_assignable<absl::int128&, TypeParam>::value, "absl::int128 must not be assignable from TypeParam");
    static_assert(!std::is_assignable<TypeParam&, absl::int128>::value, "TypeParam must not be assignable from absl::int128");
}
...
{
    static_assert(std::is_constructible<absl::int128, __int128>::value, "absl::int128 must be constructible from __int128");
    static_assert(std::is_assignable<absl::int128&, __int128>::value, "absl::int128 must be assignable from __int128");
    static_assert(!std::is_assignable<__int128&, absl::int128>::value, "__int128 must not be assignable from absl::int128");

    static_assert(std::is_constructible<absl::int128, unsigned __int128>::value, "absl::int128 must be constructible from unsigned __int128");
    static_assert(!std::is_assignable<absl::int128&, unsigned __int128>::value, "absl::int128 must be assignable from unsigned __int128");
    static_assert(!std::is_assignable<unsigned __int128&, absl::int128>::value, "unsigned __int128 must not be assignable from absl::int128");
}
#endif // ABSL_HAVE_INTRINSIC_INT128
...
template <typename T> class Int128IntegerConversionTest : public ::testing::Test { };
...
TYPED_TEST(Int128IntegerConversionTest, RoundTripTest)
{
    ...
    EXPECT_EQ(std::numeric_limits<TypeParam>::min(), static_cast<TypeParam>(absl::int128(std::numeric_limits<TypeParam>::min())));
    EXPECT_EQ(std::numeric_limits<TypeParam>::max(), static_cast<TypeParam>(absl::int128(std::numeric_limits<TypeParam>::max())));
}

template <typename T> class Int128FloatConversionTest : public ::testing::Test { };
...
TYPED_TEST(Int128FloatConversionTest, ConstructAndCastTest)
{
    ...
    uint64_t values[] = { 0x6d4492c24fb86199, 0x26ead65e4cb359b5, 0x2c43407433ba3fd1, 0x3b574ec668df6b55, 0x1c750e55a29f4f0f };
    ...
    absl::int128 large_values[] = { absl::MakeInt128(0x5b0640d96c7b3d9f, 0xb7a7189e51d18622), absl::MakeInt128(0x34bed042c6f65270, 0x73b236570669a089),
        absl::MakeInt128(0x43deba9e6da12724, 0xf7f0f83da686797d), absl::MakeInt128(0x71e8d383be4e5589, 0x75c3f96fb00752b6) };
    for (absl::int128 value : large_values) {
        ...
        for (int i = 0; i < 127 - std::numeric_limits<TypeParam>::digits; ++i) {
            ...
            EXPECT_EQ(int_value, static_cast<absl::int128>(static_cast<TypeParam>(int_value)));
            EXPECT_EQ(-int_value, static_cast<absl::int128>(static_cast<TypeParam>(-int_value)));
        }
    }
    ...
    // Check limits
    EXPECT_DOUBLE_EQ(std::ldexp(static_cast<TypeParam>(1), 127), static_cast<TypeParam>(absl::Int128Max()));
    EXPECT_DOUBLE_EQ(-std::ldexp(static_cast<TypeParam>(1), 127), static_cast<TypeParam>(absl::Int128Min()));
}
...
TEST(Int128, FactoryTest)
{
    ...
    EXPECT_EQ(absl::int128(std::numeric_limits<int64_t>::min()), absl::MakeInt128(-1, std::numeric_limits<int64_t>::min()));
    EXPECT_EQ(absl::int128(0), absl::MakeInt128(0, 0));
    EXPECT_EQ(absl::int128(1), absl::MakeInt128(0, 1));
    EXPECT_EQ(absl::int128(std::numeric_limits<int64_t>::max()), absl::MakeInt128(0, std::numeric_limits<int64_t>::max()));
}
...
{
    EXPECT_EQ(absl::MakeInt128(0x7fffffffffffffff, 0xffffffffffffffff), absl::Int128Max());
...
{
    __int128 intrinsic = (static_cast<__int128>(0x3a5b76c209de76f6) << 64) + 0x1f25e1d63a2b46c5;
    absl::int128 custom = absl::MakeInt128(0x3a5b76c209de76f6, 0x1f25e1d63a2b46c5);
...
TEST(Int128, ComparisonTest)
{
    ...
    for (const TestCase& pair : cases) {
        SCOPED_TRACE(::testing::Message() << "pair.smaller = " << pair.smaller << "; pair.larger = " << pair.larger);
...
{
    int64_t values64[] = { 0, 1, 12345, 0x4000000000000000, std::numeric_limits<int64_t>::max() };
...
TEST(Int128, AdditionSubtractionTest)
{
    ...
    for (const auto& pair : cases) {
        ...
        EXPECT_EQ(absl::int128(pair.second + pair.first), absl::int128(pair.second) += absl::int128(pair.first));

        EXPECT_EQ(absl::int128(pair.first - pair.second), absl::int128(pair.first) - absl::int128(pair.second));
        EXPECT_EQ(absl::int128(pair.second - pair.first), absl::int128(pair.second) -= absl::int128(pair.first));

        EXPECT_EQ(absl::MakeInt128(pair.second + pair.first, 0), absl::MakeInt128(pair.second, 0) + absl::MakeInt128(pair.first, 0));
    ...
    EXPECT_EQ(absl::MakeInt128(31, 0), absl::MakeInt128(20, 1) + absl::MakeInt128(10, std::numeric_limits<uint64_t>::max()));
}
...
TEST(Int128, MultiplicationTest)
{
    ...
    EXPECT_EQ(-absl::MakeInt128(0x55cae732029d1fce, 0xca6474b6423263e4), 0xa9b98a8ddf66bc * -absl::MakeInt128(0x81, 0x672e58231e2469d7));
    EXPECT_EQ(absl::MakeInt128(0x19c8b7620b507dc4, 0xfec042b71a5f29a4), -0x3e39341147 * -absl::MakeInt128(0x6a14b2, 0x5ed34cca42327b3c));
...
TEST(Int128, DivisionAndModuloTest)
{
    ...
    DivisionModCase manual_cases[] = {
        { absl::MakeInt128(0x6ada48d489007966, 0x3c9c5c98150d5d69), absl::MakeInt128(0x8bc308fb, 0x8cb9cc9a3b803344), 0xc3b87e08,
            absl::MakeInt128(0x1b7db5e1, 0xd9eca34b7af04b49) },
        { absl::MakeInt128(0xd6946511b5b, 0x4886c5c96546bf5f), -absl::MakeInt128(0x263b, 0xfd516279efcfe2dc), -0x59cbabf0,
...
TEST(Int128, BitwiseShiftTest)
{
    ...
    for (int i = 0; i < 63; ++i) {
        for (int j = 0; j < 64; ++j) {
            ...
            SCOPED_TRACE(::testing::Message() << "i = " << i << "; j = " << j);
            EXPECT_EQ(absl::MakeInt128(uint64_t { 1 } << i, 0), absl::int128(uint64_t { 1 } << j) << (i + 64 - j));
            EXPECT_EQ(absl::MakeInt128(uint64_t { 1 } << i, 0), absl::int128(uint64_t { 1 } << j) <<= (i + 64 - j));
        }
        for (int j = 0; j <= i; ++j) {
            // Left shift from (j + 64)-th bit to (i + 64)-th bit.
            SCOPED_TRACE(::testing::Message() << "i = " << i << "; j = " << j);
            EXPECT_EQ(absl::MakeInt128(uint64_t { 1 } << i, 0), absl::MakeInt128(uint64_t { 1 } << j, 0) << (i - j));
            EXPECT_EQ(absl::MakeInt128(uint64_t { 1 } << i, 0), absl::MakeInt128(uint64_t { 1 } << j, 0) <<= (i - j));
        }
    }
...
TEST(Int128, BitCastable)
{
    ...
    EXPECT_EQ(absl::bit_cast<absl::uint128>(absl::int128(-1)), std::numeric_limits<absl::uint128>::max());
    EXPECT_EQ(absl::bit_cast<absl::int128>(std::numeric_limits<absl::uint128>::max()), absl::int128(-1));
    EXPECT_EQ(absl::bit_cast<absl::uint128>(std::numeric_limits<absl::int128>::min()), absl::uint128(1) << 127);
    EXPECT_EQ(absl::bit_cast<absl::int128>(absl::uint128(1) << 127), std::numeric_limits<absl::int128>::min());
    EXPECT_EQ(absl::bit_cast<absl::uint128>(std::numeric_limits<absl::int128>::max()), (absl::uint128(1) << 127) - 1);
    EXPECT_EQ(absl::bit_cast<absl::int128>((absl::uint128(1) << 127) - 1), std::numeric_limits<absl::int128>::max());

    // Also verify static casting has the same behavior as bit casting.
    EXPECT_EQ(static_cast<absl::uint128>(absl::int128(-1)), std::numeric_limits<absl::uint128>::max());
    EXPECT_EQ(static_cast<absl::int128>(std::numeric_limits<absl::uint128>::max()), absl::int128(-1));
    EXPECT_EQ(static_cast<absl::uint128>(std::numeric_limits<absl::int128>::min()), absl::uint128(1) << 127);
    EXPECT_EQ(static_cast<absl::int128>(absl::uint128(1) << 127), std::numeric_limits<absl::int128>::min());
    EXPECT_EQ(static_cast<absl::uint128>(std::numeric_limits<absl::int128>::max()), (absl::uint128(1) << 127) - 1);
    EXPECT_EQ(static_cast<absl::int128>((absl::uint128(1) << 127) - 1), std::numeric_limits<absl::int128>::max());
}
