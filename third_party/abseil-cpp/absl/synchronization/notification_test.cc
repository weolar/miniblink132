...
static void BasicTests(bool notify_before_waiting, Notification* notification)
{
    ...
    EXPECT_FALSE(notification->WaitForNotificationWithTimeout(absl::Milliseconds(0)));
    ...
    EXPECT_LE(delay - slop, elapsed) << "WaitForNotificationWithTimeout returned " << delay - elapsed << " early (with " << slop << " slop), start time was "
                                     << start;
    ...
    for (int i = 0; i < kNumThreads; ++i) {
        workers.push_back(std::thread(&RunWorker, i, &ready_counter, notification, &done_counter));
    }
