...
{
    int64_t end = absl::base_internal::CycleClock::Now() + ns * absl::base_internal::CycleClock::Frequency() / 1e9;
...
template <typename MutexType> class RaiiLocker {
public:
    explicit RaiiLocker(MutexType* mu)
        : mu_(mu)
    {
        mu_->Lock();
    }
...
template <> class RaiiLocker<std::mutex> {
public:
    explicit RaiiLocker(std::mutex* mu)
        : mu_(mu)
    {
        mu_->lock();
    }
...
public:
    ...
    {
        absl::base_internal::ThreadIdentity* identity = absl::synchronization_internal::GetOrCreateCurrentThreadIdentity();
        ...
        identity->per_thread_synch.next_priority_read_cycles = std::numeric_limits<int64_t>::max();
    }
    ~ScopedThreadMutexPriority()
    {
        ...
        // priority.
        absl::synchronization_internal::GetOrCreateCurrentThreadIdentity()->per_thread_synch.next_priority_read_cycles = std::numeric_limits<int64_t>::min();
    }
};
...
void BM_MutexEnqueue(benchmark::State& state)
{
    ...
    ScopedThreadMutexPriority priority_setter((multiple_priorities && state.thread_index() != 0) ? 1 : 0);
    ...
    absl::synchronization_internal::PerThreadSem::SetThreadBlockedCounter(&shared->blocked_threads);
    ...
    ABSL_RAW_CHECK(shared->looping_threads.load(std::memory_order_relaxed) == 0 && shared->blocked_threads.load(std::memory_order_relaxed) == 0
        ...
        for (int i = 0; i < kBatchSize; i++) {
            {
                ...
                while (shared->looping_threads.load(std::memory_order_relaxed) - shared->blocked_threads.load(std::memory_order_relaxed) != 1) {
                }
            ...
            while (!shared->thread_has_mutex.load(std::memory_order_relaxed) && shared->looping_threads.load(std::memory_order_relaxed) > 1) {
            }
        }
    ...
    absl::synchronization_internal::PerThreadSem::SetThreadBlockedCounter(nullptr);
}

BENCHMARK(BM_MutexEnqueue)->Threads(4)->Threads(64)->Threads(128)->Threads(512)->ArgName("multiple_priorities")->Arg(false)->Arg(true);
...
void SetupBenchmarkArgs(benchmark::internal::Benchmark* bm, bool do_test_priorities)
{
...
BENCHMARK_TEMPLATE(BM_Contended, absl::Mutex)->Apply([](benchmark::internal::Benchmark* bm) { SetupBenchmarkArgs(bm, /*do_test_priorities=*/true); });

BENCHMARK_TEMPLATE(BM_Contended, absl::base_internal::SpinLock)->Apply([](benchmark::internal::Benchmark* bm) {
    SetupBenchmarkArgs(bm, /*do_test_priorities=*/false);
});

BENCHMARK_TEMPLATE(BM_Contended, std::mutex)->Apply([](benchmark::internal::Benchmark* bm) { SetupBenchmarkArgs(bm, /*do_test_priorities=*/false); });
...
void BM_ConditionWaiters(benchmark::State& state)
{
    ...
    struct Helper {
        ...
        {
            init->DecrementCount();
            m->LockWhen(absl::Condition(static_cast<bool (*)(int*)>([](int* v) { return *v == 0; }), p));
            m->Unlock();
        }
    };
    ...
    for (int i = 0; i < num_waiters; i++) {
        ...
        // to be equivalent.
        pool.Schedule([&, i] { Helper::Waiter(&init, &mu, &equivalence_classes[i % num_classes]); });
    }
    init.Wait();
