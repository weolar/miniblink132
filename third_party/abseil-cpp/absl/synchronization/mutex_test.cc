...
std::unique_ptr<absl::synchronization_internal::ThreadPool> CreatePool(int threads)
{
    return absl::make_unique<absl::synchronization_internal::ThreadPool>(threads);
}

std::unique_ptr<absl::synchronization_internal::ThreadPool> CreateDefaultPool()
{
...
static void ScheduleAfter(absl::synchronization_internal::ThreadPool* tp, absl::Duration after, const std::function<void()>& func)
{
...
static void TestTime(TestContext* cxt, int c, bool use_cv)
{
    ...
    if (c == 0) {
        ...
        if (use_cv) {
            ...
            CHECK(!cxt->mu.AwaitWithTimeout(false_cond, absl::Seconds(1))) << "TestTime failed";
        }
        absl::Duration elapsed = absl::Now() - start;
        CHECK(absl::Seconds(0.9) <= elapsed && elapsed <= absl::Seconds(2.0)) << "TestTime failed";
        ...
        if (use_cv) {
            ...
            CHECK(!cxt->mu.AwaitWithTimeout(false_cond, absl::Seconds(4))) << "TestTime failed";
        }
        elapsed = absl::Now() - start;
        CHECK(absl::Seconds(3.9) <= elapsed && elapsed <= absl::Seconds(6.0)) << "TestTime failed";
        ...
        if (use_cv) {
            ...
            CHECK(!cxt->mu.AwaitWithTimeout(false_cond, absl::Milliseconds(500))) << "TestTime failed";
        }
        const absl::Duration elapsed = absl::Now() - start;
        CHECK(absl::Seconds(0.4) <= elapsed && elapsed <= absl::Seconds(0.9)) << "TestTime failed";
        cxt->g0++;
    } else if (c == 2) {
        absl::MutexLock l(&cxt->mu);
        if (use_cv) {
            ...
            CHECK(cxt->mu.AwaitWithTimeout(g0ge2, absl::Seconds(100))) << "TestTime failed";
        }
        cxt->g0++;
    } else {
        absl::MutexLock l(&cxt->mu);
...
static void EndTest(int* c0, int* c1, absl::Mutex* mu, absl::CondVar* cv, const std::function<void(int)>& cb)
{
    mu->Lock();
    int c = (*c0)++;
    mu->Unlock();
...
static int RunTestCommon(TestContext* cxt, void (*test)(TestContext* cxt, int), int threads, int iterations, int operations)
{
    absl::Mutex mu2;
    absl::CondVar cv2;
    int c0 = 0;
    ...
    for (int i = 0; i != threads; i++) {
        tp.Schedule(std::bind(&EndTest, &c0, &c1, &mu2, &cv2, std::function<void(int)>(std::bind(test, cxt, std::placeholders::_1))));
    }
    mu2.Lock();
    while (c1 != threads) {
        cv2.Wait(&mu2);
...
static int RunTest(void (*test)(TestContext* cxt, int), int threads, int iterations, int operations)
{
    TestContext cxt;
...
static int RunTestWithInvariantDebugging(void (*test)(TestContext* cxt, int), int threads, int iterations, int operations, void (*invariant)(void*))
{
...
TEST(Mutex, CondVarWaitWithTimeoutSignalsAwait)
{
    ...
    // which will signal released_cv.  If not, the test will hang.
    state.release = true;
    EXPECT_TRUE(!state.released_cv.WaitWithTimeout(&state.release_mu, absl::Seconds(10))) << "; Unrecoverable test failure: CondVar::WaitWithTimeout did not "
...
TEST(Mutex, MutexTimeoutBug)
{
    ...
    x.mu.LockWhenWithTimeout(absl::Condition(&always_false), absl::Milliseconds(500));
...
INSTANTIATE_TEST_SUITE_P(CondVarWaitDeadlockTest, CondVarWaitDeadlock, ::testing::Range(0, 8), ::testing::PrintToStringParamName());
...
TEST(Mutex, MutexReaderWakeupBug)
{
    ...
    EXPECT_TRUE(x.mu2.LockWhenWithTimeout(absl::Condition(&x.done1), absl::Seconds(10)));
    x.mu2.Unlock();

    EXPECT_TRUE(x.mu2.LockWhenWithTimeout(absl::Condition(&x.done2), absl::Seconds(10)));
    x.mu2.Unlock();
}
...
template <typename T> bool TemplateEquals42(T* p)
{
    return *p == 42;
}
template <typename T> bool TemplateEquals43(T* p)
{
    return *p == 43;
}
...
TEST(Mutex, FunctionPointerCondition)
{
    ...
    EXPECT_FALSE((std::is_constructible<absl::Condition, decltype(Equals42), decltype(&const_x)>::value));
    // Validate use of is_constructible by contrasting to a well-formed case.
    EXPECT_TRUE((std::is_constructible<absl::Condition, decltype(ConstEquals42), decltype(&const_x)>::value));
}
...
TEST(Mutex, FunctionPointerConditionWithDerivedToBaseConversion)
{
    ...
    EXPECT_FALSE((std::is_constructible<absl::Condition, decltype(derived_pred), Base*>::value));
    EXPECT_FALSE((std::is_constructible<absl::Condition, decltype(derived_pred), const Base*>::value));
    // Validate use of is_constructible by contrasting to well-formed cases.
    EXPECT_TRUE((std::is_constructible<absl::Condition, decltype(derived_pred), Derived*>::value));
    EXPECT_TRUE((std::is_constructible<absl::Condition, decltype(derived_pred), const Derived*>::value));
}

struct Constable {
    bool WotsAllThisThen() const
    {
        return true;
    }
};
...
static void ReaderForReaderOnCondVar(absl::Mutex* mu, absl::CondVar* cv, int* running)
{
...
static bool ConditionWithAcquire(AcquireFromConditionStruct* x)
{
    ...
    if (x->value == 2 || x->value == 3) {
        ...
        bool always_false = false;
        x->mu1.LockWhenWithTimeout(absl::Condition(&always_false), absl::Milliseconds(100));
        x->mu1.Unlock();
    }
...
TEST(Mutex, AcquireFromCondition)
{
    ...
    x.value = 0;
    x.done = false;
    tp->Schedule(std::bind(&WaitForCond2, &x)); // run WaitForCond2() in a thread T
...
const char ScopedDisableBazelTestWarnings::kVarName[] = "TEST_WARNINGS_OUTPUT_FILE";
...
static bool DelayIsWithinBounds(absl::Duration expected_delay, absl::Duration actual_delay)
{
    bool pass = true;
    ...
    if (actual_delay < expected_delay) {
        LOG(WARNING) << "Actual delay " << actual_delay << " was too short, expected " << expected_delay << " (difference " << actual_delay - expected_delay
                     << ")";
        pass = false;
    }
    ...
    absl::Duration tolerance = expected_delay <= absl::ZeroDuration() ? absl::Milliseconds(10) : TimeoutTestAllowedSchedulingDelay();
    if (actual_delay > expected_delay + tolerance) {
        LOG(WARNING) << "Actual delay " << actual_delay << " was too long, expected " << expected_delay << " (difference " << actual_delay - expected_delay
                     << ")";
        pass = false;
    }
    return pass;
...
{
    return os << "from: " << param.from_file << ":" << param.from_line << " use_absolute_deadline: " << (param.use_absolute_deadline ? "true" : "false")
              << " wait_timeout: " << param.wait_timeout << " satisfy_condition_delay: " << param.satisfy_condition_delay
              << " expected_result: " << (param.expected_result ? "true" : "false") << " expected_delay: " << param.expected_delay;
}
...
static void RunAfterDelay(absl::Duration delay, absl::synchronization_internal::ThreadPool* pool, const std::function<void()>& callback)
{
...
class TimeoutTest : public ::testing::Test, public ::testing::WithParamInterface<TimeoutTestParam> { };
...
INSTANTIATE_TEST_SUITE_P(All, TimeoutTest, testing::ValuesIn(MakeTimeoutTestParamValues()));
...
TEST_P(TimeoutTest, Await)
{
    ...
    for (int attempt = 1;; ++attempt) {
        ...
        std::unique_ptr<absl::synchronization_internal::ThreadPool> pool = CreateDefaultPool();
        ...
        bool result
            = params.use_absolute_deadline ? mu.AwaitWithDeadline(cond, start_time + params.wait_timeout) : mu.AwaitWithTimeout(cond, params.wait_timeout);
...
TEST_P(TimeoutTest, LockWhen)
{
    ...
    for (int attempt = 1;; ++attempt) {
        ...
        absl::Time start_time = absl::Now();
        absl::Condition cond(&value);
        bool result = params.use_absolute_deadline ? mu.LockWhenWithDeadline(cond, start_time + params.wait_timeout)
...
TEST_P(TimeoutTest, ReaderLockWhen)
{
    ...
    for (int attempt = 0;; ++attempt) {
        ...
        absl::Time start_time = absl::Now();
        bool result = params.use_absolute_deadline ? mu.ReaderLockWhenWithDeadline(absl::Condition(&value), start_time + params.wait_timeout)
                                                   : mu.ReaderLockWhenWithTimeout(absl::Condition(&value), params.wait_timeout);
        mu.ReaderUnlock();
...
TEST_P(TimeoutTest, Wait)
{
    ...
    for (int attempt = 0;; ++attempt) {
        ...
        while (!value) {
            if (params.use_absolute_deadline ? cv.WaitWithDeadline(&mu, deadline) : cv.WaitWithTimeout(&mu, timeout)) {
...
TEST(Mutex, LoggingAddressReuse)
{
    ...
    auto invariant = +[](void* alive) { EXPECT_TRUE(*static_cast<bool*>(alive)); };
...
INSTANTIATE_TEST_SUITE_P(ThreadCounts, MutexVariableThreadCountTest, ::testing::ValuesIn(AllThreadCountValues()), ::testing::PrintToStringParamName());
...
TEST_P(MutexVariableThreadCountTest, Mutex)
{
    ...
    EXPECT_EQ(RunTestWithInvariantDebugging(&TestMu, threads, iterations, operations, CheckSumG0G1), operations);
#endif
}
...
TEST_P(MutexVariableThreadCountTest, Try)
{
    ...
    iterations = std::min(iterations, 10);
    operations = threads * iterations;
    EXPECT_EQ(RunTestWithInvariantDebugging(&TestTry, threads, iterations, operations, CheckSumG0G1), operations);
#endif
}
...
TEST_P(MutexVariableThreadCountTest, RW)
{
    ...
    iterations = std::min(iterations, 10);
    operations = threads * iterations;
    EXPECT_EQ(RunTestWithInvariantDebugging(&TestRW, threads, iterations, operations, CheckSumG0G1), operations / 2);
#endif
}
...
TEST_P(MutexVariableThreadCountTest, SignalAll)
{
    ...
    int operations = iterations;
    EXPECT_EQ(RunTest(&TestSignalAll, threads, iterations, operations), operations);
}
...
TEST(Mutex, Timed)
{
    ...
    int iterations = 1000;
    int operations = iterations;
    EXPECT_EQ(RunTest(&TestCVTimeout, threads, iterations, operations), operations);
}
...
TEST(Mutex, CondVarPriority)
{
    ...
    std::thread test([&]() { err = pthread_setschedparam(pthread_self(), SCHED_FIFO, &param); });
    test.join();
    if (err) {
    ...
    std::thread th([&]() {
        ...
        mu.Unlock();
        EXPECT_EQ(absl::synchronization_internal::GetOrCreateCurrentThreadIdentity()->per_thread_synch.priority, param.sched_priority);
        mu.Lock();
    ...
    mu.Unlock();
    th.join();
    EXPECT_NE(absl::synchronization_internal::GetOrCreateCurrentThreadIdentity()->per_thread_synch.priority, param.sched_priority);
}
#endif
