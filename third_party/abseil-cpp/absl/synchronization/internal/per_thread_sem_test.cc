...
public:
    ...
    void TestTiming(const char* msg, bool timeout)
    {
        ...
        t.timeout = timeout ? KernelTimeout(absl::Now() + absl::Seconds(10000)) // far in the future
                            : KernelTimeout::Never();
        ...
        std::string out = StrCat(msg, "min cycle count=", min_cycles, " avg cycle count=", absl::SixDigits(static_cast<double>(total_cycles) / kNumIterations));
        printf("%s\n", out.c_str());
...
#endif
    EXPECT_LE(delay - slop, elapsed) << "Wait returned " << delay - elapsed << " early (with " << slop << " slop), start time was " << start;
