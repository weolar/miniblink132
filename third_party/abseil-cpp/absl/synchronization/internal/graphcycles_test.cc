    ...
    for (const auto& a : *nodes) {
        for (const auto& b : *nodes) {
            ...
            if (gc_reachable != reachable) {
                ...
                LOG(FATAL) << "gc_reachable " << gc_reachable << " reachable " << reachable << " a " << a << " b " << b;
            }
        }
    }
}
...
{
    int i = 0;
    while (i != edges->size() && ((*edges)[i].from != from || (*edges)[i].to != to)) {
        i++;
    }
...
TEST(GraphCycles, RandomizedTest)
{
    ...
    auto ptr = [](intptr_t i) { return reinterpret_cast<void*>(i + kDataOffset); };
    ...
    for (int iter = 0; iter != n; iter++) {
            ...
            if (nodes.size() > 0) {
                ...
                if (EdgeIndex(&edges, nodes[from], nodes[to]) == -1) {
                    if (graph_cycles.InsertEdge(id[nodes[from]], id[nodes[to]])) {
                        ...
                        std::unordered_set<int> seen;
                        ASSERT_TRUE(IsReachable(&edges, nodes[to], nodes[from], &seen)) << "Edge " << nodes[to] << "->" << nodes[from];
                    }
                }
            }
            break;
        ...
        case 4: // Check a path
            if (nodes.size() > 0) {
                ...
                int path_len = graph_cycles.FindPath(id[nodes[from]], id[nodes[to]], ABSL_ARRAYSIZE(path), path);
                std::unordered_set<int> seen;
                bool reachable = IsReachable(&edges, nodes[from], nodes[to], &seen);
                bool gc_reachable = graph_cycles.IsReachable(Get(id, nodes[from]), Get(id, nodes[to]));
