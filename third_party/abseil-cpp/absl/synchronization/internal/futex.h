...
public:
    ...
    static int WaitAbsoluteTimeout(std::atomic<int32_t>* v, int32_t val, const struct timespec* abs_timeout)
    {
        ...
        auto err = syscall(SYS_futex, reinterpret_cast<int32_t*>(v), FUTEX_WAIT_BITSET | FUTEX_PRIVATE_FLAG | FUTEX_CLOCK_REALTIME, val,
    ...
    static int WaitRelativeTimeout(std::atomic<int32_t>* v, int32_t val, const struct timespec* rel_timeout)
    {
        ...
        auto err = syscall(SYS_futex, reinterpret_cast<int32_t*>(v), FUTEX_PRIVATE_FLAG, val, ToFutexTimespec(rel_timeout, &ts));
        if (err != 0) {
            return -errno;
        }
        return 0;
