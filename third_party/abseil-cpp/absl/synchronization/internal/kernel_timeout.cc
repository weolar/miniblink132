...
struct timespec KernelTimeout::MakeClockAbsoluteTimespec(clockid_t c) const
{
    ...
    absl::Duration from_clock_epoch = absl::DurationFromTimespec(now) + absl::Nanoseconds(nanos);
...
KernelTimeout::DWord KernelTimeout::InMillisecondsFromNow() const
{
    ...
    constexpr uint64_t kMaxValueNanos = std::numeric_limits<int64_t>::max() - kNanosInMillis + 1;
...
std::chrono::time_point<std::chrono::system_clock> KernelTimeout::ToChronoTimePoint() const
{
    ...
    auto micros = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::nanoseconds(MakeAbsNanos()));
