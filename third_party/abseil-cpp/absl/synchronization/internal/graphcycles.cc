...
private:
    ...
    {
        if (ptr_ != space_)
            base_internal::LowLevelAlloc::Free(ptr_);
    }
    ...
    void Grow(uint32_t n)
    {
        ...
        T* copy = static_cast<T*>(base_internal::LowLevelAlloc::AllocWithArena(request, arena));
...
public:
    ...
    bool insert(int32_t v)
    {
        ...
        table_[i] = v;
        // Double when 75% full.
        if (occupied_ >= table_.size() - table_.size() / 4)
            Grow();
        return true;
    }
...
class PointerMap {
public:
    explicit PointerMap(const Vec<Node*>* nodes)
        : nodes_(nodes)
    {
        table_.fill(-1);
    }
...
GraphCycles::GraphCycles()
{
    ...
    rep_ = new (base_internal::LowLevelAlloc::AllocWithArena(sizeof(Rep), arena)) Rep;
}
...
bool GraphCycles::CheckInvariants() const
{
    ...
    for (uint32_t x = 0; x < r->nodes_.size(); x++) {
        ...
        if (ptr != nullptr && static_cast<uint32_t>(r->ptrmap_.Find(ptr)) != x) {
            ABSL_RAW_LOG(FATAL, "Did not find live node in hash table %" PRIu32 " %p", x, ptr);
        }
        if (nx->visited) {
        ...
        HASH_FOR_EACH(y, nx->out)
        {
            ...
            if (nx->rank >= ny->rank) {
                ABSL_RAW_LOG(FATAL, "Edge %" PRIu32 " ->%" PRId32 " has bad rank assignment %" PRId32 "->%" PRId32, x, y, nx->rank, ny->rank);
            }
        }
    }
    return true;
...
GraphId GraphCycles::GetId(void* ptr)
{
    ...
    if (i != -1) {
        ...
        Node* n = new (base_internal::LowLevelAlloc::AllocWithArena(sizeof(Node), arena)) Node;
...
static void Sort(const Vec<Node*>& nodes, Vec<int32_t>* delta)
{
    struct ByRank {
        ...
        {
            return (*nodes)[static_cast<uint32_t>(a)]->rank < (*nodes)[static_cast<uint32_t>(b)]->rank;
        }
    };
    ByRank cmp;
    cmp.nodes = &nodes;
...
int GraphCycles::FindPath(GraphId idx, GraphId idy, int max_path_len, GraphId path[]) const
{
    Rep* r = rep_;
    if (FindNode(r, idx) == nullptr || FindNode(r, idy) == nullptr)
        return 0;
    const int32_t x = NodeIndex(idx);
    const int32_t y = NodeIndex(idy);
    ...
    while (!r->stack_.empty()) {
        ...
        if (path_len < max_path_len) {
            path[path_len] = MakeId(n, rep_->nodes_[static_cast<uint32_t>(n)]->version);
        }
        path_len++;
...
void GraphCycles::UpdateStackTrace(GraphId id, int priority, int (*get_stack_trace)(void** stack, int))
{
    Node* n = FindNode(rep_, id);
