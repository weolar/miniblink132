...
public:
    ...
    explicit MutexLock(Mutex* mu, const Condition& cond) ABSL_EXCLUSIVE_LOCK_FUNCTION(mu)
        : mu_(mu)
    {
...
public:
    ...
    explicit ReaderMutexLock(Mutex* mu, const Condition& cond) ABSL_SHARED_LOCK_FUNCTION(mu)
        : mu_(mu)
    {
...
public:
    ...
    explicit WriterMutexLock(Mutex* mu, const Condition& cond) ABSL_EXCLUSIVE_LOCK_FUNCTION(mu)
        : mu_(mu)
    {
...
public:
    ...
    explicit MutexLockMaybe(Mutex* mu, const Condition& cond) ABSL_EXCLUSIVE_LOCK_FUNCTION(mu)
        : mu_(mu)
    {
        if (this->mu_ != nullptr) {
            this->mu_->LockWhen(cond);
...
public:
    ...
    explicit ReleasableMutexLock(Mutex* mu, const Condition& cond) ABSL_EXCLUSIVE_LOCK_FUNCTION(mu)
        : mu_(mu)
    {
        this->mu_->LockWhen(cond);
    }
...
inline Condition::Condition(bool (*func)(T*), typename absl::internal::type_identity<T>::type* arg)
...
template <typename T>
inline Condition::Condition(T* object, bool (absl::internal::type_identity<T>::type::*method)())
...
template <typename T>
inline Condition::Condition(const T* object, bool (absl::internal::type_identity<T>::type::*method)() const)
