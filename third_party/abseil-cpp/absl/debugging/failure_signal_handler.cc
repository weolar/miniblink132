...
#if !(defined(TARGET_OS_OSX) && TARGET_OS_OSX) && !(defined(TARGET_OS_WATCH) && TARGET_OS_WATCH) && !(defined(TARGET_OS_TV) && TARGET_OS_TV)                   \
    && !defined(__QNX__)
...
#define MAP_STACK 0
    ...
    prctl(PR_SET_VMA, PR_SET_VMA_ANON_NAME, sigstk.ss_sp, sigstk.ss_size, "absl-signalstack");
#endif
#endif // __linux__
...
ABSL_ATTRIBUTE_NOINLINE static void WriteStackTrace(void* ucontext, bool symbolize_stacktrace, void (*writerfn)(const char*, void*), void* writerfn_arg)
{
    constexpr int kNumStackFrames = 32;
    void* stack[kNumStackFrames];
    int frame_sizes[kNumStackFrames];
    int min_dropped_frames;
    int depth = absl::GetStackFramesWithContext(stack, frame_sizes, kNumStackFrames,
    ...
    absl::debugging_internal::DumpPCAndFrameSizesAndStackTrace(
        absl::debugging_internal::GetProgramCounter(ucontext), stack, frame_sizes, depth, min_dropped_frames, symbolize_stacktrace, writerfn, writerfn_arg);
}
...
static void WriteFailureInfo(int signo, void* ucontext, int cpu, void (*writerfn)(const char*))
{
    ...
    WriteStackTrace(ucontext, fsh_options.symbolize_stacktrace, WriterFnWrapper, &writerfn_struct);
}
...
static void AbslFailureSignalHandler(int signo, siginfo_t*, void* ucontext)
{
    ...
    if (!failed_tid.compare_exchange_strong(previous_failed_tid, this_tid, std::memory_order_acq_rel, std::memory_order_relaxed)) {
        ABSL_RAW_LOG(
            ERROR, "Signal %d raised at PC=%p while already in AbslFailureSignalHandler()", signo, absl::debugging_internal::GetProgramCounter(ucontext));
    ...
    WriteFailureInfo(
        signo, ucontext, my_cpu, +[](const char* data) { absl::raw_log_internal::AsyncSignalSafeWriteError(data, strlen(data)); });
