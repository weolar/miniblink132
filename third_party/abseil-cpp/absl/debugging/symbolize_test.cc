...
static void* GetPCFromFnPtr(void* ptr)
{
    return EM_ASM_PTR({ return wasmOffsetConverter.convert(wasmTable.get($0).name, 0); }, ptr);
}
...
{
    CHECK_LE(limit, sizeof(try_symbolize_buffer)) << "try_symbolize_buffer is too small";
    ...
    if (found) {
        CHECK_LT(static_cast<int>(strnlen(heap_buffer.get(), static_cast<size_t>(limit))), limit) << "absl::Symbolize() did not properly terminate the string";
...
TEST(Symbolize, Cached)
{
    ...
    EXPECT_STREQ("nonstatic_func", TrySymbolize(GetPCFromFnPtr((void*)(&nonstatic_func))));
    // The name of an internal linkage symbol is not specified; allow either a
    // mangled or an unmangled name here.
    const char* static_func_symbol = TrySymbolize(GetPCFromFnPtr((void*)(&static_func)));
    EXPECT_TRUE(strcmp("static_func", static_func_symbol) == 0 || strcmp("static_func()", static_func_symbol) == 0);
...
TEST(Symbolize, Truncation)
{
    ...
    EXPECT_STREQ("nonstatic_func", TrySymbolizeWithLimit(GetPCFromFnPtr((void*)(&nonstatic_func)), strlen(kNonStaticFunc) + 1));
    EXPECT_STREQ("nonstatic_...", TrySymbolizeWithLimit(GetPCFromFnPtr((void*)(&nonstatic_func)), strlen(kNonStaticFunc) + 0));
    EXPECT_STREQ("nonstatic...", TrySymbolizeWithLimit(GetPCFromFnPtr((void*)(&nonstatic_func)), strlen(kNonStaticFunc) - 1));
    EXPECT_STREQ("n...", TrySymbolizeWithLimit(GetPCFromFnPtr((void*)(&nonstatic_func)), 5));
    EXPECT_STREQ("...", TrySymbolizeWithLimit(GetPCFromFnPtr((void*)(&nonstatic_func)), 4));
    EXPECT_STREQ("..", TrySymbolizeWithLimit(GetPCFromFnPtr((void*)(&nonstatic_func)), 3));
    EXPECT_STREQ(".", TrySymbolizeWithLimit(GetPCFromFnPtr((void*)(&nonstatic_func)), 2));
    EXPECT_STREQ("", TrySymbolizeWithLimit(GetPCFromFnPtr((void*)(&nonstatic_func)), 1));
    EXPECT_EQ(nullptr, TrySymbolizeWithLimit(GetPCFromFnPtr((void*)(&nonstatic_func)), 0));
}
...
TEST(Symbolize, SymbolizeWithDemangling)
{
    ...
    EXPECT_STREQ("Foo::func(int)", TrySymbolize(GetPCFromFnPtr((void*)(&Foo::func))));
#else
    EXPECT_STREQ("Foo::func()", TrySymbolize(GetPCFromFnPtr((void*)(&Foo::func))));
#endif
}

TEST(Symbolize, SymbolizeSplitTextSections)
{
    EXPECT_STREQ("unlikely_func()", TrySymbolize(GetPCFromFnPtr((void*)(&unlikely_func))));
    EXPECT_STREQ("hot_func()", TrySymbolize(GetPCFromFnPtr((void*)(&hot_func))));
    EXPECT_STREQ("startup_func()", TrySymbolize(GetPCFromFnPtr((void*)(&startup_func))));
    EXPECT_STREQ("exit_func()", TrySymbolize(GetPCFromFnPtr((void*)(&exit_func))));
    EXPECT_STREQ("regular_func()", TrySymbolize(GetPCFromFnPtr((void*)(&regular_func))));
}
...
{
    if (absl::Symbolize(g_pc_to_symbolize, g_symbolize_buffer, sizeof(g_symbolize_buffer))) {
...
static const char* SymbolizeStackConsumption(void* pc, int* stack_consumed)
{
    ...
    *stack_consumed = absl::debugging_internal::GetSignalHandlerStackConsumption(SymbolizeSignalHandler);
...
TEST(Symbolize, SymbolizeStackConsumption)
{
    ...
    const char* symbol = SymbolizeStackConsumption((void*)(&nonstatic_func), &stack_consumed);
    ...
    EXPECT_TRUE(strcmp("static_func", symbol) == 0 || strcmp("static_func()", symbol) == 0);
    EXPECT_GT(stack_consumed, 0);
    EXPECT_LT(stack_consumed, GetStackConsumptionUpperLimit());
}
...
TEST(Symbolize, SymbolizeWithDemanglingStackConsumption)
{
    ...
    const char* symbol = SymbolizeStackConsumption((void*)(&Foo::func), &stack_consumed);
...
#if !defined(ABSL_INTERNAL_HAVE_DARWIN_SYMBOLIZE) && !defined(ABSL_INTERNAL_HAVE_EMSCRIPTEN_SYMBOLIZE)
...
static int FilterElfHeader(struct dl_phdr_info* info, size_t size, void* data)
{
    for (int i = 0; i < info->dlpi_phnum; i++) {
        if (info->dlpi_phdr[i].p_type == PT_LOAD && info->dlpi_phdr[i].p_flags == (PF_R | PF_X)) {
            const void* const vaddr = absl::bit_cast<void*>(info->dlpi_addr + info->dlpi_phdr[i].p_vaddr);
            ...
            absl::debugging_internal::RegisterFileMappingHint(vaddr, reinterpret_cast<const char*>(vaddr) + segsize, info->dlpi_phdr[i].p_offset, self_exe);
...
TEST(Symbolize, SymbolizeWithMultipleMaps)
{
    ...
    for (const char* ptr : ptrs) {
        ...
        void* remapped = reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(ptr + kPageSize) & ~(kPageSize - 1ULL));
...
static void DummySymbolDecorator(const absl::debugging_internal::SymbolDecoratorArgs* args)
{
    ...
    strncat(args->symbol_buf, message->c_str(), args->symbol_buf_size - strlen(args->symbol_buf) - 1);
}
...
TEST(Symbolize, InstallAndRemoveSymbolDecorators)
{
    ...
    EXPECT_GE(ticket_a = absl::debugging_internal::InstallSymbolDecorator(DummySymbolDecorator, &a_message), 0);

    int ticket_b;
    std::string b_message("b");
    EXPECT_GE(ticket_b = absl::debugging_internal::InstallSymbolDecorator(DummySymbolDecorator, &b_message), 0);

    int ticket_c;
    std::string c_message("c");
    EXPECT_GE(ticket_c = absl::debugging_internal::InstallSymbolDecorator(DummySymbolDecorator, &c_message), 0);
...
TEST(Symbolize, ForEachSection)
{
    ...
    ASSERT_TRUE(absl::debugging_internal::ForEachSection(fd, [&sections](const absl::string_view name, const ElfW(Shdr)&) {
...
{
#if defined(ABSL_HAVE_ATTRIBUTE_NOINLINE) && (defined(__i386__) || defined(__x86_64__))
    void* pc = non_inline_func();
    const char* symbol = TrySymbolize(pc);
    ASSERT_NE(symbol, nullptr) << "TestWithPCInsideNonInlineFunction failed";
    EXPECT_STREQ(symbol, "non_inline_func") << "TestWithPCInsideNonInlineFunction failed";
#endif
}

void ABSL_ATTRIBUTE_NOINLINE TestWithPCInsideInlineFunction()
{
#if defined(ABSL_HAVE_ATTRIBUTE_ALWAYS_INLINE) && (defined(__i386__) || defined(__x86_64__))
...
{
    constexpr char kNonStaticFunc[] = "nonstatic_func";
    EXPECT_STREQ("nonstatic_func", TrySymbolizeWithLimit((void*)(&nonstatic_func), strlen(kNonStaticFunc) + 1));
    EXPECT_STREQ("nonstatic_...", TrySymbolizeWithLimit((void*)(&nonstatic_func), strlen(kNonStaticFunc) + 0));
    EXPECT_STREQ("nonstatic...", TrySymbolizeWithLimit((void*)(&nonstatic_func), strlen(kNonStaticFunc) - 1));
...
#if defined(ABSL_INTERNAL_HAVE_ELF_SYMBOLIZE) || defined(ABSL_INTERNAL_HAVE_EMSCRIPTEN_SYMBOLIZE) || defined(ABSL_INTERNAL_HAVE_DARWIN_SYMBOLIZE)
