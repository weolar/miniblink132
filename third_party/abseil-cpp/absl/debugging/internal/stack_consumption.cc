...
#if defined(__i386__) || defined(__x86_64__) || defined(__ppc__) || defined(__aarch64__) || defined(__riscv)
...
int GetStackConsumption(const void* const altstack)
{
    ...
    for (int usage_count = kAlternateStackSize; usage_count > 0; --usage_count) {
        if (*begin != kAlternateStackFillValue) {
            ABSL_RAW_CHECK(usage_count <= kAlternateStackSize - kSafetyMargin, "Buffer has overflowed or is about to overflow");
            return usage_count;
        }
        begin += increment;
    }
...
int GetSignalHandlerStackConsumption(void (*signal_handler)(int))
{
    ...
    // itself.
    void* altstack = mmap(nullptr, kAlternateStackSize, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    ...
    ABSL_RAW_CHECK(sigaltstack(&sigstk, &old_sigstk) == 0, "sigaltstack() failed");
    ...
    if (old_sigstk.ss_sp == nullptr && old_sigstk.ss_size == 0 && (old_sigstk.ss_flags & SS_DISABLE)) {
    ...
    ABSL_RAW_CHECK(sigaltstack(&old_sigstk, nullptr) == 0, "sigaltstack() failed");
    ABSL_RAW_CHECK(sigaction(SIGUSR1, &old_sa1, nullptr) == 0, "sigaction() failed");
    ABSL_RAW_CHECK(sigaction(SIGUSR2, &old_sa2, nullptr) == 0, "sigaction() failed");
