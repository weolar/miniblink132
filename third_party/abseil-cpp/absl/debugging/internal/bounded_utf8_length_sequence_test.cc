...
TEST(BoundedUtf8LengthSequenceTest, RepeatedInsertsShiftValuesOutTheRightEnd)
{
    ...
    for (uint32_t i = 1; i < 31; ++i) {
        ASSERT_EQ(seq.InsertAndReturnSumOfPredecessors(0, 1), 0) << "while moving the 2 into position " << i;
        ASSERT_EQ(seq.InsertAndReturnSumOfPredecessors(31, 1), 32) << "after moving the 2 into position " << i;
    }
    ASSERT_EQ(seq.InsertAndReturnSumOfPredecessors(0, 1), 0) << "while moving the 2 into position 31";
    EXPECT_EQ(seq.InsertAndReturnSumOfPredecessors(31, 1), 31) << "after moving the 2 into position 31";
}
...
TEST(BoundedUtf8LengthSequenceTest, InsertsIntoWord1LeaveWord0Untouched)
{
    ...
    for (uint32_t i = 0; i < 32; ++i) {
        ASSERT_EQ(seq.InsertAndReturnSumOfPredecessors(i, 2), 2 * i) << "at index " << i;
    }
