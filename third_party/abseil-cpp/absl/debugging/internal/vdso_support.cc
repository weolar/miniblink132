...
#if !defined(__UCLIBC__) && defined(__GLIBC__) && (__GLIBC__ > 2 || (__GLIBC__ == 2 && __GLIBC_MINOR__ >= 16))
...
ABSL_CONST_INIT std::atomic<VDSOSupport::GetCpuFn> VDSOSupport::getcpu_fn_(&InitAndGetCPU);

VDSOSupport::VDSOSupport()
    // If vdso_base_ is still set to kInvalidBase, we got here
    // before VDSOSupport::Init has been called. Call it now.
    : image_(vdso_base_.load(std::memory_order_relaxed) == debugging_internal::ElfMemImage::kInvalidBase ? Init() : vdso_base_.load(std::memory_order_relaxed))
{
}
...
const void* VDSOSupport::Init()
{
    ...
    if (vdso_base_.load(std::memory_order_relaxed) == kInvalidBase) {
        errno = 0;
        const void* const sysinfo_ehdr = reinterpret_cast<const void*>(getauxval(AT_SYSINFO_EHDR));
        if (errno == 0) {
...
#if defined(__NetBSD__)
                vdso_base_.store(reinterpret_cast<void*>(aux.a_v), std::memory_order_relaxed);
#else
...
{
    ABSL_RAW_CHECK(base != debugging_internal::ElfMemImage::kInvalidBase, "internal error");
