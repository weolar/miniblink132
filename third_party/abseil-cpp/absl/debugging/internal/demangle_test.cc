...
TEST(Demangle, FunctionWithTemplateParamAndFunctionRequiresClauses)
{
    ...
    //
    // foo<int>();
    ASSERT_TRUE(Demangle("_Z3fooIiQsr3stdE8integralIT_EEivQsr3stdE8integralIS0_E", tmp, sizeof(tmp)));
    EXPECT_STREQ(tmp, "foo<>()");
}
...
TEST(Demangle, TwoArgTemplateBuiltinType)
{
    ...
    //
    // foo<int, char>();
    ASSERT_TRUE(Demangle("_Z3fooIicEu17__my_builtin_typeIT_T0_Ev", tmp, sizeof(tmp)));
    EXPECT_STREQ(tmp, "foo<>()");
}
...
TEST(Demangle, TypeNestedUnderTemplatedBuiltinType)
{
    ...
    ASSERT_TRUE(Demangle("_Z1fIRK1CENu20__remove_reference_tIT_E4typeES3_", tmp, sizeof(tmp)));
    EXPECT_STREQ("f<>()", tmp);
}
...
TEST(Demangle, TemplateParamSubstitutionWithGenericLambda)
{
    ...
    // Fooer<int> f;
    // f.foo<int>({});
    ASSERT_TRUE(Demangle("_ZN5FooerIiE3fooIiEEvNS0_UlTL0__TL0_0_E_E", tmp, sizeof(tmp)));
...
TEST(Demangle, LambdaRequiresSimpleExpression)
{
    ...
    ASSERT_TRUE(Demangle("_ZNK3$_0clIiEEDaT_QeqplLi2ELi2ELi4E", tmp, sizeof(tmp)));
    EXPECT_STREQ(tmp, "$_0::operator()<>()");
}
...
TEST(Demangle, LambdaRequiresRequiresExpressionContainingConcept)
{
    ...
    ASSERT_TRUE(Demangle("_ZNK3$_0clIiEEDaT_QrqXsr3stdE7same_asIDtfp_EiEE", tmp, sizeof(tmp)));
    EXPECT_STREQ(tmp, "$_0::operator()<>()");
}
...
TEST(Demangle, LambdaRequiresRequiresExpressionContainingReturnTypeConstraint)
{
    ...
    ASSERT_TRUE(Demangle("_ZNK3$_0clIiEEDaT_QrqXplfp_fp_RNSt7same_asIDtfp_EEEE", tmp, sizeof(tmp)));
    EXPECT_STREQ(tmp, "$_0::operator()<>()");
}
...
TEST(Demangle, LambdaRequiresRequiresExpressionWithBothNoexceptAndReturnType)
{
    ...
    ASSERT_TRUE(Demangle("_ZNK3$_0clIiEEDaT_QrqXplfp_fp_NRNSt7same_asIDtfp_EEEE", tmp, sizeof(tmp)));
    EXPECT_STREQ(tmp, "$_0::operator()<>()");
}
...
TEST(Demangle, LambdaRequiresRequiresExpressionContainingTwoRequirements)
{
    ...
    ASSERT_TRUE(Demangle("_ZNK3$_0clIiEEDaT_QrqXLb1EXeqplLi2ELi2ELi4EE", tmp, sizeof(tmp)));
    EXPECT_STREQ(tmp, "$_0::operator()<>()");
}
...
TEST(Demangle, LambdaWithExplicitTypeArgument)
{
    ...
    ASSERT_TRUE(Demangle("_ZZ1fIiET_S0_ENKUlTyS0_E_clIiEEDaS0_", tmp, sizeof(tmp)));
...
TEST(Demangle, LambdaWithExplicitPackArgument)
{
    ...
    ASSERT_TRUE(Demangle("_ZZ1fIiET_S0_ENKUlTpTyDpT_E_clIJiEEEDaS2_", tmp, sizeof(tmp)));
    EXPECT_STREQ(tmp, "f<>()::{lambda()#1}::operator()<>()");
}
...
TEST(Demangle, AvoidSignedOverflowForUnfortunateParameterNumbers)
{
    ...
    ASSERT_TRUE(Demangle("_ZZN1S1fEPFvvEEd2147483645_NKUlvE_clEv", tmp, sizeof(tmp)));
    EXPECT_STREQ(tmp, "S::f()::{default arg#2147483647}::{lambda()#1}::operator()()");

    // Now <number> is an int, but <number> + 2 is not.
    ASSERT_TRUE(Demangle("_ZZN1S1fEPFvvEEd2147483646_NKUlvE_clEv", tmp, sizeof(tmp)));
    EXPECT_STREQ(tmp, "S::f()::{default arg#1}::{lambda()#1}::operator()()");

    // <number> is the largest int.
    ASSERT_TRUE(Demangle("_ZZN1S1fEPFvvEEd2147483647_NKUlvE_clEv", tmp, sizeof(tmp)));
    EXPECT_STREQ(tmp, "S::f()::{default arg#1}::{lambda()#1}::operator()()");
    ...
    ASSERT_TRUE(Demangle("_ZZN1S1fEPFvvEEd2147483648_NKUlvE_clEv", tmp, sizeof(tmp)));
    EXPECT_STREQ(tmp, "S::f()::{default arg#1}::{lambda()#1}::operator()()");
}
...
TEST(Demangle, SubstpackNotationForTroublesomeTemplatePack)
{
    ...
    ASSERT_TRUE(Demangle("_ZN1AIJEE1fIJEEEvDpO1BI_SUBSTPACK_T_E", tmp, sizeof(tmp)));
...
TEST(Demangle, Clones)
{
    ...
    EXPECT_STREQ("Foo()", tmp);
    EXPECT_TRUE(Demangle("_ZL3Foov.__uniq.12345.isra.2.constprop.18", tmp, sizeof(tmp)));
    EXPECT_STREQ("Foo()", tmp);
    ...
    EXPECT_TRUE(Demangle("_ZL3Foov.part.9.165493.constprop.775.31805", tmp, sizeof(tmp)));
    EXPECT_STREQ("Foo()", tmp);
...
TEST(Demangle, ComplexFloatingPointLiterals)
{
    ...
    EXPECT_TRUE(Demangle("_Z1fIiEvRAszpltlCdstT_ELS0_0000000000000000_4010000000000000E_c", tmp, sizeof(tmp)));
    EXPECT_STREQ("f<>()", tmp);
}
...
TEST(Demangle, TemplateParameterObject)
{
    ...
    EXPECT_TRUE(Demangle("_Z1fIXtl1SLi1ELi2EEEXadL_ZTAXtlS0_Li1ELi2EEEEEEvv", tmp, sizeof(tmp)));
    EXPECT_STREQ("f<>()", tmp);
...
TEST(Demangle, EnableIfAttributeOnNamespaceScopeFunction)
{
    ...
    EXPECT_TRUE(Demangle("_ZN2ns1fEUa9enable_ifIXgefL0p_Li0EEEl", tmp, sizeof(tmp)));
...
TEST(Demangle, EnableIfAttributeOnFunctionTemplate)
{
    ...
    EXPECT_TRUE(Demangle("_Z1fIiEUa9enable_ifIXgefL0p_tliEEET_S0_", tmp, sizeof(tmp)));
    EXPECT_STREQ("f<>()", tmp);
}
...
TEST(Demangle, UnaryFoldExpressions)
{
    ...
    EXPECT_TRUE(Demangle("_Z1fIJilEE1SIXfrooeqstT_Li4EEEDpS1_", tmp, sizeof(tmp)));
    EXPECT_STREQ("f<>()", tmp);

    // The like with a left fold.
    //
    // S<(... || (sizeof (int) == 4, sizeof (long) == 4))> f<int, long>(int, long)
    EXPECT_TRUE(Demangle("_Z1fIJilEE1SIXflooeqstT_Li4EEEDpS1_", tmp, sizeof(tmp)));
    EXPECT_STREQ("f<>()", tmp);
}
...
TEST(Demangle, BinaryFoldExpressions)
{
    ...
    // f<int, long>(int, long)
    EXPECT_TRUE(Demangle("_Z1fIJilEE1SIXfRooeqstT_Li4ELb0EEEDpS1_", tmp, sizeof(tmp)));
    EXPECT_STREQ("f<>()", tmp);
    ...
    // f<int, long>(int, long)
    EXPECT_TRUE(Demangle("_Z1fIJilEE1SIXfLooLb0EeqstT_Li4EEEDpS1_", tmp, sizeof(tmp)));
    EXPECT_STREQ("f<>()", tmp);
}
...
TEST(Demangle, DirectListInitialization)
{
    ...
    EXPECT_TRUE(Demangle("_Z1hI3XYZEDTtlT_di1xLi1Edi1yLi2Edi1zLi3EEEv", tmp, sizeof(tmp)));
    EXPECT_STREQ("h<>()", tmp);
    ...
    EXPECT_TRUE(Demangle("_Z1jI1AEDTtlT_di1adXLi1ELi3ELi42EEEv", tmp, sizeof(tmp)));
    EXPECT_STREQ("j<>()", tmp);
}
...
TEST(Demangle, PlacementNewExpression)
{
    ...
    EXPECT_TRUE(Demangle("_Z1fIiEDTtlT_denwadfp__S0_piLi42EEEES0_", tmp, sizeof(tmp)));
    EXPECT_STREQ("f<>()", tmp);
}
...
TEST(Demangle, ArrayNewExpressionWithTwoElementsInBraces)
{
    ...
    EXPECT_TRUE(Demangle("_Z1fIiEDTtlT_dena_S0_ilLi1ELi2EEEEv", tmp, sizeof(tmp)));
    EXPECT_STREQ("f<>()", tmp);
}
...
TEST(Demangle, DubiousSrStSymbols)
{
    ...
    //     std::__cxx11::basic_string<char, std::char_traits<char>,
    //                                std::allocator<char> > const&)
    EXPECT_TRUE(Demangle("_ZSteqIcEN9__gnu_cxx11__enable_if"
...
#if defined(ABSL_INTERNAL_HAVE_DEBUGGING_STACK_CONSUMPTION) && !defined(ABSL_HAVE_ADDRESS_SANITIZER) && !defined(ABSL_HAVE_MEMORY_SANITIZER)                   \
...
static const char* DemangleStackConsumption(const char* mangled, int* stack_consumed)
{
    g_mangled = mangled;
...
TEST(Demangle, DemangleStackConsumption)
{
    ...
    const char* demangled = DemangleStackConsumption("_Z6foobarv", &stack_consumed);
    EXPECT_STREQ("foobar()", demangled);
    EXPECT_GT(stack_consumed, 0);
    EXPECT_LT(stack_consumed, kStackConsumptionUpperLimit);

    const std::string nested_mangled_name0 = NestedMangledName(0);
    demangled = DemangleStackConsumption(nested_mangled_name0.c_str(), &stack_consumed);
    EXPECT_STREQ("a", demangled);
    EXPECT_GT(stack_consumed, 0);
    EXPECT_LT(stack_consumed, kStackConsumptionUpperLimit);

    const std::string nested_mangled_name1 = NestedMangledName(1);
    demangled = DemangleStackConsumption(nested_mangled_name1.c_str(), &stack_consumed);
    EXPECT_STREQ("a<>", demangled);
    EXPECT_GT(stack_consumed, 0);
    EXPECT_LT(stack_consumed, kStackConsumptionUpperLimit);

    const std::string nested_mangled_name2 = NestedMangledName(2);
    demangled = DemangleStackConsumption(nested_mangled_name2.c_str(), &stack_consumed);
    EXPECT_STREQ("a<>", demangled);
    EXPECT_GT(stack_consumed, 0);
    EXPECT_LT(stack_consumed, kStackConsumptionUpperLimit);

    const std::string nested_mangled_name3 = NestedMangledName(3);
    demangled = DemangleStackConsumption(nested_mangled_name3.c_str(), &stack_consumed);
    EXPECT_STREQ("a<>", demangled);
    EXPECT_GT(stack_consumed, 0);
    EXPECT_LT(stack_consumed, kStackConsumptionUpperLimit);
}
...
TEST(DemangleStringTest, SupportsSymbolNameReturnedByTypeId)
{
    ...
    EXPECT_THAT(DemangleString(typeid(Base).name()), ContainsRegex("absl.*debugging_internal.*anonymous namespace.*::Base"));
    EXPECT_THAT(DemangleString(typeid(Derived).name()), ContainsRegex("absl.*debugging_internal.*anonymous namespace.*::Derived"));
}
