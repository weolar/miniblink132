...
bool ConsumeOptionalAsciiPrefix(
    const char*& punycode_begin, const char* const punycode_end, char* const out_begin, char* const out_end, uint32_t& num_ascii_chars)
{
    num_ascii_chars = 0;
...
bool ScanNextDelta(const char*& punycode_begin, const char* const punycode_end, uint32_t bias, uint32_t& i)
{
    ...
    for (uint32_t k = kBase; punycode_begin != punycode_end; k += kBase) {
        ...
        // bud here.
        static_assert(kMaxI >= kMaxChars * kMaxCodePoint, "kMaxI is too small to prevent spurious failures on good input");
        if (new_i > kMaxI)
            return false;

        static_assert(kMaxI < (uint64_t { 1 } << 32), "Make kMaxI smaller or i 64 bits wide to prevent silent wraparound");
...
absl::Nullable<char*> DecodeRustPunycode(DecodeRustPunycodeOptions options)
{
    ...
    *out_begin = '\0';
    ...
    if (!ConsumeOptionalAsciiPrefix(punycode_begin, punycode_end, out_begin, out_end, num_chars)) {
        return nullptr;
    }
    ...
    while (punycode_begin != punycode_end) {
        ...
        static_assert(kMaxI + kMaxCodePoint < (uint64_t { 1 } << 32), "Make kMaxI smaller or n 64 bits wide to prevent silent wraparound");
        n += i / (num_chars + 1);
        i %= num_chars + 1;
        ...
        uint32_t n_index = utf8_lengths.InsertAndReturnSumOfPredecessors(i, utf8_for_code_point.length);
        std::memmove(out_begin + n_index + utf8_for_code_point.length, out_begin + n_index, total_utf8_bytes + 1 - n_index);
        std::memcpy(out_begin + n_index, utf8_for_code_point.bytes, utf8_for_code_point.length);
