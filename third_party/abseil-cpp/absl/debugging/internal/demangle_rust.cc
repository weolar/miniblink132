...
#define ABSL_DEMANGLER_RECURSE(callee, caller)                                                                                                                 \
    do {                                                                                                                                                       \
        if (recursion_depth_ == kStackSize)                                                                                                                    \
            return false;                                                                                                                                      \
        ...
        for (; iter < kMaxReturns && recursion_depth_ > 0; ++iter) {
            ...
            impl_path:
                ++silence_depth_;
                {
                    ...
                    if (!ParseDisambiguator(ignored_disambiguator))
                        return false;
                }
            ...
            type:
                if (IsLower(Peek())) {
                    ...
                    if (type_name == nullptr || !Emit(type_name))
                        return false;
                    continue;
                }
                if (Eat('A')) {
            ...
            fn_type:
                if (!Emit("fn..."))
                    return false;
                ++silence_depth_;
                if (!ParseOptionalBinder())
                    return false;
                (void)Eat('U');
                if (Eat('K')) {
                    if (!Eat('C') && !ParseUndisambiguatedIdentifier())
                        return false;
                }
                while (!Eat('E')) {
            ...
            dyn_trait:
                ...
                if (Peek() == 'p') {
                    ...
                    ++silence_depth_;
                    while (Eat('p')) {
                        if (!ParseUndisambiguatedIdentifier())
                            return false;
...
private:
    ...
    ABSL_MUST_USE_RESULT bool Emit(const char* token)
    {
        ...
        if (static_cast<size_t>(out_end_ - out_) < bytes_to_copy)
            return false;
    ...
    {
        if (disambiguator < 0)
            return EmitChar('?'); // parsed but too large
        if (disambiguator == 0)
            return EmitChar('0');
        ...
        for (; disambiguator > 0; disambiguator /= 10) {
            digits[--leading_digit_index] = static_cast<char>('0' + disambiguator % 10);
        }
    ...
    ABSL_MUST_USE_RESULT bool ParseDisambiguator(int& value)
    {
        ...
        int base_62_value = 0;
        if (!ParseBase62Number(base_62_value))
            return false;
    ...
    ABSL_MUST_USE_RESULT bool ParseIdentifier(char uppercase_namespace = '\0')
    {
        ...
        int disambiguator = 0;
        if (!ParseDisambiguator(disambiguator))
            return false;
    ...
    ABSL_MUST_USE_RESULT bool ParseUndisambiguatedIdentifier(char uppercase_namespace = '\0', int disambiguator = 0)
    {
        ...
        if (!IsDigit(Peek()))
            return false;
        int num_bytes = 0;
        if (!ParseDecimalNumber(num_bytes))
            return false;
        ...
        if (uppercase_namespace != '\0') {
            ...
            case 'S':
                ...
                break;
            default:
                if (!EmitChar('{') || !EmitChar(uppercase_namespace))
                    return false;
                break;
            }
            if (num_bytes > 0 && !Emit(":"))
        ...
        if (uppercase_namespace != '\0') {
            if (!EmitChar('#'))
                return false;
            if (!EmitDisambiguator(disambiguator))
                return false;
            if (!EmitChar('}'))
                return false;
        }
    ...
    {
        if (namespace_depth_ == kNamespaceStackSize)
            return false;
    ...
    char PopNamespace()
    {
        return namespace_stack_[--namespace_depth_];
    }
    ...
    {
        if (position_depth_ == kPositionStackSize)
            return false;
    ...
    int PopPosition()
    {
        return position_stack_[--position_depth_];
    }
    ...
    ABSL_MUST_USE_RESULT bool BeginBackref()
    {
        ...
        int offset = 0;
        const int offset_of_this_backref = pos_ - 2 /* _R */ - 1 /* B already consumed */;
        if (!ParseBase62Number(offset) || offset < 0 || offset >= offset_of_this_backref) {
            return false;
        }
        offset += 2;

        // Save the old position to restore later.
        if (!PushPosition(pos_))
            return false;
