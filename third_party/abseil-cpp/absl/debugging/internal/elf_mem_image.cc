...
template <typename T> const T* GetTableElement(const ElfW(Ehdr) * ehdr, ElfW(Off) table_offset, ElfW(Word) element_size, size_t index)
{
...
const ElfW(Phdr) * ElfMemImage::GetPhdr(int index) const
{
    ...
    return GetTableElement<ElfW(Phdr)>(ehdr_, ehdr_->e_phoff, ehdr_->e_phentsize, static_cast<size_t>(index));
}
...
void ElfMemImage::Init(const void* base)
{
    ...
    ElfW(Dyn)* dynamic_entry = reinterpret_cast<ElfW(Dyn)*>(static_cast<intptr_t>(dynamic_program_header->p_vaddr) + relocation);
    uint32_t* sysv_hash = nullptr;
    uint32_t* gnu_hash = nullptr;
    for (; dynamic_entry->d_tag != DT_NULL; ++dynamic_entry) {
        const auto value = static_cast<intptr_t>(dynamic_entry->d_un.d_val) + relocation;
    ...
    if ((!sysv_hash && !gnu_hash) || !dynsym_ || !dynstr_ || !versym_ || !verdef_ || !verdefnum_ || !strsize_) {
...
ElfMemImage::SymbolIterator::SymbolIterator(const void* const image, uint32_t index)
    : index_(index)
    , image_(image)
{
}
