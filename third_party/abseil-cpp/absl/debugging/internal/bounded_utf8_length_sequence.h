...
template <uint32_t max_elements> class BoundedUtf8LengthSequence {
public:
    ...
    uint32_t InsertAndReturnSumOfPredecessors(uint32_t index, uint32_t utf8_length)
    {
        ...
        const uint64_t same_word_bits_below_insertion = rep_[word_index] & lower_seminibbles_mask;
        int full_popcount = absl::popcount(same_word_bits_below_insertion);
        int odd_popcount = absl::popcount(same_word_bits_below_insertion & odd_bits_mask);
        ...
        const uint32_t sum_of_predecessors = index + static_cast<uint32_t>(full_popcount + odd_popcount);
        ...
        rep_[word_index]
            = (rep_[word_index] & lower_seminibbles_mask) | (uint64_t { utf8_length - 1 } << bit_index) | ((rep_[word_index] & higher_seminibbles_mask) << 2);
