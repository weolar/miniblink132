...
void DumpPC(OutputWriter* writer, void* writer_arg, void* const pc, const char* const prefix)
{
    char buf[100];
...
void DumpPCAndFrameSize(OutputWriter* writer, void* writer_arg, void* const pc, int framesize, const char* const prefix)
{
    char buf[100];
    if (framesize <= 0) {
...
void DumpPCAndSymbol(OutputWriter* writer, void* writer_arg, void* const pc, const char* const prefix)
{
    char tmp[1024];
    ...
    if (absl::Symbolize(reinterpret_cast<const char*>(prev_pc), tmp, sizeof(tmp)) || absl::Symbolize(pc, tmp, sizeof(tmp))) {
        symbol = tmp;
    }
    char buf[1024];
...
// appropriate.
void DumpPCAndFrameSizeAndSymbol(OutputWriter* writer, void* writer_arg, void* const pc, void* const symbolize_pc, int framesize, const char* const prefix)
{
    char tmp[1024];
    const char* symbol = "(unknown)";
...
#if defined(__aarch64__)
        return reinterpret_cast<void*>(__darwin_arm_thread_state64_get_pc(signal_ucontext->uc_mcontext->__ss));
#elif defined(__arm__)
#if __DARWIN_UNIX03
...
void DumpPCAndFrameSizesAndStackTrace(void* const pc, void* const stack[], int frame_sizes[], int depth, int min_dropped_frames, bool symbolize_stacktrace,
    OutputWriter* writer, void* writer_arg)
{
    ...
    for (int i = 0; i < depth; i++) {
        if (symbolize_stacktrace) {
            ...
            // addresses.
            DumpPCAndFrameSizeAndSymbol(writer, writer_arg, stack[i], reinterpret_cast<char*>(stack[i]) - 1, frame_sizes[i], "    ");
        } else {
...
void DumpStackTrace(int min_dropped_frames, int max_num_frames, bool symbolize_stacktrace, OutputWriter* writer, void* writer_arg)
{
    ...
    if (num_stack >= max_num_frames) {
        ...
        const size_t needed_bytes = static_cast<size_t>(max_num_frames) * sizeof(stack[0]);
    ...
    if (allocated_bytes != 0)
        Deallocate(stack, allocated_bytes);
}
