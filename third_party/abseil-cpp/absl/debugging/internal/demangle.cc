...
void ReportHighWaterMark(State* state)
{
    ...
    if (input_length + 6 > static_cast<size_t>(state->out_end_idx) || state->too_complex) {
        if (state->out_end_idx > 0)
            state->out[0] = '\0';
        return;
    }
    ...
    std::memcpy(state->out + high_water_mark + 5, state->mangled_begin + high_water_mark, input_length - high_water_mark);
...
public:
    explicit ComplexityGuard(State* state)
        : state_(state)
    {
...
static bool ParseThreeCharToken(State* state, const char* three_char_token)
{
    ...
    if (guard.IsTooComplex())
        return false;
    if (RemainingInput(state)[0] == three_char_token[0] && RemainingInput(state)[1] == three_char_token[1] && RemainingInput(state)[2] == three_char_token[2]) {
...
static bool ParseLongToken(State* state, const char* long_token)
{
    ...
    for (; long_token[i] != '\0'; ++i) {
        ...
        if (RemainingInput(state)[i] != long_token[i])
            return false;
    }
...
static bool ParseNestedName(State* state)
{
    ...
    if (guard.IsTooComplex())
        return false;
    ParseState copy = state->parse_state;
    if (ParseOneCharToken(state, 'N') && EnterNestedName(state) && Optional(ParseCVQualifiers(state)) && Optional(ParseRefQualifier(state))
...
static bool ParsePrefix(State* state)
{
    ...
    while (true) {
        ...
        if (ParseTemplateParam(state) || ParseDecltype(state) || ParseSubstitution(state, /*accept_std=*/true) ||
            ...
            ParseVendorExtendedType(state) || ParseUnscopedName(state) || (ParseOneCharToken(state, 'M') && ParseUnnamedTypeName(state))) {
            has_something = true;
...
static bool ParseUnqualifiedName(State* state)
{
    ...
    if (guard.IsTooComplex())
        return false;
    if (ParseOperatorName(state, nullptr) || ParseCtorDtorName(state) || ParseSourceName(state) || ParseLocalSourceName(state) || ParseUnnamedTypeName(state)) {
        return ParseAbiTags(state);
    }

    // DC <source-name>+ E
    ParseState copy = state->parse_state;
    if (ParseTwoCharToken(state, "DC") && OneOrMore(ParseSourceName, state) && ParseOneCharToken(state, 'E')) {
        return true;
    }
    state->parse_state = copy;
    ...
    // F <source-name>
    // F <operator-name>
    if (ParseOneCharToken(state, 'F') && MaybeAppend(state, "friend ") && (ParseSourceName(state) || ParseOperatorName(state, nullptr))) {
        return true;
    }
    state->parse_state = copy;
...
static bool ParseSourceName(State* state)
{
    ...
    ParseState copy = state->parse_state;
    int length = -1;
    if (ParseNumber(state, &length) && ParseIdentifier(state, static_cast<size_t>(length))) {
        return true;
    }
    state->parse_state = copy;
    return false;
...
static bool ParseUnnamedTypeName(State* state)
{
    ...
    if (ParseTwoCharToken(state, "Ut") && Optional(ParseNumber(state, &which)) && which <= std::numeric_limits<int>::max() - 2 && // Don't overflow.
    ...
    // Closure type.
    which = -1;
    if (ParseTwoCharToken(state, "Ul") && DisableAppend(state) && ZeroOrMore(ParseTemplateParamDecl, state) && OneOrMore(ParseType, state)
...
static bool ParseOperatorName(State* state, int* arity)
{
    ...
    ParseState copy = state->parse_state;
    if (ParseTwoCharToken(state, "cv") && MaybeAppend(state, "operator ") && EnterNestedName(state) && ParseConversionOperatorType(state)
    ...
    if (ParseTwoCharToken(state, "li") && MaybeAppend(state, "operator\"\" ") && ParseSourceName(state)) {
        return true;
    }
    state->parse_state = copy;
...
static bool ParseSpecialName(State* state)
{
    ...
    // G++ extensions
    if (ParseTwoCharToken(state, "TC") && ParseType(state) && ParseNumber(state, nullptr) && ParseOneCharToken(state, '_') && DisableAppend(state)
        && ParseType(state)) {
    ...
    if (ParseThreeCharToken(state, "GTt") && MaybeAppend(state, "transaction clone for ") && ParseEncoding(state)) {
        return true;
    }
    state->parse_state = copy;
...
static bool ParseVOffset(State* state)
{
    ...
    if (guard.IsTooComplex())
        return false;
    ParseState copy = state->parse_state;
    if (ParseNumber(state, nullptr) && ParseOneCharToken(state, '_') && ParseNumber(state, nullptr)) {
        return true;
    }
    state->parse_state = copy;
    return false;
...
static bool ParseType(State* state)
{
    ...
    if (ParseBuiltinType(state) || ParseFunctionType(state) || ParseClassEnumType(state) || ParseArrayType(state) || ParsePointerToMemberType(state)
        || ParseDecltype(state) ||
    ...
    if (ParseTwoCharToken(state, "Dv") && ParseNumber(state, nullptr) && ParseOneCharToken(state, '_') && ParseType(state)) {
        return true;
    }
    state->parse_state = copy;

    // GNU vector extension Dv <expression> _ <type>
    if (ParseTwoCharToken(state, "Dv") && ParseExpression(state) && ParseOneCharToken(state, '_') && ParseType(state)) {
        return true;
    }
    state->parse_state = copy;
...
static bool ParseCVQualifiers(State* state)
{
    ...
    while (ParseExtendedQualifier(state))
        ++num_cv_qualifiers;
...
static bool ParseBuiltinType(State* state)
{
    ...
    // DB (<number> | <expression>) _  # _BitInt(N)
    // DU (<number> | <expression>) _  # unsigned _BitInt(N)
    if (ParseTwoCharToken(state, "DB") || (ParseTwoCharToken(state, "DU") && MaybeAppend(state, "unsigned "))) {
        bool append = state->parse_state.append;
        DisableAppend(state);
        int number = -1;
...
static bool ParseVendorExtendedType(State* state)
{
    ...
    ParseState copy = state->parse_state;
    if (ParseOneCharToken(state, 'u') && ParseSourceName(state) && Optional(ParseTemplateArgs(state))) {
        return true;
    }
    state->parse_state = copy;
    return false;
...
static bool ParseExceptionSpec(State* state)
{
    ...
    state->parse_state = copy;
    if (ParseTwoCharToken(state, "Dw") && OneOrMore(ParseType, state) && ParseOneCharToken(state, 'E')) {
        return true;
    }
    state->parse_state = copy;
...
static bool ParseBareFunctionType(State* state)
{
    ...
    ParseState copy = state->parse_state;
    DisableAppend(state);
    if (ZeroOrMore(ParseOverloadAttribute, state) && OneOrMore(ParseType, state)) {
        RestoreAppend(state, copy.append);
        MaybeAppend(state, "()");
        return true;
    }
...
static bool ParseClassEnumType(State* state)
{
    ...
    if (guard.IsTooComplex())
        return false;
    ParseState copy = state->parse_state;
    if (Optional(ParseTwoCharToken(state, "Ts") || ParseTwoCharToken(state, "Tu") || ParseTwoCharToken(state, "Te")) && ParseName(state)) {
        return true;
    }
    state->parse_state = copy;
    return false;
...
static bool ParseTemplateParamDecl(State* state)
{
    ...
    if (ParseTwoCharToken(state, "Tk") && ParseName(state) && Optional(ParseTemplateArgs(state))) {
        return true;
    }
    state->parse_state = copy;
...
static bool ParseTemplateArgs(State* state)
{
    ...
    ParseState copy = state->parse_state;
    DisableAppend(state);
    if (ParseOneCharToken(state, 'I') && OneOrMore(ParseTemplateArg, state) && Optional(ParseQRequiresClauseExpr(state)) && ParseOneCharToken(state, 'E')) {
        RestoreAppend(state, copy.append);
        MaybeAppend(state, "<>");
        return true;
    }
...
static inline bool ParseUnresolvedType(State* state)
{
    ...
    return (ParseTemplateParam(state) && Optional(ParseTemplateArgs(state))) || ParseDecltype(state) || ParseSubstitution(state, /*accept_std=*/false);
}
...
static bool ParseUnresolvedName(State* state)
{
    ...
    if (ParseTwoCharToken(state, "sr") && ParseOneCharToken(state, 'N') && ParseUnresolvedType(state) && OneOrMore(ParseUnresolvedQualifierLevel, state)
        && ParseOneCharToken(state, 'E') && ParseBaseUnresolvedName(state)) {
        return true;
    }
    state->parse_state = copy;

    if (Optional(ParseTwoCharToken(state, "gs")) && ParseTwoCharToken(state, "sr") && OneOrMore(ParseUnresolvedQualifierLevel, state)
        && ParseOneCharToken(state, 'E') && ParseBaseUnresolvedName(state)) {
        return true;
    }
    state->parse_state = copy;

    if (ParseTwoCharToken(state, "sr") && ParseTwoCharToken(state, "St") && ParseSimpleId(state) && ParseSimpleId(state)) {
        return true;
    }
    state->parse_state = copy;
...
static bool ParseBracedExpression(State* state)
{
    ...
    if (ParseTwoCharToken(state, "di") && ParseSourceName(state) && ParseBracedExpression(state)) {
        return true;
    }
    state->parse_state = copy;

    if (ParseTwoCharToken(state, "dx") && ParseExpression(state) && ParseBracedExpression(state)) {
        return true;
    }
    state->parse_state = copy;

    if (ParseTwoCharToken(state, "dX") && ParseExpression(state) && ParseExpression(state) && ParseBracedExpression(state)) {
        return true;
    }
    state->parse_state = copy;
...
static bool ParseExpression(State* state)
{
    ...
    if ((ParseThreeCharToken(state, "pp_") || ParseThreeCharToken(state, "mm_")) && ParseExpression(state)) {
        return true;
    }
    state->parse_state = copy;
    ...
    if (ParseTwoCharToken(state, "cp") && ParseSimpleId(state) && ZeroOrMore(ParseExpression, state) && ParseOneCharToken(state, 'E')) {
        return true;
    }
    state->parse_state = copy;
    ...
    //
    // https://github.com/itanium-cxx-abi/cxx-abi/issues/47
    if (ParseTwoCharToken(state, "so") && ParseType(state) && ParseExpression(state) && Optional(ParseNumber(state, nullptr))
        && ZeroOrMore(ParseUnionSelector, state) && Optional(ParseOneCharToken(state, 'p')) && ParseOneCharToken(state, 'E')) {
        return true;
    }
    state->parse_state = copy;

    // <expression> ::= <function-param>
    if (ParseFunctionParam(state))
        return true;
    state->parse_state = copy;

    // <expression> ::= tl <type> <braced-expression>* E
    if (ParseTwoCharToken(state, "tl") && ParseType(state) && ZeroOrMore(ParseBracedExpression, state) && ParseOneCharToken(state, 'E')) {
        return true;
    }
    state->parse_state = copy;

    // <expression> ::= il <braced-expression>* E
    if (ParseTwoCharToken(state, "il") && ZeroOrMore(ParseBracedExpression, state) && ParseOneCharToken(state, 'E')) {
        return true;
    }
    state->parse_state = copy;
    ...
    //              ::= [gs] na <expression>* _ <type> E
    //              ::= [gs] na <expression>* _ <type> <initializer>
    if (Optional(ParseTwoCharToken(state, "gs")) && (ParseTwoCharToken(state, "nw") || ParseTwoCharToken(state, "na")) && ZeroOrMore(ParseExpression, state)
        && ParseOneCharToken(state, '_') && ParseType(state) && (ParseOneCharToken(state, 'E') || ParseInitializer(state))) {
        return true;
    }
    state->parse_state = copy;
    ...
    //              ::= [gs] da <expression>
    if (Optional(ParseTwoCharToken(state, "gs")) && (ParseTwoCharToken(state, "dl") || ParseTwoCharToken(state, "da")) && ParseExpression(state)) {
        return true;
    }
    state->parse_state = copy;
    ...
    if (ParseCharClass(state, "dscr") && ParseOneCharToken(state, 'c') && ParseType(state) && ParseExpression(state)) {
        return true;
    }
    state->parse_state = copy;
    ...
    //              ::= sZ <function-param>
    if (ParseTwoCharToken(state, "sZ") && (ParseFunctionParam(state) || ParseTemplateParam(state))) {
        return true;
    }
    state->parse_state = copy;
    ...
    if (ParseTwoCharToken(state, "sP") && ZeroOrMore(ParseTemplateArg, state) && ParseOneCharToken(state, 'E')) {
        return true;
    }
    state->parse_state = copy;
    ...
    //              ::= fr <binary operator-name> <expression>
    if ((ParseTwoCharToken(state, "fl") || ParseTwoCharToken(state, "fr")) && ParseOperatorName(state, nullptr) && ParseExpression(state)) {
        return true;
    }
    state->parse_state = copy;
    ...
    if ((ParseTwoCharToken(state, "dt") || ParseTwoCharToken(state, "pt")) && ParseExpression(state) && ParseUnresolvedName(state)) {
        return true;
    }
    state->parse_state = copy;
    ...
    if (ParseOneCharToken(state, 'u') && ParseSourceName(state) && ZeroOrMore(ParseTemplateArg, state) && ParseOneCharToken(state, 'E')) {
        return true;
    }
    state->parse_state = copy;
    ...
    if (ParseTwoCharToken(state, "rq") && OneOrMore(ParseRequirement, state) && ParseOneCharToken(state, 'E')) {
        return true;
    }
    state->parse_state = copy;
    ...
    //
    // https://github.com/itanium-cxx-abi/cxx-abi/issues/24
    if (ParseTwoCharToken(state, "rQ") && ParseBareFunctionType(state) && ParseOneCharToken(state, '_') && OneOrMore(ParseRequirement, state)
        && ParseOneCharToken(state, 'E')) {
        return true;
    }
    state->parse_state = copy;
...
static bool ParseInitializer(State* state)
{
    ...
    if (ParseTwoCharToken(state, "pi") && ZeroOrMore(ParseExpression, state) && ParseOneCharToken(state, 'E')) {
        return true;
    }
    state->parse_state = copy;
...
static bool ParseExprPrimary(State* state)
{
    ...
    if (ParseOneCharToken(state, 'L')) {
        ...
        if (ParseThreeCharToken(state, "DnE"))
            return true;
        ...
        if (RemainingInput(state)[0] == 'A' /* an array type follows */) {
            if (ParseType(state) && ParseOneCharToken(state, 'E'))
                return true;
            state->parse_state = copy;
            return false;
        }
...
static bool ParseExprCastValueAndTrailingE(State* state)
{
    ...
    if (ParseFloatNumber(state)) {
        ...
        if (ParseOneCharToken(state, '_') && ParseFloatNumber(state) && ParseOneCharToken(state, 'E')) {
            return true;
        }
    }
    state->parse_state = copy;
...
static bool ParseRequirement(State* state)
{
    ...
    if (ParseOneCharToken(state, 'X') && ParseExpression(state) && Optional(ParseOneCharToken(state, 'N')) &&
    ...
    if (ParseOneCharToken(state, 'T') && ParseType(state))
        return true;
    state->parse_state = copy;
...
static bool ParseLocalNameSuffix(State* state)
{
    ...
    if (ParseOneCharToken(state, 'd') && (IsDigit(RemainingInput(state)[0]) || RemainingInput(state)[0] == '_')) {
        int number = -1;
        ...
        MaybeAppendDecimal(state, number);
        MaybeAppend(state, "}::");
        if (ParseOneCharToken(state, '_') && ParseName(state))
            return true;
