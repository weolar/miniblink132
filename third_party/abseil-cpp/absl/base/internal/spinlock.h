...
public:
    ...
    ABSL_MUST_USE_RESULT inline bool TryLock() ABSL_EXCLUSIVE_TRYLOCK_FUNCTION(true)
    {
        ABSL_TSAN_MUTEX_PRE_LOCK(this, __tsan_mutex_try_lock);
        bool res = TryLockImpl();
        ABSL_TSAN_MUTEX_POST_LOCK(this, __tsan_mutex_try_lock | (res ? 0 : __tsan_mutex_try_lock_failed), 0);
        return res;
    }
...
inline uint32_t SpinLock::TryLockInternal(uint32_t lock_value, uint32_t wait_cycles)
{
    ...
    if (!lockword_.compare_exchange_strong(
            lock_value, kSpinLockHeld | lock_value | wait_cycles | sched_disabled_bit, std::memory_order_acquire, std::memory_order_relaxed)) {
