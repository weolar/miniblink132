...
static void TestThreadIdentityCurrent(const void* assert_no_identity)
{
    ...
    EXPECT_EQ(0, reinterpret_cast<intptr_t>(&identity->per_thread_synch) % PerThreadSynch::kAlignment);
...
TEST(ThreadIdentityTest, BasicIdentityWorksThreaded)
{
    ...
    for (int iter = 0; iter < kNumLoops; iter++) {
        ...
        for (int i = 0; i < kNumThreads; ++i) {
            threads.push_back(std::thread(TestThreadIdentityCurrent, kCheckNoIdentity));
        }
