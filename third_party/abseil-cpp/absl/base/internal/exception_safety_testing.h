...
public:
    ...
    static void ObjectConstructed(void* address, std::string description)
    {
        ...
        TrackedAddress& tracked_address = current_tracker_instance_->address_map_[address];
        if (tracked_address.is_alive) {
            ADD_FAILURE() << ErrorMessage(address, tracked_address.description, current_tracker_instance_->countdown_,
                "Object was re-constructed. Current object was constructed by " + description);
        }
 ...
 */
template <TypeSpec Spec = TypeSpec::kEverythingThrows> class ThrowingValue : private exceptions_internal::TrackedObject {
...
public:
    ...
    template <typename... Args> void operator delete(void* p, Args&&... args) noexcept
    {
...
template <TypeSpec Spec, typename T> void operator,(const ThrowingValue<Spec>&, T&&) = delete;
template <TypeSpec Spec, typename T> void operator,(T&&, const ThrowingValue<Spec>&) = delete;
 ...
 * configuration macro.
 */
template <typename T, AllocSpec Spec = AllocSpec::kEverythingThrows> class ThrowingAllocator : private exceptions_internal::TrackedObject {
...
private:
    ...
    void ReadStateAndMaybeThrow(absl::string_view msg) const
    {
        if (!IsSpecified(AllocSpec::kNoThrowAllocate)) {
            exceptions_internal::MaybeThrow(absl::Substitute("Allocator id $0 threw from $1", *dummy_, msg));
        }
    }
...
template <typename T, AllocSpec Spec> int ThrowingAllocator<T, Spec>::next_id_ = 0;
...
template <typename Operation> testing::AssertionResult TestNothrowOp(const Operation& operation)
{
    struct Cleanup {
        Cleanup()
        {
    ...
    try {
        ...
        return testing::AssertionFailure() << "TestException thrown during call to operation() when nothrow "
                                              "guarantee was expected.";
    } catch (...) {
        return testing::AssertionFailure() << "Unknown exception thrown during call to operation() when "
...
using EnableIfTestable = typename absl::enable_if_t<LazyContractsCount != 0 && !std::is_same<LazyFactory, UninitializedT>::value
    && !std::is_same<LazyOperation, UninitializedT>::value>;

template <typename Factory = UninitializedT, typename Operation = UninitializedT, typename... Contracts> class ExceptionSafetyTestBuilder;
...
template <typename Factory> using FactoryPtrType = typename FactoryPtrTypeHelper<Factory>::type;

template <typename Factory> using FactoryElementType = typename FactoryPtrType<Factory>::element_type;
...
private:
    template <typename ContractFn> Contract WrapContract(const ContractFn& contract)
    {
 ...
 *   please.
 */
template <typename Factory, typename Operation, typename... Contracts> class ExceptionSafetyTestBuilder {
public:
    /*
   ...
   *   tester.WithFactory(...).
   */
    template <typename T> ExceptionSafetyTestBuilder<DefaultFactory<T>, Operation, Contracts...> WithInitialValue(const T& t) const
    {
   ...
   */
    ...
    {
        return { factory_, operation_, std::tuple_cat(contracts_, std::tuple<absl::decay_t<MoreContracts>...>(more_contracts...)) };
    }
...
private:
    template <typename, typename, typename...> friend class ExceptionSafetyTestBuilder;
    ...
    {
        return ExceptionSafetyTest<FactoryElementType<Factory>>(factory_, selected_operation, std::get<Indices>(contracts_)...).Test();
    }
