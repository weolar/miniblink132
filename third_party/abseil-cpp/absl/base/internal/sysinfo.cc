...
#pragma comment(lib, "advapi32.lib") // For Reg* functions.
    ...
    if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, "HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\0", 0, KEY_READ, &key) == ERROR_SUCCESS) {
        ...
        auto result = RegQueryValueExA(key, "~MHz", nullptr, &type, reinterpret_cast<LPBYTE>(&data), &data_size);
        RegCloseKey(key);
...
#endif
    if (rc != 0) {
        ABSL_INTERNAL_LOG(FATAL, "clock_gettime() failed: (" + std::to_string(errno) + ")");
    }
...
pid_t GetTID()
{
    ...
    static_assert(sizeof(pid_t) == sizeof(thread), "In NaCL int expected to be the same size as a pointer");
