...
alignas(LowLevelAlloc::Arena) unsigned char unhooked_async_sig_safe_arena_storage[sizeof(LowLevelAlloc::Arena)];
#endif
...
{
    new (&default_arena_storage) LowLevelAlloc::Arena(LowLevelAlloc::kCallMallocHook);
    new (&unhooked_arena_storage) LowLevelAlloc::Arena(0);
#ifndef ABSL_LOW_LEVEL_ALLOC_ASYNC_SIGNAL_SAFE_MISSING
    new (&unhooked_async_sig_safe_arena_storage) LowLevelAlloc::Arena(LowLevelAlloc::kAsyncSignalSafe);
#endif
}
...
{
    base_internal::LowLevelCallOnce(&create_globals_once, CreateGlobalArenas);
    return reinterpret_cast<LowLevelAlloc::Arena*>(&unhooked_async_sig_safe_arena_storage);
}
#endif
...
public:
    explicit ArenaLock(LowLevelAlloc::Arena* arena) ABSL_EXCLUSIVE_LOCK_FUNCTION(arena->mu)
        : arena_(arena)
    {
#ifndef ABSL_LOW_LEVEL_ALLOC_ASYNC_SIGNAL_SAFE_MISSING
    ...
    ~ArenaLock()
    {
        ABSL_RAW_CHECK(left_, "haven't left Arena region");
    }
...
bool LowLevelAlloc::DeleteArena(Arena* arena)
{
    ...
    while (arena->freelist.next[0] != nullptr) {
        ...
        if (munmap_result != 0) {
            ABSL_RAW_LOG(FATAL, "LowLevelAlloc::DeleteArena: munmap failed: %d", errno);
        }
#endif // _WIN32
    }
    section.Leave();
...
static void* DoAllocWithArena(size_t request, LowLevelAlloc::Arena* arena)
{
    ...
    if (request != 0) {
        ...
        for (;;) { // loop until we find a suitable region
...
#ifndef ABSL_LOW_LEVEL_ALLOC_ASYNC_SIGNAL_SAFE_MISSING
            if ((arena->flags & LowLevelAlloc::kAsyncSignalSafe) != 0) {
                new_pages = base_internal::DirectMmap(nullptr, new_pages_size, PROT_WRITE | PROT_READ, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
            } else {
                new_pages = mmap(nullptr, new_pages_size, PROT_WRITE | PROT_READ, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
            }
#else
            new_pages = mmap(nullptr, new_pages_size, PROT_WRITE | PROT_READ, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
#endif // ABSL_LOW_LEVEL_ALLOC_ASYNC_SIGNAL_SAFE_MISSING
            ...
            prctl(PR_SET_VMA, PR_SET_VMA_ANON_NAME, new_pages, new_pages_size, "absl");
#endif
#endif // __linux__
#endif // _WIN32
            arena->mu.Lock();
