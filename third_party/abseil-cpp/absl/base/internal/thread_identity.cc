...
#if ABSL_THREAD_IDENTITY_MODE == ABSL_THREAD_IDENTITY_MODE_USE_TLS || ABSL_THREAD_IDENTITY_MODE == ABSL_THREAD_IDENTITY_MODE_USE_CPP11
...
ABSL_CONST_INIT // Must come before __attribute__((visibility("protected")))
    ...
    thread_local ThreadIdentity* thread_identity_ptr
    = nullptr;
...
void SetCurrentThreadIdentity(ThreadIdentity* identity, ThreadIdentityReclaimerFunction reclaimer)
{
    ...
    absl::call_once(init_thread_identity_key_once, AllocateThreadIdentityKey, reclaimer);
    ...
    // more information.
    pthread_setspecific(thread_identity_pthread_key, reinterpret_cast<void*>(identity));
#else
    ...
    thread_local std::unique_ptr<ThreadIdentity, ThreadIdentityReclaimerFunction> holder(identity, reclaimer);
    thread_identity_ptr = identity;
#else
...
#elif ABSL_THREAD_IDENTITY_MODE == ABSL_THREAD_IDENTITY_MODE_USE_POSIX_SETSPECIFIC
...
ThreadIdentity* CurrentThreadIdentityIfPresent()
{
    ...
    return reinterpret_cast<ThreadIdentity*>(pthread_getspecific(thread_identity_pthread_key));
}
#endif
