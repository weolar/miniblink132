...
template <typename E> void ExpectThrowChar(void (*f)(const char*))
{
...
template <typename E> void ExpectThrowString(void (*f)(const std::string&))
{
#ifdef ABSL_HAVE_EXCEPTIONS
    try {
        f(what_arg);
...
template <typename E> void ExpectThrowNoWhat(void (*f)())
{
#ifdef ABSL_HAVE_EXCEPTIONS
    try {
        f();
