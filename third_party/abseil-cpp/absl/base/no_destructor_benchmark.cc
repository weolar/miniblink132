...
public:
    ...
    void Verify(int val) const
    { // val must be the c-tor argument
        for (auto& d : data_)
            ABSL_INTERNAL_CHECK(d == val, "");
    }
...
template <int i> const BM_Blob& NoDestrBlobFunc()
{
...
// We'll instantiate kNumObjects of them.
template <int i> const BM_Blob& OnHeapBlobFunc()
{
...
template <int n> FuncType BlobFunc(BM_Type t, int i)
{
    if (i == n) {
        switch (t) {
        case kNoDestr:
            return &NoDestrBlobFunc<n>;
        case kOnHeap:
            return &OnHeapBlobFunc<n>;
        case kDirect:
            return nullptr;
        }
    }
...
template <> FuncType BlobFunc<0>(BM_Type t, int i)
{
    ABSL_INTERNAL_CHECK(i == 0, "");
    switch (t) {
    case kNoDestr:
        return &NoDestrBlobFunc<0>;
    case kOnHeap:
        return &OnHeapBlobFunc<0>;
    case kDirect:
        return nullptr;
    }
    return nullptr;
...
template <BM_Type t> void BM_Many(benchmark::State& state)
{
...
void BM_DirectMany(benchmark::State& state)
{
    BM_Many<kDirect>(state);
}
void BM_NoDestrMany(benchmark::State& state)
{
    BM_Many<kNoDestr>(state);
}
void BM_OnHeapMany(benchmark::State& state)
{
    BM_Many<kOnHeap>(state);
}
