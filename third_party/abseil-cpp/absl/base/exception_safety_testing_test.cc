...
template <typename F> void ExpectNoThrow(const F& f)
{
    try {
        f();
...
template <typename F> void TestOp(const F& f)
{
    ExpectNoThrow(f);
...
TEST(ThrowingValueTest, StreamOpsOutput)
{
    ...
    EXPECT_NONFATAL_FAILURE(
        {
            using Thrower = ThrowingValue<TypeSpec::kNoThrowMove | TypeSpec::kNoThrowNew>;
...
template <typename F> void TestAllocatingOp(const F& f)
{
    ExpectNoThrow(f);
...
TEST(ThrowingValueTest, NonThrowingMoveCtor)
{
    ...
    SetCountdown();
    ExpectNoThrow([&nothrow_ctor]() { ThrowingValue<TypeSpec::kNoThrowMove> nothrow1 = std::move(nothrow_ctor); });
    UnsetCountdown();
}
...
TEST(ThrowingValueTest, NonThrowingMoveAssign)
{
    ...
    SetCountdown();
    ExpectNoThrow([&nothrow_assign1, &nothrow_assign2]() { nothrow_assign1 = std::move(nothrow_assign2); });
    UnsetCountdown();
}
...
TEST(ThrowingValueTest, NonThrowingCopyCtor)
{
    ...
    SetCountdown();
    ExpectNoThrow([&nothrow_ctor]() { ThrowingValue<TypeSpec::kNoThrowCopy> nothrow1(nothrow_ctor); });
    UnsetCountdown();
}
...
TEST(ThrowingValueTest, NonThrowingCopyAssign)
{
    ...
    SetCountdown();
    ExpectNoThrow([&nothrow_assign1, &nothrow_assign2]() { nothrow_assign1 = nothrow_assign2; });
    UnsetCountdown();
}
...
TEST(ThrowingValueTest, NonThrowingPlacementDelete)
{
    ...
    alignas(ThrowingValue<>) unsigned char array_buf[kExtraSpaceLen + sizeof(ThrowingValue<>[kArrayLen])];
    auto* placed = new (&buf) ThrowingValue<>(1);
    auto placed_array = new (&array_buf) ThrowingValue<>[kArrayLen];
    auto* placed_array_end = reinterpret_cast<unsigned char*>(placed_array) + sizeof(ThrowingValue<>[kArrayLen]);
...
TEST(ThrowingAllocatorTest, NonThrowingConstruction)
{
...

    {
        ...
        SetCountdown();
        ExpectNoThrow([&]() { nothrow_alloc.construct(ptr, 2, testing::nothrow_ctor); });
...
template <typename TesterInstance, typename = void> struct NullaryTestValidator : public std::false_type { };

template <typename TesterInstance>
struct NullaryTestValidator<TesterInstance, absl::void_t<decltype(std::declval<TesterInstance>().Test())>> : public std::true_type { };

template <typename TesterInstance> bool HasNullaryTest(const TesterInstance&)
{
...
template <typename TesterInstance, typename = void> struct UnaryTestValidator : public std::false_type { };

template <typename TesterInstance>
struct UnaryTestValidator<TesterInstance, absl::void_t<decltype(std::declval<TesterInstance>().Test(DummyOp))>> : public std::true_type { };

template <typename TesterInstance> bool HasUnaryTest(const TesterInstance&)
{
...
TEST(ExceptionSafetyTesterTest, IncompleteTypesAreNotTestable)
{
    ...
    auto without_fac = testing::MakeExceptionSafetyTester().WithOperation(op).WithContracts(inv, testing::strong_guarantee);
    ...
    auto without_op = testing::MakeExceptionSafetyTester().WithContracts(inv, testing::strong_guarantee).WithFactory(fac);
    ...
    auto without_inv = testing::MakeExceptionSafetyTester().WithOperation(op).WithFactory(fac);
...
auto example_lambda_contract = [](ExampleStruct* example_struct) { return ExampleFunctionContract(example_struct); };
...
testing::AssertionResult CheckNonNegativeInvariants(NonNegative* g)
{
    ...
    return testing::AssertionFailure() << "i should be non-negative but is " << g->i;
}
...
auto tester = testing::MakeExceptionSafetyTester().WithOperation(invoker).WithContracts(CheckNonNegativeInvariants);
...
{
    auto tester_with_val = tester.WithInitialValue(BasicGuaranteeWithExtraContracts {});
    ...
    EXPECT_TRUE(tester_with_val
                    .WithContracts([](BasicGuaranteeWithExtraContracts* o) {
                        ...
                        return testing::AssertionFailure() << "i should be " << BasicGuaranteeWithExtraContracts::kExceptionSentinel << ", but is " << o->i;
                    })
                    .Test());
}
...
TEST(ExceptionCheckTest, ModifyingChecker)
{
    ...
    auto is_1000 = [](FollowsBasicGuarantee* g) { return testing::AssertionResult(g->i == 1000); };
    auto increment = [](FollowsStrongGuarantee* g) {
        ++g->i;
        return testing::AssertionSuccess();
    };

    EXPECT_FALSE(tester.WithInitialValue(FollowsBasicGuarantee {}).WithContracts(set_to_1000, is_1000).Test());
    EXPECT_TRUE(strong_tester.WithInitialValue(FollowsStrongGuarantee {}).WithContracts(increment).Test());
    EXPECT_TRUE(testing::MakeExceptionSafetyTester().WithInitialValue(HasReset {}).WithContracts(CheckHasResetContracts).Test(invoker));
}

TEST(ExceptionSafetyTesterTest, ResetsCountdown)
{
    auto test = testing::MakeExceptionSafetyTester()
...
{
    auto nec_is_strong = [](NonEqualityComparable* nec) { return testing::AssertionResult(nec->i == NonEqualityComparable().i); };
    auto strong_nec_tester = tester.WithInitialValue(NonEqualityComparable {}).WithContracts(nec_is_strong);

    EXPECT_TRUE(strong_nec_tester.Test());
    EXPECT_FALSE(strong_nec_tester.Test([](NonEqualityComparable* n) { n->ModifyOnThrow(); }));
}

template <typename T> struct ExhaustivenessTester {
    void operator()()
    {
        successes |= 1;
        T b1;
...
struct {
    template <typename T> testing::AssertionResult operator()(ExhaustivenessTester<T>*) const
    {
        return testing::AssertionSuccess();
    }
} CheckExhaustivenessTesterContracts;

template <typename T> unsigned char ExhaustivenessTester<T>::successes = 0;

TEST(ExceptionCheckTest, Exhaustiveness)
{
    auto exhaust_tester = testing::MakeExceptionSafetyTester().WithContracts(CheckExhaustivenessTesterContracts).WithOperation(invoker);

    EXPECT_TRUE(exhaust_tester.WithInitialValue(ExhaustivenessTester<int> {}).Test());
    EXPECT_EQ(ExhaustivenessTester<int>::successes, 0xF);

    EXPECT_TRUE(exhaust_tester.WithInitialValue(ExhaustivenessTester<ThrowingValue<>> {}).WithContracts(testing::strong_guarantee).Test());
