...
struct Blob {
    ...
    ~Blob()
    {
        ABSL_INTERNAL_CHECK(moved_out, "~Blob");
    }
...
TEST(NoDestructorTest, Interface)
{
    ...
    EXPECT_TRUE(std::is_trivially_destructible<absl::NoDestructor<const Blob>>::value);
    {
    ...
    {
        const absl::NoDestructor<Blob> b { { 20, 28, 40 } }; // init-list c-tor, deep const
...
TEST(NoDestructorTest, SfinaeRegressionAbstractArg)
{
    ...
    EXPECT_EQ(foo1->i, 17);
    absl::NoDestructor<UsesAbstractInConstructor> foo2(static_cast<const Abstract&>(input));
    EXPECT_EQ(foo2->i, 17);
}
...
{
    EXPECT_TRUE(std::is_trivially_destructible<absl::NoDestructor<std::string>>::value);
    EXPECT_TRUE(std::is_trivially_destructible<absl::NoDestructor<MyArray>>::value);
    EXPECT_TRUE(std::is_trivially_destructible<absl::NoDestructor<MyVector>>::value);
...
TEST(NoDestructorTest, ClassTemplateArgumentDeduction)
{
    ...
    static_assert(std::is_same<decltype(i), absl::NoDestructor<int>>::value, "Expected deduced type to be int.");
}
#endif
