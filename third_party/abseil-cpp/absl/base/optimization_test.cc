...
TEST(PredictTest, PredictTrue)
{
    ...
    if (ABSL_PREDICT_TRUE(false))
        ADD_FAILURE();
    if (!ABSL_PREDICT_TRUE(true))
        ADD_FAILURE();
...
TEST(PredictTest, PredictFalse)
{
    ...
    if (ABSL_PREDICT_FALSE(false))
        ADD_FAILURE();
    if (!ABSL_PREDICT_FALSE(true))
        ADD_FAILURE();
...
public:
    explicit ImplictlyConvertibleToBool(bool value)
        : value_(value)
    {
    }
    operator bool() const
...
TEST(PredictTest, ImplicitBoolConversion)
{
    ...
    if (!ABSL_PREDICT_TRUE(is_true))
        ADD_FAILURE();
    if (ABSL_PREDICT_TRUE(is_false))
        ADD_FAILURE();
    if (!ABSL_PREDICT_FALSE(is_true))
        ADD_FAILURE();
    if (ABSL_PREDICT_FALSE(is_false))
        ADD_FAILURE();
}

class ExplictlyConvertibleToBool {
public:
    explicit ExplictlyConvertibleToBool(bool value)
        : value_(value)
    {
    }
