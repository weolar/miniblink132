...
template <typename RealType> class UniformRealDistributionTest : public ::testing::Test { };
...
TYPED_TEST(UniformRealDistributionTest, ParamSerializeTest)
{
    ...
    constexpr const real_type kEpsilon = std::numeric_limits<real_type>::epsilon();
    constexpr const real_type kLowest = std::numeric_limits<real_type>::lowest(); // -max

    const real_type kDenormMax = std::nextafter(kMin, real_type { 0 });
    const real_type kOneMinusE = std::nextafter(real_type { 1 }, real_type { 0 }); // 1 - epsilon
    ...
    for (const auto& param : {
        ...
        if (!std::is_same<real_type, long double>::value) {
            ...
            LOG(INFO) << "Range: " << static_cast<double>(sample_min) << ", " << static_cast<double>(sample_max);
        }
    }
}
...
TYPED_TEST(UniformRealDistributionTest, ViolatesPreconditionsDeathTest)
{
    ...
    EXPECT_DEBUG_DEATH({ DistributionType dist(std::numeric_limits<real_type>::lowest(), std::numeric_limits<real_type>::max()); }, "");

    // kEpsilon guarantees that max + kEpsilon = inf.
    const auto kEpsilon = std::nexttoward(
        (std::numeric_limits<real_type>::max() - std::nexttoward(std::numeric_limits<real_type>::max(), 0.0)) / 2, std::numeric_limits<real_type>::max());
    EXPECT_DEBUG_DEATH({ DistributionType dist(-kEpsilon, std::numeric_limits<real_type>::max()); }, "");
    EXPECT_DEBUG_DEATH({ DistributionType dist(std::numeric_limits<real_type>::lowest(), kEpsilon); }, "");
    ...
    {
        DistributionType dist(std::numeric_limits<real_type>::lowest(), std::numeric_limits<real_type>::max());
        auto x = dist(gen);
        // Infinite result.
...
TYPED_TEST(UniformRealDistributionTest, ChiSquaredTest50)
{
    ...
    constexpr double kExpected = static_cast<double>(kTrials) / static_cast<double>(kBuckets);
    ...
    const int kThreshold = absl::random_internal::ChiSquareValue(kBuckets - 1, 0.999999);
    ...
    for (const auto& param : { param_type(0, 1), param_type(5, 12), param_type(-5, 13), param_type(-5, -2) }) {
        ...
        double chi_square = absl::random_internal::ChiSquareWithExpected(std::begin(counts), std::end(counts), kExpected);
        if (chi_square > kThreshold) {
            double p_value = absl::random_internal::ChiSquarePValue(chi_square, kBuckets);
            ...
            absl::StrAppend(&msg, "High ", kChiSquared, " value: ", chi_square, " > ", kThreshold);
            LOG(INFO) << msg;
            FAIL() << msg;
        }
    }
...
TYPED_TEST(UniformRealDistributionTest, StabilityTest)
{
    ...
    DistributionType dist;
    std::generate(std::begin(output), std::end(output), [&] { return static_cast<int>(real_type(1000000) * dist(urbg)); });

    EXPECT_THAT(output, //
        testing::ElementsAre(59, 999246, 762494, 395876, 167716, 82545, 925251, 77341, 12527, 708791, 834451, 932808));
}
