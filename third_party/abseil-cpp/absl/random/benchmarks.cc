...
public:
    ...
    template <typename Iterator> PrecompiledSeedSeq(Iterator begin, Iterator end)
    {
    }

    template <typename T> PrecompiledSeedSeq(std::initializer_list<T> il)
    {
    }
    ...
    size_t size() const
    {
        return ABSL_ARRAYSIZE(kSeedData);
    }
...
template <typename E> using use_default_initialization = std::false_type;
...
template <typename Engine, typename SSeq = PrecompiledSeedSeq> typename absl::enable_if_t<!use_default_initialization<Engine>::value, Engine> make_engine()
{
...
template <typename Engine, typename SSeq> void BM_Construct(benchmark::State& state)
{
    for (auto _ : state) {
...
template <typename Engine> void BM_Direct(benchmark::State& state)
{
...
template <typename Engine> void BM_Generate(benchmark::State& state)
{
...
template <typename Engine, size_t elems> void BM_Shuffle(benchmark::State& state)
{
...
template <typename Engine, size_t elems> void BM_ShuffleReuse(benchmark::State& state)
{
    // Direct use of the Engine.
    std::vector<uint32_t> v(elems);
    auto rng = make_engine<Engine>();
...
template <typename Engine, typename Dist, typename... Args> void BM_Dist(benchmark::State& state, Args&&... args)
{
...
template <typename Engine, typename Dist> void BM_Large(benchmark::State& state)
{
    using value_type = typename Dist::result_type;
...
template <typename Engine, typename Dist> void BM_Small(benchmark::State& state)
{
    using value_type = typename Dist::result_type;
    volatile value_type kMin = 0;
...
template <typename Engine, typename Dist, int A> void BM_Bernoulli(benchmark::State& state)
{
    volatile double a = static_cast<double>(A) / 1000000;
    BM_Dist<Engine, Dist>(state, a);
}

template <typename Engine, typename Dist, int A, int B> void BM_Beta(benchmark::State& state)
{
    using value_type = typename Dist::result_type;
...
template <typename Engine, typename Dist, int A> void BM_Gamma(benchmark::State& state)
{
    using value_type = typename Dist::result_type;
    volatile value_type a = static_cast<value_type>(A) / 100;
    BM_Dist<Engine, Dist>(state, a);
...
template <typename Engine, typename Dist, int A = 100> void BM_Poisson(benchmark::State& state)
{
    volatile double a = static_cast<double>(A) / 100;
    BM_Dist<Engine, Dist>(state, a);
}

template <typename Engine, typename Dist, int Q = 2, int V = 1> void BM_Zipf(benchmark::State& state)
{
    using value_type = typename Dist::result_type;
    volatile double q = Q;
    volatile double v = V;
...
template <typename Engine, typename Dist> void BM_Thread(benchmark::State& state)
{
    using value_type = typename Dist::result_type;
    auto rng = make_engine<Engine>();
    Dist dis {};
...
#define BM_BASIC(Engine)                                                                                                                                       \
    ...
    BENCHMARK_TEMPLATE(BM_Dist, Engine, absl::random_internal::FastUniformBits<uint32_t>);                                                                     \
    BENCHMARK_TEMPLATE(BM_Dist, Engine, absl::random_internal::FastUniformBits<uint64_t>);                                                                     \
    ...
    BENCHMARK_TEMPLATE(BM_Dist, Engine, absl::uniform_int_distribution<int32_t>);                                                                              \
    BENCHMARK_TEMPLATE(BM_Dist, Engine, absl::uniform_int_distribution<int64_t>);                                                                              \
    BENCHMARK_TEMPLATE(BM_Large, Engine, std::uniform_int_distribution<int32_t>);                                                                              \
    BENCHMARK_TEMPLATE(BM_Large, Engine, std::uniform_int_distribution<int64_t>);                                                                              \
    BENCHMARK_TEMPLATE(BM_Large, Engine, absl::uniform_int_distribution<int32_t>);                                                                             \
    BENCHMARK_TEMPLATE(BM_Large, Engine, absl::uniform_int_distribution<int64_t>);                                                                             \
...
#define BM_THREAD(Engine)                                                                                                                                      \
    BENCHMARK_TEMPLATE(BM_Thread, Engine, absl::uniform_int_distribution<int64_t>)->ThreadPerCpu();                                                            \
    BENCHMARK_TEMPLATE(BM_Thread, Engine, absl::uniform_real_distribution<double>)->ThreadPerCpu();                                                            \
...
#define BM_EXTENDED(Engine)                                                                                                                                    \
    /* -------------- Extended Uniform -----------------------*/                                                                                               \
    BENCHMARK_TEMPLATE(BM_Small, Engine, std::uniform_int_distribution<int32_t>);                                                                              \
    BENCHMARK_TEMPLATE(BM_Small, Engine, std::uniform_int_distribution<int64_t>);                                                                              \
    BENCHMARK_TEMPLATE(BM_Small, Engine, absl::uniform_int_distribution<int32_t>);                                                                             \
    BENCHMARK_TEMPLATE(BM_Small, Engine, absl::uniform_int_distribution<int64_t>);                                                                             \
    BENCHMARK_TEMPLATE(BM_Small, Engine, std::uniform_real_distribution<float>);                                                                               \
    BENCHMARK_TEMPLATE(BM_Small, Engine, std::uniform_real_distribution<double>);                                                                              \
    BENCHMARK_TEMPLATE(BM_Small, Engine, absl::uniform_real_distribution<float>);                                                                              \
    BENCHMARK_TEMPLATE(BM_Small, Engine, absl::uniform_real_distribution<double>);                                                                             \
    /* -------------- Other -----------------------*/                                                                                                          \
    ...
    BENCHMARK_TEMPLATE(BM_Poisson, Engine, std::poisson_distribution<int64_t>, 100);                                                                           \
    BENCHMARK_TEMPLATE(BM_Poisson, Engine, absl::poisson_distribution<int64_t>, 100);                                                                          \
    BENCHMARK_TEMPLATE(BM_Poisson, Engine, std::poisson_distribution<int64_t>, 10 * 100);                                                                      \
    BENCHMARK_TEMPLATE(BM_Poisson, Engine, absl::poisson_distribution<int64_t>, 10 * 100);                                                                     \
    BENCHMARK_TEMPLATE(BM_Poisson, Engine, std::poisson_distribution<int64_t>, 13 * 100);                                                                      \
    BENCHMARK_TEMPLATE(BM_Poisson, Engine, absl::poisson_distribution<int64_t>, 13 * 100);                                                                     \
    BENCHMARK_TEMPLATE(BM_Dist, Engine, absl::log_uniform_int_distribution<int32_t>);                                                                          \
    BENCHMARK_TEMPLATE(BM_Dist, Engine, absl::log_uniform_int_distribution<int64_t>);                                                                          \
    ...
    BENCHMARK_TEMPLATE(BM_Zipf, Engine, absl::zipf_distribution<uint64_t>, 2, 3);                                                                              \
    BENCHMARK_TEMPLATE(BM_Bernoulli, Engine, std::bernoulli_distribution, 257305);                                                                             \
    BENCHMARK_TEMPLATE(BM_Bernoulli, Engine, absl::bernoulli_distribution, 257305);                                                                            \
    BENCHMARK_TEMPLATE(BM_Beta, Engine, absl::beta_distribution<double>, 65, 41);                                                                              \
    BENCHMARK_TEMPLATE(BM_Beta, Engine, absl::beta_distribution<double>, 99, 330);                                                                             \
    BENCHMARK_TEMPLATE(BM_Beta, Engine, absl::beta_distribution<double>, 150, 150);                                                                            \
    BENCHMARK_TEMPLATE(BM_Beta, Engine, absl::beta_distribution<double>, 410, 580);                                                                            \
    BENCHMARK_TEMPLATE(BM_Beta, Engine, absl::beta_distribution<float>, 65, 41);                                                                               \
    BENCHMARK_TEMPLATE(BM_Beta, Engine, absl::beta_distribution<float>, 99, 330);                                                                              \
    BENCHMARK_TEMPLATE(BM_Beta, Engine, absl::beta_distribution<float>, 150, 150);                                                                             \
    BENCHMARK_TEMPLATE(BM_Beta, Engine, absl::beta_distribution<float>, 410, 580);                                                                             \
