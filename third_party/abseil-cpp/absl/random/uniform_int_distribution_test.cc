...
template <typename IntType> class UniformIntDistributionTest : public ::testing::Test { };

using IntTypes = ::testing::Types<int8_t, uint8_t, int16_t, uint16_t, int32_t, uint32_t, int64_t, uint64_t>;
...
TYPED_TEST(UniformIntDistributionTest, ParamSerializeTest)
{
    ...
    using param_type = typename absl::uniform_int_distribution<TypeParam>::param_type;
    ...
    for (const auto& param : {
        ...
        absl::uniform_int_distribution<TypeParam> after(Limits::min() + 3, Limits::max() - 5);
...
TYPED_TEST(UniformIntDistributionTest, TestMoments)
{
    ...
    for (const auto& param : { param_type(0, Limits::max()), param_type(13, 127) }) {
...
TYPED_TEST(UniformIntDistributionTest, ChiSquaredTest50)
{
    ...
    // Empirically validated with --runs_per_test=10000.
    const int kThreshold = absl::random_internal::ChiSquareValue(kBuckets, 0.999999);
...
TEST(UniformIntDistributionTest, StabilityTest)
{
    ...
    EXPECT_EQ(12, urbg.invocations());
    EXPECT_THAT(output, testing::ElementsAre(97, 86, 75, 41, 36, 16, 38, 92, 67, 30, 80, 38));
    ...
    EXPECT_EQ(12, urbg.invocations());
    EXPECT_THAT(output, testing::ElementsAre(9648, 8562, 7439, 4089, 3571, 1602, 3813, 9195, 6641, 2986, 7956, 3765));
}
