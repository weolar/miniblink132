...
template <typename IntType> class DiscreteDistributionTypeTest : public ::testing::Test { };
...
{
    using param_type = typename absl::discrete_distribution<TypeParam>::param_type;
    ...
    // Validate the same data via an initializer list.
    {
        ...
        absl::discrete_distribution<TypeParam> via_param { param_type(std::begin(data), std::end(data)) };
...
TEST(DiscreteDistributionTest, InitDiscreteDistribution)
{
...

    {
        ...
        std::vector<std::pair<double, size_t>> q = absl::random_internal::InitDiscreteDistribution(&p);
        ...
        EXPECT_THAT(q,
            testing::ElementsAre(Pair(0.5, 2), //
                Pair(1.0, _), //
                Pair(1.0, _)));
    }

    {
        ...
        EXPECT_THAT(p, testing::ElementsAre(1 / 13.0, 2 / 13.0, 3 / 13.0, 5 / 13.0, 2 / 13.0));
...
TEST(DiscreteDistributionTest, ChiSquaredTest50)
{
    ...
    const int kThreshold = absl::random_internal::ChiSquareValue(kBuckets, 0.99999);
    ...
    double chi_square = absl::random_internal::ChiSquare(std::begin(counts), std::end(counts), std::begin(weights), std::end(weights));
