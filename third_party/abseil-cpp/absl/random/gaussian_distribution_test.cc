...
template <typename RealType> class GaussianDistributionInterfaceTest : public ::testing::Test { };
...
using RealTypes
    = std::conditional<absl::numeric_internal::IsDoubleDouble(), ::testing::Types<float, double>, ::testing::Types<float, double, long double>>::type;
TYPED_TEST_SUITE(GaussianDistributionInterfaceTest, RealTypes);

TYPED_TEST(GaussianDistributionInterfaceTest, SerializeTest)
{
    using param_type = typename absl::gaussian_distribution<TypeParam>::param_type;
    ...
    const TypeParam kParams[] = {
        ...
        // Arbitrary values.
        TypeParam(1e-8), TypeParam(1e-4), TypeParam(2), TypeParam(1e4), TypeParam(1e8), TypeParam(1e20), TypeParam(2.5),
        // Boundary cases.
        std::numeric_limits<TypeParam>::infinity(), std::numeric_limits<TypeParam>::max(), std::numeric_limits<TypeParam>::epsilon(),
        ...
        for (const auto x : kParams) {
            ...
            for (const auto y : kParams) {
                ...
                if (!std::is_same<TypeParam, long double>::value) {
                    LOG(INFO) << "Range{" << mean << ", " << stddev << "}: " << sample_min << ", " << sample_max;
                }
...
class GaussianModel {
public:
    GaussianModel(double mean, double stddev)
        : mean_(mean)
        , stddev_(stddev)
    {
    }
...
// distribution.
class GaussianDistributionTests : public testing::TestWithParam<Param>, public GaussianModel {
public:
    ...
    template <typename D> bool SingleZTest(const double p, const size_t samples);

    // SingleChiSquaredTest provides a basic chi-squared test of the normal
    // distribution.
    template <typename D> double SingleChiSquaredTest();
...
template <typename D> bool GaussianDistributionTests::SingleZTest(const double p, const size_t samples)
{
    D dis(mean(), stddev());
    ...
    //
    const double jb = static_cast<double>(m.n) / 6.0 * (std::pow(m.skewness, 2.0) + std::pow(m.kurtosis - 3.0, 2.0) / 4.0);

    if (!pass || jb > 9.21) {
        // clang-format on
        LOG(INFO) << "p=" << p << " max_err=" << max_err
                  << "\n"
                     " mean="
                  << m.mean << " vs. " << mean()
                  << "\n"
                     " stddev="
                  << std::sqrt(m.variance) << " vs. " << stddev()
                  << "\n"
                     " skewness="
                  << m.skewness << " vs. " << skew()
                  << "\n"
                     " kurtosis="
                  << m.kurtosis << " vs. " << kurtosis()
                  << "\n"
                     " z="
                  << z
                  << " vs. 0\n"
...
template <typename D> double GaussianDistributionTests::SingleChiSquaredTest()
{
    ...
    const double expected = static_cast<double>(kSamples) / static_cast<double>(counts.size());

    double chi_square = absl::random_internal::ChiSquareWithExpected(std::begin(counts), std::end(counts), expected);
...
TEST_P(GaussianDistributionTests, ZTest)
{
    ...
    const int expected_failures = std::max(1, static_cast<int>(std::ceil(param.trials * param.p_fail)));
    const double p = absl::random_internal::RequiredSuccessProbability(param.p_fail, param.trials);

    int failures = 0;
    for (int i = 0; i < param.trials; i++) {
        failures += SingleZTest<absl::gaussian_distribution<double>>(p, kSamples) ? 0 : 1;
    }
...
TEST_P(GaussianDistributionTests, ChiSquaredTest)
{
    ...
    for (int i = 0; i < kTrials; i++) {
        double p_value = SingleChiSquaredTest<absl::gaussian_distribution<double>>();
...
std::string ParamName(const ::testing::TestParamInfo<Param>& info)
{
    ...
    std::string name = absl::StrCat("mean_", absl::SixDigits(p.mean), "__stddev_", absl::SixDigits(p.stddev));
    return absl::StrReplaceAll(name, { { "+", "_" }, { "-", "_" }, { ".", "_" } });
}

INSTANTIATE_TEST_SUITE_P(All, GaussianDistributionTests, ::testing::ValuesIn(GenParams()), ParamName);
...
TEST(GaussianDistributionTest, StabilityTest)
{
    ...
    absl::random_internal::sequence_urbg urbg(
        { 0x0003eb76f6f7f755ull, 0xFFCEA50FDB2F953Bull, 0xC332DDEFBE6C5AA5ull, 0x6558218568AB9702ull, 0x2AEF7DAD5B6E2F84ull, 0x1521B62829076170ull,
            0xECDD4775619F1510ull, 0x13CCA830EB61BD96ull, 0x0334FE1EAA0363CFull, 0xB5735C904C70A239ull, 0xD59E9E0BCBAADE14ull, 0xEECC86BC60622CA7ull });

    std::vector<int> output(11);

    {
        absl::gaussian_distribution<double> dist;
        std::generate(std::begin(output), std::end(output), [&] { return static_cast<int>(10000000.0 * dist(urbg)); });

        EXPECT_EQ(13, urbg.invocations());
        EXPECT_THAT(output, //
            testing::ElementsAre(1494, 25518841, 9991550, 1351856, -20373238, 3456682, 333530, -6804981, -15279580, -16459654, 1494));
    }

    urbg.reset();
    {
        absl::gaussian_distribution<float> dist;
        std::generate(std::begin(output), std::end(output), [&] { return static_cast<int>(1000000.0f * dist(urbg)); });

        EXPECT_EQ(13, urbg.invocations());
        EXPECT_THAT(output, //
            testing::ElementsAre(149, 2551884, 999155, 135185, -2037323, 345668, 33353, -680498, -1527958, -1645965, 149));
    }
}
...
TEST(GaussianDistributionTest, AlgorithmBounds)
{
    ...
    const uint64_t kValues[]
        = { 0x1000000000000100ull, 0x2000000000000100ull, 0x3000000000000100ull, 0x4000000000000100ull, 0x5000000000000100ull, 0x6000000000000100ull,
              // negative values
              0x9000000000000100ull, 0xa000000000000100ull, 0xb000000000000100ull, 0xc000000000000100ull, 0xd000000000000100ull, 0xe000000000000100ull };

    // 0.875 <= |values| <= 0.984375
    const uint64_t kExtraValues[] = { 0x7000000000000100ull, 0x7800000000000100ull, //
    ...
    auto make_box = [](uint64_t v, uint64_t box) { return (v & 0xffffffffffffff80ull) | box; };
    ...
    for (uint64_t box = 0; box < 0x7f; box++) {
        for (const uint64_t v : kValues) {
            ...
            absl::random_internal::sequence_urbg urbg({ make_box(v, box), 0x0003eb76f6f7f755ull, 0x5FCEA50FDB2F953Bull });
    ...
    {
        // 0.9375
        absl::random_internal::sequence_urbg urbg({ make_fallback(0x7800000000000000ull), 0x13CCA830EB61BD96ull, 0x00000076f6f7f755ull });
        tail[0] = dist(urbg);
    ...
    {
        // -0.9375
        absl::random_internal::sequence_urbg urbg({ make_fallback(0xf800000000000000ull), 0x13CCA830EB61BD96ull, 0x00000076f6f7f755ull });
        tail[1] = dist(urbg);
        EXPECT_EQ(3, urbg.invocations());
        EXPECT_LT(tail[1], 0);
    }
    ...
    {
        // 0.9921875, 0.875
        absl::random_internal::sequence_urbg urbg({ make_box(0x7f00000000000000ull, 120), 0xe000000000000001ull, 0x13CCA830EB61BD96ull });
        tail[0] = dist(urbg);
    ...
    {
        // -0.9921875, 0.875
        absl::random_internal::sequence_urbg urbg({ make_box(0xff00000000000000ull, 120), 0xe000000000000001ull, 0x13CCA830EB61BD96ull });
        tail[1] = dist(urbg);
        EXPECT_EQ(2, urbg.invocations());
        EXPECT_LT(tail[1], 0);
    }
    ...
    // Fallback rejected, try again.
    {
        // -0.9921875, 0.0625
        absl::random_internal::sequence_urbg urbg(
            { make_box(0xff00000000000000ull, 120), 0x1000000000000001, make_box(0x1000000000000100ull, 50), 0x13CCA830EB61BD96ull });
        dist(urbg);
        EXPECT_EQ(3, urbg.invocations());
    }
}
