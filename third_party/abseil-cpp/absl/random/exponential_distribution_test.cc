...
template <typename RealType> class ExponentialDistributionTypedTest : public ::testing::Test { };
...
TYPED_TEST(ExponentialDistributionTypedTest, SerializeTest)
{
    ...
    const TypeParam kParams[] = {
        ...
        // Typical cases.
        TypeParam(1e-8), TypeParam(1e-4), TypeParam(1), TypeParam(2), TypeParam(1e4), TypeParam(1e8), TypeParam(1e20), TypeParam(2.5),
        // Boundary cases.
    ...
    for (const TypeParam lambda : kParams) {
        ...
        if (!std::is_same<TypeParam, long double>::value) {
            LOG(INFO) << "Range {" << lambda << "}: " << sample_min << ", " << sample_max << ", lambda=" << lambda;
        }
...
class ExponentialDistributionTests : public testing::TestWithParam<Param>, public ExponentialModel {
public:
    ExponentialDistributionTests()
        : ExponentialModel(GetParam().lambda)
    {
    }
...
template <typename D> bool ExponentialDistributionTests::SingleZTest(const double p, const size_t samples)
{
    D dis(lambda());
...
template <typename D> double ExponentialDistributionTests::SingleChiSquaredTest()
{
...
TEST_P(ExponentialDistributionTests, ZTest)
{
    ...
    for (int i = 0; i < param.trials; i++) {
        failures += SingleZTest<absl::exponential_distribution<double>>(p, kSamples) ? 0 : 1;
    }
...
TEST_P(ExponentialDistributionTests, ChiSquaredTest)
{
    ...
    for (int i = 0; i < kTrials; i++) {
        double p_value = SingleChiSquaredTest<absl::exponential_distribution<double>>();
...
INSTANTIATE_TEST_SUITE_P(All, ExponentialDistributionTests, ::testing::ValuesIn(GenParams()), ParamName);
...
TEST(ExponentialDistributionTest, StabilityTest)
{
...

    {
        ...
        std::generate(std::begin(output), std::end(output), [&] { return static_cast<int>(10000.0 * dist(urbg)); });

        EXPECT_EQ(14, urbg.invocations());
        EXPECT_THAT(output, testing::ElementsAre(0, 71913, 14375, 5039, 1835, 861, 25936, 804, 126, 12337, 17984, 27002, 0, 71913));
    }

    urbg.reset();
    {
        absl::exponential_distribution<float> dist;
        std::generate(std::begin(output), std::end(output), [&] { return static_cast<int>(10000.0f * dist(urbg)); });
