...
template <typename IntType> class LogUniformIntDistributionTypeTest : public ::testing::Test { };
...
{
    using param_type = typename absl::log_uniform_int_distribution<TypeParam>::param_type;
...
class LogUniformIntChiSquaredTest : public testing::TestWithParam<log_uniform_i32::param_type> {
public:
...
double LogUniformIntChiSquaredTest::ChiSquaredTestImpl()
{
    ...
    const auto bucket_index = [base_log, is_2, &param](int32_t x) {
        ...
        return (y == 0) ? 0 : is_2 ? static_cast<int>(1 + std::log2(y)) : static_cast<int>(1 + std::log(y) * base_log);
    };
    ...
    double chi_square = absl::random_internal::ChiSquareWithExpected(std::begin(buckets), std::end(buckets), expected);
...
std::string ParamName(const ::testing::TestParamInfo<log_uniform_i32::param_type>& info)
{
    const auto& p = info.param;
    std::string name = absl::StrCat("min_", p.min(), "__max_", p.max(), "__base_", p.base());
    return absl::StrReplaceAll(name, { { "+", "_" }, { "-", "_" }, { ".", "_" } });
}

INSTANTIATE_TEST_SUITE_P(All, LogUniformIntChiSquaredTest, ::testing::ValuesIn(GenParams()), ParamName);
