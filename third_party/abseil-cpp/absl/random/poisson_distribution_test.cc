...
template <typename IntType> class PoissonDistributionInterfaceTest : public ::testing::Test { };

using IntTypes = ::testing::Types<int, int8_t, int16_t, int32_t, int64_t, uint8_t, uint16_t, uint32_t, uint64_t>;
...
TYPED_TEST(PoissonDistributionInterfaceTest, SerializeTest)
{
    ...
    const double kMax = std::min(1e10 /* assertion limit */, static_cast<double>(std::numeric_limits<TypeParam>::max()));
    ...
    for (const double m : kParams) {
        ...
        LOG(INFO) << "Range {" << param.mean() << "}: " << sample_min << ", " << sample_max;
...
class PoissonModel {
public:
    explicit PoissonModel(double mean)
        : mean_(mean)
    {
    }
    ...
    {
        CDF target { 0, 0, p };
        auto it = std::upper_bound(std::begin(cdf_), std::end(cdf_), target, [](const CDF& a, const CDF& b) { return a.cdf < b.cdf; });
        return *it;
    }
...
class PoissonDistributionZTest : public testing::TestWithParam<ZParam>, public PoissonModel {
public:
    PoissonDistributionZTest()
        : PoissonModel(GetParam().mean)
    {
    }
...
template <typename D> bool PoissonDistributionZTest::SingleZTest(const double p, const size_t samples)
{
    D dis(mean());
...
TEST_P(PoissonDistributionZTest, AbslPoissonDistribution)
{
    ...
    for (int i = 0; i < param.trials; i++) {
        failures += SingleZTest<absl::poisson_distribution<int32_t>>(p, param.samples) ? 0 : 1;
    }
...
INSTANTIATE_TEST_SUITE_P(All, PoissonDistributionZTest, ::testing::ValuesIn(GetZParams()), ZParamName);
...
class PoissonDistributionChiSquaredTest : public testing::TestWithParam<double>, public PoissonModel {
public:
    PoissonDistributionChiSquaredTest()
        : PoissonModel(GetParam())
    {
    }

    // The ChiSquaredTestImpl provides a chi-squared goodness of fit test for data
    // generated by the poisson distribution.
    template <typename D> double ChiSquaredTestImpl();
...
void PoissonDistributionChiSquaredTest::InitChiSquaredTest(const double buckets)
{
...
template <typename D> double PoissonDistributionChiSquaredTest::ChiSquaredTestImpl()
{
    ...
    const double chi_square = absl::random_internal::ChiSquare(std::begin(counts), std::end(counts), std::begin(e), std::end(e));
    ...
    if (chi_square > threshold) {
        ...
        LOG(INFO) << "VALUES  buckets=" << counts.size() << "  samples=" << kSamples;
        for (size_t i = 0; i < counts.size(); i++) {
            LOG(INFO) << cutoffs_[i] << ": " << counts[i] << " vs. E=" << e[i];
        }

        LOG(INFO) << kChiSquared << "(data, dof=" << dof << ") = " << chi_square << " (" << p << ")\n"
                  << " vs.\n"
...
INSTANTIATE_TEST_SUITE_P(All, PoissonDistributionChiSquaredTest, ::testing::Values(0.5, 1.0, 2.0, 10.0, 50.0, 51.0, 200.0));
...
TEST(PoissonDistributionTest, StabilityTest)
{
    ...
    // Method 1.
    {
        ...
        std::generate(std::begin(output), std::end(output), [&] { return dist(urbg); });
    }
...
TEST(PoissonDistributionTest, AlgorithmExpectedValue_3)
{
    ...
    absl::random_internal::sequence_urbg urbg({ 0x7fffffffffffffffull, 0x8000000000000000ull });
