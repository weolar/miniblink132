...
template <typename IntType> class ZipfDistributionTypedTest : public ::testing::Test { };
...
class ZipfModel {
public:
    ZipfModel(size_t k, double q, double v)
        : k_(k)
        , q_(q)
        , v_(v)
    {
    ...
    double PMF(size_t i)
    {
        return i >= hnq_.size() ? 0.0 : hnq_[i] / sum_hnq_;
    }
    ...
    void Init()
    {
        ...
        for (size_t i = 0; i < k_; i++) {
            ...
            // H(n, q-1)
            const double hnqm1 = (q_ == 2.0) ? (1.0 / x) : (q_ == 3.0) ? (1.0 / (x * x)) : std::pow(x, -qm1);
            sum_hnq_m1 += hnqm1;

            // H(n, q)
            const double hnq = (q_ == 2.0) ? (1.0 / (x * x)) : (q_ == 3.0) ? (1.0 / (x * x * x)) : std::pow(x, -q_);
            sum_hnq_ += hnq;
            hnq_.push_back(hnq);
            if (i > 1000 && hnq <= 1e-10) {
...
class ZipfTest : public testing::TestWithParam<zipf_u64::param_type>, public ZipfModel {
public:
    ZipfTest()
        : ZipfModel(GetParam().k(), GetParam().q(), GetParam().v())
    {
    }
...
TEST_P(ZipfTest, ChiSquaredTest)
{
    ...
    double avg = 0;
    {
        ...
        for (size_t i = 0; i < trials; i++) {
            ...
            auto it = std::upper_bound(std::begin(points), std::end(points), static_cast<size_t>(x));
    ...
    const double chi_square = absl::random_internal::ChiSquare(std::begin(buckets), std::end(buckets), std::begin(expected), std::end(expected));

    const double p_actual = absl::random_internal::ChiSquarePValue(chi_square, dof);
    ...
    if (chi_square > threshold) {
        ...
        LOG(INFO) << kChiSquared << "(data, " << dof << ") = " << chi_square << " (" << p_actual << ")";
        LOG(INFO) << kChiSquared << " @ 0.9995 = " << threshold;
        FAIL() << kChiSquared << " value of " << chi_square << " is above the threshold.";
    }
}
...
std::vector<zipf_u64::param_type> GenParams()
{
    ...
    return std::vector<zipf_u64::param_type> { // Default
        param(k, q, v),
        // vary K
...
std::string ParamName(const ::testing::TestParamInfo<zipf_u64::param_type>& info)
{
    const auto& p = info.param;
    std::string name = absl::StrCat("k_", p.k(), "__q_", absl::SixDigits(p.q()), "__v_", absl::SixDigits(p.v()));
    return absl::StrReplaceAll(name, { { "+", "_" }, { "-", "_" }, { ".", "_" } });
}

INSTANTIATE_TEST_SUITE_P(All, ZipfTest, ::testing::ValuesIn(GenParams()), ParamName);
...
TEST(ZipfDistributionTest, StabilityTest)
{
    ...
    urbg.reset();
    {
        absl::zipf_distribution<int32_t> dist(std::numeric_limits<int32_t>::max(), 3.3);
        std::generate(std::begin(output), std::end(output), [&] { return dist(urbg); });
