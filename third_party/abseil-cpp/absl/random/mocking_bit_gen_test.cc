...
TEST(BasicMocking, AllDistributionsAreOverridable)
{
    ...
    EXPECT_CALL(absl::MockLogUniform<int>(), Call(gen, 0, 1000000, 2)).WillOnce(Return(500000));
...
TEST(BasicMocking, OnDistribution)
{
    ...
    EXPECT_NE(absl::Uniform<int>(gen, 1, 1000000), 20);
    ON_CALL(absl::MockUniform<int>(), Call(gen, 1, 1000000)).WillByDefault(Return(20));
    EXPECT_EQ(absl::Uniform<int>(gen, 1, 1000000), 20);

    EXPECT_NE(absl::Uniform<double>(gen, 0.0, 100.0), 5.0);
    ON_CALL(absl::MockUniform<double>(), Call(gen, 0.0, 100.0)).WillByDefault(Return(5.0));
    EXPECT_EQ(absl::Uniform<double>(gen, 0.0, 100.0), 5.0);

    EXPECT_NE(absl::Exponential<double>(gen, 1.0), 42);
    ON_CALL(absl::MockExponential<double>(), Call(gen, 1.0)).WillByDefault(Return(42));
    EXPECT_EQ(absl::Exponential<double>(gen, 1.0), 42);
    ...
    EXPECT_NE(absl::Bernoulli(gen, 0.000001), true);
    ON_CALL(absl::MockBernoulli(), Call(gen, 0.000001)).WillByDefault(Return(true));
    EXPECT_EQ(absl::Bernoulli(gen, 0.000001), true);

    EXPECT_NE(absl::Zipf<int>(gen, 1000000, 2.0, 1.0), 1221);
    ON_CALL(absl::MockZipf<int>(), Call(gen, 1000000, 2.0, 1.0)).WillByDefault(Return(1221));
    EXPECT_EQ(absl::Zipf<int>(gen, 1000000, 2.0, 1.0), 1221);

    EXPECT_NE(absl::Gaussian<double>(gen, 0.0, 1.0), 0.001);
    ON_CALL(absl::MockGaussian<double>(), Call(gen, 0.0, 1.0)).WillByDefault(Return(0.001));
    EXPECT_EQ(absl::Gaussian<double>(gen, 0.0, 1.0), 0.001);

    EXPECT_NE(absl::LogUniform<int>(gen, 0, 1000000, 2), 2040);
    ON_CALL(absl::MockLogUniform<int>(), Call(gen, 0, 1000000, 2)).WillByDefault(Return(2040));
...
TEST(BasicMocking, OverridesWithMultipleGMockExpectations)
{
    ...
    EXPECT_CALL(absl::MockUniform<int>(), Call(gen, 1, 10000)).WillOnce(Return(20)).WillOnce(Return(40)).WillOnce(Return(60));
...
TEST(BasicMocking, DefaultArgument)
{
    ...
    ON_CALL(absl::MockExponential<double>(), Call(gen, 1.0)).WillByDefault(Return(200));
...
{
    auto get_value = [](absl::BitGenRef gen_ref) { return absl::Uniform(gen_ref, 1, 1000000); };
    absl::MockingBitGen unmocked_generator;
    absl::MockingBitGen mocked_with_3;
    absl::MockingBitGen mocked_with_11;

    EXPECT_CALL(absl::MockUniform<int>(), Call(mocked_with_3, 1, 1000000)).WillOnce(Return(3)).WillRepeatedly(Return(17));
    EXPECT_CALL(absl::MockUniform<int>(), Call(mocked_with_11, 1, 1000000)).WillOnce(Return(11)).WillRepeatedly(Return(17));
...
{
    absl::MockingBitGen gen;
    EXPECT_CALL(absl::MockUniform<uint32_t>(), Call(gen)).WillRepeatedly(Return(42));
...
TEST(BasicMocking, FailsOnUnsatisfiedMocks)
{
    EXPECT_NONFATAL_FAILURE(
        []() {
            absl::MockingBitGen gen;
            EXPECT_CALL(absl::MockExponential<double>(), Call(gen, 1.0)).WillOnce(Return(3.0));
...
TEST(OnUniform, RespectsUniformIntervalSemantics)
{
    ...
    EXPECT_CALL(absl::MockUniform<int>(), Call(absl::IntervalClosed, gen, 1, 1000000)).WillOnce(Return(301));
...
TEST(RepeatedlyModifier, ForceSnakeEyesForManyDice)
{
    ...
    EXPECT_LT(std::accumulate(std::begin(results), std::end(results), 0), results.size() * 6);

    // Verify that we can force all "6"-rolls, with mocking.
    ON_CALL(absl::MockUniform<int>(), Call(absl::IntervalClosed, gen, 1, 6)).WillByDefault(Return(6));
    results = roll_some_dice(gen);
    EXPECT_EQ(std::accumulate(std::begin(results), std::end(results), 0), results.size() * 6);
}

TEST(WillOnce, DistinctCounters)
{
    absl::MockingBitGen gen;
    EXPECT_CALL(absl::MockUniform<int>(), Call(gen, 1, 1000000)).Times(3).WillRepeatedly(Return(1));
    EXPECT_CALL(absl::MockUniform<int>(), Call(gen, 1000001, 2000000)).Times(3).WillRepeatedly(Return(1000001));
...
TEST(TimesModifier, ModifierSaturatesAndExpires)
{
    EXPECT_NONFATAL_FAILURE(
        []() {
            absl::MockingBitGen gen;
            EXPECT_CALL(absl::MockUniform<int>(), Call(gen, 0, 1000000)).Times(3).WillRepeatedly(Return(15)).RetiresOnSaturation();
...
TEST(AnythingMatcher, MatchesAnyArgument)
{
    ...
    {
        absl::MockingBitGen gen;
        ON_CALL(absl::MockUniform<int>(), Call(absl::IntervalClosed, gen, _, 1000)).WillByDefault(Return(11));
        ON_CALL(absl::MockUniform<int>(), Call(absl::IntervalClosed, gen, _, Ne(1000))).WillByDefault(Return(99));
    ...
    {
        absl::MockingBitGen gen;
        ON_CALL(absl::MockUniform<int>(), Call(gen, 1, _)).WillByDefault(Return(25));
        ON_CALL(absl::MockUniform<int>(), Call(gen, Ne(1), _)).WillByDefault(Return(99));
...
TEST(AnythingMatcher, WithWillByDefault)
{
    ...
    ON_CALL(absl::MockUniform<size_t>(), Call(gen, 0, _)).WillByDefault(Return(0));
    for (int i = 0; i < 100; i++) {
...
TEST(BasicMocking, WillByDefaultWithArgs)
{
    ...
    absl::MockingBitGen gen;
    ON_CALL(absl::MockPoisson<int>(), Call(gen, _)).WillByDefault([](double lambda) { return static_cast<int>(std::rint(lambda * 10)); });
...
TEST(MockingBitGen, StrictMock_TooMany)
{
    ...
    EXPECT_NONFATAL_FAILURE([&]() { EXPECT_EQ(absl::Uniform(gen, 0, 1000), 0); }(), "over-saturated and active");
}
