...
template <typename Sseq> void ConformsToInterface()
{
...
TEST(SaltedSeedSeq, SaltedSaltedSeedSeqIsNotDoubleSalted)
{
    ...
    // The second salting.
    SaltedSeedSeq<std::seed_seq> salted_salted_seq = MakeSaltedSeedSeq(std::move(salted_seq));
    uint32_t b[16];
...
TEST(SaltedSeedSeq, SeedMaterialIsSalted)
{
    ...
    SaltedSeedSeq<std::seed_seq> salted_seq(std::begin(seed_material), std::end(seed_material));
