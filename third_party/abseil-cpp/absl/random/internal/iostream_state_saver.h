...
template <typename T> class null_state_saver {
public:
    using stream_type = T;
...
template <typename OStream> class ostream_state_saver {
public:
...
ostream_state_saver<std::basic_ostream<CharT, Traits>> make_ostream_state_saver(std::basic_ostream<CharT, Traits>& os, // NOLINT(runtime/references)
...
template <typename T> struct stream_precision_helper {
    // max_digits10 may be 0 on MSVC; if so, use digits10 + 3.
    static constexpr int kPrecision = (std::numeric_limits<T>::max_digits10 > std::numeric_limits<T>::digits10) ? std::numeric_limits<T>::max_digits10
                                                                                                                : (std::numeric_limits<T>::digits10 + 3);
};

template <> struct stream_precision_helper<float> {
    static constexpr int kPrecision = 9;
};
template <> struct stream_precision_helper<double> {
    static constexpr int kPrecision = 17;
};
template <> struct stream_precision_helper<long double> {
...
// the absl random distributions.
template <typename IStream> class istream_state_saver {
public:
...
template <typename CharT, typename Traits>
istream_state_saver<std::basic_istream<CharT, Traits>> make_istream_state_saver(std::basic_istream<CharT, Traits>& is, // NOLINT(runtime/references)
...
//
template <typename T> struct stream_format_type : public std::conditional<(sizeof(T) == sizeof(char)), int, T> { };
...
template <typename T> struct stream_u128_helper;

template <> struct stream_u128_helper<absl::uint128> {
    template <typename IStream> inline absl::uint128 read(IStream& in)
    {
        uint64_t h = 0;
        uint64_t l = 0;
        in >> h >> l;
    ...
    template <typename OStream> inline void write(absl::uint128 val, OStream& out)
    {
...
template <> struct stream_u128_helper<__uint128_t> {
    template <typename IStream> inline __uint128_t read(IStream& in)
    {
        uint64_t h = 0;
        uint64_t l = 0;
        in >> h >> l;
    ...
    template <typename OStream> inline void write(__uint128_t val, OStream& out)
    {
...
template <typename FloatType, typename IStream> inline FloatType read_floating_point(IStream& is)
{
    ...
    if (is.fail() && (std::fabs(dest) == (std::numeric_limits<FloatType>::min)() || std::fpclassify(dest) == FP_SUBNORMAL)) {
