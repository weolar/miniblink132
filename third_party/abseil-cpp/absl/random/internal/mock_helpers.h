...
struct NoOpValidator {
    ...
    template <typename ResultT, typename... Args> static void Validate(ResultT, Args&&...)
    {
    }
};
...
class MockHelpers {
    ...
    template <typename KeyT> struct KeySignature;

    template <typename ResultT, typename DiscriminatorT, typename ArgTupleT> struct KeySignature<ResultT(DiscriminatorT, ArgTupleT)> {
    ...
    template <class T> using invoke_mock_t = decltype(std::declval<T*>()->InvokeMock(std::declval<IdType>(), std::declval<void*>(), std::declval<void*>()));
...
public:
    ...
    template <typename URBG> static inline bool PrivateInvokeMock(URBG* urbg, IdType type, void* args_tuple, void* result)
    {
    ...
    static auto MockFor(MockURBG& m, ValidatorT)
        -> decltype(m.template RegisterMock<typename KeySignature<KeyT>::result_type, typename KeySignature<KeyT>::arg_tuple_type>(
    ...
    // The mocked function signature will be composed from KeyT as:
    //   result_type(args...)
    template <typename KeyT, typename MockURBG> static decltype(auto) MockFor(MockURBG& m)
    {
