...
{
    static_assert(stream_precision_helper<float>::kPrecision >= 9, "stream_precision_helper<float>::kPrecision should be at least 9");
    ...
    for (const float f : kValues) {
        ...
        if (f <= static_cast<float>(std::numeric_limits<int64_t>::max()) && f >= static_cast<float>(std::numeric_limits<int64_t>::lowest())) {
...
{
    static_assert(stream_precision_helper<double>::kPrecision >= 17, "stream_precision_helper<double>::kPrecision should be at least 17");
    ...
    for (const double d : kValues) {
        ...
        // Avoid undefined behavior (overflow/underflow).
        if (d <= static_cast<double>(std::numeric_limits<int64_t>::max()) && d >= static_cast<double>(std::numeric_limits<int64_t>::lowest())) {
...
TEST(IOStreamStateSaver, RoundTripLongDoubles)
{
    ...
    static_assert(stream_precision_helper<long double>::kPrecision >= 36, "stream_precision_helper<long double>::kPrecision should be at least 36");
    ...
    for (const long double dd : kValues) {
        ...
        // Avoid undefined behavior (overflow/underflow).
        if (dd <= std::numeric_limits<double>::max() && dd >= std::numeric_limits<double>::lowest()) {
            double d = static_cast<double>(dd);
            EXPECT_EQ(d, StreamRoundTrip<double>(d));
        }

        // Avoid undefined behavior (overflow/underflow).
        if (dd <= static_cast<long double>(std::numeric_limits<int64_t>::max()) && dd >= static_cast<long double>(std::numeric_limits<int64_t>::lowest())) {
