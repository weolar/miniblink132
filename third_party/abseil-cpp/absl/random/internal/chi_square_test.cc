...
TEST(ChiSquareTest, CalcChiSquare)
{
    struct {
        ...
        { __LINE__, { 56, 234, 76, 1, 546, 1, 87, 345, 1, 234 }, { 2, 132, 4, 43, 234, 8, 345, 8, 236, 56 } },
        { __LINE__, { 123, 36, 234, 367, 345, 2, 456, 567, 234, 567 }, { 123, 56, 2345, 8, 345, 8, 2345, 23, 48, 267 } },
        { __LINE__, { 123, 234, 345, 456, 567, 678, 789, 890, 98, 76 }, { 123, 234, 345, 456, 567, 678, 789, 890, 98, 76 } },
    ...
    for (const auto& spec : specs) {
        ...
        EXPECT_NEAR(chi_square, ChiSquare(std::begin(spec.actual), std::end(spec.actual), std::begin(spec.expected), std::end(spec.expected)), 1e-5);
    }
}
...
TEST(ChiSquareTest, CalcChiSquareInt64)
{
    ...
    double b = ChiSquareWithExpected(std::begin(data), std::end(data), 910267586.0);
...
TEST(ChiSquareTest, TableData)
{
    ...
    const double data[100][5] = { /* 1*/ { 2.706, 3.841, 5.024, 6.635, 10.828 },
    ...
    for (int i = 0; i < ABSL_ARRAYSIZE(data); i++) {
        ...
        EXPECT_NEAR(ChiSquarePValue(data[i][0], i + 1), 0.10, E) << i << " " << data[i][0];
        EXPECT_NEAR(ChiSquarePValue(data[i][1], i + 1), 0.05, E) << i << " " << data[i][1];
        EXPECT_NEAR(ChiSquarePValue(data[i][2], i + 1), 0.025, E) << i << " " << data[i][2];
        EXPECT_NEAR(ChiSquarePValue(data[i][3], i + 1), 0.01, E) << i << " " << data[i][3];
        EXPECT_NEAR(ChiSquarePValue(data[i][4], i + 1), 0.001, E) << i << " " << data[i][4];
...
TEST(ChiSquareTest, ChiSquareTwoIterator)
{
    ...
    const double expected[10] = { 4.6, 8.8, 18.4, 30.0, 38.2, 38.2, 30.0, 18.4, 8.8, 4.6 };
    double chi_square = ChiSquare(std::begin(counts), std::end(counts), std::begin(expected), std::end(expected));
