...
void Cpuid(const uint32_t level, const uint32_t count, uint32_t* ABSL_RANDOM_INTERNAL_RESTRICT abcd)
{
...
template <class T> inline void PreventElision(T&& output)
{
#ifndef ABSL_OS_WIN
...
template <class T> void CountingSort(T* values, size_t num_values)
{
    ...
    for (size_t i = 0; i < num_values; ++i) {
        ...
        const auto pos = std::find_if(unique.begin(), unique.end(), [value](const Unique u) { return u.first == value; });
        if (pos == unique.end()) {
...
template <typename T> size_t MinRange(const T* const ABSL_RANDOM_INTERNAL_RESTRICT sorted, const size_t idx_begin, const size_t half_count)
{
...
template <typename T> T ModeOfSorted(const T* const ABSL_RANDOM_INTERNAL_RESTRICT sorted, const size_t num_values)
{
    size_t idx_begin = 0;
...
template <typename T> T Mode(T* values, const size_t num_values)
{
    CountingSort(values, num_values);
    return ModeOfSorted(values, num_values);
}

template <typename T, size_t N> T Mode(T (&values)[N])
{
    return Mode(&values[0], N);
}

// Returns the median value. Side effect: sorts "values".
template <typename T> T Median(T* values, const size_t num_values)
{
...
template <typename T> T MedianAbsoluteDeviation(const T* values, const size_t num_values, const T median)
{
    ABSL_RAW_CHECK(num_values != 0, "Empty input");
...
template <class Lambda> Ticks SampleUntilStable(const double max_rel_mad, double* rel_mad, const Params& p, const Lambda& lambda)
{
    ...
    *rel_mad = 0.0; // ensure initialized
    ...
    for (size_t eval = 0; eval < p.max_evals; ++eval, samples_per_eval *= 2) {
        ...
        const Ticks abs_mad = robust_statistics::MedianAbsoluteDeviation(samples.data(), samples.size(), est);
        *rel_mad = static_cast<double>(static_cast<int>(abs_mad)) / est;

        if (*rel_mad <= max_rel_mad || abs_mad <= max_abs_mad) {
            if (p.verbose) {
                ABSL_RAW_LOG(INFO, "%6zu samples => %5u (abs_mad=%4u, rel_mad=%4.2f%%)\n", samples.size(), est, abs_mad, *rel_mad * 100.0);
            }
            return est;
        }
    }
    ...
    if (p.verbose) {
        ABSL_RAW_LOG(WARNING, "rel_mad=%4.2f%% still exceeds %4.2f%% after %6zu samples.\n", *rel_mad * 100.0, max_rel_mad * 100.0, samples.size());
    }
    return est;
}
...
size_t NumSkip(const Func func, const void* arg, const InputVec& unique, const Params& p)
{
    ...
    for (const FuncInput input : unique) {
        ...
        if (elapsed >= (1ULL << 30)) {
            ABSL_RAW_LOG(WARNING, "Measurement failed: need 64-bit timer for input=%zu\n", static_cast<size_t>(input));
            return 0;
        }

        double rel_mad;
        const Ticks total = SampleUntilStable(p.target_rel_mad, &rel_mad, p, [func, arg, input]() { PreventElision(func(arg, input)); });
    ...
    const size_t num_skip = min_duration == 0 ? 0 : (max_skip + min_duration - 1) / min_duration;
    if (p.verbose) {
...
InputVec ReplicateInputs(const FuncInput* inputs, const size_t num_inputs, const size_t num_unique, const size_t num_skip, const Params& p)
{
    InputVec full;
    if (num_unique == 1) {
...
void FillSubset(const InputVec& full, const FuncInput input_to_skip, const size_t num_skip, InputVec* subset)
{
...
Ticks TotalDuration(const Func func, const void* arg, const InputVec* inputs, const Params& p, double* max_rel_mad)
{
    double rel_mad;
    const Ticks duration = SampleUntilStable(p.target_rel_mad, &rel_mad, p, [func, arg, inputs]() {
...
ABSL_RANDOM_INTERNAL_ATTRIBUTE_NEVER_INLINE FuncOutput EmptyFunc(const void* arg, const FuncInput input)
{
    return input;
}
...
size_t Measure(const Func func, const void* arg, const FuncInput* inputs, const size_t num_inputs, Result* results, const Params& p)
{
    ...
    if (num_skip == 0)
        return 0; // NumSkip already printed error message

    const InputVec full = ReplicateInputs(inputs, num_inputs, unique.size(), num_skip, p);
