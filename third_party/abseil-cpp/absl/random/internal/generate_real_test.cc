...
ABSL_FLAG(int64_t, absl_random_test_trials, 50000, "Number of trials for the probability tests.");
...
{
    auto ToFloat = [](uint64_t a) { return GenerateRealFromBits<float, GeneratePositiveTag, false>(a); };
...
{
    auto ToFloat = [](uint64_t a) { return GenerateRealFromBits<float, GeneratePositiveTag, true>(a); };
...
{
    auto ToFloat = [](uint64_t a) { return GenerateRealFromBits<float, GenerateNegativeTag, false>(a); };
    EXPECT_EQ(ToFloat(0x0000000000000000), -2.710505431e-20f);
    EXPECT_EQ(ToFloat(0x0000000000000001), -5.421010862e-20f);
    EXPECT_EQ(ToFloat(0x8000000000000000), -0.5);
    EXPECT_EQ(ToFloat(0x8000000000000001), -0.5);
...
{
    auto ToFloat = [](uint64_t a) { return GenerateRealFromBits<float, GenerateNegativeTag, true>(a); };
    EXPECT_EQ(ToFloat(0x0000000000000000), 0.0);
    EXPECT_EQ(ToFloat(0x0000000000000001), -5.421010862e-20f);
    EXPECT_EQ(ToFloat(0x8000000000000000), -0.5);
    EXPECT_EQ(ToFloat(0x8000000000000001), -0.5);
...
{
    auto ToFloat = [](uint64_t a) { return GenerateRealFromBits<float, GenerateSignedTag, false>(a); };
...
{
    auto ToFloat = [](uint64_t a) { return GenerateRealFromBits<float, GenerateSignedTag, true>(a); };
...
{
    auto ToFloat = [](uint64_t a) { return GenerateRealFromBits<float, GenerateSignedTag, true>(a, 1); };
    EXPECT_EQ(ToFloat(0x0000000000000000), 0);
...
{
    auto ToFloat = [](uint64_t a) -> float { return GenerateRealFromBits<float, GeneratePositiveTag, true>(a); };
    ...
    EXPECT_EQ(static_cast<int32_t>(ToFloat(0xFFFFFFFFFFFFFFFF) * two_to_24), two_to_24 - 1);
    EXPECT_NE(static_cast<int32_t>(ToFloat(0xFFFFFFFFFFFFFFFF) * two_to_24 * 2), two_to_24 * 2 - 1);
    ...
    exp_values[63] = 0.5f;
    for (int i = 62; i >= 0; --i)
        exp_values[i] = 0.5f * exp_values[i + 1];
    ...
    for (int i = 0; i < 64; ++i) {
        ...
        for (int j = 1; j < FLT_MANT_DIG && i - j >= 0; ++j) {
            ...
            EXPECT_EQ(ToFloat((one << i) + (one << (i - j))), exp_values[i] + exp_values[i - j]);
        }
...
{
    auto ToDouble = [](uint64_t a) { return GenerateRealFromBits<double, GeneratePositiveTag, false>(a); };
...
{
    auto ToDouble = [](uint64_t a) { return GenerateRealFromBits<double, GeneratePositiveTag, true>(a); };
...
{
    auto ToDouble = [](uint64_t a) { return GenerateRealFromBits<double, GenerateNegativeTag, false>(a); };
...
{
    auto ToDouble = [](uint64_t a) { return GenerateRealFromBits<double, GenerateNegativeTag, true>(a); };
...
{
    auto ToDouble = [](uint64_t a) { return GenerateRealFromBits<double, GenerateSignedTag, false>(a); };
...
{
    auto ToDouble = [](uint64_t a) { return GenerateRealFromBits<double, GenerateSignedTag, true>(a); };
...
{
    auto ToDouble = [](uint64_t a) { return GenerateRealFromBits<double, GenerateSignedTag, true>(a, -1); };
    EXPECT_EQ(ToDouble(0x0000000000000000), 0);
...
TEST(GenerateRealTest, U64ToDoubleTest)
{
    ...
    EXPECT_EQ(static_cast<int64_t>(ToDouble(0xFFFFFFFFFFFFFFFF) * two_to_53), two_to_53 - 1);
    EXPECT_NE(static_cast<int64_t>(ToDouble(0xFFFFFFFFFFFFFFFF) * two_to_53 * 2), two_to_53 * 2 - 1);
    ...
    exp_values[63] = 0.5;
    for (int i = 62; i >= 0; --i)
        exp_values[i] = 0.5 * exp_values[i + 1];
    constexpr uint64_t one = 1;
    for (int i = 0; i < 64; ++i) {
        EXPECT_EQ(ToDouble(one << i), exp_values[i]);
        for (int j = 1; j < DBL_MANT_DIG && i - j >= 0; ++j) {
            EXPECT_NE(exp_values[i] + exp_values[i - j], exp_values[i]);
            EXPECT_EQ(ToDouble((one << i) + (one << (i - j))), exp_values[i] + exp_values[i - j]);
        }
...
TEST(GenerateRealTest, U64ToDoubleSignedTest)
{
    ...
    // Large powers of 2.
    int64_t two_to_53 = int64_t { 1 } << 53;
    EXPECT_EQ(static_cast<int64_t>(ToDouble(0x7FFFFFFFFFFFFFFF) * two_to_53), two_to_53 - 1);
    EXPECT_EQ(static_cast<int64_t>(ToDouble(0xFFFFFFFFFFFFFFFF) * two_to_53), -(two_to_53 - 1));

    EXPECT_NE(static_cast<int64_t>(ToDouble(0x7FFFFFFFFFFFFFFF) * two_to_53 * 2), two_to_53 * 2 - 1);
...
TEST(GenerateRealTest, ExhaustiveFloat)
{
    ...
    static_assert(FLT_MANT_DIG == 24, "The float type is expected to have a 24 bit mantissa.");
