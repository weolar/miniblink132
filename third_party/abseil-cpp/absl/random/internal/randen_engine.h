...
template <typename T> class alignas(8) randen_engine {
public:
    // C++11 URBG interface:
    using result_type = T;
    ...
    template <class SeedSequence> typename absl::enable_if_t<!std::is_convertible<SeedSequence, result_type>::value> seed(SeedSequence&& seq)
    {
        // Zeroes the state.
        seed();
        reseed(seq);
    ...
    template <class SeedSequence> void reseed(SeedSequence& seq)
    {
        ...
        constexpr size_t kBufferSize = Randen::kSeedBytes / sizeof(sequence_result_type);
    ...
    bool operator==(const randen_engine& other) const
    {
        ...
        return next_ == other.next_ && std::equal(begin, begin + kStateSizeT, other.state());
    }
    ...
    { // NOLINT(runtime/references)
        using numeric_type = typename random_internal::stream_format_type<result_type>::type;
        ...
        for (auto* end = it + kStateSizeT; it < end; ++it) {
            ...
            os << static_cast<numeric_type>(little_endian::FromHost(*it)) << os.fill();
        }
        os << engine.next_;
        return os;
    }
...
private:
    static constexpr size_t kStateSizeT = Randen::kStateBytes / sizeof(result_type);
    static constexpr size_t kCapacityT = Randen::kCapacityBytes / sizeof(result_type);
    ...
    result_type* state()
    {
        return reinterpret_cast<result_type*>((reinterpret_cast<uintptr_t>(&raw_state_) & 0xf) ? (raw_state_ + 8) : raw_state_);
    }
