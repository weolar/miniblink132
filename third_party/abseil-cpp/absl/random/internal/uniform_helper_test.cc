...
TEST_F(UniformHelperTest, UniformBoundFunctionsGeneral)
{
    ...
    EXPECT_EQ(uniform_lower_bound<double>(IntervalClosedClosed, -2.0, -1.0), -2.0);
    ...
    EXPECT_GT(uniform_upper_bound<double>(IntervalClosedClosed, -2.0, -1.0), -1.0);
...
TEST_F(UniformHelperTest, UniformBoundFunctionsRealBounds)
{
    ...
    EXPECT_EQ(std::nextafter(e, 2.0), uniform_upper_bound(IntervalOpenClosed, 1.0, e));
...
template <typename A, typename B> auto InferredUniformReturnT(int) -> uniform_inferred_return_t<A, B>;
