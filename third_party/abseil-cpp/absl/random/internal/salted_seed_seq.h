...
// not modified.
template <typename SSeq> class SaltedSeedSeq {
public:
    ...
    template <typename RandomAccessIterator> void generate(RandomAccessIterator begin, RandomAccessIterator end)
    {
        ...
        using TagType = absl::conditional_t<(std::is_same<U, uint32_t>::value
...
private:
    ...
    template <typename Contiguous> void generate_impl(ContiguousAndUint32Tag, Contiguous begin, Contiguous end, size_t n)
    {
    ...
    template <typename RandomAccessIterator> void generate_impl(DefaultTag, RandomAccessIterator begin, RandomAccessIterator, size_t n)
    {
...
template <typename T, typename = void> struct is_salted_seed_seq : public std::false_type { };

template <typename T>
struct is_salted_seed_seq<T, typename std::enable_if<std::is_same<T, SaltedSeedSeq<typename T::inner_sequence_type>>::value>::type> : public std::true_type { };
