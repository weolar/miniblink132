...
bool ReadSeedMaterialFromOSEntropyImpl(absl::Span<uint32_t> values)
{
    ...
    NTSTATUS ret;
    ret = BCryptOpenAlgorithmProvider(&hProvider, BCRYPT_RNG_ALGORITHM, MS_PRIMITIVE_PROVIDER, 0);
    if (!(BCRYPT_SUCCESS(ret))) {
        ABSL_RAW_LOG(ERROR, "Failed to open crypto provider.");
        return false;
    }
    ret = BCryptGenRandom(hProvider, // provider
...
absl::optional<uint32_t> GetSaltMaterial()
{
    ...
    static const auto salt_material = []() -> absl::optional<uint32_t> {
        ...
        if (random_internal::ReadSeedMaterialFromOSEntropy(MakeSpan(&salt_value, 1))) {
            return salt_value;
        }
