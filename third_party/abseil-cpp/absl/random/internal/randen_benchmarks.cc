...
template <typename T> struct AbsorbFn : public T {
...
template <typename T> struct GenerateFn : public T {
...
template <typename UInt> struct Engine {
...
template <size_t N> void Print(const char* name, const size_t n, const Result (&results)[N], const size_t bytes)
{
    if (n == 0) {
    ...
    static auto header = [] { return printf("%20s %8s: %12s ticks; %9s  (%9s) %8s\n", "Name", "Count", "Total", "Variance", "Time", "bytes/s"); }();
    (void)header;
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        // Output
        printf("%20s %8zu: %12.2f ticks; MAD=%4.2f%%  (%6.1f ns) %8.1f Mb/s\n", name, results[i].input, results[i].ticks, results[i].variability * 100.0,
            ns_per_call, mb_per_s);
    }
}

// Fails here
template <typename Op, size_t N> void Measure(const char* name, const FuncInput (&inputs)[N])
{
    Op op;
...
void RunAll(const int argc, char* argv[])
{
    ...
    static const FuncInput inputs1[] = { unpredictable * 1000, unpredictable * 10000 };
