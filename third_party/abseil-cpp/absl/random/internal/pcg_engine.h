...
//
template <typename Params, typename Mix> class pcg_engine {
    static_assert(std::is_same<typename Params::state_type, typename Mix::state_type>::value,
...
public:
    ...
    template <class SeedSequence, typename = typename absl::enable_if_t<!std::is_same<SeedSequence, pcg_engine>::value>> explicit pcg_engine(SeedSequence&& seq)
    {
        seed(seq);
    }
    ...
    template <class SeedSequence> typename absl::enable_if_t<!std::is_convertible<SeedSequence, uint64_t>::value, void> seed(SeedSequence&& seq)
    {
        reseed(seq);
    }
    ...
    friend typename absl::enable_if_t<(sizeof(state_type) == 16), std::basic_ostream<CharT, Traits>&> operator<<(
    ...
    template <class CharT, class Traits>
    friend typename absl::enable_if_t<(sizeof(state_type) <= 8), std::basic_ostream<CharT, Traits>&> operator<<(
        std::basic_ostream<CharT, Traits>& os, // NOLINT(runtime/references)
        const pcg_engine& engine)
    {
        auto saver = random_internal::make_ostream_state_saver(os);
    ...
    template <class CharT, class Traits>
    friend typename absl::enable_if_t<(sizeof(state_type) == 16), std::basic_istream<CharT, Traits>&> operator>>(
    ...
    { // NOLINT(runtime/references)
        ...
        if (mult != pcg_engine::params_type::multiplier() || inc != pcg_engine::params_type::increment()) {
    ...
    template <class CharT, class Traits>
    friend typename absl::enable_if_t<(sizeof(state_type) <= 8), std::basic_istream<CharT, Traits>&> operator>>(
        std::basic_istream<CharT, Traits>& is, // NOLINT(runtime/references)
        pcg_engine& engine)
    { // NOLINT(runtime/references)
...
private:
    ...
    template <class SeedSequence> void reseed(SeedSequence& seq)
    {
        using sequence_result_type = typename SeedSequence::result_type;
        constexpr size_t kBufferSize = sizeof(state_type) / sizeof(sequence_result_type);
...
template <uint64_t kMultA, uint64_t kMultB, uint64_t kIncA, uint64_t kIncB> class pcg128_params {
public:
...
// This provides state_type, multiplier, and increment for pcg_engine.
template <uint64_t kMult, uint64_t kInc> class pcg64_params {
public:
...
using pcg64_2018_engine = pcg_engine<random_internal::pcg128_params<0x2360ed051fc65da4ull, 0x4385df649fccf645ull, 0x5851f42d4c957f2d, 0x14057b7ef767814f>,
