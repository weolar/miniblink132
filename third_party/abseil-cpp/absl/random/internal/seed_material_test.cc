...
TEST(ReadSeedMaterialFromOSEntropy, SuccessiveReadsAreDistinct)
{
    ...
    EXPECT_TRUE(absl::random_internal::ReadSeedMaterialFromOSEntropy(absl::Span<uint32_t>(seed_material_1, kSeedMaterialSize)));
    EXPECT_TRUE(absl::random_internal::ReadSeedMaterialFromOSEntropy(absl::Span<uint32_t>(seed_material_2, kSeedMaterialSize)));
...
TEST(ReadSeedMaterialFromOSEntropy, ReadZeroBytesIsNoOp)
{
    ...
    EXPECT_TRUE(absl::random_internal::ReadSeedMaterialFromOSEntropy(absl::Span<uint32_t>(seed_material, 0)));
...
{
#ifdef NDEBUG
    EXPECT_FALSE(absl::random_internal::ReadSeedMaterialFromOSEntropy(absl::Span<uint32_t>(nullptr, 32)));
#else
    bool result;
    ABSL_EXPECT_DEATH_IF_SUPPORTED(result = absl::random_internal::ReadSeedMaterialFromOSEntropy(absl::Span<uint32_t>(nullptr, 32)), "!= nullptr");
...
TEST(ReadSeedMaterialFromURBG, SeedMaterialEqualsVariateSequence)
{
    ...
    EXPECT_TRUE(absl::random_internal::ReadSeedMaterialFromURBG(&urbg_1, absl::Span<uint32_t>(seed_material, kSeedMaterialSize)));
...
TEST(ReadSeedMaterialFromURBG, ReadZeroBytesIsNoOp)
{
    ...
    uint32_t seed_material[32];
    std::memset(seed_material, 0xAA, sizeof(seed_material));
    EXPECT_TRUE(absl::random_internal::ReadSeedMaterialFromURBG(&urbg, absl::Span<uint32_t>(seed_material, 0)));
...
TEST(ReadSeedMaterialFromURBG, NullUrbgArgument)
{
    ...
    uint32_t seed_material[kSeedMaterialSize];
#ifdef NDEBUG
    EXPECT_FALSE(absl::random_internal::ReadSeedMaterialFromURBG<std::mt19937_64>(nullptr, absl::Span<uint32_t>(seed_material, kSeedMaterialSize)));
#else
    bool result;
    ABSL_EXPECT_DEATH_IF_SUPPORTED(
        result = absl::random_internal::ReadSeedMaterialFromURBG<std::mt19937_64>(nullptr, absl::Span<uint32_t>(seed_material, kSeedMaterialSize)),
        "!= nullptr");
    (void)result; // suppress unused-variable warning
#endif
}
...
TEST(ReadSeedMaterialFromURBG, NullPtrVectorArgument)
{
    std::mt19937_64 urbg;
#ifdef NDEBUG
    EXPECT_FALSE(absl::random_internal::ReadSeedMaterialFromURBG(&urbg, absl::Span<uint32_t>(nullptr, 32)));
#else
    bool result;
    ABSL_EXPECT_DEATH_IF_SUPPORTED(result = absl::random_internal::ReadSeedMaterialFromURBG(&urbg, absl::Span<uint32_t>(nullptr, 32)), "!= nullptr");
    (void)result; // suppress unused-variable warning
#endif
}
...
TEST(MixSequenceIntoSeedMaterial, AvalancheEffectTestOneBitLong)
{
    ...
    for (uint32_t v = 1; v != 0; v <<= 1) {
        ...
        absl::random_internal::MixIntoSeedMaterial(absl::Span<uint32_t>(&v, 1), absl::Span<uint32_t>(seed_material_copy.data(), seed_material_copy.size()));

        uint32_t changed_bits = 0;
        for (size_t i = 0; i < seed_material.size(); i++) {
            std::bitset<sizeof(uint32_t) * 8> bitset(seed_material[i] ^ seed_material_copy[i]);
