...
template <typename IntType> class FastUniformBitsTypedTest : public ::testing::Test { };
...
template <typename UIntType, UIntType Lo, UIntType Hi, UIntType Val = Lo> struct FakeUrbg {
    using result_type = UIntType;

    FakeUrbg() = default;
    explicit FakeUrbg(bool r)
        : reject(r)
    {
    }
...
TEST(FastUniformBitsTest, RangeSize)
{
    ...
    EXPECT_EQ(0, (RangeSize<FakeUrbg<uint8_t, 0, (std::numeric_limits<uint8_t>::max)()>>()));
    ...
    EXPECT_EQ(0, (RangeSize<FakeUrbg<uint16_t, 0, (std::numeric_limits<uint16_t>::max)()>>()));
    ...
    EXPECT_EQ(0, (RangeSize<FakeUrbg<uint32_t, 0, (std::numeric_limits<uint32_t>::max)()>>()));
    ...
    EXPECT_EQ(0xffffffffffffffff, (RangeSize<FakeUrbg<uint64_t, 1, 0xffffffffffffffff>>()));
    EXPECT_EQ(0xfffffffffffffffe, (RangeSize<FakeUrbg<uint64_t, 1, 0xfffffffffffffffe>>()));
    EXPECT_EQ(0xfffffffffffffffd, (RangeSize<FakeUrbg<uint64_t, 2, 0xfffffffffffffffe>>()));
    EXPECT_EQ(0, (RangeSize<FakeUrbg<uint64_t, 0, (std::numeric_limits<uint64_t>::max)()>>()));
}
...
using Urng33bits = FakeUrbg<uint64_t, 1, 0x1ffffffff, 0x013301033>; // ~32.9 bits
using Urng63bits = FakeUrbg<uint64_t, 1, 0xfffffffffffffffe,
    0xfedcba9012345678>; // ~63.9 bits
using Urng64bits = FakeUrbg<uint64_t, 0, 0xffffffffffffffff, 0x123456780fedcba9>;
