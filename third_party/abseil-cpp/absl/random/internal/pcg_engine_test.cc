...
template <typename EngineType> class PCGEngineTest : public ::testing::Test { };
...
TYPED_TEST(PCGEngineTest, RandomNumberEngineInterface)
{
    ...
    static_assert(std::is_copy_constructible<E>::value, "engine_type must be copy constructible");

    static_assert(absl::is_copy_assignable<E>::value, "engine_type must be copy assignable");

    static_assert(std::is_move_constructible<E>::value, "engine_type must be move constructible");

    static_assert(absl::is_move_assignable<E>::value, "engine_type must be move assignable");
...
TEST(PCG642018EngineTest, VerifyGoldenFromDeserializedEngine)
{
    ...
    std::istringstream stream("2549297995355413924 4865540595714422341 6364136223846793005 "
...
TEST(PCG322018EngineTest, VerifyGoldenFromDeserializedEngine)
{
    ...
    std::istringstream stream("6364136223846793005 1442695040888963407 6537028157270659894");
    stream >> engine;
    for (const auto& elem : kGolden) {
        EXPECT_EQ(elem, engine());
    }
