...
template <typename EngineType> class PoolURBGTypedTest : public ::testing::Test { };
...
TYPED_TEST(PoolURBGTypedTest, URBGInterface)
{
    ...
    static_assert(std::is_copy_constructible<E>::value, "engine must be copy constructible");

    static_assert(absl::is_copy_assignable<E>::value, "engine must be copy assignable");
    ...
    static_assert(std::is_same<decltype(e()), T>::value, "return type of operator() must be result_type");
