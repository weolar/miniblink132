...
template <> inline uint8_t RandenPoolEntry::Generate<uint8_t>()
{
...
size_t GetPoolID()
{
    ...
    uintptr_t my_pool_id = reinterpret_cast<uintptr_t>(pthread_getspecific(tid_key));
...
RandenPoolEntry* PoolAlignedAlloc()
{
    ...
    uintptr_t x = reinterpret_cast<uintptr_t>(new char[sizeof(RandenPoolEntry) + kAlignment]);
    auto y = x % kAlignment;
...
void InitPoolURBG()
{
    static constexpr size_t kSeedSize = RandenTraits::kStateBytes / sizeof(uint32_t);
    // Read the seed data from OS entropy once.
    uint32_t seed_material[kPoolSize * kSeedSize];
    if (!random_internal::ReadSeedMaterialFromOSEntropy(absl::MakeSpan(seed_material))) {
...
{
    auto* pool = GetPoolForCurrentThread();
    pool->Fill(reinterpret_cast<uint8_t*>(data.data()), data.size() * sizeof(result_type));
}
