...
private:
    ...
    template <typename Contiguous> void generate_impl(ContiguousTag, Contiguous begin, Contiguous end)
    {
        const size_t n = static_cast<size_t>(std::distance(begin, end));
        auto* a = &(*begin);
        RandenPool<uint8_t>::Fill(absl::MakeSpan(reinterpret_cast<uint8_t*>(a), sizeof(*a) * n));
    }

    // Construct a buffer of size n and fill it with values, then copy
    // those values into the seed iterators.
    template <typename RandomAccessIterator> void generate_impl(BufferTag, RandomAccessIterator begin, RandomAccessIterator end)
    {
...
public:
    ...
    template <typename OutIterator> void param(OutIterator) const
    {
    }
    ...
    template <typename RandomAccessIterator> void generate(RandomAccessIterator begin, RandomAccessIterator end)
    {
        ...
        if (begin != end) {
            ...
            using TagType = absl::conditional_t<(std::is_pointer<RandomAccessIterator>::value
...
template <typename URBG, typename Seeder = RandenPoolSeedSeq> class NonsecureURBGBase {
public:
...
private:
    ...
    template <typename SSeq> static URBG ConstructURBG(SSeq&& seq)
    { // NOLINT(runtime/references)
        auto salted_seq = random_internal::MakeSaltedSeedSeq(std::forward<SSeq>(seq));
