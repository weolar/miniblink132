...
{
    return os << absl::StrFormat("mean=%f, stddev=%f, skewness=%f, kurtosis=%f", moments.mean, std::sqrt(moments.variance), moments.skewness, moments.kurtosis);
}
...
bool Near(absl::string_view msg, double actual, double expected, double bound)
{
    ...
    std::string formatted = absl::StrCat(msg, " actual=", actual, " expected=", expected, " err=", delta / bound);
...
//
double BetaIncompleteImpl(const double x, const double p, const double q, const double beta)
{
    if (p < (p + q) * x) {
    ...
    const double xc = 1. - x;
    const double pre = std::exp(p * std::log(x) + (q - 1.) * std::log(xc) - beta) / p;
...
//
double BetaIncompleteInvImpl(const double p, const double q, const double beta, const double alpha)
{
    if (alpha < 0.5) {
    ...
    // Compute the initial estimate.
    {
        ...
        double y = r - fma(r, 0.27061, 2.30753) / fma(r, fma(r, 0.04481, 0.99229), 1.0);
        if (p > 1. && q > 1.) {
            r = (y * y - 3.) / 6.;
            double s = 1. / (p + p - 1.);
            double t = 1. / (q + q - 1.);
            double h = 2. / s + t;
            double w = y * std::sqrt(h + r) / h - (t - s) * (r + 5. / 6. - t / (3. * h));
    ...
    // BetaIncomplete.
    {
        ...
        for (;;) {
            ...
            y = (y - alpha) * std::exp(beta + r * std::log(value) + t * std::log(1.0 - value));
            if (y * yprev <= 0) {
...
{
    return (moments.mean - expected_mean) / (std::sqrt(moments.variance) / std::sqrt(static_cast<double>(moments.n)));
}
