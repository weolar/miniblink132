...
template <typename URBG> struct DistributionCaller {
    ...
    template <class T>
    using invoke_mock_t
        = decltype(std::declval<T*>()->InvokeMock(std::declval<::absl::base_internal::FastTypeIdType>(), std::declval<void*>(), std::declval<void*>()));

    using HasInvokeMock = typename detector<invoke_mock_t, void, URBG>::type;

    // Default implementation of distribution caller.
    template <typename DistrT, typename... Args> static typename DistrT::result_type Impl(std::false_type, URBG* urbg, Args&&... args)
    {
    ...
    template <typename DistrT, typename... Args> static typename DistrT::result_type Impl(std::true_type, URBG* urbg, Args&&... args)
    {
    ...
    // Default implementation of distribution caller.
    template <typename DistrT, typename... Args> static typename DistrT::result_type Call(URBG* urbg, Args&&... args)
    {
