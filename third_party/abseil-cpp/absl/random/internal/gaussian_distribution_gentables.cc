...
template <typename T, size_t N> void FormatArrayContents(std::ostream* os, T (&data)[N])
{
...
TableGenerator::TableGenerator()
{
    ...
    static_assert((ABSL_ARRAYSIZE(tables_.x) == kC + 1), "xArray must be length kMask + 2");

    static_assert((ABSL_ARRAYSIZE(tables_.x) == ABSL_ARRAYSIZE(tables_.f)), "fx and x arrays must be identical length");
