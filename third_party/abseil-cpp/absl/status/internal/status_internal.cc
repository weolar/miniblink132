...
void StatusRep::Unref() const
{
    ...
    if (ref_.load(std::memory_order_acquire) == 1 || ref_.fetch_sub(1, std::memory_order_acq_rel) - 1 == 0) {
        delete this;
    }
}

static absl::optional<size_t> FindPayloadIndexByUrl(const Payloads* payloads, absl::string_view type_url)
{
    if (payloads == nullptr)
        return absl::nullopt;
...
absl::optional<absl::Cord> StatusRep::GetPayload(absl::string_view type_url) const
{
    absl::optional<size_t> index = status_internal::FindPayloadIndexByUrl(payloads_.get(), type_url);
    if (index.has_value())
        return (*payloads_)[index.value()].payload;
...
StatusRep::EraseResult StatusRep::ErasePayload(absl::string_view type_url)
{
    absl::optional<size_t> index = status_internal::FindPayloadIndexByUrl(payloads_.get(), type_url);
    if (!index.has_value())
        return { false, Status::PointerToRep(this) };
...
void StatusRep::ForEachPayload(absl::FunctionRef<void(absl::string_view, const absl::Cord&)> visitor) const
{
...
std::string StatusRep::ToString(StatusToStringMode mode) const
{
    ...
    const bool with_payload = (mode & StatusToStringMode::kWithPayload) == StatusToStringMode::kWithPayload;
    ...
    if (with_payload) {
        ...
        this->ForEachPayload([&](absl::string_view type_url, const absl::Cord& payload) {
            ...
            absl::StrAppend(&text, " [", type_url, "='", result.has_value() ? *result : absl::CHexEscape(std::string(payload)), "']");
        });
    }
...
bool StatusRep::operator==(const StatusRep& other) const
{
    ...
    const status_internal::Payloads* larger_payloads = this_payloads ? this_payloads : &no_payloads;
    const status_internal::Payloads* smaller_payloads = other_payloads ? other_payloads : &no_payloads;
...
absl::Nonnull<std::string*> MakeCheckFailString(absl::Nonnull<const absl::Status*> status, absl::Nonnull<const char*> prefix)
{
    return new std::string(absl::StrCat(prefix, " (", status->ToString(StatusToStringMode::kWithEverything), ")"));
}
