...
TEST(StatusMatcherTest, IsOkAndHoldsFailure)
{
    ...
    EXPECT_NONFATAL_FAILURE(EXPECT_THAT(ok_str, IsOkAndHolds("expected")), "actual");
}
...
TEST(StatusMatcherTest, StatusIs)
{
    ...
    EXPECT_THAT(invalid, StatusIs(absl::StatusCode::kInvalidArgument, "ungueltig"));
}
...
TEST(StatusMatcherTest, StatusOrIs)
{
    ...
    absl::StatusOr<absl::string_view> invalid = absl::InvalidArgumentError("ungueltig");
...
TEST(StatusMatcherTest, StatusIsFailure)
{
    absl::Status unknown = absl::UnknownError("unbekannt");
    absl::Status invalid = absl::InvalidArgumentError("ungueltig");
    EXPECT_NONFATAL_FAILURE(EXPECT_THAT(absl::OkStatus(), StatusIs(absl::StatusCode::kInvalidArgument)), "OK");
    EXPECT_NONFATAL_FAILURE(EXPECT_THAT(unknown, StatusIs(absl::StatusCode::kCancelled)), "UNKNOWN");
    EXPECT_NONFATAL_FAILURE(EXPECT_THAT(unknown, StatusIs(absl::StatusCode::kUnknown, "inconnu")), "unbekannt");
    EXPECT_NONFATAL_FAILURE(EXPECT_THAT(invalid, StatusIs(absl::StatusCode::kOutOfRange)), "INVALID");
    EXPECT_NONFATAL_FAILURE(EXPECT_THAT(invalid, StatusIs(absl::StatusCode::kInvalidArgument, "invalide")), "ungueltig");
}
