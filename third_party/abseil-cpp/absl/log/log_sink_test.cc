...
TEST(LogSinkRegistryTest, AddLogSink)
{
    ...
    EXPECT_CALL(test_sink, Log(absl::LogSeverity::kWarning, __FILE__, "Danger ahead"));
    EXPECT_CALL(test_sink, Log(absl::LogSeverity::kError, __FILE__, "This is an error"));
...
TEST(LogSinkDeathTest, DeathInSend)
{
    ...
    FatalSendSink sink;
    EXPECT_EXIT({ LOG(INFO).ToSinkAlso(&sink) << "hello world"; }, DiedOfFatal, _);
}
...
protected:
    ...
    public:
        explicit ReentrantSendLogSink(absl::LogSeverity severity, absl::LogSink* sink, LogMode mode)
            : severity_(severity)
            , sink_(sink)
            , mode_(mode)
        {
        ...
        void Send(const absl::LogEntry&) override
        {
            ...
            case LogMode::kToSinkAlso:
                LOG(LEVEL(severity_)).ToSinkAlso(sink_) << "The log is coming from *inside the sink*.";
                break;
            case LogMode::kToSinkOnly:
                LOG(LEVEL(severity_)).ToSinkOnly(sink_) << "The log is coming from *inside the sink*.";
                break;
            default:
    ...
    static absl::string_view LogAndReturn(absl::LogSeverity severity, absl::string_view to_log, absl::string_view to_return)
    {
...
TEST_F(ReentrancyTest, AlsoLogSinkThatLogsInSend)
{
    ...
    EXPECT_CALL(test_sink, Log(_, _, "hello world"));
    EXPECT_CALL(test_sink, Log(_, _, "The log is coming from *inside the sink*."));
    ...
    EXPECT_EXIT(
        {
            ...
            EXPECT_CALL(test_sink, Log(_, _, "hello")).WillOnce(DeathTestExpectedLogging());
    ...
    EXPECT_EXIT(
        {
            ...
            EXPECT_CALL(test_sink, Log).Times(AnyNumber()).WillRepeatedly(DeathTestUnexpectedLogging());
            EXPECT_CALL(test_sink, Log(_, _, "hello world")).WillOnce(DeathTestExpectedLogging());
    ...
    EXPECT_EXIT(
        {
            ...
            EXPECT_CALL(test_sink, Log).Times(AnyNumber()).WillRepeatedly(DeathTestUnexpectedLogging());
            EXPECT_CALL(test_sink, Log(_, _, "hello world")).WillOnce(DeathTestExpectedLogging());
            EXPECT_CALL(test_sink, Log(_, _, "The log is coming from *inside the sink*.")).WillOnce(DeathTestExpectedLogging());
