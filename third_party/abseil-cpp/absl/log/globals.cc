...
ABSL_CONST_INIT std::atomic<int> min_log_level { static_cast<int>(absl::LogSeverityAtLeast::kInfo) };
ABSL_CONST_INIT std::atomic<int> stderrthreshold { static_cast<int>(absl::LogSeverityAtLeast::kError) };
...
{
    return static_cast<absl::LogSeverityAtLeast>(min_log_level.load(std::memory_order_acquire));
}
...
{
    return static_cast<absl::LogSeverityAtLeast>(stderrthreshold.load(std::memory_order_acquire));
}
...
void SetAndroidNativeTag(const char* tag)
{
    ...
    ABSL_INTERNAL_CHECK(android_log_tag.exchange(tag_str->c_str(), std::memory_order_acq_rel) == kDefaultAndroidTag,
...
{
    const size_t flag_hash = log_backtrace_at_hash.load(std::memory_order_relaxed);
...
{
    log_backtrace_at_hash.store(HashSiteForLogBacktraceAt(file, line), std::memory_order_relaxed);
}
