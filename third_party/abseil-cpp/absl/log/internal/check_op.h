...
#define ABSL_LOG_INTERNAL_STRIP_STRING_LITERAL(literal) (::absl::LogSeverity::kFatal >= static_cast<::absl::LogSeverity>(ABSL_MIN_LOG_LEVEL) ? (literal) : "")
#else
...
#define ABSL_LOG_INTERNAL_CHECK_OP(name, op, val1, val1_text, val2, val2_text)                                                                                 \
    while (::std::string* absl_log_internal_check_op_result ABSL_LOG_INTERNAL_ATTRIBUTE_UNUSED_IF_STRIP_LOG                                                    \
        = ::absl::log_internal::name##Impl(::absl::log_internal::GetReferenceableValue(val1), ::absl::log_internal::GetReferenceableValue(val2),               \
            ABSL_LOG_INTERNAL_STRIP_STRING_LITERAL(val1_text " " #op " " val2_text)))                                                                          \
...
#define ABSL_LOG_INTERNAL_QCHECK_OP(name, op, val1, val1_text, val2, val2_text)                                                                                \
    while (::std::string* absl_log_internal_qcheck_op_result = ::absl::log_internal::name##Impl(::absl::log_internal::GetReferenceableValue(val1),             \
               ::absl::log_internal::GetReferenceableValue(val2), ABSL_LOG_INTERNAL_STRIP_STRING_LITERAL(val1_text " " #op " " val2_text)))                    \
...
#define ABSL_LOG_INTERNAL_CHECK_STROP(func, op, expected, s1, s1_text, s2, s2_text)                                                                            \
    while (::std::string* absl_log_internal_check_strop_result                                                                                                 \
        = ::absl::log_internal::Check##func##expected##Impl((s1), (s2), ABSL_LOG_INTERNAL_STRIP_STRING_LITERAL(s1_text " " #op " " s2_text)))                  \
        ABSL_LOG_INTERNAL_CONDITION_FATAL(STATELESS, true)                                                                                                     \
...
#define ABSL_LOG_INTERNAL_QCHECK_STROP(func, op, expected, s1, s1_text, s2, s2_text)                                                                           \
    while (::std::string* absl_log_internal_qcheck_strop_result                                                                                                \
        = ::absl::log_internal::Check##func##expected##Impl((s1), (s2), ABSL_LOG_INTERNAL_STRIP_STRING_LITERAL(s1_text " " #op " " s2_text)))                  \
        ABSL_LOG_INTERNAL_CONDITION_QFATAL(STATELESS, true)                                                                                                    \
    ABSL_LOG_INTERNAL_QCHECK(*absl_log_internal_qcheck_strop_result).InternalStream()
// This one is tricky:
...
#define ABSL_LOG_INTERNAL_CHECK_OK(val, val_text)                                                                                                              \
    for (::std::pair<const ::absl::Status*, ::std::string*> absl_log_internal_check_ok_goo;                                                                    \
         absl_log_internal_check_ok_goo.first = ::absl::log_internal::AsStatus(val),                                                                           \
         absl_log_internal_check_ok_goo.second = ABSL_PREDICT_TRUE(absl_log_internal_check_ok_goo.first->ok())                                                 \
             ? nullptr                                                                                                                                         \
             : ::absl::status_internal::MakeCheckFailString(absl_log_internal_check_ok_goo.first, ABSL_LOG_INTERNAL_STRIP_STRING_LITERAL(val_text " is OK")),  \
         !ABSL_PREDICT_TRUE(absl_log_internal_check_ok_goo.first->ok());)                                                                                      \
        ABSL_LOG_INTERNAL_CONDITION_FATAL(STATELESS, true)                                                                                                     \
    ABSL_LOG_INTERNAL_CHECK(*absl_log_internal_check_ok_goo.second).InternalStream()
#define ABSL_LOG_INTERNAL_QCHECK_OK(val, val_text)                                                                                                             \
    for (::std::pair<const ::absl::Status*, ::std::string*> absl_log_internal_qcheck_ok_goo;                                                                   \
         absl_log_internal_qcheck_ok_goo.first = ::absl::log_internal::AsStatus(val),                                                                          \
         absl_log_internal_qcheck_ok_goo.second = ABSL_PREDICT_TRUE(absl_log_internal_qcheck_ok_goo.first->ok())                                               \
             ? nullptr                                                                                                                                         \
             : ::absl::status_internal::MakeCheckFailString(absl_log_internal_qcheck_ok_goo.first, ABSL_LOG_INTERNAL_STRIP_STRING_LITERAL(val_text " is OK")), \
         !ABSL_PREDICT_TRUE(absl_log_internal_qcheck_ok_goo.first->ok());)                                                                                     \
        ABSL_LOG_INTERNAL_CONDITION_QFATAL(STATELESS, true)                                                                                                    \
    ABSL_LOG_INTERNAL_QCHECK(*absl_log_internal_qcheck_ok_goo.second).InternalStream()
...
ABSL_ATTRIBUTE_PURE_FUNCTION absl::Nonnull<std::string*> MakeCheckFailString(absl::Nonnull<const absl::Status*> status, absl::Nonnull<const char*> prefix);
} // namespace status_internal
...
inline const absl::Status* AsStatus(const absl::Status& s)
{
    return &s;
}
...
template <typename T> inline void MakeCheckOpValueString(std::ostream& os, const T& v)
{
...
// not be ambiguous.
template <typename Traits> char operator<<(std::basic_ostream<char, Traits>&, char);
...
template <typename T, typename = decltype(std::declval<std::ostream&>() << std::declval<const T&>())> const T& Detect(int);
...
template <typename T> decltype(detect_specialization::operator<<(std::declval<std::ostream&>(), std::declval<const T&>())) Detect(char);
...
template <typename T> class StringifyToStreamWrapper {
public:
...
// operator<<.
template <typename T> std::enable_if_t<HasAbslStringify<T>::value,
    StringifyToStreamWrapper<T>> Detect(...); // Ellipsis has lowest preference when int passed.
} // namespace detect_specialization

template <typename T> using CheckOpStreamType = decltype(detect_specialization::Detect<T>(0));

// Build the error message string.  Specify no inlining for code size.
template <typename T1, typename T2> ABSL_ATTRIBUTE_RETURNS_NONNULL std::string* MakeCheckOpString(T1 v1, T2 v2, const char* exprtext) ABSL_ATTRIBUTE_NOINLINE;

template <typename T1, typename T2> std::string* MakeCheckOpString(T1 v1, T2 v2, const char* exprtext)
{
...
// files.
#define ABSL_LOG_INTERNAL_DEFINE_MAKE_CHECK_OP_STRING_EXTERN(x) extern template std::string* MakeCheckOpString(x, x, const char*)
...
#define ABSL_LOG_INTERNAL_CHECK_OP_IMPL_RESULT(U1, U2, v1, v2, exprtext)                                                                                       \
    ((::absl::LogSeverity::kFatal >= static_cast<::absl::LogSeverity>(ABSL_MIN_LOG_LEVEL)) ? MakeCheckOpString<U1, U2>(v1, v2, exprtext) : new std::string())
#else
#define ABSL_LOG_INTERNAL_CHECK_OP_IMPL_RESULT(U1, U2, v1, v2, exprtext) MakeCheckOpString<U1, U2>(v1, v2, exprtext)
#endif
...
#define ABSL_LOG_INTERNAL_CHECK_OP_IMPL(name, op)                                                                                                              \
    template <typename T1, typename T2> inline constexpr ::std::string* name##Impl(const T1& v1, const T2& v2, const char* exprtext)                           \
    {                                                                                                                                                          \
        using U1 = CheckOpStreamType<T1>;                                                                                                                      \
        using U2 = CheckOpStreamType<T2>;                                                                                                                      \
        return ABSL_PREDICT_TRUE(v1 op v2) ? nullptr : ABSL_LOG_INTERNAL_CHECK_OP_IMPL_RESULT(U1, U2, U1(v1), U2(v2), exprtext);                               \
    }                                                                                                                                                          \
    inline constexpr ::std::string* name##Impl(int v1, int v2, const char* exprtext)                                                                           \
    {                                                                                                                                                          \
...
template <typename T> inline constexpr const T& GetReferenceableValue(const T& t)
{
    return t;
}
