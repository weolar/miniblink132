...
bool LogEveryNSecState::ShouldLog(double seconds)
{
    ...
    if (now_cycles > next_cycles) {
        next_log_time_cycles_.store(now_cycles + seconds * CycleClock::Frequency(), std::memory_order_relaxed);
        return true;
    }
    return false;
#else
    do {
        ...
        next_cycles, now_cycles + seconds * CycleClock::Frequency(), std::memory_order_relaxed, std::memory_order_relaxed));
    return true;
#endif
}
