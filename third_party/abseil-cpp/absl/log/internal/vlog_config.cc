...
struct VModuleInfo final {
    ...
    // Allocates memory.
    VModuleInfo(absl::string_view module_pattern_arg, bool module_is_path_arg, int vlog_level_arg)
...
ABSL_CONST_INIT absl::base_internal::SpinLock mutex(absl::kConstInit, absl::base_internal::SCHEDULE_KERNEL_ONLY);
...
absl::Mutex* GetUpdateSitesMutex()
{
    ...
    static absl::NoDestructor<absl::Mutex> update_sites_mutex ABSL_ACQUIRED_AFTER(mutex);
...
ABSL_CONST_INIT std::vector<VModuleInfo>* vmodule_info ABSL_GUARDED_BY(mutex) ABSL_PT_GUARDED_BY(mutex) { nullptr };

// Only used for lisp.
ABSL_CONST_INIT std::vector<std::function<void()>>* update_callbacks ABSL_GUARDED_BY(GetUpdateSitesMutex())
    ABSL_PT_GUARDED_BY(GetUpdateSitesMutex()) { nullptr };

// Allocates memory.
std::vector<VModuleInfo>& get_vmodule_info() ABSL_EXCLUSIVE_LOCKS_REQUIRED(mutex)
{
    if (!vmodule_info)
        vmodule_info = new std::vector<VModuleInfo>;
    return *vmodule_info;
}

// Does not allocate or take locks.
int VLogLevel(absl::string_view file, const std::vector<VModuleInfo>* infos, int current_global_v)
{
...
// Allocates memory.
int AppendVModuleLocked(absl::string_view module_pattern, int log_level) ABSL_EXCLUSIVE_LOCKS_REQUIRED(mutex)
{
    ...
    get_vmodule_info().emplace_back(std::string(module_pattern), module_is_path, log_level);
    return global_v;
}

// Allocates memory.
int PrependVModuleLocked(absl::string_view module_pattern, int log_level) ABSL_EXCLUSIVE_LOCKS_REQUIRED(mutex)
{
    ...
    bool module_is_path = ModuleIsPath(module_pattern);
    auto iter = get_vmodule_info().emplace(get_vmodule_info().cbegin(), std::string(module_pattern), module_is_path, log_level);

    // This is a memory optimization to avoid storing patterns that will never
    // match due to exit early semantics. Primarily optimized for our own unit
    // tests.
    get_vmodule_info().erase(std::remove_if(++iter, get_vmodule_info().end(),
...
int RegisterAndInitialize(VLogSite* v) ABSL_LOCKS_EXCLUDED(mutex)
{
    ...
    if (v->next_.compare_exchange_strong(old, h, std::memory_order_seq_cst, std::memory_order_seq_cst)) {
        ...
        while (!site_list_head.compare_exchange_weak(h, v, std::memory_order_seq_cst, std::memory_order_seq_cst)) {
    ...
    if (v->v_.compare_exchange_strong(old_v, new_v, std::memory_order_seq_cst, std::memory_order_seq_cst)) {
        return new_v;
    }
    return old_v;
}

void UpdateVLogSites() ABSL_UNLOCK_FUNCTION(mutex) ABSL_LOCKS_EXCLUDED(GetUpdateSitesMutex())
{
...
void UpdateVModule(absl::string_view vmodule) ABSL_LOCKS_EXCLUDED(mutex, GetUpdateSitesMutex())
{
    ...
    for (absl::string_view glob_level : absl::StrSplit(vmodule, ',')) {
        ...
        if (eq == glob_level.npos)
            continue;
        const absl::string_view glob = glob_level.substr(0, eq);
        int level;
        if (!absl::SimpleAtoi(glob_level.substr(eq + 1), &level))
            continue;
...
int UpdateGlobalVLogLevel(int v) ABSL_LOCKS_EXCLUDED(mutex, GetUpdateSitesMutex())
{
    mutex.Lock(); // Unlocked by UpdateVLogSites().
...
int PrependVModule(absl::string_view module_pattern, int log_level) ABSL_LOCKS_EXCLUDED(mutex, GetUpdateSitesMutex())
{
    mutex.Lock(); // Unlocked by UpdateVLogSites().
...
void OnVLogVerbosityUpdate(std::function<void()> cb) ABSL_LOCKS_EXCLUDED(GetUpdateSitesMutex())
{
    absl::MutexLock ul(GetUpdateSitesMutex());
    if (!update_callbacks)
