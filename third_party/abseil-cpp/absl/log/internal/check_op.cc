...
#define DEFINE_CHECK_STROP_IMPL(name, func, expected)                                                                                                          \
    std::string* Check##func##expected##Impl(const char* s1, const char* s2, const char* exprtext)                                                             \
    {                                                                                                                                                          \
        bool equal = s1 == s2 || (s1 && s2 && !func(s1, s2));                                                                                                  \
        if (equal == expected) {                                                                                                                               \
            return nullptr;                                                                                                                                    \
        } else {                                                                                                                                               \
            return new std::string(absl::StrCat(exprtext, " (", s1, " vs. ", s2, ")"));                                                                        \
        }                                                                                                                                                      \
    }
...
StringifySink::StringifySink(std::ostream& os)
    : os_(os)
{
}

void StringifySink::Append(absl::string_view text)
{
    os_ << text;
}
