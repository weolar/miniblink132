    ...
    while (field.DecodeFrom(&buf)) {
        ...
        case ValueTag::kStringLiteral:
            if (field.type() == WireType::kLengthDelimited)
                if (log_internal::AppendTruncated(field.string_value(), dst) < field.string_value().size())
                    return false;
        }
    }
    return true;
...
struct LogMessage::LogMessageData final {
    ...
    absl::Span<char>& encoded_remaining()
    {
        if (encoded_remaining_actual_do_not_use_directly.data() == nullptr) {
            encoded_remaining_actual_do_not_use_directly = absl::MakeSpan(encoded_buf);
...
{
    EncodeStringTruncate(EventTag::kFileName, entry.source_filename(), &encoded_remaining());
    EncodeVarint(EventTag::kFileLine, entry.source_line(), &encoded_remaining());
    EncodeVarint(EventTag::kTimeNsecs, absl::ToUnixNanos(entry.timestamp()), &encoded_remaining());
    EncodeVarint(EventTag::kSeverity, ProtoSeverity(entry.log_severity(), entry.verbosity()), &encoded_remaining());
...
void LogMessage::LogMessageData::FinalizeEncodingAndFormat()
{
    ...
    absl::Span<const char> encoded_data(encoded_buf.data(), static_cast<size_t>(encoded_remaining().data() - encoded_buf.data()));
    ...
    entry.prefix_len_ = entry.prefix() ? log_internal::FormatLogPrefix(entry.log_severity(), entry.timestamp(), entry.tid(), entry.source_basename(),
                            entry.source_line(), log_internal::ThreadIsLoggingToLogSink() ? PrefixFormat::kRaw : PrefixFormat::kNotRaw, string_remaining)
                                       : 0;
    ...
    while (field.DecodeFrom(&encoded_data)) {
        ...
        case EventTag::kValue:
            if (field.type() != WireType::kLengthDelimited)
                continue;
            if (PrintValue(string_remaining, field.bytes_value()))
                continue;
            break;
        }
    }
    auto chars_written = static_cast<size_t>(string_remaining.data() - string_buf.data());
    string_buf[chars_written++] = '\n';
    string_buf[chars_written++] = '\0';
    entry.text_message_with_prefix_and_newline_and_nul_ = absl::MakeSpan(string_buf).subspan(0, chars_written);
}

LogMessage::LogMessage(const char* file, int line, absl::LogSeverity severity)
    : data_(absl::make_unique<LogMessageData>(file, line, severity, absl::Now()))
{
...
LogMessage::~LogMessage()
{
    ...
    if (data_->entry.log_severity() < static_cast<absl::LogSeverity>(ABSL_MIN_LOG_LEVEL) && data_->entry.log_severity() < absl::LogSeverity::kFatal) {
        return;
    }
#endif
    Flush();
...
void LogMessage::Flush()
{
    ...
    if (data_->is_perror) {
        InternalStream() << ": " << absl::base_internal::StrError(errno_saver_()) << " [" << errno_saver_() << "]";
    }

    // Have we already seen a fatal message?
    ABSL_CONST_INIT static std::atomic<bool> seen_fatal(false);
    if (data_->entry.log_severity() == absl::LogSeverity::kFatal && absl::log_internal::ExitOnDFatal()) {
        ...
        if (seen_fatal.compare_exchange_strong(expected_seen_fatal, true, std::memory_order_relaxed)) {
            data_->first_fatal = true;
        }
    }

    data_->FinalizeEncodingAndFormat();
    data_->entry.encoding_ = absl::string_view(data_->encoded_buf.data(), static_cast<size_t>(data_->encoded_remaining().data() - data_->encoded_buf.data()));
    SendToLog();
}
...
LogMessage::OstreamView::OstreamView(LogMessageData& message_data)
    : data_(message_data)
    , encoded_remaining_copy_(data_.encoded_remaining())
{
...
LogMessage::OstreamView::~OstreamView()
{
    ...
    const absl::Span<const char> contents(pbase(), static_cast<size_t>(pptr() - pbase()));
    if (contents.empty())
        return;
...
{
    return data_->entry.log_severity() == absl::LogSeverity::kFatal && absl::log_internal::ExitOnDFatal();
}
...
void LogMessage::PrepareToDie()
{
    ...
    if (!data_->fail_quietly) {
        ...
        log_internal::LogToSinks(data_->entry, absl::MakeSpan(data_->extra_sinks), data_->extra_sinks_only);
        ...
        debugging_internal::DumpStackTrace(
            0, log_internal::MaxFramesInLogStackTrace(), log_internal::ShouldSymbolizeLogStackTrace(), WriteToString, &data_->entry.stacktrace_);
    }
}
...
void LogMessage::LogBacktraceIfNeeded()
{
    ...
    if (!absl::log_internal::ShouldLogBacktraceAt(data_->entry.source_basename(), data_->entry.source_line()))
        return;
    OstreamView view(*data_);
    view.stream() << " (stacktrace:\n";
    debugging_internal::DumpStackTrace(
        1, log_internal::MaxFramesInLogStackTrace(), log_internal::ShouldSymbolizeLogStackTrace(), WriteToStream, &view.stream());
    view.stream() << ") ";
}
...
template <LogMessage::StringType str_type> void LogMessage::CopyToEncodedBuffer(absl::string_view str)
{
    auto encoded_remaining_copy = data_->encoded_remaining();
    auto start = EncodeMessageStart(EventTag::kValue, BufferSizeFor(WireType::kLengthDelimited) + str.size(), &encoded_remaining_copy);
    ...
    if (EncodeStringTruncate(str_type == StringType::kLiteral ? ValueTag::kStringLiteral : ValueTag::kString, str, &encoded_remaining_copy)) {
...
template void LogMessage::CopyToEncodedBuffer<LogMessage::StringType::kNotLiteral>(absl::string_view str);
template <LogMessage::StringType str_type> void LogMessage::CopyToEncodedBuffer(char ch, size_t num)
{
    auto encoded_remaining_copy = data_->encoded_remaining();
    auto value_start = EncodeMessageStart(EventTag::kValue, BufferSizeFor(WireType::kLengthDelimited) + num, &encoded_remaining_copy);
    auto str_start = EncodeMessageStart(str_type == StringType::kLiteral ? ValueTag::kStringLiteral : ValueTag::kString, num, &encoded_remaining_copy);
    if (str_start.data()) {
...
template void LogMessage::CopyToEncodedBuffer<LogMessage::StringType::kNotLiteral>(char ch, size_t num);
...
LogMessageFatal::LogMessageFatal(const char* file, int line, absl::string_view failure_msg)
    : LogMessage(file, line, absl::LogSeverity::kFatal)
{
...
LogMessageQuietlyDebugFatal::LogMessageQuietlyDebugFatal(const char* file, int line)
    : LogMessage(file, line, absl::LogSeverity::kFatal)
{
    SetFailQuietly();
}
...
LogMessageQuietlyFatal::LogMessageQuietlyFatal(const char* file, int line, absl::string_view failure_msg)
