...
template <typename T> inline std::enable_if_t<!std::is_signed<T>::value> PutLeadingWhitespace(T tid, char*& p)
{
    if (tid < 10)
        *p++ = ' ';
    if (tid < 100)
...
template <typename T> inline std::enable_if_t<std::is_signed<T>::value> PutLeadingWhitespace(T tid, char*& p)
{
    ...
    if (tid > -100 && tid < 1000)
        *p++ = ' ';
    if (tid > -1000 && tid < 10000)
        *p++ = ' ';
    if (tid > -10000 && tid < 100000)
        *p++ = ' ';
    if (tid > -100000 && tid < 1000000)
        *p++ = ' ';
}
...
size_t FormatBoundedFields(absl::LogSeverity severity, absl::Time timestamp, log_internal::Tid tid, absl::Span<char>& buf)
{
    ...
    if (ABSL_PREDICT_FALSE(tz == nullptr)) {
        ...
        int snprintf_result = absl::SNPrintF(buf.data(), buf.size(), "%c0000 00:00:%02d.%06d %7d ", absl::LogSeverityName(severity)[0],
            static_cast<int>(tv.tv_sec), static_cast<int>(tv.tv_usec), static_cast<int>(tid));
    ...
    *p++ = ' ';
    ...
    p += 2;
    *p++ = ':';
    absl::numbers_internal::PutTwoDigits(static_cast<uint32_t>(ci.cs.minute()), p);
    p += 2;
    *p++ = ':';
    absl::numbers_internal::PutTwoDigits(static_cast<uint32_t>(ci.cs.second()), p);
    p += 2;
    *p++ = '.';
    ...
    p += 2;
    absl::numbers_internal::PutTwoDigits(static_cast<uint32_t>(usecs / 100 % 100), p);
    p += 2;
...
{
    constexpr size_t kLineFieldMaxLen = sizeof(":] ") + (1 + std::numeric_limits<int>::digits10 + 1) - sizeof("");
...
{
    return absl::StrFormat("%c%02d%02d %02d:%02d:%02d.%06d %7d %s:%d] %s%s", absl::LogSeverityName(severity)[0], civil_second.month(), civil_second.day(),
        civil_second.hour(), civil_second.minute(), civil_second.second(), absl::ToInt64Microseconds(subsecond), tid, basename, line,
