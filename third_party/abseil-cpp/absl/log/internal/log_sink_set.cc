...
bool& ThreadIsLoggingStatus()
{
    ...
    static const bool unused = [] {
        if (pthread_key_create(&thread_is_logging_key, [](void* data) { delete reinterpret_cast<bool*>(data); })) {
    ...
    bool* thread_is_logging_ptr = reinterpret_cast<bool*>(pthread_getspecific(thread_is_logging_key));
...
public:
    ...
    {
        if (entry.log_severity() < absl::StderrThreshold() && absl::log_internal::IsInitialized()) {
            return;
        }
        ...
        absl::call_once(warn_if_not_initialized, []() {
            if (absl::log_internal::IsInitialized())
                return;
            const char w[] = "WARNING: All log messages before absl::InitializeLog() is called"
        ...
        if (!entry.stacktrace().empty()) {
            ...
            // ReprintFatalMessage
            absl::log_internal::WriteToStderr(entry.text_message_with_prefix_and_newline(), entry.log_severity());
        }
    }
};
...
public:
    ...
    void Send(const absl::LogEntry& entry) override
    {
        ...
        __android_log_write(level, tag, entry.text_message_with_prefix_and_newline_c_str());
...
public:
    ...
    void LogToSinks(const absl::LogEntry& entry, absl::Span<absl::LogSink*> extra_sinks, bool extra_sinks_only) ABSL_LOCKS_EXCLUDED(guard_)
    {
        ...
        if (!extra_sinks_only) {
            if (ThreadIsLoggingToLogSink()) {
                ...
                auto status_cleanup = absl::MakeCleanup([] { ThreadIsLoggingStatus() = false; });
...
private:
    ...
    static void SendToSinks(const absl::LogEntry& entry, absl::Span<absl::LogSink*> sinks)
    {
...
void FlushLogSinks()
{
    log_internal::GlobalSinks().FlushLogSinks();
}
