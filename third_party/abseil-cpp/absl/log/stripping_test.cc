...
public:
    explicit FileHasSubstrMatcher(absl::string_view needle)
        : needle_(needle)
    {
    }

    bool MatchAndExplain(FILE* fp, ::testing::MatchResultListener* listener) const override
    {
        std::string buf(std::max<std::string::size_type>(needle_.size() * 2, 163840000), '\0');
        ...
        while (true) {
            ...
            while (buf_data_size < buf.size()) {
                const size_t ret = fread(&buf[buf_data_size], sizeof(char), buf.size() - buf_data_size, fp);
                if (ret == 0)
                    break;
                buf_data_size += ret;
            }
    ...
    {
        *os << "contains the string \"" << needle_ << "\" (base64(\"" << Base64UnescapeOrDie(needle_) << "\"))";
    }
...
protected:
    void SetUp() override
    {
        ...
        GTEST_SKIP() << "StrippingTests skipped since this platform is not optimized";
#endif
    }
    ...
    std::unique_ptr<FILE, std::function<void(FILE*)>> OpenTestExecutable()
    {
        ...
        std::unique_ptr<FILE, std::function<void(FILE*)>> fp(fopen("/proc/self/exe", "rb"), [](FILE* fp) { fclose(fp); });
        if (!fp) {
        ...
        std::unique_ptr<FILE, std::function<void(FILE*)>> fp(
            fopen(absl::StrCat("/pkg/bin/", absl::flags_internal::ShortProgramInvocationName()).c_str(), "rb"), [](FILE* fp) { fclose(fp); });
        if (!fp) {
            const std::string err = absl::base_internal::StrError(errno);
        ...
        std::unique_ptr<FILE, std::function<void(FILE*)>> fp(fopen(path.c_str(), "rb"), [](FILE* fp) { fclose(fp); });
        if (!fp) {
            const std::string err = absl::base_internal::StrError(errno);
        ...
        while (true) {
            const uint32_t ret = ::GetModuleFileName(nullptr, &path[0], static_cast<DWORD>(path.size()));
            if (ret == 0) {
                absl::FPrintF(stderr,
        ...
        std::unique_ptr<FILE, std::function<void(FILE*)>> fp(_tfopen(path.c_str(), _T("rb")), [](FILE* fp) { fclose(fp); });
        if (!fp)
            absl::FPrintF(stderr, "Failed to open executable\n");
        return fp;
#else
        absl::FPrintF(stderr, "OpenTestExecutable() unimplemented on this platform\n");
        return nullptr;
#endif
    }
...
{
    constexpr char kEncodedPositiveControl[] = "U3RyaXBwaW5nVGVzdC5Qb3NpdGl2ZUNvbnRyb2w=";
    const std::string encoded_negative_control = absl::Base64Escape("StrippingTest.NegativeControl");

    // Verify this mainly so we can encode other strings and know definitely they
    // won't encode to `kEncodedPositiveControl`.
    EXPECT_THAT(Base64UnescapeOrDie("U3RyaXBwaW5nVGVzdC5Qb3NpdGl2ZUNvbnRyb2w="), Eq("StrippingTest.PositiveControl"));
...
TEST_F(StrippingTest, LiteralInExpression)
{
    ...
    // as would happen if we used a literal.  We might (or might not) leave it
    // lying around later; that's what the tests are for!
    const std::string needle = absl::Base64Escape("StrippingTest.LiteralInExpression");
    LOG(INFO) << absl::StrCat("secret: ", "U3RyaXBwaW5nVGVzdC5MaXRlcmFsSW5FeHByZXNzaW9u");
...
TEST_F(StrippingTest, Fatal)
{
    ...
    if (kReallyDie)
        LOG(FATAL) << "U3RyaXBwaW5nVGVzdC5GYXRhbA==";
...
TEST_F(StrippingTest, DFatal)
{
    ...
    // checking that it's stripped.
    if (kReallyDie)
        LOG(DFATAL) << "U3RyaXBwaW5nVGVzdC5ERmF0YWw=";
...
TEST_F(StrippingTest, Check)
{
    ...
    if (kReallyDie) {
        CHECK(U3RyaXBwaW5nVGVzdENoZWNrVmFy != U3RyaXBwaW5nVGVzdENoZWNrVmFy) << "U3RyaXBwaW5nVGVzdC5DaGVjaw==";
    }
...
TEST_F(StrippingTest, CheckOp)
{
    ...
    const std::string var_needle1 = absl::Base64Escape("StrippingTestCheckOpVar1");
    const std::string var_needle2 = absl::Base64Escape("StrippingTestCheckOpVar2");
    ...
    if (kReallyDie) {
        CHECK_EQ(U3RyaXBwaW5nVGVzdENoZWNrT3BWYXIx, U3RyaXBwaW5nVGVzdENoZWNrT3BWYXIy) << "U3RyaXBwaW5nVGVzdC5DaGVja09w";
    }
...
{
    // See `StrippingTest.Check` for some hairy implementation notes.
    const std::string var_needle1 = absl::Base64Escape("StrippingTestCheckStrOpVar1");
    const std::string var_needle2 = absl::Base64Escape("StrippingTestCheckStrOpVar2");
    ...
    if (kReallyDie) {
        CHECK_STREQ(U3RyaXBwaW5nVGVzdENoZWNrU3RyT3BWYXIx, U3RyaXBwaW5nVGVzdENoZWNrU3RyT3BWYXIy) << "U3RyaXBwaW5nVGVzdC5DaGVja1N0ck9w";
    }
...
TEST_F(StrippingTest, CheckOk)
{
    ...
    if (x) {
        U3RyaXBwaW5nVGVzdENoZWNrT2tWYXIx = absl::InvalidArgumentError("Stripping this is not my job!");
    }
    if (kReallyDie) {
        CHECK_OK(U3RyaXBwaW5nVGVzdENoZWNrT2tWYXIx) << "U3RyaXBwaW5nVGVzdC5DaGVja09r";
    }
