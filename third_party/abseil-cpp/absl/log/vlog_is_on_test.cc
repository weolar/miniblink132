...
TEST_F(VLogIsOnTest, PatternOverridesLessGenericOneWithoutMaxVerbosityAndMinLogLevel)
{
    if (MaxLogVerbosity().has_value() || MinLogLevel().has_value()) {
        GTEST_SKIP();
    }
...
TEST_F(VLogIsOnTest, PatternDoesNotOverridesMoreGenericOneWithoutMaxVerbosityAndMinLogLevel)
{
    if (MaxLogVerbosity().has_value() || MinLogLevel().has_value()) {
        GTEST_SKIP();
    }
