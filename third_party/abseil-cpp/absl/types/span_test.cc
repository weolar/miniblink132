...
static_assert(
    !absl::type_traits_internal::IsOwner<absl::Span<int>>::value && absl::type_traits_internal::IsView<absl::Span<int>>::value, "Span is a view, not an owner");

MATCHER_P(DataIs, data, absl::StrCat("data() ", negation ? "isn't " : "is ", testing::PrintToString(data)))
{
    return arg.data() == data;
}

template <typename T> auto SpanIs(T data, size_t size) -> decltype(testing::AllOf(DataIs(data), testing::SizeIs(size)))
{
    return testing::AllOf(DataIs(data), testing::SizeIs(size));
}

template <typename Container> auto SpanIs(const Container& c) -> decltype(SpanIs(c.data(), c.size()))
{
...
TEST(IntSpan, ArrayCtor)
{
    ...
    EXPECT_TRUE((std::is_constructible<absl::Span<const int>, const int[3]>::value));
    ...
    EXPECT_TRUE((std::is_convertible<const int[3], absl::Span<const int>>::value));
}

template <typename T> void TakesGenericSpan(absl::Span<T>)
{
}
...
TEST(IntSpan, ContainerCtor)
{
    ...
    EXPECT_TRUE((std::is_convertible<std::vector<int>&, absl::Span<const int>>::value));
    EXPECT_TRUE((std::is_convertible<absl::Span<int>&, absl::Span<const int>>::value));
...
TEST(IntSpan, ShallowConstness)
{
    ...
    absl::Span<int> s(c); // We should be able to do this even though data() is const.
    s[0] = -1;
...
{
    EXPECT_TRUE((std::is_constructible<absl::Span<const int* const>, std::vector<int*>>::value));
    EXPECT_TRUE((std::is_constructible<absl::Span<const int* const>, std::vector<const int*>>::value));
    EXPECT_FALSE((std::is_constructible<absl::Span<const int*>, std::vector<int*>>::value));
    EXPECT_FALSE((std::is_constructible<absl::Span<int*>, std::vector<const int*>>::value));
}
...
{
    EXPECT_FALSE((std::is_constructible<absl::Span<int>, TypeWithMisleadingData&>::value));
    EXPECT_FALSE((std::is_constructible<absl::Span<int>, TypeWithMisleadingSize&>::value));
}
...
TEST(IntSpan, SpanOfDerived)
{
    ...
    EXPECT_FALSE((std::is_constructible<absl::Span<Base>, std::vector<Derived>>::value));
}
...
TEST(ConstIntSpan, InitializerListConversion)
{
    ...
    EXPECT_FALSE((std::is_constructible<absl::Span<int>, std::initializer_list<int>>::value));
    EXPECT_FALSE((std::is_convertible<absl::Span<int>, std::initializer_list<int>>::value));
}
...
TEST(IntSpan, AtThrows)
{
    ...
    EXPECT_EQ(s.at(9), 9);
    ABSL_BASE_INTERNAL_EXPECT_FAIL(s.at(10), std::out_of_range, "failed bounds check");
}
...
TEST(IntSpan, Subspan)
{
    ...
    EXPECT_THAT(absl::MakeSpan(empty).subspan(0, absl::Span<const int>::npos), SpanIs(empty));
    ...
    EXPECT_THAT(absl::MakeSpan(ramp).subspan(0, absl::Span<const int>::npos), SpanIs(ramp));
    EXPECT_THAT(absl::MakeSpan(ramp).subspan(0, 3), SpanIs(ramp.data(), 3));
    EXPECT_THAT(absl::MakeSpan(ramp).subspan(5, absl::Span<const int>::npos), SpanIs(ramp.data() + 5, 5));
...
template <typename Expected, typename T> void CheckType(const T& /* value */)
{
...
TEST(IntSpan, ExposesContainerTypesAndConsts)
{
    ...
    EXPECT_TRUE((std::is_convertible<decltype(slice.begin()), absl::Span<int>::const_iterator>::value));
    CheckType<absl::Span<int>::const_iterator>(slice.cbegin());
    EXPECT_TRUE((std::is_convertible<decltype(slice.end()), absl::Span<int>::const_iterator>::value));
    CheckType<absl::Span<int>::const_iterator>(slice.cend());
    CheckType<absl::Span<int>::reverse_iterator>(slice.rend());
    EXPECT_TRUE((std::is_convertible<decltype(slice.rend()), absl::Span<int>::const_reverse_iterator>::value));
    ...
    testing::StaticAssertTypeEq<const int&, absl::Span<const int>::const_reference>();
...
TEST(IntSpan, IteratorsAndReferences)
{
    ...
    auto accept_const_reverse_iterator = [](absl::Span<int>::const_reverse_iterator) {};
...
template <int i> struct ConstexprTester { };
...
TEST(Span, Hash)
{
    ...
    EXPECT_TRUE(absl::VerifyTypeImplementsAbslHashCorrectly({ // Empties
