...
#define ABSL_VARIANT_TEST_EXPECT_FAIL(expr, exception_t, text) EXPECT_THROW(expr, exception_t)

#else

#define ABSL_VARIANT_TEST_EXPECT_FAIL(expr, exception_t, text) EXPECT_DEATH_IF_SUPPORTED(expr, text)

#endif // ABSL_HAVE_EXCEPTIONS

#define ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(...) ABSL_VARIANT_TEST_EXPECT_FAIL((void)(__VA_ARGS__), absl::bad_variant_access, "Bad variant access")
...
struct MoveCanThrow {
    ...
    MoveCanThrow(const MoveCanThrow& other)
        : v(other.v)
    {
    }
    MoveCanThrow& operator=(const MoveCanThrow& /*other*/)
    {
        return *this;
    }
    int v;
...
struct SpecialSwap {
    ...
    bool operator==(SpecialSwap other) const
    {
        return i == other.i;
    }
    int i;
...
struct MoveOnlyWithListConstructor {
    ...
    explicit MoveOnlyWithListConstructor(std::initializer_list<int> /*ilist*/, int value)
        : value(value)
    {
    }
...
template <class T> struct ExceptionOnConversion {
    operator T() const
    { // NOLINT(runtime/explicit)
...
template <class H, class... T> void ToValuelessByException(absl::variant<H, T...>& v)
{ // NOLINT
    try {
...
// value of type T
template <typename T, size_t N> struct ValueHolder {
    explicit ValueHolder(const T& x)
        : value(x)
    {
    }
...
template <typename T, size_t N> const size_t ValueHolder<T, N>::kIndex;

// The following three functions make ValueHolder compatible with
// EXPECT_EQ and EXPECT_NE
template <typename T, size_t N> inline bool operator==(const ValueHolder<T, N>& left, const ValueHolder<T, N>& right)
{
...
template <typename T, size_t N> inline std::ostream& operator<<(std::ostream& stream, const ValueHolder<T, N>& object)
{
    return stream << object.value;
}

// Makes a variant holding twelve uniquely typed T wrappers.
template <typename T> struct VariantFactory {
    typedef variant<ValueHolder<T, 1>, ValueHolder<T, 2>, ValueHolder<T, 3>, ValueHolder<T, 4>> Type;
};
...
struct IncrementInDtor {
    explicit IncrementInDtor(int* counter)
        : counter(counter)
    {
    }
    ~IncrementInDtor()
...
struct IncrementInDtorCopyCanThrow {
    explicit IncrementInDtorCopyCanThrow(int* counter)
        : counter(counter)
    {
    }
...
// counter
inline bool operator==(const IncrementInDtor& left, const IncrementInDtor& right)
{
    return left.counter == right.counter;
}

// This is defined so EXPECT_EQ can work with IncrementInDtor
inline std::ostream& operator<<(std::ostream& stream, const IncrementInDtor& object)
{
...
public:
    ...
    CopyNoAssign(const CopyNoAssign& other)
        : foo(other.foo)
    {
    }
    int foo;
...
// each type.
template <typename T> class VariantTypesTest : public ::testing::Test { };
...
TEST(VariantTest, TestDefaultConstructor)
{
    ...
    EXPECT_FALSE(std::is_default_constructible<variant<NonDefaultConstructible>>::value);
    EXPECT_FALSE((std::is_default_constructible<variant<NonDefaultConstructible, int>>::value));
    EXPECT_TRUE((std::is_default_constructible<variant<int, NonDefaultConstructible>>::value));
}
...
TYPED_TEST(VariantTypesTest, TestCopyCtor)
{
    ...
    EXPECT_TRUE(absl::holds_alternative<value_type1>(copied) || TypeParam::kIndex != 1);
    EXPECT_TRUE(absl::holds_alternative<value_type2>(copied) || TypeParam::kIndex != 2);
    EXPECT_TRUE(absl::holds_alternative<value_type3>(copied) || TypeParam::kIndex != 3);
    EXPECT_TRUE(absl::holds_alternative<value_type4>(copied) || TypeParam::kIndex != 4);
    EXPECT_TRUE((absl::get_if<value_type1>(&original) == absl::get_if<value_type1>(&copied)) || TypeParam::kIndex == 1);
    EXPECT_TRUE((absl::get_if<value_type2>(&original) == absl::get_if<value_type2>(&copied)) || TypeParam::kIndex == 2);
    EXPECT_TRUE((absl::get_if<value_type3>(&original) == absl::get_if<value_type3>(&copied)) || TypeParam::kIndex == 3);
    EXPECT_TRUE((absl::get_if<value_type4>(&original) == absl::get_if<value_type4>(&copied)) || TypeParam::kIndex == 4);
    EXPECT_TRUE((absl::get_if<value_type1>(&original) == absl::get_if<value_type1>(&copied)) || TypeParam::kIndex == 1);
    EXPECT_TRUE((absl::get_if<value_type2>(&original) == absl::get_if<value_type2>(&copied)) || TypeParam::kIndex == 2);
    EXPECT_TRUE((absl::get_if<value_type3>(&original) == absl::get_if<value_type3>(&copied)) || TypeParam::kIndex == 3);
    EXPECT_TRUE((absl::get_if<value_type4>(&original) == absl::get_if<value_type4>(&copied)) || TypeParam::kIndex == 4);
...
template <class> struct MoveOnly {
    MoveOnly() = default;
...
template <class T> union SingleUnion {
    T member;
};

// NOTE: These don't work with types that can't be union members.
//       They are just for testing.
template <class T> struct is_trivially_move_constructible : std::is_move_constructible<SingleUnion<T>>::type { };

template <class T> struct is_trivially_move_assignable : absl::is_move_assignable<SingleUnion<T>>::type { };
...
TYPED_TEST(VariantTypesTest, TestValueCtor)
{
    ...
    const TypeParam value(TypeParam::kIndex);
    Variant v(value);
    EXPECT_TRUE(absl::holds_alternative<value_type1>(v) || TypeParam::kIndex != 1);
    EXPECT_TRUE(absl::holds_alternative<value_type2>(v) || TypeParam::kIndex != 2);
    EXPECT_TRUE(absl::holds_alternative<value_type3>(v) || TypeParam::kIndex != 3);
    EXPECT_TRUE(absl::holds_alternative<value_type4>(v) || TypeParam::kIndex != 4);
    EXPECT_TRUE(nullptr != absl::get_if<value_type1>(&v) || TypeParam::kIndex != 1);
    EXPECT_TRUE(nullptr != absl::get_if<value_type2>(&v) || TypeParam::kIndex != 2);
    EXPECT_TRUE(nullptr != absl::get_if<value_type3>(&v) || TypeParam::kIndex != 3);
    EXPECT_TRUE(nullptr != absl::get_if<value_type4>(&v) || TypeParam::kIndex != 4);
    EXPECT_TRUE(nullptr != absl::get_if<value_type1>(&v) || TypeParam::kIndex != 1);
    EXPECT_TRUE(nullptr != absl::get_if<value_type2>(&v) || TypeParam::kIndex != 2);
    EXPECT_TRUE(nullptr != absl::get_if<value_type3>(&v) || TypeParam::kIndex != 3);
    EXPECT_TRUE(nullptr != absl::get_if<value_type4>(&v) || TypeParam::kIndex != 4);
...
TEST(VariantTest, InPlaceIndex)
{
    ...
    EXPECT_TRUE(absl::holds_alternative<NonCopyable>(variant<NonCopyable>(in_place_index_t<0> {})));
...
TEST(VariantTest, InPlaceIndexVariableTemplate)
{
    ...
    // Verify that a variant with only non-copyables can still be constructed.
    EXPECT_TRUE(absl::holds_alternative<NonCopyable>(variant<NonCopyable>(in_place_index<0>)));
...
TEST(VariantTest, TestEmplaceInitializerList)
{
    ...
    MoveOnlyWithListConstructor& emplace_result = v1.emplace<MoveOnlyWithListConstructor>({ 1, 2, 3, 4, 5 }, 6);
    ASSERT_TRUE(absl::holds_alternative<MoveOnlyWithListConstructor>(v1));
    EXPECT_EQ(6, absl::get<MoveOnlyWithListConstructor>(v1).value);
...
TEST(VariantTest, TestEmplaceIndexInitializerList)
{
    ...
    Var v1(absl::in_place_index<0>, 555);
    MoveOnlyWithListConstructor& emplace_result = v1.emplace<3>({ 1, 2, 3, 4, 5 }, 6);
    ASSERT_TRUE(absl::holds_alternative<MoveOnlyWithListConstructor>(v1));
    EXPECT_EQ(6, absl::get<MoveOnlyWithListConstructor>(v1).value);
    EXPECT_EQ(&emplace_result, &absl::get<MoveOnlyWithListConstructor>(v1));
}
...
{
    {
        ...
        EXPECT_TRUE((std::is_same<float, absl::variant_alternative_t<0, V>>::value));
        EXPECT_TRUE((std::is_same<const float, absl::variant_alternative_t<0, const V>>::value));
        EXPECT_TRUE((std::is_same<volatile float, absl::variant_alternative_t<0, volatile V>>::value));
        EXPECT_TRUE((std::is_same<const volatile float, absl::variant_alternative_t<0, const volatile V>>::value));

        EXPECT_TRUE((std::is_same<int, absl::variant_alternative_t<1, V>>::value));
        EXPECT_TRUE((std::is_same<const int, absl::variant_alternative_t<1, const V>>::value));
        EXPECT_TRUE((std::is_same<volatile int, absl::variant_alternative_t<1, volatile V>>::value));
        EXPECT_TRUE((std::is_same<const volatile int, absl::variant_alternative_t<1, const volatile V>>::value));

        EXPECT_TRUE((std::is_same<const char*, absl::variant_alternative_t<2, V>>::value));
        EXPECT_TRUE((std::is_same<const char* const, absl::variant_alternative_t<2, const V>>::value));
        EXPECT_TRUE((std::is_same<const char* volatile, absl::variant_alternative_t<2, volatile V>>::value));
        EXPECT_TRUE((std::is_same<const char* const volatile, absl::variant_alternative_t<2, const volatile V>>::value));
    }

    {
        ...
        EXPECT_TRUE((std::is_same<volatile int, absl::variant_alternative_t<1, V>>::value));
        EXPECT_TRUE((std::is_same<const volatile int, absl::variant_alternative_t<1, const V>>::value));
        EXPECT_TRUE((std::is_same<volatile int, absl::variant_alternative_t<1, volatile V>>::value));
        EXPECT_TRUE((std::is_same<const volatile int, absl::variant_alternative_t<1, const volatile V>>::value));
...
TEST(VariantTest, BadGetIndex)
{
...

    {
        ...
        ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(absl::get<1>(std::move(const_v))); // NOLINT
    }

    {
        ...
        ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(absl::get<0>(std::move(const_v))); // NOLINT
    }
}
...
TEST(VariantTest, BadGetType)
{
...

    {
        ...
        ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(absl::get<std::string>(std::move(v)));

        const Var& const_v = v;
        ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(absl::get<std::string>(const_v));
        ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(absl::get<std::string>(std::move(const_v))); // NOLINT
    }

    {
        ...
        ABSL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(absl::get<int>(std::move(const_v))); // NOLINT
    }
}
...
{
    variant<MoveCanThrow, std::string> int_v(1), string_v("Hello"), valueless(absl::in_place_index<0>), other_valueless(absl::in_place_index<0>);
    ToValuelessByException(valueless);
...
TEST(VariantTest, VisitRValue)
{
    ...
    struct Visitor {
        ...
        int operator()(const std::string&, const std::string&) const
        {
            return 0;
        }
        ...
        int operator()(std::string&&, std::string&&) const
        {
            return 3;
        } // NOLINT
    };
...
TEST(VariantTest, VisitResultTypeDifferent)
{
    ...
    struct Visitor {
        LValue_LValue operator()(const std::string&) const&
        {
            return {};
        }
        RValue_LValue operator()(std::string&&) const&
        {
            return {};
        } // NOLINT
        LValue_RValue operator()(const std::string&) &&
        {
            return {};
        }
        RValue_RValue operator()(std::string&&) &&
        {
            return {};
        } // NOLINT
    } visitor;

    EXPECT_TRUE((std::is_same<LValue_LValue, decltype(absl::visit(visitor, v))>::value));
    EXPECT_TRUE((std::is_same<RValue_LValue, decltype(absl::visit(visitor, std::move(v)))>::value));
    EXPECT_TRUE((std::is_same<LValue_RValue, decltype(absl::visit(Visitor {}, v))>::value));
    EXPECT_TRUE((std::is_same<RValue_RValue, decltype(absl::visit(Visitor {}, std::move(v)))>::value));
}
...
TEST(VariantTest, VisitVariadic)
{
    ...
    struct Visitor {
        ...
        std::pair<int, int> operator()(absl::string_view a, std::unique_ptr<int> b) const
        {
        ...
        std::pair<int, int> operator()(absl::string_view a, absl::string_view b) const
        {
            return { static_cast<int>(a.size()), static_cast<int>(b.size()) };
        }
    };

    EXPECT_THAT(absl::visit(Visitor(), A(1), B(std::unique_ptr<int>(new int(7)))), ::testing::Pair(1, 7));
    EXPECT_THAT(absl::visit(Visitor(), A(1), B(absl::string_view("ABC"))), ::testing::Pair(1, 3));
    EXPECT_THAT(absl::visit(Visitor(), A(std::string("BBBBB")), B(std::unique_ptr<int>(new int(7)))), ::testing::Pair(5, 7));
    EXPECT_THAT(absl::visit(Visitor(), A(std::string("BBBBB")), B(absl::string_view("ABC"))), ::testing::Pair(5, 3));
}
...
TEST(VariantTest, VisitMemberFunction)
{
    ...
    absl::variant<std::unique_ptr<const Class>> cp(absl::make_unique<const Class>());
    absl::variant<int> three = 3;
    absl::variant<int> two = 2;
...
TEST(VariantTest, VisitDataMember)
{
    ...
    absl::variant<std::unique_ptr<const Class>> cp(absl::make_unique<const Class>(Class { 42 }));
...
TEST(VariantTest, MonostateComparisons)
{
    ...
    EXPECT_TRUE(noexcept(std::declval<absl::monostate>() == std::declval<absl::monostate>()));
    EXPECT_TRUE(noexcept(std::declval<absl::monostate>() != std::declval<absl::monostate>()));
    EXPECT_TRUE(noexcept(std::declval<absl::monostate>() < std::declval<absl::monostate>()));
    EXPECT_TRUE(noexcept(std::declval<absl::monostate>() > std::declval<absl::monostate>()));
    EXPECT_TRUE(noexcept(std::declval<absl::monostate>() <= std::declval<absl::monostate>()));
...
TEST(VariantTest, Hash)
{
    ...
    static_assert(!type_traits_internal::IsHashable<variant<Hashable, NonHashable>>::value, "");
#endif
...
// result on the same value as different alternative.
...

    {
        ...
        static_assert(type_traits_internal::IsHashable<variant<const Hashable>>::value, "");
...
struct Convertible1 {
    ...
    Convertible1& operator=(const Convertible1&)
    {
        return *this;
    }
...
struct Convertible2 {
    ...
    Convertible2& operator=(const Convertible2&)
    {
        return *this;
    }
...
{
#if !defined(ABSL_USES_STD_VARIANT)
    variant<double, std::string> var(ConvertVariantTo<variant<double, std::string>>(variant<std::string, int>(0)));
    ASSERT_TRUE(absl::holds_alternative<double>(var));
    EXPECT_EQ(0.0, absl::get<double>(var));

    var = ConvertVariantTo<variant<double, std::string>>(variant<const char*, float>("foo"));
    ASSERT_TRUE(absl::holds_alternative<std::string>(var));
    EXPECT_EQ("foo", absl::get<std::string>(var));

    variant<double> singleton(ConvertVariantTo<variant<double>>(variant<int, float>(42)));
    ...
    variant<int32_t, uint32_t> variant2(ConvertVariantTo<variant<int32_t, uint32_t>>(variant<int32_t>(42)));
    ASSERT_TRUE(absl::holds_alternative<int32_t>(variant2));
    EXPECT_EQ(42, absl::get<int32_t>(variant2));

    variant2 = ConvertVariantTo<variant<int32_t, uint32_t>>(variant<uint32_t>(42u));
    ASSERT_TRUE(absl::holds_alternative<uint32_t>(variant2));
    EXPECT_EQ(42u, absl::get<uint32_t>(variant2));
#endif // !ABSL_USES_STD_VARIANT

    variant<Convertible1, Convertible2> variant3(ConvertVariantTo<variant<Convertible1, Convertible2>>((variant<Convertible2, Convertible1>(Convertible1()))));
    ASSERT_TRUE(absl::holds_alternative<Convertible1>(variant3));

    variant3 = ConvertVariantTo<variant<Convertible1, Convertible2>>(variant<Convertible2, Convertible1>(Convertible2()));
...
TEST(VariantTest, TestLvalueConversion)
{
    ...
    variant<double, std::string> destination(ConvertVariantTo<variant<double, std::string>>(source1));
    ...
    variant<int32_t, uint32_t> variant2(ConvertVariantTo<variant<int32_t, uint32_t>>(source5));
    ASSERT_TRUE(absl::holds_alternative<int32_t>(variant2));
    EXPECT_EQ(42, absl::get<int32_t>(variant2));
    ...
    variant<Convertible1, Convertible2> variant3(ConvertVariantTo<variant<Convertible1, Convertible2>>(source7));
    ASSERT_TRUE(absl::holds_alternative<Convertible1>(variant3));
...
{
    using Variant = variant<std::unique_ptr<const int>, std::unique_ptr<const std::string>>;
    using OtherVariant = variant<std::unique_ptr<int>, std::unique_ptr<std::string>>;

    Variant var(ConvertVariantTo<Variant>(OtherVariant { absl::make_unique<int>(0) }));
...
TEST(VariantTest, DoesNotMoveFromLvalues)
{
    ...
    using Variant = variant<std::shared_ptr<const int>, std::shared_ptr<const std::string>>;
    ...
    Variant v2(v1);
    EXPECT_EQ(absl::get<std::shared_ptr<const int>>(v1), absl::get<std::shared_ptr<const int>>(v2));

    // Test copy-assignment operator
    v1 = std::make_shared<const std::string>("foo");
    v2 = v1;
    EXPECT_EQ(absl::get<std::shared_ptr<const std::string>>(v1), absl::get<std::shared_ptr<const std::string>>(v2));

    // Test converting copy constructor
    OtherVariant other(std::make_shared<int>(0));
    Variant v3(ConvertVariantTo<Variant>(other));
    EXPECT_EQ(absl::get<std::shared_ptr<int>>(other), absl::get<std::shared_ptr<const int>>(v3));

    other = std::make_shared<std::string>("foo");
    v3 = ConvertVariantTo<Variant>(other);
    EXPECT_EQ(absl::get<std::shared_ptr<std::string>>(other), absl::get<std::shared_ptr<const std::string>>(v3));
}

TEST(VariantTest, TestRvalueConversionViaConvertVariantTo)
{
#if !defined(ABSL_USES_STD_VARIANT)
    variant<double, std::string> var(ConvertVariantTo<variant<double, std::string>>(variant<std::string, int>(3)));
...
TEST(VariantTest, TestMoveConversionViaConvertVariantTo)
{
    ...
    Variant var(ConvertVariantTo<Variant>(OtherVariant { absl::make_unique<int>(3) }));
    EXPECT_THAT(absl::get_if<std::unique_ptr<const int>>(&var), Pointee(Pointee(3)));
...
TEST(VariantTest, TestCopyAndMoveTypeTraits)
{
    ...
    EXPECT_FALSE(absl::is_trivially_copy_constructible<variant<std::string>>::value);
...
struct TriviallyDestructible {
    ...
    TriviallyDestructible& operator=(TriviallyDestructible&&)
    {
        return *this;
    }
...
struct TriviallyMovable {
    ...
    TriviallyMovable& operator=(const TriviallyMovable&)
    {
        return *this;
    }
};
...
struct TriviallyCopyable {
    ...
    TriviallyCopyable& operator=(const TriviallyCopyable&)
    {
        return *this;
    }
};
...
TEST(VariantTest, TestTriviality)
{
...

    {
        ...
        EXPECT_FALSE(absl::is_trivially_copy_constructible<TrivMoveAssignVar>::value);
...

    {
        ...
        EXPECT_TRUE(absl::is_trivially_copy_constructible<TrivCopyAssignVar>::value);
