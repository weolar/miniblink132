...
template <typename C> using HasSize = std::is_integral<absl::decay_t<decltype(std::declval<C&>().size())>>;
...
template <typename T, typename C> using HasData = std::is_convertible<absl::decay_t<decltype(GetData(std::declval<C&>()))>*, T* const*>;
...
template <typename From, typename To> using EnableIfConvertibleTo = typename std::enable_if<std::is_convertible<From, To>::value>::type;
...
template <typename T>
struct IsView<T, absl::void_t<decltype(span_internal::GetData(std::declval<const T&>()))>, absl::void_t<decltype(span_internal::GetData(std::declval<T&>()))>> {
private:
...
template <typename T> using EnableIfIsView = std::enable_if_t<IsView<T>::value, int>;

template <typename T> using EnableIfNotIsView = std::enable_if_t<!IsView<T>::value, int>;
