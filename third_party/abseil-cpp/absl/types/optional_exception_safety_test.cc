...
template <typename OptionalT> bool ValueThrowsBadOptionalAccess(const OptionalT& optional)
try {
...
template <typename OptionalT> AssertionResult OptionalInvariants(OptionalT* optional_ptr)
{
    ...
    if (optional.has_value() && ValueThrowsBadOptionalAccess(optional)) {
        return AssertionFailure() << "Optional with value should not throw bad_optional_access when "
                                     "accessing the value.";
    }
    if (!optional.has_value() && !ValueThrowsBadOptionalAccess(optional)) {
        return AssertionFailure() << "Optional without a value should throw bad_optional_access when "
                                     "accessing the value.";
    }
    ...
    if (optional.has_value()) {
        return AssertionFailure() << "Optional should not contain a value after being reset.";
    }
...
template <typename OptionalT> AssertionResult CheckDisengaged(OptionalT* optional_ptr)
{
    auto& optional = *optional_ptr;

    if (optional.has_value()) {
        return AssertionFailure() << "Expected optional to not contain a value but a value was found.";
    }

    return AssertionSuccess();
}

template <typename OptionalT> AssertionResult CheckEngaged(OptionalT* optional_ptr)
{
    auto& optional = *optional_ptr;

    if (!optional.has_value()) {
        return AssertionFailure() << "Expected optional to contain a value but no value was found.";
    }
...
TEST(OptionalExceptionSafety, ThrowingConstructors)
{
    ...
    testing::TestThrowingCtor<absl::optional<ThrowerVec>>(
        absl::in_place, std::initializer_list<Thrower> { Thrower(), Thrower(), Thrower() }, testing::ThrowingAllocator<Thrower>());
}
...
TEST(OptionalExceptionSafety, Emplace)
{
    ...
    auto disengaged_test = MakeExceptionSafetyTester().WithContracts(OptionalInvariants<Optional>, CheckDisengaged<Optional>);
    auto disengaged_test_empty = disengaged_test.WithInitialValue(Optional());
    auto disengaged_test_nonempty = disengaged_test.WithInitialValue(Optional(kInitialInteger));

    auto emplace_thrower_directly = [](Optional* optional_ptr) { optional_ptr->emplace(kUpdatedInteger); };
...
TEST(OptionalExceptionSafety, EverythingThrowsSwap)
{
    // Test the basic guarantee plus test the result of optional::has_value()
    // remains the same
    auto test = MakeExceptionSafetyTester().WithContracts(OptionalInvariants<Optional>);
    auto disengaged_test_empty = test.WithInitialValue(Optional()).WithContracts(CheckDisengaged<Optional>);
    auto engaged_test_nonempty = test.WithInitialValue(Optional(kInitialInteger)).WithContracts(CheckEngaged<Optional>);
    ...
    auto swap_nonempty = [](Optional* optional_ptr) {
        auto nonempty = Optional(absl::in_place, kUpdatedInteger, testing::nothrow_ctor);
...
TEST(OptionalExceptionSafety, NoThrowMoveSwap)
{
    ...
    }
    {
        ...
        EXPECT_TRUE(testing::TestNothrowOp([&]() { nonempty_to.swap(nonempty_from); }));
    }
}
...
TEST(OptionalExceptionSafety, NothrowMoveAssign)
{
    ...
    }
    {
        auto nonempty_from = MoveOptional(kUpdatedInteger);
        auto nonempty_to = MoveOptional(kInitialInteger);
        EXPECT_TRUE(testing::TestNothrowOp([&]() { nonempty_to = std::move(nonempty_from); }));
    }
    {
    ...
    }
    {
        auto thrower = MoveThrower(kUpdatedInteger);
        auto nonempty = MoveOptional(kInitialInteger);
        EXPECT_TRUE(testing::TestNothrowOp([&]() { nonempty = std::move(thrower); }));
    }
}
