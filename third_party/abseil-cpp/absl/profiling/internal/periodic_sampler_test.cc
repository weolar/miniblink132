...
TEST(PeriodicSamplerBaseTest, Sample)
{
    ...
    EXPECT_CALL(sampler, GetExponentialBiased(16)).WillOnce(Return(2)).WillOnce(Return(3)).WillOnce(Return(4));
...
TEST(PeriodicSamplerBaseTest, ImmediatelySample)
{
    ...
    EXPECT_CALL(sampler, GetExponentialBiased(16)).WillOnce(Return(1)).WillOnce(Return(2)).WillOnce(Return(3));
...
TEST(PeriodicSamplerBaseTest, Enable)
{
    ...
    EXPECT_CALL(sampler, period()).Times(2).WillRepeatedly(Return(16));
    EXPECT_CALL(sampler, GetExponentialBiased(16)).Times(2).WillRepeatedly(Return(3));
