...
//
// Thread safe.
template <typename T> class SampleRecorder {
public:
    SampleRecorder();
    ~SampleRecorder();

    // Registers for sampling.  Returns an opaque registration info.
    template <typename... Targs> T* Register(Targs&&... args);
...
private:
    ...
    template <typename... Targs> T* PopDead(Targs... args);
...
template <typename T> typename SampleRecorder<T>::DisposeCallback SampleRecorder<T>::SetDisposeCallback(DisposeCallback f)
{
...
template <typename T> SampleRecorder<T>::~SampleRecorder()
{
...
template <typename T> void SampleRecorder<T>::PushNew(T* sample)
{
...
template <typename T> void SampleRecorder<T>::PushDead(T* sample)
{
...
template <typename T> template <typename... Targs> T* SampleRecorder<T>::PopDead(Targs... args)
{
    absl::MutexLock graveyard_lock(&graveyard_.init_mu);
...
template <typename T> template <typename... Targs> T* SampleRecorder<T>::Register(Targs&&... args)
{
...
template <typename T> void SampleRecorder<T>::Unregister(T* sample)
{
    PushDead(sample);
    size_estimate_.fetch_sub(1, std::memory_order_relaxed);
}

template <typename T> int64_t SampleRecorder<T>::Iterate(const std::function<void(const T& stack)>& f)
{
    T* s = all_.load(std::memory_order_acquire);
    while (s != nullptr) {
...
template <typename T> void SampleRecorder<T>::SetMaxSamples(size_t max)
{
    max_samples_.store(max, std::memory_order_release);
}

template <typename T> size_t SampleRecorder<T>::GetMaxSamples() const
{
