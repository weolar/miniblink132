...
{
    if (z < 2) {
        return exp(-1.2337141 / z) / sqrt(z) * (2.00012 + (0.247105 - (0.0649821 - (0.0347962 - (0.011672 - 0.00168691 * z) * z) * z) * z) * z);
    }
...
double AndersonDarlingStatistic(const std::vector<double>& random_sample)
{
    ...
    for (size_t i = 0; i < n; i++) {
        ad_sum += (2 * i + 1) * std::log(random_sample[i] * (1 - random_sample[n - 1 - i]));
    }
...
TEST(ExponentialBiasedTest, TestNextRandom)
{
    for (auto n : std::vector<size_t>({
        ...
        EXPECT_GT(std::min(ad_pvalue, 1 - ad_pvalue), 0.0001) << "prng is not uniform: n = " << n << " p = " << ad_pvalue;
    }
}
