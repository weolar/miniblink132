...
public:
    void PrepareForSampling(int64_t w)
    {
        weight = w;
    }
...
std::vector<size_t> GetSizes(SampleRecorder<Info>* s)
{
    ...
    s->Iterate([&](const Info& info) { res.push_back(info.size.load(std::memory_order_acquire)); });
    return res;
}
        ...
        pool.Schedule([&sampler, &stop, i]() {
            ...
            while (!stop.HasBeenNotified()) {
                ...
                case 2: {
                    ...
                    sampler.Iterate([&](const Info& info) { oldest = std::max(oldest, absl::Now() - info.create_time); });
