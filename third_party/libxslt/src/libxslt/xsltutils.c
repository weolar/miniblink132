...
const xmlChar* xsltGetCNsProp(xsltStylesheetPtr style, xmlNodePtr node, const xmlChar* name, const xmlChar* nameSpace)
{
	 ...
	 */
        if ((xmlStrEqual(prop->name, name))
            && (((prop->ns == NULL) && (node->ns != NULL) && (xmlStrEqual(node->ns->href, nameSpace)))
    ...
    if (doc != NULL) {
		 ...
		 */
                ...
                if ((ns != NULL) && (xmlStrEqual(ns->href, nameSpace)))
                    return (xmlDictLookup(style->dict, attrDecl->defaultValue, -1));
            }
        }
    }
    return (NULL);
 ...
 */
int xsltPointerListAddSize(xsltPointerListPtr list, void* item, int initialSize)
{
    if (list->items == NULL) {
        if (initialSize <= 0)
            initialSize = 1;
        ...
        if (list->items == NULL) {
            xsltGenericError(xsltGenericErrorContext, "xsltPointerListAddSize: memory allocation failure.\n");
            return (-1);
        }
        list->number = 0;
        ...
        if (list->items == NULL) {
            xsltGenericError(xsltGenericErrorContext, "xsltPointerListAddSize: memory re-allocation failure.\n");
            list->size = 0;
            return (-1);
        }
    }
...
xsltPointerListPtr xsltPointerListCreate(int initialSize)
{
    ...
    if (ret == NULL) {
        xsltGenericError(xsltGenericErrorContext, "xsltPointerListCreate: memory allocation failure.\n");
        return (NULL);
    }
...
void xsltMessage(xsltTransformContextPtr ctxt, xmlNodePtr node, xmlNodePtr inst)
{
    ...
    if (prop != NULL) {
        if (xmlStrEqual(prop, (const xmlChar*)"yes")) {
            ...
            xsltTransformError(ctxt, NULL, inst, "xsl:message : terminate expecting 'yes' or 'no'\n");
        }
        xmlFree(prop);
    }
 ...
 */
static void LIBXSLT_ATTR_FORMAT(2, 3) xsltGenericErrorDefaultFunc(void* ctx ATTRIBUTE_UNUSED, const char* msg, ...)
{
    va_list args;
 ...
 * Default handler for out of context error messages.
 */
static void LIBXSLT_ATTR_FORMAT(2, 3) xsltGenericDebugDefaultFunc(void* ctx ATTRIBUTE_UNUSED, const char* msg, ...)
{
    va_list args;
...
const xmlChar* xsltGetQNameURI(xmlNodePtr node, xmlChar** name)
{
     ...
     */
    if ((qname[0] == 'x') && (qname[1] == 'm') && (qname[2] == 'l') && (qname[3] == ':')) {
        if (qname[4] == 0)
            return (NULL);
...
const xmlChar* xsltGetQNameURI2(xsltStylesheetPtr style, xmlNodePtr node, const xmlChar** name)
{
    ...
    if (ns == NULL) {
        if (style) {
            xsltTransformError(NULL, style, node, "No namespace bound to prefix '%s'.\n", qname);
            style->errors++;
        } else {
        ...
        if (res != NULL) {
            ...
            if (comp->number) {
                if (res->type == XPATH_NUMBER) {
                    ...
                    xsltGenericDebug(xsltGenericDebugContext, "xsltComputeSortResult: select didn't evaluate to a number\n");
#endif
                    results[i] = NULL;
                }
            } else {
...
#ifdef WITH_XSLT_DEBUG_PROCESS
                    xsltGenericDebug(xsltGenericDebugContext, "xsltComputeSortResult: select didn't evaluate to a string\n");
#endif
                    results[i] = NULL;
                }
            }
...
void xsltDefaultSortFunction(xsltTransformContextPtr ctxt, xmlNodePtr* sorts, int nbsorts)
{
    ...
    int tempstype[XSLT_MAX_SORT], temporder[XSLT_MAX_SORT], templang[XSLT_MAX_SORT];

    if ((ctxt == NULL) || (sorts == NULL) || (nbsorts <= 0) || (nbsorts >= XSLT_MAX_SORT))
        return;
    if (sorts[0] == NULL)
        return;
    comp = sorts[0]->psvi;
    ...
    for (j = 0; j < nbsorts; j++) {
        ...
        if ((comp->stype == NULL) && (comp->has_stype != 0)) {
            comp->stype = xsltEvalAttrValueTemplate(ctxt, sorts[j], (const xmlChar*)"data-type", NULL);
            if (comp->stype != NULL) {
                tempstype[j] = 1;
        ...
        if ((comp->order == NULL) && (comp->has_order != 0)) {
            comp->order = xsltEvalAttrValueTemplate(ctxt, sorts[j], (const xmlChar*)"order", NULL);
            if (comp->order != NULL) {
                temporder[j] = 1;
        ...
        if ((comp->lang == NULL) && (comp->has_lang != 0)) {
            xmlChar* lang = xsltEvalAttrValueTemplate(ctxt, sorts[j], (xmlChar*)"lang", NULL);
            if (lang != NULL) {
                templang[j] = 1;
        ...
        for (i = incr; i < len; i++) {
            ...
            while (j >= 0) {
			   ...
			   with XSLT spec */
                        ...
                        tst = xsltLocaleStrcmp(comp->locale, (xsltLocaleChar*)results[j]->stringval, (xsltLocaleChar*)results[j + incr]->stringval);
                    } else {
                        tst = xmlStrcmp(results[j]->stringval, results[j + incr]->stringval);
                    }
                    if (descending)
                        tst = -tst;
                }
			 ...
			 */
                        if (resultsTab[depth] == NULL)
                            resultsTab[depth] = xsltComputeSortResultInternal(ctxt, sorts[depth],
                                /* xfrm */ 1);
				   ...
				   accordance with XSLT spec */
                                ...
                                tst = xsltLocaleStrcmp(comp->locale, (xsltLocaleChar*)res[j]->stringval, (xsltLocaleChar*)res[j + incr]->stringval);
                            } else {
                                tst = xmlStrcmp(res[j]->stringval, res[j + incr]->stringval);
                            }
                            if (desc)
                                tst = -tst;
                        }
...
int xsltSaveResultTo(xmlOutputBufferPtr buf, xmlDocPtr result, xsltStylesheetPtr style)
{
    ...
    if ((buf == NULL) || (result == NULL) || (style == NULL))
        return (-1);
    if ((result->children == NULL) || ((result->children->type == XML_DTD_NODE) && (result->children->next == NULL)))
        return (0);

    if ((style->methodURI != NULL) && ((style->method == NULL) || (!xmlStrEqual(style->method, (const xmlChar*)"xhtml")))) {
    ...
    if ((method != NULL) && (xmlStrEqual(method, (const xmlChar*)"html"))) {
        ...
        if (indent == -1)
            indent = 1;
        htmlDocContentDumpFormatOutput(buf, result, (const char*)encoding, indent);
        ...
        if (omitXmlDecl != 1) {
            ...
            if (encoding == NULL) {
                if (result->encoding != NULL)
                    ...
                    encoding = (const xmlChar*)xmlGetCharEncodingName((xmlCharEncoding)result->charset);
            }
            if (encoding != NULL) {
        ...
        if (result->children != NULL) {
            ...
            while (child != NULL) {
                xmlNodeDumpOutput(buf, result, child, 0, (indent == 1), (const char*)encoding);
                if (indent && ((child->type == XML_DTD_NODE) || ((child->type == XML_COMMENT_NODE) && (child->next != NULL))))
                    xmlOutputBufferWriteString(buf, "\n");
                child = child->next;
            }
            if (indent)
 ...
 */
#if !defined(XSLT_WIN32_PERFORMANCE_COUNTER) && (defined(HAVE_CLOCK_GETTIME) || defined(HAVE_GETTIMEOFDAY))
...
#define XSLT_CLOCK CLOCK_REALTIME
    ...
    tics += (cur.tv_nsec - startup.tv_nsec) / (1000000000l / XSLT_TIMESTAMP_TICS_PER_SEC);
    ...
    tics = (cur.tv_sec - startup.tv_sec) * XSLT_TIMESTAMP_TICS_PER_SEC;
    tics += (cur.tv_usec - startup.tv_usec) / (1000000l / XSLT_TIMESTAMP_TICS_PER_SEC);
...
void xsltSaveProfiling(xsltTransformContextPtr ctxt, FILE* output)
{
    ...
    for (i = 0; i < nb - 1; i++) {
        for (j = i + 1; j < nb; j++) {
            if ((templates[i]->time <= templates[j]->time)
                || ((templates[i]->time == templates[j]->time) && (templates[i]->nbCalls <= templates[j]->nbCalls))) {
                templ1 = templates[j];
    ...
    for (i = 0; i < nb; i++) {
        ...
        fprintf(output, " %6ld %6ld\n", templ1->time, templ1->time / templ1->nbCalls);
    ...
    for (i = 0; i < nb; i++) {
        ...
        for (j = 0; j < templ1->templNr; j++) {
            ...
            snprintf(called_str, sizeof(called_str), "%6d/%d", templ1->templCountTab[j], /* number of times caller calls 'this' */
        ...
        fprintf(output, "%-5s %-6s %-8s %-8s %6d     %s [%d]\n", ix_str, timep_str, times_str, timec_str, templ1->nbCalls,
        ...
        for (k = 0; k < nb; k++) {
            ...
            for (l = 0; l < templ2->templNr; l++) {
                if (templ2->templCalledTab[l] == templ1) {
                    ...
                    snprintf(timec_str, sizeof(timec_str), "%8.3f", (float)childt[k] / XSLT_TIMESTAMP_TICS_PER_SEC);
                    snprintf(called_str, sizeof(called_str), "%6d/%d", templ2->templCountTab[l], /* number of times 'this' calls callee */
    ...
    for (i = 0; i < nb; i++) {
        templ1 = templates[i];
        fprintf(output, "[%d] %s (%s:%d)\n", i, templ1->name ? (char*)templ1->name : pretty_templ_match(templ1), templ1->style->doc->URL, templ1->elem->line);
    }
...
xmlDocPtr xsltGetProfileInformation(xsltTransformContextPtr ctxt)
{
    ...
    nb = 0;
    max = 10000;
    templates = (xsltTemplatePtr*)xmlMalloc(max * sizeof(xsltTemplatePtr));
    if (templates == NULL)
        return NULL;
