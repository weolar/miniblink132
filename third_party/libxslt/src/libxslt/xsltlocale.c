...
static xsltLocale xslt_locale_WINAPI(const xmlChar* languageTag)
{
    int k;
 ...
 * Returns TRUE
 */
static BOOL CALLBACK xsltCountSupportedLocales(LPSTR lcid)
{
    (void)lcid;
    ++xsltLocaleListSize;
    return (TRUE);
 ...
 */
static BOOL CALLBACK xsltIterateSupportedLocales(LPSTR lcid)
{
    static int count = 0;
    ...
    k = GetLocaleInfoA(p->lcid, LOCALE_SISO639LANGNAME, (char*)iso639lang, sizeof(iso639lang));
    if (--k < 1)
        goto end;
    l = GetLocaleInfoA(p->lcid, LOCALE_SISO3166CTRYNAME, (char*)iso3136ctry, sizeof(iso3136ctry));
    if (--l < 1)
        goto end;
