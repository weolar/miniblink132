...
{
    return xmlIsBaseCharQ(val) || xmlIsIdeographicQ(val) || xmlIsDigitQ(val);
}
...
#define IS_SPECIAL(self, letter)                                                                                                                               \
    ((xsltUTF8Charcmp((letter), (self)->zeroDigit) == 0) || (xsltUTF8Charcmp((letter), (self)->digit) == 0)                                                    \
        || (xsltUTF8Charcmp((letter), (self)->decimalPoint) == 0) || (xsltUTF8Charcmp((letter), (self)->grouping) == 0)                                        \
...
static void xsltNumberFormatDecimal(
    xmlBufferPtr buffer, double number, int digit_zero, int width, int digitsPerGroup, int groupingCharacter, int groupingCharacterLen)
{
    /*
     * This used to be
    ...
    *pointer = 0;
    ...
    while (pointer > temp_string) {
        if ((i >= width) && (fabs(number) < 1.0))
            break; /* for */
        if ((i > 0) && (groupingCharacter != 0) && (digitsPerGroup > 0) && ((i % digitsPerGroup) == 0)) {
...
static void xsltNumberFormatAlpha(xsltNumberDataPtr data, xmlBufferPtr buffer, double number, int is_upper)
{
     ...
     */
    if (number < 1.0) {
        xsltNumberFormatDecimal(buffer, number, '0', 1, data->digitsPerGroup, data->groupingCharacter, data->groupingCharacterLen);
        return;
    }
...
static void xsltNumberFormatTokenize(const xmlChar* format, xsltFormatPtr tokens)
{
     ...
     */
    while (!xsltIsLetterDigit(val = xmlStringCurrentChar(NULL, format + ix, &len))) {
    ...
    for (tokens->nTokens = 0; tokens->nTokens < MAX_TOKENS; tokens->nTokens++) {
        if (format[ix] == 0)
            break; /* for */
	 ...
	 */
        ...
        } else if ((val == 'A') || (val == 'a') || (val == 'I') || (val == 'i')) {
...
static void xsltNumberFormatInsertNumbers(xsltNumberDataPtr data, double* numbers, int numbers_max, xsltFormatPtr tokens, xmlBufferPtr buffer)
{
    ...
    for (i = 0; i < numbers_max; i++) {
         ...
         */
        if (number < 0.0) {
            xsltTransformError(NULL, NULL, NULL, "xsl-number : negative value\n");
                ...
                default:
                    if (IS_DIGIT_ZERO(token->token)) {
                        xsltNumberFormatDecimal(
                            buffer, number, token->token, token->width, data->digitsPerGroup, data->groupingCharacter, data->groupingCharacterLen);
                    }
                    break;
                }
            }
...
static int xsltNumberFormatGetAnyLevel(xsltTransformContextPtr context, xmlNodePtr node, xsltCompMatchPtr countPat, xsltCompMatchPtr fromPat, double* array)
{
    ...
    while (cur != NULL) {
        ...
        if (xsltTestCompMatchCount(context, cur, countPat, node))
            cnt++;
        if ((fromPat != NULL) && xsltTestCompMatchList(context, cur, fromPat)) {
            break; /* while */
        }
        ...
        if (cur->type == XML_NAMESPACE_DECL) {
            ...
            */
            ...
            while (
                (cur->prev != NULL) && ((cur->prev->type == XML_DTD_NODE) || (cur->prev->type == XML_XINCLUDE_START) || (cur->prev->type == XML_XINCLUDE_END)))
                cur = cur->prev;
            if (cur->prev != NULL) {
...
static int xsltNumberFormatGetMultipleLevel(
    xsltTransformContextPtr context, xmlNodePtr node, xsltCompMatchPtr countPat, xsltCompMatchPtr fromPat, double* array, int max)
{
    int amount = 0;
    int cnt;
    ...
    if (parser) {
        ...
        while ((ancestor != NULL) && (ancestor->type != XML_DOCUMENT_NODE)) {
            if ((fromPat != NULL) && xsltTestCompMatchList(context, ancestor, fromPat))
                break; /* for */
            ...
            if (xsltTestCompMatchCount(context, ancestor, countPat, node)) {
                ...
                while (preceding != NULL) {
                    if (xsltTestCompMatchCount(context, preceding, countPat, node))
                        cnt++;
                    context->xpathCtxt->node = ancestor;
...
static int xsltNumberFormatGetValue(xmlXPathContextPtr context, xmlNodePtr node, const xmlChar* value, double* number)
{
    ...
    if (pattern != NULL) {
        ...
        context->node = node;
        obj = xmlXPathEvalExpression(xmlBufferContent(pattern), context);
        if (obj != NULL) {
            *number = obj->floatval;
            amount++;
...
void xsltNumberFormat(xsltTransformContextPtr ctxt, xsltNumberDataPtr data, xmlNodePtr node)
{
    ...
    if (data->format != NULL) {
        ...
        if (data->has_format == 0)
            return;
        format = xsltEvalAttrValueTemplate(ctxt, data->node, (const xmlChar*)"format", XSLT_NAMESPACE);
        if (format == NULL)
            return;
     ...
     */
    if (data->value) {
        amount = xsltNumberFormatGetValue(ctxt->xpathCtxt, node, data->value, &number);
        if (amount == 1) {
            xsltNumberFormatInsertNumbers(data, &number, 1, &tokens, output);
        }

    } else if (data->level) {

        if (xmlStrEqual(data->level, (const xmlChar*)"single")) {
            amount = xsltNumberFormatGetMultipleLevel(ctxt, node, data->countPat, data->fromPat, &number, 1);
            if (amount == 1) {
                xsltNumberFormatInsertNumbers(data, &number, 1, &tokens, output);
            }
            ...
            amount = xsltNumberFormatGetMultipleLevel(ctxt, node, data->countPat, data->fromPat, numarray, max);
            if (amount > 0) {
                xsltNumberFormatInsertNumbers(data, numarray, amount, &tokens, output);
            }
        } else if (xmlStrEqual(data->level, (const xmlChar*)"any")) {
            amount = xsltNumberFormatGetAnyLevel(ctxt, node, data->countPat, data->fromPat, &number);
            if (amount > 0) {
                xsltNumberFormatInsertNumbers(data, &number, 1, &tokens, output);
            }
        }
...
xmlXPathError xsltFormatNumberConversion(xsltDecimalFormatPtr self, xmlChar* format, double number, xmlChar** result)
{
    ...
    *result = NULL;
    ...
    while ((*the_format != 0) && (xsltUTF8Charcmp(the_format, self->decimalPoint) != 0) && (xsltUTF8Charcmp(the_format, self->patternSeparator) != 0)) {
        ...
        if (xsltUTF8Charcmp(the_format, self->digit) == 0) {
            ...
            if (format_info.group >= 0)
                format_info.group++;
        } else if ((self_grouping_len > 0) && (!xmlStrncmp(the_format, self->grouping, self_grouping_len))) {
            /* Reset group count */
            format_info.group = 0;
    ...
    if ((*the_format != 0) && (xsltUTF8Charcmp(the_format, self->decimalPoint) == 0)) {
    ...
    if ((suffix_length < 0) || ((*the_format != 0) && (xsltUTF8Charcmp(the_format, self->patternSeparator) != 0))) {
        found_error = 1;
        goto OUTPUT_NUMBER;
    }
	 ...
	 */
	     ...
	     */
            ...
            while (*the_format != 0) {
                if ((xsltUTF8Charcmp(the_format, (self)->percent) == 0) || (xsltUTF8Charcmp(the_format, (self)->permille) == 0)) {
                    if (format_info.is_multiplier_set) {
                        found_error = 1;
                        goto OUTPUT_NUMBER;
                    }
...
OUTPUT_NUMBER:
    ...
    if ((self->grouping != NULL) && (self->grouping[0] != 0)) {
        ...
        xsltNumberFormatDecimal(buffer, floor(number), self->zeroDigit[0], format_info.integer_digits, format_info.group, gchar, len);
    } else
        xsltNumberFormatDecimal(buffer, floor(number), self->zeroDigit[0], format_info.integer_digits, format_info.group, ',', 1);

    /* Special case: java treats '.#' like '.0', '.##' like '.0#', etc. */
    if ((format_info.integer_digits + format_info.integer_hash + format_info.frac_digits == 0) && (format_info.frac_hash > 0)) {
        ++format_info.frac_digits;
        --format_info.frac_hash;
    }

    /* Add leading zero, if required */
    if ((floor(number) == 0) && (format_info.integer_digits + format_info.frac_digits == 0)) {
    ...
    if (format_info.frac_digits + format_info.frac_hash == 0) {
        if (format_info.add_decimal)
            xmlBufferAdd(buffer, self->decimalPoint, xmlUTF8Strsize(self->decimalPoint, 1));
    } else {
        number -= floor(number);
        if ((number != 0) || (format_info.frac_digits != 0)) {
            ...
            xsltNumberFormatDecimal(buffer, floor(number), self->zeroDigit[0], format_info.frac_digits + j, 0, 0, 0);
        }
    }
