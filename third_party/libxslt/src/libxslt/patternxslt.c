...
typedef enum { AXIS_CHILD = 1, AXIS_ATTRIBUTE } xsltAxis;
...
static xsltCompMatchPtr xsltNewCompMatch(void)
{
    ...
    if (cur == NULL) {
        xsltTransformError(NULL, NULL, NULL, "xsltNewCompMatch : out of memory error\n");
        return (NULL);
    }
    memset(cur, 0, sizeof(xsltCompMatch));
    cur->maxStep = 10;
    cur->nbStep = 0;
    cur->steps = (xsltStepOpPtr)xmlMalloc(sizeof(xsltStepOp) * cur->maxStep);
    if (cur->steps == NULL) {
        xsltTransformError(NULL, NULL, NULL, "xsltNewCompMatch : out of memory error\n");
        xmlFree(cur);
        return (NULL);
...
static xsltParserContextPtr xsltNewParserContext(xsltStylesheetPtr style, xsltTransformContextPtr ctxt)
{
    ...
    if (cur == NULL) {
        xsltTransformError(NULL, NULL, NULL, "xsltNewParserContext : malloc failed\n");
        return (NULL);
    }
...
static int xsltCompMatchAdd(xsltParserContextPtr ctxt, xsltCompMatchPtr comp, xsltOp op, xmlChar* value, xmlChar* value2, int novar)
{
    if (comp->nbStep >= comp->maxStep) {
        ...
        tmp = (xsltStepOpPtr)xmlRealloc(comp->steps, comp->maxStep * 2 * sizeof(xsltStepOp));
        if (tmp == NULL) {
    ...
    if (ctxt->ctxt != NULL) {
        comp->steps[comp->nbStep].previousExtra = xsltAllocateExtraCtxt(ctxt->ctxt);
        comp->steps[comp->nbStep].indexExtra = xsltAllocateExtraCtxt(ctxt->ctxt);
        comp->steps[comp->nbStep].lenExtra = xsltAllocateExtraCtxt(ctxt->ctxt);
    } else {
        comp->steps[comp->nbStep].previousExtra = xsltAllocateExtra(ctxt->style);
        comp->steps[comp->nbStep].indexExtra = xsltAllocateExtra(ctxt->style);
        comp->steps[comp->nbStep].lenExtra = xsltAllocateExtra(ctxt->style);
    }
...
#endif
        comp->steps[comp->nbStep].comp = xsltXPathCompileFlags(ctxt->style, value, flags);
        if (comp->steps[comp->nbStep].comp == NULL) {
            xsltTransformError(NULL, ctxt->style, ctxt->elem, "Failed to compile predicate\n");
            if (ctxt->style != NULL)
                ctxt->style->errors++;
        }
    }
...
static void xsltReverseCompMatch(xsltParserContextPtr ctxt, xsltCompMatchPtr comp)
{
    ...
    for (i = 0; i < comp->nbStep - 1; i++) {
        if ((comp->steps[i].op == XSLT_OP_PREDICATE) && (comp->steps[i + 1].op == XSLT_OP_PREDICATE)) {
...
static int xsltPatPushState(xsltTransformContextPtr ctxt, xsltStepStates* states, int step, xmlNodePtr node)
{
    if ((states->states == NULL) || (states->maxstates <= 0)) {
        ...
        tmp = (xsltStepStatePtr)xmlRealloc(states->states, 2 * states->maxstates * sizeof(xsltStepState));
        if (tmp == NULL) {
...
static int xsltTestCompMatchDirect(xsltTransformContextPtr ctxt, xsltCompMatchPtr comp, xmlNodePtr node, xmlNsPtr* nsList, int nsNr)
{
    ...
    prevdoc = (xmlDocPtr)XSLT_RUNTIME_EXTRA(ctxt, sel->previousExtra, ptr);
    ...
    if ((list == NULL) || (prevdoc != doc)) {
        ...
        if (nocache == 0) {
            ...
            XSLT_RUNTIME_EXTRA_LST(ctxt, sel->lenExtra) = (void*)list;
            XSLT_RUNTIME_EXTRA(ctxt, sel->previousExtra, ptr) = (void*)doc;
            XSLT_RUNTIME_EXTRA(ctxt, sel->indexExtra, ival) = 0;
            XSLT_RUNTIME_EXTRA_FREE(ctxt, sel->lenExtra) = xmlXPathFreeObjectWrapper;
        } else
            list = newlist;
    }
    if ((list->nodesetval == NULL) || (list->nodesetval->nodeNr <= 0)) {
        if (nocache == 1)
            xmlXPathFreeObject(list);
        return (0);
    }
...
static int xsltTestStepMatch(xsltTransformContextPtr ctxt, xmlNodePtr node, xsltStepOpPtr step)
{
    ...
    case XSLT_OP_KEY: {
        ...
        list = xsltGetKey(ctxt, step->value, step->value3, step->value2);
        if (list == NULL)
            return (0);
    ...
    case XSLT_OP_NODE:
        ...
        break;
    default:
        xsltTransformError(ctxt, NULL, node, "xsltTestStepMatch: unexpected step op %d\n", step->op);
        return (-1);
    }
...
static int xsltTestPredicateMatch(xsltTransformContextPtr ctxt, xsltCompMatchPtr comp, xmlNodePtr node, xsltStepOpPtr step, xsltStepOpPtr sel)
{
    ...
    oldCP = ctxt->xpathCtxt->proximityPosition;
    {
        ...
        previous = (xmlNodePtr)XSLT_RUNTIME_EXTRA(ctxt, sel->previousExtra, ptr);
        if ((previous != NULL) && (previous->parent == node->parent)) {
            /*
            ...
            if ((parent == NULL) || (node->doc == NULL))
                ...
                if (((parent->type != XML_DOCUMENT_NODE) && (parent->type != XML_HTML_DOCUMENT_NODE)) || (parent != (xmlNodePtr)node->doc))
                    nocache = 1;
            }
        }
        if (pos != 0) {
             ...
             * cache it !
             */
            if ((!isRVT) && (node->doc != NULL) && (nocache == 0)) {
                XSLT_RUNTIME_EXTRA(ctxt, sel->previousExtra, ptr) = node;
                XSLT_RUNTIME_EXTRA(ctxt, sel->indexExtra, ival) = pos;
...
static int xsltTestCompMatch(xsltTransformContextPtr ctxt, xsltCompMatchPtr comp, xmlNodePtr matchNode, const xmlChar* mode, const xmlChar* modeURI)
{
    ...
    if ((comp == NULL) || (node == NULL) || (ctxt == NULL)) {
        xsltTransformError(ctxt, NULL, node, "xsltTestCompMatch: null arg\n");
        return (-1);
    }
    if (mode != NULL) {
        if (comp->mode == NULL)
    ...
    for (; i < comp->nbStep; i++) {
        ...
        case XSLT_OP_ANCESTOR:
            ...
            if (step->value == NULL) {
                ...
                if ((step->op != XSLT_OP_ELEM) && (step->op != XSLT_OP_ALL) && (step->op != XSLT_OP_NS) && (step->op != XSLT_OP_ID)
                    && (step->op != XSLT_OP_KEY))
                    goto rollback;
            }
            if (node == NULL)
		 ...
		 */
            if (comp->direct) {
                found = xsltTestCompMatchDirect(ctxt, comp, matchNode, comp->nsList, comp->nsNr);
                goto exit;
            }
...
static xmlChar* xsltScanNCName(xsltParserContextPtr ctxt)
{
    ...
    while (xmlIsBaseCharQ(val) || xmlIsIdeographicQ(val) || xmlIsDigitQ(val) || (val == '.') || (val == '-') || (val == '_') || xmlIsCombiningQ(val)
        || xmlIsExtenderQ(val)) {
        cur += len;
        val = xmlStringCurrentChar(NULL, cur, &len);
    }
...
static void xsltCompileIdKeyPattern(xsltParserContextPtr ctxt, xmlChar* name, int aid, int novar, xsltAxis axis)
{
    ...
    if (CUR != '(') {
        xsltTransformError(NULL, NULL, NULL, "xsltCompileIdKeyPattern : ( expected\n");
        ctxt->error = 1;
        return;
    }
    if ((aid) && (xmlStrEqual(name, (const xmlChar*)"id"))) {
        if (axis != 0) {
            xsltTransformError(NULL, NULL, NULL, "xsltCompileIdKeyPattern : NodeTest expected\n");
            ctxt->error = 1;
            return;
        }
        NEXT;
        ...
        if (ctxt->error) {
            xsltTransformError(NULL, NULL, NULL, "xsltCompileIdKeyPattern : Literal expected\n");
            xmlFree(lit);
            return;
        }
        SKIP_BLANKS;
        ...
        xsltTransformError(NULL, NULL, NULL, "xsltCompileIdKeyPattern : expecting 'key' or 'id' or node type\n");
        ctxt->error = 1;
        return;
    } else {
        xsltTransformError(NULL, NULL, NULL, "xsltCompileIdKeyPattern : node type\n");
        ctxt->error = 1;
        return;
    }
error:
...
parse_node_test:
    ...
    if (token == NULL) {
        if (CUR == '*') {
            ...
            xsltTransformError(NULL, NULL, NULL, "xsltCompileStepPattern : Name expected\n");
            ctxt->error = 1;
            goto error;
        }
    }
    ...
    if (CUR == '(') {
	     ...
	     */
            ...
            if (ns == NULL) {
                xsltTransformError(NULL, NULL, NULL, "xsltCompileStepPattern : no namespace bound to prefix %s\n", prefix);
                xmlFree(prefix);
                prefix = NULL;
                ctxt->error = 1;
                goto error;
            ...
            if (axis != 0) {
                xsltTransformError(NULL, NULL, NULL, "xsltCompileStepPattern : NodeTest expected\n");
                ctxt->error = 1;
                goto error;
            }
            NEXT;
            if (xmlStrEqual(token, (const xmlChar*)"child")) {
                ...
                xsltTransformError(NULL, NULL, NULL, "xsltCompileStepPattern : 'child' or 'attribute' expected\n");
                ctxt->error = 1;
                goto error;
            }
            xmlFree(token);
...
parse_predicate:
    ...
    while (CUR == '[') {
        ...
        if (CUR == 0) {
            xsltTransformError(NULL, NULL, NULL, "xsltCompileStepPattern : ']' expected\n");
            ctxt->error = 1;
            return;
        }
...
static void xsltCompileLocationPathPattern(xsltParserContextPtr ctxt, int novar)
{
	 ...
	 */
        ...
        if (name == NULL) {
            xsltTransformError(NULL, NULL, NULL, "xsltCompileLocationPathPattern : Name expected\n");
            ctxt->error = 1;
            return;
        }
        SKIP_BLANKS;
...
static xsltCompMatchPtr xsltCompilePatternInternal(
    const xmlChar* pattern, xmlDocPtr doc, xmlNodePtr node, xsltStylesheetPtr style, xsltTransformContextPtr runtime, int novar)
{
	 ...
	 */
        ...
        if (ctxt->error) {
            xsltTransformError(NULL, style, node, "xsltCompilePattern : failed to compile '%s'\n", element->pattern);
            if (style != NULL)
                style->errors++;
            goto error;
        }
	 ...
	 */
        if (element->priority == 0) { /* if not yet determined */
            if (((element->steps[0].op == XSLT_OP_ELEM) || (element->steps[0].op == XSLT_OP_ATTR) || (element->steps[0].op == XSLT_OP_PI))
                && (element->steps[0].value != NULL) && (element->steps[1].op == XSLT_OP_END)) {
                ; /* previously preset */
            } else if ((element->steps[0].op == XSLT_OP_ATTR) && (element->steps[0].value2 != NULL) && (element->steps[1].op == XSLT_OP_END)) {
                element->priority = -0.25;
            } else if ((element->steps[0].op == XSLT_OP_NS) && (element->steps[0].value != NULL) && (element->steps[1].op == XSLT_OP_END)) {
                element->priority = -0.25;
            } else if ((element->steps[0].op == XSLT_OP_ATTR) && (element->steps[0].value == NULL) && (element->steps[0].value2 == NULL)
                && (element->steps[1].op == XSLT_OP_END)) {
                element->priority = -0.5;
            } else if (((element->steps[0].op == XSLT_OP_PI) || (element->steps[0].op == XSLT_OP_TEXT) || (element->steps[0].op == XSLT_OP_ALL)
                           || (element->steps[0].op == XSLT_OP_NODE) || (element->steps[0].op == XSLT_OP_COMMENT))
                && (element->steps[1].op == XSLT_OP_END)) {
                element->priority = -0.5;
            } else {
                element->priority = 0.5;
    ...
    if (end == 0) {
        xsltTransformError(NULL, style, node, "xsltCompilePattern : NULL pattern\n");
        if (style != NULL)
            style->errors++;
        goto error;
    }
...
int xsltAddTemplate(xsltStylesheetPtr style, xsltTemplatePtr cur, const xmlChar* mode, const xmlChar* modeURI)
{
    ...
    if (cur->name != NULL) {
        if (style->namedTemplates == NULL) {
            ...
            void* dup = xmlHashLookup2(style->namedTemplates, cur->name, cur->nameURI);
            if (dup != NULL) {
                xsltTransformError(NULL, style, cur->elem, "xsl:template: error duplicate name '%s'\n", cur->name);
                style->errors++;
                return (-1);
            }
        }
    ...
    if (cur->match == NULL) {
        if (cur->name == NULL) {
            xsltTransformError(NULL, style, cur->elem, "xsl:template: need to specify match or name attribute\n");
            style->errors++;
            return (-1);
        }
        return (0);
	 ...
	 */
        ...
        case XSLT_OP_END:
        case XSLT_OP_PREDICATE:
            xsltTransformError(NULL, style, NULL, "xsltAddTemplate: invalid compiled pattern\n");
...
#ifdef WITH_XSLT_DEBUG_PATTERN
        if (mode)
            xsltGenericDebug(xsltGenericDebugContext, "added pattern : '%s' mode '%s' priority %f\n", pat->pattern, pat->mode, pat->priority);
        else
            xsltGenericDebug(xsltGenericDebugContext, "added pattern : '%s' priority %f\n", pat->pattern, pat->priority);
#endif
...
xsltTemplatePtr xsltGetTemplate(xsltTransformContextPtr ctxt, xmlNodePtr node, xsltStylesheetPtr style)
{
    ...
    while ((curstyle != NULL) && (curstyle != style)) {
	     ...
	     */
            ...
            list = NULL;
        while (list != NULL) {
            if (xsltTestCompMatch(ctxt, list, node, ctxt->mode, ctxt->modeURI) == 1) {
                ret = list->template;
	 ...
	 */
        ...
        while (
            (list != NULL) && ((ret == NULL) || (list->priority > priority) || ((list->priority == priority) && (list->template->position > ret->position)))) {
            if (xsltTestCompMatch(ctxt, list, node, ctxt->mode, ctxt->modeURI) == 1) {
                ret = list->template;
                priority = list->priority;
                break;
	 ...
	 */
        if ((node->type == XML_DOCUMENT_NODE) || (node->type == XML_HTML_DOCUMENT_NODE) || (node->type == XML_TEXT_NODE)) {
            list = curstyle->elemMatch;
            while ((list != NULL)
                && ((ret == NULL) || (list->priority > priority) || ((list->priority == priority) && (list->template->position > ret->position)))) {
                if (xsltTestCompMatch(ctxt, list, node, ctxt->mode, ctxt->modeURI) == 1) {
                    ret = list->template;
                    priority = list->priority;
                    break;
        ...
        } else if ((node->type == XML_PI_NODE) || (node->type == XML_COMMENT_NODE)) {
            list = curstyle->elemMatch;
            while ((list != NULL)
                && ((ret == NULL) || (list->priority > priority) || ((list->priority == priority) && (list->template->position > ret->position)))) {
                if (xsltTestCompMatch(ctxt, list, node, ctxt->mode, ctxt->modeURI) == 1) {
    ...
    keyed_match:
        ...
        } else if (ctxt->hasTemplKeyPatterns && ((ctxt->document == NULL) || (ctxt->document->nbKeysComputed < ctxt->nbKeys))) {
            /*
