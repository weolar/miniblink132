 ...
 */
#define IS_XSLT_ELEM(n) (((n) != NULL) && ((n)->type == XML_ELEMENT_NODE) && ((n)->ns != NULL) && (xmlStrEqual((n)->ns->href, XSLT_NAMESPACE)))
...
#define IS_XSLT_REAL_NODE(n)                                                                                                                                   \
    (((n) != NULL)                                                                                                                                             \
        && (((n)->type == XML_ELEMENT_NODE) || ((n)->type == XML_TEXT_NODE) || ((n)->type == XML_CDATA_SECTION_NODE) || ((n)->type == XML_ATTRIBUTE_NODE)      \
            || ((n)->type == XML_DOCUMENT_NODE) || ((n)->type == XML_HTML_DOCUMENT_NODE) || ((n)->type == XML_COMMENT_NODE) || ((n)->type == XML_PI_NODE)))
...
XSLTPUBFUN void XSLTCALL xsltTransformError(xsltTransformContextPtr ctxt, xsltStylesheetPtr style, xmlNodePtr node, const char* msg, ...)
    LIBXSLT_ATTR_FORMAT(4, 5);

XSLTPUBFUN int XSLTCALL xsltSetCtxtParseOptions(xsltTransformContextPtr ctxt, int options);
/*
 * Sorting.
 */
