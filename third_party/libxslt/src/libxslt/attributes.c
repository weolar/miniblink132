...
static void xsltMergeAttrSets(xsltAttrSetPtr set, xsltAttrSetPtr other)
{
	 ...
	 */
        ...
        while (cur != NULL) {
            ...
            if ((curComp->name == oldComp->name) && (curComp->ns == oldComp->ns)) {
                add = 0;
                break;
            }
            if (cur->next == NULL)
...
void xsltParseStylesheetAttributeSet(xsltStylesheetPtr style, xmlNodePtr cur)
{
    ...
    if (xmlValidateQName(value, 0)) {
        xsltTransformError(NULL, style, cur, "xsl:attribute-set : The name '%s' is not a valid QName.\n", value);
        style->errors++;
        xmlFree(value);
        return;
    }
    ...
    if (prefix != NULL) {
        ...
        if (ns == NULL) {
            xsltTransformError(NULL, style, cur, "xsl:attribute-set : No namespace found for QName '%s:%s'\n", prefix, ncname);
            style->errors++;
            return;
        }
        nsUri = ns->href;
	...
	*/
        if ((child->type != XML_ELEMENT_NODE) || (child->ns == NULL) || (!IS_XSLT_ELEM(child))) {
            if (child->type == XML_ELEMENT_NODE)
                xsltTransformError(NULL, style, child, "xsl:attribute-set : unexpected child %s\n", child->name);
            else
                xsltTransformError(NULL, style, child, "xsl:attribute-set : child of unexpected type\n");
        } else if (!IS_XSLT_NAME(child, "attribute")) {
            xsltTransformError(NULL, style, child, "xsl:attribute-set : unexpected child xsl:%s\n", child->name);
        } else {
#ifdef WITH_XSLT_DEBUG_ATTRIBUTES
            xsltGenericDebug(xsltGenericDebugContext, "add attribute to list %s\n", ncname);
#endif
            xsltStylePreCompute(style, child);
            if (child->children != NULL) {
#ifdef XSLT_REFACTORED
                xsltParseSequenceConstructor(XSLT_CCTXT(style), child->children);
#else
    ...
    */
    ...
    if (value != NULL) {
        ...
        curval = value;
        while (*curval != 0) {
            while (IS_BLANK(*curval))
                curval++;
            if (*curval == 0)
                break;
            endval = curval;
            while ((*endval != 0) && (!IS_BLANK(*endval)))
                endval++;
            ...
            if (curval) {
                ...
                set->useAttrSets = xsltAddUseAttrSetList(set->useAttrSets, ncname2, nsUri2);
            }
            curval = endval;
        }
        xmlFree(value);
    ...
    while (use != NULL) {
        ...
        while (cur != NULL) {
            if (cur->attributeSets) {
                other = xmlHashLookup2(cur->attributeSets, use->ncname, use->ns);
                if (other != NULL) {
                    xsltResolveAttrSet(other, topStyle, cur, use->ncname, use->ns, depth + 1);
...
void xsltResolveStylesheetAttributeSet(xsltStylesheetPtr style)
{
    ...
    while (cur != NULL) {
        if (cur->attributeSets != NULL) {
            ...
            asctx.style = cur;
            xmlHashScanFull(cur->attributeSets, xsltResolveSASCallback, &asctx);
...
{
#ifdef XSLT_REFACTORED
    xsltStyleItemAttributePtr comp = (xsltStyleItemAttributePtr)castedComp;
#else
    ...
    if ((ctxt == NULL) || (contextNode == NULL) || (inst == NULL) || (inst->type != XML_ELEMENT_NODE))
        return;
	...
	*/
        if (xmlStrEqual(prop, BAD_CAST "xmlns")) {
            xsltTransformError(ctxt, NULL, inst, "xsl:attribute: The effective name 'xmlns' is not allowed.\n");
            xmlFree(prop);
            goto error;
        }
	...
	*/
    ...
    } else if ((inst->children->next == NULL) && ((inst->children->type == XML_TEXT_NODE) || (inst->children->type == XML_CDATA_SECTION_NODE))) {
        xmlNodePtr copyTxt;
	...
	*/
         ...
         */
        if ((copyTxt->content != NULL) && (xmlIsID(attr->doc, attr->parent, attr)))
    ...
    while (*curstr != 0) {
...
#ifdef WITH_DEBUGGER
            if ((style != NULL) && (style->attributeSets != NULL) && (ctxt->debugStatus != XSLT_DEBUG_NONE)) {
                set = xmlHashLookup2(style->attributeSets, ncname, nsUri);
                if ((set != NULL) && (set->attrs != NULL) && (set->attrs->attr != NULL))
                    xslHandleDebugger(set->attrs->attr->parent, node, NULL, ctxt);
            }
#endif
            /*
	    ...
	    */
            ...
            if (set != NULL) {
                ...
                while (cur != NULL) {
                    if (cur->attr != NULL) {
                        xsltAttribute(ctxt, node, cur->attr, cur->attr->psvi);
                    }
                    cur = cur->next;
                }
            }
...
{
    if (style->attributeSets != NULL)
        xmlHashFree((xmlHashTablePtr)style->attributeSets, xsltFreeAttributeSetsEntry);
