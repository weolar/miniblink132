...
#undef IS_BLANK
#endif
#define IS_BLANK(c) (((c) == 0x20) || ((c) == 0x09) || ((c) == 0xA) || ((c) == 0x0D))
...
static void xsltParseContentError(xsltStylesheetPtr style, xmlNodePtr node)
{
    ...
    if (IS_XSLT_ELEM(node))
        xsltTransformError(NULL, style, node, "The XSLT-element '%s' is not allowed at this position.\n", node->name);
    else
        xsltTransformError(NULL, style, node, "The element '%s' is not allowed at this position.\n", node->name);
    style->errors++;
}
...
static int exclPrefixPush(xsltStylesheetPtr style, xmlChar* value)
{
    ...
    if (style->exclPrefixMax == 0) {
        ...
        style->exclPrefixTab = (xmlChar**)xmlMalloc(style->exclPrefixMax * sizeof(style->exclPrefixTab[0]));
    ...
    if (style->exclPrefixNr >= style->exclPrefixMax) {
        ...
        style->exclPrefixTab = (xmlChar**)xmlRealloc(style->exclPrefixTab, style->exclPrefixMax * sizeof(style->exclPrefixTab[0]));
        if (style->exclPrefixTab == NULL) {
...
xsltDecimalFormatPtr xsltDecimalFormatGetByName(xsltStylesheetPtr style, xmlChar* name)
{
    ...
    while (style != NULL) {
        for (result = style->decimalFormat->next; result != NULL; result = result->next) {
...
xsltDecimalFormatPtr xsltDecimalFormatGetByQName(xsltStylesheetPtr style, const xmlChar* nsUri, const xmlChar* name)
{
    ...
    while (style != NULL) {
        for (result = style->decimalFormat->next; result != NULL; result = result->next) {
            if (xmlStrEqual(nsUri, result->nsUri) && xmlStrEqual(name, result->name))
                return result;
        }
        style = xsltNextImport(style);
    }
...
static xsltTemplatePtr xsltNewTemplate(void)
{
    ...
    if (cur == NULL) {
        xsltTransformError(NULL, NULL, NULL, "xsltNewTemplate : malloc failed\n");
        return (NULL);
    }
 ...
 */
    if (template->inheritedNs)
        xmlFree(template->inheritedNs);

    /* free profiling data */
    if (template->templCalledTab)
        xmlFree(template->templCalledTab);
    if (template->templCountTab)
...
static void xsltFreeNsAliasList(xsltNsAliasPtr item)
{
    xsltNsAliasPtr tmp;
...
static void xsltFreeNamespaceMap(xsltNsMapPtr item)
{
    xsltNsMapPtr tmp;
...
static xsltNsMapPtr xsltNewNamespaceMapItem(xsltCompilerCtxtPtr cctxt, xmlDocPtr doc, xmlNsPtr ns, xmlNodePtr elem)
{
    xsltNsMapPtr ret;
 ...
 */
static void xsltCompilerVarInfoFree(xsltCompilerCtxtPtr cctxt)
{
 ...
 */
static void xsltCompilationCtxtFree(xsltCompilerCtxtPtr cctxt)
{
    if (cctxt == NULL)
        return;
#ifdef WITH_XSLT_DEBUG_PARSING
    xsltGenericDebug(xsltGenericDebugContext, "Freeing compilation context\n");
    xsltGenericDebug(xsltGenericDebugContext, "### Max inodes: %d\n", cctxt->maxNodeInfos);
    xsltGenericDebug(xsltGenericDebugContext, "### Max LREs  : %d\n", cctxt->maxLREs);
#endif
    /*
    * Free node-infos.
    */
 ...
 */
static xsltCompilerCtxtPtr xsltCompilationCtxtCreate(xsltStylesheetPtr style)
{
...
static void xsltLREEffectiveNsNodesFree(xsltEffectiveNsPtr first)
{
...
static void xsltFreePrincipalStylesheetData(xsltPrincipalStylesheetDataPtr data)
{
    if (data == NULL)
        return;
    ...
    if (data->inScopeNamespaces != NULL) {
        ...
        xsltPointerListPtr list = (xsltPointerListPtr)data->inScopeNamespaces;
    ...
    if (data->exclResultNamespaces != NULL) {
        int i;
        xsltPointerListPtr list = (xsltPointerListPtr)data->exclResultNamespaces;
    ...
    if (data->extElemNamespaces != NULL) {
        xsltPointerListPtr list = (xsltPointerListPtr)data->extElemNamespaces;
        int i;
...
static xsltPrincipalStylesheetDataPtr xsltNewPrincipalStylesheetData(void)
{
    xsltPrincipalStylesheetDataPtr ret;

    ret = (xsltPrincipalStylesheetDataPtr)xmlMalloc(sizeof(xsltPrincipalStylesheetData));
    if (ret == NULL) {
        xsltTransformError(NULL, NULL, NULL, "xsltNewPrincipalStylesheetData: memory allocation failed.\n");
        return (NULL);
    }
...
static xsltStylesheetPtr xsltNewStylesheetInternal(xsltStylesheetPtr parent)
{
    ...
    if (ret == NULL) {
        xsltTransformError(NULL, NULL, NULL, "xsltNewStylesheet : malloc failed\n");
        goto internal_err;
    }
    ...
    if (parent == NULL) {
        ...
        if (ret->xpathCtxt == NULL) {
            xsltTransformError(NULL, NULL, NULL, "xsltNewStylesheet: xmlXPathNewContext failed\n");
            goto internal_err;
        }
...
int xsltAllocateExtraCtxt(xsltTransformContextPtr ctxt)
{
    if (ctxt->extrasNr >= ctxt->extrasMax) {
        ...
        if (ctxt->extrasNr == 0) {
            ctxt->extrasMax = 20;
            ctxt->extras = (xsltRuntimeExtraPtr)xmlMalloc(ctxt->extrasMax * sizeof(xsltRuntimeExtra));
            if (ctxt->extras == NULL) {
                xsltTransformError(ctxt, NULL, NULL, "xsltAllocateExtraCtxt: out of memory\n");
                return (0);
            }
            ...
            ctxt->extrasMax += 100;
            tmp = (xsltRuntimeExtraPtr)xmlRealloc(ctxt->extras, ctxt->extrasMax * sizeof(xsltRuntimeExtra));
            if (tmp == NULL) {
                xsltTransformError(ctxt, NULL, NULL, "xsltAllocateExtraCtxt: out of memory\n");
                return (0);
            }
...
void xsltFreeStylesheet(xsltStylesheetPtr style)
{
    ...
    */
    if ((style->principal == style) && (style->doc))
        xsltCleanupStylesheetTree(style->doc, xmlDocGetRootElement(style->doc));
#ifdef XSLT_REFACTORED_XSLT_NSCOMP
    /*
    * Restore changed ns-decls before freeing the document.
    */
    if ((style->doc != NULL) && XSLT_HAS_INTERNAL_NSMAP(style)) {
        xsltRestoreDocumentNamespaces(XSLT_GET_INTERNAL_NSMAP(style), style->doc);
    }
#endif /* XSLT_REFACTORED_XSLT_NSCOMP */
#else
    /*
...
static int xsltGetInheritedNsList(xsltStylesheetPtr style, xsltTemplatePtr template, xmlNodePtr node)
{
    ...
    if ((style == NULL) || (template == NULL) || (node == NULL) || (template->inheritedNsNr != 0) || (template->inheritedNs != NULL))
        return (0);
    while (node != NULL) {
        if (node->type == XML_ELEMENT_NODE) {
            ...
            while (cur != NULL) {
                ...
                if ((cur->prefix != NULL) && (xsltCheckExtPrefix(style, cur->prefix)))
                    goto skip_ns;
                /*
		...
		*/
                ...
                if (ret == NULL) {
                    ...
                    if (ret == NULL) {
                        xmlGenericError(xmlGenericErrorContext, "xsltGetInheritedNsList : out of memory!\n");
                        return (0);
                    }
                    ret[nbns] = NULL;
                }
		...
		*/
                ...
                if (i >= nbns) {
                    if (nbns >= maxns) {
                        ...
                        if (ret == NULL) {
                            xmlGenericError(xmlGenericErrorContext, "xsltGetInheritedNsList : realloc failed!\n");
                            return (0);
                        }
                    }
                    ret[nbns++] = cur;
...
void xsltParseStylesheetOutput(xsltStylesheetPtr style, xmlNodePtr cur)
{
	...
	*/
        ...
        if (prop == NULL) {
            ...
            if ((xmlStrEqual(prop, (const xmlChar*)"xml")) || (xmlStrEqual(prop, (const xmlChar*)"html")) || (xmlStrEqual(prop, (const xmlChar*)"text"))) {
                style->method = prop;
            } else {
    ...
    if (prop != NULL) {
        if (xmlStrEqual(prop, (const xmlChar*)"yes")) {
            ...
            xsltTransformError(NULL, style, cur, "invalid value for standalone: %s\n", prop);
            style->errors++;
        }
        xmlFree(prop);
    }
    ...
    if (prop != NULL) {
        if (xmlStrEqual(prop, (const xmlChar*)"yes")) {
            ...
            xsltTransformError(NULL, style, cur, "invalid value for indent: %s\n", prop);
            style->errors++;
        }
        xmlFree(prop);
    }
    ...
    if (prop != NULL) {
        if (xmlStrEqual(prop, (const xmlChar*)"yes")) {
            ...
            xsltTransformError(NULL, style, cur, "invalid value for omit-xml-declaration: %s\n", prop);
            style->errors++;
        }
        xmlFree(prop);
    }

    elements = xmlGetNsProp(cur, (const xmlChar*)"cdata-section-elements", NULL);
    if (elements != NULL) {
        if (style->cdataSection == NULL)
        ...
        while (*element != 0) {
			...
			*/
                        ...
                        xmlHashAddEntry2(style->cdataSection, element, URI, (void*)"cdata");
                        xmlFree(element);
                    }
                }
            }
...
static void xsltParseStylesheetDecimalFormat(xsltStylesheetPtr style, xmlNodePtr cur)
{
    ...
    if (prop != NULL) {
        ...
        if (xmlValidateQName(prop, 0) != 0) {
            xsltTransformError(NULL, style, cur, "xsl:decimal-format: Invalid QName '%s'.\n", prop);
            style->warnings++;
            xmlFree(prop);
            return;
        }
        ...
        */
        ...
        if (format != NULL) {
            xsltTransformError(NULL, style, cur, "xsltParseStylestyleDecimalFormat: %s already exists\n", prop);
            style->warnings++;
            xmlFree(prop);
            return;
        }
        ...
        if (format == NULL) {
            xsltTransformError(NULL, style, cur, "xsltParseStylestyleDecimalFormat: failed creating new decimal-format\n");
            style->errors++;
            xmlFree(prop);
            return;
        }
...
static void xsltParseStylesheetPreserveSpace(xsltStylesheetPtr style, xmlNodePtr cur)
{
    ...
    if (elements == NULL) {
        xsltTransformError(NULL, style, cur, "xsltParseStylesheetPreserveSpace: missing elements attribute\n");
        if (style != NULL)
            style->warnings++;
        return;
    }
    ...
    while (*element != 0) {
		...
		*/
                ...
                xmlHashAddEntry2(style->stripSpaces, element, URI, (xmlChar*)"preserve");
            }
            xmlFree(element);
        }
        element = end;
...
static void xsltParseStylesheetExtPrefix(xsltStylesheetPtr style, xmlNodePtr cur, int isXsltElem)
{
    ...
    if (isXsltElem) {
        ...
        prefixes = xmlGetNsProp(cur, (const xmlChar*)"extension-element-prefixes", NULL);
    } else {
        /* For literal result elements and extension instructions. */
        prefixes = xmlGetNsProp(cur, (const xmlChar*)"extension-element-prefixes", XSLT_NAMESPACE);
    }
    if (prefixes == NULL) {
        return;
    }
    ...
    while (*prefix != 0) {
        ...
        if (prefix) {
            ...
            if (ns == NULL) {
                xsltTransformError(NULL, style, cur, "xsl:extension-element-prefix : undefined namespace %s\n", prefix);
                if (style != NULL)
                    style->warnings++;
            } else {
#ifdef WITH_XSLT_DEBUG_PARSING
...
static void xsltParseStylesheetStripSpace(xsltStylesheetPtr style, xmlNodePtr cur)
{
    ...
    elements = xmlGetNsProp(cur, (const xmlChar*)"elements", NULL);
    if (elements == NULL) {
        xsltTransformError(NULL, style, cur, "xsltParseStylesheetStripSpace: missing elements attribute\n");
        if (style != NULL)
            style->warnings++;
        return;
    }
    ...
    while (*element != 0) {
		...
		*/
                ...
                xmlHashAddEntry2(style->stripSpaces, element, URI, (xmlChar*)"strip");
            }
            xmlFree(element);
        }
        element = end;
...
static int xsltParseStylesheetExcludePrefix(xsltStylesheetPtr style, xmlNodePtr cur, int isXsltElem)
{
    ...
    if (isXsltElem)
        prefixes = xmlGetNsProp(cur, (const xmlChar*)"exclude-result-prefixes", NULL);
    else
        prefixes = xmlGetNsProp(cur, (const xmlChar*)"exclude-result-prefixes", XSLT_NAMESPACE);
    ...
    while (*prefix != 0) {
        ...
        if (prefix) {
            ...
            if (xmlStrEqual(prefix, (const xmlChar*)"#default"))
                ...
                ns = xmlSearchNs(style->doc, cur, prefix);
            if (ns == NULL) {
                xsltTransformError(NULL, style, cur, "xsl:exclude-result-prefixes : undefined namespace %s\n", prefix);
                if (style != NULL)
                    style->warnings++;
            } else {
...
*/
static xmlNsPtr xsltTreeEnsureXMLDecl(xmlDocPtr doc)
{
    if (doc == NULL)
        return (NULL);
    if (doc->oldNs != NULL)
...
*         or internal error.
*/
static xmlNsPtr xsltTreeAcquireStoredNs(xmlDocPtr doc, const xmlChar* nsName, const xmlChar* prefix)
{
    xmlNsPtr ns;
    ...
    if (ns->next != NULL) {
        ...
        while (ns != NULL) {
            if ((ns->prefix == NULL) != (prefix == NULL)) {
                ...
                if ((ns->prefix[0] == prefix[0]) && xmlStrEqual(ns->prefix, prefix) && xmlStrEqual(ns->href, nsName))
                    return (ns);
            }
            if (ns->next == NULL)
                break;
 ...
 * its attributes.
 */
static int xsltLREBuildEffectiveNs(xsltCompilerCtxtPtr cctxt, xmlNodePtr elem)
{
    xmlNsPtr ns;
    ...
    while (alias != NULL) {
        if (/* If both namespaces are NULL... */
            ((elem->ns == NULL) && ((alias->literalNs == NULL) || (alias->literalNs->href == NULL))) ||
            /* ... or both namespace are equal */
            ((elem->ns != NULL) && (alias->literalNs != NULL) && xmlStrEqual(elem->ns->href, alias->literalNs->href))) {
            if ((alias->targetNs != NULL) && (alias->targetNs->href != NULL)) {
                /*
		* Convert namespace.
		*/
		    ...
		    */
                    ns = xmlSearchNs(elem->doc, elem, alias->targetNs->prefix);
                    /*
		    * If no matching ns-decl found, then assign a
		    * ns-decl stored in xmlDoc.
		    */
                    if ((ns == NULL) || (!xmlStrEqual(ns->href, alias->targetNs->href))) {
                        /*
			...
			*/
                        ns = xsltTreeAcquireStoredNs(elem->doc, alias->targetNs->href, alias->targetNs->prefix);
 ...
 */
static int xsltLREBuildEffectiveNsNodes(xsltCompilerCtxtPtr cctxt, xsltStyleItemLRElementInfoPtr item, xmlNodePtr elem, int isLRE)
{
    xmlNsPtr ns, tmpns;
    ...
    if ((cctxt == NULL) || (cctxt->inode == NULL) || (elem == NULL) || (item == NULL) || (item->effectiveNs != NULL))
        return (-1);
	...
	*/
		...
		*/
                if ((alias->targetNs != NULL) && (xmlStrEqual(alias->targetNs->href, ns->href))) {
                    /*
		...
		* TODO: Is xmlns="" handled already?
		*/
                if ((alias->literalNs != NULL) && (xmlStrEqual(alias->literalNs->href, ns->href))) {
                    /*
 ...
 */
static int xsltLREInfoCreate(xsltCompilerCtxtPtr cctxt, xmlNodePtr elem, int isLRE)
{
    ...
    item = (xsltStyleItemLRElementInfoPtr)xmlMalloc(sizeof(xsltStyleItemLRElementInfo));
    if (item == NULL) {
 ...
 */
static xsltVarInfoPtr xsltCompilerVarInfoPush(xsltCompilerCtxtPtr cctxt, xmlNodePtr inst, const xmlChar* name, const xmlChar* nsName)
{
    xsltVarInfoPtr ivar;
    ...
    if ((cctxt->ivar != NULL) && (cctxt->ivar->next != NULL)) {
        ...
        if (ivar == NULL) {
            xsltTransformError(NULL, cctxt->style, inst, "xsltParseInScopeVarPush: xmlMalloc() failed!\n");
            cctxt->style->errors++;
            return (NULL);
        }
 ...
 */
static void xsltCompilerVarInfoPop(xsltCompilerCtxtPtr cctxt)
{

    while ((cctxt->ivar != NULL) && (cctxt->ivar->depth > cctxt->depth)) {
...
*         NULL in case of an internal error.
*/
static xsltCompilerNodeInfoPtr xsltCompilerNodePush(xsltCompilerCtxtPtr cctxt, xmlNodePtr node)
{
	...
	*/
        inode = (xsltCompilerNodeInfoPtr)xmlMalloc(sizeof(xsltCompilerNodeInfo));
        if (inode == NULL) {
            xsltTransformError(NULL, cctxt->style, NULL, "xsltCompilerNodePush: malloc failed.\n");
            return (NULL);
        }
...
*/
static void xsltCompilerNodePop(xsltCompilerCtxtPtr cctxt, xmlNodePtr node)
{
    if (cctxt->inode == NULL) {
        xmlGenericError(xmlGenericErrorContext, "xsltCompilerNodePop: Top-node mismatch.\n");
        return;
    }
    /*
    * NOTE: Be carefull with the @node, since it might be
    ...
    */
    if (cctxt->inode->node != node) {
        xmlGenericError(xmlGenericErrorContext, "xsltCompilerNodePop: Node mismatch.\n");
        goto mismatch;
    }
    if (cctxt->inode->depth != cctxt->depth) {
        xmlGenericError(xmlGenericErrorContext, "xsltCompilerNodePop: Depth mismatch.\n");
        goto mismatch;
    }
    cctxt->depth--;
    /*
...
mismatch : {
    ...
    xmlGenericError(xmlGenericErrorContext, "xsltCompilerNodePop: Given   : '%s' URI '%s'\n", name, nsName);
    xmlGenericError(xmlGenericErrorContext, "xsltCompilerNodePop: Expected: '%s' URI '%s'\n", infname, infnsName);
}
}
...
*/
static xsltNsListContainerPtr xsltCompilerBuildInScopeNsList(xsltCompilerCtxtPtr cctxt, xmlNodePtr node)
{
    ...
    while (node != NULL) {
        if (node->type == XML_ELEMENT_NODE) {
            ...
            while (ns != NULL) {
                if (nsi == NULL) {
                    ...
                    nsi->list = (xmlNsPtr*)xmlMalloc(maxns * sizeof(xmlNsPtr));
                    if (nsi->list == NULL) {
                        xsltTransformError(NULL, cctxt->style, NULL,
                            "xsltCompilerBuildInScopeNsList: "
                            "malloc failed!\n");
		...
		*/
                for (i = 0; i < nsi->totalNumber; i++) {
                    if ((ns->prefix == nsi->list[i]->prefix) || (xmlStrEqual(ns->prefix, nsi->list[i]->prefix)))
                        break;
                }
                if (i >= nsi->totalNumber) {
                    if (nsi->totalNumber + 1 >= maxns) {
                        maxns *= 2;
                        nsi->list = (xmlNsPtr*)xmlRealloc(nsi->list, maxns * sizeof(xmlNsPtr));
                        if (nsi->list == NULL) {
                            xsltTransformError(NULL, cctxt->style, NULL,
                                "xsltCompilerBuildInScopeNsList: "
                                "realloc failed!\n");
    ...
    */
    if (xsltPointerListAddSize((xsltPointerListPtr)cctxt->psData->inScopeNamespaces, (void*)nsi, 5) == -1) {
        xmlFree(nsi);
        nsi = NULL;
        xsltTransformError(NULL, cctxt->style, NULL, "xsltCompilerBuildInScopeNsList: failed to add ns-info.\n");
        goto internal_err;
    }
    /*
...
static int xsltParseNsPrefixList(xsltCompilerCtxtPtr cctxt, xsltPointerListPtr list, xmlNodePtr node, const xmlChar* value)
{
    xmlChar *cur, *end;
    xmlNsPtr ns;
	...
	*/
        if ((cur[0] == '#') && xmlStrEqual(cur, (const xmlChar*)"#default"))
	    ...
	    */
            xsltTransformError(NULL, cctxt->style, node, "No namespace binding in scope for prefix '%s'.\n", cur);
            /*
...
#ifdef WITH_XSLT_DEBUG_PARSING
            xsltGenericDebug(xsltGenericDebugContext, "resolved prefix '%s'\n", cur);
#endif
            /*
	    * Note that we put the namespace name into the dict.
	    */
            if (xsltPointerListAddSize(list, (void*)xmlDictLookup(cctxt->style->dict, ns->href, -1), 5) == -1) {
                xmlFree(cur);
                goto internal_err;
            }
        }
 ...
 */
static xsltPointerListPtr xsltCompilerUtilsCreateMergedList(xsltPointerListPtr first, xsltPointerListPtr second)
{
    ...
    */
    if ((first != NULL) && (first->number != 0)) {
        memcpy(ret->items, first->items, first->number * sizeof(void*));
        if ((second != NULL) && (second->number != 0))
            memcpy(ret->items + first->number, second->items, second->number * sizeof(void*));
    } else if ((second != NULL) && (second->number != 0))
        memcpy(ret->items, (void*)second->items, second->number * sizeof(void*));
    ret->number = num;
    return (ret);
}
...
* Returns the ns-info or NULL if there are no namespaces in scope.
*/
static xsltPointerListPtr xsltParseExclResultPrefixes(xsltCompilerCtxtPtr cctxt, xmlNodePtr node, xsltPointerListPtr def, int instrCategory)
{
    ...
    if (instrCategory == XSLT_ELEMENT_CATEGORY_XSLT)
        ...
        attr = xmlHasNsProp(node, BAD_CAST "exclude-result-prefixes", XSLT_NAMESPACE);
    if (attr == NULL)
        return (def);
	...
	*/
    ...
    if ((attr->children != NULL) && (attr->children->content != NULL))
        ...
        xsltTransformError(NULL, cctxt->style, node, "Attribute 'exclude-result-prefixes': Invalid value.\n");
        cctxt->style->errors++;
        return (def);
    }

    if (xsltParseNsPrefixList(cctxt, cctxt->tmpList, node, BAD_CAST value) != 0)
        goto exit;
    ...
    */
    if (xsltPointerListAddSize(cctxt->psData->exclResultNamespaces, list, 5) == -1) {
        xsltPointerListFree(list);
        list = NULL;
        goto exit;
    }
...
* Returns the ns-info or NULL if there are no namespaces in scope.
*/
static xsltPointerListPtr xsltParseExtElemPrefixes(xsltCompilerCtxtPtr cctxt, xmlNodePtr node, xsltPointerListPtr def, int instrCategory)
{
    xsltPointerListPtr list = NULL;
    xmlAttrPtr attr;
    xmlChar* value;
    ...
    if (instrCategory == XSLT_ELEMENT_CATEGORY_XSLT)
        ...
        attr = xmlHasNsProp(node, BAD_CAST "extension-element-prefixes", XSLT_NAMESPACE);
    if (attr == NULL)
        return (def);
	...
	*/
    ...
    if ((attr->children != NULL) && (attr->children->content != NULL))
        ...
        xsltTransformError(NULL, cctxt->style, node, "Attribute 'extension-element-prefixes': Invalid value.\n");
        cctxt->style->errors++;
        return (def);
    }
    ...
    */
    for (i = 0; i < cctxt->tmpList->number; i++)
        xsltRegisterExtPrefix(cctxt->style, NULL, BAD_CAST cctxt->tmpList->items[i]);
    /*
    * Merge the list with the inherited list.
    */
    list = xsltCompilerUtilsCreateMergedList(def, cctxt->tmpList);
    ...
    */
    if (xsltPointerListAddSize(cctxt->psData->extElemNamespaces, list, 5) == -1) {
        xsltPointerListFree(list);
        list = NULL;
        goto exit;
    }
...
*/
static int xsltParseAttrXSLTVersion(xsltCompilerCtxtPtr cctxt, xmlNodePtr node, int instrCategory)
{
    xmlChar* value;
    xmlAttrPtr attr;
    ...
    if ((attr->children != NULL) && (attr->children->content != NULL))
        ...
        xsltTransformError(NULL, cctxt->style, node, "Attribute 'version': Invalid value.\n");
        cctxt->style->errors++;
        return (1);
    }
...
static int xsltParsePreprocessStylesheetTree(xsltCompilerCtxtPtr cctxt, xmlNodePtr node)
{
    ...
    if ((cctxt == NULL) || (cctxt->style == NULL) || (node == NULL) || (node->type != XML_ELEMENT_NODE))
        return (-1);
    ...
    */
    ...
    while (cur != NULL) {
        if (deleteNode != NULL) {
            ...
            xsltGenericDebug(xsltGenericDebugContext, "xsltParsePreprocessStylesheetTree: removing node\n");
#endif
	    ...
	    */
            if (IS_XSLT_ELEM(cur)) {
                ...
                if (cur->ns->href != nsNameXSLT) {
                    nsMapItem = xsltNewNamespaceMapItem(cctxt, doc, cur->ns, cur);
                    if (nsMapItem == NULL)
                        goto internal_err;
		...
		*/
                ...
                switch (*name) {
                case 't':
                    if ((name[0] == 't') && (name[1] == 'e') && (name[2] == 'x') && (name[3] == 't') && (name[4] == 0)) {
                        /*
			    ...
			    */
                    ...
                    break;
                case 'c':
                    if (xmlStrEqual(name, BAD_CAST "choose") || xmlStrEqual(name, BAD_CAST "call-template"))
                        cctxt->inode->stripWhitespace = 1;
                    break;
                case 'a':
                    if (xmlStrEqual(name, BAD_CAST "apply-templates") || xmlStrEqual(name, BAD_CAST "apply-imports")
			    ...
			    */
                        if (((*name == 'p') || (*name == 's')) && (xmlStrEqual(name, BAD_CAST "param") || xmlStrEqual(name, BAD_CAST "sort"))) {
                            do {
	    ...
	    */
            if (cur->properties != NULL) {
                ...
                do {
#ifdef XSLT_REFACTORED_XSLT_NSCOMP
                    if ((attr->ns) && (attr->ns->href != nsNameXSLT) && xmlStrEqual(attr->ns->href, nsNameXSLT)) {
                        nsMapItem = xsltNewNamespaceMapItem(cctxt, doc, attr->ns, cur);
                        if (nsMapItem == NULL)
                            goto internal_err;
			...
			*/
                        if ((txt != NULL) && (txt->content != NULL) && (!xmlDictOwns(style->dict, txt->content))) {
		    ...
		    * ----------------------------
		    */
                    if ((findSpaceAttr != 0) && (attr->ns != NULL) && (attr->name != NULL) && (attr->name[0] == 's') && (attr->ns->prefix != NULL)
                        && (attr->ns->prefix[0] == 'x') && (attr->ns->prefix[1] == 'm') && (attr->ns->prefix[2] == 'l') && (attr->ns->prefix[3] == 0)) {
                        value = xmlGetNsProp(cur, BAD_CAST "space", XML_XML_NAMESPACE);
                        if (value != NULL) {
                            if (xmlStrEqual(value, BAD_CAST "preserve")) {
                                ...
                                xsltTransformError(NULL, style, cur, "Attribute xml:space: Invalid value.\n");
                                cctxt->style->warnings++;
                            }
                            findSpaceAttr = 0;
                            xmlFree(value);
		...
		*/
                ...
                if ((cur->next == NULL) || (cur->next->type == XML_ELEMENT_NODE))
                    goto end_of_text;
                else
                    goto next_sibling;
            } else {
		...
		*/
        ...
        } else if ((cur->type == XML_COMMENT_NODE) || (cur->type == XML_PI_NODE)) {
            /*
	    ...
	    */
            xsltTransformError(NULL, style, cur, "Invalid type of node for the XSLT data model.\n");
            cctxt->style->errors++;
            goto next_sibling;
        }
    ...
    end_of_text:
	    ...
	    */
            if ((value == NULL) || (*value == 0)
                || (((cctxt->inode->stripWhitespace) || (!cctxt->inode->preserveWhitespace)) && IS_BLANK(*value) && xsltIsBlank(value))) {
                if (textNode != cur) {
	    ...
	    */
            ...
            if (internalize && (textNode->content != NULL) && (!xmlDictOwns(style->dict, textNode->content))) {
                /*
...
static void xsltPreprocessStylesheet(xsltStylesheetPtr style, xmlNodePtr cur)
{
    ...
    if ((cur->doc != NULL) && (style->dict != NULL) && (cur->doc->dict == style->dict))
        internalize = 1;
    else
        style->internalized = 0;

    if ((cur != NULL) && (IS_XSLT_ELEM(cur)) && (IS_XSLT_NAME(cur, "stylesheet"))) {
        styleelem = cur;
    } else {
        styleelem = NULL;
    }
    ...
    while (cur != NULL) {
	     ...
	     */
            if ((internalize) && (cur->properties != NULL)) {
                ...
                while (attr != NULL) {
                    txt = attr->children;
                    if ((txt != NULL) && (txt->type == XML_TEXT_NODE) && (txt->content != NULL) && (!xmlDictOwns(style->dict, txt->content))) {
                        xmlChar* tmp;
			 ...
			 * stylesheets.
			 */
                        tmp = (xmlChar*)xmlDictLookup(style->dict, txt->content, -1);
                        if (tmp != txt->content) {
                            xmlNodeSetContent(txt, NULL);
                            txt->content = tmp;
                        }
            ...
            if ((cur->nsDef != NULL) && (style->exclPrefixNr > 0)) {
                ...
                if ((root != NULL) && (root != cur)) {
                    while (ns != NULL) {
                        ...
                        next = ns->next;
                        for (i = 0; i < style->exclPrefixNr; i++) {
                            if ((ns->prefix != NULL) && (xmlStrEqual(ns->href, style->exclPrefixTab[i]))) {
                                /*
	 ...
	 */
        if ((cur->type == XML_ELEMENT_NODE) && (cur->ns != NULL) && (styleelem != NULL) && (cur->parent == styleelem)
            && (!xmlStrEqual(cur->ns->href, XSLT_NAMESPACE)) && (!xsltCheckExtURI(style, cur->ns->href))) {
            goto skip_children;
            ...
            while (ns != NULL) {
                if (ns->prefix != NULL) {
                    if (style->nsHash == NULL) {
                        ...
                        if (style->nsHash == NULL) {
                            xsltTransformError(NULL, style, cur, "xsltGatherNamespaces: failed to create hash table\n");
                            style->errors++;
                            return;
                        }
                    }
                    ...
                    if ((URI != NULL) && (!xmlStrEqual(URI, ns->href))) {
                        xsltTransformError(NULL, style, cur, "Namespaces prefix %s used for multiple namespaces\n", ns->prefix);
                        style->warnings++;
                    } else if (URI == NULL) {
                        xmlHashUpdateEntry(style->nsHash, ns->prefix, (void*)ns->href, NULL);
...
static xsltStyleType xsltGetXSLTElementTypeByNode(xsltCompilerCtxtPtr cctxt, xmlNodePtr node)
{
...
{
    if ((cctxt == NULL) || (elem == NULL) || (elem->type != XML_ELEMENT_NODE))
        return (-1);
    ...
    */
    if (elem->nsDef != NULL)
        cctxt->inode->inScopeNs = xsltCompilerBuildInScopeNsList(cctxt, elem);
    /*
    * xsltStylePreCompute():
    ...
    */
    ...
    case XSLT_FUNC_PARAM:
	    ...
	    */
        if ((elem->psvi != NULL) && (cctxt->ivar != NULL)) {
            ...
            do {
                if ((ivar->name == ((xsltStyleItemParamPtr)elem->psvi)->name) && (ivar->nsName == ((xsltStyleItemParamPtr)elem->psvi)->ns)) {
                    elem->psvi = NULL;
                    xsltTransformError(NULL, cctxt->style, elem, "Redefinition of variable or parameter '%s'.\n", ivar->name);
                    cctxt->style->errors++;
                    goto error;
                }
                ivar = ivar->prev;
    ...
    case XSLT_FUNC_VARIABLE:
	    ...
	    */
        if ((elem->psvi != NULL) && (cctxt->ivar != NULL)) {
            ...
            do {
                if ((ivar->name == ((xsltStyleItemVariablePtr)elem->psvi)->name) && (ivar->nsName == ((xsltStyleItemVariablePtr)elem->psvi)->ns)) {
                    elem->psvi = NULL;
                    xsltTransformError(NULL, cctxt->style, elem, "Redefinition of variable or parameter '%s'.\n", ivar->name);
                    cctxt->style->errors++;
                    goto error;
    ...
    default:
#ifdef WITH_XSLT_DEBUG_PARSING
        xsltGenericDebug(xsltGenericDebugContext, "xsltParseXSLTNode: Unhandled XSLT element '%s'.\n", elem->name);
#endif
        xsltTransformError(NULL, cctxt->style, elem,
...
empty_content:
	...
	*/
        do {
            if (((child->type != XML_TEXT_NODE) && (child->type != XML_CDATA_SECTION_NODE)) || (!IS_BLANK_NODE(child))) {
                xsltTransformError(NULL, cctxt->style, elem, "This XSLT element must have no content.\n");
                cctxt->style->errors++;
                break;
            }
            child = child->next;
...
for_each:
	...
	*/
        do {
            if ((child->type == XML_ELEMENT_NODE) && IS_XSLT_ELEM_FAST(child)) {
                if (xsltGetXSLTElementTypeByNode(cctxt, child) == XSLT_FUNC_SORT) {
                    cctxt->inode->curChildType = XSLT_FUNC_SORT;
                    xsltParseAnyXSLTElem(cctxt, child);
                } else
                    break;
...
sequence_constructor:
    ...
    */
    if ((elem->next != NULL) && ((cctxt->inode->type == XSLT_FUNC_VARIABLE) || (cctxt->inode->type == XSLT_FUNC_PARAM))) {
        if ((elem->psvi != NULL) && (((xsltStyleBasicItemVariablePtr)elem->psvi)->name)) {
            xsltCompilerVarInfoPush(cctxt, elem, ((xsltStyleBasicItemVariablePtr)elem->psvi)->name, ((xsltStyleBasicItemVariablePtr)elem->psvi)->ns);
        }
    }
 ...
 */
static xsltStyleItemUknownPtr xsltForwardsCompatUnkownItemCreate(xsltCompilerCtxtPtr cctxt)
{
 ...
 *         -1 on API or internal errors.
 */
static int xsltParseUnknownXSLTElem(xsltCompilerCtxtPtr cctxt, xmlNodePtr node)
{
	...
	*/
        xsltTransformError(NULL, cctxt->style, node, "Unknown XSLT element '%s'.\n", node->name);
        cctxt->style->errors++;
        return (1);
    }
    /*
	...
	*/
        if (node->nsDef != NULL)
            cctxt->inode->inScopeNs = xsltCompilerBuildInScopeNsList(cctxt, node);
        /*
	* Parse all xsl:fallback children.
	*/
        do {
            if ((child->type == XML_ELEMENT_NODE) && IS_XSLT_ELEM_FAST(child) && IS_XSLT_NAME(child, "fallback")) {
...
void xsltParseSequenceConstructor(xsltCompilerCtxtPtr cctxt, xmlNodePtr cur)
{
    ...
    if (cctxt == NULL) {
        xmlGenericError(xmlGenericErrorContext, "xsltParseSequenceConstructor: Bad arguments\n");
        cctxt->style->errors++;
        return;
    }
    /*
    ...
    */
    while (cur != NULL) {
        if (deleteNode != NULL) {
#ifdef WITH_XSLT_DEBUG_BLANKS
            xsltGenericDebug(xsltGenericDebugContext, "xsltParseSequenceConstructor: removing xsl:text element\n");
#endif
            xmlUnlinkNode(deleteNode);
            xmlFreeNode(deleteNode);
            deleteNode = NULL;
		...
		*/
		    ...
		    */
                    if ((tmp->type == XML_TEXT_NODE) || (tmp->next == NULL)) {
                        /*
		    ...
		    */
                    attr = cur->properties;
                    do {
                        if ((attr->ns == NULL) && (attr->name != NULL) && (attr->name[0] == 'd')
                            && xmlStrEqual(attr->name, BAD_CAST "disable-output-escaping")) {
                            /*
			    * Attr "disable-output-escaping".
			    * XSLT-2: This attribute is deprecated.
			    */
                            if ((attr->children != NULL) && xmlStrEqual(attr->children->content, BAD_CAST "yes")) {
                                /*
				...
				*/
                            ...
                            } else if ((attr->children == NULL) || (attr->children->content == NULL)
                                || (!xmlStrEqual(attr->children->content, BAD_CAST "no"))) {
                                xsltTransformError(NULL, cctxt->style, cur,
		...
		*/
                if (cur->nsDef != NULL)
                    cctxt->inode->inScopeNs = xsltCompilerBuildInScopeNsList(cctxt, cur);
                /*
		...
		*/
                if (cur->properties)
                    cctxt->inode->extElemNs = xsltParseExtElemPrefixes(cctxt, cur, cctxt->inode->extElemNs, XSLT_ELEMENT_CATEGORY_LRE);
                /*
		* Eval if we have an extension instruction here.
		*/
                if ((cur->ns != NULL) && (cctxt->inode->extElemNs != NULL) && (xsltCheckExtPrefix(cctxt->style, cur->ns->href) == 1)) {
                    /*
			...
			*/
                    ...
                    cur->psvi = (void*)xsltPreComputeExtModuleElement(cctxt->style, cur);
		    ...
		    */
                    if ((cur->children != NULL) && (cctxt->inode->extContentHandled == 0)) {
                        /*
			...
			*/
                        cctxt->inode->exclResultNs = xsltParseExclResultPrefixes(cctxt, cur, cctxt->inode->exclResultNs, XSLT_ELEMENT_CATEGORY_LRE);
                        /*
			* Attribute "xsl:version".
			*/
                        xsltParseAttrXSLTVersion(cctxt, cur, XSLT_ELEMENT_CATEGORY_LRE);
                        /*
			...
			*/
                        do {
                            if ((attr->psvi != xsltXSLTAttrMarker) && IS_XSLT_ATTR_FAST(attr)) {
                                if (!xmlStrEqual(attr->name, BAD_CAST "use-attribute-sets")) {
                                    xsltTransformError(NULL, cctxt->style, cur, "Unknown XSLT attribute '%s'.\n", attr->name);
                                    cctxt->style->errors++;
                                } else {
                                    /*
				    * XSLT attr marker.
...
{
    if ((style == NULL) || (templ == NULL) || (templ->type == XML_NAMESPACE_DECL))
        return;
	...
	*/
        do {
            if ((child->type == XML_ELEMENT_NODE) && IS_XSLT_ELEM_FAST(child) && IS_XSLT_NAME(child, "param")) {
...
void xsltParseTemplateContent(xsltStylesheetPtr style, xmlNodePtr templ)
{
    ...
    while (cur != NULL) {
        if (delete != NULL) {
#ifdef WITH_XSLT_DEBUG_BLANKS
            xsltGenericDebug(xsltGenericDebugContext, "xsltParseTemplateContent: removing text\n");
#endif
		...
		*/
                if (cur->children != NULL) {
                    ...
                    prop = xmlGetNsProp(cur, (const xmlChar*)"disable-output-escaping", NULL);
                    if (prop != NULL) {
#ifdef WITH_XSLT_DEBUG_PARSING
                        xsltGenericDebug(xsltGenericDebugContext, "Disable escaping: %s\n", text->content);
#endif
                        if (xmlStrEqual(prop, (const xmlChar*)"yes")) {
                            noesc = 1;
                        } else if (!xmlStrEqual(prop, (const xmlChar*)"no")) {
                            xsltTransformError(NULL, style, cur, "xsl:text: disable-output-escaping allows only yes or no\n");
                            style->warnings++;
                        }
                        xmlFree(prop);
                    }
                    ...
                    while (text != NULL) {
                        ...
                        if ((text->type != XML_TEXT_NODE) && (text->type != XML_CDATA_SECTION_NODE)) {
                            xsltTransformError(NULL, style, cur, "xsltParseTemplateContent: xslt:text content problem\n");
                            style->errors++;
                            break;
                        }
		     ...
		     */
                    if (text == NULL) {
                        text = cur->children;
                        while (text != NULL) {
                            if ((style->internalized) && (text->content != NULL) && (!xmlDictOwns(style->dict, text->content))) {
				 ...
				 */
                                if (text->doc->dict != NULL) {
                                    ...
                                    tmp = xmlDictLookup(text->doc->dict, text->content, -1);
                                    if (tmp != text->content) {
        ...
        } else if ((cur->ns != NULL) && (style->nsDefs != NULL) && (xsltCheckExtPrefix(style, cur->ns->prefix))) {
            /*
    ...
    while (cur != NULL) {
        if ((IS_XSLT_ELEM(cur)) && (IS_XSLT_NAME(cur, "param"))) {
            ...
            xsltTransformError(NULL, style, cur, "xsltParseTemplateContent: ignoring misplaced param element\n");
            if (style != NULL)
                style->warnings++;
            cur = cur->next;
            xmlUnlinkNode(param);
...
static void xsltParseStylesheetKey(xsltStylesheetPtr style, xmlNodePtr key)
{
	...
	*/
        ...
        xsltTransformError(NULL, style, key, "xsl:key : error missing name\n");
        if (style != NULL)
            style->errors++;
        goto error;
    }
    ...
    if (match == NULL) {
        xsltTransformError(NULL, style, key, "xsl:key : error missing match\n");
        if (style != NULL)
            style->errors++;
        goto error;
    }
    ...
    if (use == NULL) {
        xsltTransformError(NULL, style, key, "xsl:key : error missing use\n");
        if (style != NULL)
            style->errors++;
        goto error;
    }
...
static void xsltParseXSLTTemplate(xsltCompilerCtxtPtr cctxt, xmlNodePtr templNode)
{
    xsltTemplatePtr templ;
    xmlChar* prop;
    double priority;

    if ((cctxt == NULL) || (templNode == NULL) || (templNode->type != XML_ELEMENT_NODE))
        return;
...
#ifdef WITH_XSLT_DEBUG_PARSING
        xsltGenericDebug(xsltGenericDebugContext, "xsltParseXSLTTemplate: mode %s\n", templ->mode);
#endif
    }
    /*
    * Attribute "match".
	...
	*/
        ...
        curTempl = templ->next;
        while (curTempl != NULL) {
            if ((nameURI != NULL && xmlStrEqual(curTempl->name, templ->name) && xmlStrEqual(curTempl->nameURI, nameURI))
                || (nameURI == NULL && curTempl->nameURI == NULL && xmlStrEqual(curTempl->name, templ->name))) {
                xsltTransformError(NULL, cctxt->style, templNode, "xsl:template: error duplicate name '%s'\n", templ->name);
                cctxt->style->errors++;
                goto error;
            }
...
static void xsltParseStylesheetTemplate(xsltStylesheetPtr style, xmlNodePtr template)
{
    ...
    if ((style == NULL) || (template == NULL) || (template->type != XML_ELEMENT_NODE))
        return;
	...
	*/
        ...
        if (prop == NULL) {
            ...
            if (xmlValidateNCName(prop, 0)) {
                xsltTransformError(NULL, style, template, "xsl:template : error invalid name '%s'\n", prop);
                if (style != NULL)
                    style->errors++;
                xmlFree(prop);
                goto error;
 ...
 */
static xsltStyleItemIncludePtr xsltCompileXSLTIncludeElem(xsltCompilerCtxtPtr cctxt, xmlNodePtr node)
{
 ...
 */
static int xsltParseFindTopLevelElem(
    xsltCompilerCtxtPtr cctxt, xmlNodePtr cur, const xmlChar* name, const xmlChar* namespaceURI, int breakOnOtherElem, xmlNodePtr* resultNode)
{
    if (name == NULL)
        return (-1);
    ...
    while (cur != NULL) {
        if (cur->type == XML_ELEMENT_NODE) {
            if ((cur->ns != NULL) && (cur->name != NULL)) {
                if ((*(cur->name) == *name) && xmlStrEqual(cur->name, name) && xmlStrEqual(cur->ns->href, namespaceURI)) {
                    *resultNode = cur;
                    return (1);
                }
            }
...
static int xsltParseTopLevelXSLTElem(xsltCompilerCtxtPtr cctxt, xmlNodePtr node, xsltStyleType type)
{
    int ret = 0;
...
static int xsltParseXSLTStylesheetElemCore(xsltCompilerCtxtPtr cctxt, xmlNodePtr node)
{
#ifdef WITH_XSLT_DEBUG_PARSING
    int templates = 0;
#endif
    ...
    */
    if (xsltParseAttrXSLTVersion(cctxt, node, XSLT_ELEMENT_CATEGORY_XSLT) == 0) {
        /*
	* Attribute "version".
	...
	*/
        ...
        if (isXsltElem)
            xsltTransformError(NULL, cctxt->style, node, "The attribute 'version' is missing.\n");
        cctxt->style->errors++;
#else
        /* OLD behaviour. */
        xsltTransformError(NULL, cctxt->style, node, "xsl:version is missing: document may not be a stylesheet\n");
        cctxt->style->warnings++;
#endif
    }
    /*
    ...
    */
    cctxt->inode->extElemNs = xsltParseExtElemPrefixes(cctxt, node, NULL, XSLT_ELEMENT_CATEGORY_XSLT);
    /*
    * Attribute "exclude-result-prefixes".
    */
    cctxt->inode->exclResultNs = xsltParseExclResultPrefixes(cctxt, node, NULL, XSLT_ELEMENT_CATEGORY_XSLT);
    /*
    * Create/reuse info for the literal result element.
    */
    if (cctxt->inode->nsChanged)
    ...
    */
    ...
    while (cur != NULL) {
        if (cur->type == XML_TEXT_NODE) {
            xsltTransformError(NULL, style, cur, "Misplaced text node (content: '%s').\n", (cur->content != NULL) ? cur->content : BAD_CAST "");
            style->errors++;
        } else if (cur->type != XML_ELEMENT_NODE) {
	...
	*/
		...
		*/
		    ...
		    */
                    ...
                    xsltParseTopLevelXSLTElem(cctxt, cur, XSLT_FUNC_ATTRSET);
			...
			*/
                        ...
                        xsltTransformError(NULL, style, cur, "Unknown XSLT element '%s'.\n", cur->name);
                        style->errors++;
                    }
                }
            } else {
		...
		*  do exactly?
		*/
                function = xsltExtModuleTopLevelLookup(cur->name, cur->ns->href);
                if (function != NULL)
                    function(style, cur);
#ifdef WITH_XSLT_DEBUG_PARSING
...
#ifdef WITH_XSLT_DEBUG_PARSING
    xsltGenericDebug(xsltGenericDebugContext, "### END of parsing top-level elements of doc '%s'.\n", node->doc->URL);
    xsltGenericDebug(xsltGenericDebugContext, "### Templates: %d\n", templates);
#ifdef XSLT_REFACTORED
    xsltGenericDebug(xsltGenericDebugContext, "### Max inodes: %d\n", cctxt->maxNodeInfos);
    xsltGenericDebug(xsltGenericDebugContext, "### Max LREs  : %d\n", cctxt->maxLREs);
#endif /* XSLT_REFACTORED */
 ...
 *         -1 on API or internal errors.
 */
static int xsltParseXSLTStylesheetElem(xsltCompilerCtxtPtr cctxt, xmlNodePtr node)
{
    ...
    */
    cur = start;
    while ((cur != NULL) && xsltParseFindTopLevelElem(cctxt, cur, BAD_CAST "namespace-alias", XSLT_NAMESPACE, 0, &cur) == 1) {
...
static void xsltParseStylesheetTop(xsltStylesheetPtr style, xmlNodePtr top)
{
    ...
    if (prop == NULL) {
        xsltTransformError(NULL, style, top, "xsl:version is missing: document may not be a stylesheet\n");
        if (style != NULL)
            style->warnings++;
    } else {
        if ((!xmlStrEqual(prop, (const xmlChar*)"1.0")) && (!xmlStrEqual(prop, (const xmlChar*)"1.1"))) {
            xsltTransformError(NULL, style, top, "xsl:version: only 1.1 features are supported\n");
            if (style != NULL) {
    ...
    while (cur != NULL) {
        ...
        if (cur->type == XML_TEXT_NODE) {
            if (cur->content != NULL) {
                xsltTransformError(NULL, style, cur, "misplaced text node: '%s'\n", cur->content);
            }
            if (style != NULL)
                style->errors++;
            cur = cur->next;
        ...
        if (IS_XSLT_NAME(cur, "import")) {
            xsltTransformError(NULL, style, cur, "xsltParseStylesheetTop: ignoring misplaced import element\n");
            if (style != NULL)
                style->errors++;
        } else if (IS_XSLT_NAME(cur, "include")) {
            ...
            if ((style != NULL) && (style->forwards_compatible == 0)) {
                xsltTransformError(NULL, style, cur, "xsltParseStylesheetTop: unknown %s element\n", cur->name);
                if (style != NULL)
                    style->errors++;
            }
        }
 ...
 */
static int xsltParseSimplifiedStylesheetTree(xsltCompilerCtxtPtr cctxt, xmlDocPtr doc, xmlNodePtr node)
{
    xsltTemplatePtr templ;
...
#ifdef WITH_XSLT_DEBUG_PARSING
    xsltGenericDebug(xsltGenericDebugContext, "xsltParseSimplifiedStylesheetTree: document is stylesheet\n");
#endif
 ...
 */
xsltStylesheetPtr xsltParseStylesheetProcess(xsltStylesheetPtr style, xmlDocPtr doc)
{
    ...
    if (cur == NULL) {
        xsltTransformError(NULL, style, (xmlNodePtr)doc, "xsltParseStylesheetProcess : empty stylesheet\n");
        return (NULL);
    }
    oldIsSimplifiedStylesheet = cctxt->simplified;

    if ((IS_XSLT_ELEM(cur)) && ((IS_XSLT_NAME(cur, "stylesheet")) || (IS_XSLT_NAME(cur, "transform")))) {
#ifdef WITH_XSLT_DEBUG_PARSING
...
xsltStylesheetPtr xsltParseStylesheetProcess(xsltStylesheetPtr ret, xmlDocPtr doc)
{
    ...
    cur = xmlDocGetRootElement(doc);
    if (cur == NULL) {
        xsltTransformError(NULL, ret, (xmlNodePtr)doc, "xsltParseStylesheetProcess : empty stylesheet\n");
        return (NULL);
    }
	 ...
	 */
        ...
        if ((!xmlStrEqual(prop, (const xmlChar*)"1.0")) && (!xmlStrEqual(prop, (const xmlChar*)"1.1"))) {
            xsltTransformError(NULL, ret, cur, "xsl:version: only 1.1 features are supported\n");
...
xsltStylesheetPtr xsltParseStylesheetFile(const xmlChar* filename)
{
    ...
    if (sec != NULL) {
        ...
        if (res <= 0) {
            if (res == 0)
                xsltTransformError(NULL, NULL, NULL, "xsltParseStylesheetFile: read rights for %s denied\n", filename);
            return (NULL);
        }
    }

    doc = xsltDocDefaultLoader(filename, NULL, XSLT_PARSE_OPTIONS, NULL, XSLT_LOAD_START);
    if (doc == NULL) {
        xsltTransformError(NULL, NULL, NULL, "xsltParseStylesheetFile : cannot parse %s\n", filename);
        return (NULL);
    }
...
static xmlChar* xsltParseStylesheetPI(const xmlChar* value)
{
    ...
    while (CUR != 0) {
        SKIP_BLANKS;
        if ((CUR == 't') && (NXT(1) == 'y') && (NXT(2) == 'p') && (NXT(3) == 'e')) {
            SKIP(4);
            SKIP_BLANKS;
            if (CUR != '=')
                continue;
            ...
            if (val == NULL)
                return (NULL);
            if ((xmlStrcasecmp(val, BAD_CAST "text/xml")) && (xmlStrcasecmp(val, BAD_CAST "text/xsl"))) {
                xmlFree(val);
                break;
            }
            isXml = 1;
            xmlFree(val);
        } else if ((CUR == 'h') && (NXT(1) == 'r') && (NXT(2) == 'e') && (NXT(3) == 'f')) {
            SKIP(4);
            SKIP_BLANKS;
            if (CUR != '=')
                continue;
...
xsltStylesheetPtr xsltLoadStylesheetPI(xmlDocPtr doc)
{
    ...
    if (href != NULL) {
        ...
        if (URI == NULL) {
            xsltTransformError(NULL, NULL, child, "xml-stylesheet : href %s is not valid\n", href);
            xmlFree(href);
            return (NULL);
        }
        if ((URI->fragment != NULL) && (URI->scheme == NULL) && (URI->opaque == NULL) && (URI->authority == NULL) && (URI->server == NULL)
            && (URI->user == NULL) && (URI->path == NULL) && (URI->query == NULL)) {
            xmlAttrPtr ID;
            ...
            if (ID == NULL) {
                xsltTransformError(NULL, NULL, child, "xml-stylesheet : no ID %s found\n", URI->fragment);
            } else {
                xmlDocPtr fake;
