 ...
 */
static void exsltCryptoBin2Hex(const unsigned char* bin, int binlen, unsigned char* hex, int hexlen)
{
 ...
 */
static int exsltCryptoHex2Bin(const unsigned char* hex, int hexlen, unsigned char* bin, int binlen)
{
    int i = 0, j = 0;
...
static void exsltCryptoCryptoApiReportError(xmlXPathParserContextPtr ctxt, int line)
{
    char* lpMsgBuf;
    ...
    xsltTransformError(xsltXPathGetTransformContext(ctxt), NULL, NULL, "exslt:crypto error (line %d). %s", line, lpMsgBuf);
    LocalFree(lpMsgBuf);
}

static HCRYPTHASH exsltCryptoCryptoApiCreateHash(
    xmlXPathParserContextPtr ctxt, HCRYPTPROV hCryptProv, ALG_ID algorithm, const unsigned char* msg, unsigned int msglen, char* dest, unsigned int destlen)
{
    HCRYPTHASH hHash = 0;
 ...
 */
static void exsltCryptoCryptoApiHash(xmlXPathParserContextPtr ctxt, ALG_ID algorithm, const char* msg, unsigned long msglen, char dest[HASH_DIGEST_LENGTH])
{
    ...
    hHash = exsltCryptoCryptoApiCreateHash(ctxt, hCryptProv, algorithm, (unsigned char*)msg, msglen, dest, HASH_DIGEST_LENGTH);
    if (0 != hHash) {
...
static void exsltCryptoCryptoApiRc4Encrypt(
    xmlXPathParserContextPtr ctxt, const unsigned char* key, const unsigned char* msg, int msglen, unsigned char* dest, int destlen)
{
    ...
    if (msglen > destlen) {
        xsltTransformError(
            xsltXPathGetTransformContext(ctxt), NULL, NULL, "exslt:crypto : internal error exsltCryptoCryptoApiRc4Encrypt dest buffer too small.\n");
        return;
    }
    ...
    hHash = exsltCryptoCryptoApiCreateHash(ctxt, hCryptProv, CALG_SHA1, key, RC4_KEY_LENGTH, hash, HASH_DIGEST_LENGTH);
...
static void exsltCryptoCryptoApiRc4Decrypt(
    xmlXPathParserContextPtr ctxt, const unsigned char* key, const unsigned char* msg, int msglen, unsigned char* dest, int destlen)
{
    HCRYPTPROV hCryptProv;
    HCRYPTKEY hKey;
...
static void exsltCryptoGcryptInit(void)
{
 ...
 * using gcrypt
 */
static void exsltCryptoGcryptHash(xmlXPathParserContextPtr ctxt ATTRIBUTE_UNUSED,
    /* changed the enum to int */
    int algorithm, const char* msg, unsigned long msglen, char dest[HASH_DIGEST_LENGTH])
{
    exsltCryptoGcryptInit();
    gcry_md_hash_buffer(algorithm, dest, msg, msglen);
}

static void exsltCryptoGcryptRc4Encrypt(
    xmlXPathParserContextPtr ctxt, const unsigned char* key, const unsigned char* msg, int msglen, unsigned char* dest, int destlen)
{
    ...
    if (rc) {
        xsltTransformError(xsltXPathGetTransformContext(ctxt), NULL, NULL, "exslt:crypto internal error %s (gcry_cipher_open)\n", gcry_strerror(rc));
    }

    rc = gcry_cipher_setkey(cipher, key, RC4_KEY_LENGTH);
    if (rc) {
        xsltTransformError(xsltXPathGetTransformContext(ctxt), NULL, NULL, "exslt:crypto internal error %s (gcry_cipher_setkey)\n", gcry_strerror(rc));
    }

    rc = gcry_cipher_encrypt(cipher, (unsigned char*)dest, destlen, (const unsigned char*)msg, msglen);
    if (rc) {
        xsltTransformError(xsltXPathGetTransformContext(ctxt), NULL, NULL, "exslt:crypto internal error %s (gcry_cipher_encrypt)\n", gcry_strerror(rc));
    }

    gcry_cipher_close(cipher);
}

static void exsltCryptoGcryptRc4Decrypt(
    xmlXPathParserContextPtr ctxt, const unsigned char* key, const unsigned char* msg, int msglen, unsigned char* dest, int destlen)
{
    gcry_cipher_hd_t cipher;
    gcry_error_t rc = 0;
    ...
    rc = gcry_cipher_decrypt(cipher, (unsigned char*)dest, destlen, (const unsigned char*)msg, msglen);
    if (rc) {
        xsltTransformError(xsltXPathGetTransformContext(ctxt), NULL, NULL, "exslt:crypto internal error %s (gcry_cipher_decrypt)\n", gcry_strerror(rc));
    }
 ...
 * length in bytes.
 */
static int exsltCryptoPopString(xmlXPathParserContextPtr ctxt, int nargs, xmlChar** str)
{
 ...
 */
static void exsltCryptoMd4Function(xmlXPathParserContextPtr ctxt, int nargs)
{
    ...
    PLATFORM_HASH(ctxt, PLATFORM_MD4, (const char*)str, str_len, (char*)hash);
 ...
 */
static void exsltCryptoMd5Function(xmlXPathParserContextPtr ctxt, int nargs)
{
    ...
    PLATFORM_HASH(ctxt, PLATFORM_MD5, (const char*)str, str_len, (char*)hash);
    exsltCryptoBin2Hex(hash, sizeof(hash) - 1, hex, sizeof(hex) - 1);
 ...
 */
static void exsltCryptoSha1Function(xmlXPathParserContextPtr ctxt, int nargs)
{
    ...
    PLATFORM_HASH(ctxt, PLATFORM_SHA1, (const char*)str, str_len, (char*)hash);
    exsltCryptoBin2Hex(hash, sizeof(hash) - 1, hex, sizeof(hex) - 1);
 ...
 * computes the sha1 hash of a string and returns as hex
 */
static void exsltCryptoRc4EncryptFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    ...
    if (padkey == NULL) {
        xsltTransformError(tctxt, NULL, tctxt->inst, "exsltCryptoRc4EncryptFunction: Failed to allocate padkey\n");
    ...
    if ((key_len > RC4_KEY_LENGTH) || (key_len < 0)) {
        xsltTransformError(tctxt, NULL, tctxt->inst, "exsltCryptoRc4EncryptFunction: key size too long or key broken\n");
        tctxt->state = XSLT_STATE_STOPPED;
        xmlXPathReturnEmptyString(ctxt);
        goto done;
    }
    ...
    bin = xmlStrdup(str);
    if (bin == NULL) {
        xsltTransformError(tctxt, NULL, tctxt->inst, "exsltCryptoRc4EncryptFunction: Failed to allocate string\n");
        tctxt->state = XSLT_STATE_STOPPED;
        xmlXPathReturnEmptyString(ctxt);
        goto done;
    }
    ...
    if (hex == NULL) {
        xsltTransformError(tctxt, NULL, tctxt->inst, "exsltCryptoRc4EncryptFunction: Failed to allocate result\n");
        tctxt->state = XSLT_STATE_STOPPED;
        xmlXPathReturnEmptyString(ctxt);
        goto done;
    }
 ...
 * computes the sha1 hash of a string and returns as hex
 */
static void exsltCryptoRc4DecryptFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    ...
    if (xmlCheckUTF8(ret) == 0) {
        xsltTransformError(tctxt, NULL, tctxt->inst, "exsltCryptoRc4DecryptFunction: Invalid UTF-8\n");
        xmlFree(ret);
        xmlXPathReturnEmptyString(ctxt);
    } else {
        xmlXPathReturnString(ctxt, ret);
...
{
    xsltRegisterExtModuleFunction((const xmlChar*)"md4", EXSLT_CRYPTO_NAMESPACE, exsltCryptoMd4Function);
    xsltRegisterExtModuleFunction((const xmlChar*)"md5", EXSLT_CRYPTO_NAMESPACE, exsltCryptoMd5Function);
    xsltRegisterExtModuleFunction((const xmlChar*)"sha1", EXSLT_CRYPTO_NAMESPACE, exsltCryptoSha1Function);
    xsltRegisterExtModuleFunction((const xmlChar*)"rc4_encrypt", EXSLT_CRYPTO_NAMESPACE, exsltCryptoRc4EncryptFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"rc4_decrypt", EXSLT_CRYPTO_NAMESPACE, exsltCryptoRc4DecryptFunction);
}
