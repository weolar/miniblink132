...
static void exsltDynMapFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    xmlChar* str = NULL;
    ...
    if (str == NULL || !xmlStrlen(str) || !(comp = xmlXPathCtxtCompile(tctxt->xpathCtxt, str)))
        goto cleanup;
    ...
    if (nodeset && nodeset->nodeNr > 0) {
        ...
        for (i = 0; i < nodeset->nodeNr; i++) {
            ...
            if (subResult != NULL) {
                ...
                case XPATH_BOOLEAN:
                    if (container != NULL) {
                        xmlNodePtr newChildNode = xmlNewTextChild((xmlNodePtr)container, NULL, BAD_CAST "boolean", BAD_CAST(subResult->boolval ? "true" : ""));
                        if (newChildNode != NULL) {
                            newChildNode->ns = xmlNewNs(newChildNode, BAD_CAST "http://exslt.org/common", BAD_CAST "exsl");
                            xmlXPathNodeSetAddUnique(ret->nodesetval, newChildNode);
                        }
                    }
                    break;
                case XPATH_NUMBER:
                    if (container != NULL) {
                        ...
                        xmlNodePtr newChildNode = xmlNewTextChild((xmlNodePtr)container, NULL, BAD_CAST "number", val);
                        if (val != NULL)
                            xmlFree(val);
                ...
                case XPATH_STRING:
                    if (container != NULL) {
                        xmlNodePtr newChildNode = xmlNewTextChild((xmlNodePtr)container, NULL, BAD_CAST "string", subResult->stringval);
                        if (newChildNode != NULL) {
                            newChildNode->ns = xmlNewNs(newChildNode, BAD_CAST "http://exslt.org/common", BAD_CAST "exsl");
                            xmlXPathNodeSetAddUnique(ret->nodesetval, newChildNode);
                        }
