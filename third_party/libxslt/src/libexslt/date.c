...
static const long daysInMonth[12] = { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };
static const long daysInMonthLeap[12] = { 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };
...
#define VALID_DATE(dt) (VALID_MONTH(dt->mon) && VALID_MDAY(dt))

/*
    hour and min structure vals are unsigned, so normal macros give
    warnings on some compilers.
*/
#define VALID_TIME(dt) ((dt->hour <= 23) && (dt->min <= 59) && VALID_SEC(dt->sec) && VALID_TZO(dt->tzo))
 ...
 */
static int _exsltDateParseGYear(exsltDateValPtr dt, const xmlChar** str)
{
    ...
    *str = cur;
    ...
    xsltGenericDebug(xsltGenericDebugContext, "Parsed year %04ld\n", dt->year);
#endif
 ...
 * Returns 0 or the error code
 */
static int _exsltDateParseGMonth(exsltDateValPtr dt, const xmlChar** str)
{
...
#ifdef DEBUG_EXSLT_DATE
    xsltGenericDebug(xsltGenericDebugContext, "Parsed month %02i\n", dt->mon);
#endif
 ...
 * Returns 0 or the error code
 */
static int _exsltDateParseGDay(exsltDateValPtr dt, const xmlChar** str)
{
    const xmlChar* cur = *str;
    int ret = 0;
...
#ifdef DEBUG_EXSLT_DATE
    xsltGenericDebug(xsltGenericDebugContext, "Parsed day %02i\n", dt->day);
#endif
 ...
 * Returns 0 or the error code
 */
static int _exsltDateParseTime(exsltDateValPtr dt, const xmlChar** str)
{
    const xmlChar* cur = *str;
...
#ifdef DEBUG_EXSLT_DATE
    xsltGenericDebug(xsltGenericDebugContext, "Parsed time %02i:%02i:%02.f\n", dt->hour, dt->min, dt->sec);
#endif
 ...
 * Returns 0 or the error code
 */
static int _exsltDateParseTimeZone(exsltDateValPtr dt, const xmlChar** str)
{
    const xmlChar* cur;
    int ret = 0;
...
#ifdef DEBUG_EXSLT_DATE
    xsltGenericDebug(xsltGenericDebugContext, "Parsed time zone offset (%s) %i\n", dt->tz_flag ? "explicit" : "implicit", dt->tzo);
#endif
 ...
 */
static exsltDateValPtr exsltDateCreateDate(exsltDateType type)
{
    exsltDateValPtr ret;

    ret = (exsltDateValPtr)xmlMalloc(sizeof(exsltDateVal));
    if (ret == NULL) {
        xsltGenericError(xsltGenericErrorContext, "exsltDateCreateDate: out of memory\n");
        return (NULL);
    }
 ...
 * Frees up the @date
 */
static void exsltDateFreeDate(exsltDateValPtr date)
{
    if (date == NULL)
        return;
 ...
 */
static exsltDateDurValPtr exsltDateCreateDuration(void)
{
    exsltDateDurValPtr ret;

    ret = (exsltDateDurValPtr)xmlMalloc(sizeof(exsltDateDurVal));
    if (ret == NULL) {
        xsltGenericError(xsltGenericErrorContext, "exsltDateCreateDuration: out of memory\n");
        return (NULL);
    }
 ...
 */
static void exsltDateFreeDuration(exsltDateDurValPtr duration)
{
    if (duration == NULL)
        return;
 ...
 */
static exsltDateValPtr exsltDateCurrent(void)
{
    ...
    ret->tzo = (((ret->day * 1440) +
                ...
                ((gmTm.tm_mday * 1440) + (gmTm.tm_hour * 60) +
                  gmTm.tm_min));
#endif
    local_s = localTm.tm_hour * SECS_PER_HOUR + localTm.tm_min * SECS_PER_MIN + localTm.tm_sec;

    gm_s = gmTm.tm_hour * SECS_PER_HOUR + gmTm.tm_min * SECS_PER_MIN + gmTm.tm_sec;
 ...
 */
static exsltDateValPtr exsltDateParse(const xmlChar* dateTime)
{
    exsltDateValPtr dt;
    int ret;
 ...
 */
static exsltDateDurValPtr exsltDateParseDuration(const xmlChar* duration)
{
...
#ifdef DEBUG_EXSLT_DATE
    xsltGenericDebug(xsltGenericDebugContext, "Parsed duration %f\n", dur->sec);
#endif
 ...
 */
static xmlChar* exsltDateFormatDuration(const exsltDateDurValPtr dur)
{
 ...
 * Returns a newly allocated string, or NULL in case of error
 */
static xmlChar* exsltDateFormatDateTime(const exsltDateValPtr dt)
{
    xmlChar buf[100], *cur = buf;
 ...
 * Returns a newly allocated string, or NULL in case of error
 */
static xmlChar* exsltDateFormatDate(const exsltDateValPtr dt)
{
    xmlChar buf[100], *cur = buf;
 ...
 * Returns a newly allocated string, or NULL in case of error
 */
static xmlChar* exsltDateFormatTime(const exsltDateValPtr dt)
{
    xmlChar buf[100], *cur = buf;
 ...
 * Returns a newly allocated string, or NULL in case of error
 */
static xmlChar* exsltDateFormat(const exsltDateValPtr dt)
{
 ...
 */
static long _exsltDateCastYMToDays(const exsltDateValPtr dt)
{
    long ret;

    if (dt->year <= 0)
        ret = ((dt->year - 1) * 365) + (((dt->year) / 4) - ((dt->year) / 100) + ((dt->year) / 400)) + DAY_IN_YEAR(0, dt->mon, dt->year) - 1;
    else
        ret = ((dt->year - 1) * 365) + (((dt->year - 1) / 4) - ((dt->year - 1) / 100) + ((dt->year - 1) / 400)) + DAY_IN_YEAR(0, dt->mon, dt->year);
 ...
 * Returns seconds.
 */
#define TIME_TO_NUMBER(dt) ((double)((dt->hour * SECS_PER_HOUR) + (dt->min * SECS_PER_MIN)) + dt->sec)
 ...
 */
static int _exsltDateTruncateDate(exsltDateValPtr dt, exsltDateType type)
{
    if (dt == NULL)
        return 1;
 ...
 */
static long _exsltDateDayInWeek(long yday, long yr)
{
    long ret;
 ...
 */
static exsltDateValPtr _exsltDateAdd(exsltDateValPtr dt, exsltDateDurValPtr dur)
{
    exsltDateValPtr ret;
    long carry, temp;
    double sum;
 ...
 */
static exsltDateDurValPtr _exsltDateDifference(exsltDateValPtr x, exsltDateValPtr y, int flag)
{
    exsltDateDurValPtr ret;
    ...
    if (((x->type < XS_GYEAR) || (x->type > XS_DATETIME)) || ((y->type < XS_GYEAR) || (y->type > XS_DATETIME)))
        return NULL;
    ...
    if (((x->type == XS_GYEAR) || (x->type == XS_GYEARMONTH)) && (!flag)) {
        ...
        if ((x->year >= LONG_MAX / 24) || (x->year <= LONG_MIN / 24) || (y->year >= LONG_MAX / 24) || (y->year <= LONG_MIN / 24)) {
            /* Possible overflow. */
        ...
        if ((x->year > LONG_MAX / 731) || (x->year < LONG_MIN / 731) || (y->year > LONG_MAX / 731) || (y->year < LONG_MIN / 731)) {
            /* Possible overflow. */
            exsltDateFreeDuration(ret);
            return NULL;
        }
 ...
 */
static int _exsltDateAddDurCalc(exsltDateDurValPtr ret, exsltDateDurValPtr x, exsltDateDurValPtr y)
{
    /* months */
    if ((x->mon > 0 && y->mon > LONG_MAX - x->mon) || (x->mon < 0 && y->mon < LONG_MIN - x->mon)) {
        /* Overflow */
        return 0;
    }
    ret->mon = x->mon + y->mon;
    ...
    /* days */
    if ((x->day > 0 && y->day > LONG_MAX - x->day) || (x->day < 0 && y->day < LONG_MIN - x->day)) {
        /* Overflow */
        return 0;
    }
    ret->day = x->day + y->day;
 ...
 * Returns a duration pointer or NULL.
 */
static exsltDateDurValPtr _exsltDateAddDuration(exsltDateDurValPtr x, exsltDateDurValPtr y)
{
    exsltDateDurValPtr ret;
 ...
 */
static xmlChar* exsltDateDateTime(void)
{
    xmlChar* ret = NULL;
    exsltDateValPtr cur;
 ...
 */
static xmlChar* exsltDateDate(const xmlChar* dateTime)
{
 ...
 * formats, returns NULL.
 */
static xmlChar* exsltDateTime(const xmlChar* dateTime)
{
    exsltDateValPtr dt = NULL;
    xmlChar* ret = NULL;
 ...
 * returned.
 */
static double exsltDateYear(const xmlChar* dateTime)
{
    exsltDateValPtr dt;
    long year;
    double ret;
    ...
    if (dateTime == NULL) {
        ...
        if (dt == NULL)
            return xmlXPathNAN;
        if ((dt->type != XS_DATETIME) && (dt->type != XS_DATE) && (dt->type != XS_GYEARMONTH) && (dt->type != XS_GYEAR)) {
            exsltDateFreeDate(dt);
            return xmlXPathNAN;
        }
    }
    ...
    year = dt->year;
    if (year <= 0)
        year -= 1; /* Adjust for missing year 0. */
    ret = (double)year;
    exsltDateFreeDate(dt);
 ...
 * returned.
 */
static xmlXPathObjectPtr exsltDateLeapYear(const xmlChar* dateTime)
{
    exsltDateValPtr dt = NULL;
    ...
    if (dateTime == NULL) {
        ...
        dt = exsltDateParse(dateTime);
        if ((dt != NULL) && (dt->type != XS_DATETIME) && (dt->type != XS_DATE) && (dt->type != XS_GYEARMONTH) && (dt->type != XS_GYEAR)) {
            exsltDateFreeDate(dt);
            dt = NULL;
        }
    }
 ...
 * returned.
 */
static double exsltDateMonthInYear(const xmlChar* dateTime)
{
    exsltDateValPtr dt;
    double ret;
    ...
    if (dateTime == NULL) {
        ...
        if (dt == NULL)
            return xmlXPathNAN;
        if ((dt->type != XS_DATETIME) && (dt->type != XS_DATE) && (dt->type != XS_GYEARMONTH) && (dt->type != XS_GMONTH) && (dt->type != XS_GMONTHDAY)) {
            exsltDateFreeDate(dt);
            return xmlXPathNAN;
        }
    }
 ...
 */
static const xmlChar* exsltDateMonthName(const xmlChar* dateTime)
{
    static const xmlChar monthNames[13][10] = { { 0 }, { 'J', 'a', 'n', 'u', 'a', 'r', 'y', 0 }, { 'F', 'e', 'b', 'r', 'u', 'a', 'r', 'y', 0 },
        { 'M', 'a', 'r', 'c', 'h', 0 }, { 'A', 'p', 'r', 'i', 'l', 0 }, { 'M', 'a', 'y', 0 }, { 'J', 'u', 'n', 'e', 0 }, { 'J', 'u', 'l', 'y', 0 },
        { 'A', 'u', 'g', 'u', 's', 't', 0 }, { 'S', 'e', 'p', 't', 'e', 'm', 'b', 'e', 'r', 0 }, { 'O', 'c', 't', 'o', 'b', 'e', 'r', 0 },
 ...
 * 'Dec'.
 */
static const xmlChar* exsltDateMonthAbbreviation(const xmlChar* dateTime)
{
    static const xmlChar monthAbbreviations[13][4]
        = { { 0 }, { 'J', 'a', 'n', 0 }, { 'F', 'e', 'b', 0 }, { 'M', 'a', 'r', 0 }, { 'A', 'p', 'r', 0 }, { 'M', 'a', 'y', 0 }, { 'J', 'u', 'n', 0 },
 ...
 * returned.
 */
static double exsltDateWeekInYear(const xmlChar* dateTime)
{
    exsltDateValPtr dt;
 ...
 * returned.
 */
static double exsltDateWeekInMonth(const xmlChar* dateTime)
{
    exsltDateValPtr dt;
    long fdiy, fdiw, ret;
 ...
 * returned.
 */
static double exsltDateDayInYear(const xmlChar* dateTime)
{
    exsltDateValPtr dt;
    long ret;
 ...
 * returned.
 */
static double exsltDateDayInMonth(const xmlChar* dateTime)
{
    exsltDateValPtr dt;
    double ret;
    ...
    if (dateTime == NULL) {
        ...
        if (dt == NULL)
            return xmlXPathNAN;
        if ((dt->type != XS_DATETIME) && (dt->type != XS_DATE) && (dt->type != XS_GMONTHDAY) && (dt->type != XS_GDAY)) {
            exsltDateFreeDate(dt);
            return xmlXPathNAN;
        }
    }
 ...
 * returned.
 */
static double exsltDateDayOfWeekInMonth(const xmlChar* dateTime)
{
    exsltDateValPtr dt;
    long ret;
 ...
 */
static double exsltDateDayInWeek(const xmlChar* dateTime)
{
    exsltDateValPtr dt;
    long diy, ret;
 ...
 */
static const xmlChar* exsltDateDayName(const xmlChar* dateTime)
{
    static const xmlChar dayNames[8][10] = { { 0 }, { 'S', 'u', 'n', 'd', 'a', 'y', 0 }, { 'M', 'o', 'n', 'd', 'a', 'y', 0 },
        { 'T', 'u', 'e', 's', 'd', 'a', 'y', 0 }, { 'W', 'e', 'd', 'n', 'e', 's', 'd', 'a', 'y', 0 }, { 'T', 'h', 'u', 'r', 's', 'd', 'a', 'y', 0 },
        { 'F', 'r', 'i', 'd', 'a', 'y', 0 }, { 'S', 'a', 't', 'u', 'r', 'd', 'a', 'y', 0 } };
    double day;
    int index = 0;
 ...
 */
static const xmlChar* exsltDateDayAbbreviation(const xmlChar* dateTime)
{
    static const xmlChar dayAbbreviations[8][4] = { { 0 }, { 'S', 'u', 'n', 0 }, { 'M', 'o', 'n', 0 }, { 'T', 'u', 'e', 0 }, { 'W', 'e', 'd', 0 },
 ...
 * returned.
 */
static double exsltDateHourInDay(const xmlChar* dateTime)
{
    exsltDateValPtr dt;
    double ret;
 ...
 * returned.
 */
static double exsltDateMinuteInHour(const xmlChar* dateTime)
{
    exsltDateValPtr dt;
    double ret;
 ...
 */
static double exsltDateSecondInMinute(const xmlChar* dateTime)
{
    exsltDateValPtr dt;
    double ret;
 ...
 */
static xmlChar* exsltDateAdd(const xmlChar* xstr, const xmlChar* ystr)
{
 ...
 */
static xmlChar* exsltDateAddDuration(const xmlChar* xstr, const xmlChar* ystr)
{
 ...
 * Returns duration string or NULL.
 */
static void exsltDateSumFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    xmlNodeSetPtr ns;
    void* user = NULL;
    xmlChar* tmp;
 ...
 */
static double exsltDateSeconds(const xmlChar* dateTime)
{
    exsltDateValPtr dt;
 ...
 * Returns duration string or NULL.
 */
static xmlChar* exsltDateDifference(const xmlChar* xstr, const xmlChar* ystr)
{
    exsltDateValPtr x, y;
    exsltDateDurValPtr dur;
    xmlChar* ret = NULL;
 ...
 * Returns duration string or NULL.
 */
static xmlChar* exsltDateDuration(const xmlChar* number)
{
    exsltDateDurValPtr dur;
    double secs, days;
    xmlChar* ret;
 ...
 */
static void exsltDateDateTimeFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    xmlChar* ret;
 ...
 */
static void exsltDateDateFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    xmlChar *ret, *dt = NULL;
 ...
 */
static void exsltDateTimeFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    xmlChar *ret, *dt = NULL;
 ...
 */
static void exsltDateYearFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    xmlChar* dt = NULL;
    double ret;
 ...
 */
static void exsltDateLeapYearFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    xmlChar* dt = NULL;
    xmlXPathObjectPtr ret;
...
#define X_IN_Y(x, y)                                                                                                                                           \
    static void exsltDate##x##In##y##Function(xmlXPathParserContextPtr ctxt, int nargs)                                                                        \
    {                                                                                                                                                          \
        xmlChar* dt = NULL;                                                                                                                                    \
        double ret;                                                                                                                                            \
                                                                                                                                                               \
 ...
 */
static void exsltDateMonthNameFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    xmlChar* dt = NULL;
    const xmlChar* ret;
 ...
 */
static void exsltDateMonthAbbreviationFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    xmlChar* dt = NULL;
    const xmlChar* ret;
 ...
 */
static void exsltDateDayNameFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    xmlChar* dt = NULL;
    const xmlChar* ret;
 ...
 */
static void exsltDateDayAbbreviationFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    xmlChar* dt = NULL;
    const xmlChar* ret;
 ...
 */
static void exsltDateSecondsFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    xmlChar* str = NULL;
    double ret;
 ...
 */
static void exsltDateAddFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
 ...
 */
static void exsltDateAddDurationFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    xmlChar *ret, *xstr, *ystr;
 ...
 */
static void exsltDateDifferenceFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    xmlChar *ret, *xstr, *ystr;
 ...
 */
static void exsltDateDurationFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    xmlChar* ret;
    xmlChar* number = NULL;
...
{
    xsltRegisterExtModuleFunction((const xmlChar*)"add", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateAddFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"add-duration", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateAddDurationFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"date", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateDateFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"date-time", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateDateTimeFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"day-abbreviation", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateDayAbbreviationFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"day-in-month", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateDayInMonthFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"day-in-week", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateDayInWeekFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"day-in-year", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateDayInYearFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"day-name", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateDayNameFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"day-of-week-in-month", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateDayOfWeekInMonthFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"difference", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateDifferenceFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"duration", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateDurationFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"hour-in-day", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateHourInDayFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"leap-year", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateLeapYearFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"minute-in-hour", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateMinuteInHourFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"month-abbreviation", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateMonthAbbreviationFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"month-in-year", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateMonthInYearFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"month-name", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateMonthNameFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"second-in-minute", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateSecondInMinuteFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"seconds", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateSecondsFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"sum", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateSumFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"time", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateTimeFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"week-in-month", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateWeekInMonthFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"week-in-year", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateWeekInYearFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"year", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateYearFunction);
}
...
{
    if (ctxt && prefix && !xmlXPathRegisterNs(ctxt, prefix, (const xmlChar*)EXSLT_DATE_NAMESPACE)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"add", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateAddFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"add-duration", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateAddDurationFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"date", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateDateFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"date-time", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateDateTimeFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"day-abbreviation", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateDayAbbreviationFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"day-in-month", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateDayInMonthFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"day-in-week", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateDayInWeekFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"day-in-year", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateDayInYearFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"day-name", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateDayNameFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"day-of-week-in-month", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateDayOfWeekInMonthFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"difference", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateDifferenceFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"duration", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateDurationFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"hour-in-day", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateHourInDayFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"leap-year", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateLeapYearFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"minute-in-hour", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateMinuteInHourFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"month-abbreviation", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateMonthAbbreviationFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"month-in-year", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateMonthInYearFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"month-name", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateMonthNameFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"second-in-minute", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateSecondInMinuteFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"seconds", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateSecondsFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"sum", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateSumFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"time", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateTimeFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"week-in-month", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateWeekInMonthFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"week-in-year", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateWeekInYearFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"year", (const xmlChar*)EXSLT_DATE_NAMESPACE, exsltDateYearFunction)) {
        return 0;
    }
    return -1;
}
