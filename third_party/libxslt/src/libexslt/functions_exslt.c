...
static void exsltFuncFunctionFunction(xmlXPathParserContextPtr ctxt, int nargs);
 ...
 */
static void exsltFuncRegisterFunc(void* payload, void* vctxt, const xmlChar* URI, const xmlChar* name, ATTRIBUTE_UNUSED const xmlChar* ignored)
{
    ...
    xsltGenericDebug(xsltGenericDebugContext, "exsltFuncRegisterFunc: register {%s}%s\n", URI, name);
    xsltRegisterExtFunction(ctxt, name, URI, exsltFuncFunctionFunction);
}
 ...
 */
static void exsltFuncRegisterImportFunc(void* payload, void* vctxt, const xmlChar* URI, const xmlChar* name, ATTRIBUTE_UNUSED const xmlChar* ignored)
{
    exsltFuncFunctionData* data = (exsltFuncFunctionData*)payload;
    ...
    if (func == NULL) { /* Not yet present - copy it in */
        ...
        if (xmlHashAddEntry2(ch->hash, URI, name, func) < 0) {
            xsltGenericError(xsltGenericErrorContext, "Failed to register function {%s}%s\n", URI, name);
        } else { /* Do the registration */
            xsltGenericDebug(xsltGenericDebugContext, "exsltFuncRegisterImportFunc: register {%s}%s\n", URI, name);
            xsltRegisterExtFunction(ch->ctxt, name, URI, exsltFuncFunctionFunction);
        }
    }
}
 ...
 */
static void* exsltFuncInit(xsltTransformContextPtr ctxt, const xmlChar* URI)
{
    exsltFuncData* ret;
    ...
    if (ret == NULL) {
        xsltGenericError(xsltGenericErrorContext, "exsltFuncInit: not enough memory\n");
        return (NULL);
    }
 ...
 */
static void exsltFuncShutdown(xsltTransformContextPtr ctxt ATTRIBUTE_UNUSED, const xmlChar* URI ATTRIBUTE_UNUSED, void* vdata)
{
 ...
 */
static void* exsltFuncStyleInit(xsltStylesheetPtr style ATTRIBUTE_UNUSED, const xmlChar* URI ATTRIBUTE_UNUSED)
{
    return xmlHashCreate(1);
}

static void exsltFuncFreeDataEntry(void* payload, const xmlChar* name ATTRIBUTE_UNUSED)
{
    xmlFree(payload);
}
 ...
 * Called at compile-time.
 */
static void exsltFuncStyleShutdown(xsltStylesheetPtr style ATTRIBUTE_UNUSED, const xmlChar* URI ATTRIBUTE_UNUSED, void* vdata)
{
...
static exsltFuncFunctionData* exsltFuncNewFunctionData(void)
{
    ...
    if (ret == NULL) {
        xsltGenericError(xsltGenericErrorContext, "exsltFuncNewFunctionData: not enough memory\n");
        return (NULL);
    }
 ...
 */
static void exsltFreeFuncResultPreComp(xsltElemPreCompPtr ecomp)
{
...
static void exsltFuncFunctionFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
     ...
     */
    data = (exsltFuncData*)xsltGetExtData(tctxt, EXSLT_FUNCTIONS_NAMESPACE);
    oldResult = data->result;
    data->result = NULL;

    func = (exsltFuncFunctionData*)xmlHashLookup2(data->funcs, ctxt->context->functionURI, ctxt->context->function);
    if (func == NULL) {
        /* Should never happen */
        xsltGenericError(xsltGenericErrorContext, "{%s}%s: not found\n", ctxt->context->functionURI, ctxt->context->function);
     ...
     */
    if (nargs > func->nargs) {
        xsltGenericError(xsltGenericErrorContext, "{%s}%s: called with too many arguments\n", ctxt->context->functionURI, ctxt->context->function);
    ...
    */
     ...
     */
    fake = xmlNewDocNode(tctxt->output, NULL, (const xmlChar*)"fake", NULL);
    oldInsert = tctxt->insert;
    oldCtxtVar = data->ctxtVar;
    data->ctxtVar = tctxt->contextVariable;
    tctxt->insert = fake;
    tctxt->contextVariable = NULL;
    xsltApplyOneTemplate(tctxt, tctxt->node, func->content, NULL, NULL);
...
static void exsltFuncFunctionComp(xsltStylesheetPtr style, xmlNodePtr inst)
{
    xmlChar *name, *prefix;
    xmlNsPtr ns;
    ...
    if ((name == NULL) || (prefix == NULL)) {
        xsltGenericError(xsltGenericErrorContext, "func:function: not a QName\n");
        if (name != NULL)
            xmlFree(name);
        return;
    }
    ...
    if (ns == NULL) {
        xsltGenericError(xsltGenericErrorContext, "func:function: undeclared prefix %s\n", prefix);
        xmlFree(name);
        xmlFree(prefix);
        return;
    }
    ...
    while (IS_XSLT_ELEM(func->content) && IS_XSLT_NAME(func->content, "param")) {
    ...
    * import precedence.
    */
    data = (xmlHashTablePtr)xsltStyleStylesheetLevelGetExtData(style, EXSLT_FUNCTIONS_NAMESPACE);
#else
    data = (xmlHashTablePtr)xsltStyleGetExtData(style, EXSLT_FUNCTIONS_NAMESPACE);
#endif
    if (data == NULL) {
        xsltGenericError(xsltGenericErrorContext, "exsltFuncFunctionComp: no stylesheet data\n");
        xmlFree(name);
        xmlFree(func);
        return;
    }
    ...
    if (xmlHashAddEntry2(data, ns->href, name, func) < 0) {
        ...
        xsltGenericDebug(xsltGenericDebugContext, "exsltFuncFunctionComp: register {%s}%s\n", ns->href, name);
    }
    xmlFree(name);
}

static xsltElemPreCompPtr exsltFuncResultComp(xsltStylesheetPtr style, xmlNodePtr inst, xsltTransformFunction function)
{
    xmlNodePtr test;
    xmlChar* sel;
    ...
    for (test = inst->parent; test != NULL; test = test->parent) {
        if (IS_XSLT_ELEM(test) && IS_XSLT_NAME(test, "stylesheet")) {
            xsltGenericError(xsltGenericErrorContext,
        ...
        if ((test->ns != NULL) && (xmlStrEqual(test->ns->href, EXSLT_FUNCTIONS_NAMESPACE))) {
        ...
        if (IS_XSLT_ELEM(test) && (IS_XSLT_NAME(test, "variable") || IS_XSLT_NAME(test, "param"))) {
            xsltGenericError(xsltGenericErrorContext,
                "func:result element not allowed within"
     ...
     * Precomputation
     */
    ret = (exsltFuncResultPreComp*)xmlMalloc(sizeof(exsltFuncResultPreComp));
    if (ret == NULL) {
        xsltPrintErrorContext(NULL, NULL, NULL);
        xsltGenericError(xsltGenericErrorContext, "exsltFuncResultComp : malloc failed\n");
        style->errors++;
        return (NULL);
    }
    memset(ret, 0, sizeof(exsltFuncResultPreComp));

    xsltInitElemPreComp((xsltElemPreCompPtr)ret, style, inst, function, exsltFreeFuncResultPreComp);
    ret->select = NULL;
...
static void exsltFuncResultElem(xsltTransformContextPtr ctxt, xmlNodePtr node ATTRIBUTE_UNUSED, xmlNodePtr inst, xsltElemPreCompPtr ecomp)
{
    exsltFuncResultPreComp* comp = (exsltFuncResultPreComp*)ecomp;
    exsltFuncData* data;
    ...
    if (data == NULL) {
        xsltGenericError(xsltGenericErrorContext, "exsltFuncReturnElem: data == NULL\n");
        return;
    }
    if (data->result != NULL) {
        xsltGenericError(xsltGenericErrorContext, "func:result already instanciated\n");
        data->error = 1;
        return;
    }
    /*
	 ...
	 */
        ...
        if (ret == NULL) {
            xsltGenericError(xsltGenericErrorContext, "exsltFuncResultElem: ret == NULL\n");
            return;
        }
        /*
...
{
    xsltRegisterExtModuleFull(EXSLT_FUNCTIONS_NAMESPACE, exsltFuncInit, exsltFuncShutdown, exsltFuncStyleInit, exsltFuncStyleShutdown);

    xsltRegisterExtModuleTopLevel((const xmlChar*)"function", EXSLT_FUNCTIONS_NAMESPACE, exsltFuncFunctionComp);
    xsltRegisterExtModuleElement((const xmlChar*)"result", EXSLT_FUNCTIONS_NAMESPACE, exsltFuncResultComp, exsltFuncResultElem);
}
