 ...
 */
static void* exsltSaxonInit(xsltTransformContextPtr ctxt ATTRIBUTE_UNUSED, const xmlChar* URI ATTRIBUTE_UNUSED)
{
    return xmlHashCreate(1);
}

static void exsltSaxonFreeCompExprEntry(void* payload, const xmlChar* name ATTRIBUTE_UNUSED)
{
 ...
 */
static void exsltSaxonShutdown(xsltTransformContextPtr ctxt ATTRIBUTE_UNUSED, const xmlChar* URI ATTRIBUTE_UNUSED, void* vdata)
{
...
static void exsltSaxonSystemIdFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    ...
    if ((ctxt->context) && (ctxt->context->doc) && (ctxt->context->doc->URL))
...
{
    xsltRegisterExtModule(SAXON_NAMESPACE, exsltSaxonInit, exsltSaxonShutdown);
