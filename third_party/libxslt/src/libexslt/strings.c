 ...
 */
static void exsltStrTokenizeFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    ...
    if (tctxt == NULL) {
        xsltTransformError(xsltXPathGetTransformContext(ctxt), NULL, NULL, "exslt:tokenize : internal error tctxt == NULL\n");
        goto fail;
    }
        ...
        if (ret != NULL) {
            for (cur = str, token = str; *cur != 0; cur += clen) {
                ...
                if (*delimiters == 0) { /* empty string case */
                    ...
                    ctmp = *(cur + clen);
                    *(cur + clen) = 0;
                    node = xmlNewDocRawNode(container, NULL, (const xmlChar*)"token", cur);
                    xmlAddChild((xmlNodePtr)container, node);
                    xmlXPathNodeSetAddUnique(ret->nodesetval, node);
                    *(cur + clen) = ctmp; /* restore the changed byte */
                    token = cur + clen;
                } else
                    for (delimiter = delimiters; *delimiter != 0; delimiter += xmlUTF8Strsize(delimiter, 1)) {
                        if (!xmlUTF8Charcmp(cur, delimiter)) {
                            ...
                            *cur = 0; /* terminate the token */
                            node = xmlNewDocRawNode(container, NULL, (const xmlChar*)"token", token);
                            xmlAddChild((xmlNodePtr)container, node);
                            xmlXPathNodeSetAddUnique(ret->nodesetval, node);
 ...
 */
static void exsltStrSplitFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    xsltTransformContextPtr tctxt;
 ...
 */
static void exsltStrEncodeUriFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    ...
    if (str_len <= 0) {
        if (str_len < 0)
            xsltGenericError(xsltGenericErrorContext, "exsltStrEncodeUriFunction: invalid UTF-8\n");
        xmlXPathReturnEmptyString(ctxt);
        xmlFree(str);
        return;
    }
 ...
 */
static void exsltStrDecodeUriFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    int str_len = 0;
    xmlChar *str = NULL, *ret = NULL, *tmp;
    ...
    if (str_len <= 0) {
        if (str_len < 0)
            xsltGenericError(xsltGenericErrorContext, "exsltStrDecodeUriFunction: invalid UTF-8\n");
        xmlXPathReturnEmptyString(ctxt);
        xmlFree(str);
        return;
    }
 ...
 */
static void exsltStrPaddingFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    ...
    if (str_len <= 0) {
        if (str_len < 0) {
            xsltGenericError(xsltGenericErrorContext, "exsltStrPaddingFunction: invalid UTF-8\n");
            xmlXPathReturnEmptyString(ctxt);
            xmlFree(str);
            return;
        }
 ...
 */
static void exsltStrAlignFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    ...
    if (str_l < 0 || padding_l < 0) {
        xsltGenericError(xsltGenericErrorContext, "exsltStrAlignFunction: invalid UTF-8\n");
        xmlXPathReturnEmptyString(ctxt);
        xmlFree(str);
        xmlFree(padding);
        xmlFree(alignment);
 ...
 */
static void exsltStrConcatFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
 ...
 */
static int exsltStrReturnString(xmlXPathParserContextPtr ctxt, const xmlChar* str, int len)
{
 ...
 */
static void exsltStrReplaceFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    ...
    while (*src != 0) {
        ...
        for (i = 0; i < n; ++i) {
            if (*src == search[i][0] && slen[i] > max_len && xmlStrncmp(src, search[i], slen[i]) == 0) {
                i_match = i;
                max_len = slen[i];
            }
        }
        ...
        if (max_len == 0) {
            if (i_empty >= 0 && start < src) {
                if (xmlBufferAdd(buf, start, src - start) || xmlBufferAdd(buf, replace[i_empty], rlen[i_empty])) {
                    xmlXPathSetError(ctxt, XPATH_MEMORY_ERROR);
            ...
            if ((start < src && xmlBufferAdd(buf, start, src - start)) || (rlen[i_match] && xmlBufferAdd(buf, replace[i_match], rlen[i_match]))) {
                xmlXPathSetError(ctxt, XPATH_MEMORY_ERROR);
                goto fail_buffer_add;
            }
...
{
    xsltRegisterExtModuleFunction((const xmlChar*)"tokenize", EXSLT_STRINGS_NAMESPACE, exsltStrTokenizeFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"split", EXSLT_STRINGS_NAMESPACE, exsltStrSplitFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"encode-uri", EXSLT_STRINGS_NAMESPACE, exsltStrEncodeUriFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"decode-uri", EXSLT_STRINGS_NAMESPACE, exsltStrDecodeUriFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"padding", EXSLT_STRINGS_NAMESPACE, exsltStrPaddingFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"align", EXSLT_STRINGS_NAMESPACE, exsltStrAlignFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"concat", EXSLT_STRINGS_NAMESPACE, exsltStrConcatFunction);
    xsltRegisterExtModuleFunction((const xmlChar*)"replace", EXSLT_STRINGS_NAMESPACE, exsltStrReplaceFunction);
}
...
{
    if (ctxt && prefix && !xmlXPathRegisterNs(ctxt, prefix, (const xmlChar*)EXSLT_STRINGS_NAMESPACE)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"encode-uri", (const xmlChar*)EXSLT_STRINGS_NAMESPACE, exsltStrEncodeUriFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"decode-uri", (const xmlChar*)EXSLT_STRINGS_NAMESPACE, exsltStrDecodeUriFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"padding", (const xmlChar*)EXSLT_STRINGS_NAMESPACE, exsltStrPaddingFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"align", (const xmlChar*)EXSLT_STRINGS_NAMESPACE, exsltStrAlignFunction)
        && !xmlXPathRegisterFuncNS(ctxt, (const xmlChar*)"concat", (const xmlChar*)EXSLT_STRINGS_NAMESPACE, exsltStrConcatFunction)) {
        return 0;
    }
    return -1;
}
