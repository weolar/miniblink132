 ...
 */
    png_app_warning(png_ptr, "png_process_data_skip is not implemented in any current version of libpng");
    return 0;
}
...
png_push_save_buffer(png_structrp png_ptr)
{
    ...
    if (png_ptr->save_buffer_size + png_ptr->current_buffer_size > png_ptr->save_buffer_max) {
        ...
        png_ptr->save_buffer = (png_bytep)png_malloc_warn(png_ptr, (size_t)new_max);
    ...
    if (png_ptr->current_buffer_size) {
        memcpy(png_ptr->save_buffer + png_ptr->save_buffer_size, png_ptr->current_buffer_ptr, png_ptr->current_buffer_size);
...
void /* PRIVATE */
png_push_restore_buffer(png_structrp png_ptr, png_bytep buffer, size_t buffer_length)
{
...
void /* PRIVATE */
png_process_IDAT_data(png_structrp png_ptr, png_bytep buffer, size_t buffer_length)
{
...
png_read_push_finish_row(png_structrp png_ptr)
{
   ...
   */
    ...
    if (png_ptr->interlaced != 0) {
        ...
        do {
            png_ptr->pass++;
            if ((png_ptr->pass == 1 && png_ptr->width < 5) || (png_ptr->pass == 3 && png_ptr->width < 3) || (png_ptr->pass == 5 && png_ptr->width < 2))
                png_ptr->pass++;
