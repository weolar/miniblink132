 ...
 */
    ...
    defined PNG_READ_bKGD_SUPPORTED && defined PNG_READ_cHRM_SUPPORTED && defined PNG_READ_gAMA_SUPPORTED && defined PNG_READ_oFFs_SUPPORTED                   \
    && defined PNG_READ_pCAL_SUPPORTED && defined PNG_READ_pHYs_SUPPORTED && defined PNG_READ_sBIT_SUPPORTED && defined PNG_READ_sCAL_SUPPORTED                \
    && defined PNG_READ_sRGB_SUPPORTED && defined PNG_READ_sPLT_SUPPORTED && defined PNG_READ_tEXt_SUPPORTED && defined PNG_READ_tIME_SUPPORTED                \
...
void pngtest_check_io_state(png_structp png_ptr, size_t data_length, png_uint_32 io_op);
void pngtest_check_io_state(png_structp png_ptr, size_t data_length, png_uint_32 io_op)
{
...
static void PNGCBAPI pngtest_read_data(png_structp png_ptr, png_bytep data, size_t length)
{
    size_t check = 0;
    png_voidp io_ptr;
 ...
 * than changing the library.
 */
static void PNGCBAPI pngtest_write_data(png_structp png_ptr, png_bytep data, size_t length)
{
    size_t check;
...
png_voidp PNGCBAPI png_debug_malloc(png_structp png_ptr, png_alloc_size_t size)
{
    ...
    */
      ...
      buffer and once to get a new free list entry. */
    {
        ...
        if (verbose != 0)
            printf("png_malloc %lu bytes at %p\n", (unsigned long)size, pinfo->pointer);
...
static void write_vpAg_chunk(png_structp write_ptr)
{
    ...
    if (verbose != 0)
        fprintf(STDERR, " vpAg = %lu x %lu, units = %d\n", (unsigned long)user_chunk_data.vpAg_width, (unsigned long)user_chunk_data.vpAg_height,
    ...
    */
...
#ifdef PNG_READ_eXIf_SUPPORTED
    {
        ...
        if (png_get_eXIf_1(read_ptr, read_info_ptr, &exif_length, &exif) != 0) {
            if (exif_length > 1)
                fprintf(STDERR, " eXIf type %c%c, %lu bytes\n", exif[0], exif[1], (unsigned long)exif_length);
        ...
        if (png_get_tRNS(read_ptr, read_info_ptr, &trans_alpha, &num_trans, &trans_color) != 0) {
            ...
            if (!((color_type == PNG_COLOR_TYPE_GRAY && (int)trans_color->gray > sample_max)
                    || (color_type == PNG_COLOR_TYPE_RGB
                        && ((int)trans_color->red > sample_max || (int)trans_color->green > sample_max || (int)trans_color->blue > sample_max))))
...
#define pass_height height
        ...
        for (y = 0; y < pass_height; y++) {
            ...
            pngtest_debug2("\t%p (%lu bytes)", row_buf, (unsigned long)png_get_rowbytes(read_ptr, read_info_ptr));
...
int main(int argc, char* argv[])
{
    ...
    fprintf(STDERR, " library (%lu):%s", (unsigned long)png_access_version_number(), png_get_header_version(NULL));

    /* Show the version of libpng used in building the application */
    fprintf(STDERR, " pngtest (%lu):%s", (unsigned long)PNG_LIBPNG_VER, PNG_HEADER_VERSION_STRING);
