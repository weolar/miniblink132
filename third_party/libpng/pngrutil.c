...
png_crc_finish(png_structrp png_ptr, png_uint_32 skip)
{
    ...
    */
    ...
    if (png_crc_error(png_ptr) != 0) {
        if (PNG_CHUNK_ANCILLARY(png_ptr->chunk_name) != 0 ? (png_ptr->flags & PNG_FLAG_CRC_ANCILLARY_NOWARN) == 0
...
#if defined(PNG_READ_iCCP_SUPPORTED) || defined(PNG_READ_iTXt_SUPPORTED) || defined(PNG_READ_pCAL_SUPPORTED) || defined(PNG_READ_sCAL_SUPPORTED)               \
    || defined(PNG_READ_sPLT_SUPPORTED) || defined(PNG_READ_tEXt_SUPPORTED) || defined(PNG_READ_zTXt_SUPPORTED) || defined(PNG_SEQUENTIAL_READ_SUPPORTED)
    ...
    */
    {
        ...
        if (((png_ptr->options >> PNG_MAXIMUM_INFLATE_WINDOW) & 3) == PNG_OPTION_ON) {
            window_bits = 15;
...
#if ZLIB_VERNUM >= 0x1290 && defined(PNG_SET_OPTION_SUPPORTED) && defined(PNG_IGNORE_ADLER32)
 ...
 */
static int png_inflate(png_structrp png_ptr, png_uint_32 owner, int finish,
    ...
    */
    ...
    if (limit >= prefix_size + (terminate != 0)) {
        ...
        if (ret == Z_OK) {
             ...
             */
                ...
                */
                    ...
                    if (text != NULL) {
                        ...
                        ret = png_inflate(
                            png_ptr, png_ptr->chunk_name, 1 /*finish*/, png_ptr->read_buffer + prefix_size, &lzsize, text + prefix_size, newlength);
            ...
            if (ret == Z_STREAM_END) /* impossible! */
                ret = PNG_UNEXPECTED_ZLIB_RETURN;
 ...
 */
static int png_inflate_read(
    png_structrp png_ptr, png_bytep read_buffer, uInt read_size, png_uint_32p chunk_bytes, png_bytep next_out, png_alloc_size_t* out_size, int finish)
{
...
png_handle_PLTE(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length)
{
    ...
    */
    ...
    if (png_ptr->num_trans > 0 || (info_ptr != NULL && (info_ptr->valid & PNG_INFO_tRNS) != 0)) {
...
png_handle_IEND(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length)
{
    ...
    if ((png_ptr->mode & PNG_HAVE_IHDR) == 0 || (png_ptr->mode & PNG_HAVE_IDAT) == 0)
        png_chunk_error(png_ptr, "out of place");
...
png_handle_cHRM(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length)
{
    ...
    if (xy.whitex == PNG_FIXED_ERROR || xy.whitey == PNG_FIXED_ERROR || xy.redx == PNG_FIXED_ERROR || xy.redy == PNG_FIXED_ERROR || xy.greenx == PNG_FIXED_ERROR
        || xy.greeny == PNG_FIXED_ERROR || xy.bluex == PNG_FIXED_ERROR || xy.bluey == PNG_FIXED_ERROR) {
            ...
            if (keyword_length + 1 < read_length && keyword[keyword_length + 1] == PNG_COMPRESSION_TYPE_BASE) {
                ...
                if (png_inflate_claim(png_ptr, png_iCCP) == Z_OK) {
                    ...
                    (void)png_inflate_read(
                        png_ptr, local_buffer, (sizeof local_buffer), &length, profile_header, &size, 0 /*finish: don't, because the output is too small*/);
                   ...
                   */
                        ...
                        if (png_icc_check_length(png_ptr, &png_ptr->colorspace, keyword, profile_length) != 0) {
                            /* The length is apparently ok, so we can check the 132
                      * byte header.
                      */
                            if (png_icc_check_header(png_ptr, &png_ptr->colorspace, keyword, profile_length, profile_header, png_ptr->color_type) != 0) {
                                /* Now read the tag table; a variable size buffer is
                         * needed at this point, allocate one for the whole
                         * profile.  The header check has already validated
                         * that none of this stuff will overflow.
                         */
                                png_uint_32 tag_count = png_get_uint_32(profile_header + 128);
                            ...
                            */
                                  ...
                                  */
                                            ...
                                            (void)png_inflate_read(png_ptr, local_buffer, (sizeof local_buffer), &length,
                                        ...
                                        */
                                                ...
                                                png_icc_set_sRGB(png_ptr, &png_ptr->colorspace, profile, png_ptr->zstream.adler);
#endif
...
/* Note: this does not properly handle chunks that are > 64K under DOS */
{
    ...
    */
    ...
    new_palette.entries = (png_sPLT_entryp)png_malloc_warn(png_ptr, (png_alloc_size_t)new_palette.nentries * (sizeof(png_sPLT_entry)));
...
png_handle_tRNS(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length)
{
    ...
    if (png_ptr->color_type == PNG_COLOR_TYPE_GRAY) {
        ...
        if (length > (unsigned int)png_ptr->num_palette || length > (unsigned int)PNG_MAX_PALETTE_LENGTH || length == 0) {
            png_crc_finish(png_ptr, length);
            png_chunk_benign_error(png_ptr, "invalid");
            return;
        }
...
png_handle_bKGD(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length)
{
    ...
    else if ((png_ptr->mode & PNG_HAVE_IDAT) != 0 || (png_ptr->color_type == PNG_COLOR_TYPE_PALETTE && (png_ptr->mode & PNG_HAVE_PLTE) == 0)) {
        png_crc_finish(png_ptr, length);
        png_chunk_benign_error(png_ptr, "out of place");
        return;
    }
...
png_handle_eXIf(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length)
{
    ...
    info_ptr->eXIf_buf = png_voidcast(png_bytep, png_malloc_warn(png_ptr, length));
...
png_handle_hIST(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length)
{
    ...
    else if ((png_ptr->mode & PNG_HAVE_IDAT) != 0 || (png_ptr->mode & PNG_HAVE_PLTE) == 0) {
        png_crc_finish(png_ptr, length);
        png_chunk_benign_error(png_ptr, "out of place");
        return;
    }
...
png_handle_pCAL(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length)
{
    ...
    * equation types.
    */
    if ((type == PNG_EQUATION_LINEAR && nparams != 2) || (type == PNG_EQUATION_BASE_E && nparams != 3) || (type == PNG_EQUATION_ARBITRARY && nparams != 3)
    ...
    params = png_voidcast(png_charpp, png_malloc_warn(png_ptr, nparams * (sizeof(png_charp))));
    ...
    png_set_pCAL(png_ptr, info_ptr, (png_charp)buffer, X0, X1, type, nparams, (png_charp)units, params);
...
png_handle_sCAL(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length)
{
    ...
    */
    ...
    if (png_check_fp_number((png_const_charp)buffer, length, &state, &i) == 0 || i >= length || buffer[i++] != 0)
        ...
        state = 0;
        if (png_check_fp_number((png_const_charp)buffer, length, &state, &i) == 0 || i != length)
            ...
            png_set_sCAL_s(png_ptr, info_ptr, buffer[0], (png_charp)buffer + 1, (png_charp)buffer + heighti);
    }
}
#endif
...
png_handle_zTXt(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length)
{
    ...
    */
       ...
       * and text chunks.
       */
        if (png_decompress_chunk(png_ptr, length, keyword_length + 2, &uncompressed_length, 1 /*terminate*/) == Z_STREAM_END) {
            png_text text;
...
static int png_cache_unknown_chunk(png_structrp png_ptr, png_uint_32 length)
{
    ...
    if (length <= limit) {
        ...
        if (length == 0)
            ...
            png_ptr->unknown_chunk.data = png_voidcast(png_bytep, png_malloc_warn(png_ptr, length));
        }
    }
...
void /* PRIVATE */
png_handle_unknown(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length, int keep)
{
    ...
    */
...
#ifdef PNG_SAVE_UNKNOWN_CHUNKS_SUPPORTED
    {
       ...
       */
        ...
        if (keep == PNG_HANDLE_CHUNK_ALWAYS || (keep == PNG_HANDLE_CHUNK_IF_SAFE && PNG_CHUNK_ANCILLARY(png_ptr->chunk_name))) {
            if (png_cache_unknown_chunk(png_ptr, length) == 0)
                keep = PNG_HANDLE_CHUNK_NEVER;
        }
...
png_check_chunk_length(png_const_structrp png_ptr, png_uint_32 length)
{
    ...
    if (length > limit) {
        png_debug2(0, " length = %lu, limit = %lu", (unsigned long)length, (unsigned long)limit);
...
png_combine_row(png_const_structrp png_ptr, png_bytep dp, int display)
{
    ...
    */
    ...
    if (png_ptr->interlaced != 0 && (png_ptr->transformations & PNG_INTERLACE) != 0 && pass < 6
        && (display == 0 ||
...
#define S_MASK(p, d, s)                                                                                                                                        \
    MASK_EXPAND(S_MASKx(p, 0, d, s) + S_MASKx(p, 1, d, s) + S_MASKx(p, 2, d, s) + S_MASKx(p, 3, d, s) + S_MASKx(p, 4, d, s) + S_MASKx(p, 5, d, s)              \
            + S_MASKx(p, 6, d, s) + S_MASKx(p, 7, d, s),                                                                                                       \
        d)

#define B_MASK(p, d, s)                                                                                                                                        \
    MASK_EXPAND(B_MASKx(p, 0, d, s) + B_MASKx(p, 1, d, s) + B_MASKx(p, 2, d, s) + B_MASKx(p, 3, d, s) + B_MASKx(p, 4, d, s) + B_MASKx(p, 5, d, s)              \
...
#define S_MASKS(d, s)                                                                                                                                          \
    {                                                                                                                                                          \
        S_MASK(0, d, s), S_MASK(1, d, s), S_MASK(2, d, s), S_MASK(3, d, s), S_MASK(4, d, s), S_MASK(5, d, s)                                                   \
    }
...
#define DEPTH_INDEX(d) ((d) == 1 ? 0 : ((d) == 2 ? 1 : 2))
          ...
          * then pass:
          */
            static const png_uint_32 row_mask[2 /*PACKSWAP*/][3 /*depth*/][6] = { /* Little-endian byte masks for PACKSWAP */
          ...
          * pass>>1.
          */
            static const png_uint_32 display_mask[2][3][3] = { /* Little-endian byte masks for PACKSWAP */
        ...
        else /* pixel_depth >= 8 */
        {
          ...
          */
            ...
            default:
                ...
                */
                if (bytes_to_copy < 16 /*else use memcpy*/ && png_isaligned(dp, png_uint_16) && png_isaligned(sp, png_uint_16)
                    && bytes_to_copy % (sizeof(png_uint_16)) == 0 && bytes_to_jump % (sizeof(png_uint_16)) == 0) {
                    /* Everything is aligned for png_uint_16 copies, but try for
                   * png_uint_32 first.
                   */
                    if (png_isaligned(dp, png_uint_32) && png_isaligned(sp, png_uint_32) && bytes_to_copy % (sizeof(png_uint_32)) == 0
                        && bytes_to_jump % (sizeof(png_uint_32)) == 0) {
                        png_uint_32p dp32 = png_aligncast(png_uint_32p, dp);
                        png_const_uint_32p sp32 = png_aligncastconst(png_const_uint_32p, sp);
                   ...
                   */
                        ...
                        png_const_uint_16p sp16 = png_aligncastconst(png_const_uint_16p, sp);
...
#ifdef PNG_READ_INTERLACING_SUPPORTED
void /* PRIVATE */
png_do_read_interlace(png_row_infop row_info, png_bytep row, int pass, png_uint_32 transformations /* Because these may affect the byte layout */)
{
...
void /* PRIVATE */
png_read_filter_row(png_structrp pp, png_row_infop row_info, png_bytep row, png_const_bytep prev_row, int filter)
{
...
png_read_start_row(png_structrp png_ptr)
{
    ...
    */
    ...
    if ((png_ptr->transformations & PNG_USER_TRANSFORM) != 0) {
        unsigned int user_pixel_depth = png_ptr->user_transform_depth * png_ptr->user_transform_channels;
