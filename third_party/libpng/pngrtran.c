...
void PNGFAPI png_set_background_fixed(
    png_structrp png_ptr, png_const_color_16p background_color, int background_gamma_code, int need_expand, png_fixed_point background_gamma)
{
...
{
    png_set_background_fixed(png_ptr, background_color, background_gamma_code, need_expand, png_fixed(png_ptr, background_gamma, "png_set_background"));
}
...
void PNGAPI png_set_quantize(png_structrp png_ptr, png_colorp palette, int num_palette, int maximum_colors, png_const_uint_16p histogram, int full_quantize)
{
    ...
    if (full_quantize == 0) {
        ...
        png_ptr->quantize_index = (png_bytep)png_malloc(png_ptr, (png_alloc_size_t)((png_uint_32)num_palette * (sizeof(png_byte))));
    ...
    if (num_palette > maximum_colors) {
        if (histogram != NULL) {
          ...
          */
            ...
            png_ptr->quantize_sort = (png_bytep)png_malloc(png_ptr, (png_alloc_size_t)((png_uint_32)num_palette * (sizeof(png_byte))));
          ...
          */
            ...
            for (i = num_palette - 1; i >= maximum_colors; i--) {
                ...
                for (j = 0; j < i; j++) {
                    if (histogram[png_ptr->quantize_sort[j]] < histogram[png_ptr->quantize_sort[j + 1]]) {
                        png_byte t;
          ...
          */
            ...
            png_ptr->index_to_palette = (png_bytep)png_malloc(png_ptr, (png_alloc_size_t)((png_uint_32)num_palette * (sizeof(png_byte))));
            png_ptr->palette_to_index = (png_bytep)png_malloc(png_ptr, (png_alloc_size_t)((png_uint_32)num_palette * (sizeof(png_byte))));
            ...
            hash = (png_dsortpp)png_calloc(png_ptr, (png_alloc_size_t)(769 * (sizeof(png_dsortp))));
                ...
                for (i = 0; i < num_new_palette - 1; i++) {
                    ...
                    for (j = i + 1; j < num_new_palette; j++) {
                        ...
                        if (d <= max_d) {
                            ...
                            t = (png_dsortp)png_malloc_warn(png_ptr, (png_alloc_size_t)(sizeof(png_dsort)));
                                ...
                                if ((int)png_ptr->index_to_palette[p->left] < num_new_palette && (int)png_ptr->index_to_palette[p->right] < num_new_palette) {
                                    ...
                                    if (full_quantize == 0) {
                                        ...
                                        for (k = 0; k < num_palette; k++) {
                                            if (png_ptr->quantize_index[k] == png_ptr->index_to_palette[j])
                                                png_ptr->quantize_index[k] = png_ptr->index_to_palette[next_j];

                                            if ((int)png_ptr->quantize_index[k] == num_new_palette)
                                                png_ptr->quantize_index[k] = png_ptr->index_to_palette[j];
                                        }
                                    }
    ...
    if (full_quantize != 0) {
        ...
        png_ptr->palette_lookup = (png_bytep)png_calloc(png_ptr, (png_alloc_size_t)(num_entries * (sizeof(png_byte))));

        distance = (png_bytep)png_malloc(png_ptr, (png_alloc_size_t)(num_entries * (sizeof(png_byte))));
...
{
    png_set_gamma_fixed(png_ptr, convert_gamma_value(png_ptr, scrn_gamma), convert_gamma_value(png_ptr, file_gamma));
}
#endif /* FLOATING_POINT */
#endif /* READ_GAMMA */
...
void PNGAPI png_set_rgb_to_gray(png_structrp png_ptr, int error_action, double red, double green)
{
    png_set_rgb_to_gray_fixed(
        png_ptr, error_action, png_fixed(png_ptr, red, "rgb to gray red coefficient"), png_fixed(png_ptr, green, "rgb to gray green coefficient"));
}
...
png_init_palette_transformations(png_structrp png_ptr)
{
    ...
    */
    if ((png_ptr->transformations & PNG_BACKGROUND_EXPAND) != 0 && (png_ptr->transformations & PNG_EXPAND) != 0) {
        {
    ...
    {
        {
            ...
            png_ptr->background.red = png_ptr->background.green = png_ptr->background.blue = (png_uint_16)gray;

            if ((png_ptr->transformations & PNG_EXPAND_tRNS) == 0) {
                png_ptr->trans_color.red = png_ptr->trans_color.green = png_ptr->trans_color.blue = (png_uint_16)trans_gray;
            }
        }
    } /* background expand and (therefore) no alpha association. */
#endif /* READ_EXPAND && READ_BACKGROUND */
...
png_init_read_transformations(png_structrp png_ptr)
{
    ...
    */
#ifdef PNG_READ_STRIP_ALPHA_SUPPORTED
    if ((png_ptr->transformations & PNG_STRIP_ALPHA) != 0 && (png_ptr->transformations & PNG_COMPOSE) == 0) {
...
#if defined(PNG_READ_BACKGROUND_SUPPORTED) && defined(PNG_READ_EXPAND_16_SUPPORTED)
    if ((png_ptr->transformations & PNG_EXPAND_16) != 0 && (png_ptr->transformations & PNG_COMPOSE) != 0
        && (png_ptr->transformations & PNG_BACKGROUND_EXPAND) == 0 && png_ptr->bit_depth != 16) {
...
#if defined(PNG_READ_BACKGROUND_SUPPORTED) && (defined(PNG_READ_SCALE_16_TO_8_SUPPORTED) || defined(PNG_READ_STRIP_16_TO_8_SUPPORTED))
    if ((png_ptr->transformations & (PNG_16_TO_8 | PNG_SCALE_16_TO_8)) != 0 && (png_ptr->transformations & PNG_COMPOSE) != 0
        && (png_ptr->transformations & PNG_BACKGROUND_EXPAND) == 0 && png_ptr->bit_depth == 16) {
       ...
       */
        ...
        png_ptr->background.green = (png_uint_16)(png_ptr->background.green * 257);
    ...
    */
    if ((png_ptr->transformations & PNG_GAMMA) != 0
        || ((png_ptr->transformations & PNG_RGB_TO_GRAY) != 0
            && (png_gamma_significant(png_ptr->colorspace.gamma) != 0 || png_gamma_significant(png_ptr->screen_gamma) != 0))
        || ((png_ptr->transformations & PNG_COMPOSE) != 0
            && (png_gamma_significant(png_ptr->colorspace.gamma) != 0 || png_gamma_significant(png_ptr->screen_gamma) != 0
#ifdef PNG_READ_BACKGROUND_SUPPORTED
                || (png_ptr->background_gamma_type == PNG_BACKGROUND_GAMMA_UNIQUE && png_gamma_significant(png_ptr->background_gamma) != 0)
#endif
                    ))
        || ((png_ptr->transformations & PNG_ENCODE_ALPHA) != 0 && png_gamma_significant(png_ptr->screen_gamma) != 0)) {
             ...
             */
                ...
                if (png_ptr->background_gamma_type == PNG_BACKGROUND_GAMMA_FILE) {
                    ...
                    if (png_gamma_significant(gs) != 0) {
                        back.red = png_gamma_8bit_correct(png_ptr->background.red, gs);
                        back.green = png_gamma_8bit_correct(png_ptr->background.green, gs);
                        back.blue = png_gamma_8bit_correct(png_ptr->background.blue, gs);
                    }
                    ...
                    if (png_gamma_significant(g) != 0) {
                        ...
                        back_1.green = png_gamma_8bit_correct(png_ptr->background.green, g);
                        back_1.blue = png_gamma_8bit_correct(png_ptr->background.blue, g);
                    }
        ...
        if ((png_ptr->transformations & PNG_COMPOSE) != 0 && (png_ptr->color_type == PNG_COLOR_TYPE_PALETTE)) {
            int i;
    ...
    if ((png_ptr->transformations & PNG_SHIFT) != 0 && (png_ptr->transformations & PNG_EXPAND) == 0 && (png_ptr->color_type == PNG_COLOR_TYPE_PALETTE)) {
        int i;
...
png_read_transform_info(png_structrp png_ptr, png_inforp info_ptr)
{
    ...
    */
...
#ifdef PNG_READ_GRAY_TO_RGB_SUPPORTED
    if ((png_ptr->transformations & PNG_GRAY_TO_RGB) != 0)
        info_ptr->color_type = (png_byte)(info_ptr->color_type | PNG_COLOR_MASK_COLOR);
#endif
    ...
    if ((png_ptr->transformations & PNG_QUANTIZE) != 0) {
        if (((info_ptr->color_type == PNG_COLOR_TYPE_RGB) || (info_ptr->color_type == PNG_COLOR_TYPE_RGB_ALPHA)) && png_ptr->palette_lookup != 0
            && info_ptr->bit_depth == 8) {
...
#ifdef PNG_READ_EXPAND_16_SUPPORTED
    if ((png_ptr->transformations & PNG_EXPAND_16) != 0 && info_ptr->bit_depth == 8 && info_ptr->color_type != PNG_COLOR_TYPE_PALETTE) {
    ...
    if ((png_ptr->transformations & PNG_STRIP_ALPHA) != 0) {
        info_ptr->color_type = (png_byte)(info_ptr->color_type & ~PNG_COLOR_MASK_ALPHA);
        info_ptr->num_trans = 0;
    }
#endif
    ...
    if ((png_ptr->transformations & PNG_FILLER) != 0 && (info_ptr->color_type == PNG_COLOR_TYPE_RGB || info_ptr->color_type == PNG_COLOR_TYPE_GRAY)) {
        info_ptr->channels++;
...
#if defined(PNG_USER_TRANSFORM_PTR_SUPPORTED) && defined(PNG_READ_USER_TRANSFORM_SUPPORTED)
...
static void png_do_gray_to_rgb(png_row_infop row_info, png_bytep row)
{
    ...
    if (row_info->bit_depth >= 8 && (row_info->color_type & PNG_COLOR_MASK_COLOR) == 0) {
        if (row_info->color_type == PNG_COLOR_TYPE_GRAY) {
            if (row_info->bit_depth == 8) {
...
static int png_do_rgb_to_gray(png_structrp png_ptr, png_row_infop row_info, png_bytep row)
{
    ...
    if ((row_info->color_type & PNG_COLOR_MASK_PALETTE) == 0 && (row_info->color_type & PNG_COLOR_MASK_COLOR) != 0) {
        ...
        row_info->channels = (png_byte)(row_info->channels - 2);
        row_info->color_type = (png_byte)(row_info->color_type & ~PNG_COLOR_MASK_COLOR);
        row_info->pixel_depth = (png_byte)(row_info->channels * row_info->bit_depth);
        row_info->rowbytes = PNG_ROWBYTES(row_info->pixel_depth, row_width);
    }
    return rgb_error;
...
static void png_do_compose(png_row_infop row_info, png_bytep row, png_structrp png_ptr)
{
    ...
    case PNG_COLOR_TYPE_GRAY: {
        ...
        case 1: {
            ...
            shift = 7;
            for (i = 0; i < row_width; i++) {
                if ((png_uint_16)((*sp >> shift) & 0x01) == png_ptr->trans_color.gray) {
                    unsigned int tmp = *sp & (0x7f7f >> (7 - shift));
                    tmp |= (unsigned int)(png_ptr->background.gray << shift);
                ...
                for (i = 0; i < row_width; i++) {
                    if ((png_uint_16)((*sp >> shift) & 0x03) == png_ptr->trans_color.gray) {
                        ...
                        *sp = (png_byte)(tmp & 0xff);
                        ...
                        unsigned int g = (gamma_table[p | (p << 2) | (p << 4) | (p << 6)] >> 6) & 0x03;
                        unsigned int tmp = *sp & (0x3f3f >> (6 - shift));
        ...
        case 4: {
            ...
            if (gamma_table != NULL) {
                ...
                shift = 4;
                for (i = 0; i < row_width; i++) {
                    if ((png_uint_16)((*sp >> shift) & 0x0f) == png_ptr->trans_color.gray) {
                        ...
                        *sp = (png_byte)(tmp & 0xff);
                        ...
                        unsigned int g = (gamma_table[p | (p << 4)] >> 4) & 0x0f;
                        unsigned int tmp = *sp & (0x0f0f >> (4 - shift));
                        tmp |= (unsigned int)(g << shift);
                        *sp = (png_byte)(tmp & 0xff);
                    }
        ...
        if (row_info->bit_depth == 8) {
            ...
            if (gamma_table != NULL) {
                ...
                for (i = 0; i < row_width; i++, sp += 3) {
                    if (*sp == png_ptr->trans_color.red && *(sp + 1) == png_ptr->trans_color.green && *(sp + 2) == png_ptr->trans_color.blue) {
        ...
        } else /* if (row_info->bit_depth == 16) */
        {
            ...
            if (gamma_16 != NULL) {
                ...
                for (i = 0; i < row_width; i++, sp += 6) {
                    ...
                    if (r == png_ptr->trans_color.red && g == png_ptr->trans_color.green && b == png_ptr->trans_color.blue) {
                        /* Background is already in screen gamma */
                ...
                for (i = 0; i < row_width; i++, sp += 4) {
                    ...
                    if (a == 0) {
                        ...
                        *(sp + 2) = (png_byte)png_ptr->background.blue;
                        ...
                        png_composite(*(sp + 1), *(sp + 1), a, png_ptr->background.green);

                        png_composite(*(sp + 2), *(sp + 2), a, png_ptr->background.blue);
                    }
                }
            }
        } else /* if (row_info->bit_depth == 16) */
...
static void png_do_expand_palette(
    png_structrp png_ptr, png_row_infop row_info, png_bytep row, png_const_colorp palette, png_const_bytep trans_alpha, int num_trans)
{
    int shift, value;
    png_bytep sp, dp;
    png_uint_32 i;
        ...
        if (row_info->bit_depth == 8) {
            {
                if (num_trans > 0) {
                   ...
                   */
                        i = png_do_expand_palette_rgba8_neon(png_ptr, row_info, row, &sp, &dp);
                    }
#else
                    PNG_UNUSED(png_ptr)
#endif
                    ...
                    i = 0;
#ifdef PNG_ARM_NEON_INTRINSICS_AVAILABLE
                    i = png_do_expand_palette_rgb8_neon(png_ptr, row_info, row, &sp, &dp);
#else
                    PNG_UNUSED(png_ptr)
#endif
    ...
    if (row_info->color_type == PNG_COLOR_TYPE_GRAY) {
        ...
        if (trans_color != NULL) {
            if (row_info->bit_depth == 8) {
                ...
                for (i = 0; i < row_width; i++) {
                    if ((*(sp - 1) & 0xffU) == gray_high && (*(sp)&0xffU) == gray_low) {
                        *dp-- = 0;
                        *dp-- = 0;
                    }
        ...
        if (row_info->bit_depth == 8) {
            ...
            dp = row + ((size_t)row_width << 3) - 1;
            for (i = 0; i < row_width; i++) {
                if (*(sp - 5) == red_high && *(sp - 4) == red_low && *(sp - 3) == green_high && *(sp - 2) == green_low && *(sp - 1) == blue_high
                    && *(sp) == blue_low) {
                    *dp-- = 0;
                    *dp-- = 0;
                }
...
{
    if (row_info->bit_depth == 8 && row_info->color_type != PNG_COLOR_TYPE_PALETTE) {
...
static void png_do_quantize(png_row_infop row_info, png_bytep row, png_const_bytep palette_lookup, png_const_bytep quantize_lookup)
{
    ...
    if (row_info->bit_depth == 8) {
        ...
        else if (row_info->color_type == PNG_COLOR_TYPE_PALETTE && quantize_lookup) {
            sp = row;
...
png_do_read_transformations(png_structrp png_ptr, png_row_infop row_info)
{
    ...
    * demand, if necessary.
    */
    if ((png_ptr->flags & PNG_FLAG_DETECT_UNINITIALIZED) != 0 && (png_ptr->flags & PNG_FLAG_ROW_INIT) == 0) {
...
#endif
            png_do_expand_palette(png_ptr, row_info, png_ptr->row_buf + 1, png_ptr->palette, png_ptr->trans_alpha, png_ptr->num_trans);
        }
...
#ifdef PNG_READ_STRIP_ALPHA_SUPPORTED
    if ((png_ptr->transformations & PNG_STRIP_ALPHA) != 0 && (png_ptr->transformations & PNG_COMPOSE) == 0
        && (row_info->color_type == PNG_COLOR_TYPE_RGB_ALPHA || row_info->color_type == PNG_COLOR_TYPE_GRAY_ALPHA))
        png_do_strip_channel(row_info, png_ptr->row_buf + 1, 0 /* at_start == false, because SWAP_ALPHA happens later */);
#endif

#ifdef PNG_READ_RGB_TO_GRAY_SUPPORTED
    if ((png_ptr->transformations & PNG_RGB_TO_GRAY) != 0) {
        int rgb_error = png_do_rgb_to_gray(png_ptr, row_info, png_ptr->row_buf + 1);
 ...
 */
    ...
    */
    if ((png_ptr->transformations & PNG_GRAY_TO_RGB) != 0 && (png_ptr->mode & PNG_BACKGROUND_IS_GRAY) == 0)
    ...
    if ((png_ptr->transformations & PNG_GAMMA) != 0 &&
       ...
       */
        !((png_ptr->transformations & PNG_COMPOSE) != 0 && ((png_ptr->num_trans != 0) || (png_ptr->color_type & PNG_COLOR_MASK_ALPHA) != 0)) &&
#endif
...
#ifdef PNG_READ_ALPHA_MODE_SUPPORTED
    if ((png_ptr->transformations & PNG_ENCODE_ALPHA) != 0 && (row_info->color_type & PNG_COLOR_MASK_ALPHA) != 0)
    ...
    */
    ...
    if ((png_ptr->transformations & PNG_FILLER) != 0)
        png_do_read_filler(row_info, png_ptr->row_buf + 1, (png_uint_32)png_ptr->filler, png_ptr->flags);
#endif
