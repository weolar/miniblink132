...
std::string utf16ToChar(const WCHAR* lpszSrc, size_t size, unsigned int codepage)
{
    std::string sResult;
    if (lpszSrc != NULL) {
        int nUTF8Len = WideCharToMultiByte(codepage, 0, lpszSrc, size, NULL, 0, NULL, NULL);
        char* pUTF8 = new char[nUTF8Len + 1];
        if (pUTF8 != NULL) {
            ZeroMemory(pUTF8, nUTF8Len + 1);
            WideCharToMultiByte(codepage, 0, lpszSrc, size, pUTF8, nUTF8Len, NULL, NULL);
            sResult = pUTF8;
            delete[] pUTF8;
        }
    }
