...
template <typename CharT> size_t FindNextSlash(std::basic_string_view<CharT> spec, size_t begin_index)
{
...
template <typename CharT> void DoParseUNC(std::basic_string_view<CharT> url, size_t after_slashes, Parsed* parsed)
{
    ...
    if (next_slash < url_len) {
        ParsePathInternal(url.data(), MakeRange(next_slash, url_len), &parsed->path, &parsed->query, &parsed->ref);
    } else {
        parsed->path.reset();
    }
}
...
template <typename CharT> void DoParseLocalFile(std::basic_string_view<CharT> url, int path_begin, Parsed* parsed)
{
    parsed->host.reset();
    ParsePathInternal(url.data(), MakeRange(path_begin, base::checked_cast<int>(url.size())), &parsed->path, &parsed->query, &parsed->ref);
}
...
template <typename CharT> Parsed DoParseFileURL(std::basic_string_view<CharT> url)
{
...
#endif
    {
        ...
        // the foo.c: scheme.
        if (!num_slashes && ExtractScheme(&url[begin], url_len - begin, &parsed.scheme)) {
    ...
    if (!DoesBeginWindowsDriveSpec(url.data(), after_slashes, url_len) && num_slashes != 3) {
    ...
    DoParseLocalFile(url.substr(0, url_len), num_slashes > 0 ? after_scheme + num_slashes - 1 : after_scheme, &parsed);
    return parsed;
}
