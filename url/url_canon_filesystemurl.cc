...
bool DoCanonicalizeFileSystemURL(const CHAR* spec, const URLComponentSource<CHAR>& source, const Parsed& parsed, CharsetConverter* charset_converter,
    CanonOutput* output, Parsed* new_parsed)
{
    ...
    if (CompareSchemeComponent(spec, inner_parsed->scheme, url::kFileScheme)) {
        ...
        success = CanonicalizeStandardURL(spec, *inner_parsed, inner_scheme_type, charset_converter, output, &new_inner_parsed);
    } else {
    ...
    CanonicalizeQuery(source.query, parsed.query, charset_converter, output, &new_parsed->query);
...
bool CanonicalizeFileSystemURL(const char* spec, const Parsed& parsed, CharsetConverter* charset_converter, CanonOutput* output, Parsed* new_parsed)
{
    return DoCanonicalizeFileSystemURL(spec, URLComponentSource(spec), parsed, charset_converter, output, new_parsed);
}

bool CanonicalizeFileSystemURL(const char16_t* spec, const Parsed& parsed, CharsetConverter* charset_converter, CanonOutput* output, Parsed* new_parsed)
{
    return DoCanonicalizeFileSystemURL(spec, URLComponentSource(spec), parsed, charset_converter, output, new_parsed);
}

bool ReplaceFileSystemURL(const char* base, const Parsed& base_parsed, const Replacements<char>& replacements, CharsetConverter* charset_converter,
    CanonOutput* output, Parsed* new_parsed)
{
    URLComponentSource<char> source(base);
    Parsed parsed(base_parsed);
    SetupOverrideComponents(base, replacements, &source, &parsed);
    return DoCanonicalizeFileSystemURL(base, source, parsed, charset_converter, output, new_parsed);
}

bool ReplaceFileSystemURL(const char* base, const Parsed& base_parsed, const Replacements<char16_t>& replacements, CharsetConverter* charset_converter,
    CanonOutput* output, Parsed* new_parsed)
{
