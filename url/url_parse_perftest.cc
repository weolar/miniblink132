...
{
    constexpr std::string_view kUrl = "http://me:pass@host/foo/bar.html;param?query=yes#ref";
...
constexpr std::string_view kTypicalUrl1 = "http://www.google.com/"
                                          "search?q=url+parsing&ie=utf-8&oe=utf-8&aq=t&rls=org.mozilla:en-US:"
                                          "official&client=firefox-a";

constexpr std::string_view kTypicalUrl2 = "http://www.amazon.com/Stephen-King-Thrillers-Horror-People/dp/0766012336/"
                                          "ref=sr_1_2/133-4144931-4505264?ie=UTF8&s=books&qid=2144880915&sr=8-2";

constexpr std::string_view kTypicalUrl3 = "http://store.apple.com/1-800-MY-APPLE/WebObjects/AppleStore.woa/wa/"
...
TEST(URLParse, TypicalURLParseCanon)
{
    ...
    for (int i = 0; i < 333333; i++) { // divide by 3 so we get 1M
        ...
        url::CanonicalizeStandardURL(
            kTypicalUrl1.data(), url::ParseStandardURL(kTypicalUrl1), url::SCHEME_WITH_HOST_PORT_AND_USER_INFORMATION, nullptr, &output, &out_parsed);

        output.set_length(0);
        url::CanonicalizeStandardURL(
            kTypicalUrl2.data(), url::ParseStandardURL(kTypicalUrl2), url::SCHEME_WITH_HOST_PORT_AND_USER_INFORMATION, nullptr, &output, &out_parsed);

        output.set_length(0);
        url::CanonicalizeStandardURL(
            kTypicalUrl3.data(), url::ParseStandardURL(kTypicalUrl3), url::SCHEME_WITH_HOST_PORT_AND_USER_INFORMATION, nullptr, &output, &out_parsed);
    }
    canon_timer.Done();
}
...
TEST(URLParse, TypicalURLParseCanonStdString)
{
    ...
    for (int i = 0; i < 333333; i++) { // divide by 3 so we get 1M
        ...
        url::CanonicalizeStandardURL(
            kTypicalUrl1.data(), url::ParseStandardURL(kTypicalUrl1), url::SCHEME_WITH_HOST_PORT_AND_USER_INFORMATION, nullptr, &output1, &out_parsed);

        std::string out2;
        url::StdStringCanonOutput output2(&out2);
        url::CanonicalizeStandardURL(
            kTypicalUrl2.data(), url::ParseStandardURL(kTypicalUrl2), url::SCHEME_WITH_HOST_PORT_AND_USER_INFORMATION, nullptr, &output2, &out_parsed);

        std::string out3;
        url::StdStringCanonOutput output3(&out3);
        url::CanonicalizeStandardURL(
            kTypicalUrl3.data(), url::ParseStandardURL(kTypicalUrl3), url::SCHEME_WITH_HOST_PORT_AND_USER_INFORMATION, nullptr, &output3, &out_parsed);
    }
    canon_timer.Done();
}
