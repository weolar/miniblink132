...
void AppendStringOfType(const char16_t* source, size_t length, SharedCharTypes type, CanonOutput* output);
...
template <typename CHAR> inline size_t IsDot(const CHAR* spec, size_t offset, size_t end)
{
    if (spec[offset] == '.') {
        return 1;
...
template <typename UINCHAR, typename OUTCHAR> inline void AppendEscapedChar(UINCHAR ch, CanonOutputT<OUTCHAR>* output)
{
    output->push_back('%');
    std::string hex;
...
bool ReadUTFCharLossy(const char* str, size_t* begin, size_t length, base_icu::UChar32* code_point_out);
...
// character.
template <class Output, void Appender(unsigned char, Output*)> inline void DoAppendUTF8(base_icu::UChar32 char_value, Output* output)
{
    DCHECK(char_value >= 0);
...
COMPONENT_EXPORT(URL)
bool ReadUTFCharLossy(const char16_t* str, size_t* begin, size_t length, base_icu::UChar32* code_point_out);

// Equivalent to U16_APPEND_UNSAFE in ICU but uses our output method.
inline void AppendUTF16Value(base_icu::UChar32 code_point, CanonOutputT<char16_t>* output)
{
...
inline bool AppendUTF8EscapedChar(const char16_t* str, size_t* begin, size_t length, CanonOutput* output)
{
...
template <typename CHAR> bool IsInC0ControlPercentEncodeSet(CHAR ch)
{
...
template <typename CHAR> inline bool DecodeEscaped(const CHAR* spec, size_t* begin, size_t end, unsigned char* unescaped_value)
{
...
// normal.
COMPONENT_EXPORT(URL)
bool ConvertUTF16ToUTF8(const char16_t* input, size_t input_len, CanonOutput* output);
COMPONENT_EXPORT(URL)
bool ConvertUTF8ToUTF16(const char* input, size_t input_len, CanonOutputT<char16_t>* output);
...
void SetupOverrideComponents(const char* base, const Replacements<char>& repl, URLComponentSource<char>* source, Parsed* parsed);
...
// place of errors.
bool SetupUTF16OverrideComponents(
    const char* base, const Replacements<char16_t>& repl, CanonOutput* utf8_buffer, URLComponentSource<char>* source, Parsed* parsed);
...
template <size_t N> inline int _itoa_s(int value, char (&buffer)[N], int radix)
{
    return _itoa_s(value, buffer, N, radix);
}

template <size_t N> inline int _itow_s(int value, char16_t (&buffer)[N], int radix)
{
