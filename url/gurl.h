...
public:
    ...
    GURL(const char* canonical_spec, size_t canonical_spec_len, const url::Parsed& parsed, bool is_valid);
    ...
    {
        return parsed_.username.is_valid();
    }
    ...
    {
        return parsed_.password.is_valid();
    }
    ...
    static bool IsAboutPath(std::string_view actual_path, std::string_view allowed_path);
...
private:
    ...
    template <typename T, typename CharT = typename T::value_type> void InitCanonical(T input_spec, bool trim_path_end);
    ...
    std::string_view ComponentStringPiece(const url::Component& comp) const
    {
        ...
        return std::string_view(spec_).substr(static_cast<size_t>(comp.begin), static_cast<size_t>(comp.len));
    }
...
#define DEBUG_ALIAS_FOR_GURL(var_name, url) DEBUG_ALIAS_FOR_CSTR(var_name, (url).possibly_invalid_spec().c_str(), 128)
