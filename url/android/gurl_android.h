...
public:
    static GURL ToNativeGURL(JNIEnv* env, const jni_zero::JavaRef<jobject>& j_gurl);
    static jni_zero::ScopedJavaLocalRef<jobject> FromNativeGURL(JNIEnv* env, const GURL& gurl);
...
template <> inline GURL FromJniType<GURL>(JNIEnv* env, const JavaRef<jobject>& j_gurl)
{
    return url::GURLAndroid::ToNativeGURL(env, j_gurl);
}

// Convert from native GURL object to a GURL.java object pointer.
template <> inline ScopedJavaLocalRef<jobject> ToJniType<GURL>(JNIEnv* env, const GURL& gurl)
{
