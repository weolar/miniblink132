...
UIDNA* CreateIDNA(bool use_idna_non_transitional)
{
    ...
    if (U_FAILURE(err)) {
        CHECK(false) << "failed to open UTS46 data with error: " << u_errorName(err) << ". If you see this error message in a test environment "
...
bool IDNToASCII(std::u16string_view src, CanonOutputW* output)
{
    ...
    while (true) {
        ...
        int output_length = uidna_nameToASCII(uidna, src.data(), base::checked_cast<int32_t>(src.size()), output->data(), output->capacity(), &info, &err);
