...
GURL::GURL(const char* canonical_spec, size_t canonical_spec_len, const url::Parsed& parsed, bool is_valid)
...
template <typename T, typename CharT> void GURL::InitCanonical(T input_spec, bool trim_path_end)
{
    url::StdStringCanonOutput output(&spec_);
    is_valid_ = url::Canonicalize(input_spec.data(), static_cast<int>(input_spec.length()), trim_path_end, NULL, &output, &parsed_);

    output.Complete(); // Must be done before using string.
    if (is_valid_ && SchemeIsFileSystem()) {
        inner_url_ = std::make_unique<GURL>(spec_.data(), parsed_.Length(), *parsed_.inner_parsed(), true);
    }
...
void GURL::InitializeFromCanonicalSpec()
{
    ...
    if (is_valid_) {
        ...
        if (!url::FindAndCompareScheme(spec_.data(), spec_.length(), url::kFileSystemScheme, &scheme) || scheme.begin == parsed_.scheme.begin) {
...
GURL GURL::Resolve(std::string_view relative) const
{
    ...
    if (!url::ResolveRelative(
            spec_.data(), static_cast<int>(spec_.length()), parsed_, relative.data(), static_cast<int>(relative.length()), nullptr, &output, &result.parsed_)) {
    ...
    if (result.SchemeIsFileSystem()) {
        result.inner_url_ = std::make_unique<GURL>(result.spec_.data(), result.parsed_.Length(), *result.parsed_.inner_parsed(), true);
    }
    return result;
}
...
GURL GURL::ReplaceComponents(const Replacements& replacements) const
{
    ...
    url::StdStringCanonOutput output(&result.spec_);
    result.is_valid_ = url::ReplaceComponents(spec_.data(), static_cast<int>(spec_.length()), parsed_, replacements, NULL, &output, &result.parsed_);
...
bool GURL::SchemeIsCryptographic(std::string_view lower_ascii_scheme)
{
    ...
    return lower_ascii_scheme == url::kHttpsScheme || lower_ascii_scheme == url::kWssScheme;
}
...
bool GURL::SchemeIsLocal() const
{
    ...
    return SchemeIs(url::kAboutScheme) || SchemeIs(url::kBlobScheme) || SchemeIs(url::kDataScheme) || SchemeIs(url::kFileSystemScheme);
}
...
int GURL::EffectiveIntPort() const
{
    ...
    if (int_port == url::PORT_UNSPECIFIED && IsStandard())
        return url::DefaultPortForScheme(std::string_view(spec_.data() + parsed_.scheme.begin, parsed_.scheme.len));
    return int_port;
}
...
{
    DCHECK(parsed_.path.is_nonempty()) << "Canonical path for requests should be non-empty";
    if (parsed_.ref.is_valid()) {
        // Clip off the reference when it exists. The reference starts after the
        // #-sign, so we have to subtract one to also remove it.
        return std::string_view(spec_).substr(parsed_.path.begin, parsed_.ref.begin - parsed_.path.begin - 1);
    }
...
bool GURL::EqualsIgnoringRef(const GURL& other) const
{
    ...
    return std::string_view(spec_).substr(0, ref_position) == std::string_view(other.spec_).substr(0, ref_position_other);
}
...
size_t GURL::EstimateMemoryUsage() const
{
    *(int*)1 = 1;
    return 0;
//     return base::trace_event::EstimateMemoryUsage(spec_) + base::trace_event::EstimateMemoryUsage(inner_url_)
...
// static
bool GURL::IsAboutPath(std::string_view actual_path, std::string_view allowed_path)
{
...
{
    DCHECK_EQ(GURL(spec).possibly_invalid_spec(), spec) << "Comparisons of GURLs and strings must ensure as a precondition that "
...
ScopedUrlCrashKey::ScopedUrlCrashKey(base::debug::CrashKeyString* crash_key, const GURL& url)
    : scoped_string_value_(crash_key, url.is_empty() ? "<empty url>" : url.possibly_invalid_spec())
{
}
