...
template <typename UCHAR> bool ShouldEncodeMailboxCharacter(UCHAR uch)
{
...
bool DoCanonicalizeMailtoURL(const URLComponentSource<CHAR>& source, const Parsed& parsed, CanonOutput* output, Parsed* new_parsed)
{
...
{
    return DoCanonicalizeMailtoURL<char16_t, char16_t>(URLComponentSource<char16_t>(spec), parsed, output, new_parsed);
}
