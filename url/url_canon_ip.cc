...
template <typename CHAR> CanonHostInfo::Family IPv4ComponentToNumber(const CHAR* spec, const Component& component, uint32_t* number)
{
...
CanonHostInfo::Family DoIPv4AddressToNumber(const CHAR* spec, Component host, unsigned char address[4], int* num_ipv4_components)
{
    ...
    while (true) {
        ...
        CanonHostInfo::Family family
            = IPv4ComponentToNumber(spec, Component(current_position, current_component_end - current_position), &component_values[existing_components]);
    ...
    for (int i = existing_components - 1; i > 0; i--) {
        if (component_values[i] > std::numeric_limits<uint8_t>::max())
            return CanonHostInfo::BROKEN;
        address[existing_components - i - 1] = static_cast<unsigned char>(component_values[i]);
    }
...
template <typename CHAR, typename UCHAR> bool DoCanonicalizeIPv4Address(const CHAR* spec, const Component& host, CanonOutput* output, CanonHostInfo* host_info)
{
...
template <typename CHAR, typename UCHAR> bool DoParseIPv6(const CHAR* spec, const Component& host, IPv6Parsed* parsed)
{
    // Zero-out the info.
    parsed->reset();
...
template <typename CHAR> uint16_t IPv6HexComponentToNumber(const CHAR* spec, const Component& component)
{
...
template <typename CHAR, typename UCHAR> bool DoIPv6AddressToNumber(const CHAR* spec, const Component& host, unsigned char address[16])
{
    ...
    if (ipv6_parsed.ipv4_component.is_valid()) {
        ...
        bool trailing_dot = ipv6_parsed.ipv4_component.is_nonempty() && spec[ipv6_parsed.ipv4_component.end() - 1] == '.';
        ...
        if (CanonHostInfo::IPV4 != IPv4AddressToNumber(spec, ipv6_parsed.ipv4_component, &address[cur_index_in_address], &num_ipv4_components)) {
            return false;
        }
...
// is NEUTRAL, and we could use a second opinion.
template <typename CHAR, typename UCHAR> bool DoCanonicalizeIPv6Address(const CHAR* spec, const Component& host, CanonOutput* output, CanonHostInfo* host_info)
{
...
{
    DoCanonicalizeIPv6Address<char, unsigned char>(spec, host, &output, &host_info);
}

void CanonicalizeIPv6Address(const char16_t* spec, const Component& host, CanonOutput& output, CanonHostInfo& host_info)
{
    DoCanonicalizeIPv6Address<char16_t, char16_t>(spec, host, &output, &host_info);
}

CanonHostInfo::Family IPv4AddressToNumber(const char* spec, const Component& host, unsigned char address[4], int* num_ipv4_components)
{
    return DoIPv4AddressToNumber<char, unsigned char>(spec, host, address, num_ipv4_components);
}

CanonHostInfo::Family IPv4AddressToNumber(const char16_t* spec, const Component& host, unsigned char address[4], int* num_ipv4_components)
{
    return DoIPv4AddressToNumber<char16_t, char16_t>(spec, host, address, num_ipv4_components);
}
