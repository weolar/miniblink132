...
AssertionResult ComponentMatches(const char* input, const char* reference, const Component& component)
{
    // Check that the -1 sentinel is the only allowed negative value.
    if (!component.is_valid() && component.len != -1) {
        return AssertionFailure() << "-1 is the only allowed negative value for len";
    }
    ...
    if (!reference)
        return component.len == -1 ? AssertionSuccess() : AssertionFailure() << "len should be -1";
    if (!component.is_valid())
        return AssertionFailure() << "for a non null reference, the component should be valid";
    ...
    return strncmp(reference, &input[component.begin], component.len) == 0 ? AssertionSuccess() : AssertionFailure() << "characters do not match";
}
...
void URLParseCaseMatches(const URLParseCase& expected, const Parsed& parsed)
{
    ...
    SCOPED_TRACE(testing::Message() << "url: \"" << url << "\", parsed: " << parsed);
...
TEST(URLParser, CountCharactersBefore)
{
    ...
    for (const auto& count_case : count_cases) {
        ...
        Parsed parsed = count_case.url[0] == 'f' ? ParseFileURL(count_case.url) : ParseStandardURL(count_case.url);

        int chars_before = parsed.CountCharactersBefore(count_case.component, count_case.include_delimiter);
...
TEST(URLParser, PathURL)
{
    ...
    for (size_t i = 0; i < std::size(path_cases); i++) {
        ...
        EXPECT_TRUE(ComponentMatches(url, path_cases[i].scheme, parsed.scheme)) << i;
        EXPECT_TRUE(ComponentMatches(url, path_cases[i].path, parsed.GetContent())) << i;
...
static bool NthParameterIs(const char* url, int parameter, const char* expected_key, const char* expected_value)
{
    Parsed parsed = ParseStandardURL(url);
...
static FileSystemURLParseCase filesystem_cases[] = {
    ...
    { "filesystem:https://foo/persistent/bar;par/", "https", nullptr, nullptr, "foo", -1, "/persistent", "/bar;par/", nullptr, nullptr },
    { "filesystem:file:///persistent/bar;par/", "file", nullptr, nullptr, nullptr, -1, "/persistent", "/bar;par/", nullptr, nullptr },
    { "filesystem:file:///persistent/bar;par/?query#ref", "file", nullptr, nullptr, nullptr, -1, "/persistent", "/bar;par/", "query", "ref" },
    { "filesystem:file:///persistent", "file", nullptr, nullptr, nullptr, -1, "/persistent", "", nullptr, nullptr },
    { "filesystem:", nullptr, nullptr, nullptr, nullptr, -1, nullptr, nullptr, nullptr, nullptr },
};
...
TEST(URLParser, FileSystemURL)
{
    ...
    for (const auto& filesystem_case : filesystem_cases) {
        ...
        if (parsed.inner_parsed()) {
            EXPECT_TRUE(ComponentMatches(url, filesystem_case.inner_scheme, parsed.inner_parsed()->scheme));
            EXPECT_TRUE(ComponentMatches(url, filesystem_case.inner_username, parsed.inner_parsed()->username));
            EXPECT_TRUE(ComponentMatches(url, filesystem_case.inner_password, parsed.inner_parsed()->password));
            EXPECT_TRUE(ComponentMatches(url, filesystem_case.inner_host, parsed.inner_parsed()->host));
...
static URLParseCase non_special_cases[] = {
    { "git://user:pass@foo:21/bar;par?b#c", "git", "user", "pass", "foo", 21, "/bar;par", "b", "c" },
    { "git://host", "git", nullptr, nullptr, "host", -1, nullptr, nullptr, nullptr },
    { "git://host/a/../b", "git", nullptr, nullptr, "host", -1, "/a/../b", nullptr, nullptr },
    { "git://host/a b", "git", nullptr, nullptr, "host", -1, "/a b", nullptr, nullptr },
    { "git://ho\\st/", "git", nullptr, nullptr, "ho\\st", -1, "/", nullptr, nullptr },
    // Empty users
    ...
    { "git://:80", "git", nullptr, nullptr, nullptr, 80, nullptr, nullptr, nullptr },
    // Empty host cases
    { "git://", "git", nullptr, nullptr, "", -1, nullptr, nullptr, nullptr },
    ...
    { "git:/trailing-space ", "git", nullptr, nullptr, nullptr, -1, "/trailing-space", nullptr, nullptr },
};
...
static URLParseCase non_special_opaque_path_cases[] = {
    ...
    { "git:opaque", "git", nullptr, nullptr, nullptr, -1, "opaque", nullptr, nullptr },
    { "git:opaque?a=b#c", "git", nullptr, nullptr, nullptr, -1, "opaque", "a=b", "c" },
    { "git: o p a q u e ", "git", nullptr, nullptr, nullptr, -1, " o p a q u e", nullptr, nullptr },
    { "git:opa\\que", "git", nullptr, nullptr, nullptr, -1, "opa\\que", nullptr, nullptr },
};
