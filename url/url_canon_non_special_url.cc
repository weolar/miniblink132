...
bool DoCanonicalizeNonSpecialURL(
    const URLComponentSource<CHAR>& source, const Parsed& parsed, CharsetConverter* query_converter, CanonOutput& output, Parsed& new_parsed)
{
    ...
    bool have_authority = (parsed.username.is_valid() || parsed.password.is_valid() || parsed.host.is_valid() || parsed.port.is_valid());
    ...
    if (parsed.path.is_valid()) {
        if (!parsed.host.is_valid() && parsed.path.is_empty()) {
            ...
            if (!parsed.host.is_valid() && new_parsed.path.is_valid() && new_parsed.path.as_string_view_on(output.view().data()).starts_with("//")) {
...
bool CanonicalizeNonSpecialURL(const char* spec, int spec_len, const Parsed& parsed, CharsetConverter* query_converter, CanonOutput& output, Parsed& new_parsed)
{
    // Carry over the flag.
    ...
    return DoCanonicalizeNonSpecialURL(URLComponentSource(spec), parsed, query_converter, output, new_parsed);
}
    ...
    CanonOutput& output, Parsed& new_parsed)
{
    ...
    return DoCanonicalizeNonSpecialURL(source, parsed, query_converter, output, new_parsed);
}
