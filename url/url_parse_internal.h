...
// and goes until |*len|).
template <typename CHAR> inline void TrimURL(const CHAR* spec, int* begin, int* len, bool trim_path_end = true)
{
...
template <typename CHAR> inline int CountConsecutiveSlashes(const CHAR* str, int begin_offset, int str_len)
{
    int count = 0;
...
template <typename CHAR> inline int CountConsecutiveSlashesButNotCountBackslashes(const CHAR* str, int begin_offset, int str_len)
{
    int count = 0;
