...
void DoCanonicalizePathComponent(const CHAR* source, const Component& component, char separator, CanonOutput* output, Component* new_component)
{
...
template <typename CHAR, typename UCHAR>
bool DoCanonicalizePathURL(const URLComponentSource<CHAR>& source, const Parsed& parsed, CanonOutput* output, Parsed* new_parsed)
{
    ...
    // query.
    CanonicalizeQuery(source.query, parsed.query, nullptr, output, &new_parsed->query);
...
{
    return DoCanonicalizePathURL<char16_t, char16_t>(URLComponentSource<char16_t>(spec), parsed, output, new_parsed);
}

void CanonicalizePathURLPath(const char* source, const Component& component, CanonOutput* output, Component* new_component)
{
    DoCanonicalizePathComponent<char, unsigned char>(source, component, '\0', output, new_component);
}

void CanonicalizePathURLPath(const char16_t* source, const Component& component, CanonOutput* output, Component* new_component)
{
    DoCanonicalizePathComponent<char16_t, char16_t>(source, component, '\0', output, new_component);
}
