...
public:
    ...
    static std::optional<Origin> UnsafelyCreateTupleOriginWithoutNormalization(std::string_view scheme, std::string_view host, uint16_t port);
    ...
    static Origin CreateFromNormalizedTuple(std::string scheme, std::string host, uint16_t port);
...
private:
    ...
    friend COMPONENT_EXPORT(URL) std::ostream& operator<<(std::ostream& out, const Origin& origin);
    ...
    friend COMPONENT_EXPORT(URL) std::ostream& operator<<(std::ostream& out, const Nonce& nonce);
    ...
    static Origin CreateOpaqueFromNormalizedPrecursorTuple(
        std::string precursor_scheme, std::string precursor_host, uint16_t precursor_port, const Nonce& nonce);
    ...
    static std::optional<Origin> UnsafelyCreateOpaqueOriginWithoutNormalization(
        std::string_view precursor_scheme, std::string_view precursor_host, uint16_t precursor_port, const Nonce& nonce);
...
#define DEBUG_ALIAS_FOR_ORIGIN(var_name, origin) DEBUG_ALIAS_FOR_CSTR(var_name, (origin).Serialize().c_str(), 128)

namespace debug {

class COMPONENT_EXPORT(URL) ScopedOriginCrashKey {
public:
    ScopedOriginCrashKey(base::debug::CrashKeyString* crash_key, const url::Origin* value);
