...
TEST(URLCanonIcuTest, ICUCharsetConverter)
{
    ...
    for (size_t i = 0; i < std::size(icu_cases); i++) {
        ...
        std::u16string input_str(test_utils::TruncateWStringToUTF16(icu_cases[i].input));
...
TEST(URLCanonIcuTest, QueryWithConverter)
{
    ...
    for (size_t i = 0; i < std::size(query_cases); i++) {
        ...
        if (query_cases[i].input8) {
            ...
            CanonicalizeQuery(query_cases[i].input8, in_comp, &converter, &output, &out_comp);
            output.Complete();
        ...
        if (query_cases[i].input16) {
            std::u16string input16(test_utils::TruncateWStringToUTF16(query_cases[i].input16));
            int len = static_cast<int>(input16.length());
            Component in_comp(0, len);
            std::string out_str;

            StdStringCanonOutput output(&out_str);
            CanonicalizeQuery(input16.c_str(), in_comp, &converter, &output, &out_comp);
            output.Complete();
