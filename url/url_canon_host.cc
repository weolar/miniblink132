...
template <typename CHAR, typename UCHAR> void ScanHostname(const CHAR* spec, const Component& host, bool* has_non_ascii, bool* has_escaped)
{
    int end = host.end();
    *has_non_ascii = false;
    *has_escaped = false;
    ...
    for (size_t i = 0; i < host_len; ++i) {
        ...
        if (source < 0x80) {
            ...
            if (!replacement) {
                ...
                AppendEscapedChar(source, output);
                if (source == ' ' && url::IsDisallowingSpaceCharacterInURLHostParsing() && canon_mode != CanonMode::kFileURL) {
                    success = false;
                }
            } else {
...
template <CanonMode canon_mode> bool DoIDNHost(const char16_t* src, size_t src_len, CanonOutput* output)
{
    ...
    bool success = DoSimpleHost<canon_mode>(wide_output.data(), wide_output.length(), output, &has_non_ascii);
    if (has_non_ascii) {
...
// unescaping.
template <CanonMode canon_mode> bool DoComplexHost(const char* host, size_t host_len, bool has_non_ascii, bool has_escaped, CanonOutput* output)
{
    ...
    return DoIDNHost<canon_mode>(utf16.data(), utf16.length(), output) && are_all_escaped_valid;
}
...
template <CanonMode canon_mode> bool DoComplexHost(const char16_t* host, size_t host_len, bool has_non_ascii, bool has_escaped, CanonOutput* output)
{
    if (has_escaped) {
        ...
        return DoComplexHost<canon_mode>(utf8.data(), utf8.length(), has_non_ascii, has_escaped, output);
    }
...
template <typename CHAR, typename UCHAR, CanonMode canon_mode> bool DoHostSubstring(const CHAR* spec, const Component& host, CanonOutput* output)
{
    ...
    if (has_non_ascii || has_escaped) {
        return DoComplexHost<canon_mode>(&spec[host.begin], static_cast<size_t>(host.len), has_non_ascii, has_escaped, output);
    }

    const bool success = DoSimpleHost<canon_mode>(&spec[host.begin], static_cast<size_t>(host.len), output, &has_non_ascii);
    DCHECK(!has_non_ascii);
    return success;
}

template <typename CharT> bool DoOpaqueHost(const std::basic_string_view<CharT> host, CanonOutput& output)
{
...
void DoHost(const CHAR* spec, const Component& host, CanonOutput& output, CanonHostInfo& host_info)
{
    ...
    bool success;
    if constexpr (canon_mode == CanonMode::kSpecialURL || canon_mode == CanonMode::kFileURL) {
...
{
    CanonHostInfo host_info;
    DoHost<char16_t, char16_t, CanonMode::kSpecialURL>(spec, host, output, host_info);
    out_host = host_info.out_host;
    return (host_info.family != CanonHostInfo::BROKEN);
}
...
{
    CanonHostInfo host_info;
    DoHost<char, unsigned char, CanonMode::kFileURL>(spec, host, output, host_info);
    out_host = host_info.out_host;
    return (host_info.family != CanonHostInfo::BROKEN);
}
...
{
    CanonHostInfo host_info;
    DoHost<char16_t, char16_t, CanonMode::kFileURL>(spec, host, output, host_info);
    out_host = host_info.out_host;
    return (host_info.family != CanonHostInfo::BROKEN);
}
...
{
    CanonHostInfo host_info;
    DoHost<char16_t, char16_t, CanonMode::kNonSpecialURL>(spec, host, output, host_info);
    out_host = host_info.out_host;
    return (host_info.family != CanonHostInfo::BROKEN);
}
...
{
    return DoHostSubstring<char, unsigned char, CanonMode::kSpecialURL>(spec, host, output);
}

bool CanonicalizeHostSubstring(const char16_t* spec, const Component& host, CanonOutput* output)
{
    return DoHostSubstring<char16_t, char16_t, CanonMode::kSpecialURL>(spec, host, output);
}
