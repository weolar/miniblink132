...
Origin Origin::Create(const GURL& url)
{
    ...
    if (url.SchemeIsFileSystem()) {
        ...
        DCHECK(!tuple.IsValid() || url.IsStandard() || base::Contains(GetLocalSchemes(), url.scheme_piece()) || AllowNonStandardSchemesForAndroidWebView());
    }
...
// static
std::optional<Origin> Origin::UnsafelyCreateTupleOriginWithoutNormalization(std::string_view scheme, std::string_view host, uint16_t port)
{
    SchemeHostPort tuple(std::string(scheme), std::string(host), port, SchemeHostPort::CHECK_CANONICALIZATION);
    if (!tuple.IsValid())
        return std::nullopt;
    return Origin(std::move(tuple));
}
...
{
    SchemeHostPort precursor(std::string(precursor_scheme), std::string(precursor_host), precursor_port, SchemeHostPort::CHECK_CANONICALIZATION);
    ...
    if (!precursor.IsValid() && !(precursor_scheme.empty() && precursor_host.empty() && precursor_port == 0)) {
        return std::nullopt;
    }
...
// static
Origin Origin::CreateFromNormalizedTuple(std::string scheme, std::string host, uint16_t port)
{
    SchemeHostPort tuple(std::move(scheme), std::move(host), port, SchemeHostPort::ALREADY_CANONICALIZED);
    if (!tuple.IsValid())
        return Origin();
    return Origin(std::move(tuple));
}
...
Origin Origin::CreateOpaqueFromNormalizedPrecursorTuple(
    std::string precursor_scheme, std::string precursor_host, uint16_t precursor_port, const Origin::Nonce& nonce)
{
    SchemeHostPort precursor(std::move(precursor_scheme), std::move(precursor_host), precursor_port, SchemeHostPort::ALREADY_CANONICALIZED);
    // For opaque origins, it is okay for the SchemeHostPort to be invalid.
    return Origin(std::move(nonce), std::move(precursor));
}
...
bool Origin::CanBeDerivedFrom(const GURL& url) const
{
    ...
    if (base::Contains(url::GetNoAccessSchemes(), url.scheme()) && !url.SchemeIs(kAboutScheme)) {
...
std::optional<std::string> Origin::SerializeWithNonceImpl() const
{
    ...
    base::span<const uint8_t> data(static_cast<const uint8_t*>(pickle.data()), pickle.size());
...
std::optional<Origin> Origin::Deserialize(std::string_view value)
{
    ...
    base::Pickle pickle = base::Pickle::WithUnownedBuffer(base::as_byte_span(data));
    ...
    std::optional<base::UnguessableToken> nonce_token = base::UnguessableToken::Deserialize(nonce_high, nonce_low);
...
ScopedOriginCrashKey::ScopedOriginCrashKey(base::debug::CrashKeyString* crash_key, const url::Origin* value)
    : scoped_string_value_(crash_key, value ? value->GetDebugString(false /* include_nonce */) : "nullptr")
{
}
