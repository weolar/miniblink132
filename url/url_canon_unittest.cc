...
void SetupReplComp(
    void (Replacements<CHAR>::*set)(const CHAR*, const Component&), void (Replacements<CHAR>::*clear)(), Replacements<CHAR>* rep, const CHAR* str)
{
...
bool CanonicalizeSpecialPath(const char* spec, const Component& path, CanonOutput* output, Component* out_path)
{
    return CanonicalizePath(spec, path, CanonMode::kSpecialURL, output, out_path);
}

bool CanonicalizeSpecialPath(const char16_t* spec, const Component& path, CanonOutput* output, Component* out_path)
{
    return CanonicalizePath(spec, path, CanonMode::kSpecialURL, output, out_path);
}

bool CanonicalizeNonSpecialPath(const char* spec, const Component& path, CanonOutput* output, Component* out_path)
{
    return CanonicalizePath(spec, path, CanonMode::kNonSpecialURL, output, out_path);
}

bool CanonicalizeNonSpecialPath(const char16_t* spec, const Component& path, CanonOutput* output, Component* out_path)
{
    return CanonicalizePath(spec, path, CanonMode::kNonSpecialURL, output, out_path);
}
...
public:
    URLCanonTest()
    {
        scoped_feature_list_.InitAndEnableFeature(url::kDisallowSpaceCharacterInURLHostParsing);
    }
...
TEST_F(URLCanonTest, UTF)
{
    ...
    struct UTFCase {
        ...
        { "\xf0\x84\xbd\xa0\xe5\xa5\xbd", nullptr, false, "%EF%BF%BD%EF%BF%BD%EF%BF%BD%EF%BF%BD%E5%A5%BD" },
        ...
        // ...even when paired.
        { "\xed\xa0\x80\xed\xb0\x80", nullptr, false, "%EF%BF%BD%EF%BF%BD%EF%BF%BD%EF%BF%BD%EF%BF%BD%EF%BF%BD" },
    };
    ...
    for (const auto& utf_case : utf_cases) {
        ...
        if (utf_case.input16) {
            ...
            std::u16string input_str(test_utils::TruncateWStringToUTF16(utf_case.input16));
            size_t input_len = input_str.length();
            bool success = true;
            for (size_t ch = 0; ch < input_len; ch++) {
                success &= AppendUTF8EscapedChar(input_str.c_str(), &ch, input_len, &output);
            }
            output.Complete();
            EXPECT_EQ(utf_case.expected_success, success);
            EXPECT_EQ(utf_case.output, out_str);
        ...
        if (utf_case.input8 && utf_case.input16 && utf_case.expected_success) {
            ...
            std::u16string input16_str(test_utils::TruncateWStringToUTF16(utf_case.input16));
...
TEST_F(URLCanonTest, Scheme)
{
    ...
    for (const auto& scheme_case : scheme_cases) {
        ...
        success = CanonicalizeScheme(wide_input.c_str(), in_comp, &output2, &out_comp);
        output2.Complete();
...
class URLCanonHostTest : public ::testing::Test, public ::testing::WithParamInterface<IDNAMode> {
public:
    URLCanonHostTest()
    {
...
INSTANTIATE_TEST_SUITE_P(All, URLCanonHostTest, ::testing::Values(IDNAMode::kTransitional, IDNAMode::kNonTransitional));
...
TEST_P(URLCanonHostTest, Host)
{
    ...
    IPAddressCase host_cases[] = {
        ...
        { "Goo%20 goo.com", L"Goo%20 goo.com", "goo%20%20goo.com", Component(0, 16), CanonHostInfo::BROKEN, -1, "" },
        // TODO(crbug.com/40256677): Update the test after ASTERISK is
        // correctly handled.
        { "Goo%2a*goo.com", L"Goo%2a*goo.com", "goo%2A%2Agoo.com", Component(0, 16), CanonHostInfo::NEUTRAL, -1, "" },
        // Exciting different types of spaces!
        { nullptr, L"GOO\x00a0\x3000goo.com", "goo%20%20goo.com", Component(0, 16), CanonHostInfo::BROKEN, -1, "" },
        // Other types of space (no-break, zero-width, zero-width-no-break) are
        // name-prepped away to nothing.
        { nullptr, L"GOO\x200b\x2060\xfeffgoo.com", "googoo.com", Component(0, 10), CanonHostInfo::NEUTRAL, -1, "" },
        ...
        { "\xef\xb7\x90zyx.com", L"\xfdd0zyx.com", "%EF%B7%90zyx.com", Component(0, 16), CanonHostInfo::BROKEN, -1, "" },
        // ...This is the same as previous but with with escaped.
        { "%ef%b7%90zyx.com", L"%ef%b7%90zyx.com", "%EF%B7%90zyx.com", Component(0, 16), CanonHostInfo::BROKEN, -1, "" },
        ...
        { "fu\xc3\x9f"
          "ball.de",
            L"fu\x00df"
            L"ball.de",
            use_idna_non_transitional ? "xn--fuball-cta.de" : "fussball.de", use_idna_non_transitional ? Component(0, 17) : Component(0, 11),
            CanonHostInfo::NEUTRAL, -1, "" },

        // Final-sigma (U+03C3) was mapped to regular sigma (U+03C2).
        // Previously, it'd be "xn--wxaikc9b".
        { "\xcf\x83\xcf\x8c\xce\xbb\xce\xbf\xcf\x82", L"\x3c3\x3cc\x3bb\x3bf\x3c2", use_idna_non_transitional ? "xn--wxaijb9b" : "xn--wxaikc6b",
        ...
        { "a\xe2\x80\x8c"
          ...
          "c",
            ...
            L"b\x200d"
            L"c",
            use_idna_non_transitional ? "xn--abc-9m0ag" : "abc", use_idna_non_transitional ? Component(0, 13) : Component(0, 3), CanonHostInfo::NEUTRAL, -1,
            "" },
        ...
        { "\xe0\xa4\x95\xe0\xa5\x8d\xe2\x80\x8d\xe0\xa4\x9c", L"\x915\x94d\x200d\x91c", use_idna_non_transitional ? "xn--11bo0mv54g" : "xn--11bo0m",
            use_idna_non_transitional ? Component(0, 14) : Component(0, 10), CanonHostInfo::NEUTRAL, -1, "" },
        ...
        { "wow\xef\xbc\x81", L"wow\xff01", "wow!", Component(0, 4), CanonHostInfo::NEUTRAL, -1, "" },
        // U+2132 (turned capital F) is disallowed. UTS 46, table 4, row (c)
        // Allowed in IDNA 2003, but the mapping changed after Unicode 3.2
        { "\xe2\x84\xb2oo", L"\x2132oo", "%E2%84%B2oo", Component(0, 11), CanonHostInfo::BROKEN, -1, "" },
        // U+2F868 (CJK Comp) is disallowed. UTS 46, table 4, row (d)
        // Allowed in IDNA 2003, but the mapping changed after Unicode 3.2
        { "\xf0\xaf\xa1\xa8\xe5\xa7\xbb.cn", L"\xd87e\xdc68\x59fb.cn", "%F0%AF%A1%A8%E5%A7%BB.cn", Component(0, 24), CanonHostInfo::BROKEN, -1, "" },
        // Maps uppercase letters to lower case letters. UTS 46 table 4 row (e)
        { "M\xc3\x9cNCHEN", L"M\xdcNCHEN", "xn--mnchen-3ya", Component(0, 14), CanonHostInfo::NEUTRAL, -1, "" },
        // An already-IDNA host is not modified.
        { "xn--mnchen-3ya", L"xn--mnchen-3ya", "xn--mnchen-3ya", Component(0, 14), CanonHostInfo::NEUTRAL, -1, "" },
        // Symbol/punctuations are allowed in IDNA 2003/UTS46.
        // Not allowed in IDNA 2008. UTS 46 table 4 row (f).
        { "\xe2\x99\xa5ny.us", L"\x2665ny.us", "xn--ny-s0x.us", Component(0, 13), CanonHostInfo::NEUTRAL, -1, "" },
        ...
        { "\xf0\x91\x80\x93.com", L"\xd804\xdc13.com", "xn--n00d.com", Component(0, 12), CanonHostInfo::NEUTRAL, -1, "" },
        // U+0602 is disallowed in UTS46/IDNA 2008. UTS 46 table 4, row(i)
        // Used to be allowed in INDA 2003.
        { "\xd8\x82.eg", L"\x602.eg", "%D8%82.eg", Component(0, 9), CanonHostInfo::BROKEN, -1, "" },
        ...
        { "\xe2\x82\xb7.com", L"\x20b7.com", "xn--wzg.com", Component(0, 11), CanonHostInfo::NEUTRAL, -1, "" },
        ...
        { "bc\xc8\xba.com", L"bc\x23a.com", "xn--bc-is1a.com", Component(0, 15), CanonHostInfo::NEUTRAL, -1, "" },
        // Maps U+FF43 (Full Width Small Letter C) to 'c'.
        { "ab\xef\xbd\x83.xyz", L"ab\xff43.xyz", "abc.xyz", Component(0, 7), CanonHostInfo::NEUTRAL, -1, "" },
        // Maps U+1D68C (Math Monospace Small C) to 'c'.
        // U+1D68C = \xD835\xDE8C in UTF-16
        { "ab\xf0\x9d\x9a\x8c.xyz", L"ab\xd835\xde8c.xyz", "abc.xyz", Component(0, 7), CanonHostInfo::NEUTRAL, -1, "" },
        // BiDi check test
        // "Divehi" in Divehi (Thaana script) ends with BidiClass=NSM.
        // Disallowed in IDNA 2003 but now allowed in UTS 46/IDNA 2008.
        { "\xde\x8b\xde\xa8\xde\x88\xde\xac\xde\x80\xde\xa8", L"\x78b\x7a8\x788\x7ac\x780\x7a8", "xn--hqbpi0jcw", Component(0, 13), CanonHostInfo::NEUTRAL, -1,
            "" },
        ...
        { "\xd8\xac\xd8\xa7\xd8\xb1xyz", L"\x62c\x627\x631xyz", "%D8%AC%D8%A7%D8%B1xyz", Component(0, 21), CanonHostInfo::BROKEN, -1, "" },
        ...
        { "\xd8\xac\xd8\xa7\xd8\xb1xy2", L"\x62c\x627\x631xy2", "%D8%AC%D8%A7%D8%B1xy2", Component(0, 21), CanonHostInfo::BROKEN, -1, "" },
        // Labels starting with a RTL character can end with BC=AN (Arabic number)
        // Disallowed in IDNA 2003, but now allowed.
        { "\xd8\xac\xd8\xa7\xd8\xb1\xd9\xa2", L"\x62c\x627\x631\x662", "xn--mgbjq0r", Component(0, 11), CanonHostInfo::NEUTRAL, -1, "" },
        // Labels starting with a RTL character cannot have "L" characters
        // even if it ends with an BC=AN (Arabic number).
        // Disallowed in both IDNA 2003/2008.
        { "\xd8\xac\xd8\xa7\xd8\xb1xy\xd9\xa2", L"\x62c\x627\x631xy\x662", "%D8%AC%D8%A7%D8%B1xy%D9%A2", Component(0, 26), CanonHostInfo::BROKEN, -1, "" },
        // Labels starting with a RTL character cannot mix BC=EN and BC=AN
        { "\xd8\xac\xd8\xa7\xd8\xb1xy2\xd9\xa2", L"\x62c\x627\x631xy2\x662", "%D8%AC%D8%A7%D8%B1xy2%D9%A2", Component(0, 27), CanonHostInfo::BROKEN, -1, "" },
        // As of Unicode 6.2, U+20CF is not assigned. We do not allow it.
        { "\xe2\x83\x8f.com", L"\x20cf.com", "%E2%83%8F.com", Component(0, 13), CanonHostInfo::BROKEN, -1, "" },
        // U+0080 is not allowed.
        { "\xc2\x80.com", L"\x80.com", "%C2%80.com", Component(0, 10), CanonHostInfo::BROKEN, -1, "" },
        ...
        { "%E4%BD%A0%E5%A5%BD\xe4\xbd\xa0\xe5\xa5\xbd", L"%E4%BD%A0%E5%A5%BD\x4f60\x597d", "xn--6qqa088eba", Component(0, 14), CanonHostInfo::NEUTRAL, -1, "" },
        // Invalid escaped characters should fail and the percents should be
        // escaped.
        { "%zz%66%a", L"%zz%66%a", "%25zzf%25a", Component(0, 10), CanonHostInfo::BROKEN, -1, "" },
        // If we get an invalid character that has been escaped.
        { "%25", L"%25", "%25", Component(0, 3), CanonHostInfo::BROKEN, -1, "" },
        { "hello%00", L"hello%00", "hello%00", Component(0, 8), CanonHostInfo::BROKEN, -1, "" },
        // Escaped numbers should be treated like IP addresses if they are.
        { "%30%78%63%30%2e%30%32%35%30.01", L"%30%78%63%30%2e%30%32%35%30.01", "192.168.0.1", Component(0, 11), CanonHostInfo::IPV4, 3, "C0A80001" },
        { "%30%78%63%30%2e%30%32%35%30.01%2e", L"%30%78%63%30%2e%30%32%35%30.01%2e", "192.168.0.1", Component(0, 11), CanonHostInfo::IPV4, 3, "C0A80001" },
        ...
        // (incorrect).
        { "\xd1\x82(", L"\x0442(", "xn--(-8tb", Component(0, 9), CanonHostInfo::NEUTRAL, -1, "" },
        ...
        { "12345678912345.12345678912345.de", L"12345678912345.12345678912345.de", "12345678912345.12345678912345.de", Component(0, 32), CanonHostInfo::NEUTRAL,
            -1, "" },
    ...
    for (const auto& host_case : host_cases) {
        ...
        if (host_case.input8) {
            ...
            EXPECT_EQ(host_case.expected_family != CanonHostInfo::BROKEN, success) << "for input: " << host_case.input8;
            EXPECT_EQ(host_case.expected, out_str) << "for input: " << host_case.input8;
            EXPECT_EQ(host_case.expected_component.begin, out_comp.begin) << "for input: " << host_case.input8;
            EXPECT_EQ(host_case.expected_component.len, out_comp.len) << "for input: " << host_case.input8;
        }

        // Wide version.
        if (host_case.input16) {
            std::u16string input16(test_utils::TruncateWStringToUTF16(host_case.input16));
            ...
            bool success = CanonicalizeHost(input16.c_str(), in_comp, &output, &out_comp);
            output.Complete();
    ...
    for (const auto& host_case : host_cases) {
        ...
        if (host_case.input8) {
            ...
            EXPECT_EQ(host_case.expected_address_hex, base::HexEncode(host_info.address, static_cast<size_t>(host_info.AddressLength())));
...
TEST_F(URLCanonTest, SpecialHostPuncutationChar)
{
    ...
    const std::string_view allowed_host_chars[] = { "!", "\"", "$", "&", "'", "(", ")", "+", ",", "-", ".", ";", "=", "_", "`", "{", "}", "~" };
    ...
    for (const std::string_view input : allowed_host_chars) {
        ...
        Component out_comp;
        StdStringCanonOutput output(&out_str);
        bool success = CanonicalizeSpecialHost(input.data(), in_comp, output, out_comp);
    ...
    for (const std::string_view input : forbidden_host_chars) {
        ...
        Component out_comp;
        StdStringCanonOutput output(&out_str);
        EXPECT_FALSE(CanonicalizeSpecialHost(input.data(), in_comp, output, out_comp)) << "Input: " << input;
    }
    ...
    for (const auto& c : escaped_host_chars) {
        ...
        Component out_comp;
        StdStringCanonOutput output(&out_str);
        bool success = CanonicalizeSpecialHost(c.input.data(), in_comp, output, out_comp);
...
TEST_F(URLCanonTest, ForbiddenHostCodePoint)
{
    ...
    for (const std::string_view input : forbidden_host_chars) {
        ...
        Component out_comp;
        StdStringCanonOutput output(&out_str);
        EXPECT_FALSE(CanonicalizeNonSpecialHost(input.data(), in_comp, output, out_comp)) << "Input: " << input;
    }
    ...
    Component out_comp;
    StdStringCanonOutput output(&out_str);
    EXPECT_FALSE(CanonicalizeNonSpecialHost(host_with_null, in_comp, output, out_comp));
}
...
TEST_F(URLCanonTest, IPv4)
{
  ...
  IPAddressCase cases[] = {
    ...
    for (const auto& test_case : cases) {
        ...
        EXPECT_EQ(test_case.expected_address_hex, base::HexEncode(host_info.address, static_cast<size_t>(host_info.AddressLength())));
        if (host_info.family == CanonHostInfo::IPV4) {
            ...
            EXPECT_EQ(test_case.expected_num_ipv4_components, host_info.num_ipv4_components);
        }

        // 16-bit version.
        std::u16string input16(test_utils::TruncateWStringToUTF16(test_case.input16));
...
TEST_F(URLCanonTest, IPv6)
{
    IPAddressCase cases[] = {
        ...
        { "[000:01:02:003:004:5:6:007]", L"[000:01:02:003:004:5:6:007]", "[0:1:2:3:4:5:6:7]", Component(0, 17), CanonHostInfo::IPV6, -1,
            "00000001000200030004000500060007" },

        // Upper case letters should be lowercased.
        { "[A:b:c:DE:fF:0:1:aC]", L"[A:b:c:DE:fF:0:1:aC]", "[a:b:c:de:ff:0:1:ac]", Component(0, 20), CanonHostInfo::IPV6, -1,
        ...
        { "[::ffff:192.1.2]", L"[::ffff:192.1.2]", "[::ffff:c001:2]", Component(), CanonHostInfo::BROKEN, -1, "" },
        { "[::ffff:192.1]", L"[::ffff:192.1]", "[::ffff:c000:1]", Component(), CanonHostInfo::BROKEN, -1, "" },
        { "[::ffff:192.1.2.3.4]", L"[::ffff:192.1.2.3.4]", "", Component(), CanonHostInfo::BROKEN, -1, "" },

        // IPv4 using hex.
        // TODO(eroman): Should this format be disallowed?
        { "[::ffff:0xC0.0Xa8.0x0.0x1]", L"[::ffff:0xC0.0Xa8.0x0.0x1]", "[::ffff:c0a8:1]", Component(0, 15), CanonHostInfo::IPV6, -1,
    ...
    for (size_t i = 0; i < std::size(cases); i++) {
        ...
        EXPECT_EQ(cases[i].expected_address_hex, base::HexEncode(host_info.address, static_cast<size_t>(host_info.AddressLength())))
        ...
        // 16-bit version.
        std::u16string input16(test_utils::TruncateWStringToUTF16(cases[i].input16));
        component = Component(0, static_cast<int>(input16.length()));
...
TEST_F(URLCanonTest, CanonicalizeHostSubstring)
{
    // Basic sanity check.
    {
        std::string out_str;
        StdStringCanonOutput output(&out_str);
        EXPECT_TRUE(CanonicalizeHostSubstring("M\xc3\x9cNCHEN.com", Component(0, 12), &output));
        output.Complete();
    ...
    // Failure case.
    {
        std::string out_str;
        StdStringCanonOutput output(&out_str);
        EXPECT_FALSE(CanonicalizeHostSubstring(test_utils::TruncateWStringToUTF16(L"\xfdd0zyx.com").c_str(), Component(0, 8), &output));
        output.Complete();
    ...
    // Numbers that look like IP addresses should not be changed.
    {
        std::string out_str;
        StdStringCanonOutput output(&out_str);
        EXPECT_TRUE(CanonicalizeHostSubstring("01.02.03.04", Component(0, 11), &output));
        output.Complete();
...
TEST_F(URLCanonTest, UserInfo)
{
    ...
    for (const auto& user_info_case : user_info_cases) {
        ...
        bool success = CanonicalizeUserInfo(user_info_case.input, parsed.username, user_info_case.input, parsed.password, &output1, &out_user, &out_pass);
        output1.Complete();
        ...
        success = CanonicalizeUserInfo(wide_input.c_str(), parsed.username, wide_input.c_str(), parsed.password, &output2, &out_user, &out_pass);
        output2.Complete();
...
TEST_F(URLCanonTest, Port)
{
    ...
    for (const auto& port_case : port_cases) {
        ...
        std::string out_str;
        StdStringCanonOutput output1(&out_str);
        bool success = CanonicalizePort(port_case.input, in_comp, port_case.default_port, &output1, &out_comp);
        output1.Complete();
        ...
        success = CanonicalizePort(wide_input.c_str(), in_comp, port_case.default_port, &output2, &out_comp);
        output2.Complete();
...
DualComponentCase kCommonPathCases[] = {
    ...
    { nullptr, L"/foo%2\xc2\xa9zbar", "/foo%2%C3%82%C2%A9zbar", Component(0, 22), true },
...
void DoPathTest(const DualComponentCase* path_cases, size_t num_cases, CanonFunc8Bit canon_func_8, CanonFunc16Bit canon_func_16)
{
    for (size_t i = 0; i < num_cases; i++) {
        ...
        if (path_cases[i].input8) {
            ...
            std::string out_str;
            StdStringCanonOutput output(&out_str);
            bool success = canon_func_8(path_cases[i].input8, in_comp, &output, &out_comp);
            output.Complete();
        ...
        if (path_cases[i].input16) {
            std::u16string input16(test_utils::TruncateWStringToUTF16(path_cases[i].input16));
            ...
            bool success = canon_func_16(input16.c_str(), in_comp, &output, &out_comp);
            output.Complete();
...
{
    // Common test cases
    DoPathTest(kCommonPathCases, std::size(kCommonPathCases), CanonicalizeSpecialPath, CanonicalizeSpecialPath);
    ...
    std::string out_str;
    StdStringCanonOutput output(&out_str);
    bool success = CanonicalizeSpecialPath(path_with_null, in_comp, &output, &out_comp);
    output.Complete();
    EXPECT_TRUE(success);
    ...
    DoPathTest(special_path_cases, std::size(special_path_cases), CanonicalizeSpecialPath, CanonicalizePath);
}

TEST_F(URLCanonTest, NonSpecialPath)
{
    // Common test cases
    DoPathTest(kCommonPathCases, std::size(kCommonPathCases), CanonicalizeNonSpecialPath, CanonicalizeNonSpecialPath);
    ...
    DoPathTest(non_special_path_cases, std::size(non_special_path_cases), CanonicalizeNonSpecialPath, CanonicalizeNonSpecialPath);
}
...
TEST_F(URLCanonTest, PartialPath)
{
    ...
    DoPathTest(kCommonPathCases, std::size(kCommonPathCases), CanonicalizePartialPath, CanonicalizePartialPath);
    DoPathTest(partial_path_cases, std::size(partial_path_cases), CanonicalizePartialPath, CanonicalizePartialPath);
}
...
TEST_F(URLCanonTest, Query)
{
    ...
    for (const auto& query_case : query_cases) {
        ...
        if (query_case.input8) {
            ...
            StdStringCanonOutput output(&out_str);
            CanonicalizeQuery(query_case.input8, in_comp, nullptr, &output, &out_comp);
            output.Complete();
        ...
        if (query_case.input16) {
            std::u16string input16(test_utils::TruncateWStringToUTF16(query_case.input16));
            int len = static_cast<int>(input16.length());
            Component in_comp(0, len);
            std::string out_str;
    ...
    StdStringCanonOutput output(&out_str);
    Component out_comp;
    CanonicalizeQuery("a \x00z\x01", Component(0, 5), nullptr, &output, &out_comp);
    output.Complete();
...
TEST_F(URLCanonTest, Ref)
{
    ...
    DualComponentCase ref_cases[] = {
        ...
        { "hello, world", L"hello, world", "#hello,%20world", Component(1, 14), true },
        { "hello,\"world", L"hello,\"world", "#hello,%22world", Component(1, 14), true },
        { "hello,<world", L"hello,<world", "#hello,%3Cworld", Component(1, 14), true },
        { "hello,>world", L"hello,>world", "#hello,%3Eworld", Component(1, 14), true },
        { "hello,`world", L"hello,`world", "#hello,%60world", Component(1, 14), true },
        ...
        { "\xF0\x90\x8C\x80ss", L"\xd800\xdf00ss", "#%F0%90%8C%80ss", Component(1, 14), true },
    ...
    for (const auto& ref_case : ref_cases) {
        ...
        // 16-bit input
        if (ref_case.input16) {
            std::u16string input16(test_utils::TruncateWStringToUTF16(ref_case.input16));
            int len = static_cast<int>(input16.length());
            Component in_comp(0, len);
            Component out_comp;
...
TEST_F(URLCanonTest, CanonicalizeStandardURL)
{
  ...
  struct URLCase {
    ...
    for (const auto& i : cases) {
        ...
        std::string out_str;
        StdStringCanonOutput output(&out_str);
        bool success = CanonicalizeStandardURL(i.input, parsed, SCHEME_WITH_HOST_PORT_AND_USER_INFORMATION, nullptr, &output, &out_parsed);
        output.Complete();
...
TEST_F(URLCanonTest, CanonicalizeNonSpecialURL)
{
    ...
    for (const auto& i : cases) {
        ...
        std::string out_str;
        StdStringCanonOutput output(&out_str);
        bool success = CanonicalizeNonSpecialURL(i.input.data(), i.input.size(), parsed,
...
TEST_F(URLCanonTest, ReplaceStandardURL)
{
    ReplaceCase replace_cases[] = {
        ...
        { "http://www.google.com/foo?bar=baz#ref", nullptr, nullptr, nullptr, nullptr, nullptr, "/", kDeleteComp, kDeleteComp, "http://www.google.com/" },
        // Replace everything
        { "http://a:b@google.com:22/foo;bar?baz@cat", "https", "me", "pw", "host.com", "99", "/path", "query", "ref",
            "https://me:pw@host.com:99/path?query#ref" },
        // Replace nothing
        { "http://a:b@google.com:22/foo?baz@cat", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
            "http://a:b@google.com:22/foo?baz@cat" },
        // Replace scheme with filesystem. The result is garbage, but you asked
        // for it.
        { "http://a:b@google.com:22/foo?baz@cat", "filesystem", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
    ...
    for (const auto& replace_case : replace_cases) {
        ...
        StdStringCanonOutput output(&out_str);
        Parsed out_parsed;
        ReplaceStandardURL(replace_case.base, parsed, r, SCHEME_WITH_HOST_PORT_AND_USER_INFORMATION, nullptr, &output, &out_parsed);
        output.Complete();
    ...
    // The path pointer should be ignored if the address is invalid.
    {
        ...
        StdStringCanonOutput output1(&out_str1);
        Parsed new_parsed;
        ReplaceStandardURL(src, parsed, r, SCHEME_WITH_HOST_PORT_AND_USER_INFORMATION, nullptr, &output1, &new_parsed);
        output1.Complete();
        ...
        std::string out_str2;
        StdStringCanonOutput output2(&out_str2);
        ReplaceStandardURL(src, parsed, r, SCHEME_WITH_HOST_PORT_AND_USER_INFORMATION, nullptr, &output2, &new_parsed);
        output2.Complete();
...
TEST_F(URLCanonTest, ReplaceFileURL)
{
    ReplaceCase replace_cases[] = {
        // Replace everything
        { "file:///C:/gaba?query#ref", nullptr, nullptr, nullptr, "filer", nullptr, "/foo", "b", "c", "file://filer/foo?b#c" },
        // Replace nothing
        { "file:///C:/gaba?query#ref", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, "file:///C:/gaba?query#ref" },
        { "file:///Y:", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, "file:///Y:" },
        { "file:///Y:/", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, "file:///Y:/" },
        { "file:///./Y", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, "file:///Y" },
        { "file:///./Y:", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, "file:///Y:" },
        // Clear non-path components (common)
        { "file:///C:/gaba?query#ref", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, kDeleteComp, kDeleteComp, "file:///C:/gaba" },
        // Replace path with something that doesn't begin with a slash and make
        // sure it gets added properly.
        { "file:///C:/gaba", nullptr, nullptr, nullptr, nullptr, nullptr, "interesting/", nullptr, nullptr, "file:///interesting/" },
        { "file:///home/gaba?query#ref", nullptr, nullptr, nullptr, "filer", nullptr, "/foo", "b", "c", "file://filer/foo?b#c" },
        { "file:///home/gaba?query#ref", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, "file:///home/gaba?query#ref" },
        { "file:///home/gaba?query#ref", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, kDeleteComp, kDeleteComp, "file:///home/gaba" },
        { "file:///home/gaba", nullptr, nullptr, nullptr, nullptr, nullptr, "interesting/", nullptr, nullptr, "file:///interesting/" },
        // Replace scheme -- shouldn't do anything.
        { "file:///C:/gaba?query#ref", "http", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, "file:///C:/gaba?query#ref" },
    };
...
TEST_F(URLCanonTest, ReplaceFileSystemURL)
{
    ReplaceCase replace_cases[] = {
        ...
        { "filesystem:file:///temporary/gaba?query#ref", nullptr, nullptr, nullptr, nullptr, nullptr, "/foo", "b", "c",
            "filesystem:file:///temporary/foo?b#c" },
        // Replace nothing
        { "filesystem:file:///temporary/gaba?query#ref", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
            "filesystem:file:///temporary/gaba?query#ref" },
        // Clear non-path components (common)
        { "filesystem:file:///temporary/gaba?query#ref", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, kDeleteComp, kDeleteComp,
            "filesystem:file:///temporary/gaba" },
        // Replace path with something that doesn't begin with a slash and make
        // sure it gets added properly.
        { "filesystem:file:///temporary/gaba?query#ref", nullptr, nullptr, nullptr, nullptr, nullptr, "interesting/", nullptr, nullptr,
            "filesystem:file:///temporary/interesting/?query#ref" },
        // Replace scheme -- shouldn't do anything except canonicalize.
        { "filesystem:http://u:p@bar.com/t/gaba?query#ref", "http", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
            "filesystem:http://bar.com/t/gaba?query#ref" },
        // Replace username -- shouldn't do anything except canonicalize.
        { "filesystem:http://u:p@bar.com/t/gaba?query#ref", nullptr, "u2", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
            "filesystem:http://bar.com/t/gaba?query#ref" },
        // Replace password -- shouldn't do anything except canonicalize.
        { "filesystem:http://u:p@bar.com/t/gaba?query#ref", nullptr, nullptr, "pw2", nullptr, nullptr, nullptr, nullptr, nullptr,
            "filesystem:http://bar.com/t/gaba?query#ref" },
        // Replace host -- shouldn't do anything except canonicalize.
        { "filesystem:http://u:p@bar.com:80/t/gaba?query#ref", nullptr, nullptr, nullptr, "foo.com", nullptr, nullptr, nullptr, nullptr,
            "filesystem:http://bar.com/t/gaba?query#ref" },
        // Replace port -- shouldn't do anything except canonicalize.
        { "filesystem:http://u:p@bar.com:40/t/gaba?query#ref", nullptr, nullptr, nullptr, nullptr, "41", nullptr, nullptr, nullptr,
...
TEST_F(URLCanonTest, ReplacePathURL)
{
    ReplaceCase replace_cases[] = {
        // Replace everything
        { "data:foo", "javascript", nullptr, nullptr, nullptr, nullptr, "alert('foo?');", nullptr, nullptr, "javascript:alert('foo?');" },
        // Replace nothing
        { "data:foo", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, "data:foo" },
        // Replace one or the other
        { "data:foo", "javascript", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, "javascript:foo" },
        { "data:foo", nullptr, nullptr, nullptr, nullptr, nullptr, "bar", nullptr, nullptr, "data:bar" },
        { "data:foo", nullptr, nullptr, nullptr, nullptr, nullptr, kDeleteComp, nullptr, nullptr, "data:" },
    };
    ...
    for (const auto& replace_case : replace_cases) {
        ...
        StdStringCanonOutput output(&out_str);
        Parsed out_parsed;
        ReplacePathURL(cur.base, ParsePathURL(cur.base, false), r, &output, &out_parsed);
        output.Complete();
...
TEST_F(URLCanonTest, ReplaceMailtoURL)
{
    ReplaceCase replace_cases[] = {
        // Replace everything
        { "mailto:jon@foo.com?body=sup", "mailto", nullptr, nullptr, nullptr, nullptr, "addr1", "to=tony", nullptr, "mailto:addr1?to=tony" },
        // Replace nothing
        { "mailto:jon@foo.com?body=sup", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, "mailto:jon@foo.com?body=sup" },
        // Replace the path
        { "mailto:jon@foo.com?body=sup", nullptr, nullptr, nullptr, nullptr, nullptr, "jason", nullptr, nullptr, "mailto:jason?body=sup" },
        // Replace the query
        { "mailto:jon@foo.com?body=sup", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, "custom=1", nullptr, "mailto:jon@foo.com?custom=1" },
        // Replace the path and query
        { "mailto:jon@foo.com?body=sup", nullptr, nullptr, nullptr, nullptr, nullptr, "jason", "custom=1", nullptr, "mailto:jason?custom=1" },
        // Set the query to empty (should leave trailing question mark)
        { "mailto:jon@foo.com?body=sup", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, "", nullptr, "mailto:jon@foo.com?" },
        // Clear the query
        { "mailto:jon@foo.com?body=sup", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, "|", nullptr, "mailto:jon@foo.com" },
        // Clear the path
        { "mailto:jon@foo.com?body=sup", nullptr, nullptr, nullptr, nullptr, nullptr, "|", nullptr, nullptr, "mailto:?body=sup" },
        // Clear the path + query
        { "mailto:", nullptr, nullptr, nullptr, nullptr, nullptr, "|", "|", nullptr, "mailto:" },
        // Setting the ref should have no effect
        { "mailto:addr1", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, "BLAH", "mailto:addr1" },
    };
...
TEST_F(URLCanonTest, CanonicalizeFileURL)
{
    struct URLCase {
        ...
        // Busted refs shouldn't make the whole thing fail.
        { "file:///C:/asdf#\xc2", "file:///C:/asdf#%EF%BF%BD", true, Component(), Component(7, 8) },
    ...
    for (const auto& i : cases) {
        ...
        std::string out_str;
        StdStringCanonOutput output(&out_str);
        bool success = CanonicalizeFileURL(i.input, static_cast<int>(strlen(i.input)), parsed, nullptr, &output, &out_parsed);
        output.Complete();
...
TEST_F(URLCanonTest, CanonicalizeFileSystemURL)
{
    ...
    for (const auto& i : cases) {
        ...
        std::string out_str;
        StdStringCanonOutput output(&out_str);
        bool success = CanonicalizeFileSystemURL(i.input, parsed, nullptr, &output, &out_parsed);
        output.Complete();
...
TEST_F(URLCanonTest, CanonicalizePathURL)
{
    ...
    for (const auto& path_case : path_cases) {
        ...
        std::string out_str;
        StdStringCanonOutput output(&out_str);
        bool success = CanonicalizePathURL(path_case.input, url_len, ParsePathURL(path_case.input, true), &output, &out_parsed);
        output.Complete();
...
TEST_F(URLCanonTest, CanonicalizePathURLPath)
{
    struct PathCase {
        ...
        { "Foo", L"Foo", "Foo" },
        { "\":This /is interesting;?#", L"\":This /is interesting;?#", "\":This /is interesting;?#" },
        { "\uFFFF", L"\uFFFF", "%EF%BF%BF" },
    };
    ...
    for (const auto& path_case : path_cases) {
        ...
        CanonicalizePathURLPath(path_case.input.data(), Component(0, path_case.input.size()), &output, &out_component);
        output.Complete();

        EXPECT_EQ(path_case.expected, out_str);

        EXPECT_EQ(0, out_component.begin);
        EXPECT_EQ(path_case.expected.size(), static_cast<size_t>(out_component.len));

        // 16-bit string input
        std::string out_str16;
        StdStringCanonOutput output16(&out_str16);
        url::Component out_component16;
        std::u16string input16(test_utils::TruncateWStringToUTF16(path_case.input16.data()));
        CanonicalizePathURLPath(input16.c_str(), Component(0, path_case.input16.size()), &output16, &out_component16);
        output16.Complete();

        EXPECT_EQ(path_case.expected, out_str16);

        EXPECT_EQ(0, out_component16.begin);
        EXPECT_EQ(path_case.expected.size(), static_cast<size_t>(out_component16.len));
    }
}
...
TEST_F(URLCanonTest, CanonicalizeMailtoURL)
{
    struct URLCase {
        ...
        { "mailto:addr1, addr2", "mailto:addr1,%20addr2", true, Component(7, 14), Component() },
        { "mailto:addr1, addr2?subject=one two ", "mailto:addr1,%20addr2?subject=one%20two", true, Component(7, 14), Component(22, 17) },
        ...
        { "mailto:\xed\xa0\x80", "mailto:%EF%BF%BD%EF%BF%BD%EF%BF%BD", false, Component(7, 27), Component() },
        ...
        { "mailto:! \x22$&()+,-./09:;<=>@AZ[\\]&_`az{|}~\x7f?Query! \x22$&()+,-./09:;<=>@AZ[\\]&_`az{|}~",
            "mailto:!%20%22$&()+,-./09:;%3C=%3E@AZ[\\]&_%60az%7B%7C%7D~%7F?Query!%20%22$&()+,-./09:;%3C=%3E@AZ[\\]&_`az{|}~", true, Component(7, 53),
            Component(61, 47) },
    };
    ...
    for (size_t i = 0; i < std::size(cases); i++) {
        ...
        std::string out_str;
        StdStringCanonOutput output(&out_str);
        bool success = CanonicalizeMailtoURL(cases[i].input, url_len, ParseMailtoURL(std::string_view(cases[i].input, url_len)), &output, &out_parsed);
        output.Complete();
...
{
    return a.scheme.begin == b.scheme.begin && a.scheme.len == b.scheme.len && a.username.begin == b.username.begin && a.username.len == b.username.len
        && a.password.begin == b.password.begin && a.password.len == b.password.len && a.host.begin == b.host.begin && a.host.len == b.host.len
        && a.port.begin == b.port.begin && a.port.len == b.port.len && a.path.begin == b.path.begin && a.path.len == b.path.len
        && a.query.begin == b.query.begin && a.query.len == b.query.len && a.ref.begin == b.ref.begin && a.ref.len == b.ref.len;
}
...
TEST_F(URLCanonTest, ResolveRelativeURL)
{
    struct RelativeCase {
        ...
        { "http://host/a", true, false, "http://another/", true, false, false, nullptr },
        { "http://host/a", true, false, "http:////another/", true, false, false, nullptr },
        ...
        { "http://host/a", true, false, "https:host2", true, false, false, nullptr },
        { "http://host/a", true, false, "htto:/host2", true, false, false, nullptr },
        // Absolute path input
        ...
        { "file:///C:/foo", true, true, "http://host/", true, false, false, nullptr },
        ...
        // as absolute.
        { "http://host/a", true, false, "\\\\another\\path", true, false, false, nullptr },
        ...
        // to http:index.html.
        { "filesystem:http://host/t/path", true, false, "filesystem:http://host/t/path2", true, false, false, nullptr },
        { "filesystem:http://host/t/path", true, false, "filesystem:https://host/t/path2", true, false, false, nullptr },
        { "filesystem:http://host/t/path", true, false, "http://host/t/path2", true, false, false, nullptr },
        { "http://host/t/path", true, false, "filesystem:http://host/t/path2", true, false, false, nullptr },
        ...
        { "filesystem:http://host/t/path", true, false, "filesystem:http:path2", true, false, false, nullptr },
    ...
    for (const auto& cur_case : rel_cases) {
        ...
        bool succeed_is_rel = IsRelativeURL(cur_case.base, parsed, cur_case.test, test_len, cur_case.is_base_hier, &is_relative, &relative_component);
        ...
        if (succeed_is_rel && is_relative && cur_case.is_rel) {
            ...
            bool succeed_resolve
                = ResolveRelativeURL(cur_case.base, parsed, cur_case.is_base_file, cur_case.test, relative_component, nullptr, &output, &resolved_parsed);
            output.Complete();
...
public:
        ...
        : use_standard_compliant_non_special_scheme_url_parsing_(GetParam())
    {
        if (use_standard_compliant_non_special_scheme_url_parsing_) {
            scoped_feature_list_.InitAndEnableFeature(kStandardCompliantNonSpecialSchemeURLParsing);
        } else {
            scoped_feature_list_.InitAndDisableFeature(kStandardCompliantNonSpecialSchemeURLParsing);
        }
    }
...
protected:
    ...
    void TestNonSpecialResolveRelativeURL(const ResolveRelativeURLCase& relative_case)
    {
        // The following test is similar to URLCanonTest::ResolveRelativeURL, but
        // simplified.
        Parsed parsed = use_standard_compliant_non_special_scheme_url_parsing_ ? ParseNonSpecialURL(relative_case.base)
        ...
        bool is_relative;
        Component relative_component;
        bool succeed_is_rel = IsRelativeURL(relative_case.base.data(), parsed, relative_case.rel.data(), relative_case.rel.size(), relative_case.is_base_hier,
        ...
        if (succeed_is_rel && is_relative) {
            ...
            bool succeed_resolve = ResolveRelativeURL(relative_case.base.data(), parsed, relative_case.is_base_hier, relative_case.rel.data(),
...
TEST_F(URLCanonTest, ReplacementOverflow)
{
    ...
    repl.SetQuery(new_query.c_str(), Component(0, static_cast<int>(new_query.length())));
...
TEST_F(URLCanonTest, DefaultPortForScheme)
{
    ...
    for (const auto& test_case : cases) {
        ...
        EXPECT_EQ(test_case.expected_port, DefaultPortForScheme(std::string_view(test_case.scheme, strlen(test_case.scheme))));
    }
}
...
TEST_F(URLCanonTest, FindWindowsDriveLetter)
{
    struct TestCase {
        ...
        { "c:/foo", 0, -1, 0 }, { "/c:/foo", 0, -1, 1 }, { "//c:/foo", 0, -1, -1 }, // "//" does not canonicalize to "/"
        { "\\C|\\foo", 0, -1, 1 }, { "/cd:/foo", 0, -1, -1 }, // "/c" does not canonicalize to "/"
        { "/./c:/foo", 0, -1, 3 }, { "/.//c:/foo", 0, -1, -1 }, // "/.//" does not canonicalize to "/"
        { "/././c:/foo", 0, -1, 5 }, { "/abc/c:/foo", 0, -1, -1 }, // "/abc/" does not canonicalize to "/"
        { "/abc/./../c:/foo", 0, -1, 10 },
    ...
    for (const auto& c : cases) {
        ...
        EXPECT_EQ(c.expected_drive_letter_pos, FindWindowsDriveLetter(c.spec.data(), c.begin, end))
            << "for " << c.spec << "[" << c.begin << ":" << end << "] (UTF-8)";

        std::u16string spec16 = base::ASCIIToUTF16(c.spec);
        EXPECT_EQ(c.expected_drive_letter_pos, FindWindowsDriveLetter(spec16.data(), c.begin, end))
...
void ComponentCaseMatches(bool success, std::string_view out_str, const Component& out_comp, const DualComponentCase& expected)
{
...
TEST_F(URLCanonTest, OpaqueHost)
{
    ...
    for (const auto& host_case : host_cases) {
        ...
        StdStringCanonOutput output(&out_str);
        Component out_comp;
        bool success = CanonicalizeNonSpecialHost(host_case.input8, Component(0, static_cast<int>(strlen(host_case.input8))), output, out_comp);
        output.Complete();
    ...
    for (const auto& host_case : host_cases) {
        ...
        StdStringCanonOutput output(&out_str);
        Component out_comp;
        bool success = CanonicalizeNonSpecialHost(input16.c_str(), Component(0, static_cast<int>(input16.length())), output, out_comp);
        output.Complete();
        ComponentCaseMatches(success, out_str, out_comp, host_case);
    }
}

void IPAddressCaseMatches(std::string_view out_str, const CanonHostInfo& host_info, const IPAddressCase& expected)
{
    EXPECT_EQ(host_info.family, expected.expected_family);
    EXPECT_STREQ(out_str.data(), expected.expected);
    EXPECT_EQ(base::HexEncode(host_info.address, static_cast<size_t>(host_info.AddressLength())), expected.expected_address_hex);
    if (expected.expected_family == CanonHostInfo::IPV4) {
        EXPECT_EQ(host_info.num_ipv4_components, expected.expected_num_ipv4_components);
    }
}
...
TEST_F(URLCanonTest, NonSpecialHostIPv6Address)
{
    IPAddressCase ip_address_cases[] = {
        ...
        { "192.168.0.1", L"192.168.0.1", "192.168.0.1", Component(0, 11), CanonHostInfo::NEUTRAL, 0, "" },
        { "192", L"192", "192", Component(0, 3), CanonHostInfo::NEUTRAL, 0, "" },
        // "257" is allowed since the number is not considered as a part of IPv4.
        { "192.168.0.257", L"192.168.0.257", "192.168.0.257", Component(0, 13), CanonHostInfo::NEUTRAL, 0, "" },
        // IPv6.
        { "[1:0:0:2::3:0]", L"[1:0:0:2::3:0]", "[1::2:0:0:3:0]", Component(0, 14), CanonHostInfo::IPV6, -1, "00010000000000020000000000030000" },
        { "[::]", L"[::]", "[::]", Component(0, 4), CanonHostInfo::IPV6, -1, "00000000000000000000000000000000" },
        // Invalid hosts.
    ...
    for (const auto& ip_address_case : ip_address_cases) {
        SCOPED_TRACE(testing::Message() << "url: \"" << ip_address_case.input8 << "\"");
        std::string out_str;
        StdStringCanonOutput output(&out_str);
        CanonHostInfo host_info;
        CanonicalizeNonSpecialHostVerbose(ip_address_case.input8, Component(0, static_cast<int>(strlen(ip_address_case.input8))), output, host_info);
        output.Complete();
    ...
    // UTF-16 version.
    for (const auto& ip_address_case : ip_address_cases) {
        SCOPED_TRACE(testing::Message() << "url: \"" << ip_address_case.input16 << "\"");
        std::u16string input16(test_utils::TruncateWStringToUTF16(ip_address_case.input16));
        std::string out_str;
        StdStringCanonOutput output(&out_str);
        CanonHostInfo host_info;
        CanonicalizeNonSpecialHostVerbose(input16.c_str(), Component(0, static_cast<int>(input16.length())), output, host_info);
        output.Complete();
        IPAddressCaseMatches(out_str, host_info, ip_address_case);
    }
}
