...
{
    return os << "{ scheme: " << parsed.scheme << ", username: " << parsed.username << ", password: " << parsed.password << ", host: " << parsed.host
              << ", port: " << parsed.port << ", path: " << parsed.path << ", query: " << parsed.query << ", ref: " << parsed.ref
...
// to spec_len.
template <typename CHAR> int FindNextAuthorityTerminator(const CHAR* spec, int start_offset, int spec_len, ParserMode parser_mode)
{
...
template <typename CHAR> void ParseUserInfo(const CHAR* spec, const Component& user, Component* username, Component* password)
{
...
template <typename CHAR> void ParseServerInfo(const CHAR* spec, const Component& serverinfo, Component* hostname, Component* port_num)
{
    if (serverinfo.len == 0) {
...
void DoParseAuthority(
    const CHAR* spec, const Component& auth, ParserMode parser_mode, Component* username, Component* password, Component* hostname, Component* port_num)
{
...
template <typename CHAR> inline void FindQueryAndRefParts(const CHAR* spec, const Component& path, int* query_separator, int* ref_separator)
{
    if constexpr (sizeof(*spec) == 1) {
        // memchr is much faster than any scalar code we can write.
        const CHAR* ptr = spec + path.begin;
        const CHAR* first_hash = reinterpret_cast<const CHAR*>(memchr(ptr, '#', path.len));
        size_t len_before_fragment = first_hash == nullptr ? path.len : first_hash - ptr;
        const CHAR* first_question = reinterpret_cast<const CHAR*>(memchr(ptr, '?', len_before_fragment));
...
template <typename CHAR> void ParsePath(const CHAR* spec, const Component& path, Component* filepath, Component* query, Component* ref)
{
...
template <typename CharT> bool DoExtractScheme(std::basic_string_view<CharT> url, Component* scheme)
{
...
template <typename CHAR> void DoParseAfterSpecialScheme(const CHAR* spec, int spec_len, int after_scheme, Parsed* parsed)
{
    ...
    // the authority.
    int end_auth = FindNextAuthorityTerminator(spec, after_slashes, spec_len, ParserMode::kSpecialURL);
    ...
    DoParseAuthority(spec, authority, ParserMode::kSpecialURL, &parsed->username, &parsed->password, &parsed->host, &parsed->port);
    ParsePath(spec, full_path, &parsed->path, &parsed->query, &parsed->ref);
}

// The main parsing function for standard URLs. Standard URLs have a scheme,
// host, path, etc.
template <typename CharT> Parsed DoParseStandardURL(std::basic_string_view<CharT> url)
{
...
template <typename CHAR> void DoParseAfterNonSpecialScheme(const CHAR* spec, int spec_len, int after_scheme, Parsed* parsed)
{
    ...
    int num_slashes = CountConsecutiveSlashesButNotCountBackslashes(spec, after_scheme, spec_len);
    ...
    if (num_slashes >= 2) {
        ...
        // authority.
        int end_auth = FindNextAuthorityTerminator(spec, after_slashes, spec_len, ParserMode::kNonSpecialURL);
        Component authority(after_slashes, end_auth - after_slashes);

        // Now parse those two sub-parts.
        DoParseAuthority(spec, authority, ParserMode::kNonSpecialURL, &parsed->username, &parsed->password, &parsed->host, &parsed->port);
...
template <typename CharT> Parsed DoParseNonSpecialURL(std::basic_string_view<CharT> url, bool trim_path_end)
{
    // Strip leading & trailing spaces and control characters.
    int begin = 0;
    int url_len = base::checked_cast<int>(url.size());
...
template <typename CharT> Parsed DoParseFileSystemURL(std::basic_string_view<CharT> url)
{
    // Strip leading & trailing spaces and control characters.
    int begin = 0;
    int url_len = base::checked_cast<int>(url.size());
    ...
    std::basic_string_view inner_url = url.substr(inner_start, url_len - inner_start);
    ...
    } else if (CompareSchemeComponent(url.data(), inner_scheme, kFileSystemScheme)) {
...
template <typename CharT> Parsed DoParsePathURL(std::basic_string_view<CharT> url, bool trim_path_end)
{
    // Strip leading & trailing spaces and control characters.
    int scheme_begin = 0;
    int url_len = base::checked_cast<int>(url.size());
    ...
    if (ExtractScheme(&url[scheme_begin], url_len - scheme_begin, &parsed.scheme)) {
        // Offset the results since we gave ExtractScheme a substring.
    ...
    ParsePath(url.data(), MakeRange(path_begin, url_len), &parsed.path, &parsed.query, &parsed.ref);
    return parsed;
}

template <typename CharT> Parsed DoParseMailtoURL(std::basic_string_view<CharT> url)
{
    // Strip leading & trailing spaces and control characters.
    int begin = 0;
...
template <typename CHAR> int DoParsePort(const CHAR* spec, const Component& component)
{
...
template <typename CHAR> void DoExtractFileName(const CHAR* spec, const Component& path, Component* file_name)
{
...
template <typename CharT> bool DoExtractQueryKeyValue(std::basic_string_view<CharT> spec, Component* query, Component* key, Component* value)
{
...
void ExtractFileName(const char16_t* url, const Component& path, Component* file_name)
{
    DoExtractFileName(url, path, file_name);
}
...
void ParseAuthority(const char* spec, const Component& auth, Component* username, Component* password, Component* hostname, Component* port_num)
{
    DoParseAuthority(spec, auth, ParserMode::kSpecialURL, username, password, hostname, port_num);
}

void ParseAuthority(const char16_t* spec, const Component& auth, Component* username, Component* password, Component* hostname, Component* port_num)
{
    DoParseAuthority(spec, auth, ParserMode::kSpecialURL, username, password, hostname, port_num);
}
...
void ParseAuthority(
    const char* spec, const Component& auth, ParserMode parser_mode, Component* username, Component* password, Component* hostname, Component* port_num)
{
    DoParseAuthority(spec, auth, parser_mode, username, password, hostname, port_num);
}

void ParseAuthority(
    const char16_t* spec, const Component& auth, ParserMode parser_mode, Component* username, Component* password, Component* hostname, Component* port_num)
{
    DoParseAuthority(spec, auth, parser_mode, username, password, hostname, port_num);
}
...
void ParsePathInternal(const char16_t* spec, const Component& path, Component* filepath, Component* query, Component* ref)
{
    ParsePath(spec, path, filepath, query, ref);
}
...
void ParseAfterSpecialScheme(const char16_t* spec, int spec_len, int after_scheme, Parsed* parsed)
{
    DoParseAfterSpecialScheme(spec, spec_len, after_scheme, parsed);
}
...
void ParseAfterNonSpecialScheme(const char16_t* spec, int spec_len, int after_scheme, Parsed* parsed)
{
    DoParseAfterNonSpecialScheme(spec, spec_len, after_scheme, parsed);
}
