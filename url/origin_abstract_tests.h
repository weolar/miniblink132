...
template <typename TOriginTraits> class AbstractOriginTest : public testing::Test {
public:
    void SetUp() override
    {
...
protected:
    ...
    OriginType CreateWithReferenceOrigin(std::string_view url, const OriginType& reference_origin)
    {
...
#define EXPECT_SAME_ORIGIN(a, b)                                                                                                                               \
    EXPECT_TRUE(this->IsSameOrigin((a), (b))) << "When checking if \"" << this->Serialize(a) << "\" is "                                                       \
                                              << "same-origin with \"" << this->Serialize(b) << "\""

#define EXPECT_CROSS_ORIGIN(a, b)                                                                                                                              \
    EXPECT_FALSE(this->IsSameOrigin((a), (b))) << "When checking if \"" << this->Serialize(a) << "\" is "                                                      \
    ...
    void VerifyOriginInvariants(const OriginType& origin)
    {
        ...
        auto different_tuple_origin = this->CreateOriginFromString("https://not-in-the-list.test/");
        EXPECT_CROSS_ORIGIN(origin, different_tuple_origin);

        // Deriving an origin for "about:blank".
        auto about_blank_origin1 = this->CreateWithReferenceOrigin("about:blank", origin);
        auto about_blank_origin2 = this->CreateWithReferenceOrigin("about:blank?bar#foo", origin);
        ...
        for (size_t i = 0; i < derived_origins.size(); i++) {
            ...
            EXPECT_EQ(this->GetTupleOrPrecursorTupleIfOpaque(origin), this->GetTupleOrPrecursorTupleIfOpaque(derived_origin));
        }
    }
    ...
    void VerifyUniqueOpaqueOriginInvariants(const OriginType& origin)
    {
        if (!this->IsOpaque(origin)) {
            ADD_FAILURE() << "Got unexpectedly non-opaque origin: " << this->Serialize(origin);
    ...
    void TestUniqueOpaqueOrigin(std::string_view test_input)
    {
        ...
        auto origin_recreated_from_same_input = this->CreateOriginFromString(test_input);
        EXPECT_CROSS_ORIGIN(origin, origin_recreated_from_same_input);
    }

    void VerifyTupleOriginInvariants(const OriginType& origin, const SchemeHostPort& expected_tuple)
    {
        if (this->IsOpaque(origin)) {
            ADD_FAILURE() << "Got unexpectedly opaque origin";
            return; // Skip other test assertions.
        }
        SCOPED_TRACE(testing::Message() << "Actual origin: " << this->Serialize(origin));
        ...
        if (expected_tuple.scheme() != kFileScheme || expected_tuple.host() == "") {
            EXPECT_SAME_ORIGIN(origin, this->CreateOriginFromString(this->Serialize(origin)));
        }
...
TYPED_TEST_P(AbstractOriginTest, AndroidWebViewHackWithStandardCompliantNonSpecialSchemeURLParsing)
{
    EnableNonStandardSchemesForAndroidWebView();
    ...
    for (bool flag : { false, true }) {
        ...
        if (flag) {
            scoped_feature_list.InitAndEnableFeature(kStandardCompliantNonSpecialSchemeURLParsing);
        } else {
            scoped_feature_list.InitAndDisableFeature(kStandardCompliantNonSpecialSchemeURLParsing);
        }
        ...
        // Non-Standard scheme cases.
        {
            ...
            auto another_origin_a = this->CreateOriginFromString("another-non-standard://a.com:80");
...
TYPED_TEST_P(AbstractOriginTest, TupleOrigins)
{
    struct TestCase {
        ...
        { "filesystem:local-std-with-host:baz./type/", { "local-std-with-host", "baz.", 0 } },

        // IP Addresses
        { "http://192.168.9.1/", { "http", "192.168.9.1", 80 } },
        { "http://[2001:db8::1]/", { "http", "[2001:db8::1]", 80 } },
        { "http://[2001:0db8:0000:0000:0000:0000:0000:0001]/", { "http", "[2001:db8::1]", 80 } },
        ...
        { "local-std-with-host:↑↑↓↓←→←→ba.↑↑↓↓←→←→ba.0.bg", { "local-std-with-host", "xn--ba-rzuadaibfa.xn--ba-rzuadaibfa.0.bg", 0 } },
...
TYPED_TEST_P(AbstractOriginTest, CustomSchemes_TupleOrigins_StandardCompliantNonSpecialSchemeFlag)
{
    ...
    for (bool flag : { false, true }) {
        ...
        for (const TestCase& test : test_cases) {
            ...
            this->VerifyTupleOriginInvariants(
                origin, flag ? test.expected_tuple_when_standard_compliant_flag_on : test.expected_tuple_when_standard_compliant_flag_off);
        }
    }
}

REGISTER_TYPED_TEST_SUITE_P(AbstractOriginTest, NonStandardSchemeWithAndroidWebViewHack, AndroidWebViewHackWithStandardCompliantNonSpecialSchemeURLParsing,
    OpaqueOriginsFromValidUrls, OpaqueOriginsFromInvalidUrls, TupleOrigins, CustomSchemes_OpaqueOrigins, CustomSchemes_TupleOrigins,
