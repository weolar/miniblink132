...
bool IsCanonicalHost(std::string_view host, bool is_file_scheme)
{
    ...
    if (is_file_scheme) {
        CanonicalizeFileHostVerbose(host.data(), raw_host_component, canon_host_output, host_info);
    } else {
        CanonicalizeSpecialHostVerbose(host.data(), raw_host_component, canon_host_output, host_info);
    }
...
// differences).
bool IsValidInput(std::string_view scheme, std::string_view host, uint16_t port, SchemeHostPort::ConstructPolicy policy)
{
    ...
    bool is_standard = GetStandardSchemeType(scheme.data(), Component(0, base::checked_cast<int>(scheme.length())), &scheme_type);
    if (!is_standard) {
    ...
    case SCHEME_WITH_HOST_PORT_AND_USER_INFORMATION:
        ...
        // canonicalized.
        DCHECK(policy == SchemeHostPort::CHECK_CANONICALIZATION || IsCanonicalHost(host, scheme == url::kFileScheme));
        if (policy == SchemeHostPort::CHECK_CANONICALIZATION && !IsCanonicalHost(host, scheme == url::kFileScheme)) {
            return false;
        }
...
SchemeHostPort::SchemeHostPort(std::string scheme, std::string host, uint16_t port, ConstructPolicy policy)
{
    ...
    host_ = std::move(host);
    port_ = port;
    DCHECK(IsValid()) << "Scheme: " << scheme_ << " Host: " << host_ << " Port: " << port;
}
...
{
    //return base::trace_event::EstimateMemoryUsage(scheme_) + base::trace_event::EstimateMemoryUsage(host_);
    *(int*)1 = 1;
    return 0;
}
...
{
    return IsAndroidWebViewHackEnabledScheme(scheme) && IsUsingStandardCompliantNonSpecialSchemeURLParsing();
}
