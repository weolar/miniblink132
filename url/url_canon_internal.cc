...
template <typename CHAR, typename UCHAR> void DoAppendStringOfType(const CHAR* source, size_t length, SharedCharTypes type, CanonOutput* output)
{
    size_t i = 0;
...
template <typename CHAR, typename UCHAR> void DoAppendInvalidNarrowString(const CHAR* spec, size_t begin, size_t end, CanonOutput* output)
{
...
void DoOverrideComponent(const char* override_source, const Component& override_component, const char** dest, Component* dest_component)
{
    if (override_source) {
...
bool PrepareUTF16OverrideComponent(const char16_t* override_source, const Component& override_component, CanonOutput* utf8_buffer, Component* dest_component)
{
    bool success = true;
    if (override_source) {
        if (!override_component.is_valid()) {
            ...
            *dest_component = Component();
            ...
            success = ConvertUTF16ToUTF8(&override_source[override_component.begin], static_cast<size_t>(override_component.len), utf8_buffer);
