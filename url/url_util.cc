...
template <typename CHAR> inline bool DoCompareSchemeComponent(const CHAR* spec, const Component& component, const char* compare_to)
{
...
template <typename CHAR> bool DoIsInSchemes(const CHAR* spec, const Component& scheme, SchemeType* type, const std::vector<SchemeWithType>& schemes)
{
    if (scheme.is_empty())
        return false; // Empty or invalid schemes are non-standard.

    for (const SchemeWithType& scheme_with_type : schemes) {
        if (base::EqualsCaseInsensitiveASCII(std::basic_string_view(&spec[scheme.begin], scheme.len), scheme_with_type.scheme)) {
...
template <typename CHAR> bool DoIsStandard(const CHAR* spec, const Component& scheme, SchemeType* type)
{
    return DoIsInSchemes(spec, scheme, type, GetSchemeRegistry().standard_schemes);
}

template <typename CHAR> bool DoIsOpaqueNonSpecial(const CHAR* spec, const Component& scheme)
{
    if (scheme.is_empty()) {
        return false;
    }
    for (const std::string& s : GetSchemeRegistry().opaque_non_special_schemes) {
        if (base::EqualsCaseInsensitiveASCII(std::basic_string_view(&spec[scheme.begin], scheme.len), s)) {
            return true;
        }
    }
    return false;
...
template <typename CHAR> bool DoFindAndCompareScheme(const CHAR* str, int str_len, const char* compare, Component* found_scheme)
{
...
bool DoCanonicalize(const CHAR* spec, int spec_len, bool trim_path_end, WhitespaceRemovalPolicy whitespace_policy, CharsetConverter* charset_converter,
    CanonOutput* output, Parsed* output_parsed)
{
    ...
    if (whitespace_policy == REMOVE_WHITESPACE) {
        spec = RemoveURLWhitespace(spec, spec_len, &whitespace_buffer, &spec_len, &output_parsed->potentially_dangling_markup);
    }
    ...
    if (DoesBeginUNCPath(spec, 0, spec_len, false) || DoesBeginWindowsDriveSpec(spec, 0, spec_len)) {
        return CanonicalizeFileURL(spec, spec_len, ParseFileURL(std::basic_string_view(spec, spec_len)), charset_converter, output, output_parsed);
    }
#endif
    ...
    if (DoCompareSchemeComponent(spec, scheme, url::kFileScheme)) {
        ...
        success = CanonicalizeFileURL(spec, spec_len, ParseFileURL(std::basic_string_view(spec, spec_len)), charset_converter, output, output_parsed);
    } else if (DoCompareSchemeComponent(spec, scheme, url::kFileSystemScheme)) {
        // Filesystem URLs are special.
        success = CanonicalizeFileSystemURL(spec, ParseFileSystemURL(std::basic_string_view(spec, spec_len)), charset_converter, output, output_parsed);

    } else if (DoIsStandard(spec, scheme, &scheme_type)) {
        // All "normal" URLs.
        success
            = CanonicalizeStandardURL(spec, ParseStandardURL(std::basic_string_view(spec, spec_len)), scheme_type, charset_converter, output, output_parsed);

    } else if (!url::IsUsingStandardCompliantNonSpecialSchemeURLParsing() && DoCompareSchemeComponent(spec, scheme, url::kMailToScheme)) {
        ...
        success = CanonicalizeMailtoURL(spec, spec_len, ParseMailtoURL(std::basic_string_view(spec, spec_len)), output, output_parsed);
        ...
        if (url::IsUsingStandardCompliantNonSpecialSchemeURLParsing() && !DoIsOpaqueNonSpecial(spec, scheme)) {
            success = CanonicalizeNonSpecialURL(
                spec, spec_len, ParseNonSpecialURLInternal(std::basic_string_view(spec, spec_len), trim_path_end), charset_converter, *output, *output_parsed);
        } else {
            success = CanonicalizePathURL(spec, spec_len, ParsePathURL(std::basic_string_view(spec, spec_len), trim_path_end), output, output_parsed);
        }
    }
    return success;
}
...
template <typename CHAR>
bool DoResolveRelative(const char* base_spec, int base_spec_len, const Parsed& base_parsed, const CHAR* in_relative, int in_relative_length,
    CharsetConverter* charset_converter, CanonOutput* output, Parsed* output_parsed)
{
    ...
    int relative_length;
    const CHAR* relative
        = RemoveURLWhitespace(in_relative, in_relative_length, &whitespace_buffer, &relative_length, &output_parsed->potentially_dangling_markup);
    ...
    if (url::IsUsingStandardCompliantNonSpecialSchemeURLParsing()) {
        ...
        is_hierarchical_base = base_parsed.scheme.is_nonempty() && DoIsStandard(base_spec, base_parsed.scheme, &unused_scheme_type);
    }

    bool is_relative;
    Component relative_component;
    if (!IsRelativeURL(base_spec, base_parsed, relative, relative_length, (base_is_hierarchical || is_hierarchical_base), &is_relative, &relative_component)) {
        // Error resolving.
        return false;
    }
    ...
    if (is_relative && base_is_authority_based && !is_hierarchical_base) {
        Parsed base_parsed_authority = ParseStandardURL(std::string_view(base_spec, base_spec_len));
        if (base_parsed_authority.host.is_nonempty()) {
            ...
            bool did_resolve_succeed = ResolveRelativeURL(
                base_spec, base_parsed_authority, false, relative, relative_component, charset_converter, &temporary_output, output_parsed);
            ...
            // re-created.
            DoCanonicalize(temporary_output.data(), temporary_output.length(), true, REMOVE_WHITESPACE, charset_converter, output, output_parsed);
    ...
    return DoCanonicalize(relative, relative_length, true, DO_NOT_REMOVE_WHITESPACE, charset_converter, output, output_parsed);
}

template <typename CHAR>
bool DoReplaceComponents(const char* spec, int spec_len, const Parsed& parsed, const Replacements<CHAR>& replacements, CharsetConverter* charset_converter,
    CanonOutput* output, Parsed* out_parsed)
{
    ...
    if (replacements.IsSchemeOverridden()) {
        ...
        CanonicalizeScheme(replacements.sources().scheme, replacements.components().scheme, &scheme_replaced, &scheme_replaced_parsed);
        ...
        DoCanonicalize(scheme_replaced.data(), scheme_replaced.length(), true, REMOVE_WHITESPACE, charset_converter, &recanonicalized, &recanonicalized_parsed);
        ...
        return DoReplaceComponents(
            recanonicalized.data(), recanonicalized.length(), recanonicalized_parsed, replacements_no_scheme, charset_converter, output, out_parsed);
    }
    ...
    if (!IsUsingStandardCompliantNonSpecialSchemeURLParsing() && DoCompareSchemeComponent(spec, parsed.scheme, url::kMailToScheme)) {
        return ReplaceMailtoURL(spec, parsed, replacements, output, out_parsed);
    }

    if (IsUsingStandardCompliantNonSpecialSchemeURLParsing() && !DoIsOpaqueNonSpecial(spec, parsed.scheme)) {
        return ReplaceNonSpecialURL(spec, parsed, replacements, charset_converter, *output, *out_parsed);
    }
...
void DoSchemeModificationPreamble()
{
    ...
    DCHECK(!g_scheme_registries_used.load()) << "Trying to add a scheme after the lists have been used. "
...
void DoAddSchemeWithHandler(const char* new_scheme, const char* handler, std::vector<SchemeWithHandler>* schemes)
{
...
{
    DCHECK(!g_scheme_registries_used.load()) << "Schemes already used "
...
public:
    ScopedSchemeRegistryInternal()
        : registry_(std::make_unique<SchemeRegistry>(*GetSchemeRegistryWithoutLocking()))
    {
...
{
    DoAddSchemeWithHandler(new_scheme, handler, &GetSchemeRegistryWithoutLocking()->predefined_handler_schemes);
}
...
std::vector<std::pair<std::string, std::string>> GetPredefinedHandlerSchemes()
{
    ...
    for (const SchemeWithHandler& entry : GetSchemeRegistry().predefined_handler_schemes) {
...
{
    return IsStandard(scheme.data(), Component(0, base::checked_cast<int>(scheme.size())));
}
...
bool DomainIs(std::string_view canonical_host, std::string_view canonical_domain)
{
    ...
    if (std::string_view(host_first_pos, canonical_domain.length()) != canonical_domain) {
        return false;
    }
...
bool HostIsIPAddress(std::string_view host)
{
    ...
    url::CanonicalizeIPAddress(host.data(), Component(0, host.length()), &ignored_output, &host_info);
    return host_info.IsIPAddress();
}

bool Canonicalize(const char* spec, int spec_len, bool trim_path_end, CharsetConverter* charset_converter, CanonOutput* output, Parsed* output_parsed)
{
    return DoCanonicalize(spec, spec_len, trim_path_end, REMOVE_WHITESPACE, charset_converter, output, output_parsed);
}

bool Canonicalize(const char16_t* spec, int spec_len, bool trim_path_end, CharsetConverter* charset_converter, CanonOutput* output, Parsed* output_parsed)
{
    return DoCanonicalize(spec, spec_len, trim_path_end, REMOVE_WHITESPACE, charset_converter, output, output_parsed);
}

bool ResolveRelative(const char* base_spec, int base_spec_len, const Parsed& base_parsed, const char* relative, int relative_length,
    CharsetConverter* charset_converter, CanonOutput* output, Parsed* output_parsed)
{
    return DoResolveRelative(base_spec, base_spec_len, base_parsed, relative, relative_length, charset_converter, output, output_parsed);
}

bool ResolveRelative(const char* base_spec, int base_spec_len, const Parsed& base_parsed, const char16_t* relative, int relative_length,
    CharsetConverter* charset_converter, CanonOutput* output, Parsed* output_parsed)
{
    return DoResolveRelative(base_spec, base_spec_len, base_parsed, relative, relative_length, charset_converter, output, output_parsed);
}

bool ReplaceComponents(const char* spec, int spec_len, const Parsed& parsed, const Replacements<char>& replacements, CharsetConverter* charset_converter,
    CanonOutput* output, Parsed* out_parsed)
{
    return DoReplaceComponents(spec, spec_len, parsed, replacements, charset_converter, output, out_parsed);
}

bool ReplaceComponents(const char* spec, int spec_len, const Parsed& parsed, const Replacements<char16_t>& replacements, CharsetConverter* charset_converter,
    CanonOutput* output, Parsed* out_parsed)
{
    return DoReplaceComponents(spec, spec_len, parsed, replacements, charset_converter, output, out_parsed);
}
...
{
    return AllowNonStandardSchemesForAndroidWebView() && !IsStandardScheme(scheme);
}
