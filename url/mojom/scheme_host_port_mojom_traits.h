...
template <> struct COMPONENT_EXPORT(URL_MOJOM_TRAITS) StructTraits<url::mojom::SchemeHostPortDataView, url::SchemeHostPort> {
    ...
    static uint16_t port(const url::SchemeHostPort& r)
    {
        return r.port();
    }
    static bool Read(url::mojom::SchemeHostPortDataView data, url::SchemeHostPort* out);
};
