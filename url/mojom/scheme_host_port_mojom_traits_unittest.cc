...
void TestRoundTrip(const url::SchemeHostPort& in)
{
    ...
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::SchemeHostPort>(in, result)) << in.Serialize();
    EXPECT_EQ(in, result) << "Expected " << in.Serialize() << ", but got " << result.Serialize();
}
