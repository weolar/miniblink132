...
public:
    ...
    {
        SCOPED_TRACE(testing::Message() << "Input GURL: " << input.possibly_invalid_spec());
...
TEST_F(MojoGURLStructTraitsTest, WindowsDriveInPathReplacement)
{
...

    {
        ...
        GURL url_made_with_replace_components = GURL("file://hostname/").ReplaceComponents(repl);

        EXPECT_EQ(kNewPath, url_made_with_replace_components.path());
        EXPECT_EQ("hostname", url_made_with_replace_components.host());
        EXPECT_EQ("file://hostname/C:/dir/file.txt", url_made_with_replace_components.spec());
...

    {
        ...
        repl.SetPathStr(kNewPath);
        GURL url_made_with_replace_components = GURL("file://hostname/").ReplaceComponents(repl);
        EXPECT_EQ(url_created_directly.spec(), url_made_with_replace_components.spec());
    }

    {
        ...
        const std::string kNewPath = "/C:/dir/file.txt";
        repl.SetPathStr(kNewPath);
        GURL url_made_with_replace_components = GURL("file://localhost/").ReplaceComponents(repl);
        EXPECT_EQ(url_created_directly.spec(), url_made_with_replace_components.spec());
    }
}
...
{
    Origin non_unique = Origin::UnsafelyCreateTupleOriginWithoutNormalization("http", "www.google.com", 80).value();
    ...
    Origin normalized = Origin::CreateFromNormalizedTuple("http", "www.google.com", 80);
