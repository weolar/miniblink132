...
template <typename CHAR> int DoFindWindowsDriveLetter(const CHAR* spec, int begin, int end)
{
    if (begin > end)
        return -1;
...
template <typename CHAR> int FileDoDriveSpec(const CHAR* spec, int begin, int end, CanonOutput* output)
{
...
template <typename CHAR, typename UCHAR> bool DoFileCanonicalizePath(const CHAR* spec, const Component& path, CanonOutput* output, Component* out_path)
{
...
bool DoCanonicalizeFileURL(
    const URLComponentSource<CHAR>& source, const Parsed& parsed, CharsetConverter* query_converter, CanonOutput* output, Parsed* new_parsed)
{
    ...
    bool success = CanonicalizeFileHost(source.host, host_range, *output, new_parsed->host);
...
{
    return DoCanonicalizeFileURL<char, unsigned char>(URLComponentSource<char>(spec), parsed, query_converter, output, new_parsed);
}
...
{
    return DoCanonicalizeFileURL<char16_t, char16_t>(URLComponentSource<char16_t>(spec), parsed, query_converter, output, new_parsed);
}
...
{
    return DoFileCanonicalizePath<char16_t, char16_t>(spec, path, output, out_path);
}
