...
bool DoCanonicalizeStandardURL(const URLComponentSource<CHAR>& source, const Parsed& parsed, SchemeType scheme_type, CharsetConverter* query_converter,
    CanonOutput* output, Parsed* new_parsed)
{
    ...
    bool scheme_supports_ports = (scheme_type == SCHEME_WITH_HOST_PORT_AND_USER_INFORMATION || scheme_type == SCHEME_WITH_HOST_AND_PORT);

    // Authority (username, password, host, port)
    bool have_authority;
    if ((scheme_supports_user_info && (parsed.username.is_valid() || parsed.password.is_valid())) || parsed.host.is_nonempty()
        ...
        if (scheme_supports_user_info) {
            success &= CanonicalizeUserInfo(
                source.username, parsed.username, source.password, parsed.password, output, &new_parsed->username, &new_parsed->password);
        } else {
        ...
        if (scheme_supports_ports) {
            int default_port = DefaultPortForScheme(std::string_view(&output->data()[new_parsed->scheme.begin], new_parsed->scheme.len));
...
{
    return DoCanonicalizeStandardURL(URLComponentSource(spec), parsed, scheme_type, query_converter, output, new_parsed);
}
    ...
    CharsetConverter* query_converter, CanonOutput* output, Parsed* new_parsed)
{
    ...
    return DoCanonicalizeStandardURL(source, parsed, scheme_type, query_converter, output, new_parsed);
}
