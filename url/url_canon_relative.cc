...
template <typename CHAR> bool AreSchemesEqual(const char* base, const Component& base_scheme, const CHAR* cmp, const Component& cmp_scheme)
{
...
// beginning.
template <typename CHAR> bool DoesBeginSlashWindowsDriveSpec(const CHAR* spec, int start_offset, int spec_len)
{
    if (start_offset >= spec_len)
        return false;
    return IsSlashOrBackslash(spec[start_offset]) && DoesBeginWindowsDriveSpec(spec, start_offset + 1, spec_len);
}

#endif // WIN32

template <typename CHAR> bool IsValidScheme(const CHAR* url, const Component& scheme)
{
...
bool DoIsRelativeURL(
    const char* base, const Parsed& base_parsed, const CHAR* url, int url_len, bool is_base_hierarchical, bool* is_relative, Component* relative_component)
{
    *is_relative = false; // So we can default later to not relative.
    ...
    // >      url’s scheme:
    if ((IsUsingStandardCompliantNonSpecialSchemeURLParsing() && !IsStandard(base, base_parsed.scheme))
...
void CopyOneComponent(const char* source, const Component& source_component, CanonOutput* output, Component* output_component)
{
...
// path processing.
template <typename CHAR>
int CopyBaseDriveSpecIfNecessary(
...
template <typename CHAR>
bool DoResolveRelativePath(const char* base_url, const Parsed& base_parsed, bool base_is_file, const CHAR* relative_url, const Component& relative_component,
    CharsetConverter* query_converter, CanonMode canon_mode, CanonOutput* output, Parsed* out_parsed)
{
    bool success = true;
    ...
    } else if (url::IsUsingStandardCompliantNonSpecialSchemeURLParsing() && !base_parsed.host.is_valid() &&
    ...
    if (path.is_nonempty()) {
        ...
        if (base_is_file) {
            base_path_begin = CopyBaseDriveSpecIfNecessary(
                base_url, base_parsed.path.begin, base_parsed.path.end(), relative_url, relative_component.begin, relative_component.end(), output);
        ...
        if (IsSlashOrBackslash(relative_url[path.begin])) {
            ...
            success &= CanonicalizePartialPathInternal(relative_url, path, path_begin, canon_mode, output);
        ...
        if (IsUsingStandardCompliantNonSpecialSchemeURLParsing() && !base_parsed.host.is_valid() && out_parsed->path.is_valid()
            && out_parsed->path.as_string_view_on(output->view().data()).starts_with("//")) {
...
template <typename CHAR>
bool DoResolveRelativeHost(const char* base_url, const Parsed& base_parsed, const CHAR* relative_url, const Component& relative_component,
    CharsetConverter* query_converter, CanonOutput* output, Parsed* out_parsed)
{
    SchemeType scheme_type = SCHEME_WITH_HOST_PORT_AND_USER_INFORMATION;
    const bool is_standard_scheme = GetStandardSchemeType(base_url, base_parsed.scheme, &scheme_type);
    ...
    if (IsUsingStandardCompliantNonSpecialSchemeURLParsing() && !is_standard_scheme) {
        ParseAfterNonSpecialScheme(relative_url, relative_component.end(), relative_component.begin, &relative_parsed);
    } else {
        ParseAfterSpecialScheme(relative_url, relative_component.end(), relative_component.begin, &relative_parsed);
    }
    ...
    // base URL.
    output->ReserveSizeIfNeeded(replacements.components().Length() + base_parsed.CountCharactersBefore(Parsed::USERNAME, false));
    if (!is_standard_scheme) {
        if (IsUsingStandardCompliantNonSpecialSchemeURLParsing()) {
            return ReplaceNonSpecialURL(base_url, base_parsed, replacements, query_converter, *output, *out_parsed);
        }
...
bool DoResolveAbsoluteFile(
    const CharT* relative_url, const Component& relative_component, CharsetConverter* query_converter, CanonOutput* output, Parsed* out_parsed)
{
    ...
    return CanonicalizeFileURL(&relative_url[relative_component.begin], relative_component.len,
        ParseFileURL(std::basic_string_view(&relative_url[relative_component.begin], relative_component.len)), query_converter, output, out_parsed);
}

// TODO(brettw) treat two slashes as root like Mozilla for FTP?
template <typename CHAR>
bool DoResolveRelativeURL(const char* base_url, const Parsed& base_parsed, bool base_is_file, const CHAR* relative_url, const Component& relative_component,
    CharsetConverter* query_converter, CanonOutput* output, Parsed* out_parsed)
{
    ...
    *out_parsed = base_parsed;
    ...
    if (IsUsingStandardCompliantNonSpecialSchemeURLParsing() ? base_parsed.scheme.is_empty() : base_parsed.path.is_empty()) {
    ...
    if (num_slashes >= 2) {
        return DoResolveRelativeHost(base_url, base_parsed, relative_url, relative_component, query_converter, output, out_parsed);
    }
...
{
    return DoIsRelativeURL<char>(base, base_parsed, fragment, fragment_len, is_base_hierarchical, is_relative, relative_component);
}
...
bool IsRelativeURL(const char* base, const Parsed& base_parsed, const char16_t* fragment, int fragment_len, bool is_base_hierarchical, bool* is_relative,
    Component* relative_component)
{
    return DoIsRelativeURL<char16_t>(base, base_parsed, fragment, fragment_len, is_base_hierarchical, is_relative, relative_component);
}
...
bool ResolveRelativeURL(const char* base_url, const Parsed& base_parsed, bool base_is_file, const char* relative_url, const Component& relative_component,
    CharsetConverter* query_converter, CanonOutput* output, Parsed* out_parsed)
{
    return DoResolveRelativeURL<char>(base_url, base_parsed, base_is_file, relative_url, relative_component, query_converter, output, out_parsed);
}
...
bool ResolveRelativeURL(const char* base_url, const Parsed& base_parsed, bool base_is_file, const char16_t* relative_url, const Component& relative_component,
    CharsetConverter* query_converter, CanonOutput* output, Parsed* out_parsed)
{
    return DoResolveRelativeURL<char16_t>(base_url, base_parsed, base_is_file, relative_url, relative_component, query_converter, output, out_parsed);
}
