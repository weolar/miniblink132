...
template <typename CHAR> static int FileDoDriveSpec(const CHAR* spec, int begin, int end, CanonOutput* output)
{
...
template <typename CHAR, typename UCHAR> static void FileDoPath(const CHAR* spec, int begin, int end, CanonOutput* output)
{
    ...
    if (sub_path.len > 0) {
        ...
        URLCanonInternal<CHAR, UCHAR>::DoPath(spec, sub_path, output, &fake_output_path);
    }
}

template <typename CHAR, typename UCHAR>
static bool DoCanonicalizeFileURL(const URLComponentSource<CHAR>& source, const ParsedURL& parsed, CanonOutput* output, ParsedURL* new_parsed)
{
    ...
    bool success = URLCanonInternal<CHAR, UCHAR>::DoHost(source.host, parsed.host, output, &new_parsed->host);
    ...
    success &= URLCanonInternal<CHAR, UCHAR>::DoQuery(source.query, parsed.query, output, &new_parsed->query);
    success &= URLCanonInternal<CHAR, UCHAR>::DoRef(source.ref, parsed.ref, output, &new_parsed->ref);
