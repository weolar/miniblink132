...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    if (size % sizeof(char16_t) == 0) {
        std::u16string_view string_piece_input16(reinterpret_cast<const char16_t*>(data), size / sizeof(char16_t));
    ...
    // Resolve relative url tests.
    {
        ...
        size_t relative_size = *reinterpret_cast<const size_t*>(data) % (size - size_t_bytes);
        std::string relative_string(reinterpret_cast<const char*>(data + size_t_bytes), relative_size);
        std::string_view string_piece_part_input(reinterpret_cast<const char*>(data + size_t_bytes + relative_size), size - relative_size - size_t_bytes);
        ...
        if (relative_size % sizeof(char16_t) == 0) {
            std::u16string relative_string16(reinterpret_cast<const char16_t*>(data + size_t_bytes), relative_size / sizeof(char16_t));
