...
template <typename CHAR> void AppendRaw8BitQueryString(const CHAR* source, int length, CanonOutput* output)
{
...
void RunConverter(const char* spec, const Component& query, CharsetConverter* converter, CanonOutput* output)
{
    ...
    ConvertUTF8ToUTF16(&spec[query.begin], static_cast<size_t>(query.len), &utf16);
...
void RunConverter(const char16_t* spec, const Component& query, CharsetConverter* converter, CanonOutput* output)
{
    DCHECK(query.is_valid());
    converter->ConvertFromUTF16(&spec[query.begin], static_cast<size_t>(query.len), output);
}

template <typename CHAR, typename UCHAR>
void DoConvertToQueryEncoding(const CHAR* spec, const Component& query, CharsetConverter* converter, CanonOutput* output)
{
    if (converter) {
...
template <typename CHAR, typename UCHAR>
void DoCanonicalizeQuery(const CHAR* spec, const Component& query, CharsetConverter* converter, CanonOutput* output, Component* out_query)
{
    if (!query.is_valid()) {
        *out_query = Component();
        return;
