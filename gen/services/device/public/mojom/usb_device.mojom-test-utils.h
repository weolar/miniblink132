...
class UsbDeviceInterceptorForTesting : public UsbDevice {
    ...
    void ControlTransferOut(
        UsbControlTransferParamsPtr params, ::base::span<const ::uint8_t> data, uint32_t timeout, ControlTransferOutCallback callback) override;
    ...
    void IsochronousTransferIn(
        uint8_t endpoint_number, const std::vector<uint32_t>& packet_lengths, uint32_t timeout, IsochronousTransferInCallback callback) override;
...
public:
    ...
    void Open(UsbOpenDeviceResultPtr* out_result);
    UsbOpenDeviceResultPtr Open();
    void Close();

    void SetConfiguration(uint8_t value, bool* out_success);
    bool SetConfiguration(uint8_t value);
    void ClaimInterface(uint8_t interface_number, UsbClaimInterfaceResult* out_result);
    UsbClaimInterfaceResult ClaimInterface(uint8_t interface_number);
    void ReleaseInterface(uint8_t interface_number, bool* out_success);
    bool ReleaseInterface(uint8_t interface_number);
    void SetInterfaceAlternateSetting(uint8_t interface_number, uint8_t alternate_setting, bool* out_success);
    bool SetInterfaceAlternateSetting(uint8_t interface_number, uint8_t alternate_setting);
    void Reset(bool* out_success);
    bool Reset();
    void ClearHalt(UsbTransferDirection direction, uint8_t endpoint_number, bool* out_success);
    ...
    void ControlTransferOut(UsbControlTransferParamsPtr params, ::base::span<const ::uint8_t> data, uint32_t timeout, UsbTransferStatus* out_status);
    UsbTransferStatus ControlTransferOut(UsbControlTransferParamsPtr params, ::base::span<const ::uint8_t> data, uint32_t timeout);
    void GenericTransferIn(uint8_t endpoint_number, uint32_t length, uint32_t timeout, UsbTransferStatus* out_status, ::base::span<const ::uint8_t>* out_data);

    void GenericTransferOut(uint8_t endpoint_number, ::base::span<const ::uint8_t> data, uint32_t timeout, UsbTransferStatus* out_status);
    UsbTransferStatus GenericTransferOut(uint8_t endpoint_number, ::base::span<const ::uint8_t> data, uint32_t timeout);
    void IsochronousTransferIn(uint8_t endpoint_number, const std::vector<uint32_t>& packet_lengths, uint32_t timeout, ::base::span<const ::uint8_t>* out_data,
        std::vector<UsbIsochronousPacketPtr>* out_packets);

    void IsochronousTransferOut(uint8_t endpoint_number, ::base::span<const ::uint8_t> data, const std::vector<uint32_t>& packet_lengths, uint32_t timeout,
        std::vector<UsbIsochronousPacketPtr>* out_packets);
    std::vector<UsbIsochronousPacketPtr> IsochronousTransferOut(
