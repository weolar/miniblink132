// services/device/public/mojom/smart_card.mojom-test-utils.h is auto generated by mojom_bindings_generator.py, do not edit

// Copyright 2019 The Chromium Authors
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#ifndef SERVICES_DEVICE_PUBLIC_MOJOM_SMART_CARD_MOJOM_TEST_UTILS_H_
#define SERVICES_DEVICE_PUBLIC_MOJOM_SMART_CARD_MOJOM_TEST_UTILS_H_

#include "services/device/public/mojom/smart_card.mojom.h"

namespace device::mojom {

class SmartCardTransactionInterceptorForTesting : public SmartCardTransaction {
...
class SmartCardTransactionAsyncWaiter {
public:
...
class SmartCardConnectionInterceptorForTesting : public SmartCardConnection {
    ...
    void Transmit(SmartCardProtocol protocol, const std::vector<uint8_t>& data, TransmitCallback callback) override;
    void Control(uint32_t control_code, const std::vector<uint8_t>& data, ControlCallback callback) override;
    void GetAttrib(uint32_t id, GetAttribCallback callback) override;
    void SetAttrib(uint32_t id, const std::vector<uint8_t>& data, SetAttribCallback callback) override;
    void Status(StatusCallback callback) override;
    void BeginTransaction(BeginTransactionCallback callback) override;
};
class SmartCardConnectionAsyncWaiter {
public:
    ...
    void Transmit(SmartCardProtocol protocol, const std::vector<uint8_t>& data, SmartCardDataResultPtr* out_result);
    SmartCardDataResultPtr Transmit(SmartCardProtocol protocol, const std::vector<uint8_t>& data);
    void Control(uint32_t control_code, const std::vector<uint8_t>& data, SmartCardDataResultPtr* out_result);
    SmartCardDataResultPtr Control(uint32_t control_code, const std::vector<uint8_t>& data);
    void GetAttrib(uint32_t id, SmartCardDataResultPtr* out_result);
    SmartCardDataResultPtr GetAttrib(uint32_t id);
    void SetAttrib(uint32_t id, const std::vector<uint8_t>& data, SmartCardResultPtr* out_result);
    SmartCardResultPtr SetAttrib(uint32_t id, const std::vector<uint8_t>& data);
...
class SmartCardContextInterceptorForTesting : public SmartCardContext {
    virtual SmartCardContext* GetForwardingInterface() = 0;
    void ListReaders(ListReadersCallback callback) override;
    void GetStatusChange(::base::TimeDelta timeout, std::vector<SmartCardReaderStateInPtr> reader_states, GetStatusChangeCallback callback) override;
    void Cancel(CancelCallback callback) override;
    void Connect(const std::string& reader, SmartCardShareMode share_mode, SmartCardProtocolsPtr preferred_protocols, ConnectCallback callback) override;
};
class SmartCardContextAsyncWaiter {
public:
    ...
    void GetStatusChange(::base::TimeDelta timeout, std::vector<SmartCardReaderStateInPtr> reader_states, SmartCardStatusChangeResultPtr* out_result);
    SmartCardStatusChangeResultPtr GetStatusChange(::base::TimeDelta timeout, std::vector<SmartCardReaderStateInPtr> reader_states);
    void Cancel(SmartCardResultPtr* out_result);
    SmartCardResultPtr Cancel();
    void Connect(const std::string& reader, SmartCardShareMode share_mode, SmartCardProtocolsPtr preferred_protocols, SmartCardConnectResultPtr* out_result);
    SmartCardConnectResultPtr Connect(const std::string& reader, SmartCardShareMode share_mode, SmartCardProtocolsPtr preferred_protocols);

private:
    SmartCardContext* const proxy_;
};

class SmartCardContextFactoryInterceptorForTesting : public SmartCardContextFactory {
    virtual SmartCardContextFactory* GetForwardingInterface() = 0;
    void CreateContext(CreateContextCallback callback) override;
};
class SmartCardContextFactoryAsyncWaiter {
public:
...
#endif // SERVICES_DEVICE_PUBLIC_MOJOM_SMART_CARD_MOJOM_TEST_UTILS_H_
