...
public:
    ...
    void WriteData(int32_t index, int32_t offset, const std::vector<uint8_t>& data, bool truncate, int32_t* out_result);
    int32_t WriteData(int32_t index, int32_t offset, const std::vector<uint8_t>& data, bool truncate);
    void ReadData(int32_t index, int32_t offset, uint32_t length, std::vector<uint8_t>* out_data, int32_t* out_result);

    void WriteSparseData(int32_t offset, const std::vector<uint8_t>& data, int32_t* out_result);
    int32_t WriteSparseData(int32_t offset, const std::vector<uint8_t>& data);
    void ReadSparseData(int32_t offset, uint32_t length, std::vector<uint8_t>* out_data, int32_t* out_result);
...
public:
    ...
    void CreateEntry(const std::string& key, ::mojo::PendingRemote<SimpleCacheEntry>* out_entry, int32_t* out_error);

    void OpenEntry(const std::string& key, ::mojo::PendingRemote<SimpleCacheEntry>* out_entry, int32_t* out_error);

    void DoomEntry(const std::string& key, int32_t* out_result);
...
class NetworkServiceTestInterceptorForTesting : public NetworkServiceTest {
    ...
    void MockCertVerifierAddResultForCertAndHost(const ::scoped_refptr<::net::X509Certificate>& cert, const std::string& host_pattern,
        const ::net::CertVerifyResult& verify_result, int32_t rv, MockCertVerifierAddResultForCertAndHostCallback callback) override;
...
public:
    ...
    void AddRules(std::vector<RulePtr> rules);
    ...
    void SetTestDohConfig(::net::SecureDnsMode secure_dns_mode, const ::net::DnsOverHttpsConfig& doh_config);

    void GetLatestMemoryPressureLevel(::base::MemoryPressureListener::MemoryPressureLevel* out_memory_pressure_level);
    ::base::MemoryPressureListener::MemoryPressureLevel GetLatestMemoryPressureLevel();
    void GetPeerToPeerConnectionsCountChange(uint32_t* out_connection_count);
    uint32_t GetPeerToPeerConnectionsCountChange();
    void GetEnvironmentVariableValue(const std::string& name, std::string* out_value);
    ...
    void MakeRequestToServer(::network::TransferableSocket s, const ::net::IPEndPoint& endpoint, bool* out_result);
    bool MakeRequestToServer(::network::TransferableSocket s, const ::net::IPEndPoint& endpoint);
    void ResolveOwnHostnameWithSystemDns(::net::AddressList* out_addr_list, int32_t* out_os_error, int32_t* out_net_error);
