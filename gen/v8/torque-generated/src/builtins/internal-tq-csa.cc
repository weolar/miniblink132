...
TNode<Object> ForInNextSlow_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<UintPtrT> p_slot, TNode<HeapObject> p_receiver,
    TNode<Object> p_key, TNode<Object> p_cacheType, TNode<HeapObject> p_maybeFeedbackVector, UpdateFeedbackMode p_guaranteedFeedback)
{
    ...
    if (block0.is_used()) {
        ...
        CodeStubAssembler(state_).UpdateFeedback(
            TNode<Smi> { tmp1 }, TNode<HeapObject> { p_maybeFeedbackVector }, TNode<UintPtrT> { p_slot }, p_guaranteedFeedback);
...
TF_BUILTIN(ForInNext, CodeStubAssembler)
{
    ...
    if (block10.is_used()) {
        ca_.Bind(&block10);
        tmp16 = ForInNextSlow_0(state_, TNode<Context> { parameter0 }, TNode<UintPtrT> { parameter1 }, TNode<HeapObject> { parameter2 },
            TNode<Object> { tmp12 }, TNode<Object> { parameter4 }, TNode<HeapObject> { parameter6 }, UpdateFeedbackMode::kGuaranteedFeedback);
