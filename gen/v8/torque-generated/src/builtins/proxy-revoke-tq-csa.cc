...
TF_BUILTIN(ProxyRevoke, CodeStubAssembler)
{
    ...
    if (block0.is_used()) {
        ...
        std::tie(tmp2, tmp3)
            = ContextSlot_ProxyRevokeFunctionContext_ProxyRevokeFunctionContext_Null_OR_JSProxy_0(state_, TNode<Context> { tmp0 }, TNode<IntPtrT> { tmp1 })
                  .Flatten();
    ...
    compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<IntPtrT> p_index)
{
    ...
    if (block6.is_used()) {
        ...
        std::tie(tmp10, tmp11) = ReferenceCast_Null_OR_JSProxy_Object_0(
