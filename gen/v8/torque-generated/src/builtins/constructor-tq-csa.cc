...
TF_BUILTIN(FastNewFunctionContextEval, CodeStubAssembler)
{
    ...
    if (block0.is_used()) {
        ca_.Bind(&block0);
        tmp0 = ConstructorBuiltinsAssembler(state_).FastNewFunctionContext(
            TNode<ScopeInfo> { parameter1 }, TNode<Uint32T> { parameter2 }, TNode<Context> { parameter0 }, ScopeType::EVAL_SCOPE);
...
TF_BUILTIN(FastNewFunctionContextFunction, CodeStubAssembler)
{
    ...
    if (block0.is_used()) {
        ca_.Bind(&block0);
        tmp0 = ConstructorBuiltinsAssembler(state_).FastNewFunctionContext(
            TNode<ScopeInfo> { parameter1 }, TNode<Uint32T> { parameter2 }, TNode<Context> { parameter0 }, ScopeType::FUNCTION_SCOPE);
        CodeStubAssembler(state_).Return(tmp0);
    }
}
...
TF_BUILTIN(CreateRegExpLiteral, CodeStubAssembler)
{
    ...
    if (block0.is_used()) {
        ca_.Bind(&block0);
        tmp0 = ConstructorBuiltinsAssembler(state_).CreateRegExpLiteral(TNode<HeapObject> { parameter1 }, TNode<TaggedIndex> { parameter2 },
            TNode<Object> { parameter3 }, TNode<Smi> { parameter4 }, TNode<Context> { parameter0 });
        CodeStubAssembler(state_).Return(tmp0);
    }
}
...
TF_BUILTIN(CreateShallowArrayLiteral, CodeStubAssembler)
{
    ...
    if (block3.is_used()) {
        ...
        tmp2 = ConstructorBuiltinsAssembler(state_).CreateShallowArrayLiteral(TNode<FeedbackVector> { tmp0 }, TNode<TaggedIndex> { parameter2 },
            TNode<Context> { parameter0 }, AllocationSiteMode::TRACK_ALLOCATION_SITE, &label3);
        ca_.Goto(&block5);
        if (label3.is_used()) {
            ca_.Bind(&label3);
            ca_.Goto(&block6);
...
TF_BUILTIN(CreateEmptyArrayLiteral, CodeStubAssembler)
{
    ...
    if (block0.is_used()) {
        ca_.Bind(&block0);
        tmp0 = ConstructorBuiltinsAssembler(state_).CreateEmptyArrayLiteral(
            TNode<FeedbackVector> { parameter1 }, TNode<TaggedIndex> { parameter2 }, TNode<Context> { parameter0 });
        CodeStubAssembler(state_).Return(tmp0);
    }
}
