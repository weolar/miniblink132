...
TF_BUILTIN(MapGroupBy, CodeStubAssembler)
{
    ...
    if (block0.is_used()) {
        ...
        tmp0 = False_0(state_);
        tmp1 = GroupByImpl_0(
            state_, TNode<Context> { parameter0 }, TNode<Object> { parameter2 }, TNode<Object> { parameter3 }, TNode<Boolean> { tmp0 }, "Map.groupBy");
    ...
    if (block5.is_used()) {
        ...
        ca_.Branch(tmp9, &block3, std::vector<compiler::Node*> { phi_bb5_8, phi_bb5_9, phi_bb5_10 }, &block4,
    ...
    if (block3.is_used()) {
        ...
        std::tie(tmp10, tmp11, tmp12) = CollectionsBuiltinsAssembler(state_)
                                            .NextKeyValueIndexTupleUnmodifiedTable(TNode<OrderedHashMap> { tmp2 }, TNode<Int32T> { tmp3 },
                                                TNode<Int32T> { tmp4 }, TNode<IntPtrT> { phi_bb3_10 }, &label13)
                                            .Flatten();
    ...
    if (block7.is_used()) {
        ...
        CollectionsBuiltinsAssembler(state_).UnsafeStoreValueInOrderedHashMapEntry(
            TNode<OrderedHashMap> { tmp2 }, TNode<Object> { tmp16 }, TNode<IntPtrT> { tmp23 });
