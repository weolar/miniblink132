...
TF_BUILTIN(ArrayPrototypeCopyWithin, CodeStubAssembler)
{
    ...
    if (block1.is_used()) {
        ...
        tmp15 = CodeStubAssembler(state_).GetArgumentValue(
            TorqueStructArguments { TNode<RawPtrT> { torque_arguments.frame }, TNode<RawPtrT> { torque_arguments.base },
                TNode<IntPtrT> { torque_arguments.length }, TNode<IntPtrT> { torque_arguments.actual_count } },
            TNode<IntPtrT> { tmp14 });
    ...
    if (block10.is_used()) {
        ...
        ca_.Branch(tmp34, &block8, std::vector<compiler::Node*> { phi_bb10_9, phi_bb10_11, phi_bb10_14 }, &block9,
            std::vector<compiler::Node*> { phi_bb10_9, phi_bb10_11, phi_bb10_14 });
    }
    ...
    if (block8.is_used()) {
        ...
        ca_.Branch(tmp37, &block11, std::vector<compiler::Node*> { phi_bb8_9, phi_bb8_11, phi_bb8_14 }, &block12,
            std::vector<compiler::Node*> { phi_bb8_9, phi_bb8_11, phi_bb8_14 });
    }
