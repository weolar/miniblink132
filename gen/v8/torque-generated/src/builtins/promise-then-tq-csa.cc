...
TF_BUILTIN(PromisePrototypeThen, CodeStubAssembler)
{
    ...
    if (block5.is_used()) {
        ...
        PerformPromiseThenImpl_0(state_, TNode<Context> { parameter0 }, TNode<JSPromise> { tmp0 }, TNode<HeapObject> { tmp18 }, TNode<HeapObject> { tmp20 },
            TNode<HeapObject> { phi_bb5_6 });
...
TNode<HeapObject> CastOrDefault_Callable_JSAny_Undefined_0(
    compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<Object> p_x, TNode<Undefined> p_default)
{
    compiler::CodeAssembler ca_(state_);
    compiler::CodeAssembler::SourcePositionScope pos_scope(&ca_);
    compiler::CodeAssemblerParameterizedLabel<> block0(&ca_, compiler::CodeAssemblerLabel::kNonDeferred);
