    ...
    compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<NativeContext> p_nativeContext, TNode<Object> p_receiver, TNode<Object> p_string)
{
    ...
    if (block0.is_used()) {
        ...
        CodeStubAssembler(state_).ThrowIfNotJSReceiver(
            TNode<Context> { p_context }, TNode<Object> { p_receiver }, MessageTemplate::kIncompatibleMethodReceiver, "RegExp.prototype.@@matchAll");
    ...
    if (block4.is_used()) {
        ...
        tmp30
            = RegExpCreate_0(state_, TNode<Context> { p_context }, TNode<NativeContext> { p_nativeContext }, TNode<Object> { tmp28 }, TNode<String> { tmp29 });
        ...
        RegExpBuiltinsAssembler(state_).FastStoreLastIndex(TNode<JSRegExp> { tmp31 }, TNode<Smi> { tmp33 });
        tmp34 = RegExpBuiltinsAssembler(state_).FastFlagGetter(TNode<JSRegExp> { tmp31 }, JSRegExp::Flag::kGlobal);
        tmp35 = RegExpBuiltinsAssembler(state_).FastFlagGetter(TNode<JSRegExp> { tmp31 }, JSRegExp::Flag::kUnicode);
        ca_.Branch(tmp35, &block10, std::vector<compiler::Node*> {}, &block11, std::vector<compiler::Node*> {});
    }
    ...
    if (block11.is_used()) {
        ca_.Bind(&block11);
        tmp37 = RegExpBuiltinsAssembler(state_).FastFlagGetter(TNode<JSRegExp> { tmp31 }, JSRegExp::Flag::kUnicodeSets);
        ca_.Goto(&block12, tmp37);
    }

    TNode<BoolT> phi_bb12_16;
    if (block12.is_used()) {
        ca_.Bind(&block12, &phi_bb12_16);
        ca_.Goto(&block2, tmp30, tmp34, phi_bb12_16);
    }
    ...
    if (block14.is_used()) {
        ...
        ca_.Goto(&block15, tmp40);
    }

    TNode<BoolT> phi_bb15_23;
    if (block15.is_used()) {
        ca_.Bind(&block15, &phi_bb15_23);
    ...
    if (block2.is_used()) {
        ...
        tmp41 = RegExpMatchAllAssembler(state_).CreateRegExpStringIterator(TNode<NativeContext> { p_nativeContext }, TNode<Object> { phi_bb2_6 },
            TNode<String> { tmp1 }, TNode<BoolT> { phi_bb2_7 }, TNode<BoolT> { phi_bb2_8 });
        ca_.Goto(&block16);
    }
...
TF_BUILTIN(RegExpPrototypeMatchAll, CodeStubAssembler)
{
    ...
    if (block0.is_used()) {
        ca_.Bind(&block0);
        tmp0 = RegExpPrototypeMatchAllImpl_0(
            state_, TNode<Context> { parameter0 }, TNode<NativeContext> { parameter0 }, TNode<Object> { parameter1 }, TNode<Object> { parameter2 });
...
TF_BUILTIN(RegExpStringIteratorPrototypeNext, CodeStubAssembler)
{
    ...
    if (block3.is_used()) {
        ...
        tmp5 = ca_.UncheckedCast<BoolT>(CodeStubAssembler(state_).DecodeWord32FromWord<base::BitField<bool, 1, 1, uintptr_t>>(
            ca_.UncheckedCast<WordT>(ca_.BitcastTaggedToWordForTagAndSmiBits(tmp4))));
    ...
    if (block11.is_used()) {
        ...
        tmp14 = RegExpPrototypeExecBodyWithoutResultFast_1(
            state_, TNode<Context> { parameter0 }, TNode<JSRegExp> { tmp12 }, TNode<String> { tmp9 }, TNode<Number> { tmp13 }, &label15);
        ca_.Goto(&block14);
        if (label15.is_used()) {
            ca_.Bind(&label15);
            ca_.Goto(&block15);
    ...
    if (block14.is_used()) {
        ca_.Bind(&block14);
        tmp16 = RegExpBuiltinsAssembler(state_).ConstructNewResultFromMatchInfo(
            TNode<Context> { parameter0 }, TNode<JSRegExp> { tmp12 }, TNode<RegExpMatchInfo> { tmp14 }, TNode<String> { tmp9 }, TNode<Number> { tmp13 });
    ...
    if (block13.is_used()) {
        ...
        tmp21 = ca_.UncheckedCast<BoolT>(CodeStubAssembler(state_).DecodeWord32FromWord<base::BitField<bool, 2, 1, uintptr_t>>(
            ca_.UncheckedCast<WordT>(ca_.BitcastTaggedToWordForTagAndSmiBits(tmp4))));
    ...
    if (block18.is_used()) {
        ...
        tmp26 = ca_.UncheckedCast<Smi>(ca_.BitcastWordToTaggedSigned(CodeStubAssembler(state_).UpdateWord32InWord<base::BitField<bool, 1, 1, uintptr_t>>(
            ca_.UncheckedCast<WordT>(ca_.BitcastTaggedToWordForTagAndSmiBits(tmp24)), ca_.UncheckedCast<Uint32T>(tmp25))));
    ...
    if (block39.is_used()) {
        ...
        tmp52 = ca_.UncheckedCast<BoolT>(CodeStubAssembler(state_).DecodeWord32FromWord<base::BitField<bool, 3, 1, uintptr_t>>(
            ca_.UncheckedCast<WordT>(ca_.BitcastTaggedToWordForTagAndSmiBits(tmp4))));
    ...
    if (block45.is_used()) {
        ...
        tmp64 = ca_.UncheckedCast<BoolT>(CodeStubAssembler(state_).DecodeWord32FromWord<base::BitField<bool, 3, 1, uintptr_t>>(
            ca_.UncheckedCast<WordT>(ca_.BitcastTaggedToWordForTagAndSmiBits(tmp4))));
    ...
    if (block10.is_used()) {
        ...
        tmp71 = ca_.UncheckedCast<Smi>(ca_.BitcastWordToTaggedSigned(CodeStubAssembler(state_).UpdateWord32InWord<base::BitField<bool, 1, 1, uintptr_t>>(
            ca_.UncheckedCast<WordT>(ca_.BitcastTaggedToWordForTagAndSmiBits(tmp69)), ca_.UncheckedCast<Uint32T>(tmp70))));
