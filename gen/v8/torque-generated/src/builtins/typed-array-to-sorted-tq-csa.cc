...
TF_BUILTIN(TypedArrayPrototypeToSorted, CodeStubAssembler)
{
    ...
    if (block3.is_used()) {
        ...
        tmp4 = TypedArrayBuiltinsAssembler(state_).ValidateTypedArrayAndGetLength(
            TNode<Context> { parameter0 }, TNode<Object> { parameter1 }, kBuiltinNameToSorted_0(state_));
    ...
    if (block12.is_used()) {
        ca_.Bind(&block12);
        tmp20 = TypedArraySortCommon_0(
            state_, TNode<Context> { parameter0 }, TNode<JSTypedArray> { tmp6 }, TNode<UintPtrT> { tmp4 }, TNode<HeapObject> { tmp2 }, false);
