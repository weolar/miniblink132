...
TNode<BoolT> HasAccessCheckFailed_0(
    compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<NativeContext> p_nativeContext, TNode<Object> p_promiseFun, TNode<Object> p_executor)
{
    ...
    if (block0.is_used()) {
        ...
        BranchIfAccessCheckFailed_0(state_, TNode<Context> { p_context }, TNode<NativeContext> { p_nativeContext }, TNode<Object> { p_promiseFun },
            TNode<Object> { p_executor }, &label0);
        ca_.Goto(&block4);
        if (label0.is_used()) {
            ca_.Bind(&label0);
            ca_.Goto(&block5);
...
TF_BUILTIN(PromiseConstructor, CodeStubAssembler)
{
    ...
    if (block4.is_used()) {
        ...
        tmp8 = HasAccessCheckFailed_0(
            state_, TNode<Context> { parameter0 }, TNode<NativeContext> { parameter0 }, TNode<Object> { tmp7 }, TNode<Object> { parameter3 });
    ...
    if (block9.is_used()) {
        ...
        tmp16 = True_0(state_);
        std::tie(tmp17, tmp18, tmp19) = CreatePromiseResolvingFunctions_0(
            state_, TNode<Context> { parameter0 }, TNode<JSPromise> { phi_bb9_5 }, TNode<Boolean> { tmp16 }, TNode<NativeContext> { parameter0 })
                                            .Flatten();
        ...
        compiler::CodeAssemblerExceptionHandlerLabel catch25__label(&ca_, compiler::CodeAssemblerLabel::kDeferred);
        {
            ...
            tmp24 = CodeStubAssembler(state_).Call(
                TNode<Context> { parameter0 }, TNode<Object> { tmp20 }, TNode<Object> { tmp23 }, TNode<Object> { tmp17 }, TNode<Object> { tmp18 });
        }
    ...
    if (block11.is_used()) {
        ...
        std::tie(tmp31, tmp32)
            = ContextSlot_PromiseResolvingFunctionContext_PromiseResolvingFunctionContext_Boolean_0(state_, TNode<Context> { tmp29 }, TNode<IntPtrT> { tmp30 })
                  .Flatten();
...
TF_BUILTIN(PromisePrototypeCatch, CodeStubAssembler)
{
    ...
    if (block1.is_used()) {
        ...
        tmp3 = InvokeThen_1(state_, TNode<Context> { parameter0 }, TNode<NativeContext> { tmp1 }, TNode<Object> { parameter1 }, TNode<Object> { tmp2 },
            TNode<Object> { parameter2 });
