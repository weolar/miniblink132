...
TNode<Object> FastArrayForEach_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<JSReceiver> p_o, TNode<Number> p_len,
...
TF_BUILTIN(ArrayForEach, CodeStubAssembler)
{
    ...
    if (block5.is_used()) {
        ...
        tmp12 = FastArrayForEach_0(state_, TNode<Context> { parameter0 }, TNode<JSReceiver> { tmp1 }, TNode<Number> { tmp2 }, TNode<JSReceiver> { tmp7 },
            TNode<Object> { tmp10 }, &label13, &tmp14);
        ca_.Goto(&block9);
        if (label13.is_used()) {
            ca_.Bind(&label13);
            ca_.Goto(&block10);
    ...
    if (block2.is_used()) {
        ...
        tmp19 = CodeStubAssembler(state_).GetArgumentValue(
            TorqueStructArguments { TNode<RawPtrT> { torque_arguments.frame }, TNode<RawPtrT> { torque_arguments.base },
                TNode<IntPtrT> { torque_arguments.length }, TNode<IntPtrT> { torque_arguments.actual_count } },
            TNode<IntPtrT> { tmp18 });
