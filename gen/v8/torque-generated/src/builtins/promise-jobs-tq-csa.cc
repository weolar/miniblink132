...
TF_BUILTIN(PromiseResolveThenableJob, CodeStubAssembler)
{
    ...
    if (block2.is_used()) {
        ...
        std::tie(tmp20, tmp21, tmp22) = CreatePromiseResolvingFunctions_0(
            state_, TNode<Context> { parameter0 }, TNode<JSPromise> { parameter1 }, TNode<Boolean> { tmp19 }, TNode<NativeContext> { tmp0 })
                                            .Flatten();
        ...
        compiler::CodeAssemblerExceptionHandlerLabel catch27__label(&ca_, compiler::CodeAssemblerLabel::kDeferred);
        {
            ...
            tmp26 = CodeStubAssembler(state_).Call(
                TNode<Context> { parameter0 }, TNode<Object> { tmp23 }, TNode<Object> { parameter2 }, TNode<Object> { tmp20 }, TNode<Object> { tmp21 });
        }
