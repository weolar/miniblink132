...
TF_BUILTIN(ProxySetProperty, CodeStubAssembler)
{
    ...
    if (block28.is_used()) {
        ca_.Bind(&block28);
        tmp12 = CodeStubAssembler(state_).Call(TNode<Context> { parameter0 }, TNode<Object> { tmp10 }, TNode<Object> { tmp4 }, TNode<Object> { tmp9 },
            TNode<Object> { ca_.UncheckedCast<Name>(parameter2) }, TNode<Object> { parameter3 }, TNode<Object> { parameter4 });
    ...
    if (block30.is_used()) {
        ca_.Bind(&block30);
        ProxiesCodeStubAssembler(state_).CheckGetSetTrapResult(TNode<Context> { parameter0 }, TNode<JSReceiver> { tmp9 }, TNode<JSProxy> { parameter1 },
            TNode<Name> { parameter2 }, TNode<Object> { parameter3 }, JSProxy::AccessKind::kSet);
        CodeStubAssembler(state_).Return(parameter3);
    }
