...
TNode<Object> RegExpPrototypeSearchBodyFast_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<JSRegExp> p_regexp, TNode<String> p_string)
{
    ...
    if (block8.is_used()) {
        ca_.Bind(&block8);
        RegExpBuiltinsAssembler(state_).FastStoreLastIndex(TNode<JSRegExp> { p_regexp }, TNode<Smi> { tmp0 });
        tmp6 = Method_RegExpMatchInfo_GetStartOfCapture_0(
            state_, TNode<RegExpMatchInfo> { tmp3 }, (FromConstexpr_constexpr_int31_constexpr_IntegerLiteral_0(state_, IntegerLiteral(false, 0x0ull))));
...
TF_BUILTIN(RegExpPrototypeSearch, CodeStubAssembler)
{
    ...
    if (block0.is_used()) {
        ...
        CodeStubAssembler(state_).ThrowIfNotJSReceiver(
            TNode<Context> { parameter0 }, TNode<Object> { parameter1 }, MessageTemplate::kIncompatibleMethodReceiver, "RegExp.prototype.@@search");
