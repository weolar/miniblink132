...
TF_BUILTIN(TypedArrayPrototypeWith, CodeStubAssembler)
{
    ...
    if (block28.is_used()) {
        ...
        tmp35 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltinPointer(
            Builtins::CallInterfaceDescriptorFor(ExampleBuiltinForTorqueFunctionPointerType(3)), tmp34, parameter0, tmp17, tmp10, phi_bb13_8));
    ...
    if (block32.is_used()) {
        ...
        tmp41 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltinPointer(
            Builtins::CallInterfaceDescriptorFor(ExampleBuiltinForTorqueFunctionPointerType(3)), tmp34, parameter0, tmp17, phi_bb32_20, tmp40));
        tmp42 = kStoreSucceded_0(state_);
        tmp43 = CodeStubAssembler(state_).SmiEqual(TNode<Smi> { tmp41 }, TNode<Smi> { tmp42 });
        ca_.Branch(tmp43, &block36, std::vector<compiler::Node*> { phi_bb32_20, phi_bb32_20, phi_bb32_20 }, &block37,
            std::vector<compiler::Node*> { phi_bb32_20, phi_bb32_20, phi_bb32_20 });
    }
