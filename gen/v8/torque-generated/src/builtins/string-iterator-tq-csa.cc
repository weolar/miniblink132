...
TF_BUILTIN(StringIteratorPrototypeNext, CodeStubAssembler)
{
    ...
    if (block6.is_used()) {
        ca_.Bind(&block6);
        tmp13 = StringBuiltinsAssembler(state_).LoadSurrogatePairAt(
            TNode<String> { tmp4 }, TNode<IntPtrT> { tmp8 }, TNode<IntPtrT> { tmp7 }, UnicodeEncoding::UTF16);
...
TNode<JSStringIterator> Cast_JSStringIterator_1(
