...
TF_BUILTIN(ProxyHasProperty, CodeStubAssembler)
{
    ...
    if (block32.is_used()) {
        ...
        tmp10 = CodeStubAssembler(state_).Call(
            TNode<Context> { parameter0 }, TNode<Object> { tmp8 }, TNode<Object> { tmp2 }, TNode<Object> { tmp6 }, TNode<Object> { parameter2 });
    ...
    if (block35.is_used()) {
        ca_.Bind(&block35);
        ProxiesCodeStubAssembler(state_).CheckHasTrapResult(
