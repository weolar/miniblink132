...
TF_BUILTIN(RegExpSplit, CodeStubAssembler)
{
    ...
    if (block8.is_used()) {
        ...
        tmp9 = RegExpBuiltinsAssembler(state_).RegExpPrototypeSplitBody(
            TNode<Context> { parameter0 }, TNode<JSRegExp> { parameter1 }, TNode<String> { parameter2 }, TNode<Smi> { phi_bb8_4 });
...
TF_BUILTIN(RegExpPrototypeSplit, CodeStubAssembler)
{
    ...
    if (block0.is_used()) {
        ...
        CodeStubAssembler(state_).ThrowIfNotJSReceiver(
            TNode<Context> { parameter0 }, TNode<Object> { parameter1 }, MessageTemplate::kIncompatibleMethodReceiver, "RegExp.prototype.@@split");
