...
constexpr Aggregation Aggregation_MAX = LATEST_OR_DEFAULT;
constexpr int Aggregation_ARRAYSIZE = Aggregation_MAX + 1;

const std::string& Aggregation_Name(Aggregation value);
template <typename T> inline const std::string& Aggregation_Name(T enum_t_value)
{
    static_assert(::std::is_same<T, Aggregation>::value || ::std::is_integral<T>::value, "Incorrect type passed to function Aggregation_Name.");
