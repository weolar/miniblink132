...
class BLINK_COMMON_EXPORT RuntimeFeatureStateContext : public RuntimeFeatureStateReadContext {
public:
    explicit RuntimeFeatureStateContext()
    {
    ...
    {
        return SetIsEnabled(blink::mojom::RuntimeFeature::kBlinkExtensionChromeOS, enabled);
    }
    void SetBlinkExtensionChromeOSKioskEnabled(bool enabled)
    {
        return SetIsEnabled(blink::mojom::RuntimeFeature::kBlinkExtensionChromeOSKiosk, enabled);
    }
    void SetBlinkExtensionDiagnosticsEnabled(bool enabled)
    {
        return SetIsEnabled(blink::mojom::RuntimeFeature::kBlinkExtensionDiagnostics, enabled);
    }
    void SetDisableThirdPartyStoragePartitioning2Enabled(bool enabled)
    {
        return SetIsEnabled(blink::mojom::RuntimeFeature::kDisableThirdPartyStoragePartitioning2, enabled);
    }
    void SetOriginTrialsSampleAPIBrowserReadWriteEnabled(bool enabled)
    {
        return SetIsEnabled(blink::mojom::RuntimeFeature::kOriginTrialsSampleAPIBrowserReadWrite, enabled);
    }
    void SetTestFeatureEnabled(bool enabled)
    {
        return SetIsEnabled(blink::mojom::RuntimeFeature::kTestFeature, enabled);
    }
