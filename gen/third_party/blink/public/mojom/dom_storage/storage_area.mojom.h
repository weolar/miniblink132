...
class BLINK_COMMON_EXPORT StorageAreaObserver : public StorageAreaObserverInterfaceBase {
public:
...
class BLINK_COMMON_EXPORT StorageArea : public StorageAreaInterfaceBase {
public:
    using IPCStableHashFunction = uint32_t (*)();
...
// with not having this data in traces there.
    ...
    virtual void Put(const std::vector<uint8_t>& key, const std::vector<uint8_t>& value, const std::optional<std::vector<uint8_t>>& client_old_value,
        const std::string& source, PutCallback callback)
        = 0;
...
//class BLINK_COMMON_EXPORT StorageAreaObserverProxy : public StorageAreaObserver {
//public:
...
//class BLINK_COMMON_EXPORT StorageAreaProxy : public StorageArea {
//public:
...
//        const std::string& source, PutCallback callback) final;
//
//    void Delete(
//        const std::vector<uint8_t>& key, const std::optional<std::vector<uint8_t>>& client_old_value, const std::string& source, DeleteCallback callback) final;
//
    ...
    , value()
{
    ...
    KeyValue(std::vector<uint8_t> key_in, std::vector<uint8_t> value_in)
    : key(std::move(key_in))
    ...
    {
        return mojo::internal::SerializeImpl<KeyValue::DataView, std::vector<uint8_t>>(input);
    }
...
template <> struct BLINK_COMMON_EXPORT StructTraits<::blink::mojom::KeyValue::DataView, ::blink::mojom::KeyValuePtr> {
    static bool IsNull(const ::blink::mojom::KeyValuePtr& input)
    {
        return !input;
    }
    static void SetToNull(::blink::mojom::KeyValuePtr* output)
    {
        output->reset();
    }

    static const decltype(::blink::mojom::KeyValue::key)& key(const ::blink::mojom::KeyValuePtr& input)
    {
        return input->key;
    }

    static const decltype(::blink::mojom::KeyValue::value)& value(const ::blink::mojom::KeyValuePtr& input)
    {
        return input->value;
    }
