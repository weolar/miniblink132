...
TF_BUILTIN(SetPrototypeIsSupersetOf, CodeStubAssembler) {
  ...
  if (block30.is_used()) {
    ...
    CheckSetRecordHasJSMapMethods_0(state_, TorqueStructSetRecord{TNode<JSReceiver>{tmp4}, TNode<Number>{tmp5}, TNode<Object>{tmp6}, TNode<Object>{tmp7}}, &label33);
    ca_.Goto(&block32);
    if (label33.is_used()) {
      ca_.Bind(&label33);
      ca_.Goto(&block33);
  ...
  if (block44.is_used()) {
    ...
    tmp54 = IteratorBuiltinsAssembler(state_).IteratorStep(TNode<Context>{parameter0}, TorqueStructIteratorRecord{TNode<JSReceiver>{tmp51}, TNode<Object>{tmp52}}, TNode<Map>{tmp0}, &label55);
