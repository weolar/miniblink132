...
public:
    ...
    public:
        ...
        virtual bool ResolveDns(const std::string& host, net::ProxyResolveDnsOperation op, std::string* output, bool* terminate) = 0;
    ...
    static int Create(const scoped_refptr<net::PacFileData>& script_data, JSBindings* bindings, std::unique_ptr<ProxyResolverV8>* resolver);
    ...
    int GetProxyForURL(const GURL& url, net::ProxyInfo* results, JSBindings* bindings);
