...
class MockProxyHostResolver::RequestImpl : public Request, public base::SupportsWeakPtr<RequestImpl> {
public:
    ...
    int Start(net::CompletionOnceCallback callback) override
    {
        if (!synchronous_mode_) {
            ...
            base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&RequestImpl::SendResults, AsWeakPtr()));
...
std::unique_ptr<ProxyHostResolver::Request> MockProxyHostResolver::CreateRequest(
    const std::string& hostname, net::ProxyResolveDnsOperation operation, const net::NetworkAnonymizationKey& network_anonymization_key)
{
    ...
    if (fail_all_)
        return std::make_unique<RequestImpl>(std::vector<net::IPAddress>(), synchronous_mode_);

    auto match = results_.find({ hostname, operation, network_anonymization_key });
    if (match == results_.end())
        return std::make_unique<RequestImpl>(std::vector<net::IPAddress>({ net::IPAddress(127, 0, 0, 1) }), synchronous_mode_);
...
void MockProxyHostResolver::SetResult(const std::string& hostname, net::ProxyResolveDnsOperation operation,
    const net::NetworkAnonymizationKey& network_anonymization_key, std::vector<net::IPAddress> result)
{
    DCHECK(!result.empty());
    fail_all_ = false;
    results_[{ hostname, operation, network_anonymization_key }] = std::move(result);
}
...
public:
    ...
    {
        ++resolver_->num_cancelled_requests_;
    }
...
HangingProxyHostResolver::HangingProxyHostResolver(base::RepeatingClosure hang_callback)
...
std::unique_ptr<ProxyHostResolver::Request> HangingProxyHostResolver::CreateRequest(
    const std::string& hostname, net::ProxyResolveDnsOperation operation, const net::NetworkAnonymizationKey& network_anonymization_key)
{
