...
public:
    explicit ProxyResolverFactoryImpl(mojo::PendingReceiver<mojom::ProxyResolverFactory> receiver);
    ...
    // factory.
    void AddResolver(std::unique_ptr<mojom::ProxyResolver> resolver, mojo::PendingReceiver<mojom::ProxyResolver> receiver);
...
private:
    ...
    void CreateResolver(const std::string& pac_script, mojo::PendingReceiver<mojom::ProxyResolver> receiver,
        mojo::PendingRemote<mojom::ProxyResolverFactoryRequestClient> client) override;

    void RemoveJob(Job* job);

    const std::unique_ptr<ProxyResolverV8TracingFactory> proxy_resolver_impl_factory_;
