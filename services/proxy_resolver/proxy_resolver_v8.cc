...
// net::PacFileData.
class V8ExternalStringFromScriptData : public v8::String::ExternalStringResource {
public:
    explicit V8ExternalStringFromScriptData(const scoped_refptr<net::PacFileData>& script_data)
        : script_data_(script_data)
    {
    }

    V8ExternalStringFromScriptData(const V8ExternalStringFromScriptData&) = delete;
    V8ExternalStringFromScriptData& operator=(const V8ExternalStringFromScriptData&) = delete;
...
class V8ExternalASCIILiteral : public v8::String::ExternalOneByteStringResource {
public:
...
std::u16string V8StringToUTF16(v8::Isolate* isolate, v8::Local<v8::String> s)
{
    ...
    if (len > 0) {
        s->Write(isolate, reinterpret_cast<uint16_t*>(base::WriteInto(&result, len + 1)), 0, len);
    }
    return result;
}
...
// V8 string.
v8::Local<v8::String> ScriptDataToV8String(v8::Isolate* isolate, const scoped_refptr<net::PacFileData>& s)
{
    if (s->utf16().size() * 2 <= kMaxStringBytesForCopy) {
        return v8::String::NewFromTwoByte(isolate, reinterpret_cast<const uint16_t*>(s->utf16().data()), v8::NewStringType::kNormal, s->utf16().size())
            .ToLocalChecked();
    }
    return v8::String::NewExternalTwoByte(isolate, new V8ExternalStringFromScriptData(s)).ToLocalChecked();
}
...
bool V8ObjectToUTF16String(v8::Local<v8::Value> object, std::u16string* utf16_result, v8::Isolate* isolate)
{
    if (object.IsEmpty())
        return false;
...
bool GetHostnameArgument(const v8::FunctionCallbackInfo<v8::Value>& args, std::string* hostname)
{
    ...
    if (!url::IDNToASCII(hostname_utf16.data(), hostname_utf16.length(), &punycode_output)) {
        return false;
    }
    ...
    bool success = base::UTF16ToUTF8(punycode_output.data(), punycode_output.length(), hostname);
    DCHECK(success);
...
struct IPAddressSortingEntry {
    IPAddressSortingEntry(const std::string& ip_string, const net::IPAddress& ip_address)
...
bool SortIpAddressList(const std::string& ip_address_list, std::string* sorted_ip_address_list)
{
    ...
    v8::Isolate* GetSharedIsolate()
    {
        ...
        if (!holder_) {
            ...
            holder_ = std::make_unique<gin::IsolateHolder>(
                base::ThreadTaskRunnerHandle::Get(), gin::IsolateHolder::kUseLocker, gin::IsolateHolder::IsolateType::kUtility);
        }
...
public:
    ...
    int ResolveProxy(const GURL& query_url, net::ProxyInfo* results, JSBindings* bindings)
    {
        DCHECK(bindings);
        ...
        if (!v8::Function::Cast(*function)->Call(context, context->Global(), std::size(argv), argv).ToLocal(&ret)) {
        ...
        std::u16string ret_str = V8StringToUTF16(isolate_, v8::Local<v8::String>::Cast(ret));
        ...
        if (!base::IsStringASCII(ret_str)) {
            ...
            //               crbug.com/47234
            std::u16string error_message = u"FindProxyForURL() returned a non-ASCII string (crbug.com/47234): " + ret_str;
    ...
    int InitV8(const scoped_refptr<net::PacFileData>& pac_script, JSBindings* bindings)
    {
        base::AutoReset<JSBindings*> bindings_reset(&js_bindings_, bindings);
        v8::Locker locked(isolate_);
        v8::Isolate::Scope isolate_scope(isolate_);
        ...
        global_template->Set(ASCIILiteralToV8String(isolate_, "myIpAddress"), my_ip_address_template);
        ...
        global_template->Set(ASCIILiteralToV8String(isolate_, "dnsResolve"), dns_resolve_template);
        ...
        global_template->Set(ASCIILiteralToV8String(isolate_, "isPlainHostName"), is_plain_host_name_template);
        ...
        global_template->Set(ASCIILiteralToV8String(isolate_, "dnsResolveEx"), dns_resolve_ex_template);
        ...
        global_template->Set(ASCIILiteralToV8String(isolate_, "myIpAddressEx"), my_ip_address_ex_template);

        v8::Local<v8::FunctionTemplate> sort_ip_address_list_template = v8::FunctionTemplate::New(isolate_, &SortIpAddressListCallback, v8_this);
        sort_ip_address_list_template->RemovePrototype();
        global_template->Set(ASCIILiteralToV8String(isolate_, "sortIpAddressList"), sort_ip_address_list_template);
        ...
        global_template->Set(ASCIILiteralToV8String(isolate_, "isInNetEx"), is_in_net_ex_template);

        v8_context_.Reset(isolate_, v8::Context::New(isolate_, nullptr, global_template));
        ...
        int rv = RunScript(ASCIILiteralToV8String(isolate_, PAC_JS_LIBRARY PAC_JS_LIBRARY_EX), kPacUtilityResourceName);
        if (rv != net::OK) {
            NOTREACHED();
            return rv;
        }
...
private:
    int GetFindProxyForURL(v8::Local<v8::Value>* function)
    {
        ...
        if (!context->Global()->Get(context, ASCIILiteralToV8String(isolate_, "FindProxyForURL")).ToLocal(function)) {
            DCHECK(try_catch.HasCaught());
            HandleError(try_catch.Message());
        }
        ...
        if (function->IsEmpty() || try_catch.HasCaught()) {
            js_bindings()->OnError(-1, u"Accessing FindProxyForURL threw an exception.");
            return net::ERR_PAC_SCRIPT_FAILED;
        }

        if (!(*function)->IsFunction()) {
            js_bindings()->OnError(-1, u"FindProxyForURL is undefined or not a function.");
            return net::ERR_PAC_SCRIPT_FAILED;
        }
    ...
    int RunScript(v8::Local<v8::String> script, const char* script_name)
    {
        ...
        // Compile the script.
        v8::ScriptOrigin origin = v8::ScriptOrigin(isolate_, ASCIILiteralToV8String(isolate_, script_name));
        v8::ScriptCompiler::Source script_source(script, origin);
        v8::Local<v8::Script> code;
        if (!v8::ScriptCompiler::Compile(
                context, &script_source, v8::ScriptCompiler::kNoCompileOptions, v8::ScriptCompiler::NoCacheReason::kNoCacheBecausePacScript)
                 .ToLocal(&code)) {
            DCHECK(try_catch.HasCaught());
            HandleError(try_catch.Message());
            return net::ERR_PAC_SCRIPT_FAILED;
    ...
    static void MyIpAddressCallback(const v8::FunctionCallbackInfo<v8::Value>& args)
    {
        DnsResolveCallbackHelper(args, net::ProxyResolveDnsOperation::MY_IP_ADDRESS);
    }

    // V8 callback for when "myIpAddressEx()" is invoked by the PAC script.
    static void MyIpAddressExCallback(const v8::FunctionCallbackInfo<v8::Value>& args)
    {
        DnsResolveCallbackHelper(args, net::ProxyResolveDnsOperation::MY_IP_ADDRESS_EX);
    }
    ...
    static void DnsResolveExCallback(const v8::FunctionCallbackInfo<v8::Value>& args)
    {
        DnsResolveCallbackHelper(args, net::ProxyResolveDnsOperation::DNS_RESOLVE_EX);
    }

    // Shared code for implementing:
    //   - myIpAddress(), myIpAddressEx(), dnsResolve(), dnsResolveEx().
    static void DnsResolveCallbackHelper(const v8::FunctionCallbackInfo<v8::Value>& args, net::ProxyResolveDnsOperation op)
    {
        Context* context = static_cast<Context*>(v8::External::Cast(*args.Data())->Value());
        ...
        if (op == net::ProxyResolveDnsOperation::DNS_RESOLVE || op == net::ProxyResolveDnsOperation::DNS_RESOLVE_EX) {
...

        {
            ...
            success = context->js_bindings()->ResolveDns(hostname, op, &result, &terminate);
        }
    ...
    static void SortIpAddressListCallback(const v8::FunctionCallbackInfo<v8::Value>& args)
    {
    ...
    static void IsPlainHostNameCallback(const v8::FunctionCallbackInfo<v8::Value>& args)
    {
        // Need at least 1 string arguments.
        if (args.Length() < 1 || args[0].IsEmpty() || !args[0]->IsString()) {
            args.GetIsolate()->ThrowException(v8::Exception::TypeError(ASCIIStringToV8String(args.GetIsolate(), "Requires 1 string parameter")));
            return;
        }

        std::string hostname_utf8 = V8StringToUTF8(args.GetIsolate(), v8::Local<v8::String>::Cast(args[0]));
...
int ProxyResolverV8::GetProxyForURL(const GURL& query_url, net::ProxyInfo* results, ProxyResolverV8::JSBindings* bindings)
{
...
int ProxyResolverV8::Create(
    const scoped_refptr<net::PacFileData>& script_data, ProxyResolverV8::JSBindings* js_bindings, std::unique_ptr<ProxyResolverV8>* resolver)
{
    ...
    std::unique_ptr<Context> context(new Context(g_isolate_factory.Get().GetSharedIsolate()));
