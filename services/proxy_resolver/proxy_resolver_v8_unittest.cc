...
public:
    ...
    int CreateResolver(const char* filename)
    {
        ...
        return ProxyResolverV8::Create(net::PacFileData::FromUTF8(file_contents), bindings(), &resolver_);
    }
...
TEST_F(ProxyResolverV8Test, Direct)
{
    ...
    int result = resolver().GetProxyForURL(GURL("http://www.google.com"), &proxy_info, bindings());
...
TEST_F(ProxyResolverV8Test, Basic)
{
    ...
    {
        net::ProxyInfo proxy_info;
        int result = resolver().GetProxyForURL(GURL("http://query.com/path"), &proxy_info, bindings());
        EXPECT_THAT(result, IsOk());
        EXPECT_EQ("http.query.com.path.query.com:80", ProxyServerToProxyUri(proxy_info.proxy_server()));
    }
    {
        net::ProxyInfo proxy_info;
        int result = resolver().GetProxyForURL(GURL("ftp://query.com:90/path"), &proxy_info, bindings());
        EXPECT_THAT(result, IsOk());
        // Note that FindProxyForURL(url, host) does not expect |host| to contain
        // the port number.
        EXPECT_EQ("ftp.query.com.90.path.query.com:80", ProxyServerToProxyUri(proxy_info.proxy_server()));
...
TEST_F(ProxyResolverV8Test, BadReturnType)
{
    ...
    static const char* const filenames[] = { "return_undefined.js", "return_integer.js", "return_function.js", "return_object.js",
...
{
    EXPECT_THAT(CreateResolver("no_entrypoint.js"), IsError(net::ERR_PAC_SCRIPT_FAILED));

    ASSERT_EQ(1U, bindings()->errors.size());
    EXPECT_EQ("FindProxyForURL is undefined or not a function.", bindings()->errors[0]);
    EXPECT_EQ(-1, bindings()->errors_line_number[0]);
}
...
{
    EXPECT_THAT(CreateResolver("missing_close_brace.js"), IsError(net::ERR_PAC_SCRIPT_FAILED));
    ...
    EXPECT_EQ("Uncaught SyntaxError: Unexpected end of input", bindings()->errors[0]);
...
TEST_F(ProxyResolverV8Test, SideEffects)
{
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        int result = resolver().GetProxyForURL(GURL("http://www.google.com"), &proxy_info, bindings());
        EXPECT_THAT(result, IsOk());
        EXPECT_EQ(base::StringPrintf("sideffect_%d:80", i), ProxyServerToProxyUri(proxy_info.proxy_server()));
    }
...
TEST_F(ProxyResolverV8Test, UnhandledException)
{
    ...
    EXPECT_EQ(0U, bindings()->alerts.size());
    ASSERT_EQ(1U, bindings()->errors.size());
    EXPECT_EQ("Uncaught ReferenceError: undefined_variable is not defined", bindings()->errors[0]);
...
{
    EXPECT_EQ(net::ERR_PAC_SCRIPT_FAILED, CreateResolver("exception_findproxyforurl_during_init.js"));

    ASSERT_EQ(2U, bindings()->errors.size());
    EXPECT_EQ("Uncaught crash!", bindings()->errors[0]);
    EXPECT_EQ(9, bindings()->errors_line_number[0]);
    EXPECT_EQ("Accessing FindProxyForURL threw an exception.", bindings()->errors[1]);
...
{
    ASSERT_THAT(CreateResolver("exception_findproxyforurl_during_resolve.js"), IsOk());
...
TEST_F(ProxyResolverV8Test, IPv6HostnamesNotBracketed)
{
    ...
    net::ProxyInfo proxy_info;
    int result = resolver().GetProxyForURL(GURL("http://[abcd::efff]:99/watsupdawg"), &proxy_info, bindings());
...
TEST_F(ProxyResolverV8Test, Terminate)
{
    ...
    result = resolver().GetProxyForURL(GURL("http://kittens/"), &proxy_info, bindings());
