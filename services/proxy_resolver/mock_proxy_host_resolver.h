...
public:
    ...
    std::unique_ptr<Request> CreateRequest(
        const std::string& hostname, net::ProxyResolveDnsOperation operation, const net::NetworkAnonymizationKey& network_anonymization_key) override;

    void SetError(const std::string& hostname, net::ProxyResolveDnsOperation operation, const net::NetworkAnonymizationKey& network_anonymization_key);

    void SetResult(const std::string& hostname, net::ProxyResolveDnsOperation operation, const net::NetworkAnonymizationKey& network_anonymization_key,
        std::vector<net::IPAddress> result);

    void FailAll();

    unsigned num_resolve() const
    {
        return num_resolve_;
    }

private:
    using ResultKey = std::tuple<std::string, net::ProxyResolveDnsOperation, net::NetworkAnonymizationKey>;
...
public:
    ...
    explicit HangingProxyHostResolver(base::RepeatingClosure hang_callback = base::RepeatingClosure());
