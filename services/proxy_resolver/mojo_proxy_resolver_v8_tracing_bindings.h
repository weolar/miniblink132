...
template <typename Client> class MojoProxyResolverV8TracingBindings : public ProxyResolverV8Tracing::Bindings, public HostResolverMojo::Impl {
public:
...
private:
    ...
    {
        DCHECK(thread_checker_.CalledOnValidThread());
        client_->ResolveDns(hostname, operation, network_anonymization_key, std::move(client));
    }
