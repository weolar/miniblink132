...
public:
    Job(mojo::PendingRemote<mojom::ProxyResolverRequestClient> client, ProxyResolverImpl* resolver, const GURL& url);
...
ProxyResolverImpl::ProxyResolverImpl(std::unique_ptr<ProxyResolverV8Tracing> resolver)
...
void ProxyResolverImpl::GetProxyForUrl(
    const GURL& url, const net::NetworkAnonymizationKey& network_anonymization_key, mojo::PendingRemote<mojom::ProxyResolverRequestClient> client)
{
    DVLOG(1) << "GetProxyForUrl(" << url << ")";
    std::unique_ptr<Job> job = std::make_unique<Job>(std::move(client), this, url);
    Job* job_ptr = job.get();
...
ProxyResolverImpl::Job::Job(mojo::PendingRemote<mojom::ProxyResolverRequestClient> client, ProxyResolverImpl* resolver, const GURL& url)
    : resolver_(resolver)
...
void ProxyResolverImpl::Job::Start(const net::NetworkAnonymizationKey& network_anonymization_key)
{
    resolver_->resolver_->GetProxyForURL(url_, network_anonymization_key, &result_, base::BindOnce(&Job::GetProxyDone, base::Unretained(this)), &request_,
        std::make_unique<MojoProxyResolverV8TracingBindings<mojom::ProxyResolverRequestClient>>(client_.get()));
    client_.set_disconnect_handler(base::BindOnce(&ProxyResolverImpl::Job::OnDisconnect, base::Unretained(this)));
}

void ProxyResolverImpl::Job::GetProxyDone(int error)
{
    done_ = true;
    DVLOG(1) << "GetProxyForUrl(" << url_ << ") finished with error " << error << ". " << result_.proxy_list().size() << " Proxies returned:";
