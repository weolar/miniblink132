...
{
    return operation == net::ProxyResolveDnsOperation::DNS_RESOLVE_EX || operation == net::ProxyResolveDnsOperation::MY_IP_ADDRESS_EX;
}

} // namespace

class HostResolverMojo::RequestImpl : public ProxyHostResolver::Request, public mojom::HostResolverRequestClient {
public:
    RequestImpl(const std::string& hostname, net::ProxyResolveDnsOperation operation, const net::NetworkAnonymizationKey& network_anonymization_key,
        base::WeakPtr<ProxyHostResolverCache> host_cache, Impl* impl)
        : hostname_(hostname)
    ...
    int Start(net::CompletionOnceCallback callback) override
    {
        ...
        DCHECK(host_cache_);
        const std::vector<net::IPAddress>* cached_result = host_cache_->LookupEntry(hostname_, network_anonymization_key_, IsExOperation(operation_));
        if (cached_result) {
        ...
        impl_->ResolveDns(hostname_, operation_, network_anonymization_key_, receiver_.BindNewPipeAndPassRemote());
        receiver_.set_disconnect_handler(base::BindOnce(&RequestImpl::OnDisconnect, base::Unretained(this)));
    ...
    void ReportResult(int32_t error, const std::vector<net::IPAddress>& result) override
    {
        ...
        if (error == net::OK) {
            results_ = result;
            if (host_cache_) {
                host_cache_->StoreEntry(hostname_, network_anonymization_key_, IsExOperation(operation_), result);
            }
        }
        receiver_.reset();
...
private:
    void OnDisconnect()
    {
        ReportResult(net::ERR_FAILED, {} /* result */);
    }
...
std::unique_ptr<ProxyHostResolver::Request> HostResolverMojo::CreateRequest(
    const std::string& hostname, net::ProxyResolveDnsOperation operation, const net::NetworkAnonymizationKey& network_anonymization_key)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    return std::make_unique<RequestImpl>(hostname, operation, network_anonymization_key, host_cache_weak_factory_.GetWeakPtr(), impl_);
}
