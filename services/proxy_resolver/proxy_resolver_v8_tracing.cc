...
class ScopedAllowThreadJoinForProxyResolverV8Tracing : public base::ScopedAllowBaseSyncPrimitivesOutsideBlockingScope { };
...
// worker thread.
class Job : public base::RefCountedThreadSafe<Job>, public ProxyResolverV8::JSBindings {
public:
    struct Params {
        Params(const scoped_refptr<base::SingleThreadTaskRunner>& worker_task_runner, int* num_outstanding_callbacks)
            : v8_resolver(nullptr)
    ...
    // outlive it.
    Job(const Params* params, std::unique_ptr<ProxyResolverV8Tracing::Bindings> bindings);

    // Called from origin thread.
    void StartCreateV8Resolver(
        const scoped_refptr<net::PacFileData>& script_data, std::unique_ptr<ProxyResolverV8>* resolver, net::CompletionOnceCallback callback);

    // Called from origin thread.
    void StartGetProxyForURL(
        const GURL& url, const net::NetworkAnonymizationKey& network_anonymization_key, net::ProxyInfo* results, net::CompletionOnceCallback callback);
...
private:
    ...
    void Start(Operation op, bool blocking_dns, net::CompletionOnceCallback callback);
    ...
    bool ResolveDns(const std::string& host, net::ProxyResolveDnsOperation op, std::string* output, bool* terminate) override;
    void Alert(const std::u16string& message) override;
    void OnError(int line_number, const std::u16string& error) override;

    bool ResolveDnsBlocking(const std::string& host, net::ProxyResolveDnsOperation op, std::string* output);

    bool ResolveDnsNonBlocking(const std::string& host, net::ProxyResolveDnsOperation op, std::string* output, bool* terminate);

    [[nodiscard]] bool PostDnsOperationAndWait(const std::string& host, net::ProxyResolveDnsOperation op, bool* completed_synchronously);
    ...
    bool GetDnsFromLocalCache(const std::string& host, net::ProxyResolveDnsOperation op, std::string* output, bool* return_value);

    void SaveDnsToLocalCache(const std::string& host, net::ProxyResolveDnsOperation op, int net_error, const std::vector<net::IPAddress>& addresses);

    // Makes a key for looking up |host, op| in |dns_cache_|. Strings are used for
    // convenience, to avoid defining custom comparators.
    static std::string MakeDnsCacheKey(const std::string& host, net::ProxyResolveDnsOperation op);

    void HandleAlertOrError(bool is_alert, int line_number, const std::u16string& message);
    void DispatchBufferedAlertsAndErrors();
    void DispatchAlertOrErrorOnOriginThread(bool is_alert, int line_number, const std::u16string& message);
    ...
    raw_ptr<net::ProxyInfo> user_results_; // Owned by caller, lives on origin thread.
    GURL url_;
...
public:
    ProxyResolverV8TracingImpl(std::unique_ptr<base::Thread> thread, std::unique_ptr<ProxyResolverV8> resolver, std::unique_ptr<Job::Params> job_params);

    ProxyResolverV8TracingImpl(const ProxyResolverV8TracingImpl&) = delete;
    ProxyResolverV8TracingImpl& operator=(const ProxyResolverV8TracingImpl&) = delete;

    ~ProxyResolverV8TracingImpl() override;

    // ProxyResolverV8Tracing overrides.
    void GetProxyForURL(const GURL& url, const net::NetworkAnonymizationKey& network_anonymization_key, net::ProxyInfo* results,
        net::CompletionOnceCallback callback, std::unique_ptr<net::ProxyResolver::Request>* request, std::unique_ptr<Bindings> bindings) override;
...
Job::Job(const Job::Params* params, std::unique_ptr<ProxyResolverV8Tracing::Bindings> bindings)
    : origin_runner_(base::ThreadTaskRunnerHandle::Get())
    , params_(params)
    , bindings_(std::move(bindings))
    , event_(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED)
    , last_num_dns_(0)
{
...
void Job::NotifyCaller(int result)
{
    ...
    origin_runner_->PostTask(FROM_HERE, base::BindOnce(&Job::NotifyCallerOnOriginLoop, this, result));
}
...
void Job::Start(Operation op, bool blocking_dns, net::CompletionOnceCallback callback)
{
    CheckIsOnOriginThread();
    ...
    worker_task_runner()->PostTask(FROM_HERE, blocking_dns_ ? base::BindOnce(&Job::ExecuteBlocking, this) : base::BindOnce(&Job::ExecuteNonBlocking, this));
}
...
int Job::ExecuteProxyResolver()
{
    ...
    case GET_PROXY_FOR_URL: {
        result = v8_resolver()->GetProxyForURL(url_,
...
bool Job::ResolveDns(const std::string& host, net::ProxyResolveDnsOperation op, std::string* output, bool* terminate)
{
    if (cancelled_.IsSet()) {
        *terminate = true;
        return false;
    ...
    if ((op == net::ProxyResolveDnsOperation::DNS_RESOLVE || op == net::ProxyResolveDnsOperation::DNS_RESOLVE_EX) && host.empty()) {
...
bool Job::ResolveDnsBlocking(const std::string& host, net::ProxyResolveDnsOperation op, std::string* output)
{
    CheckIsOnWorkerThread();
...
bool Job::ResolveDnsNonBlocking(const std::string& host, net::ProxyResolveDnsOperation op, std::string* output, bool* terminate)
{
    CheckIsOnWorkerThread();
...
bool Job::PostDnsOperationAndWait(const std::string& host, net::ProxyResolveDnsOperation op, bool* completed_synchronously)
{
    ...
    pending_dns_op_ = op;
    origin_runner_->PostTask(FROM_HERE, base::BindOnce(&Job::DoDnsOperation, this));
...
void Job::DoDnsOperation()
{
    ...
    bool is_my_ip_request
        = pending_dns_op_ == net::ProxyResolveDnsOperation::MY_IP_ADDRESS || pending_dns_op_ == net::ProxyResolveDnsOperation::MY_IP_ADDRESS_EX;
    pending_dns_ = host_resolver()->CreateRequest(is_my_ip_request ? net::GetHostName() : pending_dns_host_, pending_dns_op_,
        is_my_ip_request ? net::NetworkAnonymizationKey() : network_anonymization_key_);
    int result = pending_dns_->Start(base::BindOnce(&Job::OnDnsOperationComplete, this));
...
void Job::OnDnsOperationComplete(int result)
{
    ...
    SaveDnsToLocalCache(pending_dns_host_, pending_dns_op_, result, pending_dns_->GetResults());
    pending_dns_.reset();
    ...
    if (!blocking_dns_ && !pending_dns_completed_synchronously_) {
        ...
        // cached items.
        worker_task_runner()->PostTask(FROM_HERE, base::BindOnce(&Job::ExecuteNonBlocking, this));
    }
}
...
bool Job::GetDnsFromLocalCache(const std::string& host, net::ProxyResolveDnsOperation op, std::string* output, bool* return_value)
{
    CheckIsOnWorkerThread();
...
void Job::SaveDnsToLocalCache(const std::string& host, net::ProxyResolveDnsOperation op, int net_error, const std::vector<net::IPAddress>& addresses)
{
    CheckIsOnOriginThread();
    ...
    } else if (op == net::ProxyResolveDnsOperation::DNS_RESOLVE || op == net::ProxyResolveDnsOperation::MY_IP_ADDRESS) {
...
std::string Job::MakeDnsCacheKey(const std::string& host, net::ProxyResolveDnsOperation op)
{
    return base::StringPrintf("%d:%s", static_cast<int>(op), host.c_str());
}

void Job::HandleAlertOrError(bool is_alert, int line_number, const std::u16string& message)
{
    CheckIsOnWorkerThread();
    ...
    if (blocking_dns_) {
        ...
        origin_runner_->PostTask(FROM_HERE, base::BindOnce(&Job::DispatchAlertOrErrorOnOriginThread, this, is_alert, line_number, message));
        return;
    }
...
void Job::DispatchAlertOrErrorOnOriginThread(bool is_alert, int line_number, const std::u16string& message)
{
    CheckIsOnOriginThread();
...
ProxyResolverV8TracingImpl::ProxyResolverV8TracingImpl(
    std::unique_ptr<base::Thread> thread, std::unique_ptr<ProxyResolverV8> resolver, std::unique_ptr<Job::Params> job_params)
...
void ProxyResolverV8TracingImpl::GetProxyForURL(const GURL& url, const net::NetworkAnonymizationKey& network_anonymization_key, net::ProxyInfo* results,
    net::CompletionOnceCallback callback, std::unique_ptr<net::ProxyResolver::Request>* request, std::unique_ptr<Bindings> bindings)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    *request = std::make_unique<RequestImpl>(job);
    ...
    job->StartGetProxyForURL(url, network_anonymization_key, results, std::move(callback));
}
...
public:
    ...
    ProxyResolverV8TracingFactoryImpl(const ProxyResolverV8TracingFactoryImpl&) = delete;
    ProxyResolverV8TracingFactoryImpl& operator=(const ProxyResolverV8TracingFactoryImpl&) = delete;

    ~ProxyResolverV8TracingFactoryImpl() override;

    void CreateProxyResolverV8Tracing(const scoped_refptr<net::PacFileData>& pac_script, std::unique_ptr<ProxyResolverV8Tracing::Bindings> bindings,
        std::unique_ptr<ProxyResolverV8Tracing>* resolver, net::CompletionOnceCallback callback,
...
class ProxyResolverV8TracingFactoryImpl::CreateJob : public net::ProxyResolverFactory::Request {
public:
    CreateJob(ProxyResolverV8TracingFactoryImpl* factory, std::unique_ptr<ProxyResolverV8Tracing::Bindings> bindings,
        const scoped_refptr<net::PacFileData>& pac_script, std::unique_ptr<ProxyResolverV8Tracing>* resolver_out, net::CompletionOnceCallback callback)
        : factory_(factory)
        ...
        , num_outstanding_callbacks_(0)
    {
        ...
        job_params_ = std::make_unique<Job::Params>(thread_->task_runner(), &num_outstanding_callbacks_);
        create_resolver_job_ = new Job(job_params_.get(), std::move(bindings));
        create_resolver_job_->StartCreateV8Resolver(
            pac_script, &v8_resolver_, base::BindOnce(&ProxyResolverV8TracingFactoryImpl::CreateJob::OnV8ResolverCreated, base::Unretained(this)));
    }
...
private:
    void OnV8ResolverCreated(int error)
    {
        ...
        if (error == net::OK) {
            ...
            *resolver_out_ = std::make_unique<ProxyResolverV8TracingImpl>(std::move(thread_), std::move(v8_resolver_), std::move(job_params_));
        } else {
            StopWorkerThread();
        }
...
ProxyResolverV8TracingFactoryImpl::ProxyResolverV8TracingFactoryImpl() = default;
...
void ProxyResolverV8TracingFactoryImpl::CreateProxyResolverV8Tracing(const scoped_refptr<net::PacFileData>& pac_script,
    std::unique_ptr<ProxyResolverV8Tracing::Bindings> bindings, std::unique_ptr<ProxyResolverV8Tracing>* resolver, net::CompletionOnceCallback callback,
    std::unique_ptr<net::ProxyResolverFactory::Request>* request)
{
    std::unique_ptr<CreateJob> job(new CreateJob(this, std::move(bindings), pac_script, resolver, std::move(callback)));
    jobs_.insert(job.get());
    *request = std::move(job);
}

void ProxyResolverV8TracingFactoryImpl::RemoveJob(ProxyResolverV8TracingFactoryImpl::CreateJob* job)
{
...
// static
std::unique_ptr<ProxyResolverV8TracingFactory> ProxyResolverV8TracingFactory::Create()
{
