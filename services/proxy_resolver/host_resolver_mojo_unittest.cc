...
public:
    MockMojoHostResolverRequest(mojo::PendingRemote<mojom::HostResolverRequestClient> client, base::OnceClosure error_callback)
        : client_(std::move(client))
        , error_callback_(std::move(error_callback))
    {
        client_.set_disconnect_handler(base::BindOnce(&MockMojoHostResolverRequest::OnDisconnect, base::Unretained(this)));
    }
...
struct HostResolverAction {
    ...
    static HostResolverAction ReturnResult(std::vector<net::IPAddress> addresses)
    {
        HostResolverAction result;
...
public:
    ...
    struct RequestInfo {
        ...
        {
            return hostname == other.hostname && network_anonymization_key == other.network_anonymization_key;
        }
    };

    explicit MockMojoHostResolver(base::RepeatingClosure request_connection_error_callback)
        : request_connection_error_callback_(std::move(request_connection_error_callback))
    {
    }
    ...
    const std::vector<RequestInfo>& request_info() const
    {
        ...
        mojo::PendingRemote<mojom::HostResolverRequestClient> client) override
    {
        ...
        case HostResolverAction::COMPLETE:
            mojo::Remote<mojom::HostResolverRequestClient>(std::move(client))
                ->ReportResult(actions_[results_returned_].error, actions_[results_returned_].addresses);
            break;
        case HostResolverAction::RETAIN:
            requests_.push_back(std::make_unique<MockMojoHostResolverRequest>(std::move(client), base::BindOnce(request_connection_error_callback_)));
            break;
...
protected:
    ...
    HostResolverMojoTest()
        : mock_resolver_(base::BindRepeating(&Waiter::NotifyEvent, base::Unretained(&waiter_), ConnectionErrorSource::REQUEST))
        , resolver_(&mock_resolver_)
    {
    }

    int Resolve(const std::string& hostname, const net::NetworkAnonymizationKey& network_anonymization_key, std::vector<net::IPAddress>* out_addresses)
    {
        std::unique_ptr<ProxyHostResolver::Request> request
            = resolver_.CreateRequest(hostname, net::ProxyResolveDnsOperation::DNS_RESOLVE_EX, network_anonymization_key);
...
{
    const net::SchemefulSite kSite = net::SchemefulSite(GURL("https://not-example.com/"));
    ...
    EXPECT_THAT(Resolve("example.com", kNetworkAnonymizationKey, &result), IsOk());
    ...
    EXPECT_EQ(kNetworkAnonymizationKey, mock_resolver_.request_info()[0].network_anonymization_key);
}
...
TEST_F(HostResolverMojoTest, ResolveCachedResult)
{
    ...
    std::vector<net::IPAddress> result;
    ASSERT_THAT(Resolve("example.com", net::NetworkAnonymizationKey(), &result), IsOk());
    ASSERT_EQ(1u, mock_resolver_.request_info().size());

    // Expect results from cache.
    result.clear();
    EXPECT_THAT(Resolve("example.com", net::NetworkAnonymizationKey(), &result), IsOk());
    EXPECT_EQ(addresses, result);
...
TEST_F(HostResolverMojoTest, ResolveCachedResultWithNetworkAnonymizationKey)
{
    ...
    std::vector<net::IPAddress> result;
    ASSERT_THAT(Resolve("example.com", kNetworkAnonymizationKey, &result), IsOk());
    ASSERT_EQ(1u, mock_resolver_.request_info().size());
...
TEST_F(HostResolverMojoTest, Multiple)
{
    ...
    mock_resolver_.AddAction(HostResolverAction::ReturnResult(addresses));
    mock_resolver_.AddAction(HostResolverAction::ReturnError(net::ERR_NAME_NOT_RESOLVED));

    std::unique_ptr<ProxyHostResolver::Request> request1
        = resolver_.CreateRequest("example.com", net::ProxyResolveDnsOperation::DNS_RESOLVE_EX, net::NetworkAnonymizationKey());
    std::unique_ptr<ProxyHostResolver::Request> request2
        = resolver_.CreateRequest("example.org", net::ProxyResolveDnsOperation::DNS_RESOLVE_EX, net::NetworkAnonymizationKey());
    ...
    EXPECT_THAT(callback2.GetResult(net::ERR_IO_PENDING), IsError(net::ERR_NAME_NOT_RESOLVED));
    EXPECT_EQ(addresses, request1->GetResults());
    ASSERT_EQ(0u, request2->GetResults().size());

    EXPECT_THAT(mock_resolver_.request_info(),
        testing::ElementsAre(MockMojoHostResolver::RequestInfo { "example.com", net::NetworkAnonymizationKey() },
            MockMojoHostResolver::RequestInfo { "example.org", net::NetworkAnonymizationKey() }));
}
...
TEST_F(HostResolverMojoTest, Error)
{
    ...
    std::vector<net::IPAddress> result;
    EXPECT_THAT(Resolve("example.com", net::NetworkAnonymizationKey(), &result), IsError(net::ERR_NAME_NOT_RESOLVED));
...
TEST_F(HostResolverMojoTest, ImplDropsClientConnection)
{
    ...
    std::vector<net::IPAddress> result;
    EXPECT_THAT(Resolve("example.com", net::NetworkAnonymizationKey(), &result), IsError(net::ERR_FAILED));
    EXPECT_TRUE(result.empty());
