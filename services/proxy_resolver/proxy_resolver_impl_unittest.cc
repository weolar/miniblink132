...
public:
    ...
    explicit TestRequestClient(mojo::PendingReceiver<mojom::ProxyResolverRequestClient> receiver);
    ...
    net::EventWaiter<Event>& event_waiter()
    {
...
private:
    ...
    void ResolveDns(const std::string& hostname, net::ProxyResolveDnsOperation operation, const net::NetworkAnonymizationKey& network_anonymization_key,
...
TestRequestClient::TestRequestClient(mojo::PendingReceiver<mojom::ProxyResolverRequestClient> receiver)
    : receiver_(this, std::move(receiver))
{
    receiver_.set_disconnect_handler(base::BindOnce(&TestRequestClient::OnDisconnect, base::Unretained(this)));
}
...
void TestRequestClient::ReportResult(int32_t error, const net::ProxyInfo& results)
{
...
void TestRequestClient::OnError(int32_t line_number, const std::string& message)
{
}

void TestRequestClient::ResolveDns(const std::string& hostname, net::ProxyResolveDnsOperation operation,
    const net::NetworkAnonymizationKey& network_anonymization_key, mojo::PendingRemote<mojom::HostResolverRequestClient> client)
{
}
...
void MockProxyResolverV8Tracing::GetProxyForURL(const GURL& url, const net::NetworkAnonymizationKey& network_anonymization_key, net::ProxyInfo* results,
...
public:
    ...
    {
        std::unique_ptr<MockProxyResolverV8Tracing> mock_resolver = std::make_unique<MockProxyResolverV8Tracing>();
        mock_proxy_resolver_ = mock_resolver.get();
        resolver_impl_ = std::make_unique<ProxyResolverImpl>(std::move(mock_resolver));
...
TEST_F(ProxyResolverImplTest, GetProxyForUrl)
{
    ...
    resolver_->GetProxyForUrl(GURL("http://example.com"), net::NetworkAnonymizationKey(), std::move(remote_client));
    ASSERT_EQ(1u, mock_proxy_resolver_->pending_jobs().size());
    MockProxyResolverV8Tracing::Job* job = mock_proxy_resolver_->pending_jobs()[0].get();
    EXPECT_EQ(GURL("http://example.com"), job->url);

    job->results->UsePacString("PROXY proxy.example.com:1; "
...
{
    const net::SchemefulSite kSite(net::SchemefulSite(GURL("https://site.test/")));
    const net::NetworkAnonymizationKey kNetworkAnonymizationKey(kSite, kSite);

    mojo::PendingRemote<mojom::ProxyResolverRequestClient> remote_client;
    TestRequestClient client(remote_client.InitWithNewPipeAndPassReceiver());

    resolver_->GetProxyForUrl(GURL("http://example.com"), kNetworkAnonymizationKey, std::move(remote_client));
    ASSERT_EQ(1u, mock_proxy_resolver_->pending_jobs().size());
    MockProxyResolverV8Tracing::Job* job = mock_proxy_resolver_->pending_jobs()[0].get();
    EXPECT_EQ(GURL("http://example.com"), job->url);
...
TEST_F(ProxyResolverImplTest, GetProxyForUrlFailure)
{
    ...
    std::vector<net::ProxyServer> proxy_servers = client.results().proxy_list().GetAll();
...
TEST_F(ProxyResolverImplTest, GetProxyForUrlMultiple)
{
    ...
    resolver_->GetProxyForUrl(GURL("http://example.com"), net::NetworkAnonymizationKey(), std::move(remote_client1));
    resolver_->GetProxyForUrl(GURL("https://example.com"), net::NetworkAnonymizationKey(), std::move(remote_client2));
    ASSERT_EQ(2u, mock_proxy_resolver_->pending_jobs().size());
    MockProxyResolverV8Tracing::Job* job1 = mock_proxy_resolver_->pending_jobs()[0].get();
    EXPECT_EQ(GURL("http://example.com"), job1->url);
    MockProxyResolverV8Tracing::Job* job2 = mock_proxy_resolver_->pending_jobs()[1].get();
    ...
    std::vector<net::ProxyServer> proxy_servers1 = client1.results().proxy_list().GetAll();
    ...
    std::vector<net::ProxyServer> proxy_servers2 = client2.results().proxy_list().GetAll();
    ASSERT_EQ(1u, proxy_servers1.size());
...
{
    mojo::PendingRemote<mojom::ProxyResolverRequestClient> remote_client;
    auto client = std::make_unique<TestRequestClient>(remote_client.InitWithNewPipeAndPassReceiver());

    resolver_->GetProxyForUrl(GURL("http://example.com"), net::NetworkAnonymizationKey(), std::move(remote_client));
    ASSERT_EQ(1u, mock_proxy_resolver_->pending_jobs().size());
    const MockProxyResolverV8Tracing::Job* job = mock_proxy_resolver_->pending_jobs()[0].get();
    EXPECT_EQ(GURL("http://example.com"), job->url);
