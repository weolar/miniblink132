...
public:
    ...
    void StoreEntry(std::string hostname, net::NetworkAnonymizationKey network_anonymization_key, bool is_ex_operation, std::vector<net::IPAddress> results);

    // Returns `nullptr` if entry not found or expired. Erases from cache if
    // expired.
    const std::vector<net::IPAddress>* LookupEntry(std::string hostname, net::NetworkAnonymizationKey network_anonymization_key, bool is_ex_operation);
...
private:
    struct Key {
        bool operator<(const Key& other) const
        {
            return std::tie(hostname, network_anonymization_key, is_ex_operation)
                < std::tie(other.hostname, other.network_anonymization_key, other.is_ex_operation);
        }
    ...
    struct Entry {
        Entry(std::vector<net::IPAddress> results, base::TimeTicks expiration, ExpirationList::iterator expiration_list_it);
        ~Entry();
        Entry(Entry&&);
