...
private:
    ...
    void UpdateAppURL(SourceId source_id, const GURL& url, const AppType app_type) override;
    void RecordNavigation(SourceId source_id, const UkmSource::NavigationData& navigation_data) override;
    //void AddEntry(mojom::UkmEntryPtr entry) override;
    void MarkSourceForDeletion(SourceId source_id) override;

    class Delegate final {
    public:
        Delegate(scoped_refptr<base::SequencedTaskRunner> task_runner, base::WeakPtr<UkmRecorder> ptr);
        Delegate(const Delegate& other);
        ~Delegate();

        void UpdateSourceURL(SourceId source_id, const GURL& url);
        void UpdateAppURL(SourceId source_id, const GURL& url, const AppType app_type);
        void RecordNavigation(SourceId source_id, const UkmSource::NavigationData& navigation_data);
