...
public:
    ...
    struct METRICS_EXPORT NavigationData {
        ...
        NavigationData CopyWithSanitizedUrls(std::vector<GURL> sanitized_urls) const;
    ...
    {
        return navigation_data_.urls.back();
    }

    const std::vector<GURL>& urls() const
    {
        return navigation_data_.urls;
    }

    const NavigationData& navigation_data() const
    {
