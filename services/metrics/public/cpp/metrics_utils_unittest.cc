...
TEST(MetricsUtilsTest, GetLinearBucketMin)
{
    ...
    for (const auto& test : int_test_cases) {
        EXPECT_EQ(test.expected_result, ukm::GetLinearBucketMin(test.sample, test.bucket_size))
            << "For sample: " << test.sample << " with bucket_size: " << test.bucket_size;
    }
...
TEST(MetricsUtilsTest, GetExponentialBucketMinForUserTiming)
{
    ...
    // Test int64_t sample cases.
    for (const auto& test : int_test_cases) {
        EXPECT_EQ(test.expected_result, ukm::GetExponentialBucketMinForUserTiming(test.sample)) << "For sample: " << test.sample;
    }
}
