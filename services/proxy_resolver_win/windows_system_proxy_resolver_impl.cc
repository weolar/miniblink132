...
bool GetProxyServerFromWinHttpResultEntry(const WINHTTP_PROXY_RESULT_ENTRY& result_entry, net::ProxyServer* out_proxy_server)
{
    ...
    std::wstring host_wide(result_entry.pwszProxy, wcslen(result_entry.pwszProxy));
    if (!base::IsStringASCII(host_wide)) {
        const int kInitialBufferSize = 256;
        url::RawCanonOutputT<char16_t, kInitialBufferSize> punycode_output;
        if (!url::IDNToASCII(base::as_u16cstr(host_wide), host_wide.length(), &punycode_output)) {
            return false;
        }

        host_wide.assign(base::as_wcstr(punycode_output.data()), punycode_output.length());
    }
...
public:
    Request(WindowsSystemProxyResolverImpl* parent, GetProxyForUrlCallback callback);
    ...
    void DoWinHttpStatusCallback(HINTERNET resolver_handle, DWORD status, int windows_error);
...
private:
    ...
    void ReportResult(const net::ProxyList& proxy_list, net::WinHttpStatus winhttp_status, int windows_error);
...
WindowsSystemProxyResolverImpl::Request::Request(WindowsSystemProxyResolverImpl* parent, GetProxyForUrlCallback callback)
    : parent_(parent)
...
bool WindowsSystemProxyResolverImpl::Request::Start(const GURL& url)
{
    ...
    if (!winhttp_api_wrapper()->CallWinHttpGetIEProxyConfigForCurrentUser(scoped_ie_config.config())) {
        const int error = GetLastError();
        ReportResult(net::ProxyList(), net::WinHttpStatus::kWinHttpGetIEProxyConfigForCurrentUserFailed, error);
        return false;
    }

    // This will create a handle specifically for WinHttpGetProxyForUrlEx().
    if (!winhttp_api_wrapper()->CallWinHttpCreateProxyResolver(&resolver_handle_)) {
        const int error = GetLastError();
        ReportResult(net::ProxyList(), net::WinHttpStatus::kWinHttpCreateProxyResolverFailed, error);
        return false;
    }
    ...
    autoproxy_options.dwFlags = WINHTTP_AUTOPROXY_ALLOW_STATIC | WINHTTP_AUTOPROXY_ALLOW_CM;
    ...
    if (scoped_ie_config.config()->lpszAutoConfigUrl) {
        ...
        autoproxy_options.lpszAutoConfigUrl = scoped_ie_config.config()->lpszAutoConfigUrl;
    }
    ...
    if (!winhttp_api_wrapper()->CallWinHttpGetProxyForUrlEx(resolver_handle_, url.spec(), &autoproxy_options, context)) {
        const int error = GetLastError();
        winhttp_api_wrapper()->CallWinHttpCloseHandle(resolver_handle_);
        resolver_handle_ = nullptr;
        ReportResult(net::ProxyList(), net::WinHttpStatus::kWinHttpGetProxyForURLExFailed, error);
        return false;
    }
...
void WindowsSystemProxyResolverImpl::Request::DoWinHttpStatusCallback(HINTERNET resolver_handle, DWORD status, int windows_error)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (callback_) {
        ...
        case WINHTTP_CALLBACK_STATUS_REQUEST_ERROR:
            ...
            ReportResult(net::ProxyList(), net::WinHttpStatus::kStatusCallbackFailed, windows_error);
            break;
        default:
            LOG(WARNING) << "DoWinHttpStatusCallback() expects only callbacks for "
                         ...
                         << status;
            ReportResult(net::ProxyList(), net::WinHttpStatus::kStatusCallbackFailed, status);
            break;
        }
    }
...
void WindowsSystemProxyResolverImpl::Request::GetProxyResultForCallback()
{
    ...
    if (!winhttp_api_wrapper()->CallWinHttpGetProxyResult(resolver_handle_, &proxy_result)) {
        const int error = GetLastError();
        ReportResult(net::ProxyList(), net::WinHttpStatus::kWinHttpGetProxyResultFailed, error);
        return;
    }
    ...
    for (DWORD i = 0u; i < proxy_result.cEntries; ++i) {
        ...
        if (GetProxyServerFromWinHttpResultEntry(proxy_result.pEntries[i], &proxy_server)) {
    ...
    net::WinHttpStatus winhttp_status = proxy_list.IsEmpty() ? net::WinHttpStatus::kEmptyProxyList : net::WinHttpStatus::kOk;
    ReportResult(proxy_list, winhttp_status, 0);
}

void WindowsSystemProxyResolverImpl::Request::ReportResult(const net::ProxyList& proxy_list, net::WinHttpStatus winhttp_status, int windows_error)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (!callback_)
        return;
...
WindowsSystemProxyResolverImpl::WindowsSystemProxyResolverImpl(mojo::PendingReceiver<mojom::WindowsSystemProxyResolver> receiver)
...
void WindowsSystemProxyResolverImpl::SetCreateWinHttpAPIWrapperForTesting(std::unique_ptr<WinHttpAPIWrapper> winhttp_api_wrapper_for_testing)
{
    winhttp_api_wrapper_ = std::move(winhttp_api_wrapper_for_testing);
}

void WindowsSystemProxyResolverImpl::GetProxyForUrl(const GURL& url, GetProxyForUrlCallback callback)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    std::unique_ptr<Request> request = std::make_unique<Request>(this, std::move(callback));
...
net::WinHttpStatus WindowsSystemProxyResolverImpl::EnsureInitialized()
{
    ...
    if (!WinHttpProxyResolverFunctions::GetInstance().are_all_functions_loaded()) {
    ...
    if (winhttp_api_wrapper_) {
        ...
        uninitialized_winhttp_api_wrapper = std::make_unique<WinHttpAPIWrapperImpl>();
    }
    ...
    if (!uninitialized_winhttp_api_wrapper->CallWinHttpSetTimeouts(10000, 10000, 5000, 5000)) {
    ...
    // above.
    if (!uninitialized_winhttp_api_wrapper->CallWinHttpSetStatusCallback(&WindowsSystemProxyResolverImpl::WinHttpStatusCallback)) {
...
// static
void CALLBACK WindowsSystemProxyResolverImpl::WinHttpStatusCallback(HINTERNET resolver_handle, DWORD_PTR context, DWORD status, void* info, DWORD info_len)
{
    ...
    request->sequenced_task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&Request::DoWinHttpStatusCallback, base::Unretained(request), resolver_handle, status, windows_error));
}
