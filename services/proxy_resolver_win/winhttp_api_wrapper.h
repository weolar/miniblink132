...
public:
    ...
    [[nodiscard]] virtual bool CallWinHttpSetTimeouts(int resolve_timeout, int connect_timeout, int send_timeout, int receive_timeout) = 0;

    // Sets the callback WinHttp will call into with the result of any
    // asynchronous call.
    [[nodiscard]] virtual bool CallWinHttpSetStatusCallback(WINHTTP_STATUS_CALLBACK internet_callback) = 0;
    ...
    // connection/user.
    [[nodiscard]] virtual bool CallWinHttpGetIEProxyConfigForCurrentUser(WINHTTP_CURRENT_USER_IE_PROXY_CONFIG* ie_proxy_config) = 0;

    // Creates a handle |resolver_handle| that should be used for the call to
    // WinHttpGetProxyForUrlEx().
    [[nodiscard]] virtual bool CallWinHttpCreateProxyResolver(HINTERNET* out_resolver_handle) = 0;
    ...
    [[nodiscard]] virtual bool CallWinHttpGetProxyResult(HINTERNET resolver_handle, WINHTTP_PROXY_RESULT* proxy_result) = 0;

    // Frees the |proxy_result| retrieved by CallWinHttpGetProxyResult().
    virtual void CallWinHttpFreeProxyResult(WINHTTP_PROXY_RESULT* proxy_result) = 0;
