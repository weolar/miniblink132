...
std::vector<std::string> CanonicalizeAllowlist(const std::vector<std::string>& allowlist, std::vector<std::string>* rejected_patterns)
{
    return SecureOriginAllowlist::CanonicalizeAllowlistForTesting(allowlist, rejected_patterns);
}
...
TEST_F(SecureOriginAllowlistTest, UnsafelyTreatInsecureOriginAsSecure)
{
    ...
    command_line->AppendSwitchASCII(switches::kUnsafelyTreatInsecureOriginAsSecure, "http://example.com,http://127.example.com");
    SecureOriginAllowlist::GetInstance().ResetForTesting();
    ...
    EXPECT_FALSE(IsUrlPotentiallyTrustworthy("http://foobar.127.example.com/a.html"));
...
TEST_F(SecureOriginAllowlistTest, HostnamePatterns)
{
    ...
    for (const auto& test : kTestCases) {
        ...
        base::test::ScopedCommandLine scoped_command_line;
        base::CommandLine* command_line = scoped_command_line.GetProcessCommandLine();
        command_line->AppendSwitchASCII(switches::kUnsafelyTreatInsecureOriginAsSecure, test.pattern);
        SecureOriginAllowlist::GetInstance().ResetForTesting();
        ...
        EXPECT_EQ(test.expected_secure, IsUrlPotentiallyTrustworthy(test.test_input));
    }
}
...
TEST_F(SecureOriginAllowlistTest, MixOfOriginAndHostnamePatterns)
{
    base::test::ScopedCommandLine scoped_command_line;
    base::CommandLine* command_line = scoped_command_line.GetProcessCommandLine();
    command_line->AppendSwitchASCII(switches::kUnsafelyTreatInsecureOriginAsSecure, "http://example.com,*.foo.com,http://10.20.30.40");
    SecureOriginAllowlist::GetInstance().ResetForTesting();
...
TEST_F(SecureOriginAllowlistTest, Canonicalization)
{
    ...
    rejected.clear();
    canonicalized = CanonicalizeAllowlist({ "*.example.com", "*.example.com:1234", "!@#$%^&---.*.com" }, &rejected);
    EXPECT_THAT(rejected, ::testing::ElementsAre("*.example.com:1234", "!@#$%^&---.*.com"));
...
INSTANTIATE_TYPED_TEST_SUITE_P(UrlOrigin, AbstractTrustworthinessTest, TrustworthinessTestTraits);
