...
public:
    explicit CloneCheckingURLLoaderFactory(scoped_refptr<base::SequencedTaskRunner> owning_thread)
...
protected:
    void SetUp() override
    {
        ...
        loader_thread_ = base::ThreadPool::CreateSequencedTaskRunner({ base::MayBlock(), base::WithBaseSyncPrimitives() });

        test_url_loader_factory_ = std::make_unique<CloneCheckingURLLoaderFactory>(loader_thread_);
        test_url_loader_factory_->SetInterceptor(base::BindRepeating(&CrossThreadPendingSharedURLLoaderFactoryTest::CheckLoaderThread, base::Unretained(this)));
        test_url_loader_factory_->AddResponse(kUrl, kData);

        shared_factory_ = base::MakeRefCounted<WeakWrapperSharedURLLoaderFactory>(test_url_loader_factory_.get());

        base::RunLoop run_loop;
        loader_thread_->PostTaskAndReply(FROM_HERE,
            base::BindOnce(&CrossThreadPendingSharedURLLoaderFactoryTest::SetupFactoryInfoOnLoaderThread, base::Unretained(this)), run_loop.QuitClosure());
        run_loop.Run();
    }
    ...
    void TearDown() override
    {
        ...
        base::RunLoop run_loop;
        loader_thread_->PostTaskAndReply(FROM_HERE,
            base::BindOnce(&CrossThreadPendingSharedURLLoaderFactoryTest::ReleaseSharedFactoryOnLoaderThread, base::Unretained(this)), run_loop.QuitClosure());
        run_loop.Run();
    }
    ...
    void SetupFactoryInfoOnLoaderThread()
    {
        ...
        pending_factory_ = std::make_unique<CrossThreadPendingSharedURLLoaderFactory>(shared_factory_);
    }
    ...
    void TestLoad(scoped_refptr<SharedURLLoaderFactory> factory, scoped_refptr<base::SequencedTaskRunner> client_runner, base::OnceClosure quit_closure)
    {
        ...
        std::unique_ptr<SimpleURLLoader> loader = SimpleURLLoader::Create(std::move(request), TRAFFIC_ANNOTATION_FOR_TESTS);
        SimpleURLLoader* loader_raw = loader.get();
        loader_raw->DownloadToStringOfUnboundedSizeUntilCrashAndDie(factory.get(),
            base::BindOnce(
                [](scoped_refptr<base::SequencedTaskRunner> client_runner, base::OnceClosure quit_closure, std::unique_ptr<SimpleURLLoader> loader,
...
TEST_F(CrossThreadPendingSharedURLLoaderFactoryTest, Basic)
{
    ...
    scoped_refptr<SharedURLLoaderFactory> main_thread_factory = SharedURLLoaderFactory::Create(std::move(pending_factory_));
...
TEST_F(CrossThreadPendingSharedURLLoaderFactoryTest, FurtherClone)
{
    ...
    scoped_refptr<SharedURLLoaderFactory> main_thread_factory = SharedURLLoaderFactory::Create(std::move(pending_factory_));
    scoped_refptr<SharedURLLoaderFactory> main_thread_factory_clone = SharedURLLoaderFactory::Create(main_thread_factory->Clone());
...
TEST_F(CrossThreadPendingSharedURLLoaderFactoryTest, CloneThirdThread)
{
    ...
    scoped_refptr<base::SequencedTaskRunner> third_thread = base::ThreadPool::CreateSequencedTaskRunner({ base::MayBlock(), base::WithBaseSyncPrimitives() });

    scoped_refptr<SharedURLLoaderFactory> main_thread_factory = SharedURLLoaderFactory::Create(std::move(pending_factory_));
    std::unique_ptr<PendingSharedURLLoaderFactory> new_pending_factory = main_thread_factory->Clone();

    base::RunLoop run_loop;
    base::OnceClosure run_loop_quit = run_loop.QuitClosure();
    third_thread->PostTask(FROM_HERE, base::BindLambdaForTesting([&]() {
        scoped_refptr<SharedURLLoaderFactory> third_thread_factory = SharedURLLoaderFactory::Create(std::move(new_pending_factory));
...
TEST_F(CrossThreadPendingSharedURLLoaderFactoryTest, CloneLoaderThread)
{
    ...
    base::RunLoop run_loop;
    base::OnceClosure run_loop_quit = run_loop.QuitClosure();
    loader_thread_->PostTask(FROM_HERE, base::BindLambdaForTesting([&]() {
        scoped_refptr<SharedURLLoaderFactory> load_thread_factory = SharedURLLoaderFactory::Create(std::move(new_pending_factory));
