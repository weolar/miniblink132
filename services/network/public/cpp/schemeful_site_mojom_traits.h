...
template <> struct COMPONENT_EXPORT(NETWORK_CPP_SCHEMEFUL) StructTraits<network::mojom::SchemefulSiteDataView, net::SchemefulSite> {
    static const url::Origin& site_as_origin(const net::SchemefulSite& input)
    {
        return input.site_as_origin_;
    }

    static bool Read(network::mojom::SchemefulSiteDataView data, net::SchemefulSite* out);
};
