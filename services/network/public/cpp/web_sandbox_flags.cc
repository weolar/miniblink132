...
WebSandboxFlags ParseWebSandboxToken(std::string_view token)
{
    ...
    static constexpr auto kTokenToWebSandboxFlags = base::MakeFixedFlatMap<std::string_view, WebSandboxFlags>({
        ...
        { "allow-popups-to-escape-sandbox", kPropagatesToAuxiliaryBrowsingContexts },
        { "allow-presentation", kPresentationController },
        { "allow-same-origin", kOrigin },
        { "allow-scripts", kAutomaticFeatures | kScripts },
        { "allow-storage-access-by-user-activation", kStorageAccessByUserActivation },
        { "allow-top-navigation", kTopNavigation | kTopNavigationToCustomProtocols },
        { "allow-top-navigation-by-user-activation", kTopNavigationByUserActivation },
        { "allow-top-navigation-to-custom-protocols", kTopNavigationToCustomProtocols },
...
WebSandboxFlagsParsingResult ParseWebSandboxPolicy(std::string_view input, WebSandboxFlags ignored_flags)
{
    WebSandboxFlagsParsingResult out;
    out.flags = WebSandboxFlags::kAll;

    std::vector<std::string> error_tokens;
    for (const auto& token : base::SplitStringPiece(input, kHtmlWhitespace, base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY)) {
        WebSandboxFlags flags = ~ParseWebSandboxToken(token);
        flags |= ignored_flags;
        out.flags &= flags;
        if (flags == WebSandboxFlags::kAll)
            error_tokens.push_back(std::string(token));
    }
    ...
    if (!error_tokens.empty()) {
        ...
        out.error_message
            = "'" + base::JoinString(error_tokens, "', '") + (error_tokens.size() > 1 ? "' are invalid sandbox flags." : "' is an invalid sandbox flag.");
    }
