...
void CheckBodyIsDrained(const std::string& body)
{
    ...
    empty_client->Drain(base::BindLambdaForTesting([&](const URLLoaderCompletionStatus& status) {
        result = status;
        loop.Quit();
    }));
    ...
    ASSERT_EQ(mojo::CreateDataPipe(/*options=*/nullptr, producer_handle, consumer_handle), MOJO_RESULT_OK);
