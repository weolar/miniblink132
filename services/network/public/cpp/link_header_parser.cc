...
std::optional<mojom::LinkRelAttribute> ParseRelAttribute(const std::optional<std::string>& attr)
{
    if (!attr.has_value())
        return std::nullopt;
...
// std::nullopt when the value isn't pre-defined.
std::optional<mojom::LinkAsAttribute> ParseAsAttribute(const std::optional<std::string>& attr)
{
    if (!attr.has_value())
        return std::nullopt;
...
// std::nullopt when the value isn't pre-defined.
std::optional<mojom::CrossOriginAttribute> ParseCrossOriginAttribute(const std::optional<std::string>& attr)
{
    if (!attr.has_value())
...
std::optional<mojom::FetchPriorityAttribute> ParseFetchPriorityAttribute(const std::optional<std::string>& attr)
{
    if (!attr.has_value()) {
...
// pre-definied.
bool ParseAttributes(const std::unordered_map<std::string, std::optional<std::string>>& attrs, mojom::LinkHeaderPtr& parsed)
{
    ...
    for (const auto& attr : attrs) {
        ...
        if (name == "rel") {
            ...
            if (is_rel_set)
                continue;
            std::optional<mojom::LinkRelAttribute> rel = ParseRelAttribute(attr.second);
            if (!rel.has_value())
                return false;
            ...
            if (parsed->cross_origin != mojom::CrossOriginAttribute::kUnspecified)
                continue;
            std::optional<mojom::CrossOriginAttribute> cross_origin = ParseCrossOriginAttribute(attr.second);
            ...
            std::optional<mojom::FetchPriorityAttribute> fetch_priority = ParseFetchPriorityAttribute(attr.second);
...
std::vector<mojom::LinkHeaderPtr> ParseLinkHeaders(const net::HttpResponseHeaders& headers, const GURL& base_url)
{
    std::vector<mojom::LinkHeaderPtr> parsed_headers;
    std::string link_header = headers.GetNormalizedHeader("link").value_or(std::string());
    for (const auto& pair : link_header_util::SplitLinkHeader(link_header)) {
        ...
        if (!link_header_util::ParseLinkHeaderValue(pair.first, pair.second, &url, &attrs)) {
            continue;
        }
