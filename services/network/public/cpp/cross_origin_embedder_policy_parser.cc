...
constexpr char kReportOnlyHeaderName[] = "cross-origin-embedder-policy-report-only";

// [spec]: https://html.spec.whatwg.org/C/#obtain-an-embedder-policy
std::pair<mojom::CrossOriginEmbedderPolicyValue, std::optional<std::string>> Parse(std::string_view header_value)
{
...
CrossOriginEmbedderPolicy ParseCrossOriginEmbedderPolicy(const net::HttpResponseHeaders& headers)
{
    CrossOriginEmbedderPolicy coep;
    if (std::optional<std::string> header_value = headers.GetNormalizedHeader(kHeaderName)) {
        std::tie(coep.value, coep.reporting_endpoint) = Parse(*header_value);
    }
    if (std::optional<std::string> header_value = headers.GetNormalizedHeader(kReportOnlyHeaderName)) {
        std::tie(coep.report_only_value, coep.report_only_reporting_endpoint) = Parse(*header_value);
    }
    return coep;
}
