...
TEST(StructuredHeadersMojomTraitsTest, Item_SerializeAndDeserialize)
{
    ...
    for (const auto& expected : kTestCases) {
        Item actual;
        EXPECT_TRUE(mojo::test::SerializeAndDeserialize<mojom::StructuredHeadersItem>(expected, actual));
...
TEST(StructuredHeadersMojomTraitsTest, Parameter_SerializeAndDeserialize)
{
    ...
    Parameter actual;
    EXPECT_TRUE(mojo::test::SerializeAndDeserialize<mojom::StructuredHeadersParameter>(kExpected, actual));
    EXPECT_EQ(kExpected, actual);
}

TEST(StructuredHeadersMojomTraitsTest, ParameterizedItem_SerializeAndDeserialize)
{
    ...
    EXPECT_TRUE(mojo::test::SerializeAndDeserialize<mojom::StructuredHeadersParameterizedItem>(kExpected, actual));
    EXPECT_EQ(kExpected, actual);
}
...
TEST(StructuredHeadersMojomTraitsTest, Dictionary_SerializeAndDeserialize)
{
    const Dictionary kExpected({
        ...
        { "def",
            ParameterizedMember({ ParameterizedItem(Item("abc"), {}), ParameterizedItem(Item("xyz"), { Parameter("y", Item("q")) }) },
                /*member_is_inner_list=*/true, { Parameter("z", Item("r")) }) },
    });

    Dictionary actual;
    EXPECT_TRUE(mojo::test::SerializeAndDeserialize<mojom::StructuredHeadersDictionary>(kExpected, actual));
    EXPECT_EQ(kExpected, actual);
}
