...
class TestNetworkConnectionObserver : public NetworkConnectionTracker::NetworkConnectionObserver {
public:
    explicit TestNetworkConnectionObserver(NetworkConnectionTracker* tracker)
        : num_notifications_(0)
        , tracker_(tracker)
        , expected_connection_type_(network::mojom::ConnectionType::CONNECTION_UNKNOWN)
    ...
    TestNetworkConnectionObserver& operator=(const TestNetworkConnectionObserver&) = delete;
    ...
    network::mojom::ConnectionType GetConnectionTypeSync()
    {
        ...
        bool sync = tracker_->GetConnectionType(&type, base::BindOnce(&TestNetworkConnectionObserver::GetConnectionTypeCallback, &run_loop, &type));
        if (!sync)
            run_loop.Run();
        return type;
    }
    ...
    size_t num_notifications() const
    {
        return num_notifications_;
    }
    void WaitForNotification(network::mojom::ConnectionType expected_connection_type)
    {
...
private:
    static void GetConnectionTypeCallback(base::RunLoop* run_loop, network::mojom::ConnectionType* out, network::mojom::ConnectionType type)
    {
        *out = type;
        run_loop->Quit();
    }
...
class TestLeakyNetworkConnectionObserver : public NetworkConnectionTracker::NetworkConnectionObserver {
public:
    ...
    TestLeakyNetworkConnectionObserver(const TestLeakyNetworkConnectionObserver&) = delete;
    TestLeakyNetworkConnectionObserver& operator=(const TestLeakyNetworkConnectionObserver&) = delete;
...
public:
    ...
    bool GetConnectionType()
    {
        ...
        return tracker_->GetConnectionType(&connection_type_, base::BindOnce(&ConnectionTypeGetter::OnGetConnectionType, base::Unretained(this)));
    }

    void WaitForConnectionType(network::mojom::ConnectionType expected_connection_type)
    {
        DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
        run_loop_.Run();
...
public:
    NetworkConnectionTrackerTest()
        : mock_network_change_notifier_(net::test::MockNetworkChangeNotifier::Create())
    {
    }

    NetworkConnectionTrackerTest(const NetworkConnectionTrackerTest&) = delete;
    NetworkConnectionTrackerTest& operator=(const NetworkConnectionTrackerTest&) = delete;
    ...
    void Initialize()
    {
        ...
        network_service_ = NetworkService::Create(network_service_remote.InitWithNewPipeAndPassReceiver());
        tracker_ = std::make_unique<NetworkConnectionTracker>(base::BindRepeating(&NetworkConnectionTrackerTest::BindReceiver, base::Unretained(this)));
        observer_ = std::make_unique<TestNetworkConnectionObserver>(tracker_.get());
    }

    void BindReceiver(mojo::PendingReceiver<network::mojom::NetworkChangeManager> receiver)
    {
    ...
    void SimulateConnectionTypeChange(net::NetworkChangeNotifier::ConnectionType type)
    {
...
private:
    ...
    std::unique_ptr<net::test::MockNetworkChangeNotifier> mock_network_change_notifier_;
...
{
    Initialize();
    EXPECT_EQ(network::mojom::ConnectionType::CONNECTION_UNKNOWN, network_connection_observer()->connection_type());

    // Simulate a network change.
    SimulateConnectionTypeChange(net::NetworkChangeNotifier::ConnectionType::CONNECTION_3G);

    network_connection_observer()->WaitForNotification(network::mojom::ConnectionType::CONNECTION_3G);
    EXPECT_EQ(network::mojom::ConnectionType::CONNECTION_3G, network_connection_observer()->connection_type());
...
{
    Initialize();
    auto network_connection_observer2 = std::make_unique<TestNetworkConnectionObserver>(network_connection_tracker());

    // Simulate a network change.
    SimulateConnectionTypeChange(net::NetworkChangeNotifier::ConnectionType::CONNECTION_WIFI);

    network_connection_observer2->WaitForNotification(network::mojom::ConnectionType::CONNECTION_WIFI);
    EXPECT_EQ(network::mojom::ConnectionType::CONNECTION_WIFI, network_connection_observer2->connection_type());
    network_connection_observer()->WaitForNotification(network::mojom::ConnectionType::CONNECTION_WIFI);
    EXPECT_EQ(network::mojom::ConnectionType::CONNECTION_WIFI, network_connection_observer()->connection_type());
    base::RunLoop().RunUntilIdle();

    network_connection_observer2.reset();

    // Simulate an another network change.
    SimulateConnectionTypeChange(net::NetworkChangeNotifier::ConnectionType::CONNECTION_2G);
    network_connection_observer()->WaitForNotification(network::mojom::ConnectionType::CONNECTION_2G);
    EXPECT_EQ(network::mojom::ConnectionType::CONNECTION_2G, network_connection_observer()->connection_type());
...
{
    Initialize();
    auto leaky_network_connection_observer = std::make_unique<TestLeakyNetworkConnectionObserver>(network_connection_tracker());

    // Simulate a network change.
    SimulateConnectionTypeChange(net::NetworkChangeNotifier::ConnectionType::CONNECTION_3G);

    leaky_network_connection_observer->WaitForNotification();
    EXPECT_EQ(network::mojom::ConnectionType::CONNECTION_3G, leaky_network_connection_observer->connection_type());
    base::RunLoop().RunUntilIdle();
...
TEST_F(NetworkConnectionTrackerTest, GetConnectionType)
{
    ...
    getter1.WaitForConnectionType(
        /*expected_connection_type=*/network::mojom::ConnectionType::CONNECTION_3G);
    ...
    EXPECT_EQ(network::mojom::ConnectionType::CONNECTION_3G, getter3.connection_type());
}
...
TEST_F(NetworkConnectionTrackerTest, GetConnectionTypeUnavailable)
{
    ...
    mojo::Remote<network::mojom::NetworkService>* network_service_remote = new mojo::Remote<network::mojom::NetworkService>;
    ...
    NetworkConnectionTracker::BindingCallback callback = base::BindRepeating(
        [](network::mojom::NetworkService* service, mojo::PendingReceiver<network::mojom::NetworkChangeManager> receiver) {
...
public:
    ...
    void GetConnectionType()
    {
        ...
        getter_ = std::make_unique<ConnectionTypeGetter>(network_connection_tracker());
...
TEST_F(NetworkGetConnectionTest, GetConnectionTypeOnDifferentThread)
{
    ...
    base::RunLoop().RunUntilIdle();
    base::subtle::NoBarrier_Store(&network_connection_tracker()->connection_type_, -1);
    {
        base::RunLoop run_loop;
        getter_thread()->task_runner()->PostTaskAndReply(FROM_HERE, base::BindOnce(&NetworkGetConnectionTest::GetConnectionType, base::Unretained(this)),
            base::BindOnce([](base::RunLoop* run_loop) { run_loop->Quit(); }, base::Unretained(&run_loop)));
        run_loop.Run();
    }

    network_connection_tracker()->OnInitialConnectionType(network::mojom::ConnectionType::CONNECTION_3G);
    {
        base::RunLoop run_loop;
        getter_thread()->task_runner()->PostTaskAndReply(FROM_HERE,
            base::BindOnce(&NetworkGetConnectionTest::WaitForConnectionType, base::Unretained(this),
