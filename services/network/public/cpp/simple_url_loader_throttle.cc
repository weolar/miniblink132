...
class BatchingDelegate : public SimpleURLLoaderThrottle::Delegate, public net::NetworkChangeNotifier::DefaultNetworkActiveObserver {
public:
    explicit BatchingDelegate(SimpleURLLoaderThrottle* owner)
        : owner_(owner)
    {
    }
    ...
    bool ShouldThrottle() override
    {
        ...
        if (base::PowerMonitor::IsInitialized() && !base::PowerMonitor::IsOnBatteryPower()) {
            return false;
        }

#if BUILDFLAG(IS_ANDROID)
        if (base::android::RadioUtils::GetConnectionType() != base::android::RadioConnectionType::kCell) {
            return false;
        }
#endif // BUILDFLAGS(IS_ANDROID)
...
public:
    ...
    bool IsBatchingEnabledForTrafficAnnotation(const net::NetworkTrafficAnnotationTag& traffic_annotation)
    {
        if (!initialized_)
            Initialize();

        return enabled_traffic_annotation_hashes_.contains(traffic_annotation.unique_id_hash_code);
    }
...
private:
    void Initialize()
    {
        ...
        std::string comma_separated_hashes
            = base::GetFieldTrialParamValueByFeature(features::kBatchSimpleURLLoader, kBatchSimpleURLLoaderEnabledTrafficAnnotationHashesParam);
        const std::vector<std::string> values = base::SplitString(comma_separated_hashes, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
// static
bool SimpleURLLoaderThrottle::IsBatchingEnabled(const net::NetworkTrafficAnnotationTag& traffic_annotation)
{
    if (!base::FeatureList::IsEnabled(features::kBatchSimpleURLLoader))
        return false;

    if (!GetBatchingConfig().IsBatchingEnabledForTrafficAnnotation(traffic_annotation)) {
        return false;
    }
...
void SimpleURLLoaderThrottle::NotifyWhenReady(base::OnceClosure callback)
{
    ...
    base::UmaHistogramBoolean("Network.Radio.SimpleURLLoaderIsThrottled", should_throttle);
    ...
    timeout_timer_.Start(FROM_HERE, timeout_, base::BindOnce(&SimpleURLLoaderThrottle::OnTimeout, base::Unretained(this)));
}
...
void SimpleURLLoaderThrottle::OnReadyToStart()
{
    ...
    base::TimeDelta throttled_time = base::TimeTicks::Now() - throttling_start_time_;
    base::UmaHistogramLongTimes("Network.Radio.SimpleURLLoaderThrottledTime", throttled_time);
...
void SimpleURLLoaderThrottle::SetDelegateForTesting(std::unique_ptr<Delegate> delegate)
{
