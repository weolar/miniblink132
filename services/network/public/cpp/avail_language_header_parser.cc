...
std::optional<std::vector<std::string>> ParseAvailLanguage(const std::string& header)
{
    ...
    std::optional<net::structured_headers::List> maybe_list = net::structured_headers::ParseList(base::ToLowerASCII(header));
    ...
    for (const auto& list_item : maybe_list.value()) {
        ...
        if (list_item.params.size() == 1 && list_item.params[0].first == "d" && list_item.params[0].second.is_boolean()
    ...
    result.insert(result.end(), non_default_languages.begin(), non_default_languages.end());
