...
using ClientHintToNameMap = base::flat_map<network::mojom::WebClientHintsType, std::string>;
...
std::optional<std::vector<network::mojom::WebClientHintsType>> COMPONENT_EXPORT(NETWORK_CPP) ParseClientHintsHeader(const std::string& header);

struct COMPONENT_EXPORT(NETWORK_CPP) ClientHintToDelegatedThirdPartiesHeader {
    ClientHintToDelegatedThirdPartiesHeader();
    ~ClientHintToDelegatedThirdPartiesHeader();
    ClientHintToDelegatedThirdPartiesHeader(const ClientHintToDelegatedThirdPartiesHeader&);

    base::flat_map<network::mojom::WebClientHintsType, std::vector<url::Origin>> map;
...
const ClientHintToDelegatedThirdPartiesHeader COMPONENT_EXPORT(NETWORK_CPP)
    ParseClientHintToDelegatedThirdPartiesHeader(const std::string& header, MetaCHType type);
...
void COMPONENT_EXPORT(NETWORK_CPP) LogClientHintsPersistenceMetrics(const base::TimeTicks& persistence_started, std::size_t hints_stored);
