...
public:
    ...
    [[nodiscard]] std::tuple<mojo::PendingReceiver<mojom::URLLoaderFactory>, mojo::PendingRemote<mojom::URLLoaderFactory>> Append();
    ...
    template <typename OutType = scoped_refptr<SharedURLLoaderFactory>, typename... Args> [[nodiscard]] OutType Finish(Args... terminal_args) &&
    {
        if (IsEmpty()) {
    ...
    template <typename OutType = void, typename... Args> void Finish(mojo::PendingReceiver<mojom::URLLoaderFactory> receiver, Args... terminal_args) &&
    {
        if (IsEmpty()) {
            // The builder has no interceptors, so connect `receiver` directly to the
            // terminal.
            ConnectTerminal(std::move(receiver), std::forward<Args>(terminal_args)...);
            return;
        }
...
private:
    ...
    template <typename OutType, typename InType> static OutType WrapAs(InType in)
    {
        return std::move(in);
    }

    template <typename OutType> static OutType WrapAs(mojom::NetworkContext* context, mojom::URLLoaderFactoryParamsPtr factory_param)
    {
        mojo::PendingRemote<mojom::URLLoaderFactory> remote;
        context->CreateURLLoaderFactory(remote.InitWithNewPipeAndPassReceiver(), std::move(factory_param));
    ...
    static void ConnectTerminal(mojo::PendingReceiver<mojom::URLLoaderFactory> pending_receiver, mojo::Remote<mojom::URLLoaderFactory> terminal_factory);
    static void ConnectTerminal(mojo::PendingReceiver<mojom::URLLoaderFactory> pending_receiver, mojo::PendingRemote<mojom::URLLoaderFactory> terminal_factory);
    static void ConnectTerminal(mojo::PendingReceiver<mojom::URLLoaderFactory> pending_receiver, scoped_refptr<SharedURLLoaderFactory> terminal_factory);
    static void ConnectTerminal(mojo::PendingReceiver<mojom::URLLoaderFactory> pending_receiver, mojom::NetworkContext* terminal_context,
...
scoped_refptr<SharedURLLoaderFactory> URLLoaderFactoryBuilder::WrapAs(mojo::PendingRemote<mojom::URLLoaderFactory> in);

// `SharedURLLoaderFactory` -> `PendingRemote`:
// Creates a new pipe and cloning.
template <>
COMPONENT_EXPORT(NETWORK_CPP)
mojo::PendingRemote<mojom::URLLoaderFactory> URLLoaderFactoryBuilder::WrapAs(scoped_refptr<SharedURLLoaderFactory> in);

// Similar to PendingSharedURLLoaderFactory, but also goes through
// `URLLoaderFactoryBuilder` when constructing the factory.
class COMPONENT_EXPORT(NETWORK_CPP) PendingSharedURLLoaderFactoryWithBuilder final : public PendingSharedURLLoaderFactory {
public:
    PendingSharedURLLoaderFactoryWithBuilder(URLLoaderFactoryBuilder factory_builder, std::unique_ptr<PendingSharedURLLoaderFactory> terminal_pending_factory);
