...
public:
    ...
    explicit DataElementDataPipe(mojo::PendingRemote<mojom::DataPipeGetter> data_pipe_getter);
...
public:
    ...
    DataElementChunkedDataPipe(mojo::PendingRemote<mojom::ChunkedDataPipeGetter> data_pipe_getter, ReadOnlyOnce read_only_once);
    ...
    const mojo::PendingRemote<mojom::ChunkedDataPipeGetter>& chunked_data_pipe_getter() const
    {
        return chunked_data_pipe_getter_;
    }
    mojo::PendingRemote<mojom::ChunkedDataPipeGetter> ReleaseChunkedDataPipeGetter();

    ReadOnlyOnce read_only_once() const
    {
        return read_only_once_;
    }
...
public:
    ...
    DataElementFile(const base::FilePath& path, uint64_t offset, uint64_t length, base::Time expected_modification_time);
...
private:
    using Variant = absl::variant<absl::monostate, DataElementBytes, DataElementDataPipe, DataElementChunkedDataPipe, DataElementFile>;
