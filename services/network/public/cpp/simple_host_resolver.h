...
public:
    using NetworkContextFactory = base::RepeatingCallback<mojom::NetworkContext*()>;

    using ResolveHostCallback = base::OnceCallback<void(int result, const net::ResolveErrorInfo& resolve_error_info,
        const std::optional<net::AddressList>& resolved_addresses, const std::optional<net::HostResolverEndpointResults>& endpoint_results_with_metadata)>;
    ...
    static std::unique_ptr<SimpleHostResolver> Create(NetworkContextFactory network_context_factory);

    // Creates a SimpleHostResolver from the given |network_context|.
    // |network_context| must outlive SimpleHostResolver.
    static std::unique_ptr<SimpleHostResolver> Create(mojom::NetworkContext* network_context);
    ...
    virtual void ResolveHost(mojom::HostResolverHostPtr host, const net::NetworkAnonymizationKey& network_anonymization_key,
        mojom::ResolveHostParametersPtr optional_parameters, ResolveHostCallback callback)
        = 0;
