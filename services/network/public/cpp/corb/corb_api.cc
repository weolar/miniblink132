...
void RemoveAllHttpResponseHeaders(const scoped_refptr<net::HttpResponseHeaders>& headers)
{
    DCHECK(headers);
...
public:
    explicit ComparingAnalyzer(PerFactoryState& state)
        : corb_analyzer_(std::make_unique<CrossOriginReadBlocking::CorbResponseAnalyzer>())
        , orb_analyzer_(std::make_unique<OpaqueResponseBlockingAnalyzer>(state))
        , is_orb_enabled_(base::FeatureList::IsEnabled(features::kOpaqueResponseBlockingV01))
    {
    }
    ...
    ~ComparingAnalyzer() override
    {
        ...
        if (corb_decision_ != Decision::kSniffMore && orb_decision_ != Decision::kSniffMore) {
    ...
    {
        corb_decision_ = corb_analyzer_->Init(request_url, request_initiator, request_mode, response);
        orb_decision_ = orb_analyzer_->Init(request_url, request_initiator, request_mode, response);
...
private:
    ...
    const std::unique_ptr<CrossOriginReadBlocking::CorbResponseAnalyzer> corb_analyzer_;
...
// static
std::unique_ptr<ResponseAnalyzer> ResponseAnalyzer::Create(PerFactoryState& state)
{
