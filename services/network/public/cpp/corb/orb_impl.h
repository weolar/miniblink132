...
class COMPONENT_EXPORT(NETWORK_CPP) OpaqueResponseBlockingAnalyzer final : public ResponseAnalyzer {
public:
    explicit OpaqueResponseBlockingAnalyzer(PerFactoryState& state);

    OpaqueResponseBlockingAnalyzer(const OpaqueResponseBlockingAnalyzer&) = delete;
    OpaqueResponseBlockingAnalyzer& operator=(const OpaqueResponseBlockingAnalyzer&) = delete;
...
private:
    ...
    BlockingDecisionReason blocking_decision_reason_ = BlockingDecisionReason::kInvalid;
};
