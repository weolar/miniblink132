...
bool IsAudioOrVideoMimeType(base::StringPiece mime_type)
{
    ...
    if (base::EqualsCaseInsensitiveASCII(mime_type, "application/dash+xml"))
        return true;
    if (base::EqualsCaseInsensitiveASCII(mime_type, "application/vnd.apple.mpegurl"))
        return true;
...
bool IsHttpStatus(const mojom::URLResponseHead& response, int expected_status_code)
{
    if (!response.headers)
        return false;
...
bool IsRangeResponseWithMiddleOfResource(const mojom::URLResponseHead& response)
{
    if (!response.headers)
        return false;
    ...
    if (!net::HttpUtil::ParseContentRangeHeaderFor206(range, &first_byte_position, &last_byte_position, &instance_length)) {
        return false;
    }
...
bool IsOpaqueResponse(const absl::optional<url::Origin>& request_initiator, mojom::RequestMode request_mode, const mojom::URLResponseHead& response)
{
...
OpaqueResponseBlockingAnalyzer::OpaqueResponseBlockingAnalyzer(PerFactoryState& state)
...
{
    base::UmaHistogramEnumeration("SiteIsolation.ORB.BlockingReason", blocking_decision_reason_);

    // TODO(https://crbug.com/1178928): Add UMA tracking the size of ORB state
    // from `per_factory_state_`.
}

Decision OpaqueResponseBlockingAnalyzer::Init(const GURL& request_url, const absl::optional<url::Origin>& request_initiator, mojom::RequestMode request_mode,
    const network::mojom::URLResponseHead& response)
{
    ...
    //    list.
    is_no_sniff_header_present_ = CrossOriginReadBlocking::CorbResponseAnalyzer::HasNoSniff(response);
    ...
    if (!mime_type_.empty()) {
        ...
        case CrossOriginReadBlocking::MimeType::kNeverSniffed:
            blocking_decision_reason_ = BlockingDecisionReason::kNeverSniffedMimeType;
    ...
    if (IsRangeResponseWithMiddleOfResource(response)) {
        if (IsAllowedAudioVideoRequest(request_url)) {
            ...
            blocking_decision_reason_ = BlockingDecisionReason::kUnexpectedRangeResponse;
...
Decision OpaqueResponseBlockingAnalyzer::Sniff(base::StringPiece data)
{
    ...
    if (!IsTextCssMimeType(mime_type_)) {
        if (CrossOriginReadBlocking::SniffForHTML(data) == CrossOriginReadBlocking::SniffingResult::kYes) {
        ...
        bool is_multimedia_mime_type = IsNonSniffableImageMimeType(mime_type_) || IsAudioOrVideoMimeType(mime_type_);
        bool is_xml_based_multimedia_mime_type = is_multimedia_mime_type && base::EndsWith(mime_type_, "+xml", base::CompareCase::INSENSITIVE_ASCII);
        if (!is_xml_based_multimedia_mime_type && CrossOriginReadBlocking::SniffForXML(data) == CrossOriginReadBlocking::SniffingResult::kYes) {
            blocking_decision_reason_ = BlockingDecisionReason::kSniffedAsXml;
            return Decision::kBlock;
        }

        // Check for JSON and JS parser breakers.
        if (CrossOriginReadBlocking::SniffForFetchOnlyResource(data) == CrossOriginReadBlocking::SniffingResult::kYes) {
...
ResponseAnalyzer::BlockedResponseHandling OpaqueResponseBlockingAnalyzer::ShouldHandleBlockedResponseAs() const
{
    ...
    // error.
    return base::FeatureList::IsEnabled(features::kOpaqueResponseBlockingV02) ? BlockedResponseHandling::kNetworkError
...
void OpaqueResponseBlockingAnalyzer::ReportOrbBlockedAndCorbDidnt() const
{
    ...
    base::UmaHistogramEnumeration("SiteIsolation.ORB.CorbVsOrb.OrbBlockedAndCorbDidnt.Reason", blocking_decision_reason_);
}

void OpaqueResponseBlockingAnalyzer::StoreAllowedAudioVideoRequest(const GURL& media_url)
{
    per_factory_state_->insert(media_url);
}

bool OpaqueResponseBlockingAnalyzer::IsAllowedAudioVideoRequest(const GURL& media_url)
{
