...
TEST(TimingAllowOriginParserTest, InvalidOriginWithEmbeddedWhitespace)
{
    ...
    EXPECT_THAT(tao->get_serialized_origins(), ElementsAre("https://exa mple.com"));
}
...
TEST(TimingAllowedOriginParserTest, SingleOrigin)
{
    ...
    ASSERT_EQ(mojom::TimingAllowOrigin::Tag::kSerializedOrigins, tao->which());
    EXPECT_THAT(tao->get_serialized_origins(), ElementsAre("https://example.com"));
}
...
{
    auto tao = ParseTimingAllowOrigin("https://a.example.com,https://b.example.com");
    ASSERT_EQ(mojom::TimingAllowOrigin::Tag::kSerializedOrigins, tao->which());
    EXPECT_THAT(tao->get_serialized_origins(), ElementsAre("https://a.example.com", "https://b.example.com"));
}
...
{
    EXPECT_EQ(mojom::TimingAllowOrigin::Tag::kAll, ParseTimingAllowOrigin("*")->which());
}
...
{
    EXPECT_EQ(mojom::TimingAllowOrigin::Tag::kAll, ParseTimingAllowOrigin("*,https://example.com")->which());

    EXPECT_EQ(mojom::TimingAllowOrigin::Tag::kAll, ParseTimingAllowOrigin("https://example.com,*")->which());
}

TEST(TimingAllowOriginParserTest, WildcardWithOtherOriginsAndWhitespacePadding)
{
    static constexpr const char* kItems1[] = {
        "*",
        " *",
        ...
        for (const char* kItem2 : kItems2) {
            {
                std::string test_case = base::StrCat({ kItem1, ",", kItem2 });
                SCOPED_TRACE(test_case);
                EXPECT_EQ(mojom::TimingAllowOrigin::Tag::kAll, ParseTimingAllowOrigin(test_case)->which());
            }
...
{
    EXPECT_EQ(mojom::TimingAllowOrigin::Tag::kAll, ParseTimingAllowOrigin("*,_")->which());
...
{
    return TimingAllowOriginCheck(ParseTimingAllowOrigin(tao), url::Origin::Create(GURL(url)));
}

TEST(TimingAllowOriginParserTest, TAOCheck)
{
    EXPECT_TRUE(TAOCheck("http://example1.com", "http://example1.com"));
    EXPECT_TRUE(TAOCheck("http://example1.com,example2.com", "http://example1.com"));
    EXPECT_TRUE(TAOCheck("http://example1.com,  http://example2.com", "http://example2.com"));
    EXPECT_TRUE(TAOCheck("http://example1.com,  http://example2.com", "http://example1.com"));
