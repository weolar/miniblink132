...
class COMPONENT_EXPORT(NETWORK_CPP) NetToMojoPendingBuffer : public base::RefCountedThreadSafe<NetToMojoPendingBuffer> {
public:
    ...
    static MojoResult BeginWrite(mojo::ScopedDataPipeProducerHandle* handle, scoped_refptr<NetToMojoPendingBuffer>* pending);
...
private:
    ...
    NetToMojoPendingBuffer(mojo::ScopedDataPipeProducerHandle handle, base::span<char> buffer);
...
class COMPONENT_EXPORT(NETWORK_CPP) NetToMojoIOBuffer : public net::WrappedIOBuffer {
public:
    // If |offset| is specified then the memory buffer passed to the Net layer
    // will be offset by that many bytes.
    explicit NetToMojoIOBuffer(scoped_refptr<NetToMojoPendingBuffer> pending_buffer, int offset = 0);
...
class COMPONENT_EXPORT(NETWORK_CPP) MojoToNetPendingBuffer : public base::RefCountedThreadSafe<MojoToNetPendingBuffer> {
public:
    ...
    // On failure or MOJO_RESULT_SHOULD_WAIT, there will be no change to the
    // handle, and *pending will be nulled out.
    static MojoResult BeginRead(mojo::ScopedDataPipeConsumerHandle* handle, scoped_refptr<MojoToNetPendingBuffer>* pending);
...
private:
    ...
    // Takes ownership of the handle.
    MojoToNetPendingBuffer(mojo::ScopedDataPipeConsumerHandle handle, base::span<const char> buffer);
...
class COMPONENT_EXPORT(NETWORK_CPP) MojoToNetIOBuffer : public net::WrappedIOBuffer {
public:
    // |bytes_to_be_read| contains the number of bytes expected to be read by
    // the consumer.
    MojoToNetIOBuffer(scoped_refptr<MojoToNetPendingBuffer> pending_buffer, int bytes_to_be_read);
