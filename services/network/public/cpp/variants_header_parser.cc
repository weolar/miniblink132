...
{
    static const base::NoDestructor<SupportedVariantsNameSet> set(MakeSupportedVariantsNameSet());
    return *set;
}

absl::optional<std::vector<mojom::VariantsHeaderPtr>> ParseVariantsHeaders(const std::string& header)
{
    ...
    absl::optional<net::structured_headers::Dictionary> maybe_dictionary = net::structured_headers::ParseDictionary(base::ToLowerASCII(header));
    ...
    for (const auto& dictionary_pair : maybe_dictionary.value()) {
        ...
        auto parsed = mojom::VariantsHeader::New(dictionary_pair.first, std::move(parsed_values));
