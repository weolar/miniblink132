...
using NetworkConnectionTrackerGetter = base::RepeatingCallback<NetworkConnectionTracker*()>;
// Defines the type of a callback that can be used to asynchronously get a
// NetworkConnectionTracker instance.
using NetworkConnectionTrackerAsyncGetter = base::RepeatingCallback<void(base::OnceCallback<void(NetworkConnectionTracker*)>)>;
...
class COMPONENT_EXPORT(NETWORK_CPP) NetworkConnectionTracker : public network::mojom::NetworkChangeManagerClient {
public:
    using BindingCallback = base::RepeatingCallback<void(mojo::PendingReceiver<network::mojom::NetworkChangeManager>)>;
    using ConnectionTypeCallback = base::OnceCallback<void(network::mojom::ConnectionType)>;
    ...
    virtual bool GetConnectionType(network::mojom::ConnectionType* type, ConnectionTypeCallback callback);
...
private:
    FRIEND_TEST_ALL_PREFIXES(NetworkGetConnectionTest, GetConnectionTypeOnDifferentThread);
    ...
    const scoped_refptr<base::ObserverListThreadSafe<NetworkConnectionObserver>> network_change_observer_list_;
    const scoped_refptr<base::ObserverListThreadSafe<NetworkConnectionObserver>> leaky_network_change_observer_list_;
...
template <> struct ScopedObservationTraits<network::NetworkConnectionTracker, network::NetworkConnectionTracker::NetworkConnectionObserver> {
    static void AddObserver(network::NetworkConnectionTracker* source, network::NetworkConnectionTracker::NetworkConnectionObserver* observer)
    {
        source->AddNetworkConnectionObserver(observer);
    }
    static void RemoveObserver(network::NetworkConnectionTracker* source, network::NetworkConnectionTracker::NetworkConnectionObserver* observer)
    {
