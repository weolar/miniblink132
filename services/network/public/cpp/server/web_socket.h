...
public:
    ...
    void Accept(const HttpServerRequestInfo& request, const net::NetworkTrafficAnnotationTag traffic_annotation);
    ParseResult Read(std::string* message);
    void Send(base::StringPiece message, net::WebSocketFrameHeader::OpCodeEnum op_code, const net::NetworkTrafficAnnotationTag traffic_annotation);
...
private:
    void Fail();
    void SendErrorResponse(const std::string& message, const net::NetworkTrafficAnnotationTag traffic_annotation);

    const raw_ptr<HttpServer> server_;
    const raw_ptr<HttpConnection> connection_;
    std::unique_ptr<WebSocketEncoder> encoder_;
    bool closed_;
    std::unique_ptr<net::NetworkTrafficAnnotationTag> traffic_annotation_ = nullptr;
};
