...
public:
    ...
    static std::unique_ptr<WebSocketEncoder> CreateServer(const std::string& extensions, net::WebSocketDeflateParameters* params);
    static std::unique_ptr<WebSocketEncoder> CreateClient(const std::string& response_extensions);

    WebSocket::ParseResult DecodeFrame(const base::StringPiece& frame, int* bytes_consumed, std::string* output);
    void EncodeTextFrame(base::StringPiece frame, int masking_key, std::string* output);
    void EncodePongFrame(base::StringPiece frame, int masking_key, std::string* output);
...
private:
    ...
    WebSocketEncoder(Type type, std::unique_ptr<net::WebSocketDeflater> deflater, std::unique_ptr<net::WebSocketInflater> inflater);
