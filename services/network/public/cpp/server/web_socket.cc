...
std::string ExtensionsHeaderString(const std::vector<net::WebSocketExtension>& extensions)
{
    if (extensions.empty())
        return std::string();
...
std::string ValidResponseString(const std::string& accept_hash, const std::vector<net::WebSocketExtension> extensions)
{
    return base::StringPrintf("HTTP/1.1 101 WebSocket Protocol Handshake\r\n"
                              "Upgrade: WebSocket\r\n"
                              "Connection: Upgrade\r\n"
...
void WebSocket::Accept(const HttpServerRequestInfo& request, const net::NetworkTrafficAnnotationTag traffic_annotation)
{
    std::string version = request.GetHeaderValue("sec-websocket-version");
    if (version != "8" && version != "13") {
        SendErrorResponse("Invalid request format. The version is not valid.", traffic_annotation);
        return;
    }

    std::string key = request.GetHeaderValue("sec-websocket-key");
    if (key.empty()) {
        SendErrorResponse("Invalid request format. Sec-WebSocket-Key is empty or isn't "
                          "specified.",
            traffic_annotation);
        return;
    }
    std::string encoded_hash;
    base::Base64Encode(base::SHA1HashString(key + net::websockets::kWebSocketGuid), &encoded_hash);
    ...
    server_->SendRaw(connection_->id(), ValidResponseString(encoded_hash, response_extensions), traffic_annotation);
    traffic_annotation_ = std::make_unique<net::NetworkTrafficAnnotationTag>(net::NetworkTrafficAnnotationTag(traffic_annotation));
}
...
WebSocket::ParseResult WebSocket::Read(std::string* message)
{
    ...
    const ParseResult result = encoder_->DecodeFrame(frame, &bytes_consumed, message);
    ...
    if (result == FRAME_PING) {
        ...
        Send(*message, net::WebSocketFrameHeader::kOpCodePong, *traffic_annotation_);
    }
    return result;
}

void WebSocket::Send(base::StringPiece message, net::WebSocketFrameHeader::OpCodeEnum op_code, const net::NetworkTrafficAnnotationTag traffic_annotation)
{
    if (closed_)
        return;
    std::string encoded;
...
void WebSocket::SendErrorResponse(const std::string& message, const net::NetworkTrafficAnnotationTag traffic_annotation)
{
    if (closed_)
        return;
    closed_ = true;
