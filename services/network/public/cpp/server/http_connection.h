...
public:
    HttpConnection(int id, mojo::PendingRemote<mojom::TCPConnectedSocket> socket, mojo::ScopedDataPipeConsumerHandle socket_receive_handle,
        mojo::ScopedDataPipeProducerHandle socket_send_handle, const net::IPEndPoint& peer_addr);
    ...
    mojom::TCPConnectedSocket* socket() const
    {
        return socket_.get();
    }
    ...
    mojo::SimpleWatcher& read_watcher()
    {
        return receive_pipe_watcher_;
    }
    ...
    {
        return send_pipe_watcher_;
    }
    const net::IPEndPoint& GetPeerAddress()
    {
        return peer_addr_;
    }
    std::string& read_buf()
    ...
    {
        max_read_buf_size_ = buf_size;
    }
    void SetWriteBufferSize(size_t buf_size)
    {
        max_write_buf_size_ = buf_size;
    }
    size_t ReadBufferSize() const
    {
        return max_read_buf_size_;
    }
    size_t WriteBufferSize() const
    {
        return max_write_buf_size_;
    }
