...
public:
    ...
    public:
        ...
        virtual void OnHttpRequest(int connection_id, const HttpServerRequestInfo& info) = 0;
        virtual void OnWebSocketRequest(int connection_id, const HttpServerRequestInfo& info) = 0;
    ...
    // callbacks yet.
    HttpServer(mojo::PendingRemote<mojom::TCPServerSocket> server_socket, HttpServer::Delegate* delegate);
    ...
    void AcceptWebSocket(int connection_id, const HttpServerRequestInfo& request, net::NetworkTrafficAnnotationTag traffic_annotation);
    void SendOverWebSocket(int connection_id, base::StringPiece data, net::NetworkTrafficAnnotationTag traffic_annotation);
    ...
    void SendRaw(int connection_id, const std::string& data, net::NetworkTrafficAnnotationTag traffic_annotation);
    // TODO(byungchul): Consider replacing function name with SendResponseInfo
    void SendResponse(int connection_id, const HttpServerResponseInfo& response, net::NetworkTrafficAnnotationTag traffic_annotation);
    void Send(int connection_id, net::HttpStatusCode status_code, const std::string& data, const std::string& mime_type,
        net::NetworkTrafficAnnotationTag traffic_annotation);
    void Send200(int connection_id, const std::string& data, const std::string& mime_type, net::NetworkTrafficAnnotationTag traffic_annotation);
    void Send404(int connection_id, net::NetworkTrafficAnnotationTag traffic_annotation);
    void Send500(int connection_id, const std::string& message, net::NetworkTrafficAnnotationTag traffic_annotation);
...
private:
    ...
    void DoAcceptLoop();
    void OnAcceptCompleted(int rv, const absl::optional<net::IPEndPoint>& remote_addr, mojo::PendingRemote<mojom::TCPConnectedSocket> connected_socket,
        mojo::ScopedDataPipeConsumerHandle receive_pipe_handle, mojo::ScopedDataPipeProducerHandle send_pipe_handle);

    void OnReadable(int connection_id, MojoResult result, const mojo::HandleSignalsState& state);
    ...
    void OnWriteCompleted(int connection_id, net::NetworkTrafficAnnotationTag traffic_annotation, int rv);
    ...
    bool ParseHeaders(const char* data, size_t data_len, HttpServerRequestInfo* info, size_t* pos);
