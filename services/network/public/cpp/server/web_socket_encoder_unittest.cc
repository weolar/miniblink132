...
TEST_F(WebSocketEncoderTest, ClientToServer)
{
    ...
    EXPECT_EQ(WebSocket::FRAME_OK_FINAL, server_->DecodeFrame(encoded, &bytes_consumed, &decoded));
    ...
    EXPECT_EQ(WebSocket::FRAME_INCOMPLETE, server_->DecodeFrame(partial, &bytes_consumed, &decoded));

    std::string extra = encoded + "more stuff";
    EXPECT_EQ(WebSocket::FRAME_OK_FINAL, server_->DecodeFrame(extra, &bytes_consumed, &decoded));
    EXPECT_EQ("ClientToServer", decoded);
    EXPECT_EQ((int)encoded.length(), bytes_consumed);

    EXPECT_EQ(WebSocket::FRAME_ERROR, server_->DecodeFrame(std::string("abcde"), &bytes_consumed, &decoded));
}
...
TEST_F(WebSocketEncoderTest, ServerToClient)
{
    ...
    EXPECT_EQ(WebSocket::FRAME_OK_FINAL, client_->DecodeFrame(encoded, &bytes_consumed, &decoded));
    EXPECT_EQ("ServerToClient", decoded);
    EXPECT_EQ((int)encoded.length(), bytes_consumed);

    std::string partial = encoded.substr(0, encoded.length() - 2);
    EXPECT_EQ(WebSocket::FRAME_INCOMPLETE, client_->DecodeFrame(partial, &bytes_consumed, &decoded));

    std::string extra = encoded + "more stuff";
    EXPECT_EQ(WebSocket::FRAME_OK_FINAL, client_->DecodeFrame(extra, &bytes_consumed, &decoded));
    EXPECT_EQ("ServerToClient", decoded);
    EXPECT_EQ((int)encoded.length(), bytes_consumed);

    EXPECT_EQ(WebSocket::FRAME_ERROR, client_->DecodeFrame(std::string("abcde"), &bytes_consumed, &decoded));
}
...
TEST_F(WebSocketEncoderTest, DecodeFragmentedMessageClientToServerDivided2)
{
    ...
    EXPECT_EQ(WebSocket::FRAME_OK_MIDDLE, server_->DecodeFrame(kEncodedFirstFrame, &bytes_consumed, &decoded));
    EXPECT_EQ("", decoded);
    EXPECT_EQ(static_cast<int>(kEncodedFirstFrame.length()), bytes_consumed);
    EXPECT_EQ(WebSocket::FRAME_OK_FINAL, server_->DecodeFrame(kEncodedLastFrame, &bytes_consumed, &decoded));
...
TEST_F(WebSocketEncoderTest, DecodeFragmentedMessageClientToServerDivided3)
{
    ...
    EXPECT_EQ("", decoded);
    EXPECT_EQ(static_cast<int>(kEncodedFirstFrame.length()), bytes_consumed);
    EXPECT_EQ(WebSocket::FRAME_OK_MIDDLE, server_->DecodeFrame(kEncodedSecondFrame, &bytes_consumed, &decoded));
    EXPECT_EQ("", decoded);
...
TEST_F(WebSocketEncoderTest, DecodeFragmentedMessageServerToClientDivided2)
{
    ...
    // kEncodedFirstFrame -> kEncodedLastFrame
    EXPECT_EQ(WebSocket::FRAME_OK_MIDDLE, client_->DecodeFrame(kEncodedFirstFrame, &bytes_consumed, &decoded));
    EXPECT_EQ("", decoded);
    EXPECT_EQ(static_cast<int>(kEncodedFirstFrame.length()), bytes_consumed);
    EXPECT_EQ(WebSocket::FRAME_OK_FINAL, client_->DecodeFrame(kEncodedLastFrame, &bytes_consumed, &decoded));
    EXPECT_EQ("abcdefghijklmnop", decoded);
    EXPECT_EQ(static_cast<int>(kEncodedLastFrame.length()), bytes_consumed);
}
...
TEST_F(WebSocketEncoderTest, DecodeFragmentedMessageServerToClientDivided3)
{
    ...
    EXPECT_EQ("", decoded);
    EXPECT_EQ(static_cast<int>(kEncodedFirstFrame.length()), bytes_consumed);
    EXPECT_EQ(WebSocket::FRAME_OK_MIDDLE, client_->DecodeFrame(kEncodedSecondFrame, &bytes_consumed, &decoded));
    EXPECT_EQ("", decoded);
    EXPECT_EQ(static_cast<int>(kEncodedSecondFrame.length()), bytes_consumed);
    EXPECT_EQ(WebSocket::FRAME_OK_FINAL, client_->DecodeFrame(kEncodedLastFrame, &bytes_consumed, &decoded));
    EXPECT_EQ("abcdefghijklmnop", decoded);
