...
public:
    TestHttpClient()
        : url_request_context_(net::CreateTestURLRequestContextBuilder()->Build())
    ...
    int ConnectAndWait(const net::IPEndPoint& address)
    {
        ...
        factory_.CreateTCPConnectedSocket(absl::nullopt /* local address */, addresses, nullptr /* tcp_connected_socket_options */,
            TRAFFIC_ANNOTATION_FOR_TESTS, socket_.BindNewPipeAndPassReceiver(), mojo::NullRemote() /* observer */,
            base::BindOnce(
                [](base::RunLoop* run_loop, int* result_out, mojo::ScopedDataPipeConsumerHandle* receive_pipe_handle_out,
                    mojo::ScopedDataPipeProducerHandle* send_pipe_handle_out, int result, const absl::optional<net::IPEndPoint>& local_addr,
                    const absl::optional<net::IPEndPoint>& peer_addr, mojo::ScopedDataPipeConsumerHandle receive_pipe_handle,
                ...
                base::Unretained(&run_loop), base::Unretained(&net_error), &receive_pipe_handle_, &send_pipe_handle_));
        run_loop.Run();
        return net_error;
    }
    ...
    void Send(const std::string& message)
    {
        ...
        while (write_size > 0) {
            ...
            MojoResult result = send_pipe_handle_->WriteData(message.data() + index, &write_size, MOJO_WRITE_DATA_FLAG_NONE);
    ...
    bool Read(std::string* data, size_t num_bytes)
    {
        while (data->size() < num_bytes) {
            ...
            MojoResult result = receive_pipe_handle_->ReadData(buffer.data(), &read_size, MOJO_READ_DATA_FLAG_NONE);
            if (result == MOJO_RESULT_SHOULD_WAIT)
                continue;
            if (result != MOJO_RESULT_OK)
                return false;
...
private:
    bool IsCompleteResponse(const std::string& response)
    {
        ...
        size_t end_of_headers = net::HttpUtil::LocateEndOfHeaders(response.data(), response.size());
        ...
        auto headers = base::MakeRefCounted<net::HttpResponseHeaders>(net::HttpUtil::AssembleRawHeaders(base::StringPiece(response.data(), end_of_headers)));
...
public:
    ...
    void SetUp() override
    {
        int net_error = net::ERR_FAILED;
        base::RunLoop run_loop;
        factory_.CreateTCPServerSocket(net::IPEndPoint(net::IPAddress::IPv6Localhost(), 0), 1 /* backlog */, TRAFFIC_ANNOTATION_FOR_TESTS,
            server_socket_.InitWithNewPipeAndPassReceiver(),
            base::BindOnce(
                [](base::RunLoop* run_loop, int* result_out, net::IPEndPoint* local_addr_out, int result, const absl::optional<net::IPEndPoint>& local_addr) {
                    *result_out = result;
                    if (local_addr)
                        *local_addr_out = local_addr.value();
                    run_loop->Quit();
                },
                base::Unretained(&run_loop), base::Unretained(&net_error), base::Unretained(&server_address_)));
        run_loop.Run();
    ...
    void RunUntilRequestsReceived(size_t count)
    {
        ...
        base::RunLoop run_loop;
        base::AutoReset<base::RepeatingClosure> run_loop_quit_func(&run_loop_quit_func_, run_loop.QuitClosure());
        run_loop.Run();
...
std::string EncodeFrame(std::string message, net::WebSocketFrameHeader::OpCodeEnum op_code, bool mask, bool finish)
{
    ...
    if (mask) {
        ...
        WriteWebSocketFrameHeader(header, &masking_key, &frame_header[0], header_size);
...
TEST_F(WebSocketAcceptingTest, SendPingFrameWithNoMessage)
{
    ...
    const std::string ping_frame = EncodeFrame(message, net::WebSocketFrameHeader::OpCodeEnum::kOpCodePing,
        /* mask= */ true, /* finish= */ true);
    const std::string pong_frame = EncodeFrame(message, net::WebSocketFrameHeader::OpCodeEnum::kOpCodePong,
...
TEST_F(WebSocketAcceptingTest, SendLongTextFrame)
{
    ...
    const std::string text_encoded_frame = EncodeFrame(text_frame, net::WebSocketFrameHeader::OpCodeEnum::kOpCodeText,
        /* mask= */ true,
        /* finish= */ false);
    const std::string continuation_encoded_frame = EncodeFrame(continuation_frame, net::WebSocketFrameHeader::OpCodeEnum::kOpCodeContinuation,
        /* mask= */ true, /* finish= */ true);
...
TEST_F(HttpServerTest, Send200)
{
    ...
    client.Send("GET /test HTTP/1.1\r\n\r\n");
    RunUntilRequestsReceived(1);
    server_->Send200(GetConnectionId(0), "Response!", "text/plain", TRAFFIC_ANNOTATION_FOR_TESTS);

    std::string response;
    ASSERT_TRUE(client.ReadResponse(&response));
    ASSERT_TRUE(base::StartsWith(response, "HTTP/1.1 200 OK", base::CompareCase::SENSITIVE));
    ASSERT_TRUE(base::EndsWith(response, "Response!", base::CompareCase::SENSITIVE));
}
...
TEST_F(HttpServerTest, SendRaw)
{
    ...
    client.Send("GET /test HTTP/1.1\r\n\r\n");
    RunUntilRequestsReceived(1);
    server_->SendRaw(GetConnectionId(0), "Raw Data ", TRAFFIC_ANNOTATION_FOR_TESTS);
    server_->SendRaw(GetConnectionId(0), "More Data", TRAFFIC_ANNOTATION_FOR_TESTS);
    server_->SendRaw(GetConnectionId(0), "Third Piece of Data", TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(HttpServerTest, SendRawOverTwoConnections)
{
    ...
    server_->SendRaw(GetConnectionId(0), "Raw Data ", TRAFFIC_ANNOTATION_FOR_TESTS);
    server_->SendRaw(GetConnectionId(1), "More Data", TRAFFIC_ANNOTATION_FOR_TESTS);
    server_->SendRaw(GetConnectionId(0), "Third Piece of Data", TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST_F(HttpServerTest, MultipleRequestsOnSameConnection)
{
    ...
    ASSERT_TRUE(base::StartsWith(response1, "HTTP/1.1 200 OK", base::CompareCase::SENSITIVE));
    ASSERT_TRUE(base::EndsWith(response1, "Content for /test", base::CompareCase::SENSITIVE));
    ...
    ASSERT_TRUE(base::StartsWith(response2, "HTTP/1.1 404 Not Found", base::CompareCase::SENSITIVE));
    ...
    ASSERT_TRUE(base::StartsWith(response3, "HTTP/1.1 200 OK", base::CompareCase::SENSITIVE));
    ASSERT_TRUE(base::EndsWith(response3, "Content for /test3", base::CompareCase::SENSITIVE));
}
