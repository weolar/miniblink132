...
constexpr net::NetworkTrafficAnnotationTag kHttpServerErrorResponseTrafficAnnotation
    = net::DefineNetworkTrafficAnnotation("services_http_server_error_response",
        R"(
      semantics {
        sender: "HTTP Server"
...
HttpServer::HttpServer(mojo::PendingRemote<mojom::TCPServerSocket> server_socket, HttpServer::Delegate* delegate)
...
void HttpServer::AcceptWebSocket(int connection_id, const HttpServerRequestInfo& request, net::NetworkTrafficAnnotationTag traffic_annotation)
{
...
void HttpServer::SendOverWebSocket(int connection_id, base::StringPiece data, net::NetworkTrafficAnnotationTag traffic_annotation)
{
    HttpConnection* connection = FindConnection(connection_id);
    if (connection == NULL)
        return;
    DCHECK(connection->web_socket());
    connection->web_socket()->Send(data, net::WebSocketFrameHeader::OpCodeEnum::kOpCodeText, traffic_annotation);
}

void HttpServer::SendRaw(int connection_id, const std::string& data, net::NetworkTrafficAnnotationTag traffic_annotation)
{
    HttpConnection* connection = FindConnection(connection_id);
    if (connection == NULL)
        return;

    if (connection->write_buf().size() + data.size() > connection->WriteBufferSize()) {
        LOG(ERROR) << "Write buffer is full.";
        return;
    }
    connection->write_buf().append(data);
    if (!connection->write_watcher().IsWatching()) {
        connection->write_watcher().Watch(connection->send_handle(), MOJO_HANDLE_SIGNAL_WRITABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED,
            base::BindRepeating(&HttpServer::OnWritable, weak_ptr_factory_.GetWeakPtr(), connection->id()));
    }
}

void HttpServer::SendResponse(int connection_id, const HttpServerResponseInfo& response, net::NetworkTrafficAnnotationTag traffic_annotation)
{
    SendRaw(connection_id, response.Serialize(), traffic_annotation);
}

void HttpServer::Send(int connection_id, net::HttpStatusCode status_code, const std::string& data, const std::string& content_type,
...
void HttpServer::Send200(int connection_id, const std::string& data, const std::string& content_type, net::NetworkTrafficAnnotationTag traffic_annotation)
{
    Send(connection_id, net::HTTP_OK, data, content_type, traffic_annotation);
}

void HttpServer::Send404(int connection_id, net::NetworkTrafficAnnotationTag traffic_annotation)
{
    SendResponse(connection_id, HttpServerResponseInfo::CreateFor404(), traffic_annotation);
}

void HttpServer::Send500(int connection_id, const std::string& message, net::NetworkTrafficAnnotationTag traffic_annotation)
{
    SendResponse(connection_id, HttpServerResponseInfo::CreateFor500(message), traffic_annotation);
}
...
void HttpServer::DoAcceptLoop()
{
    server_socket_->Accept(mojo::NullRemote(), /* observer */
        base::BindOnce(&HttpServer::OnAcceptCompleted, weak_ptr_factory_.GetWeakPtr()));
}

void HttpServer::OnAcceptCompleted(int rv, const absl::optional<net::IPEndPoint>& remote_addr, mojo::PendingRemote<mojom::TCPConnectedSocket> connected_socket,
    mojo::ScopedDataPipeConsumerHandle receive_pipe_handle, mojo::ScopedDataPipeProducerHandle send_pipe_handle)
{
    ...
    std::unique_ptr<HttpConnection> connection_ptr = std::make_unique<HttpConnection>(
        ++last_id_, std::move(connected_socket), std::move(receive_pipe_handle), std::move(send_pipe_handle), remote_addr.value());
    ...
    if (!HasClosedConnection(connection)) {
        connection->read_watcher().Watch(connection->receive_handle(), MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED,
            MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED, base::BindRepeating(&HttpServer::OnReadable, weak_ptr_factory_.GetWeakPtr(), connection->id()));
    }

    DoAcceptLoop();
}

void HttpServer::OnReadable(int connection_id, MojoResult result, const mojo::HandleSignalsState& state)
{
    ...
    result = connection->receive_handle().BeginReadData(&read_buffer, &bytes_read, MOJO_READ_DATA_FLAG_NONE);
    ...
    if (connection->read_buf().size() + bytes_read > connection->ReadBufferSize()) {
        LOG(ERROR) << "Read buffer is full.";
        connection->receive_handle().EndReadData(bytes_read);
        return;
    }

    connection->read_buf().append(static_cast<const char*>(read_buffer), bytes_read);
    connection->receive_handle().EndReadData(bytes_read);
...
void HttpServer::HandleReadResult(HttpConnection* connection, MojoResult rv)
{
    ...
    while (!connection->read_buf().empty()) {
        ...
        size_t pos = 0;
        if (!ParseHeaders(connection->read_buf().data(), connection->read_buf().size(), &request, &pos)) {
        ...
        if (request.headers.count(kContentLength) > 0) {
            ...
            if (!base::StringToSizeT(request.GetHeaderValue(kContentLength), &content_length) || content_length > kMaxBodySize) {
                SendResponse(connection->id(),
                    HttpServerResponseInfo::CreateFor500("request content-length too big or unknown: " + request.GetHeaderValue(kContentLength)),
...
void HttpServer::OnWritable(int connection_id, MojoResult result)
{
    ...
    if (!connection->write_buf().empty()) {
        result = connection->send_handle().WriteData(write_buf.data(), &buffer_size, MOJO_WRITE_DATA_FLAG_NONE);
        if (result == MOJO_RESULT_SHOULD_WAIT)
            return;
...
bool HttpServer::ParseHeaders(const char* data, size_t data_len, HttpServerRequestInfo* info, size_t* ppos)
{
    size_t& pos = *ppos;
    int state = ST_METHOD;
    std::string buffer;
