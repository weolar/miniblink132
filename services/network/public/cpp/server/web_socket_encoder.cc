...
const char WebSocketEncoder::kClientExtensions[] = "permessage-deflate; client_max_window_bits";
...
WebSocket::ParseResult DecodeFrameHybi17(const base::StringPiece& frame, bool client_frame, int* bytes_consumed, std::string* output, bool* compressed)
{
    ...
    *compressed = reserved1;
    ...
    case net::WebSocketFrameHeader::OpCodeEnum::kOpCodeContinuation: // Treated in the same as kOpCodeText.
    case net::WebSocketFrameHeader::OpCodeEnum::kOpCodePing:
    case net::WebSocketFrameHeader::OpCodeEnum::kOpCodePong:
        break;

    case net::WebSocketFrameHeader::OpCodeEnum::kOpCodeBinary: // We don't support
        // binary frames yet.
    default:
...
void EncodeFrameHybi17(base::StringPiece message, int masking_key, bool compressed, net::WebSocketFrameHeader::OpCodeEnum op_code, std::string* output)
{
...
// static
std::unique_ptr<WebSocketEncoder> WebSocketEncoder::CreateServer(const std::string& extensions, net::WebSocketDeflateParameters* deflate_parameters)
{
    ...
    for (const auto& extension : parser.extensions()) {
        ...
        auto deflater = std::make_unique<net::WebSocketDeflater>(response.server_context_take_over_mode());
        auto inflater = std::make_unique<net::WebSocketInflater>(kInflaterChunkSize, kInflaterChunkSize);
        ...
        *deflate_parameters = response;
        return base::WrapUnique(new WebSocketEncoder(FOR_SERVER, std::move(deflater), std::move(inflater)));
    }
...
// static
std::unique_ptr<WebSocketEncoder> WebSocketEncoder::CreateClient(const std::string& response_extensions)
{
    ...
    auto deflater = std::make_unique<net::WebSocketDeflater>(params.client_context_take_over_mode());
    auto inflater = std::make_unique<net::WebSocketInflater>(kInflaterChunkSize, kInflaterChunkSize);
    ...
    return base::WrapUnique(new WebSocketEncoder(FOR_CLIENT, std::move(deflater), std::move(inflater)));
}

WebSocketEncoder::WebSocketEncoder(Type type, std::unique_ptr<net::WebSocketDeflater> deflater, std::unique_ptr<net::WebSocketInflater> inflater)
    : type_(type)
...
WebSocket::ParseResult WebSocketEncoder::DecodeFrame(const base::StringPiece& frame, int* bytes_consumed, std::string* output)
{
    bool compressed;
    std::string current_output;
    WebSocket::ParseResult result = DecodeFrameHybi17(frame, type_ == FOR_SERVER, bytes_consumed, &current_output, &compressed);
    if (result == WebSocket::FRAME_OK_FINAL || result == WebSocket::FRAME_OK_MIDDLE || result == WebSocket::FRAME_PING) {
...
void WebSocketEncoder::EncodeTextFrame(base::StringPiece frame, int masking_key, std::string* output)
{
...
void WebSocketEncoder::EncodePongFrame(base::StringPiece frame, int masking_key, std::string* output)
{
...
bool WebSocketEncoder::Inflate(std::string* message)
{
    ...
    while (inflater_->CurrentOutputSize() > 0) {
        scoped_refptr<net::IOBufferWithSize> chunk = inflater_->GetOutput(inflater_->CurrentOutputSize());
        if (!chunk.get())
            return false;
...
bool WebSocketEncoder::Deflate(base::StringPiece message, std::string* output)
{
    ...
    if (!deflater_->Finish())
        return false;
    scoped_refptr<net::IOBufferWithSize> buffer = deflater_->GetOutput(deflater_->CurrentOutputSize());
    if (!buffer.get())
        return false;
