...
HttpServerResponseInfo::HttpServerResponseInfo(const HttpServerResponseInfo& other) = default;
...
// static
HttpServerResponseInfo HttpServerResponseInfo::CreateFor500(const std::string& body)
{
...
void HttpServerResponseInfo::AddHeader(const std::string& name, const std::string& value)
{
    headers_.emplace_back(name, value);
}

void HttpServerResponseInfo::SetBody(const std::string& body, const std::string& content_type)
{
    DCHECK(body_.empty());
    body_ = body;
...
void HttpServerResponseInfo::SetContentHeaders(size_t content_length, const std::string& content_type)
{
    AddHeader(net::HttpRequestHeaders::kContentLength, base::StringPrintf("%" PRIuS, content_length));
