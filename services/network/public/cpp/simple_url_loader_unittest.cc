...
const char kShortUploadBody[] = "Though this upload be but little, it is fierce.";
...
std::string GetLongUploadBody(size_t size = SimpleURLLoader::kMaxUploadStringSizeToCopy)
{
...
public:
    ...
    enum class DownloadType { TO_STRING, TO_FILE, TO_TEMP_FILE, HEADERS_ONLY, AS_STREAM };

    explicit SimpleLoaderTestHelper(std::unique_ptr<network::ResourceRequest> resource_request, DownloadType download_type)
        : download_type_(download_type)
        , simple_url_loader_(SimpleURLLoader::Create(std::move(resource_request), TRAFFIC_ANNOTATION_FOR_TESTS))
    {
    ...
    void StartSimpleLoader(network::mojom::URLLoaderFactory* url_loader_factory, int64_t max_body_size = -1)
    {
        EXPECT_FALSE(done_);
        ...
        case DownloadType::TO_STRING:
            if (max_body_size < 0) {
                simple_url_loader_->DownloadToStringOfUnboundedSizeUntilCrashAndDie(
                    url_loader_factory, base::BindOnce(&SimpleLoaderTestHelper::DownloadedToString, base::Unretained(this)));
            } else {
                simple_url_loader_->DownloadToString(
                    url_loader_factory, base::BindOnce(&SimpleLoaderTestHelper::DownloadedToString, base::Unretained(this)), max_body_size);
            }
            break;
        case DownloadType::TO_FILE:
            if (max_body_size < 0) {
                simple_url_loader_->DownloadToFile(
                    url_loader_factory, base::BindOnce(&SimpleLoaderTestHelper::DownloadedToFile, base::Unretained(this)), dest_path_);
            } else {
                simple_url_loader_->DownloadToFile(
                    url_loader_factory, base::BindOnce(&SimpleLoaderTestHelper::DownloadedToFile, base::Unretained(this)), dest_path_, max_body_size);
            }
            break;
        case DownloadType::TO_TEMP_FILE:
            if (max_body_size < 0) {
                simple_url_loader_->DownloadToTempFile(url_loader_factory, base::BindOnce(&SimpleLoaderTestHelper::DownloadedToFile, base::Unretained(this)));
            } else {
                simple_url_loader_->DownloadToTempFile(
                    url_loader_factory, base::BindOnce(&SimpleLoaderTestHelper::DownloadedToFile, base::Unretained(this)), max_body_size);
            }
            break;
        case DownloadType::HEADERS_ONLY:
            simple_url_loader_->DownloadHeadersOnly(url_loader_factory, base::BindOnce(&SimpleLoaderTestHelper::DownloadedHeadersOnly, base::Unretained(this)));
            break;
    ...
    void StartSimpleLoaderAndWait(network::mojom::URLLoaderFactory* url_loader_factory, int64_t max_body_size = -1)
    {
    ...
    void set_download_to_stream_async_resume(bool download_to_stream_async_resume)
    {
    ...
    void set_download_to_stream_capture_resume(bool download_to_stream_capture_resume)
    {
    ...
    // OnDataReceived.
    void set_download_to_stream_destroy_on_data_received(bool download_to_stream_destroy_on_data_received)
    {
        download_to_stream_destroy_on_data_received_ = download_to_stream_destroy_on_data_received;
    }
    ...
    void set_download_to_stream_destroy_on_retry(bool download_to_stream_destroy_on_retry)
    {
    ...
    SimpleURLLoader* simple_url_loader()
    {
        return simple_url_loader_.get();
    }
...
private:
    ...
    void DownloadedToFile(base::FilePath file_path)
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
        EXPECT_FALSE(done_);
        EXPECT_TRUE(download_type_ == DownloadType::TO_FILE || download_type_ == DownloadType::TO_TEMP_FILE);
        EXPECT_FALSE(response_body_);
        ...
        if (download_type_ == DownloadType::TO_FILE) {
            ...
            EXPECT_EQ(!file_path.empty() || expect_path_exists_on_error_, base::PathExists(dest_path_));
    ...
    void OnDataReceived(std::string_view string_piece, base::OnceClosure resume) override
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
        EXPECT_FALSE(done_);
        ...
        if (download_to_stream_async_resume_) {
            base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, std::move(resume));
            return;
        }
        std::move(resume).Run();
    }
    ...
    void OnComplete(bool success) override
    {
        ...
        if (success || (allow_http_error_results_ && simple_url_loader_->ResponseInfo())) {
            response_body_ = std::make_unique<std::string>(download_as_stream_response_body_);
        }
    ...
    void OnRetry(base::OnceClosure start_retry) override
    {
        ...
        if (download_to_stream_async_retry_) {
            base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, std::move(start_retry));
            return;
        }
...
std::unique_ptr<net::test_server::HttpResponse> HandleResponseSize(const net::test_server::HttpRequest& request)
{
    if (request.GetURL().path_piece() != kResponseSizePath)
        return nullptr;

    std::unique_ptr<net::test_server::BasicHttpResponse> response = std::make_unique<net::test_server::BasicHttpResponse>();
...
std::unique_ptr<net::test_server::HttpResponse> HandleInvalidGzip(const net::test_server::HttpRequest& request)
{
...
std::unique_ptr<net::test_server::HttpResponse> HandleTruncatedBody(const net::test_server::HttpRequest& request)
{
    if (request.GetURL().path_piece() != kTruncatedBodyPath)
        return nullptr;

    std::unique_ptr<net::test_server::RawHttpResponse> response
        = std::make_unique<net::test_server::RawHttpResponse>(base::StringPrintf("HTTP/1.1 200 OK\r\n"
...
std::unique_ptr<net::test_server::HttpResponse> FailOnceThenEchoBody(bool* has_failed_request, const net::test_server::HttpRequest& request)
{
...
public:
        ...
        : task_environment_(base::test::TaskEnvironment::MainThreadType::IO)
    {
        ...
        network_service_ = network::NetworkService::Create(network_service_remote.BindNewPipeAndPassReceiver());
        network::mojom::NetworkContextParamsPtr context_params = network::mojom::NetworkContextParams::New();

        // Use a dummy CertVerifier that always passes cert verification, since
        // these unittests don't need to test CertVerifier behavior.
        context_params->cert_verifier_params = FakeTestCertVerifierParamsFactory::GetCertVerifierParams();

        network_service_remote->CreateNetworkContext(network_context_.BindNewPipeAndPassReceiver(), std::move(context_params));

        mojo::PendingReceiver<network::mojom::URLLoaderNetworkServiceObserver> default_observer_receiver;
        network::mojom::NetworkServiceParamsPtr network_service_params = network::mojom::NetworkServiceParams::New();
        network_service_params->default_observer = default_observer_receiver.InitWithNewPipeAndPassRemote();
        network_service_remote->SetParams(std::move(network_service_params));

        mojo::PendingRemote<network::mojom::NetworkContextClient> network_context_client_remote;
        network_context_client_ = std::make_unique<TestNetworkContextClient>(network_context_client_remote.InitWithNewPipeAndPassReceiver());
        network_context_->SetClient(std::move(network_context_client_remote));

        mojom::URLLoaderFactoryParamsPtr params = mojom::URLLoaderFactoryParams::New();
        ...
        network_context_->CreateURLLoaderFactory(url_loader_factory_.BindNewPipeAndPassReceiver(), std::move(params));

        test_server_.AddDefaultHandlers(base::FilePath(FILE_PATH_LITERAL("")));
        test_server_.RegisterRequestHandler(base::BindRepeating(&HandleResponseSize));
        test_server_.RegisterRequestHandler(base::BindRepeating(&HandleInvalidGzip));
        test_server_.RegisterRequestHandler(base::BindRepeating(&HandleTruncatedBody));
        test_server_.RegisterRequestHandler(base::BindRepeating(&FailOnceThenEchoBody, base::Owned(new bool(false))));
...
using DownloadTypeAndOptions = std::tuple<SimpleLoaderTestHelper::DownloadType, ReadAndDiscardBodyType>;
...
struct URLLoaderFactoryTestConfig {
    ReadAndDiscardBodyType read_and_discard_body_type = ReadAndDiscardBodyType::kDisabled;
    bool expect_read_and_discard_option = false;
};

class SimpleURLLoaderTest : public SimpleURLLoaderTestBase, public testing::TestWithParam<DownloadTypeAndOptions> {
public:
    SimpleURLLoaderTest()
    {
        if (GetReadAndDiscardBodyType() == ReadAndDiscardBodyType::kDisabled) {
            scoped_feature_list_.InitAndDisableFeature(kSimpleURLLoaderUseReadAndDiscardBodyOption);
        } else {
            scoped_feature_list_.InitAndEnableFeature(kSimpleURLLoaderUseReadAndDiscardBodyOption);
        }
    }
    ...
    URLLoaderFactoryTestConfig GetURLLoaderFactoryTestConfig()
    {
        ...
        return { read_and_discard_body_type, IsHeadersOnly() && read_and_discard_body_type != ReadAndDiscardBodyType::kDisabled };
    }

    // Many tests need to check this, so provide a short-cut.
    bool IsHeadersOnly() const
    {
        return GetDownloadType() == SimpleLoaderTestHelper::DownloadType::HEADERS_ONLY;
    }

    std::unique_ptr<SimpleLoaderTestHelper> CreateHelper(std::unique_ptr<network::ResourceRequest> resource_request)
    {
        EXPECT_TRUE(resource_request);
        return std::make_unique<SimpleLoaderTestHelper>(std::move(resource_request), GetDownloadType());
    }

    std::unique_ptr<SimpleLoaderTestHelper> CreateHelperForURL(const GURL& url, const char* method = "GET")
    {
        ...
        resource_request->trusted_params->isolation_info
            = net::IsolationInfo::Create(net::IsolationInfo::RequestType::kOther, request_origin, request_origin, net::SiteForCookies());
        return std::make_unique<SimpleLoaderTestHelper>(std::move(resource_request), GetDownloadType());
    }
...
TEST_P(SimpleURLLoaderTest, BasicRequest)
{
    ...
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelper(std::move(resource_request));
    ...
    EXPECT_EQ(net::OK, test_helper->simple_url_loader()->CompletionStatus()->error_code);
    ...
    if (!IsHeadersOnly()) {
        ...
        EXPECT_EQ(kExpectedResponseSize, test_helper->simple_url_loader()->GetContentSize());
        EXPECT_EQ(kExpectedResponseSize, test_helper->simple_url_loader()->CompletionStatus()->decoded_body_length);
    }
}
...
TEST_P(SimpleURLLoaderTest, GzipBody)
{
    ...
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL(base::StringPrintf("/gzip-body?%s", content.c_str())));
    test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get());
    ...
    if (!IsHeadersOnly()) {
        ...
        EXPECT_EQ(static_cast<int64_t>(content.size()), test_helper->simple_url_loader()->GetContentSize());
        ASSERT_TRUE(test_helper->simple_url_loader()->CompletionStatus());
        EXPECT_EQ(static_cast<int64_t>(content.size()), test_helper->simple_url_loader()->CompletionStatus()->decoded_body_length);
        EXPECT_LT(test_helper->simple_url_loader()->CompletionStatus()->encoded_body_length,
            test_helper->simple_url_loader()->CompletionStatus()->decoded_body_length);
    }
}
...
{
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL("/server-redirect?" + test_server_.GetURL("/echo").spec()));
    test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get());
...
{
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL("/server-redirect?file:///etc/passwd"));
    test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get());

    EXPECT_EQ(net::ERR_UNKNOWN_URL_SCHEME, test_helper->simple_url_loader()->NetError());
    ASSERT_TRUE(test_helper->simple_url_loader()->CompletionStatus());
    EXPECT_EQ(net::ERR_UNKNOWN_URL_SCHEME, test_helper->simple_url_loader()->CompletionStatus()->error_code);
...
{
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL("/server-redirect?data:text/plain;charset=utf-8;base64,Zm9v"));
    test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get());
...
{
    const GURL kInitialURL = test_server_.GetURL("/server-redirect?" + test_server_.GetURL("/echo").spec());
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(kInitialURL);
    ...
    test_helper->simple_url_loader()->SetOnRedirectCallback(base::BindRepeating(
        [](int* num_redirects, GURL* url_before_redirect_ptr, net::RedirectInfo* redirect_info_ptr, network::mojom::URLResponseHeadPtr* response_head_ptr,
            const GURL& url_before_redirect, const net::RedirectInfo& redirect_info, const network::mojom::URLResponseHead& response_head,
        ...
        base::Unretained(&num_redirects), base::Unretained(&url_before_redirect), base::Unretained(&redirect_info), base::Unretained(&response_head)));
...
{
    const GURL kRedirectURL = test_server_.GetURL("/server-redirect?" + test_server_.GetURL("/echo").spec());
    const GURL kInitialURL = test_server_.GetURL("/server-redirect?" + kRedirectURL.spec());
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(kInitialURL);
    int num_redirects = 0;
    std::vector<GURL> urls_before_redirect;
    test_helper->simple_url_loader()->SetOnRedirectCallback(base::BindRepeating(
        [](int* num_redirects, std::vector<GURL>* urls_before_redirect, const GURL& url_before_redirect, const net::RedirectInfo& redirect_info,
            const network::mojom::URLResponseHead& response_head, std::vector<std::string>* to_be_removed_headers) {
            ++*num_redirects;
            urls_before_redirect->push_back(url_before_redirect);
        },
        base::Unretained(&num_redirects), base::Unretained(&urls_before_redirect)));
...
TEST_P(SimpleURLLoaderTest, DeleteInOnRedirectCallback)
{
    ...
    test_helper->simple_url_loader()->SetOnRedirectCallback(
        base::BindLambdaForTesting([&](const GURL& url_before_redirect, const net::RedirectInfo& redirect_info,
                                       const network::mojom::URLResponseHead& response_head, std::vector<std::string>* to_be_removed_headers) {
            CHECK(test_helper);
            test_helper.reset();
...
TEST_P(SimpleURLLoaderTest, UploadShortStringWithRedirect)
{
    ...
    std::unique_ptr<SimpleLoaderTestHelper> test_helper
        = CreateHelperForURL(test_server_.GetURL("/server-redirect-307?" + test_server_.GetURL("/echo").spec()), "POST");
    test_helper->simple_url_loader()->AttachStringForUpload(kShortUploadBody, "text/plain");

    int num_redirects = 0;
    test_helper->simple_url_loader()->SetOnRedirectCallback(
        base::BindRepeating([](int* num_redirects, const GURL& url_before_redirect, const net::RedirectInfo& redirect_info,
                                const network::mojom::URLResponseHead& response_head, std::vector<std::string>* to_be_removed_headers) { ++*num_redirects; },
...
TEST_P(SimpleURLLoaderTest, UploadLongStringWithRedirect)
{
    ...
    std::unique_ptr<SimpleLoaderTestHelper> test_helper
        = CreateHelperForURL(test_server_.GetURL("/server-redirect-307?" + test_server_.GetURL("/echo").spec()), "POST");
    test_helper->simple_url_loader()->AttachStringForUpload(GetLongUploadBody(), "text/plain");
...
TEST_P(SimpleURLLoaderTest, OnRedirectCallbackReturnsExistingToBeRemovedHeaders)
{
    ...
    int num_redirects = 0;
    test_helper->simple_url_loader()->SetOnRedirectCallback(base::BindRepeating(
        [](int* num_redirects, const GURL& url_before_redirect, const net::RedirectInfo& redirect_info, const network::mojom::URLResponseHead& response_head,
            std::vector<std::string>* to_be_removed_headers) {
            ++*num_redirects;
...
TEST_P(SimpleURLLoaderTest, OnRedirectCallbackReturnsNonExistingToBeRemovedHeaders)
{
    GURL url = test_server_.GetURL("/echoheader?foo");
    std::unique_ptr<network::ResourceRequest> resource_request = std::make_unique<network::ResourceRequest>();
    resource_request->url = test_server_.GetURL("/server-redirect?" + url.spec());
...
TEST_P(SimpleURLLoaderTest, OnResponseStartedCallback)
{
    ...
    test_helper->simple_url_loader()->SetOnResponseStartedCallback(base::BindOnce(
        [](GURL* out_final_url, std::string* foo_header_value, base::OnceClosure quit_closure, const GURL& final_url,
            const mojom::URLResponseHead& response_head) {
            *out_final_url = final_url;
            if (response_head.headers) {
                *foo_header_value = response_head.headers->GetNormalizedHeader("foo").value_or(std::string());
            }
...
{
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL("/echo"));

    SimpleLoaderTestHelper* unowned_test_helper = test_helper.get();
    base::RunLoop run_loop;
    unowned_test_helper->simple_url_loader()->SetOnResponseStartedCallback(base::BindOnce(
        [](std::unique_ptr<SimpleLoaderTestHelper> test_helper, base::OnceClosure quit_closure, const GURL& final_url,
            const mojom::URLResponseHead& response_head) {
...
{
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL("/echo?status=400"));
    test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get());

    EXPECT_EQ(net::ERR_HTTP_RESPONSE_CODE_FAILURE, test_helper->simple_url_loader()->NetError());
    EXPECT_FALSE(test_helper->simple_url_loader()->CompletionStatus());
...
TEST_P(SimpleURLLoaderTest, HttpErrorStatusCodeResponseAllowed)
{
    ...
    if (!IsHeadersOnly()) {
        ...
        EXPECT_EQ(4, test_helper->simple_url_loader()->CompletionStatus()->decoded_body_length);
    }
}

TEST_P(SimpleURLLoaderTest, EmptyResponseBody)
{
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL("/nocontent"));
    test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get());
    ...
    if (!IsHeadersOnly()) {
        ...
        EXPECT_EQ(0, test_helper->simple_url_loader()->GetContentSize());
        EXPECT_EQ(0, test_helper->simple_url_loader()->CompletionStatus()->decoded_body_length);
    }
}
...
TEST_P(SimpleURLLoaderTest, BigResponseBody)
{
    ...
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL(base::StringPrintf("/response-size?%u", kResponseSize)));
    test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get());
    ...
    if (!IsHeadersOnly()) {
        ...
        EXPECT_EQ(kResponseSize, test_helper->simple_url_loader()->GetContentSize());
        EXPECT_EQ(kResponseSize, test_helper->simple_url_loader()->CompletionStatus()->decoded_body_length);
    }
}
...
TEST_P(SimpleURLLoaderTest, ResponseBodyWithSizeMatchingLimit)
{
    ...
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL(base::StringPrintf("/response-size?%u", kResponseSize)));
    test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get(), kResponseSize);
...
TEST_P(SimpleURLLoaderTest, ResponseBodyWithSizeBelowLimit)
{
    ...
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL(base::StringPrintf("/response-size?%u", kResponseSize)));
    test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get(), kMaxResponseSize);
...
TEST_P(SimpleURLLoaderTest, ResponseBodyWithSizeAboveLimit)
{
    ...
    const uint32_t kResponseSize = 16;
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL(base::StringPrintf("/response-size?%u", kResponseSize)));
    test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get(), kResponseSize - 1);
    ...
    if (IsHeadersOnly()) {
        ...
        EXPECT_FALSE(test_helper->simple_url_loader()->CompletionStatus());
        EXPECT_EQ(net::ERR_INSUFFICIENT_RESOURCES, test_helper->simple_url_loader()->NetError());
        EXPECT_EQ(0, test_helper->simple_url_loader()->GetContentSize());
    }
    EXPECT_FALSE(test_helper->response_body());
}
...
TEST_P(SimpleURLLoaderTest, ResponseBodyWithSizeAboveLimitPartialResponse)
{
    ...
    if (IsHeadersOnly()) {
        ...
        EXPECT_EQ(kMaxResponseSize, test_helper->simple_url_loader()->GetContentSize());
    }
}
...
{
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL("/close-socket"));
    test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get());

    EXPECT_EQ(net::ERR_EMPTY_RESPONSE, test_helper->simple_url_loader()->NetError());
    ASSERT_TRUE(test_helper->simple_url_loader()->CompletionStatus());
    EXPECT_EQ(net::ERR_EMPTY_RESPONSE, test_helper->simple_url_loader()->CompletionStatus()->error_code);
    EXPECT_FALSE(test_helper->simple_url_loader()->ResponseInfo());
    EXPECT_FALSE(test_helper->response_body());
    EXPECT_EQ(0, test_helper->simple_url_loader()->GetContentSize());
    EXPECT_EQ(0, test_helper->simple_url_loader()->CompletionStatus()->decoded_body_length);
...
{
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL(kInvalidGzipPath));
    test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get());

    EXPECT_EQ(net::ERR_CONTENT_DECODING_FAILED, test_helper->simple_url_loader()->NetError());
    ASSERT_TRUE(test_helper->simple_url_loader()->CompletionStatus());
    EXPECT_EQ(net::ERR_CONTENT_DECODING_FAILED, test_helper->simple_url_loader()->CompletionStatus()->error_code);
    EXPECT_EQ(200, test_helper->GetResponseCode());
    EXPECT_FALSE(test_helper->response_body());
    EXPECT_EQ(0, test_helper->simple_url_loader()->GetContentSize());
    EXPECT_EQ(0, test_helper->simple_url_loader()->CompletionStatus()->decoded_body_length);
...
{
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL(kTruncatedBodyPath));
    test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get());

    EXPECT_EQ(net::ERR_CONTENT_LENGTH_MISMATCH, test_helper->simple_url_loader()->NetError());
    ASSERT_TRUE(test_helper->simple_url_loader()->CompletionStatus());
    EXPECT_EQ(net::ERR_CONTENT_LENGTH_MISMATCH, test_helper->simple_url_loader()->CompletionStatus()->error_code);
    EXPECT_EQ(200, test_helper->GetResponseCode());
    EXPECT_FALSE(test_helper->response_body());
    EXPECT_EQ(static_cast<int64_t>(strlen(kTruncatedBody)), test_helper->simple_url_loader()->GetContentSize());
    EXPECT_EQ(static_cast<int64_t>(strlen(kTruncatedBody)), test_helper->simple_url_loader()->CompletionStatus()->decoded_body_length);
}
...
{
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL("/hung"));
    test_helper->StartSimpleLoader(url_loader_factory_.get());
    {
...
{
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL("/echo"), "POST");
    test_helper->simple_url_loader()->AttachStringForUpload(kShortUploadBody, "text/plain");
    test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get());
    EXPECT_EQ(net::OK, test_helper->simple_url_loader()->NetError());
    ASSERT_TRUE(test_helper->simple_url_loader()->CompletionStatus());
...
TEST_P(SimpleURLLoaderTest, UploadLongString)
{
    ...
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL("/echo"), "POST");
    test_helper->simple_url_loader()->AttachStringForUpload(long_string, "text/plain");
    test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get());
    EXPECT_EQ(net::OK, test_helper->simple_url_loader()->NetError());
    ASSERT_TRUE(test_helper->simple_url_loader()->CompletionStatus());
    EXPECT_EQ(net::OK, test_helper->simple_url_loader()->CompletionStatus()->error_code);
...
TEST_P(SimpleURLLoaderTest, UploadEmptyString)
{
    ...
    if (!IsHeadersOnly()) {
        ASSERT_TRUE(test_helper->response_body());
        EXPECT_NE(std::string::npos, test_helper->response_body()->find("Content-Type: text/plain"));
...
{
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL(kFailOnceThenEchoBody), "POST");
    test_helper->simple_url_loader()->AttachStringForUpload(kShortUploadBody, "text/plain");
    test_helper->simple_url_loader()->SetRetryOptions(1, SimpleURLLoader::RETRY_ON_5XX);
    test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get());
    EXPECT_EQ(net::OK, test_helper->simple_url_loader()->NetError());
    ASSERT_TRUE(test_helper->simple_url_loader()->CompletionStatus());
    EXPECT_EQ(net::OK, test_helper->simple_url_loader()->CompletionStatus()->error_code);
...
{
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL("/echo"), "POST");
    test_helper->simple_url_loader()->AttachFileForUpload(GetTestFilePath(), "text/plain");
    test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get());
    EXPECT_EQ(net::OK, test_helper->simple_url_loader()->NetError());
    ASSERT_TRUE(test_helper->simple_url_loader()->CompletionStatus());
    EXPECT_EQ(net::OK, test_helper->simple_url_loader()->CompletionStatus()->error_code);
...
TEST_P(SimpleURLLoaderTest, UploadFileRange)
{
    ...
    test_helper->simple_url_loader()->AttachFileForUpload(GetTestFilePath(), "text/plain", kOffset, kLength);
    test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get());
    EXPECT_EQ(net::OK, test_helper->simple_url_loader()->NetError());
    ASSERT_TRUE(test_helper->simple_url_loader()->CompletionStatus());
    EXPECT_EQ(net::OK, test_helper->simple_url_loader()->CompletionStatus()->error_code);
    ...
    if (!IsHeadersOnly()) {
        ASSERT_TRUE(test_helper->response_body());
        EXPECT_EQ(GetTestFileContents().substr(kOffset, kLength), *test_helper->response_body());
    }
}

TEST_P(SimpleURLLoaderTest, UploadFileWithPut)
{
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL("/echo"), "PUT");
    test_helper->simple_url_loader()->AttachFileForUpload(GetTestFilePath(), "text/plain");
    test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get());
    EXPECT_EQ(net::OK, test_helper->simple_url_loader()->NetError());
    ASSERT_TRUE(test_helper->simple_url_loader()->CompletionStatus());
    ...
    test_helper->simple_url_loader()->AttachFileForUpload(GetTestFilePath(), "text/salted");
    test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get());
    EXPECT_EQ(net::OK, test_helper->simple_url_loader()->NetError());
    ASSERT_TRUE(test_helper->simple_url_loader()->CompletionStatus());
    EXPECT_EQ(net::OK, test_helper->simple_url_loader()->CompletionStatus()->error_code);
    ...
    if (!IsHeadersOnly()) {
        ASSERT_TRUE(test_helper->response_body());
        EXPECT_NE(std::string::npos, test_helper->response_body()->find("Content-Type: text/salted"));
        EXPECT_EQ(std::string::npos, test_helper->response_body()->find("Content-Type: text/plain"));
...
TEST_P(SimpleURLLoaderTest, UploadFileWithRetry)
{
    ...
    if (!IsHeadersOnly()) {
        ASSERT_TRUE(test_helper->response_body());
        EXPECT_EQ(GetTestFileContents(), *test_helper->response_body());
        EXPECT_EQ(static_cast<int64_t>(GetTestFileContents().size()), test_helper->simple_url_loader()->GetContentSize());
        EXPECT_EQ(static_cast<int64_t>(GetTestFileContents().size()), test_helper->simple_url_loader()->CompletionStatus()->decoded_body_length);
    }
...
TEST_P(SimpleURLLoaderTest, UploadNonexistentFile)
{
    ...
    base::FilePath path_to_nonexistent_file = GetTestFilePath().DirName().AppendASCII("this/file/does/not/exist");
    // Appending a path to the end of a file should guarantee no such file exists.
    test_helper->simple_url_loader()->AttachFileForUpload(path_to_nonexistent_file, "text/plain");
    test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get());
    EXPECT_EQ(net::ERR_FILE_NOT_FOUND, test_helper->simple_url_loader()->NetError());
    ASSERT_TRUE(test_helper->simple_url_loader()->CompletionStatus());
    EXPECT_EQ(net::ERR_FILE_NOT_FOUND, test_helper->simple_url_loader()->CompletionStatus()->error_code);
    EXPECT_FALSE(test_helper->simple_url_loader()->ResponseInfo());
    EXPECT_FALSE(test_helper->response_body());
    EXPECT_EQ(0, test_helper->simple_url_loader()->GetContentSize());
    EXPECT_EQ(0, test_helper->simple_url_loader()->CompletionStatus()->decoded_body_length);
...
{
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(GURL("http://does_not_matter:7/"), "POST");
    test_helper->simple_url_loader()->AttachFileForUpload(GetTestFilePath(), "text/plain");
...
{
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL("/hung"), "POST");
    test_helper->simple_url_loader()->AttachFileForUpload(GetTestFilePath(), "text/plain");
    test_helper->simple_url_loader()->SetRetryOptions(2, SimpleURLLoader::RETRY_ON_5XX);
    test_helper->StartSimpleLoader(url_loader_factory_.get());
    task_environment_.RunUntilIdle();
    test_helper.reset();
    task_environment_.RunUntilIdle();
...
public:
    MockURLLoader(base::test::TaskEnvironment* task_environment, mojo::PendingReceiver<network::mojom::URLLoader> url_loader_receiver,
        mojo::PendingRemote<network::mojom::URLLoaderClient> client, std::vector<TestLoaderEvent> test_events,
        scoped_refptr<network::ResourceRequestBody> request_body, bool should_create_data_pipe)
        ...
        , should_create_data_pipe_(should_create_data_pipe)
    {
        if (request_body && request_body->elements()->size() == 1
            && (*request_body->elements())[0].type() == network::mojom::DataElementDataView::Tag::kDataPipe) {
            const auto& element = (*request_body->elements())[0].As<network::DataElementDataPipe>();
    ...
    void RunTest()
    {
        for (auto test_event : test_events_) {
            ...
            case TestLoaderEvent::kStartReadLongUploadBody: {
                ...
                ASSERT_EQ(mojo::CreateDataPipe(nullptr, producer_handle, upload_data_pipe_), MOJO_RESULT_OK);
                data_pipe_getter_->Read(
                    std::move(producer_handle), base::BindOnce(&MockURLLoader::OnReadComplete, weak_factory_for_data_pipe_callbacks_.GetWeakPtr()));
            ...
            case TestLoaderEvent::kReadLongUploadBody: {
                ...
                while (true) {
                    ...
                    MojoResult result = upload_data_pipe_->ReadData(MOJO_READ_DATA_FLAG_NONE, base::as_writable_byte_span(read_buffer), actually_read_bytes);
                    ...
                    if (result != MOJO_RESULT_OK)
                        break;
                    upload_body.append(std::string_view(read_buffer).substr(0, actually_read_bytes));
                }
            ...
            case TestLoaderEvent::kReadFirstByteOfLongUploadBody: {
                ...
                size_t read_size = 0;
                while (true) {
                    result = upload_data_pipe_->ReadData(MOJO_READ_DATA_FLAG_NONE, base::span_from_ref(byte), read_size);
                    if (result != MOJO_RESULT_SHOULD_WAIT)
                        break;
                    base::RunLoop().RunUntilIdle();
                }
            ...
            case TestLoaderEvent::kReceivedRedirect: {
                ...
                std::string headers("HTTP/1.0 301 The Response Has Moved to Another Server\n"
                                    "Location: bar://foo/");
                response_info->headers = base::MakeRefCounted<net::HttpResponseHeaders>(net::HttpUtil::AssembleRawHeaders(headers));
            ...
            case TestLoaderEvent::kReceivedResponse: {
                ...
                if (should_create_data_pipe_) {
                    ASSERT_EQ(mojo::CreateDataPipe(1024, body_stream_, consumer_handle), MOJO_RESULT_OK);
                }
                client_->OnReceiveResponse(std::move(response_info), std::move(consumer_handle), std::nullopt);
                break;
            }
            ...
            case TestLoaderEvent::kReceivedResponseNoData: {
                ...
                std::string headers("HTTP/1.0 200 OK");
                response_info->headers = base::MakeRefCounted<net::HttpResponseHeaders>(net::HttpUtil::AssembleRawHeaders(headers));
                client_->OnReceiveResponse(std::move(response_info), mojo::ScopedDataPipeConsumerHandle(), std::nullopt);
                break;
            }
            case TestLoaderEvent::kBodyDataRead: {
                if (should_create_data_pipe_) {
                    ...
                    EXPECT_EQ(MOJO_RESULT_OK, body_stream_->WriteData(base::byte_span_from_cstring("a"), MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes));
    ...
    network::mojom::URLLoaderClient* client() const
    {
        return client_.get();
    }
...
private:
    ...
    base::WeakPtrFactory<MockURLLoader> weak_factory_for_data_pipe_callbacks_ { this };
};

class MockURLLoaderFactory : public network::mojom::URLLoaderFactory {
public:
    explicit MockURLLoaderFactory(base::test::TaskEnvironment* task_environment, URLLoaderFactoryTestConfig test_config)
        : task_environment_(task_environment)
    ...
    void CreateLoaderAndStart(mojo::PendingReceiver<network::mojom::URLLoader> url_loader_receiver, int32_t request_id, uint32_t options,
        const network::ResourceRequest& url_request, mojo::PendingRemote<network::mojom::URLLoaderClient> client,
        const net::MutableNetworkTrafficAnnotationTag& traffic_annotation) override
    {
        ...
        const bool read_and_discard_body_option_set = options & mojom::kURLLoadOptionReadAndDiscardBody;
        ...
        if (read_and_discard_body_option_set && test_config_.read_and_discard_body_type != ReadAndDiscardBodyType::kEnabledButIgnored) {
            should_create_data_pipe = false;
        }
        url_loaders_.push_back(std::make_unique<MockURLLoader>(
            task_environment_, std::move(url_loader_receiver), std::move(client), test_events_.front(), url_request.request_body, should_create_data_pipe));
    ...
    void RunTest(SimpleLoaderTestHelper* test_helper, bool wait_for_completion = true)
    {
    ...
    const std::list<GURL>& requested_urls() const
    {
...
{
    MockURLLoaderFactory loader_factory(&task_environment_, GetURLLoaderFactoryTestConfig());
    loader_factory.AddEvents({ TestLoaderEvent::kResponseComplete });
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(GURL("foo://bar/"));
...
{
    MockURLLoaderFactory loader_factory(&task_environment_, GetURLLoaderFactoryTestConfig());
    loader_factory.AddEvents({ TestLoaderEvent::kReceivedResponseNoData, TestLoaderEvent::kResponseComplete });
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(GURL("foo://bar/"));
    loader_factory.RunTest(test_helper.get());
...
{
    MockURLLoaderFactory loader_factory(&task_environment_, GetURLLoaderFactoryTestConfig());
    loader_factory.AddEvents({ TestLoaderEvent::kReceivedResponseNoData, TestLoaderEvent::kClientPipeClosed });
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(GURL("foo://bar/"));
    loader_factory.RunTest(test_helper.get());
...
{
    if (GetReadAndDiscardBodyType() == ReadAndDiscardBodyType::kEnabled && IsHeadersOnly()) {
    ...
    for (const auto close_client_order : kClientCloseOrder) {
        for (const auto close_client_event : kClientCloseEvents) {
            for (uint32_t bytes_received = 0; bytes_received < 3; bytes_received++) {
                for (int allow_partial_results = 0; allow_partial_results < 2; allow_partial_results++) {
                    if (close_client_order == ClientCloseOrder::kDuringData && bytes_received < 2) {
                        continue;
                    }
                    MockURLLoaderFactory loader_factory(&task_environment_, GetURLLoaderFactoryTestConfig());
                    ...
                    if (close_client_event != TestLoaderEvent::kResponseComplete) {
                        if (close_client_event == TestLoaderEvent::kResponseCompleteFailed) {
                            EXPECT_EQ(net::ERR_TIMED_OUT, test_helper->simple_url_loader()->NetError());
                            ASSERT_TRUE(test_helper->simple_url_loader()->CompletionStatus());
                            EXPECT_EQ(net::ERR_TIMED_OUT, test_helper->simple_url_loader()->CompletionStatus()->error_code);
                        } else {
                            EXPECT_EQ(net::ERR_FAILED, test_helper->simple_url_loader()->NetError());
                            EXPECT_FALSE(test_helper->simple_url_loader()->CompletionStatus());
                        }
                        if (!allow_partial_results) {
                            ...
                            ASSERT_TRUE(test_helper->response_body());
                            EXPECT_EQ(std::string(bytes_received, 'a'), *test_helper->response_body());
                        }
                    } else {
                        EXPECT_EQ(net::OK, test_helper->simple_url_loader()->NetError());
                        ASSERT_TRUE(test_helper->simple_url_loader()->CompletionStatus());
...
{
    MockURLLoaderFactory loader_factory(&task_environment_, GetURLLoaderFactoryTestConfig());
    loader_factory.AddEvents({ TestLoaderEvent::kReceivedResponseNoData, TestLoaderEvent::kResponseCompleteFailed, TestLoaderEvent::kClientPipeClosed });
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(GURL("foo://bar/"));
    loader_factory.RunTest(test_helper.get());
...
{
    MockURLLoaderFactory loader_factory(&task_environment_, GetURLLoaderFactoryTestConfig());
    loader_factory.AddEvents({ TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kResponseComplete, TestLoaderEvent::kClientPipeClosed,
...
{
    MockURLLoaderFactory loader_factory(&task_environment_, GetURLLoaderFactoryTestConfig());
    loader_factory.AddEvents({ TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kBodyDataRead, TestLoaderEvent::kBodyBufferClosed,
...
{
    MockURLLoaderFactory loader_factory(&task_environment_, GetURLLoaderFactoryTestConfig());
    loader_factory.AddEvents({ TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kReceivedResponse });
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(GURL("foo://bar/"));
    loader_factory.RunTest(test_helper.get());
...
{
    MockURLLoaderFactory loader_factory(&task_environment_, GetURLLoaderFactoryTestConfig());
    loader_factory.AddEvents({ TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kReceivedRedirect });
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(GURL("foo://bar/"));
    loader_factory.RunTest(test_helper.get());
...
{
    MockURLLoaderFactory loader_factory(&task_environment_, GetURLLoaderFactoryTestConfig());
    loader_factory.AddEvents({ TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kBodyDataRead, TestLoaderEvent::kReceivedRedirect });
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(GURL("foo://bar/"));
    loader_factory.RunTest(test_helper.get());
...
{
    MockURLLoaderFactory loader_factory(&task_environment_, GetURLLoaderFactoryTestConfig());
    loader_factory.AddEvents({ TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kBodyDataRead, TestLoaderEvent::kReceivedResponse });
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(GURL("foo://bar/"));
    loader_factory.RunTest(test_helper.get());
...
TEST_P(SimpleURLLoaderTest, UnexpectedMessageAfterBodyComplete)
{
    ...
    loader_factory.AddEvents(
        { TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kBodyDataRead, TestLoaderEvent::kBodyBufferClosed, TestLoaderEvent::kReceivedResponse });
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(GURL("foo://bar/"));
    loader_factory.RunTest(test_helper.get());
...
TEST_P(SimpleURLLoaderTest, MoreDataThanExpected)
{
    ...
    MockURLLoaderFactory loader_factory(&task_environment_, GetURLLoaderFactoryTestConfig());
    loader_factory.AddEvents({ TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kBodyDataRead, TestLoaderEvent::kBodyDataRead,
        TestLoaderEvent::kBodyBufferClosed, TestLoaderEvent::kResponseCompleteWithExtraData });
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(GURL("foo://bar/"));
    loader_factory.RunTest(test_helper.get());
...
TEST_P(SimpleURLLoaderTest, DownloadProgressCallbackIncremental)
{
    ...
    test_helper->simple_url_loader()->SetOnDownloadProgressCallback(base::BindLambdaForTesting([&](uint64_t current) { progress.push_back(current); }));
...
TEST_P(SimpleURLLoaderTest, RetryOn5xx)
{
    ...
    struct TestCase {
        ...
        { 1, SimpleURLLoader::RETRY_ON_5XX | SimpleURLLoader::RETRY_ON_NETWORK_CHANGE, 1, true, 2 },
    ...
    for (const auto& test_case : kTestCases) {
        ...
        if (test_case.expect_success) {
            ...
            loader_factory.AddEvents(
                { TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kBodyDataRead, TestLoaderEvent::kBodyBufferClosed, TestLoaderEvent::kResponseComplete });
        }

        std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(GURL(kInitialURL));
        test_helper->simple_url_loader()->SetRetryOptions(test_case.max_retries, test_case.retry_mode);
        loader_factory.RunTest(test_helper.get());
        ...
        EXPECT_EQ(static_cast<size_t>(test_case.expected_num_requests), loader_factory.requested_urls().size());
        for (const auto& url : loader_factory.requested_urls()) {
            EXPECT_EQ(kInitialURL, url);
        }

        if (GetDownloadType() == SimpleLoaderTestHelper::DownloadType::AS_STREAM) {
            EXPECT_EQ(test_case.expected_num_requests - 1, test_helper->download_as_stream_retries());
        }

        EXPECT_EQ(test_case.expected_num_requests - 1, test_helper->simple_url_loader()->GetNumRetries());
    }
}
...
TEST_P(SimpleURLLoaderTest, RetryOnNameNotResolved)
{
    ...
    struct TestCase {
        ...
        { 1, SimpleURLLoader::RETRY_ON_NAME_NOT_RESOLVED | SimpleURLLoader::RETRY_ON_NETWORK_CHANGE, 1, true, 2 },
    ...
    for (const auto& test_case : kTestCases) {
        ...
        if (test_case.expect_success) {
            ...
            EXPECT_EQ(net::ERR_NAME_NOT_RESOLVED, test_helper->simple_url_loader()->NetError());
            ASSERT_TRUE(test_helper->simple_url_loader()->CompletionStatus());
            EXPECT_EQ(net::ERR_NAME_NOT_RESOLVED, test_helper->simple_url_loader()->CompletionStatus()->error_code);
        }
...
TEST_P(SimpleURLLoaderTest, RetryAfterRedirect)
{
    const GURL kInitialURL("foo://bar/initial");
    MockURLLoaderFactory loader_factory(&task_environment_, GetURLLoaderFactoryTestConfig());
    loader_factory.AddEvents({ TestLoaderEvent::kReceivedRedirect, TestLoaderEvent::kReceived501Response });
    loader_factory.AddEvents(
        { TestLoaderEvent::kReceivedRedirect, TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kBodyBufferClosed, TestLoaderEvent::kResponseComplete });
...
TEST_P(SimpleURLLoaderTest, RetryOnNetworkChange)
{
    ...
    const std::vector<std::vector<TestLoaderEvent>> kNetworkChangedEvents = {
        ...
        { TestLoaderEvent::kReceivedResponseNoData, TestLoaderEvent::kResponseCompleteNetworkChanged },
        { TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kBodyBufferClosed, TestLoaderEvent::kResponseCompleteNetworkChanged },
        { TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kResponseCompleteNetworkChanged, TestLoaderEvent::kBodyBufferClosed },
        { TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kBodyDataRead, TestLoaderEvent::kBodyBufferClosed,
            TestLoaderEvent::kResponseCompleteNetworkChanged },
        { TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kBodyDataRead, TestLoaderEvent::kResponseCompleteNetworkChanged,
            TestLoaderEvent::kBodyBufferClosed },
        { TestLoaderEvent::kReceivedRedirect, TestLoaderEvent::kResponseCompleteNetworkChanged },
    };
    ...
    struct TestCase {
        ...
        { 1, SimpleURLLoader::RETRY_ON_NETWORK_CHANGE | SimpleURLLoader::RETRY_ON_5XX, 1, true, 2 },
    ...
    for (const auto& network_events : kNetworkChangedEvents) {
        for (const auto& test_case : kTestCases) {
            ...
            if (test_case.expect_success) {
                ...
                EXPECT_EQ(net::ERR_NETWORK_CHANGED, test_helper->simple_url_loader()->NetError());
                ASSERT_TRUE(test_helper->simple_url_loader()->CompletionStatus());
                EXPECT_EQ(net::ERR_NETWORK_CHANGED, test_helper->simple_url_loader()->CompletionStatus()->error_code);
                EXPECT_FALSE(test_helper->response_body());
            }
    ...
    for (const auto& network_events : kNetworkChangedEvents) {
        ...
        std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(GURL(kInitialURL));
        test_helper->simple_url_loader()->SetRetryOptions(1, SimpleURLLoader::RETRY_ON_NETWORK_CHANGE);
        loader_factory.RunTest(test_helper.get());
...
TEST_P(SimpleURLLoaderTest, UploadLongStringStartReadTwice)
{
    ...
    loader_factory.AddEvents(
        { TestLoaderEvent::kStartReadLongUploadBody, TestLoaderEvent::kStartReadLongUploadBody, TestLoaderEvent::kWaitForLongUploadBodySize,
            TestLoaderEvent::kReadLongUploadBody, TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kResponseComplete, TestLoaderEvent::kBodyBufferClosed });
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(GURL("foo://bar/"), "POST");
    test_helper->simple_url_loader()->AttachStringForUpload(long_string, "text/plain");
    loader_factory.RunTest(test_helper.get());
...
// pipe.
TEST_P(SimpleURLLoaderTest, UploadLongStringReadPartOfUploadBodyBeforeRestartBodyRead)
{
    std::string long_string = GetLongUploadBody();
    MockURLLoaderFactory loader_factory(&task_environment_, GetURLLoaderFactoryTestConfig());
    loader_factory.AddEvents({ TestLoaderEvent::kStartReadLongUploadBody, TestLoaderEvent::kWaitForLongUploadBodySize,
        TestLoaderEvent::kReadFirstByteOfLongUploadBody, TestLoaderEvent::kStartReadLongUploadBody, TestLoaderEvent::kWaitForLongUploadBodySize,
        TestLoaderEvent::kReadLongUploadBody, TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kResponseComplete, TestLoaderEvent::kBodyBufferClosed });
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(GURL("foo://bar/"), "POST");
    test_helper->simple_url_loader()->AttachStringForUpload(long_string, "text/plain");
    loader_factory.RunTest(test_helper.get());
...
{
    GURL url = test_server_.GetURL("/echo");
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL("/server-redirect?" + url.spec()));
    test_helper->StartSimpleLoaderAndWait(url_loader_factory_.get());
...
INSTANTIATE_TEST_SUITE_P(All, SimpleURLLoaderTest,
    testing::Combine(testing::Values(SimpleLoaderTestHelper::DownloadType::TO_STRING, SimpleLoaderTestHelper::DownloadType::TO_FILE,
                         SimpleLoaderTestHelper::DownloadType::TO_TEMP_FILE, SimpleLoaderTestHelper::DownloadType::HEADERS_ONLY,
                         SimpleLoaderTestHelper::DownloadType::AS_STREAM),
        testing::Values(ReadAndDiscardBodyType::kDisabled, ReadAndDiscardBodyType::kEnabled, ReadAndDiscardBodyType::kEnabledButIgnored)),
    ::testing::PrintToStringParamName());

class SimpleURLLoaderFileTest : public SimpleURLLoaderTestBase, public testing::Test {
public:
    ...
    std::unique_ptr<SimpleLoaderTestHelper> CreateHelperForURL(const GURL& url)
    {
        std::unique_ptr<network::ResourceRequest> resource_request = std::make_unique<network::ResourceRequest>();
        resource_request->url = url;
        return std::make_unique<SimpleLoaderTestHelper>(std::move(resource_request), SimpleLoaderTestHelper::DownloadType::TO_FILE);
    }
};
...
TEST_F(SimpleURLLoaderFileTest, FileCreateError)
{
    ...
    EXPECT_EQ(net::ERR_ACCESS_DENIED, test_helper->simple_url_loader()->NetError());
    EXPECT_FALSE(test_helper->simple_url_loader()->CompletionStatus());
    ...
    for (bool body_data_read : { false, true }) {
        for (bool body_buffer_closed : { false, true }) {
            for (bool client_pipe_closed : { false, true }) {
                ...
                MockURLLoaderFactory loader_factory(&task_environment_, { ReadAndDiscardBodyType::kEnabledButIgnored, false });
                std::vector<TestLoaderEvent> events;
                events.push_back(TestLoaderEvent::kReceivedResponse);
                if (body_data_read)
                    events.push_back(TestLoaderEvent::kBodyDataRead);
                ...
                // request will hang.
                loader_factory.RunTest(test_helper.get(), false /* wait_for_completion */);
...
class SimpleURLLoaderStreamTest : public SimpleURLLoaderTestBase, public testing::Test {
public:
    ...
    std::unique_ptr<SimpleLoaderTestHelper> CreateHelperForURL(const GURL& url, const std::string& method = "GET")
    {
        std::unique_ptr<network::ResourceRequest> resource_request = std::make_unique<network::ResourceRequest>();
        resource_request->url = url;
        resource_request->method = method;
        return std::make_unique<SimpleLoaderTestHelper>(std::move(resource_request), SimpleLoaderTestHelper::DownloadType::AS_STREAM);
    }
};
...
{
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL("/response-size?1"));
...
public:
    ...
    void SetUp() override
    {
        SimpleURLLoader::SetTimeoutTickClockForTest(task_environment_.GetMockTickClock());
    }
    ...
    std::unique_ptr<SimpleLoaderTestHelper> CreateHelper()
    {
        ...
        resource_request->enable_upload_progress = true;
        return std::make_unique<SimpleLoaderTestHelper>(std::move(resource_request), SimpleLoaderTestHelper::DownloadType::TO_STRING);
    }
...
{
    MockURLLoaderFactory loader_factory(&task_environment_, { ReadAndDiscardBodyType::kEnabled, false });
    loader_factory.AddEvents(
        { TestLoaderEvent::kAdvanceOneSecond, TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kResponseComplete, TestLoaderEvent::kBodyBufferClosed });
...
{
    MockURLLoaderFactory loader_factory(&task_environment_, { ReadAndDiscardBodyType::kEnabled, false });
    loader_factory.AddEvents({ TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kBodyDataRead, TestLoaderEvent::kAdvanceOneSecond,
    ...
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, std::move(captured_resume));
    // Make sure no pending task results in a crash.
    base::RunLoop().RunUntilIdle();
}
...
{
    MockURLLoaderFactory loader_factory(&task_environment_, { ReadAndDiscardBodyType::kEnabled, false });
    loader_factory.AddEvents({ TestLoaderEvent::kAdvanceOneSecond, TestLoaderEvent::kReceivedRedirect, TestLoaderEvent::kAdvanceOneSecond,
        TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kBodyBufferClosed, TestLoaderEvent::kResponseComplete });
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelper();
    test_helper->simple_url_loader()->SetTimeoutDuration(base::Seconds(2));
...
{
    MockURLLoaderFactory loader_factory(&task_environment_, { ReadAndDiscardBodyType::kEnabled, false });
    loader_factory.AddEvents({ TestLoaderEvent::kAdvanceOneSecond, TestLoaderEvent::kReceived501Response });
    loader_factory.AddEvents(
        { TestLoaderEvent::kAdvanceOneSecond, TestLoaderEvent::kReceivedResponse, TestLoaderEvent::kBodyBufferClosed, TestLoaderEvent::kResponseComplete });
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelper();
    test_helper->simple_url_loader()->SetTimeoutDuration(base::Seconds(2));
    test_helper->simple_url_loader()->SetRetryOptions(1, SimpleURLLoader::RETRY_ON_5XX);
...
TEST_F(SimpleURLLoaderMockTimeTest, TimeoutAfterRetryTriggered)
{
    ...
    loader_factory.AddEvents({ TestLoaderEvent::kAdvanceOneSecond, TestLoaderEvent::kReceived501Response });
    loader_factory.AddEvents(
        { TestLoaderEvent::kAdvanceOneSecond, TestLoaderEvent::kAdvanceOneSecond, TestLoaderEvent::kBodyBufferClosed, TestLoaderEvent::kResponseComplete });
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelper();
    test_helper->simple_url_loader()->SetTimeoutDuration(base::Milliseconds(1900));
    test_helper->simple_url_loader()->SetRetryOptions(1, SimpleURLLoader::RETRY_ON_5XX);
...
TEST_P(SimpleURLLoaderTest, OnUploadProgressCallback)
{
    ...
    uint64_t progress = 0;
    test_helper->simple_url_loader()->SetOnUploadProgressCallback(base::BindLambdaForTesting([&](uint64_t current, uint64_t total) {
...
TEST_P(SimpleURLLoaderTest, OnDownloadProgressCallback)
{
    ...
    std::unique_ptr<SimpleLoaderTestHelper> test_helper = CreateHelperForURL(test_server_.GetURL(base::StringPrintf("/response-size?%u", kResponseSize)));
    uint64_t progress = 0;
    test_helper->simple_url_loader()->SetOnDownloadProgressCallback(base::BindLambdaForTesting([&](uint64_t current) {
