...
mojo::PendingRemote<network::mojom::URLLoaderFactory> CreateUnconnectedURLLoaderFactory()
{
...
public:
    MojoCertVerifierTest()
        ...
        , mojo_cert_verifier_(cv_service_receiver_.BindNewPipeAndPassRemote(), cv_service_client_.BindNewPipeAndPassReceiver(),
              CreateUnconnectedURLLoaderFactory(), base::BindRepeating(&MojoCertVerifierTest::ReconnectCb, base::Unretained(this)))
    {
    ...
    public:
        explicit DummyCVService(MojoCertVerifierTest* test)
            : test_(test)
        {
        }
        void Verify(const net::CertVerifier::RequestParams& params, const net::NetLogSource& net_log_source,
            mojo::PendingRemote<mojom::CertVerifierRequest> cert_verifier_request) override
        {
        ...
        void EnableNetworkAccess(mojo::PendingRemote<network::mojom::URLLoaderFactory> url_loader_factory,
            mojo::PendingRemote<mojom::URLLoaderFactoryConnector> reconnector) override
        {
            reconnector_.Bind(std::move(reconnector));
        }

        const net::CertVerifier::Config* config() const
        {
            return &config_;
        }
    ...
    MojoCertVerifier* mojo_cert_verifier()
    {
        return &mojo_cert_verifier_;
    }

    DummyCVService* dummy_cv_service()
    {
        return &dummy_cv_service_;
    }
    ...
    void SimulateCVServiceDisconnect()
    {
        cv_service_receiver_.reset();
    }

    void ReconnectCb(mojo::PendingReceiver<network::mojom::URLLoaderFactory> mojo_cert_verifier)
    {
        if (reconnect_cb_)
    ...
    void ExpectReceivedNetlogSource(const net::CertVerifier::RequestParams& params, net::NetLogSourceType type, uint32_t id, base::TimeTicks start_time)
    {
...
private:
    std::map<net::CertVerifier::RequestParams, mojo::Remote<mojom::CertVerifierRequest>> received_requests_;
    std::map<net::CertVerifier::RequestParams, net::NetLogSource> received_netlogsources_;
...
TEST_F(MojoCertVerifierTest, BasicVerify)
{
    ...
    auto net_log(net::NetLogWithSource::Make(net::NetLog::Get(), net::NetLogSourceType::CERT_VERIFIER_JOB));

    int net_error = mojo_cert_verifier()->Verify(dummy_params, cert_verify_result.get(), test_cb.callback(), &request, net_log);
    ...
    ExpectReceivedNetlogSource(dummy_params, net_log.source().type, net_log.source().id, net_log.source().start_time);
}
...
TEST_F(MojoCertVerifierTest, BasicVerifyOtherNetLogSourceType)
{
    ...
    net::TestCompletionCallback test_cb;
    std::unique_ptr<net::CertVerifier::Request> request;
    auto net_log(net::NetLogWithSource::Make(net::NetLog::Get(), net::NetLogSourceType::SOCKET));
...
TEST_F(MojoCertVerifierTest, CVRequestDeletionCausesDisconnect)
{
    ...
    int net_error = mojo_cert_verifier()->Verify(dummy_params, cert_verify_result.get(), std::move(verify_cb), &request, net_log);
    ASSERT_EQ(net_error, net::ERR_IO_PENDING);
...
TEST_F(MojoCertVerifierTest, ReconnectorCallsCb)
{
    ...
    SetReconnectCb(base::BindLambdaForTesting([&](mojo::PendingReceiver<network::mojom::URLLoaderFactory> receiver) { run_loop.Quit(); }));

    mojo::PendingRemote<network::mojom::URLLoaderFactory> dummy_url_loader_factory_remote;
    ...
    // on construction.
    dummy_cv_service()->reconnector()->CreateURLLoaderFactory(dummy_url_loader_factory_remote.InitWithNewPipeAndPassReceiver());
