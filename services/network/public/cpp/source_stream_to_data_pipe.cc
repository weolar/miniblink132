...
SourceStreamToDataPipe::SourceStreamToDataPipe(std::unique_ptr<net::SourceStream> source, mojo::ScopedDataPipeProducerHandle dest)
    : source_(std::move(source))
    , dest_(std::move(dest))
    , writable_handle_watcher_(FROM_HERE, mojo::SimpleWatcher::ArmingPolicy::MANUAL, base::SequencedTaskRunner::GetCurrentDefault())
{
    writable_handle_watcher_.Watch(
        dest_.get(), MOJO_HANDLE_SIGNAL_WRITABLE, base::BindRepeating(&SourceStreamToDataPipe::OnDataPipeWritable, base::Unretained(this)));
}

SourceStreamToDataPipe::~SourceStreamToDataPipe() = default;

void SourceStreamToDataPipe::Start(base::OnceCallback<void(int)> completion_callback)
{
...
void SourceStreamToDataPipe::ReadMore()
{
    ...
    MojoResult mojo_result = network::NetToMojoPendingBuffer::BeginWrite(&dest_, &pending_write_);
    switch (mojo_result) {
    case MOJO_RESULT_OK:
        break;
    ...
    int result = source_->Read(buffer.get(), num_bytes, base::BindOnce(&SourceStreamToDataPipe::DidRead, weak_factory_.GetWeakPtr()));
...
void SourceStreamToDataPipe::DidRead(int result)
{
    ...
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&SourceStreamToDataPipe::ReadMore, weak_factory_.GetWeakPtr()));
}
