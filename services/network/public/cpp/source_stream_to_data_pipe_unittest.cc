...
struct SourceStreamToDataPipeTestParam {
    SourceStreamToDataPipeTestParam(uint32_t pipe_capacity, net::MockSourceStream::Mode read_mode, ReadResultType read_result_type)
...
public:
    DummyPendingSourceStream()
        : net::SourceStream(SourceStream::TYPE_NONE)
    {
    }
    ...
    int Read(net::IOBuffer* dest_buffer, int buffer_size, net::CompletionOnceCallback callback) override
    {
...
class SourceStreamToDataPipeTest : public ::testing::TestWithParam<SourceStreamToDataPipeTestParam> {
protected:
    ...
    void Init()
    {
        ...
        const MojoCreateDataPipeOptions data_pipe_options { sizeof(MojoCreateDataPipeOptions), MOJO_CREATE_DATA_PIPE_FLAG_NONE, 1, GetParam().pipe_capacity };
        mojo::ScopedDataPipeProducerHandle producer_end;
        CHECK_EQ(MOJO_RESULT_OK, mojo::CreateDataPipe(&data_pipe_options, producer_end, consumer_end_));

        adapter_ = std::make_unique<SourceStreamToDataPipe>(std::move(source), std::move(producer_end));
    }

    base::OnceCallback<void(int)> callback()
    {
        return base::BindOnce(&SourceStreamToDataPipeTest::FinishedReading, base::Unretained(this));
    }
    ...
    int ReadPipe(std::string* output)
    {
        ...
        while (result == MOJO_RESULT_OK || result == MOJO_RESULT_SHOULD_WAIT) {
            ...
            size_t read_size = 0;
            result = consumer_end().ReadData(MOJO_READ_DATA_FLAG_NONE, base::as_writable_byte_span(buffer), read_size);
    ...
    SourceStreamToDataPipe* adapter()
    {
        return adapter_.get();
    }
    ...
    mojo::DataPipeConsumerHandle consumer_end()
    {
        return consumer_end_.get();
    }

    void CloseConsumerHandle()
    {
        consumer_end_.reset();
    }
    void RunUntilIdle()
...
private:
    void FinishedReading(int result)
    {
...
INSTANTIATE_TEST_SUITE_P(SourceStreamToDataPipeTests, SourceStreamToDataPipeTest,
    ::testing::Values(SourceStreamToDataPipeTestParam(kBigPipeCapacity, net::MockSourceStream::SYNC, ReadResultType::EVERYTHING_AT_ONCE),
        SourceStreamToDataPipeTestParam(kBigPipeCapacity, net::MockSourceStream::ASYNC, ReadResultType::EVERYTHING_AT_ONCE),
        SourceStreamToDataPipeTestParam(kBigPipeCapacity, net::MockSourceStream::SYNC, ReadResultType::ONE_BYTE_AT_A_TIME),
        SourceStreamToDataPipeTestParam(kSmallPipeCapacity, net::MockSourceStream::SYNC, ReadResultType::ONE_BYTE_AT_A_TIME),
        SourceStreamToDataPipeTestParam(kBigPipeCapacity, net::MockSourceStream::ASYNC, ReadResultType::ONE_BYTE_AT_A_TIME),
        SourceStreamToDataPipeTestParam(kSmallPipeCapacity, net::MockSourceStream::ASYNC, ReadResultType::ONE_BYTE_AT_A_TIME)));
...
TEST_P(SourceStreamToDataPipeTest, Simple)
{
    ...
    Init();
    source()->AddReadResult(message, sizeof(message) - 1, net::OK, GetParam().mode);
    source()->AddReadResult(nullptr, 0, net::OK, GetParam().mode);
    adapter()->Start(callback());
...
TEST(SourceStreamToDataPipeCallbackTest, CompletionCallbackAfterDestructed)
{
    ...
    std::unique_ptr<DummyPendingSourceStream> source = std::make_unique<DummyPendingSourceStream>();
    DummyPendingSourceStream* source_ptr = source.get();
    const MojoCreateDataPipeOptions data_pipe_options { sizeof(MojoCreateDataPipeOptions), MOJO_CREATE_DATA_PIPE_FLAG_NONE, 1, 1 };
    mojo::ScopedDataPipeProducerHandle producer_end;
    mojo::ScopedDataPipeConsumerHandle consumer_end;
    CHECK_EQ(MOJO_RESULT_OK, mojo::CreateDataPipe(&data_pipe_options, producer_end, consumer_end));

    std::unique_ptr<SourceStreamToDataPipe> adapter = std::make_unique<SourceStreamToDataPipe>(std::move(source), std::move(producer_end));
    bool callback_called = false;
    adapter->Start(base::BindLambdaForTesting([&](int result) { callback_called = true; }));
