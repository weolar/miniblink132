...
bool PatternCanMatchIpV4Host(const std::string& hostname_pattern)
{
    ...
    std::vector<std::string_view> components = base::SplitStringPiece(hostname_pattern, ".", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);
    ...
    return ip_address.AssignFromIPLiteral(wildcards_replaced) && ip_address.IsIPv4();
}
...
bool IsValidWildcardPattern(const std::string& hostname_pattern)
{
    ...
    // hostname is valid.
    url::SchemeHostPort scheme_host_port(GURL(base::StringPrintf("http://%s:80", wildcards_replaced.c_str())));
    ...
    size_t registry_length = net::registry_controlled_domains::PermissiveGetHostRegistryLength(
        hostname_pattern, net::registry_controlled_domains::INCLUDE_UNKNOWN_REGISTRIES, net::registry_controlled_domains::INCLUDE_PRIVATE_REGISTRIES);
    ...
    // invalid.
    if (hostname_pattern.find('*', hostname_pattern.size() - registry_length) != std::string::npos)
        return false;
    ...
    std::string host_before_registrar = hostname_pattern.substr(0, hostname_pattern.size() - registry_length);
    std::vector<std::string> components = base::SplitString(host_before_registrar, ".", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
std::string CanonicalizePatternComponents(const std::string& hostname_pattern)
{
    ...
    for (size_t current = 0; current < hostname_pattern.length(); current++) {
        ...
        if (hostname_pattern.substr(begin, current_len) == "*"
            || !url::CanonicalizeHostSubstring(hostname_pattern.data(), url::Component(base::checked_cast<int>(begin), current_len), &canon_output)) {
            // Failed to canonicalize this component; append as-is.
            canon_output.Append(hostname_pattern.substr(begin, current_len).data(), current_len);
        }
...
std::vector<std::string> CanonicalizeAllowlist(const std::vector<std::string>& origins_and_patterns_list, std::vector<std::string>* rejected_patterns)
{
    ...
    for (const std::string& origin_or_pattern : origins_and_patterns_list) {
        if (origin_or_pattern.find("*") != std::string::npos) {
            if (IsValidWildcardPattern(origin_or_pattern)) {
                std::string canonicalized_pattern = CanonicalizePatternComponents(origin_or_pattern);
                if (!canonicalized_pattern.empty()) {
                    result.push_back(canonicalized_pattern);
                    continue;
                }
            }
            LOG(ERROR) << "Allowlisted secure origin pattern " << origin_or_pattern << " is not valid; ignoring.";
...
std::vector<std::string> ParseSecureOriginAllowlist(const std::string& origins_str, std::vector<std::string>* rejected_patterns = nullptr)
{
    std::vector<std::string> origin_patterns
        = CanonicalizeAllowlist(base::SplitString(origins_str, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL), rejected_patterns);
...
std::vector<std::string> ParseSecureOriginAllowlistFromCmdline()
{
    ...
    if (command_line.HasSwitch(switches::kUnsafelyTreatInsecureOriginAsSecure)) {
        origins_str = command_line.GetSwitchValueASCII(switches::kUnsafelyTreatInsecureOriginAsSecure);
    }

    std::vector<std::string> origin_patterns = ParseSecureOriginAllowlist(origins_str);
...
bool IsAllowlisted(const std::vector<std::string>& allowlist, const url::Origin& origin)
{
...
void SecureOriginAllowlist::SetAuxiliaryAllowlist(const std::string& auxiliary_allowlist, std::vector<std::string>* rejected_patterns)
{
    std::vector<std::string> parsed_list = ParseSecureOriginAllowlist(auxiliary_allowlist, rejected_patterns);
...
bool SecureOriginAllowlist::IsOriginAllowlisted(const url::Origin& origin)
{
    base::AutoLock lock(lock_);
    ParseCmdlineIfNeeded();
    return IsAllowlisted(cmdline_allowlist_, origin) || IsAllowlisted(auxiliary_allowlist_, origin);
}

// static
std::vector<std::string> SecureOriginAllowlist::CanonicalizeAllowlistForTesting(
    const std::vector<std::string>& allowlist, std::vector<std::string>* rejected_patterns)
{
