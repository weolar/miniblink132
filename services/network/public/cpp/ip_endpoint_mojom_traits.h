...
namespace mojo {
template <> struct COMPONENT_EXPORT(NETWORK_CPP_IP_ADDRESS) StructTraits<network::mojom::IPEndPointDataView, net::IPEndPoint> {
    static const net::IPAddress& address(const net::IPEndPoint& obj)
    {
        return obj.address();
    }
    static uint16_t port(const net::IPEndPoint& obj)
    {
        return obj.port();
    }

    static bool Read(network::mojom::IPEndPointDataView obj, net::IPEndPoint* out);
};
