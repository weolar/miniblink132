...
TEST(DevToolsObserverUtilTest, ExtractURLResponseHead)
{
    ...
    head.headers = base::MakeRefCounted<net::HttpResponseHeaders>("HTTP/1.1 200 OK\0");
    head.mime_type = "mime_type";
    head.load_timing.first_early_hints_time = base::TimeTicks() + base::Microseconds(11);
    head.cert_status = 12;
    ...
    head.service_worker_router_info = mojom::ServiceWorkerRouterInfo::New(
        /*rule_id_matched*/ 1, mojom::ServiceWorkerRouterSourceType::kNetwork, mojom::ServiceWorkerRouterSourceType::kNetwork, 1, base::TimeDelta::Min(),
    ...
    EXPECT_EQ(head_info->load_timing.first_early_hints_time, head.load_timing.first_early_hints_time);
    ...
    EXPECT_EQ(head_info->was_fetched_via_service_worker, head.was_fetched_via_service_worker);
    ...
    EXPECT_EQ(head_info->service_worker_response_source, head.service_worker_response_source);
    ASSERT_TRUE(head_info->service_worker_router_info);
    EXPECT_EQ(*head_info->service_worker_router_info, *head.service_worker_router_info);
...
TEST(DevToolsObserverUtilTest, ExtractResourceRequest)
{
    ...
    request.referrer_policy = net::ReferrerPolicy::CLEAR_ON_TRANSITION_CROSS_ORIGIN;
    ...
    EXPECT_EQ(*request_info->trust_token_params, request.trust_token_params.value());
}
