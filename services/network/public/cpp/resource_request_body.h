...
class COMPONENT_EXPORT(NETWORK_CPP_BASE) ResourceRequestBody : public base::RefCountedThreadSafe<ResourceRequestBody> {
public:
    ...
    static scoped_refptr<ResourceRequestBody> CreateFromBytes(const char* bytes, size_t length);

    void AppendBytes(std::vector<uint8_t> bytes);
    void AppendBytes(const char* bytes, int bytes_len);
    void AppendFileRange(const base::FilePath& file_path, uint64_t offset, uint64_t length, const base::Time& expected_modification_time);

    void AppendDataPipe(mojo::PendingRemote<mojom::DataPipeGetter> data_pipe_getter);
    ...
    void SetToChunkedDataPipe(mojo::PendingRemote<mojom::ChunkedDataPipeGetter> chunked_data_pipe_getter, ReadOnlyOnce read_only_once);
    ...
    void SetToReadOnceStream(mojo::PendingRemote<mojom::ChunkedDataPipeGetter> chunked_data_pipe_getter);
    ...
    const std::vector<DataElement>* elements() const
    {
        return &elements_;
    }
    std::vector<DataElement>* elements_mutable()
    {
        return &elements_;
    }
    ...
    void set_identifier(int64_t id)
    {
        identifier_ = id;
    }
    ...
    {
        return contains_sensitive_info_;
    }

private:
    friend class base::RefCountedThreadSafe<ResourceRequestBody>;
    friend struct mojo::StructTraits<blink::mojom::FetchAPIRequestBodyDataView, scoped_refptr<network::ResourceRequestBody>>;
    friend struct mojo::StructTraits<network::mojom::URLRequestBodyDataView, scoped_refptr<network::ResourceRequestBody>>;
    ~ResourceRequestBody();
