...
template <> struct UnionTraits<network::mojom::TransferableDirectoryDataView, network::TransferableDirectory> {
    static network::mojom::TransferableDirectoryDataView::Tag GetTag(network::TransferableDirectory& input);
    static base::FilePath path(const network::TransferableDirectory& value);
    static mojo::PlatformHandle handle_for_ipc(network::TransferableDirectory& value);
    static bool Read(network::mojom::TransferableDirectoryDataView in, network::TransferableDirectory* out);
};
