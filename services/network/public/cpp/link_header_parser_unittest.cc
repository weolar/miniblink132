...
TEST(LinkHeaderParserTest, NoLinkHeader)
{
    ...
    std::vector<mojom::LinkHeaderPtr> parsed_headers = ParseLinkHeaders(*headers, kBaseUrl);
...
TEST(LinkHeaderParserTest, UndefinedAttribute)
{
    ...
    headers->AddHeader("link", "</style.css>; rel=preload; as=style; unknownattr");
...
TEST(LinkHeaderParserTest, CrossOriginAttribute)
{
    ...
    headers->AddHeader("link", "<https://cross.example.com/>; rel=preload; crossorigin");
    headers->AddHeader("link", "<https://cross.example.com/>; rel=preload; crossorigin=anonymous");
    headers->AddHeader("link", "<https://cross.example.com/>; rel=preload; crossorigin=use-credentials");

    std::vector<mojom::LinkHeaderPtr> parsed_headers = ParseLinkHeaders(*headers, kBaseUrl);
    ASSERT_EQ(parsed_headers.size(), 4UL);
    EXPECT_EQ(parsed_headers[0]->cross_origin, mojom::CrossOriginAttribute::kUnspecified);
    EXPECT_EQ(parsed_headers[1]->cross_origin, mojom::CrossOriginAttribute::kAnonymous);
    EXPECT_EQ(parsed_headers[2]->cross_origin, mojom::CrossOriginAttribute::kAnonymous);
    EXPECT_EQ(parsed_headers[3]->cross_origin, mojom::CrossOriginAttribute::kUseCredentials);
}
...
TEST(LinkHeaderParserTest, FetchPriorityAttribute)
{
    ...
    EXPECT_EQ(parsed_headers[0]->fetch_priority, mojom::FetchPriorityAttribute::kAuto);
    EXPECT_EQ(parsed_headers[1]->fetch_priority, mojom::FetchPriorityAttribute::kAuto);
    EXPECT_EQ(parsed_headers[2]->fetch_priority, mojom::FetchPriorityAttribute::kHigh);
    EXPECT_EQ(parsed_headers[3]->fetch_priority, mojom::FetchPriorityAttribute::kLow);
    EXPECT_EQ(parsed_headers[4]->fetch_priority, mojom::FetchPriorityAttribute::kAuto);
}
...
TEST(LinkHeaderParserTest, TwoHeaders)
{
    ...
    EXPECT_EQ(parsed_headers[1]->href, GURL("https://cross.example.com/font.woff2"));
...
{
    auto headers = base::MakeRefCounted<net::HttpResponseHeaders>("HTTP/1.1 200 OK\n");
    headers->AddHeader("link", "</image.jpg>; REL=preload; as=IMAGE; fetchpriority=HIGH");
    headers->AddHeader("link",
        "<https://cross.example.com/font.woff2>; rel=PRELOAD; "
    ...
    EXPECT_EQ(parsed_headers[0]->rel, mojom::LinkRelAttribute::kPreload);
    EXPECT_EQ(parsed_headers[0]->as, mojom::LinkAsAttribute::kImage);
    EXPECT_EQ(parsed_headers[0]->fetch_priority, mojom::FetchPriorityAttribute::kHigh);

    EXPECT_EQ(parsed_headers[1]->href, GURL("https://cross.example.com/font.woff2"));
    EXPECT_EQ(parsed_headers[1]->rel, mojom::LinkRelAttribute::kPreload);
    EXPECT_EQ(parsed_headers[1]->as, mojom::LinkAsAttribute::kFont);
    EXPECT_EQ(parsed_headers[1]->cross_origin, mojom::CrossOriginAttribute::kUseCredentials);
    EXPECT_EQ(parsed_headers[1]->mime_type, "font/woff2");
    EXPECT_EQ(parsed_headers[1]->fetch_priority, mojom::FetchPriorityAttribute::kLow);
}
