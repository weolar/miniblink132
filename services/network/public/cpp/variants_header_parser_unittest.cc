...
TEST(VariantsHeaderTest, ParseVariantsHeader)
{
    ...
    EXPECT_EQ(result.value()[0]->available_values, (std::vector<std::string>) { "en" });
    ...
    result = ParseVariantsHeaders("\"accept-language=(en)\", \"accept-encoding=(gzip)\"");
    ...
    ASSERT_EQ(result.value().size(), 1UL);
    EXPECT_EQ(result.value()[0]->name, "accept-language");
    EXPECT_EQ(result.value()[0]->available_values, std::vector<std::string>({ "en", "de", "zh" }));
    ...
    ASSERT_EQ(result.value().size(), 1UL);
    EXPECT_EQ(result.value()[0]->name, "accept-language");
    EXPECT_EQ(result.value()[0]->available_values, std::vector<std::string>({ "de" }));
}
