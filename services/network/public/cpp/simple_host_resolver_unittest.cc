...
net::IPEndPoint CreateExpectedEndPoint(std::string_view address, uint16_t port)
{
...
std::string CreateMappingRules(std::vector<std::pair<std::string_view, std::string_view>> host_ip_address_pairs)
{
    std::vector<std::string> map_rules;
    for (auto [host, ip_address] : host_ip_address_pairs) {
        map_rules.push_back(base::StringPrintf("MAP %s %s", host.data(), ip_address.data()));
    }
...
public:
    ...
    static std::unique_ptr<MockNetworkContext> CreateNetworkContext(std::string_view host_mapping_rules)
    {
        return std::make_unique<MockNetworkContext>(
            net::HostResolver::CreateStandaloneResolver(net::NetLog::Get(), /*options=*/std::nullopt, host_mapping_rules,
    ...
    void ResolveHost(network::mojom::HostResolverHostPtr host, const ::net::NetworkAnonymizationKey& network_anonymization_key,
        network::mojom::ResolveHostParametersPtr optional_parameters, ::mojo::PendingRemote<network::mojom::ResolveHostClient> pending_response_client) override
    {
        ...
        ResolveHostImpl(std::move(host), network_anonymization_key, std::move(optional_parameters), std::move(pending_response_client));
    }
...
protected:
    base::test::TaskEnvironment task_environment_ { base::test::TaskEnvironment::TimeSource::MOCK_TIME, base::test::TaskEnvironment::MainThreadType::IO };
};
...
using ResolveHostFuture
    = base::test::TestFuture<int, const net::ResolveErrorInfo&, const std::optional<net::AddressList>&, const std::optional<net::HostResolverEndpointResults>&>;

TEST_F(SimpleHostResolverTest, ResolveFourAddresses)
{
    auto network_context
        = MockNetworkContext::CreateNetworkContext(CreateMappingRules({ { "example.test", "98.76.54.32" }, { "another-example.test", "11.22.33.44" } }));
    ...
    std::vector<std::pair<HostResolverRequest, HostResolverResult>> test_cases = {
        { { .host_port_pair = net::HostPortPair("example.test", 160) }, { .result = net::OK, .resolved_address = CreateExpectedEndPoint("98.76.54.32", 160) } },
        { { .host_port_pair = net::HostPortPair("unknown", 15) }, { .result = net::ERR_NAME_NOT_RESOLVED } },
        { { .host_port_pair = net::HostPortPair("another-example.test", 85) },
            { .result = net::OK, .resolved_address = CreateExpectedEndPoint("11.22.33.44", 85) } },
        { { .host_port_pair = net::HostPortPair("example.test", 3800), .reset_client = true }, { .result = net::ERR_FAILED } }
    };

    EXPECT_EQ(simple_resolver->GetNumOutstandingRequestsForTesting(), 0U);

    std::vector<std::pair<std::unique_ptr<ResolveHostFuture>, HostResolverResult>> futures;
    for (auto [request, result] : test_cases) {
        ...
        simple_resolver->ResolveHost(network::mojom::HostResolverHost::NewHostPortPair(request.host_port_pair), net::NetworkAnonymizationKey(),
    ...
    for (const auto& [future, resolver_result] : futures) {
        const auto& [result, resolve_error_info, resolved_addresses, endpoint_results_with_metadata] = future->Get();
        EXPECT_EQ(result, resolver_result.result);
        if (!resolver_result.resolved_address) {
            EXPECT_FALSE(resolved_addresses);
        } else {
            EXPECT_THAT(resolved_addresses->endpoints(), testing::ElementsAre(*resolver_result.resolved_address));
        }
    }
