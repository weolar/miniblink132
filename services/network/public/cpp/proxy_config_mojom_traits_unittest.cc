...
bool TestProxyConfigRoundTrip(net::ProxyConfigWithAnnotation& original_config)
{
    ...
    EXPECT_TRUE(mojo::test::SerializeAndDeserialize<mojom::ProxyConfigWithAnnotation>(original_config, copied_config));

    return original_config.value().Equals(copied_config.value()) && original_config.traffic_annotation() == copied_config.traffic_annotation();
}

TEST(ProxyConfigTraitsTest, AutoDetect)
{
    net::ProxyConfigWithAnnotation proxy_config(net::ProxyConfig::CreateAutoDetect(), TRAFFIC_ANNOTATION_FOR_TESTS);
    EXPECT_TRUE(TestProxyConfigRoundTrip(proxy_config));
}

TEST(ProxyConfigTraitsTest, Direct)
{
    net::ProxyConfigWithAnnotation proxy_config = net::ProxyConfigWithAnnotation::CreateDirect();
    EXPECT_TRUE(TestProxyConfigRoundTrip(proxy_config));
}
...
TEST(ProxyConfigTraitsTest, FromSystem)
{
    ...
    net::ProxyConfigWithAnnotation proxy_config(base_config, TRAFFIC_ANNOTATION_FOR_TESTS);
    EXPECT_TRUE(TestProxyConfigRoundTrip(proxy_config));
}

TEST(ProxyConfigTraitsTest, CustomPacURL)
{
    net::ProxyConfigWithAnnotation proxy_config(net::ProxyConfig::CreateFromCustomPacURL(GURL("http://foo/")), TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST(ProxyConfigTraitsTest, MultiProxy)
{
    ...
    proxy_config.proxy_rules().single_proxies.AddProxyChain(net::ProxyChain(
        { ProxyUriToProxyServer("foo:333", net::ProxyServer::SCHEME_HTTPS), ProxyUriToProxyServer("foo:444", net::ProxyServer::SCHEME_HTTPS) }));
    net::ProxyConfigWithAnnotation annotated_config(proxy_config, TRAFFIC_ANNOTATION_FOR_TESTS);
...
TEST(ProxyConfigTraitsTest, BypassRules)
{
    ...
    for (const char* test_case : kTestCases) {
        ...
        annotated_config = net::ProxyConfigWithAnnotation(proxy_config, TRAFFIC_ANNOTATION_FOR_TESTS);
        EXPECT_TRUE(TestProxyConfigRoundTrip(annotated_config));
    }
}
