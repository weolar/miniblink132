...
BASE_FEATURE(kSimpleURLLoaderUseReadAndDiscardBodyOption, "SimpleURLLoaderUseReadAndDiscardBodyOption", base::FEATURE_DISABLED_BY_DEFAULT);
...
// optional<string>.
void GetFromUniquePtrToOptional(SimpleURLLoader::BodyAsStringCallback body_as_string_callback, std::unique_ptr<std::string> response_body)
{
    std::move(body_as_string_callback).Run(response_body ? std::make_optional<std::string>(std::move(*response_body)) : std::nullopt);
}
...
public:
    StringUploadDataPipeGetter(std::string upload_string, const base::Location& url_loader_created_from)
        : upload_string_(std::move(upload_string))
    {
    }

    StringUploadDataPipeGetter(const StringUploadDataPipeGetter&) = delete;
    StringUploadDataPipeGetter& operator=(const StringUploadDataPipeGetter&) = delete;
...
private:
    ...
    void Read(mojo::ScopedDataPipeProducerHandle pipe, ReadCallback callback) override
    {
        ...
        handle_watcher_
            = std::make_unique<mojo::SimpleWatcher>(FROM_HERE, mojo::SimpleWatcher::ArmingPolicy::MANUAL, base::SequencedTaskRunner::GetCurrentDefault());
        handle_watcher_->Watch(upload_body_pipe_.get(),
            // Don't bother watching for close - rely on read pipes for errors.
            MOJO_HANDLE_SIGNAL_WRITABLE, MOJO_WATCH_CONDITION_SATISFIED,
            base::BindRepeating(&StringUploadDataPipeGetter::MojoReadyCallback, base::Unretained(this)));
        WriteData();
    }
    ...
    void MojoReadyCallback(MojoResult result, const mojo::HandleSignalsState& state)
    {
        TRACE_EVENT("toplevel", "SimpleURLLoader_BodyReader mojo callback", [&](perfetto::EventContext& ctx) {
            ctx.event()->set_source_location_iid(base::trace_event::InternedSourceLocation::Get(&ctx, url_loader_created_from_));
        });
        WriteData();
    }
    ...
    void WriteData()
    {
        ...
        while (true) {
            ...
            MojoResult result = upload_body_pipe_->WriteData(bytes, MOJO_WRITE_DATA_FLAG_NONE, actually_written_bytes);
...
class SimpleURLLoaderImpl : public SimpleURLLoader, public mojom::URLLoaderClient {
public:
    SimpleURLLoaderImpl(
        std::unique_ptr<ResourceRequest> resource_request, const net::NetworkTrafficAnnotationTag& annotation_tag, const base::Location& created_from);
    ...
    void DownloadToString(mojom::URLLoaderFactory* url_loader_factory, BodyAsStringCallbackDeprecated body_as_string_callback, size_t max_body_size) override;
    void DownloadToString(mojom::URLLoaderFactory* url_loader_factory, BodyAsStringCallback body_as_string_callback, size_t max_body_size) override;
    void DownloadToStringOfUnboundedSizeUntilCrashAndDie(
        mojom::URLLoaderFactory* url_loader_factory, BodyAsStringCallbackDeprecated body_as_string_callback) override;
    void DownloadToStringOfUnboundedSizeUntilCrashAndDie(mojom::URLLoaderFactory* url_loader_factory, BodyAsStringCallback body_as_string_callback) override;
    void DownloadHeadersOnly(mojom::URLLoaderFactory* url_loader_factory, HeadersOnlyCallback headers_only_callback) override;
    void DownloadToFile(mojom::URLLoaderFactory* url_loader_factory, DownloadToFileCompleteCallback download_to_file_complete_callback,
        const base::FilePath& file_path, int64_t max_body_size) override;
    void DownloadToTempFile(
        mojom::URLLoaderFactory* url_loader_factory, DownloadToFileCompleteCallback download_to_file_complete_callback, int64_t max_body_size) override;
    void DownloadAsStream(mojom::URLLoaderFactory* url_loader_factory, SimpleURLLoaderStreamConsumer* stream_consumer) override;
    void SetOnRedirectCallback(const OnRedirectCallback& on_redirect_callback) override;
    void SetOnResponseStartedCallback(OnResponseStartedCallback on_response_started_callback) override;
    void SetOnUploadProgressCallback(UploadProgressCallback on_upload_progress_callback) override;
    void SetOnDownloadProgressCallback(DownloadProgressCallback on_download_progress_callback) override;
    ...
    void AttachStringForUpload(std::string_view upload_data, std::string_view upload_content_type) override;
    void AttachStringForUpload(std::string_view upload_data) override;
    void AttachStringForUpload(const char* upload_data, std::string_view upload_content_type) override;
    void AttachStringForUpload(const char* upload_data) override;
    void AttachStringForUpload(std::string&& upload_data, std::string_view upload_content_type) override;
    void AttachStringForUpload(std::string&& upload_data) override;
    void AttachFileForUpload(const base::FilePath& upload_file_path, const std::string& upload_content_type, uint64_t offset = 0,
        uint64_t length = std::numeric_limits<uint64_t>::max()) override;
    void AttachFileForUpload(const base::FilePath& upload_file_path, uint64_t offset = 0, uint64_t length = std::numeric_limits<uint64_t>::max()) override;
    ...
    const std::optional<URLLoaderCompletionStatus>& CompletionStatus() const override;
    ...
    const base::Location& created_from() const
    {
...
private:
    ...
    // bodies.
    void AttachStringForUploadInternal(std::string_view upload_data, base::optional_ref<std::string_view> upload_content_type);
    void AttachStringForUploadInternal(std::string&& upload_data, base::optional_ref<std::string_view> upload_content_type);

    void AttachFileForUpload(const base::FilePath& upload_file_path, const std::string* const upload_content_type, uint64_t offset = 0,
    ...
    void OnReceiveResponse(
        mojom::URLResponseHeadPtr response_head, mojo::ScopedDataPipeConsumerHandle body, std::optional<mojo_base::BigBuffer> cached_metadata) override;
    void OnReceiveRedirect(const net::RedirectInfo& redirect_info, mojom::URLResponseHeadPtr response_head) override;
...
public:
    ...
    BodyReader(Delegate* delegate, int64_t max_body_size, const base::Location& url_loader_created_from)
        : delegate_(delegate)
    ...
    void Start(mojo::ScopedDataPipeConsumerHandle body_data_pipe)
    {
        ...
        handle_watcher_
            = std::make_unique<mojo::SimpleWatcher>(FROM_HERE, mojo::SimpleWatcher::ArmingPolicy::MANUAL, base::SequencedTaskRunner::GetCurrentDefault());
        handle_watcher_->Watch(body_data_pipe_.get(), MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED, MOJO_WATCH_CONDITION_SATISFIED,
            base::BindRepeating(&BodyReader::MojoReadyCallback, base::Unretained(this)));
        ReadData();
    }
...
private:
    ...
    void ReadData()
    {
        while (true) {
            ...
            MojoResult result = body_data_pipe_->BeginReadData(MOJO_READ_DATA_FLAG_NONE, body);
            if (result == MOJO_RESULT_SHOULD_WAIT) {
                handle_watcher_->ArmOrNotify();
                return;
            }
            ...
            size_t limit = base::saturated_cast<size_t>(max_body_size_ - total_bytes_read_);
            if (copy_size > limit) {
                copy_size = limit;
            }
            ...
            base::WeakPtr<BodyReader> weak_this = this->weak_ptr_factory_.GetWeakPtr();
            ...
            // in progress.
            mojo::ScopedDataPipeConsumerHandle body_data_pipe = std::move(body_data_pipe_);
...
public:
    ...
    BodyHandler(SimpleURLLoaderImpl* simple_url_loader, bool want_download_progress)
    ...
    virtual bool RequiresBodyDataPipe()
    {
        return true;
    }
    ...
    // file fails.
    virtual void OnStartLoadingResponseBody(mojo::ScopedDataPipeConsumerHandle body_data_pipe) = 0;
...
class SaveToStringBodyHandler : public BodyHandler, public BodyReader::Delegate {
public:
    SaveToStringBodyHandler(SimpleURLLoaderImpl* simple_url_loader, bool want_download_progress,
        SimpleURLLoader::BodyAsStringCallbackDeprecated body_as_string_callback, int64_t max_body_size)
    ...
    void OnStartLoadingResponseBody(mojo::ScopedDataPipeConsumerHandle body_data_pipe) override
    {
        DCHECK(!body_);
        DCHECK(!body_reader_);

        body_ = std::make_unique<std::string>();
        body_reader_ = std::make_unique<BodyReader>(this, max_body_size_, url_loader_created_from_);
...
public:
    HeadersOnlyBodyHandler(SimpleURLLoaderImpl* simple_url_loader, SimpleURLLoader::HeadersOnlyCallback headers_only_callback)
    ...
    // BodyHandler implementation
    bool RequiresBodyDataPipe() override
    {
        return false;
    }
    ...
    void OnStartLoadingResponseBody(mojo::ScopedDataPipeConsumerHandle body_data_pipe) override
    {
        ...
        DCHECK(!body_reader_);
        body_reader_ = std::make_unique<BodyReader>(this, std::numeric_limits<int64_t>::max(), url_loader_created_from_);
        body_reader_->Start(std::move(body_data_pipe));
    }
    ...
    {
        body_reader_.reset();
        std::move(headers_only_callback_).Run(simple_url_loader()->ResponseInfo() ? simple_url_loader()->ResponseInfo()->headers : nullptr);
    }
...
public:
    ...
    SaveToFileBodyHandler(SimpleURLLoaderImpl* simple_url_loader, bool want_download_progress,
        SimpleURLLoader::DownloadToFileCompleteCallback download_to_file_complete_callback, const base::FilePath& path, bool create_temp_file,
        uint64_t max_body_size, base::TaskPriority task_priority)
        : BodyHandler(simple_url_loader, want_download_progress)
        , download_to_file_complete_callback_(std::move(download_to_file_complete_callback))
    {
        ...
        file_writer_ = std::make_unique<FileWriter>(path, create_temp_file, max_body_size, task_priority,
            want_download_progress ? base::BindRepeating(&SaveToFileBodyHandler::ReportProgress, weak_ptr_factory_.GetWeakPtr())
    ...
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
        file_writer_->StartWriting(std::move(body_data_pipe), base::BindOnce(&SaveToFileBodyHandler::OnDone, weak_ptr_factory_.GetWeakPtr()));
    }
    ...
    void NotifyConsumerOfCompletion(bool destroy_results) override
    {
        ...
        if (destroy_results) {
            ...
            file_writer_->DeleteFile(base::BindOnce(&SaveToFileBodyHandler::InvokeCallbackAsynchronously, weak_ptr_factory_.GetWeakPtr()));
            FileWriter::Destroy(std::move(file_writer_));
            return;
        }
...
private:
    ...
    public:
        using OnDoneCallback = base::OnceCallback<void(net::Error error, int64_t total_bytes, const base::FilePath& path)>;

        FileWriter(const base::FilePath& path, bool create_temp_file, int64_t max_body_size, base::TaskPriority priority,
            base::RepeatingCallback<void(int64_t)> progress_callback, const base::Location& url_loader_created_from)
            : body_handler_task_runner_(base::SequencedTaskRunner::GetCurrentDefault())
            , file_writer_task_runner_(base::ThreadPool::CreateSequencedTaskRunner({ base::MayBlock(), priority, base::TaskShutdownBehavior::BLOCK_SHUTDOWN }))
            , path_(path)
        ...
        void StartWriting(mojo::ScopedDataPipeConsumerHandle body_data_pipe, OnDoneCallback on_done_callback)
        {
            DCHECK(body_handler_task_runner_->RunsTasksInCurrentSequence());
            file_writer_task_runner_->PostTask(FROM_HERE,
                base::BindOnce(&FileWriter::StartWritingOnFileSequence, base::Unretained(this), std::move(body_data_pipe), std::move(on_done_callback)));
        }
        ...
        void DeleteFile(base::OnceClosure on_file_deleted_closure)
        {
            ...
            file_writer_task_runner_->PostTask(
                FROM_HERE, base::BindOnce(&FileWriter::DeleteFileOnFileSequence, base::Unretained(this), std::move(on_file_deleted_closure)));
        }

        // Destroys the FileWriter on the file TaskRunner.
        static void Destroy(std::unique_ptr<FileWriter> file_writer)
        {
            DCHECK(file_writer->body_handler_task_runner_->RunsTasksInCurrentSequence());
            ...
            base::SequencedTaskRunner* task_runner = file_writer->file_writer_task_runner_.get();
    ...
    private:
        void StartWritingOnFileSequence(mojo::ScopedDataPipeConsumerHandle body_data_pipe, OnDoneCallback on_done_callback)
        {
            DCHECK(file_writer_task_runner_->RunsTasksInCurrentSequence());
            ...
            if (have_path) {
                ...
                file_.Initialize(path_, base::File::FLAG_WRITE | base::File::FLAG_CREATE_ALWAYS);
            }

            // If CreateTemporaryFile() or File::Initialize() failed, report failure.
            if (!file_.IsValid()) {
                body_handler_task_runner_->PostTask(
                    FROM_HERE, base::BindOnce(std::move(on_done_callback), net::MapSystemError(logging::GetLastSystemErrorCode()), 0, base::FilePath()));
                return;
            }
        ...
        net::Error OnDataRead(uint32_t length, const char* data) override
        {
            DCHECK(file_writer_task_runner_->RunsTasksInCurrentSequence());
            while (length > 0) {
                int written = file_.WriteAtCurrentPos(data, std::min(length, static_cast<uint32_t>(std::numeric_limits<int>::max())));
                if (written < 0)
            ...
            if (progress_callback_) {
                body_handler_task_runner_->PostTask(FROM_HERE, base::BindOnce(progress_callback_, body_reader_->total_bytes_read()));
            }
        ...
        void OnDone(net::Error error, int64_t total_bytes) override
        {
            ...
            body_handler_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(on_done_callback_), error, total_bytes, path_));
        }
        ...
        void DeleteFileOnFileSequence(base::OnceClosure on_file_deleted_closure)
        {
            ...
            if (on_file_deleted_closure) {
                body_handler_task_runner_->PostTask(FROM_HERE, std::move(on_file_deleted_closure));
            }
        }
    ...
    void OnDone(net::Error error, int64_t total_bytes, const base::FilePath& path)
    {
        path_ = path;
        simple_url_loader()->OnBodyHandlerDone(error, total_bytes);
    }
    ...
    SimpleURLLoader::DownloadToFileCompleteCallback download_to_file_complete_callback_;
...
class DownloadAsStreamBodyHandler : public BodyHandler, public BodyReader::Delegate {
public:
    DownloadAsStreamBodyHandler(SimpleURLLoaderImpl* simple_url_loader, bool want_download_progress, SimpleURLLoaderStreamConsumer* stream_consumer)
        : BodyHandler(simple_url_loader, want_download_progress)
    ...
    DownloadAsStreamBodyHandler& operator=(const DownloadAsStreamBodyHandler&) = delete;
    ...
    {
        body_reader_.reset();
        stream_consumer_.ExtractAsDangling()->OnComplete(simple_url_loader()->NetError() == net::OK);
    }
...
private:
    ...
    net::Error OnDataRead(uint32_t length, const char* data) override
    {
        ...
        base::WeakPtr<DownloadAsStreamBodyHandler> weak_this(weak_ptr_factory_.GetWeakPtr());
        stream_consumer_->OnDataReceived(std::string_view(data, length), base::BindOnce(&DownloadAsStreamBodyHandler::Resume, weak_ptr_factory_.GetWeakPtr()));
    ...
    void Resume()
    {
        ...
        if (in_recursive_call_) {
            base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
                FROM_HERE, base::BindOnce(&DownloadAsStreamBodyHandler::Resume, weak_ptr_factory_.GetWeakPtr()));
            return;
        }
        if (!body_reader_) {
    ...
    , timeout_timer_(timeout_tick_clock_)
{
    ...
    if (resource_request_->request_body) {
        for (const DataElement& element : *resource_request_->request_body->elements()) {
            ...
            DCHECK(element.type() != mojom::DataElementDataView::Tag::kFile && element.type() != mojom::DataElementDataView::Tag::kBytes);
        }
    }
#endif // DCHECK_IS_ON()
}
...
void SimpleURLLoaderImpl::DownloadToString(
    mojom::URLLoaderFactory* url_loader_factory, BodyAsStringCallbackDeprecated body_as_string_callback, size_t max_body_size)
{
    DCHECK_LE(max_body_size, kMaxBoundedStringDownloadSize);
    body_handler_
        = std::make_unique<SaveToStringBodyHandler>(this, !on_download_progress_callback_.is_null(), std::move(body_as_string_callback), max_body_size);
    Start(url_loader_factory);
}

void SimpleURLLoaderImpl::DownloadToString(mojom::URLLoaderFactory* url_loader_factory, BodyAsStringCallback body_as_string_callback, size_t max_body_size)
{
    DownloadToString(url_loader_factory, base::BindOnce(GetFromUniquePtrToOptional, std::move(body_as_string_callback)), max_body_size);
}

void SimpleURLLoaderImpl::DownloadToStringOfUnboundedSizeUntilCrashAndDie(
    mojom::URLLoaderFactory* url_loader_factory, BodyAsStringCallbackDeprecated body_as_string_callback)
{
    body_handler_ = std::make_unique<SaveToStringBodyHandler>(this, !on_download_progress_callback_.is_null(), std::move(body_as_string_callback),
...
void SimpleURLLoaderImpl::DownloadToStringOfUnboundedSizeUntilCrashAndDie(
    mojom::URLLoaderFactory* url_loader_factory, BodyAsStringCallback body_as_string_callback)
{
    DownloadToStringOfUnboundedSizeUntilCrashAndDie(url_loader_factory, base::BindOnce(GetFromUniquePtrToOptional, std::move(body_as_string_callback)));
}

void SimpleURLLoaderImpl::DownloadHeadersOnly(mojom::URLLoaderFactory* url_loader_factory, HeadersOnlyCallback headers_only_callback)
{
    on_download_progress_callback_.Reset();
    body_handler_ = std::make_unique<HeadersOnlyBodyHandler>(this, std::move(headers_only_callback));
    if (base::FeatureList::IsEnabled(kSimpleURLLoaderUseReadAndDiscardBodyOption)) {
        url_loader_factory_options_ |= mojom::kURLLoadOptionReadAndDiscardBody;
    }
    Start(url_loader_factory);
}

void SimpleURLLoaderImpl::DownloadToFile(mojom::URLLoaderFactory* url_loader_factory, DownloadToFileCompleteCallback download_to_file_complete_callback,
    const base::FilePath& file_path, int64_t max_body_size)
{
    DCHECK(!file_path.empty());
    body_handler_ = std::make_unique<SaveToFileBodyHandler>(this, !on_download_progress_callback_.is_null(), std::move(download_to_file_complete_callback),
        file_path, false /* create_temp_file */, max_body_size, GetTaskPriority());
    Start(url_loader_factory);
}

void SimpleURLLoaderImpl::DownloadToTempFile(
    mojom::URLLoaderFactory* url_loader_factory, DownloadToFileCompleteCallback download_to_file_complete_callback, int64_t max_body_size)
{
    body_handler_ = std::make_unique<SaveToFileBodyHandler>(this, !on_download_progress_callback_.is_null(), std::move(download_to_file_complete_callback),
        base::FilePath(), true /* create_temp_file */, max_body_size, GetTaskPriority());
    Start(url_loader_factory);
}

void SimpleURLLoaderImpl::DownloadAsStream(mojom::URLLoaderFactory* url_loader_factory, SimpleURLLoaderStreamConsumer* stream_consumer)
{
    body_handler_ = std::make_unique<DownloadAsStreamBodyHandler>(this, !on_download_progress_callback_.is_null(), stream_consumer);
    Start(url_loader_factory);
}

void SimpleURLLoaderImpl::SetOnRedirectCallback(const OnRedirectCallback& on_redirect_callback)
{
...
void SimpleURLLoaderImpl::SetOnResponseStartedCallback(OnResponseStartedCallback on_response_started_callback)
{
    // Check if a request has not yet been started.
    DCHECK(!body_handler_);
...
void SimpleURLLoaderImpl::SetOnUploadProgressCallback(UploadProgressCallback on_upload_progress_callback)
{
    // Check if a request has not yet been started.
    DCHECK(!body_handler_);
...
void SimpleURLLoaderImpl::SetOnDownloadProgressCallback(DownloadProgressCallback on_download_progress_callback)
{
    // Check if a request has not yet been started.
    DCHECK(!body_handler_);
...
void SimpleURLLoaderImpl::SetAllowHttpErrorResults(bool allow_http_error_results)
{
    // Check if a request has not yet been started.
    DCHECK(!body_handler_);
...
void SimpleURLLoaderImpl::AttachStringForUploadInternal(std::string_view upload_data, base::optional_ref<std::string_view> upload_content_type)
{
    // Currently only allow a single string to be attached.
    DCHECK(!resource_request_->request_body);
    DCHECK(resource_request_->method != net::HttpRequestHeaders::kGetMethod && resource_request_->method != net::HttpRequestHeaders::kHeadMethod);
    ...
    if (upload_data.length() <= kMaxUploadStringSizeToCopy) {
        ...
        resource_request_->request_body->AppendBytes(upload_data.data(), copy_length);
    } else {
        // Don't attach the upload body here.  A new pipe will need to be created
        // each time the request is tried.
        string_upload_data_pipe_getter_ = std::make_unique<StringUploadDataPipeGetter>(std::string(upload_data), created_from_);
    }

    if (upload_content_type) {
        resource_request_->headers.SetHeader(net::HttpRequestHeaders::kContentType, *upload_content_type);
    }
}

void SimpleURLLoaderImpl::AttachStringForUploadInternal(std::string&& upload_data, base::optional_ref<std::string_view> upload_content_type)
{
    // Currently only allow a single string to be attached.
    DCHECK(!resource_request_->request_body);
    DCHECK(resource_request_->method != net::HttpRequestHeaders::kGetMethod && resource_request_->method != net::HttpRequestHeaders::kHeadMethod);
    ...
    if (upload_data.length() <= kMaxUploadStringSizeToCopy) {
        ...
        // Don't attach the upload body here.  A new pipe will need to be created
        // each time the request is tried.
        string_upload_data_pipe_getter_ = std::make_unique<StringUploadDataPipeGetter>(std::move(upload_data), created_from_);
    }
...
void SimpleURLLoaderImpl::AttachStringForUpload(std::string_view upload_data, std::string_view upload_content_type)
{
...
void SimpleURLLoaderImpl::AttachStringForUpload(const char* upload_data, std::string_view upload_content_type)
{
    AttachStringForUploadInternal(std::string_view(upload_data), upload_content_type);
}
...
void SimpleURLLoaderImpl::AttachStringForUpload(std::string&& upload_data, std::string_view upload_content_type)
{
...
void SimpleURLLoaderImpl::AttachFileForUpload(
    const base::FilePath& upload_file_path, const std::string* const upload_content_type, uint64_t offset, uint64_t length)
{
    ...
    resource_request_->request_body->AppendFileRange(upload_file_path, offset, length, base::Time());
...
void SimpleURLLoaderImpl::AttachFileForUpload(const base::FilePath& upload_file_path, const std::string& upload_content_type, uint64_t offset, uint64_t length)
{
    AttachFileForUpload(upload_file_path, &upload_content_type, offset, length);
}

void SimpleURLLoaderImpl::AttachFileForUpload(const base::FilePath& upload_file_path, uint64_t offset, uint64_t length)
{
...
const std::optional<URLLoaderCompletionStatus>& SimpleURLLoaderImpl::CompletionStatus() const
{
    // Should only be called once the request is complete.
    DCHECK(request_state_->finished);
...
void SimpleURLLoaderImpl::OnBodyHandlerDone(net::Error error, int64_t received_body_size)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(request_state_->body_started);
...
void SimpleURLLoaderImpl::OnBodyHandlerProgress(int64_t progress)
{
    if (on_download_progress_callback_) {
        base::SequencedTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&SimpleURLLoaderImpl::DispatchDownloadProgress, weak_ptr_factory_.GetWeakPtr(), progress));
    }
}
...
void SimpleURLLoaderImpl::DispatchDownloadProgress(int64_t downloaded)
{
    ...
    if (!request_state_->body_started || request_state_->request_completed || request_state_->finished) {
        return;
    }
...
void SimpleURLLoaderImpl::FinishWithResult(int net_error)
{
    ...
    bool destroy_results = request_state_->net_error != net::OK && !allow_partial_results_;
...
void SimpleURLLoaderImpl::Start(mojom::URLLoaderFactory* url_loader_factory)
{
    ...
    if (remaining_retries_ > 0) {
        ...
        url_loader_factory->Clone(url_loader_factory_remote_.BindNewPipeAndPassReceiver());
    }
...
void SimpleURLLoaderImpl::StartRequest(mojom::URLLoaderFactory* url_loader_factory)
{
    DCHECK(resource_request_);
    ...
    if (string_upload_data_pipe_getter_) {
        resource_request_->request_body = new ResourceRequestBody();
        resource_request_->request_body->AppendDataPipe(string_upload_data_pipe_getter_->GetRemoteForNewUpload());
    }
    url_loader_factory->CreateLoaderAndStart(url_loader_.BindNewPipeAndPassReceiver(), request_id_, url_loader_factory_options_, *resource_request_,
        client_receiver_.BindNewPipeAndPassRemote(), net::MutableNetworkTrafficAnnotationTag(annotation_tag_));
    client_receiver_.set_disconnect_handler(base::BindOnce(&SimpleURLLoaderImpl::OnMojoDisconnect, base::Unretained(this)));

    // Note that this ends up restarting the timer on each retry.
    if (!timeout_duration_.is_zero()) {
        timeout_timer_.Start(
            FROM_HERE, timeout_duration_, base::BindOnce(&SimpleURLLoaderImpl::FinishWithResult, weak_ptr_factory_.GetWeakPtr(), net::ERR_TIMED_OUT));
    }
...
void SimpleURLLoaderImpl::Retry()
{
    ...
    request_state_ = std::make_unique<RequestState>();
    body_handler_->PrepareToRetry(base::BindOnce(&SimpleURLLoaderImpl::StartRequest, weak_ptr_factory_.GetWeakPtr(), url_loader_factory_remote_.get()));
}

void SimpleURLLoaderImpl::OnReceiveEarlyHints(network::mojom::EarlyHintsPtr early_hints)
{
}

void SimpleURLLoaderImpl::OnReceiveResponse(
    mojom::URLResponseHeadPtr response_head, mojo::ScopedDataPipeConsumerHandle body, std::optional<mojo_base::BigBuffer> cached_metadata)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    // retry the request.
    if (response_code / 100 == 5 && remaining_retries_ > 0 && (retry_mode_ & RETRY_ON_5XX)) {
        Retry();
        return;
    }
...
void SimpleURLLoaderImpl::OnReceiveRedirect(const net::RedirectInfo& redirect_info, mojom::URLResponseHeadPtr response_head)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (request_state_->response_info) {
    ...
    if (on_redirect_callback_) {
        ...
        on_redirect_callback_.Run(url_before_redirect, redirect_info, *response_head, &removed_headers);
        // If deleted by the callback, bail now.
        if (!weak_this)
            return;
    }
    ...
    url_loader_->FollowRedirect(removed_headers, {} /* modified_headers */, {} /* modified_cors_exempt_headers */, {} /* new_url */);
}

void SimpleURLLoaderImpl::OnTransferSizeUpdated(int32_t transfer_size_diff)
{
    network::RecordOnTransferSizeUpdatedUMA(network::OnTransferSizeUpdatedFrom::kSimpleURLLoaderImpl);
}

void SimpleURLLoaderImpl::OnUploadProgress(int64_t current_position, int64_t total_size, OnUploadProgressCallback ack_callback)
{
    if (on_upload_progress_callback_)
...
void SimpleURLLoaderImpl::MaybeComplete()
{
    ...
    if (request_state_->net_error == net::ERR_NAME_NOT_RESOLVED && remaining_retries_ > 0 && (retry_mode_ & RETRY_ON_NAME_NOT_RESOLVED)) {
        Retry();
        return;
    }
    ...
    // on the body pipe.
    if (request_state_->net_error == net::ERR_NETWORK_CHANGED && remaining_retries_ > 0 && (retry_mode_ & RETRY_ON_NETWORK_CHANGE)) {
        Retry();
        return;
    }
    ...
    if (request_state_->received_body_size == kReceivedBodySizeUnknown) {
        request_state_->received_body_size = request_state_->completion_status ? request_state_->completion_status->decoded_body_length : 0;
    }

    // When OnCompleted sees a success result, still need to report an error if
    // the size isn't what was expected.
    if (request_state_->net_error == net::OK && request_state_->completion_status
        && request_state_->completion_status->decoded_body_length != request_state_->received_body_size) {
        if (request_state_->completion_status->decoded_body_length > request_state_->received_body_size) {
...
std::unique_ptr<SimpleURLLoader> SimpleURLLoader::Create(
    std::unique_ptr<ResourceRequest> resource_request, const net::NetworkTrafficAnnotationTag& annotation_tag, base::Location created_from)
{
    DCHECK(resource_request);
    return std::make_unique<SimpleURLLoaderImpl>(std::move(resource_request), annotation_tag, created_from);
}

void SimpleURLLoader::SetTimeoutTickClockForTest(const base::TickClock* timeout_tick_clock)
{
