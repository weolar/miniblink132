...
DataPipeToSourceStream::DataPipeToSourceStream(mojo::ScopedDataPipeConsumerHandle body)
...
{
    handle_watcher_.Watch(body_.get(), MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED,
        base::BindRepeating(&DataPipeToSourceStream::OnReadable, base::Unretained(this)));
}
...
int DataPipeToSourceStream::Read(net::IOBuffer* buf, int buf_size, net::CompletionOnceCallback callback)
{
    ...
    switch (result) {
    case MOJO_RESULT_OK: {
        size_t consume = std::min(base::checked_cast<size_t>(buf_size), buffer.size());
...
void DataPipeToSourceStream::OnReadable(MojoResult unused)
{
    ...
    switch (result) {
    case MOJO_RESULT_OK: {
        size_t consume = std::min(base::checked_cast<size_t>(output_buf_size_), buffer.size());
