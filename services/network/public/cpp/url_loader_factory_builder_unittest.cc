...
public:
    ...
    void AddInterceptors(URLLoaderFactoryBuilder& factory_builder, int num_interceptors);
    ...
    template <typename T> void CreateLoaderAndStart(const T& result)
    {
        mojo::PendingRemote<mojom::URLLoader> loader;
        mojo::PendingReceiver<mojom::URLLoaderClient> client;

        result->CreateLoaderAndStart(loader.InitWithNewPipeAndPassReceiver(), 0 /* request_id */, 0 /* options */, ResourceRequest(),
...
public:
    LoggingURLLoaderFactory(
        URLLoaderFactoryBuilderTest* test, std::string name, mojo::PendingRemote<mojom::URLLoaderFactory> target, base::OnceClosure on_create_loader_and_start)
    ...
    static scoped_refptr<SharedURLLoaderFactory> Create(URLLoaderFactoryBuilderTest* test, std::string name,
        mojo::PendingRemote<mojom::URLLoaderFactory> target, base::OnceClosure on_create_loader_and_start = {})
    {
        auto factory = base::MakeRefCounted<LoggingURLLoaderFactory>(test, std::move(name), std::move(target), std::move(on_create_loader_and_start));
    ...
    static scoped_refptr<SharedURLLoaderFactory> CreateTerminal(
        URLLoaderFactoryBuilderTest* test, std::string name, base::OnceClosure on_create_loader_and_start)
    {
        return Create(test, std::move(name), mojo::NullRemote(), std::move(on_create_loader_and_start));
    }

    static mojo::PendingRemote<mojom::URLLoaderFactory> CreateTerminalPendingRemote(
        URLLoaderFactoryBuilderTest* test, std::string name, base::OnceClosure on_create_loader_and_start)
    {
        mojo::PendingRemote<mojom::URLLoaderFactory> remote;
        mojo::PendingReceiver<mojom::URLLoaderFactory> receiver = remote.InitWithNewPipeAndPassReceiver();

        CreateTerminal(test, std::move(name), std::move(on_create_loader_and_start))->Clone(std::move(receiver));
        return remote;
    }

    void CreateLoaderAndStart(mojo::PendingReceiver<mojom::URLLoader> loader, int32_t request_id, uint32_t options, const ResourceRequest& request,
        mojo::PendingRemote<mojom::URLLoaderClient> client, const net::MutableNetworkTrafficAnnotationTag& traffic_annotation) override
    {
        CHECK(test_);
        test_->AddLog(name_);
        if (target_) {
            target_->CreateLoaderAndStart(std::move(loader), request_id, options, request, std::move(client), traffic_annotation);
        }
...
public:
    LoggingNetworkContext(URLLoaderFactoryBuilderTest* test, std::string name, base::OnceClosure on_create_loader_and_start)
        : test_(test->GetWeakPtr())
        , name_(std::move(name))
        , on_create_loader_and_start_(std::move(on_create_loader_and_start))
    {
    ...
    void CreateURLLoaderFactory(mojo::PendingReceiver<mojom::URLLoaderFactory> receiver, mojom::URLLoaderFactoryParamsPtr factory_param) override
    {
        LoggingURLLoaderFactory::CreateTerminal(test_.get(), name_, std::move(on_create_loader_and_start_))->Clone(std::move(receiver));
    }
...
TEST_F(URLLoaderFactoryBuilderTest, Basic)
{
    ...
    // Append the first intercepting step.
    {
        ...
        scoped_refptr<SharedURLLoaderFactory> interceptor = LoggingURLLoaderFactory::Create(this, "Interceptor1", std::move(remote));
    ...
    // Append the second intercepting step.
    {
        ...
        scoped_refptr<SharedURLLoaderFactory> interceptor = LoggingURLLoaderFactory::Create(this, "Interceptor2", std::move(remote));
        interceptor->Clone(std::move(receiver));
    }
    ...
    scoped_refptr<SharedURLLoaderFactory> terminal = LoggingURLLoaderFactory::CreateTerminal(this, "Terminal", run_loop.QuitClosure());
    scoped_refptr<SharedURLLoaderFactory> result = std::move(factory_builder).Finish(std::move(terminal));
    ...
    run_loop.Run();
    EXPECT_THAT(GetLogs(), ElementsAre("Interceptor1", "Interceptor2", "Terminal"));
}
...
TEST_F(URLLoaderFactoryBuilderTest, BasicWithPendingReceiver)
{
    ...
    mojo::PendingReceiver<mojom::URLLoaderFactory> root_receiver = root_remote.BindNewPipeAndPassReceiver();
    ...
    base::RunLoop run_loop;
    scoped_refptr<SharedURLLoaderFactory> terminal = LoggingURLLoaderFactory::CreateTerminal(this, "Terminal", run_loop.QuitClosure());
    std::move(factory_builder).Finish(std::move(root_receiver), std::move(terminal));
...
void URLLoaderFactoryBuilderTest::AddInterceptors(URLLoaderFactoryBuilder& factory_builder, int num_interceptors)
{
    ...
    for (int i = 0; i < num_interceptors; ++i) {
        ...
        scoped_refptr<SharedURLLoaderFactory> interceptor
            = LoggingURLLoaderFactory::Create(this, "Interceptor" + base::NumberToString(i + 1), std::move(remote));
        interceptor->Clone(std::move(receiver));
    }
}
...
TEST_F(URLLoaderFactoryBuilderTest, URLLoaderFactoryFromTerminalPendingRemote)
{
    ...
    base::RunLoop run_loop;
    mojo::PendingRemote<mojom::URLLoaderFactory> terminal(LoggingURLLoaderFactory::CreateTerminalPendingRemote(this, "Terminal", run_loop.QuitClosure()));
    scoped_refptr<SharedURLLoaderFactory> result = std::move(factory_builder).Finish(std::move(terminal));
...
TEST_F(URLLoaderFactoryBuilderTest, URLLoaderFactoryFromTerminalNetworkContext)
{
    URLLoaderFactoryBuilder factory_builder;
    AddInterceptors(factory_builder, 2);

    base::RunLoop run_loop;
    auto terminal = std::make_unique<LoggingNetworkContext>(this, "Terminal", run_loop.QuitClosure());
    scoped_refptr<SharedURLLoaderFactory> result = std::move(factory_builder).Finish(terminal.get(), mojom::URLLoaderFactoryParams::New());
...
TEST_F(URLLoaderFactoryBuilderTest, PendingRemoteFromTerminalPendingRemote)
{
    ...
    base::RunLoop run_loop;
    mojo::PendingRemote<mojom::URLLoaderFactory> terminal(LoggingURLLoaderFactory::CreateTerminalPendingRemote(this, "Terminal", run_loop.QuitClosure()));
    mojo::PendingRemote<mojom::URLLoaderFactory> result = std::move(factory_builder).Finish<mojo::PendingRemote<mojom::URLLoaderFactory>>(std::move(terminal));

    CreateLoaderAndStart(mojo::Remote<mojom::URLLoaderFactory>(std::move(result)));
    run_loop.Run();
    EXPECT_THAT(GetLogs(), ElementsAre("Interceptor1", "Interceptor2", "Terminal"));
}
...
TEST_F(URLLoaderFactoryBuilderTest, PendingRemoteFromTerminalNetworkContext)
{
    ...
    mojo::PendingRemote<mojom::URLLoaderFactory> result
        = std::move(factory_builder).Finish<mojo::PendingRemote<mojom::URLLoaderFactory>>(terminal.get(), mojom::URLLoaderFactoryParams::New());
