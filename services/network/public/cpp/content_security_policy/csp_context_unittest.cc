...
public:
    ...
    void SanitizeDataForUseInCspViolation(CSPDirectiveName directive, GURL* blocked_url, network::mojom::SourceLocation* source_location) const override
    {
        if (!sanitize_data_for_use_in_csp_violation_)
            return;
        *blocked_url = blocked_url->DeprecatedGetOriginAsURL();
        source_location->url = GURL(source_location->url).DeprecatedGetOriginAsURL().spec();
...
mojom::ContentSecurityPolicyPtr BuildPolicy(mojom::CSPSourcePtr self_source, CSPDirectiveName directive_name, mojom::CSPSourcePtr source)
{
...
// Build a new policy made of only one directive and no report endpoints.
mojom::ContentSecurityPolicyPtr BuildPolicy(
    mojom::CSPSourcePtr self_source, CSPDirectiveName directive_name, mojom::CSPSourcePtr source_1, mojom::CSPSourcePtr source_2)
{
    auto source_list = mojom::CSPSourceList::New();
...
TEST(CSPContextTest, SchemeShouldBypassCSP)
{
    ...
    policies.push_back(BuildPolicy(self_source.Clone(), CSPDirectiveName::DefaultSrc, BuildCSPSource("", "example.com")));

    EXPECT_FALSE(context.IsAllowedByCsp(
        policies, CSPDirectiveName::FrameSrc, GURL("data:text/html,<html></html>"), GURL(), false, SourceLocation(), CSPContext::CHECK_ALL_CSP, false));
...
TEST(CSPContextTest, MultiplePolicies)
{
    ...
    std::vector<mojom::ContentSecurityPolicyPtr> policies;
    policies.push_back(BuildPolicy(self_source.Clone(), CSPDirectiveName::FrameSrc, BuildCSPSource("", "a.com"), BuildCSPSource("", "b.com")));
    policies.push_back(BuildPolicy(self_source.Clone(), CSPDirectiveName::FrameSrc, BuildCSPSource("", "a.com"), BuildCSPSource("", "c.com")));

    EXPECT_TRUE(context.IsAllowedByCsp(
        policies, CSPDirectiveName::FrameSrc, GURL("http://a.com"), GURL("http://a.com"), false, SourceLocation(), CSPContext::CHECK_ALL_CSP, false));
    EXPECT_FALSE(context.IsAllowedByCsp(
        policies, CSPDirectiveName::FrameSrc, GURL("http://b.com"), GURL("http://b.com"), false, SourceLocation(), CSPContext::CHECK_ALL_CSP, false));
    EXPECT_FALSE(context.IsAllowedByCsp(
        policies, CSPDirectiveName::FrameSrc, GURL("http://c.com"), GURL("http://c.com"), false, SourceLocation(), CSPContext::CHECK_ALL_CSP, false));
    EXPECT_FALSE(context.IsAllowedByCsp(
        policies, CSPDirectiveName::FrameSrc, GURL("http://d.com"), GURL("http://d.com"), false, SourceLocation(), CSPContext::CHECK_ALL_CSP, false));
}
...
TEST(CSPContextTest, SanitizeDataForUseInCspViolation)
{
    ...
    std::vector<mojom::ContentSecurityPolicyPtr> policies;
    policies.push_back(
        BuildPolicy(self_source.Clone(), CSPDirectiveName::FrameSrc, mojom::CSPSource::New("", "a.com", url::PORT_UNSPECIFIED, "/iframe", false, false)));
    policies.push_back(
        BuildPolicy(self_source.Clone(), CSPDirectiveName::FormAction, mojom::CSPSource::New("", "a.com", url::PORT_UNSPECIFIED, "/form", false, false)));

    GURL blocked_url("http://a.com/login?password=1234");
    GURL original_url("http://b.com/login?password=1234");
    auto source_location = network::mojom::SourceLocation::New("http://a.com/login", 10u, 20u);

    // When the |blocked_url| and |source_location| aren't sensitive information,
    // for frame-src.
    {
        EXPECT_FALSE(
            context.IsAllowedByCsp(policies, CSPDirectiveName::FrameSrc, blocked_url, original_url, false, source_location, CSPContext::CHECK_ALL_CSP, false));
        ASSERT_EQ(1u, context.violations().size());
        EXPECT_EQ(context.violations()[0]->blocked_url, blocked_url);
        EXPECT_EQ(context.violations()[0]->source_location->url, "http://a.com/login");
    ...
    {
        EXPECT_FALSE(context.IsAllowedByCsp(
            policies, CSPDirectiveName::FormAction, blocked_url, original_url, false, source_location, CSPContext::CHECK_ALL_CSP, false));
        ASSERT_EQ(2u, context.violations().size());
        EXPECT_EQ(context.violations()[1]->blocked_url, original_url);
        EXPECT_EQ(context.violations()[1]->source_location->url, "http://a.com/login");
    ...
    // frame-src.
    {
        ...
        EXPECT_EQ(context.violations()[2]->blocked_url, blocked_url.DeprecatedGetOriginAsURL());
...
TEST(CSPContextTest, MultipleInfringement)
{
    ...
    std::vector<mojom::ContentSecurityPolicyPtr> policies;
    policies.push_back(BuildPolicy(self_source.Clone(), CSPDirectiveName::FrameSrc, BuildCSPSource("", "a.com")));
    policies.push_back(BuildPolicy(self_source.Clone(), CSPDirectiveName::FrameSrc, BuildCSPSource("", "b.com")));
    policies.push_back(BuildPolicy(self_source.Clone(), CSPDirectiveName::FrameSrc, BuildCSPSource("", "c.com")));

    EXPECT_FALSE(context.IsAllowedByCsp(
        policies, CSPDirectiveName::FrameSrc, GURL("http://c.com"), GURL("http://c.com"), false, SourceLocation(), CSPContext::CHECK_ALL_CSP, false));
    ASSERT_EQ(2u, context.violations().size());
    const char console_message_a[] = "Refused to frame 'http://c.com/' because it violates the following "
                                     "Content Security Policy directive: \"frame-src a.com\".\n";
    const char console_message_b[] = "Refused to frame 'http://c.com/' because it violates the following "
...
TEST(CSPContextTest, CheckCSPDisposition)
{
    ...
    auto enforce_csp = BuildPolicy(self_source.Clone(), CSPDirectiveName::FrameSrc, BuildCSPSource("", "example.com"));
    // Add a report-only policy.
    auto report_only_csp = BuildPolicy(self_source.Clone(), CSPDirectiveName::DefaultSrc, BuildCSPSource("", "example.com"));
    ...
    // be reported.
    EXPECT_FALSE(context.IsAllowedByCsp(policies, CSPDirectiveName::FrameSrc, GURL("https://not-example.com"), GURL("https://not-example.com"), false,
        SourceLocation(), CSPContext::CHECK_ALL_CSP, false));
    ASSERT_EQ(2u, context.violations().size());
    const char console_message_a[] = "Refused to frame 'https://not-example.com/' because it violates the "
                                     "following "
                                     "Content Security Policy directive: \"frame-src example.com\".\n";
    const char console_message_b[] = "[Report Only] Refused to frame 'https://not-example.com/' because it "
    ...
    EXPECT_TRUE((console_message_a == context.violations()[0]->console_message && console_message_b == context.violations()[1]->console_message)
        || (console_message_a == context.violations()[1]->console_message && console_message_b == context.violations()[0]->console_message));

    // With CHECK_REPORT_ONLY_CSP, the request should be allowed but reported.
    context.ClearViolations();
    EXPECT_TRUE(context.IsAllowedByCsp(policies, CSPDirectiveName::FrameSrc, GURL("https://not-example.com"), GURL("https://not-example.com"), false,
...
TEST(CSPContextTest, BlockedDespiteWildcard)
{
    ...
    auto policies = ParseContentSecurityPolicies(
        "frame-src *", mojom::ContentSecurityPolicyType::kEnforce, mojom::ContentSecurityPolicySource::kMeta, GURL("https://www.example.org"));
