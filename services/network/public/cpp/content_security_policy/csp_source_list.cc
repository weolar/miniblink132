...
bool AllowFromSources(const GURL& url, const std::vector<mojom::CSPSourcePtr>& sources, const mojom::CSPSource& self_source, bool has_followed_redirect,
    bool is_opaque_fenced_frame)
{
    for (const auto& source : sources) {
        if (CheckCSPSource(*source, url, self_source, CSPSourceContext::ContentSecurityPolicy, has_followed_redirect, is_opaque_fenced_frame)) {
            return true;
        }
    }
    return false;
...
void IntersectNonces(base::flat_set<std::string>& a, const base::flat_set<std::string>& b)
{
    base::EraseIf(a, [&b](const std::string& s) { return !b.contains(s); });
}

// Removes from |a| elements not contained in |b|.
void IntersectHashes(base::flat_set<mojom::CSPHashSourcePtr>& a, const base::flat_set<mojom::CSPHashSourcePtr>& b)
{
    base::EraseIf(a, [&b](const mojom::CSPHashSourcePtr& h) { return !b.contains(h); });
}

bool IsScriptDirective(CSPDirectiveName directive)
{
    return directive == CSPDirectiveName::ScriptSrc || directive == CSPDirectiveName::ScriptSrcAttr || directive == CSPDirectiveName::ScriptSrcElem
        || directive == CSPDirectiveName::DefaultSrc;
}

bool IsStyleDirective(CSPDirectiveName directive)
{
    return directive == CSPDirectiveName::StyleSrc || directive == CSPDirectiveName::StyleSrcAttr || directive == CSPDirectiveName::StyleSrcElem
        || directive == CSPDirectiveName::DefaultSrc;
}

bool AllowAllInline(CSPDirectiveName directive, const mojom::CSPSourceList& source)
{
    return source.allow_inline && source.hashes.empty() && source.nonces.empty() && (!IsScriptDirective(directive) || !source.allow_dynamic);
}

void AddSourceSchemesToSet(base::flat_set<std::string>& set, const mojom::CSPSource* source)
{
...
base::flat_set<std::string> IntersectSchemesOnly(const std::vector<mojom::CSPSourcePtr>& list_a, const std::vector<mojom::CSPSourcePtr>& list_b)
{
    ...
    for (const auto& source_b : list_b) {
        if (CSPSourceIsSchemeOnly(*source_b)) {
            ...
            else if (source_b->scheme == url::kHttpScheme && schemes_a.contains(url::kHttpsScheme)) {
                intersection.emplace(url::kHttpsScheme);
            } else if (source_b->scheme == url::kWsScheme && schemes_a.contains(url::kWssScheme)) {
...
std::vector<mojom::CSPSourcePtr> ExpandSchemeStarAndSelf(const mojom::CSPSourceList& source_list, const mojom::CSPSource* self)
{
    ...
    if (source_list.allow_star) {
        result.push_back(mojom::CSPSource::New(url::kFtpScheme, "", url::PORT_UNSPECIFIED, "", false, false));
        result.push_back(mojom::CSPSource::New(url::kWsScheme, "", url::PORT_UNSPECIFIED, "", false, false));
        result.push_back(mojom::CSPSource::New(url::kHttpScheme, "", url::PORT_UNSPECIFIED, "", false, false));
        if (self && !self->scheme.empty()) {
            result.push_back(mojom::CSPSource::New(self->scheme, "", url::PORT_UNSPECIFIED, "", false, false));
        }
    }
...
std::vector<mojom::CSPSourcePtr> IntersectSources(
    const mojom::CSPSourceList& source_list_a, const std::vector<mojom::CSPSourcePtr>& source_list_b, const mojom::CSPSource* self)
{
    ...
    for (const auto& it : schemes) {
        ...
        if ((it != url::kHttpsScheme || !schemes.count(url::kHttpScheme)) && (it != url::kWssScheme || !schemes.count(url::kWsScheme))) {
            normalized.emplace_back(mojom::CSPSource::New(it, "", url::PORT_UNSPECIFIED, "", false, false));
        }
    }

    std::vector<mojom::CSPSourcePtr> sources_a = ExpandSchemeStarAndSelf(source_list_a, self);
    for (auto& source_a : sources_a) {
        ...
        for (auto& source_b : source_list_b) {
            ...
            if (schemes.contains(source_b->scheme))
                continue;
            if (mojom::CSPSourcePtr local_match = CSPSourcesIntersect(*source_a, *source_b)) {
...
bool UrlSourceListSubsumes(const std::vector<mojom::CSPSourcePtr>& source_list_a, const std::vector<mojom::CSPSourcePtr>& source_list_b)
{
    ...
    return base::ranges::all_of(source_list_b, [&](const auto& source_b) {
        return base::ranges::any_of(source_list_a, [&](const auto& source_a) { return CSPSourceSubsumes(*source_a, *source_b); });
    });
}

} // namespace

CSPCheckResult CheckCSPSourceList(mojom::CSPDirectiveName directive_name, const mojom::CSPSourceList& source_list, const GURL& url,
    const mojom::CSPSource& self_source, bool has_followed_redirect, bool is_opaque_fenced_frame)
{
    ...
    if (source_list.allow_self
        && CheckCSPSource(self_source, url, self_source, CSPSourceContext::ContentSecurityPolicy, has_followed_redirect, is_opaque_fenced_frame)) {
        return CSPCheckResult::Allowed();
    }

    if (AllowFromSources(url, source_list.sources, self_source, has_followed_redirect, is_opaque_fenced_frame)) {
        return CSPCheckResult::Allowed();
    }
    ...
    if (source_list.allow_star) {
        ...
        if (url.SchemeIs("ftp")) {
            return base::FeatureList::IsEnabled(features::kCspStopMatchingWildcardDirectivesToFtp) ? CSPCheckResult::Blocked()
...
bool CSPSourceListSubsumes(const mojom::CSPSourceList& source_list_a, const std::vector<const mojom::CSPSourceList*>& source_list_b, CSPDirectiveName directive,
    const mojom::CSPSource* origin_b)
{
    ...
    bool is_hash_or_nonce_present_b = !(*it)->nonces.empty() || !(*it)->hashes.empty();
    base::flat_set<std::string> nonces_b((*it)->nonces);
    base::flat_set<mojom::CSPHashSourcePtr> hashes_b(mojo::Clone((*it)->hashes));

    std::vector<mojom::CSPSourcePtr> normalized_sources_b = ExpandSchemeStarAndSelf(**it, origin_b);
    ...
    for (; it != source_list_b.end(); ++it) {
        ...
        is_hash_or_nonce_present_b = is_hash_or_nonce_present_b && (!(*it)->nonces.empty() || !(*it)->hashes.empty());
        base::flat_set<std::string> item_nonces((*it)->nonces);
        IntersectNonces(nonces_b, item_nonces);
        base::flat_set<mojom::CSPHashSourcePtr> item_hashes(mojo::Clone((*it)->hashes));
        IntersectHashes(hashes_b, item_hashes);
        normalized_sources_b = IntersectSources(**it, normalized_sources_b, origin_b);
    }
    ...
    if (!hashes_b.empty()) {
        base::flat_set<mojom::CSPHashSourcePtr> hashes_a(mojo::Clone(source_list_a.hashes));
    ...
    if (IsScriptDirective(directive) || IsStyleDirective(directive)) {
        ...
        bool allow_all_inline_b = allow_inline_b && !is_hash_or_nonce_present_b && (!IsScriptDirective(directive) || !allow_dynamic_b);
        if (!AllowAllInline(directive, source_list_a) && allow_all_inline_b)
            return false;
    }

    if (IsScriptDirective(directive) && (source_list_a.allow_dynamic || allow_dynamic_b)) {
    ...
    std::vector<mojom::CSPSourcePtr> normalized_sources_a = ExpandSchemeStarAndSelf(source_list_a, origin_b);
...
{
    bool is_none = !source_list->allow_self && !source_list->allow_star && source_list->sources.empty();
    if (is_none)
        return "'none'";
    if (source_list->allow_star)
        return "*";
