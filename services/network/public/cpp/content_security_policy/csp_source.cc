...
enum class PortMatchingResult { NotMatching, MatchingWildcard, MatchingUpgrade, MatchingExact };
enum class SchemeMatchingResult { NotMatching, MatchingUpgrade, MatchingExact };

SchemeMatchingResult MatchScheme(const std::string& scheme_a, const std::string& scheme_b)
{
    if (scheme_a == scheme_b)
        return SchemeMatchingResult::MatchingExact;
    if ((scheme_a == url::kHttpScheme && scheme_b == url::kHttpsScheme) || (scheme_a == url::kWsScheme && scheme_b == url::kWssScheme)) {
        return SchemeMatchingResult::MatchingUpgrade;
    }
    return SchemeMatchingResult::NotMatching;
}

SchemeMatchingResult SourceAllowScheme(const mojom::CSPSource& source, const GURL& url, const mojom::CSPSource& self_source)
{
    ...
    const std::string& allowed_scheme = source.scheme.empty() ? self_source.scheme : source.scheme;
...
PortMatchingResult SourceAllowPort(const mojom::CSPSource& source, int port, const std::string& scheme)
{
...
PortMatchingResult SourceAllowPort(const mojom::CSPSource& source, const GURL& url)
{
...
bool SourceAllowPath(const mojom::CSPSource& source, const std::string& path)
{
    ...
    if (base::EndsWith(source.path, "/", base::CompareCase::SENSITIVE)) {
        return base::StartsWith(path_decoded, source.path, base::CompareCase::SENSITIVE);
    }
...
bool SourceAllowPath(const mojom::CSPSource& source, const GURL& url, bool has_followed_redirect)
{
...
{
    return result == SchemeMatchingResult::MatchingUpgrade && context == CSPSourceContext::ContentSecurityPolicy;
}
...
bool CheckCSPSource(const mojom::CSPSource& source, const GURL& url, const mojom::CSPSource& self_source, CSPSourceContext context, bool has_followed_redirect,
    bool is_opaque_fenced_frame)
{
    ...
    if (CSPSourceIsSchemeOnly(source)) {
        ...
        return SourceAllowScheme(source, url, self_source) != SchemeMatchingResult::NotMatching;
    }

    // Only allow 'https://*:*' for opaque fenced frames. 'https://*' is not
    // allowed as it could leak data about ports.
    if (is_opaque_fenced_frame) {
        return source.scheme == url::kHttpsScheme && source.is_host_wildcard && source.is_port_wildcard;
    }

    PortMatchingResult portResult = SourceAllowPort(source, url);
    SchemeMatchingResult schemeResult = SourceAllowScheme(source, url, self_source);
    ...
    return schemeResult != SchemeMatchingResult::NotMatching && SourceAllowHost(source, url) && portResult != PortMatchingResult::NotMatching
        && SourceAllowPath(source, url, has_followed_redirect);
}

mojom::CSPSourcePtr CSPSourcesIntersect(const mojom::CSPSource& source_a, const mojom::CSPSource& source_b)
{
    ...
    if (MatchScheme(source_a.scheme, source_b.scheme) != SchemeMatchingResult::NotMatching) {
        result->scheme = source_b.scheme;
    } else if (MatchScheme(source_b.scheme, source_a.scheme) != SchemeMatchingResult::NotMatching) {
    ...
    const std::string host_a = (source_a.is_host_wildcard ? "*." : "") + source_a.host;
    const std::string host_b = (source_b.is_host_wildcard ? "*." : "") + source_b.host;
    ...
    if (source_b.is_port_wildcard) {
    ...
    } else if (SourceAllowPort(source_a, source_b.port, source_b.scheme) != PortMatchingResult::NotMatching &&
        ...
        !(source_a.port != url::PORT_UNSPECIFIED && source_b.port == url::PORT_UNSPECIFIED)) {
        result->port = source_b.port;
    } else if (SourceAllowPort(source_b, source_a.port, source_a.scheme) != PortMatchingResult::NotMatching) {
        result->port = source_a.port;
    } else {
        return nullptr;
    }
...
bool CSPSourceSubsumes(const mojom::CSPSource& source_a, const mojom::CSPSource& source_b)
{
    // If the original source expressions didn't have a scheme, we should have
    // filled that already with origin's scheme.
    DCHECK(!source_a.scheme.empty());
    ...
    if (!SourceAllowHost(source_a, (source_b.is_host_wildcard ? "*." : "") + source_b.host)) {
        return false;
    }

    if (source_b.is_port_wildcard && !source_a.is_port_wildcard)
        return false;
    PortMatchingResult port_matching = SourceAllowPort(source_a, source_b.port, source_b.scheme);
