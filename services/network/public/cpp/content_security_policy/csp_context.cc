...
bool ShouldCheckPolicy(const mojom::ContentSecurityPolicyPtr& policy, CSPContext::CheckCSPDisposition check_csp_disposition)
{
...
CSPCheckResult CSPContext::IsAllowedByCsp(const std::vector<mojom::ContentSecurityPolicyPtr>& policies, mojom::CSPDirectiveName directive_name, const GURL& url,
    const GURL& url_before_redirects, bool has_followed_redirect, const mojom::SourceLocationPtr& source_location, CheckCSPDisposition check_csp_disposition,
    bool is_form_submission, bool is_opaque_fenced_frame)
{
    ...
    for (const auto& policy : policies) {
        if (ShouldCheckPolicy(policy, check_csp_disposition)) {
            result &= CheckContentSecurityPolicy(
                policy, directive_name, url, url_before_redirects, has_followed_redirect, this, source_location, is_form_submission, is_opaque_fenced_frame);
        }
    }

    DCHECK(result.IsAllowed() || check_csp_disposition != CSPContext::CHECK_REPORT_ONLY_CSP);
...
void CSPContext::SanitizeDataForUseInCspViolation(mojom::CSPDirectiveName directive, GURL* blocked_url, network::mojom::SourceLocation* source_location) const
{
}

void CSPContext::ReportContentSecurityPolicyViolation(network::mojom::CSPViolationPtr violation)
{
}
