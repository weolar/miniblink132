...
using DirectivesMap = std::vector<std::pair<std::string_view, std::string_view>>;

namespace features {
BASE_FEATURE(kCspStopMatchingWildcardDirectivesToFtp, "CspStopMatchingWildcardDirectivesToFtp", base::FEATURE_ENABLED_BY_DEFAULT);
}
...
void ReportViolation(CSPContext* context, const mojom::ContentSecurityPolicyPtr& policy, const CSPDirectiveName effective_directive_name,
    const CSPDirectiveName directive_name, const GURL& url, const mojom::SourceLocationPtr& source_location)
{
    ...
    // renderers.
    GURL blocked_url = (directive_name == CSPDirectiveName::FrameAncestors) ? GURL(ToString(*policy->self_origin)) : url;
    std::string blocked_url_scheme = blocked_url.scheme();
    auto safe_source_location = source_location ? source_location->Clone() : mojom::SourceLocation::New();

    context->SanitizeDataForUseInCspViolation(directive_name, &blocked_url, safe_source_location.get());
    ...
    message << base::ReplaceStringPlaceholders(ErrorMessage(directive_name),
        { ElideURLForReportViolation(blocked_url), ToString(effective_directive_name) + " " + ToString(policy->directives[effective_directive_name]) },
        nullptr);

    if (effective_directive_name != directive_name) {
        message << " Note that '" << ToString(directive_name) << "' was not explicitly set, so '" << ToString(effective_directive_name)
    ...
    context->ReportContentSecurityPolicyViolation(mojom::CSPViolation::New(ToString(effective_directive_name), ToString(directive_name), message.str(),
        blocked_url, policy->report_endpoints, policy->use_reporting_api, policy->header->header_value, policy->header->type, std::move(safe_source_location)));
}
...
bool ShouldBypassContentSecurityPolicy(CSPContext* context, CSPDirectiveName directive, const GURL& url)
{
...
bool ParseScheme(std::string_view scheme, mojom::CSPSource* csp_source)
{
    ...
    auto is_scheme_character = [](auto c) { return base::IsAsciiAlpha(c) || base::IsAsciiDigit(c) || c == '+' || c == '-' || c == '.'; };
...
bool ParseHost(std::string_view host, mojom::CSPSource* csp_source)
{
    ...
    std::vector<std::string_view> host_pieces = base::SplitStringPiece(host, ".", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);
    for (int i = 0; std::string_view piece : host_pieces) {
        // Only a trailing dot is allowed.
        if ((piece.empty() && i + 1 < std::ssize(host_pieces))
            || !base::ranges::all_of(piece, [](auto c) { return base::IsAsciiAlpha(c) || base::IsAsciiDigit(c) || c == '-'; })) {
            return false;
        }
        ++i;
    }
...
bool ParsePort(std::string_view port, mojom::CSPSource* csp_source)
{
    ...
    if (!base::ranges::all_of(port, base::IsAsciiDigit<std::string_view::value_type>)) {
        return false;
    }
...
bool ParsePath(std::string_view path, mojom::CSPSource* csp_source)
{
    ...
    url::DecodeURLEscapeSequences(path, url::DecodeURLMode::kUTF8OrIsomorphic, &unescaped);
...
{
    return base::IsAsciiAlpha(c) || base::IsAsciiDigit(c) || c == '+' || c == '-' || c == '_' || c == '/';
}

int EatChar(std::string_view::const_iterator* it, std::string_view::const_iterator end, bool (*predicate)(char))
{
    int count = 0;
    while (*it != end) {
        if (!predicate(**it))
...
bool ParseNonce(std::string_view expression, std::string* nonce)
{
    ...
    std::string_view subexpression = expression.substr(7, expression.length() - 8);
...
bool ParseHash(std::string_view expression, mojom::CSPHashSource* hash)
{
    static const SupportedPrefixesStruct SupportedPrefixes[]
        = { { "'sha256-", 8, mojom::CSPHashAlgorithm::SHA256 }, { "'sha384-", 8, mojom::CSPHashAlgorithm::SHA384 },
              { "'sha512-", 8, mojom::CSPHashAlgorithm::SHA512 }, { "'sha-256-", 9, mojom::CSPHashAlgorithm::SHA256 },
              { "'sha-384-", 9, mojom::CSPHashAlgorithm::SHA384 }, { "'sha-512-", 9, mojom::CSPHashAlgorithm::SHA512 } };

    for (auto item : SupportedPrefixes) {
        if (base::StartsWith(expression, item.prefix, base::CompareCase::INSENSITIVE_ASCII)) {
            std::string_view subexpression = expression.substr(item.prefix_length, expression.length() - item.prefix_length - 1);
            if (!IsBase64(subexpression))
                return false;
...
mojom::CSPSourceListPtr ParseSourceList(CSPDirectiveName directive_name, std::string_view directive_value, std::vector<std::string>& parsing_errors)
{
    std::string_view value = base::TrimString(directive_value, base::kWhitespaceASCII, base::TRIM_ALL);
    ...
    std::vector<std::string_view> tokens = base::SplitStringPiece(value, base::kWhitespaceASCII, base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    ...
    for (const auto& expression : tokens) {
        ...
        if (ToCSPDirectiveName(expression) != CSPDirectiveName::Unknown) {
            parsing_errors.emplace_back(base::StringPrintf("The Content-Security-Policy directive '%s' contains '%s' as a "
        ...
        if (ParseSource(directive_name, expression, csp_source.get(), parsing_errors)) {
        ...
        if (directive_name == CSPDirectiveName::FrameAncestors) {
            ...
            parsing_errors.emplace_back(base::StringPrintf("The Content-Security-Policy directive 'frame-ancestors' does not "
        ...
        if (base::EqualsCaseInsensitiveASCII(expression, "'inline-speculation-rules'")) {
            if (directive_name == CSPDirectiveName::ScriptSrc || directive_name == CSPDirectiveName::ScriptSrcElem) {
        ...
        parsing_errors.emplace_back(base::StringPrintf("The source list for the Content Security Policy directive '%s' "
                                                       "contains an invalid source: '%s'. It will be ignored.",
            ToString(directive_name).c_str(), std::string(expression).c_str()));
    }

    if (contains_none && base::ranges::any_of(tokens, [](const auto& token) -> bool {
            return !base::EqualsCaseInsensitiveASCII(token, "'report-sample'") && !base::EqualsCaseInsensitiveASCII(token, "'none'");
        })) {
        parsing_errors.emplace_back(base::StringPrintf("The Content-Security-Policy directive '%s' contains the keyword "
...
network::mojom::CSPRequireTrustedTypesFor ParseRequireTrustedTypesFor(std::string_view value, std::vector<std::string>& parsing_errors)
{
    network::mojom::CSPRequireTrustedTypesFor out = network::mojom::CSPRequireTrustedTypesFor::None;
    for (const auto expression : base::SplitStringPiece(value, base::kWhitespaceASCII, base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY)) {
        if (expression == "'script'") {
            out = network::mojom::CSPRequireTrustedTypesFor::Script;
        } else {
            const char* hint = nullptr;
            if (expression == "script" || expression == "scripts" || expression == "'scripts'") {
                hint = " Did you mean 'script'?";
            }

            parsing_errors.emplace_back(base::StringPrintf("Invalid expression in 'require-trusted-types-for' "
                                                           "Content Security Policy directive: %s.%s\n",
                std::string(expression).c_str(), hint));
        }
    }
    if (out == network::mojom::CSPRequireTrustedTypesFor::None)
        parsing_errors.emplace_back(base::StringPrintf("'require-trusted-types-for' Content Security Policy "
...
{
    return base::ranges::all_of(value, [](char c) { return base::IsAsciiAlpha(c) || base::IsAsciiDigit(c) || base::Contains("-#=_/@.%", c); });
}

// Parse the 'trusted-types' directive.
// https://w3c.github.io/trusted-types/dist/spec/#trusted-types-csp-directive
network::mojom::CSPTrustedTypesPtr ParseTrustedTypes(std::string_view value, std::vector<std::string>& parsing_errors)
{
    auto out = network::mojom::CSPTrustedTypes::New();
    std::vector<std::string_view> pieces = base::SplitStringPiece(value, base::kWhitespaceASCII, base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    ...
    for (const auto expression : pieces) {
        if (expression == "*")
            ...
            parsing_errors.emplace_back("The value of the Content Security Policy directive "
            ...
            parsing_errors.emplace_back(base::StringPrintf("The value of the Content Security Policy directive "
...
void ParseReportDirective(const GURL& request_url, std::string_view value, bool using_reporting_api, std::vector<std::string>* report_endpoints,
    std::vector<std::string>& parsing_errors)
{
    std::vector<std::string_view> values = base::SplitStringPiece(value, base::kWhitespaceASCII, base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);

    if (using_reporting_api && values.size() > 1) {
        parsing_errors.emplace_back("The Content Security Policy directive 'report-to' contains more than "
    ...
    for (const auto& uri : values) {
        ...
        if (using_reporting_api) {
            ...
            if (request_url.SchemeIs(url::kHttpsScheme) && !IsUrlPotentiallyTrustworthy(url)) {
                parsing_errors.emplace_back(base::StringPrintf("The Content Security Policy directive 'report-uri' specifies as "
            ...
            if (!url.is_valid()) {
                parsing_errors.emplace_back(base::StringPrintf("The Content Security Policy directive 'report-uri' specifies an "
...
void WarnIfDirectiveValueNotEmpty(const std::pair<std::string_view, std::string_view>& directive, std::vector<std::string>& parsing_errors)
{
    if (!directive.second.empty()) {
        parsing_errors.emplace_back(base::StringPrintf("The Content Security Policy directive '%s' should be empty, but was "
                                                       "delivered with a value of '%s'. The directive has been applied, and "
                                                       "the value ignored.",
            std::string(directive.first).c_str(), std::string(directive.second).c_str()));
    }
}
...
void AddContentSecurityPolicyFromHeader(std::string_view header, mojom::ContentSecurityPolicyType type, mojom::ContentSecurityPolicySource source,
    const GURL& base_url, mojom::ContentSecurityPolicyPtr& out)
{
    ...
    for (auto directive : directives) {
        if (!base::ranges::all_of(directive.first, IsDirectiveNameCharacter)) {
            out->parsing_errors.emplace_back(base::StringPrintf("The Content-Security-Policy directive name '%s' contains one or "
        ...
        if (out->raw_directives.count(directive_name)) {
            out->parsing_errors.emplace_back(
                base::StringPrintf("Ignoring duplicate Content-Security-Policy directive '%s'.", std::string(directive.first).c_str()));
            continue;
        }
        ...
        if (!base::ranges::all_of(directive.second, IsDirectiveValueCharacter)) {
            out->parsing_errors.emplace_back(base::StringPrintf("The value for the Content-Security-Policy directive '%s' contains "
        ...
        if (type == mojom::ContentSecurityPolicyType::kReport && !SupportedInReportOnly(directive_name)) {
            out->parsing_errors.emplace_back(base::StringPrintf("The Content Security Policy directive '%s' is "
        ...
        case CSPDirectiveName::Sandbox:
            ...
            // Blink's CSP parser is already in charge of it.
            {
                ...
                if (!sandbox.error_message.empty())
                    out->parsing_errors.emplace_back("Error while parsing the 'sandbox' Content Security Policy "
                                                     "directive: "
...
std::pair<CSPDirectiveName, const mojom::CSPSourceList*> GetSourceList(CSPDirectiveName directive, const mojom::ContentSecurityPolicy& policy)
{
    for (CSPDirectiveName effective_directive = directive; effective_directive != CSPDirectiveName::Unknown;
         effective_directive = CSPFallbackDirective(effective_directive, directive)) {
...
CSPCheckResult& CSPCheckResult::operator&=(const CSPCheckResult& other)
{
    ...
    allowed_if_wildcard_does_not_match_ws_ &= other.allowed_if_wildcard_does_not_match_ws_;
    allowed_if_wildcard_does_not_match_ftp_ &= other.allowed_if_wildcard_does_not_match_ftp_;
    return *this;
}

bool CSPCheckResult::operator==(const CSPCheckResult& other) const
{
    return allowed_ == other.allowed_ && allowed_if_wildcard_does_not_match_ws_ == other.allowed_if_wildcard_does_not_match_ws_
        && allowed_if_wildcard_does_not_match_ftp_ == other.allowed_if_wildcard_does_not_match_ftp_;
}
...
CSPCheckResult::CSPCheckResult(bool allowed, bool allowed_if_wildcard_does_not_match_ws, bool allowed_if_wildcard_does_not_match_ftp)
    : allowed_(allowed)
    , allowed_if_wildcard_does_not_match_ws_(allowed_if_wildcard_does_not_match_ws)
    , allowed_if_wildcard_does_not_match_ftp_(allowed_if_wildcard_does_not_match_ftp)
{
}

CSPDirectiveName CSPFallbackDirective(CSPDirectiveName directive, CSPDirectiveName original_directive)
{
    switch (directive) {
    case CSPDirectiveName::ConnectSrc:
    case CSPDirectiveName::FontSrc:
...
void AddContentSecurityPolicyFromHeaders(const net::HttpResponseHeaders& headers, const GURL& base_url, std::vector<mojom::ContentSecurityPolicyPtr>* out)
{
    size_t iter = 0;

    while (std::optional<std::string_view> header_value = headers.EnumerateHeader(&iter, "content-security-policy")) {
        std::vector<mojom::ContentSecurityPolicyPtr> parsed
            = ParseContentSecurityPolicies(*header_value, mojom::ContentSecurityPolicyType::kEnforce, mojom::ContentSecurityPolicySource::kHTTP, base_url);
        out->insert(out->end(), std::make_move_iterator(parsed.begin()), std::make_move_iterator(parsed.end()));
    }

    iter = 0;
    while (std::optional<std::string_view> header_value = headers.EnumerateHeader(&iter, "content-security-policy-report-only")) {
        std::vector<mojom::ContentSecurityPolicyPtr> parsed
            = ParseContentSecurityPolicies(*header_value, mojom::ContentSecurityPolicyType::kReport, mojom::ContentSecurityPolicySource::kHTTP, base_url);
        out->insert(out->end(), std::make_move_iterator(parsed.begin()), std::make_move_iterator(parsed.end()));
    }
}

std::vector<mojom::ContentSecurityPolicyPtr> ParseContentSecurityPolicies(
    std::string_view header_value, mojom::ContentSecurityPolicyType type, mojom::ContentSecurityPolicySource source, const GURL& base_url)
{
...
mojom::AllowCSPFromHeaderValuePtr ParseAllowCSPFromHeader(const net::HttpResponseHeaders& headers)
{
    std::optional<std::string> allow_csp_from = headers.GetNormalizedHeader("Allow-CSP-From");
    if (!allow_csp_from) {
        return nullptr;
    }

    std::string_view trimmed = base::TrimWhitespaceASCII(*allow_csp_from, base::TRIM_ALL);
    ...
    if (!parsed_url.is_valid()) {
        return mojom::AllowCSPFromHeaderValue::NewErrorMessage("The 'Allow-CSP-From' header contains neither '*' nor a valid origin.");
    }
    return mojom::AllowCSPFromHeaderValue::NewOrigin(url::Origin::Create(parsed_url));
}

bool ParseSource(CSPDirectiveName directive_name, std::string_view expression, mojom::CSPSource* csp_source, std::vector<std::string>& parsing_errors)
{
    ...
    if (expression[0] == ':') {
        ...
        std::string_view port = expression.substr(1, port_end == std::string::npos ? std::string::npos : port_end - 1);
    ...
    if (path_parsed && position != std::string::npos) {
        const char* ignoring = expression[position] == '?' ? "The query component, including the '?', will be ignored."
                                                           : "The fragment identifier, including the '#', will be ignored.";
        parsing_errors.emplace_back(base::StringPrintf("The source list for Content Security Policy directive '%s' "
                                                       "contains a source with an invalid path: '%s'. %s",
            ToString(directive_name).c_str(), std::string(expression).c_str(), ignoring));
    }

    return path_parsed;
}

CSPCheckResult CheckContentSecurityPolicy(const mojom::ContentSecurityPolicyPtr& policy, CSPDirectiveName directive_name, const GURL& url,
    const GURL& url_before_redirects, bool has_followed_redirect, CSPContext* context, const mojom::SourceLocationPtr& source_location, bool is_form_submission,
    bool is_opaque_fenced_frame)
{
    DCHECK(policy->self_origin);

    if (is_opaque_fenced_frame && directive_name != CSPDirectiveName::FencedFrameSrc)
        return CSPCheckResult::Blocked();

    if (!is_opaque_fenced_frame && ShouldBypassContentSecurityPolicy(context, directive_name, url)) {
        return CSPCheckResult::Allowed();
    }

    for (CSPDirectiveName effective_directive_name = directive_name; effective_directive_name != CSPDirectiveName::Unknown;
         effective_directive_name = CSPFallbackDirective(effective_directive_name, directive_name)) {
        ...
        CSPCheckResult result = CheckCSPSourceList(directive_name, *source_list, url, *(policy->self_origin), has_followed_redirect, is_opaque_fenced_frame);

        if (!result) {
            ReportViolation(context, policy, effective_directive_name, directive_name,
                is_opaque_fenced_frame
                    ? GURL("urn:uuid")
                    : (directive_name == CSPDirectiveName::FrameSrc || directive_name == CSPDirectiveName::FencedFrameSrc ? url : url_before_redirects),
                source_location);
        }

        return policy->header->type == mojom::ContentSecurityPolicyType::kReport ? CSPCheckResult::Allowed() : result;
    }
    return CSPCheckResult::Allowed();
}

bool ShouldUpgradeInsecureRequest(const std::vector<mojom::ContentSecurityPolicyPtr>& policies)
{
...
bool ShouldTreatAsPublicAddress(const std::vector<mojom::ContentSecurityPolicyPtr>& policies)
{
    for (const auto& policy : policies) {
...
bool IsValidRequiredCSPAttr(const std::vector<mojom::ContentSecurityPolicyPtr>& policy, const mojom::ContentSecurityPolicy* context, std::string& error_message)
{
    ...
    if (!policy[0]->report_endpoints.empty() ||
        ...
        // endpoints.
        policy[0]->raw_directives.contains(mojom::CSPDirectiveName::ReportURI) || policy[0]->raw_directives.contains(mojom::CSPDirectiveName::ReportTo)) {
        error_message = "The csp attribute cannot contain the directives 'report-to' or "
                        "'report-uri'.";
        return false;
    }

    if (context && !Subsumes(*context, policy)) {
        error_message = "The csp attribute Content-Security-Policy is not subsumed by the "
...
bool Subsumes(const mojom::ContentSecurityPolicy& policy_a, const std::vector<mojom::ContentSecurityPolicyPtr>& policies_b)
{
    ...
    static const CSPDirectiveName directives[] = { CSPDirectiveName::ChildSrc, CSPDirectiveName::ConnectSrc, CSPDirectiveName::FontSrc,
        ...
        CSPDirectiveName::ScriptSrc, CSPDirectiveName::ScriptSrcAttr, CSPDirectiveName::ScriptSrcElem, CSPDirectiveName::StyleSrc,
        CSPDirectiveName::StyleSrcAttr, CSPDirectiveName::StyleSrcElem, CSPDirectiveName::WorkerSrc, CSPDirectiveName::BaseURI,
        CSPDirectiveName::FrameAncestors, CSPDirectiveName::FormAction, CSPDirectiveName::FencedFrameSrc };
    ...
    return base::ranges::all_of(directives, [&](CSPDirectiveName directive) {
        ...
        return CSPSourceListSubsumes(*required.second, returned, required.first, origin_b);
    });
}
...
bool AllowCspFromAllowOrigin(const url::Origin& request_origin, const network::mojom::AllowCSPFromHeaderValue* allow_csp_from)
{
    if (!allow_csp_from) {
        return false;
    }
    ...
    if (allow_csp_from->is_origin() && request_origin.IsSameOriginWith(allow_csp_from->get_origin())) {
        return true;
    }
...
bool AllowsBlanketEnforcementOfRequiredCSP(const url::Origin& request_origin, const GURL& response_url,
    const network::mojom::AllowCSPFromHeaderValue* allow_csp_from, network::mojom::ContentSecurityPolicyPtr& required_csp)
{
    if (response_url.SchemeIs(url::kAboutScheme) || response_url.SchemeIs(url::kDataScheme) || response_url.SchemeIsFile() || response_url.SchemeIsFileSystem()
        || response_url.SchemeIsBlob()) {
