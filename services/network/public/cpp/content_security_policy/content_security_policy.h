...
private:
    CSPCheckResult(bool allowed, bool allowed_if_wildcard_does_not_match_ws, bool allowed_if_wildcard_does_not_match_ftp);
...
COMPONENT_EXPORT(NETWORK_CPP)
mojom::CSPDirectiveName CSPFallbackDirective(mojom::CSPDirectiveName directive, mojom::CSPDirectiveName original_directive);
...
std::vector<mojom::ContentSecurityPolicyPtr> ParseContentSecurityPolicies(
    std::string_view header, mojom::ContentSecurityPolicyType type, mojom::ContentSecurityPolicySource source, const GURL& base_url);
...
COMPONENT_EXPORT(NETWORK_CPP)
bool ParseSource(mojom::CSPDirectiveName directive_name, std::string_view expression, mojom::CSPSource* csp_source, std::vector<std::string>& parsing_errors);
...
COMPONENT_EXPORT(NETWORK_CPP)
CSPCheckResult CheckContentSecurityPolicy(const mojom::ContentSecurityPolicyPtr& policy, mojom::CSPDirectiveName directive, const GURL& url,
...
bool IsValidRequiredCSPAttr(
    const std::vector<mojom::ContentSecurityPolicyPtr>& policy, const mojom::ContentSecurityPolicy* context, std::string& error_message);
