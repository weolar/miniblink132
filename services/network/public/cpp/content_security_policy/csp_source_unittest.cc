...
network::mojom::CSPSourcePtr CSPSource(const std::string& raw)
{
    ...
    return std::move(policies[0]->directives[mojom::CSPDirectiveName::ScriptSrc]->sources[0]);
}
...
class CSPSourceTest : public testing::TestWithParam<std::tuple<CSPSourceContext, NonSpecialUrlBehavior>> {
public:
    CSPSourceTest()
    {
        scoped_feature_list_.InitWithFeatureState(
            url::kStandardCompliantNonSpecialSchemeURLParsing, std::get<1>(GetParam()) == NonSpecialUrlBehavior::Compliant);
    }

    bool Allow(const network::mojom::CSPSource& source, const GURL& url, const network::mojom::CSPSource& self_source = no_self, bool is_redirect = false,
...
INSTANTIATE_TEST_SUITE_P(All, CSPSourceTest,
    ::testing::Combine(testing::Values(CSPSourceContext::ContentSecurityPolicy, CSPSourceContext::PermissionsPolicy),
        testing::Values(NonSpecialUrlBehavior::Compliant, NonSpecialUrlBehavior::NonCompliant)));

TEST_P(CSPSourceTest, BasicMatching)
{
    auto source = network::mojom::CSPSource::New("http", "example.com", 8000, "/foo/", false, false);
...
TEST_P(CSPSourceTest, AllowScheme)
{
    // http -> {http, https}.
    {
        auto source = network::mojom::CSPSource::New("http", "", url::PORT_UNSPECIFIED, "", false, false);
    ...
    // ws -> {ws, wss}.
    {
        auto source = network::mojom::CSPSource::New("ws", "", url::PORT_UNSPECIFIED, "", false, false);
    ...
    {
        auto source = network::mojom::CSPSource::New("ftp", "", url::PORT_UNSPECIFIED, "", false, false);
    ...
    {
        auto source = network::mojom::CSPSource::New("https", "", url::PORT_UNSPECIFIED, "", false, false);
        EXPECT_TRUE(Allow(*source, GURL("https://a.com")));
        EXPECT_FALSE(Allow(*source, GURL("http://a.com")));
    }
    ...
    {
        auto source = network::mojom::CSPSource::New("wss", "", url::PORT_UNSPECIFIED, "", false, false);
        EXPECT_TRUE(Allow(*source, GURL("wss://a.com")));
        EXPECT_FALSE(Allow(*source, GURL("ws://a.com")));
    }
    ...
    {
        auto source = network::mojom::CSPSource::New("", "a.com", url::PORT_UNSPECIFIED, "", false, false);
        EXPECT_FALSE(Allow(*source, GURL("http://a.com")));

        {
            // Self's scheme is http.
            auto self_source = network::mojom::CSPSource::New("http", "a.com", 80, "", false, false);
            EXPECT_TRUE(Allow(*source, GURL("http://a.com"), *self_source));
            // Upgrades are disallowed for permission policies.
            EXPECT_EQ(Allow(*source, GURL("https://a.com"), *self_source), !IsPermissionsPolicyContext());
            EXPECT_FALSE(Allow(*source, GURL("ftp://a.com"), *self_source));
        }

        {
            // Self's is https.
            auto self_source = network::mojom::CSPSource::New("https", "a.com", 443, "", false, false);
...

        {
            ...
            auto self_source = network::mojom::CSPSource::New("ftp", "a.com", 21, "", false, false);
            EXPECT_FALSE(Allow(*source, GURL("http://a.com"), *self_source));
...

        {
            ...
            auto self_source = network::mojom::CSPSource::New("non-standard-scheme", "a.com", url::PORT_UNSPECIFIED, "", false, false);
            EXPECT_FALSE(Allow(*source, GURL("http://a.com"), *self_source));
            EXPECT_FALSE(Allow(*source, GURL("non-standard-scheme://a.com"), *self_source));
        }
...
{
    auto self_source = network::mojom::CSPSource::New("http", "example.com", 80, "", false, false);
    // Host is * (source-expression = "http://*")
    {
        auto source = network::mojom::CSPSource::New("http", "", url::PORT_UNSPECIFIED, "", true, false);
        EXPECT_TRUE(Allow(*source, GURL("http://a.com"), *self_source));
    ...
    // Host is *.foo.bar
    {
        auto source = network::mojom::CSPSource::New("", "foo.bar", url::PORT_UNSPECIFIED, "", true, false);
        EXPECT_FALSE(Allow(*source, GURL("http://a.com"), *self_source));
    ...
    // Host is exact.
    {
        auto source = network::mojom::CSPSource::New("", "foo.bar", url::PORT_UNSPECIFIED, "", false, false);
        EXPECT_TRUE(Allow(*source, GURL("http://foo.bar"), *self_source));
...
TEST_P(CSPSourceTest, AllowPort)
{
    ...
    // Source's port unspecified.
    {
        ...
        // Upgrades are disallowed for permission policies.
        EXPECT_EQ(Allow(*source, GURL("https://a.com:443"), *self_source), !IsPermissionsPolicyContext());
    ...
    // Source's port is "*".
    {
        auto source = network::mojom::CSPSource::New("", "a.com", url::PORT_UNSPECIFIED, "", false, true);
        EXPECT_TRUE(Allow(*source, GURL("http://a.com"), *self_source));
        EXPECT_TRUE(Allow(*source, GURL("http://a.com:80"), *self_source));
        EXPECT_TRUE(Allow(*source, GURL("http://a.com:8080"), *self_source));
        // Upgrades are disallowed for permission policies.
        EXPECT_EQ(Allow(*source, GURL("https://a.com:8080"), *self_source), !IsPermissionsPolicyContext());
        EXPECT_EQ(Allow(*source, GURL("https://a.com:0"), *self_source), !IsPermissionsPolicyContext());
        EXPECT_EQ(Allow(*source, GURL("https://a.com"), *self_source), !IsPermissionsPolicyContext());
    }

    // Source has a port.
    {
        auto source = network::mojom::CSPSource::New("", "a.com", 80, "", false, false);
        EXPECT_TRUE(Allow(*source, GURL("http://a.com:80"), *self_source));
        EXPECT_TRUE(Allow(*source, GURL("http://a.com"), *self_source));
        EXPECT_FALSE(Allow(*source, GURL("http://a.com:8080"), *self_source));
        // Upgrades are disallowed for permission policies.
    ...
    {
        auto source = network::mojom::CSPSource::New("http", "", 111, "", true, false);
...
TEST_P(CSPSourceTest, AllowPath)
{
    ...
    // Path to a file
    {
        auto source = network::mojom::CSPSource::New("", "a.com", url::PORT_UNSPECIFIED, "/path/to/file", false, false);
        EXPECT_TRUE(Allow(*source, GURL("http://a.com/path/to/file"), *self_source));
        EXPECT_FALSE(Allow(*source, GURL("http://a.com/path/to/"), *self_source));
        EXPECT_FALSE(Allow(*source, GURL("http://a.com/path/to/file/subpath"), *self_source));
        EXPECT_FALSE(Allow(*source, GURL("http://a.com/path/to/something"), *self_source));
        EXPECT_FALSE(Allow(*source, GURL("http://a.com/"), *self_source));
        EXPECT_FALSE(Allow(*source, GURL("http://a.com"), *self_source));
    }
    ...
    // Path to a directory
    {
        auto source = network::mojom::CSPSource::New("", "a.com", url::PORT_UNSPECIFIED, "/path/to/", false, false);
        EXPECT_TRUE(Allow(*source, GURL("http://a.com/path/to/file"), *self_source));
    ...
    // Path encoded.
    {
        auto source = network::mojom::CSPSource::New("http", "a.com", url::PORT_UNSPECIFIED, "/Hello Günter", false, false);
        EXPECT_TRUE(Allow(*source, GURL("http://a.com/Hello%20G%C3%BCnter"), *self_source));
        EXPECT_TRUE(Allow(*source, GURL("http://a.com/Hello Günter"), *self_source));
    }

    // Host is * but path is specified.
    {
        auto source = network::mojom::CSPSource::New("http", "", url::PORT_UNSPECIFIED, "/allowed-path", true, false);
        EXPECT_TRUE(Allow(*source, GURL("http://a.com/allowed-path"), *self_source));
        EXPECT_FALSE(Allow(*source, GURL("http://a.com/disallowed-path"), *self_source));
    }
}

TEST_P(CSPSourceTest, RedirectMatching)
{
    auto source = network::mojom::CSPSource::New("http", "a.com", 8000, "/bar/", false, false);
    ...
    EXPECT_FALSE(Allow(*source, GURL("http://not-a.com:8000/foo"), no_self, true));
...
TEST_P(CSPSourceTest, Intersect)
{
    struct TestCase {
        ...
        { "http://example.org", "http://example.org/page.html", "http://example.org/page.html" },
        { "http://example.org", "http://example.org/page.html", "http://example.org/page.html" },
        { "http://example.org/page.html", "http://example.org/page.html", "http://example.org/page.html" },
        { "http:", "https://example.org/page.html", "https://example.org/page.html" },
        { "http://example.org:80", "http://example.org", "http://example.org:80" },
        { "http://example.org:80", "http://example.org:*", "http://example.org:80" },
        { "http://example.org:90", "http://example.org:*", "http://example.org:90" },
        // Nontrivial intersection.
        { "https:", "http://example.org/page.html", "https://example.org/page.html" },
        { "https://*.org/page.html", "https://example.org/", "https://example.org/page.html" },
        { "http://*.org/page.html", "https://example.org/", "https://example.org/page.html" },
        { "http://example.org:*/page.html", "https://example.org/", "https://example.org/page.html" },
        { "http://example.org:*/page.html", "https://example.org/", "https://example.org/page.html" },
        ...
        { "http://example.org/page.html", "http://example.org/about.html", nullptr },
    };
    ...
    for (const auto& test : cases) {
        ...
        if (test.intersection) {
            EXPECT_EQ(test.intersection, ToString(*a_intersect_b))
                << "The intersection of " << test.a << " and " << test.b << " should be " << test.intersection;
            // Intersection should be symmetric.
            EXPECT_EQ(test.intersection, ToString(*b_intersect_a))
                << "The intersection of " << test.b << " and " << test.a << " should be " << test.intersection;
        } else {
            EXPECT_FALSE(a_intersect_b) << "The intersection of " << test.a << " and " << test.b << " should be empty.";
            EXPECT_FALSE(b_intersect_a) << "The intersection of " << test.b << " and " << test.a << " should be empty.";
        }
    }
}
...
TEST_P(CSPSourceTest, DoesNotSubsume)
{
    ...
    for (const auto& test : cases) {
        ...
        EXPECT_FALSE(CSPSourceSubsumes(*a, *b)) << test.a << " should not subsume " << test.b;
        EXPECT_FALSE(CSPSourceSubsumes(*b, *a)) << test.b << " should not subsume " << test.a;
    }
}
...
TEST_P(CSPSourceTest, Subsumes)
{
    ...
    for (const auto& test : cases) {
        ...
        EXPECT_EQ(CSPSourceSubsumes(*a, *b), test.expected_a_subsumes_b) << test.a << " subsumes " << test.b << " should return " << test.expected_a_subsumes_b;
        EXPECT_EQ(CSPSourceSubsumes(*b, *a), test.expected_b_subsumes_a) << test.b << " subsumes " << test.a << " should return " << test.expected_b_subsumes_a;

        a->is_host_wildcard = true;
        EXPECT_FALSE(CSPSourceSubsumes(*b, *a)) << test.b << " should not subsume " << ToString(*a);
        ...
        EXPECT_EQ(CSPSourceSubsumes(*b, *a), test.expected_b_subsumes_a)
            << ToString(*b) << " subsumes " << ToString(*a) << " should return " << test.expected_b_subsumes_a;
    }
}
...
TEST_P(CSPSourceTest, HostWildcardSubsumes)
{
    ...
    EXPECT_TRUE(CSPSourceSubsumes(*source_a, *source_b)) << a << " should subsume " << b;
    EXPECT_FALSE(CSPSourceSubsumes(*source_b, *source_a)) << b << " should not subsume " << a;

    // *.example.com and example.com have no relations.
    EXPECT_FALSE(CSPSourceSubsumes(*source_a, *source_c)) << a << " should not subsume " << c;
    EXPECT_FALSE(CSPSourceSubsumes(*source_c, *source_a)) << c << " should not subsume " << a;

    // https://*.example.com and http://www.example.com have no relations.
    EXPECT_FALSE(CSPSourceSubsumes(*source_d, *source_b)) << d << " should not subsume " << b;
    EXPECT_FALSE(CSPSourceSubsumes(*source_b, *source_d)) << b << " should not subsume " << d;
}
...
TEST_P(CSPSourceTest, PortWildcardSubsumes)
{
    ...
    EXPECT_FALSE(CSPSourceSubsumes(*source_b, *source_c)) << b << " should not subsume " << c;
    EXPECT_FALSE(CSPSourceSubsumes(*source_c, *source_b)) << c << " should not subsume " << b;
}
...
TEST_P(CSPSourceTest, SchemesOnlySubsumes)
{
    ...
    for (const auto& test : cases) {
        ...
        EXPECT_EQ(CSPSourceSubsumes(*source_a, *source_b), test.expected) << test.a << " subsumes " << test.b << " should return " << test.expected;
    }
}
...
TEST_P(CSPSourceTest, ToString)
{
    ...
    {
        auto source = network::mojom::CSPSource::New("http", "a.com", url::PORT_UNSPECIFIED, "", false, false);
    ...
    {
        auto source = network::mojom::CSPSource::New("", "a.com", url::PORT_UNSPECIFIED, "", true, false);
        EXPECT_EQ("*.a.com", ToString(*source));
    }
    {
        auto source = network::mojom::CSPSource::New("", "", url::PORT_UNSPECIFIED, "", true, false);
    ...
    {
        auto source = network::mojom::CSPSource::New("", "a.com", url::PORT_UNSPECIFIED, "/path", false, false);
...
{
    auto source = network::mojom::CSPSource::New("http", "a.com", 80, "", false, false);
    ...
    // Upgrades are disallowed for permission policies.
    EXPECT_EQ(Allow(*source, GURL("https://a.com:443"), no_self, true), !IsPermissionsPolicyContext());
    EXPECT_EQ(Allow(*source, GURL("https://a.com"), no_self, true), !IsPermissionsPolicyContext());
}
