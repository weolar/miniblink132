...
std::vector<mojom::ContentSecurityPolicyPtr> ParseCSP(std::string expression)
{
    ...
    AddContentSecurityPolicyFromHeaders(*headers, GURL("https://example.com/"), &policies);
    return policies;
}

static void TestFrameAncestorsCSPParser(const std::string& header, const ExpectedResult* expected_result)
{
    scoped_refptr<net::HttpResponseHeaders> headers(new net::HttpResponseHeaders("HTTP/1.1 200 OK"));
    ...
    auto& frame_ancestors = policies[0]->directives[mojom::CSPDirectiveName::FrameAncestors];
    EXPECT_EQ(frame_ancestors->sources.size(), expected_result->parsed_sources.size());
    EXPECT_EQ(policies[0]->raw_directives[mojom::CSPDirectiveName::FrameAncestors], header);
    for (size_t i = 0; i < expected_result->parsed_sources.size(); i++) {
        EXPECT_EQ(frame_ancestors->sources[i]->scheme, expected_result->parsed_sources[i].scheme);
        EXPECT_EQ(frame_ancestors->sources[i]->host, expected_result->parsed_sources[i].host);
        EXPECT_EQ(frame_ancestors->sources[i]->port, expected_result->parsed_sources[i].port);
        EXPECT_EQ(frame_ancestors->sources[i]->path, expected_result->parsed_sources[i].path);
        EXPECT_EQ(frame_ancestors->sources[i]->is_host_wildcard, expected_result->parsed_sources[i].is_host_wildcard);
        EXPECT_EQ(frame_ancestors->sources[i]->is_port_wildcard, expected_result->parsed_sources[i].is_port_wildcard);
    }
...
mojom::ContentSecurityPolicyPtr EmptyCSP()
{
    ...
    policy->self_origin = network::mojom::CSPSource::New("", "", url::PORT_UNSPECIFIED, "", false, false);
    return policy;
}

// Build a new policy made of only one directive and no report endpoints.
mojom::ContentSecurityPolicyPtr BuildPolicy(CSPDirectiveName directive_name, mojom::CSPSourcePtr source)
{
...
{
    return mojom::CSPSource::New(scheme, host, url::PORT_UNSPECIFIED, "", false, false);
}

// Return "Content-Security-Policy: default-src <host>"
mojom::ContentSecurityPolicyPtr DefaultSrc(const char* scheme, const char* host)
{
    return BuildPolicy(CSPDirectiveName::DefaultSrc, BuildCSPSource(scheme, host));
}
...
TEST(ContentSecurityPolicy, ParseFrameAncestors)
{
    TestData test_data[] = {
        ...
        // Parse path.
        // Encoded.
        { "example.com/%48%65%6c%6c%6f%20%57%6f%72%6c%64", { { { "", "example.com", url::PORT_UNSPECIFIED, "/Hello World" } } } },
        ...
        { "example.com/path", { { { "", "example.com", url::PORT_UNSPECIFIED, "/path" } } } },
        { "https://example.com", { { { "https", "example.com" } } } },
        { "https://example.com/path", { { { "https", "example.com", url::PORT_UNSPECIFIED, "/path" } } } },
        { "https://example.com:1234", { { { "https", "example.com", 1234, "" } } } },
        { "https://example.com:2345/some/path", { { { "https", "example.com", 2345, "/some/path" } } } },
...
TEST(ContentSecurityPolicy, ParseDirectives)
{
    ...
    {
        scoped_refptr<net::HttpResponseHeaders> headers(new net::HttpResponseHeaders("HTTP/1.1 200 OK"));
        headers->SetHeader("Content-Security-Policy", "ScrIPT-sRc 'none'; img-src 'none'");
        std::vector<mojom::ContentSecurityPolicyPtr> policies;
        AddContentSecurityPolicyFromHeaders(*headers, GURL("https://example.com/"), &policies);
        EXPECT_EQ(2U, policies[0]->directives.size());
        EXPECT_EQ(2U, policies[0]->raw_directives.size());

        EXPECT_EQ(policies[0]->raw_directives[mojom::CSPDirectiveName::ScriptSrc], "'none'");
        auto& script_src = policies[0]->directives[mojom::CSPDirectiveName::ScriptSrc];
        EXPECT_EQ(script_src->sources.size(), 0U);
        EXPECT_EQ(script_src->allow_self, false);
        EXPECT_EQ(script_src->allow_star, false);

        EXPECT_EQ(policies[0]->raw_directives[mojom::CSPDirectiveName::ImgSrc], "'none'");
    ...
    // One duplicate directive.
    {
        ...
        EXPECT_EQ(policies[0]->raw_directives[mojom::CSPDirectiveName::FrameAncestors], "example.com");
        auto& frame_ancestors = policies[0]->directives[mojom::CSPDirectiveName::FrameAncestors];
        ...
        EXPECT_EQ(policies[0]->raw_directives[mojom::CSPDirectiveName::ScriptSrc], "example2.com");
        auto& script_src = policies[0]->directives[mojom::CSPDirectiveName::ScriptSrc];
        ...
        EXPECT_EQ("Ignoring duplicate Content-Security-Policy directive "
                  "'frame-ancestors'.",
    ...
    // One invalid directive.
    {
        ...
        EXPECT_EQ(policies[0]->raw_directives[mojom::CSPDirectiveName::FrameAncestors], "example.org");
        auto& frame_ancestors = policies[0]->directives[mojom::CSPDirectiveName::FrameAncestors];
        EXPECT_EQ(frame_ancestors->sources.size(), 1U);
        EXPECT_EQ(frame_ancestors->sources[0]->scheme, "");
        ...
        EXPECT_EQ(1U, policies[0]->parsing_errors.size());
        EXPECT_EQ("Unrecognized Content-Security-Policy directive 'other-directive'.", policies[0]->parsing_errors[0]);
    }

    // Invalid characters in directive name.
    {
        scoped_refptr<net::HttpResponseHeaders> headers(new net::HttpResponseHeaders("HTTP/1.1 200 OK"));
        headers->SetHeader("Content-Security-Policy", "frame_ancestors example.com;");
        std::vector<mojom::ContentSecurityPolicyPtr> policies;
        AddContentSecurityPolicyFromHeaders(*headers, GURL("https://example.com/"), &policies);
        ...
        EXPECT_EQ(1U, policies[0]->parsing_errors.size());
        EXPECT_EQ("The Content-Security-Policy directive name 'frame_ancestors' contains "
    ...
    // Invalid characters in directive value.
    {
        ...
        EXPECT_EQ(1U, policies[0]->parsing_errors.size());
        EXPECT_EQ("The value for the Content-Security-Policy directive 'frame-ancestors' "
    ...
    // Missing semicolon between directive names.
    {
        ...
        EXPECT_EQ(policies[0]->raw_directives[mojom::CSPDirectiveName::FrameAncestors], "object-src");
        auto& frame_ancestors = policies[0]->directives[mojom::CSPDirectiveName::FrameAncestors];
        EXPECT_EQ(frame_ancestors->sources.size(), 1U);
        EXPECT_EQ(frame_ancestors->sources[0]->scheme, "");
        ...
        EXPECT_EQ(1U, policies[0]->parsing_errors.size());
        EXPECT_EQ("The Content-Security-Policy directive 'frame-ancestors' contains "
    ...
    {
        scoped_refptr<net::HttpResponseHeaders> headers(new net::HttpResponseHeaders("HTTP/1.1 200 OK"));
        headers->SetHeader("Content-Security-Policy", "frame-ancestors http://example.org/index.html?a=b;");
        std::vector<mojom::ContentSecurityPolicyPtr> policies;
        AddContentSecurityPolicyFromHeaders(*headers, GURL("https://example.com/"), &policies);
        EXPECT_EQ(1U, policies[0]->directives.size());
        EXPECT_EQ(1U, policies[0]->raw_directives.size());

        EXPECT_EQ(policies[0]->raw_directives[mojom::CSPDirectiveName::FrameAncestors], "http://example.org/index.html?a=b");
        auto& frame_ancestors = policies[0]->directives[mojom::CSPDirectiveName::FrameAncestors];
        EXPECT_EQ(frame_ancestors->sources.size(), 1U);
        ...
        EXPECT_EQ(1U, policies[0]->parsing_errors.size());
        EXPECT_EQ("The source list for Content Security Policy directive "
    ...
    {
        scoped_refptr<net::HttpResponseHeaders> headers(new net::HttpResponseHeaders("HTTP/1.1 200 OK"));
        headers->SetHeader("Content-Security-Policy", "frame-ancestors http://example.org/index.html#a;");
        std::vector<mojom::ContentSecurityPolicyPtr> policies;
        AddContentSecurityPolicyFromHeaders(*headers, GURL("https://example.com/"), &policies);
        EXPECT_EQ(1U, policies[0]->directives.size());
        EXPECT_EQ(1U, policies[0]->raw_directives.size());

        EXPECT_EQ(policies[0]->raw_directives[mojom::CSPDirectiveName::FrameAncestors], "http://example.org/index.html#a");
        auto& frame_ancestors = policies[0]->directives[mojom::CSPDirectiveName::FrameAncestors];
        EXPECT_EQ(frame_ancestors->sources.size(), 1U);
        EXPECT_EQ(frame_ancestors->sources[0]->scheme, "http");
        EXPECT_EQ(frame_ancestors->sources[0]->host, "example.org");
    ...
    // Multiple policies should be created.
    {
        ...
        EXPECT_EQ(policies[0]->raw_directives[mojom::CSPDirectiveName::FrameAncestors], "example.com");
        EXPECT_EQ(policies[1]->raw_directives[mojom::CSPDirectiveName::FrameAncestors], "example.org");

        auto& frame_ancestors0 = policies[0]->directives[mojom::CSPDirectiveName::FrameAncestors];
        auto& frame_ancestors1 = policies[1]->directives[mojom::CSPDirectiveName::FrameAncestors];
    ...
    {
        scoped_refptr<net::HttpResponseHeaders> headers(new net::HttpResponseHeaders("HTTP/1.1 200 OK"));
        headers->SetHeader("Content-Security-Policy", "other-directive value, frame-ancestors example.org");
        std::vector<mojom::ContentSecurityPolicyPtr> policies;
        AddContentSecurityPolicyFromHeaders(*headers, GURL("https://example.com/"), &policies);
    ...
    {
        scoped_refptr<net::HttpResponseHeaders> headers(new net::HttpResponseHeaders("HTTP/1.1 200 OK"));
        headers->SetHeader("Content-Security-Policy", "frame-ancestors example.com, frame-ancestors example.org");
        std::vector<mojom::ContentSecurityPolicyPtr> policies;
        AddContentSecurityPolicyFromHeaders(*headers, GURL("https://example.com/"), &policies);
    ...
    {
        scoped_refptr<net::HttpResponseHeaders> headers(new net::HttpResponseHeaders("HTTP/1.1 200 OK"));
        headers->SetHeader("Content-Security-Policy", "report-to http://example.com/report; frame-ancestors example.com");
        std::vector<mojom::ContentSecurityPolicyPtr> policies;
        AddContentSecurityPolicyFromHeaders(*headers, GURL("https://example.com/"), &policies);
...
{
    std::vector<mojom::ContentSecurityPolicyPtr> policies = ParseCSP("allow    'none'");
...
{
    std::vector<mojom::ContentSecurityPolicyPtr> policies = ParseCSP("options 'unsafe-inline'");
    EXPECT_EQ(policies[0]->directives.size(), 0u);
    EXPECT_EQ(policies[0]->parsing_errors[0],
...
{
    std::vector<mojom::ContentSecurityPolicyPtr> policies = ParseCSP("policy-uri   https://example.com/my-csp");
    EXPECT_EQ(policies[0]->directives.size(), 0u);
    EXPECT_EQ(policies[0]->parsing_errors[0],
...
{
    std::vector<mojom::ContentSecurityPolicyPtr> policies = ParseCSP("plugin-types    application/pdf text/plain");
    EXPECT_EQ(policies[0]->directives.size(), 0u);
    EXPECT_EQ(policies[0]->parsing_errors[0],
...
TEST(ContentSecurityPolicy, ParseRequireTrustedTypesFor)
{
    ...
    for (const auto& testCase : cases) {
        std::vector<mojom::ContentSecurityPolicyPtr> policies = ParseCSP(base::StringPrintf("require-trusted-types-for %s", testCase.input));
        EXPECT_EQ(policies[0]->raw_directives[mojom::CSPDirectiveName::RequireTrustedTypesFor], testCase.input);
        EXPECT_EQ(policies[0]->directives.size(), 0u);
...
{
    {
        std::vector<mojom::ContentSecurityPolicyPtr> policies = ParseCSP("script-src 'none'");
        EXPECT_EQ(policies[0]->directives.size(), 1u);
        EXPECT_FALSE(policies[0]->trusted_types);
    }

    {
        std::vector<mojom::ContentSecurityPolicyPtr> policies = ParseCSP("trusted-types 'none'");
        EXPECT_EQ(policies[0]->raw_directives[mojom::CSPDirectiveName::TrustedTypes], "'none'");
        EXPECT_EQ(policies[0]->directives.size(), 0u);
    ...
    {
        std::vector<mojom::ContentSecurityPolicyPtr> policies = ParseCSP("trusted-types policy   'none'  other_policy@ invalid~policy");
        EXPECT_EQ(policies[0]->raw_directives[mojom::CSPDirectiveName::TrustedTypes], "policy   'none'  other_policy@ invalid~policy");
        EXPECT_EQ(policies[0]->directives.size(), 0u);
        EXPECT_TRUE(policies[0]->trusted_types);
...
TEST(ContentSecurityPolicy, ParseBlockAllMixedContent)
{
    ...
    {
        std::vector<mojom::ContentSecurityPolicyPtr> policies = ParseCSP("block-all-mixed-content");
        EXPECT_EQ(policies[0]->directives.size(), 0u);
    ...
    {
        std::vector<mojom::ContentSecurityPolicyPtr> policies = ParseCSP("block-all-mixed-content true");
        EXPECT_EQ(policies[0]->directives.size(), 0u);
        EXPECT_TRUE(policies[0]->block_all_mixed_content);
...
TEST(ContentSecurityPolicy, ParseReportEndpoint)
{
    // report-uri directive.
    {
        ...
        std::vector<mojom::ContentSecurityPolicyPtr> policies;
        AddContentSecurityPolicyFromHeaders(*headers, GURL("http://example.com/"), &policies);
    ...
    {
        scoped_refptr<net::HttpResponseHeaders> headers(new net::HttpResponseHeaders("HTTP/1.1 200 OK"));
        headers->SetHeader("Content-Security-Policy", "report-uri http://example.com/report");
        std::vector<mojom::ContentSecurityPolicyPtr> policies;
        AddContentSecurityPolicyFromHeaders(*headers, GURL("https://example.com/"), &policies);
    ...
    {
        scoped_refptr<net::HttpResponseHeaders> headers(new net::HttpResponseHeaders("HTTP/1.1 200 OK"));
        headers->SetHeader("Content-Security-Policy", "report-to group1 group2 group3");
        std::vector<mojom::ContentSecurityPolicyPtr> policies;
        AddContentSecurityPolicyFromHeaders(*headers, GURL("https://example.com/"), &policies);
    ...
    {
        scoped_refptr<net::HttpResponseHeaders> headers(new net::HttpResponseHeaders("HTTP/1.1 200 OK"));
        headers->AddHeader("Content-Security-Policy", "report-to http://example.com/report1");
        headers->AddHeader("Content-Security-Policy", "report-uri http://example.com/report2");
        std::vector<mojom::ContentSecurityPolicyPtr> policies;
        AddContentSecurityPolicyFromHeaders(*headers, GURL("http://example.com/"), &policies);
...
TEST(ContentSecurityPolicy, ParseStoresSelfOrigin)
{
    struct {
        ...
        {
            "https://example.com",
            network::mojom::CSPSource::New("https", "example.com", 443, "", false, false),
        },
        {
            "http://example.com/main/index.html",
            network::mojom::CSPSource::New("http", "example.com", 80, "", false, false),
        },
        {
            "file://localhost/var/www/index.html",
            network::mojom::CSPSource::New("file", "", url::PORT_UNSPECIFIED, "", false, false),
        },
    };
    ...
    for (const auto& testCase : testCases) {
        ...
        std::vector<mojom::ContentSecurityPolicyPtr> policies;
        AddContentSecurityPolicyFromHeaders(*headers, GURL(testCase.url), &policies);
...
TEST(ContentSecurityPolicy, NoDirective)
{
    ...
    EXPECT_TRUE(
        CheckContentSecurityPolicy(EmptyCSP(), CSPDirectiveName::FormAction, GURL("http://www.example.com"), GURL(), false, &context, SourceLocation(), true));
...
{
    CSPContextTest context;
    auto policy = BuildPolicy(CSPDirectiveName::FormAction, BuildCSPSource("", "www.example.com"));

    EXPECT_FALSE(CheckContentSecurityPolicy(
        policy, CSPDirectiveName::FormAction, GURL("http://www.not-example.com"), GURL("http://www.example.com"), false, &context, SourceLocation(), true));

    ASSERT_EQ(1u, context.violations().size());
    const char console_message[] = "Refused to send form data to 'http://www.example.com/' because it "
...
TEST(ContentSecurityPolicy, DirectiveFallback)
{
...

    {
        ...
        EXPECT_FALSE(CheckContentSecurityPolicy(policy, CSPDirectiveName::FrameSrc, GURL("http://b.com"), GURL(), false, &context, SourceLocation(), false));
        ASSERT_EQ(1u, context.violations().size());
        const char console_message[] = "Refused to frame 'http://b.com/' because it violates "
                                       "the following Content Security Policy directive: \"default-src "
                                       "http://a.com\". Note that 'frame-src' was not explicitly "
                                       "set, so 'default-src' is used as a fallback.\n";
        EXPECT_EQ(console_message, context.violations()[0]->console_message);
        EXPECT_TRUE(CheckContentSecurityPolicy(policy, CSPDirectiveName::FrameSrc, GURL("http://a.com"), GURL(), false, &context, SourceLocation(), false));
    }
    {
        CSPContextTest context;
        auto policy = EmptyCSP();
...
TEST(ContentSecurityPolicy, RequestsAllowedWhenBypassingCSP)
{
    ...
    EXPECT_TRUE(CheckContentSecurityPolicy(policy, CSPDirectiveName::FrameSrc, GURL("https://example.com/"), GURL(), false, &context, SourceLocation(), false));
    EXPECT_FALSE(
        CheckContentSecurityPolicy(policy, CSPDirectiveName::FrameSrc, GURL("https://not-example.com/"), GURL(), false, &context, SourceLocation(), false));
...
TEST(ContentSecurityPolicy, FilesystemAllowedWhenBypassingCSP)
{
    ...
    EXPECT_FALSE(CheckContentSecurityPolicy(
        policy, CSPDirectiveName::FrameSrc, GURL("filesystem:https://example.com/file.txt"), GURL(), false, &context, SourceLocation(), false));
    EXPECT_FALSE(CheckContentSecurityPolicy(
        policy, CSPDirectiveName::FrameSrc, GURL("filesystem:https://not-example.com/file.txt"), GURL(), false, &context, SourceLocation(), false));
...
TEST(ContentSecurityPolicy, BlobAllowedWhenBypassingCSP)
{
    ...
    EXPECT_FALSE(
        CheckContentSecurityPolicy(policy, CSPDirectiveName::FrameSrc, GURL("blob:https://example.com/"), GURL(), false, &context, SourceLocation(), false));
    EXPECT_FALSE(CheckContentSecurityPolicy(
        policy, CSPDirectiveName::FrameSrc, GURL("blob:https://not-example.com/"), GURL(), false, &context, SourceLocation(), false));
...
{
    scoped_refptr<net::HttpResponseHeaders> headers(new net::HttpResponseHeaders("HTTP/1.1 200 OK"));
    headers->SetHeader("Content-Security-Policy", "sandbox allow-downloads allow-scripts");
    std::vector<mojom::ContentSecurityPolicyPtr> policies;
    AddContentSecurityPolicyFromHeaders(*headers, GURL("https://example.com/"), &policies);
    EXPECT_EQ(policies[0]->raw_directives[mojom::CSPDirectiveName::Sandbox], "allow-downloads allow-scripts");
    EXPECT_EQ(policies[0]->sandbox, ~mojom::WebSandboxFlags::kDownloads & ~mojom::WebSandboxFlags::kScripts & ~mojom::WebSandboxFlags::kAutomaticFeatures);
}
...
TEST(ContentSecurityPolicy, ParseSerializedSourceList)
{
    struct TestCase {
        ...
        },
        {
            ...
            base::BindOnce([] {
                auto csp = mojom::CSPSourceList::New();
                csp->hashes.push_back(mojom::CSPHashSource::New(mojom::CSPHashAlgorithm::SHA256, std::vector<uint8_t> { 'a', 'b', 'c' }));
                csp->hashes.push_back(mojom::CSPHashSource::New(mojom::CSPHashAlgorithm::SHA256, std::vector<uint8_t> { 'A', 'B', 'C' }));
                csp->nonces.push_back("cde");
                return csp;
            }),
    ...
    for (auto& test : cases) {
        ...
        scoped_refptr<net::HttpResponseHeaders> headers(new net::HttpResponseHeaders("HTTP/1.1 200 OK"));
        headers->SetHeader("Content-Security-Policy", "script-src " + test.directive_value);
        std::vector<mojom::ContentSecurityPolicyPtr> policies;
        AddContentSecurityPolicyFromHeaders(*headers, GURL("https://example.com/"), &policies);
        EXPECT_TRUE(std::move(test.expected).Run().Equals(policies[0]->directives[mojom::CSPDirectiveName::ScriptSrc]));

        EXPECT_EQ(policies[0]->raw_directives[mojom::CSPDirectiveName::ScriptSrc], std::string(base::TrimString(test.directive_value, " ", base::TRIM_ALL)));
...
TEST(ContentSecurityPolicy, ParseHash)
{
    ...
    for (auto& test : cases) {
        ...
        std::vector<mojom::ContentSecurityPolicyPtr> policies;
        AddContentSecurityPolicyFromHeaders(*headers, GURL("https://example.com/"), &policies);
        const std::vector<mojom::CSPHashSourcePtr>& hashes = policies[0]->directives[mojom::CSPDirectiveName::ScriptSrc]->hashes;
        if (test.expected_algorithm != Algo::None) {
            EXPECT_EQ(1u, hashes.size()) << test.hash << " should parse to one hash";
            EXPECT_EQ(test.expected_algorithm, hashes[0]->algorithm) << test.hash << " should have algorithm " << test.expected_algorithm;
            EXPECT_EQ(test.expected_hash, hashes[0]->value) << test.hash << " has not been base64decoded correctly";
        } else {
...
{
    std::vector<mojom::ContentSecurityPolicyPtr> script_src_policies = ParseCSP("script-src 'inline-speculation-rules'");
    ASSERT_EQ(1u, script_src_policies.size());
    ASSERT_EQ(1u, script_src_policies[0]->directives.size());
    ASSERT_TRUE(script_src_policies[0]->directives.contains(mojom::CSPDirectiveName::ScriptSrc));
    EXPECT_TRUE(script_src_policies[0]->directives[mojom::CSPDirectiveName::ScriptSrc]->allow_inline_speculation_rules);
    EXPECT_EQ(0u, script_src_policies[0]->parsing_errors.size());

    std::vector<mojom::ContentSecurityPolicyPtr> script_src_elem_policies = ParseCSP("script-src-elem 'inline-speculation-rules'");
    ASSERT_EQ(1u, script_src_elem_policies.size());
    ASSERT_EQ(1u, script_src_elem_policies[0]->directives.size());
    ASSERT_TRUE(script_src_elem_policies[0]->directives.contains(mojom::CSPDirectiveName::ScriptSrcElem));
    EXPECT_TRUE(script_src_elem_policies[0]->directives[mojom::CSPDirectiveName::ScriptSrcElem]->allow_inline_speculation_rules);
    EXPECT_EQ(0u, script_src_elem_policies[0]->parsing_errors.size());

    std::vector<mojom::ContentSecurityPolicyPtr> img_src_policies = ParseCSP("img-src 'inline-speculation-rules'");
    ASSERT_EQ(1u, img_src_policies.size());
    ASSERT_EQ(1u, img_src_policies[0]->directives.size());
    ASSERT_TRUE(img_src_policies[0]->directives.contains(mojom::CSPDirectiveName::ImgSrc));
    EXPECT_FALSE(img_src_policies[0]->directives[mojom::CSPDirectiveName::ImgSrc]->allow_inline_speculation_rules);
    ASSERT_EQ(1u, img_src_policies[0]->parsing_errors.size());
    EXPECT_EQ("The Content-Security-Policy directive 'img-src' contains "
...
TEST(ContentSecurityPolicy, IsValidRequiredCSPAttr)
{
    ...
    for (auto& test : cases) {
        ...
        auto required_csp_headers = base::MakeRefCounted<net::HttpResponseHeaders>("HTTP/1.1 200 OK");
        required_csp_headers->SetHeader("Content-Security-Policy", test.csp);
        AddContentSecurityPolicyFromHeaders(*required_csp_headers, GURL("https://example.com/"), &csp);
...
TEST(ContentSecurityPolicy, Subsumes)
{
    ...
    for (auto& test : cases) {
        std::vector<mojom::ContentSecurityPolicyPtr> required_csp = ParseCSP(test.required);

        auto returned_csp_headers = base::MakeRefCounted<net::HttpResponseHeaders>("HTTP/1.1 200 OK");
        if (test.returned_is_report_only)
            returned_csp_headers->SetHeader("Content-Security-Policy-Report-Only", test.returned);
        else
        ...
        AddContentSecurityPolicyFromHeaders(*returned_csp_headers, GURL("https://example.com/"), &returned_csp);
        EXPECT_EQ(test.expected, Subsumes(*required_csp[0], returned_csp)) << test.name;
    }
}

TEST(ContentSecurityPolicy, SubsumesBasedOnCSPSourcesOnly)
{
    const char* csp_a = "script-src http://*.one.com; img-src https://sub.one.com "
                        "http://two.com/imgs/";
    ...
    for (const auto& test : cases) {
        std::vector<mojom::ContentSecurityPolicyPtr> policies_b = ParseCSP(test.policies);
        EXPECT_EQ(Subsumes(*policy_a[0], policies_b), test.expected) << csp_a << " should " << (test.expected ? "" : "not ") << "subsume " << test.policies;

        if (!policies_b.empty()) {
            // Check if first policy of `listB` subsumes `A`.
            EXPECT_EQ(Subsumes(*policies_b[0], policy_a), test.expected_first_policy_opposite)
                << csp_a << " should " << (test.expected_first_policy_opposite ? "" : "not ") << "subsume " << test.policies;
        }
    }
}
...
TEST(ContentSecurityPolicy, SubsumesIfNoneIsPresent)
{
    struct TestCase {
        ...
        { "script-src ", "script-src http://*.one.com, img-src https://two.com", false },
        { "script-src 'none'", "script-src http://*.one.com, img-src https://two.com", false },
        { "script-src 'none'", "script-src http://*.one.com, script-src https://two.com", true },
        { "script-src 'none'", "script-src http://*.one.com, script-src 'self'", true },
        ...
        { "script-src 'none'", "script-src 'none', script-src 'none', script-src 'none'", true },
        { "script-src 'none'",
            "script-src http://*.one.com, script-src http://sub.one.com,"
            "script-src 'none'",
            true },
        ...
        { "script-src 'none'", "script-src 'unsafe-inline', script-src 'none'", true },
        { "script-src ",
        ...
        { "script-src ",
            ...
            false },
        { "script-src 'unsafe-eval'", "script-src 'unsafe-eval', script 'unsafe-inline'", true },
        { "script-src 'unsafe-inline'", "script-src  , script http://example.com", true },
    };

    for (const auto& test : cases) {
        std::vector<mojom::ContentSecurityPolicyPtr> policy_a = ParseCSP(test.policy_a);
        std::vector<mojom::ContentSecurityPolicyPtr> policies_b = ParseCSP(test.policies_b);
        EXPECT_EQ(Subsumes(*policy_a[0], policies_b), test.expected)
            << test.policy_a << " should " << (test.expected ? "" : "not ") << "subsume " << test.policies_b;
    }
}
...
TEST(ContentSecurityPolicy, InvalidPolicyInReportOnlySandbox)
{
    ...
    EXPECT_EQ("The Content Security Policy directive 'sandbox' is ignored when "
...
{
    mojom::ContentSecurityPolicyPtr policy = ParseOneCspReportOnly("upgrade-insecure-requests");

    EXPECT_FALSE(policy->upgrade_insecure_requests);
    ASSERT_EQ(1u, policy->parsing_errors.size());
    EXPECT_EQ("The Content Security Policy directive 'upgrade-insecure-requests' is "
...
{
    mojom::ContentSecurityPolicyPtr policy = ParseOneCspReportOnly("treat-as-public-address");

    EXPECT_FALSE(policy->treat_as_public_address);
    ASSERT_EQ(1u, policy->parsing_errors.size());
    EXPECT_EQ("The Content Security Policy directive 'treat-as-public-address' is "
              "ignored when delivered in a report-only policy.",
        policy->parsing_errors[0]);
}
...
{
    std::vector<mojom::ContentSecurityPolicyPtr> policies = ParseContentSecurityPolicies("frame-ancestors https://www.example.org; script-src 'none'",
        mojom::ContentSecurityPolicyType::kEnforce, mojom::ContentSecurityPolicySource::kMeta, GURL("https://www.example.org"));
    ...
    ASSERT_EQ(1u, policy->parsing_errors.size());
    EXPECT_EQ("The Content Security Policy directive 'frame-ancestors' is ignored when "
...
{
    std::vector<mojom::ContentSecurityPolicyPtr> policies = ParseContentSecurityPolicies("report-uri https://www.example.org; script-src 'none'",
        mojom::ContentSecurityPolicyType::kEnforce, mojom::ContentSecurityPolicySource::kMeta, GURL("https://www.example.org"));
    ...
    ASSERT_EQ(1u, policy->parsing_errors.size());
    EXPECT_EQ("The Content Security Policy directive 'report-uri' is ignored when "
              "delivered via a <meta> element.",
        policy->parsing_errors[0]);
}
...
{
    std::vector<mojom::ContentSecurityPolicyPtr> policies = ParseContentSecurityPolicies(
        "sandbox; script-src 'none'", mojom::ContentSecurityPolicyType::kEnforce, mojom::ContentSecurityPolicySource::kMeta, GURL("https://www.example.org"));
...
{
    std::vector<mojom::ContentSecurityPolicyPtr> policies = ParseContentSecurityPolicies("treat-as-public-address; script-src 'none'",
        mojom::ContentSecurityPolicyType::kEnforce, mojom::ContentSecurityPolicySource::kMeta, GURL("https://www.example.org"));
...
TEST(ContentSecurityPolicy, AllowsBlanketEnforcementOfRequiredCSP)
{
    ...
    for (const auto& test : cases) {
        ...
        bool actual = AllowsBlanketEnforcementOfRequiredCSP(
            url::Origin::Create(GURL(test.request_origin)), GURL(test.response_origin), allow_csp_from.get(), required_csp);
        EXPECT_EQ(test.expected_result, actual);
        if (test.expected_self_origin) {
            GURL expected_self_origin(test.expected_self_origin);
            EXPECT_EQ(expected_self_origin.scheme(), required_csp->self_origin->scheme);
            EXPECT_EQ(expected_self_origin.host(), required_csp->self_origin->host);
            EXPECT_EQ(expected_self_origin.EffectiveIntPort(), required_csp->self_origin->port);
        }
    }
}
...
TEST(ContentSecurityPolicy, FencedFrameSrcFallback)
{
...

    {
        ...
        policy->directives[CSPDirectiveName::DefaultSrc] = allow_host("a.com");
        EXPECT_FALSE(
            CheckContentSecurityPolicy(policy, CSPDirectiveName::FencedFrameSrc, GURL("http://b.com"), GURL(), false, &context, SourceLocation(), false));
        ASSERT_EQ(1u, context.violations().size());
        const char kConsoleMessage[] = "Refused to frame 'http://b.com/' as a fenced frame because it "
        ...
        EXPECT_TRUE(
            CheckContentSecurityPolicy(policy, CSPDirectiveName::FencedFrameSrc, GURL("http://a.com"), GURL(), false, &context, SourceLocation(), false));
    }
    {
        CSPContextTest context;
        auto policy = EmptyCSP();
...
TEST(ContentSecurityPolicy, FencedFrameSrcOpaqueURL)
{
    CSPContextTest context;
    auto policy = EmptyCSP();
    policy->directives[CSPDirectiveName::FencedFrameSrc] = mojom::CSPSourceList::New();
    EXPECT_FALSE(CheckContentSecurityPolicy(policy, CSPDirectiveName::FencedFrameSrc, GURL("https://a.com"), GURL(),
        /*has_followed_redirect=*/false, &context, SourceLocation(),
        /*is_form_submission=*/false,
        /*is_opaque_fenced_frame=*/true));
    ASSERT_EQ(1u, context.violations().size());
    const char kConsoleMessage[] = "Refused to frame 'urn:uuid' as a fenced frame because it violates the "
