...
struct P2PHostAndIPEndPoint {
    ...
    P2PHostAndIPEndPoint(const std::string& hostname, const net::IPEndPoint& ip_address)
        : hostname(hostname)
...
struct P2PSendPacketMetrics {
    ...
    P2PSendPacketMetrics(uint64_t packet_id, int32_t rtc_packet_id, int64_t send_time_ms)
        : packet_id(packet_id)
...
struct P2PPacketInfo {
    ...
    P2PPacketInfo(const net::IPEndPoint& destination, const rtc::PacketOptions& packet_options, uint64_t packet_id)
