...
{
    for (auto referrer_policy : { net::ReferrerPolicy::CLEAR_ON_TRANSITION_FROM_SECURE_TO_INSECURE,
             net::ReferrerPolicy::REDUCE_GRANULARITY_ON_TRANSITION_CROSS_ORIGIN, net::ReferrerPolicy::ORIGIN_ONLY_ON_TRANSITION_CROSS_ORIGIN,
             net::ReferrerPolicy::NEVER_CLEAR, net::ReferrerPolicy::ORIGIN, net::ReferrerPolicy::CLEAR_ON_TRANSITION_CROSS_ORIGIN,
             net::ReferrerPolicy::ORIGIN_CLEAR_ON_TRANSITION_FROM_SECURE_TO_INSECURE, net::ReferrerPolicy::NO_REFERRER }) {
        int32_t serialized = -1;
        using URLRequestReferrerPolicySerializer = mojo::internal::Serializer<mojom::URLRequestReferrerPolicy, net::ReferrerPolicy>;
...
TEST(URLRequestMojomTraitsTest, Roundtrips_ResourceRequest)
{
    ...
    original.site_for_cookies = net::SiteForCookies::FromUrl(GURL("https://example.com/index.html"));
    original.update_first_party_url_on_redirect = false;
    original.request_initiator = url::Origin::Create(original.url);
    original.isolated_world_origin = url::Origin::Create(GURL("chrome-extension://blah"));
    original.referrer = GURL("https://referrer.com/");
    original.referrer_policy = net::ReferrerPolicy::ORIGIN_ONLY_ON_TRANSITION_CROSS_ORIGIN;
    original.headers.SetHeader("Accept", "text/xml");
    original.cors_exempt_headers.SetHeader("X-Requested-With", "ForTesting");
    original.load_flags = net::LOAD_VALIDATE_CACHE | net::LOAD_BYPASS_CACHE | net::LOAD_SHOULD_BYPASS_HSTS;
    original.resource_type = 2;
    original.priority = net::IDLE;
    original.priority_incremental = net::kDefaultPriorityIncremental;
    original.cors_preflight_policy = mojom::CorsPreflightPolicy::kConsiderPreflight;
    ...
    original.web_bundle_token_params = std::make_optional(ResourceRequest::WebBundleTokenParams(
        GURL("https://bundle.test/"), base::UnguessableToken::Create(), mojo::PendingRemote<network::mojom::WebBundleHandle>()));
    original.net_log_create_info = std::make_optional(net::NetLogSource(net::NetLogSourceType::URL_REQUEST, net::NetLog::Get()->NextID()));
    original.net_log_reference_info = std::make_optional(net::NetLogSource(net::NetLogSourceType::URL_REQUEST, net::NetLog::Get()->NextID()));
    original.devtools_accepted_stream_types = std::vector<net::SourceStream::SourceType>(
    ...
    original.storage_access_api_status = net::StorageAccessApiStatus::kAccessViaAPI;

    original.trusted_params = ResourceRequest::TrustedParams();
    original.trusted_params->isolation_info = net::IsolationInfo::Create(
        net::IsolationInfo::RequestType::kMainFrame, url::Origin::Create(original.url), url::Origin::Create(original.url), original.site_for_cookies);
    ...
    original.trust_token_params->issuers.push_back(url::Origin::Create(GURL("https://issuer.com")));
    original.trust_token_params->operation = mojom::TrustTokenOperationType::kRedemption;
    original.trust_token_params->include_timestamp_header = true;
    original.trust_token_params->sign_request_data = mojom::TrustTokenSignRequestData::kInclude;
    original.trust_token_params->additional_signed_headers.push_back("some_header");
    ...
    EXPECT_TRUE(mojo::test::SerializeAndDeserialize<mojom::URLRequest>(original, copied));
...
TEST_F(DataElementDeserializationTest, DataPipe)
{
    ...
    DataElement dest;
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::DataElement>(src, dest));
    ASSERT_EQ(dest.type(), network::DataElement::Tag::kDataPipe);

    mojo::Remote<mojom::DataPipeGetter> remote(dest.As<DataElementDataPipe>().ReleaseDataPipeGetter());
...
TEST_F(DataElementDeserializationTest, ChunkedDataPipe)
{
    ...
    auto pending_receiver = pending_remote.InitWithNewPipeAndPassReceiver();
    DataElement src(DataElementChunkedDataPipe(std::move(pending_remote), DataElementChunkedDataPipe::ReadOnlyOnce(true)));
    DataElement dest;
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::DataElement>(src, dest));
    ...
    mojo::Remote<mojom::ChunkedDataPipeGetter> remote(dest.As<DataElementChunkedDataPipe>().ReleaseChunkedDataPipeGetter());
...
TEST_F(DataElementDeserializationTest, File)
{
    ...
    DataElement src(DataElementFile(kPath, /*offset=*/3, /*length=*/8, base::Time::UnixEpoch() + base::Minutes(2)));
    DataElement dest;
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::DataElement>(src, dest));
    ...
    EXPECT_EQ(src_file.expected_modification_time(), dest_file.expected_modification_time());
}
