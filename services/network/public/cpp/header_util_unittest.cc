...
TEST(HeaderUtilTest, IsRequestHeaderSafe)
{
    const struct HeaderKeyValuePair {
        ...
        { net::HttpRequestHeaders::kProxyAuthorization, "Basic Zm9vOmJhcg==", false },
        { "Proxy-Foo", "bar", false },
        { "PrOxY-FoO", "bar", false },
...
TEST(HeaderUtilTest, AreRequestHeadersSafe)
{
    ...
    for (const auto& header1 : kHeaders) {
        ...
        for (const auto& header2 : kHeaders) {
            ...
            EXPECT_EQ(header1.is_safe && header2.is_safe, AreRequestHeadersSafe(request_headers2));
        }
    }
}
...
TEST(HeaderUtilTest, ParseReferrerPolicy)
{
    ...
    const TestCase kTests[] = {
        ...
        { "no-referrer-when-downgrade", mojom::ReferrerPolicy::kNoReferrerWhenDowngrade },
        { "origin", mojom::ReferrerPolicy::kOrigin },
        { "origin-when-cross-origin", mojom::ReferrerPolicy::kOriginWhenCrossOrigin },
        ...
        { "strict-origin-when-cross-origin", mojom::ReferrerPolicy::kStrictOriginWhenCrossOrigin },
        // Unknown value.
    ...
    for (const auto& test : kTests) {
        SCOPED_TRACE(::testing::Message() << "referrer_policy_value: \"" << test.referrer_policy_value << "\"");

        auto headers = net::HttpResponseHeaders::TryToCreate(base::StringPrintf("HTTP/1.1 200 OK\r\nReferrer-Policy: %s", test.referrer_policy_value));
        ASSERT_TRUE(headers);
