...
constexpr char kCrossOriginOpenerPolicyHeaderReportOnly[] = "Cross-Origin-Opener-Policy-Report-Only";
...
void ParseHeader(std::string_view header_value, mojom::CrossOriginOpenerPolicyValue* value, mojom::CrossOriginOpenerPolicyValue* soap_by_default_value,
    ...
    if (item && item->item.is_token()) {
        ...
        if (policy_item == kSameOriginAllowPopups) {
            *value = mojom::CrossOriginOpenerPolicyValue::kSameOriginAllowPopups;
            if (soap_by_default_value) {
                *soap_by_default_value = mojom::CrossOriginOpenerPolicyValue::kSameOriginAllowPopups;
            }
        }
        ...
        if ((base::FeatureList::IsEnabled(features::kCoopRestrictProperties) || base::FeatureList::IsEnabled(features::kCoopRestrictPropertiesOriginTrial))
            && policy_item == kRestrictProperties) {
            *value = mojom::CrossOriginOpenerPolicyValue::kRestrictProperties;
            if (soap_by_default_value) {
                *soap_by_default_value = mojom::CrossOriginOpenerPolicyValue::kRestrictProperties;
            }
        }
        ...
        if ((policy_item == kNoopenerAllowPopups) && base::FeatureList::IsEnabled(features::kCoopNoopenerAllowPopups)) {
            *value = mojom::CrossOriginOpenerPolicyValue::kNoopenerAllowPopups;
            if (soap_by_default_value) {
                *soap_by_default_value = mojom::CrossOriginOpenerPolicyValue::kNoopenerAllowPopups;
            }
        }
        auto it = base::ranges::find(item->params, kReportTo, &std::pair<std::string, Item>::first);
...
CrossOriginOpenerPolicy ParseCrossOriginOpenerPolicy(const net::HttpResponseHeaders& headers)
{
    ...
    coop.soap_by_default_value = mojom::CrossOriginOpenerPolicyValue::kSameOriginAllowPopups;

    // Parse Cross-Origin-Opener-Policy:
    if (std::optional<std::string> header_value = headers.GetNormalizedHeader(kCrossOriginOpenerPolicyHeader)) {
        ParseHeader(*header_value, &coop.value, &coop.soap_by_default_value, &coop.reporting_endpoint);
    }

    if (base::FeatureList::IsEnabled(features::kCrossOriginOpenerPolicyByDefault)) {
        coop.value = coop.soap_by_default_value;
    }

    // Parse Cross-Origin-Opener-Policy-Report-Only:
    if (std::optional<std::string> header_value = headers.GetNormalizedHeader(kCrossOriginOpenerPolicyHeaderReportOnly)) {
        ParseHeader(*header_value, &coop.report_only_value, nullptr, &coop.report_only_reporting_endpoint);
    }
