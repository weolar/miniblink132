...
namespace mojo {
template <> struct StructTraits<network::mojom::TransferableSocketDataView, network::TransferableSocket>;
}
...
#if BUILDFLAG(IS_WIN)
    TransferableSocket(net::SocketDescriptor socket, const base::Process& destination_process);
...
private:
    friend struct mojo::StructTraits<network::mojom::TransferableSocketDataView, network::TransferableSocket>;
#if BUILDFLAG(IS_WIN)
