...
constexpr char kReportOnlyHeaderName[] = "document-isolation-policy-report-only";

std::pair<mojom::DocumentIsolationPolicyValue, std::optional<std::string>> Parse(std::string_view header_value)
{
...
DocumentIsolationPolicy ParseDocumentIsolationPolicy(const net::HttpResponseHeaders& headers)
{
    ...
    if (std::optional<std::string> header_value = headers.GetNormalizedHeader(kReportOnlyHeaderName)) {
        std::tie(dip.report_only_value, dip.report_only_reporting_endpoint) = Parse(*header_value);
    }
    return dip;
}
