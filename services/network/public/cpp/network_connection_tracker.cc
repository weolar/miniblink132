...
void OnGetConnectionType(
    scoped_refptr<base::TaskRunner> task_runner, NetworkConnectionTracker::ConnectionTypeCallback user_callback, network::mojom::ConnectionType connection_type)
{
    task_runner->PostTask(FROM_HERE,
        base::BindOnce([](NetworkConnectionTracker::ConnectionTypeCallback callback, network::mojom::ConnectionType type) { std::move(callback).Run(type); },
...
NetworkConnectionTracker::NetworkConnectionTracker(BindingCallback callback)
    ...
    , network_change_observer_list_(new base::ObserverListThreadSafe<NetworkConnectionObserver>(base::ObserverListPolicy::EXISTING_ONLY))
    , leaky_network_change_observer_list_(new base::ObserverListThreadSafe<NetworkConnectionObserver>(base::ObserverListPolicy::EXISTING_ONLY))
{
    Initialize();
...
bool NetworkConnectionTracker::GetConnectionType(network::mojom::ConnectionType* const type, ConnectionTypeCallback callback)
{
    ...
    base::subtle::Atomic32 type_value = base::subtle::NoBarrier_Load(&connection_type_);
    ...
    if (!task_runner_->RunsTasksInCurrentSequence()) {
        connection_type_callbacks_.push_back(base::BindOnce(&OnGetConnectionType, base::SequencedTaskRunner::GetCurrentDefault(), std::move(callback)));
    } else {
...
// static
bool NetworkConnectionTracker::IsConnectionCellular(const network::mojom::ConnectionType type)
{
    switch (type) {
...
void NetworkConnectionTracker::AddNetworkConnectionObserver(NetworkConnectionObserver* observer)
{
    network_change_observer_list_->AddObserver(observer);
}

void NetworkConnectionTracker::AddLeakyNetworkConnectionObserver(NetworkConnectionObserver* observer)
{
    leaky_network_change_observer_list_->AddObserver(observer);
}

void NetworkConnectionTracker::RemoveNetworkConnectionObserver(NetworkConnectionObserver* observer)
{
...
void NetworkConnectionTracker::OnInitialConnectionType(network::mojom::ConnectionType type)
{
    base::AutoLock lock(lock_);
    base::subtle::NoBarrier_Store(&connection_type_, static_cast<base::subtle::Atomic32>(type));
...
void NetworkConnectionTracker::OnNetworkChanged(network::mojom::ConnectionType type)
{
    base::subtle::NoBarrier_Store(&connection_type_, static_cast<base::subtle::Atomic32>(type));
    network_change_observer_list_->Notify(FROM_HERE, &NetworkConnectionObserver::OnConnectionChanged, type);
    leaky_network_change_observer_list_->Notify(FROM_HERE, &NetworkConnectionObserver::OnConnectionChanged, type);
}
...
void NetworkConnectionTracker::Initialize()
{
    ...
    receiver_.set_disconnect_handler(base::BindOnce(&NetworkConnectionTracker::HandleNetworkServicePipeBroken, base::Unretained(this)));
}
