...
mojom::SupportsLoadingModePtr ParseSupportsLoadingMode(std::string_view header_value)
{
    ...
    for (const net::structured_headers::ParameterizedMember& member : *list) {
        ...
        const auto* it = base::ranges::find(kKnownLoadingModes, token, &KnownLoadingMode::token);
...
mojom::SupportsLoadingModePtr ParseSupportsLoadingMode(const net::HttpResponseHeaders& headers)
{
    std::optional<std::string> header_value = headers.GetNormalizedHeader(kSupportsLoadingMode);
    if (!header_value) {
        return nullptr;
    }
