...
TEST_F(TransferableDirectoryTest, MojoTraitsWithPath)
{
    ...
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::TransferableDirectory>(dir, roundtripped));
...
public:
    ...
    void SetUp() override
    {
        ...
        ASSERT_TRUE(base::WriteFile(temp_dir_.GetPath().Append(kFileFromParent), kData));
...
{
    {
        ...
        EXPECT_TRUE(base::PathExists(transferable_temp_dir_.path().Append(kFileFromParent)));

        // Write a file into the directory, for the parent to see.
        EXPECT_FALSE(base::PathExists(transferable_temp_dir_.path().Append(kFileFromChild)));
        ASSERT_TRUE(base::WriteFile(temp_dir_.GetPath().Append(kFileFromChild), kData));
        EXPECT_TRUE(base::PathExists(transferable_temp_dir_.path().Append(kFileFromChild)));
    }
...
TEST_F(TransferableDirectoryTestWithHandle, MojoTraitsWithHandle)
{
    ...
    TransferableDirectory roundtripped;
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::TransferableDirectory>(transferable_temp_dir_, roundtripped));
