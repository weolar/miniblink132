...
#if BUILDFLAG(IS_WIN)
TransferableSocket::TransferableSocket(net::SocketDescriptor socket, const base::Process& destination_process)
    : wsa_info_buffer_(sizeof(WSAPROTOCOL_INFOW), 0)
{
    ...
    if (::WSADuplicateSocketW(socket, destination_process.Pid(), reinterpret_cast<LPWSAPROTOCOL_INFOW>(wsa_info_buffer_.data())) != 0) {
        wsa_info_buffer_.clear();
    }
...
TransferableSocket::~TransferableSocket() = default;
TransferableSocket& TransferableSocket::operator=(TransferableSocket&& other) = default;
...
{
#if DCHECK_IS_ON()
    DCHECK(has_been_transferred_) << "Cannot take a socket before transferring across processes.";
#endif
#if BUILDFLAG(IS_WIN)
    LPWSAPROTOCOL_INFOW protocol_info;
    if (wsa_info_buffer_.empty())
        return net::kInvalidSocket;
    protocol_info = reinterpret_cast<LPWSAPROTOCOL_INFOW>(const_cast<uint8_t*>(wsa_info_buffer_.data()));
    // These fields are documented here:
    // https://docs.microsoft.com/en-us/windows/win32/api/winsock2/ns-winsock2-wsaprotocol_infoa.
    SOCKET s = ::WSASocketW(FROM_PROTOCOL_INFO, FROM_PROTOCOL_INFO, FROM_PROTOCOL_INFO, protocol_info, 0, WSA_FLAG_OVERLAPPED);
    return s;
#elif BUILDFLAG(IS_POSIX) || BUILDFLAG(IS_FUCHSIA)
