...
NetToMojoPendingBuffer::NetToMojoPendingBuffer(mojo::ScopedDataPipeProducerHandle handle, base::span<char> buffer)
...
MojoResult NetToMojoPendingBuffer::BeginWrite(mojo::ScopedDataPipeProducerHandle* handle, scoped_refptr<NetToMojoPendingBuffer>* pending)
{
    base::span<uint8_t> buf;
    MojoResult result = (*handle)->BeginWriteData(kMaxBufSize, MOJO_WRITE_DATA_FLAG_NONE, buf);
    ...
    *pending = new NetToMojoPendingBuffer(std::move(*handle), base::as_writable_chars(buf));
    return MOJO_RESULT_OK;
}

mojo::ScopedDataPipeProducerHandle NetToMojoPendingBuffer::Complete(uint32_t num_bytes)
{
...
NetToMojoIOBuffer::NetToMojoIOBuffer(scoped_refptr<NetToMojoPendingBuffer> pending_buffer, int offset)
...
MojoToNetPendingBuffer::MojoToNetPendingBuffer(mojo::ScopedDataPipeConsumerHandle handle, base::span<const char> buffer)
    : handle_(std::move(handle))
    , buffer_(buffer)
{
    CHECK_LE(buffer_.size(), std::numeric_limits<uint32_t>::max());
...
// static
MojoResult MojoToNetPendingBuffer::BeginRead(mojo::ScopedDataPipeConsumerHandle* handle, scoped_refptr<MojoToNetPendingBuffer>* pending)
{
    ...
    *pending = new MojoToNetPendingBuffer(std::move(*handle), base::as_string_view(buffer));
    return MOJO_RESULT_OK;
}
...
MojoToNetIOBuffer::MojoToNetIOBuffer(scoped_refptr<MojoToNetPendingBuffer> pending_buffer, int bytes_to_be_read)
