...
TEST(FileEnumerationEntryMojomTraitsTest, SuccessCase)
{
    ...
    constexpr base::Time last_accessed = base::Time::UnixEpoch() + base::Seconds(99);
    constexpr base::Time last_modified = base::Time::UnixEpoch() + base::Seconds(33);

    const disk_cache::BackendFileOperations::FileEnumerationEntry original(path, size, last_accessed, last_modified);

    disk_cache::BackendFileOperations::FileEnumerationEntry deserialized;
    EXPECT_TRUE(mojo::test::SerializeAndDeserialize<mojom::FileEnumerationEntry>(original, deserialized));
