...
bool IsRequestHeaderSafe(std::string_view key, std::string_view value)
{
    ...
    for (const auto& header : kUnsafeHeaderValues) {
        if (base::EqualsCaseInsensitiveASCII(header.name, key) && base::EqualsCaseInsensitiveASCII(header.value, value)) {
            return false;
        }
    }
...
mojom::ReferrerPolicy ParseReferrerPolicy(const net::HttpResponseHeaders& response_headers)
{
    using enum net::ReferrerPolicy;
    using enum mojom::ReferrerPolicy;
    std::optional<std::string> referrer_policy_header = response_headers.GetNormalizedHeader("Referrer-Policy");
    if (!referrer_policy_header) {
        return kDefault;
    }

    std::optional<net::ReferrerPolicy> net_policy = net::ReferrerPolicyFromHeader(*referrer_policy_header);
...
bool ShouldSniffContent(const GURL& url, const mojom::URLResponseHead& response)
{
    std::string content_type_options;
    if (response.headers) {
        content_type_options = response.headers->GetNormalizedHeader("x-content-type-options").value_or(std::string());
    }
    bool sniffing_blocked = base::EqualsCaseInsensitiveASCII(content_type_options, "nosniff");
    bool we_would_like_to_sniff = net::ShouldSniffMimeType(url, response.mime_type);
