...
std::ostream& network::mojom::operator<<(std::ostream& a, ::network::mojom::CorsError b)
{
    *(int*)1 = 1;
    return a << (int)(b);
}
...
CorsErrorStatus::CorsErrorStatus(mojom::CorsError cors_error, const std::string& failed_parameter)
    : cors_error(cors_error)
    , failed_parameter(failed_parameter)
{
}

CorsErrorStatus::CorsErrorStatus(mojom::CorsError cors_error, mojom::IPAddressSpace target_address_space, mojom::IPAddressSpace resource_address_space)
    : cors_error(cors_error)
...
bool CorsErrorStatus::operator==(const CorsErrorStatus& rhs) const
{
    ...
    return cors_error == rhs.cors_error && failed_parameter == rhs.failed_parameter && target_address_space == rhs.target_address_space
        && resource_address_space == rhs.resource_address_space;
}

std::ostream& operator<<(std::ostream& os, const CorsErrorStatus& status)
{
    return os << "CorsErrorStatus{ cors_error = " << status.cors_error << ", failed_parameter = " << status.failed_parameter
              << ", target_address_space = " << status.target_address_space << ", resource_address_space = " << status.resource_address_space
