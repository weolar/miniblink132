...
const auto kDisallowSubdomains = mojom::CorsDomainMatchMode::kDisallowSubdomains;
const auto kAllowAnyPort = mojom::CorsPortMatchMode::kAllowAnyPort;
const auto kAllowOnlySpecifiedPort = mojom::CorsPortMatchMode::kAllowOnlySpecifiedPort;
...
public:
    OriginAccessListTest()
        ...
        , https_another_port_example_origin_(url::Origin::Create(GURL("https://example.com:10443")))
        , https_sub_example_origin_(url::Origin::Create(GURL("https://sub.example.com")))
...
protected:
    ...
    OriginAccessList::AccessState CheckAccess(const url::Origin& request_initiator, const std::optional<url::Origin>& isolated_world_origin, const GURL& url)
    {
    ...
    {
        return list_.CheckAccessState(source_origin_, destination_origin.GetURL()) == OriginAccessList::AccessState::kAllowed;
    }
    void SetAllowListEntry(const std::string& protocol, const std::string& host, const uint16_t port, const mojom::CorsDomainMatchMode domain_match_mode,
        const mojom::CorsPortMatchMode port_match_mode)
    {
        std::vector<mojom::CorsOriginPatternPtr> patterns;
        patterns.push_back(
            mojom::CorsOriginPattern::New(protocol, host, port, domain_match_mode, port_match_mode, mojom::CorsOriginAccessMatchPriority::kDefaultPriority));
        list_.SetAllowListForOrigin(source_origin_, patterns);
    }
    void AddAllowListEntry(const std::string& protocol, const std::string& host, const uint16_t port, const mojom::CorsDomainMatchMode domain_match_mode,
        const mojom::CorsPortMatchMode port_match_mode, const mojom::CorsOriginAccessMatchPriority priority)
    {
        list_.AddAllowListEntryForOrigin(source_origin_, protocol, host, port, domain_match_mode, port_match_mode, priority);
    }
    void SetBlockListEntry(const std::string& protocol, const std::string& host, const int16_t port, const mojom::CorsDomainMatchMode domain_match_mode,
        const mojom::CorsPortMatchMode port_match_mode)
    {
        std::vector<mojom::CorsOriginPatternPtr> patterns;
        patterns.push_back(
    ...
    void AddBlockListEntry(const std::string& protocol, const std::string& host, const uint16_t port, const mojom::CorsDomainMatchMode domain_match_mode,
        const mojom::CorsPortMatchMode port_match_mode, const mojom::CorsOriginAccessMatchPriority priority)
    {
        list_.AddBlockListEntryForOrigin(source_origin_, protocol, host, port, domain_match_mode, port_match_mode, priority);
    }
    void ResetLists()
    {
        list_.Clear();
...
TEST_F(OriginAccessListTest, IsAccessAllowedWithPort)
{
    ...
    SetAllowListEntry("https", "example.com", kHttpsPort, kDisallowSubdomains, kAllowOnlySpecifiedPort);
    ...
    SetAllowListEntry("https", "example.com", kAnyPort, kDisallowSubdomains, kAllowAnyPort);
    EXPECT_TRUE(IsAllowed(https_example_origin()));
...
TEST_F(OriginAccessListTest, IsAccessAllowedForIsolatedWorldOrigin)
{
    ...
    SetAllowListEntry(target.scheme(), target.host(), kHttpsPort, kDisallowSubdomains, kAllowOnlySpecifiedPort);
    ...
    // OriginAccessList.
    EXPECT_EQ(OriginAccessList::AccessState::kAllowed, CheckAccess(source_origin(), std::nullopt, target));
    ...
    // OriginAccessList.
    EXPECT_EQ(OriginAccessList::AccessState::kAllowed, CheckAccess(https_another_port_example_origin(), source_origin(), target));
    ...
    EXPECT_EQ(OriginAccessList::AccessState::kNotListed, CheckAccess(source_origin(), https_another_port_example_origin(), target));
}
...
TEST_F(OriginAccessListTest, IsAccessAllowed)
{
    ...
    // subdomains.
    AddAllowListEntry("https", "example.com", kAnyPort, kAllowSubdomains, kAllowAnyPort, mojom::CorsOriginAccessMatchPriority::kDefaultPriority);
    EXPECT_TRUE(IsAllowed(https_example_origin()));
...
TEST_F(OriginAccessListTest, IsAccessAllowedWithBlockListEntry)
{
    ...
    SetAllowListEntry("https", "example.com", kAnyPort, kAllowSubdomains, kAllowAnyPort);
    SetBlockListEntry("https", "example.com", kAnyPort, kDisallowSubdomains, kAllowAnyPort);
...
{
    SetAllowListEntry("https", "", kAnyPort, kAllowSubdomains, kAllowAnyPort);
    AddBlockListEntry("https", "google.com", kAnyPort, kDisallowSubdomains, kAllowAnyPort, mojom::CorsOriginAccessMatchPriority::kDefaultPriority);
...
TEST_F(OriginAccessListTest, IsPriorityRespected)
{
    ...
    AddBlockListEntry("https", "example.com", kAnyPort, kAllowSubdomains, kAllowAnyPort, mojom::CorsOriginAccessMatchPriority::kLowPriority);
    EXPECT_FALSE(IsAllowed(https_example_origin()));
    EXPECT_FALSE(IsAllowed(https_sub_example_origin()));

    // Higher priority allowlist overrides lower priority blocklist.
    AddAllowListEntry("https", "example.com", kAnyPort, kDisallowSubdomains, kAllowAnyPort, mojom::CorsOriginAccessMatchPriority::kMediumPriority);
    EXPECT_TRUE(IsAllowed(https_example_origin()));
    EXPECT_FALSE(IsAllowed(https_sub_example_origin()));
}
...
{
    AddAllowListEntry("https", "example.com", kAnyPort, kAllowSubdomains, kAllowAnyPort, mojom::CorsOriginAccessMatchPriority::kLowPriority);
    EXPECT_TRUE(IsAllowed(https_example_origin()));
...
{
    const url::Origin kOrigin1 = url::Origin::Create(GURL("https://foo.google.com"));
    const url::Origin kOrigin2 = url::Origin::Create(GURL("https://bar.google.com"));
    ...
    list.AddAllowListEntryForOrigin(
        kOrigin1, kProtocol, kDomain1, kPort1, kAllowSubdomains, kAllowAnyPort, mojom::CorsOriginAccessMatchPriority::kMediumPriority);
    list.AddBlockListEntryForOrigin(
        kOrigin2, kProtocol, kDomain2, kPort2, kDisallowSubdomains, kAllowOnlySpecifiedPort, mojom::CorsOriginAccessMatchPriority::kDefaultPriority);

    std::vector<mojom::CorsOriginAccessPatternsPtr> patterns = list.CreateCorsOriginAccessPatternsList();
    ...
    for (const auto& pattern : patterns) {
        if (pattern->source_origin == kOrigin1) {
            ...
            EXPECT_EQ(kAllowSubdomains, pattern->allow_patterns[0]->domain_match_mode);
            EXPECT_EQ(kAllowAnyPort, pattern->allow_patterns[0]->port_match_mode);
            EXPECT_EQ(mojom::CorsOriginAccessMatchPriority::kMediumPriority, pattern->allow_patterns[0]->priority);
            ...
            EXPECT_EQ(kDisallowSubdomains, pattern->block_patterns[0]->domain_match_mode);
            EXPECT_EQ(kAllowOnlySpecifiedPort, pattern->block_patterns[0]->port_match_mode);
            EXPECT_EQ(mojom::CorsOriginAccessMatchPriority::kDefaultPriority, pattern->block_patterns[0]->priority);
        } else {
            FAIL();
        }
    }
