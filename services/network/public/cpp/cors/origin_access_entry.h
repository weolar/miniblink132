...
bool COMPONENT_EXPORT(NETWORK_CPP) IsSubdomainOfHost(const std::string& subdomain, const std::string& host);
...
public:
    enum MatchResult { kMatchesOrigin, kMatchesOriginButIsPublicSuffix, kDoesNotMatchOrigin };
    ...
    // MatchesOrigin().
    OriginAccessEntry(const std::string& protocol, const std::string& domain, const uint16_t port, const mojom::CorsDomainMatchMode domain_match_mode,
        const mojom::CorsPortMatchMode port_match_mode,
        const mojom::CorsOriginAccessMatchPriority priority = mojom::CorsOriginAccessMatchPriority::kDefaultPriority);
    ...
    bool host_is_ip_address() const
    {
        return host_is_ip_address_;
    }
    mojom::CorsOriginAccessMatchPriority priority() const
    {
        return priority_;
    }
