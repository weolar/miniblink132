...
bool IsCorsUnsafeRequestHeaderByte(char c)
{
    ...
    return (u < 0x20 && u != 0x09) || u == 0x22 || u == 0x28 || u == 0x29 || u == 0x3a || u == 0x3c || u == 0x3e || u == 0x3f || u == 0x40 || u == 0x5b
        || u == 0x5c || u == 0x5d || u == 0x7b || u == 0x7d || u == 0x7f;
}
...
bool IsCorsSafelistedLowerCaseContentType(const std::string& value)
{
    ...
    std::optional<std::string> mime_type = net::ExtractMimeTypeFromMediaType(value,
        /*accept_comma_separated=*/false);
    if (!mime_type.has_value()) {
        return false;
    }

    return *mime_type == "application/x-www-form-urlencoded" || *mime_type == "multipart/form-data" || *mime_type == "text/plain";
}

bool IsNoCorsSafelistedHeaderNameLowerCase(const std::string& lower_name)
{
    if (lower_name != "accept" && lower_name != "accept-language" && lower_name != "content-language" && lower_name != "content-type") {
        return false;
    }
    return true;
}
...
const char kAccessControlAllowPrivateNetwork[] = "Access-Control-Allow-Private-Network";
...
const char kAccessControlRequestPrivateNetwork[] = "Access-Control-Request-Private-Network";
...
base::expected<void, CorsErrorStatus> CheckAccess(const GURL& response_url, const std::optional<std::string>& allow_origin_header,
    const std::optional<std::string>& allow_credentials_header, mojom::CredentialsMode credentials_mode, const url::Origin& origin)
{
    if (allow_origin_header == kAsterisk) {
        ...
        if (response_url.SchemeIsHTTPOrHTTPS()) {
            return base::unexpected(CorsErrorStatus(mojom::CorsError::kWildcardOriginNotAllowed));
        }
    } else if (!allow_origin_header) {
        return base::unexpected(CorsErrorStatus(mojom::CorsError::kMissingAllowOriginHeader));
        ...
        if (allow_origin_header->find_first_of(" ,") != std::string::npos) {
            return base::unexpected(CorsErrorStatus(mojom::CorsError::kMultipleAllowOriginValues, *allow_origin_header));
        }

        // Check valid "null" first since GURL assumes it as invalid.
        if (*allow_origin_header == "null") {
            return base::unexpected(CorsErrorStatus(mojom::CorsError::kAllowOriginMismatch, *allow_origin_header));
        }
        ...
        if (!header_origin.is_valid()) {
            return base::unexpected(CorsErrorStatus(mojom::CorsError::kInvalidAllowOriginValue, *allow_origin_header));
        }
    ...
    if (credentials_mode == mojom::CredentialsMode::kInclude) {
        ...
        if (allow_credentials_header != kLowerCaseTrue) {
            return base::unexpected(CorsErrorStatus(mojom::CorsError::kInvalidAllowCredentials, allow_credentials_header.value_or(std::string())));
        }
    }
    return base::ok();
}

base::expected<void, CorsErrorStatus> CheckAccessAndReportMetrics(const GURL& response_url, const std::optional<std::string>& allow_origin_header,
    const std::optional<std::string>& allow_credentials_header, mojom::CredentialsMode credentials_mode, const url::Origin& origin)
{
    auto check_result = CheckAccess(response_url, allow_origin_header, allow_credentials_header, credentials_mode, origin);
    cors::AccessCheckResult result = check_result.has_value() ? cors::AccessCheckResult::kPermitted : cors::AccessCheckResult::kNotPermitted;

    base::UmaHistogramEnumeration("Net.Cors.AccessCheckResult", result);
    if (!IsOriginPotentiallyTrustworthy(origin)) {
        base::UmaHistogramEnumeration("Net.Cors.AccessCheckResult.NotSecureRequestor", result);
    }
    return check_result;
}
...
{
    return mode == mojom::RequestMode::kCors || mode == mojom::RequestMode::kCorsWithForcedPreflight;
}
...
bool IsCorsSafelistedMethod(const std::string& method)
{
    ...
    return method_upper == net::HttpRequestHeaders::kGetMethod || method_upper == net::HttpRequestHeaders::kHeadMethod
...
bool IsCorsSafelistedHeader(const std::string& name, const std::string& value)
{
    ...
    if (lower_name == "accept") {
    ...
    } else if (lower_name == "accept-language" || lower_name == "content-language") {
        return base::ranges::all_of(value, [](char c) {
            return (0x30 <= c && c <= 0x39) || (0x41 <= c && c <= 0x5a) || (0x61 <= c && c <= 0x7a) || c == 0x20 || c == 0x2a || c == 0x2c || c == 0x2d
                || c == 0x2e || c == 0x3b || c == 0x3d;
        });
        ...
        if (base::ranges::any_of(lower_value, [](char c) { return net::HttpUtil::IsLWS(c) || c == ','; })) {
            return false;
        }
        ...
        if (ranges.size() != 1 || ranges[0].IsSuffixByteRange())
            return false;
        return true;
    } else if (lower_name == "device-memory" || lower_name == "dpr" || lower_name == "downlink") {
        return IsSimilarToDoubleABNF(value);
    } else if (lower_name == "width" || lower_name == "viewport-width" || lower_name == "rtt") {
        return IsSimilarToIntABNF(value);
    } else if (lower_name == "ect") {
        auto* const* begin = network::kWebEffectiveConnectionTypeMapping;
        auto* const* end = network::kWebEffectiveConnectionTypeMapping + network::kWebEffectiveConnectionTypeMappingCount;
...
{
    const std::string lower_name = base::ToLowerASCII(name);
    const std::vector<std::string> privileged_no_cors_header_names = PrivilegedNoCorsHeaderNames();
...
bool IsNoCorsSafelistedHeader(const std::string& name, const std::string& value)
{
    const std::string lower_name = base::ToLowerASCII(name);
...
std::vector<std::string> CorsUnsafeRequestHeaderNames(const net::HttpRequestHeaders::HeaderVector& headers)
{
    ...
    if (safe_list_value_size > kSafeListValueSizeMax) {
        header_names.insert(header_names.end(), potentially_unsafe_names.begin(), potentially_unsafe_names.end());
    }
    return header_names;
}
...
bool IsForbiddenMethod(const std::string& method)
{
    ...
    return upper_method == net::HttpRequestHeaders::kConnectMethod || upper_method == net::HttpRequestHeaders::kTraceMethod
...
bool CalculateCredentialsFlag(mojom::CredentialsMode credentials_mode, mojom::FetchResponseType response_tainting)
{
...
mojom::FetchResponseType CalculateResponseType(mojom::RequestMode mode, bool is_request_considered_same_origin)
{
    if (is_request_considered_same_origin || mode == network::mojom::RequestMode::kNavigate || mode == network::mojom::RequestMode::kSameOrigin) {
