...
bool IsPublicSuffixSubdomainOfHost(const std::string& subdomain, const std::string& host)
{
    size_t public_suffix_length = net::registry_controlled_domains::PermissiveGetHostRegistryLength(
        subdomain, net::registry_controlled_domains::INCLUDE_UNKNOWN_REGISTRIES, net::registry_controlled_domains::INCLUDE_PRIVATE_REGISTRIES);
    return public_suffix_length != std::string::npos && public_suffix_length != 0
        && IsSubdomainOfHost(subdomain.substr(subdomain.length() - public_suffix_length), host);
}
...
OriginAccessEntry::OriginAccessEntry(const std::string& protocol, const std::string& host, const uint16_t port,
    ...
    , host_is_public_suffix_(false)
{
    ...
    size_t public_suffix_length = net::registry_controlled_domains::PermissiveGetHostRegistryLength(
        host_, net::registry_controlled_domains::INCLUDE_UNKNOWN_REGISTRIES, net::registry_controlled_domains::INCLUDE_PRIVATE_REGISTRIES);
    ...
    } else if (domain_match_mode_ == mojom::CorsDomainMatchMode::kAllowRegistrableDomains) {
        DCHECK(public_suffix_length);
        // The "2" in the next line is 1 for the '.', plus a 1-char minimum label
        // length.
        const size_t dot = host_.rfind('.', host_.length() - public_suffix_length - 2);
...
OriginAccessEntry::MatchResult OriginAccessEntry::MatchesOrigin(const url::Origin& origin) const
{
    if (protocol_ != origin.scheme())
        return kDoesNotMatchOrigin;

    if (port_match_mode_ == mojom::CorsPortMatchMode::kAllowOnlySpecifiedPort && port_ != origin.port()) {
        return kDoesNotMatchOrigin;
    }
...
OriginAccessEntry::MatchResult OriginAccessEntry::MatchesDomain(const std::string& domain) const
{
    // Special case: Include subdomains and empty host means "all hosts, including
    // ip addresses".
    if (domain_match_mode_ != mojom::CorsDomainMatchMode::kDisallowSubdomains && host_.empty()) {
    ...
    case mojom::CorsDomainMatchMode::kAllowRegistrableDomains:
        ...
        if (registrable_domain_.empty()) {
            if (!IsSubdomainOfHost(domain, host_))
                return kDoesNotMatchOrigin;
        } else if (registrable_domain_ != domain && !IsSubdomainOfHost(domain, registrable_domain_)) {
            return kDoesNotMatchOrigin;
        }
        break;
    };
...
mojo::StructPtr<mojom::CorsOriginPattern> OriginAccessEntry::CreateCorsOriginPattern() const
{
    return mojom::CorsOriginPattern::New(protocol_, host_, port_, domain_match_mode_, port_match_mode_, priority_);
}
