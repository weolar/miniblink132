...
TEST_F(CorsTest, CheckAccessDetectsWildcardOriginNotAllowed)
{
    ...
    const auto result = CheckAccess(
        response_url, allow_all_header /* allow_origin_header */, std::nullopt /* allow_credentials_header */, network::mojom::CredentialsMode::kOmit, origin);
    EXPECT_TRUE(result.has_value());

    // Access-Control-Allow-Origin '*' should not be allowed if credentials mode
    // is kInclude.
    const auto result2 = CheckAccess(response_url, allow_all_header /* allow_origin_header */, std::nullopt /* allow_credentials_header */,
        network::mojom::CredentialsMode::kInclude, origin);
    ASSERT_FALSE(result2.has_value());
    EXPECT_EQ(mojom::CorsError::kWildcardOriginNotAllowed, result2.error().cors_error);
}
...
TEST_F(CorsTest, CheckAccessDetectsMissingAllowOriginHeader)
{
    ...
    const auto result = CheckAccess(
        response_url, std::nullopt /* allow_origin_header */, std::nullopt /* allow_credentials_header */, network::mojom::CredentialsMode::kOmit, origin);
    ASSERT_FALSE(result.has_value());
    EXPECT_EQ(mojom::CorsError::kMissingAllowOriginHeader, result.error().cors_error);
}
...
TEST_F(CorsTest, CheckAccessDetectsMultipleAllowOriginValues)
{
    ...
    const std::string space_separated_multiple_origins("http://example.com http://another.example.com");
    const auto result1 = CheckAccess(response_url, space_separated_multiple_origins /* allow_origin_header */, std::nullopt /* allow_credentials_header */,
        network::mojom::CredentialsMode::kOmit, origin);
    ASSERT_FALSE(result1.has_value());
    EXPECT_EQ(mojom::CorsError::kMultipleAllowOriginValues, result1.error().cors_error);

    const std::string comma_separated_multiple_origins("http://example.com,http://another.example.com");
    const auto result2 = CheckAccess(response_url, comma_separated_multiple_origins /* allow_origin_header */, std::nullopt /* allow_credentials_header */,
        network::mojom::CredentialsMode::kOmit, origin);
    ASSERT_FALSE(result2.has_value());
    EXPECT_EQ(mojom::CorsError::kMultipleAllowOriginValues, result2.error().cors_error);
}
...
TEST_F(CorsTest, CheckAccessDetectsInvalidAllowOriginValue)
{
    ...
    const auto result = CheckAccess(response_url, std::string("invalid.origin") /* allow_origin_header */, std::nullopt /* allow_credentials_header */,
        network::mojom::CredentialsMode::kOmit, origin);
    ASSERT_FALSE(result.has_value());
    EXPECT_EQ(mojom::CorsError::kInvalidAllowOriginValue, result.error().cors_error);
...
TEST_F(CorsTest, CheckAccessDetectsAllowOriginMismatch)
{
    ...
    const auto result1 = CheckAccess(response_url, origin.Serialize() /* allow_origin_header */, std::nullopt /* allow_credentials_header */,
        network::mojom::CredentialsMode::kOmit, origin);
    ASSERT_TRUE(result1.has_value());

    const auto result2 = CheckAccess(response_url, std::string("http://not.google.com") /* allow_origin_header */, std::nullopt /* allow_credentials_header */,
        network::mojom::CredentialsMode::kOmit, origin);
    ASSERT_FALSE(result2.has_value());
    ...
    const auto result3 = CheckAccess(
        response_url, null_string /* allow_origin_header */, std::nullopt /* allow_credentials_header */, network::mojom::CredentialsMode::kOmit, null_origin);
...
TEST_F(CorsTest, CheckAccessDetectsInvalidAllowCredential)
{
    ...
    const auto result1 = CheckAccess(response_url, origin.Serialize() /* allow_origin_header */, std::string("true") /* allow_credentials_header */,
        network::mojom::CredentialsMode::kInclude, origin);
    ASSERT_TRUE(result1.has_value());

    const auto restul2 = CheckAccess(response_url, origin.Serialize() /* allow_origin_header */, std::string("fuga") /* allow_credentials_header */,
        network::mojom::CredentialsMode::kInclude, origin);
    ASSERT_FALSE(restul2.has_value());
    EXPECT_EQ(mojom::CorsError::kInvalidAllowCredentials, restul2.error().cors_error);
...
constexpr char kAccessCheckHistogramNotSecure[] = "Net.Cors.AccessCheckResult.NotSecureRequestor";
...
TEST_F(CorsTest, CheckAccessAndReportMetricsForPermittedSecureOrigin)
{
    ...
    EXPECT_TRUE(CheckAccessAndReportMetrics(
        response_url, origin.Serialize() /* allow_origin_header */, std::nullopt /* allow_credentials_header */, network::mojom::CredentialsMode::kOmit, origin)
                    .has_value());
    histogram_tester.ExpectUniqueSample(kAccessCheckHistogram, AccessCheckResult::kPermitted, 1);
...
TEST_F(CorsTest, CheckAccessAndReportMetricsForPermittedNotSecureOrigin)
{
    ...
    EXPECT_TRUE(CheckAccessAndReportMetrics(
        response_url, origin.Serialize() /* allow_origin_header */, std::nullopt /* allow_credentials_header */, network::mojom::CredentialsMode::kOmit, origin)
                    .has_value());
    histogram_tester.ExpectUniqueSample(kAccessCheckHistogram, AccessCheckResult::kPermitted, 1);
    histogram_tester.ExpectUniqueSample(kAccessCheckHistogramNotSecure, AccessCheckResult::kPermitted, 1);
}
...
TEST_F(CorsTest, CheckAccessAndReportMetricsForNotPermittedSecureOrigin)
{
    ...
    histogram_tester.ExpectUniqueSample(kAccessCheckHistogram, AccessCheckResult::kNotPermitted, 1);
    histogram_tester.ExpectTotalCount(kAccessCheckHistogramNotSecure, 0);
}
...
TEST_F(CorsTest, SafelistedAccept)
{
    ...
    constexpr char kAllowed[] = "\t !#$%&'*+,-./0123456789;="
    ...
    for (int i = 0; i < 128; ++i) {
        ...
        EXPECT_EQ(std::find(kAllowed, end, c) != end, IsCorsSafelistedHeader("accept", std::string(1, c)));
        EXPECT_EQ(std::find(kAllowed, end, c) != end, IsCorsSafelistedHeader("AccepT", std::string(1, c)));
    }
...
TEST_F(CorsTest, SafelistedAcceptLanguage)
{
    ...
    constexpr char kAllowed[] = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz *,-.;=";
    for (int i = CHAR_MIN; i <= CHAR_MAX; ++i) {
        SCOPED_TRACE(testing::Message() << "c = static_cast<char>(" << i << ")");
        char c = static_cast<char>(i);
        // 1 for the trailing null character.
        auto* end = kAllowed + std::size(kAllowed) - 1;
        EXPECT_EQ(std::find(kAllowed, end, c) != end, IsCorsSafelistedHeader("aCcEPT-lAngUAge", std::string(1, c)));
    }
    EXPECT_TRUE(IsCorsSafelistedHeader("accept-language", std::string(128, 'a')));
    EXPECT_FALSE(IsCorsSafelistedHeader("accept-language", std::string(129, 'a')));
    EXPECT_TRUE(IsCorsSafelistedHeader("aCcEPT-lAngUAge", std::string(128, 'a')));
    EXPECT_FALSE(IsCorsSafelistedHeader("aCcEPT-lAngUAge", std::string(129, 'a')));
}
...
{
    EXPECT_TRUE(IsCorsSafelistedHeader("Sec-CH-Prefers-Color-Scheme", "\"Prefers-Color-Scheme!\""));
}

TEST_F(CorsTest, SafelistedSecCHPrefersReducedMotion)
{
    EXPECT_TRUE(IsCorsSafelistedHeader("Sec-CH-Prefers-Reduced-Motion", "\"Prefers-Reduced-Motion!\""));
}

TEST_F(CorsTest, SafelistedSecCHPrefersReducedTransparency)
{
    EXPECT_TRUE(IsCorsSafelistedHeader("Sec-CH-Prefers-Reduced-Transparency", "\"Prefers-Reduced-Transparency!\""));
}

TEST_F(CorsTest, SafelistedSecCHUAFormFactors)
{
    EXPECT_TRUE(IsCorsSafelistedHeader("Sec-CH-UA-Form-Factors", "\"Form Factors!\""));
}
...
TEST_F(CorsTest, SafelistedSecCHUA)
{
    ...
    EXPECT_TRUE(IsCorsSafelistedHeader("Sec-CH-UA-Platform-Version", "\"Platform-Version!\""));
...
{
    EXPECT_TRUE(IsCorsSafelistedHeader("Sec-CH-Viewport-Height", "\"Viewport-Height!\""));
}
...
TEST_F(CorsTest, SafelistedContentLanguage)
{
    ...
    for (int i = CHAR_MIN; i <= CHAR_MAX; ++i) {
        ...
        // 1 for the trailing null character.
        auto* end = kAllowed + std::size(kAllowed) - 1;
        EXPECT_EQ(std::find(kAllowed, end, c) != end, IsCorsSafelistedHeader("content-language", std::string(1, c)));
        EXPECT_EQ(std::find(kAllowed, end, c) != end, IsCorsSafelistedHeader("cONTent-LANguaGe", std::string(1, c)));
    }
    EXPECT_TRUE(IsCorsSafelistedHeader("content-language", std::string(128, 'a')));
    EXPECT_FALSE(IsCorsSafelistedHeader("content-language", std::string(129, 'a')));
    EXPECT_TRUE(IsCorsSafelistedHeader("cONTent-LANguaGe", std::string(128, 'a')));
    EXPECT_FALSE(IsCorsSafelistedHeader("cONTent-LANguaGe", std::string(129, 'a')));
}
...
TEST_F(CorsTest, SafelistedContentType)
{
    ...
    EXPECT_TRUE(IsCorsSafelistedHeader("CoNtEnt-TyPE", "text/plain"));
    EXPECT_TRUE(IsCorsSafelistedHeader("content-type", "text/plain; charset=utf-8"));
    EXPECT_TRUE(IsCorsSafelistedHeader("content-type", "  text/plain ; charset=UTF-8"));
    EXPECT_TRUE(IsCorsSafelistedHeader("content-type", "text/plain; param=BOGUS"));
    EXPECT_TRUE(IsCorsSafelistedHeader("content-type", "application/x-www-form-urlencoded"));
    ...
    EXPECT_FALSE(IsCorsSafelistedHeader("CoNtEnt-TyPE", "image/png"));
    EXPECT_TRUE(IsCorsSafelistedHeader("content-type", "text/plain; charset=" + std::string(108, 'a')));
    EXPECT_TRUE(IsCorsSafelistedHeader("cONTent-tYPE", "text/plain; charset=" + std::string(108, 'a')));
    EXPECT_FALSE(IsCorsSafelistedHeader("content-type", "text/plain; charset=" + std::string(109, 'a')));
    EXPECT_FALSE(IsCorsSafelistedHeader("cONTent-tYPE", "text/plain; charset=" + std::string(109, 'a')));
}
...
TEST_F(CorsTest, CorsUnsafeRequestHeaderNames)
{
    ...
    EXPECT_EQ(CorsUnsafeRequestHeaderNames({ { "content-type", "text/plain" }, { "dpr", "12345" }, { "aCCept", "en,ja" }, { "accept-charset", "utf-8" },
                  { "uSer-Agent", "foo" }, { "hogE", "fuga" } }),
        List({ "accept-charset", "user-agent", "hoge" }));

    // All headers are not safelisted.
    EXPECT_EQ(CorsUnsafeRequestHeaderNames({ { "content-type", "text/html" }, { "dpr", "123-45" }, { "aCCept", "en,ja" }, { "accept-charset", "utf-8" },
                  { "uSer-Agent", "foo" }, { "hogE", "fuga" } }),
        List({ "content-type", "dpr", "accept-charset", "user-agent", "hoge" }));

    // |safelistValueSize| is 1024.
    EXPECT_EQ(CorsUnsafeRequestHeaderNames({ { "content-type", "text/plain; charset=" + std::string(108, '1') }, { "accept", std::string(128, '1') },
                  { "accept-language", std::string(128, '1') }, { "content-language", std::string(128, '1') }, { "dpr", std::string(128, '1') },
                  { "device-memory", std::string(128, '1') }, { "save-data", "on" }, { "viewport-width", std::string(128, '1') },
                  { "width", std::string(126, '1') }, { "hogE", "fuga" } }),
        List({ "hoge" }));
    ...
    EXPECT_EQ(CorsUnsafeRequestHeaderNames({ { "content-type", "text/plain; charset=" + std::string(108, '1') }, { "accept", std::string(128, '1') },
                  { "accept-language", std::string(128, '1') }, { "content-language", std::string(128, '1') }, { "dpr", std::string(128, '1') },
                  { "device-memory", std::string(128, '1') }, { "save-data", "on" }, { "viewport-width", std::string(128, '1') },
                  { "width", std::string(127, '1') }, { "hogE", "fuga" } }),
        List({ "hoge", "content-type", "accept", "accept-language", "content-language", "dpr", "device-memory", "save-data", "viewport-width", "width" }));

    // |safelistValueSize| is 897 because "content-type" is not safelisted.
    EXPECT_EQ(CorsUnsafeRequestHeaderNames({ { "content-type", "text/plain; charset=" + std::string(128, '1') }, { "accept", std::string(128, '1') },
                  { "accept-language", std::string(128, '1') }, { "content-language", std::string(128, '1') }, { "dpr", std::string(128, '1') },
                  { "device-memory", std::string(128, '1') }, { "save-data", "on" }, { "viewport-width", std::string(128, '1') },
                  { "width", std::string(127, '1') }, { "hogE", "fuga" } }),
