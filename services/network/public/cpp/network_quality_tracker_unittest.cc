...
class TestEffectiveConnectionTypeObserver : public NetworkQualityTracker::EffectiveConnectionTypeObserver {
public:
    explicit TestEffectiveConnectionTypeObserver(NetworkQualityTracker* tracker)
        : num_notifications_(0)
        , tracker_(tracker)
        , expected_effective_connection_type_(net::EFFECTIVE_CONNECTION_TYPE_UNKNOWN)
        , effective_connection_type_(net::EFFECTIVE_CONNECTION_TYPE_UNKNOWN)
    {
        tracker_->AddEffectiveConnectionTypeObserver(this);
    }

    TestEffectiveConnectionTypeObserver(const TestEffectiveConnectionTypeObserver&) = delete;
    TestEffectiveConnectionTypeObserver& operator=(const TestEffectiveConnectionTypeObserver&) = delete;
    ...
    void WaitForNotification(net::EffectiveConnectionType expected_effective_connection_type)
    {
...
class TestRTTAndThroughputEstimatesObserver : public NetworkQualityTracker::RTTAndThroughputEstimatesObserver {
public:
    ...
    TestRTTAndThroughputEstimatesObserver(const TestRTTAndThroughputEstimatesObserver&) = delete;
    TestRTTAndThroughputEstimatesObserver& operator=(const TestRTTAndThroughputEstimatesObserver&) = delete;
    ...
    void OnRTTOrThroughputEstimatesComputed(base::TimeDelta http_rtt, base::TimeDelta transport_rtt, int32_t downstream_throughput_kbps) override
    {
        ...
        EXPECT_EQ(downstream_throughput_kbps, tracker_->GetDownstreamThroughputKbps());
    ...
    void VerifyNetworkQualityMatchesWithTracker() const
    {
        ...
        EXPECT_EQ(tracker_->GetDownstreamThroughputKbps(), downstream_throughput_kbps_);
    }
...
public:
        ...
        : task_environment_(base::test::TaskEnvironment::MainThreadType::IO)
    {
        ...
        network_service_ = network::NetworkService::Create(network_service_remote.InitWithNewPipeAndPassReceiver());
        tracker_ = std::make_unique<NetworkQualityTracker>(base::BindRepeating(&NetworkQualityTrackerTest::mojom_network_service, base::Unretained(this)));
        ect_observer_ = std::make_unique<TestEffectiveConnectionTypeObserver>(tracker_.get());
        rtt_throughput_observer_ = std::make_unique<TestRTTAndThroughputEstimatesObserver>(tracker_.get());
    }

    NetworkQualityTrackerTest(const NetworkQualityTrackerTest&) = delete;
    NetworkQualityTrackerTest& operator=(const NetworkQualityTrackerTest&) = delete;
    ...
    NetworkQualityTracker* network_quality_tracker()
    {
        return tracker_.get();
    }
    ...
    void SimulateEffectiveConnectionTypeChange(net::EffectiveConnectionType type)
    {
        network_service()->network_quality_estimator()->SimulateNetworkQualityChangeForTesting(type);
    }
...
private:
    ...
    std::unique_ptr<TestRTTAndThroughputEstimatesObserver> rtt_throughput_observer_;
};

TEST_F(NetworkQualityTrackerTest, ECTObserverNotified)
{
    EXPECT_EQ(net::EFFECTIVE_CONNECTION_TYPE_UNKNOWN, effective_connection_type_observer()->effective_connection_type());
    ...
    EXPECT_EQ(net::EFFECTIVE_CONNECTION_TYPE_3G, effective_connection_type_observer()->effective_connection_type());
    ...
    EXPECT_EQ(base::Milliseconds(400), network_quality_tracker()->GetTransportRTT());
    ...
    effective_connection_type_observer()->WaitForNotification(net::EFFECTIVE_CONNECTION_TYPE_2G);
    EXPECT_EQ(net::EFFECTIVE_CONNECTION_TYPE_2G, effective_connection_type_observer()->effective_connection_type());
    // Typical RTT and downlink values when effective connection type is 3G. Taken
    // from net::NetworkQualityEstimatorParams.
    EXPECT_EQ(base::Milliseconds(1800), network_quality_tracker()->GetHttpRTT());
    EXPECT_EQ(base::Milliseconds(1500), network_quality_tracker()->GetTransportRTT());
...
TEST_F(NetworkQualityTrackerTest, ECTObserverNotifiedUnknown)
{
    ...
    effective_connection_type_observer()->WaitForNotification(net::EFFECTIVE_CONNECTION_TYPE_UNKNOWN);
    EXPECT_EQ(net::EFFECTIVE_CONNECTION_TYPE_UNKNOWN, effective_connection_type_observer()->effective_connection_type());
    ...
    EXPECT_EQ(std::numeric_limits<int32_t>::max(), network_quality_tracker()->GetDownstreamThroughputKbps());
}
...
TEST_F(NetworkQualityTrackerTest, ECTObserverNotifiedOnAddition)
{
    ...
    auto ect_observer_2 = std::make_unique<TestEffectiveConnectionTypeObserver>(network_quality_tracker());
    ect_observer_2->WaitForNotification(net::EFFECTIVE_CONNECTION_TYPE_3G);
    EXPECT_EQ(net::EFFECTIVE_CONNECTION_TYPE_3G, ect_observer_2->effective_connection_type());
}
...
TEST_F(NetworkQualityTrackerTest, RttThroughputObserverNotifiedOnAddition)
{
    ...
    auto rtt_throughput_observer_2 = std::make_unique<TestRTTAndThroughputEstimatesObserver>(network_quality_tracker());
