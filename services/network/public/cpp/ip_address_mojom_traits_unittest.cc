...
TEST(IPAddressStructTraitsTest, Ipv4)
{
    ...
    EXPECT_TRUE(mojo::test::SerializeAndDeserialize<network::mojom::IPAddress>(original, deserialized));
...
TEST(IPAddressStructTraitsTest, InvalidAddress)
{
    ...
    IPAddress deserialized;
    EXPECT_FALSE(mojo::test::SerializeAndDeserialize<network::mojom::IPAddress>(original, deserialized));
}
