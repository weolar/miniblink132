...
class /*COMPONENT_EXPORT(NETWORK_CPP)*/ SharedURLLoaderFactory : public base::RefCounted<SharedURLLoaderFactory>, public mojom::URLLoaderFactory {
public:
    static scoped_refptr<SharedURLLoaderFactory> Create(std::unique_ptr<PendingSharedURLLoaderFactory> pending_factory);

    // From mojom::URLLoaderFactory:
    void Clone(mojo::PendingReceiver<mojom::URLLoaderFactory> receiver) override = 0;
...
public:
    ...
    PendingSharedURLLoaderFactory& operator=(const PendingSharedURLLoaderFactory&) = delete;
