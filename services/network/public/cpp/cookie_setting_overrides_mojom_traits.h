...
template <> struct COMPONENT_EXPORT(NETWORK_CPP_BASE) StructTraits<network::mojom::CookieSettingOverridesDataView, net::CookieSettingOverrides> {
    static uint64_t overrides_bitmask(const net::CookieSettingOverrides& input)
    {
        return input.ToEnumBitmask();
    }

    static bool Read(network::mojom::CookieSettingOverridesDataView data, net::CookieSettingOverrides* out);
};
