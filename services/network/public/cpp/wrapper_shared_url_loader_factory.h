...
class COMPONENT_EXPORT(NETWORK_CPP) WrapperPendingSharedURLLoaderFactory : public network::PendingSharedURLLoaderFactory {
public:
    WrapperPendingSharedURLLoaderFactory();
    explicit WrapperPendingSharedURLLoaderFactory(mojo::PendingRemote<network::mojom::URLLoaderFactory> factory_remote);
...
template <template <typename> class RemoteTemplateType> class WrapperSharedURLLoaderFactoryBase : public network::SharedURLLoaderFactory {
public:
        ...
        const net::MutableNetworkTrafficAnnotationTag& traffic_annotation) override
    {
        if (!factory_remote_)
            return;
        factory_remote_->CreateLoaderAndStart(std::move(loader), request_id, options, request, std::move(client), traffic_annotation);
    }
    ...
    std::unique_ptr<network::PendingSharedURLLoaderFactory> Clone() override
    {
        ...
        if (factory_remote_) {
            factory_remote_->Clone(pending_factory_remote.InitWithNewPipeAndPassReceiver());
        }
        return std::make_unique<WrapperPendingSharedURLLoaderFactory>(std::move(pending_factory_remote));
    }
...
using WrapperSharedURLLoaderFactory = WrapperSharedURLLoaderFactoryBase<mojo::Remote>;
