...
// https://fetch.spec.whatwg.org/#concept-tao-check
bool TimingAllowOriginCheck(const mojom::TimingAllowOriginPtr& tao, const url::Origin& origin)
{
    return tao && (tao->which() == mojom::TimingAllowOrigin::Tag::kAll || ::base::Contains(tao->get_serialized_origins(), origin.Serialize()));
}
