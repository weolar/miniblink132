...
{
    static auto* crash_key = base::debug::AllocateCrashKeyString("request_initiator_origin_lock", base::debug::CrashKeySize::Size64);
    return crash_key;
}
...
InitiatorLockCompatibility VerifyRequestInitiatorLock(
    const std::optional<url::Origin>& request_initiator_origin_lock, const std::optional<url::Origin>& request_initiator)
{
...
ScopedRequestInitiatorOriginLockCrashKey::ScopedRequestInitiatorOriginLockCrashKey(const std::optional<url::Origin>& request_initiator_origin_lock)
    : ScopedOriginCrashKey(GetRequestInitiatorOriginLockCrashKey(), base::OptionalToPtr(request_initiator_origin_lock))
{
}

ScopedRequestInitiatorOriginLockCrashKey::~ScopedRequestInitiatorOriginLockCrashKey() = default;
