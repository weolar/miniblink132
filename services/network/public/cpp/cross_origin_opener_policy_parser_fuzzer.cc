...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    std::string header_string = base::StrCat({ "HTTP/1.1 200 OK\nCross-Origin-Opener-Policy: ", test_data, "\n\n" });

    std::replace(header_string.begin(), header_string.end(), '\n', '\0');
    scoped_refptr<net::HttpResponseHeaders> headers = base::MakeRefCounted<net::HttpResponseHeaders>(header_string);
