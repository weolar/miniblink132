...
{
    handle_ = mojo::PlatformHandle(base::OpenWritableDirectoryHandle(path_).TakeChannel());
}
...
[[nodiscard]] base::OnceClosure TransferableDirectory::Mount()
{
    ...
    path_ = base::FilePath(base::StringPrintf(kMountFormatString, base::UnguessableToken::Create().ToString().c_str()));
    CHECK(!base::PathExists(path_));

    zx_status_t status = fdio_ns_bind(GetNamespace(), path_.value().data(), handle_.ReleaseHandle());
    ...
    return base::BindOnce(
        [](base::FilePath path) {
            zx_status_t status = fdio_ns_unbind(GetNamespace(), path.value().data());
