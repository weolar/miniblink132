...
public:
    ...
    // during the callback.
    using BodyAsStringCallbackDeprecated = base::OnceCallback<void(std::unique_ptr<std::string> response_body)>;
    using BodyAsStringCallback = base::OnceCallback<void(std::optional<std::string> response_body)>;
    ...
    using HeadersOnlyCallback = base::OnceCallback<void(scoped_refptr<net::HttpResponseHeaders> headers)>;
    ...
    // callback.
    using DownloadToFileCompleteCallback = base::OnceCallback<void(base::FilePath path)>;
    ...
    using OnRedirectCallback = base::RepeatingCallback<void(const GURL& url_before_redirect, const net::RedirectInfo& redirect_info,
        const mojom::URLResponseHead& response_head, std::vector<std::string>* removed_headers)>;

    // Callback used when a response is received. It is safe to delete the
    // SimpleURLLoader during the callback.
    using OnResponseStartedCallback = base::OnceCallback<void(const GURL& final_url, const mojom::URLResponseHead& response_head)>;

    // Callback used when an upload progress is reported. It is safe to
    // delete the SimpleURLLoader during the callback.
    using UploadProgressCallback = base::RepeatingCallback<void(uint64_t position, uint64_t total)>;
    ...
    using DownloadProgressCallback = base::RepeatingCallback<void(uint64_t current)>;
    ...
    // be reused.
    static std::unique_ptr<SimpleURLLoader> Create(std::unique_ptr<ResourceRequest> resource_request, const net::NetworkTrafficAnnotationTag& annotation_tag,
    ...
    static void SetTimeoutTickClockForTest(const base::TickClock* timeout_tick_clock);
    ...
    // called.
    virtual void DownloadToString(mojom::URLLoaderFactory* url_loader_factory, BodyAsStringCallbackDeprecated body_as_string_callback, size_t max_body_size)
        = 0;
    virtual void DownloadToString(mojom::URLLoaderFactory* url_loader_factory, BodyAsStringCallback body_as_string_callback, size_t max_body_size) = 0;
    ...
    virtual void DownloadToStringOfUnboundedSizeUntilCrashAndDie(
        ...
        = 0;
    virtual void DownloadToStringOfUnboundedSizeUntilCrashAndDie(mojom::URLLoaderFactory* url_loader_factory, BodyAsStringCallback body_as_string_callback) = 0;
    ...
    // this callback.
    virtual void DownloadHeadersOnly(mojom::URLLoaderFactory* url_loader_factory, HeadersOnlyCallback headers_only_callback) = 0;
    ...
    virtual void DownloadToFile(mojom::URLLoaderFactory* url_loader_factory, DownloadToFileCompleteCallback download_to_file_complete_callback,
        const base::FilePath& file_path, int64_t max_body_size = std::numeric_limits<int64_t>::max())
        = 0;

    // Same as DownloadToFile, but creates a temporary file instead of taking a
    // FilePath.
    virtual void DownloadToTempFile(mojom::URLLoaderFactory* url_loader_factory, DownloadToFileCompleteCallback download_to_file_complete_callback,
    ...
    // SimpleURLLoader.
    virtual void DownloadAsStream(mojom::URLLoaderFactory* url_loader_factory, SimpleURLLoaderStreamConsumer* stream_consumer) = 0;

    // Sets callback to be invoked during redirects. Callback may delete the
    // SimpleURLLoader.
    virtual void SetOnRedirectCallback(const OnRedirectCallback& on_redirect_callback) = 0;

    // Sets callback to be invoked when the response has started.
    // Callback may delete the SimpleURLLoader.
    virtual void SetOnResponseStartedCallback(OnResponseStartedCallback on_response_started_callback) = 0;
    ...
    virtual void SetOnUploadProgressCallback(UploadProgressCallback on_upload_progress_callback) = 0;
    ...
    //
    // Callback may delete the SimpleURLLoader.
    virtual void SetOnDownloadProgressCallback(DownloadProgressCallback on_download_progress_callback) = 0;
    ...
    virtual void AttachStringForUpload(std::string_view upload_data, std::string_view upload_content_type) = 0;
    virtual void AttachStringForUpload(std::string_view upload_data) = 0;
    virtual void AttachStringForUpload(const char* upload_data, std::string_view upload_content_type) = 0;
    virtual void AttachStringForUpload(const char* upload_data) = 0;
    virtual void AttachStringForUpload(std::string&& upload_data, std::string_view upload_content_type) = 0;
    ...
    // |content_type| will overwrite any Content-Type header in the
    // ResourceRequest passed to Create().
    virtual void AttachFileForUpload(const base::FilePath& upload_file_path, const std::string& upload_content_type, uint64_t offset = 0,
        uint64_t length = std::numeric_limits<uint64_t>::max())
        = 0;
    virtual void AttachFileForUpload(const base::FilePath& upload_file_path, uint64_t offset, uint64_t length) = 0;
    void AttachFileForUpload(const base::FilePath& upload_file_path)
    {
        AttachFileForUpload(upload_file_path, 0, std::numeric_limits<uint64_t>::max());
    }
    ...
    virtual const std::optional<URLLoaderCompletionStatus>& CompletionStatus() const = 0;
