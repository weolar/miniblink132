...
TEST_F(StructTraitsTest, RendererSettings)
{
    ...
    mojom::RendererSettings::Deserialize(mojom::RendererSettings::Serialize(&input), &output);
    EXPECT_EQ(input.allow_antialiasing, output.allow_antialiasing);
    EXPECT_EQ(input.force_antialiasing, output.force_antialiasing);
    EXPECT_EQ(input.force_blending_with_shaders, output.force_blending_with_shaders);
    EXPECT_EQ(input.partial_swap_enabled, output.partial_swap_enabled);
    EXPECT_EQ(input.should_clear_root_render_pass, output.should_clear_root_render_pass);
    EXPECT_EQ(input.release_overlay_resources_after_gpu_query, output.release_overlay_resources_after_gpu_query);
    EXPECT_EQ(input.highp_threshold_min, output.highp_threshold_min);
    EXPECT_EQ(input.occlusion_culler_settings.quad_split_limit, output.occlusion_culler_settings.quad_split_limit);
    EXPECT_EQ(input.occlusion_culler_settings.maximum_occluder_complexity, output.occlusion_culler_settings.maximum_occluder_complexity);
    EXPECT_EQ(input.occlusion_culler_settings.minimum_fragments_reduced, output.occlusion_culler_settings.minimum_fragments_reduced);
}
...
TEST_F(StructTraitsTest, DebugRendererSettings)
{
    ...
    mojom::DebugRendererSettings::Deserialize(mojom::DebugRendererSettings::Serialize(&input), &output);
    ...
    EXPECT_EQ(input.tint_composited_content_modulate, output.tint_composited_content_modulate);
    EXPECT_EQ(input.show_dc_layer_debug_borders, output.show_dc_layer_debug_borders);
}
