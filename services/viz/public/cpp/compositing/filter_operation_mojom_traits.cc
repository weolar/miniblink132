...
viz::mojom::FilterType CCFilterTypeToMojo(const cc::FilterOperation::FilterType& type)
{
    switch (type) {
...
cc::FilterOperation::FilterType MojoFilterTypeToCC(const viz::mojom::FilterType& type)
{
    switch (type) {
...
// static
viz::mojom::FilterType StructTraits<viz::mojom::FilterOperationDataView, cc::FilterOperation>::type(const cc::FilterOperation& op)
{
    return CCFilterTypeToMojo(op.type());
}

// static
bool StructTraits<viz::mojom::FilterOperationDataView, cc::FilterOperation>::Read(viz::mojom::FilterOperationDataView data, cc::FilterOperation* out)
{
    ...
    case cc::FilterOperation::DROP_SHADOW: {
        ...
        if (!data.ReadOffset(&offset) || !data.ReadDropShadowColor(&drop_shadow_color))
            return false;
