...
template <> struct StructTraits<viz::mojom::ReturnedResourceDataView, viz::ReturnedResource> {
    static const viz::ResourceId& id(const viz::ReturnedResource& resource)
    {
        return resource.id;
    }

    static const gpu::SyncToken& sync_token(const viz::ReturnedResource& resource)
    {
        return resource.sync_token;
    }

    static gfx::GpuFenceHandle release_fence(const viz::ReturnedResource& resource)
    {
    ...
    static bool Read(viz::mojom::ReturnedResourceDataView data, viz::ReturnedResource* out)
    {
        if (!data.ReadSyncToken(&out->sync_token) || !data.ReadReleaseFence(&out->release_fence) || !data.ReadId(&out->id)) {
            return false;
        }
