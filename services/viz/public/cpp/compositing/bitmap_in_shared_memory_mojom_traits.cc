...
// static
const SkImageInfo StructTraits<viz::mojom::BitmapInSharedMemoryDataView, viz::CopyOutputResult::ScopedSkBitmap>::image_info(
...
// static
uint64_t StructTraits<viz::mojom::BitmapInSharedMemoryDataView, viz::CopyOutputResult::ScopedSkBitmap>::row_bytes(
    const viz::CopyOutputResult::ScopedSkBitmap& scoped_bitmap)
{
    auto sk_bitmap = scoped_bitmap.bitmap();
...
// static
std::optional<base::WritableSharedMemoryRegion> StructTraits<viz::mojom::BitmapInSharedMemoryDataView, viz::CopyOutputResult::ScopedSkBitmap>::pixels(
    const viz::CopyOutputResult::ScopedSkBitmap& scoped_bitmap)
{
    auto sk_bitmap = scoped_bitmap.bitmap();
    ...
    base::WritableSharedMemoryRegion region = base::WritableSharedMemoryRegion::Create(byte_size);
    {
...
// static
bool StructTraits<viz::mojom::BitmapInSharedMemoryDataView, SkBitmap>::Read(viz::mojom::BitmapInSharedMemoryDataView data, SkBitmap* sk_bitmap)
{
    ...
    *sk_bitmap = SkBitmap();
    ...
    auto mapping_ptr = std::make_unique<base::WritableSharedMemoryMapping>(region_opt->Map());
    ...
    if (!sk_bitmap->installPixels(image_info, mapping_ptr->memory(), data.row_bytes(), &DeleteSharedMemoryMapping, mapping_ptr.get())) {
        return false;
    }
    mapping_ptr.release();
    return true;
