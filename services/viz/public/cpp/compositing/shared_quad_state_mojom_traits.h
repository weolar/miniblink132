...
template <> struct StructTraits<viz::mojom::SharedQuadStateDataView, OptSharedQuadState> {
    static bool IsNull(const OptSharedQuadState& input)
    {
        return !input.sqs;
    }

    static void SetToNull(OptSharedQuadState* output)
    {
        output->sqs = nullptr;
    }

    static const gfx::Transform& quad_to_target_transform(const OptSharedQuadState& input)
    {
    ...
    static const gfx::Rect& visible_quad_layer_rect(const OptSharedQuadState& input)
    {
        return input.sqs->visible_quad_layer_rect;
    }

    static const std::optional<gfx::MaskFilterInfo> mask_filter_info(const OptSharedQuadState& input)
    {
        return input.sqs->mask_filter_info.IsEmpty() ? std::nullopt : std::optional<gfx::MaskFilterInfo>(input.sqs->mask_filter_info);
    }

    static const std::optional<gfx::Rect>& clip_rect(const OptSharedQuadState& input)
    {
...
template <> struct StructTraits<viz::mojom::SharedQuadStateDataView, viz::SharedQuadState> {
    static const gfx::Transform& quad_to_target_transform(const viz::SharedQuadState& sqs)
    {
    ...
    static const gfx::Rect& visible_quad_layer_rect(const viz::SharedQuadState& sqs)
    {
        return sqs.visible_quad_layer_rect;
    }

    static const gfx::MaskFilterInfo& mask_filter_info(const viz::SharedQuadState& sqs)
    {
        return sqs.mask_filter_info;
    }

    static const std::optional<gfx::Rect>& clip_rect(const viz::SharedQuadState& sqs)
    {
    ...
    static float opacity(const viz::SharedQuadState& sqs)
    {
        return sqs.opacity;
    }
    ...
    static bool Read(viz::mojom::SharedQuadStateDataView data, viz::SharedQuadState* out)
    {
        if (!data.ReadQuadToTargetTransform(&out->quad_to_target_transform) || !data.ReadQuadLayerRect(&out->quad_layer_rect)
            || !data.ReadVisibleQuadLayerRect(&out->visible_quad_layer_rect) || !data.ReadClipRect(&out->clip_rect) || !data.ReadOffsetTag(&out->offset_tag)) {
            return false;
        }
