...
template <> struct StructTraits<viz::mojom::SharedElementResourceIdDataView, viz::SharedElementResourceId> {
    static uint32_t id(const viz::SharedElementResourceId& resource_id)
    {
        return resource_id.id();
    }

    static bool Read(viz::mojom::SharedElementResourceIdDataView data, viz::SharedElementResourceId* out)
    {
