...
// static
viz::mojom::SynchronizationType EnumTraits<viz::mojom::SynchronizationType, viz::TransferableResource::SynchronizationType>::ToMojom(
...
// static
bool EnumTraits<viz::mojom::SynchronizationType, viz::TransferableResource::SynchronizationType>::FromMojom(
    viz::mojom::SynchronizationType input, viz::TransferableResource::SynchronizationType* out)
{
    switch (input) {
    ...
    case viz::mojom::SynchronizationType::kGpuCommandsCompleted:
        *out = viz::TransferableResource::SynchronizationType::kGpuCommandsCompleted;
        return true;
...
// static
bool StructTraits<viz::mojom::TransferableResourceDataView, viz::TransferableResource>::Read(
    viz::mojom::TransferableResourceDataView data, viz::TransferableResource* out)
{
    viz::ResourceId id;

    gpu::SyncToken sync_token;
    viz::MemoryBufferId memory_buffer_id;
    if (!data.ReadSize(&out->size) || !data.ReadFormat(&out->format) || !data.ReadMemoryBufferId(&memory_buffer_id) || !data.ReadSyncToken(&sync_token)
        || !data.ReadColorSpace(&out->color_space) || !data.ReadHdrMetadata(&out->hdr_metadata) || !data.ReadYcbcrInfo(&out->ycbcr_info) || !data.ReadId(&id)
...
// static
viz::mojom::MemoryBufferIdDataView::Tag UnionTraits<viz::mojom::MemoryBufferIdDataView, viz::MemoryBufferId>::GetTag(
    const viz::MemoryBufferId& memory_buffer_id)
{
    return absl::visit(base::Overloaded {
                           [](gpu::Mailbox) { return viz::mojom::MemoryBufferIdDataView::Tag::kMailbox; },

                           [](viz::SharedBitmapId) { return viz::mojom::MemoryBufferIdDataView::Tag::kSharedBitmapId; },
                       },
        memory_buffer_id);
}

// static
bool UnionTraits<viz::mojom::MemoryBufferIdDataView, viz::MemoryBufferId>::Read(viz::mojom::MemoryBufferIdDataView memory_buffer_id, viz::MemoryBufferId* out)
{
