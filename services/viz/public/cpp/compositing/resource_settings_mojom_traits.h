...
template <> struct StructTraits<viz::mojom::ResourceSettingsDataView, viz::ResourceSettings> {
    static bool use_gpu_memory_buffer_resources(const viz::ResourceSettings& input)
    {
        return input.use_gpu_memory_buffer_resources;
    }

    static bool Read(viz::mojom::ResourceSettingsDataView data, viz::ResourceSettings* out);
};
