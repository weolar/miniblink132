...
template <> struct StructTraits<viz::mojom::FilterOperationDataView, cc::FilterOperation> {
    static viz::mojom::FilterType type(const cc::FilterOperation& op);

    static float amount(const cc::FilterOperation& operation)
    {
        if (operation.type() == cc::FilterOperation::ALPHA_THRESHOLD || operation.type() == cc::FilterOperation::COLOR_MATRIX
    ...
    {
        if (operation.type() != cc::FilterOperation::DROP_SHADOW && operation.type() != cc::FilterOperation::OFFSET)
            return gfx::Point();
    ...
    static sk_sp<cc::PaintFilter> image_filter(const cc::FilterOperation& operation)
    {
    ...
    static std::optional<base::span<const float>> matrix(const cc::FilterOperation& operation)
    {
    ...
    static base::span<const gfx::Rect> shape(const cc::FilterOperation& operation)
    {
        if (operation.type() != cc::FilterOperation::ALPHA_THRESHOLD)
            return base::span<gfx::Rect>();
        const cc::FilterOperation::ShapeRects& sh = operation.shape();
        return base::span<const gfx::Rect>(sh.data(), sh.size());
    }
    ...
    static bool Read(viz::mojom::FilterOperationDataView data, cc::FilterOperation* out);
};
