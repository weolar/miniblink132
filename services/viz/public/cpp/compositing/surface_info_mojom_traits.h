...
template <> struct StructTraits<viz::mojom::SurfaceInfoDataView, viz::SurfaceInfo> {
    static const viz::SurfaceId& surface_id(const viz::SurfaceInfo& surface_info)
    {
    ...
    static bool Read(viz::mojom::SurfaceInfoDataView data, viz::SurfaceInfo* out)
    {
        out->device_scale_factor_ = data.device_scale_factor();
        return data.ReadSurfaceId(&out->id_) && data.ReadSizeInPixels(&out->size_in_pixels_) && out->is_valid();
    }
};
