...
void ExpectEqual(const cc::FilterOperation& input, const cc::FilterOperation& output)
{
    ...
    case cc::FilterOperation::REFERENCE: {
        ...
        if (input.image_filter()) {
            EXPECT_TRUE(input.image_filter()->EqualsForTesting(*output.image_filter()));
        }
        break;
    }
...
{
    cc::FilterOperation input = cc::FilterOperation::CreateDropShadowFilter(gfx::Point(4, 4), 4.0f, SkColor4f { 0.15f, 0.0f, 0.0f, 1.0f });
...
{
    cc::FilterOperation input = cc::FilterOperation::CreateReferenceFilter(sk_make_sp<cc::DropShadowPaintFilter>(SkIntToScalar(3), SkIntToScalar(8),
        SkIntToScalar(4), SkIntToScalar(9), SkColors::kBlack, cc::DropShadowPaintFilter::ShadowMode::kDrawShadowAndForeground, nullptr));
...
{
    LocalSurfaceId input(42, base::UnguessableToken::CreateForTesting(0x12345678, 0x9abcdef0));
...
TEST_F(StructTraitsTest, CopyOutputRequest_BitmapRequest)
{
    ...
    const auto result_destination = CopyOutputRequest::ResultDestination::kSystemMemory;
    const gfx::Rect area(5, 7, 44, 55);
    const auto source = base::UnguessableToken::CreateForTesting(0xdeadbeef, 0xdeadf00d);
    ...
    std::unique_ptr<CopyOutputRequest> input(new CopyOutputRequest(result_format, result_destination,
        base::BindOnce(
            [](base::OnceClosure quit_closure, const gfx::Rect& expected_rect, std::unique_ptr<CopyOutputResult> result) {
    ...
    SkBitmap bitmap;
    bitmap.allocPixels(SkImageInfo::MakeN32Premul(result_rect.width(), result_rect.height(), SkColorSpace::MakeSRGB()));
    output->SendResult(std::make_unique<CopyOutputSkBitmapResult>(result_rect, std::move(bitmap)));
...
TEST_F(StructTraitsTest, CopyOutputRequest_MessagePipeBroken)
{
    ...
    auto request = std::make_unique<CopyOutputRequest>(CopyOutputRequest::ResultFormat::RGBA, CopyOutputRequest::ResultDestination::kSystemMemory,
        base::BindOnce(
            [](base::OnceClosure quit_closure, std::unique_ptr<CopyOutputResult> result) {
                EXPECT_TRUE(result->IsEmpty());
                std::move(quit_closure).Run();
            },
            run_loop.QuitClosure()));
    auto result_sender = mojo::StructTraits<mojom::CopyOutputRequestDataView, std::unique_ptr<CopyOutputRequest>>::result_sender(request);
    result_sender.reset();
...
TEST_F(StructTraitsTest, CopyOutputRequest_TextureRequest)
{
    ...
    const auto result_format = CopyOutputRequest::ResultFormat::RGBA;
    const auto result_destination = CopyOutputRequest::ResultDestination::kNativeTextures;

    const int8_t mailbox_name[GL_MAILBOX_SIZE_CHROMIUM] = { 0, 9, 8, 7, 6, 5, 4, 3, 2, 1, 9, 7, 5, 3, 1, 3 };
    gpu::Mailbox mailbox;
    ...
    release_callbacks.push_back(base::BindOnce(
        [](base::OnceClosure quit_closure, const gpu::SyncToken& expected_sync_token, const gpu::SyncToken& sync_token, bool is_lost) {
    ...
    output->SendResult(std::make_unique<CopyOutputTextureResult>(
        result_format, result_rect, CopyOutputResult::TextureResult(mailbox, gfx::ColorSpace::CreateSRGB()), std::move(release_callbacks)));
...
TEST_F(StructTraitsTest, CopyOutputRequest_CallbackRunsOnce)
{
    ...
    int n_called = 0;
    auto request = std::make_unique<CopyOutputRequest>(CopyOutputRequest::ResultFormat::RGBA, CopyOutputRequest::ResultDestination::kSystemMemory,
        base::BindOnce([](int* n_called, std::unique_ptr<CopyOutputResult> result) { ++*n_called; }, base::Unretained(&n_called)));
    auto result_sender_pending_remote = mojo::StructTraits<mojom::CopyOutputRequestDataView, std::unique_ptr<CopyOutputRequest>>::result_sender(request);

    mojo::Remote<mojom::CopyOutputResultSender> result_sender_remote(std::move(result_sender_pending_remote));
    for (int i = 0; i < 10; i++)
        result_sender_remote->SendResult(std::make_unique<CopyOutputResult>(request->result_format(), request->result_destination(), gfx::Rect(), false));
...
{
    const auto quad_to_target_transform = gfx::Transform::RowMajor(1.f, 2.f, 3.f, 4.f, 5.f, 6.f, 7.f, 8.f, 9.f, 10.f, 11.f, 12.f, 13.f, 14.f, 15.f, 16.f);
    const gfx::Rect layer_rect(1234, 5678);
    const gfx::Rect visible_layer_rect(12, 34, 56, 78);
    const gfx::MaskFilterInfo mask_filter_info(gfx::RRectF(gfx::RectF(1.f, 2.f, 30.f, 40.f), 5));
    ...
    input_sqs.SetAll(quad_to_target_transform, layer_rect, visible_layer_rect, mask_filter_info, clip_rect, are_contents_opaque, opacity, blend_mode,
        sorting_context_id, /*layer_id=*/0u, is_fast_rounded_corner);
    SharedQuadState output_sqs;
    mojo::test::SerializeAndDeserialize<mojom::SharedQuadState>(input_sqs, output_sqs);
    ...
    EXPECT_EQ(mask_filter_info.rounded_corner_bounds(), output_sqs.mask_filter_info.rounded_corner_bounds());
...
TEST_F(StructTraitsTest, CompositorFrame)
{
    ...
    render_pass->SetNew(CompositorRenderPassId { 1 }, gfx::Rect(5, 6), gfx::Rect(2, 3), gfx::Transform());

    // SharedQuadState.
    const auto sqs_quad_to_target_transform = gfx::Transform::RowMajor(1.f, 2.f, 3.f, 4.f, 5.f, 6.f, 7.f, 8.f, 9.f, 10.f, 11.f, 12.f, 13.f, 14.f, 15.f, 16.f);
    const gfx::Rect sqs_layer_rect(1234, 5678);
    const gfx::Rect sqs_visible_layer_rect(12, 34, 56, 78);
    const gfx::MaskFilterInfo sqs_mask_filter_info(gfx::RRectF(gfx::RectF(3.f, 4.f, 50.f, 15.f), 3));
    ...
    sqs->SetAll(sqs_quad_to_target_transform, sqs_layer_rect, sqs_visible_layer_rect, sqs_mask_filter_info, sqs_clip_rect, sqs_are_contents_opaque, sqs_opacity,
        sqs_blend_mode, sqs_sorting_context_id, /*layer_id=*/0u,
    ...
    const CompositorRenderPass* out_render_pass = output.render_pass_list[0].get();
    ...
    const DebugBorderDrawQuad* out_debug_border_draw_quad = DebugBorderDrawQuad::MaterialCast(out_render_pass->quad_list.ElementAt(0));
    ...
    EXPECT_EQ(force_anti_aliasing_off, out_solid_color_draw_quad->force_anti_aliasing_off);
}

TEST_F(StructTraitsTest, CompositorFrameTransitionDirective)
{
    auto frame = CompositorFrameBuilder().AddDefaultRenderPass().AddDefaultRenderPass().AddDefaultRenderPass().Build();
    ...
    element.view_transition_element_resource_id = ViewTransitionElementResourceId(transition_token, 1);
    uint32_t sequence_id = 1u;
    frame.metadata.transition_directives.push_back(
        CompositorFrameTransitionDirective::CreateSave(transition_token, /*maybe_cross_frame_sink=*/true, sequence_id, { element }, {}));

    // This ensures de-serialization succeeds if all passes are present.
    CompositorFrame output;
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::CompositorFrame>(frame, output));
    ...
    element.render_pass_id = CompositorRenderPassId(frame.render_pass_list.back()->id.GetUnsafeValue() + 1);
    frame.metadata.transition_directives.push_back(
        CompositorFrameTransitionDirective::CreateSave(transition_token, /*maybe_cross_frame_sink=*/true, sequence_id, { element }, {}));

    // This ensures de-serialization fails if a pass is missing.
    ASSERT_FALSE(mojo::test::SerializeAndDeserialize<mojom::CompositorFrame>(frame, output));
}
...
TEST_F(StructTraitsTest, ViewTransitionElementResourceId)
{
    ...
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::ViewTransitionElementResourceId>(empty_id, empty_output_id));
    ...
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::ViewTransitionElementResourceId>(valid_id, valid_output_id));
...
{
    const SurfaceId surface_id(FrameSinkId(1234, 4321), LocalSurfaceId(5678, base::UnguessableToken::CreateForTesting(143254, 144132)));
...
TEST_F(StructTraitsTest, ReturnedResource)
{
    ...
    const gpu::CommandBufferId command_buffer_id(gpu::CommandBufferId::FromUnsafeValue(0xdeadbeef));
    const uint64_t release_count = 0xdeadbeefdead;
    gpu::SyncToken sync_token(command_buffer_namespace, command_buffer_id, release_count);
...
TEST_F(StructTraitsTest, CompositorFrameMetadata)
{
    ...
    SurfaceId id(FrameSinkId(1234, 4321), LocalSurfaceId(5678, base::UnguessableToken::Create()));
    referenced_surfaces.emplace_back(id);
    std::vector<SurfaceId> activation_dependencies;
    SurfaceId id2(FrameSinkId(4321, 1234), LocalSurfaceId(8765, base::UnguessableToken::Create()));
    ...
    input.begin_frame_ack.frame_id.sequence_number = begin_frame_ack_sequence_number;
    ...
    mojo::test::SerializeAndDeserialize<mojom::CompositorFrameMetadata>(input, output);
    ...
    EXPECT_TRUE(output.latency_info[0].FindLatency(ui::INPUT_EVENT_LATENCY_BEGIN_RWH_COMPONENT, nullptr));
    ...
    EXPECT_EQ(activation_dependencies.size(), output.activation_dependencies.size());
    ...
    EXPECT_EQ(begin_frame_ack_sequence_number, output.begin_frame_ack.frame_id.sequence_number);
...
TEST_F(StructTraitsTest, CompositorFrameMetadataBadOffsetTagDefinition)
{
...

    {
        ...
        CompositorFrameMetadata output;
        bool result = mojo::test::SerializeAndDeserialize<mojom::CompositorFrameMetadata>(input, output);
        EXPECT_TRUE(result);
    }

    SurfaceId surface_id(FrameSinkId(1337, 1234), LocalSurfaceId(0xfbadbeef, base::UnguessableToken::Create()));
...
TEST_F(StructTraitsTest, RenderPass)
{
    ...
    constexpr gfx::Transform kTransformToRoot = gfx::Transform::Affine(1.0, 0.5, 0.5, -0.5, -1.0, 0.0);
    constexpr gfx::Rect kDamageRect(56, 123, 19, 43);
    const std::optional<gfx::RRectF> kBackdropFilterBounds({ 10, 20, 130, 140, 1, 2, 3, 4, 5, 6, 7, 8 });
    ...
    input->SetAll(kRenderPassId, kOutputRect, kDamageRect, kTransformToRoot, filters, backdrop_filters, kBackdropFilterBounds, kSubtreeCaptureId,
        kOutputRect.size(), ViewTransitionElementResourceId(), kHasTransparentBackground, kCacheRenderPass, kHasDamageFromContributingContent, kGenerateMipmap,
        kHasPerQuadDamage);
    ...
    shared_state_1->SetAll(gfx::Transform::RowMajor(16.1f, 15.3f, 14.3f, 13.7f, 12.2f, 11.4f, 10.4f, 9.8f, 8.1f, 7.3f, 6.3f, 5.7f, 4.8f, 3.4f, 2.4f, 1.2f),
        gfx::Rect(1, 2), gfx::Rect(1337, 5679, 9101112, 131415), gfx::MaskFilterInfo(gfx::RRectF(gfx::RectF(5.f, 6.f, 70.f, 89.f), 10.f)),
    ...
    shared_state_2->SetAll(gfx::Transform::RowMajor(1.1f, 2.3f, 3.3f, 4.7f, 5.2f, 6.4f, 7.4f, 8.8f, 9.1f, 10.3f, 11.3f, 12.7f, 13.8f, 14.4f, 15.4f, 16.2f),
        gfx::Rect(1337, 1234), gfx::Rect(1234, 5678, 9101112, 13141516), gfx::MaskFilterInfo(gfx::RRectF(gfx::RectF(23.f, 45.f, 60.f, 70.f), 8.f)),
        gfx::Rect(1357, 2468, 121314, 1337), /*contents_opaque=*/true,
        /*opacity_f=*/2, SkBlendMode::kSrcOver, /*sorting_context=*/1,
        /*layer_id=*/0u, /*fast_rounded_corner=*/false);
    ...
    debug_quad->SetNew(shared_state_1, debug_quad_rect, debug_quad_rect, SkColors::kBlue, 1337);
    ...
    color_quad->SetNew(shared_state_2, color_quad_rect, color_quad_rect, SkColors::kRed, true);
    ...
    surface_quad->SetNew(shared_state_2, surface_quad_rect, surface_quad_rect,
        SurfaceRange(std::nullopt, SurfaceId(FrameSinkId(1337, 1234), LocalSurfaceId(1234, base::UnguessableToken::Create()))), SkColors::kYellow, false);
    ...
    mojo::test::SerializeAndDeserialize<mojom::CompositorRenderPass>(input, output);

    EXPECT_EQ(input->quad_list.size(), output->quad_list.size());
    EXPECT_EQ(input->shared_quad_state_list.size(), output->shared_quad_state_list.size());
    ...
    EXPECT_EQ(kHasDamageFromContributingContent, output->has_damage_from_contributing_content);
    ...
    EXPECT_EQ(shared_state_1->quad_to_target_transform, out_sqs1->quad_to_target_transform);
    EXPECT_EQ(shared_state_1->quad_layer_rect, out_sqs1->quad_layer_rect);
    EXPECT_EQ(shared_state_1->visible_quad_layer_rect, out_sqs1->visible_quad_layer_rect);
    ...
    EXPECT_EQ(shared_state_2->quad_to_target_transform, out_sqs2->quad_to_target_transform);
    EXPECT_EQ(shared_state_2->quad_layer_rect, out_sqs2->quad_layer_rect);
    EXPECT_EQ(shared_state_2->visible_quad_layer_rect, out_sqs2->visible_quad_layer_rect);
    ...
    EXPECT_EQ(color_quad->force_anti_aliasing_off, out_color_quad->force_anti_aliasing_off);
    ...
    EXPECT_EQ(surface_quad->default_background_color, out_surface_quad->default_background_color);
    EXPECT_EQ(surface_quad->stretch_content_to_fill_bounds, out_surface_quad->stretch_content_to_fill_bounds);
...
TEST_F(StructTraitsTest, RenderPassWithEmptySharedQuadStateList)
{
    ...
    auto input = CompositorRenderPass::Create();
    input->SetAll(kRenderPassId, kOutputRect, kDamageRect, kTransformToRoot, cc::FilterOperations(), cc::FilterOperations(), kBackdropFilterBounds,
        kEmptySubtreeCaptureId, kOutputRect.size(), ViewTransitionElementResourceId(), kHasTransparentBackground, kCacheRenderPass,
        kHasDamageFromContributingContent, kGenerateMipmap, kHasPerQuadDamage);
...
{
    auto render_pass = CompositorRenderPass::Create();
    render_pass->SetNew(CompositorRenderPassId { 1 }, gfx::Rect(), gfx::Rect(), gfx::Transform());
    ...
    const SurfaceId primary_surface_id(FrameSinkId(1234, 4321), LocalSurfaceId(5678, base::UnguessableToken::Create()));
    const SurfaceId fallback_surface_id(FrameSinkId(2468, 1357), LocalSurfaceId(1234, base::UnguessableToken::Create()));
    SurfaceDrawQuad* primary_surface_quad = render_pass->CreateAndAppendDrawQuad<SurfaceDrawQuad>();
    primary_surface_quad->SetNew(sqs, rect3, rect3, SurfaceRange(fallback_surface_id, primary_surface_id), SkColors::kBlue, false);
    ...
    CompositorRenderPassDrawQuad* render_pass_quad = render_pass->CreateAndAppendDrawQuad<CompositorRenderPassDrawQuad>();
    render_pass_quad->SetAll(sqs, rect4, rect4, needs_blending, render_pass_id, resource_id4, mask_uv_rect, mask_texture_size, filters_scale, filters_origin,
        tex_coord_rect, force_anti_aliasing_off, backdrop_filter_quality, intersects_damage_under);
    ...
    const gfx::ProtectedVideoType protected_video_type = gfx::ProtectedVideoType::kClear;
    ...
    texture_draw_quad->SetAll(sqs, rect5, rect5, needs_blending, resource_id5, resource_size_in_pixels5, premultiplied_alpha, uv_top_left, uv_bottom_right,
        background_color, y_flipped, nearest_neighbor, secure_output_only, protected_video_type);
    ...
    TextureDrawQuad* stream_video_draw_quad = render_pass->CreateAndAppendDrawQuad<TextureDrawQuad>();
    stream_video_draw_quad->SetAll(sqs, rect6, rect6, needs_blending6, resource_id6, resource_size_in_pixels6, false, uv_top_left, uv_bottom_right,
    ...
    TextureDrawQuad* rounded_display_mask_quad = render_pass->CreateAndAppendDrawQuad<TextureDrawQuad>();
    rounded_display_mask_quad->SetAll(sqs, rect7, rect7, needs_blending7, resource_id7, resource_size_in_pixels7, false, uv_top_left, uv_bottom_right,
        SkColors::kTransparent, false, false, false, protected_video_type);
    rounded_display_mask_quad->rounded_display_masks_info = TextureDrawQuad::RoundedDisplayMasksInfo::CreateRoundedDisplayMasksInfo(
        origin_rounded_display_mask_radius, other_rounded_display_mask_radius, is_horizontally_positioned);

    std::unique_ptr<CompositorRenderPass> output;
    mojo::test::SerializeAndDeserialize<mojom::CompositorRenderPass>(render_pass, output);
    ...
    const SurfaceDrawQuad* out_primary_surface_draw_quad = SurfaceDrawQuad::MaterialCast(output->quad_list.ElementAt(2));
    ...
    EXPECT_EQ(primary_surface_id, out_primary_surface_draw_quad->surface_range.end());
    EXPECT_EQ(SkColors::kBlue, out_primary_surface_draw_quad->default_background_color);
    EXPECT_EQ(fallback_surface_id, out_primary_surface_draw_quad->surface_range.start());

    const CompositorRenderPassDrawQuad* out_render_pass_draw_quad = CompositorRenderPassDrawQuad::MaterialCast(output->quad_list.ElementAt(3));
    ...
    EXPECT_EQ(backdrop_filter_quality, out_render_pass_draw_quad->backdrop_filter_quality);
    EXPECT_EQ(intersects_damage_under, out_render_pass_draw_quad->intersects_damage_under);
    ...
    EXPECT_EQ(resource_size_in_pixels5, out_texture_draw_quad->resource_size_in_pixels());
    ...
    const TextureDrawQuad* out_stream_video_draw_quad = TextureDrawQuad::MaterialCast(output->quad_list.ElementAt(5));
    ...
    EXPECT_EQ(resource_size_in_pixels6, out_stream_video_draw_quad->resource_size_in_pixels());
    ...
    const TextureDrawQuad* out_rounded_display_mask_quad = TextureDrawQuad::MaterialCast(output->quad_list.ElementAt(6));
    ...
    EXPECT_EQ(resource_size_in_pixels7, out_rounded_display_mask_quad->resource_size_in_pixels());
    ...
    EXPECT_EQ(origin_rounded_display_mask_radius,
        out_rounded_display_mask_quad->rounded_display_masks_info.radii[TextureDrawQuad::RoundedDisplayMasksInfo::kOriginRoundedDisplayMaskIndex]);
    EXPECT_EQ(other_rounded_display_mask_radius,
        out_rounded_display_mask_quad->rounded_display_masks_info.radii[TextureDrawQuad::RoundedDisplayMasksInfo::kOtherRoundedDisplayMaskIndex]);
    EXPECT_EQ(is_horizontally_positioned, out_rounded_display_mask_quad->rounded_display_masks_info.is_horizontally_positioned);
}

TEST_F(StructTraitsTest, SurfaceId)
{
    static constexpr FrameSinkId frame_sink_id(1337, 1234);
    static LocalSurfaceId local_surface_id(0xfbadbeef, base::UnguessableToken::Create());
...
TEST_F(StructTraitsTest, OffsetTagDefinition)
{
    ...
    mojo::test::SerializeAndDeserialize<mojom::OffsetTagDefinition>(input, output);
    EXPECT_EQ(input.tag, output.tag);
...
TEST_F(StructTraitsTest, TransferableResource)
{
    ...
    const int8_t mailbox_name[GL_MAILBOX_SIZE_CHROMIUM] = { 0, 9, 8, 7, 6, 5, 4, 3, 2, 1, 9, 7, 5, 3, 1, 2 };
    const gpu::CommandBufferNamespace command_buffer_namespace = gpu::IN_PROCESS;
    const gpu::CommandBufferId command_buffer_id(gpu::CommandBufferId::FromUnsafeValue(0xdeadbeef));
    const uint64_t release_count = 0xdeadbeefdeadL;
    const uint32_t texture_target = 1337;
    const TransferableResource::SynchronizationType sync_type = TransferableResource::SynchronizationType::kGpuCommandsCompleted;
    const bool is_software = false;
    const bool is_overlay_candidate = true;

    gpu::MailboxHolder mailbox_holder;
    mailbox_holder.mailbox.SetName(mailbox_name);
    mailbox_holder.sync_token = gpu::SyncToken(command_buffer_namespace, command_buffer_id, release_count);
    ...
    mojo::test::SerializeAndDeserialize<mojom::TransferableResource>(input, output);
...
TEST_F(StructTraitsTest, SharedImageFormatWithUnknownPlane)
{
    ...
    SharedImageFormat output;
    EXPECT_CHECK_DEATH(mojo::test::SerializeAndDeserialize<mojom::SharedImageFormat>(input, output));
}

TEST_F(StructTraitsTest, CopyOutputResult_EmptyBitmap)
{
    auto input
        = std::make_unique<CopyOutputResult>(CopyOutputRequest::ResultFormat::RGBA, CopyOutputRequest::ResultDestination::kSystemMemory, gfx::Rect(), false);
    ...
    EXPECT_EQ(output->destination(), CopyOutputResult::Destination::kSystemMemory);
...
TEST_F(StructTraitsTest, CopyOutputResult_EmptyTexture)
{
    ...
    auto input
        = std::make_unique<CopyOutputResult>(CopyOutputRequest::ResultFormat::RGBA, CopyOutputRequest::ResultDestination::kNativeTextures, gfx::Rect(), false);
    ...
    EXPECT_TRUE(output->IsEmpty());
    EXPECT_EQ(output->format(), CopyOutputResult::Format::RGBA);
    EXPECT_EQ(output->destination(), CopyOutputResult::Destination::kNativeTextures);
    EXPECT_TRUE(output->rect().IsEmpty());
    EXPECT_EQ(output->GetTextureResult(), nullptr);
}
...
TEST_F(StructTraitsTest, CopyOutputResult_Bitmap)
{
    ...
    SkBitmap bitmap;
    const sk_sp<SkColorSpace> adobe_rgb = SkColorSpace::MakeRGB(SkNamedTransferFn::kSRGB, SkNamedGamut::kAdobeRGB);
    bitmap.allocPixels(SkImageInfo::MakeN32Premul(7, 8, adobe_rgb));
    bitmap.eraseARGB(123, 213, 77, 33);
    std::unique_ptr<CopyOutputResult> input = std::make_unique<CopyOutputSkBitmapResult>(result_rect, std::move(bitmap));
    ...
    EXPECT_EQ(0, std::memcmp(expected_bitmap.getPixels(), out_bitmap.getPixels(), expected_bitmap.computeByteSize()));
    EXPECT_TRUE(SkColorSpace::Equals(expected_bitmap.colorSpace(), out_bitmap.colorSpace()));
}
...
TEST_F(StructTraitsTest, CopyOutputResult_Texture)
{
    ...
    const gfx::ColorSpace result_color_space = gfx::ColorSpace::CreateDisplayP3D65();
    const int8_t mailbox_name[GL_MAILBOX_SIZE_CHROMIUM] = { 0, 9, 8, 7, 6, 5, 4, 3, 2, 1, 9, 7, 5, 3, 1, 3 };
    gpu::SyncToken sync_token(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(0x123), 71234838);
    sync_token.SetVerifyFlush();
    base::RunLoop run_loop;
    CopyOutputResult::ReleaseCallbacks release_callbacks;
    release_callbacks.push_back(base::BindOnce(
    ...
    gpu::Mailbox mailbox;
    mailbox.SetName(mailbox_name);
    std::unique_ptr<CopyOutputResult> input = std::make_unique<CopyOutputTextureResult>(
        CopyOutputResult::Format::RGBA, result_rect, CopyOutputResult::TextureResult(mailbox, result_color_space), std::move(release_callbacks));
    ...
    CopyOutputResult::ReleaseCallbacks out_callbacks = output->TakeTextureOwnership();
