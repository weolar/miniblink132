...
template <> struct StructTraits<viz::mojom::CompositorFrameDataView, viz::CompositorFrame> {
    static const viz::CompositorFrameMetadata& metadata(const viz::CompositorFrame& input)
    {
        return input.metadata;
    }

    static const std::vector<viz::TransferableResource>& resources(const viz::CompositorFrame& input)
    {
        return input.resource_list;
    }

    static const viz::CompositorRenderPassList& passes(const viz::CompositorFrame& input)
    {
        ...
        // regressions.
        if (input.render_pass_list.empty() || input.render_pass_list.back()->output_rect.size().IsEmpty()) {
            base::debug::DumpWithoutCrashing();
        }
        return input.render_pass_list;
    }

    static bool Read(viz::mojom::CompositorFrameDataView data, viz::CompositorFrame* out);
};
