...
// static
bool StructTraits<viz::mojom::CompositorRenderPassDataView, std::unique_ptr<viz::CompositorRenderPass>>::Read(
    viz::mojom::CompositorRenderPassDataView data, std::unique_ptr<viz::CompositorRenderPass>* out)
{
    *out = viz::CompositorRenderPass::Create();
    std::optional<viz::ViewTransitionElementResourceId> opt_view_transition_element_resource_id;
    if (!data.ReadOutputRect(&(*out)->output_rect) || !data.ReadDamageRect(&(*out)->damage_rect)
        || !data.ReadTransformToRootTarget(&(*out)->transform_to_root_target) || !data.ReadFilters(&(*out)->filters)
        || !data.ReadBackdropFilters(&(*out)->backdrop_filters) || !data.ReadBackdropFilterBounds(&(*out)->backdrop_filter_bounds)
        || !data.ReadSubtreeCaptureId(&(*out)->subtree_capture_id) || !data.ReadSubtreeSize(&(*out)->subtree_size)
        || !data.ReadCopyRequests(&(*out)->copy_requests) || !data.ReadViewTransitionElementResourceId(&opt_view_transition_element_resource_id)
        || !data.ReadId(&(*out)->id)) {
        return false;
    }

    if (opt_view_transition_element_resource_id) {
        (*out)->view_transition_element_resource_id = *opt_view_transition_element_resource_id;
    }
    ...
    if ((*out)->subtree_size.width() > (*out)->output_rect.size().width() || (*out)->subtree_size.height() > (*out)->output_rect.size().height()) {
        return false;
    }
    ...
    (*out)->has_damage_from_contributing_content = data.has_damage_from_contributing_content();
    ...
    for (size_t i = 0; i < quads.size(); ++i) {
        ...
        viz::DrawQuad* quad = AllocateAndConstruct(quad_state_data_view.tag(), &(*out)->quad_list);
        if (!quad) {
        ...
        if (!sqs_data_view.is_null()) {
            using SqsTraits = StructTraits<viz::mojom::SharedQuadStateDataView, viz::SharedQuadState>;
