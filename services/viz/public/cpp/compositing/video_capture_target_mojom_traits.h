...
template <> struct UnionTraits<viz::mojom::VideoCaptureSubTargetDataView, viz::VideoCaptureSubTarget> {
    ...
    static const viz::RegionCaptureCropId& region_capture_crop_id(const viz::VideoCaptureSubTarget& data)
    {
        return absl::get<viz::RegionCaptureCropId>(data);
    }

    static viz::SubtreeCaptureId subtree_capture_id(const viz::VideoCaptureSubTarget& data)
    {
    ...
    static bool Read(viz::mojom::VideoCaptureSubTargetDataView data, viz::VideoCaptureSubTarget* out)
    {
        switch (data.tag()) {
...
template <> struct StructTraits<viz::mojom::VideoCaptureTargetDataView, viz::VideoCaptureTarget> {
    static viz::FrameSinkId frame_sink_id(const viz::VideoCaptureTarget& input)
    {
        return input.frame_sink_id;
    }

    static const viz::VideoCaptureSubTarget& sub_target(const viz::VideoCaptureTarget& input)
    {
        return input.sub_target;
    }

    static bool Read(viz::mojom::VideoCaptureTargetDataView data, viz::VideoCaptureTarget* out)
    {
        viz::VideoCaptureTarget target;
        if (!data.ReadSubTarget(&target.sub_target) || !data.ReadFrameSinkId(&target.frame_sink_id))
            return false;
