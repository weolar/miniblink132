...
template <> struct StructTraits<viz::mojom::SurfaceRangeDataView, viz::SurfaceRange> {
    static const std::optional<viz::SurfaceId>& start(const viz::SurfaceRange& range)
    {
    ...
    static bool Read(viz::mojom::SurfaceRangeDataView data, viz::SurfaceRange* out)
    {
        return data.ReadStart(&out->start_) && data.ReadEnd(&out->end_) && out->IsValid();
    }
};
