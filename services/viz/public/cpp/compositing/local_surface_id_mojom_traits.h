...
template <> struct StructTraits<viz::mojom::LocalSurfaceIdDataView, viz::LocalSurfaceId> {
    static uint32_t parent_sequence_number(const viz::LocalSurfaceId& local_surface_id)
    {
        return local_surface_id.parent_sequence_number();
    }

    static uint32_t child_sequence_number(const viz::LocalSurfaceId& local_surface_id)
    {
        return local_surface_id.child_sequence_number();
    }

    static const base::UnguessableToken& embed_token(const viz::LocalSurfaceId& local_surface_id)
    {
        return local_surface_id.embed_token();
    }

    static bool Read(viz::mojom::LocalSurfaceIdDataView data, viz::LocalSurfaceId* out)
    {
