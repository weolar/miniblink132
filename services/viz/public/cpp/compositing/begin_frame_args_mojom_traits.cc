...
// static
viz::mojom::BeginFrameArgsType EnumTraits<viz::mojom::BeginFrameArgsType, viz::BeginFrameArgs::BeginFrameArgsType>::ToMojom(
...
// static
bool EnumTraits<viz::mojom::BeginFrameArgsType, viz::BeginFrameArgs::BeginFrameArgsType>::FromMojom(
    viz::mojom::BeginFrameArgsType input, viz::BeginFrameArgs::BeginFrameArgsType* out)
{
    switch (input) {
...
// static
bool StructTraits<viz::mojom::BeginFrameArgsDataView, viz::BeginFrameArgs>::Read(viz::mojom::BeginFrameArgsDataView data, viz::BeginFrameArgs* out)
{
    if (!data.ReadFrameTime(&out->frame_time) || !data.ReadDeadline(&out->deadline) || !data.ReadInterval(&out->interval) || !data.ReadType(&out->type)
        || !data.ReadDispatchTime(&out->dispatch_time) || !data.ReadClientArrivalTime(&out->client_arrival_time)) {
        return false;
    }
...
// static
bool StructTraits<viz::mojom::BeginFrameAckDataView, viz::BeginFrameAck>::Read(viz::mojom::BeginFrameAckDataView data, viz::BeginFrameAck* out)
{
    if (data.sequence_number() < viz::BeginFrameArgs::kStartingFrameNumber) {
        viz::SetDeserializationCrashKeyString("Invalid begin frame ack sequence number");
        return false;
    }
    out->frame_id.source_id = data.source_id();
    out->frame_id.sequence_number = data.sequence_number();
    ...
    if (out->preferred_frame_interval && out->preferred_frame_interval->is_negative()) {
        return false;
    }
    return true;
}
