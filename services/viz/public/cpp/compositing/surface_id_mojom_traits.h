...
template <> struct StructTraits<viz::mojom::SurfaceIdDataView, viz::SurfaceId> {
    ...
    {
        return data.ReadFrameSinkId(&out->frame_sink_id_) && data.ReadLocalSurfaceId(&out->local_surface_id_);
    }
};
