...
template <> struct StructTraits<viz::mojom::SharedBitmapIdDataView, viz::SharedBitmapId> {
    static base::span<const uint8_t> name(const viz::SharedBitmapId& shared_bitmap_id)
    {
        return base::span<const uint8_t>(shared_bitmap_id.name.data(), shared_bitmap_id.name.size());
    }
    static bool Read(viz::mojom::SharedBitmapIdDataView data, viz::SharedBitmapId* out)
    {
        base::span<uint8_t> shared_bitmap_id_name(out->name.data(), out->name.size());
