...
bool RenderPassExists(viz::CompositorRenderPassId pass_id, const viz::CompositorRenderPassList& render_passes)
{
...
// static
bool StructTraits<viz::mojom::CompositorFrameDataView, viz::CompositorFrame>::Read(viz::mojom::CompositorFrameDataView data, viz::CompositorFrame* out)
{
    if (!data.ReadPasses(&out->render_pass_list))
        return false;

    if (out->render_pass_list.empty()) {
        viz::SetDeserializationCrashKeyString("CompositorFrame::render_pass_list empty");
        return false;
    }
    ...
    for (const auto& directive : out->metadata.transition_directives) {
        if (directive.type() != viz::CompositorFrameTransitionDirective::Type::kSave) {
        ...
        for (const auto& shared_element : directive.shared_elements()) {
            ...
            if (!RenderPassExists(shared_element.render_pass_id, out->render_pass_list)) {
                return false;
            }
        }
    }
    ...
    if (!data.ReadResources(&out->resource_list)) {
        viz::SetDeserializationCrashKeyString("Failed read CompositorFrame::resource_list");
        return false;
    }
