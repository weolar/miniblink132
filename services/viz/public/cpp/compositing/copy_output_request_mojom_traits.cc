...
public:
    CopyOutputResultSenderImpl(viz::CopyOutputRequest::ResultFormat result_format, viz::CopyOutputRequest::ResultDestination result_destination,
        viz::CopyOutputRequest::CopyOutputRequestCallback result_callback, scoped_refptr<base::SequencedTaskRunner> callback_task_runner)
    ...
    {
        if (result_callback_) {
            result_callback_task_runner_->PostTask(FROM_HERE,
                base::BindOnce(std::move(result_callback_), std::make_unique<viz::CopyOutputResult>(result_format_, result_destination_, gfx::Rect(), false)));
        }
    }
    ...
    void SendResult(std::unique_ptr<viz::CopyOutputResult> result) override
    {
        ...
        if (!result_callback_)
            return;
        result_callback_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(result_callback_), std::move(result)));
    }
...
void SendResult(mojo::PendingRemote<viz::mojom::CopyOutputResultSender> pending_remote, std::unique_ptr<viz::CopyOutputResult> result)
{
    TRACE_EVENT0("viz", "viz::mojom::CopyOutputResultSender::SendResult");
    mojo::Remote<viz::mojom::CopyOutputResultSender> remote(std::move(pending_remote));
...
StructTraits<viz::mojom::CopyOutputRequestDataView, std::unique_ptr<viz::CopyOutputRequest>>::result_sender(
    const std::unique_ptr<viz::CopyOutputRequest>& request)
{
    ...
    auto impl = std::make_unique<CopyOutputResultSenderImpl>(request->result_format(), request->result_destination(), std::move(request->result_callback_),
        request->has_result_task_runner() ? request->result_task_runner_ : base::SequencedTaskRunner::GetCurrentDefault());
    auto runner = base::ThreadPool::CreateSequencedTaskRunner({ base::MayBlock() });
    runner->PostTask(FROM_HERE,
        base::BindOnce([](std::unique_ptr<CopyOutputResultSenderImpl> impl,
                           mojo::PendingReceiver<viz::mojom::CopyOutputResultSender> receiver) { MakeSelfOwnedReceiver(std::move(impl), std::move(receiver)); },
            std::move(impl), std::move(pending_receiver)));
    return result_sender;
}

// static
bool StructTraits<viz::mojom::CopyOutputRequestDataView, std::unique_ptr<viz::CopyOutputRequest>>::Read(
    viz::mojom::CopyOutputRequestDataView data, std::unique_ptr<viz::CopyOutputRequest>* out_p)
{
    ...
    auto result_sender = data.TakeResultSender<mojo::PendingRemote<viz::mojom::CopyOutputResultSender>>();

    auto request = std::make_unique<viz::CopyOutputRequest>(result_format, result_destination, base::BindOnce(SendResult, std::move(result_sender)));
    ...
    if (!data.ReadSource(&request->source_) || !data.ReadArea(&request->area_) || !data.ReadResultSelection(&request->result_selection_)) {
        return false;
    }
