...
public:
    ...
    void Initialize(mojo::PendingRemote<mojom::Gpu> gpu_remote, mojo::PendingReceiver<gpu::mojom::ClientGmbInterface> client_gmb_interface_receiver)
    {
        DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);

        gpu_remote_.Bind(std::move(gpu_remote));
        gpu_remote_.set_disconnect_handler(base::BindOnce(&GpuPtrIO::ConnectionError, base::Unretained(this)));
        gpu_remote_->CreateClientGpuMemoryBufferFactory(std::move(client_gmb_interface_receiver));
    }
    ...
    void EstablishGpuChannel(scoped_refptr<EstablishRequest> establish_request)
    {
        ...
        if (gpu_remote_.is_connected()) {
            gpu_remote_->EstablishGpuChannel(base::BindOnce(&GpuPtrIO::OnEstablishedGpuChannel, base::Unretained(this)));
        } else {
            ConnectionError();
        }
    }
    ...
    void CreateJpegDecodeAccelerator(mojo::PendingReceiver<chromeos_camera::mojom::MjpegDecodeAccelerator> receiver)
    {
        DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    void CreateVideoEncodeAcceleratorProvider(mojo::PendingReceiver<media::mojom::VideoEncodeAcceleratorProvider> receiver)
    {
        DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
private:
    ...
    void OnEstablishedGpuChannel(int client_id, mojo::ScopedMessagePipeHandle channel_handle, const gpu::GPUInfo& gpu_info,
...
class Gpu::EstablishRequest : public base::RefCountedThreadSafe<Gpu::EstablishRequest> {
public:
    EstablishRequest(Gpu* parent, scoped_refptr<base::SingleThreadTaskRunner> main_task_runner)
        : parent_(parent)
    ...
    {
        return gpu_remote_disconnected_;
    }
    ...
    void OnEstablishedGpuChannel(int client_id, mojo::ScopedMessagePipeHandle channel_handle, const gpu::GPUInfo& gpu_info,
        const gpu::GpuFeatureInfo& gpu_feature_info, const gpu::SharedImageCapabilities& shared_image_capabilities, bool gpu_remote_disconnected)
    {
        DCHECK(!main_task_runner_->BelongsToCurrentThread());
        base::AutoLock lock(lock_);
        ...
        if (channel_handle.is_valid()) {
            gpu_channel_
                = base::MakeRefCounted<gpu::GpuChannelHost>(client_id, gpu_info, gpu_feature_info, shared_image_capabilities, std::move(channel_handle));
        }
        ...
        if (establish_event_) {
            ...
            main_task_runner_->PostTask(FROM_HERE, base::BindOnce(&EstablishRequest::FinishOnMain, this));
        }
    }
...
void Gpu::GpuPtrIO::ConnectionError()
{
    ...
    establish_request_->OnEstablishedGpuChannel(
        0, mojo::ScopedMessagePipeHandle(), gpu::GPUInfo(), gpu::GpuFeatureInfo(), gpu::SharedImageCapabilities(), /*gpu_remote_disconnected=*/true);
    establish_request_.reset();
}

void Gpu::GpuPtrIO::OnEstablishedGpuChannel(int client_id, mojo::ScopedMessagePipeHandle channel_handle, const gpu::GPUInfo& gpu_info,
    const gpu::GpuFeatureInfo& gpu_feature_info, const gpu::SharedImageCapabilities& shared_image_capabilities)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(establish_request_);

    establish_request_->OnEstablishedGpuChannel(client_id, std::move(channel_handle), std::move(gpu_info), std::move(gpu_feature_info),
        std::move(shared_image_capabilities),
        /*gpu_remote_disconnected=*/false);
    establish_request_.reset();
}

Gpu::Gpu(mojo::PendingRemote<mojom::Gpu> gpu_remote, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    ...
    , gpu_(new GpuPtrIO(), base::OnTaskRunnerDeleter(io_task_runner_))
{
    ...
    auto client_gmb_interface_receiver = client_gmb_interface.InitWithNewPipeAndPassReceiver();

    // Note that since |gpu_memory_buffer_manager_| is a owned by this object, it
    // is safe to provide a |this| pointer to it.
    gpu_memory_buffer_manager_ = std::make_unique<ClientGpuMemoryBufferManager>(std::move(client_gmb_interface));
    ...
    io_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&GpuPtrIO::Initialize, base::Unretained(gpu_.get()), std::move(gpu_remote), std::move(client_gmb_interface_receiver)));
}
...
std::unique_ptr<Gpu> Gpu::Create(
    service_manager::Connector* connector, const std::string& service_name, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
...
// static
std::unique_ptr<Gpu> Gpu::Create(mojo::PendingRemote<mojom::Gpu> remote, scoped_refptr<base::SingleThreadTaskRunner> io_task_runner)
{
    return base::WrapUnique(new Gpu(std::move(remote), std::move(io_task_runner)));
}

#if BUILDFLAG(IS_CHROMEOS_ASH)
void Gpu::CreateJpegDecodeAccelerator(mojo::PendingReceiver<chromeos_camera::mojom::MjpegDecodeAccelerator> jda_receiver)
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
    io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&GpuPtrIO::CreateJpegDecodeAccelerator, base::Unretained(gpu_.get()), std::move(jda_receiver)));
}
#endif // BUILDFLAG(IS_CHROMEOS_ASH)

void Gpu::CreateVideoEncodeAcceleratorProvider(mojo::PendingReceiver<media::mojom::VideoEncodeAcceleratorProvider> vea_provider_receiver)
{
    DCHECK(main_task_runner_->BelongsToCurrentThread());
    io_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&GpuPtrIO::CreateVideoEncodeAcceleratorProvider, base::Unretained(gpu_.get()), std::move(vea_provider_receiver)));
}
...
scoped_refptr<gpu::GpuChannelHost> Gpu::EstablishGpuChannelSync()
{
    ...
    base::WaitableEvent event(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::SIGNALED);
    ...
    if (first_run_in_process) {
        ...
        base::UmaHistogramTimes("GPU.EstablishGpuChannelSyncTime.FirstRun", timer.Elapsed());
    }
    return gpu_channel_;
}
...
void Gpu::SendEstablishGpuChannelRequest(base::WaitableEvent* waitable_event)
{
    ...
    pending_request_ = base::MakeRefCounted<EstablishRequest>(this, main_task_runner_);
    if (waitable_event) {
        pending_request_->SetWaitableEvent(waitable_event);
    }
    io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&EstablishRequest::SendRequest, pending_request_, base::Unretained(gpu_.get())));
}
