...
public:
    ...
    void CloseBindingOnRequest()
    {
        close_binding_on_request_ = true;
    }
    ...
    void EstablishGpuChannel(EstablishGpuChannelCallback callback) override
    {
        ...
        std::move(callback).Run(client_id, std::move(handle), gpu::GPUInfo(), gpu::GpuFeatureInfo(), gpu::SharedImageCapabilities());
    }
...
public:
    ...
    TestGpuImpl* gpu_impl()
    {
        return gpu_impl_.get();
    }
    ...
    void FlushMainAndIO()
    {
        ...
        io_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce([](base::OnceClosure callback) { std::move(callback).Run(); }, run_loop.QuitClosure()));
        run_loop.Run();
    }
...
private:
    mojo::PendingRemote<mojom::Gpu> GetRemote()
    {
        ...
        io_thread_.task_runner()->PostTask(
            FROM_HERE, base::BindOnce(&TestGpuImpl::BindReceiver, base::Unretained(gpu_impl_.get()), remote.InitWithNewPipeAndPassReceiver()));
        return remote;
    }
    ...
    void DestroyGpuImplOnIO()
    {
        ...
        io_thread_.task_runner()->PostTask(FROM_HERE,
            base::BindOnce(
                [](std::unique_ptr<TestGpuImpl> gpu_impl, base::WaitableEvent* event) {
                    gpu_impl.reset();
                    event->Signal();
                },
...
TEST_F(GpuTest, EstablishRequestsQueued)
{
    ...
    auto callback = base::BindRepeating(
        [](int* counter, base::OnceClosure callback, scoped_refptr<gpu::GpuChannelHost> channel) {
            EXPECT_TRUE(channel);
            --(*counter);
            if (*counter == 0)
...
TEST_F(GpuTest, EstablishRequestOnFailureOnPreviousRequest)
{
    ...
    auto callback = base::BindOnce(
        [](scoped_refptr<gpu::GpuChannelHost>* out_host, base::OnceClosure callback, scoped_refptr<gpu::GpuChannelHost> host) {
            std::move(callback).Run();
            *out_host = std::move(host);
        },
        &host, run_loop.QuitClosure());
    gpu()->EstablishGpuChannel(base::BindOnce(
        [](Gpu* gpu, TestGpuImpl* gpu_impl, gpu::GpuChannelEstablishedCallback callback, scoped_refptr<gpu::GpuChannelHost> host) {
            EXPECT_FALSE(host);
...
TEST_F(GpuTest, AsyncConnectionError)
{
    ...
    // counter reaches 0.
    auto callback = base::BindRepeating(
        [](int* counter, const base::RepeatingClosure& callback, scoped_refptr<gpu::GpuChannelHost> channel) {
            EXPECT_FALSE(channel);
            --(*counter);
            if (*counter == 0)
                callback.Run();
...
{
    int counter = 0;
    gpu()->EstablishGpuChannel(base::BindOnce([](int* counter, scoped_refptr<gpu::GpuChannelHost> host) { ++(*counter); }, base::Unretained(&counter)));
