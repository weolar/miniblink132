...
ssize_t SocketWritev(base::PlatformFile socket, struct iovec* iov, size_t num_iov)
{
#if BUILDFLAG(IS_APPLE)
...
ssize_t SendmsgWithHandles(base::PlatformFile socket, struct iovec* iov, size_t num_iov, const std::vector<base::ScopedFD>& descriptors)
{
    DCHECK(iov);
...
ssize_t SocketRecvmsg(base::PlatformFile socket, void* buf, size_t num_bytes, std::vector<base::ScopedFD>* descriptors, bool block)
{
    ...
    ssize_t result = HANDLE_EINTR(recvmsg(socket, &msg, block ? 0 : MSG_DONTWAIT));
    if (result < 0)
        return result;
...
bool AcceptSocketConnection(base::PlatformFile server_fd, base::ScopedFD* connection_fd, bool check_peer_user)
{
