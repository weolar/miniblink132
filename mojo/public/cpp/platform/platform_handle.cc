...
base::win::ScopedHandle CloneHandle(const base::win::ScopedHandle& handle)
{
    ...
    if (handle.Get() != INVALID_HANDLE_VALUE) {
        result = ::DuplicateHandle(::GetCurrentProcess(), handle.Get(), ::GetCurrentProcess(), &dupe, 0, FALSE, DUPLICATE_SAME_ACCESS);
    }
    if (!result)
...
#elif BUILDFLAG(IS_APPLE)
base::mac::ScopedMachSendRight CloneMachPort(const base::mac::ScopedMachSendRight& mach_port)
{
    DCHECK(mach_port.is_valid());

    kern_return_t kr = mach_port_mod_refs(mach_task_self(), mach_port.get(), MACH_PORT_RIGHT_SEND, 1);
    if (kr != KERN_SUCCESS) {
...
PlatformHandle::PlatformHandle(base::mac::ScopedMachSendRight mach_port)
    : type_(Type::kMachSend)
    , mach_send_(std::move(mach_port))
{
}
PlatformHandle::PlatformHandle(base::mac::ScopedMachReceiveRight mach_port)
    : type_(Type::kMachReceive)
    , mach_receive_(std::move(mach_port))
{
}
#endif
...
// static
void PlatformHandle::ToMojoPlatformHandle(PlatformHandle handle, MojoPlatformHandle* out_handle)
{
    DCHECK(out_handle);
    ...
    do {
        ...
        out_handle->value = static_cast<uint64_t>(HandleToLong(handle.TakeHandle().Take()));
        break;
#elif BUILDFLAG(IS_FUCHSIA)
        if (handle.is_handle()) {
        ...
        if (handle.is_mach_send()) {
            ...
            out_handle->value = static_cast<uint64_t>(handle.ReleaseMachReceiveRight());
            break;
        }
#endif
...
// static
PlatformHandle PlatformHandle::FromMojoPlatformHandle(const MojoPlatformHandle* handle)
{
    if (handle->struct_size < sizeof(*handle) || handle->type == MOJO_PLATFORM_HANDLE_TYPE_INVALID) {
    ...
    if (handle->type != MOJO_PLATFORM_HANDLE_TYPE_WINDOWS_HANDLE)
        return PlatformHandle();
    return PlatformHandle(base::win::ScopedHandle(LongToHandle(static_cast<long>(handle->value))));
#elif BUILDFLAG(IS_FUCHSIA)
    ...
    if (handle->type == MOJO_PLATFORM_HANDLE_TYPE_MACH_SEND_RIGHT) {
        return PlatformHandle(base::mac::ScopedMachSendRight(static_cast<mach_port_t>(handle->value)));
    } else if (handle->type == MOJO_PLATFORM_HANDLE_TYPE_MACH_RECEIVE_RIGHT) {
        return PlatformHandle(base::mac::ScopedMachReceiveRight(static_cast<mach_port_t>(handle->value)));
    }
#endif
