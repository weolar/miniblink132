...
public:
    ...
    void PrepareToPassRemoteEndpoint(HandlePassingInfo* info, base::CommandLine* command_line);
    ...
    void PrepareToPassRemoteEndpoint(base::LaunchOptions* options, base::CommandLine* command_line);
    ...
    [[nodiscard]] static PlatformChannelEndpoint RecoverPassedEndpointFromString(base::StringPiece value);

    // Like above but extracts the input string from |command_line| via the
    // |kHandleSwitch| flag.
    [[nodiscard]] static PlatformChannelEndpoint RecoverPassedEndpointFromCommandLine(const base::CommandLine& command_line);

    // Indicates whether |RecoverPassedEndpointFromCommandLine()| would succeed.
    static bool CommandLineHasPassedEndpoint(const base::CommandLine& command_line);
