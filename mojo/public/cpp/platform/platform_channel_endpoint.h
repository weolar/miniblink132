...
#error "Unsupported platform."
    ...
    const PlatformHandle& platform_handle() const
    {
        return handle_;
    }
    ...
    void PrepareToPass(base::LaunchOptions& options, base::CommandLine& command_line);

    // Must be called after the corresponding process launch attempt if
    // PrepareToPass() was called.
    void ProcessLaunchAttempted();

    [[nodiscard]] static PlatformChannelEndpoint RecoverFromString(base::StringPiece value);
