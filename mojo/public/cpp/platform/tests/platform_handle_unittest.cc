...
class PlatformHandleTest : public testing::Test, public testing::WithParamInterface<HandleType> {
public:
...
private:
    ...
    PlatformHandle SetUpFile()
    {
        ...
        base::File test_file(temp_dir_.GetPath().AppendASCII("test"), base::File::FLAG_CREATE | base::File::FLAG_WRITE | base::File::FLAG_READ);
        test_file.WriteAtCurrentPos(kTestData.data(), static_cast<int>(kTestData.size()));

#if BUILDFLAG(IS_WIN)
        return PlatformHandle(base::win::ScopedHandle(test_file.TakePlatformFile()));
#else
...
// Let |handle| retain ownership.
    ...
    PlatformHandle SetUpSharedMemory()
    {
        ...
        auto generic_region = base::UnsafeSharedMemoryRegion::TakeHandleForSerialization(std::move(region));
    ...
    std::string GetSharedMemoryContents(const PlatformHandle& handle)
    {
        ...
        auto generic_region = base::subtle::PlatformSharedMemoryRegion::Take(
            std::move(region_handle), base::subtle::PlatformSharedMemoryRegion::Mode::kUnsafe, kTestData.size(), shm_guid_);
        auto region = base::UnsafeSharedMemoryRegion::Deserialize(std::move(generic_region));
        auto mapping = region.Map();
        std::string contents(static_cast<char*>(mapping.memory()), kTestData.size());

        // Let |handle| retain ownership.
        generic_region = base::UnsafeSharedMemoryRegion::TakeHandleForSerialization(std::move(region));
...
INSTANTIATE_TEST_SUITE_P(All, PlatformHandleTest,
#if BUILDFLAG(IS_WIN)
    testing::Values(HandleType::kHandle)
#elif BUILDFLAG(IS_FUCHSIA)
    testing::Values(HandleType::kHandle, HandleType::kFileDescriptor)
#elif BUILDFLAG(IS_MAC)
    testing::Values(HandleType::kFileDescriptor, HandleType::kMachPort)
