...
constexpr int kAndroidClientHandleDescriptor = base::GlobalDescriptors::kBaseDescriptor + 10000;
#elif BUILDFLAG(IS_POSIX) && !BUILDFLAG(IS_MAC)
bool IsTargetDescriptorUsed(const base::FileHandleMappingVector& mapping, int target_fd)
{
...
PlatformChannelEndpoint::PlatformChannelEndpoint(PlatformChannelEndpoint&& other) = default;
...
PlatformChannelEndpoint& PlatformChannelEndpoint::operator=(PlatformChannelEndpoint&& other) = default;
...
void PlatformChannelEndpoint::PrepareToPass(HandlePassingInfo& info, base::CommandLine& command_line)
{
    std::string value;
...
void PlatformChannelEndpoint::PrepareToPass(HandlePassingInfo& info, std::string& value)
{
    DCHECK(is_valid());
#if BUILDFLAG(IS_WIN)
    info.push_back(platform_handle().GetHandle().Get());
    value = base::NumberToString(HandleToLong(platform_handle().GetHandle().Get()));
#elif BUILDFLAG(IS_FUCHSIA)
    const uint32_t id = base::LaunchOptions::AddHandleToTransfer(&info, platform_handle().GetHandle().get());
    ...
    base::mac::ScopedMachReceiveRight receive_right = TakePlatformHandle().TakeMachReceiveRight();
    ...
    auto it = info.insert(std::make_pair(rendezvous_key, base::MachRendezvousPort(std::move(receive_right))));
...
void PlatformChannelEndpoint::PrepareToPass(base::LaunchOptions& options, base::CommandLine& command_line)
{
#if BUILDFLAG(IS_WIN)
...
// static
PlatformChannelEndpoint PlatformChannelEndpoint::RecoverFromString(base::StringPiece value)
{
#if BUILDFLAG(IS_WIN)
    int handle_value = 0;
    ...
    return PlatformChannelEndpoint(PlatformHandle(base::win::ScopedHandle(LongToHandle(handle_value))));
#elif BUILDFLAG(IS_FUCHSIA)
    ...
    return PlatformChannelEndpoint(PlatformHandle(zx::handle(zx_take_startup_handle(base::checked_cast<uint32_t>(handle_value)))));
#elif BUILDFLAG(IS_ANDROID)
    ...
    return PlatformChannelEndpoint(PlatformHandle(base::ScopedFD(base::GlobalDescriptors::GetInstance()->Get(key))));
#elif BUILDFLAG(IS_MAC)
...
#elif BUILDFLAG(IS_POSIX)
    int fd = -1;
    if (value.empty() || !base::StringToInt(value, &fd) || fd < base::GlobalDescriptors::kBaseDescriptor) {
        DLOG(ERROR) << "Invalid PlatformChannel endpoint string.";
        return PlatformChannelEndpoint();
    }
