...
NamedPlatformChannel::ServerName GenerateRandomServerName(const NamedPlatformChannel::Options& options)
{
    return options.socket_dir.AppendASCII(base::NumberToString(base::RandUint64())).value();
}
...
bool MakeUnixAddr(const NamedPlatformChannel::ServerName& server_name, bool use_abstract_namespace, net::SockaddrStorage* addr_storage)
{
    DCHECK(addr_storage);
    ...
    return net::FillUnixAddress(server_name, use_abstract_namespace, addr_storage);
}
...
PlatformChannelEndpoint NamedPlatformChannel::CreateClientEndpoint(const Options& options)
{
    ...
    net::SockaddrStorage storage;
    if (!MakeUnixAddr(options.server_name, options.use_abstract_namespace, &storage))
    ...
    if (HANDLE_EINTR(connect(handle.GetFD().get(), storage.addr, storage.addr_len)) < 0) {
