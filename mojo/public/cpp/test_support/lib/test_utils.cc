...
{
    MojoResult rv = WriteMessageRaw(handle, text.data(), static_cast<uint32_t>(text.size()), nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE);
...
bool ReadTextMessage(const MessagePipeHandle& handle, std::string* text)
{
    ...
    if (ReadMessageRaw(handle, &bytes, &handles, MOJO_READ_MESSAGE_FLAG_NONE) != MOJO_RESULT_OK) {
        return false;
    }
...
void IterateAndReportPerf(const char* test_name, const char* sub_test_name, PerfTestSingleIteration single_iteration, void* closure)
{
    ...
    MojoTestSupportLogPerfResult(test_name, sub_test_name, 1000000.0 * iterations / (end_time - start_time), "iterations/second");
}

BadMessageObserver::BadMessageObserver()
    : got_bad_message_(false)
{
    mojo::SetDefaultProcessErrorHandler(base::BindRepeating(&BadMessageObserver::OnReportBadMessage, base::Unretained(this)));
}
