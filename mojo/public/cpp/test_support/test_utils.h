...
template <typename MojomType, typename UserStructType, std::enable_if_t<!std::is_enum<UserStructType>::value, int> = 0>
bool SerializeAndDeserialize(UserStructType& input, std::remove_const_t<UserStructType>& output)
{
...
template <typename MojomType, typename UserStructType, typename MojomStructPtr,
    std::enable_if_t<(std::is_same<mojo::InlinedStructPtr<MojomType>, std::remove_const_t<MojomStructPtr>>::value
                         || std::is_same<mojo::StructPtr<MojomType>, std::remove_const_t<MojomStructPtr>>::value)
...
template <typename MojomType, typename UserEnumType, std::enable_if_t<std::is_enum<UserEnumType>::value, int> = 0>
...
void IterateAndReportPerf(const char* test_name, const char* sub_test_name, PerfTestSingleIteration single_iteration, void* closure);
...
public:
    ...
    bool got_bad_message() const
    {
        return got_bad_message_;
    }
...
template <typename T> class ScopedSwapImplForTesting {
public:
    ...
    ImplPointerType old_impl() const
    {
        return old_impl_;
    }
