...
{
    static constexpr int kTestCases[] = { 0, -1, 1, std::numeric_limits<int>::min(), std::numeric_limits<int>::max() };
    for (auto& test_case : kTestCases) {
        base::Value in(test_case);
        base::Value out;
        ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::Value>(in, out));
...
{
    static constexpr double kTestCases[] = { -0.0, +0.0, -1.0, +1.0, std::numeric_limits<double>::min(), std::numeric_limits<double>::max() };
    for (auto& test_case : kTestCases) {
        base::Value in(test_case);
        base::Value out;
        ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::Value>(in, out));
...
TEST(ValuesStructTraitsTest, DictionaryValue)
{
    ...
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::DeprecatedDictionaryValue>(in, out));
    EXPECT_EQ(in, out);

    base::Value::Dict in_dict = in.GetDict().Clone();
    base::Value::Dict out_dict;
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::DictionaryValue>(in_dict, out_dict));
...
TEST(ValuesStructTraitsTest, SerializeInvalidDictionaryValue)
{
    ...
    base::Value out;
    EXPECT_DCHECK_DEATH(mojo::test::SerializeAndDeserialize<mojom::DeprecatedDictionaryValue>(in, out));
}
...
TEST(ValuesStructTraitsTest, ListValue)
{
    ...
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::ListValue>(in_list, out_list));
...
TEST(ValuesStructTraitsTest, DeeplyNestedValue)
{
    ...
    {
        mojo::internal::ValidationErrorObserverForTesting warning_observer { base::DoNothing() };
        base::Value out;
        ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::Value>(in, out));
        EXPECT_EQ(mojo::internal::VALIDATION_ERROR_NONE, warning_observer.last_error());
    }
    ...
    // It gets VALIDATION_ERROR_MAX_RECURSION_DEPTH error.
    {
        ...
        EXPECT_EQ(mojo::internal::VALIDATION_ERROR_MAX_RECURSION_DEPTH, warning_observer.last_error());
    }
}
