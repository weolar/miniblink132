...
template <> struct COMPONENT_EXPORT(MOJO_BASE_SHARED_TRAITS) StructTraits<mojo_base::mojom::ReadOnlyBufferDataView, base::span<const uint8_t>> {
    static base::span<const uint8_t> buffer(base::span<const uint8_t> input)
    {
        return input;
    }

    static bool Read(mojo_base::mojom::ReadOnlyBufferDataView input, base::span<const uint8_t>* out);
};
