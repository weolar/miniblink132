...
BigBufferSharedMemoryRegion::BigBufferSharedMemoryRegion(mojo::ScopedSharedBufferHandle buffer_handle, size_t size)
    : size_(size)
...
BigBufferSharedMemoryRegion::BigBufferSharedMemoryRegion(BigBufferSharedMemoryRegion&& other) = default;

BigBufferSharedMemoryRegion::~BigBufferSharedMemoryRegion() = default;

BigBufferSharedMemoryRegion& BigBufferSharedMemoryRegion::operator=(BigBufferSharedMemoryRegion&& other) = default;
...
BigBuffer::BigBuffer(const std::vector<uint8_t>& data)
    : BigBuffer(base::span<const uint8_t>(data.data(), data.size()))
{
}
...
const uint8_t* BigBuffer::data() const
{
    ...
    case StorageType::kSharedMemory:
        ...
        return static_cast<const uint8_t*>(const_cast<const void*>(shared_memory_->buffer_mapping_.get()));
...
BigBufferView::BigBufferView(base::span<const uint8_t> bytes)
{
    ...
    if (storage_type_ == BigBuffer::StorageType::kSharedMemory) {
        ...
        std::copy(bytes.begin(), bytes.end(), static_cast<uint8_t*>(shared_memory_->memory()));
        return;
    }
    if (storage_type_ == BigBuffer::StorageType::kBytes) {
...
void BigBufferView::SetSharedMemory(internal::BigBufferSharedMemoryRegion shared_memory)
{
    DCHECK(bytes_.empty());
    DCHECK(!shared_memory_);
...
base::span<const uint8_t> BigBufferView::data() const
{
    if (storage_type_ == BigBuffer::StorageType::kBytes) {
        ...
        return base::make_span(static_cast<const uint8_t*>(const_cast<const void*>(shared_memory_->memory())), shared_memory_->size());
    }
