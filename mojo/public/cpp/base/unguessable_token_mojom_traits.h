...
static_assert(sizeof(base::UnguessableToken) == 2 * sizeof(uint64_t), "base::UnguessableToken should be of size 2 * sizeof(uint64_t).");

template <> struct COMPONENT_EXPORT(MOJO_BASE_SHARED_TRAITS) StructTraits<mojo_base::mojom::UnguessableTokenDataView, base::UnguessableToken> {
    ...
    static bool Read(mojo_base::mojom::UnguessableTokenDataView data, base::UnguessableToken* out);
};
