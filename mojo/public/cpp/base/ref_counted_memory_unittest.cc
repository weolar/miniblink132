...
TEST(RefCountedMemoryTest, Data)
{
    ...
    scoped_refptr<base::RefCountedMemory> in = new base::RefCountedStaticMemory(&data, std::size(data));

    scoped_refptr<base::RefCountedMemory> out;
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::RefCountedMemory>(in, out));
...
TEST(RefCountedMemoryTest, Null)
{
    ...
    uint8_t data[] = { 'a', 'b', 'c', 'd', 'e' };
    scoped_refptr<base::RefCountedMemory> out = new base::RefCountedStaticMemory(&data, std::size(data));
