...
{
    return a.size() == b.size() && std::equal(a.data(), a.data() + a.size(), b.data(), b.data() + b.size());
}
...
TEST(BigBufferTest, InvalidBuffer)
{
    ...
    EXPECT_EQ(invalid_view.storage_type(), BigBuffer::StorageType::kInvalidBuffer);
    EXPECT_FALSE(mojo::test::SerializeAndDeserialize<mojom::BigBuffer>(invalid_view, out_view));

    BigBuffer out_buffer;
    auto invalid_buffer = BigBufferView::ToBigBuffer(BigBufferView::CreateInvalidForTest());
    EXPECT_EQ(invalid_buffer.storage_type(), BigBuffer::StorageType::kInvalidBuffer);
    EXPECT_FALSE(mojo::test::SerializeAndDeserialize<mojom::BigBuffer>(invalid_buffer, out_buffer));
}
