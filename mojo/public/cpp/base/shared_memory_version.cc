...
SharedMemoryVersionController::SharedMemoryVersionController()
    : mapped_region_(
        ...
        // confusion.
        base::AtomicSharedMemory<VersionType>::Create(shared_memory_version::kInitialVersion).value())
{
}

SharedMemoryVersionController::~SharedMemoryVersionController() = default;

base::ReadOnlySharedMemoryRegion SharedMemoryVersionController::GetSharedMemoryRegion() const
{
...
void SharedMemoryVersionController::Increment()
{
    // Relaxed memory order because no other memory operation depends on the
    // version.
    const VersionType version = mapped_region_.WritableRef().fetch_add(1, std::memory_order_relaxed);
...
SharedMemoryVersionClient::SharedMemoryVersionClient(base::ReadOnlySharedMemoryRegion shared_region)
    : read_only_mapping_(base::AtomicSharedMemory<VersionType>::MapReadOnlyRegion(std::move(shared_region)).value())
{
}

SharedMemoryVersionClient::~SharedMemoryVersionClient() = default;

bool SharedMemoryVersionClient::SharedVersionIsLessThan(VersionType version) const
{
...
bool SharedMemoryVersionClient::SharedVersionIsGreaterThan(VersionType version) const
{
    // Invalid version numbers cannot be compared. Default to IPC.
    if (version == shared_memory_version::kInvalidVersion) {
        return true;
