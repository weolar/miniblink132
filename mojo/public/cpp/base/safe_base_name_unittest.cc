...
TEST(SafeBaseNameTest, PathEmpty)
{
    ...
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::SafeBaseName>(*basename, *basename_out));
    EXPECT_EQ(basename->path(), basename_out->path());
}

TEST(SafeBaseNameTest, PathContainsNoSeparators)
{
    absl::optional<base::SafeBaseName> basename(base::SafeBaseName::Create(FILE_PATH_LITERAL("hello")));
    absl::optional<base::SafeBaseName> basename_out = CreateSafeBaseName();
...
{
    base::FilePath file = base::FilePath(FILE_PATH_LITERAL("hello")).Append(FILE_PATH_LITERAL("world"));
...
{
    base::FilePath file = base::FilePath(FILE_PATH_LITERAL("hello")).Append(FILE_PATH_LITERAL("world")).AsEndingWithSeparator();
    absl::optional<base::SafeBaseName> basename(base::SafeBaseName::Create(file));
    absl::optional<base::SafeBaseName> basename_out = CreateSafeBaseName();
...
{
    mojo_base::mojom::SafeBaseNamePtr mojom_basename = mojo_base::mojom::SafeBaseName::New();
    mojom_basename->path = base::FilePath(FILE_PATH_LITERAL("C:\\"));
    absl::optional<base::SafeBaseName> basename_out = CreateSafeBaseName();

    // Expect deserialization to fail because "C:\\ is an absolute path. See
    // safe_base_name.h
    ASSERT_FALSE(mojo::test::SerializeAndDeserialize<mojom::SafeBaseName>(mojom_basename, *basename_out));
}
#else
...
{
    absl::optional<base::SafeBaseName> basename(base::SafeBaseName::Create(FILE_PATH_LITERAL("C:\\foo.txt")));
    absl::optional<base::SafeBaseName> basename_out = CreateSafeBaseName();
