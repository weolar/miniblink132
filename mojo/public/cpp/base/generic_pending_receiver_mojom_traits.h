...
template <> struct COMPONENT_EXPORT(MOJO_BASE_SHARED_TRAITS) StructTraits<mojo_base::mojom::GenericPendingReceiverDataView, GenericPendingReceiver> {
    static bool IsNull(const GenericPendingReceiver& receiver);
    static void SetToNull(GenericPendingReceiver* receiver);

    static base::StringPiece interface_name(const GenericPendingReceiver& receiver)
    {
        DCHECK(receiver.interface_name().has_value());
        return receiver.interface_name().value();
    }

    static mojo::ScopedMessagePipeHandle receiving_pipe(GenericPendingReceiver& receiver)
    {
        return receiver.PassPipe();
    }

    static bool Read(mojo_base::mojom::GenericPendingReceiverDataView data, GenericPendingReceiver* out);
};
