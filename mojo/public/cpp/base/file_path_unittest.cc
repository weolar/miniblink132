...
mojom::RelativeFilePathPtr CreateArbitraryRelativeFilePath(const base::FilePath& file_path)
{
    auto mojo_file_path = mojom::RelativeFilePath::New();
#if BUILDFLAG(IS_WIN)
    const auto* data_ptr = reinterpret_cast<const uint16_t*>(file_path.value().data());
    mojo_file_path->path = std::vector(data_ptr, data_ptr + file_path.value().size());
#else
...
TEST(FilePathTest, File)
{
    ...
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::FilePath>(file, file_out));
...
TEST(FilePathTest, RelativeFilePath)
{
    ...
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::RelativeFilePath>(file, file_out));
    ASSERT_EQ(file, file_out);
    ...
    base::FilePath ignored_out;
    {
        ...
        EXPECT_CHECK_DEATH(mojo::test::SerializeAndDeserialize<mojom::RelativeFilePath>(in_path, ignored_out));

        auto in_struct = CreateArbitraryRelativeFilePath(in_path);
        EXPECT_FALSE(mojo::test::SerializeAndDeserialize<mojom::RelativeFilePath>(in_struct, ignored_out));
    }
