...
template <> struct COMPONENT_EXPORT(MOJO_BASE_TRAITS) StructTraits<mojo_base::mojom::ProcessIdDataView, base::ProcessId> {
    static uint32_t pid(const base::ProcessId& process_id)
    {
        return static_cast<uint32_t>(process_id);
    }

    static bool Read(mojo_base::mojom::ProcessIdDataView data, base::ProcessId* process_id);
};
