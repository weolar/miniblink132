...
template <> struct COMPONENT_EXPORT(MOJO_BASE_SHARED_TRAITS) StructTraits<mojo_base::mojom::Int128DataView, absl::int128> {
    static int64_t high(const absl::int128& v)
    {
        return absl::Int128High64(v);
    }
    static uint64_t low(const absl::int128& v)
    {
...
template <> struct COMPONENT_EXPORT(MOJO_BASE_SHARED_TRAITS) StructTraits<mojo_base::mojom::Uint128DataView, absl::uint128> {
    static uint64_t high(const absl::uint128& v)
    {
        return absl::Uint128High64(v);
    }
    static uint64_t low(const absl::uint128& v)
    {
