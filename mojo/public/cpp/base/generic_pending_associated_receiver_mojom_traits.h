...
template <>
struct COMPONENT_EXPORT(MOJO_BASE_SHARED_TRAITS) StructTraits<mojo_base::mojom::GenericPendingAssociatedReceiverDataView, GenericPendingAssociatedReceiver> {
    ...
    static base::StringPiece interface_name(const GenericPendingAssociatedReceiver& receiver)
    {
        DCHECK(receiver.interface_name().has_value());
        return receiver.interface_name().value();
    }

    static mojo::ScopedInterfaceEndpointHandle receiver(GenericPendingAssociatedReceiver& receiver)
    {
        return receiver.PassHandle();
    }

    static bool Read(mojo_base::mojom::GenericPendingAssociatedReceiverDataView data, GenericPendingAssociatedReceiver* out);
};
