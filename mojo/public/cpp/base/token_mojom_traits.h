...
template <> struct COMPONENT_EXPORT(MOJO_BASE_SHARED_TRAITS) StructTraits<mojo_base::mojom::TokenDataView, base::Token> {
    static uint64_t high(const base::Token& token)
    {
        return token.high();
    }
    static uint64_t low(const base::Token& token)
    {
        return token.low();
    }
