...
template <> struct MapTraits<base::Value::Dict> {
    using Key = std::string;
    using Value = base::Value;
    using Iterator = base::Value::Dict::const_iterator;

    static size_t GetSize(const base::Value::Dict& in)
    {
        return in.size();
    }

    static Iterator GetBegin(const base::Value::Dict& in)
    {
        return in.cbegin();
    }

    static void AdvanceIterator(Iterator& it)
    {
        ++it;
    }

    static const Key& GetKey(const Iterator& it)
    {
        return it->first;
    }

    static const Value& GetValue(const Iterator& it)
    {
        return it->second;
    }
};

template <> struct COMPONENT_EXPORT(MOJO_BASE_SHARED_TRAITS) StructTraits<mojo_base::mojom::DictionaryValueDataView, base::Value::Dict> {
    static const base::Value::Dict& storage(const base::Value::Dict& in)
    {
        return in;
    }

    static bool Read(mojo_base::mojom::DictionaryValueDataView data, base::Value::Dict* out);
};

template <> struct ArrayTraits<base::Value::List> {
    using Element = base::Value;

    static size_t GetSize(const base::Value::List& in)
    {
        return in.size();
    }
...
template <> struct COMPONENT_EXPORT(MOJO_BASE_SHARED_TRAITS) StructTraits<mojo_base::mojom::ListValueDataView, base::Value::List> {
    static const base::Value::List& storage(const base::Value::List& in)
    {
        return in;
    }

    static bool Read(mojo_base::mojom::ListValueDataView data, base::Value::List* out);
};

template <> struct COMPONENT_EXPORT(MOJO_BASE_SHARED_TRAITS) UnionTraits<mojo_base::mojom::ValueDataView, base::Value> {
    ...
    static uint8_t null_value(const base::Value& value)
    {
        return 0;
    }
    ...
    static bool Read(mojo_base::mojom::ValueDataView view, base::Value* value_out);
};
