...
TEST(MemoryAllocatorDumpCrossProcessUidTest, SerializeFailsOnZeroValue)
{
    ...
    ASSERT_FALSE(mojo::test::SerializeAndDeserialize<mojom::MemoryAllocatorDumpCrossProcessUid>(in, out));
    EXPECT_EQ(in, out);
}
...
TEST(MemoryAllocatorDumpCrossProcessUidTest, SerializeSucceedsOnValidIntValue)
{
    ...
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::MemoryAllocatorDumpCrossProcessUid>(in, out));
    EXPECT_EQ(in, out);
}

TEST(MemoryAllocatorDumpCrossProcessUidTest, SerializeSucceedsOnValidStringValue)
{
    base::trace_event::MemoryAllocatorDumpGuid in("12345");
    base::trace_event::MemoryAllocatorDumpGuid out;
