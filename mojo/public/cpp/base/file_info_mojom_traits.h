...
template <> struct COMPONENT_EXPORT(MOJO_BASE_SHARED_TRAITS) StructTraits<mojo_base::mojom::FileInfoDataView, base::File::Info> {
    static int64_t size(const base::File::Info& info)
    {
        return info.size;
    }
    ...
    static bool Read(mojo_base::mojom::FileInfoDataView data, base::File::Info* out);
};
