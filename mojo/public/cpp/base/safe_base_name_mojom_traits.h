...
template <> struct COMPONENT_EXPORT(MOJO_BASE_SHARED_TRAITS) StructTraits<mojo_base::mojom::SafeBaseNameDataView, base::SafeBaseName> {
    static const base::FilePath& path(const base::SafeBaseName& path)
    {
        return path.path();
    }

    static bool Read(mojo_base::mojom::SafeBaseNameDataView data, base::SafeBaseName* out);
};
