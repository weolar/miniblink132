...
{
    static const auto nt_query_object = reinterpret_cast<decltype(&NtQueryObject)>(GetProcAddress(GetModuleHandle(L"ntdll.dll"), "NtQueryObject"));
    PUBLIC_OBJECT_BASIC_INFORMATION info;
    ULONG len = sizeof(info);
    ULONG consumed = 0;
    auto ret = nt_query_object(handle, ObjectBasicInformation, &info, len, &consumed);
    if (ret)
        return false;
...
bool IsReadOnlyFile(base::File& file)
{
    ...
    is_readonly = !(flags & (FILE_APPEND_DATA | FILE_WRITE_ATTRIBUTES | FILE_WRITE_DATA | FILE_WRITE_EA | WRITE_DAC | WRITE_OWNER | DELETE));
#elif BUILDFLAG(IS_FUCHSIA) || (BUILDFLAG(IS_POSIX) && !BUILDFLAG(IS_NACL) && !BUILDFLAG(IS_AIX))
    is_readonly = (fcntl(file.GetPlatformFile(), F_GETFL) & O_ACCMODE) == O_RDONLY;
#endif
    return is_readonly;
}
...
mojo::PlatformHandle StructTraits<mojo_base::mojom::ReadOnlyFileDataView, base::File>::fd(base::File& file)
{
    DCHECK(file.IsValid());
    ...
    return mojo::PlatformHandle(base::ScopedPlatformFile(file.TakePlatformFile()));
}

bool StructTraits<mojo_base::mojom::ReadOnlyFileDataView, base::File>::Read(mojo_base::mojom::ReadOnlyFileDataView data, base::File* file)
{
