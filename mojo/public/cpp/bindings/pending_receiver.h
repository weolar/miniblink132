...
template <typename T> class PendingRemote;

template <typename T> struct PendingReceiverConverter;
#if ASSOCIATED_AS_ALIAS == 1
template <typename T> struct PendingAssociatedReceiverConverter;
#endif
...
template <typename Interface> class PendingReceiver {
public:
    ...
    {
        if (state_.pipe.is_valid()) {
            MOJO_RECORD_CALL(state_.pipe->value(), __PRETTY_FUNCTION__);
            MojoSetMessagePipeIsRemoteOrReceiver(state_.pipe->value(), false);
        }
    }

    //     explicit PendingReceiver(scoped_refptr<RawPtrMessagePipe> pipe)
    //         : state_(pipe)
    //     {
    //     }
    ...
    template <typename T,
        std::enable_if_t<std::is_same<PendingReceiver<Interface>,
            std::invoke_result_t<decltype(&PendingReceiverConverter<T>::template To<Interface>), T&&>>::value>* = nullptr>
    PendingReceiver(T&& other)
        : PendingReceiver(PendingReceiverConverter<T>::template To<Interface>(std::forward<T>(other)))
    {
    }
#if ASSOCIATED_AS_ALIAS == 1
    template <typename T,
        std::enable_if_t<std::is_same<PendingReceiver<Interface>,
            std::invoke_result_t<decltype(&PendingAssociatedReceiverConverter<T>::template To<Interface>), T&&>>::value>* = nullptr>
    PendingReceiver(T&& other)
        : PendingReceiver(PendingAssociatedReceiverConverter<T>::template To<Interface>(std::move(other)))
    {
    }
#endif
    ...
    bool is_valid() const
    {
        return /*state_.raw_ptr_pipe.get();*/ state_.pipe.is_valid();
    }
    ...
    void ResetWithReason(uint32_t reason, const std::string& description)
    {
        ...
        //         Message message = PipeControlMessageProxy::ConstructPeerEndpointClosedMessage(kPrimaryInterfaceId, DisconnectReason(reason, description));
        //         MojoResult result = WriteMessageNew(state_.pipe.get(), message.TakeMojoMessage(), MOJO_WRITE_MESSAGE_FLAG_NONE);
    ...
    //     void** PassPtrAddr()
    //     {
    //         void** ret = state_.interface_addr;
    //         state_.interface_addr = nullptr;
    //         return ret;
    //     }
    //
    //     Interface** PassPtrAddrT()
    //     {
    //         return (Interface**)PassPtrAddr();
    //     }
    ...
    //     void set_connection_group(ConnectionGroup::Ref ref) { state_.connection_group = std::move(ref); }
    //
    //     const ConnectionGroup::Ref& connection_group() const { return state_.connection_group; }
    //
    //     // Passes ownership of this PendingReceiver's ConnectionGroup Ref, removing it
    //     // from its group.
    //     ConnectionGroup::Ref PassConnectionGroupRef() { return std::move(state_.connection_group); }
    ...
    [[nodiscard]] REINITIALIZES_AFTER_MOVE PendingRemote<Interface> InitWithNewPipeAndPassRemote();

#if ASSOCIATED_AS_ALIAS == 1 // for PendingAssociatedReceiver--
    [[nodiscard]] REINITIALIZES_AFTER_MOVE PendingRemote<Interface> InitWithNewEndpointAndPassRemote()
    {
        return InitWithNewPipeAndPassRemote();
    }
    ScopedInterfaceEndpointHandle PassHandle()
    {
        return PendingReceiver<Interface>::PassPipe();
    }

    void EnableUnassociatedUsage()
    {
        state_.EnableUnassociatedUsage();
    }
#endif //=======

    // For internal Mojo use only.
    internal::PendingReceiverState* internal_state()
    {
        return &state_;
    }
...
template <typename Interface> PendingRemote<Interface> PendingReceiver<Interface>::InitWithNewPipeAndPassRemote()
{
    ...
    MOJO_RECORD_CALL(state_.pipe->value(), __PRETTY_FUNCTION__);
    MojoSetMessagePipeIsRemoteOrReceiver(state_.pipe->value(), false);
    MojoSetMessagePipeIsRemoteOrReceiver(pipe.handle1->value(), true);
    return PendingRemote<Interface>(std::move(pipe.handle1), 0u);

    //     DCHECK(!state_.raw_ptr_pipe);
    //     state_.raw_ptr_pipe = base::MakeRefCounted<RawPtrMessagePipe>();
    //     return PendingRemote<Interface>(state_.raw_ptr_pipe, 0u);
}
