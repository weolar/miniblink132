...
template <typename K, typename V> struct MapTraits<::google::protobuf::Map<K, V>> {
    using Key = K;
    using Value = V;
    ...
    static bool Insert(::google::protobuf::Map<K, V>& input, MaybeConstKeyType&& key, MaybeConstValueType&& value)
    {
        input.insert({ std::forward<MaybeConstKeyType>(key), std::forward<MaybeConstValueType>(value) });
        return true;
    }
