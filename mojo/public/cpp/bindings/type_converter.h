...
template <typename T, typename U> inline T ConvertTo(const U& obj);

// The following specialization is useful when you are converting between
// Array<POD> and std::vector<POD>.
template <typename T> struct TypeConverter<T, T> {
...
template <typename T, typename Container> struct TypeConverter<std::vector<T>, Container> {
