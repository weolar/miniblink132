...
template <typename K, typename V> struct MapTraits<WTF::HashMap<K, V>> {
    using Key = K;
    using Value = V;
    ...
    static Iterator GetBegin(WTF::HashMap<K, V>& input)
    {
        return input.begin();
    }
    ...
    template <typename IK, typename IV> static bool Insert(WTF::HashMap<K, V>& input, IK&& key, IV&& value)
    {
    ...
    static void SetToEmpty(WTF::HashMap<K, V>* output)
    {
        output->clear();
    }
};
