...
template <typename Interface> class SelfOwnedReceiver;

} // namespace internal

template <typename Interface> using SelfOwnedReceiverRef = base::WeakPtr<internal::SelfOwnedReceiver<Interface>>;
...
public:
    ...
    static SelfOwnedReceiverRef<Interface> Create(
        std::unique_ptr<Interface> impl, PendingReceiver<Interface> receiver, scoped_refptr<base::SequencedTaskRunner> task_runner = nullptr)
    {
        SelfOwnedReceiver* self_owned = new SelfOwnedReceiver(std::move(impl), std::move(receiver), std::move(task_runner));
...
private:
    SelfOwnedReceiver(std::unique_ptr<Interface> impl, PendingReceiver<Interface> receiver, scoped_refptr<base::SequencedTaskRunner> task_runner)
        : impl_(std::move(impl))
        , receiver_(impl_.get(), std::move(receiver), std::move(task_runner))
    {
        receiver_.set_disconnect_with_reason_handler(base::BindOnce(&SelfOwnedReceiver::OnDisconnect, base::Unretained(this)));
    }
...
SelfOwnedReceiverRef<Interface> MakeSelfOwnedReceiver(
    std::unique_ptr<Impl> impl, PendingReceiver<Interface> receiver, scoped_refptr<base::SequencedTaskRunner> task_runner = nullptr)
{
    return internal::SelfOwnedReceiver<Interface>::Create(std::move(impl), std::move(receiver), std::move(task_runner));
}
