...
template <typename T> class OptionalAsPointer {
public:
    explicit OptionalAsPointer(T* ptr)
        : value_(ptr)
    {
    }
    ...
    template <typename U>
    OptionalAsPointer(const OptionalAsPointer<U>& other)
        : value_(other.value_)
    {
    }
...
private:
    template <typename U> friend class OptionalAsPointer;
...
template <int&... ExplicitArgumentBarrier, typename T> OptionalAsPointer<T> MakeOptionalAsPointer(T* ptr)
{
