...
template <typename T> class PendingReceiver;

template <typename T> struct PendingRemoteConverter;
...
public:
    ...
    PendingRemote(PendingRemote&& other) noexcept
        : state_(other.PassPipe(), other.state_.version)
    {
    }
        ...
        : state_(std::move(pipe), version)
    {
        if (state_.pipe.is_valid()) {
            ...
            MojoSetMessagePipeIsRemoteOrReceiver(state_.pipe->value(), true);
        }
    }

    //     PendingRemote(scoped_refptr<RawPtrMessagePipe> pipe, uint32_t version)
    //         : state_(std::move(pipe), version)
    //     {
    //     }
    ...
    template <typename T,
        std::enable_if_t<
            std::is_same<PendingRemote<Interface>, std::invoke_result_t<decltype(&PendingRemoteConverter<T>::template To<Interface>), T&&>>::value>* = nullptr>
    PendingRemote(T&& other)
        : PendingRemote(PendingRemoteConverter<T>::template To<Interface>(std::move(other)))
    {
    }

#if ASSOCIATED_AS_ALIAS == 1
    template <typename T,
        std::enable_if_t<std::is_same<PendingRemote<Interface>,
            std::invoke_result_t<decltype(&PendingAssociatedRemoteConverter<T>::template To<Interface>), T&&>>::value>* = nullptr>
    PendingRemote(T&& other)
        : PendingRemote(PendingAssociatedRemoteConverter<T>::template To<Interface>(std::move(other)))
    {
    }
#endif
    ...
    bool is_valid() const
    {
        return state_.pipe.is_valid() /*state_.raw_ptr_pipe.get()*/;
    }
    ...
    //     [[nodiscard]] scoped_refptr<RawPtrMessagePipe> PassPipe()
    //     {
    //         state_.version = 0;
    //         scoped_refptr<RawPtrMessagePipe> ret = state_.raw_ptr_pipe;
    //         state_.raw_ptr_pipe.reset();
    //         return ret;
    //     }
    ...
    const ScopedMessagePipeHandle& Pipe() const
    {
        return state_.pipe;
    }

#if ASSOCIATED_AS_ALIAS == 1
    [[nodiscard]] REINITIALIZES_AFTER_MOVE PendingReceiver<Interface> InitWithNewEndpointAndPassReceiver()
    {
        return InitWithNewPipeAndPassReceiver();
    }
    ...
    [[nodiscard]] REINITIALIZES_AFTER_MOVE PendingReceiver<Interface> InitWithNewPipeAndPassReceiver();

    // For internal Mojo use only.
    internal::PendingRemoteState* internal_state()
    {
        return &state_;
    }
...
// failure.
template <typename Interface> bool FusePipes(PendingReceiver<Interface> receiver, PendingRemote<Interface> remote)
{
...
template <typename Interface> PendingReceiver<Interface> PendingRemote<Interface>::InitWithNewPipeAndPassReceiver()
{
    ...
    MOJO_RECORD_CALL(state_.pipe->value(), __PRETTY_FUNCTION__);
    MojoSetMessagePipeIsRemoteOrReceiver(state_.pipe->value(), true);
    MojoSetMessagePipeIsRemoteOrReceiver(pipe.handle1->value(), false);
    return PendingReceiver<Interface>(std::move(pipe.handle1));

    //     CHECK(!state_.raw_ptr_pipe);
    //     state_.raw_ptr_pipe = base::MakeRefCounted<RawPtrMessagePipe>();
    //     return PendingReceiver<Interface>(state_.raw_ptr_pipe);
}

//----
#if ASSOCIATED_AS_ALIAS == 1
template <typename Interface> using PendingAssociatedRemote = PendingRemote<Interface>;
