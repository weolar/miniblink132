...
template <typename T, typename SFINAE = void> struct HasEqualsMethod : std::false_type {
    static_assert(sizeof(T), "T must be a complete type.");
};

template <typename T> struct HasEqualsMethod<T, std::void_t<decltype(std::declval<const T&>().Equals(std::declval<const T&>()))>> : std::true_type { };

template <typename T> bool Equals(const T& a, const T& b);

template <typename T> struct EqualsTraits {
...
template <typename T> struct EqualsTraits<absl::optional<T>> {
...
template <typename T> struct EqualsTraits<std::vector<T>> {
...
template <typename K, typename V> struct EqualsTraits<base::flat_map<K, V>> {
    static bool Equals(const base::flat_map<K, V>& a, const base::flat_map<K, V>& b)
    {
        if (a.size() != b.size())
            return false;
