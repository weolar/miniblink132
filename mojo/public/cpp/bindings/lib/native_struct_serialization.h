...
struct /*COMPONENT_EXPORT(MOJO_CPP_BINDINGS)*/ UnmappedNativeStructSerializerImpl {
    ...
    static void SerializeMessageContents(IPC::Message* ipc_message, MessageFragment<native::internal::NativeStruct_Data>& fragment);

    static bool DeserializeMessageAttachments(native::internal::NativeStruct_Data* data, Message* message, IPC::Message* ipc_message);
};

template <typename MaybeConstUserType> struct NativeStructSerializerImpl {
    using UserType = typename std::remove_const<MaybeConstUserType>::type;
    using Traits = IPC::ParamTraits<UserType>;

    static void Serialize(MaybeConstUserType& value, MessageFragment<native::internal::NativeStruct_Data>& fragment)
    {
        IPC::Message ipc_message;
        Traits::Write(&ipc_message, value);
        UnmappedNativeStructSerializerImpl::SerializeMessageContents(&ipc_message, fragment);
    }

    static bool Deserialize(native::internal::NativeStruct_Data* data, UserType* out, Message* message)
    {
        if (!data)
            return false;
...

        {
            ...
            IPC::Message message_view(reinterpret_cast<const char*>(header), header->num_bytes + sizeof(ArrayHeader));
            base::PickleIterator iter(message_view);
            if (!UnmappedNativeStructSerializerImpl::DeserializeMessageAttachments(data, message, &message_view)) {
                return false;
            }
...
template <> struct NativeStructSerializerImpl<native::NativeStructPtr> : public UnmappedNativeStructSerializerImpl { };

template <> struct NativeStructSerializerImpl<const native::NativeStructPtr> : public UnmappedNativeStructSerializerImpl { };

template <typename MaybeConstUserType>
struct Serializer<native::NativeStructDataView, MaybeConstUserType> : public NativeStructSerializerImpl<MaybeConstUserType> { };
