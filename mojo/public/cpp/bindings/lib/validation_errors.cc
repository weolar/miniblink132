...
SerializationWarningObserverForTesting* g_serialization_warning_observer = nullptr;
...
std::string MessageHeaderAsHexString(Message* message)
{
    ...
    if (message->data_num_bytes() < sizeof(*message->header())) {
        return base::StrCat({ "<incomplete>", base::HexEncode(message->data(), message->data_num_bytes()) });
    }
...
void ReportValidationError(ValidationContext* context, ValidationError error, const char* description)
{
#if !BUILDFLAG(IS_NACL)
    SCOPED_CRASH_KEY_STRING64("mojo-message", "header-bytes", MessageHeaderAsHexString(context->message()));
#endif // !BUILDFLAG(IS_NACL)
    ...
    if (description) {
        if (!g_suppress_logging) {
            LOG(ERROR) << "Invalid message: " << ValidationErrorToString(error) << " (" << description << ")";
        }
        if (context->message()) {
            context->message()->NotifyBadMessage(
                base::StringPrintf("Validation failed for %s [%s (%s)]", context->GetFullDescription().c_str(), ValidationErrorToString(error), description));
        }
    } else {
        if (!g_suppress_logging)
        ...
        if (context->message()) {
            context->message()->NotifyBadMessage(
                base::StringPrintf("Validation failed for %s [%s]", context->GetFullDescription().c_str(), ValidationErrorToString(error)));
        }
    }
}
...
{
    std::string description = base::StringPrintf("%s.%d %s", interface_name, method_ordinal, is_response ? " response" : "");
    ValidationContext validation_context(nullptr, 0, 0, 0, message, description.c_str());

    std::string output = "ReportValidationErrorForMessage:" + description;
    output += "\n";
...
ValidationErrorObserverForTesting::ValidationErrorObserverForTesting(base::RepeatingClosure callback)
