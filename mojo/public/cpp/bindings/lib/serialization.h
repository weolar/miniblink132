...
template <typename MojomType, typename EnableType = void> struct MojomSerializationImplTraits;

template <typename MojomType>
struct MojomSerializationImplTraits<MojomType, typename std::enable_if<BelongsTo<MojomType, MojomTypeCategory::kStruct>::value>::type> {
    template <typename MaybeConstUserType, typename FragmentType> static void Serialize(MaybeConstUserType& input, FragmentType& fragment)
    {
...
template <typename MojomType>
struct MojomSerializationImplTraits<MojomType, typename std::enable_if<BelongsTo<MojomType, MojomTypeCategory::kUnion>::value>::type> {
    template <typename MaybeConstUserType, typename FragmentType> static void Serialize(MaybeConstUserType& input, FragmentType& fragment)
    {
...
template <typename MojomType, typename UserType> mojo::Message SerializeAsMessageImpl(UserType* input)
{
    ...
    mojo::Message message(0, 0, 0, 0, MOJO_CREATE_MESSAGE_FLAG_UNLIMITED_SIZE, nullptr);
...
template <typename MojomType, typename DataArrayType, typename UserType> DataArrayType SerializeImpl(UserType* input)
{
    static_assert(BelongsTo<MojomType, MojomTypeCategory::kStruct>::value || BelongsTo<MojomType, MojomTypeCategory::kUnion>::value, "Unexpected type.");
...
bool DeserializeImpl(Message& message, const void* data, size_t data_num_bytes, UserType* output, bool (*validate_func)(const void*, ValidationContext*))
{
    static_assert(BelongsTo<MojomType, MojomTypeCategory::kStruct>::value || BelongsTo<MojomType, MojomTypeCategory::kUnion>::value, "Unexpected type.");
    ...
    ValidationContext validation_context(input_buffer, static_cast<uint32_t>(data_num_bytes), message.handles()->size(), 0);
    bool result = false;
    if (validate_func(input_buffer, &validation_context)) {
        result = Deserialize<MojomType>(reinterpret_cast<DataType*>(const_cast<void*>(input_buffer)), output, &message);
    }
