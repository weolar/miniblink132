...
template <typename T> size_t WTFHashCombine(size_t seed, const T& value)
{
...
template <typename T, bool has_hash_method = HasHashMethod<T>::value> struct WTFHashTraits;

template <typename T> size_t WTFHash(size_t seed, const T& value);

template <typename T> struct WTFHashTraits<T, true> {
...
template <typename T> struct WTFHashTraits<T, false> {
    static size_t Hash(size_t seed, const T& value)
    {
...
template <> struct WTFHashTraits<WTF::String, false> {
...
template <typename T> struct StructPtrHashFn {
...
template <typename T> struct InlinedStructPtrHashFn {
    static unsigned GetHash(const InlinedStructPtr<T>& value)
    {
        return static_cast<unsigned>(value.Hash(kHashSeed));
    }
    static bool Equal(const InlinedStructPtr<T>& left, const InlinedStructPtr<T>& right)
    {
        return left.Equals(right);
    }
    static const bool safe_to_compare_to_empty_or_deleted = false;
...
// template <typename T> struct DefaultHash<mojo::StructPtr<T>> {
//     using Hash = mojo::internal::StructPtrHashFn<T>;
// };

template <typename T> struct HashTraits<mojo::StructPtr<T>> : public GenericHashTraits<mojo::StructPtr<T>> {
    ...
    {
        return mojo::internal::StructPtrWTFHelper<T>::IsHashTableDeletedValue(value);
    }
};

// template <typename T> struct DefaultHash<mojo::InlinedStructPtr<T>> {
//     using Hash = mojo::internal::InlinedStructPtrHashFn<T>;
// };

template <typename T> struct HashTraits<mojo::InlinedStructPtr<T>> : public GenericHashTraits<mojo::InlinedStructPtr<T>> {
    static const bool kHasIsEmptyValueFunction = true;
    ...
    {
        return mojo::internal::InlinedStructPtrWTFHelper<T>::IsHashTableDeletedValue(value);
    }
};
