...
struct ArraySerializer<MojomType,
        ...
        const ContainerValidateParams* validate_params)
    {
        ...
        for (size_t i = 0; i < size; ++i) {
            ...
            decltype(auto) next = input->GetNext();
            SerializeCaller<Element>::Run(next, data_fragment, validate_params->element_validate_params);
            fragment->at(i).Set(data_fragment.is_null() ? nullptr : data_fragment.data());
            MOJO_INTERNAL_DLOG_SERIALIZATION_WARNING(
                !validate_params->element_is_nullable && data_fragment.is_null(),
                VALIDATION_ERROR_UNEXPECTED_NULL_POINTER,
                MakeMessageWithArrayIndex("null in array expecting valid pointers", size, i));
        }
    }
    static bool DeserializeElements(Data* input,
        UserType* output,
        Message* message) 
    {
        ...
        ArrayIterator<Traits, UserType> iterator(*output);
        for (size_t i = 0; i < input->size(); ++i) {
            if (!Deserialize<Element>(input->at(i).Get(), &iterator.GetNext(), message))
                return false;
        }
        return true;
    }
...
private:
    ...
    struct SerializeCaller<T, true> {
        ...
        static void Run(InputElementType&& input,
            ...
            const ContainerValidateParams* validate_params) 
        {
            Serialize<T>(std::forward<InputElementType>(input), fragment,                validate_params);
        }
    };
};
