...
public:
    ...
    void Start()
    {
        runner_for_pauses_->PostTask(FROM_HERE, base::BindOnce(&RandomMojoDelays::PauseRandomBindingStateBases, base::Unretained(this)));
    }
    ...
    void AddBindingStateBase(scoped_refptr<base::SequencedTaskRunner> runner, base::WeakPtr<BindingStateBase> binding_state_base)
    {
        runner_for_pauses_->PostTask(FROM_HERE,
            base::BindOnce(&RandomMojoDelays::AddBindingStateBaseInternal, base::Unretained(this), std::move(runner), std::move(binding_state_base)));
    }
...
private:
    ...
    void AddBindingStateBaseInternal(scoped_refptr<base::SequencedTaskRunner> runner, base::WeakPtr<BindingStateBase> binding_state_base)
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(runner_for_pauses_sequence_checker);

        binding_state_base_map_[std::move(runner)].push_back(std::move(binding_state_base));
    }
    ...
    void AddBindingStateBaseList(scoped_refptr<base::SequencedTaskRunner> runner, BindingList binding_state_bases)
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(runner_for_pauses_sequence_checker);
    ...
    void ResumeFrozenBindingStateBasesOnTaskRunner(BindingList binding_state_bases, BindingList paused_binding_state_bases)
    {
        ...
        if (!paused_binding_state_bases.empty()) {
            ...
            base::SequencedTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE,
                base::BindOnce(&RandomMojoDelays::ResumeFrozenBindingStateBasesOnTaskRunner, base::Unretained(this), std::move(binding_state_bases),
        ...
        runner_for_pauses_->PostTask(FROM_HERE,
            base::BindOnce(
                &RandomMojoDelays::AddBindingStateBaseList, base::Unretained(this), base::SequencedTaskRunnerHandle::Get(), std::move(binding_state_bases)));
    }

    // Pause a random selection of bindings in the list |binding_state_bases|,
    // and set them to resume in the future.
    void PauseRandomBindingStateBasesOnTaskRunner(BindingList binding_state_bases)
    {
        ...
        if (paused_binding_state_bases.size() > 0) {
            ...
            runner_for_pauses_->PostTask(FROM_HERE,
                base::BindOnce(&RandomMojoDelays::AddBindingStateBaseList, base::Unretained(this), base::SequencedTaskRunnerHandle::Get(),
    ...
    void PauseRandomBindingStateBases()
    {
        ...
        while (map_it != binding_state_base_map_.end()) {
            ...
            map_it->first->PostTask(
                FROM_HERE, base::BindOnce(&RandomMojoDelays::PauseRandomBindingStateBasesOnTaskRunner, base::Unretained(this), std::move(map_it->second)));
        ...
        base::SequencedTaskRunnerHandle::Get()->PostDelayedTask(
            FROM_HERE, base::BindOnce(&RandomMojoDelays::PauseRandomBindingStateBases, base::Unretained(this)), kPauseBindingsFrequency);
    }

    scoped_refptr<base::SequencedTaskRunner> runner_for_pauses_;
    std::map<scoped_refptr<base::SequencedTaskRunner>, BindingList> binding_state_base_map_;
...
void MakeBindingRandomlyPaused(scoped_refptr<base::SequencedTaskRunner> runner, base::WeakPtr<BindingStateBase> binding_state_base)
{
    GetRandomMojoDelays().AddBindingStateBase(std::move(runner), binding_state_base);
}
