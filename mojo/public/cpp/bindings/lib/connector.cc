...
Connector::OutgoingSerializationMode g_default_outgoing_serialization_mode = Connector::OutgoingSerializationMode::kLazy;

// The default incoming serialization mode for new Connectors.
Connector::IncomingSerializationMode g_default_incoming_serialization_mode = Connector::IncomingSerializationMode::kDispatchAsIs;
...
bool EnableTaskPerMessage()
{
    ...
    static const bool enable = base::FeatureList::IsEnabled(features::kTaskPerMessage);
    return enable;
}
...
class Connector::RunLoopNestingObserver : public base::RunLoop::NestingObserver {
public:
    ...
    static RunLoopNestingObserver* GetForThread()
    {
        ...
        static base::SequenceLocalStorageSlot<RunLoopNestingObserver> sls_nesting_observer;
...
Connector::ActiveDispatchTracker::ActiveDispatchTracker(const base::WeakPtr<Connector>& connector)
    : connector_(connector)
    , nesting_observer_(connector_->nesting_observer_)
{
...
Connector::Connector(ScopedMessagePipeHandle message_pipe, ConnectorConfig config, const char* interface_name)
    ...
    , header_validator_(base::JoinString({ interface_name ? interface_name : "Generic", "MessageHeaderValidator" }, ""))
{
...
Connector::Connector(ScopedMessagePipeHandle message_pipe, ConnectorConfig config, scoped_refptr<base::SequencedTaskRunner> runner, const char* interface_name)
...
Connector::~Connector()
{
    if (quota_checker_) {
        ...
        UMA_HISTOGRAM_COUNTS_1M("Mojo.Connector.MaxUnreadMessageQuotaUsed", quota_checker_->GetMaxQuotaUsage());
    }
...
void Connector::StartReceiving(scoped_refptr<base::SequencedTaskRunner> task_runner, bool allow_woken_up_by_others)
{
    DCHECK(!task_runner_);
    ...
    if (task_runner_->RunsTasksInCurrentSequence()) {
        ...
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&Connector::WaitToReadMore, weak_factory_.GetWeakPtr()));
    }
}
...
bool Connector::WaitForIncomingMessage()
{
    ...
    if (rv != MOJO_RESULT_OK) {
        ...
        HandleError(rv != MOJO_RESULT_FAILED_PRECONDITION /* force_pipe_reset */, false /* force_async_handler */);
        return false;
    }
...
bool Connector::Accept(Message* message)
{
    ...
    if (!message->is_serialized()) {
        ...
        if (!peer_remoteness_tracker_ && task_runner_) {
            peer_remoteness_tracker_.emplace(message_pipe_.get(), MOJO_HANDLE_SIGNAL_PEER_REMOTE, task_runner_);
        }
    }
    ...
    MojoResult rv = WriteMessageNew(message_pipe_.get(), message->TakeMojoMessage(), MOJO_WRITE_MESSAGE_FLAG_NONE);
...
void Connector::SetMessageQuotaChecker(scoped_refptr<internal::MessageQuotaChecker> checker)
{
...
// static
void Connector::OverrideDefaultSerializationBehaviorForTesting(OutgoingSerializationMode outgoing_mode, IncomingSerializationMode incoming_mode)
{
...
void Connector::WaitToReadMore()
{
    ...
    handle_watcher_ = std::make_unique<SimpleWatcher>(FROM_HERE, SimpleWatcher::ArmingPolicy::MANUAL, task_runner_, interface_name_);
    MojoResult rv = handle_watcher_->Watch(
        message_pipe_.get(), MOJO_HANDLE_SIGNAL_READABLE, base::BindRepeating(&Connector::OnWatcherHandleReady, base::Unretained(this)));
    ...
    if (rv != MOJO_RESULT_OK) {
        ...
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&Connector::OnWatcherHandleReady, weak_self_, rv));
    } else {
...
uint64_t Connector::QueryPendingMessageCount() const
{
    ...
    MojoQueryQuota(message_pipe_.get().value(), MOJO_QUOTA_TYPE_RECEIVE_QUEUE_LENGTH,
...
{
    return ReadMessageNew(message_pipe_.get(), &message, MOJO_READ_MESSAGE_FLAG_NONE);
}
...
bool Connector::DispatchMessage(ScopedMessageHandle handle)
{
    ...
    if (message.IsNull()) {
        ...
        NotifyBadMessage(handle.get(), base::StrCat({ interface_name_, " One or more handle attachments were invalid." }));
    ...
    if (incoming_serialization_mode_ == IncomingSerializationMode::kSerializeBeforeDispatchForTesting) {
        message.SerializeIfNecessary();
    } else {
        DCHECK_EQ(IncomingSerializationMode::kDispatchAsIs, incoming_serialization_mode_);
    }
    ...
    TRACE_EVENT(TRACE_DISABLED_BY_DEFAULT("mojom"), "Connector::DispatchMessage", [&](perfetto::EventContext& ctx) {
        ctx.event()->set_chrome_mojo_event_info()->set_mojo_interface_tag(interface_name_);
    ...
    bool receiver_result = incoming_receiver_ && incoming_receiver_->Accept(&message);
    if (!weak_self)
...
void Connector::PostDispatchNextMessageFromPipe()
{
    ...
    task_runner_->PostTask(FROM_HERE, base::BindOnce(&Connector::CallDispatchNextMessageFromPipe, weak_self_));
}
...
void Connector::ScheduleDispatchOfPendingMessagesOrWaitForMore(uint64_t pending_message_count)
{
...
void Connector::EnsureSyncWatcherExists()
{
    ...
    sync_watcher_ = std::make_unique<SyncHandleWatcher>(
        message_pipe_.get(), MOJO_HANDLE_SIGNAL_READABLE, base::BindRepeating(&Connector::OnSyncHandleWatcherHandleReady, base::Unretained(this)));
}
