...
SyncHandleWatcher::SyncHandleWatcher(const Handle& handle, MojoHandleSignals handle_signals, const SyncHandleRegistry::HandleCallback& callback)
    : handle_(handle)
...
void SyncHandleWatcher::IncrementRegisterCount()
{
    ...
    if (!registered_) {
        registered_ = registry_->RegisterHandle(handle_, handle_signals_, callback_);
    }
}
