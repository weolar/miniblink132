...
template <typename MaybeConstUserType> struct NativeEnumSerializerImpl {
    ...
    static_assert(sizeof(NativeEnum) >= sizeof(int), "Cannot store the serialization result in NativeEnum.");
    ...
    static void Serialize(UserType input, int32_t* output)
    {
        ...
        *output = 0;
        memcpy(reinterpret_cast<char*>(output), pickle.payload_bytes().data(), pickle.payload_bytes().size());
    }
    ...
    static bool Deserialize(int32_t input, UserType* output)
    {
        ...
        base::Pickle pickle_view(reinterpret_cast<const char*>(&data), sizeof(PickleData));
...
template <> struct NativeEnumSerializerImpl<NativeEnum> : public UnmappedNativeEnumSerializerImpl { };

template <> struct NativeEnumSerializerImpl<const NativeEnum> : public UnmappedNativeEnumSerializerImpl { };

template <typename MaybeConstUserType> struct Serializer<NativeEnum, MaybeConstUserType> : public NativeEnumSerializerImpl<MaybeConstUserType> { };
