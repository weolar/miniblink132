...
bool IsValidMessageHeader(const internal::MessageHeader* header, internal::ValidationContext* validation_context)
{
    ...
    do {
        ...
        internal::ReportValidationError(validation_context, internal::VALIDATION_ERROR_UNEXPECTED_STRUCT_HEADER);
        return false;
    } while (false);
    ...
    constexpr uint32_t kRequestIdFlags = Message::kFlagExpectsResponse | Message::kFlagIsResponse;
    if (header->version == 0 && (header->flags & kRequestIdFlags)) {
        internal::ReportValidationError(validation_context, internal::VALIDATION_ERROR_MESSAGE_HEADER_MISSING_REQUEST_ID);
        return false;
    }

    // These flags are mutually exclusive.
    if ((header->flags & kRequestIdFlags) == kRequestIdFlags) {
        internal::ReportValidationError(validation_context, internal::VALIDATION_ERROR_MESSAGE_HEADER_INVALID_FLAGS);
        return false;
    }
    ...
    //   payload type.
    if (!internal::ValidatePointerNonNullable(header_v2->payload, 5, validation_context) || !internal::ValidatePointer(header_v2->payload, validation_context)
    ...
    if (!internal::ValidateContainer(header_v2->payload_interface_ids, validation_context, &validate_params)) {
        return false;
    }
    ...
    if (!header_v2->payload_interface_ids.is_null()) {
        ...
        for (size_t i = 0; i < num_ids; ++i) {
            if (!IsValidInterfaceId(ids[i]) || IsPrimaryInterfaceId(ids[i])) {
                internal::ReportValidationError(validation_context, internal::VALIDATION_ERROR_ILLEGAL_INTERFACE_ID);
                return false;
            }
        }
    }
...
bool MessageHeaderValidator::Accept(Message* message)
{
    ...
    internal::ValidationContext validation_context(message->data(), message->data_num_bytes(), 0, 0, message, description_.c_str());

    if (!internal::ValidateStructHeaderAndClaimMemory(message->data(), &validation_context))
        return false;
