...
public:
    ...
    public:
        ...
        // |when| falls into.
        static base::TimeTicks GetNextSamplingIntervalForTesting(base::TimeTicks when);
    ...
    MessageQuotaChecker(const Configuration* config, base::PassKey<MessageQuotaChecker>);
    ...
    static scoped_refptr<MessageQuotaChecker> MaybeCreateForTesting(const Configuration& config);

private:
    friend class base::RefCountedThreadSafe<MessageQuotaChecker>;
    ~MessageQuotaChecker();
    static Configuration GetConfiguration();
    static scoped_refptr<MessageQuotaChecker> MaybeCreateImpl(const Configuration& config);
...
struct MessageQuotaChecker::Configuration {
    ...
    void (*maybe_crash_function)(size_t quota_used, absl::optional<size_t> message_pipe_quota_used, int64_t seconds_since_construction,
