...
NOINLINE void MaybeDumpWithoutCrashing(size_t total_quota_used, absl::optional<size_t> message_pipe_quota_used, int64_t seconds_since_construction,
    double average_write_rate, uint64_t messages_enqueued, uint64_t messages_dequeued, uint64_t messages_written)
{
    ...
    double average_write_rate_per_second = average_write_rate / MessageQuotaChecker::DecayingRateAverage::kSamplingInterval.InSecondsF();
    ...
    user_data.SetUint("average_write_rate_per_second", static_cast<uint64_t>(average_write_rate_per_second));
...
{
    return (when - base::TimeTicks::UnixEpoch()).IntDiv(MessageQuotaChecker::DecayingRateAverage::kSamplingInterval);
}

base::TimeTicks FromSamplingInterval(int64_t sampling_interval)
{
    return MessageQuotaChecker::DecayingRateAverage::kSamplingInterval * sampling_interval + base::TimeTicks::UnixEpoch();
}

} // namespace

constexpr base::TimeDelta MessageQuotaChecker::DecayingRateAverage::kSamplingInterval;
...
scoped_refptr<MessageQuotaChecker> MessageQuotaChecker::MaybeCreate()
{
    ...
    do {
        flag = _InterlockedCompareExchange((long volatile*)&(run_once), 2, 0);
        if (0 == flag) {
            ...
            MessageBoxA(0, "MessageQuotaChecker fail 1", 0, 0);
        }
    } while (true);

    if (!config)
        MessageBoxA(0, "MessageQuotaChecker fail 2", 0, 0);
...
void MessageQuotaChecker::SetMessagePipe(MessagePipeHandle message_pipe)
{
    ...
    MojoResult rv = MojoSetQuota(message_pipe.value(), MOJO_QUOTA_TYPE_UNREAD_MESSAGE_COUNT, config_->unread_message_count_quota, nullptr);
...
// static
MessageQuotaChecker::Configuration MessageQuotaChecker::GetConfigurationForTesting()
{
    return GetConfiguration();
}

// static
scoped_refptr<MessageQuotaChecker> MessageQuotaChecker::MaybeCreateForTesting(const Configuration& config)
{
    return MaybeCreateImpl(config);
}

MessageQuotaChecker::MessageQuotaChecker(const Configuration* config, base::PassKey<MessageQuotaChecker>)
    : config_(config)
...
MessageQuotaChecker::Configuration MessageQuotaChecker::GetConfiguration()
{
    ...
    ret.is_enabled = base::FeatureList::IsEnabled(features::kMojoRecordUnreadMessageCount);
    ...
    ret.unread_message_count_quota = std::max(kMinQuotaValue, kMojoRecordUnreadMessageCountQuotaValue.Get());
...
// static
scoped_refptr<MessageQuotaChecker> MessageQuotaChecker::MaybeCreateImpl(const Configuration& config)
{
    ...
    return base::MakeRefCounted<MessageQuotaChecker>(&config, base::PassKey<MessageQuotaChecker>());
}
...
absl::optional<size_t> MessageQuotaChecker::GetCurrentMessagePipeQuota()
{
    ...
    uint64_t limit = 0;
    uint64_t usage = 0;
    MojoResult rv = MojoQueryQuota(message_pipe_.value(), MOJO_QUOTA_TYPE_UNREAD_MESSAGE_COUNT, nullptr, &limit, &usage);
...
void MessageQuotaChecker::QuotaCheckImpl(size_t num_enqueued)
{
    ...
    if (new_max && config_->crash_threshold != 0 && total_quota_used >= config_->crash_threshold) {
        DCHECK(!now.is_null());
        int64_t seconds_since_construction = (now - creation_time_).InSeconds();
        config_->maybe_crash_function(total_quota_used, message_pipe_quota_used, seconds_since_construction, average_write_rate, messages_enqueued_.load(),
...
void MessageQuotaChecker::DecayingRateAverage::AccrueEvent(base::TimeTicks when)
{
    ...
    const int64_t decayed_average_age = sampling_interval - events_sampling_interval_ - 1;
...
double MessageQuotaChecker::DecayingRateAverage::GetDecayedRateAverage(base::TimeTicks when) const
{
    // Three cases:
    ...
    const int64_t sampling_interval = ToSamplingInterval(when);
    double age_in_sampling_intervals = (when - FromSamplingInterval(events_sampling_interval_)) / kSamplingInterval;
...
// static
base::TimeTicks MessageQuotaChecker::DecayingRateAverage::GetNextSamplingIntervalForTesting(base::TimeTicks when)
{
