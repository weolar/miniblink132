...
ValidationContext::ValidationContext(const void* data, size_t data_num_bytes, size_t num_handles, size_t num_associated_endpoint_handles, Message* message,
    const char* description, int stack_depth, ValidatorType validator_type)
    : message_(message)
...
ValidationContext::ValidationContext(Message* message, const char* description, ValidatorType validator_type)
    : ValidationContext(message->payload(), message->payload_num_bytes(), message->handles()->size(), message->payload_num_interface_ids(), message,
        description, 0, validator_type)
{
}
