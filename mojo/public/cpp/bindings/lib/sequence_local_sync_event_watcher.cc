...
using WatcherStateMap = std::map<const SequenceLocalSyncEventWatcher*, scoped_refptr<WatcherState>>;
...
public:
    SequenceLocalState()
        ...
        , event_watcher_(&event_, base::BindRepeating(&SequenceLocalState::OnEventSignaled, base::Unretained(this)))
    {
    ...
    WatcherStateMap::iterator RegisterWatcher(const SequenceLocalSyncEventWatcher* watcher)
    {
        auto result = registered_watchers_.emplace(watcher, base::MakeRefCounted<WatcherState>());
    ...
    bool SyncWatch(const SequenceLocalSyncEventWatcher* watcher, WatcherState* watcher_state, const bool* should_stop)
    {
        ...
        const bool* stop_flags[] = { should_stop, &top_watcher_state_->watcher_was_destroyed };
...
public:
    ...
    SequenceLocalState* shared_state() const
    {
        return shared_state_;
    }
    WatcherState* watcher_state()
    {
        return watcher_state_.get();
    }
...
SequenceLocalSyncEventWatcher::SequenceLocalSyncEventWatcher(const base::RepeatingClosure& callback)
...
bool SequenceLocalSyncEventWatcher::SyncWatch(const bool* should_stop)
{
    ...
    return registration_->shared_state()->SyncWatch(this, registration_->watcher_state(), should_stop);
}
