...
// within the Message.
template <typename T> class MessageFragment {
public:
    // Constructs a null MessageFragment for `message`.
    explicit MessageFragment(Message& message)
        : message_(message)
    {
    }
    ...
    bool is_null() const
    {
        return index_ == kInvalidFragmentIndex;
    }
    ...
    void Claim(void* ptr)
    {
        const char* start = static_cast<const char*>(message_.payload_buffer()->data());
...
template <typename ElementType> struct MessageFragmentArrayTraits {
    static constexpr uint32_t kMaxNumElements = (std::numeric_limits<uint32_t>::max() - sizeof(ArrayHeader)) / sizeof(ElementType);
...
template <> struct MessageFragmentArrayTraits<bool> {
    static constexpr uint32_t kMaxNumElements = std::numeric_limits<uint32_t>::max() - 7;
    static uint32_t GetStorageSize(uint32_t num_elements)
    {
        DCHECK_LE(num_elements, kMaxNumElements);
...
template <typename T> class MessageFragment<Array_Data<T>> {
public:
    ...
    {
        static_assert(std::numeric_limits<uint32_t>::max() > Traits::kMaxNumElements, "Max num elements castable to 32bit");
        CHECK_LE(num_elements, Traits::kMaxNumElements);

        const uint32_t num_bytes = Traits::GetStorageSize(static_cast<uint32_t>(num_elements));
