...
SyncHandleRegistry::Subscription::Subscription(base::OnceClosure remove_closure, EventCallbackList* callbacks, EventCallback event_callback)
...
SyncHandleRegistry::Subscription& SyncHandleRegistry::Subscription::operator=(Subscription&&) = default;
...
{
    static base::SequenceLocalStorageSlot<scoped_refptr<SyncHandleRegistry>> g_current_sync_handle_watcher;
    ...
    if (!base::SequencedTaskRunnerHandle::IsSet()) {
        return base::MakeRefCounted<SyncHandleRegistry>(base::PassKey<SyncHandleRegistry>());
    }

    if (!g_current_sync_handle_watcher) {
        g_current_sync_handle_watcher.emplace(base::MakeRefCounted<SyncHandleRegistry>(base::PassKey<SyncHandleRegistry>()));
    }
...
bool SyncHandleRegistry::RegisterHandle(const Handle& handle, MojoHandleSignals handle_signals, HandleCallback callback)
{
...
SyncHandleRegistry::EventCallbackSubscription SyncHandleRegistry::RegisterEvent(base::WaitableEvent* event, EventCallback callback)
{
    ...
    const auto remove_closure = [](EventCallbackList* callbacks, WaitSet* wait_set, base::WaitableEvent* event) {
    ...
    return std::make_unique<Subscription>(base::BindOnce(remove_closure, it->second.get(), &wait_set_, event), it->second.get(), std::move(callback));
}
...
bool SyncHandleRegistry::Wait(const bool* should_stop[], size_t count)
{
    ...
    while (true) {
        ...
        num_ready_handles = 1;
        wait_set_.Wait(&ready_event, &num_ready_handles, &ready_handle, &ready_handle_result);
        ...
        if (ready_event) {
            ...
            if (!in_nested_wait_) {
                base::EraseIf(events_, [](const auto& entry) { return entry.second->empty(); });
            }
        }
    }
}
