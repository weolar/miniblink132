...
COMPONENT_EXPORT(MOJO_CPP_BINDINGS_BASE)
void ReportValidationError(ValidationContext* context, ValidationError error, const char* description = nullptr);

COMPONENT_EXPORT(MOJO_CPP_BINDINGS_BASE)
void ReportValidationErrorForMessage(mojo::Message* message, ValidationError error, const char* interface_name, unsigned int method_ordinal, bool is_response);
...
class COMPONENT_EXPORT(MOJO_CPP_BINDINGS_BASE) ScopedSuppressValidationErrorLoggingForTests {
public:
    ScopedSuppressValidationErrorLoggingForTests();

    ScopedSuppressValidationErrorLoggingForTests(const ScopedSuppressValidationErrorLoggingForTests&) = delete;
    ScopedSuppressValidationErrorLoggingForTests& operator=(const ScopedSuppressValidationErrorLoggingForTests&) = delete;
...
// validation.
class COMPONENT_EXPORT(MOJO_CPP_BINDINGS_BASE) ValidationErrorObserverForTesting {
public:
    ...
    ValidationError last_error() const
    {
        return last_error_;
    }
...
class COMPONENT_EXPORT(MOJO_CPP_BINDINGS_BASE) SerializationWarningObserverForTesting {
public:
    SerializationWarningObserverForTesting();

    SerializationWarningObserverForTesting(const SerializationWarningObserverForTesting&) = delete;
    SerializationWarningObserverForTesting& operator=(const SerializationWarningObserverForTesting&) = delete;

    ~SerializationWarningObserverForTesting();

    ValidationError last_warning() const
    {
        return last_warning_;
    }
    void set_last_warning(ValidationError error)
    {
        last_warning_ = error;
    }
...
#define MOJO_INTERNAL_DLOG_SERIALIZATION_WARNING(condition, error, description)                                                                                \
    DLOG_IF(FATAL, (condition) && !ReportSerializationWarning(error))                                                                                          \
        << "The outgoing message will trigger " << ValidationErrorToString(error) << " at the receiving side (" << description << ")."
