...
// and the values.
template <typename Key, typename Value> class Map_Data {
public:
    ...
    static bool Validate(const void* data, ValidationContext* validation_context, const ContainerValidateParams* validate_params)
    {
        ...
        if (object->header_.num_bytes != sizeof(Map_Data) || object->header_.version != 0) {
        ...
        if (!ValidatePointerNonNullable(object->keys, 0, validation_context)
            || !ValidateContainer(object->keys, validation_context, validate_params->key_validate_params)) {
            return false;
        }

        if (!ValidatePointerNonNullable(object->values, 1, validation_context)
            || !ValidateContainer(object->values, validation_context, validate_params->element_validate_params)) {
            return false;
        }

        if (object->keys.Get()->size() != object->values.Get()->size()) {
            ReportValidationError(validation_context, VALIDATION_ERROR_DIFFERENT_SIZED_ARRAYS_IN_MAP);
            return false;
        }
