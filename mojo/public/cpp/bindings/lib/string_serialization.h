...
template <typename MaybeConstUserType> struct Serializer<StringDataView, MaybeConstUserType> {
    using UserType = typename std::remove_const<MaybeConstUserType>::type;
    using Traits = StringTraits<UserType>;

    static void Serialize(MaybeConstUserType& input, MessageFragment<String_Data>& fragment)
    {
    ...
    static bool Deserialize(String_Data* input, UserType* output, Message* message)
    {
        if (!input)
