...
bool ValidateRequestGeneric(Message* message, const char* class_name, base::span<const std::pair<uint32_t, GenericValidationInfo>>);

// As above, but assumes that the ordinals (names) are packed such that a
// constant-time indexed table access is sufficient.
COMPONENT_EXPORT(MOJO_CPP_BINDINGS)
bool ValidateRequestGenericPacked(Message* message, const char* class_name, base::span<const GenericValidationInfo>);
...
// constant-time indexed table access is sufficient.
COMPONENT_EXPORT(MOJO_CPP_BINDINGS)
bool ValidateResponseGenericPacked(Message* message, const char* class_name, base::span<const GenericValidationInfo>);
