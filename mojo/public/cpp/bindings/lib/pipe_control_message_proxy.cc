...
Message ConstructRunOrClosePipeMessage(pipe_control::RunOrClosePipeInputPtr input_ptr)
{
    ...
    internal::MessageFragment<pipe_control::internal::RunOrClosePipeMessageParams_Data> fragment(message);
    internal::Serialize<pipe_control::RunOrClosePipeMessageParamsDataView>(params_ptr, fragment);
...
void PipeControlMessageProxy::NotifyPeerEndpointClosed(InterfaceId id, const absl::optional<DisconnectReason>& reason)
{
...
{
    auto input = pipe_control::RunOrClosePipeInput::NewPauseUntilFlushCompletes(pipe_control::PauseUntilFlushCompletes::New(flush.PassPipe()));
...
{
    auto input = pipe_control::RunOrClosePipeInput::NewFlushAsync(pipe_control::FlushAsync::New(flusher.PassPipe()));
    Message message(ConstructRunOrClosePipeMessage(std::move(input)));
    std::ignore = receiver_->Accept(&message);
}

// static
Message PipeControlMessageProxy::ConstructPeerEndpointClosedMessage(InterfaceId id, const absl::optional<DisconnectReason>& reason)
{
    ...
    auto input = pipe_control::RunOrClosePipeInput::NewPeerAssociatedEndpointClosedEvent(std::move(event));
