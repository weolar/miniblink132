...
void BindingStateBase::PauseRemoteCallbacksUntilFlushCompletes(PendingFlush flush)
{
...
void BindingStateBase::CloseWithReason(uint32_t custom_reason, base::StringPiece description)
{
    if (endpoint_client_)
...
ReportBadMessageCallback BindingStateBase::GetBadMessageCallback()
{
    return base::BindOnce(
        [](ReportBadMessageCallback inner_callback, base::WeakPtr<BindingStateBase> binding, base::StringPiece error) {
...
void BindingStateBase::BindInternal(PendingReceiverState* receiver_state, scoped_refptr<base::SequencedTaskRunner> runner, const char* interface_name,
...
{
    DCHECK(!is_bound()) << "Attempting to bind interface that is already bound: " << interface_name;

    auto sequenced_runner = GetTaskRunnerToUseFromUserProvidedTaskRunner(std::move(runner));

    MultiplexRouter::Config config = passes_associated_kinds
        ? MultiplexRouter::MULTI_INTERFACE
        : (!sync_method_ordinals.empty() ? MultiplexRouter::SINGLE_INTERFACE_WITH_SYNC_METHODS : MultiplexRouter::SINGLE_INTERFACE);
    router_ = MultiplexRouter::CreateAndStartReceiving(std::move(receiver_state->pipe), config, false, sequenced_runner, interface_name);
    router_->SetConnectionGroup(std::move(receiver_state->connection_group));

    endpoint_client_ = std::make_unique<InterfaceEndpointClient>(router_->CreateLocalEndpointHandle(kPrimaryInterfaceId), stub, std::move(request_validator),
        sync_method_ordinals, std::move(sequenced_runner), interface_version, interface_name, method_info_callback, method_name_callback);
    endpoint_client_->SetIdleTrackingEnabledCallback(base::BindOnce(&MultiplexRouter::SetConnectionGroup, router_));

#if BUILDFLAG(MOJO_RANDOM_DELAYS_ENABLED)
    MakeBindingRandomlyPaused(base::SequencedTaskRunnerHandle::Get(), weak_ptr_factory_.GetWeakPtr());
#endif
}
