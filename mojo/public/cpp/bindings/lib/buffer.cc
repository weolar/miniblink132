...
Buffer::Buffer(MessageHandle message, size_t message_payload_size, void* data, size_t size)
    : message_(message)
...
bool Buffer::AttachHandles(std::vector<ScopedHandle>* handles)
{
    ...
    MojoResult rv = MojoAppendMessageData(
        message_.value(), 0, reinterpret_cast<MojoHandle*>(handles->data()), static_cast<uint32_t>(handles->size()), nullptr, &data_, &new_size);
...
void Buffer::Seal()
{
    ...
    void* data;
    uint32_t size;
    MojoResult rv = MojoAppendMessageData(message_.value(), static_cast<uint32_t>(additional_bytes), nullptr, 0, &options, &data, &size);
    DCHECK_EQ(MOJO_RESULT_OK, rv);
