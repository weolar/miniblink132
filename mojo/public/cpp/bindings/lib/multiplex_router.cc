...
// this object.
class MultiplexRouter::InterfaceEndpoint : public base::RefCountedThreadSafe<InterfaceEndpoint>, public InterfaceEndpointController {
public:
    ...
    void set_disconnect_reason(const absl::optional<DisconnectReason>& disconnect_reason)
    {
        router_->AssertLockAcquired();
    ...
    InterfaceEndpointClient* client() const
    {
        return client_;
    }

    void AttachClient(InterfaceEndpointClient* client, scoped_refptr<base::SequencedTaskRunner> runner)
    {
        router_->AssertLockAcquired();
        DCHECK(!client_);
        DCHECK(!closed_);
...
private:
    ...
    void EnsureSyncWatcherExists()
    {
        ...
        MayAutoLock locker(&router_->lock_);
        sync_watcher_ = std::make_unique<SequenceLocalSyncEventWatcher>(base::BindRepeating(&InterfaceEndpoint::OnSyncEventSignaled, base::Unretained(this)));
        if (sync_message_event_signaled_)
            sync_watcher_->SignalEvent();
    }
...
public:
    ...
    static std::unique_ptr<Task> CreateMessageTask(MessageWrapper message_wrapper)
    {
        auto task = std::make_unique<Task>(MESSAGE, base::PassKey<Task>());
        task->message_wrapper = std::move(message_wrapper);
        return task;
    }
    static std::unique_ptr<Task> CreateNotifyErrorTask(InterfaceEndpoint* endpoint)
    {
    ...
    explicit Task(Type in_type, base::PassKey<Task>)
        : type(in_type)
    {
    }
    ...
    bool IsMessageTask() const
    {
        return type == MESSAGE;
    }
    bool IsNotifyErrorTask() const
    {
...
// static
scoped_refptr<MultiplexRouter> MultiplexRouter::Create(ScopedMessagePipeHandle message_pipe, Config config, bool set_interface_id_namespace_bit,
    scoped_refptr<base::SequencedTaskRunner> runner, const char* primary_interface_name)
{
    return base::MakeRefCounted<MultiplexRouter>(
        base::PassKey<MultiplexRouter>(), std::move(message_pipe), config, set_interface_id_namespace_bit, runner, primary_interface_name);
}

// static
scoped_refptr<MultiplexRouter> MultiplexRouter::CreateAndStartReceiving(ScopedMessagePipeHandle message_pipe, Config config,
    bool set_interface_id_namespace_bit, scoped_refptr<base::SequencedTaskRunner> runner, const char* primary_interface_name)
{
    auto router = Create(std::move(message_pipe), config, set_interface_id_namespace_bit, runner, primary_interface_name);
    router->StartReceiving();
    return router;
}

MultiplexRouter::MultiplexRouter(base::PassKey<MultiplexRouter>, ScopedMessagePipeHandle message_pipe, Config config, bool set_interface_id_namespace_bit,
    scoped_refptr<base::SequencedTaskRunner> runner, const char* primary_interface_name)
    : config_(config)
    , set_interface_id_namespace_bit_(set_interface_id_namespace_bit)
    , task_runner_(runner)
    , dispatcher_(this)
    , connector_(std::move(message_pipe), config == MULTI_INTERFACE ? Connector::MULTI_THREADED_SEND : Connector::SINGLE_THREADED_SEND, primary_interface_name)
    ...
    , control_message_proxy_(&connector_)
{
    ...
    if (primary_interface_name) {
        control_message_handler_.SetDescription(base::JoinString({ primary_interface_name, "[primary] PipeControlMessageHandler" }, " "));
    }
}

void MultiplexRouter::StartReceiving()
{
    connector_.set_connection_error_handler(base::BindOnce(&MultiplexRouter::OnPipeConnectionError, base::Unretained(this), false /* force_async_dispatch */));
    ...
    const bool allow_woken_up_by_others = config_ == SINGLE_INTERFACE_WITH_SYNC_METHODS || config_ == MULTI_INTERFACE;
...
void MultiplexRouter::SetIncomingMessageFilter(std::unique_ptr<MessageFilter> filter)
{
...
InterfaceId MultiplexRouter::AssociateInterface(ScopedInterfaceEndpointHandle handle_to_send)
{
...
ScopedInterfaceEndpointHandle MultiplexRouter::CreateLocalEndpointHandle(InterfaceId id)
{
...
void MultiplexRouter::CloseEndpointHandle(InterfaceId id, const absl::optional<DisconnectReason>& reason)
{
    if (!IsValidInterfaceId(id))
        return;
...
InterfaceEndpointController* MultiplexRouter::AttachEndpointClient(
    const ScopedInterfaceEndpointHandle& handle, InterfaceEndpointClient* client, scoped_refptr<base::SequencedTaskRunner> runner)
{
...
void MultiplexRouter::DetachEndpointClient(const ScopedInterfaceEndpointHandle& handle)
{
    const InterfaceId id = handle.id();
...
void MultiplexRouter::RaiseError()
{
    if (task_runner_->RunsTasksInCurrentSequence()) {
        ...
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&MultiplexRouter::RaiseError, base::WrapRefCounted(this)));
    }
}
...
void MultiplexRouter::ResumeIncomingMethodCallProcessing()
{
    ...
    for (auto iter = endpoints_.begin(); iter != endpoints_.end(); ++iter) {
        ...
        if (iter->second->peer_closed() || (sync_iter != sync_message_tasks_.end() && !sync_iter->second.empty())) {
...
bool MultiplexRouter::CanUnblockExternalSyncWait(const Message& message)
{
    ...
    if (!message.has_flag(Message::kFlagIsResponse) || !message.has_flag(Message::kFlagIsSync)) {
        return false;
    }
...
bool MultiplexRouter::CanUnblockExclusiveSameThreadSyncWait(const Message& message)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    return exclusive_sync_wait_->interface_id == message.interface_id() && exclusive_sync_wait_->request_id == message.request_id();
}
...
bool MultiplexRouter::Accept(Message* message)
{
    ...
    ClientCallBehavior client_call_behavior
        = connector_.during_sync_handle_watcher_callback() ? ALLOW_DIRECT_CLIENT_CALLS_FOR_SYNC_MESSAGES : ALLOW_DIRECT_CLIENT_CALLS;
    ...
    const bool processed = can_process && ProcessIncomingMessage(&message_wrapper, client_call_behavior, connector_.task_runner());
...
bool MultiplexRouter::OnPeerAssociatedEndpointClosed(InterfaceId id, const absl::optional<DisconnectReason>& reason)
{
    MayAutoLock locker(&lock_);
...
bool MultiplexRouter::WaitForFlushToComplete(ScopedMessagePipeHandle pipe)
{
    ...
    flush_pipe_watcher_.emplace(FROM_HERE, SimpleWatcher::ArmingPolicy::MANUAL, task_runner_);
    flush_pipe_watcher_->Watch(pipe.get(), MOJO_HANDLE_SIGNAL_PEER_CLOSED, MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED,
        base::BindRepeating(&MultiplexRouter::OnFlushPipeSignaled, base::WrapRefCounted(this)));
...
void MultiplexRouter::OnPipeConnectionError(bool force_async_dispatch)
{
    ...
    for (const auto& endpoint : endpoint_vector) {
        if (endpoint->client()) {
            base::flat_set<uint64_t> request_ids = endpoint->UnregisterAllExternalSyncWaiters();
...
void MultiplexRouter::OnFlushPipeSignaled(MojoResult result, const HandleSignalsState& state)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    flush_pipe_watcher_.reset();
    active_flush_pipe_.reset();
...
bool MultiplexRouter::ExclusiveSyncWaitForReply(InterfaceId interface_id, uint64_t request_id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void MultiplexRouter::ProcessTasks(ClientCallBehavior client_call_behavior, base::SequencedTaskRunner* current_task_runner)
{
    AssertLockAcquired();
    ...
    while (!tasks_.empty() && !paused_) {
        ...
        bool sync_message = task->IsMessageTask() && !task->message_wrapper.value().IsNull() && task->message_wrapper.value().has_flag(Message::kFlagIsSync);
        if (sync_message) {
        ...
        bool processed = task->IsNotifyErrorTask() ? ProcessNotifyErrorTask(task.get(), client_call_behavior, current_task_runner)
                                                   : ProcessIncomingMessage(&task->message_wrapper, client_call_behavior, current_task_runner);
...
bool MultiplexRouter::ProcessFirstSyncMessageForEndpoint(InterfaceId id)
{
    ...
    bool processed = ProcessIncomingMessage(&message_wrapper, ALLOW_DIRECT_CLIENT_CALLS_FOR_SYNC_MESSAGES, nullptr);
    DCHECK(processed);
...
bool MultiplexRouter::ProcessNotifyErrorTask(Task* task, ClientCallBehavior client_call_behavior, base::SequencedTaskRunner* current_task_runner)
{
    DCHECK(!current_task_runner || current_task_runner->RunsTasksInCurrentSequence());
    DCHECK(!paused_);
    ...
    if (client_call_behavior != ALLOW_DIRECT_CLIENT_CALLS || endpoint->task_runner() != current_task_runner) {
    ...
    absl::optional<DisconnectReason> disconnect_reason(endpoint->disconnect_reason());
...
bool MultiplexRouter::ProcessIncomingMessage(
    MessageWrapper* message_wrapper, ClientCallBehavior client_call_behavior, base::SequencedTaskRunner* current_task_runner)
{
    DCHECK(!current_task_runner || current_task_runner->RunsTasksInCurrentSequence());
    DCHECK(!paused_);
    ...
    if (PipeControlMessageHandler::IsPipeControlMessage(message)) {
...

        {
            ...
            result = !tmp_message.IsNull() && control_message_handler_.Accept(&tmp_message);
        }
    ...
    if (message->has_flag(Message::kFlagIsSync)) {
        if (!message->has_flag(Message::kFlagIsResponse) && !base::Contains(endpoint->client()->sync_method_ordinals(), message->name())) {
            RaiseErrorInNonTestingMode();
            return true;
        }
        can_direct_call = client_call_behavior != NO_DIRECT_CLIENT_CALLS && endpoint->task_runner()->RunsTasksInCurrentSequence();
    } else {
        can_direct_call = client_call_behavior == ALLOW_DIRECT_CLIENT_CALLS && endpoint->task_runner() == current_task_runner;
    }
    ...
    bool result = false;
    {
        ...
        MayAutoUnlock unlocker(&lock_);
        Message tmp_message = message_wrapper->DeserializeEndpointHandlesAndTake();
        result = !tmp_message.IsNull() && client->HandleIncomingMessage(&tmp_message);
    }
    if (!result)
        RaiseErrorInNonTestingMode();
...
void MultiplexRouter::MaybePostToProcessTasks(base::SequencedTaskRunner* task_runner)
{
    AssertLockAcquired();
    if (posted_to_process_tasks_)
        return;
    ...
    task_runner->PostTask(FROM_HERE, base::BindOnce(&MultiplexRouter::LockAndCallProcessTasks, base::WrapRefCounted(this)));
}
...
void MultiplexRouter::LockAndCallProcessTasks()
{
    ...
    scoped_refptr<base::SequencedTaskRunner> runner(std::move(posted_to_task_runner_));
    ProcessTasks(ALLOW_DIRECT_CLIENT_CALLS, runner.get());
}

void MultiplexRouter::UpdateEndpointStateMayRemove(InterfaceEndpoint* endpoint, EndpointStateUpdateType type)
{
...
MultiplexRouter::InterfaceEndpoint* MultiplexRouter::FindOrInsertEndpoint(InterfaceId id, bool* inserted)
{
    AssertLockAcquired();
...
MultiplexRouter::InterfaceEndpoint* MultiplexRouter::FindEndpoint(InterfaceId id)
{
    AssertLockAcquired();
...
bool MultiplexRouter::InsertEndpointsForMessage(const Message& message)
{
    ...
    for (uint32_t i = 0; i < num_ids; ++i) {
        ...
        if (set_interface_id_namespace_bit_ == HasInterfaceIdNamespaceBitSet(ids[i])) {
            return false;
        }
