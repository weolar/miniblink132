...
class ScopedInterfaceEndpointHandle::State : public base::RefCountedThreadSafe<State> {
public:
    State() = default;

    State(InterfaceId id, scoped_refptr<AssociatedGroupController> group_controller)
        : id_(id)
    ...
    void SetAssociationEventHandler(AssociationEventCallback handler)
    {
        ...
        if (!pending_association_) {
            runner_->PostTask(FROM_HERE, base::BindOnce(&ScopedInterfaceEndpointHandle::State::RunAssociationEventHandler, this, runner_, ASSOCIATED));
        } else if (!peer_state_) {
            runner_->PostTask(
                FROM_HERE, base::BindOnce(&ScopedInterfaceEndpointHandle::State::RunAssociationEventHandler, this, runner_, PEER_CLOSED_BEFORE_ASSOCIATION));
        }
    }
...
private:
    ...
    void OnAssociated(InterfaceId id, scoped_refptr<AssociatedGroupController> group_controller)
    {
    ...
    // Called by the peer, maybe from a different sequence.
    void OnPeerClosedBeforeAssociation(const absl::optional<DisconnectReason>& reason)
    {
        AssociationEventCallback handler;
        {
            internal::MayAutoLock locker(&lock_);
            ...
            if (!association_event_handler_.is_null()) {
                if (runner_->RunsTasksInCurrentSequence()) {
                    ...
                    runner_->PostTask(FROM_HERE,
                        base::BindOnce(&ScopedInterfaceEndpointHandle::State::RunAssociationEventHandler, this, runner_, PEER_CLOSED_BEFORE_ASSOCIATION));
                }
            }
        }
    ...
    void RunAssociationEventHandler(scoped_refptr<base::SequencedTaskRunner> posted_to_runner, AssociationEvent event)
    {
        AssociationEventCallback handler;
...
// static
void ScopedInterfaceEndpointHandle::CreatePairPendingAssociation(ScopedInterfaceEndpointHandle* handle0, ScopedInterfaceEndpointHandle* handle1)
{
...
ScopedInterfaceEndpointHandle::ScopedInterfaceEndpointHandle(ScopedInterfaceEndpointHandle&& other)
    : state_(base::MakeRefCounted<State>())
{
...
ScopedInterfaceEndpointHandle& ScopedInterfaceEndpointHandle::operator=(ScopedInterfaceEndpointHandle&& other)
{
    reset();
    state_.swap(other.state_);
    return *this;
...
AssociatedGroupController* ScopedInterfaceEndpointHandle::group_controller() const
{
    return state_->group_controller();
}

const absl::optional<DisconnectReason>& ScopedInterfaceEndpointHandle::disconnect_reason() const
{
    return state_->disconnect_reason();
}

void ScopedInterfaceEndpointHandle::SetAssociationEventHandler(AssociationEventCallback handler)
{
...
void ScopedInterfaceEndpointHandle::ResetWithReason(uint32_t custom_reason, base::StringPiece description)
{
    ResetInternal(DisconnectReason(custom_reason, std::string(description)));
}

ScopedInterfaceEndpointHandle::ScopedInterfaceEndpointHandle(InterfaceId id, scoped_refptr<AssociatedGroupController> group_controller)
    : state_(base::MakeRefCounted<State>(id, std::move(group_controller)))
{
    DCHECK(!IsValidInterfaceId(state_->id()) || state_->group_controller());
}

bool ScopedInterfaceEndpointHandle::NotifyAssociation(InterfaceId id, scoped_refptr<AssociatedGroupController> peer_group_controller)
{
    return state_->NotifyAssociation(id, peer_group_controller);
}

void ScopedInterfaceEndpointHandle::ResetInternal(const absl::optional<DisconnectReason>& reason)
{
...
base::RepeatingCallback<AssociatedGroupController*()> ScopedInterfaceEndpointHandle::CreateGroupControllerGetter() const
{
