...
public:
    ...
    // such errors.
    ValidationContext(const void* data, size_t data_num_bytes, size_t num_handles, size_t num_associated_endpoint_handles, Message* message = nullptr,
        const char* description = "", int stack_depth = 0, ValidatorType validator_type = kUnspecifiedValidator);
    ...
    ValidationContext(Message* message, const char* description, ValidatorType validator_type);
    ...
    bool ClaimAssociatedEndpointHandle(const AssociatedEndpointHandle_Data& encoded_handle)
    {
        uint32_t index = encoded_handle.value;
        if (index == kEncodedInvalidHandleValue)
            return true;

        if (index < associated_endpoint_handle_begin_ || index >= associated_endpoint_handle_end_)
            return false;
    ...
    public:
        ...
        explicit ScopedDepthTracker(ValidationContext* ctx)
            : ctx_(ctx)
        {
            ++ctx_->stack_depth_;
        }
