...
public:
    ...
    ThreadSafeInterfaceEndpointClientProxy(base::WeakPtr<InterfaceEndpointClient> endpoint, scoped_refptr<ThreadSafeProxy::Target> target,
        const AssociatedGroup& associated_group, scoped_refptr<base::SequencedTaskRunner> task_runner)
    ...
    ThreadSafeInterfaceEndpointClientProxy(const ThreadSafeInterfaceEndpointClientProxy&) = delete;
    ThreadSafeInterfaceEndpointClientProxy& operator=(const ThreadSafeInterfaceEndpointClientProxy&) = delete;

    // ThreadSafeProxy:
    void SendMessage(Message& message) override
    {
        message.SerializeHandles(associated_group_.GetController());
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&ThreadSafeInterfaceEndpointClientProxy::ForwardMessage, this, std::move(message)));
    }

    void SendMessageWithResponder(Message& message, std::unique_ptr<MessageReceiver> responder) override;
...
private:
    ...
    struct SyncResponseInfo : public base::RefCountedThreadSafe<SyncResponseInfo> {
    ...
    struct InProgressSyncCalls : public base::RefCountedThreadSafe<InProgressSyncCalls> {
    ...
    private:
        bool Accept(Message* message) override
        {
            ...
            caller_task_runner_->PostTask(
                FROM_HERE, base::BindOnce(&ForwardToCallingThread::CallAcceptAndDeleteResponder, std::move(responder_), std::move(*message)));
            return true;
        }

        static void CallAcceptAndDeleteResponder(std::unique_ptr<MessageReceiver> responder, Message message)
        {
    ...
    public:
        explicit ForwardSameThreadResponder(scoped_refptr<ThreadSafeProxy> proxy, std::unique_ptr<MessageReceiver> responder)
    ...
    void ForwardMessageWithResponder(Message message, InterfaceEndpointClient::SyncSendMode sync_send_mode, std::unique_ptr<MessageReceiver> responder)
    {
        DCHECK(task_runner_->RunsTasksInCurrentSequence());
        if (!endpoint_)
            return;

        endpoint_->SendMessageWithResponder(&message, /*is_control_message=*/false, sync_send_mode, std::move(responder));
    }
    ...
    const scoped_refptr<InProgressSyncCalls> sync_calls_ { base::MakeRefCounted<InProgressSyncCalls>() };
};

void DetermineIfEndpointIsConnected(const base::WeakPtr<InterfaceEndpointClient>& client, base::OnceCallback<void(bool)> callback)
{
...
public:
    explicit ResponderThunk(const base::WeakPtr<InterfaceEndpointClient>& endpoint_client, scoped_refptr<base::SequencedTaskRunner> runner)
    ...
    ~ResponderThunk() override
    {
        if (!accept_was_invoked_) {
            ...
            if (task_runner_->RunsTasksInCurrentSequence()) {
                ...
                task_runner_->PostTask(FROM_HERE, base::BindOnce(&InterfaceEndpointClient::RaiseError, endpoint_client_));
            }
        }
    }
    ...
    void IsConnectedAsync(base::OnceCallback<void(bool)> callback) override
    {
        if (task_runner_->RunsTasksInCurrentSequence()) {
            ...
            task_runner_->PostTask(FROM_HERE, base::BindOnce(&DetermineIfEndpointIsConnected, endpoint_client_, std::move(callback)));
        }
    }
...
InterfaceEndpointClient::PendingAsyncResponse::PendingAsyncResponse(uint32_t request_message_name, std::unique_ptr<MessageReceiver> responder)
    : request_message_name(request_message_name)
    , responder(std::move(responder))
{
}

InterfaceEndpointClient::PendingAsyncResponse::PendingAsyncResponse(PendingAsyncResponse&&) = default;

InterfaceEndpointClient::PendingAsyncResponse& InterfaceEndpointClient::PendingAsyncResponse::operator=(PendingAsyncResponse&&) = default;

InterfaceEndpointClient::PendingAsyncResponse::~PendingAsyncResponse() = default;

InterfaceEndpointClient::SyncResponseInfo::SyncResponseInfo(uint32_t request_message_name, bool* in_response_received)
    : request_message_name(request_message_name)
...
InterfaceEndpointClient::HandleIncomingMessageThunk::HandleIncomingMessageThunk(InterfaceEndpointClient* owner)
    : owner_(owner)
{
}
...
bool InterfaceEndpointClient::HandleIncomingMessageThunk::Accept(Message* message)
{
...
void ThreadSafeInterfaceEndpointClientProxy::SendMessageWithResponder(Message& message, std::unique_ptr<MessageReceiver> responder)
{
    message.SerializeHandles(associated_group_.GetController());
    ...
    if (!message.has_flag(Message::kFlagIsSync)) {
        auto reply_forwarder = std::make_unique<ForwardToCallingThread>(std::move(responder));
        task_runner_->PostTask(FROM_HERE,
            base::BindOnce(&ThreadSafeInterfaceEndpointClientProxy ::ForwardMessageWithResponder, this, std::move(message),
                InterfaceEndpointClient::SyncSendMode::kForceAsync, std::move(reply_forwarder)));
        return;
    }
    ...
    if (task_runner_->RunsTasksInCurrentSequence()) {
        ForwardMessageWithResponder(std::move(message), InterfaceEndpointClient::SyncSendMode::kAllowSyncWait,
            std::make_unique<ForwardSameThreadResponder>(this, std::move(responder)));
        return;
    }

    // If the Remote is bound on another sequence, post the call.
    const bool allow_interrupt = SyncCallRestrictions::AreSyncCallInterruptsEnabled() && !message.has_flag(Message::kFlagNoInterrupt);
    auto response = base::MakeRefCounted<SyncResponseInfo>();
    auto response_signaler = std::make_unique<SyncResponseSignaler>(response);
    task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&ThreadSafeInterfaceEndpointClientProxy::ForwardMessageWithResponder, this, std::move(message),
            InterfaceEndpointClient::SyncSendMode::kForceAsync, std::move(response_signaler)));
    ...
    if (allow_interrupt) {
        ...
        SyncEventWatcher watcher(&response->event, base::BindRepeating(set_flag, &signaled));
...
InterfaceEndpointClient::InterfaceEndpointClient(ScopedInterfaceEndpointHandle handle, MessageReceiverWithResponderStatus* receiver,
    std::unique_ptr<MessageReceiver> payload_validator, base::span<const uint32_t> sync_method_ordinals, scoped_refptr<base::SequencedTaskRunner> task_runner,
    uint32_t interface_version, const char* interface_name, MessageToMethodInfoCallback method_info_callback, MessageToMethodNameCallback method_name_callback)
    ...
    , method_name_callback_(method_name_callback)
{
    ...
    if (handle_.pending_association()) {
        if (task_runner_->RunsTasksInCurrentSequence()) {
            handle_.SetAssociationEventHandler(base::BindOnce(&InterfaceEndpointClient::OnAssociationEvent, base::Unretained(this)));
        } else {
            handle_.SetAssociationEventHandler(
                base::BindPostTask(task_runner_, base::BindOnce(&InterfaceEndpointClient::OnAssociationEvent, weak_ptr_factory_.GetWeakPtr())));
        }
    } else {
...
scoped_refptr<ThreadSafeProxy> InterfaceEndpointClient::CreateThreadSafeProxy(scoped_refptr<ThreadSafeProxy::Target> target)
{
    return base::MakeRefCounted<ThreadSafeInterfaceEndpointClientProxy>(weak_ptr_factory_.GetWeakPtr(), std::move(target), *associated_group_, task_runner_);
}
...
ScopedInterfaceEndpointHandle InterfaceEndpointClient::PassHandle()
{
    ...
    handle_.SetAssociationEventHandler(ScopedInterfaceEndpointHandle::AssociationEventCallback());
...
void InterfaceEndpointClient::CloseWithReason(uint32_t custom_reason, base::StringPiece description)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void InterfaceEndpointClient::SendControlMessageWithResponder(Message* message, std::unique_ptr<MessageReceiver> responder)
{
    SendMessageWithResponder(message, true /* is_control_message */, SyncSendMode::kAllowSyncWait, std::move(responder));
}
...
bool InterfaceEndpointClient::AcceptWithResponder(Message* message, std::unique_ptr<MessageReceiver> responder)
{
    return SendMessageWithResponder(message, false /* is_control_message */, SyncSendMode::kAllowSyncWait, std::move(responder));
}

bool InterfaceEndpointClient::SendMessage(Message* message, bool is_control_message)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
bool InterfaceEndpointClient::SendMessageWithResponder(
    Message* message, bool is_control_message, SyncSendMode sync_send_mode, std::unique_ptr<MessageReceiver> responder)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(message->has_flag(Message::kFlagExpectsResponse));
    DCHECK(!handle_.pending_association());
    ...
    const bool exclusive_wait = message->has_flag(Message::kFlagNoInterrupt) || !SyncCallRestrictions::AreSyncCallInterruptsEnabled();
    if (!controller_->SendMessage(message))
        return false;
    ...
    if (!is_sync || sync_send_mode == SyncSendMode::kForceAsync) {
        ...
        async_responders_.emplace(request_id, PendingAsyncResponse { message_name, std::move(responder) });
        return true;
    }
    ...
    sync_responses_.insert(std::make_pair(request_id, std::make_unique<SyncResponseInfo>(message_name, &response_received)));

    base::WeakPtr<InterfaceEndpointClient> weak_self = weak_ptr_factory_.GetWeakPtr();
    if (exclusive_wait)
...
bool InterfaceEndpointClient::HandleIncomingMessage(Message* message)
{
    ...
    if (!dispatcher_.Accept(message)) {
        LOG(ERROR) << "Message " << name << " rejected by interface " << interface_name;
        return false;
    }
...
void InterfaceEndpointClient::NotifyError(const absl::optional<DisconnectReason>& reason)
{
    TRACE_EVENT("toplevel", "Closed mojo endpoint", [&](perfetto::EventContext& ctx) {
    ...
    if (error_handler_) {
        ...
        if (reason) {
            std::move(error_with_reason_handler_).Run(reason->custom_reason, reason->description);
        } else {
...
void InterfaceEndpointClient::QueryVersion(base::OnceCallback<void(uint32_t)> callback)
{
...
void InterfaceEndpointClient::SetIdleHandler(base::TimeDelta timeout, base::RepeatingClosure handler)
{
...
void InterfaceEndpointClient::SetIdleTrackingEnabledCallback(IdleTrackingEnabledCallback callback)
{
...
bool InterfaceEndpointClient::AcceptEnableIdleTracking(base::TimeDelta timeout)
{
    ...
    if (idle_tracking_enabled_callback_) {
        idle_tracking_connection_group_
            = ConnectionGroup::Create(base::BindRepeating(&InterfaceEndpointClient::MaybeStartIdleTimer, weak_ptr_factory_.GetWeakPtr()), task_runner_);
        std::move(idle_tracking_enabled_callback_).Run(idle_tracking_connection_group_.WeakCopy());
    }
...
void InterfaceEndpointClient::MaybeStartIdleTimer()
{
    ...
    // fire.
    if (idle_tracking_connection_group_ && idle_tracking_connection_group_.HasZeroRefs()) {
        DCHECK(idle_timeout_);
        notify_idle_timer_.emplace();
        notify_idle_timer_->Start(FROM_HERE, *idle_timeout_, base::BindOnce(&InterfaceEndpointClient::MaybeSendNotifyIdle, base::Unretained(this)));
    } else {
...
void InterfaceEndpointClient::InitControllerIfNecessary()
{
    ...
    controller_ = handle_.group_controller()->AttachEndpointClient(handle_, this, task_runner_);
    if (!sync_method_ordinals_.empty() && task_runner_->RunsTasksInCurrentSequence())
        controller_->AllowWokenUpBySyncWatchOnSameThread();
}

void InterfaceEndpointClient::OnAssociationEvent(ScopedInterfaceEndpointHandle::AssociationEvent event)
{
    if (event == ScopedInterfaceEndpointHandle::ASSOCIATED) {
        InitControllerIfNecessary();
    } else if (event == ScopedInterfaceEndpointHandle::PEER_CLOSED_BEFORE_ASSOCIATION) {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&InterfaceEndpointClient::NotifyError, weak_ptr_factory_.GetWeakPtr(), handle_.disconnect_reason()));
    }
}

bool InterfaceEndpointClient::HandleValidatedMessage(Message* message)
{
    TRACE_EVENT("toplevel", perfetto::StaticString { method_name_callback_(*message) }, [&](perfetto::EventContext& ctx) {
        auto* info = ctx.event()->set_chrome_mojo_event_info();
        ...
        if (method_info) {
            ...
            const absl::optional<size_t> location_iid = base::trace_event::InternedUnsymbolizedSourceLocation::Get(&ctx, method_address);
            if (location_iid) {
                info->set_mojo_interface_method_iid(*location_iid);
            }
        }

        static const uint8_t* flow_enabled = TRACE_EVENT_API_GET_CATEGORY_GROUP_ENABLED("toplevel.flow");
        if (!*flow_enabled)
            return;
    ...
    if (message->has_flag(Message::kFlagExpectsResponse)) {
        has_response = true;
        auto responder = std::make_unique<ResponderThunk>(weak_ptr_factory_.GetWeakPtr(), task_runner_);
        if (mojo::internal::ControlMessageHandler::IsControlMessage(message)) {
            return control_message_handler_.AcceptWithResponder(message, std::move(responder));
        } else {
            if (idle_tracking_connection_group_)
                responder->set_connection_group(idle_tracking_connection_group_);
            accepted_interface_message = incoming_receiver_->AcceptWithResponder(message, std::move(responder));
        }
    ...
    if (weak_self && accepted_interface_message && idle_tracking_connection_group_) {
