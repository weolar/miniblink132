...
public:
    ...
    Connector(ScopedMessagePipeHandle message_pipe, ConnectorConfig config, const char* interface_name = "unknown interface");
    ...
    // ---mojo lite:
    MessageReceiver* incoming_receiver() const
    {
    ...
    void StartReceiving(scoped_refptr<base::SequencedTaskRunner> task_runner, bool allow_woken_up_by_others = false);
    ...
    void SetMessageQuotaChecker(scoped_refptr<internal::MessageQuotaChecker> checker);
    ...
    static void OverrideDefaultSerializationBehaviorForTesting(OutgoingSerializationMode outgoing_mode, IncomingSerializationMode incoming_mode);
...
private:
    ...
    void ScheduleDispatchOfPendingMessagesOrWaitForMore(uint64_t pending_message_count);
    ...
    // ---mojo lite:
    static void OnConnectorAcceptMsg(
        base::WeakPtr<mojo::Connector> connector, MojoHandle handle, size_t id, mojo::MessageReceiver* incomingReceiver, std::vector<uint8_t>* data);
    size_t mojo_lite_id_ = 0;
    //--------------
