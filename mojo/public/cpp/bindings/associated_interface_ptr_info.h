...
template <typename Interface> class AssociatedInterfacePtrInfo {
public:
    ...
    AssociatedInterfacePtrInfo(ScopedInterfaceEndpointHandle handle, uint32_t version)
        : handle_(std::move(handle))
        , version_(version)
    {
    }

    //     AssociatedInterfacePtrInfo(void** ptr, uint32_t version)
    //         : interface_addr_(ptr)
    //         , version_(version)
    //     {
    //     }
    ...
    AssociatedInterfacePtrInfo& operator=(const AssociatedInterfacePtrInfo&) = delete;
