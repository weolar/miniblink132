...
template <typename T, typename SFINAE = void> struct HasCloneMethod : std ::false_type {
    static_assert(sizeof(T), "T must be a complete type.");
};

template <typename T> struct HasCloneMethod<T, std::void_t<decltype(std::declval<const T&>().Clone())>> : std::true_type { };

template <typename T> T Clone(const T& input);

template <typename T> struct CloneTraits {
...
template <typename T> struct CloneTraits<absl::optional<T>> {
...
template <typename T> struct CloneTraits<std::vector<T>> {
...
template <typename K, typename V> struct CloneTraits<base::flat_map<K, V>> {
    static base::flat_map<K, V> Clone(const base::flat_map<K, V>& input)
    {
        base::flat_map<K, V> result;
        for (const auto& element : input) {
            result.insert(std::make_pair(mojo::Clone(element.first), mojo::Clone(element.second)));
        }
        return result;
    }
};
