...
template <typename Interface> class Remote {
public:
    ...
    // this Remote.
    Remote(PendingRemote<Interface> pending_remote, scoped_refptr<base::SequencedTaskRunner> task_runner)
    {
    ...
    {
        remote_impl_.Swap(&other.remote_impl_);
        return *this;
    }
    ...
    {
        DCHECK(is_bound()) << "Cannot issue Interface method calls on an unbound Remote";
    ...
    {
        return remote_impl_.GetReceiverTaskRunner();
    }
#endif
    ...
    {
        DCHECK(is_bound());
        return !remote_impl_.encountered_error();
    }
    ...
    void set_disconnect_with_reason_handler(ConnectionErrorWithReasonCallback handler)
    {
        remote_impl_.set_connection_error_with_reason_handler(std::move(handler));
    }

    // weolar: no use
    void set_connection_error_handler_is_sync()
    {
        remote_impl_.set_connection_error_handler_is_sync();
    }
    ...
    void reset_on_disconnect()
    {
        ...
        set_disconnect_handler(base::BindOnce(&Remote::reset, base::Unretained(this)));
    }
    ...
    {
        remote_impl_.set_idle_handler(timeout, std::move(handler));
    }
    ...
    {
        //set_idle_handler(timeout, base::BindRepeating(&Remote::reset, base::Unretained(this)));
    }

    //     void ResetWithCallback(base::OnceClosure cb) // weolar: for asyn close
    //     {
    //         remote_impl_.set_reset_handler(std::move(cb));
    //         State doomed_state;
    //         remote_impl_.Swap(&doomed_state);
    //     }
    ...
    {
        if (remote_impl_.is_bound())
            remote_impl_.CloseWithReason(custom_reason, description);
        reset();
    }
    ...
    {
        return remote_impl_.version();
    }

    // for AssociatedRemote ---
#if ASSOCIATED_AS_ALIAS == 1
    PendingReceiver<Interface> BindNewEndpointAndPassReceiver(scoped_refptr<base::SequencedTaskRunner> task_runner = nullptr)
    {
        return BindNewPipeAndPassReceiver(task_runner);
    }

    [[nodiscard]] PendingReceiver<Interface> BindNewEndpointAndPassDedicatedReceiver()
    {
        PendingReceiver<Interface> ret = BindNewPipeAndPassReceiver();
        ret.EnableUnassociatedUsage();
        return ret;
    }
#endif
    //=====
    ...
    {
        DCHECK(!is_bound()) << "Remote for " << Interface::Name_ << " is already bound";
        return BindNewPipeAndPassReceiver(nullptr);
    }

    // Like above, but the Remote will schedule response callbacks and disconnection notifications on |task_runner| instead of the default SequencedTaskRunner.
    // |task_runner| must run tasks on the same sequence that owns this Remote.
    [[nodiscard]] PendingReceiver<Interface> BindNewPipeAndPassReceiver(scoped_refptr<base::SequencedTaskRunner> task_runner)
    {
        DCHECK(!is_bound()) << "Remote for " << Interface::Name_ << " is already bound";
        MessagePipe pipe;
        Bind(PendingRemote<Interface>(std::move(pipe.handle0), 0), std::move(task_runner));
        return PendingReceiver<Interface>(std::move(pipe.handle1));

        //         scoped_refptr<RawPtrMessagePipe> pipe = base::AdoptRef(new RawPtrMessagePipe());
        //         remote_impl_.SetRawPtrMessagePipe(pipe);
        //         PendingRemote<Interface> pending_remote(remote_impl_.GetRawPtrMessagePipe(), 0);
        //         Bind(std::move(pending_remote), std::move(task_runner));
        //         return PendingReceiver<Interface>(remote_impl_.GetRawPtrMessagePipe());
    }

    // Binds this Remote by consuming |pending_remote|, which must be valid. The Remote will schedule any response callbacks or disconnection notifications
    // on the default SequencedTaskRunner (i.e. base::SequencedTaskRunnerHandle::Get() at the time of this call). Must only
    // be called on an unbound Remote.
    void Bind(PendingRemote<Interface> pending_remote) // 此函数要求pending_remote绑定了远端的Receiver
    {
        DCHECK(!is_bound()) << "Remote for " << Interface::Name_ << " is already bound";
    ...
    // Like above, but the Remote will schedule response callbacks and disconnection notifications on |task_runner| instead of the default
    // SequencedTaskRunner. Must only be called on an unbound Remote. |task_runner| must run tasks on the same sequence that owns this Remote.
    void Bind(PendingRemote<Interface> pending_remote, scoped_refptr<base::SequencedTaskRunner> task_runner) // 此函数要求pending_remote绑定了远端的Receiver
    {
        DCHECK(!is_bound()) << "Remote for " << Interface::Name_ << " is already bound";
        if (!pending_remote) {
            reset();
        ...
        // -> RemoteImplBase::Bind
        remote_impl_.Bind(pending_remote.internal_state(), std::move(task_runner));
        ...
        //std::ignore = remote_impl_.instance();
    }
    ...
    [[nodiscard]] PendingRemote<Interface> Unbind()
    {
        ...
        DCHECK(is_bound());
        //CHECK(!remote_impl_.has_pending_callbacks());
        State state;
        remote_impl_.Swap(&state);
        internal::PendingRemoteState pending_state = state.Unbind();
        return PendingRemote<Interface>(std::move(pending_state.pipe), pending_state.version);
    }
    ...
    {
        remote_impl_.QueryVersion(std::move(callback));
    }
    ...
    {
        remote_impl_.RequireVersion(version);
    }
    ...
    {
        remote_impl_.PauseReceiverUntilFlushCompletes(std::move(flush));
    }
    ...
    {
        remote_impl_.FlushAsync(std::move(flusher));
    }
    ...
    {
        remote_impl_.FlushAsyncForTesting(std::move(callback));
    }
    ...
    {
        return remote_impl_.GetNumUnackedMessagesForTesting();
    }

    scoped_refptr<base::SequencedTaskRunner> runner()
    {
        return remote_impl_.runner();
    } // weolar

    // DO NOT USE. Exposed only for internal use and for testing.
    internal::RemoteImpl<Interface>* internal_state()
    {
        return &remote_impl_;
    }

private:
    using State = internal::RemoteImpl<Interface>;
    mutable State remote_impl_;
};

#if ASSOCIATED_AS_ALIAS == 1
template <typename Interface> using AssociatedRemote = Remote<Interface>;
#endif
