...
Message CreateRawMessage(size_t size)
{
    ...
    rv = MojoAppendMessageData(handle->value(), static_cast<uint32_t>(size), nullptr, 0, &options, &buffer, &buffer_size);
    DCHECK_EQ(MOJO_RESULT_OK, rv);

    return Message::CreateFromMessageHandle(&handle);
}

template <typename T> void Append(std::vector<uint8_t>* data_vector, T data)
{
...
bool TestInputParser(const std::string& input, bool expected_result, const std::vector<uint8_t>& expected_data, size_t expected_num_handles)
{
    std::vector<uint8_t> data;
    size_t num_handles;
    std::string error_message;

    bool result = ParseValidationTestInput(input, &data, &num_handles, &error_message);
    if (expected_result) {
        if (result && error_message.empty() && expected_data == data && expected_num_handles == num_handles) {
            return true;
        }
...
std::vector<std::string> GetMatchingTests(const std::vector<std::string>& names, const std::string& prefix)
{
    const std::string suffix = ".data";
    std::vector<std::string> tests;
    for (size_t i = 0; i < names.size(); ++i) {
        if (names[i].size() >= suffix.size() && names[i].substr(0, prefix.size()) == prefix && names[i].substr(names[i].size() - suffix.size()) == suffix)
...
bool ReadAndParseDataFile(const std::string& path, std::vector<uint8_t>* data, size_t* num_handles)
{
    std::string input;
...
{
    return "mojo/public/interfaces/bindings/tests/data/validation/" + root + suffix;
}

// |message| should be a newly created object.
bool ReadTestCase(const std::string& test, Message* message, std::string* expected)
{
    std::vector<uint8_t> data;
    size_t num_handles;
    if (!ReadAndParseDataFile(GetPath(test, ".data"), &data, &num_handles) || !ReadResultFile(GetPath(test, ".expected"), expected)) {
        return false;
    }
...
void RunValidationTests(const std::string& prefix, MessageReceiver* test_message_receiver)
{
    std::vector<std::string> names = EnumerateSourceRootRelativeDirectory(GetPath("", ""));
    ...
    for (size_t i = 0; i < tests.size(); ++i) {
        ...
        mojo::internal::ValidationErrorObserverForTesting observer(run_loop.QuitClosure());
...
public:
    TwoStepValidator(std::unique_ptr<MessageReceiver> first_validator, std::unique_ptr<MessageReceiver> second_validator)
    ...
    {
        return first_validator_->Accept(message) && second_validator_->Accept(message);
    }
...
public:
    ...
    void SetUp() override
    {
        ...
        ASSERT_EQ(MOJO_RESULT_OK, CreateMessagePipe(nullptr, &tester_endpoint, &testee_endpoint_));
        test_message_receiver_ = new TestMessageReceiver(this, std::move(tester_endpoint));
    }
...
private:
    ...
    public:
        TestMessageReceiver(ValidationIntegrationTest* owner, ScopedMessagePipeHandle handle)
            : owner_(owner)
            , connector_(std::move(handle), mojo::Connector::SINGLE_THREADED_SEND, base::ThreadTaskRunnerHandle::Get())
        {
...
TEST_F(ValidationTest, InputParser)
{
    ...
    {
        const char* error_inputs[] = { "/ hello world", "[u1]x", "[u2]-1000", "[u1]0x100", "[s2]-0x8001", "[b]1", "[b]1111111k", "[dist4]unmatched",
            "[anchr]hello [dist8]hello", "[dist4]a [dist4]a [anchr]a", "[dist4]a [anchr]a [dist4]a [anchr]a", "0 [handles]50", nullptr };
...
TEST_F(ValidationTest, Conformance)
{
    ...
    validators.SetValidator(
        std::make_unique<TwoStepValidator>(std::make_unique<mojo::MessageHeaderValidator>(), std::make_unique<ConformanceTestInterface::RequestValidator_>()));
...
TEST_F(ValidationTest, AssociatedConformace)
{
    ...
    validators.SetValidator(std::make_unique<TwoStepValidator>(
        std::make_unique<mojo::MessageHeaderValidator>(), std::make_unique<AssociatedConformanceTestInterface::RequestValidator_>()));
...
TEST_F(ValidationTest, BoundsCheck)
{
    ...
    mojo::MessageDispatcher validators(&dummy_receiver);
    validators.SetValidator(
        std::make_unique<TwoStepValidator>(std::make_unique<mojo::MessageHeaderValidator>(), std::make_unique<BoundsCheckTestInterface::RequestValidator_>()));
...
TEST_F(ValidationTest, ResponseConformance)
{
    ...
    mojo::MessageDispatcher validators(&dummy_receiver);
    validators.SetValidator(
        std::make_unique<TwoStepValidator>(std::make_unique<mojo::MessageHeaderValidator>(), std::make_unique<ConformanceTestInterface::ResponseValidator_>()));
...
TEST_F(ValidationTest, ResponseBoundsCheck)
{
    ...
    mojo::MessageDispatcher validators(&dummy_receiver);
    validators.SetValidator(
        std::make_unique<TwoStepValidator>(std::make_unique<mojo::MessageHeaderValidator>(), std::make_unique<BoundsCheckTestInterface::ResponseValidator_>()));
...
{
    Remote<IntegrationTestInterface> remote(PendingRemote<IntegrationTestInterface>(testee_endpoint(), 0u));
...
TEST_F(ValidationIntegrationTest, Receiver)
{
    ...
    Receiver<IntegrationTestInterface> receiver(&interface_impl, PendingReceiver<IntegrationTestInterface>(testee_endpoint()));
