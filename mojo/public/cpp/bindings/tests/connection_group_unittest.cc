...
public:
    explicit TestInterfaceImpl(PendingReceiver<mojom::TestInterface> receiver)
    {
        ...
        receivers_.set_disconnect_handler(base::BindRepeating(&TestInterfaceImpl::OnDisconnect, base::Unretained(this)));
    }
...
private:
    ...
    // mojom::TestInterface:
    void BindReceiver(mojo::PendingReceiver<mojom::TestInterface> receiver) override
    {
        receivers_.Add(this, std::move(receiver));
    }
...
{
    ConnectionGroup::Ref ref = ConnectionGroup::Create(base::DoNothing(), nullptr);
...
{
    base::RunLoop loop;
    ConnectionGroup::Ref ref = ConnectionGroup::Create(loop.QuitClosure(), base::SequencedTaskRunnerHandle::Get());
    auto group = ref.GetGroupForTesting();
...
TEST_F(ConnectionGroupTest, NotifyOnDecrementToZeroMultipleTimes)
{
    ...
    ConnectionGroup::Ref ref = ConnectionGroup::Create(base::BindLambdaForTesting([&] {
