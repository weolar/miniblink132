...
public:
    ...
    auto TieForCmp() const
    {
        return std::tie(x_, y_, width_, height_);
    }
...
template <> struct hash<mojo::test::RectChromium> {
    size_t operator()(const mojo::test::RectChromium& value)
    {
        // Terrible hash function:
        return (std::hash<int>()(value.x()) ^ std::hash<int>()(value.y()) ^ std::hash<int>()(value.width()) ^ std::hash<int>()(value.height()));
    }
};
