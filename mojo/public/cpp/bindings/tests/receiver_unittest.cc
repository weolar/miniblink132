...
public:
    ...
    explicit ServiceImpl(bool* was_deleted)
        : destruction_callback_(base::BindLambdaForTesting([was_deleted] { *was_deleted = true; }))
    {
    }
...
private:
    ...
    void Frobinate(sample::FooPtr foo, BazOptions options, PendingRemote<sample::Port> port, FrobinateCallback callback) override
    {
...
TEST_P(ReceiverTest, Reset)
{
    ...
    ServiceImpl impl;
    Receiver<sample::Service> receiver(&impl, remote.BindNewPipeAndPassReceiver());
...
TEST_P(ReceiverTest, DestroyClosesMessagePipe)
{
    ...
    base::RunLoop run_loop2;
    {
        ...
        remote->Frobinate(nullptr, sample::Service::BazOptions::REGULAR, NullRemote(), base::BindLambdaForTesting([&](int32_t) {
            run_loop2.Quit();
            called = true;
        }));
        run_loop2.Run();
    ...
    called = false;
    remote->Frobinate(nullptr, sample::Service::BazOptions::REGULAR, NullRemote(), base::BindLambdaForTesting([&](int32_t) { called = true; }));
...
TEST_P(ReceiverTest, ResetDoesntCallDisconnectHandler)
{
    ...
    Receiver<sample::Service> receiver(&impl, remote.BindNewPipeAndPassReceiver());
    bool called = false;
    receiver.set_disconnect_handler(base::BindLambdaForTesting([&] { called = true; }));
    receiver.reset();
    base::RunLoop().RunUntilIdle();
    EXPECT_FALSE(called);
...
public:
    ServiceImplWithReceiver(bool* was_deleted, base::OnceClosure closure, PendingReceiver<sample::Service> receiver)
        : ServiceImpl(was_deleted)
        , receiver_(this, std::move(receiver))
        , closure_(std::move(closure))
    {
        receiver_.set_disconnect_handler(base::BindOnce(&ServiceImplWithReceiver::OnDisconnect, base::Unretained(this)));
    }
...
private:
    ~ServiceImplWithReceiver() override
    {
...
TEST_P(ReceiverTest, SelfDeleteOnDisconnect)
{
    ...
    new ServiceImplWithReceiver(&was_deleted, run_loop.QuitClosure(), remote.BindNewPipeAndPassReceiver());
    remote.reset();
...
public:
    explicit CallbackFilter(const base::RepeatingClosure& will_dispatch_callback, const base::RepeatingClosure& did_dispatch_callback)
    ...
    static std::unique_ptr<CallbackFilter> Wrap(const base::RepeatingClosure& will_dispatch_callback, const base::RepeatingClosure& did_dispatch_callback)
    {
        return std::make_unique<CallbackFilter>(will_dispatch_callback, did_dispatch_callback);
    }
...
TEST_P(ReceiverTest, MessageFilter)
{
    ...
    Receiver<test::PingService> receiver(&impl, remote.BindNewPipeAndPassReceiver());
...
TEST_P(ReceiverTest, DisconnectWithReason)
{
    ...
    base::RunLoop run_loop;
    receiver.set_disconnect_with_reason_handler(base::BindLambdaForTesting([&](uint32_t custom_reason, const std::string& description) {
...
TEST_P(ReceiverTest, PendingRemoteResetWithReason)
{
    ...
    PendingRemote<sample::Service> pending_remote = receiver.BindNewPipeAndPassRemote();
...
template <typename T> struct WeakPtrImplRefTraits {
    using PointerType = base::WeakPtr<T>;

    static bool IsNull(const base::WeakPtr<T>& ptr)
    {
        return !ptr;
    }
    static T* GetRawPointer(base::WeakPtr<T>* ptr)
    {
        return ptr->get();
    }
};

template <typename T> using WeakReceiver = Receiver<T, WeakPtrImplRefTraits<T>>;
...
TEST_P(ReceiverTest, CustomImplPointerType)
{
    ...
    Remote<test::PingService> remote;
    WeakReceiver<test::PingService> receiver(weak_factory.GetWeakPtr(), remote.BindNewPipeAndPassReceiver());
...
TEST_P(ReceiverTest, ReportBadMessage)
{
    ...
    impl.set_ping_handler(base::BindLambdaForTesting([&] { receiver.ReportBadMessage("received bad message"); }));
...
TEST_P(ReceiverTest, GetBadMessageCallback)
{
    ...
    {
        Receiver<test::PingService> receiver(&impl, remote.BindNewPipeAndPassReceiver());
        impl.set_ping_handler(base::BindLambdaForTesting([&] { bad_message_callback = receiver.GetBadMessageCallback(); }));
        remote->Ping(run_loop.QuitClosure());
        run_loop.Run();
...
TEST_P(ReceiverTest, GenericPendingAssociatedReceiver)
{
    ...
    receiver = GenericPendingAssociatedReceiver(remote.BindNewEndpointAndPassReceiver());
    ASSERT_TRUE(receiver.is_valid());
    EXPECT_EQ(sample::Service::Name_, receiver.interface_name());
...
public:
    explicit RebindTestImpl(base::WaitableEvent* event)
        : event_(event)
    {
        DCHECK(event_);
    }
...
TEST_P(ReceiverTest, RebindWithScheduledSyncMessage)
{
    ...
    auto pending_receiver = remote.BindNewPipeAndPassReceiver();
    receiver_thread.task_runner()->PostTask(FROM_HERE, base::BindLambdaForTesting([&]() { receiver1.Bind(std::move(pending_receiver)); }));
    ...
    receiver_thread.task_runner()->PostTask(FROM_HERE, base::BindLambdaForTesting([&]() { receiver2.Bind(receiver1.Unbind()); }));
    event.Signal();
    ...
    // Cleanup
    remote.reset();
    receiver_thread.task_runner()->PostTask(FROM_HERE, base::BindLambdaForTesting([&]() {
        receiver1.reset();
        receiver2.reset();
    }));
    receiver_thread.FlushForTesting();
...
public:
    explicit TestGenericBinderImpl(PendingReceiver<mojom::TestGenericBinder> receiver)
        : receiver_(this, std::move(receiver))
    {
        receiver_.set_disconnect_handler(base::BindOnce(&TestGenericBinderImpl::OnDisconnect, base::Unretained(this)));
    }
    ...
    void WaitForNextAssociatedReceiver(GenericPendingAssociatedReceiver* storage)
    {
        wait_loop_.emplace();
    ...
    void BindOptionalAssociatedReceiver(GenericPendingAssociatedReceiver receiver) override
    {
    ...
    void BindAssociatedReceiver(GenericPendingAssociatedReceiver receiver) override
    {
        if (next_associated_receiver_storage_) {
            *next_associated_receiver_storage_ = std::move(receiver);
            next_associated_receiver_storage_ = nullptr;
...
private:
    ...
    raw_ptr<GenericPendingAssociatedReceiver> next_associated_receiver_storage_ = nullptr;
};
...
TEST_P(ReceiverSerializationTest, NullGenericPendingReceiver)
{
    ...
    remote->BindReceiver(mojo::Remote<mojom::TestInterface1>().BindNewPipeAndPassReceiver());
    binder.WaitForNextReceiver(&receiver);
    EXPECT_TRUE(receiver.is_valid());
    EXPECT_FALSE(receiver.As<mojom::TestInterface2>());
    EXPECT_TRUE(receiver.As<mojom::TestInterface1>());

    remote->BindReceiver(mojo::Remote<mojom::TestInterface2>().BindNewPipeAndPassReceiver());
    binder.WaitForNextReceiver(&receiver);
    EXPECT_TRUE(receiver.is_valid());
...
#if DCHECK_IS_ON()
    EXPECT_EQ(mojo::internal::VALIDATION_ERROR_UNEXPECTED_NULL_POINTER, observer.last_warning());
#endif
...
TEST_P(ReceiverSerializationTest, NullGenericPendingAssociatedReceiver)
{
    ...
    remote->BindAssociatedReceiver(mojo::AssociatedRemote<mojom::TestInterface1>().BindNewEndpointAndPassReceiver());
    binder.WaitForNextAssociatedReceiver(&receiver);
    EXPECT_TRUE(receiver.is_valid());
    EXPECT_FALSE(receiver.As<mojom::TestInterface2>());
    EXPECT_TRUE(receiver.As<mojom::TestInterface1>());

    remote->BindAssociatedReceiver(mojo::AssociatedRemote<mojom::TestInterface2>().BindNewEndpointAndPassReceiver());
    binder.WaitForNextAssociatedReceiver(&receiver);
    EXPECT_TRUE(receiver.is_valid());
    EXPECT_FALSE(receiver.As<mojom::TestInterface1>());
    EXPECT_TRUE(receiver.As<mojom::TestInterface2>());
    ...
    // because the it must never receive the invalid request. Instead the Wait
    // should be terminated by disconnection.
    receiver = mojo::AssociatedRemote<mojom::TestInterface1>().BindNewEndpointAndPassReceiver();
    binder.WaitForNextAssociatedReceiver(&receiver);
    EXPECT_TRUE(receiver.is_valid());
    EXPECT_TRUE(receiver.As<mojom::TestInterface1>());
    EXPECT_FALSE(binder.connected());
...
TEST_P(SelfOwnedReceiverTest, CloseDestroysImplAndPipe)
{
    ...
    auto binding = MakeSelfOwnedReceiver<sample::Service>(std::make_unique<ServiceImpl>(&was_deleted), std::move(receiver));
    remote->Frobinate(nullptr, sample::Service::BazOptions::REGULAR, NullRemote(), base::BindLambdaForTesting([&](int32_t) {
        called = true;
        run_loop2.Quit();
    }));
...
TEST_P(SelfOwnedReceiverTest, DisconnectDestroysImplAndPipe)
{
    ...
    MakeSelfOwnedReceiver<sample::Service>(std::make_unique<ServiceImpl>(base::BindLambdaForTesting([&] {
        was_deleted = true;
        run_loop.Quit();
    })),
...
class MultiprocessReceiverTest : public core::test::MojoTestBase, public mojom::TestInterface1 {
public:
...
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(MultiprocessReceiverClient, MultiprocessReceiverTest, test_pipe)
{
    ...
    base::RunLoop loop;
    InterfaceDropper dropper { PendingReceiver<mojom::InterfaceDropper>(MakeScopedHandle(MessagePipeHandle(dropper_pipe))) };
...
TEST_F(MultiprocessReceiverTest, MultiprocessReceiver)
{
    ...
    RunTestClient("MultiprocessReceiverClient", [&](MojoHandle client) {
        ...
        MojoHandle dropper_pipe = dropper.BindNewPipeAndPassReceiver().PassPipe().release().value();
        WriteMessageWithHandles(client, "", &dropper_pipe, 1);

        constexpr size_t kNumIterations = 1000;
        constexpr size_t kNumReceiversPerIteration = 10;
        for (size_t i = 0; i < kNumIterations; ++i) {
            std::vector<absl::optional<Receiver<mojom::TestInterface1>>> receivers(kNumReceiversPerIteration);
...
INSTANTIATE_TEST_SUITE_P(All, ReceiverSerializationTest, testing::Values(mojo::BindingsTestSerializationMode::kSerializeBeforeSend));
