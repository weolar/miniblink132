...
template <> struct StructTraits<test::SharedTypemappedRectDataView, test::SharedRect> {
    static int x(const test::SharedRect& r)
    {
        return r.x();
    }
    static int y(const test::SharedRect& r)
    {
        return r.y();
    }
    static int width(const test::SharedRect& r)
    {
        return r.width();
    }
    static int height(const test::SharedRect& r)
    {
        return r.height();
    }

    static bool Read(test::SharedTypemappedRectDataView r, test::SharedRect* out)
    {
        out->set_x(r.x());
        out->set_y(r.y());
