...
public:
    TestTaskRunner()
        ...
        , quit_called_(false)
        , task_ready_(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED)
    {
    }
...
template <typename ReceiverType, typename PendingReceiverType> class IntegerSenderImpl : public IntegerSender {
public:
    IntegerSenderImpl(PendingReceiverType receiver, scoped_refptr<base::SequencedTaskRunner> runner)
    ...
    void set_echo_handler(const EchoHandler& handler)
    {
...
public:
    using SenderType = IntegerSenderImpl<AssociatedReceiver<IntegerSender>, PendingAssociatedReceiver<IntegerSender>>;

    explicit IntegerSenderConnectionImpl(PendingReceiver<IntegerSenderConnection> receiver, scoped_refptr<base::SequencedTaskRunner> runner,
    ...
    {
        sender_impl_ = std::make_unique<SenderType>(std::move(receiver), sender_runner_);
    ...
    SenderType* sender_impl()
    {
        return sender_impl_.get();
    }
...
protected:
    void SetUp() override
    {
        ...
        impl_ = std::make_unique<ImplType>(std::move(receiver), receiver_task_runner_);
    }
    ...
    using ImplType = IntegerSenderImpl<Receiver<IntegerSender>, PendingReceiver<IntegerSender>>;
...
protected:
    void SetUp() override
    {
        connection_receiver_task_runner_ = scoped_refptr<TestTaskRunner>(new TestTaskRunner);
        connection_remote_task_runner_ = scoped_refptr<TestTaskRunner>(new TestTaskRunner);
        sender_receiver_task_runner_ = scoped_refptr<TestTaskRunner>(new TestTaskRunner);
        sender_remote_task_runner_ = scoped_refptr<TestTaskRunner>(new TestTaskRunner);

        auto connection_receiver = connection_remote_.BindNewPipeAndPassReceiver(connection_remote_task_runner_);
        connection_impl_
            = std::make_unique<IntegerSenderConnectionImpl>(std::move(connection_receiver), connection_receiver_task_runner_, sender_receiver_task_runner_);

        connection_impl_->set_get_sender_notification(base::BindOnce(&AssociatedBindTaskRunnerTest::QuitTaskRunner, base::Unretained(this)));

        connection_remote_->GetSender(sender_remote_.BindNewEndpointAndPassReceiver(sender_remote_task_runner_));
...
TEST_F(BindTaskRunnerTest, MethodCall)
{
    ...
    impl_->set_echo_handler(base::BindLambdaForTesting([&](int32_t value, IntegerSender::EchoCallback callback) {
...
{
    bool echo_called = false;
    connection_impl_->sender_impl()->set_echo_handler(base::BindLambdaForTesting([&](int32_t value, IntegerSender::EchoCallback callback) {
        EXPECT_EQ(1024, value);
        echo_called = true;
        std::move(callback).Run(value);
    }));
...
TEST_F(AssociatedBindTaskRunnerTest, ReceiverDisconnectHandler)
{
    ...
    connection_impl_->sender_impl()->receiver()->set_disconnect_handler(base::BindLambdaForTesting([&] {
        sender_impl_disconnected = true;
        sender_receiver_task_runner_->Quit();
    }));
    bool connection_impl_disconnected = false;
    connection_impl_->receiver()->set_disconnect_handler(base::BindLambdaForTesting([&] {
