...
public:
    ...
    template <typename Func> void set_ping_handler(Func handler)
    {
        ping_handler_ = base::BindLambdaForTesting(handler);
    }

    using EchoHandler = base::RepeatingCallback<void(int32_t, base::OnceCallback<void(int32_t)>)>;
    template <typename Func> void set_echo_handler(Func handler)
    {
        echo_handler_ = base::BindLambdaForTesting(handler);
    }

    using AsyncEchoHandler = base::RepeatingCallback<void(int32_t, base::OnceCallback<void(int32_t)>)>;
    template <typename Func> void set_async_echo_handler(Func handler)
    {
        async_echo_handler_ = base::BindLambdaForTesting(handler);
    }

    using SendRemoteHandler = base::RepeatingCallback<void(PendingAssociatedRemote<TestSync>)>;
    template <typename Func> void set_send_remote_handler(Func handler)
    {
        send_remote_handler_ = base::BindLambdaForTesting(handler);
    }

    using SendReceiverHandler = base::RepeatingCallback<void(PendingAssociatedReceiver<TestSync>)>;
    template <typename Func> void set_send_receiver_handler(Func handler)
    {
    ...
    void AsyncEchoImpl(int32_t value, base::OnceCallback<void(int32_t)> callback)
    {
...
public:
    ...
    Receiver<TestSyncPrimary>* receiver()
    {
        return &receiver_;
    }
...
public:
    ...
    AssociatedReceiver<TestSync>* receiver()
    {
        return &receiver_;
    }
...
template <typename Interface> struct ImplTraits;

template <> struct ImplTraits<TestSync> {
    using Type = TestSyncImpl;
};

template <> struct ImplTraits<TestSyncPrimary> {
    using Type = TestSyncPrimaryImpl;
};

template <typename Interface> using ImplTypeFor = typename ImplTraits<Interface>::Type;

// A wrapper for either a Remote or SharedRemote that exposes the Remote
// interface.
template <typename Interface> class RemoteWrapper {
public:
...
template <typename InterfaceT, bool use_shared_remote, BindingsTestSerializationMode serialization_mode> struct TestParams {
    ...
    static RemoteWrapper<InterfaceT> Wrap(PendingRemote<Interface> remote)
    {
        if (kIsSharedRemoteTest) {
            return RemoteWrapper<Interface>(SharedRemote<Interface>(std::move(remote)));
        } else {
            return RemoteWrapper<Interface>(Remote<Interface>(std::move(remote)));
        }
    }

    static const BindingsTestSerializationMode kSerializationMode = serialization_mode;
};

template <typename Interface> class TestSyncServiceSequence {
public:
...
template <typename TypeParam> class SyncMethodCommonTest : public SyncMethodTest {
public:
...
protected:
    void SetUp() override
    {
        primary_impl_ = std::make_unique<TestSyncPrimaryImpl>(primary_remote_.BindNewPipeAndPassReceiver());

        impl_pending_sync_receiver_ = impl_pending_sync_remote_.InitWithNewEndpointAndPassReceiver();
        client_pending_sync_receiver_ = client_pending_sync_remote_.InitWithNewEndpointAndPassReceiver();

        primary_impl_->set_send_remote_handler([this](PendingAssociatedRemote<TestSync> remote) { client_pending_sync_remote_ = std::move(remote); });
        base::RunLoop run_loop;
        primary_impl_->set_send_receiver_handler([this, &run_loop](PendingAssociatedReceiver<TestSync> receiver) {
...
void RunTestOnSequencedTaskRunner(std::unique_ptr<SequencedTaskRunnerTestBase> test);
...
protected:
    ...
    {
        return base::BindOnce(&SequencedTaskRunnerTestBase::Done, base::Unretained(this));
    }

private:
    friend void RunTestOnSequencedTaskRunner(std::unique_ptr<SequencedTaskRunnerTestBase> test);
...
template <typename TypeParam> class SequencedTaskRunnerTestLauncher : public testing::Test {
    base::test::TaskEnvironment task_environment;
};

// Similar to SyncMethodCommonTest, but the test body runs on a
// SequencedTaskRunner.
template <typename TypeParam> class SyncMethodOnSequenceCommonTest : public SequencedTaskRunnerTestBase {
public:
    void SetUp() override
    {
        BindingsTestBase::SetupSerializationBehavior(TypeParam::kSerializationMode);
        impl_ = std::make_unique<ImplTypeFor<typename TypeParam::Interface>>(remote_.BindNewPipeAndPassReceiver());
    }
...
void RunTestOnSequencedTaskRunner(std::unique_ptr<SequencedTaskRunnerTestBase> test)
{
    ...
    base::ThreadPool::CreateSequencedTaskRunner({ base::WithBaseSyncPrimitives() })
        ->PostTask(FROM_HERE, base::BindOnce(&SequencedTaskRunnerTestBase::RunTest, base::Unretained(test.release())));
    run_loop.Run();
}
...
using InterfaceTypes = testing::Types<TestParams<TestSync, true, BindingsTestSerializationMode::kSerializeBeforeSend>,
    TestParams<TestSync, false, BindingsTestSerializationMode::kSerializeBeforeSend>,
    TestParams<TestSyncPrimary, true, BindingsTestSerializationMode::kSerializeBeforeSend>,
    TestParams<TestSyncPrimary, false, BindingsTestSerializationMode::kSerializeBeforeSend>,
    TestParams<TestSync, true, BindingsTestSerializationMode::kSerializeBeforeDispatch>,
    TestParams<TestSync, false, BindingsTestSerializationMode::kSerializeBeforeDispatch>,
    TestParams<TestSyncPrimary, true, BindingsTestSerializationMode::kSerializeBeforeDispatch>,
    TestParams<TestSyncPrimary, false, BindingsTestSerializationMode::kSerializeBeforeDispatch>,
    TestParams<TestSync, true, BindingsTestSerializationMode::kNeverSerialize>, TestParams<TestSync, false, BindingsTestSerializationMode::kNeverSerialize>,
    TestParams<TestSyncPrimary, true, BindingsTestSerializationMode::kNeverSerialize>,
    TestParams<TestSyncPrimary, false, BindingsTestSerializationMode::kNeverSerialize>>;
...
#define SEQUENCED_TASK_RUNNER_TYPED_TEST_NAME(fixture_name, name) fixture_name##name##_SequencedTaskRunnerTestSuffix
...
#define SEQUENCED_TASK_RUNNER_TYPED_TEST(fixture_name, name)                                                                                                   \
    ...
    {                                                                                                                                                          \
        RunTestOnSequencedTaskRunner(std::make_unique < SEQUENCED_TASK_RUNNER_TYPED_TEST_NAME(fixture_name, name) < TypeParam >> ());                          \
    }                                                                                                                                                          \
    template <typename TypeParam> void SEQUENCED_TASK_RUNNER_TYPED_TEST_NAME(fixture_name, name)<TypeParam>::Run()

#define SEQUENCED_TASK_RUNNER_TYPED_TEST_F(fixture_name, name)                                                                                                 \
    template <typename TypeParam> class SEQUENCED_TASK_RUNNER_TYPED_TEST_NAME(fixture_name, name);                                                             \
    TYPED_TEST(SequencedTaskRunnerTestLauncher, name)                                                                                                          \
    {                                                                                                                                                          \
        RunTestOnSequencedTaskRunner(std::make_unique < SEQUENCED_TASK_RUNNER_TYPED_TEST_NAME(fixture_name, name) < TypeParam >> ());                          \
    }                                                                                                                                                          \
...
SEQUENCED_TASK_RUNNER_TYPED_TEST(SyncMethodOnSequenceCommonTest, CallSyncMethodAsynchronously)
{
...
TYPED_TEST(SyncMethodCommonTest, BasicSyncCalls)
{
    ...
    service_sequence.task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&TestSyncServiceSequence<Interface>::SetUp, base::Unretained(&service_sequence), std::move(receiver)));
    ...
    service_sequence.task_runner()->PostTaskAndReply(
        FROM_HERE, base::BindOnce(&TestSyncServiceSequence<Interface>::TearDown, base::Unretained(&service_sequence)), run_loop.QuitClosure());
    run_loop.Run();
}
...
SEQUENCED_TASK_RUNNER_TYPED_TEST(SyncMethodOnSequenceCommonTest, ReenteredBySyncMethodReceiver)
{
...
SEQUENCED_TASK_RUNNER_TYPED_TEST(SyncMethodOnSequenceCommonTest, RemoteDestroyedDuringSyncCall)
{
    // Test that it won't result in crash or hang if a remote is destroyed while
    // waiting for a sync call response.
...
SEQUENCED_TASK_RUNNER_TYPED_TEST(SyncMethodOnSequenceCommonTest, ReceiverDestroyedDuringSyncCall)
{
    // Test that it won't result in crash or hang if a receiver is reset
    // (and therefore the message pipe handle is closed) while its corresponding
    // remote is waiting for a sync call response.
...
SEQUENCED_TASK_RUNNER_TYPED_TEST(SyncMethodOnSequenceCommonTest, NestedSyncCallsWithInOrderResponses)
{
    // Test that we can call a sync method on a remote, while there is already a
    // sync call ongoing. The responses arrive in order.
    ...
    bool first_call = true;
    this->impl_->set_echo_handler([&](int32_t value, TestSync::EchoCallback callback) {
        if (first_call) {
            first_call = false;
...
SEQUENCED_TASK_RUNNER_TYPED_TEST(SyncMethodOnSequenceCommonTest, NestedSyncCallsWithOutOfOrderResponses)
{
    // Test that we can call a sync method on a remote while there is already a
...
TYPED_TEST(SyncMethodCommonTest, AsyncResponseQueuedDuringSyncCall)
{
    ...
    impl.set_async_echo_handler([&](int32_t value, TestSync::AsyncEchoCallback callback) {
    ...
    wrapped_remote->AsyncEcho(123, base::BindLambdaForTesting([&](int32_t result) {
...
SEQUENCED_TASK_RUNNER_TYPED_TEST_F(SyncMethodOnSequenceCommonTest, AsyncResponseQueuedDuringSyncCall)
{
    // Test that while a remote is waiting for the response to a sync call, async
    // responses are queued until the sync call completes.
    ...
    void Run() override
    {
        this->impl_->set_async_echo_handler([this](int32_t value, TestSync::AsyncEchoCallback callback) {
            async_echo_request_value_ = value;
            async_echo_request_callback_ = std::move(callback);
            OnAsyncEchoReceived();
        });

        this->remote_->AsyncEcho(123, base::BindLambdaForTesting([&](int32_t result) {
    ...
    {
        this->impl_->set_echo_handler([this](int32_t value, TestSync::EchoCallback callback) {
            // Send back the async response first.
...
SEQUENCED_TASK_RUNNER_TYPED_TEST_F(SyncMethodOnSequenceCommonTest, AsyncRequestQueuedDuringSyncCall)
{
    // Test that while a remote is waiting for the response to a sync call, async
    // requests for a binding running on the same thread are queued until the sync
    // call completes.
...
TYPED_TEST(SyncMethodCommonTest, QueuedMessagesProcessedBeforeErrorNotification)
{
    // Test that while a remote is waiting for the response to a sync call, async
...
SEQUENCED_TASK_RUNNER_TYPED_TEST_F(SyncMethodOnSequenceCommonTest, QueuedMessagesProcessedBeforeErrorNotification)
{
    // Test that while a remote is waiting for the response to a sync call, async
    // responses are queued. If the message pipe is disconnected before the queued
    ...
    void Run() override
    {
        this->impl_->set_async_echo_handler(
            [this](int32_t value, TestSync::AsyncEchoCallback callback) { OnAsyncEchoReachedService(value, std::move(callback)); });
    ...
    void OnAsyncEchoReachedService(int32_t value, TestSync::AsyncEchoCallback callback)
    {
        async_echo_request_value_ = value;
        async_echo_request_callback_ = std::move(callback);
        this->impl_->set_echo_handler([this](int32_t value, TestSync::EchoCallback callback) {
...
TYPED_TEST(SyncMethodCommonTest, InvalidMessageDuringSyncCall)
{
    ...
    PendingRemote<Interface> remote;
    ScopedMessagePipeHandle receiving_handle = remote.InitWithNewPipeAndPassReceiver().PassPipe();
    auto wrapped_remote = TypeParam::Wrap(std::move(remote));

    auto raw_receiving_handle = receiving_handle.get();
    ImplTypeFor<Interface> impl(PendingReceiver<Interface>(std::move(receiving_handle)));

    impl.set_echo_handler([&](int32_t value, TestSync::EchoCallback callback) {
        // Write a 1-byte message, which is considered invalid.
        char invalid_message = 0;
        MojoResult result = WriteMessageRaw(raw_receiving_handle, &invalid_message, 1u, nullptr, 0u, MOJO_WRITE_MESSAGE_FLAG_NONE);
...
SEQUENCED_TASK_RUNNER_TYPED_TEST_F(SyncMethodOnSequenceCommonTest, InvalidMessageDuringSyncCall)
{
    // Test that while a remote is waiting for the response to a sync call, an
    ...
    void Run() override
    {
        MessagePipeHandle raw_receiving_handle = this->impl_->receiver()->internal_state()->handle();
...
TEST_F(SyncMethodAssociatedTest, ReenteredBySyncMethodAssociatedReceiverOfSameRouter)
{
    ...
    AssociatedRemote<TestSync> client_remote(std::move(client_pending_sync_remote_));

    primary_impl_->set_echo_handler([&](int32_t value, TestSyncPrimary::EchoCallback callback) {
        int32_t result_value = -1;
...
TEST_F(SyncMethodAssociatedTest, ReenteredBySyncMethodAssociatedReceiverOfDifferentRouter)
{
    // Test that a remote waiting for a sync call response can be reentered by an
    // associated receiver serving sync methods on the same thread. The associated
    // receiver belongs to the same MultiplexRouter as the waiting remote.
...
private:
    ...
    class PongSender {
    public:
        PongSender(mojom::PongSendMode pong_send_mode, PendingRemote<mojom::Ponger> ponger)
            : pong_send_mode_(pong_send_mode)
            , ponger_(std::move(ponger))
        {
        }

        PongSender(mojom::PongSendMode pong_send_mode, PendingAssociatedRemote<mojom::Ponger> same_pipe_ponger)
            : pong_send_mode_(pong_send_mode)
        ...
        {
            mojom::Ponger& ponger = ponger_.is_bound() ? *ponger_.get() : *same_pipe_ponger_.get();
    ...
    // mojom::Pinger:
    void BindAssociated(PendingAssociatedReceiver<mojom::Pinger> receiver, BindAssociatedCallback callback) override
    {
        associated_receivers_.Add(this, std::move(receiver));
        std::move(callback).Run();
    }

    void SetPonger(mojom::PongSendMode send_mode, PendingRemote<mojom::Ponger> ponger, SetPongerCallback callback) override
    {
        DCHECK(!pong_sender_thread_.IsRunning());
        DCHECK(!pong_sender_);
        pong_sender_thread_.Start();
        pong_sender_ = base::SequenceBound<PongSender>(pong_sender_thread_.task_runner(), send_mode, std::move(ponger));
        std::move(callback).Run();
    }

    void SetSamePipePonger(mojom::PongSendMode send_mode, PendingAssociatedRemote<mojom::Ponger> same_pipe_ponger, SetSamePipePongerCallback callback) override
    {
        DCHECK(!same_pipe_pong_sender_thread_.IsRunning());
        DCHECK(!same_pipe_pong_sender_);
        same_pipe_pong_sender_thread_.Start();
        same_pipe_pong_sender_ = base::SequenceBound<PongSender>(same_pipe_pong_sender_thread_.task_runner(), send_mode, std::move(same_pipe_ponger));
        std::move(callback).Run();
    }
    ...
    void DoPong()
    {
        ...
        base::RepeatingClosure barrier = base::BarrierClosure(3, wait_to_reply.QuitClosure());
        ...
        base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, barrier, base::Milliseconds(10));
        wait_to_reply.Run();
    }
...
public:
    ...
    int num_sync_pongs() const
    {
        return num_sync_pongs_;
    }
    int num_async_pongs() const
    {
        return num_async_pongs_;
    }
    ...
    PendingAssociatedRemote<mojom::Ponger> MakeAssociatedRemote()
    {
        ...
        associated_receivers_.Add(this, remote.InitWithNewEndpointAndPassReceiver());
        return remote;
    }
...
public:
    SyncInterruptTest()
    {
        ...
        receiver_thread_.task_runner()->PostTask(FROM_HERE,
            base::BindOnce(
                [](PendingReceiver<mojom::Pinger> receiver, PendingReceiver<mojom::SimplePinger> simple_receiver,
                    PendingReceiver<mojom::Pinger> shared_receiver) {
                    MakeSelfOwnedReceiver(std::make_unique<PingerImpl>(), std::move(receiver));
                    MakeSelfOwnedReceiver(std::make_unique<PingerImpl>(), std::move(simple_receiver));
                    MakeSelfOwnedReceiver(std::make_unique<PingerImpl>(), std::move(shared_receiver));
                },
                pinger_.BindNewPipeAndPassReceiver(), simple_pinger_.BindNewPipeAndPassReceiver(), shared_remote.InitWithNewPipeAndPassReceiver()));

        shared_pinger_thread_.Start();
        shared_pinger_ = SharedRemote<mojom::Pinger>(std::move(shared_remote), shared_pinger_thread_.task_runner());

        PendingAssociatedRemote<mojom::Pinger> associated_remote;
        CHECK(shared_pinger_->BindAssociated(associated_remote.InitWithNewEndpointAndPassReceiver()));
        shared_associated_pinger_ = SharedAssociatedRemote<mojom::Pinger>(std::move(associated_remote), shared_pinger_thread_.task_runner());
    }
    ...
    mojom::SimplePinger& simple_pinger()
    {
        return *simple_pinger_.get();
    }
    mojom::Pinger& shared_pinger()
    {
        return *shared_pinger_.get();
    }
    ...
    const PongerImpl& ponger() const
    {
        return ponger_;
    }
    const PongerImpl& same_pipe_ponger() const
    {
        return same_pipe_ponger_;
    }
    ...
    {
        pinger_->SetSamePipePonger(send_mode, same_pipe_ponger_.MakeAssociatedRemote());
        shared_pinger_->SetSamePipePonger(send_mode, same_pipe_ponger_.MakeAssociatedRemote());
    }
...
TEST_P(DisableSyncInterruptTest, NoInterruptWhenDisabled)
{
    ...
    base::SequenceBound<SyncService> background_service { background_service_thread.task_runner(), caller.BindNewPipeAndPassReceiver() };

    base::Thread interrupter_thread("Interrupter");
    interrupter_thread.Start();
    interrupter_thread.task_runner()->PostTask(FROM_HERE, base::BindLambdaForTesting([&interrupter] {
...
TEST_P(DisableSyncInterruptTest, SharedRemoteNoInterruptWhenDisabled)
{
    ...
    base::SequenceBound<SyncService> background_service { background_service_thread.task_runner(),
        caller.BindNewPipeAndPassReceiver(background_client_thread.task_runner()) };
...
public:
    ...
    explicit NoSyncImpl(PendingAssociatedReceiver<mojom::NoSync> associated_receiver)
...
public:
    ...
    explicit OneSyncImpl(PendingAssociatedReceiver<mojom::OneSync> associated_receiver)
        : associated_receiver_(this, std::move(associated_receiver))
    {
    }
...
void NoSyncImpl::BindOneSync(PendingAssociatedReceiver<mojom::OneSync> receiver)
{
...
void OneSyncImpl::BindNoSync(PendingAssociatedReceiver<mojom::NoSync> receiver)
{
    associated_no_sync_ = std::make_unique<NoSyncImpl>(std::move(receiver));
}

void OneSyncImpl::BindOneSync(PendingAssociatedReceiver<mojom::OneSync> receiver)
{
    associated_one_sync_ = std::make_unique<OneSyncImpl>(std::move(receiver));
}
...
protected:
    Message MakeNoSyncMethodMessage()
    {
        ...
        ::mojo::internal::MessageFragment<mojom::internal::NoSync_Method_Params_Data> params(message);
        params.Allocate();
        return message;
    }
    ...
    Message MakeOneSyncMethodMessage()
    {
        const uint32_t flags =
            ...
            // a responder.
            Message::kFlagExpectsResponse | GetParam();
        Message message(mojom::internal::kOneSync_Method_Name, flags, 0, 0, nullptr);
        ::mojo::internal::MessageFragment<mojom::internal::NoSync_Method_Params_Data> params(message);
        params.Allocate();
        return message;
    }
    ...
    {
        base::RunLoop run_loop;
        base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, run_loop.QuitClosure());
        run_loop.Run();
    }
...
TEST_P(SyncFlagValidationTest, NonSync)
{
    ...
    ::mojo::internal::SendMojoMessage(*remote.internal_state()->endpoint_client_for_test(), message, std::move(responder));
}
...
TEST_P(SyncFlagValidationTest, NoSyncAssociatedWithNoSync)
{
    ...
    auto responder = std::make_unique<NoResponseExpectedResponder>();
    ASSERT_TRUE(remote.internal_state()->endpoint_client_for_test());
    ::mojo::internal::SendMojoMessage(*associated_remote.internal_state()->endpoint_client_for_test(), message, std::move(responder));
}
...
INSTANTIATE_TEST_SUITE_P(, SyncFlagValidationTest, ::testing::Values(0, Message::kFlagIsResponse));
