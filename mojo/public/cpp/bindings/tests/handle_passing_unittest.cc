...
void RecordString(std::string* storage, base::OnceClosure closure, const std::string& str)
{
    *storage = str;
    std::move(closure).Run();
}

base::OnceCallback<void(const std::string&)> MakeStringRecorder(std::string* storage, base::OnceClosure closure)
{
...
public:
    ImportedInterfaceImpl(PendingReceiver<imported::ImportedInterface> receiver, base::OnceClosure closure)
    ...
    static int do_something_count()
    {
        return do_something_count_;
    }
...
public:
    ...
    void DoStuff(sample::RequestPtr request, ScopedMessagePipeHandle pipe, DoStuffCallback callback) override
    {
        std::string text1;
        if (pipe.is_valid())
    ...
    void DoStuff2(ScopedDataPipeConsumerHandle pipe, DoStuff2Callback callback) override
    {
        ...
        ASSERT_EQ(MOJO_RESULT_OK, mojo::Wait(pipe.get(), MOJO_HANDLE_SIGNAL_READABLE, &state));
        ASSERT_TRUE(state.satisfied_signals & MOJO_HANDLE_SIGNAL_READABLE);
        ASSERT_EQ(MOJO_RESULT_OK, pipe->ReadData(nullptr, &data_size, MOJO_READ_DATA_FLAG_QUERY));
        ASSERT_NE(0, static_cast<int>(data_size));
        char data[64];
        ASSERT_LT(static_cast<int>(data_size), 64);
        ASSERT_EQ(MOJO_RESULT_OK, pipe->ReadData(data, &data_size, MOJO_READ_DATA_FLAG_ALL_OR_NONE));

        std::move(callback).Run(data);
    }

    void CreateNamedObject(PendingReceiver<sample::NamedObject> object_receiver) override
    {
        EXPECT_TRUE(object_receiver.is_valid());
        object_receivers_.Add(std::make_unique<SampleNamedObjectImpl>(), std::move(object_receiver));
    }
    ...
    // interfaces.
    void RequestImportedInterface(PendingReceiver<imported::ImportedInterface> imported, RequestImportedInterfaceCallback callback) override
    {
    }
    void TakeImportedInterface(PendingRemote<imported::ImportedInterface> imported, TakeImportedInterfaceCallback callback) override
    {
    }
...
void DoStuff(bool* got_response, std::string* got_text_reply, base::OnceClosure closure, sample::ResponsePtr response, const std::string& text_reply)
{
...
void DoStuff2(bool* got_response, std::string* got_text_reply, base::OnceClosure closure, const std::string& text_reply)
{
    *got_response = true;
    *got_text_reply = text_reply;
    std::move(closure).Run();
...
TEST_P(HandlePassingTest, Basic)
{
    ...
    ImportedInterfaceImpl imported_impl(imported.InitWithNewPipeAndPassReceiver(), run_loop.QuitClosure());

    sample::RequestPtr request(sample::Request::New(1, std::move(pipe1.handle0), absl::nullopt, std::move(imported)));
    bool got_response = false;
    std::string got_text_reply;
    base::RunLoop run_loop2;
    factory->DoStuff(std::move(request), std::move(pipe0.handle0), base::BindOnce(&DoStuff, &got_response, &got_text_reply, run_loop2.QuitClosure()));
...
TEST_P(HandlePassingTest, PassInvalid)
{
    ...
    sample::RequestPtr request(sample::Request::New(1, ScopedMessagePipeHandle(), absl::nullopt, NullRemote()));

    bool got_response = false;
    std::string got_text_reply;
    base::RunLoop run_loop;
    factory->DoStuff(std::move(request), ScopedMessagePipeHandle(), base::BindOnce(&DoStuff, &got_response, &got_text_reply, run_loop.QuitClosure()));
...
TEST_P(HandlePassingTest, DataPipe)
{
    ...
    MojoCreateDataPipeOptions options = { sizeof(MojoCreateDataPipeOptions), MOJO_CREATE_DATA_PIPE_FLAG_NONE, 1, 1024 };
    ASSERT_EQ(MOJO_RESULT_OK, CreateDataPipe(&options, producer_handle, consumer_handle));
    std::string expected_text_reply = "got it";
    // +1 for \0.
    uint32_t data_size = static_cast<uint32_t>(expected_text_reply.size() + 1);
    ASSERT_EQ(MOJO_RESULT_OK, producer_handle->WriteData(expected_text_reply.c_str(), &data_size, MOJO_WRITE_DATA_FLAG_ALL_OR_NONE));

    bool got_response = false;
    std::string got_text_reply;
    base::RunLoop run_loop;
    factory->DoStuff2(std::move(consumer_handle), base::BindOnce(&DoStuff2, &got_response, &got_text_reply, run_loop.QuitClosure()));
