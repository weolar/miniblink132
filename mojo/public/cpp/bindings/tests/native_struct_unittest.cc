...
class NativeStructTest : public BindingsTestBase, public test::NativeTypeTester {
public:
    NativeStructTest()
...
private:
    ...
    void PassNativeStruct(const test::TestNativeStruct& s, PassNativeStructCallback callback) override
    {
        std::move(callback).Run(s);
    }

    void PassNativeStructWithAttachments(test::TestNativeStructWithAttachments s, PassNativeStructWithAttachmentsCallback callback) override
    {
...
TEST_P(NativeStructTest, NativeStruct)
{
    ...
    base::RunLoop loop;
    remote()->PassNativeStruct(s,
        base::BindOnce(
            [](test::TestNativeStruct* expected_struct, base::RunLoop* loop, const test::TestNativeStruct& passed) {
...
TEST_P(NativeStructTest, NativeStructWithAttachments)
{
    ...
    test::TestNativeStructWithAttachments s(kTestMessage, std::move(pipe.handle0));
    base::RunLoop loop;
    remote()->PassNativeStructWithAttachments(std::move(s),
        base::BindOnce(
            [](const std::string& expected_message, mojo::ScopedMessagePipeHandle peer_pipe, base::RunLoop* loop,
                ...
                WriteMessageRaw(peer_pipe.get(), "ping", 4, nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE);
                auto pipe = passed.PassPipe();
                EXPECT_EQ(MOJO_RESULT_OK, Wait(pipe.get(), MOJO_HANDLE_SIGNAL_READABLE));
                std::vector<uint8_t> bytes;
                EXPECT_EQ(MOJO_RESULT_OK, ReadMessageRaw(pipe.get(), &bytes, nullptr, MOJO_READ_MESSAGE_FLAG_NONE));
