...
template <> struct StructTraits<test::default_construct::mojom::TestStructDataView, test::default_construct::TestStruct> {
    static int value(const test::default_construct::TestStruct& in)
    {
        return in.value;
    }

    static bool Read(test::default_construct::mojom::TestStructDataView in, test::default_construct::TestStruct* out)
    {
        out->value = in.value();
        return true;
    }
