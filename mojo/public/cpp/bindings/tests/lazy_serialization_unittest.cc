...
public:
    explicit TestUnserializedStructImpl(PendingReceiver<test::TestUnserializedStruct> receiver)
        : receiver_(this, std::move(receiver))
    {
    }

    TestUnserializedStructImpl(const TestUnserializedStructImpl&) = delete;
    TestUnserializedStructImpl& operator=(const TestUnserializedStructImpl&) = delete;

    ~TestUnserializedStructImpl() override = default;

    // test::TestUnserializedStruct:
    void PassUnserializedStruct(const test::StructWithUnreachableTraitsImpl& s, PassUnserializedStructCallback callback) override
    {
...
public:
    ...
    void SendForceSerializedStruct(const test::StructForceSerializeImpl& s, SendForceSerializedStructCallback callback) override
    {
        std::move(callback).Run(s);
    }

    void SendNestedForceSerializedStruct(const test::StructNestedForceSerializeImpl& s, SendNestedForceSerializedStructCallback callback) override
    {
        std::move(callback).Run(s);
    }
...
TEST_F(LazySerializationTest, NeverSerialize)
{
    ...
    base::RunLoop loop;
    remote->PassUnserializedStruct(data, base::BindLambdaForTesting([&](const test::StructWithUnreachableTraitsImpl& passed) {
...
TEST_F(LazySerializationTest, ForceSerialize)
{
    ...
    tester->SendForceSerializedStruct(in, base::BindLambdaForTesting([&](const test::StructForceSerializeImpl& passed) {
...
TEST_F(LazySerializationTest, ForceSerializeNested)
{
    ...
    EXPECT_FALSE(in.was_serialized());
    EXPECT_FALSE(in.was_deserialized());
    tester->SendNestedForceSerializedStruct(in, base::BindLambdaForTesting([&](const test::StructNestedForceSerializeImpl& passed) {
