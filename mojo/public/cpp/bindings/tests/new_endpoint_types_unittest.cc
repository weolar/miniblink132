...
public:
    ...
    void AddObserver(mojo::PendingRemote<mojom::WidgetObserver> observer) override
    {
...
public:
    ...
    // mojom::WidgetFactory:
    void CreateWidget(mojo::PendingReceiver<mojom::Widget> receiver, mojo::PendingRemote<mojom::WidgetClient> client) override
    {
        widgets_.Add(std::make_unique<WidgetImpl>(std::move(client)), std::move(receiver));
    }
...
public:
    ...
    mojo::PendingRemote<mojom::WidgetObserver> BindNewPipeAndPassRemote()
    {
        ...
        receiver_.set_disconnect_handler(base::BindOnce(&ObserverImpl::OnDisconnect, base::Unretained(this)));
        return remote;
    }
...
public:
    explicit AssociatedPingerHostImpl(mojo::PendingReceiver<mojom::AssociatedPingerHost> receiver)
        : receiver_(this, std::move(receiver))
    {
    }
    ...
    int ping_count() const
    {
        return pinger_.ping_count();
    }

private:
    // mojom::AssociatedPingerHost:
    void AddEndpoints(mojo::PendingAssociatedReceiver<mojom::Pinger> receiver, mojo::PendingAssociatedRemote<mojom::Pinger> remote) override
    {
...
TEST(NewEndpointTypesTest, BasicUsage)
{
    ...
    factory->CreateWidget(widget.BindNewPipeAndPassReceiver(), client.BindNewPipeAndPassRemote());
...
TEST(NewEndpointTypesTest, AssociatedTypes)
{
    ...
    test_pinger_impl.AddReceiver(test_pinger1.InitWithNewEndpointAndPassReceiver());
    test_pinger_impl.AddReceiver(test_pinger2.InitWithNewEndpointAndPassReceiver());
    ...
    host->AddEndpoints(host_pinger1.BindNewEndpointAndPassReceiver(), std::move(test_pinger1));
    host->AddEndpoints(host_pinger2.BindNewEndpointAndPassReceiver(), std::move(test_pinger2));
