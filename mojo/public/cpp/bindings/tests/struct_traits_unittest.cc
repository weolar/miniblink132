...
template <typename T> void DoExpectResult(const T& expected, base::OnceClosure callback, const T& actual)
{
...
template <typename T> base::OnceCallback<void(const T&)> ExpectResult(const T& r, base::OnceClosure callback)
{
    return base::BindOnce(&DoExpectResult<T>, r, std::move(callback));
}

template <typename T> void DoFail(const std::string& reason, const T&)
{
    EXPECT_TRUE(false) << reason;
}

template <typename T> base::OnceCallback<void(const T&)> Fail(const std::string& reason)
{
    return base::BindOnce(&DoFail<T>, reason);
}

template <typename T> void ExpectError(Remote<T>* proxy, base::OnceClosure callback)
{
...
public:
    ...
    void PassSharedRect(const SharedRect& r, PassSharedRectCallback callback) override
    {
...
protected:
    ...
    {
        chromium_receivers_.Add(&chromium_service_, ConvertPendingReceiver<RectService>(std::move(receiver)));
    }
    ...
    {
        blink_receivers_.Add(&blink_service_, ConvertPendingReceiver<blink::RectService>(std::move(receiver)));
    }
...
private:
    // TraitsTestService:
    void EchoStructWithTraits(const StructWithTraitsImpl& s, EchoStructWithTraitsCallback callback) override
    {
        std::move(callback).Run(s);
    }

    void EchoTrivialStructWithTraits(TrivialStructWithTraitsImpl s, EchoTrivialStructWithTraitsCallback callback) override
    {
        std::move(callback).Run(s);
    }

    void EchoMoveOnlyStructWithTraits(MoveOnlyStructWithTraitsImpl s, EchoMoveOnlyStructWithTraitsCallback callback) override
    {
        std::move(callback).Run(std::move(s));
    }

    void EchoNullableMoveOnlyStructWithTraits(absl::optional<MoveOnlyStructWithTraitsImpl> s, EchoNullableMoveOnlyStructWithTraitsCallback callback) override
    {
        std::move(callback).Run(std::move(s));
    }

    void EchoEnumWithTraits(EnumWithTraitsImpl e, EchoEnumWithTraitsCallback callback) override
    {
    ...
    void EchoUnionWithTraits(std::unique_ptr<test::UnionWithTraitsBase> u, EchoUnionWithTraitsCallback callback) override
    {
...
TEST_F(StructTraitsTest, ChromiumProxyToChromiumService)
{
    ...
    BindToChromiumService(chromium_proxy.BindNewPipeAndPassReceiver());
    {
        ...
        chromium_proxy->GetLargestRect(ExpectResult(RectChromium(1, 1, 4, 5), loop.QuitClosure()));
        loop.Run();
    }
    {
        base::RunLoop loop;
        chromium_proxy->PassSharedRect({ 1, 2, 3, 4 }, ExpectResult(SharedRect({ 1, 2, 3, 4 }), loop.QuitClosure()));
        loop.Run();
    }
}
...
TEST_F(StructTraitsTest, ChromiumToBlinkService)
{
    ...
    BindToBlinkService(chromium_proxy.BindNewPipeAndPassReceiver());
    {
        ...
        chromium_proxy->GetLargestRect(ExpectResult(RectChromium(2, 2, 5, 5), loop.QuitClosure()));
        loop.Run();
    }
    {
        base::RunLoop loop;
    ...
    // deserializer rejects negative origins.
    {
        ...
        chromium_proxy->AddRect(RectChromium(-1, -1, 2, 2));
        chromium_proxy->GetLargestRect(Fail<RectChromium>("The pipe should have been closed."));
        loop.Run();
    }
}
...
TEST_F(StructTraitsTest, BlinkProxyToBlinkService)
{
    ...
    BindToBlinkService(blink_proxy.BindNewPipeAndPassReceiver());
    {
        ...
        blink_proxy->GetLargestRect(ExpectResult(RectBlink(10, 10, 20, 20), loop.QuitClosure()));
        loop.Run();
    }
    {
        base::RunLoop loop;
        blink_proxy->PassSharedRect({ 4, 3, 2, 1 }, ExpectResult(SharedRect({ 4, 3, 2, 1 }), loop.QuitClosure()));
        loop.Run();
    }
}
...
TEST_F(StructTraitsTest, BlinkProxyToChromiumService)
{
    ...
    BindToChromiumService(blink_proxy.BindNewPipeAndPassReceiver());
    {
        ...
        blink_proxy->GetLargestRect(ExpectResult(RectBlink(1, 1, 4, 5), loop.QuitClosure()));
        loop.Run();
    }
    {
        base::RunLoop loop;
...
void ExpectStructWithTraits(const StructWithTraitsImpl& expected, base::OnceClosure closure, const StructWithTraitsImpl& passed)
{
...
TEST_F(StructTraitsTest, EchoStructWithTraits)
{
    ...
    proxy->EchoStructWithTraits(input, base::BindOnce(&ExpectStructWithTraits, input, loop.QuitClosure()));
    loop.Run();
}
...
void ExpectTrivialStructWithTraits(TrivialStructWithTraitsImpl expected, base::OnceClosure closure, TrivialStructWithTraitsImpl passed)
{
...
TEST_F(StructTraitsTest, EchoTrivialStructWithTraits)
{
    ...
    proxy->EchoTrivialStructWithTraits(input, base::BindOnce(&ExpectTrivialStructWithTraits, input, loop.QuitClosure()));
    loop.Run();
}

void CaptureMessagePipe(ScopedMessagePipeHandle* storage, base::OnceClosure closure, MoveOnlyStructWithTraitsImpl passed)
{
    storage->reset(MessagePipeHandle(passed.get_mutable_handle().release().value()));
    std::move(closure).Run();
}
...
TEST_F(StructTraitsTest, EchoMoveOnlyStructWithTraits)
{
    ...
    proxy->EchoMoveOnlyStructWithTraits(std::move(input), base::BindOnce(&CaptureMessagePipe, &received, loop.QuitClosure()));
    loop.Run();
    ...
    EXPECT_EQ(MOJO_RESULT_OK, ReadMessageRaw(received.get(), &bytes, &handles, MOJO_READ_MESSAGE_FLAG_NONE));
...
void CaptureNullableMoveOnlyStructWithTraitsImpl(
    absl::optional<MoveOnlyStructWithTraitsImpl>* storage, base::OnceClosure closure, absl::optional<MoveOnlyStructWithTraitsImpl> passed)
{
...
TEST_F(StructTraitsTest, EchoNullableMoveOnlyStructWithTraits)
{
    ...
    proxy->EchoNullableMoveOnlyStructWithTraits(absl::nullopt, base::BindOnce(&CaptureNullableMoveOnlyStructWithTraitsImpl, &received, loop.QuitClosure()));
    loop.Run();

    EXPECT_FALSE(received);
}

void ExpectEnumWithTraits(EnumWithTraitsImpl expected_value, base::OnceClosure closure, EnumWithTraitsImpl value)
{
...
TEST_F(StructTraitsTest, EchoEnumWithTraits)
{
    ...
    proxy->EchoEnumWithTraits(
        EnumWithTraitsImpl::CUSTOM_VALUE_1, base::BindOnce(&ExpectEnumWithTraits, EnumWithTraitsImpl::CUSTOM_VALUE_1, loop.QuitClosure()));
    loop.Run();
}
...
void ExpectUniquePtr(std::unique_ptr<int> expected, base::OnceClosure closure, std::unique_ptr<int> value)
{
...
TEST_F(StructTraitsTest, TypemapUniquePtr)
{
...

    {
        ...
        proxy->EchoStructWithTraitsForUniquePtr(
            std::make_unique<int>(12345), base::BindOnce(&ExpectUniquePtr, std::make_unique<int>(12345), loop.QuitClosure()));
        loop.Run();
    }
    {
        base::RunLoop loop;
        proxy->EchoNullableStructWithTraitsForUniquePtr(nullptr, base::BindOnce(&ExpectUniquePtr, nullptr, loop.QuitClosure()));
        loop.Run();
    }
}
...
TEST_F(StructTraitsTest, EchoUnionWithTraits)
{
    ...
    {
        std::unique_ptr<test::UnionWithTraitsBase> input(new test::UnionWithTraitsInt32(1234));
        base::RunLoop loop;
        proxy->EchoUnionWithTraits(std::move(input),
            base::BindOnce(
                [](base::OnceClosure quit_closure, std::unique_ptr<test::UnionWithTraitsBase> passed) {
                    ASSERT_EQ(test::UnionWithTraitsBase::Type::INT32, passed->type());
                    EXPECT_EQ(1234, static_cast<test::UnionWithTraitsInt32*>(passed.get())->value());
    ...
    {
        std::unique_ptr<test::UnionWithTraitsBase> input(new test::UnionWithTraitsStruct(4321));
        base::RunLoop loop;
        proxy->EchoUnionWithTraits(std::move(input),
            base::BindOnce(
                [](base::OnceClosure quit_closure, std::unique_ptr<test::UnionWithTraitsBase> passed) {
                    ASSERT_EQ(test::UnionWithTraitsBase::Type::STRUCT, passed->type());
                    EXPECT_EQ(4321, static_cast<test::UnionWithTraitsStruct*>(passed.get())->get_struct().value);
                    std::move(quit_closure).Run();
                },
                loop.QuitClosure()));
        loop.Run();
