...
public:
    ...
    void SetUp() override
    {
        MessagePipe pipe;
        router0_
            = MultiplexRouter::CreateAndStartReceiving(std::move(pipe.handle0), MultiplexRouter::MULTI_INTERFACE, false, base::ThreadTaskRunnerHandle::Get());
        router1_
            = MultiplexRouter::CreateAndStartReceiving(std::move(pipe.handle1), MultiplexRouter::MULTI_INTERFACE, true, base::ThreadTaskRunnerHandle::Get());
        ScopedInterfaceEndpointHandle::CreatePairPendingAssociation(&endpoint0_, &endpoint1_);
...
{
    InterfaceEndpointClient client0(std::move(endpoint0_), nullptr, std::make_unique<PassThroughFilter>(), {}, base::ThreadTaskRunnerHandle::Get(), 0u,
        kTestInterfaceName, MessageToMethodInfo, MessageToMethodName);
    ResponseGenerator generator;
    InterfaceEndpointClient client1(std::move(endpoint1_), &generator, std::make_unique<PassThroughFilter>(), {}, base::ThreadTaskRunnerHandle::Get(), 0u,
        kTestInterfaceName, MessageToMethodInfo, MessageToMethodName);
    ...
    client0.AcceptWithResponder(&request, std::make_unique<MessageAccumulator>(&message_queue, run_loop.QuitClosure()));
    ...
    EXPECT_EQ(std::string("hello world!"), std::string(reinterpret_cast<const char*>(response.payload())));
    ...
    client0.AcceptWithResponder(&request2, std::make_unique<MessageAccumulator>(&message_queue, run_loop2.QuitClosure()));
    ...
    EXPECT_EQ(std::string("hello again world!"), std::string(reinterpret_cast<const char*>(response.payload())));
}
...
TEST_F(MultiplexRouterTest, BasicRequestResponse_Synchronous)
{
    ...
    MessageQueue message_queue;
    client0.AcceptWithResponder(&request, std::make_unique<MessageAccumulator>(&message_queue));
    ...
    client0.AcceptWithResponder(&request2, std::make_unique<MessageAccumulator>(&message_queue));
...
{
    InterfaceEndpointClient client0(std::move(endpoint0_), nullptr, base::WrapUnique(new PassThroughFilter()), {}, base::ThreadTaskRunnerHandle::Get(), 0u,
        kTestInterfaceName, MessageToMethodInfo, MessageToMethodName);
    base::RunLoop run_loop;
    LazyResponseGenerator generator(run_loop.QuitClosure());
    InterfaceEndpointClient client1(std::move(endpoint1_), &generator, base::WrapUnique(new PassThroughFilter()), {}, base::ThreadTaskRunnerHandle::Get(), 0u,
        kTestInterfaceName, MessageToMethodInfo, MessageToMethodName);
    ...
    MessageQueue message_queue;
    base::RunLoop run_loop2;
    client0.AcceptWithResponder(&request, std::make_unique<MessageAccumulator>(&message_queue, run_loop2.QuitClosure()));
    run_loop.Run();
    ...
    client0.AcceptWithResponder(&request2, std::make_unique<MessageAccumulator>(&message_queue, run_loop4.QuitClosure()));
    run_loop3.Run();
...
TEST_F(MultiplexRouterTest, MissingResponses)
{
    ...
    client0.set_connection_error_handler(base::BindOnce(&ForwardErrorHandler, &error_handler_called0, run_loop0.QuitClosure()));
    ...
    client1.set_connection_error_handler(base::BindOnce(&ForwardErrorHandler, &error_handler_called1, run_loop1.QuitClosure()));
