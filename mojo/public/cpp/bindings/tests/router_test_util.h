...
public:
    MessageAccumulator(MessageQueue* queue, base::OnceClosure closure = base::OnceClosure());
...
public:
    ...
    bool SendResponse(uint32_t name, uint64_t request_id, const char* request_string, MessageReceiver* responder);
};

class LazyResponseGenerator : public ResponseGenerator {
public:
    explicit LazyResponseGenerator(base::OnceClosure closure = base::OnceClosure());
