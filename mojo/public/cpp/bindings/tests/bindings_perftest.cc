...
public:
    ...
    void Run(unsigned int iterations)
    {
        ...
        remote_->Ping(base::BindOnce(&PingPongTest::OnPingDone, base::Unretained(this)));
        run_loop.Run();
    }
...
struct BoundPingService {
    BoundPingService()
        : receiver(&impl, remote.BindNewPipeAndPassReceiver())
    {
    }
...
TEST_F(MojoBindingsPerftest, InProcessPingPong)
{
    ...
    PingServiceImpl impl;
    Receiver<test::PingService> receiver(&impl, remote.InitWithNewPipeAndPassReceiver());
...

    {
        ...
        test::LogPerfResult("InProcessPingPong", "0_Inactive", kIterations / MojoTicksToSeconds(end_time - start_time), "pings/second");
    }

    {
        const size_t kNumInactiveServices = 1000;
        BoundPingService* inactive_services = new BoundPingService[kNumInactiveServices];

        const unsigned int kIterations = 10000;
        const MojoTimeTicks start_time = MojoGetTimeTicksNow();
        test.Run(kIterations);
        const MojoTimeTicks end_time = MojoGetTimeTicksNow();
        test::LogPerfResult("InProcessPingPong", "1000_Inactive", kIterations / MojoTicksToSeconds(end_time - start_time), "pings/second");
...
public:
    PingPongPaddle(MessageReceiver* sender)
        : sender_(sender)
    {
    }

    void set_sender(MessageReceiver* sender)
    {
        sender_ = sender;
    }
    ...
    bool AcceptWithResponder(Message* message, std::unique_ptr<MessageReceiverWithStatus> responder) override
    {
        NOTREACHED();
        return true;
    }
...
{
    MessagePipe pipe;
    scoped_refptr<internal::MultiplexRouter> router0(internal::MultiplexRouter::CreateAndStartReceiving(
        std::move(pipe.handle0), internal::MultiplexRouter::SINGLE_INTERFACE, true, base::ThreadTaskRunnerHandle::Get()));
    scoped_refptr<internal::MultiplexRouter> router1(internal::MultiplexRouter::CreateAndStartReceiving(
    ...
    InterfaceEndpointClient client0(router0->CreateLocalEndpointHandle(kPrimaryInterfaceId), &paddle0, nullptr, {}, base::ThreadTaskRunnerHandle::Get(), 0u,
        kTestInterfaceName, MessageToMethodInfo, MessageToMethodName);
    InterfaceEndpointClient client1(router1->CreateLocalEndpointHandle(kPrimaryInterfaceId), &paddle1, nullptr, {}, base::ThreadTaskRunnerHandle::Get(), 0u,
        kTestInterfaceName, MessageToMethodInfo, MessageToMethodName);
    ...
    test::LogPerfResult("MultiplexRouterPingPong", nullptr, kTestIterations / duration.InSecondsF(), "pings/second");
}
...
TEST_F(MojoBindingsPerftest, MultiplexRouterDispatchCost)
{
    ...
    scoped_refptr<internal::MultiplexRouter> router
        = internal::MultiplexRouter::Create(std::move(pipe.handle0), internal::MultiplexRouter::SINGLE_INTERFACE, true, base::ThreadTaskRunnerHandle::Get());
    CounterReceiver receiver;
    InterfaceEndpointClient client(router->CreateLocalEndpointHandle(kPrimaryInterfaceId), &receiver, nullptr, {}, base::ThreadTaskRunnerHandle::Get(), 0u,
        kTestInterfaceName, MessageToMethodInfo, MessageToMethodName);
    ...
    for (size_t i = 0; i < 2; ++i) {
        ...
        for (size_t j = 0; j < kIterations[i]; ++j) {
            ...
            bool result = router->SimulateReceivingMessageForTesting(message.TakeMojoMessage());
            DCHECK(result);
        }
        ...
        if (i == 1) {
            test::LogPerfResult("MultiplexRouterDispatchCost", nullptr, kIterations[i] / duration.InSecondsF(), "times/second");
        }
    }
}
