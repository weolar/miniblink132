...
public:
    ...
    TestNativeStructWithAttachments(const std::string& message, ScopedMessagePipeHandle pipe);

    TestNativeStructWithAttachments(const TestNativeStructWithAttachments&) = delete;
    TestNativeStructWithAttachments& operator=(const TestNativeStructWithAttachments&) = delete;

    ~TestNativeStructWithAttachments();

    TestNativeStructWithAttachments& operator=(TestNativeStructWithAttachments&& other);
    ...
    void set_pipe(mojo::ScopedMessagePipeHandle pipe)
    {
        pipe_ = std::move(pipe);
    }
    mojo::ScopedMessagePipeHandle PassPipe() const
    {
...
template <> struct ParamTraits<mojo::test::TestNativeStruct> {
...
template <> struct ParamTraits<mojo::test::TestNativeStructWithAttachments> {
