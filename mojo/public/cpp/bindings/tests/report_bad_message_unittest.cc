...
public:
    ...
    void SetUp() override
    {
        mojo::SetDefaultProcessErrorHandler(base::BindRepeating(&ReportBadMessageTest::OnProcessError, base::Unretained(this)));
    ...
    TestBadMessages* remote()
    {
        return remote_.get();
    }

    TestBadMessagesImpl* impl()
    {
        return &impl_;
    }
...
TEST_P(ReportBadMessageTest, ResponseAsync)
{
    ...
    // handler immediately.
    std::move(bad_message_callback).Run("this message is bad and should feel bad");
    EXPECT_TRUE(error);
}
