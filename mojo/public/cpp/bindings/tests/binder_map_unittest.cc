...
public:
    ...
    void Bind(scoped_refptr<base::SequencedTaskRunner> expected_task_runner, mojo::PendingReceiver<mojom::TestInterface1> receiver)
    {
...
public:
    ...
    void Bind(scoped_refptr<base::SequencedTaskRunner> expected_task_runner, mojo::PendingReceiver<mojom::TestInterface2> receiver)
    {
        if (expected_task_runner)
            EXPECT_TRUE(expected_task_runner->RunsTasksInCurrentSequence());
        receiver_.Bind(std::move(receiver));
...
TEST_F(BinderMapTest, BasicMatch)
{
    ...
    BinderMap map;
    map.Add<mojom::TestInterface1>(base::BindRepeating(&TestInterface1Impl::Bind, base::Unretained(&impl), nullptr), base::SequencedTaskRunnerHandle::Get());
...
TEST_F(BinderMapTest, WithContext)
{
    ...
    map.Add<mojom::TestInterface1>(base::BindRepeating(
        [](TestInterface1Impl* impl, int* expected_context, int* context, mojo::PendingReceiver<mojom::TestInterface1> receiver) {
...
TEST_F(BinderMapTest, CorrectSequence)
{
    ...
    BinderMap map;
    map.Add<mojom::TestInterface1>(base::BindRepeating(&TestInterface1Impl::Bind, base::Unretained(&impl1), task_runner1), task_runner1);
    map.Add<mojom::TestInterface2>(base::BindRepeating(&TestInterface2Impl::Bind, base::Unretained(impl2.get()), task_runner2), task_runner2);
