...
template <> struct TypeConverter<int32_t, sample::BarPtr> {
    static int32_t Convert(const sample::BarPtr& bar)
    {
        return static_cast<int32_t>(bar->alpha) << 16 | static_cast<int32_t>(bar->beta) << 8 | static_cast<int32_t>(bar->gamma);
    }
};
...
FooPtr MakeFoo()
{
    ...
    return Foo::New(name, 1, 2, false, true, false, std::move(bar), std::move(extra_bars), std::move(data), std::move(pipe.handle1), std::move(input_streams),
        std::move(output_streams), std::move(array_of_array_of_bools), absl::nullopt, absl::nullopt);
}
...
template <typename H> void Print(int depth, const char* name, const mojo::ScopedHandleBase<H>& value)
{
    PrintSpacer(depth);
...
template <typename T> void Print(int depth, const char* name, const std::vector<T>& array)
{
    PrintSpacer(depth);
    std::cout << name << ":" << std::endl;
    ++depth;
...
template <typename T> void Print(int depth, const char* name, const absl::optional<std::vector<T>>& array)
{
    if (array)
...
void DumpHex(const uint8_t* bytes, size_t num_bytes)
{
    for (size_t i = 0; i < num_bytes; ++i) {
        std::cout << std::setw(2) << std::setfill('0') << std::hex << uint32_t(bytes[i]);
...
public:
    void Frobinate(FooPtr foo, BazOptions baz, mojo::PendingRemote<Port> pending_port, Service::FrobinateCallback callback) override
    {
...
public:
    ...
    bool Accept(mojo::Message* message) override
    {
        ...
        if (g_dump_message_as_hex) {
            DumpHex(reinterpret_cast<const uint8_t*>(message->data()), message->data_num_bytes());
        }
    ...
    bool AcceptWithResponder(mojo::Message* message, std::unique_ptr<mojo::MessageReceiver> responder) override
    {
        return false;
    }
};
...
TEST_P(BindingsSampleTest, Basic)
{
    ...
    service->Frobinate(std::move(foo), Service::BazOptions::EXTRA, mojo::NullRemote(), Service::FrobinateCallback());
