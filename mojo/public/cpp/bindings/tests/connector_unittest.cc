...
public:
    ...
    Message CreateMessage(const char* text, std::vector<ScopedHandle> handles = std::vector<ScopedHandle>())
    {
...
{
    Connector connector0(std::move(handle0_), Connector::SINGLE_THREADED_SEND, base::ThreadTaskRunnerHandle::Get());
    Connector connector1(std::move(handle1_), Connector::SINGLE_THREADED_SEND, base::ThreadTaskRunnerHandle::Get());
    ...
    EXPECT_EQ(std::string(kText), std::string(reinterpret_cast<const char*>(message_received.payload())));
}
...
TEST_F(ConnectorTest, Basic_TwoMessages)
{
    ...
    for (size_t i = 0; i < std::size(kText); ++i) {
        ...
        EXPECT_EQ(std::string(kText[i]), std::string(reinterpret_cast<const char*>(message_received.payload())));
    }
}
...
TEST_F(ConnectorTest, Basic_TwoMessages_Synchronous)
{
    ...
    EXPECT_EQ(std::string(kText[0]), std::string(reinterpret_cast<const char*>(message_received.payload())));
...
TEST_F(ConnectorTest, MessageWithHandles)
{
    ...
    // to the orginal pipe.
    auto pipe_handle = ScopedMessagePipeHandle::From(std::move(message_received.mutable_handles()->front()));
    Connector connector_received(std::move(pipe_handle), Connector::SINGLE_THREADED_SEND, base::ThreadTaskRunnerHandle::Get());
    Connector connector_original(std::move(pipe.handle1), Connector::SINGLE_THREADED_SEND, base::ThreadTaskRunnerHandle::Get());
...
{
    Connector connector0(std::move(handle0_), Connector::SINGLE_THREADED_SEND, base::ThreadTaskRunnerHandle::Get());
    Connector* connector1 = new Connector(std::move(handle1_), Connector::SINGLE_THREADED_SEND, base::ThreadTaskRunnerHandle::Get());
...
TEST_F(ConnectorTest, RaiseError)
{
    ...
    connector0.set_connection_error_handler(base::BindOnce(&ForwardErrorHandler, &error_handler_called0, run_loop.QuitClosure()));

    Connector connector1(std::move(handle1_), Connector::SINGLE_THREADED_SEND, base::ThreadTaskRunnerHandle::Get());
    bool error_handler_called1 = false;
    connector1.set_connection_error_handler(base::BindOnce(&ForwardErrorHandler, &error_handler_called1, run_loop2.QuitClosure()));
...
void PauseConnectorAndRunClosure(Connector* connector, base::OnceClosure closure)
{
...
TEST_F(ConnectorTest, PauseWithQueuedMessages)
{
    ...
    // received.
    MessageAccumulator accumulator(base::BindOnce(&PauseConnectorAndRunClosure, &connector1, run_loop.QuitClosure()));
    connector1.set_incoming_receiver(&accumulator);
...
void AccumulateWithNestedLoop(MessageAccumulator* accumulator, base::OnceClosure closure)
{
...
TEST_F(ConnectorTest, ProcessWhenNested)
{
    ...
    accumulator.set_closure(base::BindOnce(&AccumulateWithNestedLoop, &accumulator, run_loop.QuitClosure()));
    connector1.set_incoming_receiver(&accumulator);
...
{
    std::unique_ptr<Connector> connector(new Connector(std::move(handle0_), Connector::SINGLE_THREADED_SEND, base::ThreadTaskRunnerHandle::Get()));
    ...
    another_thread.task_runner()->PostTaskAndReply(
        FROM_HERE, base::BindOnce([](std::unique_ptr<Connector> connector) { connector.reset(); }, std::move(connector)), run_loop.QuitClosure());
