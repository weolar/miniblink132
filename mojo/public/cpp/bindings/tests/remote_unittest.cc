...
public:
    ...
    Receiver<math::Calculator>& receiver()
    {
        return receiver_;
    }
...
public:
    ...
    {
        return calculator_.is_connected();
    }
    ...
    {
        calculator_->Add(value, base::BindOnce(&MathCalculatorUI::Output, base::Unretained(this), std::move(closure)));
    }

    void Multiply(double value, base::OnceClosure closure)
    {
        calculator_->Multiply(value, base::BindOnce(&MathCalculatorUI::Output, base::Unretained(this), std::move(closure)));
    }
...
public:
    explicit SelfDestructingMathCalculatorUI(PendingRemote<math::Calculator> calculator)
        : calculator_(std::move(calculator))
        , nesting_level_(0)
    {
        ++num_instances_;
    ...
    void BeginTest(bool nested, base::OnceClosure closure)
    {
        ...
        calculator_->Add(1.0, base::BindOnce(&SelfDestructingMathCalculatorUI::Output, base::Unretained(this), std::move(closure)));
    }
...
private:
    ~SelfDestructingMathCalculatorUI()
    {
        --num_instances_;
    }
...
public:
    ...
    void Frobinate(sample::FooPtr foo, sample::Service::BazOptions baz, PendingRemote<sample::Port> port, sample::Service::FrobinateCallback callback) override
    {
...
public:
    ...
    void set_closure(base::OnceClosure closure)
    {
...
public:
    void RunTest(const scoped_refptr<base::SequencedTaskRunner> runner)
    {
        ...
        runner->PostTask(FROM_HERE, base::BindOnce(&EndToEndRemoteTest::RunTestImpl, base::Unretained(this)));
        run_loop.Run();
    }
...
private:
    void RunTestImpl()
    {
        ...
        calc_impl_ = std::make_unique<MathCalculatorImpl>(calc.InitWithNewPipeAndPassReceiver());
        calculator_ui_ = std::make_unique<MathCalculatorUI>(std::move(calc));
        calculator_ui_->Add(2.0, base::BindOnce(&EndToEndRemoteTest::AddDone, base::Unretained(this)));
        calculator_ui_->Multiply(5.0, base::BindOnce(&EndToEndRemoteTest::MultiplyDone, base::Unretained(this)));
        EXPECT_EQ(0.0, calculator_ui_->GetOutput());
    }

    void AddDone()
    {
        EXPECT_EQ(2.0, calculator_ui_->GetOutput());
    }
...
TEST_P(RemoteTest, DestroyRemoteOnMethodResponse)
{
    ...
    SelfDestructingMathCalculatorUI* impl = new SelfDestructingMathCalculatorUI(std::move(remote));
    base::RunLoop run_loop;
...
TEST_P(RemoteTest, ReentrantWaitForIncomingCall)
{
    ...
    remote->Frobinate(nullptr, sample::Service::BazOptions::REGULAR, NullRemote(), base::BindLambdaForTesting([&](int) { run_loop.Quit(); }));
    remote->Frobinate(nullptr, sample::Service::BazOptions::REGULAR, NullRemote(), base::BindLambdaForTesting([&](int) { run_loop2.Quit(); }));
...
TEST_P(RemoteTest, QueryVersion)
{
    ...
    Receiver<sample::IntegerAccessor> receiver(&impl, remote.BindNewPipeAndPassReceiver());
...
TEST(StrongConnectorTest, Math)
{
    ...
    receivers.Add(std::make_unique<StrongMathCalculatorImpl>(&destroyed), calc.InitWithNewPipeAndPassReceiver());
...
public:
    WeakMathCalculatorImpl(PendingReceiver<math::Calculator> receiver, bool* disconnected, bool* destroyed, base::OnceClosure closure)
        : destroyed_(destroyed)
        , receiver_(this, std::move(receiver))
    {
...
TEST(WeakConnectorTest, Math)
{
    ...
    MessagePipe pipe;
    base::RunLoop run_loop;
    WeakMathCalculatorImpl impl(PendingReceiver<math::Calculator>(std::move(pipe.handle0)), &disconnected, &destroyed, run_loop.QuitClosure());

    {
        MathCalculatorUI calculator_ui(PendingRemote<math::Calculator>(std::move(pipe.handle1), 0u));
...
TEST_P(RemoteTest, Fusion)
{
    ...
    EXPECT_TRUE(FusePipes(remote.BindNewPipeAndPassReceiver(), std::move(pending_remote)));
...
TEST_P(RemoteTest, FlushForTestingWithClosedPeer)
{
    ...
    bool called = false;
    calc.set_disconnect_handler(base::BindLambdaForTesting([&] { called = true; }));
...
TEST_P(RemoteTest, DisconnectWithReason)
{
    ...
    base::RunLoop run_loop;
    calc.set_disconnect_with_reason_handler(base::BindLambdaForTesting([&](uint32_t custom_reason, const std::string& description) {
...
TEST_P(RemoteTest, CallbackIsPassedRemote)
{
    ...
    raw_proxy->Ping(base::BindOnce([](Remote<sample::PingTest>) {}, std::move(remote)));
...
TEST_P(RemoteTest, SharedRemoteWithTaskRunner)
{
    ...
    SharedRemote<math::Calculator> shared_remote(std::move(remote), other_thread_task_runner);
...
public:
    ...
    void Bind(PendingAssociatedReceiver<mojom::SequenceChecker> receiver) override
    {
        receivers_.Add(this, std::move(receiver));
    }

    void AddClient(PendingAssociatedRemote<mojom::SequenceCheckerClient> client) override
    {
...
TEST_P(RemoteTest, SharedRemotePassAssociatedEndpointsEarly)
{
    ...
    other_thread_task_runner->PostTask(FROM_HERE,
        base::BindOnce(
            [](PendingReceiver<mojom::SequenceChecker> receiver) { MakeSelfOwnedReceiver(std::make_unique<SequenceCheckerImpl>(), std::move(receiver)); },
            remote.InitWithNewPipeAndPassReceiver()));

    SharedRemote<mojom::SequenceChecker> checker(std::move(remote), other_thread_task_runner);
    PendingAssociatedRemote<mojom::SequenceChecker> pending_associated_checker;
    checker->Bind(pending_associated_checker.InitWithNewEndpointAndPassReceiver());

    SharedAssociatedRemote<mojom::SequenceChecker> associated_checker
        = mojo::SharedAssociatedRemote<mojom::SequenceChecker>(std::move(pending_associated_checker), other_thread_task_runner);

    PendingAssociatedRemote<mojom::SequenceChecker> later_pending_associated_checker;
    auto later_associated_receiver = later_pending_associated_checker.InitWithNewEndpointAndPassReceiver();
    SharedAssociatedRemote<mojom::SequenceChecker> later_associated_checker
        = mojo::SharedAssociatedRemote<mojom::SequenceChecker>(std::move(later_pending_associated_checker), other_thread_task_runner);
...
TEST_P(RemoteTest, DISABLED_DisconnectDuringOffThreadSyncWaitWithUnprocessedTasks)
{
    ...
    const scoped_refptr<base::SequencedTaskRunner> impl_sequence = base::ThreadPool::CreateSequencedTaskRunner({});
    const scoped_refptr<base::SequencedTaskRunner> associated_sequence = base::ThreadPool::CreateSequencedTaskRunner({});
    ...
    impl_sequence->PostTask(FROM_HERE, base::BindLambdaForTesting([&] {
        ...
        impl->set_quit_callback(base::BindLambdaForTesting([&] { checkers.Clear(); }));
        checkers.Add(std::move(impl), remote.BindNewPipeAndPassReceiver(impl_sequence));
    ...
    remote->Bind(associated_remote.BindNewEndpointAndPassReceiver(associated_sequence));
...
TEST_P(RemoteTest, SharedRemoteDisconnectCallback)
{
    ...
    const scoped_refptr<base::SequencedTaskRunner> main_task_runner = base::ThreadPool::CreateSequencedTaskRunner({});
    SharedRemote<math::Calculator> shared_remote(std::move(remote), main_task_runner);
...
public:
    ...
    void Fetch(FetchCallback callback) override
    {
        ...
        // by the test below.
        base::SequencedTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(callback), kMagicNumber));
    }
};
...
TEST_P(RemoteTest, SharedRemoteSyncOnlyBlocksCallingSequence)
{
    ...
    const scoped_refptr<base::SequencedTaskRunner> bound_task_runner = base::ThreadPool::CreateSequencedTaskRunner({ base::WithBaseSyncPrimitives() });

    PendingRemote<mojom::SharedRemoteSyncTest> pending_remote;
    auto receiver = pending_remote.InitWithNewPipeAndPassReceiver();

    SharedRemote<mojom::SharedRemoteSyncTest> remote(std::move(pending_remote), bound_task_runner);
    bound_task_runner->PostTask(FROM_HERE,
        base::BindOnce(
            [](PendingReceiver<mojom::SharedRemoteSyncTest> receiver) {
                MakeSelfOwnedReceiver(std::make_unique<SharedRemoteSyncTestImpl>(), std::move(receiver));
            },
            std::move(receiver)));
...
TEST_P(RemoteTest, SharedRemoteSyncCallsFromOffBoundConstructionSequence)
{
    ...
    const scoped_refptr<base::SequencedTaskRunner> background_task_runner = base::ThreadPool::CreateSequencedTaskRunner({ base::WithBaseSyncPrimitives() });
    ...
    Receiver<mojom::SharedRemoteSyncTest> receiver(&impl, pending_remote.InitWithNewPipeAndPassReceiver());

    int32_t value = 0;
    base::RunLoop loop;
    base::OnceClosure quit = loop.QuitClosure();
    SharedRemote<mojom::SharedRemoteSyncTest> remote(std::move(pending_remote));
    background_task_runner->PostTask(FROM_HERE, base::BindLambdaForTesting([remote, &value, &quit] {
...
TEST_P(RemoteTest, SharedRemoteSyncCallsFromBoundNonConstructionSequence)
{
    ...
    base::RunLoop loop;
    base::OnceClosure quit = loop.QuitClosure();
    SharedRemote<mojom::SharedRemoteSyncTest> remote(std::move(pending_remote), background_task_runner);
    background_task_runner->PostTask(FROM_HERE, base::BindLambdaForTesting([remote, &value, &quit] {
        EXPECT_TRUE(remote->Fetch(&value));
        EXPECT_EQ(kMagicNumber, value);
        std::move(quit).Run();
...
TEST_P(RemoteTest, RemoteSet)
{
    ...
    // Send a message to each and wait for a reply.
    {
        ...
        for (auto& id : ids) {
            remotes.Get(id)->Add(kValue, base::BindLambdaForTesting([&](double total) {
    ...
    {
        base::RunLoop loop;
        remotes.set_disconnect_handler(base::BindLambdaForTesting([&](RemoteSetElementId id) {
            EXPECT_EQ(id, id0);
...
public:
    explicit LargeMessageTestImpl(PendingReceiver<mojom::LargeMessageTest> receiver)
        : receiver_(this, std::move(receiver))
    {
    }
...
TEST_P(RemoteTest, DISABLED_SendVeryLargeMessages)
{
    ...
    constexpr size_t kBigDataSize = core::test::MojoTestBase::kMaxMessageSizeInTests * 2;
