...
protected:
    ...
    static constexpr base::TimeDelta kOneSamplingInterval = MessageQuotaChecker::DecayingRateAverage::kSamplingInterval;
    ...
    static void RecordDumpAttempt(size_t total_quota_used, absl::optional<size_t> message_pipe_quota_used, int64_t seconds_since_construction,
...
const MessageQuotaCheckerTest::Configuration MessageQuotaCheckerTest::enabled_config_ = { true, 1, 100, 200, &RecordDumpAttempt };
...
TEST_F(MessageQuotaCheckerTest, ReadsConfigurationFromFeatures)
{
    ...
    feature_list.InitAndEnableFeatureWithParameters(features::kMojoRecordUnreadMessageCount, params);

    // Validate that the configuration reads from the feature configuration.
    const MessageQuotaChecker::Configuration config = MessageQuotaChecker::GetConfigurationForTesting();
...
TEST_F(MessageQuotaCheckerTest, CreatesWhenEnabled)
{
    ...
    for (size_t i = 0; i < 1000; ++i)
        EXPECT_NE(nullptr, MessageQuotaChecker::MaybeCreateForTesting(enabled_config_));
}
...
TEST_F(MessageQuotaCheckerTest, CountsRight)
{
    ...
    scoped_refptr<MessageQuotaChecker> checker = MessageQuotaChecker::MaybeCreateForTesting(enabled_config_);
...
TEST_F(MessageQuotaCheckerTest, CountsMessagePipeAlso)
{
    ...
    uint64_t limit = 0;
    uint64_t usage = 0;
    MojoResult rv = MojoQueryQuota(pipe.handle0.get().value(), MOJO_QUOTA_TYPE_UNREAD_MESSAGE_COUNT, nullptr, &limit, &usage);
    ...
    rv = MojoQueryQuota(pipe.handle0.get().value(), MOJO_QUOTA_TYPE_UNREAD_MESSAGE_COUNT, nullptr, &limit, &usage);
    ASSERT_EQ(MOJO_RESULT_OK, rv);
    ...
    for (size_t i = 0; i < 10; ++i) {
        checker->BeforeWrite();
        ASSERT_EQ(MOJO_RESULT_OK, WriteMessageRaw(pipe.handle0.get(), kMessage, sizeof(kMessage), nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE));
    }
...
TEST_F(MessageQuotaCheckerTest, DumpsCoreOnOverrun)
{
    ...
    base::TimeTicks t0 = MessageQuotaChecker::DecayingRateAverage::GetNextSamplingIntervalForTesting(base::TimeTicks::Now());
    ...
    EXPECT_EQ((12 * kOneSamplingInterval).InSeconds(), last_seconds_since_construction_);
...
TEST_F(MessageQuotaCheckerTest, DecayingRateAverage)
{
    ...
    EXPECT_NEAR(0.619, avg.GetDecayedRateAverage(t0 + 2.5 * kOneSamplingInterval), 0.001);
    ...
    EXPECT_EQ(1.0 + 1.75 / 1024.0, avg.GetDecayedRateAverage(t0 + kOneSamplingInterval));
    ...
    EXPECT_EQ(4.0, avg2.GetDecayedRateAverage(t0));
    EXPECT_NEAR(4.332, avg2.GetDecayedRateAverage(t0 + 1.0 / 8.0 * kOneSamplingInterval), 0.001);
    EXPECT_EQ(4.0, avg2.GetDecayedRateAverage(t0 + 1.0 / 4.0 * kOneSamplingInterval));
    avg2.AccrueEvent(t0);
    EXPECT_EQ(4.0, avg2.GetDecayedRateAverage(t0 + 2.0 / 4.0 * kOneSamplingInterval));
    avg2.AccrueEvent(t0);
    EXPECT_EQ(4.0, avg2.GetDecayedRateAverage(t0 + 3.0 / 4.0 * kOneSamplingInterval));
}
