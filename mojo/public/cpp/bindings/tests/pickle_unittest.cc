...
template <typename T> void DoExpectResult(int foo, int bar, base::OnceClosure callback, T actual)
{
...
template <typename T> base::OnceCallback<void(T)> ExpectResult(const T& t, base::OnceClosure callback)
{
    return base::BindOnce(&DoExpectResult<T>, t.foo(), t.bar(), std::move(callback));
}

template <typename T> void DoFail(const std::string& reason, T)
{
    EXPECT_TRUE(false) << reason;
}

template <typename T> base::OnceCallback<void(T)> Fail(const std::string& reason)
{
    return base::BindOnce(&DoFail<T>, reason);
}

template <typename T> void DoExpectEnumResult(T expected, base::OnceClosure callback, T actual)
{
    EXPECT_EQ(expected, actual);
    std::move(callback).Run();
}

template <typename T> base::OnceCallback<void(T)> ExpectEnumResult(T t, base::OnceClosure callback)
{
    return base::BindOnce(&DoExpectEnumResult<T>, t, std::move(callback));
}

template <typename T> void DoEnumFail(const std::string& reason, T)
{
    EXPECT_TRUE(false) << reason;
}

template <typename T> base::OnceCallback<void(T)> EnumFail(const std::string& reason)
{
...
template <typename Func, typename Arg> void RunSimpleLambda(Func func, Arg arg)
{
    func(std::move(arg));
}

template <typename Arg, typename Func> base::OnceCallback<void(Arg)> BindSimpleLambda(Func func)
{
...
public:
    ...
    void PassPickledStruct(PickledStructChromium pickle, PassPickledStructCallback callback) override
    {
        std::move(callback).Run(std::move(pickle));
    }

    void PassPickledEnum(PickledEnumChromium pickle, PassPickledEnumCallback callback) override
    {
    ...
    void PassPickles(std::vector<PickledStructChromium> pickles, PassPicklesCallback callback) override
    {
        std::move(callback).Run(std::move(pickles));
    }

    void PassPickleArrays(std::vector<std::vector<PickledStructChromium>> pickle_arrays, PassPickleArraysCallback callback) override
    {
...
public:
    ...
    void PassPickledStruct(PickledStructBlink pickle, PassPickledStructCallback callback) override
    {
        std::move(callback).Run(std::move(pickle));
    }

    void PassPickledEnum(PickledEnumBlink pickle, PassPickledEnumCallback callback) override
    {
        std::move(callback).Run(pickle);
    }

    void PassPickleContainer(blink::PickleContainerPtr container, PassPickleContainerCallback callback) override
    {
        std::move(callback).Run(std::move(container));
    }

    void PassPickles(WTF::Vector<PickledStructBlink> pickles, PassPicklesCallback callback) override
    {
        std::move(callback).Run(std::move(pickles));
    }

    void PassPickleArrays(WTF::Vector<WTF::Vector<PickledStructBlink>> pickle_arrays, PassPickleArraysCallback callback) override
    {
        std::move(callback).Run(std::move(pickle_arrays));
    }
};
...
public:
    ...
    template <typename ProxyType = PicklePasser> Remote<ProxyType> ConnectToChromiumService()
    {
        Remote<ProxyType> proxy;
        chromium_receivers_.Add(&chromium_service_, ConvertPendingReceiver<PicklePasser>(proxy.BindNewPipeAndPassReceiver()));
        return proxy;
    }

    template <typename ProxyType = blink::PicklePasser> Remote<ProxyType> ConnectToBlinkService()
    {
        Remote<ProxyType> proxy;
        blink_receivers_.Add(&blink_service_, ConvertPendingReceiver<blink::PicklePasser>(proxy.BindNewPipeAndPassReceiver()));
        return proxy;
    }
...
protected:
    static void ForceMessageSerialization(bool forced)
    {
        ...
        Connector::OverrideDefaultSerializationBehaviorForTesting(
            forced ? Connector::OutgoingSerializationMode::kEager : Connector::OutgoingSerializationMode::kLazy,
            Connector::IncomingSerializationMode::kDispatchAsIs);
    }

    class ScopedForceMessageSerialization {
    public:
        ScopedForceMessageSerialization()
        {
            ForceMessageSerialization(true);
        }

        ScopedForceMessageSerialization(const ScopedForceMessageSerialization&) = delete;
        ScopedForceMessageSerialization& operator=(const ScopedForceMessageSerialization&) = delete;

        ~ScopedForceMessageSerialization()
        {
            ForceMessageSerialization(false);
        }
    };
...
TEST_F(PickleTest, ChromiumProxyToChromiumService)
{
    ...
    {
        base::RunLoop loop;
        chromium_proxy->PassPickledStruct(PickledStructChromium(1, 2), ExpectResult(PickledStructChromium(1, 2), loop.QuitClosure()));
        loop.Run();
    }
    {
        base::RunLoop loop;
        chromium_proxy->PassPickledStruct(PickledStructChromium(4, 5), ExpectResult(PickledStructChromium(4, 5), loop.QuitClosure()));
        loop.Run();
    }

    {
        base::RunLoop loop;
        chromium_proxy->PassPickledEnum(PickledEnumChromium::VALUE_1, ExpectEnumResult(PickledEnumChromium::VALUE_1, loop.QuitClosure()));
        loop.Run();
    }
}
...
TEST_F(PickleTest, ChromiumProxyToBlinkService)
{
    ...
    {
        base::RunLoop loop;
        chromium_proxy->PassPickledStruct(PickledStructChromium(-1, -1), Fail<PickledStructChromium>("Blink service should reject this."));
    ...
    {
        base::RunLoop loop;
        chromium_proxy->PassPickledEnum(PickledEnumChromium::VALUE_0, ExpectEnumResult(PickledEnumChromium::VALUE_0, loop.QuitClosure()));
        loop.Run();
    }
    ...
    {
        base::RunLoop loop;
        chromium_proxy->PassPickledEnum(PickledEnumChromium::VALUE_2, EnumFail<PickledEnumChromium>("Blink service should reject this."));
        ExpectError(&chromium_proxy, loop.QuitClosure());
        loop.Run();
    }
}
...
TEST_F(PickleTest, BlinkProxyToBlinkService)
{
    ...
    {
        base::RunLoop loop;
        blink_proxy->PassPickledStruct(PickledStructBlink(1, 1), ExpectResult(PickledStructBlink(1, 1), loop.QuitClosure()));
        loop.Run();
    }

    {
        base::RunLoop loop;
        blink_proxy->PassPickledEnum(PickledEnumBlink::VALUE_0, ExpectEnumResult(PickledEnumBlink::VALUE_0, loop.QuitClosure()));
        loop.Run();
    }
}
...
TEST_F(PickleTest, BlinkProxyToChromiumService)
{
    ...
    {
        base::RunLoop loop;
        blink_proxy->PassPickledEnum(PickledEnumBlink::VALUE_1, ExpectEnumResult(PickledEnumBlink::VALUE_1, loop.QuitClosure()));
        loop.Run();
    }
}
...
TEST_F(PickleTest, PickleArray)
{
    ...
    pickles[1].set_baz(100);
    {
        ...
        proxy->PassPickles(std::move(pickles), BindSimpleLambda<std::vector<PickledStructChromium>>([&](std::vector<PickledStructChromium> passed) {
...
TEST_F(PickleTest, PickleArrayArray)
{
    ...
    pickle_arrays[1][1].set_baz(100);
    {
        ...
        proxy->PassPickleArrays(std::move(pickle_arrays),
            BindSimpleLambda<std::vector<std::vector<PickledStructChromium>>>([&](std::vector<std::vector<PickledStructChromium>> passed) {
                ASSERT_EQ(2u, passed.size());
...
TEST_F(PickleTest, PickleContainer)
{
    ...
    {
        base::RunLoop run_loop;
        proxy->PassPickleContainer(std::move(pickle_container), BindSimpleLambda<PickleContainerPtr>([&](PickleContainerPtr passed) {
