...
public:
    EchoServiceImpl(PendingReceiver<test::EchoService> receiver, base::OnceClosure quit_closure)
        : receiver_(this, std::move(receiver))
        , quit_closure_(std::move(quit_closure))
    {
        receiver_.set_disconnect_handler(base::BindOnce(&EchoServiceImpl::OnDisconnect, base::Unretained(this)));
    }
...
public:
    explicit PingPongTest(PendingRemote<test::EchoService> service)
        ...
        , ping_done_callback_(base::BindRepeating(&PingPongTest::OnPingDone, base::Unretained(this)))
    {
    }
...
void PingPongTest::RunTest(int iterations, int batch_size, int message_size)
{
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&PingPongTest::DoPing, base::Unretained(this)));
    run_loop.Run();
}
...
public:
    void RunTestOnTaskRunner(base::TaskRunner* runner, MojoHandle client_mp, const std::string& test_name)
    {
        if (runner == base::ThreadTaskRunnerHandle::Get().get()) {
            ...
            runner->PostTaskAndReply(
                FROM_HERE, base::BindOnce(&MojoE2EPerftest::RunTests, base::Unretained(this), client_mp, test_name), run_loop.QuitClosure());
            run_loop.Run();
        }
    }
...
private:
    void RunTests(MojoHandle client_mp, const std::string& test_name)
    {
        ...
        PendingRemote<test::EchoService> service(ScopedMessagePipeHandle(MessagePipeHandle(client_mp)), 0);
        ...
        for (int batch_size : kBatchSizes) {
            for (int message_size : kMessageSizes) {
                ...
                if (message_size == 65536)
                    num_messages /= 10;
                std::string sub_test_name = base::StringPrintf("%s/%dx%d/%dbytes", test_name.c_str(), num_messages / batch_size, batch_size, message_size);
...
void CreateAndRunService(PendingReceiver<test::EchoService> receiver, base::OnceClosure quit_closure)
{
    // Self-owned.
...
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(PingService, MojoE2EPerftest, mp)
{
    ...
    auto receiver = PendingReceiver<test::EchoService>(ScopedMessagePipeHandle(MessagePipeHandle(service_mp)));
    base::RunLoop run_loop;
    core::GetIOTaskRunner()->PostTask(FROM_HERE,
        base::BindOnce(&CreateAndRunService, std::move(receiver),
            base::BindOnce(base::IgnoreResult(&base::TaskRunner::PostTask), task_environment_.GetMainThreadTaskRunner(), FROM_HERE, run_loop.QuitClosure())));
    run_loop.Run();
}
...
TEST_F(MojoE2EPerftest, MultiProcessEchoMainThread)
{
    RunTestClient("PingService", [&](MojoHandle mp) {
        ...
        RunTestOnTaskRunner(task_environment_.GetMainThreadTaskRunner().get(), client_mp, "MultiProcessEchoMainThread");
    });
}
...
TEST_F(MojoE2EPerftest, MultiProcessEchoIoThread)
{
    RunTestClient("PingService", [&](MojoHandle mp) {
        ...
        CreateMessagePipe(&client_mp, &service_mp);
        WriteMessageWithHandles(mp, "hello", &service_mp, 1);
        RunTestOnTaskRunner(core::GetIOTaskRunner().get(), client_mp, "MultiProcessEchoIoThread");
    });
}
