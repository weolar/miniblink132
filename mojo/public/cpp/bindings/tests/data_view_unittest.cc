...
std::unique_ptr<DataViewHolder> SerializeTestStruct(TestStructPtr input)
{
    ...
    mojo::internal::MessageFragment<internal::TestStruct_Data> fragment(result->message);
    mojo::internal::Serialize<TestStructDataView>(input, fragment);
    result->data_view = std::make_unique<TestStructDataView>(fragment.data(), &result->message);
    return result;
}
...
TEST_F(DataViewTest, String)
{
    ...
    EXPECT_EQ(std::string("hello"), std::string(string_data_view.storage(), string_data_view.size()));
}
