...
public:
    ...
    void BounceOne(ScopedMessagePipeHandle one, BounceOneCallback callback) override
    {
        std::move(callback).Run(std::move(one));
    }

    void BounceTwo(ScopedMessagePipeHandle one, ScopedMessagePipeHandle two, BounceTwoCallback callback) override
    {
...
TEST_F(TestHelperTest, AsyncWaiter)
{
    ...
    WriteMessageRaw(pipe.handle0.get(), kTestString.data(), kTestString.size(), nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE);
    wait_for_trampoline.BounceOne(std::move(pipe.handle0), &handle0);
    wait_for_trampoline.BounceTwo(std::move(handle0), std::move(pipe.handle1), &handle0, &handle1);
