...
bool AreEqualRectArrays(const std::vector<test::RectPtr>& rects1, const std::vector<test::RectPtr>& rects2)
{
    if (rects1.size() != rects2.size())
        return false;

    for (size_t i = 0; i < rects1.size(); ++i) {
        if (rects1[i]->x != rects2[i]->x || rects1[i]->y != rects2[i]->y || rects1[i]->width != rects2[i]->width || rects1[i]->height != rects2[i]->height) {
            return false;
        }
    }
...
template <> struct TypeConverter<test::RectPtr, RedmondRect> {
    static test::RectPtr Convert(const RedmondRect& input)
    {
        return test::Rect::New(input.left, input.top, input.right - input.left, input.bottom - input.top);
    }
};

template <> struct TypeConverter<RedmondRect, test::RectPtr> {
...
template <> struct TypeConverter<test::NamedRegionPtr, RedmondNamedRegion> {
    static test::NamedRegionPtr Convert(const RedmondNamedRegion& input)
    {
        return test::NamedRegion::New(input.name, ConvertTo<std::vector<test::RectPtr>>(input.rects));
    }
};

template <> struct TypeConverter<RedmondNamedRegion, test::NamedRegionPtr> {
