...
public:
    ...
    void ResetLastServerValueSeen()
    {
        last_server_value_seen_ = 0;
    }

    int32_t last_server_value_seen() const
    {
        return last_server_value_seen_;
    }
...
TEST_F(ReceiverCallbackTest, Basic)
{
    ...
    Receiver<sample::Provider> receiver(&server_impl, remote_.BindNewPipeAndPassReceiver());
...
TEST_F(ReceiverCallbackTest, DeleteReceiverThenRunCallback)
{
    ...
    base::RunLoop run_loop;
    {
        ...
        remote_->EchoInt(7, base::BindLambdaForTesting([&](int32_t value) { last_client_callback_value_seen_ = value; }));
        run_loop2.Run();
    }
