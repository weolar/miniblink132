...
template <typename ReceiverSetType, typename ContextType> void ExpectContextHelper(const ReceiverSetType* receiver_set, ContextType expected_context)
{
    EXPECT_EQ(expected_context, receiver_set->current_context());
}

template <typename ReceiverSetType, typename ContextType>
base::RepeatingClosure ExpectContext(const ReceiverSetType* receiver_set, ContextType expected_context)
{
    return base::BindRepeating(&ExpectContextHelper<ReceiverSetType, ContextType>, receiver_set, expected_context);
}

template <typename ReceiverSetType, typename ContextType>
void ExpectMutableContextHelper(ReceiverSetType* receiver_set, ContextType expected_context, ContextType new_context)
{
    {
...
template <typename ReceiverSetType, typename ContextType>
base::RepeatingClosure ExpectMutableContext(ReceiverSetType* receiver_set, ContextType expected_context, ContextType new_context)
{
    return base::BindRepeating(&ExpectMutableContextHelper<ReceiverSetType, ContextType>, receiver_set, expected_context, new_context);
}

template <typename ReceiverSetType> void ExpectReceiverIdHelper(ReceiverSetType* receiver_set, ReceiverId expected_receiver_id)
{
    EXPECT_EQ(expected_receiver_id, receiver_set->current_receiver());
}

template <typename ReceiverSetType> base::RepeatingClosure ExpectReceiverId(ReceiverSetType* receiver_set, ReceiverId expected_receiver_id)
{
    return base::BindRepeating(&ExpectReceiverIdHelper<ReceiverSetType>, receiver_set, expected_receiver_id);
}

template <typename ReceiverSetType> void ReportBadMessageHelper(ReceiverSetType* receiver_set, const std::string& error)
{
    receiver_set->ReportBadMessage(error);
}

template <typename ReceiverSetType> base::RepeatingClosure ReportBadMessage(ReceiverSetType* receiver_set, const std::string& error)
{
    return base::BindRepeating(&ReportBadMessageHelper<ReceiverSetType>, receiver_set, error);
}

template <typename ReceiverSetType> void SaveBadMessageCallbackHelper(ReceiverSetType* receiver_set, ReportBadMessageCallback* callback)
{
    *callback = receiver_set->GetBadMessageCallback();
}

template <typename ReceiverSetType> base::RepeatingClosure SaveBadMessageCallback(ReceiverSetType* receiver_set, ReportBadMessageCallback* callback)
{
    return base::BindRepeating(&SaveBadMessageCallbackHelper<ReceiverSetType>, receiver_set, callback);
}

base::RepeatingClosure Sequence(base::RepeatingClosure first, base::RepeatingClosure second)
{
...
TEST_P(ReceiverSetTest, ReceiverSetContext)
{
    ...
    {
        base::RunLoop loop;
        receivers.set_disconnect_handler(Sequence(ExpectContext(&receivers, 1), loop.QuitClosure()));
        ping_a.reset();
        loop.Run();
    }
    ...
    {
        base::RunLoop loop;
        receivers.set_disconnect_handler(Sequence(ExpectContext(&receivers, 2), loop.QuitClosure()));
        ping_b.reset();
        loop.Run();
    }
...
TEST_P(ReceiverSetTest, ReceiverSetDispatchReceiver)
{
    ...
    ReceiverSet<PingService, int> receivers;
    Remote<PingService> ping_a, ping_b;
    ReceiverId id_a = receivers.Add(&impl, ping_a.BindNewPipeAndPassReceiver(), 1);
    ReceiverId id_b = receivers.Add(&impl, ping_b.BindNewPipeAndPassReceiver(), 2);
    ...
    {
        base::RunLoop loop;
        receivers.set_disconnect_handler(Sequence(ExpectReceiverId(&receivers, id_a), loop.QuitClosure()));
        ping_a.reset();
        loop.Run();
    }
    ...
    {
        base::RunLoop loop;
        receivers.set_disconnect_handler(Sequence(ExpectReceiverId(&receivers, id_b), loop.QuitClosure()));
        ping_b.reset();
        loop.Run();
    }
...
TEST_P(ReceiverSetTest, ReceiverSetConnectionErrorWithReason)
{
    ...
    receivers.set_disconnect_with_reason_handler(base::BindLambdaForTesting([&](uint32_t custom_reason, const std::string& description) {
...
TEST_P(ReceiverSetTest, ReceiverSetReportBadMessage)
{
    ...
    SetDefaultProcessErrorHandler(base::BindRepeating([](std::string* out_error, const std::string& error) { *out_error = error; }, &last_received_error));
...
TEST_P(ReceiverSetTest, ReceiverSetGetBadMessageCallback)
{
    ...
    {
        impl.set_ping_handler(SaveBadMessageCallback(&receivers, &bad_message_callback_a));
        base::RunLoop loop;
        ping_a->Ping(loop.QuitClosure());
        loop.Run();
        ping_a.reset();
    ...
    {
        impl.set_ping_handler(SaveBadMessageCallback(&receivers, &bad_message_callback_b));
        base::RunLoop loop;
        ping_b->Ping(loop.QuitClosure());
        loop.Run();
    }
...
TEST_P(ReceiverSetTest, ReceiverSetGetBadMessageCallbackOutlivesReceiverSet)
{
    ...
    ReportBadMessageCallback bad_message_callback;
    {
        ...
        impl.set_ping_handler(SaveBadMessageCallback(&receivers, &bad_message_callback));
        base::RunLoop loop;
        ping_a->Ping(loop.QuitClosure());
        loop.Run();
    }
...
public:
    ...
    void set_new_ping_context(int context)
    {
        new_ping_context_ = context;
    }
...
TEST_P(ReceiverSetTest, AssociatedReceiverSetContext)
{
    ...
    Receiver<AssociatedPingProvider> receiver(&impl, provider.BindNewPipeAndPassReceiver());
    ...
    {
        base::RunLoop loop;
        impl.ping_receivers().set_disconnect_handler(Sequence(ExpectContext(&impl.ping_receivers(), 1), loop.QuitClosure()));
        ping_a.reset();
        loop.Run();
    }
    ...
    {
        base::RunLoop loop;
        impl.ping_receivers().set_disconnect_handler(Sequence(ExpectContext(&impl.ping_receivers(), 2), loop.QuitClosure()));
        ping_b.reset();
        loop.Run();
    }
...
TEST_P(ReceiverSetTest, PrimaryInterfaceReceiverSetContext)
{
...

    {
        ...
        base::RunLoop loop;
        impl.set_new_ping_handler(Sequence(ExpectContext(&receivers, 1), loop.QuitClosure()));
...

    {
        AssociatedRemote<PingService> ping;
        base::RunLoop loop;
        impl.set_new_ping_handler(Sequence(ExpectContext(&receivers, 2), loop.QuitClosure()));
...
TEST_P(ReceiverSetTest, PrimaryInterfaceReceiverSetDispatchReceiver)
{
    ...
    ReceiverId id_a = receivers.Add(&impl, provider_a.BindNewPipeAndPassReceiver(), 1);
    ReceiverId id_b = receivers.Add(&impl, provider_b.BindNewPipeAndPassReceiver(), 2);

    {
        AssociatedRemote<PingService> ping;
        base::RunLoop loop;
        impl.set_new_ping_handler(Sequence(ExpectReceiverId(&receivers, id_a), loop.QuitClosure()));
        provider_a->GetPing(ping.BindNewEndpointAndPassReceiver());
        loop.Run();
    }

    {
        AssociatedRemote<PingService> ping;
        base::RunLoop loop;
        impl.set_new_ping_handler(Sequence(ExpectReceiverId(&receivers, id_b), loop.QuitClosure()));
        provider_b->GetPing(ping.BindNewEndpointAndPassReceiver());
        loop.Run();
    }
...
TEST_P(ReceiverSetTest, AssociatedReceiverSetConnectionErrorWithReason)
{
    ...
    Receiver<AssociatedPingProvider> primary_receiver(&primary_impl, remote_primary.BindNewPipeAndPassReceiver());

    base::RunLoop run_loop;
    primary_impl.ping_receivers().set_disconnect_with_reason_handler(base::BindLambdaForTesting([&](uint32_t custom_reason, const std::string& description) {
...
TEST_P(ReceiverSetTest, UniqueReceiverSetDestruction)
{
    ...
    receivers->Add(std::make_unique<PingInstanceCounter>(), ping_a.BindNewPipeAndPassReceiver());
    EXPECT_EQ(1, PingInstanceCounter::instance_count);

    receivers->Add(std::make_unique<PingInstanceCounter>(), ping_b.BindNewPipeAndPassReceiver());
...
TEST_P(ReceiverSetTest, UniqueReceiverSetRemoveEntry)
{
    Remote<PingService> ping_a, ping_b;
    UniqueReceiverSet<PingService> receivers;
    ReceiverId receiver_id_a = receivers.Add(std::make_unique<PingInstanceCounter>(), ping_a.BindNewPipeAndPassReceiver());
    ReceiverId receiver_id_b = receivers.Add(std::make_unique<PingInstanceCounter>(), ping_b.BindNewPipeAndPassReceiver());
    EXPECT_EQ(2, PingInstanceCounter::instance_count);
