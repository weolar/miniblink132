...
class KeyValueStoreImpl : public base::RefCountedThreadSafe<KeyValueStoreImpl>, public mojom::KeyValueStore {
public:
    KeyValueStoreImpl()
    ...
    {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&KeyValueStoreImpl::BindOnTaskRunner, this, std::move(receiver)));
    }

    void ShutDown(base::OnceClosure callback)
    {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&KeyValueStoreImpl::ShutDownOnTaskRunner, this, std::move(callback)));
    }
...
private:
    ...
    public:
        ...
        {
            task_runner_->PostTask(FROM_HERE, base::BindOnce(&WriterImpl::BindOnTaskRunner, base::Unretained(this), std::move(receiver)));
        }

        void ShutDown(base::OnceClosure callback)
        {
            task_runner_->PostTask(FROM_HERE, base::BindOnce(&WriterImpl::ShutDownOnTaskRunner, base::Unretained(this), std::move(callback)));
        }
    ...
    private:
        ...
        {
            receiver_ = std::make_unique<Receiver<mojom::Writer>>(this, std::move(receiver));
        }
    ...
    {
        receiver_ = std::make_unique<Receiver<mojom::KeyValueStore>>(this, std::move(receiver));
    }
...
public:
    FlushAsyncTest()
    {
        key_value_store_->Bind(remote_key_value_store_.BindNewPipeAndPassReceiver());
    }
...
private:
    ...
    scoped_refptr<KeyValueStoreImpl> key_value_store_ { base::MakeRefCounted<KeyValueStoreImpl>() };
};
...
TEST_P(FlushAsyncTest, WaitForMultipleFlushes)
{
    ...
    key_value_store()->GetSnapshot(base::BindLambdaForTesting([&](const base::flat_map<std::string, std::string>& contents) {
        snapshot = contents;
        loop.Quit();
    }));
    loop.Run();
...
TEST_P(FlushAsyncTest, DroppedFlusherCompletesPendingFlush)
{
    ...
    key_value_store().PauseReceiverUntilFlushCompletes(PendingFlush(&flusher.value()));
    key_value_store()->GetSnapshot(base::BindLambdaForTesting([&](const base::flat_map<std::string, std::string>& contents) {
        snapshot = contents;
        loop.Quit();
    }));
...
public:
    ...
    Receiver<mojom::Pinger>& receiver()
    {
        return receiver_;
    }
...
public:
    explicit KeyValueStoreClientImpl(PendingReceiver<mojom::KeyValueStoreClient> receiver)
        : receiver_(this, std::move(receiver))
    {
    }
    ~KeyValueStoreClientImpl() override = default;

    Receiver<mojom::KeyValueStoreClient>& receiver()
    {
        return receiver_;
    }
...
TEST_P(FlushAsyncTest, PauseRemote)
{
    ...
    // client notification.
    base::RunLoop loop;
    key_value_store()->GetSnapshot(base::BindLambdaForTesting([&](const base::flat_map<std::string, std::string>&) {
