...
void CreateTestMessagePayload(std::vector<uint8_t>* bytes, std::vector<ScopedHandle>* handles)
{
    Message message(kTestMessageName, kTestMessageFlags, 0, kTestPayloadSize, nullptr);
    message.header()->trace_nonce = 0;
    bytes->resize(message.data_num_bytes());
    std::copy(message.data(), message.data() + message.data_num_bytes(), bytes->begin());
...
TEST(BindingsMessageTest, ConstructFromPayload)
{
    ...
    WriteMessageRaw(pipe.handle0.get(), in_bytes1.data(), in_bytes1.size(), reinterpret_cast<const MojoHandle*>(in_handles1.data()), in_handles1.size(),
    ...
    WriteMessageNew(pipe.handle0.get(), message.TakeMojoMessage(), MOJO_WRITE_MESSAGE_FLAG_NONE);
    ...
    ASSERT_EQ(MOJO_RESULT_OK, ReadMessageRaw(pipe.handle1.get(), &out_bytes1, &out_handles1, MOJO_READ_MESSAGE_FLAG_NONE));
    std::vector<uint8_t> out_bytes2;
    std::vector<ScopedHandle> out_handles2;
    ASSERT_EQ(MOJO_RESULT_OK, ReadMessageRaw(pipe.handle1.get(), &out_bytes2, &out_handles2, MOJO_READ_MESSAGE_FLAG_NONE));
