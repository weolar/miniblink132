...
public:
    ...
    void EchoString(const absl::optional<std::string>& str, EchoStringCallback callback) override
    {
        std::move(callback).Run(str);
    }

    void EchoStringArray(const absl::optional<std::vector<absl::optional<std::string>>>& arr, EchoStringArrayCallback callback) override
    {
        std::move(callback).Run(std::move(arr));
    }

    void EchoStringMap(const absl::optional<base::flat_map<std::string, absl::optional<std::string>>>& str_map, EchoStringMapCallback callback) override
    {
...
void ExpectString(const WTF::String& expected_string, base::OnceClosure closure, const WTF::String& string)
{
    EXPECT_EQ(expected_string, string);
    std::move(closure).Run();
}

void ExpectStringArray(absl::optional<WTF::Vector<WTF::String>>* expected_arr, base::OnceClosure closure, const absl::optional<WTF::Vector<WTF::String>>& arr)
{
    EXPECT_EQ(*expected_arr, arr);
    std::move(closure).Run();
}

void ExpectStringMap(absl::optional<WTF::HashMap<WTF::String, WTF::String>>* expected_map, base::OnceClosure closure,
...
TEST_F(WTFTypesTest, Serialization_WTFVectorToWTFVector)
{
    ...
    mojo::internal::ContainerValidateParams validate_params(0, true, new mojo::internal::ContainerValidateParams(0, false, nullptr));
...
TEST_F(WTFTypesTest, SendString)
{
    ...
    TestWTFImpl impl(ConvertPendingReceiver<TestWTF>(remote.BindNewPipeAndPassReceiver()));
    ...
    for (size_t i = 0; i < strs.size(); ++i) {
        ...
        remote->EchoString(strs[i], base::BindOnce(&ExpectString, strs[i], loop.QuitClosure()));
        loop.Run();
    }
}
...
TEST_F(WTFTypesTest, SendStringArray)
{
    ...
    for (size_t i = 0; i < std::size(arrs); ++i) {
        ...
        remote->EchoStringArray(arrs[i], base::BindOnce(&ExpectStringArray, base::Unretained(&arrs[i]), loop.QuitClosure()));
        loop.Run();
    }
}
...
TEST_F(WTFTypesTest, SendStringMap)
{
    ...
    for (size_t i = 0; i < std::size(maps); ++i) {
        ...
        //     WTF::String>>.
        remote->EchoStringMap(maps[i], base::BindOnce(&ExpectStringMap, base::Unretained(&maps[i]), loop.QuitClosure()));
        loop.Run();
    }
}
...
TEST_F(WTFTypesTest, NestedStruct_CloneAndEquals)
{
    ...
    a->map_struct.insert(blink::TestWTFStruct::New("baz", 4), blink::TestWTFStruct::New("baz", 5));
    auto b = a.Clone();
    EXPECT_EQ(a, b);
