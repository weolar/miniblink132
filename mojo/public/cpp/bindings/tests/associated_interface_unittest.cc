...
public:
    ...
    void set_notify_send_method_called(base::RepeatingCallback<void(int32_t)> callback)
    {
    ...
    {
        notify_send_method_called_.Run(value);
    }

    AssociatedReceiver<IntegerSender>* receiver()
    {
        return &receiver_;
    }
...
public:
    explicit IntegerSenderConnectionImpl(PendingReceiver<IntegerSenderConnection> receiver)
        : receiver_(this, std::move(receiver))
    {
    }
    ...
    Receiver<IntegerSenderConnection>* receiver()
    {
        return &receiver_;
    }
...
public:
    ...
    PendingAssociatedRemote<T> EmulatePassingAssociatedRemote(
        PendingAssociatedRemote<T> remote, scoped_refptr<MultiplexRouter> source, scoped_refptr<MultiplexRouter> target)
    {
        ...
        return PendingAssociatedRemote<T>(target->CreateLocalEndpointHandle(id), remote.version());
    }

    void CreateRouterPair(scoped_refptr<MultiplexRouter>* router0, scoped_refptr<MultiplexRouter>* router1)
    {
        MessagePipe pipe;
        *router0 = MultiplexRouter::CreateAndStartReceiving(std::move(pipe.handle0), MultiplexRouter::MULTI_INTERFACE, true, main_runner_);
        *router1 = MultiplexRouter::CreateAndStartReceiving(std::move(pipe.handle1), MultiplexRouter::MULTI_INTERFACE, false, main_runner_);
    }

    void CreateIntegerSenderWithExistingRouters(scoped_refptr<MultiplexRouter> router0, PendingAssociatedRemote<IntegerSender>* remote0,
        scoped_refptr<MultiplexRouter> router1, PendingAssociatedReceiver<IntegerSender>* receiver1)
    {
        *receiver1 = remote0->InitWithNewEndpointAndPassReceiver();
        *remote0 = EmulatePassingAssociatedRemote(std::move(*remote0), router1, router0);
    }

    void CreateIntegerSender(PendingAssociatedRemote<IntegerSender>* remote, PendingAssociatedReceiver<IntegerSender>* receiver)
    {
...
public:
    ...
    void SetUp(PendingAssociatedRemote<IntegerSender> remote, TestSender* next_sender, int32_t max_value_to_send)
    {
    ...
    void Send(int32_t value)
    {
        ...
        next_sender_->task_runner()->PostTask(FROM_HERE, base::BindOnce(&TestSender::Send, base::Unretained(next_sender_), ++value));
    }
...
public:
    ...
    void SetUp(PendingAssociatedReceiver<IntegerSender> receiver0, PendingAssociatedReceiver<IntegerSender> receiver1, size_t expected_calls,
        base::OnceClosure notify_finish)
    {
        CHECK(task_runner()->RunsTasksInCurrentSequence());

        impl0_ = std::make_unique<IntegerSenderImpl>(std::move(receiver0));
        impl0_->set_notify_send_method_called(base::BindRepeating(&TestReceiver::SendMethodCalled, base::Unretained(this)));
        impl1_ = std::make_unique<IntegerSenderImpl>(std::move(receiver1));
        impl1_->set_notify_send_method_called(base::BindRepeating(&TestReceiver::SendMethodCalled, base::Unretained(this)));
...
TEST_F(AssociatedInterfaceTest, MultiThreadAccess)
{
    ...
    for (size_t i = 0; i < 4; ++i) {
        CreateIntegerSenderWithExistingRouters(router1, &pending_remotes[i], router0, &pending_receivers[i]);
    }

    TestSender senders[4];
    for (size_t i = 0; i < 4; ++i) {
        senders[i].task_runner()->PostTask(
            FROM_HERE, base::BindOnce(&TestSender::SetUp, base::Unretained(&senders[i]), std::move(pending_remotes[i]), nullptr, kMaxValue * (i + 1) / 4));
    }
    ...
    for (size_t i = 0; i < 2; ++i) {
        receivers[i].task_runner()->PostTask(FROM_HERE,
            base::BindOnce(&TestReceiver::SetUp, base::Unretained(&receivers[i]), std::move(pending_receivers[2 * i]), std::move(pending_receivers[2 * i + 1]),
                static_cast<size_t>(kMaxValue / 2), base::BindOnce(&NotificationCounter::OnGotNotification, base::Unretained(&counter))));
    }

    for (size_t i = 0; i < 4; ++i) {
        senders[i].task_runner()->PostTask(FROM_HERE, base::BindOnce(&TestSender::Send, base::Unretained(&senders[i]), kMaxValue * i / 4 + 1));
    }
    ...
    for (size_t i = 0; i < 4; ++i) {
        ...
        senders[i].task_runner()->PostTaskAndReply(FROM_HERE, base::BindOnce(&TestSender::TearDown, base::Unretained(&senders[i])), run_loop2.QuitClosure());
        run_loop2.Run();
    }
    ...
    for (size_t i = 0; i < 2; ++i) {
        ...
        receivers[i].task_runner()->PostTaskAndReply(
            FROM_HERE, base::BindOnce(&TestReceiver::TearDown, base::Unretained(&receivers[i])), run_loop2.QuitClosure());
        run_loop2.Run();
    }
    ...
    all_values.insert(all_values.end(), receivers[0].values().begin(), receivers[0].values().end());
    all_values.insert(all_values.end(), receivers[1].values().begin(), receivers[1].values().end());
...
TEST_F(AssociatedInterfaceTest, FIFO)
{
    ...
    TestSender senders[4];
    for (size_t i = 0; i < 4; ++i) {
        senders[i].task_runner()->PostTask(FROM_HERE,
            base::BindOnce(
                &TestSender::SetUp, base::Unretained(&senders[i]), std::move(pending_remotes[i]), base::Unretained(&senders[(i + 1) % 4]), kMaxValue));
    }
    ...
    senders[0].task_runner()->PostTask(FROM_HERE, base::BindOnce(&TestSender::Send, base::Unretained(&senders[0]), 1));
...
TEST_F(AssociatedInterfaceTest, PassAssociatedInterfaces)
{
    ...
    IntegerSenderConnectionImpl connection(connection_remote.BindNewPipeAndPassReceiver());
    ...
    base::RunLoop run_loop2;
    connection_remote->AsyncGetSender(base::BindLambdaForTesting([&](PendingAssociatedRemote<IntegerSender> sender) {
...
TEST_F(AssociatedInterfaceTest, ReceiverWaitAndPauseWhenNoAssociatedInterfaces)
{
    Remote<IntegerSenderConnection> connection_remote;
    IntegerSenderConnectionImpl connection(connection_remote.BindNewPipeAndPassReceiver());
    ...
    EXPECT_FALSE(connection.receiver()->internal_state()->HasAssociatedInterfaces());
    ...
    // wait or pause.
    EXPECT_TRUE(connection.receiver()->internal_state()->HasAssociatedInterfaces());
}
...
public:
    ...
    AssociatedReceiver<PingService>& receiver()
    {
        return receiver_;
    }
...
public:
    ...
    void GetPing(PendingAssociatedReceiver<PingService> receiver) override
    {
        ...
        if (expected_receivers_count_ > 0 && ping_services_.size() == expected_receivers_count_ && quit_waiting_) {
...
TEST_F(AssociatedInterfaceTest, ReceiverWithFilters)
{
    ...
    ping_a_impl.receiver().SetFilter(CallbackFilter::Wrap(base::BindLambdaForTesting([&] {
        EXPECT_EQ(0, a_status);
        EXPECT_EQ(0, b_status);
        a_status = 1;
    })));

    ping_b_impl.receiver().SetFilter(CallbackFilter::Wrap(base::BindLambdaForTesting([&] {
...
TEST_F(AssociatedInterfaceTest, AssociatedRemoteFlushForTestingWithClosedPeer)
{
    ...
    AssociatedRemote<IntegerSender> remote0(std::move(remote));
    bool called = false;
    remote0.set_disconnect_handler(base::BindLambdaForTesting([&] { called = true; }));
    receiver.reset();
...
TEST_F(AssociatedInterfaceTest, AssociatedReceiverFlushForTestingWithClosedPeer)
{
    scoped_refptr<MultiplexRouter> router0;
    scoped_refptr<MultiplexRouter> router1;
    CreateRouterPair(&router0, &router1);
    ...
    bool called = false;
    impl.receiver()->set_disconnect_handler(base::BindLambdaForTesting([&] { called = true; }));
...
TEST_F(AssociatedInterfaceTest, ReceiverFlushForTesting)
{
    ...
    bool called = false;
    remote->AsyncGetSender(base::BindLambdaForTesting([&](PendingAssociatedRemote<IntegerSender> remote) { called = true; }));
    EXPECT_FALSE(called);
...
TEST_F(AssociatedInterfaceTest, RemoteFlushForTestingWithClosedPeer)
{
    ...
    bool called = false;
    remote.set_disconnect_handler(base::BindLambdaForTesting([&] { called = true; }));
    EXPECT_FALSE(called);
    remote.FlushForTesting();
    EXPECT_TRUE(called);
    remote.FlushForTesting();
...
TEST_F(AssociatedInterfaceTest, AssociatedReceiverConnectionErrorWithReason)
{
    ...
    base::RunLoop run_loop;
    impl.receiver()->set_disconnect_with_reason_handler(base::BindLambdaForTesting([&](uint32_t custom_reason, const std::string& description) {
...
TEST_F(AssociatedInterfaceTest, PendingAssociatedReceiverConnectionErrorWithReason)
{
...
TEST_F(AssociatedInterfaceTest, AssociatedRemoteConnectionErrorWithReason)
{
    ...
    base::RunLoop run_loop;
    remote.set_disconnect_with_reason_handler(base::BindLambdaForTesting([&](uint32_t custom_reason, const std::string& description) {
...
TEST_F(AssociatedInterfaceTest, PendingAssociatedRemoteConnectionErrorWithReason)
{
...
TEST_F(AssociatedInterfaceTest, SharedAssociatedRemote)
{
    ...
    PendingAssociatedRemote<IntegerSender> pending_remote;
    connection_remote->GetSender(pending_remote.InitWithNewEndpointAndPassReceiver());
    ...
    sender_task_runner->PostTask(FROM_HERE, base::BindLambdaForTesting([&] {
        shared_sender->Echo(123, base::BindLambdaForTesting([&](int32_t value) {
            EXPECT_EQ(123, value);
...
{
    const scoped_refptr<base::SequencedTaskRunner> other_thread_task_runner = base::ThreadPool::CreateSequencedTaskRunner({});

    ForwarderTestContext* context = new ForwarderTestContext();
    PendingAssociatedRemote<IntegerSender> pending_remote;
    base::WaitableEvent sender_bound_event(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED);
    other_thread_task_runner->PostTask(FROM_HERE, base::BindLambdaForTesting([&] {
        context->interface_impl = std::make_unique<IntegerSenderConnectionImpl>(context->connection_remote.BindNewPipeAndPassReceiver());
        context->connection_remote->GetSender(pending_remote.InitWithNewEndpointAndPassReceiver());
    ...
    SharedAssociatedRemote<IntegerSender> shared_sender(std::move(pending_remote), other_thread_task_runner);
...
class DiscardingAssociatedPingProviderProvider : public AssociatedPingProviderProvider {
public:
    void GetPingProvider(PendingAssociatedReceiver<AssociatedPingProvider> receiver) override
    {
    }
};
...
TEST_F(AssociatedInterfaceTest, CloseWithoutBindingAssociatedReceiver)
{
    ...
    mojo::Receiver<AssociatedPingProviderProvider> receiver(&ping_provider_provider);
...
TEST_F(AssociatedInterfaceTest, AssociateWithDisconnectedPipe)
{
    ...
    AssociateWithDisconnectedPipe(sender.BindNewEndpointAndPassReceiver().PassHandle());
    sender->Send(42);
}
...
TEST_F(AssociatedInterfaceTest, AssociatedReceiverReportBadMessage)
{
    ...
    SetDefaultProcessErrorHandler(base::BindLambdaForTesting([&](const std::string& error) { received_error = error; }));
...
{
    PendingAssociatedRemote<IntegerSender> pending_remote;
    PendingAssociatedReceiver<IntegerSender> pending_receiver = pending_remote.InitWithNewEndpointAndPassReceiver();
    ...
    {
        base::RunLoop run_loop;
        impl.set_notify_send_method_called(base::BindLambdaForTesting([&](int32_t x) {
            EXPECT_EQ(88, x);
            run_loop.Quit();
        }));
