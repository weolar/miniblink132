...
public:
    ...
    void EchoMessagePipeHandle(ScopedMessagePipeHandle a, EchoMessagePipeHandleCallback callback) override
    {
...
TEST_P(RequestResponseTest, EchoString)
{
    ...
    provider->EchoString(kTestMessage, base::BindLambdaForTesting([&](const std::string& response) {
...
TEST_P(RequestResponseTest, EchoStrings)
{
    ...
    provider->EchoStrings(kTestMessage1, kTestMessage2, base::BindLambdaForTesting([&](const std::string& response1, const std::string& response2) {
...
TEST_P(RequestResponseTest, EchoMessagePipeHandle)
{
    ...
    base::RunLoop run_loop;
    constexpr const char kTestMessage[] = "hello";
    provider->EchoMessagePipeHandle(std::move(pipe.handle1), base::BindLambdaForTesting([&](ScopedMessagePipeHandle handle) {
...
TEST_P(RequestResponseTest, EchoEnum)
{
    ...
    base::RunLoop run_loop;
    provider->EchoEnum(sample::Enum::VALUE, base::BindLambdaForTesting([&](sample::Enum value) {
