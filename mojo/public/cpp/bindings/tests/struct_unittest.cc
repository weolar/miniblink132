...
template <typename StructType> struct SerializeStructHelperTraits {
    using DataView = typename StructType::DataView;
};

template <> struct SerializeStructHelperTraits<native::NativeStruct> {
...
template <typename InputType, typename DataType> size_t SerializeStruct(InputType& input, mojo::Message* message, DataType** out_data)
{
    ...
    using DataViewType = typename SerializeStructHelperTraits<StructType>::DataView;
...
{
    MessagePipe pipe;
    return MultiVersionStruct::New(123, MakeRect(5), std::string("hello"), std::vector<int8_t> { 10, 9, 8 }, std::move(pipe.handle0), false, 42);
}

template <typename U, typename T> U SerializeAndDeserialize(T input)
{
    using InputMojomType = typename T::Struct::DataView;
    using OutputMojomType = typename U::Struct::DataView;

    using InputDataType = typename mojo::internal::MojomTypeTraits<InputMojomType>::Data*;
    using OutputDataType = typename mojo::internal::MojomTypeTraits<OutputMojomType>::Data*;
    ...
    OutputDataType output_data = reinterpret_cast<OutputDataType>(message.mutable_payload());
...
TEST_F(StructTest, Serialization_ArrayPointers)
{
    ...
    NamedRegionPtr region(NamedRegion::New(std::string("region"), std::move(rects)));
...
{
    {
        ...
        MultiVersionStructPtr output = SerializeAndDeserialize<MultiVersionStructPtr>(std::move(input));
        EXPECT_TRUE(output);
...

    {
        ...
        MultiVersionStructPtr expected_output(MultiVersionStruct::New(123, MakeRect(5)));
    ...
    {
        MultiVersionStructV3Ptr input(MultiVersionStructV3::New(123, MakeRect(5), std::string("hello")));
        MultiVersionStructPtr expected_output(MultiVersionStruct::New(123, MakeRect(5), std::string("hello")));
    ...
    {
        MultiVersionStructV5Ptr input(MultiVersionStructV5::New(123, MakeRect(5), std::string("hello"), std::vector<int8_t> { 10, 9, 8 }));
        MultiVersionStructPtr expected_output(MultiVersionStruct::New(123, MakeRect(5), std::string("hello"), std::vector<int8_t> { 10, 9, 8 }));
    ...
    {
        MessagePipe pipe;
        MultiVersionStructV7Ptr input(
            MultiVersionStructV7::New(123, MakeRect(5), std::string("hello"), std::vector<int8_t> { 10, 9, 8 }, std::move(pipe.handle0), false));
...
{
    {
        ...
        MultiVersionStructV7Ptr expected_output(MultiVersionStructV7::New(123, MakeRect(5), std::string("hello"), std::vector<int8_t> { 10, 9, 8 }));
        // Save the raw handle value separately so that we can compare later.
        MojoHandle expected_handle = input->f_message_pipe.get().value();

        MultiVersionStructV7Ptr output = SerializeAndDeserialize<MultiVersionStructV7Ptr>(std::move(input));
        EXPECT_TRUE(output);
        EXPECT_EQ(expected_handle, output->f_message_pipe.get().value());
        output->f_message_pipe.reset();
        EXPECT_TRUE(output->Equals(*expected_output));
    ...
    {
        MultiVersionStructPtr input = MakeMultiVersionStruct();
        MultiVersionStructV5Ptr expected_output(MultiVersionStructV5::New(123, MakeRect(5), std::string("hello"), std::vector<int8_t> { 10, 9, 8 }));

        MultiVersionStructV5Ptr output = SerializeAndDeserialize<MultiVersionStructV5Ptr>(std::move(input));
        EXPECT_TRUE(output);
        EXPECT_TRUE(output->Equals(*expected_output));
    }
    ...
    {
        MultiVersionStructPtr input = MakeMultiVersionStruct();
        MultiVersionStructV3Ptr expected_output(MultiVersionStructV3::New(123, MakeRect(5), std::string("hello")));

        MultiVersionStructV3Ptr output = SerializeAndDeserialize<MultiVersionStructV3Ptr>(std::move(input));
        EXPECT_TRUE(output);
        EXPECT_TRUE(output->Equals(*expected_output));
    }
    ...
    {
        MultiVersionStructPtr input = MakeMultiVersionStruct();
        MultiVersionStructV1Ptr expected_output(MultiVersionStructV1::New(123, MakeRect(5)));

        MultiVersionStructV1Ptr output = SerializeAndDeserialize<MultiVersionStructV1Ptr>(std::move(input));
        EXPECT_TRUE(output);
        EXPECT_TRUE(output->Equals(*expected_output));
    }

    {
        ...
        MultiVersionStructV0Ptr output = SerializeAndDeserialize<MultiVersionStructV0Ptr>(std::move(input));
        EXPECT_TRUE(output);
        EXPECT_TRUE(output->Equals(*expected_output));
    }
}
...
TEST_F(StructTest, Serialization_NativeStruct)
{
    using Data = native::internal::NativeStruct_Data;
    {
        ...
        mojo::internal::Deserialize<native::NativeStructDataView>(data, &output_native, &message);
...
{
    EnumNestedStructPtr enum_nested_struct = EnumNestedStruct::New(EnumNestedStruct::StructEnum::FIRST);
    EXPECT_EQ(EnumNestedStruct::StructEnum::FIRST, enum_nested_struct->local_enum_state_);

    enum_nested_struct->local_enum_state_ = EnumNestedStruct::StructEnum::SECOND;

    EnumNestedStructPtr expected_output(EnumNestedStruct::New(EnumNestedStruct::StructEnum::SECOND));

    EnumNestedStructPtr output = SerializeAndDeserialize<EnumNestedStructPtr>(std::move(enum_nested_struct));
    EXPECT_TRUE(output.Equals(expected_output));
...
TEST_F(StructTest, Matcher)
{
    ...
    EXPECT_THAT(rect, Pointee(AllOf(Field(&Rect::x, 1), Field(&Rect::y, 2), Field(&Rect::width, 10), Field(&Rect::height, 20))));

    mojo::StructPtr<MultiVersionStruct> multi = MakeMultiVersionStruct();
    EXPECT_THAT(multi, Pointee(AllOf(Field(&MultiVersionStruct::f_int32, 123), Field(&MultiVersionStruct::f_rect, Pointee(Field(&Rect::x, 5))))));
}
