...
public:
    ...
    ArrayDataView<K>& keys()
    {
        return keys_;
    }
    const ArrayDataView<K>& keys() const
    {
        return keys_;
    }

    template <typename U> bool ReadKeys(U* output)
    {
        return internal::Deserialize<ArrayDataView<K>>(keys_.data_, output, keys_.message_);
    }

    ArrayDataView<V>& values()
    {
        return values_;
    }
    const ArrayDataView<V>& values() const
    {
        return values_;
    }

    template <typename U> bool ReadValues(U* output)
    {
        return internal::Deserialize<ArrayDataView<V>>(values_.data_, output, values_.message_);
    }
