...
template <typename K, typename V, typename Compare> struct MapTraits<base::flat_map<K, V, Compare>> {
    using Key = K;
    using Value = V;
    ...
    static bool Insert(base::flat_map<K, V, Compare>& input, MaybeConstKeyType&& key, MaybeConstValueType&& value)
    {
        input.emplace(std::forward<MaybeConstKeyType>(key), std::forward<MaybeConstValueType>(value));
        return true;
    }
