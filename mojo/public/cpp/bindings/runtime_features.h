...
template <typename Interface> inline constexpr bool kIsRuntimeFeatureGuarded = false;

// Helper to allow this to be a no-op on non-feature annotated interfaces.
template <typename Interface> inline constexpr bool GetRuntimeFeature_IsEnabled()
{
    return true;
}
...
template <typename Interface> inline constexpr bool GetRuntimeFeature_ExpectEnabled()
{
    return true;
}
