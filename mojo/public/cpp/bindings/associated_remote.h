...
// class AssociatedRemote : public Remote<Interface> {
// public:
//     PendingAssociatedReceiver<Interface> BindNewEndpointAndPassReceiver(scoped_refptr<base::SequencedTaskRunner> task_runner = nullptr)
//     {
//         return Remote<Interface>::BindNewPipeAndPassReceiver(task_runner);
//     }
// };
...
// template <typename Interface> class AssociatedRemote {
// public:
...
//     AssociatedRemote(AssociatedRemote&& other) noexcept { *this = std::move(other); }
//
...
//     AssociatedRemote(PendingAssociatedRemote<Interface> pending_remote, scoped_refptr<base::SequencedTaskRunner> task_runner)
//     {
...
//     {
//         DCHECK(is_bound()) << "Cannot issue Interface method calls on an unbound AssociatedRemote";
...
//     typename Interface::Proxy_& operator*() const { return *get(); }
// #else
//     typename Interface* get() const
//     {
//         return internal_state_.instance();
//     }
//     typename Interface* operator->() const { return get(); }
//     typename Interface& operator*() const { return *get(); }
// #endif
//
...
//     void set_disconnect_with_reason_handler(ConnectionErrorWithReasonCallback handler) { internal_state_.set_connection_error_with_reason_handler(std::move(handler)); }
//
...
//             return;
//         }
//         set_disconnect_handler(base::BindOnce(&AssociatedRemote::reset, base::Unretained(this)));
//     }
//
...
//     // SequencedTaskRunner.
//     [[nodiscard]] PendingAssociatedReceiver<Interface> BindNewEndpointAndPassReceiver(scoped_refptr<base::SequencedTaskRunner> task_runner = nullptr)
//     {
//         //DCHECK(!is_bound()) << "AssociatedRemote for " << Interface::Name_ << " is already bound";
//
//         ScopedInterfaceEndpointHandle remote_handle;
//         ScopedInterfaceEndpointHandle receiver_handle;
//         ScopedInterfaceEndpointHandle::CreatePairPendingAssociation(&remote_handle, &receiver_handle);
//         Bind(PendingAssociatedRemote<Interface>(std::move(remote_handle), 0), std::move(task_runner));
...
//     // run on |task_runner|. If |task_runner| is null, defaults to the current
//     // SequencedTaskRunner.
//     void Bind(PendingAssociatedRemote<Interface> pending_remote, scoped_refptr<base::SequencedTaskRunner> task_runner = nullptr)
//     {
//         //DCHECK(!is_bound()) << "AssociatedRemote for " << Interface::Name_ << " is already bound";
//         if (!pending_remote) {
//             reset();
...
//         }
//
//         // -> AssociatedInterfacePtrStateBase::Bind2
//         internal_state_.Bind(AssociatedInterfacePtrInfo<Interface>(pending_remote.PassHandle(), pending_remote.version()), std::move(task_runner));
//
...
//     // endpoints in tests.
//     [[nodiscard]] PendingAssociatedReceiver<Interface> BindNewEndpointAndPassDedicatedReceiver()
//     {
//         //DCHECK(!is_bound()) << "AssociatedRemote for " << Interface::Name_ << " is already bound";
//
//         PendingAssociatedReceiver<Interface> receiver = BindNewEndpointAndPassReceiver();
...
//         return PendingAssociatedRemote<Interface>(info.PassHandle(), info.version());
//     }
//
...
//
//     internal::AssociatedInterfacePtrState<Interface>* internal_state() { return &internal_state_; }
//
// private:
