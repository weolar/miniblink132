...
bool ServiceFactory::CanRunService(const GenericPendingReceiver& receiver) const
{
    DCHECK(receiver.is_valid());
    return base::Contains(constructors_, *receiver.interface_name());
}

bool ServiceFactory::RunService(GenericPendingReceiver receiver, base::OnceClosure termination_callback)
{
    DCHECK(receiver.is_valid());
    ...
    auto disconnect_callback = base::BindOnce(&ServiceFactory::OnInstanceDisconnected, weak_ptr_factory_.GetWeakPtr(), instance.get());
    if (termination_callback) {
        disconnect_callback = std::move(disconnect_callback).Then(std::move(termination_callback));
    }
...
void ServiceFactory::InstanceHolderBase::WatchPipe(MessagePipeHandle pipe, base::OnceClosure disconnect_callback)
{
    DCHECK(!disconnect_callback_);
    disconnect_callback_ = std::move(disconnect_callback);
    watcher_.Watch(pipe, MOJO_HANDLE_SIGNAL_READABLE, MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED,
        base::BindRepeating(&InstanceHolderBase::OnPipeSignaled, base::Unretained(this)));
}

void ServiceFactory::InstanceHolderBase::OnPipeSignaled(MojoResult result, const HandleSignalsState& state)
{
    ...
    if (result == MOJO_RESULT_FAILED_PRECONDITION || result == MOJO_RESULT_CANCELLED) {
        watcher_.Cancel();
        DCHECK(disconnect_callback_);
