...
// static
ConnectionGroup::Ref ConnectionGroup::Create(base::RepeatingClosure callback, scoped_refptr<base::SequencedTaskRunner> task_runner)
{
    return Ref(base::WrapRefCounted(new ConnectionGroup(std::move(callback), std::move(task_runner))));
}

ConnectionGroup::ConnectionGroup(base::RepeatingClosure callback, scoped_refptr<base::SequencedTaskRunner> task_runner)
...
void ConnectionGroup::ReleaseGroupRef()
{
    ...
    if (num_refs_ == 0 && notification_task_runner_) {
        notification_task_runner_->PostTask(FROM_HERE, base::BindOnce(notification_callback_));
    }
}
