...
// class AssociatedReceiver : public Receiver<Interface, ImplRefTraits> {
// public:
//     using ImplPointerType = typename ImplRefTraits::PointerType;
//
//     explicit AssociatedReceiver(ImplPointerType impl)
//         : Receiver<Interface, ImplRefTraits>(std::move(impl))
//     {
//     }
//
...
//     {
//         return Receiver<Interface>::BindNewPipeAndPassRemote(task_runner);
//     }
// };
...
//     void BindImpl(ScopedInterfaceEndpointHandle handle, MessageReceiverWithResponderStatus* receiver, std::unique_ptr<MessageReceiver> payload_validator,
//         base::span<const uint32_t> sync_method_ordinals, scoped_refptr<base::SequencedTaskRunner> runner, uint32_t interface_version, const char* interface_name,
//         MessageToMethodInfoCallback method_info_callback, MessageToMethodNameCallback method_name_callback);
//
//     void BindImpl2(ScopedInterfaceEndpointHandle handle, void* impl,
...
//         impl_ = (void*)ImplRefTraits::GetRawPointer(&impl); // ImplPointerType也可能是std::unique_ptr
//     }
//
...
//     // construction time).
//     AssociatedReceiver(ImplPointerType impl, PendingAssociatedReceiver<Interface> pending_receiver)
//         : AssociatedReceiver(std::move(impl), std::move(pending_receiver), nullptr)
//     {
//     }
//
...
//     AssociatedReceiver(ImplPointerType impl, PendingAssociatedReceiver<Interface> pending_receiver, scoped_refptr<base::SequencedTaskRunner> task_runner)
...
//     // to run on |task_runner|. If |task_runner| is null, this defaults to the
//     // current SequencedTaskRunner.
//     void Bind(PendingAssociatedReceiver<Interface> pending_receiver, scoped_refptr<base::SequencedTaskRunner> task_runner = nullptr)
//     {
//         // DCHECK(!is_bound()) << "AssociatedReceiver for " << Interface::Name_ << " is already bound";
//         if (pending_receiver) {
...
//             BindImpl2(pending_receiver.PassHandle(), impl_, std::move(task_runner), Interface::Version_, Interface::Name_);
//         } else {
//             reset();
//         }
//     }
...
//         PendingAssociatedReceiver<Interface> pending_receiver(endpoint_client_->PassHandle());
...
//     void ReportBadMessage(const std::string& error) { GetBadMessageCallback().Run(error); }
//
...
//             [](ReportBadMessageCallback inner_callback, base::WeakPtr<AssociatedReceiver> receiver, base::StringPiece error) {
