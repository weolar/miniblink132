...
template <typename T, size_t Extent> struct ArrayTraits<base::span<T, Extent>> {
    using Element = T;

    static size_t GetSize(const base::span<T>& input)
    {
        return input.size();
    }

    static T* GetData(base::span<T>& input)
    {
        return input.data();
    }

    static const T* GetData(const base::span<T>& input)
    {
        return input.data();
    }
