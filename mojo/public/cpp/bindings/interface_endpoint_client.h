...
class /*COMPONENT_EXPORT(MOJO_CPP_BINDINGS) */InterfaceEndpointClient : public MessageReceiverWithResponder {
public:
    ...
    InterfaceEndpointClient(ScopedInterfaceEndpointHandle handle, MessageReceiverWithResponderStatus* receiver,
        std::unique_ptr<MessageReceiver> payload_validator, base::span<const uint32_t> sync_method_ordinals,
        scoped_refptr<base::SequencedTaskRunner> task_runner, uint32_t interface_version, const char* interface_name,
    ...
    void SendControlMessageWithResponder(Message* message, std::unique_ptr<MessageReceiver> responder);
    ...
    bool AcceptWithResponder(Message* message, std::unique_ptr<MessageReceiver> responder) override;
    ...
    bool SendMessageWithResponder(Message* message, bool is_control_message, SyncSendMode sync_send_mode, std::unique_ptr<MessageReceiver> responder);
    ...
    // this endpoint.
    using IdleTrackingEnabledCallback = base::OnceCallback<void(ConnectionGroup::Ref connection_group)>;
...
private:
    ...
    public:
        PendingAsyncResponse(uint32_t request_message_name, std::unique_ptr<MessageReceiver> responder);
    ...
    public:
        ...
        HandleIncomingMessageThunk& operator=(const HandleIncomingMessageThunk&) = delete;
    ...
    //void OnAssociationEvent(ScopedInterfaceEndpointHandle::AssociationEvent event);
    ...
    RAW_PTR_EXCLUSION MessageReceiverWithResponderStatus* const incoming_receiver_ = nullptr;
