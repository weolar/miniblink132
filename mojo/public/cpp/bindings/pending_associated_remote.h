...
// template <typename T> class PendingAssociatedReceiver;
// template <typename T> struct PendingAssociatedRemoteConverter;

// template <typename Interface>
// class PendingAssociatedRemote : public PendingRemote<Interface> {
// public:
//     [[nodiscard]] REINITIALIZES_AFTER_MOVE PendingAssociatedReceiver<Interface> InitWithNewEndpointAndPassReceiver();
// };
...
// template <typename Interface> class PendingAssociatedRemote {
// public:
...
//     {
//     }
//     PendingAssociatedRemote(ScopedInterfaceEndpointHandle handle, uint32_t version)
//         : handle_(std::move(handle))
//         , version_(version)
//     {
//     }
//
// //     PendingAssociatedRemote(void** interface_addr, uint32_t version)
...
//     template <typename T,
//         std::enable_if_t<
//             std::is_same<PendingAssociatedRemote<Interface>, std::invoke_result_t<decltype(&PendingAssociatedRemoteConverter<T>::template To<Interface>), T&&>>::value>* = nullptr>
//     PendingAssociatedRemote(T&& other)
//         : PendingAssociatedRemote(PendingAssociatedRemoteConverter<T>::template To<Interface>(std::move(other)))
//     {
//     }
...
// template <typename Interface> PendingAssociatedReceiver<Interface> PendingAssociatedRemote<Interface>::InitWithNewEndpointAndPassReceiver()
// {
// //     ScopedInterfaceEndpointHandle receiver_handle;
// //     ScopedInterfaceEndpointHandle::CreatePairPendingAssociation(&handle_, &receiver_handle);
// //     set_version(0);
// //     return PendingAssociatedReceiver<Interface>(std::move(receiver_handle));
//     return PendingRemote<Interface>::InitWithNewPipeAndPassReceiver();
// }
