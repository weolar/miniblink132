...
template <typename Interface, template <typename> class RemoteType, template <typename> class PendingRemoteType> class RemoteSetImpl {
public:
    ...
    struct Iterator {
        ...
        explicit Iterator(typename Storage::const_iterator it)
            : it_(it)
        {
        }
    ...
    RemoteSetElementId Add(RemoteType<Interface> remote)
    {
        ...
        remote.set_disconnect_handler(base::BindOnce(&RemoteSetImpl::OnDisconnect, base::Unretained(this), id));
    ...
    RemoteSetElementId Add(PendingRemoteType<Interface> remote, scoped_refptr<base::SequencedTaskRunner> task_runner = nullptr)
    {
        DCHECK(remote.is_valid());
        return Add(RemoteType<Interface>(std::move(remote), std::move(task_runner)));
    }

    // Removes a remote from the set given |id|, if present.
    void Remove(RemoteSetElementId id)
    {
        storage_.erase(id);
    }

    // Indicates whether a remote with the given ID is present in the set.
    bool Contains(RemoteSetElementId id)
    {
        return base::Contains(storage_, id);
    }
...
template <typename Interface> using RemoteSet = RemoteSetImpl<Interface, Remote, PendingRemote>;

template <typename Interface> using AssociatedRemoteSet = RemoteSetImpl<Interface, AssociatedRemote, PendingAssociatedRemote>;
