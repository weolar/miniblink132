...
template <typename U, WTF::wtf_size_t InlineCapacity> struct ArrayTraits<WTF::Vector<U, InlineCapacity>> {
    using Element = U;
    ...
    static const U& GetAt(const WTF::Vector<U, InlineCapacity>& input, size_t index)
    {
        return input[static_cast<wtf_size_t>(index)];
    }
