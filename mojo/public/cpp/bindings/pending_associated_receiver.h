...
// template <typename T> class PendingAssociatedRemote;
// template <typename T> struct PendingAssociatedReceiverConverter;

// template <typename Interface>
// class PendingAssociatedReceiver : public PendingReceiver<Interface> {
// public:
//     [[nodiscard]] REINITIALIZES_AFTER_MOVE PendingAssociatedRemote<Interface> InitWithNewEndpointAndPassRemote();
//
//     ScopedInterfaceEndpointHandle PassHandle() { return PendingReceiver<Interface>::PassPipe(); }
//
// //     const ScopedInterfaceEndpointHandle& handle() const { return handle_; }
// //     void set_handle(ScopedInterfaceEndpointHandle handle) { handle_ = std::move(handle); }
// };
...
//     template <typename T,
//         std::enable_if_t<std::is_same<PendingAssociatedReceiver<Interface>,
...
//         : PendingAssociatedReceiver(PendingAssociatedReceiverConverter<T>::template To<Interface>(std::move(other)))
//     {
//     }
// #endif // !BUILDFLAG(IS_NACL)
//
//     PendingAssociatedReceiver(const PendingAssociatedReceiver&) = delete;
//     PendingAssociatedReceiver& operator=(const PendingAssociatedReceiver&) = delete;
//
//     ~PendingAssociatedReceiver() = default;
//
...
// //     void** PassAddr()
// //     {
// //         void** ret = interface_addr_;
// //         interface_addr_ = nullptr;
// //         return ret;
// //     }
//
...
//     void ResetWithReason(uint32_t custom_reason, const std::string& description) { handle_.ResetWithReason(custom_reason, description); }
//
//     [[nodiscard]] REINITIALIZES_AFTER_MOVE PendingAssociatedRemote<Interface> InitWithNewEndpointAndPassRemote();
//
...
// //             std::move(pipe.handle0), internal::MultiplexRouter::MULTI_INTERFACE, false, base::SequencedTaskRunnerHandle::Get());
// //         scoped_refptr<internal::MultiplexRouter> router1
// //             = internal::MultiplexRouter::CreateAndStartReceiving(std::move(pipe.handle1), internal::MultiplexRouter::MULTI_INTERFACE, true, base::SequencedTaskRunnerHandle::Get());
// //
...
// template <typename Interface>
// PendingAssociatedRemote<Interface> PendingAssociatedReceiver<Interface>::
// InitWithNewEndpointAndPassRemote()
// {
// //     ScopedInterfaceEndpointHandle remote_handle;
// //     ScopedInterfaceEndpointHandle::CreatePairPendingAssociation(&handle_, &remote_handle);
// //     return PendingAssociatedRemote<Interface>(std::move(remote_handle), 0u);
//
//     return PendingReceiver<Interface>::InitWithNewPipeAndPassRemote();
// }
