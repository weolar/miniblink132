...
// cases.
template <typename ContextType> class BinderMapWithContext {
public:
    ...
    template <typename Interface> using BinderType = typename Traits::template BinderType<Interface>;
    ...
    template <typename Interface> void Add(std::common_type_t<BinderType<Interface>> binder, scoped_refptr<base::SequencedTaskRunner> task_runner = nullptr)
    {
        binders_[Interface::Name_]
            = std::make_unique<internal::GenericCallbackBinderWithContext<ContextType>>(Traits::MakeGenericBinder(std::move(binder)), std::move(task_runner));
    }

    // Returns true if this map contains a binder for `Interface` receivers.
    template <typename Interface> bool Contains()
    {
    ...
    [[nodiscard]] bool TryBind(ContextValueType context, mojo::GenericPendingReceiver* receiver)
    {
        static_assert(!IsVoidContext::value,
    ...
    using DefaultBinder = base::RepeatingCallback<bool(ContextValueType context, mojo::GenericPendingReceiver&)>;
...
private:
    ...
    std::map<std::string, std::unique_ptr<internal::GenericCallbackBinderWithContext<ContextType>>> binders_;
