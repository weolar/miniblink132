...
template <typename RemoteType> struct SharedRemoteTraits;

template <typename Interface> struct SharedRemoteTraits<Remote<Interface>> {
    static void BindDisconnected(Remote<Interface>& remote) { std::ignore = remote.BindNewPipeAndPassReceiver(); }
};
...
template <typename Interface> class SharedRemote;

template <typename RemoteType> class SharedRemoteBase : public base::RefCountedThreadSafe<SharedRemoteBase<RemoteType>> {
public:
    ...
    InterfaceType* get()
    {
        // return &forwarder_->proxy();
        return nullptr;
    }
    InterfaceType* operator->() { return get(); }
    InterfaceType& operator*() { return *get(); }

    void set_disconnect_handler(base::OnceClosure handler, scoped_refptr<base::SequencedTaskRunner> handler_task_runner)
    {
        wrapper_->set_disconnect_handler(std::move(handler), std::move(handler_task_runner));
    }
...
private:
    ...
    template <typename Interface> friend class SharedRemote;
    template <typename Interface> friend class SharedAssociatedRemote;
    ...
    // callbacks.
    class RemoteWrapper : public base::RefCountedThreadSafe<RemoteWrapper, RemoteWrapperDeleter> {
    public:
        RemoteWrapper(PendingType remote, scoped_refptr<base::SequencedTaskRunner> task_runner)
        ...
        void set_disconnect_handler(base::OnceClosure handler, scoped_refptr<base::SequencedTaskRunner> handler_task_runner)
        {
            if (!task_runner_->RunsTasksInCurrentSequence()) {
                ...
                // correct sequence.
                task_runner_->PostTask(FROM_HERE, base::BindOnce(&RemoteWrapper::set_disconnect_handler, this, std::move(handler), std::move(handler_task_runner)));
                return;
            }
            // The actual handler will post a task to run |handler| on
            // |handler_task_runner|.
            auto wrapped_handler = base::BindOnce(base::IgnoreResult(&base::TaskRunner::PostTask), handler_task_runner, FROM_HERE, std::move(handler));
        ...
        {
            if (!task_runner_->RunsTasksInCurrentSequence()) {
                task_runner_->PostTask(FROM_HERE, base::BindOnce(&RemoteWrapper::Disconnect, this));
                return;
            }
    ...
    private:
        ...
        void DeleteOnCorrectThread() const
        {
            if (!task_runner_->RunsTasksInCurrentSequence()) {
                ...
                task_runner_->PostTask(FROM_HERE, base::BindOnce(&RemoteWrapper::DeleteOnCorrectThread, base::Unretained(this)));
            } else {
                delete this;
            }
        }
    ...
    struct RemoteWrapperDeleter {
        static void Destruct(const RemoteWrapper* wrapper) { wrapper->DeleteOnCorrectThread(); }
    };

    explicit SharedRemoteBase(scoped_refptr<RemoteWrapper> wrapper)
        : wrapper_(std::move(wrapper))
    //, forwarder_(wrapper_->CreateForwarder())
    {
        *(int*)1 = 1;
    }
    ...
    static scoped_refptr<SharedRemoteBase> Create(PendingType pending_remote, scoped_refptr<base::SequencedTaskRunner> task_runner)
    {
        return new SharedRemoteBase(base::MakeRefCounted<RemoteWrapper>(std::move(pending_remote), std::move(task_runner)));
    }
...
public:
    ...
    explicit SharedRemote(PendingRemote<Interface> pending_remote) { Bind(std::move(pending_remote), nullptr); }
    ...
    SharedRemote(PendingRemote<Interface> pending_remote, scoped_refptr<base::SequencedTaskRunner> bind_task_runner)
    {
    ...
    void set_disconnect_handler(base::OnceClosure handler, scoped_refptr<base::SequencedTaskRunner> handler_task_runner)
    {
        remote_->set_disconnect_handler(std::move(handler), std::move(handler_task_runner));
    }
    ...
    void Bind(PendingRemote<Interface> pending_remote, scoped_refptr<base::SequencedTaskRunner> bind_task_runner)
    {
        if (bind_task_runner && pending_remote) {
            remote_ = SharedRemoteBase<Remote<Interface>>::Create(std::move(pending_remote), std::move(bind_task_runner));
        } else if (pending_remote) {
            remote_ = SharedRemoteBase<Remote<Interface>>::Create(std::move(pending_remote), base::SequencedTaskRunnerHandle::Get());
        }
    }
    ...
    PendingReceiver<Interface> BindNewPipeAndPassReceiver(scoped_refptr<base::SequencedTaskRunner> bind_task_runner = nullptr)
    {
...
template <typename Interface> using SharedRemote = Remote<Interface>;
