...
ReceiverSetState::Entry::Entry(ReceiverSetState& state, ReceiverId id, std::unique_ptr<ReceiverState> receiver)
    : state_(state)
    , id_(id)
    , receiver_(std::move(receiver))
{
    receiver_->InstallDispatchHooks(
        std::make_unique<DispatchFilter>(*this), base::BindRepeating(&ReceiverSetState::Entry::OnDisconnect, base::Unretained(this)));
}
...
void ReceiverSetState::Entry::OnDisconnect(uint32_t custom_reason_code, const std::string& description)
{
    WillDispatch();
...
void ReceiverSetState::set_disconnect_with_reason_handler(RepeatingConnectionErrorWithReasonCallback handler)
{
...
ReportBadMessageCallback ReceiverSetState::GetBadMessageCallback()
{
    ...
    return base::BindOnce(
        [](ReportBadMessageCallback error_callback, base::WeakPtr<ReceiverSetState> receiver_set, ReceiverId receiver_id, base::StringPiece error) {
            std::move(error_callback).Run(error);
            if (receiver_set)
                receiver_set->Remove(receiver_id);
        },
        mojo::GetBadMessageCallback(), weak_ptr_factory_.GetWeakPtr(), current_receiver());
}

ReceiverId ReceiverSetState::Add(std::unique_ptr<ReceiverState> receiver)
{
    ReceiverId id = next_receiver_id_++;
    auto result = entries_.emplace(id, std::make_unique<Entry>(*this, id, std::move(receiver)));
...
bool ReceiverSetState::RemoveWithReason(ReceiverId id, uint32_t custom_reason_code, const std::string& description)
{
    auto it = entries_.find(id);
    if (it == entries_.end())
        return false;
...
void ReceiverSetState::SetDispatchContext(void* context, ReceiverId receiver_id)
{
    current_context_ = context;
    current_receiver_ = receiver_id;
}

void ReceiverSetState::OnDisconnect(ReceiverId id, uint32_t custom_reason_code, const std::string& description)
{
    auto it = entries_.find(id);
