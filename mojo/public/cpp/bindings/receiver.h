...
template <typename Interface, typename ImplRefTraits = RawPtrImplRefTraits<Interface>> class Receiver {
public:
    ...
    Receiver(ImplPointerType impl, PendingReceiver<Interface> pending_receiver, scoped_refptr<base::SequencedTaskRunner> task_runner)
        : receiver_impl_(std::move(impl))
    {
    ...
    bool is_bound() const
    {
        return receiver_impl_.is_bound();
    }
    ...
    {
        receiver_impl_.set_connection_error_handler(std::move(handler));
    }
    ...
    void set_disconnect_with_reason_handler(ConnectionErrorWithReasonCallback error_handler)
    {
        DCHECK(is_bound());
        receiver_impl_.set_connection_error_with_reason_handler(std::move(error_handler));
    }

    // weolar: just for VizClient::destroy
    void set_connection_error_handler_is_sync()
    {
        receiver_impl_.set_connection_error_handler_is_sync();
    }
    ...
    void reset()
    {
        receiver_impl_.Close();
    }

    // Similar to the method above, but also specifies a disconnect reason.
    void ResetWithReason(uint32_t custom_reason_code, base::StringPiece description)
    {
        receiver_impl_.CloseWithReason(custom_reason_code, description);
    }

    //     void ResetWithCallback(base::OnceClosure cb) // weolar: for asyn close
    //     {
    //         receiver_impl_.set_reset_handler(std::move(cb));
    //         receiver_impl_.Close();
    //     }

    // for --
#if ASSOCIATED_AS_ALIAS == 1
    [[nodiscard]] PendingAssociatedRemote<Interface> BindNewEndpointAndPassRemote(scoped_refptr<base::SequencedTaskRunner> task_runner = nullptr)
    {
        return BindNewPipeAndPassRemote(task_runner);
    }
#endif
    //=======
    ...
    [[nodiscard]] PendingRemote<Interface> BindNewPipeAndPassRemoteWithVersion(scoped_refptr<base::SequencedTaskRunner> task_runner = nullptr)
    {
    ...
    [[nodiscard]] PendingRemote<Interface> BindNewPipeAndPassRemote(scoped_refptr<base::SequencedTaskRunner> task_runner)
    {
        DCHECK(!is_bound()) << "Receiver for " << Interface::Name_ << " is already bound";
        PendingRemote<Interface> remote /*(receiver_impl_.impl_addr(), 0)*/;
    ...
    // Receiver.
    void Bind(PendingReceiver<Interface> pending_receiver, scoped_refptr<base::SequencedTaskRunner> task_runner)
    {
        DCHECK(!is_bound()) << "Receiver for " << Interface::Name_ << " is already bound";
        if (pending_receiver) {
            receiver_impl_.Bind(pending_receiver.internal_state(), std::move(task_runner)); // -> ReceiverImplBase::BindInternal2
        } else {
            reset();
        }
    }
    ...
    {
        DCHECK(is_bound());
        // CHECK(!receiver_impl_.HasAssociatedInterfaces());
        return receiver_impl_.Unbind();
    }
    ...
    {
        DCHECK(is_bound());
        receiver_impl_.SetFilter(std::move(filter));
    }
    ...
    {
        CHECK(!receiver_impl_.HasAssociatedInterfaces());
        receiver_impl_.PauseIncomingMethodCallProcessing();
    }

    void Resume()
    {
        receiver_impl_.ResumeIncomingMethodCallProcessing();
    }
    ...
    {
        return receiver_impl_.WaitForIncomingMethodCall();
    }
    ...
    {
        receiver_impl_.PauseRemoteCallbacksUntilFlushCompletes(std::move(flush));
    }
    ...
    {
        receiver_impl_.FlushAsync(std::move(flusher));
    }
    ...
    {
        receiver_impl_.FlushForTesting();
    }

    // Exposed for testing, should not generally be used.
    void EnableTestingMode()
    {
        receiver_impl_.EnableTestingMode();
    }
    ...
    // endpoints in tests.
    [[nodiscard]] PendingAssociatedRemote<Interface> BindNewEndpointAndPassDedicatedRemote()
    {
        DCHECK(!is_bound()) << "AssociatedReceiver for " << Interface::Name_ << " is already bound";
    ...
    {
        return receiver_impl_.SwapImplForTesting(std::move(new_impl));
    }
    ...
    {
        return receiver_impl_.GetBadMessageCallback();
    }

    // DO NOT USE. Exposed only for internal use and for testing.
    internal::ReceiverImpl<Interface, ImplRefTraits>* internal_state()
    {
        return &receiver_impl_;
    }

private:
    internal::ReceiverImpl<Interface, ImplRefTraits> receiver_impl_;
};

#if ASSOCIATED_AS_ALIAS == 1
template <typename Interface, typename ImplRefTraits = RawPtrImplRefTraits<Interface>> using AssociatedReceiver = Receiver<Interface, ImplRefTraits>;
#endif
