...
template <typename T> struct ArrayTraits<std::unordered_set<T>> {
    using Element = T;
...
template <typename T> struct ArrayTraits<std::vector<T>> {
    ...
    static size_t GetSize(const std::vector<T>& input)
    {
        return input.size();
    }

    static T* GetData(std::vector<T>& input)
    {
        return input.data();
    }

    static const T* GetData(const std::vector<T>& input)
    {
        return input.data();
    }

    static typename std::vector<T>::reference GetAt(std::vector<T>& input, size_t index)
    {
        return input[index];
    }

    static typename std::vector<T>::const_reference GetAt(const std::vector<T>& input, size_t index)
    {
        return input[index];
    }
...
template <typename T> struct ArrayTraits<std::set<T>> {
    using Element = T;
    ...
    static size_t GetSize(const std::set<T>& input)
    {
        return input.size();
    }
...
// This ArrayTraits specialization is used only for serialization.
template <typename T> struct ArrayTraits<base::flat_set<T>> {
    using Element = T;
    ...
    static size_t GetSize(const base::flat_set<T>& input)
    {
        return input.size();
    }
...
template <typename K, typename V> struct MapValuesArrayView {
    explicit MapValuesArrayView(const std::map<K, V>& map)
        : map(map)
    {
    }
...
template <typename K, typename V> MapValuesArrayView<K, V> MapValuesToArray(const std::map<K, V>& map)
{
...
template <typename K, typename V> struct ArrayTraits<MapValuesArrayView<K, V>> {
    using Element = V;
    ...
    static const V& GetValue(ConstIterator& iterator)
    {
...
template <typename T, size_t N> struct ArrayTraits<std::array<T, N>> {
    using Element = T;

    static bool IsNull(const std::array<T, N>& input)
    {
        return false;
    }

    static size_t GetSize(const std::array<T, N>& input)
    {
        return N;
    }
