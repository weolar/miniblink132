...
void OnWatcherSignaled(std::unique_ptr<MessagePipeScoperBase> scoper, MojoResult result, const HandleSignalsState& state)
{
    DCHECK(scoper);
...
// static
void MessagePipeScoperBase::StartWatchingPipe(std::unique_ptr<MessagePipeScoperBase> scoper)
{
    ...
    unowned_scoper->pipe_watcher_.Watch(unowned_scoper->pipe_.get(), MOJO_HANDLE_SIGNAL_PEER_CLOSED, MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED,
