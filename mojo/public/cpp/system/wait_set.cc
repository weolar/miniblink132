...
public:
    State()
        : handle_event_(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED)
    {
    ...
    MojoResult AddHandle(Handle handle, MojoHandleSignals signals)
    {
        ...
        MojoResult rv
            = MojoAddTrigger(trap_handle_.get().value(), handle.value(), signals, MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED, context->context_value(), nullptr);
    ...
    MojoResult RemoveHandle(Handle handle)
    {
        ...
        // |lock_| while calling it.
        MojoResult rv = MojoRemoveTrigger(trap_handle_.get().value(), context->context_value(), nullptr);
        ...
        base::WaitableEvent** ready_event, size_t* num_ready_handles, Handle* ready_handles, MojoResult* ready_results, MojoHandleSignalsState* signals_states)
    {
        ...
        DCHECK(ready_results);
        {
            ...
            if (ready_handles_.empty()) {
                ...
                uint32_t num_blocking_events = static_cast<uint32_t>(*num_ready_handles);
                ...
                for (size_t i = 0; i < num_blocking_events; ++i) {
                    blocking_events.container()[i].struct_size = sizeof(blocking_events.container()[i]);
                }
                MojoResult rv = MojoArmTrap(trap_handle_.get().value(), nullptr, &num_blocking_events, blocking_events.container().data());
                ...
                if (rv == MOJO_RESULT_FAILED_PRECONDITION) {
                    ...
                    for (size_t i = 0; i < num_blocking_events; ++i) {
                        ...
                        ready_handles_[it->second->handle()] = { event.result, event.signals_state };
                    }
        ...
        size_t index = base::WaitableEvent::WaitMany(events.container().data(), events.container().size());
        base::AutoLock lock(lock_);
...
private:
    ...
    public:
        ...
        {
            reinterpret_cast<Context*>(event->trigger_context)->Notify(event->result, event->signals_state);
        }
    ...
    void Notify(Handle handle, MojoResult result, MojoHandleSignalsState signals_state, Context* context)
    {
        base::AutoLock lock(lock_);
...
void WaitSet::Wait(
    base::WaitableEvent** ready_event, size_t* num_ready_handles, Handle* ready_handles, MojoResult* ready_results, MojoHandleSignalsState* signals_states)
{
    state_->Wait(ready_event, num_ready_handles, ready_handles, ready_results, signals_states);
}
