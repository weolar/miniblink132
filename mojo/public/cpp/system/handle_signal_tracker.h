...
public:
    using NotificationCallback = base::RepeatingCallback<void(const HandleSignalsState& signals_state)>;
    ...
    HandleSignalTracker(
        Handle handle, MojoHandleSignals signals, scoped_refptr<base::SequencedTaskRunner> task_runner = base::SequencedTaskRunner::GetCurrentDefault());
