...
public:
    ...
    MojoHandleSignalsState wait_state() const
    {
        return wait_state_;
    }
...
MojoResult Wait(Handle handle, MojoHandleSignals signals, MojoTriggerCondition condition, MojoHandleSignalsState* signals_state)
{
    ...
    rv = MojoAddTrigger(trap.get().value(), handle.value(), signals, condition, context->context_value(), nullptr);
    ...
    rv = MojoArmTrap(trap.get().value(), nullptr, &num_blocking_events, &blocking_event);
...
MojoResult WaitMany(const Handle* handles, const MojoHandleSignals* signals, size_t num_handles, size_t* result_index, MojoHandleSignalsState* signals_states)
{
    if (!handles || !signals)
    ...
    for (size_t i = 0; i < num_handles; ++i) {
        ...
        rv = MojoAddTrigger(
            trap.get().value(), handles[i].value(), signals[i], MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED, contexts[i]->context_value(), nullptr);
        if (rv == MOJO_RESULT_INVALID_ARGUMENT) {
            if (result_index)
                *result_index = i;
