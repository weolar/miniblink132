...
MOJO_CPP_SYSTEM_EXPORT MojoResult LoadCoreLibrary(absl::optional<base::FilePath> path);

// Initializes the dynamic Mojo Core library previously loaded by
// |LoadCoreLibrary()| above.
//
// This may only be called in a process that hasn't already initialized Mojo.
MOJO_CPP_SYSTEM_EXPORT MojoResult InitializeCoreLibrary(MojoInitializeFlags flags);
...
// application.
MOJO_CPP_SYSTEM_EXPORT MojoResult LoadAndInitializeCoreLibrary(absl::optional<base::FilePath> path, MojoInitializeFlags flags);
