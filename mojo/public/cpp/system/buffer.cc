...
{
    MojoCreateSharedBufferOptions options = { sizeof(options), MOJO_CREATE_SHARED_BUFFER_FLAG_NONE };
    SharedBufferHandle handle;
    MojoCreateSharedBuffer(num_bytes, &options, handle.mutable_value());
    return MakeScopedHandle(handle);
}

ScopedSharedBufferHandle SharedBufferHandle::Clone(SharedBufferHandle::AccessMode access_mode) const
{
...
ScopedSharedBufferMapping SharedBufferHandle::MapAtOffset(uint64_t size, uint64_t offset) const
{
    void* buffer = nullptr;
...
uint64_t SharedBufferHandle::GetSize() const
{
    ...
    return MojoGetBufferInfo(value(), nullptr, &buffer_info) == MOJO_RESULT_OK ? buffer_info.size : 0;
}
