...
public:
    ...
    // |signals| on |handle|.
    static scoped_refptr<Context> Create(base::WeakPtr<SimpleWatcher> watcher, scoped_refptr<base::SequencedTaskRunner> task_runner, TrapHandle trap_handle,
        Handle handle, MojoHandleSignals signals, MojoTriggerCondition condition, int watch_id, MojoResult* result, const char* handler_tag)
    {
        scoped_refptr<Context> context = new Context(watcher, task_runner, watch_id, handler_tag);
        ...
        *result = MojoAddTrigger(trap_handle.value(), handle.value(), signals, condition, context->value(), nullptr);
...
private:
    ...
    Context(base::WeakPtr<SimpleWatcher> weak_watcher, scoped_refptr<base::SequencedTaskRunner> task_runner, int watch_id, const char* handler_tag)
    ...
    void Notify(MojoResult result, MojoHandleSignalsState signals_state, MojoTrapEventFlags flags)
    {
        HandleSignalsState state(signals_state.satisfied_signals, signals_state.satisfiable_signals);
        if (!(flags & MOJO_TRAP_EVENT_FLAG_WITHIN_API_CALL) && task_runner_->RunsTasksInCurrentSequence() && weak_watcher_
...
SimpleWatcher::SimpleWatcher(
    const base::Location& from_here, ArmingPolicy arming_policy, scoped_refptr<base::SequencedTaskRunner> runner, const char* handler_tag)
    : arming_policy_(arming_policy)
    , task_runner_(std::move(runner))
    , is_default_task_runner_(base::SequencedTaskRunner::HasCurrentDefault() && task_runner_ == base::SequencedTaskRunner::GetCurrentDefault())
...
MojoResult SimpleWatcher::Watch(Handle handle, MojoHandleSignals signals, MojoTriggerCondition condition, ReadyCallbackWithState callback)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(!IsWatching());
    ...
    context_ = Context::Create(weak_factory_.GetWeakPtr(), task_runner_, trap_handle_.get(), handle_, signals, condition, watch_id_, &result, handler_tag_);
    if (!context_) {
...
void SimpleWatcher::Cancel()
{
    ...
    MojoResult rv = MojoRemoveTrigger(trap_handle_.get().value(), context->value(), nullptr);
...
MojoResult SimpleWatcher::Arm(MojoResult* ready_result, HandleSignalsState* ready_state)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    uint32_t num_blocking_events = 1;
    MojoTrapEvent blocking_event = { sizeof(blocking_event) };
    MojoResult rv = MojoArmTrap(trap_handle_.get().value(), nullptr, &num_blocking_events, &blocking_event);
    if (rv == MOJO_RESULT_FAILED_PRECONDITION) {
        ...
        if (ready_state) {
            *ready_state = HandleSignalsState(blocking_event.signals_state.satisfied_signals, blocking_event.signals_state.satisfiable_signals);
        }
    }
...
void SimpleWatcher::ArmOrNotify()
{
    ...
    DCHECK_EQ(MOJO_RESULT_FAILED_PRECONDITION, rv);
    {
        // Annotate the posted task with |handler_tag_| as the IPC interface.
        base::TaskAnnotator::ScopedSetIpcHash scoped_set_ipc_hash(handler_tag_);
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&SimpleWatcher::OnHandleReady, weak_factory_.GetWeakPtr(), watch_id_, ready_result, ready_state));
    }
}

void SimpleWatcher::OnHandleReady(int watch_id, MojoResult result, const HandleSignalsState& state)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (!callback.is_null()) {
        ...
        //             [this](perfetto::EventContext ctx) { ctx.event()->set_chrome_mojo_event_info()->set_watcher_notify_interface_tag(handler_tag_); });
