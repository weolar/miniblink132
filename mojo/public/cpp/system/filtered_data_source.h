...
class MOJO_CPP_SYSTEM_EXPORT FilteredDataSource final : public DataPipeProducer::DataSource {
public:
    class Filter {
    public:
        virtual ~Filter()
    ...
    FilteredDataSource(std::unique_ptr<DataPipeProducer::DataSource> source, std::unique_ptr<Filter> filter);
