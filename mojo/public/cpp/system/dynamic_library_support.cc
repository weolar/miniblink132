...
struct InitializationState {
    InitializationState(const absl::optional<base::FilePath>& path, MojoInitializeFlags flags)
    {
        options.flags = flags;
...
MojoResult LoadAndInitializeCoreLibrary(absl::optional<base::FilePath> path, MojoInitializeFlags flags)
{
    DCHECK_EQ(flags & MOJO_INITIALIZE_FLAG_LOAD_ONLY, 0u);
