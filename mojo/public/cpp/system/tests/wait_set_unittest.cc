...
TEST_F(WaitSetTest, Satisfied)
{
    ...
    EXPECT_TRUE((ready_handles[0] == p.handle0.get() && ready_handles[1] == p.handle1.get())
        || (ready_handles[0] == p.handle1.get() && ready_handles[1] == p.handle0.get()));
    EXPECT_EQ(MOJO_RESULT_OK, ready_results[0]);
    ...
    wait_set.Wait(nullptr, &num_ready_handles, ready_handles, ready_results, hss);
    EXPECT_EQ(1u, num_ready_handles);
    EXPECT_TRUE(ready_handles[0] == p.handle0.get() || ready_handles[0] == p.handle1.get());
    EXPECT_EQ(MOJO_RESULT_OK, ready_results[0]);
...
TEST_F(WaitSetTest, NoStarvation)
{
    ...
    for (auto& event_ptr : events) {
        event_ptr = std::make_unique<base::WaitableEvent>(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED);
        event_ptr->Signal();
    ...
    for (size_t i = 0; i < 3; ++i) {
        ...
        while (ready_events.size() < kNumTestEvents || ready_handles.size() < kNumTestPipes * 2) {
            base::WaitableEvent* ready_event = nullptr;
            size_t num_ready_handles = 1;
            Handle ready_handle;
            MojoResult ready_result = MOJO_RESULT_UNKNOWN;
