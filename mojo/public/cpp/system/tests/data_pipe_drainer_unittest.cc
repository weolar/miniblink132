...
template <typename Functor> base::RepeatingClosure BindLambda(Functor callable)
{
    return base::BindRepeating([](Functor callable) { callable(); }, callable);
}

class DataPipeDrainerTest : public testing::Test, public DataPipeDrainer::Client {
public:
...
protected:
    DataPipeDrainerTest()
    {
        ...
        EXPECT_EQ(CreateDataPipe(nullptr, producer_handle, consumer_handle), MOJO_RESULT_OK);
        drainer_ = std::make_unique<DataPipeDrainer>(this, std::move(consumer_handle));
...
TEST_F(DataPipeDrainerTest, TestCompleteIsCalledOnce)
{
    ...
    uint32_t size = 5;
    EXPECT_EQ(MOJO_RESULT_OK, producer_handle_->WriteData("hello", &size, MOJO_WRITE_DATA_FLAG_NONE));
