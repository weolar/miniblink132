...
const MojoHandleSignals kSignalReadableWritable = MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_WRITABLE;
...
TEST(CoreCppTest, GetTimeTicksNow)
{
    ...
    EXPECT_NE(static_cast<MojoTimeTicks>(0), start) << "GetTimeTicksNow should return nonzero value";
}
...
TEST(CoreCppTest, Basic)
{
    // Basic |Handle| implementation:
    {
        ...
        *h1.mutable_value() = static_cast<MojoHandle>(456);
        ...
        EXPECT_TRUE(handle_to_int.find(Handle(static_cast<MojoHandle>(13579))) == handle_to_int.end());
    ...
    // |Handle|/|ScopedHandle| functions:
    {
        ...
        EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, Wait(h.get(), ~MOJO_HANDLE_SIGNAL_NONE));
    ...
    // |MessagePipeHandle|/|ScopedMessagePipeHandle| functions:
    {
        ...
        EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, WriteMessageRaw(h_invalid, nullptr, 0, nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE));
        char buffer[10] = { 0 };
        EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, WriteMessageRaw(h_invalid, buffer, sizeof(buffer), nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE));
        ...
        // Basic tests of waiting and closing.
        {
            ...
            size_t result_index;
            MojoResult rv = WaitMany(wh.data(), sigs.data(), wh.size(), &result_index, states.data());
            ...
            EXPECT_EQ(MOJO_RESULT_FAILED_PRECONDITION, Wait(h0.get(), MOJO_HANDLE_SIGNAL_READABLE, &state));
        ...
        // Actually test writing/reading messages.
        {
            ...
            EXPECT_EQ(MOJO_RESULT_OK, Wait(h1.get(), MOJO_HANDLE_SIGNAL_READABLE, &state));
            EXPECT_EQ(kSignalReadableWritable, state.satisfied_signals);
            EXPECT_EQ(kSignalAll, state.satisfiable_signals);

            std::vector<uint8_t> bytes;
            EXPECT_EQ(MOJO_RESULT_OK, ReadMessageRaw(h1.get(), &bytes, nullptr, MOJO_READ_MESSAGE_FLAG_NONE));
            ...
            EXPECT_EQ(MOJO_RESULT_OK, WriteMessageRaw(mp.handle0.get(), kWorld, kWorldSize - 1, nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE));
            ...
            EXPECT_EQ(MOJO_RESULT_OK, WriteMessageRaw(h1.get(), kHello, kHelloSize - 1, handles, handles_count, MOJO_WRITE_MESSAGE_FLAG_NONE));

            // Read "hello" and the sent handle.
            EXPECT_EQ(MOJO_RESULT_OK, Wait(h0.get(), MOJO_HANDLE_SIGNAL_READABLE, &state));
            EXPECT_EQ(kSignalReadableWritable, state.satisfied_signals);
            EXPECT_EQ(kSignalAll, state.satisfiable_signals);

            std::vector<ScopedHandle> read_handles;
            EXPECT_EQ(MOJO_RESULT_OK, ReadMessageRaw(h0.get(), &bytes, &read_handles, MOJO_READ_MESSAGE_FLAG_NONE));
            EXPECT_EQ(kHello, std::string(bytes.begin(), bytes.end()));
            ...
            EXPECT_EQ(MOJO_RESULT_OK, Wait(mp.handle1.get(), MOJO_HANDLE_SIGNAL_READABLE, &state));
            EXPECT_EQ(kSignalReadableWritable, state.satisfied_signals);
            EXPECT_EQ(kSignalAll, state.satisfiable_signals);

            read_handles.clear();
            EXPECT_EQ(MOJO_RESULT_OK, ReadMessageRaw(mp.handle1.get(), &bytes, &read_handles, MOJO_READ_MESSAGE_FLAG_NONE));
...
TEST(CoreCppTest, TearDownWithMessagesEnqueued)
{
    ...
    // message also having a valid message pipe handle.
    {
        ...
        const char kWorld[] = "world!";
        const uint32_t kWorldSize = static_cast<uint32_t>(sizeof(kWorld));
        EXPECT_EQ(MOJO_RESULT_OK, WriteMessageRaw(h2.get(), kWorld, kWorldSize, nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE));
        // And also a message to |h3|.
        EXPECT_EQ(MOJO_RESULT_OK, WriteMessageRaw(h3.get(), kWorld, kWorldSize, nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE));
        ...
        EXPECT_EQ(MOJO_RESULT_OK, WriteMessageRaw(h1.get(), kHello, kHelloSize, &h3_value, 1, MOJO_WRITE_MESSAGE_FLAG_NONE));
...
TEST(CoreCppTest, BasicSharedBuffer)
{
    ...
    // Duplicate |h0| to |h1|.
    ScopedSharedBufferHandle h1 = h0->Clone(SharedBufferHandle::AccessMode::READ_ONLY);
