...
public:
    explicit DataPipeReader(ScopedDataPipeConsumerHandle consumer_handle, size_t read_size, base::OnceClosure on_read_done)
        : consumer_handle_(std::move(consumer_handle))
        , read_size_(read_size)
        , on_read_done_(std::move(on_read_done))
        , watcher_(FROM_HERE, SimpleWatcher::ArmingPolicy::AUTOMATIC, base::SequencedTaskRunnerHandle::Get())
    {
        watcher_.Watch(consumer_handle_.get(), MOJO_HANDLE_SIGNAL_READABLE, MOJO_WATCH_CONDITION_SATISFIED,
            base::BindRepeating(&DataPipeReader::OnDataAvailable, base::Unretained(this)));
    }
...
private:
    void OnDataAvailable(MojoResult result, const HandleSignalsState& state)
    {
        if (result == MOJO_RESULT_OK) {
            ...
            do {
                read_result = consumer_handle_->ReadData(buffer.data(), &size, MOJO_READ_DATA_FLAG_NONE);
                if (read_result == MOJO_RESULT_OK) {
                    std::copy(buffer.begin(), buffer.begin() + size, std::back_inserter(data_));
                }
...
protected:
    ...
    {
        base::FilePath temp_file_path = temp_dir_.GetPath().AppendASCII(base::StringPrintf("tmp%d", tmp_file_id_++));
        base::File temp_file(temp_file_path, base::File::FLAG_CREATE | base::File::FLAG_WRITE);
        int bytes_written = temp_file.WriteAtCurrentPos(contents.data(), static_cast<int>(contents.size()));
        CHECK_EQ(static_cast<int>(contents.size()), bytes_written);
        return temp_file_path;
    }

    static void WriteFromFileThenCloseWriter(std::unique_ptr<DataPipeProducer> producer, std::unique_ptr<FilteredDataSource::Filter> filter, base::File file)
    {
        DataPipeProducer* raw_producer = producer.get();
        raw_producer->Write(std::make_unique<FilteredDataSource>(std::make_unique<FileDataSource>(std::move(file)), std::move(filter)),
            base::BindOnce([](std::unique_ptr<DataPipeProducer> producer, MojoResult result) {}, std::move(producer)));
    }

    static void WriteFromFileThenCloseWriter(
        std::unique_ptr<DataPipeProducer> producer, std::unique_ptr<FilteredDataSource::Filter> filter, base::File file, uint64_t max_bytes)
    {
        DataPipeProducer* raw_producer = producer.get();
        auto data_source = std::make_unique<FileDataSource>(std::move(file));
        data_source->SetRange(0u, max_bytes);
        raw_producer->Write(std::make_unique<FilteredDataSource>(std::move(data_source), std::move(filter)),
            base::BindOnce([](std::unique_ptr<DataPipeProducer> producer, MojoResult result) {}, std::move(producer)));
    }
...
public:
    ...
    void OnRead(base::span<char> buffer, FilteredDataSource::ReadResult* result) override
    {
...
TEST_F(DataPipeProducerTest, WriteFromFile)
{
    ...
    ASSERT_EQ(CreateDataPipe(16, producer_handle, consumer_handle), MOJO_RESULT_OK);
    ...
    WriteFromFileThenCloseWriter(std::make_unique<DataPipeProducer>(std::move(producer_handle)), std::move(observer), std::move(file));
    loop.Run();
...
TEST_F(DataPipeProducerTest, WriteFromFilePartial)
{
    ...
    ScopedDataPipeProducerHandle producer_handle;
    ScopedDataPipeConsumerHandle consumer_handle;
    ASSERT_EQ(CreateDataPipe(static_cast<uint32_t>(kTestString.size()), producer_handle, consumer_handle), MOJO_RESULT_OK);
    DataPipeReader reader(std::move(consumer_handle), kTestString.size(), loop.QuitClosure());

    base::File file(path, base::File::FLAG_OPEN | base::File::FLAG_READ);
    DataPipeObserverData observer_data;
    auto observer = std::make_unique<TestObserver>(&observer_data);
    WriteFromFileThenCloseWriter(std::make_unique<DataPipeProducer>(std::move(producer_handle)), std::move(observer), std::move(file), kBytesToWrite);
    loop.Run();
...
TEST_F(DataPipeProducerTest, WriteFromInvalidFile)
{
    ...
    ScopedDataPipeProducerHandle producer_handle;
    ScopedDataPipeConsumerHandle consumer_handle;
    ASSERT_EQ(CreateDataPipe(kBytesToWrite, producer_handle, consumer_handle), MOJO_RESULT_OK);
    DataPipeObserverData observer_data;
    auto observer = std::make_unique<TestObserver>(&observer_data);
    DataPipeReader reader(std::move(consumer_handle), kBytesToWrite, loop.QuitClosure());
...
TEST_F(DataPipeProducerTest, HugeFile)
{
    ...
    ScopedDataPipeProducerHandle producer_handle;
    ScopedDataPipeConsumerHandle consumer_handle;
    ASSERT_EQ(CreateDataPipe(kDataPipeSize, producer_handle, consumer_handle), MOJO_RESULT_OK);
    DataPipeReader reader(std::move(consumer_handle), kDataPipeSize, loop.QuitClosure());
