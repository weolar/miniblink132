...
void WriteMessage(const ScopedMessagePipeHandle& handle, const base::StringPiece& message)
{
    MojoResult rv = WriteMessageRaw(handle.get(), message.data(), static_cast<uint32_t>(message.size()), nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE);
...
std::string ReadMessage(const ScopedMessagePipeHandle& handle)
{
    ...
    MojoResult rv = ReadMessageRaw(handle.get(), &bytes, nullptr, MOJO_READ_MESSAGE_FLAG_NONE);
    CHECK_EQ(MOJO_RESULT_OK, rv);
...
TEST_F(WaitTest, InvalidArguments)
{
    ...
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, Wait(invalid_handle, MOJO_HANDLE_SIGNAL_READABLE));

    MessagePipe p;
    Handle valid_handles[2] = { p.handle0.get(), p.handle1.get() };
    Handle invalid_handles[2];
    MojoHandleSignals signals[2] = { MOJO_HANDLE_SIGNAL_NONE, MOJO_HANDLE_SIGNAL_NONE };
    size_t result_index = 0;
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, WaitMany(invalid_handles, signals, 2, &result_index));
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, WaitMany(nullptr, signals, 2, &result_index));
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, WaitMany(valid_handles, nullptr, 2, &result_index));
}
...
TEST_F(WaitTest, Basic)
{
    ...
    EXPECT_EQ(MOJO_RESULT_OK, Wait(p.handle1.get(), MOJO_HANDLE_SIGNAL_READABLE, &c_hss));
    EXPECT_EQ(MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_WRITABLE, c_hss.satisfied_signals);

    HandleSignalsState hss;
    EXPECT_EQ(MOJO_RESULT_OK, Wait(p.handle1.get(), MOJO_HANDLE_SIGNAL_READABLE, &hss));
    EXPECT_TRUE(hss.readable() && hss.writable() && !hss.peer_closed());
    EXPECT_FALSE(hss.never_readable() || hss.never_writable() || hss.never_peer_closed());

    // Now close the writing end and wait for peer closure.

    p.handle0.reset();
    EXPECT_EQ(MOJO_RESULT_OK, Wait(p.handle1.get(), MOJO_HANDLE_SIGNAL_PEER_CLOSED, &hss));
    ...
    EXPECT_TRUE(hss.never_writable() && !hss.never_readable() && !hss.never_peer_closed());
    ...
    EXPECT_EQ(MOJO_RESULT_FAILED_PRECONDITION, Wait(p.handle1.get(), MOJO_HANDLE_SIGNAL_READABLE, &hss));

    // Sanity check the signals state again.
    EXPECT_TRUE(hss.peer_closed() && !hss.readable() && !hss.writable());
    EXPECT_TRUE(hss.never_readable() && hss.never_writable() && !hss.never_peer_closed());
}
...
TEST_F(WaitTest, CloseWhileWaiting)
{
    ...
    ThreadedRunner close_after_delay(base::BindOnce(
        ...
        &p.handle0));
    close_after_delay.Start();
    EXPECT_EQ(MOJO_RESULT_CANCELLED, Wait(p.handle0.get(), MOJO_HANDLE_SIGNAL_READABLE));
}
...
TEST_F(WaitManyTest, Basic)
{
    ...
    MojoHandleSignals signals[2] = { MOJO_HANDLE_SIGNAL_READABLE, MOJO_HANDLE_SIGNAL_READABLE };
    size_t result_index = 0;
    MojoHandleSignalsState c_hss[2];
    HandleSignalsState hss[2];

    EXPECT_EQ(MOJO_RESULT_OK, WaitMany(handles, signals, 2, &result_index, c_hss));
    EXPECT_EQ(1u, result_index);
    EXPECT_EQ(MOJO_HANDLE_SIGNAL_WRITABLE, c_hss[0].satisfied_signals);
    EXPECT_EQ(MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_WRITABLE, c_hss[1].satisfied_signals);
    ...
    EXPECT_TRUE(!hss[0].never_readable() && !hss[0].never_writable() && !hss[0].never_peer_closed());
    EXPECT_TRUE(hss[1].readable() && hss[1].writable() && !hss[1].peer_closed());
    EXPECT_TRUE(!hss[1].never_readable() && !hss[1].never_writable() && !hss[1].never_peer_closed());
    ...
    handles[0] = handles[1];
    EXPECT_EQ(MOJO_RESULT_FAILED_PRECONDITION, WaitMany(handles, signals, 1, &result_index, hss));
...
TEST_F(WaitManyTest, CloseWhileWaiting)
{
    ...
    MojoHandleSignals signals[3] = { MOJO_HANDLE_SIGNAL_READABLE, MOJO_HANDLE_SIGNAL_READABLE, MOJO_HANDLE_SIGNAL_READABLE };
    ...
    size_t result_index = 0;
    EXPECT_EQ(MOJO_RESULT_CANCELLED, WaitMany(handles, signals, 3, &result_index));
...
TEST_F(WaitManyTest, DelayedPeerClosure)
{
    ...
    EXPECT_EQ(MOJO_RESULT_FAILED_PRECONDITION, WaitMany(handles, signals, 3, &result_index, hss));
    EXPECT_EQ(2u, result_index);
    EXPECT_TRUE(!hss[2].readable() && !hss[2].writable() && hss[2].peer_closed());
    EXPECT_TRUE(hss[2].never_readable() && hss[2].never_writable() && !hss[2].never_peer_closed());
}
