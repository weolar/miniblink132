...
TEST_F(ScopeToMessagePipeTest, ObjectDestroyedOnPeerClosure)
{
    ...
    MessagePipe pipe;
    ScopeToMessagePipe(std::make_unique<RunCallbackOnDestruction>(wait_for_destruction.QuitClosure()), std::move(pipe.handle0));
    pipe.handle1.reset();
...
TEST_F(ScopeToMessagePipeTest, PipeClosedOnPeerClosure)
{
    ...
    watcher.Watch(pipe.handle0.get(), MOJO_HANDLE_SIGNAL_READABLE, MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED,
        base::BindLambdaForTesting([&](MojoResult result, const HandleSignalsState& state) {
