...
class InvitationCppTest : public testing::Test, public testing::WithParamInterface<TransportType> {
public:
...
protected:
    void LaunchChildTestClient(const std::string& test_client_name, ScopedMessagePipeHandle* primordial_pipes, size_t num_primordial_pipes,
        InvitationType invitation_type, TransportType transport_type, const ProcessErrorCallback& error_callback = {})
    {
        ...
        case TransportType::kChannel: {
            command_line.AppendSwitchASCII(kTransportTypeSwitch, kTransportTypeChannel);
            channel.emplace();
        ...
        case TransportType::kChannelServer: {
            command_line.AppendSwitchASCII(kTransportTypeSwitch, kTransportTypeChannelServer);
        ...
        child_process_ = base::SpawnMultiProcessTestChild(test_client_name, command_line, launch_options);
        if (channel)
        ...
        case TransportType::kChannel:
            ...
            if (invitation_type == InvitationType::kNormal) {
                OutgoingInvitation::Send(std::move(invitation), child_process_.Handle(), std::move(channel_endpoint), error_callback);
            } else {
                DCHECK(primordial_pipes);
                DCHECK_EQ(num_primordial_pipes, 1u);
                primordial_pipes[0] = OutgoingInvitation::SendIsolated(std::move(channel_endpoint));
            }
            break;
#if !BUILDFLAG(IS_FUCHSIA)
        case TransportType::kChannelServer:
            ...
            if (invitation_type == InvitationType::kNormal) {
                OutgoingInvitation::Send(std::move(invitation), child_process_.Handle(), std::move(server_endpoint), error_callback);
            } else {
                DCHECK(primordial_pipes);
                DCHECK_EQ(num_primordial_pipes, 1u);
                primordial_pipes[0] = OutgoingInvitation::SendIsolated(std::move(server_endpoint));
            }
            break;
#endif // !BUILDFLAG(IS_FUCHSIA)
        }
    ...
    {
        int wait_result = -1;
        base::WaitForMultiprocessTestChildExit(child_process_, TestTimeouts::action_timeout(), &wait_result);
        child_process_.Close();
        EXPECT_EQ(0, wait_result);
    }

    static void WriteMessage(const ScopedMessagePipeHandle& pipe, base::StringPiece message)
    {
        CHECK_EQ(MOJO_RESULT_OK, WriteMessageRaw(pipe.get(), message.data(), message.size(), nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE));
    }
    ...
    static std::string ReadMessage(const ScopedMessagePipeHandle& pipe)
    {
        ...
        CHECK_EQ(MOJO_RESULT_OK, ReadMessageRaw(pipe.get(), &payload, &handles, MOJO_READ_MESSAGE_FLAG_NONE));
...
#if !BUILDFLAG(IS_FUCHSIA)
        std::string transport_type_string = command_line.GetSwitchValueASCII(kTransportTypeSwitch);
...
TEST_P(InvitationCppTest, Send)
{
    ...
    LaunchChildTestClient("CppSendClient", &pipe, 1, InvitationType::kNormal, GetParam());
...
{
    ScopedMessagePipeHandle pipe;
    LaunchChildTestClient("CppSendIsolatedClient", &pipe, 1, InvitationType::kIsolated, GetParam());
    WriteMessage(pipe, kTestMessage1);
    WaitForChildExit();
}
...
TEST_P(InvitationCppTest, SendWithMultiplePipes)
{
    ...
    LaunchChildTestClient("CppSendWithMultiplePipesClient", pipes, 2, InvitationType::kNormal, GetParam());
...
TEST(InvitationCppTest_NoParam, SendIsolatedInvitationWithDuplicateName)
{
    ...
    ScopedMessagePipeHandle pipe0 = OutgoingInvitation::SendIsolated(channel1.TakeLocalEndpoint(), kConnectionName);
    ScopedMessagePipeHandle pipe1 = OutgoingInvitation::SendIsolated(channel2.TakeLocalEndpoint(), kConnectionName);
...
TEST_P(InvitationCppTest, MAYBE_ProcessErrors)
{
    ...
    LaunchChildTestClient(
        "CppProcessErrorsClient", &pipe, 1, InvitationType::kNormal, GetParam(), base::BindLambdaForTesting([&](const std::string& error_message) {
    ...
    EXPECT_EQ(MOJO_RESULT_OK, MojoReadMessage(pipe.get().value(), nullptr, &message));

    // Report the message as bad and expect to be notified through the process
    // error callback.
    base::RunLoop error_loop;
    actual_error_callback = base::BindLambdaForTesting([&](const std::string& error_message) {
...
INSTANTIATE_TEST_SUITE_P(All, InvitationCppTest,
