...
template <typename Handler> void RunResultHandler(Handler f, MojoResult result)
{
    f(result);
}

template <typename Handler> SimpleWatcher::ReadyCallback OnReady(Handler f)
{
...
TEST_F(SimpleWatcherTest, WatchBasic)
{
    ...
    SimpleWatcher b_watcher(FROM_HERE, SimpleWatcher::ArmingPolicy::AUTOMATIC, base::SequencedTaskRunnerHandle::Get());
    EXPECT_EQ(MOJO_RESULT_OK, b_watcher.Watch(b.get(), MOJO_HANDLE_SIGNAL_READABLE, OnReady([&](MojoResult result) {
    ...
    EXPECT_EQ(MOJO_RESULT_OK, WriteMessageRaw(a.get(), "hello", 5, nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE));
    run_loop.Run();
...
TEST_F(SimpleWatcherTest, WatchUnsatisfiable)
{
    ...
    SimpleWatcher b_watcher(FROM_HERE, SimpleWatcher::ArmingPolicy::MANUAL, base::SequencedTaskRunnerHandle::Get());
    EXPECT_EQ(MOJO_RESULT_OK, b_watcher.Watch(b.get(), MOJO_HANDLE_SIGNAL_READABLE, NotReached()));
...
TEST_F(SimpleWatcherTest, WatchFailedPreconditionNoSpam)
{
    ...
    ASSERT_EQ(CreateDataPipe(nullptr, producer_handle, consumer_handle), MOJO_RESULT_OK);
    bool had_failed_precondition = false;

    SimpleWatcher watcher(FROM_HERE, SimpleWatcher::ArmingPolicy::AUTOMATIC);
    MojoResult rc = watcher.Watch(consumer_handle.get(), MOJO_HANDLE_SIGNAL_READABLE, OnReady([&](MojoResult result) {
        EXPECT_FALSE(had_failed_precondition);
        switch (result) {
        case MOJO_RESULT_OK:
            const void* begin;
            uint32_t num_bytes;
            consumer_handle->BeginReadData(&begin, &num_bytes, MOJO_READ_DATA_FLAG_NONE);
    ...
    uint32_t size = 5;
    EXPECT_EQ(MOJO_RESULT_OK, producer_handle->WriteData("hello", &size, MOJO_WRITE_DATA_FLAG_NONE));
...
TEST_F(SimpleWatcherTest, WatchInvalidHandle)
{
    ...
    SimpleWatcher b_watcher(FROM_HERE, SimpleWatcher::ArmingPolicy::AUTOMATIC, base::SequencedTaskRunnerHandle::Get());
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, b_watcher.Watch(b.get(), MOJO_HANDLE_SIGNAL_READABLE, NotReached()));
...
TEST_F(SimpleWatcherTest, CloseAndCancel)
{
    ...
    SimpleWatcher b_watcher(FROM_HERE, SimpleWatcher::ArmingPolicy::AUTOMATIC, base::SequencedTaskRunnerHandle::Get());
    EXPECT_EQ(MOJO_RESULT_OK, b_watcher.Watch(b.get(), MOJO_HANDLE_SIGNAL_READABLE, OnReady([](MojoResult result) { FAIL(); })));
    EXPECT_TRUE(b_watcher.IsWatching());
...
TEST_F(SimpleWatcherTest, ManualArmOrNotifyWhileSignaled)
{
    ...
    EXPECT_EQ(MOJO_RESULT_OK,
        b_watcher1.Watch(b.get(), MOJO_HANDLE_SIGNAL_READABLE,
            base::BindRepeating(
    ...
    EXPECT_EQ(MOJO_RESULT_OK,
        b_watcher2.Watch(b.get(), MOJO_HANDLE_SIGNAL_READABLE,
            base::BindRepeating(
                [](base::RunLoop* loop, bool* notified, MojoResult result) {
                    EXPECT_EQ(result, MOJO_RESULT_OK);
                    *notified = true;
