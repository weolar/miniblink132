...
public:
    ...
    {
        base::RunLoop loop;
        tracker->set_notification_callback(base::BindRepeating([](base::RunLoop* loop, const HandleSignalsState& signals_state) { loop->Quit(); }, &loop));
        loop.Run();
        tracker->set_notification_callback(HandleSignalTracker::NotificationCallback());
    }
...
TEST_F(HandleSignalTrackerTest, StartsWithCorrectState)
{
    ...
    WriteMessageRaw(pipe.handle1.get(), "hi", 2, nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE);
    Wait(pipe.handle0.get(), MOJO_HANDLE_SIGNAL_READABLE, MOJO_WATCH_CONDITION_SATISFIED);
...
TEST_F(HandleSignalTrackerTest, BasicTracking)
{
    ...
    ReadMessageRaw(pipe.handle0.get(), &bytes, nullptr, MOJO_READ_MESSAGE_FLAG_NONE);
    WaitForNextNotification(&tracker);
    EXPECT_FALSE(tracker.last_known_state().readable());
}
...
TEST_F(HandleSignalTrackerTest, DoesntUpdateOnIrrelevantChanges)
{
    ...
    MessagePipe pipe;
    HandleSignalTracker readable_tracker(pipe.handle0.get(), MOJO_HANDLE_SIGNAL_READABLE);
    HandleSignalTracker peer_closed_tracker(pipe.handle0.get(), MOJO_HANDLE_SIGNAL_PEER_CLOSED);
