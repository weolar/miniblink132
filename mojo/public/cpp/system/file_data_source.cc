...
uint64_t CalculateEndOffset(base::File* file, MojoResult* result)
{
    ...
    if (length < 0) {
        *result = FileDataSource::ConvertFileErrorToMojoResult(file->GetLastFileError());
        return 0u;
    }
    return length;
}
...
// static
MojoResult FileDataSource::ConvertFileErrorToMojoResult(base::File::Error error)
{
    switch (error) {
...
DataPipeProducer::DataSource::ReadResult FileDataSource::Read(uint64_t offset, base::span<char> buffer)
{
    ReadResult result;
    if (error_ != MOJO_RESULT_OK)
    ...
    uint64_t read_size = std::min(static_cast<uint64_t>(std::numeric_limits<int>::max()), std::min(static_cast<uint64_t>(buffer.size()), readable_size));
    ...
    int bytes_read = file_.Read(static_cast<int64_t>(read_offset), buffer.data(), read_size);
    if (bytes_read < 0) {
        result.bytes_read = 0;
