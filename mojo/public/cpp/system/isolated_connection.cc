...
IsolatedConnection::~IsolatedConnection()
{
    ...
    OutgoingInvitation::SendIsolated(channel.TakeLocalEndpoint(), token_.ToString());
}

ScopedMessagePipeHandle IsolatedConnection::Connect(PlatformChannelEndpoint endpoint)
{
    return OutgoingInvitation::SendIsolated(std::move(endpoint), token_.ToString());
}

ScopedMessagePipeHandle IsolatedConnection::Connect(PlatformChannelServerEndpoint endpoint)
{
    return OutgoingInvitation::SendIsolated(std::move(endpoint), token_.ToString());
}
