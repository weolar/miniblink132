...
HandleSignalTracker::HandleSignalTracker(Handle handle, MojoHandleSignals signals, scoped_refptr<base::SequencedTaskRunner> task_runner)
    : high_watcher_(FROM_HERE, SimpleWatcher::ArmingPolicy::MANUAL, task_runner)
    , low_watcher_(FROM_HERE, SimpleWatcher::ArmingPolicy::MANUAL, std::move(task_runner))
{
    MojoResult rv = high_watcher_.Watch(
        handle, signals, MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED, base::BindRepeating(&HandleSignalTracker::OnNotify, base::Unretained(this)));
    DCHECK_EQ(MOJO_RESULT_OK, rv);

    rv = low_watcher_.Watch(
        handle, signals, MOJO_TRIGGER_CONDITION_SIGNALS_UNSATISFIED, base::BindRepeating(&HandleSignalTracker::OnNotify, base::Unretained(this)));
    DCHECK_EQ(MOJO_RESULT_OK, rv);
...
void HandleSignalTracker::OnNotify(MojoResult result, const HandleSignalsState& state)
{
