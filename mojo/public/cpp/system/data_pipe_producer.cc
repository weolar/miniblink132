...
class DataPipeProducer::SequenceState : public base::RefCountedDeleteOnSequence<SequenceState> {
public:
    using CompletionCallback = base::OnceCallback<void(ScopedDataPipeProducerHandle producer, MojoResult result)>;

    SequenceState(ScopedDataPipeProducerHandle producer_handle, scoped_refptr<base::SequencedTaskRunner> file_task_runner, CompletionCallback callback,
        scoped_refptr<base::SequencedTaskRunner> callback_task_runner)
        : base::RefCountedDeleteOnSequence<SequenceState>(std::move(file_task_runner))
    ...
    void Cancel()
    {
        ...
        is_cancelled_ = true;
        owning_task_runner()->PostTask(FROM_HERE, base::BindOnce(&SequenceState::CancelOnSequence, this));
    }

    void Start(std::unique_ptr<DataSource> data_source)
    {
        owning_task_runner()->PostTask(FROM_HERE, base::BindOnce(&SequenceState::StartOnSequence, this, std::move(data_source)));
    }
...
private:
    ...
    void StartOnSequence(std::unique_ptr<DataSource> data_source)
    {
        ...
        if (producer_handle_.is_valid()) {
            ...
            watcher_ = std::make_unique<SimpleWatcher>(FROM_HERE, SimpleWatcher::ArmingPolicy::AUTOMATIC, base::SequencedTaskRunnerHandle::Get());
            watcher_->Watch(
                producer_handle_.get(), MOJO_HANDLE_SIGNAL_WRITABLE, MOJO_WATCH_CONDITION_SATISFIED, base::BindRepeating(&SequenceState::OnHandleReady, this));
        }
    }
    ...
    void TransferSomeBytes()
    {
        while (true) {
            ...
            const uint64_t max_data_size = data_source_->GetLength() - bytes_transferred_;
            if (max_data_size == 0) {
            ...
            MojoResult mojo_result = producer_handle_->BeginWriteData(&pipe_buffer, &size, MOJO_WRITE_DATA_FLAG_NONE);
            ...
            DataSource::ReadResult result = data_source_->Read(bytes_transferred_, read_buffer);
    ...
    void Finish(MojoResult result)
    {
        watcher_.reset();
        data_source_.reset();
        callback_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(callback_), std::move(producer_handle_), result));
    }
...
void DataPipeProducer::Write(std::unique_ptr<DataSource> data_source, CompletionCallback callback)
{
...
void DataPipeProducer::InitializeNewRequest(CompletionCallback callback)
{
    ...
    auto file_task_runner = base::ThreadPool::CreateSequencedTaskRunner({ base::MayBlock(), base::TaskPriority::USER_VISIBLE });
    sequence_state_ = new SequenceState(std::move(producer_), file_task_runner,
        base::BindOnce(&DataPipeProducer::OnWriteComplete, weak_factory_.GetWeakPtr(), std::move(callback)), base::SequencedTaskRunnerHandle::Get());
}

void DataPipeProducer::OnWriteComplete(CompletionCallback callback, ScopedDataPipeProducerHandle producer, MojoResult ready_result)
{
    producer_ = std::move(producer);
