...
public:
    ...
    // of the notification.
    using ReadyCallbackWithState = base::RepeatingCallback<void(MojoResult result, const HandleSignalsState& state)>;
    ...
    SimpleWatcher(const base::Location& from_here, ArmingPolicy arming_policy,
        scoped_refptr<base::SequencedTaskRunner> runner = base::SequencedTaskRunner::GetCurrentDefault(), const char* handler_tag = nullptr);
    ...
    MojoResult Watch(Handle handle, MojoHandleSignals signals, MojoTriggerCondition condition, ReadyCallbackWithState callback);
    ...
    // a notification.
    MojoResult Watch(Handle handle, MojoHandleSignals signals, ReadyCallback callback)
    {
        return Watch(handle, signals, MOJO_WATCH_CONDITION_SATISFIED, base::BindRepeating(&DiscardReadyState, std::move(callback)));
    }
    ...
    MojoResult Arm(MojoResult* ready_result = nullptr, HandleSignalsState* ready_state = nullptr);
    ...
    ReadyCallbackWithState ready_callback() const
    {
        return callback_;
    }
...
private:
    ...
    static void DiscardReadyState(const ReadyCallback& callback, MojoResult result, const HandleSignalsState& state)
    {
        callback.Run(result);
    }

    void OnHandleReady(int watch_id, MojoResult result, const HandleSignalsState& state);
