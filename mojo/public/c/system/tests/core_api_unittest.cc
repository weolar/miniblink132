...
const MojoHandleSignals kSignalReadadableWritable = MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_WRITABLE;

const MojoHandleSignals kSignalAll = MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_WRITABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED | MOJO_HANDLE_SIGNAL_PEER_REMOTE
    | MOJO_HANDLE_SIGNAL_QUOTA_EXCEEDED;

TEST(CoreAPITest, GetTimeTicksNow)
{
    const MojoTimeTicks start = MojoGetTimeTicksNow();
    EXPECT_NE(static_cast<MojoTimeTicks>(0), start) << "MojoGetTimeTicksNow should return nonzero value";
}
...
TEST(CoreAPITest, InvalidHandle)
{
    ...
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoWriteMessage(h0, MOJO_MESSAGE_HANDLE_INVALID, nullptr));
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoReadMessage(h0, nullptr, nullptr));

    // Data pipe:
    buffer_size = static_cast<uint32_t>(sizeof(buffer));
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoWriteData(h0, buffer, &buffer_size, nullptr));
    write_pointer = nullptr;
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoBeginWriteData(h0, nullptr, &write_pointer, &buffer_size));
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoEndWriteData(h0, 1, nullptr));
    buffer_size = static_cast<uint32_t>(sizeof(buffer));
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoReadData(h0, nullptr, buffer, &buffer_size));
    read_pointer = nullptr;
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoBeginReadData(h0, nullptr, &read_pointer, &buffer_size));
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoEndReadData(h0, 1, nullptr));

    // Shared buffer:
    h1 = MOJO_HANDLE_INVALID;
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoDuplicateBufferHandle(h0, nullptr, &h1));
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoMapBuffer(h0, 0, 1, nullptr, &write_pointer));
}
...
TEST(CoreAPITest, BasicMessagePipe)
{
    ...
    EXPECT_EQ(MOJO_RESULT_OK, MojoSetMessageContext(message, kTestMessageContext, nullptr, nullptr, nullptr));
    ...
    Handle handle0(h0);
    EXPECT_EQ(MOJO_RESULT_OK, mojo::WaitMany(&handle0, &sig, 1, &result_index, states));
    ...
    EXPECT_EQ(MOJO_RESULT_OK, MojoSetMessageContext(message, 0, nullptr, nullptr, nullptr));
    ...
    EXPECT_EQ(MOJO_RESULT_OK, mojo::Wait(mojo::Handle(h1), MOJO_HANDLE_SIGNAL_PEER_CLOSED, &state));

    // |h1| should no longer be readable or writable.
    EXPECT_EQ(MOJO_RESULT_FAILED_PRECONDITION, mojo::Wait(mojo::Handle(h1), MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_WRITABLE, &state));
...
TEST(CoreAPITest, BasicDataPipe)
{
    ...
    EXPECT_EQ(MOJO_HANDLE_SIGNAL_WRITABLE, state.satisfied_signals);
    EXPECT_EQ(MOJO_HANDLE_SIGNAL_WRITABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED | MOJO_HANDLE_SIGNAL_PEER_REMOTE, state.satisfiable_signals);

    // Try to read from |hc|.
    buffer_size = static_cast<uint32_t>(sizeof(buffer));
    EXPECT_EQ(MOJO_RESULT_SHOULD_WAIT, MojoReadData(hc, nullptr, buffer, &buffer_size));

    // Try to begin a two-phase read from |hc|.
    read_pointer = nullptr;
    EXPECT_EQ(MOJO_RESULT_SHOULD_WAIT, MojoBeginReadData(hc, nullptr, &read_pointer, &buffer_size));
    ...
    EXPECT_EQ(MOJO_RESULT_OK, mojo::WaitMany(&consumer_handle, &sig, 1, &result_index, states));

    EXPECT_EQ(0u, result_index);
    EXPECT_EQ(MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_NEW_DATA_READABLE, states[0].satisfied_signals);
    ...
    EXPECT_EQ(MOJO_RESULT_OK, MojoBeginWriteData(hp, nullptr, &write_pointer, &buffer_size));
    static const char kWorld[] = "world";
    ASSERT_GE(buffer_size, sizeof(kWorld));
    // Include the terminating null.
    memcpy(write_pointer, kWorld, sizeof(kWorld));
    EXPECT_EQ(MOJO_RESULT_OK, MojoEndWriteData(hp, static_cast<uint32_t>(sizeof(kWorld)), nullptr));
    ...
    EXPECT_EQ(MOJO_RESULT_OK, mojo::Wait(mojo::Handle(hc), MOJO_HANDLE_SIGNAL_PEER_CLOSED, &state));
    ...
    for (;;) {
        read_pointer = nullptr;
        const MojoResult result = MojoBeginReadData(hc, nullptr, &read_pointer, &buffer_size);
    ...
    EXPECT_EQ(MOJO_RESULT_FAILED_PRECONDITION, mojo::Wait(mojo::Handle(hc), MOJO_HANDLE_SIGNAL_READABLE, &state));
...
TEST(CoreAPITest, BasicSharedBuffer)
{
    ...
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoGetBufferInfo(h0, nullptr, &buffer_info));

    // Create a shared buffer (|h0|).
    EXPECT_EQ(MOJO_RESULT_OK, MojoCreateSharedBuffer(100, nullptr, &h0));
    EXPECT_NE(h0, MOJO_HANDLE_INVALID);

    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoGetBufferInfo(h0, nullptr, nullptr));
