...
public:
    ...
    void Run() override
    {
        ...
        for (;;) {
            MojoResult result = mojo::WriteMessageRaw(mojo::MessagePipeHandle(handle_), buffer, num_bytes_, nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE);
            ...
            assert(result == MOJO_RESULT_INVALID_ARGUMENT || result == MOJO_RESULT_FAILED_PRECONDITION);
            break;
        }
    }
...
public:
    ...
    void Run() override
    {
        for (;;) {
            ...
            MojoResult result = mojo::ReadMessageRaw(mojo::MessagePipeHandle(handle_), &bytes, nullptr, MOJO_READ_MESSAGE_FLAG_NONE);
            if (result == MOJO_RESULT_OK) {
                num_reads_++;
                continue;
            }
...
public:
    ...
    static void MessagePipe_CreateAndClose(void* closure)
    {
        ...
        [[maybe_unused]] MojoResult result = MojoCreateMessagePipe(nullptr, &self->h0_, &self->h1_);
    ...
    static void MessagePipe_WriteAndRead(void* closure)
    {
        ...
        [[maybe_unused]] MojoResult result
            = mojo::WriteMessageRaw(mojo::MessagePipeHandle(self->h0_), self->buffer_.data(), self->buffer_.size(), nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE);
        assert(result == MOJO_RESULT_OK);
        result = mojo::ReadMessageRaw(mojo::MessagePipeHandle(self->h1_), &self->buffer_, nullptr, MOJO_READ_MESSAGE_FLAG_NONE);
        assert(result == MOJO_RESULT_OK);
    }
    ...
    static void MessagePipe_EmptyRead(void* closure)
    {
        ...
        [[maybe_unused]] MojoResult result = MojoReadMessage(self->h0_, nullptr, &message);
        assert(result == MOJO_RESULT_SHOULD_WAIT);
    }

protected:
#if !defined(WIN32)
    void DoMessagePipeThreadedTest(unsigned num_writers, unsigned num_readers, uint32_t num_bytes)
    {
        ...
        [[maybe_unused]] MojoResult result = MojoCreateMessagePipe(nullptr, &h0_, &h1_);
        assert(result == MOJO_RESULT_OK);
        ...
        snprintf(sub_test_name, sizeof(sub_test_name), "%uw_%ur_%ubytes", num_writers, num_readers, static_cast<unsigned>(num_bytes));
        mojo::test::LogPerfResult(
            "MessagePipe_Threaded_Writes", sub_test_name, 1000000.0 * static_cast<double>(num_writes) / (end_time - start_time), "writes/second");
        mojo::test::LogPerfResult(
            "MessagePipe_Threaded_Reads", sub_test_name, 1000000.0 * static_cast<double>(num_reads) / (end_time - start_time), "reads/second");
    }
#endif // !defined(WIN32)
...
{
    mojo::test::IterateAndReportPerf("Iterate_NoOp", nullptr, &CorePerftest::NoOp, this);
}

TEST_F(CorePerftest, MessagePipe_CreateAndClose)
{
    mojo::test::IterateAndReportPerf("MessagePipe_CreateAndClose", nullptr, &CorePerftest::MessagePipe_CreateAndClose, this);
}
...
TEST_F(CorePerftest, MessagePipe_WriteAndRead)
{
    ...
    assert(result == MOJO_RESULT_OK);
    buffer_.resize(10);
    mojo::test::IterateAndReportPerf("MessagePipe_WriteAndRead", "10bytes", &CorePerftest::MessagePipe_WriteAndRead, this);
    buffer_.resize(100);
    mojo::test::IterateAndReportPerf("MessagePipe_WriteAndRead", "100bytes", &CorePerftest::MessagePipe_WriteAndRead, this);
    buffer_.resize(1000);
    mojo::test::IterateAndReportPerf("MessagePipe_WriteAndRead", "1000bytes", &CorePerftest::MessagePipe_WriteAndRead, this);
    buffer_.resize(10000);
    mojo::test::IterateAndReportPerf("MessagePipe_WriteAndRead", "10000bytes", &CorePerftest::MessagePipe_WriteAndRead, this);
    result = MojoClose(h0_);
    assert(result == MOJO_RESULT_OK);
    result = MojoClose(h1_);
    assert(result == MOJO_RESULT_OK);
...
TEST_F(CorePerftest, MessagePipe_EmptyRead)
{
    [[maybe_unused]] MojoResult result = MojoCreateMessagePipe(nullptr, &h0_, &h1_);
    assert(result == MOJO_RESULT_OK);
    mojo::test::IterateAndReportPerf("MessagePipe_EmptyRead", nullptr, &CorePerftest::MessagePipe_EmptyRead, this);
    result = MojoClose(h0_);
    assert(result == MOJO_RESULT_OK);
    result = MojoClose(h1_);
    assert(result == MOJO_RESULT_OK);
