...
MOJO_STATIC_ASSERT(sizeof(int64_t) == 2 * sizeof(int32_t), "Bad static_assert() failure in global scope");

TEST(MacrosTest, CompileAssert)
{
    // Then in a local scope.
    MOJO_STATIC_ASSERT(sizeof(int32_t) == 2 * sizeof(int16_t), "Bad static_assert() failure");
}
