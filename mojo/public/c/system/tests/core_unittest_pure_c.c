...
#define FAILURE(message) __FILE__ "(" STRINGIFY2(__LINE__) "): Failure: " message
...
const char* MinimalCTest(void)
{
    ...
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoQueryHandleSignalsState(handle0, NULL));
    ...
    EXPECT_EQ(MOJO_RESULT_OK, MojoAppendMessageData(message, message_size, NULL, 0, &options, &buffer, NULL));
    ...
    EXPECT_EQ(MOJO_RESULT_OK, MojoGetMessageData(message, NULL, &buffer, &message_size, NULL, NULL));
