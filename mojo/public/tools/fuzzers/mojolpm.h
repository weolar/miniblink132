...
{
    return std::string(__PRETTY_FUNCTION__).substr(38, strlen(__PRETTY_FUNCTION__) - 39);
}
#endif
...
public:
    ...
    template <typename T> T* GetInstance(uint32_t id);
    ...
    // matching instance.
    template <typename T> std::unique_ptr<T> GetAndRemoveInstance(uint32_t id);

    // Lookup the previously stored instance of type T using a fuzzy-match on
    // the provided id, and remove that instance from the object storage.
    template <typename T> void RemoveInstance(uint32_t id);
    ...
    template <typename T> uint32_t AddInstance(T instance);
    ...
    template <typename T> uint32_t AddInstance(uint32_t id, T instance);
    ...
    template <typename T> uint32_t NextId();
...
private:
    ...
    struct Storage {
        ...
        template <typename T> explicit Storage(T&& value);
    ...
    template <typename T> class StorageTraits {
    public:
    ...
    template <typename T> class StorageTraits<::mojo::Remote<T>> {
    public:
        explicit StorageTraits(Context* context)
            : context_(context)
        {
    ...
    template <typename T> class StorageTraits<::mojo::AssociatedRemote<T>> {
    public:
        explicit StorageTraits(Context* context)
            : context_(context)
        {
    ...
    template <typename T> class StorageTraits<std::unique_ptr<::mojo::Receiver<T>>> {
    public:
        explicit StorageTraits(Context* context)
            : context_(context)
        {
    ...
    template <typename T> class StorageTraits<std::unique_ptr<::mojo::AssociatedReceiver<T>>> {
    public:
        explicit StorageTraits(Context* context)
            : context_(context)
        {
    ...
    std::map<TypeId, std::map<uint32_t, Storage>> instances_ GUARDED_BY_CONTEXT(sequence_checker_);
...
template <typename T> Context::Storage::Storage(T&& value)
{
    wrapper_ = std::make_unique<StorageWrapper<T>>(std::forward<T>(value));
}

template <typename T> T& Context::Storage::Storage::get()
{
...
template <typename T> const T& Context::Storage::Storage::get() const
{
    DCHECK(wrapper_->type() == type_id<T>());
    DCHECK(static_cast<StorageWrapper<T>*>(wrapper_.get()));
    return static_cast<StorageWrapper<T>*>(wrapper_.get())->value();
...
template <typename T> std::unique_ptr<T> Context::Storage::Storage::release()
{
    DCHECK(wrapper_->type() == type_id<T>());
    DCHECK(static_cast<StorageWrapper<T>*>(wrapper_.get()));
    return std::make_unique<T>(std::move(static_cast<StorageWrapper<T>*>(wrapper_.get())->value()));
}
...
template <typename T> TypeId Context::Storage::StorageWrapper<T>::type() const
{
    return type_id<T>();
}

template <typename T> T& Context::Storage::StorageWrapper<T>::value()
{
    return value_;
}

template <typename T> const T& Context::Storage::StorageWrapper<T>::value() const
{
    return value_;
}

template <typename T> void Context::StorageTraits<::mojo::Remote<T>>::OnInstanceAdded(uint32_t id)
{
    ...
    instance->set_disconnect_handler(base::BindOnce(&Context::RemoveInstance<::mojo::Remote<T>>, base::Unretained(context_), id));
}

template <typename T> void Context::StorageTraits<::mojo::AssociatedRemote<T>>::OnInstanceAdded(uint32_t id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(context_->sequence_checker_);
    ...
    CHECK(instance);
    // Unretained is safe here since context_ owns instance.
    instance->set_disconnect_handler(base::BindOnce(&Context::RemoveInstance<::mojo::AssociatedRemote<T>>, base::Unretained(context_), id));
}

template <typename T> void Context::StorageTraits<std::unique_ptr<::mojo::Receiver<T>>>::OnInstanceAdded(uint32_t id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(context_->sequence_checker_);
    context_->interface_type_ids_.insert(type_id<std::unique_ptr<::mojo::Receiver<T>>>());
    auto instance = context_->GetInstance<std::unique_ptr<::mojo::Receiver<T>>>(id);
    CHECK(instance);
    // Unretained is safe here since context_ owns instance.
    (*instance)->set_disconnect_handler(base::BindOnce(&Context::RemoveInstance<std::unique_ptr<::mojo::Receiver<T>>>, base::Unretained(context_), id));
}

template <typename T> void Context::StorageTraits<std::unique_ptr<::mojo::AssociatedReceiver<T>>>::OnInstanceAdded(uint32_t id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(context_->sequence_checker_);
    context_->interface_type_ids_.insert(type_id<std::unique_ptr<::mojo::AssociatedReceiver<T>>>());
    auto instance = context_->GetInstance<std::unique_ptr<::mojo::AssociatedReceiver<T>>>(id);
    CHECK(instance);
    // Unretained is safe here since context_ owns instance.
    (*instance)->set_disconnect_handler(
        base::BindOnce(&Context::RemoveInstance<std::unique_ptr<::mojo::AssociatedReceiver<T>>>, base::Unretained(context_), id));
}

template <typename T> T* Context::GetInstance(uint32_t id)
{
...
template <typename T> std::unique_ptr<T> Context::GetAndRemoveInstance(uint32_t id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
template <typename T> void Context::RemoveInstance(uint32_t id)
{
    mojolpmdbg("RemoveInstance(%s, %u) = ", type_name<T>().c_str(), id);
    RemoveInstance(type_id<T>(), id);
}

template <typename T> uint32_t Context::AddInstance(T instance)
{
    return AddInstance(1, std::move(instance));
}

template <typename T> uint32_t Context::AddInstance(uint32_t id, T instance)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    auto instances_iter = instances_.find(type_id<T>());
    if (instances_iter == instances_.end()) {
...
template <typename T> uint32_t Context::NextId()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    uint32_t id = 1;
    auto instances_iter = instances_.find(type_id<T>());
...
template <typename T> std::unique_ptr<mojo::Remote<T>> NewRemote()
{
    // mojolpmdbg("default NewInstance<>\n");
    return nullptr;
}

template <typename T> std::unique_ptr<mojo::AssociatedRemote<T>> NewAssociatedRemote()
{
...
template <typename ProtoTestcase, typename ProtoAction, int kMaxActionCount = 128, int kMaxActionSize = 300 * 1024 * 1024> class Testcase {
protected:
    ...
    //
    // This will be called from the fuzzer sequence.
    virtual void RunAction(const ProtoAction& action, base::OnceClosure done_closure) = 0;

private:
    template <typename T> friend void RunTestcase(T* testcase, scoped_refptr<base::SequencedTaskRunner> fuzzer_task_runner, base::OnceClosure done_closure);
    ...
    void Run(scoped_refptr<base::TaskRunner> fuzzer_task_runner, base::OnceClosure done_closure);
...
template <typename ProtoTestcase, typename ProtoAction, int kMaxActionCount, int kMaxActionSize>
Testcase<ProtoTestcase, ProtoAction, kMaxActionCount, kMaxActionSize>::Testcase(const ProtoTestcase& proto_testcase)
...
template <typename ProtoTestcase, typename ProtoAction, int kMaxActionCount, int kMaxActionSize>
bool Testcase<ProtoTestcase, ProtoAction, kMaxActionCount, kMaxActionSize>::IsFinished()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void Testcase<ProtoTestcase, ProtoAction, kMaxActionCount, kMaxActionSize>::Run(
    scoped_refptr<base::TaskRunner> fuzzer_task_runner, base::OnceClosure done_closure)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (IsFinished()) {
        ...
        auto run_closure = base::BindOnce(
            &Testcase<ProtoTestcase, ProtoAction, kMaxActionCount, kMaxActionSize>::Run, base::Unretained(this), fuzzer_task_runner, std::move(done_closure));

        auto sequence_idx = proto_testcase_.sequence_indexes(next_sequence_idx_);
        const auto& sequence = proto_testcase_.sequences(sequence_idx % proto_testcase_.sequences_size());
        if (next_action_idx_ < sequence.action_indexes_size()) {
            auto action_idx = sequence.action_indexes(next_action_idx_++);
            const auto& action = proto_testcase_.actions(action_idx % proto_testcase_.actions_size());
...
template <typename T> void RunTestcase(T* testcase, scoped_refptr<base::SequencedTaskRunner> fuzzer_task_runner, base::OnceClosure done_closure)
{
    auto teardown = base::BindOnce(&T::TearDown, base::Unretained(testcase), std::move(done_closure));

    auto start_fuzzing = base::BindOnce(&T::Run, base::Unretained(testcase), fuzzer_task_runner, std::move(teardown));

    fuzzer_task_runner->PostTask(FROM_HERE, base::BindOnce(&T::SetUp, base::Unretained(testcase), std::move(start_fuzzing)));
}
...
bool FromProto(const ::mojolpm::DataPipeConsumerHandle& input, mojo::ScopedDataPipeConsumerHandle& output);
bool ToProto(const mojo::ScopedDataPipeConsumerHandle& input, ::mojolpm::DataPipeConsumerHandle& output);
bool FromProto(const ::mojolpm::DataPipeProducerHandle& input, mojo::ScopedDataPipeProducerHandle& output);
bool ToProto(const mojo::ScopedDataPipeProducerHandle& input, ::mojolpm::DataPipeProducerHandle& output);
bool FromProto(const ::mojolpm::MessagePipeHandle& input, mojo::ScopedMessagePipeHandle& output);
bool ToProto(const mojo::ScopedMessagePipeHandle& input, ::mojolpm::MessagePipeHandle& output);
bool FromProto(const ::mojolpm::SharedBufferHandle& input, mojo::ScopedSharedBufferHandle& output);
bool ToProto(const mojo::ScopedSharedBufferHandle& input, ::mojolpm::SharedBufferHandle& output);
bool FromProto(const ::mojolpm::PlatformHandle& input, mojo::PlatformHandle& output);
bool ToProto(const mojo::PlatformHandle& input, ::mojolpm::PlatformHandle& output);
