...
struct Environment {
    Environment()
    {
        base::ThreadPoolInstance::CreateAndStartWithDefaultParams("MojoFuzzerMessageDumpProcess");
        mojo::core::Init();
    }
...
public:
    ...
    {
        base::FilePath path = directory_.Append(FILE_PATH_LITERAL("message_") + base::NumberToString(count_++) + FILE_PATH_LITERAL(".mojomsg"));
...
auto GetComplexFuzzUnion(fuzz::mojom::FuzzUnionPtr in)
{
    ...
    std::remove_reference<decltype(outer[fuzz::mojom::FuzzEnum::FUZZ_VALUE0])>::type inner;
...
fuzz::mojom::FuzzStructPtr GetPopulatedFuzzStruct()
{
    ...
    auto union_struct_map = GetStructMapFuzzUnion(fuzz::mojom::FuzzDummyStruct::New());
    ...
    auto fuzz_union_array = GetFuzzStructUnionArrayValue(std::move(union_complex));
    auto fuzz_struct_array = GetFuzzStructStructArrayValue(fuzz::mojom::FuzzStruct::New());
    ...
    return fuzz::mojom::FuzzStruct::New(true, /* fuzz_bool */
        -1, /* fuzz_int8 */
        1, /* fuzz_uint8 */
        -(1 << 8), /* fuzz_int16 */
        1 << 8, /* fuzz_uint16 */
...
void DumpMessages(std::string output_directory)
{
    ...
    env->impl->receiver_.internal_state()->RouterForTesting()->SetIncomingMessageFilter(std::make_unique<MessageDumper>(output_directory));
    ...
    fuzz->FuzzArgs(fuzz::mojom::FuzzStruct::New(), fuzz::mojom::FuzzStructPtr(nullptr));
    fuzz->FuzzArgs(fuzz::mojom::FuzzStruct::New(), GetPopulatedFuzzStruct());
    fuzz->FuzzArgsResp(fuzz::mojom::FuzzStruct::New(), GetPopulatedFuzzStruct(), base::BindOnce(FuzzCallback));
    fuzz->FuzzArgsResp(fuzz::mojom::FuzzStruct::New(), GetPopulatedFuzzStruct(), base::BindOnce(FuzzCallback));
    fuzz->FuzzArgsSyncResp(fuzz::mojom::FuzzStruct::New(), GetPopulatedFuzzStruct(), base::BindOnce(FuzzCallback));
    fuzz->FuzzArgsSyncResp(fuzz::mojom::FuzzStruct::New(), GetPopulatedFuzzStruct(), base::BindOnce(FuzzCallback));
...
int main(int argc, char** argv)
{
    ...
    env->main_thread_task_executor.task_runner()->PostTask(FROM_HERE, base::BindOnce(&DumpMessages, output_directory));
