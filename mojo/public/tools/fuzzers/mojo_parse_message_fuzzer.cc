...
void FuzzMessage(const uint8_t* data, size_t size, base::RunLoop* run)
{
    ...
    MojoAppendMessageData(handle->value(), static_cast<uint32_t>(size), nullptr, 0, &options, &buffer, &buffer_size);
...
struct Environment {
    Environment()
        : main_thread_task_executor(base::MessagePumpType::UI)
    {
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    base::RunLoop run;
    env->main_thread_task_executor.task_runner()->PostTask(FROM_HERE, base::BindOnce(&FuzzMessage, data, size, &run));
    run.Run();
