...
// API function.
const char* ParseValidationTest(const char* input, std::size_t* num_handles, std::uint8_t** data, std::size_t* data_len)
{
    std::vector<std::uint8_t> data_vec;
    std::string error;
    if (!mojo::test::ParseValidationTestInput(std::string(input), &data_vec, num_handles, &error)) {
