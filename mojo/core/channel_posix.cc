...
public:
    ...
    {
        if (message_) {
            UMA_HISTOGRAM_TIMES("Mojo.Channel.WriteMessageLatency", base::TimeTicks::Now() - start_time_);
        }
    }
    ...
    {
        return message_->data_num_bytes() - offset_;
    }
    ...
    size_t num_handles_sent()
    {
        return num_handles_sent_;
    }
...
void ChannelPosix::Start()
{
    if (io_task_runner_->RunsTasksInCurrentSequence()) {
        ...
        io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&ChannelPosix::StartOnIOThread, this));
    }
}
...
void ChannelPosix::ShutDownImpl()
{
    ...
    io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&ChannelPosix::ShutDownOnIOThread, this));
}
...
void ChannelPosix::Write(MessagePtr message)
{
...
#endif
    if (log_histograms) {
        UMA_HISTOGRAM_COUNTS_100000("Mojo.Channel.WriteMessageSize", message->data_num_bytes());
        UMA_HISTOGRAM_COUNTS_100("Mojo.Channel.WriteMessageHandles", message->NumHandlesForTransit());
    }
    ...
    if (write_error) {
        ...
        io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&ChannelPosix::OnWriteError, this, Error::kDisconnected));
    }
}
...
bool ChannelPosix::GetReadPlatformHandles(const void* payload, size_t payload_size, size_t num_handles, const void* extra_header, size_t extra_header_size,
...
bool ChannelPosix::GetReadPlatformHandlesForIpcz(size_t num_handles, std::vector<PlatformHandle>& handles)
{
...
void ChannelPosix::StartOnIOThread()
{
    ...
    read_watcher_ = std::make_unique<base::MessagePumpForIO::FdWatchController>(FROM_HERE);
    base::CurrentThread::Get()->AddDestructionObserver(this);
    if (server_.is_valid()) {
        base::CurrentIOThread::Get()->WatchFileDescriptor(
            server_.platform_handle().GetFD().get(), false /* persistent */, base::MessagePumpForIO::WATCH_READ, read_watcher_.get(), this);
    } else {
        write_watcher_ = std::make_unique<base::MessagePumpForIO::FdWatchController>(FROM_HERE);
        base::CurrentIOThread::Get()->WatchFileDescriptor(socket_.get(), true /* persistent */, base::MessagePumpForIO::WATCH_READ, read_watcher_.get(), this);
        base::AutoLock lock(write_lock_);
...
void ChannelPosix::WaitForWriteOnIOThreadNoLock()
{
    ...
    if (io_task_runner_->RunsTasksInCurrentSequence()) {
        pending_write_ = true;
        base::CurrentIOThread::Get()->WatchFileDescriptor(
            socket_.get(), false /* persistent */, base::MessagePumpForIO::WATCH_WRITE, write_watcher_.get(), this);
    } else {
        io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&ChannelPosix::WaitForWriteOnIOThread, this));
    }
}
...
void ChannelPosix::OnFileCanReadWithoutBlocking(int fd)
{
    ...
    } while (bytes_read == buffer_capacity && total_bytes_read < kMaxBatchReadCapacity && next_read_size > 0);
    if (read_error) {
...
bool ChannelPosix::WriteNoLock(MessageView message_view)
{
    ...
    do {
        ...
        if (handles_written < num_handles) {
            iovec iov = { const_cast<void*>(message_view.data()), message_view.data_num_bytes() };
            size_t num_handles_to_send = std::min(num_handles - handles_written, kMaxSendmsgHandles);
            ...
            if (result >= 0) {
                ...
                MessagePtr fds_message = Message::CreateMessage(sizeof(int) * fds.size(), 0, Message::MessageType::HANDLES_SENT);
                ...
                for (size_t i = 0; i < fds.size(); ++i) {
                    handles[i + handles_written] = PlatformHandleInTransit(PlatformHandle(std::move(fds[i])));
                }
            }
        } else {
...
bool ChannelPosix::FlushOutgoingMessagesNoLock()
{
    ...
    if (!messages.empty()) {
        UMA_HISTOGRAM_COUNTS_1000("Mojo.Channel.WriteQueuePendingMessages", messages.size());
    }
...
bool ChannelPosix::WriteOutgoingMessagesWithWritev()
{
    ...
    size_t num_messages_to_send = std::min<size_t>(IOV_MAX, outgoing_messages_.size());
    iovec iov[num_messages_to_send];
    memset(&iov[0], 0, sizeof(iov));

    // Populate the iov.
    size_t num_iovs_set = 0;
    for (auto it = outgoing_messages_.begin(); num_iovs_set < num_messages_to_send; ++it) {
    ...
    while (iov_offset < num_iovs_set) {
        ssize_t bytes_written = SocketWritev(socket_.get(), &iov[iov_offset], num_iovs_set - iov_offset);
        if (bytes_written < 0) {
        ...
        while (bytes_remaining > 0) {
            if (bytes_remaining >= outgoing_messages_.front().data_num_bytes()) {
                ...
                iov[iov_offset].iov_base = const_cast<void*>(outgoing_messages_.front().data());
...
bool ChannelPosix::FlushOutgoingMessagesWritevNoLock()
{
    do {
        ...
        // single message.
        while (!outgoing_messages_.empty() && (outgoing_messages_.front().num_handles_remaining() > 0 || outgoing_messages_.size() == 1)) {
...
bool ChannelPosix::OnControlMessage(Message::MessageType message_type, const void* payload, size_t payload_size, std::vector<PlatformHandle> handles)
{
    switch (message_type) {
    ...
    case Message::MessageType::HANDLES_SENT: {
        if (payload_size == 0)
            break;
        MessagePtr message = Message::CreateMessage(payload_size, 0, Message::MessageType::HANDLES_SENT_ACK);
...
scoped_refptr<Channel> Channel::Create(
    Delegate* delegate, ConnectionParams connection_params, HandlePolicy handle_policy, scoped_refptr<base::SingleThreadTaskRunner> io_task_runner)
{
#if !BUILDFLAG(IS_NACL) && (BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_ANDROID))
    return new ChannelLinux(delegate, std::move(connection_params), handle_policy, io_task_runner);
#else
    return new ChannelPosix(delegate, std::move(connection_params), handle_policy, io_task_runner);
#endif
}
