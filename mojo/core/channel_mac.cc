...
class ChannelMac : public Channel, public base::CurrentThread::DestructionObserver, public base::MessagePumpKqueue::MachPortWatcher {
public:
    ChannelMac(Delegate* delegate, ConnectionParams connection_params, HandlePolicy handle_policy, scoped_refptr<base::SingleThreadTaskRunner> io_task_runner)
        ...
        , watch_controller_(FROM_HERE)
    {
        ...
        if (connection_params.server_endpoint().is_valid()) {
            channel_handle = connection_params.TakeServerEndpoint().TakePlatformHandle();
        } else {
    ...
    void Start() override
    {
        io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&ChannelMac::StartOnIOThread, this));
    }

    void ShutDownImpl() override
    {
        io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&ChannelMac::ShutDownOnIOThread, this));
    }
    ...
    bool GetReadPlatformHandles(const void* payload, size_t payload_size, size_t num_handles, const void* extra_header, size_t extra_header_size,
        std::vector<PlatformHandle>* handles, bool* deferred) override
    {
        ...
        if (extra_header_size < sizeof(Message::MachPortsExtraHeader) + (incoming_handles.size() * sizeof(Message::MachPortsEntry))) {
            return false;
        }

        const auto* mach_ports_header = reinterpret_cast<const Message::MachPortsExtraHeader*>(extra_header);
        if (mach_ports_header->num_ports != incoming_handles.size()) {
            return false;
        }

        for (uint16_t i = 0; i < mach_ports_header->num_ports; ++i) {
            auto type = static_cast<PlatformHandle::Type>(mach_ports_header->entries[i].type);
            if (type == PlatformHandle::Type::kNone) {
                return false;
            } else if (type == PlatformHandle::Type::kFd && incoming_handles[i].is_mach_send()) {
    ...
    bool GetReadPlatformHandlesForIpcz(size_t num_handles, std::vector<PlatformHandle>& handles) override
    {
...
private:
    ...
    void StartOnIOThread()
    {
        ...
        kern_return_t kr = vm_allocate(mach_task_self(), &address, size, VM_MAKE_TAG(VM_MEMORY_MACH_MSG) | VM_FLAGS_ANYWHERE);
        ...
        kr = vm_allocate(mach_task_self(), &address, size, VM_MAKE_TAG(VM_MEMORY_MACH_MSG) | VM_FLAGS_ANYWHERE);
        MACH_CHECK(kr == KERN_SUCCESS, kr) << "vm_allocate";
        ...
        if (send_port_ != MACH_PORT_NULL) {
            ...
            CHECK(base::mac::CreateMachPort(&receive_port_, nullptr, MACH_PORT_QLIMIT_LARGE));
        ...
        base::CurrentIOThread::Get()->WatchMachReceivePort(receive_port_.get(), &watch_controller_, this);
    }
    ...
    bool RequestSendDeadNameNotification()
    {
        ...
        kern_return_t kr = mach_port_request_notification(mach_task_self(), send_port_.get(), MACH_NOTIFY_DEAD_NAME, 0, receive_port_.get(),
            MACH_MSG_TYPE_MAKE_SEND_ONCE, base::mac::ScopedMachSendRight::Receiver(previous).get());
        if (kr != KERN_SUCCESS) {
            // If port is already a dead name (i.e. the receiver is already gone),
            // then the channel should be shut down by the caller.
            MACH_LOG_IF(ERROR, kr != KERN_INVALID_ARGUMENT, kr) << "mach_port_request_notification";
            return false;
        }
        return true;
    }
    ...
    void SendHandshake()
    {
        ...
        message.msgh_bits = MACH_MSGH_BITS(MACH_MSG_TYPE_COPY_SEND, MACH_MSG_TYPE_MAKE_SEND);
        ...
        kern_return_t kr = mach_msg(&message, MACH_SEND_MSG, sizeof(message), 0, MACH_PORT_NULL, MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL);
        if (kr != KERN_SUCCESS) {
    ...
    bool ReceiveHandshake(base::BufferIterator<const char> buffer)
    {
        ...
        if (message->msgh_id != kChannelMacHandshakeMsgId || message->msgh_local_port == MACH_PORT_NULL) {
            OnError(Error::kConnectionFailed);
            return false;
        }
        ...
        memcpy(peer_audit_token_.get(), &trailer->msgh_audit, sizeof(audit_token_t));
    ...
    void SendPendingMessagesLocked() EXCLUSIVE_LOCKS_REQUIRED(write_lock_)
    {
        ...
        if (send_buffer_contains_message_ && !reject_writes_) {
            auto* header = reinterpret_cast<mach_msg_header_t*>(send_buffer_.address());
    ...
    bool SendMessageLocked(MessagePtr message) EXCLUSIVE_LOCKS_REQUIRED(write_lock_)
    {
        DCHECK(!send_buffer_contains_message_);
        base::BufferIterator<char> buffer(reinterpret_cast<char*>(send_buffer_.address()), send_buffer_.size());
        ...
        *header = mach_msg_header_t {};
        ...
        const size_t mach_header_size = sizeof(mach_msg_header_t) + sizeof(mach_msg_body_t) + (handles.size() * sizeof(mach_msg_port_descriptor_t));
        const size_t expected_message_size = round_msg(mach_header_size + sizeof(uint64_t) + message->data_num_bytes() + sizeof(mach_msg_audit_trailer_t));
        const bool transfer_message_ool = expected_message_size >= send_buffer_.size();

        const bool is_complex = !handles.empty() || transfer_message_ool;

        header->msgh_bits = MACH_MSGH_BITS_REMOTE(MACH_MSG_TYPE_COPY_SEND) | (is_complex ? MACH_MSGH_BITS_COMPLEX : 0);
        header->msgh_remote_port = send_port_.get();
        header->msgh_id = transfer_message_ool ? kChannelMacOOLMsgId : kChannelMacInlineMsgId;

        auto* body = buffer.MutableObject<mach_msg_body_t>();
        body->msgh_descriptor_count = handles.size();

        auto descriptors = buffer.MutableSpan<mach_msg_port_descriptor_t>(handles.size());
        for (size_t i = 0; i < handles.size(); ++i) {
            ...
            case PlatformHandle::Type::kFd: {
                ...
                // closed.
                kern_return_t kr = fileport_makeport(handle.GetFD().get(), &descriptor->name);
                if (kr != KERN_SUCCESS) {
            ...
            default:
                NOTREACHED() << "Unsupported handle type " << static_cast<int>(handle.type());
                OnWriteErrorLocked(Error::kDisconnected);
            }
        }
    ...
    bool MachMessageSendLocked(mach_msg_header_t* header) EXCLUSIVE_LOCKS_REQUIRED(write_lock_)
    {
        ...
        if (kr != KERN_SUCCESS) {
            if (kr == MACH_SEND_TIMED_OUT) {
                ...
                io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&ChannelMac::SendPendingMessages, this));
            } else {
    ...
    void OnMachMessageReceived(mach_port_t port) override
    {
        ...
        base::BufferIterator<const char> buffer(reinterpret_cast<const char*>(receive_buffer_.address()), receive_buffer_.size());
        auto* header = buffer.MutableObject<mach_msg_header_t>();
        *header = mach_msg_header_t {};
        ...
        kern_return_t kr = mach_msg(header, rcv_options, 0, header->msgh_size, receive_port_.get(),
            /*timeout=*/0, MACH_PORT_NULL);
        if (kr != KERN_SUCCESS) {
        ...
        if (header->msgh_id == MACH_NOTIFY_DEAD_NAME) {
            ...
            if (memcmp(&trailer->msgh_audit, &kernel_audit_token, sizeof(audit_token_t)) == 0) {
        ...
        if (peer_audit_token_) {
            ...
            auto* trailer = buffer.Object<mach_msg_audit_trailer_t>();
            if (memcmp(&trailer->msgh_audit, peer_audit_token_.get(), sizeof(audit_token_t)) != 0) {
        ...
        auto descriptors = buffer.MutableSpan<mach_msg_port_descriptor_t>(mojo_handle_count);
        for (auto& descriptor : descriptors) {
            ...
            case MACH_MSG_TYPE_MOVE_SEND:
                incoming_handles_.emplace_back(base::mac::ScopedMachSendRight(descriptor.name));
                descriptor.name = MACH_PORT_NULL;
                break;
            case MACH_MSG_TYPE_MOVE_RECEIVE:
                incoming_handles_.emplace_back(base::mac::ScopedMachReceiveRight(descriptor.name));
                descriptor.name = MACH_PORT_NULL;
                break;
            default:
                DLOG(ERROR) << "Unhandled descriptor disposition " << descriptor.disposition;
                OnError(Error::kReceivedMalformedData);
                return;
            }
        }
        ...
        if (transfer_message_ool) {
            ...
            payload = base::span<const char>(reinterpret_cast<const char*>(descriptor->address), descriptor->size);
            ...
            ool_memory.reset_unaligned(reinterpret_cast<vm_address_t>(descriptor->address), descriptor->size);
        } else {
    ...
    {
        reject_writes_ = true;
        io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&ChannelMac::OnError, this, error));
    }
...
scoped_refptr<Channel> Channel::Create(Channel::Delegate* delegate, ConnectionParams connection_params, Channel::HandlePolicy handle_policy,
    scoped_refptr<base::SingleThreadTaskRunner> io_task_runner)
{
    return new ChannelMac(delegate, std::move(connection_params), handle_policy, io_task_runner);
}
