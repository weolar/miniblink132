...
public:
    ...
    static std::unique_ptr<ports::UserMessageEvent> CreateEventForNewMessage(MojoCreateMessageFlags flags);
    ...
    // returning.
    static Channel::MessagePtr FinalizeEventMessage(std::unique_ptr<ports::UserMessageEvent> event);
    ...
    {
        return !IsSerialized() || is_committed_;
    }
    ...
    void set_source_node(const ports::NodeName& name)
    {
        source_node_ = name;
    }
    const ports::NodeName& source_node() const
    {
        return source_node_;
    }

    MojoResult SetContext(uintptr_t context, MojoMessageContextSerializer serializer, MojoMessageContextDestructor destructor);
    MojoResult AppendData(uint32_t additional_payload_size, const MojoHandle* handles, uint32_t num_handles);
    ...
    MojoResult ExtractSerializedHandles(ExtractBadHandlePolicy bad_handle_policy, MojoHandle* handles);
...
private:
    ...
    UserMessageImpl(ports::UserMessageEvent* message_event, MojoCreateMessageFlags flags);
    ...
    UserMessageImpl(ports::UserMessageEvent* message_event, Channel::MessagePtr channel_message, void* header, size_t header_size, void* user_payload,
