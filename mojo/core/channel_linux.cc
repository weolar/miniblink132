...
void KernelVersionNumbers(int32_t* major_version, int32_t* minor_version, int32_t* bugfix_version)
{
    struct utsname info;
    if (uname(&info) < 0) {
        NOTREACHED();
...
base::ScopedFD CreateSealedMemFD(size_t size)
{
    ...
    base::ScopedFD fd(syscall(__NR_memfd_create, "mojo_channel_linux", MFD_CLOEXEC | MFD_ALLOW_SEALING));
    if (!fd.is_valid()) {
...
class EventFDNotifier : public DataAvailableNotifier, public base::MessagePumpForIO::FdWatcher {
public:
    ...
    static std::unique_ptr<EventFDNotifier> CreateWriteNotifier()
    {
        static bool zero_on_wake_supported = []() -> bool {
            base::ScopedFD fd(syscall(__NR_eventfd2, 0, kEfdFlags | EFD_ZERO_ON_WAKE));
            return fd.is_valid();
        }();
    ...
    static std::unique_ptr<EventFDNotifier> CreateReadNotifier(
        base::ScopedFD efd, base::RepeatingClosure cb, scoped_refptr<base::SingleThreadTaskRunner> io_task_runner, bool zero_on_wake)
    {
        DCHECK(io_task_runner->RunsTasksInCurrentSequence());
        DCHECK(cb);

        return WrapFDWithCallback(std::move(efd), std::move(cb), io_task_runner, zero_on_wake);
    }
    ...
    bool Clear() override
    {
        ...
        uint64_t value = 0;
        ssize_t res = HANDLE_EINTR(read(fd_.get(), reinterpret_cast<void*>(&value), sizeof(value)));
    ...
    bool zero_on_wake() const
    {
        return zero_on_wake_;
    }
...
private:
    ...
    explicit EventFDNotifier(base::ScopedFD fd, base::RepeatingClosure cb, scoped_refptr<base::SingleThreadTaskRunner> io_task_runner, bool zero_on_wake)
        : DataAvailableNotifier(std::move(cb))
        , zero_on_wake_(zero_on_wake)
        , fd_(std::move(fd))
        , io_task_runner_(io_task_runner)
    {
        watcher_ = std::make_unique<base::MessagePumpForIO::FdWatchController>(FROM_HERE);
        WaitForEventFDOnIOThread();
    }

    static std::unique_ptr<EventFDNotifier> WrapFD(base::ScopedFD fd, bool zero_on_wake)
    {
        return base::WrapUnique<EventFDNotifier>(new EventFDNotifier(std::move(fd), zero_on_wake));
    }

    static std::unique_ptr<EventFDNotifier> WrapFDWithCallback(
        base::ScopedFD fd, base::RepeatingClosure cb, scoped_refptr<base::SingleThreadTaskRunner> io_task_runner, bool zero_on_wake)
    {
        return base::WrapUnique<EventFDNotifier>(new EventFDNotifier(std::move(fd), std::move(cb), io_task_runner, zero_on_wake));
    }

    void WaitForEventFDOnIOThread()
    {
        DCHECK(io_task_runner_->RunsTasksInCurrentSequence());
        base::CurrentIOThread::Get()->WatchFileDescriptor(fd_.get(), true, base::MessagePumpForIO::WATCH_READ, watcher_.get(), this);
    }
...
public:
    ...
    static std::unique_ptr<SharedBuffer> Create(const base::ScopedFD& memfd, size_t size)
    {
        if (!memfd.is_valid()) {
            return nullptr;
        }
        ...
        if (!base::SharedMemorySecurityPolicy::AcquireReservationForMapping(size)) {
            LOG(ERROR) << "Unable to create shared buffer: unable to acquire reservation";
            return nullptr;
        }

        uint8_t* ptr = reinterpret_cast<uint8_t*>(mmap(nullptr, size, PROT_READ | PROT_WRITE, MAP_SHARED, memfd.get(), 0));
    ...
    uint8_t* usable_region_ptr() const
    {
        return base_ptr_ + kReservedSpace;
    }
    size_t usable_len() const
    {
        return len_ - kReservedSpace;
    }
    bool is_valid() const
    {
        return base_ptr_ != nullptr && len_ > 0;
    }
    ...
    void Initialize()
    {
        new (static_cast<void*>(base_ptr_)) ControlStructure;
    }
    ...
    Error TryWrite(const void* data, size_t len)
    {
        ...
        if (len > usable_len()) {
            UMA_HISTOGRAM_COUNTS_100000("Mojo.Channel.Linux.SharedMemWriteBytes_Fail_TooLarge", len);
            return Error::kGeneralError;
        }

        if (!TryLockForWriting()) {
            UMA_HISTOGRAM_COUNTS_100000("Mojo.Channel.Linux.SharedMemWriteBytes_Fail_NoLock", len);
            return Error::kGeneralError;
        }
        ...
        uint32_t space_available = usable_len() - NumBytesInUse(cur_read_pos, cur_write_pos);

        if (space_available <= len) {
            UnlockForWriting();
            UMA_HISTOGRAM_COUNTS_100000("Mojo.Channel.Linux.SharedMemWriteBytes_Fail_NoSpace", len);
        ...
        if ((usable_len() - cur_write_pos) > len) {
            ...
            memcpy(usable_region_ptr(), reinterpret_cast<const uint8_t*>(data) + copy1_len, len - copy1_len);
        }
        ...
        if (write_pos().exchange((cur_write_pos + len) % usable_len()) != cur_write_pos) {
            UnlockForWriting();
            return Error::kControlCorruption;
        }
    ...
    Error TryReadLocked(void* data, uint32_t len, uint32_t* bytes_read)
    {
        ...
        uint32_t bytes_available_to_read = NumBytesInUse(cur_read_pos, cur_write_pos);
        ...
        if (cur_read_pos < cur_write_pos) {
            ...
            bytes_from_read_to_end = std::min(bytes_from_read_to_end, bytes_available_to_read);
            memcpy(data, usable_region_ptr() + cur_read_pos, bytes_from_read_to_end);

            if (bytes_from_read_to_end < bytes_available_to_read) {
                memcpy(reinterpret_cast<uint8_t*>(data) + bytes_from_read_to_end, usable_region_ptr(), bytes_available_to_read - bytes_from_read_to_end);
            }
        }
        ...
        uint32_t new_read_pos = (cur_read_pos + bytes_available_to_read) % usable_len();
    ...
    {
        read_flag().clear(std::memory_order_release);
    }
...
private:
    ...
    constexpr static size_t kReservedSpace = RoundUpToWordBoundary(sizeof(ControlStructure));
    ...
    SharedBuffer(uint8_t* ptr, size_t len)
        : base_ptr_(ptr)
        , len_(len)
    {
    }
...
ChannelLinux::ChannelLinux(
    Delegate* delegate, ConnectionParams connection_params, HandlePolicy handle_policy, scoped_refptr<base::SingleThreadTaskRunner> io_task_runner)
    : ChannelPosix(delegate, std::move(connection_params), handle_policy, io_task_runner)
...
void ChannelLinux::Write(MessagePtr message)
{
    ...
    SharedBuffer::Error write_result = write_buffer_->TryWrite(message->data(), message->data_num_bytes());
    if (write_result == SharedBuffer::Error::kGeneralError) {
        ...
        // horribly wrong.
        io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&ChannelLinux::OnWriteError, this, Channel::Error::kReceivedMalformedData));
        return;
    }
...
bool ChannelLinux::OnControlMessage(Message::MessageType message_type, const void* payload, size_t payload_size, std::vector<PlatformHandle> handles)
{
    switch (message_type) {
    case Message::MessageType::UPGRADE_OFFER: {
        ...
        const UpgradeOfferMessage* msg = reinterpret_cast<const UpgradeOfferMessage*>(payload);
        if (!UpgradeOfferMessage::IsValidVersion(msg->version)) {
            LOG(ERROR) << "Reject shared mem upgrade unexpected version: " << msg->version;
            RejectUpgradeOffer();
            return true;
        }
        ...
        if (msg->version == UpgradeOfferMessage::kEventFdNotifier || msg->version == UpgradeOfferMessage::kEventFdZeroWakeNotifier) {
            bool zero_on_wake = msg->version == UpgradeOfferMessage::kEventFdZeroWakeNotifier;
            read_notifier = EventFDNotifier::CreateReadNotifier(
                handles[1].TakeFD(), base::BindRepeating(&ChannelLinux::SharedMemReadReady, this), io_task_runner_, zero_on_wake);
        }
        ...
        std::unique_ptr<SharedBuffer> read_sb = SharedBuffer::Create(std::move(memfd), msg->num_pages * base::GetPageSize());
    ...
    return ChannelPosix::OnControlMessage(message_type, payload, payload_size, std::move(handles));
}
...
void ChannelLinux::SharedMemReadReady()
{
    ...
    if (read_buffer_->TryLockForReading()) {
        ...
        do {
            ...
            SharedBuffer::Error read_res = read_buffer_->TryReadLocked(read_buf_.data(), read_buf_.size(), &bytes_read);
            ...
            while (bytes_read - data_offset > 0) {
                ...
                DispatchResult result
                    = TryDispatchMessage(base::make_span(reinterpret_cast<char*>(read_buf_.data() + data_offset), bytes_read - data_offset), &read_size_hint);
...
void ChannelLinux::OfferSharedMemUpgradeInternal()
{
    ...
    std::unique_ptr<SharedBuffer> write_buffer = SharedBuffer::Create(memfd, kSize);
    ...
    std::unique_ptr<EventFDNotifier> write_notifier = EventFDNotifier::CreateWriteNotifier();
    if (!write_notifier) {
    ...
    MessagePtr msg = Message::CreateMessage(sizeof(UpgradeOfferMessage),
        /*num handles=*/fds.size(), Message::MessageType::UPGRADE_OFFER);
...
bool ChannelLinux::KernelSupportsUpgradeRequirements()
{
    static bool supported = []() -> bool {
        ...
        KernelVersionNumbers(&os_major_version, &os_minor_version, &os_bugfix_version);
        ...
        if (base::android::BuildInfo::GetInstance()->sdk_int() < base::android::SdkVersion::SDK_VERSION_Q) {
            return false;
        }
#endif
...
// static
void ChannelLinux::SetSharedMemParameters(bool enabled, uint32_t num_pages, bool use_zero_on_wake)
{
