...
class MOJO_SYSTEM_IMPL_EXPORT Dispatcher : public base::RefCountedThreadSafe<Dispatcher> {
public:
    ...
    virtual MojoResult WatchDispatcher(scoped_refptr<Dispatcher> dispatcher, MojoHandleSignals signals, MojoTriggerCondition condition, uintptr_t context);
    ...
    virtual MojoResult Arm(uint32_t* num_blocking_events, MojoTrapEvent* blocking_events);
    ...
    virtual MojoResult WriteMessage(std::unique_ptr<ports::UserMessageEvent> message);
    ...
    virtual MojoResult ReadMessage(std::unique_ptr<ports::UserMessageEvent>* message);
    ...
    virtual MojoResult DuplicateBufferHandle(const MojoDuplicateBufferHandleOptions* options, scoped_refptr<Dispatcher>* new_dispatcher);
    ...
    virtual MojoResult MapBuffer(uint64_t offset, uint64_t num_bytes, std::unique_ptr<PlatformSharedMemoryMapping>* mapping);
    ...
    virtual MojoResult ReadData(const MojoReadDataOptions& options, void* elements, uint32_t* num_bytes);
    ...
    virtual MojoResult BeginReadData(const void** buffer, uint32_t* buffer_num_bytes);
    ...
    virtual MojoResult WriteData(const void* elements, uint32_t* num_bytes, const MojoWriteDataOptions& options);

    // Supports the the |MojoBeginWriteData()| API if implemented by this
    // Dispatcher. Arguments correspond to the ones given to the original API
    // call. See |MojoBeginWriteData()| documentation.
    virtual MojoResult BeginWriteData(void** buffer, uint32_t* buffer_num_bytes, MojoBeginWriteDataFlags flags);
    ...
    virtual MojoResult AttachMessagePipe(base::StringPiece name, ports::PortRef remote_peer_port);
    ...
    virtual MojoResult ExtractMessagePipe(base::StringPiece name, MojoHandle* message_pipe_handle);
    ...
    virtual MojoResult QueryQuota(MojoQuotaType type, uint64_t* limit, uint64_t* usage);
    ...
    // removals.
    virtual MojoResult AddWatcherRef(const scoped_refptr<WatcherDispatcher>& watcher, uintptr_t context);

    // Removes a WatcherDispatcher reference from this dispatcher.
    virtual MojoResult RemoveWatcherRef(WatcherDispatcher* watcher, uintptr_t context);
    ...
    virtual void StartSerialize(uint32_t* num_bytes, uint32_t* num_ports, uint32_t* num_platform_handles);
    ...
    // condition.
    virtual bool EndSerialize(void* destination, ports::PortName* ports, PlatformHandle* handles);
    ...
    static scoped_refptr<Dispatcher> Deserialize(Type type, const void* bytes, size_t num_bytes, const ports::PortName* ports, size_t num_ports,
        PlatformHandle* platform_handles, size_t platform_handle_count);
...
MOJO_SYSTEM_IMPL_EXPORT inline std::ostream& operator<<(std::ostream& out, Dispatcher::Type type)
{
