...
#if BUILDFLAG(IS_NACL)
template <typename T> void GenerateRandomName(T* out)
{
...
Channel::MessagePtr SerializeEventMessage(ports::ScopedEvent event)
{
    if (event->type() == ports::Event::Type::kUserMessage) {
        ...
        return UserMessageImpl::FinalizeEventMessage(ports::Event::Cast<ports::UserMessageEvent>(&event));
    }
...
ports::ScopedEvent DeserializeEventMessage(const ports::NodeName& from_node, Channel::MessagePtr channel_message)
{
    void* data;
    size_t size;
    ...
    auto message = UserMessageImpl::CreateFromChannelMessage(
        message_event.get(), std::move(channel_message), static_cast<uint8_t*>(data) + event_size, size - event_size);
    if (!message)
        return nullptr;
...
// at that time.
class ThreadDestructionObserver : public base::CurrentThread::DestructionObserver {
public:
    static void Create(scoped_refptr<base::SingleThreadTaskRunner> task_runner, base::OnceClosure callback)
    {
        if (task_runner->RunsTasksInCurrentSequence()) {
            // Owns itself.
            new ThreadDestructionObserver(std::move(callback));
        } else {
            task_runner->PostTask(FROM_HERE, base::BindOnce(&Create, task_runner, std::move(callback)));
        }
    }

    ThreadDestructionObserver(const ThreadDestructionObserver&) = delete;
    ThreadDestructionObserver& operator=(const ThreadDestructionObserver&) = delete;
...
ConnectionParams CreateSyncNodeConnectionParams(const base::Process& target_process, ConnectionParams connection_params,
    const ProcessErrorCallback& process_error_callback, Channel::HandlePolicy& handle_policy)
{
    ...
    BrokerHost* broker_host
        = new BrokerHost(target_process.IsValid() ? target_process.Duplicate() : base::Process(), std::move(connection_params), process_error_callback);
    ...
    if (!target_process.IsValid()) {
        ...
        node_connection_params = ConnectionParams(named_channel.TakeServerEndpoint());
    ...
    node_connection_params.set_is_untrusted_process(is_untrusted_process);
    bool channel_ok = broker_host->SendChannel(node_channel.TakeRemoteEndpoint().TakePlatformHandle());
    DCHECK(channel_ok);
...
void NodeController::SetIOTaskRunner(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
{
    io_task_runner_ = task_runner;
    ThreadDestructionObserver::Create(io_task_runner_, base::BindOnce(&NodeController::DropAllPeers, base::Unretained(this)));
}

void NodeController::SendBrokerClientInvitation(base::Process target_process, ConnectionParams connection_params,
    const std::vector<std::pair<std::string, ports::PortRef>>& attached_ports, const ProcessErrorCallback& process_error_callback)
{
    ...
    io_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&NodeController::SendBrokerClientInvitationOnIOThread, base::Unretained(this), std::move(target_process), std::move(connection_params),
            temporary_node_name, process_error_callback));
}

void NodeController::AcceptBrokerClientInvitation(ConnectionParams connection_params)
{
    ...
    if (!connection_params.is_async()) {
        ...
        broker_ = std::make_unique<Broker>(connection_params.TakeEndpoint().TakePlatformHandle(),
        ...
        broker_ = std::make_unique<Broker>(channel.TakeLocalEndpoint().TakePlatformHandle(),
    ...
    io_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(
            &NodeController::AcceptBrokerClientInvitationOnIOThread, base::Unretained(this), std::move(connection_params), std::move(broker_host_handle)));
}

void NodeController::ConnectIsolated(ConnectionParams connection_params, const ports::PortRef& port, base::StringPiece connection_name)
{
    io_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&NodeController::ConnectIsolatedOnIOThread, base::Unretained(this), std::move(connection_params), port, std::string(connection_name)));
}

void NodeController::SetPortObserver(const ports::PortRef& port, scoped_refptr<PortObserver> observer)
{
...
int NodeController::SendUserMessage(const ports::PortRef& port, std::unique_ptr<ports::UserMessageEvent> message)
{
    return node_->SendUserMessage(port, std::move(message));
}

void NodeController::MergePortIntoInviter(const std::string& name, const ports::PortRef& port)
{
    ...
    if (reject_merge) {
        ...
        DVLOG(2) << "Rejecting port merge for name " << name << " due to closed inviter channel.";
        return;
    }
...
int NodeController::MergeLocalPorts(const ports::PortRef& port0, const ports::PortRef& port1)
{
    return node_->MergeLocalPorts(port0, port1);
}

base::WritableSharedMemoryRegion NodeController::CreateSharedBuffer(size_t num_bytes)
{
#if !BUILDFLAG(IS_APPLE) && !BUILDFLAG(IS_NACL) && !BUILDFLAG(IS_FUCHSIA)
...
void NodeController::NotifyBadMessageFrom(const ports::NodeName& source_node, const std::string& error)
{
...
void NodeController::ForceDisconnectProcessForTesting(base::ProcessId process_id)
{
    io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&NodeController::ForceDisconnectProcessForTestingOnIOThread, base::Unretained(this), process_id));
}
...
// static
void NodeController::DeserializeRawBytesAsEventForFuzzer(base::span<const unsigned char> data)
{
    void* payload;
...
// static
void NodeController::DeserializeMessageAsEventForFuzzer(Channel::MessagePtr message)
{
    DeserializeEventMessage(ports::NodeName(), std::move(message));
}

void NodeController::SendBrokerClientInvitationOnIOThread(
    base::Process target_process, ConnectionParams connection_params, ports::NodeName temporary_node_name, const ProcessErrorCallback& process_error_callback)
{
    ...
    if (connection_params.is_async()) {
        ...
        node_connection_params = CreateSyncNodeConnectionParams(target_process, std::move(connection_params), process_error_callback, handle_policy);
    }

    scoped_refptr<NodeChannel> channel = NodeChannel::Create(this, std::move(node_connection_params), handle_policy, io_task_runner_, process_error_callback);
#else // !BUILDFLAG(IS_APPLE) && !BUILDFLAG(IS_NACL) && !BUILDFLAG(IS_FUCHSIA)
    scoped_refptr<NodeChannel> channel
        = NodeChannel::Create(this, std::move(connection_params), Channel::HandlePolicy::kAcceptHandles, io_task_runner_, process_error_callback);
#endif // !BUILDFLAG(IS_APPLE) && !BUILDFLAG(IS_NACL) && !BUILDFLAG(IS_FUCHSIA)
...
void NodeController::AcceptBrokerClientInvitationOnIOThread(ConnectionParams connection_params, absl::optional<PlatformHandle> broker_host_handle)
{
    DCHECK(io_task_runner_->RunsTasksInCurrentSequence());

    {
        ...
        bootstrap_inviter_channel_
            = NodeChannel::Create(this, std::move(connection_params), Channel::HandlePolicy::kAcceptHandles, io_task_runner_, ProcessErrorCallback());
...
void NodeController::ConnectIsolatedOnIOThread(ConnectionParams connection_params, ports::PortRef port, const std::string& connection_name)
{
    DCHECK(io_task_runner_->RunsTasksInCurrentSequence());
    ...
    scoped_refptr<NodeChannel> channel = NodeChannel::Create(this, std::move(connection_params), Channel::HandlePolicy::kAcceptHandles, io_task_runner_, {});

    RequestContext request_context;
    ports::NodeName token;
    GenerateRandomName(&token);
    pending_isolated_connections_.emplace(token, IsolatedConnection { channel, port, connection_name });
...
scoped_refptr<NodeChannel> NodeController::GetPeerChannel(const ports::NodeName& name)
{
...
void NodeController::AddPeer(const ports::NodeName& name, scoped_refptr<NodeChannel> channel, bool start_channel, bool allow_name_reuse)
{
    DCHECK(io_task_runner_->RunsTasksInCurrentSequence());
...
void NodeController::DropPeer(const ports::NodeName& node_name, NodeChannel* channel)
{
    DCHECK(io_task_runner_->RunsTasksInCurrentSequence());
    ...
    {
        base::AutoLock lock(inviter_lock_);
        is_inviter = (name == inviter_name_ || (channel && channel == bootstrap_inviter_channel_));
    }
...
void NodeController::SendPeerEvent(const ports::NodeName& name, ports::ScopedEvent event)
{
...
void NodeController::ForwardEvent(const ports::NodeName& node, ports::ScopedEvent event)
{
    DCHECK(event);
    if (node == name_)
...
void NodeController::OnAcceptInvitee(const ports::NodeName& from_node, const ports::NodeName& inviter_name, const ports::NodeName& token)
{
    DCHECK(io_task_runner_->RunsTasksInCurrentSequence());
    ...
    DVLOG(1) << "Broker client " << name_ << " accepting invitation from " << inviter_name;
}

void NodeController::OnAcceptInvitation(const ports::NodeName& from_node, const ports::NodeName& token, const ports::NodeName& invitee_name)
{
    DCHECK(io_task_runner_->RunsTasksInCurrentSequence());

    auto it = pending_invitations_.find(from_node);
    if (it == pending_invitations_.end() || token != from_node) {
        DLOG(ERROR) << "Received unexpected AcceptInvitation message from " << from_node;
        DropPeer(from_node, nullptr);
        return;
    }

    {
        ...
        if (reserved_ports_it != reserved_ports_.end()) {
            ...
            // the real node name.
            auto result = reserved_ports_.emplace(invitee_name, std::move(reserved_ports_it->second));
            DCHECK(result.second);
    ...
    if (broker) {
        ...
        if (!inviter) {
            ...
            channel->AcceptBrokerClient(name_, PlatformHandle(), channel->LocalCapabilities());
        } else {
...
void NodeController::OnAddBrokerClient(const ports::NodeName& from_node, const ports::NodeName& client_name, base::ProcessHandle process_handle)
{
    ...
    DVLOG(1) << "Broker " << name_ << " accepting client " << client_name << " from peer " << from_node;

    sender->BrokerClientAdded(client_name, broker_channel.TakeRemoteEndpoint().TakePlatformHandle());
}

void NodeController::OnBrokerClientAdded(const ports::NodeName& from_node, const ports::NodeName& client_name, PlatformHandle broker_channel)
{
    ...
    client->AcceptBrokerClient(from_node, std::move(broker_channel), GetBrokerChannel()->RemoteCapabilities());
}

void NodeController::OnAcceptBrokerClient(
    const ports::NodeName& from_node, const ports::NodeName& broker_name, PlatformHandle broker_channel, const uint64_t broker_capabilities)
{
    if (GetConfiguration().is_broker_process) {
    ...
    if (inviter_name != from_node || !inviter || broker_name == ports::kInvalidNodeName) {
    ...
    std::unordered_map<ports::NodeName, OutgoingMessageQueue> pending_relay_messages;
    {
        base::AutoLock lock(broker_lock_);
    ...
    if (broker_name == inviter_name) {
        ...
        broker = NodeChannel::Create(this, ConnectionParams(PlatformChannelEndpoint(std::move(broker_channel))), Channel::HandlePolicy::kAcceptHandles,
    ...
    while (!pending_broker_clients.empty()) {
        ...
        if (it != peers_.end()) {
            broker->AddBrokerClient(invitee_name, it->second->CloneRemoteProcessHandle());
        }
...
#endif
    if (inviter->HasLocalCapability(kNodeCapabilitySupportsUpgrade) && inviter->HasRemoteCapability(kNodeCapabilitySupportsUpgrade)) {
...
void NodeController::OnEventMessage(const ports::NodeName& from_node, Channel::MessagePtr channel_message)
{
    DCHECK(io_task_runner_->RunsTasksInCurrentSequence());
...
void NodeController::OnRequestPortMerge(const ports::NodeName& from_node, const ports::PortName& connector_port_name, const std::string& name)
{
    DCHECK(io_task_runner_->RunsTasksInCurrentSequence());

    DVLOG(2) << "Node " << name_ << " received RequestPortMerge for name " << name << " and port " << connector_port_name << "@" << from_node;
    ...
    ports::PortRef local_port;
    {
        ...
        if (port_it == port_map.end()) {
            DVLOG(1) << "Ignoring request to connect to port for unknown name " << name << " from node " << from_node;
            return;
        }
...
void NodeController::OnRequestIntroduction(const ports::NodeName& from_node, const ports::NodeName& name)
{
    DCHECK(io_task_runner_->RunsTasksInCurrentSequence());
    ...
    if (from_node == name || name == ports::kInvalidNodeName || !requestor) {
        DLOG(ERROR) << "Rejecting invalid OnRequestIntroduction message from " << from_node;
        DropPeer(from_node, nullptr);
        return;
    }
    ...
    if (!new_friend) {
        ...
        requestor->Introduce(name, new_channel.TakeLocalEndpoint().TakePlatformHandle(), new_friend->RemoteCapabilities());
        new_friend->Introduce(from_node, new_channel.TakeRemoteEndpoint().TakePlatformHandle(), requestor->RemoteCapabilities());
    }
}

void NodeController::OnIntroduce(
    const ports::NodeName& from_node, const ports::NodeName& name, PlatformHandle channel_handle, const uint64_t remote_capabilities)
{
    DCHECK(io_task_runner_->RunsTasksInCurrentSequence());
    ...
    scoped_refptr<NodeChannel> channel = NodeChannel::Create(
        this, ConnectionParams(PlatformChannelEndpoint(std::move(channel_handle))), kPeerHandlePolicy, io_task_runner_, ProcessErrorCallback());
    ...
    if (channel->HasLocalCapability(kNodeCapabilitySupportsUpgrade) && channel->HasRemoteCapability(kNodeCapabilitySupportsUpgrade)) {
        channel->OfferChannelUpgrade();
    }
}

void NodeController::OnBroadcast(const ports::NodeName& from_node, Channel::MessagePtr message)
{
    ...
    if (!event) {
        // We silently ignore unparseable events, as they may come from a process
        // running a newer version of Mojo.
        DVLOG(1) << "Ignoring request to broadcast invalid or unknown event from " << from_node;
        return;
    }
    ...
    for (auto& iter : peers_) {
        ...
        if (!clone) {
            DVLOG(1) << "Ignoring request to broadcast invalid event from " << from_node << " [type=" << static_cast<uint32_t>(event->type()) << "]";
            return;
        }
...
#if BUILDFLAG(IS_WIN)
void NodeController::OnRelayEventMessage(
    const ports::NodeName& from_node, base::ProcessHandle from_process, const ports::NodeName& destination, Channel::MessagePtr message)
{
...
void NodeController::OnEventMessageFromRelay(const ports::NodeName& from_node, const ports::NodeName& source_node, Channel::MessagePtr message)
{
...
void NodeController::OnAcceptPeer(
    const ports::NodeName& from_node, const ports::NodeName& token, const ports::NodeName& peer_name, const ports::PortName& port_name)
{
    DCHECK(io_task_runner_->RunsTasksInCurrentSequence());
    ...
    if (name_ != peer_name) {
        ...
        AddPeer(peer_name, channel, false /* start_channel */, true /* allow_name_reuse */);
...
void NodeController::OnChannelError(const ports::NodeName& from_node, NodeChannel* channel)
{
    if (io_task_runner_->RunsTasksInCurrentSequence()) {
        RequestContext request_context(RequestContext::Source::SYSTEM);
        DropPeer(from_node, channel);
    } else {
        io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&NodeController::OnChannelError, base::Unretained(this), from_node, base::RetainedRef(channel)));
    }
}
...
void NodeController::AttemptShutdownIfRequested()
{
    ...
    base::OnceClosure callback;
    {
        ...
        if (shutdown_callback_.is_null())
            return;
        if (!node_->CanShutdownCleanly(ports::Node::ShutdownPolicy::ALLOW_LOCAL_PORTS)) {
...
void NodeController::ForceDisconnectProcessForTestingOnIOThread(base::ProcessId process_id)
{
...
NodeController::IsolatedConnection::IsolatedConnection(const IsolatedConnection& other) = default;

NodeController::IsolatedConnection::IsolatedConnection(IsolatedConnection&& other) = default;

NodeController::IsolatedConnection::IsolatedConnection(scoped_refptr<NodeChannel> channel, const ports::PortRef& local_port, base::StringPiece name)
...
NodeController::IsolatedConnection& NodeController::IsolatedConnection::operator=(const IsolatedConnection& other) = default;

NodeController::IsolatedConnection& NodeController::IsolatedConnection::operator=(IsolatedConnection&& other) = default;
