...
public:
    ...
    void OnChannelMessage(const void* payload, size_t payload_size, std::vector<mojo::PlatformHandle> handles) override
    {
    }
...
DEFINE_BINARY_PROTO_FUZZER(mojo_fuzzer::ChannelMac proto)
{
    ...
    mach_port_t send_port = platform_channel.local_endpoint().platform_handle().GetMachSendRight().get();
    ...
    } else if (proto.endpoint_type() == mojo_fuzzer::ChannelMac_EndpointType_REMOTE) {
    ...
    auto channel = mojo::core::Channel::Create(
        &delegate, mojo::core::ConnectionParams(std::move(fuzzed_endpoint)), mojo::core::Channel::HandlePolicy::kAcceptHandles, environment.io_task_runner());
    channel->Start();
    ...
    if (!proto.fuzz_handshake()) {
        peer_channel = mojo::core::Channel::Create(&delegate, mojo::core::ConnectionParams(std::move(other_endpoint)),
            mojo::core::Channel::HandlePolicy::kAcceptHandles, environment.io_task_runner());
        peer_channel->Start();
    }
    ...
    for (auto& message : *proto.mutable_messages()) {
        if (message.HasExtension(mojo_fuzzer::MojoMessage::mojo_message)) {
            ...
            const auto& mojo_message = message.GetExtension(mojo_fuzzer::MojoMessage::mojo_message);

            // If the fuzz data do not specify an explicit length, just use the byte
            // length.
            uint64_t data_length = mojo_message.has_data_length() ? mojo_message.data_length() : mojo_message.data().size();
            std::string data;
            data.append(reinterpret_cast<const char*>(&data_length), sizeof(data_length));
        ...
        std::move(result.message.ports.begin(), result.message.ports.end(), std::back_inserter(ports_to_destroy));
    }
