...
WatcherSet::WatcherSet(Dispatcher* owner)
    : owner_(owner)
{
}
...
MojoResult WatcherSet::Add(const scoped_refptr<WatcherDispatcher>& watcher, uintptr_t context, const HandleSignalsState& current_state)
{
    auto it = watchers_.find(watcher.get());
    if (it == watchers_.end()) {
        auto result = watchers_.insert(std::make_pair(watcher.get(), Entry { watcher }));
        it = result.first;
    }
    ...
    if (last_known_state_.has_value() && !current_state.equals(last_known_state_.value())) {
