...
bool UnwrapFdioHandle(PlatformHandleInTransit handle, PlatformHandleInTransit* out_handle, Channel::Message::HandleInfoEntry* info)
{
    ...
    zx::handle result;
    zx_status_t status = fdio_fd_transfer_or_clone(handle.TakeHandle().ReleaseFD(), result.reset_and_get_address());
    if (status != ZX_OK) {
...
PlatformHandle WrapFdioHandle(zx::handle handle, Channel::Message::HandleInfoEntry info)
{
    if (!info.is_file_descriptor)
        return PlatformHandle(std::move(handle));

    base::ScopedFD out_fd;
    zx_status_t status = fdio_fd_create(handle.release(), base::ScopedFD::Receiver(out_fd).get());
    if (status != ZX_OK) {
...
public:
    ...
    std::vector<PlatformHandleInTransit> TakeHandles(bool unwrap_fds)
    {
        ...
        auto* handles_info = reinterpret_cast<Channel::Message::HandleInfoEntry*>(message_->mutable_extra_header());
        ...
        for (size_t i = 0; i < handles_.size(); i++) {
            if (!UnwrapFdioHandle(std::move(handles_[i]), &handles_[i], &handles_info[i])) {
...
class ChannelFuchsia : public Channel, public base::CurrentThread::DestructionObserver, public base::MessagePumpForIO::ZxHandleWatcher {
public:
    ChannelFuchsia(
    ...
    void Start() override
    {
        if (io_task_runner_->RunsTasksInCurrentSequence()) {
            ...
            io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&ChannelFuchsia::StartOnIOThread, this));
        }
    }
    ...
    void ShutDownImpl() override
    {
        ...
        io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&ChannelFuchsia::ShutDownOnIOThread, this));
    }
    ...
    void Write(MessagePtr message) override
    {
        ...
        if (write_error) {
            ...
            io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&ChannelFuchsia::OnWriteError, this, Error::kDisconnected));
        }
    }
        ...
        std::vector<PlatformHandle>* handles, bool* deferred) override
    {
        ...
        if (!extra_header)
            return false;
        const auto* handles_info = reinterpret_cast<const Channel::Message::HandleInfoEntry*>(extra_header);
        ...
        for (size_t i = 0; i < num_handles; ++i) {
            handles->emplace_back(WrapFdioHandle(std::move(incoming_handles_.front()), handles_info[i]));
...
private:
    ...
    void StartOnIOThread()
    {
        ...
        read_watch_ = std::make_unique<base::MessagePumpForIO::ZxHandleWatchController>(FROM_HERE);
        base::CurrentIOThread::Get()->WatchZxHandle(
            handle_.get(), true /* persistent */, ZX_CHANNEL_READABLE | ZX_CHANNEL_PEER_CLOSED, read_watch_.get(), this);
    }
    ...
    void OnZxHandleSignalled(zx_handle_t handle, zx_signals_t signals) override
    {
        ...
        do {
            ...
            zx_status_t read_result = handle_.read(0, buffer, handles, buffer_capacity, std::size(handles), &bytes_read, &handles_read);
            if (read_result == ZX_OK) {
                ...
                ZX_DLOG_IF(ERROR, read_result != ZX_ERR_PEER_CLOSED, read_result) << "zx_channel_read";
                read_error = true;
                break;
            }
    ...
    bool WriteNoLock(MessageView message_view)
    {
        ...
        do {
            ...
            std::vector<PlatformHandleInTransit> outgoing_handles = message_view.TakeHandles(/*unwrap_fds=*/!is_for_ipcz());
            zx_handle_t handles[ZX_CHANNEL_MAX_MSG_HANDLES] = {};
            ...
            write_bytes = std::min(message_view.data_num_bytes(), static_cast<size_t>(ZX_CHANNEL_MAX_MSG_BYTES));
            zx_status_t result = handle_.write(0, message_view.data(), write_bytes, handles, handles_count);
            ...
            if (result != ZX_OK) {
                ...
                ZX_DLOG_IF(ERROR, result != ZX_ERR_PEER_CLOSED, result) << "WriteNoLock(zx_channel_write)";
                return false;
            }
...
scoped_refptr<Channel> Channel::Create(
    Delegate* delegate, ConnectionParams connection_params, HandlePolicy handle_policy, scoped_refptr<base::SingleThreadTaskRunner> io_task_runner)
{
    return new ChannelFuchsia(delegate, std::move(connection_params), handle_policy, std::move(io_task_runner));
}
