...
#if BUILDFLAG(IS_WIN)
HANDLE TransferHandle(HANDLE handle, base::ProcessHandle from_process, base::ProcessHandle to_process, PlatformHandleInTransit::TransferTargetTrustLevel trust)
{
    ...
    HANDLE out_handle;
    BOOL result = ::DuplicateHandle(from_process, handle, to_process, &out_handle, 0, FALSE, DUPLICATE_SAME_ACCESS | DUPLICATE_CLOSE_SOURCE);
    if (result) {
        return out_handle;
    }
    ...
    // largely unavoidable.
    if (error == ERROR_ACCESS_DENIED && base::win::GetLastNtStatus() == STATUS_PROCESS_IS_TERMINATING) {
        DVLOG(1) << "DuplicateHandle from " << from_process << " to " << to_process << " for handle " << handle << " failed due to process termination";
    ...
    PLOG(FATAL) << "DuplicateHandle failed from " << from_process << " to " << to_process << " for handle " << handle;
    return INVALID_HANDLE_VALUE;
}
...
void CloseHandleInProcess(HANDLE handle, const base::Process& process)
{
    ...
    BOOL result = ::DuplicateHandle(process.Handle(), handle, nullptr, &handle, 0, FALSE, DUPLICATE_CLOSE_SOURCE);
    if (!result) {
...
PlatformHandleInTransit::PlatformHandleInTransit(PlatformHandleInTransit&& other)
{
    *this = std::move(other);
}
...
PlatformHandleInTransit& PlatformHandleInTransit::operator=(PlatformHandleInTransit&& other)
{
#if BUILDFLAG(IS_WIN)
    if (owning_process_.IsValid()) {
...
bool PlatformHandleInTransit::TransferToProcess(base::Process target_process, TransferTargetTrustLevel trust)
{
    DCHECK(target_process.IsValid());
    DCHECK(!owning_process_.IsValid());
    DCHECK(handle_.is_valid());
#if BUILDFLAG(IS_WIN)
    remote_handle_ = TransferHandle(handle_.ReleaseHandle(), base::GetCurrentProcessHandle(), target_process.Handle(), trust);
...
// static
PlatformHandle PlatformHandleInTransit::TakeIncomingRemoteHandle(HANDLE handle, base::ProcessHandle owning_process)
{
    return PlatformHandle(base::win::ScopedHandle(TransferHandle(handle, owning_process, base::GetCurrentProcessHandle(), kTrustedTarget)));
}
#endif
