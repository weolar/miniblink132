...
public:
    ...
    bool is_valid() const
    {
        return handle_ != IPCZ_INVALID_HANDLE;
    }
    IpczHandle get() const
    {
        return handle_;
    }
...
static_assert(sizeof(IpczHandle) == sizeof(ScopedIpczHandle), "ScopedIpczHandle must be the same size as IpczHandle.");
