...
const uint32_t kSizeOfOptions = static_cast<uint32_t>(sizeof(MojoCreateDataPipeOptions));
...
public:
    ...
    MojoResult ReadEmptyMessageWithHandles(MojoHandle pipe, MojoHandle* out_handles, uint32_t num_handles)
    {
        std::vector<uint8_t> bytes;
        std::vector<ScopedHandle> handles;
        MojoResult rv = ReadMessageRaw(MessagePipeHandle(pipe), &bytes, &handles, MOJO_READ_MESSAGE_FLAG_NONE);
    ...
    MojoResult WriteData(const void* elements, uint32_t* num_bytes, bool all_or_none = false)
    {
        MojoWriteDataOptions options;
        options.struct_size = sizeof(options);
        options.flags = all_or_none ? MOJO_WRITE_DATA_FLAG_ALL_OR_NONE : MOJO_WRITE_DATA_FLAG_NONE;
        return MojoWriteData(producer_, elements, num_bytes, &options);
    }

    MojoResult ReadData(void* elements, uint32_t* num_bytes, bool all_or_none = false, bool peek = false)
    {
...
TEST_F(DataPipeTest, Basic)
{
    ...
    ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(consumer_, MOJO_HANDLE_SIGNAL_READABLE, &state));
    EXPECT_EQ(MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_NEW_DATA_READABLE, state.satisfied_signals);
...
TEST_F(DataPipeTest, CreateAndMaybeTransfer)
{
    ...
    for (size_t i = 0; i < std::size(test_options); i++) {
        ...
        ASSERT_EQ(MOJO_RESULT_OK, MojoCreateDataPipe(options, &producer_handle, &consumer_handle));
...
TEST_F(DataPipeTest, SimpleReadWrite)
{
    ...
    // Wait.
    ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(consumer_, MOJO_HANDLE_SIGNAL_READABLE, &hss));
    EXPECT_EQ(MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_NEW_DATA_READABLE, hss.satisfied_signals);
    EXPECT_EQ(MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED | MOJO_HANDLE_SIGNAL_NEW_DATA_READABLE | MOJO_HANDLE_SIGNAL_PEER_REMOTE,
    ...
    elements[1] = -1;
    num_bytes = static_cast<uint32_t>(2u * sizeof(elements[0]));
    ASSERT_EQ(MOJO_RESULT_OUT_OF_RANGE, ReadData(elements, &num_bytes, true, false));
...
TEST_F(DataPipeTest, BasicProducerWaiting)
{
    ...
    ASSERT_EQ(MOJO_HANDLE_SIGNAL_WRITABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED | MOJO_HANDLE_SIGNAL_PEER_REMOTE, hss.satisfiable_signals);
    ...
    ASSERT_EQ(MOJO_RESULT_OK, EndWriteData(static_cast<uint32_t>(1u * sizeof(elements[0]))));
    ...
    ASSERT_EQ(MOJO_RESULT_OK, EndReadData(static_cast<uint32_t>(1u * sizeof(elements[0]))));
    ...
    ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(producer_, MOJO_HANDLE_SIGNAL_PEER_CLOSED, &hss));
...
TEST_F(DataPipeTest, PeerClosedConsumerWaiting)
{
    ...
    // It should be signaled.
    hss = MojoHandleSignalsState();
    ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(consumer_, MOJO_HANDLE_SIGNAL_PEER_CLOSED, &hss));
    ASSERT_EQ(MOJO_HANDLE_SIGNAL_PEER_CLOSED, hss.satisfied_signals);
    ASSERT_EQ(MOJO_HANDLE_SIGNAL_PEER_CLOSED, hss.satisfiable_signals);
}
...
TEST_F(DataPipeTest, BasicConsumerWaiting)
{
    ...
    // Never writable.
    hss = MojoHandleSignalsState();
    ASSERT_EQ(MOJO_RESULT_FAILED_PRECONDITION, WaitForSignals(consumer_, MOJO_HANDLE_SIGNAL_WRITABLE, &hss));
    ...
    hss = MojoHandleSignalsState();
    ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(consumer_, MOJO_HANDLE_SIGNAL_READABLE, &hss));
    EXPECT_TRUE(hss.satisfied_signals & (MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_NEW_DATA_READABLE));
    EXPECT_EQ(MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED | MOJO_HANDLE_SIGNAL_NEW_DATA_READABLE, hss.satisfiable_signals);

    // Wait for the peer closed signal.
    hss = MojoHandleSignalsState();
    ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(consumer_, MOJO_HANDLE_SIGNAL_PEER_CLOSED, &hss));
    EXPECT_EQ(MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_NEW_DATA_READABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED, hss.satisfied_signals);
    EXPECT_EQ(MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED | MOJO_HANDLE_SIGNAL_NEW_DATA_READABLE, hss.satisfiable_signals);
    ...
    hss = MojoHandleSignalsState();
    ASSERT_EQ(MOJO_RESULT_FAILED_PRECONDITION, WaitForSignals(consumer_, MOJO_HANDLE_SIGNAL_READABLE, &hss));
    ASSERT_EQ(MOJO_HANDLE_SIGNAL_PEER_CLOSED, hss.satisfied_signals);
    ASSERT_EQ(MOJO_HANDLE_SIGNAL_PEER_CLOSED, hss.satisfiable_signals);
}
...
TEST_F(DataPipeTest, ConsumerNewDataReadable)
{
    ...
    EXPECT_EQ(MOJO_RESULT_OK, WaitForSignals(consumer_, MOJO_HANDLE_SIGNAL_READABLE));
    EXPECT_TRUE(GetSignalsState(consumer_).satisfied_signals & MOJO_HANDLE_SIGNAL_NEW_DATA_READABLE);
    ...
    EXPECT_EQ(MOJO_RESULT_OUT_OF_RANGE, MojoReadData(consumer_, &read_options, read_elements, &num_read_bytes));

    // The consumer should still appear to be readable but not with new data.
    EXPECT_TRUE(GetSignalsState(consumer_).satisfied_signals & MOJO_HANDLE_SIGNAL_READABLE);
    EXPECT_FALSE(GetSignalsState(consumer_).satisfied_signals & MOJO_HANDLE_SIGNAL_NEW_DATA_READABLE);
    ...
    EXPECT_EQ(MOJO_RESULT_OK, MojoReadData(consumer_, &read_options, read_elements, &num_read_bytes));

    // And now the consumer is unreadable.
    EXPECT_FALSE(GetSignalsState(consumer_).satisfied_signals & MOJO_HANDLE_SIGNAL_READABLE);
    EXPECT_FALSE(GetSignalsState(consumer_).satisfied_signals & MOJO_HANDLE_SIGNAL_NEW_DATA_READABLE);
}
...
TEST_F(DataPipeTest, AllOrNone)
{
    ...
    ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(consumer_, MOJO_HANDLE_SIGNAL_READABLE, &hss));
    EXPECT_EQ(MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_NEW_DATA_READABLE, hss.satisfied_signals);
    EXPECT_EQ(MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_NEW_DATA_READABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED | MOJO_HANDLE_SIGNAL_PEER_REMOTE,
        hss.satisfiable_signals);
    ...
    memset(buffer, 0xab, sizeof(buffer));
    ASSERT_EQ(MOJO_RESULT_FAILED_PRECONDITION, ReadData(buffer, &num_bytes, true));
    memset(expected_buffer, 0xab, sizeof(expected_buffer));
    ASSERT_EQ(0, memcmp(buffer, expected_buffer, sizeof(buffer)));
...
TEST_F(DataPipeTest, WrapAround)
{
    ...
    while (total_num_bytes < 90) {
        // Wait to write.
        ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(producer_, MOJO_HANDLE_SIGNAL_WRITABLE, &hss));
        ASSERT_EQ(hss.satisfied_signals, MOJO_HANDLE_SIGNAL_WRITABLE);
        ASSERT_EQ(hss.satisfiable_signals, MOJO_HANDLE_SIGNAL_WRITABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED | MOJO_HANDLE_SIGNAL_PEER_REMOTE);

        // Write as much as we can.
        num_bytes = 100;
        ASSERT_EQ(MOJO_RESULT_OK, WriteData(&test_data[20 + total_num_bytes], &num_bytes, false));
    ...
    num_bytes = static_cast<uint32_t>(std::size(read_buffer) * sizeof(read_buffer[0]));
...
TEST_F(DataPipeTest, TwoPhaseWriteReadCloseConsumer)
{
    ...
    num_bytes = 0u;
    ASSERT_EQ(MOJO_RESULT_FAILED_PRECONDITION, BeginWriteData(&write_buffer_ptr, &num_bytes));
}
...
TEST_F(DataPipeTest, WriteCloseProducerReadNoData)
{
    ...
    hss = MojoHandleSignalsState();
    ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(consumer_, MOJO_HANDLE_SIGNAL_PEER_CLOSED, &hss));
    EXPECT_EQ(MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED | MOJO_HANDLE_SIGNAL_NEW_DATA_READABLE, hss.satisfied_signals);
    EXPECT_EQ(MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED | MOJO_HANDLE_SIGNAL_NEW_DATA_READABLE, hss.satisfiable_signals);
    ...
    const void* read_buffer_ptr = nullptr;
    num_bytes = 0u;
    ASSERT_EQ(MOJO_RESULT_FAILED_PRECONDITION, BeginReadData(&read_buffer_ptr, &num_bytes));
...
TEST_F(DataPipeTest, TwoPhaseMoreInvalidArguments)
{
    ...
    ASSERT_EQ(MOJO_RESULT_FAILED_PRECONDITION, EndWriteData(1u * sizeof(int32_t)));
    ...
    void* write_ptr = nullptr;
    ASSERT_EQ(MOJO_RESULT_OK, BeginWriteData(&write_ptr, &num_bytes));
    ASSERT_EQ(MOJO_RESULT_INVALID_ARGUMENT, EndWriteData(num_bytes + static_cast<uint32_t>(sizeof(int32_t))));
    ...
    const void* read_ptr = nullptr;
    ASSERT_EQ(MOJO_RESULT_OK, BeginReadData(&read_ptr, &num_bytes));
    ASSERT_EQ(MOJO_RESULT_INVALID_ARGUMENT, EndReadData(num_bytes + static_cast<uint32_t>(sizeof(int32_t))));
...
TEST_F(DataPipeTest, SendProducer)
{
    ...
    ASSERT_EQ(MOJO_RESULT_OK, WriteMessageRaw(MessagePipeHandle(pipe0), nullptr, 0, &producer_, 1, MOJO_WRITE_MESSAGE_FLAG_NONE));
    producer_ = MOJO_HANDLE_INVALID;
    ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(pipe1, MOJO_HANDLE_SIGNAL_READABLE, &hss));
...
TEST_F(DataPipeTest, ConsumerWithClosedProducerSent)
{
    ...
    MojoHandleSignalsState state;
    ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(consumer_, MOJO_HANDLE_SIGNAL_PEER_CLOSED, &state));
    EXPECT_EQ(MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED | MOJO_HANDLE_SIGNAL_NEW_DATA_READABLE, state.satisfied_signals);
    ASSERT_EQ(MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED | MOJO_HANDLE_SIGNAL_NEW_DATA_READABLE, state.satisfiable_signals);

    // Now send the consumer over a MP so that it's serialized.
    MojoHandle pipe0, pipe1;
    ASSERT_EQ(MOJO_RESULT_OK, MojoCreateMessagePipe(nullptr, &pipe0, &pipe1));

    ASSERT_EQ(MOJO_RESULT_OK, WriteMessageRaw(MessagePipeHandle(pipe0), nullptr, 0, &consumer_, 1, MOJO_WRITE_MESSAGE_FLAG_NONE));
    consumer_ = MOJO_HANDLE_INVALID;
    ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(pipe1, MOJO_HANDLE_SIGNAL_READABLE, &state));
    ASSERT_EQ(MOJO_RESULT_OK, ReadEmptyMessageWithHandles(pipe1, &consumer_, 1));

    ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(consumer_, MOJO_HANDLE_SIGNAL_PEER_CLOSED, &state));
    EXPECT_EQ(MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED | MOJO_HANDLE_SIGNAL_NEW_DATA_READABLE, state.satisfied_signals);
    EXPECT_EQ(MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED | MOJO_HANDLE_SIGNAL_NEW_DATA_READABLE, state.satisfiable_signals);
...
bool WriteAllData(MojoHandle producer, const void* elements, uint32_t num_bytes)
{
    for (size_t i = 0; i < kMaxPoll; i++) {
        // Write as much data as we can.
        uint32_t write_bytes = num_bytes;
        MojoResult result = MojoWriteData(producer, elements, &write_bytes, nullptr);
        ...
        EXPECT_EQ(MOJO_RESULT_OK, test::MojoTestBase::WaitForSignals(producer, MOJO_HANDLE_SIGNAL_WRITABLE, &hss));
        EXPECT_TRUE(hss.satisfied_signals & MOJO_HANDLE_SIGNAL_WRITABLE);
        EXPECT_EQ(MOJO_HANDLE_SIGNAL_WRITABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED | MOJO_HANDLE_SIGNAL_PEER_REMOTE, hss.satisfiable_signals);
    }
...
bool ReadAllData(MojoHandle consumer, void* elements, uint32_t num_bytes, bool expect_empty)
{
    for (size_t i = 0; i < kMaxPoll; i++) {
        ...
        MojoHandleSignalsState hss = MojoHandleSignalsState();
        EXPECT_EQ(MOJO_RESULT_OK, test::MojoTestBase::WaitForSignals(consumer, MOJO_HANDLE_SIGNAL_READABLE, &hss));
...
{
    const uint32_t kTestDataSize = static_cast<uint32_t>(sizeof(kMultiprocessTestData));
    const MojoCreateDataPipeOptions options = {
        kSizeOfOptions, // |struct_size|.
        MOJO_CREATE_DATA_PIPE_FLAG_NONE, // |flags|.
        1, // |element_num_bytes|.
    ...
    RunTestClient("MultiprocessClient", [&](MojoHandle server_mp) {
        ...
        uint32_t num_bytes = kTestDataSize;
        ASSERT_EQ(MOJO_RESULT_OK, WriteData(kMultiprocessTestData, &num_bytes, true));
        ASSERT_EQ(kTestDataSize, num_bytes);

        // Send child process the data pipe.
        ASSERT_EQ(MOJO_RESULT_OK, WriteMessageRaw(MessagePipeHandle(server_mp), nullptr, 0, &consumer_, 1, MOJO_WRITE_MESSAGE_FLAG_NONE));
        ...
        // Swap ends.
        ASSERT_EQ(MOJO_RESULT_OK, WriteMessageRaw(MessagePipeHandle(server_mp), nullptr, 0, &producer_, 1, MOJO_WRITE_MESSAGE_FLAG_NONE));

        // Receive the consumer from the other side.
        producer_ = MOJO_HANDLE_INVALID;
        MojoHandleSignalsState hss = MojoHandleSignalsState();
        ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(server_mp, MOJO_HANDLE_SIGNAL_READABLE, &hss));
        ASSERT_EQ(MOJO_RESULT_OK, ReadEmptyMessageWithHandles(server_mp, &consumer_, 1));
...
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(MultiprocessClient, DataPipeTest, client_mp)
{
    ...
    MojoHandleSignalsState hss = MojoHandleSignalsState();
    ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(client_mp, MOJO_HANDLE_SIGNAL_READABLE, &hss));
    ASSERT_EQ(MOJO_RESULT_OK, ReadEmptyMessageWithHandles(client_mp, &consumer, 1));
    ...
    // Swap ends.
    ASSERT_EQ(MOJO_RESULT_OK, WriteMessageRaw(MessagePipeHandle(client_mp), nullptr, 0, &consumer, 1, MOJO_WRITE_MESSAGE_FLAG_NONE));

    // Receive the producer from the other side.
    MojoHandle producer = MOJO_HANDLE_INVALID;
    hss = MojoHandleSignalsState();
    ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(client_mp, MOJO_HANDLE_SIGNAL_READABLE, &hss));
    ASSERT_EQ(MOJO_RESULT_OK, ReadEmptyMessageWithHandles(client_mp, &producer, 1));
...
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(WriteAndCloseProducer, DataPipeTest, h)
{
    ...
    EXPECT_EQ(MOJO_RESULT_OK, MojoWriteData(p, message.data(), &num_bytes, nullptr));
...
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(CreateAndWrite, DataPipeTest, h)
{
    ...
    EXPECT_EQ(MOJO_RESULT_OK, MojoWriteData(p, kMessage.data(), &num_bytes, nullptr));
...
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(DataPipeStatusChangeInTransitClient, DataPipeTest, parent)
{
    ...
    // Wait on producer 0
    EXPECT_EQ(MOJO_RESULT_OK, WaitForSignals(producers[0], MOJO_HANDLE_SIGNAL_PEER_CLOSED));

    // Wait on consumer 0
    EXPECT_EQ(MOJO_RESULT_OK, WaitForSignals(consumers[0], MOJO_HANDLE_SIGNAL_PEER_CLOSED));
    ...
    // Wait on producer 1 and consumer 1 using SimpleWatchers.
    {
        ...
        auto callback = base::BindRepeating(
            ...
            &run_loop, &count);
        SimpleWatcher producer_watcher(FROM_HERE, SimpleWatcher::ArmingPolicy::AUTOMATIC, base::SequencedTaskRunnerHandle::Get());
        SimpleWatcher consumer_watcher(FROM_HERE, SimpleWatcher::ArmingPolicy::AUTOMATIC, base::SequencedTaskRunnerHandle::Get());
        producer_watcher.Watch(Handle(producers[1]), MOJO_HANDLE_SIGNAL_PEER_CLOSED, callback);
        consumer_watcher.Watch(Handle(consumers[1]), MOJO_HANDLE_SIGNAL_PEER_CLOSED, callback);
        run_loop.Run();
        EXPECT_EQ(2, count);
    }
...
TEST_F(DataPipeTest, StatusChangeInTransit)
{
    ...
    RunTestClient("DataPipeStatusChangeInTransitClient", [&](MojoHandle child) {
        MojoHandle handles[] = { producers[0], producers[1], producers[2], consumers[3], consumers[4], consumers[5] };
...
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(CreateOversizedChild, DataPipeTest, h)
{
    ...
    MojoHandle p, c;
    ASSERT_EQ(MOJO_RESULT_RESOURCE_EXHAUSTED, MojoCreateDataPipe(&options, &p, &c));
