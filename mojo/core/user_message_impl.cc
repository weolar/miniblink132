...
static_assert(sizeof(DispatcherHeader) % 8 == 0, "Invalid DispatcherHeader size.");
...
// here.
MojoResult CreateOrExtendSerializedEventMessage(ports::UserMessageEvent* event, size_t payload_size, size_t payload_buffer_size,
    const Dispatcher::DispatcherInTransit* new_dispatchers, size_t num_new_dispatchers, Channel::MessagePtr* out_message, void** out_header,
    size_t* out_header_size, void** out_user_payload)
{
    ...
    if (*out_message) {
        ...
        original_payload_size
            = original_message->payload_size() - (static_cast<char*>(original_user_payload) - static_cast<char*>(original_message->mutable_payload()));
    }
    ...
    for (size_t i = 0; i < num_new_dispatchers; ++i) {
        ...
        d->StartSerialize(&new_dispatcher_info[i].num_bytes, &new_dispatcher_info[i].num_ports, &new_dispatcher_info[i].num_handles);
    ...
    const size_t total_buffer_size = event_size + header_size + payload_buffer_size;
    void* data;
    Channel::MessagePtr message = NodeChannel::CreateEventMessage(total_buffer_size, total_size, &data, num_handles);
    auto* header = reinterpret_cast<MessageHeader*>(static_cast<uint8_t*>(data) + event_size);
    ...
    if (original_message) {
        ...
        size_t original_dispatcher_headers_size = original_header->num_dispatchers * sizeof(DispatcherHeader);
        memcpy(header, original_header, original_dispatcher_headers_size + sizeof(MessageHeader));
        new_dispatcher_headers = reinterpret_cast<DispatcherHeader*>(reinterpret_cast<uint8_t*>(header + 1) + original_dispatcher_headers_size);
        total_num_dispatchers += original_header->num_dispatchers;
        size_t total_dispatcher_headers_size = total_num_dispatchers * sizeof(DispatcherHeader);
        char* original_dispatcher_data = reinterpret_cast<char*>(original_header + 1) + original_dispatcher_headers_size;
        char* dispatcher_data = reinterpret_cast<char*>(header + 1) + total_dispatcher_headers_size;
        size_t original_dispatcher_data_size = original_header_size - sizeof(MessageHeader) - original_dispatcher_headers_size;
        memcpy(dispatcher_data, original_dispatcher_data, original_dispatcher_data_size);
        ...
        memcpy(reinterpret_cast<char*>(header) + header_size, reinterpret_cast<char*>(original_header) + original_header_size, original_payload_size);
    } else {
        ...
        // DispatcherHeaders.
        new_dispatcher_data = reinterpret_cast<char*>(new_dispatcher_headers + num_new_dispatchers);
    }

    if (handles.empty() && num_new_handles)
        handles.resize(num_new_handles);

    header->num_dispatchers = base::CheckedNumeric<uint32_t>(total_num_dispatchers).ValueOrDie();
    ...
    if (num_new_dispatchers > 0) {
        ...
        for (size_t i = 0; i < num_new_dispatchers; ++i) {
            ...
            if (g_always_fail_handle_serialization
                || !d->EndSerialize(
                    static_cast<void*>(new_dispatcher_data), event->ports() + port_index, !handles.empty() ? handles.data() + handle_index : nullptr)) {
                fail = true;
                break;
            }
...
public:
    ...
    {
        base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "MojoMessages", nullptr);
    }

    MessageMemoryDumpProvider(const MessageMemoryDumpProvider&) = delete;
    MessageMemoryDumpProvider& operator=(const MessageMemoryDumpProvider&) = delete;
...
private:
    ...
    bool OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd) override
    {
        ...
        dump->AddScalar(base::trace_event::MemoryAllocatorDump::kNameObjectCount, base::trace_event::MemoryAllocatorDump::kUnitsObjects,
...
{
    [[maybe_unused]] static base::NoDestructor<MessageMemoryDumpProvider> provider;
}
...
UserMessageImpl::~UserMessageImpl()
{
    if (HasContext() && context_destructor_) {
        ...
        MojoResult result = ExtractSerializedHandles(ExtractBadHandlePolicy::kSkip, handles.data());
        ...
        if (!pending_handle_attachments_.empty()) {
            Core::Get()->ReleaseDispatchersForTransit(pending_handle_attachments_, false);
...
// static
std::unique_ptr<ports::UserMessageEvent> UserMessageImpl::CreateEventForNewMessage(MojoCreateMessageFlags flags)
{
    auto message_event = std::make_unique<ports::UserMessageEvent>(0);
    message_event->AttachMessage(base::WrapUnique(new UserMessageImpl(message_event.get(), flags)));
    return message_event;
}

// static
MojoResult UserMessageImpl::CreateEventForNewSerializedMessage(
    uint32_t num_bytes, const Dispatcher::DispatcherInTransit* dispatchers, uint32_t num_dispatchers, std::unique_ptr<ports::UserMessageEvent>* out_event)
{
    ...
    MojoResult rv = CreateOrExtendSerializedEventMessage(
        event.get(), num_bytes, num_bytes, dispatchers, num_dispatchers, &channel_message, &header, &header_size, &user_payload);
    if (rv != MOJO_RESULT_OK)
        return rv;
    event->AttachMessage(base::WrapUnique(new UserMessageImpl(event.get(), std::move(channel_message), header, header_size, user_payload, num_bytes)));
...
std::unique_ptr<UserMessageImpl> UserMessageImpl::CreateFromChannelMessage(
    ports::UserMessageEvent* message_event, Channel::MessagePtr channel_message, void* payload, size_t payload_size)
{
    ...
    return base::WrapUnique(new UserMessageImpl(message_event, std::move(channel_message), header, header_size, user_payload, user_payload_size));
}

// static
Channel::MessagePtr UserMessageImpl::FinalizeEventMessage(std::unique_ptr<ports::UserMessageEvent> message_event)
{
    ...
    if (channel_message) {
        ...
        // received.
        bool result = NodeChannel::GetEventMessageData(*channel_message, &data, &size);
        DCHECK(result);
...
{
    DCHECK(IsSerialized());
    const size_t user_payload_offset = static_cast<uint8_t*>(user_payload_) - static_cast<const uint8_t*>(channel_message_->payload());
...
MojoResult UserMessageImpl::SetContext(uintptr_t context, MojoMessageContextSerializer serializer, MojoMessageContextDestructor destructor)
{
...
MojoResult UserMessageImpl::AppendData(uint32_t additional_payload_size, const MojoHandle* handles, uint32_t num_handles)
{
    if (HasContext())
        return MOJO_RESULT_FAILED_PRECONDITION;
    ...
    if (num_handles > 0) {
        MojoResult acquire_result = Core::Get()->AcquireDispatchersForTransit(handles, num_handles, &dispatchers);
    ...
    if (!IsSerialized()) {
        ...
        Channel::MessagePtr channel_message;
        MojoResult rv
            = CreateOrExtendSerializedEventMessage(message_event_, additional_payload_size, std::max(additional_payload_size, kMinimumPayloadBufferSize),
                dispatchers.data(), num_handles, &channel_message, &header_, &header_size_, &user_payload_);
        if (num_handles > 0) {
            Core::Get()->ReleaseDispatchersForTransit(dispatchers, rv == MOJO_RESULT_OK);
        }
        if (rv != MOJO_RESULT_OK)
        ...
        std::copy(dispatchers.begin(), dispatchers.end(), std::back_inserter(pending_handle_attachments_));

        if (additional_payload_size) {
            size_t header_offset = static_cast<uint8_t*>(header_) - static_cast<const uint8_t*>(channel_message_->payload());
            size_t user_payload_offset = static_cast<uint8_t*>(user_payload_) - static_cast<const uint8_t*>(channel_message_->payload());
            Channel::Message::ExtendPayload(channel_message_, user_payload_offset + user_payload_size_ + additional_payload_size);
            header_ = static_cast<uint8_t*>(channel_message_->mutable_payload()) + header_offset;
            user_payload_ = static_cast<uint8_t*>(channel_message_->mutable_payload()) + user_payload_offset;
            user_payload_size_ += additional_payload_size;
        }
    }

    if (!unlimited_size_ && user_payload_size_ > GetConfiguration().max_message_num_bytes) {
...
MojoResult UserMessageImpl::CommitSize()
{
    ...
    if (!pending_handle_attachments_.empty()) {
        CreateOrExtendSerializedEventMessage(message_event_, user_payload_size_, user_payload_size_, pending_handle_attachments_.data(),
            pending_handle_attachments_.size(), &channel_message_, &header_, &header_size_, &user_payload_);
        Core::Get()->ReleaseDispatchersForTransit(pending_handle_attachments_, true);
...
MojoResult UserMessageImpl::SerializeIfNecessary()
{
    ...
    context_ = 0;
    context_serializer_(reinterpret_cast<MojoMessageHandle>(message_event_), context);
...
MojoResult UserMessageImpl::ExtractSerializedHandles(ExtractBadHandlePolicy bad_handle_policy, MojoHandle* handles)
{
    if (!IsSerialized())
        return MOJO_RESULT_FAILED_PRECONDITION;
    ...
    const DispatcherHeader* dispatcher_headers = reinterpret_cast<const DispatcherHeader*>(header + 1);
    ...
    std::vector<Dispatcher::DispatcherInTransit> dispatchers(header->num_dispatchers);

    size_t data_payload_index = sizeof(MessageHeader) + header->num_dispatchers * sizeof(DispatcherHeader);
    if (data_payload_index > header->header_size)
        return MOJO_RESULT_ABORTED;
    const char* dispatcher_data = reinterpret_cast<const char*>(dispatcher_headers + header->num_dispatchers);
    size_t port_index = 0;
    size_t platform_handle_index = 0;
    std::vector<PlatformHandleInTransit> handles_in_transit = channel_message_->TakeHandles();
    ...
    for (size_t i = 0; i < header->num_dispatchers; ++i) {
        ...
        if (!next_payload_index.IsValid() || header->header_size < next_payload_index.ValueOrDie()) {
            return MOJO_RESULT_ABORTED;
        }

        base::CheckedNumeric<size_t> next_port_index = port_index;
        next_port_index += dh.num_ports;
        if (!next_port_index.IsValid() || message_event_->num_ports() < next_port_index.ValueOrDie()) {
            return MOJO_RESULT_ABORTED;
        }

        base::CheckedNumeric<size_t> next_platform_handle_index = platform_handle_index;
        next_platform_handle_index += dh.num_platform_handles;
        if (!next_platform_handle_index.IsValid() || msg_handles.size() < next_platform_handle_index.ValueOrDie()) {
            return MOJO_RESULT_ABORTED;
        }

        PlatformHandle* out_handles = !msg_handles.empty() ? msg_handles.data() + platform_handle_index : nullptr;
        dispatchers[i].dispatcher = Dispatcher::Deserialize(
            type, dispatcher_data, dh.num_bytes, message_event_->ports() + port_index, dh.num_ports, out_handles, dh.num_platform_handles);
        if (!dispatchers[i].dispatcher && bad_handle_policy == ExtractBadHandlePolicy::kAbort) {
            return MOJO_RESULT_ABORTED;
        }
...
UserMessageImpl::UserMessageImpl(ports::UserMessageEvent* message_event, MojoCreateMessageFlags flags)
...
UserMessageImpl::UserMessageImpl(
    ports::UserMessageEvent* message_event, Channel::MessagePtr channel_message, void* header, size_t header_size, void* user_payload, size_t user_payload_size)
    : ports::UserMessage(&kUserMessageTypeInfo)
    , message_event_(message_event)
    , channel_message_(std::move(channel_message))
    , has_serialized_handles_(true)
