...
public:
    ...
    FakeMessagePipeDispatcherForTesting(const FakeMessagePipeDispatcherForTesting&) = delete;
    FakeMessagePipeDispatcherForTesting& operator=(const FakeMessagePipeDispatcherForTesting&) = delete;
...
std::vector<MojoHandle> AddDispatchersForTesting(const int num_dispatchers_to_add, HandleTable* handle_table)
{
...
TEST(HandleTablePerfTest, GetDispatcherDifferentHandles)
{
    ...
    const std::vector<MojoHandle> handles = AddDispatchersForTesting(kNumDispatchers, &handle_table);
...
TEST(HandleTablePerfTest, AddAndRemoveDispatcher)
{
    ...
    while (!timer.HasTimeLimitExpired()) {
        ...
        EXPECT_THAT(handle, Ne(MOJO_HANDLE_INVALID));
        const MojoResult result = handle_table.GetAndRemoveDispatcher(handle, &dispatcher);
