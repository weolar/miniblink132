...
public:
    ...
    explicit UserOptionsReader(const Options* options)
    {
        ...
        static_assert(offsetof(Options, struct_size) == 0, "struct_size not first member of Options");
...
private:
    ...
    template <size_t alignment> static bool IsAligned(const void* pointer)
    {
...
#define OPTIONS_STRUCT_HAS_MEMBER(Options, member, reader) reader.HasMember(offsetof(Options, member), sizeof(reader.options().member))
