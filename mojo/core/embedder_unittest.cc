...
template <typename T> MojoResult CreateSharedBufferFromRegion(T&& region, MojoHandle* handle)
{
    if (IsMojoIpczEnabled()) {
        *handle = ipcz_driver::SharedBuffer::Box(ipcz_driver::SharedBuffer::MakeForRegion(std::move(region)));
        return MOJO_RESULT_OK;
    }

    scoped_refptr<SharedBufferDispatcher> buffer;
    MojoResult result = SharedBufferDispatcher::CreateFromPlatformSharedMemoryRegion(T::TakeHandleForSerialization(std::forward<T>(region)), &buffer);
...
template <typename T> MojoResult ExtractRegionFromSharedBuffer(MojoHandle handle, T* region)
{
    base::subtle::PlatformSharedMemoryRegion platform_region;
    if (IsMojoIpczEnabled()) {
        platform_region = std::move(ipcz_driver::SharedBuffer::Unbox(handle)->region());
    } else {
        scoped_refptr<Dispatcher> dispatcher = Core::Get()->GetAndRemoveDispatcher(handle);
...
TEST_F(EmbedderTest, MultiprocessChannels)
{
    RunTestClient("MultiprocessChannelsClient", [&](MojoHandle server_mp) {
        ...
        ASSERT_EQ(MOJO_RESULT_FAILED_PRECONDITION, WaitForSignals(mp2, MOJO_HANDLE_SIGNAL_READABLE, &state));
...
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(MultiprocessChannelsClient, EmbedderTest, client_mp)
{
    ...
    MojoHandleSignalsState state;
    ASSERT_EQ(MOJO_RESULT_FAILED_PRECONDITION, WaitForSignals(mp1, MOJO_HANDLE_SIGNAL_READABLE, &state));
    ASSERT_EQ(MOJO_HANDLE_SIGNAL_PEER_CLOSED, state.satisfied_signals);
    ASSERT_FALSE(state.satisfiable_signals & MOJO_HANDLE_SIGNAL_READABLE);
    ASSERT_FALSE(state.satisfiable_signals & MOJO_HANDLE_SIGNAL_WRITABLE);
...
TEST_F(EmbedderTest, MultiprocessBaseSharedMemory)
{
    RunTestClient("MultiprocessSharedMemoryClient", [&](MojoHandle server_mp) {
        ...
        MojoHandle sb1;
        ASSERT_EQ(MOJO_RESULT_OK, CreateSharedBufferFromRegion(shared_memory.Duplicate(), &sb1));

        // 2. Map |sb1| and write something into it.
        char* buffer = nullptr;
        ASSERT_EQ(MOJO_RESULT_OK, MojoMapBuffer(sb1, 0, 123, nullptr, reinterpret_cast<void**>(&buffer)));
        ASSERT_TRUE(buffer);
...
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(MultiprocessSharedMemoryClient, EmbedderTest, client_mp)
{
    ...
    RunTestClient("MultiprocessMixMachAndFdsClient", [&](MojoHandle server_mp) {
        ...
        for (size_t i = 0; i < std::size(kTestHandleTypes); i++) {
            ...
            if (type == HandleType::POSIX) {
                ...
                base::File file(base::FilePath("/dev/null"), base::File::FLAG_OPEN | base::File::FLAG_WRITE);
                ...
                ASSERT_TRUE(shared_memory.IsValid());
                auto shm_handle = base::UnsafeSharedMemoryRegion::TakeHandleForSerialization(std::move(shared_memory)).PassPlatformHandle();
                scoped_handle = PlatformHandle(std::move(shm_handle));
                ASSERT_TRUE(scoped_handle.is_valid_mach_port());
            }
            platform_handles[i] = WrapPlatformHandle(std::move(scoped_handle)).release().value();
        }

        // 2. Send all the handles to the child.
        WriteMessageWithHandles(server_mp, "hello", platform_handles, std::size(kTestHandleTypes));
...
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(MultiprocessMixMachAndFdsClient, EmbedderTest, client_mp)
{
    ...
    // |kNumHandles| handles.
    EXPECT_EQ("hello", ReadMessageWithHandles(client_mp, platform_handles, kNumHandles));

    // 2. Extract each handle, and verify the type.
    for (int i = 0; i < kNumHandles; i++) {
        const auto type = kTestHandleTypes[i];
        PlatformHandle scoped_handle = UnwrapPlatformHandle(ScopedHandle(Handle(platform_handles[i])));
        if (type == HandleType::POSIX) {
