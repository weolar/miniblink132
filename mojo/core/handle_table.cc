...
const scoped_refptr<Dispatcher>* HandleTable::EntriesAccessor::GetDispatcher(const MojoHandle handle)
{
...
HandleTable::Entry* HandleTable::EntriesAccessor::GetMutable(const MojoHandle handle)
{
    const auto iter = handles_.find(handle);
...
MojoResult HandleTable::EntriesAccessor::Remove(
    const MojoHandle handle, const HandleTable::EntriesAccessor::RemovalCondition removal_condition, scoped_refptr<Dispatcher>* dispatcher)
{
    ...
    const bool remove_only_if_busy = removal_condition == RemovalCondition::kRemoveOnlyIfBusy;
...
const std::unordered_map<MojoHandle, HandleTable::Entry>& HandleTable::EntriesAccessor::GetUnderlyingMap() const
{
    return handles_;
}
...
bool HandleTable::AddDispatchersFromTransit(const std::vector<Dispatcher::DispatcherInTransit>& dispatchers, MojoHandle* handles)
{
    // Oops, we're out of handles.
    if (next_available_handle_ == MOJO_HANDLE_INVALID) {
        return false;
    ...
    const uintptr_t num_handles_available = std::numeric_limits<uintptr_t>::max() - next_available_handle_ + 1;
    ...
    for (size_t i = 0; i < dispatchers.size(); ++i) {
        ...
        if (dispatchers[i].dispatcher) {
            ...
            const bool inserted = entries_.Add(handle, Entry(dispatchers[i].dispatcher));
            DCHECK(inserted);
        }
        handles[i] = handle;
    }
...
MojoResult HandleTable::GetAndRemoveDispatcher(MojoHandle handle, scoped_refptr<Dispatcher>* dispatcher)
{
    scoped_refptr<Dispatcher> removed_dispatcher;
    const MojoResult remove_result = entries_.Remove(handle, EntriesAccessor::RemovalCondition::kRemoveOnlyIfNotBusy, &removed_dispatcher);
...
MojoResult HandleTable::BeginTransit(const MojoHandle* handles, size_t num_handles, std::vector<Dispatcher::DispatcherInTransit>* dispatchers)
{
...
void HandleTable::CompleteTransitAndClose(const std::vector<Dispatcher::DispatcherInTransit>& dispatchers)
{
    for (const auto& dispatcher : dispatchers) {
        const MojoResult remove_result = entries_.Remove(dispatcher.local_handle, EntriesAccessor::RemovalCondition::kRemoveOnlyIfBusy,
            /*dispatcher=*/nullptr);
...
void HandleTable::CancelTransit(const std::vector<Dispatcher::DispatcherInTransit>& dispatchers)
{
    for (const auto& dispatcher : dispatchers) {
...
bool HandleTable::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    ...
    for (const auto& entry : handle_count) {
        base::trace_event::MemoryAllocatorDump* inner_dump = pmd->CreateAllocatorDump(std::string("mojo/") + GetNameForDispatcherType(entry.first));
        inner_dump->AddScalar(base::trace_event::MemoryAllocatorDump::kNameObjectCount, base::trace_event::MemoryAllocatorDump::kUnitsObjects, entry.second);
    }
