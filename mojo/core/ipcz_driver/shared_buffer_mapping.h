...
public:
    SharedBufferMapping(std::unique_ptr<base::SharedMemoryMapping> mapping, void* memory);

    static constexpr Type object_type()
    {
        return kSharedBufferMapping;
    }
    ...
    static scoped_refptr<SharedBufferMapping> Create(base::subtle::PlatformSharedMemoryRegion& region, size_t offset, size_t size);

    // Maps the whole region.
    static scoped_refptr<SharedBufferMapping> Create(base::subtle::PlatformSharedMemoryRegion& region);
