...
public:
    ...
    IpczHandle validator() const
    {
        return validator_;
    }

    // Sets the contents of this message, as read from a portal by ipcz.
    bool SetContents(std::vector<uint8_t> data, std::vector<IpczHandle> handles, IpczHandle validator);
    ...
    // MojoGetMessageData();
    IpczResult GetData(void** buffer, uint32_t* num_bytes, MojoHandle* handles, uint32_t* num_handles, bool consume_handles);
