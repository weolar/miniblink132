...
public:
    ...
    template <typename RegionType> static scoped_refptr<SharedBuffer> MakeForRegion(RegionType region)
    {
        return base::MakeRefCounted<SharedBuffer>(RegionType::TakeHandleForSerialization(std::move(region)));
    }

    static constexpr Type object_type()
    {
        return kSharedBuffer;
    }
    ...
    static scoped_refptr<SharedBuffer> CreateForMojoWrapper(base::span<const MojoPlatformHandle> mojo_platform_handles, uint32_t size,
    ...
    static scoped_refptr<SharedBuffer> Deserialize(base::span<const uint8_t> data, base::span<PlatformHandle> handles);
