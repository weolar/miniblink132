...
// pipe endpoint.
void GetConditionsForSignals(MojoHandleSignals signals, IpczTrapConditions* conditions, DataPipe* data_pipe)
{
...
// endpoint.
void TranslateIpczToMojoEvent(MojoHandleSignals trigger_signals, uintptr_t trigger_context, DataPipe* data_pipe, IpczTrapConditionFlags current_condition_flags,
    const IpczPortalStatus& current_status, MojoTrapEvent* event)
{
    event->flags = 0;
    ...
    if (current_status.flags & IPCZ_PORTAL_STATUS_PEER_CLOSED) {
        ...
        if (!data_pipe || current_status.num_remote_bytes < data_pipe->byte_capacity()) {
            satisfied |= kWrite;
        }
    }
...
struct MojoTrap::Trigger : public base::RefCountedThreadSafe<Trigger> {
    ...
    Trigger(scoped_refptr<MojoTrap> mojo_trap, MojoHandle handle, DataPipe* data_pipe, MojoHandleSignals signals, uintptr_t trigger_context)
...
MojoResult MojoTrap::AddTrigger(MojoHandle handle, MojoHandleSignals signals, MojoTriggerCondition condition, uintptr_t trigger_context)
{
    ...
    auto trigger = base::MakeRefCounted<Trigger>(this, handle, data_pipe, signals, trigger_context);
    ...
    IpczPortalStatus status = { sizeof(status) };
    {
        ...
        IpczResult result
            = GetIpczAPI().Trap(handle, &removal_conditions, &TrapRemovalEventHandler, trigger->ipcz_context(), IPCZ_NO_FLAGS, nullptr, nullptr, nullptr);
        ...
        pending_mojo_events_->emplace_back(MojoTrapEvent { .struct_size = sizeof(MojoTrapEvent) });
        MojoTrapEvent& event = pending_mojo_events_->back();
        TranslateIpczToMojoEvent(signals, trigger_context, data_pipe, flags, status, &event);
...
MojoResult MojoTrap::Arm(MojoTrapEvent* blocking_events, uint32_t* num_blocking_events)
{
    const uint32_t event_capacity = num_blocking_events ? *num_blocking_events : 0;
    if (event_capacity > 0 && !blocking_events) {
        return MOJO_RESULT_INVALID_ARGUMENT;
    }

    if (event_capacity > 0 && blocking_events[0].struct_size < sizeof(blocking_events[0])) {
        return MOJO_RESULT_INVALID_ARGUMENT;
    }
    ...
    do {
        ...
        TranslateIpczToMojoEvent(trigger->signals, trigger->trigger_context, trigger->data_pipe.get(), flags, status, &event);
...
void MojoTrap::HandleEvent(const IpczTrapEvent& event)
{
    ...
    if (trigger->data_pipe && (event.condition_flags & IPCZ_TRAP_NEW_LOCAL_PARCEL)) {
...

    {
        ...
        TranslateIpczToMojoEvent(trigger->signals, trigger->trigger_context, trigger->data_pipe.get(), event.condition_flags, *event.status, &mojo_event);
...
IpczResult MojoTrap::ArmTrigger(Trigger& trigger, IpczTrapConditionFlags* satisfied_flags, IpczPortalStatus* status)
{
    lock_.AssertAcquired();
    if (trigger.armed) {
    ...
    const bool watching_writable = trigger.conditions.flags & (IPCZ_TRAP_BELOW_MAX_REMOTE_BYTES | IPCZ_TRAP_BELOW_MAX_REMOTE_PARCELS);
    const bool watching_readable = trigger.conditions.flags & (IPCZ_TRAP_ABOVE_MIN_LOCAL_PARCELS | IPCZ_TRAP_ABOVE_MIN_LOCAL_BYTES);
    const bool watching_new_data = trigger.conditions.flags & IPCZ_TRAP_NEW_LOCAL_PARCEL;
    const bool watching_anything_else = trigger.conditions.flags != 0 && !(watching_writable || watching_readable || watching_new_data);
    if (trigger.is_for_data_producer() && (watching_readable || watching_new_data) && !watching_anything_else) {
        return IPCZ_RESULT_FAILED_PRECONDITION;
    }
    ...
    trigger.AddRef();
    IpczResult result
        = GetIpczAPI().Trap(trigger.handle, &trigger.conditions, &TrapEventHandler, trigger.ipcz_context(), IPCZ_NO_FLAGS, nullptr, satisfied_flags, status);
    if (result == IPCZ_RESULT_OK) {
        trigger.armed = true;
    } else {
