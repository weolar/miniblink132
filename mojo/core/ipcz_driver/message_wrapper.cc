...
MessageWrapper::MessageWrapper(std::unique_ptr<MojoMessage> message, IpczHandle portal)
...
bool MessageWrapper::GetSerializedDimensions(Transport& transmitter, size_t& num_bytes, size_t& num_handles)
{
...
bool MessageWrapper::Serialize(Transport& transmitter, base::span<uint8_t> data, base::span<PlatformHandle> handles)
{
    ...
    if (std::unique_ptr<MojoMessage> message = std::move(message_)) {
        ...
        const IpczResult result = GetIpczAPI().Put(
            portal_, message->data().data(), message->data().size(), message->handles().data(), message->handles().size(), IPCZ_NO_FLAGS, nullptr);
