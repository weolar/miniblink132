...
// an access mode.
base::subtle::ScopedPlatformSharedMemoryHandle CreateRegionHandleFromPlatformHandles(
    base::span<PlatformHandle> handles, base::subtle::PlatformSharedMemoryRegion::Mode mode)
{
    if (handles.empty()) {
        return {};
    }
...
std::pair<scoped_refptr<SharedBuffer>, IpczResult> SharedBuffer::Duplicate(bool read_only)
{
    ...
    return { base::MakeRefCounted<ipcz_driver::SharedBuffer>(std::move(new_region)), IPCZ_RESULT_OK };
}

// static
scoped_refptr<SharedBuffer> SharedBuffer::CreateForMojoWrapper(base::span<const MojoPlatformHandle> mojo_platform_handles, uint32_t size,
    const MojoSharedBufferGuid& mojo_guid, MojoPlatformSharedMemoryRegionAccessMode access)
{
    ...
    auto guid = base::UnguessableToken::Deserialize(mojo_guid.high, mojo_guid.low);
    if (guid.is_empty()) {
        return nullptr;
    }
    ...
    for (size_t i = 0; i < mojo_platform_handles.size(); ++i) {
        handles[i] = PlatformHandle::FromMojoPlatformHandle(&mojo_platform_handles[i]);
    }

    auto handle = CreateRegionHandleFromPlatformHandles({ &handles[0], mojo_platform_handles.size() }, mode);
    auto region = base::subtle::PlatformSharedMemoryRegion::Take(std::move(handle), mode, size, guid);
    if (!region.IsValid()) {
        return nullptr;
    }
...
bool SharedBuffer::GetSerializedDimensions(Transport& transmitter, size_t& num_bytes, size_t& num_handles)
{
    num_bytes = sizeof(BufferHeader);
#if BUILDFLAG(IS_WIN) || BUILDFLAG(IS_APPLE) || BUILDFLAG(IS_FUCHSIA) || BUILDFLAG(IS_ANDROID)
    num_handles = 1;
#else
...
bool SharedBuffer::Serialize(Transport& transmitter, base::span<uint8_t> data, base::span<PlatformHandle> handles)
{
    if (!region_.IsValid()) {
        return false;
    }
...
// static
scoped_refptr<SharedBuffer> SharedBuffer::Deserialize(base::span<const uint8_t> data, base::span<PlatformHandle> handles)
{
    if (data.size() < sizeof(BufferHeader) || handles.empty()) {
        return nullptr;
    }

    const BufferHeader& header = *reinterpret_cast<const BufferHeader*>(data.data());
    ...
    auto guid = base::UnguessableToken::Deserialize(header.guid_high, header.guid_low);

    auto handle = CreateRegionHandleFromPlatformHandles(handles, mode);
    auto region = base::subtle::PlatformSharedMemoryRegion::Take(std::move(handle), mode, header.size, guid);
    if (!region.IsValid()) {
        return nullptr;
    }
