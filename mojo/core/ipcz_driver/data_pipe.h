...
public:
    ...
    public:
        ...
        IpczHandle handle() const
        {
            return handle_;
        }
        void set_handle(IpczHandle handle)
        {
            handle_ = handle;
        }
    ...
    {
        return limits_.max_queued_bytes;
    }
    ...
    IpczResult WriteData(const void* elements, uint32_t& num_bytes, MojoWriteDataFlags flags);
    IpczResult BeginWriteData(void*& data, uint32_t& num_bytes, MojoBeginWriteDataFlags flags);
    IpczResult EndWriteData(size_t num_bytes_produced);
    IpczResult ReadData(void* elements, uint32_t& num_bytes, MojoReadDataFlags flags);
    ...
    static scoped_refptr<DataPipe> Deserialize(base::span<const uint8_t> data, base::span<const PlatformHandle> handles);
