...
bool TransmissiblePlatformHandle::GetSerializedDimensions(Transport& transmitter, size_t& num_bytes, size_t& num_handles)
{
...
bool TransmissiblePlatformHandle::Serialize(Transport& transmitter, base::span<uint8_t> data, base::span<PlatformHandle> handles)
{
...
// static
scoped_refptr<TransmissiblePlatformHandle> TransmissiblePlatformHandle::Deserialize(base::span<const uint8_t> data, base::span<PlatformHandle> handles)
{
    if (handles.size() != 1) {
        return nullptr;
    }
    return base::MakeRefCounted<TransmissiblePlatformHandle>(std::move(handles[0]));
}
