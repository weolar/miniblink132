...
IpczDriverHandle CreateTransportForMojoEndpoint(Transport::EndpointTypes endpoint_types, const MojoInvitationTransportEndpoint& endpoint,
    bool leak_channel_on_shutdown, base::Process remote_process = base::Process(), MojoProcessErrorHandler error_handler = nullptr,
    uintptr_t error_handler_context = 0)
{
    CHECK_EQ(endpoint.num_platform_handles, 1u);
    auto handle = PlatformHandle::FromMojoPlatformHandle(&endpoint.platform_handles[0]);
    if (!handle.is_valid()) {
    ...
    auto transport = base::MakeRefCounted<Transport>(endpoint_types, std::move(channel_endpoint), std::move(remote_process));
...
// static
void Invitation::SetDefaultProcessErrorHandler(MojoDefaultProcessErrorHandler handler)
{
...
MojoResult Invitation::Attach(base::span<const uint8_t> name, MojoHandle* handle)
{
    ...
    IpczResult result = GetIpczAPI().OpenPortals(GetIpczNode(), IPCZ_NO_FLAGS, nullptr, &attachment, handle);
...
MojoResult Invitation::Extract(base::span<const uint8_t> name, MojoHandle* handle)
{
...
MojoResult Invitation::Send(const MojoPlatformProcessHandle* process_handle, const MojoInvitationTransportEndpoint* transport_endpoint,
    MojoProcessErrorHandler error_handler, uintptr_t error_handler_context, const MojoSendInvitationOptions* options)
{
    if (!transport_endpoint || transport_endpoint->struct_size < sizeof(*transport_endpoint) || transport_endpoint->num_platform_handles == 0
    ...
    if (process_handle) {
        if (UnwrapAndClonePlatformProcessHandle(process_handle, remote_process) != MOJO_RESULT_OK) {
            return MOJO_RESULT_INVALID_ARGUMENT;
        }
    }

    const bool is_isolated = options && (options->flags & MOJO_SEND_INVITATION_FLAG_ISOLATED) != 0;
    ...
    IpczDriverHandle transport = CreateTransportForMojoEndpoint(
        { .source = config.is_broker ? Transport::kBroker : Transport::kNonBroker, .destination = is_isolated ? Transport::kBroker : Transport::kNonBroker },
        *transport_endpoint,
        /*leak_channel_on_shutdown=*/false, std::move(remote_process), error_handler, error_handler_context);
    ...
    IpczResult result = GetIpczAPI().ConnectNode(GetIpczNode(), transport, num_attachments_ + 1, flags, nullptr, portals);
    ...
    for (size_t i = 0; i < num_attachments_; ++i) {
        result = GetIpczAPI().MergePortals(attachments_[i].release(), portals[i + 1], IPCZ_NO_FLAGS, nullptr);
        CHECK_EQ(result, IPCZ_RESULT_OK);
    }
...
// static
MojoHandle Invitation::Accept(const MojoInvitationTransportEndpoint* transport_endpoint, const MojoAcceptInvitationOptions* options)
{
    if (!transport_endpoint || transport_endpoint->struct_size < sizeof(*transport_endpoint) || transport_endpoint->num_platform_handles == 0
        || !transport_endpoint->platform_handles) {
        return MOJO_RESULT_INVALID_ARGUMENT;
    ...
    if (options) {
        ...
        leak_transport = (options->flags & MOJO_ACCEPT_INVITATION_FLAG_LEAK_TRANSPORT_ENDPOINT);
    ...
    IpczHandle portals[kMaxAttachments + 1];
    IpczDriverHandle transport = CreateTransportForMojoEndpoint(
        { .source = is_isolated ? Transport::kBroker : Transport::kNonBroker, .destination = Transport::kBroker }, *transport_endpoint, leak_transport);
    if (transport == IPCZ_INVALID_DRIVER_HANDLE) {
        return IPCZ_INVALID_DRIVER_HANDLE;
    }
    ...
    if (base::IsCurrentProcessElevated()) {
        launcher = base::Process(::OpenProcess(PROCESS_DUP_HANDLE, FALSE, base::GetParentProcessId(base::GetCurrentProcessHandle())));
    }
#endif
    if (launcher.IsValid()) {
    ...
    IpczResult result = GetIpczAPI().ConnectNode(GetIpczNode(), transport, kMaxAttachments + 1, flags, nullptr, portals);
    CHECK_EQ(result, IPCZ_RESULT_OK);
    ...
    for (size_t i = 0; i < kMaxAttachments; ++i) {
        IpczHandle attachment;
        IpczHandle bridge;
        GetIpczAPI().OpenPortals(GetIpczNode(), IPCZ_NO_FLAGS, nullptr, &attachment, &bridge);
        result = GetIpczAPI().MergePortals(portals[i + 1], bridge, IPCZ_NO_FLAGS, nullptr);
