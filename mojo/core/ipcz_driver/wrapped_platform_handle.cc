...
{
    zx::handle result;
    const zx_status_t status = fdio_fd_transfer_or_clone(fd.release(), result.reset_and_get_address());
    if (status != ZX_OK) {
...
{
    base::ScopedFD fd;
    zx_status_t status = fdio_fd_create(handle.ReleaseHandle(), base::ScopedFD::Receiver(fd).get());
    if (status != ZX_OK) {
...
PlatformHandle MakeFDTransmissible(base::ScopedFD fd)
{
    ...
    kern_return_t kr = fileport_makeport(fd.get(), base::mac::ScopedMachSendRight::Receiver(port).get());
    if (kr != KERN_SUCCESS) {
...
bool WrappedPlatformHandle::GetSerializedDimensions(Transport& transmitter, size_t& num_bytes, size_t& num_handles)
{
...
bool WrappedPlatformHandle::Serialize(Transport& transmitter, base::span<uint8_t> data, base::span<PlatformHandle> handles)
{
    DCHECK(handle_.is_valid());
...
// static
scoped_refptr<WrappedPlatformHandle> WrappedPlatformHandle::Deserialize(base::span<const uint8_t> data, base::span<PlatformHandle> handles)
{
    if (data.size() < sizeof(WrappedPlatformHandleHeader) || handles.size() != 1) {
        return nullptr;
    }

    const auto& header = *reinterpret_cast<const WrappedPlatformHandleHeader*>(data.data());
