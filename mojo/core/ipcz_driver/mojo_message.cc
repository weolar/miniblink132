...
MojoMessage::MojoMessage(std::vector<uint8_t> data, std::vector<IpczHandle> handles)
{
...
bool MojoMessage::SetContents(std::vector<uint8_t> data, std::vector<IpczHandle> handles, IpczHandle validator)
{
...
MojoResult MojoMessage::AppendData(
    uint32_t additional_num_bytes, const MojoHandle* handles, uint32_t num_handles, void** buffer, uint32_t* buffer_size, bool commit_size)
{
...
IpczResult MojoMessage::GetData(void** buffer, uint32_t* num_bytes, MojoHandle* handles, uint32_t* num_handles, bool consume_handles)
{
    if (context_ || !size_committed_) {
        return MOJO_RESULT_FAILED_PRECONDITION;
    }
...
MojoResult MojoMessage::SetContext(uintptr_t context, MojoMessageContextSerializer serializer, MojoMessageContextDestructor destructor)
{
    if (context_ && context) {
...
MojoResult MojoMessage::Serialize()
{
    ...
    const MojoMessageContextSerializer serializer = std::exchange(serializer_, nullptr);
    const MojoMessageContextDestructor destructor = std::exchange(destructor_, nullptr);
