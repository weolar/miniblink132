...
class MOJO_SYSTEM_IMPL_EXPORT ObjectBase : public base::RefCountedThreadSafe<ObjectBase> {
public:
    ...
    virtual bool GetSerializedDimensions(Transport& transmitter, size_t& num_bytes, size_t& num_handles);
    ...
    virtual bool Serialize(Transport& transmitter, base::span<uint8_t> data, base::span<PlatformHandle> handles);
...
public:
    ...
    template <typename... Args> static IpczHandle MakeBoxed(Args&&... args)
    {
    ...
    static T* FromBox(IpczHandle box)
    {
        return FromHandle(PeekBox(box));
    }
