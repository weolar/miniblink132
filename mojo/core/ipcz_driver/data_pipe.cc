...
IpczResult EndReadDataImpl(IpczHandle portal, size_t element_size, size_t num_bytes_consumed)
{
    if (num_bytes_consumed == 0) {
        return GetIpczAPI().EndGet(portal, 0, 0, IPCZ_NO_FLAGS, nullptr, nullptr, nullptr);
    }
    ...
    if (num_bytes_consumed % element_size != 0) {
        ...
        result = GetIpczAPI().EndGet(portal, num_bytes_consumed, 0, IPCZ_NO_FLAGS, nullptr, nullptr, nullptr);
    ...
    if (result != IPCZ_RESULT_OK) {
        ...
        GetIpczAPI().EndGet(portal, 0, 0, IPCZ_END_GET_ABORT, nullptr, nullptr, nullptr);
    }
    return result;
}
...
DataPipe::PortalWrapper::PortalWrapper(IpczHandle handle)
    : handle_(handle)
{
}
...
DataPipe::Pair DataPipe::CreatePair(const Config& config)
{
    IpczHandle producer;
    IpczHandle consumer;
    const IpczResult result = GetIpczAPI().OpenPortals(GetIpczNode(), IPCZ_NO_FLAGS, nullptr, &producer, &consumer);
    DCHECK_EQ(result, IPCZ_RESULT_OK);

    Pair pair;
    pair.consumer = base::MakeRefCounted<DataPipe>(Config { .element_size = config.element_size, .byte_capacity = 0 });
...
void DataPipe::AdoptPortal(IpczHandle portal)
{
    ...
    if (byte_capacity() == 0) {
        ...
        GetIpczAPI().QueryPortalStatus(wrapper->handle(), IPCZ_NO_FLAGS, nullptr, &status);
...
IpczResult DataPipe::WriteData(const void* elements, uint32_t& num_bytes, MojoWriteDataFlags flags)
{
    ...
    const MojoBeginWriteDataFlags begin_write_flags
        = (flags & MOJO_WRITE_DATA_FLAG_ALL_OR_NONE) ? MOJO_BEGIN_WRITE_DATA_FLAG_ALL_OR_NONE : MOJO_BEGIN_WRITE_DATA_FLAG_NONE;

    const uint32_t max_num_bytes = num_bytes;
    void* data;
    const IpczResult begin_result = BeginWriteData(data, num_bytes, begin_write_flags);
...
IpczResult DataPipe::BeginWriteData(void*& data, uint32_t& num_bytes, MojoBeginWriteDataFlags flags)
{
    ...
    const IpczBeginPutFlags begin_put_flags = allow_partial ? IPCZ_BEGIN_PUT_ALLOW_PARTIAL : IPCZ_NO_FLAGS;
    ...
    size_t put_num_bytes = (num_bytes || !allow_partial) ? num_bytes : kDefaultBufferSize;
    if (allow_partial) {
        put_num_bytes = std::min(put_num_bytes, limits_.max_queued_bytes);
    }
    const IpczResult begin_put_result = GetIpczAPI().BeginPut(portal->handle(), begin_put_flags, &begin_put_options, &put_num_bytes, &data);
...
IpczResult DataPipe::EndWriteData(size_t num_bytes_produced)
{
    ...
    if (num_bytes_produced % element_size_ != 0) {
        ...
        result = GetIpczAPI().EndPut(portal->handle(), num_bytes_produced, nullptr, 0, flags, nullptr);
    }

    if (result != IPCZ_RESULT_OK) {
        // Unlike with ipcz, Mojo's two-phase operations are expected to terminate
        // in all failure modes.
        GetIpczAPI().EndPut(portal->handle(), 0, nullptr, 0, IPCZ_END_PUT_ABORT, nullptr);
    }
...
IpczResult DataPipe::ReadData(void* elements, uint32_t& num_bytes, MojoReadDataFlags flags)
{
    ...
    IpczPortalStatus status = { .size = sizeof(status) };
    {
        ...
        const IpczResult query_result = GetIpczAPI().QueryPortalStatus(portal, IPCZ_NO_FLAGS, nullptr, &status);
    ...
    if (!allow_partial) {
        if (num_bytes_available < num_bytes) {
            return is_peer_closed ? IPCZ_RESULT_FAILED_PRECONDITION : IPCZ_RESULT_OUT_OF_RANGE;
        }

        auto out_data = base::span<uint8_t>(static_cast<uint8_t*>(elements), num_bytes);
        while (!out_data.empty()) {
            const void* data;
            size_t get_num_bytes;
            const IpczResult begin_result = GetIpczAPI().BeginGet(portal, IPCZ_NO_FLAGS, nullptr, &data, &get_num_bytes, nullptr);

            // Hitting this DCHECK implies that another thread is reading our portal.
            DCHECK_EQ(begin_result, IPCZ_RESULT_OK);
            const size_t num_bytes_to_consume = std::min(get_num_bytes, out_data.size());
            if (!discard) {
    ...
    const IpczResult begin_result = BeginReadData(data, num_bytes_available_for_get);
    if (begin_result != IPCZ_RESULT_OK) {
        return begin_result;
    }

    const size_t num_bytes_to_consume = std::min(num_bytes_available_for_get, num_bytes);
    if (!discard) {
...
IpczResult DataPipe::BeginReadData(const void*& buffer, uint32_t& buffer_num_bytes)
{
    base::AutoLock lock(lock_);
    if (!portal_) {
        return IPCZ_RESULT_INVALID_ARGUMENT;
    ...
    size_t num_bytes;
    const IpczResult begin_result = GetIpczAPI().BeginGet(portal_->handle(), IPCZ_NO_FLAGS, nullptr, &buffer, &num_bytes, nullptr);
    if (begin_result != IPCZ_RESULT_OK) {
        return begin_result;
    }
...
bool DataPipe::GetSerializedDimensions(Transport& transmitter, size_t& num_bytes, size_t& num_handles)
{
...
bool DataPipe::Serialize(Transport& transmitter, base::span<uint8_t> data, base::span<PlatformHandle> handles)
{
...
// static
scoped_refptr<DataPipe> DataPipe::Deserialize(base::span<const uint8_t> data, base::span<const PlatformHandle> handles)
{
    ...
    return base::MakeRefCounted<DataPipe>(Config { .element_size = header.element_size, .byte_capacity = header.byte_capacity });
}
...
void DataPipe::WatchForNewData()
{
    ...
    const IpczResult result = GetIpczAPI().Trap(portal->handle(), &conditions, handler, context, IPCZ_NO_FLAGS, nullptr, nullptr, nullptr);
    DCHECK_EQ(result, IPCZ_RESULT_OK);
}
