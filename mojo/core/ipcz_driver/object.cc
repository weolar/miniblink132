...
bool ObjectBase::GetSerializedDimensions(Transport& transmitter, size_t& num_bytes, size_t& num_handles)
{
    return false;
}

bool ObjectBase::Serialize(Transport& transmitter, base::span<uint8_t> data, base::span<PlatformHandle> handles)
{
    return false;
}
...
IpczHandle ObjectBase::Box(scoped_refptr<ObjectBase> object)
{
    ...
    IpczHandle box;
    const IpczResult result = GetIpczAPI().Box(GetIpczNode(), handle, IPCZ_NO_FLAGS, nullptr, &box);
