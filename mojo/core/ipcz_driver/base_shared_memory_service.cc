...
void CreateBrokerHostOnIOThread(PlatformChannelEndpoint endpoint)
{
    ...
    new BrokerHost(base::Process(), ConnectionParams(std::move(endpoint)), base::NullCallback());
}
...
void CreateServiceFromNextParcel(ScopedIpczHandle portal)
{
    ...
    const IpczResult result = GetIpczAPI().Get(portal.get(), IPCZ_NO_FLAGS, nullptr, nullptr, nullptr, &box, &num_handles, nullptr);
    ...
    Transport::GetIOTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&CreateBrokerHostOnIOThread, transport->TakeEndpoint()));
}
...
void WaitForClientConnection(ScopedIpczHandle portal)
{
    ...
    const IpczResult result = GetIpczAPI().Trap(
        portal.get(), &conditions, handler, reinterpret_cast<uintptr_t>(portal.get()), IPCZ_NO_FLAGS, nullptr, &satisfied_conditions, nullptr);
    ...
    if (result == IPCZ_RESULT_FAILED_PRECONDITION && (satisfied_conditions & IPCZ_TRAP_ABOVE_MIN_LOCAL_PARCELS) != 0) {
        CreateServiceFromNextParcel(std::move(portal));
        return;
    }
}
...
base::MappedReadOnlyRegion CreateReadOnlySharedMemoryRegion(size_t size, base::SharedMemoryMapper* mapper)
{
    ...
    return { base::WritableSharedMemoryRegion::ConvertToReadOnly(std::move(writable_region)), std::move(mapping) };
}
...
base::UnsafeSharedMemoryRegion CreateUnsafeSharedMemoryRegion(size_t size)
{
    ...
    return base::WritableSharedMemoryRegion::ConvertToUnsafe(std::move(writable_region));
}
...
void BaseSharedMemoryService::CreateClient(ScopedIpczHandle portal)
{
    ...
    IpczHandle box = Transport::Box(Transport::Create({ .source = Transport::kBroker, .destination = Transport::kNonBroker }, channel.TakeRemoteEndpoint()));
    const IpczResult result = GetIpczAPI().Put(portal.get(), nullptr, 0, &box, 1, IPCZ_NO_FLAGS, nullptr);
...
{
    base::SharedMemoryHooks::SetCreateHooks(&CreateReadOnlySharedMemoryRegion, &CreateUnsafeSharedMemoryRegion, &CreateWritableSharedMemoryRegion);
}

// static
base::WritableSharedMemoryRegion BaseSharedMemoryService::CreateWritableRegion(size_t size)
{
#if BUILDFLAG(IS_POSIX) && !BUILDFLAG(IS_NACL) && !BUILDFLAG(IS_APPLE)
    if (!g_client) {
        return {};
