...
void EncodeHandle(PlatformHandle& handle, const base::Process& remote_process, HandleOwner handle_owner, HANDLE& out_handle)
{
    ...
    BOOL result = ::DuplicateHandle(
        ::GetCurrentProcess(), handle.ReleaseHandle(), remote_process.Handle(), &out_handle, 0, FALSE, DUPLICATE_SAME_ACCESS | DUPLICATE_CLOSE_SOURCE);
    DCHECK(result);
}
...
PlatformHandle DecodeHandle(HANDLE handle, const base::Process& remote_process, Transport::EndpointType sender_type, HandleOwner handle_owner)
{
    ...
    ::DuplicateHandle(remote_process.Handle(), handle, ::GetCurrentProcess(), &local_dupe, 0, FALSE, DUPLICATE_SAME_ACCESS | DUPLICATE_CLOSE_SOURCE);
...
Transport::Transport(EndpointTypes endpoint_types, Channel::Endpoint endpoint, base::Process remote_process)
...
// static
scoped_refptr<Transport> Transport::Create(EndpointTypes endpoint_types, Channel::Endpoint endpoint, base::Process remote_process)
{
    return base::MakeRefCounted<Transport>(endpoint_types, std::move(endpoint), std::move(remote_process));
}

// static
std::pair<scoped_refptr<Transport>, scoped_refptr<Transport>> Transport::CreatePair(EndpointType first_type, EndpointType second_type)
{
    PlatformChannel channel;
    auto one = Create({ .source = first_type, .destination = second_type }, channel.TakeLocalEndpoint());
    auto two = Create({ .source = second_type, .destination = first_type }, channel.TakeRemoteEndpoint());
    return { one, two };
}
...
// static
void Transport::SetIOTaskRunner(scoped_refptr<base::SingleThreadTaskRunner> runner)
{
    GetIOTaskRunnerStorage() = std::move(runner);
}

// static
const scoped_refptr<base::SingleThreadTaskRunner>& Transport::GetIOTaskRunner()
{
...
void Transport::ReportBadActivity(const std::string& error_message)
{
    ...
    const MojoProcessErrorDetails details {
        .struct_size = sizeof(details),
        .error_message_length = base::checked_cast<uint32_t>(error_message.size()),
...
bool Transport::Activate(IpczHandle transport, IpczTransportActivityHandler activity_handler)
{
    ...
    std::vector<PendingTransmission> pending_transmissions;
    {
        ...
        channel_ = Channel::CreateForIpczDriver(this, std::move(inactive_endpoint_), GetIOTaskRunner());
        channel_->Start();
        if (leak_channel_on_shutdown_) {
            GetIOTaskRunner()->PostTask(FROM_HERE, base::BindOnce([](scoped_refptr<Channel> channel) { channel->LeakHandle(); }, channel_));
        }
    ...
    for (auto& transmission : pending_transmissions) {
        channel->Write(Channel::Message::CreateIpczMessage(base::make_span(transmission.bytes), std::move(transmission.handles)));
    }
...
bool Transport::Transmit(base::span<const uint8_t> data, base::span<const IpczDriverHandle> handles)
{
#if BUILDFLAG(IS_WIN)
    ...
    for (IpczDriverHandle handle : handles) {
        auto transmissible_handle = TransmissiblePlatformHandle::TakeFromHandle(handle);
    ...
    channel->Write(Channel::Message::CreateIpczMessage(data, std::move(platform_handles)));
    return true;
}

IpczResult Transport::SerializeObject(ObjectBase& object, void* data, size_t* num_bytes, IpczDriverHandle* handles, size_t* num_handles)
{
    size_t object_num_bytes;
    size_t object_num_handles;
    if (!object.GetSerializedDimensions(*this, object_num_bytes, object_num_handles)) {
...
#if BUILDFLAG(IS_WIN)
    const size_t required_num_bytes = sizeof(ObjectHeader) + object_num_bytes + sizeof(HANDLE) * object_num_handles;
    ...
    if (data_capacity < required_num_bytes || handle_capacity < required_num_handles) {
    ...
    const HandleOwner handle_owner = remote_process_.IsValid() && source_type() == kBroker ? HandleOwner::kRecipient : HandleOwner::kSender;
    header.handle_owner = handle_owner;

    auto handle_data = base::make_span(reinterpret_cast<HANDLE*>(&header + 1), object_num_handles);
    auto object_data = base::make_span(reinterpret_cast<uint8_t*>(&header + 1) + object_num_handles * sizeof(HANDLE), object_num_bytes);
#else
    auto object_data = base::make_span(reinterpret_cast<uint8_t*>(&header + 1), object_num_bytes);
#endif
    ...
    if (!object.Serialize(*this, object_data, base::make_span(platform_handles.container()))) {
        return IPCZ_RESULT_INVALID_ARGUMENT;
    }

    for (size_t i = 0; i < object_num_handles; ++i) {
#if BUILDFLAG(IS_WIN)
        EncodeHandle(platform_handles[i], remote_process_, handle_owner, handle_data[i]);
#else
        handles[i] = TransmissiblePlatformHandle::ReleaseAsHandle(base::MakeRefCounted<TransmissiblePlatformHandle>(std::move(platform_handles[i])));
#endif
    }
    return IPCZ_RESULT_OK;
}

IpczResult Transport::DeserializeObject(base::span<const uint8_t> bytes, base::span<const IpczDriverHandle> handles, scoped_refptr<ObjectBase>& object)
{
    ...
    auto handle_data = base::make_span(reinterpret_cast<const HANDLE*>(bytes.data() + header_size), num_handles);
...
#if BUILDFLAG(IS_WIN)
        platform_handles[i] = DecodeHandle(handle_data[i], remote_process_, destination_type(), handle_owner);
#else
        platform_handles[i] = TransmissiblePlatformHandle::TakeFromHandle(handles[i])->TakeHandle();
#endif
    ...
    case ObjectBase::kTransmissiblePlatformHandle:
        object = TransmissiblePlatformHandle::Deserialize(object_data, object_handles);
        break;
...
bool Transport::GetSerializedDimensions(Transport& transmitter, size_t& num_bytes, size_t& num_handles)
{
...
bool Transport::Serialize(Transport& transmitter, base::span<uint8_t> data, base::span<PlatformHandle> handles)
{
    ...
    if (ShouldSerializeProcessHandle(transmitter)) {
        ...
        handles[1] = PlatformHandle(base::win::ScopedHandle(remote_process_.Duplicate().Release()));
    } else {
    ...
    handles[0] = absl::get<PlatformChannelEndpoint>(inactive_endpoint_).TakePlatformHandle();
    return true;
}

// static
scoped_refptr<Transport> Transport::Deserialize(Transport& from_transport, base::span<const uint8_t> data, base::span<PlatformHandle> handles)
{
...
#endif
    if (header.is_same_remote_process && from_transport.remote_process().IsValid()) {
        process = from_transport.remote_process().Duplicate();
    }
    return Create(
        { .source = from_transport.source_type(), .destination = header.destination_type }, PlatformChannelEndpoint(std::move(handles[0])), std::move(process));
}
...
void Transport::OnChannelMessage(const void* payload, size_t payload_size, std::vector<PlatformHandle> handles)
{
    std::vector<IpczDriverHandle> driver_handles(handles.size());
    for (size_t i = 0; i < handles.size(); ++i) {
        driver_handles[i] = TransmissiblePlatformHandle::ReleaseAsHandle(base::MakeRefCounted<TransmissiblePlatformHandle>(std::move(handles[i])));
    }

    const IpczResult result = activity_handler_(
        ipcz_transport_, static_cast<const uint8_t*>(payload), payload_size, driver_handles.data(), driver_handles.size(), IPCZ_NO_FLAGS, nullptr);
...
{
    activity_handler_(ipcz_transport_, nullptr, 0, nullptr, 0, IPCZ_TRANSPORT_ACTIVITY_ERROR, nullptr);
}

void Transport::OnChannelDestroyed()
{
    activity_handler_(ipcz_transport_, nullptr, 0, nullptr, 0, IPCZ_TRANSPORT_ACTIVITY_DEACTIVATED, nullptr);
...
bool Transport::IsEndpointValid() const
{
    return absl::visit(base::Overloaded {
                           [](const PlatformChannelEndpoint& endpoint) { return endpoint.is_valid(); },
                           [](const PlatformChannelServerEndpoint& endpoint) { return endpoint.is_valid(); },
                       },
        inactive_endpoint_);
}
...
bool Transport::CanTransmitHandles() const
{
    ...
    return remote_process_.IsValid() || destination_type() == kBroker || source_type() == kBroker;
#else
    return true;
#endif
}
...
{
#if BUILDFLAG(IS_WIN)
    return remote_process_.IsValid() && !remote_process_.is_current() && transmitter.destination_type() == kBroker;
#else
...
Transport::PendingTransmission::PendingTransmission(PendingTransmission&&) = default;

Transport::PendingTransmission& Transport::PendingTransmission::operator=(PendingTransmission&&) = default;
