...
public:
    ...
    MojoResult AddTrigger(MojoHandle handle, MojoHandleSignals signals, MojoTriggerCondition condition, uintptr_t trigger_context);
...
private:
    ...
    IpczResult ArmTrigger(Trigger& trigger, IpczTrapConditionFlags* satisfied_flags, IpczPortalStatus* status);

    void MaybeEnqueueTriggerRemoval(Trigger& trigger) EXCLUSIVE_LOCKS_REQUIRED(lock_);
