...
class MojoIpczInProcessTestNodeController : public ipcz::test::TestNode::TestNodeController {
public:
    class NodeThreadDelegate : public base::DelegateSimpleThread::Delegate {
    public:
        NodeThreadDelegate(std::unique_ptr<ipcz::test::TestNode> node, ipcz::test::TestDriver* driver)
            : node_(std::move(node))
            , driver_(driver)
        {
        }
    ...
    MojoIpczInProcessTestNodeController(const std::string& node_name, std::unique_ptr<ipcz::test::TestNode> test_node, ipcz::test::TestDriver* test_driver)
...
class MojoIpczChildTestNodeController : public ipcz::test::TestNode::TestNodeController {
public:
    ...
    bool WaitForShutdown() override
    {
        ...
        int rv = -1;
        base::WaitForMultiprocessTestChildExit(process_, TestTimeouts::action_timeout(), &rv);
        process_.Close();
        result_ = (rv == 0);
        return *result_;
    }
...
private:
    ~MojoIpczChildTestNodeController() override
    {
...
public:
    ...
    explicit MojoIpczTestDriver(Mode mode)
        : mode_(mode)
    {
    }
    ...
    ipcz::test::TestNode::TransportPair CreateTransports(ipcz::test::TestNode& source, bool for_broker_target) const override
    {
        std::pair<scoped_refptr<Transport>, scoped_refptr<Transport>> transports;
        transports = Transport::CreatePair(Transport::kBroker, for_broker_target ? Transport::kBroker : Transport::kNonBroker);
        return {
    ...
    ipcz::Ref<ipcz::test::TestNode::TestNodeController> SpawnTestNode(
        ipcz::test::TestNode& source, const ipcz::test::TestNodeDetails& details, IpczDriverHandle our_transport, IpczDriverHandle their_transport) override
    {
        if (mode_ == kInProcess) {
            return SpawnTestNodeThread(source, details, our_transport, their_transport);
        }
        return SpawnTestNodeProcess(source, details, our_transport, their_transport);
    }
    ...
    IpczDriverHandle GetClientTestNodeTransport() override
    {
        ...
        PlatformChannelEndpoint endpoint = PlatformChannelEndpoint::RecoverFromString(command_line.GetSwitchValueASCII(PlatformChannel::kHandleSwitch));
        ...
        const std::string parent_handle_switch = command_line.GetSwitchValueASCII(kParentHandle);
        int parent_handle_value;
        if (!parent_handle_switch.empty() && base::StringToInt(parent_handle_switch, &parent_handle_value)) {
            parent_process = base::Process(LongToHandle(parent_handle_value));
        }
#endif // BUILDFLAG(IS_WIN)
        return Transport::ReleaseAsHandle(
            Transport::Create({ .source = parent_process.IsValid() ? Transport::kBroker : Transport::kNonBroker, .destination = Transport::kBroker },
                std::move(endpoint), std::move(parent_process)));
    }

private:
    ipcz::Ref<ipcz::test::TestNode::TestNodeController> SpawnTestNodeThread(
        ipcz::test::TestNode& source, const ipcz::test::TestNodeDetails& details, IpczDriverHandle our_transport, IpczDriverHandle their_transport)
    {
        Transport::FromHandle(our_transport)->set_remote_process(base::Process::Current());
        if (details.is_broker) {
            Transport::FromHandle(their_transport)->set_remote_process(base::Process::Current());
        }
        std::unique_ptr<ipcz::test::TestNode> node = details.factory();
        node->SetTransport(their_transport);
        return ipcz::MakeRefCounted<MojoIpczInProcessTestNodeController>(std::string(details.name.begin(), details.name.end()), std::move(node), this);
    }

    ipcz::Ref<ipcz::test::TestNode::TestNodeController> SpawnTestNodeProcess(
        ipcz::test::TestNode& source, const ipcz::test::TestNodeDetails& details, IpczDriverHandle our_transport, IpczDriverHandle their_transport)
    {
        const std::string test_child_main = base::StrCat({ details.name.data(), "/", kMojoIpczMultiprocessTestDriverName });
        base::CommandLine command_line(base::GetMultiProcessTestChildBaseCommandLine().GetProgram());
        ...
        for (const auto& entry : base::CommandLine::ForCurrentProcess()->GetSwitches()) {
            if (uninherited_args.find(entry.first) == uninherited_args.end())
                command_line.AppendSwitchNative(entry.first, entry.second);
        }

        base::LaunchOptions options;
        scoped_refptr<Transport> transport = Transport::TakeFromHandle(their_transport);
        ...
        if (details.is_broker) {
            ...
            HANDLE dupe;
            BOOL ok = ::DuplicateHandle(::GetCurrentProcess(), ::GetCurrentProcess(), ::GetCurrentProcess(), &dupe, 0, TRUE, DUPLICATE_SAME_ACCESS);
            CHECK(ok);
            this_process = base::Process(dupe);

            options.handles_to_inherit.push_back(this_process.Handle());
            command_line.AppendSwitchASCII(kParentHandle, base::NumberToString(HandleToLong(this_process.Handle())));
        }
#endif

        base::Process child = base::SpawnMultiProcessTestChild(test_child_main, command_line, options);
        endpoint.ProcessLaunchAttempted();
        Transport::FromHandle(our_transport)->set_remote_process(child.Duplicate());
        return ipcz::MakeRefCounted<MojoIpczChildTestNodeController>(std::move(child));
    }

    const Mode mode_;
};

ipcz::test::TestDriverRegistration<MojoIpczTestDriver> kRegisterInProcessDriver { MojoIpczTestDriver::kInProcess };

#if !BUILDFLAG(IS_IOS)
ipcz::test::TestDriverRegistration<MojoIpczTestDriver> kRegisterMultiprocessDriver { MojoIpczTestDriver::kMultiprocess };
#endif
