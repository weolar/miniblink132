...
Watch::Watch(const scoped_refptr<WatcherDispatcher>& watcher, const scoped_refptr<Dispatcher>& dispatcher, uintptr_t context, MojoHandleSignals signals,
...
bool Watch::NotifyState(const HandleSignalsState& state, bool allowed_to_call_callback)
{
    ...
    const bool notify_success = (state.satisfies_any(signals_) && condition_ == MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED)
        || (!state.satisfies_all(signals_) && condition_ == MOJO_TRIGGER_CONDITION_SIGNALS_UNSATISFIED);
    if (notify_success) {
        rv = MOJO_RESULT_OK;
        if (allowed_to_call_callback && rv != last_known_result_) {
            request_context->AddWatchNotifyFinalizer(this, MOJO_RESULT_OK, state);
        }
    } else if (condition_ == MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED && !state.can_satisfy_any(signals_)) {
        rv = MOJO_RESULT_FAILED_PRECONDITION;
        if (allowed_to_call_callback && rv != last_known_result_) {
            request_context->AddWatchNotifyFinalizer(this, MOJO_RESULT_FAILED_PRECONDITION, state);
        }
    }

    last_known_signals_state_ = *static_cast<const MojoHandleSignalsState*>(&state);
...
void Watch::InvokeCallback(MojoResult result, const HandleSignalsState& state, MojoTrapEventFlags flags)
{
