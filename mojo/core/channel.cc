...
static_assert(IsAlignedForChannelMessage(sizeof(Channel::Message::LegacyHeader)), "Invalid LegacyHeader size.");

static_assert(IsAlignedForChannelMessage(sizeof(Channel::Message::Header)), "Invalid Header size.");

static_assert(sizeof(Channel::Message::LegacyHeader) == 8, "LegacyHeader must be 8 bytes on ChromeOS and Android");

static_assert(offsetof(Channel::Message::LegacyHeader, num_bytes) == offsetof(Channel::Message::Header, num_bytes),
    "num_bytes should be at the same offset in both Header structs.");
static_assert(offsetof(Channel::Message::LegacyHeader, message_type) == offsetof(Channel::Message::Header, message_type),
...
struct IpczMessage : public Channel::Message {
    IpczMessage(base::span<const uint8_t> data, std::vector<PlatformHandle> handles)
...
struct ComplexMessage : public Channel::Message {
    ...
    ComplexMessage(size_t capacity, size_t max_handles, size_t payload_size, MessageType message_type);
...
struct TrivialMessage : public Channel::Message {
    ...
    static Channel::MessagePtr TryConstruct(size_t payload_size, MessageType message_type);
...
static_assert(sizeof(TrivialMessage) == 256, "Expected TrivialMessage to be 256 bytes");

} // namespace

// static
Channel::MessagePtr Channel::Message::CreateMessage(size_t payload_size, size_t max_handles)
{
    return CreateMessage(payload_size, payload_size, max_handles);
}

// static
Channel::MessagePtr Channel::Message::CreateMessage(size_t payload_size, size_t max_handles, MessageType message_type)
{
    return CreateMessage(payload_size, payload_size, max_handles, message_type);
}

// static
Channel::MessagePtr Channel::Message::CreateMessage(size_t capacity, size_t payload_size, size_t max_handles)
{
#if defined(MOJO_CORE_LEGACY_PROTOCOL)
    return CreateMessage(capacity, payload_size, max_handles, Message::MessageType::NORMAL_LEGACY);
#else
    return CreateMessage(capacity, payload_size, max_handles, Message::MessageType::NORMAL);
#endif
}

// static
Channel::MessagePtr Channel::Message::CreateMessage(size_t capacity, size_t payload_size, size_t max_handles, MessageType message_type)
{
    if (g_use_trivial_messages && (capacity + std::max(sizeof(Header), sizeof(LegacyHeader))) <= TrivialMessage::kInternalCapacity && max_handles == 0) {
    ...
    return base::WrapUnique<Channel::Message>(new ComplexMessage(capacity, payload_size, max_handles, message_type));
}

// static
Channel::MessagePtr Channel::Message::CreateIpczMessage(base::span<const uint8_t> data, std::vector<PlatformHandle> handles)
{
...
// static
Channel::MessagePtr Channel::Message::CreateRawForFuzzing(base::span<const unsigned char> data)
{
...
// static
Channel::MessagePtr Channel::Message::Deserialize(const void* data, size_t data_num_bytes, HandlePolicy handle_policy, base::ProcessHandle from_process)
{
    if (data_num_bytes < sizeof(LegacyHeader))
        return nullptr;

    const LegacyHeader* legacy_header = reinterpret_cast<const LegacyHeader*>(data);
    if (legacy_header->num_bytes != data_num_bytes) {
        DLOG(ERROR) << "Decoding invalid message: " << legacy_header->num_bytes << " != " << data_num_bytes;
        return nullptr;
    }
    ...
    if (!header) {
        ...
        if (header->num_bytes < header->num_header_bytes || header->num_header_bytes < sizeof(Header)) {
            DLOG(ERROR) << "Decoding invalid message: " << header->num_bytes << " < " << header->num_header_bytes;
            return nullptr;
        }
    ...
    if (extra_header_size > 0 && extra_header_size < sizeof(MachPortsExtraHeader)) {
        DLOG(ERROR) << "Decoding invalid message: " << extra_header_size << " < " << sizeof(MachPortsExtraHeader);
        return nullptr;
    }
    uint32_t max_handles = extra_header_size == 0 ? 0 : (extra_header_size - sizeof(MachPortsExtraHeader)) / sizeof(MachPortsEntry);
#else
    ...
    const uint16_t num_handles = header ? header->num_handles : legacy_header->num_handles;
    if (num_handles > max_handles || max_handles > kMaxAttachedHandles) {
        DLOG(ERROR) << "Decoding invalid message: " << num_handles << " > " << max_handles;
        return nullptr;
    }
    ...
    MessagePtr message = CreateMessage(payload_size, max_handles, legacy_header->message_type);
    ...
    if (header) {
        ...
        if (message->extra_header_size()) {
            ...
            memcpy(message->mutable_extra_header(), static_cast<const char*>(data) + sizeof(Header), message->extra_header_size());
        }
    ...
    for (size_t i = 0; i < num_handles; i++) {
        HANDLE handle = base::win::Uint32ToHandle(static_cast<ComplexMessage*>(message.get())->handles_[i].handle);
        if (PlatformHandleInTransit::IsPseudoHandle(handle))
            return nullptr;
        if (from_process == base::kNullProcessHandle) {
            handles[i] = PlatformHandleInTransit(PlatformHandle(base::win::ScopedHandle(handle)));
        } else {
            handles[i] = PlatformHandleInTransit(PlatformHandleInTransit::TakeIncomingRemoteHandle(handle, from_process));
        }
    }
...
// static
void Channel::Message::ExtendPayload(MessagePtr& message, size_t new_payload_size)
{
    ...
    auto m = base::WrapUnique<Channel::Message>(new ComplexMessage(new_payload_size, new_payload_size, 0, message->legacy_header()->message_type));
...
void* Channel::Message::mutable_payload()
{
    ...
    return reinterpret_cast<uint8_t*>(mutable_data()) + header()->num_header_bytes;
}
...
{
    return is_legacy_message() ? legacy_header()->num_handles : header()->num_handles;
}

bool Channel::Message::has_handles() const
{
    return (is_legacy_message() ? legacy_header()->num_handles : header()->num_handles) > 0;
}
...
ComplexMessage::ComplexMessage(size_t capacity, size_t payload_size, size_t max_handles, MessageType message_type)
    : max_handles_(max_handles)
{
    ...
    if (max_handles) {
        extra_header_size = sizeof(MachPortsExtraHeader) + (max_handles * sizeof(MachPortsEntry));
    }
#endif
    // Pad extra header data to be aliged to |kChannelMessageAlignment| bytes.
    if (!IsAlignedForChannelMessage(extra_header_size)) {
        extra_header_size += kChannelMessageAlignment - (extra_header_size % kChannelMessageAlignment);
    }
    DCHECK(IsAlignedForChannelMessage(extra_header_size));
    const size_t header_size = is_legacy_message ? sizeof(LegacyHeader) : sizeof(Header);
    ...
    DCHECK(base::IsValueInRangeForNumericType<uint16_t>(header_size + extra_header_size));
    legacy_header()->message_type = message_type;

    if (is_legacy_message) {
        legacy_header()->num_handles = static_cast<uint16_t>(max_handles);
    } else {
        header()->num_header_bytes = static_cast<uint16_t>(header_size + extra_header_size);
    }
...
#elif BUILDFLAG(IS_MAC)
        mach_ports_header_ = reinterpret_cast<MachPortsExtraHeader*>(mutable_extra_header());
...
bool ComplexMessage::ExtendPayload(size_t new_payload_size)
{
    ...
    if (new_payload_size > capacity_without_header) {
        size_t new_capacity = std::max(static_cast<size_t>(capacity_without_header * kGrowthFactor), new_payload_size) + header_size;
...
void ComplexMessage::SetHandles(std::vector<PlatformHandleInTransit> new_handles)
{
    if (is_legacy_message()) {
    ...
    if (mach_ports_header_) {
        ...
        for (size_t i = 0; i < handle_vector_.size(); i++) {
            mach_ports_header_->entries[i].type = static_cast<uint8_t>(handle_vector_[i].handle().type());
        }
...
// static
Channel::MessagePtr TrivialMessage::TryConstruct(size_t payload_size, MessageType message_type)
{
    const bool is_legacy_message = (message_type == MessageType::NORMAL_LEGACY);
    const size_t header_size = is_legacy_message ? sizeof(LegacyHeader) : sizeof(Header);
...
void TrivialMessage::SetHandles(std::vector<PlatformHandleInTransit> new_handles)
{
    CHECK(new_handles.empty());
}
...
public:
    ...
    char* Reserve(size_t num_bytes)
    {
        if (num_occupied_bytes_ + num_bytes > size_) {
            size_ = std::max(static_cast<size_t>(size_ * kGrowthFactor), num_occupied_bytes_ + num_bytes);
    ...
    void Discard(size_t num_bytes)
    {
        ...
        if (num_discarded_bytes_ > kMaxUnusedReadBufferCapacity) {
            ...
            AlignedBuffer new_data = MakeAlignedBuffer(size_);
            memcpy(new_data.get(), data_.get() + num_discarded_bytes_, num_preserved_bytes);
            data_ = std::move(new_data);
            num_discarded_bytes_ = 0;
...
Channel::Channel(Delegate* delegate, HandlePolicy handle_policy, DispatchBufferPolicy buffer_policy)
    : is_for_ipcz_(delegate ? delegate->IsIpczTransport() : false)
    , delegate_(delegate)
    , handle_policy_(handle_policy)
    , read_buffer_(buffer_policy == DispatchBufferPolicy::kManaged ? new ReadBuffer : nullptr)
{
}
...
// static
scoped_refptr<Channel> Channel::CreateForIpczDriver(Delegate* delegate, Endpoint endpoint, scoped_refptr<base::SingleThreadTaskRunner> io_task_runner)
{
#if BUILDFLAG(IS_NACL)
    return nullptr;
#else
    ConnectionParams params = absl::visit(base::Overloaded {
                                              [](PlatformChannelEndpoint& endpoint) { return ConnectionParams(std::move(endpoint)); },
                                              [](PlatformChannelServerEndpoint& endpoint) { return ConnectionParams(std::move(endpoint)); },
                                          },
        endpoint);
    return Create(delegate, std::move(params), HandlePolicy::kAcceptHandles, std::move(io_task_runner));
#endif
}
...
bool Channel::OnReadComplete(size_t bytes_read, size_t* next_read_size_hint)
{
    ...
    *next_read_size_hint = kReadBufferSize;
    ...
    const size_t header_size = is_for_ipcz_ ? sizeof(Message::IpczHeader) : sizeof(Message::LegacyHeader);
    while (read_buffer_->num_occupied_bytes() >= header_size) {
        ...
        if (!IsAlignedForChannelMessage(reinterpret_cast<uintptr_t>(read_buffer_->occupied_bytes()))) {
            read_buffer_->Realign();
        }

        DispatchResult result = TryDispatchMessage(base::make_span(read_buffer_->occupied_bytes(), read_buffer_->num_occupied_bytes()), next_read_size_hint);
...
Channel::DispatchResult Channel::TryDispatchMessage(base::span<const char> buffer, size_t* size_hint)
{
    TRACE_EVENT(TRACE_DISABLED_BY_DEFAULT("toplevel.ipc"), "Mojo dispatch message");
    if (is_for_ipcz_) {
        // This has already been validated.
        DCHECK_GE(buffer.size(), sizeof(Message::IpczHeader));

        const auto& header = *reinterpret_cast<const Message::IpczHeader*>(buffer.data());
        ...
        if (num_handles > 0) {
            if (handle_policy_ == HandlePolicy::kRejectHandles || !GetReadPlatformHandlesForIpcz(num_handles, handles)) {
                return DispatchResult::kError;
            }
            if (handles.empty()) {
    ...
    const Message::LegacyHeader* legacy_header = reinterpret_cast<const Message::LegacyHeader*>(buffer.data());
    ...
    if (header) {
        if (header->num_header_bytes < sizeof(Message::Header) || header->num_header_bytes > header->num_bytes) {
        ...
        payload = payload_size ? reinterpret_cast<Message::Header*>(const_cast<char*>(buffer.data()) + header->num_header_bytes) : nullptr;
    } else {
        payload_size = legacy_header->num_bytes - sizeof(Message::LegacyHeader);
        payload = payload_size ? const_cast<Message::LegacyHeader*>(&legacy_header[1]) : nullptr;
    }
    ...
    if (num_handles > 0) {
        ...
        if (!GetReadPlatformHandles(payload, payload_size, num_handles, extra_header, extra_header_size, &handles, &deferred)) {
            return DispatchResult::kError;
        }
    ...
    if (legacy_header->message_type != Message::MessageType::NORMAL_LEGACY && legacy_header->message_type != Message::MessageType::NORMAL) {
        DCHECK(!deferred);
        if (!OnControlMessage(legacy_header->message_type, payload, payload_size, std::move(handles))) {
            return DispatchResult::kError;
        }
...
bool Channel::OnControlMessage(Message::MessageType message_type, const void* payload, size_t payload_size, std::vector<PlatformHandle> handles)
{
    return false;
}

// Currently only Non-nacl CrOs, Linux, and Android support upgrades.
#if BUILDFLAG(IS_NACL) || (!(BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_ANDROID)))
// static
