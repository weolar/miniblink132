...
class MOJO_SYSTEM_IMPL_EXPORT Channel : public base::RefCountedThreadSafe<Channel> {
public:
...
#pragma pack(push, 1)
        ...
        static_assert(sizeof(MachPortsEntry) == 1, "sizeof(MachPortsEntry) must be 1 byte");
        ...
        static_assert(sizeof(MachPortsExtraHeader) == 2, "sizeof(MachPortsExtraHeader) must be 2 bytes");
        ...
        static_assert(sizeof(HandleEntry) == 4, "sizeof(HandleEntry) must be 4 bytes");
#endif
#pragma pack(pop)
        ...
        static MessagePtr CreateMessage(size_t payload_size, size_t max_handles, MessageType message_type);
        static MessagePtr CreateMessage(size_t capacity, size_t payload_size, size_t max_handles);
        static MessagePtr CreateMessage(size_t capacity, size_t max_handles, size_t payload_size, MessageType message_type);

        static MessagePtr CreateIpczMessage(base::span<const uint8_t> data, std::vector<PlatformHandle> handles);
        ...
        static MessagePtr Deserialize(
            const void* data, size_t data_num_bytes, HandlePolicy handle_policy, base::ProcessHandle from_process = base::kNullProcessHandle);
        ...
        virtual void SetHandles(std::vector<PlatformHandleInTransit> new_handles) = 0;
    ...
    public:
        ...
        virtual void OnChannelMessage(const void* payload, size_t payload_size, std::vector<PlatformHandle> handles) = 0;
    ...
    // care of that.
    using Endpoint = absl::variant<PlatformChannelEndpoint, PlatformChannelServerEndpoint>;
    static scoped_refptr<Channel> CreateForIpczDriver(Delegate* delegate, Endpoint endpoint, scoped_refptr<base::SingleThreadTaskRunner> io_task_runner);
    ...
    void set_handle_policy(HandlePolicy policy)
    {
        handle_policy_ = policy;
    }
    ...
    const base::Process& remote_process() const
    {
...
protected:
    ...
    // the implementation.
    Channel(Delegate* delegate, HandlePolicy handle_policy, DispatchBufferPolicy buffer_policy = DispatchBufferPolicy::kManaged);
    virtual ~Channel();
    ...
    HandlePolicy handle_policy() const
    {
        return handle_policy_;
    }
    ...
    DispatchResult TryDispatchMessage(base::span<const char> buffer, size_t* size_hint);
    ...
    virtual bool GetReadPlatformHandles(const void* payload, size_t payload_size, size_t num_handles, const void* extra_header, size_t extra_header_size,
    ...
    // this returns false.
    virtual bool GetReadPlatformHandlesForIpcz(size_t num_handles, std::vector<PlatformHandle>& handles) = 0;

    // Handles a received control message. Returns |true| if the message is
    // accepted, or |false| otherwise.
    virtual bool OnControlMessage(Message::MessageType message_type, const void* payload, size_t payload_size, std::vector<PlatformHandle> handles);
