...
base::LazyInstance<base::ThreadLocalPointer<RequestContext>>::Leaky g_current_context = LAZY_INSTANCE_INITIALIZER;

} // namespace

RequestContext::RequestContext()
    : RequestContext(Source::LOCAL_API_CALL)
{
}

RequestContext::RequestContext(Source source)
    : source_(source)
    , tls_context_(g_current_context.Pointer())
{
...
RequestContext::~RequestContext()
{
    if (IsCurrent()) {
        ...
        for (const scoped_refptr<Watch>& watch : watch_cancel_finalizers_.container()) {
...
void RequestContext::AddWatchNotifyFinalizer(scoped_refptr<Watch> watch, MojoResult result, const HandleSignalsState& state)
{
    DCHECK(IsCurrent());
    watch_notify_finalizers_->push_back(WatchNotifyFinalizer(std::move(watch), result, state));
}
...
RequestContext::WatchNotifyFinalizer::WatchNotifyFinalizer(scoped_refptr<Watch> watch, MojoResult result, const HandleSignalsState& state)
...
RequestContext::WatchNotifyFinalizer::WatchNotifyFinalizer(const WatchNotifyFinalizer& other) = default;
