...
std::string ReadMessageAsString(mojo::MessagePipeHandle handle)
{
    ...
    CHECK_EQ(MOJO_RESULT_OK, mojo::ReadMessageRaw(handle, &data, nullptr, MOJO_READ_MESSAGE_FLAG_NONE));
...
TEST(MojoCoreTest, SanityCheck)
{
    ...
    EXPECT_EQ(MOJO_RESULT_OK, MojoSetMessageContext(m, 42, nullptr, nullptr, nullptr));
...
class MojoCoreMultiprocessTest : public ::testing::TestWithParam<std::tuple<InitializationMode, LoadPathSpec>> {
public:
...
TEST_P(MojoCoreMultiprocessTest, BasicMultiprocess)
{
    ...
    base::Process child_process = base::SpawnMultiProcessTestChild("BasicMultiprocessClientMain", child_cmd, options);
    ...
    mojo::WriteMessageRaw(child_pipe.get(), kTestReply, sizeof(kTestReply) - 1, nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE);

    int rv = -1;
    ASSERT_TRUE(base::WaitForMultiprocessTestChildExit(child_process, TestTimeouts::action_timeout(), &rv));
    EXPECT_EQ(0, rv);
}

MULTIPROCESS_TEST_MAIN(BasicMultiprocessClientMain)
{
    auto endpoint = mojo::PlatformChannel::RecoverPassedEndpointFromCommandLine(*base::CommandLine::ForCurrentProcess());
    auto invitation = mojo::IncomingInvitation::Accept(std::move(endpoint));
    auto parent_pipe = invitation.ExtractMessagePipe(kTestPipeName);

    mojo::WriteMessageRaw(parent_pipe.get(), kTestMessage, sizeof(kTestMessage) - 1, nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE);
...
INSTANTIATE_TEST_SUITE_P(, MojoCoreMultiprocessTest,
    ::testing::Combine(::testing::Values(InitializationMode::kCombinedLoadAndInit, InitializationMode::kLoadBeforeInit),
