...
// static
scoped_refptr<PlatformHandleDispatcher> PlatformHandleDispatcher::Create(PlatformHandle platform_handle)
{
...
void PlatformHandleDispatcher::StartSerialize(uint32_t* num_bytes, uint32_t* num_ports, uint32_t* num_handles)
{
    *num_bytes = 0;
    *num_ports = 0;
    *num_handles = 1;
...
bool PlatformHandleDispatcher::EndSerialize(void* destination, ports::PortName* ports, PlatformHandle* handles)
{
    base::AutoLock lock(lock_);
    if (is_closed_)
        return false;
...
scoped_refptr<PlatformHandleDispatcher> PlatformHandleDispatcher::Deserialize(
    const void* bytes, size_t num_bytes, const ports::PortName* ports, size_t num_ports, PlatformHandle* handles, size_t num_handles)
{
...
PlatformHandleDispatcher::PlatformHandleDispatcher(PlatformHandle platform_handle)
