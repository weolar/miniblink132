...
ScopedMessage NewUserMessageEvent(const base::StringPiece& payload, size_t num_ports)
{
...
public:
    ...
    virtual void ForwardEvent(TestNode* from_node, const NodeName& node_name, ScopedEvent event) = 0;
...
public:
    explicit TestNode(uint64_t id)
        ...
        , events_available_event_(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED)
        , idle_event_(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::SIGNALED)
    {
    }
    ...
    bool IsIdle()
    {
        ...
        return started_ && !dispatching_ && (incoming_events_.empty() || (block_on_event_ && blocked_));
    }
    ...
    void Start(MessageRouter* router)
    {
        router_ = router;
        node_thread_.Start();
        node_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&TestNode::ProcessEvents, base::Unretained(this)));
    }
    ...
    int SendStringMessageWithPort(const PortRef& port, const std::string& s, const PortName& sent_port_name)
    {
    ...
    int SendStringMessageWithPort(const PortRef& port, const std::string& s, const PortRef& sent_port)
    {
    ...
    {
        {
            base::AutoLock lock(lock_);
            if (drop_messages_) {
                DVLOG(1) << "Dropping ForwardMessage from node " << node_name_ << " to " << node_name;
    ...
    void ProcessEvents()
    {
        for (;;) {
            ...
            while (!incoming_events_.empty()) {
                if (block_on_event_ && incoming_events_.front().second->type() == blocked_event_type_) {
                    blocked_ = true;
                ...
                base::AutoUnlock unlock(lock_);
                node_.AcceptEvent(node_event_pair.first, std::move(node_event_pair.second));
            }
...
public:
    ...
    void CreatePortPair(TestNode* node0, PortRef* port0, TestNode* node1, PortRef* port1)
    {
        if (node0 == node1) {
            ...
            EXPECT_EQ(OK, node0->node().InitializePort(*port0, node1->name(), port1->name(), node1->name(), port1->name()));
            EXPECT_EQ(OK, node1->node().InitializePort(*port1, node0->name(), port0->name(), node0->name(), port0->name()));
        }
    }

private:
    // MessageRouter:
    void ForwardEvent(TestNode* from_node, const NodeName& node_name, ScopedEvent event) override
    {
        base::AutoLock global_lock(global_lock_);
        base::AutoLock lock(lock_);
        ...
        if (event->type() == Event::Type::kUserMessage) {
            ...
            message_copy->AttachMessage(std::make_unique<TestMessage>(message_event->GetMessage<TestMessage>()->payload()));
        }
...
TEST_F(PortsTest, LostConnectionToNode2)
{
    ...
    ScopedMessage message;
    EXPECT_EQ(ERROR_PORT_PEER_CLOSED, node0.node().GetMessage(a0, &message, nullptr));
...
TEST_F(PortsTest, GetMessage1)
{
    ...
    EXPECT_EQ(ERROR_PORT_PEER_CLOSED, node.node().GetMessage(a0, &message, nullptr));
    EXPECT_FALSE(message);
...
TEST_F(PortsTest, SendFailure)
{
    ...
    EXPECT_EQ(ERROR_PORT_CANNOT_SEND_SELF, node.SendStringMessageWithPort(A, "oops", A));

    // Try to send B over A.

    EXPECT_EQ(ERROR_PORT_CANNOT_SEND_PEER, node.SendStringMessageWithPort(A, "nope", B));
...
TEST_F(PortsTest, AllowShutdownWithLocalPortsOpen)
{
    ...
    EXPECT_TRUE(node.node().CanShutdownCleanly(Node::ShutdownPolicy::ALLOW_LOCAL_PORTS));
...
TEST_F(PortsTest, MergePorts)
{
    ...
    EXPECT_TRUE(node0.node().CanShutdownCleanly(Node::ShutdownPolicy::ALLOW_LOCAL_PORTS));
    EXPECT_TRUE(node1.node().CanShutdownCleanly(Node::ShutdownPolicy::ALLOW_LOCAL_PORTS));
...
TEST_F(PortsTest, MergePortsFailsGracefully)
{
    ...
    EXPECT_EQ(OK, node0.node().InitializePort(X, node1.name(), Y.name(), node1.name(), Y.name()));
    EXPECT_EQ(OK, node1.node().InitializePort(Y, node0.name(), X.name(), node0.name(), X.name()));
