...
base::LazyInstance<RandomNameGenerator>::Leaky g_name_generator = LAZY_INSTANCE_INITIALIZER;
...
bool Node::CanShutdownCleanly(ShutdownPolicy policy)
{
    ...
    if (policy == ShutdownPolicy::DONT_ALLOW_LOCAL_PORTS) {
        ...
        for (auto& entry : ports_) {
            DVLOG(2) << "Port " << entry.first << " referencing node " << entry.second->peer_node_name << " is blocking shutdown of "
    ...
    for (auto& entry : ports_) {
        ...
        if (port->peer_node_name != name_ && port->state != Port::kReceiving) {
            can_shutdown = false;
#if DCHECK_IS_ON()
            DVLOG(2) << "Port " << entry.first << " referencing node " << port->peer_node_name << " is blocking shutdown of "
...
int Node::InitializePort(
    const PortRef& port_ref, const NodeName& peer_node_name, const PortName& peer_port_name, const NodeName& prev_node_name, const PortName& prev_port_name)
{
    {
        ...
        UpdatePortPeerAddress(port_ref.name(), port, peer_node_name, peer_port_name);
...
int Node::CreatePortPair(PortRef* port0_ref, PortRef* port1_ref)
{
    ...
    rv = InitializePort(*port0_ref, name_, port1_ref->name(), name_, port1_ref->name());
    if (rv != OK)
        return rv;

    rv = InitializePort(*port1_ref, name_, port0_ref->name(), name_, port0_ref->name());
    if (rv != OK)
        return rv;
...
int Node::SetUserData(const PortRef& port_ref, scoped_refptr<UserData> user_data)
{
    SinglePortLocker locker(&port_ref);
    auto* port = locker.port();
    if (port->state == Port::kClosed)
...
int Node::GetUserData(const PortRef& port_ref, scoped_refptr<UserData>* user_data)
{
    SinglePortLocker locker(&port_ref);
    auto* port = locker.port();
    if (port->state == Port::kClosed)
...
int Node::ClosePort(const PortRef& port_ref)
{
    ...
    if (was_initialized) {
        DVLOG(2) << "Sending ObserveClosure from " << port_ref.name() << "@" << name_ << " to " << peer_port_name << "@" << peer_node_name;
        delegate_->ForwardEvent(peer_node_name, std::make_unique<ObserveClosureEvent>(peer_port_name, port_ref.name(), sequence_num, last_sequence_num));
...
int Node::GetStatus(const PortRef& port_ref, PortStatus* port_status)
{
    ...
    port_status->queued_message_count = port->message_queue.queued_message_count();
    port_status->queued_num_bytes = port->message_queue.queued_num_bytes();
    port_status->unacknowledged_message_count = port->next_sequence_num_to_send - port->last_sequence_num_acknowledged - 1;

    return OK;
}

int Node::GetMessage(const PortRef& port_ref, std::unique_ptr<UserMessageEvent>* message, MessageFilter* filter)
{
    *message = nullptr;
    ...
    ScopedEvent ack_event;
    {
        ...
        if (*message && (*message)->sequence_num() == port->sequence_num_to_acknowledge) {
            peer_node_name = port->peer_node_name;
            ack_event = std::make_unique<UserMessageReadAckEvent>(
                port->peer_port_name, port_ref.name(), port->next_control_sequence_num_to_send++, port->sequence_num_to_acknowledge);
        }
        if (*message) {
    ...
    if (*message) {
        for (size_t i = 0; i < (*message)->num_ports(); ++i) {
            ...
            DCHECK_EQ(OK, rv) << "Port " << new_port_ref.name() << "@" << name_ << " does not exist!";
...
int Node::SendUserMessage(const PortRef& port_ref, std::unique_ptr<UserMessageEvent> message)
{
...
int Node::SetAcknowledgeRequestInterval(const PortRef& port_ref, uint64_t sequence_num_acknowledge_interval)
{
    NodeName peer_node_name;
    PortName peer_port_name;
    ...
    uint64_t sequence_num = 0;
    {
        ...
        sequence_num_to_request_ack = port->last_sequence_num_acknowledged + sequence_num_acknowledge_interval;
        sequence_num = port->next_control_sequence_num_to_send++;
    }

    delegate_->ForwardEvent(
        peer_node_name, std::make_unique<UserMessageReadAckRequestEvent>(peer_port_name, port_ref.name(), sequence_num, sequence_num_to_request_ack));
    return OK;
}
...
int Node::AcceptEventInternal(const PortRef& port_ref, const NodeName& from_node, ScopedEvent event)
{
    switch (event->type()) {
    case Event::Type::kUserMessage:
        return OnUserMessage(port_ref, from_node, Event::Cast<UserMessageEvent>(&event));
    case Event::Type::kPortAccepted:
    ...
    case Event::Type::kObserveProxyAck:
        return OnObserveProxyAck(port_ref, Event::Cast<ObserveProxyAckEvent>(&event));
    case Event::Type::kObserveClosure:
        return OnObserveClosure(port_ref, Event::Cast<ObserveClosureEvent>(&event));
    case Event::Type::kMergePort:
        return OnMergePort(port_ref, Event::Cast<MergePortEvent>(&event));
    case Event::Type::kUserMessageReadAckRequest:
        return OnUserMessageReadAckRequest(port_ref, Event::Cast<UserMessageReadAckRequestEvent>(&event));
    case Event::Type::kUserMessageReadAck:
        return OnUserMessageReadAck(port_ref, Event::Cast<UserMessageReadAckEvent>(&event));
    case Event::Type::kUpdatePreviousPeer:
        return OnUpdatePreviousPeer(port_ref, Event::Cast<UpdatePreviousPeerEvent>(&event));
    }
...
int Node::AcceptEvent(const NodeName& from_node, ScopedEvent event)
{
    ...
    DVLOG(2) << "AcceptEvent type: " << event->type() << ", " << event->from_port() << "@" << from_node << " => " << port_ref.name() << "@" << name_
             << " seq nr: " << event->control_sequence_num() << " port valid? " << port_ref.is_valid();
    ...
    // Before processing the event, verify the sender and sequence number.
    {
        ...
        if (!port->IsNextEvent(from_node, *event)) {
            DVLOG(2) << "Buffering event (type " << event->type() << "): " << event->from_port() << "@" << from_node << " => " << port_ref.name() << "@"
                     << name_ << " seq nr: " << event->control_sequence_num() << " / " << port->next_control_sequence_num_to_receive << ", want "
    ...
    while (true) {
        ...
        NodeName next_from_node;
        {
            ...
            if (next_event) {
                DVLOG(2) << "Handling buffered event (type " << next_event->type() << "): " << next_event->from_port() << "@" << next_from_node << " => "
                         << port_ref.name() << "@" << name_ << " seq nr: " << next_event->control_sequence_num() << " / "
                         << port->next_control_sequence_num_to_receive;
            }
        }
        if (!next_event)
...
int Node::MergePorts(const PortRef& port_ref, const NodeName& destination_node_name, const PortName& destination_port_name)
{
    ...
    PendingUpdatePreviousPeer pending_update_event { .from_port = port_ref.name() };
    {
        ...
        DVLOG(1) << "Sending MergePort from " << port_ref.name() << "@" << name_ << " to " << destination_port_name << "@" << destination_node_name;
        ...
        ConvertToProxy(locker.port(), destination_node_name, &new_port_name, &new_port_descriptor, &pending_update_event);
    }

#ifndef MOJO_BACKWARDS_COMPAT
    delegate_->ForwardEvent(pending_update_event.receiver,
        std::make_unique<UpdatePreviousPeerEvent>(pending_update_event.port, pending_update_event.from_port, pending_update_event.sequence_num,
            pending_update_event.new_prev_node, pending_update_event.new_prev_port));
#endif

    if (new_port_descriptor.peer_node_name == name_ && destination_node_name != name_) {
    ...
    delegate_->ForwardEvent(destination_node_name,
        std::make_unique<MergePortEvent>(destination_port_name, kInvalidPortName, kInvalidSequenceNum, new_port_name, new_port_descriptor));
    return OK;
}

int Node::MergeLocalPorts(const PortRef& port0_ref, const PortRef& port1_ref)
{
    DVLOG(1) << "Merging local ports " << port0_ref.name() << "@" << name_ << " and " << port1_ref.name() << "@" << name_;
    return MergePortsInternal(port0_ref, port1_ref, true /* allow_close_on_bad_state */);
}
...
int Node::LostConnectionToNode(const NodeName& node_name)
{
    ...
    DVLOG(1) << "Observing lost connection from node " << name_ << " to node " << node_name;

    DestroyAllPortsWithPeer(node_name, kInvalidPortName);
    return OK;
}

int Node::OnUserMessage(const PortRef& port_ref, const NodeName& from_node, std::unique_ptr<UserMessageEvent> message)
{
#if DCHECK_IS_ON()
    ...
    DVLOG(4) << "OnUserMessage " << message->sequence_num() << " [ports=" << ports_buf.str() << "] at " << message->port_name() << "@" << name_;
#endif
...
int Node::OnPortAccepted(const PortRef& port_ref, std::unique_ptr<PortAcceptedEvent> event)
{
    if (!port_ref.is_valid())
        return ERROR_PORT_UNKNOWN;
    ...
    {
        SinglePortLocker locker(&port_ref);
        DVLOG(2) << "PortAccepted at " << port_ref.name() << "@" << name_ << " pointing to " << locker.port()->peer_port_name << "@"
...
int Node::OnObserveProxy(const PortRef& port_ref, std::unique_ptr<ObserveProxyEvent> event)
{
    ...
    if (!port_ref.is_valid()) {
        DVLOG(1) << "ObserveProxy: " << event->port_name() << "@" << name_ << " not found";
        return OK;
    }

    DVLOG(2) << "ObserveProxy at " << port_ref.name() << "@" << name_ << ", proxy at " << event->proxy_port_name() << "@" << event->proxy_node_name()
             << " pointing to " << event->proxy_target_port_name() << "@" << event->proxy_target_node_name();
    ...
    NodeName event_target_node;
    {
        ...
        if (port->peer_node_name == event->proxy_node_name() && port->peer_port_name == event->proxy_port_name()) {
            if (port->state == Port::kReceiving) {
                ...
                UpdatePortPeerAddress(port_ref.name(), port, event->proxy_target_node_name(), event->proxy_target_port_name());
                event_target_node = event->proxy_node_name();
                event_to_forward
                    = std::make_unique<ObserveProxyAckEvent>(event->proxy_port_name(), port_ref.name(), sequence_num, port->next_sequence_num_to_send - 1);
                peer_changed = true;
                ...
                DVLOG(2) << "Delaying ObserveProxyAck to " << event->proxy_port_name() << "@" << event->proxy_node_name();

                port->send_on_proxy_removal = std::make_unique<std::pair<NodeName, ScopedEvent>>(event->proxy_node_name(),
                    std::make_unique<ObserveProxyAckEvent>(event->proxy_port_name(), port_ref.name(), kInvalidSequenceNum, kInvalidSequenceNum));
            }
        } else {
...
int Node::OnObserveProxyAck(const PortRef& port_ref, std::unique_ptr<ObserveProxyAckEvent> event)
{
    DVLOG(2) << "ObserveProxyAck at " << event->port_name() << "@" << name_ << " (last_sequence_num=" << event->last_sequence_num() << ")";
    ...
    bool erase_port = false;
    {
        ...
        if (port->state == Port::kProxying) {
            ...
            try_remove_proxy_immediately = event->last_sequence_num() != kInvalidSequenceNum;
...
int Node::OnObserveClosure(const PortRef& port_ref, std::unique_ptr<ObserveClosureEvent> event)
{
    ...
    bool erase_port = false;
    {
        ...
        DVLOG(2) << "ObserveClosure at " << port_ref.name() << "@" << name_ << " (state=" << port->state << ") pointing to " << port->peer_port_name << "@"
                 << port->peer_node_name << " (last_sequence_num=" << event->last_sequence_num() << ")";
        ...
        if (port->state == Port::kReceiving) {
            ...
            port->last_sequence_num_acknowledged = port->next_sequence_num_to_send - 1;
        } else if (port->state == Port::kClosed) {
        ...
        DVLOG(2) << "Forwarding ObserveClosure from " << port_ref.name() << "@" << name_ << " to peer " << port->peer_port_name << "@" << port->peer_node_name
...
int Node::OnMergePort(const PortRef& port_ref, std::unique_ptr<MergePortEvent> event)
{
    DVLOG(1) << "MergePort at " << port_ref.name() << "@" << name_ << " merging with proxy " << event->new_port_name() << "@" << name_ << " pointing to "
             << event->new_port_descriptor().peer_port_name << "@" << event->new_port_descriptor().peer_node_name << " referred by "
             << event->new_port_descriptor().referring_port_name << "@" << event->new_port_descriptor().referring_node_name;
    ...
    bool peer_allowed = true;
    {
        ...
        if (!port->pending_merge_peer) {
            LOG(ERROR) << "MergePort called on unexpected port: " << event->port_name();
            peer_allowed = false;
        } else {
    ...
    return MergePortsInternal(port_ref, new_port_ref, false /* allow_close_on_bad_state */);
}

int Node::OnUserMessageReadAckRequest(const PortRef& port_ref, std::unique_ptr<UserMessageReadAckRequestEvent> event)
{
    DVLOG(1) << "AckRequest " << port_ref.name() << "@" << name_ << " sequence " << event->sequence_num_to_acknowledge();
    ...
    std::unique_ptr<Event> event_to_send;
    {
        ...
        if (port->state == Port::kProxying) {
            ...
            uint64_t current_sequence_num = port->message_queue.next_sequence_num() - 1;
            ...
            if (current_sequence_num >= event->sequence_num_to_acknowledge()) {
                ...
                event_to_send = std::make_unique<UserMessageReadAckEvent>(
                    port->peer_port_name, port_ref.name(), port->next_control_sequence_num_to_send++, current_sequence_num);

                if (port->state == Port::kBuffering) {
                    port->control_message_queue.push({ peer_node_name, std::move(event_to_send) });
                }
                ...
                bool has_queued_ack_request = port->sequence_num_to_acknowledge > current_sequence_num;
...
int Node::OnUserMessageReadAck(const PortRef& port_ref, std::unique_ptr<UserMessageReadAckEvent> event)
{
    DVLOG(1) << "Acknowledge " << port_ref.name() << "@" << name_ << " sequence " << event->sequence_num_acknowledged();
    ...
    if (port_ref.is_valid()) {
        ...
        if (event->sequence_num_acknowledged() <= port->last_sequence_num_acknowledged) {
        ...
        if (port->sequence_num_acknowledge_interval && !port->peer_closed) {
            peer_node_name = port->peer_node_name;
            ack_request_event = std::make_unique<UserMessageReadAckRequestEvent>(port->peer_port_name, port_ref.name(),
                port->next_control_sequence_num_to_send++, port->last_sequence_num_acknowledged + port->sequence_num_acknowledge_interval);
...
int Node::OnUpdatePreviousPeer(const PortRef& port_ref, std::unique_ptr<UpdatePreviousPeerEvent> event)
{
    DVLOG(1) << "OnUpdatePreviousPeer port: " << event->port_name() << " changing to " << event->new_node_name() << ", port: " << event->from_port() << " => "
             << event->new_port_name();
...
int Node::AddPortWithName(const PortName& port_name, scoped_refptr<Port> port)
{
    ...
    if (port->peer_port_name != kInvalidPortName) {
        ...
        peer_port_maps_[port->peer_node_name][port->peer_port_name].emplace(port_name, PortRef(port_name, port));
    }
...
int Node::SendUserMessageInternal(const PortRef& port_ref, std::unique_ptr<UserMessageEvent>* message)
{
    ...
    int rv = PrepareToForwardUserMessage(port_ref, Port::kReceiving, false /* ignore_closed_peer */, m.get(), &target_node);
    if (rv != OK)
        return rv;
...
int Node::MergePortsInternal(const PortRef& port0_ref, const PortRef& port1_ref, bool allow_close_on_bad_state)
{
    ...
    uint64_t original_sequence_number[2];
    {
        ...
        if (port0->state != Port::kReceiving || port1->state != Port::kReceiving
            || (port0->peer_node_name == name_ && port0->peer_port_name == port1_ref.name())
            || (port1->peer_node_name == name_ && port1->peer_port_name == port0_ref.name()) || port0->next_sequence_num_to_send != kInitialSequenceNum
            ...
            const bool close_port0 = port0->state == Port::kReceiving || allow_close_on_bad_state;
        ...
        pending_update_events[0] = { .receiver = port0->peer_node_name,
        ...
        pending_update_events[1] = { .receiver = port1->peer_node_name,
    ...
    if (ForwardUserMessagesFromProxy(port0_ref) == OK && ForwardUserMessagesFromProxy(port1_ref) == OK) {
#ifndef MOJO_BACKWARDS_COMPAT
        ...
        for (const auto* const port_ref : port_refs) {
            ...
            NodeName closure_event_target_node;
            {
                ...
                if (try_remove_proxy_immediately || port->peer_closed) {
                    ...
                    closure_event = std::make_unique<ObserveClosureEvent>(
                        port->peer_port_name, port_ref->name(), port->next_control_sequence_num_to_send++, port->last_sequence_num_to_receive);
                }
            }
            if (try_remove_proxy_immediately)
                TryRemoveProxy(*port_ref);
            ...
            if (closure_event) {
                delegate_->ForwardEvent(closure_event_target_node, std::move(closure_event));
            }
        }
...
void Node::ConvertToProxy(
    Port* port, const NodeName& to_node_name, PortName* port_name, Event::PortDescriptor* port_descriptor, PendingUpdatePreviousPeer* pending_update)
{
    ...
    *port_name = new_port_name;
    ...
    port_descriptor->next_sequence_num_to_receive = port->message_queue.next_sequence_num();
...
int Node::AcceptPort(const PortName& port_name, const Event::PortDescriptor& port_descriptor)
{
    scoped_refptr<Port> port = base::MakeRefCounted<Port>(port_descriptor.next_sequence_num_to_send, port_descriptor.next_sequence_num_to_receive);
    port->state = Port::kReceiving;
    ...
    port->last_sequence_num_to_receive = port_descriptor.last_sequence_num_to_receive;
    port->peer_closed = port_descriptor.peer_closed;

    DVLOG(2) << "Accepting port " << port_name << " [peer_closed=" << port->peer_closed
    ...
    delegate_->ForwardEvent(
        port_descriptor.referring_node_name, std::make_unique<PortAcceptedEvent>(port_descriptor.referring_port_name, kInvalidPortName, kInvalidSequenceNum));
    return OK;
}

int Node::PrepareToForwardUserMessage(
    const PortRef& forwarding_port_ref, Port::State expected_port_state, bool ignore_closed_peer, UserMessageEvent* message, NodeName* forward_to_node)
{
    ...
    for (;;) {
        ...
        PortLocker locker(ports_to_lock.container().data(), ports_to_lock.container().size());
        ...
        if (message->num_ports() > 0) {
            ...
            if (target_is_remote) {
                ...
                for (size_t i = 0; i < message->num_ports(); ++i) {
                    ...
                    PendingUpdatePreviousPeer update_event = { .from_port = attached_port_refs[i].name() };
                    ConvertToProxy(port, target_node_name, message->ports() + i, port_descriptors + i, &update_event);
...
#if DCHECK_IS_ON()
        DVLOG(4) << "Sending message " << message->sequence_num() << " [ports=" << ports_buf.str() << "]"
                 << " from " << forwarding_port_ref.name() << "@" << name_ << " to " << forwarding_port->peer_port_name << "@" << target_node_name;
#endif
        ...
        *forward_to_node = target_node_name;
        ...
        message->set_control_sequence_num(forwarding_port->next_control_sequence_num_to_send++);
        break;
    }
...
int Node::BeginProxying(const PortRef& port_ref)
{
    ...
    while (!control_message_queue.empty()) {
        ...
        delegate_->ForwardEvent(node_event_pair.first, std::move(node_event_pair.second));
    }
...
int Node::ForwardUserMessagesFromProxy(const PortRef& port_ref)
{
    for (;;) {
        ...
        NodeName target_node;
        int rv = PrepareToForwardUserMessage(port_ref, Port::kProxying, true /* ignore_closed_peer */, message.get(), &target_node);
        {
...
void Node::InitiateProxyRemoval(const PortRef& port_ref)
{
    ...
    delegate_->ForwardEvent(peer_node_name,
        std::make_unique<ObserveProxyEvent>(peer_port_name, port_ref.name(), sequence_num, name_, port_ref.name(), peer_node_name, peer_port_name));
}
...

    {
        ...
        if (!CanAcceptMoreMessages(port)) {
            ...
            if (port->send_on_proxy_removal) {
                ...
                if (removal_event) {
                    removal_event->set_control_sequence_num(port->next_control_sequence_num_to_send++);
            ...
            pending_update_event = { .receiver = port->peer_node_name,
            ...
            DVLOG(2) << "Cannot remove port " << port_ref.name() << "@" << name_ << " now; waiting for more messages";
        }
    }
...
void Node::DestroyAllPortsWithPeer(const NodeName& node_name, const PortName& port_name)
{
...

    {
        ...
        for (auto peer_port_iter = peer_ports_begin; peer_port_iter != peer_ports_end; ++peer_port_iter) {
            ...
            for (auto local_port_iter = local_ports.begin(); local_port_iter != local_ports.end(); ++local_port_iter) {
                ...
                if (port_name != kInvalidPortName) {
                    ...
                    closure_event_target_node = port->peer_node_name;
                    closure_event = std::make_unique<ObserveClosureEvent>(
                        port->peer_port_name, local_port_ref.name(), port->next_control_sequence_num_to_send++, port->last_sequence_num_to_receive);
                }
    ...
    for (const auto& proxy_name : dead_proxies_to_broadcast) {
        ...
        delegate_->BroadcastEvent(std::make_unique<ObserveProxyEvent>(
            kInvalidPortName, kInvalidPortName, kInvalidSequenceNum, name_, proxy_name, kInvalidNodeName, kInvalidPortName));
...
void Node::UpdatePortPeerAddress(const PortName& local_port_name, Port* local_port, const NodeName& new_peer_node, const PortName& new_peer_port)
{
    ...
    if (new_peer_port != kInvalidPortName) {
        peer_port_maps_[new_peer_node][new_peer_port].emplace(local_port_name, PortRef(local_port_name, base::WrapRefCounted<Port>(local_port)));
    }
}

void Node::RemoveFromPeerPortMap(const PortName& local_port_name, Port* local_port)
{
...
void Node::SwapPortPeers(const PortName& port0_name, Port* port0, const PortName& port1_name, Port* port1)
{
    ports_lock_.AssertAcquired();
    port0->AssertLockAcquired();
    port1->AssertLockAcquired();

    auto& peer0_ports = peer_port_maps_[port0->peer_node_name][port0->peer_port_name];
    auto& peer1_ports = peer_port_maps_[port1->peer_node_name][port1->peer_port_name];
    peer0_ports.erase(port0_name);
    peer1_ports.erase(port1_name);
    peer0_ports.emplace(port1_name, PortRef(port1_name, base::WrapRefCounted<Port>(port1)));
    peer1_ports.emplace(port0_name, PortRef(port0_name, base::WrapRefCounted<Port>(port0)));
...
void Node::MaybeResendAckRequest(const PortRef& port_ref)
{
    ...
    ScopedEvent ack_request_event;
    {
        ...
        peer_node_name = port->peer_node_name;
        ack_request_event = std::make_unique<UserMessageReadAckRequestEvent>(port->peer_port_name, port_ref.name(), port->next_control_sequence_num_to_send++,
            port->last_sequence_num_acknowledged + port->sequence_num_acknowledge_interval);
    }
...
void Node::MaybeResendAck(const PortRef& port_ref)
{
    ...
    ScopedEvent ack_event;
    {
        ...
        uint64_t last_sequence_num_read = port->message_queue.next_sequence_num() - 1;
        if (!port->sequence_num_to_acknowledge || !last_sequence_num_read)
            return;

        peer_node_name = port->peer_node_name;
        ack_event = std::make_unique<UserMessageReadAckEvent>(
            port->peer_port_name, port_ref.name(), port->next_control_sequence_num_to_send++, last_sequence_num_read);
    }
