...
#pragma pack(pop)
    ...
    template <typename T> static std::unique_ptr<T> Cast(ScopedEvent* event)
    {
    ...
    const PortName& port_name() const
    {
        return port_name_;
    }
    ...
    const PortName& from_port() const
    {
        return from_port_;
    }
    void set_from_port(const PortName& from_port)
    {
        from_port_ = from_port;
    }

    uint64_t control_sequence_num() const
    {
        return control_sequence_num_;
    }
...
protected:
    Event(Type type, const PortName& port_name, const PortName& from_port, uint64_t control_sequence_num);
...
public:
    ...
    template <typename T> T* GetMessage()
    {
        DCHECK(HasMessage());
    ...
    template <typename T> const T* GetMessage() const
    {
        DCHECK(HasMessage());
        DCHECK_EQ(&T::kUserMessageTypeInfo, message_->type_info());
    ...
    void set_sequence_num(uint64_t sequence_num)
    {
    ...
    static ScopedEvent Deserialize(const PortName& port_name, const PortName& from_port, uint64_t control_sequence_num, const void* buffer, size_t num_bytes);

    size_t GetSizeIfSerialized() const;

private:
    UserMessageEvent(const PortName& port_name, const PortName& from_port, uint64_t control_sequence_num, uint64_t sequence_num);
...
public:
    explicit PortAcceptedEvent(const PortName& port_name, const PortName& from_port, uint64_t control_sequence_num);
...
public:
    ObserveProxyEvent(const PortName& port_name, const PortName& from_port, uint64_t control_sequence_num, const NodeName& proxy_node_name,
    ...
    const NodeName& proxy_node_name() const
    {
        return proxy_node_name_;
    }
    const PortName& proxy_port_name() const
    {
        return proxy_port_name_;
    }
...
public:
    ObserveProxyAckEvent(const PortName& port_name, const PortName& from_port, uint64_t control_sequence_num, uint64_t last_sequence_num);
    ...
    uint64_t last_sequence_num() const
    {
        return last_sequence_num_;
    }
...
public:
    ObserveClosureEvent(const PortName& port_name, const PortName& from_port, uint64_t control_sequence_num, uint64_t last_sequence_num);
...
public:
    MergePortEvent(const PortName& port_name, const PortName& from_port, uint64_t control_sequence_num, const PortName& new_port_name,
    ...
    const PortName& new_port_name() const
    {
        return new_port_name_;
    }
...
class COMPONENT_EXPORT(MOJO_CORE_PORTS) UserMessageReadAckRequestEvent : public Event {
public:
    UserMessageReadAckRequestEvent(const PortName& port_name, const PortName& from_port, uint64_t control_sequence_num, uint64_t sequence_num_to_acknowledge);
...
public:
    UserMessageReadAckEvent(const PortName& port_name, const PortName& from_port, uint64_t control_sequence_num, uint64_t sequence_num_acknowledged);
...
public:
    ...
    const NodeName& new_node_name() const
    {
        return new_node_name_;
    }
