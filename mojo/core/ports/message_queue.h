...
public:
    ...
    void set_signalable(bool value)
    {
        signalable_ = value;
    }

    uint64_t next_sequence_num() const
    {
        return next_sequence_num_;
    }
    ...
    void GetNextMessage(std::unique_ptr<UserMessageEvent>* message, MessageFilter* filter);
    ...
    size_t queued_num_bytes() const
    {
        return total_queued_bytes_;
    }
