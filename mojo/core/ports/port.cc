...
inline bool operator<(const std::unique_ptr<Event>& a, const std::unique_ptr<Event>& b)
{
    return a->control_sequence_num() > b->control_sequence_num();
}

Port::Port(uint64_t next_sequence_num_to_send, uint64_t next_sequence_num_to_receive)
...
void Port::NextEvent(NodeName* from_node, ScopedEvent* event)
{
    ...
    if (msg_queue[0]->control_sequence_num() != next_control_sequence_num_to_receive)
        return;
...
void Port::TakePendingMessages(std::vector<std::unique_ptr<UserMessageEvent>>& messages)
{
