...
public:
    ...
    {
        if (base::StartsWith(test_info.name(), kManualTestPrefix, base::CompareCase::SENSITIVE)
...
void MojoTestSuiteBase::MaybeInitializeChildProcessEnvironment()
{
    ...
    if (!command_line.HasSwitch(switches::kTestChildProcess) && !command_line.HasSwitch(kDeathTestSwitch)) {
        // Not in a child process. Do nothing.
        return;
    }

    const std::string enabled = command_line.GetSwitchValueASCII(switches::kEnableFeatures);
    const std::string disabled = command_line.GetSwitchValueASCII(switches::kDisableFeatures);
