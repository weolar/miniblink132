...
PlatformHandle PlatformHandleFromFILE(base::ScopedFILE fp)
{
    ...
    PCHECK(DuplicateHandle(
        GetCurrentProcess(), reinterpret_cast<HANDLE>(_get_osfhandle(_fileno(fp.get()))), GetCurrentProcess(), &rv, 0, TRUE, DUPLICATE_SAME_ACCESS))
        << "DuplicateHandle";
...
base::ScopedFILE FILEFromPlatformHandle(PlatformHandle h, const char* mode)
{
    ...
    if (strchr(mode, 't'))
        flags |= _O_TEXT;
    base::ScopedFILE rv(_fdopen(_open_osfhandle(reinterpret_cast<intptr_t>(h.ReleaseHandle()), flags), mode));
    PCHECK(rv) << "_fdopen";
    return rv;
}
