...
base::FilePath ResolveSourceRootRelativePath(const char* relative_path)
{
    ...
    for (const base::StringPiece& component : base::SplitStringPiece(relative_path, "/", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL)) {
        if (!component.empty())
...
void TestSupportImpl::LogPerfResult(const char* test_name, const char* sub_test_name, double value, const char* units)
{
    DCHECK(test_name);
    if (sub_test_name) {
...
char** TestSupportImpl::EnumerateSourceRootRelativeDirectory(const char* relative_path)
{
    std::vector<std::string> names;
    base::FileEnumerator e(ResolveSourceRootRelativePath(relative_path), false, base::FileEnumerator::FILES);
