...
public:
    ...
    public:
        ClientController(const std::string& client_name, MojoTestBase* test, LaunchType launch_type);
        ...
        {
            return helper_.test_child();
        }
#endif

        MojoHandle pipe() const
        {
            return pipe_.get().value();
        }
    ...
    template <typename HandlerFunc> void RunTestClient(const std::string& client_name, HandlerFunc handler)
    {
        EXPECT_EQ(0, RunTestClientAndGetExitCode(client_name, handler));
    }

    template <typename HandlerFunc> int RunTestClientAndGetExitCode(const std::string& client_name, HandlerFunc handler)
    {
    ...
    template <typename HandlerFunc> void RunTestClientWithController(const std::string& client_name, HandlerFunc handler)
    {
        ClientController& c = StartClient(client_name);
        handler(c);
    ...
    static void WriteMessageWithHandles(MojoHandle mp, const std::string& message, const MojoHandle* handles, uint32_t num_handles);
    ...
    static std::string ReadMessageWithHandles(MojoHandle mp, MojoHandle* handles, uint32_t expected_num_handles);

    // Reads a string from the pipe, expecting either zero or one handles.
    // If no handle is read, |handle| will be reset.
    static std::string ReadMessageWithOptionalHandle(MojoHandle mp, MojoHandle* handle);
    ...
    static void VerifyTransmission(MojoHandle in, MojoHandle out, const std::string& message);
    ...
    static void WriteToBuffer(MojoHandle h, size_t offset, const base::StringPiece& s);

    // Maps a buffer, tests the value of some of its contents, and unmaps it.
    static void ExpectBufferContents(MojoHandle h, size_t offset, const base::StringPiece& s);

    //////// Data pipe test utilities /////////

    // Creates a new data pipe.
    static void CreateDataPipe(MojoHandle* producer, MojoHandle* consumer, size_t capacity);
    ...
    static MojoResult WaitForSignals(MojoHandle handle, MojoHandleSignals signals, MojoTriggerCondition condition, MojoHandleSignalsState* state = nullptr);

    // Like above but only waits for signals to go high.
    static MojoResult WaitForSignals(MojoHandle handle, MojoHandleSignals signals, MojoHandleSignalsState* state = nullptr);

    void set_launch_type(LaunchType launch_type)
    {
...
#define DEFINE_TEST_CLIENT_WITH_PIPE(client_name, test_base, pipe_name)                                                                                        \
    ...
    MULTIPROCESS_TEST_MAIN_WITH_SETUP(client_name##TestChildMain, ::mojo::core::test::MultiprocessTestHelper::ChildSetup)                                      \
    {                                                                                                                                                          \
        client_name##_MainFixture test;                                                                                                                        \
        return ::mojo::core::test::MultiprocessTestHelper::RunClientMain(base::BindOnce(&client_name##_MainFixture::Main, base::Unretained(&test)));           \
    }                                                                                                                                                          \
...
#define DEFINE_TEST_CLIENT_TEST_WITH_PIPE(client_name, test_base, pipe_name)                                                                                   \
    ...
    {                                                                                                                                                          \
        client_name##_MainFixture test;                                                                                                                        \
        return ::mojo::core::test::MultiprocessTestHelper::RunClientTestMain(base::BindOnce(&client_name##_MainFixture::Main, base::Unretained(&test)));       \
    }                                                                                                                                                          \
