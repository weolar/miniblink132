...
public:
    ...
    ScopedMessagePipeHandle StartChild(const std::string& test_child_name, LaunchType launch_type = LaunchType::CHILD);
    ...
    const base::Process& test_child() const
    {
        return test_child_;
    }
    ...
    static int RunClientMain(base::OnceCallback<int(MojoHandle)> main, bool pass_pipe_ownership_to_main = false);
