...
public:
    CoreInstance()
    {
        ...
        mojo_config.max_message_num_bytes = mojo::core::test::MojoTestBase::kMaxMessageSizeInTests;
        if (!base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kTestChildProcess)
            || base::CommandLine::ForCurrentProcess()->HasSwitch(test_switches::kMojoIsBroker)) {
            mojo_config.is_broker_process = true;
        }

        if (base::CommandLine::ForCurrentProcess()->HasSwitch(kDisableAllCapabilities)) {
    ...
    : core_(std::make_unique<CoreInstance>())
{
    ...
    test_io_thread_.PostTask(FROM_HERE, base::BindLambdaForTesting([&] { io_thread_initialized.Signal(); }));
