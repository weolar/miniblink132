...
PlatformSharedMemoryMapping::PlatformSharedMemoryMapping(base::subtle::PlatformSharedMemoryRegion* region, size_t offset, size_t length)
{
    switch (region->GetMode()) {
    case base::subtle::PlatformSharedMemoryRegion::Mode::kReadOnly: {
        auto read_only_region = base::ReadOnlySharedMemoryRegion::Deserialize(std::move(*region));
        mapping_ = read_only_region.MapAt(offset, length);
        *region = base::ReadOnlySharedMemoryRegion::TakeHandleForSerialization(std::move(read_only_region));
        return;
    }
    case base::subtle::PlatformSharedMemoryRegion::Mode::kUnsafe: {
        auto unsafe_region = base::UnsafeSharedMemoryRegion::Deserialize(std::move(*region));
        mapping_ = unsafe_region.MapAt(offset, length);
        *region = base::UnsafeSharedMemoryRegion::TakeHandleForSerialization(std::move(unsafe_region));
        return;
    }
    case base::subtle::PlatformSharedMemoryRegion::Mode::kWritable: {
        auto writable_region = base::WritableSharedMemoryRegion::Deserialize(std::move(*region));
        mapping_ = writable_region.MapAt(offset, length);
        *region = base::WritableSharedMemoryRegion::TakeHandleForSerialization(std::move(writable_region));
        return;
    }
    }
    CHECK(false);
