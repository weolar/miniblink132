...
class DataPipeConsumerDispatcher::PortObserverThunk : public NodeController::PortObserver {
public:
    explicit PortObserverThunk(scoped_refptr<DataPipeConsumerDispatcher> dispatcher)
...
// static
scoped_refptr<DataPipeConsumerDispatcher> DataPipeConsumerDispatcher::Create(NodeController* node_controller, const ports::PortRef& control_port,
    base::UnsafeSharedMemoryRegion shared_ring_buffer, const MojoCreateDataPipeOptions& options, uint64_t pipe_id)
{
    scoped_refptr<DataPipeConsumerDispatcher> consumer
        = new DataPipeConsumerDispatcher(node_controller, control_port, std::move(shared_ring_buffer), options, pipe_id);
...
MojoResult DataPipeConsumerDispatcher::ReadData(const MojoReadDataOptions& options, void* elements, uint32_t* num_bytes)
{
    base::AutoLock lock(lock_);
    ...
    if ((options.flags & MOJO_READ_DATA_FLAG_QUERY)) {
        if ((options.flags & MOJO_READ_DATA_FLAG_PEEK) || (options.flags & MOJO_READ_DATA_FLAG_DISCARD))
            return MOJO_RESULT_INVALID_ARGUMENT;
    ...
    if (min_num_bytes_to_read > bytes_available_) {
        if (had_new_data)
            watchers_.NotifyState(GetHandleSignalsStateNoLock());
        return peer_closed_ ? MOJO_RESULT_FAILED_PRECONDITION : MOJO_RESULT_OUT_OF_RANGE;
    }
    ...
    if (!discard) {
        const uint8_t* data = static_cast<const uint8_t*>(ring_buffer_mapping_.memory());
        CHECK(data);
        ...
        uint32_t tail_bytes_to_copy = std::min(options_.capacity_num_bytes - read_offset_, bytes_to_read);
...
MojoResult DataPipeConsumerDispatcher::BeginReadData(const void** buffer, uint32_t* buffer_num_bytes)
{
    base::AutoLock lock(lock_);
    if (!shared_ring_buffer_.IsValid() || in_transit_)
        return MOJO_RESULT_INVALID_ARGUMENT;
    ...
    uint32_t bytes_to_read = std::min(bytes_available_, options_.capacity_num_bytes - read_offset_);
...
MojoResult DataPipeConsumerDispatcher::EndReadData(uint32_t num_bytes_read)
{
    ...
    MojoResult rv;
    if (num_bytes_read > two_phase_max_bytes_read_ || num_bytes_read % options_.element_num_bytes != 0) {
        rv = MOJO_RESULT_INVALID_ARGUMENT;
    } else {
        rv = MOJO_RESULT_OK;
        read_offset_ = (read_offset_ + num_bytes_read) % options_.capacity_num_bytes;
...
MojoResult DataPipeConsumerDispatcher::AddWatcherRef(const scoped_refptr<WatcherDispatcher>& watcher, uintptr_t context)
{
    base::AutoLock lock(lock_);
...
MojoResult DataPipeConsumerDispatcher::RemoveWatcherRef(WatcherDispatcher* watcher, uintptr_t context)
{
    base::AutoLock lock(lock_);
    if (is_closed_ || in_transit_)
        return MOJO_RESULT_INVALID_ARGUMENT;
...
void DataPipeConsumerDispatcher::StartSerialize(uint32_t* num_bytes, uint32_t* num_ports, uint32_t* num_handles)
{
    base::AutoLock lock(lock_);
    DCHECK(in_transit_);
...
bool DataPipeConsumerDispatcher::EndSerialize(void* destination, ports::PortName* ports, PlatformHandle* platform_handles)
{
...
// static
scoped_refptr<DataPipeConsumerDispatcher> DataPipeConsumerDispatcher::Deserialize(
    const void* data, size_t num_bytes, const ports::PortName* ports, size_t num_ports, PlatformHandle* handles, size_t num_handles)
{
    ...
    if (!state->options.capacity_num_bytes || !state->options.element_num_bytes || state->options.capacity_num_bytes < state->options.element_num_bytes
        || state->read_offset >= state->options.capacity_num_bytes || state->bytes_available > state->options.capacity_num_bytes) {
        AssertNotExtractingHandlesFromMessage();
        return nullptr;
    }
    ...
    scoped_refptr<DataPipeConsumerDispatcher> dispatcher
        = new DataPipeConsumerDispatcher(node_controller, port, std::move(ring_buffer), state->options, state->pipe_id);
...
DataPipeConsumerDispatcher::DataPipeConsumerDispatcher(NodeController* node_controller, const ports::PortRef& control_port,
    base::UnsafeSharedMemoryRegion shared_ring_buffer, const MojoCreateDataPipeOptions& options, uint64_t pipe_id)
    : options_(options)
...
{
    DCHECK(is_closed_ && !shared_ring_buffer_.IsValid() && !ring_buffer_mapping_.IsValid() && !in_transit_);
}
...
HandleSignalsState DataPipeConsumerDispatcher::GetHandleSignalsStateNoLock() const
{
    lock_.AssertAcquired();
...
{
    DVLOG(1) << "Data pipe consumer " << pipe_id_ << " notifying peer: " << num_bytes << " bytes read. [control_port=" << control_port_.name() << "]";

    SendDataPipeControlMessage(node_controller_, control_port_, DataPipeCommand::DATA_WAS_READ, num_bytes);
}
...
void DataPipeConsumerDispatcher::UpdateSignalsStateNoLock()
{
    ...
    if (rv != ports::OK || !port_status.receiving_messages) {
        ...
        do {
            ...
            if (message_event) {
                ...
                TRACE_EVENT0("ipc", "DataPipeConsumerDispatcher received DATA_WAS_WRITTEN");

                uint32_t new_bytes_available;
                if (!base::CheckAdd(bytes_available_, m->num_bytes).AssignIfValid(&new_bytes_available) || new_bytes_available > options_.capacity_num_bytes) {
                    DLOG(ERROR) << "Producer claims to have written too many bytes.";
                    peer_closed_ = true;
                    break;
                }

                DVLOG(1) << "Data pipe consumer " << pipe_id_ << " is aware that " << m->num_bytes
                         << " bytes were written. [control_port=" << control_port_.name() << "]";
    ...
    if (peer_closed_ != was_peer_closed || has_new_data || peer_remote_ != was_peer_remote) {
        watchers_.NotifyState(GetHandleSignalsStateNoLock());
    }
}
