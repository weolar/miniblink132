...
public:
    ...
    MOCK_METHOD7(GetReadPlatformHandles,
        bool(const void* payload, size_t payload_size, size_t num_handles, const void* extra_header, size_t extra_header_size,
            std::vector<PlatformHandle>* handles, bool* deferred));
    MOCK_METHOD2(GetReadPlatformHandlesForIpcz, bool(size_t, std::vector<PlatformHandle>&));
...
public:
    ...
    size_t GetReceivedPayloadSize() const
    {
        return payload_size_;
    }

    const void* GetReceivedPayload() const
    {
        return payload_.get();
    }

protected:
    void OnChannelMessage(const void* payload, size_t payload_size, std::vector<PlatformHandle> handles) override
    {
...
Channel::MessagePtr CreateDefaultMessage(bool legacy_message)
{
    ...
    Channel::MessagePtr message = Channel::Message::CreateMessage(
        payload_size, 0, legacy_message ? Channel::Message::MessageType::NORMAL_LEGACY : Channel::Message::MessageType::NORMAL);
...
void TestMemoryEqual(const void* data1, size_t data1_size, const void* data2, size_t data2_size)
{
...
void TestMessagesAreEqual(Channel::Message* message1, Channel::Message* message2, bool legacy_messages)
{
    ...
    TestMemoryEqual(message1->payload(), message1->payload_size(), message2->payload(), message2->payload_size());

    if (legacy_messages)
        return;

    ASSERT_EQ(message1->extra_header_size(), message2->extra_header_size());
    TestMemoryEqual(message1->extra_header(), message1->extra_header_size(), message2->extra_header(), message2->extra_header_size());
}

TEST(ChannelTest, LegacyMessageDeserialization)
{
    Channel::MessagePtr message = CreateDefaultMessage(true /* legacy_message */);
    Channel::MessagePtr deserialized_message = Channel::Message::Deserialize(message->data(), message->data_num_bytes(), Channel::HandlePolicy::kAcceptHandles);
    TestMessagesAreEqual(message.get(), deserialized_message.get(), true /* legacy_message */);
}

TEST(ChannelTest, NonLegacyMessageDeserialization)
{
    Channel::MessagePtr message = CreateDefaultMessage(false /* legacy_message */);
    Channel::MessagePtr deserialized_message = Channel::Message::Deserialize(message->data(), message->data_num_bytes(), Channel::HandlePolicy::kAcceptHandles);
    TestMessagesAreEqual(message.get(), deserialized_message.get(), false /* legacy_message */);
}
...
TEST(ChannelTest, OnReadLegacyMessage)
{
    ...
    EXPECT_TRUE(channel->OnReadCompleteTest(message->data_num_bytes(), &next_read_size_hint));

    TestMemoryEqual(message->payload(), message->payload_size(), channel_delegate.GetReceivedPayload(), channel_delegate.GetReceivedPayloadSize());
}
...
public:
    ChannelTestShutdownAndWriteDelegate(PlatformChannelEndpoint endpoint, scoped_refptr<base::SingleThreadTaskRunner> task_runner,
        scoped_refptr<Channel> client_channel, std::unique_ptr<base::Thread> client_thread, base::RepeatingClosure quit_closure)
        : quit_closure_(std::move(quit_closure))
        , client_channel_(std::move(client_channel))
        , client_thread_(std::move(client_thread))
    {
        channel_ = Channel::Create(this, ConnectionParams(std::move(endpoint)), Channel::HandlePolicy::kAcceptHandles, std::move(task_runner));
        channel_->Start();
    }
    ~ChannelTestShutdownAndWriteDelegate() override
    {
        channel_->ShutDown();
    }
    ...
    void OnChannelMessage(const void* payload, size_t payload_size, std::vector<PlatformHandle> handles) override
    {
        ...
        if (client_channel_) {
            ...
            client_thread_->task_runner()->PostTask(FROM_HERE, base::BindOnce(&Channel::Write, client_channel_, std::move(message)));
...
{
    base::test::SingleThreadTaskEnvironment task_environment(base::test::TaskEnvironment::MainThreadType::IO);
    PlatformChannel channel;

    // Create a "client" Channel with one end of the pipe, and Start() it.
    std::unique_ptr<base::Thread> client_thread = std::make_unique<base::Thread>("clientio_thread");
    client_thread->StartWithOptions(base::Thread::Options(base::MessagePumpType::IO, 0));

    scoped_refptr<Channel> client_channel
        = Channel::Create(nullptr, ConnectionParams(channel.TakeRemoteEndpoint()), Channel::HandlePolicy::kAcceptHandles, client_thread->task_runner());
    client_channel->Start();

    // On the "client" IO thread, create and write a message.
    Channel::MessagePtr message = CreateDefaultMessage(false);
    client_thread->task_runner()->PostTask(FROM_HERE, base::BindOnce(&Channel::Write, client_channel, std::move(message)));
    ...
    ChannelTestShutdownAndWriteDelegate server_delegate(
        channel.TakeLocalEndpoint(), base::ThreadTaskRunnerHandle::Get(), std::move(client_channel), std::move(client_thread), run_loop.QuitClosure());
...
TEST(ChannelTest, RejectHandles)
{
    ...
    scoped_refptr<Channel> receiver = Channel::Create(
        &receiver_delegate, ConnectionParams(platform_channel.TakeLocalEndpoint()), Channel::HandlePolicy::kRejectHandles, base::ThreadTaskRunnerHandle::Get());
    receiver->Start();

    RejectHandlesDelegate sender_delegate;
    scoped_refptr<Channel> sender = Channel::Create(
        &sender_delegate, ConnectionParams(platform_channel.TakeRemoteEndpoint()), Channel::HandlePolicy::kRejectHandles, base::ThreadTaskRunnerHandle::Get());
    sender->Start();
    ...
    auto message = Channel::Message::CreateMessage(0 /* payload_size */, 1 /* max_handles */);
...
TEST(ChannelTest, DeserializeMessage_BadExtraHeaderSize)
{
    ...
    constexpr uint16_t kTotalHeaderSize = sizeof(Channel::Message::Header) + kBadAlignment;
    ...
    Channel::Message::Header* header = reinterpret_cast<Channel::Message::Header*>(&message[0]);
    ...
    EXPECT_EQ(nullptr, Channel::Message::Deserialize(&message[0], kMessageSize, Channel::HandlePolicy::kAcceptHandles, base::kNullProcessHandle));
}
...
TEST(ChannelTest, DeserializeMessage_NonZeroExtraHeaderSize)
{
    ...
    constexpr uint16_t kTotalHeaderSize = sizeof(Channel::Message::Header) + kChannelMessageAlignment;
    constexpr uint32_t kEmptyPayloadSize = 8;
    constexpr uint32_t kMessageSize = kTotalHeaderSize + kEmptyPayloadSize;
    char message[kMessageSize];
    memset(message, 0, kMessageSize);
...
TEST(ChannelTest, PeerStressTest)
{
    ...
    auto quit_when_both_channels_received_final_message = base::BindRepeating(
        [](std::atomic_int* count_channels_received_final_message, base::OnceClosure quit_closure) {
            if (++(*count_channels_received_final_message) == 2) {
                std::move(quit_closure).Run();
            }
        },
        base::Unretained(&count_channels_received_final_message), run_loop.QuitClosure());
    ...
    CountingChannelDelegate delegate_a(quit_when_both_channels_received_final_message);
    scoped_refptr<Channel> channel_a = Channel::Create(
        &delegate_a, ConnectionParams(platform_channel.TakeLocalEndpoint()), Channel::HandlePolicy::kRejectHandles, base::ThreadTaskRunnerHandle::Get());

    CountingChannelDelegate delegate_b(quit_when_both_channels_received_final_message);
    scoped_refptr<Channel> channel_b = Channel::Create(
        &delegate_b, ConnectionParams(platform_channel.TakeRemoteEndpoint()), Channel::HandlePolicy::kRejectHandles, peer_thread.task_runner());
    ...
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(send_lots_of_messages, channel_a));
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(send_lots_of_messages, channel_a));
    base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(send_final_message, channel_a));

    peer_thread.task_runner()->PostTask(FROM_HERE, base::BindOnce(send_lots_of_messages, channel_b));
    peer_thread.task_runner()->PostTask(FROM_HERE, base::BindOnce(send_lots_of_messages, channel_b));
    peer_thread.task_runner()->PostTask(FROM_HERE, base::BindOnce(send_final_message, channel_b));
...
TEST(ChannelTest, MessageSizeTest)
{
    ...
    scoped_refptr<Channel> receiver = Channel::Create(
        &receiver_delegate, ConnectionParams(platform_channel.TakeLocalEndpoint()), Channel::HandlePolicy::kAcceptHandles, base::ThreadTaskRunnerHandle::Get());
    receiver->Start();

    MockChannelDelegate sender_delegate;
    scoped_refptr<Channel> sender = Channel::Create(
        &sender_delegate, ConnectionParams(platform_channel.TakeRemoteEndpoint()), Channel::HandlePolicy::kAcceptHandles, base::ThreadTaskRunnerHandle::Get());
    sender->Start();
    ...
    for (uint32_t i = 0; i < base::GetPageSize() * 4; ++i) {
        ...
        base::RunLoop loop;
        receiver_delegate.set_on_message(base::BindLambdaForTesting([&got_message, &loop]() {
            got_message = true;
            loop.Quit();
        }));
        receiver_delegate.set_on_error(base::BindLambdaForTesting([&got_error, &loop]() {
            got_error = true;
            loop.Quit();
        }));
        loop.Run();
...
TEST(ChannelTest, SendToDeadMachPortName)
{
    ...
    mach_port_t send_name = platform_channel.local_endpoint().platform_handle().GetMachSendRight().get();

    auto get_send_name_refs = [&send, &dead, send_name]() {
        kern_return_t kr = mach_port_get_refs(mach_task_self(), send_name, MACH_PORT_RIGHT_SEND, &send);
        ASSERT_EQ(kr, KERN_SUCCESS);
        kr = mach_port_get_refs(mach_task_self(), send_name, MACH_PORT_RIGHT_DEAD_NAME, &dead);
        ASSERT_EQ(kr, KERN_SUCCESS);
    };
    ...
    ASSERT_EQ(KERN_SUCCESS, mach_port_mod_refs(mach_task_self(), send_name, MACH_PORT_RIGHT_SEND, 1));
    get_send_name_refs();
    EXPECT_EQ(2u, send);
    EXPECT_EQ(0u, dead);
    ...
    scoped_refptr<Channel> channel_a = Channel::Create(
        &delegate_a, ConnectionParams(platform_channel.TakeLocalEndpoint()), Channel::HandlePolicy::kAcceptHandles, base::ThreadTaskRunnerHandle::Get());
    channel_a->Start();

    // Channel B gets the receive right.
    MockChannelDelegate delegate_b;
    scoped_refptr<Channel> channel_b = Channel::Create(
        &delegate_b, ConnectionParams(platform_channel.TakeRemoteEndpoint()), Channel::HandlePolicy::kAcceptHandles, peer_thread.task_runner());
    channel_b->Start();
    ...
    peer_thread.task_runner()->PostTask(FROM_HERE, base::BindOnce(&base::WaitableEvent::Signal, base::Unretained(&event)));
    event.Wait();
...
TEST(ChannelTest, ShutDownStress)
{
    ...
    base::Thread peer_thread("channel_b_io");
    peer_thread.StartWithOptions(base::Thread::Options(base::MessagePumpType::IO, 0));
    ...
    CallbackChannelDelegate delegate_a;
    scoped_refptr<Channel> channel_a = Channel::Create(
        &delegate_a, ConnectionParams(platform_channel.TakeLocalEndpoint()), Channel::HandlePolicy::kRejectHandles, task_environment.GetMainThreadTaskRunner());
    channel_a->Start();

    scoped_refptr<Channel> channel_b
        = Channel::Create(nullptr, ConnectionParams(platform_channel.TakeRemoteEndpoint()), Channel::HandlePolicy::kRejectHandles, peer_thread.task_runner());
    channel_b->Start();
    ...
    // thread.
    peer_thread.task_runner()->PostTask(FROM_HERE, base::BindOnce(&base::WaitableEvent::Wait, base::Unretained(&go_event)));
