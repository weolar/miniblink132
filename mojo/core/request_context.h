...
public:
    ...
    // arguments.
    void AddWatchNotifyFinalizer(scoped_refptr<Watch> watch, MojoResult result, const HandleSignalsState& state);
...
private:
    ...
    struct WatchNotifyFinalizer {
        WatchNotifyFinalizer(scoped_refptr<Watch> watch, MojoResult result, const HandleSignalsState& state);
    ...
    using WatchNotifyFinalizerList = base::StackVector<WatchNotifyFinalizer, kStaticWatchFinalizersCapacity>;
    using WatchCancelFinalizerList = base::StackVector<scoped_refptr<Watch>, kStaticWatchFinalizersCapacity>;
