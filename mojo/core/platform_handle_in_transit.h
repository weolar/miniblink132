...
public:
    ...
    const base::Process& owning_process() const
    {
        return owning_process_;
    }
    ...
    bool TransferToProcess(base::Process target_process, TransferTargetTrustLevel trust = kTrustedTarget);

#if BUILDFLAG(IS_WIN)
    HANDLE remote_handle() const
    {
    ...
    static PlatformHandle TakeIncomingRemoteHandle(HANDLE handle, base::ProcessHandle owning_process);
#endif
