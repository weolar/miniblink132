...
public:
    static scoped_refptr<MockDispatcher> Create(CoreTestBase::MockHandleInfo* info)
    {
    ...
    MojoResult WriteMessage(std::unique_ptr<ports::UserMessageEvent> message_event) override
    {
        info_->IncrementWriteMessageCallCount();
        return MOJO_RESULT_OK;
    }

    MojoResult ReadMessage(std::unique_ptr<ports::UserMessageEvent>* message_event) override
    {
    ...
    MojoResult ReadData(const MojoReadDataOptions& options, void* elements, uint32_t* num_bytes) override
    {
