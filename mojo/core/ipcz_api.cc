...
IpczNodeOptions g_options = { .is_broker = false, .use_local_shared_memory_allocation = false };
...
{
    g_options = options;
    IpczCreateNodeFlags flags = options.is_broker ? IPCZ_CREATE_NODE_AS_BROKER : IPCZ_NO_FLAGS;
    IpczResult result = GetIpczAPI().CreateNode(&ipcz_driver::kDriver, IPCZ_INVALID_DRIVER_HANDLE, flags, nullptr, &g_node);
