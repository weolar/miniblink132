...
public:
    ...
    MessagePipeDispatcher(NodeController* node_controller, const ports::PortRef& port, uint64_t pipe_id, int endpoint);
    ...
    MojoResult WriteMessage(std::unique_ptr<ports::UserMessageEvent> message) override;
    MojoResult ReadMessage(std::unique_ptr<ports::UserMessageEvent>* message) override;
    MojoResult SetQuota(MojoQuotaType type, uint64_t limit) override;
    MojoResult QueryQuota(MojoQuotaType type, uint64_t* limit, uint64_t* usage) override;
