...
void WatcherDispatcher::NotifyHandleState(Dispatcher* dispatcher, const HandleSignalsState& state)
{
...
void WatcherDispatcher::InvokeWatchCallback(uintptr_t context, MojoResult result, const HandleSignalsState& state, MojoTrapEventFlags flags)
{
    MojoTrapEvent event;
...
MojoResult WatcherDispatcher::WatchDispatcher(
    scoped_refptr<Dispatcher> dispatcher, MojoHandleSignals signals, MojoTriggerCondition condition, uintptr_t context)
{
    ...
    // after we've updated all our own relevant state and released |lock_|.
    {
        ...
        scoped_refptr<Watch> watch = new Watch(this, dispatcher, context, signals, condition);
        watches_.insert({ context, watch });
        auto result = watched_handles_.insert(std::make_pair(dispatcher.get(), watch));
...
MojoResult WatcherDispatcher::Arm(uint32_t* num_blocking_events, MojoTrapEvent* blocking_events)
{
    base::AutoLock lock(lock_);
    ...
    if (num_blocking_events) {
        ...
        *num_blocking_events = std::min(*num_blocking_events, static_cast<uint32_t>(ready_watches_.size()));
        ...
        if (last_watch_to_block_arming_) {
            ...
            next_ready_iter = ready_watches_.find(reinterpret_cast<const Watch*>(last_watch_to_block_arming_));
