...
struct MOJO_SYSTEM_IMPL_EXPORT HandleSignalsState final : public MojoHandleSignalsState {
    HandleSignalsState()
    {
    ...
    HandleSignalsState(MojoHandleSignals satisfied, MojoHandleSignals satisfiable)
    {
    ...
    {
        return satisfied_signals == other.satisfied_signals && satisfiable_signals == other.satisfiable_signals;
    }
    ...
    bool readable() const
    {
        return satisfies_any(MOJO_HANDLE_SIGNAL_READABLE);
    }
...
static_assert(sizeof(HandleSignalsState) == sizeof(MojoHandleSignalsState), "HandleSignalsState should add no overhead");
