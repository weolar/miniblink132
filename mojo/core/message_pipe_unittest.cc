...
public:
    ...
    MojoResult WriteMessage(MojoHandle message_pipe_handle, const void* bytes, uint32_t num_bytes)
    {
        return mojo::WriteMessageRaw(MessagePipeHandle(message_pipe_handle), bytes, num_bytes, nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE);
    }

    MojoResult ReadMessage(MojoHandle message_pipe_handle, void* bytes, uint32_t* num_bytes, bool may_discard = false)
    {
        MojoMessageHandle message_handle;
        MojoResult rv = MojoReadMessage(message_pipe_handle, nullptr, &message_handle);
        if (rv != MOJO_RESULT_OK)
            return rv;

        const uint32_t expected_num_bytes = *num_bytes;
        void* buffer;
        rv = MojoGetMessageData(message_handle, nullptr, &buffer, num_bytes, nullptr, nullptr);
...
{
    ASSERT_EQ(MOJO_RESULT_OK, WriteMessage(pipe0_, kHelloWorld, sizeof(kHelloWorld)));
}
...
TEST_F(MessagePipeTest, Basic)
{
    ...
    ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(pipe0_, MOJO_HANDLE_SIGNAL_READABLE, &state));
    ...
    ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(pipe1_, MOJO_HANDLE_SIGNAL_READABLE, &state));
    ...
    ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(pipe1_, MOJO_HANDLE_SIGNAL_PEER_CLOSED, &state));

    // Try to write from port 1 (to port 0).
    buffer[0] = 456789012;
    buffer[1] = 0;
    ASSERT_EQ(MOJO_RESULT_FAILED_PRECONDITION, WriteMessage(pipe1_, buffer, sizeof(buffer[0])));
    ...
    buffer_size = kBufferSize;
    ASSERT_EQ(MOJO_RESULT_FAILED_PRECONDITION, ReadMessage(pipe1_, buffer, &buffer_size));
}
...
TEST_F(MessagePipeTest, BasicWaiting)
{
    ...
    ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(pipe1_, MOJO_HANDLE_SIGNAL_READABLE, &hss));
    ASSERT_EQ(MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_WRITABLE, hss.satisfied_signals);
    ASSERT_EQ(kAllSignals, hss.satisfiable_signals);
    // ... and still writable.
    hss = MojoHandleSignalsState();
    ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(pipe1_, MOJO_HANDLE_SIGNAL_WRITABLE, &hss));
    ASSERT_EQ(MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_WRITABLE, hss.satisfied_signals);
    ASSERT_EQ(kAllSignals, hss.satisfiable_signals);
    ...
    hss = MojoHandleSignalsState();
    ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(pipe1_, MOJO_HANDLE_SIGNAL_PEER_CLOSED, &hss));
    ...
    ASSERT_EQ(MOJO_RESULT_FAILED_PRECONDITION, WaitForSignals(pipe1_, MOJO_HANDLE_SIGNAL_WRITABLE, &hss));
    ...
    hss = MojoHandleSignalsState();
    ASSERT_EQ(MOJO_RESULT_FAILED_PRECONDITION, WaitForSignals(pipe1_, MOJO_HANDLE_SIGNAL_READABLE, &hss));
...
TEST_F(FuseMessagePipeTest, NoFuseAfterWrite)
{
    ...
    EXPECT_EQ(MOJO_RESULT_FAILED_PRECONDITION, MojoFuseMessagePipes(b, c, nullptr));
...
TEST_F(FuseMessagePipeTest, NoFuseSelf)
{
    ...
    EXPECT_EQ(MOJO_RESULT_FAILED_PRECONDITION, MojoFuseMessagePipes(a, b, nullptr));
}
