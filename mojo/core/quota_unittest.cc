...
TEST_F(QuotaTest, InvalidArguments)
{
    ...
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoSetQuota(MOJO_HANDLE_INVALID, MOJO_QUOTA_TYPE_RECEIVE_QUEUE_LENGTH, 2, nullptr));
    ...
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoSetQuota(message_pipe0, kInvalidQuotaType, 0, nullptr));

    const MojoSetQuotaOptions kInvalidSetQuotaOptions = { 0 };
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoSetQuota(message_pipe0, MOJO_QUOTA_TYPE_RECEIVE_QUEUE_LENGTH, 0, &kInvalidSetQuotaOptions));

    uint64_t limit = 0;
    uint64_t usage = 0;
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoQueryQuota(message_pipe0, kInvalidQuotaType, nullptr, &limit, &usage));

    const MojoQueryQuotaOptions kInvalidQueryQuotaOptions = { 0 };
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoQueryQuota(message_pipe0, MOJO_QUOTA_TYPE_RECEIVE_QUEUE_LENGTH, &kInvalidQueryQuotaOptions, &limit, &usage));
    ...
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoSetQuota(producer, MOJO_QUOTA_TYPE_RECEIVE_QUEUE_LENGTH, 0, nullptr));
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoSetQuota(producer, MOJO_QUOTA_TYPE_RECEIVE_QUEUE_MEMORY_SIZE, 0, nullptr));
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoSetQuota(consumer, MOJO_QUOTA_TYPE_RECEIVE_QUEUE_LENGTH, 0, nullptr));
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoSetQuota(consumer, MOJO_QUOTA_TYPE_RECEIVE_QUEUE_MEMORY_SIZE, 0, nullptr));
    MojoClose(producer);
    MojoClose(consumer);
}
...
TEST_F(QuotaTest, BasicReceiveQueueLength)
{
    ...
    uint64_t limit = 0;
    uint64_t usage = 0;
    EXPECT_EQ(MOJO_RESULT_OK, MojoQueryQuota(a, MOJO_QUOTA_TYPE_RECEIVE_QUEUE_LENGTH, nullptr, &limit, &usage));
    EXPECT_EQ(MOJO_QUOTA_LIMIT_NONE, limit);
    EXPECT_EQ(0u, usage);

    const uint64_t kTestLimit = 42;
    EXPECT_EQ(MOJO_RESULT_OK, MojoSetQuota(a, MOJO_QUOTA_TYPE_RECEIVE_QUEUE_LENGTH, kTestLimit, nullptr));
...
TEST_F(QuotaTest, BasicReceiveQueueMemorySize)
{
    ...
    uint64_t limit = 0;
    uint64_t usage = 0;
    EXPECT_EQ(MOJO_RESULT_OK, MojoQueryQuota(a, MOJO_QUOTA_TYPE_RECEIVE_QUEUE_MEMORY_SIZE, nullptr, &limit, &usage));
    EXPECT_EQ(MOJO_QUOTA_LIMIT_NONE, limit);
    EXPECT_EQ(0u, usage);

    const uint64_t kTestLimit = 42;
    EXPECT_EQ(MOJO_RESULT_OK, MojoSetQuota(a, MOJO_QUOTA_TYPE_RECEIVE_QUEUE_MEMORY_SIZE, kTestLimit, nullptr));
...
TEST_F(QuotaTest, ReceiveQueueLengthLimitExceeded)
{
    ...
    EXPECT_EQ(MOJO_RESULT_OK, MojoSetQuota(b, MOJO_QUOTA_TYPE_RECEIVE_QUEUE_LENGTH, kMaxMessages, nullptr));
    ...
    uint64_t limit = 0;
    uint64_t usage = 0;
    EXPECT_EQ(MOJO_RESULT_OK, MojoQueryQuota(b, MOJO_QUOTA_TYPE_RECEIVE_QUEUE_LENGTH, nullptr, &limit, &usage));
    ...
    WaitForSignals(b, MOJO_HANDLE_SIGNAL_QUOTA_EXCEEDED, MOJO_TRIGGER_CONDITION_SIGNALS_UNSATISFIED);
...
TEST_F(QuotaTest, ReceiveQueueMemorySizeLimitExceeded)
{
    ...
    EXPECT_EQ(MOJO_RESULT_OK, MojoSetQuota(b, MOJO_QUOTA_TYPE_RECEIVE_QUEUE_MEMORY_SIZE, kMaxMessageBytes, nullptr));
    ...
    uint64_t limit = 0;
    uint64_t usage = 0;
    EXPECT_EQ(MOJO_RESULT_OK, MojoQueryQuota(b, MOJO_QUOTA_TYPE_RECEIVE_QUEUE_MEMORY_SIZE, nullptr, &limit, &usage));
...
TEST_F(QuotaTest, BasicUnreadMessageCount)
{
    ...
    uint64_t limit = 0;
    uint64_t usage = 0;
    EXPECT_EQ(MOJO_RESULT_OK, MojoQueryQuota(a, MOJO_QUOTA_TYPE_UNREAD_MESSAGE_COUNT, nullptr, &limit, &usage));
    EXPECT_EQ(MOJO_QUOTA_LIMIT_NONE, limit);
    EXPECT_EQ(0u, usage);

    const uint64_t kTestLimit = 42;
    EXPECT_EQ(MOJO_RESULT_OK, MojoSetQuota(a, MOJO_QUOTA_TYPE_UNREAD_MESSAGE_COUNT, kTestLimit, nullptr));
...
TEST_F(QuotaTest, UnreadMessageCountLimitExceeded)
{
    ...
    EXPECT_EQ(MOJO_RESULT_OK, MojoSetQuota(a, MOJO_QUOTA_TYPE_UNREAD_MESSAGE_COUNT, kMaxUnreadMessageCount, nullptr));
    ...
    EXPECT_EQ(kTestMessage, ReadMessage(b));
    EXPECT_EQ(kTestMessage, ReadMessage(b));
    WaitForSignals(a, MOJO_HANDLE_SIGNAL_QUOTA_EXCEEDED, MOJO_TRIGGER_CONDITION_SIGNALS_UNSATISFIED);
...
TEST_F(QuotaTest, TrapQuotaExceeded)
{
    ...
    EXPECT_EQ(MOJO_RESULT_OK, MojoCreateTrap(&QuotaExceededEventHandler, nullptr, &quota_trap));
    EXPECT_EQ(MOJO_RESULT_OK,
        MojoAddTrigger(quota_trap, b, MOJO_HANDLE_SIGNAL_QUOTA_EXCEEDED, MOJO_TRIGGER_CONDITION_SIGNALS_SATISFIED,
