...
#endif
    base::FilePath executable_dir = base::CommandLine::ForCurrentProcess()->GetProgram().DirName();
...
int main(int argc, char** argv)
{
    ...
    if (command_line.HasSwitch(switches::kMojoLoadBeforeInit)) {
        ...
        CHECK_EQ(MOJO_RESULT_OK, mojo::LoadAndInitializeCoreLibrary(library_path, flags));
    }

    int result = base::LaunchUnitTests(argc, argv, base::BindOnce(&base::TestSuite::Run, base::Unretained(&test_suite)));
