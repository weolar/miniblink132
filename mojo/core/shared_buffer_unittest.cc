...
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(CreateAndPassBufferParent, SharedBufferTest, parent)
{
...
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(ReceiveAndEditBufferParent, SharedBufferTest, parent)
{
...
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(ReadAndMapWriteSharedBuffer, SharedBufferTest, h)
{
    ...
    if (IsMojoIpczEnabled()) {
        ...
        EXPECT_EQ(buffer->region().GetMode(), base::subtle::PlatformSharedMemoryRegion::Mode::kReadOnly);
    } else {
        auto* dispatcher = static_cast<SharedBufferDispatcher*>(Core::Get()->GetDispatcher(b).get());
        base::subtle::PlatformSharedMemoryRegion& region = dispatcher->GetRegionForTesting();
        EXPECT_EQ(region.GetMode(), base::subtle::PlatformSharedMemoryRegion::Mode::kReadOnly);
        EXPECT_EQ(MOJO_RESULT_OK, MojoClose(b));
    }
...
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(CreateAndPassReadOnlyBuffer, SharedBufferTest, h)
{
    // Create a new shared buffer.
    MojoHandle b = CreateBuffer(1234);
    WriteToBuffer(b, 0, "hello");
