...
void RevalidateCreateOptions(const MojoCreateSharedBufferOptions& validated_options)
{
    ...
    EXPECT_EQ(MOJO_RESULT_OK, SharedBufferDispatcher::ValidateCreateOptions(&validated_options, &revalidated_options));
...
public:
    ...
    SharedBufferDispatcherTest& operator=(const SharedBufferDispatcherTest&) = delete;
...
TEST_F(SharedBufferDispatcherTest, ValidateCreateOptionsValid)
{
    // Default options.
    {
        ...
        EXPECT_EQ(MOJO_RESULT_OK, SharedBufferDispatcher::ValidateCreateOptions(nullptr, &validated_options));
        RevalidateCreateOptions(validated_options);
    }

    // Different flags.
    MojoCreateSharedBufferFlags flags_values[] = { MOJO_CREATE_SHARED_BUFFER_FLAG_NONE };
    for (size_t i = 0; i < std::size(flags_values); i++) {
        ...
        for (uint32_t capacity = 1; capacity <= 100 * 1000 * 1000; capacity *= 10) {
            ...
            MojoCreateSharedBufferOptions validated_options = {};
            EXPECT_EQ(MOJO_RESULT_OK, SharedBufferDispatcher::ValidateCreateOptions(&options, &validated_options)) << capacity;
            RevalidateCreateOptions(validated_options);
...
TEST_F(SharedBufferDispatcherTest, ValidateCreateOptionsInvalid)
{
    // Invalid |struct_size|.
    {
        ...
        EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, SharedBufferDispatcher::ValidateCreateOptions(&options, &unused));
    }
    ...
    // Unknown |flags|.
    {
        ...
        MojoCreateSharedBufferOptions unused;
        EXPECT_EQ(MOJO_RESULT_UNIMPLEMENTED, SharedBufferDispatcher::ValidateCreateOptions(&options, &unused));
    }
}
...
TEST_F(SharedBufferDispatcherTest, CreateAndMapBuffer)
{
    ...
    EXPECT_EQ(MOJO_RESULT_OK, SharedBufferDispatcher::Create(SharedBufferDispatcher::kDefaultCreateOptions, nullptr, 100, &dispatcher));
...
{
    base::WritableSharedMemoryRegion region = base::WritableSharedMemoryRegion::Create(100);
    ASSERT_TRUE(region.IsValid());
    scoped_refptr<SharedBufferDispatcher> dispatcher;
    EXPECT_EQ(MOJO_RESULT_OK,
        SharedBufferDispatcher::CreateFromPlatformSharedMemoryRegion(
            base::WritableSharedMemoryRegion::TakeHandleForSerialization(std::move(region)), &dispatcher));
    ASSERT_TRUE(dispatcher);
    EXPECT_EQ(Dispatcher::Type::SHARED_BUFFER, dispatcher->GetType());
...
TEST_F(SharedBufferDispatcherTest, DuplicateBufferHandle)
{
    ...
    EXPECT_EQ(MOJO_RESULT_OK, SharedBufferDispatcher::Create(SharedBufferDispatcher::kDefaultCreateOptions, nullptr, 100, &dispatcher1));
    ...
    EXPECT_EQ(MOJO_RESULT_OK, dispatcher1->DuplicateBufferHandle(nullptr, &dispatcher2));
...
TEST_F(SharedBufferDispatcherTest, DuplicateBufferHandleOptionsValid)
{
    ...
    EXPECT_EQ(MOJO_RESULT_OK, SharedBufferDispatcher::Create(SharedBufferDispatcher::kDefaultCreateOptions, nullptr, 100, &dispatcher2));

    MojoDuplicateBufferHandleOptions kReadOnlyOptions = { sizeof(MojoCreateSharedBufferOptions), MOJO_DUPLICATE_BUFFER_HANDLE_FLAG_READ_ONLY };
    ...
    EXPECT_EQ(MOJO_RESULT_OK, dispatcher1->DuplicateBufferHandle(nullptr, &writable_duped_dispatcher1));
    EXPECT_TRUE(writable_duped_dispatcher1);
    EXPECT_EQ(Dispatcher::Type::SHARED_BUFFER, writable_duped_dispatcher1->GetType());
    {
        std::unique_ptr<PlatformSharedMemoryMapping> mapping;
        EXPECT_EQ(MOJO_RESULT_OK, writable_duped_dispatcher1->MapBuffer(0, 100, &mapping));
    }
    EXPECT_EQ(MOJO_RESULT_FAILED_PRECONDITION, dispatcher1->DuplicateBufferHandle(&kReadOnlyOptions, &read_only_duped_dispatcher1));
    ...
    EXPECT_EQ(MOJO_RESULT_OK, dispatcher2->DuplicateBufferHandle(&kReadOnlyOptions, &read_only_duped_dispatcher2));
    EXPECT_TRUE(read_only_duped_dispatcher2);
    EXPECT_EQ(Dispatcher::Type::SHARED_BUFFER, read_only_duped_dispatcher2->GetType());
    {
        std::unique_ptr<PlatformSharedMemoryMapping> mapping;
        EXPECT_EQ(MOJO_RESULT_OK, read_only_duped_dispatcher2->MapBuffer(0, 100, &mapping));
    }
    EXPECT_EQ(MOJO_RESULT_FAILED_PRECONDITION, dispatcher2->DuplicateBufferHandle(nullptr, &writable_duped_dispatcher2));
...
TEST_F(SharedBufferDispatcherTest, DuplicateBufferHandleOptionsInvalid)
{
    ...
    // Invalid |struct_size|.
    {
        MojoDuplicateBufferHandleOptions options = { 1u, MOJO_DUPLICATE_BUFFER_HANDLE_FLAG_NONE };
        scoped_refptr<Dispatcher> dispatcher2;
        EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, dispatcher1->DuplicateBufferHandle(&options, &dispatcher2));
        EXPECT_FALSE(dispatcher2);
    }

    // Unknown |flags|.
    {
        MojoDuplicateBufferHandleOptions options = { sizeof(MojoDuplicateBufferHandleOptions), ~0u };
        scoped_refptr<Dispatcher> dispatcher2;
        EXPECT_EQ(MOJO_RESULT_UNIMPLEMENTED, dispatcher1->DuplicateBufferHandle(&options, &dispatcher2));
        EXPECT_FALSE(dispatcher2);
    }
...
TEST_F(SharedBufferDispatcherTest, CreateInvalidNumBytes)
{
    ...
    EXPECT_EQ(MOJO_RESULT_RESOURCE_EXHAUSTED,
        SharedBufferDispatcher::Create(SharedBufferDispatcher::kDefaultCreateOptions, nullptr, std::numeric_limits<uint64_t>::max(), &dispatcher));
    EXPECT_FALSE(dispatcher);

    // Zero size.
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, SharedBufferDispatcher::Create(SharedBufferDispatcher::kDefaultCreateOptions, nullptr, 0, &dispatcher));
    EXPECT_FALSE(dispatcher);
}
...
TEST_F(SharedBufferDispatcherTest, MapBufferInvalidArguments)
{
    ...
    std::unique_ptr<PlatformSharedMemoryMapping> mapping;
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, dispatcher->MapBuffer(0, info.size + 1, &mapping));
    EXPECT_FALSE(mapping);

    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, dispatcher->MapBuffer(1, info.size, &mapping));
    EXPECT_FALSE(mapping);

    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, dispatcher->MapBuffer(0, 0, &mapping));
    EXPECT_FALSE(mapping);
