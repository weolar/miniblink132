...
void ExtractPlatformHandlesFromSharedMemoryRegionHandle(
    base::subtle::ScopedPlatformSharedMemoryHandle handle, PlatformHandle* extracted_handle, PlatformHandle* extracted_readonly_handle)
{
#if BUILDFLAG(IS_WIN)
...
base::subtle::ScopedPlatformSharedMemoryHandle CreateSharedMemoryRegionHandleFromPlatformHandles(PlatformHandle handle, PlatformHandle readonly_handle)
{
#if BUILDFLAG(IS_WIN)
...
MojoResult UnwrapAndClonePlatformProcessHandle(const MojoPlatformProcessHandle* process_handle, base::Process& process)
{
    if (process_handle->struct_size < sizeof(*process_handle))
        return MOJO_RESULT_INVALID_ARGUMENT;

#if BUILDFLAG(IS_WIN)
    base::ProcessHandle in_handle = reinterpret_cast<base::ProcessHandle>(static_cast<uintptr_t>(process_handle->value));
#else
    base::ProcessHandle in_handle = static_cast<base::ProcessHandle>(process_handle->value);
#endif
    ...
    if (!::DuplicateHandle(::GetCurrentProcess(), in_handle, ::GetCurrentProcess(), &out_handle, 0, FALSE, DUPLICATE_SAME_ACCESS)) {
        return MOJO_RESULT_INVALID_ARGUMENT;
    }
    process = base::Process(out_handle);
#elif BUILDFLAG(IS_FUCHSIA)
    zx::process out;
    if (zx::unowned_process(in_handle)->duplicate(ZX_RIGHT_SAME_RIGHTS, &out) != ZX_OK) {
        return MOJO_RESULT_INVALID_ARGUMENT;
    }
