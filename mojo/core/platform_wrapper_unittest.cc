...
#define SHARED_BUFFER_PLATFORM_HANDLE_TYPE MOJO_PLATFORM_HANDLE_TYPE_FUCHSIA_HANDLE
#elif BUILDFLAG(IS_MAC)
...
TEST_F(PlatformWrapperTest, WrapPlatformHandle)
{
    ...
    EXPECT_EQ(base::WriteFile(temp_file_path, kMessage.data(), static_cast<int>(kMessage.size())), static_cast<int>(kMessage.size()));
    ...
    RunTestClient("ReadPlatformFile", [&](MojoHandle h) {
        ...
        base::File file(temp_file_path, base::File::FLAG_OPEN | base::File::FLAG_READ);
        ...
        os_file.value = PlatformHandleValueFromPlatformFile(file.TakePlatformFile());
        EXPECT_EQ(MOJO_RESULT_OK, MojoWrapPlatformHandle(&os_file, nullptr, &wrapped_handle));
...
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(ReadPlatformFile, PlatformWrapperTest, h)
{
    ...
    ASSERT_EQ(MOJO_RESULT_OK, MojoUnwrapPlatformHandle(wrapped_handle, nullptr, &platform_handle));
    ...
    EXPECT_EQ(file.ReadAtCurrentPos(data.data(), static_cast<int>(data.size())), static_cast<int>(data.size()));
...
TEST_F(PlatformWrapperTest, WrapPlatformSharedMemoryRegion)
{
    ...
    RunTestClient("ReadPlatformSharedBuffer", [&](MojoHandle h) {
        ...
        // expected message.
        base::subtle::PlatformSharedMemoryRegion platform_region = base::UnsafeSharedMemoryRegion::TakeHandleForSerialization(region.Duplicate());
...
#if BUILDFLAG(IS_WIN)
        os_buffer.value = reinterpret_cast<uint64_t>(platform_region.PassPlatformHandle().Take());
...
#error Unsupported platform
        ...
        ASSERT_EQ(MOJO_RESULT_OK,
            MojoWrapPlatformSharedMemoryRegion(
                &os_buffer, 1, kMessage.size(), &mojo_guid, MOJO_PLATFORM_SHARED_MEMORY_REGION_ACCESS_MODE_UNSAFE, nullptr, &wrapped_handle));
        WriteMessageWithHandles(h, kMessage, &wrapped_handle, 1);
        ...
        // receiving end.
        WriteMessageRaw(MessagePipeHandle(h), &mojo_guid, sizeof(mojo_guid), nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE);
    });
}

DEFINE_TEST_CLIENT_TEST_WITH_PIPE(ReadPlatformSharedBuffer, PlatformWrapperTest, h)
{
    ...
    ASSERT_EQ(MOJO_RESULT_OK, MojoUnwrapPlatformSharedMemoryRegion(wrapped_handle, nullptr, &os_buffer, &num_handles, &size, &mojo_guid, &access_mode));
    ...
    base::UnguessableToken guid = base::UnguessableToken::Deserialize(mojo_guid.high, mojo_guid.low);
#if BUILDFLAG(IS_WIN)
    ASSERT_EQ(MOJO_PLATFORM_HANDLE_TYPE_WINDOWS_HANDLE, os_buffer.type);
    auto platform_handle = base::win::ScopedHandle(reinterpret_cast<HANDLE>(os_buffer.value));
    ...
    auto platform_handle = base::mac::ScopedMachSendRight(static_cast<mach_port_t>(os_buffer.value));
#elif BUILDFLAG(IS_POSIX)
...
#endif
    base::subtle::PlatformSharedMemoryRegion platform_region = base::subtle::PlatformSharedMemoryRegion::Take(std::move(platform_handle), mode, size, guid);
    base::UnsafeSharedMemoryRegion region = base::UnsafeSharedMemoryRegion::Deserialize(std::move(platform_region));
    ASSERT_TRUE(region.IsValid());

    base::WritableSharedMemoryMapping mapping = region.Map();
    ASSERT_TRUE(mapping.memory());
    EXPECT_TRUE(std::equal(message.begin(), message.end(), static_cast<const char*>(mapping.memory())));
    ...
    EXPECT_EQ(MOJO_RESULT_OK, ReadMessageRaw(MessagePipeHandle(h), &guid_bytes, nullptr, MOJO_READ_MESSAGE_FLAG_NONE));
    EXPECT_EQ(sizeof(MojoSharedBufferGuid), guid_bytes.size());
    auto* expected_guid = reinterpret_cast<MojoSharedBufferGuid*>(guid_bytes.data());
...
TEST_F(PlatformWrapperTest, InvalidHandle)
{
    ...
    EXPECT_EQ(MOJO_RESULT_OK, MojoWrapPlatformHandle(&invalid_handle, nullptr, &wrapped_handle));
    EXPECT_EQ(MOJO_RESULT_OK, MojoUnwrapPlatformHandle(wrapped_handle, nullptr, &invalid_handle));
...
TEST_F(PlatformWrapperTest, InvalidArgument)
{
    ...
    EXPECT_EQ(MOJO_RESULT_INVALID_ARGUMENT, MojoWrapPlatformHandle(&platform_handle, nullptr, &wrapped_handle));
}
