...
scoped_refptr<NodeChannel> CreateNodeChannel(NodeChannel::Delegate* delegate, PlatformChannelEndpoint endpoint)
{
    return NodeChannel::Create(delegate, ConnectionParams(std::move(endpoint)), Channel::HandlePolicy::kAcceptHandles, GetIOTaskRunner(), base::NullCallback());
}
...
TEST_F(NodeChannelTest, DestructionIsSafe)
{
    ...
    auto local_channel = CreateNodeChannel(&local_delegate, channel.TakeLocalEndpoint());
    local_channel->Start();
    MockNodeChannelDelegate remote_delegate;
    auto remote_channel = CreateNodeChannel(&remote_delegate, channel.TakeRemoteEndpoint());
    ...
    base::RunLoop loop;
    EXPECT_CALL(local_delegate, OnAcceptInvitee(ports::kInvalidNodeName, kRemoteNodeName, kToken)).WillRepeatedly([&] { loop.Quit(); });
    ...
    EXPECT_CALL(remote_delegate, OnChannelError).WillOnce([&] { error_loop.Quit(); });
    local_channel.reset();
    error_loop.Run();
}
...
TEST_F(NodeChannelTest, MessagesCannotBeSmallerThanOldestVersion)
{
    ...
    auto message = Channel::Message::CreateMessage(capacity, capacity, /*num_handles=*/0);
