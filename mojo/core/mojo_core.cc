...
public:
    ...
    ~IPCSupport()
    {
        ...
        mojo::core::Core::Get()->RequestShutdown(base::BindOnce(&base::WaitableEvent::Signal, base::Unretained(&wait)));
        wait.Wait();
    }
...
MojoResult InitializeImpl(const struct MojoInitializeOptions* options)
{
    ...
    const bool should_initialize_ipc_support = !options || ((options->flags & MOJO_INITIALIZE_FLAG_LOAD_ONLY) == 0);
    ...
    const bool was_global_state_already_initialized = !global_state_initializer.Initialize(argc, argv);
    ...
    config.is_broker_process = options && options->flags & MOJO_INITIALIZE_FLAG_AS_BROKER;
    config.force_direct_shared_memory_allocation = options && options->flags & MOJO_INITIALIZE_FLAG_FORCE_DIRECT_SHARED_MEMORY_ALLOCATION;
