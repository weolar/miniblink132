...
MojoResult MojoReadMessage(MojoHandle pipe, void* out_bytes, uint32_t* num_bytes, MojoHandle* out_handles, uint32_t* num_handles, MojoReadMessageFlags flags)
{
    std::vector<uint8_t> bytes;
    std::vector<ScopedHandle> handles;
    MojoResult rv = ReadMessageRaw(MessagePipeHandle(pipe), &bytes, &handles, flags);
...
MojoResult MojoWriteMessage(
    MojoHandle pipe, const void* bytes, uint32_t num_bytes, const MojoHandle* handles, uint32_t num_handles, MojoWriteMessageFlags flags)
{
    return WriteMessageRaw(MessagePipeHandle(pipe), bytes, num_bytes, handles, num_handles, flags);
}
...
protected:
    ...
    public:
        explicit CommandDrivenClientController(MojoHandle h)
            : h_(h)
        {
        }
...
class MultiprocessMessagePipeTestWithPeerSupport : public MultiprocessMessagePipeTest, public testing::WithParamInterface<test::MojoTestBase::LaunchType> {
protected:
    void SetUp() override
    {
        ...
        const bool is_peer_launch = GetParam() == test::MojoTestBase::LaunchType::PEER;
#if BUILDFLAG(IS_FUCHSIA)
        const bool is_named_peer_launch = false;
#else
        const bool is_named_peer_launch = GetParam() == test::MojoTestBase::LaunchType::NAMED_PEER;
#endif
...
DEFINE_TEST_CLIENT_WITH_PIPE(EchoEcho, MultiprocessMessagePipeTest, h)
{
    ...
    for (;; rv = (rv + 1) % 100) {
        ...
        CHECK_EQ(MojoReadMessage(h, &read_buffer[0], &read_buffer_size, nullptr, 0, MOJO_READ_MESSAGE_FLAG_NONE), MOJO_RESULT_OK);
        ...
        CHECK_EQ(
            MojoWriteMessage(h, write_buffer.data(), static_cast<uint32_t>(write_buffer.size()), nullptr, 0u, MOJO_WRITE_MESSAGE_FLAG_NONE), MOJO_RESULT_OK);
    }
...
TEST_P(MultiprocessMessagePipeTestWithPeerSupport, Basic)
{
    int exit_code = RunTestClientAndGetExitCode("EchoEcho", [&](MojoHandle h) {
        ...
        ASSERT_EQ(MOJO_RESULT_OK, MojoWriteMessage(h, hello.data(), static_cast<uint32_t>(hello.size()), nullptr, 0u, MOJO_WRITE_MESSAGE_FLAG_NONE));

        HandleSignalsState hss;
        ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(h, MOJO_HANDLE_SIGNAL_READABLE, &hss));
        ...
        CHECK_EQ(
            MojoWriteMessage(h, quitquitquit.data(), static_cast<uint32_t>(quitquitquit.size()), nullptr, 0u, MOJO_WRITE_MESSAGE_FLAG_NONE), MOJO_RESULT_OK);
    });
    ...
    int exit_code = RunTestClientAndGetExitCode("EchoEcho", [&](MojoHandle h) {
        for (size_t i = 0; i < kNumMessages; i++) {
            ...
            ASSERT_EQ(MOJO_RESULT_OK,
                MojoWriteMessage(h, write_buffer.data(), static_cast<uint32_t>(write_buffer.size()), nullptr, 0u, MOJO_WRITE_MESSAGE_FLAG_NONE));
        }
        ...
        for (size_t i = 0; i < kNumMessages; i++) {
            ...
            uint32_t read_buffer_size = static_cast<uint32_t>(read_buffer.size());
            ASSERT_EQ(MojoReadMessage(h, &read_buffer[0], &read_buffer_size, nullptr, 0, MOJO_READ_MESSAGE_FLAG_NONE), MOJO_RESULT_OK);
            read_buffer.resize(read_buffer_size);
        ...
        const std::string quitquitquit("quitquitquit");
        ASSERT_EQ(
            MOJO_RESULT_OK, MojoWriteMessage(h, quitquitquit.data(), static_cast<uint32_t>(quitquitquit.size()), nullptr, 0u, MOJO_WRITE_MESSAGE_FLAG_NONE));

        // Wait for it to become readable, which should fail (since we sent
        // "quitquitquit").
        HandleSignalsState hss;
        ASSERT_EQ(MOJO_RESULT_FAILED_PRECONDITION, WaitForSignals(h, MOJO_HANDLE_SIGNAL_READABLE, &hss));
...
DEFINE_TEST_CLIENT_WITH_PIPE(CheckSharedBuffer, MultiprocessMessagePipeTest, h)
{
    // Wait for the first message from our parent.
    HandleSignalsState hss;
    CHECK_EQ(WaitForSignals(h, MOJO_HANDLE_SIGNAL_READABLE, &hss), MOJO_RESULT_OK);
    ...
    CHECK_EQ(MojoReadMessage(h, &read_buffer[0], &num_bytes, &handles[0], &num_handlers, MOJO_READ_MESSAGE_FLAG_NONE), MOJO_RESULT_OK);
    ...
    CHECK_EQ(MojoWriteMessage(h, go2.data(), static_cast<uint32_t>(go2.size()), nullptr, 0u, MOJO_WRITE_MESSAGE_FLAG_NONE), MOJO_RESULT_OK);
    ...
    CHECK_EQ(MojoReadMessage(h, &read_buffer[0], &num_bytes, nullptr, 0, MOJO_READ_MESSAGE_FLAG_NONE), MOJO_RESULT_OK);
    read_buffer.resize(num_bytes);
...
TEST_F(MultiprocessMessagePipeTest, SharedBufferPassing)
{
    RunTestClient("CheckSharedBuffer", [&](MojoHandle h) {
        ...
        ASSERT_EQ(MOJO_RESULT_OK, MojoCreateSharedBuffer(100, &options, &shared_buffer));
        MojoSharedBufferInfo buffer_info;
        buffer_info.struct_size = sizeof(buffer_info);
        ASSERT_EQ(MOJO_RESULT_OK, MojoGetBufferInfo(shared_buffer, nullptr, &buffer_info));
        ...
        ASSERT_EQ(MOJO_RESULT_OK, MojoDuplicateBufferHandle(shared_buffer, nullptr, &duplicated_shared_buffer));
        buffer_info.size = 0;
        ASSERT_EQ(MOJO_RESULT_OK, MojoGetBufferInfo(shared_buffer, nullptr, &buffer_info));
        EXPECT_GE(buffer_info.size, 100U);
        ...
        ASSERT_EQ(
            MOJO_RESULT_OK, MojoWriteMessage(h, &go1[0], static_cast<uint32_t>(go1.size()), &handles[0], std::size(handles), MOJO_WRITE_MESSAGE_FLAG_NONE));
        ...
        std::string read_buffer(100, '\0');
        uint32_t num_bytes = static_cast<uint32_t>(read_buffer.size());
        ASSERT_EQ(MOJO_RESULT_OK, MojoReadMessage(h, &read_buffer[0], &num_bytes, nullptr, 0, MOJO_READ_MESSAGE_FLAG_NONE));
        read_buffer.resize(num_bytes);
        ...
        static const char kHello[] = "hello";
        void* buffer;
        CHECK_EQ(MojoMapBuffer(shared_buffer, 0, 100, nullptr, &buffer), MOJO_RESULT_OK);
        ...
        ASSERT_EQ(MOJO_RESULT_OK, MojoWriteMessage(h, &go3[0], static_cast<uint32_t>(go3.size()), nullptr, 0u, MOJO_WRITE_MESSAGE_FLAG_NONE));
...
DEFINE_TEST_CLIENT_WITH_PIPE(CheckPlatformHandleFile, MultiprocessMessagePipeTest, h)
{
    HandleSignalsState hss;
    CHECK_EQ(WaitForSignals(h, MOJO_HANDLE_SIGNAL_READABLE, &hss), MOJO_RESULT_OK);
    CHECK(hss.satisfied_signals & MOJO_HANDLE_SIGNAL_READABLE);
    ...
    for (int i = 0; i < num_handles; ++i) {
        PlatformHandle handle = UnwrapPlatformHandle(ScopedHandle(Handle(handles[i])));
...
class MultiprocessMessagePipeTestWithPipeCount : public MultiprocessMessagePipeTest, public testing::WithParamInterface<size_t> { };
...
TEST_P(MultiprocessMessagePipeTestWithPipeCount, PlatformHandlePassing)
{
    ...
    RunTestClient("CheckPlatformHandleFile", [&](MojoHandle h) {
        ...
        for (size_t i = 0; i < pipe_count; ++i) {
            ...
            base::ScopedFILE fp = CreateAndOpenTemporaryStreamInDir(temp_dir.GetPath(), &unused);
            const std::string world("world");
            CHECK_EQ(fwrite(&world[0], 1, world.size(), fp.get()), world.size());
            fflush(fp.get());
            rewind(fp.get());
            ScopedHandle handle = WrapPlatformHandle(test::PlatformHandleFromFILE(std::move(fp)));
            ASSERT_TRUE(handle.is_valid());
            handles.push_back(handle.release().value());
        }

        char message[128];
        snprintf(message, sizeof(message), "hello %d", static_cast<int>(pipe_count));
        ASSERT_EQ(MOJO_RESULT_OK,
            MojoWriteMessage(
                h, message, static_cast<uint32_t>(strlen(message)), &handles[0], static_cast<uint32_t>(handles.size()), MOJO_WRITE_MESSAGE_FLAG_NONE));
...
INSTANTIATE_TEST_SUITE_P(PipeCount, MultiprocessMessagePipeTestWithPipeCount,
...
DEFINE_TEST_CLIENT_WITH_PIPE(CheckMessagePipe, MultiprocessMessagePipeTest, h)
{
    ...
    MojoHandle handles[10];
    uint32_t num_handlers = std::size(handles);
    CHECK_EQ(MojoReadMessage(h, nullptr, nullptr, &handles[0], &num_handlers, MOJO_READ_MESSAGE_FLAG_NONE), MOJO_RESULT_OK);
    CHECK_EQ(num_handlers, 1u);

    // Read data from the received message pipe.
    CHECK_EQ(WaitForSignals(handles[0], MOJO_HANDLE_SIGNAL_READABLE, &hss), MOJO_RESULT_OK);
    CHECK(hss.satisfied_signals & MOJO_HANDLE_SIGNAL_READABLE);
    CHECK(hss.satisfied_signals & MOJO_HANDLE_SIGNAL_WRITABLE);

    std::string read_buffer(100, '\0');
    uint32_t read_buffer_size = static_cast<uint32_t>(read_buffer.size());
    CHECK_EQ(MojoReadMessage(handles[0], &read_buffer[0], &read_buffer_size, nullptr, 0, MOJO_READ_MESSAGE_FLAG_NONE), MOJO_RESULT_OK);
    read_buffer.resize(read_buffer_size);
    CHECK_EQ(read_buffer, std::string("hello"));

    // Now write some data into the message pipe.
    std::string write_buffer = "world";
    CHECK_EQ(MojoWriteMessage(handles[0], write_buffer.data(), static_cast<uint32_t>(write_buffer.size()), nullptr, 0u, MOJO_WRITE_MESSAGE_FLAG_NONE),
        MOJO_RESULT_OK);
    MojoClose(handles[0]);
    return 0;
}
...
TEST_P(MultiprocessMessagePipeTestWithPeerSupport, MessagePipePassing)
{
    RunTestClient("CheckMessagePipe", [&](MojoHandle h) {
        ...
        ASSERT_EQ(MOJO_RESULT_OK, MojoWriteMessage(mp1, &hello[0], static_cast<uint32_t>(hello.size()), nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE));
        ASSERT_EQ(MOJO_RESULT_OK, MojoWriteMessage(h, nullptr, 0, &mp2, 1, MOJO_WRITE_MESSAGE_FLAG_NONE));

        // Wait for a message from the child.
        HandleSignalsState hss;
        ASSERT_EQ(MOJO_RESULT_OK, WaitForSignals(mp1, MOJO_HANDLE_SIGNAL_READABLE, &hss));
        EXPECT_TRUE((hss.satisfied_signals & MOJO_HANDLE_SIGNAL_READABLE));
        EXPECT_TRUE((hss.satisfiable_signals & MOJO_HANDLE_SIGNAL_READABLE));

        std::string read_buffer(100, '\0');
        uint32_t read_buffer_size = static_cast<uint32_t>(read_buffer.size());
        CHECK_EQ(MojoReadMessage(mp1, &read_buffer[0], &read_buffer_size, nullptr, 0, MOJO_READ_MESSAGE_FLAG_NONE), MOJO_RESULT_OK);
        read_buffer.resize(read_buffer_size);
...
TEST_P(MultiprocessMessagePipeTestWithPeerSupport, MessagePipeTwoPassing)
{
    RunTestClient("CheckMessagePipe", [&](MojoHandle h) {
        ...
        // end.
        const std::string hello("hello");
        ASSERT_EQ(MOJO_RESULT_OK, MojoWriteMessage(mp1, &hello[0], static_cast<uint32_t>(hello.size()), nullptr, 0u, MOJO_WRITE_MESSAGE_FLAG_NONE));
        ASSERT_EQ(MOJO_RESULT_OK, MojoWriteMessage(h, nullptr, 0u, &mp2, 1u, MOJO_WRITE_MESSAGE_FLAG_NONE));
...
DEFINE_TEST_CLIENT_WITH_PIPE(DataPipeConsumer, MultiprocessMessagePipeTest, h)
{
    ...
    CHECK_EQ(hss.satisfiable_signals,
        MOJO_HANDLE_SIGNAL_READABLE | MOJO_HANDLE_SIGNAL_WRITABLE | MOJO_HANDLE_SIGNAL_PEER_CLOSED | MOJO_HANDLE_SIGNAL_PEER_REMOTE
...
TEST_F(MultiprocessMessagePipeTest, DataPipeConsumer)
{
    RunTestClient("DataPipeConsumer", [&](MojoHandle h) {
        ...
        const std::string hello("hello");
        ASSERT_EQ(MOJO_RESULT_OK, MojoWriteMessage(mp1, &hello[0], static_cast<uint32_t>(hello.size()), nullptr, 0u, MOJO_WRITE_MESSAGE_FLAG_NONE));
        ASSERT_EQ(MOJO_RESULT_OK, MojoWriteMessage(h, nullptr, 0, &mp2, 1u, MOJO_WRITE_MESSAGE_FLAG_NONE));
...
DEFINE_TEST_CLIENT_WITH_PIPE(ChannelEchoClient, MultiprocessMessagePipeTest, h)
{
    for (;;) {
...
DEFINE_TEST_CLIENT_WITH_PIPE(EchoServiceClient, MultiprocessMessagePipeTest, h)
{
    MojoHandle p;
...
DEFINE_TEST_CLIENT_WITH_PIPE(EchoServiceFactoryClient, MultiprocessMessagePipeTest, h)
{
    MojoHandle p;
    ReadMessageWithHandles(h, &p, 1);
    ...
    for (;;) {
        size_t index;
        CHECK_EQ(mojo::WaitMany(handles.data(), signals.data(), handles.size(), &index), MOJO_RESULT_OK);
...
TEST_P(MultiprocessMessagePipeTestWithPeerSupport, PassMoarMessagePipesCrossProcess)
{
    ...
    RunTestClient("EchoServiceFactoryClient", [&](MojoHandle h) {
        WriteMessageWithHandles(h, "gief factory naow plz", &echo_factory_request, 1);

        WriteMessageWithHandles(echo_factory_proxy, "give me an echo service plz!", &echo_request_a, 1);
        WriteMessageWithHandles(echo_factory_proxy, "give me one too!", &echo_request_b, 1);
        ...
        WriteMessageWithHandles(echo_factory_proxy, "hook me up also thanks", &echo_request_c, 1);
...
#if BUILDFLAG(IS_ANDROID)
#define MAYBE_ChannelPipesWithMultipleChildren DISABLED_ChannelPipesWithMultipleChildren
#else
#define MAYBE_ChannelPipesWithMultipleChildren ChannelPipesWithMultipleChildren
#endif
TEST_P(MultiprocessMessagePipeTestWithPeerSupport, MAYBE_ChannelPipesWithMultipleChildren)
{
...
// transient proxies.
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(PingPongPipeClient, MultiprocessMessagePipeTest, h)
{
...
DEFINE_TEST_CLIENT_WITH_PIPE(CommandDrivenClient, MultiprocessMessagePipeTest, h)
{
    std::unordered_map<std::string, MojoHandle> named_pipes;
    for (;;) {
        MojoHandle p;
        auto parts = base::SplitString(ReadMessageWithOptionalHandle(h, &p), ":", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);
        CHECK(!parts.empty());
...
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(ReceivePipeWithClosedPeer, MultiprocessMessagePipeTest, h)
{
    MojoHandle p;
...
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(SendOtherChildPipeWithClosedPeer, MultiprocessMessagePipeTest, h)
{
...
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(ReceivePipeWithClosedPeerFromOtherChild, MultiprocessMessagePipeTest, h)
{
    ...
    EXPECT_EQ("c2s lol", ReadMessageWithHandles(application_client, &service_client, 1));

    // Wait for the service client to signal closure.
    EXPECT_EQ(MOJO_RESULT_OK, WaitForSignals(service_client, MOJO_HANDLE_SIGNAL_PEER_CLOSED));
...
#if BUILDFLAG(IS_ANDROID)
// Android multi-process tests are not executing the new process. This is flaky.
#define MAYBE_SendPipeWithClosedPeerBetweenChildren DISABLED_SendPipeWithClosedPeerBetweenChildren
#else
#define MAYBE_SendPipeWithClosedPeerBetweenChildren SendPipeWithClosedPeerBetweenChildren
#endif
TEST_F(MultiprocessMessagePipeTest, MAYBE_SendPipeWithClosedPeerBetweenChildren)
{
    RunTestClient("SendOtherChildPipeWithClosedPeer", [&](MojoHandle kid_a) {
        RunTestClient("ReceivePipeWithClosedPeerFromOtherChild", [&](MojoHandle kid_b) {
            // Receive an "application request" from the first child and forward
            // it to the second child.
            MojoHandle application_request;
            EXPECT_EQ("c2a plz", ReadMessageWithHandles(kid_a, &application_request, 1));
...
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(WriteCloseSendPeerClient, MultiprocessMessagePipeTest, h)
{
    MojoHandle pipe[2];
...
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(MessagePipeStatusChangeInTransitClient, MultiprocessMessagePipeTest, parent)
{
    ...
    EXPECT_EQ(MOJO_RESULT_OK, WaitForSignals(handles[0], MOJO_HANDLE_SIGNAL_PEER_CLOSED));
    ...
    // Wait on handle 1 using a SimpleWatcher.
    {
        ...
        SimpleWatcher watcher(FROM_HERE, SimpleWatcher::ArmingPolicy::AUTOMATIC, base::SequencedTaskRunnerHandle::Get());
    ...
    MojoResult result;
    do {
        result = MojoReadMessage(handles[2], nullptr, nullptr, nullptr, nullptr, MOJO_READ_MESSAGE_FLAG_NONE);
    } while (result == MOJO_RESULT_SHOULD_WAIT);
    EXPECT_EQ(MOJO_RESULT_FAILED_PRECONDITION, result);

    // Wait on handle 3 by polling with MojoWriteMessage.
    do {
        result = MojoWriteMessage(handles[3], nullptr, 0, nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE);
...
TEST_P(MultiprocessMessagePipeTestWithPeerSupport, ReceiveMessagesSentJustBeforeProcessDeath)
{
...
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(SpotaneouslyDyingProcess, MultiprocessMessagePipeTest, parent)
{
    MojoHandle sender;
    MojoHandle receiver;
...
TEST_F(MultiprocessMessagePipeTest, MessagePipeStatusChangeInTransit)
{
    ...
    RunTestClient("MessagePipeStatusChangeInTransitClient", [&](MojoHandle child) {
...
DEFINE_TEST_CLIENT_TEST_WITH_PIPE(BadMessageClient, MultiprocessMessagePipeTest, parent)
{
    MojoHandle pipe;
...
INSTANTIATE_TEST_SUITE_P(All, MultiprocessMessagePipeTestWithPeerSupport,
    testing::Values(test::MojoTestBase::LaunchType::CHILD, test::MojoTestBase::LaunchType::CHILD_WITHOUT_CAPABILITIES, test::MojoTestBase::LaunchType::PEER,
        test::MojoTestBase::LaunchType::ASYNC
#if !BUILDFLAG(IS_FUCHSIA)
        // Fuchsia has no named pipe support.
        ,
        test::MojoTestBase::LaunchType::NAMED_CHILD, test::MojoTestBase::LaunchType::NAMED_PEER
