...
static_assert(sizeof(SerializedState) % 8 == 0, "Invalid SerializedState size.");
...
class DataPipeProducerDispatcher::PortObserverThunk : public NodeController::PortObserver {
public:
    explicit PortObserverThunk(scoped_refptr<DataPipeProducerDispatcher> dispatcher)
...
private:
    ...
    {
        dispatcher_->OnPortStatusChanged();
    }
...
// static
scoped_refptr<DataPipeProducerDispatcher> DataPipeProducerDispatcher::Create(NodeController* node_controller, const ports::PortRef& control_port,
    base::UnsafeSharedMemoryRegion shared_ring_buffer, const MojoCreateDataPipeOptions& options, uint64_t pipe_id)
{
    scoped_refptr<DataPipeProducerDispatcher> producer
        = new DataPipeProducerDispatcher(node_controller, control_port, std::move(shared_ring_buffer), options, pipe_id);
...
MojoResult DataPipeProducerDispatcher::WriteData(const void* elements, uint32_t* num_bytes, const MojoWriteDataOptions& options)
{
    base::AutoLock lock(lock_);
    ...
    if ((options.flags & MOJO_WRITE_DATA_FLAG_ALL_OR_NONE) && (*num_bytes > available_capacity_)) {
    ...
    *num_bytes = num_bytes_to_write;
    ...
    uint32_t tail_bytes_to_write = std::min(options_.capacity_num_bytes - write_offset_, num_bytes_to_write);
    ...
    write_offset_ = (write_offset_ + num_bytes_to_write) % options_.capacity_num_bytes;
...
MojoResult DataPipeProducerDispatcher::BeginWriteData(void** buffer, uint32_t* buffer_num_bytes, MojoBeginWriteDataFlags flags)
{
    base::AutoLock lock(lock_);
    if (!shared_ring_buffer_.IsValid() || in_transit_)
        return MOJO_RESULT_INVALID_ARGUMENT;
    ...
    if (available_capacity_ == 0) {
        return peer_closed_ ? MOJO_RESULT_FAILED_PRECONDITION : MOJO_RESULT_SHOULD_WAIT;
    }

    in_two_phase_write_ = true;
    *buffer_num_bytes = std::min(options_.capacity_num_bytes - write_offset_, available_capacity_);
...
MojoResult DataPipeProducerDispatcher::EndWriteData(uint32_t num_bytes_written)
{
    base::AutoLock lock(lock_);
    ...
    if (num_bytes_written > available_capacity_ || num_bytes_written % options_.element_num_bytes != 0
        ...
        write_offset_ = (write_offset_ + num_bytes_written) % options_.capacity_num_bytes;
...
MojoResult DataPipeProducerDispatcher::AddWatcherRef(const scoped_refptr<WatcherDispatcher>& watcher, uintptr_t context)
{
    base::AutoLock lock(lock_);
    if (is_closed_ || in_transit_)
        return MOJO_RESULT_INVALID_ARGUMENT;
...
MojoResult DataPipeProducerDispatcher::RemoveWatcherRef(WatcherDispatcher* watcher, uintptr_t context)
{
    base::AutoLock lock(lock_);
    if (is_closed_ || in_transit_)
        return MOJO_RESULT_INVALID_ARGUMENT;
...
void DataPipeProducerDispatcher::StartSerialize(uint32_t* num_bytes, uint32_t* num_ports, uint32_t* num_handles)
{
    base::AutoLock lock(lock_);
    DCHECK(in_transit_);
...
bool DataPipeProducerDispatcher::EndSerialize(void* destination, ports::PortName* ports, PlatformHandle* platform_handles)
{
    ...
    auto region_handle = base::UnsafeSharedMemoryRegion::TakeHandleForSerialization(std::move(shared_ring_buffer_));
    ...
    ExtractPlatformHandlesFromSharedMemoryRegionHandle(region_handle.PassPlatformHandle(), &handle, &ignored_handle);
...
// static
scoped_refptr<DataPipeProducerDispatcher> DataPipeProducerDispatcher::Deserialize(
    const void* data, size_t num_bytes, const ports::PortName* ports, size_t num_ports, PlatformHandle* handles, size_t num_handles)
{
    if (num_ports != 1 || num_handles != 1 || num_bytes != sizeof(SerializedState)) {
        AssertNotExtractingHandlesFromMessage();
        return nullptr;
    }

    const SerializedState* state = static_cast<const SerializedState*>(data);
    if (!state->options.capacity_num_bytes || !state->options.element_num_bytes || state->options.capacity_num_bytes < state->options.element_num_bytes
        || state->write_offset >= state->options.capacity_num_bytes || state->available_capacity > state->options.capacity_num_bytes) {
        AssertNotExtractingHandlesFromMessage();
        return nullptr;
    }
    ...
    auto region_handle = CreateSharedMemoryRegionHandleFromPlatformHandles(std::move(handles[0]), PlatformHandle());
    auto region = base::subtle::PlatformSharedMemoryRegion::Take(std::move(region_handle), base::subtle::PlatformSharedMemoryRegion::Mode::kUnsafe,
        state->options.capacity_num_bytes, base::UnguessableToken::Deserialize(state->buffer_guid_high, state->buffer_guid_low));
    auto ring_buffer = base::UnsafeSharedMemoryRegion::Deserialize(std::move(region));
    ...
    scoped_refptr<DataPipeProducerDispatcher> dispatcher
        = new DataPipeProducerDispatcher(node_controller, port, std::move(ring_buffer), state->options, state->pipe_id);

    {
        ...
        if (state->options.capacity_num_bytes > dispatcher->ring_buffer_mapping_.mapped_size()) {
            AssertNotExtractingHandlesFromMessage();
            return nullptr;
        }
...
DataPipeProducerDispatcher::DataPipeProducerDispatcher(NodeController* node_controller, const ports::PortRef& control_port,
    base::UnsafeSharedMemoryRegion shared_ring_buffer, const MojoCreateDataPipeOptions& options, uint64_t pipe_id)
    : options_(options)
...
{
    DCHECK(is_closed_ && !in_transit_ && !shared_ring_buffer_.IsValid() && !ring_buffer_mapping_.IsValid());
}
...
bool DataPipeProducerDispatcher::InitializeNoLock()
{
    ...
    base::AutoUnlock unlock(lock_);
    node_controller_->SetPortObserver(control_port_, base::MakeRefCounted<PortObserverThunk>(this));
...
HandleSignalsState DataPipeProducerDispatcher::GetHandleSignalsStateNoLock() const
{
    lock_.AssertAcquired();
    HandleSignalsState rv;
    if (!peer_closed_) {
        if (!in_two_phase_write_ && shared_ring_buffer_.IsValid() && available_capacity_ > 0)
            rv.satisfied_signals |= MOJO_HANDLE_SIGNAL_WRITABLE;
        if (peer_remote_)
            rv.satisfied_signals |= MOJO_HANDLE_SIGNAL_PEER_REMOTE;
        rv.satisfiable_signals |= MOJO_HANDLE_SIGNAL_WRITABLE | MOJO_HANDLE_SIGNAL_PEER_REMOTE;
    } else {
...
{
    DVLOG(1) << "Data pipe producer " << pipe_id_ << " notifying peer: " << num_bytes << " bytes written. [control_port=" << control_port_.name() << "]";

    SendDataPipeControlMessage(node_controller_, control_port_, DataPipeCommand::DATA_WAS_WRITTEN, num_bytes);
}
...
void DataPipeProducerDispatcher::UpdateSignalsStateNoLock()
{
    ...
    if (rv != ports::OK || !port_status.receiving_messages) {
        ...
        do {
            rv = node_controller_->node()->GetMessage(control_port_, &message_event, nullptr);
            if (rv != ports::OK)
                peer_closed_ = true;
            if (message_event) {
                ...
                const DataPipeControlMessage* m = static_cast<const DataPipeControlMessage*>(message->user_payload());
                ...
                TRACE_EVENT0("ipc", "DataPipeProducerDispatcher received DATA_WAS_READ");
                ...
                DVLOG(1) << "Data pipe producer " << pipe_id_ << " is aware that " << m->num_bytes << " bytes were read. [control_port=" << control_port_.name()
                         << "]";
    ...
    if (peer_closed_ != was_peer_closed || available_capacity_ != previous_capacity || was_peer_remote != peer_remote_) {
        watchers_.NotifyState(GetHandleSignalsStateNoLock());
    }
}
