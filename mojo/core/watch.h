...
public:
    ...
    Watch(const scoped_refptr<WatcherDispatcher>& watcher, const scoped_refptr<Dispatcher>& dispatcher, uintptr_t context, MojoHandleSignals signals,
    ...
    bool NotifyState(const HandleSignalsState& state, bool allowed_to_call_callback);
    ...
    void InvokeCallback(MojoResult result, const HandleSignalsState& state, MojoTrapEventFlags flags);

    const scoped_refptr<Dispatcher>& dispatcher() const
    {
        return dispatcher_;
    }
    ...
    {
        AssertWatcherLockAcquired();
        return last_known_result_ == MOJO_RESULT_OK || last_known_result_ == MOJO_RESULT_FAILED_PRECONDITION;
    }
