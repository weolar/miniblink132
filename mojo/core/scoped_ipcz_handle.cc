...
ScopedIpczHandle::ScopedIpczHandle(IpczHandle handle)
    : handle_(handle)
{
}
...
{
    if (is_valid()) {
        const IpczResult result = GetIpczAPI().Close(std::exchange(handle_, IPCZ_INVALID_HANDLE), IPCZ_NO_FLAGS, nullptr);
