...
void InitFeatures()
{
    ...
    Channel::set_posix_use_writev(base::FeatureList::IsEnabled(kMojoPosixUseWritev));

#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_ANDROID)
    bool shared_mem_enabled = base::FeatureList::IsEnabled(kMojoLinuxChannelSharedMem);
    ...
    ChannelLinux::SetSharedMemParameters(shared_mem_enabled, static_cast<unsigned int>(num_pages), use_zero_on_wake);
#endif // BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) ||                                                                                                     \
    // BUILDFLAG(IS_ANDROID)
#endif // BUILDFLAG(IS_POSIX) && !BUILDFLAG(IS_NACL) && !BUILDFLAG(IS_MAC)

    Channel::set_use_trivial_messages(base::FeatureList::IsEnabled(kMojoInlineMessagePayloads));

    Core::set_avoid_random_pipe_id(base::FeatureList::IsEnabled(kMojoAvoidRandomPipeId));
    ...
    const bool kIsIpczSupported = base::win::GetVersion() >= base::win::Version::WIN8_1;
#else
...
void Init(const Configuration& configuration)
{
    ...
    if (IsMojoIpczEnabled()) {
        CHECK(InitializeIpczNodeForProcess({
            ...
            .use_local_shared_memory_allocation = configuration.is_broker_process || configuration.force_direct_shared_memory_allocation,
        }));
