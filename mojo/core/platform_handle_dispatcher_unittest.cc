...
TEST(PlatformHandleDispatcherTest, Basic)
{
    ...
    ASSERT_TRUE(fp);
    EXPECT_EQ(sizeof(kHelloWorld), fwrite(kHelloWorld, 1, sizeof(kHelloWorld), fp.get()));

    PlatformHandle h = test::PlatformHandleFromFILE(std::move(fp));
    EXPECT_FALSE(fp);
    ASSERT_TRUE(h.is_valid());

    scoped_refptr<PlatformHandleDispatcher> dispatcher = PlatformHandleDispatcher::Create(std::move(h));
    ...
    EXPECT_EQ(sizeof(kHelloWorld), fread(read_buffer, 1, sizeof(read_buffer), fp.get()));
...
TEST(PlatformHandleDispatcherTest, Serialization)
{
    ...
    scoped_refptr<PlatformHandleDispatcher> dispatcher = PlatformHandleDispatcher::Create(test::PlatformHandleFromFILE(std::move(fp)));
    ...
    dispatcher = static_cast<PlatformHandleDispatcher*>(
        Dispatcher::Deserialize(Dispatcher::Type::PLATFORM_HANDLE, nullptr, num_bytes, nullptr, num_ports, &received_handle, 1u).get());
    ...
    rewind(fp.get());
    char read_buffer[1000] = {};
    EXPECT_EQ(sizeof(kFooBar), fread(read_buffer, 1, sizeof(read_buffer), fp.get()));
