...
protected:
    ...
    {
        CHECK_EQ(WriteMessageRaw(MessagePipeHandle(mp), payload_.data(), payload_.size(), nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE), MOJO_RESULT_OK);
        HandleSignalsState hss;
        CHECK_EQ(WaitForSignals(mp, MOJO_HANDLE_SIGNAL_READABLE, &hss), MOJO_RESULT_OK);
        CHECK_EQ(ReadMessageRaw(MessagePipeHandle(mp), &read_buffer_, nullptr, MOJO_READ_MESSAGE_FLAG_NONE), MOJO_RESULT_OK);
        CHECK_EQ(read_buffer_.size(), payload_.size());
    }

    void SendQuitMessage(MojoHandle mp)
    {
        CHECK_EQ(WriteMessageRaw(MessagePipeHandle(mp), "", 0, nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE), MOJO_RESULT_OK);
    }
    ...
    void Measure(MojoHandle mp)
    {
        ...
        std::string test_name = base::StringPrintf("IPC_Perf_%dx_%u", message_count_, static_cast<unsigned>(message_size_));
...
protected:
    ...
    static int RunPingPongClient(MojoHandle mp)
    {
        ...
        while (true) {
            ...
            CHECK_EQ(ReadMessageRaw(MessagePipeHandle(mp), &buffer, nullptr, MOJO_READ_MESSAGE_FLAG_NONE), MOJO_RESULT_OK);

            // Empty message indicates quit.
            if (buffer.empty())
                break;

            CHECK_EQ(WriteMessageRaw(MessagePipeHandle(mp), buffer.data(), buffer.size(), nullptr, 0, MOJO_WRITE_MESSAGE_FLAG_NONE), MOJO_RESULT_OK);
        }
...
TEST_F(MessagePipePerfTest, PingPong)
{
    ...
    client_thread.Start();
    client_thread.task_runner()->PostTask(FROM_HERE, base::BindOnce(base::IgnoreResult(&RunPingPongClient), client_handle));
