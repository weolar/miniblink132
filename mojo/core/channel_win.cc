...
{
    base::debug::GlobalActivityTracker* activity_tracker = base::debug::GlobalActivityTracker::Get();
    if (!activity_tracker)
        return nullptr;

    static std::atomic<uint64_t>* sum = activity_tracker->process_data().SetUint("channel_win_total_outgoing_messages", 0u);
...
public:
    ...
    Channel::Message* GetFirst() const
    {
...
class ChannelWin : public Channel, public base::CurrentThread::DestructionObserver, public base::MessagePumpForIO::IOHandler {
public:
    ChannelWin(Delegate* delegate, ConnectionParams connection_params, HandlePolicy handle_policy, scoped_refptr<base::SingleThreadTaskRunner> io_task_runner)
        ...
        , io_task_runner_(io_task_runner)
    {
        if (connection_params.server_endpoint().is_valid()) {
            handle_ = connection_params.TakeServerEndpoint().TakePlatformHandle().TakeHandle();
    ...
    void Start() override
    {
        io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&ChannelWin::StartOnIOThread, this));
    }

    void ShutDownImpl() override
    {
        // Always shut down asynchronously when called through the public interface.
        io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&ChannelWin::ShutDownOnIOThread, this));
    }
    ...
    void Write(MessagePtr message) override
    {
            ...
            for (auto& handle : handles) {
                if (handle.handle().is_valid()) {
                    handle.TransferToProcess(remote_process().Duplicate(),
                        is_untrusted_process_ ? PlatformHandleInTransit::kUntrustedTarget : PlatformHandleInTransit::kTrustedTarget);
                }
            }
        ...
        if (write_error) {
            ...
            io_task_runner_->PostTask(FROM_HERE, base::BindOnce(&ChannelWin::OnWriteError, this, Error::kDisconnected));
        }
    }
        ...
        std::vector<PlatformHandle>* handles, bool* deferred) override
    {
        ...
        const HandleEntry* extra_header_handles = reinterpret_cast<const HandleEntry*>(extra_header);
        for (size_t i = 0; i < num_handles; i++) {
            HANDLE handle_value = base::win::Uint32ToHandle(extra_header_handles[i].handle);
            ...
            if (remote_process().IsValid() && handle_value != INVALID_HANDLE_VALUE) {
                ...
                handle_value = PlatformHandleInTransit::TakeIncomingRemoteHandle(handle_value, remote_process().Handle()).ReleaseHandle();
            }
...
private:
    ...
    void OnIOCompleted(base::MessagePumpForIO::IOContext* context, DWORD bytes_transfered, DWORD error) override
    {
    ...
    bool WriteNoLock(Channel::Message* message)
    {
        ...
        BOOL ok = WriteFile(handle_.Get(), message->data(), static_cast<DWORD>(message->data_num_bytes()), NULL, &write_context_.overlapped);
        if (ok || GetLastError() == ERROR_IO_PENDING) {
...
{
    return new ChannelWin(delegate, std::move(connection_params), handle_policy, io_task_runner);
}
