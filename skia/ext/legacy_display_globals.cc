...
void GetContrastAndGammaValuesFromCommandLine(float& out_contrast, float& out_gamma)
{
    base::CommandLine* command_line = base::CommandLine::ForCurrentProcess();
    std::string contrast_string = command_line->GetSwitchValueASCII(switches::kTextContrast);
    ...
    std::string gamma_string = command_line->GetSwitchValueASCII(switches::kTextGamma);
    double gamma_double;
...
// static
void LegacyDisplayGlobals::SetCachedParams(SkPixelGeometry pixel_geometry, float text_contrast, float text_gamma)
{
...
SkSurfaceProps LegacyDisplayGlobals::ComputeSurfaceProps(bool can_use_lcd_text)
{
    uint32_t flags = 0;
    if (can_use_lcd_text) {
