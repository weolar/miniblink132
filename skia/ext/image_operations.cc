...
class ResizeFilter {
public:
    ResizeFilter(ImageOperations::ResizeMethod method, int src_full_width, int src_full_height, int dest_width, int dest_height, const SkIRect& dest_subset);

    ResizeFilter(const ResizeFilter&) = delete;
    ResizeFilter& operator=(const ResizeFilter&) = delete;

    // Returns the filled filter values.
    const ConvolutionFilter1D& x_filter()
    {
        return x_filter_;
    }
    const ConvolutionFilter1D& y_filter()
    {
        return y_filter_;
    }
...
private:
    ...
    void ComputeFilters(int src_size, int dest_subset_lo, int dest_subset_size, float scale, ConvolutionFilter1D* output);
...
ResizeFilter::ResizeFilter(
    ImageOperations::ResizeMethod method, int src_full_width, int src_full_height, int dest_width, int dest_height, const SkIRect& dest_subset)
    : method_(method)
{
    ...
    ComputeFilters(src_full_height, dest_subset.fTop, dest_subset.height(), scale_y, &y_filter_);
}
...
void ResizeFilter::ComputeFilters(int src_size, int dest_subset_lo, int dest_subset_size, float scale, ConvolutionFilter1D* output)
{
    ...
    for (int dest_subset_i = dest_subset_lo; dest_subset_i < dest_subset_hi; dest_subset_i++) {
        ...
        for (int cur_filter_pixel = src_begin; cur_filter_pixel <= src_end; cur_filter_pixel++) {
        ...
        output->AddFilter(src_begin, &fixed_filter_values[0], static_cast<int>(fixed_filter_values.size()));
    }

    output->PaddingForSIMD();
}

ImageOperations::ResizeMethod ResizeMethodToAlgorithmMethod(ImageOperations::ResizeMethod method)
{
...
// static
SkBitmap ImageOperations::Resize(
    const SkPixmap& source, ResizeMethod method, int dest_width, int dest_height, const SkIRect& dest_subset, SkBitmap::Allocator* allocator)
{
    TRACE_EVENT2("disabled-by-default-skia", "ImageOperations::Resize", "src_pixels", source.width() * source.height(), "dst_pixels", dest_width * dest_height);
    // Ensure that the ResizeMethod enumeration is sound.
    SkASSERT(((RESIZE_FIRST_QUALITY_METHOD <= method) && (method <= RESIZE_LAST_QUALITY_METHOD))
        || ((RESIZE_FIRST_ALGORITHM_METHOD <= method) && (method <= RESIZE_LAST_ALGORITHM_METHOD)));
    ...
    ResizeFilter filter(method, source.width(), source.height(), dest_width, dest_height, dest_subset);

    // Get a source bitmap encompassing this touched area. We construct the
    // offsets and row strides such that it looks like a new bitmap, while
    // referring to the old data.
    const uint8_t* source_subset = reinterpret_cast<const uint8_t*>(source.addr());

    // Convolve into the result.
    SkBitmap result;
    result.setInfo(source.info().makeWH(dest_subset.width(), dest_subset.height()));
    if (!result.tryAllocPixels(allocator) || !result.readyToDraw())
        return SkBitmap();

    BGRAConvolve2D(source_subset, static_cast<int>(source.rowBytes()), !source.isOpaque(), filter.x_filter(), filter.y_filter(),
        static_cast<int>(result.rowBytes()), static_cast<unsigned char*>(result.getPixels()), true);
...
// static
SkBitmap ImageOperations::Resize(
    const SkBitmap& source, ResizeMethod method, int dest_width, int dest_height, const SkIRect& dest_subset, SkBitmap::Allocator* allocator)
{
    SkPixmap pixmap;
    if (!source.peekPixels(&pixmap))
        return SkBitmap();
    return Resize(pixmap, method, dest_width, dest_height, dest_subset, allocator);
}

// static
SkBitmap ImageOperations::Resize(const SkBitmap& source, ResizeMethod method, int dest_width, int dest_height, SkBitmap::Allocator* allocator)
{
    SkIRect dest_subset = { 0, 0, dest_width, dest_height };
    return Resize(source, method, dest_width, dest_height, dest_subset, allocator);
}
