...
bool ReadSkFontIdentity(base::PickleIterator* iter, SkFontConfigInterface::FontIdentity* identity)
{
    uint32_t reply_id;
...
bool ReadSkFontStyle(base::PickleIterator* iter, SkFontStyle* style)
{
    ...
    if (style) {
        *style = SkFontStyle(reply_weight, reply_width, static_cast<SkFontStyle::Slant>(reply_slant));
    }
    return true;
}
...
void WriteSkFontIdentity(base::Pickle* pickle, const SkFontConfigInterface::FontIdentity& identity)
{
...
{
    DCHECK(out);
    if (in.colorType() == kUnknown_SkColorType && in.alphaType() == kUnknown_SkAlphaType && in.empty() && in.isNull()) {
    ...
    if (stride_matches_width && info.colorType() == kN32_SkColorType && (info.alphaType() == kPremul_SkAlphaType || info.alphaType() == kOpaque_SkAlphaType)) {
    ...
    SkImageInfo new_info
        = info.makeColorType(kN32_SkColorType).makeAlphaType(info.alphaType() == kOpaque_SkAlphaType ? kOpaque_SkAlphaType : kPremul_SkAlphaType);
...
std::string SkColorToHexString(SkColor color)
{
    if (SkColorGetA(color) == 0xFF) {
        return base::StringPrintf("#%02X%02X%02X", SkColorGetR(color), SkColorGetG(color), SkColorGetB(color));
    } else {
        return base::StringPrintf("#%02X%02X%02X%02X", SkColorGetR(color), SkColorGetG(color), SkColorGetB(color), SkColorGetA(color));
    }
}
...
std::string SkColorSpaceToString(const SkColorSpace* cs)
{
    ...
    cs->transferFn(&trfn);
    return base::StringPrintf("{trfn:%s, toXYZD50:%s}", SkcmsTransferFunctionToString(trfn).c_str(), SkcmsMatrix3x3ToString(to_xyzd50).c_str());
}

std::string SkcmsTransferFunctionToString(const skcms_TransferFunction& fn)
{
    return base::StringPrintf("{%1.4f*x + %1.4f if abs(x) < %1.4f else "
...
{
    return base::StringPrintf("{rows:[[%1.4f,%1.4f,%1.4f],[%1.4f,%1.4f,%1.4f],[%1.4f,%1.4f,%1.4f]]}", m.vals[0][0], m.vals[0][1], m.vals[0][2], m.vals[1][0],
        m.vals[1][1], m.vals[1][2], m.vals[2][0], m.vals[2][1], m.vals[2][2]);
}
