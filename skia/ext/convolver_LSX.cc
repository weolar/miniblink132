...
namespace skia {
static __m128i emulate_lsx_set_epi16(uint16_t a, uint16_t b, uint16_t c, uint16_t d, uint16_t e, uint16_t f, uint16_t g, uint16_t h)
{
...
void ConvolveHorizontally_LSX(const unsigned char* src_data, const ConvolutionFilter1D& filter, unsigned char* out_row, bool /*has_alpha*/)
{
...
void Convolve4RowsHorizontally_LSX(const unsigned char* src_data[4], const ConvolutionFilter1D& filter, unsigned char* out_row[4])
{
    int num_values = filter.num_values();
    int filter_offset, filter_length;
    __m128i zero = __lsx_vldi(0);
    ...
    const ConvolutionFilter1D::Fixed* filter_values, int filter_length, unsigned char* const* source_data_rows, int pixel_width, unsigned char* out_row)
{
    ...
    for (int out_x = 0; out_x < width; out_x += 4) {
        ...
        for (int filter_y = 0; filter_y < filter_length; filter_y++) {
            ...
            // [8] a3 b3 g3 r3 a2 b2 g2 r2 a1 b1 g1 r1 a0 b0 g0 r0
            src = reinterpret_cast<const __m128i*>(&source_data_rows[filter_y][out_x << 2]);
    ...
    if (pixel_width & 3) {
        ...
        for (int filter_y = 0; filter_y < filter_length; ++filter_y) {
            ...
            coeff16 = __lsx_vldrepl_h(&values, 0);
            // [8] a3 b3 g3 r3 a2 b2 g2 r2 a1 b1 g1 r1 a0 b0 g0 r0
            src = reinterpret_cast<const __m128i*>(&source_data_rows[filter_y][width << 2]);
            __m128i src8 = LSX_LD(src);
            // [16] a1 b1 g1 r1 a0 b0 g0 r0
            __m128i src16 = __lsx_vilvl_b(zero, src8);
            __m128i mul_hi = __lsx_vmuh_h(src16, coeff16);
...
void ConvolveVertically_LSX(const ConvolutionFilter1D::Fixed* filter_values, int filter_length, unsigned char* const* source_data_rows, int pixel_width,
    unsigned char* out_row, bool has_alpha)
{
    if (has_alpha) {
        ConvolveVertically_LSX<true>(filter_values, filter_length, source_data_rows, pixel_width, out_row);
    } else {
        ConvolveVertically_LSX<false>(filter_values, filter_length, source_data_rows, pixel_width, out_row);
    }
}
} // namespace skia
