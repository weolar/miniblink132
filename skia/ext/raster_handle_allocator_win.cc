...
static void DeleteHDCCallback(void*, void* context)
{
    ...
    HBITMAP new_bitmap = static_cast<HBITMAP>(SelectObject(rec->hdc_, rec->prev_bitmap_));
...
// on error.
static bool Create(int width, int height, HANDLE shared_section, bool do_clear, SkRasterHandleAllocator::Rec* rec)
{
    void* pixels;
    base::win::ScopedBitmap new_bitmap = skia::CreateHBitmapXRGB8888(width, height, shared_section, &pixels);
    ...
    HBITMAP prev_bitmap = static_cast<HBITMAP>(SelectObject(hdc, new_bitmap.release()));
    DCHECK(prev_bitmap);
...
std::unique_ptr<SkCanvas> CreatePlatformCanvasWithSharedSection(int width, int height, bool is_opaque, HANDLE shared_section, OnFailureType failure_type)
{
    ...
    if (base::win::IsUser32AndGdi32Available()) {
        ...
        if (Create(width, height, shared_section, false, &rec))
            return SkRasterHandleAllocator::MakeCanvas(std::make_unique<GDIAllocator>(), info, &rec);
    } else {
        DCHECK(shared_section != NULL);
        void* pixels = MapViewOfFile(shared_section, FILE_MAP_WRITE, 0, 0, width * bpp * height);
        if (pixels) {
            SkBitmap bitmap;
            if (bitmap.installPixels(info, pixels, width * bpp, unmap_view_proc, nullptr)) {
                return std::make_unique<SkCanvas>(bitmap, LegacyDisplayGlobals::GetSkSurfaceProps());
            }
        }
    }
...
static HBITMAP CreateReturnPixels(int width, int height, uint8_t** pixels_out, const SkImageInfo& info, SkRasterHandleAllocator::Rec* rec)
{
    // The HBITMAP is 32-bit RGB data. A size_t causes a type change from int when
    // multiplying against the dimensions.
    const size_t bpp = 4;
    ...
    base::win::ScopedBitmap new_bitmap = CreateHBitmapXRGB8888(width, height, nullptr, (void**)(&pixels));
    if (!new_bitmap.is_valid()) {
        LOG(ERROR) << "CreateHBitmap failed";
        return nullptr;
    }
    ...
    SkBitmap bitmap;
    bitmap.installPixels(info, pixels, width * bpp, nullptr, nullptr);
    *pixels_out = pixels;
    ...
    // The |new_bitmap| will be destroyed by |rec|'s DeleteHDCCallback().
    HBITMAP new_h_bitmap = new_bitmap.release();
    HBITMAP prev_bitmap = static_cast<HBITMAP>(SelectObject(hdc, new_h_bitmap));
    DCHECK(prev_bitmap);
...
std::unique_ptr<SkCanvas> CreatePlatformCanvasReturnPixels(int width, int height, bool is_opaque, uint8_t** pixels)
{
    SkAlphaType alpha = is_opaque ? kOpaque_SkAlphaType : kPremul_SkAlphaType;
    SkImageInfo info = SkImageInfo::MakeN32(width, height, alpha);
    SkRasterHandleAllocator::Rec rec;
    if (HBITMAP new_h_bitmap = CreateReturnPixels(width, height, pixels, info, &rec))
        return SkRasterHandleAllocator::MakeCanvas(std::make_unique<GDIAllocator>(), info, &rec);
    return nullptr;
}
