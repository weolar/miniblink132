...
uint32_t AveragePixel(const SkBitmap& bmp, int x_min, int x_max, int y_min, int y_max)
{
    ...
    return SkColorSetARGB(static_cast<unsigned char>(accum[3] / count), static_cast<unsigned char>(accum[2] / count),
        static_cast<unsigned char>(accum[1] / count), static_cast<unsigned char>(accum[0] / count));
}
...
SkColor AveragePixel(const SkColor colors[], size_t color_count)
{
    ...
    const SkColor average_color = SkColorSetARGB(static_cast<uint8_t>(accum[0] / color_count), static_cast<uint8_t>(accum[1] / color_count),
        static_cast<uint8_t>(accum[2] / color_count), static_cast<uint8_t>(accum[3] / color_count));
    return average_color;
}

void PrintPixel(const SkBitmap& bmp, int x_min, int x_max, int y_min, int y_max)
{
    char str[128];
...
void FillDataToBitmap(int w, int h, SkBitmap* bmp)
{
    ...
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
            ...
            const SkColor pixel = SkColorSetARGB(component, component, component, component);
...
void DrawCheckerToBitmap(int w, int h, SkColor color1, SkColor color2, int rect_w, int rect_h, SkBitmap* bmp)
{
    ASSERT_GT(rect_w, 0);
    ASSERT_GT(rect_h, 0);
...
{
    std::optional<std::vector<uint8_t>> png = gfx::PNGCodec::Encode(reinterpret_cast<const unsigned char*>(bmp.getPixels()), gfx::PNGCodec::FORMAT_RGBA,
        gfx::Size(bmp.width(), bmp.height()), static_cast<int>(bmp.rowBytes()), false, std::vector<gfx::PNGCodec::Comment>());
    if (!png) {
...
void CheckResizeMethodShouldAverageGrid(
    const SkBitmap& src, const TestedResizeMethod& tested_method, int dest_w, int dest_h, SkColor average_color, bool* method_passed)
{
    *method_passed = false;

    const TestedPixel tested_pixels[] = { // Corners
        { 0, 0, 2.3f, "Top left corner" }, { 0, dest_h - 1, 2.3f, "Bottom left corner" }, { dest_w - 1, 0, 2.3f, "Top right corner" },
        { dest_w - 1, dest_h - 1, 2.3f, "Bottom right corner" },
        // Middle points of each side
        { dest_w / 2, 0, 1.0f, "Top middle" }, { dest_w / 2, dest_h - 1, 1.0f, "Bottom middle" }, { 0, dest_h / 2, 1.0f, "Left middle" },
    ...
    for (size_t pixel_index = 0; pixel_index < std::size(tested_pixels); ++pixel_index) {
        const TestedPixel& tested_pixel = tested_pixels[pixel_index];

        const int x = tested_pixel.x;
        const int y = tested_pixel.y;
        const float max_allowed_distance = std::max(tested_pixel.max_color_distance, tested_method.max_color_distance_override);
        ...
        EXPECT_LE(distance, max_allowed_distance) << "Resizing method: " << tested_method.name << ", pixel tested: " << tested_pixel.name << "(" << x << ", "
                                                  << y << ")" << std::hex << std::showbase << ", expected (avg) hex: " << average_color
    ...
    if (!all_pixels_ok) {
        ADD_FAILURE() << "Maximum observed color distance for method " << tested_method.name << ": " << max_observed_distance;

#if DEBUG_BITMAP_GENERATION
        char path[128];
        base::snprintf(path, sizeof(path), "/tmp/ResizeShouldAverageColors_%s_dest.png", tested_method.name);
...
TEST(ImageOperations, Halve)
{
    ...
    SkBitmap actual_results = skia::ImageOperations::Resize(src, skia::ImageOperations::RESIZE_BOX, src_w / 2, src_h / 2);
    ASSERT_EQ(src_w / 2, actual_results.width());
    ASSERT_EQ(src_h / 2, actual_results.height());
    EXPECT_TRUE(actual_results.colorSpace() && actual_results.colorSpace()->isSRGB());
    ...
    for (int y = 0; y < actual_results.height(); y++) {
        for (int x = 0; x < actual_results.width(); x++) {
            ...
            const uint32_t expected_color = AveragePixel(src, first_x, last_x, first_y, last_y);
            const uint32_t actual_color = *actual_results.getAddr32(x, y);
            const bool close = ColorsClose(expected_color, actual_color);
            EXPECT_TRUE(close);
            if (!close) {
                char str[128];
                base::snprintf(str, sizeof(str), "exp[%d,%d] = %08X, actual[%d,%d] = %08X", x, y, expected_color, x, y, actual_color);
                ADD_FAILURE() << str;
...
TEST(ImageOperations, HalveSubset)
{
    ...
    // Do a halving of the full bitmap.
    SkBitmap full_results = skia::ImageOperations::Resize(src, skia::ImageOperations::RESIZE_BOX, src_w / 2, src_h / 2);
    ...
    SkBitmap subset_results = skia::ImageOperations::Resize(src, skia::ImageOperations::RESIZE_BOX, src_w / 2, src_h / 2, subset_rect);
    ASSERT_EQ(subset_rect.width(), subset_results.width());
    ASSERT_EQ(subset_rect.height(), subset_results.height());
    EXPECT_TRUE(subset_results.colorSpace() && subset_results.colorSpace()->isSRGB());
    ...
    for (int y = 0; y < subset_rect.height(); y++) {
        for (int x = 0; x < subset_rect.width(); x++) {
            ASSERT_EQ(*full_results.getAddr32(x + subset_rect.fLeft, y + subset_rect.fTop), *subset_results.getAddr32(x, y));
        }
    }
}
...
TEST(ImageOperations, InvalidParams)
{
    ...
    // Scale it, don't die.
    SkBitmap full_results = skia::ImageOperations::Resize(src, skia::ImageOperations::RESIZE_BOX, 10, 20);
}
...
TEST(ImageOperations, ResizeShouldAverageColors)
{
    ...
    SkBitmap src;
    DrawCheckerToBitmap(src_w, src_h, checker_color1, checker_color2, checker_rect_w, checker_rect_h, &src);
    ...
    for (size_t method_index = 0; method_index < std::size(tested_methods); ++method_index) {
        bool pass = true;
        CheckResizeMethodShouldAverageGrid(src, tested_methods[method_index], dest_w, dest_h, average_color, &pass);
        if (!pass) {
...
TEST(ImageOperations, ScaleUp)
{
    ...
    const int dst_h = 9;
    SkBitmap src;
    src.allocPixels(SkImageInfo::MakeN32Premul(src_w, src_h, SkColorSpace::MakeSRGB()));

    for (int src_y = 0; src_y < src_h; ++src_y) {
        for (int src_x = 0; src_x < src_w; ++src_x) {
            *src.getAddr32(src_x, src_y) = SkColorSetARGB(255, 10 + src_x * 100, 10 + src_y * 100, 0);
        }
    }

    SkBitmap dst = skia::ImageOperations::Resize(src, skia::ImageOperations::RESIZE_LANCZOS3, dst_w, dst_h);
