...
void CreateBitmapHeaderWithColorDepth(LONG width, LONG height, WORD color_depth, BITMAPINFOHEADER* hdr)
{
...
void CreateBitmapV5HeaderForARGB8888(LONG width, LONG height, LONG image_size, BITMAPV5HEADER* hdr)
{
...
{
    POINT win_point = { SkScalarRoundToInt(point.fX), SkScalarRoundToInt(point.fY) };
    return win_point;
}

SkRect RECTToSkRect(const RECT& rect)
{
    SkRect sk_rect = { SkIntToScalar(rect.left), SkIntToScalar(rect.top), SkIntToScalar(rect.right), SkIntToScalar(rect.bottom) };
    return sk_rect;
}
...
void CopyHDC(HDC source, HDC destination, int x, int y, bool is_opaque, const RECT& src_rect, const SkMatrix& transform)
{
    ...
    if (is_opaque) {
        BitBlt(destination, x, y, copy_width, copy_height, source, src_rect.left, src_rect.top, SRCCOPY);
    } else {
        SkASSERT(copy_width != 0 && copy_height != 0);
        BLENDFUNCTION blend_function = { AC_SRC_OVER, 0, 255, AC_SRC_ALPHA };
        GdiAlphaBlend(destination, x, y, copy_width, copy_height, source, src_rect.left, src_rect.top, copy_width, copy_height, blend_function);
    }
...
{
    HBITMAP backing_handle = static_cast<HBITMAP>(GetCurrentObject(context, OBJ_BITMAP));
    const size_t backing_size = sizeof *backing;
    return (GetObject(backing_handle, backing_size, backing) == backing_size) ? SkImageInfo::MakeN32Premul(backing->bmWidth, backing->bmHeight) : SkImageInfo();
}
...
sk_sp<SkSurface> MapPlatformSurface(HDC context)
{
    ...
    return size.isEmpty() ? nullptr : SkSurfaces::WrapPixels(size, backing.bmBits, backing.bmWidthBytes, &props);
}
...
void CreateBitmapHeaderForN32SkBitmap(const SkBitmap& bitmap, BITMAPINFOHEADER* hdr)
{
...
HGLOBAL CreateHGlobalForByteArray(const std::vector<unsigned char>& byte_array)
{
...
HGLOBAL CreateDIBV5ImageDataFromN32SkBitmap(const SkBitmap& bitmap)
{
    ...
    auto* dst_pixels = reinterpret_cast<uint8_t*>(header.data()) + sizeof(BITMAPV5HEADER);
    ...
    SkImageInfo infoSRGB = bitmap.info().makeColorSpace(SkColorSpace::MakeSRGB()).makeWH(bitmap.width(), 1);
...
base::win::ScopedBitmap CreateHBitmapFromN32SkBitmap(const SkBitmap& bitmap)
{
    ...
    HBITMAP hbitmap;
    {
        ...
        hbitmap = CreateDIBSection(screen_dc, reinterpret_cast<BITMAPINFO*>(&header), DIB_RGB_COLORS, &bits, nullptr, 0);
    }
    if (hbitmap) {
...
void CreateBitmapHeaderForXRGB888(int width, int height, BITMAPINFOHEADER* hdr)
{
    CreateBitmapHeaderWithColorDepth(width, height, 32, hdr);
}

base::win::ScopedBitmap CreateHBitmapXRGB8888(int width, int height, HANDLE shared_section, void** data)
{
    ...
    CreateBitmapHeaderWithColorDepth(width, height, 32, &hdr);
    HBITMAP hbitmap = CreateDIBSection(NULL, reinterpret_cast<BITMAPINFO*>(&hdr), 0, data, shared_section, 0);
