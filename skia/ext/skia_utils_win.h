...
SK_API void CopyHDC(HDC source, HDC destination, int x, int y, bool is_opaque, const RECT& src_rect, const SkMatrix& transform);
...
SK_API void CreateBitmapHeaderForN32SkBitmap(const SkBitmap& bitmap, BITMAPINFOHEADER* hdr);
...
SK_API HGLOBAL CreateHGlobalForByteArray(const std::vector<unsigned char>& byte_array);
...
SK_API base::win::ScopedBitmap CreateHBitmapFromN32SkBitmap(const SkBitmap& bitmap);
...
// will be for a bitmap with 32-bits-per-pixel RGB data (the high bits are
// unused in each pixel).
SK_API void CreateBitmapHeaderForXRGB888(int width, int height, BITMAPINFOHEADER* hdr);

// Creates an HBITMAP backed by 32-bits-per-pixel RGB data (the high bits are
// unused in each pixel).
SK_API base::win::ScopedBitmap CreateHBitmapXRGB8888(int width, int height, HANDLE shared_section = nullptr, void** data = nullptr);
