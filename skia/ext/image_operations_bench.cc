...
bool StringToMethod(const std::string& arg, skia::ImageOperations::ResizeMethod* method)
{
...
public:
    ...
    {
        std::vector<std::string_view> strings = base::SplitStringPiece(arg, "x", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
...
void Benchmark::Usage()
{
    printf("image_operations_bench -source wxh -destination wxh "
        ...
        Benchmark::kDefaultNumberIterations, MethodToString(Benchmark::kDefaultResizeMethod));
    PrintMethods();
...
bool Benchmark::ParseArgs(const base::CommandLine* command_line)
{
    ...
    for (base::CommandLine::SwitchMap::const_iterator iter = switches.begin(); iter != switches.end(); ++iter) {
...
bool Benchmark::Run() const
{
    ...
    for (int i = 0; i < num_iterations_; ++i) {
        dest = skia::ImageOperations::Resize(source, method_, dest_.width(), dest_.height());
    }
    ...
    printf("%" PRIu64 " MB/s,\telapsed = %" PRIu64 " source=%d dest=%d\n", static_cast<uint64_t>(elapsed_us == 0 ? 0 : num_bytes / elapsed_us),
        static_cast<uint64_t>(elapsed_us), GetBitmapSize(&source), GetBitmapSize(&dest));
