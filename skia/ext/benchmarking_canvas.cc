...
base::Value AsValue(const SkPaint& paint)
{
    ...
    if (paint.getStyle() != default_paint.getStyle()) {
        ...
        DCHECK_LT(static_cast<size_t>(paint.getStyle()), std::size(gStyleStrings));
...
{
    static const char* gOpStrings[] = { "Difference", "Intersect", "Union", "XOR", "ReverseDifference", "Replace" };
...
base::Value AsValue(const SkPath& path)
{
    ...
    static_assert(std::size(gVerbStrings) == static_cast<size_t>(SkPath::kDone_Verb + 1), "gVerbStrings size mismatch");
    static_assert(std::size(gVerbStrings) == std::size(gPtsPerVerb), "gPtsPerVerb size mismatch");
    static_assert(std::size(gVerbStrings) == std::size(gPtOffsetPerVerb), "gPtOffsetPerVerb size mismatch");
...
template <typename T> base::Value AsListValue(const T array[], size_t count)
{
    base::Value::List val;
...
public:
    ...
    AutoOp(BenchmarkingCanvas* canvas, const char op_name[], const SkPaint* paint = nullptr)
        : canvas_(canvas)
    {
        DCHECK(canvas);
        DCHECK(op_name);
...
SkCanvas::SaveLayerStrategy BenchmarkingCanvas::getSaveLayerStrategy(const SaveLayerRec& rec)
{
...
void BenchmarkingCanvas::onClipRect(const SkRect& rect, SkClipOp region_op, SkCanvas::ClipEdgeStyle style)
{
...
void BenchmarkingCanvas::onClipRRect(const SkRRect& rrect, SkClipOp region_op, SkCanvas::ClipEdgeStyle style)
{
...
void BenchmarkingCanvas::onClipPath(const SkPath& path, SkClipOp region_op, SkCanvas::ClipEdgeStyle style)
{
...
void BenchmarkingCanvas::onClipRegion(const SkRegion& region, SkClipOp region_op)
{
...
void BenchmarkingCanvas::onDrawPoints(PointMode mode, size_t count, const SkPoint pts[], const SkPaint& paint)
{
...
void BenchmarkingCanvas::onDrawDRRect(const SkRRect& outer, const SkRRect& inner, const SkPaint& paint)
{
...
void BenchmarkingCanvas::onDrawPicture(const SkPicture* picture, const SkMatrix* matrix, const SkPaint* paint)
{
    DCHECK(picture);
...
void BenchmarkingCanvas::onDrawImage2(const SkImage* image, SkScalar left, SkScalar top, const SkSamplingOptions& sampling, const SkPaint* paint)
{
    DCHECK(image);
...
void BenchmarkingCanvas::onDrawImageRect2(
    const SkImage* image, const SkRect& src, const SkRect& dst, const SkSamplingOptions& sampling, const SkPaint* paint, SrcRectConstraint constraint)
{
    ...
    INHERITED::onDrawImageRect2(image, src, dst, sampling, op.paint(), constraint);
}

void BenchmarkingCanvas::onDrawTextBlob(const SkTextBlob* blob, SkScalar x, SkScalar y, const SkPaint& paint)
{
    DCHECK(blob);
