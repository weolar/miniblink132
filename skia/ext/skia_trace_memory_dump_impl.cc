...
SkiaTraceMemoryDumpImpl::SkiaTraceMemoryDumpImpl(
    const std::string& dump_name_prefix, base::trace_event::MemoryDumpLevelOfDetail level_of_detail, base::trace_event::ProcessMemoryDump* process_memory_dump)
    : dump_name_prefix_(dump_name_prefix)
    , process_memory_dump_(process_memory_dump)
    , request_level_(level_of_detail == base::trace_event::MemoryDumpLevelOfDetail::kLight ? SkTraceMemoryDump::kLight_LevelOfDetail
...
void SkiaTraceMemoryDumpImpl::dumpNumericValue(const char* dump_name, const char* value_name, const char* units, uint64_t value)
{
    auto* dump = process_memory_dump_->GetOrCreateAllocatorDump(dump_name);
    dump->AddScalar(value_name, units, value);
}

void SkiaTraceMemoryDumpImpl::dumpStringValue(const char* dump_name, const char* value_name, const char* value)
{
    auto* dump = process_memory_dump_->GetOrCreateAllocatorDump(dump_name);
...
void SkiaTraceMemoryDumpImpl::setDiscardableMemoryBacking(const char* dump_name, const SkDiscardableMemory& discardable_memory)
{
    std::string name = dump_name_prefix_ + dump_name;
    DCHECK(!process_memory_dump_->GetAllocatorDump(name));
    const SkDiscardableMemoryChrome& discardable_memory_chrome = static_cast<const SkDiscardableMemoryChrome&>(discardable_memory);
    auto* dump = discardable_memory_chrome.CreateMemoryAllocatorDump(name.c_str(), process_memory_dump_);
    DCHECK(dump);
}

SkTraceMemoryDump::LevelOfDetail SkiaTraceMemoryDumpImpl::getRequestedDetails() const
{
...
void SkiaTraceMemoryDumpImpl::dumpWrappedState(const char* dump_name, bool wrapped)
{
    NOTREACHED();
}
...
void SkiaTraceMemoryDumpImpl::dumpBudgetedState(const char* dump_name, bool budgeted)
{
    auto* dump = process_memory_dump_->GetOrCreateAllocatorDump(dump_name);
