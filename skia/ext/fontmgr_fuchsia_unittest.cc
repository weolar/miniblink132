...
{
    sk_sp<SkTypeface> sans(font_manager_->matchFamilyStyle("sans", SkFontStyle()));
    EXPECT_TRUE(sans);

    sk_sp<SkTypeface> sans2(font_manager_->matchFamilyStyle("sans", SkFontStyle()));
    ...
    sk_sp<SkTypeface> serif(font_manager_->matchFamilyStyle("serif", SkFontStyle()));
...
TEST_F(FuchsiaFontManagerTest, ReleaseThenCreateAgain)
{
    ...
    sk_sp<SkTypeface> serif2(font_manager_->matchFamilyStyle("serif", SkFontStyle()));
    EXPECT_TRUE(serif2);
}
