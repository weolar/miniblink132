...
TEST(SkiaUtilsBase, ConvertWeirdStrideToN32)
{
    ...
    SkBitmap bitmap;
    // Stride is > 4 * width.
    bitmap.allocPixels(SkImageInfo::MakeN32(width, height, kPremul_SkAlphaType), width * 4 + 4);
...
TEST(SkiaUtilsBase, ConvertWeirdFormatToN32)
{
    ...
    {
        SkBitmap bitmap;
        bitmap.allocPixels(SkImageInfo::Make(width, height, kRGBA_F16_SkColorType, kPremul_SkAlphaType));
