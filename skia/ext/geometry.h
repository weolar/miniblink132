...
SK_API SkRect ScaleSkRectProportional(const SkRect& output_bounds, const SkRect& input_bounds, const SkRect& input_rect);
...
public:
    ...
    Tiling(const SkRect& dest_rect, std::vector<SkRect> source_rects, std::vector<sk_sp<SkImage>> source_images, int32_t source_max_size);
    ~Tiling();

    // Return the number of horizontal and vertical tiles in the tiling.
    int GetTileCountX() const
    {
        return tile_count_.width();
    }
    int GetTileCountY() const
    {
        return tile_count_.height();
    }
