...
SK_API std::unique_ptr<SkCanvas> CreatePlatformCanvasWithSharedSection(
    int width, int height, bool is_opaque, HANDLE shared_section, OnFailureType failure_type);
...
#elif defined(__linux__) || defined(__FreeBSD__) || defined(__OpenBSD__) || defined(__sun) || defined(ANDROID) || defined(__APPLE__) || defined(__Fuchsia__)
// Construct a canvas from the given memory region. The memory is not cleared
// first. @data must be, at least, @height * StrideForWidth(@width) bytes.
SK_API std::unique_ptr<SkCanvas> CreatePlatformCanvasWithPixels(int width, int height, bool is_opaque, uint8_t* data, OnFailureType failure_type);
#endif

inline std::unique_ptr<SkCanvas> CreatePlatformCanvas(int width, int height, bool is_opaque)
{
#if defined(WIN32)
    return CreatePlatformCanvasWithSharedSection(width, height, is_opaque, 0, CRASH_ON_FAILURE);
#else
    return CreatePlatformCanvasWithPixels(width, height, is_opaque, nullptr, CRASH_ON_FAILURE);
#endif
}

inline std::unique_ptr<SkCanvas> TryCreateBitmapCanvas(int width, int height, bool is_opaque)
{
#if defined(WIN32)
    return CreatePlatformCanvasWithSharedSection(width, height, is_opaque, 0, RETURN_NULL_ON_FAILURE);
#else
    return CreatePlatformCanvasWithPixels(width, height, is_opaque, nullptr, RETURN_NULL_ON_FAILURE);
#endif
}
