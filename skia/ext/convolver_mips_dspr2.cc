...
void ConvolveHorizontally_mips_dspr2(const unsigned char* src_data, const ConvolutionFilter1D& filter, unsigned char* out_row, bool has_alpha)
{
#if SIMD_MIPS_DSPR2
    int row_to_filter = 0;
    ...
    if (has_alpha) {
        for (int out_x = 0; out_x < num_values; out_x++) {
            ...
            __asm__ __volatile__(
                ...
                ".set pop                                   \n"
                : [fx] "+r"(filter_x), [out_x] "+r"(out_x), [out_row] "+r"(out_row), [rtf] "+r"(row_to_filter)
                : [filter_val] "r"(filter_values), [filter_len] "r"(filter_length), [kShiftBits] "r"(ConvolutionFilter1D::kShiftBits),
                [filter_offset] "r"(filter_offset), [src_data] "r"(src_data)
                : "lo", "hi", "$ac1lo", "$ac1hi", "$ac2lo", "$ac2hi", "$ac3lo", "$ac3hi", "t0", "t1", "t2", "t3", "t4", "t5", "t6", "t7", "t8");
        }
    } else {
        for (int out_x = 0; out_x < num_values; out_x++) {
            // Get the filter that determines the current output pixel.
...
void ConvolveVertically_mips_dspr2(const ConvolutionFilter1D::Fixed* filter_val, int filter_length, unsigned char* const* source_data_rows, int pixel_width,
    ...
    if (has_alpha) {
        for (int out_x = 0; out_x < pixel_width; out_x++) {
            __asm__ __volatile__(
                ...
                ".set pop                                    \n"
                : [filter_val] "+r"(filter_val), [filter_len] "+r"(filter_length), [offset] "+r"(byte_offset), [fy] "+r"(filter_y), [cnt] "+r"(cnt),
                [out_x] "+r"(out_x), [pixel_width] "+r"(pixel_width)
                : [src_data_rows] "r"(source_data_rows), [out_row] "r"(out_row), [kShiftBits] "r"(ConvolutionFilter1D::kShiftBits)
                : "lo", "hi", "$ac1lo", "$ac1hi", "$ac2lo", "$ac2hi", "$ac3lo", "$ac3hi", "t0", "t1", "t2", "t3", "t4", "t5", "t6", "t7", "memory");
        }
    } else {
        for (int out_x = 0; out_x < pixel_width; out_x++) {
            __asm__ __volatile__(".set push                                   \n"
                                 ...
                                 ".set pop                                    \n"
                                 : [filter_val] "+r"(filter_val), [filter_len] "+r"(filter_length), [offset] "+r"(byte_offset), [fy] "+r"(filter_y),
                                 [cnt] "+r"(cnt), [out_x] "+r"(out_x), [pixel_width] "+r"(pixel_width)
                                 : [src_data_rows] "r"(source_data_rows), [out_row] "r"(out_row), [kShiftBits] "r"(ConvolutionFilter1D::kShiftBits)
                                 : "lo", "hi", "$ac1lo", "$ac1hi", "$ac2lo", "$ac2hi", "$ac3lo", "$ac3hi", "t0", "t1", "t2", "t3", "t4", "t5", "t6", "memory");
        }
    }
#endif
}
