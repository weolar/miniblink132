...
#if defined(ARCH_CPU_ARM_FAMILY) && (defined(__ARM_NEON__) || defined(__ARM_NEON))
#define SIMD_NEON 1
#endif
...
public:
    ...
    static float FixedToFloat(Fixed x)
    {
        ...
        static_assert(sizeof(Fixed) == 2, "fixed type should fit in float mantissa");
    ...
    SK_API void AddFilter(int filter_offset, const float* filter_values, int filter_length);

    // Same as the above version, but the input is already fixed point.
    void AddFilter(int filter_offset, const Fixed* filter_values, int filter_length);
    ...
    inline const Fixed* FilterForValue(int value_offset, int* filter_offset, int* filter_length) const
    {
    ...
    SK_API const Fixed* GetSingleFilter(int* specified_filter_length, int* filter_offset, int* filter_length) const;
...
SK_API void BGRAConvolve2D(const unsigned char* source_data, int source_byte_row_stride, bool source_has_alpha, const ConvolutionFilter1D& xfilter,
    const ConvolutionFilter1D& yfilter, int output_byte_row_stride, unsigned char* output, bool use_simd_if_possible);
...
SK_API void SingleChannelConvolveX1D(const unsigned char* source_data, int source_byte_row_stride, int input_channel_index, int input_channel_count,
...
// a single channel of the bitmap.
SK_API void SingleChannelConvolveY1D(const unsigned char* source_data, int source_byte_row_stride, int input_channel_index, int input_channel_count,
    const ConvolutionFilter1D& filter, const SkISize& image_size, unsigned char* output, int output_byte_row_stride, int output_channel_index,
    int output_channel_count, bool absolute_values);
...
SK_API void SetUpGaussianConvolutionKernel(ConvolutionFilter1D* filter, float kernel_sigma, bool derivative);
