...
std::optional<SkImageInfo> MakeSkImageInfo(SkColorType color_type, SkAlphaType alpha_type, int width, int height,
    mojo::ArrayDataView<float> color_transfer_function, mojo::ArrayDataView<float> color_to_xyz_matrix)
{
    if (width < 0 || height < 0) {
        return std::nullopt;
    }
    ...
    return SkImageInfo::Make(width, height, color_type, alpha_type, std::move(color_space));
}

} // namespace

// static
skia::mojom::AlphaType EnumTraits<skia::mojom::AlphaType, SkAlphaType>::ToMojom(SkAlphaType type)
{
    switch (type) {
...
// static
bool EnumTraits<skia::mojom::AlphaType, SkAlphaType>::FromMojom(skia::mojom::AlphaType in, SkAlphaType* out)
{
    switch (in) {
...
// static
skia::mojom::ColorType EnumTraits<skia::mojom::ColorType, SkColorType>::ToMojom(SkColorType type)
{
    switch (type) {
...
// static
bool EnumTraits<skia::mojom::ColorType, SkColorType>::FromMojom(skia::mojom::ColorType in, SkColorType* out)
{
    switch (in) {
...
// static
uint32_t StructTraits<skia::mojom::ImageInfoDataView, SkImageInfo>::width(const SkImageInfo& info)
{
...
// static
uint32_t StructTraits<skia::mojom::ImageInfoDataView, SkImageInfo>::height(const SkImageInfo& info)
{
...
// static
std::optional<std::vector<float>> StructTraits<skia::mojom::ImageInfoDataView, SkImageInfo>::color_transfer_function(const SkImageInfo& info)
{
...
// static
std::optional<std::vector<float>> StructTraits<skia::mojom::ImageInfoDataView, SkImageInfo>::color_to_xyz_matrix(const SkImageInfo& info)
{
    SkColorSpace* color_space = info.colorSpace();
    if (!color_space) {
        return std::nullopt;
    ...
    static_assert(sizeof(to_xyz_matrix.vals) == sizeof(float) * 9, "matrix must be 3x3 floats");
    float* values = &to_xyz_matrix.vals[0][0];
    return std::vector<float>(values, values + 9);
}

// static
bool StructTraits<skia::mojom::ImageInfoDataView, SkImageInfo>::Read(skia::mojom::ImageInfoDataView data, SkImageInfo* info)
{
    ...
    std::optional<SkImageInfo> maybe_info
        = MakeSkImageInfo(color_type, alpha_type, width.ValueOrDie(), height.ValueOrDie(), std::move(color_transfer_function), std::move(color_to_xyz_matrix));
...
// static
bool StructTraits<skia::mojom::BitmapN32ImageInfoDataView, SkImageInfo>::Read(skia::mojom::BitmapN32ImageInfoDataView data, SkImageInfo* info)
{
    SkAlphaType alpha_type;
    ...
    std::optional<SkImageInfo> maybe_info = MakeSkImageInfo(
        kN32_SkColorType, alpha_type, width.ValueOrDie(), height.ValueOrDie(), std::move(color_transfer_function), std::move(color_to_xyz_matrix));
    if (!maybe_info.has_value()) {
        return false;
    }
    *info = *maybe_info;
