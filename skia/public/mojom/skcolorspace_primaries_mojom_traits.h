...
template <> struct StructTraits<skia::mojom::SkColorSpacePrimariesDataView, ::SkColorSpacePrimaries> {
    ...
    static bool Read(skia::mojom::SkColorSpacePrimariesDataView data, ::SkColorSpacePrimaries* color)
    {
        color->fRX = data.rX();
        color->fRY = data.rY();
        color->fGX = data.gX();
