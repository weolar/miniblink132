...
template <> struct COMPONENT_EXPORT(SKIA_SHARED_TRAITS) EnumTraits<skia::mojom::AlphaType, SkAlphaType> {
    static skia::mojom::AlphaType ToMojom(SkAlphaType type);
    static bool FromMojom(skia::mojom::AlphaType in, SkAlphaType* out);
};

template <> struct COMPONENT_EXPORT(SKIA_SHARED_TRAITS) EnumTraits<skia::mojom::ColorType, SkColorType> {
    static skia::mojom::ColorType ToMojom(SkColorType type);
    static bool FromMojom(skia::mojom::ColorType in, SkColorType* out);
};

template <> struct COMPONENT_EXPORT(SKIA_SHARED_TRAITS) StructTraits<skia::mojom::ImageInfoDataView, SkImageInfo> {
    ...
    static std::optional<std::vector<float>> color_transfer_function(const SkImageInfo& info);
    static std::optional<std::vector<float>> color_to_xyz_matrix(const SkImageInfo& info);

    static bool Read(skia::mojom::ImageInfoDataView data, SkImageInfo* info);
};

template <> struct COMPONENT_EXPORT(SKIA_SHARED_TRAITS) StructTraits<skia::mojom::BitmapN32ImageInfoDataView, SkImageInfo> {
    static SkAlphaType alpha_type(const SkImageInfo& info)
    {
    ...
    static bool Read(skia::mojom::BitmapN32ImageInfoDataView data, SkImageInfo* info);

private:
    using ImageInfoStructTraits = StructTraits<skia::mojom::ImageInfoDataView, SkImageInfo>;
};
