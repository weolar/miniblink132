...
template <> struct StructTraits<cc::mojom::ElementIdDataView, cc::ElementId> {
    static uint64_t id(const cc::ElementId& id)
    {
        return id.GetInternalValue();
    }

    static bool IsNull(const cc::ElementId& id)
    {
        return !id;
    }

    static void SetToNull(cc::ElementId* out)
    {
        *out = cc::ElementId();
    }
