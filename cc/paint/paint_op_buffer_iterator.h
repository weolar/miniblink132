...
class CC_PAINT_EXPORT PaintOpBuffer::Iterator : public PaintOpBufferIteratorBase {
public:
    ...
    {
        return reinterpret_cast<const PaintOp*>(ptr_);
    }
    ...
    Iterator end() const
    {
        return Iterator(*buffer_, buffer_->data_.get() + buffer_->used_, buffer_->used_);
    }
    ...
    {
        return op_offset_ < buffer_->used_;
    }
...
class CC_PAINT_EXPORT PaintOpBuffer::OffsetIterator : public PaintOpBufferIteratorBase {
public:
    // Offsets and paint op buffer must come from the same DisplayItemList.
    OffsetIterator(const PaintOpBuffer& buffer, const std::vector<size_t>& offsets)
        : buffer_(&buffer)
        , ptr_(buffer_->data_.get())
        , offsets_(&offsets)
    {
        if (offsets.empty()) {
            *this = end();
    ...
    OffsetIterator begin() const
    {
        return OffsetIterator(*buffer_, *offsets_);
    }
    OffsetIterator end() const
    {
        return OffsetIterator(*buffer_, buffer_->data_.get() + buffer_->used_, buffer_->used_, *offsets_);
    }
    ...
    OffsetIterator& operator++()
    {
        ...
        DCHECK(!*this || (*this)->type <= static_cast<uint32_t>(PaintOpType::kLastPaintOpType));
        return *this;
    }
...
private:
    OffsetIterator(const PaintOpBuffer& buffer, const char* ptr, size_t op_offset, const std::vector<size_t>& offsets)
        : buffer_(&buffer)
        , ptr_(ptr)
        , offsets_(&offsets)
        , op_offset_(op_offset)
...
class CC_PAINT_EXPORT PaintOpBuffer::CompositeIterator : public PaintOpBufferIteratorBase {
public:
    // Offsets and paint op buffer must come from the same DisplayItemList.
    CompositeIterator(const PaintOpBuffer& buffer, const std::vector<size_t>* offsets);
    ...
    {
        return absl::holds_alternative<Iterator>(iter_) ? CompositeIterator(absl::get<Iterator>(iter_).begin())
                                                        : CompositeIterator(absl::get<OffsetIterator>(iter_).begin());
    }
    CompositeIterator end() const
    {
        return absl::holds_alternative<Iterator>(iter_) ? CompositeIterator(absl::get<Iterator>(iter_).end())
...
private:
    explicit CompositeIterator(OffsetIterator iter)
        : iter_(std::move(iter))
    {
    }
    explicit CompositeIterator(Iterator iter)
        : iter_(std::move(iter))
    {
    }
...
class CC_PAINT_EXPORT PaintOpBuffer::PlaybackFoldingIterator : public PaintOpBufferIteratorBase {
public:
    PlaybackFoldingIterator(const PaintOpBuffer& buffer, const std::vector<size_t>* offsets);
