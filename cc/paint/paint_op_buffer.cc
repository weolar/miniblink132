...
PlaybackCallbacks& PlaybackCallbacks::operator=(const PlaybackCallbacks&) = default;

PlaybackParams::PlaybackParams(ImageProvider* image_provider, const SkM44& original_ctm, const PlaybackCallbacks& callbacks)
...
PaintOpBuffer::SerializeOptions::SerializeOptions(ImageProvider* image_provider, TransferCacheSerializeHelper* transfer_cache, ClientPaintCache* paint_cache,
    SkStrikeServer* strike_server, sk_sp<SkColorSpace> color_space, SkottieSerializationHistory* skottie_serialization_history, bool can_use_lcd_text,
    bool context_supports_distance_field_text, int max_texture_size, const ScrollOffsetMap* raster_inducing_scroll_offsets)
    : image_provider(image_provider)
    ...
    , context_supports_distance_field_text(context_supports_distance_field_text)
...
PaintOpBuffer::SerializeOptions::SerializeOptions(const SerializeOptions&) = default;
PaintOpBuffer::SerializeOptions& PaintOpBuffer::SerializeOptions::operator=(const SerializeOptions&) = default;
PaintOpBuffer::SerializeOptions::~SerializeOptions() = default;
...
PaintRecord PaintOpBuffer::DeepCopyAsRecord()
{
    ...
    for (const PaintOp& op : *this) {
        ...
        case PaintOpType::kDrawArc: {
            ...
            result->push<DrawArcOp>(o.oval, o.start_angle_degrees, o.sweep_angle_degrees, o.flags);
        } break;
        case PaintOpType::kDrawArcLite: {
            const auto& o = static_cast<const DrawArcLiteOp&>(op);
            result->push<DrawArcLiteOp>(o.oval, o.start_angle_degrees, o.sweep_angle_degrees, o.core_paint_flags);
        } break;
        ...
        case PaintOpType::kDrawImageRect: {
            ...
            result->push<DrawImageRectOp>(o.image, o.src, o.dst, o.sampling, &o.flags, o.constraint);
        } break;
        ...
        case PaintOpType::kDrawLineLite: {
            ...
            result->push<DrawLineLiteOp>(o.x0, o.y0, o.x1, o.y1, o.core_paint_flags);
        } break;
        ...
        case PaintOpType::kDrawScrollingContents: {
            ...
            result->push<DrawScrollingContentsOp>(o.scroll_element_id, o.display_item_list);
        } break;
        case PaintOpType::kDrawSkottie: {
            const auto& o = static_cast<const DrawSkottieOp&>(op);
            result->push<DrawSkottieOp>(o.skottie, o.dst, o.t, o.images, o.color_map, o.text_map);
        } break;
        ...
        case PaintOpType::kSaveLayerFilters: {
            const SaveLayerFiltersOp& o = static_cast<const SaveLayerFiltersOp&>(op);
            std::vector<sk_sp<PaintFilter>> f = o.filters;
            base::span<const sk_sp<PaintFilter>> filters(f.data(), f.size());
            result->push<SaveLayerFiltersOp>(std::move(filters), o.flags);
        } break;
...
PaintOpBuffer& PaintOpBuffer::operator=(PaintOpBuffer&& other)
{
    ...
    has_effects_preventing_lcd_text_for_save_layer_alpha_ = other.has_effects_preventing_lcd_text_for_save_layer_alpha_;
...
void PaintOpBuffer::Playback(SkCanvas* canvas, const PlaybackParams& params, bool local_ctm) const
{
...
void PaintOpBuffer::Playback(SkCanvas* canvas, const PlaybackParams& params, bool local_ctm, const std::vector<size_t>* offsets) const
{
    if (!op_count_)
        return;
    ...
    bool save_layer_alpha_should_preserve_lcd_text
        = (!params.save_layer_alpha_should_preserve_lcd_text.has_value() || *params.save_layer_alpha_should_preserve_lcd_text) && has_draw_text_ops_
    ...
    new_params.save_layer_alpha_should_preserve_lcd_text = save_layer_alpha_should_preserve_lcd_text;
    for (PlaybackFoldingIterator iter(*this, offsets); iter; ++iter) {
        ...
        const bool skip_op = new_params.image_provider && PaintOp::OpHasDiscardableImages(*op) && PaintOp::QuickRejectDraw(*op, canvas);
        if (skip_op)
            continue;
...
//             }
            ...
            const ScopedRasterFlags scoped_flags(&flags_op.flags, new_params.image_provider, canvas->getTotalMatrix(), max_texture_size, iter.alpha());
...
bool PaintOpBuffer::Deserialize(const volatile void* input, size_t input_size, const PaintOp::DeserializeOptions& options)
{
    size_t total_bytes_read = 0u;
    while (total_bytes_read < input_size) {
        const volatile void* next_op = static_cast<const volatile char*>(input) + total_bytes_read;
        size_t read_bytes = 0;
        if (!PaintOp::DeserializeIntoPaintOpBuffer(next_op, input_size - total_bytes_read, this, &read_bytes, options)) {
            return false;
        }
...
// static
sk_sp<PaintOpBuffer> PaintOpBuffer::MakeFromMemory(const volatile void* input, size_t input_size, const PaintOp::DeserializeOptions& options)
{
...
// static
SkRect PaintOpBuffer::GetFixedScaleBounds(const SkMatrix& ctm, const SkRect& bounds, int max_texture_size)
{
    SkSize scale;
    if (!ctm.decomposeScale(&scale)) {
        // Decomposition failed, use an approximation.
        scale.set(SkScalarSqrt(ctm.getScaleX() * ctm.getScaleX() + ctm.getSkewX() * ctm.getSkewX()),
            SkScalarSqrt(ctm.getScaleY() * ctm.getScaleY() + ctm.getSkewY() * ctm.getSkewY()));
    }
    ...
    if (max_texture_size > 0) {
        ...
        down_scale = std::min(down_scale, max_texture_size / std::max(raster_width, raster_height));
    }

    if (down_scale < 1.f) {
        scale.set(down_scale * scale.width(), down_scale * scale.height());
    }
    return SkRect::MakeXYWH(bounds.fLeft * scale.width(), bounds.fTop * scale.height(), SkScalarCeilToInt(SkScalarAbs(scale.width() * bounds.width())),
...
PaintOpBuffer::BufferDataPtr PaintOpBuffer::ReallocBuffer(size_t new_size)
{
    ...
    std::unique_ptr<char, base::AlignedFreeDeleter> new_data(static_cast<char*>(base::AlignedAlloc(new_size, kPaintOpAlign)));
    if (data_)
...
bool PaintOpBuffer::EqualsForTesting(const PaintOpBuffer& other) const
{
    ...
    if (op_count_ != other.op_count_ || used_ != other.used_ || num_slow_paths_up_to_min_for_MSAA_ != other.num_slow_paths_up_to_min_for_MSAA_
        || subrecord_op_count_ != other.subrecord_op_count_ || has_draw_ops_ != other.has_draw_ops_ || has_draw_text_ops_ != other.has_draw_text_ops_
        || has_effects_preventing_lcd_text_for_save_layer_alpha_ != other.has_effects_preventing_lcd_text_for_save_layer_alpha_
        || has_non_aa_paint_ != other.has_non_aa_paint_ || has_discardable_images_ != other.has_discardable_images_) {
        return false;
    }

    return base::ranges::equal(*this, other, [](const PaintOp& a, const PaintOp& b) {
        return a.EqualsForTesting(b); // IN-TEST
    });
}

bool PaintOpBuffer::NeedsAdditionalInvalidationForLCDText(const PaintOpBuffer& old_buffer) const
{
    ...
    if (!old_buffer.has_draw_text_ops() || !old_buffer.has_save_layer_alpha_ops())
        return false;
    return has_effects_preventing_lcd_text_for_save_layer_alpha() != old_buffer.has_effects_preventing_lcd_text_for_save_layer_alpha();
}
