...
public:
    ...
    static FilterOperation CreateBlurFilter(float amount, SkTileMode tile_mode = SkTileMode::kDecal)
    {
        return FilterOperation(BLUR, amount, tile_mode);
    }

    static FilterOperation CreateDropShadowFilter(const gfx::Point& offset, float std_deviation, SkColor4f color)
    {
    ...
    static FilterOperation CreateReferenceFilter(sk_sp<PaintFilter> image_filter)
    {
    ...
    gfx::Rect MapRect(const gfx::Rect& rect, const std::optional<SkMatrix>& ctm = std::nullopt) const;
...
private:
    ...
    FilterOperation(FilterType type, const gfx::Point& offset, float stdDeviation, SkColor4f color);
