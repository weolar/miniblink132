...
public:
    ...
    SkIRect MapRect(const SkIRect& src, const SkMatrix* ctm, MapDirection direction) const;

    const CropRect* GetCropRect() const;

    bool has_discardable_images() const
    {
        return has_discardable_images_;
    }
    ...
    {
        image_analysis_state_ = has_animated_images ? ImageAnalysisState::kAnimatedImages : ImageAnalysisState::kNoAnimatedImages;
    }
    ...
    static std::vector<sk_sp<SkImageFilter>> ToSkImageFilters(base::span<const sk_sp<PaintFilter>> filters);

protected:
    PaintFilter(Type type, const CropRect* crop_rect, bool has_discardable_images);
    ...
    virtual sk_sp<PaintFilter> SnapshotWithImagesInternal(ImageProvider* image_provider) const = 0;
...
public:
    const sk_sp<PaintFilter>& input() const
    {
        return input_;
    }
...
protected:
    OneInputPaintFilter(Type type, sk_sp<PaintFilter> input, const CropRect* crop_rect = nullptr);
...
protected:
    TwoInputPaintFilter(Type type, sk_sp<PaintFilter> first, sk_sp<PaintFilter> second, const CropRect* crop_rect = nullptr);
...
class CC_PAINT_EXPORT ColorFilterPaintFilter final : public OneInputPaintFilter {
public:
    static constexpr Type kType = Type::kColorFilter;
    ColorFilterPaintFilter(sk_sp<ColorFilter> color_filter, sk_sp<PaintFilter> input, const CropRect* crop_rect = nullptr);
    ~ColorFilterPaintFilter() override;

    const sk_sp<ColorFilter>& color_filter() const
    {
...
protected:
    sk_sp<PaintFilter> SnapshotWithImagesInternal(ImageProvider* image_provider) const override;
...
public:
    ...
    BlurPaintFilter(SkScalar sigma_x, SkScalar sigma_y, SkTileMode tile_mode, sk_sp<PaintFilter> input, const CropRect* crop_rect = nullptr);
...
public:
    enum class ShadowMode { kDrawShadowAndForeground, kDrawShadowOnly, kMaxValue = kDrawShadowOnly };
    static constexpr Type kType = Type::kDropShadow;
    DropShadowPaintFilter(SkScalar dx, SkScalar dy, SkScalar sigma_x, SkScalar sigma_y, SkColor4f color, ShadowMode shadow_mode, sk_sp<PaintFilter> input,
...
public:
    ...
    MagnifierPaintFilter(const SkRect& lens_bounds, SkScalar zoom_amount, SkScalar inset, sk_sp<PaintFilter> input, const CropRect* crop_rect = nullptr);
    ~MagnifierPaintFilter() override;

    const SkRect& lens_bounds() const
    {
        return lens_bounds_;
    }
    SkScalar zoom_amount() const
    {
        return zoom_amount_;
    }
    SkScalar inset() const
    {
        return inset_;
    }
...
public:
    ...
    const sk_sp<PaintFilter>& outer() const
    {
        return first_;
    }
    const sk_sp<PaintFilter>& inner() const
    {
        return second_;
    }
...
class CC_PAINT_EXPORT AlphaThresholdPaintFilter final : public OneInputPaintFilter {
public:
    static constexpr Type kType = Type::kAlphaThreshold;
    AlphaThresholdPaintFilter(const SkRegion& region, sk_sp<PaintFilter> input, const CropRect* crop_rect = nullptr);
...
public:
    ...
    XfermodePaintFilter(SkBlendMode blend_mode, sk_sp<PaintFilter> background, sk_sp<PaintFilter> foreground, const CropRect* crop_rect = nullptr);
    ...
    const sk_sp<PaintFilter>& background() const
    {
        return first_;
    }
    const sk_sp<PaintFilter>& foreground() const
    {
        return second_;
    }
...
public:
    ...
    ArithmeticPaintFilter(float k1, float k2, float k3, float k4, bool enforce_pm_color, sk_sp<PaintFilter> background, sk_sp<PaintFilter> foreground,
        const CropRect* crop_rect = nullptr);
    ...
    bool enforce_pm_color() const
    {
        return enforce_pm_color_;
    }
    const sk_sp<PaintFilter>& background() const
    {
        return first_;
...
class CC_PAINT_EXPORT MatrixConvolutionPaintFilter final : public OneInputPaintFilter {
public:
    static constexpr Type kType = Type::kMatrixConvolution;
    MatrixConvolutionPaintFilter(const SkISize& kernel_size, base::span<const SkScalar> kernel, SkScalar gain, SkScalar bias, const SkIPoint& kernel_offset,
        SkTileMode tile_mode, bool convolve_alpha, sk_sp<PaintFilter> input, const CropRect* crop_rect = nullptr);
    ~MatrixConvolutionPaintFilter() override;

    const SkISize& kernel_size() const
    {
        return kernel_size_;
    }
    SkScalar kernel_at(size_t i) const
    {
        return kernel_[i];
    }
    SkScalar gain() const
...
class CC_PAINT_EXPORT DisplacementMapEffectPaintFilter final : public TwoInputPaintFilter {
public:
    static constexpr Type kType = Type::kDisplacementMapEffect;
    DisplacementMapEffectPaintFilter(SkColorChannel channel_x, SkColorChannel channel_y, SkScalar scale, sk_sp<PaintFilter> displacement,
        sk_sp<PaintFilter> color, const CropRect* crop_rect = nullptr);
    ~DisplacementMapEffectPaintFilter() override;

    SkColorChannel channel_x() const
    {
        return channel_x_;
    }
    SkColorChannel channel_y() const
    {
        return channel_y_;
    }
    ...
    const sk_sp<PaintFilter>& displacement() const
    {
        return first_;
    }
    const sk_sp<PaintFilter>& color() const
    {
        return second_;
    }
...
public:
    ...
    ImagePaintFilter(PaintImage image, const SkRect& src_rect, const SkRect& dst_rect, PaintFlags::FilterQuality filter_quality);
    ...
    const SkRect& src_rect() const
    {
        return src_rect_;
    }
    const SkRect& dst_rect() const
    {
        return dst_rect_;
    }
    PaintFlags::FilterQuality filter_quality() const
    {
...
public:
    ...
    RecordPaintFilter(PaintRecord record, const SkRect& record_bounds, const gfx::SizeF& raster_scale = { 1.f, 1.f },
    ...
    sk_sp<RecordPaintFilter> CreateScaledPaintRecord(const SkMatrix& ctm, int max_texture_size) const;

    const PaintRecord& record() const
    {
        return record_;
    }
    SkRect record_bounds() const
    {
        return record_bounds_;
    }
    gfx::SizeF raster_scale() const
    {
        return raster_scale_;
    }
    ScalingBehavior scaling_behavior() const
    {
        return scaling_behavior_;
    }
...
public:
    ...
    explicit MergePaintFilter(base::span<const sk_sp<PaintFilter>> filters, const CropRect* crop_rect = nullptr);
...
private:
    MergePaintFilter(base::span<const sk_sp<PaintFilter>> filters, const CropRect* crop_rect, ImageProvider* image_provider);
...
public:
    ...
    MorphologyPaintFilter(MorphType morph_type, float radius_x, float radius_y, sk_sp<PaintFilter> input, const CropRect* crop_rect = nullptr);
    ~MorphologyPaintFilter() override;

    MorphType morph_type() const
    {
        return morph_type_;
    }
...
public:
    ...
    OffsetPaintFilter(SkScalar dx, SkScalar dy, sk_sp<PaintFilter> input, const CropRect* crop_rect = nullptr);
...
public:
    ...
    TilePaintFilter(const SkRect& src, const SkRect& dst, sk_sp<PaintFilter> input);
    ~TilePaintFilter() override;

    const SkRect& src() const
    {
        return src_;
    }
...
public:
    ...
    enum class TurbulenceType { kTurbulence, kFractalNoise, kMaxValue = kFractalNoise };
    TurbulencePaintFilter(TurbulenceType turbulence_type, SkScalar base_frequency_x, SkScalar base_frequency_y, int num_octaves, SkScalar seed,
        const SkISize* tile_size, const CropRect* crop_rect = nullptr);
    ~TurbulencePaintFilter() override;

    TurbulenceType turbulence_type() const
    {
        return turbulence_type_;
    }
    SkScalar base_frequency_x() const
    {
        return base_frequency_x_;
    }
    SkScalar base_frequency_y() const
    {
...
public:
    ...
    ShaderPaintFilter(
        sk_sp<PaintShader> shader, float alpha, PaintFlags::FilterQuality filter_quality, SkImageFilters::Dither dither, const CropRect* crop_rect = nullptr);
    // This declaration prevents int alpha from being passed.
    ShaderPaintFilter(sk_sp<PaintShader>, unsigned alpha, PaintFlags::FilterQuality, SkImageFilters::Dither, const CropRect* = nullptr) = delete;

    ~ShaderPaintFilter() override;

    const PaintShader& shader() const
    {
        return *shader_;
    }
    float alpha() const
    ...
    SkImageFilters::Dither dither() const
    {
        return dither_;
    }
...
public:
    ...
    MatrixPaintFilter(const SkMatrix& matrix, PaintFlags::FilterQuality filter_quality, sk_sp<PaintFilter> input);
...
class CC_PAINT_EXPORT LightingDistantPaintFilter final : public OneInputPaintFilter {
public:
    ...
    LightingDistantPaintFilter(LightingType lighting_type, const SkPoint3& direction, SkColor4f light_color, SkScalar surface_scale, SkScalar kconstant,
        SkScalar shininess, sk_sp<PaintFilter> input, const CropRect* crop_rect = nullptr);
    ~LightingDistantPaintFilter() override;

    LightingType lighting_type() const
    {
        return lighting_type_;
    }
    const SkPoint3& direction() const
    {
        return direction_;
    }
    SkColor4f light_color() const
    {
        return light_color_;
    }
    SkScalar surface_scale() const
    {
        return surface_scale_;
    }
    SkScalar kconstant() const
    {
        return kconstant_;
    }
    SkScalar shininess() const
    {
        return shininess_;
    }
...
class CC_PAINT_EXPORT LightingPointPaintFilter final : public OneInputPaintFilter {
public:
    static constexpr Type kType = Type::kLightingPoint;
    // kConstant refers to the kd (diffuse) or ks (specular) depending on the
    // LightingType.
    // For specular lighting type only, shininess denotes the specular exponent.
    LightingPointPaintFilter(LightingType lighting_type, const SkPoint3& location, SkColor4f light_color, SkScalar surface_scale, SkScalar kconstant,
        SkScalar shininess, sk_sp<PaintFilter> input, const CropRect* crop_rect = nullptr);
...
class CC_PAINT_EXPORT LightingSpotPaintFilter final : public OneInputPaintFilter {
public:
    ...
    LightingSpotPaintFilter(LightingType lighting_type, const SkPoint3& location, const SkPoint3& target, SkScalar specular_exponent, SkScalar cutoff_angle,
        SkColor4f light_color, SkScalar surface_scale, SkScalar kconstant, SkScalar shininess, sk_sp<PaintFilter> input, const CropRect* crop_rect = nullptr);
    ...
    const SkPoint3& target() const
    {
        return target_;
    }
    SkScalar specular_exponent() const
    {
        return specular_exponent_;
    }
    SkScalar cutoff_angle() const
    {
