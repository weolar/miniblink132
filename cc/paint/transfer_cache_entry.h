...
public:
    static std::unique_ptr<ServiceTransferCacheEntry> Create(TransferCacheEntryType type);
    ...
    static bool SafeConvertToType(uint32_t raw_type, TransferCacheEntryType* type);
    ...
    virtual bool Deserialize(GrDirectContext* gr_context, skgpu::graphite::Recorder* graphite_recorder, base::span<const uint8_t> data) = 0;
};

// Helpers to simplify subclassing.
template <class Base, TransferCacheEntryType EntryType> class TransferCacheEntryBase : public Base {
public:
...
template <TransferCacheEntryType EntryType> using ClientTransferCacheEntryBase = TransferCacheEntryBase<ClientTransferCacheEntry, EntryType>;

template <TransferCacheEntryType EntryType> using ServiceTransferCacheEntryBase = TransferCacheEntryBase<ServiceTransferCacheEntry, EntryType>;
