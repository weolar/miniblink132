...
FilterOperations& FilterOperations::operator=(const FilterOperations& other) = default;
...
gfx::Rect FilterOperations::MapRect(const gfx::Rect& rect, const std::optional<SkMatrix>& ctm) const
{
    auto accumulate_rect = [&ctm](const gfx::Rect& rect, const FilterOperation& op) { return op.MapRect(rect, ctm); };
    return std::accumulate(operations_.begin(), operations_.end(), rect, accumulate_rect);
}

gfx::Rect FilterOperations::MapRectReverse(const gfx::Rect& rect, const SkMatrix& ctm) const
{
    auto accumulate_rect = [&ctm](const gfx::Rect& rect, const FilterOperation& op) { return op.MapRectReverse(rect, ctm); };
    return std::accumulate(operations_.rbegin(), operations_.rend(), rect, accumulate_rect);
}
...
gfx::Rect FilterOperations::ExpandRectForPixelMovement(const gfx::Rect& rect) const
{
...
float FilterOperations::MaximumPixelMovement() const
{
    ...
    for (size_t i = 0; i < operations_.size(); ++i) {
        ...
        case FilterOperation::DROP_SHADOW:
            // |op.amount| here is the blur radius.
            max_movement = fmax(max_movement, fmax(std::abs(op.offset().x()), std::abs(op.offset().y())) + op.amount() * 3.f);
            continue;
        case FilterOperation::ZOOM:
        ...
        case FilterOperation::OFFSET:
            ...
            // moving filters.
            max_movement = fmax(std::abs(op.offset().x()), std::abs(op.offset().y()));
            continue;
        case FilterOperation::OPACITY:
        case FilterOperation::COLOR_MATRIX:
        case FilterOperation::GRAYSCALE:
...
FilterOperations FilterOperations::Blend(const FilterOperations& from, double progress) const
{
    ...
    for (size_t i = 0; i < shorter_size; i++) {
        blended_filters.Append(FilterOperation::Blend(&from.at(i), &at(i), progress));
    }

    if (from_is_longer) {
        for (size_t i = shorter_size; i < longer_size; i++) {
            blended_filters.Append(FilterOperation::Blend(&from.at(i), nullptr, progress));
        }
    } else {
        for (size_t i = shorter_size; i < longer_size; i++)
...
void FilterOperations::AsValueInto(base::trace_event::TracedValue* value) const
{
    for (size_t i = 0; i < operations_.size(); ++i) {
