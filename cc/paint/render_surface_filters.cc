...
sk_sp<PaintFilter> CreateMatrixImageFilter(const float matrix[20], sk_sp<PaintFilter> input)
{
    return sk_make_sp<ColorFilterPaintFilter>(ColorFilter::MakeMatrix(matrix), std::move(input));
}

} // namespace

sk_sp<PaintFilter> RenderSurfaceFilters::BuildImageFilter(const FilterOperations& filters, const gfx::Rect& layer_bounds)
{
    ...
    for (size_t i = 0; i < filters.size(); ++i) {
        ...
        case FilterOperation::BLUR: {
            ...
            if (!sk_layer_bounds.isEmpty() && op.blur_tile_mode() != SkTileMode::kDecal) {
                crop_rect = &sk_layer_bounds;
            }
            image_filter = sk_make_sp<BlurPaintFilter>(op.amount(), op.amount(), op.blur_tile_mode(), std::move(image_filter), crop_rect);
            break;
        }
        case FilterOperation::DROP_SHADOW:
            image_filter = sk_make_sp<DropShadowPaintFilter>(SkIntToScalar(op.offset().x()), SkIntToScalar(op.offset().y()), SkIntToScalar(op.amount()),
                SkIntToScalar(op.amount()), op.drop_shadow_color(), DropShadowPaintFilter::ShadowMode::kDrawShadowAndForeground, std::move(image_filter));
            break;
        case FilterOperation::COLOR_MATRIX:
            image_filter = CreateMatrixImageFilter(op.matrix().data(), std::move(image_filter));
            break;
        case FilterOperation::ZOOM: {
            ...
            if (layer_bounds.IsEmpty()) {
                ...
                image_filter = sk_make_sp<MagnifierPaintFilter>(gfx::RectToSkRect(layer_bounds), op.amount(), op.zoom_inset(), std::move(image_filter));
            }
            break;
        }
        ...
        case FilterOperation::REFERENCE: {
            ...
            if (op.image_filter()->type() == PaintFilter::Type::kColorFilter && !op.image_filter()->GetCropRect()) {
                auto* color_paint_filter = static_cast<ColorFilterPaintFilter*>(op.image_filter().get());
                cf = color_paint_filter->color_filter();
                has_input = !!color_paint_filter->input();
            }

            if (cf && !has_input) {
                image_filter = sk_make_sp<ColorFilterPaintFilter>(std::move(cf), std::move(image_filter));
            } else if (image_filter) {
                image_filter = sk_make_sp<ComposePaintFilter>(op.image_filter(), std::move(image_filter));
            } else {
        ...
        case FilterOperation::ALPHA_THRESHOLD: {
            ...
            image_filter = sk_make_sp<AlphaThresholdPaintFilter>(region, std::move(image_filter));
            break;
        }
        case FilterOperation::OFFSET: {
            image_filter = sk_make_sp<OffsetPaintFilter>(SkIntToScalar(op.offset().x()), SkIntToScalar(op.offset().y()), std::move(image_filter));
            break;
        }
        }
    }
