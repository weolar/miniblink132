...
constexpr SkYUVColorSpace kJpegYUVColorSpace = SkYUVColorSpace::kJPEG_SkYUVColorSpace;
...
bool CheckRectIsSolidColor(const sk_sp<SkImage>& image, SkColor expected_color, const SkIRect& subset)
{
...
bool CheckImageIsSolidColor(const sk_sp<SkImage>& image, SkColor expected_color)
{
    return CheckRectIsSolidColor(image, expected_color, SkIRect::MakeWH(image->width(), image->height()));
}

// TODO(crbug.com/40266937): Implement test with Skia Graphite backend.
class ImageTransferCacheEntryTest : public testing::TestWithParam<SkYUVAInfo::PlaneConfig> {
public:
    void SetUp() override
    {
        // Initialize a GL GrContext for Skia.
        auto surface = gl::init::CreateOffscreenGLSurface(gl::GetDefaultDisplay(), gfx::Size());
        ASSERT_TRUE(surface);
        ...
        sk_sp<GrGLInterface> gl_interface(gl::init::CreateGrGLInterface(*gl_context_->GetVersionInfo()));
    ...
    // on failure.
    std::vector<sk_sp<SkImage>> CreateTestYUVImage(base::HeapArray<bool>& release_flags)
    {
        std::vector<sk_sp<SkImage>> plane_images;
        release_flags = base::HeapArray<bool>();
        if (GetParam() == SkYUVAInfo::PlaneConfig::kY_U_V || GetParam() == SkYUVAInfo::PlaneConfig::kY_V_U) {
            release_flags = base::HeapArray<bool>::CopiedFrom(std::to_array<bool>({ false, false, false }));
            plane_images = { CreateSolidPlane(gr_context(), 64, 64, GL_R8_EXT, SkColors::kWhite, &release_flags[0]),
                CreateSolidPlane(gr_context(), 32, 32, GL_R8_EXT, SkColors::kWhite, &release_flags[1]),
                CreateSolidPlane(gr_context(), 32, 32, GL_R8_EXT, SkColors::kWhite, &release_flags[2]) };
        } else if (GetParam() == SkYUVAInfo::PlaneConfig::kY_UV) {
            release_flags = base::HeapArray<bool>::CopiedFrom(std::to_array<bool>({ false, false }));
            plane_images = { CreateSolidPlane(gr_context(), 64, 64, GL_R8_EXT, SkColors::kWhite, &release_flags[0]),
                CreateSolidPlane(gr_context(), 32, 32, GL_RG8_EXT, SkColors::kWhite, &release_flags[1]) };
        } else {
            NOTREACHED();
        }
...
private:
    ...
    sk_sp<SkImage> CreateSolidPlane(GrDirectContext* gr_context, int width, int height, GrGLenum texture_format, const SkColor4f& color, bool* released)
    {
        GrBackendTexture allocated_texture = gr_context->createBackendTexture(
            width, height, GrBackendFormats::MakeGL(texture_format, GL_TEXTURE_2D), color, skgpu::Mipmapped::kNo, GrRenderable::kNo);
        if (!allocated_texture.isValid())
            return nullptr;
        textures_to_free_.push_back(allocated_texture);
        GrGLTextureInfo allocated_texture_info;
        if (!GrBackendTextures::GetGLTextureInfo(allocated_texture, &allocated_texture_info)) {
            return nullptr;
        }
        ...
        *released = false;
        return SkImages::BorrowTextureFrom(gr_context, allocated_texture, kTopLeft_GrSurfaceOrigin,
            texture_format == GL_RG8_EXT ? kR8G8_unorm_SkColorType : kAlpha_8_SkColorType, kOpaque_SkAlphaType, nullptr /* colorSpace */, MarkTextureAsReleased,
            released);
    }
...
TEST_P(ImageTransferCacheEntryTest, MAYBE_Deserialize)
{
    ...
    SkYUVAInfo yuva_info({ image_width, image_height }, SkYUVAInfo::PlaneConfig::kY_U_V, SkYUVAInfo::Subsampling::k420, kJpegYUVColorSpace);
    SkYUVAPixmapInfo yuva_pixmap_info(yuva_info, SkYUVAPixmapInfo::DataType::kUnorm8, yuv_strides);
    SkYUVAPixmaps yuva_pixmaps = SkYUVAPixmaps::Allocate(yuva_pixmap_info);

    // rgb (255, 121, 255) -> yuv (255, 255, 255)
    const SkIRect bottom_color_rect = SkIRect::MakeXYWH(0, image_height / 2, image_width, image_height / 2);
    ...
    auto client_entry(std::make_unique<ClientImageTransferCacheEntry>(
        ClientImageTransferCacheEntry::Image(yuva_pixmaps.planes(), yuva_info, nullptr /* decoded color space*/), true /* needs_mips */, std::nullopt));
    uint32_t size = client_entry->SerializedSize();
    auto data = PaintOpWriter::AllocateAlignedBuffer<uint8_t>(size);
    ASSERT_TRUE(client_entry->Serialize(base::make_span(static_cast<uint8_t*>(data.get()), size)));

    // Create service-side entry from the client-side serialize info
    auto entry(std::make_unique<ServiceImageTransferCacheEntry>());
    ASSERT_TRUE(entry->Deserialize(gr_context(), /*graphite_recorder=*/nullptr, base::make_span(static_cast<uint8_t*>(data.get()), size)));
    ASSERT_TRUE(entry->is_yuv());

    // Check color of pixels
    ASSERT_TRUE(CheckRectIsSolidColor(entry->image(), SkColorSetRGB(178, 0, 225), top_color_rect));
    ASSERT_TRUE(CheckRectIsSolidColor(entry->image(), SkColorSetRGB(255, 121, 255), bottom_color_rect));
...
TEST_P(ImageTransferCacheEntryTest, HardwareDecodedNoMipsAtCreation)
{
    ...
    ASSERT_EQ(static_cast<size_t>(SkYUVAInfo::NumPlanes(GetParam())), plane_images_size);
    ...
    auto entry(std::make_unique<ServiceImageTransferCacheEntry>());
    EXPECT_TRUE(entry->BuildFromHardwareDecodedImage(gr_context(), std::move(plane_images), GetParam() /* plane_images_format */, SkYUVAInfo::Subsampling::k420,
    ...
    EXPECT_TRUE(std::none_of(release_flags.begin(), release_flags.end(), [](bool released) { return released; }));
    entry.reset();
    EXPECT_TRUE(std::all_of(release_flags.begin(), release_flags.end(), [](bool released) { return released; }));
}
...
TEST_P(ImageTransferCacheEntryTest, HardwareDecodedMipsAtCreation)
{
    ...
    EXPECT_TRUE(CheckImageIsSolidColor(entry->image(), SkColorSetRGB(255, 121, 255)));
}
...
std::string TestParamToString(const testing::TestParamInfo<SkYUVAInfo::PlaneConfig>& param_info)
{
...
INSTANTIATE_TEST_SUITE_P(All, ImageTransferCacheEntryTest,
    ::testing::Values(SkYUVAInfo::PlaneConfig::kY_U_V, SkYUVAInfo::PlaneConfig::kY_V_U, SkYUVAInfo::PlaneConfig::kY_UV), TestParamToString);
...
TEST(ImageTransferCacheEntryTestNoYUV, CPUImageWithMips)
{
    ...
    SkBitmap bitmap;
    bitmap.allocPixels(SkImageInfo::MakeN32Premul(gr_context->maxTextureSize() + 1, 10));

    ClientImageTransferCacheEntry client_entry(ClientImageTransferCacheEntry::Image(&bitmap.pixmap()), true, std::nullopt);
    ...
    service_entry.Deserialize(gr_context.get(),
        /*graphite_recorder=*/nullptr, base::make_span(storage.get(), storage_size));
...
TEST(ImageTransferCacheEntryTestNoYUV, CPUImageAddMipsLater)
{
    ...
    SkBitmap bitmap;
    bitmap.allocPixels(SkImageInfo::MakeN32Premul(gr_context->maxTextureSize() + 1, 10));
    ClientImageTransferCacheEntry client_entry(ClientImageTransferCacheEntry::Image(&bitmap.pixmap()), false, std::nullopt);
    const uint32_t storage_size = client_entry.SerializedSize();
    auto storage = PaintOpWriter::AllocateAlignedBuffer<uint8_t>(storage_size);
    client_entry.Serialize(base::make_span(storage.get(), storage_size));
