...
sk_sp<SkPicture> ToSkPicture(const PaintRecord& record, const SkRect& bounds, const gfx::SizeF* raster_scale, ImageProvider* image_provider)
{
    SkPictureRecorder recorder;
    SkCanvas* canvas = recorder.beginRecording(SkRect::MakeWH(bounds.width(), bounds.height()));
...
bool CompareMatricesForTesting(const SkMatrix& a, // IN-TEST
    const SkMatrix& b, bool ignore_scaling_differences)
{
...
sk_sp<PaintShader> PaintShader::MakeLinearGradient(const SkPoint points[], const SkColor4f colors[], const SkScalar pos[], int count, SkTileMode mode,
    SkGradientShader::Interpolation interpolation, uint32_t flags, const SkMatrix* local_matrix, SkColor4f fallback_color)
{
...
sk_sp<PaintShader> PaintShader::MakeRadialGradient(const SkPoint& center, SkScalar radius, const SkColor4f colors[], const SkScalar pos[], int count,
    SkTileMode mode, SkGradientShader::Interpolation interpolation, uint32_t flags, const SkMatrix* local_matrix, SkColor4f fallback_color)
{
...
sk_sp<PaintShader> PaintShader::MakeTwoPointConicalGradient(const SkPoint& start, SkScalar start_radius, const SkPoint& end, SkScalar end_radius,
    const SkColor4f colors[], const SkScalar pos[], int count, SkTileMode mode, SkGradientShader::Interpolation interpolation, uint32_t flags,
    const SkMatrix* local_matrix, SkColor4f fallback_color)
{
...
sk_sp<PaintShader> PaintShader::MakeSweepGradient(SkScalar cx, SkScalar cy, const SkColor4f colors[], const SkScalar pos[], int color_count, SkTileMode mode,
    SkScalar start_degrees, SkScalar end_degrees, SkGradientShader::Interpolation interpolation, uint32_t flags, const SkMatrix* local_matrix,
...
sk_sp<PaintShader> PaintShader::MakeImage(const PaintImage& image, SkTileMode tx, SkTileMode ty, const SkMatrix* local_matrix, const SkRect* tile_rect)
{
...
sk_sp<PaintShader> PaintShader::MakePaintRecord(
    PaintRecord record, const SkRect& tile, SkTileMode tx, SkTileMode ty, const SkMatrix* local_matrix, ScalingBehavior scaling_behavior)
{
...
size_t PaintShader::GetSerializedSize(const PaintShader* shader)
{
    ...
    return (base::CheckedNumeric<size_t>(PaintOpWriter::SerializedSize<bool>()) + PaintOpWriter::SerializedSize(shader->shader_type_)
        + PaintOpWriter::SerializedSize(shader->flags_) + PaintOpWriter::SerializedSize(shader->end_radius_)
        + PaintOpWriter::SerializedSize(shader->start_radius_) + PaintOpWriter::SerializedSize(shader->tx_) + PaintOpWriter::SerializedSize(shader->ty_)
        + PaintOpWriter::SerializedSize(shader->fallback_color_) + PaintOpWriter::SerializedSize(shader->scaling_behavior_)
        + PaintOpWriter::SerializedSize(shader->local_matrix_) + PaintOpWriter::SerializedSize(shader->center_) + PaintOpWriter::SerializedSize(shader->tile_)
        + PaintOpWriter::SerializedSize(shader->start_point_) + PaintOpWriter::SerializedSize(shader->end_point_)
        + PaintOpWriter::SerializedSize(shader->start_degrees_) + PaintOpWriter::SerializedSize(shader->end_degrees_)
        + PaintOpWriter::SerializedSize(shader->gradient_interpolation_) + PaintOpWriter::SerializedSize(shader->image_)
        + PaintOpWriter::SerializedSize(shader->id_) + PaintOpWriter::SerializedSize(shader->record_)
        + PaintOpWriter::SerializedSizeOfElements(shader->colors_.data(), shader->colors_.size())
        + PaintOpWriter::SerializedSizeOfElements(shader->positions_.data(), shader->positions_.size()))
        .ValueOrDie();
}
...
bool PaintShader::HasDiscardableImages(gfx::ContentColorUsage* content_color_usage) const
{
    switch (shader_type_) {
    case Type::kEmpty:
    case Type::kColor:
    ...
    case Type::kImage:
        if (image_ && !image_.IsTextureBacked()) {
            if (content_color_usage) {
                *content_color_usage = std::max(*content_color_usage, image_.GetContentColorUsage());
            }
            return true;
        }
        return false;
    case Type::kPaintRecord:
        if (record_ && record_->has_discardable_images()) {
            if (content_color_usage) {
                *content_color_usage = std::max(*content_color_usage, record_->content_color_usage());
            }
            return true;
        }
        return false;
...
bool PaintShader::GetClampedRasterizationTileRect(const SkMatrix& ctm, int max_texture_size, SkRect* tile_rect) const
{
    ...
    *tile_rect = PaintRecord::GetFixedScaleBounds(matrix, tile_, max_texture_size);
    if (tile_rect->isEmpty())
        return false;
    return true;
}

sk_sp<PaintShader> PaintShader::CreateScaledPaintRecord(const SkMatrix& ctm, int max_texture_size, gfx::SizeF* raster_scale) const
{
    DCHECK_EQ(shader_type_, Type::kPaintRecord);
    ...
    *raster_scale = gfx::SizeF(SkIntToScalar(tile_rect.width()) / tile_.width(), SkIntToScalar(tile_rect.height()) / tile_.height());
    SkMatrix local_matrix_with_inv_raster_scale = GetLocalMatrix();
    local_matrix_with_inv_raster_scale.preScale(1 / raster_scale->width(), 1 / raster_scale->height());
...
sk_sp<PaintShader> PaintShader::CreatePaintWorkletRecord(ImageProvider* image_provider) const
{
    DCHECK_EQ(shader_type_, Type::kImage);
    DCHECK(image_ && image_.IsPaintWorklet());

    ImageProvider::ScopedResult result = image_provider->GetRasterContent(DrawImage(image_));
    if (!result || !result.has_paint_record()) {
        return nullptr;
    }
    SkMatrix local_matrix = GetLocalMatrix();
    return PaintShader::MakePaintRecord(result.ReleaseAsRecord(), tile_, tx_, ty_, &local_matrix);
}

sk_sp<PaintShader> PaintShader::CreateDecodedImage(const SkMatrix& ctm, PaintFlags::FilterQuality quality, ImageProvider* image_provider,
    uint32_t* transfer_cache_entry_id, PaintFlags::FilterQuality* raster_quality, bool* needs_mips, gpu::Mailbox* mailbox) const
{
    DCHECK_EQ(shader_type_, Type::kImage);
    if (!image_)
        return nullptr;
    ...
    DrawImage draw_image(image_, false, int_src_rect, quality, SkM44(total_image_matrix));
    ...
    if (need_scale) {
        final_matrix.preScale(1.f / decoded_image.scale_adjustment().width(), 1.f / decoded_image.scale_adjustment().height());
    }
    ...
    if (decoded_image.transfer_cache_entry_id()) {
        ...
        *mailbox = decoded_image.mailbox();
        ...
        sk_sp<SkImage> sk_image = sk_ref_sp<SkImage>(const_cast<SkImage*>(decoded_image.image().get()));
        decoded_paint_image = PaintImageBuilder::WithDefault()
                                  .set_id(image_.stable_id())
                                  .set_texture_image(std::move(sk_image), image_.GetContentIdForFrame(0u))
                                  .TakePaintImage();
    }
...
sk_sp<SkShader> PaintShader::GetSkShader(PaintFlags::FilterQuality quality) const
{
    SkSamplingOptions sampling(PaintFlags::FilterQualityToSkSamplingOptions(quality, PaintFlags::ScalingOperation::kUnknown));
    ...
    case Type::kLinearGradient: {
        ...
        return SkGradientShader::MakeLinear(points, colors_.data(), nullptr /*sk_sp<SkColorSpace>*/, positions_.empty() ? nullptr : positions_.data(),
            static_cast<int>(colors_.size()), tx_, gradient_interpolation_, base::OptionalToPtr(local_matrix_));
    }
    case Type::kRadialGradient:
        return SkGradientShader::MakeRadial(center_, start_radius_, colors_.data(), nullptr /*sk_sp<SkColorSpace>*/,
            positions_.empty() ? nullptr : positions_.data(), static_cast<int>(colors_.size()), tx_, gradient_interpolation_,
            base::OptionalToPtr(local_matrix_));
    case Type::kTwoPointConicalGradient:
        return SkGradientShader::MakeTwoPointConical(start_point_, start_radius_, end_point_, end_radius_, colors_.data(), nullptr /*sk_sp<SkColorSpace>*/,
            positions_.empty() ? nullptr : positions_.data(), static_cast<int>(colors_.size()), tx_, gradient_interpolation_,
            base::OptionalToPtr(local_matrix_));
    case Type::kSweepGradient:
        return SkGradientShader::MakeSweep(center_.x(), center_.y(), colors_.data(), nullptr /*sk_sp<SkColorSpace>*/,
            positions_.empty() ? nullptr : positions_.data(), static_cast<int>(colors_.size()), tx_, start_degrees_, end_degrees_, gradient_interpolation_,
            base::OptionalToPtr(local_matrix_));
    case Type::kImage:
        if (sk_cached_image_) {
            return sk_cached_image_->makeShader(tx_, ty_, sampling, base::OptionalToPtr(local_matrix_));
        }
        break;
    case Type::kPaintRecord:
        if (sk_cached_picture_) {
            ...
            case ScalingBehavior::kRasterAtScale:
                return sk_cached_picture_->makeShader(tx_, ty_, sampling.filter, base::OptionalToPtr(local_matrix_), nullptr);
            ...
            case ScalingBehavior::kFixedScale: {
                auto image = SkImages::DeferredFromPicture(
                    sk_cached_picture_, SkISize::Make(tile_.width(), tile_.height()), nullptr, nullptr, SkImages::BitDepth::kU8, SkColorSpace::MakeSRGB());
                return image->makeShader(tx_, ty_, sampling, base::OptionalToPtr(local_matrix_));
            }
            }
            break;
        }
...
void PaintShader::ResolveSkObjects(const gfx::SizeF* raster_scale, ImageProvider* image_provider)
{
    switch (shader_type_) {
    case Type::kImage:
    ...
    case Type::kPaintRecord: {
        ...
        sk_cached_picture_ = ToSkPicture(*record_, tile_, raster_scale, image_provider);
        break;
    }
    default:
        break;
...
void PaintShader::SetColorsAndPositions(const SkColor4f* colors, const SkScalar* positions, int count)
{
#if DCHECK_IS_ON()
...
void PaintShader::SetMatrixAndTiling(const SkMatrix* matrix, SkTileMode tx, SkTileMode ty)
{
    if (matrix) {
...
void PaintShader::SetFlagsAndFallback(uint32_t flags, SkColor4f fallback_color)
{
    flags_ = flags;
...
bool PaintShader::IsValid() const
{
    ...
    case Type::kColor:
        return true;
    case Type::kSweepGradient:
        if (!std::isfinite(start_degrees_) || !std::isfinite(end_degrees_) || start_degrees_ >= end_degrees_) {
            return false;
        }
        [[fallthrough]];
    case Type::kLinearGradient:
    case Type::kRadialGradient:
    case Type::kTwoPointConicalGradient:
        return colors_.size() >= 2 && (positions_.empty() || positions_.size() == colors_.size());
    case Type::kImage:
...
bool PaintShader::EqualsForTesting(const PaintShader& other) const
{
    ...
    const bool ignore_scaling_differences = shader_type_ == PaintShader::Type::kPaintRecord || shader_type_ == PaintShader::Type::kImage;

    // Variables that all shaders use.
    const SkMatrix& local_matrix = local_matrix_ ? *local_matrix_ : SkMatrix::I();
    const SkMatrix& other_local_matrix = other.local_matrix_ ? *other.local_matrix_ : SkMatrix::I();
    if (!CompareMatricesForTesting(local_matrix, other_local_matrix, // IN-TEST
            ignore_scaling_differences)) {
        return false;
    }

    if (fallback_color_ != other.fallback_color_ || flags_ != other.flags_ || tx_ != other.tx_ || ty_ != other.ty_) {
        return false;
    }
    if (!ignore_scaling_differences && scaling_behavior_ != other.scaling_behavior_) {
        return false;
    }
    ...
    case Type::kColor:
        break;
    case Type::kSweepGradient:
        if (start_degrees_ != other.start_degrees_ || end_degrees_ != other.end_degrees_) {
            return false;
        }
        [[fallthrough]];
    case Type::kLinearGradient:
    case Type::kRadialGradient:
    case Type::kTwoPointConicalGradient:
        if (start_radius_ != other.start_radius_ || end_radius_ != other.end_radius_ || center_ != other.center_ || start_point_ != other.start_point_
            || end_point_ != other.end_point_ || colors_ != other.colors_ || positions_ != other.positions_) {
            return false;
        }
        break;
    case Type::kImage:
