...
public:
    ...
    using SerializeCallback = size_t (*)(void*, const PaintOp&, const PaintOp::SerializeOptions&, const PaintFlags*, const SkM44&, const SkM44&);

    PaintOpBufferSerializer(SerializeCallback serialize_cb, void* callback_data, const PaintOp::SerializeOptions& options);
    ...
    void Serialize(const PaintOpBuffer& buffer, const std::vector<size_t>* offsets, const Preamble& preamble);
    ...
    // rasterization.
    void Serialize(const PaintOpBuffer& buffer, const gfx::Rect& playback_rect, const gfx::SizeF& post_scale);
...
private:
    ...
    void SerializePreamble(SkCanvas* canvas, const Preamble& preamble, const PlaybackParams& params);
    // Serialize `buffer`, using the current transform as `original_ctm`.
    void SerializeBuffer(SkCanvas* canvas, const PaintOpBuffer& buffer, const std::vector<size_t>* offsets);
    // Serialize `buffer` using the provided `params`.
    void SerializeBufferWithParams(SkCanvas* canvas, const PlaybackParams& params, const PaintOpBuffer& buffer, const std::vector<size_t>* offsets);
    ...
    bool WillSerializeNextOp(const PaintOp& op, SkCanvas* canvas, const PlaybackParams& params, F alpha);
    template <typename F>
        requires std::same_as<F, float>
    bool SerializeOpWithFlags(SkCanvas* canvas, const PaintOpWithFlags& flags_op, const PlaybackParams& params, F alpha);

    ALWAYS_INLINE bool SerializeOp(SkCanvas* canvas, const PaintOp& op, const PaintFlags* flags_to_serialize, const PlaybackParams& params);
    void Save(SkCanvas* canvas, const PlaybackParams& params);
    void RestoreToCount(SkCanvas* canvas, int count, const PlaybackParams& params);
    void ClearForOpaqueRaster(SkCanvas* canvas, const Preamble& preamble, const PlaybackParams& params);
    void PlaybackOnAnalysisCanvas(SkCanvas* canvas, const PaintOp& op, const PaintFlags* flags_to_serialize, const PlaybackParams& params);
...
public:
    SimpleBufferSerializer(void* memory, size_t size, const PaintOp::SerializeOptions& options);
...
private:
    ...
    static size_t SerializeToMemory(void* instance, const PaintOp& op, const PaintOp::SerializeOptions& options, const PaintFlags* flags_to_serialize,
        const SkM44& current_ctm, const SkM44& original_ctm)
    {
        return reinterpret_cast<SimpleBufferSerializer*>(instance)->SerializeToMemoryImpl(op, options, flags_to_serialize, current_ctm, original_ctm);
    }
