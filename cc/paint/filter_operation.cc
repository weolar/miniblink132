...
bool FilterOperation::operator==(const FilterOperation& other) const
{
    ...
    if (type_ == DROP_SHADOW) {
        return amount_ == other.amount_ && offset_ == other.offset_ && drop_shadow_color_ == other.drop_shadow_color_;
    }
    if (type_ == REFERENCE) {
...
FilterOperation::FilterOperation(FilterType type, float amount, SkTileMode tile_mode)
    : type_(type)
    , amount_(amount)
    , offset_(0, 0)
    , drop_shadow_color_(SkColors::kTransparent)
...
FilterOperation::FilterOperation(FilterType type, const gfx::Point& offset, float stdDeviation, SkColor4f color)
    : type_(type)
...
FilterOperation::FilterOperation(FilterType type, sk_sp<PaintFilter> image_filter)
    : type_(type)
    , amount_(0)
    , offset_(0, 0)
    , drop_shadow_color_(SkColors::kTransparent)
...
static FilterOperation CreateNoOpFilter(FilterOperation::FilterType type)
{
    ...
    case FilterOperation::DROP_SHADOW:
        return FilterOperation::CreateDropShadowFilter(gfx::Point(0, 0), 0.f, SkColors::kTransparent);
    ...
    case FilterOperation::ALPHA_THRESHOLD:
        return FilterOperation::CreateAlphaThresholdFilter(FilterOperation::ShapeRects());
...
static float ClampAmountForFilterType(float amount, FilterOperation::FilterType type)
{
    switch (type) {
    case FilterOperation::GRAYSCALE:
    case FilterOperation::SEPIA:
...
FilterOperation FilterOperation::Blend(const FilterOperation* from, const FilterOperation* to, double progress)
{
    ...
    blended_filter.set_amount(ClampAmountForFilterType(gfx::Tween::FloatValueBetween(progress, from_op.amount(), to_op.amount()), to_op.type()));
    ...
    if (to_op.type() == FilterOperation::BLUR) {
        ...
        gfx::Point blended_offset(gfx::Tween::LinearIntValueBetween(progress, from_op.offset().x(), to_op.offset().x()),
            gfx::Tween::LinearIntValueBetween(progress, from_op.offset().y(), to_op.offset().y()));
        blended_filter.set_offset(blended_offset);
        blended_filter.set_drop_shadow_color(gfx::Tween::ColorValueBetween(progress, from_op.drop_shadow_color(), to_op.drop_shadow_color()));
    } else if (to_op.type() == FilterOperation::ZOOM) {
        blended_filter.set_zoom_inset(std::max(gfx::Tween::LinearIntValueBetween(progress, from_op.zoom_inset(), to_op.zoom_inset()), 0));
    }
...
void FilterOperation::AsValueInto(base::trace_event::TracedValue* value) const
{
    ...
    case FilterOperation::REFERENCE: {
        ...
        if (image_filter_) {
            value->SetString("filter_type", PaintFilter::TypeToString(image_filter_->type()));
        }
        break;
    }
    case FilterOperation::ALPHA_THRESHOLD: {
...
gfx::Rect MapRectInternal(const FilterOperation& op, const gfx::Rect& rect, const SkMatrix* ctm, SkImageFilter::MapDirection direction)
{
    switch (op.type()) {
    case FilterOperation::BLUR: {
    ...
    case FilterOperation::DROP_SHADOW: {
        ...
        SkVector mapped_drop_shadow_offset = SkVector::Make(op.offset().x(), op.offset().y());
        if (ctm) {
        ...
        result += gfx::Vector2dF(mapped_drop_shadow_offset.x(), mapped_drop_shadow_offset.y());
    ...
    case FilterOperation::REFERENCE: {
        if (!op.image_filter())
            return rect;
        return gfx::SkIRectToRect(op.image_filter()->MapRect(gfx::RectToSkIRect(rect), ctm, direction));
    }
    case FilterOperation::OFFSET: {
        ...
        return gfx::ToEnclosingRect(gfx::RectF(rect) + gfx::Vector2dF(mapped_offset.x(), mapped_offset.y()));
    }
    default:
        return rect;
    }
...
gfx::Rect FilterOperation::MapRect(const gfx::Rect& rect, const std::optional<SkMatrix>& ctm) const
{
    return MapRectInternal(*this, rect, base::OptionalToPtr(ctm), SkImageFilter::kForward_MapDirection);
}

gfx::Rect FilterOperation::MapRectReverse(const gfx::Rect& rect, const SkMatrix& ctm) const
{
    return MapRectInternal(*this, rect, &ctm, SkImageFilter::kReverse_MapDirection);
}
