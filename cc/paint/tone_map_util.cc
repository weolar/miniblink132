...
bool ToneMapUtil::UseGlobalToneMapFilter(const SkColorSpace* cs)
{
    ...
    cs->transferFn(&fn);
    return skcms_TransferFunction_isHLGish(&fn) || skcms_TransferFunction_isPQish(&fn);
}

void ToneMapUtil::AddGlobalToneMapFilterToPaint(SkPaint& paint, const PaintImage& image, sk_sp<SkColorSpace> dst_color_space)
{
    if (!dst_color_space) {
    ...
    if (SkColorSpace::Equals(image.cached_sk_image_->colorSpace(), dst_color_space.get())) {
        return;
    }
    gfx::ColorConversionSkFilterCache cache;
    sk_sp<SkColorFilter> filter = cache.Get(gfx::ColorSpace(*image.cached_sk_image_->colorSpace()), gfx::ColorSpace(*dst_color_space.get()),
        /*src_bit_depth=*/std::nullopt, image.hdr_metadata_, gfx::ColorSpace::kDefaultSDRWhiteLevel, image.target_hdr_headroom_);
    if (paint.getColorFilter()) {
        // Perform tone mapping before the existing color filter.
        paint.setColorFilter(paint.getColorFilter()->makeComposed(std::move(filter)));
    } else {
