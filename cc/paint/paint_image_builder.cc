...
PaintImageBuilder& PaintImageBuilder::operator=(PaintImageBuilder&& other) = default;
PaintImageBuilder::~PaintImageBuilder() = default;
...
PaintImage PaintImageBuilder::TakePaintImage()
{
    ...
    if (paint_image_.HasGainmap()) {
        DCHECK(paint_image_.paint_image_generator_ || paint_image_.gainmap_sk_image_);
    }
    ...
    if (paint_image_.reinterpret_as_srgb_) {
        if (paint_image_.sk_image_) {
            paint_image_.sk_image_ = paint_image_.sk_image_->reinterpretColorSpace(SkColorSpace::MakeSRGB());
        }
        if (paint_image_.cached_sk_image_) {
            paint_image_.cached_sk_image_ = paint_image_.cached_sk_image_->reinterpretColorSpace(SkColorSpace::MakeSRGB());
        }
    }
