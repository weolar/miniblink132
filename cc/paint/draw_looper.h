...
public:
    ...
    template <typename DrawProc> void Apply(SkCanvas* canvas, const SkPaint& paint, DrawProc proc) const
    {
...
private:
    ...
    enum {
        kAllFlagsMask = kPostTransformFlag | kOverrideAlphaFlag | kDontModifyPaintFlag,
    };
    ...
    void UpdateForLayer(const Layer& layer, SkCanvas* canvas, SkPaint* paint) const;
...
public:
    ...
    void AddShadow(SkPoint offset, float blur_sigma, SkColor4f color, uint32_t flags, bool add_on_top = false);
