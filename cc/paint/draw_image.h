...
public:
    ...
    DrawImage(PaintImage image, bool use_dark_mode, const SkIRect& src_rect, PaintFlags::FilterQuality filter_quality, const SkM44& matrix,
    ...
    // color params.
    DrawImage(const DrawImage& other, float scale_adjustment, size_t frame_index, const TargetColorParams& target_color_params);
    ...
    bool use_dark_mode() const
    {
        return use_dark_mode_;
    }
    const SkSize& scale() const
    {
        return scale_;
    }
    const SkIRect& src_rect() const
    {
        return src_rect_;
    }
    ...
    bool matrix_is_decomposable() const
    {
        return matrix_is_decomposable_;
    }
