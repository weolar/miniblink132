...
bool IsSolidColorPaint(const PaintFlags& flags)
{
    ...
    bool is_solid_color = IsSolidColorBlendMode(blendmode) && !flags.HasShader() && !flags.getLooper() && !flags.getColorFilter() && !flags.getImageFilter()
...
template <typename T> bool IsFullQuad(const SkCanvas& canvas, const T& drawn_shape)
{
    ...
    SkV4 lower_right = inverse.map(clip_bounds.right(), clip_bounds.bottom(), 0, 1);
    SkRect transformed_clip_bounds = SkRect::MakeLTRB(upper_left.x, upper_left.y, lower_right.x, lower_right.y);
    return drawn_shape.contains(transformed_clip_bounds);
}

void CalculateSolidColor(SkColor4f src_color, SkBlendMode blendmode, SkColor4f* dst_color, bool* is_solid_color)
{
...
void CheckIfSolidColor(const SkCanvas& canvas, SkColor4f color, SkBlendMode blendmode, bool* is_solid_color, bool* is_transparent, SkColor4f* out_color)
{
    SkRect rect;
    ...
    bool solid_color_candidate = does_cover_canvas && IsSolidColorBlendMode(blendmode);
    ...
    if (solid_color_candidate) {
        CalculateSolidColor(color /* src_color */, blendmode, out_color /* dst_color */, is_solid_color);
    } else {
        *is_solid_color = false;
    }
}
...
template <typename T>
void CheckIfSolidShape(const SkCanvas& canvas, const T& shape, const PaintFlags& flags, bool* is_solid_color, bool* is_transparent, SkColor4f* color)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "SolidColorAnalyzer::CheckIfSolidShape");
    ...
    if (does_cover_canvas && IsSolidColorPaint(flags)) {
        CalculateSolidColor(flags.getColor4f() /* src_color */, flags.getBlendMode(), color /* dst_color */, is_solid_color);
    } else {
        *is_solid_color = false;
    }
}

bool CheckIfRRectClipCoversCanvas(const SkCanvas& canvas, const SkRRect& rrect)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "SolidColorAnalyzer::CheckIfRRectClipCoversCanvas");
...
std::optional<SkColor4f> SolidColorAnalyzer::DetermineIfSolidColor(
    const PaintOpBuffer& buffer, const gfx::Rect& rect, int max_ops_to_analyze, const std::vector<size_t>* offsets)
{
    ...
    struct Frame {
        Frame(PaintOpBuffer::CompositeIterator iter, const SkM44& original_ctm, int save_count)
            : iter(iter)
    ...
    stack.reserve(2);
    stack.emplace_back(PaintOpBuffer::CompositeIterator(buffer, offsets), canvas.getLocalToDevice(), canvas.getSaveCount());
    ...
    while (!stack.empty()) {
        ...
        case PaintOpType::kDrawRecord: {
            ...
            stack.emplace_back(PaintOpBuffer::CompositeIterator(record_op.record.buffer(), nullptr), canvas.getLocalToDevice(), canvas.getSaveCount());
            continue;
        }
        ...
        case PaintOpType::kDrawRRect: {
            ...
            CheckIfSolidShape(canvas, rrect_op.rrect, rrect_op.flags, &is_solid, &is_transparent, &color);
            break;
        }
        ...
        case PaintOpType::kClipRRect: {
            ...
            bool does_cover_canvas = CheckIfRRectClipCoversCanvas(canvas, rrect_op.rrect);
        ...
        case PaintOpType::kDrawRect: {
            ...
            CheckIfSolidShape(canvas, rect_op.rect, rect_op.flags, &is_solid, &is_transparent, &color);
            break;
        }
        case PaintOpType::kDrawColor: {
            ...
            CheckIfSolidColor(canvas, color_op.color, color_op.mode, &is_solid, &is_transparent, &color);
            break;
        }
