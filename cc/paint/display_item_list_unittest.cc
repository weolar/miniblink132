...
bool CompareN32Pixels(void* actual_pixels, void* expected_pixels, int width, int height)
{
    ...
    actual_bitmap.installPixels(actual_info, actual_pixels, actual_info.minRowBytes());

    SkImageInfo expected_info = SkImageInfo::MakeN32Premul(width, height);
    SkBitmap expected_bitmap;
    expected_bitmap.installPixels(expected_info, expected_pixels, expected_info.minRowBytes());
...
protected:
    std::unique_ptr<base::Value> ToBaseValue(const DisplayItemList* list, bool include_items)
    {
...
TEST_F(DisplayItemListTest, SingleUnpairedRange)
{
    ...
    list->StartPaint();
    list->push<SaveOp>();
    list->push<TranslateOp>(static_cast<float>(offset.x()), static_cast<float>(offset.y()));
    ...
    expected_canvas.drawRect(SkRect::MakeLTRB(0.f + offset.x(), 0.f + offset.y(), 60.f + offset.x(), 60.f + offset.y()), red_paint);
    expected_canvas.drawRect(SkRect::MakeLTRB(50.f + offset.x(), 50.f + offset.y(), 75.f + offset.x(), 75.f + offset.y()), blue_flags);
...
TEST_F(DisplayItemListTest, ClipPairedRange)
{
...

    {
        list->StartPaint();
        list->push<SaveOp>();
        list->push<TranslateOp>(static_cast<float>(first_offset.x()), static_cast<float>(first_offset.y()));
    ...
    gfx::Rect clip_rect(60, 60, 10, 10);
    {
        list->StartPaint();
        list->push<SaveOp>();
        list->push<ClipRectOp>(gfx::RectToSkRect(clip_rect), SkClipOp::kIntersect, true);
    ...
    gfx::Rect second_recording_rect(second_offset, layer_rect.size());
    {
        list->StartPaint();
        list->push<SaveOp>();
        list->push<TranslateOp>(static_cast<float>(second_offset.x()), static_cast<float>(second_offset.y()));
        list->push<DrawRectOp>(SkRect::MakeLTRB(50.f, 50.f, 75.f, 75.f), blue_flags);
        list->push<RestoreOp>();
    ...
    SkiaPaintCanvas expected_canvas(expected_bitmap);
    expected_canvas.clipRect(gfx::RectToSkRect(layer_rect));
    expected_canvas.drawRect(SkRect::MakeLTRB(0.f + first_offset.x(), 0.f + first_offset.y(), 60.f + first_offset.x(), 60.f + first_offset.y()), red_paint);
    expected_canvas.clipRect(gfx::RectToSkRect(clip_rect));
    expected_canvas.drawRect(
        SkRect::MakeLTRB(50.f + second_offset.x(), 50.f + second_offset.y(), 75.f + second_offset.x(), 75.f + second_offset.y()), blue_flags);
...
TEST_F(DisplayItemListTest, FilterPairedRange)
{
    ...
    sk_sp<SkSurface> source_surface = SkSurfaces::Raster(SkImageInfo::MakeN32Premul(50, 50));
    ...
    PaintImage source_image = PaintImageBuilder::WithDefault()
                                  ...
                                  .set_image(source_surface->makeImageSnapshot(), PaintImage::GetNextContentId())
                                  .TakePaintImage();
    ...
    sk_sp<PaintFilter> image_filter = sk_make_sp<ImagePaintFilter>(source_image, rect, rect, PaintFlags::FilterQuality::kHigh);
    ...
    // Include a rect drawing so that filter is actually applied to something.
    {
        ...
        list->push<DrawRectOp>(SkRect::MakeLTRB(filter_bounds.x(), filter_bounds.y(), filter_bounds.right(), filter_bounds.bottom()), red_flags);
...
TEST_F(DisplayItemListTest, AsValueWithOps)
{
    ...
    // The traced value has a params dictionary as its root.
    {
        ...
        // The real contents of the traced value is in here.
        {
            const base::Value::List* layer_rect_list = params_dict->FindList("layer_rect");
            ...
            const char* expected_names[] = { "SaveOp", "ConcatOp", "SaveLayerOp", "TranslateOp", "DrawRectOp", "RestoreOp", "RestoreOp" };
            ...
            for (int i = 0; i < 7; ++i) {
                ...
                const base::Value::List* visual_rect = item_dict.FindList("visual_rect");
                ASSERT_NE(nullptr, visual_rect);
                ...
                EXPECT_EQ(expected_has_skp[i], item_dict.FindString("skp64") != nullptr);
            }
        }
    }
...
TEST_F(DisplayItemListTest, SizeMultiple)
{
    ...
    gfx::Rect clip_bounds(5, 6, 7, 8);
    {
        list->StartPaint();
        list->push<SaveOp>();
        list->push<ClipRectOp>(gfx::RectToSkRect(clip_bounds), SkClipOp::kIntersect, false);
        list->EndPaintOfPairedBegin();
    }
    {
        list->StartPaint();
...
TEST_F(DisplayItemListTest, AppendVisualRectDrawingFollowedByBlockContainingEscapedDrawing)
{
    auto list = base::MakeRefCounted<DisplayItemList>();
...
TEST_F(DisplayItemListTest, AppendVisualRectTwoBlocksTwoDrawingsInnerDrawingEscaped)
{
    auto list = base::MakeRefCounted<DisplayItemList>();
...
TEST_F(DisplayItemListTest, AppendVisualRectTwoBlocksTwoDrawingsOuterDrawingEscaped)
{
    auto list = base::MakeRefCounted<DisplayItemList>();
...
TEST_F(DisplayItemListTest, AppendVisualRectTwoBlocksTwoDrawingsBothDrawingsEscaped)
{
    auto list = base::MakeRefCounted<DisplayItemList>();
...
TEST_F(DisplayItemListTest, AreaOfDrawText)
{
    ...
    gfx::Size text_blob1_size(ceilf(text_blob1->bounds().width()), ceilf(text_blob1->bounds().height()));
    auto text_blob1_area = text_blob1_size.width() * text_blob1_size.height();
    auto text_blob2 = SkTextBlob::MakeFromString("EFG", font);
    gfx::Size text_blob2_size(ceilf(text_blob2->bounds().width()), ceilf(text_blob2->bounds().height()));
    ...
    EXPECT_EQ(static_cast<int>(text_blob1_area + text_blob2_area), static_cast<int>(list->AreaOfDrawText(gfx::Rect(200, 200))));
    // This includes all DrawTextBlobOps.
    EXPECT_EQ(static_cast<int>(text_blob1_area * 2 + text_blob2_area * 2), static_cast<int>(list->AreaOfDrawText(gfx::Rect(500, 500))));
}
