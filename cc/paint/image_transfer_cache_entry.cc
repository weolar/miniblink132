...
sk_sp<class SkImage> SkImages::TextureFromImage(skgpu::graphite::Recorder*, SkImage const*, SkImage::RequiredProperties)
{
    *(int*)1 = 1;
    return nullptr;
}

int skgpu::graphite::Recorder::maxTextureSize(void) const
{
    *(int*)1 = 1;
    return 0;
}
...
bool IsYUVAInfoValid(SkYUVAInfo::PlaneConfig plane_config, SkYUVAInfo::Subsampling subsampling, SkYUVColorSpace yuv_color_space)
{
    if (plane_config == SkYUVAInfo::PlaneConfig::kUnknown) {
        return subsampling == SkYUVAInfo::Subsampling::kUnknown && yuv_color_space == kIdentity_SkYUVColorSpace;
    }
...
sk_sp<SkImage> MakeYUVImageFromUploadedPlanes(GrDirectContext* gr_context, skgpu::graphite::Recorder* graphite_recorder,
    const std::vector<sk_sp<SkImage>>& plane_images, const SkYUVAInfo& yuva_info, sk_sp<SkColorSpace> image_color_space)
{
    *(int*)1 = 1;
    return nullptr;
...
//     DCHECK_EQ(static_cast<size_t>(SkYUVAInfo::NumPlanes(yuva_info.planeConfig())), plane_images.size());
//     DCHECK_LE(plane_images.size(), base::checked_cast<size_t>(SkYUVAInfo::kMaxPlanes));
// 
//     if (graphite_recorder) {
//         sk_sp<SkImage> image = SkImages::TextureFromYUVAImages(graphite_recorder, yuva_info, plane_images, image_color_space);
//         if (!image) {
...
//         if (!SkImages::GetBackendTextureFromImage(plane_images[plane], &plane_backend_textures[plane], true /* flushPendingGrContextIO */)) {
...
//     GrYUVABackendTextures yuva_backend_textures(yuva_info, plane_backend_textures.data(), kTopLeft_GrSurfaceOrigin);
//     Context* ctx = new Context { plane_images };
//     sk_sp<SkImage> image = SkImages::TextureFromYUVATextures(gr_context, yuva_backend_textures, std::move(image_color_space), ReleaseContext, ctx);
//     if (!image) {
//         DLOG(ERROR) << "Could not create YUV image";
//         return nullptr;
//     }
...
base::CheckedNumeric<uint32_t> SafeSizeForImage(const ClientImageTransferCacheEntry::Image& image)
{
    base::CheckedNumeric<uint32_t> safe_size;
...
bool ReadPixmap(PaintOpReader& reader, SkPixmap& pixmap)
{
    ...
    if (color_type == kUnknown_SkColorType || color_type == kRGB_101010x_SkColorType || color_type > kLastEnum_SkColorType) {
    ...
    auto image_info = SkImageInfo::Make(width, height, color_type, kPremul_SkAlphaType);
    size_t row_bytes = 0;
    reader.ReadSize(&row_bytes);
    if (row_bytes < image_info.minRowBytes()) {
        DLOG(ERROR) << "Row bytes " << row_bytes << " less than minimum " << image_info.minRowBytes();
        return false;
    }
    size_t data_size = 0;
...
bool WriteImage(PaintOpWriter& writer, const ClientImageTransferCacheEntry::Image& image)
{
    DCHECK(IsYUVAInfoValid(image.yuv_plane_config, image.yuv_subsampling, image.yuv_color_space));
...
sk_sp<SkImage> ReadImage(PaintOpReader& reader, GrDirectContext* gr_context, skgpu::graphite::Recorder* graphite_recorder, bool mip_mapped_for_upload,
    std::optional<SkYUVAInfo>* out_yuva_info = nullptr, std::vector<sk_sp<SkImage>>* out_yuva_plane_images = nullptr)
{
    int max_size;
    if (gr_context) {
    ...
    if (plane_config < SkYUVAInfo::PlaneConfig::kUnknown || plane_config > SkYUVAInfo::PlaneConfig::kLast) {
    ...
    if (subsampling < SkYUVAInfo::Subsampling::kUnknown || subsampling > SkYUVAInfo::Subsampling::kLast) {
    ...
    if (yuv_color_space < kJPEG_Full_SkYUVColorSpace || yuv_color_space > kLastEnum_SkYUVColorSpace) {
    ...
    for (int i = 0; i < num_pixmaps; ++i) {
        ...
        fits_on_gpu &= pixmaps[i].width() <= max_size && pixmaps[i].height() <= max_size;
    ...
    if (plane_config == SkYUVAInfo::PlaneConfig::kUnknown) {
        ...
        if (fits_on_gpu) {
            if (gr_context) {
                image = SkImages::TextureFromImage(
                    gr_context, image, mip_mapped_for_upload ? skgpu::Mipmapped::kYes : skgpu::Mipmapped::kNo, skgpu::Budgeted::kNo);
            } else {
        ...
        for (int i = 0; i < num_pixmaps; i++) {
            sk_sp<SkImage> plane = SkImages::RasterFromPixmap(pixmaps[i], nullptr, nullptr);
            if (!plane) {
            ...
            if (gr_context) {
                plane = SkImages::TextureFromImage(
                    gr_context, plane, mip_mapped_for_upload ? skgpu::Mipmapped::kYes : skgpu::Mipmapped::kNo, skgpu::Budgeted::kNo);
        ...
        SkYUVAInfo yuva_info(plane_images[0]->dimensions(), plane_config, subsampling, yuv_color_space);

        // Build the YUV image from its planes.
        auto image = MakeYUVImageFromUploadedPlanes(gr_context, graphite_recorder, plane_images, yuva_info, color_space);
        if (!image) {
...
ClientImageTransferCacheEntry::Image& ClientImageTransferCacheEntry::Image::operator=(const Image&) = default;
...
ClientImageTransferCacheEntry::Image::Image(base::span<const SkPixmap> yuva_pixmaps, const SkYUVAInfo& yuva_info, const SkColorSpace* color_space)
...
ClientImageTransferCacheEntry::ClientImageTransferCacheEntry(
    const Image& image, bool needs_mips, const std::optional<gfx::HDRMetadata>& hdr_metadata, sk_sp<SkColorSpace> target_color_space)
...
ClientImageTransferCacheEntry::ClientImageTransferCacheEntry(const Image& image, const Image& gainmap_image, const SkGainmapInfo& gainmap_info, bool needs_mips)
    : needs_mips_(needs_mips)
    , id_(GetNextId())
    , image_(image)
...
ServiceImageTransferCacheEntry::ServiceImageTransferCacheEntry(ServiceImageTransferCacheEntry&& other) = default;
ServiceImageTransferCacheEntry& ServiceImageTransferCacheEntry::operator=(ServiceImageTransferCacheEntry&& other) = default;

bool ServiceImageTransferCacheEntry::BuildFromHardwareDecodedImage(GrDirectContext* gr_context, std::vector<sk_sp<SkImage>> plane_images,
    SkYUVAInfo::PlaneConfig plane_config, SkYUVAInfo::Subsampling subsampling, SkYUVColorSpace yuv_color_space, size_t buffer_byte_size, bool needs_mips)
{
    ...
    if (needs_mips) {
        ...
        for (size_t plane = 0; plane < plane_images.size(); plane++) {
            plane_images[plane] = SkImages::TextureFromImage(gr_context_, plane_images[plane], skgpu::Mipmapped::kYes, skgpu::Budgeted::kNo);
    ...
    if (static_cast<size_t>(SkYUVAInfo::NumPlanes(plane_config)) != plane_images_.size()) {
        DLOG(ERROR) << "Expected " << SkYUVAInfo::NumPlanes(plane_config) << " planes, got " << plane_images_.size();
        return false;
    }
    yuva_info_ = SkYUVAInfo(plane_images_[0]->dimensions(), plane_config, subsampling, yuv_color_space);
    ...
    image_ = MakeYUVImageFromUploadedPlanes(gr_context_, /*graphite_recorder=*/nullptr, plane_images_, yuva_info_.value(), SkColorSpace::MakeSRGB());
    if (!image_) {
        return false;
    }
...
bool ServiceImageTransferCacheEntry::Deserialize(GrDirectContext* gr_context, skgpu::graphite::Recorder* graphite_recorder, base::span<const uint8_t> data)
{
    gr_context_ = gr_context;
    ...
    image_ = ReadImage(reader, gr_context, graphite_recorder, mip_mapped_for_upload, &yuva_info_, &plane_images_);
    if (!image_) {
    ...
    if (has_gainmap_) {
        gainmap_image_ = ReadImage(reader, gr_context, graphite_recorder, mip_mapped_for_upload);
        if (!gainmap_image_) {
    ...
    const bool is_tone_mapped = has_gainmap_ || ToneMapUtil::UseGlobalToneMapFilter(image_->colorSpace());
    ...
    if (target_color_space && !is_tone_mapped) {
        if (graphite_recorder_) {
            ...
            image_ = image_->makeColorSpace(graphite_recorder_, target_color_space, props);
        } else {
            ...
            if (needs_mips && gr_context_ && image_ && image_->isTextureBacked()) {
                image_ = SkImages::TextureFromImage(gr_context, image_, skgpu::Mipmapped::kYes, skgpu::Budgeted::kNo);
            }
        }
        if (!image_) {
    ...
    // this this call).
    auto copy_from_transfer_buffer = [](sk_sp<SkImage>& image, sk_sp<SkImage> image_referencing_transfer_buffer) {
    ...
    if (!copy_from_transfer_buffer(gainmap_image_, gainmap_image_referencing_transfer_buffer)) {
        return false;
    }
...
const sk_sp<SkImage>& ServiceImageTransferCacheEntry::GetPlaneImage(size_t index) const
{
    DCHECK_GE(index, 0u);
...
void ServiceImageTransferCacheEntry::EnsureMips()
{
    ...
    if (is_yuv()) {
        ...
        DCHECK_EQ(static_cast<size_t>(SkYUVAInfo::NumPlanes(yuva_info_->planeConfig())), plane_images_.size());
        ...
        for (size_t plane = 0; plane < plane_images_.size(); plane++) {
            ...
            if (gr_context_) {
                mipped_plane = SkImages::TextureFromImage(gr_context_, plane_images_.at(plane), skgpu::Mipmapped::kYes, skgpu::Budgeted::kNo);
            } else {
                CHECK(graphite_recorder_);
                SkImage::RequiredProperties props { .fMipmapped = true };
                mipped_plane = SkImages::TextureFromImage(graphite_recorder_, plane_images_.at(plane), props);
            }
            if (!mipped_plane) {
        ...
        sk_sp<SkImage> mipped_image = MakeYUVImageFromUploadedPlanes(
            gr_context_, graphite_recorder_, mipped_planes, yuva_info_.value(), image_->refColorSpace() /* image_color_space */);
        if (!mipped_image) {
        ...
        if (gr_context_) {
            mipped_image = SkImages::TextureFromImage(gr_context_, image_, skgpu::Mipmapped::kYes, skgpu::Budgeted::kNo);
        } else {
            CHECK(graphite_recorder_);
            SkImage::RequiredProperties props { .fMipmapped = true };
            mipped_image = SkImages::TextureFromImage(graphite_recorder_, image_, props);
        }
        if (!mipped_image) {
