...
public:
    ...
    using CustomDataRasterCallback = base::RepeatingCallback<void(SkCanvas* canvas, uint32_t id)>;
    ...
    using ConvertOpCallback = base::RepeatingCallback<const PaintOp*(const PaintOp& op)>;
...
public:
    explicit PlaybackParams(
        ImageProvider* image_provider = nullptr, const SkM44& original_ctm = SkM44(), const PlaybackCallbacks& callbacks = PlaybackCallbacks());
    ~PlaybackParams();
...
public:
    ...
    virtual sk_sp<SkImage> OpenSharedImageForRead(const gpu::Mailbox& mailbox, Error& error) = 0;
};
...
public:
    ...
    public:
        SerializeOptions();
        SerializeOptions(ImageProvider* image_provider, TransferCacheSerializeHelper* transfer_cache, ClientPaintCache* paint_cache,
            ...
            bool context_supports_distance_field_text, int max_texture_size, const ScrollOffsetMap* raster_inducing_scroll_offsets = nullptr);
    ...
    template <typename Op> static constexpr uint16_t ComputeOpAlignedSize()
    {
    ...
    void Playback(SkCanvas* canvas, const PlaybackParams& params, bool local_ctm = true) const;

    // Deserialize PaintOps from |input|. The original content will be
    // overwritten.
    bool Deserialize(const volatile void* input, size_t input_size, const DeserializeOptions& options);

    static sk_sp<PaintOpBuffer> MakeFromMemory(const volatile void* input, size_t input_size, const DeserializeOptions& options);
    ...
    static SkRect GetFixedScaleBounds(const SkMatrix& ctm, const SkRect& bounds, int max_texture_size = 0);
    ...
    size_t paint_ops_size() const
    {
        return used_ + subrecord_bytes_used_;
    }
    // Returns the total number of ops including sub-records.
    size_t total_op_count() const
    {
        return op_count_ + subrecord_op_count_;
    }

    size_t next_op_offset() const
    {
        return used_;
    }
    ...
    bool has_non_aa_paint() const
    {
        return has_non_aa_paint_;
    }
    bool has_draw_ops() const
    {
        return has_draw_ops_;
    }
    bool has_draw_text_ops() const
    {
        return has_draw_text_ops_;
    }
    bool has_save_layer_ops() const
    {
        return has_save_layer_ops_;
    }
    bool has_save_layer_alpha_ops() const
    {
        return has_save_layer_alpha_ops_;
    }
    ...
    bool NeedsAdditionalInvalidationForLCDText(const PaintOpBuffer& old_buffer) const;
    ...
    template <typename T, typename... Args> const T& push(Args&&... args)
    {
        DCHECK(is_mutable());
    ...
    template <typename T> void AnalyzeAddedOp(const T* op)
    {
        static_assert(!std::is_same<T, PaintOp>::value, "AnalyzeAddedOp needs a subtype of PaintOp");
        DCHECK(is_mutable());
        DCHECK(op->IsValid());
        ...
        has_effects_preventing_lcd_text_for_save_layer_alpha_ |= op->HasEffectsPreventingLCDTextForSaveLayerAlpha();

        has_discardable_images_ |= op->HasDiscardableImages(&content_color_usage_);
        has_discardable_images_ |= op->HasDiscardableImagesFromFlags(&content_color_usage_);
    }

    size_t GetOpOffsetForTracing(const PaintOp& op) const
    {
        DCHECK_GE(reinterpret_cast<const char*>(&op), data_.get());
        size_t result = static_cast<size_t>(reinterpret_cast<const char*>(&op) - data_.get());
        DCHECK_LT(result, used_);
        return result;
    }

    const char* DataBufferForTesting() const
    {
        return data_.get();
    }
...
private:
    ...
    void Playback(SkCanvas* canvas, const PlaybackParams& params, bool local_ctm, const std::vector<size_t>* offsets) const;
