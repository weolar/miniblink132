...
public:
    ...
    template <typename T> T* GetEntryAs(uint32_t id)
    {
    ...
    virtual void CreateLocalEntry(uint32_t id, std::unique_ptr<ServiceTransferCacheEntry> entry) = 0;

    size_t GetTotalEntrySizes() const
    {
        return total_size_;
    }

private:
    virtual ServiceTransferCacheEntry* GetEntryInternal(TransferCacheEntryType entry_type, uint32_t entry_id) = 0;
