...
public:
    BlendColorFilter(const SkColor4f& color, SkBlendMode blend_mode)
        : ColorFilter(Type::kBlend, SkColorFilters::Blend(color, nullptr, blend_mode))
        , color_(color)
...
private:
    size_t SerializedDataSize() const override
    {
        return PaintOpWriter::SerializedSize(color_) + PaintOpWriter::SerializedSize(blend_mode_);
    }
    void SerializeData(PaintOpWriter& writer) const override
    {
        writer.Write(color_);
...
public:
    SRGBToLinearGammaColorFilter()
        : ColorFilter(Type::kSRGBToLinearGamma, SkColorFilters::SRGBToLinearGamma())
    {
    }
};
...
public:
    LinearToSRGBGammaColorFilter()
        : ColorFilter(Type::kLinearToSRGBGamma, SkColorFilters::LinearToSRGBGamma())
    {
    }
};
...
public:
    LumaColorFilter()
        : ColorFilter(Type::kLuma, SkLumaColorFilter::Make())
    {
    }
};
...
ColorFilter::ColorFilter(Type type, sk_sp<SkColorFilter> sk_color_filter)
    : type_(type)
    , sk_color_filter_(std::move(sk_color_filter))
{
...
sk_sp<ColorFilter> ColorFilter::MakeBlend(const SkColor4f& color, SkBlendMode blend_mode)
{
...
sk_sp<ColorFilter> ColorFilter::MakeTableARGB(const uint8_t a_table[256], const uint8_t r_table[256], const uint8_t g_table[256], const uint8_t b_table[256])
{
...
{
    return sk_color_filter_ ? sk_color_filter_->filterColor4f(color, nullptr, nullptr) : color;
}
