...
public:
    DecodedDrawImage(sk_sp<SkImage> image, sk_sp<ColorFilter> dark_mode_color_filter, const SkSize& src_rect_offset, const SkSize& scale_adjustment,
        PaintFlags::FilterQuality filter_quality, bool is_budgeted);
    DecodedDrawImage(const gpu::Mailbox& mailbox, PaintFlags::FilterQuality filter_quality);
    DecodedDrawImage(std::optional<uint32_t> transfer_cache_entry_id, sk_sp<ColorFilter> dark_mode_color_filter, const SkSize& src_rect_offset,
    ...
    const SkSize& scale_adjustment() const
    {
        return scale_adjustment_;
    }
