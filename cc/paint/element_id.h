...
struct CC_PAINT_EXPORT ElementId {
    ...
    static constexpr InternalValue kDeletedElementId = std::numeric_limits<InternalValue>::max();
    ...
    explicit constexpr ElementId(InternalValue id)
        : id_(id)
    {
    ...
    {
        return IsValidInternalValue(id_);
    }
...
CC_PAINT_EXPORT std::ostream& operator<<(std::ostream& out, const ElementId& id);
