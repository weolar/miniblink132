...
static constexpr uint32_t kInvalidImageTransferCacheEntryId = static_cast<uint32_t>(-1);
...
class CC_PAINT_EXPORT ClientImageTransferCacheEntry final : public ClientTransferCacheEntryBase<TransferCacheEntryType::kImage> {
public:
    ...
    struct CC_PAINT_EXPORT Image {
        ...
        Image(base::span<const SkPixmap> yuva_pixmaps, const SkYUVAInfo& yuva_info, const SkColorSpace* color_space);

        // The pixmaps for each plane.
        std::array<const SkPixmap*, SkYUVAInfo::kMaxPlanes> pixmaps = { nullptr, nullptr, nullptr, nullptr };

        // The YUVA parameters. These should be unchanged for RGBA images.
        SkYUVAInfo::PlaneConfig yuv_plane_config = SkYUVAInfo::PlaneConfig::kUnknown;
    ...
    ClientImageTransferCacheEntry(const Image& image, bool needs_mips, const std::optional<gfx::HDRMetadata>& hdr_metadata = std::nullopt,
        sk_sp<SkColorSpace> target_color_space = nullptr);
    ClientImageTransferCacheEntry(const Image& image, const Image& gainmap_image, const SkGainmapInfo& gainmap_info, bool needs_mips);
...
class CC_PAINT_EXPORT ServiceImageTransferCacheEntry final : public ServiceTransferCacheEntryBase<TransferCacheEntryType::kImage> {
public:
    ...
    ServiceImageTransferCacheEntry& operator=(ServiceImageTransferCacheEntry&& other);
    ...
    bool BuildFromHardwareDecodedImage(GrDirectContext* gr_context, std::vector<sk_sp<SkImage>> plane_images, SkYUVAInfo::PlaneConfig plane_config,
        SkYUVAInfo::Subsampling subsampling, SkYUVColorSpace yuv_color_space, size_t buffer_byte_size, bool needs_mips);

    // ServiceTransferCacheEntry implementation:
    size_t CachedSize() const final;
    bool Deserialize(GrDirectContext* gr_context, skgpu::graphite::Recorder* graphite_recorder, base::span<const uint8_t> data) final;
    ...
    {
        return gainmap_image_ != nullptr;
    }
    const sk_sp<SkImage>& gainmap_image() const
    {
        return gainmap_image_;
    }
    const SkGainmapInfo& gainmap_info() const
    {
        return gainmap_info_;
    }
    ...
    bool is_yuv() const
    {
        return !plane_images_.empty();
    }
    size_t num_planes() const
    {
        return plane_images_.size();
    }
