...
public:
    ...
    using AnimatedPropertyValues = base::flat_map<PaintWorkletInput::PropertyKey, PaintWorkletInput::PropertyValue>;
    PaintWorkletJob(int layer_id, scoped_refptr<const PaintWorkletInput> input, AnimatedPropertyValues animated_property_values);
    ...
    const scoped_refptr<const PaintWorkletInput>& input() const
    {
        return input_;
    }
...
using PaintWorkletJobVector = base::RefCountedData<std::vector<PaintWorkletJob>>;
using PaintWorkletId = int;
using PaintWorkletJobMap = base::flat_map<PaintWorkletId, scoped_refptr<PaintWorkletJobVector>>;
