...
public:
    ...
    struct ImageAssetMetadata {
        ImageAssetMetadata(base::FilePath resource_path_in, std::optional<gfx::Size> size_in);
    ...
    using Storage = base::flat_map<std::string /*resource_id*/, ImageAssetMetadata>;
    ...
    // invalid.
    bool RegisterAsset(std::string_view resource_path, std::string_view resource_name, std::string_view resource_id, std::optional<gfx::Size> size);

    // Returns all external image assets.
    const Storage& asset_storage() const
    {
...
using SkottieResourceIdHash = base::IdType<SkottieResourceMetadataMap, size_t, 0>;
SkottieResourceIdHash CC_PAINT_EXPORT HashSkottieResourceId(std::string_view resource_id);
