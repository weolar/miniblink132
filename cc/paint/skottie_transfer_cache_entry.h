...
class CC_PAINT_EXPORT ClientSkottieTransferCacheEntry final : public ClientTransferCacheEntryBase<TransferCacheEntryType::kSkottie> {
public:
    explicit ClientSkottieTransferCacheEntry(scoped_refptr<SkottieWrapper> skottie);
...
class CC_PAINT_EXPORT ServiceSkottieTransferCacheEntry final : public ServiceTransferCacheEntryBase<TransferCacheEntryType::kSkottie> {
public:
    ...
    bool Deserialize(GrDirectContext* context, skgpu::graphite::Recorder* graphite_recorder, base::span<const uint8_t> data) final;

    const scoped_refptr<SkottieWrapper>& skottie() const
    {
        return skottie_;
    }
