...
sk_sp<SkImage> MakeSkImage(const gfx::Size& size, sk_sp<SkColorSpace> color_space = nullptr)
{
    SkBitmap bitmap;
    bitmap.allocPixelsFlags(SkImageInfo::MakeN32Premul(size.width(), size.height(), color_space), SkBitmap::kZeroPixels_AllocFlag);
...
constexpr PaintFlags::FilterQuality kDefaultFilterQuality = PaintFlags::FilterQuality::kNone;

class OopPixelTest : public testing::Test, public gpu::raster::GrShaderCache::Client {
public:
    OopPixelTest()
        : gr_shader_cache_(kCacheLimitBytes, this)
    {
    }
    ...
    void InitializeOOPContext()
    {
        ...
        raster_context_provider_ = base::MakeRefCounted<viz::TestInProcessContextProvider>(
            viz::TestContextType::kGpuRaster, /*support_locking=*/false, &gr_shader_cache_, &use_shader_cache_shm_count_);
        gpu::ContextResult result = raster_context_provider_->BindToCurrentSequence();
        DCHECK_EQ(result, gpu::ContextResult::kSuccess);
        const int raster_max_texture_size = raster_context_provider_->ContextCapabilities().max_texture_size;
        oop_image_cache_ = std::make_unique<GpuImageDecodeCache>(
            raster_context_provider_.get(), true, kRGBA_8888_SkColorType, kWorkingSetSize, raster_max_texture_size, nullptr);
    }
    ...
    public:
        ...
        PlaybackImageProvider::RasterMode image_provider_raster_mode = PlaybackImageProvider::RasterMode::kSoftware;
    ...
    SkBitmap Raster(scoped_refptr<DisplayItemList> display_item_list, const gfx::Size& playback_size)
    {
        RasterOptions options(playback_size);
        return Raster(display_item_list, options);
    }

    SkBitmap Raster(scoped_refptr<DisplayItemList> display_item_list, const RasterOptions& options)
    {
        ...
        PlaybackImageProvider image_provider(oop_image_cache_.get(), options.target_color_params, std::move(settings));
        ...
        gpu::SharedImageUsageSet flags = gpu::SHARED_IMAGE_USAGE_RASTER_READ | gpu::SHARED_IMAGE_USAGE_RASTER_WRITE | gpu::SHARED_IMAGE_USAGE_OOP_RASTERIZATION;
        auto client_shared_image = sii->CreateSharedImage(
            { viz::SinglePlaneFormat::kRGBA_8888, gfx::Size(width, height), options.target_color_params.color_space, flags, "TestLabel" },
            gpu::kNullSurfaceHandle);
        EXPECT_TRUE(client_shared_image->mailbox().Verify());
        ri->WaitSyncTokenCHROMIUM(sii->GenUnverifiedSyncToken().GetConstData());

        // Assume legacy MSAA if sample count is positive.
        gpu::raster::MsaaMode msaa_mode = options.msaa_sample_count > 0 ? gpu::raster::kMSAA : gpu::raster::kNoMSAA;

        if (options.preclear) {
            ri->BeginRasterCHROMIUM(options.preclear_color,
                /*needs_clear=*/options.preclear, options.msaa_sample_count, msaa_mode, options.use_lcd_text,
                /*visible=*/true, options.target_color_params.color_space, options.target_color_params.hdr_max_luminance_relative,
        ...
        ri->BeginRasterCHROMIUM(options.background_color,
            /*needs_clear=*/!options.preclear, options.msaa_sample_count, msaa_mode, options.use_lcd_text,
            /*visible=*/true, options.target_color_params.color_space, options.target_color_params.hdr_max_luminance_relative,
            client_shared_image->mailbox().name);
        size_t max_op_size_limit = gpu::raster::RasterInterface::kDefaultMaxOpSizeHint;
        ri->RasterCHROMIUM(display_item_list.get(), &image_provider, options.content_size, options.full_raster_rect, options.playback_rect,
            options.post_translate, gfx::Vector2dF(options.post_scale, options.post_scale), options.requires_clear, /*raster_inducing_scroll_offsets=*/nullptr,
            &max_op_size_limit);
        for (const auto& list : options.additional_lists) {
            ri->RasterCHROMIUM(list.get(), &image_provider, options.content_size, options.full_raster_rect, options.playback_rect, options.post_translate,
                gfx::Vector2dF(options.post_scale, options.post_scale), options.requires_clear,
        ...
        SkBitmap result = ReadbackMailbox(ri, client_shared_image->mailbox(), options.resource_size);
    ...
    SkBitmap ReadbackMailbox(
        gpu::raster::RasterInterface* ri, const gpu::Mailbox& mailbox, const gfx::Size& image_size, sk_sp<SkColorSpace> color_space = nullptr)
    {
        SkImageInfo image_info = SkImageInfo::MakeN32Premul(image_size.width(), image_size.height(), color_space);
        SkBitmap result;
    ...
    scoped_refptr<gpu::ClientSharedImage> CreateClientSharedImage(gpu::raster::RasterInterface* ri, gpu::SharedImageInterface* sii,
        const RasterOptions& options, viz::SharedImageFormat format, std::optional<gfx::ColorSpace> color_space = std::nullopt)
    {
        // These SharedImages serve as both the source of reads and destination of
        // writes via the raster interface in these tests.
        gpu::SharedImageUsageSet flags = gpu::SHARED_IMAGE_USAGE_RASTER_READ | gpu::SHARED_IMAGE_USAGE_RASTER_WRITE | gpu::SHARED_IMAGE_USAGE_OOP_RASTERIZATION;
        auto client_shared_image = sii->CreateSharedImage(
            { format, options.resource_size, color_space.value_or(options.target_color_params.color_space), flags, "TestLabel" }, gpu::kNullSurfaceHandle);
        EXPECT_TRUE(client_shared_image->mailbox().Verify());
        ri->WaitSyncTokenCHROMIUM(sii->GenUnverifiedSyncToken().GetConstData());
    ...
    void UploadPixels(gpu::raster::RasterInterface* ri, const gpu::Mailbox& mailbox, const SkImageInfo& info, const SkBitmap& bitmap)
    {
        ri->WritePixels(mailbox, /*dst_x_offset=*/0, /*dst_y_offset=*/0,
            /*texture_target=*/0, SkPixmap(info, bitmap.getPixels(), info.minRowBytes()));
        EXPECT_EQ(ri->GetError(), static_cast<unsigned>(GL_NO_ERROR));
    }

    void UploadPixelsYUV(gpu::raster::RasterInterface* ri, const gpu::Mailbox& mailbox, const SkYUVAPixmaps& yuv_pixmap)
    {
    ...
    void ExpectEquals(const SkBitmap& actual, const base::FilePath::StringType& ref_filename, const PixelComparator& comparator = ExactPixelComparator())
    {
        base::FilePath test_data_dir;
        ASSERT_TRUE(base::PathService::Get(viz::Paths::DIR_TEST_DATA, &test_data_dir));
    ...
    void ExpectEquals(SkBitmap actual, SkBitmap expected, const PixelComparator& comparator = ExactPixelComparator())
    {
...
class OopClearPixelTest : public OopPixelTest, public ::testing::WithParamInterface<bool> {
public:
    bool IsPartialRaster() const
    {
        return GetParam();
    }
};
...
TEST_F(OopPixelTest, DrawColorWithTargetColorSpace)
{
    ...
    SkBitmap expected = MakeSolidColorBitmap(rect.size(), SkColor4f::FromColor(SkColorSetARGB(255, 38, 15, 221)));
...
TEST_F(OopPixelTest, DrawRect)
{
    ...
    std::vector<std::pair<SkRect, PaintFlags>> input
        = { { SkRect::MakeXYWH(0, 0, 5, 5), color_paint(0, 0, 255) }, { SkRect::MakeXYWH(5, 0, 5, 5), color_paint(0, 255, 0) },
              { SkRect::MakeXYWH(0, 5, 5, 5), color_paint(0, 255, 255) }, { SkRect::MakeXYWH(5, 5, 5, 5), color_paint(255, 0, 0) } };

    auto display_item_list = base::MakeRefCounted<DisplayItemList>();
    for (auto& op : input) {
        display_item_list->StartPaint();
        display_item_list->push<DrawRectOp>(op.first, op.second);
        display_item_list->EndPaintOfUnpaired(gfx::ToEnclosingRect(gfx::SkRectToRectF(op.first)));
    }
    display_item_list->Finalize();
    ...
    for (int h = 0; h < rect.height(); ++h) {
        ...
        SkPMColor left_color = SkPreMultiplyColor(h < 5 ? input[0].second.getColor() : input[2].second.getColor());
        SkPMColor right_color = SkPreMultiplyColor(h < 5 ? input[1].second.getColor() : input[3].second.getColor());

        std::fill(start, start + 5, left_color);
        std::fill(start + 5, start + 10, right_color);
    }
    SkBitmap expected;
    expected.installPixels(SkImageInfo::MakeN32Premul(rect.width(), rect.height()), expected_pixels.data(), rect.width() * sizeof(SkPMColor));
    ExpectEquals(actual, expected);
}
...
TEST_F(OopPixelTest, DrawRecordPaintFilterTranslatedBounds)
{
    ...
    filter_buffer.push<DrawRectOp>(SkRect::MakeLTRB(output_size.width() / 2.f, 0.f, output_size.width(), output_size.height()), internal_flags);
    sk_sp<RecordPaintFilter> record_filter = sk_make_sp<RecordPaintFilter>(
        filter_buffer.ReleaseAsRecord(), SkRect::MakeLTRB(output_size.width() / 2.f, output_size.height() / 2.f, output_size.width(), output_size.height()));
    ...
    SkImageInfo ii = SkImageInfo::MakeN32Premul(output_size.width(), output_size.height());
    SkBitmap expected;
    expected.allocPixels(ii, ii.minRowBytes());
    expected.eraseColor(SkColors::kWhite);
    expected.erase(
        SkColors::kGreen.toSkColor(), SkIRect::MakeLTRB(output_size.width() / 2, output_size.height() / 2, output_size.width(), output_size.height()));
...
TEST_F(OopPixelTest, DrawImage)
{
    ...
    auto builder = PaintImageBuilder::WithDefault().set_image(image, 0).set_id(kSomeId);
    auto paint_image = builder.TakePaintImage();

    auto display_item_list = base::MakeRefCounted<DisplayItemList>();
    display_item_list->StartPaint();
    SkSamplingOptions sampling(PaintFlags::FilterQualityToSkSamplingOptions(kDefaultFilterQuality));

    display_item_list->push<DrawImageOp>(paint_image, 0.f, 0.f, sampling, nullptr);
    display_item_list->EndPaintOfUnpaired(rect);
    display_item_list->Finalize();
...
TEST_F(OopPixelTest, DrawImageScaled)
{
    ...
    sk_sp<SkImage> image = MakeSkImage(rect.size());
    auto builder = PaintImageBuilder::WithDefault().set_image(image, 0).set_id(PaintImage::GetNextId());
    auto paint_image = builder.TakePaintImage();
...
TEST_F(OopPixelTest, DrawImageShaderScaled)
{
    ...
    auto builder = PaintImageBuilder::WithDefault().set_image(image, 0).set_id(PaintImage::GetNextId());
    auto paint_image = builder.TakePaintImage();
    auto paint_image_shader = PaintShader::MakeImage(paint_image, SkTileMode::kRepeat, SkTileMode::kRepeat, nullptr);
...
TEST_F(OopPixelTest, DrawRecordShaderWithImageScaled)
{
    ...
    auto paint_record_shader
        = PaintShader::MakePaintRecord(paint_buffer.ReleaseAsRecord(), gfx::RectToSkRect(rect), SkTileMode::kRepeat, SkTileMode::kRepeat, nullptr);
...
TEST_F(OopPixelTest, DrawRecordShaderTranslatedTileRect)
{
    ...
    shader_buffer.push<DrawRectOp>(SkRect::MakeXYWH(x_offset, y_offset, 1, 2), internal_flags);

    SkRect tile_rect = SkRect::MakeXYWH(x_offset, y_offset, 2, 3);
    sk_sp<PaintShader> paint_record_shader = PaintShader::MakePaintRecord(
        shader_buffer.ReleaseAsRecord(), tile_rect, SkTileMode::kRepeat, SkTileMode::kRepeat, nullptr, PaintShader::ScalingBehavior::kRasterAtScale);
...
TEST_F(OopPixelTest, DrawImageWithTargetColorSpace)
{
    ...
    RasterOptions options(rect.size());
    options.target_color_params.color_space = gfx::ColorSpace::CreateDisplayP3D65();
    ...
    ExpectEquals(actual, FILE_PATH_LITERAL("oop_image_target_color_space.png"), comparator);
...
TEST_F(OopPixelTest, DrawGainmapImage)
{
    ...
    auto base_color_space = SkColorSpace::MakeRGB(SkNamedTransferFn::k2Dot2, SkNamedGamut::kSRGB);
    auto base_info = SkImageInfo::MakeN32Premul(kSize.width(), kSize.height(), base_color_space);
    ...
    auto gain_info = SkImageInfo::MakeN32Premul(kSize.width(), kSize.height(), SkColorSpace::MakeSRGB());
    ...
    SkSamplingOptions sampling = SkSamplingOptions(SkCubicResampler::CatmullRom());
    display_item_list->push<DrawImageOp>(paint_image, 0.f, 0.f, sampling, nullptr);
    ...
    auto dest_color_space = SkColorSpace::MakeRGB(skia::ScaleTransferFunction(SkNamedTransferFn::k2Dot2, kDestScale), SkNamedGamut::kSRGB);

    // Raster with headroom of 1, and the result should be the input, converted
    // to the output space.
    {
        RasterOptions options(kSize);
        options.target_color_params.color_space = gfx::ColorSpace(*dest_color_space);
...
TEST_F(OopPixelTest, DrawGainmapImageCubic)
{
    ...
    auto info = SkImageInfo::MakeN32Premul(kSrcSize, kSrcSize, SkColorSpace::MakeSRGBLinear());
    sk_sp<FakePaintImageGenerator> generators[2];
    uint8_t pixel_values[2][2] = { { 100, 20 },
        { static_cast<uint8_t>(std::round(255.f * std::log(2.f) / std::log(kRatioMax))),
            static_cast<uint8_t>(std::round(255.f * std::log(3.f) / std::log(kRatioMax))) } };
    for (int i = 0; i < 2; ++i) {
    ...
    {
        display_item_list->StartPaint();
        display_item_list->push<DrawImageRectOp>(paint_image, SkRect::MakeWH(kSrcSize, kSrcSize), SkRect::MakeWH(kDstSize, kDstSize),
            SkSamplingOptions(SkCubicResampler::CatmullRom()), nullptr, SkCanvas::kStrict_SrcRectConstraint);
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        float base_value = ((3.f - i) * pixel_values[0][0] + i * pixel_values[0][1]) / (255.f * 3.f);
        float gain_value = ((3.f - i) * pixel_values[1][0] + i * pixel_values[1][1]) / (255.f * 3.f);
...
TEST_F(OopPixelTest, DrawGainmapImageFiltering)
{
    ...
    auto base_info = SkImageInfo::MakeN32Premul(kSize.width(), kSize.height(), SkColorSpace::MakeSRGB());
    auto base_image_generator = sk_make_sp<FakePaintImageGenerator>(base_info);
    {
        SkBitmap bitmap;
        bitmap.installPixels(base_image_generator->GetPixmap());
        ...
        paint.setColor({ kValueBase, kValueBase, kValueBase, 1.f }, SkColorSpace::MakeSRGBLinear().get());
        canvas.drawRect(SkRect(0, 0, 4, 4), paint);
    }

    // The gainmap is fully applied at headroom 2, and has a maximum scale of 4.
    const float kRatioMax = 4.f;
    auto gain_info = SkImageInfo::MakeN32Premul(kGainSize.width(), kGainSize.height(), SkColorSpace::MakeSRGBLinear());
    auto gain_image_generator = sk_make_sp<FakePaintImageGenerator>(gain_info);
    SkGainmapInfo gainmap_info = {
        { 1.f, 1.f, 1.f, 1.f },
        { kRatioMax, kRatioMax, kRatioMax, 1.f },
    ...
    {
        display_item_list->StartPaint();
        display_item_list->push<DrawImageOp>(paint_image, 0.f, 0.f, SkSamplingOptions(SkFilterMode::kNearest), nullptr);
        display_item_list->EndPaintOfUnpaired(kRect);
        display_item_list->Finalize();
    }
...
TEST_F(OopPixelTest, DrawHdrImageWithMetadata)
{
    ...
    sk_sp<SkImage> image;
    {
        ...
        SkBitmap bitmap;
        bitmap.allocPixelsFlags(SkImageInfo::MakeN32Premul(kSize.width(), kSize.height(), SkColorSpace::MakeSRGB()), SkBitmap::kZeroPixels_AllocFlag);
        ...
        image = image->reinterpretColorSpace(gfx::ColorSpace::CreateHDR10().ToSkColorSpace());
    }

    const auto make_display_item_list = [&](int i, float peak_luminance, PaintFlags* paint_flags = nullptr) {
        auto image_generator = sk_make_sp<FakePaintImageGenerator>(image->imageInfo());
        {
            ImageHeaderMetadata image_metadata;
            image_metadata.hdr_metadata.emplace(gfx::HdrMetadataCta861_3(peak_luminance, kContentAvgNits));
        ...
        auto paint_image = PaintImageBuilder::WithDefault().set_id(kSomeId).set_paint_image_generator(image_generator).TakePaintImage();

        auto display_item_list = base::MakeRefCounted<DisplayItemList>();
        display_item_list->StartPaint();
        SkSamplingOptions sampling(PaintFlags::FilterQualityToSkSamplingOptions(kDefaultFilterQuality));
        display_item_list->push<DrawImageOp>(paint_image, 0.f, 0.f, sampling, paint_flags);
        display_item_list->EndPaintOfUnpaired(kRect);
        display_item_list->Finalize();
    ...
    // metadata.
    scoped_refptr<DisplayItemList> display_item_list_10k_nits = make_display_item_list(0, 10000.f);
    scoped_refptr<DisplayItemList> display_item_list_500_nits = make_display_item_list(1, 500.f);
    RasterOptions options(kSize);
    ...
    // despite the (still) increased headroom.
    {
        ...
        sdr_paint_flags.setDynamicRangeLimit(PaintFlags::DynamicRangeLimitMixture(PaintFlags::DynamicRangeLimit::kStandard));
        scoped_refptr<DisplayItemList> display_item_list_10k_nits_sdr = make_display_item_list(2, 10000.f, &sdr_paint_flags);
...
#if BUILDFLAG(IS_ANDROID)
    // Android has slight differences in color.
    auto comparator = FuzzyPixelComparator().SetErrorPixelsPercentageLimit(100.0f).SetAvgAbsErrorLimit(1.2f).SetAbsErrorLimit(2);
#else
    ExactPixelComparator comparator;
#endif

    ExpectEquals(actual, FILE_PATH_LITERAL("oop_draw_image_source_color_space.png"), comparator);
}
...
TEST_F(OopPixelTest, DrawImageWithSourceAndTargetColorSpace)
{
    ...
    ExpectEquals(actual, FILE_PATH_LITERAL("oop_draw_image_both_color_space.png"), comparator);
}
...
TEST_F(OopPixelTest, MAYBE_DrawImageReinterpretedAsSRGB)
{
    ...
    const PaintImage::Id kSomeId = 32;
    auto builder = PaintImageBuilder::WithDefault().set_image(image, 0).set_id(kSomeId).set_reinterpret_as_srgb(true);
    auto paint_image = builder.TakePaintImage();
...
TEST_F(OopPixelTest, DrawMailboxBackedImage)
{
    ...
    SkImageInfo backing_info = SkImageInfo::MakeN32Premul(options.resource_size.width(), options.resource_size.height());
    ...
    auto* ri = raster_context_provider_->RasterInterface();
    auto* sii = raster_context_provider_->SharedImageInterface();
    scoped_refptr<gpu::ClientSharedImage> src_client_si = CreateClientSharedImage(ri, sii, options, viz::SinglePlaneFormat::kRGBA_8888);

    UploadPixels(ri, src_client_si->mailbox(), expected_bitmap.info(), expected_bitmap);

    auto src_paint_image
        = PaintImageBuilder::WithDefault()
              .set_id(PaintImage::GetNextId())
              .set_texture_backing(sk_sp<TestMailboxBacking>(new TestMailboxBacking(src_client_si->mailbox(), backing_info)), PaintImage::GetNextContentId())
              .TakePaintImage();
...
TEST_P(OopClearPixelTest, ClearingOpaqueCorner)
{
    ...
    if (IsPartialRaster()) {
        options.playback_rect = gfx::Rect(
            options.full_raster_rect.x() + 1, options.full_raster_rect.y() + 1, options.full_raster_rect.width() - 1, options.full_raster_rect.height() - 1);
    } else {
    ...
    SkBitmap bitmap;
    bitmap.allocPixelsFlags(SkImageInfo::MakeN32Premul(options.resource_size.width(), options.resource_size.height()), SkBitmap::kZeroPixels_AllocFlag);
...
TEST_F(OopPixelTest, ClearingOpaqueCornerPartialRaster)
{
    ...
    options.full_raster_rect = gfx::Rect(arbitrary_offset, gfx::Size(8, 7));
    options.content_size = gfx::Size(options.full_raster_rect.right(), options.full_raster_rect.bottom());
    options.playback_rect = gfx::Rect(arbitrary_offset.x() + 5, arbitrary_offset.y() + 3, 2, 3);
    options.background_color = SkColors::kGreen;
    options.requires_clear = false;
    options.preclear = true;
    options.preclear_color = SkColors::kRed;
...
TEST_P(OopClearPixelTest, ClearingOpaqueLeftEdge)
{
    ...
    options.content_size = gfx::Size(options.full_raster_rect.right() + 1000, options.full_raster_rect.bottom() + 1000);
    if (IsPartialRaster()) {
        ...
        options.playback_rect = gfx::Rect(
            options.full_raster_rect.x(), options.full_raster_rect.y() + 1, options.full_raster_rect.width() - 1, options.full_raster_rect.height() - 2);
    } else {
        options.playback_rect = options.full_raster_rect;
    }
...
TEST_P(OopClearPixelTest, ClearingOpaqueRightEdge)
{
    ...
    options.content_size = gfx::Size(options.full_raster_rect.right(), options.full_raster_rect.bottom() + 1000);
    if (IsPartialRaster()) {
        // Ignore the left column of pixels here to force partial raster.
        // Additionally ignore the top and bottom rows of pixels to make sure
        // that things are not cleared outside the rect.
        options.playback_rect = gfx::Rect(
            options.full_raster_rect.x() + 1, options.full_raster_rect.y() + 1, options.full_raster_rect.width() - 1, options.full_raster_rect.height() - 2);
    } else {
        options.playback_rect = options.full_raster_rect;
    }
...
TEST_P(OopClearPixelTest, ClearingOpaqueTopEdge)
{
    ...
    if (IsPartialRaster()) {
        ...
        // that things are not cleared outside the rect.
        options.playback_rect = gfx::Rect(
            options.full_raster_rect.x() + 1, options.full_raster_rect.y(), options.full_raster_rect.width() - 2, options.full_raster_rect.height() - 1);
    } else {
        options.playback_rect = options.full_raster_rect;
    }
    options.background_color = SkColors::kGreen;
...
TEST_P(OopClearPixelTest, ClearingOpaqueBottomEdge)
{
    ...
    options.content_size = gfx::Size(options.full_raster_rect.right() + 1000, options.full_raster_rect.bottom());
    if (IsPartialRaster()) {
        // Ignore the top row of pixels here to force partial raster.
        // Additionally ignore the left and right columns of pixels to make sure
        // that things are not cleared outside the rect.
        options.playback_rect = gfx::Rect(
            options.full_raster_rect.x() + 1, options.full_raster_rect.y() + 1, options.full_raster_rect.width() - 2, options.full_raster_rect.height() - 1);
    } else {
        options.playback_rect = options.full_raster_rect;
    }
    options.background_color = SkColors::kGreen;
...
TEST_F(OopPixelTest, ClearingTransparentCornerPartialRaster)
{
    ...
    options.full_raster_rect = gfx::Rect(arbitrary_offset, gfx::Size(8, 7));
    options.content_size = gfx::Size(options.full_raster_rect.right(), options.full_raster_rect.bottom());
    options.playback_rect = gfx::Rect(arbitrary_offset.x() + 5, arbitrary_offset.y() + 3, 2, 4);
    options.background_color = SkColors::kTransparent;
...
TEST_F(OopPixelTest, DrawRectTransformOptionsFullRaster)
{
    ...
    auto actual = Raster(display_item_list, options);
    auto expected = MakeSolidColorBitmap(options.resource_size, SkColor4f::FromColor(SkColorSetARGB(255, 64, 128, 32)));
...
{
    auto display_item_list = base::MakeRefCounted<DisplayItemList>();
    std::vector<SkColor> colors = { SkColorSetARGB(255, 0, 0, 255), SkColorSetARGB(255, 0, 255, 0), SkColorSetARGB(255, 0, 255, 255),
        SkColorSetARGB(255, 255, 0, 0), SkColorSetARGB(255, 255, 0, 255), SkColorSetARGB(255, 255, 255, 0), SkColorSetARGB(255, 255, 255, 255) };
    ...
    auto actual = Raster(display_item_list, options);
    ExpectEquals(actual, FILE_PATH_LITERAL("oop_draw_rect_query.png"), FuzzyPixelOffByOneComparator());
}
...
TEST_F(OopPixelTest, DrawRectColorSpace)
{
    ...
    display_item_list->push<DrawRectOp>(gfx::RectToSkRect(gfx::Rect(options.resource_size)), flags);
    display_item_list->EndPaintOfUnpaired(options.full_raster_rect);
    display_item_list->Finalize();

    SkBitmap expected = MakeSolidColorBitmap(options.resource_size, SkColor4f::FromColor(SkColorSetARGB(255, 117, 251, 76)));

    auto actual = Raster(display_item_list, options);
    ExpectEquals(actual, expected);
}

sk_sp<SkTextBlob> BuildTextBlob(sk_sp<SkTypeface> typeface = skia::DefaultTypeface(), bool use_lcd_text = false)
{
    if (!typeface) {
...
using TextBlobTestConfig = ::testing::tuple<TextBlobStrategy, FilterStrategy, MatrixStrategy, LCDStrategy>;

class OopTextBlobPixelTest : public OopPixelTest, public ::testing::WithParamInterface<TextBlobTestConfig> {
public:
    void RunTest()
    {
        RasterOptions options;
        ...
        const bool save_layer = GetFilterStrategy(GetParam()) == FilterStrategy::kSaveLayer;
        ...
        if (GetMatrixStrategy(GetParam()) == MatrixStrategy::kComplex) {
            const bool is_record_filter = GetTextBlobStrategy(GetParam()) == TextBlobStrategy::kRecordFilter;
            error_pixels_percentage = std::max(is_record_filter ? 12.f : 0.2f, error_pixels_percentage);
        ...
        auto comparator
            = FuzzyPixelComparator().SetErrorPixelsPercentageLimit(error_pixels_percentage).SetAvgAbsErrorLimit(avg_error).SetAbsErrorLimit(max_abs_error);
    ...
    SkBitmap GetExpected(const gfx::Size& image_size)
    {
        ...
        gpu_service->gpu_main_thread_task_runner()->PostTask(FROM_HERE,
            base::BindOnce(&OopTextBlobPixelTest::DrawExpectedOnGpuThread, base::Unretained(this), image_size, std::ref(bitmap), std::ref(waitable)));
        waitable.Wait();
    ...
    void DrawExpectedOnGpuThread(const gfx::Size& image_size, SkBitmap& expected, base::WaitableEvent& waitable)
    {
        ...
        gpu::raster::GrShaderCache::ScopedCacheUse cache_use(gpu_service->gr_shader_cache(), gpu::kDisplayCompositorClientId);

        // Setup a GPU accelerated SkSurface to draw to.
        SkImageInfo image_info = SkImageInfo::MakeN32Premul(image_size.width(), image_size.height());
        SkSurfaceProps surface_props = UseLcdText() ? skia::LegacyDisplayGlobals::GetSkSurfaceProps(0) : SkSurfaceProps(0, kUnknown_SkPixelGeometry);
        sk_sp<SkSurface> surface = nullptr;
        if (context_state->gr_context()) {
            surface = SkSurfaces::RenderTarget(context_state->gr_context(), skgpu::Budgeted::kNo, image_info, 0, kTopLeft_GrSurfaceOrigin, &surface_props);
        } else if (context_state->graphite_context()) {
            surface = SkSurfaces::RenderTarget(context_state->gpu_main_graphite_recorder(), image_info, skgpu::Mipmapped::kNo, &surface_props);
        } else {
            NOTREACHED();
        }
        ...
        if (context_state->gr_context()) {
            ...
            success = gpu::GraphiteReadPixelsSync(context_state->graphite_context(), context_state->gpu_main_graphite_recorder(), surface.get(), image_info,
                expected.pixmap().writable_addr(), expected.pixmap().rowBytes(), 0, 0);
        }
    ...
    void DrawExpectedToCanvas(SkCanvas& canvas)
    {
        ...
        if (GetFilterStrategy(config) != FilterStrategy::kNone) {
            filter = SkImageFilters::Blur(.1f, .1f, SkTileMode::kDecal, nullptr, nullptr);
        }

        const bool save_layer = GetFilterStrategy(config) == FilterStrategy::kSaveLayer;
        ...
        if (filter && (strategy == TextBlobStrategy::kDirect || strategy == TextBlobStrategy::kDrawRecord)) {
        ...
        if (strategy == TextBlobStrategy::kDirect) {
            ...
            record_canvas->drawTextBlob(std::move(text_blob), 0, kTextBlobY, text_paint);
            ...
            if (strategy == TextBlobStrategy::kDrawRecord) {
                ...
                auto draw_as_shader = recording->makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkFilterMode::kLinear, nullptr, &shader_rect);
                ...
                sk_sp<SkImageFilter> draw_as_filter = SkImageFilters::Picture(std::move(recording));
                if (filter) {
                    draw_as_filter = SkImageFilters::Compose(std::move(filter), std::move(draw_as_filter));
                    filter = nullptr;
                }
    ...
    void PushDrawOp(scoped_refptr<DisplayItemList> display_list, sk_sp<PaintFilter> filter)
    {
        ...
        if (strategy == TextBlobStrategy::kDirect) {
            display_list->push<DrawTextBlobOp>(std::move(text_blob), 0.0f, kTextBlobY, text_flags);
            return;
        }

        // All remaining strategies add the DrawTextBlobOp to an inner paint record.
        PaintOpBuffer paint_buffer;
        paint_buffer.push<DrawTextBlobOp>(std::move(text_blob), 0.0f, kTextBlobY, text_flags);
        if (strategy == TextBlobStrategy::kDrawRecord) {
        ...
        if (strategy == TextBlobStrategy::kRecordShader) {
            auto paint_record_shader = PaintShader::MakePaintRecord(paint_buffer.ReleaseAsRecord(), SkRect::MakeWH(25, 25), SkTileMode::kRepeat,
                SkTileMode::kRepeat, nullptr, PaintShader::ScalingBehavior::kRasterAtScale);
            ...
            sk_sp<PaintFilter> paint_record_filter = sk_make_sp<RecordPaintFilter>(paint_buffer.ReleaseAsRecord(), SkRect::MakeWH(100, 100));
            // If there's an additional filter, we have to compose it with the
            // paint record filter.
            if (filter) {
                paint_record_filter = sk_make_sp<ComposePaintFilter>(std::move(filter), std::move(paint_record_filter));
            }
    ...
    static TextBlobStrategy GetTextBlobStrategy(const TextBlobTestConfig& config)
    {
    ...
    static std::string PrintTestName(const ::testing::TestParamInfo<TextBlobTestConfig>& info)
    {
        std::stringstream ss;
...
INSTANTIATE_TEST_SUITE_P(P, OopTextBlobPixelTest,
    ::testing::Combine(
        ::testing::Values(TextBlobStrategy::kDirect, TextBlobStrategy::kDrawRecord, TextBlobStrategy::kRecordShader, TextBlobStrategy::kRecordFilter),
        ::testing::Values(FilterStrategy::kNone, FilterStrategy::kPaintFlags, FilterStrategy::kSaveLayer),
        ::testing::Values(MatrixStrategy::kIdentity, MatrixStrategy::kScaled, MatrixStrategy::kComplex, MatrixStrategy::kPerspective),
...
TEST_F(OopPixelTest, DrawTextMultipleRasterCHROMIUM)
{
    ...
    flags.setStyle(PaintFlags::kFill_Style);
    flags.setColor(SkColors::kGreen);
    display_item_list->push<DrawTextBlobOp>(BuildTextBlob(sk_typeface_1), 0.0f, kTextBlobY, flags);
    display_item_list->EndPaintOfUnpaired(options.full_raster_rect);
    display_item_list->Finalize();
    ...
    display_item_list_2->push<DrawTextBlobOp>(BuildTextBlob(sk_typeface_2), 0.0f, kTextBlobY, flags);
    ...
    raster_context_provider_->ExecuteOnGpuThread(base::BindOnce(&ClearFontCache, &event));
    event.Wait();
...
TEST_F(OopPixelTest, DrawTextBlobPersistentShaderCache)
{
    ...
    flags.setStyle(PaintFlags::kFill_Style);
    flags.setColor(SkColors::kGreen);
    display_item_list->push<DrawTextBlobOp>(BuildTextBlob(), 0.0f, kTextBlobY, flags);
    display_item_list->EndPaintOfUnpaired(options.full_raster_rect);
    display_item_list->Finalize();
    ...
    // bitmap.
    SkBitmap expected = MakeSolidColorBitmap(options.resource_size, SkColors::kBlack);
    ...
    // GPU canvas.
    auto comparator = FuzzyPixelComparator().SetErrorPixelsPercentageLimit(1.0f).SetAbsErrorLimit(1);
...
TEST_F(OopPixelTest, WritePixels)
{
    ...
    auto* ri = raster_context_provider_->RasterInterface();
    auto* sii = raster_context_provider_->SharedImageInterface();
    auto dest_client_si = CreateClientSharedImage(ri, sii, options, viz::SinglePlaneFormat::kRGBA_8888);
    std::vector<SkPMColor> expected_pixels(dest_size.width() * dest_size.height(), SkPreMultiplyARGB(255, 0, 0, 255));
    SkBitmap expected;
    expected.installPixels(SkImageInfo::MakeN32Premul(dest_size.width(), dest_size.height()), expected_pixels.data(), dest_size.width() * sizeof(SkColor));

    UploadPixels(ri, dest_client_si->mailbox(), expected.info(), expected);

    SkBitmap actual = ReadbackMailbox(ri, dest_client_si->mailbox(), options.resource_size);
    gpu::SyncToken sync_token;
    ri->GenUnverifiedSyncTokenCHROMIUM(sync_token.GetData());
...
TEST_F(OopPixelTest, CopySharedImage)
{
    ...
    const gfx::ColorSpace dest_color_space = gfx::ColorSpace::CreateDisplayP3D65();

    // Create data to upload in sRGB (solid green).
    SkBitmap upload_bitmap;
    {
        upload_bitmap.allocPixels(SkImageInfo::MakeN32Premul(size.width(), size.height(), source_color_space.ToSkColorSpace()));
    ...
    scoped_refptr<gpu::ClientSharedImage> source_client_si;
    {
        ...
        source_client_si = CreateClientSharedImage(ri, sii, options, viz::SinglePlaneFormat::kRGBA_8888);
        ri->WaitSyncTokenCHROMIUM(sii->GenUnverifiedSyncToken().GetConstData());
    ...
    scoped_refptr<gpu::ClientSharedImage> dest_client_si;
    {
        ...
        dest_client_si = CreateClientSharedImage(ri, sii, options, viz::SinglePlaneFormat::kRGBA_8888);
        ri->WaitSyncTokenCHROMIUM(sii->GenUnverifiedSyncToken().GetConstData());

        ri->CopySharedImage(source_client_si->mailbox(), dest_client_si->mailbox(), 0, 0, 0, 0, size.width(), size.height());
    }

    // Read the data back as DisplayP3, from the Display P3 SharedImage.
    SkBitmap readback_bitmap;
    {
        readback_bitmap.allocPixels(SkImageInfo::MakeN32Premul(size.width(), size.height(), dest_color_space.ToSkColorSpace()));

        ri->ReadbackImagePixels(dest_client_si->mailbox(), readback_bitmap.info(), readback_bitmap.rowBytes(), 0, 0,
...
class OopYUVToRGBPixelTest : public OopPixelTest, public ::testing::WithParamInterface<gfx::ColorSpace> {
public:
...
TEST_P(OopYUVToRGBPixelTest, CopyI420SharedImage)
{
    ...
    RasterOptions options(gfx::Size(16, 16));
    RasterOptions uv_options(gfx::Size(options.resource_size.width() / 2, options.resource_size.height() / 2));
    auto* ri = raster_context_provider_->RasterInterface();
    auto* sii = raster_context_provider_->SharedImageInterface();

    auto dest_client_si = CreateClientSharedImage(ri, sii, options, viz::SinglePlaneFormat::kRGBA_8888, dest_color_space);

    scoped_refptr<gpu::ClientSharedImage> yuv_client_si = CreateClientSharedImage(ri, sii, options, viz::MultiPlaneFormat::kI420);

    SkPixmap pixmaps[SkYUVAInfo::kMaxPlanes] = {};

    SkImageInfo y_info = SkImageInfo::Make(options.resource_size.width(), options.resource_size.height(), kGray_8_SkColorType, kPremul_SkAlphaType,
        options.target_color_params.color_space.ToSkColorSpace());

    SkImageInfo uv_info = SkImageInfo::Make(uv_options.resource_size.width(), uv_options.resource_size.height(), kGray_8_SkColorType, kPremul_SkAlphaType,
    ...
    SkYUVAInfo yuv_info = SkYUVAInfo(SizeToSkISize(options.resource_size), SkYUVAInfo::PlaneConfig::kY_U_V, SkYUVAInfo::Subsampling::k420, yuv_color_space);
    SkYUVAPixmaps yuv_pixmap = SkYUVAPixmaps::FromExternalPixmaps(yuv_info, pixmaps);

    // Upload initial Y+U+V planes and convert to RGB.
    UploadPixelsYUV(ri, yuv_client_si->mailbox(), yuv_pixmap);

    ri->CopySharedImage(yuv_client_si->mailbox(), dest_client_si->mailbox(), 0, 0, 0, 0, options.resource_size.width(), options.resource_size.height());

    SkBitmap actual_bitmap = ReadbackMailbox(ri, dest_client_si->mailbox(), options.resource_size, dest_color_space.ToSkColorSpace());

    SkColor expected_color = SkColorSetARGB(255, 0, 0, 254);
    SkBitmap expected_bitmap = MakeSolidColorBitmap(options.resource_size, SkColor4f::FromColor(expected_color));

    // Allow slight rounding error on all pixels.
    auto comparator = FuzzyPixelComparator().SetErrorPixelsPercentageLimit(100.0f).SetAbsErrorLimit(2);
...
INSTANTIATE_TEST_SUITE_P(
    P, OopYUVToRGBPixelTest, ::testing::Values(gfx::ColorSpace(gfx::ColorSpace::PrimaryID::BT2020, gfx::ColorSpace::TransferID::SRGB), gfx::ColorSpace()));
...
TEST_F(OopPixelTest, CopyNV12SharedImage)
{
    ...
    scoped_refptr<gpu::ClientSharedImage> dest_client_si = CreateClientSharedImage(ri, sii, options, viz::SinglePlaneFormat::kRGBA_8888);
    scoped_refptr<gpu::ClientSharedImage> y_uv_client_si
        = CreateClientSharedImage(ri, sii, options, viz::MultiPlaneFormat::kNV12, gfx::ColorSpace::CreateJpeg());
    ...
    SkImageInfo uv_info = SkImageInfo::Make(uv_options.resource_size.width(), uv_options.resource_size.height(), kR8G8_unorm_SkColorType, kPremul_SkAlphaType,
        uv_options.target_color_params.color_space.ToSkColorSpace());
    ...
    SkYUVAInfo yuv_info = SkYUVAInfo(SizeToSkISize(options.resource_size), SkYUVAInfo::PlaneConfig::kY_UV, SkYUVAInfo::Subsampling::k420, yuv_color_space);
    SkYUVAPixmaps yuv_pixmap = SkYUVAPixmaps::FromExternalPixmaps(yuv_info, pixmaps);

    // Upload initial Y+UV planes and convert to RGB.
    UploadPixelsYUV(ri, y_uv_client_si->mailbox(), yuv_pixmap);

    ri->CopySharedImage(y_uv_client_si->mailbox(), dest_client_si->mailbox(), 0, 0, 0, 0, options.resource_size.width(), options.resource_size.height());

    SkBitmap actual_bitmap = ReadbackMailbox(ri, dest_client_si->mailbox(), options.resource_size);

    SkBitmap expected_bitmap = MakeSolidColorBitmap(options.resource_size, SkColor4f::FromColor(SkColorSetARGB(255, 2, 0, 254)));
...
class OopPathPixelTest : public OopPixelTest, public ::testing::WithParamInterface<bool> {
public:
    ...
    void RunTest()
    {
        auto* ri = static_cast<gpu::raster::RasterImplementation*>(raster_context_provider_->RasterInterface());
        uint32_t max_inlined_entry_size = AllowInlining() ? std::numeric_limits<uint32_t>::max() : 0u;
...
{
    const int max_texture_size = raster_context_provider_->ContextCapabilities().max_texture_size;
    ...
    auto shader = PaintShader::MakePaintRecord(shader_buffer.ReleaseAsRecord(), rect, SkTileMode::kRepeat, SkTileMode::kRepeat, nullptr);
    ...
    auto actual = Raster(display_item_list, options);
    ExpectEquals(actual, FILE_PATH_LITERAL("oop_record_shader_max_texture_size.png"));
}
