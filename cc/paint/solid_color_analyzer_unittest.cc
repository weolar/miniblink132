...
protected:
    ...
    void Reset()
    {
        canvas_.ReleaseAsRecord();
    }

    bool IsSolidColor(int max_ops_to_analyze = 1, const gfx::Rect& rect = gfx::Rect(0, 0, 100, 100))
    {
        auto color = SolidColorAnalyzer::DetermineIfSolidColor(canvas_.ReleaseAsRecord().buffer(), rect, max_ops_to_analyze, nullptr);
        return !!color;
    }

    SkColor4f GetColor(int max_ops_to_analyze = 1, const gfx::Rect rect = gfx::Rect(0, 0, 100, 100))
    {
        auto color = SolidColorAnalyzer::DetermineIfSolidColor(canvas_.ReleaseAsRecord().buffer(), rect, max_ops_to_analyze, nullptr);
        EXPECT_TRUE(color);
...
TEST_F(SolidColorAnalyzerTest, DrawRectSolidWithSrcOverBlending)
{
    ...
    EXPECT_EQ(SkColorSetARGB(160, 16, 26, 36), GetColor(2 /* max_ops_to_analyze */).toSkColor());
#endif // BUILDFLAG(IS_MAC)
}
...
TEST_F(SolidColorAnalyzerTest, ClipRRectCoversCanvas)
{
    ...
    SkVector radii_scale[4] = {
        SkVector::Make(100.0, 150.0),
        SkVector::Make(200.0, 250.0),
        SkVector::Make(300.0, 350.0),
        SkVector::Make(400.0, 450.0),
    };
    ...
    struct {
        ...
        { SkVector::Make(canvas_size - rr_size, 0), SkVector::Make(canvas_size - rr_size, 0), false },

        // Between UR and LR.
        { SkVector::Make(canvas_size - rr_size, -50), SkVector::Make(-305, -80), true },

        // Intersects LR corner.
        { SkVector::Make(canvas_size - rr_size, canvas_size - rr_size), SkVector::Make(canvas_size - rr_size, canvas_size - rr_size), false },

        // Between LL and LR
        { SkVector::Make(-50, canvas_size - rr_size), SkVector::Make(-205, -310), true },

        // Intersects LL corner
        { SkVector::Make(0, canvas_size - rr_size), SkVector::Make(0, canvas_size - rr_size), false },
    ...
    for (int case_scale = 0; case_scale < 2; ++case_scale) {
        ...
        for (size_t i = 0; i < std::size(cases); ++i) {
            ...
            SkRect bounding_rect = SkRect::MakeXYWH(
                scaled ? cases[i].offset_scale.x() : cases[i].offset.x(), scaled ? cases[i].offset_scale.y() : cases[i].offset.y(), rr_size, rr_size);
            ...
            EXPECT_EQ(cases[i].expected, IsSolidColor(1, canvas_rect)) << "Case " << i << ", " << scaled << " failed.";
        }
    }
}
