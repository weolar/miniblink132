...
TEST(PaintOpWriterReaderTest, SizeT)
{
    ...
    PaintOpWriter writer(buffer, std::size(buffer), options_provider.serialize_options(),
    ...
    PaintOpReader reader(buffer, writer.size(), options_provider.deserialize_options(),
        /*enable_security_constraints*/ true);
    int read_i;
