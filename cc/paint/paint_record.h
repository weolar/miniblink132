...
public:
    ...
    {
        return buffer_->has_non_aa_paint();
    }
    ...
    {
        return buffer_->has_draw_text_ops();
    }
    bool has_save_layer_ops() const
    {
        return buffer_->has_save_layer_ops();
    }
    ...
    static SkRect GetFixedScaleBounds(const SkMatrix& ctm, const SkRect& bounds, int max_texture_size);

    sk_sp<SkPicture> ToSkPicture(const SkRect& bounds, ImageProvider* image_provider = nullptr, const PlaybackCallbacks& callbacks = PlaybackCallbacks()) const;

    // Replays the paint record into the canvas.
    void Playback(SkCanvas* canvas) const
    {
        buffer_->Playback(canvas);
    }
