...
// Skottie.
base::flat_map</*asset_id*/ std::string, gfx::Size> ParseImageAssetDimensions(std::string_view animation_json)
{
    base::flat_map<std::string, gfx::Size> image_asset_sizes;

    std::optional<base::Value> animation_dict = base::JSONReader::Read(animation_json);
    if (!animation_dict || !animation_dict->is_dict()) {
        LOG(ERROR) << "Failed to parse Lottie animation json";
        return image_asset_sizes;
    }

    const base::Value::List* assets = animation_dict->GetDict().FindList(kAssetsKey);
    ...
    for (const base::Value& asset : *assets) {
        if (!asset.is_dict()) {
            LOG(ERROR) << "Found invalid asset in animation with type " << base::Value::GetTypeName(asset.type());
            continue;
        }
        ...
        if (id && width && height && *width > 0 && *height > 0 && !image_asset_sizes.emplace(*id, gfx::Size(*width, *height)).second) {
...
public:
    ...
    ImageAssetImpl(SkottieResourceIdHash asset_id, FrameDataCallback frame_data_cb)
        : asset_id_(asset_id)
    ...
    FrameData getFrameData(float t) override
    {
        ...
        SkottieWrapper::FrameDataFetchResult result = frame_data_cb_.Run(asset_id_, t, new_frame_data.image, new_frame_data.sampling);
        switch (result) {
...
SkottieMRUResourceProvider::SkottieMRUResourceProvider(FrameDataCallback frame_data_cb, std::string_view animation_json)
    : frame_data_cb_(std::move(frame_data_cb))
...
const SkottieResourceMetadataMap& SkottieMRUResourceProvider::GetImageAssetMetadata() const
{
    ...
    const char resource_path[], const char resource_name[], const char resource_id[]) const
{
    ...
    if (!image_asset_metadata_.RegisterAsset(resource_path, resource_name, resource_id, std::move(size))) {
        return nullptr;
    }
    return sk_make_sp<ImageAssetImpl>(HashSkottieResourceId(resource_id), frame_data_cb_);
}
