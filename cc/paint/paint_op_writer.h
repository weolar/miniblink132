...
public:
    ...
    PaintOpWriter(void* memory, size_t size, const PaintOp::SerializeOptions& options, bool enable_security_constraints = false)
    ...
    template <typename T = char> static std::unique_ptr<T, base::AlignedFreeDeleter> AllocateAlignedBuffer(size_t size)
    {
        return std::unique_ptr<T, base::AlignedFreeDeleter>(static_cast<T*>(base::AlignedAlloc(size, kMaxAlignment)));
    }
...
private:
    template <typename T> static constexpr size_t SerializedSizeSimple();
...
public:
    ...
    template <typename T> static constexpr size_t SerializedSize();
    template <typename T> static constexpr size_t SerializedSize(const T& data);
    ...
    template <typename T> static inline size_t SerializedSize(const T* p);
    template <typename T> static inline size_t SerializedSize(const std::unique_ptr<T>& p);
    template <typename T> static inline size_t SerializedSize(const scoped_refptr<T>& p);
    template <typename T> static inline size_t SerializedSize(const raw_ptr<T>& p);

    template <typename T> static inline size_t SerializedSize(T* p)
    {
    ...
    template <typename T> static size_t SerializedSize(const std::optional<T>& o)
    {
        if (o) {
            return (base::CheckedNumeric<size_t>(SerializedSize<bool>()) + SerializedSize<T>(*o)).ValueOrDie();
        }
    ...
    {
        return (base::CheckedNumeric<size_t>(SerializedSize<size_t>()) + base::bits::AlignUp(num_bytes, kDefaultAlignment)).ValueOrDie();
    }
    // Size of serialized (size_t, elements>
    template <typename T> static size_t SerializedSizeOfElements(const T* elements, size_t count)
    {
        return (SerializedSize<size_t>() + base::CheckedNumeric<size_t>(count) * SerializedSize(*elements)).ValueOrDie();
    }
    ...
    static void WriteHeaderForTesting(void* memory, uint8_t type, size_t serialized_size);
    ...
    size_t size() const
    {
        return valid_ ? size_ - remaining_bytes() : 0u;
    }
    ...
    void Write(const PaintShader* shader, PaintFlags::FilterQuality quality, const SkM44& current_ctm);
    ...
    {
        WriteEnum(constraint);
    }
    void Write(SkColorType color_type)
    {
        WriteEnum(color_type);
    }
    ...
    {
        WriteEnum(blend_mode);
    }
    void Write(SkTileMode tile_mode)
    {
        WriteEnum(tile_mode);
    }
    void Write(SkFilterMode filter_mode)
    {
        WriteEnum(filter_mode);
    }
    void Write(SkMipmapMode mipmap_mode)
    {
        WriteEnum(mipmap_mode);
    }
    ...
    static void AssertAlignment(const volatile uint8_t* memory, size_t alignment)
    {
#if DCHECK_IS_ON()
    ...
    template <typename T> void Write(const sk_sp<T>&)
    {
        ...
        static_assert(sizeof(T) == 0, "Attempted to call a non-existent sk_sp override.");
    }
    template <typename T> void Write(const T*)
    {
        static_assert(sizeof(T) == 0, "Attempted to call a non-existent T* override.");
    }
    ...
    // yourself.
    template <typename... Types> ALWAYS_INLINE void WriteSimpleMultiple(Types... vals)
    {
        AssertFieldAlignment();
        static constexpr size_t total_size = (base::bits::AlignUp(sizeof(vals), kDefaultAlignment) + ...);
...
private:
    template <typename T> void WriteSimple(const T& val)
    {
        static_assert(std::is_trivially_copyable_v<T>);

        AssertFieldAlignment();
        static constexpr size_t size = base::bits::AlignUp(sizeof(T), kDefaultAlignment);
        EnsureBytes(size);
        if (!valid_) {
            return;
        }
    ...
    template <typename Enum> void WriteEnum(Enum value)
    {
    ...
    void Write(const MatrixConvolutionPaintFilter& filter, const SkM44& current_ctm);
    void Write(const DisplacementMapEffectPaintFilter& filter, const SkM44& current_ctm);
    ...
    void Write(const LightingDistantPaintFilter& filter, const SkM44& current_ctm);
    ...
    void Write(const PaintRecord& record, const gfx::Rect& playback_rect, const gfx::SizeF& post_scale);
    void Write(const SkRegion& region);
    void WriteImage(const DecodedDrawImage& decoded_draw_image, bool reinterpret_as_srgb);
    ...
    sk_sp<PaintShader> TransformShaderIfNecessary(const PaintShader* original, PaintFlags::FilterQuality quality, const SkM44& current_ctm,
...
template <typename T> constexpr size_t PaintOpWriter::SerializedSizeSimple()
{
...
template <> constexpr size_t PaintOpWriter::SerializedSizeSimple<size_t>()
{
    return base::bits::AlignUp(2 * sizeof(uint32_t), kDefaultAlignment);
}

template <typename T> constexpr size_t PaintOpWriter::SerializedSize()
{
    static_assert(std::is_arithmetic_v<T> || std::is_enum_v<T>);
    return SerializedSizeSimple<T>();
}
template <typename T> constexpr size_t PaintOpWriter::SerializedSize(const T& data)
{
    return SerializedSizeSimple<T>();
}
