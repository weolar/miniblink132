...
bool CalculateClearForOpaqueRasterRects(const gfx::Vector2dF& translation, const gfx::Vector2dF& scale, const gfx::Size& content_size,
    const gfx::Rect& canvas_bitmap_rect, const gfx::Rect& canvas_playback_rect, gfx::Rect& outer_rect, gfx::Rect& inner_rect)
{
    ...
    outer_rect.Inset(gfx::Insets::TLBR(0, 0, bottom_opaque ? 0 : -1, right_opaque ? 0 : -1));
    inner_rect.Inset(gfx::Insets::TLBR(top_opaque ? 0 : 1, left_opaque ? 0 : 1, bottom_opaque ? 0 : 1, right_opaque ? 0 : 1));
    ...
    bool touches_right_edge = !right_opaque && content_size.width() == canvas_playback_rect.right();
    bool touches_bottom_edge = !bottom_opaque && content_size.height() == canvas_playback_rect.bottom();
    gfx::Rect adjusted_playback_rect = canvas_playback_rect;
    adjusted_playback_rect.Inset(
        gfx::Insets::TLBR(touches_top_edge ? -1 : 0, touches_left_edge ? -1 : 0, touches_bottom_edge ? -1 : 0, touches_right_edge ? -1 : 0));
