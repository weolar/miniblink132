...
size_t PaintOpWriter::SerializedSize(const PaintImage& image)
{
    ...
    base::CheckedNumeric<size_t> image_size = SerializedSize<PaintOp::SerializedImageType>();
    if (image) {
        auto info = SkImageInfo::Make(image.width(), image.height(), kN32_SkColorType, kPremul_SkAlphaType);
...
{
    return SerializedSizeOfBytes(color_space ? color_space->writeToMemory(nullptr) : 0u);
}

// static
size_t PaintOpWriter::SerializedSize(const gfx::HDRMetadata& hdr_metadata)
{
    return SerializedSizeOfBytes(gfx::mojom::HDRMetadata::Serialize(&hdr_metadata).size());
}
...
{
    return SerializedSize(config.fGrayscale) + SerializedSize(config.fInvertStyle) + SerializedSize(config.fContrast);
}
...
size_t PaintOpWriter::SerializedSize(const DrawLooper* looper)
{
    ...
    base::CheckedNumeric<size_t> size = SerializedSizeSimple<bool>() + SerializedSize(count);
    if (count > 0) {
        const DrawLooper::Layer& layer = looper->layers_.front();
        base::CheckedNumeric<size_t> layer_size
            = SerializedSize(layer.offset) + SerializedSize(layer.blur_sigma) + SerializedSize(layer.color) + SerializedSize(layer.flags);
...
void PaintOpWriter::WriteHeaderForTesting(void* memory, uint8_t type, size_t serialized_size)
{
...
void PaintOpWriter::Write(const SkPath& path, UsePaintCache use_paint_cache)
{
    ...
    if (use_paint_cache == UsePaintCache::kEnabled && options_.paint_cache->Get(PaintCacheDataType::kPath, id)) {
...
{
    if (flags.path_effect_ == nullptr && flags.color_filter_ == nullptr && flags.draw_looper_ == nullptr && flags.image_filter_ == nullptr
        ...
        WriteSimpleMultiple(flags.color_, flags.width_, flags.miter_limit_, flags.bitfields_uint_,
            // flags.path_effect_.
...
void PaintOpWriter::Write(const DrawImage& draw_image, SkSize* scale_adjustment)
{
    ...
    DCHECK(!decoded_image.decoded_image().image()) << "Use transfer cache for image serialization";
    const DecodedDrawImage& decoded_draw_image = decoded_image.decoded_image();
    DCHECK(decoded_draw_image.src_rect_offset().isEmpty()) << "We shouldn't ask for image subsets";
...
void PaintOpWriter::Write(scoped_refptr<SkottieWrapper> skottie)
{
    ...
    bool locked = options_.transfer_cache->LockEntry(TransferCacheEntryType::kSkottie, id);

    // Add a cache entry for the skottie animation.
    size_t bytes_written = 0u;
    if (!locked) {
        bytes_written = options_.transfer_cache->CreateEntry(ClientSkottieTransferCacheEntry(skottie), memory_);
...
void PaintOpWriter::WriteImage(const DecodedDrawImage& decoded_draw_image, bool reinterpret_as_srgb)
{
    ...
    WriteImage(id.value_or(kInvalidImageTransferCacheEntryId), decoded_draw_image.transfer_cache_entry_needs_mips());
}

void PaintOpWriter::WriteImage(uint32_t transfer_cache_entry_id, bool needs_mips)
{
    ...
    Write(static_cast<uint8_t>(PaintOp::SerializedImageType::kTransferCacheEntry));
    Write(transfer_cache_entry_id);
    Write(needs_mips);
}

void PaintOpWriter::WriteImage(const gpu::Mailbox& mailbox, bool reinterpret_as_srgb)
{
...
void PaintOpWriter::Write(const SkGradientShader::Interpolation& interpolation)
{
...
{
    std::vector<uint8_t> bytes = gfx::mojom::HDRMetadata::Serialize(&hdr_metadata);
...
void PaintOpWriter::Write(const sk_sp<sktext::gpu::Slug>& slug)
{
    ...
    if (slug) {
        ...
        // slug?
        bytes_written = slug->serialize(memory_, base::bits::AlignDown(remaining_bytes(), kDefaultAlignment));
        if (bytes_written == 0u) {
            valid_ = false;
            return;
        }
...
sk_sp<PaintShader> PaintOpWriter::TransformShaderIfNecessary(const PaintShader* original, PaintFlags::FilterQuality quality, const SkM44& current_ctm,
    uint32_t* paint_image_transfer_cache_entry_id, gfx::SizeF* paint_record_post_scale, bool* paint_image_needs_mips, gpu::Mailbox* mailbox_out)
{
    ...
    if (type == PaintShader::Type::kImage) {
        if (!original->paint_image().IsPaintWorklet()) {
            return original->CreateDecodedImage(
                ctm, quality, options_.image_provider, paint_image_transfer_cache_entry_id, &quality, paint_image_needs_mips, mailbox_out);
        }
        sk_sp<PaintShader> record_shader = original->CreatePaintWorkletRecord(options_.image_provider);
        if (!record_shader) {
            return nullptr;
        }
        return record_shader->CreateScaledPaintRecord(ctm, options_.max_texture_size, paint_record_post_scale);
    }

    if (type == PaintShader::Type::kPaintRecord) {
        return original->CreateScaledPaintRecord(ctm, options_.max_texture_size, paint_record_post_scale);
    }
...
void PaintOpWriter::Write(const PaintShader* shader, PaintFlags::FilterQuality quality, const SkM44& current_ctm)
{
    ...
    if (!enable_security_constraints_ && shader) {
        transformed_shader = TransformShaderIfNecessary(
            shader, quality, current_ctm, &paint_image_transfer_cache_id, &paint_record_post_scale, &paint_image_needs_mips, &mailbox);
    ...
    if (enable_security_constraints_) {
        DrawImage draw_image(shader->image_, false, MakeSrcRect(shader->image_), quality, SkM44());
    ...
    if (shader->record_) {
        ...
        const gfx::Rect playback_rect(gfx::ToEnclosingRect(gfx::SkRectToRectF(shader->tile())));
    ...
    WriteData(shader->colors_.size() * (shader->colors_.size() > 0 ? sizeof(shader->colors_[0]) : 0u), shader->colors_.data());

    WriteSize(shader->positions_.size());
    WriteData(shader->positions_.size() * sizeof(SkScalar), shader->positions_.data());
...
void PaintOpWriter::Write(const PaintFilter* filter, const SkM44& current_ctm)
{
    ...
    case PaintFilter::Type::kAlphaThreshold:
        Write(static_cast<const AlphaThresholdPaintFilter&>(*filter), current_ctm);
        break;
    ...
    case PaintFilter::Type::kMatrixConvolution:
        Write(static_cast<const MatrixConvolutionPaintFilter&>(*filter), current_ctm);
        break;
    case PaintFilter::Type::kDisplacementMapEffect:
        Write(static_cast<const DisplacementMapEffectPaintFilter&>(*filter), current_ctm);
        break;
    ...
    case PaintFilter::Type::kLightingDistant:
        Write(static_cast<const LightingDistantPaintFilter&>(*filter), current_ctm);
        break;
...
void PaintOpWriter::Write(const ColorFilterPaintFilter& filter, const SkM44& current_ctm)
{
    Write(filter.color_filter().get());
    Write(filter.input().get(), current_ctm);
}

void PaintOpWriter::Write(const BlurPaintFilter& filter, const SkM44& current_ctm)
{
...
void PaintOpWriter::Write(const DropShadowPaintFilter& filter, const SkM44& current_ctm)
{
...
void PaintOpWriter::Write(const MagnifierPaintFilter& filter, const SkM44& current_ctm)
{
...
void PaintOpWriter::Write(const ComposePaintFilter& filter, const SkM44& current_ctm)
{
    Write(filter.outer().get(), current_ctm);
    Write(filter.inner().get(), current_ctm);
}

void PaintOpWriter::Write(const AlphaThresholdPaintFilter& filter, const SkM44& current_ctm)
{
    Write(filter.region());
    Write(filter.input().get(), current_ctm);
}

void PaintOpWriter::Write(const XfermodePaintFilter& filter, const SkM44& current_ctm)
{
...
void PaintOpWriter::Write(const ArithmeticPaintFilter& filter, const SkM44& current_ctm)
{
...
void PaintOpWriter::Write(const MatrixConvolutionPaintFilter& filter, const SkM44& current_ctm)
{
...
void PaintOpWriter::Write(const DisplacementMapEffectPaintFilter& filter, const SkM44& current_ctm)
{
...
void PaintOpWriter::Write(const ImagePaintFilter& filter, const SkM44& current_ctm)
{
    DrawImage draw_image(filter.image(), false, SkIRect::MakeWH(filter.image().width(), filter.image().height()), filter.filter_quality(), SkM44());
    SkSize scale_adjustment = SkSize::Make(1.f, 1.f);
    Write(draw_image, &scale_adjustment);
    DCHECK_EQ(scale_adjustment.width(), 1.f);
    DCHECK_EQ(scale_adjustment.height(), 1.f);
...
void PaintOpWriter::Write(const RecordPaintFilter& filter, const SkM44& current_ctm)
{
    ...
    // from the cache).
    auto scaled_filter = filter.CreateScaledPaintRecord(current_ctm.asM33(), options_.max_texture_size);
    if (!scaled_filter) {
        WriteSimple(false);
        return;
    }
...
void PaintOpWriter::Write(const MergePaintFilter& filter, const SkM44& current_ctm)
{
...
void PaintOpWriter::Write(const MorphologyPaintFilter& filter, const SkM44& current_ctm)
{
...
void PaintOpWriter::Write(const OffsetPaintFilter& filter, const SkM44& current_ctm)
{
    WriteSimple(filter.dx());
    WriteSimple(filter.dy());
    Write(filter.input().get(), current_ctm);
...
void PaintOpWriter::Write(const TilePaintFilter& filter, const SkM44& current_ctm)
{
...
void PaintOpWriter::Write(const TurbulencePaintFilter& filter, const SkM44& current_ctm)
{
...
void PaintOpWriter::Write(const ShaderPaintFilter& filter, const SkM44& current_ctm)
{
...
void PaintOpWriter::Write(const MatrixPaintFilter& filter, const SkM44& current_ctm)
{
    Write(filter.matrix());
    Write(filter.filter_quality());
    Write(filter.input().get(), current_ctm);
...
void PaintOpWriter::Write(const LightingDistantPaintFilter& filter, const SkM44& current_ctm)
{
...
void PaintOpWriter::Write(const LightingPointPaintFilter& filter, const SkM44& current_ctm)
{
    WriteEnum(filter.lighting_type());
...
void PaintOpWriter::Write(const LightingSpotPaintFilter& filter, const SkM44& current_ctm)
{
    WriteEnum(filter.lighting_type());
    WriteSimple(filter.location());
...
void PaintOpWriter::Write(const PaintRecord& record, const gfx::Rect& playback_rect, const gfx::SizeF& post_scale)
{
    ...
    SimpleBufferSerializer serializer(memory_, remaining_bytes(), lcd_disabled_options);
    ...
    static_assert(sizeof(serializer.written()) == sizeof(size_t), "written() return type size is different from sizeof(size_t)");
