...
class CC_PAINT_EXPORT DisplayItemList : public base::RefCountedThreadSafe<DisplayItemList> {
public:
    DisplayItemList();
    DisplayItemList(const DisplayItemList&) = delete;
    DisplayItemList& operator=(const DisplayItemList&) = delete;

    void Raster(SkCanvas* canvas, ImageProvider* image_provider = nullptr, const ScrollOffsetMap* raster_inducing_scroll_offsets = nullptr) const;
    ...
    // |NodeId|s in |content|.
    void CaptureContent(const gfx::Rect& rect, std::vector<NodeInfo>* content) const;
    ...
    void PushDrawScrollingContentsOp(ElementId scroll_element_id, scoped_refptr<DisplayItemList> display_item_list, const gfx::Rect& visual_rect);
    ...
    const PaintOpBuffer& paint_op_buffer() const
    {
    ...
    // intersect `query`.
    void SearchOpsByRect(const gfx::Rect& query, std::vector<size_t>* op_indices) const
    {
    ...
    // scaling the layer.
    std::optional<DirectlyCompositedImageInfo> GetDirectlyCompositedImageInfo() const;
    ...
    {
        return paint_op_buffer_.has_non_aa_paint();
    }

    // This gives the total number of PaintOps.
    size_t TotalOpCount() const
    {
        return paint_op_buffer_.total_op_count();
    }
    ...
    {
        return paint_op_buffer_.paint_ops_size();
    }
    ...
    bool GetColorIfSolidInRect(const gfx::Rect& rect, SkColor4f* color, int max_ops_to_analyze = 1) const;

    std::string ToString() const;

    bool has_draw_ops() const
    {
        return paint_op_buffer_.has_draw_ops();
    }
    ...
    {
        return paint_op_buffer_.has_effects_preventing_lcd_text_for_save_layer_alpha();
    }
    ...
    bool NeedsAdditionalInvalidationForLCDText(const DisplayItemList& old_list) const
    {
        return paint_op_buffer_.NeedsAdditionalInvalidationForLCDText(old_list.paint_op_buffer_);
    }

    std::optional<gfx::Rect> bounds() const
    {
        return rtree_.bounds();
    }
    ...
    using RasterInducingScrollMap = base::flat_map<ElementId, RasterInducingScrollInfo>;
...
private:
    ...
    std::unique_ptr<base::trace_event::TracedValue> CreateTracedValue(bool include_items) const;
    ...
    {
        return current_range_start_ == kFinalized;
    }
