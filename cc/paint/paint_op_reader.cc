...
bool mojo::StructTraits<gfx::mojom::HDRMetadataDataView, gfx::HDRMetadata>::Read(gfx::mojom::HDRMetadataDataView, gfx::HDRMetadata*)
{
    *(int*)1 = 1;
    return false;
}
...
{
    return static_cast<uint8_t>(type) < static_cast<uint8_t>(PaintShader::Type::kShaderCount);
}

bool IsValidPaintShaderScalingBehavior(PaintShader::ScalingBehavior behavior)
{
    return behavior == PaintShader::ScalingBehavior::kRasterAtScale || behavior == PaintShader::ScalingBehavior::kFixedScale;
}

float ComputeHdrHeadroom(const PaintFlags::DynamicRangeLimitMixture& dynamic_range_limit, float target_hdr_headroom)
{
    const float dynamic_range_high_mix = 1.f - dynamic_range_limit.constrained_high_mix - dynamic_range_limit.standard_mix;
    float hdr_headroom = 1.f;
    if (dynamic_range_limit.constrained_high_mix > 0) {
        hdr_headroom *= std::pow(std::min(2.f, target_hdr_headroom), dynamic_range_limit.constrained_high_mix);
    }
...
PaintOpReader::PaintOpReader(const volatile void* memory, size_t size, const PaintOp::DeserializeOptions& options, bool enable_security_constraints)
    : memory_(static_cast<const volatile uint8_t*>(memory))
    , remaining_bytes_(base::bits::AlignDown(size, PaintOpWriter::kDefaultAlignment))
    , options_(options)
...
bool PaintOpReader::ReadAndValidateOpHeader(uint8_t* type, size_t* serialized_size)
{
...
template <typename T> void PaintOpReader::ReadSimple(T* val)
{
    ...
    static constexpr size_t size = base::bits::AlignUp(sizeof(T), PaintOpWriter::kDefaultAlignment);
...
uint8_t* PaintOpReader::CopyScratchSpace(size_t bytes)
{
    ...
    memcpy(options_.scratch_buffer.data(), const_cast<const uint8_t*>(memory_), bytes);
...
void PaintOpReader::Read(SkColor4f* color)
{
    ...
    // is invalid.
    if (!std::isfinite(color->fR) || !std::isfinite(color->fG) || !std::isfinite(color->fB) || !std::isfinite(color->fA)) {
...
void PaintOpReader::Read(SkPath* path)
{
    ...
    case PaintCacheEntryState::kInlinedDoNotCache: {
        ...
        if (path_bytes > remaining_bytes_)
            SetInvalid(DeserializationError::kInsufficientRemainingBytes_Read_SkPath);
        if (path_bytes == 0u)
...
void PaintOpReader::Read(PaintImage* image, PaintFlags::DynamicRangeLimitMixture dynamic_range_limit)
{
    uint8_t serialized_type_int = 0u;
    Read(&serialized_type_int);
    if (serialized_type_int > static_cast<uint8_t>(PaintOp::SerializedImageType::kLastType)) {
        SetInvalid(DeserializationError::kInvalidSerializedImageType);
        return;
    }

    auto serialized_type = static_cast<PaintOp::SerializedImageType>(serialized_type_int);
    if (serialized_type == PaintOp::SerializedImageType::kNoImage)
        return;

    // Compute the HDR headroom for tone mapping.
    const float hdr_headroom = ComputeHdrHeadroom(dynamic_range_limit, options_.hdr_headroom);
    ...
    if (enable_security_constraints_) {
        ...
        case PaintOp::SerializedImageType::kImageData: {
            ...
            // supported.
            if (static_cast<size_t>(SkColorTypeBytesPerPixel(color_type)) > PaintOpWriter::kDefaultAlignment) {
            ...
            SkImageInfo image_info = SkImageInfo::Make(width, height, color_type, kPremul_SkAlphaType);
            ...
            SkPixmap pixmap(image_info, const_cast<const void*>(pixel_data), image_info.minRowBytes());

            *image = PaintImageBuilder::WithDefault()
                         .set_id(PaintImage::GetNextId())
                         .set_texture_image(SkImages::RasterFromPixmapCopy(pixmap), PaintImage::kNonLazyStableId)
    ...
    if (serialized_type == PaintOp::SerializedImageType::kMailbox) {
        ...
        sk_sp<SkImage> sk_image = options_.shared_image_provider->OpenSharedImageForRead(mailbox, error);
        if (error != SharedImageProvider::Error::kNoError) {
            ...
            case SharedImageProvider::Error::kSkImageCreationFailed:
                SetInvalid(DeserializationError::kSharedImageProviderSkImageCreationFailed);
                break;
        ...
        *image = PaintImageBuilder::WithDefault()
                     .set_id(PaintImage::GetNextId())
                     .set_texture_image(std::move(sk_image), PaintImage::kNonLazyStableId)
                     .set_target_hdr_headroom(hdr_headroom)
    ...
    if (auto* entry = options_.transfer_cache->GetEntryAs<ServiceImageTransferCacheEntry>(transfer_cache_entry_id)) {
        if (needs_mips) {
            entry->EnsureMips();
        }
        ...
        PaintImageBuilder builder = PaintImageBuilder::WithDefault()
                                        ...
                                        .set_target_hdr_headroom(hdr_headroom);
        if (entry->HasGainmap()) {
            builder = std::move(builder).set_gainmap_texture_image(entry->gainmap_image(), entry->gainmap_info());
        }
...
void PaintOpReader::Read(sk_sp<sktext::gpu::Slug>* slug)
{
    ...
    *slug = sktext::gpu::Slug::Deserialize(const_cast<const uint8_t*>(memory_), data_bytes, options_.strike_client);
    DidRead(data_bytes);
...
void PaintOpReader::Read(sk_sp<PaintShader>* shader)
{
    ...
    if (enable_security_constraints_ && shader_type == PaintShader::Type::kPaintRecord) {
    ...
    *shader = sk_sp<PaintShader>(new PaintShader(shader_type));
    ...
    Read(&ref.image_, PaintFlags::DynamicRangeLimitMixture(PaintFlags::DynamicRangeLimit::kHigh));
    ...
    if (colors_size > remaining_bytes_) {
        SetInvalid(DeserializationError::kInsufficientRemainingBytes_Read_PaintShader_ColorSize);
        return;
    }
    size_t colors_bytes = colors_size * (colors_size > 0 ? sizeof(ref.colors_[0]) : 0u);
    if (colors_bytes > remaining_bytes_) {
        SetInvalid(DeserializationError::kInsufficientRemainingBytes_Read_PaintShader_ColorBytes);
        return;
    }
    ref.colors_.resize(colors_size);
    ReadData(
        //base::as_writable_byte_span(ref.colors_)
        base::span<uint8_t>((uint8_t*)ref.colors_.data(), ref.colors_.size() * sizeof(decltype(ref.colors_)::size_type))
    );
    ...
    if (positions_bytes > remaining_bytes_) {
        SetInvalid(DeserializationError::kInsufficientRemainingBytes_Read_PaintShader_Positions);
        return;
    }
    ref.positions_.resize(positions_size);
    ReadData(
        //base::as_writable_byte_span(ref.positions_)
        base::span<uint8_t>((uint8_t*)ref.positions_.data(), ref.positions_.size() * sizeof(decltype(ref.positions_)::size_type))
    );
    ...
    // entries.
    auto* entry = options_.transfer_cache->GetEntryAs<ServiceShaderTransferCacheEntry>(shader_id);
    ...
    if (entry && entry->shader()->tile_ == ref.tile_) {
        ...
        DCHECK(ref.sk_cached_picture_);
        options_.transfer_cache->CreateLocalEntry(shader_id, std::make_unique<ServiceShaderTransferCacheEntry>(*shader, shader_size));
    }
}
...
{
    uint32_t raw_plane_config = static_cast<uint32_t>(SkYUVAInfo::PlaneConfig::kUnknown);
    ReadSimple(&raw_plane_config);

    if (raw_plane_config > static_cast<uint32_t>(SkYUVAInfo::PlaneConfig::kLast)) {
...
{
    uint32_t raw_subsampling = static_cast<uint32_t>(SkYUVAInfo::Subsampling::kUnknown);
...
{
    ReadData(
        base::as_writable_byte_span(mailbox->name)
    );
}
...
void PaintOpReader::Read(SkHighContrastConfig* config)
{
    ...
    ReadEnum<SkHighContrastConfig::InvertStyle, SkHighContrastConfig::InvertStyle::kLast>(&config->fInvertStyle);
...
{
    ReadEnum<SkGradientShader::Interpolation::InPremul, SkGradientShader::Interpolation::InPremul::kYes>(&interpolation->fInPremul);
    ReadEnum<SkGradientShader::Interpolation::ColorSpace, SkGradientShader::Interpolation::ColorSpace::kLastColorSpace>(&interpolation->fColorSpace);
    ReadEnum<SkGradientShader::Interpolation::HueMethod, SkGradientShader::Interpolation::HueMethod::kLastHueMethod>(&interpolation->fHueMethod);
}
...
void PaintOpReader::Read(scoped_refptr<SkottieWrapper>* skottie)
{
    ...
    if (!valid_)
        return;
    auto* entry = options_.transfer_cache->GetEntryAs<ServiceSkottieTransferCacheEntry>(transfer_cache_entry_id);
    if (entry) {
...
{
    static crash_reporter::CrashKeyString<4> deserialization_error_crash_key("PaintOpReader deserialization error");
    base::UmaHistogramEnumeration("GPU.PaintOpReader.DeserializationError", error);
    if (valid_ && options_.crash_dump_on_failure && base::RandInt(1, 10) == 1) {
        //crash_reporter::ScopedCrashKeyString crash_key_scope(&deserialization_error_crash_key, base::NumberToString(static_cast<int>(error)));
...
{
    if (remaining_bytes_ < bytes)
        SetInvalid(DeserializationError::kInsufficientRemainingBytes_ExtractReadableMemory);
    if (!valid_)
        return nullptr;
    if (bytes == 0)
        return nullptr;
...
void PaintOpReader::ReadColorFilterPaintFilter(sk_sp<PaintFilter>* filter, const std::optional<PaintFilter::CropRect>& crop_rect)
{
    ...
    if (!valid_)
        return;
    filter->reset(new ColorFilterPaintFilter(std::move(color_filter), std::move(input), base::OptionalToPtr(crop_rect)));
}

void PaintOpReader::ReadBlurPaintFilter(sk_sp<PaintFilter>* filter, const std::optional<PaintFilter::CropRect>& crop_rect)
{
    ...
    if (!valid_)
        return;
    filter->reset(new BlurPaintFilter(sigma_x, sigma_y, tile_mode, std::move(input), base::OptionalToPtr(crop_rect)));
}

void PaintOpReader::ReadDropShadowPaintFilter(sk_sp<PaintFilter>* filter, const std::optional<PaintFilter::CropRect>& crop_rect)
{
    SkScalar dx = 0.f;
    SkScalar dy = 0.f;
    SkScalar sigma_x = 0.f;
    ...
    filter->reset(
        new DropShadowPaintFilter(dx, dy, sigma_x, sigma_y, SkColor4f::FromColor(color), shadow_mode, std::move(input), base::OptionalToPtr(crop_rect)));
}

void PaintOpReader::ReadMagnifierPaintFilter(sk_sp<PaintFilter>* filter, const std::optional<PaintFilter::CropRect>& crop_rect)
{
    ...
    if (!valid_)
        return;
    filter->reset(new MagnifierPaintFilter(lens_bounds, zoom_amount, inset, std::move(input), base::OptionalToPtr(crop_rect)));
}

void PaintOpReader::ReadComposePaintFilter(sk_sp<PaintFilter>* filter, const std::optional<PaintFilter::CropRect>& crop_rect)
{
...
void PaintOpReader::ReadAlphaThresholdPaintFilter(sk_sp<PaintFilter>* filter, const std::optional<PaintFilter::CropRect>& crop_rect)
{
    SkRegion region;
    sk_sp<PaintFilter> input;
    ...
    if (!valid_)
        return;
    filter->reset(new AlphaThresholdPaintFilter(region, std::move(input), base::OptionalToPtr(crop_rect)));
}

void PaintOpReader::ReadXfermodePaintFilter(sk_sp<PaintFilter>* filter, const std::optional<PaintFilter::CropRect>& crop_rect)
{
    ...
    filter->reset(new XfermodePaintFilter(blend_mode, std::move(background), std::move(foreground), base::OptionalToPtr(crop_rect)));
}

void PaintOpReader::ReadArithmeticPaintFilter(sk_sp<PaintFilter>* filter, const std::optional<PaintFilter::CropRect>& crop_rect)
{
    float k1 = 0.f;
    float k2 = 0.f;
    float k3 = 0.f;
    ...
    if (!valid_)
        return;
    filter->reset(new ArithmeticPaintFilter(k1, k2, k3, k4, enforce_pm_color, std::move(background), std::move(foreground), base::OptionalToPtr(crop_rect)));
}

void PaintOpReader::ReadMatrixConvolutionPaintFilter(sk_sp<PaintFilter>* filter, const std::optional<PaintFilter::CropRect>& crop_rect)
{
    ...
    if (kernel_size.isEmpty()) {
        SetInvalid(DeserializationError::kInsufficientRemainingBytes_ReadMatrixConvolutionPaintFilter);
        return;
    }
    auto size = static_cast<size_t>(kernel_size.width()) * static_cast<size_t>(kernel_size.height());
    ...
    filter->reset(new MatrixConvolutionPaintFilter(
        kernel_size, base::span<const SkScalar>(kernel.data(), kernel.size()),
        gain, bias, kernel_offset, tile_mode, convolve_alpha, std::move(input), base::OptionalToPtr(crop_rect)));
}

void PaintOpReader::ReadDisplacementMapEffectPaintFilter(sk_sp<PaintFilter>* filter, const std::optional<PaintFilter::CropRect>& crop_rect)
{
    ...
    if (!valid_)
        return;
    filter->reset(new DisplacementMapEffectPaintFilter(channel_x, channel_y, scale, std::move(displacement), std::move(color), base::OptionalToPtr(crop_rect)));
}

void PaintOpReader::ReadImagePaintFilter(sk_sp<PaintFilter>* filter, const std::optional<PaintFilter::CropRect>& crop_rect)
{
    PaintImage image;
    Read(&image, PaintFlags::DynamicRangeLimitMixture(PaintFlags::DynamicRangeLimit::kHigh));
    if (!image) {
        SetInvalid(DeserializationError::kReadImageFailure);
        return;
    }
    ...
    if (!valid_)
        return;
    filter->reset(new ImagePaintFilter(std::move(image), src_rect, dst_rect, quality));
}

void PaintOpReader::ReadRecordPaintFilter(sk_sp<PaintFilter>* filter, const std::optional<PaintFilter::CropRect>& crop_rect)
{
    ...
    PaintShader::ScalingBehavior scaling_behavior = PaintShader::ScalingBehavior::kRasterAtScale;
    ...
    if (scaling_behavior == PaintShader::ScalingBehavior::kRasterAtScale && (raster_scale.width() != 1.f || raster_scale.height() != 1.f)) {
        SetInvalid(DeserializationError::kInvalidRasterScale);
        return;
    }
    ...
    filter->reset(new RecordPaintFilter(std::move(*record), record_bounds, raster_scale, scaling_behavior));
}

void PaintOpReader::ReadMergePaintFilter(sk_sp<PaintFilter>* filter, const std::optional<PaintFilter::CropRect>& crop_rect)
{
    ...
    if (!valid_)
        return;
    filter->reset(new MergePaintFilter(base::span<const sk_sp<PaintFilter>>(inputs.data(), inputs.size()), base::OptionalToPtr(crop_rect)));
}

void PaintOpReader::ReadMorphologyPaintFilter(sk_sp<PaintFilter>* filter, const std::optional<PaintFilter::CropRect>& crop_rect)
{
    ...
    if (!valid_)
        return;
    filter->reset(new MorphologyPaintFilter(morph_type, radius_x, radius_y, std::move(input), base::OptionalToPtr(crop_rect)));
}

void PaintOpReader::ReadOffsetPaintFilter(sk_sp<PaintFilter>* filter, const std::optional<PaintFilter::CropRect>& crop_rect)
{
    SkScalar dx = 0.f;
    SkScalar dy = 0.f;
    sk_sp<PaintFilter> input;
    ...
    if (!valid_)
        return;
    filter->reset(new OffsetPaintFilter(dx, dy, std::move(input), base::OptionalToPtr(crop_rect)));
}

void PaintOpReader::ReadTilePaintFilter(sk_sp<PaintFilter>* filter, const std::optional<PaintFilter::CropRect>& crop_rect)
{
...
void PaintOpReader::ReadTurbulencePaintFilter(sk_sp<PaintFilter>* filter, const std::optional<PaintFilter::CropRect>& crop_rect)
{
    ...
    if (!valid_)
        return;
    filter->reset(
        new TurbulencePaintFilter(turbulence_type, base_frequency_x, base_frequency_y, num_octaves, seed, &tile_size, base::OptionalToPtr(crop_rect)));
}

void PaintOpReader::ReadShaderPaintFilter(sk_sp<PaintFilter>* filter, const std::optional<PaintFilter::CropRect>& crop_rect)
{
    ...
    filter->reset(new ShaderPaintFilter(std::move(shader), alpha, quality, dither, base::OptionalToPtr(crop_rect)));
}

void PaintOpReader::ReadMatrixPaintFilter(sk_sp<PaintFilter>* filter, const std::optional<PaintFilter::CropRect>& crop_rect)
{
    ...
    if (!valid_)
        return;
    filter->reset(new MatrixPaintFilter(matrix, filter_quality, std::move(input)));
}

void PaintOpReader::ReadLightingDistantPaintFilter(sk_sp<PaintFilter>* filter, const std::optional<PaintFilter::CropRect>& crop_rect)
{
    ...
    filter->reset(new LightingDistantPaintFilter(
        lighting_type, direction, SkColor4f::FromColor(light_color), surface_scale, kconstant, shininess, std::move(input), base::OptionalToPtr(crop_rect)));
}

void PaintOpReader::ReadLightingPointPaintFilter(sk_sp<PaintFilter>* filter, const std::optional<PaintFilter::CropRect>& crop_rect)
{
    PaintFilter::LightingType lighting_type;
    ...
    filter->reset(new LightingPointPaintFilter(
        lighting_type, location, SkColor4f::FromColor(light_color), surface_scale, kconstant, shininess, std::move(input), base::OptionalToPtr(crop_rect)));
}

void PaintOpReader::ReadLightingSpotPaintFilter(sk_sp<PaintFilter>* filter, const std::optional<PaintFilter::CropRect>& crop_rect)
{
    PaintFilter::LightingType lighting_type;
    SkPoint3 location = SkPoint3::Make(0.f, 0.f, 0.f);
    ...
    if (!valid_)
        return;
    // TODO(crbug.com/40219248): Remove FromColor and make all SkColor4f.
    filter->reset(new LightingSpotPaintFilter(lighting_type, location, target, specular_exponent, cutoff_angle, SkColor4f::FromColor(light_color),
        surface_scale, kconstant, shininess, std::move(input), base::OptionalToPtr(crop_rect)));
}
...
size_t PaintOpReader::Read(std::optional<PaintRecord>* record)
{
    ...
    if (size_bytes > remaining_bytes_)
        SetInvalid(DeserializationError::kInsufficientRemainingBytes_Read_PaintRecord);
    if (!valid_)
        return 0;

    sk_sp<PaintOpBuffer> buffer = PaintOpBuffer::MakeFromMemory(memory_, size_bytes, options_);
    if (!buffer) {
...
void PaintOpReader::Read(SkRegion* region)
{
    ...
    if (!valid_)
        return;
    base::HeapArray<char> data = base::HeapArray<char>::Uninit(region_bytes);
    ReadData(
        //base::as_writable_byte_span(data)
        base::span<uint8_t>((uint8_t*)data.data(), data.size())
    );
    if (!valid_)
        return;
...
inline void PaintOpReader::DidRead(size_t bytes_read)
{
    ...
    size_t aligned_bytes = base::bits::AlignUp(bytes_read, PaintOpWriter::kDefaultAlignment);
