...
template <typename Function> void IterateTextContent(const PaintOpBuffer& buffer, const Function& yield, const gfx::Rect& rect)
{
    ...
    for (const PaintOp& op : buffer) {
        if (op.GetType() == PaintOpType::kDrawTextBlob) {
            ...
            IterateTextContent(static_cast<const DrawRecordOp&>(op).record.buffer(), yield, rect);
        }
    }
}

template <typename Function>
void IterateTextContentByOffsets(const PaintOpBuffer& buffer, const std::vector<size_t>& offsets, const std::vector<gfx::Rect>& rects, const Function& yield)
{
    ...
    for (const PaintOp& op : PaintOpBuffer::OffsetIterator(buffer, offsets)) {
        if (op.GetType() == PaintOpType::kDrawTextBlob) {
            ...
            IterateTextContent(static_cast<const DrawRecordOp&>(op).record.buffer(), yield, rects[index]);
        }
        ++index;
    }
}

constexpr gfx::Rect kMaxBounds(std::numeric_limits<int>::max(), std::numeric_limits<int>::max());
...
void DisplayItemList::Raster(SkCanvas* canvas, ImageProvider* image_provider, const ScrollOffsetMap* raster_inducing_scroll_offsets) const
{
...
void DisplayItemList::Raster(SkCanvas* canvas, const PlaybackParams& params) const
{
#if DCHECK_IS_ON()
    DCHECK(IsFinalized());
#endif

    TRACE_EVENT_BEGIN1("cc", "DisplayItemList::Raster", "total_op_count", TotalOpCount());
    ...
    if (trace_enabled) {
        ...
        for (PaintOpBuffer::PlaybackFoldingIterator it(paint_op_buffer_, &offsets); it; ++it) {
            rastered_op_count += 1 + it->AdditionalOpCount();
        }
        TRACE_EVENT_END1("cc", "DisplayItemList::Raster", "rastered_op_count", rastered_op_count);
    }
}

std::vector<size_t> DisplayItemList::OffsetsOfOpsToRaster(SkCanvas* canvas) const
{
...
void DisplayItemList::CaptureContent(const gfx::Rect& rect, std::vector<NodeInfo>* content) const
{
#if DCHECK_IS_ON()
    DCHECK(IsFinalized());
#endif
    ...
    IterateTextContentByOffsets(paint_op_buffer_, offsets, rects, [content](const DrawTextBlobOp& op, const gfx::Rect& rect) {
...
double DisplayItemList::AreaOfDrawText(const gfx::Rect& rect) const
{
    ...
    double area = 0;
    size_t index = 0;
    for (const PaintOp& op : PaintOpBuffer::OffsetIterator(paint_op_buffer_, offsets)) {
        if (op.GetType() == PaintOpType::kDrawTextBlob ||
            ...
            (op.GetType() == PaintOpType::kDrawRecord && static_cast<const DrawRecordOp&>(op).record.has_draw_text_ops())) {
...
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "DisplayItemList::Finalize");
#if DCHECK_IS_ON()
    ...
    rtree_.Build(
        visual_rects_.size(), [this](size_t index) { return visual_rects_[index]; }, [this](size_t index) { return offsets_[index]; });
    visual_rects_.clear();
...
{
    bool include_items;
    TRACE_EVENT_CATEGORY_GROUP_ENABLED(TRACE_DISABLED_BY_DEFAULT("cc.debug.display_items"), &include_items);
    TRACE_EVENT_OBJECT_SNAPSHOT_WITH_ID(TRACE_DISABLED_BY_DEFAULT("cc.debug.display_items") "," TRACE_DISABLED_BY_DEFAULT(
                                            "cc.debug.picture") "," TRACE_DISABLED_BY_DEFAULT("devtools.timeline.picture"),
        "cc::DisplayItemList", TRACE_ID_LOCAL(this), CreateTracedValue(include_items));
}
...
std::unique_ptr<base::trace_event::TracedValue> DisplayItemList::CreateTracedValue(bool include_items) const
{
...
void DisplayItemList::AddToValue(base::trace_event::TracedValue* state, bool include_items) const
{
    ...
    if (include_items) {
        ...
        for (const PaintOp& op : paint_op_buffer_) {
            ...
            MathUtil::AddToTracedValue("visual_rect", visual_rects[paint_op_buffer_.GetOpOffsetForTracing(op)], state);
...
scoped_refptr<DiscardableImageMap> DisplayItemList::GenerateDiscardableImageMap() const
{
#if DCHECK_IS_ON()
    DCHECK(IsFinalized());
#endif
    ...
    return DiscardableImageMap::Generate(paint_op_buffer_, bounds().value_or(kMaxBounds));
}

bool DisplayItemList::GetColorIfSolidInRect(const gfx::Rect& rect, SkColor4f* color, int max_ops_to_analyze) const
{
#if DCHECK_IS_ON()
    DCHECK(IsFinalized());
#endif
    ...
    std::optional<SkColor4f> solid_color = SolidColorAnalyzer::DetermineIfSolidColor(paint_op_buffer_, rect, max_ops_to_analyze, offsets_to_use);
    if (solid_color) {
        *color = *solid_color;
        return true;
    }
...
std::optional<DirectlyCompositedImageInfo> DirectlyCompositedImageInfoForPaintOpBuffer(const PaintOpBuffer& op_buffer)
{
    ...
    for (const PaintOp& op : op_buffer) {
        ...
        case PaintOpType::kDrawImageRect: {
            if (result)
                return std::nullopt;
            const auto& draw_image_rect_op = static_cast<const DrawImageRectOp&>(op);
            const SkRect& src = draw_image_rect_op.src;
            const SkRect& dst = draw_image_rect_op.dst;
            if (src.isEmpty() || dst.isEmpty())
                return std::nullopt;
            result.emplace();
            result->default_raster_scale = gfx::Vector2dF(src.width() / dst.width(), src.height() / dst.height());
            // Ensure the layer will use nearest neighbor when drawn by the display
            // compositor, if required.
            result->nearest_neighbor = draw_image_rect_op.flags.getFilterQuality() == PaintFlags::FilterQuality::kNone;
            break;
        }
        case PaintOpType::kDrawRecord:
            if (result)
                return std::nullopt;
            result = DirectlyCompositedImageInfoForPaintOpBuffer(static_cast<const DrawRecordOp&>(op).record.buffer());
            if (!result)
                return std::nullopt;
            break;
        default:
...
std::optional<DirectlyCompositedImageInfo> DisplayItemList::GetDirectlyCompositedImageInfo() const
{
#if DCHECK_IS_ON()
    DCHECK(IsFinalized());
#endif
...
void DisplayItemList::PushDrawScrollingContentsOp(ElementId scroll_element_id, scoped_refptr<DisplayItemList> display_item_list, const gfx::Rect& visual_rect)
{
    StartPaint();
    push<DrawScrollingContentsOp>(scroll_element_id, display_item_list);
    for (auto& [nested_scroll_element_id, info] : std::move(display_item_list->raster_inducing_scrolls_)) {
        ...
        raster_inducing_scrolls_[nested_scroll_element_id] = RasterInducingScrollInfo { visual_rect, info.has_discardable_images };
    }
    raster_inducing_scrolls_[scroll_element_id] = RasterInducingScrollInfo { visual_rect, display_item_list->has_discardable_images() };
