...
BASE_FEATURE(kUseLitePaintOps, "UseLitePaintOps", base::FEATURE_ENABLED_BY_DEFAULT);

// In a future CL, convert DrawImage to explicitly take sampling instead of
// quality
PaintFlags::FilterQuality sampling_to_quality(const SkSamplingOptions& sampling)
{
    ...
    return sampling.filter == SkFilterMode::kLinear ? PaintFlags::FilterQuality::kLow : PaintFlags::FilterQuality::kNone;
}

DrawImage CreateDrawImage(const PaintImage& image, const PaintFlags* flags, const PaintFlags::FilterQuality& quality, const SkM44& matrix)
{
    if (!image)
        return DrawImage();
    return DrawImage(image, flags->useDarkModeForImage(), SkIRect::MakeWH(image.width(), image.height()), quality, matrix);
}

bool IsScaleAdjustmentIdentity(const SkSize& scale_adjustment)
{
    return std::abs(scale_adjustment.width() - 1.f) < FLT_EPSILON && std::abs(scale_adjustment.height() - 1.f) < FLT_EPSILON;
}
...
SkRect AdjustSrcRectForScale(SkRect original, SkSize scale_adjustment)
{
    ...
    return SkRect::MakeXYWH(original.x() * x_scale, original.y() * y_scale, original.width() * x_scale, original.height() * y_scale);
}
...
void DrawImageRect(SkCanvas* canvas, const SkImage* image, const SkRect& src, const SkRect& dst, const SkSamplingOptions& options, const SkPaint* paint,
    SkCanvas::SrcRectConstraint constraint)
{
    if (!image)
        return;
    if (constraint == SkCanvas::kStrict_SrcRectConstraint && options.mipmap != SkMipmapMode::kNone && src.contains(SkRect::Make(image->dimensions()))) {
        SkMatrix m;
    ...
    SkTiledImageUtils::DrawImageRect(canvas, image, src, dst, options, paint, constraint);
}
...
PaintFlags::ScalingOperation MatrixToScalingOperation(SkMatrix m)
{
    ...
    if (m.decomposeScale(&scale)) {
        return (scale.width() > 1 && scale.height() > 1) ? PaintFlags::ScalingOperation::kUpscale : PaintFlags::ScalingOperation::kUnknown;
    }
...
template <typename T, bool HasFlags> struct Rasterizer {
    static void RasterWithFlags(const T* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params)
    {
        static_assert(!T::kHasPaintFlags, "This function should not be used for a PaintOp that has PaintFlags");
        DCHECK(op->IsValid());
        NOTREACHED();
    }
...
template <typename T> struct Rasterizer<T, true> {
    static void RasterWithFlags(const T* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params)
    {
        static_assert(T::kHasPaintFlags, "This function expects the PaintOp to have PaintFlags");
        DCHECK(op->IsValid());
...
using RasterFunction = void (*)(const PaintOp* op, SkCanvas* canvas, const PlaybackParams& params);
#define M(T)                                                                                                                                                   \
    [](const PaintOp* op, SkCanvas* canvas, const PlaybackParams& params) {                                                                                    \
        Rasterizer<T, T::kHasPaintFlags>::Raster(static_cast<const T*>(op), canvas, params);                                                                   \
    },
static const RasterFunction g_raster_functions[kNumOpTypes] = { TYPES(M) };
#undef M

using RasterWithFlagsFunction = void (*)(const PaintOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params);
#define M(T)                                                                                                                                                   \
    [](const PaintOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params) {                                                           \
        Rasterizer<T, T::kHasPaintFlags>::RasterWithFlags(static_cast<const T*>(op), flags, canvas, params);                                                   \
    },
static const RasterWithFlagsFunction g_raster_with_flags_functions[kNumOpTypes] = { TYPES(M) };
#undef M

using SerializeFunction
    = void (*)(const PaintOp& op, PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm);
template <typename T>
ALWAYS_INLINE void Serialize(
    const PaintOp& op, PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm)
{
...
using DeserializeFunction = PaintOp* (*)(PaintOpReader& reader, void* output, size_t output_size);
template <typename T> PaintOp* Deserialize(PaintOpReader& reader, void* output, size_t output_size)
{
...
static const DeserializeFunction g_deserialize_functions[kNumOpTypes] = { TYPES(M) };
#undef M

using AreEqualForTestingFunction = bool (*)(const PaintOp&, const PaintOp&);
template <typename T> bool AreEqualForTesting(const PaintOp& a, const PaintOp& b)
{
    return static_cast<const T&>(a).EqualsForTesting( // IN-TEST
        static_cast<const T&>(b));
}
#define M(T) &AreEqualForTesting<T>,
static const AreEqualForTestingFunction g_equal_for_testing_functions[kNumOpTypes] = { TYPES(M) };
#undef M
...
#define M(T) !std::is_trivially_destructible<T>::value ? [](PaintOp* op) { static_cast<T*>(op)->~T(); } : static_cast<VoidFunction>(nullptr),
static const VoidFunction g_destructor_functions[kNumOpTypes] = { TYPES(M) };
#undef M

#define M(T) static_assert(sizeof(T) <= sizeof(LargestPaintOp), #T " must be no bigger than LargestPaintOp");
TYPES(M)
#undef M

#define M(T) static_assert(alignof(T) <= PaintOpBuffer::kPaintOpAlign, #T " must have alignment no bigger than PaintOpAlign");
TYPES(M)
#undef M

using AnalyzeOpFunc = void (*)(PaintOpBuffer*, const PaintOp*);
#define M(T) [](PaintOpBuffer* buffer, const PaintOp* op) { buffer->AnalyzeAddedOp(static_cast<const T*>(op)); },
...
bool IsDiscardableImage(const PaintImage& image, gfx::ContentColorUsage* content_color_usage)
{
    if (!image || image.IsTextureBacked()) {
        return false;
    }
    if (content_color_usage) {
        *content_color_usage = std::max(*content_color_usage, image.GetContentColorUsage());
    }
    return true;
}
...
bool OpHasDiscardableImagesImpl(const PaintOp& op)
{
    ...
    if (op.IsPaintOpWithFlags() && static_cast<const PaintOpWithFlags&>(op).HasDiscardableImagesFromFlags(unused_content_color_usage)) {
        return true;
    }
    switch (op.GetType()) {
#define M(T)                                                                                                                                                   \
    case T::kType:                                                                                                                                             \
        return static_cast<const T&>(op).HasDiscardableImages(unused_content_color_usage);
...
void AnnotateOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
...
void ClipPathOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
...
void ClipRectOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(rect);
    writer.Write(op);
    writer.Write(antialias);
...
void ClipRRectOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(rrect);
    writer.Write(op);
    writer.Write(antialias);
...
void ConcatOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(matrix);
}

void CustomDataOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(id);
}

void DrawColorOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(color);
    writer.Write(mode);
}

void DrawDRRectOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
...
void DrawImageOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(*flags_to_serialize, current_ctm);

    SkSize serialized_scale_adjustment = SkSize::Make(1.f, 1.f);
    writer.Write(CreateDrawImage(image, flags_to_serialize, GetImageQuality(), current_ctm), &serialized_scale_adjustment);
...
void DrawImageRectOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(*flags_to_serialize, current_ctm);
    ...
    SkSize serialized_scale_adjustment = SkSize::Make(1.f, 1.f);
    writer.Write(CreateDrawImage(image, flags_to_serialize, GetImageQuality(), matrix), &serialized_scale_adjustment);
    writer.Write(serialized_scale_adjustment.width());
    writer.Write(serialized_scale_adjustment.height());
...
void DrawIRectOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(*flags_to_serialize, current_ctm);
    writer.Write(rect);
}

void DrawLineOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(*flags_to_serialize, current_ctm);
    writer.WriteSimpleMultiple(x0, y0, x1, y1, draw_as_path);
}

void DrawLineLiteOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.WriteSimpleMultiple(x0, y0, x1, y1, core_paint_flags);
}

void DrawArcOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(*flags_to_serialize, current_ctm);
    writer.Write(oval);
...
void DrawArcLiteOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.WriteSimpleMultiple(oval, start_angle_degrees, sweep_angle_degrees, core_paint_flags);
}

void DrawOvalOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(*flags_to_serialize, current_ctm);
    writer.Write(oval);
}

void DrawPathOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(*flags_to_serialize, current_ctm);
    writer.Write(path, use_cache);
...
void DrawRecordOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    // These are flattened in PaintOpBufferSerializer.
    NOTREACHED();
}

void DrawRectOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(*flags_to_serialize, current_ctm);
    writer.Write(rect);
}

void DrawRRectOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(*flags_to_serialize, current_ctm);
    writer.Write(rrect);
}

void DrawScrollingContentsOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    // These are flattened in PaintOpBufferSerializer.
    NOTREACHED();
}

void DrawVerticesOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(*flags_to_serialize, current_ctm);
...
void SerializeSkottieMap(
    const base::flat_map<SkottieResourceIdHash, T>& map, PaintOpWriter& writer, base::FunctionRef<void(const T&, PaintOpWriter&)> value_serializer)
{
...
void SerializeSkottieFrameData(const SkM44& current_ctm, const SkottieFrameData& frame_data, PaintOpWriter& writer)
{
    ...
    if (frame_data.image) {
        draw_image = DrawImage(
            frame_data.image, /*use_dark_mode=*/false, SkIRect::MakeWH(frame_data.image.width(), frame_data.image.height()), frame_data.quality, current_ctm);
    }
...
void DrawSkottieOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(dst);
    ...
    if (writer.options().skottie_serialization_history) {
        writer.options().skottie_serialization_history->FilterNewSkottieFrameState(*skottie, images_to_serialize, text_map_to_serialize);
    }

    SerializeSkottieMap<SkottieFrameData>(images_to_serialize, writer,
        [&current_ctm](const SkottieFrameData& frame_data, PaintOpWriter& writer) { SerializeSkottieFrameData(current_ctm, frame_data, writer); });
    SerializeSkottieMap<SkColor>(color_map, writer, [](const SkColor& color, PaintOpWriter& writer) { writer.Write(color); });
    SerializeSkottieMap<SkottieTextPropertyValue>(text_map_to_serialize, writer, [](const SkottieTextPropertyValue& text_property_val, PaintOpWriter& writer) {
        ...
        // text).
        writer.WriteData(text_property_val.text().size(), text_property_val.text().c_str());
        writer.Write(gfx::RectFToSkRect(text_property_val.box()));
    });
}

void DrawSlugOp::SerializeSlugs(const sk_sp<sktext::gpu::Slug>& slug, const std::vector<sk_sp<sktext::gpu::Slug>>& extra_slugs, PaintOpWriter& writer,
    const PaintFlags* flags_to_serialize, const SkM44& current_ctm)
{
    writer.Write(*flags_to_serialize, current_ctm);
...
void DrawSlugOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    DrawSlugOp::SerializeSlugs(slug, extra_slugs, writer, flags_to_serialize, current_ctm);
}

void DrawTextBlobOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    DrawSlugOp::SerializeSlugs(slug, extra_slugs, writer, flags_to_serialize, current_ctm);
}

void NoopOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
}

void RestoreOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
}

void RotateOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(degrees);
}

void SaveOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
}

void SaveLayerOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(*flags_to_serialize, current_ctm);
    writer.Write(bounds);
}

void SaveLayerAlphaOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(bounds);
    writer.Write(alpha);
}

void SaveLayerFiltersOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(*flags_to_serialize, current_ctm);
    writer.Write(filters, current_ctm);
}

void ScaleOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(sx);
    writer.Write(sy);
}

void SetMatrixOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    // Use original_ctm here because SetMatrixOp replaces current_ctm
    writer.Write(original_ctm * matrix);
}

void SetNodeIdOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(node_id);
}

void TranslateOp::Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    writer.Write(dx);
    writer.Write(dy);
}
...
PaintOp* DrawScrollingContentsOp::Deserialize(PaintOpReader& reader, void* output)
{
    // These are flattened during serialization.
    return nullptr;
}
...
PaintOp* DrawVerticesOp::Deserialize(PaintOpReader& reader, void* output)
{
    ...
    op->vertices = base::MakeRefCounted<RefCountedBuffer<SkPoint>>(std::move(vertices));
    ...
    reader.Read(&indices);
    op->indices = base::MakeRefCounted<RefCountedBuffer<uint16_t>>(std::move(indices));
...
template <typename T>
bool DeserializeSkottieMap(base::flat_map<SkottieResourceIdHash, T>& map, std::optional<size_t> max_map_size, PaintOpReader& reader,
    base::FunctionRef<T(PaintOpReader& reader)> value_deserializer)
{
    ...
    for (size_t i = 0; i < map_size; ++i) {
        ...
        SkottieResourceIdHash resource_id_hash = SkottieResourceIdHash::FromUnsafeValue(resource_id_hash_raw);
        if (!resource_id_hash)
            return false;
...
SkottieFrameData DeserializeSkottieFrameData(PaintOpReader& reader)
{
    ...
    reader.Read(&frame_data.image, PaintFlags::DynamicRangeLimitMixture(PaintFlags::DynamicRangeLimit::kHigh));
...
SkottieTextPropertyValue DeserializeSkottieTextPropertyValue(PaintOpReader& reader)
{
    size_t text_size = 0u;
    reader.ReadSize(&text_size);
    std::string text(text_size, char());
    reader.ReadData(
        //base::as_writable_byte_span(text)
        base::span<uint8_t>((uint8_t*)text.data(), text.size())
    );
    SkRect box;
    reader.Read(&box);
...
PaintOp* DrawSkottieOp::Deserialize(PaintOpReader& reader, void* output)
{
    ...
    size_t num_assets_in_animation = op->skottie->GetImageAssetMetadata().asset_storage().size();
    size_t num_text_nodes_in_animation = op->skottie->GetTextNodeNames().size();
    bool deserialized_all_maps
        = DeserializeSkottieMap<SkottieFrameData>(op->images, /*max_map_size=*/num_assets_in_animation, reader, DeserializeSkottieFrameData)
        && DeserializeSkottieMap<SkColor>(op->color_map,
            /*max_map_size=*/std::nullopt, reader, DeserializeSkottieColor)
        && DeserializeSkottieMap<SkottieTextPropertyValue>(
            op->text_map, /*max_map_size=*/num_text_nodes_in_animation, reader, DeserializeSkottieTextPropertyValue);
...
PaintOp* DrawSlugOp::Deserialize(PaintOpReader& reader, void* output)
{
    ...
    if (count > 0) {
        reader.Read(&op->slug);
        const size_t remaining_slug_count = std::min<size_t>(op->extra_slugs.max_size(), count - 1);
...
void AnnotateOp::Raster(const AnnotateOp* op, SkCanvas* canvas, const PlaybackParams& params)
{
...
void ClipPathOp::Raster(const ClipPathOp* op, SkCanvas* canvas, const PlaybackParams& params)
{
    canvas->clipPath(op->path, op->op, op->antialias);
}

void ClipRectOp::Raster(const ClipRectOp* op, SkCanvas* canvas, const PlaybackParams& params)
{
    canvas->clipRect(op->rect, op->op, op->antialias);
}

void ClipRRectOp::Raster(const ClipRRectOp* op, SkCanvas* canvas, const PlaybackParams& params)
{
    canvas->clipRRect(op->rrect, op->op, op->antialias);
}

void ConcatOp::Raster(const ConcatOp* op, SkCanvas* canvas, const PlaybackParams& params)
{
    canvas->concat(op->matrix);
}

void CustomDataOp::Raster(const CustomDataOp* op, SkCanvas* canvas, const PlaybackParams& params)
{
...
void DrawColorOp::Raster(const DrawColorOp* op, SkCanvas* canvas, const PlaybackParams& params)
{
    canvas->drawColor(op->color, op->mode);
}

void DrawDRRectOp::RasterWithFlags(const DrawDRRectOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params)
{
    flags->DrawToSk(canvas, [op](SkCanvas* c, const SkPaint& p) { c->drawDRRect(op->outer, op->inner, p); });
}

void DrawImageOp::RasterWithFlags(const DrawImageOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params)
{
    ...
    if (!params.image_provider) {
        ...
        if (needs_scale) {
            canvas->scale(1.f / op->scale_adjustment.width(), 1.f / op->scale_adjustment.height());
        }
        ...
        if (ToneMapUtil::UseGainmapShader(op->image)) {
            *(int*)1 = 1;
//             skia::DrawGainmapImage(canvas, op->image.cached_sk_image_, op->image.gainmap_sk_image_, op->image.gainmap_info_.value(),
//                 op->image.target_hdr_headroom_, op->left, op->top, op->sampling, paint);
            return;
        }
        ...
        if (ToneMapUtil::UseGlobalToneMapFilter(op->image)) {
            ...
            ToneMapUtil::AddGlobalToneMapFilterToPaint(paint, op->image, dst_color_space);
            sk_image = sk_image->reinterpretColorSpace(dst_color_space);
        }

        SkTiledImageUtils::DrawImage(canvas, sk_image.get(), op->left, op->top, op->sampling, &paint);
        return;
    }

    if (op->image.IsDeferredPaintRecord()) {
        ImageProvider::ScopedResult result = params.image_provider->GetRasterContent(DrawImage(op->image));
    ...
    DrawImage draw_image(op->image, false, SkIRect::MakeWH(op->image.width(), op->image.height()), op->GetImageQuality(), canvas->getLocalToDevice());
    ...
    SkSize scale_adjustment = SkSize::Make(
        op->scale_adjustment.width() * decoded_image.scale_adjustment().width(), op->scale_adjustment.height() * decoded_image.scale_adjustment().height());
    const bool needs_scale = !IsScaleAdjustmentIdentity(scale_adjustment);
    SkAutoCanvasRestore save_restore(canvas, needs_scale);
    if (needs_scale) {
        canvas->scale(1.f / scale_adjustment.width(), 1.f / scale_adjustment.height());
    }
    PaintFlags::ScalingOperation scale = MatrixToScalingOperation(canvas->getLocalToDeviceAs3x3());
    SkTiledImageUtils::DrawImage(
        canvas, decoded_image.image().get(), op->left, op->top, PaintFlags::FilterQualityToSkSamplingOptions(decoded_image.filter_quality(), scale), &paint);
}

void DrawImageRectOp::RasterWithFlags(const DrawImageRectOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params)
{
    ...
    if (!params.image_provider) {
        ...
        SkM44 matrix = canvas->getLocalToDevice() * SkM44(SkMatrix::RectToRect(adjusted_src, op->dst));
        ...
        SkSamplingOptions sampling = PaintFlags::FilterQualityToSkSamplingOptions(quality, scale);
        flags->DrawToSk(canvas, [op, adjusted_src, sampling](SkCanvas* c, const SkPaint& p) {
            sk_sp<SkImage> sk_image;
            if (op->image.IsTextureBacked()) {
                sk_image = op->image.GetAcceleratedSkImage();
            ...
            if (ToneMapUtil::UseGainmapShader(op->image)) {
                *(int*)1 = 1;
//                 skia::DrawGainmapImageRect(c, op->image.cached_sk_image_, op->image.gainmap_sk_image_, op->image.gainmap_info_.value(),
//                     op->image.target_hdr_headroom_, adjusted_src, op->dst, sampling, p);
                return;
            }
            ...
            if (ToneMapUtil::UseGlobalToneMapFilter(op->image)) {
                ...
                ToneMapUtil::AddGlobalToneMapFilterToPaint(tonemap_paint, op->image, c->imageInfo().refColorSpace());
                sk_image = sk_image->reinterpretColorSpace(c->imageInfo().refColorSpace());
                DrawImageRect(c, sk_image.get(), adjusted_src, op->dst, sampling, &tonemap_paint, op->constraint);
                return;
            }

            DrawImageRect(c, sk_image.get(), adjusted_src, op->dst, sampling, &p, op->constraint);
        });
        return;
    }

    SkM44 matrix = canvas->getLocalToDevice() * SkM44(SkMatrix::RectToRect(op->src, op->dst));

    SkIRect int_src_rect;
    op->src.roundOut(&int_src_rect);

    // Dark mode is applied only for OOP raster during serialization.
    DrawImage draw_image(op->image, false, int_src_rect, op->GetImageQuality(), matrix);
    auto scoped_result = params.image_provider->GetRasterContent(draw_image);
    if (!scoped_result)
        return;
    ...
    SkSize scale_adjustment = SkSize::Make(
        op->scale_adjustment.width() * decoded_image.scale_adjustment().width(), op->scale_adjustment.height() * decoded_image.scale_adjustment().height());
    SkRect adjusted_src = op->src.makeOffset(decoded_image.src_rect_offset().width(), decoded_image.src_rect_offset().height());
    adjusted_src = AdjustSrcRectForScale(adjusted_src, scale_adjustment);
    PaintFlags::ScalingOperation scale = MatrixToScalingOperation(matrix.asM33());
    SkSamplingOptions sampling = PaintFlags::FilterQualityToSkSamplingOptions(decoded_image.filter_quality(), scale);
    flags->DrawToSk(canvas, [op, &decoded_image, adjusted_src, sampling](SkCanvas* c, const SkPaint& p) {
        DrawImageRect(c, decoded_image.image().get(), adjusted_src, op->dst, sampling, &p, op->constraint);
    });
}

void DrawIRectOp::RasterWithFlags(const DrawIRectOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params)
{
    flags->DrawToSk(canvas, [op](SkCanvas* c, const SkPaint& p) { c->drawIRect(op->rect, p); });
}

void DrawLineOp::RasterWithFlags(const DrawLineOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params)
{
...
void DrawLineLiteOp::Raster(const DrawLineLiteOp* op, SkCanvas* canvas, const PlaybackParams& params)
{
    PaintFlags flags(op->core_paint_flags);
    flags.DrawToSk(canvas, [op](SkCanvas* c, const SkPaint& p) { c->drawLine(op->x0, op->y0, op->x1, op->y1, p); });
}

void DrawArcImpl(SkCanvas* canvas, const SkRect& oval, float start_angle_degrees, float sweep_angle_degrees, const SkPaint& paint, const PaintFlags& flags)
{
    if (!flags.isArcClosed()) {
        // drawArc can only handle open arcs.
        canvas->drawArc(oval, start_angle_degrees, sweep_angle_degrees, false, paint);
        return;
    }
...
void DrawArcOp::RasterWithFlags(const DrawArcOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params)
{
    op->RasterWithFlagsImpl(flags, canvas);
}

void DrawArcOp::RasterWithFlagsImpl(const PaintFlags* flags, SkCanvas* canvas) const
{
    flags->DrawToSk(canvas, [this, flags](SkCanvas* c, const SkPaint& p) { DrawArcImpl(c, oval, start_angle_degrees, sweep_angle_degrees, p, *flags); });
}

void DrawArcLiteOp::Raster(const DrawArcLiteOp* op, SkCanvas* canvas, const PlaybackParams& params)
{
    PaintFlags flags(op->core_paint_flags);
    flags.DrawToSk(
        canvas, [op, &flags](SkCanvas* c, const SkPaint& p) { DrawArcImpl(c, op->oval, op->start_angle_degrees, op->sweep_angle_degrees, p, flags); });
}

void DrawOvalOp::RasterWithFlags(const DrawOvalOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params)
{
    flags->DrawToSk(canvas, [op](SkCanvas* c, const SkPaint& p) { c->drawOval(op->oval, p); });
}

void DrawPathOp::RasterWithFlags(const DrawPathOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params)
{
    flags->DrawToSk(canvas, [op](SkCanvas* c, const SkPaint& p) { c->drawPath(op->path, p); });
}

void DrawRecordOp::Raster(const DrawRecordOp* op, SkCanvas* canvas, const PlaybackParams& params)
{
    // Don't use drawPicture here, as it adds an implicit clip.
    op->record.Playback(canvas, params, op->local_ctm);
}

void DrawRectOp::RasterWithFlags(const DrawRectOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params)
{
    flags->DrawToSk(canvas, [op](SkCanvas* c, const SkPaint& p) { c->drawRect(op->rect, p); });
}

void DrawRRectOp::RasterWithFlags(const DrawRRectOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params)
{
    flags->DrawToSk(canvas, [op](SkCanvas* c, const SkPaint& p) { c->drawRRect(op->rrect, p); });
}

void DrawScrollingContentsOp::Raster(const DrawScrollingContentsOp* op, SkCanvas* canvas, const PlaybackParams& params)
{
    canvas->save();
    CHECK(params.raster_inducing_scroll_offsets);
    gfx::PointF scroll_offset = params.raster_inducing_scroll_offsets->at(op->scroll_element_id);
    canvas->translate(-scroll_offset.x(), -scroll_offset.y());
    op->display_item_list->Raster(canvas, params);
    canvas->restore();
}

void DrawVerticesOp::RasterWithFlags(const DrawVerticesOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params)
{
    CHECK_EQ(op->vertices->data().size(), op->uvs->data().size());
    SkVertices::Builder vertices_builder(SkVertices::kTriangles_VertexMode, base::checked_cast<int>(op->vertices->data().size()),
        base::checked_cast<int>(op->indices->data().size()), SkVertices::kHasTexCoords_BuilderFlag);

    std::copy(op->vertices->data().data(), op->vertices->data().data() + op->vertices->data().size(), vertices_builder.positions());
    std::copy(op->uvs->data().data(), op->uvs->data().data() + op->uvs->data().size(), vertices_builder.texCoords());
    std::copy(op->indices->data().data(), op->indices->data().data() + op->indices->data().size(), vertices_builder.indices());

    const sk_sp<SkVertices> skverts = vertices_builder.detach();

    flags->DrawToSk(canvas, [&skverts](SkCanvas* c, const SkPaint& p) { c->drawVertices(skverts, SkBlendMode::kSrcOver, p); });
}

void DrawSkottieOp::Raster(const DrawSkottieOp* op, SkCanvas* canvas, const PlaybackParams& params)
{
    ...
    op->skottie->Draw(canvas, op->t, op->dst, base::BindRepeating(&DrawSkottieOp::GetImageAssetForRaster, base::Unretained(op), canvas, std::cref(params)),
        op->color_map, op->text_map);
}

SkottieWrapper::FrameDataFetchResult DrawSkottieOp::GetImageAssetForRaster(SkCanvas* canvas, const PlaybackParams& params, SkottieResourceIdHash asset_id,
    float t_frame, sk_sp<SkImage>& sk_image, SkSamplingOptions& sampling_out) const
{
    ...
    if (!frame_data.image) {
        ...
        // currently.
        DrawImage draw_image(
            frame_data.image, /*use_dark_mode=*/false, SkIRect::MakeWH(frame_data.image.width(), frame_data.image.height()), frame_data.quality, matrix);
        auto scoped_result = params.image_provider->GetRasterContent(draw_image);
        if (scoped_result) {
    ...
    PaintFlags::ScalingOperation scale = MatrixToScalingOperation(matrix.asM33());
    sampling_out = PaintFlags::FilterQualityToSkSamplingOptions(frame_data.quality, scale);
    return SkottieWrapper::FrameDataFetchResult::kNewDataAvailable;
}

void DrawTextBlobOp::RasterWithFlags(const DrawTextBlobOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params)
{
    if (op->node_id)
...
void DrawSlugOp::RasterWithFlags(const DrawSlugOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params)
{
    size_t i = 0;
    flags->DrawToSk(canvas, [op, &params, &i](SkCanvas* c, const SkPaint& p) {
...
void RestoreOp::Raster(const RestoreOp* op, SkCanvas* canvas, const PlaybackParams& params)
{
    canvas->restore();
}

void RotateOp::Raster(const RotateOp* op, SkCanvas* canvas, const PlaybackParams& params)
{
    canvas->rotate(op->degrees);
}

void SaveOp::Raster(const SaveOp* op, SkCanvas* canvas, const PlaybackParams& params)
{
    canvas->save();
}

void SaveLayerOp::RasterWithFlags(const SaveLayerOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params)
{
...
void SaveLayerAlphaOp::Raster(const SaveLayerAlphaOp* op, SkCanvas* canvas, const PlaybackParams& params)
{
    // See PaintOp::kUnsetRect
    bool unset = op->bounds.left() == SK_ScalarInfinity;
    ...
    SkCanvas::SaveLayerRec rec(unset ? nullptr : &op->bounds, base::OptionalToPtr(paint));
    if (params.save_layer_alpha_should_preserve_lcd_text.has_value() && *params.save_layer_alpha_should_preserve_lcd_text) {
        rec.fSaveLayerFlags = SkCanvas::kPreserveLCDText_SaveLayerFlag | SkCanvas::kInitWithPrevious_SaveLayerFlag;
    }
    canvas->saveLayer(rec);
}

void SaveLayerFiltersOp::RasterWithFlags(const SaveLayerFiltersOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params)
{
    SkPaint paint = flags->ToSkPaint();
    base::span<const sk_sp<PaintFilter>> filters(op->filters.data(), op->filters.size());
...
void ScaleOp::Raster(const ScaleOp* op, SkCanvas* canvas, const PlaybackParams& params)
{
    canvas->scale(op->sx, op->sy);
}

void SetMatrixOp::Raster(const SetMatrixOp* op, SkCanvas* canvas, const PlaybackParams& params)
{
    canvas->setMatrix(params.original_ctm * op->matrix);
}

void SetNodeIdOp::Raster(const SetNodeIdOp* op, SkCanvas* canvas, const PlaybackParams& params)
{
    SkPDF::SetNodeId(canvas, op->node_id);
}

void TranslateOp::Raster(const TranslateOp* op, SkCanvas* canvas, const PlaybackParams& params)
{
...
{
    return annotation_type == other.annotation_type && rect == other.rect && !data == !other.data && (!data || data->equals(other.data.get()));
}
...
{
    return x0 == other.x0 && y0 == other.y0 && x1 == other.x1 && y1 == other.y1 && core_paint_flags == other.core_paint_flags;
}

bool DrawArcOp::EqualsForTesting(const DrawArcOp& other) const
{
    return flags.EqualsForTesting(other.flags) && // IN-TEST
        oval == other.oval && start_angle_degrees == other.start_angle_degrees && sweep_angle_degrees == other.sweep_angle_degrees;
}

bool DrawArcLiteOp::EqualsForTesting(const DrawArcLiteOp& other) const
{
    return oval == other.oval && start_angle_degrees == other.start_angle_degrees && sweep_angle_degrees == other.sweep_angle_degrees
...
bool DrawScrollingContentsOp::EqualsForTesting(const DrawScrollingContentsOp& other) const
{
    return scroll_element_id == other.scroll_element_id && display_item_list == other.display_item_list;
}

bool DrawVerticesOp::EqualsForTesting(const DrawVerticesOp& other) const
{
    return flags.EqualsForTesting(other.flags) && // IN-TEST
        *vertices == *other.vertices && *uvs == *other.uvs && *indices == *other.indices;
}
...
bool DrawSkottieOp::EqualsForTesting(const DrawSkottieOp& other) const
{
    ...
    if (t != other.t || dst != other.dst || color_map != other.color_map || text_map != other.text_map) {
        return false;
    }
    return base::ranges::equal(images, other.images, [](const auto& a, const auto& b) {
        return a.first == b.first &&
            ...
            // PaintImage.
            a.second.image.width() == b.second.image.width() && a.second.image.height() == b.second.image.height() && a.second.quality == b.second.quality;
    });
}
...
{
    return flags.EqualsForTesting(other.flags) && // IN-TEST
        !slug == !other.slug && (!slug || slug->serialize()->equals(other.slug->serialize().get()));
}
...
bool SaveLayerFiltersOp::EqualsForTesting(const SaveLayerFiltersOp& other) const
{
    return flags.EqualsForTesting(other.flags) && // IN-TEST
        base::ranges::equal(filters, other.filters, [](const sk_sp<PaintFilter>& lhs, const sk_sp<PaintFilter>& rhs) {
            return base::ValuesEquivalent(lhs, rhs, [](const PaintFilter& x, const PaintFilter& y) {
...
size_t PaintOp::Serialize(
    void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const
{
    ...
    g_serialize_functions[type](*this, writer, flags_to_serialize, current_ctm, original_ctm);
...
PaintOp* PaintOp::Deserialize(
    const volatile void* input, size_t input_size, void* output, size_t output_size, size_t* read_bytes, const DeserializeOptions& options)
{
...
PaintOp* PaintOp::DeserializeIntoPaintOpBuffer(
    const volatile void* input, size_t input_size, PaintOpBuffer* buffer, size_t* read_bytes, const DeserializeOptions& options)
{
    uint8_t type;
    PaintOpReader reader(input, input_size, options);
    if (!reader.ReadAndValidateOpHeader(&type, read_bytes)) {
    ...
    if (auto* op = g_deserialize_functions[type](reader, buffer->AllocatePaintOp(op_aligned_size), op_aligned_size)) {
...
bool PaintOp::GetBounds(const PaintOp& op, SkRect* rect)
{
    ...
    case PaintOpType::kDrawImage: {
        ...
        *rect = SkRect::MakeXYWH(image_op.left, image_op.top, image_op.image.width(), image_op.image.height());
        rect->sort();
        return true;
    }
    ...
    case PaintOpType::kDrawVertices: {
        ...
        rect->setBounds(vertices_op.vertices->data().data(), base::checked_cast<int>(vertices_op.vertices->data().size()));
        return true;
    }
...
// static
gfx::Rect PaintOp::ComputePaintRect(const PaintOp& op, const SkRect& clip_rect, const SkMatrix& ctm)
{
    ...
    if (!PaintOp::GetBounds(op, &op_rect)) {
        ...
        const PaintFlags* flags = op.IsPaintOpWithFlags() ? &(static_cast<const PaintOpWithFlags&>(op).flags) : nullptr;
        SkRect paint_rect = MapRect(ctm, op_rect);
        if (flags) {
            SkPaint paint = flags->ToSkPaint();
            paint_rect = paint.canComputeFastBounds() && paint_rect.isFinite() ? paint.computeFastBounds(paint_rect, &paint_rect) : clip_rect;
        }
...
bool PaintOpWithFlags::HasDiscardableImagesFromFlags(gfx::ContentColorUsage* content_color_usage) const
{
    return flags.HasDiscardableImages(content_color_usage);
}

void PaintOpWithFlags::RasterWithFlags(SkCanvas* canvas, const PaintFlags* raster_flags, const PlaybackParams& params) const
{
...
int DrawLineOp::CountSlowPaths() const
{
    if (const PathEffect* effect = flags.getPathEffect().get()) {
        if (flags.getStrokeCap() != PaintFlags::kRound_Cap && effect->dash_interval_count() == 2) {
...
int DrawPathOp::CountSlowPaths() const
{
    ...
    if (paintStyle == PaintFlags::kStroke_Style && flags.getStrokeWidth() == 0) {
        ...
        return 0;
    } else if (paintStyle == PaintFlags::kFill_Style && pathBounds.width() < 64.f && pathBounds.height() < 64.f && !path.isVolatile()) {
...
bool DrawRecordOp::HasDiscardableImages(gfx::ContentColorUsage* content_color_usage) const
{
    bool has_discardable_images = record.has_discardable_images();
    if (has_discardable_images && content_color_usage) {
        *content_color_usage = std::max(*content_color_usage, record.content_color_usage());
    }
...
bool DrawScrollingContentsOp::HasEffectsPreventingLCDTextForSaveLayerAlpha() const
{
    return display_item_list->has_effects_preventing_lcd_text_for_save_layer_alpha();
}

bool DrawScrollingContentsOp::HasDiscardableImages(gfx::ContentColorUsage* content_color_usage) const
{
    bool has_discardable_images = display_item_list->has_discardable_images();
    if (has_discardable_images && content_color_usage) {
        *content_color_usage = std::max(*content_color_usage, display_item_list->content_color_usage());
    }
    return has_discardable_images;
}
...
AnnotateOp::AnnotateOp(PaintCanvas::AnnotationType annotation_type, const SkRect& rect, sk_sp<SkData> data)
    : PaintOp(kType)
...
DrawImageOp::DrawImageOp(const PaintImage& image, SkScalar left, SkScalar top, const SkSamplingOptions& sampling, const PaintFlags* flags)
...
bool DrawImageOp::HasDiscardableImages(gfx::ContentColorUsage* content_color_usage) const
{
...
DrawImageRectOp::DrawImageRectOp(const PaintImage& image, const SkRect& src, const SkRect& dst, SkCanvas::SrcRectConstraint constraint)
    : PaintOpWithFlags(kType, PaintFlags())
    , image(image)
    , src(src)
    , dst(dst)
...
DrawImageRectOp::DrawImageRectOp(const PaintImage& image, const SkRect& src, const SkRect& dst, const SkSamplingOptions& sampling, const PaintFlags* flags,
    SkCanvas::SrcRectConstraint constraint)
    : PaintOpWithFlags(kType, flags ? *flags : PaintFlags())
    , image(image)
    , src(src)
...
bool DrawImageRectOp::HasDiscardableImages(gfx::ContentColorUsage* content_color_usage) const
{
    return IsDiscardableImage(image, content_color_usage);
}
...
DrawScrollingContentsOp::DrawScrollingContentsOp(ElementId scroll_element_id, scoped_refptr<DisplayItemList> display_item_list)
    : PaintOp(kType)
...
DrawVerticesOp::DrawVerticesOp(scoped_refptr<RefCountedBuffer<SkPoint>> vertices, scoped_refptr<RefCountedBuffer<SkPoint>> uvs,
    scoped_refptr<RefCountedBuffer<uint16_t>> indices, const PaintFlags& flags)
...
DrawSkottieOp::DrawSkottieOp(scoped_refptr<SkottieWrapper> skottie, SkRect dst, float t, SkottieFrameDataMap images, const SkottieColorMap& color_map,
...
bool DrawSkottieOp::HasDiscardableImages(gfx::ContentColorUsage* content_color_usage) const
{
    if (images.empty()) {
        return false;
    }
    if (content_color_usage) {
        for (auto& [_, frame_data] : images) {
            *content_color_usage = std::max(*content_color_usage, frame_data.image.GetContentColorUsage());
        }
    }
    return true;
}
...
DrawTextBlobOp::DrawTextBlobOp(sk_sp<SkTextBlob> blob, SkScalar x, SkScalar y, const PaintFlags& flags)
    : PaintOpWithFlags(kType, flags)
    , blob(std::move(blob))
    , x(x)
    , y(y)
...
DrawTextBlobOp::DrawTextBlobOp(sk_sp<SkTextBlob> blob, SkScalar x, SkScalar y, NodeId node_id, const PaintFlags& flags)
    : PaintOpWithFlags(kType, flags)
    , blob(std::move(blob))
    , x(x)
    , y(y)
...
SaveLayerFiltersOp::SaveLayerFiltersOp(base::span<const sk_sp<PaintFilter>> filters, const PaintFlags& flags)
    : PaintOpWithFlags(kType, flags)
