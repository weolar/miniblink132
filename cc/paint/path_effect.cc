...
template <typename T> bool AreEqualForTesting(const PathEffect& a, const PathEffect& b)
{
...
private:
    ...
    {
        return (
            base::CheckedNumeric<size_t>(PaintOpWriter::SerializedSizeOfElements(intervals_.data(), intervals_.size())) + PaintOpWriter::SerializedSize(phase_))
            .ValueOrDie();
    }
    ...
    {
        return SkDashPathEffect::Make(intervals_.data(), base::checked_cast<int>(intervals_.size()), phase_);
    }

    size_t dash_interval_count() const override
    {
...
sk_sp<PathEffect> PathEffect::MakeDash(const float* intervals, int count, float phase)
{
...
sk_sp<PathEffect> PathEffect::Deserialize(PaintOpReader& reader, Type type)
{
    ...
    case Type::kDash: {
        ...
        reader.Read(&phase);
        return reader.valid() ? MakeDash(intervals.data(), base::checked_cast<int>(intervals.size()), phase) : nullptr;
    }
    case Type::kCorner: {
        float radius;
        reader.Read(&radius);
