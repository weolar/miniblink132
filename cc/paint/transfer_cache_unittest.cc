...
public:
    TransferCacheTest()
        : test_client_entry_(std::vector<uint8_t>(100))
    {
    }
    ...
    void SetUp() override
    {
        ...
        auto result = context_->Initialize(viz::TestGpuServiceHolder::GetInstance()->task_executor(), attribs, gpu::SharedMemoryLimits(), nullptr, nullptr);
    ...
    int decoder_id()
    {
        return context_->GetRasterDecoderIdForTest();
    }
    ...
    void CreateEntry(const ClientTransferCacheEntry& entry)
    {
        ...
        context_support->UnmapAndCreateTransferCacheEntry(entry.UnsafeType(), entry.Id());
    }
...
TEST_F(TransferCacheTest, Basic)
{
    ...
    EXPECT_NE(nullptr, service_cache->GetEntry(gpu::ServiceTransferCache::EntryKey(decoder_id(), entry.Type(), entry.Id())));

    // Unlock on client side and flush to service.
    context_support->UnlockTransferCacheEntries({ { entry.UnsafeType(), entry.Id() } });
    ri()->Finish();

    // Re-lock on client side and validate state. No need to flush as lock is
    // local.
    EXPECT_TRUE(context_support->ThreadsafeLockTransferCacheEntry(entry.UnsafeType(), entry.Id()));
    ...
    ri()->Finish();
    EXPECT_EQ(nullptr, service_cache->GetEntry(gpu::ServiceTransferCache::EntryKey(decoder_id(), entry.Type(), entry.Id())));
}
...
TEST_F(TransferCacheTest, MemoryEviction)
{
    ...
    EXPECT_FALSE(context_support->ThreadsafeLockTransferCacheEntry(entry.UnsafeType(), entry.Id()));
}
...
TEST_F(TransferCacheTest, CountEviction)
{
    ...
    for (int i = 0; i < 10; ++i) {
        entries.emplace_back(std::make_unique<ClientRawMemoryTransferCacheEntry>(std::vector<uint8_t>(4)));
        CreateEntry(*entries[i]);
        context_support->UnlockTransferCacheEntries({ { entries[i]->UnsafeType(), entries[i]->Id() } });
        ri()->Finish();
    }
...
TEST_F(TransferCacheTest, RawMemoryTransferSmall)
{
    ...
    ServiceTransferCacheEntry* service_entry
        = service_cache->GetEntry(gpu::ServiceTransferCache::EntryKey(decoder_id(), client_entry.Type(), client_entry.Id()));
    EXPECT_EQ(service_entry->Type(), client_entry.Type());
    const std::vector<uint8_t> service_data = static_cast<ServiceRawMemoryTransferCacheEntry*>(service_entry)->data();
