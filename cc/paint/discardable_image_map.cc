...
public:
    Generator(DiscardableImageMap& map, SkNoDrawCanvas& canvas, const PaintOpBuffer& buffer)
        : map_(map)
        , canvas_(canvas)
    {
...
private:
    ...
    public:
        ...
        {
            generator_.AddImage(draw_image.paint_image(), false, SkRect::Make(draw_image.src_rect()), op_rect_, SkM44(), draw_image.filter_quality());
            return ScopedResult();
        }
    ...
    void GatherDiscardableImages(const PaintOpBuffer& buffer, const gfx::Rect* top_level_op_rect)
    {
        ...
        for (const PaintOp& op : buffer) {
            ...
            if (op.IsPaintOpWithFlags()) {
                AddImageFromFlags(op_rect, static_cast<const PaintOpWithFlags&>(op).flags, ctm);
            }

            PaintOpType op_type = op.GetType();
            if (op_type == PaintOpType::kDrawImage) {
                const auto& image_op = static_cast<const DrawImageOp&>(op);
                PaintFlags::FilterQuality quality = base::FeatureList::IsEnabled(features::kPreserveDiscardableImageMapQuality)
                    ? image_op.GetImageQuality()
                    : image_op.flags.getFilterQuality();
                AddImage(image_op.image, image_op.flags.useDarkModeForImage(), SkRect::MakeIWH(image_op.image.width(), image_op.image.height()), op_rect, ctm,
                    quality);
                ...
                // in MathUtil.
                SkM44 matrix = ctm * SkM44(SkMatrix::RectToRect(image_rect_op.src, image_rect_op.dst));
                PaintFlags::FilterQuality quality = base::FeatureList::IsEnabled(features::kPreserveDiscardableImageMapQuality)
                    ? image_rect_op.GetImageQuality()
                    : image_rect_op.flags.getFilterQuality();
                AddImage(image_rect_op.image, image_rect_op.flags.useDarkModeForImage(), image_rect_op.src, op_rect, matrix, quality);
                ...
                for (const auto& image_pair : skottie_op.images) {
                    ...
                    SkRect image_src_rect = SkRect::MakeIWH(frame_data.image.width(), frame_data.image.height());
                    ...
                    static constexpr SkMatrix::ScaleToFit kScalingMode = SkMatrix::kCenter_ScaleToFit;
                    SkRect skottie_frame_native_size = SkRect::MakeSize(skottie_op.skottie->size());
                    SkM44 matrix = ctm * SkM44(SkMatrix::RectToRect(skottie_frame_native_size, gfx::RectToSkRect(dst_rect), kScalingMode));
                    AddImage(frame_data.image, /*use_dark_mode=*/false, std::move(image_src_rect), std::move(dst_rect), matrix, frame_data.quality);
                }
            } else if (op_type == PaintOpType::kDrawRecord) {
                GatherDiscardableImages(static_cast<const DrawRecordOp&>(op).record.buffer(), top_level_op_rect);
            } else if (op_type == PaintOpType::kDrawScrollingContents) {
                const auto& draw_scrolling_contents_op = static_cast<const DrawScrollingContentsOp&>(op);
                ...
                GatherDiscardableImages(draw_scrolling_contents_op.display_item_list->paint_op_buffer(), &op_rect);
            }
        }
    }

    void AddImageFromFlags(const gfx::Rect& op_rect, const PaintFlags& flags, const SkM44& ctm)
    {
        // TODO(prashant.n): Add dark mode support for images from shaders/filters.
        AddImageFromShader(op_rect, flags.getShader(), ctm, flags.getFilterQuality());
        AddImageFromFilter(op_rect, flags.getImageFilter().get());
    }

    void AddImageFromShader(const gfx::Rect& op_rect, const PaintShader* shader, const SkM44& ctm, PaintFlags::FilterQuality filter_quality)
    {
        ...
        if (shader->shader_type() == PaintShader::Type::kImage) {
            ...
            AddImage(paint_image, false, SkRect::MakeWH(paint_image.width(), paint_image.height()), op_rect, matrix, filter_quality);
            return;
        }
        ...
        if (shader->shader_type() == PaintShader::Type::kPaintRecord) {
            ...
            // images.
            if (shader->image_analysis_state() == ImageAnalysisState::kNoAnimatedImages) {
                return;
            }
            ...
            SkNoDrawCanvas canvas(scaled_tile_rect.width(), scaled_tile_rect.height());
            ...
            const_cast<PaintShader*>(shader)->set_has_animated_images(has_animated_images);
        }
    }
    ...
    void AddImageFromFilter(const gfx::Rect& op_rect, const PaintFilter* filter)
    {
        ...
        if (!filter || !filter->has_discardable_images() || filter->image_analysis_state() == ImageAnalysisState::kNoAnimatedImages) {
            return;
        }
        ...
        DCHECK_GE(map_.images_.size(), prev_images_size);
        const bool has_animated_images = map_.images_.size() > prev_images_size;
        const_cast<PaintFilter*>(filter)->set_has_animated_images(has_animated_images);
    }

    void AddImage(PaintImage paint_image, bool use_dark_mode, const SkRect& src_rect, const gfx::Rect& image_rect, const SkM44& matrix,
        PaintFlags::FilterQuality filter_quality)
    {
        ...
        if (paint_image.IsPaintWorklet()) {
            map_.paint_worklet_inputs_.emplace_back(paint_image.GetPaintWorkletInput(), paint_image.stable_id());
        }
        ...
        if (paint_image.IsLazyGenerated()) {
            auto decoding_mode_it = map_.decoding_mode_map_.find(paint_image.stable_id());
            ...
            if (decoding_mode_it == map_.decoding_mode_map_.end()) {
                map_.decoding_mode_map_[paint_image.stable_id()] = paint_image.decoding_mode();
            } else {
                decoding_mode_it->second = PaintImage::GetConservative(decoding_mode_it->second, paint_image.decoding_mode());
            }
        }

        if (paint_image.ShouldAnimate()) {
            map_.animated_images_metadata_.emplace_back(paint_image.stable_id(), paint_image.completion_state(), paint_image.GetFrameMetadata(),
                paint_image.repetition_count(), paint_image.reset_animation_sequence_id());
        }
        ...
        if (add_image) {
            map_.images_.emplace_back(DrawImage(std::move(paint_image), use_dark_mode, src_irect, filter_quality, matrix), image_rect);
        }
    }
...
scoped_refptr<DiscardableImageMap> DiscardableImageMap::Generate(const PaintOpBuffer& paint_op_buffer, const gfx::Rect& bounds)
{
...
base::flat_map<PaintImage::Id, PaintImage::DecodingMode> DiscardableImageMap::TakeDecodingModeMap()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    return std::move(decoding_mode_map_);
}

std::vector<const DrawImage*> DiscardableImageMap::GetDiscardableImagesInRect(const gfx::Rect& rect) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (!images_rtree_) {
        ...
        images_rtree_->Build(
            images_.size(), [this](size_t index) { return images_[index].second; }, [this](size_t index) { return &images_[index].first; });
    }
    images_rtree_->Search(rect, &result);
    return result;
}

const DiscardableImageMap::Rects& DiscardableImageMap::GetRectsForImage(PaintImage::Id image_id) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
DiscardableImageMap::AnimatedImageMetadata::AnimatedImageMetadata(PaintImage::Id paint_image_id, PaintImage::CompletionState completion_state,
    std::vector<FrameMetadata> frames, int repetition_count, PaintImage::AnimationSequenceId reset_animation_sequence_id)
...
DiscardableImageMap::AnimatedImageMetadata::AnimatedImageMetadata(const AnimatedImageMetadata& other) = default;
