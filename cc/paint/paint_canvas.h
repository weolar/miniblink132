...
public:
    ...
    virtual void* accessTopLayerPixels(SkImageInfo* info, size_t* rowBytes, SkIPoint* origin = nullptr) = 0;
    ...
    // its shadow.
    virtual int saveLayerFilters(base::span<const sk_sp<PaintFilter>> filters, const PaintFlags& flags) = 0;
    ...
    virtual void clipRect(const SkRect& rect, SkClipOp op, bool do_anti_alias) = 0;
    ...
    virtual void clipRRect(const SkRRect& rrect, SkClipOp op, bool do_anti_alias) = 0;
    ...
    virtual void clipPath(const SkPath& path, SkClipOp op, bool do_anti_alias, UsePaintCache) = 0;
    ...
    {
        clipPath(path, SkClipOp::kIntersect, do_anti_alias, UsePaintCache::kEnabled);
    }
    ...
    void drawColor(SkColor4f color)
    {
    ...
    virtual void drawLine(SkScalar x0, SkScalar y0, SkScalar x1, SkScalar y1, const PaintFlags& flags) = 0;
    virtual void drawArc(const SkRect& oval, SkScalar start_angle_degrees, SkScalar sweep_angle_degrees, const PaintFlags& flags) = 0;
    ...
    virtual void drawDRRect(const SkRRect& outer, const SkRRect& inner, const PaintFlags& flags) = 0;
    virtual void drawRoundRect(const SkRect& rect, SkScalar rx, SkScalar ry, const PaintFlags& flags) = 0;
    virtual void drawPath(const SkPath& path, const PaintFlags& flags, UsePaintCache) = 0;
    void drawPath(const SkPath& path, const PaintFlags& flags)
    {
        drawPath(path, flags, UsePaintCache::kEnabled);
    }
    virtual void drawImage(const PaintImage& image, SkScalar left, SkScalar top, const SkSamplingOptions&, const PaintFlags* flags) = 0;
    void drawImage(const PaintImage& image, SkScalar left, SkScalar top)
    {
        drawImage(image, left, top, SkSamplingOptions(), nullptr);
    }

    virtual void drawImageRect(const PaintImage& image, const SkRect& src, const SkRect& dst, const SkSamplingOptions&, const PaintFlags* flags,
        SkCanvas::SrcRectConstraint constraint)
        = 0;
    void drawImageRect(const PaintImage& image, const SkRect& src, const SkRect& dst, SkCanvas::SrcRectConstraint constraint)
    {
        drawImageRect(image, src, dst, SkSamplingOptions(), nullptr, constraint);
    }

    virtual void drawVertices(scoped_refptr<RefCountedBuffer<SkPoint>> vertices, scoped_refptr<RefCountedBuffer<SkPoint>> uvs,
    ...
    virtual void drawSkottie(scoped_refptr<SkottieWrapper> skottie, const SkRect& dst, float t, SkottieFrameDataMap images, const SkottieColorMap& color_map,
        SkottieTextPropertyValueMap text_map)
        = 0;

    virtual void drawTextBlob(sk_sp<SkTextBlob> blob, SkScalar x, SkScalar y, const PaintFlags& flags) = 0;

    virtual void drawTextBlob(sk_sp<SkTextBlob> blob, SkScalar x, SkScalar y, NodeId node_id, const PaintFlags& flags) = 0;
    ...
    virtual void Annotate(AnnotationType type, const SkRect& rect, sk_sp<SkData> data) = 0;
    printing::MetafileSkia* GetPrintingMetafile() const
    {
        return metafile_;
    }
...
private:
    ...
    raw_ptr<paint_preview::PaintPreviewTracker, DanglingUntriaged> tracker_ = nullptr;
};
