...
public:
    explicit ThreadsafePath(const SkPath& path)
        : SkPath(path)
    {
        updateBoundsCache();
        getGenerationID();
...
#define HAS_SERIALIZATION_FUNCTIONS()                                                                                                                          \
    void Serialize(PaintOpWriter& writer, const PaintFlags* flags_to_serialize, const SkM44& current_ctm, const SkM44& original_ctm) const;                    \
...
public:
    ...
    explicit PaintOp(PaintOpType type)
    ...
    {
        return static_cast<PaintOpType>(type);
    }
    ...
    uint16_t AlignedSize() const
    {
        return g_type_to_aligned_size[type];
    }

    bool EqualsForTesting(const PaintOp& other) const;

    // Indicates how PaintImages are serialized.
    enum class SerializedImageType : uint8_t { kNoImage, kImageData, kTransferCacheEntry, kMailbox, kLastType = kMailbox };
    ...
    size_t Serialize(void* memory, size_t size, const SerializeOptions& options, const PaintFlags* flags_to_serialize, const SkM44& current_ctm,
    ...
    // this canvas.
    static gfx::Rect ComputePaintRect(const PaintOp& op, const SkRect& clip_rect, const SkMatrix& ctm);
    ...
    int CountSlowPathsFromFlags() const
    {
        return 0;
    }
    ...
    bool HasDiscardableImagesFromFlags(gfx::ContentColorUsage* content_color_usage) const
    {
        return false;
    }
    ...
    {
        return static_cast<uint32_t>(mode) <= static_cast<uint32_t>(SkBlendMode::kLastCoeffMode);
    }

    // PaintFlags can have more complex blend modes than kDrawColor.
    static bool IsValidPaintFlagsSkBlendMode(SkBlendMode mode)
    {
        return static_cast<uint32_t>(mode) <= static_cast<uint32_t>(SkBlendMode::kLastMode);
    }

    static bool IsValidSkClipOp(SkClipOp op)
    {
        return static_cast<uint32_t>(op) <= static_cast<uint32_t>(SkClipOp::kMax_EnumValue);
    }
    ...
    static constexpr size_t kNumOpTypes = static_cast<size_t>(PaintOpType::kLastPaintOpType) + 1;
...
public:
    ...
    int CountSlowPathsFromFlags() const
    {
        return flags.getPathEffect() ? 1 : 0;
    }
    bool HasNonAAPaint() const
    {
    ...
    void RasterWithFlags(SkCanvas* canvas, const PaintFlags* flags, const PlaybackParams& params) const;
...
protected:
    ...
    explicit PaintOpWithFlags(PaintOpType type)
        : PaintOp(type)
    {
    }
};
...
public:
    ...
    AnnotateOp(PaintCanvas::AnnotationType annotation_type, const SkRect& rect, sk_sp<SkData> data);
    ~AnnotateOp();
    static void Raster(const AnnotateOp* op, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
...
public:
    ...
    ClipPathOp(SkPath path, SkClipOp op, bool antialias, UsePaintCache use_paint_cache = UsePaintCache::kEnabled)
        : PaintOp(kType)
        , path(path)
        , op(op)
        , antialias(antialias)
    ...
    static void Raster(const ClipPathOp* op, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
...
public:
    ...
    static void Raster(const ClipRectOp* op, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return IsValidSkClipOp(op) && rect.isFinite();
    }
...
public:
    ...
    static void Raster(const ClipRRectOp* op, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return IsValidSkClipOp(op) && rrect.isValid();
    }
...
public:
    ...
    static void Raster(const ConcatOp* op, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return true;
    }
...
public:
    ...
    explicit CustomDataOp(uint32_t id)
        : PaintOp(kType)
        , id(id)
    {
    }
    static void Raster(const CustomDataOp* op, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return true;
    }
...
public:
    ...
    static void Raster(const DrawColorOp* op, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return IsValidDrawColorSkBlendMode(mode);
    }
...
public:
    ...
    static constexpr bool kIsDrawOp = true;
    DrawDRRectOp(const SkRRect& outer, const SkRRect& inner, const PaintFlags& flags)
        : PaintOpWithFlags(kType, flags)
        , outer(outer)
        , inner(inner)
    {
    }
    static void RasterWithFlags(const DrawDRRectOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
...
public:
    ...
    DrawImageOp(const PaintImage& image, SkScalar left, SkScalar top, const SkSamplingOptions&, const PaintFlags* flags);
    ~DrawImageOp();
    static void RasterWithFlags(const DrawImageOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return flags.IsValid() && std::isfinite(scale_adjustment.width()) && std::isfinite(scale_adjustment.height());
    }
...
public:
    ...
    static constexpr bool kIsDrawOp = true;
    DrawImageRectOp(const PaintImage& image, const SkRect& src, const SkRect& dst, SkCanvas::SrcRectConstraint constraint);
    DrawImageRectOp(const PaintImage& image, const SkRect& src, const SkRect& dst, const SkSamplingOptions&, const PaintFlags* flags,
        SkCanvas::SrcRectConstraint constraint);
    ~DrawImageRectOp();
    static void RasterWithFlags(const DrawImageRectOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return flags.IsValid() && src.isFinite() && dst.isFinite() && std::isfinite(scale_adjustment.width()) && std::isfinite(scale_adjustment.height());
    }
...
public:
    ...
    static void RasterWithFlags(const DrawIRectOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
...
public:
    ...
    static constexpr bool kIsDrawOp = true;
    DrawLineOp(SkScalar x0, SkScalar y0, SkScalar x1, SkScalar y1, const PaintFlags& flags, bool draw_as_path = false)
        : PaintOpWithFlags(kType, flags)
        , x0(x0)
        , y0(y0)
        , x1(x1)
    ...
    static void RasterWithFlags(const DrawLineOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return flags.IsValid();
    }
...
public:
    ...
    static constexpr bool kIsDrawOp = true;
    DrawLineLiteOp(SkScalar x0, SkScalar y0, SkScalar x1, SkScalar y1, const CorePaintFlags& core_paint_flags)
        : PaintOp(kType)
        , x0(x0)
        , y0(y0)
        , x1(x1)
    ...
    static void Raster(const DrawLineLiteOp* op, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return core_paint_flags.IsValid();
    }
...
public:
    ...
    static constexpr bool kIsDrawOp = true;
    DrawArcLiteOp(const SkRect& oval, SkScalar start_angle_degrees, SkScalar sweep_angle_degrees, const CorePaintFlags& core_paint_flags)
        : PaintOp(kType)
        , oval(oval)
    ...
    static void Raster(const DrawArcLiteOp* op, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return core_paint_flags.IsValid() && oval.isFinite() && std::isfinite(start_angle_degrees) && std::isfinite(sweep_angle_degrees);
    }
...
public:
    ...
    static constexpr bool kIsDrawOp = true;
    DrawArcOp(const SkRect& oval, SkScalar start_angle_degrees, SkScalar sweep_angle_degrees, const PaintFlags& flags)
        : PaintOpWithFlags(kType, flags)
        , oval(oval)
        , start_angle_degrees(start_angle_degrees)
        , sweep_angle_degrees(sweep_angle_degrees)
    ...
    static void RasterWithFlags(const DrawArcOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params);
    // Actual implementation for rastering.
    void RasterWithFlagsImpl(const PaintFlags* flags, SkCanvas* canvas) const;
    bool IsValid() const
    {
        return flags.IsValid() && oval.isFinite() && std::isfinite(start_angle_degrees) && std::isfinite(sweep_angle_degrees);
    }
...
public:
    ...
    static void RasterWithFlags(const DrawOvalOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
...
public:
    ...
    static constexpr bool kIsDrawOp = true;
    DrawPathOp(const SkPath& path, const PaintFlags& flags, UsePaintCache use_paint_cache = UsePaintCache::kEnabled)
        : PaintOpWithFlags(kType, flags)
        , path(path)
    ...
    static void RasterWithFlags(const DrawPathOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return flags.IsValid() && IsValidPath(path);
    }
...
public:
    ...
    ~DrawRecordOp();
    static void Raster(const DrawRecordOp* op, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return true;
    }
...
public:
    ...
    static void RasterWithFlags(const DrawRectOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return flags.IsValid() && rect.isFinite();
    }
...
public:
    ...
    static void RasterWithFlags(const DrawRRectOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return flags.IsValid() && rrect.isValid();
    }
...
public:
    ...
    static constexpr bool kIsDrawOp = true;
    DrawScrollingContentsOp(ElementId scroll_element_id, scoped_refptr<DisplayItemList> display_item_list);
    ~DrawScrollingContentsOp();
    static void Raster(const DrawScrollingContentsOp* op, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return scroll_element_id && display_item_list;
    }
...
public:
    ...
    DrawVerticesOp(scoped_refptr<RefCountedBuffer<SkPoint>> vertices, scoped_refptr<RefCountedBuffer<SkPoint>> uvs,
        scoped_refptr<RefCountedBuffer<uint16_t>> indices, const PaintFlags&);
    ~DrawVerticesOp();

    static void RasterWithFlags(const DrawVerticesOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params);

    bool IsValid() const
    {
        return flags.IsValid() && vertices && uvs && indices && vertices->data().size() > 0 && vertices->data().size() == uvs->data().size()
...
public:
    ...
    static constexpr bool kIsDrawOp = true;
    DrawSkottieOp(scoped_refptr<SkottieWrapper> skottie, SkRect dst, float t, SkottieFrameDataMap images, const SkottieColorMap& color_map,
        SkottieTextPropertyValueMap text_map);
    ~DrawSkottieOp();
    static void Raster(const DrawSkottieOp* op, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return skottie && skottie->is_valid() && !dst.isEmpty() && t >= 0 && t <= 1.f;
    }
...
private:
    SkottieWrapper::FrameDataFetchResult GetImageAssetForRaster(SkCanvas* canvas, const PlaybackParams& params, SkottieResourceIdHash asset_id, float t_frame,
        sk_sp<SkImage>& image_out, SkSamplingOptions& sampling_out) const;
...
public:
    ...
    ~DrawSlugOp();
    static void SerializeSlugs(const sk_sp<sktext::gpu::Slug>& slug, const std::vector<sk_sp<sktext::gpu::Slug>>& extra_slugs, PaintOpWriter& writer,
        const PaintFlags* flags_to_serialize, const SkM44& current_ctm);
    static void RasterWithFlags(const DrawSlugOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return flags.IsValid();
    }
...
public:
    ...
    static constexpr bool kIsDrawOp = true;
    DrawTextBlobOp(sk_sp<SkTextBlob> blob, SkScalar x, SkScalar y, const PaintFlags& flags);
    DrawTextBlobOp(sk_sp<SkTextBlob> blob, SkScalar x, SkScalar y, NodeId node_id, const PaintFlags& flags);
    ~DrawTextBlobOp();
    static void RasterWithFlags(const DrawTextBlobOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return flags.IsValid();
    }
...
public:
    ...
    static void Raster(const NoopOp* op, SkCanvas* canvas, const PlaybackParams& params)
    {
    }
    bool IsValid() const
    {
...
public:
    ...
    static void Raster(const RestoreOp* op, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return true;
    }
...
public:
    ...
    static void Raster(const RotateOp* op, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return true;
    }
...
public:
    ...
    static void Raster(const SaveOp* op, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return true;
    }
...
public:
    ...
    static void RasterWithFlags(const SaveLayerOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return flags.IsValid() && IsValidOrUnsetRect(bounds);
    }
...
public:
    ...
    static void Raster(const SaveLayerAlphaOp* op, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return IsValidOrUnsetRect(bounds);
    }
...
public:
    ...
    explicit SaveLayerFiltersOp(base::span<const sk_sp<PaintFilter>> filters, const PaintFlags& flags);
    ~SaveLayerFiltersOp();
    static void RasterWithFlags(const SaveLayerFiltersOp* op, const PaintFlags* flags, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
...
public:
    ...
    static void Raster(const ScaleOp* op, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return true;
    }
...
public:
    ...
    static void Raster(const SetMatrixOp* op, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return true;
    }
...
public:
    ...
    static void Raster(const SetNodeIdOp* op, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return true;
    }
...
public:
    ...
    static void Raster(const TranslateOp* op, SkCanvas* canvas, const PlaybackParams& params);
    bool IsValid() const
    {
        return true;
    }
...
using LargestPaintOp = typename std::conditional<(sizeof(DrawImageRectOp) > sizeof(DrawDRRectOp)), DrawImageRectOp, DrawDRRectOp>::type;
...
inline constexpr size_t kLargestPaintOpAlignedSize = PaintOpBuffer::ComputeOpAlignedSize<LargestPaintOp>();
