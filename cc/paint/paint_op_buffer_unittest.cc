...
std::unique_ptr<char, base::AlignedFreeDeleter> AllocateSerializedBuffer(size_t size = kDefaultSerializedBufferSize)
{
    return PaintOpWriter::AllocateAlignedBuffer(size);
}

bool ReadAndValidateOpHeader(const void* memory, size_t size, uint8_t* op_type, size_t* serialized_size)
{
    return PaintOpReader(memory, size, TestOptionsProvider().deserialize_options()).ReadAndValidateOpHeader(op_type, serialized_size);
}
...
public:
    static void FillArbitraryShaderValues(PaintShader* shader, bool use_matrix)
    {
        ...
        shader->colors_ = { { 0.1f, 0.2f, 0.3f, 0.4f }, { 0.05f, 0.15f, 0.25f, 0.35f }, { 0.0f, 0.5f, 0.9f, 0.1f } };
...
public:
    ...
    {
        EXPECT_THAT(*buffer,
            ElementsAre(PaintOpEq<SaveLayerOp>(rect_, flags_), PaintOpEq<SaveOp>(), PaintOpEq<DrawColorOp>(draw_color_, blend_), PaintOpEq<RestoreOp>()));
    }
...
TEST(PaintOpBufferTest, SaveDrawTextBlobRestore)
{
    ...
    buffer.push<DrawTextBlobOp>(SkTextBlob::MakeFromString("abc", font), 0.0f, 0.0f, paint_flags);
    buffer.push<RestoreOp>();
...
TEST(PaintOpBufferTest, SaveSaveLayerFiltersRestore)
{
    ...
    PaintFlags paint_flags;
    EXPECT_TRUE(paint_flags.SupportsFoldingAlpha());
    buffer.push<SaveLayerFiltersOp>(std::vector<sk_sp<PaintFilter>> {}, paint_flags);
    buffer.push<RestoreOp>();
    buffer.push<RestoreOp>();
...
TEST(PaintOpBufferTest, SaveLayerRestore_DrawColor)
{
    ...
    buffer.push<SaveLayerAlphaOp>(alpha);
    buffer.push<DrawColorOp>(SkColor4f::FromColor(original), SkBlendMode::kSrcOver);
    buffer.push<RestoreOp>();
...
{
    scoped_refptr<TestPaintWorkletInput> input = base::MakeRefCounted<TestPaintWorkletInput>(gfx::SizeF(32.0f, 32.0f));
    PaintOpBuffer buffer;
...
TEST(PaintOpBufferTest, DiscardableImagesTracking_PaintWorkletImageRect)
{
    ...
    buffer.push<DrawImageRectOp>(image, src, dst, SkCanvas::kStrict_SrcRectConstraint);
    EXPECT_TRUE(buffer.has_discardable_images());
}
...
TEST(PaintOpBufferTest, DiscardableImagesTracking_DrawImageRect)
{
    PaintOpBuffer buffer;
    PaintImage image = CreateDiscardablePaintImage(gfx::Size(100, 100));
    buffer.push<DrawImageRectOp>(image, SkRect::MakeWH(100, 100), SkRect::MakeWH(100, 100), SkCanvas::kFast_SrcRectConstraint);
    EXPECT_TRUE(buffer.has_discardable_images());
}
...
TEST(PaintOpBufferTest, DiscardableImagesTracking_OpWithFlags)
{
    ...
    flags.setShader(PaintShader::MakeImage(std::move(image), SkTileMode::kClamp, SkTileMode::kClamp, nullptr));
    buffer.push<DrawRectOp>(SkRect::MakeWH(100, 100), flags);
    EXPECT_TRUE(buffer.has_discardable_images());
}
...
TEST(PaintOpBufferTest, SlowPaths)
{
    ...
    buffer.push<ClipPathOp>(path, SkClipOp::kIntersect, /*antialias=*/true, UsePaintCache::kDisabled);
    EXPECT_EQ(buffer.num_slow_paths_up_to_min_for_MSAA(), 1);
    ...
    buffer.push<ClipPathOp>(concave, SkClipOp::kIntersect, /*antialias=*/true, UsePaintCache::kDisabled);
    EXPECT_EQ(buffer.num_slow_paths_up_to_min_for_MSAA(), 2);
    buffer.push<ClipPathOp>(concave, SkClipOp::kIntersect, /*antialias=*/false, UsePaintCache::kDisabled);
    EXPECT_EQ(buffer.num_slow_paths_up_to_min_for_MSAA(), 2);
...
TEST(PaintOpBufferTest, NonAAPaint)
{
    ...
    // ClipPathOp
    {
        ...
        buffer.push<ClipPathOp>(path, SkClipOp::kIntersect, /*antialias=*/false, UsePaintCache::kDisabled);
        EXPECT_TRUE(buffer.has_non_aa_paint());
    }
    ...
    // ClipRRectOp
    {
        ...
        // ClipRRectOp with AA
        buffer.push<ClipRRectOp>(SkRRect::MakeEmpty(), SkClipOp::kIntersect, true /* antialias */);
        EXPECT_FALSE(buffer.has_non_aa_paint());

        // ClipRRectOp without AA
        buffer.push<ClipRRectOp>(SkRRect::MakeEmpty(), SkClipOp::kIntersect, false /* antialias */);
        EXPECT_TRUE(buffer.has_non_aa_paint());
    }
    ...
    // DrawImageOp
    {
        ...
        buffer.push<DrawImageOp>(image, SkIntToScalar(0), SkIntToScalar(0), SkSamplingOptions(), &non_aa_flags);
...
public:
    ...
    template <typename T, typename... Args> void push_op(Args&&... args)
    {
    ...
    {
        buffer_.Playback(canvas, PlaybackParams(nullptr), /*local_ctm=*/true, &offsets);
    }
...
TEST_F(PaintOpBufferOffsetsTest, EmptyClipRectShouldRejectAnOp)
{
    ...
    push_op<DrawImageRectOp>(image, src, dst, SkCanvas::kStrict_SrcRectConstraint);
    std::vector<size_t> offsets = Select({ 0 });
    for (PaintOpBuffer::PlaybackFoldingIterator iter(buffer_, &offsets); iter; ++iter) {
...
TEST_F(PaintOpBufferOffsetsTest, NonContiguousIndicesWithSaveLayerAlphaRestore)
{
    testing::StrictMock<MockCanvas> canvas;
...
TEST_F(PaintOpBufferOffsetsTest, ContiguousIndicesWithSaveLayerAlphaDrawRestore)
{
    testing::StrictMock<MockCanvas> canvas;
...
TEST_F(PaintOpBufferOffsetsTest, NonContiguousIndicesWithSaveLayerAlphaDrawRestore)
{
    testing::StrictMock<MockCanvas> canvas;
...
std::vector<float> test_floats
    = { 0.f, 1.f, -1.f, 2384.981971f, 0.0001f, std::numeric_limits<float>::min(), std::numeric_limits<float>::max(), std::numeric_limits<float>::infinity() };

std::vector<float> test_angles = { 0.f, 1.f, 0.f, -1.f, 1.f, 3.14, 0, 6.24, 0.f, 0.f };
...
std::vector<SkRect> test_rects = {
    SkRect::MakeXYWH(1, 2.5, 3, 4),
    SkRect::MakeXYWH(0, 0, 0, 0),
    make_largest_skrect(),
    SkRect::MakeXYWH(0.5f, 0.5f, 8.2f, 8.2f),
    SkRect::MakeXYWH(-1, -1, 0, 0),
    SkRect::MakeXYWH(-100, -101, -102, -103),
    SkRect::MakeXYWH(0, 0, 0, 0),
    SkRect::MakeXYWH(0, 0, 0, 0),
    SkRect::MakeXYWH(0, 0, 0, 0),
    SkRect::MakeXYWH(0, 0, 0, 0),
...
std::vector<SkRRect> test_rrects = {
    SkRRect::MakeEmpty(),
    SkRRect::MakeOval(SkRect::MakeXYWH(1, 2, 3, 4)),
...
std::vector<SkIRect> test_irects = { SkIRect::MakeXYWH(1, 2, 3, 4), SkIRect::MakeXYWH(0, 0, 0, 0), make_largest_skirect(), SkIRect::MakeXYWH(0, 0, 10, 10),
...
std::vector<PaintFlags> test_flags = {
    ...
    [] {
        ...
        flags.setColorFilter(ColorFilter::MakeBlend({ 0.4f, 0.3f, 0.2f, 0.1f }, SkBlendMode::kMultiply));
        ...
        PaintOpSerializationTestUtils::FillArbitraryShaderValues(shader.get(), true);
    ...
    [] {
        PaintFlags flags;
        sk_sp<PaintShader> shader = PaintShader::MakeColor(SkColors::kTransparent);
        PaintOpSerializationTestUtils::FillArbitraryShaderValues(shader.get(), false);
        flags.setShader(std::move(shader));
    ...
    [] {
        ...
        SkColor4f colors[3] = { { 0.1f, 0.2f, 0.3f, 0.4f }, { 0.4f, 0.3f, 0.2f, 0.1f }, { 0.2f, 0.4f, 0.6f, 0.0f } };
        SkScalar positions[3] = { 0.f, 0.3f, 1.f };
        flags.setShader(PaintShader::MakeLinearGradient(points, colors, positions, 3, SkTileMode::kMirror));
    ...
    [] {
        PaintFlags flags;
        SkColor4f colors[3] = { { 0.1f, 0.2f, 0.3f, 0.4f }, { 0.4f, 0.3f, 0.2f, 0.1f }, { 0.2f, 0.4f, 0.6f, 0.0f } };
        flags.setShader(PaintShader::MakeSweepGradient(0.2f, -0.8f, colors, nullptr, 3, SkTileMode::kMirror, 10, 20));
        return flags;
    }(),
    PaintFlags(),
    PaintFlags(),
...
std::vector<std::vector<SkPoint>> test_point_arrays = {
    ...
    { SkPoint::Make(0, 0), SkPoint::Make(5, 6), SkPoint::Make(-1, -1), SkPoint::Make(9, 9), SkPoint::Make(50, 50), SkPoint::Make(100, 100) },
};
...
public:
    ...
    void Serialize(const PaintOpBuffer& buffer)
    {
        ...
        for (const PaintOp& op : buffer) {
            size_t bytes_written = op.Serialize(current_, remaining_, options_provider_.serialize_options(), nullptr, SkM44(), SkM44());
            if (!bytes_written)
                return;

            uint8_t type = 0;
            size_t bytes_to_read = 0;
            EXPECT_TRUE(ReadAndValidateOpHeader(current_, remaining_, &type, &bytes_to_read));
            ...
            // entirely.
            if (remaining_ != 0u) {
                EXPECT_EQ(bytes_written, base::bits::AlignUp(bytes_written, PaintOpWriter::kMaxAlignment));
            }
        }
    }

    const std::vector<size_t>& bytes_written() const
    {
        return bytes_written_;
    }
    size_t TotalBytesWritten() const
    {
        return output_size_ - remaining_;
    }
...
public:
    DeserializerIterator(const void* input, size_t input_size, const PaintOp::DeserializeOptions& options)
        : DeserializerIterator(input, static_cast<const char*>(input), input_size, input_size, options)
    {
    }
    ...
    {
        return DeserializerIterator(input_, static_cast<const char*>(input_), input_size_, input_size_, options_);
    }
    DeserializerIterator end()
    {
        return DeserializerIterator(input_, static_cast<const char*>(input_) + input_size_, input_size_, 0, options_);
    }
    bool operator!=(const DeserializerIterator& other)
    {
        return input_ != other.input_ || current_ != other.current_ || input_size_ != other.input_size_ || remaining_ != other.remaining_;
    }
    ...
    const PaintOp* operator->() const
    {
        return deserialized_op_;
    }
    const PaintOp& operator*() const
    {
        return *deserialized_op_;
...
private:
    DeserializerIterator(const void* input, const char* current, size_t input_size, size_t remaining, const PaintOp::DeserializeOptions& options)
        : input_(input)
        , current_(current)
    ...
    void DeserializeCurrentOp()
    {
        ...
        if (!remaining_)
            return;
        deserialized_op_ = PaintOp::Deserialize(current_, remaining_, paint_op_buffer_data_, std::size(paint_op_buffer_data_), &last_bytes_read_, options_);
    }
    ...
    alignas(PaintOpBuffer::kPaintOpAlign) char paint_op_buffer_data_[kLargestPaintOpAlignedSize];
    raw_ptr<PaintOp> deserialized_op_ = nullptr;
};

void PushAnnotateOps(PaintOpBuffer* buffer)
{
    buffer->push<AnnotateOp>(PaintCanvas::AnnotationType::kUrl, test_rects[0], SkData::MakeWithCString("thingerdoowhatchamagig"));
    // Deliberately test both null and empty SkData.
    buffer->push<AnnotateOp>(PaintCanvas::AnnotationType::kLinkToDestination, test_rects[1], nullptr);
    buffer->push<AnnotateOp>(PaintCanvas::AnnotationType::kNameDestination, test_rects[2], SkData::MakeEmpty());
...
void PushClipPathOps(PaintOpBuffer* buffer)
{
    for (size_t i = 0; i < test_paths.size(); ++i) {
        ...
        buffer->push<ClipPathOp>(test_paths[i], op, /*antialias=*/!!(i % 2), UsePaintCache::kDisabled);
    }
...
void PushDrawDRRectOps(PaintOpBuffer* buffer)
{
    ...
    for (size_t i = 0; i < len; ++i) {
        buffer->push<DrawDRRectOp>(test_rrects[i], test_rrects[i + 1], test_flags[i]);
    }
...
{
    size_t len = std::min({ test_images.size(), test_flags.size(), test_floats.size() - 1 });
    for (size_t i = 0; i < len; ++i) {
        buffer->push<DrawImageOp>(
            test_images[i], test_floats[i], test_floats[i + 1], PaintFlags::FilterQualityToSkSamplingOptions(test_flags[i].getFilterQuality()), &test_flags[i]);
    }
...
{
    size_t len = std::min({ test_images.size(), test_flags.size(), test_rects.size() - 1 });
    for (size_t i = 0; i < len; ++i) {
        SkCanvas::SrcRectConstraint constraint = i % 2 ? SkCanvas::kStrict_SrcRectConstraint : SkCanvas::kFast_SrcRectConstraint;
        buffer->push<DrawImageRectOp>(test_images[i], test_rects[i], test_rects[i + 1],
            PaintFlags::FilterQualityToSkSamplingOptions(test_flags[i].getFilterQuality()), &test_flags[i], constraint);
    }

    // Test optional flags.
    buffer->push<DrawImageRectOp>(test_images[0], test_rects[0], test_rects[1], SkCanvas::kStrict_SrcRectConstraint);
...
void PushDrawLineOps(PaintOpBuffer* buffer)
{
    ...
    for (size_t i = 0; i < len; ++i) {
        buffer->push<DrawLineOp>(test_floats[i], test_floats[i + 1], test_floats[i + 2], test_floats[i + 3], test_flags[i]);
    }
...
void PushDrawLineLiteOps(PaintOpBuffer* buffer)
{
    ...
    for (size_t i = 0; i < len; ++i) {
        if (test_flags[i].CanConvertToCorePaintFlags()) {
            buffer->push<DrawLineLiteOp>(test_floats[i], test_floats[i + 1], test_floats[i + 2], test_floats[i + 3], test_flags[i].ToCorePaintFlags());
        } else {
            buffer->push<DrawLineOp>(test_floats[i], test_floats[i + 1], test_floats[i + 2], test_floats[i + 3], test_flags[i]);
        }
    }
...
{
    size_t len = std::min({ test_rects.size(), test_angles.size() / 2, test_flags.size() });
    for (size_t i = 0; i < len; ++i) {
        buffer->push<DrawArcOp>(test_rects[i], test_angles[2 * i], test_angles[2 * i + 1], test_flags[i]);
    }
...
void PushDrawArcLiteOps(PaintOpBuffer* buffer)
{
    ...
    for (size_t i = 0; i < len; ++i) {
        if (test_flags[i].CanConvertToCorePaintFlags()) {
            buffer->push<DrawArcLiteOp>(test_rects[i], test_angles[2 * i], test_angles[2 * i + 1], test_flags[i].ToCorePaintFlags());
        } else {
            buffer->push<DrawArcOp>(test_rects[i], test_angles[2 * i], test_angles[2 * i + 1], test_flags[i]);
        }
    }
...
void PushDrawPathOps(PaintOpBuffer* buffer)
{
    ...
    for (size_t i = 0; i < len; ++i)
        buffer->push<DrawPathOp>(test_paths[i], test_flags[i], UsePaintCache::kDisabled);
...
void PushDrawVerticesOps(PaintOpBuffer* buffer)
{
    ...
    std::vector<VerticesTest> test_vertices = {
        ...
        { .verts = { { 0, 0 }, { 0, 100 } }, .uvs = { { 0, 0 }, { 0, 1 } }, .indices = { 0, 1, 0 } },
        { .verts = { { 0, 0 }, { 0, 100 }, { 100, 100 } }, .uvs = { { 0, 0 }, { 0, 1 }, { 1, 1 } }, .indices = { 0, 1, 2 } },
    };

    size_t len = std::min(test_vertices.size(), test_flags.size());
    for (size_t i = 0; i < len; ++i) {
        buffer->push<DrawVerticesOp>(base::MakeRefCounted<RefCountedBuffer<SkPoint>>(test_vertices[i].verts),
            base::MakeRefCounted<RefCountedBuffer<SkPoint>>(test_vertices[i].uvs), base::MakeRefCounted<RefCountedBuffer<uint16_t>>(test_vertices[i].indices),
            test_flags[i]);
    }
...
SkottieFrameDataMap GetTestImagesForSkottie(SkottieWrapper& skottie, const SkRect& skottie_rect, PaintFlags::FilterQuality quality, float t)
{
    SkottieFrameDataMap images;
    skottie.Seek(t, base::BindLambdaForTesting([&](SkottieResourceIdHash asset_id, float t_frame, sk_sp<SkImage>& image_out, SkSamplingOptions& sampling_out) {
        SkottieFrameData frame_data;
        frame_data.image = CreateBitmapImage(gfx::Size(skottie_rect.width() / 2, skottie_rect.height() / 2));
...
void PushDrawSkottieOps(PaintOpBuffer* buffer)
{
    ...
    if (kIsSkottieSupported) {
        test_skotties = { CreateSkottie(gfx::Size(10, 20), 4), CreateSkottie(gfx::Size(100, 40), 5), CreateSkottie(gfx::Size(80, 70), 6),
            CreateSkottieFromString(kLottieDataWith2Assets), CreateSkottieFromString(kLottieDataWith2Assets),
            CreateSkottieFromTestDataDir(kLottieDataWith2TextFileName) };
        test_skottie_floats = { 0, 0.1f, 1.f, 0.2f, 0.2f, 0.3f };
        test_skottie_rects = { SkRect::MakeXYWH(10, 20, 30, 40), SkRect::MakeXYWH(0, 5, 10, 20), SkRect::MakeXYWH(6, 0, 3, 50),
            SkRect::MakeXYWH(10, 10, 100, 100), SkRect::MakeXYWH(10, 10, 100, 100), SkRect::MakeXYWH(5, 5, 50, 50) };
        test_skottie_images = { SkottieFrameDataMap(), SkottieFrameDataMap(), SkottieFrameDataMap(),
            GetTestImagesForSkottie(*test_skotties[3], test_skottie_rects[3], PaintFlags::FilterQuality::kHigh, test_skottie_floats[3]),
            GetNullImagesForSkottie(*test_skotties[4], test_skottie_floats[4]), SkottieFrameDataMap() };
        test_skottie_color_maps = { { SkottieMapColor("green", SK_ColorGREEN), SkottieMapColor("yellow", SK_ColorYELLOW), SkottieMapColor("red", SK_ColorRED),
                                        SkottieMapColor("blue", SK_ColorBLUE) },
            {}, { SkottieMapColor("green", SK_ColorGREEN) }, { SkottieMapColor("transparent", SK_ColorTRANSPARENT) }, {}, {} };
        test_skottie_text_maps = { {}, {}, {}, {}, {},
            { { HashSkottieResourceId(kLottieDataWith2TextNode1),
                  SkottieTextPropertyValue(std::string(kLottieDataWith2TextNode1Text.data()), kLottieDataWith2TextNode1Box) },
                { HashSkottieResourceId(kLottieDataWith2TextNode2),
                    SkottieTextPropertyValue(std::string(kLottieDataWith2TextNode2Text.data()), kLottieDataWith2TextNode2Box) } } };
    }

    size_t len = std::min(test_skotties.size(), test_flags.size());
    for (size_t i = 0; i < len; i++) {
        buffer->push<DrawSkottieOp>(
            test_skotties[i], test_skottie_rects[i], test_skottie_floats[i], test_skottie_images[i], test_skottie_color_maps[i], test_skottie_text_maps[i]);
    }
...
void PushDrawTextBlobOps(PaintOpBuffer* buffer)
{
    static std::vector<std::vector<sk_sp<SkTypeface>>> test_typefaces = {
        ...
        [] {
            return std::vector<sk_sp<SkTypeface>> { skia::DefaultTypeface(), skia::DefaultTypeface() };
        }(),
    };
    ...
    size_t len = std::min({ test_paint_blobs.size(), test_flags.size(), test_floats.size() - 1 });
    for (size_t i = 0; i < len; ++i) {
        buffer->push<DrawTextBlobOp>(test_paint_blobs[i], test_floats[i], test_floats[i + 1], test_flags[i]);
    }
...
void PushSaveLayerFiltersOps(PaintOpBuffer* buffer)
{
    size_t len = std::min(test_flags.size(), test_rects.size());
    for (size_t i = 0; i < len; ++i) {
        sk_sp<PaintFilter> filter = sk_make_sp<OffsetPaintFilter>(-1.f, -2.f, nullptr);
...
public:
    ...
    {
        // TODO(crbug.com/40223786): fix the test for kDrawTextBlobs
        if (GetParamType() == PaintOpType::kDrawTextBlob || GetParamType() == PaintOpType::kDrawSlug) {
            return false;
        }
        ...
        // in PushTestOps.
        return GetParamType() != PaintOpType::kDrawRecord && GetParamType() != PaintOpType::kDrawScrollingContents
...
sk_sp<PaintOpBuffer> SerializeAndDeserialize(const PaintOpBuffer& buffer, const PaintOpBufferSerializer::Preamble* preamble = nullptr)
{
    auto memory = AllocateSerializedBuffer();
    TestOptionsProvider options_provider;
    SimpleBufferSerializer serializer(memory.get(), kDefaultSerializedBufferSize, options_provider.serialize_options());
    if (preamble) {
    ...
    sk_sp<PaintOpBuffer> deserialized_buffer = PaintOpBuffer::MakeFromMemory(memory.get(), serializer.written(), options_provider.deserialize_options());
    return deserialized_buffer;
}

INSTANTIATE_TEST_SUITE_P(P, PaintOpSerializationTest, ::testing::Range(static_cast<uint8_t>(0), static_cast<uint8_t>(PaintOpType::kLastPaintOpType)));
...
TEST_P(PaintOpSerializationTest, SmokeTest)
{
    ...
    auto canvas = serializer.options_provider()->strike_server()->makeAnalysisCanvas(1024, 768, {}, nullptr, true);
    ...
    if (!strike_data.empty()) {
        serializer.options_provider()->strike_client()->readStrikeData(strike_data.data(), strike_data.size());
    }
    serializer.Serialize(buffer_);

    // Expect all ops to write more than 0 bytes.
    for (size_t i = 0; i < buffer_.size(); ++i) {
        SCOPED_TRACE(base::StringPrintf("%s #%zd", PaintOpTypeToString(GetParamType()).c_str(), i));
    ...
    for (const PaintOp& base_written :
        DeserializerIterator(output_.get(), serializer.TotalBytesWritten(), serializer.options_provider()->deserialize_options())) {
        ASSERT_TRUE(iter);
...
TEST_P(PaintOpSerializationTest, SerializationFailures)
{
    ...
    // to CorePaintFlags.
    if (!IsTypeSupported() || GetParamType() == PaintOpType::kDrawArcLite || GetParamType() == PaintOpType::kDrawLineLite) {
        return;
    }
    ...
    for (const PaintOp& op : buffer_) {
        SCOPED_TRACE(base::StringPrintf("%s #%zu", PaintOpTypeToString(GetParamType()).c_str(), op_idx));
        size_t expected_bytes = bytes_written[op_idx];
        EXPECT_GT(expected_bytes, 0u);
        EXPECT_EQ(expected_bytes, base::bits::AlignUp(expected_bytes, PaintOpWriter::kMaxAlignment));
        ...
        for (size_t i = 0; i < bytes_written[op_idx] + 2; ++i) {
            ...
            size_t written_bytes = op.Serialize(output_.get(), i, options_provider.serialize_options(), nullptr, SkM44(), SkM44());
            if (i >= expected_bytes) {
...
TEST_P(PaintOpSerializationTest, DeserializationFailures)
{
    ...
    for (size_t op_idx = 0; op_idx < buffer_.size(); ++op_idx) {
        ...
        ReadAndValidateOpHeader(current, PaintOpWriter::kHeaderBytes, &serialized_type, &serialized_size);
        ...
        for (size_t read_size = 0; read_size < serialized_size + kAlign * 2 + 2; ++read_size) {
            SCOPED_TRACE(base::StringPrintf("%s #%zd, read_size: %zu, align: %zu, serialized_size: %zu", PaintOpTypeToString(GetParamType()).c_str(), op_idx,
                read_size, kAlign, serialized_size));
            ...
            size_t bytes_read = 0;
            PaintOp* written
                = PaintOp::Deserialize(current, read_size, deserialize_buffer.get(), kOutputOpSize, &bytes_read, options_provider->deserialize_options());
            ...
            // corrupted op.
            auto deserialized_buffer = PaintOpBuffer::MakeFromMemory(first, total_read + read_size, options_provider->deserialize_options());
        ...
        PaintOpWriter::WriteHeaderForTesting(current, serialized_type, serialized_size);
...
TEST_P(PaintOpSerializationTest, UsesOverridenFlags)
{
    ...
    TestOptionsProvider options_provider;
    alignas(PaintOpBuffer::kPaintOpAlign) char deserialized[kLargestPaintOpAlignedSize];
    for (const PaintOp& op : buffer_) {
        size_t bytes_written = op.Serialize(output_.get(), output_size_, options_provider.serialize_options(), nullptr, SkM44(), SkM44());
        size_t bytes_read = 0u;
        PaintOp* written
            = PaintOp::Deserialize(output_.get(), bytes_written, deserialized, std::size(deserialized), &bytes_read, options_provider.deserialize_options());
        ...
        bytes_written = op.Serialize(output_.get(), output_size_, options_provider.serialize_options(), &override_flags, SkM44(), SkM44());
        written
            = PaintOp::Deserialize(output_.get(), bytes_written, deserialized, std::size(deserialized), &bytes_read, options_provider.deserialize_options());
        ASSERT_TRUE(written);
        ASSERT_TRUE(written->IsPaintOpWithFlags());
        EXPECT_EQ(static_cast<const PaintOpWithFlags*>(written)->flags.getAlphaf(), override_flags.getAlphaf());
        written->DestroyThis();
        written = nullptr;
    }
}
...
TEST(PaintOpSerializationTest, CompleteBufferSerialization)
{
    ...
    EXPECT_THAT(SerializeAndDeserialize(buffer, &preamble),
        Pointee(ElementsAre(
            // Preamble:
            PaintOpEq<SaveOp>(), PaintOpIs<DrawColorOp>(), PaintOpIs<ClipRectOp>(),
            // Serialized buffer:
...
TEST(PaintOpSerializationTest, Preamble)
{
    ...
    EXPECT_THAT(SerializeAndDeserialize(buffer, &preamble),
        Pointee(ElementsAre(
            // Preamble:
            PaintOpEq<SaveOp>(), PaintOpEq<TranslateOp>(-preamble.full_raster_rect.x(), -preamble.full_raster_rect.y()),
            PaintOpEq<ClipRectOp>(RectToSkRect(preamble.playback_rect), SkClipOp::kIntersect, /*antialias=*/false),
            PaintOpEq<TranslateOp>(preamble.post_translation.x(), preamble.post_translation.y()),
            PaintOpEq<ScaleOp>(preamble.post_scale.x(), preamble.post_scale.y()), PaintOpEq<DrawColorOp>(SkColors::kTransparent, SkBlendMode::kSrc),
...
TEST(PaintOpSerializationTest, ConvertToDrawSlugWhenSerializationAndRasterization)
{
    PaintOpBuffer buffer;
    ...
    std::unique_ptr<char, base::AlignedFreeDeleter> output = AllocateSerializedBuffer(output_size);
    ...
    size_t i = 0;
    for (const PaintOp& base_written :
        DeserializerIterator(output.get(), serializer.TotalBytesWritten(), serializer.options_provider()->deserialize_options())) {
        ASSERT_TRUE(iter);
...
TEST(PaintOpSerializationTest, SerializesNestedRecords)
{
    ...
    EXPECT_THAT(SerializeAndDeserialize(buffer),
        Pointee(ElementsAre(PaintOpEq<SaveOp>(),
            // From the serialized buffer:
            PaintOpEq<ScaleOp>(0.5f, 0.75f), PaintOpEq<DrawRectOp>(SkRect::MakeWH(10.f, 20.f), PaintFlags()),
            // End restore:
            PaintOpEq<RestoreOp>())));
}
...
TEST(PaintOpBufferTest, ClipsImagesDuringSerialization)
{
    struct {
        ...
        gfx::Rect image_rect;
        bool should_draw;
    } test_cases[] = { { gfx::Rect(0, 0, 100, 100), gfx::Rect(50, 50, 100, 100), true }, { gfx::Rect(0, 0, 100, 100), gfx::Rect(105, 105, 100, 100), false },
        { gfx::Rect(0, 0, 500, 500), gfx::Rect(450, 450, 100, 100), true }, { gfx::Rect(0, 0, 500, 500), gfx::Rect(750, 750, 100, 100), false },
        { gfx::Rect(250, 250, 250, 250), gfx::Rect(450, 450, 100, 100), true }, { gfx::Rect(250, 250, 250, 250), gfx::Rect(50, 50, 100, 100), false },
        { gfx::Rect(0, 0, 100, 500), gfx::Rect(250, 250, 100, 100), false }, { gfx::Rect(0, 0, 200, 500), gfx::Rect(100, 250, 100, 100), true } };

    for (const auto& test_case : test_cases) {
        PaintOpBuffer buffer;
        buffer.push<DrawImageOp>(CreateDiscardablePaintImage(test_case.image_rect.size()), static_cast<SkScalar>(test_case.image_rect.x()),
            static_cast<SkScalar>(test_case.image_rect.y()));

        PaintOpBufferSerializer::Preamble preamble;
        preamble.playback_rect = test_case.clip_rect;
        preamble.full_raster_rect = gfx::Rect(0, 0, test_case.clip_rect.right(), test_case.clip_rect.bottom());
        // Avoid clearing.
        preamble.content_size = gfx::Size(1000, 1000);
        ...
        EXPECT_THAT(SerializeAndDeserialize(buffer, &preamble), Pointee(ElementsAreArray(matchers)));
    }
}
    ...
    EXPECT_THAT(SerializeAndDeserialize(buffer, &preamble),
        Pointee(ElementsAre(PaintOpIs<SaveOp>(), PaintOpIs<ClipRectOp>(),
            AllOf(PaintOpIs<DrawRectOp>(),
                ...
                ResultOf([](const PaintOp& op) { return static_cast<const DrawRectOp&>(op).flags.getAlphaf(); }, FloatEq(alpha * draw_rect_alpha))),
            PaintOpIs<RestoreOp>())));
}
...
TEST(PaintOpBufferTest, PaintOpDeserialize)
{
    ...
    TestOptionsProvider options_provider;
    size_t bytes_written = op->Serialize(input.get(), kSerializedBytesPerOp, options_provider.serialize_options(), nullptr, SkM44(), SkM44());
    ASSERT_GT(bytes_written, 0u);

    // Can deserialize from exactly the right size.
    size_t bytes_read = 0;
    PaintOp* success = PaintOp::Deserialize(input.get(), bytes_written, output, std::size(output), &bytes_read, options_provider.deserialize_options());
    ASSERT_TRUE(success);
    ...
    for (size_t i = 0; i < bytes_written - 1; ++i) {
        EXPECT_FALSE(PaintOp::Deserialize(input.get(), i, output, std::size(output), &bytes_read, options_provider.deserialize_options()));
    }

    // Unaligned serialized_size should fail to deserialize.
    uint8_t serialized_type = 0;
    size_t serialized_size = 0;
    ReadAndValidateOpHeader(input.get(), bytes_written, &serialized_type, &serialized_size);
    EXPECT_EQ(serialized_size, base::bits::AlignUp(serialized_size, PaintOpWriter::kMaxAlignment));
    PaintOpWriter::WriteHeaderForTesting(input.get(), serialized_type, serialized_size - 1);
    EXPECT_FALSE(PaintOp::Deserialize(input.get(), bytes_written, output, std::size(output), &bytes_read, options_provider.deserialize_options()));

    // Bogus types fail to deserialize.
    PaintOpWriter::WriteHeaderForTesting(input.get(), static_cast<uint8_t>(PaintOpType::kLastPaintOpType) + 1, serialized_size);
    EXPECT_FALSE(PaintOp::Deserialize(input.get(), bytes_written, output, std::size(output), &bytes_read, options_provider.deserialize_options()));
}
...
TEST(PaintOpBufferTest, ValidateRects)
{
    ...
    PaintOpBuffer buffer;
    buffer.push<AnnotateOp>(PaintCanvas::AnnotationType::kUrl, rect, SkData::MakeWithCString("test1"));
    buffer.push<ClipRectOp>(rect, SkClipOp::kDifference, true);

    buffer.push<DrawImageRectOp>(test_images[0], rect, test_rects[1], SkCanvas::kStrict_SrcRectConstraint);
    buffer.push<DrawImageRectOp>(test_images[0], test_rects[0], rect, SkCanvas::kStrict_SrcRectConstraint);
    ...
    int op_idx = 0;
    for (const PaintOp& op : buffer) {
        size_t bytes_written = op.Serialize(serialized.get(), kSerializedBytesPerOp, options_provider.serialize_options(), nullptr, SkM44(), SkM44());
        ASSERT_GT(bytes_written, sizeof(float));

        size_t bytes_read = 0;
        PaintOp* deserialized_op
            = PaintOp::Deserialize(serialized.get(), bytes_written, deserialized, std::size(deserialized), &bytes_read, options_provider.deserialize_options());
...
{
    SkPath path;
    ClipPathOp good(path, SkClipOp::kMax_EnumValue, /*antialias=*/true, UsePaintCache::kDisabled);
    EXPECT_TRUE(good.IsValid());

    SkClipOp bad_clip = static_cast<SkClipOp>(static_cast<uint32_t>(SkClipOp::kMax_EnumValue) + 1);
...
{
    for (uint8_t i = 0; i < static_cast<uint8_t>(SkBlendMode::kLastMode) + 1; i++) {
...
TEST(PaintOpBufferTest, BoundingRect_DrawImageOp)
{
    ...
    for (const PaintOp& base_op : buffer) {
        ...
        SkRect image_rect = SkRect::MakeXYWH(op.left, op.top, op.image.width(), op.image.height());
...
public:
    ...
    MockImageProvider(std::vector<SkSize> src_rect_offset, std::vector<SkSize> scale, std::vector<PaintFlags::FilterQuality> quality)
    ...
    ImageProvider::ScopedResult GetRasterContent(const DrawImage& draw_image) override
    {
        ...
        SkBitmap bitmap;
        bitmap.allocPixelsFlags(SkImageInfo::MakeN32Premul(10, 10), SkBitmap::kZeroPixels_AllocFlag);
        sk_sp<SkImage> image = SkImages::RasterFromBitmap(bitmap);
        size_t i = index_++;
        return ScopedResult(DecodedDrawImage(image, nullptr, src_rect_offset_[i], scale_[i], quality_[i], true));
    }

    void SetRecord(PaintRecord record)
    {
...
TEST(PaintOpBufferTest, SkipsOpsOutsideClip)
{
    ...
    buffer.push<ClipRectOp>(SkRect::MakeXYWH(0, 0, 100, 100), SkClipOp::kIntersect, false);
    ...
    image_flags.setShader(PaintShader::MakeImage(paint_image, SkTileMode::kRepeat, SkTileMode::kRepeat, nullptr));
...
TEST(PaintOpBufferTest, RasterPaintWorkletImageRectBasicCase)
{
    ...
    paint_worklet_buffer.push<SaveLayerOp>(SkRect::MakeXYWH(0, 0, 100, 100), PaintFlags());
    PaintFlags draw_flags;
    ...
    blink_buffer.push<DrawImageRectOp>(image, src, dst, SkCanvas::kStrict_SrcRectConstraint);
...
{
    PaintOpBuffer paint_worklet_buffer;
    paint_worklet_buffer.push<SaveLayerOp>(SkRect::MakeXYWH(0, 0, 10, 10), PaintFlags());
    PaintImage paint_image = CreateDiscardablePaintImage(gfx::Size(10, 10));
    paint_worklet_buffer.push<DrawImageOp>(paint_image, 0.0f, 0.0f, SkSamplingOptions(SkFilterMode::kLinear), nullptr);

    std::vector<SkSize> src_rect_offset = { SkSize::MakeEmpty() };
    std::vector<SkSize> scale_adjustment = { SkSize::Make(0.2f, 0.2f) };
    std::vector<PaintFlags::FilterQuality> quality = { PaintFlags::FilterQuality::kLow };
    ...
    EXPECT_CALL(canvas, OnSaveLayer());
    EXPECT_CALL(canvas, willSave());
    EXPECT_CALL(canvas, didScale(1.0f / scale_adjustment[0].width(), 1.0f / scale_adjustment[0].height()));
    EXPECT_CALL(canvas, onDrawImageRect2(NonLazyImage(), SkRect::MakeWH(10, 10), SkRect::MakeWH(10, 10), sampling, _, SkCanvas::kFast_SrcRectConstraint));
    EXPECT_CALL(canvas, willRestore());
    EXPECT_CALL(canvas, willRestore());
    EXPECT_CALL(canvas, willRestore());
    EXPECT_CALL(canvas, willRestore());
...
TEST(PaintOpBufferTest, RasterPaintWorkletImageRectScaled)
{
    ...
    std::vector<SkSize> src_rect_offset = { SkSize::MakeEmpty() };
    std::vector<SkSize> scale_adjustment = { SkSize::Make(0.2f, 0.2f) };
    std::vector<PaintFlags::FilterQuality> quality = { PaintFlags::FilterQuality::kHigh };
    MockImageProvider provider(src_rect_offset, scale_adjustment, quality);
    provider.SetRecord(paint_worklet_buffer.ReleaseAsRecord());
...
{
    PaintOpBuffer paint_worklet_buffer;
    paint_worklet_buffer.push<SaveLayerOp>(SkRect::MakeXYWH(0, 0, 60, 60), PaintFlags());
    SkSamplingOptions linear(SkFilterMode::kLinear);
    PaintImage paint_image = CreateDiscardablePaintImage(gfx::Size(10, 10));
    // One rect inside the src-rect, one outside.
    paint_worklet_buffer.push<DrawImageOp>(paint_image, 0.0f, 0.0f, linear, nullptr);
    paint_worklet_buffer.push<DrawImageOp>(paint_image, 50.0f, 50.0f, linear, nullptr);
...
{
    std::vector<SkSize> src_rect_offset = { SkSize::MakeEmpty(), SkSize::Make(2.0f, 2.0f), SkSize::Make(3.0f, 3.0f) };
    std::vector<SkSize> scale_adjustment = { SkSize::Make(3.0f, 3.0f), SkSize::Make(0.3f, 0.3f), SkSize::Make(0.4f, 0.4f) };
    std::vector<PaintFlags::FilterQuality> quality = { PaintFlags::FilterQuality::kHigh, PaintFlags::FilterQuality::kMedium, PaintFlags::FilterQuality::kLow };
    ...
    buffer.push<DrawImageRectOp>(paint_image, rect, rect, sampling, nullptr, SkCanvas::kFast_SrcRectConstraint);
    PaintFlags flags;
    flags.setShader(PaintShader::MakeImage(paint_image, SkTileMode::kRepeat, SkTileMode::kRepeat, nullptr));
    ...
    EXPECT_CALL(canvas, onDrawImageRect2(NonLazyImage(), SkRect::MakeWH(10, 10), SkRect::MakeWH(10, 10), sampling1, _, SkCanvas::kFast_SrcRectConstraint))
        .InSequence(on_draw_image_rect_seq);
    EXPECT_CALL(canvas, willRestore()).InSequence(save_restore_seq);

    // DrawImageRectop.
    SkRect src_rect = rect.makeOffset(src_rect_offset[1].width(), src_rect_offset[1].height());
    EXPECT_CALL(canvas,
        onDrawImageRect2(NonLazyImage(), MatchesRect(src_rect, scale_adjustment[1]), SkRect::MakeWH(10, 10), sampling1, _, SkCanvas::kFast_SrcRectConstraint))
        .InSequence(on_draw_image_rect_seq);

    // DrawOvalop.
    EXPECT_CALL(canvas, onDrawOval(SkRect::MakeWH(10, 10), MatchesShader(flags, scale_adjustment[2])));
...
TEST(PaintOpBufferTest, DrawImageRectOpWithLooperNoImageProvider)
{
    ...
    buffer.push<DrawImageRectOp>(
        image, SkRect::MakeWH(100, 100), SkRect::MakeWH(100, 100), SkSamplingOptions(), &paint_flags, SkCanvas::kFast_SrcRectConstraint);
...
TEST(PaintOpBufferTest, ReplacesImagesFromProviderOOP)
{
    ...
    for (const PaintOp& op : *deserialized_buffer) {
        ...
        if (op.GetType() == PaintOpType::kDrawImage) {
            // Save/scale/image/restore from DrawImageop.
            EXPECT_CALL(canvas, willSave());
            EXPECT_CALL(canvas, didScale(1.0f / expected_scale.width(), 1.0f / expected_scale.height()));
            EXPECT_CALL(canvas, onDrawImageRect2(NonLazyImage(), SkRect::MakeWH(10, 10), SkRect::MakeWH(10, 10), _, _, SkCanvas::kFast_SrcRectConstraint));
            EXPECT_CALL(canvas, willRestore());
            op.Raster(&canvas, params);
        } else if (op.GetType() == PaintOpType::kDrawImageRect) {
            EXPECT_CALL(
                canvas, onDrawImageRect2(NonLazyImage(), MatchesRect(rect, expected_scale), SkRect::MakeWH(10, 10), _, _, SkCanvas::kFast_SrcRectConstraint));
            op.Raster(&canvas, params);
        } else if (op.GetType() == PaintOpType::kDrawOval) {
            EXPECT_CALL(canvas, onDrawOval(SkRect::MakeWH(10, 10), MatchesShader(flags, expected_scale)));
            op.Raster(&canvas, params);
        }
    }
}
...
INSTANTIATE_TEST_SUITE_P(PaintFilterSerializationTests, PaintFilterSerializationTest, ::testing::Values(true, false));

TEST_P(PaintFilterSerializationTest, Basic)
{
    SkScalar scalars[9] = { 1.f, 2.f, 3.f, 4.f, 5.f, 6.f, 7.f, 8.f, 9.f };
    std::vector<sk_sp<PaintFilter>> filters = { sk_sp<PaintFilter> { new ColorFilterPaintFilter(ColorFilter::MakeLinearToSRGBGamma(), nullptr) },
        sk_sp<PaintFilter> { new BlurPaintFilter(0.5f, 0.3f, SkTileMode::kRepeat, nullptr) },
        sk_sp<PaintFilter> { new DropShadowPaintFilter(5.f, 10.f, 0.1f, 0.3f, SkColors::kBlue, DropShadowPaintFilter::ShadowMode::kDrawShadowOnly, nullptr) },
        sk_sp<PaintFilter> { new MagnifierPaintFilter(SkRect::MakeXYWH(5, 6, 7, 8), 2.f, 10.5f, nullptr) },
        sk_sp<PaintFilter> { new AlphaThresholdPaintFilter(SkRegion(SkIRect::MakeXYWH(0, 0, 100, 200)), nullptr) },
        sk_sp<PaintFilter> {
            new MatrixConvolutionPaintFilter(SkISize::Make(3, 3), scalars, 30.f, 123.f, SkIPoint::Make(0, 0), SkTileMode::kDecal, true, nullptr) },
        sk_sp<PaintFilter> { new MorphologyPaintFilter(MorphologyPaintFilter::MorphType::kErode, 15.5f, 30.2f, nullptr) },
        sk_sp<PaintFilter> { new OffsetPaintFilter(-1.f, -2.f, nullptr) },
        sk_sp<PaintFilter> { new TilePaintFilter(SkRect::MakeXYWH(1, 2, 3, 4), SkRect::MakeXYWH(4, 3, 2, 1), nullptr) },
        sk_sp<PaintFilter> { new TurbulencePaintFilter(TurbulencePaintFilter::TurbulenceType::kFractalNoise, 3.3f, 4.4f, 2, 123, nullptr) },
        sk_sp<PaintFilter> { new MatrixPaintFilter(SkMatrix::I(), PaintFlags::FilterQuality::kHigh, nullptr) },
        sk_sp<PaintFilter> {
            new LightingDistantPaintFilter(PaintFilter::LightingType::kSpecular, SkPoint3::Make(1, 2, 3), SkColors::kCyan, 1.1f, 2.2f, 3.3f, nullptr) },
        sk_sp<PaintFilter> {
            new LightingPointPaintFilter(PaintFilter::LightingType::kDiffuse, SkPoint3::Make(2, 3, 4), SkColors::kRed, 1.2f, 3.4f, 5.6f, nullptr) },
        sk_sp<PaintFilter> { new LightingSpotPaintFilter(
            PaintFilter::LightingType::kSpecular, SkPoint3::Make(100, 200, 300), SkPoint3::Make(400, 500, 600), 1, 2, SkColors::kMagenta, 3, 4, 5, nullptr) },
        sk_sp<PaintFilter> { new ImagePaintFilter(
            CreateDiscardablePaintImage(gfx::Size(100, 100)), SkRect::MakeWH(50, 50), SkRect::MakeWH(70, 70), PaintFlags::FilterQuality::kMedium) } };

    filters.emplace_back(new ComposePaintFilter(filters[0], filters[1]));
    filters.emplace_back(new XfermodePaintFilter(SkBlendMode::kDst, filters[2], filters[3]));
    filters.emplace_back(new ArithmeticPaintFilter(1.1f, 2.2f, 3.3f, 4.4f, false, filters[4], filters[5]));
    filters.emplace_back(new DisplacementMapEffectPaintFilter(SkColorChannel::kR, SkColorChannel::kG, 10, filters[6], filters[7]));
    filters.emplace_back(new MergePaintFilter(filters));
    filters.emplace_back(new RecordPaintFilter(PaintRecord(), SkRect::MakeXYWH(10, 15, 20, 25)));
    ...
    for (size_t i = 0; i < filters.size(); ++i) {
        ...
        size_t buffer_size = filter->type() == PaintFilter::Type::kPaintRecord ? kDefaultSerializedBufferSize : PaintOpWriter::SerializedSize(filter.get());
        auto memory = AllocateSerializedBuffer(buffer_size);

        PaintOpWriter writer(memory.get(), buffer_size, options_provider.serialize_options(), GetParam());
        ...
        PaintOpReader reader(memory.get(), writer.size(), options_provider.deserialize_options(), GetParam());
        ...
        if (filter->type() == PaintFilter::Type::kPaintRecord) {
            ...
            const RecordPaintFilter& expected = static_cast<const RecordPaintFilter&>(*filter);
            const RecordPaintFilter& actual = static_cast<const RecordPaintFilter&>(*deserialized_filter);

            EXPECT_EQ(actual.scaling_behavior(), RecordPaintFilter::ScalingBehavior::kFixedScale);

            SkRect expected_bounds = SkRect::MakeXYWH(scale_x * expected.record_bounds().x(), scale_y * expected.record_bounds().y(),
                scale_x * expected.record_bounds().width(), scale_y * expected.record_bounds().height());
            ...
            if (!GetParam()) {
                EXPECT_THAT(actual.record(), ElementsAre(PaintOpEq<ScaleOp>(scale_x, scale_y)));
            }
        } else {
...
TEST(PaintOpBufferTest, RecordPaintFilterDeserializationInvalidPaintOp)
{
    float rect_size = 0x8.765432p1;
    PaintOpBuffer buffer;
    buffer.push<ClipRectOp>(SkRect::MakeWH(rect_size, rect_size), SkClipOp::kDifference, true);
    auto filter = sk_make_sp<RecordPaintFilter>(buffer.ReleaseAsRecord(), SkRect::MakeWH(100, 100));
    ...
    PaintOpWriter writer(memory.get(), memory_size, options_provider.serialize_options(), false);
    ...
    sk_sp<PaintFilter> deserialized_filter;
    PaintOpReader reader(memory.get(), writer.size(), options_provider.deserialize_options(), false);
    reader.Read(&deserialized_filter);
...
TEST(PaintOpBufferTest, PaintRecordShaderSerialization)
{
    ...
    PaintFlags flags;
    flags.setShader(PaintShader::MakePaintRecord(shader_buffer.ReleaseAsRecord(), SkRect::MakeWH(10, 10), SkTileMode::kClamp, SkTileMode::kRepeat, nullptr));
    PaintOpBuffer buffer;
    buffer.push<DrawRectOp>(SkRect::MakeXYWH(1, 2, 3, 4), flags);

    EXPECT_THAT(SerializeAndDeserialize(buffer), Pointee(ElementsAre(PaintOpEq<DrawRectOp>(SkRect::MakeXYWH(1, 2, 3, 4), flags))));
}
...
TEST(PaintOpBufferTest, DrawSkottieOpSerializationFailureFromUnPrivilegedProcess)
{
    auto memory = AllocateSerializedBuffer();

    scoped_refptr<SkottieWrapper> skottie = CreateSkottie(gfx::Size(100, 100), /*duration_secs=*/1);
    ASSERT_TRUE(skottie->is_valid());
    const SkRect input_rect = SkRect::MakeIWH(400, 300);
    const float input_t = 0.4f;

    PaintOpBuffer buffer;
    buffer.push<DrawSkottieOp>(skottie, input_rect, input_t, GetTestImagesForSkottie(*skottie, input_rect, PaintFlags::FilterQuality::kHigh, input_t),
    ...
    auto deserialized_buffer = PaintOpBuffer::MakeFromMemory(memory.get(), serializer.written(), d_options);
    ASSERT_FALSE(deserialized_buffer);
}

TEST(PaintOpBufferTest, DrawSkottieOpRasterWithoutImageAssets)
{
    scoped_refptr<SkottieWrapper> skottie = CreateSkottie(gfx::Size(100, 100), /*duration_secs=*/5);
    SkRect skottie_rect = SkRect::MakeWH(100, 100);
    DrawSkottieOp skottie_op(skottie, skottie_rect, /*t=*/0.1,
        /*images=*/SkottieFrameDataMap(), SkottieColorMap(), SkottieTextPropertyValueMap());
...
{
    scoped_refptr<SkottieWrapper> skottie = CreateSkottieFromString(kLottieDataWith2Assets);
    SkRect skottie_rect = SkRect::MakeWH(100, 100);

    SkottieFrameDataMap images_in = GetNullImagesForSkottie(*skottie, /*t=*/0.1f);
    ASSERT_FALSE(images_in.empty());
    DrawSkottieOp skottie_op(skottie, skottie_rect, /*t=*/0.1, images_in, SkottieColorMap(), SkottieTextPropertyValueMap());
    PlaybackParams playback_params(/*image_provider=*/nullptr);
    {
        NiceMock<MockCanvas> canvas;
        EXPECT_CALL(canvas, onDrawImageRect2(_, _, _, _, _, _)).Times(0);
...
TEST(PaintOpBufferTest, DrawSkottieOpRasterWithoutImageProvider)
{
    scoped_refptr<SkottieWrapper> skottie = CreateSkottieFromString(kLottieDataWith2Assets);
    SkRect skottie_rect = SkRect::MakeWH(100, 100);
    SkottieFrameDataMap images_in = GetTestImagesForSkottie(*skottie, skottie_rect, PaintFlags::FilterQuality::kHigh, /*t=*/0.1f);
    ASSERT_FALSE(images_in.empty());
    DrawSkottieOp skottie_op(skottie, skottie_rect, /*t=*/0.1, images_in, SkottieColorMap(), SkottieTextPropertyValueMap());
    PlaybackParams playback_params(/*image_provider=*/nullptr);
    {
        ...
        EXPECT_CALL(canvas, onDrawImageRect2(NotNull(), _, _, _, _, _)).Times(AtLeast(1));
        DrawSkottieOp::Raster(&skottie_op, &canvas, playback_params);
    }
}
...
TEST(PaintOpBufferTest, DrawSkottieOpRasterWithImageProvider)
{
    scoped_refptr<SkottieWrapper> skottie = CreateSkottieFromString(kLottieDataWith2Assets);
    SkRect skottie_rect = SkRect::MakeWH(100, 100);
    std::vector<SkSize> src_rect_offset = { SkSize::Make(2.0f, 2.0f), SkSize::Make(3.0f, 3.0f) };
    std::vector<SkSize> scale_adjustment = { SkSize::Make(0.2f, 0.2f), SkSize::Make(0.3f, 0.3f) };
    std::vector<PaintFlags::FilterQuality> quality = { PaintFlags::FilterQuality::kHigh, PaintFlags::FilterQuality::kMedium };

    MockImageProvider image_provider(src_rect_offset, scale_adjustment, quality);
    PlaybackParams playback_params(&image_provider);
    ASSERT_TRUE(image_provider.decoded_images().empty());
    {
        SkottieFrameDataMap images_in = GetTestImagesForSkottie(*skottie, skottie_rect, PaintFlags::FilterQuality::kHigh, /*t=*/0.25f);
        ASSERT_THAT(images_in, Contains(Key(HashSkottieResourceId("image_0"))));
        DrawSkottieOp skottie_op(skottie, skottie_rect, /*t=*/0.25, images_in, SkottieColorMap(), SkottieTextPropertyValueMap());
        NiceMock<MockCanvas> canvas;
        EXPECT_CALL(canvas, onDrawImageRect2(NotNull(), _, _, _, _, _)).Times(AtLeast(1));
        DrawSkottieOp::Raster(&skottie_op, &canvas, playback_params);
        ASSERT_EQ(image_provider.decoded_images().size(), 1u);
        EXPECT_THAT(image_provider.decoded_images(), Contains(MatchesPaintImage(images_in.at(HashSkottieResourceId("image_0")).image)));
    }
    {
        SkottieFrameDataMap images_in = GetTestImagesForSkottie(*skottie, skottie_rect, PaintFlags::FilterQuality::kHigh, /*t=*/0.75f);
        ASSERT_THAT(images_in, Contains(Key(HashSkottieResourceId("image_1"))));
        DrawSkottieOp skottie_op(skottie, skottie_rect, /*t=*/0.75, images_in, SkottieColorMap(), SkottieTextPropertyValueMap());
        NiceMock<MockCanvas> canvas;
        EXPECT_CALL(canvas, onDrawImageRect2(NotNull(), _, _, _, _, _)).Times(AtLeast(1));
        DrawSkottieOp::Raster(&skottie_op, &canvas, playback_params);
        ASSERT_EQ(image_provider.decoded_images().size(), 2u);
        EXPECT_THAT(image_provider.decoded_images(), Contains(MatchesPaintImage(images_in.at(HashSkottieResourceId("image_1")).image)));
    }
}
...
{
    PaintOpBuffer buffer;
    buffer.push<DrawSkottieOp>(CreateSkottie(gfx::Size(100, 100), /*duration_secs=*/1),
        /*dst=*/SkRect::MakeWH(100, 100), /*t=*/0.1f, SkottieFrameDataMap(), SkottieColorMap(), SkottieTextPropertyValueMap());
    EXPECT_FALSE(buffer.has_discardable_images());
}
...
TEST(PaintOpBufferTest, DiscardableImagesTrackingSkottieOpWithImages)
{
    ...
    SkottieFrameDataMap images_in = GetTestImagesForSkottie(*skottie, skottie_rect, PaintFlags::FilterQuality::kHigh, /*t=*/0.1f);
    ASSERT_FALSE(images_in.empty());
    buffer.push<DrawSkottieOp>(skottie, skottie_rect, /*t=*/0.1f, images_in, SkottieColorMap(), SkottieTextPropertyValueMap());
    EXPECT_TRUE(buffer.has_discardable_images());
}
...
TEST(PaintOpBufferTest, OpHasDiscardableImagesSkottieOpWithImages)
{
    ...
    SkottieFrameDataMap images_in = GetTestImagesForSkottie(*skottie, skottie_rect, PaintFlags::FilterQuality::kHigh, /*t=*/0.1f);
    ASSERT_FALSE(images_in.empty());
    DrawSkottieOp op(skottie, skottie_rect, /*t=*/0.1f, images_in, SkottieColorMap(), SkottieTextPropertyValueMap());
...
TEST(PaintOpBufferTest, CustomData)
{
    // Basic tests: size, move, comparison.
    {
        ...
        EXPECT_GT(buffer.bytes_used(), sizeof(PaintOpBuffer) + sizeof(CustomDataOp));
    ...
    // Push and verify.
    {
        ...
        EXPECT_THAT(buffer, ElementsAre(PaintOpEq<SaveOp>(), PaintOpEq<CustomDataOp>(0xFFFFFFFF), PaintOpEq<RestoreOp>()));
    }
    ...
    // Playback.
    {
        ...
        callbacks.custom_callback = base::BindRepeating(&MockCanvas::onCustomCallback, base::Unretained(&canvas));
...
TEST(PaintOpBufferTest, SecurityConstrainedImageSerialization)
{
    ...
    sk_sp<PaintFilter> filter = sk_make_sp<ImagePaintFilter>(image, SkRect::MakeWH(10, 10), SkRect::MakeWH(10, 10), PaintFlags::FilterQuality::kLow);
    const bool enable_security_constraints = true;

    auto memory = AllocateSerializedBuffer();
    TestOptionsProvider options_provider;
    PaintOpWriter writer(memory.get(), kDefaultSerializedBufferSize, options_provider.serialize_options(), enable_security_constraints);
    writer.Write(filter.get(), SkM44());

    sk_sp<PaintFilter> out_filter;
    PaintOpReader reader(memory.get(), writer.size(), options_provider.deserialize_options(), enable_security_constraints);
...
TEST(PaintOpBufferTest, DrawImageRectSerializeScaledImages)
{
    ...
    buffer.push<DrawImageRectOp>(CreateDiscardablePaintImage(gfx::Size(32, 16)), src, dst, SkCanvas::kStrict_SrcRectConstraint);
    auto memory = AllocateSerializedBuffer();
    TestOptionsProvider options_provider;
    SimpleBufferSerializer serializer(memory.get(), kDefaultSerializedBufferSize, options_provider.serialize_options());
    serializer.Serialize(buffer);
...
{
    PaintOpBuffer shader_buffer;
    shader_buffer.push<DrawImageOp>(CreateDiscardablePaintImage(gfx::Size(10, 10)), 0.f, 0.f);

    auto shader = PaintShader::MakePaintRecord(shader_buffer.ReleaseAsRecord(), SkRect::MakeWH(10.f, 10.f), SkTileMode::kRepeat, SkTileMode::kRepeat, nullptr);
...
TEST(PaintOpBufferTest, RecordShadersCached)
{
    ...
    size_t memory_scaled_written = 0;
    {
        ...
        SimpleBufferSerializer serializer(memory_scaled.get(), kDefaultSerializedBufferSize, options_provider.serialize_options());
        serializer.Serialize(buffer);
    ...
    PaintOp::DeserializeOptions deserialize_options { .transfer_cache = transfer_cache,
    ...
    for (size_t i = 0; i < 4; ++i) {
        if (i < 2) {
            buffers[i] = PaintOpBuffer::MakeFromMemory(memory.get(), memory_written, deserialize_options);
        } else {
            buffers[i] = PaintOpBuffer::MakeFromMemory(memory_scaled.get(), memory_scaled_written, deserialize_options);
        }

        auto* entry = transfer_cache->GetEntryAs<ServiceShaderTransferCacheEntry>(shader_id);
        ASSERT_TRUE(entry);
        if (i < 2)
...
TEST(PaintOpBufferTest, RecordShadersCachedSize)
{
    ...
    PaintOp::DeserializeOptions deserialize_options { .transfer_cache = transfer_cache,
        ...
        .scratch_buffer = scratch_buffer,
        .is_privileged = true };
    auto deserialized = PaintOpBuffer::MakeFromMemory(memory.get(), serializer.written(), deserialize_options);
    auto* shader_entry = transfer_cache->GetEntryAs<ServiceShaderTransferCacheEntry>(shader_id);
...
TEST(PaintOpBufferTest, RecordFilterSerializeScaledImages)
{
    ...
    filter_buffer.push<DrawImageOp>(CreateDiscardablePaintImage(gfx::Size(10, 10)), 0.f, 0.f);

    auto filter = sk_make_sp<RecordPaintFilter>(filter_buffer.ReleaseAsRecord(), SkRect::MakeWH(10.f, 10.f));
    PaintOpBuffer buffer;
    buffer.push<ScaleOp>(0.5f, 0.8f);
    PaintFlags flags;
...
TEST(PaintOpBufferTest, NullImages)
{
    ...
    EXPECT_THAT(SerializeAndDeserialize(buffer), Pointee(ElementsAre(PaintOpEq<DrawImageOp>(PaintImage(), 0.f, 0.f))));
}
...
TEST(PaintOpBufferTest, HasDrawOpsAndHasDrawTextOps)
{
    ...
    EXPECT_FALSE(buffer2.has_draw_text_ops());
    SkFont font = skia::DefaultFont();
    buffer2.push<DrawTextBlobOp>(SkTextBlob::MakeFromString("abc", font), 0.0f, 0.0f, PaintFlags());
    EXPECT_TRUE(buffer2.has_draw_ops());
...
TEST(PaintOpBufferTest, NeedsAdditionalInvalidationForLCDText)
{
    ...
    PaintRecord record2 = buffer2.ReleaseAsRecord();
    {
        ...
        EXPECT_FALSE(record2.buffer().NeedsAdditionalInvalidationForLCDText(buffer3));
        EXPECT_FALSE(buffer3.NeedsAdditionalInvalidationForLCDText(record2.buffer()));
    }
    {
        buffer1.push<DrawTextBlobOp>(SkTextBlob::MakeFromString("abc", font), 0.0f, 0.0f, PaintFlags());
        ...
        EXPECT_TRUE(record1.buffer().NeedsAdditionalInvalidationForLCDText(buffer3));
        EXPECT_TRUE(buffer3.NeedsAdditionalInvalidationForLCDText(record1.buffer()));
...
TEST(PaintOpBufferTest, PlaybackSetMatrixWithLocalCTM)
{
    ...
    EXPECT_CALL(canvas, didSetM44(SkM44(SkM44::Scale(1, 2), SkM44::Translate(3, 4))));
    EXPECT_CALL(canvas, willRestore());
...
TEST(PaintOpBufferTest, PlaybackDrawRecordWithLocalCTM)
{
    ...
    EXPECT_THAT(SerializeAndDeserialize(buffer),
        Pointee(ElementsAre(PaintOpEq<ScaleOp>(1, 2), //
            PaintOpEq<SaveOp>(), //
            PaintOpEq<SetMatrixOp>(SkM44(SkM44::Scale(1, 2), SkM44::Translate(3, 4))), PaintOpEq<RestoreOp>())));
}
...
TEST(PaintOpBufferTest, PlaybackDrawRecordWithNonLocalCTM)
{
    ...
    EXPECT_THAT(SerializeAndDeserialize(buffer), Pointee(ElementsAre(PaintOpEq<ScaleOp>(1, 2), PaintOpEq<SetMatrixOp>(SkM44::Translate(3, 4)))));
}
...
TEST(PaintOpBufferTest, PlaybackDrawRecordNestedLocalAndNonLocalCTM)
{
    ...
    EXPECT_CALL(canvas, willSave());
    EXPECT_CALL(canvas, didSetM44(SkM44(SkM44::Translate(1, 2), SkM44::Scale(3, 4))));
    EXPECT_CALL(canvas, didSetM44(SkM44(SkM44::Translate(1, 2), SkM44::Scale(5, 6))));
    EXPECT_CALL(canvas, willRestore());
    ...
    EXPECT_THAT(SerializeAndDeserialize(buffer),
        Pointee(ElementsAre(PaintOpEq<TranslateOp>(1, 2), //
            PaintOpEq<SaveOp>(), //
            PaintOpEq<SetMatrixOp>(SkM44(SkM44::Translate(1, 2), SkM44::Scale(3, 4))),
            PaintOpEq<SetMatrixOp>(SkM44(SkM44::Translate(1, 2), SkM44::Scale(5, 6))), PaintOpEq<RestoreOp>())));
}
...
TEST(PaintOpBufferTest, PlaybackDrawRecordNestedNonLocalAndNonLocalCTM)
{
    ...
    EXPECT_THAT(SerializeAndDeserialize(buffer),
        Pointee(ElementsAre(PaintOpEq<TranslateOp>(1, 2), PaintOpEq<SetMatrixOp>(SkM44::Scale(3, 4)), PaintOpEq<SetMatrixOp>(SkM44::Scale(5, 6)))));
}
...
TEST(PaintOpBufferTest, PathCaching)
{
    ...
    EXPECT_TRUE(options_provider.client_paint_cache()->Get(PaintCacheDataType::kPath, path.getGenerationID()));

    sk_sp<PaintOpBuffer> deserialized_buffer = PaintOpBuffer::MakeFromMemory(memory.get(), serializer.written(), options_provider.deserialize_options());
    EXPECT_THAT(*deserialized_buffer, ElementsAre(PaintOpIs<DrawPathOp>()));

    SkPath cached_path;
    EXPECT_TRUE(options_provider.service_paint_cache()->GetPath(path.getGenerationID(), &cached_path));
}
...
TEST(IteratorTest, StlContainerLikeIterationTest)
{
    ...
    EXPECT_THAT(buffer, ElementsAre(PaintOpEq<SaveOp>(), PaintOpEq<SetMatrixOp>(SkM44::Scale(1, 2))));
}
...
TEST(IteratorTest, IterationTest)
{
    ...
    buffer.push<SaveOp>();
    buffer.push<SetMatrixOp>(SkM44::Scale(1, 2));
    EXPECT_THAT(PaintOpBuffer::Iterator(buffer), ElementsAre(PaintOpEq<SaveOp>(), PaintOpEq<SetMatrixOp>(SkM44::Scale(1, 2))));
}
...
TEST(IteratorTest, OffsetIterationTest)
{
    ...
    std::vector<size_t> offsets = { 0, static_cast<size_t>(op1.AlignedSize() + op2.AlignedSize()) };
    EXPECT_THAT(PaintOpBuffer::OffsetIterator(buffer, offsets), ElementsAre(PaintOpEq<SaveOp>(), PaintOpEq<SetMatrixOp>(SkM44::Scale(1, 2))));
}
...
TEST(IteratorTest, CompositeIterationTest)
{
    ...
    EXPECT_THAT(PaintOpBuffer::CompositeIterator(buffer, /*offsets=*/nullptr),
        ElementsAre(PaintOpEq<SaveOp>(), PaintOpEq<RestoreOp>(), PaintOpEq<SetMatrixOp>(SkM44::Scale(1, 2))));

    EXPECT_THAT(PaintOpBuffer::CompositeIterator(buffer, &offsets), ElementsAre(PaintOpEq<SaveOp>(), PaintOpEq<SetMatrixOp>(SkM44::Scale(1, 2))));
}
...
TEST(PaintOpBufferTest, ContentColorUsageFromImages)
{
    ...
    PaintImage srgb = CreateDiscardablePaintImage(gfx::Size(1, 1), gfx::ColorSpace::CreateSRGB().ToSkColorSpace());
    ...
    PaintImage wcg = CreateDiscardablePaintImage(gfx::Size(1, 1), gfx::ColorSpace::CreateDisplayP3D65().ToSkColorSpace());
    ASSERT_EQ(gfx::ContentColorUsage::kWideColorGamut, wcg.GetContentColorUsage());
    buffer.push<DrawImageOp>(wcg, 0.f, 0.f);
    EXPECT_EQ(gfx::ContentColorUsage::kWideColorGamut, buffer.content_color_usage());

    // Adding a HDR image should switch to HDR.
    PaintOpBuffer sub_buffer;
    PaintImage hdr = CreateDiscardablePaintImage(gfx::Size(1, 1), gfx::ColorSpace::CreateHDR10().ToSkColorSpace());
    ASSERT_EQ(gfx::ContentColorUsage::kHDR, hdr.GetContentColorUsage());
    sub_buffer.push<DrawImageRectOp>(hdr, SkRect::MakeWH(10, 10), SkRect::MakeWH(20, 20), SkCanvas::kStrict_SrcRectConstraint);
...
TEST(PaintOpBufferTest, ContentColorUsageFromShader)
{
    PaintOpBuffer buffer1;
    PaintFlags flags1;
    PaintImage wcg = CreateDiscardablePaintImage(gfx::Size(2, 2), gfx::ColorSpace::CreateDisplayP3D65().ToSkColorSpace());
    ASSERT_EQ(gfx::ContentColorUsage::kWideColorGamut, wcg.GetContentColorUsage());
    auto shader1 = PaintShader::MakeImage(wcg, SkTileMode::kClamp, SkTileMode::kClamp, nullptr);
    flags1.setShader(std::move(shader1));
    buffer1.push<DrawRectOp>(SkRect::MakeWH(10, 20), flags1);
    EXPECT_EQ(gfx::ContentColorUsage::kWideColorGamut, buffer1.content_color_usage());

    PaintOpBuffer buffer2;
    PaintFlags flags2;
    auto shader2 = PaintShader::MakePaintRecord(buffer1.ReleaseAsRecord(), SkRect::MakeWH(2, 2), SkTileMode::kClamp, SkTileMode::kClamp, nullptr);
    flags2.setShader(std::move(shader2));
    buffer2.push<DrawRectOp>(SkRect::MakeWH(10, 20), flags2);
    EXPECT_EQ(gfx::ContentColorUsage::kWideColorGamut, buffer2.content_color_usage());
}
...
TEST(PaintOpBufferTest, ContentColorUsageFromFilter)
{
    ...
    PaintImage wcg = CreateDiscardablePaintImage(gfx::Size(2, 2), gfx::ColorSpace::CreateDisplayP3D65().ToSkColorSpace());
    ASSERT_EQ(gfx::ContentColorUsage::kWideColorGamut, wcg.GetContentColorUsage());
    auto filter1 = sk_make_sp<ImagePaintFilter>(wcg, SkRect::MakeWH(5, 5), SkRect::MakeWH(5, 5), PaintFlags::FilterQuality::kNone);
    ...
    PaintOpBuffer buffer2;
    PaintFlags flags2;
    auto filter2 = sk_make_sp<BlurPaintFilter>(0.1f, 0.2f, SkTileMode::kClamp, sk_make_sp<RecordPaintFilter>(buffer1.ReleaseAsRecord(), SkRect::MakeWH(5, 5)));
