...
TEST(PaintShaderTest, RasterizationRectForRecordShaders)
{
    ...
    auto record_shader = PaintShader::MakePaintRecord(PaintRecord(), SkRect::MakeWH(100, 100), SkTileMode::kClamp, SkTileMode::kClamp, &local_matrix);
...
TEST(PaintShaderTest, DecodePaintRecord)
{
    ...
    // ImageProvider.
    auto generator = sk_make_sp<testing::StrictMock<MockImageGenerator>>(gfx::Size(100, 100));
    ...
    SkMatrix local_matrix = SkMatrix::Scale(0.5f, 0.5f);
    auto record_shader
        = PaintShader::MakePaintRecord(shader_buffer.ReleaseAsRecord(), SkRect::MakeWH(100, 100), SkTileMode::kClamp, SkTileMode::kClamp, &local_matrix);
    ...
    EXPECT_TRUE(image_provider.draw_image().paint_image().IsSameForTesting(paint_image));
