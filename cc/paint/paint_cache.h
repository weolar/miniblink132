...
enum class PaintCacheEntryState : uint32_t { kEmpty, kCached, kInlined, kInlinedDoNotCache, kLast = kInlinedDoNotCache };

constexpr size_t PaintCacheDataTypeCount = static_cast<uint32_t>(PaintCacheDataType::kLast) + 1u;
...
private:
    ...
    template <typename Iterator> void EraseFromMap(Iterator it);
...
public:
    ...
    void Purge(PaintCacheDataType type, size_t n, const volatile PaintCacheId* ids);
    void PurgeAll();
    bool empty() const
    {
        return cached_paths_.empty();
    }
