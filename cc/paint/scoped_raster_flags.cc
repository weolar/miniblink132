...
{
    if (!flags()->HasShader() || flags()->getShader()->shader_type() != PaintShader::Type::kImage)
        return;

    PaintImage image = flags()->getShader()->paint_image();
    if (image.IsPaintWorklet()) {
        ImageProvider::ScopedResult result = decode_stashing_image_provider_->GetRasterContent(DrawImage(image));
        if (result && result.has_paint_record()) {
            const PaintShader* shader = flags()->getShader();
            SkMatrix local_matrix = shader->GetLocalMatrix();
            auto decoded_shader = PaintShader::MakePaintRecord(result.ReleaseAsRecord(), shader->tile(), shader->tx(), shader->tx(), &local_matrix);
    ...
    gpu::Mailbox mailbox;
    auto decoded_shader = flags()->getShader()->CreateDecodedImage(ctm, flags()->getFilterQuality(), &*decode_stashing_image_provider_,
        &transfer_cache_entry_id, &raster_quality, &transfer_cache_entry_needs_mips, &mailbox);
...
void ScopedRasterFlags::DecodeRecordShader(const SkMatrix& ctm, int max_texture_size)
{
    if (!flags()->HasShader() || flags()->getShader()->shader_type() != PaintShader::Type::kPaintRecord)
        return;
    ...
    if (flags()->getShader()->image_analysis_state() != ImageAnalysisState::kAnimatedImages) {
        return;
    }

    gfx::SizeF raster_scale(1.f, 1.f);
    auto decoded_shader = flags()->getShader()->CreateScaledPaintRecord(ctm, max_texture_size, &raster_scale);
    decoded_shader->ResolveSkObjects(&raster_scale, &*decode_stashing_image_provider_);
    MutableFlags()->setShader(std::move(decoded_shader));
}

void ScopedRasterFlags::DecodeFilter()
{
    if (!flags()->getImageFilter() || !flags()->getImageFilter()->has_discardable_images()
        || flags()->getImageFilter()->image_analysis_state() != ImageAnalysisState::kAnimatedImages) {
        return;
    }

    MutableFlags()->setImageFilter(flags()->getImageFilter()->SnapshotWithImages(&*decode_stashing_image_provider_));
}
...
void ScopedRasterFlags::AdjustStrokeIfNeeded(const SkMatrix& ctm)
{
    ...
    const auto stroke_vec = SkVector::Make(flags()->getStrokeWidth() * scale.width(), flags()->getStrokeWidth() * scale.height());
    if (stroke_vec.x() >= 1.f && stroke_vec.y() >= 1.f)
        return; // safe to raster

    const auto can_substitute_hairline = flags()->getStrokeCap() == PaintFlags::kDefault_Cap && flags()->getStrokeJoin() == PaintFlags::kDefault_Join;
    if (can_substitute_hairline && stroke_vec.x() < 1.f && stroke_vec.y() < 1.f) {
        ...
        MutableFlags()->setAlphaf(flags()->getAlphaf() * std::sqrt(stroke_vec.x() * stroke_vec.y()));
        return;
    }
