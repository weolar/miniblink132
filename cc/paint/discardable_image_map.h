...
class CC_PAINT_EXPORT DiscardableImageMap : public base::RefCounted<DiscardableImageMap> {
public:
    using Rects = absl::InlinedVector<gfx::Rect, 1>;

    struct CC_PAINT_EXPORT AnimatedImageMetadata {
        AnimatedImageMetadata(PaintImage::Id paint_image_id, PaintImage::CompletionState completion_state, std::vector<FrameMetadata> frames,
            int repetition_count, PaintImage::AnimationSequenceId reset_animation_sequence_id);
    ...
    static scoped_refptr<DiscardableImageMap> Generate(const PaintOpBuffer& paint_op_buffer, const gfx::Rect& bounds);
    ...
    std::vector<const DrawImage*> GetDiscardableImagesInRect(const gfx::Rect& rect) const;
    ...
    base::flat_map<PaintImage::Id, PaintImage::DecodingMode> TakeDecodingModeMap();

    using PaintWorkletInputWithImageId = std::pair<scoped_refptr<PaintWorkletInput>, PaintImage::Id>;
    const std::vector<PaintWorkletInputWithImageId>& paint_worklet_inputs() const
    {
