...
struct PositionScaleDrawImage {
    ...
    {
        *os << "image: <paint image> image_rect: " << img.image_rect.ToString() << " scale: " << gfx::SkSizeToSizeF(img.scale).ToString();
    }
};

PaintRecord CreateRecording(const PaintImage& discardable_image, const gfx::Rect& visible_rect)
{
...
public:
    std::vector<PositionScaleDrawImage> GetDiscardableImagesInRect(const DiscardableImageMap& image_map, const gfx::Rect& rect)
    {
        ...
        std::vector<const DrawImage*> draw_image_ptrs = image_map.GetDiscardableImagesInRect(rect);
        std::vector<DrawImage> draw_images;
        for (const auto* image : draw_image_ptrs)
            draw_images.push_back(DrawImage(*image, 1.f, PaintImage::kDefaultFrameIndex, target_color_params));
        ...
        for (const DrawImage* image : results) {
            ...
            position_draw_images.push_back(
                PositionScaleDrawImage { image->paint_image(), ImageRectsToRegion(image_map.GetRectsForImage(image_id)).bounds(), image->scale() });
        }

        EXPECT_EQ(draw_images.size(), position_draw_images.size());
        for (size_t i = 0; i < draw_images.size(); ++i) {
            EXPECT_TRUE(draw_images[i].paint_image().IsSameForTesting(position_draw_images[i].image));
            EXPECT_EQ(draw_images[i].target_color_space(), target_color_params.color_space);
        }
    ...
    std::vector<gfx::Rect> InsetImageRects(const std::vector<PositionScaleDrawImage>& images)
    {
...
TEST_F(DiscardableImageMapTest, GetDiscardableImagesInRectTest)
{
    ...
    for (int y = 0; y < 4; ++y) {
        for (int x = 0; x < 4; ++x) {
            if ((x + y) & 1) {
                discardable_image[y][x] = CreateDiscardablePaintImage(gfx::Size(500, 500));
                content_layer_client.add_draw_image(discardable_image[y][x], gfx::Point(x * 512 + 6, y * 512 + 6));
            }
        }
    }
    ...
    for (int y = 0; y < 4; ++y) {
        for (int x = 0; x < 4; ++x) {
            std::vector<PositionScaleDrawImage> images = GetDiscardableImagesInRect(*image_map, gfx::Rect(x * 512, y * 512, 500, 500));
            std::vector<gfx::Rect> inset_rects = InsetImageRects(images);
            if ((x + y) & 1) {
                EXPECT_EQ(1u, images.size()) << x << " " << y;
                EXPECT_TRUE(images[0].image.IsSameForTesting(discardable_image[y][x])) << x << " " << y;
                EXPECT_EQ(gfx::Rect(x * 512 + 6, y * 512 + 6, 500, 500), inset_rects[0]);
            } else {
...
TEST_F(DiscardableImageMapTest, GetDiscardableImagesInRectNonZeroLayer)
{
    ...
    for (int y = 0; y < 4; ++y) {
        for (int x = 0; x < 4; ++x) {
            if ((x + y) & 1) {
                discardable_image[y][x] = CreateDiscardablePaintImage(gfx::Size(500, 500));
                content_layer_client.add_draw_image(discardable_image[y][x], gfx::Point(1024 + x * 512 + 6, y * 512 + 6));
            }
        }
    }
    ...
    for (int y = 0; y < 4; ++y) {
        for (int x = 0; x < 4; ++x) {
            std::vector<PositionScaleDrawImage> images = GetDiscardableImagesInRect(*image_map, gfx::Rect(1024 + x * 512, y * 512, 500, 500));
            std::vector<gfx::Rect> inset_rects = InsetImageRects(images);
            if ((x + y) & 1) {
                EXPECT_EQ(1u, images.size()) << x << " " << y;
                EXPECT_TRUE(images[0].image.IsSameForTesting(discardable_image[y][x])) << x << " " << y;
                EXPECT_EQ(gfx::Rect(1024 + x * 512 + 6, y * 512 + 6, 500, 500), inset_rects[0]);
            } else {
                EXPECT_EQ(0u, images.size()) << x << " " << y;
            }
        }
    ...
    // Capture 4 pixel refs.
    {
        std::vector<PositionScaleDrawImage> images = GetDiscardableImagesInRect(*image_map, gfx::Rect(1024 + 512, 512, 2048, 2048));
        std::vector<gfx::Rect> inset_rects = InsetImageRects(images);
        EXPECT_EQ(4u, images.size());
        ...
        EXPECT_TRUE(images[2].image.IsSameForTesting(discardable_image[2][3]));
        EXPECT_EQ(gfx::Rect(1024 + 3 * 512 + 6, 2 * 512 + 6, 500, 500), inset_rects[2]);

        EXPECT_TRUE(images[3].image.IsSameForTesting(discardable_image[3][2]));
        EXPECT_EQ(gfx::Rect(1024 + 2 * 512 + 6, 3 * 512 + 6, 500, 500), inset_rects[3]);
    }
    ...
    {
        std::vector<PositionScaleDrawImage> images = GetDiscardableImagesInRect(*image_map, gfx::Rect(3500, 1100, 1000, 1000));
        EXPECT_EQ(0u, images.size());
    }
...
TEST_F(DiscardableImageMapTest, GetDiscardableImagesInRectOnePixelQuery)
{
    ...
    for (int y = 0; y < 4; ++y) {
        for (int x = 0; x < 4; ++x) {
            std::vector<PositionScaleDrawImage> images = GetDiscardableImagesInRect(*image_map, gfx::Rect(x * 512 + 256, y * 512 + 256, 1, 1));
            std::vector<gfx::Rect> inset_rects = InsetImageRects(images);
            if ((x + y) & 1) {
                EXPECT_EQ(1u, images.size()) << x << " " << y;
                EXPECT_TRUE(images[0].image.IsSameForTesting(discardable_image[y][x])) << x << " " << y;
...
TEST_F(DiscardableImageMapTest, GetDiscardableImagesInRectMassiveImage)
{
    ...
    PaintImage discardable_image = CreateDiscardablePaintImage(gfx::Size(1 << 25, 1 << 25), nullptr, false /* allocate_encoded_memory */);
...
{
    PaintFlags flags;
    SkRect rect = SkRect::MakeWH(static_cast<float>(INT_MAX), static_cast<float>(INT_MAX));
    scoped_refptr<DisplayItemList> display_list = new DisplayItemList;
    display_list->StartPaint();
...
TEST_F(DiscardableImageMapTest, RestoreSavedTransformedLayers)
{
    ...
    PaintImage discardable_image1 = CreateDiscardablePaintImage(gfx::Size(25, 25));
    PaintImage discardable_image2 = CreateDiscardablePaintImage(gfx::Size(25, 25));
    PaintImage discardable_image3 = CreateDiscardablePaintImage(gfx::Size(25, 25));
    ...
    scoped_refptr<DiscardableImageMap> image_map = display_list->GenerateDiscardableImageMap();
    std::vector<PositionScaleDrawImage> images = GetDiscardableImagesInRect(*image_map, gfx::Rect(0, 0, 200, 200));
...
TEST_F(DiscardableImageMapTest, NullPaintOnSaveLayer)
{
    ...
    scoped_refptr<DisplayItemList> display_list = new DisplayItemList;
    display_list->StartPaint();
    display_list->push<SaveLayerOp>(gfx::RectToSkRect(visible_rect), PaintFlags());
    display_list->push<DrawRecordOp>(std::move(record));
    display_list->push<RestoreOp>();
    display_list->EndPaintOfUnpaired(visible_rect);
    display_list->Finalize();
...
TEST_F(DiscardableImageMapTest, GetDiscardableImagesInRectMaxImage)
{
    ...
    PaintImage discardable_image = CreateDiscardablePaintImage(gfx::Size(dimension, dimension), no_color_space, false /* allocate_encoded_memory */);
...
TEST_F(DiscardableImageMapTest, GetDiscardableImagesInRectMaxImageMaxLayer)
{
    ...
    int dimension = static_cast<int>(static_cast<float>(std::numeric_limits<int>::max() - 64));
    gfx::Rect visible_rect(dimension, dimension);
    FakeContentLayerClient content_layer_client;
    content_layer_client.set_bounds(visible_rect.size());

    sk_sp<SkColorSpace> no_color_space;
    PaintImage discardable_image1 = CreateDiscardablePaintImage(gfx::Size(dimension, dimension), no_color_space, false /* allocate_encoded_memory */);
    PaintImage discardable_image2 = CreateDiscardablePaintImage(gfx::Size(dimension, dimension), no_color_space, false /* allocate_encoded_memory */);
    PaintImage discardable_image3 = CreateDiscardablePaintImage(gfx::Size(dimension, dimension), no_color_space, false /* allocate_encoded_memory */);
    ...
    content_layer_client.add_draw_image(discardable_image3, gfx::Point(-10000, 500));
...
TEST_F(DiscardableImageMapTest, GetDiscardableImagesRectInBounds)
{
    ...
    PaintImage discardable_image1 = CreateDiscardablePaintImage(gfx::Size(100, 100));
    PaintImage discardable_image2 = CreateDiscardablePaintImage(gfx::Size(100, 100));
    PaintImage long_discardable_image = CreateDiscardablePaintImage(gfx::Size(10000, 100));
    ...
    content_layer_client.add_draw_image(long_discardable_image, gfx::Point(-100, 500));
...
TEST_F(DiscardableImageMapTest, GetDiscardableImagesInShader)
{
    ...
    for (int y = 0; y < 4; ++y) {
        for (int x = 0; x < 4; ++x) {
            if ((x + y) & 1) {
                discardable_image[y][x] = PaintImageBuilder::WithDefault()
                                              .set_id(y * 4 + x)
                                              .set_paint_image_generator(CreatePaintImageGenerator(gfx::Size(500, 500)))
                                              .TakePaintImage();
                SkMatrix scale = SkMatrix::Scale(std::max(x * 0.5f, kMinScale), std::max(y * 0.5f, kMinScale));
                PaintFlags flags;
                flags.setShader(PaintShader::MakeImage(discardable_image[y][x], SkTileMode::kClamp, SkTileMode::kClamp, &scale));
                content_layer_client.add_draw_rect(gfx::Rect(x * 512 + 6, y * 512 + 6, 500, 500), flags);
            }
        }
    }
    ...
    for (int y = 0; y < 4; ++y) {
        for (int x = 0; x < 4; ++x) {
            std::vector<const DrawImage*> draw_images = image_map->GetDiscardableImagesInRect(gfx::Rect(x * 512, y * 512, 500, 500));
            if ((x + y) & 1) {
                EXPECT_EQ(1u, draw_images.size()) << x << " " << y;
                EXPECT_TRUE(draw_images[0]->paint_image().IsSameForTesting(discardable_image[y][x])) << x << " " << y;
                EXPECT_EQ(std::max(x * 0.5f, kMinScale), draw_images[0]->scale().fWidth);
                EXPECT_EQ(std::max(y * 0.5f, kMinScale), draw_images[0]->scale().fHeight);
            } else {
    ...
    // Capture 4 pixel refs.
    std::vector<const DrawImage*> draw_images = image_map->GetDiscardableImagesInRect(gfx::Rect(512, 512, 2048, 2048));
    EXPECT_EQ(4u, draw_images.size());
    EXPECT_TRUE(draw_images[0]->paint_image().IsSameForTesting(discardable_image[1][2]));
    EXPECT_TRUE(draw_images[1]->paint_image().IsSameForTesting(discardable_image[2][1]));
    EXPECT_TRUE(draw_images[2]->paint_image().IsSameForTesting(discardable_image[2][3]));
    EXPECT_TRUE(draw_images[3]->paint_image().IsSameForTesting(discardable_image[3][2]));
}
...
TEST_F(DiscardableImageMapTest, ClipsImageRects)
{
    ...
    PaintImage discardable_image = CreateDiscardablePaintImage(gfx::Size(500, 500));
    PaintRecord record = CreateRecording(discardable_image, visible_rect);

    scoped_refptr<DisplayItemList> display_list = new DisplayItemList;

    display_list->StartPaint();
    display_list->push<ClipRectOp>(gfx::RectToSkRect(gfx::Rect(250, 250)), SkClipOp::kIntersect, false);
    display_list->push<DrawRecordOp>(std::move(record));
    ...
    scoped_refptr<DiscardableImageMap> image_map = display_list->GenerateDiscardableImageMap();
    std::vector<PositionScaleDrawImage> images = GetDiscardableImagesInRect(*image_map, visible_rect);
    std::vector<gfx::Rect> inset_rects = InsetImageRects(images);
    EXPECT_EQ(1u, images.size());
    EXPECT_TRUE(images[0].image.IsSameForTesting(discardable_image));
...
TEST_F(DiscardableImageMapTest, GathersDiscardableImagesFromNestedOps)
{
    ...
    PaintImage discardable_image = CreateDiscardablePaintImage(gfx::Size(100, 100));
    ...
    scoped_refptr<DiscardableImageMap> image_map = DiscardableImageMap::Generate(root_buffer, gfx::Rect(200, 200));

    std::vector<const DrawImage*> images = image_map->GetDiscardableImagesInRect(gfx::Rect(0, 0, 5, 95));
    EXPECT_EQ(1u, images.size());
...
TEST_F(DiscardableImageMapTest, GathersAnimatedImages)
{
    ...
    std::vector<FrameMetadata> frames = { FrameMetadata(true, base::Milliseconds(2)), FrameMetadata(true, base::Milliseconds(3)) };

    gfx::Size image_size(100, 100);
    PaintImage static_image = CreateDiscardablePaintImage(image_size);
    PaintImage animated_loop_none = CreateAnimatedImage(image_size, frames, kAnimationNone);
    PaintImage animation_loop_infinite = CreateAnimatedImage(image_size, frames, 1u);
    ...
    content_layer_client.add_draw_image(animation_loop_infinite, gfx::Point(200, 200));
    ...
    EXPECT_EQ(animated_images_metadata[0].paint_image_id, animation_loop_infinite.stable_id());
    EXPECT_EQ(animated_images_metadata[0].completion_state, animation_loop_infinite.completion_state());
    EXPECT_EQ(animated_images_metadata[0].frames, animation_loop_infinite.GetFrameMetadata());
    EXPECT_EQ(animated_images_metadata[0].repetition_count, animation_loop_infinite.repetition_count());

    std::vector<const DrawImage*> images = image_map->GetDiscardableImagesInRect(visible_rect);
    ...
    EXPECT_TRUE(images[2]->paint_image().IsSameForTesting(animation_loop_infinite));
...
TEST_F(DiscardableImageMapTest, GathersPaintWorklets)
{
    ...
    gfx::Size image_size(100, 100);
    PaintImage static_image = CreateDiscardablePaintImage(image_size);
    scoped_refptr<TestPaintWorkletInput> input = base::MakeRefCounted<TestPaintWorkletInput>(gfx::SizeF(image_size));
...
TEST_F(DiscardableImageMapTest, CapturesImagesInPaintRecordShaders)
{
    ...
    flags.setShader(PaintShader::MakePaintRecord(shader_buffer.ReleaseAsRecord(), tile, SkTileMode::kClamp, SkTileMode::kClamp, nullptr));
    display_list->push<DrawRectOp>(SkRect::MakeWH(200, 200), flags);
    display_list->EndPaintOfUnpaired(visible_rect);
    display_list->Finalize();

    EXPECT_EQ(flags.getShader()->image_analysis_state(), ImageAnalysisState::kNoAnalysis);
    scoped_refptr<DiscardableImageMap> image_map = display_list->GenerateDiscardableImageMap();
    EXPECT_EQ(flags.getShader()->image_analysis_state(), ImageAnalysisState::kAnimatedImages);

    // The image rect is set to the rect for the DrawRectOp, and only animated
    // images in a shader are tracked.
    std::vector<PositionScaleDrawImage> draw_images = GetDiscardableImagesInRect(*image_map, visible_rect);
...
TEST_F(DiscardableImageMapTest, CapturesImagesInPaintFilters)
{
    ...
    display_list->StartPaint();
    PaintFlags flags;
    flags.setImageFilter(sk_make_sp<RecordPaintFilter>(filter_buffer.ReleaseAsRecord(), SkRect::MakeWH(150.f, 150.f)));
    display_list->push<DrawRectOp>(SkRect::MakeWH(200, 200), flags);
    display_list->EndPaintOfUnpaired(visible_rect);
    display_list->Finalize();

    EXPECT_EQ(flags.getImageFilter()->image_analysis_state(), ImageAnalysisState::kNoAnalysis);
    scoped_refptr<DiscardableImageMap> image_map = display_list->GenerateDiscardableImageMap();
    EXPECT_EQ(flags.getImageFilter()->image_analysis_state(), ImageAnalysisState::kAnimatedImages);
...
TEST_F(DiscardableImageMapTest, CapturesImagesInSaveLayers)
{
    ...
    flags.setShader(PaintShader::MakeImage(image, SkTileMode::kClamp, SkTileMode::kClamp, nullptr));
...
TEST_F(DiscardableImageMapTest, EmbeddedShaderWithAnimatedImages)
{
    ...
    PaintImage animated_image = CreateAnimatedImage(gfx::Size(100, 100), frames);
    shader_buffer.push<DrawImageOp>(animated_image, 0.f, 0.f);
    auto shader_with_image = PaintShader::MakePaintRecord(shader_buffer.ReleaseAsRecord(), tile, SkTileMode::kClamp, SkTileMode::kClamp, nullptr);
    ...
    auto shader_with_shader_with_image
        = PaintShader::MakePaintRecord(second_shader_buffer.ReleaseAsRecord(), tile, SkTileMode::kClamp, SkTileMode::kClamp, nullptr);
    ...
    display_list->Finalize();
    display_list->GenerateDiscardableImageMap();
    EXPECT_EQ(shader_with_image->image_analysis_state(), ImageAnalysisState::kAnimatedImages);
    EXPECT_EQ(shader_with_shader_with_image->image_analysis_state(), ImageAnalysisState::kAnimatedImages);
}
...
{
    gfx::Rect visible_rect(100, 100);
    PaintImage unspecified_image1 = PaintImageBuilder::WithCopy(CreateDiscardablePaintImage(gfx::Size(10, 10)))
                                        .set_id(1)
                                        .set_decoding_mode(PaintImage::DecodingMode::kUnspecified)
                                        .TakePaintImage();
    PaintImage async_image1 = PaintImageBuilder::WithCopy(CreateDiscardablePaintImage(gfx::Size(10, 10)))
                                  .set_id(1)
                                  .set_decoding_mode(PaintImage::DecodingMode::kAsync)
                                  .TakePaintImage();

    PaintImage unspecified_image2 = PaintImageBuilder::WithCopy(CreateDiscardablePaintImage(gfx::Size(10, 10)))
                                        .set_id(2)
                                        .set_decoding_mode(PaintImage::DecodingMode::kUnspecified)
                                        .TakePaintImage();
    PaintImage sync_image2 = PaintImageBuilder::WithCopy(CreateDiscardablePaintImage(gfx::Size(10, 10)))
                                 .set_id(2)
                                 .set_decoding_mode(PaintImage::DecodingMode::kSync)
                                 .TakePaintImage();

    PaintImage async_image3 = PaintImageBuilder::WithCopy(CreateDiscardablePaintImage(gfx::Size(10, 10)))
                                  .set_id(3)
                                  .set_decoding_mode(PaintImage::DecodingMode::kAsync)
                                  .TakePaintImage();
    PaintImage sync_image3 = PaintImageBuilder::WithCopy(CreateDiscardablePaintImage(gfx::Size(10, 10)))
                                 .set_id(3)
                                 .set_decoding_mode(PaintImage::DecodingMode::kSync)
                                 .TakePaintImage();
...
TEST_F(DiscardableImageMapTest, TracksImageRegions)
{
    ...
    std::vector<gfx::Rect> rects = { gfx::Rect(100, 100), gfx::Rect(400, 400, 100, 100) };
    ...
    EXPECT_EQ(ImageRectsToRegion(image_map->GetRectsForImage(image.stable_id())), expected_region);
}
...
TEST_F(DiscardableImageMapTest, ImagesUnderDrawScrollingContentsOp)
{
    ...
    display_list->PushDrawScrollingContentsOp(scroll_element_id1, scrolling_contents_list, gfx::Rect(200, 200));
    display_list->StartPaint();
    display_list->push<RestoreOp>();
    ...
    display_list->push<ClipRectOp>(SkRect::MakeWH(200, 200), SkClipOp::kIntersect, false);
    display_list->EndPaintOfPairedBegin();
    display_list->PushDrawScrollingContentsOp(scroll_element_id2, scrolling_contents_list, gfx::Rect(100, 300, 200, 200));
    display_list->StartPaint();
    display_list->push<RestoreOp>();
    display_list->EndPaintOfPairedEnd();
    display_list->Finalize();
    ...
    EXPECT_THAT(GetDiscardableImagesInRect(*image_map, gfx::Rect(1000, 1000)),
        UnorderedElementsAre(FieldsAre(ImageIsSame(image1), expected_rect, expected_scale), FieldsAre(ImageIsSame(image1), expected_rect, expected_scale),
            FieldsAre(ImageIsSame(image2), expected_rect, expected_scale), FieldsAre(ImageIsSame(image2), expected_rect, expected_scale)));
}

#if BUILDFLAG(SKIA_SUPPORT_SKOTTIE)
TEST_F(DiscardableImageMapTest, GetDiscardableImagesInRectSkottieWithoutImages)
{
    gfx::Rect visible_rect(2048, 2048);
    FakeContentLayerClient content_layer_client;
    content_layer_client.set_bounds(visible_rect.size());
    content_layer_client.add_draw_skottie(FakeContentLayerClient::SkottieData(CreateSkottie(gfx::Size(2048, 2048), /*duration_secs=*/1.f),
        /*dst=*/gfx::Rect(2048, 2048), /*t=*/0.1f, SkottieFrameDataMap(), SkottieColorMap(), SkottieTextPropertyValueMap()));

    scoped_refptr<DisplayItemList> display_list = content_layer_client.PaintContentsToDisplayList();
    scoped_refptr<DiscardableImageMap> image_map = display_list->GenerateDiscardableImageMap();
    EXPECT_THAT(GetDiscardableImagesInRect(*image_map, gfx::Rect(2048, 2048)), IsEmpty());
}
...
TEST_F(DiscardableImageMapTest, GetDiscardableImagesInRectSkottieWithImages)
{
    ...
    PaintImage image_0 = CreateDiscardablePaintImage(gfx::Size(kLottieDataWith2AssetsWidth, kLottieDataWith2AssetsHeight));
    PaintImage image_1 = CreateDiscardablePaintImage(gfx::Size(kLottieDataWith2AssetsWidth, kLottieDataWith2AssetsHeight));
    images_in[HashSkottieResourceId("image_0")] = { .image = image_0, .quality = PaintFlags::FilterQuality::kHigh };
    images_in[HashSkottieResourceId("image_1")] = { .image = image_1, .quality = PaintFlags::FilterQuality::kHigh };
    content_layer_client.add_draw_skottie(FakeContentLayerClient::SkottieData(CreateSkottieFromString(kLottieDataWith2Assets),
    ...
    EXPECT_THAT(GetDiscardableImagesInRect(*image_map, gfx::Rect(1023, 2048)), IsEmpty());
    // Right Half of screen should return 2 images.
    std::vector<PositionScaleDrawImage> images_out = GetDiscardableImagesInRect(*image_map, gfx::Rect(1024, 0, 1024, 2048));
    ASSERT_THAT(images_out, SizeIs(2));
    EXPECT_THAT(images_out, Contains(Field(&PositionScaleDrawImage::image, ImageIsSame(image_0))));
    EXPECT_THAT(images_out, Contains(Field(&PositionScaleDrawImage::image, ImageIsSame(image_1))));
}

TEST_F(DiscardableImageMapTest, GetDiscardableImagesInRectSkottieWithImagesScalesProperly)
{
    gfx::Rect visible_rect(kLottieDataWith2AssetsWidth * 2, kLottieDataWith2AssetsHeight * 3);
    FakeContentLayerClient content_layer_client;
    content_layer_client.set_bounds(visible_rect.size());
    scoped_refptr<SkottieWrapper> skottie = CreateSkottieFromString(kLottieDataWith2Assets);
    ...
    scoped_refptr<DisplayItemList> display_list = content_layer_client.PaintContentsToDisplayList();
    scoped_refptr<DiscardableImageMap> image_map = display_list->GenerateDiscardableImageMap();
    std::vector<PositionScaleDrawImage> images_out = GetDiscardableImagesInRect(*image_map, visible_rect);
    ASSERT_THAT(images_out, SizeIs(2));
...
TEST_F(DiscardableImageMapTest, PreserveImageQuality)
{
    ...
    scoped_feature_list.InitAndEnableFeature(features::kPreserveDiscardableImageMapQuality);
    ...
    SkSamplingOptions low_quality_sampling(PaintFlags::FilterQualityToSkSamplingOptions(PaintFlags::FilterQuality::kLow));
    display_list->push<DrawImageOp>(low_quality_image, 0.f, 0.f, low_quality_sampling, nullptr);

    PaintImage medium_quality_image = CreateDiscardablePaintImage(gfx::Size(25, 25));
    SkSamplingOptions medium_quality_sampling(PaintFlags::FilterQualityToSkSamplingOptions(PaintFlags::FilterQuality::kMedium));
    display_list->push<DrawImageOp>(medium_quality_image, 0.f, 0.f, medium_quality_sampling, nullptr);

    PaintImage high_quality_image = CreateDiscardablePaintImage(gfx::Size(25, 25));
    SkSamplingOptions high_quality_sampling(PaintFlags::FilterQualityToSkSamplingOptions(PaintFlags::FilterQuality::kHigh));
    display_list->push<DrawImageOp>(high_quality_image, 0.f, 0.f, high_quality_sampling, nullptr);
    display_list->EndPaintOfUnpaired(visible_rect);
    display_list->Finalize();

    scoped_refptr<DiscardableImageMap> image_map = display_list->GenerateDiscardableImageMap();
    std::vector<const DrawImage*> images = image_map->GetDiscardableImagesInRect(gfx::Rect(0, 0, 200, 200));
    EXPECT_EQ(3u, images.size());
