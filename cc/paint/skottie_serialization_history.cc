...
SkottieSerializationHistory::SkottieFrameDataId::SkottieFrameDataId(const SkottieFrameData& frame_data)
    : paint_image_id(frame_data.image ? frame_data.image.stable_id() : PaintImage::kInvalidId)
    , quality(frame_data.quality)
{
}

bool SkottieSerializationHistory::SkottieFrameDataId::operator==(const SkottieFrameDataId& other) const
{
...
SkottieSerializationHistory::SkottieWrapperHistory::SkottieWrapperHistory(
    const SkottieFrameDataMap& initial_images, const SkottieTextPropertyValueMap& initial_text_map)
    : accumulated_text_map_(initial_text_map)
{
    for (const auto& image_asset_pair : initial_images) {
        ...
        last_frame_data_per_asset_.emplace(
            /*asset id*/ image_asset_pair.first, SkottieFrameDataId(image_asset_pair.second));
    }
}

SkottieSerializationHistory::SkottieWrapperHistory::SkottieWrapperHistory(const SkottieWrapperHistory& other) = default;

SkottieSerializationHistory::SkottieWrapperHistory& SkottieSerializationHistory::SkottieWrapperHistory::operator=(const SkottieWrapperHistory& other) = default;

SkottieSerializationHistory::SkottieWrapperHistory::~SkottieWrapperHistory() = default;

void SkottieSerializationHistory::SkottieWrapperHistory::FilterNewState(SkottieFrameDataMap& images, SkottieTextPropertyValueMap& text_map)
{
...
void SkottieSerializationHistory::SkottieWrapperHistory::FilterNewFrameImages(SkottieFrameDataMap& images)
{
    ...
    while (images_iter != images.end()) {
        ...
        auto [result_iterator, is_new_insertion] = last_frame_data_per_asset_.emplace(asset_id, new_frame_data_id);
        SkottieFrameDataId& existing_frame_data_id = result_iterator->second;

        bool asset_has_updated_frame_data = is_new_insertion || existing_frame_data_id != new_frame_data_id;
...
void SkottieSerializationHistory::SkottieWrapperHistory::FilterNewTextPropertyValues(SkottieTextPropertyValueMap& text_map_in)
{
    ...
    while (text_map_in_iter != text_map_in.end()) {
        ...
        const SkottieTextPropertyValue& new_text_property_val = text_map_in_iter->second;
        auto [accumulated_iter, is_new_insertion] = accumulated_text_map_.insert(*text_map_in_iter);
...
void SkottieSerializationHistory::FilterNewSkottieFrameState(const SkottieWrapper& skottie, SkottieFrameDataMap& images, SkottieTextPropertyValueMap& text_map)
{
    DCHECK(skottie.is_valid());
    base::AutoLock lock(mutex_);
    auto [result_iterator, is_new_insertion] = history_per_animation_.try_emplace(skottie.id(), images, text_map);
    if (is_new_insertion) {
        DVLOG(1) << "Encountered new SkottieWrapper with id " << skottie.id() << " and " << images.size() << " images";
    } else {
...
void SkottieSerializationHistory::RequestInactiveAnimationsPurge()
{
    ...
    while (animation_history_iter != history_per_animation_.end()) {
        SkottieWrapperHistory& skottie_wrapper_history = animation_history_iter->second;
        if (skottie_wrapper_history.current_sequence_id() == skottie_wrapper_history.sequence_id_at_last_purge_check()) {
            DVLOG(1) << "Purging Skottie animation with id " << animation_history_iter->first << ". No update to animation's state since last purge check.";
            animation_history_iter = history_per_animation_.erase(animation_history_iter);
        } else {
