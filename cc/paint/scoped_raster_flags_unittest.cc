...
public:
    ...
    ~MockImageProvider() override
    {
        EXPECT_EQ(ref_count_, 0);
    }
    ...
    ScopedResult GetRasterContent(const DrawImage& draw_image) override
    {
        ...
        return ScopedResult(DecodedDrawImage(image, nullptr, SkSize::MakeEmpty(), SkSize::Make(1.0f, 1.0f), draw_image.filter_quality(), true),
...
TEST(ScopedRasterFlagsTest, DecodePaintWorkletImageShader)
{
    ...
    auto shader = PaintShader::MakeImage(image, SkTileMode::kRepeat, SkTileMode::kRepeat, &pattern_matrix, &tile_rect);
    PaintFlags flags;
    ...
    EXPECT_TRUE(scoped_flags.flags()->getShader()->shader_type() == PaintShader::Type::kPaintRecord);
}

TEST(ScopedRasterFlagsTest, KeepsDecodesAlive)
{
    PaintOpBuffer buffer;
    buffer.push<DrawImageOp>(CreateDiscardablePaintImage(gfx::Size(10, 10)), 0.f, 0.f);
    buffer.push<DrawImageOp>(CreateDiscardablePaintImage(gfx::Size(10, 10)), 0.f, 0.f);
    buffer.push<DrawImageOp>(CreateDiscardablePaintImage(gfx::Size(10, 10)), 0.f, 0.f);
    auto record_shader
        = PaintShader::MakePaintRecord(buffer.ReleaseAsRecord(), SkRect::MakeWH(100, 100), SkTileMode::kClamp, SkTileMode::kClamp, &SkMatrix::I());
...
TEST(ScopedRasterFlagsTest, NoImageProvider)
{
    ...
    flags.setShader(PaintShader::MakeImage(CreateDiscardablePaintImage(gfx::Size(10, 10)), SkTileMode::kClamp, SkTileMode::kClamp, &SkMatrix::I()));
...
TEST(ScopedRasterFlagsTest, ThinAliasedStroke)
{
    ...
    for (const auto& test : tests) {
        ...
        EXPECT_LE(std::abs(scoped_flags.flags()->getAlphaf() - test.expect_alpha), 0.01f);
    }
}
