...
PaintWorkletInput::PropertyKey::PropertyKey(const std::string& custom_property_name, ElementId element_id)
    : custom_property_name(custom_property_name)
    , element_id(element_id)
{
}

PaintWorkletInput::PropertyKey::PropertyKey(NativePropertyType native_property_type, ElementId element_id)
    : native_property_type(native_property_type)
    , element_id(element_id)
{
}
...
PaintWorkletInput::PropertyKey& PaintWorkletInput::PropertyKey::operator=(const PropertyKey& other) = default;

PaintWorkletInput::PropertyKey& PaintWorkletInput::PropertyKey::operator=(PropertyKey&& other) = default;
...
{
    return custom_property_name == other.custom_property_name && native_property_type == other.native_property_type && element_id == other.element_id;
}
...
bool PaintWorkletInput::PropertyKey::operator<(const PropertyKey& other) const
{
    if (custom_property_name.has_value() && !other.custom_property_name.has_value())
        return true;
    if (!custom_property_name.has_value() && other.custom_property_name.has_value())
        return false;
    if (custom_property_name.has_value() && other.custom_property_name.has_value()) {
...
{
    DCHECK(float_value.has_value() != color_value.has_value() || (!float_value.has_value() && !color_value.has_value()));
...
bool PaintWorkletInput::ValueChangeShouldCauseRepaint(const PropertyValue& val1, const PropertyValue& val2) const
{
    return val1.color_value != val2.color_value || val1.float_value != val2.float_value;
}
