...
static constexpr std::array<AuxImage, 2> kAllAuxImages = { AuxImage::kDefault, AuxImage::kGainmap };
...
static constexpr size_t kAuxImageIndexDefault = AuxImageIndex(AuxImage::kDefault);
static constexpr size_t kAuxImageIndexGainmap = AuxImageIndex(AuxImage::kGainmap);
...
public:
    ...
    SkISize GetSupportedDecodeSize(const SkISize& requested_size, AuxImage aux_image = AuxImage::kDefault) const;
    ...
    bool Decode(SkPixmap pixmap, size_t frame_index, AuxImage aux_image, GeneratorClientId client_id) const;
    ...
    bool DecodeYuv(const SkYUVAPixmaps& pixmaps, size_t frame_index, AuxImage aux_image, GeneratorClientId client_id) const;
    ...
    bool readPixels(const SkImageInfo& dst_info, void* dst_pixels, size_t dst_row_bytes, int src_x, int src_y) const;
    ...
    CompletionState completion_state() const
    {
        return completion_state_;
    }
    bool is_multipart() const
    {
        return is_multipart_;
    }
    bool is_high_bit_depth() const
    {
    ...
    DecodingMode decoding_mode() const
    {
    ...
    bool IsDeferredPaintRecord() const
    {
        return !!deferred_paint_record_;
    }
    bool IsPaintWorklet() const
    {
        return deferred_paint_record_ && deferred_paint_record_->IsPaintWorkletInput();
    }
    ...
    int width() const
    {
        return GetSkImageInfo().width();
    }
    int height() const
    {
        return GetSkImageInfo().height();
    }
    ...
    bool GetReinterpretAsSRGB() const
    {
    ...
    // transformation.
    bool IsYuv(const SkYUVAPixmapInfo::SupportedDataTypes& supported_data_types, AuxImage aux_image, SkYUVAPixmapInfo* info = nullptr) const;
    ...
    sk_sp<SkImage> GetSkImageForFrame(size_t index, GeneratorClientId client_id) const;
    ...
    {
        DCHECK_EQ(gainmap_paint_image_generator_ != nullptr || gainmap_sk_image_ != nullptr, gainmap_info_.has_value());
...
private:
    ...
    bool DecodeFromSkImage(SkPixmap pixmap, size_t frame_index, GeneratorClientId client_id) const;
    void CreateSkImage();
