...
FrameSinkImpl::UploadedUIResource::~UploadedUIResource() = default;
FrameSinkImpl::UploadedUIResource::UploadedUIResource(const UploadedUIResource&) = default;
FrameSinkImpl::UploadedUIResource& FrameSinkImpl::UploadedUIResource::operator=(const UploadedUIResource&) = default;

FrameSinkImpl::FrameSinkImpl(scoped_refptr<base::SingleThreadTaskRunner> task_runner,
    mojo::PendingAssociatedRemote<viz::mojom::CompositorFrameSink> compositor_frame_sink_associated_remote,
    mojo::PendingReceiver<viz::mojom::CompositorFrameSinkClient> client_receiver, scoped_refptr<viz::RasterContextProvider> context_provider,
    base::PlatformThreadId io_thread_id, std::unique_ptr<Scheduler> scheduler)
    : task_runner_(std::move(task_runner))
    , scheduler_(std::move(scheduler))
    , pending_compositor_frame_sink_associated_remote_(std::move(compositor_frame_sink_associated_remote))
...
FrameSinkImpl::~FrameSinkImpl()
{
    ...
    auto resource_ids = base::MakeFlatSet<viz::ResourceId>(uploaded_resources_, {}, [](auto& resource_pair) { return resource_pair.second.viz_resource_id; });
...
void FrameSinkImpl::SetLocalSurfaceId(const viz::LocalSurfaceId& local_surface_id)
{
...
bool FrameSinkImpl::BindToClient(FrameSinkImplClient* client)
{
    ...
    frame_sink_remote_.Bind(std::move(pending_compositor_frame_sink_associated_remote_));
    frame_sink_remote_.set_disconnect_handler(base::BindOnce(&FrameSinkImpl::OnContextLost, base::Unretained(this)));
    client_receiver_.Bind(std::move(pending_client_receiver_), task_runner_);

    frame_sink_ = frame_sink_remote_.get();
    frame_sink_->InitializeCompositorFrameSinkType(viz::mojom::CompositorFrameSinkType::kLayerTree);

#if BUILDFLAG(IS_ANDROID)
    std::vector<viz::Thread> threads;
    threads.push_back({ base::PlatformThread::CurrentId(), viz::Thread::Type::kMain });
...
void FrameSinkImpl::UploadUIResource(cc::UIResourceId resource_id, cc::UIResourceBitmap resource_bitmap)
{
    gfx::Size size = resource_bitmap.GetSize();
    TRACE_EVENT1("cc", "slim::FrameSinkImpl::UploadUIResource", "size", size.ToString());
    ...
    uploaded_resource.shared_image = sii->CreateSharedImage(
        { format, resource_bitmap.GetSize(), color_space, shared_image_usage, "SlimCompositorUIResource" }, resource_bitmap.GetPixels());
    ...
    uploaded_resource.viz_resource_id = resource_provider_.ImportResource(
        viz::TransferableResource::MakeGpu(uploaded_resource.shared_image, /*texture_target=*/GL_TEXTURE_2D, sync_token, resource_bitmap.GetSize(), format,
            /*is_overlay_candidate=*/false, viz::TransferableResource::ResourceSource::kUI),
        base::BindOnce(&FrameSinkImpl::UIResourceReleased, base::Unretained(this), resource_id));
...
void FrameSinkImpl::UIResourceReleased(cc::UIResourceId ui_resource_id, const gpu::SyncToken& sync_token, bool is_lost)
{
...
void FrameSinkImpl::DidReceiveCompositorFrameAck(std::vector<viz::ReturnedResource> resources)
{
...
void FrameSinkImpl::ReclaimResources(std::vector<viz::ReturnedResource> resources)
{
    resource_provider_.ReceiveReturnsFromParent(std::move(resources));
}

void FrameSinkImpl::OnBeginFrame(
    const viz::BeginFrameArgs& begin_frame_args, const viz::FrameTimingDetailsMap& timing_details, bool frame_ack, std::vector<viz::ReturnedResource> resources)
{
...
bool FrameSinkImpl::DoBeginFrame(const viz::BeginFrameArgs& begin_frame_args)
{
    ...
    {
        TRACE_EVENT("viz,benchmark,graphics.pipeline", "Graphics.Pipeline", perfetto::Flow::Global(begin_frame_args.trace_id), [&](perfetto::EventContext ctx) {
        ...
        if (!client_->BeginFrame(begin_frame_args, frame, viz_resource_ids, hit_test_region_list)) {
            return false;
        }
        ...
        resource_provider_.PrepareSendToParent(std::move(viz_resource_ids).extract(), &frame.resource_list, context_provider_.get());

        bool send_new_hit_test_region_list = false;
        if (!hit_test_region_list_ || !viz::HitTestRegionList::IsEqual(*hit_test_region_list_, hit_test_region_list)) {
        ...
        {
            TRACE_EVENT(
                "viz,benchmark,graphics.pipeline", "Graphics.Pipeline", perfetto::Flow::Global(begin_frame_args.trace_id), [&](perfetto::EventContext ctx) {
                    auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
                    auto* data = event->set_chrome_graphics_pipeline();
            ...
            frame_sink_->SubmitCompositorFrame(local_surface_id_, std::move(frame), send_new_hit_test_region_list ? hit_test_region_list_ : std::nullopt, 0);
        }
    }
    num_unacked_frames_++;
...
void FrameSinkImpl::SendDidNotProduceFrame(const viz::BeginFrameArgs& begin_frame_args)
{
    TRACE_EVENT("viz,benchmark,graphics.pipeline", "Graphics.Pipeline", perfetto::Flow::Global(begin_frame_args.trace_id), [&](perfetto::EventContext ctx) {
        auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
        auto* data = event->set_chrome_graphics_pipeline();
        data->set_step(perfetto::protos::pbzero::ChromeGraphicsPipeline::StepName::STEP_DID_NOT_PRODUCE_COMPOSITOR_FRAME);
        data->set_surface_frame_trace_id(begin_frame_args.trace_id);
    });
