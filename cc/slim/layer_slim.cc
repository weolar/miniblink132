...
cc::FilterOperations ToCcFilters(std::vector<cc::slim::Filter> filters)
{
    ...
    for (const auto& slim_filter : filters) {
        ...
        case cc::slim::Filter::kBrightness:
            cc_filters.Append(cc::FilterOperation::CreateBrightnessFilter(slim_filter.amount()));
            break;
        case cc::slim::Filter::kSaturation:
            cc_filters.Append(cc::FilterOperation::CreateSaturateFilter(slim_filter.amount()));
            break;
        }
    }
    return cc_filters;
...
void Layer::ReplaceChild(Layer* old_child, scoped_refptr<Layer> new_child)
{
    ...
    auto it = base::ranges::find_if(children_, [&](auto& ptr) { return ptr.get() == old_child; });
...
void Layer::RemoveFromParentSlim()
{
    ...
    std::erase_if(parent_->children_, [&](auto& ptr) { return ptr.get() == this; });
...
gfx::Transform Layer::ComputeTransformToParent() const
{
    ...
    gfx::Transform transform = gfx::Transform::MakeTranslation(position_.x(), position_.y());
...
void Layer::AppendQuads(viz::CompositorRenderPass& render_pass, FrameData& data, const gfx::Transform& transform_to_root,
    const gfx::Transform& transform_to_target, const gfx::Rect* clip_in_target, const gfx::Rect& visible_rect, float opacity)
{
}

viz::SharedQuadState* Layer::CreateAndAppendSharedQuadState(viz::CompositorRenderPass& render_pass, FrameData& data, const gfx::Transform& transform_to_target,
    const gfx::Rect* clip_in_target, const gfx::Rect& visible_rect, float opacity)
{
    ...
    quad_state->SetAll(transform_to_target, layer_rect, visible_rect, data.mask_filter_info_in_target, clip_opt, contents_opaque(), opacity,
