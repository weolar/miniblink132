...
class LayerTreeImplScopedKeepSurfaceAlive : public LayerTree::ScopedKeepSurfaceAlive {
public:
    LayerTreeImplScopedKeepSurfaceAlive(base::WeakPtr<LayerTreeImpl> layer_tree, const viz::SurfaceId& surface_id)
...
LayerTreeImpl::PresentationCallbackInfo::PresentationCallbackInfo(
    uint32_t frame_token, std::vector<PresentationCallback> presentation_callbacks, std::vector<SuccessfulCallback> success_callbacks)
...
LayerTreeImpl::PresentationCallbackInfo::PresentationCallbackInfo(PresentationCallbackInfo&&) = default;
LayerTreeImpl::PresentationCallbackInfo& LayerTreeImpl::PresentationCallbackInfo::operator=(PresentationCallbackInfo&&) = default;

LayerTreeImpl::LayerTreeImpl(LayerTreeClient* client, uint32_t num_unneeded_begin_frame_before_stop, int min_occlusion_tracking_dimension)
    : client_(client)
    , num_unneeded_begin_frame_before_stop_(num_unneeded_begin_frame_before_stop)
...
void LayerTreeImpl::SetViewportRectAndScale(const gfx::Rect& device_viewport_rect, float device_scale_factor, const viz::LocalSurfaceId& local_surface_id)
{
    bool id_updated = local_surface_id_allocator_.UpdateFromParent(local_surface_id);
    if (device_viewport_rect_ == device_viewport_rect && device_scale_factor_ == device_scale_factor && !id_updated) {
        return;
    }
    if (frame_sink_) {
        frame_sink_->SetLocalSurfaceId(local_surface_id_allocator_.GetCurrentLocalSurfaceId());
    }
...
void LayerTreeImpl::RequestPresentationTimeForNextFrame(PresentationCallback callback)
{
    presentation_callback_for_next_frame_.emplace_back(std::move(callback));
}

void LayerTreeImpl::RequestSuccessfulPresentationTimeForNextFrame(SuccessfulCallback callback)
{
...
void LayerTreeImpl::RequestCopyOfOutput(std::unique_ptr<viz::CopyOutputRequest> request)
{
    if (request->has_source()) {
        const base::UnguessableToken& source = request->source();
        auto it = base::ranges::find_if(
            copy_requests_for_next_frame_, [&source](const std::unique_ptr<viz::CopyOutputRequest>& x) { return x->has_source() && x->source() == source; });
...
base::OnceClosure LayerTreeImpl::DeferBeginFrame()
{
    ...
    return base::BindOnce(&LayerTreeImpl::ReleaseDeferBeginFrame, weak_factory_.GetWeakPtr());
}
...
std::unique_ptr<LayerTree::ScopedKeepSurfaceAlive> LayerTreeImpl::CreateScopedKeepSurfaceAlive(const viz::SurfaceId& surface_id)
{
    return std::make_unique<LayerTreeImplScopedKeepSurfaceAlive>(weak_factory_.GetWeakPtr(), surface_id);
}

const LayerTree::SurfaceRangesAndCounts& LayerTreeImpl::GetSurfaceRangesForTesting() const
{
...
bool LayerTreeImpl::BeginFrame(const viz::BeginFrameArgs& args, viz::CompositorFrame& out_frame, base::flat_set<viz::ResourceId>& out_resource_ids,
    viz::HitTestRegionList& out_hit_test_region_list)
{
    ...
    GenerateCompositorFrame(args, out_frame, out_resource_ids, out_hit_test_region_list);
    UpdateNeedsBeginFrame();
    return true;
}
...
void LayerTreeImpl::DidPresentCompositorFrame(uint32_t frame_token, const viz::FrameTimingDetails& details)
{
    const bool success = !details.presentation_feedback.failed();
    for (auto itr = pending_presentation_callbacks_.begin(); itr != pending_presentation_callbacks_.end();) {
...
void LayerTreeImpl::AddSurfaceRange(const viz::SurfaceRange& range)
{
    ...
    DCHECK(!referenced_surfaces_.contains(range) || referenced_surfaces_[range] >= 1);
...
void LayerTreeImpl::RegisterOffsetTag(const viz::OffsetTag& tag, SurfaceLayer* owner)
{
...
void LayerTreeImpl::UnregisterOffsetTag(const viz::OffsetTag& tag, SurfaceLayer* owner)
{
...
{
    return NeedsDraw() || num_begin_frames_with_no_draw_ < num_unneeded_begin_frame_before_stop_;
}

void LayerTreeImpl::GenerateCompositorFrame(const viz::BeginFrameArgs& args, viz::CompositorFrame& out_frame, base::flat_set<viz::ResourceId>& out_resource_ids,
    viz::HitTestRegionList& out_hit_test_region_list)
{
    for (auto& resource_request : ui_resource_manager_.TakeUIResourcesRequests()) {
        ...
        case cc::UIResourceRequest::Type::kCreate:
            frame_sink_->UploadUIResource(resource_request.GetId(), resource_request.GetBitmap());
            break;
    ...
    out_hit_test_region_list.flags = viz::HitTestRegionFlags::kHitTestMine | viz::HitTestRegionFlags::kHitTestMouse | viz::HitTestRegionFlags::kHitTestTouch;
    ...
    out_frame.metadata.begin_frame_ack = viz::BeginFrameAck(args, /*has_damage=*/true);
    ...
    for (auto& [tag, layer] : registered_offset_tags_) {
        ...
        if (layer->surface_id().is_valid()) {
            out_frame.metadata.offset_tag_definitions.push_back(layer->GetOffsetTagDefinition(tag));
        }
    }
    ...
    bool viewport_fully_occluded = frame_data.occlusion_in_target.Contains(device_viewport_rect_);
    render_pass->has_transparent_background = !background_opaque && !viewport_fully_occluded;
    if (background_color_.fA && !viewport_fully_occluded) {
        ...
        for (size_t i = 0; i < frame_data.occlusion_in_target.GetRegionComplexity(); ++i) {
            unoccluded_region.Subtract(frame_data.occlusion_in_target.GetRect(i));
        }
        if (!unoccluded_region.IsEmpty()) {
            viz::SharedQuadState* quad_state = render_pass->CreateAndAppendSharedQuadState();
            gfx::Rect gutter_bounding_rect = unoccluded_region.bounds();
            bool contents_opaque = background_opaque && unoccluded_region.GetRegionComplexity() <= 1;
            quad_state->SetAll(gfx::Transform(), gutter_bounding_rect, gutter_bounding_rect, gfx::MaskFilterInfo(),
            ...
            for (gfx::Rect unoccluded_rect : unoccluded_region) {
                viz::SolidColorDrawQuad* quad = render_pass->CreateAndAppendDrawQuad<viz::SolidColorDrawQuad>();
                quad->SetNew(quad_state, unoccluded_rect, unoccluded_rect, background_color_, /*anti_aliasing_off=*/false);
            }
        }
    }
    ...
    out_frame.metadata.activation_dependencies
        = std::vector<viz::SurfaceId>(frame_data.activation_dependencies.begin(), frame_data.activation_dependencies.end());
    out_frame.metadata.deadline
        = viz::FrameDeadline(args.frame_time, frame_data.deadline_in_frames.value_or(0u), args.interval, frame_data.use_default_lower_bound_deadline);
    ...
    if (!presentation_callback_for_next_frame_.empty() || !success_callback_for_next_frame_.empty()) {
        pending_presentation_callbacks_.emplace_back(
            out_frame.metadata.frame_token, std::move(presentation_callback_for_next_frame_), std::move(success_callback_for_next_frame_));
    }
}

void LayerTreeImpl::Draw(Layer& layer, viz::CompositorRenderPass& parent_pass, FrameData& data, const gfx::Transform& parent_transform_to_root,
    const gfx::Transform& parent_transform_to_target, const gfx::RectF* parent_clip_in_target, const gfx::RectF& clip_in_parent, float parent_opacity)
{
    ...
    std::optional<gfx::Transform> transform_from_parent = layer.ComputeTransformFromParent();
    ...
    if (layer.offset_tag() && registered_offset_tags_.contains(layer.offset_tag())) {
        ...
        CHECK(!data.mask_filter_info_in_target.HasRoundedCorners() && !data.mask_filter_info_in_target.HasGradientMask());
        ...
        gfx::RectF expanded_parent_clip_in_target = parent_clip_in_target ? *parent_clip_in_target : gfx::RectF(parent_pass.output_rect);
        auto tag_constraints = registered_offset_tags_[layer.offset_tag()]->GetOffsetTagDefinition(layer.offset_tag()).constraints;
        tag_constraints.ExpandVisibleRect(expanded_parent_clip_in_target);
        clip_in_layer = transform_to_target.GetCheckedInverse().MapRect(expanded_parent_clip_in_target);
    } else {
        clip_in_layer = transform_from_parent->MapRect(clip_in_parent);
    }

    const bool mask_to_bounds = layer.masks_to_bounds() || layer.HasNonTrivialMaskFilterInfo();
    if (mask_to_bounds) {
    ...
    {
        const int num_drawing_layers_in_subtree = layer.GetNumDrawingLayersInSubtree();
        ...
        const bool mask_filter_needs_pass
            = layer.HasNonTrivialMaskFilterInfo() && (data.mask_filter_info_in_target.HasRoundedCorners() || data.mask_filter_info_in_target.HasGradientMask());
        const bool clip_needs_pass = !is_root && mask_to_bounds && !transform_to_target.Preserves2dAxisAlignment();
        const bool opacity_needs_pass = layer.opacity() != 1.0f && num_drawing_layers_in_subtree > 1;
        if (!filters_needs_pass && !clip_needs_pass && !mask_filter_needs_pass && !opacity_needs_pass) {
            ...
            DrawChildrenAndAppendQuads(
                layer, parent_pass, data, transform_to_root, transform_to_target, clip_in_target, clip_in_layer, parent_opacity * layer.opacity());
            return;
        }
    }
    ...
    gfx::Transform transform_new_pass_to_parent_target;
    {
        ...
        scale_to_new_pass = gfx::ComputeTransform2dScaleComponents(transform_to_root, /*fallback_value=*/1.0f);
        ...
        transform_to_target = gfx::Transform::MakeScale(scale_to_new_pass.x(), scale_to_new_pass.y());
    ...
    data.render_pass_damage.clear();
    {
        ...
        base::AutoReset<viz::OffsetTag> render_pass_offset_tag_reset(&data.offset_tag, viz::OffsetTag());

        DrawChildrenAndAppendQuads(layer, *new_pass, data, transform_to_root, transform_to_target, clip_in_target, clip_in_layer,
            /*opacity=*/1.0f);
        ...
        if (transform_new_pass_to_parent_target.Preserves2dAxisAlignment()) {
            ...
            for (size_t i = 0; i < occlusion_in_new_pass.GetRegionComplexity(); ++i) {
                ...
                gfx::Rect occlusion_in_parent_target
                    = gfx::ToEnclosedRect(transform_new_pass_to_parent_target.MapRect(gfx::RectF(occlusion_in_new_pass.GetRect(i))));
    ...
    viz::SharedQuadState* shared_quad_state = parent_pass.CreateAndAppendSharedQuadState();

    // Union through quad list in new pass to compute content rect.
    gfx::Rect content_rect;
    for (const auto* new_pass_quad : new_pass->quad_list) {
        content_rect.Union(new_pass_quad->shared_quad_state->quad_to_target_transform.MapRect(new_pass_quad->rect));
    }
    ...
    const bool new_pass_contents_opaque = occlusion_in_new_pass.Contains(content_rect);
    shared_quad_state->SetAll(transform_new_pass_to_parent_target, content_rect, content_rect, data.mask_filter_info_in_target, clip_opt,
        new_pass_contents_opaque, parent_opacity * layer.opacity(), SkBlendMode::kSrcOver, /*sorting_context=*/0,
        /*layer_id=*/0u, /*fast_rounded_corner=*/true);
    shared_quad_state->offset_tag = data.offset_tag;

    auto* quad = parent_pass.CreateAndAppendDrawQuad<viz::CompositorRenderPassDrawQuad>();
    ...
    parent_pass_damage.emplace_back(
        layer.id(), DamageData(new_pass->has_damage_from_contributing_content, transform_new_pass_to_parent_target.MapRect(content_rect)));
    data.render_pass_damage = std::move(parent_pass_damage);

    data.frame->render_pass_list.push_back(std::move(new_pass));
}

void LayerTreeImpl::DrawChildrenAndAppendQuads(Layer& layer, viz::CompositorRenderPass& render_pass, FrameData& data, const gfx::Transform& transform_to_root,
    const gfx::Transform& transform_to_target, const gfx::RectF* clip_in_target, const gfx::RectF& clip_in_layer, float opacity)
{
    const bool subtree_property_changed = layer.GetAndResetSubtreePropertyChanged() || data.subtree_property_changed_from_parent;
    std::optional<base::AutoReset<gfx::MaskFilterInfo>> auto_reset_mask_filter_info;
    if (layer.HasNonTrivialMaskFilterInfo()) {
        gfx::MaskFilterInfo info(gfx::RRectF(gfx::RectF(gfx::Rect(layer.bounds())), layer.corner_radii()), layer.gradient_mask());
        info.ApplyTransform(transform_to_target);
        auto_reset_mask_filter_info.emplace(&data.mask_filter_info_in_target, info);
    }

    {
        base::AutoReset reset(&data.subtree_property_changed_from_parent, subtree_property_changed);
        for (auto& child : base::Reversed(layer.children())) {
            Draw(*child, render_pass, data, transform_to_root, transform_to_target, clip_in_target, clip_in_layer, opacity);
        }
    }
    ...
    gfx::RectF visible_rectf_in_target = transform_to_target.MapRect(visible_rectf);
    if (!visible_rectf.IsEmpty() && layer.HasDrawableContent()
        && UpdateOcclusionRect(layer, data, transform_to_target, opacity, visible_rectf_in_target, visible_rectf)) {
        gfx::Rect visible_rect = gfx::ToEnclosingRect(visible_rectf);
        layer.AppendQuads(render_pass, data, transform_to_root, transform_to_target, clip_in_target ? &integer_clip_in_target : nullptr, visible_rect, opacity);
        data.render_pass_damage.emplace_back(
            layer.id(), DamageData(layer.GetAndResetPropertyChanged() || subtree_property_changed, gfx::ToEnclosingRect(visible_rectf_in_target)));
    }
}

bool LayerTreeImpl::UpdateOcclusionRect(Layer& layer, FrameData& data, const gfx::Transform& transform_to_target, float opacity,
    const gfx::RectF& visible_rectf_in_target, gfx::RectF& visible_rect)
{
    if (data.offset_tag) {
    ...
    if (data.occlusion_in_target.Contains(gfx::ToEnclosingRect(visible_rectf_in_target))) {
        return false;
    }
    ...
    if (transform_to_target.GetInverse(&from_target)) {
        for (size_t i = 0; i < data.occlusion_in_target.GetRegionComplexity(); ++i) {
            visible_rect.Subtract(from_target.MapRect(gfx::RectF(data.occlusion_in_target.GetRect(i))));
        }
    }

    if (opacity < 1.0f || !layer.contents_opaque() || layer.HasNonTrivialMaskFilterInfo()) {
        return true;
    }

    // Add unoccluded visible rect to occlusion.
    if (visible_rectf_in_target.width() >= min_occlusion_tracking_dimension_ || visible_rectf_in_target.height() >= min_occlusion_tracking_dimension_) {
        // Use ToEnclosedRect to avoid including extra pixels as occluded due to
        // rounding error.
        data.occlusion_in_target.Union(gfx::ToEnclosedRect(visible_rectf_in_target));
    }
    return true;
}

void LayerTreeImpl::ProcessDamageForRenderPass(viz::CompositorRenderPass& render_pass, FrameData& data)
{
    ...
    for (auto& [layer_id, layer_data] : data.render_pass_damage) {
        ...
        while (previous_data_itr != previous_data.cend() && previous_data_itr->first < layer_id) {
        ...
        bool layer_is_new = previous_data_itr == previous_data.cend() || previous_data_itr->first > layer_id;
    ...
    auto insert_result = data.current_frame_damage.try_emplace(render_pass.id.value(), std::move(data.render_pass_damage));
    ...
    render_pass.has_damage_from_contributing_content = !render_pass.damage_rect.IsEmpty();
}
