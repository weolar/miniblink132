...
public:
    ...
    void ExpectNeedsBeginFrameThenReset(const base::WeakPtr<TestFrameSinkImpl>& sink)
    {
    ...
    base::WeakPtr<TestFrameSinkImpl> SetupLayerTreeForDraw()
    {
        ...
        layer_tree_->SetViewportRectAndScale(viewport, scale_factor, local_surface_id);
        {
...
TEST_F(SlimLayerTreeTest, SmokeTest)
{
    ...
    layer_tree_->set_display_transform_hint(gfx::OVERLAY_TRANSFORM_FLIP_HORIZONTAL);
}
...
TEST_F(SlimLayerTreeTest, ReferencedSurfaceRange)
{
    ...
    EXPECT_THAT(layer_tree_->referenced_surfaces(), testing::UnorderedElementsAre(std::make_pair(viz::SurfaceRange(start, end), 1)));

    viz::SurfaceId new_end(viz::FrameSinkId(1u, 2u), viz::LocalSurfaceId(7u, 8u, token));
    layer->SetSurfaceId(new_end, cc::DeadlinePolicy::UseDefaultDeadline());
    EXPECT_THAT(layer_tree_->referenced_surfaces(), testing::UnorderedElementsAre(std::make_pair(viz::SurfaceRange(start, new_end), 1)));
