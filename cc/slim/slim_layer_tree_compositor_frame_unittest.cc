...
public:
    void SetUp() override
    {
        ...
        layer_tree_->SetViewportRectAndScale(viewport_, /*device_scale_factor=*/1.0f, local_surface_id_);
    }
    ...
    void IncrementLocalSurfaceId()
    {
        ...
        local_surface_id_
            = viz::LocalSurfaceId(local_surface_id_.parent_sequence_number(), local_surface_id_.child_sequence_number() + 1, local_surface_id_.embed_token());
        DCHECK(local_surface_id_.is_valid());
    }

    viz::CompositorFrame ProduceFrame(std::optional<viz::HitTestRegionList>* out_list = nullptr)
    {
        ...
        viz::BeginFrameArgs begin_frame_args = viz::BeginFrameArgs::Create(BEGINFRAME_FROM_HERE,
            /*source_id=*/1, ++sequence_id_, frame_time, frame_time + interval, interval, viz::BeginFrameArgs::NORMAL);
    ...
    scoped_refptr<SolidColorLayer> CreateSolidColorLayer(const gfx::Size& bounds, SkColor4f color)
    {
...
{
    auto solid_color_layer = CreateSolidColorLayer(viewport_.size(), SkColors::kGray);
    ...
    layer_tree_->SetViewportRectAndScale(viewport_, /*device_scale_factor=*/2.0f, local_surface_id_);
    layer_tree_->set_background_color(SkColors::kBlue);
    layer_tree_->set_display_transform_hint(gfx::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_90);
    {
        viz::CompositorFrame frame = ProduceFrame();
        viz::CompositorFrameMetadata& metadata = frame.metadata;
        EXPECT_NE(0u, metadata.frame_token);
        ...
        EXPECT_EQ(gfx::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_90, metadata.display_transform_hint);
    }
}
...
TEST_F(SlimLayerTreeCompositorFrameTest, OneSolidColorQuad)
{
    ...
    ASSERT_THAT(pass->quad_list,
        ElementsAre(AllOf(viz::IsSolidColorQuad(SkColors::kGray), viz::HasRect(viewport_), viz::HasVisibleRect(viewport_), viz::HasTransform(gfx::Transform()),
...
TEST_F(SlimLayerTreeCompositorFrameTest, LayerTransform)
{
    ...
    auto check_child_quad = [&](gfx::Rect expected_rect_in_root) {
        ...
        auto& pass = frame.render_pass_list.back();
        ASSERT_THAT(pass->quad_list,
            ElementsAre(AllOf(viz::IsSolidColorQuad(SkColors::kGreen), viz::HasRect(gfx::Rect(10, 20)), viz::HasVisibleRect(gfx::Rect(10, 20))),
                AllOf(viz::IsSolidColorQuad(SkColors::kGray), viz::HasRect(viewport_), viz::HasVisibleRect(viewport_))));
        ...
        gfx::Rect rect_in_root = shared_quad_state->quad_to_target_transform.MapRect(quad->rect);
...
TEST_F(SlimLayerTreeCompositorFrameTest, ChildOrder)
{
    ...
    scoped_refptr<SolidColorLayer> children[] = { CreateSolidColorLayer(gfx::Size(10, 10), SkColors::kBlue),
        CreateSolidColorLayer(gfx::Size(10, 10), SkColors::kGreen), CreateSolidColorLayer(gfx::Size(10, 10), SkColors::kMagenta),
        CreateSolidColorLayer(gfx::Size(10, 10), SkColors::kRed), CreateSolidColorLayer(gfx::Size(10, 10), SkColors::kYellow) };
    ...
    gfx::Point expected_origins[]
        = { gfx::Point(40.0f, 40.0f), gfx::Point(30.0f, 30.0f), gfx::Point(20.0f, 20.0f), gfx::Point(10.0f, 10.0f), gfx::Point(00.0f, 00.0f) };

    viz::CompositorFrame frame = ProduceFrame();
    ASSERT_EQ(frame.render_pass_list.size(), 1u);
    auto& pass = frame.render_pass_list.back();
    ASSERT_THAT(pass->quad_list,
        ElementsAre(viz::IsSolidColorQuad(SkColors::kBlue), viz::IsSolidColorQuad(SkColors::kGreen), viz::IsSolidColorQuad(SkColors::kMagenta),
            viz::IsSolidColorQuad(SkColors::kRed), viz::IsSolidColorQuad(SkColors::kYellow), viz::IsSolidColorQuad(SkColors::kGray)));

    for (size_t i = 0; i < std::size(expected_origins); ++i) {
        auto* quad = pass->quad_list.ElementAt(i);
        EXPECT_EQ(quad->shared_quad_state->quad_to_target_transform.MapPoint(gfx::Point()), expected_origins[i]);
    }
}
...
TEST_F(SlimLayerTreeCompositorFrameTest, AxisAlignedClip)
{
...

    {
        ...
        ASSERT_EQ(frame.render_pass_list.size(), 1u);
        auto& pass = frame.render_pass_list.back();
        ASSERT_THAT(pass->quad_list, ElementsAre(viz::IsSolidColorQuad(SkColors::kRed), viz::IsSolidColorQuad(SkColors::kGray)));
    ...
    clip_layer->SetPosition(gfx::PointF(5, 5));
    {
        ...
        EXPECT_EQ(quad->shared_quad_state->clip_rect.value(), gfx::Rect(5, 5, 10, 20));
    }
}
...
TEST_F(SlimLayerTreeCompositorFrameTest, PresentationCallback)
{
    ...
    layer_tree_->RequestPresentationTimeForNextFrame(base::BindLambdaForTesting([&](const gfx::PresentationFeedback& feedback) { feedback_opt_1 = feedback; }));
    layer_tree_->RequestPresentationTimeForNextFrame(base::BindLambdaForTesting([&](const gfx::PresentationFeedback& feedback) { feedback_opt_2 = feedback; }));
...
TEST_F(SlimLayerTreeCompositorFrameTest, SuccessPresentationCallback)
{
    ...
    layer_tree_->RequestSuccessfulPresentationTimeForNextFrame(
        base::BindLambdaForTesting([&](const viz::FrameTimingDetails& details) { feedback_time_opt_1 = details.presentation_feedback.timestamp; }));
    layer_tree_->RequestSuccessfulPresentationTimeForNextFrame(
        base::BindLambdaForTesting([&](const viz::FrameTimingDetails& details) { feedback_time_opt_2 = details.presentation_feedback.timestamp; }));
    viz::CompositorFrame frame1 = ProduceFrame();
    ...
    EXPECT_EQ(feedback_time_opt_1.value(), details.presentation_feedback.timestamp);
    EXPECT_EQ(feedback_time_opt_2.value(), details.presentation_feedback.timestamp);
}

TEST_F(SlimLayerTreeCompositorFrameTest, SuccessPresentationCallbackNotCalledForFailedFrame)
{
    auto solid_color_layer = CreateSolidColorLayer(viewport_.size(), SkColors::kGray);
    layer_tree_->SetRoot(solid_color_layer);
    ...
    {
        viz::FrameTimingDetailsMap timing_map;
        viz::FrameTimingDetails details = BuildFrameTimingDetails(gfx::PresentationFeedback::kFailure);
        timing_map[frame1.metadata.frame_token] = details;
        SetNextFrameTimingDetailsMap(std::move(timing_map));
        viz::CompositorFrame frame4 = ProduceFrame();
...
TEST_F(SlimLayerTreeCompositorFrameTest, CopyOutputRequest)
{
    ...
    auto copy_request_no_source_1 = std::make_unique<viz::CopyOutputRequest>(
        viz::CopyOutputRequest::ResultFormat::RGBA, viz::CopyOutputRequest::ResultDestination::kSystemMemory, base::DoNothing());
    ...
    auto copy_request_with_difference_source = std::make_unique<viz::CopyOutputRequest>(
        viz::CopyOutputRequest::ResultFormat::RGBA, viz::CopyOutputRequest::ResultDestination::kSystemMemory, base::DoNothing());
    ...
    layer_tree_->RequestCopyOfOutput(std::move(copy_request_with_difference_source));
...
TEST_F(SlimLayerTreeCompositorFrameTest, UIResourceLayerAppendQuads)
{
    ...
    {
        auto image_info = SkImageInfo::Make(1, 1, kN32_SkColorType, kPremul_SkAlphaType);
        SkBitmap bitmap;
        ...
        auto& pass = frame.render_pass_list.back();
        ASSERT_THAT(pass->quad_list,
            ElementsAre(AllOf(viz::IsTextureQuad(), viz::HasRect(viewport_), viz::HasVisibleRect(viewport_), viz::HasTransform(gfx::Transform()))));
        const viz::TextureDrawQuad* texture_quad = viz::TextureDrawQuad::MaterialCast(pass->quad_list.front());
        ...
        EXPECT_EQ(frame_sink_->uploaded_resources().begin()->second.viz_resource_id, texture_quad->resource_id());
    }

    ui_resource_layer->SetUV(gfx::PointF(0.25f, 0.25f), gfx::PointF(0.75f, 0.75f));
    {
        auto image_info = SkImageInfo::Make(2, 2, kN32_SkColorType, kPremul_SkAlphaType);
        SkBitmap bitmap;
        bitmap.allocPixels(image_info);
        bitmap.setImmutable();
        ui_resource_layer->SetBitmap(bitmap);
...
TEST_F(SlimLayerTreeCompositorFrameTest, ReclaimResources)
{
    ...
    for (size_t i = 0; i < kNumLayers; ++i) {
        EXPECT_TRUE(frame_sink_->client_resource_provider()->InUseByConsumer(frame.resource_list[i].id));
    }
    ...
    for (size_t i = 0; i < kNumLayers; i += 2) {
        EXPECT_FALSE(frame_sink_->client_resource_provider()->InUseByConsumer(frame.resource_list[i].id));
    }
...
TEST_F(SlimLayerTreeCompositorFrameTest, NinePatchLayerAppendQuads)
{
    ...
    auto image_info = SkImageInfo::Make(10, 10, kN32_SkColorType, kPremul_SkAlphaType);
    SkBitmap bitmap;
    bitmap.allocPixels(image_info);
    bitmap.setImmutable();
    ...
    ASSERT_THAT(pass->quad_list,
        ElementsAre(
            // Top left.
            AllOf(viz::IsTextureQuad(), viz::HasRect(gfx::Rect(10, 10)), viz::HasVisibleRect(gfx::Rect(10, 10))),
            // Top right.
            AllOf(viz::IsTextureQuad(), viz::HasRect(gfx::Rect(90, 0, 10, 10)), viz::HasVisibleRect(gfx::Rect(90, 0, 10, 10))),
            // Bottom left.
            AllOf(viz::IsTextureQuad(), viz::HasRect(gfx::Rect(0, 90, 10, 10)), viz::HasVisibleRect(gfx::Rect(0, 90, 10, 10))),
            // Bottom right.
            AllOf(viz::IsTextureQuad(), viz::HasRect(gfx::Rect(90, 90, 10, 10)), viz::HasVisibleRect(gfx::Rect(90, 90, 10, 10))),
            // Top.
            AllOf(viz::IsTextureQuad(), viz::HasRect(gfx::Rect(10, 0, 80, 10)), viz::HasVisibleRect(gfx::Rect(10, 0, 80, 10))),
            // Left.
            AllOf(viz::IsTextureQuad(), viz::HasRect(gfx::Rect(0, 10, 10, 80)), viz::HasVisibleRect(gfx::Rect(0, 10, 10, 80))),
            // Right.
            AllOf(viz::IsTextureQuad(), viz::HasRect(gfx::Rect(90, 10, 10, 80)), viz::HasVisibleRect(gfx::Rect(90, 10, 10, 80))),
            // Bottom.
            AllOf(viz::IsTextureQuad(), viz::HasRect(gfx::Rect(10, 90, 80, 10)), viz::HasVisibleRect(gfx::Rect(10, 90, 80, 10))),
            // Center.
            AllOf(viz::IsTextureQuad(), viz::HasRect(gfx::Rect(10, 10, 80, 80)))));
    ...
    for (size_t i = 0; i < std::size(expected_uv_top_left); ++i) {
        const viz::TextureDrawQuad* texture_quad = viz::TextureDrawQuad::MaterialCast(pass->quad_list.ElementAt(i));
        EXPECT_NE(viz::kInvalidResourceId, texture_quad->resource_id());
...
TEST_F(SlimLayerTreeCompositorFrameTest, SurfaceLayerAppendQuads)
{
...

    {
        ...
        ASSERT_EQ(frame.render_pass_list.size(), 1u);
        auto& pass = frame.render_pass_list.back();
        ASSERT_THAT(pass->quad_list, ElementsAre(AllOf(viz::IsSolidColorQuad(), viz::HasRect(viewport_), viz::HasVisibleRect(viewport_))));
    }

    base::UnguessableToken token = base::UnguessableToken::Create();
    viz::SurfaceId start(viz::FrameSinkId(1u, 2u), viz::LocalSurfaceId(3u, 4u, token));
    {
        viz::SurfaceId end(viz::FrameSinkId(1u, 2u), viz::LocalSurfaceId(5u, 6u, token));
        ...
        ASSERT_EQ(frame.render_pass_list.size(), 1u);
        auto& pass = frame.render_pass_list.back();
        ASSERT_THAT(pass->quad_list, ElementsAre(AllOf(viz::IsSurfaceQuad(), viz::HasRect(viewport_), viz::HasVisibleRect(viewport_))));
        ...
        viz::CompositorFrameMetadata& metadata = frame.metadata;
        EXPECT_EQ(metadata.referenced_surfaces, std::vector<viz::SurfaceRange> { viz::SurfaceRange(start, end) });
        EXPECT_EQ(metadata.activation_dependencies, std::vector<viz::SurfaceId> { end });
        EXPECT_FALSE(metadata.deadline.deadline_in_frames());
        EXPECT_TRUE(metadata.deadline.use_default_lower_bound_deadline());
    }

    {
        viz::SurfaceId end(viz::FrameSinkId(1u, 2u), viz::LocalSurfaceId(5u, 7u, token));
        cc::DeadlinePolicy deadline_policy = cc::DeadlinePolicy::UseSpecifiedDeadline(2u);
        surface_layer->SetSurfaceId(end, deadline_policy);
...
TEST_F(SlimLayerTreeCompositorFrameTest, SimpleHitTestRegionList)
{
    ...
    {
        base::UnguessableToken token = base::UnguessableToken::Create();
        viz::SurfaceId surface_id(viz::FrameSinkId(1u, 2u), viz::LocalSurfaceId(3u, 4u, token));
        cc::DeadlinePolicy deadline_policy = cc::DeadlinePolicy::UseDefaultDeadline();
    ...
    base::UnguessableToken token = base::UnguessableToken::Create();
    viz::SurfaceId surface_id(viz::FrameSinkId(2u, 3u), viz::LocalSurfaceId(4u, 5u, token));
    cc::DeadlinePolicy deadline_policy = cc::DeadlinePolicy::UseDefaultDeadline();
...

    {
        ...
        gfx::Transform expected_transform = gfx::Transform::MakeTranslation(5.0f, 5.0f);
        ...
        EXPECT_TRUE(child_region.async_hit_test_reasons | viz::AsyncHitTestReasons::kIrregularClip);
    }
}
...
TEST_F(SlimLayerTreeCompositorFrameTest, HitTestRegionInNonRootPass)
{
...

    {
        ...
        EXPECT_EQ(hit_test_region.transform, gfx::Transform::MakeScale(2.0f) * gfx::Transform::MakeTranslation(-10.0f, -10.0f));
    }
}
...
TEST_F(SlimLayerTreeCompositorFrameTest, NonInvertibleTransform)
{
    ...
    ASSERT_THAT(pass->quad_list,
        ElementsAre(
            AllOf(viz::IsSolidColorQuad(SkColors::kGray), viz::HasRect(viewport_), viz::HasVisibleRect(viewport_), viz::HasTransform(gfx::Transform()))));
}
...
TEST_F(SlimLayerTreeCompositorFrameTest, VisibleRect)
{
    ...
    auto& pass = frame.render_pass_list.back();
    ASSERT_THAT(pass->quad_list,
        ElementsAre(AllOf(viz::IsSolidColorQuad(SkColors::kRed), viz::HasRect(gfx::Rect(50, 50)), viz::HasVisibleRect(gfx::Rect(49, 49)),
    ...
    EXPECT_EQ(child_quad->shared_quad_state->visible_quad_layer_rect, gfx::Rect(49, 49));
...
TEST_F(SlimLayerTreeCompositorFrameTest, NonAxisAlignedClip)
{
    ...
    auto solid_color_layer = CreateSolidColorLayer(gfx::Size(80, 80), SkColors::kRed);
    ...
    gfx::Transform child_pass_transform = gfx::Transform::MakeTranslation(25.0f, 25.0f);
    child_pass_transform.PreConcat(transform);
    child_pass_transform.PreConcat(gfx::Transform::MakeTranslation(-25.0f, -25.0f));
    EXPECT_EQ(child_pass->transform_to_root_target, child_pass_transform);
    EXPECT_THAT(child_pass->quad_list,
        ElementsAre(AllOf(viz::IsSolidColorQuad(SkColors::kRed), viz::HasRect(gfx::Rect(80, 80)), viz::HasVisibleRect(gfx::Rect(50, 50)),
            viz::HasTransform(gfx::Transform()))));

    auto& root_pass = frame.render_pass_list.back();
    ASSERT_THAT(root_pass->quad_list,
        ElementsAre(AllOf(viz::IsCompositorRenderPassQuad(child_pass->id), viz::HasRect(gfx::Rect(50, 50)), viz::HasVisibleRect(gfx::Rect(50, 50)),
                        viz::HasTransform(child_pass_transform)),
            AllOf(viz::IsSolidColorQuad(SkColors::kGray), viz::HasRect(viewport_), viz::HasVisibleRect(viewport_), viz::HasTransform(gfx::Transform()))));
    auto* render_pass_quad = viz::CompositorRenderPassDrawQuad::MaterialCast(root_pass->quad_list.ElementAt(0));
...
TEST_F(SlimLayerTreeCompositorFrameTest, ChildPassOutputRect)
{
    ...
    ASSERT_THAT(child_pass->quad_list,
        ElementsAre(AllOf(viz::IsSolidColorQuad(SkColors::kRed), viz::HasRect(gfx::Rect(80, 80)),
            // Visible rect is clipped.
            viz::HasVisibleRect(gfx::Rect(30, 30)), viz::HasTransform(gfx::Transform::MakeTranslation(20.0f, 20.0f)))));
    {
        // SharedQuadState should match the quad.
        auto* shared_quad_state = child_pass->quad_list.ElementAt(0)->shared_quad_state;
    ...
    auto& root_pass = frame.render_pass_list.back();
    ASSERT_THAT(root_pass->quad_list,
        ElementsAre(AllOf(viz::IsCompositorRenderPassQuad(child_pass->id), viz::HasRect(gfx::Rect(20, 20, 30, 30)),
                        viz::HasVisibleRect(gfx::Rect(20, 20, 30, 30)), viz::HasTransform(child_pass_transform)),
            AllOf(viz::IsSolidColorQuad(SkColors::kGray), viz::HasRect(viewport_), viz::HasVisibleRect(viewport_), viz::HasTransform(gfx::Transform()))));
    {
        auto* render_pass_quad = viz::CompositorRenderPassDrawQuad::MaterialCast(root_pass->quad_list.ElementAt(0));
        auto* shared_quad_state = render_pass_quad->shared_quad_state;
        EXPECT_EQ(shared_quad_state->quad_layer_rect, gfx::Rect(20, 20, 30, 30));
        EXPECT_EQ(shared_quad_state->visible_quad_layer_rect, gfx::Rect(20, 20, 30, 30));
        EXPECT_EQ(shared_quad_state->clip_rect, std::nullopt);
    }
}
...
TEST_F(SlimLayerTreeCompositorFrameTest, Filters)
{
    ...
    // Pass is translated.
    gfx::Transform child_pass_transform = gfx::Transform::MakeTranslation(10.0f, 10.0f);
    EXPECT_EQ(child_pass->transform_to_root_target, child_pass_transform);
    ASSERT_THAT(child_pass->quad_list,
        ElementsAre(AllOf(viz::IsSolidColorQuad(SkColors::kRed), viz::HasRect(gfx::Rect(80, 80)),
            // Visible rect is clipped.
    ...
    EXPECT_THAT(child_pass->filters.operations(), ElementsAre(cc::FilterOperation::CreateBrightnessFilter(0.5f)));

    auto& root_pass = frame.render_pass_list.back();
    ASSERT_THAT(root_pass->quad_list,
        ElementsAre(AllOf(viz::IsCompositorRenderPassQuad(child_pass->id),
                        // Render pass quad matches pass size.
                        viz::HasRect(gfx::Rect(40, 40)), viz::HasVisibleRect(gfx::Rect(40, 40)),
...
TEST_F(SlimLayerTreeCompositorFrameTest, FiltersOnNonDrawingLayer)
{
    ...
    filter_layer->SetFilters({ cc::slim::Filter::CreateBrightness(0.5f) });
    auto solid_color_layer = CreateSolidColorLayer(gfx::Size(50, 50), SkColors::kRed);
...

    {
        ...
        auto& child_pass = frame.render_pass_list.front();
        ASSERT_THAT(child_pass->quad_list,
            ElementsAre(AllOf(viz::IsSolidColorQuad(SkColors::kRed), viz::HasRect(gfx::Rect(50, 50)), viz::HasVisibleRect(gfx::Rect(50, 50)))));
        EXPECT_EQ(child_pass->output_rect, gfx::Rect(50, 50));
        EXPECT_THAT(child_pass->filters.operations(), ElementsAre(cc::FilterOperation::CreateBrightnessFilter(0.5f)));
        auto& root_pass = frame.render_pass_list.back();
        ASSERT_THAT(root_pass->quad_list,
    ...
    filter_layer->SetMasksToBounds(true);
    {
        ...
        auto& child_pass = frame.render_pass_list.front();
        ASSERT_THAT(child_pass->quad_list,
            ElementsAre(AllOf(viz::IsSolidColorQuad(SkColors::kRed), viz::HasRect(gfx::Rect(50, 50)), viz::HasVisibleRect(gfx::Rect(25, 25)))));
        EXPECT_EQ(child_pass->output_rect, gfx::Rect(25, 25));
        EXPECT_THAT(child_pass->filters.operations(), ElementsAre(cc::FilterOperation::CreateBrightnessFilter(0.5f)));
        auto& root_pass = frame.render_pass_list.back();
        ASSERT_THAT(root_pass->quad_list,
            ElementsAre(AllOf(viz::IsCompositorRenderPassQuad(child_pass->id), viz::HasRect(gfx::Rect(25, 25)), viz::HasVisibleRect(gfx::Rect(25, 25))),
                viz::IsSolidColorQuad(SkColors::kGray)));
    }
    ...
    filter_layer->SetBounds(gfx::Size(0, 0));
    {
        ...
        ASSERT_EQ(frame.render_pass_list.size(), 1u);
        auto& root_pass = frame.render_pass_list.back();
        ASSERT_THAT(root_pass->quad_list, ElementsAre(viz::IsSolidColorQuad(SkColors::kGray)));
    }
}
...
TEST_F(SlimLayerTreeCompositorFrameTest, Opacity)
{
    ...
    // 2 drawing layers.
    auto grand_child_layer = CreateSolidColorLayer(gfx::Size(40, 40), SkColors::kGreen);
    ...
    ASSERT_THAT(child_pass->quad_list,
        ElementsAre(AllOf(viz::IsSolidColorQuad(SkColors::kGreen), viz::HasRect(gfx::Rect(40, 40)), viz::HasVisibleRect(gfx::Rect(40, 40)),
                        viz::HasTransform(gfx::Transform()),
                        // The pass is drawn with the child_layer's opacity, so there is
                        // no multiplicative opacity here.
                        viz::HasOpacity(0.5f)),
            AllOf(viz::IsSolidColorQuad(SkColors::kRed), viz::HasRect(gfx::Rect(80, 80)), viz::HasVisibleRect(gfx::Rect(80, 80)),
                viz::HasTransform(gfx::Transform()), viz::HasOpacity(1.0f))));

    auto& root_pass = frame.render_pass_list.back();
    ASSERT_THAT(root_pass->quad_list,
        ElementsAre(AllOf(viz::IsCompositorRenderPassQuad(child_pass->id), viz::HasRect(gfx::Rect(80, 80)), viz::HasVisibleRect(gfx::Rect(80, 80)),
...
TEST_F(SlimLayerTreeCompositorFrameTest, SimpleOcclusion)
{
    ...
    auto partially_occluded_layer = CreateSolidColorLayer(gfx::Size(50, 50), SkColors::kRed);
    partially_occluded_layer->SetPosition(gfx::PointF(25.0f, 25.0f));

    // Occlude top 10 pixels.
    auto sibling_occlusion_layer = CreateSolidColorLayer(gfx::Size(50, 10), SkColors::kGreen);
    // Position relative to root.
    sibling_occlusion_layer->SetPosition(gfx::PointF(25.0f, 25.0f));

    // Occlude the next top 10 pixels.
    auto child_occlusion_layer = CreateSolidColorLayer(gfx::Size(50, 10), SkColors::kBlue);
    ...
    auto& pass = frame.render_pass_list.back();
    ASSERT_THAT(pass->quad_list,
        ElementsAre(viz::IsSolidColorQuad(SkColors::kGreen), viz::IsSolidColorQuad(SkColors::kBlue),
            AllOf(viz::IsSolidColorQuad(SkColors::kRed), viz::HasRect(gfx::Rect(50, 50)), viz::HasVisibleRect(gfx::Rect(0, 20, 50, 30))),
            AllOf(viz::IsSolidColorQuad(SkColors::kGray), viz::HasRect(viewport_), viz::HasVisibleRect(viewport_))));
}
...
TEST_F(SlimLayerTreeCompositorFrameTest, OcclusionWithNonOpaqueLayer)
{
    ...
    auto middle_layer = CreateSolidColorLayer(gfx::Size(50, 50), SkColors::kGreen);
    ...
    EXPECT_THAT(pass->quad_list,
        ElementsAre(AllOf(viz::IsSolidColorQuad(SkColors::kBlue), viz::HasVisibleRect(gfx::Rect(50, 50))),
...
TEST_F(SlimLayerTreeCompositorFrameTest, OcclusionWithRenderPass)
{
    ...
    auto child_pass_layer = CreateSolidColorLayer(gfx::Size(200, 100), SkColors::kRed);
...

    {
        ...
        auto& child_pass = frame.render_pass_list.front();
        ASSERT_THAT(child_pass->quad_list,
            ElementsAre(AllOf(viz::IsSolidColorQuad(SkColors::kRed), viz::HasRect(gfx::Rect(200, 100)), viz::HasVisibleRect(gfx::Rect(200, 100)))));
    ...
    child_pass_layer->SetBounds(gfx::Size(200, 50));
    {
        ...
        auto& child_pass = frame.render_pass_list.front();
        ASSERT_THAT(child_pass->quad_list,
            ElementsAre(AllOf(viz::IsSolidColorQuad(SkColors::kRed), viz::HasRect(gfx::Rect(200, 50)),
        ...
        ASSERT_THAT(root_pass->quad_list,
            ElementsAre(AllOf(viz::IsCompositorRenderPassQuad(child_pass->id),
                            ...
                            viz::HasRect(gfx::Rect(100, 25)), viz::HasVisibleRect(gfx::Rect(100, 25)), viz::AreContentsOpaque(true)),
                AllOf(viz::IsSolidColorQuad(SkColors::kGray), viz::HasRect(viewport_),
                    // Occluded by child pass.
    ...
    // covered.
    auto child_pass_layer_2 = CreateSolidColorLayer(gfx::Size(100, 50), SkColors::kGreen);
    ...
    child_pass_root->AddChild(child_pass_layer_2);
    {
        ...
        auto& child_pass = frame.render_pass_list.front();
        ASSERT_THAT(child_pass->quad_list,
            ElementsAre(AllOf(viz::IsSolidColorQuad(SkColors::kGreen), viz::HasRect(gfx::Rect(100, 50)), viz::HasVisibleRect(gfx::Rect(100, 50))),
                AllOf(viz::IsSolidColorQuad(SkColors::kRed), viz::HasRect(gfx::Rect(200, 50)), viz::HasVisibleRect(gfx::Rect(200, 50)))));

        auto& root_pass = frame.render_pass_list.back();
        ASSERT_THAT(root_pass->quad_list,
            ElementsAre(AllOf(viz::IsCompositorRenderPassQuad(child_pass->id), viz::HasRect(gfx::Rect(100, 50)), viz::HasVisibleRect(gfx::Rect(100, 50)),
    ...
    auto child_pass_layer_3 = CreateSolidColorLayer(gfx::Size(100, 50), SkColors::kBlue);
    ...
    child_pass_root->AddChild(child_pass_layer_3);
    {
        ...
        auto& child_pass = frame.render_pass_list.front();
        ASSERT_THAT(child_pass->quad_list,
            ElementsAre(AllOf(viz::IsSolidColorQuad(SkColors::kBlue), viz::HasRect(gfx::Rect(100, 50)), viz::HasVisibleRect(gfx::Rect(100, 50))),
                AllOf(viz::IsSolidColorQuad(SkColors::kGreen), viz::HasRect(gfx::Rect(100, 50)), viz::HasVisibleRect(gfx::Rect(100, 50))),
                AllOf(viz::IsSolidColorQuad(SkColors::kRed), viz::HasRect(gfx::Rect(200, 50)), viz::HasVisibleRect(gfx::Rect(200, 50)))));
    ...
    child_pass_layer->SetBounds(gfx::Size(200, 100));
    {
        ...
        ASSERT_THAT(child_pass->quad_list,
            ElementsAre(AllOf(viz::IsSolidColorQuad(SkColors::kBlue), viz::HasRect(gfx::Rect(100, 50)), viz::HasVisibleRect(gfx::Rect(100, 50))),
                AllOf(viz::IsSolidColorQuad(SkColors::kGreen), viz::HasRect(gfx::Rect(100, 50)), viz::HasVisibleRect(gfx::Rect(100, 50))),
                AllOf(viz::IsSolidColorQuad(SkColors::kRed), viz::HasRect(gfx::Rect(200, 100)),
                    // Partially occluded.
                    viz::HasVisibleRect(gfx::Rect(200, 50)))));
...
TEST_F(SlimLayerTreeCompositorFrameTest, OccludedNonOpaqueBackgroundColor)
{
    ...
    ASSERT_EQ(frame.render_pass_list.size(), 1u);
    auto& pass = frame.render_pass_list.back();
    ASSERT_THAT(pass->quad_list, ElementsAre(viz::IsSolidColorQuad(SkColors::kGray)));
...
TEST_F(SlimLayerTreeCompositorFrameTest, Guttering)
{
    ...
    EXPECT_THAT(pass->quad_list,
        ElementsAre(AllOf(viz::IsSolidColorQuad(SkColors::kRed), viz::HasRect(gfx::Rect(50, 50)), viz::HasVisibleRect(gfx::Rect(50, 50))),
            ...
            viz::IsSolidColorQuad(SkColors::kBlue), viz::IsSolidColorQuad(SkColors::kBlue), viz::IsSolidColorQuad(SkColors::kBlue),
...
TEST_F(SlimLayerTreeCompositorFrameTest, PropertyDamage)
{
    ...
    auto check_frame = [&](SkColor4f color, gfx::Rect damage) {
        ...
        auto& pass = frame.render_pass_list.back();
        EXPECT_THAT(pass->quad_list,
            ElementsAre(AllOf(viz::IsSolidColorQuad(color), viz::HasRect(gfx::Rect(50, 50))),
                AllOf(viz::IsSolidColorQuad(SkColors::kGray), viz::HasRect(viewport_), viz::HasVisibleRect(viewport_), viz::HasTransform(gfx::Transform()))));
...
TEST_F(SlimLayerTreeCompositorFrameTest, PropertyChangeFromParentDamage)
{
    ...
    auto check_frame = [&](gfx::Rect damage) {
        ...
        auto& pass = frame.render_pass_list.back();
        EXPECT_THAT(pass->quad_list,
            ElementsAre(AllOf(viz::IsSolidColorQuad(SkColors::kRed), viz::HasRect(gfx::Rect(50, 50))),
                AllOf(viz::IsSolidColorQuad(SkColors::kGray), viz::HasRect(viewport_), viz::HasVisibleRect(viewport_), viz::HasTransform(gfx::Transform()))));
        EXPECT_EQ(pass->damage_rect, damage);
    };
...
TEST_F(SlimLayerTreeCompositorFrameTest, NonRootPassDamage)
{
    ...
    parent->SetFilters({ cc::slim::Filter::CreateBrightness(0.5f) });
    {
        ...
        auto& root_pass = frame.render_pass_list.back();
        EXPECT_THAT(root_pass->quad_list,
            ElementsAre(AllOf(viz::IsCompositorRenderPassQuad(child_pass->id), viz::HasRect(gfx::Rect(50, 50))),
                AllOf(viz::IsSolidColorQuad(SkColors::kGray), viz::HasRect(viewport_), viz::HasVisibleRect(viewport_), viz::HasTransform(gfx::Transform()))));
    ...
    solid_color_layer->SetBackgroundColor(SkColors::kBlue);
    {
        ...
        auto& child_pass = frame.render_pass_list.front();
        EXPECT_THAT(child_pass->quad_list,
            ElementsAre(AllOf(viz::IsSolidColorQuad(SkColors::kBlue), viz::HasRect(gfx::Rect(50, 50)), viz::HasVisibleRect(gfx::Rect(50, 50)))));
        EXPECT_EQ(child_pass->damage_rect, gfx::Rect(50, 50));
    ...
    root_layer->AddChild(CreateSolidColorLayer(gfx::Size(10, 10), SkColors::kGreen));
    {
        viz::CompositorFrame frame = ProduceFrame();
        ASSERT_EQ(frame.render_pass_list.size(), 2u);
        auto& child_pass = frame.render_pass_list.front();
        ...
        auto& root_pass = frame.render_pass_list.back();
        EXPECT_THAT(root_pass->quad_list,
            ElementsAre(AllOf(viz::IsSolidColorQuad(SkColors::kGreen), viz::HasRect(gfx::Rect(10, 10)), viz::HasVisibleRect(gfx::Rect(10, 10))),
                AllOf(viz::IsCompositorRenderPassQuad(child_pass->id), viz::HasRect(gfx::Rect(50, 50))),
                AllOf(viz::IsSolidColorQuad(SkColors::kGray), viz::HasRect(viewport_), viz::HasVisibleRect(viewport_), viz::HasTransform(gfx::Transform()))));
...
TEST_F(SlimLayerTreeCompositorFrameTest, SimpleRoundedCorner)
{
    ...
    ASSERT_THAT(pass->quad_list,
        ElementsAre(
            AllOf(viz::IsSolidColorQuad(SkColors::kRed), viz::HasRect(gfx::Rect(50, 50)), viz::HasTransform(gfx::Transform::MakeTranslation(10.0f, 10.0f))),
            AllOf(viz::IsSolidColorQuad(SkColors::kGray), viz::HasRect(viewport_), viz::HasVisibleRect(viewport_), viz::HasTransform(gfx::Transform()))));
    auto* quad = pass->quad_list.front();
    auto* shared_quad_state = quad->shared_quad_state;
    EXPECT_TRUE(shared_quad_state->mask_filter_info.HasRoundedCorners());
    EXPECT_TRUE(shared_quad_state->is_fast_rounded_corner);
    EXPECT_EQ(shared_quad_state->mask_filter_info.rounded_corner_bounds(), gfx::RRectF(10.0f, 10.0f, 50.0f, 50.0f, 20.0f));
}
...
TEST_F(SlimLayerTreeCompositorFrameTest, RoundedCornerWithChild)
{
    ...
    auto rounded_corner_layer = CreateSolidColorLayer(gfx::Size(50, 50), SkColors::kRed);
    ...
    auto& pass = frame.render_pass_list.back();
    ASSERT_THAT(pass->quad_list,
        ElementsAre(AllOf(viz::IsSolidColorQuad(SkColors::kBlue), viz::HasRect(gfx::Rect(50, 50)), viz::HasVisibleRect(gfx::Rect(40, 40)),
                        viz::HasTransform(gfx::Transform::MakeTranslation(20.0f, 20.0f))),
            AllOf(viz::IsSolidColorQuad(SkColors::kRed), viz::HasRect(gfx::Rect(50, 50)), viz::HasTransform(gfx::Transform::MakeTranslation(10.0f, 10.0f))),
            AllOf(viz::IsSolidColorQuad(SkColors::kGray), viz::HasRect(viewport_), viz::HasVisibleRect(viewport_), viz::HasTransform(gfx::Transform()))));

    const gfx::RRectF expected_rounded_conrer_in_target(10.0f, 10.0f, 50.0f, 50.0f, 20.0f);
    {
        auto* quad = pass->quad_list.front();
        auto* shared_quad_state = quad->shared_quad_state;
        EXPECT_TRUE(shared_quad_state->mask_filter_info.HasRoundedCorners());
        EXPECT_TRUE(shared_quad_state->is_fast_rounded_corner);
        EXPECT_EQ(shared_quad_state->mask_filter_info.rounded_corner_bounds(), expected_rounded_conrer_in_target);
    }
...
TEST_F(SlimLayerTreeCompositorFrameTest, NonAxisAlignedRoundedCorner)
{
            ...
            viz::HasTransform(gfx::Transform()))));
    {
        ...
        EXPECT_TRUE(shared_quad_state->mask_filter_info.HasRoundedCorners());
        EXPECT_TRUE(shared_quad_state->is_fast_rounded_corner);
        EXPECT_EQ(shared_quad_state->mask_filter_info.rounded_corner_bounds(), gfx::RRectF(0.0f, 0.0f, 50.0f, 50.0f, 20.0f));
    }
    ...
    child_pass_transform.PreConcat(gfx::Transform::MakeTranslation(-25.0f, -25.0f));
    ASSERT_THAT(root_pass->quad_list,
        ElementsAre(AllOf(viz::IsCompositorRenderPassQuad(child_pass->id), viz::HasRect(gfx::Rect(50, 50)), viz::HasTransform(child_pass_transform)),
            AllOf(viz::IsSolidColorQuad(SkColors::kGray), viz::HasRect(viewport_), viz::HasVisibleRect(viewport_), viz::HasTransform(gfx::Transform()))));
    {
...
TEST_F(SlimLayerTreeCompositorFrameTest, RoundedCornerOnParentAndChild)
{
            ...
            viz::HasTransform(gfx::Transform()))));
    {
        ...
        EXPECT_TRUE(shared_quad_state->mask_filter_info.HasRoundedCorners());
        EXPECT_TRUE(shared_quad_state->is_fast_rounded_corner);
        EXPECT_EQ(shared_quad_state->mask_filter_info.rounded_corner_bounds(), gfx::RRectF(0.0f, 0.0f, 50.0f, 50.0f, 15.0f));
    }

    auto& root_pass = frame.render_pass_list.back();
    ASSERT_THAT(root_pass->quad_list,
        ElementsAre(AllOf(viz::IsCompositorRenderPassQuad(child_pass->id), viz::HasRect(gfx::Rect(40, 40)), viz::HasVisibleRect(gfx::Rect(40, 40)),
                        viz::HasTransform(gfx::Transform::MakeTranslation(20.0f, 20.0f))),
            AllOf(viz::IsSolidColorQuad(SkColors::kRed), viz::HasRect(gfx::Rect(50, 50)), viz::HasTransform(gfx::Transform::MakeTranslation(10.0f, 10.0f))),
            AllOf(viz::IsSolidColorQuad(SkColors::kGray), viz::HasRect(viewport_), viz::HasVisibleRect(viewport_), viz::HasTransform(gfx::Transform()))));
...
TEST_F(SlimLayerTreeCompositorFrameTest, GradientMaskWithChild)
{
    ...
    auto gradient_layer = CreateSolidColorLayer(gfx::Size(50, 50), SkColors::kRed);
...
TEST_F(SlimLayerTreeCompositorFrameTest, GradientMaskOnParentAndChild)
{
            ...
            viz::HasTransform(gfx::Transform()))));
    {
        ...
        auto* shared_quad_state = quad->shared_quad_state;
        EXPECT_TRUE(shared_quad_state->mask_filter_info.HasGradientMask());
        EXPECT_EQ(shared_quad_state->mask_filter_info.gradient_mask(), child_gradient);
    }
            ...
            AllOf(viz::IsSolidColorQuad(SkColors::kGray), viz::HasRect(viewport_), viz::HasVisibleRect(viewport_), viz::HasTransform(gfx::Transform()))));
    {
        ...
        auto* shared_quad_state = quad->shared_quad_state;
        EXPECT_TRUE(shared_quad_state->mask_filter_info.HasGradientMask());
        EXPECT_EQ(shared_quad_state->mask_filter_info.gradient_mask(), parent_gradient);
    }
...
TEST_F(SlimLayerTreeCompositorFrameTest, AddRemoveSurfaceRangesTriggerSetNeedsDraw)
{
    auto surface_layer = SurfaceLayer::Create();
    surface_layer->SetBounds(viewport_.size());
    surface_layer->SetIsDrawable(true);
    ...
    // Add/remove a SurfaceRange different from the one of the `surface_layer`.
    {
        ...
        EXPECT_THAT(frame.metadata.referenced_surfaces, testing::UnorderedElementsAre(viz::SurfaceRange(start, end), viz::SurfaceRange(end, end)));
    }
    {
        layer_tree_->RemoveSurfaceRange(viz::SurfaceRange(end, end));
        const viz::CompositorFrame frame = ProduceFrame();
        EXPECT_THAT(frame.metadata.referenced_surfaces, testing::UnorderedElementsAre(viz::SurfaceRange(start, end)));
    }
...
TEST_F(SlimLayerTreeCompositorFrameTest, OffsetTagLayers)
{
    ...
    const viz::SurfaceId surface_id(frame_sink_id, viz::LocalSurfaceId(3u, 4u, token));
    surface_layer->SetSurfaceId(surface_id, cc::DeadlinePolicy::UseDefaultDeadline());
    surface_layer->SetIsDrawable(true);
...

    {
        ...
        EXPECT_THAT(root_pass->quad_list,
            testing::ElementsAre(
                ...
                testing::AllOf(viz::IsSolidColorQuad(SkColors::kRed), viz::HasOffsetTag({}))));
    }
...

    {
        ...
        EXPECT_THAT(root_pass->quad_list,
            testing::ElementsAre(
                ...
                testing::AllOf(viz::IsSolidColorQuad(SkColors::kRed), viz::HasOffsetTag(offset_tag)),
                // Quad for `background_layer`. Since `solid_color_layer` has an
                // offset tag background layer is no longer occluded.
                testing::AllOf(viz::IsSolidColorQuad(SkColors::kBlack), viz::HasOffsetTag({}))));
    }
...

    {
        ...
        EXPECT_THAT(root_pass->quad_list,
            testing::ElementsAre(
                ...
                testing::AllOf(viz::IsSolidColorQuad(SkColors::kGreen), viz::HasOffsetTag({}))));
    }
...

    {
        ...
        EXPECT_THAT(root_pass->quad_list,
            testing::ElementsAre(
                ...
                testing::AllOf(viz::IsCompositorRenderPassQuad(child_pass->id), viz::HasOffsetTag(offset_tag)),
                // Quad for `solid_color_layer`.
                testing::AllOf(viz::IsSolidColorQuad(SkColors::kRed), viz::HasOffsetTag(offset_tag)),
                // Generated background quad that's green since `solid_color_layer`
                // no longer counts as opaque and `background_layer` isn't drawn.
        ...
        EXPECT_THAT(child_pass->quad_list,
            testing::ElementsAre(
                ...
                testing::AllOf(viz::IsSolidColorQuad(SkColors::kBlue), viz::HasVisibleRect(gfx::Rect(10, 10)), viz::HasOffsetTag({}))));
    }
}
...
TEST_F(SlimLayerTreeCompositorFrameTest, OffsetTagVisibleRect)
{
    ...
    surface_layer->SetBounds(viewport_.size());
    const viz::FrameSinkId frame_sink_id(1u, 2u);
    const viz::SurfaceId surface_id(frame_sink_id, viz::LocalSurfaceId(3u, 4u, base::UnguessableToken::Create()));
    surface_layer->SetSurfaceId(surface_id, cc::DeadlinePolicy::UseDefaultDeadline());
    surface_layer->SetIsDrawable(true);
    ...
    outside_viewport_layer->SetTransform(gfx::Transform::MakeTranslation(-10, -10));
...

    {
        ...
        EXPECT_THAT(root_pass->quad_list, testing::ElementsAre(testing::AllOf(viz::HasVisibleRect(gfx::Rect(10, 10, 100, 100)))));
    }

    {
        ...
        EXPECT_THAT(root_pass->quad_list,
            testing::ElementsAre(
                testing::AllOf(viz::IsSolidColorQuad(), viz::HasOffsetTag(offset_tag), viz::HasVisibleRect(gfx::Rect(0, 0, 140, 140))), viz::IsSurfaceQuad()));
...
TEST_F(SlimLayerTreeCompositorFrameTest, OffsetTagClipping)
{
    ...
    EXPECT_THAT(root_pass->quad_list,
        testing::ElementsAre(
            ...
            testing::AllOf(viz::IsSolidColorQuad(SkColors::kRed), viz::HasOffsetTag(offset_tag), viz::HasVisibleRect(gfx::Rect(5, 5, 35, 35)),
            ...
            testing::AllOf(viz::IsSurfaceQuad(), viz::HasOffsetTag(offset_tag), viz::HasVisibleRect(gfx::Rect(surface_layer->bounds())),
                viz::HasClipRect(parent_clip_rect))));
}
