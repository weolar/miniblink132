...
FakeLayerTreeHostImpl::FakeLayerTreeHostImpl(TaskRunnerProvider* task_runner_provider, TaskGraphRunner* task_graph_runner)
    : FakeLayerTreeHostImpl(CommitToPendingTreeLayerListSettings(), task_runner_provider, task_graph_runner)
{
}

FakeLayerTreeHostImpl::FakeLayerTreeHostImpl(const LayerTreeSettings& settings, TaskRunnerProvider* task_runner_provider, TaskGraphRunner* task_graph_runner)
    : FakeLayerTreeHostImpl(settings, task_runner_provider, task_graph_runner, nullptr)
{
}

FakeLayerTreeHostImpl::FakeLayerTreeHostImpl(const LayerTreeSettings& settings, TaskRunnerProvider* task_runner_provider, TaskGraphRunner* task_graph_runner,
    scoped_refptr<base::SequencedTaskRunner> image_worker_task_runner)
    : LayerTreeHostImpl(settings, &client_, task_runner_provider, &stats_instrumentation_, task_graph_runner,
        AnimationHost::CreateForTesting(ThreadInstance::kImpl), nullptr, 0, std::move(image_worker_task_runner),
    ...
    , notify_tile_state_changed_called_(false)
{
    ...
    WillBeginImplFrame(viz::CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 1, time_ticks));
}
...
void FakeLayerTreeHostImpl::CreatePendingTree()
{
    ...
    pending_tree()->PushPageScaleFromMainThread(1.f, 1.f / arbitrary_large_page_scale, arbitrary_large_page_scale);
...
TargetColorParams FakeLayerTreeHostImpl::GetTargetColorParams(gfx::ContentColorUsage content_color_usage) const
{
    return target_color_params_.value_or(LayerTreeHostImpl::GetTargetColorParams(content_color_usage));
}

const viz::BeginFrameArgs& FakeLayerTreeHostImpl::CurrentBeginFrameArgs() const
{
...
{
    viz::BeginFrameArgs next_begin_frame_args = current_begin_frame_tracker_.Current();
