...
PixelTestOutputSurface::PixelTestOutputSurface(std::unique_ptr<viz::SoftwareOutputDevice> software_device)
...
void PixelTestOutputSurface::SwapBuffers(viz::OutputSurfaceFrame frame)
{
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&PixelTestOutputSurface::SwapBuffersCallback, weak_ptr_factory_.GetWeakPtr()));
}
...
void PixelTestOutputSurface::SwapBuffersCallback()
{
    ...
    client_->DidReceivePresentationFeedback(gfx::PresentationFeedback(base::TimeTicks::Now(), base::TimeDelta(), 0));
}

void PixelTestOutputSurface::SetUpdateVSyncParametersCallback(viz::UpdateVSyncParametersCallback callback)
{
}
...
void PixelTestOutputSurface::ReadbackForTesting(base::OnceCallback<void(std::unique_ptr<viz::CopyOutputResult>)> callback)
{
    SkBitmap bitmap = software_device()->ReadbackForTesting();
    std::move(callback).Run(std::make_unique<viz::CopyOutputSkBitmapResult>(gfx::Rect(gfx::SkISizeToSize(bitmap.dimensions())), std::move(bitmap)));
}
