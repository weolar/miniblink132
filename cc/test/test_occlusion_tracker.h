...
public:
    ...
    SimpleEnclosedRegion occlusion_on_contributing_surface_from_inside_target() const
    {
    ...
    SimpleEnclosedRegion occlusion_on_contributing_surface_from_outside_target() const
    {
        size_t stack_size = stack_.size();
        if (stack_size < 2)
            return SimpleEnclosedRegion();
    ...
    void set_occlusion_on_contributing_surface_from_outside_target(const SimpleEnclosedRegion& region)
    {
        size_t stack_size = stack_.size();
        DCHECK_GE(stack_size, 2u);
        stack_[stack_size - 2].occlusion_from_outside_target = region;
    }
    void set_occlusion_on_contributing_surface_from_inside_target(const SimpleEnclosedRegion& region)
    {
        size_t stack_size = stack_.size();
        DCHECK_GE(stack_size, 2u);
