...
static bool CanRectFBeSafelyRoundedToRect(const gfx::RectF& r)
{
    ...
    gfx::RectF floored_rect(std::floor(r.x()), std::floor(r.y()), std::floor(r.width()), std::floor(r.height()));
    return floored_rect == r;
}

void VerifyQuadsExactlyCoverRect(const viz::QuadList& quads, const gfx::Rect& rect)
{
    ...
    for (auto iter = quads.cbegin(); iter != quads.cend(); ++iter) {
        ...
        gfx::RectF quad_rectf = MathUtil::MapClippedRect(iter->shared_quad_state->quad_to_target_transform, gfx::RectF(iter->visible_rect));
        ...
        EXPECT_TRUE(rect.Contains(quad_rect)) << kQuadString << iter.index() << " rect: " << rect.ToString() << " quad: " << quad_rect.ToString();
        EXPECT_TRUE(remaining.Contains(quad_rect)) << kQuadString << iter.index() << " remaining: " << remaining.ToString()
...
// static
void VerifyQuadsAreOccluded(const viz::QuadList& quads, const gfx::Rect& occluded, size_t* partially_occluded_count)
{
    // No quad should exist if it's fully occluded.
    for (auto* quad : quads) {
        gfx::Rect target_visible_rect = MathUtil::MapEnclosingClippedRect(quad->shared_quad_state->quad_to_target_transform, quad->visible_rect);
        EXPECT_FALSE(occluded.Contains(target_visible_rect));
    }

    // Quads that are fully occluded on one axis only should be shrunken.
    for (auto* quad : quads) {
        gfx::Rect target_rect = MathUtil::MapEnclosingClippedRect(quad->shared_quad_state->quad_to_target_transform, quad->rect);
        if (!quad->shared_quad_state->quad_to_target_transform.IsIdentityOrIntegerTranslation()) {
            DCHECK(quad->shared_quad_state->quad_to_target_transform.IsPositiveScaleOrTranslation())
                << quad->shared_quad_state->quad_to_target_transform.ToString();
            gfx::RectF target_rectf = MathUtil::MapClippedRect(quad->shared_quad_state->quad_to_target_transform, gfx::RectF(quad->rect));
...
void PrepareForUpdateDrawProperties(LayerTreeImpl* layer_tree_impl)
{
    ...
    property_trees->effect_tree_mutable().CreateOrReuseRenderSurfaces(&old_render_surfaces, layer_tree_impl);
    layer_tree_impl->MoveChangeTrackingToLayers();
    property_trees->ResetCachedData();
}

void UpdateDrawProperties(LayerTreeImpl* layer_tree_impl, LayerImplList* output_update_layer_list)
{
    PrepareForUpdateDrawProperties(layer_tree_impl);
    layer_tree_impl->UpdateDrawProperties(
        /*update_tiles=*/true, /*update_image_animation_controller=*/true, output_update_layer_list);
}

void UpdateDrawProperties(LayerTreeHost* layer_tree_host, LayerList* output_update_layer_list)
{
    ...
    draw_property_utils::FindLayersThatNeedUpdates(layer_tree_host, &update_layer_list);

    if (output_update_layer_list)
        *output_update_layer_list = std::move(update_layer_list);
}

void SetDeviceScaleAndUpdateViewportRect(LayerTreeImpl* layer_tree_impl, float device_scale_factor)
{
    layer_tree_impl->SetDeviceScaleFactor(device_scale_factor);
    gfx::Size root_bounds = layer_tree_impl->root_layer()->bounds();
    layer_tree_impl->SetDeviceViewportRect(gfx::Rect(root_bounds.width() * device_scale_factor, root_bounds.height() * device_scale_factor));
}

void SetDeviceScaleAndUpdateViewportRect(LayerTreeHost* layer_tree_host, float device_scale_factor)
{
    gfx::Size root_bounds = layer_tree_host->root_layer()->bounds();
    gfx::Rect viewport_rect(root_bounds.width() * device_scale_factor, root_bounds.height() * device_scale_factor);
    layer_tree_host->SetViewportRectAndScale(viewport_rect, device_scale_factor, viz::LocalSurfaceId());
}
