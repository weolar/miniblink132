...
void TestLayerTreeHostBase::SetUp()
{
    ...
    host_impl_ = CreateHostImpl(CreateSettings(), &task_runner_provider_, task_graph_runner_.get());
    InitializeFrameSink();
...
std::unique_ptr<LayerTreeFrameSink> TestLayerTreeHostBase::CreateLayerTreeFrameSink()
{
    return FakeLayerTreeFrameSink::Create3d();
}

std::unique_ptr<FakeLayerTreeHostImpl> TestLayerTreeHostBase::CreateHostImpl(
    const LayerTreeSettings& settings, TaskRunnerProvider* task_runner_provider, TaskGraphRunner* task_graph_runner)
{
    return std::make_unique<FakeLayerTreeHostImpl>(settings, task_runner_provider, task_graph_runner);
}

std::unique_ptr<TaskGraphRunner> TestLayerTreeHostBase::CreateTaskGraphRunner()
{
...
void TestLayerTreeHostBase::ResetLayerTreeFrameSink(std::unique_ptr<LayerTreeFrameSink> layer_tree_frame_sink)
{
...
void TestLayerTreeHostBase::SetupDefaultTrees(const gfx::Size& layer_bounds)
{
    ...
    scoped_refptr<FakeRasterSource> active_raster_source = FakeRasterSource::CreateFilledWithText(layer_bounds);

    SetupTrees(std::move(pending_raster_source), std::move(active_raster_source));
}

void TestLayerTreeHostBase::SetupTrees(scoped_refptr<RasterSource> pending_raster_source, scoped_refptr<RasterSource> active_raster_source)
{
...
void TestLayerTreeHostBase::SetupSyncTree(scoped_refptr<RasterSource> raster_source, const gfx::Size& tile_size, const Region& invalidation)
{
    if (host_impl()->CommitsToActiveTree()) {
        host_impl()->active_tree()->PushPageScaleFromMainThread(1.f, 0.00001f, 100000.f);
    } else {
        host_impl()->CreatePendingTree();
        host_impl()->pending_tree()->PushPageScaleFromMainThread(1.f, 0.00001f, 100000.f);
        host_impl()->pending_tree()->SetDeviceViewportRect(host_impl()->active_tree()->GetDeviceViewport());
        host_impl()->pending_tree()->SetDeviceScaleFactor(host_impl()->active_tree()->device_scale_factor());
    }
...
void TestLayerTreeHostBase::ActivateTree()
{
    ...
    pending_layer_ = nullptr;
    active_layer_ = static_cast<FakePictureLayerImpl*>(host_impl()->active_tree()->LayerById(old_pending_layer_->id()));
...
void TestLayerTreeHostBase::PerformImplSideInvalidation()
{
    ...
    host_impl()->CreatePendingTree();
    host_impl()->sync_tree()->InvalidateRegionForImages(host_impl()->tile_manager()->TakeImagesToInvalidateOnSyncTree());
    pending_layer_ = old_pending_layer_;
    old_pending_layer_ = nullptr;
}
...
void TestLayerTreeHostBase::SetInitialTreePriority()
{
    ...
    host_impl_->resource_pool()->SetResourceUsageLimits(state.soft_memory_limit_in_bytes, state.num_resources_limit);
