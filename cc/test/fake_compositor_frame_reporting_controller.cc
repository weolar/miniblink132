...
void FakeCompositorFrameReportingController::WillBeginMainFrame(const viz::BeginFrameArgs& args)
{
...
void FakeCompositorFrameReportingController::BeginMainFrameAborted(const viz::BeginFrameId& id, CommitEarlyOutReason reason)
{
...
void FakeCompositorFrameReportingController::WillActivate()
{
    ...
    // commit.
    if (!HasReporterAt(PipelineStage::kCommit) && !next_activate_has_invalidation())
        DidCommit();
...
{
    CompositorFrameReportingController::DidSubmitCompositorFrame(submit_info, current_frame_id, last_activated_frame_id);

    viz::FrameTimingDetails details;
    details.presentation_feedback.timestamp = base::TimeTicks::Now();
    CompositorFrameReportingController::DidPresentCompositorFrame(submit_info.frame_token, details);
}

void FakeCompositorFrameReportingController::DidPresentCompositorFrame(uint32_t frame_token, const viz::FrameTimingDetails& details)
{
}
} // namespace cc
