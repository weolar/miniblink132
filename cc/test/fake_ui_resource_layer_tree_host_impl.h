...
public:
    explicit FakeUIResourceLayerTreeHostImpl(TaskRunnerProvider* task_runner_provider, TaskGraphRunner* task_graph_runner);
    ~FakeUIResourceLayerTreeHostImpl() override;

    void CreateUIResource(UIResourceId uid, const UIResourceBitmap& bitmap) override;
...
private:
    std::unordered_map<UIResourceId, LayerTreeHostImpl::UIResourceData> fake_ui_resource_map_;
};
