...
public:
    struct ImageData {
        ImageData(PaintImage image, const gfx::Point& point, const SkSamplingOptions&, const PaintFlags& flags);
        ImageData(PaintImage image, const gfx::Transform& transform, const SkSamplingOptions&, const PaintFlags& flags);
    ...
    struct SkottieData {
        SkottieData(scoped_refptr<SkottieWrapper> skottie, const gfx::Rect& dst, float t, SkottieFrameDataMap images, SkottieColorMap color_map,
    ...
    void set_has_draw_text_op()
    {
        has_draw_text_op_ = true;
    }
    ...
    void add_draw_image(sk_sp<SkImage> image, const gfx::Point& point, const SkSamplingOptions& sampling, const PaintFlags& flags)
    {
        add_draw_image(
            PaintImageBuilder::WithDefault().set_id(PaintImage::GetNextId()).set_image(std::move(image), PaintImage::GetNextContentId()).TakePaintImage(),
    ...
    void add_draw_image(PaintImage image, const gfx::Point& point, const SkSamplingOptions& sampling, const PaintFlags& flags)
    {
    ...
    void add_draw_image_with_transform(PaintImage image, const gfx::Transform& transform, const SkSamplingOptions& sampling, const PaintFlags& flags)
    {
