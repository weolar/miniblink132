...
public:
    struct TaskInfo {
        TaskInfo(int namespace_index, unsigned id, unsigned dependent_id, unsigned dependent_count, unsigned category, unsigned priority,
...
protected:
    ...
    public:
        FakeDependentTaskImpl(TaskGraphRunnerTestBase* test, int namespace_index, int id)
...
template <typename TaskRunnerTestDelegate> class TaskGraphRunnerTest : public TaskGraphRunnerTestBase, public testing::Test {
public:
...
TYPED_TEST_P(TaskGraphRunnerTest, Basic)
{
    ...
    for (int i = 0; i < kNamespaceCount; ++i) {
        ...
        this->ScheduleTasks(i, std::vector<TaskInfo>(1, TaskInfo(i, 0u, 0u, 0u, 0u, 0u, false)));
    }
    ...
    for (int i = 0; i < kNamespaceCount; ++i)
        this->ScheduleTasks(i, std::vector<TaskInfo>(1, TaskInfo(i, 0u, 0u, 1u, 0u, 0u, false)));
    ...
    for (int i = 0; i < kNamespaceCount; ++i)
        this->ScheduleTasks(i, std::vector<TaskInfo>(1, TaskInfo(i, 0u, 0u, 2u, 0u, 0u, false)));
...
REGISTER_TYPED_TEST_SUITE_P(TaskGraphRunnerTest, Basic, Dependencies, ExternalDependency, Categories);

template <typename TaskRunnerTestDelegate> using SingleThreadTaskGraphRunnerTest = TaskGraphRunnerTest<TaskRunnerTestDelegate>;
...
TYPED_TEST_P(SingleThreadTaskGraphRunnerTest, Priority)
{
    ...
    for (int i = 0; i < kNamespaceCount; ++i) {
        ...
        this->ScheduleTasks(i, std::vector<TaskInfo>(tasks, tasks + std::size(tasks)));
    }
