...
public:
    ...
    virtual std::unique_ptr<viz::DisplayCompositorMemoryAndTaskController> CreateDisplayController() = 0;
    virtual std::unique_ptr<viz::SkiaOutputSurface> CreateSkiaOutputSurface(viz::DisplayCompositorMemoryAndTaskController*) = 0;
    virtual std::unique_ptr<viz::OutputSurface> CreateSoftwareOutputSurface() = 0;

    virtual void DisplayReceivedLocalSurfaceId(const viz::LocalSurfaceId& local_surface_id) = 0;
    virtual void DisplayReceivedCompositorFrame(const viz::CompositorFrame& frame) = 0;
    virtual void DisplayWillDrawAndSwap(bool will_draw_and_swap, viz::AggregatedRenderPassList* render_passes) = 0;
...
public:
    ...
    TestLayerTreeFrameSink(scoped_refptr<viz::RasterContextProvider> compositor_context_provider,
        scoped_refptr<viz::RasterContextProvider> worker_context_provider, gpu::GpuMemoryBufferManager* gpu_memory_buffer_manager,
        const viz::RendererSettings& renderer_settings, const viz::DebugRendererSettings* debug_settings, TaskRunnerProvider* task_runner_provider,
        bool synchronous_composite, bool disable_display_vsync, double refresh_rate, viz::BeginFrameSource* begin_frame_source = nullptr);
    ...
    void DidAllocateSharedBitmap(base::ReadOnlySharedMemoryRegion buffer, const viz::SharedBitmapId& id) override;
...
private:
    ...
    raw_ptr<viz::BeginFrameSource> client_provided_begin_frame_source_; // Not owned.
    raw_ptr<viz::BeginFrameSource> display_begin_frame_source_ = nullptr; // Not owned.
