...
public:
    ...
    static std::unique_ptr<FakePictureLayerImpl> Create(LayerTreeImpl* tree_impl, int id, scoped_refptr<RasterSource> raster_source = nullptr)
    {
        return base::WrapUnique(new FakePictureLayerImpl(tree_impl, id, raster_source));
    }
    ...
    size_t CountTilesRequired(TileRequirementCheck is_tile_required_callback) const;
    ...
    {
        last_append_quads_tilings_.push_back(AddTiling(gfx::AxisTransform2d(scale, gfx::Vector2dF())));
    }

    float raster_page_scale() const
    {
        return raster_page_scale_;
    }
    void set_raster_page_scale(float scale)
    {
        raster_page_scale_ = scale;
    }
    ...
    {
        return tilings_->num_tilings();
    }

    size_t GetNumberOfTilesWithResources() const;

    PictureLayerTilingSet* tilings()
    {
        return tilings_.get();
    }
    ...
    void SetRasterSource(scoped_refptr<RasterSource> raster_source, const Region& invalidation);
    size_t append_quads_count()
    {
        return append_quads_count_;
    }

    const Region& invalidation() const
    {
    ...
    PictureLayerTilingSet* GetTilings()
    {
        return tilings_.get();
    }
    ...
    size_t release_resources_count() const
    {
        return release_resources_count_;
    }
...
protected:
    FakePictureLayerImpl(LayerTreeImpl* tree_impl, int id, scoped_refptr<RasterSource> raster_source = nullptr);
