...
LayerTreeImplTestBase::LayerTreeImplTestBase(std::unique_ptr<LayerTreeFrameSink> layer_tree_frame_sink)
    : LayerTreeImplTestBase(CommitToPendingTreeLayerListSettings(), std::move(layer_tree_frame_sink))
{
}
...
LayerTreeImplTestBase::LayerTreeImplTestBase(const LayerTreeSettings& settings, std::unique_ptr<LayerTreeFrameSink> layer_tree_frame_sink)
    ...
    , layer_impl_id_(2)
{
    ...
    animation_host_->PushPropertiesTo(host_impl()->animation_host(), *host_->property_trees());
...
void LayerTreeImplTestBase::AppendQuadsWithOcclusion(LayerImpl* layer_impl, const gfx::Rect& occluded)
{
    AppendQuadsData data;
    ...
    Occlusion occlusion(layer_impl->DrawTransform(), SimpleEnclosedRegion(occluded), SimpleEnclosedRegion());
...
void LayerTreeImplTestBase::AppendQuadsForPassWithOcclusion(LayerImpl* layer_impl, viz::CompositorRenderPass* given_render_pass, const gfx::Rect& occluded)
{
    AppendQuadsData data;
...
void LayerTreeImplTestBase::AppendSurfaceQuadsWithOcclusion(RenderSurfaceImpl* surface_impl, const gfx::Rect& occluded)
{
    AppendQuadsData data;
    ...
    surface_impl->set_occlusion_in_content_space(Occlusion(gfx::Transform(), SimpleEnclosedRegion(occluded), SimpleEnclosedRegion()));
    surface_impl->AppendQuads(DRAW_MODE_HARDWARE, render_pass_.get(), &data);
}

void LayerTreeImplTestBase::UpdateActiveTreeDrawProperties(float device_scale_factor)
{
    SetDeviceScaleAndUpdateViewportRect(host_impl()->active_tree(), device_scale_factor);
    UpdateDrawProperties(host_impl()->active_tree(), &update_layer_impl_list_);
}

void LayerTreeImplTestBase::UpdatePendingTreeDrawProperties(float device_scale_factor)
{
    SetDeviceScaleAndUpdateViewportRect(host_impl()->pending_tree(), device_scale_factor);
