...
bool ExactPixelComparator::Compare(const SkBitmap& actual_bmp, const SkBitmap& expected_bmp) const
{
...
ManhattanDistancePixelComparator::ManhattanDistancePixelComparator(int tolerance)
    : tolerance_(tolerance)
{
}

bool ManhattanDistancePixelComparator::Compare(const SkBitmap& actual_bmp, const SkBitmap& expected_bmp) const
{
    // Check that bitmaps have identical dimensions.
    DCHECK_EQ(actual_bmp.width(), expected_bmp.width());
    DCHECK_EQ(actual_bmp.height(), expected_bmp.height());
    ...
    for (int y = 0; y < actual_bmp.height(); ++y) {
        for (int x = 0; x < actual_bmp.width(); ++x) {
            ...
            int manhattan_distance = std::abs(pixel_b - ref_pixel_b) + std::abs(pixel_g - ref_pixel_g) + std::abs(pixel_r - ref_pixel_r);
...
bool FuzzyPixelComparator::Compare(const SkBitmap& actual_bmp, const SkBitmap& expected_bmp) const
{
    // Check that bitmaps have identical dimensions.
    DCHECK_EQ(actual_bmp.width(), expected_bmp.width());
    DCHECK_EQ(actual_bmp.height(), expected_bmp.height());
    ...
    for (int x = 0; x < actual_bmp.width(); ++x) {
        for (int y = 0; y < actual_bmp.height(); ++y) {
            ...
            if (actual_color != expected_color) {
                ...
                if (abs_error_r <= small_abs_error_limit_ && abs_error_g <= small_abs_error_limit_ && abs_error_b <= small_abs_error_limit_
                    && abs_error_a <= small_abs_error_limit_) {
                    ++small_error_pixels_count;
                }

                if (abs_error_a != 0 && (expected_alpha == 0 || expected_alpha == 0xff)) {
    ...
    large_error_pixels_percentage = static_cast<float>(large_error_pixels_count) / pixels_count * 100.0f;
    ...
    if (large_error_pixels_percentage > large_error_pixels_percentage_limit_ || small_error_pixels_percentage > small_error_pixels_percentage_limit_
        || avg_abs_error_r > avg_abs_error_limit_ || avg_abs_error_g > avg_abs_error_limit_ || avg_abs_error_b > avg_abs_error_limit_
        || avg_abs_error_a > avg_abs_error_limit_ || max_abs_error_r > max_abs_error_limit_ || max_abs_error_g > max_abs_error_limit_
        || max_abs_error_b > max_abs_error_limit_ || max_abs_error_a > max_abs_error_limit_ || critial_error_pixels_count) {
        if (small_abs_error_limit_) {
            LOG(ERROR) << "Percentage of pixels with errors >" << small_abs_error_limit_ << ": " << large_error_pixels_percentage;
            LOG(ERROR) << "Percentage of pixels with errors <=" << small_abs_error_limit_ << ": " << small_error_pixels_percentage;
        } else {
            LOG(ERROR) << "Percentage of pixels with an error: " << large_error_pixels_percentage;
        }
