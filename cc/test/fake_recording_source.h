...
public:
    ...
    void add_draw_rect_with_flags(const gfx::Rect& rect, const PaintFlags& flags)
    {
    ...
    void add_draw_rectf_with_flags(const gfx::RectF& rect, const PaintFlags& flags)
    {
    ...
    {
        client_->add_draw_image(std::move(image), point, SkSamplingOptions(), default_flags_);
    }
    ...
    void add_draw_image_with_transform(PaintImage image, const gfx::Transform& transform)
    {
        client_->add_draw_image_with_transform(std::move(image), transform, SkSamplingOptions(), default_flags_);
    }

    void add_draw_image_with_flags(sk_sp<SkImage> image, const gfx::Point& point, const SkSamplingOptions& sampling, const PaintFlags& flags)
    {
        client_->add_draw_image(std::move(image), point, sampling, flags);
    }

    void set_default_flags(const PaintFlags& flags)
    {
