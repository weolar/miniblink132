...
{
    gfx::Size size(std::numeric_limits<int>::max() / 10, std::numeric_limits<int>::max() / 10);
    return CreateFilled(size);
}

scoped_refptr<FakeRasterSource> FakeRasterSource::CreateFilled(const gfx::Size& size)
{
...
scoped_refptr<FakeRasterSource> FakeRasterSource::CreateFilledWithImages(const gfx::Size& size)
{
    FakeRecordingSource recording_source(size);

    for (int y = 0; y < size.height(); y += 100) {
        for (int x = 0; x < size.width(); x += 100) {
            recording_source.add_draw_image(CreateDiscardablePaintImage(gfx::Size(100, 100)), gfx::Point(x, y));
        }
    }
    recording_source.Rerecord();
...
scoped_refptr<FakeRasterSource> FakeRasterSource::CreateFilledWithText(const gfx::Size& size)
{
    FakeRecordingSource recording_source(size);
...
scoped_refptr<FakeRasterSource> FakeRasterSource::CreateFilledWithPaintWorklet(const gfx::Size& size)
{
    FakeRecordingSource recording_source(size);

    auto input = base::MakeRefCounted<TestPaintWorkletInput>(gfx::SizeF(size));
    recording_source.add_draw_image(CreatePaintWorkletPaintImage(std::move(input)), gfx::Point(0, 0));

    recording_source.Rerecord();
    return base::MakeRefCounted<FakeRasterSource>(recording_source);
}

scoped_refptr<FakeRasterSource> FakeRasterSource::CreateFilledSolidColor(const gfx::Size& size)
{
    FakeRecordingSource recording_source(size);
...
scoped_refptr<FakeRasterSource> FakeRasterSource::CreatePartiallyFilled(const gfx::Size& size, const gfx::Rect& recorded_bounds)
{
    ...
    gfx::Rect smaller_rect(recorded_bounds.origin(), recorded_bounds.size() - gfx::Size(10, 10));
...
scoped_refptr<FakeRasterSource> FakeRasterSource::CreateEmpty(const gfx::Size& size)
{
    return base::MakeRefCounted<FakeRasterSource>(FakeRecordingSource(size));
}

scoped_refptr<FakeRasterSource> FakeRasterSource::CreateFromRecordingSource(const RecordingSource& recording_source)
{
    return base::MakeRefCounted<FakeRasterSource>(recording_source);
}

scoped_refptr<FakeRasterSource> FakeRasterSource::CreateFromRecordingSourceWithWaitable(
    const RecordingSource& recording_source, base::WaitableEvent* playback_allowed_event)
{
    return base::MakeRefCounted<FakeRasterSource>(recording_source, playback_allowed_event);
}
...
FakeRasterSource::FakeRasterSource(const RecordingSource& recording_source, base::WaitableEvent* playback_allowed_event)
    : RasterSource(recording_source)
...
void FakeRasterSource::PlaybackDisplayListToCanvas(SkCanvas* canvas, const PlaybackSettings& settings) const
{
...
void FakeRasterSource::SetDirectlyCompositedImageDefaultRasterScale(gfx::Vector2dF scale)
{
