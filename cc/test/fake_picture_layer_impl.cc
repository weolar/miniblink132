...
FakePictureLayerImpl::FakePictureLayerImpl(LayerTreeImpl* tree_impl, int id, scoped_refptr<RasterSource> raster_source)
...
std::unique_ptr<LayerImpl> FakePictureLayerImpl::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
...
void FakePictureLayerImpl::AppendQuads(viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data)
{
    PictureLayerImpl::AppendQuads(render_pass, append_quads_data);
    ++append_quads_count_;
}

gfx::Size FakePictureLayerImpl::CalculateTileSize(const gfx::Size& content_bounds)
{
...
void FakePictureLayerImpl::SetRasterSource(scoped_refptr<RasterSource> raster_source, const Region& invalidation)
{
    Region invalidation_temp = invalidation;
    set_gpu_raster_max_texture_size(layer_tree_impl()->GetDeviceViewport().size());
...
{
    for (size_t tiling_idx = 0; tiling_idx < tilings_->num_tilings(); ++tiling_idx) {
        PictureLayerTiling* tiling = tilings_->tiling_at(tiling_idx);
        SetAllTilesReadyInTiling(tiling);
    }
}

void FakePictureLayerImpl::SetAllTilesReadyInTiling(PictureLayerTiling* tiling)
{
...
{
    return use_set_valid_tile_priorities_flag_ ? has_valid_tile_priorities_ : PictureLayerImpl::HasValidTilePriorities();
}

size_t FakePictureLayerImpl::CountTilesRequired(TileRequirementCheck is_tile_required_callback) const
{
    ...
    for (size_t i = 0; i < tilings_->num_tilings(); ++i) {
        ...
        for (PictureLayerTiling::CoverageIterator iter(tiling, 1.f, rect); iter; ++iter) {
