...
template <typename LayerType> void SetupRootPropertiesInternal(LayerType* root)
{
    root->set_property_tree_sequence_number(GetPropertyTrees(root)->sequence_number());
    ...
    auto& root_scroll_node = CreateScrollNode(root, gfx::Size(), kRootPropertyNodeId);
    DCHECK_EQ(root_scroll_node.id, kSecondaryRootPropertyNodeId);
}

template <typename LayerType> void CopyPropertiesInternal(const LayerType* from, LayerType* to)
{
...
#define ID_OR_DEFAULT(id, default_id) ((id) == kInvalidPropertyNodeId ? (default_id) : (id))

template <typename LayerType> TransformNode& CreateTransformNodeInternal(LayerType* layer, PropertyTrees* property_trees, int parent_id)
{
    auto& transform_tree = property_trees->transform_tree_mutable();
    int id = transform_tree.Insert(TransformNode(), ID_OR_DEFAULT(parent_id, layer->transform_tree_index()));
    ...
    if (layer) {
        ...
        if (node->element_id) {
            property_trees->transform_tree_mutable().SetElementIdForNodeId(node->id, node->element_id);
        }
    }
    if (const auto* parent_node = transform_tree.Node(node->parent_id)) {
        node->in_subtree_of_page_scale_layer = parent_node->in_subtree_of_page_scale_layer;
    }
...
ClipNode& CreateClipNodeInternal(LayerType* layer, PropertyTrees* property_trees, int parent_id, int transform_id = kInvalidPropertyNodeId)
{
    auto& clip_tree = property_trees->clip_tree_mutable();
    int id = clip_tree.Insert(ClipNode(), ID_OR_DEFAULT(parent_id, layer->clip_tree_index()));
    auto* node = clip_tree.Node(id);
    node->transform_id = ID_OR_DEFAULT(transform_id, layer->transform_tree_index());
    if (layer) {
        layer->SetClipTreeIndex(id);
        node->clip = gfx::RectF(gfx::PointAtOffsetFromOrigin(layer->offset_to_transform_parent()), gfx::SizeF(layer->bounds()));
    }
...
EffectNode& CreateEffectNodeInternal(
    LayerType* layer, PropertyTrees* property_trees, int parent_id, int transform_id = kInvalidPropertyNodeId, int clip_id = kInvalidPropertyNodeId)
{
    auto& effect_tree = property_trees->effect_tree_mutable();
    int id = effect_tree.Insert(EffectNode(), ID_OR_DEFAULT(parent_id, layer->effect_tree_index()));
    auto* node = effect_tree.Node(id);
    if (layer) {
        layer->SetEffectTreeIndex(id);
        node->element_id = layer->element_id() ? layer->element_id() : LayerIdToElementIdForTesting(layer->id());
        if (layer->element_id()) {
            property_trees->effect_tree_mutable().SetElementIdForNodeId(node->id, node->element_id);
        }
    }
    node->transform_id = ID_OR_DEFAULT(transform_id, layer->transform_tree_index());
...
template <typename LayerType> ScrollNode& CreateScrollNodeInternal(LayerType* layer, const gfx::Size& scroll_container_bounds, int parent_id)
{
    auto* property_trees = GetPropertyTrees(layer);
    auto& scroll_tree = property_trees->scroll_tree_mutable();
    int id = scroll_tree.Insert(ScrollNode(), ID_OR_DEFAULT(parent_id, layer->scroll_tree_index()));
    layer->SetScrollTreeIndex(id);
    auto* node = scroll_tree.Node(id);
    node->element_id = layer->element_id();
    if (node->element_id) {
        property_trees->scroll_tree_mutable().SetElementIdForNodeId(node->id, node->element_id);
    }
    node->bounds = layer->bounds();
    node->container_bounds = scroll_container_bounds;
    node->user_scrollable_horizontal = node->user_scrollable_vertical = !scroll_container_bounds.IsEmpty();
...
template <typename LayerType, typename MaskLayerType> void SetupMaskPropertiesInternal(LayerType* masked_layer, MaskLayerType* mask_layer)
{
    ...
    mask_layer->SetOffsetToTransformParent(masked_layer->offset_to_transform_parent());
    CreateEffectNode(mask_layer).blend_mode = SkBlendMode::kDstIn;
}

template <typename LayerType> void SetScrollOffsetInternal(LayerType* layer, const gfx::PointF& scroll_offset)
{
    DCHECK(layer->has_transform_node());
    auto* transform_node = GetTransformNode(layer);
    transform_node->scroll_offset = scroll_offset;
    SetLocalTransformChanged(layer);
    GetPropertyTrees(layer)->scroll_tree_mutable().SetScrollOffset(layer->element_id(), scroll_offset);
}
...
template <typename LayerType>
ViewportPropertyIds SetupViewportProperties(LayerType* root, LayerType* inner_viewport_scroll_layer, LayerType* outer_viewport_scroll_layer)
{
    ViewportPropertyIds viewport_property_ids;
    auto* property_trees = GetPropertyTrees(root);

    viewport_property_ids.overscroll_elasticity_transform = CreateTransformNode(property_trees, root->transform_tree_index()).id;

    auto& page_scale_transform = CreateTransformNode(property_trees, viewport_property_ids.overscroll_elasticity_transform);
    ...
    CreateTransformNode(inner_viewport_scroll_layer, viewport_property_ids.page_scale_transform);
    auto& inner_scroll = CreateScrollNode(inner_viewport_scroll_layer, root->bounds());
    ...
    auto& outer_clip = CreateClipNode(property_trees, root->clip_tree_index(), inner_scroll.transform_id);
    outer_clip.clip = gfx::RectF(gfx::SizeF(inner_viewport_scroll_layer->bounds()));
    ...
    auto& outer_scroll = CreateScrollNode(outer_viewport_scroll_layer, inner_viewport_scroll_layer->bounds());
...
TransformNode& CreateTransformNode(PropertyTrees* property_trees, int parent_id)
{
...
ClipNode& CreateClipNode(PropertyTrees* property_trees, int parent_id, int transform_id)
{
    return CreateClipNodeInternal<Layer>(nullptr, property_trees, parent_id, transform_id);
}
...
EffectNode& CreateEffectNode(PropertyTrees* property_trees, int parent_id, int transform_id, int clip_id)
{
    return CreateEffectNodeInternal<Layer>(nullptr, property_trees, parent_id, transform_id, clip_id);
}

ScrollNode& CreateScrollNode(Layer* layer, const gfx::Size& scroll_container_bounds, int parent_id)
{
    DCHECK(layer->layer_tree_host()->IsUsingLayerLists());
    return CreateScrollNodeInternal(layer, scroll_container_bounds, parent_id);
}

ScrollNode& CreateScrollNode(LayerImpl* layer, const gfx::Size& scroll_container_bounds, int parent_id)
{
    return CreateScrollNodeInternal(layer, scroll_container_bounds, parent_id);
}

ScrollNode& CreateScrollNodeForNonCompositedScroller(PropertyTrees* property_trees, int parent_id, ElementId element_id, const gfx::Size& bounds,
    const gfx::Size& scroll_container_bounds, const gfx::Point& scroll_container_origin)
{
    auto& scroll_tree = property_trees->scroll_tree_mutable();
    ...
    property_trees->scroll_tree_mutable().SetElementIdForNodeId(node->id, element_id);
    ...
    // Create a matching transform node.
    {
        ...
        int transform_id = transform_tree.Insert(TransformNode(), scroll_parent.transform_id);
        auto* transform_node = transform_tree.Node(transform_id);
        transform_node->element_id = element_id;
        property_trees->transform_tree_mutable().SetElementIdForNodeId(transform_node->id, transform_node->element_id);

        if (const auto* parent_transform_node = transform_tree.Node(transform_node->parent_id)) {
            transform_node->in_subtree_of_page_scale_layer = parent_transform_node->in_subtree_of_page_scale_layer;
        }
...
void SetScrollOffsetFromImplSide(Layer* layer, const gfx::PointF& scroll_offset)
{
    if (layer->layer_tree_host()->IsUsingLayerLists())
        SetScrollOffsetInternal(layer, scroll_offset);
    else
...
void SetupViewport(Layer* root, scoped_refptr<Layer> outer_viewport_scroll_layer, const gfx::Size& outer_viewport_size)
{
    DCHECK(root);
    ...
    auto viewport_property_ids = SetupViewportProperties(root, inner_viewport_scroll_layer.get(), outer_viewport_scroll_layer.get());
    root->layer_tree_host()->RegisterViewportPropertyIds(viewport_property_ids);
}

void SetupViewport(Layer* root, const gfx::Size& outer_viewport_size, const gfx::Size& content_size)
{
...
void SetupViewport(LayerImpl* root, const gfx::Size& outer_viewport_size, const gfx::Size& content_size)
{
    DCHECK(root);
    ...
    std::unique_ptr<LayerImpl> inner_viewport_scroll_layer = LayerImpl::Create(layer_tree_impl, 10000);
    inner_viewport_scroll_layer->SetBounds(outer_viewport_size);
    inner_viewport_scroll_layer->SetHitTestOpaqueness(HitTestOpaqueness::kOpaque);
    inner_viewport_scroll_layer->SetElementId(LayerIdToElementIdForTesting(inner_viewport_scroll_layer->id()));

    std::unique_ptr<LayerImpl> outer_viewport_scroll_layer = LayerImpl::Create(layer_tree_impl, 10001);
    outer_viewport_scroll_layer->SetBounds(content_size);
    ...
    outer_viewport_scroll_layer->SetElementId(LayerIdToElementIdForTesting(outer_viewport_scroll_layer->id()));
...
const RenderSurfaceImpl* GetRenderSurface(const LayerImpl* layer)
{
    ...
    if (const auto* surface = effect_tree.GetRenderSurface(layer->effect_tree_index()))
        return surface;
    return effect_tree.GetRenderSurface(GetEffectNode(layer)->target_id);
}
...
{
    return GetPropertyTrees(layer)->scroll_tree().GetScrollOffsetBaseForTesting(layer->element_id());
}

gfx::Vector2dF ScrollDelta(const LayerImpl* layer)
{
    return GetPropertyTrees(layer)->scroll_tree().GetScrollOffsetDeltaForTesting(layer->element_id());
}

gfx::PointF CurrentScrollOffset(const Layer* layer, const PropertyTrees* property_trees)
{
    if (!property_trees && layer->IsAttached())
        property_trees = layer->layer_tree_host()->property_trees();
    auto result = property_trees->scroll_tree().current_scroll_offset(layer->element_id());
...
{
    return GetPropertyTrees(layer)->scroll_tree().current_scroll_offset(layer->element_id());
}

gfx::PointF MaxScrollOffset(const LayerImpl* layer)
{
    return GetPropertyTrees(layer)->scroll_tree().MaxScrollOffset(layer->scroll_tree_index());
}
