...
bool WritePNGFile(const SkBitmap& bitmap, const base::FilePath& file_path, bool discard_transparency)
{
    std::optional<std::vector<uint8_t>> png_data = gfx::PNGCodec::EncodeBGRASkBitmap(bitmap, discard_transparency);
...
{
    std::optional<std::vector<uint8_t>> png_data = gfx::PNGCodec::EncodeBGRASkBitmap(bitmap, /*discard_transparency=*/false);
    return base::StrCat({ "data:image/png;base64,", base::Base64Encode(png_data.value_or(std::vector<uint8_t>())) });
}

SkBitmap ReadPNGFile(const base::FilePath& file_path)
{
    std::optional<std::vector<uint8_t>> png_data = base::ReadFileToBytes(file_path);
    if (!png_data) {
        return SkBitmap();
    }
...
bool MatchesBitmap(const SkBitmap& gen_bmp, const SkBitmap& ref_bmp, const PixelComparator& comparator)
{
...
bool MatchesPNGFile(const SkBitmap& gen_bmp, base::FilePath ref_img_path, const PixelComparator& comparator)
{
