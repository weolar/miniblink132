...
FakeUIResourceLayerTreeHostImpl::FakeUIResourceLayerTreeHostImpl(TaskRunnerProvider* task_runner_provider, TaskGraphRunner* task_graph_runner)
...
void FakeUIResourceLayerTreeHostImpl::CreateUIResource(UIResourceId uid, const UIResourceBitmap& bitmap)
{
    ...
    data.resource_id_for_export
        = resource_provider()->ImportResource(viz::TransferableResource::MakeGpu(gpu::Mailbox::Generate(), GL_TEXTURE_2D, gpu::SyncToken(), bitmap.GetSize(),
...
viz::ResourceId FakeUIResourceLayerTreeHostImpl::ResourceIdForUIResource(UIResourceId uid) const
{
...
bool FakeUIResourceLayerTreeHostImpl::IsUIResourceOpaque(UIResourceId uid) const
{
    auto iter = fake_ui_resource_map_.find(uid);
