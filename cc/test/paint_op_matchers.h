...
template <typename OpT> class PaintOpEq {
public:
    using is_gtest_matcher = void;

    template <typename... Args>
    explicit PaintOpEq(Args&&... args)
        : expected_op_(base::MakeRefCounted<base::RefCountedData<OpT>>(std::in_place, std::forward<Args>(args)...))
    {
    }

    bool MatchAndExplain(const PaintOp& op, testing::MatchResultListener* listener) const
    {
...
template <typename OpT, typename OpU = OpT> class PaintOpIs {
public:
    using is_gtest_matcher = void;
        ...
        if (op.GetType() != OpT::kType && op.GetType() != OpU::kType) {
            if (listener->IsInterested()) {
                if constexpr (std::is_same_v<OpT, OpU>) {
                    *listener->stream() << "Unexpected PaintOp type. Expected: " << PaintOpTypeToString(OpT::kType)
                                        << ", Actual: " << PaintOpTypeToString(op.GetType());
                } else {
                    *listener->stream() << "Unexpected PaintOp type. Expected: " << PaintOpTypeToString(OpT::kType) << " or " << PaintOpTypeToString(OpU::kType)
                                        << ", Actual: " << PaintOpTypeToString(op.GetType());
                }
            }
            return false;
...
//                                          PaintOpEq<RestoreOp>())));
template <typename... Args> testing::Matcher<PaintOp> DrawRecordOpEq(Args... args)
{
    return testing::AllOf(PaintOpIs<DrawRecordOp>(),
        testing::ResultOf([](const PaintOp& record) { return static_cast<const DrawRecordOp&>(record).record; }, testing::ElementsAre(args...)));
}
