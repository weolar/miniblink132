...
TransformNode& CreateTransformNode(Layer*, int parent_id = kInvalidPropertyNodeId);
TransformNode& CreateTransformNode(LayerImpl*, int parent_id = kInvalidPropertyNodeId);
...
EffectNode& CreateEffectNode(LayerImpl*, int parent_id = kInvalidPropertyNodeId);
// The scroll node is not scrollable if |scroll_container_bounds| is empty.
ScrollNode& CreateScrollNode(Layer*, const gfx::Size& scroll_container_bounds, int parent_id = kInvalidPropertyNodeId);
ScrollNode& CreateScrollNode(LayerImpl*, const gfx::Size& scroll_container_bounds, int parent_id = kInvalidPropertyNodeId);
...
EffectNode& CreateEffectNode(PropertyTrees*, int parent_id, int transform_id, int clip_id);
...
ScrollNode& CreateScrollNodeForNonCompositedScroller(PropertyTrees* property_trees, int parent_id, ElementId element_id, const gfx::Size& bounds,
    const gfx::Size& scroll_container_bounds, const gfx::Point& scroll_container_origin = gfx::Point());
...
template <typename LayerType> TransformNode* GetTransformNode(LayerType* layer)
{
    return GetPropertyTrees(layer)->transform_tree_mutable().Node(layer->transform_tree_index());
}
template <typename LayerType> const TransformNode* GetTransformNode(const LayerType* layer)
{
    return GetPropertyTrees(layer)->transform_tree().Node(layer->transform_tree_index());
}
template <typename LayerType> ClipNode* GetClipNode(LayerType* layer)
{
    return GetPropertyTrees(layer)->clip_tree_mutable().Node(layer->clip_tree_index());
}
template <typename LayerType> const ClipNode* GetClipNode(const LayerType* layer)
{
    return GetPropertyTrees(layer)->clip_tree().Node(layer->clip_tree_index());
}
template <typename LayerType> EffectNode* GetEffectNode(LayerType* layer)
{
    return GetPropertyTrees(layer)->effect_tree_mutable().Node(layer->effect_tree_index());
}
template <typename LayerType> const EffectNode* GetEffectNode(const LayerType* layer)
{
    return GetPropertyTrees(layer)->effect_tree().Node(layer->effect_tree_index());
}
template <typename LayerType> ScrollNode* GetScrollNode(LayerType* layer)
{
    return GetPropertyTrees(layer)->scroll_tree_mutable().Node(layer->scroll_tree_index());
}
template <typename LayerType> const ScrollNode* GetScrollNode(const LayerType* layer)
{
...
template <typename LayerType> void SetLocalTransformChanged(LayerType* layer)
{
...
template <typename LayerType> void SetWillChangeTransform(LayerType* layer, bool will_change_transform)
{
    DCHECK(layer->has_transform_node());
    auto* transform_node = GetTransformNode(layer);
    transform_node->will_change_transform = will_change_transform;
    transform_node->node_or_ancestors_will_change_transform = will_change_transform;
    transform_node->transform_changed = true;
    TransformTree& transform_tree = GetPropertyTrees(layer)->transform_tree_mutable();
    transform_tree.UpdateNodeOrAncestorsWillChangeTransform(transform_node, transform_tree.parent(transform_node));
    transform_tree.set_needs_update(true);
}

template <typename LayerType> void SetTransform(LayerType* layer, const gfx::Transform& transform)
{
    GetTransformNode(layer)->local = transform;
    SetLocalTransformChanged(layer);
}

template <typename LayerType> void SetTransformOrigin(LayerType* layer, const gfx::Point3F& origin)
{
    GetTransformNode(layer)->origin = origin;
    SetLocalTransformChanged(layer);
}

template <typename LayerType> void SetPostTranslation(LayerType* layer, const gfx::Vector2dF& post_translation)
{
...
template <typename LayerType> void SetOpacity(LayerType* layer, float opacity)
{
...
// This will affect all layers associated with this layer's effect node.
template <typename LayerType> void SetFilter(LayerType* layer, const FilterOperations& filters)
{
    auto* effect_node = GetEffectNode(layer);
...
// This will affect all layers associated with this layer's effect node.
template <typename LayerType> void SetRenderSurfaceReason(LayerType* layer, RenderSurfaceReason reason)
{
    auto* effect_node = GetEffectNode(layer);
...
// This will affect all layers associated with this layer's effect node.
template <typename LayerType> void SetBackdropFilter(LayerType* layer, const FilterOperations& filters)
{
    auto* effect_node = GetEffectNode(layer);
...
template <typename LayerType> void SetClipRect(LayerType* layer, const gfx::RectF& clip)
{
...
gfx::PointF CurrentScrollOffset(const Layer* layer, const PropertyTrees* property_trees = nullptr);
