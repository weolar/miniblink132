...
class TestOptionsProvider::DiscardableManager : public SkStrikeServer::DiscardableHandleManager, public SkStrikeClient::DiscardableHandleManager {
public:
...
TestOptionsProvider::TestOptionsProvider()
    ...
    , skottie_serialization_history_(kSkottieSerializationHistoryTestPurgePeriod)
    , client_paint_cache_(std::numeric_limits<size_t>::max())
    , serialize_options_(this, this, &client_paint_cache_, &strike_server_, color_space_, &skottie_serialization_history_, can_use_lcd_text_,
          context_supports_distance_field_text_, max_texture_size_)
    , deserialize_options_ {
        .transfer_cache = this, .paint_cache = &service_paint_cache_, .strike_client = &strike_client_, .scratch_buffer = scratch_buffer_, .is_privileged = true
    }
{
}
...
ImageProvider::ScopedResult TestOptionsProvider::GetRasterContent(const DrawImage& draw_image)
{
    ...
    if (LockEntryDirect(entry_key)) {
        return ScopedResult(DecodedDrawImage(image_id, nullptr, SkSize::MakeEmpty(), draw_image.scale(), draw_image.filter_quality(), false, true));
    }

    decoded_images_.push_back(draw_image);
    SkBitmap bitmap;
    const auto& paint_image = draw_image.paint_image();
    bitmap.allocPixelsFlags(SkImageInfo::MakeN32Premul(paint_image.width(), paint_image.height()), SkBitmap::kZeroPixels_AllocFlag);

    // Create a transfer cache entry for this image.
    ClientImageTransferCacheEntry cache_entry(ClientImageTransferCacheEntry::Image(&bitmap.pixmap()), false /* needs_mips */, std::nullopt);
