...
public:
    ...
    template <typename T> static std::string ToString(const T* ptr)
    {
        return ptr ? ToString(*ptr) : "(nil)";
    }

    template <typename T> static std::string ToString(const sk_sp<T>& ptr)
    {
        return ptr ? ToString(*ptr) : "(nil)";
    }

    template <typename T> static std::string ToString(scoped_refptr<T> ptr)
    {
        return ptr ? ToString(*ptr) : "(nil)";
    }

    template <typename T> static std::string ToString(const std::optional<T>& opt)
    {
        return opt.has_value() ? ToString(*opt) : "(nil)";
    }

    template <typename T> static std::string ToString(const std::vector<T>& vec)
    {
    ...
    static std::string ToString(const PaintOp& base_op)
    {
        ...
        case PaintOpType::kAnnotate: {
            ...
            str << "type=" << ToString(op.annotation_type) << ", rect=" << ToString(op.rect) << ", data=" << ToString(op.data);
            break;
        }
        case PaintOpType::kClipPath: {
            const auto& op = static_cast<const ClipPathOp&>(base_op);
            str << "path=" << ToString(op.path) << ", op=" << ToString(op.op) << ", antialias=" << op.antialias
        ...
        case PaintOpType::kClipRect: {
            ...
            str << "rect=" << ToString(op.rect) << ", op=" << ToString(op.op) << ", antialias=" << op.antialias;
            break;
        }
        case PaintOpType::kClipRRect: {
            const auto& op = static_cast<const ClipRRectOp&>(base_op);
            str << "rrect=" << ToString(op.rrect) << ", op=" << ToString(op.op) << ", antialias=" << op.antialias;
            break;
        }
        ...
        case PaintOpType::kDrawDRRect: {
            ...
            str << "outer=" << ToString(op.outer) << ", inner=" << ToString(op.inner) << ", flags=" << ToString(op.flags);
            break;
        }
        case PaintOpType::kDrawImage: {
            const auto& op = static_cast<const DrawImageOp&>(base_op);
            str << "image=" << ToString(op.image) << ", left=" << ToString(op.left) << ", top=" << ToString(op.top) << ", flags=" << ToString(op.flags);
            break;
        }
        case PaintOpType::kDrawImageRect: {
            const auto& op = static_cast<const DrawImageRectOp&>(base_op);
            str << "image=" << ToString(op.image) << ", src=" << ToString(op.src) << ", dst=" << ToString(op.dst) << ", constraint=" << ToString(op.constraint)
        ...
        case PaintOpType::kDrawLine: {
            ...
            str << "x0=" << ToString(op.x0) << ", y0=" << ToString(op.y0) << ", x1=" << ToString(op.x1) << ", y1=" << ToString(op.y1)
                << ", flags=" << ToString(op.flags);
            break;
        }
        ...
        case PaintOpType::kDrawArc: {
            ...
            str << "oval=" << ToString(op.oval) << ", start_angle=" << ToString(op.start_angle_degrees) << ", sweep_angle=" << ToString(op.sweep_angle_degrees)
                << ", flags=" << ToString(op.flags);
            break;
        }
        ...
        case PaintOpType::kDrawPath: {
            ...
            str << "path=" << ToString(op.path) << ", flags=" << ToString(op.flags) << ", use_cache=" << (op.use_cache == UsePaintCache::kEnabled);
            break;
        }
        ...
        case PaintOpType::kDrawRRect: {
            ...
            str << "rrect=" << ToString(op.rrect) << ", flags=" << ToString(op.flags);
            break;
        }
        case PaintOpType::kDrawScrollingContents: {
            str << "scroll_element_id=" << static_cast<const DrawScrollingContentsOp&>(base_op).scroll_element_id;
            break;
        }
        case PaintOpType::kDrawSkottie: {
            const auto& op = static_cast<const DrawSkottieOp&>(base_op);
            str << "skottie=" << ToString(op.skottie) << ", dst=" << ToString(op.dst) << ", t=" << op.t;
            break;
        }
        case PaintOpType::kDrawTextBlob: {
            const auto& op = static_cast<const DrawTextBlobOp&>(base_op);
            str << "blob=" << ToString(op.blob) << ", x=" << ToString(op.x) << ", y=" << ToString(op.y) << ", flags=" << ToString(op.flags);
            break;
        }
    ...
    {
        return base::StringPrintf("[%.3f,%.3f %.3fx%.3f]", rect.x(), rect.y(), rect.width(), rect.height());
    }

    static std::string ToString(const SkIRect& rect)
    {
        return base::StringPrintf("[%d,%d %dx%d]", rect.x(), rect.y(), rect.width(), rect.height());
    }

    static std::string ToString(const SkRRect& rect)
    {
        return base::StringPrintf("[bounded by %.3f,%.3f %.3fx%.3f]", rect.rect().x(), rect.rect().y(), rect.rect().width(), rect.rect().height());
    }
    ...
    {
        return base::StringPrintf("[%8.4f %8.4f %8.4f %8.4f][%8.4f %8.4f %8.4f %8.4f][%8.4f %8.4f %8.4f "
                                  "%8.4f][%8.4f %8.4f %8.4f %8.4f]]",
            matrix.rc(0, 0), matrix.rc(0, 1), matrix.rc(0, 2), matrix.rc(0, 3), matrix.rc(1, 0), matrix.rc(1, 1), matrix.rc(1, 2), matrix.rc(1, 3),
            matrix.rc(2, 0), matrix.rc(2, 1), matrix.rc(2, 2), matrix.rc(2, 3), matrix.rc(3, 0), matrix.rc(3, 1), matrix.rc(3, 2), matrix.rc(3, 3));
    }

    static std::string ToString(const SkMatrix& matrix)
    {
        return base::StringPrintf("[%8.4f %8.4f %8.4f][%8.4f %8.4f %8.4f][%8.4f %8.4f %8.4f]]", matrix.rc(0, 0), matrix.rc(0, 1), matrix.rc(0, 2),
            matrix.rc(1, 0), matrix.rc(1, 1), matrix.rc(1, 2), matrix.rc(2, 0), matrix.rc(2, 1), matrix.rc(2, 2));
    }

    static std::string ToString(const SkColor& color)
    {
        return base::StringPrintf("rgba(%d, %d, %d, %d)", SkColorGetR(color), SkColorGetG(color), SkColorGetB(color), SkColorGetA(color));
    }

    static std::string ToString(const SkColor4f& color)
    {
        return base::StringPrintf("rgba(%f, %f, %f, %f)", color.fR, color.fG, color.fB, color.fA);
    }
    ...
    {
        return base::StringPrintf("SkPoint3(%s, %s, %s)", ToString(point.x()).c_str(), ToString(point.y()).c_str(), ToString(point.z()).c_str());
    }
    ...
    static std::string ToString(const SkData& data)
    {
        return "<SkData>";
    }
    ...
    static std::string ToString(const PathEffect& effect)
    {
        return "PathEffect";
    }

    static std::string ToString(const DrawLooper& looper)
    {
        return "DrawLooper";
    }
    ...
    static std::string ToString(const PaintFilter& base_filter)
    {
        ...
        case PaintFilter::Type::kColorFilter: {
            const auto& filter = static_cast<const ColorFilterPaintFilter&>(base_filter);
            str << "ColorFilterPaintFilter("
                << "color_filter=" << ToString(filter.color_filter()) << ", input=" << ToString(filter.input())
        ...
        case PaintFilter::Type::kBlur: {
            ...
            str << "BlurPaintFilter("
                << "sigma_x=" << ToString(filter.sigma_x()) << ", sigma_y=" << ToString(filter.sigma_y()) << ", tile_mode=" << ToString(filter.tile_mode())
                << ", input=" << ToString(filter.input()) << ", crop_rect=" << ToString(filter.GetCropRect()) << ")";
            break;
        }
        case PaintFilter::Type::kDropShadow: {
            const auto& filter = static_cast<const DropShadowPaintFilter&>(base_filter);
            str << "DropShadowPaintFilter("
                << "dx=" << ToString(filter.dx()) << ", dy=" << ToString(filter.dy()) << ", sigma_x=" << ToString(filter.sigma_x())
                << ", sigma_y=" << ToString(filter.sigma_y()) << ", color=" << ToString(filter.color()) << ", shadow_mode=" << ToString(filter.shadow_mode())
                << ", input=" << ToString(filter.input()) << ", crop_rect=" << ToString(filter.GetCropRect()) << ")";
            break;
        }
        case PaintFilter::Type::kMagnifier: {
            const auto& filter = static_cast<const MagnifierPaintFilter&>(base_filter);
            str << "MagnifierPaintFilter("
                << "lens_bounds=" << ToString(filter.lens_bounds()) << ", zoom_amount=" << ToString(filter.zoom_amount())
                << ", inset=" << ToString(filter.inset()) << ", input=" << ToString(filter.input()) << ", crop_rect=" << ToString(filter.GetCropRect()) << ")";
            break;
        }
        case PaintFilter::Type::kCompose: {
            const auto& filter = static_cast<const ComposePaintFilter&>(base_filter);
            str << "ComposePaintFilter("
                << "outer=" << ToString(filter.outer()) << ", inner=" << ToString(filter.inner()) << ", crop_rect=" << ToString(filter.GetCropRect()) << ")";
            break;
        }
        case PaintFilter::Type::kAlphaThreshold: {
            const auto& filter = static_cast<const AlphaThresholdPaintFilter&>(base_filter);
            str << "AlphaThresholdPaintFilter("
                << "region=" << ToString(filter.region()) << ", input=" << ToString(filter.input()) << ", crop_rect=" << ToString(filter.GetCropRect()) << ")";
            break;
        }
        case PaintFilter::Type::kXfermode: {
            const auto& filter = static_cast<const XfermodePaintFilter&>(base_filter);
            str << "XfermodePaintFilter("
                << "blend_mode=" << ToString(filter.blend_mode()) << ", background=" << ToString(filter.background())
                << ", foreground=" << ToString(filter.foreground()) << ", crop_rect=" << ToString(filter.GetCropRect()) << ")";
            break;
        }
        case PaintFilter::Type::kArithmetic: {
            const auto& filter = static_cast<const ArithmeticPaintFilter&>(base_filter);
            str << "ArithmeticPaintFilter("
                << "k1=" << filter.k1() << ", k2=" << filter.k2() << ", k3=" << filter.k3() << ", k4=" << filter.k4()
                << ", enfore_pm_color=" << filter.enforce_pm_color() << ", background=" << ToString(filter.background())
                << ", foreground=" << ToString(filter.foreground()) << ", crop_rect=" << ToString(filter.GetCropRect()) << ")";
            break;
        }
        case PaintFilter::Type::kMatrixConvolution: {
            const auto& filter = static_cast<const MatrixConvolutionPaintFilter&>(base_filter);
            const SkISize& kernel_size = filter.kernel_size();
            str << "MatrixConvolutionPaintFilter("
                << "kernel_size=" << ToString(kernel_size) << ", kernel=[";
            for (int32_t i = 0; i < kernel_size.width() * kernel_size.height(); ++i) {
                str << (i != 0 ? ", " : "") << ToString(filter.kernel_at(i));
            }
            str << "], gain=" << ToString(filter.gain()) << ", bias=" << ToString(filter.bias()) << ", kernel_offset=" << ToString(filter.kernel_offset())
                << ", tile_mode=" << ToString(filter.tile_mode()) << ", convolve_alpha=" << filter.convolve_alpha() << ", input=" << ToString(filter.input())
                << ", crop_rect=" << ToString(filter.GetCropRect()) << ")";
            break;
        }
        case PaintFilter::Type::kDisplacementMapEffect: {
            const auto& filter = static_cast<const DisplacementMapEffectPaintFilter&>(base_filter);
            str << "DisplacementMapEffectPaintFilter("
                << "channel_x=" << ToString(filter.channel_x()) << ", channel_y=" << ToString(filter.channel_y()) << ", scale=" << ToString(filter.scale())
                << ", displacement=" << ToString(filter.displacement()) << ", color=" << ToString(filter.color())
                << ", crop_rect=" << ToString(filter.GetCropRect()) << ")";
            break;
        }
        case PaintFilter::Type::kImage: {
            ...
            str << "ImagePaintFilter("
                << "image=" << ToString(filter.image()) << ", src_rect=" << ToString(filter.src_rect()) << ", dst_rect=" << ToString(filter.dst_rect())
                << ", filter_quality=" << ToString(filter.filter_quality()) << ", crop_rect=" << ToString(filter.GetCropRect()) << ")";
            break;
        }
        case PaintFilter::Type::kPaintRecord: {
            const auto& filter = static_cast<const RecordPaintFilter&>(base_filter);
            str << "RecordPaintFilter("
                << "record=" << ToString(filter.record()) << ", record_bounds=" << ToString(filter.record_bounds())
                << ", raster_scale=" << ToString(filter.raster_scale()) << ", scaling_behavior=" << ToString(filter.scaling_behavior())
                << ", crop_rect=" << ToString(filter.GetCropRect()) << ")";
            break;
        }
        ...
        case PaintFilter::Type::kMorphology: {
            const auto& filter = static_cast<const MorphologyPaintFilter&>(base_filter);
            str << "MorphologyPaintFilter("
                << "morph_type=" << ToString(filter.morph_type()) << ", radius_x=" << filter.radius_x() << ", radius_y=" << filter.radius_y()
                << ", input=" << ToString(filter.input()) << ", crop_rect=" << ToString(filter.GetCropRect()) << ")";
            break;
        }
        case PaintFilter::Type::kOffset: {
            ...
            str << "OffsetPaintFilter("
                << "dx=" << filter.dx() << ", dy=" << filter.dy() << ", input=" << ToString(filter.input()) << ", crop_rect=" << ToString(filter.GetCropRect())
                << ")";
            break;
        }
        case PaintFilter::Type::kTile: {
            ...
            str << "TilePaintFilter("
                << "src=" << ToString(filter.src()) << ", dst=" << ToString(filter.dst()) << ", input=" << ToString(filter.input())
                << ", crop_rect=" << ToString(filter.GetCropRect()) << ")";
            break;
        }
        case PaintFilter::Type::kTurbulence: {
            const auto& filter = static_cast<const TurbulencePaintFilter&>(base_filter);
            str << "TurbulencePaintFilter("
                << "turbulence_type=" << ToString(filter.turbulence_type()) << ", base_frequency_x=" << ToString(filter.base_frequency_x())
                << ", base_frequency_y=" << ToString(filter.base_frequency_y()) << ", num_octaves=" << filter.num_octaves()
                << ", seed=" << ToString(filter.seed()) << ", tile_size=" << ToString(filter.tile_size()) << ", crop_rect=" << ToString(filter.GetCropRect())
                << ")";
            break;
        }
        case PaintFilter::Type::kShader: {
            ...
            str << "ShaderPaintFilter("
                << "shader=" << ToString(filter.shader()) << ", alpha=" << ToString(filter.alpha()) << ", filter_quality=" << ToString(filter.filter_quality())
                << ", dither=" << ToString(filter.dither()) << ", crop_rect=" << ToString(filter.GetCropRect()) << ")";
            break;
        }
        case PaintFilter::Type::kMatrix: {
            const auto& filter = static_cast<const MatrixPaintFilter&>(base_filter);
            str << "MatrixPaintFilter("
                << "matrix=" << ToString(filter.matrix()) << ", filter_quality=" << ToString(filter.filter_quality()) << ", input=" << ToString(filter.input())
                << ", crop_rect=" << ToString(filter.GetCropRect()) << ")";
            break;
        }
        case PaintFilter::Type::kLightingDistant: {
            const auto& filter = static_cast<const LightingDistantPaintFilter&>(base_filter);
            str << "LightingDistantPaintFilter("
                << "lighting_type=" << ToString(filter.lighting_type()) << ", direction=" << ToString(filter.direction())
                << ", light_color=" << ToString(filter.light_color()) << ", surface_scale=" << ToString(filter.surface_scale())
                << ", kconstant=" << ToString(filter.kconstant()) << ", shininess=" << ToString(filter.shininess()) << ", input=" << ToString(filter.input())
                << ", crop_rect=" << ToString(filter.GetCropRect()) << ")";
            break;
        }
        case PaintFilter::Type::kLightingPoint: {
            const auto& filter = static_cast<const LightingPointPaintFilter&>(base_filter);
            str << "LightingPointPaintFilter("
                << "lighting_type=" << ToString(filter.lighting_type()) << ", location=" << ToString(filter.location())
                << ", light_color=" << ToString(filter.light_color()) << ", surface_scale=" << ToString(filter.surface_scale())
                << ", kconstant=" << ToString(filter.kconstant()) << ", shininess=" << ToString(filter.shininess()) << ", input=" << ToString(filter.input())
                << ", crop_rect=" << ToString(filter.GetCropRect()) << ")";
            break;
        ...
        case PaintFilter::Type::kLightingSpot: {
            const auto& filter = static_cast<const LightingSpotPaintFilter&>(base_filter);
            str << "LightingSpotPaintFilter("
                << "lighting_type=" << ToString(filter.lighting_type()) << ", location=" << ToString(filter.location())
                << ", target=" << ToString(filter.target()) << ", specular_exponent=" << ToString(filter.specular_exponent())
                << ", cutoff_angle=" << ToString(filter.cutoff_angle()) << ", light_color=" << ToString(filter.light_color())
                << ", surface_scale=" << ToString(filter.surface_scale()) << ", kconstant=" << ToString(filter.kconstant())
                << ", shininess=" << ToString(filter.shininess()) << ", input=" << ToString(filter.input()) << ", crop_rect=" << ToString(filter.GetCropRect())
                << ")";
        }
        }
        return str.str();
