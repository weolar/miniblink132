...
public:
    ...
    // support.
    static constexpr viz::RendererType kDefaultRendererType = viz::RendererType::kSkiaGL;
    ...
    void PostAddNoDamageAnimationToMainThread(Animation* animation_to_receive_animation);
    void PostAddOpacityAnimationToMainThread(Animation* animation_to_receive_animation);
    void PostAddOpacityAnimationToMainThreadInstantly(Animation* animation_to_receive_animation);
    void PostAddOpacityAnimationToMainThreadDelayed(Animation* animation_to_receive_animation);
    void PostSetLocalSurfaceIdToMainThread(const viz::LocalSurfaceId& local_surface_id);
    void PostRequestNewLocalSurfaceIdToMainThread();
    void PostGetDeferMainFrameUpdateToMainThread(std::unique_ptr<ScopedDeferMainFrameUpdate>* scoped_defer_main_frame_update);
    void PostReturnDeferMainFrameUpdateToMainThread(std::unique_ptr<ScopedDeferMainFrameUpdate> scoped_defer_main_frame_update);
    ...
    void SetUseLayerLists()
    {
        settings_.use_layer_lists = true;
    }

protected:
    explicit LayerTreeTest(viz::RendererType renderer_type = kDefaultRendererType);
    ...
    std::unique_ptr<LayerTreeFrameSink> ReleaseLayerTreeFrameSinkOnLayerTreeHost();
    ...
    virtual void SetUpUnboundContextProviders(viz::TestContextProvider* context_provider, viz::TestContextProvider* worker_context_provider);
    ...
    // created.
    virtual std::unique_ptr<TestLayerTreeFrameSink> CreateLayerTreeFrameSink(const viz::RendererSettings& renderer_settings, double refresh_rate,
    ...
    std::unique_ptr<viz::OutputSurface> CreateSoftwareOutputSurfaceOnThread() override;
    ...
    {
        return renderer_type_ == viz::RendererType::kSkiaGraphiteDawn || renderer_type_ == viz::RendererType::kSkiaGraphiteMetal;
    }
...
private:
    virtual void DispatchAddNoDamageAnimation(Animation* animation_to_receive_animation, double animation_duration);
    virtual void DispatchAddOpacityAnimation(Animation* animation_to_receive_animation, double animation_duration);
    void DispatchSetLocalSurfaceId(const viz::LocalSurfaceId& local_surface_id);
    void DispatchRequestNewLocalSurfaceId();
    void DispatchGetDeferMainFrameUpdate(std::unique_ptr<ScopedDeferMainFrameUpdate>* scoped_defer_main_frame_update);
    void DispatchReturnDeferMainFrameUpdate(std::unique_ptr<ScopedDeferMainFrameUpdate> scoped_defer_main_frame_update);
    ...
    std::unique_ptr<LayerTreeTestLayerTreeFrameSinkClient> layer_tree_frame_sink_client_;
...
#define MULTI_THREAD_BLOCKNOTIFY_TEST_F(TEST_FIXTURE_NAME) MULTI_THREAD_TEST_F(TEST_FIXTURE_NAME)
