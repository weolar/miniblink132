...
template <typename Images> auto ImagesAreSame(const Images& images)
{
    return ::testing::Pointwise(TwoImagesAreSame(), images);
}
template <typename Image> auto ImagesAreSame(std::initializer_list<Image> list)
{
...
{
    return arg.first == HashSkottieResourceId(resource_id) && arg.second.image.IsSameForTesting(frame_data.image) && arg.second.quality == frame_data.quality;
}

MATCHER_P3(SkottieImageIs, resource_id, image, quality, "")
{
    return arg.first == HashSkottieResourceId(resource_id) && arg.second.image.IsSameForTesting(image) && arg.second.quality == quality;
}

MATCHER_P3(SkottieTextIs, resource_id, text, box, "")
{
    return arg.first == HashSkottieResourceId(resource_id) && arg.second == SkottieTextPropertyValue(text, box);
}
