...
void TransferCacheTestHelper::CreateEntryDirect(const EntryKey& key, base::span<uint8_t> data)
{
    // Deserialize into a service transfer cache entry.
    std::unique_ptr<ServiceTransferCacheEntry> service_entry = ServiceTransferCacheEntry::Create(key.first);
    if (!service_entry)
        return;

    bool success = service_entry->Deserialize(context_, /*graphite_recorder=*/nullptr, data);
    if (!success)
        return;
...
void TransferCacheTestHelper::CreateLocalEntry(uint32_t id, std::unique_ptr<ServiceTransferCacheEntry> entry)
{
...
void TransferCacheTestHelper::UnlockEntriesDirect(const std::vector<EntryKey>& keys)
{
...
ServiceTransferCacheEntry* TransferCacheTestHelper::GetEntryInternal(TransferCacheEntryType type, uint32_t id)
{
...
uint32_t TransferCacheTestHelper::CreateEntryInternal(const ClientTransferCacheEntry& client_entry, uint8_t* memory)
{
