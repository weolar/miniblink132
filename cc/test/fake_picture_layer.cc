...
std::unique_ptr<LayerImpl> FakePictureLayer::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
...
scoped_refptr<RasterSource> FakePictureLayer::CreateRasterSource() const
{
    if (playback_allowed_event_) {
        return FakeRasterSource::CreateFromRecordingSourceWithWaitable(GetRecordingSourceForTesting(), playback_allowed_event_);
    }
