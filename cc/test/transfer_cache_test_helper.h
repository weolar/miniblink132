...
class TransferCacheTestHelper : public TransferCacheDeserializeHelper, public TransferCacheSerializeHelper {
public:
    ...
    ServiceTransferCacheEntry* GetEntryInternal(TransferCacheEntryType type, uint32_t id) override;

    const EntryKey& GetLastAddedEntry() const
    {
        return last_added_entry_;
    }

    void CreateLocalEntry(uint32_t id, std::unique_ptr<ServiceTransferCacheEntry> entry) override;
...
protected:
    ...
    uint32_t CreateEntryInternal(const ClientTransferCacheEntry& entry, uint8_t* memory) override;
