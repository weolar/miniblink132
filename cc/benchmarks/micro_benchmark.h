...
public:
    ...
    std::unique_ptr<MicroBenchmarkImpl> GetBenchmarkImpl(scoped_refptr<base::SingleThreadTaskRunner> origin_task_runner);

protected:
    void NotifyDone(base::Value::Dict result);

    virtual std::unique_ptr<MicroBenchmarkImpl> CreateBenchmarkImpl(scoped_refptr<base::SingleThreadTaskRunner> origin_task_runner);
