...
std::unique_ptr<MicroBenchmark> CreateBenchmark(const std::string& name, base::Value::Dict settings, MicroBenchmark::DoneCallback callback)
{
    if (name == "invalidation_benchmark") {
        return std::make_unique<InvalidationBenchmark>(std::move(settings), std::move(callback));
    } else if (name == "rasterize_and_record_benchmark") {
        return std::make_unique<RasterizeAndRecordBenchmark>(std::move(settings), std::move(callback));
    } else if (name == "unittest_only_benchmark") {
        return std::make_unique<UnittestOnlyBenchmark>(std::move(settings), std::move(callback));
    }
    return nullptr;
}
...
MicroBenchmarkController::MicroBenchmarkController(LayerTreeHost* host)
    : host_(host)
    , main_controller_task_runner_(base::SingleThreadTaskRunner::HasCurrentDefault() ? base::SingleThreadTaskRunner::GetCurrentDefault() : nullptr)
{
    DCHECK(host_);
}
...
int MicroBenchmarkController::ScheduleRun(const std::string& micro_benchmark_name, base::Value::Dict settings, MicroBenchmark::DoneCallback callback)
{
    std::unique_ptr<MicroBenchmark> benchmark = CreateBenchmark(micro_benchmark_name, std::move(settings), std::move(callback));
    if (benchmark.get()) {
...
std::vector<std::unique_ptr<MicroBenchmarkImpl>> MicroBenchmarkController::CreateImplBenchmarks() const
{
...
{
    std::erase_if(benchmarks_, [](const std::unique_ptr<MicroBenchmark>& benchmark) { return benchmark->IsDone(); });
}
