...
public:
    void SetUp() override
    {
        ...
        layer_tree_host_impl_ = std::make_unique<FakeLayerTreeHostImpl>(impl_task_runner_provider_.get(), &task_graph_runner_);

        animation_host_ = AnimationHost::CreateForTesting(ThreadInstance::kMain);
        layer_tree_host_ = FakeLayerTreeHost::Create(&layer_tree_host_client_, &task_graph_runner_, animation_host_.get());
        layer_tree_host_->SetRootLayer(Layer::Create());
        layer_tree_host_->InitializeForTesting(
            TaskRunnerProvider::Create(base::SingleThreadTaskRunner::GetCurrentDefault(), nullptr), std::unique_ptr<Proxy>(new FakeProxy));
    }
...
{
    int id = layer_tree_host_->ScheduleMicroBenchmark("non_existant_benchmark", base::Value::Dict(), base::DoNothing());
    EXPECT_EQ(id, 0);
}
...
TEST_F(MicroBenchmarkControllerTest, CommitScheduled)
{
    ...
    int id = layer_tree_host_->ScheduleMicroBenchmark("unittest_only_benchmark", base::Value::Dict(), base::DoNothing());
    EXPECT_GT(id, 0);
...
TEST_F(MicroBenchmarkControllerTest, BenchmarkRan)
{
    ...
    int id = layer_tree_host_->ScheduleMicroBenchmark(
        "unittest_only_benchmark", base::Value::Dict(), base::BindOnce(&IncrementCallCount, base::Unretained(&run_count)));
    EXPECT_GT(id, 0);
...
TEST_F(MicroBenchmarkControllerTest, BenchmarkImplRan)
{
    ...
    int id = layer_tree_host_->ScheduleMicroBenchmark(
        "unittest_only_benchmark", std::move(settings), base::BindOnce(&IncrementCallCount, base::Unretained(&run_count)));
    EXPECT_GT(id, 0);

    // Scheduling benchmarks on the impl thread is usually done during
    // LayerTreeHostImpl::FinishCommit().
    for (auto& benchmark : layer_tree_host_->GetMicroBenchmarkController()->CreateImplBenchmarks()) {
