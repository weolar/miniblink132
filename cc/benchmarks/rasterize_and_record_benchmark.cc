...
RasterizeAndRecordBenchmark::RasterizeAndRecordBenchmark(base::Value::Dict settings, MicroBenchmark::DoneCallback callback)
...
void RasterizeAndRecordBenchmark::DidUpdateLayers(LayerTreeHost* layer_tree_host)
{
    ...
    layer_tree_host->client()->RunPaintBenchmark(record_repeat_count_, paint_benchmark_result);

    DCHECK(results_.empty());
    results_.Set("pixels_recorded", record_results_.pixels_recorded);
    results_.Set("paint_op_memory_usage", static_cast<int>(record_results_.paint_op_memory_usage));
    results_.Set("paint_op_count", static_cast<int>(record_results_.paint_op_count));
    results_.Set("record_time_ms", paint_benchmark_result.record_time_ms);
    results_.Set("record_time_caching_disabled_ms", paint_benchmark_result.record_time_caching_disabled_ms);
    results_.Set("record_time_subsequence_caching_disabled_ms", paint_benchmark_result.record_time_subsequence_caching_disabled_ms);
    results_.Set("raster_invalidation_and_convert_time_ms", paint_benchmark_result.raster_invalidation_and_convert_time_ms);
    results_.Set("paint_artifact_compositor_update_time_ms", paint_benchmark_result.paint_artifact_compositor_update_time_ms);
    results_.Set("painter_memory_usage", static_cast<int>(paint_benchmark_result.painter_memory_usage));
    main_thread_benchmark_done_ = true;
}

void RasterizeAndRecordBenchmark::RecordRasterResults(base::Value::Dict results_value)
{
...
std::unique_ptr<MicroBenchmarkImpl> RasterizeAndRecordBenchmark::CreateBenchmarkImpl(scoped_refptr<base::SingleThreadTaskRunner> origin_task_runner)
{
    return base::WrapUnique(new RasterizeAndRecordBenchmarkImpl(
        origin_task_runner, rasterize_repeat_count_, base::BindOnce(&RasterizeAndRecordBenchmark::RecordRasterResults, weak_ptr_factory_.GetWeakPtr())));
}
...
void RasterizeAndRecordBenchmark::RunOnLayer(PictureLayer* layer)
{
    ...
    scoped_refptr<DisplayItemList> display_list = painter->PaintContentsToDisplayList();
    record_results_.paint_op_memory_usage += display_list->BytesUsed();
    record_results_.paint_op_count += display_list->TotalOpCount();
    gfx::Rect bounds = display_list->bounds().value_or(gfx::Rect(layer->bounds()));
