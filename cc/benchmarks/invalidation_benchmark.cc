...
InvalidationBenchmark::InvalidationBenchmark(base::Value::Dict settings, MicroBenchmark::DoneCallback callback)
    : MicroBenchmark(std::move(callback))
{
    ...
    if (mode_string == "fixed_size") {
        ...
        CHECK(false) << "Invalid mode: " << mode_string << ". One of {fixed_size, layer, viewport, random} expected.";
    }
}
...
void InvalidationBenchmark::RunOnLayer(PictureLayer* layer)
{
    ...
    gfx::Transform from_screen = layer->ScreenSpaceTransform().InverseOrIdentity();
    gfx::Rect viewport_rect = MathUtil::ProjectEnclosingClippedRect(from_screen, layer->layer_tree_host()->device_viewport_rect());
...
float InvalidationBenchmark::LCGRandom()
{
    ...
    seed_ = a * seed_ + c;
    return static_cast<float>(seed_) / static_cast<float>(std::numeric_limits<uint32_t>::max());
}
