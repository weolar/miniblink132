...
void RunBenchmark(const PictureLayerTiling& tiling, ImageDecodeCache* image_decode_cache, const gfx::Rect& content_rect, const gfx::Vector2dF& contents_scale,
    size_t repeat_count, base::TimeDelta* min_time, bool* is_solid_color)
{
    ...
    *min_time = base::TimeDelta::Max();
    for (size_t i = 0; i < repeat_count; ++i) {
        ...
        base::LapTimer timer(kWarmupRuns, base::Milliseconds(kTimeLimitMillis), kTimeCheckInterval);
        SkColor4f color = SkColors::kTransparent;
        gfx::Rect layer_rect = gfx::ScaleToEnclosingRect(content_rect, 1.f / contents_scale.x(), 1.f / contents_scale.y());
        *is_solid_color = raster_source->PerformSolidColorAnalysis(layer_rect, &color);

        do {
            SkBitmap bitmap;
            bitmap.allocPixels(SkImageInfo::MakeN32Premul(content_rect.width(), content_rect.height()));
            ...
            PlaybackImageProvider image_provider(image_decode_cache, TargetColorParams(), std::move(image_settings));
            RasterSource::PlaybackSettings settings;
            ScrollOffsetMap raster_inducing_scroll_offsets = tiling.client()->GetRasterInducingScrollOffsets();
            settings.image_provider = &image_provider;
            settings.raster_inducing_scroll_offsets = &raster_inducing_scroll_offsets;

            raster_source->PlaybackToCanvas(&canvas, raster_source->GetContentSize(contents_scale), content_rect, content_rect,
                gfx::AxisTransform2d::FromScaleAndTranslation(contents_scale, gfx::Vector2dF()), settings);
...
class FixedInvalidationPictureLayerTilingClient : public PictureLayerTilingClient {
public:
    FixedInvalidationPictureLayerTilingClient(PictureLayerTilingClient* base_client, const Region& invalidation)
    ...
    const PictureLayerTiling* GetPendingOrActiveTwinTiling(const PictureLayerTiling* tiling) const override
    {
    ...
    std::vector<const DrawImage*> GetDiscardableImagesInRect(const gfx::Rect& rect) const override
    {
...
RasterizeAndRecordBenchmarkImpl::RasterizeAndRecordBenchmarkImpl(
    scoped_refptr<base::SingleThreadTaskRunner> origin_task_runner, int rasterize_repeat_count, MicroBenchmarkImpl::DoneCallback callback)
...
void RasterizeAndRecordBenchmarkImpl::DidCompleteCommit(LayerTreeHostImpl* host)
{
    ...
    result.Set("rasterize_time_ms", rasterize_results_.total_best_time.InMillisecondsF());
    result.Set("pixels_rasterized", rasterize_results_.pixels_rasterized);
    result.Set("pixels_rasterized_with_non_solid_color", rasterize_results_.pixels_rasterized_with_non_solid_color);
    result.Set("pixels_rasterized_as_opaque", rasterize_results_.pixels_rasterized_as_opaque);
    ...
    result.Set("total_picture_layers_with_no_content", rasterize_results_.total_picture_layers_with_no_content);
    result.Set("total_picture_layers_off_screen", rasterize_results_.total_picture_layers_off_screen);

    base::Value::Dict lcd_text_pixels;
    for (size_t i = 0; i < kLCDTextDisallowedReasonCount; i++) {
        lcd_text_pixels.Set(
            LCDTextDisallowedReasonToString(static_cast<LCDTextDisallowedReason>(i)), rasterize_results_.visible_pixels_by_lcd_text_disallowed_reason[i]);
    }
    result.Set("visible_pixels_by_lcd_text_disallowed_reason", std::move(lcd_text_pixels));
...
void RasterizeAndRecordBenchmarkImpl::RunOnLayer(PictureLayerImpl* layer)
{
    ...
    int text_pixels = layer->GetRasterSource()->GetDisplayItemList()->AreaOfDrawText(layer->visible_layer_rect());
    rasterize_results_.visible_pixels_by_lcd_text_disallowed_reason[static_cast<size_t>(layer->lcd_text_disallowed_reason())] += text_pixels;

    FixedInvalidationPictureLayerTilingClient client(layer, gfx::Rect(layer->bounds()));
    ...
    std::unique_ptr<PictureLayerTilingSet> tiling_set
        = PictureLayerTilingSet::Create(layer->IsActive() ? ACTIVE_TREE : PENDING_TREE, &client, settings.tiling_interest_area_padding,
    ...
    PictureLayerTiling* tiling
        = tiling_set->AddTiling(gfx::AxisTransform2d::FromScaleAndTranslation(layer->raster_contents_scale_, gfx::Vector2dF()), layer->GetRasterSource());
    tiling->set_resolution(HIGH_RESOLUTION);
    tiling->CreateAllTilesForTesting();
    for (PictureLayerTiling::CoverageIterator it(tiling, tiling->contents_scale_key(), layer->visible_layer_rect()); it; ++it) {
        DCHECK(*it);
        ...
        RunBenchmark(
            *tiling, layer->layer_tree_impl()->image_decode_cache(), content_rect, contents_scale, rasterize_repeat_count_, &min_time, &is_solid_color);
...
RasterizeAndRecordBenchmarkImpl::RasterizeResults::~RasterizeResults() = default;
