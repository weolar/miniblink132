...
scoped_refptr<AnimationTimeline> AnimationTimeline::Create(int id, bool impl_only)
{
...
void AnimationTimeline::AttachAnimation(scoped_refptr<Animation> animation)
{
    ...
    id_to_animation_map_.Write(*this).insert(std::make_pair(animation->id(), std::move(animation)));
...
Animation* AnimationTimeline::GetAnimationById(int animation_id) const
{
    ...
    return f == id_to_animation_map_.Read(*this).end() ? nullptr : f->second.get();
}
...
bool AnimationTimeline::TickTimeLinkedAnimations(
    const std::vector<scoped_refptr<Animation>>& ticking_animations, base::TimeTicks monotonic_time, bool tick_finished)
{
...
void AnimationTimeline::PushAttachedAnimationsToImplThread(AnimationTimeline* timeline_impl) const
{
    for (auto& kv : id_to_animation_map_.Read(*this)) {
        auto& animation = kv.second;
        Animation* animation_impl = timeline_impl->GetAnimationById(animation->id());
        if (animation_impl)
            continue;
...
void AnimationTimeline::RemoveDetachedAnimationsFromImplThread(AnimationTimeline* timeline_impl) const
{
    IdToAnimationMap& animations_impl = timeline_impl->id_to_animation_map_.Write(*this);
...
void AnimationTimeline::PushPropertiesToImplThread(AnimationTimeline* timeline_impl)
{
    for (auto& kv : id_to_animation_map_.Read(*this)) {
        Animation* animation = kv.second.get();
        if (Animation* animation_impl = timeline_impl->GetAnimationById(animation->id())) {
