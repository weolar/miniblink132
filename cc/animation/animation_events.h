...
struct CC_ANIMATION_EXPORT AnimationEvent {
    ...
    AnimationEvent(Type type, UniqueKeyframeModelId uid, int group_id, int target_property, base::TimeTicks monotonic_time);

    // Constructs AnimationEvent of TIME_UPDATED type.
    AnimationEvent(int timeline_id, int animation_id, std::optional<base::TimeDelta> local_time);
...
public:
    ...
    bool needs_time_updated_events() const
    {
        return needs_time_updated_events_;
    }
