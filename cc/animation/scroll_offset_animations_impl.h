...
public:
    ...
    ScrollOffsetAnimationImpl& operator=(const ScrollOffsetAnimationImpl&) = delete;

    void AutoScrollAnimationCreate(ElementId element_id, const gfx::PointF& target_offset, const gfx::PointF& current_offset, float autoscroll_velocity,
        base::TimeDelta animation_start_offset);

    // |delayed_by| shrinks the duration of the
    // animation. |animation_start_offset| causes us to start the animation
    // partway through.
    void MouseWheelScrollAnimationCreate(ElementId element_id, const gfx::PointF& target_offset, const gfx::PointF& current_offset, base::TimeDelta delayed_by,
        base::TimeDelta animation_start_offset);
    ...
    void ScrollAnimationApplyAdjustment(ElementId element_id, const gfx::Vector2dF& adjustment);
...
private:
    void ScrollAnimationCreateInternal(ElementId element_id, std::unique_ptr<gfx::AnimationCurve> curve, base::TimeDelta animation_start_offset);
...
public:
    ...
    ScrollOffsetAnimationsImpl& operator=(const ScrollOffsetAnimationsImpl&) = delete;
    ...
    std::optional<gfx::PointF> ScrollAnimationUpdateTarget(const gfx::Vector2dF& scroll_delta, const gfx::PointF& max_scroll_offset,
...
private:
    ...
    base::flat_map<ElementId, std::unique_ptr<ScrollOffsetAnimationImpl>> element_to_animation_map_;
};
