...
WorkletAnimation::WorkletAnimation(int cc_animation_id, WorkletAnimationId worklet_animation_id, const std::string& name, double playback_rate_value,
    std::unique_ptr<AnimationOptions> options, std::unique_ptr<AnimationEffectTimings> effect_timings, bool is_controlling_instance)
...
scoped_refptr<WorkletAnimation> WorkletAnimation::Create(WorkletAnimationId worklet_animation_id, const std::string& name, double playback_rate_value,
    std::unique_ptr<AnimationOptions> options, std::unique_ptr<AnimationEffectTimings> effect_timings)
{
    return WrapRefCounted(new WorkletAnimation(
        AnimationIdProvider::NextAnimationId(), worklet_animation_id, name, playback_rate_value, std::move(options), std::move(effect_timings), false));
}

scoped_refptr<Animation> WorkletAnimation::CreateImplInstance() const
{
    return WrapRefCounted(new WorkletAnimation(id(), worklet_animation_id_, name(), playback_rate(), CloneOptions(), CloneEffectTimings(), true));
}
...
void WorkletAnimation::UpdateState(bool start_ready_animations, AnimationEvents* events)
{
...
void WorkletAnimation::TakeTimeUpdatedEvent(AnimationEvents* events)
{
    ...
    if (last_synced_local_time_.Read(*this) != local_time_.Read(*this)) {
        AnimationEvent event(animation_timeline()->id(), id_, local_time_.Read(*this));
        events->events_.push_back(event);
        last_synced_local_time_.Write(*this) = local_time_.Read(*this);
    }
}

void WorkletAnimation::UpdateInputState(MutatorInputState* input_state, base::TimeTicks monotonic_time, const ScrollTree& scroll_tree, bool is_active_tree)
{
    ...
    if (!start_time_.Read(*this).has_value() && is_timeline_active)
        start_time_.Write(*this) = animation_timeline()->IsScrollTimeline() ? base::TimeTicks() : monotonic_time;
    ...
    // State::REMOVED.
    std::optional<base::TimeDelta> current_time = CurrentTime(monotonic_time, scroll_tree, is_active_tree);
    ...
    has_pending_tree_lock_.Write(*this) = !is_active_tree && animation_timeline()->IsScrollTimeline();

    switch (state_.Read(*this)) {
    case State::kPending:
        input_state->Add({ worklet_animation_id(), name(), current_time->InMillisecondsF(), CloneOptions(), CloneEffectTimings() });
    ...
    case State::kRunning:
        ...
        input_state->Update({ worklet_animation_id(), current_time->InMillisecondsF() });
        break;
    case State::kRemoved:
...
void WorkletAnimation::SetOutputState(const MutatorOutputState::AnimationState& state)
{
...
void WorkletAnimation::SetPlaybackRate(double rate)
{
    ...
    if (start_time_.Read(*this) && last_current_time_.Read(*this)) {
        ...
        start_time_.Write(*this) = start_time_.Read(*this).value() + current_time / playback_rate() - current_time / rate;
    }
...
std::optional<base::TimeDelta> WorkletAnimation::CurrentTime(base::TimeTicks monotonic_time, const ScrollTree& scroll_tree, bool is_active_tree)
{
    DCHECK(IsTimelineActive(scroll_tree, is_active_tree));
    base::TimeTicks timeline_time;
    if (animation_timeline()->IsScrollTimeline()) {
        std::optional<base::TimeTicks> scroll_monotonic_time = ToScrollTimeline(animation_timeline())->CurrentTime(scroll_tree, is_active_tree);
...
bool WorkletAnimation::NeedsUpdate(base::TimeTicks monotonic_time, const ScrollTree& scroll_tree, bool is_active_tree)
{
...
bool WorkletAnimation::IsTimelineActive(const ScrollTree& scroll_tree, bool is_active_tree) const
{
    if (!animation_timeline()->IsScrollTimeline())
        return true;

    return ToScrollTimeline(animation_timeline())->IsActive(scroll_tree, is_active_tree);
}
