...
{
    return direction == ScrollTimeline::ScrollUp || direction == ScrollTimeline::ScrollDown;
}

bool IsReverse(ScrollTimeline::ScrollDirection direction)
{
    return direction == ScrollTimeline::ScrollUp || direction == ScrollTimeline::ScrollLeft;
}

} // namespace

ScrollTimeline::ScrollTimeline(
    std::optional<ElementId> scroller_id, ScrollDirection direction, std::optional<ScrollOffsets> scroll_offsets, int animation_timeline_id)
...
scoped_refptr<ScrollTimeline> ScrollTimeline::Create(
    std::optional<ElementId> scroller_id, ScrollTimeline::ScrollDirection direction, std::optional<ScrollOffsets> scroll_offsets)
{
    return base::WrapRefCounted(new ScrollTimeline(scroller_id, direction, scroll_offsets, AnimationIdProvider::NextTimelineId()));
}

scoped_refptr<AnimationTimeline> ScrollTimeline::CreateImplInstance() const
{
    return base::WrapRefCounted(new ScrollTimeline(pending_id(), direction(), pending_offsets(), id()));
}

bool ScrollTimeline::IsActive(const ScrollTree& scroll_tree, bool is_active_tree) const
{
    // Blink passes empty scroll offsets when the timeline is inactive.
    if ((is_active_tree && !active_offsets()) || (!is_active_tree && !pending_offsets())) {
        return false;
    }
    ...
    ElementId scroller_id = is_active_tree ? active_id().value() : pending_id().value();
...
std::optional<base::TimeTicks> ScrollTimeline::CurrentTime(const ScrollTree& scroll_tree, bool is_active_tree) const
{
    ...
    ElementId scroller_id = is_active_tree ? active_id().value() : pending_id().value();
    const ScrollNode* scroll_node = scroll_tree.FindNodeFromElementId(scroller_id);

    gfx::PointF offset = scroll_tree.GetScrollOffsetForScrollTimeline(*scroll_node);
    DCHECK_GE(offset.x(), 0);
    DCHECK_GE(offset.y(), 0);

    gfx::PointF scroll_dimensions = scroll_tree.MaxScrollOffset(scroll_node->id);

    double max_offset = IsVertical(direction()) ? scroll_dimensions.y() : scroll_dimensions.x();
    double current_physical_offset = IsVertical(direction()) ? offset.y() : offset.x();
    double current_offset = IsReverse(direction()) ? max_offset - current_physical_offset : current_physical_offset;
    ...
    int64_t progress_us = base::ClampRound((current_offset - start_offset) * kScrollTimelineMicrosecondsPerPixel);
    return base::TimeTicks() + base::Microseconds(progress_us);
}

std::optional<base::TimeTicks> ScrollTimeline::Duration(const ScrollTree& scroll_tree, bool is_active_tree) const
{
    double start_offset = 0;
    double end_offset = 0;
    if (is_active_tree) {
    ...
    int64_t duration_us = base::ClampRound((end_offset - start_offset) * kScrollTimelineMicrosecondsPerPixel);
...
bool ScrollTimeline::TickScrollLinkedAnimations(
    const std::vector<scoped_refptr<Animation>>& ticking_animations, const ScrollTree& scroll_tree, bool is_active_tree)
{
    std::optional<base::TimeTicks> tick_time = CurrentTime(scroll_tree, is_active_tree);
    if (!tick_time)
        return false;
...
void ScrollTimeline::UpdateScrollerIdAndScrollOffsets(std::optional<ElementId> pending_id, std::optional<ScrollOffsets> pending_offsets)
{
    if (pending_id_.Read(*this) == pending_id && pending_offsets_.Read(*this) == pending_offsets) {
        return;
    }
