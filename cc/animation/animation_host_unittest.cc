...
public:
    ...
    void AttachWorkletAnimation()
    {
        ...
        worklet_animation_ = WorkletAnimation::Create(worklet_animation_id_, "test_name", 1, nullptr, nullptr);
        ...
        worklet_animation_impl_ = ToWorkletAnimation(timeline_impl_->GetAnimationById(cc_id));
    }
...
TEST_F(AnimationHostTest, SyncTimelinesAddRemove)
{
    ...
    scoped_refptr<AnimationTimeline> timeline(AnimationTimeline::Create(timeline_id));
...
TEST_F(AnimationHostTest, ImplOnlyTimeline)
{
    ...
    scoped_refptr<AnimationTimeline> timeline(AnimationTimeline::Create(timeline_id1));
    scoped_refptr<AnimationTimeline> timeline_impl(AnimationTimeline::Create(timeline_id2, /* is_impl_only */ true));
...
TEST_F(AnimationHostTest, ImplOnlyScrollAnimationUpdateTargetIfDetached)
{
    ...
    host_impl_->ImplOnlyScrollAnimationCreate(element_id_, target_offset, current_offset, base::TimeDelta(), base::TimeDelta());
    ...
    EXPECT_TRUE(host_impl_->ImplOnlyScrollAnimationUpdateTarget(scroll_delta, max_scroll_offset, time, base::TimeDelta(), element_id_));

    // Detach all animations from layers and timelines.
    host_impl_->ClearMutators();

    time += base::Seconds(0.1);
    EXPECT_FALSE(host_impl_->ImplOnlyScrollAnimationUpdateTarget(scroll_delta, max_scroll_offset, time, base::TimeDelta(), element_id_));
}
...
TEST_F(AnimationHostTest, FastLayerTreeMutatorUpdateTakesEffectInSameFrame)
{
    ...
    const float expected_opacity = start_opacity + (end_opacity - start_opacity) / 2;
    AddOpacityTransitionToAnimation(worklet_animation_.get(), duration, start_opacity, end_opacity, true);

    base::TimeDelta local_time = base::Seconds(duration / 2);

    MockLayerTreeMutator* mock_mutator = new NiceMock<MockLayerTreeMutator>();
    host_impl_->SetLayerTreeMutator(base::WrapUnique<LayerTreeMutator>(mock_mutator));
    ON_CALL(*mock_mutator, HasMutators()).WillByDefault(Return(true));
    ON_CALL(*mock_mutator, MutateRef(_)).WillByDefault(InvokeWithoutArgs([this, local_time]() { this->SetOutputState(local_time); }));
    ...
    TestLayer* layer = client_.FindTestLayer(element_id_, ElementListType::ACTIVE);
    EXPECT_FALSE(layer->is_property_mutated(TargetProperty::OPACITY));
    client_impl_.ExpectOpacityPropertyMutated(element_id_, ElementListType::ACTIVE, expected_opacity);
}
...
bool Animation1TimeEquals20(MutatorInputState* input)
{
    ...
    return in && in->added_and_updated_animations.size() == 1 && in->added_and_updated_animations[0].worklet_animation_id.animation_id == 22;
...
void CreateScrollingNodeForElement(ElementId element_id, PropertyTrees* property_trees, bool is_composited = true)
{
    ...
    property_trees->transform_tree_mutable().SetElementIdForNodeId(transform_node_id, element_id);
    ...
    if (!property_trees->is_main_thread()) {
        property_trees->scroll_tree_mutable().GetOrCreateSyncedScrollOffsetForTesting(element_id);
    }
    property_trees->scroll_tree_mutable().SetElementIdForNodeId(scroll_node_id, element_id);
}

void SetScrollOffset(PropertyTrees* property_trees, ElementId element_id, gfx::PointF offset)
{
    // Update both scroll and transform trees
    property_trees->scroll_tree_mutable().SetScrollOffset(element_id, offset);
    TransformNode* transform_node = property_trees->transform_tree_mutable().FindNodeFromElementId(element_id);
...
TEST_F(AnimationHostTest, DISABLED_LayerTreeMutatorUpdateReflectsScrollAnimations)
{
    ...
    scoped_refptr<MockAnimation> mock_scroll_animation(new MockAnimation(animation_id1));
    EXPECT_CALL(*mock_scroll_animation, Tick(_)).WillOnce(InvokeWithoutArgs([&]() -> bool {
    ...
    auto scroll_timeline = ScrollTimeline::Create(element_id, ScrollTimeline::ScrollDown, scroll_offsets);

    // Create a worklet animation that is bound to the scroll timeline.
    scoped_refptr<WorkletAnimation> worklet_animation(new WorkletAnimation(animation_id2, worklet_animation_id, "test_name", 1, nullptr, nullptr, true));
    ...
    host_impl_->SetLayerTreeMutator(base::WrapUnique<LayerTreeMutator>(mock_mutator));
    ON_CALL(*mock_mutator, HasMutators()).WillByDefault(Return(true));
    EXPECT_CALL(*mock_mutator, MutateRef(::testing::Truly(Animation1TimeEquals20))).Times(1);
    ...
    // frame.
    host_impl_->TickAnimations(base::TimeTicks(), property_trees.scroll_tree(), false);
}
...
TEST_F(AnimationHostTest, TickScrollLinkedAnimation)
{
    ...
    // animation together.
    ScrollTimeline::ScrollOffsets scroll_offsets(0, 100);
    auto scroll_timeline = ScrollTimeline::Create(element_id_, ScrollTimeline::ScrollDown, scroll_offsets);
    ...
    EXPECT_EQ(keyframe_model->run_state(), KeyframeModel::WAITING_FOR_TARGET_AVAILABILITY);
    ...
    double tick_time = ToMilliseconds(scroll_timeline->CurrentTime(scroll_tree, false));
    double duration = ToMilliseconds(scroll_timeline->Duration(scroll_tree, false));
...
TEST_F(AnimationHostTest, TickScrollLinkedAnimationNonCompositedScroll)
{
    ...
    EXPECT_EQ(keyframe_model->run_state(), KeyframeModel::STARTING);
    double tick_time = (scroll_timeline->CurrentTime(scroll_tree, false).value() - base::TimeTicks()).InMillisecondsF();
...
TEST_F(AnimationHostTest, TickScrollLinkedAnimationSmooth)
{
    ...
    double duration = 0.001 * ToMilliseconds(scroll_timeline->Duration(scroll_tree, false));
    ...
    scoped_refptr<MockAnimation> mock_scroll_animation(new MockAnimation(scroll_animation_id));
    EXPECT_CALL(*mock_scroll_animation, Tick(_)).WillOnce(InvokeWithoutArgs([&]() -> bool {
    ...
    client_impl_.ExpectOpacityPropertyMutated(element_id, ElementListType::ACTIVE, expected_opacity);
}
...
TEST_F(AnimationHostTest, ScrollTimelineOffsetUpdatedByScrollAnimation)
{
    ...
    int animation_id = 11;
    scoped_refptr<MockAnimation> mock_scroll_animation(new MockAnimation(animation_id));
    EXPECT_CALL(*mock_scroll_animation, Tick(_)).WillOnce(InvokeWithoutArgs([&]() -> bool {
        // Scroll to 20% of the max value.
        SetScrollOffset(&property_trees, element_id_, gfx::PointF(0, 20));
        return true;
    ...
    double tick_time = ToMilliseconds(scroll_timeline->CurrentTime(property_trees.scroll_tree(), false));
    double duration = ToMilliseconds(scroll_timeline->Duration(property_trees.scroll_tree(), false));
    EXPECT_NEAR(tick_time, 0.2 * duration, 1e-6);
}
