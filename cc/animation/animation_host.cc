...
AnimationWorkletMutationState ToAnimationWorkletMutationState(MutateStatus status)
{
    switch (status) {
...
std::unique_ptr<AnimationHost> AnimationHost::CreateForTesting(ThreadInstance thread_instance)
{
...
std::unique_ptr<MutatorHost> AnimationHost::CreateImplInstance() const
{
    ...
    auto mutator_host_impl = base::WrapUnique<MutatorHost>(new AnimationHost(ThreadInstance::kImpl));
...
AnimationTimeline* AnimationHost::GetTimelineById(int timeline_id)
{
    ...
    return f == id_to_timeline_map_.Write(*this).end() ? nullptr : f->second.get();
}
...
base::TimeDelta AnimationHost::MinimumTickInterval() const
{
    ...
    for (const auto& animation : ticking_animations_.Read(*this)) {
        ...
        base::TimeDelta interval = animation->keyframe_effect()->MinimumTickInterval();
        if (interval.is_zero())
            return interval;
...
void AnimationHost::AddAnimationTimeline(scoped_refptr<AnimationTimeline> timeline)
{
    DCHECK(timeline->id());
    id_to_timeline_map_.Write(*this).insert(std::make_pair(timeline->id(), timeline));
...
void AnimationHost::DetachAnimationTimeline(scoped_refptr<AnimationTimeline> timeline)
{
    if (InProtectedSequence()) {
        // Defer cleanup until post-commit.
        detached_timeline_map_.Write(*this).insert(std::make_pair(timeline->id(), timeline));
    } else {
...
void AnimationHost::RegisterAnimationForElement(ElementId element_id, Animation* animation)
{
    DCHECK(element_id);
    DCHECK(animation);
#if DCHECK_IS_ON()
    for (const auto& keyframe_model : animation->keyframe_effect()->keyframe_models()) {
        KeyframeModel* cc_keyframe_model = KeyframeModel::ToCcKeyframeModel(keyframe_model.get());
        ElementId model_element_id = cc_keyframe_model->element_id() ? cc_keyframe_model->element_id() : element_id;
        DCHECK(cc_keyframe_model->affects_active_elements() || cc_keyframe_model->affects_pending_elements());
        DCHECK(!cc_keyframe_model->affects_active_elements() || mutator_host_client()->IsElementInPropertyTrees(model_element_id, ElementListType::ACTIVE));
        // Test thread_instance_ because LayerTreeHost has no pending tree.
        DCHECK(thread_instance_ == ThreadInstance::kMain || !cc_keyframe_model->affects_pending_elements()
            || mutator_host_client()->IsElementInPropertyTrees(model_element_id, ElementListType::PENDING));
    }
#endif
    ...
    if (!element_animations) {
        ...
        element_to_animations_map_.Write(*this)[element_animations->element_id()] = element_animations;
    }
...
void AnimationHost::UnregisterAnimationForElement(ElementId element_id, Animation* animation)
{
    DCHECK(element_id);
    DCHECK(animation);
    ...
    PropertyToElementIdMap element_id_map = element_animations->GetPropertyToElementIdMap();
    ...
    if (element_animations->IsEmpty()) {
        ...
        element_to_animations_map_.Write(*this).erase(element_animations->element_id());
...
void AnimationHost::UpdateClientAnimationStateForElementAnimations(ElementId element_id)
{
...
void AnimationHost::SetMutatorHostClient(MutatorHostClient* client)
{
    ...
    if (mutator_host_client() && !scroll_offset_animations_impl_.Read(*this)) {
        if (thread_instance_ == ThreadInstance::kImpl) {
            scroll_offset_animations_impl_.Write(*this) = std::make_unique<ScrollOffsetAnimationsImpl>(this);
        } else {
            scroll_offset_animations_.Write(*this) = std::make_unique<ScrollOffsetAnimations>(this);
        }
    }
...
void AnimationHost::PushPropertiesTo(MutatorHost* mutator_host_impl, const PropertyTrees& property_trees)
{
    auto* host_impl = static_cast<AnimationHost*>(mutator_host_impl);

    base::AutoReset<raw_ptr<const PropertyTrees>> properties(&property_trees_, &property_trees);
    ...
    host_impl->main_thread_animations_count_.Write(*host_impl) = main_thread_animations_count_.Read(*this);
...
void AnimationHost::PushTimelinesToImplThread(AnimationHost* host_impl) const
{
    for (auto& kv : id_to_timeline_map_.Read(*this)) {
        ...
        const AnimationTimeline* timeline_impl = host_impl->GetTimelineById(timeline->id());
        if (timeline_impl)
            continue;
...
void AnimationHost::RemoveTimelinesFromImplThread(AnimationHost* host_impl) const
{
    IdToTimelineMap& timelines_impl = host_impl->id_to_timeline_map_.Write(*host_impl);
...
{
    base::AutoReset<raw_ptr<const PropertyTrees>> properties(&host_impl->property_trees_, property_trees_);
    ...
    for (auto& kv : id_to_timeline_map_.Read(*this)) {
        ...
        if (AnimationTimeline* timeline_impl = host_impl->GetTimelineById(timeline->id())) {
    ...
    for (auto& kv : element_to_animations_map_.Read(*this)) {
        ...
        if (auto element_animations_impl = host_impl->GetElementAnimationsForElementId(kv.first)) {
            element_animations->PushPropertiesTo(std::move(element_animations_impl));
        }
    }

    // Update the impl-only scroll offset animations.
    scroll_offset_animations_.Write(*this)->PushPropertiesTo(host_impl->scroll_offset_animations_impl_.Write(*host_impl).get());
    ...
    host_impl->pending_throughput_tracker_infos_.Write(*host_impl) = TakePendingThroughputTrackerInfos();
}

const ElementAnimations* AnimationHost::GetElementAnimationsForElementId(ElementId element_id) const
{
    if (!element_id)
        return nullptr;
    auto iter = element_to_animations_map_.Read(*this).find(element_id);
    return iter == element_to_animations_map_.Read(*this).end() ? nullptr : iter->second.get();
}

scoped_refptr<ElementAnimations> AnimationHost::GetElementAnimationsForElementId(ElementId element_id)
{
    if (!element_id)
        return nullptr;
    auto iter = element_to_animations_map_.Write(*this).find(element_id);
    return iter == element_to_animations_map_.Write(*this).end() ? nullptr : iter->second;
}

scoped_refptr<const ElementAnimations> AnimationHost::GetElementAnimationsForElementIdForTesting(ElementId element_id) const
{
    return GetElementAnimationsForElementId(element_id);
}

gfx::PointF AnimationHost::GetScrollOffsetForAnimation(ElementId element_id) const
{
    DCHECK(property_trees_);
    return property_trees_->scroll_tree().current_scroll_offset(element_id);
}

void AnimationHost::SetScrollAnimationDurationForTesting(base::TimeDelta duration)
{
...
void AnimationHost::TickMutator(base::TimeTicks monotonic_time, const ScrollTree& scroll_tree, bool is_active_tree)
{
    ...
    DCHECK(IsOwnerThread());
    DCHECK(!InProtectedSequence());
    std::unique_ptr<MutatorInputState> state = CollectWorkletAnimationsState(monotonic_time, scroll_tree, is_active_tree);
    if (state->IsEmpty())
        return;

    ElementListType tree_type = is_active_tree ? ElementListType::ACTIVE : ElementListType::PENDING;

    auto on_done = base::BindOnce(
        [](base::WeakPtr<AnimationHost> animation_host, ElementListType tree_type, MutateStatus status) {
            if (animation_host->mutator_host_client()) {
                animation_host->mutator_host_client()->NotifyAnimationWorkletStateChange(ToAnimationWorkletMutationState(status), tree_type);
            }
        },
        weak_factory_.GetWeakPtr(), tree_type);

    MutateQueuingStrategy queuing_strategy = is_active_tree ? MutateQueuingStrategy::kQueueAndReplaceNormalPriority : MutateQueuingStrategy::kQueueHighPriority;
    if (mutator->Mutate(std::move(state), queuing_strategy, std::move(on_done))) {
        mutator_host_client()->NotifyAnimationWorkletStateChange(AnimationWorkletMutationState::STARTED, tree_type);
    }
}
...
bool AnimationHost::TickAnimations(base::TimeTicks monotonic_time, const ScrollTree& scroll_tree, bool is_active_tree)
{
    ...
    for (auto& kv : id_to_timeline_map_.Read(*this)) {
        ...
        if (timeline->IsScrollTimeline()) {
            ...
            animated |= timeline->TickTimeLinkedAnimations(ticking_animations_.Read(*this), monotonic_time, !is_active_tree);
        }
    }
    ...
    for (auto* timeline : scroll_timelines) {
        animated |= timeline->TickScrollLinkedAnimations(ticking_animations_.Read(*this), scroll_tree, is_active_tree);
    }
...
void AnimationHost::TickScrollAnimations(base::TimeTicks monotonic_time, const ScrollTree& scroll_tree)
{
...
std::unique_ptr<MutatorInputState> AnimationHost::CollectWorkletAnimationsState(
    base::TimeTicks monotonic_time, const ScrollTree& scroll_tree, bool is_active_tree)
{
    TRACE_EVENT0("cc", "AnimationHost::CollectWorkletAnimationsState");
    std::unique_ptr<MutatorInputState> result = std::make_unique<MutatorInputState>();

    for (auto& animation : ticking_animations_.Read(*this)) {
        if (!animation->IsWorkletAnimation())
            continue;

        ToWorkletAnimation(animation.get())->UpdateInputState(result.get(), monotonic_time, scroll_tree, is_active_tree);
    }
...
void AnimationHost::SetAnimationEvents(std::unique_ptr<MutatorEvents> mutator_events)
{
    DCHECK_EQ(thread_instance_, ThreadInstance::kMain);
    auto events = base::WrapUnique(static_cast<AnimationEvents*>(mutator_events.release()));

    for (size_t event_index = 0; event_index < events->events_.size(); ++event_index) {
...
bool AnimationHost::ScrollOffsetAnimationWasInterrupted(ElementId element_id) const
{
    const auto* element_animations = GetElementAnimationsForElementId(element_id);
    return element_animations ? element_animations->ScrollOffsetAnimationWasInterrupted() : false;
}

bool AnimationHost::IsAnimatingProperty(ElementId element_id, ElementListType list_type, TargetProperty::Type property) const
{
    const auto* element_animations = GetElementAnimationsForElementId(element_id);
    return element_animations ? element_animations->IsCurrentlyAnimatingProperty(property, list_type) : false;
}

bool AnimationHost::HasPotentiallyRunningAnimationForProperty(ElementId element_id, ElementListType list_type, TargetProperty::Type property) const
{
    const auto* element_animations = GetElementAnimationsForElementId(element_id);
    return element_animations ? element_animations->IsPotentiallyAnimatingProperty(property, list_type) : false;
}

bool AnimationHost::HasAnyAnimationTargetingProperty(ElementId element_id, TargetProperty::Type property) const
{
    const auto* element_animations = GetElementAnimationsForElementId(element_id);
    if (!element_animations)
        return false;

    return element_animations->HasAnyAnimationTargetingProperty(property, element_id);
}

bool AnimationHost::AnimationsPreserveAxisAlignment(ElementId element_id) const
{
    const auto* element_animations = GetElementAnimationsForElementId(element_id);
    return element_animations ? element_animations->AnimationsPreserveAxisAlignment() : true;
}

float AnimationHost::MaximumScale(ElementId element_id, ElementListType list_type) const
{
    if (const auto* element_animations = GetElementAnimationsForElementId(element_id)) {
...
bool AnimationHost::HasTickingKeyframeModelForTesting(ElementId element_id) const
{
    const auto* element_animations = GetElementAnimationsForElementId(element_id);
    return element_animations ? element_animations->HasTickingKeyframeEffect() : false;
}

void AnimationHost::ImplOnlyAutoScrollAnimationCreate(ElementId element_id, const gfx::PointF& target_offset, const gfx::PointF& current_offset,
    float autoscroll_velocity, base::TimeDelta animation_start_offset)
{
    DCHECK(scroll_offset_animations_impl_.Read(*this));
    scroll_offset_animations_impl_.Write(*this)->AutoScrollAnimationCreate(
        element_id, target_offset, current_offset, autoscroll_velocity, animation_start_offset);
}

void AnimationHost::ImplOnlyScrollAnimationCreate(ElementId element_id, const gfx::PointF& target_offset, const gfx::PointF& current_offset,
    base::TimeDelta delayed_by, base::TimeDelta animation_start_offset)
{
    DCHECK(scroll_offset_animations_impl_.Read(*this));
    scroll_offset_animations_impl_.Write(*this)->MouseWheelScrollAnimationCreate(element_id, target_offset, current_offset, delayed_by, animation_start_offset);
}

std::optional<gfx::PointF> AnimationHost::ImplOnlyScrollAnimationUpdateTarget(const gfx::Vector2dF& scroll_delta, const gfx::PointF& max_scroll_offset,
    base::TimeTicks frame_monotonic_time, base::TimeDelta delayed_by, ElementId element_id)
{
    DCHECK(scroll_offset_animations_impl_.Read(*this));
    return scroll_offset_animations_impl_.Write(*this)->ScrollAnimationUpdateTarget(
        scroll_delta, max_scroll_offset, frame_monotonic_time, delayed_by, element_id);
}
...
{
    DCHECK(scroll_offset_animations_impl_.Read(*this));
    scroll_offset_animations_impl_.Write(*this)->ScrollAnimationAbort(false /* needs_completion */, element_id);
}

bool AnimationHost::ElementHasImplOnlyScrollAnimation(ElementId element_id) const
{
    return scroll_offset_animations_impl_.Read(*this)->ElementHasImplOnlyScrollAnimation(element_id);
}

bool AnimationHost::HasImplOnlyScrollAnimatingElement() const
{
    return scroll_offset_animations_impl_.Read(*this)->HasImplOnlyScrollAnimatingElement();
}

bool AnimationHost::HasImplOnlyAutoScrollAnimatingElement() const
{
    return scroll_offset_animations_impl_.Read(*this)->HasImplOnlyAutoScrollAnimatingElement();
}

bool AnimationHost::IsElementInPropertyTrees(ElementId element_id, bool commits_to_active) const
{
    return mutator_host_client()->IsElementInPropertyTrees(element_id, commits_to_active ? ElementListType::ACTIVE : ElementListType::PENDING);
}

void AnimationHost::HandleRemovedScrollAnimatingElements(bool commits_to_active)
{
    scroll_offset_animations_impl_.Write(*this)->HandleRemovedScrollAnimatingElements(commits_to_active);
}
...
{
    auto to_erase = base::ranges::find(ticking_animations_.Write(*this), animation);
    if (to_erase != ticking_animations_.Write(*this).end()) {
        ticking_animations_.Write(*this).erase(to_erase);
    }
}

const AnimationHost::AnimationsList& AnimationHost::ticking_animations_for_testing() const
{
    return ticking_animations_.Read(*this);
}

const AnimationHost::ElementToAnimationsMap& AnimationHost::element_animations_for_testing() const
{
    return element_to_animations_map_.Read(*this);
}

void AnimationHost::SetLayerTreeMutator(std::unique_ptr<LayerTreeMutator> mutator)
{
...
WorkletAnimation* AnimationHost::FindWorkletAnimation(WorkletAnimationId id)
{
    ...
    auto animation = base::ranges::find_if(
        ticking_animations_.Read(*this), [id](auto& it) { return it->IsWorkletAnimation() && ToWorkletAnimation(it.get())->worklet_animation_id() == id; });
...
void AnimationHost::SetMutationUpdate(std::unique_ptr<MutatorOutputState> output_state)
{
    if (!output_state)
        return;
...
void AnimationHost::SetAnimationCounts(size_t total_animations_count)
{
    ...
    main_thread_animations_count_.Write(*this) = total_animations_count - ticking_animations_count;
...
AnimationHost::PendingThroughputTrackerInfos AnimationHost::TakePendingThroughputTrackerInfos()
{
    PendingThroughputTrackerInfos infos = std::move(pending_throughput_tracker_infos_.Write(*this));
    pending_throughput_tracker_infos_.Write(*this) = {};
    return infos;
}

void AnimationHost::StartThroughputTracking(TrackedAnimationSequenceId sequence_id)
{
    pending_throughput_tracker_infos_.Write(*this).push_back({ sequence_id, true });
    SetNeedsPushProperties();
}

void AnimationHost::StopThroughputTracking(TrackedAnimationSequenceId sequnece_id)
{
    pending_throughput_tracker_infos_.Write(*this).push_back({ sequnece_id, false });
    SetNeedsPushProperties();
}
