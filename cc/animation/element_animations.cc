...
ElementId CalculateTargetElementId(const ElementAnimations* element_animations, const gfx::KeyframeModel* keyframe_model)
{
    if (KeyframeModel::ToCcKeyframeModel(keyframe_model)->element_id()) [[likely]] {
...
bool UsingPaintWorklet(int property_index)
{
    ...
    return property_index == TargetProperty::CSS_CUSTOM_PROPERTY || property_index == TargetProperty::NATIVE_PROPERTY;
}

} // namespace

scoped_refptr<ElementAnimations> ElementAnimations::Create(AnimationHost* host, ElementId element_id)
{
    DCHECK(element_id);
    DCHECK(host);
...
void ElementAnimations::ClearAffectedElementTypes(const PropertyToElementIdMap& element_id_map)
{
    DCHECK(animation_host_);

    gfx::TargetProperties disable_properties = GetPropertiesMaskForAnimationState();
    ...
    if (animation_host_->mutator_host_client()) {
        animation_host_->mutator_host_client()->ElementIsAnimatingChanged(element_id_map, ElementListType::ACTIVE, disabled_state_mask, disabled_state);
        animation_host_->mutator_host_client()->ElementIsAnimatingChanged(element_id_map, ElementListType::PENDING, disabled_state_mask, disabled_state);
    }
...
void ElementAnimations::PushPropertiesTo(scoped_refptr<ElementAnimations> element_animations_impl) const
{
...
float ElementAnimations::MaximumScale(ElementId element_id, ElementListType list_type) const
{
    float maximum_scale = kInvalidScale;
    for (auto& keyframe_effect : keyframe_effects_list_) {
        maximum_scale = std::max(maximum_scale, keyframe_effect.MaximumScale(element_id, list_type));
    }
...
void ElementAnimations::OnFloatAnimated(const float& value, int target_property_id, gfx::KeyframeModel* keyframe_model)
{
    ...
    case TargetProperty::NATIVE_PROPERTY:
        ...
        if (KeyframeModelAffectsPendingElements(keyframe_model))
            OnCustomPropertyAnimated(PaintWorkletInput::PropertyValue(value), KeyframeModel::ToCcKeyframeModel(keyframe_model), target_property_id);
        break;
...
void ElementAnimations::OnFilterAnimated(const FilterOperations& filters, int target_property_id, gfx::KeyframeModel* keyframe_model)
{
    switch (keyframe_model->TargetProperty()) {
    case TargetProperty::BACKDROP_FILTER:
        if (KeyframeModelAffectsActiveElements(keyframe_model))
            OnBackdropFilterAnimated(ElementListType::ACTIVE, filters, keyframe_model);
        if (KeyframeModelAffectsPendingElements(keyframe_model))
            OnBackdropFilterAnimated(ElementListType::PENDING, filters, keyframe_model);
        break;
...
void ElementAnimations::OnColorAnimated(const SkColor& value, int target_property_id, gfx::KeyframeModel* keyframe_model)
{
    DCHECK_EQ(keyframe_model->TargetProperty(), TargetProperty::CSS_CUSTOM_PROPERTY);
    // TODO(crbug.com/40219248): Remove FromColor and make all SkColor4f.
    OnCustomPropertyAnimated(
        PaintWorkletInput::PropertyValue(SkColor4f::FromColor(value)), KeyframeModel::ToCcKeyframeModel(keyframe_model), target_property_id);
}

void ElementAnimations::OnTransformAnimated(const gfx::TransformOperations& operations, int target_property_id, gfx::KeyframeModel* keyframe_model)
{
...
void ElementAnimations::OnScrollOffsetAnimated(const gfx::PointF& scroll_offset, int target_property_id, gfx::KeyframeModel* keyframe_model)
{
    if (KeyframeModelAffectsActiveElements(keyframe_model))
...
void ElementAnimations::UpdateMaximumScale(ElementId element_id, ElementListType list_type, float* cached_scale)
{
    if (element_id) {
        float maximum_scale = MaximumScale(element_id, list_type);
        if (*cached_scale != maximum_scale) {
            animation_host_->mutator_host_client()->MaximumScaleChanged(element_id, list_type, maximum_scale);
...
void ElementAnimations::UpdateClientAnimationState()
{
    ...
    for (auto& keyframe_effect : keyframe_effects_list_) {
        PropertyAnimationState keyframe_effect_pending_state, keyframe_effect_active_state;
        keyframe_effect.GetPropertyAnimationState(&keyframe_effect_pending_state, &keyframe_effect_active_state);
        pending_state_ |= keyframe_effect_pending_state;
        active_state_ |= keyframe_effect_active_state;
    }

    gfx::TargetProperties allowed_properties = GetPropertiesMaskForAnimationState();
    ...
    if (prev_active != active_state_) {
        ...
        animation_host_->mutator_host_client()->ElementIsAnimatingChanged(element_id_map, ElementListType::ACTIVE, diff_active, active_state_);
    }

    UpdateMaximumScale(transform_element_id, ElementListType::ACTIVE, &transform_property_active_maximum_scale_);
    UpdateMaximumScale(scale_element_id, ElementListType::ACTIVE, &scale_property_active_maximum_scale_);
#if DCHECK_IS_ON()
    DCHECK(IsInvalidOrOne(MaximumScale(rotate_element_id, ElementListType::ACTIVE)));
    DCHECK(IsInvalidOrOne(MaximumScale(translate_element_id, ElementListType::ACTIVE)));
#endif

    if (prev_pending != pending_state_) {
        PropertyAnimationState diff_pending = prev_pending ^ pending_state_;
        animation_host_->mutator_host_client()->ElementIsAnimatingChanged(element_id_map, ElementListType::PENDING, diff_pending, pending_state_);
    }

    UpdateMaximumScale(transform_element_id, ElementListType::PENDING, &transform_property_pending_maximum_scale_);
    UpdateMaximumScale(scale_element_id, ElementListType::PENDING, &scale_property_pending_maximum_scale_);
#if DCHECK_IS_ON()
    DCHECK(IsInvalidOrOne(MaximumScale(rotate_element_id, ElementListType::PENDING)));
    DCHECK(IsInvalidOrOne(MaximumScale(translate_element_id, ElementListType::PENDING)));
#endif
}
...
bool ElementAnimations::HasAnyAnimationTargetingProperty(TargetProperty::Type property, ElementId element_id) const
{
    for (auto& keyframe_effect : keyframe_effects_list_) {
...
bool ElementAnimations::IsPotentiallyAnimatingProperty(TargetProperty::Type target_property, ElementListType list_type) const
{
    for (auto& keyframe_effect : keyframe_effects_list_) {
        if (keyframe_effect.IsPotentiallyAnimatingProperty(target_property, list_type))
            return true;
    }
...
bool ElementAnimations::IsCurrentlyAnimatingProperty(TargetProperty::Type target_property, ElementListType list_type) const
{
    for (auto& keyframe_effect : keyframe_effects_list_) {
        if (keyframe_effect.IsCurrentlyAnimatingProperty(target_property, list_type))
            return true;
    }
...
void ElementAnimations::OnFilterAnimated(ElementListType list_type, const FilterOperations& filters, gfx::KeyframeModel* keyframe_model)
{
    ElementId target_element_id = CalculateTargetElementId(this, keyframe_model);
    DCHECK(target_element_id);
    DCHECK(animation_host_);
    DCHECK(animation_host_->mutator_host_client());
    animation_host_->mutator_host_client()->SetElementFilterMutated(target_element_id, list_type, filters);
}

void ElementAnimations::OnBackdropFilterAnimated(ElementListType list_type, const FilterOperations& backdrop_filters, gfx::KeyframeModel* keyframe_model)
{
    ElementId target_element_id = CalculateTargetElementId(this, keyframe_model);
    DCHECK(target_element_id);
    DCHECK(animation_host_);
    DCHECK(animation_host_->mutator_host_client());
    animation_host_->mutator_host_client()->SetElementBackdropFilterMutated(target_element_id, list_type, backdrop_filters);
}

void ElementAnimations::OnOpacityAnimated(ElementListType list_type, float opacity, gfx::KeyframeModel* keyframe_model)
{
    ElementId target_element_id = CalculateTargetElementId(this, keyframe_model);
    DCHECK(target_element_id);
    DCHECK(animation_host_);
    DCHECK(animation_host_->mutator_host_client());
    animation_host_->mutator_host_client()->SetElementOpacityMutated(target_element_id, list_type, opacity);
}

void ElementAnimations::OnCustomPropertyAnimated(PaintWorkletInput::PropertyValue property_value, KeyframeModel* keyframe_model, int target_property_id)
{
    DCHECK(animation_host_);
    DCHECK(animation_host_->mutator_host_client());
    ...
    PaintWorkletInput::PropertyKey property_key = target_property_id == TargetProperty::NATIVE_PROPERTY
        ? PaintWorkletInput::PropertyKey(keyframe_model->native_property_type(), id)
        : PaintWorkletInput::PropertyKey(keyframe_model->custom_property_name(), id);
    animation_host_->mutator_host_client()->OnCustomPropertyMutated(std::move(property_key), std::move(property_value));
}

void ElementAnimations::OnTransformAnimated(ElementListType list_type, const gfx::Transform& transform, gfx::KeyframeModel* keyframe_model)
{
    ElementId target_element_id = CalculateTargetElementId(this, keyframe_model);
    DCHECK(target_element_id);
    DCHECK(animation_host_);
    DCHECK(animation_host_->mutator_host_client());
    animation_host_->mutator_host_client()->SetElementTransformMutated(target_element_id, list_type, transform);
}

void ElementAnimations::OnScrollOffsetAnimated(ElementListType list_type, const gfx::PointF& scroll_offset, gfx::KeyframeModel* keyframe_model)
{
    ElementId target_element_id = CalculateTargetElementId(this, keyframe_model);
    DCHECK(target_element_id);
    DCHECK(animation_host_);
    DCHECK(animation_host_->mutator_host_client());
    animation_host_->mutator_host_client()->SetElementScrollOffsetMutated(target_element_id, list_type, scroll_offset);
}
...
PropertyToElementIdMap ElementAnimations::GetPropertyToElementIdMap() const
{
    ...
    for (int property_index = TargetProperty::FIRST_TARGET_PROPERTY; property_index <= TargetProperty::LAST_TARGET_PROPERTY; ++property_index) {
        ...
        if (UsingPaintWorklet(property_index))
            continue;
        TargetProperty::Type property = static_cast<TargetProperty::Type>(property_index);
        ElementId element_id_for_property;
        for (auto& keyframe_effect : keyframe_effects_list_) {
            KeyframeModel* model = KeyframeModel::ToCcKeyframeModel(keyframe_effect.GetKeyframeModel(property));
            if (model) {
                ...
                // ElementId.
                if (model->element_id()) {
                    DCHECK(!element_id_for_property || element_id_for_property == model->element_id())
                    ...
                    DCHECK(!element_id_for_property || element_id_for_property == element_id()) << "Either all models should have an ElementId or none should";
...
unsigned int ElementAnimations::CountKeyframesForTesting() const
{
    ...
    for (auto it = keyframe_effects_list_.begin(); it != keyframe_effects_list_.end(); it++)
        count++;
    return count;
}
...
bool ElementAnimations::HasKeyframeEffectForTesting(const KeyframeEffect* keyframe) const
{
    return keyframe_effects_list_.HasObserver(keyframe);
}

bool ElementAnimations::KeyframeModelAffectsActiveElements(gfx::KeyframeModel* keyframe_model) const
{
    ...
    if (!keyframe_model)
        return true;
    return KeyframeModel::ToCcKeyframeModel(keyframe_model)->affects_active_elements();
}

bool ElementAnimations::KeyframeModelAffectsPendingElements(gfx::KeyframeModel* keyframe_model) const
{
    // When we force a keyframe_model update due to a notification, we do not have
    ...
    if (!keyframe_model)
        return true;
    return KeyframeModel::ToCcKeyframeModel(keyframe_model)->affects_pending_elements();
}
