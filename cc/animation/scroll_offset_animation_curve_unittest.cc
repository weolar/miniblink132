...
TEST(ScrollOffsetAnimationCurveTest, GetValue)
{
    ...
    EXPECT_POINTF_NEAR(gfx::PointF(6.f, 30.f), curve->GetValue(duration * 0.5f), 0.00025);
    EXPECT_POINTF_EQ(target_value, curve->GetValue(duration));
    EXPECT_POINTF_EQ(target_value, curve->GetValue(duration + base::Seconds(1.0)));
...
TEST(ScrollOffsetAnimationCurveTest, Clone)
{
    ...
    ScrollOffsetAnimationCurve* cloned_curve = ScrollOffsetAnimationCurve::ToScrollOffsetAnimationCurve(clone.get());
    ...
    EXPECT_POINTF_NEAR(gfx::PointF(6.f, 30.f), cloned_curve->GetValue(duration * 0.5f), 0.00025);
    EXPECT_POINTF_EQ(target_value, cloned_curve->GetValue(duration));
    EXPECT_POINTF_EQ(target_value, cloned_curve->GetValue(duration + base::Seconds(1.f)));
...
TEST(ScrollOffsetAnimationCurveTest, EaseInOutUpdateTarget)
{
    ...
    std::unique_ptr<ScrollOffsetAnimationCurve> curve(
        ScrollOffsetAnimationCurveFactory::CreateEaseInOutAnimationForTesting(target_value, DurationBehavior::kConstant));
    curve->SetInitialValue(initial_value);
    EXPECT_NEAR(duration, curve->Duration().InSecondsF(), 0.0002f);
    EXPECT_NEAR(1800.0, curve->GetValue(base::Seconds(duration / 2.0)).y(), 0.0002f);
    ...
    EXPECT_NEAR(9900.0, curve->GetValue(base::Seconds(duration * 1.5)).y(), 0.0002f);
    ...
    EXPECT_NEAR(6827.6, curve->GetValue(base::Seconds(duration)).y(), 0.1f);
    EXPECT_NEAR(7200.0, curve->GetValue(base::Seconds(duration * 1.08)).y(), 0.0002f);
}
...
TEST(ScrollOffsetAnimationCurveTest, ImpulseUpdateTarget)
{
    ...
    std::unique_ptr<ScrollOffsetAnimationCurve> curve(ScrollOffsetAnimationCurveFactory::CreateImpulseAnimationForTesting(initial_target_value));
    curve->SetInitialValue(initial_value);

    base::TimeDelta initial_duration = ScrollOffsetAnimationCurve::ImpulseSegmentDuration(initial_delta, base::TimeDelta());
    EXPECT_NEAR(initial_duration.InSecondsF(), curve->Duration().InSecondsF(), 0.0002f);
    EXPECT_NEAR(initial_delta.y() * halfway_through_default_impulse_curve, curve->GetValue(initial_duration / 2).y(), 0.01f);
    EXPECT_NEAR(initial_delta.y(), curve->GetValue(initial_duration).y(), 0.0002f);

    base::TimeDelta time_of_update = initial_duration / 2;
    gfx::PointF distance_halfway_through_initial_animation = curve->GetValue(time_of_update);

    gfx::PointF new_target_value(0.f, 9900.f);
    curve->UpdateTarget(time_of_update, new_target_value);

    gfx::Vector2dF new_delta = new_target_value - distance_halfway_through_initial_animation;
    base::TimeDelta updated_segment_duration = ScrollOffsetAnimationCurve::ImpulseSegmentDuration(new_delta, base::TimeDelta());

    base::TimeDelta overall_duration = time_of_update + updated_segment_duration;
    EXPECT_NEAR(overall_duration.InSecondsF(), curve->Duration().InSecondsF(), 0.0002f);
    EXPECT_NEAR(distance_halfway_through_initial_animation.y(), curve->GetValue(time_of_update).y(), 0.01f);
    EXPECT_NEAR(new_target_value.y(), curve->GetValue(overall_duration).y(), 0.0002f);
    ...
    EXPECT_LT(new_delta.y() * halfway_through_default_impulse_curve + 0.01f, curve->GetValue(time_of_update + (updated_segment_duration / 2)).y());
}
...
TEST(ScrollOffsetAnimationCurveTest, ImpulseUpdateTargetSwitchDirections)
{
    ...
    double initial_duration = ScrollOffsetAnimationCurve::ImpulseSegmentDuration(initial_target_value.OffsetFromOrigin(), base::TimeDelta()).InSecondsF();

    std::unique_ptr<ScrollOffsetAnimationCurve> curve(ScrollOffsetAnimationCurveFactory::CreateImpulseAnimationForTesting(initial_target_value));
    curve->SetInitialValue(initial_value);
    EXPECT_NEAR(initial_duration, curve->Duration().InSecondsF(), 0.0002f);
    EXPECT_NEAR(initial_target_value.y() * halfway_through_default_impulse_curve, curve->GetValue(base::Seconds(initial_duration / 2.0)).y(), 0.01f);

    // Animate back to 0. This should force the new curve's initial velocity to be
    // 0, so the default curve will be generated.
    gfx::PointF updated_initial_value(0, initial_target_value.y() * halfway_through_default_impulse_curve);
    ...
    double updated_duration
        = ScrollOffsetAnimationCurve::ImpulseSegmentDuration(gfx::Vector2dF(updated_initial_value.x(), updated_initial_value.y()), base::TimeDelta())
              .InSecondsF();
    EXPECT_NEAR(updated_initial_value.y() * (1.0 - halfway_through_default_impulse_curve),
        curve->GetValue(base::Seconds(initial_duration / 2.0 + updated_duration / 2.0)).y(), 0.01f);
    EXPECT_NEAR(0.0, curve->GetValue(base::Seconds(initial_duration / 2.0 + updated_duration)).y(), 0.0002f);
}

TEST(ScrollOffsetAnimationCurveTest, InverseDeltaDuration)
{
    std::unique_ptr<ScrollOffsetAnimationCurve> curve(
        ScrollOffsetAnimationCurveFactory::CreateEaseInOutAnimationForTesting(gfx::PointF(0.f, 100.f), DurationBehavior::kInverseDelta));
...
TEST(ScrollOffsetAnimationCurveTest, LinearAnimation)
{
    ...
    std::unique_ptr<ScrollOffsetAnimationCurve> curve(ScrollOffsetAnimationCurveFactory::CreateLinearAnimationForTesting(target_offset));
...
TEST(ScrollOffsetAnimationCurveTest, ImpulseDuration)
{
    ...
    base::TimeDelta duration = ScrollOffsetAnimationCurve::ImpulseSegmentDuration(small_delta, base::TimeDelta());
    EXPECT_FLOAT_EQ(duration.InMillisecondsF(), 200.f);

    duration = ScrollOffsetAnimationCurve::ImpulseSegmentDuration(moderate_delta, base::TimeDelta());
    EXPECT_NEAR(duration.InMillisecondsF(), moderate_delta.y() * 1.5f, 0.0002f);

    duration = ScrollOffsetAnimationCurve::ImpulseSegmentDuration(large_delta, base::TimeDelta());
...
TEST(ScrollOffsetAnimationCurveTest, CurveWithLargeDelay)
{
    ...
    std::unique_ptr<ScrollOffsetAnimationCurve> curve(
        ScrollOffsetAnimationCurveFactory::CreateEaseInOutAnimationForTesting(gfx::PointF(0.f, 100.f), duration_hint));
    ...
    double duration = ScrollOffsetAnimationCurve::EaseInOutSegmentDuration(gfx::Vector2dF(0.f, 200.f), duration_hint, base::Seconds(0.01)).InSecondsF();
    ...
    duration = ScrollOffsetAnimationCurve::EaseInOutSegmentDuration(gfx::Vector2dF(0.f, 500.f), duration_hint, base::Seconds(0.01)).InSecondsF();
    EXPECT_EQ(duration, curve->Duration().InSecondsF());

    EXPECT_POINTF_EQ(gfx::PointF(0.f, 500.f), curve->GetValue(base::Seconds(1.0)));
}
...
TEST(ScrollOffsetAnimationCurveTest, UpdateTargetZeroLastSegmentDuration)
{
    ...
    gfx::Vector2dF new_delta = gfx::PointF(0.f, 200.f) - curve->GetValue(base::Seconds(0.05));
    double expected_duration = ScrollOffsetAnimationCurve::EaseInOutSegmentDuration(new_delta, duration_hint, base::TimeDelta()).InSecondsF() + 0.05;
    ...
    expected_duration = ScrollOffsetAnimationCurve::EaseInOutSegmentDuration(new_delta, duration_hint, base::Seconds(0.15)).InSecondsF();
