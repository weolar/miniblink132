...
public:
    ...
    void AttachWorkletAnimation()
    {
        ...
        worklet_animation_ = WrapRefCounted(new WorkletAnimation(1, worklet_animation_id_, "test_name", 1, nullptr, nullptr, true /* controlling instance*/));
...
public:
    MockScrollTimeline()
        : ScrollTimeline(ElementId(), ScrollTimeline::ScrollDown,
            /* scroll_offsets */ std::nullopt, AnimationIdProvider::NextTimelineId())
    {
    }
    MOCK_CONST_METHOD2(CurrentTime, std::optional<base::TimeTicks>(const ScrollTree&, bool));
...
TEST_F(WorkletAnimationTest, NonImplInstanceDoesNotTickKeyframe)
{
    scoped_refptr<WorkletAnimation> worklet_animation
        = WrapRefCounted(new WorkletAnimation(1, worklet_animation_id_, "test_name", 1, nullptr, nullptr, false /* not impl instance*/));
    std::unique_ptr<MockKeyframeEffect> effect = std::make_unique<MockKeyframeEffect>(worklet_animation.get());
...
TEST_F(WorkletAnimationTest, LocalTimeIsUsedWhenTicking)
{
    ...
    KeyframeModel* keyframe_model = worklet_animation_->GetKeyframeModel(TargetProperty::OPACITY);
    ...
    client_.ExpectOpacityPropertyMutated(element_id_, ElementListType::ACTIVE, expected_opacity);
}
...
TEST_F(WorkletAnimationTest, CurrentTimeCorrectlyUsesScrollTimeline)
{
    ...
    EXPECT_CALL(*scroll_timeline, CurrentTime(_, _)).WillRepeatedly(Return((base::TimeTicks() + base::Milliseconds(1234))));
    scoped_refptr<WorkletAnimation> worklet_animation = WorkletAnimation::Create(worklet_animation_id_, "test_name", 1, nullptr, nullptr);
    host_->AddAnimationTimeline(scroll_timeline);
    scroll_timeline->AttachAnimation(worklet_animation);

    ScrollTree scroll_tree;
    std::unique_ptr<MutatorInputState> state = std::make_unique<MutatorInputState>();
    worklet_animation->UpdateInputState(state.get(), base::TimeTicks::Now(), scroll_tree, true);
    std::unique_ptr<AnimationWorkletInput> input = state->TakeWorkletState(worklet_animation_id_.worklet_id);
...
TEST_F(WorkletAnimationTest, CurrentTimeFromRegularTimelineIsOffsetByStartTime)
{
    ...
    base::TimeTicks second_ticks = base::TimeTicks() + base::Milliseconds(111 + 123.4);
    base::TimeTicks third_ticks = base::TimeTicks() + base::Milliseconds(111 + 246.8);

    ScrollTree scroll_tree;
    std::unique_ptr<MutatorInputState> state = std::make_unique<MutatorInputState>();
    worklet_animation->UpdateInputState(state.get(), first_ticks, scroll_tree, true);
    ...
    worklet_animation->UpdateInputState(state.get(), second_ticks, scroll_tree, true);
    ...
    state = std::make_unique<MutatorInputState>();
    worklet_animation->UpdateInputState(state.get(), third_ticks, scroll_tree, true);
    input = state->TakeWorkletState(worklet_animation_id_.worklet_id);
...
TEST_F(WorkletAnimationTest, DocumentTimelineSetPlaybackRate)
{
    ...
    scoped_refptr<WorkletAnimation> worklet_animation = WorkletAnimation::Create(worklet_animation_id_, "test_name", playback_rate_double, nullptr, nullptr);
    ...
    base::TimeTicks first_ticks = base::TimeTicks() + base::Milliseconds(111);
    base::TimeTicks second_ticks = base::TimeTicks() + base::Milliseconds(111 + 123.4);
    base::TimeTicks third_ticks = base::TimeTicks() + base::Milliseconds(111 + 123.4 + 200.0);
    ...
    // time.
    EXPECT_EQ(123.4 * playback_rate_double, input->updated_animations[0].current_time);
    ...
    EXPECT_EQ(123.4 * playback_rate_double + 200.0 * playback_rate_half, input->updated_animations[0].current_time);
}
...
TEST_F(WorkletAnimationTest, ScrollTimelineSetPlaybackRate)
{
    ...
    host_->AddAnimationTimeline(scroll_timeline);
    scroll_timeline->AttachAnimation(worklet_animation);
    const MockScrollTimeline* mock_timeline = static_cast<const MockScrollTimeline*>(worklet_animation->animation_timeline());

    ScrollTree scroll_tree;
    std::unique_ptr<MutatorInputState> state = std::make_unique<MutatorInputState>();
    // Start the animation.
    EXPECT_CALL(*mock_timeline, IsActive(_, _)).WillRepeatedly(Return(true));
    EXPECT_CALL(*mock_timeline, CurrentTime(_, _)).WillRepeatedly(Return(base::TimeTicks() + base::Milliseconds(50)));
    worklet_animation->UpdateInputState(state.get(), base::TimeTicks(), scroll_tree, true);
    Mock::VerifyAndClearExpectations(&mock_timeline);
    std::unique_ptr<AnimationWorkletInput> input = state->TakeWorkletState(worklet_animation_id_.worklet_id);

    // Verify that the current time is updated twice faster than the timeline
    // time.
    EXPECT_EQ(50 * playback_rate_double, input->added_and_updated_animations[0].current_time);
    ...
    EXPECT_CALL(*mock_timeline, IsActive(_, _)).WillRepeatedly(Return(true));
    EXPECT_CALL(*mock_timeline, CurrentTime(_, _)).WillRepeatedly(Return(base::TimeTicks() + base::Milliseconds(100)));
    worklet_animation->UpdateInputState(state.get(), base::TimeTicks(), scroll_tree, true);
    Mock::VerifyAndClearExpectations(&mock_timeline);
    input = state->TakeWorkletState(worklet_animation_id_.worklet_id);

    // Verify that the current time is updated half as fast as the timeline time.
    EXPECT_EQ(50 * playback_rate_double + 50 * playback_rate_half, input->updated_animations[0].current_time);
}
...
TEST_F(WorkletAnimationTest, InactiveScrollTimeline)
{
    ...
    scoped_refptr<WorkletAnimation> worklet_animation = WorkletAnimation::Create(worklet_animation_id_, "test_name",
    ...
    EXPECT_CALL(*mock_timeline, IsActive(_, _)).WillRepeatedly(Return(false));
    EXPECT_CALL(*mock_timeline, CurrentTime(_, _)).WillRepeatedly(Return(base::TimeTicks() + base::Milliseconds(200)));
    worklet_animation->UpdateInputState(state.get(), base::TimeTicks(), scroll_tree, true);
    Mock::VerifyAndClearExpectations(&mock_timeline);
    input = state->TakeWorkletState(worklet_animation_id_.worklet_id);
...
TEST_F(WorkletAnimationTest, UpdateInputStateProducesCorrectState)
{
    ...
    int keyframe_model_id = AddOpacityTransitionToAnimation(worklet_animation_.get(), duration, start_opacity, end_opacity, true);
...
std::optional<base::TimeTicks> FakeIncreasingScrollTimelineTime(Unused, Unused)
{
...
TEST_F(WorkletAnimationTest, SkipLockedAnimations)
{
    auto scroll_timeline = base::WrapRefCounted(new MockScrollTimeline());
    EXPECT_CALL(*scroll_timeline, IsActive(_, _)).WillRepeatedly(Return(true));
    EXPECT_CALL(*scroll_timeline, CurrentTime(_, _)).WillRepeatedly(Invoke(FakeIncreasingScrollTimelineTime));
    scoped_refptr<WorkletAnimation> worklet_animation = WorkletAnimation::Create(worklet_animation_id_, "test_name", 1, nullptr, nullptr);
    host_->AddAnimationTimeline(scroll_timeline);
    scroll_timeline->AttachAnimation(worklet_animation);
