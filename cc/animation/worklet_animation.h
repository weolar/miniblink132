...
public:
    ...
    WorkletAnimation(int cc_animation_id, WorkletAnimationId worklet_animation_id, const std::string& name, double playback_rate_value,
    ...
    static scoped_refptr<WorkletAnimation> Create(WorkletAnimationId worklet_animation_id, const std::string& name, double playback_rate_value,
    ...
    void UpdateState(bool start_ready_animations, AnimationEvents* events) override;

    void TakeTimeUpdatedEvent(AnimationEvents* events) override;
    void UpdateInputState(MutatorInputState* input_state, base::TimeTicks monotonic_time, const ScrollTree& scroll_tree, bool is_active_tree);
...
private:
    ...
    {
        return playback_rate_.Read(*this);
    }
    ...
    std::optional<base::TimeDelta> CurrentTime(base::TimeTicks monotonic_time, const ScrollTree& scroll_tree, bool is_active_tree);
    ...
    bool NeedsUpdate(base::TimeTicks monotonic_time, const ScrollTree& scroll_tree, bool is_active_tree);
    ...
    bool IsTimelineActive(const ScrollTree& scroll_tree, bool is_active_tree) const;
    ...
    ProtectedSequenceReadable<std::optional<base::TimeDelta>> last_synced_local_time_;
