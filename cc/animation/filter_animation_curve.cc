...
void FilterAnimationCurve::Tick(
    base::TimeDelta t, int property_id, gfx::KeyframeModel* keyframe_model, gfx::TimingFunction::LimitDirection limit_direction) const
{
    if (target_) {
        target_->OnFilterAnimated(GetTransformedValue(t, limit_direction), property_id, keyframe_model);
    }
}
...
const FilterAnimationCurve* FilterAnimationCurve::ToFilterAnimationCurve(const gfx::AnimationCurve* c)
{
    DCHECK_EQ(gfx::AnimationCurve::FILTER, c->Type());
    return static_cast<const FilterAnimationCurve*>(c);
}

FilterAnimationCurve* FilterAnimationCurve::ToFilterAnimationCurve(gfx::AnimationCurve* c)
{
...
std::unique_ptr<FilterKeyframe> FilterKeyframe::Create(
    base::TimeDelta time, const FilterOperations& value, std::unique_ptr<gfx::TimingFunction> timing_function)
{
    return base::WrapUnique(new FilterKeyframe(time, value, std::move(timing_function)));
}

FilterKeyframe::FilterKeyframe(base::TimeDelta time, const FilterOperations& value, std::unique_ptr<gfx::TimingFunction> timing_function)
...
void KeyframedFilterAnimationCurve::AddKeyframe(std::unique_ptr<FilterKeyframe> keyframe)
{
...
std::unique_ptr<gfx::AnimationCurve> KeyframedFilterAnimationCurve::Clone() const
{
...
FilterOperations KeyframedFilterAnimationCurve::GetValue(base::TimeDelta t) const
{
    NOTREACHED();
}

FilterOperations KeyframedFilterAnimationCurve::GetTransformedValue(base::TimeDelta t, gfx::TimingFunction::LimitDirection limit_direction) const
{
...
std::unique_ptr<KeyframedFilterAnimationCurve> KeyframedFilterAnimationCurve::Create()
{
