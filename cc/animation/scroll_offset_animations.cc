...
ScrollOffsetAnimationUpdate ScrollOffsetAnimations::GetUpdateForElementId(ElementId element_id) const
{
    DCHECK(element_id);
    auto iter = element_to_update_map_.find(element_id);
    return iter == element_to_update_map_.end() ? ScrollOffsetAnimationUpdate(element_id) : iter->second;
}

void ScrollOffsetAnimations::AddAdjustmentUpdate(ElementId element_id, gfx::Vector2dF adjustment)
{
    DCHECK(element_id);
...
void ScrollOffsetAnimations::PushPropertiesTo(ScrollOffsetAnimationsImpl* animations)
{
    DCHECK(animations);
    ...
    for (auto& kv : element_to_update_map_) {
        ...
        if (update.takeover_)
            animations->ScrollAnimationAbort(true /*needs_completion*/, update.element_id_);
        else
            animations->ScrollAnimationApplyAdjustment(update.element_id_, update.adjustment_);
    }
