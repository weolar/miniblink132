...
TEST_F(AnimationTest, AttachDetachLayerIfTimelineAttached)
{
    ...
    EXPECT_EQ(animation_impl_->keyframe_effect(), GetImplKeyframeEffectForLayerId(element_id_));
...
TEST_F(AnimationTest, HaveInvalidationAndNativePropertyAnimations)
{
    ...
    AddAnimatedCustomPropertyToAnimation(animation_.get(), duration, start_value, end_value);
    AddOpacityTransitionToAnimation(animation_.get(), duration, start_opacity, end_opacity, false);
    CheckKeyframeEffectTimelineNeedsPushProperties(true);
...
TEST_F(AnimationTest, PropertiesMutate)
{
    ...
    AddAnimatedTransformToAnimation(animation_.get(), duration, transform_x, transform_y);
    AddAnimatedFilterToAnimation(animation_.get(), duration, start_brightness, end_brightness);
    AddAnimatedBackdropFilterToAnimation(animation_.get(), duration, start_invert, end_invert);
    CheckKeyframeEffectTimelineNeedsPushProperties(true);

    host_->PushPropertiesTo(host_impl_, client_.GetPropertyTrees());
    CheckKeyframeEffectTimelineNeedsPushProperties(false);

    EXPECT_FALSE(client_.IsPropertyMutated(element_id_, ElementListType::ACTIVE, TargetProperty::OPACITY));
    EXPECT_FALSE(client_.IsPropertyMutated(element_id_, ElementListType::ACTIVE, TargetProperty::TRANSFORM));
    EXPECT_FALSE(client_.IsPropertyMutated(element_id_, ElementListType::ACTIVE, TargetProperty::FILTER));
    EXPECT_FALSE(client_.IsPropertyMutated(element_id_, ElementListType::ACTIVE, TargetProperty::BACKDROP_FILTER));

    EXPECT_FALSE(client_impl_.IsPropertyMutated(element_id_, ElementListType::ACTIVE, TargetProperty::OPACITY));
    EXPECT_FALSE(client_impl_.IsPropertyMutated(element_id_, ElementListType::ACTIVE, TargetProperty::TRANSFORM));
    EXPECT_FALSE(client_impl_.IsPropertyMutated(element_id_, ElementListType::ACTIVE, TargetProperty::FILTER));
    EXPECT_FALSE(client_impl_.IsPropertyMutated(element_id_, ElementListType::ACTIVE, TargetProperty::BACKDROP_FILTER));
    ...
    client_.ExpectOpacityPropertyMutated(element_id_, ElementListType::ACTIVE, end_opacity);
    client_.ExpectTransformPropertyMutated(element_id_, ElementListType::ACTIVE, transform_x, transform_y);
    client_.ExpectFilterPropertyMutated(element_id_, ElementListType::ACTIVE, end_brightness);
    client_.ExpectBackdropFilterPropertyMutated(element_id_, ElementListType::ACTIVE, end_invert);

    client_impl_.ExpectOpacityPropertyMutated(element_id_, ElementListType::ACTIVE, end_opacity);
    client_impl_.ExpectTransformPropertyMutated(element_id_, ElementListType::ACTIVE, transform_x, transform_y);
    client_impl_.ExpectFilterPropertyMutated(element_id_, ElementListType::ACTIVE, end_brightness);
    client_impl_.ExpectBackdropFilterPropertyMutated(element_id_, ElementListType::ACTIVE, end_invert);

    client_impl_.ExpectOpacityPropertyMutated(element_id_, ElementListType::PENDING, end_opacity);
    client_impl_.ExpectTransformPropertyMutated(element_id_, ElementListType::PENDING, transform_x, transform_y);
    client_impl_.ExpectFilterPropertyMutated(element_id_, ElementListType::PENDING, end_brightness);
    client_impl_.ExpectBackdropFilterPropertyMutated(element_id_, ElementListType::PENDING, end_invert);
}
...
TEST_F(AnimationTest, AttachTwoAnimationsToOneLayer)
{
    ...
    AddOpacityTransitionToAnimation(animation1.get(), duration, start_opacity, end_opacity, false);
    AddAnimatedTransformToAnimation(animation2.get(), duration, transform_x, transform_y);
...
TEST_F(AnimationTest, AddRemoveAnimationToNonAttachedAnimation)
{
    ...
    const int filter_id = AddAnimatedFilterToAnimation(animation_.get(), duration, 0.1f, 0.9f);
    AddOpacityTransitionToAnimation(animation_.get(), duration, start_opacity, end_opacity, false);
    ...
    EXPECT_TRUE(animation_->keyframe_effect()->element_animations());
    EXPECT_FALSE(animation_->keyframe_effect()->element_animations()->HasAnyAnimationTargetingProperty(TargetProperty::FILTER, element_id_));
    EXPECT_TRUE(animation_->keyframe_effect()->element_animations()->HasAnyAnimationTargetingProperty(TargetProperty::OPACITY, element_id_));
    EXPECT_TRUE(animation_->keyframe_effect()->needs_push_properties());
...
TEST_F(AnimationDeathTest, RemoveAddInSameFrame)
{
    ...
    const int keyframe_model_id = AddOpacityTransitionToAnimation(animation_.get(), 1., .7f, .3f, false);
    host_->PushPropertiesTo(host_impl_, client_.GetPropertyTrees());

    animation_->RemoveKeyframeModel(keyframe_model_id);
    AddOpacityTransitionToAnimation(animation_.get(), 1., .7f, .3f, false, keyframe_model_id);
    EXPECT_DCHECK_DEATH(host_->PushPropertiesTo(host_impl_, client_.GetPropertyTrees()));
}
...
TEST_F(AnimationTest, SwitchToLayer)
{
    ...
    EXPECT_EQ(animation_->keyframe_effect(), GetKeyframeEffectForElementId(new_element_id));
    EXPECT_TRUE(animation_->keyframe_effect()->element_animations());
    ...
    EXPECT_EQ(animation_impl_->keyframe_effect(), GetImplKeyframeEffectForLayerId(new_element_id));
    EXPECT_TRUE(animation_impl_->keyframe_effect()->element_animations());
...
TEST_F(AnimationTest, ToString)
{
    animation_->AttachElement(element_id_);
    EXPECT_EQ(
        base::StringPrintf("Animation{id=%d, element_id=%s, keyframe_models=[]}", animation_->id(), element_id_.ToString().c_str()), animation_->ToString());
    ...
    animation_->AddKeyframeModel(
        KeyframeModel::Create(std::make_unique<FakeFloatAnimationCurve>(18), 45, 76, KeyframeModel::TargetPropertyId(TargetProperty::BOUNDS)));
    EXPECT_EQ(base::StringPrintf("Animation{id=%d, element_id=%s, "
                                 "keyframe_models=[KeyframeModel{id=42, "
...
TEST_F(AnimationTest, AnimationReplacementDeletesKeyframeModels)
{
    ...
    int group_id = 1;
    int original_model_id = AddOpacityTransitionToAnimation(animation_.get(), /*duration=*/1., /*start_opacity=*/1.0f,
        /*end_opacity=*/0.3f, /*use_timing_function=*/false, /*id=*/std::nullopt, group_id);
    ...
    KeyframeModel* original_model_impl = KeyframeModel::ToCcKeyframeModel(animation_impl_->keyframe_effect()->keyframe_models().front().get());
    ...
    int replacing_model_id = AddOpacityTransitionToAnimation(replacing_animation.get(), /*duration=*/1., /*start_opacity=*/1.0f,
        /*end_opacity=*/0.3f, /*use_timing_function=*/false, /*id=*/std::nullopt, group_id);
    ...
    // finished.
    host_impl_->ActivateAnimations(nullptr);
    EXPECT_EQ(original_model_impl->run_state(), gfx::KeyframeModel::WAITING_FOR_DELETION);
    ...
    EXPECT_EQ(animation_impl_->keyframe_effect()->keyframe_models().front()->id(), replacing_model_id);
}
