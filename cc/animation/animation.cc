...
void Animation::SetAnimationTimeline(AnimationTimeline* timeline)
{
    ...
    // observers properly.
    if (keyframe_effect()->has_attached_element() && keyframe_effect()->has_bound_element_animations()) {
        UnregisterAnimation();
    }
...
void Animation::SetKeyframeEffectForTesting(std::unique_ptr<KeyframeEffect> effect)
{
...
void Animation::RegisterAnimation()
{
    ...
    animation_host()->RegisterAnimationForElement(keyframe_effect()->element_id(), this);
}
...
void Animation::UnregisterAnimation()
{
    ...
    animation_host()->UnregisterAnimationForElement(keyframe_effect()->element_id(), this);
}
...
void Animation::PushPropertiesTo(Animation* animation_impl)
{
    ...
    if (is_replacement_ && !keyframe_effect()->keyframe_models().empty()) {
        auto* cc_keyframe_model = KeyframeModel::ToCcKeyframeModel(keyframe_effect()->keyframe_models().front().get());
        animation_impl->keyframe_effect()->set_replaced_group(cc_keyframe_model->group());
    }
    ...
    keyframe_effect()->PushPropertiesTo(animation_impl->keyframe_effect(), impl_start_time);
}
...
void Animation::UpdateState(bool start_ready_animations, AnimationEvents* events)
{
...
void Animation::DispatchAndDelegateAnimationEvent(const AnimationEvent& event)
{
    if (event.ShouldDispatchToKeyframeEffectAndModel()) {
        if (!keyframe_effect() || !keyframe_effect()->DispatchAnimationEventToKeyframeModel(event)) {
...
void Animation::DelegateAnimationEvent(const AnimationEvent& event)
{
    if (animation_delegate_) {
        ...
        case AnimationEvent::Type::kStarted:
            animation_delegate_->NotifyAnimationStarted(event.monotonic_time, event.target_property, event.group_id);
            break;

        case AnimationEvent::Type::kFinished:
            animation_delegate_->NotifyAnimationFinished(event.monotonic_time, event.target_property, event.group_id);
            break;

        case AnimationEvent::Type::kAborted:
            animation_delegate_->NotifyAnimationAborted(event.monotonic_time, event.target_property, event.group_id);
            break;
        ...
        case AnimationEvent::Type::kTakeOver:
            ...
            DCHECK(event.curve);
            animation_delegate_->NotifyAnimationTakeover(event.monotonic_time, event.target_property, event.animation_start_time, event.curve->Clone());
            break;
...
KeyframeModel* Animation::GetKeyframeModel(TargetProperty::Type target_property) const
{
    return KeyframeModel::ToCcKeyframeModel(keyframe_effect()->GetKeyframeModel(target_property));
}

std::string Animation::ToString() const
{
    return base::StringPrintf("Animation{id=%d, element_id=%s, keyframe_models=[%s]}", id_, keyframe_effect()->element_id().ToString().c_str(),
...
void Animation::PauseKeyframeModel(int keyframe_model_id, base::TimeDelta time_offset)
{
...
void Animation::AbortKeyframeModelsWithProperty(TargetProperty::Type target_property, bool needs_completion)
{
    keyframe_effect()->AbortKeyframeModelsWithProperty(target_property, needs_completion);
}

void Animation::NotifyKeyframeModelFinishedForTesting(int timeline_id, int keyframe_model_id, TargetProperty::Type target_property, int group_id)
{
    AnimationEvent event(AnimationEvent::Type::kFinished, { timeline_id, id(), keyframe_model_id }, group_id, target_property, base::TimeTicks());
