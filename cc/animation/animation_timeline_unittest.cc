...
{
    std::unique_ptr<AnimationHost> host(AnimationHost::CreateForTesting(ThreadInstance::kMain));
    std::unique_ptr<AnimationHost> host_impl(AnimationHost::CreateForTesting(ThreadInstance::kImpl));

    const int timeline_id = AnimationIdProvider::NextTimelineId();
    const int animation_id = AnimationIdProvider::NextAnimationId();

    scoped_refptr<AnimationTimeline> timeline_impl(AnimationTimeline::Create(timeline_id));
    ...
    scoped_refptr<Animation> animation_impl = timeline_impl->GetAnimationById(animation_id);
