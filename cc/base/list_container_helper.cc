...
public:
    ...
    struct InnerList {
        InnerList(size_t capacity, size_t element_size, size_t alignment)
            : data(static_cast<char*>(base::AlignedAlloc(capacity * element_size, alignment)))
            , capacity(capacity)
            , size(0)
            , step(element_size)
        {
        ...
        void InsertBefore(size_t alignment, char** position, size_t count)
        {
            ...
            std::unique_ptr<char[], base::AlignedFreeDeleter> new_data(static_cast<char*>(base::AlignedAlloc(size * step, alignment)));
            ...
            *position = new_data.get() + position_offset;
            ...
            memcpy(new_data.get() + position_offset + count * step, data.get() + position_offset, old_size * step - position_offset);
        ...
        size_t NumElementsAvailable() const
        {
            return capacity - size;
        }
    ...
    void InsertBefore(ListContainerHelper::Iterator* position, size_t count)
    {
        ...
        if (!position->item_iterator) {
            ...
            storage_[position->vector_index].InsertBefore(alignment_, &position->item_iterator, count);
            size_ += count;
        }
    }
...
//////////////////////////////////////////////////////
ListContainerHelper::PositionInCharAllocator::PositionInCharAllocator(const ListContainerHelper::PositionInCharAllocator& other) = default;

ListContainerHelper::PositionInCharAllocator& ListContainerHelper::PositionInCharAllocator::operator=(const ListContainerHelper::PositionInCharAllocator& other)
    = default;

ListContainerHelper::PositionInCharAllocator::PositionInCharAllocator(ListContainerHelper::CharAllocator* container, size_t vector_ind, char* item_iter)
...
bool ListContainerHelper::PositionInCharAllocator::operator==(const ListContainerHelper::PositionInCharAllocator& other) const
{
...
ListContainerHelper::PositionInCharAllocator ListContainerHelper::PositionInCharAllocator::Increment()
{
...
ListContainerHelper::PositionInCharAllocator ListContainerHelper::PositionInCharAllocator::ReverseIncrement()
{
    const auto& list = ptr_to_container->InnerListById(vector_index);
...
// ListContainerHelper
////////////////////////////////////////////
ListContainerHelper::ListContainerHelper(size_t alignment, size_t max_size_for_derived_class, size_t num_of_elements_to_reserve_for)
...
void ListContainerHelper::EraseAndInvalidateAllPointers(ListContainerHelper::Iterator* position)
{
    data_->Erase(position);
}

void ListContainerHelper::InsertBeforeAndInvalidateAllPointers(ListContainerHelper::Iterator* position, size_t count)
{
...
ListContainerHelper::ConstReverseIterator ListContainerHelper::crbegin() const
{
    ...
    return ConstReverseIterator(data_.get(), id, data_->InnerListById(id).LastElement(), 0);
}

ListContainerHelper::ConstReverseIterator ListContainerHelper::crend() const
{
    return ConstReverseIterator(data_.get(), static_cast<size_t>(-1), nullptr, size());
}
...
ListContainerHelper::ReverseIterator ListContainerHelper::rbegin()
{
    ...
    size_t id = data_->LastInnerListId();
    return ReverseIterator(data_.get(), id, data_->InnerListById(id).LastElement(), 0);
}
...
ListContainerHelper::ConstIterator ListContainerHelper::IteratorAt(size_t index) const
{
    ...
    return ConstIterator(data_.get(), list_index, data_->InnerListById(list_index).ElementAt(index), original_index);
}
...
ListContainerHelper::Iterator ListContainerHelper::IteratorAt(size_t index)
{
    ...
    return Iterator(data_.get(), list_index, data_->InnerListById(list_index).ElementAt(index), original_index);
}

void* ListContainerHelper::Allocate(size_t alignment, size_t size_of_actual_element_in_bytes)
{
...
size_t ListContainerHelper::AvailableSizeWithoutAnotherAllocationForTesting() const
{
...
/////////////////////////////////////////////////
ListContainerHelper::Iterator::Iterator(CharAllocator* container, size_t vector_ind, char* item_iter, size_t index)
...
/////////////////////////////////////////////////
ListContainerHelper::ConstIterator::ConstIterator(const ListContainerHelper::Iterator& other)
    : PositionInCharAllocator(other)
    , index_(other.index())
{
}

ListContainerHelper::ConstIterator::ConstIterator(CharAllocator* container, size_t vector_ind, char* item_iter, size_t index)
    : PositionInCharAllocator(container, vector_ind, item_iter)
    , index_(index)
{
}
...
/////////////////////////////////////////////////
ListContainerHelper::ReverseIterator::ReverseIterator(CharAllocator* container, size_t vector_ind, char* item_iter, size_t index)
    : PositionInCharAllocator(container, vector_ind, item_iter)
    , index_(index)
{
}
...
/////////////////////////////////////////////////
ListContainerHelper::ConstReverseIterator::ConstReverseIterator(const ListContainerHelper::ReverseIterator& other)
    : PositionInCharAllocator(other)
    , index_(other.index())
{
}

ListContainerHelper::ConstReverseIterator::ConstReverseIterator(CharAllocator* container, size_t vector_ind, char* item_iter, size_t index)
    : PositionInCharAllocator(container, vector_ind, item_iter)
    , index_(index)
{
}
