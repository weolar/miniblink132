...
void SimpleEnclosedRegion::Subtract(const gfx::Rect& sub_rect)
{
    ...
    rect_.SetRect(left, horizontal_top, right - left, horizontal_bottom - horizontal_top);

    gfx::Rect vertical_rect(vertical_left, top, vertical_right - vertical_left, bottom - top);
...
void SimpleEnclosedRegion::Union(const gfx::Rect& new_rect)
{
    ...
    gfx::Rect adjusted_new_rect(new_left, new_top, new_right - new_left, new_bottom - new_top);
    int64_t adjust_new_rect_area = static_cast<int64_t>(adjusted_new_rect.width()) * adjusted_new_rect.height();
    gfx::Rect overlap = gfx::IntersectRects(rect_, adjusted_new_rect);
    int64_t overlap_area = static_cast<int64_t>(overlap.width()) * overlap.height();
