...
template <class BaseElementType> class ListContainer {
public:
    ...
    // default size.
    ListContainer(size_t max_alignment, size_t max_size_for_derived_class, size_t num_of_elements_to_reserve_for)
    ...
    {
        return ReverseIterator(helper_.rbegin());
    }
    ReverseIterator rend()
    {
        return ReverseIterator(helper_.rend());
    }
    ConstIterator cbegin() const
    {
        return ConstIterator(helper_.cbegin());
    }
    ConstIterator cend() const
    {
        return ConstIterator(helper_.cend());
    }
    ...
    Iterator begin()
    {
        return Iterator(helper_.begin());
    }
    Iterator end()
    {
        return Iterator(helper_.end());
    }
    ...
    // Allocate().
    template <typename DerivedElementType> DerivedElementType* AllocateAndConstruct()
    {
        return new (helper_.Allocate(alignof(DerivedElementType), sizeof(DerivedElementType))) DerivedElementType;
    }

    // Take in derived element type and copy construct it at location generated by
    // Allocate().
    template <typename DerivedElementType> DerivedElementType* AllocateAndCopyFrom(const DerivedElementType* source)
    {
        return new (helper_.Allocate(alignof(DerivedElementType), sizeof(DerivedElementType))) DerivedElementType(*source);
    }

    // Construct a new element on top of an existing one.
    template <typename DerivedElementType> DerivedElementType* ReplaceExistingElement(Iterator at)
    {
        at->~BaseElementType();
    ...
    Iterator InsertBeforeAndInvalidateAllPointers(Iterator at, size_t count, const std::optional<DerivedElementType> source = std::nullopt)
    {
    ...
    template <typename DerivedElementType> Iterator InsertAfterAndInvalidateAllPointers(Iterator at, size_t count)
    {
        return InsertBeforeAndInvalidateAllPointers<DerivedElementType>(at != end() ? ++at : at, count);
    }
    ...
    template <typename DerivedElementType> void swap(ListContainer<DerivedElementType>& other)
    {
        helper_.data_.swap(other.helper_.data_);
    }
    ...
    {
        return helper_.GetCapacityInBytes();
    }
    ...
    public:
        ...
        Iterator(ListContainerHelper::CharAllocator* container, size_t vector_ind, char* item_iter, size_t index)
            : ListContainerHelper::Iterator(container, vector_ind, item_iter, index)
        {
        }
    ...
    public:
        ...
        ConstIterator(ListContainerHelper::CharAllocator* container, size_t vector_ind, char* item_iter, size_t index)
            : ListContainerHelper::ConstIterator(container, vector_ind, item_iter, index)
        {
        }
    ...
    private:
        explicit ConstIterator(const ListContainerHelper::ConstIterator& base_iterator)
    ...
    public:
        ...
        ReverseIterator(ListContainerHelper::CharAllocator* container, size_t vector_ind, char* item_iter, size_t index)
            : ListContainerHelper::ReverseIterator(container, vector_ind, item_iter, index)
        {
        }
        BaseElementType* operator->() const
        {
    ...
    class ConstReverseIterator : public ListContainerHelper::ConstReverseIterator {
        // This class is only defined to reverse iterate through
        // CharAllocator.
    public:
        constexpr ConstReverseIterator() = default;

        ConstReverseIterator(ListContainerHelper::CharAllocator* container, size_t vector_ind, char* item_iter, size_t index)
            : ListContainerHelper::ConstReverseIterator(container, vector_ind, item_iter, index)
        {
        }
    ...
    private:
        explicit ConstReverseIterator(ListContainerHelper::ConstReverseIterator base_iterator)
