...
TEST(IndexRectTest, NumIndices)
{
    struct NumIndicesCase {
        ...
        int num_indices_x;
        int num_indices_y;
    } num_indices_cases[] = { { -10, 10, -10, 10, 21, 21 }, { 0, 5, 0, 10, 6, 11 }, { 1, 2, 3, 4, 2, 2 }, { 0, 0, 0, 0, 1, 1 }, { 10, 10, 10, 10, 1, 1 } };
...
TEST(IndexRectTest, ClampTo)
{
    ...
    struct ClampToCase {
        ...
        Indices expected;
        bool valid;
    } clamp_to_cases[] = { { { 0, 5, 0, 5 }, { 0, 5, 0, 5 }, { 0, 5, 0, 5 }, true }, { { 0, 10, 0, 10 }, { 0, 5, 0, 5 }, { 0, 5, 0, 5 }, true },
        { { 0, 5, 0, 5 }, { 0, 10, 0, 10 }, { 0, 5, 0, 5 }, true }, { { -10, 5, -10, 5 }, { 0, 10, 0, 10 }, { 0, 5, 0, 5 }, true },
        { { 0, 5, 0, 5 }, { 10, 20, 10, 20 }, { 0, 0, 0, 0 }, false } };

    for (size_t i = 0; i < std::size(clamp_to_cases); ++i) {
        const ClampToCase& value = clamp_to_cases[i];
        IndexRect first(value.first.left, value.first.right, value.first.top, value.first.bottom);
        IndexRect second(value.second.left, value.second.right, value.second.top, value.second.bottom);
        IndexRect expected(value.expected.left, value.expected.right, value.expected.top, value.expected.bottom);
...
TEST(IndexRectTest, Contains)
{
    struct ContainsCase {
        ...
        int index_y;
        bool contained;
    } contains_cases[] = { { -10, 10, -10, 10, -10, -10, true }, { -10, 10, -10, 10, 0, 0, true }, { -10, 10, -10, 10, 10, 10, true },
        { -10, 10, -10, 10, 5, 5, true }, { -10, 10, -10, 10, -5, -5, true }, { -10, 10, -10, 10, -20, -20, false }, { -10, 10, -10, 10, 20, 20, false },
        { -10, 10, -10, 10, 20, 5, false }, { -10, 10, -10, 10, 5, 20, false } };
