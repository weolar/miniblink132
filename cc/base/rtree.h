...
public:
    ...
    template <typename Container> void Build(const Container& items);
    ...
    void Build(size_t item_count, const BoundsFunctor& bounds_getter, const PayloadFunctor& payload_getter);
    ...
    template <typename ResultFunctor> void Search(const gfx::Rect& query, const ResultFunctor& result_handler) const;
    ...
    void Search(const gfx::Rect& query, std::vector<T>* results, std::vector<gfx::Rect>* rects = nullptr) const;
...
private:
    ...
    template <typename U> struct Branch {
    ...
    template <typename U> struct Node {
        ...
        explicit Node(uint16_t level)
            : level(level)
        {
        }
    };

    template <typename ResultFunctor> void SearchRecursive(Node<T>* root, const gfx::Rect& query, const ResultFunctor& result_handler) const;
    ...
    template <typename ResultFunctor> void SearchRecursiveFallback(Node<T>* root, const gfx::Rect& query, const ResultFunctor& result_handler) const;

    // Consumes the input array.
    Branch<T> BuildRecursive(std::vector<Branch<T>>* branches, int level);
    Node<T>* AllocateNodeAtLevel(int level);

    void GetAllBoundsRecursive(Node<T>* root, std::map<T, gfx::Rect>* results) const;
...
template <typename T> RTree<T>::RTree() = default;

template <typename T> RTree<T>::~RTree() = default;

template <typename T> template <typename Container> void RTree<T>::Build(const Container& items)
{
    Build(
        items.size(), [&items](size_t index) { return items[index]; }, [](size_t index) { return index; });
}

template <typename T>
template <typename BoundsFunctor, typename PayloadFunctor>
void RTree<T>::Build(size_t item_count, const BoundsFunctor& bounds_getter, const PayloadFunctor& payload_getter)
{
...
template <typename T> auto RTree<T>::AllocateNodeAtLevel(int level) -> Node<T>*
{
...
template <typename T> auto RTree<T>::BuildRecursive(std::vector<Branch<T>>* branches, int level) -> Branch<T>
{
...
template <typename T> template <typename ResultFunctor> void RTree<T>::Search(const gfx::Rect& query, const ResultFunctor& result_handler) const
{
...
template <typename T> void RTree<T>::Search(const gfx::Rect& query, std::vector<T>* results, std::vector<gfx::Rect>* rects) const
{
    results->clear();
    if (rects) {
        rects->clear();
...
template <typename T> void RTree<T>::SearchRefs(const gfx::Rect& query, std::vector<const T*>* results) const
{
    results->clear();
    Search(query, [results](const T& payload, const gfx::Rect&) { results->push_back(&payload); });
}

template <typename T>
template <typename ResultFunctor>
void RTree<T>::SearchRecursive(Node<T>* node, const gfx::Rect& query, const ResultFunctor& result_handler) const
{
...
template <typename T>
template <typename ResultFunctor>
void RTree<T>::SearchRecursiveFallback(Node<T>* node, const gfx::Rect& query, const ResultFunctor& result_handler) const
{
    for (uint16_t i = 0; i < node->num_children; ++i) {
        if (node->level == 0) {
            if (query.Intersects(node->children[i].bounds)) {
...
template <typename T> std::optional<gfx::Rect> RTree<T>::bounds() const
{
    if (has_valid_bounds_) {
        return root_.bounds;
    }
...
template <typename T> std::map<T, gfx::Rect> RTree<T>::GetAllBoundsForTracing() const
{
...
template <typename T> void RTree<T>::GetAllBoundsRecursive(Node<T>* node, std::map<T, gfx::Rect>* results) const
{
    for (uint16_t i = 0; i < node->num_children; ++i) {
        if (node->level == 0)
