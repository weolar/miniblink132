...
TilingData::TilingData(const gfx::Size& max_texture_size, const gfx::Rect& tiling_rect, int border_texels)
...
int TilingData::TileXIndexFromSrcCoord(int src_position) const
{
    ...
    int x = (src_position - tiling_rect_.x() - border_texels_) / (max_texture_size_.width() - 2 * border_texels_);
...
int TilingData::TileYIndexFromSrcCoord(int src_position) const
{
    ...
    int y = (src_position - tiling_rect_.y() - border_texels_) / (max_texture_size_.height() - 2 * border_texels_);
...
int TilingData::FirstBorderTileXIndexFromSrcCoord(int src_position) const
{
    ...
    int x = (src_position - tiling_rect_.x() - 2 * border_texels_) / inner_tile_size;
    return std::clamp(x, 0, num_tiles_x_ - 1);
}
...
int TilingData::FirstBorderTileYIndexFromSrcCoord(int src_position) const
{
    ...
    int y = (src_position - tiling_rect_.y() - 2 * border_texels_) / inner_tile_size;
    return std::clamp(y, 0, num_tiles_y_ - 1);
}
...
{
    num_tiles_x_ = ComputeNumTiles(max_texture_size_.width(), tiling_rect_.width(), border_texels_);
    num_tiles_y_ = ComputeNumTiles(max_texture_size_.height(), tiling_rect_.height(), border_texels_);
}
...
TilingData::Iterator::Iterator(const TilingData* tiling_data, const gfx::Rect& consider_rect, bool include_borders)
    : index_rect_(kNonPositiveQuadrantIndexRect)
{
    ...
    if (include_borders) {
        ...
        index_rect_ = IndexRect(index_x_, tiling_data->LastBorderTileXIndexFromSrcCoord(rect.right() - 1), index_y_,
        ...
        index_rect_
            = IndexRect(index_x_, tiling_data->TileXIndexFromSrcCoord(rect.right() - 1), index_y_, tiling_data->TileYIndexFromSrcCoord(rect.bottom() - 1));
        DCHECK(index_rect_.is_valid());
...
TilingData::BaseDifferenceIterator::BaseDifferenceIterator(const TilingData* tiling_data, const gfx::Rect& consider_rect, const gfx::Rect& ignore_rect)
    : consider_index_rect_(kNonPositiveQuadrantIndexRect)
    , ignore_index_rect_(kNonPositiveQuadrantIndexRect)
{
    if (tiling_data->num_tiles_x() <= 0 || tiling_data->num_tiles_y() <= 0) {
    ...
    consider_index_rect_ = IndexRect(tiling_data->TileXIndexFromSrcCoord(consider.x()), tiling_data->TileXIndexFromSrcCoord(consider.right() - 1),
        tiling_data->TileYIndexFromSrcCoord(consider.y()), tiling_data->TileYIndexFromSrcCoord(consider.bottom() - 1));
    ...
    if (!ignore.IsEmpty()) {
        ignore_index_rect_ = IndexRect(tiling_data->TileXIndexFromSrcCoord(ignore.x()), tiling_data->TileXIndexFromSrcCoord(ignore.right() - 1),
            tiling_data->TileYIndexFromSrcCoord(ignore.y()), tiling_data->TileYIndexFromSrcCoord(ignore.bottom() - 1));
...
TilingData::DifferenceIterator::DifferenceIterator(const TilingData* tiling_data, const gfx::Rect& consider_rect, const gfx::Rect& ignore_rect)
...
TilingData::SpiralDifferenceIterator::SpiralDifferenceIterator(
    const TilingData* tiling_data, const gfx::Rect& consider_rect, const gfx::Rect& ignore_rect, const gfx::Rect& center_rect)
    : BaseDifferenceIterator(tiling_data, consider_rect, ignore_rect)
{
    if (!HasConsiderRect()) {
        done();
    ...
    spiral_iterator_ = SpiralIterator(around_index_rect, consider_index_rect_, ignore_index_rect_);
...
TilingData::SpiralDifferenceIterator& TilingData::SpiralDifferenceIterator::operator++()
{
    ++spiral_iterator_;
    ...
    : BaseDifferenceIterator(tiling_data, consider_rect, ignore_rect)
{
    ...
    reverse_spiral_iterator_ = ReverseSpiralIterator(around_index_rect, consider_index_rect_, ignore_index_rect_);
...
TilingData::ReverseSpiralDifferenceIterator& TilingData::ReverseSpiralDifferenceIterator::operator++()
{
