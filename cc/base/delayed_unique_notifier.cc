...
DelayedUniqueNotifier::DelayedUniqueNotifier(base::SequencedTaskRunner* task_runner, base::RepeatingClosure closure, const base::TimeDelta& delay)
...
void DelayedUniqueNotifier::Schedule()
{
    ...
    next_notification_time_ = Now() + delay_;
    task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&DelayedUniqueNotifier::NotifyIfTime, weak_ptr_factory_.GetWeakPtr()), delay_);
...
void DelayedUniqueNotifier::NotifyIfTime()
{
    ...
    if (next_notification_time_ > now) {
        task_runner_->PostDelayedTask(
            FROM_HERE, base::BindOnce(&DelayedUniqueNotifier::NotifyIfTime, weak_ptr_factory_.GetWeakPtr()), next_notification_time_ - now);
        return;
    }
