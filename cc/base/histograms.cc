...
void SetClientNameForMetrics(const char* client_name)
{
    ...
    if (old_client_name && strcmp(old_client_name, client_name)) {
        ...
        LOG(WARNING) << "Started multiple compositor clients (" << old_client_name << ", " << client_name << ") in one process. Some metrics will be disabled.";
        return;
    }
...
bool ScopedUMAHistogramAreaTimerBase::GetHistogramValues(Sample* time_microseconds, Sample* pixels_per_ms) const
{
    return GetHistogramValues(timer_.Elapsed(), area_.ValueOrDefault(std::numeric_limits<int>::max()), time_microseconds, pixels_per_ms);
}

// static
bool ScopedUMAHistogramAreaTimerBase::GetHistogramValues(base::TimeDelta elapsed, int area, Sample* time_microseconds, Sample* pixels_per_ms)
{
