...
public:
    ...
    explicit SimpleDerivedElement(int val)
    {
        value = val;
    }
...
private:
    ...
    void CheckDestructExpectation()
    {
        if (!has_expected_destructor_calls_)
            return;
        EXPECT_EQ(expected_destructor_calls_, destructor_calls_) << "element destructor called the wrong number of times";
    }
...
const size_t kCurrentLargestDerivedElementSize = std::max(kLargestDerivedElementSize, sizeof(MockDerivedElementSubclass));
const size_t kCurrentLargestDerivedElementAlign = std::max(kLargestDerivedElementAlign, alignof(MockDerivedElementSubclass));

TEST(ListContainerTest, ConstructorCalledInAllocateAndConstruct)
{
    ListContainer<DerivedElement> list(kCurrentLargestDerivedElementAlign, kCurrentLargestDerivedElementSize, 0);
...
TEST(ListContainerTest, ClearDoesNotMalloc)
{
    ...
    ListContainer<DerivedElement> list(kCurrentLargestDerivedElementAlign, kCurrentLargestDerivedElementSize, reserve);
...
{
    ListContainer<NonDerivedElement> list(alignof(NonDerivedElement), sizeof(NonDerivedElement), 0);
...
{
    ListContainer<NonDerivedElement> list(alignof(NonDerivedElement), sizeof(NonDerivedElement), 2);
    ...
    for (std::vector<NonDerivedElement*>::const_iterator nde_iter = nde_list.begin(); nde_iter != nde_list.end(); ++nde_iter) {
...
TEST(ListContainerTest, SimpleConstIteratorIterationNonDerivedElement)
{
...

    {
        ...
        for (std::vector<const NonDerivedElement*>::const_iterator nde_iter = nde_list.begin(); nde_iter != nde_list.end(); ++nde_iter) {
            EXPECT_EQ(*nde_iter, *iter);
            ++iter;
        }
    }
...
TEST(ListContainerTest, SimpleReverseInsertionNonDerivedElement)
{
...

    {
        ...
        for (auto nde_iter = nde_list.rbegin(); nde_iter != nde_list.rend(); ++nde_iter) {
            EXPECT_EQ(*nde_iter, *iter);
            ++iter;
        }
    }
...
TEST(ListContainerTest, DeletionAllInAllocation)
{
    ...
    ListContainer<DerivedElement> list(kCurrentLargestDerivedElementAlign, kCurrentLargestDerivedElementSize, kReserve);
    std::vector<SimpleDerivedElement*> sde_list;
...
TEST(ListContainerTest, DeletionAllInAllocationReversed)
{
    ...
    size_t i = kReserve - 1;
    for (auto reverse_it = list.rbegin(); reverse_it != list.rend(); ++reverse_it) {
...
{
    ListContainer<SimpleDerivedElement> list(kCurrentLargestDerivedElementAlign, kCurrentLargestDerivedElementSize, 0);
    for (int i = 0; i < 4; ++i)
...
TEST(ListContainerTest, InsertCopyBeforeBegin)
{
    ...
    const int size = 4;
    for (int i = 0; i < size; ++i) {
        SimpleDerivedElement* element = list.AllocateAndConstruct<SimpleDerivedElement>();
        element->set_value(i);
    }
    EXPECT_EQ(static_cast<size_t>(size), list.size());
    ...
    auto iter = list.InsertBeforeAndInvalidateAllPointers<SimpleDerivedElement>(list.begin(), count, insert_element);

    const int expected_result[] = { 100, 100, 0, 1, 2, 3 };
    int iter_index = 0;
    for (iter = list.begin(); iter != list.end(); ++iter) {
        EXPECT_EQ(expected_result[iter_index], static_cast<SimpleDerivedElement*>(*iter)->get_value());
        ++iter_index;
    }
...
TEST(ListContainerTest, InsertCopyBeforeEnd)
{
    ...
    const int count = 3;
    auto insert_element = SimpleDerivedElement(100);
    auto iter = list.InsertBeforeAndInvalidateAllPointers<SimpleDerivedElement>(list.end(), count, insert_element);
...
TEST(ListContainerTest, InsertCopyBeforeMany)
{
    ...
    while (iter != list.end()) {
        ...
        int value_next = iter_next != list.end() ? static_cast<SimpleDerivedElement*>(*iter_next)->get_value() : 10;
        int count = value_next - value - 1;
        iter = list.InsertBeforeAndInvalidateAllPointers<SimpleDerivedElement>(iter_next, count, SimpleDerivedElement(100));
...
TEST(ListContainerTest, InsertBeforeBegin)
{
    ...
    const int count = 2;
    auto iter = list.InsertBeforeAndInvalidateAllPointers<SimpleDerivedElement>(list.begin(), count);
    for (int i = 0; i < count; ++i) {
...
TEST(ListContainerTest, InsertBeforeEnd)
{
    ...
    const int count = 3;
    auto iter = list.InsertBeforeAndInvalidateAllPointers<SimpleDerivedElement>(list.end(), count);
    for (int i = 0; i < count; ++i) {
        static_cast<SimpleDerivedElement*>(*iter)->set_value(100 + i);
        ++iter;
    }
...
TEST(ListContainerTest, InsertBeforeMany)
{
    ...
    while (iter != list.end()) {
        ...
        int value = static_cast<SimpleDerivedElement*>(*iter)->get_value();
        int value_next = iter_next != list.end() ? static_cast<SimpleDerivedElement*>(*iter_next)->get_value() : 100;
        int count = value_next - value - 1;

        iter = list.InsertBeforeAndInvalidateAllPointers<SimpleDerivedElement>(iter_next, count);
    ...
    int iter_index = 0;
    for (iter = list.begin(); iter != list.end(); ++iter) {
        EXPECT_EQ(iter_index, static_cast<SimpleDerivedElement*>(*iter)->get_value());
        ++iter_index;
    }
...
TEST(ListContainerTest, InsertAfterBegin)
{
    ...
    const int count = 2;
    auto iter = list.InsertAfterAndInvalidateAllPointers<SimpleDerivedElement>(list.begin(), count);
    for (int i = 0; i < count; ++i) {
        static_cast<SimpleDerivedElement*>(*iter)->set_value(100 + i);
        ++iter;
    }
...
TEST(ListContainerTest, InsertAfterEnd)
{
    ...
    const int count = 3;
    auto iter = list.InsertAfterAndInvalidateAllPointers<SimpleDerivedElement>(list.end(), count);
    for (int i = 0; i < count; ++i) {
        static_cast<SimpleDerivedElement*>(*iter)->set_value(100 + i);
        ++iter;
    }
...
TEST(ListContainerTest, InsertAfterMany)
{
    ...
    while (iter != list.end()) {
        ...
        iter = list.InsertAfterAndInvalidateAllPointers<SimpleDerivedElement>(iter, count);
        for (int i = value + 1; i < value_next; ++i) {
            static_cast<SimpleDerivedElement*>(*iter)->set_value(i);
            ++iter;
        }
...
TEST(ListContainerTest, SimpleManipulationWithIndexSimpleDerivedElement)
{
    ...
    int i = 0;
    for (std::vector<SimpleDerivedElement*>::const_iterator de_iter = de_list.begin(); de_iter != de_list.end(); ++de_iter, ++i) {
...
{
    ListContainer<DerivedElement> list(kLargestDerivedElementAlign, kLargestDerivedElementSize, 2);
    std::vector<SimpleDerivedElement*> de_list;
    int size = 10;
    for (int i = 0; i < size; ++i) {
        de_list.push_back(list.AllocateAndConstruct<SimpleDerivedElement>());
...
public:
    ...
    explicit InstanceCounter(int* counter)
    {
        SetCounter(counter);
    }
    ~InstanceCounter()
...
TEST(ListContainerTest, RemoveLastDestruction)
{
    ...
    int counter = 0;
    ListContainer<InstanceCounter> list(alignof(InstanceCounter), sizeof(InstanceCounter), 1);
    EXPECT_EQ(0, counter);
    EXPECT_EQ(0u, list.size());
...
template <typename It1, typename It2> bool Equal(It1 it1, const It1& end1, It2 it2)
{
...
TEST(ListContainerTest, RemoveLastIteration)
{
    ...
    // order matches.
    auto push = [&list, &pointers]() { pointers.push_back(list.AllocateAndConstruct<SmallStruct>()); };
...
{
    ListContainer<SimpleDerivedElement> list_1(kCurrentLargestDerivedElementAlign, kCurrentLargestDerivedElementSize, 0);
    list_1.AllocateAndConstruct<SimpleDerivedElementConstructMagicNumberOne>();
    ListContainer<SimpleDerivedElement> list_2(kCurrentLargestDerivedElementAlign, kCurrentLargestDerivedElementSize, 0);
    ...
    EXPECT_EQ(kMagicNumberToUseForSimpleDerivedElementOne, list_1.front()->get_value());
    EXPECT_EQ(1u, list_1.size());

    EXPECT_EQ(kMagicNumberToUseForSimpleDerivedElementTwo, list_2.front()->get_value());
    EXPECT_EQ(kMagicNumberToUseForSimpleDerivedElementThree, list_2.back()->get_value());
    EXPECT_EQ(2u, list_2.size());

    list_2.swap(list_1);

    EXPECT_EQ(kMagicNumberToUseForSimpleDerivedElementTwo, list_1.front()->get_value());
    EXPECT_EQ(kMagicNumberToUseForSimpleDerivedElementThree, list_1.back()->get_value());
    EXPECT_EQ(2u, list_1.size());

    EXPECT_EQ(kMagicNumberToUseForSimpleDerivedElementOne, list_2.front()->get_value());
...
TEST(ListContainerTest, GetCapacityInBytes)
{
    ...
    ListContainer<DerivedElement> list(kLargestDerivedElementAlign, kLargestDerivedElementSize, initial_capacity);
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        ASSERT_LE(capacity, std::max(list.size(), upper_bound_on_min_capacity) * max_waste_factor * kLargestDerivedElementSize);
