...
bool ExpectRegionEq(const gfx::Rect& rect, const SimpleEnclosedRegion& region)
{
    ...
    if (rect.IsEmpty() != region.IsEmpty()) {
        LOG(ERROR) << "Expected: " << rect.IsEmpty() << " Actual: " << region.IsEmpty();
        return false;
    }

    if (expected_rects.size() != actual_rects.size()) {
        LOG(ERROR) << "Expected: " << expected_rects.size() << " Actual: " << actual_rects.size();
        return false;
    }
    ...
    for (size_t i = 0; i < expected_rects.size(); ++i) {
        if (expected_rects[i] != actual_rects[i]) {
            LOG(ERROR) << "Expected: " << expected_rects[i].ToString() << " Actual: " << actual_rects[i].ToString();
            return false;
        }
    }
