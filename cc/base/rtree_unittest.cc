...
template <typename T> void SearchAndVerifyRefs(const RTree<T>& rtree, const gfx::Rect& query, std::vector<T>* results)
{
...
template <typename T> void SearchAndVerifyBounds(const RTree<T>& rtree, const gfx::Rect& query, std::vector<T>* results, std::vector<gfx::Rect>* rects)
{
...
TEST(RTreeTest, GetBoundsNonOverlapping)
{
    ...
    std::map<size_t, gfx::Rect> expected_all_bounds = { { 0, rects[0] }, { 1, rects[1] } };
...
TEST(RTreeTest, Payload)
{
    ...
    RTree<float> rtree;
    rtree.Build(
        data.size(), [&data](size_t index) { return data[index].first; }, [&data](size_t index) { return data[index].second; });
...
TEST(RTreeTest, InvalidBoundsSearch)
{
    ...
    SearchAndVerifyRefs(rtree, gfx::Rect(-INT_MAX, -INT_MAX, INT_MAX, INT_MAX), &found);
...
TEST(RTreeTest, InvalidBoundsGetAllBounds)
{
    ...
    std::map<size_t, gfx::Rect> expected_all_bounds = { { 0, rects[0] }, { 1, rects[1] }, { 2, rects[2] }, { 3, rects[3] }, { 4, rects[4] } };
