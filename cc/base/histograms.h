...
#define DEFINE_SCOPED_UMA_HISTOGRAM_AREA_TIMER(class_name, time_histogram, area_histogram)                                                                     \
    ...
    {                                                                                                                                                          \
        ...
        if (client_name && GetHistogramValues(&time_sample, &area_sample)) {                                                                                   \
            ...
            /* runtime constant. */                                                                                                                            \
            UMA_HISTOGRAM_COUNTS_1M(base::StringPrintf(time_histogram, client_name), time_sample);                                                             \
            UMA_HISTOGRAM_CUSTOM_COUNTS(base::StringPrintf(area_histogram, client_name), area_sample, 1, 100000000, 50);                                       \
        }                                                                                                                                                      \
    }
...
public:
    ScopedUMAHistogramAreaTimerBase(const ScopedUMAHistogramAreaTimerBase&) = delete;
    ScopedUMAHistogramAreaTimerBase& operator=(const ScopedUMAHistogramAreaTimerBase&) = delete;

    void AddArea(const base::CheckedNumeric<int>& area)
    {
        area_ += area;
    }
    void SetArea(const base::CheckedNumeric<int>& area)
    {
        area_ = area;
    }
...
protected:
    ...
    bool GetHistogramValues(Sample* time_microseconds, Sample* pixels_per_ms) const;

private:
    static bool GetHistogramValues(base::TimeDelta elapsed, int area, Sample* time_microseconds, Sample* pixels_per_ms);
