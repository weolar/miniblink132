...
static HomogeneousCoordinate ProjectHomogeneousPoint(const gfx::Transform& transform, const gfx::PointF& p)
{
    ...
    SkScalar z = -(transform.rc(2, 0) * p.x() + transform.rc(2, 1) * p.y() + transform.rc(2, 3)) / m22;
...
static HomogeneousCoordinate ProjectHomogeneousPoint(const gfx::Transform& transform, const gfx::PointF& p, bool* clipped)
{
...
static HomogeneousCoordinate MapHomogeneousPoint(const gfx::Transform& transform, const gfx::PointF& p)
{
...
static void homogeneousLimitAtZero(SkScalar a1, SkScalar w1, SkScalar a2, SkScalar w2, float t, float* limit)
{
...
static gfx::PointF ComputeClippedCartesianPoint2dForEdge(const HomogeneousCoordinate& h1, const HomogeneousCoordinate& h2)
{
...
static void homogeneousLimitNearZero(SkScalar a1, SkScalar w1, SkScalar a2, SkScalar w2, float t, float* limit)
{
    if (std::abs(a1 * w2 / w1 / a2 - 1.0f) > kStationaryPointEpsilon) {
...
static gfx::Point3F ComputeClippedCartesianPoint3dForEdge(const HomogeneousCoordinate& h1, const HomogeneousCoordinate& h2)
{
    // Points h1 and h2 form a line in 4d, and any point on that line can be
    // represented as an interpolation between h1 and h2:
    //    p = (1-t) h1 + (t) h2
    ...
    float t = h1.w() / w_diff;
    float max_numerator
        = std::max({ std::abs((1.0f - t) * h1.x() + t * h2.x()), std::abs((1.0f - t) * h1.y() + t * h2.y()), std::abs((1.0f - t) * h1.z() + t * h2.z()) });
    ...
    // often is).
    const float t_shift = max_numerator / w_diff / HomogeneousCoordinate::kInfiniteCoordinate;
...
static inline bool IsNearlyTheSame(float f, float g)
{
    ...
    return std::abs(f - g) < epsilon_scale * std::max({ std::abs(f), std::abs(g), epsilon_scale });
}

static inline bool IsNearlyTheSame(const gfx::PointF& lhs, const gfx::PointF& rhs)
{
    return IsNearlyTheSame(lhs.x(), rhs.x()) && IsNearlyTheSame(lhs.y(), rhs.y());
}

static inline bool IsNearlyTheSame(const gfx::Point3F& lhs, const gfx::Point3F& rhs)
{
...
static inline void AddVertexToClippedQuad3d(
    const gfx::Point3F& new_vertex, base::span<gfx::Point3F, 6> clipped_quad, int* num_vertices_in_clipped_quad, bool* need_to_clamp)
{
    ...
    if (new_vertex.x() < -HomogeneousCoordinate::kInfiniteCoordinate || new_vertex.x() > HomogeneousCoordinate::kInfiniteCoordinate
        || new_vertex.y() < -HomogeneousCoordinate::kInfiniteCoordinate || new_vertex.y() > HomogeneousCoordinate::kInfiniteCoordinate
        || new_vertex.z() < -HomogeneousCoordinate::kInfiniteCoordinate || new_vertex.z() > HomogeneousCoordinate::kInfiniteCoordinate) {
        *need_to_clamp = true;
    }
}

gfx::Rect MathUtil::MapEnclosingClippedRect(const gfx::Transform& transform, const gfx::Rect& src_rect)
{
    return MapEnclosingClippedRectIgnoringError(transform, src_rect, 0.f);
}

gfx::Rect MathUtil::MapEnclosingClippedRectIgnoringError(const gfx::Transform& transform, const gfx::Rect& src_rect, float ignore_error)
{
...
gfx::RectF MathUtil::MapClippedRect(const gfx::Transform& transform, const gfx::RectF& src_rect)
{
    ...
    HomogeneousCoordinate hc1 = MapHomogeneousPoint(transform, src_rect.top_right());
    HomogeneousCoordinate hc2 = MapHomogeneousPoint(transform, src_rect.bottom_right());
    HomogeneousCoordinate hc3 = MapHomogeneousPoint(transform, src_rect.bottom_left());

    return ComputeEnclosingClippedRect(hc0, hc1, hc2, hc3);
}

gfx::Rect MathUtil::ProjectEnclosingClippedRect(const gfx::Transform& transform, const gfx::Rect& src_rect)
{
    if (transform.IsIdentityOrIntegerTranslation())
        return src_rect + gfx::ToFlooredVector2d(transform.To2dTranslation());
...
gfx::RectF MathUtil::ProjectClippedRect(const gfx::Transform& transform, const gfx::RectF& src_rect)
{
    if (transform.IsIdentityOrTranslation())
        return src_rect + transform.To2dTranslation();
...
gfx::QuadF MathUtil::InverseMapQuadToLocalSpace(const gfx::Transform& device_transform, const gfx::QuadF& device_quad)
{
    DCHECK(device_transform.IsFlat());
    gfx::Transform inverse_device_transform = device_transform.GetCheckedInverse();
    bool clipped = false;
...
gfx::Rect MathUtil::MapEnclosedRectWith2dAxisAlignedTransform(const gfx::Transform& transform, const gfx::Rect& rect)
{
    ...
    HomogeneousCoordinate hc0 = MapHomogeneousPoint(transform, gfx::PointF(rect.origin()));
    HomogeneousCoordinate hc1 = MapHomogeneousPoint(transform, gfx::PointF(rect.bottom_right()));
...
bool MathUtil::MapClippedQuad3d(
    const gfx::Transform& transform, const gfx::QuadF& src_quad, base::span<gfx::Point3F, 6> clipped_quad, int* num_vertices_in_clipped_quad)
{
    ...
    *num_vertices_in_clipped_quad = 0;
    ...
    if (!h1.ShouldBeClipped()) {
        AddVertexToClippedQuad3d(h1.CartesianPoint3dUnclamped(), clipped_quad, num_vertices_in_clipped_quad, &need_to_clamp);
    }

    if (h1.ShouldBeClipped() ^ h2.ShouldBeClipped()) {
        AddVertexToClippedQuad3d(ComputeClippedCartesianPoint3dForEdge(h1, h2), clipped_quad, num_vertices_in_clipped_quad, &need_to_clamp);
    }

    if (!h2.ShouldBeClipped()) {
        AddVertexToClippedQuad3d(h2.CartesianPoint3dUnclamped(), clipped_quad, num_vertices_in_clipped_quad, &need_to_clamp);
    }

    if (h2.ShouldBeClipped() ^ h3.ShouldBeClipped()) {
        AddVertexToClippedQuad3d(ComputeClippedCartesianPoint3dForEdge(h2, h3), clipped_quad, num_vertices_in_clipped_quad, &need_to_clamp);
    }

    if (!h3.ShouldBeClipped()) {
        AddVertexToClippedQuad3d(h3.CartesianPoint3dUnclamped(), clipped_quad, num_vertices_in_clipped_quad, &need_to_clamp);
    }

    if (h3.ShouldBeClipped() ^ h4.ShouldBeClipped()) {
        AddVertexToClippedQuad3d(ComputeClippedCartesianPoint3dForEdge(h3, h4), clipped_quad, num_vertices_in_clipped_quad, &need_to_clamp);
    }

    if (!h4.ShouldBeClipped()) {
        AddVertexToClippedQuad3d(h4.CartesianPoint3dUnclamped(), clipped_quad, num_vertices_in_clipped_quad, &need_to_clamp);
    }

    if (h4.ShouldBeClipped() ^ h1.ShouldBeClipped()) {
        AddVertexToClippedQuad3d(ComputeClippedCartesianPoint3dForEdge(h4, h1), clipped_quad, num_vertices_in_clipped_quad, &need_to_clamp);
    }
    ...
    if (need_to_clamp) {
        ...
        if (*num_vertices_in_clipped_quad > 2) {
            gfx::Vector3dF loop_vector = clipped_quad[0] - clipped_quad[*num_vertices_in_clipped_quad - 1];
        ...
        if (std::isnormal(length)) { // exclude 0, denormals, +/- inf, NaN
            ...
            shortest_from_zero.Scale(DotProduct(normal, clipped_quad[0] - gfx::Point3F(0.0f, 0.0f, 0.0f)));
            ...
            if (shortest_from_zero.x() == 0.0f && shortest_from_zero.y() == 0.0f) {
                ...
                // opposite directions.
                gfx::Vector3dF v = CrossProduct(normal, CrossProduct(gfx::Vector3dF(0.0f, 0.0f, 1.0f), normal));
                ...
                float r = std::abs(v.x()) > std::abs(v.y()) ? shortest_from_zero.x() / v.x() : shortest_from_zero.y() / v.y();
            ...
            if (!clamp_by_points) {
                ...
                // be the visible area.
                constexpr float max_distance = 0.75 * HomogeneousCoordinate::kInfiniteCoordinate;
                ...
                for (int i = 0; i < *num_vertices_in_clipped_quad; ++i) {
                    ...
                    if (std::abs(z) > HomogeneousCoordinate::kInfiniteCoordinate) {
                        ...
                        float z_at_xy_zero_clamped = std::min(
                            float { HomogeneousCoordinate::kInfiniteCoordinate }, std::max(-HomogeneousCoordinate::kInfiniteCoordinate, z_at_xy_zero));
                        float z_offset = z - z_at_xy_zero_clamped;
                        float z_space
                            = (z > 0 ? HomogeneousCoordinate::kInfiniteCoordinate : -HomogeneousCoordinate::kInfiniteCoordinate) - z_at_xy_zero_clamped;
        ...
        if (clamp_by_points) {
            ...
            for (int i = 0; i < *num_vertices_in_clipped_quad; ++i) {
                gfx::Point3F& point = clipped_quad[i];
                point.set_x(std::clamp(point.x(), -HomogeneousCoordinate::kInfiniteCoordinate, float { HomogeneousCoordinate::kInfiniteCoordinate }));
                point.set_y(std::clamp(point.y(), -HomogeneousCoordinate::kInfiniteCoordinate, float { HomogeneousCoordinate::kInfiniteCoordinate }));
                point.set_z(std::clamp(point.z(), -HomogeneousCoordinate::kInfiniteCoordinate, float { HomogeneousCoordinate::kInfiniteCoordinate }));
            }
        }
    }
...
gfx::RectF MathUtil::ComputeEnclosingRectOfVertices(base::span<const gfx::PointF> vertices)
{
    if (vertices.size() < 2) {
        return gfx::RectF();
    }
...
gfx::RectF MathUtil::ComputeEnclosingClippedRect(
    const HomogeneousCoordinate& h1, const HomogeneousCoordinate& h2, const HomogeneousCoordinate& h3, const HomogeneousCoordinate& h4)
{
    ...
    if (!something_clipped) {
        gfx::QuadF mapped_quad = gfx::QuadF(h1.CartesianPoint2d(), h2.CartesianPoint2d(), h3.CartesianPoint2d(), h4.CartesianPoint2d());
    ...
    if (h1.ShouldBeClipped() ^ h2.ShouldBeClipped())
        ExpandBoundsToIncludePoint(&xmin, &xmax, &ymin, &ymax, ComputeClippedCartesianPoint2dForEdge(h1, h2));

    if (!h2.ShouldBeClipped())
        ExpandBoundsToIncludePoint(&xmin, &xmax, &ymin, &ymax, h2.CartesianPoint2d());

    if (h2.ShouldBeClipped() ^ h3.ShouldBeClipped())
        ExpandBoundsToIncludePoint(&xmin, &xmax, &ymin, &ymax, ComputeClippedCartesianPoint2dForEdge(h2, h3));

    if (!h3.ShouldBeClipped())
        ExpandBoundsToIncludePoint(&xmin, &xmax, &ymin, &ymax, h3.CartesianPoint2d());

    if (h3.ShouldBeClipped() ^ h4.ShouldBeClipped())
        ExpandBoundsToIncludePoint(&xmin, &xmax, &ymin, &ymax, ComputeClippedCartesianPoint2dForEdge(h3, h4));

    if (!h4.ShouldBeClipped())
        ExpandBoundsToIncludePoint(&xmin, &xmax, &ymin, &ymax, h4.CartesianPoint2d());

    if (h4.ShouldBeClipped() ^ h1.ShouldBeClipped())
        ExpandBoundsToIncludePoint(&xmin, &xmax, &ymin, &ymax, ComputeClippedCartesianPoint2dForEdge(h4, h1));
...
gfx::RectF MathUtil::ScaleRectProportional(const gfx::RectF& input_outer_rect, const gfx::RectF& scale_outer_rect, const gfx::RectF& scale_inner_rect)
{
    ...
    output_inner_rect.Inset(gfx::InsetsF::TLBR(top_left_diff.y() / scale_rect_to_input_scale_y, top_left_diff.x() / scale_rect_to_input_scale_x,
        -bottom_right_diff.y() / scale_rect_to_input_scale_y, -bottom_right_diff.x() / scale_rect_to_input_scale_x));
    return output_inner_rect;
}

float MathUtil::SmallestAngleBetweenVectors(const gfx::Vector2dF& v1, const gfx::Vector2dF& v2)
{
...
gfx::Vector2dF MathUtil::ProjectVector(const gfx::Vector2dF& source, const gfx::Vector2dF& destination)
{
    float projected_length = gfx::DotProduct(source, destination) / destination.LengthSquared();
    return gfx::Vector2dF(projected_length * destination.x(), projected_length * destination.y());
}
...
void MathUtil::AddToTracedValue(const char* name, const gfx::Size& s, base::trace_event::TracedValue* res)
{
...
void MathUtil::AddToTracedValue(const char* name, const gfx::Point3F& pt, base::trace_event::TracedValue* res)
{
    res->BeginArray(name);
    res->AppendDouble(pt.x());
    res->AppendDouble(pt.y());
...
gfx::Vector3dF MathUtil::GetXAxis(const gfx::Transform& transform)
{
    ...
    return gfx::Vector3dF(transform.rc(0, 0), transform.rc(1, 0), transform.rc(2, 0));
}
...
gfx::Vector3dF MathUtil::GetYAxis(const gfx::Transform& transform)
{
    ...
    return gfx::Vector3dF(transform.rc(0, 1), transform.rc(1, 1), transform.rc(2, 1));
}
...
bool MathUtil::IsNearlyTheSameForTesting(const gfx::PointF& left, const gfx::PointF& right)
{
    return IsNearlyTheSame(left, right);
}

bool MathUtil::IsNearlyTheSameForTesting(const gfx::Point3F& left, const gfx::Point3F& right)
{
    return IsNearlyTheSame(left, right);
}
...
{
    return (m.rc(3, 0) != 0 || m.rc(3, 1) != 0 || m.rc(3, 2) != 0 || m.rc(3, 3) != 1);
}
...
{
    return (m.rc(0, 2) == 0 && m.rc(1, 2) == 0 && m.rc(2, 2) == 1 && m.rc(2, 0) == 0 && m.rc(2, 1) == 0 && m.rc(2, 3) == 0 && m.rc(3, 2) == 0);
}
...
bool MathUtil::SkM44Preserves2DAxisAlignment(const SkM44& m)
{
    ...
    if (m.rc(0, 1) != 0 || m.rc(1, 0) != 0) {
        ...
        return (m.rc(0, 0) == 0 && m.rc(1, 1) == 0 && m.rc(0, 1) != 0 && m.rc(1, 0) != 0);
    }
