...
template <typename T> class SyncedProperty : public base::RefCounted<SyncedProperty<T>> {
public:
    ...
    {
        DeltaT& target = next_bmf ? next_reflected_delta_in_main_tree_ : reflected_delta_in_main_tree_;
    ...
    bool PushPendingToActive()
    {
        ...
        return pending_value_before_push != current_active_value || active_value_before_push != current_active_value;
    }
    ...
    void AbortCommit(bool next_bmf, bool main_frame_applied_deltas)
    {
        ...
        if (next_bmf) {
            ...
            if (main_frame_applied_deltas) {
                reflected_delta_in_main_tree_ = T::CombineDeltas(reflected_delta_in_main_tree_, next_reflected_delta_in_main_tree_);
            }
            next_reflected_delta_in_main_tree_ = T::IdentityDelta();
        } else {
    ...
    BaseT PendingBase() const
    {
        return pending_base_;
    }
    BaseT ActiveBase() const
    {
        return active_base_;
    }
    ...
    DeltaT PendingDelta() const
    {
        ...
        return T::DeltaBetweenDeltas(active_delta_, reflected_delta_in_pending_tree_);
    }
    ...
    void set_clobber_active_value()
    {
        clobber_active_value_ = true;
    }
...
template <typename BaseT, typename DeltaT = BaseT> class AdditionGroup {
public:
    using BaseType = BaseT;
    using DeltaType = DeltaT;
    static constexpr BaseT IdentityBase()
    {
        return BaseT();
    }
    static constexpr DeltaT IdentityDelta()
    {
        return DeltaT();
    }
    static BaseT ApplyDelta(BaseT v, DeltaT delta)
    {
        return v + delta;
    }
    static DeltaT DeltaBetweenBases(BaseT v1, BaseT v2)
    {
        return v1 - v2;
    }
    static DeltaT DeltaBetweenDeltas(DeltaT d1, DeltaT d2)
    {
        return d1 - d2;
    }
    static DeltaT CombineDeltas(DeltaT d1, DeltaT d2)
    {
        return d1 + d2;
    }
};
...
public:
    ...
    static constexpr float IdentityBase()
    {
        return 1.f;
    }
    static constexpr float IdentityDelta()
    {
        return 1.f;
    }
    static float ApplyDelta(float v, float delta)
    {
        return v * delta;
    }
    static float DeltaBetweenBases(float v1, float v2)
    {
        return v1 / v2;
    }
    static float DeltaBetweenDeltas(float d1, float d2)
    {
        return d1 / d2;
    }
    static float CombineDeltas(float d1, float d2)
    {
        return d1 * d2;
    }
};
