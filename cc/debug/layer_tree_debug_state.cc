...
{
    return show_paint_rects || show_property_changed_rects || show_surface_damage_rects || show_screen_space_rects || show_touch_event_handler_rects
        || show_wheel_event_handler_rects || show_scroll_event_handler_rects || show_main_thread_scroll_hit_test_rects || show_main_thread_scroll_repaint_rects
        || show_raster_inducing_scroll_rects || show_layer_animation_bounds_rects || show_layout_shift_regions;
}
...
bool LayerTreeDebugState::operator==(const LayerTreeDebugState&) const = default;
