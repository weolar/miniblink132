...
//             return;
//         }
//         mapping->OnMemoryDump(pmd, buffer_dump_guid, tracing_process_id, importance);
    }
...
public:
    ZeroCopyRasterBufferImpl(base::WaitableEvent* shutdown_event, const ResourcePool::InUsePoolResource& in_use_resource, ZeroCopyGpuBacking* backing)
        : backing_(backing)
    ...
    ~ZeroCopyRasterBufferImpl() override
    {
        ...
        sii->UpdateSharedImage(backing_->returned_sync_token, backing_->shared_image->mailbox());
        ...
        const gfx::AxisTransform2d& transform, const RasterSource::PlaybackSettings& playback_settings, const GURL& url) override
    {
        ...
        if (!backing_->shared_image) {
            gpu::SharedImageUsageSet usage = gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_SCANOUT;
            backing_->shared_image = sii->CreateSharedImage(
                { format_, resource_size_, resource_color_space_, usage, "ZeroCopyRasterTile" }, gpu::kNullSurfaceHandle, kBufferUsage);
            if (!backing_->shared_image) {
        ...
        std::unique_ptr<gpu::ClientSharedImage::ScopedMapping> mapping = backing_->shared_image->Map();
        if (!mapping) {
            LOG(ERROR) << "MapSharedImage Failed.";
            sii->DestroySharedImage(gpu::SyncToken(), std::move(backing_->shared_image));
            return;
        }

        // TODO(danakj): Implement partial raster with raster_dirty_rect.
        RasterBufferProvider::PlaybackToMemory(mapping->GetMemoryForPlane(0).data(), format_, resource_size_, mapping->Stride(0), raster_source,
...
ZeroCopyRasterBufferProvider::ZeroCopyRasterBufferProvider(viz::RasterContextProvider* compositor_context_provider, const RasterCapabilities& raster_caps)
...
std::unique_ptr<RasterBuffer> ZeroCopyRasterBufferProvider::AcquireBufferForRaster(const ResourcePool::InUsePoolResource& resource,
    uint64_t resource_content_id, uint64_t previous_content_id, bool depends_on_at_raster_decodes, bool depends_on_hardware_accelerated_jpeg_candidates,
    bool depends_on_hardware_accelerated_webp_candidates)
{
    if (!resource.gpu_backing()) {
        ...
        backing->shared_image_interface = compositor_context_provider_->SharedImageInterface();
        resource.set_gpu_backing(std::move(backing));
    }
    ZeroCopyGpuBacking* backing = static_cast<ZeroCopyGpuBacking*>(resource.gpu_backing());

    return std::make_unique<ZeroCopyRasterBufferImpl>(shutdown_event_, resource, backing);
}
...
bool ZeroCopyRasterBufferProvider::CanPartialRasterIntoProvidedResource() const
{
    return false;
}

bool ZeroCopyRasterBufferProvider::IsResourceReadyToDraw(const ResourcePool::InUsePoolResource& resource)
{
...
void ZeroCopyRasterBufferProvider::SetShutdownEvent(base::WaitableEvent* shutdown_event)
{
    shutdown_event_ = shutdown_event;
}
