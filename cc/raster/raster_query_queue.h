...
public:
    explicit RasterQueryQueue(viz::RasterContextProvider* const worker_context_provider);
...
private:
    const raw_ptr<viz::RasterContextProvider, DanglingUntriaged> worker_context_provider_;

    // Note that this lock should never be acquired while holding the raster
    // context lock.
    base::Lock pending_raster_queries_lock_;
    base::circular_deque<RasterQuery> pending_raster_queries_ GUARDED_BY(pending_raster_queries_lock_);
};
