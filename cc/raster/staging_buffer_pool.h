...
struct StagingBuffer {
    ...
    void DestroyGLResources(gpu::raster::RasterInterface* gl, gpu::SharedImageInterface* sii);
    void OnMemoryDump(base::trace_event::ProcessMemoryDump* pmd, viz::SharedImageFormat dump_format, bool is_free) const;
...
class CC_EXPORT StagingBufferPool final : public base::trace_event::MemoryDumpProvider {
public:
    StagingBufferPool(scoped_refptr<base::SequencedTaskRunner> task_runner, viz::RasterContextProvider* worker_context_provider, bool use_partial_raster,
    ...
    std::unique_ptr<StagingBuffer> AcquireStagingBuffer(const gfx::Size& size, viz::SharedImageFormat format, uint64_t previous_content_id);
    void ReleaseStagingBuffer(std::unique_ptr<StagingBuffer> staging_buffer);

private:
    void AddStagingBuffer(const StagingBuffer* staging_buffer, viz::SharedImageFormat format) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    void RemoveStagingBuffer(const StagingBuffer* staging_buffer) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    void MarkStagingBufferAsFree(const StagingBuffer* staging_buffer) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    void ReleaseBuffersNotUsedSince(base::TimeTicks time) EXCLUSIVE_LOCKS_REQUIRED(lock_);

    std::unique_ptr<base::trace_event::ConvertableToTraceFormat> StateAsValue() const;
    void StagingStateAsValueInto(base::trace_event::TracedValue* staging_state) const;
    ...
    using StagingBufferSet = std::set<raw_ptr<const StagingBuffer, SetExperimental>>;
    StagingBufferSet buffers_;
    using StagingBufferDeque = base::circular_deque<std::unique_ptr<StagingBuffer>>;
