...
{
    return DecodedDrawImage(CreateRasterImage(), nullptr, SkSize::MakeEmpty(), SkSize::Make(1.0f, 1.0f), PaintFlags::FilterQuality::kMedium, true);
}

class MockDecodeCache : public StubDecodeCache {
public:
    MockDecodeCache() = default;
    ~MockDecodeCache() override
    {
        EXPECT_EQ(refed_image_count_, 0);
    }
    ...
    void DrawWithImageFinished(const DrawImage& draw_image, const DecodedDrawImage& decoded_draw_image) override
    {
        refed_image_count_--;
    ...
    int refed_image_count() const
    {
        return refed_image_count_;
    }
    int images_decoded() const
    {
        return images_decoded_;
    }
    const DrawImage& last_image()
    {
        return last_image_;
    }
...
TEST(PlaybackImageProviderTest, SkipsAllImages)
{
    ...
    EXPECT_FALSE(provider.GetRasterContent(DrawImage(
        PaintImageBuilder::WithDefault().set_id(PaintImage::GetNextId()).set_image(CreateRasterImage(), PaintImage::GetNextContentId()).TakePaintImage(), false,
        rect, PaintFlags::FilterQuality::kMedium, matrix)));
    EXPECT_EQ(cache.images_decoded(), 0);

    EXPECT_FALSE(
        provider.GetRasterContent(CreateDiscardableDrawImage(gfx::Size(10, 10), nullptr, SkRect::Make(rect), PaintFlags::FilterQuality::kMedium, matrix)));
    EXPECT_EQ(cache.images_decoded(), 0);
}
...
TEST(PlaybackImageProviderTest, SkipsSomeImages)
{
    ...
    PlaybackImageProvider provider(&cache, TargetColorParams(), std::move(settings));

    SkIRect rect = SkIRect::MakeWH(10, 10);
    SkM44 matrix = SkM44();
    EXPECT_FALSE(provider.GetRasterContent(DrawImage(skip_image, false, rect, PaintFlags::FilterQuality::kMedium, matrix)));
    EXPECT_EQ(cache.images_decoded(), 0);
}
...
TEST(PlaybackImageProviderTest, RefAndUnrefDecode)
{
...

    {
        ...
        SkM44 matrix = SkM44();
        auto decode = provider.GetRasterContent(CreateDiscardableDrawImage(gfx::Size(10, 10), nullptr, rect, PaintFlags::FilterQuality::kMedium, matrix));
        EXPECT_TRUE(decode);
...
TEST(PlaybackImageProviderTest, SwapsGivenFrames)
{
    ...
    SkIRect rect = SkIRect::MakeWH(10, 10);
    SkM44 matrix = SkM44();
    DrawImage draw_image(image, false, rect, PaintFlags::FilterQuality::kMedium, matrix);
...
TEST(PlaybackImageProviderTest, BitmapImages)
{
...

    {
        SkIRect rect = SkIRect::MakeWH(10, 10);
        SkM44 matrix = SkM44();
        auto draw_image = DrawImage(CreateBitmapImage(gfx::Size(10, 10)), false, rect, PaintFlags::FilterQuality::kMedium, matrix);
