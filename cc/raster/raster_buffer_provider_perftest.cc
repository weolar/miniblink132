...
class PerfGLES2Interface : public gpu::gles2::GLES2InterfaceStub {
    ...
    void GetQueryObjectuivEXT(GLuint query, GLenum pname, GLuint* params) override
    {
    ...
    void GenUnverifiedSyncTokenCHROMIUM(GLbyte* sync_token) override
    {
        ...
        gpu::SyncToken sync_token_data(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId(), 0);
...
class PerfContextProvider : public base::RefCountedThreadSafe<PerfContextProvider>, public viz::RasterContextProvider {
public:
    PerfContextProvider()
        ...
        , cache_controller_(&support_, nullptr)
    {
        ...
        raster_context_ = std::make_unique<gpu::raster::RasterImplementationGLES>(context_gl_.get(), ContextSupport(), capabilities_);
    }
    ...
    unsigned int GetGrGLTextureFormat(viz::SharedImageFormat format) const override
    {
        return viz::SharedImageFormatRestrictedSinglePlaneUtils::ToGLTextureStorageFormat(format, ContextCapabilities().angle_rgbx_internal_format);
    }
...
public:
    virtual std::unique_ptr<RasterBuffer> AcquireBufferForRaster(
        const ResourcePool::InUsePoolResource& resource, uint64_t resource_content_id, uint64_t previous_content_id, bool depends_on_at_raster_decodes)
        = 0;
};
...
public:
    PerfRasterTaskImpl(
        ResourcePool* pool, ResourcePool::InUsePoolResource in_use_resource, std::unique_ptr<RasterBuffer> raster_buffer, TileTask::Vector* dependencies)
...
public:
    ...
    RasterBufferProviderPerfTestBase()
        : compositor_context_provider_(base::MakeRefCounted<PerfContextProvider>())
        ...
        , timer_(kWarmupRuns, base::Milliseconds(kTimeLimitMillis), kTimeCheckInterval)
    {
    }

    void CreateImageDecodeTasks(unsigned num_image_decode_tasks, TileTask::Vector* image_decode_tasks)
    {
    ...
    void CreateRasterTasks(
        PerfRasterBufferProviderHelper* helper, unsigned num_raster_tasks, const TileTask::Vector& image_decode_tasks, RasterTaskVector* raster_tasks)
    {
        const gfx::Size size(1, 1);

        for (unsigned i = 0; i < num_raster_tasks; ++i) {
            ResourcePool::InUsePoolResource in_use_resource = resource_pool_->AcquireResource(size, viz::SinglePlaneFormat::kRGBA_8888, gfx::ColorSpace());

            // No tile ids are given to support partial updates.
            std::unique_ptr<RasterBuffer> raster_buffer;
            if (helper)
                raster_buffer = helper->AcquireBufferForRaster(in_use_resource, 0, 0, false);
            TileTask::Vector dependencies = image_decode_tasks;
            raster_tasks->push_back(new PerfRasterTaskImpl(resource_pool_.get(), std::move(in_use_resource), std::move(raster_buffer), &dependencies));
        }
    }
    ...
    void BuildTileTaskGraph(TaskGraph* graph, const RasterTaskVector& raster_tasks)
    {
        uint16_t priority = 0;
        ...
        for (auto& raster_task : raster_tasks) {
            ...
            for (auto& decode_task : raster_task->dependencies()) {
                ...
                if (!base::Contains(graph->nodes, decode_task, &TaskGraph::Node::task)) {
                    graph->nodes.push_back(TaskGraph::Node(decode_task.get(), 0u /* group */, priority, 0u));
                }

                graph->edges.push_back(TaskGraph::Edge(decode_task.get(), raster_task.get()));
            }

            graph->nodes.push_back(TaskGraph::Node(raster_task.get(), 0u /* group */, priority, static_cast<uint32_t>(raster_task->dependencies().size())));
        }
    }
...
class RasterBufferProviderPerfTest : public RasterBufferProviderPerfTestBase,
...
public:
    ...
    void SetUp() override
    {
        ...
        case RASTER_BUFFER_PROVIDER_TYPE_ZERO_COPY:
            ...
            raster_buffer_provider_ = std::make_unique<ZeroCopyRasterBufferProvider>(compositor_context_provider_.get(), raster_caps);
            break;
        case RASTER_BUFFER_PROVIDER_TYPE_ONE_COPY:
            Create3dResourceProvider();
            raster_caps.use_gpu_rasterization = false;
            raster_buffer_provider_ = std::make_unique<OneCopyRasterBufferProvider>(task_runner_.get(), compositor_context_provider_.get(),
                worker_context_provider_.get(), std::numeric_limits<int>::max(), false, std::numeric_limits<int>::max(), raster_caps);
            break;
        case RASTER_BUFFER_PROVIDER_TYPE_GPU:
            Create3dResourceProvider();
            raster_caps.use_gpu_rasterization = true;
            raster_buffer_provider_ = std::make_unique<GpuRasterBufferProvider>(
                compositor_context_provider_.get(), worker_context_provider_.get(), raster_caps, gfx::Size(), true, pending_raster_queries_.get());
            break;
        ...
        tile_task_manager_ = TileTaskManagerImpl::Create(task_graph_runner_.get(), base::DoNothing());
    }
    ...
    std::unique_ptr<RasterBuffer> AcquireBufferForRaster(
        const ResourcePool::InUsePoolResource& resource, uint64_t resource_content_id, uint64_t previous_content_id, bool depends_on_at_raster_decodes) override
    {
        return raster_buffer_provider_->AcquireBufferForRaster(resource, resource_content_id, previous_content_id, depends_on_at_raster_decodes,
    ...
    void RunScheduleTasksTest(const std::string& test_name, unsigned num_raster_tasks, unsigned num_image_decode_tasks)
    {
        TileTask::Vector image_decode_tasks;
        RasterTaskVector raster_tasks;
        CreateImageDecodeTasks(num_image_decode_tasks, &image_decode_tasks);
        CreateRasterTasks(this, num_raster_tasks, image_decode_tasks, &raster_tasks);
    ...
    void RunScheduleAlternateTasksTest(const std::string& test_name, unsigned num_raster_tasks, unsigned num_image_decode_tasks)
    {
        ...
        for (size_t i = 0; i < kNumVersions; ++i) {
            ...
            CreateRasterTasks(this, num_raster_tasks, image_decode_tasks[i], &raster_tasks[i]);
        }
        ...
        perf_test::PerfResultReporter reporter = SetUpReporter(test_name);
        reporter.AddResult("_alternate_tasks" + TestModifierString(), timer_.LapsPerSecond());
    }

    void RunScheduleAndExecuteTasksTest(const std::string& test_name, unsigned num_raster_tasks, unsigned num_image_decode_tasks)
    {
        TileTask::Vector image_decode_tasks;
        RasterTaskVector raster_tasks;
        CreateImageDecodeTasks(num_image_decode_tasks, &image_decode_tasks);
        ...
        perf_test::PerfResultReporter reporter = SetUpReporter(test_name);
        reporter.AddResult("_and_execute_tasks" + TestModifierString(), timer_.LapsPerSecond());
    }
...
protected:
    perf_test::PerfResultReporter SetUpReporter(const std::string& story_name)
    {
        ...
        reporter.RegisterImportantMetric("_alternate_tasks" + TestModifierString(), "runs/s");
        reporter.RegisterImportantMetric("_and_execute_tasks" + TestModifierString(), "runs/s");
        return reporter;
    }
...
INSTANTIATE_TEST_SUITE_P(RasterBufferProviderPerfTests, RasterBufferProviderPerfTest,
    ::testing::Values(
...
class RasterBufferProviderCommonPerfTest : public RasterBufferProviderPerfTestBase, public testing::Test {
public:
    // Overridden from testing::Test:
    void SetUp() override
    {
    ...
    void RunBuildTileTaskGraphTest(const std::string& test_name, unsigned num_raster_tasks, unsigned num_image_decode_tasks)
    {
        TileTask::Vector image_decode_tasks;
        RasterTaskVector raster_tasks;
        CreateImageDecodeTasks(num_image_decode_tasks, &image_decode_tasks);
        CreateRasterTasks(nullptr, num_raster_tasks, image_decode_tasks, &raster_tasks);
...
protected:
    perf_test::PerfResultReporter SetUpReporter(const std::string& story_name)
    {
        perf_test::PerfResultReporter reporter("build_raster_test_graph", story_name);
