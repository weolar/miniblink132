...
public:
    ...
    virtual void ExternalDependencyCompletedForTask(NamespaceToken token, scoped_refptr<Task> task) = 0;

    // Wait for all scheduled tasks to finish running.
    virtual void WaitForTasksToFinishRunning(NamespaceToken token) = 0;

    // Collect all completed tasks in |completed_tasks|.
    virtual void CollectCompletedTasks(NamespaceToken token, Task::Vector* completed_tasks) = 0;
