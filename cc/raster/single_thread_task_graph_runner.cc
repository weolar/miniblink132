...
void SingleThreadTaskGraphRunner::Start(const std::string& thread_name, const base::SimpleThread::Options& thread_options)
{
    thread_ = std::make_unique<base::DelegateSimpleThread>(this, thread_name, thread_options);
    thread_->StartAsync();
}
...
void SingleThreadTaskGraphRunner::ScheduleTasks(NamespaceToken token, TaskGraph* graph)
{
    TRACE_EVENT2("cc", "SingleThreadTaskGraphRunner::ScheduleTasks", "num_nodes", graph->nodes.size(), "num_edges", graph->edges.size());
...
void SingleThreadTaskGraphRunner::ExternalDependencyCompletedForTask(NamespaceToken token, scoped_refptr<Task> task)
{
    base::AutoLock lock(lock_);
...
void SingleThreadTaskGraphRunner::WaitForTasksToFinishRunning(NamespaceToken token)
{
    TRACE_EVENT0("cc", "SingleThreadTaskGraphRunner::WaitForTasksToFinishRunning");
...
void SingleThreadTaskGraphRunner::CollectCompletedTasks(NamespaceToken token, Task::Vector* completed_tasks)
{
...
{
    TRACE_EVENT0("toplevel", "SingleThreadTaskGraphRunner::RunTaskWithLockAcquired");
    ...
    auto found = base::ranges::find_if_not(
        ready_to_run_namespaces, &TaskGraphWorkQueue::TaskNamespace::Vector::empty, &TaskGraphWorkQueue::ReadyNamespaces::value_type::second);
    ...
    {
        TRACE_EVENT("toplevel", "cc::SingleThreadTaskGraphRunner::RunTask", perfetto::TerminatingFlow::Global(prioritized_task.task->trace_task_id()));
