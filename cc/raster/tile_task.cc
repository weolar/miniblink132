...
TileTask::TileTask(SupportsConcurrentExecution supports_concurrent_execution, SupportsBackgroundThreadPriority supports_background_thread_priority,
    ...
    , dependencies_(dependencies ? std::move(*dependencies) : TileTask::Vector())
{
}
...
bool TileTask::TaskContainsLCPCandidateImages() const
{
    for (auto dependent : dependencies_) {
        if (!dependent->HasCompleted() && dependent->TaskContainsLCPCandidateImages()) {
            return true;
        }
    }
    return false;
