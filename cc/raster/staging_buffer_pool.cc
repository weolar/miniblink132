...
void WaitForQueryResult(gpu::raster::RasterInterface* ri, GLuint query_id)
{
    ...
    while (attempts_left--) {
        ...
        base::PlatformThread::Sleep(base::Milliseconds(kCheckForQueryResultAvailableTickRateMs));
    }
...
StagingBuffer::StagingBuffer(const gfx::Size& size, viz::SharedImageFormat format)
    : size(size)
    , format(format)
{
}
...
void StagingBuffer::DestroyGLResources(gpu::raster::RasterInterface* ri, gpu::SharedImageInterface* sii)
{
    if (query_id) {
...
void StagingBuffer::OnMemoryDump(base::trace_event::ProcessMemoryDump* pmd, viz::SharedImageFormat dump_format, bool in_free_list) const
{
    // TODO(crbug.com/40263478): Need to call through to the buffer's
    // SharedImage's ScopedMapping::OnMemoryDump()?
}

StagingBufferPool::StagingBufferPool(scoped_refptr<base::SequencedTaskRunner> task_runner, viz::RasterContextProvider* worker_context_provider,
    bool use_partial_raster, int max_staging_buffer_usage_in_bytes)
    ...
    , staging_buffer_expiration_delay_(base::Milliseconds(kStagingBufferExpirationDelayMs))
    , reduce_memory_usage_pending_(false)
{
    DCHECK(worker_context_provider_);
    //base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "cc::StagingBufferPool", base::SingleThreadTaskRunner::GetCurrentDefault());

    memory_pressure_listener_
        = std::make_unique<base::MemoryPressureListener>(FROM_HERE, base::BindRepeating(&StagingBufferPool::OnMemoryPressure, weak_ptr_factory_.GetWeakPtr()));

    reduce_memory_usage_callback_ = base::BindRepeating(&StagingBufferPool::ReduceMemoryUsage, weak_ptr_factory_.GetWeakPtr());
}
...
void StagingBufferPool::ReleaseStagingBuffer(std::unique_ptr<StagingBuffer> staging_buffer)
{
    base::AutoLock lock(lock_);
...
bool StagingBufferPool::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    base::AutoLock lock(lock_);
    ...
    if (args.level_of_detail == MemoryDumpLevelOfDetail::kBackground) {
        ...
        dump->AddScalar(MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, staging_buffer_usage_in_bytes_);
    } else {
        for (const StagingBuffer* buffer : buffers_) {
            buffer->OnMemoryDump(pmd, buffer->format, base::Contains(free_buffers_, buffer, &std::unique_ptr<StagingBuffer>::get));
        }
    }
    return true;
}

void StagingBufferPool::AddStagingBuffer(const StagingBuffer* staging_buffer, viz::SharedImageFormat format)
{
...
void StagingBufferPool::RemoveStagingBuffer(const StagingBuffer* staging_buffer)
{
    DCHECK(base::Contains(buffers_, staging_buffer));
    buffers_.erase(staging_buffer);
    int buffer_usage_in_bytes = staging_buffer->format.EstimatedSizeInBytes(staging_buffer->size);
    DCHECK_GE(staging_buffer_usage_in_bytes_, buffer_usage_in_bytes);
    staging_buffer_usage_in_bytes_ -= buffer_usage_in_bytes;
}

void StagingBufferPool::MarkStagingBufferAsFree(const StagingBuffer* staging_buffer)
{
    int buffer_usage_in_bytes = staging_buffer->format.EstimatedSizeInBytes(staging_buffer->size);
    free_staging_buffer_usage_in_bytes_ += buffer_usage_in_bytes;
}

void StagingBufferPool::MarkStagingBufferAsBusy(const StagingBuffer* staging_buffer)
{
    int buffer_usage_in_bytes = staging_buffer->format.EstimatedSizeInBytes(staging_buffer->size);
...
std::unique_ptr<StagingBuffer> StagingBufferPool::AcquireStagingBuffer(const gfx::Size& size, viz::SharedImageFormat format, uint64_t previous_content_id)
{
    base::AutoLock lock(lock_);
    ...
    gpu::SharedImageInterface* sii = worker_context_provider_->SharedImageInterface();
    DCHECK(ri);
    ...
    while (!busy_buffers_.empty()) {
        ...
        if (!busy_buffers_.front()->query_id || !CheckForQueryResult(ri, busy_buffers_.front()->query_id))
            break;
    ...
    while ((staging_buffer_usage_in_bytes_ - free_staging_buffer_usage_in_bytes_) >= max_staging_buffer_usage_in_bytes_) {
    ...
    if (use_partial_raster_ && previous_content_id) {
        StagingBufferDeque::iterator it = base::ranges::find(free_buffers_, previous_content_id, &StagingBuffer::content_id);
    ...
    if (!staging_buffer) {
        StagingBufferDeque::iterator it = base::ranges::find_if(
            free_buffers_, [&size, format](const std::unique_ptr<StagingBuffer>& buffer) { return buffer->size == size && buffer->format == format; });
        if (it != free_buffers_.end()) {
            staging_buffer = std::move(*it);
            free_buffers_.erase(it);
            MarkStagingBufferAsBusy(staging_buffer.get());
...
void StagingBufferPool::ScheduleReduceMemoryUsage()
{
    ...
    task_runner_->PostDelayedTask(FROM_HERE, reduce_memory_usage_callback_, reduce_memory_usage_time - base::TimeTicks::Now());
}
...
void StagingBufferPool::ReduceMemoryUsage()
{
    ...
    base::TimeTicks reduce_memory_usage_time = GetUsageTimeForLRUBuffer() + staging_buffer_expiration_delay_;
    task_runner_->PostDelayedTask(FROM_HERE, reduce_memory_usage_callback_, reduce_memory_usage_time - current_time);
}
...
void StagingBufferPool::OnMemoryPressure(base::MemoryPressureListener::MemoryPressureLevel level)
{
    base::AutoLock lock(lock_);
    switch (level) {
