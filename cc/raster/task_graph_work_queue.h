...
public:
    ...
    struct CC_EXPORT PrioritizedTask {
        ...
        PrioritizedTask(scoped_refptr<Task> task, TaskNamespace* task_namespace, uint16_t category, uint16_t priority);
    ...
    bool ExternalDependencyCompletedForTask(NamespaceToken token, scoped_refptr<Task> task);
    ...
    // namespace.
    void CollectCompletedTasks(NamespaceToken token, Task::Vector* completed_tasks);
    ...
    static bool HasReadyToRunTasksInNamespace(const TaskNamespace* task_namespace)
    {
        return !base::ranges::all_of(task_namespace->ready_to_run_tasks, &PrioritizedTask::Vector::empty, &TaskNamespace::ReadyTasks::value_type::second);
    }

    static bool HasTasksBlockedOnExternalDependencyInNamespace(const TaskNamespace* task_namespace)
    {
        return base::ranges::any_of(task_namespace->graph.nodes, &TaskGraph::Node::has_external_dependency);
    }

    static bool HasFinishedRunningTasksInNamespace(const TaskNamespace* task_namespace)
    {
    ...
    {
        return !base::ranges::all_of(ready_to_run_namespaces_, &TaskNamespace::Vector::empty, &ReadyNamespaces::value_type::second);
    }
    ...
    {
        return base::ranges::all_of(namespaces_, [](const TaskNamespaceMap::value_type& entry) { return HasFinishedRunningTasksInNamespace(&entry.second); });
    }
    ...
    size_t NumRunningTasksForCategory(uint16_t category) const
    {
        size_t count = 0;
        for (const auto& task_namespace_entry : namespaces_) {
            for (const auto& categorized_task : task_namespace_entry.second.running_tasks) {
    ...
    size_t NumReadyTasksForCategory(uint16_t category) const
    {
        ...
        if (found == ready_to_run_namespaces_.end())
            return 0;
        size_t count = 0;
        for (TaskGraphWorkQueue::TaskNamespace* task_namespace_entry : found->second) {
            DCHECK(base::Contains(task_namespace_entry->ready_to_run_tasks, category));
...
private:
    bool DecrementNodeDependencies(TaskGraph::Node& node, TaskNamespace* task_namespace, bool rebuild_heap);

    // Helper class used to provide NamespaceToken comparison to TaskNamespaceMap.
    class CompareToken {
    public:
        bool operator()(const NamespaceToken& lhs, const NamespaceToken& rhs) const
        {
