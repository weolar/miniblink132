...
RasterQueryQueue::RasterQueryQueue(viz::RasterContextProvider* const worker_context_provider)
...
#define UMA_HISTOGRAM_RASTER_TIME_CUSTOM_MICROSECONDS(name, total_time)                                                                                        \
    UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES(name, total_time, base::Microseconds(1), base::Milliseconds(100), 100);
...
bool RasterQueryQueue::CheckRasterFinishedQueries()
{
    ...
    while (it != pending_raster_queries_.end()) {
        GLuint complete = 0;
        ri->GetQueryObjectuivEXT(it->raster_duration_query_id, GL_QUERY_RESULT_AVAILABLE_NO_FLUSH_CHROMIUM_EXT, &complete);
        if (!complete)
            break;
        ...
        if (it->raster_start_query_id) {
            ...
            complete = 0;
            ri->GetQueryObjectuivEXT(it->raster_start_query_id, GL_QUERY_RESULT_AVAILABLE_NO_FLUSH_CHROMIUM_EXT, &complete);
            DCHECK(complete);
        }
#endif

        GLuint gpu_raster_duration = 0u;
        ri->GetQueryObjectuivEXT(it->raster_duration_query_id, GL_QUERY_RESULT_EXT, &gpu_raster_duration);
        ri->DeleteQueriesEXT(1, &it->raster_duration_query_id);

        base::TimeDelta raster_duration = it->worker_raster_duration + base::Microseconds(gpu_raster_duration);
        ...
        if (it->raster_start_query_id) {
            ...
            ri->GetQueryObjectui64vEXT(it->raster_start_query_id, GL_QUERY_RESULT_EXT, &gpu_raster_start_time);
            ...
            base::TimeDelta raster_scheduling_delay
                = base::Microseconds(base::checked_cast<int64_t>(gpu_raster_start_time)) - it->raster_buffer_creation_time.since_origin();
            ...
            UMA_HISTOGRAM_RASTER_TIME_CUSTOM_MICROSECONDS(
                base::StringPrintf("Renderer4.%s.RasterTaskSchedulingDelayNoAtRasterDecodes.All", client_name), raster_scheduling_delay);
            if (it->depends_on_hardware_accelerated_jpeg_candidates) {
                UMA_HISTOGRAM_RASTER_TIME_CUSTOM_MICROSECONDS(base::StringPrintf("Renderer4.%s.RasterTaskSchedulingDelayNoAtRasterDecodes."
        ...
        UMA_HISTOGRAM_RASTER_TIME_CUSTOM_MICROSECONDS(base::StringPrintf("Renderer4.%s.RasterTaskTotalDuration.Oop", client_name), raster_duration);
