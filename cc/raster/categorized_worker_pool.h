...
class CC_EXPORT CategorizedWorkerPool : public base::TaskRunner, public TaskGraphRunner {
public:
    ...
    void CollectCompletedTasks(NamespaceToken token, Task::Vector* completed_tasks) override;
...
protected:
    ...
    void CollectCompletedTasksWithLockAcquired(NamespaceToken token, Task::Vector* completed_tasks) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    bool ShouldRunTaskForCategoryWithLockAcquired(TaskCategory category) EXCLUSIVE_LOCKS_REQUIRED(lock_);
...
public:
    ...
    void ExternalDependencyCompletedForTask(NamespaceToken token, scoped_refptr<Task> task) override;

    // Runs a task from one of the provided categories. Categories listed first
    // have higher priority.
    void Run(base::span<const TaskCategory> categories, base::JobDelegate* job_delegate);
...
private:
    ...
    std::optional<TaskGraphWorkQueue::PrioritizedTask> GetNextTaskToRunWithLockAcquired(base::span<const TaskCategory> categories);

    base::JobHandle* ScheduleTasksWithLockAcquired(NamespaceToken token, TaskGraph* graph) EXCLUSIVE_LOCKS_REQUIRED(lock_);

    // Helper function which signals worker threads if tasks are ready to run.
    base::JobHandle* GetJobHandleToNotifyWithLockAcquired() EXCLUSIVE_LOCKS_REQUIRED(lock_);
