...
TEST(RasterSourceTest, AnalyzeIsSolidUnscaled)
{
    ...
    recording_source.add_draw_rect_with_flags(gfx::Rect(50, 50, 1, 1), non_solid_flags);
    recording_source.Rerecord();
    raster = recording_source.CreateRasterSource();

    color = SkColors::kTransparent;
    is_solid_color = raster->PerformSolidColorAnalysis(gfx::Rect(0, 0, 100, 100), &color);
    EXPECT_FALSE(is_solid_color);

    color = SkColors::kTransparent;
    is_solid_color = raster->PerformSolidColorAnalysis(gfx::Rect(100, 0, 100, 100), &color);
    EXPECT_TRUE(is_solid_color);
    EXPECT_COLOR_EQ(solid_color, color.toSkColor());

    // Boundaries should be clipped.
    color = SkColors::kTransparent;
    is_solid_color = raster->PerformSolidColorAnalysis(gfx::Rect(350, 0, 100, 100), &color);
    EXPECT_TRUE(is_solid_color);
    EXPECT_COLOR_EQ(solid_color, color.toSkColor());

    color = SkColors::kTransparent;
    is_solid_color = raster->PerformSolidColorAnalysis(gfx::Rect(0, 350, 100, 100), &color);
    EXPECT_TRUE(is_solid_color);
    EXPECT_COLOR_EQ(solid_color, color.toSkColor());

    color = SkColors::kTransparent;
    is_solid_color = raster->PerformSolidColorAnalysis(gfx::Rect(350, 350, 100, 100), &color);
    EXPECT_TRUE(is_solid_color);
    EXPECT_COLOR_EQ(solid_color, color.toSkColor());
}
...
{
    gfx::Size layer_bounds(400, 400);
    const std::vector<float> recording_scales = { 1.25f, 1.33f, 1.5f, 1.6f, 1.66f, 2.f, 2.25f, 2.5f };
    for (float recording_scale : recording_scales) {
        ...
        recording_source.add_draw_rect_with_flags(gfx::ScaleToEnclosingRect(gfx::Rect(layer_bounds), recording_scale), solid_flags);
        recording_source.Rerecord();
        ...
        for (int y = 0; y <= 300; y += 100) {
            for (int x = 0; x <= 300; x += 100) {
                gfx::Rect rect(x, y, 100, 100);
                is_solid_color = raster->PerformSolidColorAnalysis(rect, &color);
                EXPECT_TRUE(is_solid_color) << rect.ToString() << " recording_scale: " << recording_scale;
                EXPECT_COLOR_EQ(solid_color, color.toSkColor()) << rect.ToString() << " recording_scale: " << recording_scale;
            }
        }

        // Add one non-solid pixel and recreate the raster source.
        recording_source.add_draw_rect_with_flags(gfx::Rect(std::round(50 * recording_scale), std::round(50 * recording_scale), 1, 1), non_solid_flags);
        recording_source.Rerecord();
        raster = recording_source.CreateRasterSource();
        ...
        color = SkColors::kTransparent;
        is_solid_color = raster->PerformSolidColorAnalysis(gfx::Rect(0, 0, 51, 51), &color);
        EXPECT_FALSE(is_solid_color) << " recording_scale: " << recording_scale;

        color = SkColors::kTransparent;
        is_solid_color = raster->PerformSolidColorAnalysis(gfx::Rect(51, 0, 100, 100), &color);
        EXPECT_TRUE(is_solid_color) << " recording_scale: " << recording_scale;
        EXPECT_COLOR_EQ(solid_color, color.toSkColor()) << " recording_scale: " << recording_scale;
...
TEST(RasterSourceTest, PixelRefIteratorDiscardableRefsOneTile)
{
    ...
    recording_source.add_draw_image(discardable_image[1][1], gfx::Point(260, 260));
    recording_source.Rerecord();

    scoped_refptr<RasterSource> raster = recording_source.CreateRasterSource();
    scoped_refptr<DiscardableImageMap> image_map = raster->GetDisplayItemList()->GenerateDiscardableImageMap();

    // Tile sized iterators. These should find only one pixel ref.
    {
        TargetColorParams target_color_params;
        std::vector<const DrawImage*> images = image_map->GetDiscardableImagesInRect(gfx::Rect(0, 0, 256, 256));
        ASSERT_EQ(1u, images.size());
        DrawImage image(*images[0], 1.f, PaintImage::kDefaultFrameIndex, target_color_params);
        EXPECT_TRUE(discardable_image[0][0].IsSameForTesting(images[0]->paint_image()));
    ...
    // Shifted tile sized iterators. These should find only one pixel ref.
    {
        ...
        std::vector<const DrawImage*> images = image_map->GetDiscardableImagesInRect(gfx::Rect(260, 260, 256, 256));
        ASSERT_EQ(1u, images.size());
        DrawImage image(*images[0], 1.f, PaintImage::kDefaultFrameIndex, target_color_params);
        EXPECT_TRUE(discardable_image[1][1].IsSameForTesting(images[0]->paint_image()));
        EXPECT_EQ(target_color_params.color_space, image.target_color_space());
    }
    // Ensure there's no discardable pixel refs in the empty cell
    {
        std::vector<const DrawImage*> images = image_map->GetDiscardableImagesInRect(gfx::Rect(0, 256, 256, 256));
        EXPECT_EQ(0u, images.size());
    }
    // Layer sized iterators. These should find three pixel ref.
    {
        std::vector<const DrawImage*> images = image_map->GetDiscardableImagesInRect(gfx::Rect(0, 0, 512, 512));
        ASSERT_EQ(3u, images.size());
        EXPECT_TRUE(discardable_image[0][0].IsSameForTesting(images[0]->paint_image()));
        EXPECT_TRUE(discardable_image[0][1].IsSameForTesting(images[1]->paint_image()));
        EXPECT_TRUE(discardable_image[1][1].IsSameForTesting(images[2]->paint_image()));
    }
}
...
TEST(RasterSourceTest, RasterFullContents)
{
    ...
    recording_source.add_draw_rect_with_flags(gfx::Rect(layer_bounds), white_flags);
    recording_source.Rerecord();

    scoped_refptr<RasterSource> raster = recording_source.CreateRasterSource();

    gfx::Size content_bounds(gfx::ScaleToCeiledSize(layer_bounds, contents_scale));
    ...
    for (int offset_x = 0; offset_x < content_bounds.width(); offset_x += step_x) {
        for (int offset_y = 0; offset_y < content_bounds.height(); offset_y += step_y) {
            ...
            raster->PlaybackToCanvas(
                &canvas, content_bounds, canvas_rect, canvas_rect, gfx::AxisTransform2d(contents_scale, gfx::Vector2dF()), RasterSource::PlaybackSettings());

            bool all_white = true;
            for (int i = 0; i < bitmap.width(); i++) {
                for (int j = 0; j < bitmap.height(); j++) {
                    SkColor pixel = bitmap.getColor(i, j);
                    EXPECT_EQ(SkColorGetA(pixel), 255u) << offset_x + i << "," << offset_y + j;
...
TEST(RasterSourceTest, RasterFullContentsWithRasterTranslation)
{
    ...
    for (int offset_x = 0; offset_x < content_bounds.width(); offset_x += step_x) {
        for (int offset_y = 0; offset_y < content_bounds.height(); offset_y += step_y) {
            ...
            raster->PlaybackToCanvas(
                &canvas, content_bounds, canvas_rect, canvas_rect, gfx::AxisTransform2d(1.0f, gfx::Vector2dF(0.3f, 0.7f)), RasterSource::PlaybackSettings());
            ...
            for (int i = 0; i < bitmap.width(); i++) {
                for (int j = 0; j < bitmap.height(); j++) {
                    ...
                    EXPECT_EQ(expect_white, pixel == SK_ColorWHITE) << content_x << "," << content_y;
                }
            }
        }
    }
...
TEST(RasterSourceTest, RasterPartialContents)
{
    ...
    raster->PlaybackToCanvas(
        &canvas, content_bounds, raster_full_rect, playback_rect, gfx::AxisTransform2d(contents_scale, gfx::Vector2dF()), RasterSource::PlaybackSettings());
    ...
    recording_source.add_draw_rect_with_flags(gfx::Rect(layer_bounds), black_flags);
    recording_source.Rerecord();
...
TEST(RasterSourceTest, RasterPartialContentsWithRasterTranslation)
{
    ...
    gfx::Rect playback_rect(content_bounds);
    raster->PlaybackToCanvas(
        &canvas, content_bounds, raster_full_rect, playback_rect, gfx::AxisTransform2d(1.0f, gfx::Vector2dF(0.3f, 0.7f)), RasterSource::PlaybackSettings());
    ...
    for (int i = 0; i < bitmap.width(); ++i) {
        for (int j = 0; j < bitmap.height(); ++j) {
            ...
            EXPECT_EQ(expect_white, pixel == SK_ColorWHITE) << Color { pixel } << " " << i << "," << j;
        }
    }
    ...
    for (int i = 0; i < bitmap.width(); ++i) {
        for (int j = 0; j < bitmap.height(); ++j) {
            ...
            if (expect_black) {
                ...
                EXPECT_TRUE(pixel != SK_ColorBLACK && pixel != SK_ColorWHITE) << Color { pixel };
            }
        }
    }
    EXPECT_GT(num_black, 0);
...
TEST(RasterSourceTest, RasterPartialClear)
{
    ...
    recording_source_light.add_draw_rect_with_flags(gfx::Rect(layer_bounds), white_flags);
...
TEST(RasterSourceTest, RasterTransformWithoutRecordingScale)
{
    ...
    raster_source->PlaybackToCanvas(&mock_canvas, size, gfx::Rect(small_size), gfx::Rect(small_size), gfx::AxisTransform2d(), RasterSource::PlaybackSettings());
}
