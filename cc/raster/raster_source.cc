...
RasterSource::RasterSource(const RecordingSource& other)
    ...
    , size_(other.size_)
    , slow_down_raster_scale_factor_for_debug_(other.slow_down_raster_scale_factor_for_debug_)
...
{
    DCHECK(recorded_bounds_.IsEmpty() || gfx::Rect(size_).Contains(recorded_bounds_));
}

RasterSource::~RasterSource() = default;

void RasterSource::ClearForOpaqueRaster(SkCanvas* raster_canvas, const gfx::AxisTransform2d& raster_transform, const gfx::Size& content_size,
    const gfx::Rect& canvas_bitmap_rect, const gfx::Rect& canvas_playback_rect) const
{
    gfx::Rect outer_rect;
    gfx::Rect inner_rect;
    ...
    if (!CalculateClearForOpaqueRasterRects(
            raster_transform.translation(), scale, content_size, canvas_bitmap_rect, canvas_playback_rect, outer_rect, inner_rect))
        return;

    raster_canvas->save();
    raster_canvas->clipRect(gfx::RectToSkRect(outer_rect), SkClipOp::kIntersect, false);
    if (!inner_rect.IsEmpty()) {
        raster_canvas->clipRect(gfx::RectToSkRect(inner_rect), SkClipOp::kDifference, false);
    }
    raster_canvas->drawColor(background_color_, SkBlendMode::kSrc);
    raster_canvas->restore();
}

void RasterSource::PlaybackToCanvas(SkCanvas* raster_canvas, const gfx::Size& content_size, const gfx::Rect& canvas_bitmap_rect,
    const gfx::Rect& canvas_playback_rect, const gfx::AxisTransform2d& raster_transform, const PlaybackSettings& settings) const
{
    ...
    if (!requires_clear_) {
        ...
        // raster.
        ClearForOpaqueRaster(raster_canvas, raster_transform, content_size, canvas_bitmap_rect, canvas_playback_rect);
    ...
    raster_canvas->translate(raster_transform.translation().x(), raster_transform.translation().y());
    raster_canvas->scale(raster_transform.scale().x() / recording_scale_factor_, raster_transform.scale().y() / recording_scale_factor_);
...
void RasterSource::PlaybackDisplayListToCanvas(SkCanvas* raster_canvas, const PlaybackSettings& settings) const
{
    ...
    params.raster_inducing_scroll_offsets = settings.raster_inducing_scroll_offsets;
    for (int i = 0; i < repeat_count; ++i) {
        display_list_->Raster(raster_canvas, params);
    }
}

bool RasterSource::PerformSolidColorAnalysis(gfx::Rect layer_rect, SkColor4f* color, int max_ops_to_analyze) const
{
    ...
    return display_list_->GetColorIfSolidInRect(layer_rect, color, max_ops_to_analyze);
}
...
gfx::Size RasterSource::GetContentSize(const gfx::Vector2dF& content_scale) const
{
...
RasterSource::PlaybackSettings::PlaybackSettings(const PlaybackSettings&) = default;
