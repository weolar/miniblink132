...
bool CompareTaskPriority(const TaskGraphWorkQueue::PrioritizedTask& a, const TaskGraphWorkQueue::PrioritizedTask& b)
{
...
public:
    ...
    bool operator()(const TaskGraphWorkQueue::TaskNamespace* a, const TaskGraphWorkQueue::TaskNamespace* b)
    {
        ...
        return CompareTaskPriority(a->ready_to_run_tasks.at(category_).front(), b->ready_to_run_tasks.at(category_).front());
    }
...
public:
    ...
    DependentIterator& operator++()
    {
        ...
        auto it = base::ranges::find(graph_->nodes, graph_->edges[current_index_].dependent.get(), &TaskGraph::Node::task);
    ...
    operator bool() const
    {
        return current_index_ < graph_->edges.size();
    }
...
void RebuildNamespaceHeaps(TaskGraphWorkQueue::ReadyNamespaces& namespaces)
{
    ...
    for (auto& it : namespaces) {
        ...
        std::make_heap(task_namespace.begin(), task_namespace.end(), CompareTaskNamespacePriority(category));
    }
}
...
TaskGraphWorkQueue::PrioritizedTask::PrioritizedTask(scoped_refptr<Task> task, TaskNamespace* task_namespace, uint16_t category, uint16_t priority)
    : task(std::move(task))
...
void TaskGraphWorkQueue::ScheduleTasks(NamespaceToken token, TaskGraph* graph)
{
    ...
    TRACE_EVENT("toplevel", "cc::TaskGraphWorkQueue::ScheduleTasks", [&](perfetto::EventContext ctx) {
        for (const TaskGraph::Node& node : graph->nodes) {
            DCHECK(node.task->trace_task_id() != 0) << "Every raster task should be associated with a task id.\n";
    ...
    for (const TaskGraph::Node& node : graph->nodes) {
        ...
        auto old_it = base::ranges::find(task_namespace.graph.nodes, node.task, &TaskGraph::Node::task);
        ...
        if (base::Contains(task_namespace.running_tasks, node.task.get(), &CategorizedTask::second)) {
            continue;
        }

        node.task->state().DidSchedule();
        task_namespace.ready_to_run_tasks[node.category].emplace_back(node.task, &task_namespace, node.category, node.priority);
    }
    ...
    for (auto& it : task_namespace.ready_to_run_tasks) {
        ...
        std::make_heap(ready_to_run_tasks.begin(), ready_to_run_tasks.end(), CompareTaskPriority);
    }
    ...
    for (auto& namespace_it : namespaces_) {
        ...
        for (auto& ready_to_run_tasks_it : task_namespace_to_check.ready_to_run_tasks) {
...
TaskGraphWorkQueue::PrioritizedTask TaskGraphWorkQueue::GetNextTaskToRun(uint16_t category)
{
    ...
    std::pop_heap(ready_to_run_namespaces.begin(), ready_to_run_namespaces.end(), CompareTaskNamespacePriority(category));
    ...
    std::pop_heap(ready_to_run_tasks.begin(), ready_to_run_tasks.end(), CompareTaskPriority);
    ...
    if (!ready_to_run_tasks.empty()) {
        ...
        std::push_heap(ready_to_run_namespaces.begin(), ready_to_run_namespaces.end(), CompareTaskNamespacePriority(category));
    }

    // Add task to |running_tasks|.
    task.task->state().DidStart();
    task_namespace->running_tasks.push_back(std::make_pair(task.category, task.task));

    return task;
}

bool TaskGraphWorkQueue::ExternalDependencyCompletedForTask(NamespaceToken token, scoped_refptr<Task> task)
{
    TaskNamespace* task_namespace = GetNamespaceForToken(token);
    CHECK(task_namespace || task->state().IsCanceled());
    if (task_namespace) {
        auto iter = base::ranges::find(task_namespace->graph.nodes, task.get(), &TaskGraph::Node::task);
...
bool TaskGraphWorkQueue::DecrementNodeDependencies(TaskGraph::Node& node, TaskNamespace* task_namespace, bool rebuild_heap)
{
    DCHECK_LT(0u, node.dependencies);
    node.dependencies--;
    ...
    if (!node.dependencies && node.task->state().IsNew()) {
        PrioritizedTask::Vector& ready_to_run_tasks = task_namespace->ready_to_run_tasks[node.category];

        bool was_empty = ready_to_run_tasks.empty();
        node.task->state().DidSchedule();
        ready_to_run_tasks.emplace_back(node.task, task_namespace, node.category, node.priority);
        std::push_heap(ready_to_run_tasks.begin(), ready_to_run_tasks.end(), CompareTaskPriority);
        ...
        if (was_empty) {
            TaskNamespace::Vector& ready_to_run_namespaces = ready_to_run_namespaces_[node.category];
        ...
        if (rebuild_heap) {
            ...
            std::make_heap(category_namespaces.begin(), category_namespaces.end(), CompareTaskNamespacePriority(node.category));
        }
        return true;
    }
    return false;
...
void TaskGraphWorkQueue::CompleteTask(PrioritizedTask completed_task)
{
    ...
    auto it = base::ranges::find(task_namespace->running_tasks, task, &CategorizedTask::second);
    ...
    for (DependentIterator dependent_it(&task_namespace->graph, task.get()); dependent_it; ++dependent_it) {
        // rebuild_heap=false to avoid rebuilding the heap on every iteration; just
        // do it once at the end if necessary.
        ready_to_run_namespaces_has_heap_properties &= !DecrementNodeDependencies(*dependent_it, task_namespace, false /*rebuild_heap*/);
    }
...
void TaskGraphWorkQueue::CollectCompletedTasks(NamespaceToken token, Task::Vector* completed_tasks)
{
