...
private:
    ...
    enum class Value : uint16_t { kNew, kScheduled, kRunning, kFinished, kCanceled };
...
public:
    ...
    void set_frame_number(int64_t frame_number)
    {
    ...
    void set_trace_task_id(uint64_t id)
    {
        trace_task_id_ = id;
    }
...
struct CC_EXPORT TaskGraph {
    struct CC_EXPORT Node {
        ...
        Node(scoped_refptr<Task> new_task, uint16_t category, uint16_t priority, uint32_t dependencies, bool has_external_dependency = false);
