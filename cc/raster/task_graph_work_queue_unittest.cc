...
TEST(TaskGraphWorkQueueTest, TestChangingDependency)
{
    ...
    // Run the task.
    TaskGraphWorkQueue::PrioritizedTask prioritized_task = work_queue.GetNextTaskToRun(0u);
    ...
    TaskGraphWorkQueue::PrioritizedTask prioritized_dependency_task = work_queue.GetNextTaskToRun(0u);
...
TEST(TaskGraphWorkQueueTest, ExternalDependency)
{
    ...
    graph.nodes.emplace_back(task, category, priority, 1u /*dependencies*/, true /*has_external_dependency*/);
    work_queue.ScheduleTasks(token, &graph);
    ...
    EXPECT_TRUE(TaskGraphWorkQueue::HasTasksBlockedOnExternalDependencyInNamespace(task_namespace));

    // Complete external dependency; `task` should be ready to run.
    work_queue.ExternalDependencyCompletedForTask(token, task);
    EXPECT_FALSE(TaskGraphWorkQueue::DependencyMismatch(&graph));
    EXPECT_TRUE(work_queue.HasReadyToRunTasks());
    EXPECT_FALSE(TaskGraphWorkQueue::HasTasksBlockedOnExternalDependencyInNamespace(task_namespace));
    TaskGraphWorkQueue::PrioritizedTask prioritized_dependency_task = work_queue.GetNextTaskToRun(category);
    EXPECT_EQ(prioritized_dependency_task.task.get(), task.get());
