...
void SynchronousTaskGraphRunner::ScheduleTasks(NamespaceToken token, TaskGraph* graph)
{
    TRACE_EVENT2("cc", "SynchronousTaskGraphRunner::ScheduleTasks", "num_nodes", graph->nodes.size(), "num_edges", graph->edges.size());
...
void SynchronousTaskGraphRunner::ExternalDependencyCompletedForTask(NamespaceToken token, scoped_refptr<Task> task)
{
    work_queue_.ExternalDependencyCompletedForTask(token, std::move(task));
}

void SynchronousTaskGraphRunner::WaitForTasksToFinishRunning(NamespaceToken token)
{
...
void SynchronousTaskGraphRunner::CollectCompletedTasks(NamespaceToken token, Task::Vector* completed_tasks)
{
...
bool SynchronousTaskGraphRunner::RunTask()
{
    ...
    TRACE_EVENT("toplevel", "cc::SynchronousTaskGraphRunner::RunTask", perfetto::TerminatingFlow::Global(prioritized_task.task->trace_task_id()));
