...
public:
    static std::unique_ptr<TilingSetRasterQueueAll> Create(PictureLayerTilingSet* tiling_set, bool prioritize_low_res, bool is_drawing_layer);
    ...
    void Pop();
    bool IsEmpty() const;
    bool is_drawing_layer() const
    {
        return is_drawing_layer_;
    }
...
private:
    ...
    public:
        ...
        OnePriorityRectIterator(PictureLayerTiling* tiling, TilingData* tiling_data, PictureLayerTiling::PriorityRectType priority_rect_type);

        bool done() const
        {
            return !current_tile_.tile();
        }
        const PrioritizedTile& operator*() const
        {
    ...
    protected:
        ...
        template <typename TilingIteratorType> void AdvanceToNextTile(TilingIteratorType* iterator);
        template <typename TilingIteratorType> bool GetFirstTileAndCheckIfValid(TilingIteratorType* iterator);

        enum IsTileValidResult { kTileNotValid, kTileNeedsRaster, kTileNeedsCheckerImageReraster };
    ...
    public:
        ...
        PendingVisibleTilingIterator(PictureLayerTiling* tiling, TilingData* tiling_data);
    ...
    public:
        ...
        SoonBorderTilingIterator(PictureLayerTiling* tiling, TilingData* tiling_data);
    ...
    public:
        ...
        EventuallyTilingIterator(PictureLayerTiling* tiling, TilingData* tiling_data);
    ...
    enum IteratorType { LOW_RES, HIGH_RES, ACTIVE_NON_IDEAL_PENDING_HIGH_RES, NUM_ITERATORS };

    TilingSetRasterQueueAll(PictureLayerTiling* high_res_tiling, PictureLayerTiling* low_res_tiling,
