...
public:
    ...
    // reporting systems.
    struct TracingInfo {
        TracingInfo(uint64_t prepare_tiles_id, TilePriority::PriorityBin requesting_tile_bin)
    ...
    static devtools_instrumentation::ScopedImageDecodeTask::TaskType ToScopedTaskType(TaskType task_type)
    {
        using ScopedTaskType = devtools_instrumentation::ScopedImageDecodeTask::TaskType;
        switch (task_type) {
    ...
    static devtools_instrumentation::ScopedImageDecodeTask::ImageType ToScopedImageType(ImageType image_type)
    {
        using ScopedImageType = devtools_instrumentation::ScopedImageDecodeTask::ImageType;
        switch (image_type) {
    ...
    struct CC_EXPORT TaskResult {
        explicit TaskResult(bool need_unref, bool is_at_raster_decode, bool can_do_hardware_accelerated_decode);
        explicit TaskResult(scoped_refptr<TileTask> task, bool can_do_hardware_accelerated_decode);
    ...
    // a raster task.
    virtual TaskResult GetTaskForImageAndRef(ClientId client_id, const DrawImage& image, const TracingInfo& tracing_info) = 0;
    ...
    virtual TaskResult GetOutOfRasterDecodeTaskForImageAndRef(ClientId client_id, const DrawImage& image) = 0;
    ...
    virtual void DrawWithImageFinished(const DrawImage& image, const DecodedDrawImage& decoded_image) = 0;
    ...
    virtual void SetShouldAggressivelyFreeResources(bool aggressively_free_resources, bool context_lock_acquired) = 0;
