...
public:
    ...
    virtual const PictureLayerTiling* GetPendingOrActiveTwinTiling(const PictureLayerTiling* tiling) const = 0;
    ...
    virtual std::vector<const DrawImage*> GetDiscardableImagesInRect(const gfx::Rect& rect) const = 0;
    virtual ScrollOffsetMap GetRasterInducingScrollOffsets() const = 0;
    virtual const GlobalStateThatImpactsTilePriority& global_tile_state() const = 0;
...
public:
    ...
    PictureLayerTiling(WhichTree tree, const gfx::AxisTransform2d& raster_transform, scoped_refptr<RasterSource> raster_source,
    ...
    void TakeTilesAndPropertiesFrom(PictureLayerTiling* pending_twin, const Region& layer_invalidation);
    ...
    {
        return IsTileRequiredForDraw(tile, [this](const Tile* tile) { return IsTileVisible(tile); });
    }
    ...
    const gfx::Rect& tiling_rect() const
    {
        return tiling_data_.tiling_rect();
    }
    const gfx::Rect& live_tiles_rect() const
    {
    ...
    const TilingData* tiling_data() const
    {
        return &tiling_data_;
    }
    ...
    bool all_tiles_done() const
    {
        return all_tiles_done_;
    }
    ...
    const TilingData& TilingDataForTesting() const
    {
        return tiling_data_;
    }
    ...
    std::map<const Tile*, PrioritizedTile> UpdateAndGetAllPrioritizedTilesForTesting() const;

    void SetAllTilesOccludedForTesting()
    {
        gfx::Rect viewport_in_layer_space = EnclosingLayerRectFromContentsRect(current_visible_rect_);
        current_occlusion_in_layer_space_
            = Occlusion(gfx::Transform(), SimpleEnclosedRegion(viewport_in_layer_space), SimpleEnclosedRegion(viewport_in_layer_space));
    }
    const gfx::Rect& GetCurrentVisibleRectForTesting() const
    {
        return current_visible_rect_;
    }
    void SetTilePriorityRectsForTesting(const gfx::Rect& visible_rect, const gfx::Rect& skewport_rect, const gfx::Rect& soon_border_rect,
        const gfx::Rect& eventually_rect, bool evicts_tiles = false);
    ...
    void ComputeTilePriorityRects(const gfx::Rect& visible_rect_in_layer_space, const gfx::Rect& skewport_rect_in_layer_space,
...
protected:
    ...
    enum PriorityRectType { VISIBLE_RECT, PENDING_VISIBLE_RECT, SKEWPORT_RECT, SOON_BORDER_RECT, EVENTUALLY_RECT };
    ...
    template <typename VisibilityChecker> bool IsTileRequiredForActivation(const Tile* tile, VisibilityChecker is_visible, bool is_tile_occluded) const
    {
        if (tree_ == PENDING_TREE) {
            if (!can_require_tiles_for_activation_ || resolution_ != HIGH_RESOLUTION || is_tile_occluded) {
                return false;
            }
            ...
            if (active_twin->raster_source()->size() != raster_source()->size()
                || active_twin->raster_source()->recorded_bounds() != raster_source()->recorded_bounds()) {
                return true;
            }
    ...
    template <typename VisibilityChecker> bool IsTileRequiredForDraw(const Tile* tile, VisibilityChecker is_visible) const
    {
        return tree_ == ACTIVE_TREE && resolution_ == HIGH_RESOLUTION && is_visible(tile) && !IsTileOccludedOnCurrentTree(tile);
    }
    ...
    void SetPriorityRect(const gfx::Rect& rect_in_layer_space, PriorityRectType rect_type, bool evicts_tiles = false);
    ...
    PrioritizedTile MakePrioritizedTile(Tile* tile, PriorityRectType priority_rect_type, bool is_tile_occluded) const;
    TilePriority ComputePriorityForTile(const Tile* tile, PriorityRectType priority_rect_type, bool is_tile_occluded) const;
    PriorityRectType ComputePriorityRectTypeForTile(const Tile* tile) const;
    bool has_visible_rect_tiles() const
    {
        return has_visible_rect_tiles_;
    }
    bool has_skewport_rect_tiles() const
    {
        return has_skewport_rect_tiles_;
    }
    bool has_soon_border_rect_tiles() const
    {
        return has_soon_border_rect_tiles_;
    }
    bool has_eventually_rect_tiles() const
    {
        return has_eventually_rect_tiles_;
    }
    ...
    {
        const PictureLayerTiling* pending_tiling = tree_ == ACTIVE_TREE ? client_->GetPendingOrActiveTwinTiling(this) : this;
        if (pending_tiling)
    ...
    gfx::Rect EnclosingContentsRectFromLayerRect(const gfx::Rect& layer_rect) const;
    gfx::Rect EnclosingLayerRectFromContentsRect(const gfx::Rect& contents_rect) const;
...
class PictureLayerTilingCoverageIterator : public TilingCoverageIterator<PictureLayerTiling> {
public:
