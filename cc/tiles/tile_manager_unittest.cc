...
public:
    bool PostDelayedTask(const base::Location& from_here, base::OnceClosure task, base::TimeDelta delay) override
    {
...
public:
    ...
    void Playback(const RasterSource* raster_source, const gfx::Rect& raster_full_rect, const gfx::Rect& raster_dirty_rect, uint64_t new_content_id,
        const gfx::AxisTransform2d& transform, const RasterSource::PlaybackSettings& playback_settings, const GURL& url) override
    {
...
TEST_F(TileManagerTilePriorityQueueTest, RasterTilePriorityQueue)
{
    ...
    std::unique_ptr<RasterTilePriorityQueue> queue(host_impl()->BuildRasterQueue(SAME_PRIORITY_FOR_BOTH_TREES, RasterTilePriorityQueue::Type::ALL));
    ...
    queue = host_impl()->BuildRasterQueue(SMOOTHNESS_TAKES_PRIORITY, RasterTilePriorityQueue::Type::ALL);
    ...
    queue = host_impl()->BuildRasterQueue(SMOOTHNESS_TAKES_PRIORITY, RasterTilePriorityQueue::Type::REQUIRED_FOR_ACTIVATION);
    ...
    queue = host_impl()->BuildRasterQueue(SMOOTHNESS_TAKES_PRIORITY, RasterTilePriorityQueue::Type::REQUIRED_FOR_DRAW);
    ...
    pending_layer()->picture_layer_tiling_set()->UpdateTilePriorities(viewport, 1.0f, 1.0, Occlusion(), true);
    active_layer()->picture_layer_tiling_set()->UpdateTilePriorities(viewport, 1.0f, 1.0, Occlusion(), true);
    ...
    while (!queue->IsEmpty()) {
        ...
        EXPECT_LE(last_tile.priority().priority_bin, prioritized_tile.priority().priority_bin);
    ...
    EXPECT_EQ(expected_required_for_activation_tiles, required_for_activation_tiles);
    ...
    queue = host_impl()->BuildRasterQueue(NEW_CONTENT_TAKES_PRIORITY, RasterTilePriorityQueue::Type::ALL);
    tile_count = 0;
    while (!queue->IsEmpty()) {
        PrioritizedTile prioritized_tile = queue->Top();
        EXPECT_TRUE(prioritized_tile.tile());
    ...
    // Check that we have consistent required_for_activation tiles.
    queue = host_impl()->BuildRasterQueue(NEW_CONTENT_TAKES_PRIORITY, RasterTilePriorityQueue::Type::REQUIRED_FOR_ACTIVATION);
    required_for_activation_tiles.clear();
    while (!queue->IsEmpty()) {
        PrioritizedTile prioritized_tile = queue->Top();
        EXPECT_TRUE(prioritized_tile.tile()->required_for_activation());
    ...
    // Check that we have consistent required_for_draw tiles.
    queue = host_impl()->BuildRasterQueue(NEW_CONTENT_TAKES_PRIORITY, RasterTilePriorityQueue::Type::REQUIRED_FOR_DRAW);
    required_for_draw_tiles.clear();
    while (!queue->IsEmpty()) {
        PrioritizedTile prioritized_tile = queue->Top();
        EXPECT_TRUE(prioritized_tile.tile()->required_for_draw());
...
TEST_F(TileManagerTilePriorityQueueTest, RasterTilePriorityQueueAll_GetNextQueues)
{
    ...
    auto* pending_child = AddLayer<FakePictureLayerImpl>(host_impl()->pending_tree(), pending_raster_source);
    ...
    host_impl()->active_tree()->SetDeviceViewportRect(gfx::Rect(100, 100, 200, 200));
    host_impl()->AdvanceToNextFrame(base::Milliseconds(1));
    ...
    {
        std::unique_ptr<RasterTilePriorityQueue> queue(host_impl()->BuildRasterQueue(SMOOTHNESS_TAKES_PRIORITY, RasterTilePriorityQueue::Type::ALL));
        EXPECT_FALSE(queue->IsEmpty());
        ...
        while (!queue->IsEmpty()) {
            ...
            TilePriority::PriorityBin priority_bin = prioritized_tile.priority().priority_bin;
            float distance_to_visible = prioritized_tile.priority().distance_to_visible;
    ...
    {
        std::unique_ptr<RasterTilePriorityQueue> queue(host_impl()->BuildRasterQueue(NEW_CONTENT_TAKES_PRIORITY, RasterTilePriorityQueue::Type::ALL));
        EXPECT_FALSE(queue->IsEmpty());
...
TEST_F(TileManagerTilePriorityQueueTest, RasterTilePriorityQueueHighNonIdealTilings)
{
    const gfx::Size layer_bounds(1000, 1000);
    ...
    pending_layer()->tilings()->AddTiling(gfx::AxisTransform2d(1.5f, gfx::Vector2dF()), pending_layer()->raster_source());
    active_layer()->tilings()->AddTiling(gfx::AxisTransform2d(1.5f, gfx::Vector2dF()), active_layer()->raster_source());
    pending_layer()->tilings()->AddTiling(gfx::AxisTransform2d(1.7f, gfx::Vector2dF()), pending_layer()->raster_source());
    active_layer()->tilings()->AddTiling(gfx::AxisTransform2d(1.7f, gfx::Vector2dF()), active_layer()->raster_source());

    pending_layer()->tilings()->UpdateTilePriorities(gfx::Rect(viewport), 1.f, 5.0, Occlusion(), true);
    active_layer()->tilings()->UpdateTilePriorities(gfx::Rect(viewport), 1.f, 5.0, Occlusion(), true);
...
TEST_F(TileManagerTilePriorityQueueTest, RasterTilePriorityQueueHighLowTilings)
{
    const gfx::Size layer_bounds(1000, 1000);
    const gfx::Size viewport(800, 800);
    host_impl()->active_tree()->SetDeviceViewportRect(gfx::Rect(viewport));
...
TEST_F(TileManagerTilePriorityQueueTest, RasterTilePriorityQueueInvalidation)
{
    ...
    EXPECT_NE(pending_layer()->HighResTiling()->TileAt(1, 0), active_layer()->HighResTiling()->TileAt(1, 0));
    ...
    for (int i = 0; i <= 1; ++i) {
        for (int j = 0; j <= 1; ++j) {
            ...
            if (pending_layer()->HighResTiling()->TileAt(i, j)) {
                expected_now_tiles.insert(pending_layer()->HighResTiling()->TileAt(i, j));
                expected_required_for_activation_tiles.insert(pending_layer()->HighResTiling()->TileAt(i, j));
    ...
    EXPECT_NE(expected_required_for_draw_tiles, expected_required_for_activation_tiles);

    std::set<Tile*> expected_all_tiles;
    for (int i = 0; i <= 3; ++i) {
        for (int j = 0; j <= 3; ++j) {
            if (pending_layer()->HighResTiling()->TileAt(i, j))
                expected_all_tiles.insert(pending_layer()->HighResTiling()->TileAt(i, j));
    ...
    queue = host_impl()->BuildRasterQueue(SAME_PRIORITY_FOR_BOTH_TREES, RasterTilePriorityQueue::Type::REQUIRED_FOR_DRAW);
    ...
    queue = host_impl()->BuildRasterQueue(SAME_PRIORITY_FOR_BOTH_TREES, RasterTilePriorityQueue::Type::REQUIRED_FOR_ACTIVATION);
    ...
    EXPECT_EQ(expected_required_for_activation_tiles, actual_required_for_activation_tiles);
}
...
TEST_F(TileManagerTilePriorityQueueTest, ActivationComesBeforeSoon)
{
    ...
    tile_manager()->InitializeTilesWithResourcesForTesting(std::vector<Tile*>(all_tiles.begin(), all_tiles.end()));
...
TEST_F(TileManagerTilePriorityQueueTest, EvictionTilePriorityQueue)
{
    ...
    std::unique_ptr<EvictionTilePriorityQueue> empty_queue(host_impl()->BuildEvictionQueue(SAME_PRIORITY_FOR_BOTH_TREES));
    EXPECT_TRUE(empty_queue->IsEmpty());
    std::set<Tile*> all_tiles;
    size_t tile_count = 0;

    std::unique_ptr<RasterTilePriorityQueue> raster_queue(host_impl()->BuildRasterQueue(SAME_PRIORITY_FOR_BOTH_TREES, RasterTilePriorityQueue::Type::ALL));
    ...
    std::unique_ptr<EvictionTilePriorityQueue> queue(host_impl()->BuildEvictionQueue(SMOOTHNESS_TAKES_PRIORITY));
    EXPECT_FALSE(queue->IsEmpty());
    ...
    tile_manager()->ReleaseTileResourcesForTesting(std::vector<Tile*>(all_tiles.begin(), all_tiles.end()));
    ...
    while (!queue->IsEmpty()) {
        ...
        if (last_priority.priority_bin == priority.priority_bin) {
            EXPECT_LE(last_tile.tile()->required_for_activation(), tile->required_for_activation());
...
TEST_F(TileManagerTilePriorityQueueTest, DebugNameAppearsInMemoryDump)
{
    ...
    scoped_refptr<FakeRasterSource> pending_raster_source_1 = FakeRasterSource::CreateFilledWithText(layer_bounds);
    scoped_refptr<FakeRasterSource> pending_raster_source_2 = FakeRasterSource::CreateFilledWithText(layer_bounds);

    SetupPendingTree(pending_raster_source_1);

    auto* pending_child_layer = AddLayer<FakePictureLayerImpl>(host_impl()->pending_tree());
    ...
    base::trace_event::MemoryDumpArgs dump_args = { base::trace_event::MemoryDumpLevelOfDetail::kDetailed };
...
TEST_F(TileManagerTilePriorityQueueTest, EvictionTilePriorityQueueWithOcclusion)
{
    host_impl()->AdvanceToNextFrame(base::Milliseconds(1));
    ...
    scoped_refptr<FakeRasterSource> pending_raster_source = FakeRasterSource::CreateFilledWithText(layer_bounds);
    SetupPendingTree(pending_raster_source);

    auto* pending_child_layer = AddLayer<FakePictureLayerImpl>(host_impl()->pending_tree(), pending_raster_source);
    ...
    FakePictureLayerImpl* active_child_layer = static_cast<FakePictureLayerImpl*>(host_impl()->active_tree()->LayerById(child_id));
    ...
    pending_layer()->picture_layer_tiling_set()->UpdateTilePriorities(viewport, 1.0f, 1.0, Occlusion(), true);
    pending_child_layer->picture_layer_tiling_set()->UpdateTilePriorities(viewport, 1.0f, 1.0, Occlusion(), true);

    active_layer()->picture_layer_tiling_set()->UpdateTilePriorities(viewport, 1.0f, 1.0, Occlusion(), true);
    active_child_layer->picture_layer_tiling_set()->UpdateTilePriorities(viewport, 1.0f, 1.0, Occlusion(), true);

    // Populate all tiles directly from the tilings.
    all_tiles.clear();
    std::vector<Tile*> pending_high_res_tiles = pending_layer()->HighResTiling()->AllTilesForTesting();
    all_tiles.insert(pending_high_res_tiles.begin(), pending_high_res_tiles.end());
    ...
    PrioritizedTile last_tile;
    std::unique_ptr<EvictionTilePriorityQueue> queue(host_impl()->BuildEvictionQueue(tree_priority));
    while (!queue->IsEmpty()) {
        PrioritizedTile prioritized_tile = queue->Top();
        if (!last_tile.tile())
            last_tile = prioritized_tile;
        ...
        if (tile_is_occluded) {
            ...
            if (!last_tile_is_occluded) {
                ...
                EXPECT_TRUE((tile_priority_bin < last_tile_priority_bin) || prioritized_tile.tile()->required_for_activation()
                    || (prioritized_tile.tile()->raster_transform() != last_tile.tile()->raster_transform()));
            }
        }
        last_tile = prioritized_tile;
        queue->Pop();
...
TEST_F(TileManagerTilePriorityQueueTest, EvictionTilePriorityQueueWithTransparentLayer)
{
    host_impl()->AdvanceToNextFrame(base::Milliseconds(1));
    ...
    auto* pending_child_layer = AddLayer<FakePictureLayerImpl>(host_impl()->pending_tree(), pending_raster_source);
    pending_child_layer->SetElementId(LayerIdToElementIdForTesting(pending_child_layer->id()));
    CopyProperties(pending_layer(), pending_child_layer);

    // Create a fully transparent child layer so that its tile priorities are not
    // considered to be valid.
    pending_child_layer->SetDrawsContent(true);
    CreateEffectNode(pending_child_layer).render_surface_reason = RenderSurfaceReason::kTest;

    host_impl()->AdvanceToNextFrame(base::Milliseconds(1));
    UpdateDrawProperties(host_impl()->pending_tree());

    host_impl()->pending_tree()->SetOpacityMutated(pending_child_layer->element_id(), 0.0f);
    ...
    std::vector<Tile*> pending_high_res_tiles = pending_layer()->HighResTiling()->AllTilesForTesting();
    all_pending_tiles.insert(pending_high_res_tiles.begin(), pending_high_res_tiles.end());
    EXPECT_EQ(16u, pending_high_res_tiles.size());

    std::set<Tile*> all_pending_child_tiles;
    std::vector<Tile*> pending_child_high_res_tiles = pending_child_layer->HighResTiling()->AllTilesForTesting();
    all_pending_child_tiles.insert(pending_child_high_res_tiles.begin(), pending_child_high_res_tiles.end());
    EXPECT_EQ(16u, pending_child_high_res_tiles.size());

    std::set<Tile*> all_tiles = all_pending_tiles;
    all_tiles.insert(all_pending_child_tiles.begin(), all_pending_child_tiles.end());
    ...
    while (!queue->IsEmpty()) {
        ...
        EXPECT_NE(std::numeric_limits<float>::infinity(), pending_priority.distance_to_visible);
...
TEST_F(TileManagerTilePriorityQueueTest, RasterTilePriorityQueueEmptyLayers)
{
    ...
    queue = host_impl()->BuildRasterQueue(SAME_PRIORITY_FOR_BOTH_TREES, RasterTilePriorityQueue::Type::ALL);
    EXPECT_FALSE(queue->IsEmpty());
...
TEST_F(TileManagerTilePriorityQueueTest, EvictionTilePriorityQueueEmptyLayers)
{
    ...
    std::unique_ptr<EvictionTilePriorityQueue> queue(host_impl()->BuildEvictionQueue(SAME_PRIORITY_FOR_BOTH_TREES));
    EXPECT_FALSE(queue->IsEmpty());
...
TEST_F(TileManagerTilePriorityQueueTest, RasterTilePriorityQueueStaticViewport)
{
    ...
    std::unique_ptr<PictureLayerTilingSet> tiling_set = PictureLayerTilingSet::Create(ACTIVE_TREE, &client, settings.tiling_interest_area_padding,
        settings.skewport_target_time_in_seconds, settings.skewport_extrapolation_limit_in_screen_pixels, settings.max_preraster_distance_in_screen_pixels);

    scoped_refptr<FakeRasterSource> raster_source = FakeRasterSource::CreateFilled(layer_bounds);
    PictureLayerTiling* tiling = tiling_set->AddTiling(gfx::AxisTransform2d(), raster_source);
    tiling->set_resolution(HIGH_RESOLUTION);
    ...
    for (int i = 0; i < 3; ++i) {
        std::unique_ptr<TilingSetRasterQueueAll> queue = TilingSetRasterQueueAll::Create(tiling_set.get(), false, false);
        EXPECT_TRUE(queue);
        ...
        while (!queue->IsEmpty()) {
            ...
            if (last_priority.priority_bin == new_priority.priority_bin) {
                if (last_priority.priority_bin == TilePriority::EVENTUALLY) {
                    ...
                    EXPECT_LE(last_priority.distance_to_visible, new_priority.distance_to_visible);
        ...
        EXPECT_GT(eventually_bin_order_correct_count, eventually_bin_order_incorrect_count);
...
TEST_F(TileManagerTilePriorityQueueTest, RasterTilePriorityQueueMovingViewport)
{
    FakePictureLayerTilingClient client;
...
TEST_F(TileManagerTilePriorityQueueTest, SetIsLikelyToRequireADrawOnZeroMemoryBudget)
{
    const gfx::Size layer_bounds(1000, 1000);
    host_impl()->active_tree()->SetDeviceViewportRect(gfx::Rect(layer_bounds));
    SetupDefaultTrees(layer_bounds);
...
TEST_F(TileManagerTilePriorityQueueTest, SetIsLikelyToRequireADrawOnLimitedMemoryBudget)
{
    const gfx::Size layer_bounds(1000, 1000);
    host_impl()->active_tree()->SetDeviceViewportRect(gfx::Rect(layer_bounds));
    SetupDefaultTrees(layer_bounds);
    ...
    ManagedMemoryPolicy policy = host_impl()->ActualManagedMemoryPolicy();
    policy.bytes_limit_when_visible = viz::SinglePlaneFormat::kRGBA_8888.EstimatedSizeInBytes(gfx::Size(256, 256));
    host_impl()->SetMemoryPolicy(policy);
    ...
    ResourcePool::InUsePoolResource resource
        = host_impl()->resource_pool()->AcquireResource(gfx::Size(256, 256), viz::SinglePlaneFormat::kRGBA_8888, gfx::ColorSpace());
...
TEST_F(TileManagerTilePriorityQueueTest, DefaultMemoryPolicy)
{
    ...
    EXPECT_EQ(67108864u, host_impl()->global_tile_state().hard_memory_limit_in_bytes);
    EXPECT_EQ(TileMemoryLimitPolicy::ALLOW_ANYTHING, host_impl()->global_tile_state().memory_limit_policy);
    EXPECT_EQ(ManagedMemoryPolicy::kDefaultNumResourcesLimit, host_impl()->global_tile_state().num_resources_limit);
}
...
TEST_F(TileManagerTilePriorityQueueTest, RasterQueueAllUsesCorrectTileBounds)
{
    ...
    std::unique_ptr<PictureLayerTilingSet> tiling_set = PictureLayerTilingSet::Create(WhichTree::ACTIVE_TREE, &pending_client, 1.0f, 1.0f, 1000, 1000.f);
    ...
    {
        tiling->SetTilePriorityRectsForTesting(non_intersecting_rect, // Visible rect.
...
TEST_F(TileManagerTilePriorityQueueTest, NoRasterTasksforSolidColorTiles)
{
    ...
    recording_source.add_draw_rect_with_flags(gfx::Rect(layer_bounds), solid_flags);
    ...
    recording_source.add_draw_rect_with_flags(gfx::Rect(0, 0, 10, 10), non_solid_flags);
    ...
    PictureLayerTilingSet* tiling_set = pending_layer_->picture_layer_tiling_set();
...
public:
    ...
    static constexpr viz::SharedImageFormat kSharedImageFormat = viz::SinglePlaneFormat::kRGBA_8888;

    std::unique_ptr<RasterBuffer> AcquireBufferForRaster(const ResourcePool::InUsePoolResource& resource, uint64_t resource_content_id,
        ...
        bool depends_on_hardware_accelerated_webp_candidates) override
    {
        if (!resource.software_backing()) {
            ...
            backing->pixels = std::make_unique<uint32_t[]>(viz::ResourceSizes::CheckedSizeInBytes<size_t>(resource.size(), kSharedImageFormat));
            resource.set_software_backing(std::move(backing));
        }
        auto* backing = static_cast<TestSoftwareBacking*>(resource.software_backing());
        return std::make_unique<TestRasterBuffer>(resource.size(), backing->pixels.get());
    }
...
private:
    ...
    public:
        ...
        void Playback(const RasterSource* raster_source, const gfx::Rect& raster_full_rect, const gfx::Rect& raster_dirty_rect, uint64_t new_content_id,
            const gfx::AxisTransform2d& transform, const RasterSource::PlaybackSettings& playback_settings, const GURL& url) override
        {
            RasterBufferProvider::PlaybackToMemory(pixels_, kSharedImageFormat, size_, /*stride=*/0, raster_source, raster_full_rect,
                /*canvas_playback_rect=*/raster_full_rect, transform, gfx::ColorSpace(), kIsGpuCompositing, playback_settings);
        }
...
public:
    ...
    public:
        MockLayerTreeHostImpl(const LayerTreeSettings& settings, TaskRunnerProvider* task_runner_provider, TaskGraphRunner* task_graph_runner)
            : FakeLayerTreeHostImpl(settings, task_runner_provider, task_graph_runner)
        {
        }
    ...
    std::unique_ptr<FakeLayerTreeHostImpl> CreateHostImpl(
        const LayerTreeSettings& settings, TaskRunnerProvider* task_runner_provider, TaskGraphRunner* task_graph_runner) override
    {
        return std::make_unique<testing::NiceMock<MockLayerTreeHostImpl>>(settings, task_runner_provider, task_graph_runner);
    }
...
TEST_F(TileManagerTest, AllWorkFinished)
{
    // Check with no tile work enqueued.
    {
        ...
        EXPECT_CALL(MockHostImpl(), NotifyAllTileTasksCompleted()).WillOnce(testing::Invoke([&run_loop]() { run_loop.Quit(); }));
        host_impl()->tile_manager()->PrepareTiles(host_impl()->global_tile_state());
...
public:
    ...
    void PrepareTilesAndWaitUntilDone(const GlobalStateThatImpactsTilePriority& state)
    {
        base::RunLoop run_loop;
        EXPECT_CALL(MockHostImpl(), NotifyAllTileTasksCompleted()).WillOnce(testing::Invoke([&run_loop]() { run_loop.Quit(); }));
...
TEST_F(TileManagerOcclusionTest, OccludedTileEvictedForVisibleTile)
{
    ...
    GlobalStateThatImpactsTilePriority global_state = host_impl()->global_tile_state();
    const LayerTreeSettings settings = CreateSettings();
    global_state.hard_memory_limit_in_bytes = global_state.soft_memory_limit_in_bytes = settings.default_tile_size.GetArea() * 4 + 1;
    ...
    // resource.
    {
        PictureLayerImpl* initial_layer = static_cast<PictureLayerImpl*>(host_impl()->active_tree()->LayerById(initial_picture_id));
        ASSERT_NE(initial_layer, nullptr);
        ASSERT_EQ(1u, initial_layer->picture_layer_tiling_set()->num_tilings());
        std::vector<Tile*> initial_layer_tiles = initial_layer->picture_layer_tiling_set()->tiling_at(0)->AllTilesForTesting();
    ...
    auto* picture_layer = AddLayer<FakePictureLayerImpl>(host_impl()->pending_tree(), FakeRasterSource::CreateFilledWithText(layer_bounds));
    ...
    PrepareTilesAndWaitUntilDone(global_state);
    {
        ...
        PictureLayerImpl* top_most_layer = static_cast<PictureLayerImpl*>(host_impl()->active_tree()->LayerById(top_most_layer_id));
        ASSERT_NE(top_most_layer, nullptr);
        ASSERT_EQ(1u, top_most_layer->picture_layer_tiling_set()->num_tilings());
        std::vector<Tile*> top_most_layer_tiles = top_most_layer->picture_layer_tiling_set()->tiling_at(0)->AllTilesForTesting();
...
public:
    void SetUp() override
    {
        ...
        host_impl()->tile_manager()->SetRasterBufferProviderForTesting(&raster_buffer_provider_);
    }
...
TEST_F(PixelInspectTileManagerTest, LowResHasNoImage)
{
    ...
    for (size_t i = 0; i < std::size(resolutions); ++i) {
        ...
        sk_sp<SkSurface> surface = SkSurfaces::Raster(SkImageInfo::MakeN32Premul(size.width(), size.height()));
        ...
        std::unique_ptr<PictureLayerImpl> layer = PictureLayerImpl::Create(host_impl()->pending_tree(), 1);
        ...
        SkColorType ct = ToClosestSkColorType(TestSoftwareRasterBufferProvider::kIsGpuCompositing, TestSoftwareRasterBufferProvider::kSharedImageFormat);
        auto info = SkImageInfo::Make(resource_size.width(), resource_size.height(), ct, kPremul_SkAlphaType);
        // CreateLayerTreeFrameSink() sets up a software compositing, so the
        // tile resource will be a bitmap.
        auto* backing = static_cast<TestSoftwareBacking*>(tile->draw_info().GetResource().software_backing());
        SkBitmap bitmap;
...
TEST_F(ActivationTasksDoNotBlockReadyToDrawTest, ActivationTasksDoNotBlockReadyToDraw)
{
    const gfx::Size layer_bounds(1000, 1000);
    ...
    solid_flags.setColor(solid_color);
    active_tree_recording_source.add_draw_rect_with_flags(gfx::Rect(layer_bounds), solid_flags);
    ...
    pending_tree_recording_source.add_draw_rect_with_flags(gfx::Rect(5, 5, 10, 10), non_solid_flags);
    pending_tree_recording_source.Rerecord();

    scoped_refptr<RasterSource> active_tree_raster_source = active_tree_recording_source.CreateRasterSource();
    scoped_refptr<RasterSource> pending_tree_raster_source = pending_tree_recording_source.CreateRasterSource();
    ...
    base::RunLoop run_loop;
    EXPECT_CALL(MockHostImpl(), NotifyReadyToDraw()).WillOnce(testing::Invoke([&run_loop]() { run_loop.Quit(); }));
    static_cast<SynchronousTaskGraphRunner*>(task_graph_runner())->RunSingleTaskForTesting();
    run_loop.Run();
}
...
TEST_F(PartialRasterTileManagerTest, CancelledTasksHaveNoContentId)
{
    ...
    host_impl()->tile_manager()->SetTileTaskManagerForTesting(std::make_unique<FakeTileTaskManagerImpl>());
    ...
    pending_tree->SetDeviceViewportRect(host_impl()->active_tree()->GetDeviceViewport());
    ...
    EXPECT_FALSE(host_impl()->resource_pool()->TryAcquireResourceForPartialRaster(
        kInvalidatedId + 1, gfx::Rect(), kInvalidatedId, &total_invalidated_rect, gfx::ColorSpace::CreateSRGB()));
...
// tasks.
class VerifyResourceContentIdRasterBufferProvider : public FakeRasterBufferProviderImpl {
public:
    explicit VerifyResourceContentIdRasterBufferProvider(uint64_t expected_content_id)
...
void RunPartialRasterCheck(std::unique_ptr<LayerTreeHostImpl> host_impl, bool partial_raster_enabled)
{
    // Pick arbitrary IDs - they don't really matter as long as they're constant.
    const int kLayerId = 7;
    const uint64_t kInvalidatedId = 43;
    ...
    VerifyResourceContentIdRasterBufferProvider raster_buffer_provider(kExpectedId);
    host_impl->tile_manager()->SetRasterBufferProviderForTesting(&raster_buffer_provider);

    // Ensure there's a resource with our |kInvalidatedId| in the resource pool.
    ResourcePool::InUsePoolResource resource
        = host_impl->resource_pool()->AcquireResource(kTileSize, viz::SinglePlaneFormat::kRGBA_8888, gfx::ColorSpace::CreateSRGB());

    resource.set_software_backing(std::make_unique<TestSoftwareBacking>());
    host_impl->resource_pool()->PrepareForExport(resource, viz::TransferableResource::ResourceSource::kTest);
...
void RunPartialTileDecodeCheck(std::unique_ptr<LayerTreeHostImpl> host_impl, bool partial_raster_enabled)
{
    // Pick arbitrary IDs - they don't really matter as long as they're constant.
    const int kLayerId = 7;
    const uint64_t kInvalidatedId = 43;
    ...
    int dimension = 250;
    PaintImage image1 = CreateDiscardablePaintImage(gfx::Size(dimension, dimension));
    PaintImage image2 = CreateDiscardablePaintImage(gfx::Size(dimension, dimension));
    PaintImage image3 = CreateDiscardablePaintImage(gfx::Size(dimension, dimension));
    PaintImage image4 = CreateDiscardablePaintImage(gfx::Size(dimension, dimension));
    ...
    scoped_refptr<FakeRasterSource> pending_raster_source = FakeRasterSource::CreateFromRecordingSource(recording_source);
    ...
    // queue will contain dangling pointers, so wrap it in a scope.
    {
        ...
        EXPECT_EQ(host_impl->tile_manager()->decode_tasks_for_testing(tile->id()).size(), partial_raster_enabled ? 1u : 4u);
    }
...
{
    RunPartialTileDecodeCheck(TakeHostImpl(), false /* partial_raster_disabled */);
}
...
TEST_F(InvalidResourceTileManagerTest, InvalidResource)
{
    ...
    auto* tiling = active_layer()->picture_layer_tiling_set()->AddTiling(gfx::AxisTransform2d(), std::move(raster));
    tiling->set_resolution(HIGH_RESOLUTION);
    tiling->CreateAllTilesForTesting(gfx::Rect(size));
...
// functionality.
class MockReadyToDrawRasterBufferProviderImpl : public FakeRasterBufferProviderImpl {
public:
    MOCK_METHOD1(IsResourceReadyToDraw, bool(const ResourcePool::InUsePoolResource& resource));
    MOCK_METHOD3(SetReadyToDrawCallback,
        uint64_t(const std::vector<const ResourcePool::InUsePoolResource*>& resources, base::OnceClosure callback, uint64_t pending_callback_id));
...
public:
    ...
    {
        TileManagerTest::SetUp();
        host_impl()->tile_manager()->SetRasterBufferProviderForTesting(&mock_raster_buffer_provider_);

        PaintFlags solid_flags;
        SkColor solid_color = SkColorSetARGB(255, 12, 23, 34);
        solid_flags.setColor(solid_color);
        solid_color_recording_source_.add_draw_rect_with_flags(gfx::Rect(kLayerBounds), solid_flags);
        ...
        for (int i = 0; i < 100; ++i) {
            for (int j = 0; j < 100; ++j) {
                recording_source_.add_draw_rect_with_flags(gfx::Rect(10 * i, 10 * j, 5, 5), non_solid_flags);
            }
        }
    ...
    {
        scoped_refptr<RasterSource> active_tree_raster_source = recording_source_.CreateRasterSource();
        scoped_refptr<RasterSource> pending_tree_raster_source = solid_color_recording_source_.CreateRasterSource();
    ...
    {
        scoped_refptr<RasterSource> active_tree_raster_source = solid_color_recording_source_.CreateRasterSource();
        scoped_refptr<RasterSource> pending_tree_raster_source = recording_source_.CreateRasterSource();
...
private:
    StrictMock<MockReadyToDrawRasterBufferProviderImpl> mock_raster_buffer_provider_;
...
TEST_F(TileManagerReadyToDrawTest, SmoothActivationWaitsOnCallback)
{
    ...
    base::OnceClosure callback;
    {
        ...
        // ready to draw.
        EXPECT_CALL(*mock_raster_buffer_provider(), IsResourceReadyToDraw(testing::_)).WillRepeatedly(Return(false));

        EXPECT_CALL(*mock_raster_buffer_provider(), SetReadyToDrawCallback(_, _, 0))
            .WillOnce([&run_loop, &callback](
                          const std::vector<const ResourcePool::InUsePoolResource*>& resources, base::OnceClosure callback_in, uint64_t pending_callback_id) {
    ...
    {
        base::RunLoop run_loop;
        EXPECT_CALL(MockHostImpl(), NotifyReadyToActivate()).WillOnce(Invoke([&run_loop]() { run_loop.Quit(); }));
        EXPECT_CALL(*mock_raster_buffer_provider(), IsResourceReadyToDraw(testing::_)).WillRepeatedly(Return(true));
...
TEST_F(TileManagerReadyToDrawTest, NonSmoothDrawDoesNotWaitOnCallback)
{
    ...
    host_impl()->tile_manager()->PrepareTiles(host_impl()->global_tile_state());
    EXPECT_CALL(MockHostImpl(), NotifyReadyToDraw()).WillOnce(Invoke([&run_loop]() { run_loop.Quit(); }));
    run_loop.Run();
...
TEST_F(TileManagerReadyToDrawTest, NoCallbackWhenAlreadyReadyToDraw)
{
    ...
    host_impl()->tile_manager()->PrepareTiles(host_impl()->global_tile_state());
    EXPECT_CALL(MockHostImpl(), NotifyReadyToActivate()).WillOnce(Invoke([&run_loop]() { run_loop.Quit(); }));
    EXPECT_CALL(*mock_raster_buffer_provider(), IsResourceReadyToDraw(_)).WillRepeatedly(Return(true));
    run_loop.Run();
...
TEST_F(TileManagerReadyToDrawTest, TilePrioritiesUpdated)
{
    ...
    // finish.
    {
        ...
        // ready to draw.
        EXPECT_CALL(*mock_raster_buffer_provider(), IsResourceReadyToDraw(testing::_)).WillRepeatedly(Return(false));
        EXPECT_CALL(*mock_raster_buffer_provider(), SetReadyToDrawCallback(_, _, _)).WillRepeatedly(Return(1));
        host_impl()->tile_manager()->PrepareTiles(host_impl()->global_tile_state());
        run_loop.Run();
    }
...
TEST_F(TileManagerReadyToDrawTest, PrepaintTilesAreDroppedWhenIdle)
{
    ...
    base::test::ScopedFeatureList scoped_feature_list { features::kReclaimPrepaintTilesWhenIdle };

    auto task_runner = base::MakeRefCounted<base::TestMockTimeTaskRunner>();
    host_impl()->tile_manager()->SetOverridesForTesting(task_runner, task_runner->GetMockTickClock());
    ...
    base::RunLoop run_loop;
    host_impl()->tile_manager()->PrepareTiles(host_impl()->global_tile_state());
    EXPECT_CALL(MockHostImpl(), NotifyAllTileTasksCompleted()).WillOnce(Invoke([&run_loop]() { run_loop.Quit(); }));
    run_loop.Run();

    int prepaint_tiles = count_prepaint_tiles(host_impl()->tile_manager()->AllTilesForTesting());
    EXPECT_GT(prepaint_tiles, 0);
    EXPECT_TRUE(task_runner->HasPendingTask());

    // Too soon.
    task_runner->FastForwardBy(TileManager::kDelayBeforeTimeReclaim / 2);
    prepaint_tiles = count_prepaint_tiles(host_impl()->tile_manager()->AllTilesForTesting());
    EXPECT_GT(prepaint_tiles, 0);
    EXPECT_TRUE(task_runner->HasPendingTask());
...
TEST_F(TileManagerReadyToDrawTest, PrepaintTilesAreNotDropped)
{
    ...
    scoped_feature_list.InitAndDisableFeature(features::kReclaimPrepaintTilesWhenIdle);
    ...
    int before_prepaint_tiles = count_prepaint_tiles(host_impl()->tile_manager()->AllTilesForTesting());
...
TEST_F(TileManagerReadyToDrawTest, PrepaintTilesContinuousIdleTime)
{
    ...
    int after_prepaint_tiles = count_prepaint_tiles(host_impl()->tile_manager()->AllTilesForTesting());
    EXPECT_EQ(after_prepaint_tiles, before_prepaint_tiles);

    task_runner->FastForwardBy(TileManager::kDelayBeforeTimeReclaim / 2);
    EXPECT_EQ(0, count_prepaint_tiles(host_impl()->tile_manager()->AllTilesForTesting()));
...
void UpdateVisibleRect(FakePictureLayerImpl* layer, const gfx::Rect visible_rect)
{
    ...
    for (size_t j = 0; j < tiling_set->num_tilings(); ++j) {
        ...
        tiling->SetTilePriorityRectsForTesting(visible_rect, // Visible rect.
...
PaintImage MakeCheckerablePaintImage(const gfx::Size& size)
{
    ...
    return PaintImageBuilder::WithCopy(image).set_decoding_mode(PaintImage::DecodingMode::kAsync).TakePaintImage();
}
...
public:
    ...
    public:
        ...
        MOCK_METHOD4(GetPixels, bool(SkPixmap, size_t, PaintImage::GeneratorClientId, uint32_t));
    };
...
TEST_F(CheckerImagingTileManagerTest, NoImageDecodeDependencyForCheckeredTiles)
{
    ...
    auto generator = sk_make_sp<testing::StrictMock<MockImageGenerator>>(gfx::Size(512, 512));
    ...
    PictureLayerTilingSet* tiling_set = pending_layer()->picture_layer_tiling_set();
...
TEST_F(EmptyCacheTileManagerTest, AtRasterPrepaintTileRasterTasksSkipped)
{
    ...
    pending_tiling->SetTilePriorityRectsForTesting(gfx::Rect(), // An empty visual rect leads to the tile being pre-paint.
...
TEST_F(CheckerImagingTileManagerTest, BuildsImageDecodeQueueAsExpected)
{
    ...
    int dimension = 450;
    PaintImage image1 = MakeCheckerablePaintImage(gfx::Size(dimension, dimension));
    PaintImage image2 = MakeCheckerablePaintImage(gfx::Size(dimension, dimension));
    PaintImage image3 = MakeCheckerablePaintImage(gfx::Size(dimension, dimension));
    recording_source.add_draw_image(image1, gfx::Point(0, 0));
    ...
    gfx::Rect empty_rect;
    PictureLayerTiling* active_tiling = active_layer()->picture_layer_tiling_set()->tiling_at(0);
    active_tiling->SetTilePriorityRectsForTesting(gfx::Rect(empty_rect), // Visible rect.
    ...
    active_tiling->SetTilePriorityRectsForTesting(gfx::Rect(rect_to_raster), // Visible rect.
    ...
    active_tiling->SetTilePriorityRectsForTesting(gfx::Rect(layer_bounds), // Visible rect.
        gfx::Rect(layer_bounds), // Skewport rect.
        gfx::Rect(layer_bounds), // Soon rect.
        gfx::Rect(layer_bounds)); // Eventually rect.
...
TEST_F(CheckerImagingTileManagerTest, TileManagerCleanupClearsCheckerImagedDecodes)
{
    const gfx::Size layer_bounds(512, 512);
    ...
    SetupPendingTree(raster_source, gfx::Size(100, 100), Region(gfx::Rect(0, 0, 500, 500)));
    ...
    EXPECT_TRUE(host_impl()->tile_manager()->checker_image_tracker().has_locked_decodes_for_testing());

    host_impl()->pending_tree()->ReleaseTileResources();
    CleanUpTileManager();

    EXPECT_FALSE(host_impl()->tile_manager()->checker_image_tracker().has_locked_decodes_for_testing());
    EXPECT_TRUE(host_impl()->tile_manager()->TakeImagesToInvalidateOnSyncTree().empty());
}

TEST_F(CheckerImagingTileManagerTest, TileManagerCorrectlyPrioritizesCheckerImagedDecodes)
{
    ...
    host_impl()->tile_manager()->PrepareTiles(host_impl()->global_tile_state());
    EXPECT_TRUE(host_impl()->tile_manager()->HasScheduledTileTasksForTesting());
    EXPECT_TRUE(host_impl()->tile_manager()->checker_image_tracker().no_decodes_allowed_for_testing());
    while (!host_impl()->client()->ready_to_activate()) {
        static_cast<SynchronousTaskGraphRunner*>(task_graph_runner())->RunSingleTaskForTesting();
        base::RunLoop().RunUntilIdle();
    }
    EXPECT_EQ(host_impl()->tile_manager()->checker_image_tracker().decode_priority_allowed_for_testing(), CheckerImageTracker::DecodeType::kRaster);

    // Finishing all tasks allows pre-decodes.
    static_cast<SynchronousTaskGraphRunner*>(task_graph_runner())->RunUntilIdle();
    base::RunLoop().RunUntilIdle();
    EXPECT_EQ(host_impl()->tile_manager()->checker_image_tracker().decode_priority_allowed_for_testing(), CheckerImageTracker::DecodeType::kPreDecode);

    // Required for draw tiles block checker-imaged decodes.
    // Free all tile resources and perform another PrepareTiles.
    ActivateTree();
    EXPECT_TRUE(host_impl()->tile_manager()->IsReadyToDraw());
    host_impl()->tile_manager()->PrepareTiles(GlobalStateThatImpactsTilePriority());
    EXPECT_FALSE(host_impl()->tile_manager()->IsReadyToDraw());
...
class CheckerImagingTileManagerMemoryTest : public CheckerImagingTileManagerTest {
public:
    std::unique_ptr<FakeLayerTreeHostImpl> CreateHostImpl(
        const LayerTreeSettings& settings, TaskRunnerProvider* task_runner_provider, TaskGraphRunner* task_graph_runner) override
    {
        ...
        return CheckerImagingTileManagerTest::CreateHostImpl(new_settings, task_runner_provider, task_graph_runner);
    }
};
...
TEST_F(CheckerImagingTileManagerMemoryTest, AddsAllNowTilesToImageDecodeQueue)
{
    ...
    PictureLayerTiling* active_tiling = active_layer()->picture_layer_tiling_set()->tiling_at(0);
    active_tiling->SetTilePriorityRectsForTesting(complete_tiling_rect, // Visible rect.
...
public:
    ...
    ~VerifyImageProviderRasterBuffer() override
    {
        EXPECT_TRUE(did_raster_);
    }
...
class VerifyImageProviderRasterBufferProvider : public FakeRasterBufferProviderImpl {
public:
...
TEST_F(DecodedImageTrackerTileManagerTest, DecodedImageTrackerDropsLocksOnUse)
{
    ...
    host_impl()->tile_manager()->decoded_image_tracker().QueueImageDecode(image1, TargetColorParams(), base::DoNothing());
    host_impl()->tile_manager()->decoded_image_tracker().QueueImageDecode(image2, TargetColorParams(), base::DoNothing());
    EXPECT_EQ(0u, host_impl()->tile_manager()->decoded_image_tracker().NumLockedImagesForTesting());
    FlushDecodeTasks();
    EXPECT_EQ(2u, host_impl()->tile_manager()->decoded_image_tracker().NumLockedImagesForTesting());
...
public:
    void DecodeHdrImage(const gfx::ColorSpace& output_cs)
    {
        ...
        host_impl()->tile_manager()->decoded_image_tracker().QueueImageDecode(hdr_image, target_color_params, base::DoNothing());
        FlushDecodeTasks();
        ...
        if (output_cs.IsHDR()) {
            ...
            const auto& pending_tasks = host_impl()->tile_manager()->decode_tasks_for_testing(pending_tiles.back()->id());
        ...
        static_cast<SynchronousTaskGraphRunner*>(task_graph_runner())->RunUntilIdle();
        base::RunLoop().RunUntilIdle();
        ASSERT_FALSE(host_impl()->tile_manager()->HasScheduledTileTasksForTesting());

        auto expected_format = output_cs.IsHDR() ? viz::SinglePlaneFormat::kRGBA_F16 : viz::SinglePlaneFormat::kRGBA_8888;
        auto all_tiles = host_impl()->tile_manager()->AllTilesForTesting();
        for (const auto* tile : all_tiles)
            EXPECT_EQ(expected_format, tile->draw_info().resource_shared_image_format());
    }
};
...
public:
    ...
    {
        TileManagerTest::SetUp();
        host_impl()->tile_manager()->SetPendingRasterQueriesForTesting(&pending_raster_queries_);
    }

protected:
    class MockRasterQueryQueue : public FakeRasterQueryQueue {
    public:
        explicit MockRasterQueryQueue(viz::RasterContextProvider* const worker_context_provider)
...
TEST_F(TileManagerCheckRasterQueriesTest, ChecksRasterQueriesInAllTilesDoneTask)
{
    base::RunLoop run_loop;
    EXPECT_FALSE(host_impl()->tile_manager()->HasScheduledTileTasksForTesting());
    EXPECT_CALL(MockHostImpl(), NotifyAllTileTasksCompleted()).WillOnce(testing::Invoke([&run_loop]() { run_loop.Quit(); }));
...
public:
    void SetUp() override
    {
        ...
        host_impl()->tile_manager()->SetOverridesForTesting(task_runner_, task_runner_->GetMockTickClock());
...
{
    base::test::ScopedFeatureList scoped_feature_list { features::kReclaimOldPrepaintTiles };

    // Set a small viewport, so we have soon and eventually tiles.
    gfx::Rect viewport { 100, 100, 100, 100 };

    GlobalStateThatImpactsTilePriority global_tile_state = host_impl()->global_tile_state();
    ASSERT_EQ(global_tile_state.memory_limit_policy, TileMemoryLimitPolicy::ALLOW_ANYTHING);
    host_impl()->active_tree()->SetDeviceViewportRect(viewport);
    MakeFrame(global_tile_state);

    auto eviction_queue = host_impl()->BuildEvictionQueue(host_impl()->global_tile_state().tree_priority);
    ...
    for (; !eviction_queue->IsEmpty(); eviction_queue->Pop()) {
        ...
        has_eventually_tiles |= tile.priority().priority_bin == TilePriority::EVENTUALLY && tile.tile()->draw_info().IsReadyToDraw();
    ...
    eviction_queue = host_impl()->BuildEvictionQueue(host_impl()->global_tile_state().tree_priority);
    ...
    global_tile_state.memory_limit_policy = TileMemoryLimitPolicy::ALLOW_PREPAINT_ONLY;
    host_impl()->active_tree()->SetDeviceViewportRect(viewport);
    MakeFrame(global_tile_state);
    EXPECT_TRUE(task_runner()->HasPendingTask());
...
TEST_F(TileManagerTileReclaimTest, ReclaimOldPrepainTilesOldYoungTiles)
{
    ...
    for (; !eviction_queue->IsEmpty(); eviction_queue->Pop()) {
        const auto& tile = eviction_queue->Top();
        EXPECT_TRUE(tile.priority().priority_bin != TilePriority::EVENTUALLY || tile.tile() == first_eventually_tile);
