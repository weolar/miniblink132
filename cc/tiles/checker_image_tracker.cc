...
std::string ToString(PaintImage::Id paint_image_id, CheckerImagingDecision decision)
{
    std::ostringstream str;
    str << "paint_image_id[" << paint_image_id << "] decision[" << static_cast<int>(decision) << "]";
    return str.str();
}
...
CheckerImagingDecision GetSizeDecision(const SkIRect& src_rect, size_t min_bytes, size_t max_bytes)
{
...
CheckerImagingDecision GetCheckerImagingDecision(const PaintImage& image, const SkIRect& src_rect, size_t min_bytes, size_t max_bytes)
{
...
CheckerImageTracker::ImageDecodeRequest::ImageDecodeRequest(PaintImage paint_image, DecodeType type)
    : paint_image(std::move(paint_image))
    , type(type)
{
}

CheckerImageTracker::CheckerImageTracker(
    ImageController* image_controller, CheckerImageTrackerClient* client, bool enable_checker_imaging, size_t min_image_bytes_to_checker)
...
void CheckerImageTracker::ScheduleImageDecodeQueue(ImageDecodeQueue image_decode_queue)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "CheckerImageTracker::ScheduleImageDecodeQueue");
#if DCHECK_IS_ON()
...
const PaintImageIdFlatSet& CheckerImageTracker::TakeImagesToInvalidateOnSyncTree()
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "CheckerImageTracker::TakeImagesToInvalidateOnSyncTree");
    DCHECK_EQ(invalidated_images_on_current_sync_tree_.size(), 0u) << "Sync tree can not be invalidated more than once";
...
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "CheckerImageTracker::DidActivateSyncTree");
...
{
    image_async_decode_state_.insert(std::make_pair(image.stable_id(), DecodeState()));
}

void CheckerImageTracker::DidFinishImageDecode(
    PaintImage::Id image_id, ImageController::ImageDecodeRequestId request_id, ImageController::ImageDecodeResult result)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "CheckerImageTracker::DidFinishImageDecode");
    TRACE_EVENT_NESTABLE_ASYNC_END0("cc", "CheckerImageTracker::DeferImageDecode", TRACE_ID_LOCAL(image_id));
...
bool CheckerImageTracker::ShouldCheckerImage(const DrawImage& draw_image, WhichTree tree)
{
    const PaintImage& image = draw_image.paint_image();
    PaintImage::Id image_id = image.stable_id();
    TRACE_EVENT1("cc.debug", "CheckerImageTracker::ShouldCheckerImage", "image_id", image_id);
    ...
    if (invalidated_images_on_current_sync_tree_.count(image_id) != 0 && tree == WhichTree::ACTIVE_TREE) {
        return true;
    }
    ...
    PaintImage::DecodingMode decoding_mode_hint
        = decoding_mode_it == decoding_mode_map_.end() ? PaintImage::DecodingMode::kUnspecified : decoding_mode_it->second;
    ...
    auto insert_result = image_async_decode_state_.insert(std::pair<PaintImage::Id, DecodeState>(image_id, DecodeState()));
    ...
    if (insert_result.second) {
        ...
        CheckerImagingDecision decision
            = GetCheckerImagingDecision(image, draw_image.src_rect(), min_image_bytes_to_checker_, image_controller_->image_cache_max_limit_bytes());
        ...
        it->second.policy = decision == CheckerImagingDecision::kCanChecker ? DecodePolicy::ASYNC : DecodePolicy::SYNC;

        TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "CheckerImageTracker::CheckerImagingDecision", "image_params", ToString(image_id, decision));
    }
...
void CheckerImageTracker::UpdateDecodeState(const DrawImage& draw_image, PaintImage::Id paint_image_id, DecodeState* decode_state)
{
    ...
    // have now.
    if (outstanding_image_decode_.has_value() && outstanding_image_decode_.value().stable_id() == paint_image_id) {
        return;
    }
    ...
    // hit.
    decode_state->scale
        = SkSize::Make(std::max(decode_state->scale.fWidth, draw_image.scale().fWidth), std::max(decode_state->scale.fHeight, draw_image.scale().fHeight));
    decode_state->use_dark_mode = draw_image.use_dark_mode();
    decode_state->filter_quality = std::max(decode_state->filter_quality, draw_image.filter_quality());
...
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "CheckerImageTracker::ScheduleNextImageDecode");
    ...
    while (!image_decode_queue_.empty()) {
        ...
        draw_image = DrawImage(candidate, it->second.use_dark_mode, SkIRect::MakeWH(candidate.width(), candidate.height()), it->second.filter_quality,
            SkM44::Scale(it->second.scale.width(), it->second.scale.height()), it->second.frame_index, it->second.target_color_params);
    ...
    DCHECK_EQ(image_id_to_decode_.count(image_id), 0u);
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("cc", "CheckerImageTracker::DeferImageDecode", TRACE_ID_LOCAL(image_id));
    ImageController::ImageDecodeRequestId request_id
        = image_controller_->QueueImageDecode(draw_image, base::BindOnce(&CheckerImageTracker::DidFinishImageDecode, weak_factory_.GetWeakPtr(), image_id));

    image_id_to_decode_.emplace(image_id, std::make_unique<ScopedDecodeHolder>(image_controller_, request_id));
}

void CheckerImageTracker::UpdateImageDecodingHints(base::flat_map<PaintImage::Id, PaintImage::DecodingMode> decoding_mode_map)
{
    if (!enable_checker_imaging_)
        return;
    ...
    for (auto pair : decoding_mode_map) {
        ...
        if (state_it != image_async_decode_state_.end()) {
            ...
            if (state.policy == DecodePolicy::ASYNC && decoding_mode == PaintImage::DecodingMode::kSync) {
        ...
        if (decoding_mode_it == decoding_mode_map_.end()) {
            ...
            decoding_mode_it->second = PaintImage::GetConservative(decoding_mode_it->second, decoding_mode);
        }
    }
}
