...
public:
    DispatchingImageProvider(PlaybackImageProvider playback_image_provider, PaintWorkletImageProvider paint_worklet_image_provider)
    ...
    {
        return draw_image.paint_image().IsPaintWorklet() ? paint_worklet_image_provider_.GetPaintRecordResult(draw_image.paint_image().GetPaintWorkletInput())
...
public:
    RasterTaskImpl(TileManager* tile_manager, Tile* tile, ResourcePool::InUsePoolResource resource, scoped_refptr<RasterSource> raster_source,
        const RasterSource::PlaybackSettings& playback_settings, TileResolution tile_resolution, gfx::Rect invalidated_rect, uint64_t source_prepare_tiles_id,
        std::unique_ptr<RasterBuffer> raster_buffer, TileTask::Vector* dependencies, bool is_gpu_rasterization, DispatchingImageProvider image_provider,
        GURL url, ScrollOffsetMap raster_inducing_scroll_offsets)
        : TileTask(is_gpu_rasterization ? TileTask::SupportsConcurrentExecution::kNo : TileTask::SupportsConcurrentExecution::kYes,
            raster_buffer && raster_buffer->SupportsBackgroundThreadPriority() ? TileTask::SupportsBackgroundThreadPriority::kYes
        ...
        , url_(std::move(url))
        , raster_inducing_scroll_offsets_(std::move(raster_inducing_scroll_offsets))
    {
        DCHECK(origin_thread_checker_.CalledOnValidThread());
        playback_settings_.image_provider = &image_provider_;
        playback_settings_.raster_inducing_scroll_offsets = &raster_inducing_scroll_offsets_;
    }
    ...
    {
        TRACE_EVENT1("cc", "RasterizerTaskImpl::RunOnWorkerThread", "source_prepare_tiles_id", source_prepare_tiles_id_);

        DCHECK(raster_source_.get());
        DCHECK(raster_buffer_);

        frame_viewer_instrumentation::ScopedRasterTask raster_task(tile_tracing_id_, tile_resolution_, source_frame_number_, layer_id_);

        DCHECK(raster_source_);

        raster_buffer_->Playback(raster_source_.get(), content_rect_, invalid_content_rect_, new_content_id_, raster_transform_, playback_settings_, url_);
    }
    ...
    void OnTaskCompleted() override
    {
        ...
        tile_manager_->OnRasterTaskCompleted(tile_id_, std::move(resource_), state().IsCanceled());
    }
...
TaskCategory TaskCategoryForTileTask(TileTask* task, bool use_foreground_category)
{
...
void InsertNodeForTask(TaskGraph* graph, TileTask* task, uint16_t category, uint16_t priority, size_t dependencies, bool has_external_dependency = false)
{
    DCHECK(!base::Contains(graph->nodes, task, &TaskGraph::Node::task));
    graph->nodes.emplace_back(task, category, priority, dependencies, has_external_dependency);
}

class TaskSetFinishedTaskImpl : public TileTask {
public:
    explicit TaskSetFinishedTaskImpl(base::SequencedTaskRunner* task_runner, base::OnceClosure on_task_set_finished_callback)
        : TileTask(TileTask::SupportsConcurrentExecution::kYes, TileTask::SupportsBackgroundThreadPriority::kYes)
        , task_runner_(task_runner)
        , on_task_set_finished_callback_(std::move(on_task_set_finished_callback))
    {
    }
...
protected:
    ...
    void TaskSetFinished()
    {
        ...
        task_runner_->PostTask(FROM_HERE, std::move(on_task_set_finished_callback_));
    }
...
public:
    ...
    DidFinishRunningAllTilesTask(base::SequencedTaskRunner* task_runner, RasterQueryQueue* pending_raster_queries, CompletionCb completion_cb)
    ...
    void RunOnWorkerThread() override
    {
        ...
        if (pending_raster_queries_) {
            has_pending_queries = pending_raster_queries_->CheckRasterFinishedQueries();
        }
        task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(completion_cb_), has_pending_queries));
    }
...
private:
    raw_ptr<base::SequencedTaskRunner> task_runner_;
    raw_ptr<RasterQueryQueue, AcrossTasksDanglingUntriaged> pending_raster_queries_;
    CompletionCb completion_cb_;
};

gfx::ContentColorUsage GetContentColorUsageForPrioritizedTile(const PrioritizedTile& prioritized_tile)
{
    return prioritized_tile.raster_source()->GetDisplayItemList()->content_color_usage();
}
...
std::unique_ptr<base::trace_event::ConvertableToTraceFormat> RasterTaskCompletionStatsAsValue(const RasterTaskCompletionStats& stats)
{
    std::unique_ptr<base::trace_event::TracedValue> state(new base::trace_event::TracedValue());
    state->SetInteger("completed_count", base::saturated_cast<int>(stats.completed_count));
    state->SetInteger("canceled_count", base::saturated_cast<int>(stats.canceled_count));
    return std::move(state);
}

TileManager::TileManager(TileManagerClient* client, base::SequencedTaskRunner* origin_task_runner,
    scoped_refptr<base::SequencedTaskRunner> image_worker_task_runner, size_t scheduled_raster_task_limit, bool running_on_renderer_process,
    ...
    , image_controller_(origin_task_runner, std::move(image_worker_task_runner),
          base::BindRepeating(&TileManager::ExternalDependencyCompletedForRasterTask, base::Unretained(this)))
    , decoded_image_tracker_(&image_controller_, origin_task_runner)
    , checker_image_tracker_(&image_controller_, this, tile_manager_settings_.enable_checker_imaging, tile_manager_settings_.min_image_bytes_to_checker)
    , more_tiles_need_prepare_check_notifier_(task_runner_, base::BindRepeating(&TileManager::CheckIfMoreTilesNeedToBePrepared, base::Unretained(this)))
    , signals_check_notifier_(task_runner_, base::BindRepeating(&TileManager::CheckForCompletedTasksAndIssueSignals, base::Unretained(this)))
    , has_scheduled_tile_tasks_(false)
    , prepare_tiles_count_(0u)
    , next_tile_id_(0u)
{
//     if (base::SingleThreadTaskRunner::HasCurrentDefault()) {
//         base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "TileManager", base::SingleThreadTaskRunner::GetCurrentDefault());
//     }
}
...
void TileManager::ScheduleReduceTileMemoryWhenIdle(base::TimeDelta time_since_last_active)
{
    if (!base::FeatureList::IsEnabled(features::kReclaimPrepaintTilesWhenIdle) || has_pending_idle_task_) {
        return;
    }
    ...
    TaskRunnerWithOverride()->PostDelayedTask(
        FROM_HERE, base::BindOnce(&TileManager::ReduceTileMemoryWhenIdle, ready_to_draw_callback_weak_ptr_factory_.GetWeakPtr()), delay);
}
...
{
    if (!base::FeatureList::IsEnabled(features::kReclaimOldPrepaintTiles) || has_pending_tile_trimming_task_) {
        return;
    }

    has_pending_tile_trimming_task_ = true;
    TaskRunnerWithOverride()->PostDelayedTask(
        FROM_HERE, base::BindOnce(&TileManager::TrimPrepaintTiles, ready_to_draw_callback_weak_ptr_factory_.GetWeakPtr()), GetTrimPrepaintTilesDelay());
}
...
void TileManager::ReduceTileMemoryWhenIdle()
{
    ...
    base::TimeDelta time_since_last_active = NowWithOverride() - last_active_time_;
    ...
    MemoryUsage usage(resource_pool_->memory_usage_bytes(), resource_pool_->resource_count());

    // Ensures that all the resources that are not at least as important as this
    // one are evicted.
    constexpr TilePriority kVisiblePriority = TilePriority(HIGH_RESOLUTION, TilePriority::NOW, 0);
    ...
    FreeTileResourcesWithLowerPriorityUntilUsageIsWithinLimit(nullptr, limit, kVisiblePriority, &usage);
}
...
void TileManager::TrimPrepaintTiles()
{
    ...
    std::unique_ptr<EvictionTilePriorityQueue> eviction_priority_queue = client_->BuildEvictionQueue(global_state_.tree_priority);
    ...
    for (; !eviction_priority_queue->IsEmpty(); eviction_priority_queue->Pop()) {
        ...
        bool eligible = prioritized_tile.priority().priority_bin > TilePriority::SOON && TilePriorityViolatesMemoryPolicy(prioritized_tile.priority());
        if (!eligible) {
            continue;
        }
...
void TileManager::SetResources(ResourcePool* resource_pool, ImageDecodeCache* image_decode_cache, TaskGraphRunner* task_graph_runner,
    RasterBufferProvider* raster_buffer_provider, bool use_gpu_rasterization, RasterQueryQueue* pending_raster_queries)
{
    ...
    tile_task_manager_ = TileTaskManagerImpl::Create(
        task_graph_runner, base::BindRepeating(&TileManager::ExternalDependencyCompletedForNonRasterTask, base::Unretained(this)));
...
{
    TRACE_EVENT0("cc", "TileManager::DidFinishRunningTileTasksRequiredForActivation");
    TRACE_EVENT_NESTABLE_ASYNC_INSTANT1("cc", "ScheduledTasksState", TRACE_ID_LOCAL(this), "state", ScheduledTasksStateAsValue());
...
void TileManager::ExternalDependencyCompletedForNonRasterTask(scoped_refptr<TileTask> dependent)
{
    image_controller_.ExternalDependencyCompletedForTask(std::move(dependent));
}

void TileManager::ExternalDependencyCompletedForRasterTask(scoped_refptr<TileTask> dependent)
{
    tile_task_manager_->ExternalDependencyCompletedForTask(std::move(dependent));
}

bool TileManager::PrepareTiles(const GlobalStateThatImpactsTilePriority& state)
{
    ...
    TRACE_EVENT1("cc,benchmark", "TileManager::PrepareTiles", "prepare_tiles_id", prepare_tiles_count_);

    if (!tile_task_manager_) {
        TRACE_EVENT_INSTANT0("cc", "PrepareTiles aborted", TRACE_EVENT_SCOPE_THREAD);
        return false;
    }
    ...
    client_->SetIsLikelyToRequireADraw(!prioritized_work.tiles_to_raster.empty() && prioritized_work.tiles_to_raster.front().tile()->required_for_draw());

    // Schedule tile tasks.
    ScheduleTasks(std::move(prioritized_work));

    TRACE_EVENT_INSTANT1("cc", "DidPrepareTiles", TRACE_EVENT_SCOPE_THREAD, "state", BasicStateAsValue());
    return true;
}
...
void TileManager::PrepareToDraw()
{
    ...
    if (!tile_task_manager_) {
        TRACE_EVENT_INSTANT0("cc", "TileManager::PrepareToDrawAborted", TRACE_EVENT_SCOPE_THREAD);
        return;
    }
    ...
    TRACE_EVENT_INSTANT1(
        "cc", "TileManager::PrepareToDrawFinished", TRACE_EVENT_SCOPE_THREAD, "stats", RasterTaskCompletionStatsAsValue(raster_task_completion_stats_));
...
std::unique_ptr<base::trace_event::ConvertableToTraceFormat> TileManager::BasicStateAsValue() const
{
    std::unique_ptr<base::trace_event::TracedValue> value(new base::trace_event::TracedValue());
    BasicStateAsValueInto(value.get());
    return std::move(value);
}

void TileManager::BasicStateAsValueInto(base::trace_event::TracedValue* state) const
{
    *(int*)1 = 1;
...
std::unique_ptr<EvictionTilePriorityQueue> TileManager::FreeTileResourcesUntilUsageIsWithinLimit(
    std::unique_ptr<EvictionTilePriorityQueue> eviction_priority_queue, const MemoryUsage& limit, MemoryUsage* usage)
{
...
std::unique_ptr<EvictionTilePriorityQueue> TileManager::FreeTileResourcesWithLowerPriorityUntilUsageIsWithinLimit(
    std::unique_ptr<EvictionTilePriorityQueue> eviction_priority_queue, const MemoryUsage& limit, const TilePriority& other_priority, MemoryUsage* usage)
{
    while (usage->Exceeds(limit)) {
        if (!eviction_priority_queue) {
            eviction_priority_queue = client_->BuildEvictionQueue(global_state_.tree_priority);
...
bool TileManager::TilePriorityViolatesMemoryPolicy(const TilePriority& priority)
{
...
TileManager::PrioritizedWorkToSchedule TileManager::AssignGpuMemoryToTiles()
{
    ...
    for (; !raster_priority_queue->IsEmpty(); raster_priority_queue->Pop()) {
        ...
        if (!tile->is_solid_color_analysis_performed() && tile->use_picture_analysis() && kUseColorEstimator) {
            ...
            bool is_solid_color = prioritized_tile.raster_source()->PerformSolidColorAnalysis(tile->enclosing_layer_rect(), &color, kMaxOpsToAnalyze);
            if (is_solid_color) {
        ...
        auto content_color_usage = GetContentColorUsageForPrioritizedTile(prioritized_tile);
        const auto target_color_params = client_->GetTargetColorParams(content_color_usage);
        ...
        if (!tile->draw_info().NeedsRaster()) {
            ...
            AddCheckeredImagesToDecodeQueue(
                prioritized_tile, target_color_params, CheckerImageTracker::DecodeType::kRaster, &work_to_schedule.checker_image_decode_queue);
            continue;
        }
        ...
        if (!tile->raster_task_.get()) {
            memory_required_by_tile_to_be_scheduled = MemoryUsage::FromConfig(tile->desired_texture_size(), DetermineFormat(tile));
        }
        ...
        eviction_priority_queue = FreeTileResourcesWithLowerPriorityUntilUsageIsWithinLimit(
            std::move(eviction_priority_queue), scheduled_tile_memory_limit, priority, &memory_usage);
        ...
        if (tile->HasRasterTask()) {
            if (tile->raster_task_scheduled_with_checker_images() && prioritized_tile.should_decode_checkered_images_for_tile()) {
                AddCheckeredImagesToDecodeQueue(
                    prioritized_tile, target_color_params, CheckerImageTracker::DecodeType::kRaster, &work_to_schedule.checker_image_decode_queue);
            }
        } else {
            ...
            auto raster_task = CreateRasterTask(prioritized_tile, target_color_params, &work_to_schedule);
            if (!raster_task) {
                continue;
            }
    ...
    if (!had_enough_memory_to_schedule_tiles_needed_now && num_of_tiles_with_checker_images_ > 0) {
        for (; !raster_priority_queue->IsEmpty(); raster_priority_queue->Pop()) {
            const PrioritizedTile& prioritized_tile = raster_priority_queue->Top();
            ...
            Tile* tile = prioritized_tile.tile();
            if (tile->draw_info().is_checker_imaged() || tile->raster_task_scheduled_with_checker_images()) {
                AddCheckeredImagesToDecodeQueue(
                    prioritized_tile, target_color_params, CheckerImageTracker::DecodeType::kRaster, &work_to_schedule.checker_image_decode_queue);
            }
        }
    ...
    if (metrics_sub_sampler_.ShouldSample(0.01)) {
        if (running_on_renderer_process_) {
            UMA_HISTOGRAM_BOOLEAN("Compositing.TileManager.EnoughMemory.Renderer", had_enough_memory_to_schedule_tiles_needed_now);
        } else {
            UMA_HISTOGRAM_BOOLEAN("Compositing.TileManager.EnoughMemory.Browser", had_enough_memory_to_schedule_tiles_needed_now);
        }
        if (did_oom_on_last_assign_) {
            auto memory_limit = hard_memory_limit.memory_bytes() / (1024 * 1024);
            if (running_on_renderer_process_) {
                UMA_HISTOGRAM_MEMORY_MEDIUM_MB("Compositing.TileManager.LimitWhenNotEnoughMemory.Renderer", memory_limit);
            } else {
                UMA_HISTOGRAM_MEMORY_MEDIUM_MB("Compositing.TileManager.LimitWhenNotEnoughMemory.Browser", memory_limit);
            }
        }
    }
    ...
    TRACE_EVENT_END2("cc", "TileManager::AssignGpuMemoryToTiles", "all_tiles_that_need_to_be_rasterized_are_scheduled",
        all_tiles_that_need_to_be_rasterized_are_scheduled_, "had_enough_memory_to_schedule_tiles_needed_now", had_enough_memory_to_schedule_tiles_needed_now);
...
{
    std::unique_ptr<TilesWithResourceIterator> iterator = client_->CreateTilesWithResourceIterator();
...
void TileManager::FreeResourcesForTileAndNotifyClientIfTileWasReadyToDraw(Tile* tile)
{
...
void TileManager::PartitionImagesForCheckering(const PrioritizedTile& prioritized_tile, const TargetColorParams& target_color_params,
    std::vector<DrawImage>* sync_decoded_images, std::vector<PaintImage>* checkered_images, const gfx::Rect* invalidated_rect,
    base::flat_map<PaintImage::Id, size_t>* image_to_frame_index)
{
    Tile* tile = prioritized_tile.tile();
    gfx::Rect enclosing_rect = tile->enclosing_layer_rect();
    if (invalidated_rect) {
        enclosing_rect = ToEnclosingRect(tile->raster_transform().InverseMapRect(gfx::RectF(*invalidated_rect)));
    }
    std::vector<const DrawImage*> images_in_tile = prioritized_tile.source_tiling()->client()->GetDiscardableImagesInRect(enclosing_rect);
    ...
    for (const auto* original_draw_image : images_in_tile) {
        ...
        DrawImage draw_image(*original_draw_image, tile->contents_scale_key(), frame_index, target_color_params);
...
void TileManager::AddCheckeredImagesToDecodeQueue(const PrioritizedTile& prioritized_tile, const TargetColorParams& target_color_params,
    CheckerImageTracker::DecodeType decode_type, CheckerImageTracker::ImageDecodeQueue* image_decode_queue)
{
    Tile* tile = prioritized_tile.tile();
    std::vector<const DrawImage*> images_in_tile = prioritized_tile.source_tiling()->client()->GetDiscardableImagesInRect(tile->enclosing_layer_rect());
    WhichTree tree = tile->tiling()->tree();
    for (const auto* original_draw_image : images_in_tile) {
        size_t frame_index = client_->GetFrameIndexForImage(original_draw_image->paint_image(), tree);
        DrawImage draw_image(*original_draw_image, tile->contents_scale_key(), frame_index, target_color_params);
        if (checker_image_tracker_.ShouldCheckerImage(draw_image, tree)) {
...
void TileManager::ScheduleTasks(PrioritizedWorkToSchedule work_to_schedule)
{
    ...
    TRACE_EVENT1("cc", "TileManager::ScheduleTasks", "count", tiles_that_need_to_be_rasterized.size());

    DCHECK(did_check_for_completed_tasks_since_last_schedule_tasks_);

    if (!has_scheduled_tile_tasks_) {
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("cc", "ScheduledTasks", TRACE_ID_LOCAL(this));
    }
    ...
    scoped_refptr<TileTask> required_for_activation_done_task = CreateTaskSetFinishedTask(&TileManager::DidFinishRunningTileTasksRequiredForActivation);
    scoped_refptr<TileTask> required_for_draw_done_task = CreateTaskSetFinishedTask(&TileManager::DidFinishRunningTileTasksRequiredForDraw);

    auto all_done_cb = base::BindOnce(&TileManager::DidFinishRunningAllTileTasks, task_set_finished_weak_ptr_factory_.GetWeakPtr());
    scoped_refptr<TileTask> all_done_task = base::MakeRefCounted<DidFinishRunningAllTilesTask>(task_runner_, pending_raster_queries_, std::move(all_done_cb));
    ...
    for (auto& prioritized_tile : tiles_that_need_to_be_rasterized) {
        ...
        bool use_foreground_category
            = tile->required_for_draw() || tile->required_for_activation() || prioritized_tile.priority().priority_bin == TilePriority::NOW;
    ...
    for (const PrioritizedTile& prioritized_tile : tiles_to_process_for_images) {
        ...
        PartitionImagesForCheckering(prioritized_tile, target_color_params, &sync_decoded_images, &checkered_images, nullptr);

        // Add the sync decoded images to |new_locked_images| so they can be added
        // to the task graph.
        new_locked_images.insert(
            new_locked_images.end(), std::make_move_iterator(sync_decoded_images.begin()), std::make_move_iterator(sync_decoded_images.end()));

        // For checkered-images, send them to the decode service.
        for (auto& image : checkered_images) {
            work_to_schedule.checker_image_decode_queue.emplace_back(std::move(image), CheckerImageTracker::DecodeType::kPreDecode);
        }
    }

    new_locked_images.insert(new_locked_images.end(), work_to_schedule.extra_prepaint_images.begin(), work_to_schedule.extra_prepaint_images.end());
    ...
    ImageDecodeCache::TracingInfo tracing_info(prepare_tiles_count_, TilePriority::SOON);
    std::vector<scoped_refptr<TileTask>> new_locked_image_tasks = image_controller_.SetPredecodeImages(new_locked_images, tracing_info);
    ...
    for (auto& task : new_locked_image_tasks) {
        auto decode_it = base::ranges::find(graph_.nodes, task.get(), &TaskGraph::Node::task);
    ...
    InsertNodeForTask(&graph_, required_for_activation_done_task.get(), TASK_CATEGORY_NONCONCURRENT_FOREGROUND, kRequiredForActivationDoneTaskPriority,
        required_for_activate_count);
    InsertNodeForTask(
        &graph_, required_for_draw_done_task.get(), TASK_CATEGORY_NONCONCURRENT_FOREGROUND, kRequiredForDrawDoneTaskPriority, required_for_draw_count);
    InsertNodeForTask(&graph_, all_done_task.get(), TASK_CATEGORY_NONCONCURRENT_FOREGROUND, kAllDoneTaskPriority, all_count);
    ...
    checker_image_tracker_.ScheduleImageDecodeQueue(std::move(work_to_schedule.checker_image_decode_queue));
...
scoped_refptr<TileTask> TileManager::CreateRasterTask(
    const PrioritizedTile& prioritized_tile, const TargetColorParams& target_color_params, PrioritizedWorkToSchedule* work_to_schedule)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "TileManager::CreateRasterTask");
    Tile* tile = prioritized_tile.tile();
    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "TileManager::CreateRasterTask", "Tile", tile->id());

    const int msaa_sample_count = client_->GetMSAASampleCountForRaster(*prioritized_tile.raster_source()->GetDisplayItemList());
    ...
    if (target_color_params.color_space.IsHDR() && GetContentColorUsageForPrioritizedTile(prioritized_tile) == gfx::ContentColorUsage::kHDR) {
    ...
    if (UsePartialRaster(msaa_sample_count) && tile->invalidated_id()) {
        const std::string& debug_name = prioritized_tile.source_tiling()->raster_source()->debug_name();
        resource = resource_pool_->TryAcquireResourceForPartialRaster(
            tile->id(), tile->invalidated_content_rect(), tile->invalidated_id(), &invalidated_rect, target_color_params.color_space, debug_name);
        ...
        if (metrics_sub_sampler_.ShouldSample(kLogProbability)) {
            ...
            if (auto tile_area = tile->desired_texture_size().Area64(); tile_area >= kMinAreaForReporting) {
                auto percentage_invalidated = (100 * invalidated_rect.size().Area64()) / tile_area;
                UMA_HISTOGRAM_PERCENTAGE("Compositing.TileManager.TileInvalidationArea", percentage_invalidated);
            }
        }
    }
    ...
    if (resource) {
        ...
        const std::string& debug_name = prioritized_tile.source_tiling()->raster_source()->debug_name();
        resource = resource_pool_->AcquireResource(tile->desired_texture_size(), format, target_color_params.color_space, debug_name);
        DCHECK(resource);
    }

    // For LOW_RESOLUTION tiles, we don't draw or predecode images.
    RasterSource::PlaybackSettings playback_settings;
    const bool skip_images = prioritized_tile.priority().resolution == LOW_RESOLUTION;
    playback_settings.use_lcd_text = tile->can_use_lcd_text();
    playback_settings.msaa_sample_count = msaa_sample_count;
    playback_settings.visible = tile->required_for_activation() || tile->required_for_draw();
    playback_settings.hdr_headroom = target_color_params.hdr_max_luminance_relative;
    ...
    std::vector<DrawImage>& sync_decoded_images = scheduled_draw_images_[tile->id()];
    sync_decoded_images.clear();
    std::vector<PaintImage> checkered_images;
    base::flat_map<PaintImage::Id, size_t> image_id_to_current_frame_index;
    if (!skip_images) {
        PartitionImagesForCheckering(prioritized_tile, target_color_params, &sync_decoded_images, &checkered_images,
            partial_tile_decode ? &invalidated_rect : nullptr, &image_id_to_current_frame_index);
    }

    // Get the tasks for the required images.
    ImageDecodeCache::TracingInfo tracing_info(prepare_tiles_count_, prioritized_tile.priority().priority_bin);
    ...
    image_controller_.ConvertImagesToTasks(&sync_decoded_images, &decode_tasks, &has_at_raster_images, &has_hardware_accelerated_jpeg_candidates,
    ...
    if (has_at_raster_images && tile->is_prepaint()) {
        work_to_schedule->extra_prepaint_images.insert(work_to_schedule->extra_prepaint_images.end(), sync_decoded_images.begin(), sync_decoded_images.end());
        // This will unref the images, but ScheduleTasks will schedule them
        // right away anyway.
        OnRasterTaskCompleted(tile->id(), std::move(resource), true /* was_canceled */);
        return nullptr;
    }
    ...
    for (const auto& image : checkered_images) {
        ...
        if (prioritized_tile.should_decode_checkered_images_for_tile()) {
            work_to_schedule->checker_image_decode_queue.emplace_back(image, CheckerImageTracker::DecodeType::kRaster);
        }
    }
    ...
    if (!skip_images) {
        ...
        settings->image_to_current_frame_index = std::move(image_id_to_current_frame_index);
        if (use_gpu_rasterization_) {
            settings->raster_mode = PlaybackImageProvider::RasterMode::kOop;
        }
    }

    PlaybackImageProvider image_provider(image_controller_.cache(), target_color_params, std::move(settings));
    ...
    PaintWorkletRecordMap paint_worklet_records = prioritized_tile.GetPaintWorkletRecords();
    PaintWorkletImageProvider paint_worklet_image_provider(std::move(paint_worklet_records));
    DispatchingImageProvider dispatching_image_provider(std::move(image_provider), std::move(paint_worklet_image_provider));

    return base::MakeRefCounted<RasterTaskImpl>(this, tile, std::move(resource), prioritized_tile.raster_source(), playback_settings,
        prioritized_tile.priority().resolution, invalidated_rect, prepare_tiles_count_, std::move(raster_buffer), &decode_tasks, use_gpu_rasterization_,
        std::move(dispatching_image_provider), active_url_, prioritized_tile.GetRasterInducingScrollOffsets());
}

void TileManager::InsertNodeForDecodeTask(TileTask* task, uint16_t priority, bool use_foreground_category)
{
    ...
    InsertNodeForTask(&graph_, task, TaskCategoryForTileTask(task, use_foreground_category), priority, dependency_count, has_external_dependency);
}

void TileManager::InsertNodesForRasterTask(TileTask* raster_task, uint16_t priority, bool use_foreground_category)
{
    size_t dependencies = 0u;

    // Insert image decode tasks.
    for (auto it = raster_task->dependencies().begin(); it != raster_task->dependencies().end(); ++it) {
        ...
        auto decode_it = base::ranges::find(graph_.nodes, decode_task, &TaskGraph::Node::task);
        ...
        // crbug.com/605234
        if (decode_it != graph_.nodes.end() && use_foreground_category && !IsForegroundCategory(decode_it->category)) {
    ...
    InsertNodeForTask(&graph_, raster_task, TaskCategoryForTileTask(raster_task, use_foreground_category), priority, dependencies);
}
...
void TileManager::OnRasterTaskCompleted(Tile::Id tile_id, ResourcePool::InUsePoolResource resource, bool was_canceled)
{
    ...
    if (found != tiles_.end()) {
        ...
        raster_task_was_scheduled_with_checker_images = tile->set_raster_task_scheduled_with_checker_images(false);
    ...
    bool exported = resource_pool_->PrepareForExport(resource, viz::TransferableResource::ResourceSource::kTileRasterTask);
    ...
    if (exported) {
        ...
        draw_info.SetResource(std::move(resource), raster_task_was_scheduled_with_checker_images, is_premultiplied);
    } else {
        resource_pool_->ReleaseResource(std::move(resource));
        draw_info.set_oom();
    }
...
std::unique_ptr<Tile> TileManager::CreateTile(const Tile::CreateInfo& info, int layer_id, int source_frame_number, int flags)
{
    // We need to have a tile task worker pool to do anything meaningful with
    // tiles.
    DCHECK(tile_task_manager_);
    std::unique_ptr<Tile> tile(new Tile(this, info, layer_id, source_frame_number, flags));
...
bool TileManager::AreRequiredTilesReadyToDraw(RasterTilePriorityQueue::Type type) const
{
    std::unique_ptr<RasterTilePriorityQueue> raster_priority_queue(client_->BuildRasterQueue(global_state_.tree_priority, type));
...
#if DCHECK_IS_ON()
    std::unique_ptr<RasterTilePriorityQueue> all_queue(client_->BuildRasterQueue(global_state_.tree_priority, type));
...
bool TileManager::IsReadyToActivate() const
{
    ...
    return pending_required_for_activation_callback_id_ == 0 && AreRequiredTilesReadyToDraw(RasterTilePriorityQueue::Type::REQUIRED_FOR_ACTIVATION);
}

bool TileManager::IsReadyToDraw() const
{
    TRACE_EVENT0("cc,benchmark", "TileManager::IsReadyToDraw");
    return pending_required_for_draw_callback_id_ == 0 && AreRequiredTilesReadyToDraw(RasterTilePriorityQueue::Type::REQUIRED_FOR_DRAW);
}
...
void TileManager::ScheduleCheckRasterFinishedQueries()
{
    ...
    check_pending_tile_queries_callback_.Reset(base::BindOnce(&TileManager::CheckRasterFinishedQueries, base::Unretained(this)));
    task_runner_->PostDelayedTask(FROM_HERE, check_pending_tile_queries_callback_.callback(), base::Milliseconds(100));
}
...
void TileManager::CheckRasterFinishedQueries()
{
    ...
    if (pending_raster_queries_) {
        has_pending_queries_ = pending_raster_queries_->CheckRasterFinishedQueries();
    }
    if (has_pending_queries_)
...
{
    // Ready to activate.
    if (signals_.activate_tile_tasks_completed && signals_.activate_gpu_work_completed && !signals_.did_notify_ready_to_activate) {
        ...
        if (client_->HasPendingTree() && IsReadyToActivate()) {
            TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "TileManager::IssueSignals - ready to activate");
    ...
    // Ready to draw.
    if (signals_.draw_tile_tasks_completed && signals_.draw_gpu_work_completed && !signals_.did_notify_ready_to_draw) {
        if (tile_manager_settings_.needs_notify_ready_to_draw && IsReadyToDraw()) {
            TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "TileManager::IssueSignals - ready to draw");
    ...
    if (signals_.all_tile_tasks_completed && !signals_.did_notify_all_tile_tasks_completed) {
        if (!has_scheduled_tile_tasks_) {
            TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "TileManager::IssueSignals - all tile tasks completed");
    ...
    bool notify_ready_to_activate_pending = client_->HasPendingTree() && !signals_.did_notify_ready_to_activate;
    bool notify_ready_to_draw_pending = tile_manager_settings_.needs_notify_ready_to_draw && !signals_.did_notify_ready_to_draw;
    ...
    if (signals_.did_notify_all_tile_tasks_completed) {
        checker_image_tracker_.SetMaxDecodePriorityAllowed(CheckerImageTracker::DecodeType::kPreDecode);
    } else if (!notify_ready_to_activate_pending && !notify_ready_to_draw_pending) {
        checker_image_tracker_.SetMaxDecodePriorityAllowed(CheckerImageTracker::DecodeType::kRaster);
    }
}
...
void TileManager::CheckIfMoreTilesNeedToBePrepared()
{
    ...
    // Inform the client that will likely require a draw if the highest priority
    // tile that will be rasterized is required for draw.
    client_->SetIsLikelyToRequireADraw(!work_to_schedule.tiles_to_raster.empty() && work_to_schedule.tiles_to_raster.front().tile()->required_for_draw());
    ...
    if (global_state_.tree_priority != SMOOTHNESS_TAKES_PRIORITY) {
        image_controller_.SetPredecodeImages(std::vector<DrawImage>(), ImageDecodeCache::TracingInfo());
        locked_image_tasks_.clear();
    }
    ...
    MarkTilesOutOfMemory(client_->BuildRasterQueue(global_state_.tree_priority, RasterTilePriorityQueue::Type::REQUIRED_FOR_ACTIVATION));
    MarkTilesOutOfMemory(client_->BuildRasterQueue(global_state_.tree_priority, RasterTilePriorityQueue::Type::REQUIRED_FOR_DRAW));
...
void TileManager::MarkTilesOutOfMemory(std::unique_ptr<RasterTilePriorityQueue> queue) const
{
...
void TileManager::ClearCheckerImageTracking(bool can_clear_decode_policy_tracking)
{
    checker_image_tracker_.ClearTracker(can_clear_decode_policy_tracking);
}
...
std::unique_ptr<base::trace_event::ConvertableToTraceFormat> TileManager::ScheduledTasksStateAsValue() const
{
    std::unique_ptr<base::trace_event::TracedValue> state(new base::trace_event::TracedValue());
    state->BeginDictionary("tasks_pending");
    state->SetBoolean("activate_tile_tasks_completed", signals_.activate_tile_tasks_completed);
    state->SetBoolean("draw_tile_tasks_completed", signals_.draw_tile_tasks_completed);
    state->SetBoolean("all_tile_tasks_completed", signals_.all_tile_tasks_completed);
    state->EndDictionary();
    return std::move(state);
}
...
bool TileManager::UsePartialRaster(int msaa_sample_count) const
{
    ...
    return tile_manager_settings_.use_partial_raster && raster_buffer_provider_->CanPartialRasterIntoProvidedResource() && msaa_sample_count == 0;
}

void TileManager::CheckPendingGpuWorkAndIssueSignals()
{
//     TRACE_EVENT2("cc", "TileManager::CheckPendingGpuWorkAndIssueSignals", "pending_gpu_work_tiles", pending_gpu_work_tiles_.size(), "tree_priority",
//         TreePriorityToString(global_state_.tree_priority));
    ...
    for (auto it = pending_gpu_work_tiles_.begin(); it != pending_gpu_work_tiles_.end();) {
        Tile* tile = *it;
        DCHECK(tile->draw_info().has_resource());
        const ResourcePool::InUsePoolResource& resource = tile->draw_info().GetResource();
        ...
        if (global_state_.tree_priority != SMOOTHNESS_TAKES_PRIORITY || raster_buffer_provider_->IsResourceReadyToDraw(resource)) {
    ...
    if (required_for_activation.empty()) {
        ...
        pending_required_for_activation_callback_id_ = raster_buffer_provider_->SetReadyToDrawCallback(required_for_activation,
            base::BindOnce(&TileManager::CheckPendingGpuWorkAndIssueSignals, ready_to_draw_callback_weak_ptr_factory_.GetWeakPtr()),
    ...
    if (required_for_draw.empty()) {
        ...
        pending_required_for_draw_callback_id_ = raster_buffer_provider_->SetReadyToDrawCallback(required_for_draw,
            base::BindOnce(&TileManager::CheckPendingGpuWorkAndIssueSignals, ready_to_draw_callback_weak_ptr_factory_.GetWeakPtr()),
            pending_required_for_draw_callback_id_);
    }

    // Update our signals now that we know whether we have pending resources.
    signals_.activate_gpu_work_completed = (pending_required_for_activation_callback_id_ == 0);
    signals_.draw_gpu_work_completed = (pending_required_for_draw_callback_id_ == 0);
...
scoped_refptr<TileTask> TileManager::CreateTaskSetFinishedTask(void (TileManager::*callback)())
{
    return base::MakeRefCounted<TaskSetFinishedTaskImpl>(task_runner_, base::BindOnce(callback, task_set_finished_weak_ptr_factory_.GetWeakPtr()));
}

std::unique_ptr<base::trace_event::ConvertableToTraceFormat> TileManager::ActivationStateAsValue()
{
...
void TileManager::ActivationStateAsValueInto(base::trace_event::TracedValue* state) const
{
    *(int*)1 = 1;
//     state->SetString("tree_priority", TreePriorityToString(global_state_.tree_priority));
//     state->SetInteger("soft_memory_limit", global_state_.soft_memory_limit_in_bytes);
//     state->SetInteger("hard_memory_limit", global_state_.hard_memory_limit_in_bytes);
//     state->SetInteger("pending_required_for_activation_callback_id", pending_required_for_activation_callback_id_);
//     state->SetInteger("current_memory_usage", resource_pool_->memory_usage_bytes());
...
//     auto tile_as_value = [](const PrioritizedTile& prioritized_tile, base::trace_event::TracedValue* value) {
//         Tile* tile = prioritized_tile.tile();
//         TilePriority priority = prioritized_tile.priority();
// 
...
//         value->SetString("priority_bin", TilePriorityBinToString(priority.priority_bin));
//         value->SetDouble("distance_to_visible", priority.distance_to_visible);
//         value->SetBoolean("required_for_activation", tile->required_for_activation());
...
//         client_->BuildRasterQueue(global_state_.tree_priority, RasterTilePriorityQueue::Type::REQUIRED_FOR_ACTIVATION));
...
void TileManager::SetOverridesForTesting(scoped_refptr<base::TaskRunner> task_runner_for_testing, const base::TickClock* clock)
{
    task_runner_for_testing_ = task_runner_for_testing;
    tick_clock_for_testing_ = clock;
}

bool TileManager::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    if (args.level_of_detail != base::trace_event::MemoryDumpLevelOfDetail::kDetailed || !resource_pool_) {
        return true;
    }

    std::string manager_path = base::StringPrintf("cc/tile_manager_%d", resource_pool_->tracing_id());
    auto* dump = pmd->CreateAllocatorDump(manager_path);
    dump->AddString("memory_policy", "", TileMemoryLimitPolicyToString(global_state_.memory_limit_policy));
    dump->AddScalar("soft_memory_limit", "bytes", global_state_.soft_memory_limit_in_bytes);
    dump->AddScalar("hard_memory_limit", "bytes", global_state_.hard_memory_limit_in_bytes);
    dump->AddScalar("num_resources_limit", "count", global_state_.num_resources_limit);
    ...
    while (!eviction_priority_queue->IsEmpty()) {
        ...
        std::string name = base::StringPrintf("%s/tile_%u", manager_path.c_str(), static_cast<unsigned int>(tile.tile()->id()));
        auto* tile_dump = pmd->CreateAllocatorDump(name);
        tile_dump->AddString("priority", "", TilePriorityBinToString(tile.priority().priority_bin));
        tile_dump->AddScalar("distance_to_visible", "px", tile.priority().distance_to_visible);

        tile_dump->AddScalar("is_prepaint", "bool", tile.tile()->is_prepaint());
        tile_dump->AddScalar("gpu_memory", "bytes", tile.tile()->GPUMemoryUsageInBytes());
...
{
    return (global_state_.memory_limit_policy != ALLOW_NOTHING && global_state_.memory_limit_policy != ALLOW_ABSOLUTE_MINIMUM);
}

base::TimeTicks TileManager::NowWithOverride() const
{
    return tick_clock_for_testing_ ? tick_clock_for_testing_->NowTicks() : base::TimeTicks::Now();
}

base::TaskRunner* TileManager::TaskRunnerWithOverride() const
{
    return task_runner_for_testing_ ? task_runner_for_testing_.get() : task_runner_;
}
...
TileManager::MemoryUsage::MemoryUsage(size_t memory_bytes, size_t resource_count)
    ...
    , resource_count_(static_cast<int>(resource_count))
{
    ...
    DCHECK_LE(memory_bytes, static_cast<size_t>(std::numeric_limits<int64_t>::max()));
    DCHECK_LE(resource_count, static_cast<size_t>(std::numeric_limits<int>::max()));
}

// static
TileManager::MemoryUsage TileManager::MemoryUsage::FromConfig(const gfx::Size& size, viz::SharedImageFormat format)
{
...
TileManager::MemoryUsage& TileManager::MemoryUsage::operator+=(const MemoryUsage& other)
{
...
TileManager::PrioritizedWorkToSchedule::PrioritizedWorkToSchedule(PrioritizedWorkToSchedule&& other) = default;
TileManager::PrioritizedWorkToSchedule::~PrioritizedWorkToSchedule() = default;
