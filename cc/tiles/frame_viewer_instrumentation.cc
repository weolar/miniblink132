...
constexpr const char kCategory[] = "cc," TRACE_DISABLED_BY_DEFAULT("devtools.timeline");
...
std::unique_ptr<base::trace_event::ConvertableToTraceFormat> TileDataAsValue(
    const void* tile_id, TileResolution tile_resolution, int source_frame_number, int layer_id)
{
    std::unique_ptr<base::trace_event::TracedValue> res(new base::trace_event::TracedValue());
...
ScopedAnalyzeTask::ScopedAnalyzeTask(const void* tile_id, TileResolution tile_resolution, int source_frame_number, int layer_id)
{
    TRACE_EVENT_BEGIN1(kCategory, kAnalyzeTask, kTileData, TileDataAsValue(tile_id, tile_resolution, source_frame_number, layer_id));
}
...
ScopedRasterTask::ScopedRasterTask(const void* tile_id, TileResolution tile_resolution, int source_frame_number, int layer_id)
{
    TRACE_EVENT_BEGIN1(kCategory, kRasterTask, kTileData, TileDataAsValue(tile_id, tile_resolution, source_frame_number, layer_id));
}
