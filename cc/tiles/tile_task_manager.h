...
private:
    explicit TileTaskManagerImpl(TaskGraphRunner* task_graph_runner, base::RepeatingCallback<void(scoped_refptr<TileTask>)> notify_external_dependent);

    raw_ptr<TaskGraphRunner> task_graph_runner_;
    base::RepeatingCallback<void(scoped_refptr<TileTask>)> notify_external_dependent_;
