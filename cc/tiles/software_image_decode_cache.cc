...
public:
    AutoRemoveKeyFromTaskMap(std::unordered_map<SoftwareImageDecodeCache::CacheKey, scoped_refptr<TileTask>, SoftwareImageDecodeCache::CacheKeyHash>* task_map,
    ...
    {
        task_map_->erase(*key_);
    }

private:
    raw_ptr<std::unordered_map<SoftwareImageDecodeCache::CacheKey, scoped_refptr<TileTask>, SoftwareImageDecodeCache::CacheKeyHash>> task_map_;
    const raw_ref<const SoftwareImageDecodeCache::CacheKey> key_;
};

class SoftwareImageDecodeTaskImpl : public TileTask {
public:
    SoftwareImageDecodeTaskImpl(SoftwareImageDecodeCache* cache, const SoftwareImageDecodeCache::CacheKey& image_key, const PaintImage& paint_image,
        ImageDecodeCache::TaskType task_type, const ImageDecodeCache::TracingInfo& tracing_info)
    ...
    SoftwareImageDecodeTaskImpl& operator=(const SoftwareImageDecodeTaskImpl&) = delete;

    // Overridden from Task:
    void RunOnWorkerThread() override
    {
        TRACE_EVENT2("cc", "SoftwareImageDecodeTaskImpl::RunOnWorkerThread", "mode", "software", "source_prepare_tiles_id", tracing_info_.prepare_tiles_id);
        ...
        devtools_instrumentation::ScopedImageDecodeTask image_decode_task(paint_image_.GetSwSkImage().get(),
            devtools_instrumentation::ScopedImageDecodeTask::DecodeType::kSoftware, ImageDecodeCache::ToScopedTaskType(task_type_),
            ImageDecodeCache::ToScopedImageType(image_type));
        SoftwareImageDecodeCache::TaskProcessingResult result = cache_->DecodeImageInTask(image_key_, paint_image_, task_type_);
...
SkSize GetScaleAdjustment(const SoftwareImageDecodeCache::CacheKey& key)
{
    ...
    if (key.type() != SoftwareImageDecodeCache::CacheKey::kSubrectAndScale) {
        ...
        return MipMapUtil::GetScaleAdjustmentForSize(key.src_rect().size(), key.target_size());
    }
}
...
// neighbor).
PaintFlags::FilterQuality GetDecodedFilterQuality(const SoftwareImageDecodeCache::CacheKey& key)
{
    return key.is_nearest_neighbor() ? PaintFlags::FilterQuality::kNone : PaintFlags::FilterQuality::kLow;
}

} // namespace

SoftwareImageDecodeCache::SoftwareImageDecodeCache(SkColorType color_type, size_t locked_memory_limit_bytes)
...
//             this, "cc::SoftwareImageDecodeCache", base::SingleThreadTaskRunner::GetCurrentDefault());
//     }
}
...
ImageDecodeCache::TaskResult SoftwareImageDecodeCache::GetTaskForImageAndRef(ClientId client_id, const DrawImage& image, const TracingInfo& tracing_info)
{
    DCHECK_EQ(client_id, ImageDecodeCache::kDefaultClientId) << "SoftwareImageDecodeCache cannot be shared between multiple clients.";
    return GetTaskForImageAndRefInternal(image, tracing_info, TaskType::kInRaster);
}

ImageDecodeCache::TaskResult SoftwareImageDecodeCache::GetOutOfRasterDecodeTaskForImageAndRef(ClientId client_id, const DrawImage& image)
{
    DCHECK_EQ(client_id, ImageDecodeCache::kDefaultClientId) << "SoftwareImageDecodeCache cannot be shared between multiple clients.";
    return GetTaskForImageAndRefInternal(image, TracingInfo(0, TilePriority::NOW), TaskType::kOutOfRaster);
}

ImageDecodeCache::TaskResult SoftwareImageDecodeCache::GetTaskForImageAndRefInternal(
    const DrawImage& image, const TracingInfo& tracing_info, TaskType task_type)
{
    CacheKey key = CacheKey::FromDrawImage(image, GetColorTypeForPaintImage(image.target_color_params(), image.paint_image()));
    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "SoftwareImageDecodeCache::GetTaskForImageAndRefInternal", "key", key.ToString());
    ...
    scoped_refptr<TileTask>& task = task_type == TaskType::kInRaster ? cache_entry->in_raster_task : cache_entry->out_of_raster_task;
    if (!task) {
        // Ref image once for the decode task.
        ++cache_entry->ref_count;
        task = base::MakeRefCounted<SoftwareImageDecodeTaskImpl>(this, key, image.paint_image(), task_type, tracing_info);
        ...
        } else if (task_type == TaskType::kOutOfRaster && cache_entry->in_raster_task && !cache_entry->in_raster_task->HasCompleted()) {
...
void SoftwareImageDecodeCache::AddBudgetForImage(const CacheKey& key, CacheEntry* entry)
{
    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "SoftwareImageDecodeCache::AddBudgetForImage", "key", key.ToString());
...
void SoftwareImageDecodeCache::RemoveBudgetForImage(const CacheKey& key, CacheEntry* entry)
{
    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "SoftwareImageDecodeCache::RemoveBudgetForImage", "key", key.ToString());
...
{
    const CacheKey& key = CacheKey::FromDrawImage(image, GetColorTypeForPaintImage(image.target_color_params(), image.paint_image()));
    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "SoftwareImageDecodeCache::UnrefImage", "key", key.ToString());
...
SoftwareImageDecodeCache::TaskProcessingResult SoftwareImageDecodeCache::DecodeImageInTask(
    const CacheKey& key, const PaintImage& paint_image, TaskType task_type)
{
    TRACE_EVENT1("cc,benchmark", "SoftwareImageDecodeCache::DecodeImageInTask", "key", key.ToString());
    base::AutoLock lock(lock_);
    ...
    TaskProcessingResult result = DecodeImageIfNecessary(key, paint_image, cache_entry);
    DCHECK(cache_entry->decode_failed || cache_entry->is_locked);
    return result;
}

SoftwareImageDecodeCache::TaskProcessingResult SoftwareImageDecodeCache::DecodeImageIfNecessary(
    const CacheKey& key, const PaintImage& paint_image, CacheEntry* entry)
{
    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "SoftwareImageDecodeCache::DecodeImageIfNecessary", "key", key.ToString());
    DCHECK_GT(entry->ref_count, 0);
    ...
    if (key.type() == CacheKey::kOriginal) {
        ...
        local_cache_entry = Utils::DoDecodeImage(key, paint_image, GetColorTypeForPaintImage(key.target_color_params(), paint_image), generator_client_id_,
            base::BindOnce(&SoftwareImageDecodeCache::ClearCache, base::Unretained(this)));
    } else {
        ...
        const bool should_decode_to_scale =
            ...
            // is requested.
            key.src_rect() == gfx::Rect(paint_image.width(), paint_image.height()) &&
        ...
        if (!should_decode_to_scale && !candidate_key) {
            ...
            auto src_rect
                = key.type() == CacheKey::kSubrectOriginal ? SkIRect::MakeWH(paint_image.width(), paint_image.height()) : gfx::RectToSkIRect(key.src_rect());
            DrawImage candidate_draw_image(
                paint_image, false, src_rect, PaintFlags::FilterQuality::kNone, SkM44(), key.frame_key().frame_index(), key.target_color_params());
            candidate_key.emplace(CacheKey::FromDrawImage(candidate_draw_image, GetColorTypeForPaintImage(key.target_color_params(), paint_image)));
        }

        if (candidate_key) {
            CHECK(*candidate_key != key) << key.ToString();
            auto decoded_draw_image = GetDecodedImageForDrawInternal(*candidate_key, paint_image);
            if (!decoded_draw_image.image()) {
                ...
                local_cache_entry = Utils::GenerateCacheEntryFromCandidate(
                    key, decoded_draw_image, candidate_key->type() == CacheKey::kOriginal, GetColorTypeForPaintImage(key.target_color_params(), paint_image));
            }
...
std::optional<SoftwareImageDecodeCache::CacheKey> SoftwareImageDecodeCache::FindCachedCandidate(const CacheKey& key)
{
    ...
    // won't be empty.
    CHECK(image_keys_it != frame_key_to_image_keys_.end(), base::NotFatalUntil::M130);

    auto& available_keys = image_keys_it->second;
    std::sort(available_keys.begin(), available_keys.end(), [](const CacheKey& one, const CacheKey& two) {
        // Return true if |one| scale is less than |two| scale.
        return one.target_size().width() < two.target_size().width() && one.target_size().height() < two.target_size().height();
    });
    ...
    for (auto& available_key : available_keys) {
        ...
        if (available_key.target_size().width() < key.target_size().width() || available_key.target_size().height() < key.target_size().height()) {
            continue;
        }
...
bool SoftwareImageDecodeCache::UseCacheForDrawImage(const DrawImage& draw_image) const
{
    ...
    if (dst_color_space.IsValid() && dst_color_space != gfx::ColorSpace::CreateSRGB()) {
        return true;
    }
...
DecodedDrawImage SoftwareImageDecodeCache::GetDecodedImageForDraw(const DrawImage& draw_image)
{
    DCHECK(UseCacheForDrawImage(draw_image));

    base::AutoLock hold(lock_);
    return GetDecodedImageForDrawInternal(
        CacheKey::FromDrawImage(draw_image, GetColorTypeForPaintImage(draw_image.target_color_params(), draw_image.paint_image())), draw_image.paint_image());
}

DecodedDrawImage SoftwareImageDecodeCache::GetDecodedImageForDrawInternal(const CacheKey& key, const PaintImage& paint_image)
{
    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "SoftwareImageDecodeCache::GetDecodedImageForDrawInternal", "key", key.ToString());
    ...
    auto decoded_draw_image = DecodedDrawImage(
        std::move(decoded_image), nullptr, cache_entry->src_rect_offset(), GetScaleAdjustment(key), GetDecodedFilterQuality(key), cache_entry->is_budgeted);
    return decoded_draw_image;
}

void SoftwareImageDecodeCache::DrawWithImageFinished(const DrawImage& image, const DecodedDrawImage& decoded_image)
{
    DCHECK(UseCacheForDrawImage(image));
    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "SoftwareImageDecodeCache::DrawWithImageFinished", "key",
        CacheKey::FromDrawImage(image, GetColorTypeForPaintImage(image.target_color_params(), image.paint_image())).ToString());
    UnrefImage(image);
}

void SoftwareImageDecodeCache::ReduceCacheUsageUntilWithinLimit(size_t limit)
{
    TRACE_EVENT0("cc", "SoftwareImageDecodeCache::ReduceCacheUsageUntilWithinLimit");
    for (auto it = decoded_images_.rbegin(); decoded_images_.size() > limit && it != decoded_images_.rend();) {
...
void SoftwareImageDecodeCache::OnImageDecodeTaskCompleted(const CacheKey& key, TaskType task_type)
{
    base::AutoLock hold(lock_);
    ...
    UMA_HISTOGRAM_BOOLEAN("Compositing.DecodeLCPCandidateImage.Software", key.may_be_lcp_candidate());
    auto& task = task_type == TaskType::kInRaster ? cache_entry->in_raster_task : cache_entry->out_of_raster_task;
    task = nullptr;

    UnrefImage(key);
}

bool SoftwareImageDecodeCache::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    base::AutoLock lock(lock_);
    ...
    if (args.level_of_detail == MemoryDumpLevelOfDetail::kBackground) {
        ...
        dump->AddScalar("locked_size", MemoryAllocatorDump::kUnitsBytes, locked_images_budget_.GetCurrentUsageSafe());
    } else {
        for (const auto& image_pair : decoded_images_) {
            ...
            std::string dump_name = base::StringPrintf("cc/image_memory/cache_0x%" PRIXPTR "/%s/image_%" PRIu64 "_id_%d", reinterpret_cast<uintptr_t>(this),
                entry->is_budgeted ? "budgeted" : "at_raster", entry->tracing_id(), image_id);
            // CreateMemoryAllocatorDump will automatically add tracking values for
            // the total size. We also add a "locked_size" below.
            MemoryAllocatorDump* dump = entry->memory->CreateMemoryAllocatorDump(dump_name.c_str(), pmd);
            DCHECK(dump);
            size_t locked_bytes = entry->is_locked ? image_pair.first.locked_bytes() : 0u;
            dump->AddScalar("locked_size", MemoryAllocatorDump::kUnitsBytes, locked_bytes);
        }
    }
...
SoftwareImageDecodeCache::CacheEntry* SoftwareImageDecodeCache::AddCacheEntry(const CacheKey& key)
{
...
SkColorType SoftwareImageDecodeCache::GetColorTypeForPaintImage(const TargetColorParams& target_color_params, const PaintImage& paint_image)
{
    ...
    if (paint_image.GetColorType() == kRGBA_F16_SkColorType && paint_image.GetContentColorUsage() == gfx::ContentColorUsage::kHDR
