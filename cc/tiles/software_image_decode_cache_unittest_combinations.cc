...
public:
    ...
    {
        cache_ = CreateCache();
        *const_cast<ImageDecodeCache::ClientId*>(&image_cache_client_id_) = cache_->GenerateClientId();
    ...
    DrawImage CreateDrawImageForScale(float scale, const SkIRect src_rect = SkIRect::MakeEmpty())
    {
        return DrawImage(paint_image(), false, src_rect.isEmpty() ? SkIRect::MakeWH(paint_image().width(), paint_image().height()) : src_rect,
            PaintFlags::FilterQuality::kMedium, CreateMatrix(SkSize::Make(scale, scale), true), PaintImage::kDefaultFrameIndex, GetTargetColorParams());
    }
...
protected:
    ...
    virtual void VerifyEntryExists(int line, const DrawImage& draw_image, const gfx::Size& expected_size) = 0;
...
protected:
    ...
    {
        return std::make_unique<SoftwareImageDecodeCache>(kN32_SkColorType, kLockedMemoryLimitBytes);
    }
};
...
protected:
    std::unique_ptr<SoftwareImageDecodeCache> CreateCache() override
    {
        return std::make_unique<SoftwareImageDecodeCache>(kARGB_4444_SkColorType, kLockedMemoryLimitBytes);
    }
};
...
protected:
    std::unique_ptr<SoftwareImageDecodeCache> CreateCache() override
    {
        return std::make_unique<SoftwareImageDecodeCache>(kRGBA_F16_SkColorType, kLockedMemoryLimitBytes);
    }
};
...
protected:
    ...
    void VerifyEntryExists(int line, const DrawImage& draw_image, const gfx::Size& expected_size) override
    {
...
protected:
    CacheEntryResult GenerateCacheEntry(const DrawImage& image) override
    {
        auto task_result = cache().GetTaskForImageAndRef(cache_client_id(), image, ImageDecodeCache::TracingInfo());
    ...
    {
        auto decoded = cache().GetDecodedImageForDraw(draw_image);
        EXPECT_TRUE(SkColorSpace::Equals(decoded.image()->colorSpace(), draw_image.target_color_space().ToSkColorSpace().get()));
        SCOPED_TRACE(base::StringPrintf("Failure from line %d", line));
        EXPECT_EQ(decoded.image()->width(), expected_size.width());
        EXPECT_EQ(decoded.image()->height(), expected_size.height());
        cache().DrawWithImageFinished(draw_image, decoded);
...
protected:
    PaintImage CreatePaintImage(const gfx::Size& size) override
    {
        PaintImage paint_image = CreateDiscardablePaintImage(
            size, GetColorSpace().ToSkColorSpace(), true /*allocate_encoded_memory*/, PaintImage::kInvalidId, kRGBA_F16_SkColorType);
        return paint_image;
    }
};
...
protected:
    TargetColorParams GetTargetColorParams() const override
    {
        return TargetColorParams(gfx::ColorSpace(gfx::ColorSpace::PrimaryID::XYZ_D50, gfx::ColorSpace::TransferID::SRGB));
    }
};
...
protected:
    TargetColorParams GetTargetColorParams() const override
    {
        return TargetColorParams(gfx::ColorSpace(gfx::ColorSpace::PrimaryID::P3, gfx::ColorSpace::TransferID::LINEAR));
    }
};
...
protected:
    TargetColorParams GetTargetColorParams() const override
    {
        TargetColorParams result(gfx::ColorSpace(gfx::ColorSpace::PrimaryID::P3, gfx::ColorSpace::TransferID::SRGB_HDR));
        result.hdr_max_luminance_relative = 4.f;
        return result;
    }
};

class SoftwareImageDecodeCacheTest_Typical : public N32Cache, public Predecode, public NoDecodeToScaleSupport, public DefaultColorSpace { };
...
TEST_F(SoftwareImageDecodeCacheTest_Typical, UseClosestAvailableDecodeNotSmaller)
{
...
TEST_F(SoftwareImageDecodeCacheTest_Typical, UseClosestAvailableDecodeFirstImageSubrected)
{
...
TEST_F(SoftwareImageDecodeCacheTest_Typical, UseClosestAvailableDecodeSecondImageSubrected)
{
    auto draw_image_50 = CreateDrawImageForScale(0.5f);
    auto result = GenerateCacheEntry(draw_image_50);
    EXPECT_TRUE(result.has_task);
    ...
    auto draw_image_125 = CreateDrawImageForScale(0.125f, SkIRect::MakeWH(400, 400));
    result = GenerateCacheEntry(draw_image_125);
    EXPECT_TRUE(result.has_task);
    EXPECT_TRUE(result.needs_unref);
...
TEST_F(SoftwareImageDecodeCacheTest_Typical, UseClosestAvailableDecodeBothSubrected)
{
...
TEST_F(SoftwareImageDecodeCacheTest_Typical, UseClosestAvailableDecodeBothPastPostScaleSize)
{
    auto draw_image_50 = CreateDrawImageForScale(0.5f, SkIRect::MakeXYWH(300, 300, 52, 52));
    auto result = GenerateCacheEntry(draw_image_50);
    EXPECT_TRUE(result.has_task);
    EXPECT_TRUE(result.needs_unref);
    ...
    auto draw_image_25 = CreateDrawImageForScale(0.25, SkIRect::MakeXYWH(300, 300, 52, 52));
...
class SoftwareImageDecodeCacheTest_AtRaster : public N32Cache, public AtRaster, public NoDecodeToScaleSupport, public DefaultColorSpace { };
...
TEST_F(SoftwareImageDecodeCacheTest_AtRaster, UseClosestAvailableDecodeSubrected)
{
    auto draw_image_50 = CreateDrawImageForScale(0.5f, SkIRect::MakeWH(500, 500));
    auto decoded = cache().GetDecodedImageForDraw(draw_image_50);
    {
...
class SoftwareImageDecodeCacheTest_ExoticColorSpace : public N32Cache, public Predecode, public NoDecodeToScaleSupport, public ExoticColorSpace { };

TEST_F(SoftwareImageDecodeCacheTest_ExoticColorSpace, UseClosestAvailableDecode)
{
    auto draw_image_50 = CreateDrawImageForScale(0.5f);
    auto result = GenerateCacheEntry(draw_image_50);
...
class SoftwareImageDecodeCacheTest_F16_ExoticColorSpace : public RGBA_F16Cache, public Predecode, public NoDecodeToScaleSupportF16, public ExoticColorSpace { };

TEST_F(SoftwareImageDecodeCacheTest_F16_ExoticColorSpace, UseClosestAvailableDecode_F16_ExoticColorSpace)
{
    auto draw_image_50 = CreateDrawImageForScale(0.5f);
    auto result = GenerateCacheEntry(draw_image_50);
    EXPECT_TRUE(result.has_task);
...
class SoftwareImageDecodeCacheTest_F16_WideGamutCanvasColorSpace : public RGBA_F16Cache,
                                                                   public Predecode,
                                                                   public NoDecodeToScaleSupportF16,
                                                                   public WideGamutCanvasColorSpace { };

TEST_F(SoftwareImageDecodeCacheTest_F16_WideGamutCanvasColorSpace, UseClosestAvailableDecode_F16_WideGamutCanvasColorSpace)
{
    auto draw_image_50 = CreateDrawImageForScale(0.5f);
    auto result = GenerateCacheEntry(draw_image_50);
    EXPECT_TRUE(result.has_task);
...
protected:
    ...
    {
        PaintImage paint_image = CreateDiscardablePaintImage(
            size, gfx::ColorSpace::CreateHDR10().ToSkColorSpace(), true /*allocate_encoded_memory*/, PaintImage::kInvalidId, kN32_SkColorType);
        return paint_image;
    }
};

class SoftwareImageDecodeCacheTest_N32HDR : public N32Cache, public Predecode, public PaintImageN32HDR, public HdrCanvasColorSpace { };
...
protected:
    ...
    {
        PaintImage paint_image = CreateDiscardablePaintImage(
            size, gfx::ColorSpace::CreateHDR10().ToSkColorSpace(), true /*allocate_encoded_memory*/, PaintImage::kInvalidId, kRGBA_F16_SkColorType);
        return paint_image;
    }
};

class SoftwareImageDecodeCacheTest_F16HDR : public N32Cache, public Predecode, public PaintImageF16HDR, public HdrCanvasColorSpace { };
