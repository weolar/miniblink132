...
public:
    ...
    {
        auto it = base::ranges::find(locked_ids_, id, &LockedIds::value_type::first);
    ...
    {
        auto id = next_id_++;
        locked_ids_.insert(std::make_pair(id, SoftwareImageDecodeCache::CacheKey::FromDrawImage(image, kRGBA_8888_SkColorType)));
    ...
    {
        SoftwareImageDecodeCache::CacheKey key = SoftwareImageDecodeCache::CacheKey::FromDrawImage(image, kRGBA_8888_SkColorType);
        return base::Contains(locked_ids_, key, &LockedIds::value_type::second);
    }

    size_t num_locked_images()
    {
        return locked_ids_.size();
    }

private:
    using LockedIds = std::unordered_map<ImageDecodeRequestId, SoftwareImageDecodeCache::CacheKey>;
...
public:
    ...
    {
        decoded_image_tracker_.SetTickClockForTesting(task_runner_->GetMockTickClock());
    }

    TestImageController* image_controller()
    {
        return &image_controller_;
    }
...
{
    bool locked = false;
    decoded_image_tracker()->QueueImageDecode(CreateDiscardablePaintImage(gfx::Size(1, 1)), TargetColorParams(),
        base::BindOnce([](bool* locked, bool success) { *locked = true; }, base::Unretained(&locked)));
    EXPECT_TRUE(locked);
...
{
    bool locked = false;
    gfx::ColorSpace decoded_color_space(gfx::ColorSpace::PrimaryID::XYZ_D50, gfx::ColorSpace::TransferID::SRGB);
    ...
    decoded_image_tracker()->QueueImageDecode(
        paint_image, target_color_params, base::BindOnce([](bool* locked, bool success) { *locked = true; }, base::Unretained(&locked)));
    ...
    DrawImage locked_draw_image(
        paint_image, false, SkIRect::MakeWH(1, 1), PaintFlags::FilterQuality::kHigh, SkM44(), PaintImage::kDefaultFrameIndex, target_color_params);
    EXPECT_TRUE(image_controller()->IsDrawImageLocked(locked_draw_image));
    DrawImage srgb_draw_image(
        paint_image, false, SkIRect::MakeWH(1, 1), PaintFlags::FilterQuality::kHigh, SkM44(), PaintImage::kDefaultFrameIndex, srgb_target_color_params);
...
TEST_F(DecodedImageTrackerTest, ImageUsedInDraw)
{
    ...
    decoded_image_tracker()->QueueImageDecode(
        paint_image_1, TargetColorParams(), base::BindOnce([](bool* locked, bool success) { *locked = true; }, base::Unretained(&locked)));
    EXPECT_TRUE(locked);
    EXPECT_EQ(1u, image_controller()->num_locked_images());

    auto paint_image_2 = CreateDiscardablePaintImage(gfx::Size(1, 1));
    decoded_image_tracker()->QueueImageDecode(
        paint_image_2, target_color_params, base::BindOnce([](bool* locked, bool success) { *locked = true; }, base::Unretained(&locked)));
    EXPECT_TRUE(locked);
    EXPECT_EQ(2u, image_controller()->num_locked_images());

    // Create dummy draw images for each:
    DrawImage draw_image_1(paint_image_1, false, SkIRect::MakeWH(1, 1), PaintFlags::FilterQuality::kHigh, SkM44(), 0, target_color_params);
    DrawImage draw_image_2(paint_image_2, false, SkIRect::MakeWH(1, 1), PaintFlags::FilterQuality::kHigh, SkM44(), 0, target_color_params);
