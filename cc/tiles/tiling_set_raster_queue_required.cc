...
// static
std::unique_ptr<TilingSetRasterQueueRequired> TilingSetRasterQueueRequired::Create(PictureLayerTilingSet* tiling_set, RasterTilePriorityQueue::Type type)
{
    DCHECK_NE(static_cast<int>(type), static_cast<int>(RasterTilePriorityQueue::Type::ALL));
...
TilingSetRasterQueueRequired::TilingSetRasterQueueRequired(PictureLayerTiling* tiling, RasterTilePriorityQueue::Type type)
    : type_(type)
{
    DCHECK(tiling);
    ...
    if (type == RasterTilePriorityQueue::Type::REQUIRED_FOR_ACTIVATION) {
        iterator_ = TilingIterator(tiling, &tiling->tiling_data_, tiling->pending_visible_rect());
    } else {
        iterator_ = TilingIterator(tiling, &tiling->tiling_data_, tiling->current_visible_rect());
    }
...
bool TilingSetRasterQueueRequired::IsTileRequired(const PrioritizedTile& prioritized_tile) const
{
    return (type_ == RasterTilePriorityQueue::Type::REQUIRED_FOR_ACTIVATION && prioritized_tile.tile()->required_for_activation())
...
TilingSetRasterQueueRequired::TilingIterator::TilingIterator(PictureLayerTiling* tiling, TilingData* tiling_data, const gfx::Rect& rect)
    : tiling_(tiling)
    , tiling_data_(tiling_data)
{
    ...
    if (tile && tile->draw_info().NeedsRaster() && !tiling_->IsTileOccluded(tile)) {
        current_tile_ = tiling_->MakePrioritizedTile(tile, tiling_->ComputePriorityRectTypeForTile(tile), false);
        return;
    }
    ++(*this);
}
...
TilingSetRasterQueueRequired::TilingIterator& TilingSetRasterQueueRequired::TilingIterator::operator++()
{
    Tile* tile = nullptr;
    while (true) {
        ++visible_iterator_;
