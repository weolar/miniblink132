...
public:
    ...
    void InitializeFrameSink() override
    {
        ...
        tile_manager()->SetTileTaskManagerForTesting(std::make_unique<FakeTileTaskManagerImpl>());
    }

    void SetupDefaultTreesWithFixedTileSize(const gfx::Size& layer_bounds, const gfx::Size& tile_size)
    {
        scoped_refptr<FakeRasterSource> pending_raster_source = FakeRasterSource::CreateFilledWithImages(layer_bounds);
        scoped_refptr<FakeRasterSource> active_raster_source = FakeRasterSource::CreateFilledWithImages(layer_bounds);
    ...
    perf_test::PerfResultReporter SetUpReporter(const std::string& story_name)
    {
        ...
        reporter.RegisterImportantMetric("_raster_tile_queue_construct_and_iterate", "runs/s");
        reporter.RegisterImportantMetric("_eviction_tile_queue_construct", "runs/s");
        reporter.RegisterImportantMetric("_eviction_tile_queue_construct_and_iterate", "runs/s");
        return reporter;
    }

    void RunRasterQueueConstructTest(const std::string& test_name, int layer_count)
    {
        TreePriority priorities[] = { SAME_PRIORITY_FOR_BOTH_TREES, SMOOTHNESS_TAKES_PRIORITY, NEW_CONTENT_TAKES_PRIORITY };
        ...
        timer_.Reset();
        do {
            std::unique_ptr<RasterTilePriorityQueue> queue(host_impl()->BuildRasterQueue(priorities[priority_count], RasterTilePriorityQueue::Type::ALL));
    ...
    void RunRasterQueueConstructAndIterateTest(const std::string& test_name, int layer_count, int tile_count)
    {
        TreePriority priorities[] = { SAME_PRIORITY_FOR_BOTH_TREES, SMOOTHNESS_TAKES_PRIORITY, NEW_CONTENT_TAKES_PRIORITY };
        ...
        perf_test::PerfResultReporter reporter = SetUpReporter(test_name);
        reporter.AddResult("_raster_tile_queue_construct_and_iterate", timer_.LapsPerSecond());
    }

    void RunEvictionQueueConstructTest(const std::string& test_name, int layer_count)
    {
        TreePriority priorities[] = { SAME_PRIORITY_FOR_BOTH_TREES, SMOOTHNESS_TAKES_PRIORITY, NEW_CONTENT_TAKES_PRIORITY };
        int priority_count = 0;
        ...
        for (auto* layer : layers) {
            ...
            for (size_t i = 0; i < layer->num_tilings(); ++i) {
                tile_manager()->InitializeTilesWithResourcesForTesting(layer->tilings()->tiling_at(i)->AllTilesForTesting());
            }
        }

        timer_.Reset();
        do {
            std::unique_ptr<EvictionTilePriorityQueue> queue(host_impl()->BuildEvictionQueue(priorities[priority_count]));
            priority_count = (priority_count + 1) % std::size(priorities);
            timer_.NextLap();
        } while (!timer_.HasTimeLimitExpired());

        perf_test::PerfResultReporter reporter = SetUpReporter(test_name);
        reporter.AddResult("_eviction_tile_queue_construct", timer_.LapsPerSecond());
    }

    void RunEvictionQueueConstructAndIterateTest(const std::string& test_name, int layer_count, int tile_count)
    {
        TreePriority priorities[] = { SAME_PRIORITY_FOR_BOTH_TREES, SMOOTHNESS_TAKES_PRIORITY, NEW_CONTENT_TAKES_PRIORITY };
        int priority_count = 0;
        ...
        perf_test::PerfResultReporter reporter = SetUpReporter(test_name);
        reporter.AddResult("_eviction_tile_queue_construct_and_iterate", timer_.LapsPerSecond());
    }

    std::vector<FakePictureLayerImpl*> CreateLayers(int layer_count, int num_tiles_in_high_res)
    {
        ...
        scoped_refptr<FakeRasterSource> raster_source = FakeRasterSource::CreateFilledWithImages(layer_bounds);
        while (static_cast<int>(layers.size()) < layer_count) {
            auto* child_layer = AddLayer<FakePictureLayerImpl>(host_impl()->pending_tree(), raster_source);
    ...
    void RunPrepareTilesTest(const std::string& test_name, int layer_count, int approximate_tile_count_per_layer)
    {
