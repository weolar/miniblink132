...
PictureLayerTiling::PictureLayerTiling(WhichTree tree, const gfx::AxisTransform2d& raster_transform, scoped_refptr<RasterSource> raster_source,
    PictureLayerTilingClient* client, float min_preraster_distance, float max_preraster_distance, bool can_use_lcd_text)
    ...
    , can_use_lcd_text_(can_use_lcd_text)
{
...
#if DCHECK_IS_ON()
    gfx::SizeF scaled_source_size(
        gfx::ScaleSize(gfx::SizeF(raster_source_->recorded_bounds().size()), raster_transform.scale().x(), raster_transform.scale().y()));
    gfx::Size floored_size = gfx::ToFlooredSize(scaled_source_size);
    bool is_width_empty = !floored_size.width() && !MathUtil::IsWithinEpsilon(scaled_source_size.width(), 1.f);
    bool is_height_empty = !floored_size.height() && !MathUtil::IsWithinEpsilon(scaled_source_size.height(), 1.f);
    DCHECK(!is_width_empty && !is_height_empty) << "Tiling created with scale too small as contents become empty."
    ...
    tiling_data_.SetMaxTextureSize(client_->CalculateTileSize(tiling_rect.size()));
}
...
{
    const PictureLayerTiling* active_twin = tree_ == PENDING_TREE ? client_->GetPendingOrActiveTwinTiling(this) : nullptr;
    const Region* invalidation = active_twin ? client_->GetPendingInvalidation() : nullptr;

    bool include_borders = false;
    for (TilingData::Iterator iter(&tiling_data_, live_tiles_rect_, include_borders); iter; ++iter) {
            ...
            if (tile && invalidation && TilingMatchesTileIndices(active_twin)) {
                if (const Tile* old_tile = active_twin->TileAt(index)) {
                    ...
                    for (gfx::Rect rect : *invalidation) {
                        gfx::Rect invalid_content_rect = EnclosingContentsRectFromLayerRect(rect);
...
void PictureLayerTiling::TakeTilesAndPropertiesFrom(PictureLayerTiling* pending_twin, const Region& layer_invalidation)
{
    ...
    ComputeTilePriorityRects(pending_twin->current_visible_rect_in_layer_space_, pending_twin->current_skewport_rect_in_layer_space_,
        pending_twin->current_soon_border_rect_in_layer_space_, pending_twin->current_eventually_rect_in_layer_space_,
        pending_twin->current_content_to_screen_scale_ * contents_scale_key(), pending_twin->current_occlusion_in_layer_space_);
...
bool PictureLayerTiling::SetRasterSourceAndResize(scoped_refptr<RasterSource> raster_source)
{
    DCHECK(!raster_source->IsSolidColor());
    ...
    if (tile_size != tiling_data_.max_texture_size() || tiling_rect.origin() != tiling_data_.tiling_rect().origin()) {
        SetTilingRect(tiling_rect);
    ...
    // change.
    int before_eventually_left = tiling_data_.TileXIndexFromSrcCoord(current_eventually_rect_.x());
    int before_eventually_top = tiling_data_.TileYIndexFromSrcCoord(current_eventually_rect_.y());
    int before_eventually_right = tiling_data_.TileXIndexFromSrcCoord(current_eventually_rect_.right() - 1);
    int before_eventually_bottom = tiling_data_.TileYIndexFromSrcCoord(current_eventually_rect_.bottom() - 1);

    int before_live_left = tiling_data_.TileXIndexFromSrcCoord(live_tiles_rect_.x());
    int before_live_top = tiling_data_.TileYIndexFromSrcCoord(live_tiles_rect_.y());
    int before_live_right = tiling_data_.TileXIndexFromSrcCoord(live_tiles_rect_.right() - 1);
    int before_live_bottom = tiling_data_.TileYIndexFromSrcCoord(live_tiles_rect_.bottom() - 1);
    ...
    if (!live_tiles_rect_.IsEmpty()) {
        after_live_right = tiling_data_.TileXIndexFromSrcCoord(live_tiles_rect_.right() - 1);
        after_live_bottom = tiling_data_.TileYIndexFromSrcCoord(live_tiles_rect_.bottom() - 1);
    }
    ...
    for (int i = before_eventually_left; i <= after_eventually_right; ++i) {
        for (int j = after_eventually_bottom + 1; j <= before_eventually_bottom; ++j) {
            TakeTileAt(i, j);
        }
    }
...
void PictureLayerTiling::RemoveTilesInRegion(const Region& layer_invalidation, bool recreate_tiles)
{
    ...
    gfx::Rect expanded_eventually_rect = tiling_data_.ExpandRectToTileBounds(current_eventually_rect_);
    for (gfx::Rect layer_rect : layer_invalidation) {
        // The pixels which are invalid in content space.
        gfx::Rect invalid_content_rect = EnclosingContentsRectFromLayerRect(layer_rect);
        ...
        for (TilingData::Iterator iter(&tiling_data_, coverage_content_rect, include_borders); iter; ++iter) {
...
Tile::CreateInfo PictureLayerTiling::CreateInfoForTile(int i, int j) const
{
    ...
    gfx::Rect enclosing_layer_rect = EnclosingLayerRectFromContentsRect(tile_rect);
    return Tile::CreateInfo { this, i, j, enclosing_layer_rect, tile_rect, raster_transform_, can_use_lcd_text_ };
}

bool PictureLayerTiling::ShouldCreateTileAt(const Tile::CreateInfo& info) const
{
    const int i = info.tiling_i_index;
    const int j = info.tiling_j_index;
    ...
    if (can_use_lcd_text() != active_twin->can_use_lcd_text() || raster_transform() != active_twin->raster_transform())
        return true;
...
bool PictureLayerTiling::TilingMatchesTileIndices(const PictureLayerTiling* twin) const
{
    return tiling_data_.max_texture_size() == twin->tiling_data_.max_texture_size() && tiling_rect().origin() == twin->tiling_rect().origin();
}
...
void PictureLayerTiling::SetTilePriorityRectsForTesting(
    const gfx::Rect& visible_rect, const gfx::Rect& skewport_rect, const gfx::Rect& soon_border_rect, const gfx::Rect& eventually_rect, bool evicts_tiles)
{
    current_occlusion_in_layer_space_ = Occlusion();
    current_content_to_screen_scale_ = 1.0;

    SetPriorityRect(EnclosingLayerRectFromContentsRect(visible_rect), VISIBLE_RECT);
    SetPriorityRect(EnclosingLayerRectFromContentsRect(skewport_rect), SKEWPORT_RECT);
    SetPriorityRect(EnclosingLayerRectFromContentsRect(soon_border_rect), SOON_BORDER_RECT);
    SetPriorityRect(EnclosingLayerRectFromContentsRect(eventually_rect), EVENTUALLY_RECT, evicts_tiles);
    ...
    max_skewport_extent_in_screen_space_
        = std::max({ current_visible_rect_.x() - current_skewport_rect_.x(), current_skewport_rect_.right() - current_visible_rect_.right(),
            current_visible_rect_.y() - current_skewport_rect_.y(), current_skewport_rect_.bottom() - current_visible_rect_.bottom() });
}
...
void PictureLayerTiling::ComputeTilePriorityRects(const gfx::Rect& visible_rect_in_layer_space, const gfx::Rect& skewport_rect_in_layer_space,
    const gfx::Rect& soon_border_rect_in_layer_space, const gfx::Rect& eventually_rect_in_layer_space, float ideal_contents_scale,
    const Occlusion& occlusion_in_layer_space)
{
    ...
    TileMemoryLimitPolicy memory_limit_policy = client_->global_tile_state().memory_limit_policy;

    current_occlusion_in_layer_space_ = occlusion_in_layer_space;
    current_content_to_screen_scale_ = ideal_contents_scale / contents_scale_key();
    ...
    // "skewport extent". Also note that this math can't produce negative numbers,
    // since skewport.Contains(visible_rect) is always true.
    max_skewport_extent_in_screen_space_ = current_content_to_screen_scale_
        * std::max({ current_visible_rect_.x() - current_skewport_rect_.x(), current_skewport_rect_.right() - current_visible_rect_.right(),
            current_visible_rect_.y() - current_skewport_rect_.y(), current_skewport_rect_.bottom() - current_visible_rect_.bottom() });
...
void PictureLayerTiling::SetPriorityRect(const gfx::Rect& rect_in_layer_space, PriorityRectType rect_type, bool evicts_tiles)
{
    ...
    case VISIBLE_RECT:
        if (current_visible_rect_in_layer_space_ != rect_in_layer_space) {
            ...
            current_visible_rect_ = EnclosingContentsRectFromLayerRect(rect_in_layer_space);
    ...
    case SKEWPORT_RECT:
        if (current_skewport_rect_in_layer_space_ != rect_in_layer_space) {
            ...
            current_skewport_rect_ = EnclosingContentsRectFromLayerRect(rect_in_layer_space);
    ...
    case SOON_BORDER_RECT:
        if (current_soon_border_rect_in_layer_space_ != rect_in_layer_space) {
            ...
            current_soon_border_rect_ = EnclosingContentsRectFromLayerRect(rect_in_layer_space);
    ...
    case EVENTUALLY_RECT:
        if (current_eventually_rect_in_layer_space_ != rect_in_layer_space) {
            ...
            gfx::Rect rect = EnclosingContentsRectFromLayerRect(rect_in_layer_space);
            if (evicts_tiles) {
                // Iterate to delete all tiles outside of our new live_tiles rect.
                for (TilingData::DifferenceIterator iter(&tiling_data_, current_eventually_rect_, rect); iter; ++iter) {
...
void PictureLayerTiling::SetLiveTilesRect(const gfx::Rect& new_live_tiles_rect)
{
    DCHECK(new_live_tiles_rect.IsEmpty() || tiling_rect().Contains(new_live_tiles_rect))
        << "tiling_rect: " << tiling_rect().ToString() << " new_live_tiles_rect: " << new_live_tiles_rect.ToString();
    ...
    for (TilingData::DifferenceIterator iter(&tiling_data_, new_live_tiles_rect, live_tiles_rect_); iter; ++iter) {
...
void PictureLayerTiling::VerifyTiles() const
{
    ...
    for (auto it = tiles_.begin(); it != tiles_.end(); ++it) {
        ...
        DCHECK(index.i < tiling_data_.num_tiles_x()) << this << " " << index.i << "," << index.j << " num_tiles_x " << tiling_data_.num_tiles_x()
                                                     << " eventually_rect " << current_eventually_rect_.ToString();
        DCHECK(index.j < tiling_data_.num_tiles_y()) << this << " " << index.i << "," << index.j << " num_tiles_y " << tiling_data_.num_tiles_y()
                                                     << " eventually_rect " << current_eventually_rect_.ToString();
        DCHECK(tiling_data_.TileBounds(index.i, index.j).Intersects(current_eventually_rect_))
            << this << " " << index.i << "," << index.j << " tile bounds " << tiling_data_.TileBounds(index.i, index.j).ToString() << " eventually_rect "
...
bool PictureLayerTiling::ShouldDecodeCheckeredImagesForTile(const Tile* tile) const
{
...
void PictureLayerTiling::UpdateRequiredStatesOnTile(Tile* tile) const
{
    tile->set_required_for_activation(IsTileRequiredForActivation(
        tile, [this](const Tile* tile) { return IsTileVisible(tile); }, IsTileOccluded(tile)));
    tile->set_required_for_draw(IsTileRequiredForDraw(tile, [this](const Tile* tile) { return IsTileVisible(tile); }));
}

PictureLayerTiling::CoverageIterator PictureLayerTiling::Cover(const gfx::Rect& rect, float scale) const
{
    return CoverageIterator(this, scale, rect);
}

PrioritizedTile PictureLayerTiling::MakePrioritizedTile(Tile* tile, PriorityRectType priority_rect_type, bool is_tile_occluded) const
{
    DCHECK(tile);
    DCHECK(raster_source()->IntersectsRect(tile->enclosing_layer_rect()))
        << "Recording rect: " << EnclosingLayerRectFromContentsRect(tile->content_rect()).ToString();

    tile->set_required_for_activation(IsTileRequiredForActivation(
        tile, [priority_rect_type](const Tile*) { return priority_rect_type == VISIBLE_RECT; }, is_tile_occluded));
    tile->set_required_for_draw(IsTileRequiredForDraw(tile, [priority_rect_type](const Tile*) { return priority_rect_type == VISIBLE_RECT; }));

    const auto& tile_priority = ComputePriorityForTile(tile, priority_rect_type, is_tile_occluded);
    DCHECK((!tile->required_for_activation() && !tile->required_for_draw()) || tile_priority.priority_bin == TilePriority::NOW
    ...
    bool process_for_images_only = tile_priority.distance_to_visible > min_preraster_distance_
        && (tile_priority.distance_to_visible > max_preraster_distance_ || tile_priority.distance_to_visible > 0.5f * max_skewport_extent_in_screen_space_);

    return PrioritizedTile(tile, this, tile_priority, is_tile_occluded, process_for_images_only, ShouldDecodeCheckeredImagesForTile(tile));
}

void PictureLayerTiling::CreateAllTilesForTesting(const gfx::Rect& rect_to_raster)
{
...
std::map<const Tile*, PrioritizedTile> PictureLayerTiling::UpdateAndGetAllPrioritizedTilesForTesting() const
{
    std::map<const Tile*, PrioritizedTile> result;
    for (const auto& index_tile_pair : tiles_) {
        Tile* tile = index_tile_pair.second.get();
        PrioritizedTile prioritized_tile = MakePrioritizedTile(tile, ComputePriorityRectTypeForTile(tile), IsTileOccluded(tile));
        result.insert(std::make_pair(prioritized_tile.tile(), prioritized_tile));
    }
    return result;
}

TilePriority PictureLayerTiling::ComputePriorityForTile(const Tile* tile, PriorityRectType priority_rect_type, bool is_tile_occluded) const
{
...
PictureLayerTiling::PriorityRectType PictureLayerTiling::ComputePriorityRectTypeForTile(const Tile* tile) const
{
    DCHECK_EQ(TileAt(tile->tiling_i_index(), tile->tiling_j_index()), tile);
    gfx::Rect tile_bounds = tiling_data_.TileBounds(tile->tiling_i_index(), tile->tiling_j_index());
...
void PictureLayerTiling::GetAllPrioritizedTilesForTracing(std::vector<PrioritizedTile>* prioritized_tiles) const
{
    for (const auto& tile_pair : tiles_) {
        Tile* tile = tile_pair.second.get();
        prioritized_tiles->push_back(MakePrioritizedTile(tile, ComputePriorityRectTypeForTile(tile), IsTileOccluded(tile)));
    }
}

void PictureLayerTiling::AsValueInto(base::trace_event::TracedValue* state) const
{
    ...
    MathUtil::AddToTracedValue("eventually_rect", current_eventually_rect_, state);
...
gfx::Rect PictureLayerTiling::EnclosingContentsRectFromLayerRect(const gfx::Rect& layer_rect) const
{
    return ToEnclosingRect(raster_transform_.MapRect(gfx::RectF(layer_rect)));
}

gfx::Rect PictureLayerTiling::EnclosingLayerRectFromContentsRect(const gfx::Rect& contents_rect) const
{
    return ToEnclosingRect(raster_transform_.InverseMapRect(gfx::RectF(contents_rect)));
}
...
gfx::Rect PictureLayerTiling::ComputeTilingRect() const
{
    ...
    if (recorded_bounds != layer_bounds) {
        gfx::Rect layer_contents_rect = EnclosingContentsRectFromLayerRect(layer_bounds);
        // Snap tiling_rect to avoid full tiling invalidation on small change of
        // tiling rect origin.
        constexpr int kSnapTexels = 128;
        tiling_rect.SetByBounds(MathUtil::UncheckedRoundDown(tiling_rect.x(), kSnapTexels), MathUtil::UncheckedRoundDown(tiling_rect.y(), kSnapTexels),
            // Snap to the layer edge if the tiling edge is near the layer edge.
            tiling_rect.right() + kSnapTexels > layer_contents_rect.right() ? layer_contents_rect.right() : tiling_rect.right(),
            tiling_rect.bottom() + kSnapTexels > layer_contents_rect.bottom() ? layer_contents_rect.bottom() : tiling_rect.bottom());
