...
public:
    ...
    TestImageController()
        : ImageController(base::SingleThreadTaskRunner::GetCurrentDefault().get(), base::SingleThreadTaskRunner::GetCurrentDefault(), base::DoNothing())
    {
        SetMaxImageCacheLimitBytesForTesting(kMaxImageCacheSizeBytes);
    }

    ~TestImageController() override
    {
        DCHECK_EQ(locked_images_.size(), 0U);
    }

    int num_of_locked_images() const
    {
        return locked_images_.size();
    }
    ...
    ImageDecodeRequestId QueueImageDecode(const DrawImage& image, ImageDecodedCallback callback) override
    {
        ...
        // ImageController.
        worker_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(callback), request_id, ImageDecodeResult::SUCCESS));
...
class CheckerImageTrackerTest : public testing::Test, public CheckerImageTrackerClient {
public:
    enum class ImageType { CHECKERABLE, SMALL_NON_CHECKERABLE, LARGE_NON_CHECKERABLE };

    void SetUpTracker(bool checker_images_enabled)
    {
        size_t size_to_checker = 512 * 1024;
        checker_image_tracker_ = std::make_unique<CheckerImageTracker>(&image_controller_, this, checker_images_enabled, size_to_checker);
        checker_image_tracker_->SetMaxDecodePriorityAllowed(CheckerImageTracker::DecodeType::kPreDecode);
    }

    void TearDown() override
    {
        checker_image_tracker_.reset();
    }

    DrawImage CreateImage(ImageType image_type, PaintImage::AnimationType animation = PaintImage::AnimationType::kStatic,
        PaintImage::CompletionState completion = PaintImage::CompletionState::kDone, bool is_multipart = false)
    {
        int dimension = 0;
        switch (image_type) {
        ...
        checker_image_tracker_->UpdateImageDecodingHints({ { id, PaintImage::DecodingMode::kAsync } });
        return DrawImage(PaintImageBuilder::WithDefault()
                             ...
                             .TakePaintImage(),
            false, SkIRect::MakeWH(dimension, dimension), PaintFlags::FilterQuality::kNone, SkM44(), PaintImage::kDefaultFrameIndex, kDefaultTargetColorParams);
    }
    ...
    CheckerImageTracker::ImageDecodeQueue BuildImageDecodeQueue(std::vector<DrawImage> images, WhichTree tree)
    {
        CheckerImageTracker::ImageDecodeQueue decode_queue;
        for (const auto& image : images) {
            if (ShouldCheckerImage(image, tree))
                decode_queue.push_back(CheckerImageTracker::ImageDecodeRequest(image.paint_image(), CheckerImageTracker::DecodeType::kRaster));
        }
        return decode_queue;
    }
...
TEST_F(CheckerImageTrackerTest, UpdatesImagesAtomically)
{
    ...
    DrawImage small_non_checkerable_image = CreateImage(ImageType::SMALL_NON_CHECKERABLE);
    DrawImage large_non_checkerable_image = CreateImage(ImageType::LARGE_NON_CHECKERABLE);
    CheckerImageTracker::ImageDecodeQueue image_decode_queue;

    // First request to filter images.
    std::vector<DrawImage> draw_images = { checkerable_image, small_non_checkerable_image, large_non_checkerable_image, checkerable_image };
    image_decode_queue = BuildImageDecodeQueue(draw_images, WhichTree::PENDING_TREE);

    ASSERT_EQ(2u, image_decode_queue.size());
    EXPECT_TRUE(checkerable_image.paint_image().IsSameForTesting(image_decode_queue[0].paint_image));
    EXPECT_TRUE(checkerable_image.paint_image().IsSameForTesting(image_decode_queue[1].paint_image));
    ...
    PaintImageIdFlatSet invalidated_images = checker_image_tracker_->TakeImagesToInvalidateOnSyncTree();
    EXPECT_EQ(invalidated_images.size(), 1U);
    EXPECT_EQ(invalidated_images.count(checkerable_image.paint_image().stable_id()), 1U);
    ...
    EXPECT_FALSE(ShouldCheckerImage(small_non_checkerable_image, WhichTree::PENDING_TREE));
    EXPECT_FALSE(ShouldCheckerImage(large_non_checkerable_image, WhichTree::PENDING_TREE));
    ...
    EXPECT_FALSE(ShouldCheckerImage(small_non_checkerable_image, WhichTree::ACTIVE_TREE));
    EXPECT_FALSE(ShouldCheckerImage(large_non_checkerable_image, WhichTree::ACTIVE_TREE));
...
TEST_F(CheckerImageTrackerTest, NoConsecutiveCheckeringForImage)
{
    ...
    CheckerImageTracker::ImageDecodeQueue image_decode_queue = BuildImageDecodeQueue(draw_images, WhichTree::PENDING_TREE);
...
TEST_F(CheckerImageTrackerTest, TracksCheckeredImagesSeperatelyInConsecutiveFrames)
{
    ...
    PaintImageIdFlatSet invalidated_images = checker_image_tracker_->TakeImagesToInvalidateOnSyncTree();
    EXPECT_EQ(invalidated_images.size(), 1U);
    EXPECT_EQ(invalidated_images.count(checkerable_image1.paint_image().stable_id()), 1U);
...
TEST_F(CheckerImageTrackerTest, CancelsScheduledDecodes)
{
    ...
    checker_image_tracker_->ScheduleImageDecodeQueue(std::move(image_decode_queue));

    // Only the first image in the queue should have been decoded.
    EXPECT_EQ(image_controller_.decodes_requested().size(), 1U);
    EXPECT_EQ(image_controller_.decodes_requested().count(checkerable_image1.paint_image().stable_id()), 1U);
    ...
    EXPECT_EQ(image_controller_.decodes_requested().count(checkerable_image3.paint_image().stable_id()), 1U);
...
TEST_F(CheckerImageTrackerTest, ClearsTracker)
{
    ...
    DrawImage checkerable_image = CreateImage(ImageType::CHECKERABLE);
    CheckerImageTracker::ImageDecodeQueue image_decode_queue = BuildImageDecodeQueue({ checkerable_image }, WhichTree::PENDING_TREE);
    EXPECT_EQ(image_decode_queue.size(), 1U);
    checker_image_tracker_->ScheduleImageDecodeQueue(std::move(image_decode_queue));
    base::RunLoop().RunUntilIdle();
    checker_image_tracker_->TakeImagesToInvalidateOnSyncTree();
    ...
    image_decode_queue = BuildImageDecodeQueue({ checkerable_image }, WhichTree::PENDING_TREE);
    ...
    checker_image_tracker_->UpdateImageDecodingHints({ { checkerable_image.paint_image().stable_id(), PaintImage::DecodingMode::kAsync } });
    image_decode_queue = BuildImageDecodeQueue({ checkerable_image }, WhichTree::PENDING_TREE);
    EXPECT_EQ(image_decode_queue.size(), 1U);
...
TEST_F(CheckerImageTrackerTest, CheckersOnlyStaticCompletedImages)
{
    ...
    DrawImage animated_image = CreateImage(ImageType::CHECKERABLE, PaintImage::AnimationType::kAnimated);
    DrawImage partial_image = CreateImage(ImageType::CHECKERABLE, PaintImage::AnimationType::kStatic, PaintImage::CompletionState::kPartiallyDone);
    DrawImage video_image = CreateImage(ImageType::CHECKERABLE, PaintImage::AnimationType::kVideo);
    std::vector<DrawImage> draw_images = { static_image, animated_image, partial_image, video_image };

    CheckerImageTracker::ImageDecodeQueue image_decode_queue = BuildImageDecodeQueue(draw_images, WhichTree::PENDING_TREE);
    EXPECT_EQ(image_decode_queue.size(), 1U);
    EXPECT_TRUE(image_decode_queue[0].paint_image.IsSameForTesting(static_image.paint_image()));

    // Change the partial image to complete and try again. It should sstill not
    // be checkered.
    gfx::Size image_size = gfx::Size(partial_image.paint_image().width(), partial_image.paint_image().height());
    DrawImage completed_paint_image = DrawImage(PaintImageBuilder::WithDefault()
                                                    .set_id(partial_image.paint_image().stable_id())
                                                    .set_paint_image_generator(CreatePaintImageGenerator(image_size))
                                                    .TakePaintImage(),
        false, SkIRect::MakeWH(image_size.width(), image_size.height()), PaintFlags::FilterQuality::kNone, SkM44(), PaintImage::kDefaultFrameIndex,
        kDefaultTargetColorParams);
    EXPECT_FALSE(ShouldCheckerImage(completed_paint_image, WhichTree::PENDING_TREE));
}
...
TEST_F(CheckerImageTrackerTest, ChoosesMaxScaleAndQuality)
{
    ...
    DrawImage image = CreateImage(ImageType::CHECKERABLE);
    DrawImage scaled_image1(image, 0.5f, PaintImage::kDefaultFrameIndex, TargetColorParams());
    DrawImage scaled_image2 = DrawImage(image.paint_image(), false, image.src_rect(), PaintFlags::FilterQuality::kHigh, SkM44::Scale(1.8f, 1.8f),
    ...
    EXPECT_EQ(image_controller_.decoded_images()[0].scale(), SkSize::Make(1.8f, 1.8f));
    EXPECT_EQ(image_controller_.decoded_images()[0].filter_quality(), PaintFlags::FilterQuality::kHigh);
}
...
TEST_F(CheckerImageTrackerTest, DontCheckerMultiPartImages)
{
    ...
    DrawImage multi_part_image = CreateImage(ImageType::CHECKERABLE, PaintImage::AnimationType::kStatic, PaintImage::CompletionState::kDone, true);
...
TEST_F(CheckerImageTrackerTest, RespectsDecodePriority)
{
    ...
    CheckerImageTracker::ImageDecodeQueue image_decode_queue = BuildImageDecodeQueue({ image1, image2, image3, image4 }, WhichTree::PENDING_TREE);
    ...
    base::RunLoop().RunUntilIdle();
    EXPECT_THAT(image_controller_.decoded_images(), ImagesAreSame({ image1, image2 }));

    // All decodes allowed. The complete queue should be flushed.
    checker_image_tracker_->SetMaxDecodePriorityAllowed(CheckerImageTracker::DecodeType::kPreDecode);
    base::RunLoop().RunUntilIdle();
    EXPECT_THAT(image_controller_.decoded_images(), ImagesAreSame({ image1, image2, image3, image4 }));
}
...
TEST_F(CheckerImageTrackerTest, UseSrcRectForSize)
{
    ...
    DrawImage image = CreateImage(ImageType::CHECKERABLE);
    image = DrawImage(
        image.paint_image(), false, SkIRect::MakeWH(200, 200), image.filter_quality(), SkM44(), PaintImage::kDefaultFrameIndex, image.target_color_params());
    EXPECT_FALSE(ShouldCheckerImage(image, WhichTree::PENDING_TREE));
}
...
TEST_F(CheckerImageTrackerTest, DecodingModeHints)
{
    ...
    base::flat_map<PaintImage::Id, PaintImage::DecodingMode> hints
        = { { 1, PaintImage::DecodingMode::kUnspecified }, { 2, PaintImage::DecodingMode::kSync }, { 3, PaintImage::DecodingMode::kAsync } };
    checker_image_tracker_->UpdateImageDecodingHints(hints);

    EXPECT_EQ(PaintImage::DecodingMode::kUnspecified, checker_image_tracker_->get_decoding_mode_hint_for_testing(1));
    EXPECT_EQ(PaintImage::DecodingMode::kSync, checker_image_tracker_->get_decoding_mode_hint_for_testing(2));
    EXPECT_EQ(PaintImage::DecodingMode::kAsync, checker_image_tracker_->get_decoding_mode_hint_for_testing(3));

    hints = { { 1, PaintImage::DecodingMode::kAsync }, { 2, PaintImage::DecodingMode::kAsync }, { 3, PaintImage::DecodingMode::kAsync } };
    checker_image_tracker_->UpdateImageDecodingHints(hints);
    ...
    hints = { { 1, PaintImage::DecodingMode::kUnspecified }, { 2, PaintImage::DecodingMode::kUnspecified }, { 3, PaintImage::DecodingMode::kUnspecified } };
    checker_image_tracker_->UpdateImageDecodingHints(hints);

    EXPECT_EQ(PaintImage::DecodingMode::kUnspecified, checker_image_tracker_->get_decoding_mode_hint_for_testing(1));
    EXPECT_EQ(PaintImage::DecodingMode::kSync, checker_image_tracker_->get_decoding_mode_hint_for_testing(2));
    EXPECT_EQ(PaintImage::DecodingMode::kUnspecified, checker_image_tracker_->get_decoding_mode_hint_for_testing(3));

    hints = { { 1, PaintImage::DecodingMode::kSync } };
    checker_image_tracker_->UpdateImageDecodingHints(hints);

    EXPECT_EQ(PaintImage::DecodingMode::kSync, checker_image_tracker_->get_decoding_mode_hint_for_testing(1));
    EXPECT_EQ(PaintImage::DecodingMode::kSync, checker_image_tracker_->get_decoding_mode_hint_for_testing(2));
    EXPECT_EQ(PaintImage::DecodingMode::kUnspecified, checker_image_tracker_->get_decoding_mode_hint_for_testing(3));
}
...
TEST_F(CheckerImageTrackerTest, UsageHintsMakeImagesSync)
{
    ...
    base::flat_map<PaintImage::Id, PaintImage::DecodingMode> hints = { { image.paint_image().stable_id(), PaintImage::DecodingMode::kSync } };
    checker_image_tracker_->UpdateImageDecodingHints(hints);
