...
public:
    ...
    bool required_for_draw() const
    {
        return required_for_draw_;
    }
    ...
    {
        return !!(flags_ & IS_OPAQUE);
    }

    void AsValueInto(base::trace_event::TracedValue* value) const;

    const TileDrawInfo& draw_info() const
    {
        return draw_info_;
    }
    ...
    {
        return draw_info().IsReadyToDraw();
    }

    size_t GPUMemoryUsageInBytes() const;

    const gfx::Size& desired_texture_size() const
    {
        return content_rect_.size();
    }
    ...
    void SetInvalidated(const gfx::Rect& invalid_content_rect, Id previous_tile_id)
    {
        invalidated_content_rect_ = invalid_content_rect;
        invalidated_id_ = previous_tile_id;
    }

    Id invalidated_id() const
    {
        return invalidated_id_;
    }
    ...
    void set_tiling(const PictureLayerTiling* tiling)
    {
        tiling_ = tiling;
    }
...
private:
    ...
    Tile(TileManager* tile_manager, const CreateInfo& info, int layer_id, int source_frame_number, int flags);
    ...
    std::vector<std::pair<const gfx::Rect, const gfx::AxisTransform2d>> raster_rects_;
