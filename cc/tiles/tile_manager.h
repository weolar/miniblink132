...
public:
    ...
    virtual std::unique_ptr<RasterTilePriorityQueue> BuildRasterQueue(TreePriority tree_priority, RasterTilePriorityQueue::Type type) = 0;
    ...
    // Note if the queue was previously built, Reset must be called on it.
    virtual std::unique_ptr<EvictionTilePriorityQueue> BuildEvictionQueue(TreePriority tree_priority) = 0;

    // Returns an iterator over all the tiles that have a resource.
    virtual std::unique_ptr<TilesWithResourceIterator> CreateTilesWithResourceIterator() = 0;
    ...
    virtual TargetColorParams GetTargetColorParams(gfx::ContentColorUsage content_color_usage) const = 0;
    ...
    virtual size_t GetFrameIndexForImage(const PaintImage& paint_image, WhichTree tree) const = 0;

    // Returns the sample count to use if MSAA is enabled for a tile.
    virtual int GetMSAASampleCountForRaster(const DisplayItemList& display_list) const = 0;
...
class CC_EXPORT TileManager : CheckerImageTrackerClient, public base::trace_event::MemoryDumpProvider {
public:
    TileManager(TileManagerClient* client, base::SequencedTaskRunner* origin_task_runner, scoped_refptr<base::SequencedTaskRunner> image_worker_task_runner,
        size_t scheduled_raster_task_limit, bool running_on_renderer_process, const TileManagerSettings& tile_manager_settings);
    ...
    // SetResources.
    void SetResources(ResourcePool* resource_pool, ImageDecodeCache* image_decode_cache, TaskGraphRunner* task_graph_runner,
    ...
    std::unique_ptr<Tile> CreateTile(const Tile::CreateInfo& info, int layer_id, int source_frame_number, int flags);
    ...
    std::unique_ptr<base::trace_event::ConvertableToTraceFormat> BasicStateAsValue() const;
    ...
    void InitializeTilesWithResourcesForTesting(const std::vector<Tile*>& tiles)
    {
        for (size_t i = 0; i < tiles.size(); ++i) {
            ...
            ResourcePool::InUsePoolResource resource = resource_pool_->AcquireResource(tiles[i]->desired_texture_size(), raster_buffer_provider_->GetFormat(),
                client_->GetTargetColorParams(gfx::ContentColorUsage::kSRGB).color_space);
            ...
            if (resource.gpu_backing()) {
                ...
                resource.gpu_backing()->mailbox_sync_token.Set(gpu::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(1), 1);
            }
            bool exported = resource_pool_->PrepareForExport(resource, viz::TransferableResource::ResourceSource::kTest);
            DCHECK(exported);
    ...
    void SetRasterBufferProviderForTesting(RasterBufferProvider* raster_buffer_provider)
    {
        raster_buffer_provider_ = raster_buffer_provider;
    }

    void SetPendingRasterQueriesForTesting(RasterQueryQueue* pending_raster_queries)
    {
    ...
    void OnRasterTaskCompleted(Tile::Id tile_id, ResourcePool::InUsePoolResource resource, bool was_canceled);
    ...
    std::unique_ptr<base::trace_event::ConvertableToTraceFormat> ActivationStateAsValue();
    ...
    void set_active_url(const GURL& url)
    {
        active_url_ = url;
    }

    void SetOverridesForTesting(scoped_refptr<base::TaskRunner> task_runner_for_testing, const base::TickClock* clock);
...
private:
    ...
    public:
        ...
        static MemoryUsage FromConfig(const gfx::Size& size, viz::SharedImageFormat format);
    ...
    std::unique_ptr<EvictionTilePriorityQueue> FreeTileResourcesUntilUsageIsWithinLimit(
        std::unique_ptr<EvictionTilePriorityQueue> eviction_priority_queue, const MemoryUsage& limit, MemoryUsage* usage);
    std::unique_ptr<EvictionTilePriorityQueue> FreeTileResourcesWithLowerPriorityUntilUsageIsWithinLimit(
        std::unique_ptr<EvictionTilePriorityQueue> eviction_priority_queue, const MemoryUsage& limit, const TilePriority& oother_priority, MemoryUsage* usage);
    ...
    void MarkTilesOutOfMemory(std::unique_ptr<RasterTilePriorityQueue> queue) const;
    ...
    void ExternalDependencyCompletedForRasterTask(scoped_refptr<TileTask> dependent);
    void ExternalDependencyCompletedForNonRasterTask(scoped_refptr<TileTask> dependent);

    scoped_refptr<TileTask> CreateTaskSetFinishedTask(void (TileManager::*callback)());
    PrioritizedWorkToSchedule AssignGpuMemoryToTiles();
    void ScheduleTasks(PrioritizedWorkToSchedule work_to_schedule);

    void PartitionImagesForCheckering(const PrioritizedTile& prioritized_tile, const TargetColorParams& target_color_params,
    ...
    void AddCheckeredImagesToDecodeQueue(const PrioritizedTile& prioritized_tile, const TargetColorParams& target_color_params,
        CheckerImageTracker::DecodeType decode_type, CheckerImageTracker::ImageDecodeQueue* image_decode_queue);

    std::unique_ptr<base::trace_event::ConvertableToTraceFormat> ScheduledTasksStateAsValue() const;
...
private:
    void InsertNodesForRasterTask(TileTask* raster_task, uint16_t priority, bool use_foreground_category);
    void InsertNodeForDecodeTask(TileTask* task, uint16_t priority, bool use_foreground_category);
    ...
    base::WeakPtrFactory<TileManager> ready_to_draw_callback_weak_ptr_factory_ { this };
};
