...
Tile::Tile(TileManager* tile_manager, const CreateInfo& info, int layer_id, int source_frame_number, int flags)
...
Tile::~Tile()
{
    TRACE_EVENT_OBJECT_DELETED_WITH_ID(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "cc::Tile", this);
    tile_manager_->Release(this);
}

void Tile::AsValueInto(base::trace_event::TracedValue* value) const
{
    viz::TracedValue::MakeDictIntoImplicitSnapshotWithCategory(TRACE_DISABLED_BY_DEFAULT("cc.debug"), value, "cc::Tile", this);
    ...
    value->SetBoolean("is_using_gpu_memory", draw_info().has_resource() || HasRasterTask());
    value->SetInteger("scheduled_priority", scheduled_priority_);
    value->SetBoolean("use_picture_analysis", use_picture_analysis());
    value->SetInteger("gpu_memory_usage", base::saturated_cast<int>(GPUMemoryUsageInBytes()));
}
...
size_t Tile::GPUMemoryUsageInBytes() const
{
    if (draw_info_.resource_) {
        ...
        return draw_info_.resource_shared_image_format().EstimatedSizeInBytes(draw_info_.resource_size());
    }
    return 0;
}
