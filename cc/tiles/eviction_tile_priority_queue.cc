...
public:
    ...
    bool operator()(const std::unique_ptr<TilingSetEvictionQueue>& a_queue, const std::unique_ptr<TilingSetEvictionQueue>& b_queue) const
    {
        ...
        if (b_priority.priority_bin == a_priority.priority_bin && b_queue->is_drawing_layer() != a_queue->is_drawing_layer()) {
...
void CreateTilingSetEvictionQueues(const std::vector<raw_ptr<PictureLayerImpl, VectorExperimental>>& layers, TreePriority tree_priority,
    std::vector<std::unique_ptr<TilingSetEvictionQueue>>* queues)
{
    ...
    for (PictureLayerImpl* layer : layers) {
        std::unique_ptr<TilingSetEvictionQueue> tiling_set_queue
            = std::make_unique<TilingSetEvictionQueue>(layer->picture_layer_tiling_set(), layer->contributes_to_drawn_render_surface());
        // Queues will only contain non empty tiling sets.
        if (!tiling_set_queue->IsEmpty())
            queues->push_back(std::move(tiling_set_queue));
    }
    std::make_heap(queues->begin(), queues->end(), EvictionOrderComparator(tree_priority));
}
...
void EvictionTilePriorityQueue::Build(const std::vector<raw_ptr<PictureLayerImpl, VectorExperimental>>& active_layers,
    const std::vector<raw_ptr<PictureLayerImpl, VectorExperimental>>& pending_layers, TreePriority tree_priority)
{
    tree_priority_ = tree_priority;

    CreateTilingSetEvictionQueues(active_layers, tree_priority, &active_queues_);
    CreateTilingSetEvictionQueues(pending_layers, tree_priority, &pending_queues_);
}
...
void EvictionTilePriorityQueue::Pop()
{
    ...
    std::pop_heap(next_queues.begin(), next_queues.end(), EvictionOrderComparator(tree_priority_));
    ...
    if (queue->IsEmpty()) {
        ...
        std::push_heap(next_queues.begin(), next_queues.end(), EvictionOrderComparator(tree_priority_));
    }
}

std::vector<std::unique_ptr<TilingSetEvictionQueue>>& EvictionTilePriorityQueue::GetNextQueues()
{
    ...
    return const_cast<std::vector<std::unique_ptr<TilingSetEvictionQueue>>&>(const_queues);
}

const std::vector<std::unique_ptr<TilingSetEvictionQueue>>& EvictionTilePriorityQueue::GetNextQueues() const
{
    DCHECK(!IsEmpty());
    ...
    if (active_priority.priority_bin == pending_priority.priority_bin
        && active_tile.tile()->required_for_activation() != pending_tile.tile()->required_for_activation()) {
        return active_tile.tile()->required_for_activation() ? pending_queues_ : active_queues_;
    }
