...
std::unique_ptr<TileTaskManagerImpl> TileTaskManagerImpl::Create(
    TaskGraphRunner* task_graph_runner, base::RepeatingCallback<void(scoped_refptr<TileTask>)> notify_external_dependent)
{
    return base::WrapUnique<TileTaskManagerImpl>(new TileTaskManagerImpl(task_graph_runner, std::move(notify_external_dependent)));
}

TileTaskManagerImpl::TileTaskManagerImpl(TaskGraphRunner* task_graph_runner, base::RepeatingCallback<void(scoped_refptr<TileTask>)> notify_external_dependent)
...
void TileTaskManagerImpl::ExternalDependencyCompletedForTask(scoped_refptr<TileTask> task)
{
    task_graph_runner_->ExternalDependencyCompletedForTask(namespace_token_, std::move(task));
}
