...
public:
    TestablePictureLayerTilingSet(WhichTree tree, PictureLayerTilingClient* client, int tiling_interest_area_padding, float skewport_target_time_in_seconds,
...
std::unique_ptr<TestablePictureLayerTilingSet> CreateTilingSetWithSettings(PictureLayerTilingClient* client, const LayerTreeSettings& settings)
{
    return std::make_unique<TestablePictureLayerTilingSet>(ACTIVE_TREE, client, settings.tiling_interest_area_padding, settings.skewport_target_time_in_seconds,
        settings.skewport_extrapolation_limit_in_screen_pixels, settings.max_preraster_distance_in_screen_pixels);
}

std::unique_ptr<TestablePictureLayerTilingSet> CreateTilingSet(PictureLayerTilingClient* client)
{
...
TEST(PictureLayerTilingSetTest, TilingRange)
{
    ...
    low_res_tiling = set->AddTiling(gfx::AxisTransform2d(0.25, gfx::Vector2dF()), raster_source);
    ...
    set_without_low_res->AddTiling(gfx::AxisTransform2d(2.0, gfx::Vector2dF()), raster_source);
    high_res_tiling = set_without_low_res->AddTiling(gfx::AxisTransform2d(), raster_source);
    high_res_tiling->set_resolution(HIGH_RESOLUTION);
    set_without_low_res->AddTiling(gfx::AxisTransform2d(0.5, gfx::Vector2dF()), raster_source);
    set_without_low_res->AddTiling(gfx::AxisTransform2d(0.25, gfx::Vector2dF()), raster_source);

    higher_than_high_res_range = set_without_low_res->GetTilingRange(PictureLayerTilingSet::HIGHER_THAN_HIGH_RES);
    EXPECT_EQ(0u, higher_than_high_res_range.start);
    EXPECT_EQ(1u, higher_than_high_res_range.end);
    ...
    between_high_and_low_res_range = set_without_low_res->GetTilingRange(PictureLayerTilingSet::BETWEEN_HIGH_AND_LOW_RES);
    EXPECT_EQ(2u, between_high_and_low_res_range.start);
    ...
    lower_than_low_res_range = set_without_low_res->GetTilingRange(PictureLayerTilingSet::LOWER_THAN_LOW_RES);
    EXPECT_EQ(0u, lower_than_low_res_range.end - lower_than_low_res_range.start);

    std::unique_ptr<TestablePictureLayerTilingSet> set_with_only_high_and_low_res = CreateTilingSet(&client);
    high_res_tiling = set_with_only_high_and_low_res->AddTiling(gfx::AxisTransform2d(), raster_source);
    high_res_tiling->set_resolution(HIGH_RESOLUTION);
    low_res_tiling = set_with_only_high_and_low_res->AddTiling(gfx::AxisTransform2d(0.5, gfx::Vector2dF()), raster_source);
    low_res_tiling->set_resolution(LOW_RESOLUTION);

    higher_than_high_res_range = set_with_only_high_and_low_res->GetTilingRange(PictureLayerTilingSet::HIGHER_THAN_HIGH_RES);
    EXPECT_EQ(0u, higher_than_high_res_range.end - higher_than_high_res_range.start);

    high_res_range = set_with_only_high_and_low_res->GetTilingRange(PictureLayerTilingSet::HIGH_RES);
    EXPECT_EQ(0u, high_res_range.start);
    EXPECT_EQ(1u, high_res_range.end);

    between_high_and_low_res_range = set_with_only_high_and_low_res->GetTilingRange(PictureLayerTilingSet::BETWEEN_HIGH_AND_LOW_RES);
    EXPECT_EQ(0u, between_high_and_low_res_range.end - between_high_and_low_res_range.start);

    low_res_range = set_with_only_high_and_low_res->GetTilingRange(PictureLayerTilingSet::LOW_RES);
    EXPECT_EQ(1u, low_res_range.start);
    EXPECT_EQ(2u, low_res_range.end);

    lower_than_low_res_range = set_with_only_high_and_low_res->GetTilingRange(PictureLayerTilingSet::LOWER_THAN_LOW_RES);
    EXPECT_EQ(0u, lower_than_low_res_range.end - lower_than_low_res_range.start);

    std::unique_ptr<TestablePictureLayerTilingSet> set_with_only_high_res = CreateTilingSet(&client);
    high_res_tiling = set_with_only_high_res->AddTiling(gfx::AxisTransform2d(), raster_source);
    high_res_tiling->set_resolution(HIGH_RESOLUTION);

    higher_than_high_res_range = set_with_only_high_res->GetTilingRange(PictureLayerTilingSet::HIGHER_THAN_HIGH_RES);
    EXPECT_EQ(0u, higher_than_high_res_range.end - higher_than_high_res_range.start);
    ...
    between_high_and_low_res_range = set_with_only_high_res->GetTilingRange(PictureLayerTilingSet::BETWEEN_HIGH_AND_LOW_RES);
    EXPECT_EQ(0u, between_high_and_low_res_range.end - between_high_and_low_res_range.start);

    low_res_range = set_with_only_high_res->GetTilingRange(PictureLayerTilingSet::LOW_RES);
    EXPECT_EQ(0u, low_res_range.end - low_res_range.start);

    lower_than_low_res_range = set_with_only_high_res->GetTilingRange(PictureLayerTilingSet::LOWER_THAN_LOW_RES);
    EXPECT_EQ(0u, lower_than_low_res_range.end - lower_than_low_res_range.start);
}

class PictureLayerTilingSetTestWithResources : public testing::Test {
public:
    void RunTest(int num_tilings, float min_scale, float scale_increment, float ideal_contents_scale, float expected_scale)
    {
        scoped_refptr<viz::TestContextProvider> context_provider = viz::TestContextProvider::CreateRaster();
        ASSERT_EQ(context_provider->BindToCurrentSequence(), gpu::ContextResult::kSuccess);
        std::unique_ptr<viz::ClientResourceProvider> resource_provider = std::make_unique<viz::ClientResourceProvider>();

        FakePictureLayerTilingClient client(resource_provider.get(), context_provider.get());
        ...
        for (int i = 0; i < num_tilings; ++i, scale += scale_increment) {
            PictureLayerTiling* tiling = set->AddTiling(gfx::AxisTransform2d(scale, gfx::Vector2dF()), raster_source);
        ...
        gfx::Size content_bounds(gfx::ScaleToCeiledSize(layer_bounds, max_contents_scale));
        gfx::Rect content_rect(content_bounds);

        Region remaining(content_rect);
        for (auto iter = set->Cover(content_rect, max_contents_scale, ideal_contents_scale); iter; ++iter) {
...
TEST(PictureLayerTilingSetTest, TileSizeChange)
{
    ...
    std::unique_ptr<PictureLayerTilingSet> pending_set = PictureLayerTilingSet::Create(PENDING_TREE, &pending_client, 1000, 1.f, 1000, 1000.f);
    std::unique_ptr<PictureLayerTilingSet> active_set = PictureLayerTilingSet::Create(ACTIVE_TREE, &active_client, 1000, 1.f, 1000, 1000.f);
    ...
    pending_set->UpdateTilePriorities(gfx::Rect(layer_bounds), 1.f, 1.0, Occlusion(), false);
    EXPECT_EQ(tile_size1, pending_set->tiling_at(0)->tile_size());
    ...
    pending_set->UpdateTilingsToCurrentRasterSourceForCommit(raster_source.get(), Region(), 1.f, 1.f);
    // The tiling should get the correct tile size.
    EXPECT_EQ(tile_size2, pending_set->tiling_at(0)->tile_size());

    // Set a priority rect so we get tiles.
    pending_set->UpdateTilePriorities(gfx::Rect(layer_bounds), 1.f, 2.0, Occlusion(), false);
    EXPECT_EQ(tile_size2, pending_set->tiling_at(0)->tile_size());
    ...
    active_set->UpdateTilingsToCurrentRasterSourceForActivation(raster_source.get(), pending_set.get(), Region(), 1.f, 1.f);
    ...
    // Set a priority rect so we get tiles.
    pending_set->UpdateTilePriorities(gfx::Rect(layer_bounds), 1.f, 3.0, Occlusion(), false);
    EXPECT_EQ(tile_size3, pending_set->tiling_at(0)->tile_size());
...
TEST(PictureLayerTilingSetTest, MaxContentScale)
{
    ...
    pending_set->AddTiling(gfx::AxisTransform2d(), raster_source);
    pending_set->AddTiling(gfx::AxisTransform2d(2.f, gfx::Vector2dF()), raster_source);
    pending_set->AddTiling(gfx::AxisTransform2d(3.f, gfx::Vector2dF()), raster_source);
    ...
    pending_set->UpdateTilingsToCurrentRasterSourceForCommit(raster_source.get(), Region(), 1.f, max_content_scale);
    ...
    active_set->UpdateTilingsToCurrentRasterSourceForActivation(raster_source.get(), pending_set.get(), Region(), 1.f, max_content_scale);
...
TEST(PictureLayerTilingSetTest, ComputeSkewportExtremeCases)
{
    ...
    time += 0.016;
    EXPECT_TRUE(tiling_set->ComputeSkewport(viewport2, time, 1.f).Contains(viewport2));
    tiling_set->UpdateTilePriorities(viewport2, 1.f, time, Occlusion(), true);
    time += 0.016;
    EXPECT_TRUE(tiling_set->ComputeSkewport(viewport3, time, 1.f).Contains(viewport3));
    ...
    PictureLayerTiling* tiling = tiling_set->AddTiling(gfx::AxisTransform2d(1000.f, gfx::Vector2dF()), raster_source);
...
TEST(PictureLayerTilingTest, ViewportDistanceWithScale)
{
    ...
    scoped_refptr<FakeRasterSource> raster_source = FakeRasterSource::CreateFilled(layer_bounds);
    std::unique_ptr<TestablePictureLayerTilingSet> tiling_set = CreateTilingSet(&client);
    auto* tiling = tiling_set->AddTiling(gfx::AxisTransform2d(0.25f, gfx::Vector2dF()), raster_source);
    tiling->set_resolution(HIGH_RESOLUTION);
    ...
    for (int i = 0; i < 47; ++i) {
        for (int j = 0; j < 47; ++j) {
            ...
            gfx::Rect tile_rect = tiling->TilingDataForTesting().TileBounds(i, j);
            if (viewport_in_content_space.Intersects(tile_rect)) {
                EXPECT_EQ(TilePriority::NOW, priority.priority_bin) << "i: " << i << " j: " << j;
                EXPECT_FLOAT_EQ(0.f, priority.distance_to_visible) << "i: " << i << " j: " << j;
                have_now = true;
            } else if (skewport_in_content_space.Intersects(tile_rect) || soon_border_rect_in_content_space.Intersects(tile_rect)) {
                EXPECT_EQ(TilePriority::SOON, priority.priority_bin) << "i: " << i << " j: " << j;
                EXPECT_GT(priority.distance_to_visible, 0.f) << "i: " << i << " j: " << j;
                have_soon = true;
            } else {
                EXPECT_EQ(TilePriority::EVENTUALLY, priority.priority_bin) << "i: " << i << " j: " << j;
                EXPECT_GT(priority.distance_to_visible, 0.f) << "i: " << i << " j: " << j;
                have_eventually = true;
            }
        }
    ...
    tiling = tiling_set->AddTiling(gfx::AxisTransform2d(0.2f, gfx::Vector2dF()), raster_source);
    tiling->set_resolution(HIGH_RESOLUTION);
...
TEST(PictureLayerTilingTest, InvalidateAfterComputeTilePriorityRects)
{
    ...
    auto* pending_tiling = tiling_set->AddTiling(gfx::AxisTransform2d(), raster_source);
    ...
    double time = 1.;
    gfx::Rect viewport(0, 0, 100, 100);
    EXPECT_TRUE(tiling_set->UpdateTilePriorities(viewport, 1.f, time, Occlusion(), true));
    EXPECT_FALSE(tiling_set->UpdateTilePriorities(viewport, 1.f, time, Occlusion(), true));
...
TEST(PictureLayerTilingTest, InvalidateAfterUpdateRasterSourceForCommit)
{
    ...
    auto* pending_tiling = pending_set->AddTiling(gfx::AxisTransform2d(), raster_source);
    pending_tiling->set_resolution(HIGH_RESOLUTION);
    active_client.set_twin_tiling_set(pending_set.get());
    ...
    pending_set->UpdateTilingsToCurrentRasterSourceForCommit(raster_source, Region(), 1.f, 1.f);
    // UpdateTilePriorities for pending set gets called during UDP in commit.
    EXPECT_TRUE(pending_set->UpdateTilePriorities(viewport, 1.f, time, Occlusion(), true));
    // The active set doesn't have tilings yet.
    EXPECT_FALSE(active_set->UpdateTilePriorities(viewport, 1.f, time, Occlusion(), true));

    // On activation tilings are copied from pending set to active set.
    active_set->UpdateTilingsToCurrentRasterSourceForActivation(raster_source, pending_set.get(), Region(), 1.f, 1.f);
    // Pending set doesn't have any tilings now.
    EXPECT_FALSE(pending_set->UpdateTilePriorities(viewport, 1.f, time, Occlusion(), true));
    // UpdateTilePriorities for active set gets called during UDP in draw.
    EXPECT_TRUE(active_set->UpdateTilePriorities(viewport, 1.f, time, Occlusion(), true));
...
TEST(PictureLayerTilingSetTest, TilingTranslationChanges)
{
    ...
    std::unique_ptr<PictureLayerTilingSet> pending_set = PictureLayerTilingSet::Create(PENDING_TREE, &pending_client, 0, 1.f, 0, 0.f);
    std::unique_ptr<PictureLayerTilingSet> active_set = PictureLayerTilingSet::Create(ACTIVE_TREE, &active_client, 0, 1.f, 0, 0.f);
    active_client.set_twin_tiling_set(pending_set.get());
    ...
    pending_set->UpdateTilePriorities(gfx::Rect(1, 1), 1.f, 1.0, Occlusion(), false);
