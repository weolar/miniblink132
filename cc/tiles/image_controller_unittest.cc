...
public:
    ~TestableCache() override
    {
        EXPECT_EQ(number_of_refs_, 0);
    }

    TaskResult GetTaskForImageAndRef(uint32_t client_id, const DrawImage& image, const TracingInfo& tracing_info) override
    {
        // Return false for large images to mimic "won't fit in memory"
        // behavior.
        if (image.paint_image() && image.paint_image().width() * image.paint_image().height() >= 1000 * 1000) {
    ...
    TaskResult GetOutOfRasterDecodeTaskForImageAndRef(uint32_t client_id, const DrawImage& image) override
    {
    ...
    int number_of_refs() const
    {
        return number_of_refs_;
    }
    void SetTaskToUse(scoped_refptr<TileTask> task)
    {
        task_to_use_ = task;
    }
...
public:
    ...
    void Callback(base::OnceClosure quit_closure, ImageController::ImageDecodeRequestId id, ImageController::ImageDecodeResult result)
    {
        id_ = id;
        result_ = result;
...
private:
    ...
    ImageController::ImageDecodeResult result_ = ImageController::ImageDecodeResult::FAILURE;
};
...
{
    return DrawImage(CreateDiscardablePaintImage(size), false, SkIRect::MakeWH(size.width(), size.height()), PaintFlags::FilterQuality::kNone, SkM44(),
        PaintImage::kDefaultFrameIndex, TargetColorParams());
}

DrawImage CreateBitmapDrawImage(gfx::Size size)
{
    return DrawImage(CreateBitmapImage(size), false, SkIRect::MakeWH(size.width(), size.height()), PaintFlags::FilterQuality::kNone, SkM44(),
...
public:
    ...
    void SetUp() override
    {
        controller_ = std::make_unique<ImageController>(task_runner_, base::ThreadPool::CreateSequencedTaskRunner(base::TaskTraits()), base::DoNothing());
    ...
    {
        task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&ImageControllerTest::Timeout, weak_ptr_factory_.GetWeakPtr(), base::Unretained(run_loop)),
    ...
    {
        scoped_refptr<TestPaintWorkletInput> input = base::MakeRefCounted<TestPaintWorkletInput>(gfx::SizeF(width, height));
...
TEST_F(ImageControllerTest, QueueImageDecode)
{
    ...
    ImageController::ImageDecodeRequestId expected_id
        = controller()->QueueImageDecode(image(), base::BindOnce(&DecodeClient::Callback, base::Unretained(&decode_client), run_loop.QuitClosure()));
    RunOrTimeout(&run_loop);
    EXPECT_EQ(expected_id, decode_client.id());
    EXPECT_EQ(ImageController::ImageDecodeResult::SUCCESS, decode_client.result());
}
...
TEST_F(ImageControllerTest, QueueImageDecodeNonLazy)
{
    ...
    RunOrTimeout(&run_loop);
    EXPECT_EQ(expected_id, decode_client.id());
    EXPECT_EQ(ImageController::ImageDecodeResult::DECODE_NOT_REQUIRED, decode_client.result());
}
...
TEST_F(ImageControllerTest, QueueImageDecodeTooLarge)
{
    ...
    RunOrTimeout(&run_loop);
    EXPECT_EQ(expected_id, decode_client.id());
    EXPECT_EQ(ImageController::ImageDecodeResult::FAILURE, decode_client.result());
}
...
TEST_F(ImageControllerTest, QueueImageDecodeMultipleImages)
{
    ...
    ImageController::ImageDecodeRequestId expected_id1
        = controller()->QueueImageDecode(image(), base::BindOnce(&DecodeClient::Callback, base::Unretained(&decode_client1), base::DoNothing()));
    DecodeClient decode_client2;
    ImageController::ImageDecodeRequestId expected_id2
        = controller()->QueueImageDecode(image(), base::BindOnce(&DecodeClient::Callback, base::Unretained(&decode_client2), base::DoNothing()));
    DecodeClient decode_client3;
    ImageController::ImageDecodeRequestId expected_id3
        = controller()->QueueImageDecode(image(), base::BindOnce(&DecodeClient::Callback, base::Unretained(&decode_client3), run_loop.QuitClosure()));
    RunOrTimeout(&run_loop);
    EXPECT_EQ(expected_id1, decode_client1.id());
    EXPECT_EQ(ImageController::ImageDecodeResult::SUCCESS, decode_client1.result());
    EXPECT_EQ(expected_id2, decode_client2.id());
    EXPECT_EQ(ImageController::ImageDecodeResult::SUCCESS, decode_client2.result());
    EXPECT_EQ(expected_id3, decode_client3.id());
    EXPECT_EQ(ImageController::ImageDecodeResult::SUCCESS, decode_client3.result());
}
...
TEST_F(ImageControllerTest, QueueImageDecodeChangeControllerWithTaskQueued)
{
    ...
    DecodeClient decode_client2;
    ImageController::ImageDecodeRequestId expected_id2
        = controller()->QueueImageDecode(image(), base::BindOnce(&DecodeClient::Callback, base::Unretained(&decode_client2), run_loop.QuitClosure()));
...
TEST_F(ImageControllerTest, QueueImageDecodeImageAlreadyLocked)
{
    ...
    DecodeClient decode_client1;
    ImageController::ImageDecodeRequestId expected_id1
        = controller()->QueueImageDecode(image(), base::BindOnce(&DecodeClient::Callback, base::Unretained(&decode_client1), run_loop1.QuitClosure()));
    ...
    DecodeClient decode_client2;
    ImageController::ImageDecodeRequestId expected_id2
        = controller()->QueueImageDecode(image(), base::BindOnce(&DecodeClient::Callback, base::Unretained(&decode_client2), run_loop2.QuitClosure()));
...
TEST_F(ImageControllerTest, DispatchesDecodeCallbacksAfterCacheReset)
{
    ...
    EXPECT_EQ(ImageController::ImageDecodeResult::FAILURE, decode_client1.result());
    EXPECT_EQ(ImageController::ImageDecodeResult::FAILURE, decode_client2.result());
}
...
TEST_F(ImageControllerTest, QueueImageDecodeNonLazyCancelImmediately)
{
    ...
    ImageController::ImageDecodeRequestId expected_id1
        = controller()->QueueImageDecode(image1, base::BindOnce(&DecodeClient::Callback, base::Unretained(&decode_client1), base::DoNothing()));
    ImageController::ImageDecodeRequestId expected_id2
        = controller()->QueueImageDecode(image2, base::BindOnce(&DecodeClient::Callback, base::Unretained(&decode_client2), base::DoNothing()));
