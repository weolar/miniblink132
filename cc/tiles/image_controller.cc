...
ImageController::ImageDecodeRequestId ImageController::s_next_image_decode_queue_id_ = 1;

ImageController::ImageController(scoped_refptr<base::SequencedTaskRunner> origin_task_runner, scoped_refptr<base::SequencedTaskRunner> worker_task_runner,
    base::RepeatingCallback<void(scoped_refptr<TileTask>)> notify_external_dependent)
    : worker_task_runner_(std::move(worker_task_runner))
    , notify_external_dependent_(std::move(notify_external_dependent))
{
    worker_state_ = std::make_unique<WorkerState>(std::move(origin_task_runner), weak_ptr_factory_.GetWeakPtr());
}
...
ImageController::~ImageController()
{
    ...
    if (worker_task_runner_) {
        ...
        worker_task_runner_->PostTask(FROM_HERE, base::DoNothingWithBoundArgs(std::move(worker_state_)));
    }
}

ImageController::WorkerState::WorkerState(scoped_refptr<base::SequencedTaskRunner> origin_task_runner, base::WeakPtr<ImageController> weak_ptr)
...
void ImageController::StopWorkerTasks()
{
...

    {
        ...
        if (worker_state_->task_state == WorkerTaskState::kRunningTask) {
            ...
            worker_task_runner_->PostTask(FROM_HERE, base::BindOnce(&CompletionEvent::Signal, base::Unretained(&completion_event)));
...
bool ImageController::HasReadyToRunTask() const
{
    ...
    return base::ranges::any_of(
        worker_state_->image_decode_queue.begin(), worker_state_->image_decode_queue.end(),
        [](const ImageDecodeRequest& request) -> bool { return !request.has_external_dependency; },
...
void ImageController::SetImageDecodeCache(ImageDecodeCache* cache)
{
    ...
    if (!cache) {
        SetPredecodeImages(std::vector<DrawImage>(), ImageDecodeCache::TracingInfo());
        StopWorkerTasks();
...
void ImageController::ConvertImagesToTasks(std::vector<DrawImage>* sync_decoded_images, std::vector<scoped_refptr<TileTask>>* tasks, bool* has_at_raster_images,
    bool* has_hardware_accelerated_jpeg_candidates, bool* has_hardware_accelerated_webp_candidates, const ImageDecodeCache::TracingInfo& tracing_info)
{
    DCHECK(cache_);
    ...
    *has_hardware_accelerated_webp_candidates = false;
    ...
    for (auto it = sync_decoded_images->begin(); it != sync_decoded_images->end();) {
        ...
        ImageDecodeCache::TaskResult result = cache_->GetTaskForImageAndRef(image_cache_client_id_, *it, tracing_info);
        *has_at_raster_images |= result.is_at_raster_decode;

        ImageType image_type = it->paint_image().GetImageHeaderMetadata() ? it->paint_image().GetImageHeaderMetadata()->image_type : ImageType::kInvalid;
        *has_hardware_accelerated_jpeg_candidates |= (result.can_do_hardware_accelerated_decode && image_type == ImageType::kJPEG);
        *has_hardware_accelerated_webp_candidates |= (result.can_do_hardware_accelerated_decode && image_type == ImageType::kWEBP);
...
std::vector<scoped_refptr<TileTask>> ImageController::SetPredecodeImages(std::vector<DrawImage> images, const ImageDecodeCache::TracingInfo& tracing_info)
{
    ...
    ConvertImagesToTasks(
        &images, &new_tasks, &has_at_raster_images, &has_hardware_accelerated_jpeg_candidates, &has_hardware_accelerated_webp_candidates, tracing_info);
    UnrefImages(predecode_locked_images_);
    predecode_locked_images_ = std::move(images);
    return new_tasks;
}

ImageController::ImageDecodeRequestId ImageController::QueueImageDecode(const DrawImage& draw_image, ImageDecodedCallback callback)
{
    ...
    if (is_image_lazy)
        result = cache_->GetOutOfRasterDecodeTaskForImageAndRef(image_cache_client_id_, draw_image);
    ...
    base::AutoLock hold(worker_state_->lock);
    bool has_external_dependency = result.task && !result.task->dependencies().empty();
    CHECK(!has_external_dependency || result.task->dependencies()[0]->IsRasterTask());
    worker_state_->image_decode_queue[id]
        = ImageDecodeRequest(id, draw_image, std::move(callback), std::move(result.task), result.need_unref, has_external_dependency);
    ScheduleImageDecodeOnWorkerIfNeeded();

    return id;
}

void ImageController::ExternalDependencyCompletedForTask(scoped_refptr<TileTask> task)
{
    base::AutoLock hold(worker_state_->lock);
    auto external_dependency_completed = [&task](ImageDecodeRequest& request) -> void {
        if (request.task == task) {
            request.has_external_dependency = false;
        }
    };
    base::ranges::for_each(worker_state_->image_decode_queue.begin(), worker_state_->image_decode_queue.end(), external_dependency_completed,
        &std::pair<const ImageDecodeRequestId, ImageDecodeRequest>::second);
    base::ranges::for_each(orphaned_decode_requests_.begin(), orphaned_decode_requests_.end(), external_dependency_completed);
    ScheduleImageDecodeOnWorkerIfNeeded();
}
...
// static
void ImageController::ProcessNextImageDecodeOnWorkerThread(WorkerState* worker_state)
{
    ...
    CHECK(decode_it != worker_state->image_decode_queue.end(), base::NotFatalUntil::M130);
    // Skip tasks that have an unmet external dependency.
    while (decode_it != worker_state->image_decode_queue.end() && decode_it->second.has_external_dependency) {
        decode_it++;
    }
    ...
    worker_state->requests_needing_completion[decode_id] = std::move(decode_it->second);
    ...
    worker_state->origin_task_runner->PostTask(FROM_HERE, base::BindOnce(&ImageController::ImageDecodeCompleted, worker_state->weak_ptr, decode_id));
...
void ImageController::GenerateTasksForOrphanedRequests()
{
    ...
    for (auto& request : orphaned_decode_requests_) {
        ...
        if (request.draw_image.paint_image().IsLazyGenerated()) {
            ...
            ImageDecodeCache::TaskResult result = cache_->GetOutOfRasterDecodeTaskForImageAndRef(image_cache_client_id_, request.draw_image);
...
{
    if (worker_state_->task_state == WorkerTaskState::kNoTask && HasReadyToRunTask()) {
        ...
        // ~ImageController).
        worker_task_runner_->PostTask(FROM_HERE, base::BindOnce(&ImageController::ProcessNextImageDecodeOnWorkerThread, base::Unretained(worker_state_.get())));
    }
}

ImageController::ImageDecodeRequest::ImageDecodeRequest() = default;
ImageController::ImageDecodeRequest::ImageDecodeRequest(ImageDecodeRequestId id, const DrawImage& draw_image, ImageDecodedCallback callback,
    scoped_refptr<TileTask> task, bool need_unref, bool has_external_dependency)
    : id(id)
...
ImageController::ImageDecodeRequest::ImageDecodeRequest(ImageDecodeRequest&& other) = default;
ImageController::ImageDecodeRequest::~ImageDecodeRequest() = default;

ImageController::ImageDecodeRequest& ImageController::ImageDecodeRequest::operator=(ImageDecodeRequest&& other) = default;
