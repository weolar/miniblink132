...
public:
    ...
    using ImageDecodedCallback = base::OnceCallback<void(ImageDecodeRequestId, ImageDecodeResult)>;
    explicit ImageController(scoped_refptr<base::SequencedTaskRunner> origin_task_runner, scoped_refptr<base::SequencedTaskRunner> worker_task_runner,
    ...
    void ConvertImagesToTasks(std::vector<DrawImage>* sync_decoded_images, std::vector<scoped_refptr<TileTask>>* tasks, bool* has_at_raster_images,
    ...
    std::vector<scoped_refptr<TileTask>> SetPredecodeImages(std::vector<DrawImage> predecode_images, const ImageDecodeCache::TracingInfo& tracing_info);
    ...
    // Virtual for testing.
    virtual ImageDecodeRequestId QueueImageDecode(const DrawImage& draw_image, ImageDecodedCallback callback);
...
private:
    struct ImageDecodeRequest {
        ImageDecodeRequest();
        ImageDecodeRequest(ImageDecodeRequestId id, const DrawImage& draw_image, ImageDecodedCallback callback, scoped_refptr<TileTask> task, bool need_unref,
    ...
    struct WorkerState {
        WorkerState(scoped_refptr<base::SequencedTaskRunner> origin_task_runner, base::WeakPtr<ImageController> weak_ptr);
        ~WorkerState();

        base::Lock lock;
        std::map<ImageDecodeRequestId, ImageDecodeRequest> image_decode_queue GUARDED_BY(lock);
        std::map<ImageDecodeRequestId, ImageDecodeRequest> requests_needing_completion GUARDED_BY(lock);
    ...
    void ScheduleImageDecodeOnWorkerIfNeeded() EXCLUSIVE_LOCKS_REQUIRED(worker_state_->lock);
    ...
    const base::RepeatingCallback<void(scoped_refptr<TileTask>)> notify_external_dependent_;
