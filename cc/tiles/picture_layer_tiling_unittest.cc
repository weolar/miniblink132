...
static gfx::Rect ViewportInLayerSpace(const gfx::Transform& transform, const gfx::Size& device_viewport)
{
    ...
    return MathUtil::ProjectEnclosingClippedRect(inverse, gfx::Rect(device_viewport));
}
...
public:
    ...
    static std::unique_ptr<TestablePictureLayerTiling> Create(WhichTree tree, const gfx::AxisTransform2d& raster_transform,
        scoped_refptr<RasterSource> raster_source, PictureLayerTilingClient* client, const LayerTreeSettings& settings)
    {
        return base::WrapUnique(new TestablePictureLayerTiling(tree, raster_transform, raster_source, client, settings.tiling_interest_area_padding,
            settings.skewport_target_time_in_seconds, settings.skewport_extrapolation_limit_in_screen_pixels, 312.f, /* min_preraster_distance */
            settings.max_preraster_distance_in_screen_pixels));
    }

    gfx::Rect live_tiles_rect() const
    {
        return live_tiles_rect_;
    }
    ...
    {
        SetPriorityRect(EnclosingLayerRectFromContentsRect(rect), PriorityRectType::EVENTUALLY_RECT,
            /*evicts_tiles=*/true);
    }
...
protected:
    TestablePictureLayerTiling(WhichTree tree, const gfx::AxisTransform2d& raster_transform, scoped_refptr<RasterSource> raster_source,
        PictureLayerTilingClient* client, size_t tiling_interest_area_padding, float skewport_target_time, int skewport_extrapolation_limit,
        float min_preraster_distance, float max_preraster_distance)
        : PictureLayerTiling(tree, raster_transform, raster_source, client, min_preraster_distance, max_preraster_distance,
...
public:
    using VerifyTilesCallback = base::RepeatingCallback<void(Tile* tile, const gfx::Rect& geometry_rect)>;

    PictureLayerTilingIteratorTest() = default;
    PictureLayerTilingIteratorTest(const PictureLayerTilingIteratorTest&) = delete;
    ~PictureLayerTilingIteratorTest() override = default;

    PictureLayerTilingIteratorTest& operator=(const PictureLayerTilingIteratorTest&) = delete;

    void Initialize(const gfx::Size& tile_size, scoped_refptr<FakeRasterSource> raster_source,
        const gfx::AxisTransform2d& raster_transform = gfx::AxisTransform2d(), WhichTree tree = PENDING_TREE)
    {
        client_.SetTileSize(tile_size);
        tiling_ = TestablePictureLayerTiling::Create(tree, raster_transform, std::move(raster_source), &client_, LayerTreeSettings());
        tiling_->set_resolution(HIGH_RESOLUTION);
    }

    void InitializeFilled(const gfx::Size& tile_size, float contents_scale, const gfx::Size& layer_bounds, WhichTree tree = PENDING_TREE)
    {
        Initialize(tile_size, FakeRasterSource::CreateFilled(layer_bounds), gfx::AxisTransform2d(contents_scale, gfx::Vector2dF()), tree);
    }

    void InitializePartiallyFilled(
        const gfx::Size& tile_size, float contents_scale, const gfx::Size& layer_bounds, const gfx::Rect& recorded_bounds, WhichTree tree = PENDING_TREE)
    {
        Initialize(
            tile_size, FakeRasterSource::CreatePartiallyFilled(layer_bounds, recorded_bounds), gfx::AxisTransform2d(contents_scale, gfx::Vector2dF()), tree);
    }

    void InitializeWithTransform(
        const gfx::Size& tile_size, const gfx::AxisTransform2d& raster_transform, const gfx::Size& layer_bounds, WhichTree tree = PENDING_TREE)
    {
        Initialize(tile_size, FakeRasterSource::CreateFilled(layer_bounds), raster_transform, tree);
    }

    void InitializeActive(const gfx::Size& tile_size, float contents_scale, const gfx::Size& layer_bounds)
    {
    ...
    void VerifyTilesExactlyCoverRect(float rect_scale, const gfx::Rect& request_rect, const gfx::Rect& expect_rect)
    {
        ...
        for (PictureLayerTiling::CoverageIterator iter(tiling_.get(), rect_scale, request_rect); iter; ++iter) {
    ...
    void VerifyTiles(float rect_scale, const gfx::Rect& rect, VerifyTilesCallback callback)
    {
        VerifyTiles(tiling_.get(), rect_scale, rect, callback);
    }

    void VerifyTiles(PictureLayerTiling* tiling, float rect_scale, const gfx::Rect& rect, VerifyTilesCallback callback)
    {
        Region remaining = rect;
        for (PictureLayerTiling::CoverageIterator iter(tiling, rect_scale, rect); iter; ++iter) {
    ...
    void VerifyTilesCoverNonContainedRect(float rect_scale, const gfx::Rect& dest_rect)
    {
        float dest_to_contents_scale = tiling_->contents_scale_key() / rect_scale;
        gfx::Rect clamped_rect = gfx::ScaleToEnclosingRect(tiling_->tiling_rect(), 1.f / dest_to_contents_scale);
...
TEST_F(PictureLayerTilingIteratorTest, ResizeLayer)
{
    ...
    scoped_refptr<FakeRasterSource> raster_source = FakeRasterSource::CreateFilled(new_layer_size);
...
TEST_F(PictureLayerTilingIteratorTest, RecordedBoundsChange)
{
    ...
    tiling_->SetRasterSourceAndResize(FakeRasterSource::CreatePartiallyFilled(layer_size, new_recorded_bounds1));
    ...
    // newly exposed tiles.
    tiling_->SetRasterSourceAndResize(FakeRasterSource::CreatePartiallyFilled(layer_size, new_recorded_bounds2));
    ...
    tiling_->SetRasterSourceAndResize(FakeRasterSource::CreatePartiallyFilled(layer_size, new_recorded_bounds3));
...
TEST_F(PictureLayerTilingIteratorTest, ResizeLiveTileRectOverTileBorders)
{
    ...
    SetLiveRectAndVerifyTiles(gfx::Rect(left - 1, top - 1, 250 - left, 350 - top));
    EXPECT_TRUE(tiling_->TileAt(0, 3));
    EXPECT_TRUE(tiling_->TileAt(0, 2));
...
TEST_F(PictureLayerTilingIteratorTest, ShrinkWidthExpandHeightTilingRect)
{
    ...
    scoped_refptr<FakeRasterSource> raster_source = FakeRasterSource::CreateFilled(gfx::Size(310, 310));
    tiling_->SetRasterSourceAndResize(raster_source);
...
TEST_F(PictureLayerTilingIteratorTest, CreateTileJustCoverBorderUp)
{
    ...
    std::unique_ptr<TestablePictureLayerTiling> active_tiling = TestablePictureLayerTiling::Create(
        ACTIVE_TREE, gfx::AxisTransform2d(content_scale, gfx::Vector2dF()), raster_source, &active_client, LayerTreeSettings());
    ...
    EXPECT_FALSE(scaled_invalid_rect.Intersects(tiling_->TilingDataForTesting().TileBounds(0, 2)));
    EXPECT_TRUE(scaled_invalid_rect.Intersects(tiling_->TilingDataForTesting().TileBoundsWithBorder(0, 2)));
    EXPECT_TRUE(tiling_->TileAt(0, 2));
...
static void TileExists(bool exists, Tile* tile, const gfx::Rect& geometry_rect)
{
...
TEST_F(PictureLayerTilingIteratorTest, TilesExist)
{
    ...
    VerifyTiles(1.f, gfx::Rect(layer_bounds), base::BindRepeating(&TileExists, false));

    tiling_->ComputeTilePriorityRects(gfx::Rect(layer_bounds), // visible rect
        gfx::Rect(layer_bounds), // skewport
        gfx::Rect(layer_bounds), // soon border rect
        gfx::Rect(layer_bounds), // eventually rect
        1.f, // current contents scale
        Occlusion());
    VerifyTiles(1.f, gfx::Rect(layer_bounds), base::BindRepeating(&TileExists, true));

    // Make the viewport rect empty. All tiles are killed and become zombies.
    tiling_->ComputeTilePriorityRects(gfx::Rect(), gfx::Rect(), gfx::Rect(), gfx::Rect(), 1.f, Occlusion());
    VerifyTiles(1.f, gfx::Rect(layer_bounds), base::BindRepeating(&TileExists, false));
}
...
TEST_F(PictureLayerTilingIteratorTest, TilesExistGiantViewport)
{
    ...
    tiling_->ComputeTilePriorityRects(giant_rect, giant_rect, giant_rect, giant_rect, 1.f, Occlusion());
    VerifyTiles(1.f, gfx::Rect(layer_bounds), base::BindRepeating(&TileExists, true));
}
...
TEST_F(PictureLayerTilingIteratorTest, TilesExistOutsideViewport)
{
    ...
    tiling_->ComputeTilePriorityRects(viewport_rect, viewport_rect, viewport_rect, eventually_rect, 1.f, Occlusion());
    VerifyTiles(1.f, gfx::Rect(layer_bounds), base::BindRepeating(&TileExists, true));
}

static void TilesIntersectingRectExist(const gfx::Rect& rect, bool intersect_exists, Tile* tile, const gfx::Rect& geometry_rect)
{
    bool intersects = rect.Intersects(geometry_rect);
    bool expected_exists = intersect_exists ? intersects : !intersects;
    EXPECT_EQ(expected_exists, tile != nullptr) << "Rects intersecting " << rect.ToString() << " should exist. "
                                                << "Current tile rect is " << geometry_rect.ToString();
}

TEST_F(PictureLayerTilingIteratorTest, TilesExistLargeViewportAndLayerWithSmallVisibleArea)
{
    ...
    tiling_ = TestablePictureLayerTiling::Create(PENDING_TREE, gfx::AxisTransform2d(), raster_source, &client_, settings);
    tiling_->set_resolution(HIGH_RESOLUTION);
    VerifyTilesExactlyCoverRect(1.f, gfx::Rect(layer_bounds));
    VerifyTiles(1.f, gfx::Rect(layer_bounds), base::BindRepeating(&TileExists, false));
    ...
    tiling_->ComputeTilePriorityRects(visible_rect, // visible rect
        ...
        1.f, // current contents scale
        Occlusion());
    VerifyTiles(1.f, gfx::Rect(layer_bounds), base::BindRepeating(&TilesIntersectingRectExist, visible_rect, true));
}
...
TEST(ComputeTilePriorityRectsTest, VisibleTiles)
{
    ...
    gfx::Rect viewport_in_layer_space = ViewportInLayerSpace(current_screen_transform, device_viewport);
    ...
    std::unique_ptr<TestablePictureLayerTiling> tiling
        = TestablePictureLayerTiling::Create(ACTIVE_TREE, gfx::AxisTransform2d(), raster_source, &client, LayerTreeSettings());
    ...
    tiling->ComputeTilePriorityRects(
        viewport_in_layer_space, viewport_in_layer_space, viewport_in_layer_space, eventually_rect, current_layer_contents_scale, Occlusion());
...
TEST(PictureLayerTilingTest, RecycledTilesClearedOnReset)
{
    ...
    std::unique_ptr<TestablePictureLayerTiling> active_tiling
        = TestablePictureLayerTiling::Create(ACTIVE_TREE, gfx::AxisTransform2d(), raster_source, &active_client, LayerTreeSettings());
    active_tiling->set_resolution(HIGH_RESOLUTION);
    // Create all tiles on this tiling.
    gfx::Rect visible_rect = gfx::Rect(0, 0, 100, 100);
    active_tiling->ComputeTilePriorityRects(visible_rect, visible_rect, visible_rect, visible_rect, 1.f, Occlusion());
    ...
    std::unique_ptr<TestablePictureLayerTiling> recycle_tiling
        = TestablePictureLayerTiling::Create(PENDING_TREE, gfx::AxisTransform2d(), raster_source, &recycle_client, settings);
    recycle_tiling->set_resolution(HIGH_RESOLUTION);

    // Create all tiles on the recycle tiling.
    recycle_tiling->ComputeTilePriorityRects(visible_rect, visible_rect, visible_rect, visible_rect, 1.0f, Occlusion());
...
TEST(PictureLayerTilingTest, EdgeCaseTileNowAndRequired)
{
    ...
    pending_tiling->ComputeTilePriorityRects(visible_rect, visible_rect, visible_rect, eventually_rect, 1.f, Occlusion());

    Tile* tile = pending_tiling->TileAt(1, 0);
    EXPECT_NE(pending_tiling->visible_rect_type(), pending_tiling->ComputePriorityRectTypeForTile(tile));
    EXPECT_FALSE(pending_tiling->IsTileRequiredForActivation(tile));
    EXPECT_TRUE(tile->content_rect().Intersects(visible_rect));
    EXPECT_FALSE(pending_tiling->tiling_data()->TileBounds(tile->tiling_i_index(), tile->tiling_j_index()).Intersects(visible_rect));

    // Now the tile at (1, 0) should be intersecting the visible rect.
    visible_rect = gfx::Rect(0, 0, 100, 100);
    pending_tiling->ComputeTilePriorityRects(visible_rect, visible_rect, visible_rect, eventually_rect, 1.f, Occlusion());
    EXPECT_EQ(pending_tiling->visible_rect_type(), pending_tiling->ComputePriorityRectTypeForTile(tile));
    EXPECT_TRUE(pending_tiling->IsTileRequiredForActivation(tile));
    EXPECT_TRUE(tile->content_rect().Intersects(visible_rect));
    EXPECT_TRUE(pending_tiling->tiling_data()->TileBounds(tile->tiling_i_index(), tile->tiling_j_index()).Intersects(visible_rect));
}
...
TEST_F(PictureLayerTilingIteratorTest, GiantRect)
{
    ...
    tiling_ = TestablePictureLayerTiling::Create(
        PENDING_TREE, gfx::AxisTransform2d(contents_scale, gfx::Vector2dF()), raster_source, &client_, LayerTreeSettings());
...
static void TileHasGeometryRect(const gfx::Rect& expected_rect, Tile* tile, const gfx::Rect& geometry_rect)
{
...
TEST_F(PictureLayerTilingIteratorTest, UseLeastTilesToCover)
{
    ...
    VerifyTiles(2.f, gfx::Rect(199, 199), base::BindRepeating(&TileHasGeometryRect, gfx::Rect(199, 199)));
}
...
TEST_F(PictureLayerTilingIteratorTest, UseLeastTilesToCover2)
{
    ...
    VerifyTiles(2.f, dest_rect, base::BindRepeating(&TileHasGeometryRect, dest_rect));
}
...
TEST_F(PictureLayerTilingIteratorTest, TilesStoreTilings)
{
    ...
    auto active_tiling = TestablePictureLayerTiling::Create(ACTIVE_TREE, gfx::AxisTransform2d(), raster_source, &client_, LayerTreeSettings());
    active_tiling->set_resolution(HIGH_RESOLUTION);

    active_tiling->TakeTilesAndPropertiesFrom(tiling_.get(), Region(gfx::Rect(bounds)));
    for (const auto* tile : tiles) {
...
{
    InitializeWithTransform(gfx::Size(256, 256), gfx::AxisTransform2d(1.f, gfx::Vector2dF(0.125f, 0.125f)), gfx::Size(1000, 1));
    ...
    for (PictureLayerTiling::CoverageIterator iter(tiling_.get(), 1.f, gfx::Rect(1000, 1)); iter; ++iter) {
    ...
    for (PictureLayerTiling::CoverageIterator iter(tiling_.get(), 1.375f, gfx::Rect(1375, 2)); iter; ++iter) {
        gfx::Rect geometry_rect = iter.geometry_rect();
        gfx::RectF texture_rect = iter.texture_rect();
...
TEST_F(PictureLayerTilingIteratorTest, FractionalTranslatedTilingPartiallyFilled)
{
    Initialize(gfx::Size(256, 256), FakeRasterSource::CreatePartiallyFilled(gfx::Size(2000, 3), gfx::Rect(320, 0, 600, 1)),
    ...
    // Verifies the texture coordinate is correctly translated.
    int i = 0;
    for (PictureLayerTiling::CoverageIterator iter(tiling_.get(), 1.f, gfx::Rect(2000, 6)); iter; ++iter, ++i) {
        gfx::Rect geometry_rect = iter.geometry_rect();
        gfx::RectF texture_rect = iter.texture_rect();
        switch (i) {
        case 0:
...
TEST_F(PictureLayerTilingIteratorTest, FractionalTranslatedTilingOverflow)
{
    ...
    InitializeWithTransform(gfx::Size(256, 256), gfx::AxisTransform2d(1.5f, gfx::Vector2dF(0.5f, 0.5f)), gfx::Size(9, 9));
    EXPECT_EQ(tiling_->tiling_rect(), gfx::Rect(14, 14));
    SetLiveRectAndVerifyTiles(gfx::Rect(9, 9));

    PictureLayerTiling::CoverageIterator iter(tiling_.get(), 1.56f, gfx::Rect(15, 15));
    ASSERT_TRUE(iter);
    gfx::Rect geometry_rect = iter.geometry_rect();
    gfx::RectF texture_rect = iter.texture_rect();
...
TEST_F(PictureLayerTilingIteratorTest, EdgeCaseLargeIntBounds)
{
    ...
    int count = 0;
    for (PictureLayerTiling::CoverageIterator iter(tiling_.get(), scale, coverage_rect); iter && count < 200; ++count, ++iter) {
...
TEST_F(PictureLayerTilingIteratorTest, EdgeCaseLargeIntBounds2)
{
    ...
    InitializeFilled(tile_size, scale, layer_bounds);
    for (PictureLayerTiling::CoverageIterator iter(tiling_.get(), scale, coverage_rect); iter; ++iter) {
        EXPECT_FALSE(iter.geometry_rect().IsEmpty());
    }
}
...
TEST_F(PictureLayerTilingIteratorTest, TilingSizeChange)
{
    ...
    tiling_->ComputeTilePriorityRects(gfx::Rect(visible_rect), // visible rect
...
{
    EXPECT_EQ(TileMemoryLimitPolicy::ALLOW_ANYTHING, client_.global_tile_state().memory_limit_policy);
    gfx::Size tile_size(100, 100);
...
TEST_F(PictureLayerTilingIteratorTest, ComputeTilePriorityRectsAllowPrepaintOnly)
{
    client_.set_memory_limit_policy(TileMemoryLimitPolicy::ALLOW_PREPAINT_ONLY);
    EXPECT_EQ(TileMemoryLimitPolicy::ALLOW_PREPAINT_ONLY, client_.global_tile_state().memory_limit_policy);
    gfx::Size tile_size(100, 100);
    gfx::Size layer_size(800, 600);
