...
void AppendTilingSetRequiredQueues(
    const std::vector<raw_ptr<PictureLayerImpl, VectorExperimental>>& layers, std::vector<std::unique_ptr<TilingSetRasterQueueRequired>>* queues)
{
    ...
    for (PictureLayerImpl* layer : layers) {
        ...
        std::unique_ptr<TilingSetRasterQueueRequired> tiling_set_queue
            = TilingSetRasterQueueRequired::Create(tiling_set, RasterTilePriorityQueueRequired::Type::REQUIRED_FOR_ACTIVATION);
...
void RasterTilePriorityQueueRequired::Build(const std::vector<raw_ptr<PictureLayerImpl, VectorExperimental>>& active_layers,
    const std::vector<raw_ptr<PictureLayerImpl, VectorExperimental>>& pending_layers, Type type)
{
...
void RasterTilePriorityQueueRequired::BuildRequiredForDraw(const std::vector<raw_ptr<PictureLayerImpl, VectorExperimental>>& active_layers)
{
    const bool cc_slimming_enabled = features::IsCCSlimmingEnabled();
    for (PictureLayerImpl* layer : active_layers) {
        ...
        std::unique_ptr<TilingSetRasterQueueRequired> tiling_set_queue = TilingSetRasterQueueRequired::Create(tiling_set, Type::REQUIRED_FOR_DRAW);
        if (tiling_set_queue && !tiling_set_queue->IsEmpty()) {
...
void RasterTilePriorityQueueRequired::BuildRequiredForActivation(const std::vector<raw_ptr<PictureLayerImpl, VectorExperimental>>& active_layers,
    const std::vector<raw_ptr<PictureLayerImpl, VectorExperimental>>& pending_layers)
{
