...
TilingSetRasterQueueAll::IterationStage::IterationStage(IteratorType type, TilePriority::PriorityBin bin)
    : iterator_type(type)
    , tile_type(bin)
{
}
...
// static
std::unique_ptr<TilingSetRasterQueueAll> TilingSetRasterQueueAll::Create(PictureLayerTilingSet* tiling_set, bool prioritize_low_res, bool is_drawing_layer)
{
    DCHECK(tiling_set);
    ...
    if (features::IsCCSlimmingEnabled()) {
        ...
        if (!tiling_set->num_tilings()) {
            return base::WrapUnique(new TilingSetRasterQueueAll(nullptr, nullptr, nullptr, is_drawing_layer));
        }
    }
    ...
    bool use_active_non_ideal_pending_high_res_tiling = active_non_ideal_pending_high_res_tiling && active_non_ideal_pending_high_res_tiling->has_tiles()
        && !active_non_ideal_pending_high_res_tiling->all_tiles_done();

    if (!use_low_res_tiling && !use_high_res_tiling && !use_active_non_ideal_pending_high_res_tiling && features::IsCCSlimmingEnabled()) {
        return nullptr;
    }

    return base::WrapUnique(new TilingSetRasterQueueAll(use_high_res_tiling ? high_res_tiling : nullptr, use_low_res_tiling ? low_res_tiling : nullptr,
        use_active_non_ideal_pending_high_res_tiling ? active_non_ideal_pending_high_res_tiling : nullptr, is_drawing_layer));
}

TilingSetRasterQueueAll::TilingSetRasterQueueAll(PictureLayerTiling* high_res_tiling, PictureLayerTiling* low_res_tiling,
    PictureLayerTiling* active_non_ideal_pending_high_res_tiling, bool is_drawing_layer)
    : current_stage_(0)
    , is_drawing_layer_(is_drawing_layer)
{
    if (!high_res_tiling && !low_res_tiling && !active_non_ideal_pending_high_res_tiling) {
    ...
    if (active_non_ideal_pending_high_res_tiling) {
        MakeTilingIterator(ACTIVE_NON_IDEAL_PENDING_HIGH_RES, active_non_ideal_pending_high_res_tiling);
    } else if (!features::IsCCSlimmingEnabled()) {
    ...
    if (active_non_ideal_pending_high_res_tiling) {
        stages_.push_back(IterationStage(ACTIVE_NON_IDEAL_PENDING_HIGH_RES, TilePriority::NOW));
        stages_.push_back(IterationStage(ACTIVE_NON_IDEAL_PENDING_HIGH_RES, TilePriority::SOON));
    }
    ...
    if (!iterators_[index] || iterators_[index]->done() || iterators_[index]->type() != tile_type) {
        AdvanceToNextStage();
    }
}
...
void TilingSetRasterQueueAll::MakeTilingIterator(IteratorType type, PictureLayerTiling* tiling)
{
...
TilingSetRasterQueueAll::OnePriorityRectIterator::OnePriorityRectIterator(
    PictureLayerTiling* tiling, TilingData* tiling_data, PictureLayerTiling::PriorityRectType priority_rect_type)
    : tiling_(tiling)
...
template <typename TilingIteratorType> void TilingSetRasterQueueAll::OnePriorityRectIterator::AdvanceToNextTile(TilingIteratorType* iterator)
{
    for (;;) {
        ++(*iterator);
        if (!(*iterator)) {
        ...
        bool is_tile_occluded = result != kTileNeedsRaster && tiling_->IsTileOccluded(tile);
        current_tile_ = tiling_->MakePrioritizedTile(tile, priority_rect_type_, is_tile_occluded);
        break;
    }
}

template <typename TilingIteratorType> bool TilingSetRasterQueueAll::OnePriorityRectIterator::GetFirstTileAndCheckIfValid(TilingIteratorType* iterator)
{
    Tile* tile = tiling_->TileAt(iterator->index_x(), iterator->index_y());
    auto result = IsTileValid(tile);
    if (result == kTileNotValid) {
...
TilingSetRasterQueueAll::OnePriorityRectIterator::IsTileValidResult TilingSetRasterQueueAll::OnePriorityRectIterator::IsTileValid(const Tile* tile) const
{
    if (!tile)
        return kTileNotValid;

    // A tile is valid for raster if it needs raster and is unoccluded.
    bool tile_is_valid_for_raster = tile->draw_info().NeedsRaster() && !tiling_->IsTileOccluded(tile);
    ...
    if (!tile_is_valid_for_raster) {
        ...
        bool tile_is_valid_for_checker_images = tile->draw_info().is_checker_imaged() && tiling_->ShouldDecodeCheckeredImagesForTile(tile);
...
TilingSetRasterQueueAll::VisibleTilingIterator::VisibleTilingIterator(PictureLayerTiling* tiling, TilingData* tiling_data)
    : OnePriorityRectIterator(tiling, tiling_data, PictureLayerTiling::VISIBLE_RECT)
{
    if (!tiling_->has_visible_rect_tiles())
        return;
    iterator_ = TilingData::Iterator(tiling_data_, tiling_->current_visible_rect(), false /* include_borders */);
    if (!iterator_)
        return;
...
TilingSetRasterQueueAll::VisibleTilingIterator& TilingSetRasterQueueAll::VisibleTilingIterator::operator++()
{
...
TilingSetRasterQueueAll::PendingVisibleTilingIterator::PendingVisibleTilingIterator(PictureLayerTiling* tiling, TilingData* tiling_data)
    : OnePriorityRectIterator(tiling, tiling_data, PictureLayerTiling::PENDING_VISIBLE_RECT)
{
    iterator_ = TilingData::DifferenceIterator(tiling_data_, pending_visible_rect_, tiling_->current_visible_rect());
    if (!iterator_)
        return;
    if (!GetFirstTileAndCheckIfValid(&iterator_))
        ++(*this);
...
TilingSetRasterQueueAll::PendingVisibleTilingIterator& TilingSetRasterQueueAll::PendingVisibleTilingIterator::operator++()
{
    AdvanceToNextTile(&iterator_);
    return *this;
}
...
TilingSetRasterQueueAll::SkewportTilingIterator::SkewportTilingIterator(PictureLayerTiling* tiling, TilingData* tiling_data)
    : OnePriorityRectIterator(tiling, tiling_data, PictureLayerTiling::SKEWPORT_RECT)
{
    if (!tiling_->has_skewport_rect_tiles())
        return;
    iterator_ = TilingData::SpiralDifferenceIterator(
        tiling_data_, tiling_->current_skewport_rect(), tiling_->current_visible_rect(), tiling_->current_visible_rect());
    if (!iterator_)
        return;
    if (!GetFirstTileAndCheckIfValid(&iterator_)) {
        ++(*this);
...
TilingSetRasterQueueAll::SkewportTilingIterator& TilingSetRasterQueueAll::SkewportTilingIterator::operator++()
{
    AdvanceToNextTile(&iterator_);
    return *this;
}
...
TilingSetRasterQueueAll::SoonBorderTilingIterator::SoonBorderTilingIterator(PictureLayerTiling* tiling, TilingData* tiling_data)
    : OnePriorityRectIterator(tiling, tiling_data, PictureLayerTiling::SOON_BORDER_RECT)
{
    if (!tiling_->has_soon_border_rect_tiles())
        return;
    iterator_ = TilingData::SpiralDifferenceIterator(
        tiling_data_, tiling_->current_soon_border_rect(), tiling_->current_skewport_rect(), tiling_->current_visible_rect());
    if (!iterator_)
        return;
    if (!GetFirstTileAndCheckIfValid(&iterator_)) {
        ++(*this);
...
TilingSetRasterQueueAll::SoonBorderTilingIterator& TilingSetRasterQueueAll::SoonBorderTilingIterator::operator++()
{
    AdvanceToNextTile(&iterator_);
    return *this;
}
...
TilingSetRasterQueueAll::EventuallyTilingIterator::EventuallyTilingIterator(PictureLayerTiling* tiling, TilingData* tiling_data)
    : OnePriorityRectIterator(tiling, tiling_data, PictureLayerTiling::EVENTUALLY_RECT)
{
    if (!tiling_->has_eventually_rect_tiles())
        return;
    iterator_ = TilingData::SpiralDifferenceIterator(
        tiling_data_, tiling_->current_eventually_rect(), tiling_->current_skewport_rect(), tiling_->current_soon_border_rect());
    if (!iterator_)
        return;
    if (!GetFirstTileAndCheckIfValid(&iterator_)) {
        ++(*this);
...
TilingSetRasterQueueAll::EventuallyTilingIterator& TilingSetRasterQueueAll::EventuallyTilingIterator::operator++()
{
    AdvanceToNextTile(&iterator_);
    return *this;
}
...
TilingSetRasterQueueAll::TilingIterator::TilingIterator(PictureLayerTiling* tiling, TilingData* tiling_data)
    : tiling_(tiling)
    , tiling_data_(tiling_data)
