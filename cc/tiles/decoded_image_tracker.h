...
public:
    explicit DecodedImageTracker(ImageController* controller, scoped_refptr<base::SequencedTaskRunner> task_runner);
    ...
    // successful or not.
    void QueueImageDecode(const PaintImage& image, const TargetColorParams& target_color_params, base::OnceCallback<void(bool)> callback);
    ...
    // Test only functions:
    size_t NumLockedImagesForTesting() const
    {
...
private:
    ...
    void ImageDecodeFinished(base::OnceCallback<void(bool)> callback, PaintImage::Id image_id, ImageController::ImageDecodeRequestId request_id,
    ...
    class ImageLock {
    public:
        ImageLock(DecodedImageTracker* tracker, ImageController::ImageDecodeRequestId request_id, base::TimeTicks lock_time);
        ImageLock(const ImageLock&) = delete;
        ~ImageLock();

        ImageLock& operator=(const ImageLock&) = delete;
        base::TimeTicks lock_time() const
        {
            return lock_time_;
        }
