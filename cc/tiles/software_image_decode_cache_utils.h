...
private:
    ...
    public:
        ...
        static CacheKey FromDrawImage(const DrawImage& image, SkColorType color_type);
        ...
        bool operator==(const CacheKey& other) const
        {
            ...
            return frame_key_ == other.frame_key_ && type_ == other.type_ && target_color_params_ == other.target_color_params_
                && (type_ == kOriginal || (src_rect_ == other.src_rect_ && target_size_ == other.target_size_));
        }
        ...
        const PaintImage::FrameKey& frame_key() const
        {
            return frame_key_;
        }
        PaintImage::Id stable_id() const
        {
            return stable_id_;
        }
        ProcessingType type() const
        {
            return type_;
        }
        bool is_nearest_neighbor() const
        {
            return is_nearest_neighbor_;
        }
        bool may_be_lcp_candidate() const
        {
            return may_be_lcp_candidate_;
        }
        gfx::Rect src_rect() const
        {
            return src_rect_;
        }
    ...
    private:
        CacheKey(PaintImage::FrameKey frame_key, PaintImage::Id stable_id, ProcessingType type, bool is_nearest_neighbor, bool may_be_lcp_candidate,
            const gfx::Rect& src_rect, const gfx::Size& size, const TargetColorParams& target_color_params);
    ...
    public:
        CacheEntry();
        CacheEntry(const SkImageInfo& info, std::unique_ptr<base::DiscardableMemory> memory, const SkSize& src_rect_offset);
        ~CacheEntry();
        ...
        const SkSize& src_rect_offset() const
        {
            return src_rect_offset_;
        }
        ...
        void mark_out_of_raster()
        {
            usage_stats_.first_lock_out_of_raster = true;
        }
    ...
    static std::unique_ptr<CacheEntry> DoDecodeImage(
        const CacheKey& key, const PaintImage& image, SkColorType color_type, PaintImage::GeneratorClientId client_id, base::OnceClosure on_no_memory);
    static std::unique_ptr<CacheEntry> GenerateCacheEntryFromCandidate(
        const CacheKey& key, const DecodedDrawImage& candidate, bool needs_extract_subset, SkColorType color_type);
};
