...
public:
    ...
    bool operator()(const std::unique_ptr<TilingSetRasterQueueAll>& a_queue, const std::unique_ptr<TilingSetRasterQueueAll>& b_queue) const
    {
...
void CreateTilingSetRasterQueues(const std::vector<raw_ptr<PictureLayerImpl, VectorExperimental>>& layers, TreePriority tree_priority,
    std::vector<std::unique_ptr<TilingSetRasterQueueAll>>* queues)
{
    ...
    for (PictureLayerImpl* layer : layers) {
        ...
        std::unique_ptr<TilingSetRasterQueueAll> tiling_set_queue
            = TilingSetRasterQueueAll::Create(tiling_set, prioritize_low_res, layer->contributes_to_drawn_render_surface());
    ...
    std::make_heap(queues->begin(), queues->end(), RasterOrderComparator(tree_priority));
}
...
void RasterTilePriorityQueueAll::Build(const std::vector<raw_ptr<PictureLayerImpl, VectorExperimental>>& active_layers,
...
void RasterTilePriorityQueueAll::Pop()
{
    ...
    std::pop_heap(next_queues.begin(), next_queues.end(), RasterOrderComparator(tree_priority_));
    ...
    if (queue->IsEmpty()) {
        ...
        std::push_heap(next_queues.begin(), next_queues.end(), RasterOrderComparator(tree_priority_));
    }
}

std::vector<std::unique_ptr<TilingSetRasterQueueAll>>& RasterTilePriorityQueueAll::GetNextQueues()
{
    const auto* const_this = static_cast<const RasterTilePriorityQueueAll*>(this);
    const auto& const_queues = const_this->GetNextQueues();
    return const_cast<std::vector<std::unique_ptr<TilingSetRasterQueueAll>>&>(const_queues);
}

const std::vector<std::unique_ptr<TilingSetRasterQueueAll>>& RasterTilePriorityQueueAll::GetNextQueues() const
{
    DCHECK(!IsEmpty());
    ...
    if (pending_priority.priority_bin == TilePriority::NOW && active_priority.priority_bin == TilePriority::NOW) {
