...
void TileDrawInfo::AsValueInto(base::trace_event::TracedValue* state) const
{
    ...
    state->SetBoolean("is_transparent", mode_ == SOLID_COLOR_MODE && !solid_color_.isOpaque());
}

void TileDrawInfo::SetResource(ResourcePool::InUsePoolResource resource, bool resource_is_checker_imaged, bool is_premultiplied)
{
    DCHECK(!resource_);
    DCHECK(resource);
