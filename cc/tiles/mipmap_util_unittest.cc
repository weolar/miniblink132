...
TEST(MipMapUtilTest, Basic)
{
    ...
    EXPECT_SKSIZE_EQ(expected_scale, MipMapUtil::GetScaleAdjustmentForLevel(src_size, target_level));
    EXPECT_EQ(target_size, MipMapUtil::GetSizeForLevel(src_size, target_level));
    EXPECT_SKSIZE_EQ(expected_scale, MipMapUtil::GetScaleAdjustmentForSize(src_size, target_size));
}
...
TEST(MipMapUtilTest, Upscale)
{
    ...
    EXPECT_SKSIZE_EQ(result_size, MipMapUtil::GetScaleAdjustmentForSize(src_size, target_size));
...
{
    int max_mip_level = MipMapUtil::GetLevelForSize(gfx::Size(std::numeric_limits<int>::max(), std::numeric_limits<int>::max()), gfx::Size(1, 1));
...
TEST(MipMapUtilTest, NonSquare)
{
    ...
    const SkSize expected_scale = SkSize::Make(static_cast<float>(target_size.width()) / src_size.width(), 1);
...
TEST(MipMapUtilTest, Rounding)
{
    ...
    const SkSize expected_scale_smaller = SkSize::Make(
        static_cast<float>(target_size_smaller.width()) / src_size.width(), static_cast<float>(target_size_smaller.height()) / src_size.height());

    EXPECT_EQ(target_level_larger, MipMapUtil::GetLevelForSize(src_size, target_size_larger));
    EXPECT_EQ(target_level_smaller, MipMapUtil::GetLevelForSize(src_size, target_size_smaller));
    EXPECT_SKSIZE_EQ(expected_scale_larger, MipMapUtil::GetScaleAdjustmentForLevel(src_size, target_level_larger));
    EXPECT_SKSIZE_EQ(expected_scale_smaller, MipMapUtil::GetScaleAdjustmentForLevel(src_size, target_level_smaller));
    EXPECT_EQ(src_size, MipMapUtil::GetSizeForLevel(src_size, target_level_larger));
    EXPECT_EQ(target_size_smaller, MipMapUtil::GetSizeForLevel(src_size, target_level_smaller));
    EXPECT_SKSIZE_EQ(expected_scale_larger, MipMapUtil::GetScaleAdjustmentForSize(src_size, target_size_larger));
    EXPECT_SKSIZE_EQ(expected_scale_smaller, MipMapUtil::GetScaleAdjustmentForSize(src_size, target_size_smaller));
}

// Ensures that we round up during mip calculation.
TEST(MipMapUtilTest, RoundUp)
{
    const gfx::Size src_sizes[] = { gfx::Size(3, 3), gfx::Size(5, 7), gfx::Size(11, 14), gfx::Size(17, 31) };
    for (int i = 0; i < 4; ++i) {
        EXPECT_EQ(MipMapUtil::GetSizeForLevel(src_sizes[i], i + 1), gfx::Size(2, 2));
    }
}
