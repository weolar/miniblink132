...
class CC_MOJO_EMBEDDER_EXPORT VizLayerContext : public LayerContext, public viz::mojom::LayerContextClient {
public:
    // Constructs a VizLayerContext which submits content on behalf of
    // `frame_sink`. `client` must outlive this object.
    VizLayerContext(viz::mojom::CompositorFrameSink& frame_sink, LayerTreeHostImpl& host_impl);
    ~VizLayerContext() override;

    // LayerContext:
    void SetVisible(bool visible) override;
    void UpdateDisplayTreeFrom(LayerTreeImpl& tree, viz::ClientResourceProvider& resource_provider, viz::RasterContextProvider& context_provider) override;
    void UpdateDisplayTile(
        PictureLayerImpl& layer, const Tile& tile, viz::ClientResourceProvider& resource_provider, viz::RasterContextProvider& context_provider) override;
...
private:
    ...
    void SerializeAnimationUpdates(LayerTreeImpl& tree, viz::mojom::LayerTreeUpdate& update);

    // Serializes any changes to `timeline` since the last push to Viz. If there
    // have been no changes, this returns null.
    viz::mojom::AnimationTimelinePtr MaybeSerializeAnimationTimeline(AnimationTimeline& timeline);

    const raw_ref<LayerTreeHostImpl> host_impl_;

    mojo::AssociatedReceiver<viz::mojom::LayerContextClient> client_receiver_ { this };
