...
// {
//     using ProtoReason = ::perfetto::protos::pbzero::ChromeGraphicsPipeline::FrameSkippedReason;
//     switch (reason) {
...
AsyncLayerTreeFrameSink::AsyncLayerTreeFrameSink(scoped_refptr<viz::RasterContextProvider> context_provider,
    scoped_refptr<RasterContextProviderWrapper> worker_context_provider_wrapper, scoped_refptr<gpu::ClientSharedImageInterface> shared_image_interface,
    InitParams* params)
    : LayerTreeFrameSink(std::move(context_provider), std::move(worker_context_provider_wrapper), std::move(params->compositor_task_runner),
        params->gpu_memory_buffer_manager, std::move(shared_image_interface))
    ...
    , use_begin_frame_presentation_feedback_(params->use_begin_frame_presentation_feedback)
{
...
bool AsyncLayerTreeFrameSink::BindToClient(LayerTreeFrameSinkClient* client)
{
...
#if 1 // weolar
    while (!is_compositor_frame_sink_bind_) {
    ...
    threads.push_back({ base::PlatformThread::CurrentId(), viz::Thread::Type::kCompositor });
...
//         DCHECK_EQ(last_submitted_size_in_pixels_.width(), frame.size_in_pixels().width());
    ...
    compositor_frame_sink_ptr_->SubmitCompositorFrame(local_surface_id_, std::move(frame), std::move(hit_test_region_list), 0);
}
...
std::unique_ptr<LayerContext> AsyncLayerTreeFrameSink::CreateLayerContext(LayerTreeHostImpl& host_impl)
{
    CHECK(compositor_frame_sink_ptr_);
    return std::make_unique<VizLayerContext>(*compositor_frame_sink_ptr_, host_impl);
}
...
//         auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
//         auto* data = event->set_chrome_graphics_pipeline();
//         data->set_step(needs_begin_frames_ ? perfetto::protos::pbzero::ChromeGraphicsPipeline::StepName::STEP_RECEIVE_BEGIN_FRAME
//                                            : perfetto::protos::pbzero::ChromeGraphicsPipeline::StepName::STEP_RECEIVE_BEGIN_FRAME_DISCARD);
...
//     });
    ...
    for (const auto& pair : timing_details) {
        ...
        if (synthetic_begin_frame_source_ && use_begin_frame_presentation_feedback_) {
            const auto& feedback = pair.second.presentation_feedback;
            synthetic_begin_frame_source_->OnUpdateVSyncParameters(feedback.timestamp, feedback.interval);
        }
    }
    ...
    if (!needs_begin_frames_) {
        ...
        DidNotProduceFrame(viz::BeginFrameAck(adjusted_args, false), FrameSkippedReason::kNoDamage);
        return;
    }
...
void AsyncLayerTreeFrameSink::OnSurfaceEvicted(const viz::LocalSurfaceId& local_surface_id)
{
...
void AsyncLayerTreeFrameSink::OnBindCompositorFrameSinkClient()
{
    is_compositor_frame_sink_bind_ = true;
}

void AsyncLayerTreeFrameSink::UpdateNeedsBeginFramesInternal(bool needs_begin_frames)
{
