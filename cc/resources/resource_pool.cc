...
bool ResourceMeetsSizeRequirements(const gfx::Size& requested_size, const gfx::Size& actual_size, bool disallow_non_exact_reuse)
{
    ...
    // smaller request.
    if (actual_size.width() < requested_size.width() || actual_size.height() < requested_size.height())
        return false;
...
ResourcePool::ResourcePool(viz::ClientResourceProvider* resource_provider, viz::RasterContextProvider* context_provider,
    scoped_refptr<base::SingleThreadTaskRunner> task_runner, const base::TimeDelta& expiration_delay, bool disallow_non_exact_reuse)
...
{
    //base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "cc::ResourcePool", task_runner_.get());
    memory_pressure_listener_
        = std::make_unique<base::MemoryPressureListener>(FROM_HERE, base::BindRepeating(&ResourcePool::OnMemoryPressure, weak_ptr_factory_.GetWeakPtr()));
}
...
ResourcePool::PoolResource* ResourcePool::ReuseResource(const gfx::Size& size, viz::SharedImageFormat format, const gfx::ColorSpace& color_space)
{
    ...
    for (auto it = unused_resources_.begin(); it != unused_resources_.end(); ++it) {
        ...
        if (!ResourceMeetsSizeRequirements(size, resource->size(), disallow_non_exact_reuse_))
            continue;
...
ResourcePool::PoolResource* ResourcePool::CreateResource(const gfx::Size& size, viz::SharedImageFormat format, const gfx::ColorSpace& color_space)
{
    DCHECK(format.VerifySizeInBytes(size));

    auto pool_resource = std::make_unique<PoolResource>(this, next_resource_unique_id_++, size, format, color_space);
...
ResourcePool::InUsePoolResource ResourcePool::AcquireResource(
    const gfx::Size& size, viz::SharedImageFormat format, const gfx::ColorSpace& color_space, const std::string& debug_name)
{
...
ResourcePool::InUsePoolResource ResourcePool::TryAcquireResourceForPartialRaster(uint64_t new_content_id, const gfx::Rect& new_invalidated_rect,
    uint64_t previous_content_id, gfx::Rect* total_invalidated_rect, const gfx::ColorSpace& raster_color_space, const std::string& debug_name)
{
    ...
    *total_invalidated_rect = gfx::Rect();
    ...
    for (auto it = unused_resources_.begin(); it != unused_resources_.end(); ++it) {
        ...
        if (resource->content_id() == previous_content_id) {
            ...
            UpdateResourceContentIdAndInvalidation(resource, new_content_id, new_invalidated_rect);

            // Return the resource with the smallest invalidation.
            int area = resource->invalidated_rect().size().GetCheckedArea().ValueOrDefault(std::numeric_limits<int>::max());
            if (iter_resource_to_return == unused_resources_.end() || area < minimum_area) {
    ...
    for (const auto& resource : busy_resources_) {
        if (resource->content_id() == previous_content_id) {
            UpdateResourceContentIdAndInvalidation(resource.get(), new_content_id, new_invalidated_rect);
        }
    }
    ...
    if (iter_resource_to_return != unused_resources_.end()) {
        ...
        // Transfer resource to |in_use_resources_|.
        resource->set_state(PoolResource::kInUse);
        in_use_resources_[resource->unique_id()] = std::move(*iter_resource_to_return);
...
void ResourcePool::OnResourceReleased(size_t unique_id, const gpu::SyncToken& sync_token, bool lost)
{
    ...
    auto busy_it = base::ranges::find(busy_resources_, unique_id, &PoolResource::unique_id);
...
bool ResourcePool::PrepareForExport(const InUsePoolResource& in_use_resource, viz::TransferableResource::ResourceSource resource_source)
{
    ...
    if (resource->gpu_backing()) {
        ...
        transferable = viz::TransferableResource::MakeGpu(gpu_backing->shared_image->mailbox(), texture_target, gpu_backing->mailbox_sync_token,
            resource->size(), resource->format(), gpu_backing->overlay_candidate, resource_source);
        if (gpu_backing->wait_on_fence_required)
            transferable.synchronization_type = viz::TransferableResource::SynchronizationType::kGpuCommandsCompleted;
    } else {
        SoftwareBacking* software_backing = resource->software_backing();
        transferable = software_backing->shared_image ? viz::TransferableResource::MakeSoftwareSharedImage(
                           software_backing->shared_image, software_backing->mailbox_sync_token, resource->size(), resource->format(), resource_source)
                                                      : viz::TransferableResource::MakeSoftwareSharedBitmap(software_backing->shared_bitmap_id,
                                                          software_backing->mailbox_sync_token, resource->size(), resource->format(), resource_source);
    }
    transferable.color_space = resource->color_space();
    resource->set_resource_id(resource_provider_->ImportResource(
        std::move(transferable), base::BindOnce(&ResourcePool::OnResourceReleased, weak_ptr_factory_.GetWeakPtr(), resource->unique_id())));
    return true;
}
...
void ResourcePool::InvalidateResources()
{
    while (!unused_resources_.empty()) {
        DCHECK_GE(unused_memory_usage_bytes_, unused_resources_.back()->memory_usage());
...
void ResourcePool::ReleaseResource(InUsePoolResource in_use_resource)
{
    ...
    if (it == in_use_resources_.end()) {
        ...
        // list.
        CHECK(!base::Contains(busy_resources_, pool_resource->unique_id(), &PoolResource::unique_id));

        // Also check if the resource exists in our unused resources list.
        CHECK(!base::Contains(unused_resources_, pool_resource->unique_id(), &PoolResource::unique_id));
...
void ResourcePool::OnContentReplaced(const InUsePoolResource& in_use_resource, uint64_t content_id)
{
    PoolResource* resource = in_use_resource.resource_;
    DCHECK(resource);
...
void ResourcePool::SetResourceUsageLimits(size_t max_memory_usage_bytes, size_t max_resource_count)
{
...
void ResourcePool::DeleteResource(std::unique_ptr<PoolResource> resource)
{
    ...
    if (flush_evicted_resources_deadline_ == base::TimeTicks::Max()) {
        flush_evicted_resources_deadline_ = clock_->NowTicks() + kDefaultMaxFlushDelay;
    }
}

void ResourcePool::UpdateResourceContentIdAndInvalidation(PoolResource* resource, uint64_t new_content_id, const gfx::Rect& new_invalidated_rect)
{
...
void ResourcePool::DidFinishUsingResource(std::unique_ptr<PoolResource> resource)
{
    unused_memory_usage_bytes_ += resource->memory_usage();
    unused_resources_.push_front(std::move(resource));
}

void ResourcePool::ScheduleEvictExpiredResourcesIn(base::TimeDelta time_from_now)
{
    ...
    task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&ResourcePool::EvictExpiredResources, weak_ptr_factory_.GetWeakPtr()), time_from_now);
}
...
void ResourcePool::EvictExpiredResources()
{
    ...
    if (unused_resources_.empty() || flush_evicted_resources_deadline_ <= current_time) {
    ...
    if (!unused_resources_.empty()) {
        ...
        ScheduleEvictExpiredResourcesIn(std::min(GetUsageTimeForLRUResource() + resource_expiration_delay_, flush_evicted_resources_deadline_) - current_time);
    }
}
...
bool ResourcePool::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    if (args.level_of_detail == MemoryDumpLevelOfDetail::kBackground) {
        std::string dump_name = base::StringPrintf("cc/tile_memory/provider_0x%x", tracing_id_);
        MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(dump_name);
        dump->AddScalar(MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, total_memory_usage_bytes_);
    } else {
        for (const auto& resource : unused_resources_) {
            resource->OnMemoryDump(pmd, tracing_id_, resource_provider_, true /* is_free */, false /* is_busy */);
        }
        for (const auto& resource : busy_resources_) {
            resource->OnMemoryDump(pmd, tracing_id_, resource_provider_, false /* is_free */, true /* is_busy */);
        }
        for (const auto& entry : in_use_resources_) {
            entry.second->OnMemoryDump(pmd, tracing_id_, resource_provider_, false /* is_free */, false /* is_busy */);
        }
    }
    return true;
}

void ResourcePool::OnMemoryPressure(base::MemoryPressureListener::MemoryPressureLevel level)
{
    switch (level) {
...
ResourcePool::PoolResource::PoolResource(
    ResourcePool* resource_pool, size_t unique_id, const gfx::Size& size, viz::SharedImageFormat format, const gfx::ColorSpace& color_space)
...
void ResourcePool::PoolResource::OnMemoryDump(
    base::trace_event::ProcessMemoryDump* pmd, int tracing_id, const viz::ClientResourceProvider* resource_provider, bool is_free, bool is_busy) const
{
    *(int*)1 = 1;
    // Resource IDs are not process-unique, so log with the ResourcePool's unique
    // tracing id.
//     const std::string dump_name = base::StringPrintf(
//         "cc/tile_memory/provider_%d/%s%sresource_%zd", tracing_id, debug_name_.empty() ? "" : debug_name_.c_str(), debug_name_.empty() ? "" : "/", unique_id_);
//     MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(dump_name);
// 
...
//     // the root ownership.
//     const int kImportance = static_cast<int>(gpu::TracingImportance::kClientOwner);
...
//         software_backing_->OnMemoryDump(pmd, dump->guid(), tracing_process_id, kImportance);
//     } else if (gpu_backing_) {
//         gpu_backing_->OnMemoryDump(pmd, dump->guid(), tracing_process_id, kImportance);
//     }
// 
//     uint64_t total_bytes = memory_usage();
//     dump->AddScalar(MemoryAllocatorDump::kNameSize, MemoryAllocatorDump::kUnitsBytes, total_bytes);
// 
