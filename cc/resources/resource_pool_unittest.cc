...
public:
    void SetUp() override
    {
        ...
        context_provider_ = viz::TestContextProvider::CreateRaster(std::move(context_support));
        context_provider_->BindToCurrentSequence();
        resource_provider_ = std::make_unique<viz::ClientResourceProvider>();
        test_task_runner_ = base::MakeRefCounted<base::TestMockTimeTaskRunner>();
        resource_pool_ = std::make_unique<ResourcePool>(
            resource_provider_.get(), context_provider_.get(), test_task_runner_, ResourcePool::kDefaultExpirationDelay, false);
...
protected:
    ...
    void SetBackingOnResource(const ResourcePool::InUsePoolResource& resource)
    {
        ...
        backing->mailbox_sync_token.Set(gpu::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(1), 1);
...
TEST_F(ResourcePoolTest, AcquireRelease)
{
    ...
    ResourcePool::InUsePoolResource resource = resource_pool_->AcquireResource(size, format, color_space);
...
TEST_F(ResourcePoolTest, EventuallyEvictAndFlush)
{
    ...
    test_task_runner_->FastForwardBy(ResourcePool::kDefaultExpirationDelay + ResourcePool::kDefaultMaxFlushDelay);
...
TEST_F(ResourcePoolTest, FlushEvenIfMoreUnusedToEvict)
{
    ...
    viz::SharedImageFormat format = viz::SinglePlaneFormat::kRGBA_8888;
    gfx::ColorSpace color_space = gfx::ColorSpace::CreateSRGB();
    ResourcePool::InUsePoolResource resource1 = resource_pool_->AcquireResource(size, format, color_space);
    ResourcePool::InUsePoolResource resource2 = resource_pool_->AcquireResource(size, format, color_space);
    ...
    // last_usage_gap).
    const base::TimeDelta last_usage_gap = ResourcePool::kDefaultMaxFlushDelay * 2;
    ...
    test_task_runner_->FastForwardBy(ResourcePool::kDefaultExpirationDelay - last_usage_gap);
    EXPECT_EQ(1u, resource_pool_->GetTotalResourceCountForTesting());
...
TEST_F(ResourcePoolTest, SimpleResourceReuse)
{
    ...
    CheckAndReturnResource(resource_pool_->AcquireResource(size, format, color_space1));
    EXPECT_EQ(1u, resource_pool_->GetTotalResourceCountForTesting());

    // Same size/format should re-use resource.
    ResourcePool::InUsePoolResource resource = resource_pool_->AcquireResource(size, format, color_space1);
    EXPECT_EQ(1u, resource_pool_->GetTotalResourceCountForTesting());
    ...
    resource = resource_pool_->AcquireResource(gfx::Size(50, 50), viz::SinglePlaneFormat::kLUMINANCE_8, color_space1);
    EXPECT_EQ(2u, resource_pool_->GetTotalResourceCountForTesting());
    CheckAndReturnResource(std::move(resource));
    EXPECT_EQ(2u, resource_pool_->GetTotalResourceCountForTesting());
    EXPECT_EQ(0u, resource_pool_->GetBusyResourceCountForTesting());
...
TEST_F(ResourcePoolTest, LostResource)
{
    ...
    SetBackingOnResource(resource);
    EXPECT_TRUE(resource_pool_->PrepareForExport(resource, viz::TransferableResource::ResourceSource::kTest));

    std::vector<viz::ResourceId> export_ids = { resource.resource_id_for_export() };
    std::vector<viz::TransferableResource> transferable_resources;
    resource_provider_->PrepareSendToParent(export_ids, &transferable_resources, static_cast<viz::RasterContextProvider*>(context_provider_.get()));
    auto returned_resources = viz::TransferableResource::ReturnResources(transferable_resources);
...
TEST_F(ResourcePoolTest, BusyResourcesNotFreed)
{
    ...
    resource_provider_->PrepareSendToParent(
        { resource.resource_id_for_export() }, &transfers, static_cast<viz::RasterContextProvider*>(context_provider_.get()));
...
TEST_F(ResourcePoolTest, UnusedResourcesEventuallyFreed)
{
    ...
    resource_provider_->ReceiveReturnsFromParent(viz::TransferableResource::ReturnResources(transfers));
    EXPECT_EQ(40000u, resource_pool_->GetTotalMemoryUsageForTesting());
    EXPECT_EQ(1u, resource_pool_->GetTotalResourceCountForTesting());
    EXPECT_EQ(0u, resource_pool_->resource_count());
    EXPECT_EQ(0u, resource_pool_->GetBusyResourceCountForTesting());
...
TEST_F(ResourcePoolTest, UpdateContentId)
{
    ...
    ResourcePool::InUsePoolResource reacquired_resource
        = resource_pool_->TryAcquireResourceForPartialRaster(new_content_id, new_invalidated_rect, content_id, &invalidated_rect, color_space);
...
TEST_F(ResourcePoolTest, UpdateContentIdAndInvalidatedRect)
{
    ...
    ResourcePool::InUsePoolResource reacquired_resource
        = resource_pool_->TryAcquireResourceForPartialRaster(content_ids[1], invalidated_rect, content_ids[0], &new_invalidated_rect, color_space);
    ...
    reacquired_resource
        = resource_pool_->TryAcquireResourceForPartialRaster(content_ids[2], second_invalidated_rect, content_ids[1], &total_invalidated_rect, color_space);
    EXPECT_EQ(original_id, reacquired_resource.unique_id_for_testing());
...
TEST_F(ResourcePoolTest, LargeInvalidatedRect)
{
    ...
    gfx::Rect large_invalidated_rect(0, 0, std::numeric_limits<int>::max() / 2, std::numeric_limits<int>::max() / 2);
    ...
    gfx::Rect new_invalidated_rect;
    ResourcePool::InUsePoolResource reacquired_resource
        = resource_pool_->TryAcquireResourceForPartialRaster(content_ids[1], large_invalidated_rect, content_ids[0], &new_invalidated_rect, color_space);
    EXPECT_FALSE(!!reacquired_resource);
    ...
    resource = resource_pool_->TryAcquireResourceForPartialRaster(content_ids[2], large_invalidated_rect, content_ids[1], &new_invalidated_rect, color_space);
    EXPECT_TRUE(!!resource);
    resource_pool_->ReleaseResource(std::move(resource));
}
...
TEST_F(ResourcePoolTest, ReuseResource)
{
    ...
    ResourcePool::InUsePoolResource original = resource_pool_->AcquireResource(gfx::Size(100, 100), format, color_space);
    ...
    EXPECT_EQ(nullptr, resource_pool_->ReuseResource(gfx::Size(101, 100), format, color_space));
    EXPECT_EQ(nullptr, resource_pool_->ReuseResource(gfx::Size(100, 101), format, color_space));
    EXPECT_EQ(nullptr, resource_pool_->ReuseResource(gfx::Size(90, 120), format, color_space));
    EXPECT_EQ(nullptr, resource_pool_->ReuseResource(gfx::Size(120, 120), format, color_space));

    // Try some cases that are more than 2x smaller than 100x100 in area and
    // won't be re-used.
    EXPECT_EQ(nullptr, resource_pool_->ReuseResource(gfx::Size(49, 100), format, color_space));
    EXPECT_EQ(nullptr, resource_pool_->ReuseResource(gfx::Size(100, 49), format, color_space));
    EXPECT_EQ(nullptr, resource_pool_->ReuseResource(gfx::Size(50, 50), format, color_space));
    EXPECT_EQ(nullptr, resource_pool_->ReuseResource(gfx::Size(70, 70), format, color_space));
    ...
    if (resource_pool_->AllowsNonExactReUseForTesting()) {
        ResourcePool::InUsePoolResource reused = resource_pool_->AcquireResource(gfx::Size(50, 100), format, color_space);
        EXPECT_EQ(original_id, reused.unique_id_for_testing());
        CheckAndReturnResource(std::move(reused));
        reused = resource_pool_->AcquireResource(gfx::Size(100, 50), format, color_space);
        EXPECT_EQ(original_id, reused.unique_id_for_testing());
        CheckAndReturnResource(std::move(reused));
        reused = resource_pool_->AcquireResource(gfx::Size(71, 71), format, color_space);
        EXPECT_EQ(original_id, reused.unique_id_for_testing());
        CheckAndReturnResource(std::move(reused));
    } else {
        EXPECT_EQ(nullptr, resource_pool_->ReuseResource(gfx::Size(50, 100), format, color_space));
        EXPECT_EQ(nullptr, resource_pool_->ReuseResource(gfx::Size(100, 50), format, color_space));
        EXPECT_EQ(nullptr, resource_pool_->ReuseResource(gfx::Size(71, 71), format, color_space));
    }
    ...
    ResourcePool::InUsePoolResource resource = resource_pool_->AcquireResource(gfx::Size(100, 100), format, color_space);
    EXPECT_EQ(nullptr, resource_pool_->ReuseResource(gfx::Size(100, 100), format, color_space));
    CheckAndReturnResource(std::move(resource));
}
...
TEST_F(ResourcePoolTest, InvalidateResources)
{
    ...
    viz::SharedImageFormat format = viz::SinglePlaneFormat::kRGBA_8888;
    gfx::ColorSpace color_space = gfx::ColorSpace::CreateSRGB();
    ResourcePool::InUsePoolResource busy_resource = resource_pool_->AcquireResource(size, format, color_space);
    SetBackingOnResource(busy_resource);
    EXPECT_TRUE(resource_pool_->PrepareForExport(busy_resource, viz::TransferableResource::ResourceSource::kTest));
    EXPECT_EQ(1u, resource_pool_->GetTotalResourceCountForTesting());
    EXPECT_EQ(0u, resource_pool_->GetBusyResourceCountForTesting());
    EXPECT_EQ(1u, resource_pool_->resource_count());

    // Make a 2nd resource which will be left available in the pool.
    ResourcePool::InUsePoolResource avail_resource = resource_pool_->AcquireResource(size, format, color_space);
    EXPECT_EQ(2u, resource_pool_->GetTotalResourceCountForTesting());
    EXPECT_EQ(0u, resource_pool_->GetBusyResourceCountForTesting());
    EXPECT_EQ(2u, resource_pool_->resource_count());

    // Make a 3nd resource which will be kept in use.
    ResourcePool::InUsePoolResource in_use_resource = resource_pool_->AcquireResource(size, format, color_space);
    EXPECT_EQ(3u, resource_pool_->GetTotalResourceCountForTesting());
    EXPECT_EQ(0u, resource_pool_->GetBusyResourceCountForTesting());
    ...
    std::vector<viz::TransferableResource> transfers;
    resource_provider_->PrepareSendToParent(
        { busy_resource.resource_id_for_export() }, &transfers, static_cast<viz::RasterContextProvider*>(context_provider_.get()));
...
TEST_F(ResourcePoolTest, ExactRequestsRespected)
{
    ...
    resource_pool_
        = std::make_unique<ResourcePool>(resource_provider_.get(), context_provider_.get(), test_task_runner_, ResourcePool::kDefaultExpirationDelay, true);

    // Create unused resource with size 100x100.
    CheckAndReturnResource(resource_pool_->AcquireResource(gfx::Size(100, 100), format, color_space));
...
TEST_F(ResourcePoolTest, MetadataSentToDisplayCompositor)
{
    ...
    EXPECT_NE(gfx::BufferFormat::RGBA_8888, viz::SinglePlaneSharedImageFormatToBufferFormat(format));
    gfx::ColorSpace color_space = gfx::ColorSpace::CreateSRGB();
    gpu::SyncToken sync_token(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(0x123), 7);

    ResourcePool::InUsePoolResource resource = resource_pool_->AcquireResource(size, format, color_space);
    SetBackingOnResource(resource);

    // More non-default values.
    resource.gpu_backing()->shared_image = gpu::ClientSharedImage::CreateForTesting();
    ...
    resource_provider_->PrepareSendToParent(
        { resource.resource_id_for_export() }, &transfer, static_cast<viz::RasterContextProvider*>(context_provider_.get()));
    ...
    EXPECT_EQ(transfer[0].mailbox(), resource.gpu_backing()->shared_image->mailbox());
    EXPECT_EQ(transfer[0].sync_token(), sync_token);
    EXPECT_EQ(transfer[0].texture_target(), resource.gpu_backing()->shared_image->GetTextureTarget());
    EXPECT_EQ(transfer[0].format, format);
    EXPECT_EQ(transfer[0].synchronization_type, viz::TransferableResource::SynchronizationType::kGpuCommandsCompleted);
...
TEST_F(ResourcePoolTest, InvalidResource)
{
    ...
    EXPECT_FALSE(resource_pool_->PrepareForExport(resource, viz::TransferableResource::ResourceSource::kTest));
