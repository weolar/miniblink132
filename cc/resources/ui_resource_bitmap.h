...
public:
    enum UIResourceFormat { RGBA8, ALPHA_8, ETC1 };

    gfx::Size GetSize() const
    {
        return gfx::Size(info_.width(), info_.height());
    }
    UIResourceFormat GetFormat() const
    {
        return format_;
    }
    ...
    {
        return pixel_ref_ ? pixel_ref_->rowBytes() : 0;
    }

    bool IsUniquelyOwned() const
    {
        return pixel_ref_->unique();
    }
...
private:
    ...
    void Create(sk_sp<SkPixelRef> pixel_ref, const SkImageInfo& info, UIResourceFormat format);
