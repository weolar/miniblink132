...
class CC_EXPORT CrossThreadSharedBitmap : public base::RefCountedThreadSafe<CrossThreadSharedBitmap> {
public:
    CrossThreadSharedBitmap(const viz::SharedBitmapId& id, const base::ReadOnlySharedMemoryRegion region, base::WritableSharedMemoryMapping mapping,
        const gfx::Size& size, viz::SharedImageFormat format);

    const viz::SharedBitmapId& id() const
    {
        return id_;
    }
    ...
    void* memory()
    {
        return const_cast<void*>(std::as_const(*this).memory());
    }
