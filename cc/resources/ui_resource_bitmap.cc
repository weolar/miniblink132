...
UIResourceBitmap::UIResourceFormat SkColorTypeToUIResourceFormat(SkColorType sk_type)
{
...
void UIResourceBitmap::Create(sk_sp<SkPixelRef> pixel_ref, const SkImageInfo& info, UIResourceFormat format)
{
    DCHECK(info.width());
    DCHECK(info.height());
    DCHECK(pixel_ref);
...
void UIResourceBitmap::DrawToCanvas(SkCanvas* canvas, SkPaint* paint)
{
    ...
    if (GrDirectContext* direct_context = GrAsDirectContext(canvas->recordingContext())) {
...
base::span<const uint8_t> UIResourceBitmap::GetPixels() const
{
    ...
    // could be too large.
    return UNSAFE_TODO(base::span(static_cast<const uint8_t*>(pixel_ref_->pixels()), SizeInBytes()));
}
...
UIResourceBitmap::UIResourceBitmap(const SkBitmap& skbitmap)
{
    ...
    if (features::IsDrDcEnabled()) {
        ...
        if (skbitmap.colorType() != kN32_SkColorType) {
            ...
            copy_canvas.drawImage(skbitmap.asImage(), 0, 0, SkSamplingOptions(), nullptr);
            copy.setImmutable();
            target = &copy;
        }
    ...
    Create(std::move(pixel_ref), target->info(), SkColorTypeToUIResourceFormat(target->colorType()));
}
...
UIResourceBitmap::UIResourceBitmap(const gfx::Size& size, bool is_opaque)
{
    ...
    sk_sp<SkPixelRef> pixel_ref(SkMallocPixelRef::MakeAllocate(info, info.minRowBytes()));
    pixel_ref->setImmutable();
    Create(std::move(pixel_ref), info, UIResourceBitmap::RGBA8);
}

UIResourceBitmap::UIResourceBitmap(sk_sp<SkPixelRef> pixel_ref, const gfx::Size& size)
{
    // TODO(khushalsagar): It doesn't make sense to SkPixelRef to pass around
    // encoded data.
    SkImageInfo info = SkImageInfo::Make(size.width(), size.height(), kUnknown_SkColorType, kOpaque_SkAlphaType);
