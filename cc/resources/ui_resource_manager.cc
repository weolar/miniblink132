...
UIResourceId UIResourceManager::CreateUIResource(UIResourceClient* client)
{
    ...
    UIResourceRequest request(UIResourceRequest::Type::kCreate, next_id, client->GetBitmap(next_id, resource_lost));
...
void UIResourceManager::RecreateUIResources()
{
    for (const auto& resource : ui_resource_client_map_) {
        ...
        if (!base::Contains(ui_resource_request_queue_, uid, &UIResourceRequest::GetId)) {
            UIResourceRequest request(UIResourceRequest::Type::kCreate, uid, data.client->GetBitmap(uid, resource_lost));
            ui_resource_request_queue_.push_back(request);
        }
    }
}

base::flat_map<UIResourceId, gfx::Size> UIResourceManager::GetUIResourceSizes() const
{
    base::flat_map<UIResourceId, gfx::Size>::container_type items(ui_resource_client_map_.size());
...
UIResourceId UIResourceManager::GetOrCreateUIResource(const SkBitmap& bitmap)
{
    ...
    // map.
    std::erase_if(owned_shared_resources_, [](auto& pair) { return pair.second->IsUniquelyOwned(); });
    ...
    auto scoped_resource = ScopedUIResource::Create(this, UIResourceBitmap(bitmap));
