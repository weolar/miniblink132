...
struct CC_EXPORT InputHandlerPointerResult {
    ...
    ui::ScrollGranularity scroll_units = ui::ScrollGranularity::kScrollByPrecisePixel;
...
public:
    ...
    virtual void UpdateRootLayerStateForSynchronousInputHandler(const gfx::PointF& total_scroll_offset, const gfx::PointF& max_scroll_offset,
...
public:
    ...
    static base::WeakPtr<InputHandler> Create(CompositorDelegateForInput& compositor_delegate);
    ...
    struct ScrollStatus {
        ...
        uint32_t main_thread_hit_test_reasons = MainThreadScrollingReason::kNotScrollingOnMain;
        ...
        uint32_t main_thread_repaint_reasons = MainThreadScrollingReason::kNotScrollingOnMain;
    ...
    enum class TouchStartOrMoveEventListenerType { kNoHandler, kHandler, kHandlerOnScrollingLayer };
    ...
    // coordinates.
    virtual ScrollStatus ScrollBegin(ScrollState* scroll_state, ui::ScrollInputType type);

    // Similar to ScrollBegin, except the hit test is skipped and scroll always
    // targets at the root layer.
    virtual ScrollStatus RootScrollBegin(ScrollState* scroll_state, ui::ScrollInputType type);
    ...
    // is created.
    virtual InputHandlerScrollResult ScrollUpdate(ScrollState scroll_state, base::TimeDelta delayed_by = base::TimeDelta());
    ...
    // event.
    virtual void RecordScrollBegin(ui::ScrollInputType input_type, ScrollBeginThreadState scroll_start_state);
    ...
    virtual InputHandlerPointerResult MouseMoveAt(const gfx::Point& mouse_position);
    ...
    virtual InputHandlerPointerResult MouseDown(const gfx::PointF& mouse_position, bool shift_modifier);
    ...
    virtual ElementId FindFrameElementIdAtPoint(const gfx::PointF& mouse_position);
    ...
    virtual void SetSynchronousInputHandlerRootScrollOffset(const gfx::PointF& root_content_offset);

    virtual void PinchGestureBegin(const gfx::Point& anchor, ui::ScrollInputType source);
    virtual void PinchGestureUpdate(float magnify_delta, const gfx::Point& anchor);
    ...
    virtual EventListenerProperties GetEventListenerProperties(EventListenerClass event_class) const;

    // Returns true if |viewport_point| hits a wheel event handler region that
    // could block scrolling.
    virtual bool HasBlockingWheelEventHandlerAt(const gfx::Point& viewport_point) const;
    ...
    virtual TouchStartOrMoveEventListenerType EventListenerTypeForTouchStartOrMoveAt(const gfx::Point& viewport_point, TouchAction* out_touch_action);
    ...
    virtual std::unique_ptr<LatencyInfoSwapPromiseMonitor> CreateLatencyInfoSwapPromiseMonitor(ui::LatencyInfo* latency);
    ...
    // it.
    virtual std::unique_ptr<EventsMetricsManager::ScopedMonitor> GetScopedEventMetricsMonitor(EventsMetricsManager::ScopedMonitor::DoneCallback done_callback);
    ...
    virtual bool GetScrollOffsetForLayer(ElementId element_id, gfx::PointF* offset);
    ...
    virtual bool GetSnapFlingInfoAndSetAnimatingSnapTarget(
        const gfx::Vector2dF& current_delta, const gfx::Vector2dF& natural_displacement_in_viewport, gfx::PointF* initial_offset, gfx::PointF* target_offset);
    ...
    bool CanConsumeDelta(const ScrollState& scroll_state, const ScrollNode& scroll_node);
    // Returns the amount of delta that can be applied to scroll_node, taking
    // page scale into account.
    gfx::Vector2dF ComputeScrollDelta(const ScrollNode& scroll_node, const gfx::Vector2dF& delta);

    gfx::Vector2dF ScrollSingleNode(const ScrollNode& scroll_node, const gfx::Vector2dF& delta, const gfx::Point& viewport_point, bool is_direct_manipulation);

    float LineStep() const;

    // Resolves a delta in the given granularity for the |scroll_node| into
    // physical pixels to scroll.
    gfx::Vector2dF ResolveScrollGranularityToPixels(const ScrollNode& scroll_node, const gfx::Vector2dF& scroll_delta, ui::ScrollGranularity granularity);
    ...
    bool animating_for_snap_for_testing(ElementId element_id = ElementId()) const
    {
        return IsAnimatingForSnap(element_id);
    }

    base::flat_map<ElementId, SnapAnimationData>& get_snap_animation_data_map_for_testing()
    {
        return snap_animation_data_map_;
    }

    const std::unique_ptr<SnapSelectionStrategy>& snap_strategy_for_testing() const
    {
    ...
    // system.
    void ProcessCommitDeltas(CompositorCommitData* commit_data, const MutatorHost* main_thread_mutator_host) override;
    ...
    void DidRegisterScrollbar(ElementId scroll_element_id, ScrollbarOrientation orientation) override;
    void DidUnregisterScrollbar(ElementId scroll_element_id, ScrollbarOrientation orientation) override;
...
private:
    FRIEND_TEST_ALL_PREFIXES(LayerTreeHostImplTest, AbortAnimatedScrollBeforeStartingAutoscroll);
    ...
    void DidLatchToScroller(const ScrollState& scroll_state, ui::ScrollInputType type);
    ...
    void UpdateScrollSourceInfo(const ScrollState& scroll_state, ui::ScrollInputType type);
    ...
    void ScrollLatchedScroller(ScrollState& scroll_state, base::TimeDelta delayed_by);
    ...
    bool IsInitialScrollHitTestReliable(
        const LayerImpl* layer, const LayerImpl* first_layer_scrollable_or_opaque_to_hit_test, ScrollNode*& out_node_to_scroll) const;
    ...
    ScrollNode* FindNodeToLatch(ScrollState* scroll_state, ScrollNode* starting_node, ui::ScrollInputType type);
    ...
    ScrollHitTestResult HitTestScrollNode(const gfx::PointF& device_viewport_point) const;

    bool ShouldAnimateScroll(const ScrollState& scroll_state) const;

    std::optional<gfx::PointF> ScrollAnimationUpdateTarget(const ScrollNode& scroll_node, const gfx::Vector2dF& scroll_delta, base::TimeDelta delayed_by);
    ...
    bool CalculateLocalScrollDeltaAndStartPoint(const ScrollNode& scroll_node, const gfx::PointF& viewport_point, const gfx::Vector2dF& viewport_delta,
        gfx::Vector2dF* out_local_scroll_delta, gfx::PointF* out_local_start_point = nullptr);
    gfx::Vector2dF ScrollNodeWithViewportSpaceDelta(const ScrollNode& scroll_node, const gfx::PointF& viewport_point, const gfx::Vector2dF& viewport_delta);
    gfx::Vector2dF ScrollNodeWithLocalDelta(const ScrollNode& scroll_node, const gfx::Vector2dF& local_delta) const;
    ...
    // |overflow-x: hidden|).
    gfx::Vector2dF UserScrollableDelta(const ScrollNode& node, const gfx::Vector2dF& delta) const;

    void AdjustScrollDeltaForScrollbarSnap(ScrollState& scroll_state);

    FrameSequenceTrackerType GetTrackerTypeForScroll(ui::ScrollInputType input_type) const;
    ...
    std::unique_ptr<SnapSelectionStrategy> CreateSnapStrategy(const ScrollState& scroll_state, const gfx::PointF& current_offset, SnapReason snap_reason) const;
    ...
    TargetSnapAreaElementIds GetAnimatingSnapTargetsForElement(ElementId element_id) const;
    void SetAnimatingSnapTargetsForElement(ElementId element_id, TargetSnapAreaElementIds target_ids = TargetSnapAreaElementIds());
    ...
    enum SnapFlingState { kNoFling, kNativeFling, kConstrainedNativeFling, kSnapFling };
