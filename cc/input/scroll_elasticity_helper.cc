...
ScrollElasticityHelperImpl::ScrollElasticityHelperImpl(LayerTreeHostImpl* layer_tree)
...
{
    return host_impl_->OuterViewportScrollNode() ? host_impl_->OuterViewportScrollNode()->container_bounds : gfx::Size();
}

void ScrollElasticityHelperImpl::SetStretchAmount(const gfx::Vector2dF& stretch_amount)
{
...
{
    ScrollNode* root_scroll_node = host_impl_->OuterViewportScrollNode() ? host_impl_->OuterViewportScrollNode() : host_impl_->InnerViewportScrollNode();
    if (root_scroll_node) {
        LayerTreeImpl* tree_impl = host_impl_->active_tree();
        tree_impl->property_trees()->scroll_tree_mutable().ScrollBy(*root_scroll_node, delta, tree_impl);
    }
}
...
// static
ScrollElasticityHelper* ScrollElasticityHelper::CreateForLayerTreeHostImpl(LayerTreeHostImpl* host_impl)
{
