...
public:
    enum class AnimationDirection { kNoAnimation, kShowingControls, kHidingControls };
    ...
    BrowserControlsOffsetManager& operator=(const BrowserControlsOffsetManager&) = delete;
    ...
    {
        return top_controls_animation_.IsInitialized() && top_controls_animation_.Direction() == AnimationDirection::kShowingControls;
    }
    ...
    BrowserControlsState PullConstraintForMainThread(bool* out_changed_since_commit);
    ...
    double PredictViewportBoundsDelta(double current_bounds_delta, gfx::Vector2dF scroll_distance);

    void ResetAnimations();

protected:
    BrowserControlsOffsetManager(BrowserControlsOffsetManagerClient* client, float controls_show_threshold, float controls_hide_threshold);
...
private:
    ...
    void InitAnimationForHeightChange(Animation* animation, float start_ratio, float stop_ratio);
    ...
    // during the current min-height change animation.
    std::optional<std::pair<float, float>> bottom_min_height_offset_animation_range_;
    ...
    public:
        ...
        void Initialize(AnimationDirection direction, float start_value, float stop_value, int64_t duration, bool jump_to_end_on_reset);
