...
class MockSingleScrollbarAnimationControllerClient : public ScrollbarAnimationControllerClient {
public:
    MockSingleScrollbarAnimationControllerClient(LayerTreeHostImpl* host_impl, bool is_fluent)
    ...
    MOCK_METHOD2(PostDelayedScrollbarAnimationTask, void(base::OnceClosure start_fade, base::TimeDelta delay));
...
class SingleScrollbarAnimationControllerThinningTest : public LayerTreeImplTestBase, public testing::Test, public testing::WithParamInterface<bool> {
public:
    explicit SingleScrollbarAnimationControllerThinningTest(bool is_fluent = GetParam())
...
protected:
    ...
    void SetUp() override
    {
        ...
        scrollbar_controller_ = SingleScrollbarAnimationControllerThinning::Create(
            scroll_layer->element_id(), ScrollbarOrientation::kVertical, &client_, kThinningDuration, kIdleThicknessScale);
        mouse_move_distance_to_trigger_fade_in_ = scrollbar_controller_->MouseMoveDistanceToTriggerFadeIn();
        mouse_move_distance_to_trigger_expand_ = scrollbar_controller_->MouseMoveDistanceToTriggerExpand();
    }
...
class SingleScrollbarAnimationControllerThinningAuraTest : public SingleScrollbarAnimationControllerThinningTest {
public:
...
class SingleScrollbarAnimationControllerThinningFluentTest : public SingleScrollbarAnimationControllerThinningTest {
public:
...
{
    EXPECT_FLOAT_EQ(kIdleThicknessScale, scrollbar_layer_->thumb_thickness_scale_factor());
}
...
TEST_P(SingleScrollbarAnimationControllerThinningTest, MouseNear)
{
    ...
    scrollbar_controller_->DidMouseMove(NearScrollbar(-mouse_move_distance_to_trigger_expand_, 0));
    ...
    scrollbar_controller_->DidMouseMove(NearScrollbar(-mouse_move_distance_to_trigger_expand_ + 1, 0));
    scrollbar_controller_->Animate(time);
    ...
    scrollbar_controller_->DidMouseMove(NearScrollbar(-mouse_move_distance_to_trigger_fade_in_ - 1, 0));
    scrollbar_controller_->Animate(time);
    time += kThinningDuration;
    scrollbar_controller_->Animate(time);
    EXPECT_FLOAT_EQ(kIdleThicknessScale, scrollbar_layer_->thumb_thickness_scale_factor());
...
TEST_F(SingleScrollbarAnimationControllerThinningAuraTest, MouseNearThenAwayWhileAnimating)
{
    base::TimeTicks time;
    time += base::Seconds(1);
    ...
    scrollbar_controller_->Animate(time);
    EXPECT_FLOAT_EQ(1.0f - (1.0f - kIdleThicknessScale) / 2.0f, scrollbar_layer_->thumb_thickness_scale_factor());
    ...
    time += kThinningDuration / 4;
    scrollbar_controller_->Animate(time);
    EXPECT_FLOAT_EQ(kIdleThicknessScale + 3 * (1.0f - kIdleThicknessScale) / 4.0f, scrollbar_layer_->thumb_thickness_scale_factor());
...
// the bar gets thin.
TEST_P(SingleScrollbarAnimationControllerThinningTest, MouseCaptureAndReleaseOutOfBar)
{
    base::TimeTicks time;
    time += base::Seconds(1);
...
TEST_P(SingleScrollbarAnimationControllerThinningTest, MouseCaptureAndReleaseOnBar)
{
    base::TimeTicks time;
    time += base::Seconds(1);
...
TEST_P(SingleScrollbarAnimationControllerThinningTest, ThicknessAnimated)
{
    ...
    time += kThinningDuration / 2;
    scrollbar_controller_->Animate(time);
    EXPECT_FLOAT_EQ(kIdleThicknessScale + (1.0f - kIdleThicknessScale) / 2.0f, scrollbar_layer_->thumb_thickness_scale_factor());
...
TEST_F(SingleScrollbarAnimationControllerThinningFluentTest, MouseOverHiddenBar)
{
...
TEST_F(SingleScrollbarAnimationControllerThinningFluentTest, ShowTickmarksAndReleaseOutsideBar)
{
...
// area.
TEST_F(SingleScrollbarAnimationControllerThinningFluentTest, ShowTickmarksAndMouseLeave)
{
    EXPECT_CALL(client_, SetNeedsAnimateForScrollbarAnimation()).Times(0);
    // Simulate tickmarks showing. Scrollbar should be visible and fully thick.
    scrollbar_controller_->UpdateTickmarksVisibility(true);
...
TEST_F(SingleScrollbarAnimationControllerThinningFluentTest, ThickeningToThinningOnMouseLeave)
{
    EXPECT_FLOAT_EQ(kIdleThicknessScale, scrollbar_layer_->thumb_thickness_scale_factor());
    base::TimeTicks time;
    time += base::Seconds(1);
    ...
    scrollbar_controller_->Animate(time);
    EXPECT_FLOAT_EQ(kIdleThicknessScale + (1.f - kIdleThicknessScale) / 2.f, scrollbar_layer_->thumb_thickness_scale_factor());
...
TEST_F(SingleScrollbarAnimationControllerThinningFluentTest, DoesntAnimateOnFullModeMouseLeave)
{
    EXPECT_FLOAT_EQ(kIdleThicknessScale, scrollbar_layer_->thumb_thickness_scale_factor());
    base::TimeTicks time;
    time += base::Seconds(1);
...
TEST_P(SingleScrollbarAnimationControllerThinningTest, HoverTrackAndMoveThumbUnderPointer)
{
    EXPECT_POINTF_EQ(gfx::PointF(-1, -1), scrollbar_controller_->device_viewport_last_pointer_location());
    ...
    EXPECT_POINTF_EQ(near_scrollbar, scrollbar_controller_->device_viewport_last_pointer_location());
...
TEST_F(SingleScrollbarAnimationControllerThinningAuraTest, DidScrollUpdateQueuesAnimations)
{
    base::TimeTicks time;
    time += base::Seconds(1);
...
INSTANTIATE_TEST_SUITE_P(All, SingleScrollbarAnimationControllerThinningTest, Bool());
