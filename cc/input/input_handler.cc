...
InputHandlerClient::ScrollEventDispatchMode GetScrollEventDispatchMode()
{
    ...
    if (mode_name == ::features::kScrollEventDispatchModeDispatchScrollEventsImmediately) {
        return InputHandlerClient::ScrollEventDispatchMode::kDispatchScrollEventsImmediately;
    } else if (mode_name == ::features::kScrollEventDispatchModeUseScrollPredictorForEmptyQueue) {
        return InputHandlerClient::ScrollEventDispatchMode::kUseScrollPredictorForEmptyQueue;
    } else if (mode_name == ::features::kScrollEventDispatchModeUseScrollPredictorForDeadline) {
...
// static
base::WeakPtr<InputHandler> InputHandler::Create(CompositorDelegateForInput& compositor_delegate)
{
...
InputHandler::InputHandler(CompositorDelegateForInput& compositor_delegate)
    ...
    , scrollbar_controller_(std::make_unique<ScrollbarController>(&compositor_delegate_->GetImplDeprecated()))
{
}
...
void InputHandler::BindToClient(InputHandlerClient* client)
{
    ...
    if (base::FeatureList::IsEnabled(::features::kWaitForLateScrollEvents)) {
        input_handler_client_->SetScrollEventDispatchMode(GetScrollEventDispatchMode());
    }
}

InputHandler::ScrollStatus InputHandler::ScrollBegin(ScrollState* scroll_state, ui::ScrollInputType type)
{
    DCHECK(scroll_state);
    ...
    if (scroll_state->delta_granularity() == ui::ScrollGranularity::kScrollByPrecisePixel) {
        if (ScrollNode* animating_node = GetAnimatingNodeForCurrentScrollingNode()) {
            compositor_delegate_->GetImplDeprecated().mutator_host()->ScrollAnimationAbort(animating_node->element_id);
        }
    ...
    if (target_element_id && (!scroll_state->main_thread_hit_tested_reasons() || scroll_state->is_scrollbar_interaction())) {
        TRACE_EVENT_INSTANT0("cc", "Latched scroll node provided", TRACE_EVENT_SCOPE_THREAD);
        ...
        if (target_element_id) {
            TRACE_EVENT_INSTANT0("cc", "Unlatched scroll node provided", TRACE_EVENT_SCOPE_THREAD);
            ...
            TRACE_EVENT_INSTANT0("cc", "Hit Testing for ScrollNode", TRACE_EVENT_SCOPE_THREAD);
            gfx::Point viewport_point(scroll_state->position_x(), scroll_state->position_y());
            gfx::PointF device_viewport_point = gfx::ScalePoint(gfx::PointF(viewport_point), compositor_delegate_->DeviceScaleFactor());
            ...
            ScrollHitTestResult scroll_hit_test = HitTestScrollNode(device_viewport_point);
            ...
            if (!scroll_hit_test.hit_test_successful) {
                ...
                TRACE_EVENT_INSTANT0("cc", "Request Main Thread Hit Test", TRACE_EVENT_SCOPE_THREAD);
                ...
                CHECK(MainThreadScrollingReason::AreHitTestReasons(scroll_status.main_thread_hit_test_reasons));
                return scroll_status;
            }
        ...
        if (scrolling_node && compositor_delegate_->GetSettings().enable_fluent_overlay_scrollbar && scrolling_node->element_id != starting_node->element_id) {
    ...
    if (!scrolling_node) {
        if (compositor_delegate_->GetSettings().is_for_embedded_frame) {
            ...
            TRACE_EVENT_INSTANT0("cc", "Ignored - No ScrollNode (OOPIF or FencedFrame)", TRACE_EVENT_SCOPE_THREAD);
        } else {
            ...
            TRACE_EVENT_INSTANT0("cc", "Ignored - No ScrollNode", TRACE_EVENT_SCOPE_THREAD);
        }
        scroll_status.thread = InputHandler::ScrollThread::kScrollIgnored;
        return scroll_status;
    }

    DCHECK_EQ(scroll_status.thread, InputHandler::ScrollThread::kScrollOnImplThread);
    DCHECK(scrolling_node);

    ActiveTree().SetCurrentlyScrollingNode(scrolling_node);
    scroll_status.main_thread_repaint_reasons = scroll_tree.GetMainThreadRepaintReasons(*scrolling_node);
    CHECK(MainThreadScrollingReason::AreRepaintReasons(scroll_status.main_thread_repaint_reasons));
...
InputHandler::ScrollStatus InputHandler::RootScrollBegin(ScrollState* scroll_state, ui::ScrollInputType type)
{
    ...
    scroll_state->data()->set_current_native_scrolling_element(OuterViewportScrollNode()->element_id);
...
InputHandlerScrollResult InputHandler::ScrollUpdate(ScrollState scroll_state, base::TimeDelta delayed_by)
{
    ...
    TRACE_EVENT2("cc", "InputHandler::ScrollUpdate", "dx", scroll_state.delta_x(), "dy", scroll_state.delta_y());
    ...
    // granularity.
    if (scroll_state.is_scrollbar_interaction() && scroll_state.delta_granularity() != ui::ScrollGranularity::kScrollByPrecisePixel) {
        AdjustScrollDeltaForScrollbarSnap(scroll_state);
    }

    gfx::Vector2dF resolvedScrollDelta
        = ResolveScrollGranularityToPixels(scroll_node, gfx::Vector2dF(scroll_state.delta_x(), scroll_state.delta_y()), scroll_state.delta_granularity());
    ...
    if (scroll_state.delta_granularity() != ui::ScrollGranularity::kScrollByPrecisePixel) {
        scroll_state.data()->delta_granularity = ui::ScrollGranularity::kScrollByPixel;
    }

    compositor_delegate_->AccumulateScrollDeltaForTracing(gfx::Vector2dF(scroll_state.delta_x(), scroll_state.delta_y()));

    compositor_delegate_->WillScrollContent(scroll_node.element_id);

    float initial_top_controls_offset = compositor_delegate_->GetImplDeprecated().browser_controls_manager()->ControlsTopOffset();
    ...
    delta_consumed_for_scroll_gesture_ |= scroll_state.delta_consumed_for_scroll_sequence();
    bool did_scroll_content = did_scroll_x || did_scroll_y;
    if (did_scroll_content) {
        bool is_animated_scroll = ShouldAnimateScroll(scroll_state);
        compositor_delegate_->DidScrollContent(scroll_node.element_id, is_animated_scroll);
    }
    ...
    if (GetViewport().ShouldScroll(scroll_node)) {
        unused_root_delta = gfx::Vector2dF(scroll_state.delta_x(), scroll_state.delta_y());
    }

    // When inner viewport is unscrollable, disable overscrolls.
    if (auto* inner_viewport_scroll_node = InnerViewportScrollNode()) {
        unused_root_delta = UserScrollableDelta(*inner_viewport_scroll_node, unused_root_delta);
    }

    accumulated_root_overscroll_ += unused_root_delta;

    bool did_scroll_top_controls = initial_top_controls_offset != compositor_delegate_->GetImplDeprecated().browser_controls_manager()->ControlsTopOffset();
    ...
    scroll_result.overscroll_behavior
        = scroll_state.is_scroll_chain_cut() ? OverscrollBehavior(OverscrollBehavior::Type::kNone) : ActiveTree().overscroll_behavior();
    ...
    compositor_delegate_->GetImplDeprecated().mutator_host()->TickScrollAnimations(
        compositor_delegate_->GetImplDeprecated().CurrentBeginFrameArgs().frame_time, GetScrollTree());

    return scroll_result;
}

void InputHandler::AdjustScrollDeltaForScrollbarSnap(ScrollState& scroll_state)
{
    ...
    std::unique_ptr<SnapSelectionStrategy> strategy
        = SnapSelectionStrategy::CreateForDirection(current_position, gfx::Vector2dF(scroll_state.delta_x(), scroll_state.delta_y()), true);
...
void InputHandler::InsertPendingScrollendContainer(const ElementId& element_id)
{
    ...
    // consumed a delta.
    if ((!outer_viewport_scroll_node || outer_viewport_scroll_node->element_id != element_id) || outer_viewport_consumed_delta_) {
    ...
    if (outer_viewport_scroll_node) {
        ...
        if (outer_viewport_scroll_node->element_id == element_id) {
            ...
            if (inner_viewport_consumed_delta_) {
                pending_scrollend_containers_.insert(InnerViewportScrollNode()->element_id);
                inner_viewport_consumed_delta_ = false;
            }
        }
    }
...
void InputHandler::ScrollEnd(ScrollNode* scroll_node, bool should_snap)
{
    ...
    auto end_of_scroll_cleanup = [&]() {
        compositor_delegate_->GetImplDeprecated().browser_controls_manager()->ScrollEnd();
        deferred_scroll_end_ = false;
...
void InputHandler::RecordScrollBegin(ui::ScrollInputType input_type, ScrollBeginThreadState scroll_start_state)
{
    ...
    compositor_delegate_->GetImplDeprecated().frame_trackers().StartScrollSequence(tracker_type, scrolling_thread);
}

void InputHandler::RecordScrollEnd(ui::ScrollInputType input_type)
{
    compositor_delegate_->GetImplDeprecated().frame_trackers().StopSequence(GetTrackerTypeForScroll(input_type));
}

InputHandlerPointerResult InputHandler::MouseMoveAt(const gfx::Point& viewport_point)
{
    InputHandlerPointerResult result = scrollbar_controller_->HandlePointerMove(gfx::PointF(viewport_point));
    ...
    ScrollbarAnimationController* new_animation_controller
        = compositor_delegate_->GetImplDeprecated().ScrollbarAnimationControllerForElementId(scroll_element_id);
    if (scroll_element_id != scroll_element_id_mouse_currently_over_) {
        ScrollbarAnimationController* old_animation_controller
            = compositor_delegate_->GetImplDeprecated().ScrollbarAnimationControllerForElementId(scroll_element_id_mouse_currently_over_);
...
{
    return scrollbar_controller_->HitTest(viewport_point) ? PointerResultType::kScrollbarScroll : PointerResultType::kUnhandled;
}

InputHandlerPointerResult InputHandler::MouseDown(const gfx::PointF& viewport_point, bool shift_modifier)
{
    ScrollbarAnimationController* animation_controller
        = compositor_delegate_->GetImplDeprecated().ScrollbarAnimationControllerForElementId(scroll_element_id_mouse_currently_over_);
    if (animation_controller) {
        animation_controller->DidMouseDown();
        scroll_element_id_mouse_currently_captured_ = scroll_element_id_mouse_currently_over_;
    }
    return scrollbar_controller_->HandlePointerDown(viewport_point, shift_modifier);
}

InputHandlerPointerResult InputHandler::MouseUp(const gfx::PointF& viewport_point)
{
    if (scroll_element_id_mouse_currently_captured_) {
        ScrollbarAnimationController* animation_controller
            = compositor_delegate_->GetImplDeprecated().ScrollbarAnimationControllerForElementId(scroll_element_id_mouse_currently_captured_);
...
ElementId InputHandler::FindFrameElementIdAtPoint(const gfx::PointF& viewport_point)
{
    gfx::PointF device_viewport_point = gfx::ScalePoint(gfx::PointF(viewport_point), compositor_delegate_->DeviceScaleFactor());
...
void InputHandler::SetSynchronousInputHandlerRootScrollOffset(const gfx::PointF& root_content_offset)
{
    TRACE_EVENT2("cc", "InputHandler::SetSynchronousInputHandlerRootScrollOffset", "offset_x", root_content_offset.x(), "offset_y", root_content_offset.y());

    gfx::Vector2dF physical_delta = root_content_offset - GetViewport().TotalScrollOffset();
...
void InputHandler::PinchGestureBegin(const gfx::Point& anchor, ui::ScrollInputType source)
{
    DCHECK(source == ui::ScrollInputType::kTouchscreen || source == ui::ScrollInputType::kWheel);

    pinch_gesture_active_ = true;
    pinch_gesture_end_should_clear_scrolling_node_ = !CurrentlyScrollingNode();

    TRACE_EVENT_INSTANT1("cc", "SetCurrentlyScrollingNode PinchGestureBegin", TRACE_EVENT_SCOPE_THREAD, "isNull", OuterViewportScrollNode() ? false : true);
    ...
    if (OuterViewportScrollNode()) {
        ...
        scroll_state_data.delta_granularity = ui::ScrollGranularity::kScrollByPrecisePixel;
        scroll_state_data.is_direct_manipulation = source == ui::ScrollInputType::kTouchscreen;
        ScrollState state(scroll_state_data);

        DidLatchToScroller(state, source);
    }

    compositor_delegate_->GetImplDeprecated().browser_controls_manager()->PinchBegin();
    compositor_delegate_->DidStartPinchZoom();
}

void InputHandler::PinchGestureUpdate(float magnify_delta, const gfx::Point& anchor)
{
...
void InputHandler::PinchGestureEnd(const gfx::Point& anchor)
{
    ...
    bool snap_to_min = latched_scroll_type_.has_value() && latched_scroll_type_ == ui::ScrollInputType::kWheel;
    ...
    compositor_delegate_->GetImplDeprecated().browser_controls_manager()->PinchEnd();
    SetNeedsCommit();
...
EventListenerProperties InputHandler::GetEventListenerProperties(EventListenerClass event_class) const
{
    return ActiveTree().event_listener_properties(event_class);
}

bool InputHandler::HasBlockingWheelEventHandlerAt(const gfx::Point& viewport_point) const
{
    gfx::PointF device_viewport_point = gfx::ScalePoint(gfx::PointF(viewport_point), compositor_delegate_->DeviceScaleFactor());

    LayerImpl* layer_impl_with_wheel_event_handler = ActiveTree().FindLayerThatIsHitByPointInWheelEventHandlerRegion(device_viewport_point);

    return layer_impl_with_wheel_event_handler;
}

InputHandler::TouchStartOrMoveEventListenerType InputHandler::EventListenerTypeForTouchStartOrMoveAt(
    const gfx::Point& viewport_point, TouchAction* out_touch_action)
{
    gfx::PointF device_viewport_point = gfx::ScalePoint(gfx::PointF(viewport_point), compositor_delegate_->DeviceScaleFactor());

    LayerImpl* layer_impl_with_touch_handler = ActiveTree().FindLayerThatIsHitByPointInTouchHandlerRegion(device_viewport_point);
    ...
    if (out_touch_action) {
        gfx::Transform layer_screen_space_transform = layer_impl_with_touch_handler->ScreenSpaceTransform();
        ...
        gfx::Transform inverse_layer_screen_space = layer_screen_space_transform.GetCheckedInverse();
        bool clipped = false;
        gfx::PointF hit_test_point_in_layer_space = MathUtil::ProjectPoint(inverse_layer_screen_space, device_viewport_point, &clipped);
    ...
    if (currently_scroll_node && IsScrolledBy(layer_impl, currently_scroll_node)) {
        return InputHandler::TouchStartOrMoveEventListenerType::kHandlerOnScrollingLayer;
    } else if (features::MultiImplOnlyScrollAnimationsSupported() && !snap_animation_data_map_.empty()) {
        ...
        for (const auto& entry : snap_animation_data_map_) {
            ...
            if (ScrollNode* animating_node = scroll_tree.FindNodeFromElementId(entry.first)) {
...
std::unique_ptr<LatencyInfoSwapPromiseMonitor> InputHandler::CreateLatencyInfoSwapPromiseMonitor(ui::LatencyInfo* latency)
{
    return compositor_delegate_->GetImplDeprecated().CreateLatencyInfoSwapPromiseMonitor(latency);
}

std::unique_ptr<EventsMetricsManager::ScopedMonitor> InputHandler::GetScopedEventMetricsMonitor(EventsMetricsManager::ScopedMonitor::DoneCallback done_callback)
{
    return compositor_delegate_->GetImplDeprecated().GetScopedEventMetricsMonitor(std::move(done_callback));
}
...
ScrollElasticityHelper* InputHandler::CreateScrollElasticityHelper()
{
    ...
    if (compositor_delegate_->GetSettings().enable_elastic_overscroll) {
        scroll_elasticity_helper_.reset(ScrollElasticityHelper::CreateForLayerTreeHostImpl(&compositor_delegate_->GetImplDeprecated()));
    }
...
bool InputHandler::GetScrollOffsetForLayer(ElementId element_id, gfx::PointF* offset)
{
    ScrollTree& scroll_tree = GetScrollTree();
...
bool InputHandler::ScrollLayerTo(ElementId element_id, const gfx::PointF& offset)
{
    ScrollTree& scroll_tree = GetScrollTree();
    ScrollNode* scroll_node = scroll_tree.FindNodeFromElementId(element_id);
    if (!scroll_node)
    ...
    scroll_tree.ScrollBy(*scroll_node, offset - scroll_tree.current_scroll_offset(element_id), &ActiveTree());
    return true;
}
...
std::optional<gfx::PointF> InputHandler::ConstrainFling(gfx::PointF original)
{
    ...
    if (fling_snap_constrain_x_) {
        fling.set_x(std::clamp(fling.x(), fling_snap_constrain_x_->GetMin(), fling_snap_constrain_x_->GetMax()));
    }
    if (fling_snap_constrain_y_) {
        fling.set_y(std::clamp(fling.y(), fling_snap_constrain_y_->GetMin(), fling_snap_constrain_y_->GetMax()));
    }
    return original == fling ? std::nullopt : std::make_optional(fling);
}

double InputHandler::PredictViewportBoundsDelta(gfx::Vector2dF scroll_distance)
{
    ...
    double current_bounds_delta = GetScrollTree().property_trees()->outer_viewport_container_bounds_delta().y();
    return compositor_delegate_->GetImplDeprecated().browser_controls_manager()->PredictViewportBoundsDelta(current_bounds_delta, scroll_distance);
}

bool InputHandler::GetSnapFlingInfoAndSetAnimatingSnapTarget(const gfx::Vector2dF& current_delta, const gfx::Vector2dF& natural_displacement_in_viewport,
    gfx::PointF* out_initial_position, gfx::PointF* out_target_position)
{
    ScrollNode* scroll_node = CurrentlyScrollingNode();
    if (!scroll_node || !scroll_node->snap_container_data.has_value() || snap_fling_state_ == kNativeFling) {
        return false;
    }
    SnapContainerData& data = scroll_node->snap_container_data.value();

    float scale_factor = ActiveTree().page_scale_factor_for_scroll();
    gfx::Vector2dF current_delta_in_content = gfx::ScaleVector2d(current_delta, 1.f / scale_factor);
    gfx::Vector2dF snap_displacement = gfx::ScaleVector2d(natural_displacement_in_viewport, 1.f / scale_factor);
    ...
    std::unique_ptr<SnapSelectionStrategy> strategy
        = SnapSelectionStrategy::CreateForEndAndDirection(current_offset, snap_displacement, use_fractional_offsets);

    double snapport_height_adjustment = scroll_node->scrolls_outer_viewport ? PredictViewportBoundsDelta(snap_displacement) : 0;
    SnapPositionData snap = data.FindSnapPositionWithViewportAdjustment(*strategy, snapport_height_adjustment);
    if (snap.type == SnapPositionData::Type::kNone) {
        snap_fling_state_ = kNativeFling;
        return false;
    }

    if (snap_fling_state_ == kNoFling && snap.type == SnapPositionData::Type::kCovered) {
    ...
    *out_target_position = snap.position;
    ...
    SetAnimatingSnapTargetsForElement(scroll_node->element_id, snap.target_element_ids);
...
void InputHandler::ScrollEndForSnapFling(bool did_finish)
{
    ...
    if (did_finish && scroll_node && scroll_node->snap_container_data.has_value()) {
        TargetSnapAreaElementIds target_ids = GetAnimatingSnapTargetsForElement(scroll_node->element_id);
        scroll_node->snap_container_data.value().SetTargetSnapAreaElementIds(target_ids);
...
void InputHandler::ProcessCommitDeltas(CompositorCommitData* commit_data, const MutatorHost* main_thread_mutator_host)
{
    DCHECK(commit_data);
    if (ActiveTree().LayerListIsEmpty())
        return;

    ElementId inner_viewport_scroll_element_id = InnerViewportScrollNode() ? InnerViewportScrollNode()->element_id : ElementId();
    ...
    GetScrollTree().CollectScrollDeltas(commit_data, inner_viewport_scroll_element_id, compositor_delegate_->GetSettings().commit_fractional_scroll_deltas,
    ...
    if (features::MultiImplOnlyScrollAnimationsSupported()) {
        commit_data->scroll_end_data.done_containers = std::move(pending_scrollend_containers_);
        if (commit_data->scroll_end_data.done_containers.contains(last_latched_scroller_)) {
            last_latched_scroller_ = ElementId();
        }
    } else if (commit_data->scroll_end_data.scroll_gesture_did_end) {
        last_latched_scroller_ = ElementId();
        commit_data->scroll_end_data.gesture_affects_outer_viewport_scroll = outer_viewport_consumed_delta_;
        outer_viewport_consumed_delta_ = false;
        commit_data->scroll_end_data.gesture_affects_inner_viewport_scroll = inner_viewport_consumed_delta_;
        inner_viewport_consumed_delta_ = false;
    }
}
...
void InputHandler::DidRegisterScrollbar(ElementId scroll_element_id, ScrollbarOrientation orientation)
{
    scrollbar_controller_->DidRegisterScrollbar(scroll_element_id, orientation);
}

void InputHandler::DidUnregisterScrollbar(ElementId scroll_element_id, ScrollbarOrientation orientation)
{
...
void InputHandler::ScrollOffsetAnimationFinished(ElementId element_id)
{
    ...
    bool inner_viewport_animating = InnerViewportScrollNode() && finished_node == InnerViewportScrollNode();
    ...
    if (was_animating_for_snap) {
        if (finished_node && finished_node->snap_container_data.has_value()) {
            TargetSnapAreaElementIds target_ids = GetAnimatingSnapTargetsForElement(finished_node->element_id);
            finished_node->snap_container_data.value().SetTargetSnapAreaElementIds(target_ids);
    ...
    ClearAnimatingSnapTargetsForElement(finished_node ? finished_node->element_id : element_id);
...
ActivelyScrollingType InputHandler::GetActivelyScrollingType() const
{
    ...
    if (currently_scrolling_node && last_scroll_update_state_ && delta_consumed_for_scroll_gesture_) {
...
bool InputHandler::IsCurrentScrollMainRepainted() const
{
    const ScrollNode* scroll_node = CurrentlyScrollingNode();
    if (scroll_node) {
        uint32_t repaint_reasons = GetScrollTree().GetMainThreadRepaintReasons(*scroll_node);
    ...
    if (features::MultiImplOnlyScrollAnimationsSupported()) {
        ...
        for (const auto& entry : snap_animation_data_map_) {
            ...
            if (const ScrollNode* animating_node = scroll_tree.FindNodeFromElementId(entry.first)) {
                uint32_t repaint_reasons = GetScrollTree().GetMainThreadRepaintReasons(*animating_node);
                if (repaint_reasons != MainThreadScrollingReason::kNotScrollingOnMain) {
                    return true;
                }
            }
...
FrameSequenceTrackerType InputHandler::GetTrackerTypeForScroll(ui::ScrollInputType input_type) const
{
    switch (input_type) {
...
gfx::Vector2dF InputHandler::ResolveScrollGranularityToPixels(
    const ScrollNode& scroll_node, const gfx::Vector2dF& scroll_delta, ui::ScrollGranularity granularity)
{
    ...
    if (granularity == ui::ScrollGranularity::kScrollByPercentage) {
        ...
        pixel_delta = ScrollUtils::ResolveScrollPercentageToPixels(pixel_delta, scroller_size, viewport_size);
    }
...
InputHandler::ScrollHitTestResult InputHandler::HitTestScrollNode(const gfx::PointF& device_viewport_point) const
{
    ScrollHitTestResult result;
    result.scroll_node = nullptr;
    result.hit_test_successful = false;

    std::vector<const LayerImpl*> layers = ActiveTree().FindLayersUpToFirstScrollableOrOpaqueToHitTest(device_viewport_point);
    ...
    for (const auto* layer_impl : layers) {
        if (!IsInitialScrollHitTestReliable(layer_impl, first_scrollable_or_opaque_to_hit_test_layer, node_to_scroll)) {
            TRACE_EVENT_INSTANT0("cc", "Failed Hit Test", TRACE_EVENT_SCOPE_THREAD);
            result.main_thread_hit_test_reasons = MainThreadScrollingReason::kFailedHitTest;
            return result;
        }
        ...
        // return failure.
        if (ActiveTree().PointHitsMainThreadScrollHitTestRegion(device_viewport_point, *layer_impl)) {
            result.main_thread_hit_test_reasons = MainThreadScrollingReason::kMainThreadScrollHitTestRegion;
            return result;
        }

        if (ElementId scroll_element_id = ActiveTree().PointHitsNonCompositedScroll(device_viewport_point, *layer_impl)) {
...
ScrollNode* InputHandler::GetNodeToScroll(ScrollNode* node) const
{
    ...
    DCHECK(!node->prevent_viewport_scrolling_from_inner || node->scrolls_inner_viewport);

    if (node->scrolls_inner_viewport && !node->prevent_viewport_scrolling_from_inner) {
...
ScrollNode* InputHandler::GetNodeToScrollForLayer(const LayerImpl* layer) const
{
    if (layer->IsScrollbarLayer()) {
        ...
        if (auto* scroll_node = GetScrollTree().FindNodeFromElementId(ToScrollbarLayer(layer)->scroll_element_id())) {
...
bool InputHandler::IsInitialScrollHitTestReliable(
    const LayerImpl* layer_impl, const LayerImpl* first_scrollable_or_opaque_to_hit_test_layer, ScrollNode*& out_node_to_scroll) const
{
    ...
    if ((first_scrollable_or_opaque_to_hit_test_layer && !scroll_node) || (scroll_node && !first_scrollable_or_opaque_to_hit_test_layer)) {
        return false;
    }
    ...
    if (scroll_node == GetNodeToScrollForLayer(first_scrollable_or_opaque_to_hit_test_layer)) {
        out_node_to_scroll = scroll_node;
        return true;
    }
...
gfx::Vector2dF InputHandler::ComputeScrollDelta(const ScrollNode& scroll_node, const gfx::Vector2dF& delta)
{
    ScrollTree& scroll_tree = GetScrollTree();
    ...
    gfx::PointF old_offset = scroll_tree.current_scroll_offset(scroll_node.element_id);
    gfx::PointF new_offset = scroll_tree.ClampScrollOffsetToLimits(old_offset + adjusted_scroll, scroll_node);

    return new_offset - old_offset;
}

bool InputHandler::CalculateLocalScrollDeltaAndStartPoint(const ScrollNode& scroll_node, const gfx::PointF& viewport_point,
    const gfx::Vector2dF& viewport_delta, gfx::Vector2dF* out_local_scroll_delta, gfx::PointF* out_local_start_point /*= nullptr*/)
{
    ...
    const gfx::Transform screen_space_transform = GetScrollTree().ScreenSpaceTransform(scroll_node.id);
    ...
    gfx::Transform inverse_screen_space_transform = screen_space_transform.GetCheckedInverse();

    float scale_from_viewport_to_screen_space = compositor_delegate_->DeviceScaleFactor();
    ...
    gfx::PointF local_start_point = MathUtil::ProjectPoint(inverse_screen_space_transform, screen_space_point, &start_clipped);
    gfx::PointF local_end_point = MathUtil::ProjectPoint(inverse_screen_space_transform, screen_space_end_point, &end_clipped);
...
gfx::Vector2dF InputHandler::ScrollNodeWithViewportSpaceDelta(
    const ScrollNode& scroll_node, const gfx::PointF& viewport_point, const gfx::Vector2dF& viewport_delta)
{
    ScrollTree& scroll_tree = GetScrollTree();
    ...
    TRACE_EVENT2("cc", "ScrollNodeWithViewportSpaceDelta", "delta_y", local_scroll_delta.y(), "is_outer", scrolls_outer_viewport);

    // Apply the scroll delta.
    gfx::PointF previous_offset = scroll_tree.current_scroll_offset(scroll_node.element_id);
    scroll_tree.ScrollBy(scroll_node, local_scroll_delta, &ActiveTree());
    gfx::Vector2dF scrolled = scroll_tree.current_scroll_offset(scroll_node.element_id) - previous_offset;

    TRACE_EVENT_INSTANT1("cc", "ConsumedDelta", TRACE_EVENT_SCOPE_THREAD, "y", scrolled.y());
    ...
    gfx::PointF actual_screen_space_end_point = MathUtil::MapPoint(screen_space_transform, actual_local_end_point, &end_clipped);
    DCHECK(!end_clipped);
    if (end_clipped)
        return gfx::Vector2dF();

    float scale_from_viewport_to_screen_space = compositor_delegate_->DeviceScaleFactor();
    gfx::PointF actual_viewport_end_point = gfx::ScalePoint(actual_screen_space_end_point, 1.f / scale_from_viewport_to_screen_space);
    return actual_viewport_end_point - viewport_point;
}

gfx::Vector2dF InputHandler::ScrollNodeWithLocalDelta(const ScrollNode& scroll_node, const gfx::Vector2dF& local_delta) const
{
    bool scrolls_outer_viewport = scroll_node.scrolls_outer_viewport;
    TRACE_EVENT2("cc", "ScrollNodeWithLocalDelta", "delta_y", local_delta.y(), "is_outer", scrolls_outer_viewport);
    ...
    TRACE_EVENT_INSTANT1("cc", "ConsumedDelta", TRACE_EVENT_SCOPE_THREAD, "y", consumed_scroll.y());
...
gfx::Vector2dF InputHandler::ScrollSingleNode(
    const ScrollNode& scroll_node, const gfx::Vector2dF& delta, const gfx::Point& viewport_point, bool is_direct_manipulation)
{
    ...
    if (is_direct_manipulation) {
        ...
        return ScrollNodeWithViewportSpaceDelta(scroll_node, gfx::PointF(viewport_point), scaled_delta);
    }
...
ScrollNode* InputHandler::GetAnimatingNodeForCurrentScrollingNode()
{
    ...
    if (compositor_delegate_->GetImplDeprecated().mutator_host()->ElementHasImplOnlyScrollAnimation(scroll_node->element_id)) {
        return scroll_node;
    }
    ...
    if (scroll_node->scrolls_outer_viewport && inner_viewport_scroll_node) {
        if (compositor_delegate_->GetImplDeprecated().mutator_host()->ElementHasImplOnlyScrollAnimation(inner_viewport_scroll_node->element_id)) {
...
void InputHandler::ScrollLatchedScroller(ScrollState& scroll_state, base::TimeDelta delayed_by)
{
    ...
    TRACE_EVENT2("cc", "InputHandler::ScrollLatchedScroller", "delta_x", delta.x(), "delta_y", delta.y());
    ...
    if (ShouldAnimateScroll(scroll_state)) {
        ...
        if (ScrollNode* animating_scroll_node = GetAnimatingNodeForCurrentScrollingNode()) {
            TRACE_EVENT_INSTANT0("cc", "UpdateExistingAnimation", TRACE_EVENT_SCOPE_THREAD);

            // See comment in GetAnimatingNodeForCurrentScrollingNode for explanation
            // of this DCHECK.
            DCHECK(animating_scroll_node->id == scroll_node.id || animating_scroll_node->scrolls_inner_viewport);

            snap_strategy_offset = ScrollAnimationUpdateTarget(*animating_scroll_node, delta, delayed_by);
            ...
            if (snap_strategy_offset) {
                ...
                TRACE_EVENT_INSTANT0("cc", "Didn't Update Animation", TRACE_EVENT_SCOPE_THREAD);
            }
        } else {
            TRACE_EVENT_INSTANT0("cc", "CreateNewAnimation", TRACE_EVENT_SCOPE_THREAD);
            if (scroll_node.scrolls_outer_viewport) {
                ...
                compositor_delegate_->GetImplDeprecated().ScrollAnimationCreate(scroll_node, applied_delta, delayed_by);
            }
            gfx::PointF current_scroll_offset = GetVisualScrollOffset(scroll_node);
            snap_strategy_offset = GetScrollTree().ClampScrollOffsetToLimits(current_scroll_offset + applied_delta, scroll_node);
        }
        ...
        if (GetViewport().ShouldScroll(scroll_node)) {
            ...
            // controls if needed.
            ViewportScrollResult result = GetViewport().ScrollBy(delta, viewport_point, scroll_state.is_direct_manipulation(),
                latched_scroll_type_ != ui::ScrollInputType::kWheel, scroll_node.scrolls_outer_viewport);

            applied_delta = result.consumed_delta;
            delta_applied_to_content = result.content_scrolled_delta;
            SetViewportConsumedDelta(result);
        } else {
            applied_delta = ScrollSingleNode(scroll_node, delta, viewport_point, scroll_state.is_direct_manipulation());
        }
    ...
    scroll_state.set_caused_scroll(std::abs(delta_applied_to_content.x()) > kScrollEpsilon, std::abs(delta_applied_to_content.y()) > kScrollEpsilon);
    scroll_state.ConsumeDelta(applied_delta.x(), applied_delta.y());
    ...
    if (snap_strategy_offset && !scroll_state.is_in_inertial_phase()) {
        ...
        snap_strategy_ = CreateSnapStrategy(last_scroll_update_state_.has_value() ? last_scroll_update_state_.value() : scroll_state,
            snap_strategy_offset.value(), SnapReason::kScrollOffsetAnimationFinished);
    }
}

bool InputHandler::CanPropagate(ScrollNode* scroll_node, float x, float y)
{
    return (x == 0 || scroll_node->overscroll_behavior.x == OverscrollBehavior::Type::kAuto)
        && (y == 0 || scroll_node->overscroll_behavior.y == OverscrollBehavior::Type::kAuto);
}

ScrollNode* InputHandler::FindNodeToLatch(ScrollState* scroll_state, ScrollNode* starting_node, ui::ScrollInputType type)
{
    ScrollTree& scroll_tree = GetScrollTree();
    ScrollNode* scroll_node = nullptr;
    ScrollNode* first_scrollable_node = nullptr;
    for (ScrollNode* cur_node = starting_node; cur_node; cur_node = scroll_tree.parent(cur_node)) {
        ...
        if (!cur_node->user_scrollable_horizontal && !cur_node->user_scrollable_vertical) {
            continue;
        }
        ...
        float delta_x = scroll_state->is_beginning() ? scroll_state->delta_x_hint() : scroll_state->delta_x();
        float delta_y = scroll_state->is_beginning() ? scroll_state->delta_y_hint() : scroll_state->delta_y();
...
void InputHandler::UpdateRootLayerStateForSynchronousInputHandler()
{
    if (!input_handler_client_)
        return;
    input_handler_client_->UpdateRootLayerStateForSynchronousInputHandler(ActiveTree().TotalScrollOffset(), ActiveTree().TotalMaxScrollOffset(),
        ActiveTree().ScrollableSize(), ActiveTree().current_page_scale_factor(), ActiveTree().min_page_scale_factor(), ActiveTree().max_page_scale_factor());
}

void InputHandler::DidLatchToScroller(const ScrollState& scroll_state, ui::ScrollInputType type)
{
    DCHECK(CurrentlyScrollingNode());
    deferred_scroll_end_ = false;
    compositor_delegate_->GetImplDeprecated().browser_controls_manager()->ScrollBegin();
    if (ScrollNode* animating_node = GetAnimatingNodeForCurrentScrollingNode()) {
        compositor_delegate_->GetImplDeprecated().mutator_host()->ScrollAnimationAbort(animating_node->element_id);
    }
...
bool InputHandler::CanConsumeDelta(const ScrollState& scroll_state, const ScrollNode& scroll_node)
{
    gfx::Vector2dF delta_to_scroll;
    if (scroll_state.is_beginning()) {
        delta_to_scroll = gfx::Vector2dF(scroll_state.delta_x_hint(), scroll_state.delta_y_hint());
    } else {
        delta_to_scroll = gfx::Vector2dF(scroll_state.delta_x(), scroll_state.delta_y());
    }
    ...
    if (scroll_state.is_direct_manipulation()) {
        ...
        if (!CalculateLocalScrollDeltaAndStartPoint(
                scroll_node, gfx::PointF(scroll_state.position_x(), scroll_state.position_y()), delta_to_scroll, &local_scroll_delta)) {
            return false;
        }
        delta_to_scroll = local_scroll_delta;
    } else {
        delta_to_scroll = ResolveScrollGranularityToPixels(scroll_node, delta_to_scroll, scroll_state.delta_granularity());
    }
...
bool InputHandler::ShouldAnimateScroll(const ScrollState& scroll_state) const
{
    ...
    bool has_precise_scroll_deltas = scroll_state.delta_granularity() == ui::ScrollGranularity::kScrollByPrecisePixel;
...
bool InputHandler::SnapAtScrollEnd(SnapReason reason)
{
    ...
    ScrollState& last_scroll_state = last_scroll_update_state_ ? *last_scroll_update_state_ : *last_scroll_begin_state_;
    ...
    if (!snap_strategy_ || last_scroll_state.is_in_inertial_phase()) {
        ...
        // strategy.
        snap_strategy_ = CreateSnapStrategy(last_scroll_state, current_position, reason);
    }

    double snapport_height_adjustment = scroll_node->scrolls_outer_viewport ? PredictViewportBoundsDelta(gfx::Vector2dF()) : 0;
    SnapPositionData snap = data.FindSnapPositionWithViewportAdjustment(*snap_strategy_, snapport_height_adjustment);
    if (snap.type == SnapPositionData::Type::kNone) {
        ...
        if (reason == SnapReason::kScrollOffsetAnimationFinished) {
            ...
            SetAnimatingSnapTargetsForElement(CurrentlyScrollingNode()->element_id, snap.target_element_ids);
        } else if (data.SetTargetSnapAreaElementIds(snap.target_element_ids)) {
            updated_snapped_elements_[scroll_node->element_id] = snap.target_element_ids;
        }
        return false;
    }
    ...
    if (scroll_node->scrolls_outer_viewport) {
        ...
        did_animate = compositor_delegate_->GetImplDeprecated().ScrollAnimationCreate(*scroll_node, delta, base::TimeDelta());
    }
...
{
    return GetAnimatingSnapTargetsForElement(element_id) != TargetSnapAreaElementIds();
}

gfx::PointF InputHandler::GetVisualScrollOffset(const ScrollNode& scroll_node) const
{
    if (scroll_node.scrolls_outer_viewport)
...
void InputHandler::ClearCurrentlyScrollingNode()
{
    ...
    ClearAnimatingSnapTargetsForElement(CurrentlyScrollingNode() ? CurrentlyScrollingNode()->element_id : ElementId());
...
std::optional<gfx::PointF> InputHandler::ScrollAnimationUpdateTarget(
    const ScrollNode& scroll_node, const gfx::Vector2dF& scroll_delta, base::TimeDelta delayed_by)
{
    // TODO(bokan): Remove |scroll_node| as a parameter and just use the value
    // coming from |mutator_host|.
    DCHECK(compositor_delegate_->GetImplDeprecated().mutator_host()->ElementHasImplOnlyScrollAnimation(scroll_node.element_id));

    float scale_factor = compositor_delegate_->PageScaleFactor();
    gfx::Vector2dF adjusted_delta = gfx::ScaleVector2d(scroll_delta, 1.f / scale_factor);
    adjusted_delta = UserScrollableDelta(scroll_node, adjusted_delta);

    std::optional<gfx::PointF> animation_target = compositor_delegate_->GetImplDeprecated().mutator_host()->ImplOnlyScrollAnimationUpdateTarget(adjusted_delta,
        GetScrollTree().MaxScrollOffset(scroll_node.id), compositor_delegate_->GetImplDeprecated().CurrentBeginFrameArgs().frame_time, delayed_by,
...
void InputHandler::UpdateScrollSourceInfo(const ScrollState& scroll_state, ui::ScrollInputType type)
{
    if (type == ui::ScrollInputType::kWheel && scroll_state.delta_granularity() == ui::ScrollGranularity::kScrollByPrecisePixel) {
...
{
    DCHECK(ancestor && (ancestor->user_scrollable_horizontal || ancestor->user_scrollable_vertical));
    if (!child)
        return false;
    DCHECK_EQ(child->layer_tree_impl(), &ActiveTree());
    ScrollTree& scroll_tree = GetScrollTree();
    for (ScrollNode* scroll_node = scroll_tree.Node(child->scroll_tree_index()); scroll_node; scroll_node = scroll_tree.parent(scroll_node)) {
...
gfx::Vector2dF InputHandler::UserScrollableDelta(const ScrollNode& node, const gfx::Vector2dF& delta) const
{
...
{
    compositor_delegate_->UpdateBrowserControlsState(constraints, current, animate, offset_tags_info);
}
...
bool InputHandler::CurrentScrollNeedsFrameAlignment() const
{
    ...
    if (node && compositor_delegate_->HasScrollLinkedAnimation(node->element_id)) {
        return true;
    }
    ...
    if (features::MultiImplOnlyScrollAnimationsSupported()) {
        ...
        for (const auto& entry : snap_animation_data_map_) {
            ...
            if (const ScrollNode* animating_node = scroll_tree.FindNodeFromElementId(entry.first)) {
                if (compositor_delegate_->HasScrollLinkedAnimation(animating_node->element_id)) {
                    return true;
                }
            }
        }
...
std::unique_ptr<SnapSelectionStrategy> InputHandler::CreateSnapStrategy(
    const ScrollState& scroll_state, const gfx::PointF& current_offset, SnapReason snap_reason) const
{
    const gfx::Vector2dF scroll_delta = scroll_state.DeltaOrHint();
    if (latched_scroll_type_ == ui::ScrollInputType::kWheel && scroll_state.delta_granularity() != ui::ScrollGranularity::kScrollByPrecisePixel
        && !scroll_delta.IsZero() && snap_reason == SnapReason::kScrollOffsetAnimationFinished) {
        ...
        return SnapSelectionStrategy::CreateForDirection(current_offset, scroll_delta, true);
    } else {
        return SnapSelectionStrategy::CreateForEndPosition(current_offset, did_scroll_x_for_scroll_gesture_, did_scroll_y_for_scroll_gesture_);
    }
}

void InputHandler::SetViewportConsumedDelta(const ViewportScrollResult& result)
{
    if (std::abs(result.outer_viewport_scrolled_delta.x()) > kScrollEpsilon || std::abs(result.outer_viewport_scrolled_delta.y()) > kScrollEpsilon) {
        outer_viewport_consumed_delta_ = true;
    }
    if (std::abs(result.inner_viewport_scrolled_delta.x()) > kScrollEpsilon || std::abs(result.inner_viewport_scrolled_delta.y()) > kScrollEpsilon) {
        inner_viewport_consumed_delta_ = true;
    }
}

TargetSnapAreaElementIds InputHandler::GetAnimatingSnapTargetsForElement(ElementId element_id) const
{
    if (features::MultiImplOnlyScrollAnimationsSupported()) {
...
void InputHandler::SetAnimatingSnapTargetsForElement(ElementId element_id, TargetSnapAreaElementIds target_ids)
{
    if (features::MultiImplOnlyScrollAnimationsSupported()) {
        auto entry = snap_animation_data_map_.find(element_id);
        if (entry != snap_animation_data_map_.end()) {
