...
public:
    ...
    InputHandlerPointerResult HandlePointerDown(const gfx::PointF position_in_widget, const bool jump_key_modifier);
    InputHandlerPointerResult HandlePointerMove(const gfx::PointF position_in_widget);
    InputHandlerPointerResult HandlePointerUp(const gfx::PointF position_in_widget);

    bool AutoscrollTaskIsScheduled() const
    {
        return cancelable_autoscroll_task_ != nullptr;
    }
    bool ScrollbarScrollIsActive() const
    {
        return scrollbar_scroll_is_active_;
    }
    void DidRegisterScrollbar(ElementId element_id, ScrollbarOrientation orientation);
    void DidUnregisterScrollbar(ElementId element_id, ScrollbarOrientation orientation);
    ScrollbarLayerImplBase* ScrollbarLayer() const;
    void WillBeginImplFrame();
    void ResetState();
    const ScrollbarLayerImplBase* HitTest(const gfx::PointF position_in_widget) const;
...
private:
    ...
    ScrollbarPart GetScrollbarPartFromPointerDown(const gfx::PointF position_in_widget) const;
    ...
    gfx::Vector2dF ComputeClampedDelta(const ScrollNode& target_node, const gfx::Vector2dF& scroll_delta) const;
    ...
    // tested.
    gfx::Vector2dF GetScrollDeltaForScrollbarPart(const ScrollbarPart scrollbar_part, const bool jump_key_modifier) const;
    // Returns scroll delta in the direction of the scrollbar's orientation.
    float GetScrollDistanceForScrollbarPart(const ScrollbarPart scrollbar_part, const bool jump_key_modifier) const;

    // Makes position_in_widget relative to the scrollbar.
    gfx::PointF GetScrollbarRelativePosition(const gfx::PointF position_in_widget, bool* clipped) const;
    ...
    gfx::PointF DragOriginForJumpClick(const ScrollbarLayerImplBase* scrollbar) const;
    ...
    ui::ScrollGranularity Granularity(const ScrollbarPart scrollbar_part, bool jump_key_modifier) const;

    // Calculates the distance based on position_in_widget and drag_origin.
    float GetScrollDistanceForDragPosition(const gfx::PointF pointer_position_in_widget) const;
