...
struct ScrollSnapType {
    ...
    {
        return is_none == other.is_none && axis == other.axis && strictness == other.strictness;
    }
...
struct ScrollSnapAlign {
    ...
    {
        return alignment_block == other.alignment_block && alignment_inline == other.alignment_inline;
    }
...
struct SnapSearchResultAlternative {
    explicit SnapSearchResultAlternative(const ElementId id, gfx::RectF rect, float area_cross_axis_snap_offset)
    {
        element_id = id;
        area_rect = rect;
...
public:
    ...
    SnapSearchResult(float offset, SearchAxis axis, gfx::RangeF snapport_visible_range, float max_visible);
    ...
    float snap_offset() const
    {
        return snap_offset_;
    }
    void set_snap_offset(float offset)
    {
        snap_offset_ = offset;
    }
    ...
    gfx::RangeF visible_range() const
    {
        ...
        const float rect_start = axis_ == SearchAxis::kX ? rect_.value().y() : rect_.value().x();
        const float rect_end = axis_ == SearchAxis::kX ? rect_.value().bottom() : rect_.value().right();
        return gfx::RangeF(std::clamp(rect_start - snapport_visible_range_.end(), 0.0f, snapport_max_visible_),
            std::clamp(rect_end - snapport_visible_range_.start(), 0.0f, snapport_max_visible_));
    }
    ...
    std::optional<gfx::RangeF> covered_range() const
    {
        return covered_range_;
    }
    void set_covered_range(const gfx::RangeF& range)
    {
        covered_range_ = range;
    }

    bool has_focus_within() const
    {
        return has_focus_within_;
    }
    ...
    void set_axis(const SearchAxis& axis)
    {
        axis_ = axis;
    }
    ...
    std::optional<gfx::RectF> rect() const
    {
        return rect_;
    }
    ...
    void set_alternative(const ElementId& id, const gfx::RectF& rect, float alt_cross_snap_offset)
    {
...
struct SnapAreaData {
    ...
    SnapAreaData(const ScrollSnapAlign& align, const gfx::RectF& rec, bool msnap, bool has_focus_within, ElementId id)
    ...
    {
        return (other.element_id == element_id) && (other.scroll_snap_align == scroll_snap_align) && (other.rect == rect) && (other.must_snap == must_snap)
...
struct TargetSnapAreaElementIds {
    ...
    TargetSnapAreaElementIds(ElementId x_id, ElementId y_id)
        : x(x_id)
        , y(y_id)
    {
    }
...
public:
    ...
    SnapContainerData(ScrollSnapType type, const gfx::RectF& rect, const gfx::PointF& max);
    ...
    {
        return (other.scroll_snap_type_ == scroll_snap_type_) && (other.rect_ == rect_) && (other.max_position_ == max_position_)
            && (other.proximity_range_ == proximity_range_) && (other.snap_area_list_ == snap_area_list_)
            && (other.target_snap_area_element_ids_ == target_snap_area_element_ids_) && (other.targeted_area_id_ == targeted_area_id_)
    ...
    SnapPositionData FindSnapPositionWithViewportAdjustment(const SnapSelectionStrategy& strategy, double snapport_height_adjustment);
    ...
    size_t size() const
    {
        return snap_area_list_.size();
    }
    const SnapAreaData& at(size_t index) const
    {
        return snap_area_list_[index];
    }
    ...
    ScrollSnapType scroll_snap_type() const
    {
    ...
    void set_max_position(gfx::PointF position)
    {
        max_position_ = position;
    }
    gfx::PointF max_position() const
    {
    ...
    gfx::PointF proximity_range() const
    {
        return proximity_range_;
    }
...
private:
    ...
    std::optional<SnapSearchResult> FindClosestValidAreaInternal(SearchAxis axis, const SnapSelectionStrategy& strategy,
        const SnapSearchResult& cross_axis_snap_result, bool should_consider_covering = true,
    ...
    bool FindSnapPositionForMutualSnap(const SnapSelectionStrategy& strategy, gfx::PointF* snap_position) const;
    ...
    //   axis.
    SnapSearchResult GetSnapSearchResult(SearchAxis axis, const SnapAreaData& data) const;

    bool IsSnapportCoveredOnAxis(SearchAxis axis, float current_offset, const gfx::RectF& area_rect) const;

    void UpdateSnapAreaForTesting(ElementId element_id, SnapAreaData snap_area_data);
    ...
    bool IsSnappedToArea(const SnapAreaData& area, const gfx::PointF& scroll_offset) const;
    ...
    // instead.
    void UpdateSearchAlternative(SnapSearchResult& current_result, const SnapSearchResult& candidate_result, const SnapAreaData& candidate_area,
    ...
    SnapAxis SelectAxisToFollowForMutualVisibility(const SnapSelectionStrategy&, const SnapSearchResult& x_result, const SnapSearchResult& y_result) const;
