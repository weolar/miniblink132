...
std::string MainThreadScrollingReason::AsText(uint32_t reasons)
{
    ...
    result = result.substr(array_start_pos + 1, array_end_pos - array_start_pos - 1);
    std::erase(result, '\"');
...
void MainThreadScrollingReason::AddToTracedValue(uint32_t reasons, base::trace_event::TracedValue& traced_value)
{
...
#define ADD_REASON(reason, string)                                                                                                                             \
    ...
    ADD_REASON(kHasBackgroundAttachmentFixedObjects, "Has background-attachment:fixed");
    ...
    ADD_REASON(kBackgroundNeedsRepaintOnScroll, "Background needs repaint on scroll");
    ADD_REASON(kScrollbarScrolling, "Scrollbar scrolling");
    ADD_REASON(kMainThreadScrollHitTestRegion, "Main thread scroll hit test region");
    ADD_REASON(kFailedHitTest, "Failed hit test");
    ADD_REASON(kPopupNoThreadedInput, "Popup scrolling (no threaded input handler)");
