...
public:
    ...
    static std::unique_ptr<SnapSelectionStrategy> CreateForEndPosition(const gfx::PointF& current_position, bool scrolled_x, bool scrolled_y);
    ...
    static std::unique_ptr<SnapSelectionStrategy> CreateForDirection(
        gfx::PointF current_position, gfx::Vector2dF step, bool use_fractional_offsets, SnapStopAlwaysFilter filter = SnapStopAlwaysFilter::kIgnore);
    ...
    static std::unique_ptr<SnapSelectionStrategy> CreateForTargetElement(gfx::PointF current_position);
    ...
    const gfx::PointF& current_position() const
    {
...
public:
    EndPositionStrategy(const gfx::PointF& current_position, bool scrolled_x, bool scrolled_y,
        SnapTargetsPrioritization snap_targets_prioritization = SnapTargetsPrioritization::kIgnore)
    ...
    const std::optional<SnapSearchResult>& PickBestResult(
        const std::optional<SnapSearchResult>& closest, const std::optional<SnapSearchResult>& covering) const override;
...
public:
    // |use_fractional_offsets| should be true when the current position is
    // provided in fractional pixels.
    DirectionStrategy(const gfx::PointF& current_position, const gfx::Vector2dF& step, SnapStopAlwaysFilter filter, bool use_fractional_offsets)
        : SnapSelectionStrategy(current_position)
        , step_(step)
    ...
    bool IsValidSnapPosition(SearchAxis axis, float position) const override;
    bool IsValidSnapArea(SearchAxis axis, const SnapAreaData& area) const override;
...
public:
    // |use_fractional_offsets| should be true when the current position is
    // provided in fractional pixels.
    EndAndDirectionStrategy(const gfx::PointF& current_position, const gfx::Vector2dF& displacement, bool use_fractional_offsets)
        : SnapSelectionStrategy(current_position)
