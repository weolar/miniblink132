...
{
    SnapFlingCurve active_curve(gfx::PointF(100, 100), gfx::PointF(500, 500), base::TimeTicks());
    EXPECT_FALSE(active_curve.IsFinished());

    SnapFlingCurve finished_curve(gfx::PointF(100, 100), gfx::PointF(100, 100), base::TimeTicks());
    EXPECT_TRUE(finished_curve.IsFinished());
}

TEST(SnapFlingCurveTest, AdvanceHalfwayThrough)
{
    SnapFlingCurve curve(gfx::PointF(100, 100), gfx::PointF(500, 500), base::TimeTicks());
    base::TimeDelta duration = curve.duration();
    gfx::Vector2dF delta1 = curve.GetScrollDelta(base::TimeTicks() + duration / 2);
    ...
    gfx::Vector2dF delta2 = curve.GetScrollDelta(base::TimeTicks() + duration / 2);
...
{
    SnapFlingCurve curve(gfx::PointF(100, 100), gfx::PointF(500, 500), base::TimeTicks());
    gfx::Vector2dF delta = curve.GetScrollDelta(base::TimeTicks() + curve.duration());
...
{
    SnapFlingCurve curve(gfx::PointF(0, 0), gfx::PointF(100.5, 99.5), base::TimeTicks());
    EXPECT_FALSE(curve.IsFinished());
