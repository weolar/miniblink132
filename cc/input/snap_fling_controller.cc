...
bool SnapFlingController::FilterEventForSnap(SnapFlingController::GestureScrollType gesture_scroll_type)
{
...
bool SnapFlingController::HandleGestureScrollUpdate(const SnapFlingController::GestureScrollUpdateInfo& info)
{
    DCHECK(state_ == State::kIdle || state_ == State::kIgnored);
    if (!info.is_in_inertial_phase)
        return false;

    gfx::Vector2dF ending_displacement = SnapFlingCurve::EstimateDisplacement(info.delta);

    gfx::PointF target_offset, start_offset;
    if (!client_->GetSnapFlingInfoAndSetAnimatingSnapTarget(info.delta, ending_displacement, &start_offset, &target_offset)) {
    ...
    curve_ = std::make_unique<SnapFlingCurve>(start_offset, target_offset, info.event_time);
...
void SnapFlingController::SetCurveForTest(std::unique_ptr<SnapFlingCurve> curve)
{
