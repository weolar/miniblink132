...
public:
    ...
    void SetActiveState()
    {
        controller_->SetActiveStateForTest();
    }
...
{
    EXPECT_FALSE(controller_->FilterEventForSnap(SnapFlingController::GestureScrollType::kBegin));
}

TEST_F(SnapFlingControllerTest, FiltersGSUAndGSEDependingOnState)
{
    // Should not filter GSU and GSE if the fling is not active.
    EXPECT_FALSE(controller_->FilterEventForSnap(SnapFlingController::GestureScrollType::kUpdate));
    EXPECT_FALSE(controller_->FilterEventForSnap(SnapFlingController::GestureScrollType::kEnd));

    // Should filter GSU and GSE if the fling is active.
    SetActiveState();
    EXPECT_TRUE(controller_->FilterEventForSnap(SnapFlingController::GestureScrollType::kUpdate));
    EXPECT_TRUE(controller_->FilterEventForSnap(SnapFlingController::GestureScrollType::kEnd));
}
...
TEST_F(SnapFlingControllerTest, CreatesAndAnimatesCurveOnFirstInertialGSU)
{
    ...
    EXPECT_CALL(mock_client_, GetSnapFlingInfoAndSetAnimatingSnapTarget(testing::_, testing::_, testing::_, testing::_))
        .WillOnce(testing::DoAll(testing::SetArgPointee<2>(gfx::PointF(0, 0)), testing::SetArgPointee<3>(gfx::PointF(0, 100)), testing::Return(true)));
...
TEST_F(SnapFlingControllerTest, ScrollEndWhenHasEqualOffsetsOnInertialGSU)
{
    ...
    EXPECT_CALL(mock_client_, GetSnapFlingInfoAndSetAnimatingSnapTarget(testing::_, testing::_, testing::_, testing::_))
        .WillOnce(testing::DoAll(testing::SetArgPointee<2>(gfx::PointF(0, 0)), testing::SetArgPointee<3>(gfx::PointF(0, 0)), testing::Return(true)));
...
TEST_F(SnapFlingControllerTest, DoesNotHandleNonInertialGSU)
{
    ...
    EXPECT_CALL(mock_client_, GetSnapFlingInfoAndSetAnimatingSnapTarget(testing::_, testing::_, testing::_, testing::_)).Times(0);
    EXPECT_CALL(mock_client_, RequestAnimationForSnapFling()).Times(0);
    EXPECT_CALL(mock_client_, ScrollByForSnapFling(testing::_)).Times(0);
...
{
    std::unique_ptr<MockSnapFlingCurve> mock_curve = std::make_unique<MockSnapFlingCurve>();
    ...
    EXPECT_CALL(*curve, GetScrollDelta(testing::_)).WillOnce(testing::Return(gfx::Vector2dF(100, 100)));
    EXPECT_CALL(mock_client_, RequestAnimationForSnapFling()).Times(1);
