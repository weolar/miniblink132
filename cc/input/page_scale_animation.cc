...
gfx::Vector2dF NormalizeFromViewport(const gfx::Vector2dF& denormalized, const gfx::SizeF& viewport_size)
{
    DCHECK(!viewport_size.IsEmpty());
    return gfx::ScaleVector2d(denormalized, 1.f / viewport_size.width(), 1.f / viewport_size.height());
}

gfx::Vector2dF DenormalizeToViewport(const gfx::Vector2dF& normalized, const gfx::SizeF& viewport_size)
{
    return gfx::ScaleVector2d(normalized, viewport_size.width(), viewport_size.height());
}
...
std::unique_ptr<PageScaleAnimation> PageScaleAnimation::Create(
    const gfx::PointF& start_scroll_offset, float start_page_scale_factor, const gfx::SizeF& viewport_size, const gfx::SizeF& root_layer_size)
{
    return base::WrapUnique(new PageScaleAnimation(start_scroll_offset, start_page_scale_factor, viewport_size, root_layer_size));
}
...
void PageScaleAnimation::ZoomTo(const gfx::PointF& target_scroll_offset, float target_page_scale_factor, double duration)
{
...
void PageScaleAnimation::ZoomWithAnchor(const gfx::PointF& anchor, float target_page_scale_factor, double duration)
{
...
{
    gfx::Vector2dF normalized = NormalizeFromViewport(start_anchor_ - start_scroll_offset_, StartViewportSize());
...
void PageScaleAnimation::InferTargetAnchorFromScrollOffsets()
{
    ...
    gfx::Vector2dF normalized = gfx::ScaleVector2d(start_scroll_offset_ - target_scroll_offset_, width_scale, height_scale);
...
void PageScaleAnimation::ClampTargetScrollOffset()
{
    gfx::PointF max_scroll_offset
        = gfx::PointAtOffsetFromOrigin(gfx::RectF(root_layer_size_).bottom_right() - gfx::RectF(gfx::SizeF(TargetViewportSize())).bottom_right());
...
gfx::PointF PageScaleAnimation::AnchorAt(float interp) const
{
    ...
    return gfx::PointAtOffsetFromOrigin(InterpolateBetween(start_anchor_.OffsetFromOrigin(), target_anchor_.OffsetFromOrigin(), interp));
}
...
gfx::Vector2dF PageScaleAnimation::ViewportRelativeAnchorAt(float interp) const
{
    ...
    gfx::Vector2dF start_normalized = NormalizeFromViewport(start_anchor_ - start_scroll_offset_, StartViewportSize());
    gfx::Vector2dF target_normalized = NormalizeFromViewport(target_anchor_ - target_scroll_offset_, TargetViewportSize());
