...
double EstimateFramesFromDistance(double distance)
{
    ...
    return std::ceil(-std::log(1 - distance * (1 - 1 / kRatio)) / std::log(kRatio));
}
...
{
    return std::abs(actual.x() - target.x()) < 1 && std::abs(actual.y() - target.y()) < 1;
}

} // namespace

gfx::Vector2dF SnapFlingCurve::EstimateDisplacement(const gfx::Vector2dF& first_delta)
{
...
SnapFlingCurve::SnapFlingCurve(const gfx::PointF& start_offset, const gfx::PointF& target_offset, base::TimeTicks first_gsu_time)
...
double SnapFlingCurve::GetCurrentCurveDistance(base::TimeDelta current_time)
{
    ...
    const double sum = first_delta_ * (1 - std::pow(kRatio, current_frame)) / (1 - kRatio);
...
gfx::Vector2dF SnapFlingCurve::GetScrollDelta(base::TimeTicks time_stamp)
{
    ...
    if (current_time >= std::min(duration_, kMaximumSnapDuration) || IsWithinOnePixel(current_displacement_, total_displacement_)) {
        is_finished_ = true;
        return total_displacement_ - current_displacement_;
    }

    double new_distance = GetCurrentCurveDistance(current_time);
    gfx::Vector2dF new_displacement(new_distance * ratio_x_, new_distance * ratio_y_);
