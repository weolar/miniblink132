...
public:
    Occlusion();
    Occlusion(const gfx::Transform& draw_transform, const SimpleEnclosedRegion& occlusion_from_outside_target,
        const SimpleEnclosedRegion& occlusion_from_inside_target);
    Occlusion GetOcclusionWithGivenDrawTransform(const gfx::Transform& transform) const;
...
private:
    gfx::Rect GetUnoccludedRectInTargetSurface(const gfx::Rect& content_rect) const;
