...
public:
    LayerTreeHostPerfTest()
        : draw_timer_(kWarmupRuns, base::Milliseconds(kTimeLimitMillis), kTimeCheckInterval)
    ...
    {
        reporter_ = std::make_unique<perf_test::PerfResultReporter>("layer_tree_host", story_name);
        reporter_->RegisterImportantMetric("_frame_time", "us");
        reporter_->RegisterImportantMetric("_commit_time", "us");
    }

    virtual void CleanUpAndEndTest()
    {
        EndTest();
    }
    ...
    void AfterTest() override
    {
        ...
        reporter_->AddResult("_frame_time", draw_timer_.TimePerLap().InMicrosecondsF());
        if (measure_commit_cost_) {
            reporter_->AddResult("_commit_time", commit_timer_.TimePerLap().InMicrosecondsF());
        }
    }
...
public:
    ...
    void BuildTree() override
    {
        ...
        layer_tree_host()->SetViewportRectAndScale(gfx::Rect(viewport), 1.f, viz::LocalSurfaceId());
        scoped_refptr<Layer> root = ParseTreeFromJson(json_, &fake_content_layer_client_);
...
// Simulates a tab switcher scene with two stacks of 10 tabs each.
TEST_F(LayerTreeHostPerfTestJsonReader, TenTenSingleThread_FullDamageEachFrame)
{
...
class LayerTreeHostPerfTestLeafInvalidates : public LayerTreeHostPerfTestJsonReader {
public:
    void BuildTree() override
    {
        ...
        // Find a leaf layer.
        for (layer_to_invalidate_ = layer_tree_host()->root_layer(); layer_to_invalidate_->children().size();
    ...
    void DidCommitAndDrawFrame() override
    {
        ...
        layer_to_invalidate_->SetOpacity(layer_to_invalidate_->opacity() != 1.f ? 1.f : 0.5f);
    }
...
public:
    ...
    void UpdateLayerTreeHost() override
    {
        ...
        SetScrollOffset(scrollable_.get(), CurrentScrollOffset(scrollable_.get()) + delta);
    }
...
// Timed out on Android: http://crbug.com/723821
#if BUILDFLAG(IS_ANDROID)
#define MAYBE_LongScrollablePageSingleThread DISABLED_LongScrollablePageSingleThread
#else
...
// Simulates main-thread scrolling on each frame.
class BrowserCompositorInvalidateLayerTreePerfTest : public LayerTreeHostPerfTestJsonReader {
public:
    ...
    {
        LayerTreeHostPerfTestJsonReader::BuildTree();
        tab_contents_ = static_cast<TextureLayer*>(layer_tree_host()->root_layer()->children()[0]->children()[0]->children()[0]->children()[0].get());
    ...
    void WillCommit(const CommitState&) override
    {
        ...
        gpu_mailbox.SetName(reinterpret_cast<const int8_t*>(name_stream.str().c_str()));
        auto callback = base::BindOnce(&BrowserCompositorInvalidateLayerTreePerfTest::ReleaseMailbox, base::Unretained(this));

        gpu::SyncToken next_sync_token(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(1), next_fence_sync_);
        next_sync_token.SetVerifyFlush();

        constexpr gfx::Size size(64, 64);
        viz::TransferableResource resource = viz::TransferableResource::MakeGpu(
            gpu_mailbox, GL_TEXTURE_2D, next_sync_token, size, viz::SinglePlaneFormat::kRGBA_8888, false /* is_overlay_candidate */);
        next_fence_sync_++;
    ...
    void CleanUpAndEndTest() override
    {
        ...
        MainThreadTaskRunner()->PostTask(
            FROM_HERE, base::BindOnce(&BrowserCompositorInvalidateLayerTreePerfTest::CleanUpAndEndTestOnMainThread, base::Unretained(this)));
    }
