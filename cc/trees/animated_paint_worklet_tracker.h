...
public:
    ...
    AnimatedPaintWorkletTracker& operator=(const AnimatedPaintWorkletTracker&) = delete;
    ~AnimatedPaintWorkletTracker();
    ...
    void UpdatePaintWorkletInputProperties(const std::vector<DiscardableImageMap::PaintWorkletInputWithImageId>& inputs, PictureLayerImpl* layer);
    ...
    void OnCustomPropertyMutated(PaintWorkletInput::PropertyKey property_key, PaintWorkletInput::PropertyValue property_value);
    ...
    PaintWorkletInput::PropertyValue GetPropertyAnimationValue(const PaintWorkletInput::PropertyKey& key) const;
...
private:
    ...
    struct PropertyState {
        PropertyState();
        explicit PropertyState(PaintWorkletInput::PropertyValue value, base::flat_set<raw_ptr<PictureLayerImpl, CtnExperimental>> layers);
        ...
        base::flat_set<raw_ptr<PictureLayerImpl, CtnExperimental>> associated_layers;
    };

    // The set of input properties managed by AnimatedPaintWorkletTracker.
    base::flat_map<PaintWorkletInput::PropertyKey, PropertyState> input_properties_;
    ...
    // been invalidated.
    base::flat_set<PaintWorkletInput::PropertyKey> input_properties_animated_on_impl_;
};
