...
ProxyMain::ProxyMain(LayerTreeHost* layer_tree_host, TaskRunnerProvider* task_runner_provider)
...
void ProxyMain::InitializeOnImplThread(
    CompletionEvent* completion_event, int id, const LayerTreeSettings* settings, RenderingStatsInstrumentation* rendering_stats_instrumentation)
{
    DCHECK(task_runner_provider_->IsImplThread());
    DCHECK(!proxy_impl_);
    proxy_impl_
        = std::make_unique<ProxyImpl>(weak_factory_.GetWeakPtr(), layer_tree_host_, id, settings, rendering_stats_instrumentation, task_runner_provider_);
    completion_event->Signal();
}

void ProxyMain::DestroyProxyImplOnImplThread(CompletionEvent* completion_event)
{
    DCHECK(task_runner_provider_->IsImplThread());
    proxy_impl_.reset();
    completion_event->Signal();
...
void ProxyMain::BeginMainFrame(std::unique_ptr<BeginMainFrameAndCommitState> begin_main_frame_state)
{
    DCHECK(IsMainThread());
    ...
    const viz::BeginFrameArgs& frame_args = begin_main_frame_state->begin_frame_args;
    TRACE_EVENT("cc,benchmark", "ProxyMain::BeginMainFrame",
        // "begin_frame_id" is used by the rendering benchmarks in
        // Telemetry.
        "begin_frame_id", frame_args.frame_id.sequence_number, [&](perfetto::EventContext ctx) {
            EmitMainFramePipelineStep(ctx, begin_main_frame_state->trace_id, perfetto::protos::pbzero::MainFramePipeline::Step::BEGIN_MAIN_FRAME);
        });
    ...
    layer_tree_host_->ImageDecodesFinished(std::move(begin_main_frame_state->completed_image_decode_requests));
    ...
    if (!layer_tree_host_->IsVisible()) {
        TRACE_EVENT("cc,benchmark", "MainFrameAborted", [&](perfetto::EventContext ctx) {
            auto* pipeline
                = EmitMainFramePipelineStep(ctx, begin_main_frame_state->trace_id, perfetto::protos::pbzero::MainFramePipeline::Step::ABORTED_ON_MAIN);
            pipeline->set_aborted_on_main_reason(perfetto::protos::pbzero::MainFramePipeline::AbortedOnMainReason::NOT_VISIBLE);
        });
        ...
        ImplThreadTaskRunner()->PostTask(FROM_HERE,
            base::BindOnce(&ProxyImpl::BeginMainFrameAbortedOnImpl, base::Unretained(proxy_impl_.get()), CommitEarlyOutReason::kAbortedNotVisible,
                begin_main_frame_start_time, std::move(empty_swap_promises), false /* scroll_and_viewport_changes_synced */));
        layer_tree_host_->GetSwapPromiseManager()->BreakSwapPromises(SwapPromise::COMMIT_FAILS);
        return;
    }
    ...
    if (defer_main_frame_update_ || pause_rendering_) {
        TRACE_EVENT("cc,benchmark", "MainFrameAborted", [&](perfetto::EventContext ctx) {
            auto* pipeline
                = EmitMainFramePipelineStep(ctx, begin_main_frame_state->trace_id, perfetto::protos::pbzero::MainFramePipeline::Step::ABORTED_ON_MAIN);
            pipeline->set_aborted_on_main_reason(perfetto::protos::pbzero::MainFramePipeline::AbortedOnMainReason::DEFERRED_UPDATE);
        });
        ...
        std::vector<std::unique_ptr<SwapPromise>> empty_swap_promises;
        ImplThreadTaskRunner()->PostTask(FROM_HERE,
            base::BindOnce(&ProxyImpl::BeginMainFrameAbortedOnImpl, base::Unretained(proxy_impl_.get()), CommitEarlyOutReason::kAbortedDeferredMainFrameUpdate,
                begin_main_frame_start_time, std::move(empty_swap_promises), false /* scroll_and_viewport_changes_synced */));
        // When we stop deferring main frame updates, we should resume any
        // previously requested pipeline stages.
        deferred_final_pipeline_stage_ = std::max(final_pipeline_stage_, deferred_final_pipeline_stage_);
        layer_tree_host_->GetSwapPromiseManager()->BreakSwapPromises(SwapPromise::COMMIT_FAILS);
        return;
    }
    ...
    final_pipeline_stage_ = std::max(final_pipeline_stage_, deferred_final_pipeline_stage_);
    deferred_final_pipeline_stage_ = NO_PIPELINE_STAGE;
    ...
    if (!IsDeferringCommits()) {
        ...
        layer_tree_host_->ApplyCompositorChanges(begin_main_frame_state->commit_data.get());
        scroll_and_viewport_changes_synced = true;
    }

    layer_tree_host_->ApplyMutatorEvents(std::move(begin_main_frame_state->mutator_events));
    ...
    if (skip_commit) {
        ...
        TRACE_EVENT("cc,benchmark", "MainFrameAborted", [&](perfetto::EventContext ctx) {
            auto* pipeline
                = EmitMainFramePipelineStep(ctx, begin_main_frame_state->trace_id, perfetto::protos::pbzero::MainFramePipeline::Step::ABORTED_ON_MAIN);
            pipeline->set_aborted_on_main_reason(perfetto::protos::pbzero::MainFramePipeline::AbortedOnMainReason::DEFERRED_COMMIT_ABORTED);
        });
        layer_tree_host_->RecordEndOfFrameMetrics(begin_main_frame_start_time, begin_main_frame_state->active_sequence_trackers);
        ...
        std::vector<std::unique_ptr<SwapPromise>> empty_swap_promises;
        ImplThreadTaskRunner()->PostTask(FROM_HERE,
            base::BindOnce(&ProxyImpl::BeginMainFrameAbortedOnImpl, base::Unretained(proxy_impl_.get()), CommitEarlyOutReason::kAbortedDeferredCommit,
                begin_main_frame_start_time, std::move(empty_swap_promises), scroll_and_viewport_changes_synced));
    ...
    devtools_instrumentation::ScopedCommitTrace commit_trace(layer_tree_host_->GetId(), frame_args.frame_id.sequence_number);
    ...
    auto completion_event_ptr = std::make_unique<CompletionEvent>(base::WaitableEvent::ResetPolicy::MANUAL);
    ...
    std::unique_ptr<CommitState> commit_state = layer_tree_host_->WillCommit(std::move(completion_event_ptr), has_updates);
    ...
    if (!has_updates) {
        ...
        current_pipeline_stage_ = NO_PIPELINE_STAGE;
        layer_tree_host_->DidBeginMainFrame();
        TRACE_EVENT_INSTANT0("cc,raf_investigation", "EarlyOut_NoUpdates", TRACE_EVENT_SCOPE_THREAD);
        TRACE_EVENT("cc,benchmark", "MainFrameAborted", [&](perfetto::EventContext ctx) {
            auto* pipeline
                = EmitMainFramePipelineStep(ctx, begin_main_frame_state->trace_id, perfetto::protos::pbzero::MainFramePipeline::Step::ABORTED_ON_MAIN);
            pipeline->set_aborted_on_main_reason(perfetto::protos::pbzero::MainFramePipeline::AbortedOnMainReason::NO_UPDATE);
        });
        ...
        // viewport sync.
        ImplThreadTaskRunner()->PostTask(FROM_HERE,
            base::BindOnce(&ProxyImpl::BeginMainFrameAbortedOnImpl, base::Unretained(proxy_impl_.get()), CommitEarlyOutReason::kFinishedNoUpdates,
                begin_main_frame_start_time, std::move(swap_promises), true /* scroll_and_viewport_changes_synced */));

        // Although the commit is internally aborted, this is because it has been
        // detected to be a no-op.  From the perspective of an embedder, this commit
        // went through, and input should no longer be throttled, etc.
        layer_tree_host_->CommitComplete(layer_tree_host_->SourceFrameNumber(), { base::TimeTicks(), base::TimeTicks::Now() });
        layer_tree_host_->RecordEndOfFrameMetrics(begin_main_frame_start_time, begin_main_frame_state->active_sequence_trackers);
        return;
    }

    if (synchronous_composite_for_test_callback_) {
        commit_state->pending_presentation_callbacks.push_back(
            base::BindOnce([](base::OnceClosure callback, const gfx::PresentationFeedback&) { std::move(callback).Run(); },
    ...
    {
        TRACE_EVENT("cc,benchmark", "ProxyMain::BeginMainFrame::commit", [&](perfetto::EventContext ctx) {
            EmitMainFramePipelineStep(ctx, begin_main_frame_state->trace_id, perfetto::protos::pbzero::MainFramePipeline::Step::COMMIT_ON_MAIN);
        });
        ...
        ImplThreadTaskRunner()->PostTask(FROM_HERE,
            base::BindOnce(&ProxyImpl::NotifyReadyToCommitOnImpl, base::Unretained(proxy_impl_.get()), completion_event, std::move(commit_state), &unsafe_state,
                begin_main_frame_start_time, frame_args, scroll_and_viewport_changes_synced, blocking ? &commit_timestamps : nullptr, commit_timeout));
        if (blocking)
...
void ProxyMain::DidCompleteCommit(int source_frame_number, CommitTimestamps commit_timestamps)
{
    if (layer_tree_host_)
        layer_tree_host_->CommitComplete(source_frame_number, commit_timestamps);
}

void ProxyMain::DidPresentCompositorFrame(uint32_t frame_token, std::vector<PresentationTimeCallbackBuffer::Callback> presentation_callbacks,
    std::vector<PresentationTimeCallbackBuffer::SuccessfulCallbackWithDetails> sucessful_presentation_callbacks,
    const viz::FrameTimingDetails& frame_timing_details)
{
    layer_tree_host_->DidPresentCompositorFrame(
        frame_token, std::move(presentation_callbacks), std::move(sucessful_presentation_callbacks), frame_timing_details);
}
...
void ProxyMain::DidObserveFirstScrollDelay(int source_frame_number, base::TimeDelta first_scroll_delay, base::TimeTicks first_scroll_timestamp)
{
    layer_tree_host_->DidObserveFirstScrollDelay(source_frame_number, first_scroll_delay, first_scroll_timestamp);
}

void ProxyMain::NotifyImageDecodeRequestFinished(int request_id, bool decode_succeeded)
{
    DCHECK(base::FeatureList::IsEnabled(features::kSendExplicitDecodeRequestsImmediately));
    layer_tree_host_->NotifyImageDecodeFinished(request_id, decode_succeeded);
}

void ProxyMain::NotifyTransitionRequestFinished(uint32_t sequence_id, const viz::ViewTransitionElementResourceRects& rects)
{
...
void ProxyMain::SetLayerTreeFrameSink(LayerTreeFrameSink* layer_tree_frame_sink)
{
    ImplThreadTaskRunner()->PostTask(FROM_HERE,
        base::BindOnce(&ProxyImpl::InitializeLayerTreeFrameSinkOnImpl, base::Unretained(proxy_impl_.get()), layer_tree_frame_sink,
...
void ProxyMain::SetVisible(bool visible)
{
    ...
    ImplThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyImpl::SetVisibleOnImpl, base::Unretained(proxy_impl_.get()), visible));
}

void ProxyMain::SetShouldWarmUp()
{
    TRACE_EVENT0("cc", "ProxyMain::SetShouldWarmUp");
    ImplThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyImpl::SetShouldWarmUpOnImpl, base::Unretained(proxy_impl_.get())));
}
...
void ProxyMain::SetNeedsAnimate()
{
    ...
    if (SendCommitRequestToImplThreadIfNeeded(ANIMATE_PIPELINE_STAGE)) {
        TRACE_EVENT_INSTANT0("cc", "ProxyMain::SetNeedsAnimate", TRACE_EVENT_SCOPE_THREAD);
    }
}
...
void ProxyMain::SetNeedsUpdateLayers()
{
    ...
    if (SendCommitRequestToImplThreadIfNeeded(UPDATE_LAYERS_PIPELINE_STAGE)) {
        TRACE_EVENT_INSTANT0("cc", "ProxyMain::SetNeedsUpdateLayers", TRACE_EVENT_SCOPE_THREAD);
    }
}
...
void ProxyMain::SetNeedsCommit()
{
    ...
    if (SendCommitRequestToImplThreadIfNeeded(COMMIT_PIPELINE_STAGE)) {
        TRACE_EVENT_INSTANT0("cc", "ProxyMain::SetNeedsCommit", TRACE_EVENT_SCOPE_THREAD);
    }
}
...
void ProxyMain::SetNeedsRedraw(const gfx::Rect& damage_rect)
{
    ...
    DCHECK(IsMainThread());
    ImplThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyImpl::SetNeedsRedrawOnImpl, base::Unretained(proxy_impl_.get()), damage_rect));
}

void ProxyMain::SetTargetLocalSurfaceId(const viz::LocalSurfaceId& target_local_surface_id)
{
    DCHECK(IsMainThread());
    ImplThreadTaskRunner()->PostTask(
        FROM_HERE, base::BindOnce(&ProxyImpl::SetTargetLocalSurfaceIdOnImpl, base::Unretained(proxy_impl_.get()), target_local_surface_id));
}

void ProxyMain::DetachInputDelegateAndRenderFrameObserver()
{
    DCHECK(IsMainThread());
    auto completion_event = std::make_unique<CompletionEvent>(base::WaitableEvent::ResetPolicy::MANUAL);
    ImplThreadTaskRunner()->PostTask(
        FROM_HERE, base::BindOnce(&ProxyImpl::DetachInputDelegateAndRenderFrameObserver, base::Unretained(proxy_impl_.get()), completion_event.get()));
...
void ProxyMain::SetDeferMainFrameUpdate(bool defer_main_frame_update)
{
    ...
    if (defer_main_frame_update_) {
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("cc", "ProxyMain::SetDeferMainFrameUpdate", TRACE_ID_LOCAL(this));
    } else {
        TRACE_EVENT_NESTABLE_ASYNC_END0("cc", "ProxyMain::SetDeferMainFrameUpdate", TRACE_ID_LOCAL(this));
    }
    ...
    ImplThreadTaskRunner()->PostTask(
        FROM_HERE, base::BindOnce(&ProxyImpl::SetDeferBeginMainFrameFromMain, base::Unretained(proxy_impl_.get()), defer_main_frame_update));
}
...
void ProxyMain::SetPauseRendering(bool pause_rendering)
{
    ...
    if (pause_rendering_) {
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("cc", "ProxyMain::SetPauseRendering", TRACE_ID_LOCAL(this));
    } else {
        TRACE_EVENT_NESTABLE_ASYNC_END0("cc", "ProxyMain::SetPauseRendering", TRACE_ID_LOCAL(this));
    }

    // The impl thread needs to know that it should not issue BeginFrames.
    ImplThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyImpl::SetPauseRendering, base::Unretained(proxy_impl_.get()), pause_rendering_));
}
...
bool ProxyMain::StartDeferringCommits(base::TimeDelta timeout, PaintHoldingReason reason)
{
    ...
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("cc", "ProxyMain::SetDeferCommits", TRACE_ID_LOCAL(this));
...
void ProxyMain::StopDeferringCommits(PaintHoldingCommitTrigger trigger)
{
    ...
    TRACE_EVENT_NESTABLE_ASYNC_END0("cc", "ProxyMain::SetDeferCommits", TRACE_ID_LOCAL(this));
...
void ProxyMain::Start()
{
...

    {
        ...
        ImplThreadTaskRunner()->PostTask(FROM_HERE,
            base::BindOnce(&ProxyMain::InitializeOnImplThread, base::Unretained(this), &completion, layer_tree_host_->GetId(), &layer_tree_host_->GetSettings(),
...
void ProxyMain::Stop()
{
    ...
    // renderer before shutting it down.
    {
        DebugScopedSetMainThreadBlocked main_thread_blocked(task_runner_provider_);
        CompletionEvent completion;
        ImplThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyImpl::FinishGLOnImpl, base::Unretained(proxy_impl_.get()), &completion));
        completion.Wait();
    }
    {
        DebugScopedSetMainThreadBlocked main_thread_blocked(task_runner_provider_);
        CompletionEvent completion;
        ImplThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyMain::DestroyProxyImplOnImplThread, base::Unretained(this), &completion));
        completion.Wait();
    }
...
void ProxyMain::QueueImageDecode(int request_id, const PaintImage& image)
{
    ...
    ImplThreadTaskRunner()->PostTask(
        FROM_HERE, base::BindOnce(&ProxyImpl::QueueImageDecodeOnImpl, base::Unretained(proxy_impl_.get()), request_id, std::make_unique<PaintImage>(image)));
}

void ProxyMain::SetMutator(std::unique_ptr<LayerTreeMutator> mutator)
{
    TRACE_EVENT0("cc", "ProxyMain::SetMutator");
    ImplThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyImpl::InitializeMutatorOnImpl, base::Unretained(proxy_impl_.get()), std::move(mutator)));
}

void ProxyMain::SetPaintWorkletLayerPainter(std::unique_ptr<PaintWorkletLayerPainter> painter)
{
    TRACE_EVENT0("cc", "ProxyMain::SetPaintWorkletLayerPainter");
    ImplThreadTaskRunner()->PostTask(
        FROM_HERE, base::BindOnce(&ProxyImpl::InitializePaintWorkletLayerPainterOnImpl, base::Unretained(proxy_impl_.get()), std::move(painter)));
}
...
bool ProxyMain::MainFrameWillHappenForTesting()
{
    ...
    CompletionEvent completion;
    ImplThreadTaskRunner()->PostTask(
        FROM_HERE, base::BindOnce(&ProxyImpl::MainFrameWillHappenOnImplForTesting, base::Unretained(proxy_impl_.get()), &completion, &main_frame_will_happen));
    completion.Wait();
...
void ProxyMain::ReleaseLayerTreeFrameSink()
{
    ...
    base::ScopedUmaHistogramTimer histogram_timer("Navigation.ProxyMain.ReleaseLayerTreeFrameSink");
    DCHECK(IsMainThread());
    frame_sink_bound_weak_factory_.InvalidateWeakPtrs();
    DebugScopedSetMainThreadBlocked main_thread_blocked(task_runner_provider_);
    CompletionEvent completion;
    ImplThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyImpl::ReleaseLayerTreeFrameSinkOnImpl, base::Unretained(proxy_impl_.get()), &completion));
    completion.Wait();
}
    ...
    BrowserControlsState constraints, BrowserControlsState current, bool animate, base::optional_ref<const BrowserControlsOffsetTagsInfo> offset_tags_info)
{
    DCHECK(IsMainThread());
    ImplThreadTaskRunner()->PostTask(FROM_HERE,
        base::BindOnce(&ProxyImpl::UpdateBrowserControlsStateOnImpl, base::Unretained(proxy_impl_.get()), constraints, current, animate, offset_tags_info));
}
...
void ProxyMain::RequestBeginMainFrameNotExpected(bool new_state)
{
    DCHECK(IsMainThread());
    ImplThreadTaskRunner()->PostTask(
        FROM_HERE, base::BindOnce(&ProxyImpl::RequestBeginMainFrameNotExpectedOnImpl, base::Unretained(proxy_impl_.get()), new_state));
}

bool ProxyMain::SendCommitRequestToImplThreadIfNeeded(CommitPipelineStage required_stage)
{
    DCHECK(IsMainThread());
    ...
    if (already_posted)
        return false;
    ImplThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyImpl::SetNeedsCommitOnImpl, base::Unretained(proxy_impl_.get())));
...
{
    DCHECK(IsMainThread());
    ImplThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyImpl::SetSourceURL, base::Unretained(proxy_impl_.get()), source_id, url));
}

void ProxyMain::SetUkmSmoothnessDestination(base::WritableSharedMemoryMapping ukm_smoothness_data)
{
    DCHECK(IsMainThread());
    ImplThreadTaskRunner()->PostTask(
        FROM_HERE, base::BindOnce(&ProxyImpl::SetUkmSmoothnessDestination, base::Unretained(proxy_impl_.get()), std::move(ukm_smoothness_data)));
}

void ProxyMain::SetRenderFrameObserver(std::unique_ptr<RenderFrameMetadataObserver> observer)
{
    ImplThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyImpl::SetRenderFrameObserver, base::Unretained(proxy_impl_.get()), std::move(observer)));
}

void ProxyMain::CompositeImmediatelyForTest(base::TimeTicks frame_begin_time, bool raster, base::OnceClosure callback)
{
