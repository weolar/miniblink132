...
public:
    ...
    void SetupTree() override
    {
        ...
        PaintImage checkerable_image = CreateDiscardablePaintImage(gfx::Size(450, 450));
        checkerable_image = PaintImageBuilder::WithCopy(checkerable_image).set_decoding_mode(PaintImage::DecodingMode::kAsync).TakePaintImage();
        content_layer_client_.add_draw_image(checkerable_image, gfx::Point(0, 0), SkSamplingOptions(), PaintFlags());
...
class LayerTreeHostCheckerImagingTestMergeWithMainFrame : public LayerTreeHostCheckerImagingTest {
    void BeginMainFrame(const viz::BeginFrameArgs& args) override
    {
        if (layer_tree_host()->SourceFrameNumber() == 1) {
            ...
            layer_tree_host()->root_layer()->SetNeedsDisplayRect(gfx::Rect(600, 0, 50, 500));
            layer_tree_host()->SetNeedsCommit();
        }
    }

    void DidReceiveImplSideInvalidationRequest(LayerTreeHostImpl* host_impl) override
    {
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        case 2: {
            ...
            PictureLayerImpl* sync_layer_impl = static_cast<PictureLayerImpl*>(host_impl->sync_tree()->root_layer());
            PictureLayerTiling* sync_tiling = sync_layer_impl->picture_layer_tiling_set()->FindTilingWithResolution(TileResolution::HIGH_RESOLUTION);

            for (int i = 0; i < 4; i++) {
                SCOPED_TRACE(i);
                for (int j = 0; j < 2; j++) {
                    SCOPED_TRACE(j);
                    Tile* tile = sync_tiling->TileAt(i, j) ? sync_tiling->TileAt(i, j) : nullptr;
...
class LayerTreeHostCheckerImagingTestImplSideTree : public LayerTreeHostCheckerImagingTest {
