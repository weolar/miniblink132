...
TEST(PropertyTreeTest, UndoOverscroll)
{
    ...
    EXPECT_EQ(clip_tree.Node(viewport_property_ids.outer_clip)->clip, expected_clip_rect);
}
...
TEST(PropertyTreeTest, TransformsWithFlattening)
{
    ...
    effect_tree.Node(effect_grand_parent)->render_surface_reason = RenderSurfaceReason::kTest;
    ...
    effect_tree.Node(effect_parent)->render_surface_reason = RenderSurfaceReason::kTest;
    ...
    EXPECT_TRANSFORM_EQ(flattened_rotation_about_x * rotation_about_x, tree.ToScreen(child));
    ...
    EXPECT_TRANSFORM_EQ(flattened_rotation_about_x * flattened_rotation_about_x * rotation_about_x, tree.ToScreen(grand_child));
    ...
    EXPECT_TRANSFORM_EQ(flattened_rotation_about_x * rotation_about_x * rotation_about_x, tree.ToScreen(grand_child));
...
TEST(EffectTreeTest, CopyOutputRequestsAreTransformed)
{
    ...
    EXPECT_NEAR(0.5f, static_cast<float>(request_out->scale_to().x()) / request_out->scale_from().x(), 0.000001);
    EXPECT_NEAR(0.5f, static_cast<float>(request_out->scale_to().y()) / request_out->scale_from().y(), 0.000001);
    ...
    EXPECT_EQ(gfx::Rect(1, 2, 3, 4), request_out->result_selection());
    ASSERT_TRUE(request_out->is_scaled());
    EXPECT_NEAR(3.0f / 2.0f, static_cast<float>(request_out->scale_to().x()) / request_out->scale_from().x(), 0.000001);
    EXPECT_NEAR(5.0f / 2.0f, static_cast<float>(request_out->scale_to().y()) / request_out->scale_from().y(), 0.000001);
}
...
TEST(ScrollTreeTest, GetScrollOffsetForScrollTimelineNegativeOffset)
{
    ...
    gfx::PointF offset = scroll_tree.GetScrollOffsetForScrollTimeline(*scroll_tree.Node(scroll_node_id));
...
TEST(ScrollTreeTest, PushScrollUpdatesFromMainThreadIntegerDelta)
{
    ...
    FakeLayerTreeHostImpl host_impl(CommitToPendingTreeLayerTreeSettings(), &impl_task_runner_provider, &task_graph_runner);
    host_impl.CreatePendingTree();

    // Set up pending property trees.
    PropertyTrees* pending_property_trees = host_impl.pending_tree()->property_trees();
    EXPECT_TRUE(pending_property_trees);
    ScrollTree& pending_scroll_tree = pending_property_trees->scroll_tree_mutable();
    TransformTree& pending_transform_tree = pending_property_trees->transform_tree_mutable();
    ...
    pending_property_trees->scroll_tree_mutable().SetElementIdForNodeId(scroll_node_id, element_id);

    // Push main scroll to pending.
    main_scroll_tree.SetScrollOffset(element_id, gfx::PointF(0, 1));
    pending_scroll_tree.PushScrollUpdatesFromMainThread(property_trees, host_impl.pending_tree(), use_fractional_deltas);
    const SyncedScrollOffset* scroll_offset = pending_scroll_tree.GetSyncedScrollOffset(element_id);
    EXPECT_TRUE(scroll_offset);

    // Set a fractional delta and check it is not pulled with fractional delta
    // turned off.
    pending_scroll_tree.SetScrollOffsetDeltaForTesting(element_id, gfx::Vector2dF(0, 0.25));
    main_scroll_tree.CollectScrollDeltasForTesting(use_fractional_deltas);
    EXPECT_EQ(gfx::PointF(0, 1), main_scroll_tree.current_scroll_offset(element_id));
    ...
    host_impl.pending_tree()->property_trees()->set_changed(false);
    pending_scroll_tree.PushScrollUpdatesFromMainThread(property_trees, host_impl.pending_tree(), true);
