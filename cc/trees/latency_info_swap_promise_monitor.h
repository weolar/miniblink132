...
public:
    ...
    LatencyInfoSwapPromiseMonitor(ui::LatencyInfo* latency, SwapPromiseManager* swap_promise_manager);

    // Constructor for when the monitor lives on the compositor thread.
    LatencyInfoSwapPromiseMonitor(ui::LatencyInfo* latency, LayerTreeHostImpl* host_impl);

    LatencyInfoSwapPromiseMonitor(const LatencyInfoSwapPromiseMonitor&) = delete;
    LatencyInfoSwapPromiseMonitor& operator=(const LatencyInfoSwapPromiseMonitor&) = delete;
