...
// Measured in seconds.
constexpr auto kSmoothnessTakesPriorityExpirationDelay = base::Milliseconds(250);
...
public:
    ScopedCommitCompletionEvent(int source_frame_number, CompletionEvent* event, base::TimeTicks start_time,
        base::SingleThreadTaskRunner* main_thread_task_runner, bool notify_main, base::WeakPtr<ProxyMain> proxy_main_weak_ptr)
    ...
    ~ScopedCommitCompletionEvent()
    {
        ...
        if (notify_main_) {
            main_thread_task_runner_->PostTask(
                FROM_HERE, base::BindOnce(&ProxyMain::DidCompleteCommit, proxy_main_weak_ptr_, source_frame_number_, commit_timestamps_));
        }
    }
    ScopedCommitCompletionEvent& operator=(const ScopedCommitCompletionEvent&) = delete;
...
ProxyImpl::ProxyImpl(base::WeakPtr<ProxyMain> proxy_main_weak_ptr, LayerTreeHost* layer_tree_host, int id, const LayerTreeSettings* settings,
    RenderingStatsInstrumentation* rendering_stats_instrumentation, TaskRunnerProvider* task_runner_provider)
    : layer_tree_host_id_(id)
    , next_frame_is_newly_committed_frame_(false)
    , inside_draw_(false)
    , task_runner_provider_(task_runner_provider)
    , smoothness_priority_expiration_notifier_(task_runner_provider->ImplThreadTaskRunner(),
          base::BindRepeating(&ProxyImpl::RenewTreePriority, base::Unretained(this)), kSmoothnessTakesPriorityExpirationDelay)
    , proxy_main_weak_ptr_(proxy_main_weak_ptr)
{
    ...
    std::unique_ptr<CompositorTimingHistory> compositor_timing_history(
        new CompositorTimingHistory(CompositorTimingHistory::RENDERER_UMA, rendering_stats_instrumentation));
    scheduler_ = std::make_unique<Scheduler>(this, scheduler_settings, layer_tree_host_id_, task_runner_provider_->ImplThreadTaskRunner(),
        std::move(compositor_timing_history), host_impl_->compositor_frame_reporting_controller());
...
void ProxyImpl::InitializeMutatorOnImpl(std::unique_ptr<LayerTreeMutator> mutator)
{
...
void ProxyImpl::InitializePaintWorkletLayerPainterOnImpl(std::unique_ptr<PaintWorkletLayerPainter> painter)
{
...
{
    DCHECK(IsImplThread());
    host_impl_->browser_controls_manager()->UpdateBrowserControlsState(constraints, current, animate, offset_tags_info);
}

void ProxyImpl::InitializeLayerTreeFrameSinkOnImpl(LayerTreeFrameSink* layer_tree_frame_sink, base::WeakPtr<ProxyMain> proxy_main_frame_sink_bound_weak_ptr)
{
    ...
    MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyMain::DidInitializeLayerTreeFrameSink, proxy_main_weak_ptr_, success));
    if (success)
...
void ProxyImpl::SetTargetLocalSurfaceIdOnImpl(const viz::LocalSurfaceId& target_local_surface_id)
{
    DCHECK(IsImplThread());
    host_impl_->SetTargetLocalSurfaceId(target_local_surface_id);
}

void ProxyImpl::BeginMainFrameAbortedOnImpl(CommitEarlyOutReason reason, base::TimeTicks main_thread_start_time,
    std::vector<std::unique_ptr<SwapPromise>> swap_promises, bool scroll_and_viewport_changes_synced)
{
    TRACE_EVENT1("cc", "ProxyImpl::BeginMainFrameAbortedOnImplThread", "reason", CommitEarlyOutReasonToString(reason));
    DCHECK(IsImplThread());
    DCHECK(scheduler_->CommitPending());

    host_impl_->BeginMainFrameAborted(reason, std::move(swap_promises), scheduler_->last_dispatched_begin_main_frame_args(),
        static_cast<bool>(data_for_commit_.get()), scroll_and_viewport_changes_synced);
...
void ProxyImpl::ReleaseLayerTreeFrameSinkOnImpl(CompletionEvent* completion)
{
    ...
    base::ScopedUmaHistogramTimer histogram_timer("Navigation.ProxyImpl.ReleaseLayerTreeFrameSinkOnImpl");
    DCHECK(IsImplThread());
...
void ProxyImpl::FinishGLOnImpl(CompletionEvent* completion)
{
    ...
    if (host_impl_->layer_tree_frame_sink()) {
        auto* context_provider = host_impl_->layer_tree_frame_sink()->context_provider();
        if (context_provider) {
...
void ProxyImpl::MainFrameWillHappenOnImplForTesting(CompletionEvent* completion, bool* main_frame_will_happen)
{
    DCHECK(IsImplThread());
    if (host_impl_->layer_tree_frame_sink()) {
...
void ProxyImpl::RequestBeginMainFrameNotExpectedOnImpl(bool new_state)
{
    DCHECK(IsImplThread());
    DCHECK(scheduler_);
    TRACE_EVENT1("cc", "ProxyImpl::RequestBeginMainFrameNotExpectedOnImpl", "new_state", new_state);
...
void ProxyImpl::FrameSinksToThrottleUpdated(const base::flat_set<viz::FrameSinkId>& ids)
{
    NOTREACHED();
}
...
void ProxyImpl::NotifyReadyToCommitOnImpl(CompletionEvent* completion_event, std::unique_ptr<CommitState> commit_state,
    const ThreadUnsafeCommitState* unsafe_state, base::TimeTicks main_thread_start_time, const viz::BeginFrameArgs& commit_args,
    bool scroll_and_viewport_changes_synced, CommitTimestamps* commit_timestamps, bool commit_timeout)
{
    TRACE_EVENT("cc,benchmark", "ProxyImpl::ReadyToCommit", [&](perfetto::EventContext ctx) {
        EmitMainFramePipelineStep(ctx, commit_state->trace_id, perfetto::protos::pbzero::MainFramePipeline::Step::READY_TO_COMMIT_ON_IMPL);
    });
    ...
    DCHECK_EQ((bool)commit_timestamps, task_runner_provider_->IsMainThreadBlocked());
    base::TimeTicks start_time = base::TimeTicks::Now();
    if (commit_timestamps)
        commit_timestamps->start = start_time;

    if (!host_impl_) {
        TRACE_EVENT_INSTANT0("cc", "EarlyOut_NoLayerTree", TRACE_EVENT_SCOPE_THREAD);
    ...
    host_impl_->ReadyToCommit(commit_args, scroll_and_viewport_changes_synced, begin_main_frame_metrics.get(), commit_timeout);

    int source_frame_number = commit_state->source_frame_number;
    data_for_commit_ = std::make_unique<DataForCommit>(
        std::make_unique<ScopedCommitCompletionEvent>(source_frame_number, completion_event, start_time, MainThreadTaskRunner(),
...
void ProxyImpl::DidLoseLayerTreeFrameSinkOnImplThread()
{
    ...
    DCHECK(IsImplThread());
    MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyMain::DidLoseLayerTreeFrameSink, proxy_main_weak_ptr_));
    scheduler_->DidLoseLayerTreeFrameSink();
}
...
{
    DCHECK(IsImplThread());
    TRACE_EVENT_INSTANT("cc,benchmark", "ProxyImpl::ReadyToActivate", [&](perfetto::EventContext ctx) {
        EmitMainFramePipelineStep(ctx, host_impl_->active_tree()->trace_id(), perfetto::protos::pbzero::MainFramePipeline::Step::READY_TO_ACTIVATE);
    });
...
{
    TRACE_EVENT1("cc", "ProxyImpl::SetVideoNeedsBeginFrames", "needs_begin_frames", needs_begin_frames);
    DCHECK(IsImplThread());
...
void ProxyImpl::RenewTreePriority()
{
    ...
    bool precise_scrolling_in_progress = host_impl_->GetActivelyScrollingType() == ActivelyScrollingType::kPrecise;

    bool avoid_entering_smoothness
        = (base::FeatureList::IsEnabled(features::kNewContentForCheckerboardedScrolls) && host_impl_->ScrollCheckerboardsIncompleteRecording())
        || (precise_scrolling_in_progress && host_impl_->IsCurrentScrollMainRepainted());

    bool non_scroll_interaction_in_progress = host_impl_->IsPinchGestureActive() || host_impl_->page_scale_animation_active();

    // Schedule expiration if smoothness currently takes priority.
    if ((non_scroll_interaction_in_progress || precise_scrolling_in_progress) && !avoid_entering_smoothness) {
    ...
    if (host_impl_->active_tree()->GetDeviceViewport().size().IsEmpty() || host_impl_->EvictedUIResourcesExist()
        || (host_impl_->IsCurrentScrollMainRepainted() && base::FeatureList::IsEnabled(features::kMainRepaintScrollPrefersNewContent))) {
    ...
    ScrollHandlerState scroll_handler_state = host_impl_->ScrollAffectsScrollHandler() ? ScrollHandlerState::SCROLL_AFFECTS_SCROLL_HANDLER
                                                                                       : ScrollHandlerState::SCROLL_DOES_NOT_AFFECT_SCROLL_HANDLER;
    scheduler_->SetTreePrioritiesAndScrollState(tree_priority, scroll_handler_state);
}

void ProxyImpl::PostDelayedAnimationTaskOnImplThread(base::OnceClosure task, base::TimeDelta delay)
{
    DCHECK(IsImplThread());
    task_runner_provider_->ImplThreadTaskRunner()->PostDelayedTask(FROM_HERE, std::move(task), delay);
}
...
void ProxyImpl::DidActivateSyncTree()
{
    ...
    if (activation_completion_event_) {
        TRACE_EVENT_INSTANT0("cc", "ReleaseCommitbyActivation", TRACE_EVENT_SCOPE_THREAD);
...
{
    DCHECK(IsImplThread());
    MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyMain::DidCompletePageScaleAnimation, proxy_main_weak_ptr_));
}

void ProxyImpl::OnDrawForLayerTreeFrameSink(bool resourceless_software_draw, bool skip_draw)
{
    DCHECK(IsImplThread());
    scheduler_->OnDrawForLayerTreeFrameSink(resourceless_software_draw, skip_draw);
}

void ProxyImpl::SetNeedsImplSideInvalidation(bool needs_first_draw_on_activation)
{
    DCHECK(IsImplThread());
    scheduler_->SetNeedsImplSideInvalidation(needs_first_draw_on_activation);
}

void ProxyImpl::NotifyImageDecodeRequestFinished(int request_id, bool decode_succeeded)
{
    DCHECK(IsImplThread());
    if (base::FeatureList::IsEnabled(features::kSendExplicitDecodeRequestsImmediately)) {
        MainThreadTaskRunner()->PostTask(
            FROM_HERE, base::BindOnce(&ProxyMain::NotifyImageDecodeRequestFinished, proxy_main_weak_ptr_, request_id, decode_succeeded));
    } else {
        SetNeedsCommitOnImplThread();
    }
}

void ProxyImpl::NotifyTransitionRequestFinished(uint32_t sequence_id, const viz::ViewTransitionElementResourceRects& rects)
{
    DCHECK(IsImplThread());
    MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyMain::NotifyTransitionRequestFinished, proxy_main_weak_ptr_, sequence_id, rects));
}

void ProxyImpl::DidPresentCompositorFrameOnImplThread(
    uint32_t frame_token, PresentationTimeCallbackBuffer::PendingCallbacks activated, const viz::FrameTimingDetails& details)
{
    ...
    MainThreadTaskRunner()->PostTask(FROM_HERE,
        base::BindOnce(&ProxyMain::DidPresentCompositorFrame, proxy_main_weak_ptr_, frame_token, std::move(activated.main_callbacks),
            std::move(activated.main_successful_callbacks), details));
    if (scheduler_)
        scheduler_->DidPresentCompositorFrame(frame_token, details);
}

void ProxyImpl::NotifyAnimationWorkletStateChange(AnimationWorkletMutationState state, ElementListType element_list_type)
{
    DCHECK(IsImplThread());
    Scheduler::AnimationWorkletState animation_worklet_state
        = (state == AnimationWorkletMutationState::STARTED) ? Scheduler::AnimationWorkletState::PROCESSING : Scheduler::AnimationWorkletState::IDLE;
    Scheduler::TreeType tree_type = (element_list_type == ElementListType::ACTIVE) ? Scheduler::TreeType::ACTIVE : Scheduler::TreeType::PENDING;
    scheduler_->NotifyAnimationWorkletStateChange(animation_worklet_state, tree_type);
}

void ProxyImpl::NotifyPaintWorkletStateChange(Scheduler::PaintWorkletState state)
{
    DCHECK(IsImplThread());
...
{
    DCHECK(IsImplThread());
    MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyMain::NotifyThroughputTrackerResults, proxy_main_weak_ptr_, std::move(results)));
}

void ProxyImpl::DidObserveFirstScrollDelay(int source_frame_number, base::TimeDelta first_scroll_delay, base::TimeTicks first_scroll_timestamp)
{
    DCHECK(IsImplThread());
    MainThreadTaskRunner()->PostTask(FROM_HERE,
        base::BindOnce(&ProxyMain::DidObserveFirstScrollDelay, proxy_main_weak_ptr_, source_frame_number, first_scroll_delay, first_scroll_timestamp));
}
...
void ProxyImpl::DidFinishImplFrame(const viz::BeginFrameArgs& last_activated_args)
{
    DCHECK(IsImplThread());
    host_impl_->DidFinishImplFrame(last_activated_args);
}

void ProxyImpl::DidNotProduceFrame(const viz::BeginFrameAck& ack, FrameSkippedReason reason)
{
    DCHECK(IsImplThread());
...
void ProxyImpl::ScheduledActionSendBeginMainFrame(const viz::BeginFrameArgs& args)
{
    DCHECK(IsImplThread());

    benchmark_instrumentation::ScopedBeginFrameTask begin_frame_task(benchmark_instrumentation::kSendBeginFrame, args.frame_id.sequence_number);
    std::unique_ptr<BeginMainFrameAndCommitState> begin_main_frame_state(new BeginMainFrameAndCommitState);
    begin_main_frame_state->begin_frame_args = args;
    DCHECK(!data_for_commit_ || data_for_commit_->unsafe_state->mutator_host);
    begin_main_frame_state->commit_data = host_impl_->ProcessCompositorDeltas(data_for_commit_ ? data_for_commit_->unsafe_state->mutator_host : nullptr);
    begin_main_frame_state->completed_image_decode_requests = host_impl_->TakeCompletedImageDecodeRequests();
    DCHECK(!base::FeatureList::IsEnabled(features::kSendExplicitDecodeRequestsImmediately) || begin_main_frame_state->completed_image_decode_requests.empty());
    begin_main_frame_state->mutator_events = host_impl_->TakeMutatorEvents();
    begin_main_frame_state->active_sequence_trackers = host_impl_->FrameSequenceTrackerActiveTypes();
    begin_main_frame_state->evicted_ui_resources = host_impl_->EvictedUIResourcesExist();
    host_impl_->WillSendBeginMainFrame();
    {
        TRACE_EVENT_INSTANT("cc,benchmark", "SendBeginMainFrame", [&](perfetto::EventContext ctx) {
            auto* pipeline
                = EmitMainFramePipelineStep(ctx, begin_main_frame_state->trace_id, perfetto::protos::pbzero::MainFramePipeline::Step::SEND_BEGIN_MAIN_FRAME);
    ...
    MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyMain::BeginMainFrame, proxy_main_weak_ptr_, std::move(begin_main_frame_state)));
...
void ProxyImpl::ScheduledActionUpdateDisplayTree()
{
    ...
    TRACE_EVENT("cc,benchmark", "MainFrame.UpdateDisplayTree", [&](perfetto::EventContext ctx) {
        EmitMainFramePipelineStep(ctx, host_impl_->active_tree()->trace_id(), perfetto::protos::pbzero::MainFramePipeline::Step::UPDATE_DISPLAY_TREE);
    });
    ...
    frame.origin_begin_main_frame_args = scheduler_->last_activate_origin_frame_args();
    ...
    if (next_frame_is_newly_committed_frame_) {
        next_frame_is_newly_committed_frame_ = false;
        MainThreadTaskRunner()->PostTask(
            FROM_HERE, base::BindOnce(&ProxyMain::DidCommitAndDrawFrame, proxy_main_weak_ptr_, host_impl_->active_tree()->source_frame_number()));
    }
...
{
    TRACE_EVENT("cc,benchmark", "ProxyImpl::Commit", [&](perfetto::EventContext ctx) {
        EmitMainFramePipelineStep(ctx, data_for_commit_->commit_state->trace_id, perfetto::protos::pbzero::MainFramePipeline::Step::COMMIT_ON_IMPL);
    });
    DCHECK(IsImplThread());
    DCHECK(data_for_commit_.get());
    ...
    host_impl_->BeginCommit(commit_state->source_frame_number, commit_state->trace_id);
    ...
    if (commit_state->commit_waits_for_activation) {
        ...
        activation_completion_event_ = std::move(data_for_commit_->commit_completion_event);
    }
...
{
    if (host_impl_->sync_tree() && host_impl_->sync_tree()->source_frame_number() != host_impl_->active_tree()->source_frame_number()) {
...
void ProxyImpl::ScheduledActionBeginLayerTreeFrameSinkCreation()
{
    ...
    DCHECK(IsImplThread());
    MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyMain::RequestNewLayerTreeFrameSink, proxy_main_weak_ptr_));
}
...
{
    DCHECK(IsImplThread());
    MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyMain::BeginMainFrameNotExpectedSoon, proxy_main_weak_ptr_));
}

void ProxyImpl::ScheduledActionBeginMainFrameNotExpectedUntil(base::TimeTicks time)
{
    DCHECK(IsImplThread());
    MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&ProxyMain::BeginMainFrameNotExpectedUntil, proxy_main_weak_ptr_, time));
}
...
DrawResult ProxyImpl::DrawInternal(bool forced_draw)
{
    ...
    TRACE_EVENT("cc,benchmark", "MainFrame.Draw", [&](perfetto::EventContext ctx) {
        auto* pipeline = EmitMainFramePipelineStep(ctx, host_impl_->active_tree()->trace_id(), perfetto::protos::pbzero::MainFramePipeline::Step::DRAW);
        if (next_frame_is_newly_committed_frame_) {
            viz::BeginFrameId frame_id = host_impl_->CurrentBeginFrameArgs().frame_id;
            auto* first_draw_frame_id = pipeline->set_last_begin_frame_id_during_first_draw();
    ...
    DrawResult result;
    {
        TRACE_EVENT("viz,benchmark,graphics.pipeline", "Graphics.Pipeline", perfetto::Flow::Global(host_impl_->CurrentBeginFrameArgs().trace_id),
            [&](perfetto::EventContext ctx) {
                auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
                auto* data = event->set_chrome_graphics_pipeline();
                data->set_step(perfetto::protos::pbzero::ChromeGraphicsPipeline::StepName::STEP_GENERATE_COMPOSITOR_FRAME);
                data->set_surface_frame_trace_id(host_impl_->CurrentBeginFrameArgs().trace_id);
            });
        ...
        if (draw_frame) {
            if (std::optional<SubmitInfo> submit_info = host_impl_->DrawLayers(&frame)) {
...
void ProxyImpl::QueueImageDecodeOnImpl(int request_id, std::unique_ptr<PaintImage> image)
{
...
void ProxyImpl::SetUkmSmoothnessDestination(base::WritableSharedMemoryMapping ukm_smoothness_data)
{
    DCHECK(IsImplThread());
...
void ProxyImpl::SetRenderFrameObserver(std::unique_ptr<RenderFrameMetadataObserver> observer)
{
    host_impl_->SetRenderFrameObserver(std::move(observer));
}

void ProxyImpl::DetachInputDelegateAndRenderFrameObserver(CompletionEvent* completion_event)
{
    host_impl_->DetachInputDelegateAndRenderFrameObserver();
    completion_event->Signal();
}

ProxyImpl::DataForCommit::DataForCommit(std::unique_ptr<ScopedCommitCompletionEvent> commit_completion_event, std::unique_ptr<CommitState> commit_state,
    const ThreadUnsafeCommitState* unsafe_state, CommitTimestamps* commit_timestamps)
