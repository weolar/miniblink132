...
struct CC_EXPORT WorkletAnimationId {
    ...
    {
        return (this->worklet_id == rhs.worklet_id) && (this->animation_id == rhs.animation_id);
    }
    // Returns true if the WorkletAnimationId has been initialized with a valid
    // id.
    explicit operator bool() const
    {
        return !!worklet_id || !!animation_id;
    }
};
...
struct CC_EXPORT AnimationWorkletInput {
    struct CC_EXPORT AddAndUpdateState {
        ...
        AddAndUpdateState(WorkletAnimationId worklet_animation_id, std::string name, double current_time, std::unique_ptr<AnimationOptions> options,
            std::unique_ptr<AnimationEffectTimings> effect_timings);
...
private:
    using InputMap = std::unordered_map<int, std::unique_ptr<AnimationWorkletInput>>;
...
public:
    ...
    virtual void SetMutationUpdate(std::unique_ptr<MutatorOutputState> output_state) = 0;
};
...
public:
    ...
    virtual bool Mutate(std::unique_ptr<MutatorInputState> input_state, MutateQueuingStrategy queueing_strategy, DoneCallback done_callback) = 0;
