...
public:
    static std::unique_ptr<TestLayerImpl> Create(LayerTreeImpl* tree_impl, int id)
    {
...
class TestViewTransitionContentLayerImpl : public ViewTransitionContentLayerImpl {
public:
    static std::unique_ptr<TestViewTransitionContentLayerImpl> Create(
        LayerTreeImpl* tree_impl, int id, const viz::ViewTransitionElementResourceId& resource_id, bool is_live_content_layer)
    {
        return base::WrapUnique(new TestViewTransitionContentLayerImpl(tree_impl, id, resource_id, is_live_content_layer));
    }
...
private:
    TestViewTransitionContentLayerImpl(LayerTreeImpl* tree_impl, int id, const viz::ViewTransitionElementResourceId& resource_id, bool is_live_content_layer);
...
TestViewTransitionContentLayerImpl::TestViewTransitionContentLayerImpl(
    LayerTreeImpl* tree_impl, int id, const viz::ViewTransitionElementResourceId& resource_id, bool is_live_content_layer)
    : ViewTransitionContentLayerImpl(tree_impl, id, resource_id, is_live_content_layer, gfx::RectF())
{
}

void TestViewTransitionContentLayerImpl::AddDamageRect(const gfx::Rect& damage_rect)
{
    damage_rect_.Union(damage_rect);
}
...
{
    auto* root_node = root->layer_tree_impl()->property_trees()->effect_tree_mutable().Node(root->effect_tree_index());
    root_node->has_copy_request = true;
    root->layer_tree_impl()->property_trees()->effect_tree_mutable().set_needs_update(true);
}
...
public:
    ...
    LayerImpl* CreateTestTreeWithTwoSurfaces()
    {
        ...
        CreateTransformNode(child1_).post_translation = gfx::Vector2dF(100.f, 100.f);
        CreateEffectNode(child1_).render_surface_reason = RenderSurfaceReason::kTest;
    ...
    LayerImpl* CreateTestTreeWithFourSurfaces()
    {
        ...
        CreateEffectNode(grand_child3_).render_surface_reason = RenderSurfaceReason::kTest;
        CreateTransformNode(grand_child3_).post_translation = gfx::Vector2dF(170.f, 170.f);

        grand_child4_->SetBounds(gfx::Size(15, 16));
        grand_child4_->SetDrawsContent(true);
        CopyProperties(child1_, grand_child4_);
        CreateEffectNode(grand_child4_).render_surface_reason = RenderSurfaceReason::kTest;
        CreateTransformNode(grand_child4_).post_translation = gfx::Vector2dF(180.f, 180.f);
    ...
    LayerImpl* CreateAndSetUpTestTreeWithTwoSurfacesDrawingFullyVisible()
    {
        ...
        root->layer_tree_impl()
            ...
            ->backdrop_filters.Append(FilterOperation::CreateZoomFilter(2.f /* zoom */, 0 /* inset */));
...
TEST_F(DamageTrackerTest, SanityCheckTestTreeWithOneSurface)
{
    ...
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->GetDamageRectIfValid(&root_damage_rect));
...
TEST_F(DamageTrackerTest, SanityCheckTestTreeWithTwoSurfaces)
{
    ...
    EXPECT_TRUE(GetRenderSurface(child1_)->damage_tracker()->GetDamageRectIfValid(&child_damage_rect));
    gfx::Rect root_damage_rect;
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->GetDamageRectIfValid(&root_damage_rect));
    ...
    EXPECT_EQ(gfx::Rect(190, 190, 16, 18).ToString(), child_damage_rect.ToString());
    EXPECT_EQ(gfx::Rect(500, 500).ToString(), root_damage_rect.ToString());
...
TEST_F(DamageTrackerTest, VerifyDamageForUpdateRects)
{
    ...
    gfx::Rect root_damage_rect;
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->GetDamageRectIfValid(&root_damage_rect));
    EXPECT_EQ(gfx::Rect(110, 111, 12, 13).ToString(), root_damage_rect.ToString());
...
TEST_F(DamageTrackerTest, VerifyDamageForLayerDamageRects)
{
    ...
    EXPECT_EQ(GetRenderSurface(root)->damage_tracker()->GetDamageReasons(), DamageReasonSet { DamageReason::kAnimatedImage });
    ...
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->GetDamageRectIfValid(&root_damage_rect));
    EXPECT_EQ(true, root_damage_rect.Contains(gfx::Rect(110, 111, 12, 13)));
    EXPECT_EQ(GetRenderSurface(root)->damage_tracker()->GetDamageReasons(), DamageReasonSet { DamageReason::kScrollbarFadeOutAnimation });
    ...
    child->SetDamageReasons({ DamageReason::kAnimatedImage, DamageReason::kUntracked });
    EmulateDrawingOneFrame(root);
    ...
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->has_damage_from_contributing_content());
    EXPECT_EQ(GetRenderSurface(root)->damage_tracker()->GetDamageReasons(), (DamageReasonSet { DamageReason::kAnimatedImage, DamageReason::kUntracked }));
}
...
TEST_F(DamageTrackerTest, VerifyDamageForPropertyChanges)
{
    ...
    EXPECT_EQ(GetRenderSurface(root)->damage_tracker()->GetDamageReasons(), DamageReasonSet {});

    // Then, test the actual layer movement.
    ClearDamageForAllSurfaces(root);
    CreateTransformNode(child).post_translation = child->offset_to_transform_parent();
    child->SetOffsetToTransformParent(gfx::Vector2dF());
    gfx::Transform translation;
    translation.Translate(100.f, 130.f);
    root->layer_tree_impl()->SetTransformMutated(child->element_id(), translation);
    child->SetDamageReasons({ DamageReason::kAnimatedImage });
    EmulateDrawingOneFrame(root);
    ...
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->GetDamageReasons().Has(DamageReason::kAnimatedImage));
}

TEST_F(DamageTrackerTest, VerifyDamageForPropertyChangesFromContributingContents)
{
    LayerImpl* root = CreateAndSetUpTestTreeWithTwoSurfaces();
    ...
    CreateEffectNode(grand_child1_).render_surface_reason = RenderSurfaceReason::kTest;
    EmulateDrawingOneFrame(root);
    ClearDamageForAllSurfaces(root);
...
TEST_F(DamageTrackerTest, VerifyDamageForUpdateAndDamageRectsFromContributingContents)
{
    LayerImpl* root = CreateAndSetUpTestTreeWithTwoSurfaces();
...
TEST_F(DamageTrackerTest, VerifyDamageWhenSurfaceRemoved)
{
    ...
    gfx::Rect root_damage_rect;
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->GetDamageRectIfValid(&root_damage_rect));
    EXPECT_EQ(gfx::Rect(290, 290, 16, 18).ToString(), root_damage_rect.ToString());
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->has_damage_from_contributing_content());
}
...
TEST_F(DamageTrackerTest, VerifyDamageForTransformedLayer)
{
    ...
    transform_node.origin = gfx::Point3F(child->bounds().width() * 0.5f, child->bounds().height() * 0.5f, 0.f);
    ...
    gfx::Rect expected_rect = gfx::ToEnclosingRect(gfx::RectF(expected_position, expected_position, expected_width, expected_width));
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->GetDamageRectIfValid(&root_damage_rect));
    EXPECT_EQ(expected_rect.ToString(), root_damage_rect.ToString());
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->has_damage_from_contributing_content());
...
TEST_F(DamageTrackerTest, VerifyDamageForBlurredSurface)
{
    ...
    EmulateDrawingOneFrame(root);
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->has_damage_from_contributing_content());
    EXPECT_FALSE(GetRenderSurface(surface)->damage_tracker()->has_damage_from_contributing_content());
    ...
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->has_damage_from_contributing_content());
    EXPECT_TRUE(GetRenderSurface(surface)->damage_tracker()->has_damage_from_contributing_content());
}
...
TEST_F(DamageTrackerTest, VerifyDamageForImageFilter)
{
    ...
    FilterOperations filters;
    filters.Append(FilterOperation::CreateReferenceFilter(sk_make_sp<BlurPaintFilter>(2, 2, SkTileMode::kDecal, nullptr)));
    ...
    EmulateDrawingOneFrame(root);
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->GetDamageRectIfValid(&root_damage_rect));
    EXPECT_TRUE(GetRenderSurface(child)->damage_tracker()->GetDamageRectIfValid(&child_damage_rect));
    ...
    EXPECT_FALSE(GetRenderSurface(root)->damage_tracker()->has_damage_from_contributing_content());
    EXPECT_FALSE(GetRenderSurface(child)->damage_tracker()->has_damage_from_contributing_content());
}
...
TEST_F(DamageTrackerTest, VerifyDamageForTransformedImageFilter)
{
    ...
    gfx::Rect expected_root_damage(100 - rotated_outset_left, 100 - blur_outset, expected_rotated_width, 30 + 2 * blur_outset);
    expected_root_damage.Union(gfx::Rect(100, 100, 30, 30));
    EXPECT_EQ(expected_root_damage, root_damage_rect);
    EXPECT_EQ(gfx::Rect(-blur_outset, -blur_outset, 30 + 2 * blur_outset, 30 + 2 * blur_outset), child_damage_rect);
    ...
    EXPECT_EQ(gfx::Rect(100 - blur_outset / 2, 100 - blur_outset, expect_width / 2, expect_height), root_damage_rect);
    EXPECT_EQ(gfx::Rect(-blur_outset, -blur_outset, expect_width, expect_height), child_damage_rect);
}
...
TEST_F(DamageTrackerTest, VerifyDamageForAddingAndRemovingLayer)
{
    ...
    {
        OwnedLayerImplList layers = host_impl()->active_tree()->DetachLayersKeepingRootLayerForTesting();
    ...
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->GetDamageRectIfValid(&root_damage_rect));
    EXPECT_EQ(gfx::Rect(100, 100, 30, 30).ToString(), root_damage_rect.ToString());
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->has_damage_from_contributing_content());
}
...
TEST_F(DamageTrackerTest, VerifyDamageForMultipleLayers)
{
    ...
    gfx::Rect root_damage_rect;
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->GetDamageRectIfValid(&root_damage_rect));
    EXPECT_EQ(gfx::Rect(100, 100, 303, 284).ToString(), root_damage_rect.ToString());
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->has_damage_from_contributing_content());
}
...
TEST_F(DamageTrackerTest, VerifyDamageForNestedSurfaces)
{
    ...
    EXPECT_FALSE(GetRenderSurface(grand_child1_)->damage_tracker()->has_damage_from_contributing_content());
    ...
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->GetDamageRectIfValid(&root_damage_rect));
    EXPECT_EQ(gfx::Rect(200, 200, 6, 8).ToString(), child_damage_rect.ToString());
    EXPECT_EQ(gfx::Rect(11, 11, 295, 297).ToString(), root_damage_rect.ToString());
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->has_damage_from_contributing_content());
    EXPECT_TRUE(GetRenderSurface(child1_)->damage_tracker()->has_damage_from_contributing_content());
    EXPECT_FALSE(GetRenderSurface(child2_)->damage_tracker()->has_damage_from_contributing_content());
    EXPECT_FALSE(GetRenderSurface(grand_child1_)->damage_tracker()->has_damage_from_contributing_content());
...
TEST_F(DamageTrackerTest, VerifyDamageForSurfaceChangeFromDescendantLayer)
{
    ...
    // the layers changed.
    EXPECT_EQ(gfx::Rect(190, 190, 11, 23).ToString(), child_damage_rect.ToString());
    ...
    // space).
    EXPECT_EQ(gfx::Rect(290, 290, 16, 23).ToString(), root_damage_rect.ToString());
...
TEST_F(DamageTrackerTest, VerifyDamageForSurfaceChangeFromDescendantSurface)
{
    ...
    // surface (in target space), and its old exposed area (also in target
    // space).
    EXPECT_EQ(gfx::UnionRects(gfx::Rect(100, 100, 206, 208), gfx::Rect(105, 107, 206, 208)).ToString(), root_damage_rect.ToString());
    ...
    EXPECT_TRUE(GetRenderSurface(child2_)->damage_tracker()->has_damage_from_contributing_content());
...
TEST_F(DamageTrackerTest, VerifyDamageForSurfaceChangeFromViewTransitionLayer)
{
    ...
    LayerImpl* child2 = AddLayerInActiveTree<TestViewTransitionContentLayerImpl>(viz::ViewTransitionElementResourceId(transition_token, 3), false);
    ...
    GetRenderSurface(child1)->OwningEffectNodeMutableForTest()->view_transition_element_resource_id = child2->ViewTransitionResourceId();
    EmulateDrawingOneFrame(root);
    ...
    EXPECT_TRUE(GetRenderSurface(child1)->damage_tracker()->GetDamageRectIfValid(&child1_damage_rect));
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->GetDamageRectIfValid(&root_damage_rect));

    EXPECT_EQ(gfx::Rect(30, 30, 10, 20).ToString(), child1_damage_rect.ToString());

    // The damage from the shared content render surface should contributes to
    // the view transition layer's parent surface.
    EXPECT_EQ(gfx::Rect(130, 130, 50, 70).ToString(), root_damage_rect.ToString());

    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->has_damage_from_contributing_content());
    EXPECT_TRUE(GetRenderSurface(child1)->damage_tracker()->has_damage_from_contributing_content());
}
...
TEST_F(DamageTrackerTest, VerifyDamageForSurfaceChangeFromAncestorLayer)
{
    ...
    root->layer_tree_impl()->SetTransformMutated(child1_->element_id(), translation);
    EmulateDrawingOneFrame(root);
    EXPECT_TRUE(GetRenderSurface(child1_)->damage_tracker()->GetDamageRectIfValid(&child_damage_rect));
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->GetDamageRectIfValid(&root_damage_rect));
    ...
    EXPECT_EQ(gfx::Rect(240, 240, 66, 68).ToString(), root_damage_rect.ToString());
...
TEST_F(DamageTrackerTest, VerifyDamageForMask)
{
    ...
    mask_layer->UpdateRasterSource(FakeRasterSource::CreateFilled(child->bounds()), &empty_invalidation);
    ...
    ClearDamageForAllSurfaces(root);
    auto layers = root->layer_tree_impl()->DetachLayersKeepingRootLayerForTesting();
...
TEST_F(DamageTrackerTest, DamageWhenAddedExternally)
{
    ...
    ClearDamageForAllSurfaces(root);
    child->UnionUpdateRect(gfx::Rect(10, 11, 12, 13));
    GetRenderSurface(root)->damage_tracker()->AddDamageNextUpdate(gfx::Rect(15, 16, 32, 33));
    EmulateDrawingOneFrame(root);
    gfx::Rect root_damage_rect;
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->GetDamageRectIfValid(&root_damage_rect));
    EXPECT_EQ(gfx::UnionRects(gfx::Rect(15, 16, 32, 33), gfx::Rect(100 + 10, 100 + 11, 12, 13)).ToString(), root_damage_rect.ToString());
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->has_damage_from_contributing_content());
    ...
    //
    ClearDamageForAllSurfaces(root);
    GetRenderSurface(root)->damage_tracker()->AddDamageNextUpdate(gfx::Rect(30, 31, 14, 15));
    EmulateDrawingOneFrame(root);
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->GetDamageRectIfValid(&root_damage_rect));
...
TEST_F(DamageTrackerTest, VerifyDamageWithNoContributingLayers)
{
    ...
    CreateEffectNode(empty_surface).render_surface_reason = RenderSurfaceReason::kTest;
    EmulateDrawingOneFrame(root);
    ...
    EXPECT_TRUE(target_surface->damage_tracker()->GetDamageRectIfValid(&damage_rect));
...
TEST_F(DamageTrackerTest, VerifyDamageAccumulatesUntilReset)
{
    ...
    EmulateDrawingOneFrame(root);
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->GetDamageRectIfValid(&root_damage_rect));
    EXPECT_EQ(gfx::Rect(110, 111, 11, 16).ToString(), root_damage_rect.ToString());
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->has_damage_from_contributing_content());
...
TEST_F(DamageTrackerTest, DamageRectTooBig)
{
    ...
    // Really far left.
    child1->SetOffsetToTransformParent(gfx::Vector2dF(static_cast<float>(std::numeric_limits<int>::min() + 100), 0));
    child1->SetBounds(gfx::Size(1, 1));

    // Really far right.
    child2->SetOffsetToTransformParent(gfx::Vector2dF(static_cast<float>(std::numeric_limits<int>::max() - 100), 0));
    ...
    gfx::Rect damage_rect;
    EXPECT_FALSE(GetRenderSurface(root)->damage_tracker()->GetDamageRectIfValid(&damage_rect));
    EXPECT_EQ(GetRenderSurface(root)->content_rect(), GetRenderSurface(root)->GetDamageRect());
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->has_damage_from_contributing_content());
}
...
TEST_F(DamageTrackerTest, DamageRectTooBigInRenderSurface)
{
    ...
    // Really far left.
    grand_child1_->SetOffsetToTransformParent(gfx::Vector2dF(static_cast<float>(std::numeric_limits<int>::min() + 500), 0));
    grand_child1_->SetBounds(gfx::Size(1, 1));
    grand_child1_->SetDrawsContent(true);

    // Really far right.
    grand_child2_->SetOffsetToTransformParent(gfx::Vector2dF(static_cast<float>(std::numeric_limits<int>::max() - 500), 0));
    ...
    gfx::Rect damage_rect;
    EXPECT_FALSE(GetRenderSurface(child1_)->damage_tracker()->GetDamageRectIfValid(&damage_rect));
    EXPECT_EQ(GetRenderSurface(child1_)->content_rect(), GetRenderSurface(child1_)->GetDamageRect());

    // However, the root should just use the child1_ render surface's content rect
    // as damage.
    ASSERT_TRUE(GetRenderSurface(root)->damage_tracker()->GetDamageRectIfValid(&damage_rect));
    EXPECT_TRUE(damage_rect.Contains(GetRenderSurface(root)->content_rect()));
    EXPECT_TRUE(damage_rect.Contains(gfx::ToEnclosingRect(GetRenderSurface(child1_)->DrawableContentRect())));
...
TEST_F(DamageTrackerTest, CanUseCachedBackdropFilterResultTest)
{
    ...
    EmulateDrawingOneFrame(root);
    gfx::Rect damage_rect;
    EXPECT_TRUE(GetRenderSurface(grand_child3_)->damage_tracker()->GetDamageRectIfValid(&damage_rect));
...
TEST_F(DamageTrackerTest, DamageRectOnlyVisibleContentsMoveToOutside)
{
    ...
    // the root surface.
    gfx::Rect damage_rect;
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->GetDamageRectIfValid(&damage_rect));
...
TEST_F(DamageTrackerTest, DamageRectOnlyVisibleContentsLargeTwoContents)
{
    ...
    // Really far left.
    child1->SetOffsetToTransformParent(gfx::Vector2dF(static_cast<float>(std::numeric_limits<int>::min() + 100), 100));
    child1->SetBounds(gfx::Size(std::numeric_limits<int>::max(), child1->bounds().height()));

    // Really far right.
    child2->SetOffsetToTransformParent(gfx::Vector2dF(100, 100));
    child2->SetBounds(gfx::Size(std::numeric_limits<int>::max(), child2->bounds().height()));
    EmulateDrawingOneFrame(root, 1.f);
...
TEST_F(DamageTrackerTest, DamageRectOnlyVisibleContentsHugeContentPartiallyVisible)
{
    ...
    child1->SetOffsetToTransformParent(gfx::Vector2dF(std::numeric_limits<int>::min() + offset, y));
    child1->SetBounds(gfx::Size(std::numeric_limits<int>::max(), child1->bounds().height()));

    EmulateDrawingOneFrame(root);

    gfx::Rect expected_damage_rect1(0, y, expected_width, child1->bounds().height());
    ...
    gfx::Rect expected_damage_rect2(0, y, content_width, child1->bounds().height());
    EXPECT_TRUE(GetRenderSurface(root)->damage_tracker()->GetDamageRectIfValid(&damage_rect));
