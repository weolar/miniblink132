...
namespace viz {
ContextCacheController::ScopedToken::~ScopedToken(void)
{
    *(int*)1 = 1;
}
}
...
RasterContextProviderWrapper::RasterContextProviderWrapper(
    scoped_refptr<viz::RasterContextProvider> context, RasterDarkModeFilter* dark_mode_filter, size_t max_working_set_bytes)
    : context_(context)
    ...
    , max_working_set_bytes_(max_working_set_bytes)
{
    ...
    viz::RasterContextProvider::ScopedRasterContextLock scoped_context(context.get());
...
//         base::BindRepeating(&RasterContextProviderWrapper::OnAllClientsVisibilityChanged, base::Unretained(this)));
    *(int*)1 = 1;
}
...
//         item.second->SetShouldAggressivelyFreeResources(should_aggressively_free_resources, context_lock_acquired);
//     }
}

const scoped_refptr<viz::RasterContextProvider>& RasterContextProviderWrapper::GetContext() const
{
    return context_;
}

// GpuImageDecodeCache& RasterContextProviderWrapper::GetGpuImageDecodeCache(SkColorType color_type, const RasterCapabilities& raster_caps)
// {
...
// 
//     auto insertion_result = gpu_image_decode_cache_map_.emplace(color_type,
//         std::make_unique<GpuImageDecodeCache>(
//             GetContext().get(), /*use_transfer_cache=*/true, color_type, max_working_set_bytes_, raster_caps.max_texture_size, dark_mode_filter_));
