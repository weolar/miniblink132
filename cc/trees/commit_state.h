...
struct CC_EXPORT CommitState {
    ...
    EventListenerProperties event_listener_properties[static_cast<size_t>(EventListenerClass::kLast) + 1] = { EventListenerProperties::kNone };
    ...
    // added here.
    std::vector<PresentationTimeCallbackBuffer::Callback> pending_presentation_callbacks;
    std::vector<PresentationTimeCallbackBuffer::SuccessfulCallbackWithDetails> pending_successful_presentation_callbacks;
    ...
    int64_t primary_main_frame_item_sequence_number = RenderFrameMetadata::kInvalidItemSequenceNumber;
};

struct CC_EXPORT ThreadUnsafeCommitState {
    ThreadUnsafeCommitState(MutatorHost* mh, const ProtectedSequenceSynchronizer& synchronizer);
