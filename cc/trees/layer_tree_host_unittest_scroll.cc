...
std::unique_ptr<ScrollState> BeginState(const gfx::Point& point, const gfx::Vector2dF& delta_hint)
{
...
protected:
    ...
    void SetupTree() override
    {
        ...
        gfx::Size scroll_layer_bounds(root_layer->bounds().width() + 100, root_layer->bounds().height() + 100);
    ...
    void DidCompositorScroll(ElementId element_id, const gfx::PointF& scroll_offset, const std::optional<TargetSnapAreaElementIds>& snap_target_ids) override
    {
        ...
        SetScrollOffsetFromImplSide(layer_tree_host()->LayerByElementId(element_id), scroll_offset);

        if (element_id == layer_tree_host()->OuterViewportScrollLayerForTesting()->element_id()) {
            DidScrollOuterViewport(scroll_offset);
        }
        if (snap_target_ids.has_value()) {
            ScrollNode* scroller_node = layer_tree_host()->property_trees()->scroll_tree_mutable().FindNodeFromElementId(element_id);
            scroller_node->snap_container_data.value().SetTargetSnapAreaElementIds(snap_target_ids.value());
        }
    }
...
public:
    ...
    {
        SetScrollOffset(layer_tree_host()->OuterViewportScrollLayerForTesting(), initial_scroll_);
        PostSetNeedsCommitToMainThread();
    }

    void UpdateLayerTreeHost() override
    {
        Layer* scroll_layer = layer_tree_host()->OuterViewportScrollLayerForTesting();
        if (!layer_tree_host()->SourceFrameNumber()) {
            EXPECT_POINTF_EQ(initial_scroll_, GetTransformNode(scroll_layer)->scroll_offset);
        } else {
            EXPECT_POINTF_EQ(initial_scroll_ + scroll_amount_, GetTransformNode(scroll_layer)->scroll_offset);
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        LayerImpl* scroll_layer = impl->active_tree()->OuterViewportScrollLayerForTesting();
        EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(), ScrollDelta(scroll_layer));

        scroll_layer->SetBounds(gfx::Size(root->bounds().width() + 100, root->bounds().height() + 100));
...
class LayerTreeHostScrollTestScrollMultipleRedraw : public LayerTreeHostScrollTest {
public:
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        case 1:
        case 2:
            EXPECT_POINTF_EQ(initial_scroll_ + scroll_amount_ + scroll_amount_, CurrentScrollOffset(layer_impl));
            break;
        }
    }
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        if (impl->active_tree()->source_frame_number() == 0 && impl->SourceAnimationFrameNumberForTesting() == 1) {
            ...
            scroll_layer->ScrollBy(scroll_amount_);
            EXPECT_VECTOR2DF_EQ(scroll_amount_ + scroll_amount_, ScrollDelta(scroll_layer));
            PostSetNeedsCommitToMainThread();
        } else if (impl->active_tree()->source_frame_number() == 1) {
            // Third or later draw after second commit.
            EXPECT_GE(impl->SourceAnimationFrameNumberForTesting(), 3u);
            EXPECT_POINTF_EQ(initial_scroll_ + scroll_amount_ + scroll_amount_, CurrentScrollOffset(scroll_layer));
            EndTest();
        }
    }
...
class LayerTreeHostScrollTestScrollAbortedCommit : public LayerTreeHostScrollTest {
public:
    ...
    void SetupTree() override
    {
        ...
        layer_tree_host()->OuterViewportScrollLayerForTesting()->SetBounds(scroll_layer_bounds);
    ...
    void WillBeginMainFrame() override
    {
        ...
        Layer* root_scroll_layer = layer_tree_host()->OuterViewportScrollLayerForTesting();
        ...
        case 1:
            ...
            EXPECT_POINTF_EQ(initial_scroll_, CurrentScrollOffset(root_scroll_layer));
        ...
        case 2:
            ...
            EXPECT_POINTF_EQ(initial_scroll_ + impl_scroll_, CurrentScrollOffset(root_scroll_layer));
        ...
        case 3:
            ...
            EXPECT_POINTF_EQ(initial_scroll_ + impl_scroll_ + impl_scroll_, CurrentScrollOffset(root_scroll_layer));
            EXPECT_EQ(impl_scale_ * impl_scale_, layer_tree_host()->page_scale_factor());
            SetScrollOffset(root_scroll_layer, CurrentScrollOffset(root_scroll_layer) + second_main_scroll_);
            break;
        case 4:
            ...
            EXPECT_POINTF_EQ(initial_scroll_ + delta, CurrentScrollOffset(root_scroll_layer));
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* impl) override
    {
        LayerImpl* root_scroll_layer = impl->active_tree()->OuterViewportScrollLayerForTesting();
        ...
        if (impl->active_tree()->source_frame_number() == 0 && impl->SourceAnimationFrameNumberForTesting() == 1) {
            ...
            root_scroll_layer->ScrollBy(impl_scroll_);
            EXPECT_VECTOR2DF_EQ(impl_scroll_, ScrollDelta(root_scroll_layer));
            EXPECT_POINTF_EQ(initial_scroll_ + impl_scroll_, ScrollOffsetBase(root_scroll_layer));

            EXPECT_EQ(1.f, impl->active_tree()->page_scale_delta());
            EXPECT_EQ(impl_scale_, impl->active_tree()->current_page_scale_factor());
            impl->active_tree()->SetPageScaleOnActiveTree(impl_scale_ * impl_scale_);
            EXPECT_EQ(impl_scale_, impl->active_tree()->page_scale_delta());
            EXPECT_EQ(impl_scale_ * impl_scale_, impl->active_tree()->current_page_scale_factor());
            ...
            EXPECT_POINTF_EQ(initial_scroll_ + delta, ScrollOffsetBase(root_scroll_layer));
...
public:
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        case 0:
            ...
            break;
        case 1:
            EXPECT_VECTOR2DF_EQ(gfx::ToRoundedVector2d(scroll_amount_), ScrollOffsetBase(scroll_layer).OffsetFromOrigin());
            EXPECT_VECTOR2DF_EQ(scroll_amount_ - gfx::ToRoundedVector2d(scroll_amount_), ScrollDelta(scroll_layer));
            PostSetNeedsCommitToMainThread();
            break;
        case 2:
            EXPECT_VECTOR2DF_EQ(gfx::ToRoundedVector2d(scroll_amount_ + scroll_amount_), ScrollOffsetBase(scroll_layer).OffsetFromOrigin());
            EXPECT_VECTOR2DF_EQ(scroll_amount_ + scroll_amount_ - gfx::ToRoundedVector2d(scroll_amount_ + scroll_amount_), ScrollDelta(scroll_layer));
            EndTest();
            break;
        }
        scroll_layer->ScrollBy(scroll_amount_);
...
public:
    ...
    void SetupTree() override
    {
        ...
        CopyProperties(layer_tree_host()->OuterViewportScrollLayerForTesting(), container.get());
        CreateTransformNode(container.get()).post_translation = gfx::Vector2dF(0.25, 0);
        CreateEffectNode(container.get()).render_surface_reason = RenderSurfaceReason::kTest;
        ...
        scroll_layer_->SetElementId(LayerIdToElementIdForTesting(scroll_layer_->id()));
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        switch (impl->active_tree()->source_frame_number()) {
        case 0:
            EXPECT_EQ(gfx::Transform(), scroll_layer->draw_properties().screen_space_transform);
            PostSetNeedsCommitToMainThread();
            break;
        case 1:
            translate.Translate(-2, 0);
            EXPECT_EQ(translate, scroll_layer->draw_properties().screen_space_transform);
            PostSetNeedsCommitToMainThread();
            break;
        case 2:
...
public:
    ...
    void SetupTree() override
    {
        ...
        child_layer_->SetElementId(LayerIdToElementIdForTesting(child_layer_->id()));
        ...
        CreateTransformNode(child_layer_.get()).post_translation = child_layer_offset;
    ...
    void DidCompositorScroll(ElementId element_id, const gfx::PointF& offset, const std::optional<TargetSnapAreaElementIds>& snap_target_ids) override
    {
        LayerTreeHostScrollTest::DidCompositorScroll(element_id, offset, snap_target_ids);
    ...
    void UpdateLayerTreeHost() override
    {
        EXPECT_POINTF_EQ(gfx::PointF(), CurrentScrollOffset(expected_no_scroll_layer_));

        switch (layer_tree_host()->SourceFrameNumber()) {
        case 0:
            EXPECT_POINTF_EQ(initial_offset_, CurrentScrollOffset(expected_scroll_layer_));
            break;
        case 1:
            EXPECT_POINTF_EQ(initial_offset_ + scroll_amount_, CurrentScrollOffset(expected_scroll_layer_));

            // Pretend like Javascript updated the scroll position itself.
            SetScrollOffset(expected_scroll_layer_, javascript_scroll_);
            break;
        case 2:
            EXPECT_POINTF_EQ(javascript_scroll_ + scroll_amount_, CurrentScrollOffset(expected_scroll_layer_));
            break;
        }
    }
    ...
    {
        LayerImpl* inner_scroll = impl->active_tree()->InnerViewportScrollLayerForTesting();
        LayerImpl* root_scroll_layer_impl = impl->active_tree()->OuterViewportScrollLayerForTesting();
        LayerImpl* child_layer_impl = root_scroll_layer_impl->layer_tree_impl()->LayerById(child_layer_->id());
        ...
        EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(), ScrollDelta(expected_no_scroll_layer_impl));
        ...
        case 0: {
            ...
            gfx::Point scroll_point = gfx::ToCeiledPoint(gfx::PointF(-0.5f, -0.5f) + GetTransformNode(expected_scroll_layer_impl)->post_translation);
            InputHandler::ScrollStatus status
                = impl->GetInputHandler().ScrollBegin(BeginState(scroll_point, scroll_amount_).get(), ui::ScrollInputType::kTouchscreen);
            EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
            impl->GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), scroll_amount_));
            ...
            EXPECT_EQ(scrolling_node->id, impl->active_tree()->LastScrolledScrollNodeIndex());

            // Check the scroll is applied as a delta.
            EXPECT_POINTF_EQ(initial_offset_, ScrollOffsetBase(expected_scroll_layer_impl));
            EXPECT_VECTOR2DF_EQ(scroll_amount_, ScrollDelta(expected_scroll_layer_impl));
            break;
        }
        case 1: {
            ...
            gfx::Point scroll_point = gfx::ToCeiledPoint(gfx::PointF(0.5f, 0.5f) + GetTransformNode(expected_scroll_layer_impl)->post_translation);
            InputHandler::ScrollStatus status
                = impl->GetInputHandler().ScrollBegin(BeginState(scroll_point, scroll_amount_).get(), ui::ScrollInputType::kWheel);
            EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
            impl->GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), scroll_amount_));
            impl->GetInputHandler().ScrollEnd();

            // Check the scroll is applied as a delta.
            EXPECT_POINTF_EQ(javascript_scroll_, ScrollOffsetBase(expected_scroll_layer_impl));
            EXPECT_VECTOR2DF_EQ(scroll_amount_, ScrollDelta(expected_scroll_layer_impl));
            break;
        }
        case 2:

            EXPECT_POINTF_EQ(javascript_scroll_ + scroll_amount_, ScrollOffsetBase(expected_scroll_layer_impl));
            EXPECT_VECTOR2DF_EQ(gfx::Vector2d(), ScrollDelta(expected_scroll_layer_impl));
...
#define MAYBE_DeviceScaleFactor1_ScrollChild DISABLED_DeviceScaleFactor1_ScrollChild
#else
#define MAYBE_DeviceScaleFactor1_ScrollChild DeviceScaleFactor1_ScrollChild
#endif
TEST_F(LayerTreeHostScrollTestCaseWithChild, MAYBE_DeviceScaleFactor1_ScrollChild)
{
...
#define MAYBE_DeviceScaleFactor15_ScrollChild DISABLED_DeviceScaleFactor15_ScrollChild
#else
#define MAYBE_DeviceScaleFactor15_ScrollChild DeviceScaleFactor15_ScrollChild
#endif
TEST_F(LayerTreeHostScrollTestCaseWithChild, MAYBE_DeviceScaleFactor15_ScrollChild)
{
...
// TODO(crbug.com/345499781): Test is flaky on Linux.
#if defined(ADDRESS_SANITIZER) || defined(IS_LINUX)
#define MAYBE_DeviceScaleFactor2_ScrollChild DISABLED_DeviceScaleFactor2_ScrollChild
#else
#define MAYBE_DeviceScaleFactor2_ScrollChild DeviceScaleFactor2_ScrollChild
#endif
TEST_F(LayerTreeHostScrollTestCaseWithChild, MAYBE_DeviceScaleFactor2_ScrollChild)
{
...
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_MAC) || defined(ADDRESS_SANITIZER)
#define MAYBE_DeviceScaleFactor1_ScrollRootScrollLayer DISABLED_DeviceScaleFactor1_ScrollRootScrollLayer
#else
#define MAYBE_DeviceScaleFactor1_ScrollRootScrollLayer DeviceScaleFactor1_ScrollRootScrollLayer
#endif
TEST_F(LayerTreeHostScrollTestCaseWithChild, MAYBE_DeviceScaleFactor1_ScrollRootScrollLayer)
{
    device_scale_factor_ = 1.f;
...
#if defined(ADDRESS_SANITIZER) || defined(IS_LINUX)
#define MAYBE_DeviceScaleFactor15_ScrollRootScrollLayer DISABLED_DeviceScaleFactor15_ScrollRootScrollLayer
#else
#define MAYBE_DeviceScaleFactor15_ScrollRootScrollLayer DeviceScaleFactor15_ScrollRootScrollLayer
#endif
TEST_F(LayerTreeHostScrollTestCaseWithChild, MAYBE_DeviceScaleFactor15_ScrollRootScrollLayer)
{
    device_scale_factor_ = 1.5f;
    scroll_child_layer_ = false;
    RunTest(CompositorMode::THREADED);
...
// Test is flaky on asan on multiple platforms.
#if defined(ADDRESS_SANITIZER) || defined(IS_LINUX)
#define MAYBE_DeviceScaleFactor2_ScrollRootScrollLayer DISABLED_DeviceScaleFactor2_ScrollRootScrollLayer
#else
#define MAYBE_DeviceScaleFactor2_ScrollRootScrollLayer DeviceScaleFactor2_ScrollRootScrollLayer
#endif
TEST_F(LayerTreeHostScrollTestCaseWithChild, MAYBE_DeviceScaleFactor2_ScrollRootScrollLayer)
{
    device_scale_factor_ = 2.f;
    scroll_child_layer_ = false;
    RunTest(CompositorMode::THREADED);
...
public:
    ...
    void UpdateLayerTreeHost() override
    {
        ...
        if (!layer_tree_host()->SourceFrameNumber()) {
            ...
            EXPECT_POINTF_EQ(CurrentScrollOffset(scroll_layer), initial_scroll_ + impl_thread_scroll1_);

            // Pretend like Javascript updated the scroll position itself with a
            // change of main_thread_scroll.
            SetScrollOffset(scroll_layer, initial_scroll_ + main_thread_scroll_ + impl_thread_scroll1_);
        }
    }
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        case 0:
            if (!impl->pending_tree()) {
                ...
                EXPECT_POINTF_EQ(initial_scroll_, ScrollOffsetBase(scroll_layer));
                EXPECT_VECTOR2DF_EQ(impl_thread_scroll1_ + impl_thread_scroll2_, ScrollDelta(scroll_layer));

                LayerImpl* pending_scroll_layer = impl->pending_tree()->OuterViewportScrollLayerForTesting();
                EXPECT_POINTF_EQ(initial_scroll_ + main_thread_scroll_ + impl_thread_scroll1_, ScrollOffsetBase(pending_scroll_layer));
                EXPECT_VECTOR2DF_EQ(impl_thread_scroll2_, ScrollDelta(pending_scroll_layer));
            }
            break;
        case 1:
            EXPECT_FALSE(impl->pending_tree());
            EXPECT_POINTF_EQ(initial_scroll_ + main_thread_scroll_ + impl_thread_scroll1_, ScrollOffsetBase(scroll_layer));
...
public:
    ...
    void WillCommit(const CommitState& commit_state) override
    {
        ...
        case 0:
            EXPECT_TRUE(commit_state.layers_that_should_push_properties.contains(scroll_layer));
            break;
        case 1:
            // Even if this layer doesn't need push properties, it should
            // still pick up scrolls that happen on the active layer during
            // commit.
            EXPECT_FALSE(commit_state.layers_that_should_push_properties.contains(scroll_layer));
            break;
        }
    }
    ...
    void BeginCommitOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        if (impl->active_tree()->source_frame_number() == 0) {
            ...
            LayerImpl* active_scroll_layer = impl->active_tree()->OuterViewportScrollLayerForTesting();
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        // the second commit.
        LayerImpl* active_root = impl->active_tree()->root_layer();
        LayerImpl* active_scroll_layer = active_root ? impl->active_tree()->OuterViewportScrollLayerForTesting() : nullptr;
        ...
        case 0:
            EXPECT_POINTF_EQ(initial_scroll_, ScrollOffsetBase(pending_scroll_layer));
            EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(), ScrollDelta(pending_scroll_layer));
        ...
        case 1:
            ...
            EXPECT_POINTF_EQ(initial_scroll_, ScrollOffsetBase(pending_scroll_layer));
            EXPECT_VECTOR2DF_EQ(impl_thread_scroll_, ScrollDelta(pending_scroll_layer));
            ASSERT_TRUE(active_root);
            EXPECT_POINTF_EQ(initial_scroll_, ScrollOffsetBase(active_scroll_layer));
            EXPECT_VECTOR2DF_EQ(impl_thread_scroll_, ScrollDelta(active_scroll_layer));
            break;
        case 2:
            // On the next commit, this delta should have been sent and applied.
            EXPECT_POINTF_EQ(initial_scroll_ + impl_thread_scroll_, ScrollOffsetBase(pending_scroll_layer));
        ...
        LayerImpl* scroll_layer = impl->pending_tree()->OuterViewportScrollLayerForTesting();
        gfx::PointF scroll_offset = CurrentScrollOffset(scroll_layer);
        int transform_index = scroll_layer->transform_tree_index();
        gfx::PointF transform_tree_scroll_offset = impl->pending_tree()->property_trees()->transform_tree().Node(transform_index)->scroll_offset;
    ...
    void DidActivateTreeOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        if (host_impl->active_tree()->source_frame_number() == 0) {
            ...
            const gfx::Vector2dF scroll_amount(350, -350);
            {
                ...
                InputHandler::ScrollStatus status = host_impl->GetInputHandler().ScrollBegin(scroll_state.get(), ui::ScrollInputType::kWheel);
                EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
            }
            {
                ...
                EXPECT_TRUE(host_impl->mutator_host()->HasImplOnlyScrollAnimatingElement());
            }
    ...
    void UpdateLayerTreeHost() override
    {
        ...
        if (layer_tree_host()->HasCompositorDrivenScrollAnimationForTesting()) {
            ...
            layer_tree_host()->RequestScrollAnimationEndNotification(
                base::BindOnce(&SmoothScrollAnimationEndNotification::OnScrollEnd, base::Unretained(this)));
        }
    }
...
void DoGestureScroll(LayerTreeHostImpl* host_impl, const scoped_refptr<Layer>& scroller, gfx::Vector2dF offset, ElementId scroller_element_id)
{
    ScrollStateData begin_scroll_state_data;
    begin_scroll_state_data.set_current_native_scrolling_element(scroller_element_id);
    begin_scroll_state_data.delta_x_hint = offset.x();
    begin_scroll_state_data.delta_y_hint = offset.y();
    std::unique_ptr<ScrollState> begin_scroll_state(new ScrollState(begin_scroll_state_data));
    auto scroll_status = host_impl->GetInputHandler().ScrollBegin(begin_scroll_state.get(), ui::ScrollInputType::kTouchscreen);
    ...
    host_impl->GetInputHandler().ScrollUpdate(ScrollState(update_scroll_state_data));
...
// thread.
class LayerTreeHostScrollTestImplOnlyScrollSnap : public LayerTreeHostScrollTest {
public:
    ...
    void SetupTree() override
    {
        ...
        SnapAreaData snap_area_data(ScrollSnapAlign(SnapAlignment::kStart), gfx::RectF(500, 500, 100, 100), false, false, snap_area_id_);

        // Set up snap container data.
        SnapContainerData snap_container_data(
            ScrollSnapType(false, SnapAxis::kBoth, SnapStrictness::kMandatory), gfx::RectF(0, 0, 100, 100), gfx::PointF(900, 900));
        snap_container_data.AddSnapAreaData(snap_area_data);
        CreateScrollNode(scroller_.get(), container_->bounds()).snap_container_data = snap_container_data;
    ...
    void WillSendBeginMainFrameOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        if (host_impl->active_tree()->source_frame_number() == 0) {
            LayerImpl* scroller_impl = host_impl->active_tree()->LayerById(scroller_->id());

            DoGestureScroll(host_impl, scroller_, impl_thread_scroll_, scroller_element_id_);

            EXPECT_TRUE(host_impl->GetInputHandler().animating_for_snap_for_testing(scroller_element_id_));
            EXPECT_VECTOR2DF_EQ(impl_thread_scroll_, ScrollDelta(scroller_impl));
        } else {
            snap_animation_finished_ = !host_impl->GetInputHandler().animating_for_snap_for_testing(scroller_element_id_);
        }
    }

    void UpdateLayerTreeHost() override
    {
        const ScrollNode* scroller_node = layer_tree_host()->property_trees()->scroll_tree().Node(scroller_->scroll_tree_index());
        auto snap_target_ids = scroller_node->snap_container_data.value().GetTargetSnapAreaElementIds();
        ...
        if (snap_animation_finished_) {
            ...
            EXPECT_EQ(snap_target_ids, TargetSnapAreaElementIds(snap_area_id_, snap_area_id_));
            EndTest();
        } else {
...
#if !BUILDFLAG(IS_FUCHSIA) && !BUILDFLAG(IS_CHROMEOS) && !BUILDFLAG(IS_LINUX) && !((BUILDFLAG(IS_WIN) || BUILDFLAG(IS_MAC)) && defined(ADDRESS_SANITIZER))
...
class LayerTreeHostScrollTestImplOnlyMultipleScrollSnap : public LayerTreeHostScrollTest {
public:
    ...
    void SetupTree() override
    {
        ...
        SnapAreaData snap_area_data_a(ScrollSnapAlign(SnapAlignment::kStart), gfx::RectF(500, 500, 100, 100), false, false, snap_area_a_id_);
        ...
        SnapAreaData snap_area_data_b(ScrollSnapAlign(SnapAlignment::kStart), gfx::RectF(500, 500, 100, 100), false, false, snap_area_b_id_);

        // Set up snap container data.
        SnapContainerData snap_container_data_a(
            ScrollSnapType(false, SnapAxis::kBoth, SnapStrictness::kMandatory), gfx::RectF(0, 0, 100, 100), gfx::PointF(900, 900));
        snap_container_data_a.AddSnapAreaData(snap_area_data_a);
        CreateScrollNode(scroller_a_.get(), container_->bounds()).snap_container_data = snap_container_data_a;

        // Set up snap container data.
        SnapContainerData snap_container_data_b(
            ScrollSnapType(false, SnapAxis::kBoth, SnapStrictness::kMandatory), gfx::RectF(0, 0, 100, 100), gfx::PointF(900, 900));
        snap_container_data_b.AddSnapAreaData(snap_area_data_b);
        CreateScrollNode(scroller_b_.get(), container_->bounds()).snap_container_data = snap_container_data_b;
    ...
    void UpdateLayerTreeHost() override
    {
        const ScrollNode* scroller_node_a = layer_tree_host()->property_trees()->scroll_tree().Node(scroller_a_->scroll_tree_index());
        const ScrollNode* scroller_node_b = layer_tree_host()->property_trees()->scroll_tree().Node(scroller_b_->scroll_tree_index());
        auto snap_target_ids_a = scroller_node_a->snap_container_data.value().GetTargetSnapAreaElementIds();
        auto snap_target_ids_b = scroller_node_b->snap_container_data.value().GetTargetSnapAreaElementIds();
        if (layer_tree_host()->SourceFrameNumber() == 0) {
            // On the first BeginMainFrame scrolling has not happened yet.
            // Check that the scroll offset and scroll snap targets are at the initial
            // values on the main thread.
            ...
            EXPECT_EQ(snap_target_ids_a, TargetSnapAreaElementIds(snap_area_a_id_, snap_area_a_id_));
            EXPECT_EQ(snap_target_ids_b, TargetSnapAreaElementIds(snap_area_b_id_, snap_area_b_id_));
            EndTest();
        }
    }
    ...
    void DidActivateTreeOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        // to the main thread on the next BeginMainFrame.
        if (host_impl->active_tree()->source_frame_number() == 0) {
            LayerImpl* scroller_impl_a = host_impl->active_tree()->LayerById(scroller_a_->id());
            LayerImpl* scroller_impl_b = host_impl->active_tree()->LayerById(scroller_b_->id());

            DoGestureScroll(host_impl, scroller_a_, impl_thread_scroll_a_, scroller_a_element_id_);
            DoGestureScroll(host_impl, scroller_b_, impl_thread_scroll_b_, scroller_b_element_id_);
...
class LayerTreeHostScrollTestScrollZeroMaxScrollOffset : public LayerTreeHostScrollTest {
public:
    ...
    void SetupTree() override
    {
        ...
        CopyProperties(layer_tree_host()->OuterViewportScrollLayerForTesting(), scroller_.get());
        CreateTransformNode(scroller_.get());
        CreateScrollNode(scroller_.get(), layer_tree_host()->root_layer()->bounds());
    ...
    void NextStep()
    {
        ...
        ScrollTree& scroll_tree = layer_tree_host()->property_trees()->scroll_tree_mutable();
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        const ScrollTree& scroll_tree = impl->active_tree()->property_trees()->scroll_tree();
        ...
        InputHandler::ScrollStatus status = impl->GetInputHandler().ScrollBegin(&scroll_state, ui::ScrollInputType::kTouchscreen);
        EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread) << "In Frame " << impl->active_tree()->source_frame_number();
        ...
        case 2:
            ...
            EXPECT_EQ(impl->OuterViewportScrollNode(), impl->CurrentlyScrollingNode()) << "In Frame 1";
            break;
        case 3:
            // Since the max_scroll_offset is (-1, -1) - we shouldn't target it and
            // we should instead bubble up to the viewport.
            EXPECT_EQ(impl->OuterViewportScrollNode(), impl->CurrentlyScrollingNode()) << "In Frame 2";
            EndTest();
            break;
        }
        impl->GetInputHandler().ScrollEnd();
        MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&LayerTreeHostScrollTestScrollZeroMaxScrollOffset::NextStep, base::Unretained(this)));
    }
...
class LayerTreeHostScrollTestScrollNonDrawnLayer : public LayerTreeHostScrollTest {
public:
    ...
    {
        LayerTreeHostScrollTest::SetupTree();
        layer_tree_host()->OuterViewportScrollLayerForTesting()->SetIsDrawable(false);
        SetScrollOffset(layer_tree_host()->OuterViewportScrollLayerForTesting(), gfx::PointF(20.f, 20.f));
        layer_tree_host()->OuterViewportScrollLayerForTesting()->SetMainThreadScrollHitTestRegion(gfx::Rect(20, 20, 20, 20));
    }
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        // scrollable region.
        InputHandler::ScrollStatus status
            = impl->GetInputHandler().ScrollBegin(BeginState(gfx::Point(0, 0), gfx::Vector2dF(0, 1)).get(), ui::ScrollInputType::kTouchscreen);
        EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
        EXPECT_EQ(MainThreadScrollingReason::kMainThreadScrollHitTestRegion, status.main_thread_hit_test_reasons);
        impl->GetInputHandler().ScrollEnd();

        status = impl->GetInputHandler().ScrollBegin(BeginState(gfx::Point(21, 21), gfx::Vector2dF(0, 1)).get(), ui::ScrollInputType::kTouchscreen);
        EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
...
class LayerTreeHostScrollTestImplScrollUnderMainThreadScrollingParent : public LayerTreeHostScrollTest {
public:
    ...
    {
        LayerTreeHostScrollTest::SetupTree();
        GetScrollNode(layer_tree_host()->OuterViewportScrollLayerForTesting())->main_thread_repaint_reasons
    ...
    {
        const ScrollTree& scroll_tree = impl->active_tree()->property_trees()->scroll_tree();
        const ScrollNode* scroller_scroll_node = scroll_tree.Node(scroll_tree_index_);
        ...
        // Scroll hitting the viewport layer.
        {
            ...
            EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
            EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);
            EXPECT_EQ(impl->CurrentlyScrollingNode(), impl->OuterViewportScrollNode());
            impl->GetInputHandler().ScrollEnd();
        }
...
//    LayerTreeHostScrollTestImplScrollUnderMainThreadScrollingParent);
MULTI_THREAD_TEST_F(LayerTreeHostScrollTestImplScrollUnderMainThreadScrollingParent);
#else
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostScrollTestImplScrollUnderMainThreadScrollingParent);
#endif

class ThreadCheckingInputHandlerClient : public InputHandlerClient {
public:
    ThreadCheckingInputHandlerClient(base::SingleThreadTaskRunner* runner, bool* received_stop_flinging)
        : task_runner_(runner)
...
class LayerTreeHostScrollTestLayerStructureChange : public LayerTreeHostScrollTest {
public:
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* impl) override
    {
        switch (impl->active_tree()->source_frame_number()) {
        case 0:
            SetScrollOffsetDelta(impl->active_tree()->LayerById(root_scroll_layer_id_), gfx::Vector2dF(5, 5));
            SetScrollOffsetDelta(impl->active_tree()->LayerById(child_scroll_layer_id_), gfx::Vector2dF(5, 5));
            SetScrollOffsetDelta(impl->active_tree()->LayerById(sibling_scroll_layer_id_), gfx::Vector2dF(5, 5));
            PostSetNeedsCommitToMainThread();
            break;
        case 1:
            EndTest();
    ...
    void DidCompositorScroll(ElementId element_id, const gfx::PointF&, const std::optional<TargetSnapAreaElementIds>&) override
    {
...
protected:
    Layer* CreateScrollLayer(Layer* parent)
    {
        ...
        scroll_layer->SetBounds(gfx::Size(parent->bounds().width() + 100, parent->bounds().height() + 100));
...
public:
    ...
    void UpdateLayerTreeHost() override
    {
        ...
        case 0:
            ...
            break;
        case 1:
            EXPECT_POINTF_EQ(initial_scroll_ + scroll_amount_, CurrentScrollOffset(scroll_layer));
            // Pretend like Javascript updated the scroll position itself.
            SetScrollOffset(scroll_layer, second_scroll_);
            break;
        case 2:
...
class LayerTreeHostScrollTestScrollAbortedCommitMFBA : public LayerTreeHostScrollTest {
public:
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        case 0: {
            ...
            case 2: {
                ...
                EXPECT_VECTOR2DF_EQ(impl_scroll_, ScrollDelta(root_scroll_layer));
                root_scroll_layer->ScrollBy(impl_scroll_);
                EXPECT_VECTOR2DF_EQ(impl_scroll_ + impl_scroll_, ScrollDelta(root_scroll_layer));
                EXPECT_POINTF_EQ(initial_scroll_, ScrollOffsetBase(root_scroll_layer));
        ...
        case 1: {
            ...
            case 1: {
                ...
                EXPECT_POINTF_EQ(initial_scroll_ + prev_delta, ScrollOffsetBase(root_scroll_layer));
                // Ask for another commit (which will abort).
                impl->SetNeedsCommit();
                break;
            }
...
class LayerTreeHostScrollTestElasticOverscroll : public LayerTreeHostScrollTest {
public:
    ...
    void BeginTest() override
    {
        ...
        ImplThreadTaskRunner()->PostTask(FROM_HERE,
            base::BindOnce(&LayerTreeHostScrollTestElasticOverscroll::BindInputHandler, base::Unretained(this), layer_tree_host()->GetDelegateForInput()));
        PostSetNeedsCommitToMainThread();
    }
    ...
    {
        DCHECK_NE(0, num_begin_main_frames_main_thread_) << "The first BeginMainFrame has no deltas to report";
        ...
        EXPECT_EQ(expected_elastic_overscroll, layer_tree_host()->elastic_overscroll());
    }

    void WillBeginMainFrame() override
    {
        num_begin_main_frames_main_thread_++;
    }
    ...
    void VerifyBeginMainFrameResultOnImplThread(LayerTreeHostImpl* host_impl, bool begin_main_frame_aborted)
    {
        gfx::Vector2dF expected_elastic_overscroll = elastic_overscroll_test_cases_[num_begin_main_frames_impl_thread_];
        EXPECT_EQ(expected_elastic_overscroll, scroll_elasticity_helper_->StretchAmount());
        if (!begin_main_frame_aborted)
            EXPECT_EQ(expected_elastic_overscroll, host_impl->pending_tree()->elastic_overscroll()->Current(false));
    ...
    void WillPrepareToDrawOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        // before each draw.
        EXPECT_CALL(input_handler_client_, ReconcileElasticOverscrollAndRootScroll()).Times(1);
    }
...
private:
    ...
    const gfx::Vector2dF elastic_overscroll_test_cases_[5]
        = { gfx::Vector2dF(0, 0), gfx::Vector2dF(5, 10), gfx::Vector2dF(5, 5), gfx::Vector2dF(-4, -5), gfx::Vector2dF(0, 0) };
};

MULTI_THREAD_TEST_F(LayerTreeHostScrollTestElasticOverscroll);

class LayerTreeHostScrollTestPropertyTreeUpdate : public LayerTreeHostScrollTest {
public:
    ...
    void DidActivateTreeOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        case 1:
            EXPECT_POINTF_EQ(second_scroll_, ScrollOffsetBase(scroll_layer));
            EXPECT_POINTF_EQ(second_scroll_, GetTransformNode(scroll_layer)->scroll_offset);
            EndTest();
            break;
        }
    }
...
class LayerTreeHostScrollTestImplSideInvalidation : public LayerTreeHostScrollTest {
    void BeginTest() override
    {
        PostSetNeedsCommitToMainThread();
    }
    ...
    void DidScrollOuterViewport(const gfx::PointF& offset) override
    {
        ...
        // created for the invalidation request.
        {
            ...
            task_runner_provider()->ImplThreadTaskRunner()->PostTask(
                FROM_HERE, base::BindOnce(&LayerTreeHostScrollTestImplSideInvalidation::WaitForInvalidationOnImplThread, base::Unretained(this), &completion));
            completion.Wait();
        }
        ...
        case 1: {
            ...
            Layer* outer_viewport_layer = layer_tree_host()->OuterViewportScrollLayerForTesting();
            gfx::Vector2dF delta_to_send = outer_viewport_offsets_[2] - outer_viewport_offsets_[1];
            SetScrollOffset(outer_viewport_layer, CurrentScrollOffset(outer_viewport_layer) + delta_to_send);
        } break;
        case 2:
    ...
    void WillNotifyReadyToActivateOnThread(LayerTreeHostImpl* host_impl) override
    {
        // Ensure that the scroll-offsets on the TransformTree are consistent with
        // the synced scroll offsets, for the pending tree.
        ...
        LayerImpl* scroll_layer = host_impl->pending_tree()->OuterViewportScrollLayerForTesting();
        gfx::PointF scroll_offset = CurrentScrollOffset(scroll_layer);
        int transform_index = scroll_layer->transform_tree_index();
        gfx::PointF transform_tree_scroll_offset = host_impl->pending_tree()->property_trees()->transform_tree().Node(transform_index)->scroll_offset;
        EXPECT_EQ(scroll_offset, transform_tree_scroll_offset);
    }
    ...
    void WillSendBeginMainFrameOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        case 2:
            ...
            host_impl->active_tree()->OuterViewportScrollLayerForTesting()->SetCurrentScrollOffset(outer_viewport_offsets_[1]);
    ...
    void BeginMainFrameAbortedOnThread(LayerTreeHostImpl* host_impl, CommitEarlyOutReason reason, bool /* did_sync_scroll_and_viewport */) override
    {
        ...
        EXPECT_EQ(outer_viewport_offsets_[2], CurrentScrollOffset(host_impl->active_tree()->OuterViewportScrollLayerForTesting()));
    }
    ...
    void DidActivateTreeOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        case 1:
            ...
            host_impl->active_tree()->OuterViewportScrollLayerForTesting()->SetCurrentScrollOffset(outer_viewport_offsets_[0]);
            host_impl->SetNeedsCommit();
            break;
        case 2:
            ...
            EXPECT_EQ(outer_viewport_offsets_[1], CurrentScrollOffset(host_impl->active_tree()->OuterViewportScrollLayerForTesting()));
            break;
        case 3:
    ...
    const gfx::PointF outer_viewport_offsets_[3] = { gfx::PointF(20, 20), gfx::PointF(50, 50), gfx::PointF(70, 70) };
...
public:
    ...
    {
        LayerTreeHostScrollTest::SetupTree();
        GetViewportScrollNode()->main_thread_repaint_reasons = MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects;
    }

    void UpdateLayerTreeHost() override
    {
        if (layer_tree_host()->SourceFrameNumber() == 1)
            GetViewportScrollNode()->main_thread_repaint_reasons = MainThreadScrollingReason::kNotScrollingOnMain;
    }
    ...
    void DidActivateTreeOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        auto* proxy_impl = static_cast<const ProxyImpl*>(host_impl->client_for_testing());
        ...
        if (frame_number > 2) {
            if (proxy_impl->SmoothnessPriorityExpirationNotifierForTesting().HasPendingNotification()) {
                EXPECT_EQ(SMOOTHNESS_TAKES_PRIORITY, host_impl->GetTreePriority());
                MainThreadTaskRunner()->PostDelayedTask(
                    FROM_HERE, base::BindOnce(&LayerTreeTest::PostSetNeedsCommitToMainThread, base::Unretained(this)), base::Milliseconds(50));
            } else {
...
private:
    ...
    {
        input_handler.ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(0, 10)).get(), ui::ScrollInputType::kTouchscreen);
        input_handler.ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, 10)));
    }
};

MULTI_THREAD_TEST_F(LayerTreeHostRasterPriorityTest);

class NonScrollingMainThreadScrollHitTestRegion : public LayerTreeHostScrollTest {
public:
    NonScrollingMainThreadScrollHitTestRegion()
    {
        SetUseLayerLists();
    }
    ...
    void SetupTree() override
    {
        ...
        LayerTreeHostScrollTest::SetupTree();
        Layer* outer_scroll = layer_tree_host()->OuterViewportScrollLayerForTesting();
        ...
        bottom_->SetMainThreadScrollHitTestRegion(Region(gfx::Rect(50, 50, 50, 50)));
        ...
        middle_scrollable_->SetElementId(LayerIdToElementIdForTesting(middle_scrollable_->id()));
    ...
    void WillCommit(const CommitState&) override
    {
        middle_scrollable_scroll_tree_index_ = middle_scrollable_->scroll_tree_index();
    }
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        const ScrollNode* scroll_node = impl->active_tree()->property_trees()->scroll_tree().Node(middle_scrollable_scroll_tree_index_);

        // The top-left hit should immediately hit the top layer's non-fast region.
        {
            auto status = impl->GetInputHandler().ScrollBegin(BeginState(gfx::Point(20, 20), gfx::Vector2dF(0, 1)).get(), ui::ScrollInputType::kTouchscreen);
        ...
        {
            InputHandler::ScrollStatus status
                = impl->GetInputHandler().ScrollBegin(BeginState(gfx::Point(80, 20), gfx::Vector2dF(0, 1)).get(), ui::ScrollInputType::kTouchscreen);
            EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
            EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);
        ...
        {
            InputHandler::ScrollStatus status
                = impl->GetInputHandler().ScrollBegin(BeginState(gfx::Point(80, 80), gfx::Vector2dF(0, 1)).get(), ui::ScrollInputType::kTouchscreen);
...
public:
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        const TransformTree& transform_tree = impl->active_tree()->property_trees()->transform_tree();
        ASSERT_FALSE(transform_tree.needs_update());

        // Perform a scroll over our FakePictureLayer.
        {
            InputHandler::ScrollStatus status
                = impl->GetInputHandler().ScrollBegin(BeginState(gfx::Point(0, 0), gfx::Vector2dF(0, 10)).get(), ui::ScrollInputType::kTouchscreen);

            ASSERT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
            ASSERT_EQ(scroll_tree_index_, impl->CurrentlyScrollingNode()->id);

            impl->GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, 10)));
            impl->GetInputHandler().ScrollEnd();
        }
...
//
class LayerTreeHostScrollTestViewportAbortedCommit : public LayerTreeHostScrollTest {
public:
    ...
    {
        if (is_first_frame_) {
            host_impl->browser_controls_manager()->UpdateBrowserControlsState(BrowserControlsState::kHidden, BrowserControlsState::kHidden, true, std::nullopt);
            bool changed_since_last_sync = false;
            BrowserControlsState permitted_constraint = host_impl->browser_controls_manager()->PullConstraintForMainThread(&changed_since_last_sync);
            EXPECT_EQ(BrowserControlsState::kHidden, permitted_constraint);
            EXPECT_TRUE(changed_since_last_sync);
        }
    }

    void BeginMainFrameAbortedOnThread(LayerTreeHostImpl* host_impl, CommitEarlyOutReason reason, bool scroll_and_viewport_changes_synced) override
    {
...
public:
    PreventRecreatingTilingDuringScroll()
        : initial_scale_(2.0f, 1.0f)
        , new_scale_(2.0f, 2.0f)
    {
    }
    ...
    void WillCommit(const CommitState&) override
    {
        ...
        case 0:
            ...
            transform_tree.OnTransformAnimated(child_layer_->element_id(), transform);
            break;
        case 1:
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* host_impl) override
    {
        if (host_impl->active_tree()->source_frame_number() == 1) {
            FakePictureLayerImpl* target_layer = static_cast<FakePictureLayerImpl*>(host_impl->active_tree()->LayerById(child_layer_id_));
            gfx::AxisTransform2d tiling_transform = target_layer->HighResTiling()->raster_transform();

            if (host_impl->GetActivelyScrollingType() != ActivelyScrollingType::kNone) {
    ...
    void DidActivateTreeOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        case 0:
            // start scroll
            host_impl->GetInputHandler().ScrollBegin(BeginState(gfx::Point(0, 0), gfx::Vector2dF(0, 10)).get(), ui::ScrollInputType::kTouchscreen);
            host_impl->GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, 10)));
...
public:
    ...
    void WillBeginMainFrame() override
    {
        ...
        case 1:
            ...
            layer_tree_host()->SetNeedsCommit();
            layer_tree_host()->StopDeferringCommits(PaintHoldingCommitTrigger::kTimeoutFCP);
            break;
        case 2:
