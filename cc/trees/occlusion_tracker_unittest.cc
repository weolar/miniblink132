...
public:
    ...
    SimpleEnclosedRegion VisibleOpaqueRegion() const override
    {
        if (override_opaque_contents_rect_) {
            return SimpleEnclosedRegion(gfx::IntersectRects(opaque_contents_rect_, visible_layer_rect()));
        }
...
public:
    ...
    bool OccludedLayer(const LayerImpl* layer, const gfx::Rect& content_rect) const
    {
        DCHECK(layer->visible_layer_rect().Contains(content_rect));
        return this->GetCurrentOcclusionForLayer(layer->DrawTransform()).IsOccluded(content_rect);
    }

    // Gives an unoccluded sub-rect of |content_rect| in the content space of the
    // layer. Simple wrapper around GetUnoccludedContentRect.
    gfx::Rect UnoccludedLayerContentRect(const LayerImpl* layer, const gfx::Rect& content_rect) const
    {
        DCHECK(layer->visible_layer_rect().Contains(content_rect));
        return this->GetCurrentOcclusionForLayer(layer->DrawTransform()).GetUnoccludedContentRect(content_rect);
    }

    gfx::Rect UnoccludedSurfaceContentRect(const LayerImpl* layer, const gfx::Rect& content_rect) const
    {
        const RenderSurfaceImpl* surface = GetRenderSurface(const_cast<LayerImpl*>(layer));
        return this->GetCurrentOcclusionForContributingSurface(surface->draw_transform()).GetUnoccludedContentRect(content_rect);
    }
};
...
protected:
    explicit OcclusionTrackerTest(bool opaque_layers)
        ...
        , host_(FakeLayerTreeHost::Create(&client_, &task_graph_runner_, animation_host_.get(), CommitToPendingTreeLayerListSettings()))
        , next_layer_impl_id_(1)
    {
        scoped_feature_list_.InitAndDisableFeature(features::kAllowUndamagedNonrootRenderPassToSkip);
    ...
    TestContentLayerImpl* CreateRoot(const gfx::Size& bounds)
    {
        ...
        std::unique_ptr<TestContentLayerImpl> layer(new TestContentLayerImpl(tree, id));
    ...
    LayerImpl* CreateLayer(LayerImpl* property_parent, const gfx::Transform& transform, const gfx::PointF& position, const gfx::Size& bounds)
    {
        LayerTreeImpl* tree = host_->host_impl()->active_tree();
        int id = next_layer_impl_id_++;
    ...
    void EnsureTransformNode(LayerImpl* layer)
    {
        if (!layer->has_transform_node()) {
            CreateTransformNode(layer).post_translation = layer->offset_to_transform_parent();
            layer->SetOffsetToTransformParent(gfx::Vector2dF());
        }
    }

    LayerImpl* CreateSurface(LayerImpl* parent, const gfx::Transform& transform, const gfx::PointF& position, const gfx::Size& bounds)
    {
    ...
    TestContentLayerImpl* CreateDrawingLayer(
        LayerImpl* property_parent, const gfx::Transform& transform, const gfx::PointF& position, const gfx::Size& bounds, bool opaque)
    {
        LayerTreeImpl* tree = host_->host_impl()->active_tree();
        int id = next_layer_impl_id_++;
        std::unique_ptr<TestContentLayerImpl> layer(new TestContentLayerImpl(tree, id));
    ...
    TestContentLayerImpl* CreateDrawingSurface(
        LayerImpl* property_parent, const gfx::Transform& transform, const gfx::PointF& position, const gfx::Size& bounds, bool opaque)
    {
        TestContentLayerImpl* layer = CreateDrawingLayer(property_parent, transform, position, bounds, opaque);
        EnsureTransformNode(layer);
        CreateEffectNode(layer).render_surface_reason = RenderSurfaceReason::kTest;
        return layer;
    }
    ...
    LayerImpl* CreateCopyLayer(LayerImpl* parent, const gfx::Transform& transform, const gfx::PointF& position, const gfx::Size& bounds)
    {
        ...
        effect_tree.AddCopyRequest(effect_node->id, viz::CopyOutputRequest::CreateStubForTesting());
...
#define ASSERT_EQ_WITH_IDS(a, b) ASSERT_EQ(a, b) << " ids: " << (a)->id() << " vs " << (b)->id()

    void EnterLayer(LayerImpl* layer, OcclusionTracker* occlusion)
    {
        ASSERT_EQ(EffectTreeLayerListIterator::State::kLayer, layer_iterator_->state());
    ...
    {
        ASSERT_EQ(EffectTreeLayerListIterator::State::kTargetSurface, layer_iterator_->state());
        ASSERT_EQ_WITH_IDS(GetRenderSurface(layer), layer_iterator_->target_render_surface());
        occlusion->EnterLayer(*layer_iterator_);
        occlusion->LeaveLayer(*layer_iterator_);
        ++(*layer_iterator_);
        ASSERT_EQ(EffectTreeLayerListIterator::State::kContributingSurface, layer_iterator_->state());
        occlusion->EnterLayer(*layer_iterator_);
    }
    ...
    {
        ASSERT_EQ(EffectTreeLayerListIterator::State::kContributingSurface, layer_iterator_->state());
        ASSERT_EQ_WITH_IDS(GetRenderSurface(layer), layer_iterator_->current_render_surface());
        occlusion->LeaveLayer(*layer_iterator_);
        ++(*layer_iterator_);
    }
    ...
    {
        *layer_iterator_ = EffectTreeLayerListIterator(host_->host_impl()->active_tree());
    }
...
private:
    ...
    void SetProperties(
        LayerImpl* layer, LayerImpl* property_parent, const gfx::Transform& transform, const gfx::PointF& offset_to_property_parent, const gfx::Size& bounds)
    {
        layer->SetElementId(LayerIdToElementIdForTesting(layer->id()));
        layer->SetBounds(bounds);
        CopyProperties(property_parent, layer);
        gfx::Vector2dF offset_to_transform_parent = property_parent->offset_to_transform_parent() + offset_to_property_parent.OffsetFromOrigin();
...
#define RUN_TEST_IMPL_THREAD_OPAQUE_LAYERS(ClassName)                                                                                                          \
    ...
    TEST_F(ClassName##ImplThreadOpaqueLayers, RunTest)                                                                                                         \
    {                                                                                                                                                          \
        RunMyTest();                                                                                                                                           \
    }
#define RUN_TEST_IMPL_THREAD_OPAQUE_PAINTS(ClassName)                                                                                                          \
    ...
    TEST_F(ClassName##ImplThreadOpaquePaints, RunTest)                                                                                                         \
    {                                                                                                                                                          \
        RunMyTest();                                                                                                                                           \
    }
...
protected:
    ...
    void RunMyTest() override
    {
        ...
        TestContentLayerImpl* parent = this->CreateDrawingLayer(root, this->identity_matrix, gfx::PointF(), gfx::Size(100, 100), true);
        CreateClipNode(parent);
        TestContentLayerImpl* layer = this->CreateDrawingLayer(parent, this->identity_matrix, gfx::PointF(30.f, 30.f), gfx::Size(500, 500), true);
        this->CalcDrawEtc();
...
protected:
    ...
    void RunMyTest() override
    {
        ...
        TestContentLayerImpl* parent = this->CreateDrawingLayer(root, this->identity_matrix, gfx::PointF(), gfx::Size(100, 100), true);
        CreateClipNode(parent);
        TestContentLayerImpl* layer = this->CreateDrawingLayer(parent, layer_transform, gfx::PointF(30.f, 30.f), gfx::Size(500, 500), true);
        this->CalcDrawEtc();
...
protected:
    ...
    void RunMyTest() override
    {
        ...
        CreateClipNode(parent);
        LayerImpl* child = this->CreateSurface(parent, child_transform, gfx::PointF(30.f, 30.f), gfx::Size(500, 500));
        CreateClipNode(child);
        TestContentLayerImpl* layer = this->CreateDrawingLayer(child, this->identity_matrix, gfx::PointF(10.f, 10.f), gfx::Size(500, 500), true);
        this->CalcDrawEtc();
...
protected:
    ...
    void RunMyTest() override
    {
        ...
        TestContentLayerImpl* layer1 = this->CreateDrawingSurface(parent, layer1_matrix, gfx::PointF(), gfx::Size(100, 100), true);

        gfx::Transform layer2_matrix;
        layer2_matrix.Translate(25.0, 25.0);
        TestContentLayerImpl* layer2 = this->CreateDrawingLayer(layer1, layer2_matrix, gfx::PointF(), gfx::Size(50, 50), true);
        TestContentLayerImpl* occluder = this->CreateDrawingLayer(parent, this->identity_matrix, gfx::PointF(100.f, 100.f), gfx::Size(500, 500), true);
        this->CalcDrawEtc();
...
protected:
    ...
    {
        TestContentLayerImpl* root = this->CreateRoot(gfx::Size(200, 200));
        LayerImpl* surface = this->CreateSurface(root, this->identity_matrix, gfx::PointF(30.f, 30.f), gfx::Size());
        TestContentLayerImpl* surface_child = this->CreateDrawingLayer(surface, this->identity_matrix, gfx::PointF(10.f, 10.f), gfx::Size(50, 50), true);
        ...
        TestContentLayerImpl* top_layer = this->CreateDrawingLayer(root, this->identity_matrix, gfx::PointF(40.f, 90.f), gfx::Size(50, 20), true);
        this->CalcDrawEtc();
        ...
        EXPECT_EQ(gfx::Rect().ToString(), occlusion.occlusion_from_outside_target().ToString());
        EXPECT_EQ(gfx::Rect(40, 90, 50, 20).ToString(), occlusion.occlusion_from_inside_target().ToString());

        ASSERT_NO_FATAL_FAILURE(this->VisitLayer(surface_child, &occlusion));

        EXPECT_EQ(gfx::Rect(10, 60, 50, 20).ToString(), occlusion.occlusion_from_outside_target().ToString());
        EXPECT_EQ(gfx::Rect(10, 10, 50, 50).ToString(), occlusion.occlusion_from_inside_target().ToString());

        ASSERT_NO_FATAL_FAILURE(this->EnterContributingSurface(surface, &occlusion));
        ...
        ASSERT_NO_FATAL_FAILURE(this->LeaveContributingSurface(surface, &occlusion));
        ASSERT_NO_FATAL_FAILURE(this->EnterLayer(root, &occlusion));

        EXPECT_TRUE(occlusion.occlusion_from_outside_target().IsEmpty());
        EXPECT_EQ(gfx::Rect(40, 40, 50, 70).ToString(), occlusion.occlusion_from_inside_target().ToString());
    }
};
...
protected:
    ...
    void RunMyTest() override
    {
        ...
        TestContentLayerImpl* parent = this->CreateDrawingLayer(root, this->identity_matrix, gfx::PointF(), gfx::Size(100, 100), true);
        LayerImpl* child = this->CreateSurface(parent, child_transform, gfx::PointF(30.f, 30.f), gfx::Size(500, 500));
        TestContentLayerImpl* layer = this->CreateDrawingLayer(child, layer_transform, gfx::PointF(), gfx::Size(500, 500), true);
        this->CalcDrawEtc();

        TestOcclusionTrackerWithClip occlusion(gfx::Rect(0, 0, 1000, 1000));

        gfx::Rect clipped_layer_in_child = MathUtil::MapEnclosingClippedRect(layer_transform, layer->visible_layer_rect());

        ASSERT_NO_FATAL_FAILURE(this->VisitLayer(layer, &occlusion));
        ASSERT_NO_FATAL_FAILURE(this->EnterContributingSurface(child, &occlusion));

        EXPECT_EQ(gfx::Rect().ToString(), occlusion.occlusion_from_outside_target().ToString());
        EXPECT_EQ(clipped_layer_in_child.ToString(), occlusion.occlusion_from_inside_target().ToString());
...
class OcclusionTrackerTestSurfaceWithTwoOpaqueChildren : public OcclusionTrackerTest {
protected:
    ...
    void RunMyTest() override
    {
        ...
        TestContentLayerImpl* parent = this->CreateDrawingLayer(root, this->identity_matrix, gfx::PointF(), gfx::Size(100, 100), true);
        CreateClipNode(parent);
        TestContentLayerImpl* child = this->CreateDrawingSurface(parent, child_transform, gfx::PointF(30.f, 30.f), gfx::Size(500, 500), false);
        CreateClipNode(child);
        TestContentLayerImpl* layer1 = this->CreateDrawingLayer(child, this->identity_matrix, gfx::PointF(10.f, 10.f), gfx::Size(500, 500), true);
        TestContentLayerImpl* layer2 = this->CreateDrawingLayer(child, this->identity_matrix, gfx::PointF(10.f, 450.f), gfx::Size(500, 60), true);
        this->CalcDrawEtc();
...
class OcclusionTrackerTestOverlappingSurfaceSiblings : public OcclusionTrackerTest {
protected:
    ...
    void RunMyTest() override
    {
        TestContentLayerImpl* parent = this->CreateRoot(gfx::Size(100, 100));
        CreateClipNode(parent);
        LayerImpl* child1 = this->CreateSurface(parent, this->identity_matrix, gfx::PointF(10.f, 0.f), gfx::Size());
        LayerImpl* child2 = this->CreateSurface(parent, this->identity_matrix, gfx::PointF(30.f, 0.f), gfx::Size());
        TestContentLayerImpl* layer1 = this->CreateDrawingLayer(child1, this->identity_matrix, gfx::PointF(), gfx::Size(40, 50), true);
        TestContentLayerImpl* layer2 = this->CreateDrawingLayer(child2, this->identity_matrix, gfx::PointF(10.f, 0.f), gfx::Size(40, 50), true);
        this->CalcDrawEtc();
        ...
        // layer2's occlusion.
        EXPECT_EQ(gfx::Rect().ToString(), occlusion.occlusion_from_outside_target().ToString());
        EXPECT_EQ(gfx::Rect(10, 0, 40, 50).ToString(), occlusion.occlusion_from_inside_target().ToString());
        ...
        EXPECT_EQ(gfx::Rect(30, 0, 40, 50).ToString(), occlusion.occlusion_from_outside_target().ToString());
        // layer1's occlusion.
        EXPECT_EQ(gfx::Rect(0, 0, 40, 50).ToString(), occlusion.occlusion_from_inside_target().ToString());
        ...
        EXPECT_TRUE(occlusion.occlusion_from_outside_target().IsEmpty());
        EXPECT_EQ(gfx::Rect(10, 0, 70, 50).ToString(), occlusion.occlusion_from_inside_target().ToString());
    }
};

ALL_OCCLUSIONTRACKER_TEST(OcclusionTrackerTestOverlappingSurfaceSiblings)

class OcclusionTrackerTestOverlappingSurfaceSiblingsWithTwoTransforms : public OcclusionTrackerTest {
protected:
    explicit OcclusionTrackerTestOverlappingSurfaceSiblingsWithTwoTransforms(bool opaque_layers)
        : OcclusionTrackerTest(opaque_layers)
    {
    }
    void RunMyTest() override
    {
        ...
        TestContentLayerImpl* parent = this->CreateRoot(gfx::Size(100, 100));
        CreateClipNode(parent);
        LayerImpl* child1 = this->CreateSurface(parent, child1_transform, gfx::PointF(30.f, 20.f), gfx::Size(10, 10));
        TestContentLayerImpl* layer1 = this->CreateDrawingLayer(child1, this->identity_matrix, gfx::PointF(-10.f, -20.f), gfx::Size(510, 510), true);
        LayerImpl* child2 = this->CreateDrawingSurface(parent, child2_transform, gfx::PointF(20.f, 40.f), gfx::Size(10, 10), false);
        TestContentLayerImpl* layer2 = this->CreateDrawingLayer(child2, this->identity_matrix, gfx::PointF(-10.f, -10.f), gfx::Size(510, 510), true);
        this->CalcDrawEtc();
...
protected:
    ...
    void RunMyTest() override
    {
        ...
        TestContentLayerImpl* parent = this->CreateRoot(gfx::Size(100, 100));
        CreateClipNode(parent);
        TestContentLayerImpl* blur_layer = this->CreateDrawingSurface(parent, layer_transform, gfx::PointF(30.f, 30.f), gfx::Size(500, 500), true);
        TestContentLayerImpl* opaque_layer = this->CreateDrawingSurface(parent, layer_transform, gfx::PointF(30.f, 30.f), gfx::Size(500, 500), true);
        TestContentLayerImpl* opacity_layer = this->CreateDrawingSurface(parent, layer_transform, gfx::PointF(30.f, 30.f), gfx::Size(500, 500), true);

        gfx::Transform rounded_corner_transform;
        TestContentLayerImpl* rounded_corner_layer
            = this->CreateDrawingLayer(parent, rounded_corner_transform, gfx::PointF(30.f, 30.f), gfx::Size(500, 500), true);
        ...
        CreateEffectNode(rounded_corner_layer).mask_filter_info = gfx::MaskFilterInfo(gfx::RRectF(1, 2, 3, 4, 5, 6));
        ...
        ASSERT_NO_FATAL_FAILURE(this->EnterContributingSurface(opacity_layer, &occlusion));

        EXPECT_TRUE(occlusion.occlusion_from_outside_target().IsEmpty());
        EXPECT_TRUE(occlusion.occlusion_from_inside_target().IsEmpty());

        // And has nothing to contribute to its parent surface.
        ASSERT_NO_FATAL_FAILURE(this->LeaveContributingSurface(opacity_layer, &occlusion));
        EXPECT_TRUE(occlusion.occlusion_from_outside_target().IsEmpty());
        EXPECT_TRUE(occlusion.occlusion_from_inside_target().IsEmpty());

        // Opaque layer will contribute to occlusion.
        ASSERT_NO_FATAL_FAILURE(this->VisitLayer(opaque_layer, &occlusion));
        ASSERT_NO_FATAL_FAILURE(this->EnterContributingSurface(opaque_layer, &occlusion));
        ...
        ASSERT_NO_FATAL_FAILURE(this->LeaveContributingSurface(opaque_layer, &occlusion));
        EXPECT_TRUE(occlusion.occlusion_from_outside_target().IsEmpty());
        EXPECT_EQ(gfx::Rect(30, 30, 70, 70).ToString(), occlusion.occlusion_from_inside_target().ToString());
        ...
        ASSERT_NO_FATAL_FAILURE(this->EnterContributingSurface(blur_layer, &occlusion));
        EXPECT_TRUE(occlusion.occlusion_from_outside_target().IsEmpty());
        EXPECT_TRUE(occlusion.occlusion_from_inside_target().IsEmpty());

        // But the opaque layer's occlusion is preserved on the parent.
        ASSERT_NO_FATAL_FAILURE(this->LeaveContributingSurface(blur_layer, &occlusion));
        ASSERT_NO_FATAL_FAILURE(this->EnterLayer(parent, &occlusion));
        EXPECT_TRUE(occlusion.occlusion_from_outside_target().IsEmpty());
        EXPECT_EQ(gfx::Rect(30, 30, 70, 70).ToString(), occlusion.occlusion_from_inside_target().ToString());
    }
...
class OcclusionTrackerTestFiltersRenderSurfaceOcclusion : public OcclusionTrackerTest {
protected:
    ...
    void RunMyTest() override
    {
        ...
        TestContentLayerImpl* blur_layer = CreateDrawingSurface(parent, gfx::Transform(), gfx::PointF(100.f, 100.f), gfx::Size(50, 50), true);
        TestContentLayerImpl* opacity_layer = CreateDrawingSurface(parent, gfx::Transform(), gfx::PointF(200.f, 100.f), gfx::Size(50, 50), true);

        // This layer fully covers the layer bounds of the above filtered layers,
        // but not the blur filter extent of |blur_layer|.
        TestContentLayerImpl* occluding_layer = CreateDrawingLayer(parent, gfx::Transform(), gfx::PointF(100.f, 100.f), gfx::Size(300, 100), true);
        ...
        ASSERT_NO_FATAL_FAILURE(EnterContributingSurface(opacity_layer, &occlusion));
        EXPECT_EQ(gfx::Rect(), occlusion.UnoccludedSurfaceContentRect(opacity_layer, gfx::Rect(opacity_layer->bounds())));
        ASSERT_NO_FATAL_FAILURE(LeaveContributingSurface(opacity_layer, &occlusion));
        ...
        EXPECT_EQ(gfx::Rect(blur_layer->bounds()), occlusion.UnoccludedSurfaceContentRect(blur_layer, gfx::Rect(blur_layer->bounds())));
...
class OcclusionTrackerTestOpaqueContentsRegionEmpty : public OcclusionTrackerTest {
protected:
    ...
    void RunMyTest() override
    {
        ...
        TestContentLayerImpl* layer = this->CreateDrawingSurface(parent, this->identity_matrix, gfx::PointF(), gfx::Size(200, 200), false);
        this->CalcDrawEtc();
...
class OcclusionTrackerTestOpaqueContentsRegionNonEmpty : public OcclusionTrackerTest {
protected:
    ...
    {
        TestContentLayerImpl* parent = this->CreateRoot(gfx::Size(300, 300));
        TestContentLayerImpl* layer = this->CreateDrawingLayer(parent, this->identity_matrix, gfx::PointF(100.f, 100.f), gfx::Size(200, 200), false);
        this->CalcDrawEtc();
        {
            TestOcclusionTrackerWithClip occlusion(gfx::Rect(0, 0, 1000, 1000));
...
class OcclusionTrackerTestLayerBehindCameraDoesNotOcclude : public OcclusionTrackerTest {
protected:
    explicit OcclusionTrackerTestLayerBehindCameraDoesNotOcclude(bool opaque_layers)
        : OcclusionTrackerTest(opaque_layers)
    {
    }
    void RunMyTest() override
    {
        ...
        TestContentLayerImpl* parent = this->CreateRoot(gfx::Size(100, 100));
        TestContentLayerImpl* layer = this->CreateDrawingLayer(parent, transform, gfx::PointF(), gfx::Size(100, 100), true);
...
class OcclusionTrackerTestSurfaceOcclusionTranslatesToParent : public OcclusionTrackerTest {
protected:
    explicit OcclusionTrackerTestSurfaceOcclusionTranslatesToParent(bool opaque_layers)
        : OcclusionTrackerTest(opaque_layers)
    {
    }
    void RunMyTest() override
    {
        ...
        TestContentLayerImpl* surface = this->CreateDrawingSurface(parent, surface_transform, gfx::PointF(), gfx::Size(300, 300), false);
        TestContentLayerImpl* surface2 = this->CreateDrawingSurface(parent, this->identity_matrix, gfx::PointF(50.f, 50.f), gfx::Size(300, 300), false);
        ...
        ASSERT_NO_FATAL_FAILURE(this->VisitContributingSurface(surface2, &occlusion));
        ...
        ASSERT_NO_FATAL_FAILURE(this->VisitContributingSurface(surface, &occlusion));
...
ALL_OCCLUSIONTRACKER_TEST(OcclusionTrackerTestSurfaceOcclusionTranslatesToParent)

class OcclusionTrackerTestSurfaceOcclusionTranslatesWithClipping : public OcclusionTrackerTest {
protected:
    explicit OcclusionTrackerTestSurfaceOcclusionTranslatesWithClipping(bool opaque_layers)
        : OcclusionTrackerTest(opaque_layers)
    {
    }
    void RunMyTest() override
    {
        TestContentLayerImpl* parent = this->CreateRoot(gfx::Size(300, 300));
        CreateClipNode(parent);
        TestContentLayerImpl* surface = this->CreateDrawingSurface(parent, this->identity_matrix, gfx::PointF(), gfx::Size(500, 300), false);
...
ALL_OCCLUSIONTRACKER_TEST(OcclusionTrackerTestSurfaceOcclusionTranslatesWithClipping)
...
protected:
    ...
    void RunMyTest() override
    {
        ...
        LayerImpl* surface = this->CreateDrawingSurface(parent, this->identity_matrix, gfx::PointF(), gfx::Size(100, 100), false);
        LayerImpl* surface_child = this->CreateDrawingSurface(surface, this->identity_matrix, gfx::PointF(0.f, 10.f), gfx::Size(100, 50), true);
        LayerImpl* topmost = this->CreateDrawingLayer(parent, this->identity_matrix, gfx::PointF(), gfx::Size(100, 50), true);
        this->CalcDrawEtc();
        ...
        ASSERT_NO_FATAL_FAILURE(this->EnterContributingSurface(surface_child, &occlusion));
        // The |surface_child| can't occlude its own surface, but occlusion from
        // |topmost| can.
        EXPECT_EQ(gfx::Rect().ToString(), occlusion.occlusion_on_contributing_surface_from_outside_target().ToString());
        EXPECT_EQ(gfx::Rect(0, 0, 100, 50).ToString(), occlusion.occlusion_on_contributing_surface_from_inside_target().ToString());
        ASSERT_NO_FATAL_FAILURE(this->LeaveContributingSurface(surface_child, &occlusion));
        ...
        EXPECT_EQ(gfx::Rect().ToString(), occlusion.occlusion_from_outside_target().ToString());
        EXPECT_EQ(gfx::Rect(0, 0, 100, 60).ToString(), occlusion.occlusion_from_inside_target().ToString());
    }
};

ALL_OCCLUSIONTRACKER_TEST(OcclusionTrackerTestSurfaceChildOfSurface)

class OcclusionTrackerTestDontOccludePixelsNeededForBackdropFilter : public OcclusionTrackerTest {
protected:
    explicit OcclusionTrackerTestDontOccludePixelsNeededForBackdropFilter(bool opaque_layers)
        : OcclusionTrackerTest(opaque_layers)
    {
    }
    void RunMyTest() override
    {
        ...
        for (int i = 0; i <= LAST_DIRECTION; ++i) {
            ...
            TestContentLayerImpl* parent = this->CreateRoot(gfx::Size(200, 200));
            LayerImpl* filtered_surface = this->CreateDrawingSurface(parent, scale_by_half, gfx::PointF(50.f, 50.f), gfx::Size(100, 100), false);
            ...
            LayerImpl* occluding_layer
                = this->CreateDrawingLayer(parent, this->identity_matrix, gfx::PointF(occlusion_rect.origin()), occlusion_rect.size(), true);
            this->CalcDrawEtc();
            ...
            EXPECT_EQ(occlusion_rect.ToString(), occlusion.occlusion_from_inside_target().ToString());
            EXPECT_TRUE(occlusion.occlusion_from_outside_target().IsEmpty());
            ...
            EXPECT_TRUE(occlusion.occlusion_from_inside_target().IsEmpty());
            EXPECT_EQ(occlusion_inside_surface.ToString(), occlusion.occlusion_from_outside_target().ToString());
            ...
            ASSERT_NO_FATAL_FAILURE(this->VisitContributingSurface(filtered_surface, &occlusion));
            ASSERT_NO_FATAL_FAILURE(this->EnterLayer(parent, &occlusion));
            ...
            EXPECT_EQ(expected_occlusion.ToString(), occlusion.occlusion_from_inside_target().ToString());
            EXPECT_TRUE(occlusion.occlusion_from_outside_target().IsEmpty());
...
ALL_OCCLUSIONTRACKER_TEST(OcclusionTrackerTestDontOccludePixelsNeededForBackdropFilter)

class OcclusionTrackerTestPixelsNeededForDropShadowBackdropFilter : public OcclusionTrackerTest {
protected:
    explicit OcclusionTrackerTestPixelsNeededForDropShadowBackdropFilter(bool opaque_layers)
        : OcclusionTrackerTest(opaque_layers)
    {
    }
    void RunMyTest() override
    {
        ...
        FilterOperations filters;
        filters.Append(FilterOperation::CreateDropShadowFilter(gfx::Point(10, 10), 5, SkColors::kBlack));
...
ALL_OCCLUSIONTRACKER_TEST(OcclusionTrackerTestPixelsNeededForDropShadowBackdropFilter)

class OcclusionTrackerTestTwoBackdropFiltersReduceOcclusionTwice : public OcclusionTrackerTest {
protected:
    explicit OcclusionTrackerTestTwoBackdropFiltersReduceOcclusionTwice(bool opaque_layers)
        : OcclusionTrackerTest(opaque_layers)
    {
    }
    void RunMyTest() override
    {
        ...
        LayerImpl* parent = this->CreateSurface(root, scale_by_half, gfx::PointF(), gfx::Size(150, 150));
        CreateClipNode(parent);
        LayerImpl* filtered_surface1 = this->CreateDrawingSurface(parent, scale_by_half, gfx::PointF(), gfx::Size(300, 300), false);
        LayerImpl* filtered_surface2 = this->CreateDrawingSurface(parent, scale_by_half, gfx::PointF(), gfx::Size(300, 300), false);
        LayerImpl* occluding_layer_above = this->CreateDrawingLayer(parent, this->identity_matrix, gfx::PointF(100.f, 100.f), gfx::Size(50, 50), true);
        ...
        ASSERT_NO_FATAL_FAILURE(this->VisitLayer(occluding_layer_above, &occlusion));
        EXPECT_EQ(gfx::Rect().ToString(), occlusion.occlusion_from_outside_target().ToString());
        ...
        ASSERT_NO_FATAL_FAILURE(this->VisitContributingSurface(filtered_surface2, &occlusion));
        ASSERT_NO_FATAL_FAILURE(this->VisitLayer(filtered_surface1, &occlusion));
        ASSERT_NO_FATAL_FAILURE(this->VisitContributingSurface(filtered_surface1, &occlusion));

        // Test expectations in the target.
        int blur_outset = 3;
        gfx::Rect expected_occlusion = gfx::Rect(100 / 2 + blur_outset * 2, 100 / 2 + blur_outset * 2, 50 / 2 - blur_outset * 4, 50 / 2 - blur_outset * 4);
        EXPECT_EQ(expected_occlusion.ToString(), occlusion.occlusion_from_inside_target().ToString());

        // Test expectations in the screen are the same as in the target, as the
        // render surface is 1:1 with the screen.
        EXPECT_EQ(expected_occlusion.ToString(), occlusion.occlusion_from_outside_target().ToString());
    }
};

ALL_OCCLUSIONTRACKER_TEST(OcclusionTrackerTestTwoBackdropFiltersReduceOcclusionTwice)

class OcclusionTrackerTestDontReduceOcclusionBelowBackdropFilter : public OcclusionTrackerTest {
protected:
    explicit OcclusionTrackerTestDontReduceOcclusionBelowBackdropFilter(bool opaque_layers)
        : OcclusionTrackerTest(opaque_layers)
    {
    }
    void RunMyTest() override
    {
        ...
        LayerImpl* behind_surface_layer = this->CreateDrawingLayer(parent, this->identity_matrix, gfx::PointF(60.f, 60.f), gfx::Size(30, 30), true);
        LayerImpl* filtered_surface = this->CreateDrawingSurface(parent, scale_by_half, gfx::PointF(50.f, 50.f), gfx::Size(100, 100), false);
        ...
        EXPECT_EQ(occlusion_behind_surface.ToString(), occlusion.occlusion_from_inside_target().ToString());
        EXPECT_TRUE(occlusion.occlusion_from_outside_target().IsEmpty());
    }
};

ALL_OCCLUSIONTRACKER_TEST(OcclusionTrackerTestDontReduceOcclusionBelowBackdropFilter)

class OcclusionTrackerTestDontReduceOcclusionIfBackdropFilterIsOccluded : public OcclusionTrackerTest {
protected:
    explicit OcclusionTrackerTestDontReduceOcclusionIfBackdropFilterIsOccluded(bool opaque_layers)
        : OcclusionTrackerTest(opaque_layers)
    {
    }
    void RunMyTest() override
    {
        ...
        LayerImpl* filtered_surface = this->CreateDrawingSurface(parent, scale_by_half, gfx::PointF(50.f, 50.f), gfx::Size(100, 100), false);
        LayerImpl* occluding_layer = this->CreateDrawingLayer(parent, this->identity_matrix, gfx::PointF(50.f, 50.f), gfx::Size(50, 50), true);
        ...
        ASSERT_NO_FATAL_FAILURE(this->VisitLayer(filtered_surface, &occlusion));
        {
            ...
            EXPECT_EQ(gfx::Rect().ToString(), occlusion.occlusion_from_inside_target().ToString());
            EXPECT_EQ(occlusion_above_surface.ToString(), occlusion.occlusion_from_outside_target().ToString());
        }
        ...
        ASSERT_NO_FATAL_FAILURE(this->VisitContributingSurface(filtered_surface, &occlusion));
        {
            ...
            EXPECT_EQ(occlusion_above_surface.ToString(), occlusion.occlusion_from_inside_target().ToString());
            EXPECT_EQ(gfx::Rect().ToString(), occlusion.occlusion_from_outside_target().ToString());
        }
    }
};

ALL_OCCLUSIONTRACKER_TEST(OcclusionTrackerTestDontReduceOcclusionIfBackdropFilterIsOccluded)

class OcclusionTrackerTestReduceOcclusionWhenBkgdFilterIsPartiallyOccluded : public OcclusionTrackerTest {
protected:
    explicit OcclusionTrackerTestReduceOcclusionWhenBkgdFilterIsPartiallyOccluded(bool opaque_layers)
        : OcclusionTrackerTest(opaque_layers)
    {
    }
    void RunMyTest() override
    {
        ...
        TestContentLayerImpl* parent = this->CreateRoot(gfx::Size(300, 150));
        LayerImpl* filtered_surface = this->CreateDrawingSurface(parent, scale_by_half, gfx::PointF(50.f, 50.f), gfx::Size(100, 100), false);
        LayerImpl* above_surface_layer = this->CreateDrawingLayer(parent, this->identity_matrix, gfx::PointF(70.f, 50.f), gfx::Size(30, 50), true);
        LayerImpl* beside_surface_layer = this->CreateDrawingLayer(parent, this->identity_matrix, gfx::PointF(90.f, 40.f), gfx::Size(10, 10), true);
...
ALL_OCCLUSIONTRACKER_TEST(OcclusionTrackerTestReduceOcclusionWhenBkgdFilterIsPartiallyOccluded)

class OcclusionTrackerTestRenderSurfaceOccludingBlendMode : public OcclusionTrackerTest {
protected:
    explicit OcclusionTrackerTestRenderSurfaceOccludingBlendMode(bool opaque_layers, SkBlendMode blend_mode)
        : OcclusionTrackerTest(opaque_layers)
    ...
    void RunMyTest() override
    {
        ...
        LayerImpl* blend_mode_layer = CreateDrawingSurface(parent, identity_matrix, gfx::PointF(0.f, 0.f), gfx::Size(100, 100), true);
        LayerImpl* top_layer = CreateDrawingLayer(parent, identity_matrix, gfx::PointF(10.f, 12.f), gfx::Size(20, 22), true);

        GetEffectNode(blend_mode_layer)->render_surface_reason = RenderSurfaceReason::kTest;
        ...
        // |top_layer| occludes.
        EXPECT_EQ(gfx::Rect(10, 12, 20, 22).ToString(), occlusion.occlusion_from_inside_target().ToString());
        EXPECT_TRUE(occlusion.occlusion_from_outside_target().IsEmpty());

        ASSERT_NO_FATAL_FAILURE(VisitLayer(blend_mode_layer, &occlusion));
        // |top_layer| and |blend_mode_layer| both occlude.
        EXPECT_EQ(gfx::Rect(100, 100).ToString(), occlusion.occlusion_from_inside_target().ToString());
        EXPECT_EQ(gfx::Rect(10, 12, 20, 22).ToString(), occlusion.occlusion_from_outside_target().ToString());

        ASSERT_NO_FATAL_FAILURE(this->VisitContributingSurface(blend_mode_layer, &occlusion));
...
class OcclusionTrackerTestRenderSurfaceBlendModeSrcOver : public OcclusionTrackerTestRenderSurfaceOccludingBlendMode {
protected:
    explicit OcclusionTrackerTestRenderSurfaceBlendModeSrcOver(bool opaque_layers)
        : OcclusionTrackerTestRenderSurfaceOccludingBlendMode(opaque_layers, SkBlendMode::kSrcOver)
    {
    }
};
...
class OcclusionTrackerTestRenderSurfaceBlendModeSrc : public OcclusionTrackerTestRenderSurfaceOccludingBlendMode {
protected:
    explicit OcclusionTrackerTestRenderSurfaceBlendModeSrc(bool opaque_layers)
        : OcclusionTrackerTestRenderSurfaceOccludingBlendMode(opaque_layers, SkBlendMode::kSrc)
    {
    }
};
...
class OcclusionTrackerTestRenderSurfaceNonOccludingBlendMode : public OcclusionTrackerTest {
protected:
    explicit OcclusionTrackerTestRenderSurfaceNonOccludingBlendMode(bool opaque_layers)
        : OcclusionTrackerTest(opaque_layers)
    {
    }
    void RunMyTest() override
    {
        TestContentLayerImpl* parent = this->CreateRoot(gfx::Size(100, 100));
        LayerImpl* blend_mode_layer = this->CreateDrawingSurface(parent, this->identity_matrix, gfx::PointF(0.f, 0.f), gfx::Size(100, 100), true);
        LayerImpl* top_layer = this->CreateDrawingLayer(parent, this->identity_matrix, gfx::PointF(10.f, 12.f), gfx::Size(20, 22), true);
...
ALL_OCCLUSIONTRACKER_TEST(OcclusionTrackerTestRenderSurfaceNonOccludingBlendMode)
...
class OcclusionTrackerTestNonRenderSurfaceNonOccludingBlendMode : public OcclusionTrackerTest {
protected:
    explicit OcclusionTrackerTestNonRenderSurfaceNonOccludingBlendMode(bool opaque_layers)
        : OcclusionTrackerTest(opaque_layers)
    {
    }
    void RunMyTest() override
    {
        TestContentLayerImpl* parent = CreateRoot(gfx::Size(100, 100));
        LayerImpl* top_layer = CreateDrawingSurface(parent, this->identity_matrix, gfx::PointF(10.f, 12.f), gfx::Size(20, 22), true);
        LayerImpl* blend_mode_layer = CreateDrawingLayer(top_layer, this->identity_matrix, gfx::PointF(0.f, 0.f), gfx::Size(100, 100), true);
...
ALL_OCCLUSIONTRACKER_TEST(OcclusionTrackerTestNonRenderSurfaceNonOccludingBlendMode)
...
protected:
    ...
    void RunMyTest() override
    {
        ...
        LayerImpl* large = this->CreateDrawingLayer(parent, this->identity_matrix, gfx::PointF(), tracking_size, true);
        LayerImpl* small = this->CreateDrawingLayer(parent, this->identity_matrix, gfx::PointF(), below_tracking_size, true);
        this->CalcDrawEtc();
        ...
        EXPECT_EQ(gfx::Rect().ToString(), occlusion.occlusion_from_outside_target().ToString());
        EXPECT_EQ(gfx::Rect(tracking_size).ToString(), occlusion.occlusion_from_inside_target().ToString());
    }
};
...
protected:
    ...
    void RunMyTest() override
    {
        ...
        TestContentLayerImpl* parent = this->CreateRoot(gfx::Size(400, 400));
        LayerImpl* clip = this->CreateLayer(parent, this->identity_matrix, gfx::PointF(10.f, 10.f), gfx::Size(50, 50));
        CreateClipNode(clip);
        LayerImpl* scale = this->CreateLayer(clip, scale_transform, gfx::PointF(), gfx::Size(1, 1));
        LayerImpl* scaled = this->CreateDrawingLayer(scale, this->identity_matrix, gfx::PointF(), gfx::Size(500, 500), true);
        this->CalcDrawEtc();
...
class OcclusionTrackerTestScaledLayerInSurfaceIsClipped : public OcclusionTrackerTest {
protected:
    ...
    void RunMyTest() override
    {
        ...
        LayerImpl* clip = this->CreateLayer(parent, this->identity_matrix, gfx::PointF(10.f, 10.f), gfx::Size(50, 50));
        CreateClipNode(clip);
        LayerImpl* surface = this->CreateDrawingSurface(clip, this->identity_matrix, gfx::PointF(), gfx::Size(400, 30), false);
        LayerImpl* scale = this->CreateLayer(surface, scale_transform, gfx::PointF(), gfx::Size(1, 1));
        LayerImpl* scaled = this->CreateDrawingLayer(scale, this->identity_matrix, gfx::PointF(), gfx::Size(500, 500), true);
        this->CalcDrawEtc();
...
protected:
    ...
    void RunMyTest() override
    {
        ...
        TestContentLayerImpl* parent = this->CreateDrawingLayer(root, this->identity_matrix, gfx::PointF(), gfx::Size(400, 400), true);
        LayerImpl* copy = this->CreateCopyLayer(parent, this->identity_matrix, gfx::PointF(100, 0), gfx::Size(200, 400));
        LayerImpl* copy_child = this->CreateDrawingLayer(copy, this->identity_matrix, gfx::PointF(), gfx::Size(200, 400), true);
        LayerImpl* top_layer = this->CreateDrawingLayer(root, this->identity_matrix, gfx::PointF(50, 0), gfx::Size(50, 400), true);
        this->CalcDrawEtc();
        ...
        ASSERT_NO_FATAL_FAILURE(this->VisitLayer(top_layer, &occlusion));
        EXPECT_EQ(gfx::Rect().ToString(), occlusion.occlusion_from_outside_target().ToString());
        EXPECT_EQ(gfx::Rect(50, 0, 50, 400).ToString(), occlusion.occlusion_from_inside_target().ToString());

        ASSERT_NO_FATAL_FAILURE(this->VisitLayer(copy_child, &occlusion));
        // Layers outside the copy request do not occlude.
        EXPECT_EQ(gfx::Rect().ToString(), occlusion.occlusion_from_outside_target().ToString());
        EXPECT_EQ(gfx::Rect(200, 400).ToString(), occlusion.occlusion_from_inside_target().ToString());
        ...
        EXPECT_EQ(gfx::Rect().ToString(), occlusion.occlusion_from_outside_target().ToString());
        EXPECT_EQ(gfx::Rect(50, 0, 250, 400).ToString(), occlusion.occlusion_from_inside_target().ToString());
    }
};

ALL_OCCLUSIONTRACKER_TEST(OcclusionTrackerTestCopyRequestDoesOcclude)

class OcclusionTrackerTestHiddenCopyRequestDoesNotOcclude : public OcclusionTrackerTest {
protected:
    explicit OcclusionTrackerTestHiddenCopyRequestDoesNotOcclude(bool opaque_layers)
        : OcclusionTrackerTest(opaque_layers)
    {
    }
    void RunMyTest() override
    {
        TestContentLayerImpl* root = this->CreateRoot(gfx::Size(400, 400));
        TestContentLayerImpl* parent = this->CreateDrawingLayer(root, this->identity_matrix, gfx::PointF(), gfx::Size(400, 400), true);
        LayerImpl* hide = this->CreateLayer(parent, this->identity_matrix, gfx::PointF(), gfx::Size());
        // The |copy| layer is hidden but since it is being copied, it will be
        // drawn.
        CreateEffectNode(hide).opacity = 0.f;
        LayerImpl* copy = this->CreateCopyLayer(hide, this->identity_matrix, gfx::PointF(100.f, 0.f), gfx::Size(200, 400));
        LayerImpl* copy_child = this->CreateDrawingLayer(copy, this->identity_matrix, gfx::PointF(), gfx::Size(200, 400), true);
...
protected:
    ...
    void RunMyTest() override
    {
        ...
        TestContentLayerImpl* root = this->CreateRoot(gfx::Size(200, 200));
        LayerImpl* surface = this->CreateSurface(root, this->identity_matrix, gfx::PointF(), gfx::Size(200, 200));
        LayerImpl* layer = this->CreateDrawingLayer(surface, translate, gfx::PointF(), gfx::Size(200, 200), false);
        TestContentLayerImpl* outside_layer = this->CreateDrawingLayer(root, this->identity_matrix, gfx::PointF(), gfx::Size(200, 200), false);
        this->CalcDrawEtc();
        ...
        occlusion.set_occlusion_from_outside_target(SimpleEnclosedRegion(50, 50, 100, 100));
        occlusion.set_occlusion_from_inside_target(SimpleEnclosedRegion());
        ...
        occlusion.set_occlusion_from_outside_target(SimpleEnclosedRegion());
        occlusion.set_occlusion_from_inside_target(SimpleEnclosedRegion(50, 50, 100, 100));
        EXPECT_FALSE(occlusion.OccludedLayer(layer, gfx::Rect(0, 0, 100, 100)));
        EXPECT_FALSE(occlusion.OccludedLayer(layer, gfx::Rect(90, 30, 100, 100)));
        EXPECT_FALSE(occlusion.OccludedLayer(layer, gfx::Rect(40, 0, 100, 100)));
        EXPECT_FALSE(occlusion.OccludedLayer(layer, gfx::Rect(40, 80, 100, 100)));
        ...
        occlusion.set_occlusion_from_outside_target(SimpleEnclosedRegion(50, 50, 100, 50));
        occlusion.set_occlusion_from_inside_target(SimpleEnclosedRegion(50, 100, 100, 50));
        EXPECT_FALSE(occlusion.OccludedLayer(layer, gfx::Rect(0, 0, 100, 100)));
        EXPECT_FALSE(occlusion.OccludedLayer(layer, gfx::Rect(90, 30, 100, 100)));
        EXPECT_FALSE(occlusion.OccludedLayer(layer, gfx::Rect(40, 0, 100, 100)));
        EXPECT_FALSE(occlusion.OccludedLayer(layer, gfx::Rect(40, 80, 100, 100)));
...
protected:
    ...
    void RunMyTest() override
    {
        ...
        occlusion.set_occlusion_from_outside_target(SimpleEnclosedRegion());
        occlusion.set_occlusion_from_inside_target(SimpleEnclosedRegion());
        EXPECT_EQ(gfx::Rect(100, 100), occlusion.UnoccludedLayerContentRect(layer, gfx::Rect(100, 100)));

        // Partial occlusion from outside.
        occlusion.set_occlusion_from_outside_target(SimpleEnclosedRegion(50, 50, 100, 100));
        occlusion.set_occlusion_from_inside_target(SimpleEnclosedRegion());
        EXPECT_EQ(gfx::Rect(0, 0, 100, 100), occlusion.UnoccludedLayerContentRect(layer, gfx::Rect(0, 0, 100, 100)));
        EXPECT_EQ(gfx::Rect(140, 30, 50, 100), occlusion.UnoccludedLayerContentRect(layer, gfx::Rect(90, 30, 100, 100)));
        EXPECT_EQ(gfx::Rect(40, 0, 100, 30), occlusion.UnoccludedLayerContentRect(layer, gfx::Rect(40, 0, 100, 100)));
        EXPECT_EQ(gfx::Rect(40, 130, 100, 50), occlusion.UnoccludedLayerContentRect(layer, gfx::Rect(40, 80, 100, 100)));
        EXPECT_EQ(gfx::Rect(0, 0, 80, 100), occlusion.UnoccludedLayerContentRect(layer, gfx::Rect(0, 0, 80, 100)));
        EXPECT_EQ(gfx::Rect(90, 80, 100, 100), occlusion.UnoccludedLayerContentRect(layer, gfx::Rect(90, 80, 100, 100)));
        EXPECT_EQ(gfx::Rect(0, 80, 100, 100), occlusion.UnoccludedLayerContentRect(layer, gfx::Rect(0, 80, 100, 100)));
        EXPECT_EQ(gfx::Rect(90, 0, 100, 100), occlusion.UnoccludedLayerContentRect(layer, gfx::Rect(90, 0, 100, 100)));

        // Full occlusion from outside, is occluded.
        EXPECT_EQ(gfx::Rect(), occlusion.UnoccludedLayerContentRect(layer, gfx::Rect(40, 30, 100, 100)));
        EXPECT_EQ(gfx::Rect(), occlusion.UnoccludedLayerContentRect(layer, gfx::Rect(40, 30, 10, 10)));
        EXPECT_EQ(gfx::Rect(), occlusion.UnoccludedLayerContentRect(layer, gfx::Rect(130, 120, 10, 10)));
        EXPECT_EQ(gfx::Rect(), occlusion.UnoccludedLayerContentRect(layer, gfx::Rect(80, 70, 50, 50)));
        ...
        // it's larger.
        EXPECT_EQ(gfx::Rect(90, 30, 100, 100), occlusion.UnoccludedLayerContentRect(layer, gfx::Rect(90, 30, 100, 100)));
        EXPECT_EQ(gfx::Rect(40, 0, 100, 30), occlusion.UnoccludedLayerContentRect(layer, gfx::Rect(40, 0, 100, 100)));
        EXPECT_EQ(gfx::Rect(40, 130, 100, 50), occlusion.UnoccludedLayerContentRect(layer, gfx::Rect(40, 80, 100, 100)));
        EXPECT_EQ(gfx::Rect(0, 0, 80, 100), occlusion.UnoccludedLayerContentRect(layer, gfx::Rect(0, 0, 80, 100)));
        EXPECT_EQ(gfx::Rect(90, 80, 100, 100), occlusion.UnoccludedLayerContentRect(layer, gfx::Rect(90, 80, 100, 100)));
...
protected:
    ...
    void RunMyTest() override
    {
        ...
        translate.Translate(10.0, 20.0);
        TestContentLayerImpl* root = this->CreateRoot(gfx::Size(200, 200));
        LayerImpl* surface = this->CreateSurface(root, translate, gfx::PointF(), gfx::Size(200, 200));
        LayerImpl* layer = this->CreateDrawingLayer(surface, this->identity_matrix, gfx::PointF(), gfx::Size(200, 200), false);
        TestContentLayerImpl* outside_layer = this->CreateDrawingLayer(root, this->identity_matrix, gfx::PointF(), gfx::Size(200, 200), false);
        this->CalcDrawEtc();
        ...
        // No occlusion, is not occluded.
        occlusion.set_occlusion_on_contributing_surface_from_outside_target(SimpleEnclosedRegion());
        occlusion.set_occlusion_on_contributing_surface_from_inside_target(SimpleEnclosedRegion());
        EXPECT_EQ(gfx::Rect(100, 100), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(100, 100)));

        // Partial occlusion from outside.
        occlusion.set_occlusion_on_contributing_surface_from_outside_target(SimpleEnclosedRegion(50, 50, 100, 100));
        occlusion.set_occlusion_on_contributing_surface_from_inside_target(SimpleEnclosedRegion());
        EXPECT_EQ(gfx::Rect(0, 0, 100, 100), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(0, 0, 100, 100)));
        EXPECT_EQ(gfx::Rect(140, 30, 50, 100), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(90, 30, 100, 100)));
        EXPECT_EQ(gfx::Rect(40, 0, 100, 30), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(40, 0, 100, 100)));
        EXPECT_EQ(gfx::Rect(40, 130, 100, 50), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(40, 80, 100, 100)));
        EXPECT_EQ(gfx::Rect(0, 0, 80, 100), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(0, 0, 80, 100)));
        EXPECT_EQ(gfx::Rect(90, 80, 100, 100), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(90, 80, 100, 100)));
        EXPECT_EQ(gfx::Rect(0, 80, 100, 100), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(0, 80, 100, 100)));
        EXPECT_EQ(gfx::Rect(90, 0, 100, 100), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(90, 0, 100, 100)));

        // Full occlusion from outside, is occluded.
        EXPECT_EQ(gfx::Rect(), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(40, 30, 100, 100)));
        EXPECT_EQ(gfx::Rect(), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(40, 30, 10, 10)));
        EXPECT_EQ(gfx::Rect(), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(130, 120, 10, 10)));
        EXPECT_EQ(gfx::Rect(), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(80, 70, 50, 50)));

        // Partial occlusion from inside, is not occluded.
        occlusion.set_occlusion_on_contributing_surface_from_outside_target(SimpleEnclosedRegion());
        occlusion.set_occlusion_on_contributing_surface_from_inside_target(SimpleEnclosedRegion(50, 50, 100, 100));
        EXPECT_EQ(gfx::Rect(0, 0, 100, 100), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(0, 0, 100, 100)));
        EXPECT_EQ(gfx::Rect(140, 30, 50, 100), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(90, 30, 100, 100)));
        EXPECT_EQ(gfx::Rect(40, 0, 100, 30), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(40, 0, 100, 100)));
        EXPECT_EQ(gfx::Rect(40, 130, 100, 50), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(40, 80, 100, 100)));
        ...
        // Partial occlusion from both, is not occluded.
        occlusion.set_occlusion_on_contributing_surface_from_outside_target(SimpleEnclosedRegion(50, 50, 100, 50));
        occlusion.set_occlusion_on_contributing_surface_from_inside_target(SimpleEnclosedRegion(50, 100, 100, 50));
        EXPECT_EQ(gfx::Rect(0, 0, 100, 100), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(0, 0, 100, 100)));
        // This could be (140, 30, 50, 100). But because we do a lossy subtract,
        // it's larger.
        EXPECT_EQ(gfx::Rect(90, 30, 100, 100), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(90, 30, 100, 100)));
        EXPECT_EQ(gfx::Rect(40, 0, 100, 30), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(40, 0, 100, 100)));
        EXPECT_EQ(gfx::Rect(40, 130, 100, 50), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(40, 80, 100, 100)));
        EXPECT_EQ(gfx::Rect(0, 0, 80, 100), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(0, 0, 80, 100)));
        EXPECT_EQ(gfx::Rect(90, 80, 100, 100), occlusion.UnoccludedSurfaceContentRect(surface, gfx::Rect(90, 80, 100, 100)));
