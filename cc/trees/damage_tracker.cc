...
void DamageTracker::UpdateDamageTracking(LayerTreeImpl* layer_tree_impl)
{
    ...
    EffectTree& effect_tree = layer_tree_impl->property_trees()->effect_tree_mutable();
    ...
    for (LayerImpl* layer : *layer_tree_impl) {
        ...
        if (next_target_effect_id != current_target_effect_id) {
            int lowest_common_ancestor_id = effect_tree.LowestCommonAncestorWithRenderSurface(current_target_effect_id, next_target_effect_id);
            while (current_target_effect_id != lowest_common_ancestor_id) {
                ...
                // in draw order.
                RenderSurfaceImpl* current_target = effect_tree.GetRenderSurface(current_target_effect_id);
                ...
                parent_target->damage_tracker()->AccumulateDamageFromRenderSurface(current_target);
                current_target_effect_id = effect_tree.Node(current_target_effect_id)->target_id;
            }
        ...
        if (layer != layer_tree_impl->hud_layer()) {
            target_surface->damage_tracker()->AccumulateDamageFromLayer(layer, id_to_render_surface_map);
        }
    }
    ...
    while (true) {
        ...
        next_target->damage_tracker()->AccumulateDamageFromRenderSurface(current_target);
        current_target = next_target;
    }
}

void DamageTracker::InitializeUpdateDamageTracking(LayerTreeImpl* layer_tree_impl, ViewTransitionElementResourceIdToRenderSurfaceMap& id_to_render_surface_map)
{
    for (RenderSurfaceImpl* render_surface : layer_tree_impl->GetRenderSurfaceList()) {
        ...
        auto resource_id = render_surface->OwningEffectNode()->view_transition_element_resource_id;
...
void DamageTracker::ComputeSurfaceDamage(RenderSurfaceImpl* render_surface)
{
    ...
    has_damage_from_contributing_content_ |= !damage_from_leftover_rects.IsEmpty();
    ...
    for (auto& contributing_surface : contributing_surfaces_) {
        ...
        bool has_pixel_moving_backdrop_filters = surface->BackdropFilters().HasFilterThatMovesPixels();
        if (!surface->intersects_damage_under() || has_pixel_moving_backdrop_filters) {
            if (!valid || contributing_surface.rect_in_target_space.Intersects(expanded_damage_rect)) {
    ...
    if (expanded) {
        damage_for_this_update_.Union(expanded_damage_rect, damage_from_leftover_rects.reasons());
    }
    ...
    if (render_surface->SurfacePropertyChanged() && !render_surface->AncestorPropertyChanged()) {
        ...
        if (is_rect_valid && !damage_rect.IsEmpty()) {
            damage_rect = render_surface->Filters().MapRect(damage_rect, gfx::TransformToFlattenedSkMatrix(render_surface->SurfaceScale()));
            damage_for_this_update_ = DamageAccumulator();
    ...
    // frame.
    current_damage_.Union(damage_for_this_update_, damage_for_this_update_.reasons());
}
...
DamageTracker::LayerRectMapData& DamageTracker::RectDataForLayer(int layer_id, bool* layer_is_new)
{
    LayerRectMapData data(layer_id);

    auto it = std::lower_bound(rect_history_for_layers_.begin(), rect_history_for_layers_.end(), data);
...
DamageTracker::SurfaceRectMapData& DamageTracker::RectDataForSurface(ElementId surface_id, bool* surface_is_new)
{
    SurfaceRectMapData data(surface_id);

    auto it = std::lower_bound(rect_history_for_surfaces_.begin(), rect_history_for_surfaces_.end(), data);
...
void DamageTracker::PrepareForUpdate()
{
    ...
    contributing_surfaces_.clear();
    current_view_transition_content_surfaces_by_id_.swap(previous_view_transition_content_surfaces_by_id_);
...
DamageTracker::DamageAccumulator DamageTracker::TrackDamageFromLeftoverRects()
{
    ...
    if (layer_copy_pos != rect_history_for_layers_.end())
        rect_history_for_layers_.erase(layer_copy_pos, rect_history_for_layers_.end());
    if (surface_copy_pos != rect_history_for_surfaces_.end())
        rect_history_for_surfaces_.erase(surface_copy_pos, rect_history_for_surfaces_.end());

    // If the vector has excessive storage, shrink it
    if (rect_history_for_layers_.capacity() > rect_history_for_layers_.size() * 4)
        SortedRectMapForLayers(rect_history_for_layers_).swap(rect_history_for_layers_);
    if (rect_history_for_surfaces_.capacity() > rect_history_for_surfaces_.size() * 4)
        SortedRectMapForSurfaces(rect_history_for_surfaces_).swap(rect_history_for_surfaces_);

    return damage;
}

void DamageTracker::AccumulateDamageFromLayer(LayerImpl* layer, ViewTransitionElementResourceIdToRenderSurfaceMap& id_to_render_surface_map)
{
    ...
    gfx::Rect visible_rect_in_target_space = layer->GetEnclosingVisibleRectInTargetSpace();
    ...
    if (layer->ViewTransitionResourceId().IsValid()) {
        ...
        view_transition_content_surface_damage_rect = GetViewTransitionContentSurfaceDamageInSharedElementLayerSpace(layer, id_to_render_surface_map);
    }
    ...
    if (layer_is_new || layer->LayerPropertyChanged()) {
        ...
        gfx::Rect damage_rect = gfx::UnionRects(layer->update_rect(), layer->GetDamageRect());
        ...
        if (view_transition_content_surface_damage_rect.Intersects(gfx::Rect(layer->bounds()))) {
            reasons.Put(DamageReason::kUntracked);
        }
        ...
        if (!damage_rect.IsEmpty()) {
            gfx::Rect damage_visible_rect_in_target_space = MathUtil::MapEnclosingClippedRect(layer->DrawTransform(), damage_rect);
    ...
    if (layer->LayerPropertyChangedFromPropertyTrees()) {
        ...
        property_change_on_non_target_node = layer->effect_tree_index() != effect_id || layer->transform_tree_index() != transform_id;
    }

    if (layer_is_new || !layer->update_rect().IsEmpty() || layer->LayerPropertyChangedNotFromPropertyTrees() || !layer->GetDamageRect().IsEmpty()
        || property_change_on_non_target_node || !view_transition_content_surface_damage_rect.IsEmpty()) {
        has_damage_from_contributing_content_ |= !damage_for_this_update_.IsEmpty();
    }
}

void DamageTracker::AccumulateDamageFromRenderSurface(RenderSurfaceImpl* render_surface)
{
    ...
    contributing_surfaces_.emplace_back(render_surface, surface_rect_in_target_space);
    ...
    bool intersects_damage_under = !valid || damage_on_target.Intersects(surface_rect_in_target_space);
    if (render_surface->BackdropFilters().HasFilterThatMovesPixels() && intersects_damage_under) {
        damage_for_this_update_.Union(surface_rect_in_target_space, { DamageReason::kUntracked });
    }

    if (surface_is_new || render_surface->SurfacePropertyChanged() || render_surface->AncestorPropertyChanged()) {
        DamageReasonSet reasons = render_surface->damage_tracker()->GetDamageReasons();
        if (surface_is_new) {
            reasons.Put(DamageReason::kUntracked);
        }
        ...
        bool is_valid_rect = render_surface->damage_tracker()->GetDamageRectIfValid(&damage_rect_in_local_space);
        if (is_valid_rect && !damage_rect_in_local_space.IsEmpty()) {
            ...
            gfx::Rect damage_rect_in_target_space = MathUtil::MapEnclosingClippedRect(draw_transform, damage_rect_in_local_space);
            damage_rect_in_target_space.Intersect(surface_rect_in_target_space);
            damage_for_this_update_.Union(damage_rect_in_target_space, render_surface->damage_tracker()->GetDamageReasons());
        } else if (!is_valid_rect) {
            damage_for_this_update_.Union(surface_rect_in_target_space, render_surface->damage_tracker()->GetDamageReasons());
        }
    }
...
gfx::Rect DamageTracker::GetViewTransitionContentSurfaceDamageInSharedElementLayerSpace(
    LayerImpl* layer, ViewTransitionElementResourceIdToRenderSurfaceMap& id_to_render_surface_map)
{
    ...
    if (shared_surface_it != id_to_render_surface_map.end()) {
        ...
        DCHECK(!base::Contains(current_view_transition_content_surfaces_by_id_, vt_resource_id));
    ...
    if (view_transition_content_surface) {
        bool surface_is_new = false;
        auto& data = RectDataForSurface(view_transition_content_surface->id(), &surface_is_new);
        data.Update(layer_drawable_bounds, mailbox_id_);
        if (surface_is_new || view_transition_content_surface->SurfacePropertyChanged() || view_transition_content_surface->AncestorPropertyChanged()) {
    ...
    if (base::Contains(previous_view_transition_content_surfaces_by_id_, vt_resource_id)
        != base::Contains(current_view_transition_content_surfaces_by_id_, vt_resource_id)) {
        // The whole view transition layer is considered damaged.
        return layer_drawable_bounds;
    }
    ...
    //
    gfx::Rect damage_rect_in_local_space;
    bool is_valid_rect = view_transition_content_surface->damage_tracker()->GetDamageRectIfValid(&damage_rect_in_local_space);
    if (!is_valid_rect) {
        // The whole view transition layer is considered damaged.
        return layer_drawable_bounds;
        ...
        gfx::Rect render_surface_content_rect = view_transition_content_surface->content_rect();
        gfx::Transform view_transition_transform = viz::GetViewTransitionTransform(layer_drawable_bounds, render_surface_content_rect);

        // Convert the damage from the view transition content surface space to the
        // shared element layer space.
        return MathUtil::MapEnclosingClippedRect(view_transition_transform, damage_rect_in_local_space);
    }
}
