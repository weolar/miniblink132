...
public:
    ...
    LayerTreeImpl(LayerTreeHostImpl& host_impl, viz::BeginFrameArgs begin_frame_args, scoped_refptr<SyncedScale> page_scale_factor,
    ...
    std::unique_ptr<ScrollbarAnimationController> CreateScrollbarAnimationController(ElementId scroll_element_id, float initial_opacity);
    ...
    {
        return host_impl_->mutator_host();
    }
    ...
    TargetColorParams GetTargetColorParams(gfx::ContentColorUsage content_color_usage) const;
    ...
    // Tracing methods
    // ---------------------------------------------------------------------------
    void GetAllPrioritizedTilesForTracing(std::vector<PrioritizedTile>* prioritized_tiles) const;
    ...
    void SetPropertyTrees(const PropertyTrees& property_trees, PropertyTreesChangeState& change_state, bool preserve_change_tracking);

    void SetPropertyTrees(PropertyTrees& property_trees, bool preserve_change_tracking = false);
    ...
    void PullPropertiesFrom(CommitState& commit_state, const ThreadUnsafeCommitState& unsafe_state);
    void PullPropertyTreesFrom(CommitState& commit_state, const ThreadUnsafeCommitState& unsafe_state);
    ...
    // LayerImpl*.
    template <typename Iterator> class IteratorAdapter {
    public:
        ...
        explicit IteratorAdapter(Iterator it)
            : it_(it)
        {
        }
        bool operator==(IteratorAdapter o) const
        {
            return it_ == o.it_;
        }
        bool operator!=(IteratorAdapter o) const
    ...
    using const_reverse_iterator = IteratorAdapter<OwnedLayerImplList::const_reverse_iterator>;
    const_iterator begin() const
    {
        return const_iterator(layer_list_.cbegin());
    }
    const_iterator end() const
    {
        return const_iterator(layer_list_.cend());
    }
    ...
    void SetTransformMutated(ElementId element_id, const gfx::Transform& transform);
    ...
    BeginMainFrameTraceId trace_id() const
    {
        return trace_id_;
    }
    void set_trace_id(BeginMainFrameTraceId val)
    {
        trace_id_ = val;
    }
    ...
    const HeadsUpDisplayLayerImpl* hud_layer() const
    {
        return hud_layer_;
    }
    ...
    void AddPresentationCallbacks(std::vector<PresentationTimeCallbackBuffer::Callback> callbacks);
    std::vector<PresentationTimeCallbackBuffer::Callback> TakePresentationCallbacks();

    void AddSuccessfulPresentationCallbacks(std::vector<PresentationTimeCallbackBuffer::SuccessfulCallbackWithDetails> callbacks);
    std::vector<PresentationTimeCallbackBuffer::SuccessfulCallbackWithDetails> TakeSuccessfulPresentationCallbacks();
    ...
    {
        return const_cast<TransformNode*>(const_cast<const LayerTreeImpl*>(this)->OverscrollElasticityTransformNode());
    }
    const TransformNode* PageScaleTransformNode() const;
    TransformNode* PageScaleTransformNode()
    {
        return const_cast<TransformNode*>(const_cast<const LayerTreeImpl*>(this)->PageScaleTransformNode());
    }
    const ScrollNode* InnerViewportScrollNode() const;
    ScrollNode* InnerViewportScrollNode()
    {
        return const_cast<ScrollNode*>(const_cast<const LayerTreeImpl*>(this)->InnerViewportScrollNode());
    }
    const ClipNode* OuterViewportClipNode() const;
    ClipNode* OuterViewportClipNode()
    {
        return const_cast<ClipNode*>(const_cast<const LayerTreeImpl*>(this)->OuterViewportClipNode());
    }
    const ScrollNode* OuterViewportScrollNode() const;
    ScrollNode* OuterViewportScrollNode()
    {
        return const_cast<ScrollNode*>(const_cast<const LayerTreeImpl*>(this)->OuterViewportScrollNode());
    }
    ...
    void ApplySentScrollAndScaleDeltasFromAbortedCommit(bool next_bmf, bool main_frame_applied_deltas);
    ...
    void PushPageScaleFromMainThread(float page_scale_factor, float min_page_scale_factor, float max_page_scale_factor);
    ...
    {
        return page_scale_factor()->Delta();
    }
    ...
    void set_primary_main_frame_item_sequence_number(int64_t item_sequence_number)
    {
    ...
    gfx::Rect internal_device_viewport()
    {
    ...
    {
        DCHECK(external_page_scale_factor_ == 1.f || current_page_scale_factor() == 1.f || settings().is_for_scalable_page);
    ...
    bool UpdateDrawProperties(bool update_tiles, bool update_image_animation_controller, LayerImplList* output_update_layer_list_for_testing = nullptr);
    ...
    bool needs_surface_ranges_sync() const
    {
        return needs_surface_ranges_sync_;
    }
    ...
    {
        next_activation_forces_redraw_ = true;
    }
    ...
    void PassSwapPromises(std::vector<std::unique_ptr<SwapPromise>> new_swap_promises);
    void AppendSwapPromises(std::vector<std::unique_ptr<SwapPromise>> new_swap_promises);
    ...
    const std::vector<raw_ptr<PictureLayerImpl, VectorExperimental>>& picture_layers() const
    {
        return picture_layers_;
    }

    void NotifyLayerHasPaintWorkletsChanged(PictureLayerImpl* layer, bool has_worklets);
    const base::flat_set<raw_ptr<PictureLayerImpl, CtnExperimental>>& picture_layers_with_paint_worklets() const
    {
    ...
    LayerImpl* FindLayerThatIsHitByPointInTouchHandlerRegion(const gfx::PointF& screen_space_point);

    LayerImpl* FindLayerThatIsHitByPointInWheelEventHandlerRegion(const gfx::PointF& screen_space_point);
    ...
    std::vector<const LayerImpl*> FindLayersUpToFirstScrollableOrOpaqueToHitTest(const gfx::PointF& screen_space_point);
    bool PointHitsMainThreadScrollHitTestRegion(const gfx::PointF& scree_space_point, const LayerImpl& layer) const;
    ElementId PointHitsNonCompositedScroll(const gfx::PointF& screen_space_point, const LayerImpl& layer) const;
    ...
    bool HandleVisibilityChanged() const
    {
    ...
    void PushBrowserControlsFromMainThread(float top_controls_shown_ratio, float bottom_controls_shown_ratio);
    ...
    void SetPendingPageScaleAnimation(std::unique_ptr<PendingPageScaleAnimation> pending_animation);
    ...
    {
        force_send_metadata_request_ = true;
    }
    bool TakeForceSendMetadataRequest();

    void DidUpdateScrollOffset(ElementId id, bool pushed_from_main_or_pending_tree);
    ...
    void set_event_listener_properties(EventListenerClass event_class, EventListenerProperties event_properties)
    {
    ...
    void InvalidateRasterInducingScrolls(const base::flat_set<ElementId>& scrolls_to_invalidate);

    void UpdateViewportContainerSizes();

    LayerTreeLifecycle& lifecycle()
    {
        return lifecycle_;
    }
    ...
    void set_delegated_ink_metadata(std::unique_ptr<gfx::DelegatedInkMetadata> metadata)
    {
    ...
    void clear_delegated_ink_metadata()
    {
    ...
    // the internal list.
    std::vector<std::unique_ptr<ViewTransitionRequest>> TakeViewTransitionRequests();
    ...
    void SetViewTransitionContentRect(const viz::ViewTransitionElementResourceId&, const gfx::RectF&);

protected:
    float ClampPageScaleFactorToLimits(float page_scale_factor) const;
    void PushPageScaleFactorAndLimits(const float* page_scale_factor, float min_page_scale_factor, float max_page_scale_factor);
    bool SetPageScaleFactorLimits(float min_page_scale_factor, float max_page_scale_factor);
    void DidUpdatePageScale();
    void PushBrowserControls(const float* top_controls_shown_ratio, const float* bottom_controls_shown_ratio);
...
private:
    ...
    template <typename Functor> LayerImpl* FindLayerThatIsHitByPointInEventHandlerRegion(const gfx::PointF& screen_space_point, const Functor& func);
    ...
    RAW_PTR_EXCLUSION base::flat_set<LayerImpl*> layers_that_should_push_properties_;
    ...
    base::flat_map<ElementId, ScrollbarLayerIds> element_id_to_scrollbar_layer_ids_;
    ...
    base::flat_set<raw_ptr<PictureLayerImpl, CtnExperimental>> picture_layers_with_paint_worklets_;
    ...
    EventListenerProperties event_listener_properties_[static_cast<size_t>(EventListenerClass::kLast) + 1];
    ...
    std::vector<PresentationTimeCallbackBuffer::SuccessfulCallbackWithDetails> successful_presentation_callbacks_;
    ...
    int64_t primary_main_frame_item_sequence_number_ = RenderFrameMetadata::kInvalidItemSequenceNumber;
};
