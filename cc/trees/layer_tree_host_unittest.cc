...
protected:
    ...
    template <typename T> bool CheckStep(T next, T* var)
    {
        int expected = next - 1;
...
class LayerTreeHostTestUpdateLayersFrameOrdering : public LayerTreeHostTestFrameOrdering {
    void DidCommit() override
    {
        EXPECT_TRUE(CheckStep(MAIN_COMMIT_COMPLETE, &main_));
...
public:
    ...
    void WillBeginMainFrame() override
    {
        ...
        MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&LayerTreeHostTestRequestedMainFrame::NextStep, base::Unretained(this)));
    }
    ...
    void NextStep()
    {
        ...
        case 0:
            ADD_FAILURE() << "Case 0 is the initial commit used to send the test here";
            [[fallthrough]];
        case 1:
...
public:
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* impl) override
    {
        const std::vector<raw_ptr<PictureLayerImpl, VectorExperimental>>& layers = impl->sync_tree()->picture_layers();
        required_for_activation_count_ = 0;
...
class LayerTreeHostTestReadyToActivateNonEmpty : public LayerTreeHostTestReadyToActivateEmpty {
public:
...
public:
    ...
    void NotifyReadyToDrawOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        const std::vector<raw_ptr<PictureLayerImpl, VectorExperimental>>& layers = impl->active_tree()->picture_layers();
...
class LayerTreeHostTestReadyToDrawNonEmpty : public LayerTreeHostTestReadyToDrawEmpty {
public:
    void SetupTree() override
    {
        client_.set_fill_with_nonsolid_color(true);
...
public:
    ...
    void DidFinishImplFrameOnThread(LayerTreeHostImpl* host_impl) override
    {
        if (!host_impl->visible()) {
            ...
            ImplThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&LayerTreeHostTestReadyToDrawVisibility::SetVisible, base::Unretained(this), host_impl));
        }
    }
...
public:
    std::unique_ptr<TestLayerTreeFrameSink> CreateLayerTreeFrameSink(const viz::RendererSettings& renderer_settings, double refresh_rate,
        scoped_refptr<viz::RasterContextProvider> compositor_context_provider, scoped_refptr<viz::RasterContextProvider> worker_context_provider) override
    {
        ...
        auto test_main_context_provider = viz::TestContextProvider::CreateRaster(std::move(main_support));

        // Create the main viz::RasterContextProvider with a MockContextSupport.
        auto worker_support = std::make_unique<MockContextSupport>();
        mock_worker_context_support_ = worker_support.get();
        auto test_worker_context_provider = viz::TestContextProvider::CreateWorker(std::move(worker_support));

        // At init, visibility is set to true, so SetAggressivelyFreeResources will
        // be disabled.
        EXPECT_CALL(*mock_main_context_support_, SetAggressivelyFreeResources(false));
        EXPECT_CALL(*mock_worker_context_support_, SetAggressivelyFreeResources(false));

        return LayerTreeHostTest::CreateLayerTreeFrameSink(
            renderer_settings, refresh_rate, std::move(test_main_context_provider), std::move(test_worker_context_provider));
    }
...
protected:
    ...
    raw_ptr<MockContextSupport, AcrossTasksDanglingUntriaged> mock_main_context_support_;
    raw_ptr<MockContextSupport, AcrossTasksDanglingUntriaged> mock_worker_context_support_;
};

// Test if the LTH successfully frees resources on the main/worker
// ContextSupport when visibility is set to false.
class LayerTreeHostFreesContextResourcesOnInvisible : public LayerTreeHostContextCacheTest {
public:
    void InitializedRendererOnThread(LayerTreeHostImpl* host_impl, bool success) override
    {
        ...
        EXPECT_CALL(*mock_main_context_support_, SetAggressivelyFreeResources(true));
        EXPECT_CALL(*mock_worker_context_support_, SetAggressivelyFreeResources(true)).WillOnce(testing::Invoke([this](bool is_visible) { EndTest(); }));
...
public:
    void InitializedRendererOnThread(LayerTreeHostImpl* host_impl, bool success) override
    {
        ...
        EXPECT_CALL(*mock_main_context_support_, SetAggressivelyFreeResources(true));
        EXPECT_CALL(*mock_worker_context_support_, SetAggressivelyFreeResources(true)).WillOnce(testing::Invoke([this](bool is_visible) {
            // End test after verifying both.
            EndTest();
        }));
        ManagedMemoryPolicy zero_policy(0, gpu::MemoryAllocation::CUTOFF_ALLOW_NOTHING, 0);
        host_impl->SetMemoryPolicy(zero_policy);
    }
};

SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostFreesWorkerContextResourcesOnZeroMemoryLimit);
...
class LayerTreeHostFreesWorkerContextResourcesOnZeroMemoryLimitSynchronous : public LayerTreeHostFreesWorkerContextResourcesOnZeroMemoryLimit {
public:
...
MULTI_THREAD_TEST_F(LayerTreeHostFreesWorkerContextResourcesOnZeroMemoryLimitSynchronous);

// Test if the LTH successfully frees main and worker resources when the
// OutputSurface is destroyed.
class LayerTreeHostFreeContextResourcesOnDestroy : public LayerTreeHostContextCacheTest {
public:
    void WillBeginImplFrameOnThread(LayerTreeHostImpl* host_impl, const viz::BeginFrameArgs& args, bool has_damage) override
    {
        ...
        EXPECT_CALL(*mock_worker_context_support_, SetAggressivelyFreeResources(true));
        EXPECT_CALL(*mock_main_context_support_, SetAggressivelyFreeResources(true));
        EndTest();
    }
...
class LayerTreeHostCacheBehaviorOnLayerTreeFrameSinkRecreated : public LayerTreeHostContextCacheTest {
public:
    void WillBeginImplFrameOnThread(LayerTreeHostImpl* host_impl, const viz::BeginFrameArgs& args, bool has_damage) override
    {
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostCacheBehaviorOnLayerTreeFrameSinkRecreated);
...
protected:
    ...
    {
        const EffectTree& tree = layer->layer_tree_impl()->property_trees()->effect_tree();
...
class LayerTreeHostTestInvisibleLayersSkipRenderPass : public LayerTreeHostTest {
protected:
    enum Step {
        kAllInvisible,
        kOneVisible,
    ...
    void DisplayReceivedCompositorFrameOnThread(const viz::CompositorFrame& frame) override
    {
...
protected:
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        const TransformNode* root_transform_node = property_trees->transform_tree().Node(root_transform_index_);
        const TransformNode* child_transform_node = property_trees->transform_tree().Node(child_transform_index_);
        const EffectNode* root_effect_node = property_trees->effect_tree().Node(root_effect_index_);
        const EffectNode* child_effect_node = property_trees->effect_tree().Node(child_effect_index_);
        const ClipNode* root_clip_node = property_trees->clip_tree().Node(root_clip_index_);
        const ClipNode* child_clip_node = property_trees->clip_tree().Node(child_clip_index_);
        const ScrollNode* root_scroll_node = property_trees->scroll_tree().Node(root_scroll_index_);
        const ScrollNode* child_scroll_node = property_trees->scroll_tree().Node(child_scroll_index_);
...
protected:
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        case 0:
            ...
            EXPECT_EQ(property_trees->scroll_tree().FindNodeFromElementId(child_element_id_), nullptr);
            EXPECT_EQ(property_trees->effect_tree().FindNodeFromElementId(child_element_id_), nullptr);
            EXPECT_EQ(property_trees->scroll_tree().FindNodeFromElementId(child_element_id_), nullptr);
            break;
        case 1:
            ...
            EXPECT_EQ(2, property_trees->transform_tree().FindNodeFromElementId(child_element_id_)->id);
            EXPECT_EQ(2, property_trees->effect_tree().FindNodeFromElementId(child_element_id_)->id);
            EXPECT_EQ(2, property_trees->scroll_tree().FindNodeFromElementId(child_element_id_)->id);
            break;
        case 2:
            EXPECT_EQ(2U, child_impl_->layer_tree_impl()->property_trees()->transform_tree().size());
            EXPECT_EQ(2U, child_impl_->layer_tree_impl()->property_trees()->effect_tree().size());
            EXPECT_EQ(property_trees->transform_tree().FindNodeFromElementId(child_element_id_), nullptr);
            EXPECT_EQ(property_trees->effect_tree().FindNodeFromElementId(child_element_id_), nullptr);
            EXPECT_EQ(property_trees->scroll_tree().FindNodeFromElementId(child_element_id_), nullptr);
            break;
        }
...
protected:
    ...
    DrawResult PrepareToDrawOnThread(LayerTreeHostImpl* impl, LayerTreeHostImpl::FrameData* frame_data, DrawResult draw_result) override
    {
...
protected:
    LayerTreeHostTestLayerListSurfaceDamage()
    {
        SetUseLayerLists();
    }
    ...
    DrawResult PrepareToDrawOnThread(LayerTreeHostImpl* impl, LayerTreeHostImpl::FrameData* frame_data, DrawResult draw_result) override
    {
        ...
        case 0:
            ...
            EXPECT_EQ(GetRenderSurface(child_a_impl)->GetDamageRect(), gfx::Rect(0, 0, 10, 20));
            EXPECT_EQ(GetRenderSurface(child_b_impl)->GetDamageRect(), gfx::Rect(0, 0, 20, 10));
            EXPECT_EQ(GetRenderSurface(child_c_impl)->GetDamageRect(), gfx::Rect(0, 0, 15, 15));
            PostSetNeedsCommitToMainThread();
            break;
        case 1:
...
protected:
    void SetupTree() override
    {
        ...
        layer_tree_host()->SetViewportRectAndScale(gfx::Rect(10, 10), 1.f, viz::LocalSurfaceId());
...
protected:
    void SetupTree() override
    {
        ...
        layer_tree_host()->SetViewportRectAndScale(gfx::Rect(100, 100), 1.f, viz::LocalSurfaceId());
...
protected:
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        case TRANSFORM:
            index_++;
            EXPECT_TRUE(impl->active_tree()->LayerById(root_->id())->LayerPropertyChanged());
            impl->active_tree()->ResetAllChangeTracking();
            EXPECT_FALSE(impl->active_tree()->LayerById(root_->id())->LayerPropertyChanged());
            EXPECT_FALSE(impl->active_tree()->LayerById(child_->id())->LayerPropertyChanged());
        ...
        case FILTER:
            ...
            EXPECT_TRUE(impl->active_tree()->LayerById(child_->id())->LayerPropertyChanged());
            impl->active_tree()->ResetAllChangeTracking();
...
class LayerTreeHostTestAnimationOpacityMutatedNotUsingLayerLists : public LayerTreeHostTest {
protected:
    void BeginTest() override
    {
        Layer* root = layer_tree_host()->root_layer();
        EXPECT_EQ(1.0f, root->opacity());
        layer_tree_host()->SetElementOpacityMutated(root->element_id(), ElementListType::ACTIVE, 0.3f);
...
SINGLE_THREAD_TEST_F(LayerTreeHostTestAnimationOpacityMutatedNotUsingLayerLists);

class LayerTreeHostTestAnimationOpacityMutatedUsingLayerLists : public LayerTreeHostTest {
public:
...
protected:
    ...
    {
        Layer* root = layer_tree_host()->root_layer();
        EXPECT_EQ(1.0f, layer_tree_host()->property_trees()->effect_tree().FindNodeFromElementId(root->element_id())->opacity);

        layer_tree_host()->SetElementOpacityMutated(root->element_id(), ElementListType::ACTIVE, 0.3f);

        // The opacity should have been set directly on the effect node instead.
        EXPECT_EQ(0.3f, layer_tree_host()->property_trees()->effect_tree().FindNodeFromElementId(root->element_id())->opacity);
        EndTest();
    }
};
...
class LayerTreeHostTestAnimationTransformMutatedNotUsingLayerLists : public LayerTreeHostTest {
protected:
    void SetupTree() override
    {
        root_ = Layer::Create();
    ...
    void BeginTest() override
    {
        ...
        layer_tree_host()->SetElementTransformMutated(child_->element_id(), ElementListType::ACTIVE, expected_transform);
...
SINGLE_THREAD_TEST_F(LayerTreeHostTestAnimationTransformMutatedNotUsingLayerLists);

class LayerTreeHostTestAnimationTransformMutatedUsingLayerLists : public LayerTreeHostTest {
public:
...
protected:
    ...
    {
        Layer* root = layer_tree_host()->root_layer();
        EXPECT_EQ(gfx::Transform(), layer_tree_host()->property_trees()->transform_tree().FindNodeFromElementId(root->element_id())->local);

        gfx::Transform expected_transform;
        expected_transform.Translate(42, 42);
        layer_tree_host()->SetElementTransformMutated(root->element_id(), ElementListType::ACTIVE, expected_transform);

        // The transform should have been set directly on the transform node
        // instead.
        EXPECT_EQ(expected_transform, layer_tree_host()->property_trees()->transform_tree().FindNodeFromElementId(root->element_id())->local);
        EndTest();
    }
};
...
class LayerTreeHostTestAnimationFilterMutatedNotUsingLayerLists : public LayerTreeHostTest {
protected:
    void BeginTest() override
    {
        Layer* root = layer_tree_host()->root_layer();
        ...
        filters.Append(FilterOperation::CreateOpacityFilter(0.5f));
        layer_tree_host()->SetElementFilterMutated(root->element_id(), ElementListType::ACTIVE, filters);
...
class LayerTreeHostTestAnimationFilterMutatedUsingLayerLists : public LayerTreeHostTest {
public:
...
protected:
    ...
    {
        Layer* root = layer_tree_host()->root_layer();
        EXPECT_EQ(FilterOperations(), layer_tree_host()->property_trees()->effect_tree().FindNodeFromElementId(root->element_id())->filters);
        ...
        EXPECT_EQ(filters, layer_tree_host()->property_trees()->effect_tree().FindNodeFromElementId(root->element_id())->filters);
        EndTest();
    }
};
...
protected:
    ...
    void WillCommit(const CommitState&) override
    {
        root_effect_tree_index_ = const_cast<const LayerTreeHost*>(layer_tree_host())->root_layer()->effect_tree_index();
    }

    void DidCommit() override
    {
        EffectTree& effect_tree = layer_tree_host()->property_trees()->effect_tree_mutable();
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* impl) override
    {
        EffectTree& effect_tree = impl->sync_tree()->property_trees()->effect_tree_mutable();
        ...
        case 4:
            ...
            impl->sync_tree()->SetFilterMutated(root->element_id(), brightness_filter_);
            PostSetNeedsCommitToMainThread();
            break;
        case 5:
            impl->sync_tree()->SetFilterMutated(root->element_id(), brightness_filter_);
...
protected:
    ...
    void DidCommit() override
    {
        TransformTree& transform_tree = layer_tree_host()->property_trees()->transform_tree_mutable();
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* impl) override
    {
        TransformTree& transform_tree = impl->sync_tree()->property_trees()->transform_tree_mutable();
        const LayerImpl* layer = impl->sync_tree()->LayerById(layer_->id());
        const TransformNode* node = transform_tree.Node(layer->transform_tree_index());
        gfx::Transform rotate10;
        rotate10.Rotate(10.f);
...
protected:
    ...
    void DidCommit() override
    {
        if (layer_tree_host()->SourceFrameNumber() == 1) {
            gfx::Transform scale;
            scale.Scale(2.0, 2.0);
            layer_tree_host()->SetElementTransformMutated(child_element_id_, ElementListType::ACTIVE, scale);
        }
    }
    ...
    DrawResult PrepareToDrawOnThread(LayerTreeHostImpl* impl, LayerTreeHostImpl::FrameData* frame_data, DrawResult draw_result) override
    {
        if (impl->active_tree()->source_frame_number() == 1) {
            EXPECT_FALSE(impl->active_tree()->LayerById(root_->id())->LayerPropertyChanged());
            EXPECT_TRUE(impl->active_tree()->LayerById(child_->id())->LayerPropertyChanged());
            EXPECT_TRUE(impl->active_tree()->LayerById(grand_child_->id())->LayerPropertyChanged());
            EndTest();
        }
...
protected:
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* impl) override
    {
        auto* mask_surface = GetRenderSurface(impl->sync_tree()->LayerById(mask_layer_->id()));
        auto* root_surface = GetRenderSurface(impl->sync_tree()->root_layer());
        ASSERT_TRUE(mask_surface);
        switch (index_) {
        case 0: {
            index_++;
            auto* child_surface = GetRenderSurface(impl->sync_tree()->LayerById(child_layer_->id()));
...
public:
    ...
    void BeginTest() override
    {
        ...
        layer_tree_host()->SetViewportRectAndScale(gfx::Rect(bounds_), 1.f, viz::LocalSurfaceId());
        PostSetNeedsCommitToMainThread();
        client_.set_bounds(root_layer_->bounds());
    }

    DrawResult PrepareToDrawOnThread(LayerTreeHostImpl* host_impl, LayerTreeHostImpl::FrameData* frame_data, DrawResult draw_result) override
    {
...
public:
    ...
    void BeginTest() override
    {
        ...
        layer_tree_host()->SetRootLayer(root);
        layer_tree_host()->SetViewportRectAndScale(gfx::Rect(bounds_), 1.f, GetCurrentLocalSurfaceId());

        PostSetNeedsCommitToMainThread();
        client_.set_bounds(layer_->bounds());
    }

    void SetUpUnboundContextProviders(viz::TestContextProvider* context_provider, viz::TestContextProvider* worker_provider) override
    {
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* host_impl) override
    {
        if (num_draws_ == 2) {
            ...
            auto* pending_layer_impl = static_cast<FakePictureLayerImpl*>(pending_tree->LayerById(layer_->id()));
            EXPECT_NE(pending_layer_impl, nullptr);

            auto* active_tree = host_impl->pending_tree();
            auto* active_layer_impl = static_cast<FakePictureLayerImpl*>(active_tree->LayerById(layer_->id()));
            EXPECT_NE(pending_layer_impl, nullptr);
            ...
            EXPECT_EQ(pending_tiling->TilingDataForTesting().max_texture_size().width(), active_tiling->TilingDataForTesting().max_texture_size().width());
        }
    }
    ...
    void DidCommitAndDrawFrame() override
    {
        ...
        if (num_draws_ == 1) {
            ...
            layer_tree_host()->SetViewportRectAndScale(gfx::Rect(400, 64), 1.f, GetCurrentLocalSurfaceId());
        }
        if (num_draws_ < 2) {
...
class LayerTreeHostTestNoExtraCommitFromScrollbarInvalidate : public LayerTreeHostTest {
public:
    void SetupTree() override
    {
        root_layer_ = Layer::Create();
        ...
        scrollbar_ = base::MakeRefCounted<FakePaintedScrollbarLayer>(root_layer_->element_id());
    ...
    void DidCommit() override
    {
        ...
        case 1:
            ...
            layer_tree_host()->SetViewportRectAndScale(layer_tree_host()->device_viewport_rect(), 4.f, layer_tree_host()->local_surface_id_from_parent());
            break;
        default:
            // No extra commits.
            EXPECT_EQ(2, layer_tree_host()->SourceFrameNumber());
...
public:
    ...
    DrawResult PrepareToDrawOnThread(LayerTreeHostImpl* host_impl, LayerTreeHostImpl::FrameData* frame_data, DrawResult draw_result) override
    {
        if (host_impl->active_tree()->source_frame_number() == 0) {
            ...
            EXPECT_EQ(gfx::Rect(host_impl->active_tree()->root_layer()->bounds()), root_damage_rect);
...
public:
    ...
    DrawResult PrepareToDrawOnThread(LayerTreeHostImpl* host_impl, LayerTreeHostImpl::FrameData* frame_data, DrawResult draw_result) override
    {
        ...
        case 0:
            ...
            EXPECT_TRUE(space1_ == host_impl->active_tree()->display_color_spaces());
            expected_root_damage_rect = gfx::Rect(host_impl->active_tree()->root_layer()->bounds());
            break;
        case 1:
            // Empty commit.
        ...
        case 2:
            ...
            EXPECT_FALSE(frame_data->has_no_damage);
            EXPECT_TRUE(space2_ == host_impl->active_tree()->display_color_spaces());
            expected_root_damage_rect = gfx::Rect(host_impl->active_tree()->root_layer()->bounds());
            break;
        case 3:
        ...
        case 4:
            ...
            EXPECT_FALSE(frame_data->has_no_damage);
            EXPECT_TRUE(space2_with_hdr_ == host_impl->active_tree()->display_color_spaces());
...
class LayerTreeHostTestSetNeedsCommitWithForcedRedraw : public LayerTreeHostTest {
public:
...
MULTI_THREAD_BLOCKNOTIFY_TEST_F(LayerTreeHostTestSetNeedsCommitWithForcedRedraw);
...
public:
    ...
    void DidActivateTreeOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        // damage calculation.
        FakePictureLayerImpl* child_layer_impl = static_cast<FakePictureLayerImpl*>(host_impl->active_tree()->LayerById(child_layer_->id()));
    ...
    DrawResult PrepareToDrawOnThread(LayerTreeHostImpl* host_impl, LayerTreeHostImpl::FrameData* frame_data, DrawResult draw_result) override
    {
        ...
        case 1: {
            ...
            EXPECT_EQ(child_layer_impl->GetEnclosingVisibleRectInTargetSpace(), root_damage_rect);
            EXPECT_TRUE(child_layer_impl->GetEnclosingVisibleRectInTargetSpace().Contains(gfx::Rect(child_layer_bounds_)));
            break;
        }
        default:
            NOTREACHED();
...
public:
    ...
    void BeginTest() override
    {
        layer_tree_host()->SetViewportRectAndScale(gfx::Rect(20, 20), 1.f, viz::LocalSurfaceId());
        layer_tree_host()->set_background_color(SkColors::kGray);
        layer_tree_host()->SetEventListenerProperties(EventListenerClass::kMouseWheel, EventListenerProperties::kPassive);
        layer_tree_host()->SetEventListenerProperties(EventListenerClass::kTouchStartOrMove, EventListenerProperties::kBlocking);
        layer_tree_host()->SetEventListenerProperties(EventListenerClass::kTouchEndOrCancel, EventListenerProperties::kBlockingAndPassive);
    ...
    void DidActivateTreeOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        EXPECT_EQ(EventListenerProperties::kPassive, impl->active_tree()->event_listener_properties(EventListenerClass::kMouseWheel));
        EXPECT_EQ(EventListenerProperties::kBlocking, impl->active_tree()->event_listener_properties(EventListenerClass::kTouchStartOrMove));
        EXPECT_EQ(EventListenerProperties::kBlockingAndPassive, impl->active_tree()->event_listener_properties(EventListenerClass::kTouchEndOrCancel));
...
// activation failure.
class LayerTreeHostTestFrameTimeUpdatesAfterActivationFails : public LayerTreeHostTest {
public:
    ...
    void WillBeginImplFrameOnThread(LayerTreeHostImpl* impl, const viz::BeginFrameArgs& args, bool has_damage) override
    {
...
// This test blocks activation which is not supported for single thread mode.
MULTI_THREAD_BLOCKNOTIFY_TEST_F(LayerTreeHostTestFrameTimeUpdatesAfterActivationFails);
...
public:
    ...
    void SetupTree() override
    {
        ...
        scroll_layer_->SetBounds(gfx::Size(2 * root_layer->bounds().width(), 2 * root_layer->bounds().height()));
    ...
    void ApplyViewportChanges(const ApplyViewportChangesArgs& args) override
    {
        ...
        layer_tree_host()->SetPageScaleFactorAndLimits(args.page_scale_delta, 0.5f, 2.f);
    }
    ...
    void DidCommitAndDrawFrame() override
    {
        switch (layer_tree_host()->SourceFrameNumber()) {
        case 1:
            layer_tree_host()->StartPageScaleAnimation(gfx::Point(), false, 1.25f, base::TimeDelta());
            break;
        }
    }
...
class ViewportDeltasAppliedDuringPinch : public LayerTreeHostTest, public ScrollCallbacks {
protected:
    ViewportDeltasAppliedDuringPinch()
        : sent_gesture_(false)
    {
        SetUseLayerLists();
    }
    ...
    void SetupTree() override
    {
        ...
        layer_tree_host()->property_trees()->scroll_tree_mutable().SetScrollCallbacks(weak_ptr_factory_.GetWeakPtr());
    }
    ...
    {
        if (!sent_gesture_) {
            host_impl->GetInputHandler().PinchGestureBegin(gfx::Point(100, 100), ui::ScrollInputType::kWheel);
    ...
    void ApplyViewportChanges(const ApplyViewportChangesArgs& args) override
    {
        ...
        auto* scroll_layer = layer_tree_host()->InnerViewportScrollLayerForTesting();
    ...
    // ScrollCallbacks
    void DidCompositorScroll(ElementId element_id, const gfx::PointF& scroll_offset, const std::optional<TargetSnapAreaElementIds>& snap_target_ids) override
    {
...
private:
    ...
    base::WeakPtrFactory<ViewportDeltasAppliedDuringPinch> weak_ptr_factory_ { this };
};
...
public:
    ...
    void BeginTest() override
    {
        ...
        layer_tree_host()->SetViewportRectAndScale(gfx::Rect(60, 60), 1.5f, viz::LocalSurfaceId());
        EXPECT_EQ(gfx::Size(60, 60), layer_tree_host()->device_viewport_rect().size());
    ...
    void DidActivateTreeOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        FakePictureLayerImpl* child = static_cast<FakePictureLayerImpl*>(impl->active_tree()->LayerById(child_layer_->id()));
        ...
        const RenderSurfaceList& render_surface_list = *frame_data.render_surface_list;
        ...
        scale_transform.Scale(impl->active_tree()->device_scale_factor(), impl->active_tree()->device_scale_factor());
        ...
        child_transform.Scale(child->MaximumTilingContentsScale(), child->MaximumTilingContentsScale());
...
public:
    ...
    void WillBeginImplFrameOnThread(LayerTreeHostImpl* host_impl, const viz::BeginFrameArgs& args, bool has_damage) override
    {
        ...
        case 5:
            MainThreadTaskRunner()->PostTask(FROM_HERE,
                ...
                base::BindOnce(&LayerTreeHostTestDeferMainFrameUpdate::AllowCommits, base::Unretained(this)));
            break;
        default:
    ...
    virtual bool IsCommitAllowed() const
    {
        return allow_commits_;
    }
...
class LayerTreeHostInvalidLocalSurfaceIdDefersCommit : public LayerTreeHostTestDeferMainFrameUpdate {
public:
...
// to happen.
class LayerTreeHostTestDeferMainFrameUpdateInsideBeginMainFrame : public LayerTreeHostTest {
public:
    ...
    void WillBeginMainFrame() override
    {
        ...
        MainThreadTaskRunner()->PostDelayedTask(FROM_HERE,
            // Unretained because the test doesn't end before this runs.
            base::BindOnce(&LayerTreeTest::EndTest, base::Unretained(this)), base::Milliseconds(100));
    }
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostTestDeferMainFrameUpdateInsideBeginMainFrame);

// This verifies that we can abort a commit inside the main frame, and
// we will finish the commit once it is allowed.
class LayerTreeHostTestDeferInsideBeginMainFrameWithCommitAfter : public LayerTreeHostTest {
public:
    ...
    void WillBeginMainFrame() override
    {
        ...
        MainThreadTaskRunner()->PostDelayedTask(FROM_HERE,
            // Unretained because the test doesn't end before this runs.
            base::BindOnce(&LayerTreeHostTestDeferInsideBeginMainFrameWithCommitAfter::AllowCommits, base::Unretained(this)), base::Milliseconds(100));
    }
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostTestDeferInsideBeginMainFrameWithCommitAfter);
...
class LayerTreeHostTestAnimateOnlyBeginFrames : public LayerTreeHostTest, public viz::ExternalBeginFrameSourceClient {
public:
    ...
    void IssueBeginFrame(bool animate_only)
    {
        ...
        auto args = viz::BeginFrameArgs::Create(BEGINFRAME_FROM_HERE, viz::BeginFrameArgs::kManualSourceId, sequence_number, last_begin_frame_time_,
            last_begin_frame_time_ + viz::BeginFrameArgs::DefaultInterval(), viz::BeginFrameArgs::DefaultInterval(), viz::BeginFrameArgs::NORMAL);
    ...
    void PostIssueBeginFrame(bool animate_only)
    {
        ...
        ImplThreadTaskRunner()->PostTask(
            FROM_HERE, base::BindOnce(&LayerTreeHostTestAnimateOnlyBeginFrames::IssueBeginFrame, base::Unretained(this), animate_only));
    }
    ...
    void WillBeginImplFrameOnThread(LayerTreeHostImpl* host_impl, const viz::BeginFrameArgs& args, bool has_damage) override
    {
        EXPECT_EQ(args.animate_only, (begin_frame_count_ >= 2 && begin_frame_count_ <= 4));
    }
    ...
    void DidFinishImplFrameOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        if (begin_frame_count_ < 3) {
            ...
            MainThreadTaskRunner()->PostTaskAndReply(FROM_HERE,
                base::BindOnce(&LayerTreeHostTestAnimateOnlyBeginFrames::SetNeedsCommitOnMainThread, base::Unretained(this)),
                base::BindOnce(&LayerTreeHostTestAnimateOnlyBeginFrames::PostIssueBeginFrame, base::Unretained(this), true));
        } else if (begin_frame_count_ == 4) {
            PostIssueBeginFrame(false);
        }
    }

    void SetNeedsCommitOnMainThread()
    {
        layer_tree_host()->SetNeedsCommit();
    }
...
private:
    ...
    uint64_t next_begin_frame_sequence_number_ = viz::BeginFrameArgs::kStartingFrameNumber;
    int commit_count_ = 0;
...
class LayerTreeHostTestCompositeImmediatelyStateTransitions : public LayerTreeHostTest {
public:
    enum {
        kInvalid,
        kStartedTest,
    ...
    void AfterTest() override
    {
        EXPECT_EQ(current_state_, kCompletedImplFrame);
    }
...
class LayerTreeHostTestBeginFrameNotificationShutdownWhileEnabled : public LayerTreeHostTest {
public:
    void InitializeSettings(LayerTreeSettings* settings) override
    {
        settings->using_synchronous_renderer_compositor = true;
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        ImplThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&LayerTreeHostTest::EndTest, base::Unretained(this)));
    }
};

MULTI_THREAD_TEST_F(LayerTreeHostTestBeginFrameNotificationShutdownWhileEnabled);
...
protected:
    ...
    void BeginMainFrameAbortedOnThread(LayerTreeHostImpl* host_impl, CommitEarlyOutReason reason, bool /* did_sync_scroll_and_viewport */) override
    {
        commit_abort_count_++;
...
public:
    OnDrawLayerTreeFrameSink(scoped_refptr<viz::RasterContextProvider> compositor_context_provider,
        scoped_refptr<viz::RasterContextProvider> worker_context_provider, gpu::GpuMemoryBufferManager* gpu_memory_buffer_manager,
        const viz::RendererSettings& renderer_settings, const viz::DebugRendererSettings* const debug_settings, TaskRunnerProvider* task_runner_provider,
        bool synchronous_composite, double refresh_rate, base::RepeatingClosure invalidate_callback)
        : TestLayerTreeFrameSink(std::move(compositor_context_provider), std::move(worker_context_provider), gpu_memory_buffer_manager, renderer_settings,
            debug_settings, task_runner_provider, synchronous_composite, false /* disable_display_vsync */, refresh_rate)
...
class LayerTreeHostTestAbortedCommitDoesntStallSynchronousCompositor : public LayerTreeHostTestAbortedCommitDoesntStall {
protected:
    void InitializeSettings(LayerTreeSettings* settings) override
    {
    ...
    std::unique_ptr<TestLayerTreeFrameSink> CreateLayerTreeFrameSink(const viz::RendererSettings& renderer_settings, double refresh_rate,
        scoped_refptr<viz::RasterContextProvider> compositor_context_provider, scoped_refptr<viz::RasterContextProvider> worker_context_provider) override
    {
        auto on_draw_callback = base::BindRepeating(&LayerTreeHostTestAbortedCommitDoesntStallSynchronousCompositor::CallOnDraw, base::Unretained(this));
        auto frame_sink
            = std::make_unique<OnDrawLayerTreeFrameSink>(compositor_context_provider, std::move(worker_context_provider), gpu_memory_buffer_manager(),
                renderer_settings, &debug_settings_, task_runner_provider(), false /* synchronous_composite */, refresh_rate, std::move(on_draw_callback));
    ...
    void CallOnDraw()
    {
        if (!TestEnded()) {
            ...
            ImplThreadTaskRunner()->PostTask(
                FROM_HERE, base::BindOnce(&OnDrawLayerTreeFrameSink::OnDraw, base::Unretained(layer_tree_frame_sink_), resourceless_software_draw));
        }
    }
...
class LayerTreeHostTestSynchronousCompositorActivateWithoutDraw : public LayerTreeHostTest {
protected:
    void InitializeSettings(LayerTreeSettings* settings) override
    {
        settings->using_synchronous_renderer_compositor = true;
        ...
        scoped_refptr<viz::RasterContextProvider> compositor_context_provider, scoped_refptr<viz::RasterContextProvider> worker_context_provider) override
    {
        ...
        auto frame_sink = std::make_unique<OnDrawLayerTreeFrameSink>(compositor_context_provider, std::move(worker_context_provider),
            gpu_memory_buffer_manager(), renderer_settings, &debug_settings_, task_runner_provider(),
...
class LayerTreeHostTestUninvertibleTransformDoesNotBlockActivation : public LayerTreeHostTest {
protected:
    void SetupTree() override
    {
        LayerTreeHostTest::SetupTree();
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostTestUninvertibleTransformDoesNotBlockActivation);
...
public:
    ...
    void DidCommit() override
    {
        ...
        case 2:
            ...
            layer_tree_host()->GetUIResourceManager()->DeleteUIResource(ui_resources_[0]->id());
            PostSetNeedsCommitToMainThread();
            break;
        case 3:
    ...
    void DidActivateTreeOnThread(LayerTreeHostImpl* impl) override
    {
        auto* sii = static_cast<viz::TestContextProvider*>(impl->layer_tree_frame_sink()->context_provider())->SharedImageInterface();
...
protected:
    ...
    void DidCommitAndDrawFrame() override
    {
        EXPECT_EQ(expected_push_properties_root_, root_->push_properties_count()) << "num_commits: " << num_commits_;
        EXPECT_EQ(expected_push_properties_child_, child_->push_properties_count()) << "num_commits: " << num_commits_;
        EXPECT_EQ(expected_push_properties_grandchild_, grandchild_->push_properties_count()) << "num_commits: " << num_commits_;
        EXPECT_EQ(expected_push_properties_child2_, child2_->push_properties_count()) << "num_commits: " << num_commits_;
        EXPECT_EQ(expected_push_properties_other_root_, other_root_->push_properties_count()) << "num_commits: " << num_commits_;
        EXPECT_EQ(expected_push_properties_leaf_layer_, leaf_always_pushing_layer_->push_properties_count()) << "num_commits: " << num_commits_;
        ...
        if (root_->layer_tree_host()) {
            EXPECT_FALSE(base::Contains(
                static_cast<const LayerTreeHost*>(root_->layer_tree_host())->pending_commit_state()->layers_that_should_push_properties, root_.get()));
        }
        if (child2_->layer_tree_host()) {
            EXPECT_FALSE(base::Contains(
                static_cast<const LayerTreeHost*>(child2_->layer_tree_host())->pending_commit_state()->layers_that_should_push_properties, child2_.get()));
        }
        if (leaf_always_pushing_layer_->layer_tree_host()) {
            leaf_always_pushing_layer_->SetNeedsPushProperties();
            EXPECT_TRUE(base::Contains(
                static_cast<const LayerTreeHost*>(leaf_always_pushing_layer_->layer_tree_host())->pending_commit_state()->layers_that_should_push_properties,
        ...
        if (child_->layer_tree_host()) {
            EXPECT_FALSE(base::Contains(
                static_cast<const LayerTreeHost*>(child_->layer_tree_host())->pending_commit_state()->layers_that_should_push_properties, child_.get()));
        }
        if (grandchild_->layer_tree_host()) {
            EXPECT_FALSE(
                base::Contains(static_cast<const LayerTreeHost*>(grandchild_->layer_tree_host())->pending_commit_state()->layers_that_should_push_properties,
                    grandchild_.get()));
        }

        if (other_root_->layer_tree_host()) {
            EXPECT_FALSE(
                base::Contains(static_cast<const LayerTreeHost*>(other_root_->layer_tree_host())->pending_commit_state()->layers_that_should_push_properties,
                    other_root_.get()));
        }
        ...
        case 10:
            GenerateNewLocalSurfaceId();
            layer_tree_host()->SetViewportRectAndScale(gfx::Rect(20, 20), 1.f, GetCurrentLocalSurfaceId());
            // No layers need commit.
            break;
        case 11:
...
class LayerTreeHostTestImplLayersPushProperties : public LayerTreeHostTestLayersPushProperties {
protected:
    void BeginTest() override
    {
    ...
    void DidActivateTreeOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        case 3:
            ...
            EXPECT_EQ(1u, static_cast<PushPropertiesCountingLayerImpl*>(host_impl->active_tree()->root_layer())->push_properties_count());
            return;
        case 4:
        ...
        root_impl_ = static_cast<PushPropertiesCountingLayerImpl*>(host_impl->active_tree()->root_layer());

        LayerTreeImpl* impl = root_impl_->layer_tree_impl();
        if (impl->LayerById(child_->id())) {
            child_impl_ = static_cast<PushPropertiesCountingLayerImpl*>(impl->LayerById(child_->id()));

            if (impl->LayerById(grandchild_->id()))
                grandchild_impl_ = static_cast<PushPropertiesCountingLayerImpl*>(impl->LayerById(grandchild_->id()));
        }

        if (impl->LayerById(child2_->id())) {
            child2_impl_ = static_cast<PushPropertiesCountingLayerImpl*>(impl->LayerById(child2_->id()));

            if (impl->LayerById(leaf_always_pushing_layer_->id()))
                leaf_always_pushing_layer_impl_ = static_cast<PushPropertiesCountingLayerImpl*>(impl->LayerById(leaf_always_pushing_layer_->id()));
        }

        if (root_impl_)
            EXPECT_EQ(expected_push_properties_root_impl_, root_impl_->push_properties_count());
        if (child_impl_)
            EXPECT_EQ(expected_push_properties_child_impl_, child_impl_->push_properties_count());
        if (grandchild_impl_)
            EXPECT_EQ(expected_push_properties_grandchild_impl_, grandchild_impl_->push_properties_count());
        if (child2_impl_)
            EXPECT_EQ(expected_push_properties_child2_impl_, child2_impl_->push_properties_count());
        if (leaf_always_pushing_layer_impl_)
            EXPECT_EQ(expected_push_properties_grandchild2_impl_, leaf_always_pushing_layer_impl_->push_properties_count());
    }
...
class LayerTreeHostTestPropertyChangesDuringUpdateArePushed : public LayerTreeHostTest {
protected:
    void BeginTest() override
    {
        PostSetNeedsCommitToMainThread();
    ...
    void SetupTree() override
    {
        ...
        scrollbar->set_should_paint(true);
        scrollbar->set_has_thumb(false);
        scrollbar_layer_ = base::MakeRefCounted<FakePaintedScrollbarLayer>(root_->element_id());
    ...
    void DidCommitAndDrawFrame() override
    {
        ...
        case 1: {
            ...
            EXPECT_TRUE(base::Contains(
                const_cast<const LayerTreeHost*>(layer_tree_host())->pending_commit_state()->layers_that_should_push_properties, scrollbar_layer_.get()));
            layer_tree_host()->SetNeedsCommit();
...
protected:
    ...
    void DidCommitAndDrawFrame() override
    {
        ...
        case 1: {
            ...
            // commit.
            EXPECT_FALSE(
                base::Contains(const_cast<const LayerTreeHost*>(layer_tree_host())->pending_commit_state()->layers_that_should_push_properties, root_.get()));
            EXPECT_FALSE(
                base::Contains(const_cast<const LayerTreeHost*>(layer_tree_host())->pending_commit_state()->layers_that_should_push_properties, child_.get()));
...
class LayerTreeHostTestCasePushPropertiesThreeGrandChildren : public LayerTreeHostTest {
protected:
    void BeginTest() override
    {
        expected_push_properties_root_ = 0;
...
class LayerTreeHostTestPushPropertiesAddingToTreeRequiresPush : public LayerTreeHostTestCasePushPropertiesThreeGrandChildren {
protected:
    void DidCommitAndDrawFrame() override
    {
        ...
        case 0:
            ...
            EXPECT_TRUE(
                base::Contains(const_cast<const LayerTreeHost*>(layer_tree_host())->pending_commit_state()->layers_that_should_push_properties, child_.get()));
            EXPECT_TRUE(base::Contains(
                const_cast<const LayerTreeHost*>(layer_tree_host())->pending_commit_state()->layers_that_should_push_properties, grandchild1_.get()));
            EXPECT_TRUE(base::Contains(
                const_cast<const LayerTreeHost*>(layer_tree_host())->pending_commit_state()->layers_that_should_push_properties, grandchild2_.get()));
            EXPECT_TRUE(base::Contains(
                const_cast<const LayerTreeHost*>(layer_tree_host())->pending_commit_state()->layers_that_should_push_properties, grandchild3_.get()));
            break;
        case 1:
            EndTest();
            break;
...
class LayerTreeHostTestPushPropertiesRemovingChildStopsRecursion : public LayerTreeHostTestCasePushPropertiesThreeGrandChildren {
protected:
    void DidCommitAndDrawFrame() override
    {
        int last_source_frame_number = layer_tree_host()->SourceFrameNumber() - 1;
...
class LayerTreeHostTestPushPropertiesRemovingChildStopsRecursionWithPersistence : public LayerTreeHostTestCasePushPropertiesThreeGrandChildren {
protected:
    void DidCommitAndDrawFrame() override
    {
...
MULTI_THREAD_TEST_F(LayerTreeHostTestPushPropertiesRemovingChildStopsRecursionWithPersistence);

class LayerTreeHostTestPushPropertiesSetPropertiesWhileOutsideTree : public LayerTreeHostTestCasePushPropertiesThreeGrandChildren {
protected:
    void DidCommitAndDrawFrame() override
    {
        int last_source_frame_number = layer_tree_host()->SourceFrameNumber() - 1;
...
MULTI_THREAD_TEST_F(LayerTreeHostTestPushPropertiesSetPropertiesWhileOutsideTree);

class LayerTreeHostTestPushPropertiesSetPropertyInParentThenChild : public LayerTreeHostTestCasePushPropertiesThreeGrandChildren {
protected:
    void DidCommitAndDrawFrame() override
    {
        int last_source_frame_number = layer_tree_host()->SourceFrameNumber() - 1;
...
MULTI_THREAD_TEST_F(LayerTreeHostTestPushPropertiesSetPropertyInParentThenChild);

class LayerTreeHostTestPushPropertiesSetPropertyInChildThenParent : public LayerTreeHostTestCasePushPropertiesThreeGrandChildren {
protected:
    void DidCommitAndDrawFrame() override
    {
        int last_source_frame_number = layer_tree_host()->SourceFrameNumber() - 1;
...
MULTI_THREAD_TEST_F(LayerTreeHostTestPushPropertiesSetPropertyInChildThenParent);
...
public:
    ...
    DrawResult PrepareToDrawOnThread(LayerTreeHostImpl* host_impl, LayerTreeHostImpl::FrameData* frame_data, DrawResult draw_result) override
    {
        ...
        return LayerTreeHostTest::PrepareToDrawOnThread(host_impl, frame_data, draw_result);
    }
    ...
    void SetCallback(LayerTreeHostImpl* host_impl, bool enable)
    {
        host_impl->SetTreeActivationCallback(
            enable ? base::BindRepeating(&LayerTreeHostTestTreeActivationCallback::ActivationCallback, base::Unretained(this)) : base::RepeatingClosure());
    }
...
public:
    ...
    void BeginTest() override
    {
        ASSERT_TRUE(invalidate_layer_) << "Derived tests must set this in SetupTree";
...
protected:
    ...
    void DidCommitAndDrawFrame() override
    {
        ...
        case 1:
            ...
            EXPECT_FALSE(base::Contains(
                const_cast<const LayerTreeHost*>(layer_tree_host())->pending_commit_state()->layers_that_should_push_properties, child_layer_.get()));
...
protected:
    ...
    void BeginTest() override
    {
        ...
        // thread.
        layer_tree_host()->SetViewportRectAndScale(gfx::Rect(0, 0), 1.f, viz::LocalSurfaceId());
        PostSetNeedsCommitToMainThread();
    }
...
public:
    ...
    void SetupTree() override
    {
        ...
        CopyProperties(layer_tree_host()->OuterViewportScrollLayerForTesting(), content_layer.get());
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* host_impl) override
    {
        if (host_impl->sync_tree()->source_frame_number() == 0) {
            scroll_elasticity_helper_ = host_impl->GetInputHandler().CreateScrollElasticityHelper();
        }
    }

    void VerifyOverscroll(const gfx::Vector2dF& stretch_amount, const gfx::Transform& transform)
    {
    ...
    {
        num_draws_++;
        FakePictureLayerImpl* content_layer_impl = static_cast<FakePictureLayerImpl*>(host_impl->active_tree()->LayerById(content_layer_id_));

#if BUILDFLAG(IS_ANDROID)
        // Elastic overscroll should not cause tilings with new scale to be created.
        EXPECT_EQ(1u, content_layer_impl->tilings()->num_tilings()) << "num_draws:" << num_draws_;
        EXPECT_EQ(1.f, content_layer_impl->tilings()->GetMaximumContentsScale()) << "num_draws_:" << num_draws_;
#endif // BUILDFLAG(IS_ANDROID)
        ...
        case 2:
            ...
            VerifyOverscroll(gfx::Vector2dF(5.f, 6.f), content_layer_impl->DrawTransform());

            scroll_elasticity_helper_->SetStretchAmount(gfx::Vector2dF(3.f, 2.f));
            break;
        case 3:
            VerifyOverscroll(gfx::Vector2dF(3.f, 2.f), content_layer_impl->DrawTransform());
...
private:
    ...
    raw_ptr<ScrollElasticityHelper, AcrossTasksDanglingUntriaged> scroll_elasticity_helper_;
...
public:
    ...
    void DidActivate() override
    {
        ...
        EXPECT_TRUE(!result_->did_not_swap_called || action_ == SwapPromise::DidNotSwapAction::KEEP_ACTIVE);
    ...
    DidNotSwapAction DidNotSwap(DidNotSwapReason reason, base::TimeTicks ts) override
    {
        base::AutoLock lock(result_->lock);
        EXPECT_FALSE(result_->did_swap_called);
    ...
    void set_action(DidNotSwapAction action)
    {
        action_ = action;
    }
...
protected:
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        if (frame == -1) {
            host_impl->active_tree()->QueuePinnedSwapPromise(std::make_unique<TestSwapPromise>(&pinned_active_swap_promise_result_));
            host_impl->pending_tree()->QueueSwapPromise(std::make_unique<TestSwapPromise>(&pending_swap_promise_result_));
            host_impl->active_tree()->QueueSwapPromise(std::make_unique<TestSwapPromise>(&active_swap_promise_result_));
        }
    }
...
protected:
    ...
    void WillBeginMainFrame() override
    {
        ...
        std::unique_ptr<SwapPromise> swap_promise(new TestSwapPromise(&swap_promise_result_[commit_count_]));
        layer_tree_host()->GetSwapPromiseManager()->QueueSwapPromise(std::move(swap_promise));
    }
...
public:
    ...
    void BeginTest() override
    {
        ...
        layer_tree_host()->SetViewportRectAndScale(gfx::Rect(bounds), 1.f, viz::LocalSurfaceId());
        PostSetNeedsCommitToMainThread();
    }

    void DidCommit() override
    {
        MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&LayerTreeHostTestKeepSwapPromise::ChangeFrame, base::Unretained(this)));
    }
    ...
    void ChangeFrame()
    {
        ...
        case 1:
            ...
            layer_tree_host()->GetSwapPromiseManager()->QueueSwapPromise(std::make_unique<TestSwapPromise>(&swap_promise_result_));
            break;
        case 2:
            break;
        default:
    ...
    {
        host_impl->SetTreeActivationCallback(
            enable ? base::BindRepeating(&LayerTreeHostTestKeepSwapPromise::ActivationCallback, base::Unretained(this)) : base::RepeatingClosure());
    }
...
public:
    ...
    void DidCommit() override
    {
        MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&LayerTreeHostTestKeepSwapPromiseMFBA::ChangeFrame, base::Unretained(this)));
    }
    ...
    {
        host_impl->SetTreeActivationCallback(
            enable ? base::BindRepeating(&LayerTreeHostTestKeepSwapPromiseMFBA::ActivationCallback, base::Unretained(this)) : base::RepeatingClosure());
    }
...
class LayerTreeHostTestDeferSwapPromiseForVisibility : public LayerTreeHostTest {
protected:
    void BeginTest() override
    {
        PostSetNeedsCommitToMainThread();
    ...
    {
        layer_tree_host()->SetVisible(false);
        auto swap_promise = std::make_unique<TestSwapPromise>(&swap_promise_result_);
    ...
    void WillBeginImplFrameOnThread(LayerTreeHostImpl* impl, const viz::BeginFrameArgs& args, bool has_damage) override
    {
        if (!sent_queue_request_) {
            ...
            MainThreadTaskRunner()->PostTask(
                FROM_HERE, base::BindOnce(&LayerTreeHostTestDeferSwapPromiseForVisibility::SetVisibleFalseAndQueueSwapPromise, base::Unretained(this)));
        }
    }
    ...
    {
        MainThreadTaskRunner()->PostTask(
            FROM_HERE, base::BindOnce(&LayerTreeHostTestDeferSwapPromiseForVisibility::CheckSwapPromiseNotCalled, base::Unretained(this)));
    }
...
protected:
    ...
    void WillBeginMainFrame() override
    {
        ...
        std::unique_ptr<SwapPromise> swap_promise = std::make_unique<TestSwapPromise>(&swap_promise_result_[0]);

        {
            StrictMock<MockLatencyInfoSwapPromiseMonitor> monitor(layer_tree_host()->GetSwapPromiseManager());
    ...
    void DidBeginMainFrame() override
    {
        ...
        std::unique_ptr<SwapPromise> swap_promise = std::make_unique<TestSwapPromise>(&swap_promise_result_[1]);
...
class LayerTreeHostTestLatencyInfoSwapPromiseMonitor : public LayerTreeHostTest {
public:
    void BeginTest() override
    {
        PostSetNeedsCommitToMainThread();
...
class LayerTreeHostTestHighResRequiredAfterEvictingUIResources : public LayerTreeHostTest {
protected:
    void SetupTree() override
    {
        LayerTreeHostTest::SetupTree();
    ...
    void DidActivateTreeOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        MainThreadTaskRunner()->PostTask(
            FROM_HERE, base::BindOnce(&LayerTreeHostTestHighResRequiredAfterEvictingUIResources::DeleteResourceAndEndTest, base::Unretained(this)));
    }
...
protected:
    void SetUpUnboundContextProviders(viz::TestContextProvider* context_provider, viz::TestContextProvider* worker_provider) override
    {
        ...
        gpu::Capabilities worker_caps = worker_provider->UnboundTestRasterInterface()->capabilities();
    ...
    void SetupTree() override
    {
        ...
        scoped_refptr<FakePictureLayer> layer = FakePictureLayer::Create(&layer_client_);
        layer_ = layer.get();
        layer->SetBounds(gfx::Size(10, 10));
        layer->SetIsDrawable(true);
        layer_tree_host()->root_layer()->AddChild(layer);
...
class LayerTreeHostTestGpuRasterizationSupportedButDisabled : public LayerTreeTest {
protected:
    void SetUpUnboundContextProviders(viz::TestContextProvider* context_provider, viz::TestContextProvider* worker_provider) override
    {
        context_provider->UnboundTestRasterInterface()->set_gpu_rasterization(true);
...
protected:
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* host_impl) override
    {
        auto* raster_source = static_cast<PictureLayerImpl*>(host_impl->sync_tree()->LayerById(layer_->id()))->GetRasterSource();
        EXPECT_GT(host_impl->GetMSAASampleCountForRaster(*raster_source->GetDisplayItemList()), 0);
...
class LayerTreeHostTestWillBeginImplFrameHasDidFinishImplFrame : public LayerTreeHostTest {
public:
...
::testing::AssertionResult AssertFrameTimeContained(
    const char* haystack_expr, const char* needle_expr, const std::vector<viz::BeginFrameArgs> haystack, const viz::BeginFrameArgs needle)
{
    auto failure = ::testing::AssertionFailure() << needle.frame_time << " (" << needle_expr << ") not found in " << haystack_expr;
...
class LayerTreeHostTestBeginMainFrameTimeIsAlsoImplTime : public LayerTreeHostTest {
public:
    ...
    void BeginMainFrame(const viz::BeginFrameArgs& args) override
    {
        ASSERT_GT(impl_frame_args_.size(), 0U) << "BeginMainFrame called before BeginImplFrame called!";
...
public:
    ...
    void DidSetVisibleOnImplTree(LayerTreeHostImpl* host_impl, bool visible) override
    {
        visible_ = visible;
...
public:
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* host_impl) override
    {
        if (host_impl->sync_tree()->source_frame_number() >= 1) {
            ...
            EXPECT_EQ(grand_child_impl->effect_tree_index(), GetRenderSurface(grand_child_impl)->EffectTreeIndex());
        }
    }
    ...
    void DidCommit() override
    {
        ...
        case 2:
            ...
            layer_tree_host()->SetViewportRectAndScale(gfx::Rect(), 1.f, GetCurrentLocalSurfaceId());
            child_->SetForceRenderSurfaceForTesting(false);
            break;
        case 3:
            layer_tree_host()->SetViewportRectAndScale(gfx::Rect(root_->bounds()), 1.f, GetCurrentLocalSurfaceId());
        }
    }
...
class LayerTreeHostTestSynchronousCompositeSwapPromise : public LayerTreeHostTest {
public:
        ...
        scoped_refptr<viz::RasterContextProvider> compositor_context_provider, scoped_refptr<viz::RasterContextProvider> worker_context_provider) override
    {
        ...
        return std::make_unique<TestLayerTreeFrameSink>(compositor_context_provider, std::move(worker_context_provider), gpu_memory_buffer_manager(),
            renderer_settings, &debug_settings_, task_runner_provider(), synchronous_composite, disable_display_vsync, refresh_rate);
    }
    ...
    void BeginTest() override
    {
        ...
        std::unique_ptr<SwapPromise> swap_promise0(new TestSwapPromise(&swap_promise_result_[0]));
        layer_tree_host()->GetSwapPromiseManager()->QueueSwapPromise(std::move(swap_promise0));
        layer_tree_host()->CompositeForTest(base::TimeTicks::Now(), raster, base::OnceClosure());

        // Fail to swap (no damage) if not reclaiming resources from the Display.
        std::unique_ptr<SwapPromise> swap_promise1(new TestSwapPromise(&swap_promise_result_[1]));
        layer_tree_host()->GetSwapPromiseManager()->QueueSwapPromise(std::move(swap_promise1));
        layer_tree_host()->SetNeedsCommit();
        layer_tree_host()->CompositeForTest(base::TimeTicks::Now(), raster, base::OnceClosure());

        // Fail to draw (not visible).
        std::unique_ptr<SwapPromise> swap_promise2(new TestSwapPromise(&swap_promise_result_[2]));
        layer_tree_host()->GetSwapPromiseManager()->QueueSwapPromise(std::move(swap_promise2));
...
class LayerTreeHostAcceptsDeltasFromImplWithoutRootLayer : public LayerTreeHostTest {
public:
    ...
    void ApplyViewportChanges(const ApplyViewportChangesArgs& args) override
    {
        ...
        EXPECT_EQ(commit_data_.browser_controls_constraint, args.browser_controls_constraint);
...
public:
    ...
    void BeginMainFrame(const viz::BeginFrameArgs& args) override
    {
        EXPECT_EQ(num_blocking_calls_, NumCallsToWaitForProtectedSequenceCompletion());
        EndTest();
    }
...
protected:
    LayerTreeHostTestCrispUpAfterPinchEnds()
        : playback_allowed_event_(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::SIGNALED)
    {
        SetUseLayerLists();
    }
    ...
    void SetupTree() override
    {
        ...
        // pinch.
        CopyProperties(layer_tree_host()->InnerViewportScrollLayerForTesting(), layer.get());
        root->AddChild(layer);
    ...
    float FrameQuadScaleDeltaFromIdeal(LayerTreeHostImpl::FrameData* frame_data)
    {
        ...
        viz::CompositorRenderPass* root_pass = frame_data->render_passes.back().get();
        for (auto* draw_quad : root_pass->quad_list) {
            ...
            float transform_scale = SkScalarToFloat(quad->shared_quad_state->quad_to_target_transform.rc(0, 0));
    ...
    void PostNextAfterDraw(LayerTreeHostImpl* host_impl)
    {
        if (posted_)
            return;
        posted_ = true;
        ImplThreadTaskRunner()->PostDelayedTask(FROM_HERE, base::BindOnce(&LayerTreeHostTestCrispUpAfterPinchEnds::Next, base::Unretained(this), host_impl),
    ...
    void Next(LayerTreeHostImpl* host_impl)
    {
        ...
        case 2:
            // Pinch zoom in.
            host_impl->GetInputHandler().PinchGestureBegin(gfx::Point(100, 100), ui::ScrollInputType::kWheel);
            host_impl->GetInputHandler().PinchGestureUpdate(1.5f, gfx::Point(100, 100));
            host_impl->GetInputHandler().PinchGestureEnd(gfx::Point(100, 100));
            break;
        case 3:
            // Pinch zoom back to 1.f but don't end it.
            host_impl->GetInputHandler().PinchGestureBegin(gfx::Point(100, 100), ui::ScrollInputType::kWheel);
            host_impl->GetInputHandler().PinchGestureUpdate(1.f / 1.5f, gfx::Point(100, 100));
            break;
        case 4:
    ...
    void NotifyTileStateChangedOnThread(LayerTreeHostImpl* host_impl, const Tile* tile) override
    {
        if (frame_ == 3) {
            ...
            // flakiness.
            EXPECT_EQ(gfx::AxisTransform2d(0.75f, gfx::Vector2dF()), tile->raster_transform());
            PostNextAfterDraw(host_impl);
        }
...
protected:
    ...
    void SetupTree() override
    {
        ...
        layer_tree_host()->SetRootLayer(root);
        LayerTreeHostTest::SetupTree();
        layer_tree_host()->SetViewportRectAndScale(gfx::Rect(viewport_size_), 1.f, viz::LocalSurfaceId());
        client_.set_bounds(root->bounds());
    }
...
class LayerTreeHostTestContinuousDrawWhenCreatingVisibleTiles : public LayerTreeHostTest {
protected:
...
public:
    ...
    void BeginTest() override
    {
        layer_tree_host()->SetViewportRectAndScale(gfx::Rect(16, 16), 1.f, viz::LocalSurfaceId());
        PostSetNeedsCommitToMainThread();
    }
    ...
    void AfterTest() override
    {
        ...
        EXPECT_LE(notify_ready_to_activate_count_, 1u + scheduled_prepare_tiles_count_);
    }
...
class LayerTreeHostTestNoTasksBetweenWillAndDidCommit : public LayerTreeHostTest {
public:
    ...
    {
        MainThreadTaskRunner()->PostTask(
            FROM_HERE, base::BindOnce(&LayerTreeHostTestNoTasksBetweenWillAndDidCommit::EndTestShouldRunAfterDidCommit, base::Unretained(this)));
    }
...
protected:
    void SetupTree() override
    {
        ...
        affected_by_page_scale_.push_back(layer_tree_host()->InnerViewportScrollLayerForTesting()->id());
        affected_by_page_scale_.push_back(layer_tree_host()->OuterViewportScrollLayerForTesting()->id());
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* host_impl) override
    {
        for (auto* layer : *host_impl->sync_tree()) {
            const std::vector<int>& list = layer->IsAffectedByPageScale() ? this->affected_by_page_scale_ : this->not_affected_by_page_scale_;
...
class LayerTreeHostTestDestroyWhileInitializingOutputSurface : public LayerTreeHostTest {
protected:
    void BeginTest() override
    {
...
protected:
    ...
    void DidReceivePresentationTimeOnThread(LayerTreeHostImpl* host_impl, uint32_t frame_token, const gfx::PresentationFeedback& feedback) override
    {
...
protected:
    ...
    {
        EXPECT_EQ(DrawResult::kSuccess, draw_result);
        EXPECT_EQ(GetCurrentLocalSurfaceId(), host_impl->active_tree()->local_surface_id_from_parent());
        return draw_result;
    }

    void DisplayReceivedLocalSurfaceIdOnThread(const viz::LocalSurfaceId& local_surface_id) override
    {
...
protected:
    ...
    void DisplayReceivedLocalSurfaceIdOnThread(const viz::LocalSurfaceId& local_surface_id) override
    {
        const viz::LocalSurfaceId& expected_parent_local_surface_id = GetCurrentLocalSurfaceId();
        viz::LocalSurfaceId child_local_surface_id(expected_parent_local_surface_id.parent_sequence_number(),
            expected_parent_local_surface_id.child_sequence_number() + 1, expected_parent_local_surface_id.embed_token());
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        EXPECT_FALSE(host_impl->active_tree()->new_local_surface_id_request_for_testing());
        EndTest();
    }
};
...
protected:
    ...
    {
        client_.set_fill_with_nonsolid_color(true);
        client_.add_draw_image(CreateDiscardablePaintImage(large_image_size_), gfx::Point(0, 0));
        scoped_refptr<FakePictureLayer> root = FakePictureLayer::Create(&client_);
        root->SetBounds(gfx::Size(10000, 10000));
        client_.set_bounds(root->bounds());
        root->SetContentsOpaque(true);
    ...
    void InitializedRendererOnThread(LayerTreeHostImpl* host_impl, bool success) override
    {
        ...
        viz::RasterContextProvider* context_provider = host_impl->layer_tree_frame_sink()->context_provider();
        ASSERT_TRUE(context_provider);

        const uint32_t max_texture_size = context_provider->ContextCapabilities().max_texture_size;
        ASSERT_GT(static_cast<uint32_t>(large_image_size_.width()), max_texture_size);
    }
...
protected:
        ...
        scoped_refptr<viz::RasterContextProvider> compositor_context_provider, scoped_refptr<viz::RasterContextProvider> worker_context_provider) override
    {
        return LayerTreeTest::CreateLayerTreeFrameSink(
            renderer_settings, refresh_rate, std::move(compositor_context_provider), std::move(worker_context_provider));
    }
    ...
    DrawResult PrepareToDrawOnThread(LayerTreeHostImpl* host_impl, LayerTreeHostImpl::FrameData* frame, DrawResult draw_result) override
    {
        frame->render_passes.clear();

        viz::CompositorRenderPass* child_pass
            = AddRenderPass(&frame->render_passes, viz::CompositorRenderPassId { 2 }, gfx::Rect(3, 3, 100, 100), gfx::Transform(), FilterOperations());
        std::vector<viz::ResourceId> child_resources = AddOneOfEveryQuadType(child_pass, host_impl->resource_provider(), viz::CompositorRenderPassId { 0 });

        viz::CompositorRenderPass* pass
            = AddRenderPass(&frame->render_passes, viz::CompositorRenderPassId { 1 }, gfx::Rect(3, 3, 100, 100), gfx::Transform(), FilterOperations());
        std::vector<viz::ResourceId> root_resources = AddOneOfEveryQuadType(pass, host_impl->resource_provider(), child_pass->id);

        auto append = [](std::vector<viz::ResourceId>* to, std::vector<viz::ResourceId> from) { to->insert(to->end(), from.begin(), from.end()); };
...
protected:
    ...
    void DisplayReceivedCompositorFrameOnThread(const viz::CompositorFrame& frame) override
    {
        ...
        EXPECT_EQ(viz::BeginFrameAck(current_begin_frame_args_, true), frame.metadata.begin_frame_ack);
    }
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        EXPECT_EQ(viz::BeginFrameAck(current_begin_frame_args_, true), frame_data_->begin_frame_ack);
        EndTest();
    }
...
protected:
    ...
    void WillBeginMainFrame() override
    {
        ...
        image_ = DrawImage(CreateDiscardablePaintImage(gfx::Size(400, 400)), false, SkIRect::MakeWH(400, 400), PaintFlags::FilterQuality::kNone, SkM44(),
            PaintImage::kDefaultFrameIndex, TargetColorParams());
        auto callback = base::BindRepeating(&LayerTreeHostTestQueueImageDecode::ImageDecodeFinished, base::Unretained(this));
    ...
    void ReadyToCommitOnThread(LayerTreeHostImpl* impl) override
    {
        if (one_commit_done_)
            return;
        EXPECT_FALSE(impl->tile_manager()->checker_image_tracker().ShouldCheckerImage(image_, WhichTree::PENDING_TREE));
    }
...
protected:
    ...
    void WillBeginMainFrame() override
    {
        ...
        PaintImage image = PaintImageBuilder::WithDefault()
                               ...
                               .set_image(SkImages::RasterFromBitmap(bitmap_), PaintImage::GetNextContentId())
                               .TakePaintImage();
        auto callback = base::BindOnce(&LayerTreeHostTestQueueImageDecodeNonLazy::ImageDecodeFinished, base::Unretained(this));
...
public:
    explicit LayerTreeHostTestImageAnimation(viz::RendererType type = kDefaultRendererType)
    ...
    void SetupTree() override
    {
        ...
        std::vector<FrameMetadata> frames
            = { FrameMetadata(true, base::Seconds(1)), FrameMetadata(true, base::Seconds(1)), FrameMetadata(true, base::Seconds(1)) };
        generator_ = sk_make_sp<FakePaintImageGenerator>(SkImageInfo::MakeN32Premul(500, 500, SkColorSpace::MakeSRGB()), frames);
        PaintImage image = PaintImageBuilder::WithDefault()
                               .set_id(PaintImage::GetNextId())
        ...
        layer_tree_host()->SetRootLayer(FakePictureLayer::Create(&content_layer_client_));
    ...
    void WillPrepareToDrawOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        auto* layer = static_cast<PictureLayerImpl*>(host_impl->active_tree()->root_layer());
        switch (++draw_count_) {
        case 1:
            // First draw, everything is invalid. layer->update_rect() doesn't
            // matter because a new layer always causes surface damage.
            EXPECT_EQ(layer->InvalidationForTesting().bounds(), gfx::Rect(layer->bounds()));
        ...
        case 2:
            ...
            EXPECT_EQ(layer->GetDamageReasons(), DamageReasonSet { DamageReason::kAnimatedImage });
            break;
        case 3:
            EXPECT_EQ(layer->InvalidationForTesting().bounds(), image_rect);
            EXPECT_EQ(layer->update_rect(), image_rect);
    ...
    void WillSubmitCompositorFrame(LayerTreeHostImpl* host_impl, const viz::CompositorFrame& frame) override
    {
        const viz::FrameIntervalInputs& inputs = frame.metadata.frame_interval_inputs;
        ...
        case 2:
            ...
            EXPECT_EQ(inputs.content_interval_info[0].type, viz::ContentFrameIntervalType::kAnimatingImage);
            EXPECT_EQ(inputs.content_interval_info[0].frame_interval, base::Seconds(1));
...
class LayerTreeHostTestImageAnimationDrawImage : public LayerTreeHostTestImageAnimation {
public:
    explicit LayerTreeHostTestImageAnimationDrawImage(viz::RendererType type = kDefaultRendererType)
...
class LayerTreeHostTestImageAnimationDrawImageShader : public LayerTreeHostTestImageAnimation {
    void AddImageOp(const PaintImage& image) override
    {
        PaintFlags flags;
        flags.setShader(PaintShader::MakeImage(image, SkTileMode::kRepeat, SkTileMode::kRepeat, nullptr));
...
class LayerTreeHostTestImageAnimationDrawRecordShader : public LayerTreeHostTestImageAnimation {
    void AddImageOp(const PaintImage& image) override
    {
        PaintOpBuffer buffer;
        buffer.push<DrawImageOp>(image, 0.f, 0.f);
        PaintFlags flags;
        flags.setShader(PaintShader::MakePaintRecord(buffer.ReleaseAsRecord(), SkRect::MakeWH(500, 500), SkTileMode::kClamp, SkTileMode::kClamp, nullptr));
        content_layer_client_.add_draw_rect(gfx::Rect(500, 500), flags);
    }
};
...
class LayerTreeHostTestImageAnimationPaintFilter : public LayerTreeHostTestImageAnimation {
    void AddImageOp(const PaintImage& image) override
    {
        PaintOpBuffer buffer;
        buffer.push<DrawImageOp>(image, 0.f, 0.f);
        PaintFlags flags;
        flags.setImageFilter(sk_make_sp<RecordPaintFilter>(buffer.ReleaseAsRecord(), SkRect::MakeWH(500, 500)));
        content_layer_client_.add_draw_rect(gfx::Rect(500, 500), flags);
    }
};
...
class LayerTreeHostTestImageAnimationSynchronousScheduling : public LayerTreeHostTestImageAnimationDrawImage {
public:
    explicit LayerTreeHostTestImageAnimationSynchronousScheduling(viz::RendererType type = kDefaultRendererType)
...
class LayerTreeHostTestImageAnimationSynchronousSchedulingSoftwareDraw : public LayerTreeHostTestImageAnimationSynchronousScheduling {
public:
    LayerTreeHostTestImageAnimationSynchronousSchedulingSoftwareDraw()
        : LayerTreeHostTestImageAnimationSynchronousScheduling(viz::RendererType::kSoftware)
    {
    }
...
public:
    ...
    void SetupTree() override
    {
        ...
        content_layer_client_.set_bounds(layer_size);
        content_layer_client_.set_fill_with_nonsolid_color(true);
        PaintImage async_image = PaintImageBuilder::WithCopy(CreateDiscardablePaintImage(gfx::Size(10, 10)))
                                     .set_id(1)
                                     .set_decoding_mode(PaintImage::DecodingMode::kAsync)
                                     .TakePaintImage();
        PaintImage sync_image = PaintImageBuilder::WithCopy(CreateDiscardablePaintImage(gfx::Size(10, 10)))
                                    .set_id(2)
                                    .set_decoding_mode(PaintImage::DecodingMode::kSync)
                                    .TakePaintImage();
        PaintImage unspecified_image = PaintImageBuilder::WithCopy(CreateDiscardablePaintImage(gfx::Size(10, 10)))
                                           .set_id(3)
    ...
    void WillPrepareToDrawOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        EXPECT_EQ(tracker.get_decoding_mode_hint_for_testing(1), PaintImage::DecodingMode::kAsync);
        EXPECT_EQ(tracker.get_decoding_mode_hint_for_testing(2), PaintImage::DecodingMode::kSync);
        EXPECT_EQ(tracker.get_decoding_mode_hint_for_testing(3), PaintImage::DecodingMode::kUnspecified);
        EndTest();
    }
...
protected:
    ...
    void DidCommit() override
    {
        ...
        case 1:
            ...
            break;
        case 2: {
            scoped_refptr<RasterSource> child_raster = child_->GetRecordingSourceForTesting().CreateRasterSource();
            EXPECT_FALSE(child_raster->IsSolidColor());
            scoped_refptr<RasterSource> mask_raster = mask_->GetRecordingSourceForTesting().CreateRasterSource();
...
class LayerTreeHostTestRequestForceSendMetadata : public LayerTreeHostTest, public RenderFrameMetadataObserver {
public:
    // Provides a wrapper which can be passed to LayerTreeHost, but just forwards
    // to the test class.
    class ForwardingRenderFrameMetadataObserver : public RenderFrameMetadataObserver {
    public:
        explicit ForwardingRenderFrameMetadataObserver(RenderFrameMetadataObserver* target)
            : target_(target)
        {
        }
        ...
        {
            target_->OnRenderFrameSubmission(render_frame_metadata, compositor_frame_metadata, force_send);
        }
#if BUILDFLAG(IS_ANDROID)
        void DidEndScroll() override
        {
            target_->DidEndScroll();
        }
#endif
    ...
    void BeginTest() override
    {
        ...
        layer_tree_host()->SetRenderFrameObserver(std::make_unique<ForwardingRenderFrameMetadataObserver>(this));
        layer_tree_host()->SetViewportRectAndScale(gfx::Rect(10, 10), 1.f, viz::LocalSurfaceId());
        layer_tree_host()->root_layer()->SetBounds(gfx::Size(10, 10));
...
public:
    ...
    void DisplayReceivedCompositorFrameOnThread(const viz::CompositorFrame& frame) override
    {
        ...
        case 1:
            ...
            MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&LayerTreeHostTestPartialTileDamage::DoPartialTileInvalidation, base::Unretained(this)));
...
class LayerTreeHostTopControlsDeltaTriggersViewportUpdate : public LayerTreeHostTest {
public:
    ...
    void SetupTree() override
    {
        ...
        layer_tree_host()->SetBrowserControlsParams({ kTopControlsHeight, 0, kBottomControlsHeight, 0, false /* animate */, true /* shrink */ });
        layer_tree_host()->SetBrowserControlsShownRatio(kTopControlsShownRatio, kBottomControlsShownRatio);
    }
    ...
    void BeginCommitOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        float bounds_delta = impl->pending_tree()->property_trees()->inner_viewport_container_bounds_delta().y();
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        float bounds_delta = impl->pending_tree()->property_trees()->inner_viewport_container_bounds_delta().y();
        EXPECT_EQ(bounds_delta, kTopControlsHeight - kTopControlsHeight * kTopControlsShownRatio);
        EndTest();
    }
...
public:
    ...
    {
        std::unique_ptr<BeginMainFrameMetrics> metrics = std::make_unique<BeginMainFrameMetrics>();
...
class LayerTreeHostTestDelegatedInkMetadataBase : public LayerTreeHostTest, public RenderFrameMetadataObserver {
public:
    // Provides a wrapper which can be passed to LayerTreeHost, but just forwards
    // to the test class.
    class ForwardingRenderFrameMetadataObserver : public RenderFrameMetadataObserver {
    ...
    void BeginTest() override
    {
        ...
        layer_tree_host()->SetDelegatedInkMetadata(std::make_unique<gfx::DelegatedInkMetadata>(expected_metadata_.value()));
    }

    // Check DelegatedInkMetadata on the CompositorFrameMetadata when the
    // compositor frame is submitted.
    void ExpectMetadata(std::optional<DelegatedInkBrowserMetadata> browser_delegated_ink_metadata, gfx::DelegatedInkMetadata* actual_metadata)
    {
        if (expected_metadata_.has_value()) {
            EXPECT_TRUE(browser_delegated_ink_metadata.has_value());
            EXPECT_TRUE(actual_metadata);
            EXPECT_TRUE(browser_delegated_ink_metadata.value().delegated_ink_is_hovering);
            ...
            EXPECT_EQ(expected_metadata_->is_hovering(), actual_metadata->is_hovering());
    ...
    void OnRenderFrameSubmission(
        const RenderFrameMetadata& render_frame_metadata, viz::CompositorFrameMetadata* compositor_frame_metadata, bool force_send) override
    {
        ExpectMetadata(render_frame_metadata.delegated_ink_metadata, compositor_frame_metadata->delegated_ink_metadata.get());
    }
#if BUILDFLAG(IS_ANDROID)
    void DidEndScroll() override
    {
...
class LayerTreeHostTestDelegatedInkMetadataOnAndOff : public LayerTreeHostTestDelegatedInkMetadataBase {
public:
    void DidCommitAndDrawFrame() override
    {
...
class LayerTreeHostTestDelegatedInkMetadataCompositorOnlyFrame : public viz::ExternalBeginFrameSourceClient, public LayerTreeHostTestDelegatedInkMetadataBase {
public:
    ...
    void DidFinishImplFrameOnThread(LayerTreeHostImpl* host_impl) override
    {
        if (begin_frame_count_ < 3) {
            ...
            MainThreadTaskRunner()->PostTaskAndReply(FROM_HERE,
                base::BindOnce(&LayerTreeHostTestDelegatedInkMetadataCompositorOnlyFrame::SetNeedsCommitOnMainThread, base::Unretained(this)),
                base::BindOnce(&LayerTreeHostTestDelegatedInkMetadataCompositorOnlyFrame::IssueBeginFrame, base::Unretained(this), true));
        } else if (begin_frame_count_ == 4) {
            PostIssueBeginFrame(false);
        }
        host_impl->SetNeedsRedraw();
    ...
    void PostIssueBeginFrame(bool animate_only)
    {
        // Post a new task so that BeginFrame is not issued within same callstack.
        ImplThreadTaskRunner()->PostTask(
            FROM_HERE, base::BindOnce(&LayerTreeHostTestDelegatedInkMetadataCompositorOnlyFrame::IssueBeginFrame, base::Unretained(this), animate_only));
    }
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostTestDelegatedInkMetadataCompositorOnlyFrame);
#endif
...
protected:
    ...
    {
        MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&LayerTreeHostTestEventsMetrics::SimulateEventOnMain, base::Unretained(this)));
    }
    ...
    {
        MainThreadTaskRunner()->PostTask(
            FROM_HERE, base::BindOnce(&LayerTreeHostTestEventsMetrics::VerifyMainSavedEventsMetricsCountOnMain, base::Unretained(this), count));
    }

    // Verifies the number of events metrics copied from the main thread to the
    // impl thread.
    void VerifyImplEventsMetricsFromMainCount(LayerTreeHostImpl* impl, size_t count) const
    {
        EXPECT_EQ(count, impl->active_tree()->events_metrics_from_main_thread_count_for_testing());
    }
...
private:
    void SimulateEventOnMain()
    {
        ...
        tick_clock.Advance(base::Microseconds(10));
        std::unique_ptr<EventMetrics> metrics = ScrollUpdateEventMetrics::CreateForTesting(ui::EventType::kGestureScrollUpdate, ui::ScrollInputType::kWheel,
            /*is_inertial=*/false, ScrollUpdateEventMetrics::ScrollUpdateType::kContinued,
            /*delta=*/10.0f, event_time, arrived_in_browser_main_timestamp, &tick_clock, std::nullopt);
        DCHECK_NE(metrics, nullptr);
        {
            tick_clock.Advance(base::Microseconds(10));
            metrics->SetDispatchStageTimestamp(EventMetrics::DispatchStage::kRendererCompositorStarted);
            auto done_callback = base::BindOnce(
                [](std::unique_ptr<EventMetrics> metrics, base::SimpleTestTickClock* tick_clock, bool handled) {
                    tick_clock->Advance(base::Microseconds(10));
                    metrics->SetDispatchStageTimestamp(EventMetrics::DispatchStage::kRendererCompositorFinished);
                    std::unique_ptr<EventMetrics> result = handled ? std::move(metrics) : nullptr;
                    return result;
                },
                std::move(metrics), &tick_clock);
            auto scoped_event_monitor = layer_tree_host()->GetScopedEventMetricsMonitor(std::move(done_callback));
            layer_tree_host()->SetNeedsAnimate();
        }
    ...
    {
        EXPECT_EQ(count, layer_tree_host()->saved_events_metrics_count_for_testing());
    }
};
...
class LayerTreeHostTestKeepEventsMetricsForVisibility : public LayerTreeHostTestEventsMetrics {
protected:
    void WillBeginImplFrameOnThread(LayerTreeHostImpl* impl, const viz::BeginFrameArgs& args, bool has_damage) override
    {
    ...
    void BeginMainFrameAbortedOnThread(LayerTreeHostImpl* impl, CommitEarlyOutReason reason, bool /* did_sync_scroll_and_viewport */) override
    {
...
private:
    ...
    {
        MainThreadTaskRunner()->PostTask(
            FROM_HERE, base::BindOnce(&LayerTreeHostTestKeepEventsMetricsForVisibility::SetLayerTreeHostVisibleOnMain, base::Unretained(this), visible));
    }
...
// commit happens.
class LayerTreeHostTestKeepEventsMetricsForDeferredMainFrameUpdate : public LayerTreeHostTestEventsMetrics {
protected:
    void WillBeginImplFrameOnThread(LayerTreeHostImpl* impl, const viz::BeginFrameArgs& args, bool has_damage) override
    {
        // Skip if we have already received a begin-impl-frame and acted on it.
...
private:
    ...
    {
        MainThreadTaskRunner()->PostTask(
            FROM_HERE, base::BindOnce(&LayerTreeHostTestKeepEventsMetricsForDeferredMainFrameUpdate::DeferMainFrameUpdateOnMain, base::Unretained(this)));
    }
    ...
    {
        MainThreadTaskRunner()->PostTask(FROM_HERE,
            base::BindOnce(&LayerTreeHostTestKeepEventsMetricsForDeferredMainFrameUpdate::StopDeferringMainFrameUpdateOnMain, base::Unretained(this)));
    }
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostTestKeepEventsMetricsForDeferredMainFrameUpdate);
...
class LayerTreeHostTestKeepEventsMetricsForDeferredCommit : public LayerTreeHostTestEventsMetrics {
protected:
    void WillBeginImplFrameOnThread(LayerTreeHostImpl* impl, const viz::BeginFrameArgs& args, bool has_damage) override
    {
        // Skip if we have already received a begin-impl-frame and acted on it.
...
private:
    ...
    {
        layer_tree_host()->StartDeferringCommits(base::Days(1), PaintHoldingReason::kFirstContentfulPaint);
    }

    void PostDeferCommit()
    {
        MainThreadTaskRunner()->PostTask(
            FROM_HERE, base::BindOnce(&LayerTreeHostTestKeepEventsMetricsForDeferredCommit::DeferCommitOnMain, base::Unretained(this)));
    }

    void StopDeferringCommitOnMain()
    {
        layer_tree_host()->StopDeferringCommits(PaintHoldingCommitTrigger::kFirstContentfulPaint);
    }

    void PostStopDeferringCommit()
    {
        MainThreadTaskRunner()->PostTask(
            FROM_HERE, base::BindOnce(&LayerTreeHostTestKeepEventsMetricsForDeferredCommit::StopDeferringCommitOnMain, base::Unretained(this)));
    }

    bool received_will_begin_impl_frame_ = false;
};

SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostTestKeepEventsMetricsForDeferredCommit);
...
class LayerTreeHostTestIgnoreEventsMetricsForNoUpdate : public LayerTreeHostTestEventsMetrics {
protected:
    void WillBeginImplFrameOnThread(LayerTreeHostImpl* impl, const viz::BeginFrameArgs& args, bool has_damage) override
    {
...
public:
    ...
    void DidFinishImplFrameOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        host_impl->dropped_frame_counter()->OnEndFrame(last_args_, CreateFakeImplDroppedFrameInfo());
        host_impl->SetNeedsRedraw();
        --frames_counter_;
    }
...
class LayerTreeHostTestViewTransitionsPropagatedToMetadata : public LayerTreeHostTest {
protected:
    void SetupTree() override
    {
        SetInitialRootBounds(gfx::Size(10, 10));
    ...
    void BeginTest() override
    {
        layer_tree_host()->AddViewTransitionRequest(ViewTransitionRequest::CreateCapture(blink::ViewTransitionToken(), /*maybe_cross_frame_sink=*/false, {},
            base::BindLambdaForTesting([this](const viz::ViewTransitionElementResourceRects&) { CommitLambdaCalled(); })));
    }

    void CommitLambdaCalled()
    {
        ++num_lambda_calls_;
    }
    ...
    void DisplayReceivedCompositorFrameOnThread(const viz::CompositorFrame& frame) override
    {
        ...
        EXPECT_EQ(save.type(), viz::CompositorFrameTransitionDirective::Type::kSave);
    }

    void DidPresentCompositorFrame(uint32_t frame_token, const viz::FrameTimingDetails& frame_timing_details) override
    {
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostTestViewTransitionsPropagatedToMetadata);
...
public:
    scoped_refptr<FakePictureLayer> CreateAndAddFakePictureLayer(const gfx::Size& size, Layer* parent = nullptr)
    {
        if (!parent)
...
class LayerTreeHostTestHideLayerAndSubtree : public LayerTreeHostTestWithHelper {
public:
    void InitializeSettings(LayerTreeSettings* settings) override
    {
    ...
    {
        ++num_commits_;
        FakePictureLayerImpl* picture_layer_impl = static_cast<FakePictureLayerImpl*>(impl->sync_tree()->LayerById(picture_layer_->id()));
        switch (num_commits_) {
        case 1:
    ...
    void DidCommit() override
    {
        picture_layer_->SetHideLayerAndSubtree(true);
    }
...
class LayerTreeHostTestHideLayerAndSubtreeOnParent : public LayerTreeHostTestWithHelper {
public:
    void InitializeSettings(LayerTreeSettings* settings) override
    {
        settings->release_tile_resources_for_hidden_layers = true;
    ...
    void BeginTest() override
    {
        ...
        picture_layer_ = CreateAndAddFakePictureLayer(gfx::Size(10, 10), parent_layer_.get());
...
class LayerTreeHostTestOccludedTileReleased : public LayerTreeHostTestWithHelper {
public:
    void InitializeSettings(LayerTreeSettings* settings) override
    {
        settings->memory_policy.priority_cutoff_when_visible = gpu::MemoryAllocation::CUTOFF_ALLOW_REQUIRED_ONLY;
    ...
    void NotifyTileStateChangedOnThread(LayerTreeHostImpl* host_impl, const Tile* tile) override
    {
        FakePictureLayerImpl* picture_layer_impl = static_cast<FakePictureLayerImpl*>(host_impl->sync_tree()->LayerById(picture_layer_->id()));
        EXPECT_EQ(1u, picture_layer_impl->GetNumberOfTilesWithResources());
        MainThreadTaskRunner()->PostTask(
            FROM_HERE, base::BindOnce(&LayerTreeHostTestOccludedTileReleased::AddLayerThatObscuresPictureLayer, base::Unretained(this)));
    }
...
class LayerTreeHostTestDelayRecreateTiling : public LayerTreeHostTestWithHelper {
public:
    ...
    void SetupTree() override
    {
        ...
        layer_on_main_ = CreateAndAddFakePictureLayer(gfx::Size(30, 30), root_layer.get());
    ...
    {
        TransformTree& transform_tree = layer_tree_host()->property_trees()->transform_tree_mutable();
        TransformNode* node = transform_tree.Node(layer_on_main_->transform_tree_index());
        ...
        case 1:
            ...
            transform_tree.OnTransformAnimated(layer_on_main_->element_id(), transform);
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* host_impl) override
    {
        FakePictureLayerImpl* layer_impl = static_cast<FakePictureLayerImpl*>(host_impl->pending_tree()->LayerById(layer_id_));

        TransformTree& transform_tree = host_impl->pending_tree()->property_trees()->transform_tree_mutable();
        TransformNode* node = transform_tree.Node(layer_impl->transform_tree_index());
    ...
    void DidActivateTreeOnThread(LayerTreeHostImpl* host_impl) override
    {
        FakePictureLayerImpl* layer_impl = static_cast<FakePictureLayerImpl*>(host_impl->active_tree()->LayerById(layer_id_));

        gfx::AxisTransform2d tiling_transform = layer_impl->HighResTiling()->raster_transform();
...
class LayerTreeHostTestInvalidateImplSideForRerasterTiling : public LayerTreeHostTestWithHelper {
public:
    void SetupTree() override
    {
        client_.set_fill_with_nonsolid_color(true);
    ...
    void ClearAnimationForLayer(LayerTreeImpl* tree_impl, FakePictureLayerImpl* layer_impl)
    {
        if (!tree_impl)
            return;

        TransformTree& transform_tree = tree_impl->property_trees()->transform_tree_mutable();
        TransformNode* node = transform_tree.Node(layer_impl->transform_tree_index());
    ...
    TransformNode* TransformNodeForLayer(LayerTreeImpl* tree_impl, FakePictureLayerImpl* layer_impl)
    {
        TransformTree& transform_tree = tree_impl->property_trees()->transform_tree_mutable();
        TransformNode* node = transform_tree.Node(layer_impl->transform_tree_index());
        return node;
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* host_impl) override
    {
        if (host_impl->active_tree()->source_frame_number() == 2) {
            FakePictureLayerImpl* target_layer = static_cast<FakePictureLayerImpl*>(host_impl->active_tree()->LayerById(layer_id_));
            gfx::AxisTransform2d tiling_transform = target_layer->HighResTiling()->raster_transform();
            TransformNode* node = TransformNodeForLayer(host_impl->active_tree(), target_layer);
...
class LayerTreeHostTestNeedsNotifyReadyToDrawAndActivate : public LayerTreeHostTest {
public:
    void BeginTest() override
    {
        PostSetNeedsCommitToMainThread();
...
class LayerTreeHostTestNeedsNotifyReadyToDrawOnly : public LayerTreeHostTestNeedsNotifyReadyToDrawAndActivate {
public:
...
class LayerTreeHostTestNeedsNotifyReadyToActivateOnly : public LayerTreeHostTestNeedsNotifyReadyToDrawAndActivate {
public:
...
protected:
    ...
    std::unique_ptr<TestLayerTreeFrameSink> CreateLayerTreeFrameSink(const viz::RendererSettings& renderer_settings, double refresh_rate,
        scoped_refptr<viz::RasterContextProvider> compositor_context_provider, scoped_refptr<viz::RasterContextProvider> worker_context_provider) override
    {
        std::unique_ptr<TestLayerTreeFrameSink> frame_sink = LayerTreeHostTest::CreateLayerTreeFrameSink(
            renderer_settings, refresh_rate, std::move(compositor_context_provider), std::move(worker_context_provider));
        layer_tree_frame_sink_ = frame_sink.get();
        return frame_sink;
    }
...
public:
    LayerTreeHostUpdateViewportContainerSize()
    {
        SetUseLayerLists();
    }
    ...
    void SetupTree() override
    {
        ...
        layer_tree_host()->SetBrowserControlsShownRatio(top_shown_ratio_, bottom_shown_ratio_);
    }
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        gfx::Vector2dF pending_bounds_delta = host_impl->pending_tree()->property_trees()->inner_viewport_container_bounds_delta();
        gfx::Vector2dF active_bounds_delta = host_impl->active_tree()->property_trees()->inner_viewport_container_bounds_delta();
        switch (host_impl->pending_tree()->source_frame_number()) {
        case 1:
        case 2:
        case 3:
...
// and animations.
class LayerTreeHostTestForceRecreateTilingForLCDText : public LayerTreeHostTestWithHelper {
public:
    ...
    void WillCommit(const CommitState&) override
    {
        ...
        case 3:
            ...
            layer_on_main_->SetContentsOpaque(false);
            layer_on_main_->SetBackgroundColor(SkColor4f::FromColor(SK_ColorTRANSPARENT));
            break;
        }
    }
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        case 0:
            ...
            host_impl->GetInputHandler().PinchGestureBegin(gfx::Point(1, 1), ui::ScrollInputType::kWheel);
            PostSetNeedsCommitToMainThread();
            break;
        case 1:
            ASSERT_TRUE(host_impl->IsPinchGestureActive());
            EXPECT_FALSE(layer_impl->can_use_lcd_text());
            EXPECT_EQ(layer_impl->lcd_text_disallowed_reason(), LCDTextDisallowedReason::kContentsNotOpaque);
        ...
        case 3:
            ASSERT_TRUE(host_impl->IsPinchGestureActive());
            EXPECT_FALSE(layer_impl->can_use_lcd_text());
            EXPECT_EQ(layer_impl->lcd_text_disallowed_reason(), LCDTextDisallowedReason::kBackgroundColorNotOpaque);
            EXPECT_FALSE(layer_impl->HighResTiling()->can_use_lcd_text());
            EndTest();
            break;
        }
...
class LayerTreeHostTestDamagePropagatesFromViewTransitionSurface : public LayerTreeHostTest {
public:
    ...
    void BeginMainFrame(const viz::BeginFrameArgs& args) override
    {
        ...
        case 3:
            ...
            // layer's target.
            layer_with_view_transition_content_->SetNeedsDisplayRect(gfx::Rect(0, 0, 5, 5));
        }
    }
    ...
    void SetupTree() override
    {
        ...
        layer_with_view_transition_content_->SetBackgroundColor(SkColor4f::FromColor(SK_ColorBLUE));
        layer_with_view_transition_content_->SetBounds(layer_with_view_transition_content_rect_.size());
        layer_with_view_transition_content_->SetOffsetToTransformParent(layer_with_view_transition_content_rect_.OffsetFromOrigin());
        ...
        view_transition_layer_ = ViewTransitionContentLayer::Create(resource_id_, /*is_live_content_layer=*/true);
        ...
        view_transition_layer_->SetOffsetToTransformParent(view_transition_layer_rect_.OffsetFromOrigin());
        ...
        unrelated_layer_->SetOffsetToTransformParent(unrelated_layer_rect_.OffsetFromOrigin());
        unrelated_layer_->SetBackgroundColor(SkColor4f::FromColor(SK_ColorRED));
        root->AddChild(unrelated_layer_);

        auto& layer_with_view_transition_content_node = CreateEffectNode(layer_with_view_transition_content_.get(), kContentsRootPropertyNodeId);
        layer_with_view_transition_content_->SetEffectTreeIndex(layer_with_view_transition_content_node.id);
        layer_with_view_transition_content_node.view_transition_element_resource_id = resource_id_;
        layer_with_view_transition_content_node.render_surface_reason = RenderSurfaceReason::kViewTransitionParticipant;

        auto& view_transition_layer_node = CreateEffectNode(view_transition_layer_.get(), kContentsRootPropertyNodeId);
        view_transition_layer_->SetEffectTreeIndex(view_transition_layer_node.id);
        view_transition_layer_node.render_surface_reason = RenderSurfaceReason::kBlendMode;
    }
    ...
    void WillSubmitCompositorFrame(LayerTreeHostImpl* host_impl, const viz::CompositorFrame& frame) override
    {
        ...
        case 0: {
            ...
            EXPECT_EQ(vt_content_pass->view_transition_element_resource_id, resource_id_);
            EXPECT_EQ(vt_content_pass->output_rect, layer_with_view_transition_content_rect_);
            EXPECT_TRUE(vt_content_pass->has_damage_from_contributing_content);

            const auto& vt_layer_pass = frame.render_pass_list.at(1);
            EXPECT_FALSE(vt_layer_pass->view_transition_element_resource_id.IsValid());
        ...
        case 1: {
            ...
            const auto& root_pass = frame.render_pass_list.back();
            EXPECT_EQ(root_pass->damage_rect, gfx::Rect(unrelated_layer_rect_.origin(), gfx::Size(5, 5)));
            PostSetNeedsCommitToMainThread();
            break;
        }
        ...
        case 2: {
            ...
            const auto& root_pass = frame.render_pass_list.back();
            EXPECT_EQ(root_pass->damage_rect, gfx::Rect(view_transition_layer_rect_.origin(), gfx::Size(5, 5)));
            PostSetNeedsCommitToMainThread();
            break;
        }
        ...
        case 3: {
            ...
            gfx::Rect total_damage(layer_with_view_transition_content_rect_.origin(), gfx::Size(5, 5));
            total_damage.Union(gfx::Rect(view_transition_layer_rect_.origin(), gfx::Size(5, 5)));
...
private:
    ...
    const gfx::Rect layer_with_view_transition_content_rect_ = gfx::Rect(0, 0, 10, 10);
...
protected:
    ...
    void DidBeginMainFrame() override
    {
        EXPECT_EQ(main_frame_num_ % 2 == 0, layer_tree_host()->WaitedForCommitForTesting());
    }
    void DidCommit() override
    {
...
class LayerTreeHostTestDetachInputDelegateAndRenderFrameObserver : public LayerTreeTest, StubInputHandlerClient {
public:
    void BeginTest() override
    {
        impl_task_runner_ = layer_tree_host()->GetTaskRunnerProvider()->ImplThreadTaskRunner();

        layer_tree_host()->SetRenderFrameObserver(std::make_unique<FakeRenderFrameMetadataObserver>(this));

        CompletionEvent event;
        impl_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(&LayerTreeHostTestDetachInputDelegateAndRenderFrameObserver::SetUpInputHandlerOnImpl, base::Unretained(this),
                base::Unretained(&event), layer_tree_host()->GetDelegateForInput()));
        event.Wait();
    ...
    void DidCommit() override
    {
        ...
        impl_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(
                &LayerTreeHostTestDetachInputDelegateAndRenderFrameObserver::AssertDestroyedOnImpl, base::Unretained(this), base::Unretained(&event)));
        event.Wait();
...
private:
    ...
    public:
        FakeRenderFrameMetadataObserver(LayerTreeHostTestDetachInputDelegateAndRenderFrameObserver* test)
            : test_(test)
        {
        }
    ...
    void SetUpInputHandlerOnImpl(CompletionEvent* event, base::WeakPtr<CompositorDelegateForInput> compositor_delegate)
    {
        CHECK(compositor_delegate);
        base::WeakPtr<InputHandler> input_handler = InputHandler::Create(*compositor_delegate);
        CHECK(input_handler);
