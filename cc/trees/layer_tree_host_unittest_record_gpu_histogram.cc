...
TEST(LayerTreeHostRecordGpuHistogramTest, SingleThreaded)
{
    ...
    std::unique_ptr<FakeLayerTreeHost> host
        = FakeLayerTreeHost::Create(&host_client, &task_graph_runner, animation_host.get(), settings, CompositorMode::SINGLE_THREADED);
    EXPECT_FALSE(host->GetPendingCommitState()->needs_gpu_rasterization_histogram);
    host->CreateFakeLayerTreeHostImpl();
    auto commit_state = host->WillCommit(/*completion=*/nullptr, /*has_updates=*/true);
...
TEST(LayerTreeHostRecordGpuHistogramTest, Threaded)
{
    ...
    auto animation_host = AnimationHost::CreateForTesting(ThreadInstance::kMain);
    std::unique_ptr<FakeLayerTreeHost> host
        = FakeLayerTreeHost::Create(&host_client, &task_graph_runner, animation_host.get(), settings, CompositorMode::THREADED);
