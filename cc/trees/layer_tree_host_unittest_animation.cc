...
#define SAMPLE(curve, time)                                                                                                                                    \
    curve->GetTransformedValue(time, time < base::TimeDelta() ? gfx::TimingFunction::LimitDirection::LEFT : gfx::TimingFunction::LimitDirection::RIGHT)
...
public:
    ...
    void GetImplTimelineAndAnimationByID(const LayerTreeHostImpl& host_impl)
    {
        ...
        animation_child_impl_ = timeline_impl_->GetAnimationById(animation_child_id_);
    ...
    AnimationHost* GetImplAnimationHost(const LayerTreeHostImpl* host_impl) const
    {
...
// be set.
class LayerTreeHostAnimationTestSetNeedsAnimateShouldNotSetCommitRequested : public LayerTreeHostAnimationTest {
public:
...
// flow.
class LayerTreeHostAnimationTestSetNeedsAnimateInsideAnimationCallback : public LayerTreeHostAnimationTest {
public:
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostAnimationTestSetNeedsAnimateInsideAnimationCallback);

// Add a layer animation and confirm that
// LayerTreeHostImpl::UpdateAnimationState does get called.
class LayerTreeHostAnimationTestAddKeyframeModel : public LayerTreeHostAnimationTest {
public:
    ...
    void UpdateAnimationState(LayerTreeHostImpl* host_impl, bool has_unfinished_animation) override
    {
    ...
    void NotifyAnimationStarted(base::TimeTicks monotonic_time, int target_property, int group) override
    {
        ...
        gfx::KeyframeModel* keyframe_model = animation_->GetKeyframeModel(TargetProperty::OPACITY);
        if (keyframe_model)
...
class LayerTreeHostAnimationTestNoDamageAnimation : public LayerTreeHostAnimationTest {
...
public:
    ...
    void AnimateLayers(LayerTreeHostImpl* host_impl, base::TimeTicks monotonic_time) override
    {
    ...
    void NotifyAnimationFinished(base::TimeTicks monotonic_time, int target_property, int group) override
    {
        base::AutoLock auto_lock(lock_);
...
class LayerTreeHostAnimationTestCheckerboardDoesNotStarveDraws : public LayerTreeHostAnimationTest {
public:
...
class LayerTreeHostAnimationTestAnimationsGetDeleted : public LayerTreeHostAnimationTest {
public:
    ...
    void AnimateLayers(LayerTreeHostImpl* host_impl, base::TimeTicks monotonic_time) override
    {
        bool have_animations = !GetImplAnimationHost(host_impl)->ticking_animations_for_testing().empty();
...
class LayerTreeHostAnimationTestAddKeyframeModelWithTimingFunction : public LayerTreeHostAnimationTest {
public:
    ...
    void AnimateLayers(LayerTreeHostImpl* host_impl, base::TimeTicks monotonic_time) override
    {
        ...
        scoped_refptr<Animation> animation_child_impl = timeline_impl->GetAnimationById(animation_child_id_);

        KeyframeModel* keyframe_model = animation_child_impl->GetKeyframeModel(TargetProperty::OPACITY);

        const gfx::FloatAnimationCurve* curve = gfx::FloatAnimationCurve::ToFloatAnimationCurve(keyframe_model->curve());
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostAnimationTestAddKeyframeModelWithTimingFunction);

// Ensures that main thread animations have their start times synchronized with
// impl thread animations.
class LayerTreeHostAnimationTestSynchronizeAnimationStartTimes : public LayerTreeHostAnimationTest {
public:
    void SetupTree() override
    {
        LayerTreeHostAnimationTest::SetupTree();
    ...
    void NotifyAnimationStarted(base::TimeTicks monotonic_time, int target_property, int group) override
    {
        KeyframeModel* keyframe_model = animation_child_->GetKeyframeModel(TargetProperty::OPACITY);
        main_start_time_ = keyframe_model->start_time();
        animation_child_->RemoveKeyframeModel(keyframe_model->id());
        EndTest();
    }

    void UpdateAnimationState(LayerTreeHostImpl* impl_host, bool has_unfinished_animation) override
    {
        scoped_refptr<const AnimationTimeline> timeline_impl = GetImplAnimationHost(impl_host)->GetTimelineById(timeline_id_);
        scoped_refptr<Animation> animation_child_impl = timeline_impl->GetAnimationById(animation_child_id_);
...
class LayerTreeHostAnimationTestAnimationFinishedEvents : public LayerTreeHostAnimationTest {
public:
    void BeginTest() override
    {
        AttachAnimationsToTimeline();
    ...
    void NotifyAnimationFinished(base::TimeTicks monotonic_time, int target_property, int group) override
    {
        KeyframeModel* keyframe_model = animation_->GetKeyframeModel(TargetProperty::OPACITY);
        if (keyframe_model)
            animation_->RemoveKeyframeModel(keyframe_model->id());
        EndTest();
    }
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostAnimationTestAnimationFinishedEvents);

// Ensures that when opacity is being animated, this value does not cause the
// subtree to be skipped.
class LayerTreeHostAnimationTestDoNotSkipLayersWithAnimatedOpacity : public LayerTreeHostAnimationTest {
public:
    ...
    {
        scoped_refptr<const AnimationTimeline> timeline_impl = GetImplAnimationHost(host_impl)->GetTimelineById(timeline_id_);
        scoped_refptr<Animation> animation_impl = timeline_impl->GetAnimationById(animation_id_);

        KeyframeModel* keyframe_model_impl = animation_impl->GetKeyframeModel(TargetProperty::OPACITY);
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostAnimationTestDoNotSkipLayersWithAnimatedOpacity);

// Layers added to tree with existing active animations should have the
// animation correctly recognized.
class LayerTreeHostAnimationTestLayerAddedWithAnimation : public LayerTreeHostAnimationTest {
public:
    void BeginTest() override
    {
        PostSetNeedsCommitToMainThread();
    ...
    void DidCommit() override
    {
        if (layer_tree_host()->SourceFrameNumber() == 1) {
            ...
            std::unique_ptr<KeyframeModel> keyframe_model(
                KeyframeModel::Create(std::move(curve), 1, 1, KeyframeModel::TargetPropertyId(TargetProperty::OPACITY)));
    ...
    void AnimateLayers(LayerTreeHostImpl* impl_host, base::TimeTicks monotonic_time) override
    {
        EndTest();
    }
};

SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostAnimationTestLayerAddedWithAnimation);

class LayerTreeHostAnimationTestCancelAnimateCommit : public LayerTreeHostAnimationTest {
public:
...
class LayerTreeHostAnimationTestForceRedraw : public LayerTreeHostAnimationTest {
public:
...
class LayerTreeHostAnimationTestAnimateAfterSetNeedsCommit : public LayerTreeHostAnimationTest {
public:
...
// checkerboard.
class LayerTreeHostAnimationTestCheckerboardDoesntStartAnimations : public LayerTreeHostAnimationTest {
    void SetupTree() override
    {
        LayerTreeHostAnimationTest::SetupTree();
        picture_ = FakePictureLayer::Create(&client_);
    ...
    void DidCommitAndDrawFrame() override
    {
        ...
        case 1:
            ...
            AddOpacityTransitionToAnimation(animation_child_.get(), 0.1, 0.2f, 0.8f, false);
            break;
        case 2:
...
// the main thread.
class LayerTreeHostAnimationTestScrollOffsetChangesArePropagated : public LayerTreeHostAnimationTest {
public:
    void SetupTree() override
    {
        LayerTreeHostAnimationTest::SetupTree();
    ...
    void DidCommit() override
    {
        switch (layer_tree_host()->SourceFrameNumber()) {
        case 1: {
            std::unique_ptr<ScrollOffsetAnimationCurve> curve(ScrollOffsetAnimationCurveFactory::CreateEaseInOutAnimationForTesting(gfx::PointF(500.f, 550.f)));
            std::unique_ptr<KeyframeModel> keyframe_model(
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostAnimationTestScrollOffsetChangesArePropagated);
...
class LayerTreeHostAnimationTestScrollOffsetAnimationTakeover : public LayerTreeHostAnimationTest {
public:
    ...
    void DidCommit() override
    {
        if (layer_tree_host()->SourceFrameNumber() == 1) {
            ...
            // path.
            animation_host()->scroll_offset_animations().AddTakeoverUpdate(scroll_layer_->element_id());
            EXPECT_TRUE(animation_host()->scroll_offset_animations().HasUpdatesForTesting());
        }
    }
    ...
    void WillCommitCompleteOnThread(LayerTreeHostImpl* host_impl) override
    {
        if (host_impl->sync_tree()->source_frame_number() == 0) {
            GetImplAnimationHost(host_impl)->ImplOnlyScrollAnimationCreate(
                scroll_layer_->element_id(), gfx::PointF(650.f, 750.f), gfx::PointF(10, 20), base::TimeDelta(), base::TimeDelta());
        }
    }

    void NotifyAnimationTakeover(
        base::TimeTicks monotonic_time, int target_property, base::TimeTicks animation_start_time, std::unique_ptr<gfx::AnimationCurve> curve) override
    {
        EndTest();
    }
...
class LayerTreeHostAnimationTestScrollOffsetAnimationAdjusted : public LayerTreeHostAnimationTest {
public:
    ...
    const KeyframeEffect& ScrollOffsetKeyframeEffect(const LayerTreeHostImpl& host_impl, ElementId element_id) const
    {
        scoped_refptr<const ElementAnimations> element_animations = GetImplAnimationHost(&host_impl)->GetElementAnimationsForElementIdForTesting(element_id);
        DCHECK(element_animations);
        const KeyframeEffect* keyframe_effect = &*element_animations->FirstKeyframeEffectForTesting();
    ...
    void DidCommit() override
    {
        if (layer_tree_host()->SourceFrameNumber() == 2) {
            ...
            // path.
            animation_host()->scroll_offset_animations().AddAdjustmentUpdate(scroll_layer_element_id_, gfx::Vector2dF(100.f, 100.f));
            EXPECT_TRUE(animation_host()->scroll_offset_animations().HasUpdatesForTesting());
        } else if (layer_tree_host()->SourceFrameNumber() == 3) {
            // Verify that the update queue is cleared after the update is applied.
            EXPECT_FALSE(animation_host()->scroll_offset_animations().HasUpdatesForTesting());
        }
    }
    ...
    void BeginCommitOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        if (host_impl->sync_tree()->source_frame_number() == 2) {
            ...
            gfx::KeyframeModel* keyframe_model
                = ScrollOffsetKeyframeEffect(*host_impl, scroll_layer_element_id_).GetKeyframeModel(TargetProperty::SCROLL_OFFSET);
            DCHECK(keyframe_model);
            const ScrollOffsetAnimationCurve* curve = ScrollOffsetAnimationCurve::ToScrollOffsetAnimationCurve(keyframe_model->curve());
    ...
    void WillActivateTreeOnThread(LayerTreeHostImpl* host_impl) override
    {
        if (host_impl->sync_tree()->source_frame_number() == 0) {
            ...
            GetImplAnimationHost(host_impl)->ImplOnlyScrollAnimationCreate(
                scroll_layer_element_id_, gfx::PointF(650.f, 750.f), gfx::PointF(10, 20), base::TimeDelta(), base::TimeDelta());
        }
    }
...
class LayerTreeHostPresentationDuringAnimation : public LayerTreeHostAnimationTest {
public:
    void SetupTree() override
    {
        LayerTreeHostAnimationTest::SetupTree();
        ...
        std::unique_ptr<ScrollOffsetAnimationCurve> curve(ScrollOffsetAnimationCurveFactory::CreateEaseInOutAnimationForTesting(gfx::PointF(6500.f, 7500.f)));
        std::unique_ptr<KeyframeModel> keyframe_model(
            KeyframeModel::Create(std::move(curve), 1, 0, KeyframeModel::TargetPropertyId(TargetProperty::SCROLL_OFFSET)));
        keyframe_model->set_needs_synchronized_start_time(true);
    ...
    {
        PostSetNeedsCommitToMainThread();
        if (const_cast<const LayerTreeHost*>(layer_tree_host())->pending_commit_state()->source_frame_number == 2) {
            layer_tree_host()->RequestSuccessfulPresentationTimeForNextFrame(
                base::BindOnce(&LayerTreeHostPresentationDuringAnimation::OnPresentation, base::Unretained(this)));
        }
    }
    ...
    void AfterTest() override
    {
        ...
        EXPECT_TRUE(base::StatisticsRecorder::FindHistogram("CompositorLatency.TotalLatency"));
        EXPECT_FALSE(base::StatisticsRecorder::FindHistogram("CompositorLatency.Universal.TotalLatency"));
    }

private:
    void OnPresentation(const viz::FrameTimingDetails& details)
    {
        EndTest();
    }
...
// delta.
class LayerTreeHostAnimationTestScrollOffsetAnimationRemoval : public LayerTreeHostAnimationTest {
public:
    ...
    void BeginMainFrame(const viz::BeginFrameArgs& args) override
    {
        ...
        case 1: {
            ...
            KeyframeModel* keyframe_model = animation_child_->GetKeyframeModel(TargetProperty::SCROLL_OFFSET);
            animation_child_->RemoveKeyframeModel(keyframe_model->id());
    ...
    void BeginCommitOnThread(LayerTreeHostImpl* host_impl) override
    {
        host_impl->BlockNotifyReadyToActivateForTesting(ShouldBlockActivation(host_impl));
    }
...
private:
    bool ShouldBlockActivation(LayerTreeHostImpl* host_impl)
    {
        ...
        scoped_refptr<const AnimationTimeline> timeline_impl = GetImplAnimationHost(host_impl)->GetTimelineById(timeline_id_);
        scoped_refptr<Animation> animation_impl = timeline_impl->GetAnimationById(animation_child_id_);

        LayerImpl* scroll_layer_impl = host_impl->active_tree()->LayerById(scroll_layer_->id());
        KeyframeModel* keyframe_model = animation_impl->GetKeyframeModel(TargetProperty::SCROLL_OFFSET);

        if (!keyframe_model || keyframe_model->run_state() != KeyframeModel::RUNNING)
            return false;
...
class LayerTreeHostAnimationTestScrollOffsetAnimationCompletion : public LayerTreeHostAnimationTest {
public:
    ...
    void SetupTree() override
    {
        ...
        std::unique_ptr<ScrollOffsetAnimationCurve> curve(ScrollOffsetAnimationCurveFactory::CreateEaseInOutAnimationForTesting(final_position_));
        std::unique_ptr<KeyframeModel> keyframe_model(
            KeyframeModel::Create(std::move(curve), 1, 0, KeyframeModel::TargetPropertyId(TargetProperty::SCROLL_OFFSET)));
        keyframe_model->set_needs_synchronized_start_time(true);
    ...
    void BeginMainFrame(const viz::BeginFrameArgs& args) override
    {
        ...
        case 0:
            EXPECT_EQ(scroll_layer_->scroll_offset().x(), 100);
            EXPECT_EQ(scroll_layer_->scroll_offset().y(), 200);
            EXPECT_EQ(KeyframeModel::RunState::WAITING_FOR_TARGET_AVAILABILITY, keyframe_model->run_state());
            break;
        case 1:
            EXPECT_EQ(KeyframeModel::RunState::RUNNING, keyframe_model->run_state());
            break;
        default:
            break;
        }
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        KeyframeModel* keyframe_model = animation_child_impl_->GetKeyframeModel(TargetProperty::SCROLL_OFFSET);
        if (!keyframe_model || keyframe_model->run_state() == KeyframeModel::RunState::WAITING_FOR_DELETION)
            EndTest();
    }
    ...
    void DidFinishImplFrameOnThread(LayerTreeHostImpl* host_impl) override
    {
        if (!animation_child_impl_)
            return;
        if (KeyframeModel* keyframe_model = animation_child_impl_->GetKeyframeModel(TargetProperty::SCROLL_OFFSET)) {
    ...
    void AfterTest() override
    {
        ...
        EXPECT_EQ(nullptr, animation_child_->GetKeyframeModel(TargetProperty::SCROLL_OFFSET));
        EXPECT_EQ(nullptr, animation_child_impl_->GetKeyframeModel(TargetProperty::SCROLL_OFFSET));
...
class LayerTreeHostAnimationTestAnimationsAddedToNewAndExistingLayers : public LayerTreeHostAnimationTest {
public:
    ...
    void DidCommit() override
    {
        if (layer_tree_host()->SourceFrameNumber() == 1) {
            ...
            animation_child_->set_animation_delegate(this);
            AddOpacityTransitionToAnimation(animation_child_.get(), 1, 0.f, 0.5f, true);
        }
    }
    ...
    void UpdateAnimationState(LayerTreeHostImpl* host_impl, bool has_unfinished_animation) override
    {
        ...
        KeyframeModel* root_keyframe_model = animation_impl->GetKeyframeModel(TargetProperty::OPACITY);
        if (!root_keyframe_model || root_keyframe_model->run_state() != KeyframeModel::RUNNING)
            return;

        KeyframeModel* child_keyframe_model = animation_child_impl->GetKeyframeModel(TargetProperty::OPACITY);
        EXPECT_EQ(KeyframeModel::RUNNING, child_keyframe_model->run_state());
        EXPECT_EQ(root_keyframe_model->start_time(), child_keyframe_model->start_time());
        animation_impl->AbortKeyframeModelsWithProperty(TargetProperty::OPACITY, false);
        animation_impl->AbortKeyframeModelsWithProperty(TargetProperty::TRANSFORM, false);
        animation_child_impl->AbortKeyframeModelsWithProperty(TargetProperty::OPACITY, false);
        EndTest();
    }
...
class LayerTreeHostAnimationTestAddKeyframeModelAfterAnimating : public LayerTreeHostAnimationTest {
public:
    void SetupTree() override
    {
        LayerTreeHostAnimationTest::SetupTree();
    ...
    void DidCommit() override
    {
        ...
        case 2:
            ...
            AddOpacityTransitionToAnimation(animation_child_.get(), 0.1, 5, 5, false);
            break;
        }
    }
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        if (!TestEnded()) {
            ImplThreadTaskRunner()->PostTask(
                FROM_HERE, base::BindOnce(&LayerTreeHostAnimationTestAddKeyframeModelAfterAnimating::CheckAnimations, base::Unretained(this), host_impl));
        }
    }
    ...
    void CheckAnimations(LayerTreeHostImpl* host_impl)
    {
        ...
        EXPECT_EQ(2u, GetImplAnimationHost(host_impl)->ticking_animations_for_testing().size());

        KeyframeModel* root_anim = animation_impl_->GetKeyframeModel(TargetProperty::TRANSFORM);
        EXPECT_LT(base::TimeTicks(), root_anim->start_time());

        KeyframeModel* anim = animation_child_impl_->GetKeyframeModel(TargetProperty::OPACITY);
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostAnimationTestAddKeyframeModelAfterAnimating);

class LayerTreeHostAnimationTestRemoveKeyframeModel : public LayerTreeHostAnimationTest {
public:
    void SetupTree() override
    {
        LayerTreeHostAnimationTest::SetupTree();
    ...
    void DidCommit() override
    {
        ...
        case 1:
            ...
            break;
        case 2:
            KeyframeModel* keyframe_model = animation_child_->GetKeyframeModel(TargetProperty::TRANSFORM);
            animation_child_->RemoveKeyframeModel(keyframe_model->id());
    ...
    void CommitCompleteOnThread(LayerTreeHostImpl* host_impl) override
    {
        if (host_impl->sync_tree()->source_frame_number() >= last_frame_number_) {
            ...
            EXPECT_FALSE(animation_child_impl_->keyframe_effect()->has_any_keyframe_model());
            EndTest();
        }
    }
    ...
    void UpdateAnimationState(LayerTreeHostImpl* host_impl, bool has_unfinished_animation) override
    {
        ...
        if (animation_stopped_ && !has_unfinished_animation) {
            last_frame_number_ = std::min(last_frame_number_, host_impl->active_tree()->source_frame_number() + 1);
            PostSetNeedsCommitToMainThread();
        }
    }
...
class LayerTreeHostAnimationTestIsAnimating : public LayerTreeHostAnimationTest {
public:
    void SetupTree() override
    {
        LayerTreeHostAnimationTest::SetupTree();
    ...
    void DidCommit() override
    {
        ...
        case 1:
            ...
            break;
        case 2:
            KeyframeModel* keyframe_model = animation_->GetKeyframeModel(TargetProperty::TRANSFORM);
            EXPECT_EQ(KeyframeModel::RunState::RUNNING, keyframe_model->run_state());
            animation_->RemoveKeyframeModel(keyframe_model->id());
            break;
        }
    ...
    void DidActivateTreeOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        case 1:
            ...
            EXPECT_EQ(KeyframeModel::RunState::STARTING, animation_impl_->GetKeyframeModel(TargetProperty::TRANSFORM)->run_state());
            break;
        case 2:
            // After activation, the KeyframeModel should be waiting for deletion.
            EXPECT_EQ(KeyframeModel::RunState::WAITING_FOR_DELETION, animation_impl_->GetKeyframeModel(TargetProperty::TRANSFORM)->run_state());
            break;
        case 3:
            // The animation KeyframeModel is cleaned up.
            EXPECT_EQ(nullptr, animation_impl_->GetKeyframeModel(TargetProperty::TRANSFORM));
            EndTest();
            break;
        default:
            NOTREACHED();
...
#if BUILDFLAG(IS_LINUX) || defined(ADDRESS_SANITIZER) || !defined(NDEBUG) || BUILDFLAG(IS_ANDROID)
...
class LayerTreeHostAnimationTestAnimationFinishesDuringCommit : public LayerTreeHostAnimationTest {
public:
...
class LayerTreeHostAnimationTestImplSideInvalidation : public LayerTreeHostAnimationTest {
public:
    void SetupTree() override
    {
        LayerTreeHostAnimationTest::SetupTree();
    ...
    void UpdateAnimationState(LayerTreeHostImpl* host_impl, bool has_unfinished_animation) override
    {
        if (host_impl->active_tree()->source_frame_number() == 1 && !has_unfinished_animation && !did_request_impl_side_invalidation_) {
...
class LayerTreeHostAnimationTestImplSideInvalidationWithoutCommit : public LayerTreeHostAnimationTest {
public:
    void SetupTree() override
    {
        LayerTreeHostAnimationTest::SetupTree();
...
class ImplSideInvalidationWithoutCommitTestOpacity : public LayerTreeHostAnimationTestImplSideInvalidationWithoutCommit {
public:
    void BeginTest() override
    {
        AddOpacityTransitionToAnimation(animation_child_.get(), 0.04, 0.2f, 0.8f, false);
        PostSetNeedsCommitToMainThread();
    }
    ...
    void DidInvalidateContentOnImplSide(LayerTreeHostImpl* host_impl) override
    {
        ...
        LayerTreeHostAnimationTestImplSideInvalidationWithoutCommit::DidInvalidateContentOnImplSide(host_impl);
    }
};

MULTI_THREAD_TEST_F(ImplSideInvalidationWithoutCommitTestOpacity);

class ImplSideInvalidationWithoutCommitTestTransform : public LayerTreeHostAnimationTestImplSideInvalidationWithoutCommit {
public:
    void BeginTest() override
    {
        AddAnimatedTransformToAnimation(animation_child_.get(), 0.04, 5, 5);
...
class ImplSideInvalidationWithoutCommitTestFilter : public LayerTreeHostAnimationTestImplSideInvalidationWithoutCommit {
public:
    void BeginTest() override
    {
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* host_impl) override
    {
        if (num_draws_++ > 0)
            return;
        EXPECT_EQ(0, host_impl->active_tree()->source_frame_number());
        EXPECT_EQ(std::string("{\"FilterOperations\":[{\"type\":5,\"amount\":0.0}]}"),
            host_impl->active_tree()->property_trees()->effect_tree().FindNodeFromElementId(layer_element_id_)->filters.ToString());
    }
    ...
    void DidInvalidateContentOnImplSide(LayerTreeHostImpl* host_impl) override
    {
        ...
        EXPECT_EQ(std::string("{\"FilterOperations\":[{\"type\":5,\"amount\":1.0}]}"),
            host_impl->sync_tree()->property_trees()->effect_tree().FindNodeFromElementId(layer_element_id_)->filters.ToString());
        LayerTreeHostAnimationTestImplSideInvalidationWithoutCommit::DidInvalidateContentOnImplSide(host_impl);
    }
};
...
class ImplSideInvalidationWithoutCommitTestScroll : public LayerTreeHostAnimationTestImplSideInvalidationWithoutCommit {
public:
    void SetupTree() override
    {
...
class LayerTreeHostAnimationTestNotifyAnimationFinished : public LayerTreeHostAnimationTest {
public:
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostAnimationTestNotifyAnimationFinished);

// Check that SetTransformIsPotentiallyAnimatingChanged is called
// if we destroy ElementAnimations.
class LayerTreeHostAnimationTestSetPotentiallyAnimatingOnLacDestruction : public LayerTreeHostAnimationTest {
public:
    void SetupTree() override
    {
    ...
    {
        if (host_impl->pending_tree()->source_frame_number() <= 1) {
            EXPECT_TRUE(host_impl->pending_tree()->root_layer()->screen_space_transform_is_animating());
        } else {
            EXPECT_FALSE(host_impl->pending_tree()->root_layer()->screen_space_transform_is_animating());
        }
    }
    ...
    DrawResult PrepareToDrawOnThread(LayerTreeHostImpl* host_impl, LayerTreeHostImpl::FrameData* frame_data, DrawResult draw_result) override
    {
        const bool screen_space_transform_is_animating = host_impl->active_tree()->root_layer()->screen_space_transform_is_animating();
...
MULTI_THREAD_TEST_F(LayerTreeHostAnimationTestSetPotentiallyAnimatingOnLacDestruction);

// Check that we invalidate property trees on Animation::SetNeedsCommit.
class LayerTreeHostAnimationTestRebuildPropertyTreesOnAnimationSetNeedsCommit : public LayerTreeHostAnimationTest {
public:
    void SetupTree() override
    {
        LayerTreeHostAnimationTest::SetupTree();
...
MULTI_THREAD_TEST_F(LayerTreeHostAnimationTestRebuildPropertyTreesOnAnimationSetNeedsCommit);
...
public:
    ...
    void WillBeginImplFrameOnThread(LayerTreeHostImpl* host_impl, const viz::BeginFrameArgs& args, bool has_damage) override
    {
        ...
        if (++impl_frames_while_activation_delayed_ == kNumOfFramesToDelayActivation) {
