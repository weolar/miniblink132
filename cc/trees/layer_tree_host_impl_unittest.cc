...
viz::SurfaceId MakeSurfaceId(const viz::FrameSinkId& frame_sink_id, uint32_t parent_id)
{
    return viz::SurfaceId(frame_sink_id, viz::LocalSurfaceId(parent_id, base::UnguessableToken::CreateForTesting(0, 1u)));
}
...
class LayerTreeHostImplTestBase : public testing::Test, public LayerTreeHostImplClient {
public:
    ...
    void CreatePendingTree()
    {
        ...
        pending_tree->SetDeviceViewportRect(host_impl_->active_tree()->GetDeviceViewport());
        pending_tree->SetDeviceScaleFactor(host_impl_->active_tree()->device_scale_factor());
    ...
    void PostDelayedAnimationTaskOnImplThread(base::OnceClosure task, base::TimeDelta delay) override
    {
    ...
    void DidActivateSyncTree() override
    {
        ...
        host_impl_->active_tree()->SetLocalSurfaceIdFromParent(viz::LocalSurfaceId(1, base::UnguessableToken::CreateForTesting(2u, 3u)));
    }
    ...
    void OnDrawForLayerTreeFrameSink(bool resourceless_software_draw, bool skip_draw) override
    {
        ...
        viz::CompositorRenderPass::CopyAllForTest(frame->render_passes, &last_on_draw_render_passes_);
        host_impl_->DrawLayers(frame.get());
        host_impl_->DidDrawAllLayers(*frame);
        last_on_draw_frame_ = std::move(frame);
    }
    void SetNeedsImplSideInvalidation(bool needs_first_draw_on_activation) override
    {
        did_request_impl_side_invalidation_ = true;
    }
    void NotifyImageDecodeRequestFinished(int request_id, bool decode_succeeded) override
    {
    }
    void DidPresentCompositorFrameOnImplThread(
        uint32_t frame_token, PresentationTimeCallbackBuffer::PendingCallbacks activated, const viz::FrameTimingDetails& details) override
    {
        // We don't call main thread callbacks in this test.
        activated.main_callbacks.clear();
        activated.main_successful_callbacks.clear();

        host_impl_->NotifyDidPresentCompositorFrameOnImplThread(frame_token, std::move(activated.compositor_successful_callbacks), details);
    }
    void NotifyAnimationWorkletStateChange(AnimationWorkletMutationState state, ElementListType tree_type) override
    {
    }
    void NotifyPaintWorkletStateChange(Scheduler::PaintWorkletState state) override
    {
    }
    ...
    void DidObserveFirstScrollDelay(int source_frame_number, base::TimeDelta first_scroll_delay, base::TimeTicks first_scroll_timestamp) override
    {
    ...
    void FrameSinksToThrottleUpdated(const base::flat_set<viz::FrameSinkId>& ids) override
    {
    }
    ...
    void NotifyTransitionRequestFinished(uint32_t sequence_id, const viz::ViewTransitionElementResourceRects&) override
    {
    }
    ...
    virtual bool CreateHostImpl(const LayerTreeSettings& settings, std::unique_ptr<LayerTreeFrameSink> layer_tree_frame_sink)
    {
        if (host_impl_)
            host_impl_->ReleaseLayerTreeFrameSink();
        host_impl_.reset();
        InitializeImageWorker(settings);
        host_impl_ = LayerTreeHostImpl::Create(settings, this, &task_runner_provider_, &stats_instrumentation_, &task_graph_runner_,
            AnimationHost::CreateForTesting(ThreadInstance::kImpl), nullptr, 0, image_worker_ ? image_worker_->task_runner() : nullptr, nullptr);
        ...
        auto args = viz::CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 1, base::TimeTicks() + base::Milliseconds(1));
    ...
    template <typename T, typename... Args> T* SetupRootLayer(LayerTreeImpl* layer_tree_impl, const gfx::Size& viewport_size, Args&&... args)
    {
        const int kRootLayerId = 1;
        DCHECK(!layer_tree_impl->root_layer());
        DCHECK(!layer_tree_impl->LayerById(kRootLayerId));
        layer_tree_impl->SetRootLayerForTesting(T::Create(layer_tree_impl, kRootLayerId, std::forward<Args>(args)...));
        auto* root = layer_tree_impl->root_layer();
        root->SetBounds(viewport_size);
        layer_tree_impl->SetDeviceViewportRect(gfx::Rect(DipSizeToPixelSize(viewport_size)));
        SetupRootProperties(root);
        return static_cast<T*>(root);
    }

    LayerImpl* SetupDefaultRootLayer(const gfx::Size& viewport_size = kDefaultLayerSize)
    {
    ...
    {
        return gfx::ScaleToRoundedSize(size, host_impl_->active_tree()->device_scale_factor());
    }

    void PushScrollOffsetsToPendingTree(const base::flat_map<ElementId, gfx::PointF>& offsets)
    {
        PropertyTrees property_trees(*host_impl_);
        auto& scroll_tree = host_impl_->active_tree()->property_trees()->scroll_tree_mutable();
        if (auto* layer = InnerViewportScrollLayer()) {
            property_trees.scroll_tree_mutable().SetBaseScrollOffset(layer->element_id(), scroll_tree.current_scroll_offset(layer->element_id()));
        }
        if (auto* layer = OuterViewportScrollLayer()) {
            property_trees.scroll_tree_mutable().SetBaseScrollOffset(layer->element_id(), scroll_tree.current_scroll_offset(layer->element_id()));
        }
        for (auto& entry : offsets) {
            property_trees.scroll_tree_mutable().SetBaseScrollOffset(entry.first, entry.second);
        }
        host_impl_->sync_tree()->property_trees()->scroll_tree_mutable().PushScrollUpdatesFromMainThread(
            property_trees, host_impl_->sync_tree(), host_impl_->settings().commit_fractional_scroll_deltas);
    }
    ...
    static ::testing::AssertionResult ScrollInfoContains(const CompositorCommitData& commit_data, ElementId id, const gfx::Vector2dF& scroll_delta)
    {
        ...
        for (size_t i = 0; i < commit_data.scrolls.size(); ++i) {
            ...
            if (scroll_delta != commit_data.scrolls[i].scroll_delta) {
                return ::testing::AssertionFailure() << "Expected " << scroll_delta.ToString() << ", not " << commit_data.scrolls[i].scroll_delta.ToString();
            }
            times_encountered++;
        }
        ...
        if (id == commit_data.inner_viewport_scroll.element_id) {
            if (scroll_delta != commit_data.inner_viewport_scroll.scroll_delta) {
                return ::testing::AssertionFailure() << "Expected " << scroll_delta.ToString() << ", not "
    ...
    template <typename T, typename... Args> T* AddLayer(LayerTreeImpl* layer_tree_impl, Args&&... args)
    {
        std::unique_ptr<T> layer = T::Create(layer_tree_impl, next_layer_id_++, std::forward<Args>(args)...);
        T* result = layer.get();
    ...
    void SetupViewportLayersInnerScrolls(const gfx::Size& inner_viewport_size, const gfx::Size& content_size)
    {
    ...
    // viewport only.
    void SetupViewportLayersOuterScrolls(const gfx::Size& viewport_size, const gfx::Size& content_size)
    {
        SetupViewportLayers(host_impl_->active_tree(), viewport_size, viewport_size, content_size);
    }
    ...
    void CreateAndTestNonScrollableLayers(bool transparent_layer)
    {
        ...
        LayerImpl* scroll = AddScrollableLayer(outer_scroll, content_size, scroll_content_size);
        ...
        auto* scrollbar = AddLayer<PaintedScrollbarLayerImpl>(layer_tree_impl, ScrollbarOrientation::kVertical, false, true);
        ...
        InputHandler::ScrollStatus status = GetInputHandler().ScrollBegin(
            BeginState(gfx::Point(230, 150), gfx::Vector2dF(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
        ASSERT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
        ASSERT_EQ(MainThreadScrollingReason::kFailedHitTest, status.main_thread_hit_test_reasons);

        // The point hits squash1 layer and also scrollbar layer.
        status = GetInputHandler().ScrollBegin(
            BeginState(gfx::Point(350, 150), gfx::Vector2dF(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
        ASSERT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
        ASSERT_EQ(MainThreadScrollingReason::kFailedHitTest, status.main_thread_hit_test_reasons);
        ...
        status = GetInputHandler().ScrollBegin(
            BeginState(gfx::Point(230, 450), gfx::Vector2dF(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
        ASSERT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
    }

    LayerImpl* AddScrollableLayer(LayerImpl* container, const gfx::Size& scroll_container_bounds, const gfx::Size& content_size)
    {
    ...
    void SetupScrollbarLayerCommon(LayerImpl* scroll_layer, ScrollbarLayerImplBase* scrollbar)
    {
    ...
    void SetupScrollbarLayer(LayerImpl* scroll_layer, SolidColorScrollbarLayerImpl* scrollbar)
    {
    ...
    void SetupScrollbarLayer(LayerImpl* scroll_layer, PaintedScrollbarLayerImpl* scrollbar)
    {
        SetupScrollbarLayerCommon(scroll_layer, scrollbar);
    ...
    std::unique_ptr<ScrollState> BeginState(const gfx::Point& point, const gfx::Vector2dF& delta_hint, ui::ScrollInputType type)
    {
        ...
        scroll_state_data.is_direct_manipulation = type == ui::ScrollInputType::kTouchscreen;
        std::unique_ptr<ScrollState> scroll_state(new ScrollState(scroll_state_data));
        return scroll_state;
    }

    ScrollState UpdateState(const gfx::Point& point, const gfx::Vector2dF& delta, ui::ScrollInputType type)
    {
        ScrollStateData scroll_state_data;
    ...
    ScrollState AnimatedUpdateState(const gfx::Point& point, const gfx::Vector2dF& delta)
    {
    ...
    {
        auto args
            = viz::CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, viz::BeginFrameArgs::kManualSourceId, 1, base::TimeTicks() + base::Milliseconds(1));
    ...
    void AllowedTouchActionTestHelper(float device_scale_factor, float page_scale_factor)
    {
        ...
        host_impl_->active_tree()->PushPageScaleFromMainThread(page_scale_factor, min_page_scale_factor, max_page_scale_factor);
        ...
        child_touch_action_region.Union(TouchAction::kPanLeft, gfx::Rect(0, 0, 25, 25));
        child->SetTouchActionRegion(child_touch_action_region);

        TouchAction touch_action = TouchAction::kAuto;
        GetInputHandler().EventListenerTypeForTouchStartOrMoveAt(gfx::Point(10, 10), &touch_action);
        EXPECT_EQ(TouchAction::kPanLeft, touch_action);
        touch_action = TouchAction::kAuto;
        GetInputHandler().EventListenerTypeForTouchStartOrMoveAt(gfx::Point(30, 30), &touch_action);
    ...
    LayerImpl* CreateLayerForSnapping()
    {
        ...
        LayerImpl* overflow = AddScrollableLayer(OuterViewportScrollLayer(), gfx::Size(100, 100), overflow_size);
        SnapContainerData container_data(ScrollSnapType(false, SnapAxis::kBoth, SnapStrictness::kMandatory), gfx::RectF(0, 0, 200, 200), gfx::PointF(300, 300));
        SnapAreaData area_data(ScrollSnapAlign(SnapAlignment::kStart), gfx::RectF(50, 50, 100, 100), false, false, ElementId(10));
    ...
    {
        return GetScrollNode(layer) ? GetScrollNode(layer)->snap_container_data : std::nullopt;
    }
    ...
    void pinch_zoom_pan_viewport_and_scroll_boundary_test(float device_scale_factor);
...
protected:
    ...
    static void SetScrollOffsetDelta(LayerImpl* layer_impl, const gfx::Vector2dF& delta)
    {
        if (layer_impl->layer_tree_impl()->property_trees()->scroll_tree_mutable().SetScrollOffsetDeltaForTesting(layer_impl->element_id(), delta))
            layer_impl->layer_tree_impl()->DidUpdateScrollOffset(layer_impl->element_id(), /*pushed_from_main_or_pending_tree=*/false);
    }

    void BeginImplFrameAndAnimate(viz::BeginFrameArgs begin_frame_args, base::TimeTicks frame_time)
    {
    ...
    public:
        void OnMemoryDump(base::trace_event::ProcessMemoryDump* pmd, const base::trace_event::MemoryAllocatorDumpGuid& buffer_dump_guid,
            uint64_t tracing_process_id, int importance) const override
        {
        }
    };
...
class CommitToActiveTreeLayerTreeHostImplTest : public LayerTreeHostImplTestBase {
public:
...
class LayerTreeHostImplTest : public LayerTreeHostImplTestBase, public testing::WithParamInterface<bool> {
public:
...
class OccludedSurfaceThrottlingLayerTreeHostImplTest : public LayerTreeHostImplTest {
public:
    LayerTreeSettings DefaultSettings() override
    {
...
INSTANTIATE_COMMIT_TO_TREE_TEST_P(OccludedSurfaceThrottlingLayerTreeHostImplTest);
...
class FluentOverlayScrollbarLayerTreeHostImplTest : public CommitToPendingTreeLayerTreeHostImplTest {
public:
    void SetUp() override
    {
        LayerTreeSettings settings = DefaultSettings();
    ...
    PaintedScrollbarLayerImpl* CreateAndRegisterPaintedScrollbarLayer()
    {
        ...
        auto& effect_node = CreateEffectNode(scrollbar, layer_tree_impl->root_layer()->effect_tree_index());
        SetupScrollbarLayerCommon(scroll_layer, scrollbar);
...
class FluentOverlayScrollbarOpacityLayerTreeHostImplTest : public FluentOverlayScrollbarLayerTreeHostImplTest, public testing::WithParamInterface<int> {
public:
    constexpr static int kParamSteps = 10;

    void VerifyCorrectOpacityForThickness(PaintedScrollbarLayerImpl* scrollbar, float thickness, float expected_opacity)
    {
        ...
        if (expected_opacity == 0.f) {
            ...
            EXPECT_FLOAT_EQ(expected_opacity, render_pass->shared_quad_state_list.back()->opacity);
        }
    }
};
...
public:
    ...
    void UpdateRootLayerStateForSynchronousInputHandler(const gfx::PointF& total_scroll_offset, const gfx::PointF& max_scroll_offset,
        const gfx::SizeF& scrollable_size, float page_scale_factor, float min_page_scale_factor, float max_page_scale_factor) override
    {
    ...
    void SetScrollEventDispatchMode(InputHandlerClient::ScrollEventDispatchMode mode) override
    {
    }

    gfx::PointF last_set_scroll_offset()
    {
        return last_set_scroll_offset_;
    }

    gfx::PointF max_scroll_offset() const
    {
...
TEST_P(LayerTreeHostImplTest, LocalAndExternalPinchState)
{
    ...
    GetInputHandler().PinchGestureBegin(gfx::Point(), ui::ScrollInputType::kTouchscreen);
...
TEST_P(LayerTreeHostImplTest, ResourcelessDrawWithEmptyViewport)
{
    ...
    auto* fake_layer_tree_frame_sink = static_cast<FakeLayerTreeFrameSink*>(host_impl_->layer_tree_frame_sink());
...
TEST_P(LayerTreeHostImplTest, ScrollDeltaNoLayers)
{
    ...
    std::unique_ptr<CompositorCommitData> commit_data = host_impl_->ProcessCompositorDeltas(
...
TEST_P(LayerTreeHostImplTest, ScrollDeltaTreeButNoChanges)
{
    ...
    std::unique_ptr<AnimationHost> main_thread_animation_host(AnimationHost::CreateForTesting(ThreadInstance::kMain));
    ...
    commit_data = host_impl_->ProcessCompositorDeltas(main_thread_animation_host.get());
    ASSERT_EQ(commit_data->scrolls.size(), 0u);
    ExpectClearedScrollDeltasRecursive(root);
}
...
TEST_F(CommitToActiveTreeLayerTreeHostImplTest, ScrollDeltaRepeatedScrolls)
{
    ...
    root->layer_tree_impl()->property_trees()->scroll_tree_mutable().UpdateScrollOffsetBaseForTesting(root->element_id(), scroll_offset);
    UpdateDrawProperties(host_impl_->active_tree());
    ...
    EXPECT_TRUE(ScrollInfoContains(*commit_data1, root->element_id(), scroll_delta));

    std::unique_ptr<CompositorCommitData> commit_data2;
    gfx::Vector2dF scroll_delta2(-5, 27);
    root->ScrollBy(scroll_delta2);
    // Non-null mutator_host implies pipelined main frame
    commit_data2 = host_impl_->ProcessCompositorDeltas(main_thread_animation_host.get());
    ASSERT_EQ(commit_data2->scrolls.size(), 1u);
    EXPECT_TRUE(ScrollInfoContains(*commit_data2, root->element_id(), scroll_delta2));

    // Simulate first commit by pushing base scroll offsets to pending tree
    PushScrollOffsetsToPendingTree({ { root->element_id(), gfx::PointAtOffsetFromOrigin(scroll_delta) } });
    ClearNonScrollSyncTreeDeltasForTesting();
    EXPECT_EQ(host_impl_->sync_tree()->property_trees()->scroll_tree().GetScrollOffsetDeltaForTesting(root->element_id()), scroll_delta2);

    // Simulate second commit by pushing base scroll offsets to pending tree
    PushScrollOffsetsToPendingTree({ { root->element_id(), gfx::PointAtOffsetFromOrigin(scroll_delta2) } });
    EXPECT_EQ(host_impl_->sync_tree()->property_trees()->scroll_tree().GetScrollOffsetDeltaForTesting(root->element_id()), gfx::Vector2dF(0, 0));
}
...
TEST_F(CommitToPendingTreeLayerTreeHostImplTest, SyncedScrollAbortedCommit)
{
    ...
    gfx::PointF scroll_offset(20, 30);
    auto* root = SetupDefaultRootLayer(gfx::Size(110, 110));
    auto& scroll_tree = root->layer_tree_impl()->property_trees()->scroll_tree_mutable();
    root->SetHitTestOpaqueness(HitTestOpaqueness::kMixed);
    std::unique_ptr<AnimationHost> main_thread_animation_host(AnimationHost::CreateForTesting(ThreadInstance::kMain));
    ...
    ClearNonScrollSyncTreeDeltasForTesting();
    host_impl_->active_tree()->property_trees()->scroll_tree_mutable().PushScrollUpdatesFromPendingTree(
        host_impl_->pending_tree()->property_trees(), host_impl_->active_tree());

    CreateScrollNode(root, gfx::Size(10, 10));
    auto* synced_scroll = scroll_tree.GetSyncedScrollOffset(root->element_id());
    ASSERT_TRUE(synced_scroll);
    scroll_tree.UpdateScrollOffsetBaseForTesting(root->element_id(), scroll_offset);
    UpdateDrawProperties(host_impl_->active_tree());
    ...
    EXPECT_EQ(gfx::Vector2dF(), synced_scroll->next_reflected_delta_in_main_tree());
    ...
    EXPECT_EQ(scroll_delta + scroll_delta2, synced_scroll->reflected_delta_in_main_tree());
    EXPECT_EQ(gfx::Vector2dF(), synced_scroll->next_reflected_delta_in_main_tree());
    ...
    PushScrollOffsetsToPendingTree({ { root->element_id(), scroll_offset + scroll_delta + scroll_delta2 } });
    EXPECT_EQ(scroll_offset + scroll_delta + scroll_delta2, synced_scroll->PendingBase());
...
TEST_P(LayerTreeHostImplTest, ScrollBeforeRootLayerAttached)
{
    InputHandler::ScrollStatus status
        = GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(0, 1), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    EXPECT_EQ(ScrollThread::kScrollIgnored, status.thread);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);

    status = GetInputHandler().RootScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(0, 1), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    EXPECT_EQ(ScrollThread::kScrollIgnored, status.thread);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);
}
...
TEST_P(LayerTreeHostImplTest, ScrollUpdateAndEndNoOpWithoutBegin)
{
    ...
    // DCHECK.
    {
        ...
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 10), ui::ScrollInputType::kTouchscreen));
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 10), ui::ScrollInputType::kTouchscreen));
        EXPECT_FALSE(host_impl_->CurrentlyScrollingNode());
    ...
    {
        InputHandler::ScrollStatus status = GetInputHandler().ScrollBegin(
            BeginState(gfx::Point(), gfx::Vector2dF(0, 10), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
        EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
        EXPECT_TRUE(host_impl_->CurrentlyScrollingNode());

        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 10), ui::ScrollInputType::kTouchscreen));
        EXPECT_POINTF_EQ(gfx::PointF(0, 10), CurrentScrollOffset(OuterViewportScrollLayer()));
...
TEST_P(LayerTreeHostImplTest, TargetMainThreadScroller)
{
    ...
    ScrollStateData scroll_state_data;
    scroll_state_data.set_current_native_scrolling_element(host_impl_->OuterViewportScrollNode()->element_id);
    std::unique_ptr<ScrollState> scroll_state(new ScrollState(scroll_state_data));

    // Try on a node that should scroll on the compositor. Confirm it works.
    {
        InputHandler::ScrollStatus status = GetInputHandler().ScrollBegin(scroll_state.get(), ui::ScrollInputType::kWheel);
        EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
        EXPECT_EQ(host_impl_->OuterViewportScrollNode(), host_impl_->CurrentlyScrollingNode());
        GetInputHandler().ScrollEnd();
    }

    // Now add a main-thread repaint reason. ScrollBegin should still succeed.
    host_impl_->OuterViewportScrollNode()->main_thread_repaint_reasons = MainThreadScrollingReason::kPreferNonCompositedScrolling;

    {
        InputHandler::ScrollStatus status = GetInputHandler().ScrollBegin(scroll_state.get(), ui::ScrollInputType::kWheel);
        EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
        EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);
        EXPECT_EQ(MainThreadScrollingReason::kPreferNonCompositedScrolling, host_impl_->CurrentlyScrollingNode()->main_thread_repaint_reasons);
    }
}
...
TEST_P(LayerTreeHostImplTest, ScrollRootCallsCommitAndRedraw)
{
    ...
    InputHandler::ScrollStatus status
        = GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);
...
TEST_P(LayerTreeHostImplTest, ActivelyScrollingOnlyAfterScrollMovement)
{
    ...
    // consumed.
    {
        ...
        EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
        EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);
        EXPECT_EQ(host_impl_->GetActivelyScrollingType(), ActivelyScrollingType::kNone);
        ...
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 10), ui::ScrollInputType::kTouchscreen));
        EXPECT_EQ(host_impl_->GetActivelyScrollingType(), ActivelyScrollingType::kPrecise);
        GetInputHandler().ScrollEnd();
        EXPECT_EQ(host_impl_->GetActivelyScrollingType(), ActivelyScrollingType::kNone);
    }
    ...
    // smoothness mode (i.e. the value of GetParam().animate);
    {
        ...
        GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2dF(0, 10)));
        EXPECT_EQ(host_impl_->GetActivelyScrollingType(), ActivelyScrollingType::kAnimated);

        base::TimeTicks cur_time = base::TimeTicks() + base::Milliseconds(100);
        viz::BeginFrameArgs begin_frame_args = viz::CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 1);
...
TEST_P(LayerTreeHostImplTest, ScrollWithoutRenderer)
{
    ...
    compositor_context->UnboundTestRasterInterface()->LoseContextCHROMIUM(GL_GUILTY_CONTEXT_RESET_ARB, GL_INNOCENT_CONTEXT_RESET_ARB);

    // Initialization will fail.
    EXPECT_FALSE(CreateHostImpl(DefaultSettings(), FakeLayerTreeFrameSink::Create3d(std::move(compositor_context))));
...
TEST_P(LayerTreeHostImplTest, ReplaceTreeWhileScrolling)
{
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler().ScrollBegin(BeginState(gfx::Point(), scroll_delta, ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel).thread);
    ClearLayersAndPropertyTrees(host_impl_->active_tree());
    ...
    // new tree.
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), scroll_delta, ui::ScrollInputType::kWheel));
    GetInputHandler().ScrollEnd();
    std::unique_ptr<CompositorCommitData> commit_data = host_impl_->ProcessCompositorDeltas(
        /* main_thread_mutator_host */ nullptr);
    EXPECT_TRUE(ScrollInfoContains(*commit_data, scroll_layer->element_id(), scroll_delta));
}

TEST_F(CommitToPendingTreeLayerTreeHostImplTest, ActivateTreeScrollingNodeDisappeared)
{
    SetupViewportLayersOuterScrolls(gfx::Size(100, 100), gfx::Size(1000, 1000));

    auto status
        = GetInputHandler().ScrollBegin(BeginState(gfx::Point(30, 30), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel));
    ...
    pending_property_trees.set_sequence_number(host_impl_->active_tree()->property_trees()->sequence_number() + 1);
...
TEST_P(LayerTreeHostImplTest, ScrollBlocksOnWheelEventHandlers)
{
    ...
    host_impl_->active_tree()->set_event_listener_properties(EventListenerClass::kMouseWheel, EventListenerProperties::kBlocking);
    EXPECT_EQ(EventListenerProperties::kBlocking, GetInputHandler().GetEventListenerProperties(EventListenerClass::kMouseWheel));

    // LTHI should know the wheel event handler region and only block mouse events
    // in that region.
    EXPECT_TRUE(GetInputHandler().HasBlockingWheelEventHandlerAt(gfx::Point(10, 10)));
    EXPECT_FALSE(GetInputHandler().HasBlockingWheelEventHandlerAt(gfx::Point(30, 30)));

    // But they don't influence the actual handling of the scroll gestures.
    InputHandler::ScrollStatus status
        = GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);
    GetInputHandler().ScrollEnd();
...
TEST_P(LayerTreeHostImplTest, ScrollBlocksOnTouchEventHandlers)
{
    ...
    touch_action_region.Union(TouchAction::kPanRight, gfx::Rect(25, 25, 100, 100));
    root->SetTouchActionRegion(std::move(touch_action_region));
    EXPECT_EQ(
        InputHandler::TouchStartOrMoveEventListenerType::kHandler, GetInputHandler().EventListenerTypeForTouchStartOrMoveAt(gfx::Point(10, 10), &touch_action));
    EXPECT_EQ(TouchAction::kPanLeft, touch_action);

    // But they don't influence the actual handling of the scroll gestures.
    InputHandler::ScrollStatus status = GetInputHandler().ScrollBegin(
        BeginState(gfx::Point(), gfx::Vector2d(0, 10), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);
    GetInputHandler().ScrollEnd();
    ...
    EXPECT_EQ(
        InputHandler::TouchStartOrMoveEventListenerType::kHandler, GetInputHandler().EventListenerTypeForTouchStartOrMoveAt(gfx::Point(10, 30), &touch_action));
    root->SetTouchActionRegion(TouchActionRegion());
    EXPECT_EQ(InputHandler::TouchStartOrMoveEventListenerType::kNoHandler,
        GetInputHandler().EventListenerTypeForTouchStartOrMoveAt(gfx::Point(10, 30), &touch_action));
...
{
    SetupViewportLayersOuterScrolls(gfx::Size(50, 50), gfx::Size(100, 100));
    host_impl_->OuterViewportScrollNode()->main_thread_repaint_reasons = MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects;
    DrawFrame();

    InputHandler::ScrollStatus status
        = GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
    EXPECT_EQ(MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects, status.main_thread_repaint_reasons);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);
    EXPECT_EQ(MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects, host_impl_->CurrentlyScrollingNode()->main_thread_repaint_reasons);
...
TEST_P(LayerTreeHostImplTest, ScrollWithOverlappingTransparentNonScrollableLayer)
{
...
TEST_P(LayerTreeHostImplTest, ScrolledOverlappingDrawnScrollbarLayer)
{
    ...
    SetupViewportLayersNoScrolls(content_size);
    LayerImpl* scroll = AddScrollableLayer(OuterViewportScrollLayer(), content_size, scroll_content_size);

    auto* drawn_scrollbar = AddLayer<PaintedScrollbarLayerImpl>(layer_tree_impl, ScrollbarOrientation::kVertical, false, true);
    ...
    auto status
        = GetInputHandler().ScrollBegin(BeginState(gfx::Point(350, 150), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);
    GetInputHandler().ScrollEnd();
    ...
    status
        = GetInputHandler().ScrollBegin(BeginState(gfx::Point(350, 350), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
    EXPECT_EQ(MainThreadScrollingReason::kFailedHitTest, status.main_thread_hit_test_reasons);
    GetInputHandler().ScrollEnd();

    // The point hits the drawn scrollbar layer completely and should scroll on
    // the impl thread.
    status
        = GetInputHandler().ScrollBegin(BeginState(gfx::Point(350, 500), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);
    GetInputHandler().ScrollEnd();
...
{
    auto feedback = gfx::PresentationFeedback(base::TimeTicks() + base::Milliseconds(42), base::Microseconds(18),
        gfx::PresentationFeedback::Flags::kVSync | gfx::PresentationFeedback::Flags::kHWCompletion);
#if BUILDFLAG(IS_MAC)
...
class LayerTreeHostImplTestInvokePresentationCallbacks : public LayerTreeHostImplTest {
public:
    void DidPresentCompositorFrameOnImplThread(
        uint32_t frame_token, PresentationTimeCallbackBuffer::PendingCallbacks activated, const viz::FrameTimingDetails& details) override
    {
...
INSTANTIATE_COMMIT_TO_TREE_TEST_P(LayerTreeHostImplTestInvokePresentationCallbacks);
...
TEST_P(LayerTreeHostImplTestInvokePresentationCallbacks, PresentationFeedbackCallbacksFire)
{
    ...
    host_impl_->RegisterCompositorThreadSuccessfulPresentationTimeCallbackForTesting(
        frame_token_1, base::BindLambdaForTesting([&](base::TimeTicks presentation_timestamp) {
            DCHECK(compositor_successful_callback_presentation_timestamp.is_null());
            DCHECK(!presentation_timestamp.is_null());
            compositor_successful_callback_fired = true;
            compositor_successful_callback_presentation_timestamp = presentation_timestamp;
        }));

    // Register a main-thread presentation callback to run when the presentation
    // feedback for `frame_token_2` is received.
    host_impl_->RegisterMainThreadPresentationTimeCallbackForTesting(frame_token_2, base::BindLambdaForTesting([&](const gfx::PresentationFeedback& feedback) {
    ...
    host_impl_->RegisterMainThreadSuccessfulPresentationTimeCallbackForTesting(
        frame_token_2, base::BindLambdaForTesting([&](const viz::FrameTimingDetails& details) {
            base::TimeTicks presentation_timestamp = details.presentation_feedback.timestamp;
            DCHECK(main_successful_callback_presentation_timestamp.is_null());
            DCHECK(!presentation_timestamp.is_null());
            main_successful_callback_fired = true;
            main_successful_callback_presentation_timestamp = presentation_timestamp;
        }));
    ...
    EXPECT_EQ(compositor_successful_callback_presentation_timestamp, mock_details.presentation_feedback.timestamp);
    ...
    EXPECT_FALSE(main_successful_callback_fired);
    EXPECT_EQ(main_callback_presentation_feedback, mock_details.presentation_feedback);
    EXPECT_TRUE(main_successful_callback_presentation_timestamp.is_null());
    ...
    EXPECT_EQ(main_successful_callback_presentation_timestamp, mock_details.presentation_feedback.timestamp);
}
...
TEST_P(LayerTreeHostImplTest, MainThreadScrollHitTestRegionBasic)
{
    ...
    InputHandler::ScrollStatus status
        = GetInputHandler().ScrollBegin(BeginState(gfx::Point(25, 25), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
    EXPECT_EQ(MainThreadScrollingReason::kMainThreadScrollHitTestRegion, status.main_thread_hit_test_reasons);

    status = GetInputHandler().ScrollBegin(
        BeginState(gfx::Point(25, 25), gfx::Vector2d(0, 10), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
    EXPECT_EQ(MainThreadScrollingReason::kMainThreadScrollHitTestRegion, status.main_thread_hit_test_reasons);
    ...
    status
        = GetInputHandler().ScrollBegin(BeginState(gfx::Point(75, 75), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);
    ...
    status = GetInputHandler().ScrollBegin(
        BeginState(gfx::Point(75, 75), gfx::Vector2d(0, 10), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 10), ui::ScrollInputType::kTouchscreen));
...
TEST_P(LayerTreeHostImplTest, MainThreadScrollHitTestRegionInNonScrollingRoot)
{
    ...
    std::unique_ptr<ScrollState> begin_state = BeginState(gfx::Point(20, 20), gfx::Vector2dF(0, 10), ui::ScrollInputType::kWheel);
    InputHandler::ScrollStatus status;

    for (LayerImpl* layer : layers) {
        layer->SetMainThreadScrollHitTestRegion(gfx::Rect(10, 10, 20, 20));
        status = input_handler.ScrollBegin(begin_state.get(), ui::ScrollInputType::kWheel);
...
TEST_P(LayerTreeHostImplTest, MainThreadScrollHitTestRegionWithOffset)
{
    ...
    InputHandler::ScrollStatus status
        = GetInputHandler().ScrollBegin(BeginState(gfx::Point(40, 10), gfx::Vector2d(0, 1), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);

    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 1), ui::ScrollInputType::kWheel));
    GetInputHandler().ScrollEnd();

    // This point is still inside the MainThreadScrollHitTestRegion.
    status = GetInputHandler().ScrollBegin(BeginState(gfx::Point(10, 10), gfx::Vector2d(0, 1), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
    EXPECT_EQ(MainThreadScrollingReason::kMainThreadScrollHitTestRegion, status.main_thread_hit_test_reasons);
}
...
TEST_P(LayerTreeHostImplTest, LayerOverlapsMainThreadScrollHitTestRegionInLayer)
{
    ...
    {
        ASSERT_EQ(layer_b, host_impl_->active_tree()->FindLayerThatIsHitByPoint(gfx::PointF(60, 50)));
        InputHandler::ScrollStatus status = GetInputHandler().ScrollBegin(
            BeginState(gfx::Point(60, 50), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
        ASSERT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
        ASSERT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
        ASSERT_EQ(MainThreadScrollingReason::kMainThreadScrollHitTestRegion, status.main_thread_hit_test_reasons);
        GetInputHandler().ScrollEnd();
    }
    ...
    {
        ASSERT_EQ(layer_c, host_impl_->active_tree()->FindLayerThatIsHitByPoint(gfx::PointF(40, 50)));
        InputHandler::ScrollStatus status = GetInputHandler().ScrollBegin(
            BeginState(gfx::Point(40, 50), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
        ASSERT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
        EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
        EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);
        ASSERT_EQ(host_impl_->CurrentlyScrollingNode(), host_impl_->OuterViewportScrollNode());
        GetInputHandler().ScrollEnd();
    }
    ...
    {
        ASSERT_EQ(layer_c, host_impl_->active_tree()->FindLayerThatIsHitByPoint(gfx::PointF(60, 50)));
        InputHandler::ScrollStatus status = GetInputHandler().ScrollBegin(
            BeginState(gfx::Point(60, 50), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
        EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
        EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
...
TEST_P(LayerTreeHostImplTest, LayerOverlapsMainThreadScrollHitTestRegionInNonScrollAncestorLayer)
{
    SetupViewportLayersOuterScrolls(gfx::Size(100, 100), gfx::Size(200, 200));
    LayerImpl* outer_scroll = OuterViewportScrollLayer();

    LayerImpl* layer_a = AddScrollableLayer(outer_scroll, gfx::Size(100, 100), gfx::Size(200, 200));
    ...
    {
        ASSERT_EQ(layer_b, host_impl_->active_tree()->FindLayerThatIsHitByPoint(gfx::PointF(75, 50)));
        InputHandler::ScrollStatus status = GetInputHandler().ScrollBegin(
            BeginState(gfx::Point(75, 50), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
        ASSERT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
        ASSERT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
        ASSERT_EQ(MainThreadScrollingReason::kFailedHitTest, status.main_thread_hit_test_reasons);
        GetInputHandler().ScrollEnd();
    ...
    // scrolling in Layer A in either case since C scrolls with A.
    {
        ...
        EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
        EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);
        ASSERT_EQ(host_impl_->CurrentlyScrollingNode()->id, layer_a->scroll_tree_index());
        GetInputHandler().ScrollEnd();
    }
...
// the inner viewport.
TEST_P(LayerTreeHostImplTest, FixedLayerOverlapsMainThreadScrollHitTestRegionInLayer)
{
    SetupViewportLayersOuterScrolls(gfx::Size(100, 100), gfx::Size(200, 200));
    ...
    // scrolling in Layer A in either case since C scrolls with A.
    {
        ...
        ASSERT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
        ASSERT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
        ASSERT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);
        ASSERT_EQ(host_impl_->CurrentlyScrollingNode(), host_impl_->OuterViewportScrollNode());
        GetInputHandler().ScrollEnd();
    }
...
TEST_P(LayerTreeHostImplTest, FixedLayerOverNonFixedLayer)
{
    ...
    {
        GetScrollNode(InnerViewportScrollLayer())->prevent_viewport_scrolling_from_inner = true;
...
TEST_P(LayerTreeHostImplTest, ScrollHandlerNotPresent)
{
    ...
    GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2d(0, 10), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
    EXPECT_FALSE(host_impl_->ScrollAffectsScrollHandler());
    GetInputHandler().ScrollEnd();
    EXPECT_FALSE(host_impl_->ScrollAffectsScrollHandler());
}
...
TEST_P(LayerTreeHostImplTest, ScrollUpdateReturnsCorrectValue)
{
    ...
    InputHandler::ScrollStatus status = GetInputHandler().ScrollBegin(
        BeginState(gfx::Point(), gfx::Vector2d(-10, 0), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);

    // Trying to scroll to the left/top will not succeed.
    EXPECT_FALSE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(-10, 0), ui::ScrollInputType::kTouchscreen)).did_scroll);
    EXPECT_FALSE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, -10), ui::ScrollInputType::kTouchscreen)).did_scroll);
    EXPECT_FALSE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(-10, -10), ui::ScrollInputType::kTouchscreen)).did_scroll);

    // Scrolling to the right/bottom will succeed.
    EXPECT_TRUE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(10, 0), ui::ScrollInputType::kTouchscreen)).did_scroll);
    EXPECT_TRUE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 10), ui::ScrollInputType::kTouchscreen)).did_scroll);
    EXPECT_TRUE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(10, 10), ui::ScrollInputType::kTouchscreen)).did_scroll);
    ...
    EXPECT_TRUE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(5000, 5000), ui::ScrollInputType::kTouchscreen)).did_scroll);
}
...
TEST_P(LayerTreeHostImplTest, ScrollSnapOnX)
{
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler().ScrollBegin(BeginState(pointer_position, x_delta, ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel).thread);
    EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(overflow));
    EXPECT_EQ(TargetSnapAreaElementIds(), GetSnapContainerData(overflow)->GetTargetSnapAreaElementIds());

    GetInputHandler().ScrollUpdate(UpdateState(pointer_position, x_delta, ui::ScrollInputType::kWheel));
    EXPECT_EQ(TargetSnapAreaElementIds(), GetSnapContainerData(overflow)->GetTargetSnapAreaElementIds());
    ...
    EXPECT_TRUE(GetInputHandler().animating_for_snap_for_testing(overflow->element_id()));
    EXPECT_EQ(TargetSnapAreaElementIds(), GetSnapContainerData(overflow)->GetTargetSnapAreaElementIds());

    base::TimeTicks start_time = base::TimeTicks() + base::Milliseconds(100);
    BeginImplFrameAndAnimate(begin_frame_args, start_time);
    BeginImplFrameAndAnimate(begin_frame_args, start_time + base::Milliseconds(50));
    BeginImplFrameAndAnimate(begin_frame_args, start_time + base::Milliseconds(1000));

    EXPECT_FALSE(GetInputHandler().animating_for_snap_for_testing(overflow->element_id()));
    EXPECT_POINTF_EQ(gfx::PointF(50, 0), CurrentScrollOffset(overflow));
    EXPECT_EQ(TargetSnapAreaElementIds(ElementId(10), ElementId()), GetSnapContainerData(overflow)->GetTargetSnapAreaElementIds());
}
...
TEST_P(LayerTreeHostImplTest, ScrollSnapOnY)
{
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler().ScrollBegin(BeginState(pointer_position, y_delta, ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel).thread);
    EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(overflow));
    EXPECT_EQ(TargetSnapAreaElementIds(), GetSnapContainerData(overflow)->GetTargetSnapAreaElementIds());

    GetInputHandler().ScrollUpdate(UpdateState(pointer_position, y_delta, ui::ScrollInputType::kWheel));
    EXPECT_EQ(TargetSnapAreaElementIds(), GetSnapContainerData(overflow)->GetTargetSnapAreaElementIds());
    ...
    EXPECT_EQ(TargetSnapAreaElementIds(ElementId(), ElementId(10)), GetSnapContainerData(overflow)->GetTargetSnapAreaElementIds());
}
...
TEST_P(LayerTreeHostImplTest, ScrollSnapOnBoth)
{
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler().ScrollBegin(BeginState(pointer_position, delta, ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel).thread);
    EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(overflow));
    EXPECT_EQ(TargetSnapAreaElementIds(), GetSnapContainerData(overflow)->GetTargetSnapAreaElementIds());

    GetInputHandler().ScrollUpdate(UpdateState(pointer_position, delta, ui::ScrollInputType::kWheel));
    EXPECT_EQ(TargetSnapAreaElementIds(), GetSnapContainerData(overflow)->GetTargetSnapAreaElementIds());
    ...
    EXPECT_EQ(TargetSnapAreaElementIds(ElementId(10), ElementId(10)), GetSnapContainerData(overflow)->GetTargetSnapAreaElementIds());
}
...
TEST_P(LayerTreeHostImplTest, ScrollSnapAfterAnimatedScroll)
{
    ...
    EXPECT_EQ(overflow->scroll_tree_index(), host_impl_->CurrentlyScrollingNode()->id);
    ...
    BeginImplFrameAndAnimate(begin_frame_args, start_time + base::Milliseconds(1500));
    EXPECT_POINTF_EQ(gfx::PointF(50, 50), CurrentScrollOffset(overflow));
    EXPECT_FALSE(GetInputHandler().animating_for_snap_for_testing(overflow->element_id()));
    EXPECT_EQ(TargetSnapAreaElementIds(ElementId(10), ElementId(10)), GetSnapContainerData(overflow)->GetTargetSnapAreaElementIds());
}
...
TEST_P(LayerTreeHostImplTest, SnapAnimationTargetUpdated)
{
    ...
    BeginImplFrameAndAnimate(begin_frame_args, start_time + base::Milliseconds(100));
    EXPECT_TRUE(GetInputHandler().animating_for_snap_for_testing(overflow->element_id()));
    EXPECT_EQ(TargetSnapAreaElementIds(), GetSnapContainerData(overflow)->GetTargetSnapAreaElementIds());
    ...
    // animation.
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(10, 10), gfx::Vector2dF(0, -10)));
    EXPECT_FALSE(GetInputHandler().animating_for_snap_for_testing(overflow->element_id()));
    // Finish the smooth scroll animation for wheel.
    const int scroll_animation_duration_ms = features::IsImpulseScrollAnimationEnabled() ? 300 : 150;
    BeginImplFrameAndAnimate(begin_frame_args, start_time + base::Milliseconds(scroll_animation_duration_ms));
...
TEST_P(LayerTreeHostImplTest, SnapAnimationCancelledByScroll)
{
    ...
    auto begin_state = BeginState(pointer_position, x_delta, ui::ScrollInputType::kWheel);
    begin_state->data()->delta_granularity = ui::ScrollGranularity::kScrollByPrecisePixel;
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, GetInputHandler().ScrollBegin(begin_state.get(), ui::ScrollInputType::kWheel).thread);
    EXPECT_FALSE(GetInputHandler().animating_for_snap_for_testing(overflow->element_id()));
    EXPECT_EQ(TargetSnapAreaElementIds(), GetSnapContainerData(overflow)->GetTargetSnapAreaElementIds());

    BeginImplFrameAndAnimate(begin_frame_args, start_time + base::Milliseconds(150));
...
TEST_P(LayerTreeHostImplTest, SnapAnimationShouldNotStartWhenScrollEndsAtSnapTarget)
{
    LayerImpl* overflow = CreateLayerForSnapping();
...
TEST_P(LayerTreeHostImplTest, GetSnapFlingInfoAndSetAnimatingSnapTargetWhenZoomed)
{
    LayerImpl* overflow = CreateLayerForSnapping();
    ...
    InputHandlerScrollResult result = GetInputHandler().ScrollUpdate(UpdateState(pointer_position, delta, ui::ScrollInputType::kWheel));
    ...
    EXPECT_TRUE(GetInputHandler().GetSnapFlingInfoAndSetAnimatingSnapTarget(gfx::Vector2dF(1, 1), gfx::Vector2dF(10, 10), &initial_offset, &target_offset));
    EXPECT_TRUE(GetInputHandler().animating_for_snap_for_testing(overflow->element_id()));
    ...
    GetInputHandler().ScrollUpdate(UpdateState(pointer_position, gfx::Vector2dF(6, 6), ui::ScrollInputType::kWheel));
...
TEST_P(LayerTreeHostImplTest, NativeFlingInSnapArea)
{
    ...
    LayerImpl* overflow = AddScrollableLayer(OuterViewportScrollLayer(), view_size, overflow_size);

    SnapContainerData container(ScrollSnapType(false, SnapAxis::kY, SnapStrictness::kMandatory), gfx::RectF(0, 0, 100, 100), gfx::PointF(0, 900));
    ScrollSnapAlign start = ScrollSnapAlign(SnapAlignment::kStart);
    container.AddSnapAreaData(SnapAreaData(start, snap_area_1, false, false, ElementId(10)));
    container.AddSnapAreaData(SnapAreaData(start, snap_area_2, false, false, ElementId(20)));
    ...
    handler.ScrollBegin(&*BeginState(position, gfx::Vector2dF(0, 100), type), type);
    ...
    EXPECT_FALSE(handler.GetSnapFlingInfoAndSetAnimatingSnapTarget(gfx::Vector2dF(0, 10), gfx::Vector2dF(0, 400), &initial_offset, &target_offset));
    ...
    EXPECT_TRUE(handler.GetSnapFlingInfoAndSetAnimatingSnapTarget(gfx::Vector2dF(0, 100), gfx::Vector2dF(0, 4000), &initial_offset, &target_offset));
    ...
    EXPECT_TRUE(handler.GetSnapFlingInfoAndSetAnimatingSnapTarget(gfx::Vector2dF(0, 10), gfx::Vector2dF(0, 400), &initial_offset, &target_offset));
    EXPECT_TRUE(handler.animating_for_snap_for_testing(overflow->element_id()));
    EXPECT_POINTF_EQ(gfx::PointF(0, 550), initial_offset);
...
TEST_P(LayerTreeHostImplTest, OverscrollBehaviorPreventsPropagation)
{
    ...
    GetScrollNode(overflow)->overscroll_behavior = OverscrollBehavior(OverscrollBehavior::Type::kContain, OverscrollBehavior::Type::kAuto);
    ...
    // case.
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler().ScrollBegin(BeginState(pointer_position, diagonal_delta, ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel).thread);
    EXPECT_POINTF_EQ(gfx::PointF(20, 20), CurrentScrollOffset(scroll_layer));
    EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(overflow));

    GetInputHandler().ScrollUpdate(UpdateState(pointer_position, diagonal_delta, ui::ScrollInputType::kWheel));
    GetInputHandler().ScrollEnd();
    EXPECT_POINTF_EQ(gfx::PointF(20, 20), CurrentScrollOffset(scroll_layer));
    EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(overflow));

    // Changing scroll-boundary-behavior to y axis.
    GetScrollNode(overflow)->overscroll_behavior = OverscrollBehavior(OverscrollBehavior::Type::kAuto, OverscrollBehavior::Type::kContain);
    ...
    // overscroll-behavior.
    GetScrollNode(overflow)->overscroll_behavior = OverscrollBehavior(OverscrollBehavior::Type::kContain, OverscrollBehavior::Type::kContain);
...
TEST_P(LayerTreeHostImplTest, ScrollWithUserUnscrollableLayers)
{
    ...
    gfx::Size overflow_size(400, 400);
    LayerImpl* overflow = AddScrollableLayer(scroll_layer, gfx::Size(100, 100), overflow_size);
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler().ScrollBegin(BeginState(scroll_position, scroll_delta, ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel).thread);
    EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(scroll_layer));
    EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(overflow));

    GetInputHandler().ScrollUpdate(UpdateState(scroll_position, scroll_delta, ui::ScrollInputType::kWheel));
    GetInputHandler().ScrollEnd();
    EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(scroll_layer));
    EXPECT_POINTF_EQ(gfx::PointF(10, 10), CurrentScrollOffset(overflow));
...
TEST_P(LayerTreeHostImplTest, ScrollNodeWithoutScrollLayer)
{
    ...
    InputHandler::ScrollStatus status
        = GetInputHandler().ScrollBegin(BeginState(gfx::Point(25, 25), gfx::Vector2dF(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
...
TEST_F(CommitToPendingTreeLayerTreeHostImplTest, AnimationSchedulingPendingTree)
{
    CreatePendingTree();
    auto* root = SetupRootLayer<LayerImpl>(host_impl_->pending_tree(), gfx::Size(50, 50));
    ...
    scoped_refptr<Animation> animation = Animation::Create(AnimationIdProvider::NextAnimationId());
    ...
    animation->GetKeyframeModel(TargetProperty::TRANSFORM)->set_affects_active_elements(false);
...
TEST_F(CommitToPendingTreeLayerTreeHostImplTest, AnimationSchedulingActiveTree)
{
    ...
    AddAnimatedTransformToElementWithAnimation(child->element_id(), timeline(), 4.0, start, end);
    UpdateDrawProperties(host_impl_->active_tree());

    base::TimeTicks now = base::TimeTicks::Now();
    host_impl_->WillBeginImplFrame(viz::CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 2, now));
...
TEST_F(CommitToActiveTreeLayerTreeHostImplTest, AnimationSchedulingCommitToActiveTree)
{
    ...
    AddAnimatedTransformToElementWithAnimation(child->element_id(), timeline(), 10.0, 3, 0);
...
TEST_P(LayerTreeHostImplTest, AnimationSchedulingOnLayerDestruction)
{
    ...
    timeline()->AttachAnimation(animation);
    animation->AttachElement(child->element_id());
    int keyframe_model_id = AddAnimatedTransformToAnimation(animation.get(), 4.0, start, end);
    UpdateDrawProperties(host_impl_->active_tree());
...
public:
    static std::unique_ptr<IncompleteRecordingLayer> Create(LayerTreeImpl* tree_impl, int id)
    {
    ...
    void AppendQuads(viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data) override
    {
...
TEST_P(LayerTreeHostImplTest, ScrollCheckerboardsIncompleteRecording)
{
    ...
    scoped_feature_list.InitAndEnableFeature(features::kNewContentForCheckerboardedScrolls);
    LayerTreeSettings settings = DefaultSettings();
    CreateHostImpl(settings, CreateLayerTreeFrameSink());
    ...
    auto* scroll_layer = AddLayer<IncompleteRecordingLayer>(host_impl_->active_tree());
    ...
    GetInputHandler().ScrollBegin(
        BeginState(gfx::Point(250, 250), gfx::Vector2dF(), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
    ...
    // Send scroll update.
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(10, 10), scroll_delta, ui::ScrollInputType::kWheel));
...
TEST_P(LayerTreeHostImplTest, ImplPinchZoom)
{
    ...
    {
        host_impl_->active_tree()->PushPageScaleFromMainThread(page_scale_factor, min_page_scale, max_page_scale);
        ...
        GetInputHandler().ScrollBegin(
            BeginState(gfx::Point(50, 50), gfx::Vector2dF(), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
        GetInputHandler().PinchGestureBegin(gfx::Point(50, 50), ui::ScrollInputType::kWheel);
    ...
    // scroll deltas have the page scale factor applied.
    {
        ...
        float page_scale_delta = 2;
        GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
        GetInputHandler().PinchGestureBegin(gfx::Point(), ui::ScrollInputType::kWheel);
        ...
        EXPECT_EQ(ScrollThread::kScrollOnImplThread,
            GetInputHandler().ScrollBegin(BeginState(gfx::Point(5, 5), scroll_delta, ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel).thread);
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), scroll_delta, ui::ScrollInputType::kWheel));
        GetInputHandler().ScrollEnd();

        std::unique_ptr<CompositorCommitData> commit_data = host_impl_->ProcessCompositorDeltas(
            /* main_thread_mutator_host */ nullptr);
        EXPECT_TRUE(ScrollInfoContains(*commit_data.get(), scroll_layer->element_id(), gfx::Vector2dF(0, scroll_delta.y() / page_scale_delta)));
    }
}
...
TEST_P(LayerTreeHostImplTest, ViewportScrollbarGeometry)
{
    ...
    const float minimum_scale = viewport_size.width() / static_cast<float>(content_size.width());
    ...
    auto* v_scrollbar = AddLayer<PaintedScrollbarLayerImpl>(active_tree, ScrollbarOrientation::kVertical, false, true);
    auto* h_scrollbar = AddLayer<PaintedScrollbarLayerImpl>(active_tree, ScrollbarOrientation::kHorizontal, false, true);
...
TEST_P(LayerTreeHostImplTest, ViewportScrollOrder)
{
    ...
    GetInputHandler().ScrollBegin(
        BeginState(gfx::Point(0, 0), gfx::Vector2dF(100, 100), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(0, 0), gfx::Vector2dF(100, 100), ui::ScrollInputType::kTouchscreen));
    GetInputHandler().ScrollEnd();

    EXPECT_POINTF_EQ(gfx::PointF(50, 50), CurrentScrollOffset(inner_scroll_layer));
    EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(outer_scroll_layer));

    // Scroll down - outer viewport should start scrolling after the inner is at
    // its maximum.
    GetInputHandler().ScrollBegin(
        BeginState(gfx::Point(0, 0), gfx::Vector2dF(1000, 1000), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(0, 0), gfx::Vector2dF(1000, 1000), ui::ScrollInputType::kTouchscreen));
    GetInputHandler().ScrollEnd();

    EXPECT_POINTF_EQ(gfx::PointF(250, 250), CurrentScrollOffset(inner_scroll_layer));
    EXPECT_POINTF_EQ(gfx::PointF(300, 300), CurrentScrollOffset(outer_scroll_layer));
}
...
TEST_P(LayerTreeHostImplTest, ScrollViewportWithFractionalAmounts)
{
    ...
    GetInputHandler().ScrollBegin(
        BeginState(gfx::Point(250, 250), gfx::Vector2dF(0.125f, 0.125f), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(250, 250), gfx::Vector2dF(0.125f, 0.125f), ui::ScrollInputType::kTouchscreen));
    EXPECT_POINTF_EQ(gfx::PointF(0.125f, 0.125f), CurrentScrollOffset(outer_scroll_layer));
    EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(inner_scroll_layer));
    GetInputHandler().ScrollEnd();
    ...
    GetInputHandler().ScrollBegin(
        BeginState(gfx::Point(250, 250), gfx::Vector2dF(0.5f, 0.5f), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(250, 250), gfx::Vector2dF(0.5f, 0.5f), ui::ScrollInputType::kTouchscreen));
    EXPECT_POINTF_EQ(gfx::PointF(0.125f, 0.125f), CurrentScrollOffset(outer_scroll_layer));
    EXPECT_POINTF_EQ(gfx::PointF(0.25f, 0.25f), CurrentScrollOffset(inner_scroll_layer));
    GetInputHandler().ScrollEnd();
}
...
TEST_P(LayerTreeHostImplTest, ScrollDuringPinchGesture)
{
    ...
    GetInputHandler().ScrollBegin(
        BeginState(gfx::Point(250, 250), gfx::Vector2dF(), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
    GetInputHandler().PinchGestureBegin(gfx::Point(250, 250), ui::ScrollInputType::kWheel);

    GetInputHandler().PinchGestureUpdate(2, gfx::Point(250, 250));
    EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(outer_scroll_layer));
    EXPECT_POINTF_EQ(gfx::PointF(125, 125), CurrentScrollOffset(inner_scroll_layer));

    // Needed so that the pinch is accounted for in draw properties.
    DrawFrame();

    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(250, 250), gfx::Vector2dF(10, 10), ui::ScrollInputType::kTouchscreen));
    EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(outer_scroll_layer));
    EXPECT_POINTF_EQ(gfx::PointF(130, 130), CurrentScrollOffset(inner_scroll_layer));

    DrawFrame();

    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(250, 250), gfx::Vector2dF(400, 400), ui::ScrollInputType::kTouchscreen));
    EXPECT_POINTF_EQ(gfx::PointF(80, 80), CurrentScrollOffset(outer_scroll_layer));
    EXPECT_POINTF_EQ(gfx::PointF(250, 250), CurrentScrollOffset(inner_scroll_layer));
...
TEST_P(LayerTreeHostImplTest, PinchZoomSnapsToScreenEdge)
{
    ...
    gfx::Point anchor(viewport_size.width() - offsetFromEdge, viewport_size.height() - offsetFromEdge);

    // Pinch in within the margins. The scroll should stay exactly locked to the
    // bottom and right.
    GetInputHandler().ScrollBegin(BeginState(anchor, gfx::Vector2dF(), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
    ...
    EXPECT_POINTF_EQ(gfx::PointF(250, 250), CurrentScrollOffset(InnerViewportScrollLayer()));
    ...
    GetInputHandler().ScrollBegin(BeginState(anchor, gfx::Vector2dF(), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
    GetInputHandler().PinchGestureBegin(gfx::Point(100, 100), ui::ScrollInputType::kWheel);
    GetInputHandler().PinchGestureUpdate(2, anchor);
    GetInputHandler().PinchGestureEnd(anchor);
    GetInputHandler().ScrollEnd();

    EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(InnerViewportScrollLayer()));
    ...
    EXPECT_POINTF_EQ(gfx::PointF(50, 50), CurrentScrollOffset(InnerViewportScrollLayer()));
    ...
    // Pinch in just outside the margin. There should be no snapping.
    offsetFromEdge = Viewport::kPinchZoomSnapMarginDips;
    anchor = gfx::Point(viewport_size.width() - offsetFromEdge, viewport_size.height() - offsetFromEdge);
    GetInputHandler().ScrollBegin(BeginState(anchor, gfx::Vector2dF(), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
    GetInputHandler().PinchGestureBegin(anchor, ui::ScrollInputType::kWheel);
    GetInputHandler().PinchGestureUpdate(2, anchor);
    GetInputHandler().PinchGestureEnd(anchor);
    ...
    EXPECT_POINTF_EQ(gfx::PointF(200, 200), CurrentScrollOffset(InnerViewportScrollLayer()));
}
...
TEST_P(LayerTreeHostImplTest, ImplPinchZoomWheelBubbleBetweenViewports)
{
    ...
    // viewport.
    GetInputHandler().ScrollBegin(BeginState(gfx::Point(0, 0), gfx::Vector2dF(10, 20), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(0, 0), gfx::Vector2dF(10, 20), ui::ScrollInputType::kWheel));
    GetInputHandler().ScrollEnd();
    ...
    GetInputHandler().ScrollBegin(BeginState(gfx::Point(0, 0), gfx::Vector2dF(100, 100), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(0, 0), gfx::Vector2dF(100, 100), ui::ScrollInputType::kWheel));
    GetInputHandler().ScrollEnd();
    ...
    // outer viewport.
    GetInputHandler().ScrollBegin(BeginState(gfx::Point(0, 0), gfx::Vector2dF(190, 180), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(0, 0), gfx::Vector2dF(190, 180), ui::ScrollInputType::kWheel));
    GetInputHandler().ScrollEnd();

    EXPECT_POINTF_EQ(gfx::PointF(100, 100), CurrentScrollOffset(outer_scroll_layer));
    EXPECT_POINTF_EQ(gfx::PointF(50, 50), CurrentScrollOffset(inner_scroll_layer));
}
...
TEST_P(LayerTreeHostImplTest, ScrollWithSwapPromises)
{
    ...
    std::unique_ptr<SwapPromise> swap_promise(new LatencyInfoSwapPromise(latency_info));

    SetupViewportLayersInnerScrolls(gfx::Size(50, 50), gfx::Size(100, 100));
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler()
            .ScrollBegin(BeginState(gfx::Point(), gfx::Vector2d(0, 10), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
            .thread);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 10), ui::ScrollInputType::kTouchscreen));
    host_impl_->QueueSwapPromiseForMainThreadScrollUpdate(std::move(swap_promise));
    GetInputHandler().ScrollEnd();

    std::unique_ptr<CompositorCommitData> commit_data = host_impl_->ProcessCompositorDeltas(
        /* main_thread_mutator_host */ nullptr);
    EXPECT_EQ(1u, commit_data->swap_promises.size());
    EXPECT_EQ(latency_info.trace_id(), commit_data->swap_promises[0]->GetTraceId());
}
...
TEST_P(LayerTreeHostImplTest, ScrollDoesntBubble)
{
    ...
    LayerImpl* scroll_parent = AddScrollableLayer(viewport_scroll, gfx::Size(5, 5), gfx::Size(10, 10));
    ...
    LayerImpl* scroll_child = AddScrollableLayer(scroll_child_clip, gfx::Size(5, 5), gfx::Size(10, 10));
    ...
    {
        GetInputHandler().ScrollBegin(
            BeginState(gfx::Point(21, 21), gfx::Vector2d(5, 5), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(21, 21), gfx::Vector2d(5, 5), ui::ScrollInputType::kTouchscreen));
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(21, 21), gfx::Vector2d(100, 100), ui::ScrollInputType::kTouchscreen));
        GetInputHandler().ScrollEnd();
        ...
        EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(InnerViewportScrollLayer()));
        EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(OuterViewportScrollLayer()));
    }

    {
        GetInputHandler().ScrollBegin(
            BeginState(gfx::Point(21, 21), gfx::Vector2d(3, 4), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(21, 21), gfx::Vector2d(3, 4), ui::ScrollInputType::kTouchscreen));
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(21, 21), gfx::Vector2d(2, 1), ui::ScrollInputType::kTouchscreen));
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(21, 21), gfx::Vector2d(2, 1), ui::ScrollInputType::kTouchscreen));
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(21, 21), gfx::Vector2d(2, 1), ui::ScrollInputType::kTouchscreen));
        GetInputHandler().ScrollEnd();
...
TEST_P(LayerTreeHostImplTest, PinchGesture)
{
    ...
    {
        host_impl_->active_tree()->PushPageScaleFromMainThread(1, min_page_scale, max_page_scale);
        SetScrollOffsetDelta(scroll_layer, gfx::Vector2d());
    ...
    // Zoom-out clamping
    {
        host_impl_->active_tree()->PushPageScaleFromMainThread(1, min_page_scale, max_page_scale);
        SetScrollOffsetDelta(scroll_layer, gfx::Vector2d());
        scroll_layer->layer_tree_impl()->property_trees()->scroll_tree_mutable().CollectScrollDeltasForTesting();
        ClearMainThreadDeltasForTesting(host_impl_.get());
        scroll_layer->layer_tree_impl()->property_trees()->scroll_tree_mutable().UpdateScrollOffsetBaseForTesting(
    ...
    // Two-finger panning should not happen based on pinch events only
    {
        ...
        GetInputHandler().ScrollBegin(
            BeginState(gfx::Point(10, 10), gfx::Vector2dF(), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
        GetInputHandler().PinchGestureBegin(gfx::Point(10, 10), ui::ScrollInputType::kWheel);
    ...
    // Two-finger panning should work with interleaved scroll events
    {
        ...
        float page_scale_delta = 1;
        GetInputHandler().ScrollBegin(
            BeginState(gfx::Point(10, 10), gfx::Vector2dF(-10, -10), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
        GetInputHandler().PinchGestureBegin(gfx::Point(10, 10), ui::ScrollInputType::kWheel);
        GetInputHandler().PinchGestureUpdate(page_scale_delta, gfx::Point(10, 10));
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(10, 10), gfx::Vector2d(-10, -10), ui::ScrollInputType::kTouchscreen));
        GetInputHandler().PinchGestureUpdate(page_scale_delta, gfx::Point(20, 20));
        GetInputHandler().PinchGestureEnd(gfx::Point(20, 20));
        GetInputHandler().ScrollEnd();
        ...
        std::unique_ptr<CompositorCommitData> commit_data = host_impl_->ProcessCompositorDeltas(
            /* main_thread_mutator_host */ nullptr);
        EXPECT_EQ(commit_data->page_scale_delta, page_scale_delta);
        EXPECT_TRUE(ScrollInfoContains(*commit_data, scroll_layer->element_id(), gfx::Vector2dF(-10, -10)));
        ClearMainThreadDeltasForTesting(host_impl_.get());
    }
    ...
    // Two-finger panning should work when starting fully zoomed out.
    {
        ...
        GetInputHandler().ScrollBegin(
            BeginState(gfx::Point(0, 0), gfx::Vector2dF(), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
        GetInputHandler().PinchGestureBegin(gfx::Point(0, 0), ui::ScrollInputType::kWheel);
        GetInputHandler().PinchGestureUpdate(2, gfx::Point(0, 0));
        ...
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(0, 0), gfx::Vector2d(10, 10), ui::ScrollInputType::kTouchscreen));
...
TEST_P(LayerTreeHostImplTest, SyncSubpixelScrollDelta)
{
    ...
    scroll_layer->layer_tree_impl()->property_trees()->scroll_tree_mutable().CollectScrollDeltasForTesting();
    ClearMainThreadDeltasForTesting(host_impl_.get());
    scroll_layer->layer_tree_impl()->property_trees()->scroll_tree_mutable().UpdateScrollOffsetBaseForTesting(scroll_layer->element_id(), gfx::PointF(0, 20));

    float page_scale_delta = 1;
    GetInputHandler().ScrollBegin(BeginState(gfx::Point(10, 10), gfx::Vector2dF(), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
    GetInputHandler().PinchGestureBegin(gfx::Point(10, 10), ui::ScrollInputType::kWheel);
    GetInputHandler().PinchGestureUpdate(page_scale_delta, gfx::Point(10, 10));
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(10, 10), gfx::Vector2dF(0, -1.001f), ui::ScrollInputType::kTouchscreen));
    ...
    std::unique_ptr<CompositorCommitData> commit_data = host_impl_->ProcessCompositorDeltas(
        /* main_thread_mutator_host */ nullptr);
    EXPECT_EQ(commit_data->page_scale_delta, page_scale_delta);
    EXPECT_TRUE(ScrollInfoContains(*commit_data, scroll_layer->element_id(), gfx::Vector2dF(0, -1)));

    // Verify this scroll delta is consistent with the snapped position of the
    // scroll layer.
    draw_property_utils::ComputeTransforms(
        &scroll_layer->layer_tree_impl()->property_trees()->transform_tree_mutable(), scroll_layer->layer_tree_impl()->viewport_property_ids());
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0, -19), scroll_layer->ScreenSpaceTransform().To2dTranslation());
}
...
TEST_P(LayerTreeHostImplTest, SyncSubpixelScrollFromFractionalActiveBase)
{
    ...
    ClearMainThreadDeltasForTesting(host_impl_.get());
    scroll_layer->layer_tree_impl()->property_trees()->scroll_tree_mutable().UpdateScrollOffsetBaseForTesting(
        scroll_layer->element_id(), gfx::PointF(0, 20.5f));

    GetInputHandler().ScrollBegin(BeginState(gfx::Point(10, 10), gfx::Vector2dF(0, -1), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(10, 10), gfx::Vector2dF(0, -1), ui::ScrollInputType::kWheel));
    GetInputHandler().ScrollEnd();

    gfx::PointF active_base = host_impl_->active_tree()->property_trees()->scroll_tree().GetScrollOffsetBaseForTesting(scroll_layer->element_id());
    EXPECT_POINTF_EQ(active_base, gfx::PointF(0, 20.5));
    // Fractional active base should not affect the scroll delta.
    std::unique_ptr<CompositorCommitData> commit_data = host_impl_->ProcessCompositorDeltas(
        /* main_thread_mutator_host */ nullptr);
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0, -1), commit_data->inner_viewport_scroll.scroll_delta);
}
...
TEST_P(LayerTreeHostImplTest, PageScaleAnimation)
{
    ...
    // Non-anchor zoom-in
    {
        ...
        did_request_redraw_ = false;
        did_request_next_frame_ = false;
        host_impl_->active_tree()->SetPendingPageScaleAnimation(std::make_unique<PendingPageScaleAnimation>(gfx::Point(), false, 2, duration));
        host_impl_->ActivateSyncTree();
        EXPECT_FALSE(did_request_redraw_);
        EXPECT_TRUE(did_request_next_frame_);
        ...
        std::unique_ptr<CompositorCommitData> commit_data = host_impl_->ProcessCompositorDeltas(
            /* main_thread_mutator_host */ nullptr);
        EXPECT_EQ(commit_data->page_scale_delta, 2);
        EXPECT_TRUE(ScrollInfoContains(*commit_data, scroll_layer->element_id(), gfx::Vector2dF(-50, -50)));
        ClearMainThreadDeltasForTesting(host_impl_.get());
    }
    ...
    // Anchor zoom-out
    {
        ...
        host_impl_->active_tree()->SetPendingPageScaleAnimation(
            std::make_unique<PendingPageScaleAnimation>(gfx::Point(25, 25), true, min_page_scale, duration));
        host_impl_->ActivateSyncTree();
        EXPECT_FALSE(did_request_redraw_);
        EXPECT_TRUE(did_request_next_frame_);
...
TEST_P(LayerTreeHostImplTest, PageScaleAnimationNoOp)
{
    ...
    // Anchor zoom with unchanged page scale should not change scroll or scale.
    {
        ...
        host_impl_->active_tree()->SetPendingPageScaleAnimation(std::make_unique<PendingPageScaleAnimation>(gfx::Point(), true, 1, duration));
        host_impl_->ActivateSyncTree();
        begin_frame_args.frame_time = start_time;
        begin_frame_args.frame_id.sequence_number++;
        host_impl_->WillBeginImplFrame(begin_frame_args);
...
TEST_P(LayerTreeHostImplTest, PageScaleAnimationTransferedOnSyncTreeActivate)
{
    ...
    SetupViewportLayers(host_impl_->sync_tree(), gfx::Size(50, 50), gfx::Size(100, 100), gfx::Size(100, 100));
    host_impl_->ActivateSyncTree();
    DrawFrame();
    ...
    float max_page_scale = 4;
    EnsureSyncTree();
    host_impl_->sync_tree()->PushPageScaleFromMainThread(1, min_page_scale, max_page_scale);
    host_impl_->ActivateSyncTree();
    ...
    scroll_layer->layer_tree_impl()->property_trees()->scroll_tree_mutable().UpdateScrollOffsetBaseForTesting(scroll_layer->element_id(), gfx::PointF(50, 50));

    // Make sure TakePageScaleAnimation works properly.
    EnsureSyncTree();
    host_impl_->sync_tree()->SetPendingPageScaleAnimation(std::make_unique<PendingPageScaleAnimation>(gfx::Point(), false, target_scale, duration));
    ...
    host_impl_->ActivateSyncTree();
    EnsureSyncTree();
    EXPECT_EQ(nullptr, host_impl_->sync_tree()->TakePendingPageScaleAnimation().get());
    EXPECT_FALSE(did_request_redraw_);
    EXPECT_TRUE(did_request_next_frame_);
...
TEST_P(LayerTreeHostImplTest, MaxScrollOffsetAffectedByViewportBoundsDelta)
{
    ...
    LayerImpl* inner_scroll = InnerViewportScrollLayer();
    DCHECK(inner_scroll);
    EXPECT_EQ(gfx::SizeF(50, 50), host_impl_->active_tree()->ScrollableViewportSize());
    ...
    EXPECT_EQ(gfx::SizeF(65, 65), host_impl_->active_tree()->ScrollableViewportSize());
...
TEST_P(LayerTreeHostImplTest, AnimatedGranularityCausesSmoothScroll)
{
    ...
    std::vector<ui::ScrollInputType> types = { ui::ScrollInputType::kScrollbar, ui::ScrollInputType::kWheel };
    for (auto type : types) {
        auto begin_state = BeginState(position, offset, type);
        begin_state->data()->set_current_native_scrolling_element(host_impl_->OuterViewportScrollNode()->element_id);
        begin_state->data()->delta_granularity = ui::ScrollGranularity::kScrollByPixel;

        auto update_state = UpdateState(position, offset, type);
        update_state.data()->delta_granularity = ui::ScrollGranularity::kScrollByPixel;
        ...
        GetImplAnimationHost()->ScrollAnimationAbort(host_impl_->OuterViewportScrollNode()->element_id);
        ASSERT_FALSE(GetImplAnimationHost()->HasImplOnlyScrollAnimatingElement());
        ...
        {
            begin_state->data()->delta_granularity = ui::ScrollGranularity::kScrollByPrecisePixel;
            update_state.data()->delta_granularity = ui::ScrollGranularity::kScrollByPrecisePixel;
...
public:
    LayerTreeHostImplOverridePhysicalTime(const LayerTreeSettings& settings, LayerTreeHostImplClient* client, TaskRunnerProvider* task_runner_provider,
        TaskGraphRunner* task_graph_runner, RenderingStatsInstrumentation* rendering_stats_instrumentation)
        : LayerTreeHostImpl(settings, client, task_runner_provider, rendering_stats_instrumentation, task_graph_runner,
            AnimationHost::CreateForTesting(ThreadInstance::kImpl), nullptr, 0, nullptr, nullptr)
    {
    }
    ...
    {
        current_begin_frame_args_ = viz::CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 1, fake_now);
    }
...
protected:
    void SetupLayers(LayerTreeSettings settings)
    {
        ...
        LayerTreeHostImplOverridePhysicalTime* host_impl_override_time
            = new LayerTreeHostImplOverridePhysicalTime(settings, this, &task_runner_provider_, &task_graph_runner_, &stats_instrumentation_);
        InputHandler::Create(static_cast<CompositorDelegateForInput&>(*host_impl_override_time));
        ...
        auto* scrollbar = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::kVertical, 10, 0, false);
    ...
    void RunTest(LayerTreeSettings::ScrollbarAnimator animator)
    {
        ...
        GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
        GetInputHandler().ScrollEnd();
        EXPECT_FALSE(did_request_next_frame_);
        EXPECT_FALSE(did_request_redraw_);
        EXPECT_EQ(base::TimeDelta(), requested_animation_delay_);
        ...
        GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, 0), ui::ScrollInputType::kWheel));
        GetInputHandler().ScrollEnd();
        EXPECT_FALSE(did_request_next_frame_);
        EXPECT_FALSE(did_request_redraw_);
        if (animator == LayerTreeSettings::AURA_OVERLAY) {
        ...
        viz::BeginFrameArgs begin_frame_args = viz::CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 2, fake_now);
        host_impl_->WillBeginImplFrame(begin_frame_args);
        host_impl_->Animate();
        EXPECT_FALSE(did_request_next_frame_);
        did_request_next_frame_ = false;
        ...
        // now.
        GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(0, 5), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, 5), ui::ScrollInputType::kWheel));
        EXPECT_FALSE(did_request_next_frame_);
        EXPECT_TRUE(did_request_redraw_);
        did_request_redraw_ = false;
        ...
        if (expecting_animations) {
            ...
            begin_frame_args = viz::CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 3, fake_now);
            host_impl_->WillBeginImplFrame(begin_frame_args);
            host_impl_->Animate();
            EXPECT_FALSE(did_request_next_frame_);
            did_request_next_frame_ = false;
            ...
            begin_frame_args = viz::CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 4, fake_now);
            host_impl_->WillBeginImplFrame(begin_frame_args);
            host_impl_->Animate();
            EXPECT_TRUE(did_request_next_frame_);
            did_request_next_frame_ = false;
        ...
        if (host_impl_->active_tree()->property_trees()->scroll_tree_mutable().UpdateScrollOffsetBaseForTesting(
                InnerViewportScrollLayer()->element_id(), gfx::PointF(5, 5)))
            host_impl_->active_tree()->DidUpdateScrollOffset(InnerViewportScrollLayer()->element_id(),
...
class LayerTreeHostImplTestScrollbarOpacity : public CommitToPendingTreeLayerTreeHostImplTest {
protected:
    void RunTest(LayerTreeSettings::ScrollbarAnimator animator)
    {
        LayerTreeSettings settings = DefaultSettings();
        ...
        CreateHostImpl(settings, CreateLayerTreeFrameSink());
        CreatePendingTree();
        SetupViewportLayers(host_impl_->pending_tree(), viewport_size, content_size, content_size);

        LayerImpl* scroll = host_impl_->pending_tree()->OuterViewportScrollLayerForTesting();
        auto* scrollbar = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->pending_tree(), ScrollbarOrientation::kVertical, 10, 0, false);
        SetupScrollbarLayer(scroll, scrollbar);
        ...
        LayerImpl* active_scrollbar_layer = host_impl_->active_tree()->LayerById(scrollbar->id());

        EffectNode* active_tree_node = GetEffectNode(active_scrollbar_layer);
        EXPECT_FLOAT_EQ(active_scrollbar_layer->Opacity(), active_tree_node->opacity);

        if (expecting_animations) {
            host_impl_->ScrollbarAnimationControllerForElementId(scroll->element_id())->DidMouseMove(gfx::PointF(0, 90));
        } else {
            EXPECT_EQ(nullptr, host_impl_->ScrollbarAnimationControllerForElementId(scroll->element_id()));
        }
        GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(0, 5), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, 5), ui::ScrollInputType::kWheel));
        GetInputHandler().ScrollEnd();
        ...
        auto& new_effect_node = CreateEffectNode(
            GetPropertyTrees(pending_root), pending_root->effect_tree_index(), pending_root->transform_tree_index(), pending_root->clip_tree_index());
        new_effect_node.render_surface_reason = RenderSurfaceReason::kTest;
        new_effect_node.element_id = ElementId(123);
        LayerImpl* pending_scrollbar_layer = host_impl_->pending_tree()->LayerById(scrollbar->id());
...
public:
    void SetUpLayers(LayerTreeSettings settings)
    {
        is_aura_scrollbar_ = settings.scrollbar_animator == LayerTreeSettings::AURA_OVERLAY;
        gfx::Size viewport_size(300, 200);
        gfx::Size content_size(1000, 1000);
        ...
        SetupViewportLayers(host_impl_->active_tree(), viewport_size, content_size, content_size);
        LayerImpl* root_scroll = OuterViewportScrollLayer();

        // scrollbar_1 on root scroll.
        scrollbar_1_ = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::kVertical, 15, 0, true);
        ...
        auto* child = AddScrollableLayer(root_scroll, gfx::Size(100, 100), child_layer_size);
        GetTransformNode(child)->post_translation = gfx::Vector2dF(50, 50);

        scrollbar_2_ = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::kVertical, 15, 0, true);
...
TEST_P(LayerTreeHostImplTestMultiScrollable, ScrollbarFlashAfterAnyScrollUpdate)
{
    LayerTreeSettings settings = DefaultSettings();
    settings.scrollbar_fade_delay = base::Milliseconds(500);
    settings.scrollbar_fade_duration = base::Milliseconds(300);
    ...
    GetInputHandler().RootScrollBegin(BeginState(gfx::Point(20, 20), gfx::Vector2dF(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(20, 20), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel));
    GetInputHandler().ScrollEnd();
    ...
    GetInputHandler().ScrollBegin(BeginState(gfx::Point(70, 70), gfx::Vector2dF(0, 100), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(70, 70), gfx::Vector2d(0, 100)));
    GetInputHandler().ScrollEnd();
...
TEST_P(LayerTreeHostImplTest, ScrollHitTestOnScrollbar)
{
    ...
    // scrollbar_1 on root scroll.
    auto* scrollbar_1 = AddLayer<PaintedScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::kVertical, true, true);
    ...
    LayerImpl* child = AddScrollableLayer(root_scroll, gfx::Size(100, 100), child_layer_size);
    GetTransformNode(child)->post_translation = gfx::Vector2dF(50, 50);

    // scrollbar_2 on child.
    auto* scrollbar_2 = AddLayer<PaintedScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::kVertical, true, true);
    ...
    {
        InputHandler::ScrollStatus status
            = GetInputHandler().ScrollBegin(BeginState(gfx::Point(1, 1), gfx::Vector2dF(), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
        EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
        GetInputHandler().ScrollEnd();
    }
    ...
    {
        InputHandler::ScrollStatus status = GetInputHandler().ScrollBegin(
            BeginState(gfx::Point(1, 1), gfx::Vector2dF(), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
        EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
        EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
        EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);
    }
    ...
    {
        InputHandler::ScrollStatus status
            = GetInputHandler().ScrollBegin(BeginState(gfx::Point(51, 51), gfx::Vector2dF(), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
        EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
        EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
        EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);
        GetInputHandler().ScrollEnd();
    ...
    {
        InputHandler::ScrollStatus status = GetInputHandler().ScrollBegin(
            BeginState(gfx::Point(51, 51), gfx::Vector2dF(), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
        EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
        EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
        EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);
    }
...
TEST_P(LayerTreeHostImplTest, NullScrollerLayerForScrollbarLayer)
{
    ...
    LayerImpl* scroll = AddScrollableLayer(root_layer(), gfx::Size(100, 100), gfx::Size(1000, 1000));
    auto* scrollbar = AddLayer<PaintedScrollbarLayerImpl>(layer_tree_impl, ScrollbarOrientation::kVertical, false, true);
    SetupScrollbarLayer(scroll, scrollbar);
    ...
    auto status
        = GetInputHandler().ScrollBegin(BeginState(gfx::Point(90, 50), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_hit_test_reasons);
    GetInputHandler().ScrollEnd();
    ...
    scrollbar->SetScrollElementId(ElementId(scrollbar->scroll_element_id().GetInternalValue() + 123));
    UpdateDrawProperties(layer_tree_impl);
    status
        = GetInputHandler().ScrollBegin(BeginState(gfx::Point(90, 50), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    EXPECT_EQ(ScrollThread::kScrollIgnored, status.thread);
...
TEST_F(CommitToPendingTreeLayerTreeHostImplTest, ScrollbarVisibilityChangeCausesRedrawAndCommit)
{
    LayerTreeSettings settings = DefaultSettings();
    settings.scrollbar_animator = LayerTreeSettings::AURA_OVERLAY;
    settings.scrollbar_fade_delay = base::Milliseconds(20);
    ...
    ScrollbarAnimationController* scrollbar_controller = host_impl_->ScrollbarAnimationControllerForElementId(scroll->element_id());
    ...
    EXPECT_THAT(commit_data->scrollbars, testing::ElementsAre(ScrollbarsInfo { scroll->element_id(), false }));
    EXPECT_FALSE(scrollbar_controller->visibility_changed());
    {
        ...
        commit_data = host_impl_->ProcessCompositorDeltas(
            /* main_thread_mutator_host */ nullptr);
        EXPECT_THAT(commit_data->scrollbars, testing::ElementsAre(ScrollbarsInfo { scroll->element_id(), true }));
        EXPECT_FALSE(scrollbar_controller->visibility_changed());
    }
...
TEST_P(LayerTreeHostImplTest, ScrollbarInnerLargerThanOuter)
{
    ...
    SetupViewportLayers(host_impl_->active_tree(), inner_viewport_size, outer_viewport_size, content_size);
    LayerImpl* root_scroll = OuterViewportScrollLayer();
    auto* horiz_scrollbar = AddLayer<PaintedScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::kHorizontal, true, true);
...
TEST_P(LayerTreeHostImplTest, ScrollbarRegistration)
{
    ...
    auto* vert_1_scrollbar = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::kVertical, 5, 5, true);
    CopyProperties(container, vert_1_scrollbar);

    auto* horiz_1_scrollbar = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::kHorizontal, 5, 5, true);
    CopyProperties(container, horiz_1_scrollbar);

    auto* vert_2_scrollbar = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::kVertical, 5, 5, true);
    CopyProperties(container, vert_2_scrollbar);

    auto* horiz_2_scrollbar = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::kHorizontal, 5, 5, true);
    ...
    EXPECT_EQ(nullptr, host_impl_->ScrollbarAnimationControllerForElementId(root_scroll->element_id()));
    ...
    EXPECT_TRUE(host_impl_->ScrollbarAnimationControllerForElementId(root_scroll->element_id()));
    ...
    animation_task_.Reset();
    GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(10, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(10, 10), ui::ScrollInputType::kWheel));
    GetInputHandler().ScrollEnd();
    EXPECT_FALSE(animation_task_.is_null());
    animation_task_.Reset();

    // Check scrollbar registration on a sublayer.
    LayerImpl* child = AddScrollableLayer(root_scroll, viewport_size, gfx::Size(200, 200));
    ElementId child_scroll_element_id = child->element_id();
    EXPECT_EQ(0ul, host_impl_->ScrollbarsFor(child_scroll_element_id).size());
    EXPECT_EQ(nullptr, host_impl_->ScrollbarAnimationControllerForElementId(child_scroll_element_id));
    ...
    EXPECT_EQ(nullptr, host_impl_->ScrollbarAnimationControllerForElementId(root_scroll_element_id));
    EXPECT_EQ(0ul, host_impl_->ScrollbarsFor(child_scroll_element_id).size());
    EXPECT_EQ(nullptr, host_impl_->ScrollbarAnimationControllerForElementId(root_scroll_element_id));
}
...
TEST_P(LayerTreeHostImplTest, ScrollBeforeMouseMove)
{
    ...
    auto* vert_scrollbar = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::kVertical, kScrollbarThickness, 0, false);
    SetupScrollbarLayer(root_scroll, vert_scrollbar);
    vert_scrollbar->SetBounds(gfx::Size(10, 200));
    vert_scrollbar->SetOffsetToTransformParent(gfx::Vector2dF(300 - kScrollbarThickness, 0));

    host_impl_->active_tree()->UpdateAllScrollbarGeometriesForTesting();

    EXPECT_EQ(1ul, host_impl_->ScrollbarsFor(root_scroll->element_id()).size());
    auto* scrollbar_controller = host_impl_->ScrollbarAnimationControllerForElementId(root_scroll->element_id());

    const float kDistanceToTriggerThumb = vert_scrollbar->ComputeThumbQuadRect().height()
        + scrollbar_controller->GetScrollbarAnimationController(ScrollbarOrientation::kVertical).MouseMoveDistanceToTriggerExpand();
    ...
    EXPECT_TRUE(scrollbar_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::kVertical));

    // Move the mouse away from the thumb.
    GetInputHandler().MouseMoveAt(gfx::Point(295, kDistanceToTriggerThumb + 1));
    EXPECT_FALSE(scrollbar_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::kVertical));

    // Scroll the page down which moves the thumb down to the viewport bottom.
    GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(0, 800), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 800), ui::ScrollInputType::kWheel));
    GetInputHandler().ScrollEnd();
...
void LayerTreeHostImplTestBase::SetupMouseMoveAtWithDeviceScale(float device_scale_factor)
{
    LayerTreeSettings settings = DefaultSettings();
    settings.scrollbar_fade_delay = base::Milliseconds(500);
    settings.scrollbar_fade_duration = base::Milliseconds(300);
    ...
    auto* scrollbar = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::kVertical, thumb_thickness, 0, true);
    ...
    ScrollbarAnimationController* scrollbar_animation_controller = host_impl_->ScrollbarAnimationControllerForElementId(root_scroll->element_id());

    const float kMouseMoveDistanceToTriggerFadeIn
        = scrollbar_animation_controller->GetScrollbarAnimationController(ScrollbarOrientation::kVertical).MouseMoveDistanceToTriggerFadeIn();

    const float kMouseMoveDistanceToTriggerExpand
        = scrollbar_animation_controller->GetScrollbarAnimationController(ScrollbarOrientation::kVertical).MouseMoveDistanceToTriggerExpand();

    GetInputHandler().MouseMoveAt(gfx::Point(thumb_thickness + kMouseMoveDistanceToTriggerFadeIn + 1, 1));
    EXPECT_FALSE(scrollbar_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::kVertical));
    EXPECT_FALSE(scrollbar_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::kVertical));

    GetInputHandler().MouseMoveAt(gfx::Point(thumb_thickness + kMouseMoveDistanceToTriggerExpand, 10));
    EXPECT_TRUE(scrollbar_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::kVertical));
    EXPECT_TRUE(scrollbar_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::kVertical));

    GetInputHandler().MouseMoveAt(gfx::Point(thumb_thickness + kMouseMoveDistanceToTriggerFadeIn + 1, 100));
    EXPECT_FALSE(scrollbar_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::kVertical));
    EXPECT_FALSE(scrollbar_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::kVertical));

    did_request_redraw_ = false;
    EXPECT_FALSE(scrollbar_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::kVertical));
    GetInputHandler().MouseMoveAt(gfx::Point(10, 10));
    EXPECT_TRUE(scrollbar_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::kVertical));
...
TEST_P(LayerTreeHostImplTest, ActivationDependenciesInMetadata)
{
    ...
    std::vector<viz::SurfaceId> primary_surfaces
        = { MakeSurfaceId(viz::FrameSinkId(1, 1), 1), MakeSurfaceId(viz::FrameSinkId(2, 2), 2), MakeSurfaceId(viz::FrameSinkId(3, 3), 3) };

    std::vector<viz::SurfaceId> fallback_surfaces
        = { MakeSurfaceId(viz::FrameSinkId(4, 4), 1), MakeSurfaceId(viz::FrameSinkId(4, 4), 2), MakeSurfaceId(viz::FrameSinkId(4, 4), 3) };

    for (size_t i = 0; i < primary_surfaces.size(); ++i) {
        auto* child = AddLayer<SurfaceLayerImpl>(host_impl_->active_tree());
        child->SetBounds(gfx::Size(1, 1));
        child->SetDrawsContent(true);
        child->SetRange(viz::SurfaceRange(fallback_surfaces[i], primary_surfaces[i]), 2u);
        CopyProperties(root, child);
    ...
    for (size_t i = 0; i < fallback_surfaces.size(); ++i) {
        surfaces_set.insert(viz::SurfaceRange(fallback_surfaces[i], primary_surfaces[i]));
    }
    ...
    {
        auto* fake_layer_tree_frame_sink = static_cast<FakeLayerTreeFrameSink*>(host_impl_->layer_tree_frame_sink());
        const viz::CompositorFrameMetadata& metadata = fake_layer_tree_frame_sink->last_sent_frame()->metadata;
        EXPECT_THAT(metadata.activation_dependencies, testing::UnorderedElementsAre(primary_surfaces[0], primary_surfaces[1]));
        EXPECT_THAT(metadata.referenced_surfaces,
            testing::UnorderedElementsAre(viz::SurfaceRange(fallback_surfaces[0], primary_surfaces[0]),
                viz::SurfaceRange(fallback_surfaces[1], primary_surfaces[1]), viz::SurfaceRange(fallback_surfaces[2], primary_surfaces[2])));
...
TEST_P(LayerTreeHostImplTest, SurfaceReferencesChangeCausesDamage)
{
    ...
    {
        const viz::CompositorFrameMetadata& metadata = fake_layer_tree_frame_sink->last_sent_frame()->metadata;
        EXPECT_THAT(metadata.referenced_surfaces, testing::UnorderedElementsAre(viz::SurfaceRange(surface_id)));
    }
}
...
TEST_P(LayerTreeHostImplTest, CompositorFrameMetadata)
{
    ...
    DrawFrame();
    {
        viz::CompositorFrameMetadata metadata = host_impl_->MakeCompositorFrameMetadata();
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel).thread);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel));
    {
        viz::CompositorFrameMetadata metadata = host_impl_->MakeCompositorFrameMetadata();
...
public:
    static std::unique_ptr<DidDrawCheckLayer> Create(LayerTreeImpl* tree_impl, int id)
    {
        return base::WrapUnique(new DidDrawCheckLayer(tree_impl, id));
    }

    bool WillDraw(DrawMode draw_mode, viz::ClientResourceProvider* provider) override
    {
    ...
    bool will_draw_returned_true() const
    {
        return will_draw_returned_true_;
    }
    bool append_quads_called() const
    {
        return append_quads_called_;
    }
    bool did_draw_called() const
    {
        return did_draw_called_;
    }

    void set_will_draw_returns_false()
    {
        will_draw_returns_false_ = true;
    }
...
{
    auto* root = SetupRootLayer<DidDrawCheckLayer>(host_impl_->active_tree(), gfx::Size(10, 10));
    ...
    EXPECT_EQ(host_impl_->active_tree()->background_color(), SkColors::kTransparent);
...
TEST_P(LayerTreeHostImplTest, WillDrawNotCalledOnOccludedLayer)
{
    ...
    auto* root = SetupRootLayer<DidDrawCheckLayer>(host_impl_->active_tree(), big_size);
...
public:
    static std::unique_ptr<MissingTextureAnimatingLayer> Create(
        LayerTreeImpl* tree_impl, int id, bool tile_missing, bool had_incomplete_tile, bool animating, scoped_refptr<AnimationTimeline> timeline)
    {
        return base::WrapUnique(new MissingTextureAnimatingLayer(tree_impl, id, tile_missing, had_incomplete_tile, animating, timeline));
    }
...
private:
        ...
        , had_incomplete_tile_(had_incomplete_tile)
    {
        if (animating) {
            ...
            AddAnimatedTransformToElementWithAnimation(this->element_id(), timeline, 10.0, 3, 0);
        }
    }
...
public:
    void CreateLayerFromState(DidDrawCheckLayer* root, const scoped_refptr<AnimationTimeline>& timeline, const PrepareToDrawSuccessTestCase::State& state)
    {
        auto* layer
            = AddLayer<MissingTextureAnimatingLayer>(root->layer_tree_impl(), state.has_missing_tile, state.has_incomplete_tile, state.is_animating, timeline);
        CopyProperties(root, layer);
...
TEST_P(LayerTreeHostImplPrepareToDrawTest, PrepareToDrawSucceedsAndFails)
{
    ...
    cases.push_back(PrepareToDrawSuccessTestCase(CommitsToActiveTree() ? DrawResult::kSuccess : DrawResult::kAbortedCheckerboardAnimations));
    cases.back().layer_between.has_missing_tile = true;
    cases.back().layer_between.is_animating = true;
    ...
    cases.push_back(PrepareToDrawSuccessTestCase(DrawResult::kAbortedMissingHighResContent));
    cases.back().high_res_required = true;
    cases.back().layer_between.has_incomplete_tile = true;
...
TEST_P(LayerTreeHostImplPrepareToDrawTest, PrepareToDrawWhenDrawAndSwapFullViewportEveryFrame)
{
    CreateHostImpl(DefaultSettings(), FakeLayerTreeFrameSink::CreateSoftware());
    ...
    const bool resourceless_software_draw = true;
    host_impl_->SetExternalTilePriorityConstraints(external_viewport, external_transform);
    ...
    host_impl_->OnDraw(external_transform, external_viewport, resourceless_software_draw, false);
...
TEST_F(CommitToPendingTreeLayerTreeHostImplTest, ClampingAfterActivation)
{
    CreatePendingTree();
    host_impl_->pending_tree()->PushPageScaleFromMainThread(1, 1, 1);
    SetupViewportLayers(host_impl_->pending_tree(), gfx::Size(50, 50), gfx::Size(100, 100), gfx::Size(100, 100));
    host_impl_->ActivateSyncTree();

    CreatePendingTree();
    const gfx::PointF pending_scroll(-100, -100);
    LayerImpl* active_outer_layer = OuterViewportScrollLayer();
    LayerImpl* pending_outer_layer = host_impl_->pending_tree()->OuterViewportScrollLayerForTesting();
    pending_outer_layer->layer_tree_impl()->property_trees()->scroll_tree_mutable().UpdateScrollOffsetBaseForTesting(
...
public:
    ...
    {
        viewport_size_ = gfx::Size(clip_size_.width(), clip_size_.height() + top_controls_height_);
    }

    bool CreateHostImpl(const LayerTreeSettings& settings, std::unique_ptr<LayerTreeFrameSink> layer_tree_frame_sink) override
    {
        bool init = LayerTreeHostImplTest::CreateHostImpl(settings, std::move(layer_tree_frame_sink));
        if (init) {
            host_impl_->active_tree()->SetBrowserControlsParams({ top_controls_height_, 0, 0, 0, false, false });
...
protected:
    void SetupBrowserControlsAndScrollLayerWithVirtualViewport(
        const gfx::Size& inner_viewport_size, const gfx::Size& outer_viewport_size, const gfx::Size& scroll_layer_size)
    {
        SetupBrowserControlsAndScrollLayerWithVirtualViewport(host_impl_->active_tree(), inner_viewport_size, outer_viewport_size, scroll_layer_size);
    }

    void SetupBrowserControlsAndScrollLayerWithVirtualViewport(
        LayerTreeImpl* tree_impl, const gfx::Size& inner_viewport_size, const gfx::Size& outer_viewport_size, const gfx::Size& scroll_layer_size)
    {
        tree_impl->SetBrowserControlsParams({ top_controls_height_, 0, 0, 0, false, true });
...
#define EXPECT_VIEWPORT_GEOMETRIES(expected_browser_controls_shown_ratio)                                                                                      \
    do {                                                                                                                                                       \
        ...
        EXPECT_EQ(expected_browser_controls_shown_ratio, tree->CurrentTopControlsShownRatio());                                                                \
        EXPECT_EQ(tree->top_controls_height() * expected_browser_controls_shown_ratio, host_impl_->browser_controls_manager()->ContentTopOffset());            \
        int delta = (tree->top_controls_height() + tree->bottom_controls_height()) * (1 - expected_browser_controls_shown_ratio);                              \
        ...
        EXPECT_EQ(gfx::RectF(gfx::SizeF(inner_scroll_bounds)), tree->OuterViewportClipNode()->clip);                                                           \
        EXPECT_EQ(gfx::Vector2dF(0, delta), property_trees->inner_viewport_container_bounds_delta());                                                          \
        EXPECT_EQ(gfx::Vector2dF(0, scaled_delta), property_trees->outer_viewport_container_bounds_delta());                                                   \
    } while (false)
...
// right place.
TEST_P(LayerTreeHostImplBrowserControlsTest, HidingBrowserControlsExpandsScrollableSize)
{
    SetupBrowserControlsAndScrollLayerWithVirtualViewport(gfx::Size(50, 50), gfx::Size(50, 50), gfx::Size(50, 50));
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler()
            .ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(0, 25), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
            .thread);
...
TEST_P(LayerTreeHostImplBrowserControlsTest, HidingBrowserControlsExpandsClipAncestorsOfReplacedOuterScroller)
{
    SetupBrowserControlsAndScrollLayerWithVirtualViewport(gfx::Size(180, 180), gfx::Size(180, 180), gfx::Size(180, 180));
    ...
    LayerImpl* scroll_layer = AddScrollableLayer(clip_layer, gfx::Size(150, 150), gfx::Size(300, 300));
    ...
    EXPECT_EQ(GetScrollNode(scroll_layer), active_tree->OuterViewportScrollNode());

    EXPECT_EQ(1, active_tree->CurrentTopControlsShownRatio());
    EXPECT_EQ(50, host_impl_->browser_controls_manager()->ContentTopOffset());
    EXPECT_EQ(gfx::Vector2dF(), property_trees->inner_viewport_container_bounds_delta());
    EXPECT_EQ(gfx::Vector2dF(), property_trees->outer_viewport_container_bounds_delta());
    ...
    EXPECT_EQ(gfx::Vector2dF(0, 25), property_trees->inner_viewport_container_bounds_delta());
    EXPECT_EQ(gfx::Vector2dF(0, 25), property_trees->outer_viewport_container_bounds_delta());
    EXPECT_EQ(gfx::SizeF(300, 300), active_tree->ScrollableSize());
    ...
    EXPECT_EQ(gfx::Vector2dF(0, 50), property_trees->inner_viewport_container_bounds_delta());
    EXPECT_EQ(gfx::Vector2dF(0, 50), property_trees->outer_viewport_container_bounds_delta());
    EXPECT_EQ(gfx::SizeF(300, 300), active_tree->ScrollableSize());
...
TEST_P(LayerTreeHostImplBrowserControlsTest, HidingBrowserControlsAdjustsSnapFling)
{
    gfx::Size view_size(100, 100);
    gfx::Size content_size(100, 1000);
    gfx::RectF snap_area_1(0, 0, 100, 700);
    SetupBrowserControlsAndScrollLayerWithVirtualViewport(view_size, view_size, content_size);
    ...
    handler.ScrollBegin(&*BeginState(position, gfx::Vector2dF(0, 15), type), type);
    ...
    EXPECT_EQ(gfx::Vector2dF(0, 15), property_trees->outer_viewport_container_bounds_delta());
    EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(outer_scroll_layer));

    // Enter "constrained native fling" mode inside snap_area_1.
    EXPECT_FALSE(handler.GetSnapFlingInfoAndSetAnimatingSnapTarget(gfx::Vector2dF(0, 30), gfx::Vector2dF(0, 600), &initial_offset, &target_offset));
    ...
    EXPECT_POINTF_EQ(gfx::PointF(0, 400), CurrentScrollOffset(outer_scroll_layer));

    // Try to scroll past the bottom of snap_area_1.
    EXPECT_TRUE(handler.GetSnapFlingInfoAndSetAnimatingSnapTarget(gfx::Vector2dF(0, 300), gfx::Vector2dF(0, 1000), &initial_offset, &target_offset));
    ...
    EXPECT_TRUE(handler.animating_for_snap_for_testing(host_impl_->OuterViewportScrollNode()->element_id));
...
// region of the page.
TEST_P(LayerTreeHostImplBrowserControlsTest, MovingBrowserControlsOuterClipDeltaScaled)
{
    gfx::Size inner_size = gfx::Size(100, 100);
    gfx::Size outer_size = gfx::Size(100, 100);
    gfx::Size content_size = gfx::Size(200, 1000);
    SetupBrowserControlsAndScrollLayerWithVirtualViewport(inner_size, outer_size, content_size);
    ...
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(0, 0), gfx::Vector2dF(0, 25), ui::ScrollInputType::kTouchscreen));
    EXPECT_VIEWPORT_GEOMETRIES(0.5f);
...
TEST_P(LayerTreeHostImplBrowserControlsTest, HidingBrowserControlsAdjustsScrollbarPosition)
{
    SetupBrowserControlsAndScrollLayerWithVirtualViewport(gfx::Size(50, 50), gfx::Size(50, 50), gfx::Size(50, 50));
    ...
    auto* scrollbar_layer = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::kHorizontal, 3, 20, false);
    ...
    // the actual content bounds shouldn't.
    {
        ...
        EXPECT_EQ(gfx::SizeF(50, 75), active_tree->ScrollableSize());
        EXPECT_EQ(gfx::Size(50, 15), scrollbar_layer->bounds());
        EXPECT_EQ(gfx::Rect(20, 25, 10, 3), scrollbar_layer->ComputeThumbQuadRect());
    }
    ...
    // Fully hide the browser controls.
    {
        ...
        EXPECT_EQ(gfx::SizeF(50, 100), active_tree->ScrollableSize());
        EXPECT_EQ(gfx::Size(50, 15), scrollbar_layer->bounds());
        EXPECT_EQ(gfx::Rect(20, 50, 10, 3), scrollbar_layer->ComputeThumbQuadRect());
    }
...
TEST_P(LayerTreeHostImplBrowserControlsTest, ScrollBrowserControlsByFractionalAmount)
{
    SetupBrowserControlsAndScrollLayerWithVirtualViewport(gfx::Size(10, 10), gfx::Size(10, 10), gfx::Size(10, 10));
    DrawFrame();

    gfx::Vector2dF top_controls_scroll_delta(0, 5.25f);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler()
            .ScrollBegin(BeginState(gfx::Point(), top_controls_scroll_delta, ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
            .thread);
    ...
    EXPECT_FLOAT_EQ(top_controls_scroll_delta.y(), property_trees->inner_viewport_container_bounds_delta().y());
}
...
// viewport.
TEST_P(LayerTreeHostImplBrowserControlsTest, BrowserControlsOuterViewportBecomesScrollable)
{
    SetupBrowserControlsAndScrollLayerWithVirtualViewport(gfx::Size(10, 50), gfx::Size(10, 50), gfx::Size(10, 100));
    DrawFrame();
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler()
            .ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(0, 50), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
            .thread);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, 50), ui::ScrollInputType::kTouchscreen));
    ...
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, 100), ui::ScrollInputType::kTouchscreen));
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler()
            .ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(0, -50), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
            .thread);
    EXPECT_EQ(host_impl_->CurrentlyScrollingNode()->id, outer_scroll->scroll_tree_index());
    ...
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, -20), ui::ScrollInputType::kTouchscreen));
    EXPECT_EQ(25, CurrentScrollOffset(outer_scroll).y());
    EXPECT_EQ(15, CurrentScrollOffset(inner_scroll).y());

    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, -30), ui::ScrollInputType::kTouchscreen));
    EXPECT_EQ(25, CurrentScrollOffset(outer_scroll).y());
    EXPECT_EQ(0, CurrentScrollOffset(inner_scroll).y());

    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0.f, -50), ui::ScrollInputType::kTouchscreen));
    GetInputHandler().ScrollEnd();
...
{
    SetupBrowserControlsAndScrollLayerWithVirtualViewport(gfx::Size(100, 100), gfx::Size(100, 100), gfx::Size(100, 100));
    DrawFrame();
    host_impl_->active_tree()->PushPageScaleFromMainThread(1, 1, 2);
    ...
    host_impl_->browser_controls_manager()->ScrollBegin();
    host_impl_->browser_controls_manager()->ScrollBy(top_controls_scroll_delta);
    EXPECT_FLOAT_EQ(top_controls_height_ - top_controls_scroll_delta.y(), host_impl_->browser_controls_manager()->ContentTopOffset());

    auto* property_trees = host_impl_->active_tree()->property_trees();
    EXPECT_FLOAT_EQ(top_controls_scroll_delta.y(), property_trees->outer_viewport_container_bounds_delta().y());
    GetInputHandler().ScrollEnd();
    ...
    host_impl_->browser_controls_manager()->ScrollBy(top_controls_scroll_delta);
    EXPECT_EQ(0, host_impl_->browser_controls_manager()->ContentTopOffset());
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0, top_controls_height_), property_trees->outer_viewport_container_bounds_delta());
    GetInputHandler().ScrollEnd();
    ...
    host_impl_->browser_controls_manager()->ScrollBegin();
    host_impl_->browser_controls_manager()->ScrollBy(-top_controls_scroll_delta);
    EXPECT_EQ(top_controls_scroll_delta.y(), host_impl_->browser_controls_manager()->ContentTopOffset());
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0, top_controls_height_ - top_controls_scroll_delta.y()), property_trees->outer_viewport_container_bounds_delta());
    host_impl_->browser_controls_manager()->ScrollEnd();
    GetInputHandler().ScrollEnd();
}
...
TEST_P(LayerTreeHostImplBrowserControlsTest, BrowserControlsScrollableSublayer)
{
    gfx::Size sub_content_size(100, 400);
    gfx::Size sub_content_layer_size(100, 300);
    SetupBrowserControlsAndScrollLayerWithVirtualViewport(gfx::Size(100, 50), gfx::Size(100, 100), gfx::Size(100, 100));
    DrawFrame();
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler()
            .ScrollBegin(BeginState(gfx::Point(), scroll_delta, ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
            .thread);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), scroll_delta, ui::ScrollInputType::kTouchscreen));
    GetInputHandler().ScrollEnd();

    // Browser controls should be hidden
    EXPECT_EQ(scroll_delta.y(), top_controls_height_ - host_impl_->browser_controls_manager()->ContentTopOffset());
}
...
TEST_P(LayerTreeHostImplBrowserControlsTest, PositionBrowserControlsToActiveTreeExplicitly)
{
    SetupBrowserControlsAndScrollLayerWithVirtualViewport(layer_size_, layer_size_, layer_size_);
    DrawFrame();

    host_impl_->active_tree()->SetCurrentBrowserControlsShownRatio(0, 0);
    host_impl_->active_tree()->top_controls_shown_ratio()->PushMainToPending(30 / top_controls_height_);
    host_impl_->active_tree()->top_controls_shown_ratio()->PushPendingToActive();
    EXPECT_FLOAT_EQ(30, host_impl_->browser_controls_manager()->ContentTopOffset());
    EXPECT_FLOAT_EQ(-20, host_impl_->browser_controls_manager()->ControlsTopOffset());

    host_impl_->active_tree()->SetCurrentBrowserControlsShownRatio(0, 0);
    EXPECT_FLOAT_EQ(0, host_impl_->browser_controls_manager()->ContentTopOffset());
    EXPECT_FLOAT_EQ(-50, host_impl_->browser_controls_manager()->ControlsTopOffset());
...
TEST_P(LayerTreeHostImplBrowserControlsTest, PositionBrowserControlsToPendingTreeExplicitly)
{
    EnsureSyncTree();
    SetupBrowserControlsAndScrollLayerWithVirtualViewport(host_impl_->sync_tree(), layer_size_, layer_size_, layer_size_);
    ...
    EXPECT_FLOAT_EQ(top_controls_height_, host_impl_->sync_tree()->property_trees()->inner_viewport_container_bounds_delta().y());
    host_impl_->SetCurrentBrowserControlsShownRatio(0.5f, 0.5f);
    EXPECT_FLOAT_EQ(0.5f * top_controls_height_, host_impl_->sync_tree()->property_trees()->inner_viewport_container_bounds_delta().y());
    host_impl_->SetCurrentBrowserControlsShownRatio(1, 1);
    EXPECT_FLOAT_EQ(0, host_impl_->sync_tree()->property_trees()->inner_viewport_container_bounds_delta().y());
...
TEST_P(LayerTreeHostImplBrowserControlsTest, ApplyDeltaOnTreeActivation)
{
    ...
    host_impl_->active_tree()->top_controls_shown_ratio()->PushMainToPending(20 / top_controls_height_);
    host_impl_->active_tree()->top_controls_shown_ratio()->PushPendingToActive();
    host_impl_->active_tree()->SetCurrentBrowserControlsShownRatio(15 / top_controls_height_, 15 / top_controls_height_);
    host_impl_->active_tree()->top_controls_shown_ratio()->PullDeltaForMainThread(
        /* next_bmf */ false);
    host_impl_->active_tree()->SetCurrentBrowserControlsShownRatio(0, 0);
    EnsureSyncTree();
    host_impl_->sync_tree()->PushBrowserControlsFromMainThread(15 / top_controls_height_, 15 / top_controls_height_);
    ...
    EXPECT_EQ(0, host_impl_->browser_controls_manager()->ContentTopOffset());
    EXPECT_EQ(CommitsToActiveTree() ? gfx::Vector2dF(0, 50) : gfx::Vector2dF(), property_trees->inner_viewport_container_bounds_delta());
    EXPECT_FLOAT_EQ(-15, host_impl_->active_tree()->top_controls_shown_ratio()->Delta() * top_controls_height_);
    EXPECT_FLOAT_EQ(15, host_impl_->active_tree()->top_controls_shown_ratio()->ActiveBase() * top_controls_height_);
}
...
TEST_P(LayerTreeHostImplBrowserControlsTest, BrowserControlsLayoutHeightChanged)
{
    SetupBrowserControlsAndScrollLayerWithVirtualViewport(layer_size_, layer_size_, layer_size_);
    DrawFrame();
    ...
    EnsureSyncTree();
    host_impl_->sync_tree()->PushBrowserControlsFromMainThread(1, 1);
    host_impl_->sync_tree()->SetBrowserControlsParams({ top_controls_height_, 0, 0, 0, false, true });
...
TEST_P(LayerTreeHostImplBrowserControlsTest, BrowserControlsViewportOffsetClamping)
{
    SetupBrowserControlsAndScrollLayerWithVirtualViewport(gfx::Size(100, 100), gfx::Size(200, 200), gfx::Size(200, 400));
    DrawFrame();
    ...
    EXPECT_FLOAT_EQ(scroll_delta.y(), top_controls_height_ - host_impl_->browser_controls_manager()->ContentTopOffset());

    // We should still be fully scrolled.
    EXPECT_EQ(host_impl_->active_tree()->TotalMaxScrollOffset(), host_impl_->active_tree()->TotalScrollOffset());
...
TEST_P(LayerTreeHostImplBrowserControlsTest, BrowserControlsAspectRatio)
{
    ...
    EXPECT_FLOAT_EQ(top_controls_height_, host_impl_->browser_controls_manager()->ContentTopOffset());
...
{
    SetupBrowserControlsAndScrollLayerWithVirtualViewport(gfx::Size(100, 100), gfx::Size(100, 100), gfx::Size(100, 200));
    ...
    EXPECT_EQ(top_controls_height_, host_impl_->browser_controls_manager()->ContentTopOffset());
}

// Test that scrolling the outer viewport affects the browser controls.
TEST_P(LayerTreeHostImplBrowserControlsTest, BrowserControlsScrollOuterViewport)
{
    SetupBrowserControlsAndScrollLayerWithVirtualViewport(gfx::Size(100, 100), gfx::Size(200, 200), gfx::Size(200, 400));
    DrawFrame();
    ...
    EXPECT_EQ(OuterViewportScrollLayer()->scroll_tree_index(), host_impl_->CurrentlyScrollingNode()->id);
    GetInputHandler().ScrollEnd();
    ...
    EXPECT_EQ(top_controls_height_, host_impl_->browser_controls_manager()->ContentTopOffset());
    EXPECT_FLOAT_EQ(inner_viewport_offset.y() + (scroll_delta.y() + top_controls_height_), ScrollDelta(InnerViewportScrollLayer()).y());

    GetInputHandler().ScrollEnd();
}

TEST_P(LayerTreeHostImplBrowserControlsTest, ScrollNonScrollableRootWithBrowserControls)
{
    SetupBrowserControlsAndScrollLayerWithVirtualViewport(layer_size_, layer_size_, layer_size_);
    DrawFrame();
    ...
    host_impl_->browser_controls_manager()->ScrollBegin();
    host_impl_->browser_controls_manager()->ScrollBy(gfx::Vector2dF(0, scroll_increment_y));
    EXPECT_FLOAT_EQ(-scroll_increment_y, host_impl_->browser_controls_manager()->ContentTopOffset());
    // Now that browser controls have moved, expect the clip to resize.
    EXPECT_EQ(gfx::Vector2dF(0, 25), property_trees->inner_viewport_container_bounds_delta());

    host_impl_->browser_controls_manager()->ScrollBy(gfx::Vector2dF(0, scroll_increment_y));
    host_impl_->browser_controls_manager()->ScrollEnd();
    EXPECT_FLOAT_EQ(-2 * scroll_increment_y, host_impl_->browser_controls_manager()->ContentTopOffset());
    // Now that browser controls have moved, expect the clip to resize.
    EXPECT_EQ(gfx::Vector2dF(), property_trees->inner_viewport_container_bounds_delta());
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler()
            .ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(0, 10), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
            .thread);
}
...
// crbug.com/597266.
TEST_P(LayerTreeHostImplBrowserControlsTest, ViewportBoundsDeltaOnTreeActivation)
{
    ...
    // Initialization
    {
        ...
        CreatePendingTree();
        SetupBrowserControlsAndScrollLayerWithVirtualViewport(host_impl_->pending_tree(), inner_viewport_size, outer_viewport_size, content_size);
        host_impl_->pending_tree()->SetBrowserControlsParams({ top_controls_height_, 0, 0, 0, false, false });
        UpdateDrawProperties(host_impl_->pending_tree());

        // Fully scroll the viewport.
        GetInputHandler().ScrollBegin(
            BeginState(gfx::Point(75, 75), gfx::Vector2dF(0, 2000), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 2000), ui::ScrollInputType::kTouchscreen));
        GetInputHandler().ScrollEnd();
    }

    LayerImpl* outer_scroll = OuterViewportScrollLayer();

    ASSERT_FLOAT_EQ(0, host_impl_->browser_controls_manager()->ContentTopOffset());
    ...
    host_impl_->browser_controls_manager()->UpdateBrowserControlsState(BrowserControlsState::kBoth, BrowserControlsState::kShown, true, std::nullopt);
    ...
    // controls.
    {
        ...
        ASSERT_GT(delta, 0);
        ASSERT_LT(delta, 1);
        host_impl_->active_tree()->top_controls_shown_ratio()->PullDeltaForMainThread(/* next_bmf */ false);
        host_impl_->active_tree()->top_controls_shown_ratio()->PushMainToPending(delta);
    }
    ...
    // active tree.
    {
        host_impl_->pending_tree()->property_trees()->scroll_tree_mutable().SetScrollOffsetDeltaForTesting(outer_scroll->element_id(), gfx::Vector2dF(0, 1050));
        host_impl_->ActivateSyncTree();
...
TEST_P(LayerTreeHostImplTest, ScrollNonCompositedRoot)
{
    ...
    CreateEffectNode(scroll_container_layer).render_surface_reason = RenderSurfaceReason::kTest;

    LayerImpl* scroll_layer = AddScrollableLayer(scroll_container_layer, surface_size, contents_size);
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler()
            .ScrollBegin(BeginState(gfx::Point(5, 5), gfx::Vector2dF(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel)
            .thread);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel));
    GetInputHandler().ScrollEnd();
    EXPECT_TRUE(did_request_redraw_);
...
TEST_P(LayerTreeHostImplTest, ScrollChildCallsCommitAndRedraw)
{
    ...
    CreateEffectNode(content_root).render_surface_reason = RenderSurfaceReason::kTest;
...
TEST_P(LayerTreeHostImplTest, ScrollMissesChild)
{
    ...
    // scrolling.
    InputHandler::ScrollStatus status
        = GetInputHandler().ScrollBegin(BeginState(gfx::Point(15, 5), gfx::Vector2dF(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
    EXPECT_EQ(host_impl_->CurrentlyScrollingNode(), host_impl_->OuterViewportScrollNode());
...
TEST_P(LayerTreeHostImplTest, ScrollMissesBackfacingChild)
{
    ...
    SetupViewportLayersOuterScrolls(viewport_size, surface_size);
    LayerImpl* child = AddScrollableLayer(OuterViewportScrollLayer(), viewport_size, surface_size);
    ...
    InputHandler::ScrollStatus status
        = GetInputHandler().ScrollBegin(BeginState(gfx::Point(5, 5), gfx::Vector2dF(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
    EXPECT_EQ(host_impl_->CurrentlyScrollingNode(), host_impl_->OuterViewportScrollNode());
...
TEST_P(LayerTreeHostImplTest, ScrollLayerWithMainThreadReason)
{
    ...
    LayerImpl* scroll_layer = AddScrollableLayer(root, scroll_container_size, surface_size);
    LayerImpl* content_layer = AddScrollableLayer(scroll_layer, scroll_container_size, surface_size);
    GetScrollNode(content_layer)->main_thread_repaint_reasons = MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects;
    DrawFrame();
...
TEST_P(LayerTreeHostImplTest, ScrollRootAndChangePageScaleOnMainThread)
{
    ...
    std::unique_ptr<CompositorCommitData> commit_data = host_impl_->ProcessCompositorDeltas(
        /* main_thread_mutator_host */ nullptr);
    LayerImpl* inner_scroll = InnerViewportScrollLayer();
    EXPECT_TRUE(ScrollInfoContains(*commit_data.get(), inner_scroll->element_id(), expected_scroll_delta));
...
{
    SetupViewportLayers(host_impl_->active_tree(), gfx::Size(15, 15), gfx::Size(30, 30), gfx::Size(50, 50));
    LayerImpl* outer_scroll = OuterViewportScrollLayer();
    LayerImpl* inner_scroll = InnerViewportScrollLayer();
    DrawFrame();
    ...
    float page_scale = 2;
    host_impl_->active_tree()->PushPageScaleFromMainThread(page_scale, 1, page_scale);
    DrawOneFrame();
...
TEST_P(LayerTreeHostImplTest, ScrollChildBeyondLimit)
{
    ...
    LayerImpl* grand_child_layer = AddScrollableLayer(child_layer, surface_size, content_size);

    UpdateDrawProperties(host_impl_->active_tree());
    host_impl_->active_tree()->DidBecomeActive();

    grand_child_layer->layer_tree_impl()->property_trees()->scroll_tree_mutable().UpdateScrollOffsetBaseForTesting(
        grand_child_layer->element_id(), gfx::PointF(0, 5));
    child_layer->layer_tree_impl()->property_trees()->scroll_tree_mutable().UpdateScrollOffsetBaseForTesting(child_layer->element_id(), gfx::PointF(3, 0));
    ...
    DrawFrame();
    {
        ...
        EXPECT_TRUE(ScrollInfoContains(*commit_data.get(), grand_child_layer->element_id(), gfx::Vector2dF(0, -5)));
...
TEST_P(LayerTreeHostImplTimelinesTest, ScrollAnimatedLatchToChild)
{
    ...
    child_layer->layer_tree_impl()->property_trees()->scroll_tree_mutable().UpdateScrollOffsetBaseForTesting(child_layer->element_id(), gfx::PointF(0, 50));
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2d(0, -100), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel).thread);
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(0, -100)));
...
TEST_F(CommitToActiveTreeLayerTreeHostImplTest, ScrollWithoutBubbling)
{
    ...
    SetupViewportLayersNoScrolls(surface_size);
    LayerImpl* child_layer = AddScrollableLayer(InnerViewportScrollLayer(), viewport_size, surface_size);
    LayerImpl* grand_child_layer = AddScrollableLayer(child_layer, viewport_size, surface_size);
    UpdateDrawProperties(host_impl_->active_tree());
    host_impl_->active_tree()->DidBecomeActive();
    ...
    child_layer->layer_tree_impl()->property_trees()->scroll_tree_mutable().UpdateScrollOffsetBaseForTesting(child_layer->element_id(), child_base);
    ...
    DrawFrame();
    {
        ...
        EXPECT_TRUE(ScrollInfoContains(*commit_data.get(), grand_child_layer->element_id(), grand_child_delta));
        ...
        PushScrollOffsetsToPendingTree({ { child_layer->element_id(), child_base }, { grand_child_layer->element_id(), grand_child_base } });

        // The next time we scroll we should only scroll the parent.
        scroll_delta = gfx::Vector2d(0, -3);
        EXPECT_EQ(ScrollThread::kScrollOnImplThread,
            GetInputHandler()
                .ScrollBegin(BeginState(gfx::Point(5, 5), scroll_delta, ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
                .thread);
        EXPECT_EQ(host_impl_->CurrentlyScrollingNode()->id, child_layer->scroll_tree_index());
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), scroll_delta, ui::ScrollInputType::kTouchscreen));
        EXPECT_EQ(host_impl_->CurrentlyScrollingNode()->id, child_layer->scroll_tree_index());
        GetInputHandler().ScrollEnd();
        ...
        EXPECT_TRUE(ScrollInfoContains(*commit_data.get(), child_layer->element_id(), child_delta));
        ...
        child_base += child_delta;
        grand_child_base += grand_child_delta;
        PushScrollOffsetsToPendingTree({ { grand_child_layer->element_id(), grand_child_base }, { child_layer->element_id(), child_base } });
        ...
        EXPECT_EQ(ScrollThread::kScrollOnImplThread,
            GetInputHandler()
                .ScrollBegin(BeginState(gfx::Point(5, 5), scroll_delta, ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
                .thread);
        EXPECT_EQ(host_impl_->CurrentlyScrollingNode()->id, grand_child_layer->scroll_tree_index());
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), scroll_delta, ui::ScrollInputType::kTouchscreen));
        EXPECT_EQ(host_impl_->CurrentlyScrollingNode()->id, grand_child_layer->scroll_tree_index());
        GetInputHandler().ScrollEnd();
        ...
        EXPECT_EQ(ScrollThread::kScrollOnImplThread,
            GetInputHandler()
                .ScrollBegin(BeginState(gfx::Point(1, 1), scroll_delta, ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
                .thread);
        EXPECT_EQ(grand_child_layer->scroll_tree_index(), host_impl_->CurrentlyScrollingNode()->id);
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), scroll_delta, ui::ScrollInputType::kTouchscreen));
        GetInputHandler().ScrollEnd();
        ...
        EXPECT_TRUE(ScrollInfoContains(*commit_data.get(), grand_child_layer->element_id(), gfx::Vector2dF(0, -1)));
    }
}
...
TEST_P(LayerTreeHostImplTest, ChildrenOfInnerScrollNodeCanScrollOnThread)
{
    ...
    DrawFrame();
    {
        ...
        EXPECT_TRUE(ScrollInfoContains(*commit_data.get(), host_impl_->OuterViewportScrollNode()->element_id, scroll_delta));
    }
}
...
TEST_F(CommitToActiveTreeLayerTreeHostImplTest, ScrollAxisAlignedRotatedLayer)
{
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler()
            .ScrollBegin(BeginState(gfx::Point(), gesture_scroll_delta, ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
            .thread);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gesture_scroll_delta, ui::ScrollInputType::kTouchscreen));
    GetInputHandler().ScrollEnd();

    // The layer should have scrolled down in its local coordinates.
    std::unique_ptr<CompositorCommitData> commit_data = host_impl_->ProcessCompositorDeltas(
        /* main_thread_mutator_host */ nullptr);
    EXPECT_TRUE(ScrollInfoContains(*commit_data.get(), scroll_layer->element_id(), gfx::Vector2dF(0, gesture_scroll_delta.x())));

    // Push scrolls to pending tree
    PushScrollOffsetsToPendingTree({ { scroll_layer->element_id(), gfx::PointF(10, 0) } });
    ClearNonScrollSyncTreeDeltasForTesting();
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler().ScrollBegin(BeginState(gfx::Point(), wheel_scroll_delta, ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel).thread);
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), wheel_scroll_delta, ui::ScrollInputType::kWheel));
    GetInputHandler().ScrollEnd();

    // The layer should have scrolled down in its local coordinates.
    commit_data = host_impl_->ProcessCompositorDeltas(
        /* main_thread_mutator_host */ nullptr);
    EXPECT_TRUE(ScrollInfoContains(*commit_data.get(), scroll_layer->element_id(), wheel_scroll_delta));
}

TEST_F(CommitToActiveTreeLayerTreeHostImplTest, ScrollNonAxisAlignedRotatedLayer)
{
    SetupViewportLayersInnerScrolls(gfx::Size(50, 50), gfx::Size(100, 100));
    auto* scroll_layer = InnerViewportScrollLayer();
    ...
    gfx::Size scroll_container_bounds(content_size.width(), content_size.height() / 2);
    LayerImpl* clip_layer = AddLayerInActiveTree();
    ...
    clip_layer_transform_node.origin = gfx::Point3F(clip_layer->bounds().width() * 0.5f, clip_layer->bounds().height(), 0);
    clip_layer_transform_node.local = rotate_transform;

    LayerImpl* child = AddScrollableLayer(clip_layer, scroll_container_bounds, content_size);
    ...
    DrawFrame();
    {
        ...
        EXPECT_EQ(ScrollThread::kScrollOnImplThread,
            GetInputHandler()
                .ScrollBegin(BeginState(gfx::Point(1, 1), gesture_scroll_delta, ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
                .thread);
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gesture_scroll_delta, ui::ScrollInputType::kTouchscreen));
        GetInputHandler().ScrollEnd();
        ...
        gfx::Vector2dF expected_scroll_delta(0, std::floor(gesture_scroll_delta.y() * std::cos(base::DegToRad(child_layer_angle))));
        std::unique_ptr<CompositorCommitData> commit_data = host_impl_->ProcessCompositorDeltas(
            /* main_thread_mutator_host */ nullptr);
        EXPECT_TRUE(ScrollInfoContains(*commit_data.get(), child_scroll_id, expected_scroll_delta));
        ...
        PushScrollOffsetsToPendingTree({ { child_scroll_id, gfx::PointAtOffsetFromOrigin(expected_scroll_delta) } });
        ClearNonScrollSyncTreeDeltasForTesting();
    }
    {
        ...
        // The child layer should have scrolled down in its local coordinates an
        // amount proportional to the angle between it and the input scroll delta.
        gfx::Vector2dF expected_scroll_delta(0, std::floor(-gesture_scroll_delta.x() * std::sin(base::DegToRad(child_layer_angle))));
        std::unique_ptr<CompositorCommitData> commit_data = host_impl_->ProcessCompositorDeltas(
            /* main_thread_mutator_host */ nullptr);
        EXPECT_TRUE(ScrollInfoContains(*commit_data.get(), child_scroll_id, expected_scroll_delta));
...
TEST_F(CommitToActiveTreeLayerTreeHostImplTest, ScrollPerspectiveTransformedLayer)
{
    ...
    // is a different size than the clip, so make sure the clip layer's origin
    // lines up over the child.
    clip_layer_transform_node.origin = gfx::Point3F(clip_layer->bounds().width(), clip_layer->bounds().height(), 0);
    clip_layer_transform_node.local = perspective_transform;

    LayerImpl* child = AddScrollableLayer(clip_layer, clip_layer->bounds(), scroll_layer->bounds());
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        EXPECT_EQ(ScrollThread::kScrollOnImplThread,
            GetInputHandler()
                .ScrollBegin(BeginState(viewport_point, gesture_scroll_deltas[i], ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
                .thread);
        GetInputHandler().ScrollUpdate(UpdateState(viewport_point, gesture_scroll_deltas[i], ui::ScrollInputType::kTouchscreen));
        viewport_point += gfx::ToFlooredVector2d(gesture_scroll_deltas[i]);
        GetInputHandler().ScrollEnd();

        commit_data = host_impl_->ProcessCompositorDeltas(
            /* main_thread_mutator_host */ nullptr);
        EXPECT_TRUE(ScrollInfoContains(*commit_data.get(), child->element_id(), expected_scroll_deltas[i]));

        // The root scroll layer should not have scrolled, because the input delta
        // was close to the layer's axis of movement.
        EXPECT_EQ(commit_data->scrolls.size(), 1u);

        PushScrollOffsetsToPendingTree({ { child->element_id(), gfx::PointAtOffsetFromOrigin(expected_scroll_deltas[i]) } });
        ClearMainThreadDeltasForTesting(host_impl_.get());
    }
}
...
TEST_F(CommitToActiveTreeLayerTreeHostImplTest, ScrollScaledLayer)
{
    ...
    std::unique_ptr<CompositorCommitData> commit_data = host_impl_->ProcessCompositorDeltas(
        /* main_thread_mutator_host */ nullptr);
    EXPECT_TRUE(ScrollInfoContains(*commit_data.get(), scroll_layer->element_id(), gfx::Vector2dF(0, scroll_delta.y() / scale)));
    PushScrollOffsetsToPendingTree({ { scroll_layer->element_id(), gfx::PointAtOffsetFromOrigin(gfx::Vector2dF(0, scroll_delta.y() / scale)) } });
    ClearNonScrollSyncTreeDeltasForTesting();
...
TEST_F(CommitToPendingTreeLayerTreeHostImplTest, RootLayerScrollOffsetDelegation)
{
    ...
    scroll_layer->layer_tree_impl()->property_trees()->scroll_tree_mutable().UpdateScrollOffsetBaseForTesting(scroll_layer->element_id(), gfx::PointF());
    ...
    EXPECT_EQ(initial_scroll_delta, scroll_watcher.last_set_scroll_offset().OffsetFromOrigin());
    ...
    host_impl_->LayerTreeHostImpl::StartPageScaleAnimation(gfx::Point(0, 0), false, 1, base::TimeDelta());
    host_impl_->Animate();
    EXPECT_EQ(1, scroll_watcher.page_scale_factor());
    EXPECT_EQ(.5f, scroll_watcher.min_page_scale_factor());
    EXPECT_EQ(4, scroll_watcher.max_page_scale_factor());
    ...
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), scroll_delta, ui::ScrollInputType::kTouchscreen));
    EXPECT_EQ(current_offset + scroll_delta, scroll_watcher.last_set_scroll_offset());
    ...
    CreatePendingTree();
    host_impl_->pending_tree()->PushPageScaleFromMainThread(1, 1, 1);
    SetupViewportLayers(host_impl_->pending_tree(), new_viewport_size, new_content_size, new_content_size);
    host_impl_->ActivateSyncTree();
...
TEST_P(LayerTreeHostImplTest, ExternalRootLayerScrollOffsetDelegationReflectedInNextDraw)
{
    SetupViewportLayersInnerScrolls(gfx::Size(10, 20), gfx::Size(100, 100));
    auto* scroll_layer = InnerViewportScrollLayer();
    scroll_layer->SetDrawsContent(true);
    ...
    TestFrameData frame;
    auto args = viz::CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, viz::BeginFrameArgs::kManualSourceId, 1, base::TimeTicks() + base::Milliseconds(1));
    host_impl_->WillBeginImplFrame(args);
    EXPECT_EQ(DrawResult::kSuccess, host_impl_->PrepareToDraw(&frame));
    host_impl_->DrawLayers(&frame);
    host_impl_->DidDrawAllLayers(frame);
...
TEST_P(LayerTreeHostImplTest, ViewportUserScrollable)
{
    ...
    ScrollTree& scroll_tree = host_impl_->active_tree()->property_trees()->scroll_tree_mutable();
    ...
    float page_scale_factor = 2;
    host_impl_->active_tree()->PushPageScaleFromMainThread(page_scale_factor, page_scale_factor, page_scale_factor);
    ...
    {
        auto begin_state = BeginState(gfx::Point(), scroll_delta, ui::ScrollInputType::kTouchscreen);
        EXPECT_EQ(ScrollThread::kScrollOnImplThread, GetInputHandler().ScrollBegin(begin_state.get(), ui::ScrollInputType::kTouchscreen).thread);

        // Try scrolling right, the inner viewport should be allowed to scroll.
        auto update_state = UpdateState(gfx::Point(), scroll_delta, ui::ScrollInputType::kTouchscreen);
        GetInputHandler().ScrollUpdate(update_state);

        EXPECT_POINTF_EQ(gfx::PointF(30, 0), scroll_tree.current_scroll_offset(inner_element_id));
        EXPECT_POINTF_EQ(gfx::PointF(0, 0), scroll_tree.current_scroll_offset(outer_element_id));
        ...
        update_state = UpdateState(gfx::Point(), scroll_delta, ui::ScrollInputType::kTouchscreen);
        GetInputHandler().ScrollUpdate(update_state);
        update_state = UpdateState(gfx::Point(), scroll_delta, ui::ScrollInputType::kTouchscreen);
        GetInputHandler().ScrollUpdate(update_state);
        update_state = UpdateState(gfx::Point(), scroll_delta, ui::ScrollInputType::kTouchscreen);
        ...
        EXPECT_POINTF_EQ(gfx::PointF(50, 0), scroll_tree.current_scroll_offset(inner_element_id));
        EXPECT_POINTF_EQ(gfx::PointF(0, 0), scroll_tree.current_scroll_offset(outer_element_id));
    ...
    {
        auto begin_state = BeginState(gfx::Point(), scroll_delta, ui::ScrollInputType::kWheel);
        EXPECT_EQ(ScrollThread::kScrollOnImplThread, GetInputHandler().ScrollBegin(begin_state.get(), ui::ScrollInputType::kWheel).thread);
...
#define ANIMATE(time_ms)                                                                                                                                       \
    ...
    host_impl_->DidFinishImplFrame(begin_frame_args);
        ...
        EXPECT_POINTF_EQ(gfx::PointF(50, 50), scroll_tree.current_scroll_offset(inner_element_id));
        ...
        EXPECT_POINTF_EQ(gfx::PointF(0, 100), scroll_tree.current_scroll_offset(outer_element_id));
        EXPECT_POINTF_EQ(gfx::PointF(50, 50), scroll_tree.current_scroll_offset(inner_element_id));
...
TEST_P(LayerTreeHostImplTest, SetRootScrollOffsetUserScrollable)
{
    ...
    // Disable scrolling the inner viewport. Only the outer should scroll.
    {
        ...
        GetInputHandler().SetSynchronousInputHandlerRootScrollOffset(scroll_offset);
        EXPECT_POINTF_EQ(gfx::PointF(0, 0), scroll_tree.current_scroll_offset(inner_element_id));
        EXPECT_POINTF_EQ(scroll_offset, scroll_tree.current_scroll_offset(outer_element_id));
        EXPECT_TRUE(did_request_redraw_);
...
TEST_P(LayerTreeHostImplTest, OverscrollRoot)
{
    ...
    host_impl_->active_tree()->PushPageScaleFromMainThread(1, 0.5f, 4);
    DrawFrame();
    EXPECT_EQ(gfx::Vector2dF(), GetInputHandler().accumulated_root_overscroll_for_testing());

    // In-bounds scrolling does not affect overscroll.
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel).thread);
    scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel));
    ...
    scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 50), ui::ScrollInputType::kWheel));
    EXPECT_TRUE(scroll_result.did_scroll);
    EXPECT_TRUE(scroll_result.did_overscroll_root);
    EXPECT_EQ(gfx::Vector2dF(0, 10), scroll_result.unused_scroll_delta);
    EXPECT_EQ(gfx::Vector2dF(0, 10), GetInputHandler().accumulated_root_overscroll_for_testing());
    EXPECT_EQ(scroll_result.accumulated_root_overscroll, GetInputHandler().accumulated_root_overscroll_for_testing());
    ...
    EXPECT_FALSE(scroll_result.did_overscroll_root);
    EXPECT_EQ(gfx::Vector2dF(), scroll_result.unused_scroll_delta);
    EXPECT_EQ(gfx::Vector2dF(0, 0), GetInputHandler().accumulated_root_overscroll_for_testing());
    EXPECT_EQ(scroll_result.accumulated_root_overscroll, GetInputHandler().accumulated_root_overscroll_for_testing());
    ...
    scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(10, 0), ui::ScrollInputType::kWheel));
    EXPECT_TRUE(scroll_result.did_scroll);
    EXPECT_FALSE(scroll_result.did_overscroll_root);
    ...
    scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(-15, 0), ui::ScrollInputType::kWheel));
    EXPECT_TRUE(scroll_result.did_scroll);
    EXPECT_TRUE(scroll_result.did_overscroll_root);
    EXPECT_EQ(gfx::Vector2dF(-5, 0), scroll_result.unused_scroll_delta);
    EXPECT_EQ(gfx::Vector2dF(-5, -10), GetInputHandler().accumulated_root_overscroll_for_testing());
    EXPECT_EQ(scroll_result.accumulated_root_overscroll, GetInputHandler().accumulated_root_overscroll_for_testing());

    scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, 60), ui::ScrollInputType::kWheel));
    EXPECT_TRUE(scroll_result.did_scroll);
    EXPECT_TRUE(scroll_result.did_overscroll_root);
    EXPECT_EQ(gfx::Vector2dF(0, 10), scroll_result.unused_scroll_delta);
    EXPECT_EQ(gfx::Vector2dF(-5, 10), GetInputHandler().accumulated_root_overscroll_for_testing());
    ...
    scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(10, -60), ui::ScrollInputType::kWheel));
    EXPECT_TRUE(scroll_result.did_scroll);
    EXPECT_TRUE(scroll_result.did_overscroll_root);
    EXPECT_EQ(gfx::Vector2dF(0, -10), scroll_result.unused_scroll_delta);
    EXPECT_EQ(gfx::Vector2dF(0, -10), GetInputHandler().accumulated_root_overscroll_for_testing());
    ...
    // as no scroll occurs.
    scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, -20), ui::ScrollInputType::kWheel));
    EXPECT_FALSE(scroll_result.did_scroll);
    EXPECT_TRUE(scroll_result.did_overscroll_root);
    EXPECT_EQ(gfx::Vector2dF(0, -20), scroll_result.unused_scroll_delta);
    EXPECT_EQ(gfx::Vector2dF(0, -30), GetInputHandler().accumulated_root_overscroll_for_testing());
    EXPECT_EQ(scroll_result.accumulated_root_overscroll, GetInputHandler().accumulated_root_overscroll_for_testing());
    ...
    EXPECT_TRUE(scroll_result.did_overscroll_root);
    EXPECT_EQ(gfx::Vector2dF(0, -20), scroll_result.unused_scroll_delta);
    EXPECT_EQ(gfx::Vector2dF(0, -50), GetInputHandler().accumulated_root_overscroll_for_testing());
    EXPECT_EQ(scroll_result.accumulated_root_overscroll, GetInputHandler().accumulated_root_overscroll_for_testing());
...
TEST_P(LayerTreeHostImplTest, OverscrollChildWithoutBubbling)
{
    ...
    gfx::Size surface_size(10, 10);
    SetupViewportLayersNoScrolls(surface_size);
    LayerImpl* root = AddScrollableLayer(OuterViewportScrollLayer(), scroll_container_size, surface_size);
    LayerImpl* child_layer = AddScrollableLayer(root, scroll_container_size, surface_size);
    LayerImpl* grand_child_layer = AddScrollableLayer(child_layer, scroll_container_size, surface_size);

    UpdateDrawProperties(host_impl_->active_tree());
    host_impl_->active_tree()->DidBecomeActive();

    child_layer->layer_tree_impl()->property_trees()->scroll_tree_mutable().UpdateScrollOffsetBaseForTesting(child_layer->element_id(), gfx::PointF(0, 3));
    grand_child_layer->layer_tree_impl()->property_trees()->scroll_tree_mutable().UpdateScrollOffsetBaseForTesting(
    ...
    DrawFrame();
    {
        ...
        EXPECT_EQ(ScrollThread::kScrollOnImplThread,
            GetInputHandler()
                .ScrollBegin(BeginState(gfx::Point(), scroll_delta, ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
                .thread);
        scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), scroll_delta, ui::ScrollInputType::kTouchscreen));
        EXPECT_EQ(host_impl_->CurrentlyScrollingNode()->id, grand_child_layer->scroll_tree_index());
        EXPECT_TRUE(scroll_result.did_scroll);
        EXPECT_FALSE(scroll_result.did_overscroll_root);
        EXPECT_EQ(gfx::Vector2dF(), GetInputHandler().accumulated_root_overscroll_for_testing());
...
TEST_P(LayerTreeHostImplTest, OverscrollChildEventBubbling)
{
    ...
    DrawFrame();
    {
        ...
        EXPECT_EQ(ScrollThread::kScrollOnImplThread,
            GetInputHandler().ScrollBegin(BeginState(gfx::Point(5, 5), scroll_delta, ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel).thread);
        scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), scroll_delta, ui::ScrollInputType::kWheel));
        EXPECT_TRUE(scroll_result.did_scroll);
        EXPECT_FALSE(scroll_result.did_overscroll_root);
        EXPECT_EQ(gfx::Vector2dF(), GetInputHandler().accumulated_root_overscroll_for_testing());
        scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), scroll_delta, ui::ScrollInputType::kWheel));
        EXPECT_TRUE(scroll_result.did_scroll);
        EXPECT_TRUE(scroll_result.did_overscroll_root);
        EXPECT_EQ(gfx::Vector2dF(0, 6), GetInputHandler().accumulated_root_overscroll_for_testing());
        scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), scroll_delta, ui::ScrollInputType::kWheel));
        EXPECT_FALSE(scroll_result.did_scroll);
        EXPECT_TRUE(scroll_result.did_overscroll_root);
        EXPECT_EQ(gfx::Vector2dF(0, 14), GetInputHandler().accumulated_root_overscroll_for_testing());
        GetInputHandler().ScrollEnd();
    }
}
...
TEST_P(LayerTreeHostImplTest, NoOverscrollWhenNotAtEdge)
{
    ...
    DrawFrame();
    {
        ...
        EXPECT_EQ(ScrollThread::kScrollOnImplThread,
            GetInputHandler()
                .ScrollBegin(BeginState(gfx::Point(0, 0), gfx::Vector2dF(0, 100), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel)
                .thread);
        scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, 100), ui::ScrollInputType::kWheel));
        EXPECT_TRUE(scroll_result.did_scroll);
        EXPECT_FALSE(scroll_result.did_overscroll_root);
        EXPECT_EQ(gfx::Vector2dF(), GetInputHandler().accumulated_root_overscroll_for_testing());
        scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, -2.30f), ui::ScrollInputType::kWheel));
        EXPECT_TRUE(scroll_result.did_scroll);
        EXPECT_FALSE(scroll_result.did_overscroll_root);
        EXPECT_EQ(gfx::Vector2dF(), GetInputHandler().accumulated_root_overscroll_for_testing());
        GetInputHandler().ScrollEnd();
        ...
        EXPECT_EQ(ScrollThread::kScrollOnImplThread,
            GetInputHandler()
                .ScrollBegin(BeginState(gfx::Point(0, 0), gfx::Vector2dF(0, 20), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
                .thread);
        scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, 20), ui::ScrollInputType::kTouchscreen));
        EXPECT_TRUE(scroll_result.did_scroll);
        EXPECT_TRUE(scroll_result.did_overscroll_root);
        EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.000000f, 17.699997f), GetInputHandler().accumulated_root_overscroll_for_testing());

        scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0.02f, -0.01f), ui::ScrollInputType::kTouchscreen));
        EXPECT_FALSE(scroll_result.did_scroll);
        EXPECT_FALSE(scroll_result.did_overscroll_root);
        EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.000000f, 17.699997f), GetInputHandler().accumulated_root_overscroll_for_testing());
        GetInputHandler().ScrollEnd();
        ...
        EXPECT_EQ(ScrollThread::kScrollOnImplThread,
            GetInputHandler()
                .ScrollBegin(BeginState(gfx::Point(0, 0), gfx::Vector2dF(-0.12f, 0.1f), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel)
                .thread);
        scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(-0.12f, 0.1f), ui::ScrollInputType::kWheel));
        EXPECT_FALSE(scroll_result.did_scroll);
        EXPECT_FALSE(scroll_result.did_overscroll_root);
        EXPECT_EQ(gfx::Vector2dF(), GetInputHandler().accumulated_root_overscroll_for_testing());
        GetInputHandler().ScrollEnd();
...
TEST_P(LayerTreeHostImplTest, NoOverscrollOnNonViewportLayers)
{
    ...
    LayerImpl* scroll_layer = AddScrollableLayer(content_layer, content_size, gfx::Size(400, 400));
    ...
    // doesn't consume the delta but it isn't counted as overscroll.
    {
        InputHandlerScrollResult result
            = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(0, 0), gfx::Vector2dF(120, 140), ui::ScrollInputType::kTouchscreen));
    ...
    {
        InputHandlerScrollResult result
            = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(0, 0), gfx::Vector2dF(20, 40), ui::ScrollInputType::kTouchscreen));
...
TEST_P(LayerTreeHostImplTest, ScrollFromOuterViewportSibling)
{
    ...
    host_impl_->active_tree()->SetBrowserControlsParams({ 10, 0, 0, 0, false, false });
    host_impl_->active_tree()->SetCurrentBrowserControlsShownRatio(1.f, 1.f);
    ...
    // browser controls.
    LayerImpl* scroll_layer = AddScrollableLayer(inner_scroll_layer, viewport_size, gfx::Size(400, 400));
    ...
    {
        gfx::Vector2d scroll_delta(0, 10);
        GetInputHandler().ScrollBegin(BeginState(gfx::Point(0, 0), scroll_delta, ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), scroll_delta, ui::ScrollInputType::kTouchscreen));
        EXPECT_EQ(0, host_impl_->active_tree()->CurrentTopControlsShownRatio());
        EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(inner_scroll_layer));
        ...
        EXPECT_POINTF_EQ(gfx::PointF(0, 10), CurrentScrollOffset(inner_scroll_layer));
        EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(outer_scroll_layer));
        GetInputHandler().ScrollEnd();
    }
}
...
TEST_P(LayerTreeHostImplTest, ScrollChainingWithReplacedOuterViewport)
{
    ...
    LayerImpl* scroll_layer = AddScrollableLayer(content_layer, content_size, gfx::Size(400, 400));
    GetScrollNode(scroll_layer)->scrolls_outer_viewport = true;
    LayerImpl* child_scroll_layer = AddScrollableLayer(scroll_layer, gfx::Size(300, 300), gfx::Size(500, 500));
    ...
    {
        GetInputHandler().ScrollBegin(
            BeginState(gfx::Point(0, 0), gfx::Vector2dF(200, 200), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(0, 0), gfx::Vector2dF(200, 200), ui::ScrollInputType::kTouchscreen));
        GetInputHandler().ScrollEnd();

        EXPECT_POINTF_EQ(gfx::PointF(200, 200), CurrentScrollOffset(child_scroll_layer));
...
TEST_P(LayerTreeHostImplTest, RootScrollerScrollNonDescendant)
{
    ...
    LayerImpl* outer_scroll_layer = AddScrollableLayer(content_layer, content_size, gfx::Size(1200, 1200));
    LayerImpl* sibling_scroll_layer = AddScrollableLayer(content_layer, gfx::Size(600, 600), gfx::Size(1200, 1200));
    ...
    ASSERT_EQ(outer_scroll_layer, layer_tree_impl->OuterViewportScrollLayerForTesting());
    ...
    // propagate to the outer viewport scroll layer.
    {
        ...
        EXPECT_POINTF_EQ(gfx::PointF(600, 600), CurrentScrollOffset(sibling_scroll_layer));
        EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(outer_scroll_layer));
    ...
    gfx::PointF viewport_bottom_right(viewport_size.width(), viewport_size.height());
    ...
    // viewport layer.
    {
        ...
        EXPECT_POINTF_EQ(gfx::PointF(anchor.x() / 2, anchor.y() / 2), CurrentScrollOffset(inner_scroll_layer));

        GetInputHandler().ScrollUpdate(UpdateState(anchor, viewport_bottom_right.OffsetFromOrigin(), ui::ScrollInputType::kTouchscreen));

        EXPECT_POINTF_EQ(gfx::ScalePoint(viewport_bottom_right, 1 / page_scale_factor), CurrentScrollOffset(inner_scroll_layer));
        // TODO(bokan): This doesn't yet work but we'll probably want to fix this
        // at some point.
        // EXPECT_VECTOR2DF_EQ(
        //     gfx::Vector2dF(),
        //     CurrentScrollOffset(outer_scroll_layer));
        EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(sibling_scroll_layer));
    ...
    // around. The outer viewport should be unaffected.
    {
        ...
        GetInputHandler().ScrollBegin(
            BeginState(gfx::Point(0, 0), gfx::Vector2dF(2000, 2000), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(0, 0), gfx::Vector2dF(2000, 2000), ui::ScrollInputType::kTouchscreen));
        GetInputHandler().ScrollEnd();
        ...
        EXPECT_POINTF_EQ(ScalePoint(viewport_bottom_right, 1 / page_scale_factor), CurrentScrollOffset(inner_scroll_layer));
        EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(outer_scroll_layer));
...
TEST_P(LayerTreeHostImplTest, OverscrollOnImplThread)
{
    ...
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, scroll_node->main_thread_repaint_reasons);
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler()
            .ScrollBegin(BeginState(gfx::Point(0, 60), gfx::Vector2dF(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel)
            .thread);
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler()
            .ScrollBegin(BeginState(gfx::Point(0, 0), gfx::Vector2dF(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel)
            .thread);
}

class BlendStateCheckLayer : public LayerImpl {
public:
    static std::unique_ptr<BlendStateCheckLayer> Create(LayerTreeImpl* tree_impl, int id, viz::ClientResourceProvider* resource_provider)
    {
        return base::WrapUnique(new BlendStateCheckLayer(tree_impl, id, resource_provider));
    }

    BlendStateCheckLayer(LayerTreeImpl* tree_impl, int id, viz::ClientResourceProvider* resource_provider)
        : LayerImpl(tree_impl, id)
    ...
    {
        resource_id_ = resource_provider_->ImportResource(viz::TransferableResource::MakeSoftwareSharedBitmap(viz::SharedBitmap::GenerateId(), gpu::SyncToken(),
    ...
    void AppendQuads(viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data) override
    {
        ...
        auto* test_blending_draw_quad = render_pass->CreateAndAppendDrawQuad<viz::TileDrawQuad>();
        test_blending_draw_quad->SetNew(
            shared_quad_state, quad_rect_, visible_quad_rect, needs_blending, resource_id_, gfx::RectF(0, 0, 1, 1), gfx::Size(1, 1), false, false, false);

        EXPECT_EQ(blend_, test_blending_draw_quad->ShouldDrawWithBlending());
        EXPECT_EQ(has_render_surface_, GetRenderSurface(this) != GetRenderSurface(comparison_layer_));
    }

    void SetExpectation(bool blend, bool has_render_surface, LayerImpl* comparison_layer)
    {
        blend_ = blend;
    ...
    bool quads_appended() const
    {
        return quads_appended_;
    }

    void SetQuadRect(const gfx::Rect& rect)
    {
        quad_rect_ = rect;
    }
    void SetQuadVisibleRect(const gfx::Rect& rect)
    {
        quad_visible_rect_ = rect;
    }
...
TEST_P(LayerTreeHostImplTest, BlendingOffWhenDrawingOpaqueLayers)
{
    ...
    auto* layer1 = AddLayer<BlendStateCheckLayer>(host_impl_->active_tree(), host_impl_->resource_provider());
    CopyProperties(root, layer1);
    ...
    auto* layer2 = AddLayer<BlendStateCheckLayer>(host_impl_->active_tree(), host_impl_->resource_provider());
    CopyProperties(layer1, layer2);
...
TEST_P(LayerTreeHostImplTest, MayContainVideo)
{
...

    {
        ...
        OwnedLayerImplList layers = host_impl_->active_tree()->DetachLayersKeepingRootLayerForTesting();
...
class LayerTreeHostImplViewportCoveredTest : public CommitToPendingTreeLayerTreeHostImplTest {
protected:
    ...
    std::unique_ptr<LayerTreeFrameSink> CreateFakeLayerTreeFrameSink(bool software)
    {
        if (software)
    ...
    void SetupActiveTreeLayers()
    {
        ...
        child_ = AddLayer<BlendStateCheckLayer>(host_impl_->active_tree(), host_impl_->resource_provider());
    ...
    void SetLayerGeometry(const gfx::Rect& layer_rect)
    {
        ...
        child_->SetOffsetToTransformParent(gfx::Vector2dF(layer_rect.OffsetFromOrigin()));
    }
    ...
    {
        SetLayerGeometry(gfx::Rect());
    }

    void VerifyEmptyLayerRenderPasses(const viz::CompositorRenderPassList& render_passes)
    {
    ...
    void VerifyLayerInMiddleOfViewport(const viz::CompositorRenderPassList& render_passes)
    {
        ASSERT_EQ(1u, render_passes.size());
    ...
    {
        SetLayerGeometry(gfx::Rect(viewport_size_.width() + 10, viewport_size_.height() + 10));
    }

    void VerifyLayerIsLargerThanViewport(const viz::CompositorRenderPassList& render_passes)
    {
        ASSERT_EQ(1u, render_passes.size());
...
protected:
    ...
    {
        VerifyQuadsExactlyCoverRect(quad_list, gfx::Rect(DipSizeToPixelSize(viewport_size_)));
    }
    ...
    void ValidateTextureDrawQuads(const viz::QuadList& quad_list)
    {
        for (auto* quad : quad_list) {
            if (quad->material != viz::DrawQuad::Material::kTextureContent)
                continue;
            const viz::TextureDrawQuad* texture_quad = viz::TextureDrawQuad::MaterialCast(quad);
            gfx::SizeF gutter_texture_size_pixels = gfx::ScaleSize(gfx::SizeF(gutter_texture_size_), host_impl_->active_tree()->device_scale_factor());
            EXPECT_EQ(texture_quad->uv_top_left.x(), texture_quad->rect.x() / gutter_texture_size_pixels.width());
            EXPECT_EQ(texture_quad->uv_top_left.y(), texture_quad->rect.y() / gutter_texture_size_pixels.height());
            EXPECT_EQ(texture_quad->uv_bottom_right.x(), texture_quad->rect.right() / gutter_texture_size_pixels.width());
            EXPECT_EQ(texture_quad->uv_bottom_right.y(), texture_quad->rect.bottom() / gutter_texture_size_pixels.height());
        }
    }
...
TEST_F(LayerTreeHostImplViewportCoveredTest, ActiveTreeShrinkViewportInvalid)
{
    ...
    CreatePendingTree();
    gfx::Size larger_viewport(viewport_size_.width() + 100, viewport_size_.height() + 100);
    host_impl_->active_tree()->SetDeviceViewportRect(gfx::Rect(DipSizeToPixelSize(larger_viewport)));
    host_impl_->ActivateSyncTree();
    EXPECT_TRUE(did_activate_pending_tree_);

    // Shrink pending tree viewport without activating.
    CreatePendingTree();
    host_impl_->active_tree()->SetDeviceViewportRect(gfx::Rect(DipSizeToPixelSize(viewport_size_)));
...
{
    std::unique_ptr<FakeLayerTreeFrameSink> layer_tree_frame_sink = FakeLayerTreeFrameSink::Create3d();
    FakeLayerTreeFrameSink* fake_layer_tree_frame_sink = layer_tree_frame_sink.get();

    // This test creates its own LayerTreeHostImpl, so
    // that we can force partial swap enabled.
    LayerTreeSettings settings = DefaultSettings();
    std::unique_ptr<LayerTreeHostImpl> layer_tree_host_impl = LayerTreeHostImpl::Create(settings, this, &task_runner_provider_, &stats_instrumentation_,
        &task_graph_runner_, AnimationHost::CreateForTesting(ThreadInstance::kImpl), nullptr, 0, nullptr, nullptr);
    layer_tree_host_impl->SetVisible(true);
    layer_tree_host_impl->InitializeFrameSink(layer_tree_frame_sink.get());

    LayerImpl* root = SetupRootLayer<LayerImpl>(layer_tree_host_impl->active_tree(), gfx::Size(500, 500));
    ...
    layer_tree_host_impl->active_tree()->SetLocalSurfaceIdFromParent(viz::LocalSurfaceId(1, base::UnguessableToken::CreateForTesting(2u, 3u)));
    ...
    args = viz::CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, viz::BeginFrameArgs::kManualSourceId, 1, base::TimeTicks() + base::Milliseconds(1));
    layer_tree_host_impl->WillBeginImplFrame(args);
    EXPECT_EQ(DrawResult::kSuccess, layer_tree_host_impl->PrepareToDraw(&frame));
    layer_tree_host_impl->DrawLayers(&frame);
    layer_tree_host_impl->DidDrawAllLayers(frame);
...
public:
    ...
    void AppendQuads(viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data) override
    {
        ...
        auto* my_quad = render_pass->CreateAndAppendDrawQuad<viz::SolidColorDrawQuad>();
...
{
    scoped_refptr<viz::TestContextProvider> context_provider = viz::TestContextProvider::Create();
    gpu::TestSharedImageInterface* sii = context_provider->SharedImageInterface();
    std::unique_ptr<LayerTreeFrameSink> layer_tree_frame_sink(FakeLayerTreeFrameSink::Create3d(context_provider));
    CreateHostImpl(DefaultSettings(), std::move(layer_tree_frame_sink));

    LayerImpl* root_layer = SetupDefaultRootLayer(gfx::Size(10, 10));

    scoped_refptr<VideoFrame> softwareFrame = media::VideoFrame::CreateColorFrame(gfx::Size(4, 4), 0x80, 0x80, 0x80, base::TimeDelta());
    FakeVideoFrameProvider provider;
    provider.set_frame(softwareFrame);
    auto* video_layer = AddLayer<VideoLayerImpl>(host_impl_->active_tree(), &provider, media::VIDEO_ROTATION_0);
...
TEST_P(LayerTreeHostImplTest, HasTransparentBackground)
{
    ...
    EXPECT_EQ(DrawResult::kSuccess, host_impl_->PrepareToDraw(&frame));
    {
        ...
        EXPECT_EQ(viz::DrawQuad::Material::kSolidColor, root_pass->quad_list.front()->material);
    }
    host_impl_->DrawLayers(&frame);
    host_impl_->DidDrawAllLayers(frame);
    host_impl_->DidFinishImplFrame(args);
...
protected:
    ...
    void DrawFrameAndTestDamage(const gfx::Rect& expected_damage, const LayerImpl* child)
    {
        ...
        if (!expect_to_draw) {
            ...
            const viz::CompositorRenderPass* root_render_pass = frame.render_passes.back().get();
            ...
            EXPECT_EQ(expected_child_visible_rect, root_render_pass->quad_list.front()->visible_rect);

            LayerImpl* root = root_layer();
            gfx::Rect expected_root_visible_rect(root->bounds());
            EXPECT_EQ(expected_root_visible_rect, root_render_pass->quad_list.ElementAt(1)->visible_rect);
        }
...
{
    auto* root = SetupRootLayer<SolidColorLayerImpl>(host_impl_->active_tree(), gfx::Size(10, 10));
    root->SetDrawsContent(true);
    ...
    gfx::Rect full_frame_damage(host_impl_->active_tree()->GetDeviceViewport().size());
...
TEST_F(CommitToPendingTreeLayerTreeHostImplTest, FarAwayQuadsDontNeedAA)
{
    ...
    auto* scrolling_layer = AddScrollableLayer(root, content_layer_bounds, gfx::Size());
    scrolling_layer->SetNeedsPushProperties();

    scoped_refptr<FakeRasterSource> raster_source(FakeRasterSource::CreateFilled(content_layer_bounds));

    auto* content_layer = AddLayer<FakePictureLayerImpl>(host_impl_->pending_tree(), raster_source);
    ...
    scrolling_layer->layer_tree_impl()->property_trees()->scroll_tree_mutable().UpdateScrollOffsetBaseForTesting(scrolling_layer->element_id(), scroll_offset);
    host_impl_->ActivateSyncTree();
    ...
    bool clipped = false;
    MathUtil::MapQuad(quad->shared_quad_state->quad_to_target_transform, gfx::QuadF(gfx::RectF(quad->shared_quad_state->visible_quad_layer_rect)), &clipped);
...
{
    SetupRootLayer<FakeLayerWithQuads>(host_impl_->active_tree(), gfx::Size(10, 10));
    UpdateDrawProperties(host_impl_->active_tree());
    DrawFrame();
...
TEST_P(LayerTreeHostImplTest, ForcedDrawToSoftwareDeviceSkipsUnsupportedLayers)
{
    set_reduce_memory_result(false);
    EXPECT_TRUE(CreateHostImpl(DefaultSettings(), FakeLayerTreeFrameSink::CreateSoftware()));
    ...
    FakeVideoFrameProvider provider;
    LayerImpl* video_layer = AddLayer<VideoLayerImpl>(host_impl_->active_tree(), &provider, media::VIDEO_ROTATION_0);
    video_layer->SetBounds(gfx::Size(10, 10));
    video_layer->SetDrawsContent(true);
    CopyProperties(root, video_layer);
    UpdateDrawProperties(host_impl_->active_tree());
    ...
    EXPECT_EQ(host_impl_->active_tree()->root_layer(), last_on_draw_frame_->will_draw_layers[0]);
}
...
TEST_P(LayerTreeHostImplTest, MemoryLimits)
{
    ...
    const TileMemoryLimitPolicy kNothingTileCutoff = ManagedMemoryPolicy::PriorityCutoffToTileMemoryLimitPolicy(gpu::MemoryAllocation::CUTOFF_ALLOW_NOTHING);
    EXPECT_NE(kGpuTileCutoff, kNothingTileCutoff);

    LayerTreeSettings settings = DefaultSettings();
    settings.memory_policy = ManagedMemoryPolicy(kGpuByteLimit, kGpuCutoff, kGpuResourceLimit);
    host_impl_ = LayerTreeHostImpl::Create(settings, this, &task_runner_provider_, &stats_instrumentation_, &task_graph_runner_,
        AnimationHost::CreateForTesting(ThreadInstance::kImpl), nullptr, 0, nullptr, nullptr);
    InputHandler::Create(static_cast<CompositorDelegateForInput&>(*host_impl_));
...
{
    gfx::Rect full_frame_damage(host_impl->active_tree()->GetDeviceViewport().size());
    TestFrameData frame;
    auto args = viz::CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, viz::BeginFrameArgs::kManualSourceId, 1, base::TimeTicks() + base::Milliseconds(1));
    host_impl->WillBeginImplFrame(args);
    EXPECT_EQ(DrawResult::kSuccess, host_impl->PrepareToDraw(&frame));
...
TEST_P(LayerTreeHostImplTestDrawAndTestDamage, RequireHighResAndRedrawWhenVisible)
{
    ASSERT_TRUE(host_impl_->active_tree());

    LayerImpl* root = SetupRootLayer<SolidColorLayerImpl>(host_impl_->active_tree(), gfx::Size(10, 10));
    root->SetBackgroundColor(SkColors::kRed);
    UpdateDrawProperties(host_impl_->active_tree());
...
public:
    void SetUp() override
    {
        fake_host_impl_ = new FakeLayerTreeHostImpl(LayerTreeSettings(), &task_runner_provider_, &task_graph_runner_);
...
TEST_P(LayerTreeHostImplTest, UIResourceManagement)
{
    ...
    gpu::TestSharedImageInterface* sii = test_context_provider->SharedImageInterface();
    CreateHostImpl(DefaultSettings(), FakeLayerTreeFrameSink::Create3d(std::move(test_context_provider)));
    ...
    EXPECT_EQ(viz::kInvalidResourceId, host_impl_->ResourceIdForUIResource(ui_resource_id));
    EXPECT_EQ(0u, sii->shared_image_count());
...
TEST_P(LayerTreeHostImplTest, ScrollHitTestIsNotReliable)
{
    ...
    occluder_layer->SetTransformTreeIndex(host_impl_->active_tree()->PageScaleTransformNode()->id);
...
TEST_P(LayerTreeHostImplTest, ScrollHitTestAncestorMismatch)
{
    ...
    LayerImpl* child_scroll = AddScrollableLayer(child_scroll_clip, viewport_size, content_size);
...
TEST_P(LayerTreeHostImplTest, ScrollInvisibleScroller)
{
    ...
    LayerImpl* scroll_layer = InnerViewportScrollLayer();
    LayerImpl* child_scroll = AddScrollableLayer(scroll_layer, viewport_size, content_size);
    ...
    EXPECT_EQ(child_scroll->scroll_tree_index(), host_impl_->CurrentlyScrollingNode()->id);
}

// Make sure LatencyInfo carried by LatencyInfoSwapPromise are passed
// in viz::CompositorFrameMetadata.
TEST_F(CommitToPendingTreeLayerTreeHostImplTest, LatencyInfoPassedToCompositorFrameMetadata)
{
    CreateHostImpl(DefaultSettings(), CreateLayerTreeFrameSink());
    SetupRootLayer<SolidColorLayerImpl>(host_impl_->active_tree(), gfx::Size(10, 10));
    UpdateDrawProperties(host_impl_->active_tree());
    ...
    const auto& metadata_latency_after = fake_layer_tree_frame_sink->last_sent_frame()->metadata.latency_info;
    EXPECT_EQ(1u, metadata_latency_after.size());
    EXPECT_TRUE(metadata_latency_after[0].FindLatency(ui::INPUT_EVENT_LATENCY_BEGIN_RWH_COMPONENT, nullptr));
}

TEST_F(CommitToPendingTreeLayerTreeHostImplTest, CompositorFrameMetadataFrameIntervalInputs)
{
    CreateHostImpl(DefaultSettings(), CreateLayerTreeFrameSink());
    SetupRootLayer<SolidColorLayerImpl>(host_impl_->active_tree(), gfx::Size(10, 10));
    UpdateDrawProperties(host_impl_->active_tree());
    ...
    host_impl_->SetNeedsRedraw();
    auto args
        = viz::CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, viz::BeginFrameArgs::kManualSourceId, 1, base::TimeTicks() + base::Milliseconds(1234));
    DrawFrameWithArgs(args);

    const auto& frame_interval_inputs = fake_layer_tree_frame_sink->last_sent_frame()->metadata.frame_interval_inputs;
...
TEST_P(LayerTreeHostImplTest, SelectionBoundsPassedToCompositorFrameMetadata)
{
    ...
    const viz::Selection<gfx::SelectionBound>& selection_after = metadata.selection;
...
public:
    ...
    {
        CreateHostImpl(DefaultSettings(), CreateLayerTreeFrameSink());
        host_impl_->active_tree()->SetBrowserControlsParams({ static_cast<float>(top_controls_height_), 0, 0, 0, false, false });
        host_impl_->active_tree()->SetCurrentBrowserControlsShownRatio(1.f, 1.f);
    }
    ...
    void Scroll(float y)
    {
        ASSERT_EQ(ScrollThread::kScrollOnImplThread,
            GetInputHandler()
                .ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(0, 50), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
                .thread);
        ASSERT_TRUE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, y), ui::ScrollInputType::kTouchscreen)).did_scroll);
        GetInputHandler().ScrollEnd();
    }

    void RunAnimation()
    {
        viz::BeginFrameArgs begin_frame_args = viz::CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 1, base::TimeTicks::Now());
        do {
            did_request_next_frame_ = false;
            host_impl_->WillBeginImplFrame(begin_frame_args);
            host_impl_->Animate();
...
TEST_P(LayerTreeHostImplWithBrowserControlsTest, NoIdleAnimations)
{
    SetupViewportLayersInnerScrolls(gfx::Size(50, 50), gfx::Size(100, 100));
    auto* scroll_layer = InnerViewportScrollLayer();
    scroll_layer->layer_tree_impl()->property_trees()->scroll_tree_mutable().UpdateScrollOffsetBaseForTesting(scroll_layer->element_id(), gfx::PointF(0, 10));
    viz::BeginFrameArgs begin_frame_args = viz::CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 2);
    host_impl_->WillBeginImplFrame(begin_frame_args);
    host_impl_->Animate();
    EXPECT_FALSE(did_request_redraw_);
    host_impl_->DidFinishImplFrame(begin_frame_args);
...
TEST_P(LayerTreeHostImplWithBrowserControlsTest, BrowserControlsHeightIsCommitted)
{
    if (CommitsToActiveTree()) {
        GTEST_SKIP();
    }
    ...
    EXPECT_FALSE(did_request_redraw_);
    CreatePendingTree();
    host_impl_->sync_tree()->SetBrowserControlsParams({ 100, 0, 0, 0, false, false });
    host_impl_->ActivateSyncTree();
    EXPECT_EQ(100, host_impl_->browser_controls_manager()->TopControlsHeight());
}

TEST_P(LayerTreeHostImplWithBrowserControlsTest, BrowserControlsStayFullyVisibleOnHeightChange)
{
    if (CommitsToActiveTree()) {
        GTEST_SKIP();
    }
    ...
    CreatePendingTree();
    host_impl_->sync_tree()->SetBrowserControlsParams({ 50, 0, 0, 0, false, false });
    host_impl_->ActivateSyncTree();
    EXPECT_EQ(0, host_impl_->browser_controls_manager()->ControlsTopOffset());
}

TEST_P(LayerTreeHostImplWithBrowserControlsTest, BrowserControlsAnimationScheduling)
{
    SetupViewportLayersInnerScrolls(gfx::Size(50, 50), gfx::Size(100, 100));
    auto* scroll_layer = InnerViewportScrollLayer();
    scroll_layer->layer_tree_impl()->property_trees()->scroll_tree_mutable().UpdateScrollOffsetBaseForTesting(scroll_layer->element_id(), gfx::PointF(0, 10));
...
TEST_P(LayerTreeHostImplWithBrowserControlsTest, ScrollHandledByBrowserControls)
{
    InputHandlerScrollResult result;
    ...
    host_impl_->browser_controls_manager()->UpdateBrowserControlsState(BrowserControlsState::kBoth, BrowserControlsState::kShown, false, std::nullopt);
    DrawFrame();
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler()
            .ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(0, offset), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
            .thread);
    EXPECT_EQ(0, host_impl_->browser_controls_manager()->ControlsTopOffset());
    EXPECT_EQ(gfx::PointF(), CurrentScrollOffset(scroll_layer));

    result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, offset), ui::ScrollInputType::kTouchscreen));
    EXPECT_EQ(result.unused_scroll_delta, gfx::Vector2d(0, 0));
    EXPECT_TRUE(result.did_scroll);
    EXPECT_FLOAT_EQ(-offset, host_impl_->browser_controls_manager()->ControlsTopOffset());
    EXPECT_EQ(gfx::PointF(), CurrentScrollOffset(scroll_layer));
    ...
    EXPECT_TRUE(result.did_scroll);
    EXPECT_EQ(result.unused_scroll_delta, gfx::Vector2d(0, 0));
    EXPECT_EQ(-top_controls_height_, host_impl_->browser_controls_manager()->ControlsTopOffset());
...
TEST_P(LayerTreeHostImplWithBrowserControlsTest, WheelUnhandledByBrowserControls)
{
    SetupViewportLayersInnerScrolls(gfx::Size(50, 100), gfx::Size(100, 200));
    host_impl_->active_tree()->SetBrowserControlsParams({ top_controls_height_, 0, 0, 0, false, true });
    host_impl_->browser_controls_manager()->UpdateBrowserControlsState(BrowserControlsState::kBoth, BrowserControlsState::kShown, false, std::nullopt);
    DrawFrame();
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler()
            .ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(0, delta), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel)
            .thread);
    EXPECT_EQ(0, host_impl_->browser_controls_manager()->ControlsTopOffset());
    ...
    EXPECT_TRUE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, delta), ui::ScrollInputType::kWheel)).did_scroll);
    EXPECT_FLOAT_EQ(0, host_impl_->browser_controls_manager()->ControlsTopOffset());
    EXPECT_POINTF_EQ(gfx::PointF(0, delta), CurrentScrollOffset(viewport_layer));

    EXPECT_TRUE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, delta), ui::ScrollInputType::kWheel)).did_scroll);
    EXPECT_FLOAT_EQ(0, host_impl_->browser_controls_manager()->ControlsTopOffset());
    EXPECT_POINTF_EQ(gfx::PointF(0, delta * 2), CurrentScrollOffset(viewport_layer));
}

TEST_P(LayerTreeHostImplWithBrowserControlsTest, BrowserControlsAnimationAtOrigin)
{
    SetupViewportLayersInnerScrolls(gfx::Size(50, 100), gfx::Size(100, 200));
    auto* scroll_layer = InnerViewportScrollLayer();
    UpdateDrawProperties(host_impl_->active_tree());
    ...
    EXPECT_TRUE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, offset), ui::ScrollInputType::kTouchscreen)).did_scroll);
    EXPECT_FLOAT_EQ(-offset, host_impl_->browser_controls_manager()->ControlsTopOffset());
    EXPECT_EQ(gfx::PointF(), CurrentScrollOffset(scroll_layer));
...
TEST_P(LayerTreeHostImplWithBrowserControlsTest, BrowserControlsAnimationAfterScroll)
{
    SetupViewportLayersInnerScrolls(gfx::Size(50, 100), gfx::Size(100, 200));
    auto* scroll_layer = InnerViewportScrollLayer();
    UpdateDrawProperties(host_impl_->active_tree());
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler()
            ...
            .thread);
    EXPECT_EQ(0, host_impl_->browser_controls_manager()->ControlsTopOffset());
    EXPECT_EQ(gfx::PointF(0, initial_scroll_offset), CurrentScrollOffset(scroll_layer));
...
TEST_P(LayerTreeHostImplWithBrowserControlsTest, BrowserControlsScrollDeltaInOverScroll)
{
    ...
    EXPECT_TRUE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, offset), ui::ScrollInputType::kTouchscreen)).did_scroll);
    EXPECT_EQ(-offset, host_impl_->browser_controls_manager()->ControlsTopOffset());
    EXPECT_EQ(gfx::PointF(), CurrentScrollOffset(scroll_layer));
    ...
    EXPECT_EQ(gfx::PointF(0, MaxScrollOffset(scroll_layer).y()), CurrentScrollOffset(scroll_layer));

    float overscrollamount = 10;

    // Overscroll the content
    EXPECT_FALSE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, overscrollamount), ui::ScrollInputType::kTouchscreen)).did_scroll);
    EXPECT_EQ(gfx::PointF(0, 2 * offset), CurrentScrollOffset(scroll_layer));
    EXPECT_EQ(gfx::Vector2dF(0, overscrollamount), GetInputHandler().accumulated_root_overscroll_for_testing());

    EXPECT_TRUE(GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2d(0, -2 * offset), ui::ScrollInputType::kTouchscreen)).did_scroll);
...
TEST_P(LayerTreeHostImplWithBrowserControlsTest, AnimationDoesntScrollUnscrolledViewport)
{
...
TEST_P(LayerTreeHostImplWithBrowserControlsTest, AnimationScrollsScrolledViewport)
{
    // Initialize with 50px browser controls, 200px contents, and 100px viewport.
    SetupViewportLayersInnerScrolls(gfx::Size(100, 100), gfx::Size(100, 200));
...
TEST_P(LayerTreeHostImplWithBrowserControlsTest, AnimationScrollsViewportWhenOnlyExpandTopControlsAtPageTopIsSet)
{
    ...
    SetupViewportLayersInnerScrolls(gfx::Size(100, 100), gfx::Size(100, 200));
    LayerTreeImpl* layer_tree_impl = host_impl_->active_tree();
    layer_tree_impl->SetBrowserControlsParams({ /*top_controls_height=*/50.f, 0.f, 0.f, 0.f, false, false,
...
TEST_P(LayerTreeHostImplBrowserControlsTest, ReplacedOuterViewportScrollsBrowserControls)
{
    const gfx::Size scroll_content_size(400, 400);
    const gfx::Size root_layer_size(200, 200);
    const gfx::Size viewport_size(100, 100);

    SetupBrowserControlsAndScrollLayerWithVirtualViewport(viewport_size, viewport_size, root_layer_size);
    ...
    CreateClipNode(clip_layer);
    LayerImpl* scroll_layer = AddScrollableLayer(clip_layer, root_layer_size, scroll_content_size);
    GetScrollNode(scroll_layer)->scrolls_outer_viewport = true;
...
TEST_P(LayerTreeHostImplTest, RootScrollBothInnerAndOuterLayer)
{
    ...
    SetupViewportLayers(host_impl_->active_tree(), inner_viewport, outer_viewport, content_size);
...
TEST_P(LayerTreeHostImplTest, DiagonalScrollBubblesPerfectlyToInner)
{
    ...
    DrawFrame();
    {
        ...
        gfx::Vector2dF scroll_delta(inner_viewport.width() / 2, inner_viewport.height() / 2);
        ...
        GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::ScaleVector2d(scroll_delta, 2), ui::ScrollInputType::kTouchscreen));
...
TEST_P(LayerTreeHostImplTest, ScrollBeginEventThatTargetsViewportLayerSkipsHitTest)
{
    gfx::Size content_size = gfx::Size(100, 160);
    gfx::Size outer_viewport = gfx::Size(50, 80);
    gfx::Size inner_viewport = gfx::Size(25, 40);
    ...
    LayerImpl* outer_scroll = OuterViewportScrollLayer();
    LayerImpl* child_scroll = AddScrollableLayer(outer_scroll, inner_viewport, outer_viewport);
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler()
            .RootScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(0, 10), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
            .thread);
    EXPECT_EQ(host_impl_->CurrentlyScrollingNode(), host_impl_->OuterViewportScrollNode());
    GetInputHandler().ScrollEnd();
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler()
            .ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(0, 10), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
            .thread);
    EXPECT_EQ(host_impl_->CurrentlyScrollingNode()->id, child_scroll->scroll_tree_index());
    GetInputHandler().ScrollEnd();
}
...
TEST_P(LayerTreeHostImplTest, NoOverscrollWhenInnerViewportCantScroll)
{
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler()
            .ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(0, 100), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
            .thread);
    scroll_result = GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, 100), ui::ScrollInputType::kTouchscreen));
    EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(inner_scroll));
...
TEST_P(LayerTreeHostImplWithImplicitLimitsTest, ImplicitMemoryLimits)
{
    ...
    EXPECT_EQ(host_impl_->global_tile_state().hard_memory_limit_in_bytes, 300u * 1024u * 1024u);
    EXPECT_EQ(host_impl_->global_tile_state().soft_memory_limit_in_bytes, 150u * 1024u * 1024u);
}
...
TEST_P(LayerTreeHostImplTest, ExternalTransformReflectedInNextDraw)
{
    ...
    host_impl_->SetExternalTilePriorityConstraints(external_viewport, external_transform);
    host_impl_->OnDraw(external_transform, external_viewport, resourceless_software_draw, false);
    EXPECT_TRANSFORM_EQ(external_transform, layer->draw_properties().target_space_transform);
...
TEST_P(LayerTreeHostImplTest, ExternalTransformSetNeedsRedraw)
{
    ...
    // Clear any damage.
    host_impl_->SetExternalTilePriorityConstraints(viewport_for_tile_priority1, transform_for_tile_priority);
    host_impl_->OnDraw(draw_transform, draw_viewport, resourceless_software_draw, false);
    last_on_draw_frame_.reset();

    // Setting new constraints needs redraw.
    did_request_redraw_ = false;
    host_impl_->SetExternalTilePriorityConstraints(viewport_for_tile_priority2, transform_for_tile_priority);
    EXPECT_TRUE(did_request_redraw_);
    host_impl_->OnDraw(draw_transform, draw_viewport, resourceless_software_draw, false);
...
TEST_P(LayerTreeHostImplTest, OnMemoryPressure)
{
    ...
    ResourcePool::InUsePoolResource resource = host_impl_->resource_pool()->AcquireResource(size, format, color_space);
    ...
    current_memory_usage = host_impl_->resource_pool()->GetTotalMemoryUsageForTesting();

    base::MemoryPressureListener::SimulatePressureNotification(base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_CRITICAL);
    base::RunLoop().RunUntilIdle();

    size_t memory_usage_after_memory_pressure = host_impl_->resource_pool()->GetTotalMemoryUsageForTesting();
...
TEST_P(LayerTreeHostImplTest, OnDrawConstraintSetNeedsRedraw)
{
    ...
    // Clear any damage.
    host_impl_->OnDraw(draw_transform, draw_viewport1, resourceless_software_draw, false);
    last_on_draw_frame_.reset();
    ...
    did_request_redraw_ = false;
    host_impl_->OnDraw(draw_transform, draw_viewport2, resourceless_software_draw, false);
    EXPECT_TRUE(did_request_redraw_);
    EXPECT_FALSE(last_on_draw_frame_->has_no_damage);
}
...
TEST_P(LayerTreeHostImplTest, FullViewportDamageAfterOnDraw)
{
    ...
    host_impl_->SetFullViewportDamage();
    EXPECT_EQ(host_impl_->active_tree()->internal_device_viewport(), host_impl_->viewport_damage_rect_for_testing());
}

class ResourcelessSoftwareLayerTreeHostImplTest : public CommitToPendingTreeLayerTreeHostImplTest {
protected:
    std::unique_ptr<LayerTreeFrameSink> CreateLayerTreeFrameSink() override
    {
        return FakeLayerTreeFrameSink::Create3d();
...
TEST_F(ResourcelessSoftwareLayerTreeHostImplTest, ResourcelessSoftwareSetNeedsRedraw)
{
    const gfx::Size viewport_size(100, 100);
    SetupDefaultRootLayer(viewport_size);
    UpdateDrawProperties(host_impl_->active_tree());
...
TEST_F(ResourcelessSoftwareLayerTreeHostImplTest, ResourcelessSoftwareDrawSkipsUpdateTiles)
{
    const gfx::Size viewport_size(100, 100);

    CreatePendingTree();
    scoped_refptr<FakeRasterSource> raster_source(FakeRasterSource::CreateFilled(viewport_size));
    auto* root = SetupRootLayer<FakePictureLayerImpl>(host_impl_->pending_tree(), viewport_size, raster_source);
    root->SetBounds(viewport_size);
    root->SetDrawsContent(true);
    ...
    resourceless_software_draw = true;
    did_request_prepare_tiles_ = false;
    host_impl_->OnDraw(draw_transform, new_draw_viewport, resourceless_software_draw, false);
    EXPECT_FALSE(did_request_prepare_tiles_);
}

TEST_F(CommitToPendingTreeLayerTreeHostImplTest, ExternalTileConstraintReflectedInPendingTree)
{
    const gfx::Size layer_size(100, 100);
...
TEST_P(LayerTreeHostImplTest, ExternalTransformAffectsSublayerScaleFactor)
{
    ...
    CreateEffectNode(test_layer).render_surface_reason = RenderSurfaceReason::kTest;
...
TEST_F(CommitToPendingTreeLayerTreeHostImplTest, OneScrollForFirstScrollDelay)
{
    ...
    ui::LatencyInfo latency_info;
    latency_info.set_trace_id(5);
    latency_info.AddLatencyNumber(ui::INPUT_EVENT_LATENCY_SCROLL_UPDATE_ORIGINAL_COMPONENT);
    std::unique_ptr<SwapPromise> swap_promise(new LatencyInfoSwapPromise(latency_info));
    host_impl_->active_tree()->QueuePinnedSwapPromise(std::move(swap_promise));
...
TEST_F(CommitToPendingTreeLayerTreeHostImplTest, OtherInputsForFirstScrollDelay)
{
    CreateHostImpl(DefaultSettings(), CreateLayerTreeFrameSink());
    SetupRootLayer<SolidColorLayerImpl>(host_impl_->active_tree(), gfx::Size(10, 10));
    UpdateDrawProperties(host_impl_->active_tree());
...
TEST_F(CommitToPendingTreeLayerTreeHostImplTest, MultipleScrollsForFirstScrollDelay)
{
    CreateHostImpl(DefaultSettings(), CreateLayerTreeFrameSink());
    SetupRootLayer<SolidColorLayerImpl>(host_impl_->active_tree(), gfx::Size(10, 10));
    UpdateDrawProperties(host_impl_->active_tree());
    ...
    latency_info2.AddLatencyNumber(ui::INPUT_EVENT_LATENCY_SCROLL_UPDATE_ORIGINAL_COMPONENT);
    std::unique_ptr<SwapPromise> swap_promise2(new LatencyInfoSwapPromise(latency_info2));
...
TEST_P(LayerTreeHostImplTest, ScrollAnimated)
{
...

    {
        ...
        EXPECT_EQ(ScrollThread::kScrollOnImplThread,
            GetInputHandler()
                .ScrollBegin(BeginState(gfx::Point(), gfx::Vector2d(0, 50), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel)
                .thread);
        GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(0, 50)));
    }

    LayerImpl* scrolling_layer = OuterViewportScrollLayer();
    EXPECT_EQ(scrolling_layer->scroll_tree_index(), host_impl_->CurrentlyScrollingNode()->id);
...
TEST_P(LayerTreeHostImplTest, ScrollAnimatedLatching)
{
    ...
    AddScrollableLayer(OuterViewportScrollLayer(), gfx::Size(10, 10), gfx::Size(20, 20));
    ...
    // Start an animated scroll over the outer viewport.
    {
        ...
        auto begin_state = BeginState(position, gfx::Vector2d(0, 50), ui::ScrollInputType::kWheel);
        ...
        GetInputHandler().ScrollUpdate(update_state);
        EXPECT_EQ(outer_scroll->scroll_tree_index(), host_impl_->CurrentlyScrollingNode()->id);
...
TEST_P(LayerTreeHostImplTest, ScrollAnimatedWhileZoomed)
{
    ...
    // animation update code.
    {
        EXPECT_EQ(ScrollThread::kScrollOnImplThread,
            GetInputHandler()
                .ScrollBegin(BeginState(gfx::Point(10, 10), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel)
                .thread);
        GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(10, 10), gfx::Vector2d(0, 10)));
        GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(10, 10), gfx::Vector2d(0, 20)));
...
TEST_P(LayerTreeHostImplTest, ScrollAnimatedUpdateInnerViewport)
{
    ...
    {
        auto begin_state = BeginState(gfx::Point(10, 10), gfx::Vector2d(0, 60), ui::ScrollInputType::kWheel);
        GetInputHandler().ScrollBegin(begin_state.get(), ui::ScrollInputType::kWheel);
        GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(10, 10), gfx::Vector2d(0, 60)));
        ...
        EXPECT_TRUE(GetImplAnimationHost()->ElementHasImplOnlyScrollAnimation(InnerViewportScrollLayer()->element_id()));
    }
...
TEST_P(FluentOverlayScrollbarOpacityLayerTreeHostImplTest, PaintedOverlayScrollbarTrackOpacityTest)
{
    auto* scrollbar = CreateAndRegisterPaintedScrollbarLayer();
    // Make the scrollbar visible.
    scrollbar->SetOverlayScrollbarLayerOpacityAnimated(1.f, /*fade_out_animation=*/false);
    UpdateDrawProperties(host_impl_->active_tree());

    EXPECT_EQ(1.f, scrollbar->draw_properties().opacity);
    int const step = GetParam();
    float const thickness_scale_step = (1 - scrollbar->GetIdleThicknessScale()) / kParamSteps;
    VerifyCorrectOpacityForThickness(scrollbar, scrollbar->GetIdleThicknessScale() + thickness_scale_step * step, step / static_cast<float>(kParamSteps));
}

INSTANTIATE_TEST_SUITE_P(
    ScaleOpacity, FluentOverlayScrollbarOpacityLayerTreeHostImplTest, Range(0, FluentOverlayScrollbarOpacityLayerTreeHostImplTest::kParamSteps + 1));

TEST_F(FluentOverlayScrollbarLayerTreeHostImplTest, FluentScrollbarFlashAfterScrollUpdate)
{
    ...
    auto* child_scrollbar = AddLayer<PaintedScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::kVertical, false, true);
    // SetupScrollbarLayerCommon will register the scrollbar, which sets the
    // layer's opacity to 0. An effect node for the scrollbar layer object needs
    // to be registered in the EffectTree before this happens.
    auto& effect_node = CreateEffectNode(child_scrollbar, child->effect_tree_index());
    ...
    auto reset_scrollbars = [root_scrollbar, child_scrollbar](LayerTreeImpl* active_tree, base::OnceClosure& animation_task) {
...
TEST_F(FluentOverlayScrollbarLayerTreeHostImplTest, PaintedOverlayLayerOnLoadOpacityTest)
{
...
TEST_F(FluentOverlayScrollbarLayerTreeHostImplTest, DoesntGetHitTestedWhenInvisible)
{
    EXPECT_FLOAT_EQ(CreateAndRegisterPaintedScrollbarLayer()->Opacity(), 0.f);
    ...
    InputHandlerPointerResult result = GetInputHandler().MouseMoveAt(gfx::Point(352, 300));
...
TEST_P(LayerTreeHostImplTest, FadedOutPaintedOverlayScrollbarHitTest)
{
    ...
    LayerTreeImpl* layer_tree_impl = host_impl_->active_tree();
    auto* scrollbar = AddLayer<NinePatchThumbScrollbarLayerImpl>(layer_tree_impl, ScrollbarOrientation::kVertical, false);
    SetupScrollbarLayerCommon(scroll_layer, scrollbar);
    scrollbar->SetHitTestOpaqueness(HitTestOpaqueness::kMixed);
    ...
    GetInputHandler().ScrollBegin(BeginState(gfx::Point(350, 18), gfx::Vector2dF(), ui::ScrollInputType::kScrollbar).get(), ui::ScrollInputType::kScrollbar);
    ...
    InputHandlerPointerResult result = GetInputHandler().MouseMoveAt(gfx::Point(350, 28));
...
TEST_P(LayerTreeHostImplTest, ScrollOnLargeThumb)
{
    ...
    GetInputHandler().MouseDown(gfx::PointF(350, 300),
        /*jump_key_modifier*/ false);
    EXPECT_TRUE(GetInputHandler().scrollbar_controller_for_testing()->drag_state_.has_value());

    // Moving the mouse downwards should result in no scroll.
    InputHandlerPointerResult res = GetInputHandler().MouseMoveAt(gfx::Point(350, 600));
...
TEST_P(LayerTreeHostImplTest, AutoscrollOnDeletedScrollbar)
{
    ...
    host_impl_->DidUnregisterScrollbarLayer(scroll_layer->element_id(), ScrollbarOrientation::kHorizontal);
    EXPECT_TRUE(GetInputHandler().scrollbar_controller_for_testing()->AutoscrollTaskIsScheduled());
    ...
    GetInputHandler().scrollbar_controller_for_testing()->cancelable_autoscroll_task_->callback().Run();
    GetInputHandler().scrollbar_controller_for_testing()->cancelable_autoscroll_task_.reset();
    EXPECT_EQ(GetInputHandler().scrollbar_controller_for_testing()->autoscroll_state_->status, ScrollbarController::AutoScrollStatus::kAutoscrollReady);
    ...
    GetInputHandler().scrollbar_controller_for_testing()->cancelable_autoscroll_task_->callback().Run();
    GetInputHandler().scrollbar_controller_for_testing()->cancelable_autoscroll_task_.reset();
    EXPECT_EQ(GetInputHandler().scrollbar_controller_for_testing()->autoscroll_state_->status, ScrollbarController::AutoScrollStatus::kAutoscrollScrolling);
...
TEST_P(LayerTreeHostImplTest, PointerMoveOutOfSequence)
{
    ...
    InputHandlerPointerResult result = GetInputHandler().MouseMoveAt(gfx::Point(350, 19));
    EXPECT_GT(result.scroll_delta.y(), 0u);
...
TEST_P(LayerTreeHostImplTest, FadedOutPaintedScrollbarHitTest)
{
    ...
    InputHandlerPointerResult result = GetInputHandler().MouseDown(gfx::PointF(350, 100), /*jump_key_modifier*/ false);
    EXPECT_EQ(result.scroll_delta.y(), 0u);
...
TEST_P(LayerTreeHostImplTest, SingleGSUForScrollbarThumbDragPerFrame)
{
    ...
    scrollbar->SetBackButtonRect(gfx::Rect(gfx::Point(345, 0), gfx::Size(15, 15)));
    scrollbar->SetForwardButtonRect(gfx::Rect(gfx::Point(345, 570), gfx::Size(15, 15)));
    ...
    InputHandlerPointerResult result = GetInputHandler().MouseDown(gfx::PointF(350, 18), /*jump_key_modifier*/ false);
    EXPECT_EQ(result.scroll_delta.y(), 0u);
...
TEST_P(LayerTreeHostImplTest, ClearTrackingManagerOnLayerTreeFrameSinkLoseContext)
{
    ...
    for (int i = 0; i < 30; i++) {
        ...
        events_metrics.push_back(ScrollUpdateEventMetrics::Create(ui::EventType::kGestureScrollUpdate, ui::ScrollInputType::kTouchscreen,
            /*is_inertial=*/false, i == 0 ? ScrollUpdateEventMetrics::ScrollUpdateType::kStarted : ScrollUpdateEventMetrics::ScrollUpdateType::kContinued,
            /*delta=*/10.0f, base::TimeTicks::Now(), base::TimeTicks::Now() + base::Milliseconds(1), base::TimeTicks(),
            /*trace_id*/ base::IdType64<class ui::LatencyInfo>(123)));
        host_impl_->active_tree()->AppendEventsMetricsFromMainThread(std::move(events_metrics));
...
{
    TotalFrameCounter* total_frame_counter = host_impl_->total_frame_counter_for_testing();
    ...
    viz::BeginFrameArgs args
        = viz::BeginFrameArgs::Create(BEGINFRAME_FROM_HERE, 1u /*source_id*/, 2u /*sequence_number*/, now, deadline, interval, viz::BeginFrameArgs::NORMAL);

    dropped_frame_counter->OnEndFrame(args, CreateFakeFrameInfo(FrameInfo::FrameFinalState::kDropped));
...
TEST_P(LayerTreeHostImplTest, FrameCounterNotReset)
{
    ...
    base::TimeTicks now = base::TimeTicks::Now();
    auto deadline = now + interval;
    viz::BeginFrameArgs arg1
        = viz::BeginFrameArgs::Create(BEGINFRAME_FROM_HERE, 1u /*source_id*/, 1u /*sequence_number*/, now, deadline, interval, viz::BeginFrameArgs::NORMAL);
    BeginMainFrameMetrics begin_frame_metrics;
    begin_frame_metrics.should_measure_smoothness = true;
...
TEST_P(LayerTreeHostImplTest, AutoscrollTaskAbort)
{
    ...
    // Set up the scrollbar and its dimensions.
    LayerTreeImpl* layer_tree_impl = host_impl_->active_tree();
    auto* scrollbar = AddLayer<PaintedScrollbarLayerImpl>(layer_tree_impl, ScrollbarOrientation::kVertical,
...

    {
        ...
        InputHandlerPointerResult result = GetInputHandler().MouseDown(gfx::PointF(350, 575), /*jump_key_modifier*/ false);
        EXPECT_EQ(result.type, PointerResultType::kScrollbarScroll);
        auto begin_state = BeginState(gfx::Point(350, 575), gfx::Vector2d(0, 40), ui::ScrollInputType::kScrollbar);
        EXPECT_EQ(ScrollThread::kScrollOnImplThread, GetInputHandler().ScrollBegin(begin_state.get(), ui::ScrollInputType::kScrollbar).thread);
        EXPECT_TRUE(GetInputHandler().scrollbar_controller_for_testing()->AutoscrollTaskIsScheduled());
    }

    {
        ...
        EXPECT_EQ(result.type, PointerResultType::kScrollbarScroll);
        GetInputHandler().ScrollEnd();
        EXPECT_FALSE(GetInputHandler().scrollbar_controller_for_testing()->AutoscrollTaskIsScheduled());
    }
...
TEST_P(LayerTreeHostImplTest, JumpOnScrollbarClick)
{
    ...
    // Verify all 4 combinations of JumpOnTrackClick and jump_key_modifier.
    {
        ...
        InputHandlerPointerResult result = GetInputHandler().MouseDown(gfx::PointF(350, 400), /*jump_key_modifier*/ false);
        EXPECT_EQ(result.type, PointerResultType::kScrollbarScroll);
...

    {
        ...
        scrollbar->SetJumpOnTrackClick(false);
        InputHandlerPointerResult result = GetInputHandler().MouseDown(gfx::PointF(350, 400), /*jump_key_modifier*/ true);
        EXPECT_EQ(result.type, PointerResultType::kScrollbarScroll);
...
TEST_P(LayerTreeHostImplTest, ThumbDragAfterJumpClick)
{
    ...
    {
        EXPECT_FALSE(GetInputHandler().scrollbar_controller_for_testing()->drag_state_.has_value());

        // Perform a pointerdown on the track part to induce a jump click.
        const InputHandlerPointerResult result
            = GetInputHandler().scrollbar_controller_for_testing()->HandlePointerDown(gfx::PointF(350, 560), /*jump_key_modifier*/ true);
        ...
        EXPECT_FLOAT_EQ(GetInputHandler().scrollbar_controller_for_testing()->drag_state_->scroll_position_at_start_, 0.0f);

        EXPECT_FLOAT_EQ(GetInputHandler().scrollbar_controller_for_testing()->drag_state_->drag_origin.y(), 15.0f + thumb_len / 2.0f);
    }
...
TEST_P(LayerTreeHostImplTest, AbortAnimatedScrollBeforeStartingAutoscroll)
{
...

    {
        ...
        GetInputHandler().scrollbar_controller_for_testing()->HandlePointerDown(gfx::PointF(350, 560), /*jump_key_modifier*/ false);
        auto begin_state = BeginState(gfx::Point(350, 560), gfx::Vector2d(0, 40), ui::ScrollInputType::kScrollbar);
        EXPECT_EQ(ScrollThread::kScrollOnImplThread, GetInputHandler().ScrollBegin(begin_state.get(), ui::ScrollInputType::kScrollbar).thread);
        auto update_state = UpdateState(gfx::Point(350, 560), gfx::Vector2dF(0, 40), ui::ScrollInputType::kScrollbar);
        update_state.data()->delta_granularity = ui::ScrollGranularity::kScrollByPixel;
        GetInputHandler().ScrollUpdate(update_state);

        // Autoscroll animations should be active.
        EXPECT_TRUE(GetInputHandler().scrollbar_controller_for_testing()->ScrollbarScrollIsActive());
        EXPECT_TRUE(GetImplAnimationHost()->HasImplOnlyScrollAnimatingElement());
    }

    {
        ...
        // failures.
        GetInputHandler().scrollbar_controller_for_testing()->autoscroll_state_ = ScrollbarController::AutoScrollState();
        GetInputHandler().scrollbar_controller_for_testing()->autoscroll_state_->velocity = 800;
        GetInputHandler().scrollbar_controller_for_testing()->autoscroll_state_->pressed_scrollbar_part = ScrollbarPart::kForwardTrack;
...
TEST_P(LayerTreeHostImplTest, AnimatedScrollYielding)
{
...

    {
        ...
        auto begin_state = BeginState(gfx::Point(), gfx::Vector2d(350, 560), ui::ScrollInputType::kWheel);
        begin_state->data()->delta_granularity = ui::ScrollGranularity::kScrollByPixel;
        ...
        EXPECT_FALSE(GetInputHandler().scrollbar_controller_for_testing()->ScrollbarLayer());

        EXPECT_EQ(ScrollThread::kScrollOnImplThread, GetInputHandler().ScrollBegin(begin_state.get(), ui::ScrollInputType::kWheel).thread);

        // Autoscroll animation should be aborted at this point.
        EXPECT_FALSE(GetImplAnimationHost()->HasImplOnlyScrollAnimatingElement());
        GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(350, 560), gfx::Vector2d(0, 40)));
...
TEST_P(LayerTreeHostImplTest, MainThreadFallback)
{
    ...
    InputHandlerPointerResult compositor_threaded_scrolling_result = GetInputHandler().MouseDown(gfx::PointF(350, 500),
        /*jump_key_modifier*/ false);
    ...
    GetScrollNode(scroll_layer)->main_thread_repaint_reasons = MainThreadScrollingReason::kPreferNonCompositedScrolling;
    compositor_threaded_scrolling_result = GetInputHandler().MouseDown(gfx::PointF(350, 500), /*jump_key_modifier*/ false);
    GetInputHandler().MouseUp(gfx::PointF(350, 500));
...
TEST_P(LayerTreeHostImplTest, AnimatedScrollUpdateTargetBeforeStarting)
{
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2d(0, 50), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel).thread);
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(0, 50)));
...
TEST_P(LayerTreeHostImplTest, ScrollAnimatedWithDelay)
{
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2d(0, 100), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel).thread);
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(0, 100)), base::Milliseconds(100));
    ...
    base::TimeTicks half_way_time = start_time - begin_frame_args.interval + base::Milliseconds(50);
    begin_frame_args.frame_time = half_way_time;
    begin_frame_args.frame_id.sequence_number++;
    host_impl_->WillBeginImplFrame(begin_frame_args);
    host_impl_->UpdateAnimationState(true);
    EXPECT_NEAR((features::IsImpulseScrollAnimationEnabled() ? 87 : 50), CurrentScrollOffset(scrolling_layer).y(), 1);
    host_impl_->DidFinishImplFrame(begin_frame_args);

    // Update target.
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(0, 100)), base::Milliseconds(150));
...
TEST_P(LayerTreeHostImplTimelinesTest, ScrollAnimatedAborted)
{
    ...
    EXPECT_TRUE(GetImplAnimationHost()->HasAnyAnimationTargetingProperty(scrolling_layer->element_id(), TargetProperty::SCROLL_OFFSET));
    ...
    auto begin_state = BeginState(gfx::Point(0, y), gfx::Vector2dF(0, 50), ui::ScrollInputType::kWheel);
    begin_state->data()->delta_granularity = ui::ScrollGranularity::kScrollByPrecisePixel;
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, GetInputHandler().ScrollBegin(begin_state.get(), ui::ScrollInputType::kWheel).thread);
    auto update_state = UpdateState(gfx::Point(0, y), gfx::Vector2d(0, 50), ui::ScrollInputType::kWheel);
    // Use "precise pixel" granularity to avoid animating.
    update_state.data()->delta_granularity = ui::ScrollGranularity::kScrollByPrecisePixel;
    GetInputHandler().ScrollUpdate(update_state);
    GetInputHandler().ScrollEnd();
    ...
    // aborted.
    EXPECT_FALSE(GetImplAnimationHost()->HasTickingKeyframeModelForTesting(scrolling_layer->element_id()));

    EXPECT_POINTF_EQ(gfx::PointF(0, y + 50), CurrentScrollOffset(scrolling_layer));
    EXPECT_EQ(nullptr, host_impl_->CurrentlyScrollingNode());
    host_impl_->DidFinishImplFrame(begin_frame_args);
}
...
TEST_P(LayerTreeHostImplTimelinesTest, ImplPinchZoomScrollAnimated)
{
    ...
    viz::BeginFrameArgs begin_frame_args = viz::CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 1);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2d(10, 20), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel).thread);
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(10, 20)));
    host_impl_->Animate();
    host_impl_->UpdateAnimationState(true);
    EXPECT_EQ(outer_scroll_layer->scroll_tree_index(), host_impl_->CurrentlyScrollingNode()->id);
    ...
    // Scroll by the inner viewport's max scroll extent, the remainder
    // should bubble up to the outer viewport.
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(100, 100)));
    host_impl_->Animate();
    host_impl_->UpdateAnimationState(true);
    EXPECT_EQ(outer_scroll_layer->scroll_tree_index(), host_impl_->CurrentlyScrollingNode()->id);

    begin_frame_args.frame_id.sequence_number++;
    BeginImplFrameAndAnimate(begin_frame_args, start_time + base::Milliseconds(350));
    EXPECT_POINTF_EQ(gfx::PointF(50, 50), CurrentScrollOffset(inner_scroll_layer));
    EXPECT_POINTF_EQ(gfx::PointF(5, 10), CurrentScrollOffset(outer_scroll_layer));

    // Scroll by the outer viewport's max scroll extent, it should all go to the
    // outer viewport.
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(190, 180)));
    host_impl_->Animate();
    host_impl_->UpdateAnimationState(true);
    EXPECT_EQ(outer_scroll_layer->scroll_tree_index(), host_impl_->CurrentlyScrollingNode()->id);

    begin_frame_args.frame_id.sequence_number++;
    BeginImplFrameAndAnimate(begin_frame_args, start_time + base::Milliseconds(850));
    EXPECT_POINTF_EQ(gfx::PointF(50, 50), CurrentScrollOffset(inner_scroll_layer));
    EXPECT_POINTF_EQ(gfx::PointF(100, 100), CurrentScrollOffset(outer_scroll_layer));
    ...
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(-110, -120)));
    host_impl_->Animate();
    host_impl_->UpdateAnimationState(true);
    EXPECT_EQ(outer_scroll_layer->scroll_tree_index(), host_impl_->CurrentlyScrollingNode()->id);

    begin_frame_args.frame_id.sequence_number++;
    BeginImplFrameAndAnimate(begin_frame_args, start_time + base::Milliseconds(1200));
    EXPECT_POINTF_EQ(gfx::PointF(0, 0), CurrentScrollOffset(inner_scroll_layer));
    EXPECT_POINTF_EQ(gfx::PointF(95, 90), CurrentScrollOffset(outer_scroll_layer));
}
...
TEST_P(LayerTreeHostImplTimelinesTest, ImplPinchZoomScrollAnimatedUpdate)
{
    ...
    viz::BeginFrameArgs begin_frame_args = viz::CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 1);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2d(90, 90), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel).thread);
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(90, 90)));
    host_impl_->Animate();
    host_impl_->UpdateAnimationState(true);
    ...
    // Update target.
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(50, 50)));
    host_impl_->Animate();
    host_impl_->UpdateAnimationState(true);
    EXPECT_EQ(outer_scroll_layer->scroll_tree_index(), host_impl_->CurrentlyScrollingNode()->id);
...
TEST_P(LayerTreeHostImplTimelinesTest, ScrollAnimatedNotUserScrollable)
{
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2d(50, 50), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel).thread);
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(50, 50)));
...
TEST_P(LayerTreeHostImplTimelinesTest, ScrollAnimatedChangingBounds)
{
    ...
    GetInputHandler().ScrollBegin(BeginState(gfx::Point(), gfx::Vector2d(500, 500), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(gfx::Point(), gfx::Vector2d(500, 500)));
...
TEST_F(CommitToPendingTreeLayerTreeHostImplTest, InvalidLayerNotAddedToRasterQueue)
{
    CreatePendingTree();

    Region empty_invalidation;
    scoped_refptr<RasterSource> raster_source_with_tiles(FakeRasterSource::CreateFilled(gfx::Size(10, 10)));

    auto* layer = SetupRootLayer<FakePictureLayerImpl>(host_impl_->pending_tree(), gfx::Size(10, 10));
    layer->set_gpu_raster_max_texture_size(host_impl_->active_tree()->GetDeviceViewport().size());
    layer->SetDrawsContent(true);
    ...
    layer->tilings()->tiling_at(0)->set_resolution(TileResolution::HIGH_RESOLUTION);
    layer->tilings()->tiling_at(0)->CreateAllTilesForTesting();
    layer->tilings()->UpdateTilePriorities(gfx::Rect(gfx::Size(10, 10)), 1, 1.0, Occlusion(), true);

    layer->set_has_valid_tile_priorities(true);
    std::unique_ptr<RasterTilePriorityQueue> non_empty_raster_priority_queue_all
        = host_impl_->BuildRasterQueue(TreePriority::SAME_PRIORITY_FOR_BOTH_TREES, RasterTilePriorityQueue::Type::ALL);
...
TEST_F(CommitToPendingTreeLayerTreeHostImplTest, DidBecomeActive)
{
    ...
    auto* pending_layer = SetupRootLayer<FakePictureLayerImpl>(pending_tree, gfx::Size(10, 10));
    ...
    std::unique_ptr<FakePictureLayerImpl> mask_layer = FakePictureLayerImpl::Create(pending_tree, next_layer_id_++);
...
size_t CountRenderPassesWithId(const viz::CompositorRenderPassList& list, viz::CompositorRenderPassId id)
{
...
TEST_P(LayerTreeHostImplTest, RemoveUnreferencedRenderPass)
{
    ...
    pass1->SetNew(viz::CompositorRenderPassId { 1 }, gfx::Rect(), gfx::Rect(), gfx::Transform());
    pass2->SetNew(viz::CompositorRenderPassId { 2 }, gfx::Rect(), gfx::Rect(), gfx::Transform());
    pass3->SetNew(viz::CompositorRenderPassId { 3 }, gfx::Rect(), gfx::Rect(), gfx::Transform());
    ...
    auto* rpdq = pass2->CreateAndAppendDrawQuad<viz::CompositorRenderPassDrawQuad>();
    ...
    EXPECT_EQ(1u, frame.render_passes.size());
    EXPECT_EQ(1u, CountRenderPassesWithId(frame.render_passes, viz::CompositorRenderPassId { 1u }));
    EXPECT_EQ(0u, CountRenderPassesWithId(frame.render_passes, viz::CompositorRenderPassId { 2u }));
    EXPECT_EQ(0u, CountRenderPassesWithId(frame.render_passes, viz::CompositorRenderPassId { 3u }));
...
TEST_P(LayerTreeHostImplTest, RemoveEmptyRenderPass)
{
    ...
    EXPECT_EQ(viz::DrawQuad::Material::kSolidColor, pass1->quad_list.ElementAt(0)->material);
}
...
public:
    ...
    bool did_draw_frame() const
    {
        return did_draw_frame_;
    }
...
TEST_P(LayerTreeHostImplTest, AddVideoFrameControllerOutsideFrame)
{
    ...
    begin_frame_args = viz::CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 3);
    EXPECT_FALSE(controller.begin_frame_args().IsValid());
    host_impl_->WillBeginImplFrame(begin_frame_args);
    EXPECT_TRUE(controller.begin_frame_args().IsValid());
...
class GpuRasterizationDisabledLayerTreeHostImplTest : public LayerTreeHostImplTest {
public:
    std::unique_ptr<LayerTreeFrameSink> CreateLayerTreeFrameSink() override
    {
        return FakeLayerTreeFrameSink::Create3d();
...
class MsaaIsSlowLayerTreeHostImplTest : public CommitToActiveTreeLayerTreeHostImplTest {
public:
    void CreateHostImplWithCaps(bool msaa_is_slow, bool avoid_stencil_buffers)
    {
        LayerTreeSettings settings = DefaultSettings();
        settings.gpu_rasterization_msaa_sample_count = 4;
        auto frame_sink = FakeLayerTreeFrameSink::Builder()
                              .AllContexts(&viz::TestRasterInterface::set_msaa_is_slow, msaa_is_slow)
                              .AllContexts(&viz::TestRasterInterface::set_gpu_rasterization, true)
                              .AllContexts(&viz::TestRasterInterface::set_avoid_stencil_buffers, avoid_stencil_buffers)
                              .Build();
...
TEST_F(CommitToPendingTreeLayerTreeHostImplTest, UpdatePageScaleFactorOnActiveTree)
{
    ...
    TransformNode* active_tree_node = host_impl_->active_tree()->PageScaleTransformNode();
    ...
    EXPECT_EQ(2, host_impl_->active_tree()->property_trees()->transform_tree().page_scale_factor());

    TransformNode* pending_tree_node = host_impl_->pending_tree()->PageScaleTransformNode();
    ...
    EXPECT_EQ(2, host_impl_->pending_tree()->property_trees()->transform_tree().page_scale_factor());
...
TEST_P(LayerTreeHostImplTest, SubLayerScaleForNodeInSubtreeOfPageScaleLayer)
{
    ...
    in_subtree_of_page_scale_layer->SetTransformTreeIndex(host_impl_->active_tree()->PageScaleTransformNode()->id);
    CreateEffectNode(in_subtree_of_page_scale_layer).render_surface_reason = RenderSurfaceReason::kTest;
...
TEST_P(LayerTreeHostImplTest, RecomputeGpuRasterOnLayerTreeFrameSinkChange)
{
    ...
    host_impl_ = LayerTreeHostImpl::Create(DefaultSettings(), this, &task_runner_provider_, &stats_instrumentation_, &task_graph_runner_,
        AnimationHost::CreateForTesting(ThreadInstance::kImpl), nullptr, 0, nullptr, nullptr);
    InputHandler::Create(static_cast<CompositorDelegateForInput&>(*host_impl_));
    host_impl_->SetVisible(true);

    // InitializeFrameSink with a gpu-raster enabled output surface.
    auto gpu_raster_layer_tree_frame_sink = FakeLayerTreeFrameSink::Create3dForGpuRasterization();
...
void LayerTreeHostImplTestBase::SetupMouseMoveAtTestScrollbarStates(bool main_thread_scrolling)
{
    LayerTreeSettings settings = DefaultSettings();
    settings.scrollbar_fade_delay = base::Milliseconds(500);
    settings.scrollbar_fade_duration = base::Milliseconds(300);
    ...
    gfx::Size content_size(1000, 1000);
    gfx::Size child_layer_size(250, 150);
    gfx::Size scrollbar_size_1(gfx::Size(thumb_thickness, viewport_size.height()));
    gfx::Size scrollbar_size_2(gfx::Size(thumb_thickness, child_layer_size.height()));
    ...
    if (main_thread_scrolling) {
        GetScrollNode(root_scroll)->main_thread_repaint_reasons = MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects;
    }

    // scrollbar_1 on root scroll.
    auto* scrollbar_1 = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::kVertical, thumb_thickness, 0, true);
    SetupScrollbarLayer(root_scroll, scrollbar_1);
    scrollbar_1->SetBounds(scrollbar_size_1);
    TouchActionRegion touch_action_region;
    touch_action_region.Union(TouchAction::kNone, gfx::Rect(scrollbar_size_1));
    ...
    ScrollbarAnimationController* scrollbar_1_animation_controller = host_impl_->ScrollbarAnimationControllerForElementId(root_scroll->element_id());
    EXPECT_TRUE(scrollbar_1_animation_controller);

    const float kMouseMoveDistanceToTriggerFadeIn
        = scrollbar_1_animation_controller->GetScrollbarAnimationController(ScrollbarOrientation::kVertical).MouseMoveDistanceToTriggerFadeIn();

    const float kMouseMoveDistanceToTriggerExpand
        = scrollbar_1_animation_controller->GetScrollbarAnimationController(ScrollbarOrientation::kVertical).MouseMoveDistanceToTriggerExpand();

    // Mouse moves close to the scrollbar, goes over the scrollbar, and
    // moves back to where it was.
    GetInputHandler().MouseMoveAt(gfx::Point(thumb_thickness + kMouseMoveDistanceToTriggerFadeIn + 1, 0));
    EXPECT_FALSE(scrollbar_1_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::kVertical));
    EXPECT_FALSE(scrollbar_1_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::kVertical));
    EXPECT_FALSE(scrollbar_1_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::kVertical));

    GetInputHandler().MouseMoveAt(gfx::Point(thumb_thickness + kMouseMoveDistanceToTriggerExpand + 1, 0));
    EXPECT_TRUE(scrollbar_1_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::kVertical));
    EXPECT_FALSE(scrollbar_1_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::kVertical));
    EXPECT_FALSE(scrollbar_1_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::kVertical));

    GetInputHandler().MouseMoveAt(gfx::Point(thumb_thickness + kMouseMoveDistanceToTriggerExpand, 0));
    EXPECT_TRUE(scrollbar_1_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::kVertical));
    EXPECT_TRUE(scrollbar_1_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::kVertical));
    EXPECT_FALSE(scrollbar_1_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::kVertical));

    GetInputHandler().MouseMoveAt(gfx::Point(10, 0));
    EXPECT_TRUE(scrollbar_1_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::kVertical));
    EXPECT_TRUE(scrollbar_1_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::kVertical));
    EXPECT_TRUE(scrollbar_1_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::kVertical));
    ...
    // scrollbar_2 on child.
    auto* scrollbar_2 = AddLayer<SolidColorScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::kVertical, thumb_thickness, 0, true);
    LayerImpl* child = AddScrollableLayer(root_scroll, gfx::Size(100, 100), child_layer_size);
    child->SetOffsetToTransformParent(gfx::Vector2dF(50, 50));
    if (main_thread_scrolling) {
        GetScrollNode(child)->main_thread_repaint_reasons = MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects;
    }
    ...
    ScrollbarAnimationController* scrollbar_2_animation_controller = host_impl_->ScrollbarAnimationControllerForElementId(child->element_id());
    ...
    EXPECT_FALSE(scrollbar_1_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::kVertical));
    EXPECT_FALSE(scrollbar_1_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::kVertical));
    EXPECT_TRUE(scrollbar_2_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::kVertical));
    EXPECT_TRUE(scrollbar_2_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::kVertical));
    EXPECT_TRUE(scrollbar_2_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::kVertical));

    GetInputHandler().MouseMoveAt(gfx::Point(50 + thumb_thickness + kMouseMoveDistanceToTriggerExpand, 50));
    EXPECT_FALSE(scrollbar_1_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::kVertical));
    EXPECT_FALSE(scrollbar_1_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::kVertical));
    EXPECT_FALSE(scrollbar_1_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::kVertical));
    EXPECT_TRUE(scrollbar_2_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::kVertical));
    EXPECT_TRUE(scrollbar_2_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::kVertical));
    EXPECT_FALSE(scrollbar_2_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::kVertical));
    GetInputHandler().MouseMoveAt(gfx::Point(thumb_thickness + kMouseMoveDistanceToTriggerExpand, 0));
    EXPECT_TRUE(scrollbar_1_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::kVertical));
    EXPECT_TRUE(scrollbar_1_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::kVertical));
    EXPECT_FALSE(scrollbar_1_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::kVertical));
    EXPECT_FALSE(scrollbar_2_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::kVertical));
    EXPECT_FALSE(scrollbar_2_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::kVertical));
    EXPECT_FALSE(scrollbar_2_animation_controller->MouseIsOverScrollbarThumb(ScrollbarOrientation::kVertical));
    GetInputHandler().MouseMoveAt(gfx::Point(10, 0));
    EXPECT_TRUE(scrollbar_1_animation_controller->MouseIsNearScrollbar(ScrollbarOrientation::kVertical));
    EXPECT_TRUE(scrollbar_1_animation_controller->MouseIsNearScrollbarThumb(ScrollbarOrientation::kVertical));
    ...
    GetInputHandler().MouseDown(gfx::PointF(40, 150),
        /*jump_key_modifier*/ false);
    host_impl_->DidUnregisterScrollbarLayer(root_scroll->element_id(), ScrollbarOrientation::kVertical);
    GetInputHandler().MouseUp(gfx::PointF(40, 150));
}

TEST_P(LayerTreeHostImplTest, LayerTreeHostImplTestScrollbarStatesInMainThreadScrolling)
{
    SetupMouseMoveAtTestScrollbarStates(true);
}

TEST_P(LayerTreeHostImplTest, LayerTreeHostImplTestScrollbarStatesInNotMainThreadScrolling)
{
    SetupMouseMoveAtTestScrollbarStates(false);
}

TEST_F(CommitToPendingTreeLayerTreeHostImplTest, CheckerImagingTileInvalidation)
{
    ...
    PaintImage checkerable_image
        = PaintImageBuilder::WithCopy(CreateDiscardablePaintImage(gfx::Size(500, 500))).set_decoding_mode(PaintImage::DecodingMode::kAsync).TakePaintImage();
    recording_source.add_draw_image(checkerable_image, gfx::Point(0, 0));

    SkColor non_solid_color = SkColorSetARGB(128, 45, 56, 67);
    PaintFlags non_solid_flags;
    non_solid_flags.setColor(non_solid_color);
    recording_source.add_draw_rect_with_flags(gfx::Rect(510, 0, 200, 600), non_solid_flags);
    recording_source.add_draw_rect_with_flags(gfx::Rect(0, 510, 200, 400), non_solid_flags);
    recording_source.Rerecord();
    scoped_refptr<RasterSource> raster_source = recording_source.CreateRasterSource();
    ...
    LayerTreeImpl* pending_tree = host_impl_->pending_tree();
    auto* root = SetupRootLayer<FakePictureLayerImpl>(pending_tree, layer_size, raster_source);
    root->SetDrawsContent(true);
    ...
    const auto expected_invalidation = ImageRectsToRegion(root->discardable_image_map()->GetRectsForImage(checkerable_image.stable_id()));
...
TEST_P(LayerTreeHostImplTest, RasterColorSpace)
{
    ...
    auto wcg_params = host_impl_->GetTargetColorParams(gfx::ContentColorUsage::kWideColorGamut);
    EXPECT_EQ(wcg_params.color_space, gfx::ColorSpace::CreateSRGB());

    // The raster color space should update with tree activation.
    host_impl_->active_tree()->SetDisplayColorSpaces(gfx::DisplayColorSpaces(gfx::ColorSpace::CreateDisplayP3D65()));
    wcg_params = host_impl_->GetTargetColorParams(gfx::ContentColorUsage::kWideColorGamut);
    EXPECT_EQ(wcg_params.color_space, gfx::ColorSpace::CreateDisplayP3D65());
    EXPECT_EQ(wcg_params.sdr_max_luminance_nits, gfx::ColorSpace::kDefaultSDRWhiteLevel);
}
...
TEST_P(LayerTreeHostImplTest, RasterColorPrefersSRGB)
{
    ...
    auto srgb_params = host_impl_->GetTargetColorParams(gfx::ContentColorUsage::kSRGB);
    ...
    EXPECT_EQ(srgb_params.sdr_max_luminance_nits, gfx::ColorSpace::kDefaultSDRWhiteLevel);
}
...
TEST_P(LayerTreeHostImplTest, RasterColorSpaceHDR)
{
    ...
    const auto srgb_params = host_impl_->GetTargetColorParams(gfx::ContentColorUsage::kSRGB);
    const auto wcg_params = host_impl_->GetTargetColorParams(gfx::ContentColorUsage::kWideColorGamut);
    const auto hdr_params = host_impl_->GetTargetColorParams(gfx::ContentColorUsage::kHDR);
...
TEST_F(CommitToPendingTreeLayerTreeHostImplTest, UpdatedTilingsForNonDrawingLayers)
{
    gfx::Size layer_bounds(500, 500);

    CreatePendingTree();
    auto* root = SetupRootLayer<LayerImpl>(host_impl_->pending_tree(), layer_bounds);

    scoped_refptr<FakeRasterSource> raster_source(FakeRasterSource::CreateFilled(layer_bounds));
    auto* animated_transform_layer = AddLayer<FakePictureLayerImpl>(host_impl_->pending_tree(), raster_source);
    ...
    AddAnimatedTransformToElementWithAnimation(animated_transform_layer->element_id(), timeline(), 10.0, start_transform_operations, end_transform_operations);
    ...
    EXPECT_FALSE(animated_transform_layer->tilings()->tiling_at(0)->can_require_tiles_for_activation());
}

TEST_F(CommitToPendingTreeLayerTreeHostImplTest, RasterTilePrioritizationForNonDrawingLayers)
{
    gfx::Size layer_bounds(500, 500);
    CreatePendingTree();
    auto* root = SetupRootLayer<LayerImpl>(host_impl_->pending_tree(), layer_bounds);
    ...
    auto* hidden_layer = AddLayer<FakePictureLayerImpl>(host_impl_->pending_tree(), raster_source);
    ...
    auto* drawing_layer = AddLayer<FakePictureLayerImpl>(host_impl_->pending_tree(), raster_source);
    ...
    hidden_tiling->SetTilePriorityRectsForTesting(layer_rect, // Visible rect.
    ...
    drawing_tiling->SetTilePriorityRectsForTesting(layer_rect, // Visible rect.
        layer_rect, // Skewport rect.
        layer_rect, // Soon rect.
        layer_rect); // Eventually rect.
    ...
    std::unique_ptr<RasterTilePriorityQueue> queue = host_impl_->BuildRasterQueue(TreePriority::SMOOTHNESS_TAKES_PRIORITY, RasterTilePriorityQueue::Type::ALL);
    ...
    queue = host_impl_->BuildRasterQueue(TreePriority::SMOOTHNESS_TAKES_PRIORITY, RasterTilePriorityQueue::Type::ALL);
    EXPECT_EQ(queue->Top().tile()->layer_id(), 3);
}

TEST_F(CommitToPendingTreeLayerTreeHostImplTest, DrawAfterDroppingTileResources)
{
    LayerTreeSettings settings = DefaultSettings();
    ...
    auto* root = SetupRootLayer<FakePictureLayerImpl>(host_impl_->pending_tree(), bounds, raster_source);
    root->SetDrawsContent(true);
    host_impl_->ActivateSyncTree();

    FakePictureLayerImpl* layer = static_cast<FakePictureLayerImpl*>(host_impl_->active_tree()->FindActiveTreeLayerById(root->id()));
...
public:
    ...
    TestRenderFrameMetadataObserver(const TestRenderFrameMetadataObserver&) = delete;
    ~TestRenderFrameMetadataObserver() override
    {
    }

    TestRenderFrameMetadataObserver& operator=(const TestRenderFrameMetadataObserver&) = delete;

    void BindToCurrentSequence() override
    {
    }
    void OnRenderFrameSubmission(
        const RenderFrameMetadata& render_frame_metadata, viz::CompositorFrameMetadata* compositor_frame_metadata, bool force_send) override
    {
...
TEST_P(LayerTreeHostImplTest, SelectionBoundsPassedToRenderFrameMetadata)
{
    ...
    const viz::Selection<gfx::SelectionBound>& selection_1 = observer_ptr->last_metadata()->selection;
    ...
    ASSERT_TRUE(observer_ptr->last_metadata());
    const viz::Selection<gfx::SelectionBound>& selection_2 = observer_ptr->last_metadata()->selection;
...
TEST_P(LayerTreeHostImplTest, VerticalScrollDirectionChangesPassedToRenderFrameMetadata)
{
    // Set up the viewport.
    SetupViewportLayersInnerScrolls(gfx::Size(50, 50), gfx::Size(100, 100));
    host_impl_->active_tree()->SetDeviceViewportRect(gfx::Rect(50, 50));
    ...
    test_legs.push_back({ /*scroll_delta=*/gfx::Vector2d(),
        /*expected_vertical_scroll_direction=*/viz::VerticalScrollDirection::kNull });
    ...
    test_legs.push_back({ /*scroll_delta=*/gfx::Vector2d(0, 10),
        /*expected_vertical_scroll_direction=*/viz::VerticalScrollDirection::kDown });
    ...
    test_legs.push_back({ /*scroll_delta=*/gfx::Vector2d(0, -10),
        /*expected_vertical_scroll_direction=*/viz::VerticalScrollDirection::kUp });
    ...
    for (auto& test_leg : test_legs) {
        ...
        if (!test_leg.scroll_delta.IsZero()) {
            GetInputHandler().ScrollBegin(BeginState(gfx::Point(), test_leg.scroll_delta, ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
            GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), test_leg.scroll_delta, ui::ScrollInputType::kWheel));
        }
        ...
        EXPECT_EQ(test_leg.expected_vertical_scroll_direction, observer_ptr->last_metadata()->new_vertical_scroll_direction);
...
TEST_P(LayerTreeHostImplTest, ScrollUpdateDoesNotSetScrollingNode)
{
    ...
    EXPECT_EQ(ScrollThread::kScrollOnImplThread,
        GetInputHandler()
            .ScrollBegin(BeginState(gfx::Point(), gfx::Vector2dF(), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen)
            .thread);
...
class HitTestRegionListGeneratingLayerTreeHostImplTest : public LayerTreeHostImplTest {
public:
    bool CreateHostImpl(const LayerTreeSettings& settings, std::unique_ptr<LayerTreeFrameSink> layer_tree_frame_sink) override
    {
        // Enable hit test data generation with the CompositorFrame.
        LayerTreeSettings new_settings = settings;
        return LayerTreeHostImplTest::CreateHostImpl(new_settings, std::move(layer_tree_frame_sink));
    }
};

INSTANTIATE_COMMIT_TO_TREE_TEST_P(HitTestRegionListGeneratingLayerTreeHostImplTest);
...
TEST_P(HitTestRegionListGeneratingLayerTreeHostImplTest, BuildHitTestData)
{
    ...
    viz::LocalSurfaceId child_local_surface_id(2, base::UnguessableToken::Create());
    viz::FrameSinkId frame_sink_id(2, 0);
    viz::SurfaceId child_surface_id(frame_sink_id, child_local_surface_id);
    surface_child1->SetRange(viz::SurfaceRange(std::nullopt, child_surface_id), std::nullopt);
    surface_child2->SetRange(viz::SurfaceRange(std::nullopt, child_surface_id), std::nullopt);
    ...
    UpdateDrawProperties(host_impl_->active_tree());
    draw_property_utils::ComputeEffects(&host_impl_->active_tree()->property_trees()->effect_tree_mutable());

    constexpr gfx::Rect kFrameRect(0, 0, 1024, 768);

    std::optional<viz::HitTestRegionList> hit_test_region_list = host_impl_->BuildHitTestData();
    ...
    uint32_t expected_flags = viz::HitTestRegionFlags::kHitTestMouse | viz::HitTestRegionFlags::kHitTestTouch | viz::HitTestRegionFlags::kHitTestMine;
    ...
    EXPECT_EQ(child_surface_id.frame_sink_id(), hit_test_region_list->regions[1].frame_sink_id);
    expected_flags = viz::HitTestRegionFlags::kHitTestMouse | viz::HitTestRegionFlags::kHitTestTouch | viz::HitTestRegionFlags::kHitTestChildSurface;
    ...
    EXPECT_TRUE(child1_transform.ApproximatelyEqual(hit_test_region_list->regions[1].transform));
    EXPECT_EQ(gfx::Rect(0, 0, 100, 100), hit_test_region_list->regions[1].rect);

    EXPECT_EQ(child_surface_id.frame_sink_id(), hit_test_region_list->regions[0].frame_sink_id);
    expected_flags = viz::HitTestRegionFlags::kHitTestMouse | viz::HitTestRegionFlags::kHitTestTouch | viz::HitTestRegionFlags::kHitTestChildSurface
    ...
    EXPECT_TRUE(child2_transform.ApproximatelyEqual(hit_test_region_list->regions[0].transform));
...
TEST_P(HitTestRegionListGeneratingLayerTreeHostImplTest, PointerEvents)
{
    ...
    EXPECT_EQ(expected_flags, hit_test_region_list->regions[0].flags);
    gfx::Transform child1_transform;
    EXPECT_TRUE(child1_transform.ApproximatelyEqual(hit_test_region_list->regions[0].transform));
    EXPECT_EQ(gfx::Rect(0, 0, 100, 100), hit_test_region_list->regions[0].rect);
}
...
TEST_P(HitTestRegionListGeneratingLayerTreeHostImplTest, ComplexPage)
{
    ...
    viz::FrameSinkId frame_sink_id(2, 0);
    viz::SurfaceId child_surface_id(frame_sink_id, child_local_surface_id);
    surface_child->SetRange(viz::SurfaceRange(std::nullopt, child_surface_id), std::nullopt);
...
TEST_P(HitTestRegionListGeneratingLayerTreeHostImplTest, InvalidFrameSinkId)
{
    ...
    surface_child2->SetRange(viz::SurfaceRange(std::nullopt, viz::SurfaceId()), std::nullopt);
...
TEST_P(LayerTreeHostImplTest, SkipOnDrawDoesNotUpdateDrawParams)
{
    ...
    host_impl_->OnDraw(transform, viewport, resourceless_software_draw, skip_draw);
    ...
    host_impl_->OnDraw(new_transform, new_viewport, resourceless_software_draw, skip_draw);
    EXPECT_EQ(transform, host_impl_->DrawTransform());
    EXPECT_EQ(viewport, host_impl_->active_tree()->GetDeviceViewport());
}
...
TEST_P(LayerTreeHostImplTest, TouchScrollOnAndroidScrollbar)
{
    ...
    SetupViewportLayersNoScrolls(viewport_size);
    LayerImpl* content = AddScrollableLayer(OuterViewportScrollLayer(), viewport_size, scroll_content_size);

    auto* scrollbar = AddLayer<SolidColorScrollbarLayerImpl>(layer_tree_impl, ScrollbarOrientation::kVertical, 10, 0, false);
    ...
    InputHandler::ScrollStatus status = GetInputHandler().ScrollBegin(
        BeginState(gfx::Point(350, 50), gfx::Vector2dF(0, 10), ui::ScrollInputType::kTouchscreen).get(), ui::ScrollInputType::kTouchscreen);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
}

TEST_F(CommitToPendingTreeLayerTreeHostImplTest, CommitWithNoPaintWorkletLayerPainter)
{
...
{
    host_impl_->SetPaintWorkletLayerPainter(std::make_unique<TestPaintWorkletLayerPainter>());
    host_impl_->CreatePendingTree();
...
TEST_F(CommitToPendingTreeLayerTreeHostImplTest, CommitWithDirtyPaintWorklets)
{
    ...
    // PaintWorklets.
    host_impl_->CreatePendingTree();
    auto* root = SetupRootLayer<PictureLayerImpl>(host_impl_->pending_tree(), gfx::Size(100, 100));
    root->SetNeedsPushProperties();

    // Add a PaintWorkletInput to the PictureLayerImpl.
    scoped_refptr<RasterSource> raster_source_with_pws(FakeRasterSource::CreateFilledWithPaintWorklet(root->bounds()));
    Region empty_invalidation;
    ...
    scoped_refptr<const PaintWorkletInput> input = root->GetPaintWorkletRecordMap().begin()->first;
    ...
    EXPECT_TRUE(root->GetPaintWorkletRecordMap().find(input)->second.second->EqualsForTesting(record));
    EXPECT_TRUE(did_prepare_tiles_);
}

TEST_F(CommitToPendingTreeLayerTreeHostImplTest, CommitWithNoDirtyPaintWorklets)
{
    host_impl_->SetPaintWorkletLayerPainter(std::make_unique<TestPaintWorkletLayerPainter>());
    ...
    ASSERT_EQ(root->GetPaintWorkletRecordMap().size(), 1u);
    root->SetPaintWorkletRecord(root->GetPaintWorkletRecordMap().begin()->first, PaintRecord());
...
class ForceActivateAfterPaintWorkletPaintLayerTreeHostImplTest : public CommitToPendingTreeLayerTreeHostImplTest {
public:
    void NotifyPaintWorkletStateChange(Scheduler::PaintWorkletState state) override
    {
...
TEST_F(ForceActivateAfterPaintWorkletPaintLayerTreeHostImplTest, ForceActivationAfterPaintWorkletsFinishPainting)
{
    auto painter_owned = std::make_unique<TestPaintWorkletLayerPainter>();
    TestPaintWorkletLayerPainter* painter = painter_owned.get();
    host_impl_->SetPaintWorkletLayerPainter(std::move(painter_owned));
...
TEST_P(LayerTreeHostImplTest, PercentBasedScrollbarDeltasDSF3)
{
    ...
    LayerImpl* content_layer = AddContentLayer();
    LayerImpl* scroll_layer = AddScrollableLayer(content_layer, gfx::Size(185, 500), gfx::Size(185, 3800));

    auto* scrollbar = AddLayer<PaintedScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::kVertical, false, true);
    SetupScrollbarLayer(scroll_layer, scrollbar);
    ...
    scrollbar->SetForwardButtonRect(gfx::Rect(gfx::Point(0, 485), gfx::Size(15, 15)));
    ...
    InputHandlerPointerResult scroll_result = GetInputHandler().MouseDown(gfx::PointF(190, 490), false);
    ...
    InputHandlerPointerResult scroll_with_dsf_1 = GetInputHandler().MouseDown(gfx::PointF(190, 490), false);
    GetInputHandler().MouseUp(gfx::PointF(190, 490));
...
TEST_P(LayerTreeHostImplTest, PageBasedScroll)
{
    ...
    auto begin_state = BeginState(gfx::Point(), kPageDelta, ui::ScrollInputType::kWheel);
    begin_state->data()->delta_granularity = ui::ScrollGranularity::kScrollByPage;
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, GetInputHandler().ScrollBegin(begin_state.get(), ui::ScrollInputType::kWheel).thread);

    auto update_state = UpdateState(gfx::Point(), kPageDelta, ui::ScrollInputType::kWheel);
    ...
    BeginImplFrameAndAnimate(begin_frame_args, start_time);
    BeginImplFrameAndAnimate(begin_frame_args, start_time + base::Milliseconds(50));
    BeginImplFrameAndAnimate(begin_frame_args, start_time + base::Milliseconds(2000));

    const gfx::PointF kExpectedOffset(
        kPageDelta.x() * kViewportSize.width() * kMinFractionToStepWhenPaging, kPageDelta.y() * kViewportSize.height() * kMinFractionToStepWhenPaging);
    const gfx::PointF kCurrentOffset
        = host_impl_->active_tree()->property_trees()->scroll_tree().current_scroll_offset(host_impl_->OuterViewportScrollNode()->element_id);
...
public:
    ...
    void SetUp() override
    {
        ...
        begin_frame_args_ = viz::CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 1);
    ...
    void CreateNonCompositedScrollerAndMainThreadScrollHitTestRegion()
    {
        ...
        CreateScrollNodeForNonCompositedScroller(
            GetPropertyTrees(), host_impl_->OuterViewportScrollNode()->id, ScrollerElementId(), scrollable_content_bounds, container_bounds);
        OuterViewportScrollLayer()->SetMainThreadScrollHitTestRegion(gfx::Rect(50, 50));
    ...
    void CreateScroller(uint32_t main_thread_repaint_reasons, HitTestOpaqueness hit_test_opaqueness = HitTestOpaqueness::kOpaque)
    {
        ...
        LayerImpl* layer = AddScrollableLayer(OuterViewportScrollLayer(), container_bounds, scrollable_content_bounds);
        layer->SetHitTestOpaqueness(hit_test_opaqueness);
        scroller_layer_ = layer;
        GetScrollNode(layer)->main_thread_repaint_reasons = main_thread_repaint_reasons;
    ...
    void CreateLayerCoveringWholeViewport(const LayerImpl* parent_scroller, HitTestOpaqueness opaqueness)
    {
        LayerImpl* layer = AddLayerInActiveTree();
    ...
    void CreateLayerCoveringWholeViewportEscapingScrollers(HitTestOpaqueness opaqueness)
    {
    ...
    void CreateLayerCoveringWholeViewportInScroller(HitTestOpaqueness opaqueness)
    {
    ...
    {
        auto scroll_state = BeginState(gfx::Point(25, 25), delta, ui::ScrollInputType::kWheel);
        ScrollStatus status = GetInputHandler().ScrollBegin(scroll_state.get(), ui::ScrollInputType::kWheel);
    ...
    {
        DCHECK(to_be_continued_scroll_begin_) << "ContinuedScrollBegin needs to come after a ScrollBegin that "
                                                 "requested a main frame";
        std::unique_ptr<ScrollState> scroll_state = std::move(to_be_continued_scroll_begin_);

        scroll_state->data()->set_current_native_scrolling_element(element_id);
        scroll_state->data()->main_thread_hit_tested_reasons = MainThreadScrollingReason::kFailedHitTest;

        return GetInputHandler().ScrollBegin(scroll_state.get(), ui::ScrollInputType::kWheel);
    }

    InputHandlerScrollResult ScrollUpdate(const gfx::Vector2d& delta)
    {
        auto scroll_state = UpdateState(gfx::Point(25, 25), delta, ui::ScrollInputType::kWheel);
    ...
    void ScrollEnd()
    {
        return GetInputHandler().ScrollEnd();
    }
    ...
    {
        return GetPropertyTrees()->scroll_tree().current_scroll_offset(node->element_id);
    }

    gfx::PointF ScrollerOffset()
    {
        return GetPropertyTrees()->scroll_tree().current_scroll_offset(ScrollerElementId());
    }
    ...
    {
        ScrollNode* node = GetPropertyTrees()->scroll_tree_mutable().FindNodeFromElementId(ScrollerElementId());
        DCHECK(node);
        return node;
    }
    LayerImpl* ScrollerLayer() const
    {
        return scroller_layer_.get();
    }
...
TEST_P(UnifiedScrollingTest, NonCompositedScrollOnCompositor)
{
    ...
    CreateScrollNodeForNonCompositedScroller(
        GetPropertyTrees(), host_impl_->OuterViewportScrollNode()->id, ScrollerElementId(), scrollable_content_bounds, container_bounds);
    OuterViewportScrollLayer()->SetNonCompositedScrollHitTestRects({ ScrollHitTestRect { ScrollerElementId(), gfx::Rect(container_bounds) } });
...
TEST_P(UnifiedScrollingTest, LayerMixedHitTestOpaquenessCausesMainThreadHitTest)
{
...
TEST_P(UnifiedScrollingTest, LayerMixedHitTestOpaquenessCausesMainThreadHitTest2)
{
    CreateScroller(MainThreadScrollingReason::kNotScrollingOnMain);
...
{
    CreateScroller(MainThreadScrollingReason::kNotScrollingOnMain);
    CreateLayerCoveringWholeViewport(InnerViewportScrollLayer(), HitTestOpaqueness::kOpaque);
...
TEST_P(UnifiedScrollingTest, LayerOpaqueToHitTestEscapingScrollersWithMixedToHitTestLayers)
{
    CreateScroller(MainThreadScrollingReason::kNotScrollingOnMain);
    CreateLayerCoveringWholeViewportInScroller(HitTestOpaqueness::kMixed);
    CreateLayerCoveringWholeViewportEscapingScrollers(HitTestOpaqueness::kOpaque);
...
TEST_P(UnifiedScrollingTest, ReliableScrollHitTestWithOpaqueAndMixedToHitTestLayers)
{
    CreateScroller(MainThreadScrollingReason::kNotScrollingOnMain);
    CreateLayerCoveringWholeViewportInScroller(HitTestOpaqueness::kMixed);
...
{
    CreateScroller(MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects);
...
{
    CreateScroller(MainThreadScrollingReason::kNotScrollingOnMain, HitTestOpaqueness::kMixed);
...
{
    CreateScroller(MainThreadScrollingReason::kNotScrollingOnMain, HitTestOpaqueness::kMixed);
    auto* scrollbar_layer = AddLayer<PaintedScrollbarLayerImpl>(host_impl_->active_tree(), ScrollbarOrientation::kVertical, false, true);
    CreateEffectNode(ScrollerLayer()).node_or_ancestor_has_fast_rounded_corner = true;
    ...
    // corner.
    auto status
        = GetInputHandler().ScrollBegin(BeginState(gfx::Point(45, 20), gfx::Vector2d(0, 10), ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);
    EXPECT_EQ(ScrollThread::kScrollOnImplThread, status.thread);
    EXPECT_EQ(MainThreadScrollingReason::kNotScrollingOnMain, status.main_thread_repaint_reasons);
    EXPECT_EQ(MainThreadScrollingReason::kFailedHitTest, status.main_thread_hit_test_reasons);
}
...
void UnifiedScrollingTest::TestNonCompositedScrollingState(bool mutates_transform_tree)
{
    ...
    TransformNode* transform_node = transform_tree.Node(ScrollerNode()->transform_id);
    ...
    // test parameter.
    {
        ...
        EXPECT_EQ(mutates_transform_tree, transform_node->needs_local_transform_update);
        ...
        if (mutates_transform_tree) {
            ...
            EXPECT_EQ(gfx::PointF(0, 10), scroll_tree.GetScrollOffsetForScrollTimeline(*ScrollerNode()));
        } else {
            EXPECT_EQ(gfx::PointF(0, 0), transform_node->scroll_offset);
            EXPECT_EQ(gfx::PointF(0, 0), scroll_tree.GetScrollOffsetForScrollTimeline(*ScrollerNode()));
        }
    }
    ...
    // Perform animated scroll update. Ensure the same things.
    {
        ...
        ASSERT_TRUE(host_impl_->mutator_host()->HasImplOnlyScrollAnimatingElement());
        ASSERT_EQ(gfx::PointF(0, 10), ScrollerOffset());
        ...
        if (mutates_transform_tree) {
            ...
            EXPECT_EQ(gfx::PointF(0, 20), scroll_tree.GetScrollOffsetForScrollTimeline(*ScrollerNode()));
        } else {
            EXPECT_EQ(gfx::PointF(0, 0), transform_node->scroll_offset);
            EXPECT_EQ(gfx::PointF(0, 0), scroll_tree.GetScrollOffsetForScrollTimeline(*ScrollerNode()));
        }
...
TEST_P(UnifiedScrollingTest, MainThreadReasonsScrollDoesntAffectTransform)
{
    ...
    ASSERT_EQ(ScrollerNode()->main_thread_repaint_reasons, MainThreadScrollingReason::kHasBackgroundAttachmentFixedObjects);
    ...
    // tree.
    {
        ScrollerNode()->main_thread_repaint_reasons = MainThreadScrollingReason::kNotScrollingOnMain;
        UpdateDrawProperties(host_impl_->active_tree());
        host_impl_->active_tree()->DidBecomeActive();
...
TEST_P(OccludedSurfaceThrottlingLayerTreeHostImplTest, ThrottleOccludedSurface)
{
    ...
    DrawFrame();
    EXPECT_EQ(host_impl_->GetFrameSinksToThrottleForTesting(), base::flat_set<viz::FrameSinkId> { end.frame_sink_id() });
}
...
TEST_P(LayerTreeHostImplTest, FrameElementIdHitTestSimple)
{
    ...
    EXPECT_EQ(GetInputHandler().FindFrameElementIdAtPoint(gfx::PointF(10, 10)), ElementId(0x10));
}
...
TEST_P(LayerTreeHostImplTest, FrameElementIdHitTestInheritance)
{
    ...
    frame_layer->SetHitTestOpaqueness(HitTestOpaqueness::kMixed);
    CopyProperties(root_layer(), frame_layer);
    CreateTransformNode(frame_layer, root_layer()->transform_tree_index()).visible_frame_element_id = ElementId(0x20);
    ...
    auto& child_node = CreateTransformNode(child_layer, frame_layer->transform_tree_index());
    ...
    EXPECT_EQ(GetInputHandler().FindFrameElementIdAtPoint(gfx::PointF(15, 15)), ElementId(0x20));

    // Ensure that hit tests on the child (non-frame) layer returns the frame
    // element id of its parent.
    EXPECT_EQ(GetInputHandler().FindFrameElementIdAtPoint(gfx::PointF(60, 60)), ElementId(0x20));
}
...
TEST_P(LayerTreeHostImplTest, FrameElementIdHitTestOverlap)
{
    ...
    auto& occluding_frame_node = CreateTransformNode(occluding_frame_layer, frame_layer->transform_tree_index());
    ...
    // overlapping.
    EXPECT_EQ(GetInputHandler().FindFrameElementIdAtPoint(gfx::PointF(15, 15)), ElementId(0x10));
    EXPECT_EQ(GetInputHandler().FindFrameElementIdAtPoint(gfx::PointF(30, 30)), ElementId(0x20));
}
...
TEST_P(LayerTreeHostImplTest, FrameElementIdHitTestOverlapSimpleClip)
{
    ...
    CreateTransformNode(clipped_frame_layer).visible_frame_element_id = ElementId(0x20);
    ...
    EXPECT_EQ(GetInputHandler().FindFrameElementIdAtPoint(gfx::PointF(30, 30)), ElementId(0x10));
}
...
TEST_P(LayerTreeHostImplTest, FrameElementIdHitTestOverlapRoundedCorners)
{
    ...
    CreateTransformNode(rounded_frame_layer, frame_layer->transform_tree_index()).visible_frame_element_id = ElementId(0x20);
    ...
    CreateEffectNode(rounded_frame_layer).mask_filter_info = gfx::MaskFilterInfo(gfx::RRectF(25, 25, 50, 50, 5));

    UpdateDrawProperties(host_impl_->active_tree());

    // The lookup should bail out in the presence of a complex clip/mask on the
    // target chain.
    EXPECT_FALSE(GetInputHandler().FindFrameElementIdAtPoint(gfx::PointF(30, 30)));
}
...
TEST_P(LayerTreeHostImplTest, FrameElementIdHitTestOverlapSibling)
{
    ...
    CreateTransformNode(sibling_frame_layer, root_layer()->transform_tree_index()).visible_frame_element_id = ElementId(0x30);
    sibling_frame_layer->SetOffsetToTransformParent(gfx::Vector2dF(25, 25));

    UpdateDrawProperties(host_impl_->active_tree());

    EXPECT_EQ(GetInputHandler().FindFrameElementIdAtPoint(gfx::PointF(15, 15)), ElementId(0x20));
    EXPECT_EQ(GetInputHandler().FindFrameElementIdAtPoint(gfx::PointF(60, 60)), ElementId(0x30));
...
TEST_P(LayerTreeHostImplTest, ViewTransitionRequestCausesDamage)
{
    ...
    host_impl_->active_tree()->AddViewTransitionRequest(
        ViewTransitionRequest::CreateAnimateRenderer(blink::ViewTransitionToken(), /*maybe_cross_frame_sink=*/false));
...
TEST_P(LayerTreeHostImplTest, CollectRegionCaptureBounds)
{
    ...
    const viz::RegionCaptureBounds kRootBounds { { { kFirstId, gfx::Rect { 0, 0, 250, 250 } }, { kSecondId, gfx::Rect {} } } };
    const viz::RegionCaptureBounds kChildBounds { { { kThirdId, gfx::Rect { 5, 6, 300, 400 } } } };
    const viz::RegionCaptureBounds kSecondChildBounds { { { kFourthId, gfx::Rect { 20, 10, 400, 500 } } } };
    ...
    child_layer->draw_properties().screen_space_transform = std::move(child_layer_transform);
    ...
    second_child_layer->draw_properties().screen_space_transform = std::move(second_layer_transform);
    ...
    const viz::RegionCaptureBounds collected_bounds = host_impl_->CollectRegionCaptureBounds();
    EXPECT_EQ(4u, collected_bounds.bounds().size());

    // Validate expectations.
    EXPECT_EQ((gfx::Rect { 0, 0, 250, 250 }), collected_bounds.bounds().find(kFirstId)->second);
    ...
    // content rect.
    EXPECT_EQ((gfx::Rect { 10, 18, 340, 342 }), collected_bounds.bounds().find(kThirdId)->second);

    // Finally, test a rotation instead of a simple scaling.
    EXPECT_EQ((gfx::Rect { 0, 21, 290, 339 }), collected_bounds.bounds().find(kFourthId)->second);
}
...
TEST_P(LayerTreeHostImplBrowserControlsTest, BrowserControlsActivelyScrollingType)
{
    ...
    // Create a content layer beneath the outer viewport scroll layer.
    scoped_refptr<FakeRasterSource> raster_source(FakeRasterSource::CreateFilled(content_size));

    auto* picture_layer = AddLayer<FakePictureLayerImpl>(host_impl_->active_tree(), raster_source);
    ...
    // shownratio == 1
    EXPECT_EQ(1, host_impl_->active_tree()->CurrentTopControlsShownRatio());
    EXPECT_EQ(host_impl_->active_tree()->GetActivelyScrollingType(), ActivelyScrollingType::kNone);

    // 0 < shownratio <1
    GetInputHandler().ScrollUpdate(UpdateState(gfx::Point(), gfx::Vector2dF(0, 25), ui::ScrollInputType::kTouchscreen));
    ...
    EXPECT_EQ(host_impl_->active_tree()->GetActivelyScrollingType(), ActivelyScrollingType::kPrecise);
...
TEST_P(LayerTreeHostImplTest, AnimatedScrollSnapStrategyCurrentOffset)
{
    ...
    GetInputHandler().ScrollBegin(BeginState(pointer_position, delta, ui::ScrollInputType::kWheel).get(), ui::ScrollInputType::kWheel);

    GetInputHandler().ScrollUpdate(AnimatedUpdateState(pointer_position, delta));

    EXPECT_NE(GetInputHandler().snap_strategy_for_testing(), nullptr);
    EXPECT_VECTOR2DF_EQ(GetInputHandler().snap_strategy_for_testing()->current_position().OffsetFromOrigin(), delta);
...
TEST_P(LayerTreeHostImplTest, AnimatedViewportScrollSnapStrategyCurrentOffset)
{
    ...
    // Start an animated scroll on the inner viewport.
    auto begin_state = BeginState(pointer_position, gfx::Vector2d(0, 60), ui::ScrollInputType::kWheel);
    GetInputHandler().ScrollBegin(begin_state.get(), ui::ScrollInputType::kWheel);
    GetInputHandler().ScrollUpdate(AnimatedUpdateState(pointer_position, scroll_delta));

    EXPECT_VECTOR2DF_EQ(GetInputHandler().snap_strategy_for_testing()->current_position().OffsetFromOrigin(),
        CurrentScrollOffset(InnerViewportScrollLayer()).OffsetFromOrigin() + CurrentScrollOffset(OuterViewportScrollLayer()).OffsetFromOrigin() + scroll_delta);
    EXPECT_POINTF_EQ(gfx::PointF(50, 50), CurrentScrollOffset(InnerViewportScrollLayer()));
    EXPECT_POINTF_EQ(gfx::PointF(5, 400), CurrentScrollOffset(OuterViewportScrollLayer()));
}

TEST_P(LayerTreeHostImplTest, NonAnimatedViewportScrollSnapStrategyCurrentOffset)
{
    const gfx::Size content_size(1000, 1000);
    const gfx::Size viewport_size(200, 200);
    SetupViewportLayersOuterScrolls(viewport_size, content_size);
    ...
    // Start an animated scroll on the inner viewport.
    auto begin_state = BeginState(pointer_position, scroll_delta, ui::ScrollInputType::kWheel);
    GetInputHandler().ScrollBegin(begin_state.get(), ui::ScrollInputType::kWheel);
    GetInputHandler().ScrollUpdate(UpdateState(pointer_position, scroll_delta, ui::ScrollInputType::kWheel));

    // NonAnimated scrolls are immediately reflected in the scroll tree.
    EXPECT_VECTOR2DF_EQ(GetInputHandler().snap_strategy_for_testing()->current_position().OffsetFromOrigin(),
        CurrentScrollOffset(InnerViewportScrollLayer()).OffsetFromOrigin() + CurrentScrollOffset(OuterViewportScrollLayer()).OffsetFromOrigin());
    // Inner viewport's delta increases by 30 (60 / |page_scale_factor|).
    EXPECT_POINTF_EQ(gfx::PointF(50, 80), CurrentScrollOffset(InnerViewportScrollLayer()));
    EXPECT_POINTF_EQ(gfx::PointF(5, 400), CurrentScrollOffset(OuterViewportScrollLayer()));
}
...
TEST_P(LayerTreeHostImplTest, FlingSnapStrategyCurrentOffset)
{
    ...
    LayerImpl* snapping_layer = AddScrollableLayer(OuterViewportScrollLayer(), viewport_size, content_size);

    SnapContainerData container(ScrollSnapType(false, SnapAxis::kY, SnapStrictness::kMandatory), gfx::RectF(0, 0, 100, 100), gfx::PointF(0, 4900));
    ScrollSnapAlign start = ScrollSnapAlign(SnapAlignment::kStart);
    container.AddSnapAreaData(SnapAreaData(start, snap_area_1, false, false, ElementId(10)));
    container.AddSnapAreaData(SnapAreaData(start, snap_area_2, false, false, ElementId(20)));
    ...
    handler.ScrollBegin(BeginState(position, gfx::Vector2dF(0, 950), type).get(), type);
    handler.ScrollUpdate(UpdateState(position, gfx::Vector2dF(0, 950), type));

    // Fling phase.
    gfx::Vector2dF fling_displacement = gfx::Vector2dF(0, 400);
    handler.GetSnapFlingInfoAndSetAnimatingSnapTarget(gfx::Vector2dF(0, 100), fling_displacement, &initial_offset, &target_offset);
    EXPECT_EQ(handler.snap_strategy_for_testing()->current_position(), initial_offset);
    // Expect to snap to snap_area_2.
    EXPECT_EQ(handler.snap_strategy_for_testing()->intended_position(), target_offset);

    // Do an inertial phase scroll update.
    auto scroll_update_state = UpdateState(position, gfx::Vector2dF(0, 100), type);
...
public:
    static std::unique_ptr<FakeLayerImpl> Create(LayerTreeImpl* tree_impl, int id)
    {
...
protected:
    FakeLayerImpl(LayerTreeImpl* tree_impl, int id)
        : LayerImpl(tree_impl, id)
    {
    }
...
public:
    void SetUp() override
    {
        scoped_feature_list_.InitAndEnableFeature(features::kMultipleImplOnlyScrollAnimations);
        LayerTreeHostImplTest::SetUp();
    }
...
gfx::PointF ConcurrentImplOnlyScrollAnimationsTest::CreateAndTickScrollAnimations()
{
    gfx::Size content_size = gfx::Size(360, 600);
    gfx::Size scroll_content_size = gfx::Size(3600, 3800);

    SetupViewportLayersNoScrolls(content_size);
    scroller1_ = AddScrollableLayer(OuterViewportScrollLayer(), content_size, scroll_content_size);
    scroller2_ = AddScrollableLayer(OuterViewportScrollLayer(), content_size, scroll_content_size);

    AnimationHost* animation_host = GetImplAnimationHost();
    animation_host->ImplOnlyScrollAnimationCreate(scroller1_->element_id(), target_offset1_, current_offset1_, base::TimeDelta(), base::TimeDelta());
    animation_host->ImplOnlyScrollAnimationCreate(scroller2_->element_id(), target_offset2_, current_offset2_, base::TimeDelta(), base::TimeDelta());

    // Bring the animations to their initial values (current_offset).
    animation_host->TickAnimations(base::TimeTicks() + base::Milliseconds(200), host_impl_->GetScrollTree(), true);
    EXPECT_EQ(host_impl_->GetScrollTree().current_scroll_offset(scroller1_->element_id()), current_offset1_);
    EXPECT_EQ(host_impl_->GetScrollTree().current_scroll_offset(scroller2_->element_id()), current_offset2_);
    ...
    animation_host->TickAnimations(base::TimeTicks() + base::Milliseconds(300), host_impl_->GetScrollTree(), true);
    EXPECT_GT(host_impl_->GetScrollTree().current_scroll_offset(scroller1_->element_id()).y(), current_offset1_.y());
    // Store the scroller2's intermediate value which might be its final value.
    gfx::PointF intermediate_scroller2_offset = host_impl_->GetScrollTree().current_scroll_offset(scroller2_->element_id());
...
void ConcurrentImplOnlyScrollAnimationsTest::CompleteScrollAnimations()
{
    ...
    animation_host->TickAnimations(base::TimeTicks() + base::Milliseconds(600), host_impl_->GetScrollTree(), true);
    // Bring the animations to a FINISHED state.
    animation_host->UpdateAnimationState(true, nullptr);

    EXPECT_FALSE(animation_host->ElementHasImplOnlyScrollAnimation(scroller1_->element_id()));
    EXPECT_FALSE(animation_host->ElementHasImplOnlyScrollAnimation(scroller2_->element_id()));

    EXPECT_EQ(host_impl_->GetScrollTree().current_scroll_offset(scroller1_->element_id()), target_offset1_);

    EXPECT_EQ(host_impl_->GetScrollTree().current_scroll_offset(scroller2_->element_id()), target_offset2_);
}
...
TEST_P(ConcurrentImplOnlyScrollAnimationsTest, Update)
{
    ...
    animation_host->ImplOnlyScrollAnimationUpdateTarget(
        update_delta1, gfx::PointF(1000., 1000.), base::TimeTicks() + base::Milliseconds(400), base::TimeDelta(), scroller1_->element_id());
    animation_host->ImplOnlyScrollAnimationUpdateTarget(
        update_delta2, gfx::PointF(1000., 1000.), base::TimeTicks() + base::Milliseconds(400), base::TimeDelta(), scroller2_->element_id());
    EXPECT_TRUE(animation_host->ElementHasImplOnlyScrollAnimation(scroller1_->element_id()));
    EXPECT_TRUE(animation_host->ElementHasImplOnlyScrollAnimation(scroller2_->element_id()));
    CompleteScrollAnimations();
}
...
TEST_P(ConcurrentImplOnlyScrollAnimationsTest, RemovedByCommit)
{
    ...
    host_impl_->active_tree()->property_trees()->scroll_tree_mutable().RemoveNodes(1);
    host_impl_->active_tree()->property_trees()->transform_tree_mutable().RemoveNodes(1);
...
public:
    void SetUp() override
    {
        ...
        snapping_layer1_ = AddScrollableLayer(OuterViewportScrollLayer(), viewport_size, content_size);

        snapping_layer2_ = AddScrollableLayer(OuterViewportScrollLayer(), viewport_size, content_size);
        SnapContainerData container1(ScrollSnapType(false, SnapAxis::kY, SnapStrictness::kMandatory), gfx::RectF(0, 0, 100, 100), gfx::PointF(0, 4900));
        SnapContainerData container2(ScrollSnapType(false, SnapAxis::kY, SnapStrictness::kMandatory), gfx::RectF(100, 100, 100, 100), gfx::PointF(0, 4900));

        // Add snap areas to both snap containers.
        ScrollSnapAlign start_alignment = ScrollSnapAlign(SnapAlignment::kStart);
        container1.AddSnapAreaData(SnapAreaData(start_alignment, snap_area_1, false, false, ElementId(10)));
        container1.AddSnapAreaData(SnapAreaData(start_alignment, snap_area_2, false, false, ElementId(20)));
        container2.AddSnapAreaData(SnapAreaData(start_alignment, snap_area_1, false, false, ElementId(30)));
        container2.AddSnapAreaData(SnapAreaData(start_alignment, snap_area_2, false, false, ElementId(40)));
...
TEST_P(ConcurrentSnapAnimationsTest, TrackAnimatingSnapTargetIds)
{
    ...
    gfx::Point position(50, 50);
    ui::ScrollInputType type = ui::ScrollInputType::kTouchscreen;
    base::flat_map<ElementId, InputHandler::SnapAnimationData>& snap_state_map = handler.get_snap_animation_data_map_for_testing();
    EXPECT_FALSE(snap_state_map.contains(container1_id_));
    EXPECT_FALSE(snap_state_map.contains(container2_id_));

    handler.ScrollBegin(BeginState(position, gfx::Vector2dF(0, 150), type).get(), type);
    ...
    EXPECT_EQ(snap_state_map.at(container1_id_).animating_snap_target_ids_.y, ElementId(10));
    EXPECT_FALSE(snap_state_map.contains(container2_id_));
    ...
    EXPECT_EQ(snap_state_map.at(container2_id_).animating_snap_target_ids_.y, ElementId(30));
    ...
    while (animation_host->HasImplOnlyScrollAnimatingElement()) {
        animation_host->TickAnimations(base::TimeTicks() + base::Milliseconds(t++ * 100), host_impl_->GetScrollTree(), true);
        animation_host->UpdateAnimationState(true, nullptr);
    }
