...
class LayerTreeHostReadbackPixelTest : public LayerTreePixelTest, public testing::WithParamInterface<ReadbackTestConfig> {
protected:
    ...
    {
        return GetParam().renderer_type;
    }

    TestReadBackType readback_type() const
    {
        return GetParam().readback_type;
    }
        ...
        if (readback_type() == TestReadBackType::kBitmap) {
            request
                = std::make_unique<viz::CopyOutputRequest>(viz::CopyOutputRequest::ResultFormat::RGBA, viz::CopyOutputRequest::ResultDestination::kSystemMemory,
                    base::BindOnce(&LayerTreeHostReadbackPixelTest::ReadbackResultAsBitmap, base::Unretained(this)));
        } else {
            DCHECK_NE(renderer_type_, viz::RendererType::kSoftware);
            request = std::make_unique<viz::CopyOutputRequest>(viz::CopyOutputRequest::ResultFormat::RGBA,
                viz::CopyOutputRequest::ResultDestination::kNativeTextures,
                base::BindOnce(&LayerTreeHostReadbackPixelTest::ReadbackResultAsTexture, base::Unretained(this)));
        }
    ...
    void BeginTest() override
    {
        if (insert_copy_request_after_frame_count_ == 0) {
            Layer* const target = readback_target_ ? readback_target_.get() : layer_tree_host()->root_layer();
    ...
    SkBitmap CopyMailboxToBitmap(const gfx::Size& size, const gpu::Mailbox& mailbox, const gpu::SyncToken& sync_token, const gfx::ColorSpace& color_space)
    {
        DCHECK(context_provider_);
        viz::RasterContextProvider::ScopedRasterContextLock lock(context_provider_.get());
        ...
        SkBitmap bitmap;
        bitmap.allocPixels(SkImageInfo::MakeN32Premul(size.width(), size.height(), color_space.ToSkColorSpace()));
    ...
    void ReadbackResultAsTexture(std::unique_ptr<viz::CopyOutputResult> result)
    {
        ...
        ASSERT_EQ(result->destination(), viz::CopyOutputResult::Destination::kNativeTextures);
        ...
        viz::CopyOutputResult::ReleaseCallbacks release_callbacks = result->TakeTextureOwnership();
        EXPECT_EQ(1u, release_callbacks.size());

        SkBitmap bitmap = CopyMailboxToBitmap(result->size(), mailbox, gpu::SyncToken(), color_space);
        std::move(release_callbacks[0]).Run(gpu::SyncToken(), false);

        ReadbackResultAsBitmap(std::make_unique<viz::CopyOutputSkBitmapResult>(result->rect(), std::move(bitmap)));
    }
...
TEST_P(LayerTreeHostReadbackPixelTest, ReadbackRootLayerWithChild)
{
    ...
    RunPixelTest(background, base::FilePath(FILE_PATH_LITERAL("green_with_blue_corner.png")));
}
...
TEST_P(LayerTreeHostReadbackPixelTest, ReadbackNonRootLayer)
{
    ...
    RunPixelTestWithReadbackTarget(background, green.get(), base::FilePath(FILE_PATH_LITERAL("green.png")));
}
...
TEST_P(LayerTreeHostReadbackPixelTest, ReadbackSmallNonRootLayer)
{
    ...
    RunPixelTestWithReadbackTarget(background, green.get(), base::FilePath(FILE_PATH_LITERAL("green_small.png")));
}
...
TEST_P(LayerTreeHostReadbackPixelTest, ReadbackSmallNonRootLayerWithChild)
{
    ...
    RunPixelTestWithReadbackTarget(background, green.get(), base::FilePath(FILE_PATH_LITERAL("green_small_with_blue_corner.png")));
}
...
TEST_P(LayerTreeHostReadbackPixelTest, ReadbackSubtreeSurroundsTargetLayer)
{
    ...
    RunPixelTestWithReadbackTarget(background, target.get(), base::FilePath(FILE_PATH_LITERAL("green_small_with_blue_corner.png")));
}

TEST_P(LayerTreeHostReadbackPixelTest, ReadbackSubtreeExtendsBeyondTargetLayer)
{
    scoped_refptr<SolidColorLayer> background = CreateSolidColorLayer(gfx::Rect(0, 0, 200, 200), SK_ColorWHITE);
...
TEST_P(LayerTreeHostReadbackPixelTest, ReadbackHiddenSubtree)
{
    ...
    RunPixelTestWithReadbackTarget(background, hidden_target.get(), base::FilePath(FILE_PATH_LITERAL("green_with_blue_corner.png")));
}

TEST_P(LayerTreeHostReadbackPixelTest, HiddenSubtreeNotVisibleWhenDrawnForReadback)
{
    scoped_refptr<SolidColorLayer> background = CreateSolidColorLayer(gfx::Rect(200, 200), SK_ColorBLACK);
    ...
    hidden_target->RequestCopyOfOutput(viz::CopyOutputRequest::CreateStubForTesting());
...
TEST_P(LayerTreeHostReadbackPixelTest, ReadbackNonRootLayerOutsideViewport)
{
    ...
    RunPixelTestWithReadbackTarget(background, green.get(), base::FilePath(FILE_PATH_LITERAL("green_with_blue_corner.png")));
}
...
TEST_P(LayerTreeHostReadbackPixelTest, ReadbackNonRootOrFirstLayer)
{
    ...
    RunPixelTestWithReadbackTarget(background, background.get(), base::FilePath(FILE_PATH_LITERAL("green_with_blue_corner.png")));
}
...
TEST_P(LayerTreeHostReadbackPixelTest, MultipleReadbacksOnLayer)
{
    ...
    background->RequestCopyOfOutput(viz::CopyOutputRequest::CreateStubForTesting());

    RunPixelTestWithReadbackTarget(background, background.get(), base::FilePath(FILE_PATH_LITERAL("green.png")));
}
...
ReadbackTestConfig const kTestConfigs[] = {
    ...
    ReadbackTestConfig { viz::RendererType::kSkiaGraphiteDawn, TestReadBackType::kBitmap },
#if BUILDFLAG(IS_IOS)
    ReadbackTestConfig { viz::RendererType::kSkiaGraphiteMetal, TestReadBackType::kBitmap },
#endif // BUILDFLAG(IS_IOS)
#endif // BUILDFLAG(ENABLE_SKIA_GRAPHITE_TESTS)
};

INSTANTIATE_TEST_SUITE_P(All, LayerTreeHostReadbackPixelTest, ::testing::ValuesIn(kTestConfigs), ::testing::PrintToStringParamName());

class LayerTreeHostReadbackDeviceScalePixelTest : public LayerTreeHostReadbackPixelTest {
protected:
    ...
    {
        EXPECT_EQ(device_scale_factor_, host_impl->active_tree()->device_scale_factor());
    }
...
INSTANTIATE_TEST_SUITE_P(All, LayerTreeHostReadbackDeviceScalePixelTest, ::testing::ValuesIn(kTestConfigs), ::testing::PrintToStringParamName());

class LayerTreeHostReadbackColorSpacePixelTest : public LayerTreeHostReadbackPixelTest {
protected:
        ...
        scoped_refptr<viz::RasterContextProvider> compositor_context_provider, scoped_refptr<viz::RasterContextProvider> worker_context_provider) override
    {
        std::unique_ptr<TestLayerTreeFrameSink> frame_sink
            = LayerTreePixelTest::CreateLayerTreeFrameSink(renderer_settings, refresh_rate, compositor_context_provider, worker_context_provider);
...
{
    scoped_refptr<FakePictureLayer> background = FakePictureLayer::Create(&green_client_);
    background->SetBounds(gfx::Size(200, 200));
    background->SetIsDrawable(true);

    // The sRGB green should be converted into P3.
    RunPixelTest(background, base::FilePath(FILE_PATH_LITERAL("srgb_green_in_p3.png")));
}

INSTANTIATE_TEST_SUITE_P(All, LayerTreeHostReadbackColorSpacePixelTest, ::testing::ValuesIn(kTestConfigs), ::testing::PrintToStringParamName());
