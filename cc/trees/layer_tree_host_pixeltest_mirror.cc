...
class LayerTreeHostMirrorPixelTest : public LayerTreePixelTest, public ::testing::WithParamInterface<viz::RendererType> {
protected:
...
INSTANTIATE_TEST_SUITE_P(All, LayerTreeHostMirrorPixelTest, ::testing::ValuesIn(viz::GetRendererTypes()), ::testing::PrintToStringParamName());
...
TEST_P(LayerTreeHostMirrorPixelTest, MirrorLayer)
{
    ...
    auto mirrored_layer = CreateSolidColorLayerWithBorder(mirrored_bounds, SK_ColorGREEN, 5, SK_ColorBLUE);
    ...
    if (use_software_renderer()) {
        pixel_comparator_ = std::make_unique<FuzzyPixelComparator>(
            FuzzyPixelComparator().DiscardAlpha().SetErrorPixelsPercentageLimit(3.f).SetAvgAbsErrorLimit(65.f).SetAbsErrorLimit(120));
    }
    ...
    RunPixelTest(background, base::FilePath(FILE_PATH_LITERAL("mirror_layer.png")));
}
