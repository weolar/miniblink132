...
class LayerTreeHostScrollbarsPixelTest : public LayerTreePixelTest, public ::testing::WithParamInterface<viz::RendererType> {
protected:
...
public:
    ...
    void set_paint_scale(int scale)
    {
        paint_scale_ = scale;
    }
...
private:
    void Paint(PaintCanvas& canvas, const gfx::Rect& rect) override
    {
        ...
        while (!inset_rect.IsEmpty()) {
            ...
            inset_rect.Inset(gfx::Insets::TLBR(big_rect, big_rect, small_rect, small_rect));
...
INSTANTIATE_TEST_SUITE_P(All, LayerTreeHostScrollbarsPixelTest, ::testing::ValuesIn(viz::GetGpuRendererTypes()), ::testing::PrintToStringParamName());
...
TEST_P(LayerTreeHostScrollbarsPixelTest, NoScale)
{
    ...
    scoped_refptr<PaintedScrollbarLayer> layer = PaintedScrollbarLayer::Create(std::move(scrollbar));
...
TEST_P(LayerTreeHostScrollbarsPixelTest, DeviceScaleFactor)
{
    ...
    RunPixelTest(background, base::FilePath(FILE_PATH_LITERAL("spiral_double_scale.png")));
}
...
TEST_P(LayerTreeHostScrollbarsPixelTest, MAYBE_HugeTransformScale)
{
    ...
    // just cut off.
    layer->SetPosition(gfx::PointF(-10.f * scale + 400.f, -400.f * scale + 400.f));
    ...
    base::FilePath expected_result = base::FilePath(FILE_PATH_LITERAL("spiral_64_scale.png"));
...
class LayerTreeHostOverlayScrollbarsPixelTest : public LayerTreeHostScrollbarsPixelTest {
protected:
...
INSTANTIATE_TEST_SUITE_P(All, LayerTreeHostOverlayScrollbarsPixelTest, ::testing::ValuesIn(viz::GetGpuRendererTypes()), ::testing::PrintToStringParamName());

// viz::GetGpuRendererTypes() can return an empty list on some platforms.
GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(LayerTreeHostOverlayScrollbarsPixelTest);
...
TEST_P(LayerTreeHostOverlayScrollbarsPixelTest, NinePatchScrollbarScaledUp)
{
    ...
    scoped_refptr<NinePatchThumbScrollbarLayer> layer = NinePatchThumbScrollbarLayer::Create(std::move(scrollbar));
    ...
    RunPixelTest(background, base::FilePath(FILE_PATH_LITERAL("overlay_scrollbar_scaled_up.png")));
}
...
TEST_P(LayerTreeHostOverlayScrollbarsPixelTest, NinePatchScrollbarScaledDown)
{
    ...
    RunPixelTest(background, base::FilePath(FILE_PATH_LITERAL("overlay_scrollbar_scaled_down.png")));
}
