...
// snapped to.
base::TimeTicks SnappedTickTimeFromFrameTime(const viz::BeginFrameArgs& args, base::TimeTicks desired_frame_time)
{
    auto snapped_tick_time = desired_frame_time.SnappedToNextTick(args.frame_time, args.interval);
...
ImageAnimationController::ImageAnimationController(base::SingleThreadTaskRunner* task_runner, Client* client, bool enable_image_animation_resync)
    : scheduler_(task_runner, client)
    , enable_image_animation_resync_(enable_image_animation_resync)
    , use_resume_behavior_(base::FeatureList::IsEnabled(features::kAnimatedImageResume))
{
}

ImageAnimationController::~ImageAnimationController() = default;

void ImageAnimationController::UpdateAnimatedImage(const DiscardableImageMap::AnimatedImageMetadata& data)
{
    AnimationState& animation_state = animation_state_map_[data.paint_image_id];
    animation_state.UpdateMetadata(data);
}

void ImageAnimationController::RegisterAnimationDriver(PaintImage::Id paint_image_id, AnimationDriver* driver)
{
...
void ImageAnimationController::UnregisterAnimationDriver(PaintImage::Id paint_image_id, AnimationDriver* driver)
{
    auto it = animation_state_map_.find(paint_image_id);
    CHECK(it != animation_state_map_.end(), base::NotFatalUntil::M130);
...
const PaintImageIdFlatSet& ImageAnimationController::AnimateForSyncTree(const viz::BeginFrameArgs& args)
{
    TRACE_EVENT1(
        "cc", "ImageAnimationController::AnimateImagesForSyncTree", "frame_time_from_now", (base::TimeTicks::Now() - args.frame_time).InMillisecondsF());
    ...
    for (auto id : registered_animations_) {
        ...
        if (!state.ShouldAnimate()) {
            TRACE_EVENT_INSTANT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "ShouldAnimate - early out", TRACE_EVENT_SCOPE_THREAD);
            continue;
        }

        // If we were able to advance this animation, invalidate it on the sync
        // tree.
        if (state.AdvanceFrame(args, enable_image_animation_resync_, use_resume_behavior_))
            images_animated_on_sync_tree_.insert(id);

        TRACE_EVENT_INSTANT1(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "AnimationState", TRACE_EVENT_SCOPE_THREAD, "state", state.ToString());
        ...
        if (!next_invalidation_time.has_value()) {
            ...
            next_invalidation_time = std::min(state.next_desired_tick_time(), next_invalidation_time.value());
        }
    }
...
void ImageAnimationController::UpdateStateFromDrivers()
{
    ...
    for (auto image_id : registered_animations_) {
        ...
        if (!next_invalidation_time.has_value()) {
            ...
            next_invalidation_time = std::min(next_invalidation_time.value(), state.next_desired_tick_time());
        }
    }
...
void ImageAnimationController::DidActivate()
{
    ...
    if (did_navigate_) {
        for (auto it = animation_state_map_.begin(); it != animation_state_map_.end();) {
            if (it->second.has_drivers())
                it++;
            else
...
size_t ImageAnimationController::GetFrameIndexForImage(PaintImage::Id paint_image_id, WhichTree tree) const
{
    const auto& it = animation_state_map_.find(paint_image_id);
    CHECK(it != animation_state_map_.end(), base::NotFatalUntil::M130);
    return tree == WhichTree::PENDING_TREE ? it->second.pending_index() : it->second.active_index();
}

void ImageAnimationController::WillBeginImplFrame(const viz::BeginFrameArgs& args)
{
    scheduler_.WillBeginImplFrame(args);
}

const base::flat_set<raw_ptr<ImageAnimationController::AnimationDriver, CtnExperimental>>& ImageAnimationController::GetDriversForTesting(
...
size_t ImageAnimationController::GetLastNumOfFramesSkippedForTesting(PaintImage::Id paint_image_id) const
{
    const auto& it = animation_state_map_.find(paint_image_id);
    CHECK(it != animation_state_map_.end(), base::NotFatalUntil::M130);
...
std::optional<ImageAnimationController::ConsistentFrameDuration> ImageAnimationController::GetConsistentContentFrameDuration()
{
    ...
    for (auto& [id, state] : animation_state_map_) {
        ...
        std::optional<base::TimeDelta> image_frame_duration = state.GetConsistentContentFrameDuration();
        if (!image_frame_duration) {
            return std::nullopt;
        }
        if (frame_duration && frame_duration.value() != image_frame_duration.value()) {
            return std::nullopt;
        }
...
ImageAnimationController::AnimationState::AnimationState(AnimationState&& other) = default;

ImageAnimationController::AnimationState& ImageAnimationController::AnimationState::operator=(AnimationState&& other) = default;
...
{
    return ShouldAnimate(current_state_.repetitions_completed, current_state_.pending_index);
}

bool ImageAnimationController::AnimationState::ShouldAnimate(int repetitions_completed, size_t pending_index) const
{
    ...
    if (completion_state_ != PaintImage::CompletionState::kDone && pending_index == last_frame_index) {
        return false;
    }
...
bool ImageAnimationController::AnimationState::AdvanceFrame(const viz::BeginFrameArgs& args, bool enable_image_animation_resync, bool use_resume_behavior)
{
    DCHECK(ShouldAnimate(current_state_.repetitions_completed, current_state_.pending_index));
    ...
    if (!animation_started_) {
        ...
        current_state_.next_desired_frame_time = args.frame_time + frames_[0].duration;
        current_state_.next_desired_tick_time = SnappedTickTimeFromFrameTime(args, current_state_.next_desired_frame_time);
    ...
    if (enable_image_animation_resync && args.frame_time - current_state_.next_desired_frame_time > kAnimationResyncCutoff) {
        TRACE_EVENT_INSTANT0("cc", "Resync - early out", TRACE_EVENT_SCOPE_THREAD);
        DCHECK_EQ(current_state_.pending_index, active_index_);
        current_state_.next_desired_frame_time = args.frame_time + frames_[current_state_.pending_index].duration;
        current_state_.next_desired_tick_time = std::max(SnappedTickTimeFromFrameTime(args, current_state_.next_desired_frame_time), next_tick_time);
        return needs_invalidation();
    }
    ...
    if (use_resume_behavior) {
        ...
        // frames.
        current_state_ = AdvanceAnimationState(current_state_, args, args.frame_time, enable_image_animation_resync);
    } else {
        // Keep catching up the animation from the last saved tick time until we
        // reach the frame we should be displaying now.
        current_state_ = AdvanceAnimationState(current_state_, args, current_state_.next_desired_tick_time, enable_image_animation_resync);
    }
...
ImageAnimationController::AnimationState::AnimationAdvancementState ImageAnimationController::AnimationState::AdvanceAnimationState(
    AnimationAdvancementState animation_advancement_state, const viz::BeginFrameArgs& args, base::TimeTicks start, bool enable_image_animation_resync) const
{
    const base::TimeTicks end = args.frame_time + args.interval;
    base::TimeTicks elapsed_time = start;
    while (elapsed_time < end && ShouldAnimate(animation_advancement_state.repetitions_completed, animation_advancement_state.pending_index)) {
        animation_advancement_state.num_of_frames_advanced++;
        size_t next_frame_index = NextFrameIndex(animation_advancement_state.pending_index);
        elapsed_time += frames_[next_frame_index].duration;
        TRACE_EVENT_INSTANT2(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "FrameDurationIndex", TRACE_EVENT_SCOPE_THREAD, "frame_index", next_frame_index, "duration",
            frames_[next_frame_index].duration.InMillisecondsF());
        base::TimeTicks next_desired_frame_time = animation_advancement_state.next_desired_frame_time + frames_[next_frame_index].duration;
        ...
        if (enable_image_animation_resync && next_frame_index == 0u && animation_advancement_state.repetitions_completed == 1
            && next_desired_frame_time <= args.frame_time) {
            animation_advancement_state.pending_index = 0u;
            animation_advancement_state.next_desired_frame_time = args.frame_time + frames_[0].duration;
            animation_advancement_state.next_desired_tick_time
                = std::max(SnappedTickTimeFromFrameTime(args, animation_advancement_state.next_desired_frame_time), end);
            animation_advancement_state.repetitions_completed = 0;
            break;
        }

        animation_advancement_state.pending_index = next_frame_index;
        animation_advancement_state.next_desired_frame_time = next_desired_frame_time;
        animation_advancement_state.next_desired_tick_time = SnappedTickTimeFromFrameTime(args, animation_advancement_state.next_desired_frame_time);
        ...
        if (animation_advancement_state.pending_index == frames_.size() - 1 && is_complete())
    ...
    size_t frames_skipped = animation_advancement_state.num_of_frames_advanced - 1u;
    switch (animation_advancement_state.repetitions_completed) {
    case 0:
        UMA_HISTOGRAM_COUNTS_100000("AnimatedImage.NumOfFramesSkipped.FirstAnimationLoop", frames_skipped);
        break;
    case 1:
        UMA_HISTOGRAM_COUNTS_100000("AnimatedImage.NumOfFramesSkipped.SecondAnimationLoop", frames_skipped);
        break;
    case 2:
    case 3:
    case 4:
        UMA_HISTOGRAM_COUNTS_100000("AnimatedImage.NumOfFramesSkipped.ThirdToFifthAnimationLoop", frames_skipped);
        break;
    }
    UMA_HISTOGRAM_COUNTS_100000("AnimatedImage.NumOfFramesSkipped.Compositor", frames_skipped);

    return animation_advancement_state;
}

void ImageAnimationController::AnimationState::UpdateMetadata(const DiscardableImageMap::AnimatedImageMetadata& data)
{
    ...
    DCHECK(frames_.size() <= data.frames.size()) << "Updated recordings can only append frames";
    frames_ = data.frames;
    cached_consistent_frame_duration_valid_ = false;
    DCHECK_GT(frames_.size(), 1u);

    DCHECK(completion_state_ != PaintImage::CompletionState::kDone || data.completion_state == PaintImage::CompletionState::kDone)
    ...
    size_t last_frame_index = frames_.size() - 1;
    if (current_state_.pending_index == last_frame_index && is_complete() && current_state_.repetitions_completed == 0)
...
std::optional<base::TimeDelta> ImageAnimationController::AnimationState::GetConsistentContentFrameDuration()
{
...
void ImageAnimationController::AnimationState::ComputeConsistentContentFrameDuration()
{
...
void ImageAnimationController::AnimationState::AddDriver(AnimationDriver* driver)
{
    drivers_.insert(driver);
}

void ImageAnimationController::AnimationState::RemoveDriver(AnimationDriver* driver)
{
...
std::string ImageAnimationController::AnimationState::ToString() const
{
    ...
    str << "paint_image_id[" << paint_image_id_ << "]\nrequested_repetitions[" << requested_repetitions_ << "]\nrepetitions_completed["
        ...
        << "]\nnext_desired_tick_time[" << (current_state_.next_desired_tick_time - animation_started_time_).InMillisecondsF()
        << "]\nshould_animate_from_drivers[" << should_animate_from_drivers_ << "]\ncompletion_state[" << static_cast<int>(completion_state_) << "]";
    return str.str();
}

size_t ImageAnimationController::AnimationState::NextFrameIndex(size_t pending_index) const
{
    if (!animation_started_)
        return 0u;
...
ImageAnimationController::InvalidationScheduler::InvalidationScheduler(base::SingleThreadTaskRunner* task_runner, Client* client)
...
void ImageAnimationController::InvalidationScheduler::Schedule(base::TimeTicks animation_time)
{
    auto now = now_callback_for_testing_.is_null() ? base::TimeTicks::Now() : now_callback_for_testing_.Run();
    ...
    // needed.
    if (state_ == InvalidationState::kPendingInvalidation || state_ == InvalidationState::kPendingImplFrame) {
        return;
    }
    ...
    // Cancel it.
    if (state_ == InvalidationState::kPendingRequestBeginFrame && animation_time == next_animation_time_)
        return;
    ...
    TRACE_EVENT1("cc", "ScheduleFrameForImageAnimation", "delta", delta.InMillisecondsF());

    state_ = InvalidationState::kPendingRequestBeginFrame;
    next_animation_time_ = animation_time;
    task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&InvalidationScheduler::RequestBeginFrame, weak_factory_.GetWeakPtr()), delta);
}
...
{
    TRACE_EVENT0("cc", "ImageAnimationController::InvalidationScheduler::RequestBeginFrame");
...
void ImageAnimationController::InvalidationScheduler::WillBeginImplFrame(const viz::BeginFrameArgs& args)
{
    switch (state_) {
...
{
    TRACE_EVENT0("cc", "ImageAnimationController::InvalidationScheduler::RequestInvalidation");
