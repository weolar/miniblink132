...
public:
    ...
    void LoseContext()
    {
        ...
        // context is not lost.
        CHECK(gl_);
        gl_->LoseContextCHROMIUM(GL_GUILTY_CONTEXT_RESET_ARB, GL_INNOCENT_CONTEXT_RESET_ARB);
        gl_ = nullptr;
    }
        ...
        scoped_refptr<viz::RasterContextProvider> compositor_context_provider, scoped_refptr<viz::RasterContextProvider> worker_context_provider) override
    {
        ...
        return LayerTreeTest::CreateLayerTreeFrameSink(renderer_settings, refresh_rate, std::move(provider), std::move(worker_context_provider));
    }
...
protected:
    ...
    raw_ptr<gpu::TestSharedImageInterface, AcrossTasksDanglingUntriaged> sii_ = nullptr;
...
class LayerTreeHostContextTestLostContextSucceeds : public LayerTreeHostContextTest {
public:
    ...
    void RequestNewLayerTreeFrameSink() override
    {
        if (async_layer_tree_frame_sink_creation_) {
            MainThreadTaskRunner()->PostTask(
                FROM_HERE, base::BindOnce(&LayerTreeHostContextTestLostContextSucceeds::AsyncRequestNewLayerTreeFrameSink, base::Unretained(this)));
        } else {
    ...
    bool NextTestCase()
    {
        ...
        async_layer_tree_frame_sink_creation_ = kTests[test_case_].async_layer_tree_frame_sink_creation;
        ++test_case_;
        return true;
    }
...
class LayerTreeHostClientNotVisibleDoesNotCreateLayerTreeFrameSink : public LayerTreeHostContextTest {
public:
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostClientNotVisibleDoesNotCreateLayerTreeFrameSink);
...
class LayerTreeHostClientTakeAwayLayerTreeFrameSink : public LayerTreeHostContextTest {
public:
    ...
    void HideAndReleaseLayerTreeFrameSink()
    {
        ...
        std::unique_ptr<LayerTreeFrameSink> surface = layer_tree_host()->ReleaseLayerTreeFrameSink();
        CHECK(surface);
        MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&LayerTreeHostClientTakeAwayLayerTreeFrameSink::MakeVisible, base::Unretained(this)));
    }
    ...
    void DidInitializeLayerTreeFrameSink() override
    {
        ...
        if (setos_counter_ == 1) {
            MainThreadTaskRunner()->PostTask(
                FROM_HERE, base::BindOnce(&LayerTreeHostClientTakeAwayLayerTreeFrameSink::HideAndReleaseLayerTreeFrameSink, base::Unretained(this)));
        } else {
            EndTest();
        }
    }
...
class MultipleCompositeDoesNotCreateLayerTreeFrameSink : public LayerTreeHostContextTest {
public:
    ...
    void BeginTest() override
    {
        layer_tree_host()->CompositeForTest(TicksFromMicroseconds(1), false, base::OnceClosure());
        layer_tree_host()->CompositeForTest(TicksFromMicroseconds(2), false, base::OnceClosure());
    }
...
class FailedCreateDoesNotCreateExtraLayerTreeFrameSink : public LayerTreeHostContextTest {
public:
...
public:
    ...
    {
        FakePictureLayerImpl* picture_impl = static_cast<FakePictureLayerImpl*>(host_impl->active_tree()->LayerById(layer_->id()));
        EXPECT_TRUE(picture_impl->HighResTiling()->TileAt(0, 0)->draw_info().IsReadyToDraw());
    }
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostContextTestLostContextSucceedsWithContent);

class LayerTreeHostContextTestCreateLayerTreeFrameSinkFailsOnce : public LayerTreeHostContextTest {
public:
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostContextTestCreateLayerTreeFrameSinkFailsOnce);

class LayerTreeHostContextTestLostContextAndEvictTextures : public LayerTreeHostContextTest {
public:
    ...
    void PostEvictTextures()
    {
        if (HasImplThread()) {
            ImplThreadTaskRunner()->PostTask(
                FROM_HERE, base::BindOnce(&LayerTreeHostContextTestLostContextAndEvictTextures::EvictTexturesOnImplThread, base::Unretained(this)));
        } else {
...
TEST_F(LayerTreeHostContextTestLostContextAndEvictTextures, LoseAfterEvict_SingleThread)
{
    lose_after_evict_ = true;
    RunTest(CompositorMode::SINGLE_THREADED);
}

TEST_F(LayerTreeHostContextTestLostContextAndEvictTextures, LoseAfterEvict_MultiThread)
{
    lose_after_evict_ = true;
    RunTest(CompositorMode::THREADED);
}

TEST_F(LayerTreeHostContextTestLostContextAndEvictTextures, LoseBeforeEvict_SingleThread)
{
    lose_after_evict_ = false;
    RunTest(CompositorMode::SINGLE_THREADED);
}

TEST_F(LayerTreeHostContextTestLostContextAndEvictTextures, LoseBeforeEvict_MultiThread)
{
    lose_after_evict_ = false;
    RunTest(CompositorMode::THREADED);
}
...
public:
    ...
    void DidActivateTreeOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        if (num_commits_ >= 2) {
            root_picture = static_cast<FakePictureLayerImpl*>(host_impl->active_tree()->root_layer());
            child_picture = static_cast<FakePictureLayerImpl*>(host_impl->active_tree()->LayerById(child_->id()));
            grandchild_picture = static_cast<FakePictureLayerImpl*>(host_impl->active_tree()->LayerById(grandchild_->id()));
        }
        switch (num_commits_) {
        case 1:
            ...
            // releasing.
            MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&LayerTreeHostContextTestLayersNotified::AttachTree, base::Unretained(this)));
            break;
        case 2:
...
class LayerTreeHostContextTestDontUseLostResources : public LayerTreeHostContextTest {
public:
    ...
    static void EmptyReleaseCallback(const gpu::SyncToken& sync_token, bool lost)
    {
    }
    ...
    void SetupTree() override
    {
        ...
        auto resource = viz::TransferableResource::MakeGpu(
            shared_image, GL_TEXTURE_2D, sync_token, size, viz::SinglePlaneFormat::kRGBA_8888, false /* is_overlay_candidate */);
        texture->SetTransferableResource(resource, base::BindOnce(&LayerTreeHostContextTestDontUseLostResources::EmptyReleaseCallback));
        ...
        color_video_frame_ = VideoFrame::CreateColorFrame(gfx::Size(4, 4), 0x80, 0x80, 0x80, base::TimeDelta());
        ASSERT_TRUE(color_video_frame_);
        hw_video_frame_ = VideoFrame::WrapSharedImage(media::PIXEL_FORMAT_ARGB, shared_image, sync_token, media::VideoFrame::ReleaseMailboxCB(),
            gfx::Size(4, 4), gfx::Rect(0, 0, 4, 4), gfx::Size(4, 4), base::TimeDelta());
        ASSERT_TRUE(hw_video_frame_);
        scaled_hw_video_frame_ = VideoFrame::WrapSharedImage(media::PIXEL_FORMAT_ARGB, shared_image, sync_token, media::VideoFrame::ReleaseMailboxCB(),
        ...
        scoped_refptr<PaintedScrollbarLayer> scrollbar = PaintedScrollbarLayer::Create(base::MakeRefCounted<FakeScrollbar>());
...
class LayerTreeHostContextTestImplSidePainting : public LayerTreeHostContextTest {
public:
    void SetupTree() override
    {
        scoped_refptr<Layer> root = Layer::Create();
...
public:
    ...
    void BeginTest() override
    {
        ...
        scrollbar_layer_ = base::MakeRefCounted<FakePaintedScrollbarLayer>(scroll_layer->element_id());
...
public:
    ...
    void PostStepCompleteToMainThread()
    {
        task_runner_provider()->MainThreadTaskRunner()->PostTask(
            FROM_HERE, base::BindOnce(&UIResourceLostTest::StepCompleteOnMainThreadInternal, base::Unretained(this), time_step_));
    }

    void PostLoseContextToImplThread()
    {
        EXPECT_TRUE(layer_tree_host()->GetTaskRunnerProvider()->IsMainThread());
        ImplThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&LayerTreeHostContextTest::LoseContext, base::Unretained(this)));
    }
...
public:
    ...
    void StepCompleteOnImplThread(LayerTreeHostImpl* impl) override
    {
        ...
        case 1:
            ...
            EXPECT_NE(viz::kInvalidResourceId, impl->ResourceIdForUIResource(ui_resource_->id()));
            PostSetNeedsCommitToMainThread();
            break;
        case 2:
            LoseContext();
...
public:
    ...
    void StepCompleteOnImplThread(LayerTreeHostImpl* impl) override
    {
        ...
        case 3:
            ...
            EXPECT_EQ(viz::kInvalidResourceId, impl->ResourceIdForUIResource(test_id0_));
            // The second resource should have been created.
            EXPECT_NE(viz::kInvalidResourceId, impl->ResourceIdForUIResource(test_id1_));
...
class UIResourceLostBeforeActivateTree : public UIResourceLostTest {
    ...
    void WillActivateTreeOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        case 1:
            ...
            EXPECT_EQ(viz::kInvalidResourceId, impl->ResourceIdForUIResource(ui_resource_->id()));
            LoseContext();
            break;
        case 3:
            LoseContext();
    ...
    void DidActivateTreeOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        case 4:
            ...
            EXPECT_EQ(viz::kInvalidResourceId, impl->ResourceIdForUIResource(test_id_));
            break;
        }
...
public:
    void StepCompleteOnMainThread(int step) override
    {
        ...
        case 0:
            ui_resource_ = FakeScopedUIResource::Create(layer_tree_host()->GetUIResourceManager());
            ui_resource2_ = FakeScopedUIResource::Create(layer_tree_host()->GetUIResourceManager());
            EXPECT_NE(0, ui_resource_->id());
        ...
        case 2:
            ...
            ui_resource3_ = FakeScopedUIResource::Create(layer_tree_host()->GetUIResourceManager());
            break;
        case 3:
    ...
    void DidSetVisibleOnImplTree(LayerTreeHostImpl* impl, bool visible) override
    {
        if (!visible) {
            ...
            EXPECT_EQ(viz::kInvalidResourceId, impl->ResourceIdForUIResource(ui_resource_->id()));
            EXPECT_EQ(viz::kInvalidResourceId, impl->ResourceIdForUIResource(ui_resource2_->id()));
            EXPECT_EQ(viz::kInvalidResourceId, impl->ResourceIdForUIResource(ui_resource3_->id()));
    ...
    void StepCompleteOnImplThread(LayerTreeHostImpl* impl) override
    {
        ...
        case 1:
            ...
            EXPECT_NE(viz::kInvalidResourceId, impl->ResourceIdForUIResource(ui_resource_->id()));
            EXPECT_NE(viz::kInvalidResourceId, impl->ResourceIdForUIResource(ui_resource2_->id()));
            EXPECT_EQ(1, ui_resource_->resource_create_count);
            EXPECT_EQ(0, ui_resource_->lost_resource_count);
        ...
        case 3:
            ...
            EXPECT_NE(viz::kInvalidResourceId, impl->ResourceIdForUIResource(ui_resource3_->id()));
...
protected:
    ...
    void DidActivateTreeOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        case 1:
            ...
            EXPECT_NE(viz::kInvalidResourceId, impl->ResourceIdForUIResource(ui_resource_->id()));
            EXPECT_EQ(1u, sii_->shared_image_count());
            MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&UIResourceFreedIfLostWhileExported::DeleteAndEndTest, base::Unretained(this)));
        }
    }
...
protected:
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* impl) override
    {
        auto* context_provider = static_cast<viz::TestContextProvider*>(impl->layer_tree_frame_sink()->worker_context_provider());
...
protected:
        ...
        scoped_refptr<viz::RasterContextProvider> compositor_context_provider, scoped_refptr<viz::RasterContextProvider> worker_context_provider) override
    {
        ...
        return LayerTreeTest::CreateLayerTreeFrameSink(renderer_settings, refresh_rate, nullptr, nullptr);
    }
...
class LayerTreeHostContextTestLoseAfterSendingBeginMainFrame : public LayerTreeHostContextTest {
protected:
    void BeginTest() override
    {
        PostSetNeedsCommitToMainThread();
    ...
    void WillBeginMainFrame() override
    {
        ...
        ImplThreadTaskRunner()->PostTask(
            FROM_HERE, base::BindOnce(&LayerTreeHostContextTestLoseAfterSendingBeginMainFrame::LoseContextOnImplThread, base::Unretained(this)));
    ...
    void LoseContextOnImplThread()
    {
        ...
        PostReturnDeferMainFrameUpdateToMainThread(std::move(scoped_defer_main_frame_update_));
    }
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostContextTestLoseAfterSendingBeginMainFrame);
...
protected:
    ...
    void WillPrepareTilesOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        viz::RasterContextProvider::ScopedRasterContextLock scoped_context(host_impl->layer_tree_frame_sink()->worker_context_provider());
        gpu::raster::RasterInterface* ri = scoped_context.RasterInterface();
        ri->LoseContextCHROMIUM(GL_GUILTY_CONTEXT_RESET_ARB, GL_INNOCENT_CONTEXT_RESET_ARB);
    }
