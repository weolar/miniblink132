...
class LayerTreeHostTilesPixelTest : public LayerTreePixelTest, public ::testing::WithParamInterface<RasterTestConfig> {
protected:
    LayerTreeHostTilesPixelTest()
    ...
    void WillPrepareTilesOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        viz::RasterContextProvider* context_provider = host_impl->layer_tree_frame_sink()->worker_context_provider();
        if (!context_provider)
            return;
...
class LayerTreeHostTilesTestPartialInvalidation : public LayerTreeHostTilesPixelTest {
public:
...
public:
    explicit PrimaryColorClient(const gfx::Size& size)
        : size_(size)
    {
    }
...
class LayerTreeHostTilesTestRasterColorSpace : public LayerTreeHostTilesPixelTest {
public:
    ...
    void WillBeginTest() override
    {
        ...
        layer_tree_host()->SetDisplayColorSpaces(gfx::DisplayColorSpaces(color_space_));
    }
...
INSTANTIATE_TEST_SUITE_P(All, LayerTreeHostTilesTestPartialInvalidation, ::testing::ValuesIn(kTestCases), ::testing::PrintToStringParamName());

#if BUILDFLAG(IS_CHROMEOS_ASH) || defined(MEMORY_SANITIZER) || defined(ADDRESS_SANITIZER)
...
TEST_P(LayerTreeHostTilesTestPartialInvalidation, MAYBE_PartialRaster)
{
    ...
    base::FilePath expected_result = base::FilePath(FILE_PATH_LITERAL("blue_yellow_partial_flipped.png"));
...
{
    base::FilePath expected_result = base::FilePath(FILE_PATH_LITERAL("blue_yellow_flipped.png"));
    if (use_skia_graphite()) {
        expected_result = expected_result.InsertBeforeExtensionASCII("_graphite");
    }
    RunSingleThreadedPixelTest(picture_layer_, expected_result);
...
using LayerTreeHostTilesTestPartialInvalidationMultiThread = LayerTreeHostTilesTestPartialInvalidation;

INSTANTIATE_TEST_SUITE_P(
    All, LayerTreeHostTilesTestPartialInvalidationMultiThread, ::testing::ValuesIn(kTestCasesMultiThread), ::testing::PrintToStringParamName());

// kTestCasesMultiThread is empty on some platforms.
GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(LayerTreeHostTilesTestPartialInvalidationMultiThread);

#if (BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)) && defined(THREAD_SANITIZER)
// Flaky on Linux TSAN. https://crbug.com/707711
#define MAYBE_PartialRaster DISABLED_PartialRaster
#elif BUILDFLAG(IS_CHROMEOS_ASH) || defined(MEMORY_SANITIZER) || defined(ADDRESS_SANITIZER)
// TODO(crbug.com/40116070): Flakes on all slower bots.
#define MAYBE_PartialRaster DISABLED_PartialRaster
#else
#define MAYBE_PartialRaster PartialRaster
#endif
TEST_P(LayerTreeHostTilesTestPartialInvalidationMultiThread, MAYBE_PartialRaster)
{
    use_partial_raster_ = true;
    base::FilePath expected_result = base::FilePath(FILE_PATH_LITERAL("blue_yellow_partial_flipped.png"));
    if (use_skia_graphite()) {
...
INSTANTIATE_TEST_SUITE_P(All, LayerTreeHostTilesTestRasterColorSpace, ::testing::ValuesIn(kTestCases), ::testing::PrintToStringParamName());
...
TEST_P(LayerTreeHostTilesTestRasterColorSpace, sRGB)
{
    ...
    RunPixelTest(picture_layer_, base::FilePath(FILE_PATH_LITERAL("primary_colors.png")));
}
...
#endif
    SetColorSpace(gfx::ColorSpace(gfx::ColorSpace::PrimaryID::APPLE_GENERIC_RGB, gfx::ColorSpace::TransferID::GAMMA18));

    // Software rasterizer ignores XYZD50 matrix
    const auto* target_png
        = renderer_type() == viz::RendererType::kSoftware ? FILE_PATH_LITERAL("primary_colors.png") : FILE_PATH_LITERAL("primary_colors_sRGB_in_AdobeRGB.png");
...
TEST_P(LayerTreeHostTilesTestRasterColorSpace, CustomColorSpace)
{
    ...
    SetColorSpace(gfx::ColorSpace::CreateCustom(to_XYZD50, gfx::ColorSpace::TransferID::SRGB));

    // Software rasterizer ignores XYZD50 matrix
    const auto* target_png
        = renderer_type() == viz::RendererType::kSoftware ? FILE_PATH_LITERAL("primary_colors.png") : FILE_PATH_LITERAL("primary_colors_icced.png");
    RunPixelTest(picture_layer_, base::FilePath(target_png));
}
...
INSTANTIATE_TEST_SUITE_P(All, LayerTreeHostTilesTestPartialInvalidationLowBitDepth,
    ::testing::Values(RasterTestConfig { viz::RendererType::kSkiaGL, TestRasterType::kGpu }), ::testing::PrintToStringParamName());

TEST_P(LayerTreeHostTilesTestPartialInvalidationLowBitDepth, DISABLED_PartialRaster)
{
    use_partial_raster_ = true;
    RunSingleThreadedPixelTest(picture_layer_, base::FilePath(FILE_PATH_LITERAL("blue_yellow_partial_flipped_dither.png")));
}

TEST_P(LayerTreeHostTilesTestPartialInvalidationLowBitDepth, DISABLED_FullRaster)
{
    RunSingleThreadedPixelTest(picture_layer_, base::FilePath(FILE_PATH_LITERAL("blue_yellow_flipped_dither.png")));
}
#endif // BUILDFLAG(ENABLE_GL_BACKEND_TESTS)
