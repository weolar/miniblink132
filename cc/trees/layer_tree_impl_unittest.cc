...
public:
    ...
    LayerImpl* HitTestSimpleTree(int top_sorting_context, int left_child_sorting_context, int right_child_sorting_context, float top_depth,
        float left_child_depth, float right_child_depth)
    {
...
TEST_F(LayerTreeImplTest, UpdateViewportAndHitTest)
{
    ...
    host_impl().active_tree()->SetDeviceViewportRect(gfx::Rect(root->bounds()));
    UpdateDrawProperties(host_impl().active_tree());
    EXPECT_EQ(gfx::RectF(gfx::SizeF(bounds)), host_impl().active_tree()->property_trees()->clip_tree().ViewportClip());
    ...
    EXPECT_EQ(gfx::RectF(gfx::SizeF(new_bounds)), host_impl().active_tree()->property_trees()->clip_tree().ViewportClip());
...
TEST_F(LayerTreeImplTest, HitTestingForSinglePositionedLayer)
{
    ...
    host_impl().active_tree()->SetDeviceViewportRect(gfx::Rect(test_layer->bounds()));
    UpdateDrawProperties(host_impl().active_tree());
...
TEST_F(LayerTreeImplTest, HitTestingForSinglePerspectiveLayer)
{
    ...
    root->SetBounds(layer->bounds());
    CopyProperties(root, layer);
    CreateTransformNode(layer).local = (perspective_projection_about_center * translation_by_z);
...
TEST_F(LayerTreeImplTest, HitTestingForMultiClippedRotatedLayer)
{
    ...
    rotated_leaf_transform.Translate(-10.0, -10.0); // cancel out the grand_parent's position
    rotated_leaf_transform.RotateAboutZAxis(-45.0); // cancel out the corner 45-degree rotation of the parent.
...
TEST_F(LayerTreeImplTest, HitTestingForMultipleLayers)
{
    ...
    grand_child1->SetOffsetToTransformParent(gfx::Vector2dF(0.f, 40.f) + child1->offset_to_transform_parent());
...
TEST_F(LayerTreeImplTest, HitTestingForMultipleLayerLists)
{
    ...
    grand_child1->SetHitTestOpaqueness(HitTestOpaqueness::kMixed);
    CopyProperties(child1, grand_child1);
    CreateTransformNode(grand_child1).post_translation = gfx::Vector2dF(0.f, 40.f);
...
TEST_F(LayerTreeImplTest, HitCheckingTouchHandlerRegionsForSingleLayer)
{
    ...
    LayerImpl* result_layer = host_impl().active_tree()->FindLayerThatIsHitByPointInTouchHandlerRegion(test_point);
    EXPECT_FALSE(result_layer);
...
TEST_F(LayerTreeImplTest, HitCheckingTouchHandlerRegionsForUninvertibleTransform)
{
    LayerImpl* root = root_layer();
...
TEST_F(LayerTreeImplTest, HitCheckingTouchHandlerRegionsForSinglePositionedLayer)
{
    TouchActionRegion touch_action_region;
    touch_action_region.Union(TouchAction::kNone, gfx::Rect(10, 10, 50, 50));
...
TEST_F(LayerTreeImplTest, HitCheckingTouchHandlerRegionsForSingleLayerWithDeviceScale)
{
    ...
    gfx::Size scaled_bounds_for_root = gfx::ScaleToCeiledSize(root->bounds(), device_scale_factor * page_scale_factor);
    host_impl().active_tree()->SetDeviceViewportRect(gfx::Rect(scaled_bounds_for_root));

    host_impl().active_tree()->SetDeviceScaleFactor(device_scale_factor);
    ViewportPropertyIds viewport_property_ids;
    viewport_property_ids.page_scale_transform = page_scale_layer->transform_tree_index();
    host_impl().active_tree()->SetViewportPropertyIds(viewport_property_ids);
    host_impl().active_tree()->PushPageScaleFromMainThread(page_scale_factor, page_scale_factor, max_page_scale_factor);
    ...
    host_impl().active_tree()->SetPageScaleOnActiveTree(page_scale_factor);
    EXPECT_EQ(page_scale_layer->transform_tree_index(), host_impl().active_tree()->PageScaleTransformNode()->id);
...
TEST_F(LayerTreeImplTest, HitCheckingTouchHandlerRegionsForSimpleClippedLayer)
{
    ...
    CopyProperties(clipping_layer, child);
    child->SetOffsetToTransformParent(gfx::Vector2dF(-50.f, -50.f) + clipping_layer->offset_to_transform_parent());
...
TEST_F(LayerTreeImplTest, HitCheckingTouchHandlerRegionsForClippedLayerWithDeviceScale)
{
    // The layer's device_scale_factor and page_scale_factor should scale the
    // content rect and we should be able to hit the touch handler region by
    // scaling the points accordingly.
...
TEST_F(LayerTreeImplTest, HitTestingTouchHandlerRegionsForLayerThatIsNotDrawn)
{
    ...
    EXPECT_FALSE(test_layer->contributes_to_drawn_render_surface());
    EXPECT_TRANSFORM_EQ(
        expected_screen_space_transform, draw_property_utils::ScreenSpaceTransform(test_layer, host_impl().active_tree()->property_trees()->transform_tree()));
...
TEST_F(LayerTreeImplTest, SelectionBoundsForSingleLayer)
{
    ...
    EXPECT_EQ(gfx::PointF(input.start.edge_start), output.start.visible_edge_start());
    ...
    EXPECT_EQ(gfx::PointF(insertion_input.start.edge_end), output.start.edge_end());
    EXPECT_EQ(gfx::PointF(insertion_input.start.edge_start), output.start.edge_start());
    EXPECT_EQ(gfx::PointF(insertion_input.start.edge_end), output.start.visible_edge_end());
    EXPECT_EQ(gfx::PointF(insertion_input.start.edge_start), output.start.visible_edge_start());
    EXPECT_TRUE(output.start.visible());
...
TEST_F(LayerTreeImplTest, SelectionBoundsForPartialOccludedLayers)
{
    ...
    clipped_layer->SetOffsetToTransformParent(clipping_layer->offset_to_transform_parent());
    ...
    std::tie(expected_output_visible_edge_start, expected_output_visible_edge_end)
        = GetVisibleSelectionEndPoints(visible_layer_rect, gfx::PointF(input.end.edge_start), gfx::PointF(input.end.edge_end));
    expected_output_visible_edge_start.Offset(clipping_offset.x(), clipping_offset.y());
    expected_output_visible_edge_end.Offset(clipping_offset.x(), clipping_offset.y());

    EXPECT_EQ(expected_output_visible_edge_start, output.end.visible_edge_start());
...
TEST_F(LayerTreeImplTest, SelectionBoundsForScaledLayers)
{
    ...
    host_impl().active_tree()->PushPageScaleFromMainThread(page_scale_factor, page_scale_factor, page_scale_factor);
    host_impl().active_tree()->SetPageScaleOnActiveTree(page_scale_factor);
    UpdateDrawProperties(host_impl().active_tree());
...
TEST_F(LayerTreeImplTest, SelectionBoundsForDSFEnabled)
{
    ...
    host_impl().active_tree()->SetDeviceScaleFactor(device_scale_factor);
    host_impl().active_tree()->set_painted_device_scale_factor(painted_device_scale_factor);
    ...
    auto expected_output_edge_start = gfx::PointF(input.start.edge_start);
    auto expected_output_edge_end = gfx::PointF(input.start.edge_end);
    expected_output_edge_start.Scale(1.f / (device_scale_factor * painted_device_scale_factor));
    expected_output_edge_end.Scale(1.f / (device_scale_factor * painted_device_scale_factor));
    EXPECT_EQ(expected_output_edge_start, output.start.edge_start());
    EXPECT_EQ(expected_output_edge_end, output.start.edge_end());
    EXPECT_EQ(expected_output_edge_start, output.start.visible_edge_start());
    EXPECT_EQ(expected_output_edge_end, output.start.visible_edge_end());
...
TEST_F(LayerTreeImplTest, SelectionBoundsWithLargeTransforms)
{
    ...
    auto point_is_valid = [](const gfx::PointF& p) { return std::isfinite(p.x()) && std::isfinite(p.y()); };
    auto selection_bound_is_valid = [&](const gfx::SelectionBound& b) {
        return point_is_valid(b.edge_start()) && point_is_valid(b.visible_edge_start()) && point_is_valid(b.edge_end()) && point_is_valid(b.visible_edge_end());
    };
    // No NaNs or infinities in SelectounBound.
    EXPECT_TRUE(selection_bound_is_valid(output.start)) << output.start.ToString();
...
{
    host_impl().active_tree()->SetDisplayColorSpaces(gfx::DisplayColorSpaces(gfx::ColorSpace::CreateXYZD50()));
    host_impl().active_tree()->UpdateDrawProperties(
        /*update_tiles=*/true, /*update_image_animation_controller=*/true);
    EXPECT_FALSE(host_impl().active_tree()->needs_update_draw_properties());
    host_impl().active_tree()->SetDisplayColorSpaces(gfx::DisplayColorSpaces(gfx::ColorSpace::CreateSRGB()));
    EXPECT_FALSE(host_impl().active_tree()->needs_update_draw_properties());
}

TEST_F(LayerTreeImplTest, HitTestingCorrectLayerWheelListener)
{
    host_impl().active_tree()->set_event_listener_properties(EventListenerClass::kMouseWheel, EventListenerProperties::kBlocking);
...
TEST_F(LayerTreeImplTest, DebugRectHistoryLayoutShiftWithoutHud)
{
    ...
    history->SaveDebugRectsForCurrentFrame(host_impl().active_tree(), nullptr, RenderSurfaceList {}, state);
...
TEST_F(LayerTreeImplTest, PersistentSwapPromisesAreKeptAlive)
{
    ...
    std::vector<std::unique_ptr<PersistentSwapPromise>> persistent_promises_to_pass;
    for (size_t i = 0; i < promises_count; ++i) {
        persistent_promises_to_pass.push_back(std::make_unique<PersistentSwapPromise>());
    }
    ...
    host_impl().active_tree()->BreakSwapPromises(SwapPromise::DidNotSwapReason::SWAP_FAILS);
...
TEST_F(LayerTreeImplTest, NotPersistentSwapPromisesAreDroppedWhenSwapFails)
{
    ...
    std::vector<std::unique_ptr<NotPersistentSwapPromise>> not_persistent_promises_to_pass;
    for (size_t i = 0; i < promises_count; ++i) {
        not_persistent_promises_to_pass.push_back(std::make_unique<NotPersistentSwapPromise>());
    }
    ...
    {
        std::unique_ptr<NotPersistentSwapPromise> promise(new NotPersistentSwapPromise());
...
TEST_F(LayerTreeImplTest, TrackPictureLayersWithPaintWorklets)
{
    ...
    scoped_refptr<RasterSource> raster_source1(FakeRasterSource::CreateFilledWithPaintWorklet(child1->bounds()));
    child1->UpdateRasterSource(raster_source1, &empty_invalidation);
    scoped_refptr<RasterSource> raster_source3(FakeRasterSource::CreateFilledWithPaintWorklet(child3->bounds()));
    child3->UpdateRasterSource(raster_source3, &empty_invalidation);

    // The set should correctly track which layers are in it.
    const base::flat_set<raw_ptr<PictureLayerImpl, CtnExperimental>>& layers = pending_tree->picture_layers_with_paint_worklets();
    ...
    scoped_refptr<RasterSource> empty_raster_source(FakeRasterSource::CreateFilled(child1->bounds()));
...
{
    const gfx::MaskFilterInfo kMaskFilterWithRoundedCorners(gfx::RectF(5, 5), gfx::RoundedCornersF(2.5), gfx::LinearGradient());
    ...
    render_surface->AppendQuads(DRAW_MODE_HARDWARE, render_pass.get(), &append_quads_data);

    ASSERT_EQ(1u, render_pass->shared_quad_state_list.size());
    viz::SharedQuadState* shared_quad_state = render_pass->shared_quad_state_list.front();
...
TEST_F(LayerTreeImplOcclusionTest, Occlusion)
{
    ...
    EXPECT_TRUE(bottom_layer->draw_properties().occlusion_in_content_space.HasOcclusion());
}
