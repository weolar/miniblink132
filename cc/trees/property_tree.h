...
using SyncedScrollOffset = SyncedProperty<AdditionGroup<gfx::PointF, gfx::Vector2dF>>;

class PropertyTrees;

template <typename T> class CC_EXPORT PropertyTree {
...
public:
    ...
    T* back()
    {
        return size() ? &nodes_.back() : nullptr;
    }
    const T* back() const
    {
        return size() ? &nodes_.back() : nullptr;
...
public:
    ...
    bool OnTransformAnimated(ElementId element_id, const gfx::Transform& transform);
    ...
    void UpdateTransforms(int id, const ViewportPropertyIds* viewport_property_ids = nullptr, UpdateTransformsData* update_data = nullptr);
    void UpdateTransformChanged(TransformNode* node, TransformNode* parent_node);
    void UpdateNodeAndAncestorsAreAnimatedOrInvertible(TransformNode* node, TransformNode* parent_node);
    void UpdateNodeOrAncestorsWillChangeTransform(TransformNode* node, TransformNode* parent_node);
    ...
    const std::vector<int>& nodes_affected_by_outer_viewport_bounds_delta() const
    {
        return nodes_affected_by_outer_viewport_bounds_delta_;
    }
    void set_nodes_affected_by_outer_viewport_bounds_delta(std::vector<int> nodes)
    {
    ...
    const std::vector<AnchorPositionScrollData>& anchor_position_scroll_data() const
    {
    ...
    void UndoOverscroll(const TransformNode* node, gfx::Vector2dF& position_adjustment, const ViewportPropertyIds* viewport_property_ids);
    ...
    const AnchorPositionScrollData* GetAnchorPositionScrollData(int node_id) const;
    ...
    void CombineTransformsBetween(int source_id, int dest_id, gfx::Transform* transform) const;
    ...
    // two nodes must be on the same ancestor chain.
    bool CombineInversesBetween(int source_id, int dest_id, gfx::Transform* transform) const;
...
private:
    ...
    gfx::Vector2dF AnchorPositionOffset(TransformNode* node, int max_updated_node_id, UpdateTransformsData* update_data);
    void UpdateLocalTransform(TransformNode* node, const ViewportPropertyIds* viewport_property_ids, UpdateTransformsData* update_data);
    void UpdateScreenSpaceTransform(TransformNode* node, TransformNode* parent_node);
    void UpdateAnimationProperties(TransformNode* node, TransformNode* parent_node);
    void UndoSnapping(TransformNode* node);
    void UpdateSnapping(TransformNode* node);
    void UpdateNodeAndAncestorsHaveIntegerTranslations(TransformNode* node, TransformNode* parent_node);
...
public:
    ...
    bool OnBackdropFilterAnimated(ElementId id, const FilterOperations& backdrop_filters);
    ...
    typedef std::unordered_multimap<int, std::unique_ptr<viz::CopyOutputRequest>> CopyRequestMap;

    void AddCopyRequest(int node_id, std::unique_ptr<viz::CopyOutputRequest> request);
    void PullCopyRequestsFrom(CopyRequestMap& new_copy_requests);
    void PushCopyRequestsTo(EffectTree* other_tree);
    void TakeCopyRequestsAndTransformToSurface(int node_id, std::vector<std::unique_ptr<viz::CopyOutputRequest>>* requests);
    bool HasCopyRequests() const;
    void ClearCopyRequests();
    void GetRenderSurfaceChangedFlags(std::vector<RenderSurfacePropertyChangedFlags>& flags) const;
    void ApplyRenderSurfaceChangedFlags(const std::vector<RenderSurfacePropertyChangedFlags>& flags);
    ...
    void TakeRenderSurfaces(std::vector<std::unique_ptr<RenderSurfaceImpl>>* render_surfaces);

    // Returns true if render surfaces changed (that is, if any render surfaces
    // were created or destroyed).
    bool CreateOrReuseRenderSurfaces(std::vector<std::unique_ptr<RenderSurfaceImpl>>* old_render_surfaces, LayerTreeImpl* layer_tree_impl);
...
private:
    ...
    void UpdateClosestAncestorSharedElement(EffectNode* node, EffectNode* parent_node);

    // Stores copy requests, keyed by node id.
    std::unordered_multimap<int, std::unique_ptr<viz::CopyOutputRequest>> copy_requests_;
...
public:
    ...
    virtual void DidCompositorScroll(ElementId scroll_element_id, const gfx::PointF&, const std::optional<TargetSnapAreaElementIds>&) = 0;
    ...
    virtual void DidChangeScrollbarsHidden(ElementId scroll_element_id, bool hidden) = 0;
...
public:
    ...
    void OnScrollOffsetAnimated(ElementId id, int scroll_tree_index, const gfx::PointF& scroll_offset, LayerTreeImpl* layer_tree_impl);
    ...
    int currently_scrolling_node() const
    {
        return currently_scrolling_node_id_;
    }
    gfx::Transform ScreenSpaceTransform(int scroll_node_id) const;

    gfx::Vector2dF ClampScrollToMaxScrollOffset(const ScrollNode& node, LayerTreeImpl*);
    ...
    void CollectScrollDeltas(CompositorCommitData* commit_data, ElementId inner_viewport_scroll_element_id, bool use_fractional_deltas,
    ...
    void ApplySentScrollDeltasFromAbortedCommit(bool next_bmf, bool main_frame_applied_deltas);

    // Pushes scroll updates from the ScrollTree on the main thread onto the
    // impl thread associated state.
    void PushScrollUpdatesFromMainThread(const PropertyTrees& main_property_trees, LayerTreeImpl* sync_tree, bool use_fractional_deltas);

    // Pushes scroll updates from the ScrollTree on the pending tree onto the
    // active tree associated state.
    void PushScrollUpdatesFromPendingTree(PropertyTrees* pending_property_trees, LayerTreeImpl* active_tree);
    ...
    bool UpdateScrollOffsetBaseForTesting(ElementId id, const gfx::PointF& offset);
    bool SetScrollOffsetDeltaForTesting(ElementId id, const gfx::Vector2dF& delta);
    ...
    gfx::Vector2dF ScrollBy(const ScrollNode& scroll_node, const gfx::Vector2dF& scroll, LayerTreeImpl* layer_tree_impl);
    gfx::PointF ClampScrollOffsetToLimits(gfx::PointF offset, const ScrollNode& scroll_node) const;
    ...
    void NotifyDidChangeScrollbarsHidden(ElementId scroll_element_id, bool hidden) const;
...
private:
    ...
    using SyncedScrollOffsetMap = base::flat_map<ElementId, scoped_refptr<SyncedScrollOffset>>;
    ...
    gfx::Vector2dF PullDeltaForMainThread(SyncedScrollOffset* scroll_offset, bool use_fractional_deltas, bool next_bmf);
};
...
public:
    ...
    const ClipTree& clip_tree() const
    {
        return clip_tree_;
    }
    ClipTree& clip_tree_mutable()
    {
        return clip_tree_;
    }
    ...
    {
        return needs_rebuild_.Read(synchronizer());
    }

    void set_changed(bool value)
    {
        changed_.Write(synchronizer()) = value;
    }
    bool changed() const
    {
        return changed_.Read(synchronizer());
    }
    ...
    {
        return is_main_thread_.Read(synchronizer());
    }

    void set_is_active(bool value)
    {
        is_active_.Write(synchronizer()) = value;
    }
    bool is_active() const
    {
        return is_active_.Read(synchronizer());
    }
    ...
    {
        sequence_number_.Write(synchronizer())++;
    }
    int sequence_number() const
    {
        return sequence_number_.Read(synchronizer());
    }
    ...
    // needed.
    bool ElementIsAnimatingChanged(
    ...
    void GetChangedNodes(std::vector<int>& effect_nodes, std::vector<int>& transform_nodes) const;
    void ApplyChangedNodes(const std::vector<int>& effect_nodes, const std::vector<int>& transform_nodes);
    ...
    void SetMaximumAnimationToScreenScaleForTesting(int transform_id, float maximum_scale, bool affected_by_invalid_scale);

    bool GetToTarget(int transform_id, int effect_id, gfx::Transform* to_target) const;
    bool GetFromTarget(int transform_id, int effect_id, gfx::Transform* from_target) const;

    void ResetCachedData();
    void UpdateTransformTreeUpdateNumber();
    gfx::Transform ToScreenSpaceTransformWithoutSurfaceContentsScale(int transform_id, int effect_id) const;
...
private:
    ...
    ProtectedSequenceReadable<gfx::Vector2dF> inner_viewport_container_bounds_delta_;
    ProtectedSequenceReadable<gfx::Vector2dF> outer_viewport_container_bounds_delta_;
    ...
    DrawTransformData& FetchDrawTransformsDataFromCache(int transform_id, int effect_id) const;
