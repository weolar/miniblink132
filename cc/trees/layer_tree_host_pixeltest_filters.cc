...
class LayerTreeHostFiltersPixelTest : public LayerTreePixelTest, public ::testing::WithParamInterface<viz::RendererType> {
protected:
    ...
    scoped_refptr<SolidColorLayer> BuildFilterWithGiantCropRect(bool masks_to_bounds)
    {
        ...
        PaintFilter::CropRect cropRect(SkRect::MakeXYWH(-40000, -40000, 80000, 80000));
        filters.Append(FilterOperation::CreateReferenceFilter(sk_make_sp<ColorFilterPaintFilter>(ColorFilter::MakeMatrix(matrix), nullptr, &cropRect)));
...
INSTANTIATE_TEST_SUITE_P(All, LayerTreeHostFiltersPixelTest, ::testing::ValuesIn(viz::GetRendererTypes()), ::testing::PrintToStringParamName());
...
INSTANTIATE_TEST_SUITE_P(All, LayerTreeHostFiltersPixelTestGPU, ::testing::ValuesIn(viz::GetGpuRendererTypes()), ::testing::PrintToStringParamName());
...
TEST_P(LayerTreeHostFiltersPixelTest, BackdropFilterBlurRect)
{
    ...
    scoped_refptr<SolidColorLayer> green = CreateSolidColorLayer(gfx::Rect(50, 50, 100, 100), kCSSGreen);
    scoped_refptr<SolidColorLayer> blur = CreateSolidColorLayer(gfx::Rect(30, 30, 140, 140), SK_ColorTRANSPARENT);
    ...
    pixel_comparator_ = std::make_unique<FuzzyPixelComparator>(FuzzyPixelComparator()
                                                                   .DiscardAlpha()
                                                                   .SetErrorPixelsPercentageLimit(1.09f) // 436px / (200*200)
                                                                   .SetAbsErrorLimit(1));
#endif

    RunPixelTest(background, base::FilePath(FILE_PATH_LITERAL("backdrop_filter_blur_.png")).InsertBeforeExtensionASCII(GetRendererSuffix()));
}
...
TEST_P(LayerTreeHostFiltersPixelTest, BackdropFilterInvalid)
{
    ...
    RunPixelTest(background, base::FilePath(FILE_PATH_LITERAL("backdrop_filter_invalid.png")));
}
...
TEST_P(LayerTreeHostFiltersPixelTest, MAYBE_BackdropFilterBlurRadius)
{
    ...
    scoped_refptr<SolidColorLayer> background = CreateSolidColorLayer(gfx::Rect(400, 400), SK_ColorRED);
    scoped_refptr<SolidColorLayer> green = CreateSolidColorLayer(gfx::Rect(200, 200, 200, 200), kCSSLime);
    gfx::Rect blur_rect(100, 100, 200, 200);
    scoped_refptr<SolidColorLayer> blur = CreateSolidColorLayer(blur_rect, SkColorSetARGB(40, 10, 20, 200));
    background->AddChild(green);
    background->AddChild(blur);
    ...
    pixel_comparator_ = std::make_unique<FuzzyPixelComparator>(
        FuzzyPixelComparator().DiscardAlpha().SetErrorPixelsPercentageLimit(percentage_pixels_error).SetAvgAbsErrorLimit(1.f).SetAbsErrorLimit(2));
#endif
    RunPixelTest(background, base::FilePath(FILE_PATH_LITERAL("backdrop_filter_blur_radius_.png")).InsertBeforeExtensionASCII(GetRendererSuffix()));
}
...
TEST_P(LayerTreeHostFiltersPixelTest, BackdropFilterBlurRounded)
{
    ...
    // should appear blurred on its edges.
    scoped_refptr<SolidColorLayer> green = CreateSolidColorLayer(gfx::Rect(50, 50, 100, 100), kCSSGreen);
    scoped_refptr<SolidColorLayer> blur = CreateSolidColorLayer(gfx::Rect(46, 46, 108, 108), SK_ColorTRANSPARENT);
    background->AddChild(green);
    background->AddChild(blur);
    ...
    filters.Append(FilterOperation::CreateBlurFilter(2.f, SkTileMode::kClamp));
    blur->SetBackdropFilters(filters);
    gfx::RRectF backdrop_filter_bounds(gfx::RectF(gfx::SizeF(blur->bounds())), 14, 16, 18, 20, 22, 30, 40, 50);
    blur->SetBackdropFilterBounds(backdrop_filter_bounds);
    ...
    pixel_comparator_ = std::make_unique<FuzzyPixelComparator>(FuzzyPixelComparator()
                                                                   .DiscardAlpha()
                                                                   .SetErrorPixelsPercentageLimit(percentage_pixels_large_error, percentage_pixels_small_error)
                                                                   .SetAvgAbsErrorLimit(average_error_allowed_in_bad_pixels)
                                                                   .SetAbsErrorLimit(large_error_limit, small_error_threshold));

    RunPixelTest(background,
        use_software_renderer() ? base::FilePath(FILE_PATH_LITERAL("backdrop_filter_blur_rounded_sw.png"))
...
TEST_P(LayerTreeHostFiltersPixelTest, BackdropFilterBlurOutsets)
{
    ...
    scoped_refptr<SolidColorLayer> green_border = CreateSolidColorLayerWithBorder(gfx::Rect(1, 1, 198, 198), SK_ColorWHITE, 10, kCSSGreen);
    scoped_refptr<SolidColorLayer> blur = CreateSolidColorLayer(gfx::Rect(12, 12, 176, 176), SK_ColorTRANSPARENT);
...
#endif
    pixel_comparator_ = std::make_unique<FuzzyPixelComparator>(
        FuzzyPixelComparator().DiscardAlpha().SetErrorPixelsPercentageLimit(percentage_pixels_error).SetAbsErrorLimit(2));
#else
    if (use_skia_vulkan()) {
        pixel_comparator_ = std::make_unique<AlphaDiscardingFuzzyPixelOffByOneComparator>();
    }
#endif

    RunPixelTest(background, base::FilePath(FILE_PATH_LITERAL("backdrop_filter_blur_outsets.png")));
}

class LayerTreeHostBlurFiltersPixelTestGPULayerList : public LayerTreeHostFiltersPixelTest {
public:
    ...
    void SetupTree() override
    {
        ...
        TransformNode& background_transform_node = CreateTransformNode(background.get());
        ...
        CreateSolidColorLayerPlusBorders(gfx::Rect(0, 0, 120, 120), SK_ColorTRANSPARENT, 1, SK_ColorBLACK, blur_layers);
        CopyProperties(background.get(), blur_layers[0].get());
        TransformNode& blur_transform_node = CreateTransformNode(blur_layers[0].get(), background_transform_node.id);
        ...
        blur_effect_node.render_surface_reason = RenderSurfaceReason::kBackdropFilter;
...
INSTANTIATE_TEST_SUITE_P(
    PixelResourceTest, LayerTreeHostBlurFiltersPixelTestGPULayerList, ::testing::ValuesIn(viz::GetGpuRendererTypes()), ::testing::PrintToStringParamName());

// viz::GetGpuRendererTypes() can return an empty list on some platforms.
GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(LayerTreeHostBlurFiltersPixelTestGPULayerList);

// TODO(michaelludwig): Re-enable after Skia roll and update expected images.
// See skbug.com/9545
TEST_P(LayerTreeHostBlurFiltersPixelTestGPULayerList, DISABLED_BackdropFilterBlurOffAxis)
{
#if BUILDFLAG(IS_WIN) || defined(ARCH_CPU_ARM64)
#if BUILDFLAG(IS_WIN)
    ...
    RunPixelTestWithLayerList(base::FilePath(FILE_PATH_LITERAL("backdrop_filter_blur_off_axis_.png")).InsertBeforeExtensionASCII(GetRendererSuffix()));
}
...
TEST_P(LayerTreeHostFiltersPixelTest, BackdropFilterBoundsWithChildren)
{
    scoped_refptr<SolidColorLayer> background = CreateSolidColorLayer(gfx::Rect(200, 200), SK_ColorWHITE);
    scoped_refptr<SolidColorLayer> green = CreateSolidColorLayer(gfx::Rect(50, 50, 100, 100), kCSSGreen);
    scoped_refptr<SolidColorLayer> invert = CreateSolidColorLayer(gfx::Rect(30, 30, 140, 40), SK_ColorTRANSPARENT);
    scoped_refptr<SolidColorLayer> invert_child1 = CreateSolidColorLayer(gfx::Rect(50, -20, 40, 20), kCSSOrange);
    scoped_refptr<SolidColorLayer> invert_child2 = CreateSolidColorLayer(gfx::Rect(50, 40, 40, 20), kCSSOrange);
    background->AddChild(green);
    ...
    gfx::RRectF backdrop_filter_bounds(gfx::RectF(gfx::SizeF(invert->bounds())), 0);
    invert->SetBackdropFilterBounds(backdrop_filter_bounds);

    RunPixelTest(background, base::FilePath(FILE_PATH_LITERAL("backdrop_filter_bounds_with_children.png")));
}

class LayerTreeHostFiltersScaledPixelTest : public LayerTreeHostFiltersPixelTest {
    void SetupTree() override
    {
...
protected:
    void RunPixelTestType(int content_size, float device_scale_factor)
    {
        ...
        scoped_refptr<SolidColorLayer> background = CreateSolidColorLayer(gfx::Rect(0, 0, content_size, content_size), SK_ColorGREEN);

        // Add a blue layer that completely covers the green layer.
        scoped_refptr<SolidColorLayer> foreground = CreateSolidColorLayer(gfx::Rect(0, 0, content_size, content_size), SK_ColorBLUE);
        ...
        alpha_shape.emplace_back(half_content, half_content, content_size, content_size);
        filters.Append(FilterOperation::CreateAlphaThresholdFilter(alpha_shape));
        foreground->SetFilters(filters);

        device_scale_factor_ = device_scale_factor;
        RunPixelTest(background, base::FilePath(FILE_PATH_LITERAL("green_small_with_blue_corner.png")));
    }

    float device_scale_factor_;
};

INSTANTIATE_TEST_SUITE_P(All, LayerTreeHostFiltersScaledPixelTest, ::testing::ValuesIn(viz::GetRendererTypes()), ::testing::PrintToStringParamName());

// viz::GetRendererTypes() can return an empty list on some platforms.
GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(LayerTreeHostFiltersScaledPixelTest);
...
TEST_P(LayerTreeHostFiltersPixelTest, CroppedFilter)
{
    ...
    sk_sp<PaintFilter> offset(sk_make_sp<OffsetPaintFilter>(0, 0, nullptr, &cropRect));
...
TEST_P(LayerTreeHostFiltersPixelTest, ImageFilterClipped)
{
    ...
    FilterOperations filters;
    filters.Append(FilterOperation::CreateReferenceFilter(sk_make_sp<ColorFilterPaintFilter>(ColorFilter::MakeMatrix(matrix), nullptr, &crop_rect)));
    ...
    RunPixelTest(background, base::FilePath(FILE_PATH_LITERAL("blue_yellow.png")));
}
...
#elif BUILDFLAG(IS_WIN) || BUILDFLAG(IS_MAC) || BUILDFLAG(IS_CHROMEOS) || defined(_MIPS_ARCH_LOONGSON) || defined(ARCH_CPU_ARM64)
#if BUILDFLAG(IS_WIN)
...
#endif
    pixel_comparator_ = std::make_unique<FuzzyPixelComparator>(
        FuzzyPixelComparator().DiscardAlpha().SetErrorPixelsPercentageLimit(percentage_pixels_error).SetAvgAbsErrorLimit(1.f).SetAbsErrorLimit(error_allowed));
#endif

    RunPixelTest(background, base::FilePath(FILE_PATH_LITERAL("backdrop_filter_on_scaled_layer_.png")).InsertBeforeExtensionASCII(GetRendererSuffix()));
}
...
TEST_P(LayerTreeHostFiltersPixelTest, MAYBE_BackdropFilterRotated)
{
    ...
    scoped_refptr<SolidColorLayer> filter_layer = CreateSolidColorLayer(gfx::Rect(100, 0, 100, 200), transparent_blue);
    ...
    gfx::RRectF backdrop_filter_bounds(gfx::RectF(gfx::SizeF(filter_layer->bounds())), 0);
    ...
    pixel_comparator_ = std::make_unique<FuzzyPixelComparator>(FuzzyPixelComparator().DiscardAlpha().SetErrorPixelsPercentageLimit(5.f).SetAbsErrorLimit(3));

    RunPixelTest(background, base::FilePath(FILE_PATH_LITERAL("backdrop_filter_rotated_.png")).InsertBeforeExtensionASCII(GetRendererSuffix()));
}
...
TEST_P(LayerTreeHostFiltersPixelTest, MAYBE_ImageRenderSurfaceScaled)
{
    ...
    RunPixelTest(background, base::FilePath(FILE_PATH_LITERAL("scaled_render_surface_layer_.png")).InsertBeforeExtensionASCII(GetRendererSuffix()));
}
...
TEST_P(LayerTreeHostFiltersPixelTest, MAYBE_ZoomFilter)
{
    ...
    root->AddChild(right);
    scoped_refptr<SolidColorLayer> top_horizontal_strip = CreateSolidColorLayer(gfx::Rect(0, 10, 300, 10), SK_ColorGREEN);
    root->AddChild(top_horizontal_strip);
    scoped_refptr<SolidColorLayer> bottom_horizontal_strip = CreateSolidColorLayer(gfx::Rect(0, 275, 300, 10), SK_ColorGREEN);
    ...
    border_filters.Append(FilterOperation::CreateZoomFilter(2.f /* zoom */, 0 /* inset */));
    border_edge_zoom->SetBackdropFilters(border_filters);
    gfx::RectF border_filter_bounds(gfx::SizeF(border_edge_zoom->bounds()));
    border_edge_zoom->SetBackdropFilterBounds(gfx::RRectF(border_filter_bounds, 0));
    ...
    top_filters.Append(FilterOperation::CreateZoomFilter(2.f /* zoom */, 0 /* inset */));
    ...
    scoped_refptr<SolidColorLayer> top_left_zoom = CreateSolidColorLayer(gfx::Rect(-24, -24, 50, 50), SK_ColorTRANSPARENT);
    FilterOperations top_left_filters;
    top_left_filters.Append(FilterOperation::CreateZoomFilter(1.25f /* zoom */, 0 /* inset */));
    top_left_zoom->SetBackdropFilters(top_left_filters);
    gfx::RectF top_left_filter_bounds(gfx::SizeF(top_left_zoom->bounds()));
    top_left_zoom->SetBackdropFilterBounds(gfx::RRectF(top_left_filter_bounds, 0));
    root->AddChild(top_left_zoom);

    // Test a zoom centered near the top right of the screen.
    scoped_refptr<SolidColorLayer> top_right_zoom = CreateSolidColorLayer(gfx::Rect(274, -24, 50, 50), SK_ColorTRANSPARENT);
    FilterOperations top_right_filters;
    top_right_filters.Append(FilterOperation::CreateZoomFilter(4.0f /* zoom */, 0 /* inset */));
    top_right_zoom->SetBackdropFilters(top_right_filters);
    gfx::RectF top_right_filter_bounds(gfx::SizeF(top_right_zoom->bounds()));
    top_right_zoom->SetBackdropFilterBounds(gfx::RRectF(top_right_filter_bounds, 0));
    root->AddChild(top_right_zoom);

    // Test a zoom centered near the bottom right of the screen.
    scoped_refptr<SolidColorLayer> bottom_right_zoom = CreateSolidColorLayer(gfx::Rect(270, 270, 50, 50), SK_ColorTRANSPARENT);
    FilterOperations bottom_right_filters;
    bottom_right_filters.Append(FilterOperation::CreateZoomFilter(1.25f /* zoom */, 0 /* inset */));
    bottom_right_zoom->SetBackdropFilters(bottom_right_filters);
    gfx::RectF bottom_right_filter_bounds(gfx::SizeF(bottom_right_zoom->bounds()));
    bottom_right_zoom->SetBackdropFilterBounds(gfx::RRectF(bottom_right_filter_bounds, 0));
    ...
    mid_filters.Append(FilterOperation::CreateZoomFilter(2.f /* zoom */, 0 /* inset */));
    ...
    RunPixelTest(std::move(root), base::FilePath(FILE_PATH_LITERAL("zoom_filter_.png")).InsertBeforeExtensionASCII(GetRendererSuffix()));
}
...
TEST_P(LayerTreeHostFiltersPixelTest, MAYBE_RotatedFilter)
{
    ...
    RunPixelTest(background, base::FilePath(FILE_PATH_LITERAL("rotated_filter_.png")).InsertBeforeExtensionASCII(GetRendererSuffix()));
}
...
TEST_P(LayerTreeHostFiltersPixelTest, RotatedDropShadowFilter)
{
    ...
    SolidColorContentLayerClient blue_client(SK_ColorBLUE, rect.size(), 1, SK_ColorBLACK);
    ...
    child->SetTransform(transform);
    FilterOperations filters;
    filters.Append(FilterOperation::CreateDropShadowFilter(gfx::Point(10.0f, 10.0f), 0.0f, SkColors::kBlack));
    child->SetFilters(filters);

    background->AddChild(child);

#if BUILDFLAG(IS_WIN) || BUILDFLAG(IS_MAC) || BUILDFLAG(IS_CHROMEOS) || defined(ARCH_CPU_ARM64) || BUILDFLAG(IS_OZONE)
#if defined(ARCH_CPU_ARM64) && (BUILDFLAG(IS_WIN) || BUILDFLAG(IS_FUCHSIA) || BUILDFLAG(IS_APPLE))
    ...
    RunPixelTest(background, base::FilePath(FILE_PATH_LITERAL("rotated_drop_shadow_filter_.png")).InsertBeforeExtensionASCII(GetRendererSuffix()));
}
...
TEST_P(LayerTreeHostFiltersPixelTest, TranslatedFilter)
{
    ...
    RunPixelTest(clip, base::FilePath(FILE_PATH_LITERAL("translated_blue_green_alpha.png")));
}
...
TEST_P(LayerTreeHostFiltersPixelTest, EnlargedTextureWithAlphaThresholdFilter)
{
    ...
    base::FilePath expected_result = base::FilePath(FILE_PATH_LITERAL("enlarged_texture_on_threshold.png"));
    if (use_skia_graphite()) {
...
TEST_P(LayerTreeHostFiltersPixelTest, EnlargedTextureWithCropOffsetFilter)
{
    ...
    filters.Append(FilterOperation::CreateReferenceFilter(sk_make_sp<OffsetPaintFilter>(0, 0, nullptr, &cropRect)));
    filter_layer->SetFilters(filters);
    ...
    base::FilePath expected_result = base::FilePath(FILE_PATH_LITERAL("enlarged_texture_on_crop_offset.png"));
    if (use_skia_graphite()) {
        expected_result = expected_result.InsertBeforeExtensionASCII("_graphite");
    }
    RunPixelTest(background, expected_result);
...
TEST_P(LayerTreeHostFiltersPixelTest, BlurFilterWithClip)
{
    ...
    RunPixelTest(filter_layer, base::FilePath(FILE_PATH_LITERAL("blur_filter_with_clip_.png")).InsertBeforeExtensionASCII(GetRendererSuffix()));
}
...
TEST_P(LayerTreeHostFiltersPixelTestGPU, FilterWithGiantCropRect)
{
    ...
    RunPixelTest(tree, base::FilePath(FILE_PATH_LITERAL("filter_with_giant_crop_rect.png")));
}
...
protected:
    void RunPixelTestType(float device_scale_factor)
    {
        ...
        scoped_refptr<SolidColorLayer> background = CreateSolidColorLayer(gfx::Rect(100, 125), SK_ColorBLACK);
        root->AddChild(background);
        scoped_refptr<SolidColorLayer> filtered = CreateSolidColorLayer(gfx::Rect(0, 100, 200, 100), SkColorSetA(SK_ColorGREEN, 127));
        FilterOperations filters;
        ...
        filters.Append(FilterOperation::CreateReferenceFilter(sk_make_sp<OffsetPaintFilter>(0, 75, nullptr)));
        ...
        base::FilePath expected_result = base::FilePath(FILE_PATH_LITERAL("offset_backdrop_filter_.png"));
        expected_result = expected_result.InsertBeforeExtensionASCII(base::NumberToString(device_scale_factor) + "x");
        if (use_software_renderer()) {
...
INSTANTIATE_TEST_SUITE_P(All, BackdropFilterOffsetTest, ::testing::ValuesIn(viz::GetRendererTypes()), ::testing::PrintToStringParamName());
...
protected:
    void RunPixelTestType(float device_scale_factor)
    {
        scoped_refptr<Layer> root = CreateSolidColorLayer(gfx::Rect(200, 200), SK_ColorGREEN);
        scoped_refptr<SolidColorLayer> filtered = CreateSolidColorLayer(gfx::Rect(50, 50, 100, 100), SK_ColorTRANSPARENT);
        FilterOperations filters;
        filters.Append(FilterOperation::CreateInvertFilter(1.0));
        filtered->SetBackdropFilters(filters);
        gfx::RRectF backdrop_filter_bounds(gfx::RectF(gfx::SizeF(filtered->bounds())), 20);
        filtered->SetBackdropFilterBounds(backdrop_filter_bounds);
        root->AddChild(filtered);
        device_scale_factor_ = device_scale_factor;
        base::FilePath expected_result = base::FilePath(FILE_PATH_LITERAL("invert_backdrop_filter_.png"));
        expected_result = expected_result.InsertBeforeExtensionASCII(base::NumberToString(device_scale_factor) + "x");
        if (use_software_renderer()) {
            expected_result = expected_result.InsertBeforeExtensionASCII("_sw");
        }
...
INSTANTIATE_TEST_SUITE_P(All, BackdropFilterInvertTest, ::testing::ValuesIn(viz::GetRendererTypes()), ::testing::PrintToStringParamName());
...
protected:
    void RunPixelTestType(float device_scale_factor)
    {
        scoped_refptr<Layer> root = CreateSolidColorLayer(gfx::Rect(110, 200), SK_ColorGREEN);
        ...
        scoped_refptr<SolidColorLayer> filtered = CreateSolidColorLayer(zoom_rect, SK_ColorTRANSPARENT);
        ...
        gfx::RRectF backdrop_filter_bounds(gfx::RectF(gfx::SizeF(filtered->bounds())), 10);
        filtered->SetBackdropFilterBounds(backdrop_filter_bounds);
        root->AddChild(filtered);

        device_scale_factor_ = device_scale_factor;

        base::FilePath expected_result = base::FilePath(FILE_PATH_LITERAL("mixed_filters_zoom_offset_.png"));
        expected_result = expected_result.InsertBeforeExtensionASCII(base::NumberToString(device_scale_factor) + "x");
        ...
        pixel_comparator_ = std::make_unique<FuzzyPixelComparator>(
            FuzzyPixelComparator().SetErrorPixelsPercentageLimit(error_percent).SetAvgAbsErrorLimit(avg_error).SetAbsErrorLimit(max_error));
        RunPixelTest(std::move(root), expected_result);
    }
...
INSTANTIATE_TEST_SUITE_P(, MixedFilterZoomAndOffsetTest, ::testing::ValuesIn(viz::GetGpuRendererTypes()), ::testing::PrintToStringParamName());
