...
struct DebugRect {
    DebugRect(
        DebugRectType new_type, const gfx::Rect& new_rect, TouchAction new_touch_action = TouchAction::kNone, uint32_t main_thread_scroll_repaint_reasons = 0)
        : type(new_type)
        , rect(new_rect)
        , touch_action(new_touch_action)
        , main_thread_scroll_repaint_reasons(main_thread_scroll_repaint_reasons)
    {
        DCHECK(type == DebugRectType::kTouchEventHandler || touch_action == TouchAction::kNone);
        DCHECK(type != DebugRectType::kMainThreadScrollRepaint || !main_thread_scroll_repaint_reasons);
    }
    DebugRectType type;
    gfx::Rect rect;
...
public:
    ...
    const std::vector<DebugRect>& debug_rects()
    {
        return debug_rects_;
    }
...
private:
    ...
    void SaveMainThreadScrollRepaintOrRasterInducingScrollRects(LayerTreeImpl* tree_impl, DebugRectType type);
