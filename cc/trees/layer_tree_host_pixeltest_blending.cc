...
SkBlendMode const kBlendModes[] = { SkBlendMode::kSrcOver, SkBlendMode::kScreen, SkBlendMode::kOverlay, SkBlendMode::kDarken, SkBlendMode::kLighten,
    SkBlendMode::kColorDodge, SkBlendMode::kColorBurn, SkBlendMode::kHardLight, SkBlendMode::kSoftLight, SkBlendMode::kDifference, SkBlendMode::kExclusion,
    SkBlendMode::kMultiply, SkBlendMode::kHue, SkBlendMode::kSaturation, SkBlendMode::kColor, SkBlendMode::kLuminosity };
...
class LayerTreeHostBlendingPixelTest : public LayerTreeHostPixelResourceTest,
                                       public ::testing::WithParamInterface<::testing::tuple<RasterTestConfig, SkBlendMode>> {
public:
...
protected:
        ...
        scoped_refptr<viz::RasterContextProvider> compositor_context_provider, scoped_refptr<viz::RasterContextProvider> worker_context_provider) override
    {
        ...
        return LayerTreeHostPixelResourceTest::CreateLayerTreeFrameSink(
            modified_renderer_settings, refresh_rate, compositor_context_provider, worker_context_provider);
    }
    ...
    sk_sp<SkSurface> CreateColorfulSurface(int width, int height)
    {
        ...
        sk_sp<SkSurface> backing_store = SkSurfaces::Raster(SkImageInfo::MakeN32Premul(width, height));
        SkCanvas* canvas = backing_store->getCanvas();
        canvas->clear(SK_ColorTRANSPARENT);
        for (int i = 0; i < kCSSTestColorsCount; ++i) {
            SkPaint paint;
            paint.setColor(kCSSTestColors[i]);
            canvas->drawRect(SkRect::MakeXYWH(0, i * kLaneHeight, kLaneWidth, kLaneHeight), paint);
        }
    ...
    scoped_refptr<Layer> CreateColorfulBackdropLayer(int width, int height)
    {
        ...
        backdrop_client_.add_draw_image(backing_store->makeImageSnapshot(), gfx::Point());
        scoped_refptr<FakePictureLayer> layer = FakePictureLayer::Create(&backdrop_client_);
    ...
    void SetupMaskLayer(scoped_refptr<Layer> layer)
    {
        ...
        sk_sp<SkSurface> surface = SkSurfaces::Raster(SkImageInfo::MakeN32Premul(bounds.width(), bounds.height()));
        ...
        canvas->drawRect(SkRect::MakeXYWH(1, 0, bounds.width() - 1, bounds.height()), paint);

        mask_client_.set_bounds(bounds);
        mask_client_.add_draw_image(surface->makeImageSnapshot(), gfx::Point());

        scoped_refptr<FakePictureLayer> mask = FakePictureLayer::Create(&mask_client_);
    ...
    void CreateBlendingColorLayers(int lane_width, int lane_height, scoped_refptr<Layer> background, RenderPassOptions flags)
    {
        gfx::Rect child_rect(lane_width, lane_height);

        scoped_refptr<SolidColorLayer> lane = CreateSolidColorLayer(child_rect, misc_opaque_color_);
    ...
    SkBitmap CreateBlendingWithRenderPassExpected(int width, int height, RenderPassOptions flags)
    {
    ...
    void RunBlendingWithRenderPass(RenderPassOptions flags)
    {
        ...
        scoped_refptr<SolidColorLayer> root = CreateSolidColorLayer(gfx::Rect(kRootWidth, kRootHeight), SK_ColorWHITE);
        scoped_refptr<Layer> background = CreateColorfulBackdropLayer(kRootWidth, kRootHeight);
        ...
        if (renderer_type_ == viz::RendererType::kSkiaVk) {
            ...
            pixel_comparator_ = std::make_unique<FuzzyPixelComparator>(FuzzyPixelComparator().SetErrorPixelsPercentageLimit(35.f).SetAbsErrorLimit(1));
        }

        RunPixelResourceTest(root, CreateBlendingWithRenderPassExpected(kRootWidth, kRootHeight, flags));
    }
...
INSTANTIATE_TEST_SUITE_P(B, LayerTreeHostBlendingPixelTest, ::testing::Combine(::testing::ValuesIn(kTestCases), ::testing::ValuesIn(kBlendModes)),
    ...
    [](const testing::TestParamInfo<testing::tuple<RasterTestConfig, SkBlendMode>>& info) -> std::string {
        std::stringstream ss;
...
TEST_P(LayerTreeHostBlendingPixelTest, BlendingWithBackdropFilter)
{
    ...
    sk_sp<PaintFilter> paint_filter = RenderSurfaceFilters::BuildImageFilter(filters);
...
TEST_P(LayerTreeHostBlendingPixelTest, BlendingWithRenderPassWithMaskColorMatrix)
{
    RunBlendingWithRenderPass(kUseMasks | kUseColorMatrix);
}

TEST_P(LayerTreeHostBlendingPixelTest, BlendingWithRenderPassWithMaskColorMatrixAA)
{
