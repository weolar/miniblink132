...
void DebugRectHistory::SaveDebugRectsForCurrentFrame(
    LayerTreeImpl* tree_impl, HeadsUpDisplayLayerImpl* hud_layer, const RenderSurfaceList& render_surface_list, const LayerTreeDebugState& debug_state)
{
    ...
    if (debug_state.show_main_thread_scroll_repaint_rects) {
        SaveMainThreadScrollRepaintOrRasterInducingScrollRects(tree_impl, DebugRectType::kMainThreadScrollRepaint);
    }
    if (debug_state.show_raster_inducing_scroll_rects) {
        SaveMainThreadScrollRepaintOrRasterInducingScrollRects(tree_impl, DebugRectType::kRasterInducingScroll);
    }
...
void DebugRectHistory::SaveLayoutShiftRects(HeadsUpDisplayLayerImpl* hud)
{
    ...
    for (gfx::Rect rect : hud->LayoutShiftRects()) {
        debug_rects_.emplace_back(DebugRectType::kLayoutShift, MathUtil::MapEnclosingClippedRect(hud->ScreenSpaceTransform(), rect));
    }
...
void DebugRectHistory::SavePaintRects(LayerTreeImpl* tree_impl)
{
    ...
    for (auto* layer : *tree_impl) {
        ...
        for (gfx::Rect rect : invalidation_region) {
            debug_rects_.emplace_back(DebugRectType::kPaint, MathUtil::MapEnclosingClippedRect(layer->ScreenSpaceTransform(), rect));
        }
    }
}

void DebugRectHistory::SavePropertyChangedRects(LayerTreeImpl* tree_impl, LayerImpl* hud_layer)
{
    for (LayerImpl* layer : *tree_impl) {
        ...
        debug_rects_.emplace_back(
            DebugRectType::kPropertyChanged, MathUtil::MapEnclosingClippedRect(layer->ScreenSpaceTransform(), gfx::Rect(layer->bounds())));
    }
}

void DebugRectHistory::SaveSurfaceDamageRects(const RenderSurfaceList& render_surface_list)
{
    for (size_t i = 0; i < render_surface_list.size(); ++i) {
        ...
        debug_rects_.emplace_back(
            DebugRectType::kSurfaceDamage, MathUtil::MapEnclosingClippedRect(render_surface->screen_space_transform(), render_surface->GetDamageRect()));
    }
}

void DebugRectHistory::SaveScreenSpaceRects(const RenderSurfaceList& render_surface_list)
{
    for (size_t i = 0; i < render_surface_list.size(); ++i) {
        size_t surface_index = render_surface_list.size() - 1 - i;
        RenderSurfaceImpl* render_surface = render_surface_list[surface_index];
        ...
        debug_rects_.emplace_back(
            DebugRectType::kScreenSpace, MathUtil::MapEnclosingClippedRect(render_surface->screen_space_transform(), render_surface->content_rect()));
    }
}
...
void DebugRectHistory::SaveTouchEventHandlerRects(LayerTreeImpl* tree_impl)
{
    for (auto* layer : *tree_impl) {
        ...
        for (int touch_action_index = static_cast<int>(TouchAction::kNone); touch_action_index != static_cast<int>(TouchAction::kMax); ++touch_action_index) {
            ...
            for (gfx::Rect rect : region) {
                debug_rects_.emplace_back(
                    DebugRectType::kTouchEventHandler, MathUtil::MapEnclosingClippedRect(layer->ScreenSpaceTransform(), rect), touch_action);
            }
        }
    }
}
...
void DebugRectHistory::SaveWheelEventHandlerRects(LayerTreeImpl* tree_impl)
{
    ...
    for (auto* layer : *tree_impl) {
        ...
        for (gfx::Rect rect : region) {
            debug_rects_.emplace_back(DebugRectType::kWheelEventHandler, MathUtil::MapEnclosingClippedRect(layer->ScreenSpaceTransform(), rect));
        }
    }
}
...
void DebugRectHistory::SaveScrollEventHandlerRects(LayerTreeImpl* tree_impl)
{
    for (auto* layer : *tree_impl) {
        ...
        debug_rects_.emplace_back(
            DebugRectType::kScrollEventHandler, MathUtil::MapEnclosingClippedRect(layer->ScreenSpaceTransform(), gfx::Rect(layer->bounds())));
    }
}

void DebugRectHistory::SaveMainThreadScrollHitTestRects(LayerTreeImpl* tree_impl)
{
    for (auto* layer : *tree_impl) {
        for (gfx::Rect rect : layer->main_thread_scroll_hit_test_region()) {
            debug_rects_.emplace_back(DebugRectType::kMainThreadScrollHitTest, MathUtil::MapEnclosingClippedRect(layer->ScreenSpaceTransform(), rect));
        }
    }
}

void DebugRectHistory::SaveMainThreadScrollRepaintOrRasterInducingScrollRects(LayerTreeImpl* tree_impl, DebugRectType type)
{
    ...
    for (auto& node : scroll_tree.nodes()) {
        if (type == DebugRectType::kMainThreadScrollRepaint ? scroll_tree.ShouldRealizeScrollsOnMain(node) : scroll_tree.CanRealizeScrollsOnPendingTree(node)) {
            if (const auto* transform_node = transform_tree.Node(node.transform_id); transform_node && transform_tree.Node(transform_node->parent_id)) {
                debug_rects_.emplace_back(type,
                    MathUtil::MapEnclosingClippedRect(
                        // Skip the scroll translation node.
                        transform_tree.ToScreen(transform_node->parent_id), gfx::Rect(node.container_origin, scroll_tree.container_bounds(node.id))));
            }
        }
    }
}
