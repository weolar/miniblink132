...
public:
    explicit DrawPropertiesTestBase(const LayerTreeSettings& settings = CommitToPendingTreeLayerListSettings())
    ...
    {
        return layer_impl->layer_tree_impl()->property_trees()->MaximumAnimationToScreenScale(layer_impl->transform_tree_index());
    }
    static bool AnimationAffectedByInvalidScale(LayerImpl* layer_impl)
    {
        return layer_impl->layer_tree_impl()->property_trees()->AnimationAffectedByInvalidScale(layer_impl->transform_tree_index());
    }
    ...
    {
        return layer ? host_impl()->pending_tree()->LayerById(layer->id()) : nullptr;
    }
    ...
    void Commit(float device_scale_factor = 1.0f)
    {
        ...
        host_impl()->pending_tree()->PullLayerTreePropertiesFrom(*host()->GetPendingCommitState());
    ...
    void CommitAndActivate(float device_scale_factor = 1.0f)
    {
        ...
        DCHECK_EQ(device_scale_factor, host_impl()->active_tree()->device_scale_factor());
    ...
    const LayerList& update_layer_list() const
    {
        return update_layer_list_;
    }
    ...
    {
        host_impl()->OnDraw(device_transform, host_impl()->external_viewport(), false, false);
    }
...
class DrawPropertiesTest : public DrawPropertiesTestBase, public testing::Test { };

class DrawPropertiesTestWithLayerTree : public DrawPropertiesTestBase, public testing::Test {
public:
...
public:
    ...
    LayerImpl* TestVisibleRectAndDrawableContentRect(const gfx::Rect& target_rect, const gfx::Transform& layer_transform, const gfx::Rect& layer_rect)
    {
        LayerImpl* root = root_layer();
        ...
        CreateTransformNode(target).post_translation = gfx::PointF(target_rect.origin()).OffsetFromOrigin();
        ...
        drawing_layer_transform_node.post_translation = gfx::PointF(layer_rect.origin()).OffsetFromOrigin();
...
TEST_F(DrawPropertiesTest, TransformsForSingleLayer)
{
    ...
    TransformTree& transform_tree = host_impl()->active_tree()->property_trees()->transform_tree_mutable();
    EffectTree& effect_tree = host_impl()->active_tree()->property_trees()->effect_tree_mutable();
    ...
    UpdateActiveTreeDrawProperties();
    EXPECT_TRANSFORM_EQ(gfx::Transform(), draw_property_utils::DrawTransform(layer, transform_tree, effect_tree));
    EXPECT_TRANSFORM_EQ(gfx::Transform(), draw_property_utils::ScreenSpaceTransform(layer, transform_tree));
    ...
    UpdateActiveTreeDrawProperties();
    EXPECT_TRANSFORM_EQ(position_transform, draw_property_utils::DrawTransform(layer, transform_tree, effect_tree));
    EXPECT_TRANSFORM_EQ(position_transform, draw_property_utils::ScreenSpaceTransform(layer, transform_tree));
    ...
    UpdateActiveTreeDrawProperties();
    EXPECT_TRANSFORM_EQ(layer_transform, draw_property_utils::DrawTransform(layer, transform_tree, effect_tree));
    EXPECT_TRANSFORM_EQ(layer_transform, draw_property_utils::ScreenSpaceTransform(layer, transform_tree));
    ...
    gfx::Transform expected_result = translation_to_anchor * layer_transform * translation_to_anchor.GetCheckedInverse();
    SetTransformOrigin(layer, gfx::Point3F(5.f, 0.f, 0.f));
    UpdateActiveTreeDrawProperties();
    EXPECT_TRANSFORM_EQ(expected_result, draw_property_utils::DrawTransform(layer, transform_tree, effect_tree));
    EXPECT_TRANSFORM_EQ(expected_result, draw_property_utils::ScreenSpaceTransform(layer, transform_tree));
    ...
    expected_result = position_transform * translation_to_anchor * layer_transform * translation_to_anchor.GetCheckedInverse();
    SetPostTranslation(layer, gfx::Vector2dF(0.f, 1.2f));
    UpdateActiveTreeDrawProperties();
    EXPECT_TRANSFORM_EQ(expected_result, draw_property_utils::DrawTransform(layer, transform_tree, effect_tree));
    EXPECT_TRANSFORM_EQ(expected_result, draw_property_utils::ScreenSpaceTransform(layer, transform_tree));
...
TEST_F(DrawPropertiesTest, TransformsAboutScrollOffset)
{
    ...
    CreateScrollNode(scroll_layer, gfx::Size(scroll_layer->bounds().width() - kMaxScrollOffset.x(), scroll_layer->bounds().height() - kMaxScrollOffset.y()));
    CopyProperties(scroll_layer, sublayer);

    auto& scroll_tree = GetPropertyTrees(scroll_layer)->scroll_tree_mutable();
    scroll_tree.UpdateScrollOffsetBaseForTesting(scroll_layer->element_id(), kScrollOffset);
    ...
    expected_transform.Translate(
        std::round(sub_layer_screen_position.x() * page_scale * kDeviceScale), std::round(sub_layer_screen_position.y() * page_scale * kDeviceScale));
    expected_transform.Scale(page_scale * kDeviceScale, page_scale * kDeviceScale);
    ...
    expected_transform.Translate(std::round(kTranslateX * page_scale * kDeviceScale + sub_layer_screen_position.x() * page_scale * kDeviceScale),
        std::round(kTranslateY * page_scale * kDeviceScale + sub_layer_screen_position.y() * page_scale * kDeviceScale));
    expected_transform.Scale(page_scale * kDeviceScale, page_scale * kDeviceScale);
    EXPECT_TRANSFORM_EQ(expected_transform, sublayer->DrawTransform());
...
TEST_F(DrawPropertiesTest, TransformsForSimpleHierarchy)
{
    ...
    EXPECT_TRANSFORM_EQ(gfx::Transform(), draw_property_utils::DrawTransform(child, transform_tree, effect_tree));
    EXPECT_TRANSFORM_EQ(gfx::Transform(), draw_property_utils::ScreenSpaceTransform(child, transform_tree));
    EXPECT_TRANSFORM_EQ(gfx::Transform(), draw_property_utils::DrawTransform(grand_child, transform_tree, effect_tree));
    EXPECT_TRANSFORM_EQ(gfx::Transform(), draw_property_utils::ScreenSpaceTransform(grand_child, transform_tree));
    ...
    UpdateActiveTreeDrawProperties();
    EXPECT_TRANSFORM_EQ(parent_position_transform, draw_property_utils::DrawTransform(child, transform_tree, effect_tree));
    EXPECT_TRANSFORM_EQ(parent_position_transform, draw_property_utils::ScreenSpaceTransform(child, transform_tree));
    EXPECT_TRANSFORM_EQ(parent_position_transform, draw_property_utils::DrawTransform(grand_child, transform_tree, effect_tree));
    EXPECT_TRANSFORM_EQ(parent_position_transform, draw_property_utils::ScreenSpaceTransform(grand_child, transform_tree));
    ...
    gfx::Transform parent_composite_transform = parent_translation_to_anchor * parent_layer_transform * parent_translation_to_anchor.GetCheckedInverse();
    SetTransform(parent, parent_layer_transform);
    SetPostTranslation(parent, gfx::Vector2dF());
    UpdateActiveTreeDrawProperties();
    EXPECT_TRANSFORM_EQ(parent_composite_transform, draw_property_utils::DrawTransform(child, transform_tree, effect_tree));
    EXPECT_TRANSFORM_EQ(parent_composite_transform, draw_property_utils::ScreenSpaceTransform(child, transform_tree));
    EXPECT_TRANSFORM_EQ(parent_composite_transform, draw_property_utils::DrawTransform(grand_child, transform_tree, effect_tree));
    EXPECT_TRANSFORM_EQ(parent_composite_transform, draw_property_utils::ScreenSpaceTransform(grand_child, transform_tree));
}
...
TEST_F(DrawPropertiesTest, TransformsForSingleRenderSurface)
{
    ...
    gfx::Transform parent_composite_transform = parent_translation_to_anchor * parent_layer_transform * parent_translation_to_anchor.GetCheckedInverse();
    gfx::Vector2dF parent_composite_scale = gfx::ComputeTransform2dScaleComponents(parent_composite_transform, 1.f);
    gfx::Transform surface_sublayer_transform;
    surface_sublayer_transform.Scale(parent_composite_scale.x(), parent_composite_scale.y());
    gfx::Transform surface_sublayer_composite_transform = parent_composite_transform * surface_sublayer_transform.GetCheckedInverse();
    ...
    EXPECT_TRANSFORM_EQ(parent_composite_transform, child->ScreenSpaceTransform());
    ...
    EXPECT_TRANSFORM_EQ(surface_sublayer_composite_transform, child->render_target()->draw_transform());

    // The screen space is the same as the target since the child surface draws
    // into the root.
    EXPECT_TRANSFORM_EQ(surface_sublayer_composite_transform, child->render_target()->screen_space_transform());
}
...
TEST_F(DrawPropertiesTest, TransformsForRenderSurfaceHierarchy)
{
    ...
    gfx::Transform A = translation_to_anchor * layer_transform * translation_to_anchor.GetCheckedInverse();

    gfx::Vector2dF surface1_parent_transform_scale = gfx::ComputeTransform2dScaleComponents(A, 1.f);
    gfx::Transform surface1_sublayer_transform;
    surface1_sublayer_transform.Scale(surface1_parent_transform_scale.x(), surface1_parent_transform_scale.y());
    ...
    gfx::Vector2dF surface2_parent_transform_scale = gfx::ComputeTransform2dScaleComponents(SS1 * A, 1.f);
    gfx::Transform surface2_sublayer_transform;
    surface2_sublayer_transform.Scale(surface2_parent_transform_scale.x(), surface2_parent_transform_scale.y());
    ...
    auto& grand_child_of_root_transform_node = CreateTransformNode(grand_child_of_root);
    ...
    render_surface1_effect_node.render_surface_reason = RenderSurfaceReason::kTest;
    ...
    auto& grand_child_of_rs1_transform_node = CreateTransformNode(grand_child_of_rs1);
    ...
    render_surface2_effect_node.render_surface_reason = RenderSurfaceReason::kTest;
    ...
    auto& grand_child_of_rs2_transform_node = CreateTransformNode(grand_child_of_rs2);
    ...
    ASSERT_NE(GetRenderSurface(render_surface2), GetRenderSurface(render_surface1));
    ASSERT_EQ(GetRenderSurface(child_of_rs2), GetRenderSurface(render_surface2));
    ASSERT_EQ(GetRenderSurface(grand_child_of_rs2), GetRenderSurface(render_surface2));
    ...
    EXPECT_EQ(GetRenderSurface(render_surface1), render_surface1->render_target());
    EXPECT_EQ(GetRenderSurface(render_surface1), child_of_rs1->render_target());
    EXPECT_EQ(GetRenderSurface(render_surface1), grand_child_of_rs1->render_target());

    EXPECT_EQ(GetRenderSurface(render_surface2), render_surface2->render_target());
    EXPECT_EQ(GetRenderSurface(render_surface2), child_of_rs2->render_target());
    EXPECT_EQ(GetRenderSurface(render_surface2), grand_child_of_rs2->render_target());
    ...
    EXPECT_TRANSFORM_EQ(A * A * A * A, grand_child_of_rs1->ScreenSpaceTransform());
    ...
    EXPECT_TRANSFORM_EQ(A * A * A * A * A, grand_child_of_rs2->ScreenSpaceTransform());

    // Verify render surface transforms.
    //
    // Draw transform of render surface 1 is described with respect to root.
    EXPECT_TRANSFORM_EQ(A * A * S1, GetRenderSurface(render_surface1)->draw_transform());
    EXPECT_TRANSFORM_EQ(A * A * S1, GetRenderSurface(render_surface1)->screen_space_transform());
    // Draw transform of render surface 2 is described with respect to render
    // surface 1.
    EXPECT_TRANSFORM_EQ(SS1 * A * S2, GetRenderSurface(render_surface2)->draw_transform());
    EXPECT_TRANSFORM_EQ(A * A * A * S2, GetRenderSurface(render_surface2)->screen_space_transform());
...
TEST_F(DrawPropertiesTest, TransformsForDegenerateIntermediateLayer)
{
    ...
    EXPECT_TRANSFORM_EQ(gfx::Transform(), GetRenderSurface(child)->draw_transform());
    EXPECT_TRANSFORM_EQ(gfx::Transform(), child->DrawTransform());
    EXPECT_TRANSFORM_EQ(gfx::Transform(), grand_child->DrawTransform());
}
...
TEST_F(DrawPropertiesTest, RenderSurfaceWithSublayerScale)
{
    ...
    CreateEffectNode(render_surface).render_surface_reason = RenderSurfaceReason::kTest;
    ...
    EXPECT_EQ(grand_child->DrawTransform().To2dTranslation(), expected_translation);
}
...
TEST_F(DrawPropertiesTest, TransformAboveRootLayer)
{
    ...
    translate.Translate(50, 50);
    {
        ...
        UpdateActiveTreeDrawProperties(device_scale_factor);
        EXPECT_TRANSFORM_EQ(translate, root->draw_properties().target_space_transform);
        EXPECT_TRANSFORM_EQ(translate, child->draw_properties().target_space_transform);
        EXPECT_TRANSFORM_EQ(gfx::Transform(), GetRenderSurface(root)->draw_transform());
    ...
    rotate.Rotate(2);
    {
        ...
        EXPECT_TRANSFORM_EQ(rotate, child->draw_properties().target_space_transform);
        EXPECT_TRANSFORM_EQ(gfx::Transform(), GetRenderSurface(root)->draw_transform());
    ...
    composite.PostConcat(rotate);
    {
        ...
        UpdateActiveTreeDrawProperties(device_scale_factor);
        EXPECT_TRANSFORM_EQ(composite, root->draw_properties().target_space_transform);
        EXPECT_TRANSFORM_EQ(composite, child->draw_properties().target_space_transform);
        EXPECT_TRANSFORM_EQ(gfx::Transform(), GetRenderSurface(root)->draw_transform());
...

    {
        ...
        EXPECT_TRANSFORM_EQ(device_scaled_translate, root->draw_properties().target_space_transform);
        EXPECT_TRANSFORM_EQ(device_scaled_translate, child->draw_properties().target_space_transform);
        EXPECT_TRANSFORM_EQ(gfx::Transform(), GetRenderSurface(root)->draw_transform());
...
TEST_F(DrawPropertiesTest, DrawableContentRectForReferenceFilter)
{
    ...
    ASSERT_TRUE(GetRenderSurface(child));
    EXPECT_EQ(gfx::RectF(50, 50, 25, 25), GetRenderSurface(child)->DrawableContentRect());
}
...
TEST_F(DrawPropertiesTest, DrawableContentRectForReferenceFilterHighDpi)
{
    ...
    ASSERT_TRUE(GetRenderSurface(child));
    EXPECT_EQ(gfx::RectF(100, 100, 50, 50), GetRenderSurface(child)->DrawableContentRect());
}
...
TEST_F(DrawPropertiesTest, ClipRectCullsRenderSurfaces)
{
    ...
    great_grand_child->SetOffsetToTransformParent(grand_child->offset_to_transform_parent());
    CopyProperties(great_grand_child, leaf_node2);
    leaf_node2->SetOffsetToTransformParent(great_grand_child->offset_to_transform_parent());
...
TEST_F(DrawPropertiesTest, ClipRectCullsSurfaceWithoutVisibleContent)
{
    ...
    CreateEffectNode(grand_child).render_surface_reason = RenderSurfaceReason::kTest;
...
TEST_F(DrawPropertiesTest, DrawableContentRectForLayers)
{
    ...
    EXPECT_EQ(gfx::Rect(5, 5, 10, 10), grand_child1->visible_drawable_content_rect());
    EXPECT_EQ(gfx::Rect(15, 15, 5, 5), grand_child3->visible_drawable_content_rect());
    EXPECT_EQ(gfx::Rect(15, 15, 5, 5), grand_child3->visible_drawable_content_rect());
...
TEST_F(DrawPropertiesTest, ClipRectIsPropagatedCorrectlyToSurfaces)
{
    ...
    CreateEffectNode(grand_child1).render_surface_reason = RenderSurfaceReason::kTest;
    CopyProperties(grand_child1, leaf_node1);
    CopyProperties(child, grand_child2);
    CreateTransformNode(grand_child2).post_translation = gfx::Vector2dF(15.f, 15.f);
    CreateEffectNode(grand_child2).render_surface_reason = RenderSurfaceReason::kTest;
    CopyProperties(grand_child2, leaf_node2);
    CopyProperties(child, grand_child3);
    CreateTransformNode(grand_child3).post_translation = gfx::Vector2dF(15.f, 15.f);
    CreateEffectNode(grand_child3).render_surface_reason = RenderSurfaceReason::kTest;
    CreateClipNode(grand_child3);
    CopyProperties(grand_child3, leaf_node3);
    CopyProperties(child, grand_child4);
    CreateTransformNode(grand_child4).post_translation = gfx::Vector2dF(45.f, 45.f);
    CreateEffectNode(grand_child4).render_surface_reason = RenderSurfaceReason::kTest;
    ...
    // MasksToBounds.
    EXPECT_EQ(gfx::Rect(0, 0, 20, 20), GetRenderSurface(grand_child1)->clip_rect());
    EXPECT_EQ(gfx::Rect(0, 0, 20, 20), GetRenderSurface(grand_child2)->clip_rect());
    EXPECT_EQ(gfx::Rect(0, 0, 20, 20), GetRenderSurface(grand_child3)->clip_rect());
}
...
TEST_F(DrawPropertiesTest, AnimationsForRenderSurfaceHierarchy)
{
    ...
    render_surface1_transform_node.local = layer_transform;
    CreateEffectNode(render_surface1).render_surface_reason = RenderSurfaceReason::kTest;
    CopyProperties(render_surface1, child_of_rs1);
    auto& child_of_rs1_transform_node = CreateTransformNode(child_of_rs1);
    ...
    grand_child_of_rs1_transform_node.post_translation = gfx::Vector2dF(2.5f, 0.f);
    grand_child_of_rs1_transform_node.local = layer_transform;
    CopyProperties(render_surface1, render_surface2);
    auto& render_surface2_transform_node = CreateTransformNode(render_surface2);
    ...
    render_surface2_transform_node.local = layer_transform;
    CreateEffectNode(render_surface2).render_surface_reason = RenderSurfaceReason::kTest;
    CopyProperties(render_surface2, child_of_rs2);
    auto& child_of_rs2_transform_node = CreateTransformNode(child_of_rs2);
    ...
    grand_child_of_rs2_transform_node.post_translation = gfx::Vector2dF(2.5f, 0.f);
    grand_child_of_rs2_transform_node.local = layer_transform;
    ...
    auto& grand_child_of_top_transform_node = CreateTransformNode(grand_child_of_top);
    grand_child_of_top_transform_node.origin = gfx::Point3F(0.25f, 0.f, 0.f);
    grand_child_of_top_transform_node.post_translation = gfx::Vector2dF(2.5f, 0.f);
    grand_child_of_top_transform_node.local = layer_transform;

    // Put an animated opacity on the render surface.
    AddOpacityTransitionToElementWithAnimation(render_surface1->element_id(), timeline_impl(), 10.0, 1.f, 0.f, false);

    // Also put an animated opacity on a layer without descendants.
    AddOpacityTransitionToElementWithAnimation(grand_child_of_top->element_id(), timeline_impl(), 10.0, 1.f, 0.f, false);

    // Put a transform animation on the render surface.
    AddAnimatedTransformToElementWithAnimation(render_surface2->element_id(), timeline_impl(), 10.0, 30, 0);

    // Also put transform animations on grand_child_of_top, and
    // grand_child_of_rs2
    AddAnimatedTransformToElementWithAnimation(grand_child_of_top->element_id(), timeline_impl(), 10.0, 30, 0);
    AddAnimatedTransformToElementWithAnimation(grand_child_of_rs2->element_id(), timeline_impl(), 10.0, 30, 0);
...
static bool TransformIsAnimating(LayerImpl* layer)
{
    ...
    return host->IsAnimatingProperty(layer->element_id(), layer->GetElementTypeForAnimation(), TargetProperty::TRANSFORM);
}

static bool HasPotentiallyRunningTransformAnimation(LayerImpl* layer)
{
    MutatorHost* host = layer->layer_tree_impl()->mutator_host();
    return host->HasPotentiallyRunningAnimationForProperty(layer->element_id(), layer->GetElementTypeForAnimation(), TargetProperty::TRANSFORM);
}

TEST_F(DrawPropertiesTest, ScreenSpaceTransformIsAnimatingWithDelayedAnimation)
{
    LayerImpl* root = root_layer();
    LayerImpl* child = AddLayerInActiveTree<LayerImpl>();
    LayerImpl* grand_child = AddLayerInActiveTree<LayerImpl>();
    ...
    std::unique_ptr<KeyframeModel> keyframe_model = KeyframeModel::Create(
        std::unique_ptr<gfx::AnimationCurve>(new FakeTransformTransition(1.0)), 0, 1, KeyframeModel::TargetPropertyId(TargetProperty::TRANSFORM));
    keyframe_model->set_fill_mode(KeyframeModel::FillMode::NONE);
    keyframe_model->set_time_offset(base::Milliseconds(-1000));
    AddKeyframeModelToElementWithAnimation(grand_child->element_id(), timeline_impl(), std::move(keyframe_model));
...
TEST_F(DrawPropertiesDrawRectsTest, DrawRectsForIdentityTransform)
{
    ...
    LayerImpl* drawing_layer = TestVisibleRectAndDrawableContentRect(target_surface_rect, layer_to_surface_transform, layer_content_rect);
    EXPECT_EQ(expected_visible_layer_rect, drawing_layer->visible_layer_rect());
    EXPECT_EQ(expected_drawable_content_rect, drawing_layer->visible_drawable_content_rect());
    ...
    drawing_layer = TestVisibleRectAndDrawableContentRect(target_surface_rect, layer_to_surface_transform, layer_content_rect);
    EXPECT_EQ(expected_visible_layer_rect, drawing_layer->visible_layer_rect());
    EXPECT_EQ(expected_drawable_content_rect, drawing_layer->visible_drawable_content_rect());
...
TEST_F(DrawPropertiesDrawRectsTest, DrawRectsFor3dOrthographicTransform)
{
    ...
    SkScalar half_width_of_rotated_layer = SkDoubleToScalar((100.0 / sqrt(2.0)) * 0.5);
    layer_to_surface_transform.MakeIdentity();
...
TEST_F(DrawPropertiesDrawRectsTest, DrawRectsFor3dOrthographicIsNotClippedBehindSurface)
{
    gfx::Rect target_surface_rect = gfx::Rect(0, 0, 100, 100);
    gfx::Rect layer_content_rect = gfx::Rect(0, 0, 100, 100);
    gfx::Transform layer_to_surface_transform;
...
TEST_F(DrawPropertiesDrawRectsTest, DrawRectsFor3dPerspectiveWhenClippedByW)
{
    ...
    bool clipped;
    MathUtil::MapQuad(layer_to_surface_transform, gfx::QuadF(gfx::RectF(layer_content_rect)), &clipped);
    ASSERT_TRUE(clipped);
...
static bool ProjectionClips(const gfx::Transform& map_transform, const gfx::RectF& mapped_rect)
{
...
TEST_F(DrawPropertiesDrawRectsTest, DrawRectsForPerspectiveUnprojection)
{
    ...
    gfx::RectF clipped_rect = MathUtil::MapClippedRect(layer_to_surface_transform, gfx::RectF(layer_content_rect));
...
TEST_F(DrawPropertiesTest, DrawableAndVisibleContentRectsForSimpleLayers)
{
    ...
    EXPECT_EQ(gfx::RectF(100.f, 100.f), GetRenderSurface(root)->DrawableContentRect());
    ...
    // the visible part.
    EXPECT_EQ(gfx::Rect(0, 0, 50, 50), child1_layer->visible_drawable_content_rect());
    EXPECT_EQ(gfx::Rect(75, 75, 25, 25), child2_layer->visible_drawable_content_rect());
    EXPECT_EQ(gfx::Rect(125, 125, 0, 0), child3_layer->visible_drawable_content_rect());
}

TEST_F(DrawPropertiesTest, DrawableAndVisibleContentRectsForLayersClippedByLayer)
{
    LayerImpl* root = root_layer();
    LayerImpl* child = AddLayerInActiveTree<LayerImpl>();
    LayerImpl* grand_child1 = AddLayerInActiveTree<LayerImpl>();
    ...
    EXPECT_EQ(gfx::Rect(5, 5, 50, 50), grand_child1->visible_drawable_content_rect());
    EXPECT_EQ(gfx::Rect(75, 75, 25, 25), grand_child2->visible_drawable_content_rect());
...
TEST_F(DrawPropertiesTest, DrawableAndVisibleContentRectsForLayersInUnclippedRenderSurface)
{
    LayerImpl* root = root_layer();
    LayerImpl* render_surface = AddLayerInActiveTree<LayerImpl>();
    LayerImpl* child1 = AddLayerInActiveTree<LayerImpl>();
...
TEST_F(DrawPropertiesTest, DrawableAndVisibleContentRectsForLayersWithUninvertibleTransform)
{
    LayerImpl* root = root_layer();
    LayerImpl* child = AddLayerInActiveTree<LayerImpl>();
...
TEST_F(DrawPropertiesTest, VisibleContentRectForLayerWithUninvertibleDrawTransform)
{
    LayerImpl* root = root_layer();
    LayerImpl* child = AddLayerInActiveTree<LayerImpl>();
    LayerImpl* grand_child = AddLayerInActiveTree<LayerImpl>();
...
TEST_F(DrawPropertiesTestWithLayerTree, OcclusionBySiblingOfTarget)
{
    ...
    scoped_feature_list.InitAndDisableFeature(features::kAllowUndamagedNonrootRenderPassToSkip);
    ...
    EXPECT_TRANSFORM_EQ(GetRenderSurfaceImpl(surface)->draw_transform(), translate);
    // surface_sibling draws into the root render surface and occludes
    // surface_child's contents.
    Occlusion actual_occlusion = GetRenderSurfaceImpl(surface_child)->occlusion_in_content_space();
    Occlusion expected_occlusion(translate, SimpleEnclosedRegion(), SimpleEnclosedRegion(gfx::Rect(200, 200)));
    ...
    expected_occlusion = Occlusion(gfx::Transform(), SimpleEnclosedRegion(gfx::Rect(-20, -20, 200, 200)), SimpleEnclosedRegion());
    EXPECT_TRUE(expected_occlusion.IsEqual(actual_occlusion));
}
...
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeature(features::kAllowUndamagedNonrootRenderPassToSkip);
...
TEST_F(DrawPropertiesTest, OcclusionForLayerWithUninvertibleDrawTransform)
{
    ...
    EXPECT_EQ(layer_bounds, grand_child->draw_properties().occlusion_in_content_space.GetUnoccludedContentRect(layer_bounds));
}

TEST_F(DrawPropertiesTest, DrawableAndVisibleContentRectsForLayersInClippedRenderSurface)
{
    LayerImpl* root = root_layer();
    LayerImpl* render_surface = AddLayerInActiveTree<LayerImpl>();
    LayerImpl* child1 = AddLayerInActiveTree<LayerImpl>();
...
TEST_F(DrawPropertiesTest, DrawableAndVisibleContentRectsForSurfaceHierarchy)
{
    ...
    // A clipped surface grows its DrawableContentRect to include all drawable
    // regions of the subtree, but also gets clamped by the ancestor's clip.
    EXPECT_EQ(gfx::RectF(5.f, 5.f, 95.f, 95.f), GetRenderSurface(render_surface1)->DrawableContentRect());
    ...
    // subtree.
    EXPECT_EQ(gfx::RectF(5.f, 5.f, 95.f, 95.f), GetRenderSurface(render_surface2)->DrawableContentRect());
...
TEST_F(DrawPropertiesTest, VisibleRectsForClippedDescendantsOfUnclippedSurfaces)
{
    LayerImpl* root = root_layer();
    LayerImpl* render_surface1 = AddLayerInActiveTree<LayerImpl>();
    LayerImpl* child1 = AddLayerInActiveTree<LayerImpl>();
...
TEST_F(DrawPropertiesTest, VisibleRectsWhenClipChildIsBetweenTwoRenderSurfaces)
{
    LayerImpl* root = root_layer();
...
TEST_F(DrawPropertiesTest, ClipRectOfSurfaceWhoseParentIsAClipChild)
{
    ...
    CreateTransformNode(render_surface_layer1).post_translation = gfx::Vector2dF(2, 2);
    CreateEffectNode(render_surface_layer1).render_surface_reason = RenderSurfaceReason::kTest;
    ...
    CreateEffectNode(render_surface_layer2).render_surface_reason = RenderSurfaceReason::kTest;
    ...
    if (base::FeatureList::IsEnabled(features::kRenderSurfaceCommonAncestorClip)) {
        EXPECT_EQ(clip_parent->clip_tree_index(), render_surface1->common_ancestor_clip_id());
        EXPECT_TRUE(render_surface1->is_clipped());
        EXPECT_EQ(gfx::Rect(2, 2, 50, 50), render_surface1->clip_rect());
    } else {
        EXPECT_EQ(clip_layer->clip_tree_index(), render_surface1->common_ancestor_clip_id());
    ...
    EXPECT_EQ(clip_parent->clip_tree_index(), render_surface2->common_ancestor_clip_id());
    if (base::FeatureList::IsEnabled(features::kRenderSurfaceCommonAncestorClip)) {
...
TEST_F(DrawPropertiesTest, RenderSurfaceCommonAncestorClipOnChangeOfChildLayerClip)
{
    if (!base::FeatureList::IsEnabled(features::kRenderSurfaceCommonAncestorClip)) {
        return;
    }
    ...
    CreateTransformNode(render_surface_layer).post_translation = gfx::Vector2dF(5, 5);
    CreateEffectNode(render_surface_layer).render_surface_reason = RenderSurfaceReason::kTest;
    ...
    EXPECT_EQ(clip_parent->clip_tree_index(), render_surface->common_ancestor_clip_id());
    ...
    EXPECT_EQ(clip_layer->clip_tree_index(), render_surface->ClipTreeIndex());
    EXPECT_EQ(clip_layer->clip_tree_index(), render_surface->common_ancestor_clip_id());
    EXPECT_TRUE(render_surface->is_clipped());
...
TEST_F(DrawPropertiesTest, VisibleRectsMultipleSurfaces)
{
    ...
    CreateEffectNode(unclipped_surface).render_surface_reason = RenderSurfaceReason::kTest;
    CopyProperties(unclipped_surface, clip_parent);
    CreateClipNode(clip_parent);
    CopyProperties(clip_parent, unclipped_desc_surface);
    CreateEffectNode(unclipped_desc_surface).render_surface_reason = RenderSurfaceReason::kTest;
    CopyProperties(unclipped_desc_surface, clip_child);
    CopyProperties(clip_child, clipped_surface);
    CreateEffectNode(clipped_surface).render_surface_reason = RenderSurfaceReason::kTest;
...
TEST_F(DrawPropertiesTest, DrawableAndVisibleContentRectsWithTransformOnUnclippedSurface)
{
    LayerImpl* root = root_layer();
    LayerImpl* render_surface = AddLayerInActiveTree<LayerImpl>();
    LayerImpl* child1 = AddLayerInActiveTree<LayerImpl>();
    ...
    gfx::Rect expected_surface_drawable_content = gfx::Rect(50 - diagonal_radius, 50 - diagonal_radius, diagonal_radius * 2, diagonal_radius * 2);
    EXPECT_EQ(gfx::RectF(expected_surface_drawable_content), GetRenderSurface(render_surface)->DrawableContentRect());

    // All layers that draw content into the unclipped surface are also unclipped.
    EXPECT_EQ(gfx::Rect(0, 0, 50, 50), child1->visible_layer_rect());
    EXPECT_EQ(expected_surface_drawable_content, child1->visible_drawable_content_rect());
}

// Layers that have non-axis aligned bounds (due to transforms) have an
// expanded, axis-aligned DrawableContentRect and visible content rect.
TEST_F(DrawPropertiesTest, DrawableAndVisibleContentRectsWithTransformOnClippedSurface)
{
    LayerImpl* root = root_layer();
    LayerImpl* render_surface = AddLayerInActiveTree<LayerImpl>();
    LayerImpl* child1 = AddLayerInActiveTree<LayerImpl>();
    ...
    root->layer_tree_impl()
        ...
        ->backdrop_filters.Append(FilterOperation::CreateZoomFilter(1.f /* zoom */, 0 /* inset */));
    ...
    // rotated layer.
    int diagonal_radius = ceil(sqrt(2.0) * 25.0);
    gfx::Rect unclipped_surface_content = gfx::Rect(50 - diagonal_radius, 50 - diagonal_radius, diagonal_radius * 2, diagonal_radius * 2);
    gfx::RectF expected_surface_drawable_content(gfx::IntersectRects(unclipped_surface_content, gfx::Rect(50, 50)));
    EXPECT_EQ(expected_surface_drawable_content, GetRenderSurface(render_surface)->DrawableContentRect());
...
{
    LayerImpl* root = root_layer();
    FakePictureLayerImpl* render_surface1 = AddLayerInActiveTree<FakePictureLayerImpl>();
    FakePictureLayerImpl* render_surface2 = AddLayerInActiveTree<FakePictureLayerImpl>();
    ...
    CreateClipNode(root);
    CopyProperties(root, render_surface1);
    CreateTransformNode(render_surface1).post_translation = gfx::Vector2dF(5.f, 5.f);
    CreateEffectNode(render_surface1).render_surface_reason = RenderSurfaceReason::kTest;
    CopyProperties(render_surface1, render_surface2);
    CreateTransformNode(render_surface2).post_translation = gfx::Vector2dF(5.f, 5.f);
    CreateEffectNode(render_surface2).render_surface_reason = RenderSurfaceReason::kTest;
    CopyProperties(render_surface2, child1);
    child1->SetOffsetToTransformParent(gfx::Vector2dF(5.f, 5.f));
    CopyProperties(render_surface2, child2);
    ...
    // device_scale_factor.
    EXPECT_EQ(gfx::RectF(200.f, 200.f), GetRenderSurface(root)->DrawableContentRect());
    EXPECT_EQ(gfx::RectF(10.f, 10.f, 190.f, 190.f), GetRenderSurface(render_surface1)->DrawableContentRect());
    ...
    // the visible content.
    EXPECT_EQ(gfx::RectF(10.f, 10.f, 180.f, 180.f), GetRenderSurface(render_surface2)->DrawableContentRect());

    EXPECT_EQ(gfx::Rect(10, 10, 100, 100), child1->visible_drawable_content_rect());
    EXPECT_EQ(gfx::Rect(150, 150, 30, 30), child2->visible_drawable_content_rect());
...
TEST_F(DrawPropertiesScalingTest, LayerTransformsInHighDPI)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(device_scale_factor, device_scale_factor), root->GetIdealContentsScale());
    EXPECT_EQ(gfx::Vector2dF(device_scale_factor, device_scale_factor), child->GetIdealContentsScale());
    EXPECT_EQ(gfx::Vector2dF(device_scale_factor, device_scale_factor), child2->GetIdealContentsScale());
    ...
    gfx::RectF expected_child_draw_rect(gfx::PointAtOffsetFromOrigin(child->offset_to_transform_parent()), gfx::SizeF(child->bounds()));
...
TEST_F(DrawPropertiesScalingTest, SurfaceLayerTransformsInHighDPI)
{
    ...
    auto& perspective_surface_transform = CreateTransformNode(perspective_surface);
    ...
    CreateEffectNode(perspective_surface).render_surface_reason = RenderSurfaceReason::kTest;
    ...
    CreateEffectNode(scale_surface).render_surface_reason = RenderSurfaceReason::kTest;
    ...
    EXPECT_EQ(gfx::Vector2dF(contents_scale_factor, contents_scale_factor), parent->GetIdealContentsScale());
    EXPECT_EQ(gfx::Vector2dF(contents_scale_factor, contents_scale_factor), perspective_surface->GetIdealContentsScale());
    ...
    gfx::Vector2dF scales = gfx::ComputeTransform2dScaleComponents(transform, 0.f);
    ...
    gfx::Vector2dF target_space_transform_scales = gfx::ComputeTransform2dScaleComponents(scale_surface->draw_properties().target_space_transform, 0.f);
    ...
    expected_parent_draw_transform.Scale(contents_scale_factor, contents_scale_factor);
    ...
    expected_perspective_surface_draw_transform.Scale(contents_scale_factor, contents_scale_factor);
    expected_perspective_surface_draw_transform.Translate(perspective_surface_offset);
    ...
    expected_perspective_surface_draw_transform.Scale(1.0f / contents_scale_factor, 1.0f / contents_scale_factor);
    gfx::Transform expected_perspective_surface_layer_draw_transform;
    expected_perspective_surface_layer_draw_transform.Scale(contents_scale_factor, contents_scale_factor);
    EXPECT_TRANSFORM_EQ(expected_perspective_surface_draw_transform, GetRenderSurface(perspective_surface)->draw_transform());
    EXPECT_TRANSFORM_EQ(expected_perspective_surface_layer_draw_transform, perspective_surface->DrawTransform());
}
...
TEST_F(DrawPropertiesScalingTest, SmallIdealScale)
{
    ...
    viewport_property_ids.page_scale_transform = page_scale->transform_tree_index();
    ...
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(expected_ideal_scale, expected_ideal_scale), parent->GetIdealContentsScale());

    expected_ideal_scale = device_scale_factor * page_scale_factor * initial_parent_scale * initial_child_scale;
    EXPECT_LT(expected_ideal_scale, 1.f);
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(expected_ideal_scale, expected_ideal_scale), child_scale->GetIdealContentsScale());
}
...
TEST_F(DrawPropertiesScalingTest, IdealScaleForAnimatingLayer)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(initial_parent_scale, initial_parent_scale), parent->GetIdealContentsScale());
    // Animating layers compute ideal scale in the same way as when
    // they are static.
    EXPECT_EQ(gfx::Vector2dF(initial_child_scale * initial_parent_scale, initial_child_scale * initial_parent_scale), child_scale->GetIdealContentsScale());
}
...
TEST_F(DrawPropertiesTest, RenderSurfaceTransformsInHighDPI)
{
    ...
    duplicate_child_non_owner->SetOffsetToTransformParent(child->offset_to_transform_parent());
    ...
    EXPECT_TRANSFORM_EQ(expected_parent_transform, parent->ScreenSpaceTransform());
    ...
    expected_screen_space_transform.Scale(device_scale_factor, device_scale_factor);
    expected_screen_space_transform.Translate(child_offset);
    EXPECT_TRANSFORM_EQ(expected_screen_space_transform, child->ScreenSpaceTransform());

    gfx::Transform expected_duplicate_child_draw_transform = child->DrawTransform();
    EXPECT_TRANSFORM_EQ(expected_duplicate_child_draw_transform, duplicate_child_non_owner->DrawTransform());
    EXPECT_TRANSFORM_EQ(child->ScreenSpaceTransform(), duplicate_child_non_owner->ScreenSpaceTransform());
    EXPECT_EQ(child->visible_drawable_content_rect(), duplicate_child_non_owner->visible_drawable_content_rect());
    EXPECT_EQ(child->bounds(), duplicate_child_non_owner->bounds());

    gfx::Transform expected_render_surface_draw_transform;
    expected_render_surface_draw_transform.Translate(device_scale_factor * child_offset.x(), device_scale_factor * child_offset.y());
    EXPECT_TRANSFORM_EQ(expected_render_surface_draw_transform, GetRenderSurface(child)->draw_transform());

    gfx::Transform expected_surface_draw_transform;
    expected_surface_draw_transform.Translate(device_scale_factor * 2.f, device_scale_factor * 2.f);
    EXPECT_TRANSFORM_EQ(expected_surface_draw_transform, GetRenderSurface(child)->draw_transform());

    gfx::Transform expected_surface_screen_space_transform;
    expected_surface_screen_space_transform.Translate(device_scale_factor * 2.f, device_scale_factor * 2.f);
    EXPECT_TRANSFORM_EQ(expected_surface_screen_space_transform, GetRenderSurface(child)->screen_space_transform());
}

TEST_F(DrawPropertiesTest, RenderSurfaceTransformsInHighDPIAccurateScaleZeroPosition)
{
    LayerImpl* parent = root_layer();
    ...
    EXPECT_TRANSFORM_EQ(gfx::Transform(), GetRenderSurface(child)->draw_transform());
    EXPECT_TRANSFORM_EQ(gfx::Transform(), GetRenderSurface(child)->draw_transform());
    EXPECT_TRANSFORM_EQ(gfx::Transform(), GetRenderSurface(child)->screen_space_transform());
}
...
TEST_F(DrawPropertiesTest, OpacityAnimatingOnPendingTree)
{
    ...
    AddOpacityTransitionToElementWithAnimation(child->element_id(), timeline_impl(), 10.0, 0.0f, 1.0f, false);
    ...
    EXPECT_TRUE(GetPropertyTrees(root)->effect_tree().ContributesToDrawnSurface(child->effect_tree_index()));
    ...
    const EffectTree& active_effect_tree = active_tree->property_trees()->effect_tree();
    EXPECT_TRUE(active_effect_tree.needs_update());

    UpdateActiveTreeDrawProperties();

    EXPECT_FALSE(GetEffectNode(active_child)->is_drawn);
    EXPECT_FALSE(active_effect_tree.ContributesToDrawnSurface(active_child->effect_tree_index()));
}

class BackfaceVisibilityInteropTest : public DrawPropertiesTestBase, public testing::Test {
public:
...
TEST_F(BackfaceVisibilityInteropTest, BackfaceInvisibleTransform)
{
    ...
    EXPECT_TRUE(draw_property_utils::IsLayerBackFaceVisibleForTesting(back_facing, host_impl()->active_tree()->property_trees()));
    EXPECT_TRUE(draw_property_utils::IsLayerBackFaceVisibleForTesting(back_facing_double_sided, host_impl()->active_tree()->property_trees()));
    EXPECT_FALSE(draw_property_utils::IsLayerBackFaceVisibleForTesting(front_facing, host_impl()->active_tree()->property_trees()));

    EXPECT_TRUE(back_facing->raster_even_if_not_drawn());
    EXPECT_TRUE(draw_property_utils::LayerShouldBeSkippedForDrawPropertiesComputation(back_facing, host_impl()->active_tree()->property_trees()));
    EXPECT_FALSE(draw_property_utils::LayerShouldBeSkippedForDrawPropertiesComputation(back_facing_double_sided, host_impl()->active_tree()->property_trees()));
    EXPECT_FALSE(draw_property_utils::LayerShouldBeSkippedForDrawPropertiesComputation(front_facing, host_impl()->active_tree()->property_trees()));
}
...
TEST_F(DrawPropertiesTestWithLayerTree, SubtreeHiddenWithCopyRequest)
{
    ...
    copy_layer->RequestCopyOfOutput(viz::CopyOutputRequest::CreateStubForTesting());

    host()->SetElementIdsForTesting();
    CommitAndActivate();

    EXPECT_TRUE(GetEffectNode(ImplOf(root))->subtree_has_copy_request);
    EXPECT_TRUE(GetEffectNode(ImplOf(copy_grand_parent))->subtree_has_copy_request);
    ...
    EXPECT_EQ(root->element_id(), GetRenderSurfaceList().at(0)->id());
    EXPECT_EQ(copy_grand_parent->element_id(), GetRenderSurfaceList().at(1)->id());
    ...
    EXPECT_TRUE(ImplOf(root)->contributes_to_drawn_render_surface());
    EXPECT_FALSE(ImplOf(copy_grand_parent)->contributes_to_drawn_render_surface());
    EXPECT_FALSE(ImplOf(copy_grand_parent_sibling_before)->contributes_to_drawn_render_surface());
    ...
    // actually hidden.
    EXPECT_FALSE(GetRenderSurfaceImpl(copy_layer)->contributes_to_drawn_surface());
}
...
TEST_F(DrawPropertiesTestWithLayerTree, VisibleRectInNonRootCopyRequest)
{
    ...
    EXPECT_EQ(gfx::Rect(15, 15), ImplOf(copy_clipped_child)->visible_layer_rect());
    ...
    EXPECT_EQ(gfx::Rect(10, 10), ImplOf(copy_clipped_child)->visible_layer_rect());
...
TEST_F(DrawPropertiesTest, TransformedClipParent)
{
    ...
    ASSERT_EQ(gfx::Rect(1, 1, 20, 20), GetRenderSurface(render_surface)->content_rect());

    // The above check implies the two below, but they nicely demonstrate that
    // we've grown, despite the intervening layer's clip.
    ASSERT_TRUE(clip_parent->clip_rect().Contains(GetRenderSurface(render_surface)->content_rect()));
    ASSERT_FALSE(intervening->clip_rect().Contains(GetRenderSurface(render_surface)->content_rect()));
}
...
TEST_F(DrawPropertiesTest, ClipParentWithInterveningRenderSurface)
{
    ...
    EXPECT_EQ(gfx::Rect(1, 1, 40, 40), GetRenderSurface(render_surface1)->clip_rect());
    ...
    EXPECT_EQ(gfx::Rect(0, 0, 0, 0), GetRenderSurface(render_surface2)->clip_rect());
    ...
    EXPECT_EQ(gfx::Rect(0, 0, 40, 40), GetRenderSurface(render_surface1)->content_rect());
    EXPECT_EQ(clip_child->visible_drawable_content_rect(), GetRenderSurface(render_surface2)->content_rect());
}
...
TEST_F(DrawPropertiesTest, ClipParentScrolledInterveningLayer)
{
    ...
    // clip child, render_surface1 can apply the clip as there are no clips
    // between the clip parent and render_surface1
    EXPECT_EQ(gfx::Rect(3, 3, 40, 40), GetRenderSurface(render_surface1)->clip_rect());
    EXPECT_TRUE(GetRenderSurface(render_surface1)->is_clipped());
    EXPECT_EQ(gfx::Rect(), render_surface1->clip_rect());
    EXPECT_FALSE(render_surface1->is_clipped());
...
TEST_F(DrawPropertiesTest, SurfacesShouldBeUnaffectedByNonDescendantClipChildren)
{
    ...
    CreateClipNode(clip_layer);
    CopyProperties(clip_layer, render_surface_layer1);
    CreateTransformNode(render_surface_layer1).post_translation = gfx::Vector2dF(5, 5);
    CreateEffectNode(render_surface_layer1).render_surface_reason = RenderSurfaceReason::kTest;
    CopyProperties(clip_layer, render_surface_layer2);
    CreateTransformNode(render_surface_layer2).post_translation = gfx::Vector2dF(5, 5);
    CreateEffectNode(render_surface_layer2).render_surface_reason = RenderSurfaceReason::kTest;
    CopyProperties(render_surface_layer1, clip_child);
    clip_child->SetClipTreeIndex(clip_parent->clip_tree_index());
    ...
    EXPECT_EQ(clip_layer->clip_tree_index(), render_surface2->common_ancestor_clip_id());
    EXPECT_TRUE(render_surface2->is_clipped());
...
TEST_F(DrawPropertiesTest, TransformAnimationUpdatesBackfaceVisibility)
{
    ...
    root->layer_tree_impl()->SetTransformMutated(back_facing->element_id(), gfx::Transform());
    root->layer_tree_impl()->SetTransformMutated(render_surface2->element_id(), rotate_about_y);
    UpdateActiveTreeDrawProperties();
    ...
    root->layer_tree_impl()->SetTransformMutated(render_surface1->element_id(), rotate_about_y);
    UpdateActiveTreeDrawProperties();
    EXPECT_TRUE(GetEffectNode(render_surface1)->hidden_by_backface_visibility);
...
TEST_F(DrawPropertiesTest, ScrollChildAndScrollParentDifferentTargets)
{
    ...
    CreateTransformNode(scroll_parent_target).post_translation = gfx::Vector2dF(10, 10);
    CreateScrollNode(scroll_parent_target, gfx::Size());
    CreateEffectNode(scroll_parent_target).render_surface_reason = RenderSurfaceReason::kTest;
    ...
    float device_scale_factor = 1.5f;
    UpdateActiveTreeDrawProperties(device_scale_factor);
    EXPECT_EQ(scroll_child->effect_tree_index(), scroll_child_target->effect_tree_index());
...
TEST_F(DrawPropertiesTest, SingularTransformSubtreesDoNotDraw)
{
    ...
    singular_transform.Scale3d(SkDoubleToScalar(1.0), SkDoubleToScalar(1.0), SkDoubleToScalar(0.0));
...
TEST_F(DrawPropertiesTest, ScrollSnapping)
{
    ...
    // Rounded to integers already.
    {
        ...
        EXPECT_VECTOR2DF_EQ(scroller->draw_properties().screen_space_transform.To2dTranslation(), container_offset - scroll_delta);
    }
    ...
    // Scroll delta requiring rounding.
    {
        ...
        EXPECT_VECTOR2DF_EQ(scroller->draw_properties().screen_space_transform.To2dTranslation(), container_offset - rounded_scroll_delta);
    }
}
...
TEST_F(DrawPropertiesTest, ScrollSnappingWithAnimatedScreenSpaceTransform)
{
    ...
    AddAnimatedTransformToElementWithAnimation(animated_layer->element_id(), timeline_impl(), 1.0, start_operations, end_operations);
    ...
    EXPECT_VECTOR2DF_EQ(expected_draw_transform_translation, scroller->DrawTransform().To2dTranslation());
}
...
TEST_F(DrawPropertiesTest, ScrollSnappingWithScrollChild)
{
    ...
    auto& scroll_child_transform = CreateTransformNode(scroll_child, scroller->transform_tree_index());
    ...
    EXPECT_VECTOR2DF_EQ(expected_scroller_screen_space_transform_translation, scroller->ScreenSpaceTransform().To2dTranslation());
    ...
    EXPECT_TRANSFORM_EQ(expected_scroll_child_screen_space_transform, scroll_child->ScreenSpaceTransform());
}
...
protected:
    ...
    scoped_refptr<Layer> CreateSticky(Layer* parent)
    {
        ...
        EnsureStickyData(sticky.get()).scroll_ancestor = parent->scroll_tree_index();
    ...
    {
        return GetPropertyTrees(layer)->transform_tree_mutable().EnsureStickyPositionData(layer->transform_tree_index());
    }
...
TEST_F(DrawPropertiesStickyPositionTest, StickyPositionTop)
{
    ...
    sticky_position.scroll_container_relative_sticky_box_rect = gfx::RectF(10, 20, 10, 10);
    sticky_position.scroll_container_relative_containing_block_rect = gfx::RectF(0, 0, 50, 50);

    CommitAndUpdateImplPointers();

    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(10.f, 20.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
    ...
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(5.f, 15.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
    ...
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(-5.f, 10.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
...
TEST_F(DrawPropertiesStickyPositionTest, StickyPositionTopRounded)
{
    ...
    SetScrollOffsetDelta(scroller_impl_, gfx::Vector2dF(15.f, 15.f));
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(-5.f, 11.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
    SetScrollOffsetDelta(scroller_impl_, gfx::Vector2dF(15.f, 25.f));
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(-5.f, 11.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
...
TEST_F(DrawPropertiesStickyPositionTest, StickyPositionSubpixelScroll)
{
    ...
    sticky_position.scroll_container_relative_sticky_box_rect = gfx::RectF(0, 200, 10, 10);
    sticky_position.scroll_container_relative_containing_block_rect = gfx::RectF(0, 0, 100, 500);
    ...
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, 80.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
}
...
TEST_F(DrawPropertiesStickyPositionTest, StickyPositionBottom)
{
    ...
    sticky_position.bottom_offset = 10.0f;
    sticky_position.constraint_box_rect = gfx::RectF(0, 0, 100, 100);
    sticky_position.scroll_container_relative_sticky_box_rect = gfx::RectF(0, 150, 10, 10);
    sticky_position.scroll_container_relative_containing_block_rect = gfx::RectF(0, 100, 50, 50);

    CommitAndUpdateImplPointers();

    // Initially the sticky element is moved up to the top of the container.
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, 100.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
    SetScrollOffsetDelta(scroller_impl_, gfx::Vector2dF(0.f, 5.f));

    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, 95.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
    ...
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, 0.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
}
...
TEST_F(DrawPropertiesStickyPositionTest, StickyPositionBottomRounded)
{
    ...
    SetScrollOffsetDelta(scroller_impl_, gfx::Vector2dF(0.f, 25.f));
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, 79.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
    SetScrollOffsetDelta(scroller_impl_, gfx::Vector2dF(0.f, 30.f));
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, 79.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
}
...
TEST_F(DrawPropertiesStickyPositionTest, StickyPositionBottomOuterViewportDelta)
{
    ...
    GetPropertyTrees(sticky_pos_.get())->transform_tree_mutable().AddNodeAffectedByOuterViewportBoundsDelta(sticky_pos_->transform_tree_index());
    auto& sticky_position = EnsureStickyData(sticky_pos_.get()).constraints;
    sticky_position.is_anchored_bottom = true;
    sticky_position.bottom_offset = 10.0f;
    sticky_position.constraint_box_rect = gfx::RectF(0, 0, 100, 100);
    sticky_position.scroll_container_relative_sticky_box_rect = gfx::RectF(0, 70, 10, 10);
    sticky_position.scroll_container_relative_containing_block_rect = gfx::RectF(0, 60, 100, 100);

    CommitAndUpdateImplPointers();

    // Initially the sticky element is moved to the bottom of the container.
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, 70.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());

    // We start to hide the toolbar, but not far enough that the sticky element
    // should be moved up yet.
    GetPropertyTrees(scroller_impl_)->SetOuterViewportContainerBoundsDelta(gfx::Vector2dF(0.f, -10.f));

    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, 70.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());

    // On hiding more of the toolbar the sticky element starts to stick.
    GetPropertyTrees(scroller_impl_)->SetOuterViewportContainerBoundsDelta(gfx::Vector2dF(0.f, -20.f));
    UpdateActiveTreeDrawProperties();

    // On hiding more the sticky element stops moving as it has reached its
    // limit.
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, 60.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());

    GetPropertyTrees(scroller_impl_)->SetOuterViewportContainerBoundsDelta(gfx::Vector2dF(0.f, -30.f));
    UpdateActiveTreeDrawProperties();
...
TEST_F(DrawPropertiesStickyPositionTest, StickyPositionLeftRight)
{
    ...
    sticky_position.constraint_box_rect = gfx::RectF(0, 0, 100, 100);
    sticky_position.scroll_container_relative_sticky_box_rect = gfx::RectF(145, 0, 10, 10);
    sticky_position.scroll_container_relative_containing_block_rect = gfx::RectF(100, 0, 100, 100);
    ...
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(100.f, 0.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
    SetScrollOffsetDelta(scroller_impl_, gfx::Vector2dF(5.f, 0.f));
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(95.f, 0.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
    ...
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(80.f, 0.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
    ...
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(50.f, 0.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
    SetScrollOffsetDelta(scroller_impl_, gfx::Vector2dF(105.f, 0.f));
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(40.f, 0.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
    ...
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(10.f, 0.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
    ...
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(-5.f, 0.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
}
...
TEST_F(DrawPropertiesStickyPositionTest, StickyPositionMainThreadUpdates)
{
    ...
    SetPostTranslation(sticky_pos_.get(), gfx::PointF(10, 25) - gfx::PointF(0, 5));
    GetPropertyTrees(scroller_.get())->transform_tree_mutable().set_needs_update(true);
...
TEST_F(DrawPropertiesStickyPositionTest, StickyPositionCompositedContainer)
{
    ...
    SetPostTranslation(sticky_pos_.get(), gfx::Vector2dF(0, 10) + sticky_container->offset_to_transform_parent());
    auto& sticky_position = EnsureStickyData(sticky_pos_.get()).constraints;
    sticky_position.is_anchored_top = true;
    sticky_position.top_offset = 10.0f;
    sticky_position.scroll_container_relative_sticky_box_rect = gfx::RectF(20, 30, 10, 10);
    sticky_position.scroll_container_relative_containing_block_rect = gfx::RectF(20, 20, 30, 30);

    CommitAndUpdateImplPointers();

    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(20.f, 30.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());

    // Scroll less than sticking point, sticky element should move with scroll as
    // we haven't gotten to the initial sticky item location yet.
    SetScrollOffsetDelta(scroller_impl_, gfx::Vector2dF(0.f, 5.f));
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(20.f, 25.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());

    // Scroll past the sticking point, the Y coordinate should now be clamped.
    SetScrollOffsetDelta(scroller_impl_, gfx::Vector2dF(0.f, 25.f));
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(20.f, 10.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());

    // Now the main thread commits the new position of the sticky element.
    SetScrollOffset(scroller_.get(), gfx::PointF(0, 25));
    // Shift the layer by -offset_for_position_sticky.
    SetPostTranslation(sticky_pos_.get(), gfx::PointF(0, 15) - gfx::PointF(0, 5) + sticky_container->offset_to_transform_parent());
    CommitAndUpdateImplPointers();
    ...
    SetScrollOffsetDelta(scroller_impl_, gfx::Vector2dF(0.f, 10.f));
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(20.f, 5.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
}
...
TEST_F(DrawPropertiesStickyPositionTest, StickyPositionScaledStickyBox)
{
    ...
    sticky_position.scroll_container_relative_sticky_box_rect = gfx::RectF(0, 20, 10, 10);
    sticky_position.scroll_container_relative_containing_block_rect = gfx::RectF(0, 0, 50, 50);

    CommitAndUpdateImplPointers();

    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, 20.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
    ...
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, 5.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
    ...
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, -10.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
}
...
TEST_F(DrawPropertiesStickyPositionTest, StickyPositionScaledContainer)
{
    ...
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, 40.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());

    // Scroll less than sticking point, sticky element should move with scroll as
    // we haven't gotten to the initial sticky item location yet.
    SetScrollOffsetDelta(scroller_impl_, gfx::Vector2dF(0.f, 15.f));
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, 25.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
    ...
    SetScrollOffsetDelta(scroller_impl_, gfx::Vector2dF(0.f, 30.f));
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, 30.f), sticky_pos_impl_->ScreenSpaceTransform().To2dTranslation());
...
TEST_F(DrawPropertiesStickyPositionTest, StickyPositionNested)
{
    ...
    outer_sticky_pos.scroll_container_relative_sticky_box_rect = gfx::RectF(0, 50, 10, 50);
    outer_sticky_pos.scroll_container_relative_containing_block_rect = gfx::RectF(0, 0, 50, 400);
    ...
    inner_sticky_pos.scroll_container_relative_sticky_box_rect = gfx::RectF(0, 50, 10, 10);
    inner_sticky_pos.scroll_container_relative_containing_block_rect = gfx::RectF(0, 50, 10, 50);
    EnsureStickyData(inner_sticky.get()).nearest_node_shifting_containing_block = sticky_pos_->transform_tree_index();
    ...
    // original positions.
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, 50.f), outer_sticky_impl->ScreenSpaceTransform().To2dTranslation());
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, 50.f), inner_sticky_impl->ScreenSpaceTransform().To2dTranslation());
    ...
    SetScrollOffsetDelta(scroller_impl_, gfx::Vector2dF(0.f, 5.f));
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, 45.f), outer_sticky_impl->ScreenSpaceTransform().To2dTranslation());
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, 45.f), inner_sticky_impl->ScreenSpaceTransform().To2dTranslation());

    // Scroll such that the inner sticky should stick, but the outer one should
    // keep going as it hasn't reached its position yet.
    SetScrollOffsetDelta(scroller_impl_, gfx::Vector2dF(0.f, 30.f));
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, 20.f), outer_sticky_impl->ScreenSpaceTransform().To2dTranslation());
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, 25.f), inner_sticky_impl->ScreenSpaceTransform().To2dTranslation());

    // Keep going, both should stick.
    SetScrollOffsetDelta(scroller_impl_, gfx::Vector2dF(0.f, 100.f));
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, 10.f), outer_sticky_impl->ScreenSpaceTransform().To2dTranslation());
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(0.f, 25.f), inner_sticky_impl->ScreenSpaceTransform().To2dTranslation());
}
...
protected:
    ...
    std::pair<scoped_refptr<Layer>, scoped_refptr<Layer>> CreateScroller(Layer* parent)
    {
    ...
    scoped_refptr<Layer> CreateAnchored(Layer* parent, std::vector<ElementId> adjustment_container_ids)
    {
        ...
        auto& data = GetPropertyTrees(anchored.get())->transform_tree_mutable().EnsureAnchorPositionScrollData(anchored->transform_tree_index());
...
TEST_F(DrawPropertiesAnchorPositionScrollTest, Basics)
{
    ...
    scoped_refptr<Layer> anchored = CreateAnchored(root_.get(), { scroller->element_id() });

    SetPostTranslation(anchored.get(), gfx::Vector2dF(10, 20));
    Commit();

    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(10, 20), GetImpl(anchored.get())->ScreenSpaceTransform().To2dTranslation());
    ...
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(5, 15), GetImpl(anchored.get())->ScreenSpaceTransform().To2dTranslation());

    SetScrollOffsetDelta(GetImpl(scroller.get()), gfx::Vector2dF(15, 25));

    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(-5, -5), GetImpl(anchored.get())->ScreenSpaceTransform().To2dTranslation());
}
...
TEST_F(DrawPropertiesAnchorPositionScrollTest, NestedScrollers)
{
    ...
    scoped_refptr<Layer> anchored = CreateAnchored(scroller1.get(), { scroller3->element_id(), scroller2->element_id() });
    ...
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(-5, 0), GetImpl(anchored.get())->ScreenSpaceTransform().To2dTranslation());
    ...
    UpdateActiveTreeDrawProperties();
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(-20, -20), GetImpl(anchored.get())->ScreenSpaceTransform().To2dTranslation());
}
class AnimationScaleFactorTrackingLayerImpl : public LayerImpl {
public:
    static std::unique_ptr<AnimationScaleFactorTrackingLayerImpl> Create(LayerTreeImpl* tree_impl, int id)
    {
        return base::WrapUnique(new AnimationScaleFactorTrackingLayerImpl(tree_impl, id));
    }

    ~AnimationScaleFactorTrackingLayerImpl() override = default;

private:
    explicit AnimationScaleFactorTrackingLayerImpl(LayerTreeImpl* tree_impl, int id)
...
{
    LayerImpl* root = root_layer();
    auto* grand_parent = AddLayerInActiveTree<AnimationScaleFactorTrackingLayerImpl>();
    ...
    auto* grand_child = AddLayerInActiveTree<AnimationScaleFactorTrackingLayerImpl>();
    SetElementIdsForTesting();
    ...
    scoped_refptr<Animation> grand_parent_animation = Animation::Create(AnimationIdProvider::NextAnimationId());
    timeline_impl()->AttachAnimation(grand_parent_animation);
    grand_parent_animation->AttachElement(grand_parent->element_id());

    scoped_refptr<Animation> parent_animation = Animation::Create(AnimationIdProvider::NextAnimationId());
    timeline_impl()->AttachAnimation(parent_animation);
    parent_animation->AttachElement(parent->element_id());

    scoped_refptr<Animation> child_animation = Animation::Create(AnimationIdProvider::NextAnimationId());
    timeline_impl()->AttachAnimation(child_animation);
    child_animation->AttachElement(child->element_id());

    scoped_refptr<Animation> grand_child_animation = Animation::Create(AnimationIdProvider::NextAnimationId());
    timeline_impl()->AttachAnimation(grand_child_animation);
    grand_child_animation->AttachElement(grand_child->element_id());

    AddAnimatedTransformToAnimation(parent_animation.get(), 1.0, gfx::TransformOperations(), translation);
    ...
    AddAnimatedTransformToAnimation(child_animation.get(), 1.0, gfx::TransformOperations(), scale);
    UpdateActiveTreeDrawProperties();
    ...
    AddAnimatedTransformToAnimation(grand_parent_animation.get(), 1.0, gfx::TransformOperations(), scale);
    UpdateActiveTreeDrawProperties();
    ...
    AddAnimatedTransformToAnimation(parent_animation.get(), 1.0, gfx::TransformOperations(), scale);
    UpdateActiveTreeDrawProperties();
    ...
    grand_parent_animation->AbortKeyframeModelsWithProperty(TargetProperty::TRANSFORM, false);
    parent_animation->AbortKeyframeModelsWithProperty(TargetProperty::TRANSFORM, false);
    child_animation->AbortKeyframeModelsWithProperty(TargetProperty::TRANSFORM, false);
    ...
    AddAnimatedTransformToAnimation(child_animation.get(), 1.0, perspective, perspective);
    UpdateActiveTreeDrawProperties();
...
static void GatherDrawnLayers(LayerTreeImpl* tree_impl, std::set<LayerImpl*>* drawn_layers)
{
    for (EffectTreeLayerListIterator it(tree_impl); it.state() != EffectTreeLayerListIterator::State::kEnd; ++it) {
        if (it.state() == EffectTreeLayerListIterator::State::kLayer) {
            drawn_layers->insert(it.current_layer());
        }

        if (it.state() != EffectTreeLayerListIterator::State::kContributingSurface) {
            continue;
        }
    }
}
...
TEST_F(DrawPropertiesTestWithLayerTree, DrawPropertyDeviceScale)
{
    ...
    AddAnimatedTransformToElementWithAnimation(child2->element_id(), timeline(), 1.0, gfx::TransformOperations(), scale);
    ...
    EXPECT_EQ(gfx::Vector2dF(16.f, 20.f), ImplOf(child2)->GetIdealContentsScale());
...
TEST_F(DrawPropertiesTest, DrawPropertyScales)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(1.f, 1.f), ImplOf(root)->GetIdealContentsScale());
    EXPECT_EQ(gfx::Vector2dF(1.f, 1.f), ImplOf(page_scale)->GetIdealContentsScale());
    EXPECT_EQ(gfx::Vector2dF(2.f, 3.f), ImplOf(child1)->GetIdealContentsScale());
    EXPECT_EQ(gfx::Vector2dF(4.f, 5.f), ImplOf(child2)->GetIdealContentsScale());
    ...
    EXPECT_EQ(gfx::Vector2dF(1.f, 1.f), ImplOf(root)->GetIdealContentsScale());
    EXPECT_EQ(gfx::Vector2dF(3.f, 3.f), ImplOf(page_scale)->GetIdealContentsScale());
    EXPECT_EQ(gfx::Vector2dF(6.f, 9.f), ImplOf(child1)->GetIdealContentsScale());
    EXPECT_EQ(gfx::Vector2dF(12.f, 15.f), ImplOf(child2)->GetIdealContentsScale());
    ...
    EXPECT_EQ(gfx::Vector2dF(4.f, 4.f), ImplOf(root)->GetIdealContentsScale());
    EXPECT_EQ(gfx::Vector2dF(12.f, 12.f), ImplOf(page_scale)->GetIdealContentsScale());
    EXPECT_EQ(gfx::Vector2dF(24.f, 36.f), ImplOf(child1)->GetIdealContentsScale());
    EXPECT_EQ(gfx::Vector2dF(48.f, 60.f), ImplOf(child2)->GetIdealContentsScale());
...
TEST_F(DrawPropertiesTest, AnimationScales)
{
    ...
    AddAnimatedTransformToElementWithAnimation(child2->element_id(), timeline_impl(), 1.0, gfx::TransformOperations(), scale);
    UpdateActiveTreeDrawProperties();
    ...
    root->layer_tree_impl()->SetTransformMutated(child1->element_id(), gfx::Transform());
    UpdateActiveTreeDrawProperties();
    ...
    host_impl()->active_tree()->property_trees()->SetMaximumAnimationToScreenScaleForTesting(child2->transform_tree_index(), 100.f, affected_by_invalid_scale);
...
TEST_F(DrawPropertiesTest, AnimationScaleFromSmallToOne)
{
    ...
    AddAnimatedTransformToElementWithAnimation(child->element_id(), timeline_impl(), 1.0, small_scale_operations, scale_one_operations);
    AddAnimatedTransformToElementWithAnimation(grandchild->element_id(), timeline_impl(), 1.0, small_scale_operations, scale_one_operations);
    UpdateActiveTreeDrawProperties();
...
TEST_F(DrawPropertiesTest, VisibleContentRectInChildRenderSurface)
{
    ...
    // by the viewport.
    EXPECT_EQ(gfx::Rect(768 / device_scale_factor, 582 / device_scale_factor), root->visible_layer_rect());

    // Layers drawing to a child render surface should still have their visible
    // content rect clipped by the viewport.
    EXPECT_EQ(gfx::Rect(768 / device_scale_factor, 582 / device_scale_factor), content->visible_layer_rect());
}
...
TEST_F(DrawPropertiesTest, ViewportBoundsDeltaAffectVisibleContentRect)
{
    ...
    gfx::Rect affected_by_delta(container_size.width(), container_size.height() + 50);
...
TEST_F(DrawPropertiesTest, VisibleContentRectForAnimatedLayer)
{
    ...
    AddOpacityTransitionToElementWithAnimation(animated->element_id(), timeline_impl(), 10.0, 0.f, 1.f, false);
    UpdatePendingTreeDrawProperties();

    EXPECT_FALSE(animated->visible_layer_rect().IsEmpty());
}

TEST_F(DrawPropertiesTest, VisibleContentRectForAnimatedLayerWithSingularTransform)
{
    host_impl()->CreatePendingTree();
    LayerImpl* root = EnsureRootLayerInPendingTree();
    ...
    AddAnimatedTransformToElementWithAnimation(animated->element_id(), timeline_impl(), 10.0, start_transform_operations, end_transform_operations);
    UpdatePendingTreeDrawProperties();
    ...
    EXPECT_EQ(gfx::Rect(2, 2), descendant_of_keyframe_model->visible_layer_rect());

    gfx::Transform zero_matrix;
    zero_matrix.Scale3d(0.f, 0.f, 0.f);
    root->layer_tree_impl()->SetTransformMutated(animated->element_id(), zero_matrix);
    UpdatePendingTreeDrawProperties();
    ...
    EXPECT_EQ(gfx::Rect(200, 200), descendant_of_keyframe_model->visible_layer_rect());
    ...
    // surface layer list.
    LayerImpl* active_animated = host_impl()->active_tree()->LayerById(animated->id());
...
TEST_F(DrawPropertiesTest, UpdateScrollChildPosition)
{
    ...
    scroll_parent->SetElementId(LayerIdToElementIdForTesting(scroll_parent->id()));
    scroll_parent->SetDrawsContent(true);
...
TEST_F(DrawPropertiesTestWithLayerTree, HasCopyRequestsInTargetSubtree)
{
    ...
    greatgrandchild->RequestCopyOfOutput(viz::CopyOutputRequest::CreateStubForTesting());
...
TEST_F(DrawPropertiesTestWithLayerTree, SkippingSubtreeMain)
{
    ...
    scoped_refptr<FakePictureLayer> greatgrandchild(FakePictureLayer::Create(&client));
    ...
    std::unique_ptr<KeyframeModel> keyframe_model = KeyframeModel::Create(std::unique_ptr<gfx::AnimationCurve>(new FakeTransformTransition(1.0)),
        keyframe_model_id, 1, KeyframeModel::TargetPropertyId(TargetProperty::TRANSFORM));
    keyframe_model->set_fill_mode(KeyframeModel::FillMode::NONE);
    keyframe_model->set_time_offset(base::Milliseconds(-1000));
    AddKeyframeModelToElementWithAnimation(child->element_id(), timeline(), std::move(keyframe_model));
    UpdateMainDrawProperties();
    EXPECT_TRUE(UpdateLayerListContains(grandchild->id()));

    RemoveKeyframeModelFromElementWithExistingKeyframeEffect(child->element_id(), timeline(), keyframe_model_id);
    child->SetTransform(gfx::Transform());
    ...
    // processed anyhow.
    grandchild->RequestCopyOfOutput(viz::CopyOutputRequest::CreateStubForTesting());
    UpdateMainDrawProperties();
    EXPECT_TRUE(UpdateLayerListContains(grandchild->id()));

    // Add an opacity animation with a start delay.
    keyframe_model_id = 1;
    keyframe_model = KeyframeModel::Create(std::unique_ptr<gfx::AnimationCurve>(new FakeFloatTransition(1.0, 0.f, 1.f)), keyframe_model_id, 1,
        KeyframeModel::TargetPropertyId(TargetProperty::OPACITY));
    keyframe_model->set_fill_mode(KeyframeModel::FillMode::NONE);
    keyframe_model->set_time_offset(base::Milliseconds(-1000));
    AddKeyframeModelToElementWithExistingKeyframeEffect(child->element_id(), timeline(), std::move(keyframe_model));
    UpdateMainDrawProperties();
    EXPECT_TRUE(UpdateLayerListContains(grandchild->id()));
}
...
TEST_F(DrawPropertiesTestWithLayerTree, SkippingLayerImpl)
{
    ...
    std::unique_ptr<gfx::KeyframedTransformAnimationCurve> curve(gfx::KeyframedTransformAnimationCurve::Create());
    ...
    curve->AddKeyframe(gfx::TransformKeyframe::Create(base::TimeDelta(), start, nullptr));
    curve->AddKeyframe(gfx::TransformKeyframe::Create(base::Seconds(1.0), operation, nullptr));
    std::unique_ptr<KeyframeModel> transform_animation(
        KeyframeModel::Create(std::move(curve), 3, 3, KeyframeModel::TargetPropertyId(TargetProperty::TRANSFORM)));
...
TEST_F(DrawPropertiesTest, LayerSkippingInSubtreeOfSingularTransform)
{
    ...
    ASSERT_EQ(timeline_impl()->animation_host(), child->layer_tree_impl()->mutator_host());
...
TEST_F(DrawPropertiesTestWithLayerTree, SkippingPendingLayerImpl)
{
    ...
    std::unique_ptr<gfx::KeyframedFloatAnimationCurve> curve(gfx::KeyframedFloatAnimationCurve::Create());
    std::unique_ptr<gfx::TimingFunction> func = gfx::CubicBezierTimingFunction::CreatePreset(gfx::CubicBezierTimingFunction::EaseType::EASE);
    curve->AddKeyframe(gfx::FloatKeyframe::Create(base::TimeDelta(), 0.9f, std::move(func)));
    curve->AddKeyframe(gfx::FloatKeyframe::Create(base::Seconds(1.0), 0.3f, nullptr));
    std::unique_ptr<KeyframeModel> keyframe_model(KeyframeModel::Create(std::move(curve), 3, 3, KeyframeModel::TargetPropertyId(TargetProperty::OPACITY)));
    scoped_refptr<Animation> animation(Animation::Create(1));
    timeline()->AttachAnimation(animation);
...
TEST_F(DrawPropertiesTest, RenderSurfaceWithUnclippedDescendantsClipsSubtree)
{
    ...
    EXPECT_TRUE(render_surface->has_contributing_layer_that_escapes_clip());
    EXPECT_EQ(between_clip_parent_and_child->clip_tree_index(), render_surface->ClipTreeIndex());
    if (base::FeatureList::IsEnabled(features::kRenderSurfaceCommonAncestorClip)) {
        EXPECT_EQ(clip_parent->clip_tree_index(), render_surface->common_ancestor_clip_id());
        EXPECT_TRUE(render_surface->is_clipped());
        EXPECT_EQ(gfx::Rect(2, 2, 30, 30), render_surface->clip_rect());
    } else {
        EXPECT_EQ(between_clip_parent_and_child->clip_tree_index(), render_surface->common_ancestor_clip_id());
        EXPECT_FALSE(render_surface->is_clipped());
    }
    EXPECT_EQ(gfx::Rect(26, 26), render_surface->content_rect());
}

TEST_F(DrawPropertiesTest, RenderSurfaceWithUnclippedDescendantsButDoesntApplyOwnClip)
{
...
TEST_F(DrawPropertiesTest, RenderSurfaceClipsSubtreeAndHasUnclippedDescendants)
{
    LayerImpl* root = root_layer();
    LayerImpl* clip_parent = AddLayerInActiveTree<LayerImpl>();
    LayerImpl* render_surface = AddLayerInActiveTree<LayerImpl>();
...
TEST_F(DrawPropertiesTest, RenderSurfaceContentRectWithMultipleSurfaces)
{
    ...
    CreateEffectNode(unclipped_desc_surface2).render_surface_reason = RenderSurfaceReason::kTest;
    ...
    UpdateActiveTreeDrawProperties();
    EXPECT_EQ(gfx::Rect(50, 50), GetRenderSurface(unclipped_surface)->content_rect());
    EXPECT_EQ(gfx::Rect(50, 50), GetRenderSurface(unclipped_desc_surface)->content_rect());
    EXPECT_EQ(gfx::Rect(50, 50), GetRenderSurface(unclipped_desc_surface2)->content_rect());
    EXPECT_EQ(gfx::Rect(50, 50), GetRenderSurface(clipped_surface)->content_rect());
}
...
TEST_F(DrawPropertiesTest, ClipBetweenClipChildTargetAndClipParentTarget)
{
    ...
    EXPECT_EQ(gfx::Rect(10, 10), GetRenderSurface(unclipped_desc_surface)->content_rect());
}
...
TEST_F(DrawPropertiesTestWithLayerTree, SubtreeIsHiddenTest)
{
    ...
    EXPECT_EQ(0.f, GetRenderSurfaceImpl(test)->OwningEffectNode()->screen_space_opacity);

    hidden->SetHideLayerAndSubtree(false);
    CommitAndActivate();
    EXPECT_EQ(1.f, GetRenderSurfaceImpl(test)->OwningEffectNode()->screen_space_opacity);
}
...
TEST_F(DrawPropertiesTest, TwoUnclippedRenderSurfaces)
{
    ...
    CreateTransformNode(render_surface1).post_translation = gfx::Vector2dF(10, 10);
    CreateEffectNode(render_surface1).render_surface_reason = RenderSurfaceReason::kTest;
    CopyProperties(render_surface1, render_surface2);
    CreateEffectNode(render_surface2).render_surface_reason = RenderSurfaceReason::kTest;
    CopyProperties(render_surface2, clip_child);
...
TEST_F(DrawPropertiesTest, SublayerScaleWithTransformNodeBetweenTwoTargets)
{
    ...
    EXPECT_EQ(gfx::Vector2dF(2.f, 2.f), GetEffectNode(render_surface1)->surface_contents_scale);
    EXPECT_EQ(gfx::Vector2dF(1.f, 1.f), GetEffectNode(between_targets)->surface_contents_scale);
    EXPECT_EQ(gfx::Vector2dF(2.f, 2.f), GetEffectNode(render_surface2)->surface_contents_scale);
...
TEST_F(DrawPropertiesTest, LargeTransformTest)
{
    ...
    EXPECT_EQ(gfx::RectF(), GetRenderSurface(render_surface1)->DrawableContentRect());

    bool is_inf_or_nan = std::isinf(child->DrawTransform().rc(0, 0)) || std::isnan(child->DrawTransform().rc(0, 0));
    EXPECT_TRUE(is_inf_or_nan);

    is_inf_or_nan = std::isinf(child->DrawTransform().rc(1, 1)) || std::isnan(child->DrawTransform().rc(1, 1));
    EXPECT_TRUE(is_inf_or_nan);
...
#if DCHECK_IS_ON()
class DrawPropertiesTestDoubleBlurCheck : public DrawPropertiesTestBase, public testing::Test {
public:
...
TEST_F(DrawPropertiesTestWithLayerTree, OpacityAnimationsTrackingTest)
{
    ...
    int keyframe_model_id = 0;
    std::unique_ptr<KeyframeModel> keyframe_model = KeyframeModel::Create(std::unique_ptr<gfx::AnimationCurve>(new FakeFloatTransition(1.0, 0.f, 1.f)),
        keyframe_model_id, 1, KeyframeModel::TargetPropertyId(TargetProperty::OPACITY));
    keyframe_model->set_fill_mode(KeyframeModel::FillMode::NONE);
    keyframe_model->set_time_offset(base::Milliseconds(-1000));
    KeyframeModel* keyframe_model_ptr = keyframe_model.get();
    AddKeyframeModelToElementWithExistingKeyframeEffect(animated->element_id(), timeline(), std::move(keyframe_model));
    ...
    animation->AbortKeyframeModelsWithProperty(TargetProperty::OPACITY, false /*needs_completion*/);
    node = GetEffectNode(animated.get());
    EXPECT_FALSE(node->is_currently_animating_opacity);
...
TEST_F(DrawPropertiesTestWithLayerTree, TransformAnimationsTrackingTest)
{
    ...
    curve->AddKeyframe(gfx::TransformKeyframe::Create(base::TimeDelta(), start, nullptr));
    curve->AddKeyframe(gfx::TransformKeyframe::Create(base::Seconds(1.0), operation, nullptr));
    std::unique_ptr<KeyframeModel> keyframe_model(KeyframeModel::Create(std::move(curve), 3, 3, KeyframeModel::TargetPropertyId(TargetProperty::TRANSFORM)));
    keyframe_model->set_fill_mode(KeyframeModel::FillMode::NONE);
    keyframe_model->set_time_offset(base::Milliseconds(-1000));
    KeyframeModel* keyframe_model_ptr = keyframe_model.get();
    AddKeyframeModelToElementWithExistingKeyframeEffect(animated->element_id(), timeline(), std::move(keyframe_model));
    ...
    animation->AbortKeyframeModelsWithProperty(TargetProperty::TRANSFORM, false /*needs_completion*/);
    node = GetTransformNode(animated.get());
    EXPECT_FALSE(node->is_currently_animating);
...
TEST_F(DrawPropertiesTestWithLayerTree, CopyRequestScalingTest)
{
    ...
    EXPECT_EQ(gfx::Rect(50, 50), GetRenderSurfaceImpl(copy_layer)->content_rect());
    EXPECT_EQ(gfx::Transform(), GetRenderSurfaceImpl(copy_layer)->draw_transform());
    EXPECT_EQ(gfx::RectF(50.0f, 50.0f), GetRenderSurfaceImpl(copy_layer)->DrawableContentRect());
...
TEST_F(DrawPropertiesTestWithLayerTree, SubtreeHiddenWithCacheRenderSurface)
{
    ...
    ASSERT_EQ(4u, GetRenderSurfaceList().size());
    EXPECT_EQ(root->element_id(), GetRenderSurfaceList().at(0)->id());
    EXPECT_EQ(cache_grand_parent->element_id(), GetRenderSurfaceList().at(1)->id());
    EXPECT_EQ(cache_parent->element_id(), GetRenderSurfaceList().at(2)->id());
    EXPECT_EQ(cache_render_surface->element_id(), GetRenderSurfaceList().at(3)->id());

    // The root render surface should have 2 contributing layers.
    EXPECT_EQ(2, GetRenderSurfaceImpl(root)->num_contributors());
    EXPECT_TRUE(ImplOf(root)->contributes_to_drawn_render_surface());
    EXPECT_FALSE(ImplOf(cache_grand_parent)->contributes_to_drawn_render_surface());
    ...
    EXPECT_TRUE(ImplOf(cache_render_surface)->contributes_to_drawn_render_surface());
    EXPECT_TRUE(ImplOf(cache_child)->contributes_to_drawn_render_surface());
    EXPECT_FALSE(ImplOf(cache_grand_child)->contributes_to_drawn_render_surface());
    ...
    // its actually hidden.
    EXPECT_FALSE(GetRenderSurfaceImpl(cache_render_surface)->contributes_to_drawn_surface());
}

// Needs layer tree mode: copy request.
TEST_F(DrawPropertiesTestWithLayerTree, VisibleRectInNonRootCacheRenderSurface)
{
    auto root = Layer::Create();
    host()->SetRootLayer(root);
    root->SetBounds(gfx::Size(50, 50));
    ...
    EXPECT_EQ(gfx::Rect(120, 120), ImplOf(cache_render_surface_layer)->visible_layer_rect());
    EXPECT_EQ(gfx::Rect(100, 100), ImplOf(copy_layer)->visible_layer_rect());
    EXPECT_EQ(gfx::Rect(20, 20), ImplOf(copy_child)->visible_layer_rect());
    EXPECT_EQ(gfx::Rect(15, 15), ImplOf(copy_clipped_child)->visible_layer_rect());
    ...
    CommitAndActivate();
    EXPECT_EQ(gfx::Rect(50, 50), ImplOf(cache_render_surface_layer)->visible_layer_rect());
    EXPECT_EQ(gfx::Rect(50, 50), ImplOf(copy_layer)->visible_layer_rect());
    EXPECT_EQ(gfx::Rect(10, 10), ImplOf(copy_child)->visible_layer_rect());
    EXPECT_EQ(gfx::Rect(10, 10), ImplOf(copy_clipped_child)->visible_layer_rect());
...
TEST_F(DrawPropertiesTestWithLayerTree, CustomLayerClipBounds)
{
    ...
    const gfx::Rect kExpectedParentLayerBounds(gfx::Point(0, 0) - kClipBounds.OffsetFromOrigin(), gfx::Size(50, 100));
    ...
    expected_parent->SetPosition(gfx::PointF(kExpectedParentLayerBounds.origin()));
    ...
    LayerImpl* expected_parent_impl = layer_tree_impl->LayerById(expected_parent->id());
    LayerImpl* expected_child_impl = layer_tree_impl->LayerById(expected_child->id());
...
TEST_F(DrawPropertiesTestWithLayerTree, CustomLayerClipBoundsWithMaskToBounds)
{
    ...
    const gfx::RectF expected_clip_bounds = gfx::IntersectRects(gfx::RectF(kClipBounds), gfx::RectF(kParentLayerBounds));
...
class DrawPropertiesWithLayerTreeTest : public DrawPropertiesTestWithLayerTree, public testing::WithParamInterface<MaskFilterTestCase> { };
...
TEST_P(DrawPropertiesWithLayerTreeTest, MaskFilterOnRenderSurface)
{
    ...
    EXPECT_NE(test_case.rounded_corners.IsEmpty(), GetRenderSurfaceImpl(child_1)->mask_filter_info().HasRoundedCorners());
    EXPECT_NE(test_case.rounded_corners.IsEmpty(), GetRenderSurfaceImpl(child_2)->mask_filter_info().HasRoundedCorners());
    EXPECT_NE(test_case.rounded_corners.IsEmpty(), GetRenderSurfaceImpl(child_3)->mask_filter_info().HasRoundedCorners());

    EXPECT_NE(test_case.gradient_mask.IsEmpty(), GetRenderSurfaceImpl(child_1)->mask_filter_info().HasGradientMask());
    EXPECT_NE(test_case.gradient_mask.IsEmpty(), GetRenderSurfaceImpl(child_2)->mask_filter_info().HasGradientMask());
    EXPECT_NE(test_case.gradient_mask.IsEmpty(), GetRenderSurfaceImpl(child_3)->mask_filter_info().HasGradientMask());
}

INSTANTIATE_TEST_SUITE_P(DrawPropertiesWithLayerTreeTests, DrawPropertiesWithLayerTreeTest,
    testing::ValuesIn<MaskFilterTestCase>({
        { "WithRoundedCorners", gfx::RoundedCornersF(10.f), gfx::LinearGradient::GetEmpty() },
        { "WithGradientMask", gfx::RoundedCornersF(0.f), gfx::LinearGradient(45) },
        { "WithRoundedCornersAndGradientMask", gfx::RoundedCornersF(10.f), gfx::LinearGradient(45) },
    }),
    [](const testing::TestParamInfo<DrawPropertiesWithLayerTreeTest::ParamType>& info) { return info.param.test_name; });
} // namespace
} // namespace cc
