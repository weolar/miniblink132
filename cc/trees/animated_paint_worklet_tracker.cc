...
AnimatedPaintWorkletTracker::PropertyState::PropertyState(
    PaintWorkletInput::PropertyValue value, base::flat_set<raw_ptr<PictureLayerImpl, CtnExperimental>> layers)
    : animation_value(value)
    , associated_layers(std::move(layers))
{
}

AnimatedPaintWorkletTracker::PropertyState::PropertyState() = default;

AnimatedPaintWorkletTracker::PropertyState::PropertyState(const PropertyState& other) = default;

AnimatedPaintWorkletTracker::PropertyState::~PropertyState() = default;

void AnimatedPaintWorkletTracker::OnCustomPropertyMutated(PaintWorkletInput::PropertyKey property_key, PaintWorkletInput::PropertyValue property_value)
{
    ...
    for (const auto& prop_key : input_properties_animated_on_impl_) {
        ...
        for (PictureLayerImpl* layer : it->second.associated_layers) {
            layer->InvalidatePaintWorklets(
                prop_key, input_properties_.find(prop_key)->second.animation_value, input_properties_.find(prop_key)->second.last_animation_value);
        }
...
PaintWorkletInput::PropertyValue AnimatedPaintWorkletTracker::GetPropertyAnimationValue(const PaintWorkletInput::PropertyKey& key) const
{
...
void AnimatedPaintWorkletTracker::UpdatePaintWorkletInputProperties(
    const std::vector<DiscardableImageMap::PaintWorkletInputWithImageId>& inputs, PictureLayerImpl* layer)
{
    ...
    for (const auto& input : inputs) {
        all_input_properties_vector.insert(
            all_input_properties_vector.end(), std::begin(input.first->GetPropertyKeys()), std::end(input.first->GetPropertyKeys()));
    }
    base::flat_set<PaintWorkletInput::PropertyKey> all_input_properties(std::move(all_input_properties_vector));
...
void AnimatedPaintWorkletTracker::ClearUnusedInputProperties()
{
    ...
    base::EraseIf(
        input_properties_, [](const std::pair<PaintWorkletInput::PropertyKey, PropertyState>& entry) { return entry.second.associated_layers.empty(); });
    for (auto& entry : input_properties_)
