...
void ThrottleDecider::ProcessRenderPass(const viz::CompositorRenderPass& render_pass)
{
    bool foreground_blurred = render_pass.filters.HasFilterOfType(FilterOperation::BLUR);
    std::vector<gfx::RectF> blur_backdrop_filter_bounds;
    for (viz::QuadList::ConstIterator it = render_pass.quad_list.begin(); it != render_pass.quad_list.end(); ++it) {
        const viz::DrawQuad* quad = *it;
        if (const auto* render_pass_quad = quad->DynamicCast<viz::CompositorRenderPassDrawQuad>()) {
            ...
            if (child_rp.backdrop_filters.HasFilterOfType(FilterOperation::BLUR)
                && render_pass_quad->resources.ids[viz::RenderPassDrawQuadInternal::kMaskResourceIdIndex] == viz::kInvalidResourceId) {
                ...
                blur_bounds = quad->shared_quad_state->quad_to_target_transform.MapRect(blur_bounds);
                if (quad->shared_quad_state->clip_rect) {
                    blur_bounds.Intersect(gfx::RectF(*quad->shared_quad_state->clip_rect));
                }
                blur_backdrop_filter_bounds.push_back(blur_bounds);
            }
        } else if (const auto* surface_quad = quad->DynamicCast<viz::SurfaceDrawQuad>()) {
            bool inside_backdrop_filter_bounds = false;
            if (!foreground_blurred && !blur_backdrop_filter_bounds.empty()) {
                gfx::RectF rect_in_target_space = quad->shared_quad_state->quad_to_target_transform.MapRect(gfx::RectF(quad->visible_rect));
                if (quad->shared_quad_state->clip_rect) {
                    rect_in_target_space.Intersect(gfx::RectF(*quad->shared_quad_state->clip_rect));
                }
