...
class LayerTreeFrameSink::ContextLostForwarder : public viz::ContextLostObserver {
public:
    ContextLostForwarder(base::WeakPtr<LayerTreeFrameSink> frame_sink, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    ...
    {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&LayerTreeFrameSink::OnContextLost, frame_sink_));
    }
...
LayerTreeFrameSink::LayerTreeFrameSink(scoped_refptr<viz::RasterContextProvider> context_provider,
    scoped_refptr<RasterContextProviderWrapper> worker_context_provider_wrapper, scoped_refptr<base::SingleThreadTaskRunner> compositor_task_runner,
    gpu::GpuMemoryBufferManager* gpu_memory_buffer_manager, scoped_refptr<gpu::ClientSharedImageInterface> shared_image_interface)
    : context_provider_(std::move(context_provider))
    , worker_context_provider_wrapper_(std::move(worker_context_provider_wrapper))
...
bool LayerTreeFrameSink::BindToClient(LayerTreeFrameSinkClient* client)
{
    ...
    if (auto* worker_context_provider_ptr = worker_context_provider()) {
        ...
        viz::RasterContextProvider::ScopedRasterContextLock lock(worker_context_provider_ptr);
        ...
        worker_context_lost_forwarder_ = std::make_unique<ContextLostForwarder>(weak_ptr_factory_.GetWeakPtr(), compositor_task_runner_);
        worker_context_provider_ptr->AddObserver(worker_context_lost_forwarder_.get());
    }

    // Add GpuChannelLost observer when in software rendering mode.
    if (shared_image_interface_ && (!context_provider_ && !worker_context_provider())) {
//         task_gpu_channel_lost_on_client_thread_
//             = base::BindPostTaskToCurrentDefault(base::BindOnce(&LayerTreeFrameSink::GpuChannelLostOnClientThread, GetWeakPtr()));
...
void LayerTreeFrameSink::DetachFromClient()
{
    ...
    if (worker_context_lost_forwarder_) {
        ...
        viz::RasterContextProvider::ScopedRasterContextLock lock(worker_context_provider_ptr);
        worker_context_provider_ptr->RemoveObserver(worker_context_lost_forwarder_.get());
...
std::unique_ptr<LayerContext> LayerTreeFrameSink::CreateLayerContext(LayerTreeHostImpl& host_impl)
{
    return nullptr;
}
...
scoped_refptr<gpu::ClientSharedImageInterface> LayerTreeFrameSink::shared_image_interface() const
{
