...
class CC_EXPORT SingleThreadProxy : public Proxy, LayerTreeHostImplClient, public SchedulerClient {
public:
    static std::unique_ptr<Proxy> Create(LayerTreeHost* layer_tree_host, LayerTreeHostSingleThreadClient* client, TaskRunnerProvider* task_runner_provider);
    ...
    void SetLayerTreeFrameSink(LayerTreeFrameSink* layer_tree_frame_sink) override;
    ...
    bool StartDeferringCommits(base::TimeDelta timeout, PaintHoldingReason reason) override;
    ...
    void SetPaintWorkletLayerPainter(std::unique_ptr<PaintWorkletLayerPainter> painter) override;
    bool MainFrameWillHappenForTesting() override;
    void RequestBeginMainFrameNotExpected(bool new_state) override;
    void SetSourceURL(ukm::SourceId source_id, const GURL& url) override;
    void SetUkmSmoothnessDestination(base::WritableSharedMemoryMapping ukm_smoothness_data) override;
    void SetRenderFrameObserver(std::unique_ptr<RenderFrameMetadataObserver> observer) override;
    void CompositeImmediatelyForTest(base::TimeTicks frame_begin_time, bool raster, base::OnceClosure callback) override;
    ...
    void DidPresentCompositorFrameOnImplThread(
        uint32_t frame_token, PresentationTimeCallbackBuffer::PendingCallbacks callbacks, const viz::FrameTimingDetails& details) override;
...
protected:
    SingleThreadProxy(LayerTreeHost* layer_tree_host, LayerTreeHostSingleThreadClient* client, TaskRunnerProvider* task_runner_provider);
...
public:
    explicit DebugScopedSetImplThreadAndMainThreadBlocked(TaskRunnerProvider* task_runner_provider)
        : impl_thread_(task_runner_provider)
        , main_thread_blocked_(task_runner_provider)
    {
    }
    DebugScopedSetImplThreadAndMainThreadBlocked(const DebugScopedSetImplThreadAndMainThreadBlocked&) = delete;
    DebugScopedSetImplThreadAndMainThreadBlocked& operator=(const DebugScopedSetImplThreadAndMainThreadBlocked&) = delete;
