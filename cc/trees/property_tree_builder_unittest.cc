...
class PropertyTreeBuilderTest : public LayerTreeImplTestBase, public testing::Test {
public:
...
TEST_F(PropertyTreeBuilderTest, EffectTreeTransformIdTest)
{
    ...
    const int transform_tree_size = GetPropertyTrees(parent.get())->transform_tree().next_available_id();
...
TEST_F(PropertyTreeBuilderTest, EffectNodesForNonAxisAlignedClips)
{
    ...
    EXPECT_EQ(rotate_and_clip->effect_tree_index(), only_clip->effect_tree_index());
    ...
    EXPECT_NE(rotate_and_clip2->effect_tree_index(), only_clip->effect_tree_index());
}

TEST_F(PropertyTreeBuilderTest, RenderSurfaceListForRenderSurfaceWithClippedLayer)
{
    auto root = Layer::Create();
    host()->SetRootLayer(root);
...
TEST_F(PropertyTreeBuilderTest, RenderSurfaceListForTransparentChild)
{
    ...
    EXPECT_EQ(1U, GetRenderSurfaceList().size());
    EXPECT_EQ(static_cast<viz::CompositorRenderPassId>(root->id()), GetRenderSurfaceList().at(0)->render_pass_id());
    EXPECT_EQ(gfx::Rect(), ImplOf(root)->visible_drawable_content_rect());
}

TEST_F(PropertyTreeBuilderTest, RenderSurfaceListForTransparentChildWithBackdropFilter)
{
    auto root = Layer::Create();
    host()->SetRootLayer(root);
    auto render_surface1 = Layer::Create();
    ...
    EXPECT_EQ(gfx::RectF(0, 0, 10, 10), GetRenderSurfaceImpl(root)->DrawableContentRect());
    ...
    host_impl()->active_tree()->SetOpacityMutated(render_surface1->element_id(), 1.f);
...
TEST_F(PropertyTreeBuilderTest, RenderSurfaceListForFilter)
{
    ...
    // (-60,-60 220x220).
    EXPECT_EQ(gfx::RectF(-60, -60, 220, 220), GetRenderSurfaceImpl(parent)->DrawableContentRect());
}
...
TEST_F(PropertyTreeBuilderTest, VisibleRectWithClippingAndFilters)
{
    ...
    EXPECT_EQ(gfx::Rect(50, 50, 10, 10), ImplOf(filter_child)->visible_layer_rect());
    ...
    EXPECT_EQ(gfx::Rect(38, 38, 34, 34), ImplOf(filter_child)->visible_layer_rect());
    EXPECT_EQ(gfx::Rect(-12, -12, 34, 34), GetRenderSurfaceImpl(filter)->content_rect());

    gfx::Transform vertical_flip;
    vertical_flip.Scale(1, -1);
    sk_sp<PaintFilter> flip_filter = sk_make_sp<MatrixPaintFilter>(gfx::TransformToFlattenedSkMatrix(vertical_flip), PaintFlags::FilterQuality::kLow, nullptr);
    FilterOperations reflection_filter;
    reflection_filter.Append(FilterOperation::CreateReferenceFilter(sk_make_sp<XfermodePaintFilter>(SkBlendMode::kSrcOver, std::move(flip_filter), nullptr)));
    filter->SetFilters(reflection_filter);

    CommitAndActivate();

    EXPECT_EQ(gfx::Rect(50, 40, 10, 20), ImplOf(filter_child)->visible_layer_rect());
    EXPECT_EQ(gfx::Rect(0, -10, 10, 20), GetRenderSurfaceImpl(filter)->content_rect());
}
...
TEST_F(PropertyTreeBuilderTest, VisibleRectWithScalingClippingAndFilters)
{
    ...
    EXPECT_EQ(gfx::Rect(38, 38, 34, 34), ImplOf(filter_child)->visible_layer_rect());
    EXPECT_EQ(gfx::Rect(-36, -36, 102, 102), GetRenderSurfaceImpl(filter)->content_rect());
    ...
    EXPECT_EQ(gfx::Rect(50, 40, 10, 20), ImplOf(filter_child)->visible_layer_rect());
    EXPECT_EQ(gfx::Rect(0, -30, 30, 60), GetRenderSurfaceImpl(filter)->content_rect());
}
...
TEST_F(PropertyTreeBuilderTest, TextureLayerSnapping)
{
    ...
    gfx::RectF layer_bounds_in_screen_space = MathUtil::MapClippedRect(child_screen_space_transform, gfx::RectF(gfx::SizeF(child->bounds())));
...
TEST_F(PropertyTreeBuilderTest, AnimatedOpacityCreatesRenderSurface)
{
    ...
    host()->SetElementIdsForTesting();
    AddOpacityTransitionToElementWithAnimation(child->element_id(), timeline(), 10.0, 1.f, 0.2f, false);
    CommitAndActivate();
...
static bool FilterIsAnimating(LayerImpl* layer)
{
    ...
    return host->IsAnimatingProperty(layer->element_id(), layer->GetElementTypeForAnimation(), TargetProperty::FILTER);
}
...
TEST_F(PropertyTreeBuilderTest, AnimatedFilterCreatesRenderSurface)
{
    ...
    host()->SetElementIdsForTesting();
    AddAnimatedFilterToElementWithAnimation(child->element_id(), timeline(), 10.0, 0.1f, 0.2f);
    CommitAndActivate();
...
{
    MutatorHost* host = layer.layer_tree_impl()->mutator_host();
    return host->HasPotentiallyRunningAnimationForProperty(layer.element_id(), layer.GetElementTypeForAnimation(), TargetProperty::FILTER);
}
...
TEST_F(PropertyTreeBuilderTest, DelayedFilterAnimationCreatesRenderSurface)
{
    ...
    curve->AddKeyframe(FilterKeyframe::Create(base::Milliseconds(100), end_filters, nullptr));
    std::unique_ptr<KeyframeModel> keyframe_model = KeyframeModel::Create(std::move(curve), 0, 1, KeyframeModel::TargetPropertyId(TargetProperty::FILTER));
...
TEST_F(PropertyTreeBuilderTest, RenderSurfaceClipsSubtree)
{
    ...
    EXPECT_TRUE(GetRenderSurfaceImpl(root));
    EXPECT_EQ(GetRenderSurfaceImpl(significant_transform), GetRenderSurfaceImpl(root));
    ...
    EXPECT_EQ(GetRenderSurfaceImpl(test_layer), GetRenderSurfaceImpl(render_surface));
...
TEST_F(PropertyTreeBuilderTest, RenderSurfaceListForTrilinearFiltering)
{
    ...
    EXPECT_EQ(gfx::RectF(0, 0, 25, 25), GetRenderSurfaceImpl(parent)->DrawableContentRect());
}
...
TEST_F(PropertyTreeBuilderTest, GradientMask)
{
    ...
    CommitAndActivate();
    {
        ...
        EXPECT_TRUE(layer_impl1->draw_properties().mask_filter_info.HasGradientMask());
    }
    ...
    CommitAndActivate();
    {
        ...
        EXPECT_FALSE(layer_impl1->draw_properties().mask_filter_info.HasRoundedCorners());
        EXPECT_TRUE(layer_impl1->draw_properties().mask_filter_info.HasGradientMask());
        EXPECT_EQ(gfx::RectF(10, 10, 300, 200), layer_impl1->draw_properties().mask_filter_info.bounds());
        // |angle| is updated by the scale transform.
        EXPECT_EQ(34, layer_impl1->draw_properties().mask_filter_info.gradient_mask()->angle());
        EXPECT_EQ(gradient_mask.steps(), layer_impl1->draw_properties().mask_filter_info.gradient_mask()->steps());
    }
    ...
    {
        auto* layer_impl1 = GetLayerImpl(child1);
        EXPECT_EQ(gfx::RRectF::Type::kEmpty, layer_impl1->draw_properties().mask_filter_info.rounded_corner_bounds().GetType());
        EXPECT_FALSE(layer_impl1->draw_properties().mask_filter_info.HasGradientMask());
    }
    ...
    EXPECT_TRUE(GetEffectNode(child1.get())->HasRenderSurface());
    {
        ...
        EXPECT_EQ(gfx::RectF(100, 100), render_surface_impl1->mask_filter_info().bounds());
        EXPECT_EQ(gradient_mask, render_surface_impl1->mask_filter_info().gradient_mask());
    }
    ...
    CommitAndActivate();
    {
        ...
        EXPECT_FALSE(render_surface_impl1->mask_filter_info().HasRoundedCorners());
        EXPECT_TRUE(render_surface_impl1->mask_filter_info().HasGradientMask());
        EXPECT_EQ(gfx::RectF(10, 10, 300, 200), render_surface_impl1->mask_filter_info().bounds());
        // |angle| is updated by the scale transform.
        EXPECT_EQ(34, render_surface_impl1->mask_filter_info().gradient_mask()->angle());
        EXPECT_EQ(gradient_mask.steps(), render_surface_impl1->mask_filter_info().gradient_mask()->steps());
    }
    ...
    {
        auto* render_surface_impl1 = GetRenderSurfaceImpl(child1);
        EXPECT_EQ(gfx::RRectF::Type::kEmpty, render_surface_impl1->mask_filter_info().rounded_corner_bounds().GetType());
...
TEST_F(PropertyTreeBuilderTest, NestedGradientMask)
{
    ...
    {
        auto* render_surface_impl1 = GetRenderSurfaceImpl(child1);
        EXPECT_EQ(gradient_mask1, render_surface_impl1->mask_filter_info().gradient_mask());
        ...
        EXPECT_EQ(gradient_mask2, draw_properties2.mask_filter_info.gradient_mask());
    }
    ...
    EXPECT_TRUE(GetEffectNode(child1.get())->HasRenderSurface());
    {
        ...
        // layer.
        auto* render_surface_impl1 = GetRenderSurfaceImpl(child1);
        EXPECT_EQ(gradient_mask1.steps(), render_surface_impl1->mask_filter_info().gradient_mask()->steps());
        // |angle| is updated by the scale transform.
        EXPECT_EQ(21, render_surface_impl1->mask_filter_info().gradient_mask()->angle());
        ...
        EXPECT_FALSE(draw_properties2.mask_filter_info.HasRoundedCorners());
        EXPECT_TRUE(draw_properties2.mask_filter_info.HasGradientMask());
        EXPECT_EQ(gfx::RectF(30, 10, 300, 150), draw_properties2.mask_filter_info.bounds());
        // |angle| is updated by the scale transform.
        EXPECT_EQ(27, draw_properties2.mask_filter_info.gradient_mask()->angle());
        EXPECT_EQ(gradient_mask2.steps(), draw_properties2.mask_filter_info.gradient_mask()->steps());
    }
...
TEST_F(PropertyTreeBuilderTest, RoundedCornerBounds)
{
    ...
    rounded_corner_layer_1->SetBounds(gfx::ToRoundedSize(kRoundedCornerLayer1Bound.size()));
    layer_1->SetBounds(gfx::Size(10, 10));
    rounded_corner_layer_2->SetBounds(gfx::ToRoundedSize(kRoundedCornerLayer2Bound.size()));
    layer_2->SetBounds(gfx::Size(25, 25));
    rounded_corner_layer_3->SetBounds(gfx::ToRoundedSize(kRoundedCornerLayer3Bound.size()));
    rounded_corner_layer_4->SetBounds(gfx::ToRoundedSize(kRoundedCornerLayer4Bound.size()));
    ...
    rounded_corner_layer_3_transform.Scale(kRoundedCorner3Scale, kRoundedCorner3Scale);
    ...
    // Set Rounded corners
    rounded_corner_layer_1->SetRoundedCorner({ kRoundedCorner1Radius, kRoundedCorner1Radius, kRoundedCorner1Radius, kRoundedCorner1Radius });
    rounded_corner_layer_2->SetRoundedCorner({ kRoundedCorner2Radius, kRoundedCorner2Radius, kRoundedCorner2Radius, kRoundedCorner2Radius });
    rounded_corner_layer_3->SetRoundedCorner({ kRoundedCorner3Radius, kRoundedCorner3Radius, kRoundedCorner3Radius, kRoundedCorner3Radius });
    rounded_corner_layer_4->SetRoundedCorner({ kRoundedCorner4Radius, kRoundedCorner4Radius, kRoundedCorner4Radius, kRoundedCorner4Radius });
    ...
    EXPECT_EQ(rounded_corner_bounds_1.rect(), gfx::RectF(kRoundedCornerLayer1Bound.size()));
    ...
    EXPECT_FLOAT_EQ(rounded_corner_bounds_2.GetSimpleRadius(), kRoundedCorner2Radius);
    EXPECT_EQ(rounded_corner_bounds_2.rect(), gfx::RectF(kRoundedCornerLayer2Bound.size()));
    ...
    gfx::RRectF rounded_corner_bounds_3 = effect_node->mask_filter_info.rounded_corner_bounds();
    EXPECT_FALSE(effect_node->HasRenderSurface());
    EXPECT_FLOAT_EQ(rounded_corner_bounds_3.GetSimpleRadius(), kRoundedCorner3Radius);
    EXPECT_EQ(rounded_corner_bounds_3.rect(), gfx::RectF(kRoundedCornerLayer3Bound.size()));
    ...
    EXPECT_EQ(rounded_corner_bounds_4.rect(), gfx::RectF(kRoundedCornerLayer4Bound.size()));

    CommitAndActivate(kDeviceScale);
    LayerTreeImpl* layer_tree_impl = host()->host_impl()->active_tree();

    // Get the layer impl for each Layer.
    LayerImpl* rounded_corner_layer_1_impl = layer_tree_impl->LayerById(rounded_corner_layer_1->id());
    LayerImpl* rounded_corner_layer_2_impl = layer_tree_impl->LayerById(rounded_corner_layer_2->id());
    LayerImpl* rounded_corner_layer_3_impl = layer_tree_impl->LayerById(rounded_corner_layer_3->id());
    LayerImpl* rounded_corner_layer_4_impl = layer_tree_impl->LayerById(rounded_corner_layer_4->id());
    ...
    const gfx::RRectF actual_rrect_1 = rounded_corner_layer_1_impl->draw_properties().mask_filter_info.rounded_corner_bounds();
    ...
    EXPECT_FLOAT_EQ(actual_rrect_1.GetSimpleRadius(), kRoundedCorner1Radius * kDeviceScale);
    ...
    // corner radius is also scaled by a factor of 1.6.
    const gfx::RRectF actual_self_rrect_2 = rounded_corner_layer_2_impl->draw_properties().mask_filter_info.rounded_corner_bounds();
    EXPECT_TRUE(actual_self_rrect_2.IsEmpty());

    bounds_in_target_space = kRoundedCornerLayer2Bound;
    bounds_in_target_space.Scale(kDeviceScale);
    const gfx::RRectF actual_render_target_rrect_2 = rounded_corner_layer_2_impl->render_target()->mask_filter_info().rounded_corner_bounds();
    EXPECT_EQ(actual_render_target_rrect_2.rect(), bounds_in_target_space);
    EXPECT_FLOAT_EQ(actual_render_target_rrect_2.GetSimpleRadius(), kRoundedCorner2Radius * kDeviceScale);
    ...
    const gfx::RRectF actual_self_rrect_3 = rounded_corner_layer_3_impl->draw_properties().mask_filter_info.rounded_corner_bounds();
    EXPECT_FALSE(actual_self_rrect_3.IsEmpty());

    bounds_in_target_space = kRoundedCornerLayer3Bound;
    bounds_in_target_space += layer_2->position().OffsetFromOrigin() + kLayer2Translation;
    bounds_in_target_space.Scale(kDeviceScale);
    ...
    EXPECT_FLOAT_EQ(actual_self_rrect_3.GetSimpleRadius(), kRoundedCorner3Radius * kDeviceScale * kRoundedCorner3Scale);
    ...
    const gfx::RRectF actual_rrect_4 = rounded_corner_layer_4_impl->draw_properties().mask_filter_info.rounded_corner_bounds();
    ...
    EXPECT_FLOAT_EQ(actual_rrect_4.GetSimpleRadius(), kRoundedCorner4Radius * kDeviceScale * kRoundedCorner3Scale);
}
...
TEST_F(PropertyTreeBuilderTest, RoundedCornerBoundsInterveningRenderTarget)
{
    ...
    const gfx::RRectF actual_rrect_2 = rounded_corner_layer_2_impl->draw_properties().mask_filter_info.rounded_corner_bounds();
    bounds_in_target_space = kRoundedCornerLayer2Bound;
    bounds_in_target_space.Scale(kDeviceScale);
    EXPECT_EQ(actual_rrect_2.rect(), bounds_in_target_space);
    EXPECT_FLOAT_EQ(actual_rrect_2.GetSimpleRadius(), kRoundedCorner2Radius * kDeviceScale);
}
...
TEST_F(PropertyTreeBuilderTest, RoundedCornerBoundsSiblingRenderTarget)
{
    ...
    EXPECT_TRUE(effect_node->HasRenderSurface());
    EXPECT_FLOAT_EQ(rounded_corner_bounds_3.GetSimpleRadius(), kRoundedCorner3Radius);
    EXPECT_EQ(rounded_corner_bounds_1.rect(), gfx::RectF(kRoundedCornerLayer3Bound.size()));
    ...
    // corner radius is also scaled by a factor of 1.6.
    const gfx::RRectF actual_self_rrect_1 = rounded_corner_layer_1_impl->draw_properties().mask_filter_info.rounded_corner_bounds();
    EXPECT_FALSE(actual_self_rrect_1.IsEmpty());

    gfx::RectF bounds_in_target_space = kRoundedCornerLayer1Bound;
    bounds_in_target_space.Scale(kDeviceScale);
    EXPECT_EQ(actual_self_rrect_1.rect(), bounds_in_target_space);
    EXPECT_FLOAT_EQ(actual_self_rrect_1.GetSimpleRadius(), kRoundedCorner1Radius * kDeviceScale);
    ...
    bounds_in_target_space = kRoundedCornerLayer3Bound;
    bounds_in_target_space.Scale(kDeviceScale);
    const gfx::RRectF actual_render_target_rrect_3 = rounded_corner_layer_3_impl->render_target()->mask_filter_info().rounded_corner_bounds();
    EXPECT_EQ(actual_render_target_rrect_3.rect(), bounds_in_target_space);
    EXPECT_FLOAT_EQ(actual_render_target_rrect_3.GetSimpleRadius(), kRoundedCorner3Radius * kDeviceScale);
    ...
    bounds_in_target_space.Scale(kDeviceScale);
    EXPECT_RECTF_EQ(actual_rrect_4.rect(), bounds_in_target_space);
    EXPECT_FLOAT_EQ(actual_rrect_4.GetSimpleRadius(), kRoundedCorner4Radius * kDeviceScale);
}
...
TEST_F(PropertyTreeBuilderTest, FastRoundedCornerDoesNotTriggerRenderSurface)
{
    ...
    // Set up layer bounds.
    root->SetBounds(gfx::Size(100, 100));
    fast_rounded_corner_layer->SetBounds(gfx::ToRoundedSize(kRoundedCornerLayer1Bound.size()));
    layer_1->SetBounds(gfx::Size(25, 25));
    layer_2->SetBounds(gfx::Size(25, 25));
    rounded_corner_layer->SetBounds(gfx::ToRoundedSize(kRoundedCornerLayer2Bound.size()));
    ...
    // Set Rounded corners
    fast_rounded_corner_layer->SetRoundedCorner({ kRoundedCorner1Radius, kRoundedCorner1Radius, kRoundedCorner1Radius, kRoundedCorner1Radius });
    rounded_corner_layer->SetRoundedCorner({ kRoundedCorner2Radius, kRoundedCorner2Radius, kRoundedCorner2Radius, kRoundedCorner2Radius });
    ...
    const EffectNode* effect_node = GetEffectNode(fast_rounded_corner_layer.get());
    gfx::RRectF rounded_corner_bounds_1 = effect_node->mask_filter_info.rounded_corner_bounds();
    EXPECT_FALSE(effect_node->HasRenderSurface());
    ...
    // Get the layer impl for each Layer.
    LayerImpl* fast_rounded_corner_layer_impl = layer_tree_impl->LayerById(fast_rounded_corner_layer->id());
    LayerImpl* layer_1_impl = layer_tree_impl->LayerById(layer_1->id());
    LayerImpl* layer_2_impl = layer_tree_impl->LayerById(layer_2->id());
    LayerImpl* rounded_corner_layer_impl = layer_tree_impl->LayerById(rounded_corner_layer->id());
    ...
    // scale factor is 1.6.
    const gfx::RRectF actual_rrect_1 = fast_rounded_corner_layer_impl->draw_properties().mask_filter_info.rounded_corner_bounds();
    gfx::RectF bounds_in_target_space = kRoundedCornerLayer1Bound;
    bounds_in_target_space.Scale(kDeviceScale);
    EXPECT_EQ(actual_rrect_1.rect(), bounds_in_target_space);
    EXPECT_FLOAT_EQ(actual_rrect_1.GetSimpleRadius(), kRoundedCorner1Radius * kDeviceScale);
    ...
    // layer.
    const gfx::RRectF layer_1_rrect = layer_1_impl->draw_properties().mask_filter_info.rounded_corner_bounds();
    const gfx::RRectF layer_2_rrect = layer_2_impl->draw_properties().mask_filter_info.rounded_corner_bounds();
    ...
    // scale factor is 1.6 thus giving the target space origin of [64, 64]. The
    // corner radius is also scaled by a factor of 1.6.
    const gfx::RRectF actual_self_rrect_2 = rounded_corner_layer_impl->draw_properties().mask_filter_info.rounded_corner_bounds();
    EXPECT_TRUE(actual_self_rrect_2.IsEmpty());

    bounds_in_target_space = kRoundedCornerLayer2Bound;
    bounds_in_target_space.Scale(kDeviceScale);
    const gfx::RRectF actual_render_target_rrect_2 = rounded_corner_layer_impl->render_target()->mask_filter_info().rounded_corner_bounds();
    EXPECT_EQ(actual_render_target_rrect_2.rect(), bounds_in_target_space);
    EXPECT_FLOAT_EQ(actual_render_target_rrect_2.GetSimpleRadius(), kRoundedCorner2Radius * kDeviceScale);

    // Layer 3 and layer 4 should have no rounded corner bounds set as their
    // parent is a render surface.
    const gfx::RRectF layer_3_rrect = layer_3_impl->draw_properties().mask_filter_info.rounded_corner_bounds();
    const gfx::RRectF layer_4_rrect = layer_4_impl->draw_properties().mask_filter_info.rounded_corner_bounds();
    EXPECT_TRUE(layer_3_rrect.IsEmpty());
    EXPECT_TRUE(layer_4_rrect.IsEmpty());
}

TEST_F(PropertyTreeBuilderTest, NonFittingFastRoundedCornerTriggersRenderSurfaceInAncestor)
{
    ...
    root->SetBounds(gfx::Size(100, 100));
    rounded_corner_layer_1->SetBounds(gfx::ToRoundedSize(kRoundedCornerLayer1Bound.size()));
    fast_rounded_corner_layer_2->SetBounds(gfx::ToRoundedSize(kRoundedCornerLayer2Bound.size()));
    rounded_corner_layer_3->SetBounds(gfx::ToRoundedSize(kRoundedCornerLayer3Bound.size()));
    rounded_corner_layer_4->SetBounds(gfx::ToRoundedSize(kRoundedCornerLayer4Bound.size()));
    rounded_corner_layer_5->SetBounds(gfx::ToRoundedSize(kRoundedCornerLayer5Bound.size()));
    fast_rounded_corner_layer_6->SetBounds(gfx::ToRoundedSize(kRoundedCornerLayer6Bound.size()));
    ...
    // Set Rounded corners
    rounded_corner_layer_1->SetRoundedCorner({ kRoundedCorner1Radius, kRoundedCorner1Radius, kRoundedCorner1Radius, kRoundedCorner1Radius });
    fast_rounded_corner_layer_2->SetRoundedCorner({ kRoundedCorner2Radius, kRoundedCorner2Radius, kRoundedCorner2Radius, kRoundedCorner2Radius });
    rounded_corner_layer_3->SetRoundedCorner({ kRoundedCorner3Radius, kRoundedCorner3Radius, kRoundedCorner3Radius, kRoundedCorner3Radius });
    rounded_corner_layer_4->SetRoundedCorner({ kRoundedCorner4Radius, kRoundedCorner4Radius, kRoundedCorner4Radius, kRoundedCorner4Radius });
    rounded_corner_layer_5->SetRoundedCorner({ kRoundedCorner5Radius, kRoundedCorner5Radius, kRoundedCorner5Radius, kRoundedCorner5Radius });
    fast_rounded_corner_layer_6->SetRoundedCorner({ kRoundedCorner6Radius, kRoundedCorner6Radius, kRoundedCorner6Radius, kRoundedCorner6Radius });
    ...
    gfx::RRectF rounded_corner_bounds_5 = effect_node->mask_filter_info.rounded_corner_bounds();
    EXPECT_TRUE(effect_node->HasRenderSurface());
    EXPECT_FALSE(effect_node->is_fast_rounded_corner);
    EXPECT_FLOAT_EQ(rounded_corner_bounds_5.GetSimpleRadius(), kRoundedCorner5Radius);
    EXPECT_EQ(rounded_corner_bounds_5.rect(), gfx::RectF(kRoundedCornerLayer5Bound.size()));

    // Since this layer has no descendant with rounded corner or drawable, it will
    // not have a render surface.
    effect_node = GetEffectNode(fast_rounded_corner_layer_6.get());
    gfx::RRectF rounded_corner_bounds_6 = effect_node->mask_filter_info.rounded_corner_bounds();
    EXPECT_FALSE(effect_node->HasRenderSurface());
    EXPECT_TRUE(effect_node->is_fast_rounded_corner);
    EXPECT_FLOAT_EQ(rounded_corner_bounds_6.GetSimpleRadius(), kRoundedCorner6Radius);
    EXPECT_EQ(rounded_corner_bounds_6.rect(), gfx::RectF(kRoundedCornerLayer6Bound.size()));

    CommitAndActivate(kDeviceScale);
    LayerTreeImpl* layer_tree_impl = host()->host_impl()->active_tree();

    // Get the layer impl for each Layer.
    LayerImpl* rounded_corner_layer_impl_1 = layer_tree_impl->LayerById(rounded_corner_layer_1->id());
    LayerImpl* fast_rounded_corner_layer_impl_2 = layer_tree_impl->LayerById(fast_rounded_corner_layer_2->id());
    LayerImpl* rounded_corner_layer_impl_3 = layer_tree_impl->LayerById(rounded_corner_layer_3->id());
    LayerImpl* rounded_corner_layer_impl_4 = layer_tree_impl->LayerById(rounded_corner_layer_4->id());
    LayerImpl* rounded_corner_layer_impl_5 = layer_tree_impl->LayerById(rounded_corner_layer_5->id());
    LayerImpl* fast_rounded_corner_layer_impl_6 = layer_tree_impl->LayerById(fast_rounded_corner_layer_6->id());
    ...
    const gfx::RRectF actual_self_rrect_1 = rounded_corner_layer_impl_1->draw_properties().mask_filter_info.rounded_corner_bounds();
    EXPECT_FALSE(actual_self_rrect_1.IsEmpty());
    ...
    const gfx::RRectF actual_self_rrect_2 = fast_rounded_corner_layer_impl_2->draw_properties().mask_filter_info.rounded_corner_bounds();
    bounds_in_target_space = kRoundedCornerLayer2Bound;
    bounds_in_target_space += kRoundedCornerLayer1Bound.OffsetFromOrigin();
    bounds_in_target_space.Scale(kDeviceScale);
    EXPECT_EQ(actual_self_rrect_2.rect(), bounds_in_target_space);
    EXPECT_FLOAT_EQ(actual_self_rrect_2.GetSimpleRadius(), kRoundedCorner2Radius * kDeviceScale);
    ...
    // scale factor is 1.6 thus giving the target space origin of [64, 64]. The
    // corner radius is also scaled by a factor of 1.6.
    const gfx::RRectF actual_self_rrect_3 = rounded_corner_layer_impl_3->draw_properties().mask_filter_info.rounded_corner_bounds();
    EXPECT_FALSE(actual_self_rrect_3.IsEmpty());

    bounds_in_target_space = kRoundedCornerLayer3Bound;
    bounds_in_target_space.Scale(kDeviceScale);
    EXPECT_EQ(actual_self_rrect_3.rect(), bounds_in_target_space);
    EXPECT_FLOAT_EQ(actual_self_rrect_3.GetSimpleRadius(), kRoundedCorner3Radius * kDeviceScale);
    ...
    // corner radius is also scaled by a factor of 1.6.
    const gfx::RRectF actual_self_rrect_4 = rounded_corner_layer_impl_4->draw_properties().mask_filter_info.rounded_corner_bounds();
    bounds_in_target_space = kRoundedCornerLayer4Bound;
    ...
    EXPECT_FLOAT_EQ(actual_self_rrect_4.GetSimpleRadius(), kRoundedCorner4Radius * kDeviceScale);
    ...
    const gfx::RRectF actual_self_rrect_5 = rounded_corner_layer_impl_5->draw_properties().mask_filter_info.rounded_corner_bounds();
    EXPECT_TRUE(actual_self_rrect_5.IsEmpty());

    bounds_in_target_space = kRoundedCornerLayer5Bound;
    bounds_in_target_space.Scale(kDeviceScale);
    const gfx::RRectF actual_render_target_rrect_5 = rounded_corner_layer_impl_5->render_target()->mask_filter_info().rounded_corner_bounds();
    EXPECT_EQ(actual_render_target_rrect_5.rect(), bounds_in_target_space);
    EXPECT_FLOAT_EQ(actual_render_target_rrect_5.GetSimpleRadius(), kRoundedCorner5Radius * kDeviceScale);
    ...
    // also scaled by a factor of 1.6.
    const gfx::RRectF actual_self_rrect_6 = fast_rounded_corner_layer_impl_6->draw_properties().mask_filter_info.rounded_corner_bounds();
    bounds_in_target_space = kRoundedCornerLayer6Bound;
    bounds_in_target_space.Scale(kDeviceScale);
    EXPECT_RECTF_EQ(actual_self_rrect_6.rect(), bounds_in_target_space);
    EXPECT_FLOAT_EQ(actual_self_rrect_6.GetSimpleRadius(), kRoundedCorner6Radius * kDeviceScale);
}

TEST_F(PropertyTreeBuilderTest, NonOverlappingRoundedCornerDoesNotTriggerRenderSurfaceFromSubtree)
{
    ...
    // Set layer positions.
    fast_rounded_corner_layer_1->SetPosition(kFastRoundedCornerLayer1Bound.origin());
    rounded_corner_layer_1->SetPosition(kRoundedCornerLayer1Bound.origin());
    rounded_corner_layer_2->SetPosition(kRoundedCornerLayer2Bound.origin());
    rounded_corner_layer_3->SetPosition(kRoundedCornerLayer3Bound.origin());
    rounded_corner_layer_4->SetPosition(kRoundedCornerLayer4Bound.origin());
    ...
    // Set up layer bounds.
    root->SetBounds(gfx::Size(100, 100));
    fast_rounded_corner_layer_1->SetBounds(gfx::ToRoundedSize(kFastRoundedCornerLayer1Bound.size()));
    rounded_corner_layer_1->SetBounds(gfx::ToRoundedSize(kRoundedCornerLayer1Bound.size()));
    rounded_corner_layer_2->SetBounds(gfx::ToRoundedSize(kRoundedCornerLayer2Bound.size()));
    rounded_corner_layer_3->SetBounds(gfx::ToRoundedSize(kRoundedCornerLayer3Bound.size()));
    rounded_corner_layer_4->SetBounds(gfx::ToRoundedSize(kRoundedCornerLayer4Bound.size()));
    ...
    fast_rounded_corner_layer_1->SetRoundedCorner(
        { kFastRoundedCorner1Radius, kFastRoundedCorner1Radius, kFastRoundedCorner1Radius, kFastRoundedCorner1Radius });
    rounded_corner_layer_1->SetRoundedCorner({ kRoundedCorner1Radius, kRoundedCorner1Radius, kRoundedCorner1Radius, kRoundedCorner1Radius });
    rounded_corner_layer_2->SetRoundedCorner({ kRoundedCorner2Radius, kRoundedCorner2Radius, kRoundedCorner2Radius, kRoundedCorner2Radius });
    rounded_corner_layer_3->SetRoundedCorner({ kRoundedCorner3Radius, kRoundedCorner3Radius, kRoundedCorner3Radius, kRoundedCorner3Radius });
    rounded_corner_layer_4->SetRoundedCorner({ kRoundedCorner4Radius, kRoundedCorner4Radius, kRoundedCorner4Radius, kRoundedCorner4Radius });
    ...
    const EffectNode* effect_node = GetEffectNode(fast_rounded_corner_layer_1.get());
    gfx::RRectF fast_rounded_corner_bounds_1 = effect_node->mask_filter_info.rounded_corner_bounds();
    EXPECT_FALSE(effect_node->HasRenderSurface());
    EXPECT_TRUE(effect_node->is_fast_rounded_corner);
    EXPECT_FLOAT_EQ(fast_rounded_corner_bounds_1.GetSimpleRadius(), kFastRoundedCorner1Radius);
    EXPECT_EQ(fast_rounded_corner_bounds_1.rect(), gfx::RectF(kFastRoundedCornerLayer1Bound.size()));
    ...
    // Get the layer impl for each Layer.
    LayerImpl* fast_rounded_corner_layer_impl_1 = layer_tree_impl->LayerById(fast_rounded_corner_layer_1->id());
    LayerImpl* rounded_corner_layer_impl_1 = layer_tree_impl->LayerById(rounded_corner_layer_1->id());
    LayerImpl* rounded_corner_layer_impl_2 = layer_tree_impl->LayerById(rounded_corner_layer_2->id());
    LayerImpl* rounded_corner_layer_impl_3 = layer_tree_impl->LayerById(rounded_corner_layer_3->id());
    LayerImpl* rounded_corner_layer_impl_4 = layer_tree_impl->LayerById(rounded_corner_layer_4->id());
    LayerImpl* rounded_corner_layer_impl_5 = layer_tree_impl->LayerById(rounded_corner_layer_5->id());
    ...
    // corner radius is also scaled by a factor of 1.6.
    const gfx::RRectF actual_self_rrect_fast_1 = fast_rounded_corner_layer_impl_1->draw_properties().mask_filter_info.rounded_corner_bounds();
    ...
    EXPECT_FLOAT_EQ(actual_self_rrect_fast_1.GetSimpleRadius(), kFastRoundedCorner1Radius * kDeviceScale);
    ...
    // corner radius is also scaled by a factor of 1.6.
    const gfx::RRectF actual_self_rrect_2 = rounded_corner_layer_impl_2->draw_properties().mask_filter_info.rounded_corner_bounds();
    ...
    bounds_in_target_space = kRoundedCornerLayer4Bound;
    bounds_in_target_space.Scale(kDeviceScale);
    const gfx::RRectF actual_render_target_rrect_4 = rounded_corner_layer_impl_4->render_target()->mask_filter_info().rounded_corner_bounds();
    EXPECT_EQ(actual_render_target_rrect_4.rect(), bounds_in_target_space);
    EXPECT_FLOAT_EQ(actual_render_target_rrect_4.GetSimpleRadius(), kRoundedCorner4Radius * kDeviceScale);
    ...
    EXPECT_FLOAT_EQ(actual_self_rrect_5.GetSimpleRadius(), kRoundedCorner5Radius * kDeviceScale);
}
