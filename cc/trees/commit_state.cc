...
CommitState::CommitState(const CommitState& prev)
    ...
    , primary_main_frame_item_sequence_number(prev.primary_main_frame_item_sequence_number)
{
    memcpy(event_listener_properties, prev.event_listener_properties, sizeof(event_listener_properties));
}
...
EventListenerProperties CommitState::GetEventListenerProperties(EventListenerClass listener_class) const
{
...
ThreadUnsafeCommitState::ThreadUnsafeCommitState(MutatorHost* mh, const ProtectedSequenceSynchronizer& synchronizer)
    : mutator_host(mh)
    , property_trees(synchronizer)
{
}
