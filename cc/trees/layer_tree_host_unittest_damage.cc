...
class LayerTreeHostDamageTestSetNeedsRedraw : public LayerTreeHostDamageTest {
    ...
    void DidCommitAndDrawFrame() override
    {
        ...
        case 1:
            layer_tree_host()->SetNeedsRedrawRect(layer_tree_host()->device_viewport_rect());
            break;
        }
    }
    ...
    DrawResult PrepareToDrawOnThread(LayerTreeHostImpl* impl, LayerTreeHostImpl::FrameData* frame_data, DrawResult draw_result) override
    {
        ...
        RenderSurfaceImpl* root_surface = GetRenderSurface(impl->active_tree()->root_layer());
        gfx::Rect root_damage;
        EXPECT_TRUE(root_surface->damage_tracker()->GetDamageRectIfValid(&root_damage));
...
class LayerTreeHostDamageTestSetViewportRectAndScale : public LayerTreeHostDamageTest {
    void SetupTree() override
    {
        // Viewport is 10x10.
        scoped_refptr<FakePictureLayer> root = FakePictureLayer::Create(&client_);
    ...
    void DidCommitAndDrawFrame() override
    {
        switch (layer_tree_host()->SourceFrameNumber()) {
        case 1:
            layer_tree_host()->SetViewportRectAndScale(gfx::Rect(15, 15), 1.f, GetCurrentLocalSurfaceId());
            break;
        }
    }
...
class LayerTreeHostDamageTestNoDamageDoesNotSwap : public LayerTreeHostDamageTest {
    void BeginTest() override
    {
        PostSetNeedsCommitToMainThread();
    }
    ...
    void DidCommit() override
    {
        ...
        case 2:
            ...
            content_->SetNeedsDisplayRect(layer_tree_host()->device_viewport_rect());
            break;
        case 3:
...
class LayerTreeHostDamageTestForcedFullDamage : public LayerTreeHostDamageTest {
    ...
    DrawResult PrepareToDrawOnThread(LayerTreeHostImpl* host_impl, LayerTreeHostImpl::FrameData* frame_data, DrawResult draw_result) override
    {
        ...
        RenderSurfaceImpl* root_surface = GetRenderSurface(host_impl->active_tree()->root_layer());
        gfx::Rect root_damage;
        EXPECT_TRUE(root_surface->damage_tracker()->GetDamageRectIfValid(&root_damage));
...
class LayerTreeHostScrollbarDamageTest : public LayerTreeHostDamageTest {
    void SetupTree() override
    {
        ...
        content_layer_->SetElementId(LayerIdToElementIdForTesting(content_layer_->id()));
        ...
        auto scrollbar_layer = base::MakeRefCounted<FakePaintedScrollbarLayer>(content_layer_->element_id());
        scrollbar_layer->SetPosition(gfx::PointF(300.f, 300.f));
        scrollbar_layer->SetBounds(gfx::Size(10, 100));
        root_layer->AddChild(scrollbar_layer);

        gfx::RectF content_rect(content_layer_->position(), gfx::SizeF(content_layer_->bounds()));
        gfx::RectF scrollbar_rect(scrollbar_layer->position(), gfx::SizeF(scrollbar_layer->bounds()));
...
class LayerTreeHostDamageTestScrollbarDoesDamage : public LayerTreeHostScrollbarDamageTest {
    void BeginTest() override
    {
        num_draws_ = 0;
        PostSetNeedsCommitToMainThread();
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* host_impl) override
    {
        ...
        case 1:
            ...
            MainThreadTaskRunner()->PostTask(
                FROM_HERE, base::BindOnce(&LayerTreeHostDamageTestScrollbarDoesDamage::ModifyContentLayerPosition, base::Unretained(this)));
            break;
        case 2:
        ...
        case 3:
            ...
            MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&LayerTreeHostDamageTestScrollbarDoesDamage::ResizeScrollLayer, base::Unretained(this)));
            break;
        }
    }
    ...
    void ResizeScrollLayer()
    {
        ...
        content_layer_->SetBounds(gfx::Size(root->bounds().width() + 60, root->bounds().height() + 100));
    }
