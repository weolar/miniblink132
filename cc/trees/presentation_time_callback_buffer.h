...
public:
    ...
    using SuccessfulCallbackWithDetails = base::OnceCallback<void(const viz::FrameTimingDetails& details)>;

    PresentationTimeCallbackBuffer();

    PresentationTimeCallbackBuffer(const PresentationTimeCallbackBuffer&) = delete;
    PresentationTimeCallbackBuffer(PresentationTimeCallbackBuffer&&);

    PresentationTimeCallbackBuffer& operator=(const PresentationTimeCallbackBuffer&) = delete;
    ...
    void RegisterMainThreadCallbacks(uint32_t frame_token, std::vector<Callback> callbacks);
    ...
    void RegisterMainThreadSuccessfulCallbacks(uint32_t frame_token, std::vector<SuccessfulCallbackWithDetails> callbacks);

    // Buffers the given `callbacks` in preparation for a successful presentation
    // at or after the given `frame_token`. Calling code invokes these callbacks
    // on the compositor thread once they're popped.
    void RegisterCompositorThreadSuccessfulCallbacks(uint32_t frame_token, std::vector<SuccessfulCallback> callbacks);
    ...
    // threads/sequences.
    PendingCallbacks PopPendingCallbacks(uint32_t frame_token, bool presentation_failed);
