...
bool AreScrollOffsetsEqual(const SyncedScrollOffset* a, const SyncedScrollOffset* b)
{
    return a->ActiveBase() == b->ActiveBase() && a->PendingBase() == b->PendingBase() && a->Delta() == b->Delta() && a->PendingDelta() == b->PendingDelta();
}

class MockLayerImpl : public LayerImpl {
public:
    static std::unique_ptr<MockLayerImpl> Create(LayerTreeImpl* tree_impl, int layer_id)
    {
...
public:
    static scoped_refptr<MockLayer> Create(std::vector<int>* layer_impl_destruction_list)
    {
    ...
    void PushPropertiesTo(LayerImpl* layer_impl, const CommitState& commit_state, const ThreadUnsafeCommitState& unsafe_state) override
    {
...
void ExpectTreesAreIdentical(Layer* root_layer, LayerImpl* root_layer_impl, LayerTreeImpl* tree_impl)
{
    auto layer_iter = root_layer->layer_tree_host()->begin();
    auto layer_impl_iter = tree_impl->begin();
    for (; layer_iter != root_layer->layer_tree_host()->end(); ++layer_iter, ++layer_impl_iter) {
        ...
        EXPECT_EQ(layer->main_thread_scroll_hit_test_region(), layer_impl->main_thread_scroll_hit_test_region());
    }
}
...
public:
    void ResetLayerTreeHost(const LayerTreeSettings& settings)
    {
        ...
        // LayerTreeHost.
        host_.reset();
        host_ = FakeLayerTreeHost::Create(&client_, &task_graph_runner_, animation_host_.get(), settings);
        host_->InitializeSingleThreaded(&single_thread_client_, base::SingleThreadTaskRunner::GetCurrentDefault());
    ...
    scoped_refptr<Layer> SetupScrollLayer()
    {
        ...
        ExpectTreesAreIdentical(layer_tree_root.get(), host_impl->active_tree()->root_layer(), host_impl->active_tree());
...
protected:
    ...
    const FakeLayerTreeHost* const_host() const
    {
        return host_.get();
    }
...
{
    TreeSynchronizer::SynchronizeTrees(*const_host()->pending_commit_state(), host_->GetThreadUnsafeCommitState(), host_->pending_tree());
...
TEST_F(TreeSynchronizerTest, SyncSimpleTreeFromEmpty)
{
    ...
    EXPECT_TRUE(base::Contains(host_->pending_tree()->LayersThatShouldPushProperties(), root));

    ExpectTreesAreIdentical(layer_tree_root.get(), host_->pending_tree()->root_layer(), host_->pending_tree());
}
...
TEST_F(TreeSynchronizerTest, SyncSimpleTreeAndPushPropertiesFromEmpty)
{
    ...
    TreeSynchronizer::PushLayerProperties(*host_->GetPendingCommitState(), host_->GetThreadUnsafeCommitState(), host_->pending_tree());

    // Now sync from pending tree to active tree. This would clear the map of
    // layers that need push properties.
    TreeSynchronizer::SynchronizeTrees(host_->pending_tree(), host_->active_tree());
    TreeSynchronizer::PushLayerProperties(host_->pending_tree(), host_->active_tree());
    EXPECT_FALSE(base::Contains(host_->pending_tree()->LayersThatShouldPushProperties(), root));

    // Set the main thread root layer needs push properties.
    layer_tree_root->SetNeedsPushProperties();
    EXPECT_TRUE(base::Contains(const_host()->pending_commit_state()->layers_that_should_push_properties, layer_tree_root.get()));
...
TEST_F(TreeSynchronizerTest, SyncSimpleTreeReusingLayers)
{
    ...
    EXPECT_TRUE(base::Contains(host_->pending_tree()->LayersThatShouldPushProperties(), layer_impl_tree_root));

    ExpectTreesAreIdentical(layer_tree_root.get(), layer_impl_tree_root, host_->pending_tree());

    // We have to push properties to pick up the destruction list pointer.
    TreeSynchronizer::PushLayerProperties(*host_->GetPendingCommitState(), host_->GetThreadUnsafeCommitState(), host_->pending_tree());

    // Add a new layer to the Layer side
    layer_tree_root->children()[0]->AddChild(MockLayer::Create(&layer_impl_destruction_list));
    // Remove one.
...
TEST_F(TreeSynchronizerTest, SyncSimpleTreeAndTrackStackingOrderChange)
{
    ...
    host_->BuildPropertyTreesForTesting();
    TreeSynchronizer::SynchronizeTrees(*const_host()->pending_commit_state(), host_->GetThreadUnsafeCommitState(), host_->active_tree());
    LayerImpl* layer_impl_tree_root = host_->active_tree()->root_layer();
    ExpectTreesAreIdentical(layer_tree_root.get(), layer_impl_tree_root, host_->active_tree());

    // We have to push properties to pick up the destruction list pointer.
    TreeSynchronizer::PushLayerProperties(*host_->GetPendingCommitState(), host_->GetThreadUnsafeCommitState(), host_->active_tree());
    ...
    host_->active_tree()->SetPropertyTrees(*layer_tree_root->layer_tree_host()->property_trees());
    TreeSynchronizer::PushLayerProperties(*host_->GetPendingCommitState(), host_->GetThreadUnsafeCommitState(), host_->active_tree());

    // Check that the impl thread properly tracked the change.
    EXPECT_FALSE(layer_impl_tree_root->LayerPropertyChanged());
    EXPECT_FALSE(host_->active_tree()->LayerById(child1_id)->LayerPropertyChanged());
    EXPECT_TRUE(host_->active_tree()->LayerById(child2_id)->LayerPropertyChanged());
    EXPECT_FALSE(host_->active_tree()->LayerById(child2_id)->LayerPropertyChangedFromPropertyTrees());
    EXPECT_TRUE(host_->active_tree()->LayerById(child2_id)->LayerPropertyChangedNotFromPropertyTrees());
...
TEST_F(TreeSynchronizerTest, SyncSimpleTreeAndProperties)
{
    ...
    gfx::Size second_layer_impl_child_bounds = layer_impl_tree_root->layer_tree_impl()->LayerById(second_child_id)->bounds();
    EXPECT_EQ(second_child_bounds.width(), second_layer_impl_child_bounds.width());
    EXPECT_EQ(second_child_bounds.height(), second_layer_impl_child_bounds.height());
}
...
TEST_F(TreeSynchronizerTest, SyncSimpleTreeThenDestroy)
{
    ...
    old_layer_tree_root->AddChild(MockLayer::Create(&layer_impl_destruction_list));
    old_layer_tree_root->AddChild(MockLayer::Create(&layer_impl_destruction_list));
    ...
    TreeSynchronizer::SynchronizeTrees(*const_host()->pending_commit_state(), host_->GetThreadUnsafeCommitState(), host_->active_tree());
    LayerImpl* layer_impl_tree_root = host_->active_tree()->root_layer();
    ExpectTreesAreIdentical(old_layer_tree_root.get(), layer_impl_tree_root, host_->active_tree());
    ...
    TreeSynchronizer::SynchronizeTrees(*const_host()->pending_commit_state(), host_->GetThreadUnsafeCommitState(), host_->active_tree());
    layer_impl_tree_root = host_->active_tree()->root_layer();
    ExpectTreesAreIdentical(new_layer_tree_root.get(), layer_impl_tree_root, host_->active_tree());

    ASSERT_EQ(3u, layer_impl_destruction_list.size());

    EXPECT_TRUE(base::Contains(layer_impl_destruction_list, old_tree_root_layer_id));
    EXPECT_TRUE(base::Contains(layer_impl_destruction_list, old_tree_first_child_layer_id));
...
TEST_F(TreeSynchronizerTest, SynchronizeCurrentlyScrollingNode)
{
    ...
    ScrollNode* scroll_node = host_impl->active_tree()->property_trees()->scroll_tree_mutable().Node(scroll_layer->scroll_tree_index());
    ...
    EXPECT_EQ(scroll_layer->scroll_tree_index(), host_impl->active_tree()->CurrentlyScrollingNode()->id);
}
...
TEST_F(TreeSynchronizerTest, SynchronizeScrollTreeScrollOffsetMap)
{
    ...
    EXPECT_TRUE(AreScrollOffsetsEqual(
        scroll_layer_offset.get(), host_impl->active_tree()->property_trees()->scroll_tree().GetSyncedScrollOffset(scroll_layer->element_id())));

    scoped_refptr<SyncedScrollOffset> transient_scroll_layer_offset = new SyncedScrollOffset;
    transient_scroll_layer_offset->PushMainToPending(transient_scroll_layer->scroll_offset());
    transient_scroll_layer_offset->PushPendingToActive();
    EXPECT_TRUE(AreScrollOffsetsEqual(transient_scroll_layer_offset.get(),
        host_impl->active_tree()->property_trees()->scroll_tree().GetSyncedScrollOffset(transient_scroll_layer->element_id())));
    ...
    ScrollTree& scroll_tree = host_impl->active_tree()->property_trees()->scroll_tree_mutable();
    scroll_tree.SetScrollOffset(scroll_layer_impl->element_id(), gfx::PointF(20, 30));
    ...
    scroll_tree.CollectScrollDeltas(commit_data.get(), ElementId(), settings.commit_fractional_scroll_deltas,
    ...
    scroll_tree.SetScrollOffset(scroll_layer_impl->element_id(), gfx::PointF(40, 50));
    host_impl->active_tree()->SetCurrentlyScrollingNode(scroll_tree.Node(scroll_layer_impl->scroll_tree_index()));
    ...
    EXPECT_TRUE(AreScrollOffsetsEqual(
        scroll_layer_offset.get(), host_impl->active_tree()->property_trees()->scroll_tree().GetSyncedScrollOffset(scroll_layer->element_id())));
    EXPECT_EQ(nullptr, host_impl->active_tree()->property_trees()->scroll_tree().GetSyncedScrollOffset(transient_scroll_layer->element_id()));
}
...
TEST_F(TreeSynchronizerTest, RefreshPropertyTreesCachedData)
{
    ...
    host_impl->active_tree()->property_trees()->SetMaximumAnimationToScreenScaleForTesting(
        transform_layer->transform_tree_index(), maximum_animation_scale, animation_affected_by_invalid_scale);
    EXPECT_EQ(maximum_animation_scale, host_impl->active_tree()->property_trees()->MaximumAnimationToScreenScale(transform_layer->transform_tree_index()));
    EXPECT_TRUE(host_impl->active_tree()->property_trees()->AnimationAffectedByInvalidScale(transform_layer->transform_tree_index()));

    host_impl->CreatePendingTree();
    host_->CommitToPendingTree();
    host_impl->ActivateSyncTree();
    EXPECT_EQ(2.0f, host_impl->active_tree()->property_trees()->MaximumAnimationToScreenScale(transform_layer->transform_tree_index()));
    EXPECT_FALSE(host_impl->active_tree()->property_trees()->AnimationAffectedByInvalidScale(transform_layer->transform_tree_index()));
}
...
TEST_F(TreeSynchronizerTest, RoundedScrollDeltasOnCommit)
{
    ...
    std::unique_ptr<CompositorCommitData> commit_data = host_impl->ProcessCompositorDeltas(
        /* main_thread_mutator_host */ nullptr);
