...
void PostConcatSurfaceContentsScale(const EffectNode* effect_node, gfx::Transform* transform)
{
    if (!effect_node) {
    ...
    transform->PostScale(effect_node->surface_contents_scale.x(), effect_node->surface_contents_scale.y());
}

bool ConvertRectBetweenSurfaceSpaces(
    const PropertyTrees* property_trees, int source_effect_id, int dest_effect_id, gfx::RectF clip_in_source_space, gfx::RectF* clip_in_dest_space)
{
    const EffectNode* source_effect_node = property_trees->effect_tree().Node(source_effect_id);
    int source_transform_id = source_effect_node->transform_id;
    const EffectNode* dest_effect_node = property_trees->effect_tree().Node(dest_effect_id);
    int dest_transform_id = dest_effect_node->transform_id;
    gfx::Transform source_to_dest;
    if (source_transform_id > dest_transform_id) {
        if (property_trees->GetToTarget(source_transform_id, dest_effect_id, &source_to_dest)) {
            ConcatInverseSurfaceContentsScale(source_effect_node, &source_to_dest);
            *clip_in_dest_space = MathUtil::MapClippedRect(source_to_dest, clip_in_source_space);
        } else {
            return false;
        }
    } else {
        if (property_trees->GetFromTarget(dest_transform_id, source_effect_id, &source_to_dest)) {
            PostConcatSurfaceContentsScale(dest_effect_node, &source_to_dest);
            *clip_in_dest_space = MathUtil::ProjectClippedRect(source_to_dest, clip_in_source_space);
        } else {
            return false;
        }
    }
...
ConditionalClip ComputeTargetRectInLocalSpace(
    gfx::RectF rect, const PropertyTrees* property_trees, int target_transform_id, int local_transform_id, const int target_effect_id)
{
    gfx::Transform target_to_local;
    bool success = property_trees->GetFromTarget(local_transform_id, target_effect_id, &target_to_local);
...
ConditionalClip ComputeLocalRectInTargetSpace(
    gfx::RectF rect, const PropertyTrees* property_trees, int current_transform_id, int target_transform_id, int target_effect_id)
{
    gfx::Transform current_to_target;
    if (!property_trees->GetToTarget(current_transform_id, target_effect_id, &current_to_target)) {
        // If transform is not invertible, cannot apply clip.
        return ConditionalClip { false, gfx::RectF() };
    }
...
ConditionalClip ComputeCurrentClip(const ClipNode* clip_node, const PropertyTrees* property_trees, int target_transform_id, int target_effect_id)
{
    if (clip_node->transform_id != target_transform_id) {
        return ComputeLocalRectInTargetSpace(clip_node->clip, property_trees, clip_node->transform_id, target_transform_id, target_effect_id);
    }
    ...
    if (surface_contents_scale.x() > 0 && surface_contents_scale.y() > 0 && clip_node->transform_id != kRootPropertyNodeId)
        current_clip.Scale(surface_contents_scale.x(), surface_contents_scale.y());
    return ConditionalClip { true /* is_clipped */, current_clip };
}

bool ExpandClipForPixelMovingFilter(const PropertyTrees* property_trees, int target_id, const EffectNode* filter_node, gfx::RectF* clip_rect)
{
    // Bring the accumulated clip to the space of the pixel-moving filter.
    gfx::RectF clip_rect_in_mapping_space;
    bool success = ConvertRectBetweenSurfaceSpaces(property_trees, target_id, filter_node->id, *clip_rect, &clip_rect_in_mapping_space);
    // If transform is not invertible, no clip will be applied.
    if (!success)
        return false;

    // Do the expansion.
    SkMatrix filter_draw_matrix = SkMatrix::Scale(filter_node->surface_contents_scale.x(), filter_node->surface_contents_scale.y());
    gfx::RectF mapped_clip_in_mapping_space(filter_node->filters.MapRect(ToEnclosingClipRect(clip_rect_in_mapping_space), filter_draw_matrix));

    // Put the expanded clip back into the original target space.
    gfx::RectF original_clip_rect = *clip_rect;
    success = ConvertRectBetweenSurfaceSpaces(property_trees, filter_node->id, target_id, mapped_clip_in_mapping_space, clip_rect);
    // If transform is not invertible, no clip will be applied.
    if (!success)
        return false;
...
bool ApplyClipNodeToAccumulatedClip(const PropertyTrees* property_trees, bool include_expanding_clips, int target_id, int target_transform_id,
    const ClipNode* clip_node, gfx::RectF* accumulated_clip)
{
    if (!clip_node->AppliesLocalClip()) {
        if (!include_expanding_clips)
            return true;
        const EffectNode* filter_node = property_trees->effect_tree().Node(clip_node->pixel_moving_filter_id);
        DCHECK(filter_node);
        return ExpandClipForPixelMovingFilter(property_trees, target_id, filter_node, accumulated_clip);
    }

    ConditionalClip current_clip = ComputeCurrentClip(clip_node, property_trees, target_transform_id, target_id);
...
ConditionalClip ComputeAccumulatedClip(PropertyTrees* property_trees, bool include_expanding_clips, int local_clip_id, int target_id)
{
    ClipRectData* cached_data = property_trees->FetchClipRectFromCache(local_clip_id, target_id);
    ...
    // that doesn't.
    while (
        target_node->clip_id > clip_node->id || property_trees->effect_tree().GetRenderSurface(target_node->id)->has_contributing_layer_that_escapes_clip()) {
    ...
    if (cache_hit && cached_clip.is_clipped) {
        ...
        bool success = ApplyClipNodeToAccumulatedClip(property_trees, include_expanding_clips, target_id, target_transform_id, clip_node, &accumulated_clip);
        if (!success) {
            // Singular transform
            cached_data->clip = unclipped;
            return unclipped;
    ...
    ConditionalClip clip = ConditionalClip { true /* is_clipped */, accumulated_clip.IsEmpty() ? gfx::RectF() : accumulated_clip };
...
int LowestCommonAncestor(int clip_id_1, int clip_id_2, const ClipTree* clip_tree)
{
...
void SetHasContributingLayerThatEscapesClip(int lca_clip_id, int target_effect_id, EffectTree* effect_tree)
{
    DCHECK(!base::FeatureList::IsEnabled(features::kRenderSurfaceCommonAncestorClip));
    ...
    while (effect_node->clip_id > lca_clip_id) {
        RenderSurfaceImpl* render_surface = effect_tree->GetRenderSurface(effect_node->id);
...
void UpdateRenderSurfaceCommonAncestorClip(LayerImpl* layer, const ClipTree* clip_tree, EffectTree* effect_tree)
{
    DCHECK(base::FeatureList::IsEnabled(features::kRenderSurfaceCommonAncestorClip));
    ...
    while (clip_id < render_surface->common_ancestor_clip_id()) {
        clip_id = LowestCommonAncestor(clip_id, render_surface->common_ancestor_clip_id(), clip_tree);
...
template <typename LayerType> const TransformNode& TransformNodeForBackfaceVisibility(LayerType* layer, const TransformTree& tree)
{
    ...
    while (node->delegates_to_parent_for_backface) {
        ...
        DCHECK(!node->sorting_context_id || parent->sorting_context_id == node->sorting_context_id);
        node = parent;
    }
    return *node;
}

bool IsTargetSpaceTransformBackFaceVisible(const LayerImpl* layer, const PropertyTrees* property_trees)
{
    const TransformNode& transform_node = TransformNodeForBackfaceVisibility(layer, property_trees->transform_tree());
    gfx::Transform to_target;
    property_trees->GetToTarget(transform_node.id, layer->render_target_effect_tree_index(), &to_target);

    return to_target.IsBackFaceVisible();
}

bool IsTransformToRootOf3DRenderingContextBackFaceVisible(const LayerImpl* layer, const PropertyTrees* property_trees)
{
    const TransformTree& transform_tree = property_trees->transform_tree();

    const TransformNode& transform_node = TransformNodeForBackfaceVisibility(layer, transform_tree);
    ...
    if (transform_node.id != root_id) {
        property_trees->transform_tree().CombineTransformsBetween(transform_node.id, root_id, &to_3d_root);
    }
    to_3d_root.PreConcat(root_node->to_parent);
    return to_3d_root.IsBackFaceVisible();
}

bool IsLayerBackFaceVisible(const Layer* layer, const PropertyTrees* property_trees)
{
...
bool IsLayerBackFaceVisible(const LayerImpl* layer, const PropertyTrees* property_trees)
{
    // A layer with singular transform is not drawn. So, we can assume that its
    // backface is not visible.
    if (HasSingularTransform(layer->transform_tree_index(), property_trees->transform_tree())) {
        return false;
    }
    if (layer->layer_tree_impl()->settings().enable_backface_visibility_interop) {
        return IsTransformToRootOf3DRenderingContextBackFaceVisible(layer, property_trees);
    } else {
        return IsTargetSpaceTransformBackFaceVisible(layer, property_trees);
    }
}

template <typename LayerType> bool LayerNeedsUpdate(LayerType* layer, bool layer_is_drawn, const PropertyTrees* property_trees)
{
    ...
    if (layer->should_check_backface_visibility() && IsLayerBackFaceVisible(layer, property_trees)) {
        return false;
    }
...
inline bool LayerShouldBeSkippedForDrawPropertiesComputation(Layer* layer, const TransformTree& transform_tree, const EffectTree& effect_tree)
{
    ...
    return !transform_node->node_and_ancestors_are_animated_or_invertible || !effect_node->is_drawn;
}

gfx::Rect LayerDrawableContentRect(const LayerImpl* layer, const gfx::Rect& layer_bounds_in_target_space, const gfx::Rect& clip_rect)
{
    if (layer->is_clipped())
...
void SetSurfaceIsClipped(const ClipTree& clip_tree, RenderSurfaceImpl* render_surface)
{
    bool is_clipped = false;
    if (render_surface->EffectTreeIndex() == kContentsRootPropertyNodeId) {
        // Root render surface is always clipped.
        is_clipped = true;
    } else if (base::FeatureList::IsEnabled(features::kRenderSurfaceCommonAncestorClip)) {
        int parent_target_clip_id = render_surface->render_target()->common_ancestor_clip_id();
        for (const ClipNode* clip_node = clip_tree.Node(render_surface->common_ancestor_clip_id()); clip_node && clip_node->id != parent_target_clip_id;
             clip_node = clip_tree.parent(clip_node)) {
            if (clip_node->AppliesLocalClip()) {
                is_clipped = true;
                break;
        ...
        CHECK_EQ(render_surface->common_ancestor_clip_id(), render_surface->ClipTreeIndex());
        // We cannot clip a surface that has a contribuitng layer which escapes the
        // clip.
        is_clipped = false;
    } else if (render_surface->ClipTreeIndex() == render_surface->render_target()->ClipTreeIndex()) {
...
void SetSurfaceDrawOpacity(const EffectTree& tree, RenderSurfaceImpl* render_surface)
{
    ...
    for (node = tree.parent(node); node && !node->HasRenderSurface(); node = tree.parent(node)) {
...
template <typename LayerType> gfx::Transform ScreenSpaceTransformInternal(LayerType* layer, const TransformTree& tree)
{
    gfx::Transform xform = gfx::Transform::MakeTranslation(layer->offset_to_transform_parent().x(), layer->offset_to_transform_parent().y());
    gfx::Transform ssxform = tree.ToScreen(layer->transform_tree_index());
    xform.PostConcat(ssxform);
    return xform;
}

void SetSurfaceClipRect(const ClipNode* parent_clip_node, PropertyTrees* property_trees, RenderSurfaceImpl* render_surface)
{
    ...
    if (render_surface->EffectTreeIndex() == kContentsRootPropertyNodeId) {
        render_surface->SetClipRect(ToEnclosingClipRect(clip_tree.Node(effect_node->clip_id)->clip));
    } else {
        DCHECK(base::FeatureList::IsEnabled(features::kRenderSurfaceCommonAncestorClip) || render_surface->common_ancestor_clip_id() == effect_node->clip_id);
        ConditionalClip accumulated_clip_rect
            = ComputeAccumulatedClip(property_trees, include_expanding_clips, render_surface->common_ancestor_clip_id(), target_node->id);
        render_surface->SetClipRect(ToEnclosingClipRect(accumulated_clip_rect.clip_rect));
    }
}

void SetSurfaceDrawTransform(const PropertyTrees* property_trees, RenderSurfaceImpl* render_surface)
{
    const TransformTree& transform_tree = property_trees->transform_tree();
    const EffectTree& effect_tree = property_trees->effect_tree();
    ...
    property_trees->GetToTarget(transform_node->id, target_effect_node->id, &render_surface_transform);
...
gfx::Rect LayerVisibleRect(PropertyTrees* property_trees, LayerImpl* layer)
{
    ...
    int lower_effect_closest_ancestor = effect_node->closest_ancestor_with_cached_render_surface_id;
    lower_effect_closest_ancestor = std::max(lower_effect_closest_ancestor, effect_node->closest_ancestor_with_copy_request_id);
    lower_effect_closest_ancestor = std::max(lower_effect_closest_ancestor, effect_node->closest_ancestor_being_captured_id);
    lower_effect_closest_ancestor = std::max(lower_effect_closest_ancestor, effect_node->closest_ancestor_with_shared_element_id);
    const bool non_root_with_render_surface = lower_effect_closest_ancestor > kContentsRootPropertyNodeId;
    ...
    if (non_root_with_render_surface) {
        ...
        ConditionalClip accumulated_clip
            = ComputeAccumulatedClip(property_trees, include_expanding_clips, layer->clip_tree_index(), lower_effect_closest_ancestor);
        ...
        accumulated_clip_in_root_space = clip_node->cached_accumulated_rect_in_screen_space;
    }

    const EffectNode* root_effect_node = non_root_with_render_surface ? property_trees->effect_tree().Node(lower_effect_closest_ancestor)
    ...
    ConditionalClip accumulated_clip_in_layer_space = ComputeTargetRectInLocalSpace(
        accumulated_clip_in_root_space, property_trees, root_effect_node->transform_id, layer->transform_tree_index(), root_effect_node->id);
...
ConditionalClip LayerClipRect(PropertyTrees* property_trees, LayerImpl* layer)
{
    const EffectTree* effect_tree = &property_trees->effect_tree();
    const EffectNode* effect_node = effect_tree->Node(layer->effect_tree_index());
    const EffectNode* target_node = effect_node->HasRenderSurface() ? effect_node : effect_tree->Node(effect_node->target_id);
    bool include_expanding_clips = false;
    return ComputeAccumulatedClip(property_trees, include_expanding_clips, layer->clip_tree_index(), target_node->id);
}

std::pair<gfx::MaskFilterInfo, bool> GetMaskFilterInfoPair(const PropertyTrees* property_trees, int effect_tree_index, bool for_render_surface)
{
    static const std::pair<gfx::MaskFilterInfo, bool> kEmptyMaskFilterInfoPair = std::make_pair(gfx::MaskFilterInfo(), false);
    ...
    while (node) {
        ...
        if (node->HasRenderSurface() && (!for_render_surface || effect_node != node)) {
            break;
        }
...
void UpdateRenderTarget(LayerTreeImpl* layer_tree_impl, EffectTree* effect_tree)
{
    int last_backdrop_filter = kInvalidNodeId;
    base::flat_map<viz::ViewTransitionElementResourceId, int> resource_to_node;

    for (int i = kContentsRootPropertyNodeId; i < static_cast<int>(effect_tree->size()); ++i) {
        EffectNode* node = effect_tree->Node(i);

        if (node->view_transition_element_resource_id.IsValid()) {
            CHECK(!resource_to_node.contains(node->view_transition_element_resource_id));
        ...
        if (!node->backdrop_filters.IsEmpty() || node->has_potential_backdrop_filter_animation)
    ...
    for (int i = last_backdrop_filter - 1; kContentsRootPropertyNodeId <= i; --i) {
        EffectNode* node = effect_tree->Node(i);
        ...
        if ((!node->backdrop_filters.IsEmpty() || node->has_potential_backdrop_filter_animation) && current_target_id == kInvalidNodeId)
...
void ComputeClips(PropertyTrees* property_trees)
{
    ...
    for (int i = kViewportPropertyNodeId; i < static_cast<int>(clip_tree->size()); ++i) {
        ...
        gfx::RectF accumulated_clip = parent_clip_node->cached_accumulated_rect_in_screen_space;
        bool success
            = ApplyClipNodeToAccumulatedClip(property_trees, include_expanding_clips, target_effect_id, target_transform_id, clip_node, &accumulated_clip);
        if (success)
...
void ComputeSurfaceDrawProperties(PropertyTrees* property_trees, RenderSurfaceImpl* render_surface)
{
    ...
    auto mask_filter_info_pair = GetMaskFilterInfoPair(property_trees, render_surface->EffectTreeIndex(),
    ...
    render_surface->SetScreenSpaceTransform(
        property_trees->ToScreenSpaceTransformWithoutSurfaceContentsScale(render_surface->TransformTreeIndex(), render_surface->EffectTreeIndex()));
...
void AddSurfaceToRenderSurfaceList(RenderSurfaceImpl* render_surface, RenderSurfaceList* render_surface_list, PropertyTrees* property_trees)
{
    ...
    bool is_root = render_surface->EffectTreeIndex() == kContentsRootPropertyNodeId;
    ...
    if (render_surface->OwningEffectNode()->view_transition_target_id != kInvalidPropertyNodeId) {
        CHECK(!is_root);
        CHECK(render_surface->OwningEffectNode()->view_transition_element_resource_id.IsValid());

        auto* vt_target = property_trees->effect_tree_mutable().GetRenderSurface(render_surface->OwningEffectNode()->view_transition_target_id);
        CHECK(vt_target);

        if (!vt_target->is_render_surface_list_member()) {
            AddSurfaceToRenderSurfaceList(vt_target, render_surface_list, property_trees);
        }
    }
    ...
    if (is_root) {
        ...
        bool contributes_to_drawn_surface = property_trees->effect_tree().ContributesToDrawnSurface(render_surface->EffectTreeIndex());
        render_surface->set_contributes_to_drawn_surface(contributes_to_drawn_surface);
    }
    ...
    const bool allow_skipping_render_pass = base::FeatureList::IsEnabled(features::kAllowUndamagedNonrootRenderPassToSkip);
    const FilterOperations& filters = render_surface->Filters();
    bool is_occlusion_immune
        = render_surface->CopyOfOutputRequired() || filters.HasReferenceFilter() || filters.HasFilterThatMovesPixels() || allow_skipping_render_pass;
    ...
    if (is_occlusion_immune) {
        ...
        render_surface->SetNearestOcclusionImmuneAncestor(render_surface->render_target()->nearest_occlusion_immune_ancestor());
    }
}

bool SkipForInvertibility(const LayerImpl* layer, PropertyTrees* property_trees)
{
    const TransformNode* transform_node = property_trees->transform_tree().Node(layer->transform_tree_index());
    const EffectNode* effect_node = property_trees->effect_tree().Node(layer->effect_tree_index());
    bool non_root_copy_request = effect_node->closest_ancestor_with_copy_request_id > kContentsRootPropertyNodeId;
    ...
    return non_root_copy_request
        ? !property_trees->GetFromTarget(layer->transform_tree_index(), effect_node->closest_ancestor_with_copy_request_id, &from_target)
        : !transform_node->ancestors_are_invertible;
}

void ComputeInitialRenderSurfaceList(LayerTreeImpl* layer_tree_impl, PropertyTrees* property_trees, RenderSurfaceList* render_surface_list,
    std::map<viz::ViewTransitionElementResourceId, ViewTransitionContentLayerImpl*>& view_transition_content_layers)
{
    ...
    RenderSurfaceImpl* root_surface = effect_tree.GetRenderSurface(kContentsRootPropertyNodeId);
    // The root surface always gets added to the render surface  list.
    AddSurfaceToRenderSurfaceList(root_surface, render_surface_list, property_trees);
    ...
    for (LayerImpl* layer : *layer_tree_impl) {
        ...
        bool skip_draw_properties_computation = draw_property_utils::LayerShouldBeSkippedForDrawPropertiesComputation(layer, property_trees);

        bool skip_for_invertibility = SkipForInvertibility(layer, property_trees);

        bool skip_layer = !is_root && (skip_draw_properties_computation || skip_for_invertibility);

        TransformNode* transform_node = property_trees->transform_tree_mutable().Node(layer->transform_tree_index());
        const bool has_will_change_transform_hint = transform_node && transform_node->will_change_transform;
        ...
        layer->set_raster_even_if_not_drawn((skip_for_invertibility && !skip_draw_properties_computation) || has_will_change_transform_hint);
        if (skip_layer)
            continue;

        bool layer_is_drawn = property_trees->effect_tree().Node(layer->effect_tree_index())->is_drawn;
        bool layer_should_be_drawn = LayerNeedsUpdate(layer, layer_is_drawn, property_trees);
        ...
        if (!render_target->is_render_surface_list_member()) {
            AddSurfaceToRenderSurfaceList(render_target, render_surface_list, property_trees);
        }
        ...
        auto* view_transition_content_layer = static_cast<ViewTransitionContentLayerImpl*>(layer);
        view_transition_content_layer->SetOriginatingSurfaceContentRect(gfx::Rect());
        view_transition_content_layers[layer->ViewTransitionResourceId()] = view_transition_content_layer;
    }
}

void ComputeSurfaceContentRects(PropertyTrees* property_trees, RenderSurfaceList* render_surface_list, int max_texture_size,
    const std::map<viz::ViewTransitionElementResourceId, ViewTransitionContentLayerImpl*> view_transition_content_layers,
    std::vector<std::pair<viz::ViewTransitionElementResourceId, gfx::RectF>>& view_transition_content_rects)
{
    // Walk the list backwards, accumulating each surface's content rect into its
    // target's content rect.
    for (RenderSurfaceImpl* render_surface : base::Reversed(*render_surface_list)) {
        if (render_surface->EffectTreeIndex() == kContentsRootPropertyNodeId) {
        ...
        render_surface->CalculateContentRectFromAccumulatedContentRect(max_texture_size);
        ...
        render_target->AccumulateContentRectFromContributingRenderSurface(render_surface);
        render_target->increment_num_contributors();
        ...
        const auto& view_transition_id = render_surface->OwningEffectNode()->view_transition_element_resource_id;

        if (!view_transition_id.IsValid()) {
            continue;
        }

        auto unscaled_content_rect = render_surface->SurfaceScale().InverseOrIdentity().MapRect(render_surface->content_rect());

        view_transition_content_rects.emplace_back(view_transition_id, gfx::RectF(unscaled_content_rect));

        if (view_transition_content_layers.contains(view_transition_id)) {
            view_transition_content_layers.at(view_transition_id)->SetOriginatingSurfaceContentRect(unscaled_content_rect);
        }
    }
}

void ComputeListOfNonEmptySurfaces(
    LayerTreeImpl* layer_tree_impl, PropertyTrees* property_trees, RenderSurfaceList* initial_surface_list, RenderSurfaceList* final_surface_list)
{
    ...
    for (RenderSurfaceImpl* surface : *initial_surface_list) {
        ...
        if (!is_root && (surface->content_rect().IsEmpty() || (!target_surface->is_render_surface_list_member() && !surface->CopyOfOutputRequired()))) {
...
void CalculateRenderSurfaceLayerList(
    LayerTreeImpl* layer_tree_impl, PropertyTrees* property_trees, RenderSurfaceList* render_surface_list, const int max_texture_size)
{
    ...
    ComputeInitialRenderSurfaceList(layer_tree_impl, property_trees, &initial_render_surface_list, view_transition_content_layers);
    ComputeSurfaceContentRects(property_trees, &initial_render_surface_list, max_texture_size, view_transition_content_layers, view_transition_content_rects);
    ComputeListOfNonEmptySurfaces(layer_tree_impl, property_trees, &initial_render_surface_list, render_surface_list);
...
void RecordRenderSurfaceReasonsForTracing(const PropertyTrees* property_trees, const RenderSurfaceList* render_surface_list)
{
//     static const auto* tracing_enabled = TRACE_EVENT_API_GET_CATEGORY_GROUP_ENABLED("cc");
//     if (!*tracing_enabled ||
//         // Don't output single root render surface.
//         render_surface_list->size() <= 1)
//         return;
// 
//     TRACE_EVENT_INSTANT1("cc", "RenderSurfaceReasonCount", TRACE_EVENT_SCOPE_THREAD, "total", render_surface_list->size());
// 
...
//         const auto* effect_node = property_trees->effect_tree().Node(render_surface->EffectTreeIndex());
...
//             "cc", "RenderSurfaceReasonCount", TRACE_EVENT_SCOPE_THREAD, RenderSurfaceReasonToString(static_cast<RenderSurfaceReason>(i)), reason_counts[i]);
//     }
}

void UpdateElasticOverscroll(PropertyTrees* property_trees, TransformNode* overscroll_elasticity_transform_node, const gfx::Vector2dF& elastic_overscroll,
    const ScrollNode* inner_viewport)
{
    ...
    if (inner_viewport && property_trees->scroll_tree().container_bounds(inner_viewport->id).IsEmpty()) {
    ...
    overscroll_elasticity_transform_node->has_potential_animation = !elastic_overscroll.IsZero();
    ...
    if (!elastic_overscroll.IsZero() && inner_viewport) {
        ...
        gfx::Size scroller_size = property_trees->scroll_tree().container_bounds(inner_viewport->id);

        overscroll_elasticity_transform_node->local.Scale(
            1.f + std::abs(elastic_overscroll.x()) / scroller_size.width(), 1.f + std::abs(elastic_overscroll.y()) / scroller_size.height());
        ...
        if (elastic_overscroll.y() > 0.f) {
            overscroll_elasticity_transform_node->origin.set_y(scroller_size.height());
        }
    }
    ...
    // overscroll amount.
    gfx::PointF overscroll_offset = gfx::PointAtOffsetFromOrigin(elastic_overscroll);
...
void ComputeDrawPropertiesOfVisibleLayers(const LayerImplList* layer_list, PropertyTrees* property_trees)
{
    const bool common_ancestor_clip_enabled = base::FeatureList::IsEnabled(features::kRenderSurfaceCommonAncestorClip);
    ...
    for (LayerImpl* layer : *layer_list) {
        ...
        layer->draw_properties().target_space_transform = DrawTransform(layer, property_trees->transform_tree(), property_trees->effect_tree());
        layer->draw_properties().screen_space_transform_is_animating = transform_node->to_screen_is_potentially_animated;
        auto mask_filter_info_pair = GetMaskFilterInfoPair(property_trees, layer->effect_tree_index(),
            /*from_render_surface=*/false);
        layer->draw_properties().mask_filter_info = mask_filter_info_pair.first;
        layer->draw_properties().is_fast_rounded_corner = mask_filter_info_pair.second;
    ...
    for (LayerImpl* layer : *layer_list) {
        ...
        if (common_ancestor_clip_enabled) {
            UpdateRenderSurfaceCommonAncestorClip(layer, &property_trees->clip_tree(), &property_trees->effect_tree_mutable());
        } else {
            RenderSurfaceImpl* render_target = layer->render_target();
            int lca_clip_id = LowestCommonAncestor(layer->clip_tree_index(), render_target->ClipTreeIndex(), &property_trees->clip_tree());
            if (lca_clip_id != render_target->ClipTreeIndex()) {
                SetHasContributingLayerThatEscapesClip(lca_clip_id, render_target->EffectTreeIndex(), &property_trees->effect_tree_mutable());
            }
        }
    }
    ...
    for (LayerImpl* layer : *layer_list) {
        ...
        layer->draw_properties().visible_layer_rect = LayerVisibleRect(property_trees, layer);
    }

    // Compute drawable content rects
    for (LayerImpl* layer : *layer_list) {
        bool only_draws_visible_content = property_trees->effect_tree().Node(layer->effect_tree_index())->only_draws_visible_content;
        ...
        gfx::Rect visible_bounds_in_target_space = MathUtil::MapEnclosingClippedRect(layer->draw_properties().target_space_transform, drawable_bounds);
        layer->draw_properties().visible_drawable_content_rect
            = LayerDrawableContentRect(layer, visible_bounds_in_target_space, layer->draw_properties().clip_rect);
    }
}
...
bool LayerShouldBeSkippedForDrawPropertiesComputation(LayerImpl* layer, const PropertyTrees* property_trees)
{
    const TransformTree& transform_tree = property_trees->transform_tree();
    const EffectTree& effect_tree = property_trees->effect_tree();
    const EffectNode* effect_node = effect_tree.Node(layer->effect_tree_index());
    ...
    if (effect_node->subtree_hidden && !effect_node->cache_render_surface && !effect_node->subtree_capture_id.is_valid()) {
        return true;
    }
    ...
    if (!transform_node->node_and_ancestors_are_animated_or_invertible || !effect_node->is_drawn)
        return true;
    if (layer->layer_tree_impl()->settings().enable_backface_visibility_interop) {
        return layer->should_check_backface_visibility() && IsLayerBackFaceVisible(layer, property_trees);
    } else {
...
void ConcatInverseSurfaceContentsScale(const EffectNode* effect_node, gfx::Transform* transform)
{
    DCHECK(effect_node->HasRenderSurface());
    if (effect_node->surface_contents_scale.x() != 0.0 && effect_node->surface_contents_scale.y() != 0.0)
        transform->Scale(1.0 / effect_node->surface_contents_scale.x(), 1.0 / effect_node->surface_contents_scale.y());
}

void FindLayersThatNeedUpdates(LayerTreeHost* layer_tree_host, LayerList* update_layer_list)
{
    const PropertyTrees* property_trees = layer_tree_host->property_trees();
    const TransformTree& transform_tree = property_trees->transform_tree();
    const EffectTree& effect_tree = property_trees->effect_tree();
    for (auto* layer : *layer_tree_host) {
        if (!IsRootLayer(layer) && LayerShouldBeSkippedForDrawPropertiesComputation(layer, transform_tree, effect_tree))
            continue;
...
void FindLayersThatNeedUpdates(LayerTreeImpl* layer_tree_impl, std::vector<LayerImpl*>* visible_layer_list)
{
    ...
    for (auto* layer_impl : *layer_tree_impl) {
        ...
        if (!IsRootLayer(layer_impl) && LayerShouldBeSkippedForDrawPropertiesComputation(layer_impl, property_trees))
            continue;
...
void ComputeTransforms(TransformTree* transform_tree, const ViewportPropertyIds& viewport_property_ids)
{
...
void UpdatePropertyTrees(LayerTreeHost* layer_tree_host)
{
    ...
    ComputeTransforms(&property_trees->transform_tree_mutable(), layer_tree_host->viewport_property_ids());
...
void UpdatePropertyTreesAndRenderSurfaces(LayerTreeImpl* layer_tree_impl, PropertyTrees* property_trees)
{
    if (property_trees->transform_tree().needs_update()) {
        property_trees->clip_tree_mutable().set_needs_update(true);
        property_trees->effect_tree_mutable().set_needs_update(true);
    ...
    ComputeTransforms(&property_trees->transform_tree_mutable(), layer_tree_impl->viewport_property_ids());
    ComputeEffects(&property_trees->effect_tree_mutable());
    // Computation of clips uses ToScreen which is updated while computing
    // transforms. So, ComputeTransforms should be before ComputeClips.
    ComputeClips(property_trees);
...
gfx::Transform DrawTransform(const LayerImpl* layer, const TransformTree& transform_tree, const EffectTree& effect_tree)
{
    ...
    // scale.
    gfx::Transform xform;
    transform_tree.property_trees()->GetToTarget(layer->transform_tree_index(), layer->render_target_effect_tree_index(), &xform);
    xform.Translate(layer->offset_to_transform_parent().x(), layer->offset_to_transform_parent().y());
    return xform;
}

gfx::Transform ScreenSpaceTransform(const Layer* layer, const TransformTree& tree)
{
    return ScreenSpaceTransformInternal(layer, tree);
}

gfx::Transform ScreenSpaceTransform(const LayerImpl* layer, const TransformTree& tree)
{
    return ScreenSpaceTransformInternal(layer, tree);
}

void UpdatePageScaleFactor(PropertyTrees* property_trees, TransformNode* page_scale_node, float page_scale_factor)
{
    // TODO(wjmaclean): Once Issue #845097 is resolved, we can change the nullptr
    // check below to a DCHECK.
    if (property_trees->transform_tree().page_scale_factor() == page_scale_factor || !page_scale_node) {
        return;
    }

    property_trees->transform_tree_mutable().set_page_scale_factor(page_scale_factor);
...
void CalculateDrawProperties(LayerTreeImpl* layer_tree_impl, RenderSurfaceList* output_render_surface_list, LayerImplList* output_update_layer_list_for_testing)
{
    ...
    UpdatePageScaleFactor(property_trees, layer_tree_impl->PageScaleTransformNode(), layer_tree_impl->current_page_scale_factor());
    UpdateElasticOverscroll(property_trees, layer_tree_impl->OverscrollElasticityTransformNode(), layer_tree_impl->current_elastic_overscroll(),
        layer_tree_impl->InnerViewportScrollNode());
    // Similarly, the device viewport and device transform are shared
    // by both trees.
    property_trees->clip_tree_mutable().SetViewportClip(gfx::RectF(layer_tree_impl->GetDeviceViewport()));
    property_trees->transform_tree_mutable().SetRootScaleAndTransform(layer_tree_impl->device_scale_factor(), layer_tree_impl->DrawTransform());
    ...
    {
        TRACE_EVENT1("cc", "draw_property_utils::ComputeDrawPropertiesOfVisibleLayers", "visible_layers", visible_layer_list.size());
        ComputeDrawPropertiesOfVisibleLayers(&visible_layer_list, property_trees);
    }

    {
        TRACE_EVENT0("cc", "CalculateRenderSurfaceLayerList");
        CalculateRenderSurfaceLayerList(layer_tree_impl, property_trees, output_render_surface_list, layer_tree_impl->max_texture_size());
    }

#if DCHECK_IS_ON()
    if (layer_tree_impl->settings().log_on_ui_double_background_blur)
        DCHECK(LogDoubleBackgroundBlur(*layer_tree_impl, *output_render_surface_list));
#endif

    RecordRenderSurfaceReasonsForTracing(property_trees, output_render_surface_list);

    // A root layer render_surface should always exist after
    // CalculateDrawProperties.
    DCHECK(property_trees->effect_tree().GetRenderSurface(kContentsRootPropertyNodeId));
...
#if DCHECK_IS_ON()
bool LogDoubleBackgroundBlur(const LayerTreeImpl& layer_tree_impl, const RenderSurfaceList& render_surface_list)
{
    const PropertyTrees& property_trees = *layer_tree_impl.property_trees();
        ...
        if (NodeMayContainBackdropBlurFilter(*effect_node)) {
            ...
            if (filters.HasFilterOfType(FilterOperation::BLUR)) {
                if (!render_surface->content_rect().IsEmpty()) {
                    const LayerImpl* layer_impl = layer_tree_impl.LayerByElementId(effect_node->element_id);
                    gfx::Rect screen_space_rect = MathUtil::MapEnclosingClippedRect(render_surface->screen_space_transform(), render_surface->content_rect());
                    auto it = base::ranges::find_if(
                        rects, [&screen_space_rect](const std::pair<const LayerImpl*, gfx::Rect>& r) { return r.second.Intersects(screen_space_rect); });
                    if (rects.end() == it) {
                        rects.push_back(std::make_pair(layer_impl, screen_space_rect));
                    } else {
                        LOG(ERROR) << "Double blur detected between layers: " << it->first->DebugName() << " and " << layer_impl->DebugName();
                        return false;
                    }
                }
            }
