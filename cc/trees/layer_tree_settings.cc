...
LayerTreeSettings::LayerTreeSettings()
    ...
    , memory_policy(64 * 1024 * 1024, gpu::MemoryAllocation::CUTOFF_ALLOW_EVERYTHING, ManagedMemoryPolicy::kDefaultNumResourcesLimit)
{
}
...
{
    return use_layer_lists && !is_display_tree && base::FeatureList::IsEnabled(features::kVizLayers);
}
...
SchedulerSettings LayerTreeSettings::ToSchedulerSettings() const
{
    ...
    scheduler_settings.wait_for_all_pipeline_stages_before_draw = wait_for_all_pipeline_stages_before_draw;
    scheduler_settings.disable_frame_rate_limit = disable_frame_rate_limit;
    scheduler_settings.use_layer_context_for_display = UseLayerContextForDisplay();

    if (base::FeatureList::IsEnabled(::features::kDeferImplInvalidation)) {
        scheduler_settings.delay_impl_invalidation_frames = ::features::kDeferImplInvalidationFrames.Get();
    }

    if (!single_thread_proxy_scheduler) {
        const std::string mode_name = ::features::kScrollEventDispatchMode.Get();
        scheduler_settings.scroll_deadline_mode_enabled = base::FeatureList::IsEnabled(::features::kWaitForLateScrollEvents)
            && (mode_name == ::features::kScrollEventDispatchModeDispatchScrollEventsImmediately
                || mode_name == ::features::kScrollEventDispatchModeUseScrollPredictorForDeadline);
        if (scheduler_settings.scroll_deadline_mode_enabled) {
            scheduler_settings.scroll_deadline_ratio = ::features::kWaitForLateScrollEventsDeadlineRatio.Get();
        }
    }
...
TileManagerSettings LayerTreeSettings::ToTileManagerSettings() const
{
    ...
    tile_manager_settings.needs_notify_ready_to_draw = commit_to_active_tree | wait_for_all_pipeline_stages_before_draw;
