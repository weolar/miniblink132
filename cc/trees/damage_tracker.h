...
private:
    using ViewTransitionElementResourceIdToRenderSurfaceMap = base::flat_map<viz::ViewTransitionElementResourceId, RenderSurfaceImpl*>;

    DamageTracker();

    class DamageAccumulator {
    public:
        template <typename Type> void Union(const Type& rect, DamageReasonSet reasons)
        {
            if (rect.IsEmpty())
                return;
        ...
        void UnionReasons(DamageReasonSet reasons)
        {
            reasons_.PutAll(reasons);
        }
        ...
        bool IsEmpty() const
        {
            return x_ == right_ || y_ == bottom_;
        }

        bool GetAsRect(gfx::Rect* rect);

        DamageReasonSet reasons() const
        {
            return reasons_;
        }
    ...
    static void InitializeUpdateDamageTracking(LayerTreeImpl* layer_tree_impl, ViewTransitionElementResourceIdToRenderSurfaceMap& id_to_render_surface_map);
    ...
    void AccumulateDamageFromLayer(LayerImpl* layer, ViewTransitionElementResourceIdToRenderSurfaceMap& id_to_render_surface_map);
    ...
    void ExpandDamageInsideRectWithFilters(const gfx::Rect& pre_filter_rect, const FilterOperations& filters);
    ...
    SurfaceRectMapData& RectDataForSurface(ElementId surface_id, bool* layer_is_new);
    ...
    std::vector<viz::ViewTransitionElementResourceId> previous_view_transition_content_surfaces_by_id_;
    std::vector<viz::ViewTransitionElementResourceId> current_view_transition_content_surfaces_by_id_;
};
