...
class CC_EXPORT RasterContextProviderWrapper : public base::RefCountedThreadSafe<RasterContextProviderWrapper> {
public:
    RasterContextProviderWrapper(scoped_refptr<viz::RasterContextProvider> context, RasterDarkModeFilter* dark_mode_filter, size_t max_working_set_bytes);
    RasterContextProviderWrapper(const RasterContextProviderWrapper&) = delete;
    RasterContextProviderWrapper& operator=(const RasterContextProviderWrapper&) = delete;
    ...
    //GpuImageDecodeCache& GetGpuImageDecodeCache(SkColorType color_type, const RasterCapabilities& raster_caps);
...
private:
    ...
    //base::flat_map<SkColorType, std::unique_ptr<GpuImageDecodeCache>> gpu_image_decode_cache_map_ GUARDED_BY(lock_);
