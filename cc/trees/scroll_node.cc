...
void ScrollNode::AsValueInto(base::trace_event::TracedValue* value) const
{
    ...
    value->SetBoolean("prevent_viewport_scrolling_from_inner", prevent_viewport_scrolling_from_inner);

    element_id.AddToTracedValue(value);
    value->SetInteger("transform_id", transform_id);
    value->SetInteger("overscroll_behavior_x", static_cast<int>(overscroll_behavior.x));
    value->SetInteger("overscroll_behavior_y", static_cast<int>(overscroll_behavior.y));

    if (snap_container_data) {
        value->SetString("snap_container_rect", snap_container_data->rect().ToString());
