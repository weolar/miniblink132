...
void SwapPromiseManager::QueueSwapPromise(std::unique_ptr<SwapPromise> swap_promise)
{
    DCHECK(swap_promise);
    swap_promise_list_.push_back(std::move(swap_promise));
}

void SwapPromiseManager::InsertLatencyInfoSwapPromiseMonitor(LatencyInfoSwapPromiseMonitor* monitor)
{
    latency_info_swap_promise_monitors_.insert(monitor);
}

void SwapPromiseManager::RemoveLatencyInfoSwapPromiseMonitor(LatencyInfoSwapPromiseMonitor* monitor)
{
...
{
    for (LatencyInfoSwapPromiseMonitor* monitor : latency_info_swap_promise_monitors_) {
...
std::vector<std::unique_ptr<SwapPromise>> SwapPromiseManager::TakeSwapPromises()
{
...
void SwapPromiseManager::BreakSwapPromises(SwapPromise::DidNotSwapReason reason)
{
    ...
    for (auto& swap_promise : swap_promise_list_) {
        if (swap_promise->DidNotSwap(reason, timestamp) == SwapPromise::DidNotSwapAction::KEEP_ACTIVE) {
