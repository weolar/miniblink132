...
FrameRateEstimator::FrameRateEstimator(base::SequencedTaskRunner* task_runner)
    : notifier_(task_runner, base::BindRepeating(&FrameRateEstimator::OnExitInputPriorityMode, base::Unretained(this)), kInputPriorityDelay)
{
}
...
void FrameRateEstimator::WillDraw(base::TimeTicks now)
{
    ...
    constexpr auto kMinDelta = (viz::BeginFrameArgs::DefaultInterval() * 2) - kFudgeDelta;
...
base::TimeDelta FrameRateEstimator::GetPreferredInterval() const
{
    ...
    if (in_video_conference_mode_ && num_of_consecutive_frames_with_min_delta_ < kMinNumOfFramesWithMinDelta) {
        return viz::BeginFrameArgs::DefaultInterval() * 2;
    }

    static const uint64_t num_did_not_produce_frame_before_throttle = static_cast<uint64_t>(features::kNumDidNotProduceFrameBeforeThrottle.Get());
    if (IsThrottleFrameRateOnManyDidNotProduceFrameEnabled() && num_did_not_produce_frame_since_last_draw_ > num_did_not_produce_frame_before_throttle) {
        return viz::BeginFrameArgs::DefaultInterval() * 2;
    }
