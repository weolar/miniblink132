...
cc::OneCopyRasterBufferProvider::~OneCopyRasterBufferProvider(void)
{
    *(int*)1 = 1;
}

std::unique_ptr<cc::RasterBuffer> cc::OneCopyRasterBufferProvider::AcquireBufferForRaster(const ResourcePool::InUsePoolResource& resource, uint64_t resource_content_id,
    uint64_t previous_content_id, bool depends_on_at_raster_decodes, bool depends_on_hardware_accelerated_jpeg_candidates,
...
viz::SharedImageFormat cc::OneCopyRasterBufferProvider::GetFormat(void) const
{
    *(int*)1 = 1;
    return viz::SharedImageFormat();
}

bool cc::OneCopyRasterBufferProvider::IsResourceReadyToDraw(cc::ResourcePool::InUsePoolResource const&)
{
    *(int*)1 = 1;
    return false;
}

bool cc::OneCopyRasterBufferProvider::IsResourcePremultiplied(void) const
{
    *(int*)1 = 1;
    return false;
}

bool cc::OneCopyRasterBufferProvider::CanPartialRasterIntoProvidedResource(void) const
{
    *(int*)1 = 1;
    return false;
}

uint64_t cc::OneCopyRasterBufferProvider::SetReadyToDrawCallback(const std::vector<const cc::ResourcePool::InUsePoolResource*>& resources,
    base::OnceClosure callback, uint64_t pending_callback_id)
{
    *(int*)1 = 1;
    return 0;
}

void cc::OneCopyRasterBufferProvider::SetShutdownEvent(base::WaitableEvent*)
{
    *(int*)1 = 1;
}

void cc::OneCopyRasterBufferProvider::Shutdown(void)
{
    *(int*)1 = 1;
}

void cc::OneCopyRasterBufferProvider::Flush(void)
{
    *(int*)1 = 1;
}

void __thiscall gpu::SharedImageInterface::Release() const
{
    *(int*)1 = 1;
}

gpu::SharedImageInterface::SharedImageMapping::~SharedImageMapping()
{
    *(int*)1 = 1;
}
...
OneCopyRasterBufferProvider::OneCopyRasterBufferProvider(scoped_refptr<base::SequencedTaskRunner> task_runner,
    viz::RasterContextProvider* compositor_context_provider, viz::RasterContextProvider* worker_context_provider, int max_copy_texture_chromium_size,
    bool use_partial_raster, int max_staging_buffer_usage_in_bytes, const RasterCapabilities& raster_caps)
    : compositor_context_provider_(compositor_context_provider)
    , worker_context_provider_(worker_context_provider)
    , max_bytes_per_copy_operation_(
        max_copy_texture_chromium_size ? std::min(kMaxBytesPerCopyOperation, max_copy_texture_chromium_size) : kMaxBytesPerCopyOperation)
    , use_partial_raster_(use_partial_raster)
    , bytes_scheduled_since_last_flush_(0)
    , tile_format_(raster_caps.tile_format)
    , tile_overlay_candidate_(raster_caps.tile_overlay_candidate)
    , staging_pool_(std::move(task_runner), worker_context_provider, use_partial_raster, max_staging_buffer_usage_in_bytes)
{
...
namespace viz {
SkColorType ToClosestSkColorType(bool gpu_compositing, SharedImageFormat format)
{
    ...
    } else if (format == SinglePlaneFormat::kBGR_565 || format == SinglePlaneFormat::kRGB_565) {
        return kRGB_565_SkColorType;
    } else if (format == SinglePlaneFormat::kLUMINANCE_8) {
        return kGray_8_SkColorType;
    } else if (format == SinglePlaneFormat::kRGBX_8888 || format == SinglePlaneFormat::kBGRX_8888 || format == SinglePlaneFormat::kETC1) {
    ...
    } else if (format == SinglePlaneFormat::kLUMINANCE_F16 || format == SinglePlaneFormat::kR_F16) {
...
static_assert(kContainsSrgbCacheSize == gfx::DisplayColorSpaces::kConfigCount / 2, "sRGB cache must match the size of DisplayColorSpaces");

bool IsMobileOptimized(LayerTreeImpl* active_tree)
{
    return util::IsMobileOptimized(active_tree->min_page_scale_factor(), active_tree->max_page_scale_factor(), active_tree->current_page_scale_factor(),
        active_tree->ScrollableViewportSize(), active_tree->ScrollableSize(), active_tree->viewport_mobile_optimized());
}

void DidVisibilityChange(LayerTreeHostImpl* id, bool visible)
{
    if (visible) {
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN1("cc,benchmark", "LayerTreeHostImpl::SetVisible", TRACE_ID_LOCAL(id), "LayerTreeHostImpl", static_cast<void*>(id));
        return;
    }

    TRACE_EVENT_NESTABLE_ASYNC_END0("cc,benchmark", "LayerTreeHostImpl::SetVisible", TRACE_ID_LOCAL(id));
}

void PopulateMetadataContentColorUsage(const LayerTreeHostImpl::FrameData* frame, viz::CompositorFrameMetadata* metadata)
{
    metadata->content_color_usage = gfx::ContentColorUsage::kSRGB;
    for (const LayerImpl* layer : frame->will_draw_layers) {
        metadata->content_color_usage = std::max(metadata->content_color_usage, layer->GetContentColorUsage());
    }
}
...
#define VERBOSE_LOG() VLOG_IF(3, VerboseLogEnabled()) << ClientNameForVerboseLog() << ": "
...
public:
    ImageDecodeCacheHolder(bool enable_shared_image_cache_for_gpu, const RasterCapabilities& raster_caps, bool gpu_compositing,
        scoped_refptr<RasterContextProviderWrapper> worker_context_provider_wrapper, size_t decoded_image_working_set_budget_bytes,
...
//                 image_decode_cache_ptr_ = &worker_context_provider_wrapper->GetGpuImageDecodeCache(color_type, raster_caps);
//             } else {
//                 image_decode_cache_ = std::make_unique<GpuImageDecodeCache>(worker_context_provider_wrapper->GetContext().get(),
//                     /*use_transfer_cache=*/true, color_type, decoded_image_working_set_budget_bytes, raster_caps.max_texture_size, dark_mode_filter);
//             }
            *(int*)1 = 1;
        } else {
            image_decode_cache_ = std::make_unique<SoftwareImageDecodeCache>(
                viz::ToClosestSkColorType(gpu_compositing, raster_caps.tile_format), decoded_image_working_set_budget_bytes);
        }
    ...
    void SetShouldAggressivelyFreeResources(bool aggressively_free_resources)
    {
        ...
        if (image_decode_cache_) {
            image_decode_cache_->SetShouldAggressivelyFreeResources(aggressively_free_resources, /*context_lock_acquired=*/false);
        }
    }
...
void LayerTreeHostImpl::AccumulateScrollDeltaForTracing(const gfx::Vector2dF& delta)
{
...
void LayerTreeHostImpl::SetDeferBeginMainFrame(bool defer_begin_main_frame) const
{
    client_->SetDeferBeginMainFrameFromImpl(defer_begin_main_frame);
}

void LayerTreeHostImpl::UpdateBrowserControlsState(
    BrowserControlsState constraints, BrowserControlsState current, bool animate, base::optional_ref<const BrowserControlsOffsetTagsInfo> offset_tags_info)
{
    browser_controls_offset_manager_->UpdateBrowserControlsState(constraints, current, animate, offset_tags_info);
}
...
LayerTreeHostImpl::UIResourceData::~UIResourceData() = default;
LayerTreeHostImpl::UIResourceData::UIResourceData(UIResourceData&&) noexcept = default;
LayerTreeHostImpl::UIResourceData& LayerTreeHostImpl::UIResourceData::operator=(UIResourceData&&) = default;

std::unique_ptr<LayerTreeHostImpl> LayerTreeHostImpl::Create(const LayerTreeSettings& settings, LayerTreeHostImplClient* client,
    TaskRunnerProvider* task_runner_provider, RenderingStatsInstrumentation* rendering_stats_instrumentation, TaskGraphRunner* task_graph_runner,
    std::unique_ptr<MutatorHost> mutator_host, RasterDarkModeFilter* dark_mode_filter, int id,
    scoped_refptr<base::SequencedTaskRunner> image_worker_task_runner, LayerTreeHostSchedulingClient* scheduling_client)
{
    return base::WrapUnique(new LayerTreeHostImpl(settings, client, task_runner_provider, rendering_stats_instrumentation, task_graph_runner,
        std::move(mutator_host), dark_mode_filter, id, std::move(image_worker_task_runner), scheduling_client));
}

LayerTreeHostImpl::LayerTreeHostImpl(const LayerTreeSettings& settings, LayerTreeHostImplClient* client, TaskRunnerProvider* task_runner_provider,
    RenderingStatsInstrumentation* rendering_stats_instrumentation, TaskGraphRunner* task_graph_runner, std::unique_ptr<MutatorHost> mutator_host,
    RasterDarkModeFilter* dark_mode_filter, int id, scoped_refptr<base::SequencedTaskRunner> image_worker_task_runner,
    ...
    , is_synchronous_single_threaded_(!task_runner_provider->HasImplThread() && !settings_.single_thread_proxy_scheduler && !use_layer_context_for_display_)
    ...
    tile_manager_(this, GetTaskRunner(), std::move(image_worker_task_runner),
        is_synchronous_single_threaded_ ? std::numeric_limits<size_t>::max() : settings.scheduled_raster_task_limit, RunningOnRendererProcess(),
    ...
    image_animation_controller_(GetTaskRunner(), this, settings_.enable_image_animation_resync)
    , compositor_frame_reporting_controller_(std::make_unique<CompositorFrameReportingController>(
    ...
    , frame_trackers_(settings.single_thread_proxy_scheduler, compositor_frame_reporting_controller_.get())
...
{
    resource_provider_ = std::make_unique<viz::ClientResourceProvider>(task_runner_provider_->MainThreadTaskRunner(),
        task_runner_provider_->HasImplThread() ? task_runner_provider_->ImplThreadTaskRunner() : task_runner_provider_->MainThreadTaskRunner(),
        base::BindRepeating(&LayerTreeHostImpl::MaybeFlushPendingWork, weak_factory_.GetWeakPtr()));
    ...
    active_tree_ = std::make_unique<LayerTreeImpl>(
        *this, viz::BeginFrameArgs(), new SyncedScale, new SyncedBrowserControls, new SyncedBrowserControls, new SyncedElasticOverscroll);
    active_tree_->property_trees()->set_is_active(true);

    viewport_ = Viewport::Create(this);

    TRACE_EVENT_OBJECT_CREATED_WITH_ID(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "cc::LayerTreeHostImpl", id_);

    browser_controls_offset_manager_ = BrowserControlsOffsetManager::Create(this, settings.top_controls_show_threshold, settings.top_controls_hide_threshold);

    if (!base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kDisableLayerTreeHostMemoryPressure)) {
        memory_pressure_listener_
            = std::make_unique<base::MemoryPressureListener>(FROM_HERE, base::BindRepeating(&LayerTreeHostImpl::OnMemoryPressure, base::Unretained(this)));
    }

    SetDebugState(settings.initial_debug_state);
    compositor_frame_reporting_controller_->SetDroppedFrameCounter(&dropped_frame_counter_);
    compositor_frame_reporting_controller_->SetFrameSequenceTrackerCollection(&frame_trackers_);
    ...
    if (is_ui) {
        ...
        compositor_frame_reporting_controller_->SetThreadAffectsSmoothness(FrameInfo::SmoothEffectDrivingThread::kMain, true);
    ...
    frame_trackers_.set_custom_tracker_results_added_callback(
        base::BindRepeating(&LayerTreeHostImpl::NotifyThroughputTrackerResults, weak_factory_.GetWeakPtr()));
}
...
LayerTreeHostImpl::~LayerTreeHostImpl()
{
    ...
    TRACE_EVENT_OBJECT_DELETED_WITH_ID(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "cc::LayerTreeHostImpl", id_);
    ...
    compositor_frame_reporting_controller_->SetFrameSequenceTrackerCollection(nullptr);
...
void LayerTreeHostImpl::BeginMainFrameAborted(CommitEarlyOutReason reason, std::vector<std::unique_ptr<SwapPromise>> swap_promises,
    const viz::BeginFrameArgs& args, bool next_bmf, bool scroll_and_viewport_changes_synced)
{
    // If the begin frame data was handled, then scroll and scale set was applied
    // by the main thread, so the active tree needs to be updated as if these sent
    // values were applied and committed.
    bool main_frame_applied_deltas = MainFrameAppliedDeltas(reason);
    active_tree_->ApplySentScrollAndScaleDeltasFromAbortedCommit(next_bmf, main_frame_applied_deltas);
    if (main_frame_applied_deltas) {
        if (pending_tree_) {
            ...
            for (const auto& swap_promise : swap_promises) {
                SwapPromise::DidNotSwapAction action = swap_promise->DidNotSwap(SwapPromise::COMMIT_NO_UPDATE, timestamp);
...
void LayerTreeHostImpl::ReadyToCommit(
    const viz::BeginFrameArgs& commit_args, bool scroll_and_viewport_changes_synced, const BeginMainFrameMetrics* begin_main_frame_metrics, bool commit_timeout)
{
    if (!is_measuring_smoothness_ && ((begin_main_frame_metrics && begin_main_frame_metrics->should_measure_smoothness) || commit_timeout)) {
...
void LayerTreeHostImpl::BeginCommit(int source_frame_number, BeginMainFrameTraceId trace_id)
{
...
void LayerTreeHostImpl::FinishCommit(CommitState& state, const ThreadUnsafeCommitState& unsafe_state)
{
    ...
    LayerTreeImpl* tree = sync_tree();
    {
        ...
        viz::ClientResourceProvider::ScopedBatchResourcesRelease scoped_resource_release = resource_provider_->CreateScopedBatchResourcesRelease();
...
void LayerTreeHostImpl::PullLayerTreeHostPropertiesFrom(const CommitState& commit_state)
{
...
void LayerTreeHostImpl::RecordGpuRasterizationHistogram()
{
    ...
    UMA_HISTOGRAM_BOOLEAN("Renderer4.GpuRasterizationEnabled", raster_caps().use_gpu_rasterization);
}
...
//     TRACE_EVENT("cc,benchmark", "LayerTreeHostImpl::CommitComplete", [&](perfetto::EventContext ctx) {
//         EmitMainFramePipelineStep(ctx, sync_tree()->trace_id(), perfetto::protos::pbzero::MainFramePipeline::Step::COMMIT_COMPLETE);
//     });
    ...
    if (mutator_host_->MainThreadAnimationsCount() > 0 || mutator_host_->HasSmilAnimation()) {
        frame_trackers_.StartSequence(FrameSequenceTrackerType::kMainThreadAnimation);
        if (mutator_host_->HasViewTransition()) {
            frame_trackers_.StartSequence(FrameSequenceTrackerType::kSETMainThreadAnimation);
        }
    }
...
void LayerTreeHostImpl::UpdateSyncTreeAfterCommitOrImplSideInvalidation()
{
    ...
    sync_tree()->UpdateDrawProperties(update_tiles, update_image_animation_controller);
    ...
    PaintImageIdFlatSet images_to_invalidate = tile_manager_.TakeImagesToInvalidateOnSyncTree();

    const auto& animated_images = image_animation_controller_.AnimateForSyncTree(CurrentBeginFrameArgs());
    ...
    if (paint_worklet_tracker_.InvalidatePaintWorkletsOnPendingTree()) {
        client_->SetNeedsImplSideInvalidation(true /* needs_first_draw_on_activation */);
    }
    PaintImageIdFlatSet dirty_paint_worklet_ids;
    PaintWorkletJobMap dirty_paint_worklets = GatherDirtyPaintWorklets(&dirty_paint_worklet_ids);
    images_to_invalidate.insert(dirty_paint_worklet_ids.begin(), dirty_paint_worklet_ids.end());
    ...
    if (!pending_invalidation_raster_inducing_scrolls_.empty()) {
        ...
        std::swap(scrolls_to_invalidate, pending_invalidation_raster_inducing_scrolls_);
    ...
    client_->NotifyPaintWorkletStateChange(Scheduler::PaintWorkletState::PROCESSING);
    auto done_callback = base::BindOnce(&LayerTreeHostImpl::OnPaintWorkletResultsReady, base::Unretained(this));
    paint_worklet_painter_->DispatchWorklets(std::move(dirty_paint_worklets), std::move(done_callback));
}

PaintWorkletJobMap LayerTreeHostImpl::GatherDirtyPaintWorklets(PaintImageIdFlatSet* dirty_paint_worklet_ids) const
{
    PaintWorkletJobMap dirty_paint_worklets;
    for (PictureLayerImpl* layer : sync_tree()->picture_layers_with_paint_worklets()) {
        for (const auto& entry : layer->GetPaintWorkletRecordMap()) {
            ...
            for (const auto& element : input->GetPropertyKeys()) {
                ...
                const PaintWorkletInput::PropertyValue& animated_property_value = paint_worklet_tracker_.GetPropertyAnimationValue(element);
            ...
            job_vector->data.emplace_back(layer->id(), input, std::move(animated_property_values));
        }
    }
...
void LayerTreeHostImpl::OnPaintWorkletResultsReady(PaintWorkletJobMap results)
{
    ...
    // the same.
    PaintImageIdFlatSet dirty_paint_worklet_ids;
    DCHECK_EQ(results.size(), GatherDirtyPaintWorklets(&dirty_paint_worklet_ids).size());
#endif

    for (const auto& entry : results) {
        for (const PaintWorkletJob& job : entry.second->data) {
            LayerImpl* layer_impl = pending_tree_->FindPendingTreeLayerById(job.layer_id());
            ...
            DCHECK(layer_impl);
            static_cast<PictureLayerImpl*>(layer_impl)->SetPaintWorkletRecord(job.input(), job.output());
        }
    }
...
void LayerTreeHostImpl::NotifyPendingTreeFullyPainted()
{
    ...
    // ongoing dispatch.
    DCHECK(!paint_worklet_painter_ || !paint_worklet_painter_->HasOngoingDispatch());
    ...
    if (!did_prepare_tiles) {
        ...
        if (CommitsToActiveTree() || settings_.wait_for_all_pipeline_stages_before_draw) {
            NotifyReadyToDraw();
        }
    }
}
...
bool LayerTreeHostImpl::CanDraw() const
{
    ...
    if (!layer_tree_frame_sink_) {
        TRACE_EVENT_INSTANT0("cc", "LayerTreeHostImpl::CanDraw no LayerTreeFrameSink", TRACE_EVENT_SCOPE_THREAD);
        return false;
    }
    ...
    // newer viz::Surface
    if (base::FeatureList::IsEnabled(features::kEvictionThrottlesDraw) && evicted_local_surface_id_.is_valid()) {
        TRACE_EVENT_INSTANT0("cc", "LayerTreeHostImpl::CanDraw viz::Surface evicted and not recreated", TRACE_EVENT_SCOPE_THREAD);
        return false;
    }

    if (active_tree_->GetDeviceViewport().IsEmpty()) {
        TRACE_EVENT_INSTANT0("cc", "LayerTreeHostImpl::CanDraw empty viewport", TRACE_EVENT_SCOPE_THREAD);
        return false;
    }
    if (EvictedUIResourcesExist()) {
        TRACE_EVENT_INSTANT0("cc", "LayerTreeHostImpl::CanDraw UI resources evicted not recreated", TRACE_EVENT_SCOPE_THREAD);
        return false;
    }
    return true;
}
...
void LayerTreeHostImpl::StartPageScaleAnimation(const gfx::Point& target_offset, bool anchor_point, float page_scale, base::TimeDelta duration)
{
    ...
    gfx::SizeF viewport_size(active_tree_->InnerViewportScrollNode()->container_bounds);
    ...
    page_scale_animation_ = PageScaleAnimation::Create(scroll_total, active_tree_->current_page_scale_factor(), viewport_size, scrollable_size);

    if (anchor_point) {
        gfx::PointF anchor(target_offset);
        page_scale_animation_->ZoomWithAnchor(anchor, page_scale, duration.InSecondsF());
    } else {
        gfx::PointF scaled_target_offset(target_offset);
        page_scale_animation_->ZoomTo(scaled_target_offset, page_scale, duration.InSecondsF());
    }
...
std::unique_ptr<LatencyInfoSwapPromiseMonitor> LayerTreeHostImpl::CreateLatencyInfoSwapPromiseMonitor(ui::LatencyInfo* latency)
{
    return std::make_unique<LatencyInfoSwapPromiseMonitor>(latency, this);
}

std::unique_ptr<EventsMetricsManager::ScopedMonitor> LayerTreeHostImpl::GetScopedEventMetricsMonitor(
...
void LayerTreeHostImpl::QueueSwapPromiseForMainThreadScrollUpdate(std::unique_ptr<SwapPromise> swap_promise)
{
    swap_promises_for_main_thread_scroll_update_.push_back(std::move(swap_promise));
}

void LayerTreeHostImpl::FrameData::AsValueInto(base::trace_event::TracedValue* value) const
{
    ...
    if (!quads_enabled) {
        TRACE_EVENT_CATEGORY_GROUP_ENABLED(TRACE_DISABLED_BY_DEFAULT("viz.quads"), &quads_enabled);
    }
    if (quads_enabled) {
...
DrawMode LayerTreeHostImpl::GetDrawMode() const
{
    ...
    } else if (layer_tree_frame_sink_->context_provider() || settings_.is_display_tree) {
...
static void AppendQuadsToFillScreen(
    viz::CompositorRenderPass* target_render_pass, const RenderSurfaceImpl* root_render_surface, SkColor4f screen_background_color, const Region& fill_region)
{
    ...
    viz::SharedQuadState* shared_quad_state = target_render_pass->CreateAndAppendSharedQuadState();
    shared_quad_state->SetAll(gfx::Transform(), root_target_rect, root_target_rect, gfx::MaskFilterInfo(), std::nullopt, screen_background_color.isOpaque(),
    ...
    for (gfx::Rect screen_space_rect : fill_region) {
        ...
        // occlusion checks.
        auto* quad = target_render_pass->CreateAndAppendDrawQuad<viz::SolidColorDrawQuad>();
        quad->SetNew(shared_quad_state, screen_space_rect, visible_screen_space_rect, screen_background_color, false);
    }
}

static viz::CompositorRenderPass* FindRenderPassById(const viz::CompositorRenderPassList& list, viz::CompositorRenderPassId id)
{
...
bool LayerTreeHostImpl::HasDamage() const
{
    ...
    if (last_draw_local_surface_id_ != child_local_surface_id_allocator_.GetCurrentLocalSurfaceId()) {
        return true;
    }
    ...
    if (last_draw_render_frame_metadata_
        && last_draw_render_frame_metadata_->primary_main_frame_item_sequence_number != active_tree->primary_main_frame_item_sequence_number()) {
        return true;
    }
    ...
    bool root_surface_has_visible_damage = root_surface->GetDamageRect().Intersects(root_surface->content_rect());
    bool hud_wants_to_draw_ = active_tree->hud_layer() && active_tree->hud_layer()->IsAnimatingHUDContents();

    return root_surface_has_visible_damage || active_tree_->property_trees()->effect_tree().HasCopyRequests() || hud_wants_to_draw_
...
DrawResult LayerTreeHostImpl::CalculateRenderPasses(FrameData* frame)
{
    ...
    frame->damage_reasons = active_tree_->RootRenderSurface()->damage_tracker()->GetDamageReasons();

    if (HasDamage()) {
        consecutive_frame_with_damage_count_++;
    } else {
        TRACE_EVENT0("cc", "LayerTreeHostImpl::CalculateRenderPasses::EmptyDamageRect");
    ...
    TRACE_EVENT_BEGIN2("cc,benchmark", "LayerTreeHostImpl::CalculateRenderPasses", "render_surface_list.size()",
        static_cast<uint64_t>(frame->render_surface_list->size()), "RequiresHighResToDraw", RequiresHighResToDraw());
    ...
    for (size_t i = 0; i < render_surface_list_size; ++i) {
        ...
        RenderSurfaceImpl* render_surface = (*frame->render_surface_list)[surface_index];

        const bool is_root_surface = render_surface->EffectTreeIndex() == kContentsRootPropertyNodeId;
        const bool should_draw_into_render_pass = is_root_surface || render_surface->contributes_to_drawn_surface() || render_surface->CopyOfOutputRequired();
        if (should_draw_into_render_pass)
            frame->render_passes.push_back(render_surface->CreateRenderPass());
        if (render_surface->OwningEffectNode()->view_transition_element_resource_id.IsValid()) {
            known_resource_ids.insert(render_surface->OwningEffectNode()->view_transition_element_resource_id);
        }
    }

    frame->has_view_transition_save_directive = active_tree_->HasViewTransitionSaveRequest();
    ...
    const bool compute_video_layer_preferred_interval = !features::UseSurfaceLayerForVideo() && features::IsUsingFrameIntervalDecider();

    if (settings_.enable_compositing_based_throttling)
        throttle_decider_.Prepare();
    for (EffectTreeLayerListIterator it(active_tree()); it.state() != EffectTreeLayerListIterator::State::kEnd; ++it) {
        auto target_render_pass_id = it.target_render_surface()->render_pass_id();
        viz::CompositorRenderPass* target_render_pass = FindRenderPassById(frame->render_passes, target_render_pass_id);
        ...
        if (it.state() == EffectTreeLayerListIterator::State::kTargetSurface) {
            ...
            if (render_surface->HasCopyRequest()) {
                active_tree()->property_trees()->effect_tree_mutable().TakeCopyRequestsAndTransformToSurface(
                    render_surface->EffectTreeIndex(), &target_render_pass->copy_requests);
            }
        ...
        } else if (it.state() == EffectTreeLayerListIterator::State::kContributingSurface) {
            RenderSurfaceImpl* render_surface = it.current_render_surface();
            if (render_surface->contributes_to_drawn_surface()) {
                render_surface->AppendQuads(draw_mode, target_render_pass, &append_quads_data);
            }
            ...
            if (layer->WillDraw(draw_mode, resource_provider_.get())) {
                ...
                if (compute_video_layer_preferred_interval && layer->GetLayerType() == mojom::LayerType::kVideo) {
                    VideoLayerImpl* video_layer = static_cast<VideoLayerImpl*>(layer);
                    std::optional<base::TimeDelta> video_preferred_interval = video_layer->GetPreferredRenderInterval();
                    if (video_preferred_interval) {
                        frame->video_layer_preferred_intervals[video_preferred_interval.value()]++;
                    }
                }
            ...
            rendering_stats_instrumentation_->AddVisibleContentArea(append_quads_data.visible_layer_area);
            rendering_stats_instrumentation_->AddApproximatedVisibleContentArea(append_quads_data.approximated_visible_content_area);

            num_missing_tiles += append_quads_data.num_missing_tiles;
            frame->checkerboarded_needs_raster |= append_quads_data.checkerboarded_needs_raster;
            frame->checkerboarded_needs_record |= append_quads_data.checkerboarded_needs_record;

            if (append_quads_data.num_missing_tiles > 0) {
                have_missing_animated_tiles |= layer->screen_space_transform_is_animating();
            }
        }
        frame->activation_dependencies.insert(
            frame->activation_dependencies.end(), append_quads_data.activation_dependencies.begin(), append_quads_data.activation_dependencies.end());
        if (append_quads_data.deadline_in_frames) {
            if (!frame->deadline_in_frames) {
                ...
                frame->deadline_in_frames = std::max(*frame->deadline_in_frames, *append_quads_data.deadline_in_frames);
            }
        }
        frame->use_default_lower_bound_deadline |= append_quads_data.use_default_lower_bound_deadline;
        frame->has_shared_element_resources |= append_quads_data.has_shared_element_resources;
    }
    ...
    DCHECK_EQ(frame->render_passes.back()->output_rect.origin(), active_tree_->GetDeviceViewport().origin());
#endif
    bool has_transparent_background = !active_tree_->background_color().isOpaque();
    ...
    if (root_render_surface && !has_transparent_background) {
        ...
        AppendQuadsToFillScreen(frame->render_passes.back().get(), root_render_surface, active_tree_->background_color(), fill_region);
    }
    ...
    TRACE_EVENT_END2("cc,benchmark", "LayerTreeHostImpl::CalculateRenderPasses", "draw_result", draw_result, "missing tiles", num_missing_tiles);
...
void LayerTreeHostImpl::InvalidateContentOnImplSide()
{
    ...
    DCHECK(impl_thread_phase_ == ImplThreadPhase::INSIDE_IMPL_FRAME || settings_.using_synchronous_renderer_compositor);
...
DrawResult LayerTreeHostImpl::PrepareToDraw(FrameData* frame)
{
    ...
    TRACE_EVENT1("cc", "LayerTreeHostImpl::PrepareToDraw", "SourceFrameNumber", active_tree_->source_frame_number());
    if (input_delegate_)
        input_delegate_->WillDraw();

    // No need to record metrics each time we draw, 1% is enough.
    constexpr double kSamplingFrequency = .01;
    if (!downsample_metrics_ || metrics_subsampler_.ShouldSample(kSamplingFrequency)) {
        // These metrics are only for the renderer process.
        if (RunningOnRendererProcess()) {
            UMA_HISTOGRAM_CUSTOM_COUNTS("Compositing.Renderer.NumActiveLayers", base::saturated_cast<int>(active_tree_->NumLayers()), 1, 1000, 20);

            UMA_HISTOGRAM_CUSTOM_COUNTS(
                "Compositing.Renderer.NumActivePictureLayers", base::saturated_cast<int>(active_tree_->picture_layers().size()), 1, 1000, 20);
        }
    }
    ...
    if (active_tree_->RootRenderSurface()) {
        active_tree_->RootRenderSurface()->damage_tracker()->AddDamageNextUpdate(viewport_damage_rect_);
...
void LayerTreeHostImpl::RemoveRenderPasses(FrameData* frame)
{
    ...
    base::flat_set<viz::ViewTransitionElementResourceId> view_transition_quad_references;
    ...
    for (size_t i = 0; i < frame->render_passes.size(); ++i) {
        ...
        for (auto it = pass->quad_list.begin(); it != pass->quad_list.end();) {
            if (it->material == viz::DrawQuad::Material::kSharedElement) {
                view_transition_quad_references.insert(viz::SharedElementDrawQuad::MaterialCast(*it)->element_resource_id);
                ++it;
                continue;
            }
            ...
            const viz::CompositorRenderPassDrawQuad* quad = viz::CompositorRenderPassDrawQuad::MaterialCast(*it);
        ...
        if (pass->quad_list.empty() && pass->copy_requests.empty() && !pass->subtree_capture_id.is_valid() && pass->filters.IsEmpty()
    ...
    for (size_t i = 0; i < frame->render_passes.size() - 1; ++i) {
        ...
        viz::CompositorRenderPass* pass = frame->render_passes[frame->render_passes.size() - 2 - i].get();
        ...
        if (pass->view_transition_element_resource_id.IsValid()
            && (frame->has_view_transition_save_directive || view_transition_quad_references.contains(pass->view_transition_element_resource_id))) {
            continue;
        }

        for (auto it = pass->quad_list.begin(); it != pass->quad_list.end(); ++it) {
            if (const viz::CompositorRenderPassDrawQuad* quad = it->DynamicCast<viz::CompositorRenderPassDrawQuad>()) {
                pass_references[quad->render_pass_id]--;
            }
        }
...
void LayerTreeHostImpl::BlockNotifyReadyToActivateForTesting(bool block, bool notify_if_blocked)
{
    NOTREACHED();
}

void LayerTreeHostImpl::BlockImplSideInvalidationRequestsForTesting(bool block)
{
    NOTREACHED();
}
...
void LayerTreeHostImpl::ResetTreesForTesting()
{
    ...
    active_tree_ = std::make_unique<LayerTreeImpl>(*this, CurrentBeginFrameArgs(), active_tree()->page_scale_factor(),
        active_tree()->top_controls_shown_ratio(), active_tree()->bottom_controls_shown_ratio(), active_tree()->elastic_overscroll());
    active_tree_->property_trees()->set_is_active(true);
...
void LayerTreeHostImpl::UpdateTileManagerMemoryPolicy(const ManagedMemoryPolicy& policy)
{
    if (!resource_pool_)
        return;
    ...
    if (visible_ && policy.bytes_limit_when_visible > 0) {
        ...
        global_tile_state_.soft_memory_limit_in_bytes
            = (static_cast<int64_t>(global_tile_state_.hard_memory_limit_in_bytes) * settings_.max_memory_for_prepaint_percentage) / 100;
    }
    ...
    resource_pool_->SetResourceUsageLimits(global_tile_state_.soft_memory_limit_in_bytes, global_tile_state_.num_resources_limit);
...
void LayerTreeHostImpl::SetTargetLocalSurfaceId(const viz::LocalSurfaceId& target_local_surface_id)
{
    target_local_surface_id_ = target_local_surface_id;
}

std::unique_ptr<RasterTilePriorityQueue> LayerTreeHostImpl::BuildRasterQueue(TreePriority tree_priority, RasterTilePriorityQueue::Type type)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "LayerTreeHostImpl::BuildRasterQueue");

    return RasterTilePriorityQueue::Create(active_tree_->picture_layers(),
        pending_tree_ && pending_tree_fully_painted_ ? pending_tree_->picture_layers() : std::vector<raw_ptr<PictureLayerImpl, VectorExperimental>>(),
        tree_priority, type);
}

std::unique_ptr<EvictionTilePriorityQueue> LayerTreeHostImpl::BuildEvictionQueue(TreePriority tree_priority)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "LayerTreeHostImpl::BuildEvictionQueue");

    std::unique_ptr<EvictionTilePriorityQueue> queue(new EvictionTilePriorityQueue);
    queue->Build(active_tree_->picture_layers(), pending_tree_ ? pending_tree_->picture_layers() : std::vector<raw_ptr<PictureLayerImpl, VectorExperimental>>(),
        tree_priority);
    return queue;
}

std::unique_ptr<TilesWithResourceIterator> LayerTreeHostImpl::CreateTilesWithResourceIterator()
{
    return std::make_unique<TilesWithResourceIterator>(&active_tree_->picture_layers(), pending_tree_ ? &pending_tree_->picture_layers() : nullptr);
}
...
void LayerTreeHostImpl::SetIsLikelyToRequireADraw(bool is_likely_to_require_a_draw)
{
...
TargetColorParams LayerTreeHostImpl::GetTargetColorParams(gfx::ContentColorUsage content_color_usage) const
{
    ...
    if (settings_.prefer_raster_in_srgb && content_color_usage == gfx::ContentColorUsage::kSRGB) {
        return params;
    }

    auto hdr_color_space = display_cs.GetOutputColorSpace(gfx::ContentColorUsage::kHDR,
        /*needs_alpha=*/false);
...
bool LayerTreeHostImpl::CheckColorSpaceContainsSrgb(const gfx::ColorSpace& color_space) const
{
...
size_t LayerTreeHostImpl::GetFrameIndexForImage(const PaintImage& paint_image, WhichTree tree) const
{
    if (!paint_image.ShouldAnimate())
        return PaintImage::kDefaultFrameIndex;

    return image_animation_controller_.GetFrameIndexForImage(paint_image.stable_id(), tree);
}

int LayerTreeHostImpl::GetMSAASampleCountForRaster(const DisplayItemList& display_list) const
{
    if (display_list.num_slow_paths_up_to_min_for_MSAA() < kMinNumberOfSlowPathsForMSAA) {
        return 0;
    }
...
void LayerTreeHostImpl::NotifyTileStateChanged(const Tile* tile)
{
    ...
    const bool is_pending_tree = tile->tiling()->tree() == WhichTree::PENDING_TREE;
    if (is_pending_tree) {
    ...
    if (settings_.UseLayerContextForDisplay() && !is_pending_tree) {
        ...
        layer_context_->UpdateDisplayTile(
            static_cast<PictureLayerImpl&>(*layer_impl), *tile, *resource_provider(), *layer_tree_frame_sink_->context_provider());
    }
...
void LayerTreeHostImpl::SetTreeActivationCallback(base::RepeatingClosure callback)
{
    DCHECK(task_runner_provider_->IsImplThread());
...
void LayerTreeHostImpl::SetMemoryPolicyImpl(const ManagedMemoryPolicy& policy)
{
    ...
    if (visible() && actual_policy.bytes_limit_when_visible >= max_memory_needed_bytes_ && old_policy.bytes_limit_when_visible >= max_memory_needed_bytes_
...
void LayerTreeHostImpl::SetExternalTilePriorityConstraints(const gfx::Rect& viewport_rect, const gfx::Transform& transform)
{
    const bool tile_priority_params_changed = viewport_rect_for_tile_priority_ != viewport_rect;
...
void LayerTreeHostImpl::DidPresentCompositorFrame(uint32_t frame_token, const viz::FrameTimingDetails& details)
{
    PresentationTimeCallbackBuffer::PendingCallbacks activated_callbacks
        = presentation_time_callbacks_.PopPendingCallbacks(frame_token, details.presentation_feedback.failed());

    // Send all tasks to the client so that it can decide which tasks
    // should run on which thread.
    client_->DidPresentCompositorFrameOnImplThread(frame_token, std::move(activated_callbacks), details);
    ...
    // ::OnBeginFrame).
    GetTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&LayerTreeHostImpl::LogAverageLagEvents, weak_factory_.GetWeakPtr(), frame_token, details));
}

void LayerTreeHostImpl::LogAverageLagEvents(uint32_t frame_token, const viz::FrameTimingDetails& details)
{
    lag_tracking_manager_.DidPresentCompositorFrame(frame_token, details);
}

void LayerTreeHostImpl::NotifyThroughputTrackerResults(const CustomTrackerResults& results)
{
...
void LayerTreeHostImpl::ReclaimResources(std::vector<viz::ReturnedResource> resources)
{
    ...
    if (base::FeatureList::IsEnabled(features::kReclaimResourcesDelayedFlushInBackground)) {
        ...
        GetTaskRunner()->PostDelayedTask(FROM_HERE, base::BindOnce(&LayerTreeHostImpl::MaybeFlushPendingWork, weak_factory_.GetWeakPtr()), base::Seconds(1));
    }
}
...
void LayerTreeHostImpl::OnDraw(const gfx::Transform& transform, const gfx::Rect& viewport, bool resourceless_software_draw, bool skip_draw)
{
    DCHECK(!resourceless_software_draw_);
    ...
    {
        base::AutoReset<bool> resourceless_software_draw_reset(&resourceless_software_draw_, resourceless_software_draw);
        ...
        client_->OnDrawForLayerTreeFrameSink(resourceless_software_draw_, skip_draw);
    }
...
void LayerTreeHostImpl::OnCompositorFrameTransitionDirectiveProcessed(uint32_t sequence_id)
{
    ...
    client_->NotifyTransitionRequestFinished(sequence_id, std::move(rects_for_this_sequence));
}

void LayerTreeHostImpl::SetViewTransitionContentRect(uint32_t sequence_id, const viz::ViewTransitionElementResourceId& id, const gfx::RectF& rect)
{
    view_transition_content_rects_[sequence_id].insert(std::make_pair(id, rect));
}

void LayerTreeHostImpl::OnSurfaceEvicted(const viz::LocalSurfaceId& local_surface_id)
{
    ...
    if (target_local_surface_id_.IsNewerThanOrEmbeddingChanged(local_surface_id)) {
        return;
    }
...
void LayerTreeHostImpl::ReportEventLatency(std::vector<EventLatencyTracker::LatencyData> latencies)
{
...
viz::RegionCaptureBounds LayerTreeHostImpl::CollectRegionCaptureBounds()
{
    ...
    for (const auto* layer : base::Reversed(*active_tree())) {
        ...
        for (const auto& bounds_pair : layer->capture_bounds()->bounds()) {
            ...
            gfx::Rect bounds_in_screen_space = MathUtil::ProjectEnclosingClippedRect(layer->ScreenSpaceTransform(), bounds_pair.second);

            const RenderSurfaceImpl* root_surface = active_tree()->RootRenderSurface();
            const gfx::Rect content_rect_in_screen_space
                = gfx::ToEnclosedRect(MathUtil::MapClippedRect(root_surface->screen_space_transform(), root_surface->DrawableContentRect()));
...
viz::CompositorFrameMetadata LayerTreeHostImpl::MakeCompositorFrameMetadata()
{
    ...
    presentation_time_callbacks_.RegisterMainThreadCallbacks(metadata.frame_token, active_tree_->TakePresentationCallbacks());
    presentation_time_callbacks_.RegisterMainThreadSuccessfulCallbacks(metadata.frame_token, active_tree_->TakeSuccessfulPresentationCallbacks());

    if (input_delegate_) {
        metadata.is_handling_interaction = GetActivelyScrollingType() != ActivelyScrollingType::kNone || input_delegate_->IsHandlingTouchSequence();
    }

    const base::flat_set<viz::SurfaceRange>& referenced_surfaces = active_tree_->SurfaceRanges();
    ...
    if (browser_controls_offset_manager_->TopControlsHeight() > 0) {
        float visible_height = browser_controls_offset_manager_->TopControlsHeight() * browser_controls_offset_manager_->TopControlsShownRatio();
        metadata.top_controls_visible_height.emplace(visible_height);

#if BUILDFLAG(IS_ANDROID)
        if (features::IsBrowserControlsInVizEnabled()) {
            const viz::OffsetTag& top_controls_offset_tag = browser_controls_offset_manager_->TopControlsOffsetTag();
            const viz::OffsetTag& content_offset_tag = browser_controls_offset_manager_->ContentOffsetTag();

            if (top_controls_offset_tag) {
                CHECK(!content_offset_tag.IsEmpty());

                float offset = browser_controls_offset_manager_->TopControlsHeight() - visible_height;
                if (visible_height == 0) {
                    ...
                    offset += browser_controls_offset_manager_->TopControlsHairlineHeight();
                }
    ...
    if (const gfx::DelegatedInkMetadata* delegated_ink_metadata_ptr = active_tree_->delegated_ink_metadata()) {
        std::unique_ptr<gfx::DelegatedInkMetadata> delegated_ink_metadata = std::make_unique<gfx::DelegatedInkMetadata>(*delegated_ink_metadata_ptr);
        delegated_ink_metadata->set_frame_time(CurrentBeginFrameArgs().frame_time);
        TRACE_EVENT_WITH_FLOW1("delegated_ink_trails", "Delegated Ink Metadata set on compositor frame metadata",
            TRACE_ID_GLOBAL(delegated_ink_metadata->trace_id()), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "metadata",
    ...
    if (!screenshot_destination_.is_empty()) {
        metadata.screenshot_destination = blink::SameDocNavigationScreenshotDestinationToken(screenshot_destination_);
...
RenderFrameMetadata LayerTreeHostImpl::MakeRenderFrameMetadata(FrameData* frame)
{
    ...
    metadata.page_scale_factor = active_tree_->current_page_scale_factor();
    metadata.external_page_scale_factor = active_tree_->external_page_scale_factor();
    ...
    metadata.top_controls_min_height_offset = browser_controls_offset_manager_->TopControlsMinHeightOffset();
    metadata.bottom_controls_min_height_offset = browser_controls_offset_manager_->BottomControlsMinHeightOffset();
    metadata.scrollable_viewport_size = active_tree_->ScrollableViewportSize();
    metadata.min_page_scale_factor = active_tree_->min_page_scale_factor();
    ...
    if (InnerViewportScrollNode()) {
        ...
        metadata.root_overflow_y_hidden = !OuterViewportScrollNode()->user_scrollable_vertical || !InnerViewportScrollNode()->user_scrollable_vertical;
    }
    metadata.has_transparent_background = frame->render_passes.back()->has_transparent_background;
    ...
    if (last_draw_render_frame_metadata_) {
        const float last_root_scroll_offset_y = last_draw_render_frame_metadata_->root_scroll_offset.value_or(gfx::PointF()).y();

        const float new_root_scroll_offset_y = metadata.root_scroll_offset.value().y();

        if (!MathUtil::IsWithinEpsilon(last_root_scroll_offset_y, new_root_scroll_offset_y)) {
            viz::VerticalScrollDirection new_vertical_scroll_direction
                = (last_root_scroll_offset_y < new_root_scroll_offset_y) ? viz::VerticalScrollDirection::kDown : viz::VerticalScrollDirection::kUp;
...
#if !BUILDFLAG(IS_ANDROID) && !BUILDFLAG(IS_IOS)
            last_draw_render_frame_metadata_->top_controls_height != metadata.top_controls_height
            || last_draw_render_frame_metadata_->top_controls_shown_ratio != metadata.top_controls_shown_ratio;
#elif BUILDFLAG(IS_ANDROID)
            last_draw_render_frame_metadata_->top_controls_height != metadata.top_controls_height
            || last_draw_render_frame_metadata_->bottom_controls_height != metadata.bottom_controls_height
            || last_draw_render_frame_metadata_->selection != metadata.selection
            || last_draw_render_frame_metadata_->has_transparent_background != metadata.has_transparent_background;
        ...
        if (features::IsBrowserControlsInVizEnabled()) {
            ...
            allocate_new_local_surface_id |= (last_draw_render_frame_metadata_->has_offset_tag && !metadata.has_offset_tag);
            ...
            if (!browser_controls_offset_manager_->HasOffsetTag()) {
                allocate_new_local_surface_id |= last_draw_render_frame_metadata_->top_controls_shown_ratio != metadata.top_controls_shown_ratio
                    || last_draw_render_frame_metadata_->bottom_controls_shown_ratio != metadata.bottom_controls_shown_ratio;
            } else {
                ...
                bool bottom_controls_exist = metadata.bottom_controls_height != 0 || last_draw_render_frame_metadata_->bottom_controls_height != 0;
                allocate_new_local_surface_id
                    |= bottom_controls_exist && last_draw_render_frame_metadata_->bottom_controls_shown_ratio != metadata.bottom_controls_shown_ratio;
            }
        } else {
            allocate_new_local_surface_id |= last_draw_render_frame_metadata_->top_controls_shown_ratio != metadata.top_controls_shown_ratio
                || last_draw_render_frame_metadata_->bottom_controls_shown_ratio != metadata.bottom_controls_shown_ratio;
    ...
    if (child_local_surface_id_allocator_.GetCurrentLocalSurfaceId().is_valid()) {
        ...
        metadata.local_surface_id = child_local_surface_id_allocator_.GetCurrentLocalSurfaceId();
    }

    metadata.primary_main_frame_item_sequence_number = active_tree()->primary_main_frame_item_sequence_number();
...
std::optional<SubmitInfo> LayerTreeHostImpl::DrawLayers(FrameData* frame)
{
    ...
    layer_tree_frame_sink_->set_source_frame_number(active_tree_->source_frame_number());
    ...
    if (last_draw_active_tree_begin_frame_args_.IsValid()) {
        ...
        drawn_with_new_layer_tree = active_tree_->CreatedBeginFrameArgs().frame_id != last_draw_active_tree_begin_frame_args_.frame_id;
    }
    last_draw_active_tree_begin_frame_args_ = active_tree_->CreatedBeginFrameArgs();
    ...
    float current_top_controls = compositor_frame.metadata.top_controls_visible_height.value_or(0.f);
...
#if DCHECK_IS_ON()
    const viz::BeginFrameId begin_frame_ack_frame_id = compositor_frame.metadata.begin_frame_ack.frame_id;
#endif

    EventMetricsSet events_metrics(active_tree()->TakeEventsMetrics(), events_metrics_manager_.TakeSavedEventsMetrics());
    lag_tracking_manager_.CollectScrollEventsFromFrame(frame_token, events_metrics);

    // Dump property trees and layers if VerboseLogEnabled().
    VERBOSE_LOG() << "Submitting a frame:\n" << viz::TransitionUtils::RenderPassListToString(compositor_frame.render_pass_list);

    base::TimeTicks submit_time = base::TimeTicks::Now();
    {
        TRACE_EVENT(
            "viz,benchmark,graphics.pipeline", "Graphics.Pipeline", perfetto::Flow::Global(CurrentBeginFrameArgs().trace_id), [&](perfetto::EventContext ctx) {
                auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
                auto* data = event->set_chrome_graphics_pipeline();
                data->set_step(perfetto::protos::pbzero::ChromeGraphicsPipeline::StepName::STEP_SUBMIT_COMPOSITOR_FRAME);
                data->set_surface_frame_trace_id(CurrentBeginFrameArgs().trace_id);
                for (const ui::LatencyInfo& latency : compositor_frame.metadata.latency_info) {
    ...
    if (!mutator_host_->NextFrameHasPendingRAF() && !mutator_host_->HasJSAnimation())
        frame_trackers_.StopSequence(FrameSequenceTrackerType::kJSAnimation);

    if (mutator_host_->MainThreadAnimationsCount() == 0 && !mutator_host_->HasSmilAnimation()) {
        frame_trackers_.StopSequence(FrameSequenceTrackerType::kMainThreadAnimation);
        frame_trackers_.StopSequence(FrameSequenceTrackerType::kSETMainThreadAnimation);
    } else if (!mutator_host_->HasViewTransition()) {
        frame_trackers_.StopSequence(FrameSequenceTrackerType::kSETMainThreadAnimation);
    }

    if (lcd_text_metrics_reporter_) {
        lcd_text_metrics_reporter_->NotifySubmitFrame(frame->origin_begin_main_frame_args);
    }
    ...
    devtools_instrumentation::DidDrawFrame(id_, frame->begin_frame_ack.frame_id.sequence_number);
    benchmark_instrumentation::IssueImplThreadRenderingStatsEvent(rendering_stats_instrumentation_->TakeImplThreadRenderingStats());

    if (settings_.enable_compositing_based_throttling && throttle_decider_.HasThrottlingChanged()) {
        client_->FrameSinksToThrottleUpdated(throttle_decider_.ids());
    }

    if (GetActivelyScrollingType() != ActivelyScrollingType::kNone && frame->checkerboarded_needs_record) {
        scroll_checkerboards_incomplete_recording_ = true;
    }

    return SubmitInfo { frame_token, submit_time, frame->checkerboarded_needs_raster, frame->checkerboarded_needs_record, top_controls_moved,
        std::move(events_metrics), drawn_with_new_layer_tree };
}

viz::CompositorFrame LayerTreeHostImpl::GenerateCompositorFrame(FrameData* frame)
{
    ...
    if (debug_state_.ShowDebugRects()) {
        debug_rect_history_->SaveDebugRectsForCurrentFrame(active_tree(), active_tree_->hud_layer(), *frame->render_surface_list, debug_state_);
    }

    TRACE_EVENT_INSTANT2(
        "cc", "Scroll Delta This Frame", TRACE_EVENT_SCOPE_THREAD, "x", scroll_accumulated_this_frame_.x(), "y", scroll_accumulated_this_frame_.y());
...

    {
        ...
        TRACE_EVENT_OBJECT_SNAPSHOT_WITH_ID(frame_viewer_instrumentation::CategoryLayerTree(), "cc::LayerTreeHostImpl", id_, AsValueWithFrame(frame));
    }
    ...
    if (active_tree_->hud_layer()) {
        ...
        active_tree_->hud_layer()->UpdateHudTexture(draw_mode, layer_tree_frame_sink_, resource_provider_.get(), raster_caps(), frame->render_passes);
    }
    ...
    for (RenderSurfaceImpl* render_surface : *frame->render_surface_list) {
        const auto& view_transition_element_resource_id = render_surface->OwningEffectNode()->view_transition_element_resource_id;
        if (!view_transition_element_resource_id.IsValid()) {
            continue;
        }

        DCHECK(!base::Contains(view_transition_element_map, view_transition_element_resource_id))
            << "Cannot map " << view_transition_element_resource_id.ToString() << " to render pass " << render_surface->render_pass_id().GetUnsafeValue()
            << "; It already maps to render pass " << view_transition_element_map[view_transition_element_resource_id].GetUnsafeValue();

        view_transition_element_map[view_transition_element_resource_id] = render_surface->render_pass_id();
    }

    auto display_color_spaces = GetDisplayColorSpaces();
    for (auto& request : active_tree_->TakeViewTransitionRequests()) {
        metadata.transition_directives.push_back(request->ConstructDirective(view_transition_element_map, display_color_spaces));
    }
    ...
    metadata.deadline = viz::FrameDeadline(
        CurrentBeginFrameArgs().frame_time, frame->deadline_in_frames.value_or(0u), CurrentBeginFrameArgs().interval, frame->use_default_lower_bound_deadline);
    metadata.frame_interval_inputs.frame_time = CurrentBeginFrameArgs().frame_time;
    metadata.frame_interval_inputs.has_input = frame_rate_estimator_.input_priority_mode();

    if (!frame->video_layer_preferred_intervals.empty() && frame->damage_reasons.Has(DamageReason::kVideoLayer)) {
        for (auto& [video_interval, count] : frame->video_layer_preferred_intervals) {
            metadata.frame_interval_inputs.content_interval_info.push_back({ viz::ContentFrameIntervalType::kVideo, video_interval, count - 1u });
        }
        frame->damage_reasons.Remove(DamageReason::kVideoLayer);
    }

    if (frame->damage_reasons.Has(DamageReason::kAnimatedImage)) {
        std::optional<ImageAnimationController::ConsistentFrameDuration> animating_image_duration
        ...
        if (animating_image_duration) {
            metadata.frame_interval_inputs.content_interval_info.push_back(
                { viz::ContentFrameIntervalType::kAnimatingImage, animating_image_duration->frame_duration, animating_image_duration->num_images - 1u });
    ...
    if (frame->damage_reasons.Has(DamageReason::kScrollbarFadeOutAnimation)) {
        ...
        metadata.frame_interval_inputs.content_interval_info.push_back({ viz::ContentFrameIntervalType::kScrollBarFadeOutAnimation, base::Hertz(20) });
    ...
    metadata.frame_interval_inputs.has_only_content_frame_interval_updates = frame->damage_reasons.empty();

    base::TimeDelta preferred_frame_interval;
    constexpr auto kFudgeDelta = base::Milliseconds(1);
    constexpr auto kTwiceOfDefaultInterval = viz::BeginFrameArgs::DefaultInterval() * 2;
    constexpr auto kMinDelta = kTwiceOfDefaultInterval - kFudgeDelta;
    if (mutator_host_->MainThreadAnimationsCount() == 0 && !mutator_host_->HasSmilAnimation() && mutator_host_->NeedsTickAnimations()
        && !frame_rate_estimator_.input_priority_mode() && mutator_host_->MinimumTickInterval() > kMinDelta) {
    ...
    if (render_frame_metadata_observer_) {
        ...
        if (gfx::DelegatedInkMetadata* ink_metadata = metadata.delegated_ink_metadata.get()) {
            last_draw_render_frame_metadata_->delegated_ink_metadata = DelegatedInkBrowserMetadata(ink_metadata->is_hovering());
        }
        ...
        if (last_draw_render_frame_metadata_->new_vertical_scroll_direction != viz::VerticalScrollDirection::kNull) {
            last_vertical_scroll_direction_ = last_draw_render_frame_metadata_->new_vertical_scroll_direction;
        }

        render_frame_metadata_observer_->OnRenderFrameSubmission(*last_draw_render_frame_metadata_, &metadata, active_tree()->TakeForceSendMetadataRequest());
    }

    if (!CommitsToActiveTree() && !metadata.latency_info.empty()) {
        base::TimeTicks draw_time = base::TimeTicks::Now();

        ApplyFirstScrollTracking(metadata.latency_info.front(), metadata.frame_token);
        for (auto& latency : metadata.latency_info) {
            latency.AddLatencyNumberWithTimestamp(ui::INPUT_EVENT_LATENCY_RENDERER_SWAP_COMPONENT, draw_time);
        }
    }
    ...
    if (!settings_.is_display_tree) {
        resource_provider_->PrepareSendToParent(resources, &compositor_frame.resource_list, layer_tree_frame_sink_->context_provider());
    }
    ...
    // matter.
    CHECK(!settings_.single_thread_proxy_scheduler || active_tree()->local_surface_id_from_parent().is_valid());

    if (settings_.is_display_tree) {
        UpdateChildLocalSurfaceId();
    }
    layer_tree_frame_sink_->SetLocalSurfaceId(child_local_surface_id_allocator_.GetCurrentLocalSurfaceId());
    ...
    if (const char* client_name = GetClientNameForMetrics()) {
        ...
        UMA_HISTOGRAM_COUNTS_1000(base::StringPrintf("Compositing.%s.CompositorFrame.Quads", client_name), total_quad_count);
    }
...
void LayerTreeHostImpl::UpdateDisplayTree(FrameData& frame)
{
    ...
    tile_manager_.PrepareToDraw();
    layer_context_->UpdateDisplayTreeFrom(*active_tree(), *resource_provider(), *layer_tree_frame_sink_->context_provider());
...
int LayerTreeHostImpl::RequestedMSAASampleCount() const
{
    if (settings_.gpu_rasterization_msaa_sample_count == -1) {
        ...
        float device_scale_factor = pending_tree_ ? pending_tree_->device_scale_factor() : active_tree_->device_scale_factor();
        ...
        if (base::FeatureList::IsEnabled(features::kDetectHiDpiForMsaa)) {
            float painted_device_scale_factor = pending_tree_ ? pending_tree_->painted_device_scale_factor() : active_tree_->painted_device_scale_factor();
...
void LayerTreeHostImpl::UpdateRasterCapabilities()
{
    ...
    auto* worker_context_provider = layer_tree_frame_sink_->worker_context_provider();
    ...
    if (!worker_context_provider) {
        ...
        raster_caps_.ui_rgba_format
            = layer_tree_frame_sink_->shared_image_interface() ? viz::SinglePlaneFormat::kBGRA_8888 : viz::SinglePlaneFormat::kRGBA_8888;
        return;
    }

    viz::RasterContextProvider::ScopedRasterContextLock scoped_lock(worker_context_provider);
    const auto& context_caps = worker_context_provider->ContextCapabilities();
    const auto& shared_image_caps = worker_context_provider->SharedImageInterface()->GetCapabilities();

    raster_caps_.max_texture_size = context_caps.max_texture_size;
    raster_caps_.ui_rgba_format = viz::PlatformColor::BestSupportedTextureFormat(context_caps);

    raster_caps_.tile_overlay_candidate = settings_.use_gpu_memory_buffer_resources && shared_image_caps.supports_scanout_shared_images;
    ...
    if (settings_.gpu_rasterization_disabled || !context_caps.gpu_rasterization) {
        ...
        raster_caps_.tile_format = settings_.use_rgba_4444 ? viz::SinglePlaneFormat::kRGBA_4444 : viz::PlatformColor::BestSupportedTextureFormat(context_caps);
        return;
    }
    ...
    raster_caps_.can_use_msaa = !context_caps.msaa_is_slow && !context_caps.avoid_stencil_buffers;

    raster_caps_.tile_format = settings_.use_rgba_4444 ? viz::SinglePlaneFormat::kRGBA_4444 : viz::PlatformColor::BestSupportedRenderBufferFormat(context_caps);
}

ImageDecodeCache* LayerTreeHostImpl::GetImageDecodeCache() const
{
    return image_decode_cache_holder_ ? image_decode_cache_holder_->image_decode_cache() : nullptr;
}

void LayerTreeHostImpl::RegisterMainThreadPresentationTimeCallbackForTesting(uint32_t frame_token, PresentationTimeCallbackBuffer::Callback callback)
{
    std::vector<PresentationTimeCallbackBuffer::Callback> as_vector;
    as_vector.push_back(std::move(callback));
    presentation_time_callbacks_.RegisterMainThreadCallbacks(frame_token, std::move(as_vector));
}

void LayerTreeHostImpl::RegisterMainThreadSuccessfulPresentationTimeCallbackForTesting(
    uint32_t frame_token, PresentationTimeCallbackBuffer::SuccessfulCallbackWithDetails callback)
{
    std::vector<PresentationTimeCallbackBuffer::SuccessfulCallbackWithDetails> as_vector;
    as_vector.push_back(std::move(callback));
    presentation_time_callbacks_.RegisterMainThreadSuccessfulCallbacks(frame_token, std::move(as_vector));
}

void LayerTreeHostImpl::RegisterCompositorThreadSuccessfulPresentationTimeCallbackForTesting(
    uint32_t frame_token, PresentationTimeCallbackBuffer::SuccessfulCallback callback)
{
    std::vector<PresentationTimeCallbackBuffer::SuccessfulCallback> as_vector;
    as_vector.push_back(std::move(callback));
    presentation_time_callbacks_.RegisterCompositorThreadSuccessfulCallbacks(frame_token, std::move(as_vector));
}

bool LayerTreeHostImpl::WillBeginImplFrame(const viz::BeginFrameArgs& args)
{
    if (!settings().single_thread_proxy_scheduler) {
        client_->SetWaitingForScrollEvent(input_delegate_ && input_delegate_->IsCurrentlyScrolling() && !input_delegate_->HasQueuedInput());
    }
    impl_thread_phase_ = ImplThreadPhase::INSIDE_IMPL_FRAME;
    current_begin_frame_tracker_.Start(args);
    frame_trackers_.NotifyBeginImplFrame(args);
    total_frame_counter_.OnBeginFrame(args);
    compositor_frame_reporting_controller_->SetNeedsRasterPropertiesAnimated(paint_worklet_tracker_.HasInputPropertiesAnimatedOnImpl());
    devtools_instrumentation::DidBeginFrame(id_, args.frame_time, args.frame_id.sequence_number);
    ...
    if (!settings_.using_synchronous_renderer_compositor) {
        const viz::LocalSurfaceId& upcoming_lsid
            = pending_tree() ? pending_tree()->local_surface_id_from_parent() : active_tree()->local_surface_id_from_parent();
    ...
    bool recent_frame_had_no_damage = consecutive_frame_with_damage_count_ < settings_.damaged_frame_limit;
    ...
    if (settings_.enable_early_damage_check && recent_frame_had_no_damage && CanDraw()) {
        bool ok = active_tree()->UpdateDrawProperties(
            /*update_tiles=*/true, /*update_image_animation_controller=*/true);
        DCHECK(ok);
        DamageTracker::UpdateDamageTracking(active_tree_.get());
...
void LayerTreeHostImpl::DidNotProduceFrame(const viz::BeginFrameAck& ack, FrameSkippedReason reason)
{
    frame_rate_estimator_.DidNotProduceFrame();
    if (layer_tree_frame_sink_) {
        static const bool feature_allowed = base::FeatureList::IsEnabled(features::kThrottleFrameRateOnManyDidNotProduceFrame);
        if (feature_allowed) {
            viz::BeginFrameAck adjust_ack = ack;
            adjust_ack.preferred_frame_interval = frame_rate_estimator_.GetPreferredInterval();
...
{
    uint32_t flags = viz::HitTestRegionFlags::kHitTestMouse | viz::HitTestRegionFlags::kHitTestTouch;
...
static void PopulateHitTestRegion(viz::HitTestRegion* hit_test_region, const LayerImpl* layer, uint32_t flags, uint32_t async_hit_test_reasons,
    const gfx::Rect& rect, const viz::SurfaceId& surface_id, float device_scale_factor)
{
    ...
    DCHECK_EQ(!!async_hit_test_reasons, !!(flags & viz::HitTestRegionFlags::kHitTestAsk));
    ...
    surface_to_root_transform.Scale(SK_Scalar1 / device_scale_factor, SK_Scalar1 / device_scale_factor);
...
std::optional<viz::HitTestRegionList> LayerTreeHostImpl::BuildHitTestData()
{
    ...
    hit_test_region_list->flags = viz::HitTestRegionFlags::kHitTestMine | viz::HitTestRegionFlags::kHitTestMouse | viz::HitTestRegionFlags::kHitTestTouch;
    ...
    for (const auto* layer : base::Reversed(*active_tree())) {
        if (layer->is_surface_layer()) {
            ...
            if (!layer->HitTestable() && !layer->draws_content() && !surface_layer->has_pointer_events_none()) {
                continue;
            }
            ...
            if (!surface_layer->surface_hit_testable() || !surface_layer->range().IsValid()) {
                ...
                if (!surface_layer->has_pointer_events_none() && !assume_overlap) {
                    overlapping_region.Union(MathUtil::MapEnclosingClippedRect(layer->ScreenSpaceTransform(), gfx::Rect(surface_layer->bounds())));
                }
                continue;
            }

            gfx::Rect content_rect(gfx::ScaleToEnclosingRect(gfx::Rect(surface_layer->bounds()), device_scale_factor));

            gfx::Rect layer_screen_space_rect = MathUtil::MapEnclosingClippedRect(surface_layer->ScreenSpaceTransform(), gfx::Rect(surface_layer->bounds()));
            auto flag = GetFlagsForSurfaceLayer(surface_layer);
            uint32_t async_hit_test_reasons = viz::AsyncHitTestReasons::kNotAsyncHitTest;
            if (surface_layer->has_pointer_events_none())
                flag |= viz::HitTestRegionFlags::kHitTestIgnore;
            if (assume_overlap || overlapping_region.Intersects(layer_screen_space_rect)) {
            ...
            bool layer_hit_test_region_is_masked
                = active_tree()->property_trees()->effect_tree().HitTestMayBeAffectedByMask(surface_layer->effect_tree_index());
            if (surface_layer->is_clipped() || layer_hit_test_region_is_masked) {
                bool layer_hit_test_region_is_rectangle = !layer_hit_test_region_is_masked && surface_layer->ScreenSpaceTransform().Preserves2dAxisAlignment()
                    && active_tree()->property_trees()->effect_tree().ClippedHitTestRegionIsRectangle(surface_layer->effect_tree_index());
                content_rect = gfx::ScaleToEnclosingRect(surface_layer->visible_layer_rect(), device_scale_factor, device_scale_factor);
            ...
            PopulateHitTestRegion(&hit_test_region_list->regions.back(), layer, flag, async_hit_test_reasons, content_rect, surface_id, device_scale_factor);
            continue;
        }
        ...
        if (num_iterated_layers > kAssumeOverlapThreshold)
            assume_overlap = true;
        if (!assume_overlap) {
            overlapping_region.Union(MathUtil::MapEnclosingClippedRect(layer->ScreenSpaceTransform(), gfx::Rect(layer->bounds())));
        }
    }
...
bool LayerTreeHostImpl::ScrollAffectsScrollHandler() const
{
    if (!input_delegate_)
        return false;
    return settings_.enable_synchronized_scrolling && scroll_affects_scroll_handler_;
}
...
void LayerTreeHostImpl::CreatePendingTree()
{
    ...
    if (recycle_tree_) {
        ...
        pending_tree_ = std::make_unique<LayerTreeImpl>(*this, CurrentBeginFrameArgs(), active_tree()->page_scale_factor(),
            active_tree()->top_controls_shown_ratio(), active_tree()->bottom_controls_shown_ratio(), active_tree()->elastic_overscroll());
    }
    ...
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN1(
        "cc", "PendingTree:waiting", TRACE_ID_LOCAL(pending_tree_.get()), "active_lsid", active_tree()->local_surface_id_from_parent().ToString());
}
...
void LayerTreeHostImpl::PushScrollbarOpacitiesFromActiveToPending()
{
    ...
    for (auto& pair : scrollbar_animation_controllers_) {
        for (auto* scrollbar : pair.second->Scrollbars()) {
            if (const EffectNode* source_effect_node = active_tree()->property_trees()->effect_tree().FindNodeFromElementId(scrollbar->element_id())) {
                if (EffectNode* target_effect_node = pending_tree()->property_trees()->effect_tree_mutable().FindNodeFromElementId(scrollbar->element_id())) {
                    ...
                    pending_tree()->property_trees()->effect_tree_mutable().set_needs_update(true);
                }
            }
        }
    }
...
{
//     TRACE_EVENT("cc,benchmark", "LayerTreeHostImpl::ActivateSyncTree", [&](perfetto::EventContext ctx) {
//         EmitMainFramePipelineStep(ctx, sync_tree()->trace_id(), perfetto::protos::pbzero::MainFramePipeline::Step::ACTIVATE);
//     });
    if (pending_tree_) {
        TRACE_EVENT_NESTABLE_ASYNC_END1(
            "cc", "PendingTree:waiting", TRACE_ID_LOCAL(pending_tree_.get()), "pending_lsid", pending_tree_->local_surface_id_from_parent().ToString());
        ...
        if (pending_tree_->needs_full_tree_sync()) {
            TreeSynchronizer::SynchronizeTrees(pending_tree_.get(), active_tree_.get());
        ...
        active_tree_->lifecycle().AdvanceTo(LayerTreeLifecycle::kSyncedPropertyTrees);

        TreeSynchronizer::PushLayerProperties(pending_tree(), active_tree());

        active_tree_->lifecycle().AdvanceTo(LayerTreeLifecycle::kSyncedLayerProperties);
    ...
    if (InnerViewportScrollNode()) {
        active_tree_->property_trees()->scroll_tree_mutable().ClampScrollToMaxScrollOffset(*InnerViewportScrollNode(), active_tree_.get());

        DCHECK(OuterViewportScrollNode());
        active_tree_->property_trees()->scroll_tree_mutable().ClampScrollToMaxScrollOffset(*OuterViewportScrollNode(), active_tree_.get());
    }
    ...
    if (child_local_surface_id_allocator_.GetCurrentLocalSurfaceId().is_valid()) {
        ...
        LOG(ERROR) << "Cannot issue a copy because the previous surface is invalid.";
    }
    ...
    if (pending_page_scale_animation) {
        StartPageScaleAnimation(pending_page_scale_animation->target_offset, pending_page_scale_animation->use_anchor, pending_page_scale_animation->scale,
...
void LayerTreeHostImpl::OnMemoryPressure(base::MemoryPressureListener::MemoryPressureLevel level)
{
    if (settings_.is_display_tree) {
        return;
    }
...
ManagedMemoryPolicy LayerTreeHostImpl::ActualManagedMemoryPolicy() const
{
    ...
    } else if (use_gpu_rasterization() && actual.priority_cutoff_when_visible == gpu::MemoryAllocation::CUTOFF_ALLOW_EVERYTHING) {
...
void LayerTreeHostImpl::CreateTileManagerResources()
{
    ...
    image_decode_cache_holder_ = std::make_unique<ImageDecodeCacheHolder>(settings_.enable_shared_image_cache_for_gpu, raster_caps(), gpu_compositing,
        layer_tree_frame_sink_->worker_context_provider_wrapper(), settings_.decoded_image_working_set_budget_bytes, dark_mode_filter_);

    if (raster_caps().use_gpu_rasterization) {
        pending_raster_queries_ = std::make_unique<RasterQueryQueue>(layer_tree_frame_sink_->worker_context_provider());
    }
    ...
    if (is_synchronous_single_threaded_) {
        DCHECK(!single_thread_synchronous_task_graph_runner_);
        single_thread_synchronous_task_graph_runner_ = std::make_unique<SynchronousTaskGraphRunner>();
        task_graph_runner = single_thread_synchronous_task_graph_runner_.get();
    }

    tile_manager_.SetResources(resource_pool_.get(), GetImageDecodeCache(), task_graph_runner, raster_buffer_provider_.get(),
        raster_caps().use_gpu_rasterization, pending_raster_queries_.get());
    tile_manager_.SetCheckerImagingForceDisabled(settings_.only_checker_images_with_gpu_raster && !raster_caps().use_gpu_rasterization);
    UpdateTileManagerMemoryPolicy(ActualManagedMemoryPolicy());
}

std::unique_ptr<RasterBufferProvider> LayerTreeHostImpl::CreateRasterBufferProvider()
{
    DCHECK(!settings_.is_display_tree);
    DCHECK(GetTaskRunner());
    viz::RasterContextProvider* compositor_context_provider = layer_tree_frame_sink_->context_provider();

    if (!compositor_context_provider) {
        return std::make_unique<BitmapRasterBufferProvider>(layer_tree_frame_sink_);
    }

    const gpu::Capabilities& caps = compositor_context_provider->ContextCapabilities();
    viz::RasterContextProvider* worker_context_provider = layer_tree_frame_sink_->worker_context_provider();

    if (raster_caps().use_gpu_rasterization) {
        DCHECK(worker_context_provider);

        return std::make_unique<GpuRasterBufferProvider>(compositor_context_provider, worker_context_provider, raster_caps_, settings_.max_gpu_raster_tile_size,
            settings_.unpremultiply_and_dither_low_bit_depth_tiles, pending_raster_queries_.get());
    }
    ...
    if (!use_zero_copy && !worker_context_provider) {
        LOG(ERROR) << "Forcing zero-copy tile initialization as worker context is missing";
        use_zero_copy = true;
    }

    if (use_zero_copy) {
        return std::make_unique<ZeroCopyRasterBufferProvider>(compositor_context_provider, raster_caps_);
    }

    const int max_copy_texture_chromium_size = caps.max_copy_texture_chromium_size;
    return std::make_unique<OneCopyRasterBufferProvider>(GetTaskRunner(), compositor_context_provider, worker_context_provider, max_copy_texture_chromium_size,
        settings_.use_partial_raster, settings_.max_staging_buffer_usage_in_bytes, raster_caps_);
}

void LayerTreeHostImpl::SetLayerTreeMutator(std::unique_ptr<LayerTreeMutator> mutator)
{
    mutator_host_->SetLayerTreeMutator(std::move(mutator));
}

void LayerTreeHostImpl::SetPaintWorkletLayerPainter(std::unique_ptr<PaintWorkletLayerPainter> painter)
{
    paint_worklet_painter_ = std::move(painter);
}

void LayerTreeHostImpl::QueueImageDecode(int request_id, const PaintImage& image)
{
    DCHECK(!settings_.is_display_tree);
    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "LayerTreeHostImpl::QueueImageDecode", "frame_key",
    ...
    tile_manager_.decoded_image_tracker().QueueImageDecode(
        image, GetTargetColorParams(content_color_usage), base::BindOnce(&LayerTreeHostImpl::ImageDecodeFinished, weak_factory_.GetWeakPtr(), request_id));
    tile_manager_.checker_image_tracker().DisallowCheckeringForImage(image);
}

void LayerTreeHostImpl::ImageDecodeFinished(int request_id, bool decode_succeeded)
{
    DCHECK(!settings_.is_display_tree);
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "LayerTreeHostImpl::ImageDecodeFinished");
    if (!base::FeatureList::IsEnabled(features::kSendExplicitDecodeRequestsImmediately)) {
...
std::vector<std::pair<int, bool>> LayerTreeHostImpl::TakeCompletedImageDecodeRequests()
{
...
void LayerTreeHostImpl::CleanUpTileManagerResources()
{
    ...
    if (layer_tree_frame_sink_) {
        if (auto* worker_context = layer_tree_frame_sink_->worker_context_provider()) {
...
bool LayerTreeHostImpl::InitializeFrameSink(LayerTreeFrameSink* layer_tree_frame_sink)
{
    ...
    if (!settings_.is_display_tree) {
        resource_pool_ = std::make_unique<ResourcePool>(resource_provider_.get(), layer_tree_frame_sink_->context_provider(), GetTaskRunner(),
            ResourcePool::kDefaultExpirationDelay, settings_.disallow_non_exact_resource_reuse);
...
void LayerTreeHostImpl::DidObserveScrollDelay(int source_frame_number, base::TimeDelta scroll_delay, base::TimeTicks scroll_timestamp)
{
    // Record First Scroll Delay.
    if (!has_observed_first_scroll_delay_) {
        client_->DidObserveFirstScrollDelay(source_frame_number, scroll_delay, scroll_timestamp);
...
void LayerTreeHostImpl::SetCurrentBrowserControlsShownRatio(float top_ratio, float bottom_ratio)
{
    if (active_tree_->SetCurrentBrowserControlsShownRatio(top_ratio, bottom_ratio))
...
void LayerTreeHostImpl::AutoScrollAnimationCreate(const ScrollNode& scroll_node, const gfx::PointF& target_offset, float autoscroll_velocity)
{
    ...
    gfx::PointF current_offset = scroll_tree.current_scroll_offset(scroll_node.element_id);

    mutator_host_->ImplOnlyAutoScrollAnimationCreate(scroll_node.element_id, target_offset, current_offset, autoscroll_velocity, animation_start_offset);

    SetNeedsOneBeginImplFrame();
}

bool LayerTreeHostImpl::ScrollAnimationCreate(const ScrollNode& scroll_node, const gfx::Vector2dF& delta, base::TimeDelta delayed_by)
{
    ScrollTree& scroll_tree = active_tree_->property_trees()->scroll_tree_mutable();
    ...
    if (!scroll_animated) {
        ...
        TRACE_EVENT_INSTANT0("cc", "no scroll animation due to small delta", TRACE_EVENT_SCOPE_THREAD);
        return false;
    }

    gfx::PointF current_offset = scroll_tree.current_scroll_offset(scroll_node.element_id);
    gfx::PointF target_offset = scroll_tree.ClampScrollOffsetToLimits(current_offset + delta, scroll_node);
    ...
    mutator_host_->ImplOnlyScrollAnimationCreate(scroll_node.element_id, target_offset, current_offset, delayed_by, animation_start_offset);
...
void LayerTreeHostImpl::UpdateImageDecodingHints(base::flat_map<PaintImage::Id, PaintImage::DecodingMode> decoding_mode_map)
{
    tile_manager_.checker_image_tracker().UpdateImageDecodingHints(std::move(decoding_mode_map));
}

void LayerTreeHostImpl::SetRenderFrameObserver(std::unique_ptr<RenderFrameMetadataObserver> observer)
{
...
void LayerTreeHostImpl::WillScrollContent(ElementId element_id)
{
    ...
    if (settings().scrollbar_flash_after_any_scroll_update) {
        ...
        if (ScrollbarAnimationController* animation_controller = ScrollbarAnimationControllerForElementId(element_id))
...
void LayerTreeHostImpl::BindToInputHandler(std::unique_ptr<InputDelegateForCompositor> delegate)
{
...
void LayerTreeHostImpl::SetVisualDeviceViewportSize(const gfx::Size& visual_device_viewport_size)
{
...
void LayerTreeHostImpl::SetMayThrottleIfUndrawnFrames(bool may_throttle_if_undrawn_frames)
{
...
void LayerTreeHostImpl::UpdateChildLocalSurfaceId()
{
    ...
    if (evicted_local_surface_id_.is_valid()
        && child_local_surface_id_allocator_.GetCurrentLocalSurfaceId().IsNewerThanOrEmbeddingChanged(evicted_local_surface_id_)) {
...
void LayerTreeHostImpl::CollectScrollbarUpdatesForCommit(CompositorCommitData* commit_data) const
{
    commit_data->scrollbars.reserve(scrollbar_animation_controllers_.size());
    for (auto& pair : scrollbar_animation_controllers_) {
        if (pair.second->visibility_changed()) {
            commit_data->scrollbars.push_back({ pair.first, pair.second->ScrollbarsHidden() });
            pair.second->ClearVisibilityChanged();
        }
    }
}

std::unique_ptr<CompositorCommitData> LayerTreeHostImpl::ProcessCompositorDeltas(const MutatorHost* main_thread_mutator_host)
{
    auto commit_data = std::make_unique<CompositorCommitData>();

    if (input_delegate_) {
        input_delegate_->ProcessCommitDeltas(commit_data.get(), main_thread_mutator_host);
    }
    CollectScrollbarUpdatesForCommit(commit_data.get());

    commit_data->page_scale_delta = active_tree_->page_scale_factor()->PullDeltaForMainThread(main_thread_mutator_host);
    commit_data->is_pinch_gesture_active = active_tree_->PinchGestureActive();
    commit_data->is_scroll_active = input_delegate_ && GetInputHandler().IsCurrentlyScrolling();
    ...
    DCHECK(settings().is_for_scalable_page || commit_data->page_scale_delta == 1.f);
    commit_data->top_controls_delta = active_tree()->top_controls_shown_ratio()->PullDeltaForMainThread(main_thread_mutator_host);
    commit_data->bottom_controls_delta = active_tree()->bottom_controls_shown_ratio()->PullDeltaForMainThread(main_thread_mutator_host);
    commit_data->elastic_overscroll_delta = active_tree_->elastic_overscroll()->PullDeltaForMainThread(main_thread_mutator_host);
    commit_data->swap_promises.swap(swap_promises_for_main_thread_scroll_update_);

    commit_data->ongoing_scroll_animation = mutator_host_->HasImplOnlyScrollAnimatingElement();
    commit_data->is_auto_scrolling = mutator_host_->HasImplOnlyAutoScrollAnimatingElement();

    if (browser_controls_manager()) {
        commit_data->browser_controls_constraint = browser_controls_manager()->PullConstraintForMainThread(&commit_data->browser_controls_constraint_changed);
    }
...
void LayerTreeHostImpl::SetFullViewportDamage()
{
    ...
    SetViewportDamage(gfx::Rect(active_tree_->internal_device_viewport().origin(), active_tree_->GetDeviceViewport().size()));
}
...
bool LayerTreeHostImpl::AnimatePageScale(base::TimeTicks monotonic_time)
{
    ...
    active_tree_->SetPageScaleOnActiveTree(page_scale_animation_->PageScaleFactorAtTime(monotonic_time));
    gfx::PointF next_scroll = page_scale_animation_->ScrollOffsetAtTime(monotonic_time);
...
bool LayerTreeHostImpl::AnimateBrowserControls(base::TimeTicks time)
{
    ...
    if (active_tree_->TotalScrollOffset().y() == 0.f || OnlyExpandTopControlsAtPageTop()) {
        return false;
    }
...
bool LayerTreeHostImpl::AnimateLayers(base::TimeTicks monotonic_time, bool is_active_tree)
{
    const ScrollTree& scroll_tree = is_active_tree ? active_tree_->property_trees()->scroll_tree() : pending_tree_->property_trees()->scroll_tree();
    const bool animated = mutator_host_->TickAnimations(monotonic_time, scroll_tree, is_active_tree);
    ...
    if (animated) {
        ...
        SetNeedsOneBeginImplFrame();
        frame_trackers_.StartSequence(FrameSequenceTrackerType::kCompositorAnimation);
        if (mutator_host_->HasInvalidationAnimation()) {
            frame_trackers_.StartSequence(FrameSequenceTrackerType::kCompositorRasterAnimation);
        } else {
            frame_trackers_.StopSequence(FrameSequenceTrackerType::kCompositorRasterAnimation);
        }
        if (mutator_host_->HasNativePropertyAnimation()) {
            frame_trackers_.StartSequence(FrameSequenceTrackerType::kCompositorNativeAnimation);
        } else {
            frame_trackers_.StopSequence(FrameSequenceTrackerType::kCompositorNativeAnimation);
        }
    } else {
        frame_trackers_.StopSequence(FrameSequenceTrackerType::kCompositorAnimation);
        frame_trackers_.StopSequence(FrameSequenceTrackerType::kCompositorRasterAnimation);
        frame_trackers_.StopSequence(FrameSequenceTrackerType::kCompositorNativeAnimation);
    }

    if (animated && mutator_host_->HasViewTransition()) {
        frame_trackers_.StartSequence(FrameSequenceTrackerType::kSETCompositorAnimation);
    } else {
        frame_trackers_.StopSequence(FrameSequenceTrackerType::kSETCompositorAnimation);
    }
...
{
    const bool has_active_animations = mutator_host_->UpdateAnimationState(start_ready_animations, mutator_events_.get());
...
{
    const bool activated = mutator_host_->ActivateAnimations(mutator_events_.get());
    if (activated) {
...
void LayerTreeHostImpl::RegisterScrollbarAnimationController(ElementId scroll_element_id, float scrollbar_opacity)
{
    if (ScrollbarAnimationControllerForElementId(scroll_element_id))
        return;

    scrollbar_animation_controllers_[scroll_element_id] = active_tree_->CreateScrollbarAnimationController(scroll_element_id, scrollbar_opacity);
}

void LayerTreeHostImpl::DidRegisterScrollbarLayer(ElementId scroll_element_id, ScrollbarOrientation orientation)
{
    if (input_delegate_)
        input_delegate_->DidRegisterScrollbar(scroll_element_id, orientation);
}

void LayerTreeHostImpl::DidUnregisterScrollbarLayer(ElementId scroll_element_id, ScrollbarOrientation orientation)
{
...
ScrollbarAnimationController* LayerTreeHostImpl::ScrollbarAnimationControllerForElementId(ElementId scroll_element_id) const
{
    ...
    if (InnerViewportScrollNode()) {
        DCHECK(OuterViewportScrollNode());
        if (scroll_element_id == InnerViewportScrollNode()->element_id || scroll_element_id == OuterViewportScrollNode()->element_id) {
            auto itr = scrollbar_animation_controllers_.find(InnerViewportScrollNode()->element_id);
            if (itr != scrollbar_animation_controllers_.end())
                return itr->second.get();

            itr = scrollbar_animation_controllers_.find(OuterViewportScrollNode()->element_id);
            if (itr != scrollbar_animation_controllers_.end())
                return itr->second.get();
...
void LayerTreeHostImpl::PostDelayedScrollbarAnimationTask(base::OnceClosure task, base::TimeDelta delay)
{
...
void LayerTreeHostImpl::AddVideoFrameController(VideoFrameController* controller)
{
...
void LayerTreeHostImpl::RemoveVideoFrameController(VideoFrameController* controller)
{
...
std::unique_ptr<base::trace_event::ConvertableToTraceFormat> LayerTreeHostImpl::AsValueWithFrame(FrameData* frame) const
{
    std::unique_ptr<base::trace_event::TracedValue> state(new base::trace_event::TracedValue());
    AsValueWithFrameInto(frame, state.get());
    return std::move(state);
}

void LayerTreeHostImpl::AsValueWithFrameInto(FrameData* frame, base::trace_event::TracedValue* state) const
{
    ...
    MathUtil::AddToTracedValue("device_viewport_size", active_tree_->GetDeviceViewport().size(), state);
...
void LayerTreeHostImpl::ActivationStateAsValueInto(base::trace_event::TracedValue* state) const
{
...
void LayerTreeHostImpl::SetDebugState(const LayerTreeDebugState& new_debug_state)
{
...
void LayerTreeHostImpl::CreateUIResource(UIResourceId uid, const UIResourceBitmap& bitmap)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "LayerTreeHostImpl::CreateUIResource");
    DCHECK_GT(uid, 0);
    ...
    if (source_size.width() > raster_caps().max_texture_size || source_size.height() > raster_caps().max_texture_size) {
    ...
    gpu::SharedImageUsageSet shared_image_usage = gpu::SHARED_IMAGE_USAGE_DISPLAY_READ;
    ...
    bool use_shared_image_software = !!layer_tree_frame_sink_->shared_image_interface();

    if (layer_tree_frame_sink_->context_provider()) {
//         viz::RasterContextProvider* context_provider = layer_tree_frame_sink_->context_provider();
//         const auto& shared_image_caps = context_provider->SharedImageInterface()->GetCapabilities();
//         overlay_candidate = settings_.use_gpu_memory_buffer_resources && shared_image_caps.supports_scanout_shared_images
...
//         }
        *(int*)1 = 1;
        ...
        auto shared_image_mapping = sii->CreateSharedImage({ format, upload_size, color_space, shared_image_usage, "LayerTreeHostUIResource" });
    ...
    if (!scaled) {
        ...
        if (layer_tree_frame_sink_->context_provider()) {
            ...
            client_shared_image
                = sii->CreateSharedImage({ format, upload_size, color_space, shared_image_usage, "LayerTreeHostUIResource" }, bitmap.GetPixels());
            CHECK(client_shared_image);
        } else {
            DCHECK_EQ(bitmap.GetFormat(), UIResourceBitmap::RGBA8);
            SkImageInfo src_info = SkImageInfo::MakeN32Premul(gfx::SizeToSkISize(source_size));
            SkImageInfo dst_info = SkImageInfo::MakeN32Premul(gfx::SizeToSkISize(upload_size));
            sk_sp<SkSurface> surface = SkSurfaces::WrapPixels(dst_info, shared_mapping.memory(), dst_info.minRowBytes());
            surface->getCanvas()->writePixels(src_info, bitmap.GetPixels().data(), bitmap.row_bytes(), 0, 0);
        }
    } else {
        ...
        SkImageInfo info = SkImageInfo::MakeN32Premul(gfx::SizeToSkISize(source_size));
        ...
        if (layer_tree_frame_sink_->context_provider()) {
            scaled_surface = SkSurfaces::Raster(SkImageInfo::MakeN32Premul(upload_size.width(), upload_size.height()));
            CHECK(scaled_surface); // This would fail in OOM situations.
        } else {
            SkImageInfo dst_info = SkImageInfo::MakeN32Premul(gfx::SizeToSkISize(upload_size));
            scaled_surface = SkSurfaces::WrapPixels(dst_info, shared_mapping.memory(), dst_info.minRowBytes());
        ...
        if (layer_tree_frame_sink_->context_provider()) {
            ...
            auto* sii = context_provider->SharedImageInterface();
            client_shared_image
                = sii->CreateSharedImage({ format, upload_size, color_space, shared_image_usage, "LayerTreeHostUIResource" }, gfx::SkPixmapToSpan(pixmap));
            CHECK(client_shared_image);
        }
    }
    ...
    if (layer_tree_frame_sink_->context_provider()) {
        gpu::SyncToken sync_token = layer_tree_frame_sink_->context_provider()->SharedImageInterface()->GenUnverifiedSyncToken();

        GLenum texture_target = client_shared_image->GetTextureTarget();
        transferable = viz::TransferableResource::MakeGpu(
            client_shared_image, texture_target, sync_token, upload_size, format, overlay_candidate, viz::TransferableResource::ResourceSource::kUI);
    } else if (use_shared_image_software) {
        auto sii = layer_tree_frame_sink_->shared_image_interface();
        gpu::SyncToken sync_token = sii->GenVerifiedSyncToken();
        transferable = viz::TransferableResource::MakeSoftwareSharedImage(
            client_shared_image, sync_token, upload_size, format, viz::TransferableResource::ResourceSource::kUI);
    } else {
        layer_tree_frame_sink_->DidAllocateSharedBitmap(std::move(shm.region), shared_bitmap_id);
        transferable = viz::TransferableResource::MakeSoftwareSharedBitmap(
            shared_bitmap_id, gpu::SyncToken(), upload_size, format, viz::TransferableResource::ResourceSource::kUI);
    }
    transferable.color_space = color_space;
    id = resource_provider_->ImportResource(transferable,
        ...
        // invalidated.
        base::BindOnce(&LayerTreeHostImpl::OnUIResourceReleased, AsWeakPtr(), uid));
...
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "LayerTreeHostImpl::DeleteUIResource");
...
void LayerTreeHostImpl::DeleteUIResourceBacking(UIResourceData data, const gpu::SyncToken& sync_token)
{
    ...
    if (data.shared_image) {
        if (layer_tree_frame_sink_->context_provider()) {
            auto* sii = layer_tree_frame_sink_->context_provider()->SharedImageInterface();
            if (sii) {
...
void LayerTreeHostImpl::OnUIResourceReleased(UIResourceId uid, const gpu::SyncToken& sync_token, bool lost)
{
...
viz::ResourceId LayerTreeHostImpl::ResourceIdForUIResource(UIResourceId uid) const
{
...
void LayerTreeHostImpl::ScheduleMicroBenchmark(std::unique_ptr<MicroBenchmarkImpl> benchmark)
{
    //micro_benchmark_controller_.ScheduleRun(std::move(benchmark));
    *(int*)1 = 1;
}

void LayerTreeHostImpl::InsertLatencyInfoSwapPromiseMonitor(LatencyInfoSwapPromiseMonitor* monitor)
{
    latency_info_swap_promise_monitor_.insert(monitor);
}

void LayerTreeHostImpl::RemoveLatencyInfoSwapPromiseMonitor(LatencyInfoSwapPromiseMonitor* monitor)
{
...
{
    for (LatencyInfoSwapPromiseMonitor* monitor : latency_info_swap_promise_monitor_) {
...
bool LayerTreeHostImpl::IsOwnerThread() const
{
    ...
    if (task_runner_provider_->ImplThreadTaskRunner()) {
        result = task_runner_provider_->ImplThreadTaskRunner()->RunsTasksInCurrentSequence();
    } else {
        result = task_runner_provider_->MainThreadTaskRunner()->RunsTasksInCurrentSequence();
...
bool LayerTreeHostImpl::IsElementInPropertyTrees(ElementId element_id, ElementListType list_type) const
{
    if (list_type == ElementListType::ACTIVE)
        return active_tree() && active_tree()->IsElementInPropertyTree(element_id);

    return (pending_tree() && pending_tree()->IsElementInPropertyTree(element_id)) || (recycle_tree() && recycle_tree()->IsElementInPropertyTree(element_id));
}
...
void LayerTreeHostImpl::SetTreeLayerScrollOffsetMutated(ElementId element_id, LayerTreeImpl* tree, const gfx::PointF& scroll_offset)
{
    if (!tree)
        return;

    PropertyTrees* property_trees = tree->property_trees();
    DCHECK_EQ(1u, property_trees->scroll_tree().element_id_to_node_index().count(element_id));
    const ScrollNode* scroll_node = property_trees->scroll_tree().FindNodeFromElementId(element_id);
    ...
    if (!scroll_node)
        return;
    property_trees->scroll_tree_mutable().OnScrollOffsetAnimated(element_id, scroll_node->id, scroll_offset, tree);
}

void LayerTreeHostImpl::SetElementFilterMutated(ElementId element_id, ElementListType list_type, const FilterOperations& filters)
{
    if (list_type == ElementListType::ACTIVE) {
...
void LayerTreeHostImpl::OnCustomPropertyMutated(PaintWorkletInput::PropertyKey property_key, PaintWorkletInput::PropertyValue property_value)
{
    paint_worklet_tracker_.OnCustomPropertyMutated(std::move(property_key), std::move(property_value));
}

bool LayerTreeHostImpl::RunsOnCurrentThread() const
{
    // If there is no impl thread, then we assume the current thread is ok.
    return !task_runner_provider_->HasImplThread() || task_runner_provider_->IsImplThread();
}

void LayerTreeHostImpl::SetElementBackdropFilterMutated(ElementId element_id, ElementListType list_type, const FilterOperations& backdrop_filters)
{
    if (list_type == ElementListType::ACTIVE) {
...
void LayerTreeHostImpl::SetElementOpacityMutated(ElementId element_id, ElementListType list_type, float opacity)
{
    if (list_type == ElementListType::ACTIVE) {
...
void LayerTreeHostImpl::SetElementTransformMutated(ElementId element_id, ElementListType list_type, const gfx::Transform& transform)
{
    if (list_type == ElementListType::ACTIVE) {
...
void LayerTreeHostImpl::SetElementScrollOffsetMutated(ElementId element_id, ElementListType list_type, const gfx::PointF& scroll_offset)
{
    if (list_type == ElementListType::ACTIVE) {
...
void LayerTreeHostImpl::ElementIsAnimatingChanged(
    const PropertyToElementIdMap& element_id_map, ElementListType list_type, const PropertyAnimationState& mask, const PropertyAnimationState& state)
{
    ...
    if (tree && tree->property_trees()->ElementIsAnimatingChanged(element_id_map, mask, state, false))
        tree->set_needs_update_draw_properties();
}

void LayerTreeHostImpl::MaximumScaleChanged(ElementId element_id, ElementListType list_type, float maximum_scale)
{
    if (LayerTreeImpl* tree = list_type == ElementListType::ACTIVE ? active_tree() : pending_tree()) {
        tree->property_trees()->MaximumAnimationScaleChanged(element_id, maximum_scale);
    }
}
...
void LayerTreeHostImpl::NotifyAnimationWorkletStateChange(AnimationWorkletMutationState state, ElementListType tree_type)
{
    ...
    if (state != AnimationWorkletMutationState::CANCELED) {
        ...
        SetNeedsOneBeginImplFrame();
        if (state == AnimationWorkletMutationState::COMPLETED_WITH_UPDATE && tree_type == ElementListType::ACTIVE) {
            SetNeedsRedraw();
        }
    }
}
...
void LayerTreeHostImpl::SetContextVisibility(bool is_visible)
{
    ...
    if (compositor_context && is_visible != !!compositor_context_visibility_) {
        if (is_visible) {
            ...
            compositor_context->CacheController()->ClientBecameNotVisible(std::move(compositor_context_visibility_));
        }
    }
    ...
    if (worker_context && is_visible != !!worker_context_visibility_) {
        ...
        if (is_visible) {
            ...
            worker_context->CacheController()->ClientBecameNotVisible(std::move(worker_context_visibility_));
        }
    }
}
...
void LayerTreeHostImpl::InitializeUkm(std::unique_ptr<ukm::UkmRecorder> recorder)
{
    compositor_frame_reporting_controller_->InitializeUkmManager(std::move(recorder));
}
...
void LayerTreeHostImpl::SetUkmSmoothnessDestination(base::WritableSharedMemoryMapping ukm_smoothness_data)
{
...
void LayerTreeHostImpl::NotifyDidPresentCompositorFrameOnImplThread(
    uint32_t frame_token, std::vector<PresentationTimeCallbackBuffer::SuccessfulCallback> callbacks, const viz::FrameTimingDetails& details)
{
...
void LayerTreeHostImpl::RequestImplSideInvalidationForRasterInducingScroll(ElementId scroll_element_id)
{
...
void LayerTreeHostImpl::ApplyFirstScrollTracking(const ui::LatencyInfo& latency, uint32_t frame_token)
{
    base::TimeTicks creation_timestamp;

    // If `latency` isn't tracking a scroll, we don't need to do extra
    // first-scroll tracking.
    if (!latency.FindLatency(ui::INPUT_EVENT_LATENCY_FIRST_SCROLL_UPDATE_ORIGINAL_COMPONENT, &creation_timestamp)
        && !latency.FindLatency(ui::INPUT_EVENT_LATENCY_SCROLL_UPDATE_ORIGINAL_COMPONENT, &creation_timestamp)) {
        return;
    }
    ...
    callbacks.push_back(base::BindOnce(
        [](base::TimeTicks event_creation, int source_frame_number, LayerTreeHostImpl* layer_tree_host_impl, base::TimeTicks presentation_timestamp) {
            layer_tree_host_impl->DidObserveScrollDelay(source_frame_number, presentation_timestamp - event_creation, event_creation);
        },
    ...
    presentation_time_callbacks_.RegisterCompositorThreadSuccessfulCallbacks(frame_token, std::move(callbacks));
}
