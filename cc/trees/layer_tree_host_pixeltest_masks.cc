...
INSTANTIATE_TEST_SUITE_P(PixelResourceTest, LayerTreeHostMasksPixelTest, ::testing::ValuesIn(kTestCases), ::testing::PrintToStringParamName());
...
public:
    ...
    scoped_refptr<DisplayItemList> PaintContentsToDisplayList() override
    {
        ...
        display_list->push<ClipRectOp>(gfx::RectToSkRect(gfx::Rect(bounds_)), SkClipOp::kIntersect, false);
...
TEST_P(LayerTreeHostMasksPixelTest, MaskOfLayer)
{
    ...
    scoped_refptr<SolidColorLayer> green = CreateSolidColorLayerWithBorder(gfx::Rect(25, 25, 50, 50), kCSSGreen, 1, SK_ColorBLACK);
    ...
    RunPixelResourceTest(background, base::FilePath(FILE_PATH_LITERAL("mask_of_layer.png")));
}
...
protected:
    ...
    void SetupTree() override
    {
        ...
        scoped_refptr<SolidColorLayer> green = CreateSolidColorLayer(gfx::Rect(25, 25, 50, 50), kCSSGreen);
...
INSTANTIATE_TEST_SUITE_P(PixelResourceTest, LayerTreeHostMaskPixelTestWithLayerList, ::testing::ValuesIn(kTestCases), ::testing::PrintToStringParamName());
...
TEST_P(LayerTreeHostMaskPixelTestWithLayerList, MaskWithEffect)
{
    ...
    pixel_comparator_ = std::make_unique<AlphaDiscardingFuzzyPixelOffByOneComparator>();
    RunPixelResourceTestWithLayerList(base::FilePath(FILE_PATH_LITERAL("mask_with_effect.png")));
}

// This tests that a solid color empty layer with mask effect works correctly.
TEST_P(LayerTreeHostMaskPixelTestWithLayerList, SolidColorLayerEmptyMaskWithEffect)
{
    // Apply a mask that is empty and solid-color. This should result in
    // the green layer being entirely clipped out.
    mask_layer_ = CreateSolidColorLayer(gfx::Rect(25, 25, 50, 50), SK_ColorTRANSPARENT);

    RunPixelResourceTestWithLayerList(base::FilePath(FILE_PATH_LITERAL("solid_color_empty_mask_with_effect.png")));
}
...
class LayerTreeHostMaskPixelTest_SolidColorEmptyMaskWithEffectAndRenderSurface : public LayerTreeHostMaskPixelTestWithLayerList {
protected:
    void SetupTree() override
    {
        LayerTreeHostMaskPixelTestWithLayerList::SetupTree();

        auto* effect = layer_tree_host()->property_trees()->effect_tree_mutable().Node(mask_layer_->effect_tree_index());
        effect->render_surface_reason = RenderSurfaceReason::kTest;
    }
};

INSTANTIATE_TEST_SUITE_P(PixelResourceTest, LayerTreeHostMaskPixelTest_SolidColorEmptyMaskWithEffectAndRenderSurface, ::testing::ValuesIn(kTestCases),
    ::testing::PrintToStringParamName());

TEST_P(LayerTreeHostMaskPixelTest_SolidColorEmptyMaskWithEffectAndRenderSurface, Test)
{
    // Apply a mask that is empty and solid-color. This should result in
    // the green layer being entirely clipped out.
    SolidColorEmptyMaskContentLayerClient client(mask_bounds_);
...
class LayerTreeHostMaskPixelTest_MaskWithEffectNoContentToMask : public LayerTreeHostMaskPixelTestWithLayerList {
protected:
    void SetupTree() override
    {
        LayerTreeHostMaskPixelTestWithLayerList::SetupTree();
...
INSTANTIATE_TEST_SUITE_P(
    PixelResourceTest, LayerTreeHostMaskPixelTest_MaskWithEffectNoContentToMask, ::testing::ValuesIn(kTestCases), ::testing::PrintToStringParamName());
...
TEST_P(LayerTreeHostMaskPixelTest_MaskWithEffectNoContentToMask, Test)
{
    ...
    RunPixelResourceTestWithLayerList(base::FilePath(FILE_PATH_LITERAL("mask_with_effect_no_content.png")));
}

class LayerTreeHostMaskPixelTest_ScaledMaskWithEffect : public LayerTreeHostMaskPixelTestWithLayerList {
protected:
...
INSTANTIATE_TEST_SUITE_P(
    PixelResourceTest, LayerTreeHostMaskPixelTest_ScaledMaskWithEffect, ::testing::ValuesIn(kTestCases), ::testing::PrintToStringParamName());
...
TEST_P(LayerTreeHostMaskPixelTest_ScaledMaskWithEffect, Test)
{
    ...
    RunPixelResourceTestWithLayerList(base::FilePath(FILE_PATH_LITERAL("scaled_mask_with_effect_.png")).InsertBeforeExtensionASCII(GetRendererSuffix()));
}
...
TEST_P(LayerTreeHostMaskPixelTestWithLayerList, MaskWithEffectDifferentSize)
{
    ...
    RunPixelResourceTestWithLayerList(base::FilePath(FILE_PATH_LITERAL("mask_with_effect_different_size.png")));
}
...
TEST_P(LayerTreeHostMaskPixelTestWithLayerList, ImageMaskWithEffect)
{
    ...
    scoped_refptr<DisplayItemList> mask_display_list = mask_client.PaintContentsToDisplayList();
    ...
    // The mask is half the size of thing it's masking. In layer-list mode,
    // the mask is not automatically scaled to match the other layer.
    RunPixelResourceTestWithLayerList(base::FilePath(FILE_PATH_LITERAL("image_mask_with_effect.png")));
}
...
TEST_P(LayerTreeHostMasksPixelTest, ImageMaskOfLayer)
{
    ...
    RunPixelResourceTest(background, base::FilePath(FILE_PATH_LITERAL("image_mask_of_layer.png")));
}
...
TEST_P(LayerTreeHostMasksPixelTest, MaskOfClippedLayer)
{
    ...
    RunPixelResourceTest(background, base::FilePath(FILE_PATH_LITERAL("mask_of_clipped_layer.png")));
}
...
TEST_P(LayerTreeHostMasksPixelTest, MaskOfLayerNonExactTextureSize)
{
    ...
    scoped_refptr<SolidColorLayer> green = CreateSolidColorLayerWithBorder(gfx::Rect(0, 0, 100, 100), kCSSGreen, 1, SK_ColorBLACK);
    background->AddChild(green);
    ...
    RunPixelResourceTest(background, base::FilePath(FILE_PATH_LITERAL("mask_with_non_exact_texture_size.png")));
}

class CheckerContentLayerClient : public ContentLayerClient {
public:
    CheckerContentLayerClient(const gfx::Size& bounds, SkColor color, bool vertical)
        : bounds_(bounds)
        , color_(color)
        , vertical_(vertical)
    {
...
public:
    ...
    scoped_refptr<DisplayItemList> PaintContentsToDisplayList() override
    {
        ...
        display_list->push<DrawOvalOp>(SkRect::MakeLTRB(circle_x - radius, circle_y - radius, circle_x + radius, circle_y + radius), flags);
        display_list->push<RestoreOp>();
        display_list->EndPaintOfUnpaired(gfx::Rect(bounds_));
        display_list->Finalize();
        return display_list;
...
class LayerTreeHostMasksForBackdropFiltersPixelTestWithLayerList : public ParameterizedPixelResourceTest {
protected:
    ...
    void SetupTree() override
    {
        ...
        scoped_refptr<SolidColorLayer> background = CreateSolidColorLayer(gfx::Rect(bounds_), SK_ColorWHITE);
        CopyProperties(root, background.get());
        root->AddChild(background);

        scoped_refptr<PictureLayer> picture = PictureLayer::Create(&picture_client_);
        picture->SetBounds(bounds_);
        picture->SetIsDrawable(true);
        CopyProperties(background.get(), picture.get());
        root->AddChild(picture);

        scoped_refptr<SolidColorLayer> blur = CreateSolidColorLayer(gfx::Rect(bounds_), SK_ColorTRANSPARENT);
        CopyProperties(background.get(), blur.get());
        CreateEffectNode(blur.get()).backdrop_filters.Append(FilterOperation::CreateGrayscaleFilter(1.0));
        root->AddChild(blur);
...
INSTANTIATE_TEST_SUITE_P(
    PixelResourceTest, LayerTreeHostMasksForBackdropFiltersPixelTestWithLayerList, ::testing::ValuesIn(kTestCases), ::testing::PrintToStringParamName());

TEST_P(LayerTreeHostMasksForBackdropFiltersPixelTestWithLayerList, Test)
{
    base::FilePath image_name = (use_accelerated_raster()) ? base::FilePath(FILE_PATH_LITERAL("mask_of_backdrop_filter_gpu.png"))
...
using LayerTreeHostMasksForBackdropFiltersPixelTestWithLayerTree = ParameterizedPixelResourceTest;

INSTANTIATE_TEST_SUITE_P(
    PixelResourceTest, LayerTreeHostMasksForBackdropFiltersPixelTestWithLayerTree, ::testing::ValuesIn(kTestCases), ::testing::PrintToStringParamName());
...
TEST_P(LayerTreeHostMasksForBackdropFiltersPixelTestWithLayerTree, Test)
{
    ...
    scoped_refptr<SolidColorLayer> blur = CreateSolidColorLayer(gfx::Rect(100, 100), SK_ColorTRANSPARENT);
...
{
    scoped_refptr<SolidColorLayer> background = CreateSolidColorLayer(gfx::Rect(128, 128), SK_ColorWHITE);

    gfx::Size picture_bounds(128, 128);
    CheckerContentLayerClient picture_client_vertical(picture_bounds, SK_ColorGREEN, true);
    ...
    CheckerContentLayerClient picture_client_horizontal(picture_bounds, SK_ColorMAGENTA, false);
    ...
    RunPixelResourceTest(background, base::FilePath(FILE_PATH_LITERAL("mask_of_layer_with_blend.png")));
}

class StaticPictureLayer : private ContentLayerClient, public PictureLayer {
public:
    static scoped_refptr<StaticPictureLayer> Create(scoped_refptr<DisplayItemList> display_list)
    {
        return base::WrapRefCounted(new StaticPictureLayer(std::move(display_list)));
    }
...
class LayerTreeHostMaskAsBlendingPixelTest : public LayerTreeHostPixelResourceTest, public ::testing::WithParamInterface<MaskTestConfig> {
public:
        ...
        , force_shaders_(GetParam().flags & kForceShaders)
    {
        ...
        pixel_comparator_
            = std::make_unique<FuzzyPixelComparator>(FuzzyPixelComparator()
    ...
    static scoped_refptr<Layer> CreateCheckerboardLayer(const gfx::Size& bounds)
    {
        ...
        static const SkColor4f color_even = SkColor4f::FromColor(SkColorSetRGB(153, 153, 153));
        static const SkColor4f color_odd = SkColor4f::FromColor(SkColorSetRGB(102, 102, 102));
        ...
        for (int j = 0; j < (bounds.height() + kGridSize - 1) / kGridSize; j++) {
            for (int i = 0; i < (bounds.width() + kGridSize - 1) / kGridSize; i++) {
                ...
                if (!is_odd_grid)
                    continue;
                display_list->push<DrawRectOp>(SkRect::MakeXYWH(i * kGridSize, j * kGridSize, kGridSize, kGridSize), flags);
            }
        }
        display_list->EndPaintOfUnpaired(gfx::Rect(bounds));
        display_list->Finalize();

        scoped_refptr<Layer> layer = StaticPictureLayer::Create(std::move(display_list));
        layer->SetIsDrawable(true);
        layer->SetBounds(bounds);
        return layer;
    }

    static scoped_refptr<Layer> CreateTestPatternLayer(const gfx::Size& bounds, int grid_size)
    {
        ...
        // and 3 opaque).
        static SkColor test_colors[7] = { SkColorSetARGB(128, 255, 0, 0), SkColorSetARGB(255, 0, 0, 255), SkColorSetARGB(128, 0, 255, 0),
            SkColorSetARGB(128, 0, 0, 255), SkColorSetARGB(255, 0, 255, 0), SkColorSetARGB(0, 0, 0, 0), SkColorSetARGB(255, 255, 0, 0) };
        ...
        for (int j = 0; j < (bounds.height() + grid_size - 1) / grid_size; j++) {
            for (int i = 0; i < (bounds.width() + grid_size - 1) / grid_size; i++) {
                ...
                display_list->push<DrawRectOp>(SkRect::MakeXYWH(i * grid_size, j * grid_size, grid_size, grid_size), flags);
            }
        }
        display_list->EndPaintOfUnpaired(gfx::Rect(bounds));
        display_list->Finalize();
...
protected:
        ...
        scoped_refptr<viz::RasterContextProvider> compositor_context_provider, scoped_refptr<viz::RasterContextProvider> worker_context_provider) override
    {
        ...
        return LayerTreeHostPixelResourceTest::CreateLayerTreeFrameSink(
            modified_renderer_settings, refresh_rate, std::move(compositor_context_provider), std::move(worker_context_provider));
    }
...
// run with zero-copy raster.
    ...
    MaskTestConfig { { viz::RendererType::kSkiaGL, TestRasterType::kZeroCopy }, kUseAntialiasing },
    MaskTestConfig { { viz::RendererType::kSkiaGL, TestRasterType::kZeroCopy }, kForceShaders },
    MaskTestConfig { { viz::RendererType::kSkiaGL, TestRasterType::kZeroCopy }, kUseAntialiasing | kForceShaders },
#endif // BUILDFLAG(ENABLE_GL_BACKEND_TESTS) && BUILDFLAG(IS_MAC)
};

INSTANTIATE_TEST_SUITE_P(All, LayerTreeHostMaskAsBlendingPixelTest, ::testing::ValuesIn(kTestConfigs), ::testing::PrintToStringParamName());
...
TEST_P(LayerTreeHostMaskAsBlendingPixelTest, PixelAlignedNoop)
{
    ...
    scoped_refptr<Layer> content = CreateTestPatternLayer(gfx::Size(400, 300), 25);
    content->SetPosition(gfx::PointF(-40, -40));
    mask_isolation->AddChild(content);

    scoped_refptr<Layer> mask_layer = CreateSolidColorLayer(gfx::Rect(350, 250), kCSSBlack);
    mask_layer->SetBlendMode(SkBlendMode::kDstIn);
    mask_isolation->AddChild(mask_layer);

    RunPixelResourceTest(root, base::FilePath(FILE_PATH_LITERAL("mask_as_blending_noop.png")));
}
...
TEST_P(LayerTreeHostMaskAsBlendingPixelTest, PixelAlignedClippedCircle)
{
    ...
    display_list->Finalize();
    scoped_refptr<Layer> mask_layer = StaticPictureLayer::Create(std::move(display_list));
    mask_layer->SetIsDrawable(true);
    mask_layer->SetBounds(gfx::Size(350, 250));
    mask_layer->SetBlendMode(SkBlendMode::kDstIn);
    mask_isolation->AddChild(mask_layer);

    RunPixelResourceTest(root, base::FilePath(FILE_PATH_LITERAL("mask_as_blending_circle.png")));
}

TEST_P(LayerTreeHostMaskAsBlendingPixelTest, PixelAlignedClippedCircleUnderflow)
{
    ...
    scoped_refptr<Layer> content = CreateTestPatternLayer(gfx::Size(330, 230), 25);
    ...
    RunPixelResourceTest(root, base::FilePath(FILE_PATH_LITERAL("mask_as_blending_circle_underflow.png")));
}
...
TEST_P(LayerTreeHostMaskAsBlendingPixelTest, RotatedClippedCircle)
{
    ...
    base::FilePath image_name = (raster_type() == TestRasterType::kBitmap) ? base::FilePath(FILE_PATH_LITERAL("mask_as_blending_rotated_circle.png"))
                                                                           : base::FilePath(FILE_PATH_LITERAL("mask_as_blending_rotated_circle_gl.png"));
...
TEST_P(LayerTreeHostMaskAsBlendingPixelTest, RotatedClippedCircleUnderflow)
{
    ...
    base::FilePath image_name = (raster_type() == TestRasterType::kBitmap)
        ? base::FilePath(FILE_PATH_LITERAL("mask_as_blending_rotated_circle_underflow.png"))
        : base::FilePath(FILE_PATH_LITERAL("mask_as_blending_rotated_circle_underflow_gl.png"));
    RunPixelResourceTest(root, image_name);
}

class LayerTreeHostMasksForBackdropFiltersAndBlendPixelTest : public ParameterizedPixelResourceTest {
protected:
    ...
    void SetupTree() override
    {
        ...
        scoped_refptr<PictureLayer> picture_vertical = PictureLayer::Create(&picture_client_vertical_);
        picture_vertical->SetBounds(bounds_);
        picture_vertical->SetIsDrawable(true);
        CopyProperties(background.get(), picture_vertical.get());
        root->AddChild(picture_vertical);

        scoped_refptr<PictureLayer> picture_horizontal = PictureLayer::Create(&picture_client_horizontal_);
        ...
        effect_node.backdrop_filters.Append(FilterOperation::CreateGrayscaleFilter(1.0));
...
INSTANTIATE_TEST_SUITE_P(
    DISABLED_PixelResourceTest, LayerTreeHostMasksForBackdropFiltersAndBlendPixelTest, ::testing::ValuesIn(kTestCases), ::testing::PrintToStringParamName());

TEST_P(LayerTreeHostMasksForBackdropFiltersAndBlendPixelTest, Test)
{
    base::FilePath result_path(FILE_PATH_LITERAL("mask_of_backdrop_filter_and_blend_.png"));
