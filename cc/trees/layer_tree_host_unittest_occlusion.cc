...
#define EXPECT_OCCLUSION_EQ(expected, actual)                                                                                                                  \
    EXPECT_TRUE(expected.IsEqual(actual)) << " Expected: " << expected.ToString() << std::endl << " Actual: " << actual.ToString();
...
class LayerTreeHostOcclusionTestDrawPropertiesOnLayer : public LayerTreeHostOcclusionTest {
public:
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        EXPECT_OCCLUSION_EQ(
            Occlusion(child->DrawTransform(), SimpleEnclosedRegion(), SimpleEnclosedRegion()), child->draw_properties().occlusion_in_content_space);
        EXPECT_OCCLUSION_EQ(Occlusion(root->DrawTransform(), SimpleEnclosedRegion(), SimpleEnclosedRegion(gfx::Rect(10, 6, 50, 59))),
...
class LayerTreeHostOcclusionTestDrawPropertiesOnSurface : public LayerTreeHostOcclusionTest {
public:
    void SetupTree() override
    {
        scoped_refptr<Layer> root = Layer::Create();
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        EXPECT_OCCLUSION_EQ(Occlusion(surface->draw_transform(), SimpleEnclosedRegion(), SimpleEnclosedRegion(gfx::Rect(13, 9, 10, 11))),
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostOcclusionTestDrawPropertiesOnSurface);

// Verify occlusion is set on mask layers.
class LayerTreeHostOcclusionTestDrawPropertiesOnMask : public LayerTreeHostOcclusionTest {
public:
    void SetupTree() override
    {
        scoped_refptr<Layer> root = Layer::Create();
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        EXPECT_OCCLUSION_EQ(
            Occlusion(transform, SimpleEnclosedRegion(), SimpleEnclosedRegion(gfx::Rect(13, 9, 10, 11))), child_surface->occlusion_in_content_space());

        // Mask layer has its own transform and render surface in layer tree mode.
        EXPECT_NE(child_surface, mask_surface);
        EXPECT_OCCLUSION_EQ(
            Occlusion(transform, SimpleEnclosedRegion(), SimpleEnclosedRegion(gfx::Rect(13, 9, 10, 11))), mask_surface->occlusion_in_content_space());

        EXPECT_OCCLUSION_EQ(Occlusion(gfx::Transform(), SimpleEnclosedRegion(gfx::Rect(3, 4, 10, 10)), SimpleEnclosedRegion()),
...
class LayerTreeHostOcclusionTestDrawPropertiesOnScaledMask : public LayerTreeHostOcclusionTest {
public:
    void SetupTree() override
    {
        scoped_refptr<Layer> root = Layer::Create();
    ...
    void DrawLayersOnThread(LayerTreeHostImpl* impl) override
    {
        ...
        EXPECT_OCCLUSION_EQ(
            Occlusion(scale, SimpleEnclosedRegion(gfx::Rect(13, 15, 10, 11)), SimpleEnclosedRegion()), mask->draw_properties().occlusion_in_content_space);
        EndTest();
    }
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostOcclusionTestDrawPropertiesOnScaledMask);
