...
{
    UMA_HISTOGRAM_BOOLEAN("Compositing.Renderer.AnimationUpdateOnMissingPropertyNode", missing_property_node);
}

AnchorPositionScrollData::AnchorPositionScrollData() = default;
AnchorPositionScrollData::~AnchorPositionScrollData() = default;
AnchorPositionScrollData::AnchorPositionScrollData(const AnchorPositionScrollData&) = default;

bool AnchorPositionScrollData::operator==(const AnchorPositionScrollData& other) const = default;

bool StickyPositionNodeData::operator==(const StickyPositionNodeData& other) const = default;
...
template <typename T> PropertyTree<T>::~PropertyTree() = default;

template <typename T> PropertyTree<T>& PropertyTree<T>::operator=(const PropertyTree<T>&) = default;

#define DCHECK_NODE_EXISTENCE(check_node_existence, state, property, needs_rebuild)                                                                            \
    DCHECK(!check_node_existence || ((!state.currently_running[property] && !state.potentially_animating[property]) || needs_rebuild))
...
template <typename T> int PropertyTree<T>::Insert(const T& tree_node, int parent_id)
{
...
template <typename T> void PropertyTree<T>::RemoveNodes(size_t n)
{
    CHECK_LE(n, nodes_.size());
    nodes_.resize(nodes_.size() - n);

    const int upper_bound = base::checked_cast<int>(nodes_.size());
    base::EraseIf(element_id_to_node_index_, [upper_bound](const auto& entry) { return entry.second >= upper_bound; });
}

template <typename T> void PropertyTree<T>::clear()
{
    needs_update_ = false;
    nodes_.clear();
    nodes_.push_back(T());
...
#if DCHECK_IS_ON()
template <typename T> bool PropertyTree<T>::operator==(const PropertyTree<T>& other) const
{
    return nodes() == other.nodes() && needs_update() == other.needs_update() && element_id_to_node_index() == other.element_id_to_node_index();
}
#endif

template <typename T> void PropertyTree<T>::AsValueInto(base::trace_event::TracedValue* value) const
{
...
bool TransformTree::OnTransformAnimated(ElementId element_id, const gfx::Transform& transform)
{
...
{
    for (int id = kContentsRootPropertyNodeId; id < static_cast<int>(size()); ++id) {
        TransformNode* node = Node(id);
        node->transform_changed = false;
    }
}

void TransformTree::UpdateAllTransforms(const ViewportPropertyIds& viewport_property_ids)
{
    if (!needs_update()) {
#if DCHECK_IS_ON()
        // If the transform tree does not need an update, no TransformNode should
        // need a local transform update.
        for (int i = kContentsRootPropertyNodeId; i < static_cast<int>(size()); ++i) {
    ...
    do {
        size_t last_num_stale_forward_dependencies = update_data.stale_forward_dependencies.size();
        for (int i = kContentsRootPropertyNodeId; i < static_cast<int>(size()); ++i) {
            UpdateTransforms(i, &viewport_property_ids, &update_data);
        }
        CHECK(last_num_stale_forward_dependencies == 0 || update_data.stale_forward_dependencies.size() < last_num_stale_forward_dependencies);
...
void TransformTree::UpdateTransforms(int id, const ViewportPropertyIds* viewport_property_ids, UpdateTransformsData* update_data)
{
    TransformNode* node = Node(id);
    ...
    if (node->sticky_position_constraint_id >= 0 || node->anchor_position_scroll_data_id >= 0 || node->needs_local_transform_update
        || node->should_undo_overscroll) {
    ...
    if (update_data && node->to_parent == old_to_parent && node->snap_amount == old_snap_amount) {
...
void TransformTree::CombineTransformsBetween(int source_id, int dest_id, gfx::Transform* transform) const
{
...
bool TransformTree::CombineInversesBetween(int source_id, int dest_id, gfx::Transform* transform) const
{
    DCHECK(source_id < dest_id);
...
gfx::Vector2dF TransformTree::StickyPositionOffset(TransformNode* node)
{
    ...
    const ScrollNode* scroll_node = property_trees()->scroll_tree().Node(sticky_data->scroll_ancestor);
    ...
    if (scroll_node->scrolls_outer_viewport) {
        clip.set_width(clip.width() + property_trees()->outer_viewport_container_bounds_delta().x());
        clip.set_height(clip.height() + property_trees()->outer_viewport_container_bounds_delta().y());
    }
    ...
    if (sticky_data->nearest_node_shifting_sticky_box != kInvalidPropertyNodeId) {
        ...
        if (sticky_data->nearest_node_shifting_sticky_box >= static_cast<int>(property_trees()->transform_tree().size()))
            return gfx::Vector2dF();
        const StickyPositionNodeData* ancestor_sticky_data = GetStickyPositionData(sticky_data->nearest_node_shifting_sticky_box);
        DCHECK(ancestor_sticky_data);
        ancestor_sticky_box_offset = ancestor_sticky_data->total_sticky_box_sticky_offset;
    }

    gfx::Vector2dF ancestor_containing_block_offset;
    if (sticky_data->nearest_node_shifting_containing_block != kInvalidPropertyNodeId) {
        // TODO(crbug.com/40053373): Investigate why there would be an invalid index
        // passed in. Early return for now.
        if (sticky_data->nearest_node_shifting_containing_block >= static_cast<int>(property_trees()->transform_tree().size()))
            return gfx::Vector2dF();
        const StickyPositionNodeData* ancestor_sticky_data = GetStickyPositionData(sticky_data->nearest_node_shifting_containing_block);
        DCHECK(ancestor_sticky_data);
        ancestor_containing_block_offset = ancestor_sticky_data->total_containing_block_sticky_offset;
    }
    ...
    gfx::RectF sticky_box_rect
        = gfx::RectF(constraint.scroll_container_relative_sticky_box_rect) + ancestor_sticky_box_offset + ancestor_containing_block_offset;
    gfx::RectF containing_block_rect = gfx::RectF(constraint.scroll_container_relative_containing_block_rect) + ancestor_containing_block_offset;
    ...
    if (constraint.is_anchored_right) {
        ...
        float right_delta = std::min<float>(0, right_limit - sticky_box_rect.right());
        float available_space = std::min<float>(0, containing_block_rect.x() - sticky_box_rect.x());
    ...
    if (constraint.is_anchored_left) {
        ...
        float available_space = std::max<float>(0, containing_block_rect.right() - sticky_box_rect.right());
    ...
    if (constraint.is_anchored_bottom) {
        ...
        float bottom_delta = std::min<float>(0, bottom_limit - sticky_box_rect.bottom());
        float available_space = std::min<float>(0, containing_block_rect.y() - sticky_box_rect.y());
    ...
    if (constraint.is_anchored_top) {
        ...
        float available_space = std::max<float>(0, containing_block_rect.bottom() - sticky_box_rect.bottom());
        if (top_delta > available_space)
            top_delta = available_space;
        sticky_offset.set_y(sticky_offset.y() + top_delta);
    }

    sticky_data->total_sticky_box_sticky_offset = ancestor_sticky_box_offset + sticky_offset;
    sticky_data->total_containing_block_sticky_offset = ancestor_sticky_box_offset + ancestor_containing_block_offset + sticky_offset;

    // return
    return gfx::ToRoundedVector2d(sticky_offset);
}

AnchorPositionScrollData& TransformTree::EnsureAnchorPositionScrollData(int node_id)
{
...
const AnchorPositionScrollData* TransformTree::GetAnchorPositionScrollData(int node_id) const
{
...
gfx::Vector2dF TransformTree::AnchorPositionOffset(TransformNode* node, int max_updated_node_id, UpdateTransformsData* update_data)
{
    ...
    for (ElementId container_id : data->adjustment_container_ids) {
        ...
        if (const ScrollNode* scroll_node = property_trees()->scroll_tree().FindNodeFromElementId(container_id)) {
        ...
        } else if (TransformNode* container_transform = property_trees()->transform_tree_mutable().FindNodeFromElementId(container_id)) {
            ...
            accumulated_offset -= AnchorPositionOffset(container_transform, max_updated_node_id, update_data);
        }
...
void TransformTree::UndoOverscroll(const TransformNode* node, gfx::Vector2dF& position_adjustment, const ViewportPropertyIds* viewport_property_ids)
{
    DCHECK(node->should_undo_overscroll);

    const int transform_id = viewport_property_ids ? viewport_property_ids->overscroll_elasticity_transform : kInvalidPropertyNodeId;
    if (transform_id == kInvalidPropertyNodeId)
        return;

    const int clip_id = viewport_property_ids ? viewport_property_ids->outer_clip : kInvalidPropertyNodeId;
    if (clip_id == kInvalidPropertyNodeId)
        return;

    const TransformNode* overscroll_node = Node(transform_id);
    const gfx::Vector2dF overscroll_offset = overscroll_node->scroll_offset.OffsetFromOrigin();
    if (overscroll_offset.IsZero())
        return;

    position_adjustment += gfx::ScaleVector2d(overscroll_offset, 1.f / page_scale_factor());
    ...
    position_adjustment.x() < 0 ? outsets.set_left(-position_adjustment.x()) : outsets.set_right(position_adjustment.x());
    position_adjustment.y() < 0 ? outsets.set_top(-position_adjustment.y()) : outsets.set_bottom(position_adjustment.y());
    clip_node->clip.Outset(outsets);
    clip_tree.set_needs_update(true);
}

void TransformTree::UpdateLocalTransform(TransformNode* node, const ViewportPropertyIds* viewport_property_ids, UpdateTransformsData* update_data)
{
    gfx::Transform transform;
    transform.Translate3d(node->post_translation.x() + node->origin.x(), node->post_translation.y() + node->origin.y(), node->origin.z());

    gfx::Vector2dF position_adjustment;
    if (node->moved_by_outer_viewport_bounds_delta_y) {
        position_adjustment.set_y(property_trees()->outer_viewport_container_bounds_delta().y());
    }
    if (node->should_undo_overscroll)
        UndoOverscroll(node, position_adjustment, viewport_property_ids);
    transform.Translate(position_adjustment - node->scroll_offset.OffsetFromOrigin());
...
void TransformTree::UpdateScreenSpaceTransform(TransformNode* node, TransformNode* parent_node)
{
    DCHECK(parent_node);
...
void TransformTree::UpdateAnimationProperties(TransformNode* node, TransformNode* parent_node)
{
    DCHECK(parent_node);
...
void TransformTree::UpdateSnapping(TransformNode* node)
{
    ...
    DCHECK(delta.IsApproximatelyIdentityOrTranslation(kTolerance)) << delta.ToString();
...
void TransformTree::UpdateTransformChanged(TransformNode* node, TransformNode* parent_node)
{
    DCHECK(parent_node);
...
void TransformTree::UpdateNodeAndAncestorsAreAnimatedOrInvertible(TransformNode* node, TransformNode* parent_node)
{
    DCHECK(parent_node);
...
void TransformTree::UpdateNodeOrAncestorsWillChangeTransform(TransformNode* node, TransformNode* parent_node)
{
    node->node_or_ancestors_will_change_transform = node->will_change_transform;
    if (parent_node) {
        node->node_or_ancestors_will_change_transform |= parent_node->node_or_ancestors_will_change_transform;
    }
}

void TransformTree::SetRootScaleAndTransform(float device_scale_factor, const gfx::Transform& device_transform)
{
    device_scale_factor_ = device_scale_factor;
    gfx::Vector2dF device_transform_scale_components = gfx::ComputeTransform2dScaleComponents(device_transform, 1.f);

    // Not handling the rare case of different x and y device scale.
    device_transform_scale_factor_ = std::max(device_transform_scale_components.x(), device_transform_scale_components.y());
    ...
    gfx::Vector2dF screen_space_scale = gfx::ComputeTransform2dScaleComponents(transform, device_scale_factor);
...
void TransformTree::SetFromScreen(int node_id, const gfx::Transform& transform)
{
    DCHECK(static_cast<int>(cached_data_.size()) > node_id);
...
{
    return PropertyTree::operator==(other) && page_scale_factor_ == other.page_scale_factor() && device_scale_factor_ == other.device_scale_factor()
        && device_transform_scale_factor_ == other.device_transform_scale_factor()
        && nodes_affected_by_outer_viewport_bounds_delta_ == other.nodes_affected_by_outer_viewport_bounds_delta() && cached_data_ == other.cached_data();
}
#endif
...
void EffectTree::UpdateSubtreeHidden(EffectNode* node, EffectNode* parent_node)
{
    if (parent_node)
...
void EffectTree::UpdateIsDrawn(EffectNode* node, EffectNode* parent_node)
{
    ...
    if (node->has_copy_request || node->cache_render_surface || node->subtree_capture_id.is_valid()) {
        node->is_drawn = true;
    } else if (EffectiveOpacity(node) == 0.f && (!node->has_potential_opacity_animation || property_trees()->is_active()) && node->backdrop_filters.IsEmpty()) {
        node->is_drawn = false;
    } else if (parent_node) {
...
void EffectTree::UpdateEffectChanged(EffectNode* node, EffectNode* parent_node)
{
...
{
    node->node_or_ancestor_has_filters = !node->filters.IsEmpty() || node->has_potential_filter_animation;
    if (parent_node) {
        node->node_or_ancestor_has_filters |= parent_node->node_or_ancestor_has_filters;
    }
}

void EffectTree::UpdateHasFastRoundedCorner(EffectNode* node, EffectNode* parent_node)
{
    node->node_or_ancestor_has_fast_rounded_corner = node->is_fast_rounded_corner;
    if (parent_node) {
        node->node_or_ancestor_has_fast_rounded_corner |= parent_node->node_or_ancestor_has_fast_rounded_corner;
    }
}

void EffectTree::UpdateBackfaceVisibility(EffectNode* node, EffectNode* parent_node)
{
    ...
    node->hidden_by_backface_visibility = property_trees()->transform_tree().cached_data()[node->transform_id].is_showing_backface;
}

void EffectTree::UpdateHasMaskingChild(EffectNode* node, EffectNode* parent_node)
{
...
void EffectTree::UpdateOnlyDrawsVisibleContent(EffectNode* node, EffectNode* parent_node)
{
    node->only_draws_visible_content = !node->has_copy_request && !node->subtree_capture_id.is_valid() && !node->view_transition_element_resource_id.IsValid();
    if (parent_node)
        node->only_draws_visible_content &= parent_node->only_draws_visible_content;
    if (!node->backdrop_filters.IsEmpty()) {
        node->only_draws_visible_content &= !node->backdrop_filters.HasFilterOfType(FilterOperation::ZOOM);
    }
}
...
void EffectTree::UpdateSurfaceContentsScale(EffectNode* effect_node)
{
    ...
    const TransformNode* transform_node = transform_tree.Node(effect_node->transform_id);
    ...
    effect_node->surface_contents_scale = gfx::ComputeTransform2dScaleComponents(transform_tree.ToScreen(transform_node->id), layer_scale_factor);
...
bool EffectTree::OnFilterAnimated(ElementId id, const FilterOperations& filters)
{
    EffectNode* node = FindNodeFromElementId(id);
    // TODO(crbug.com/40828469): Remove this when we no longer animate
    // non-existent nodes.
...
bool EffectTree::OnBackdropFilterAnimated(ElementId id, const FilterOperations& backdrop_filters)
{
    EffectNode* node = FindNodeFromElementId(id);
    // TODO(crbug.com/40828469): Remove this when we no longer animate
    // non-existent nodes.
...
void EffectTree::UpdateClosestAncestorSharedElement(EffectNode* node, EffectNode* parent_node)
{
    if (node->view_transition_element_resource_id.IsValid()) {
        node->closest_ancestor_with_shared_element_id = node->id;
    } else if (parent_node) {
        node->closest_ancestor_with_shared_element_id = parent_node->closest_ancestor_with_shared_element_id;
    }
}

void EffectTree::AddCopyRequest(int node_id, std::unique_ptr<viz::CopyOutputRequest> request)
{
...
void EffectTree::TakeCopyRequestsAndTransformToSurface(int node_id, std::vector<std::unique_ptr<viz::CopyOutputRequest>>* requests)
{
    ...
    if (transform.IsPositiveScaleOrTranslation()) {
        ...
        gfx::RectF surface_rect = transform.MapRect(gfx::RectF(0, 0, kContentVector.x(), kContentVector.y()));
        ...
        for (auto it = range.first; it != range.second; ++it) {
            ...
            if (request->has_area()) {
                ...
                request->set_area(MathUtil::MapEnclosingClippedRectIgnoringError(transform, request->area(), kEpsilon));
            }
            ...
            if (request->is_scaled() || request->has_result_selection()) {
                ...
                float scale_from_y_f = request->scale_from().y() * surface_rect.height();
                if (std::isnan(scale_from_x_f) || !base::IsValueInRangeForNumericType<int>(scale_from_x_f) || std::isnan(scale_from_y_f)
                    || !base::IsValueInRangeForNumericType<int>(scale_from_y_f)) {
                    continue;
                }
                int scale_to_x = request->scale_to().x();
                int scale_to_y = request->scale_to().y();
                if (!base::CheckMul(scale_to_x, kContentVector.x()).AssignIfValid(&scale_to_x)
                    || !base::CheckMul(scale_to_y, kContentVector.y()).AssignIfValid(&scale_to_y)) {
                    continue;
                }
                int scale_from_x = base::ClampRound(scale_from_x_f);
                int scale_from_y = base::ClampRound(scale_from_y_f);
                if (scale_from_x <= 0 || scale_from_y <= 0 || scale_to_x <= 0 || scale_to_y <= 0) {
                    // Transformed scaling ratio became illegal. Drop the request to
                    // provide an empty response.
                    continue;
                }
                request->SetScaleRatio(gfx::Vector2d(scale_from_x, scale_from_y), gfx::Vector2d(scale_to_x, scale_to_y));
            }
...
void EffectTree::GetRenderSurfaceChangedFlags(std::vector<RenderSurfacePropertyChangedFlags>& flags) const
{
    flags.resize(size());
    for (int id = kContentsRootPropertyNodeId; id < static_cast<int>(size()); ++id) {
...
void EffectTree::ApplyRenderSurfaceChangedFlags(const std::vector<RenderSurfacePropertyChangedFlags>& flags)
{
...
int EffectTree::LowestCommonAncestorWithRenderSurface(int id_1, int id_2) const
{
...
void EffectTree::TakeRenderSurfaces(std::vector<std::unique_ptr<RenderSurfaceImpl>>* render_surfaces)
{
    for (int id = kContentsRootPropertyNodeId; id < static_cast<int>(size()); ++id) {
        if (render_surfaces_[id]) {
...
bool EffectTree::CreateOrReuseRenderSurfaces(std::vector<std::unique_ptr<RenderSurfaceImpl>>* old_render_surfaces, LayerTreeImpl* layer_tree_impl)
{
    ...
    std::sort(old_render_surfaces->begin(), old_render_surfaces->end(),
        [](const std::unique_ptr<RenderSurfaceImpl>& a, const std::unique_ptr<RenderSurfaceImpl>& b) { return a->id() < b->id(); });
    ...
    while (surfaces_list_it != old_render_surfaces->end() && id_list_it != stable_id_node_id_list.end()) {
        ...
        if (id_list_it->first < (*surfaces_list_it)->id()) {
            int new_node_id = id_list_it->second;
            render_surfaces_[new_node_id] = std::make_unique<RenderSurfaceImpl>(layer_tree_impl, id_list_it->first);
            render_surfaces_[new_node_id]->set_effect_tree_index(new_node_id);
            id_list_it++;
        } else {
            surfaces_list_it++;
    ...
    if (surfaces_list_it != old_render_surfaces->end() || id_list_it != stable_id_node_id_list.end()) {
        render_surfaces_changed = true;
    }
...
bool EffectTree::ClippedHitTestRegionIsRectangle(int effect_id) const
{
    ...
    for (; effect_node->id != kContentsRootPropertyNodeId; effect_node = Node(effect_node->target_id)) {
        gfx::Transform to_target;
        if (!property_trees()->GetToTarget(effect_node->transform_id, effect_node->target_id, &to_target) || !to_target.Preserves2dAxisAlignment())
            return false;
    }
    return true;
}
...
{
    const EffectNode* effect_node = Node(effect_id);
    for (; effect_node->id != kContentsRootPropertyNodeId; effect_node = Node(effect_node->parent_id)) {
        if (!effect_node->mask_filter_info.IsEmpty() || effect_node->has_masking_child)
            return true;
    }
    return false;
}
...
bool ScrollTree::CanRealizeScrollsOnActiveTree(const ScrollNode& node) const
{
    return node.transform_id != kInvalidPropertyNodeId && node.is_composited
        && GetMainThreadRepaintReasons(node) == MainThreadScrollingReason::kNotScrollingOnMain;
}
...
{
    return node.transform_id != kInvalidPropertyNodeId && GetMainThreadRepaintReasons(node) != MainThreadScrollingReason::kNotScrollingOnMain;
}
...
gfx::PointF ScrollTree::MaxScrollOffset(int scroll_node_id) const
{
    ...
    scaled_scroll_bounds.SetSize(std::floor(scaled_scroll_bounds.width()), std::floor(scaled_scroll_bounds.height()));

    gfx::Size clip_layer_bounds = container_bounds(scroll_node->id);

    gfx::PointF max_offset(scaled_scroll_bounds.width() - clip_layer_bounds.width(), scaled_scroll_bounds.height() - clip_layer_bounds.height());
...
void ScrollTree::OnScrollOffsetAnimated(ElementId id, int scroll_tree_index, const gfx::PointF& scroll_offset, LayerTreeImpl* layer_tree_impl)
{
    ...
    TRACE_EVENT2("cc", "ScrollTree::OnScrollOffsetAnimated", "x", scroll_offset.x(), "y", scroll_offset.y());
    ScrollNode* scroll_node = Node(scroll_tree_index);
    if (SetScrollOffset(id, ClampScrollOffsetToLimits(scroll_offset, *scroll_node))) {
        layer_tree_impl->DidUpdateScrollOffset(id, /*pushed_from_main_or_pending_tree=*/false);
    }
...
gfx::Size ScrollTree::container_bounds(int scroll_node_id) const
{
    ...
    if (scroll_node->scrolls_inner_viewport) {
        container_bounds_delta.Add(property_trees()->inner_viewport_container_bounds_delta());
    } else if (scroll_node->scrolls_outer_viewport) {
        container_bounds_delta.Add(property_trees()->outer_viewport_container_bounds_delta());
    }
...
{
    return property_trees()->transform_tree().ToScreen(Node(scroll_node_id)->transform_id);
}
...
const SyncedScrollOffset* ScrollTree::GetSyncedScrollOffset(ElementId id) const
{
    return const_cast<ScrollTree*>(this)->GetSyncedScrollOffset(id);
}

gfx::Vector2dF ScrollTree::ClampScrollToMaxScrollOffset(const ScrollNode& node, LayerTreeImpl* layer_tree_impl)
{
...
gfx::PointF ScrollTree::GetScrollOffsetForScrollTimeline(const ScrollNode& scroll_node) const
{
    gfx::PointF offset = current_scroll_offset(scroll_node.element_id);
    if (!property_trees()->is_main_thread()) {
        if (const SyncedScrollOffset* synced_offset = GetSyncedScrollOffset(scroll_node.element_id)) {
            ...
            } else if (!CanRealizeScrollsOnActiveTree(scroll_node) && !CanRealizeScrollsOnPendingTree(scroll_node)) {
                offset = synced_offset->PendingBase();
            }
        }
    }

    const TransformNode* transform_node = property_trees()->transform_tree().Node(scroll_node.transform_id);
    ...
    if (transform_node && transform_node->scrolls) {
        ...
        if (transform_node->needs_local_transform_update)
            property_trees()->transform_tree_mutable().UpdateTransforms(transform_node->id);
        ...
        offset = ClampScrollOffsetToLimits(offset - transform_node->snap_amount, scroll_node);
    }

    return offset;
}

gfx::Vector2dF ScrollTree::PullDeltaForMainThread(SyncedScrollOffset* scroll_offset, bool use_fractional_deltas, bool next_bmf)
{
    ...
    gfx::PointF current_offset = scroll_offset->Current(/* is_active_tree */ true);
    ...
    gfx::Vector2dF diff_active_base = active_base - gfx::PointF(gfx::ToRoundedPoint(active_base));
...
void ScrollTree::CollectScrollDeltas(CompositorCommitData* commit_data, ElementId inner_viewport_scroll_element_id, bool use_fractional_deltas,
    const base::flat_map<ElementId, TargetSnapAreaElementIds>& snapped_elements, const MutatorHost* main_thread_mutator_host)
{
    DCHECK(!property_trees()->is_main_thread());
    ...
    for (auto map_entry : synced_scroll_offset_map_) {
        ...
        // thread.
        bool clobber = main_thread_mutator_host && main_thread_mutator_host->ScrollOffsetAnimationWasInterrupted(map_entry.first);
        if (!clobber) {
            scroll_delta = PullDeltaForMainThread(map_entry.second.get(), use_fractional_deltas, pipeline);
        }
        ...
        if (!scroll_delta.IsZero() || snap_target_ids) {
            TRACE_EVENT_INSTANT2("cc", "CollectScrollDeltas", TRACE_EVENT_SCOPE_THREAD, "x", scroll_delta.x(), "y", scroll_delta.y());
            CompositorCommitData::ScrollUpdateInfo update(id, scroll_delta, snap_target_ids);
...
void ScrollTree::PushScrollUpdatesFromMainThread(const PropertyTrees& main_property_trees, LayerTreeImpl* sync_tree, bool use_fractional_deltas)
{
    DCHECK(!property_trees()->is_main_thread());
    const ScrollOffsetMap& main_scroll_offset_map = main_property_trees.scroll_tree().scroll_offset_map_;
    ...
    for (auto map_entry = synced_scroll_offset_map_.begin(); map_entry != synced_scroll_offset_map_.end();) {
        ElementId id = map_entry->first;
    ...
    for (auto map_entry : main_scroll_offset_map) {
        ...
        bool needs_scroll_update = synced_scroll_offset->PushMainToPending(map_entry.second);
        ...
        if (needs_scroll_update) {
            sync_tree->DidUpdateScrollOffset(id, /*pushed_from_main_or_pending_tree=*/true);
        }
    }
}

void ScrollTree::PushScrollUpdatesFromPendingTree(PropertyTrees* pending_property_trees, LayerTreeImpl* active_tree)
{
    DCHECK(property_trees()->is_active());
    ...
    for (auto map_entry : pending_property_trees->scroll_tree().synced_scroll_offset_map_) {
        synced_scroll_offset_map_[map_entry.first] = map_entry.second;
        if (map_entry.second->PushPendingToActive()) {
            active_tree->DidUpdateScrollOffset(map_entry.first, /*pushed_from_main_or_pending_tree=*/true);
        }
    }
}

void ScrollTree::ApplySentScrollDeltasFromAbortedCommit(bool next_bmf, bool main_frame_applied_deltas)
{
    DCHECK(property_trees()->is_active());
    for (auto& map_entry : synced_scroll_offset_map_)
...
void ScrollTree::SetBaseScrollOffset(ElementId id, const gfx::PointF& scroll_offset)
{
    if (property_trees()->is_main_thread()) {
...
bool ScrollTree::SetScrollOffset(ElementId id, const gfx::PointF& scroll_offset)
{
    // TODO(crbug.com/40132829): Remove TRACE_EVENT call when the bug is fixed
    TRACE_EVENT2("cc", "ScrollTree::SetScrollOffset", "x", scroll_offset.x(), "y", scroll_offset.y());
    if (property_trees()->is_main_thread()) {
...
void ScrollTree::SetScrollingContentsCullRect(ElementId id, const gfx::Rect& cull_rect)
{
...
SyncedScrollOffset* ScrollTree::GetOrCreateSyncedScrollOffsetForTesting(ElementId id)
{
    auto it = synced_scroll_offset_map_.find(id);
    if (it == synced_scroll_offset_map_.end()) {
        it = synced_scroll_offset_map_.try_emplace(id, new SyncedScrollOffset()).first;
    }
    return it->second.get();
}

bool ScrollTree::UpdateScrollOffsetBaseForTesting(ElementId id, const gfx::PointF& offset)
{
    DCHECK(!property_trees()->is_main_thread());
    SyncedScrollOffset* synced_scroll_offset = GetOrCreateSyncedScrollOffsetForTesting(id); // IN-TEST
...
bool ScrollTree::SetScrollOffsetDeltaForTesting(ElementId id, const gfx::Vector2dF& delta)
{
    auto* synced_scroll_offset = GetOrCreateSyncedScrollOffsetForTesting(id); // IN-TEST
    return synced_scroll_offset->SetCurrent(synced_scroll_offset->ActiveBase() + delta);
}

const gfx::PointF ScrollTree::GetScrollOffsetBaseForTesting(ElementId id) const
{
    DCHECK(!property_trees()->is_main_thread());
    if (GetSyncedScrollOffset(id)) {
        return property_trees()->is_active() ? GetSyncedScrollOffset(id)->ActiveBase() : GetSyncedScrollOffset(id)->PendingBase();
    }
    return gfx::PointF();
}

const gfx::Vector2dF ScrollTree::GetScrollOffsetDeltaForTesting(ElementId id) const
{
    DCHECK(!property_trees()->is_main_thread());
    if (GetSyncedScrollOffset(id)) {
        return property_trees()->is_active() ? GetSyncedScrollOffset(id)->Delta() : GetSyncedScrollOffset(id)->PendingDelta();
    }
    return gfx::Vector2dF();
}

gfx::Vector2dF ScrollTree::ScrollBy(const ScrollNode& scroll_node, const gfx::Vector2dF& scroll, LayerTreeImpl* layer_tree_impl)
{
    TRACE_EVENT_BEGIN("input", "ScrollTree::ScrollBy", "scroll", scroll, "scroll_node_id", scroll_node.id);
    ...
    gfx::PointF new_offset = ClampScrollOffsetToLimits(old_offset + adjusted_scroll, scroll_node);
...
gfx::PointF ScrollTree::ClampScrollOffsetToLimits(gfx::PointF offset, const ScrollNode& scroll_node) const
{
...
void ScrollTree::NotifyDidCompositorScroll(
    ElementId scroll_element_id, const gfx::PointF& scroll_offset, const std::optional<TargetSnapAreaElementIds>& snap_target_ids)
{
    DCHECK(property_trees()->is_main_thread());
    if (callbacks_) {
        callbacks_->DidCompositorScroll(scroll_element_id, scroll_offset, snap_target_ids);
    }
}

void ScrollTree::NotifyDidChangeScrollbarsHidden(ElementId scroll_element_id, bool hidden) const
{
    DCHECK(property_trees()->is_main_thread());
    if (callbacks_)
...
PropertyTreesChangeState::~PropertyTreesChangeState() = default;
PropertyTreesChangeState::PropertyTreesChangeState(PropertyTreesChangeState&&) = default;
PropertyTreesChangeState& PropertyTreesChangeState::operator=(PropertyTreesChangeState&&) = default;
...
{
    return transform_tree() == other.transform_tree() && effect_tree() == other.effect_tree() && clip_tree() == other.clip_tree()
        && scroll_tree() == other.scroll_tree() && needs_rebuild() == other.needs_rebuild() && changed() == other.changed()
        && full_tree_damaged() == other.full_tree_damaged() && is_main_thread() == other.is_main_thread() && is_active() == other.is_active()
...
PropertyTrees& PropertyTrees::operator=(const PropertyTrees& from)
{
    ...
    SetInnerViewportContainerBoundsDelta(from.inner_viewport_container_bounds_delta());
    SetOuterViewportContainerBoundsDelta(from.outer_viewport_container_bounds_delta());
...
void PropertyTrees::SetInnerViewportContainerBoundsDelta(gfx::Vector2dF bounds_delta)
{
...
void PropertyTrees::SetOuterViewportContainerBoundsDelta(gfx::Vector2dF bounds_delta)
{
...
bool PropertyTrees::ElementIsAnimatingChanged(
    const PropertyToElementIdMap& element_id_map, const PropertyAnimationState& mask, const PropertyAnimationState& state, bool check_node_existence)
{
    bool updated_transform = false;
    for (int property = TargetProperty::FIRST_TARGET_PROPERTY; property <= TargetProperty::LAST_TARGET_PROPERTY; ++property) {
        if (!mask.currently_running[property] && !mask.potentially_animating[property])
            continue;
        ...
        case TargetProperty::TRANSLATE:
            if (TransformNode* transform_node = transform_tree_mutable().FindNodeFromElementId(element_id)) {
                ...
                DCHECK_NODE_EXISTENCE(check_node_existence, state, property, needs_rebuild()) << "Attempting to animate non existent transform node";
            }
            break;
        case TargetProperty::OPACITY:
            if (EffectNode* effect_node = effect_tree_mutable().FindNodeFromElementId(element_id)) {
                if (mask.currently_running[property])
                ...
                DCHECK_NODE_EXISTENCE(check_node_existence, state, property, needs_rebuild()) << "Attempting to animate opacity on non existent effect node";
            }
            break;
        case TargetProperty::FILTER:
            if (EffectNode* effect_node = effect_tree_mutable().FindNodeFromElementId(element_id)) {
                ...
                DCHECK_NODE_EXISTENCE(check_node_existence, state, property, needs_rebuild()) << "Attempting to animate filter on non existent effect node";
            }
            break;
        case TargetProperty::BACKDROP_FILTER:
            if (EffectNode* effect_node = effect_tree_mutable().FindNodeFromElementId(element_id)) {
                if (mask.currently_running[property])
                    effect_node->is_currently_animating_backdrop_filter = state.currently_running[property];
                if (mask.potentially_animating[property])
                    effect_node->has_potential_backdrop_filter_animation = state.potentially_animating[property];
...
void PropertyTrees::MaximumAnimationScaleChanged(ElementId element_id, float maximum_scale)
{
    if (TransformNode* transform_node = transform_tree_mutable().FindNodeFromElementId(element_id)) {
...
void PropertyTrees::UpdateChangeTracking()
{
    ...
    for (int id = kContentsRootPropertyNodeId; id < static_cast<int>(mutable_effect_tree.size()); ++id) {
    ...
    for (int i = kContentsRootPropertyNodeId; i < static_cast<int>(mutable_transform_tree.size()); ++i) {
...
void PropertyTrees::GetChangedNodes(std::vector<int>& effect_nodes, std::vector<int>& transform_nodes) const
{
    ...
    for (int id = kContentsRootPropertyNodeId; id < static_cast<int>(transform_tree().size()); ++id) {
        if (transform_tree().Node(id)->transform_changed)
            transform_nodes.push_back(id);
    }
}

void PropertyTrees::ApplyChangedNodes(const std::vector<int>& changed_effect_nodes, const std::vector<int>& changed_transform_nodes)
{
...
void PropertyTrees::GetChangeState(PropertyTreesChangeState& change_state)
{
    ...
    // snapshot that first.
    change_state.effect_tree_copy_requests = effect_tree_mutable().TakeCopyRequests();
    GetChangedNodes(change_state.changed_effect_nodes, change_state.changed_transform_nodes);
    effect_tree().GetRenderSurfaceChangedFlags(change_state.surface_property_changed_flags);
}
...
std::unique_ptr<base::trace_event::TracedValue> PropertyTrees::AsTracedValue() const
{
...
bool PropertyTrees::AnimationScaleCacheIsInvalid(int transform_id) const
{
    ...
    return cached_data_.animation_scales[transform_id].update_number == kInvalidUpdateNumber;
}
...
const AnimationScaleData& PropertyTrees::GetAnimationScaleData(int transform_id)
{
    DCHECK(!is_main_thread());

    auto& animation_scale = cached_data_.animation_scales[transform_id];
    if (animation_scale.update_number == cached_data_.transform_tree_update_number) {
    ...
    const auto* parent_animation_scale = parent_node ? &GetAnimationScaleData(parent_node->id) : nullptr;

    bool ancestor_affected_by_animation_scale = parent_node && parent_animation_scale->affected_by_animation_scale;
    bool node_affected_by_animation_scale = node->has_potential_animation && node->maximum_animation_scale != 1.0f;

    animation_scale.affected_by_animation_scale = node_affected_by_animation_scale || ancestor_affected_by_animation_scale;
    animation_scale.affected_by_invalid_scale = (parent_node && parent_animation_scale->affected_by_invalid_scale) ||
        // Computing maximum animated scale in the presence of perspective isn't
        // supported.
        node->to_parent.HasPerspective() || (node->has_potential_animation && node->maximum_animation_scale == kInvalidScale);
    ...
    bool failed_for_multiple_scale_animations = ancestor_affected_by_animation_scale && node_affected_by_animation_scale;

    float local_maximum_scale = 1.0f;
    if (animation_scale.affected_by_invalid_scale || failed_for_multiple_scale_animations) {
        ...
        gfx::Vector2dF to_screen_scales = gfx::ComputeTransform2dScaleComponents(transform_tree().ToScreen(transform_id), kInvalidScale);
        animation_scale.maximum_to_screen_scale = std::max(to_screen_scales.x(), to_screen_scales.y());
        return animation_scale;
    } else if (!node->has_potential_animation) {
        gfx::Vector2dF local_scales = gfx::ComputeTransform2dScaleComponents(node->local, 1.0f);
    ...
    if (parent_node) {
        animation_scale.maximum_to_screen_scale *= parent_animation_scale->maximum_to_screen_scale;
    }

    return animation_scale;
}

void PropertyTrees::SetMaximumAnimationToScreenScaleForTesting(int transform_id, float maximum_scale, bool affected_by_invalid_scale)
{
    auto& animation_scale = cached_data_.animation_scales[transform_id];
...
bool PropertyTrees::GetToTarget(int transform_id, int effect_id, gfx::Transform* to_target) const
{
...
bool PropertyTrees::GetFromTarget(int transform_id, int effect_id, gfx::Transform* from_target) const
{
    const TransformNode* node = transform_tree().Node(transform_id);
    if (node->ancestors_are_invertible && effect_id == kContentsRootPropertyNodeId) {
...
DrawTransformData& PropertyTrees::FetchDrawTransformsDataFromCache(int transform_id, int dest_id) const
{
    for (auto& transform_data : cached_data_.draw_transforms[transform_id]) {
        ...
        if (transform_data.target_id == dest_id || transform_data.target_id == kInvalidPropertyNodeId) {
...
ClipRectData* PropertyTrees::FetchClipRectFromCache(int clip_id, int target_id)
{
...
bool PropertyTrees::HasElement(ElementId element_id) const
{
    if (!element_id)
        return false;
    return clip_tree().FindNodeFromElementId(element_id) || effect_tree().FindNodeFromElementId(element_id) || scroll_tree().FindNodeFromElementId(element_id)
        || transform_tree().FindNodeFromElementId(element_id);
}

DrawTransforms& PropertyTrees::GetDrawTransforms(int transform_id, int effect_id) const
{
    ...
    DCHECK(data.update_number != cached_data_.transform_tree_update_number || data.target_id != kInvalidPropertyNodeId);
    ...
    if (transform_id == dest_id) {
        target_space_transform.Scale(effect_node->surface_contents_scale.x(), effect_node->surface_contents_scale.y());
        data.transforms.to_valid = true;
        data.transforms.from_valid = false;
    } else if (transform_id > dest_id) {
        transform_tree().CombineTransformsBetween(transform_id, dest_id, &target_space_transform);
        target_space_transform.PostScale(effect_node->surface_contents_scale.x(), effect_node->surface_contents_scale.y());
        data.transforms.to_valid = true;
        data.transforms.from_valid = false;
        data.transforms.might_be_invertible = true;
    } else {
        gfx::Transform combined_transform;
        transform_tree().CombineTransformsBetween(dest_id, transform_id, &combined_transform);
        if (effect_node->surface_contents_scale.x() != 0.f && effect_node->surface_contents_scale.y() != 0.f)
            combined_transform.Scale(1.0f / effect_node->surface_contents_scale.x(), 1.0f / effect_node->surface_contents_scale.y());
...
void PropertyTrees::ResetCachedData()
{
    ...
    cached_data_.draw_transforms.resize(transform_count, std::vector<DrawTransformData>(1));
...
gfx::Transform PropertyTrees::ToScreenSpaceTransformWithoutSurfaceContentsScale(int transform_id, int effect_id) const
{
    ...
    if (effect_node->surface_contents_scale.x() != 0.0 && effect_node->surface_contents_scale.y() != 0.0)
        screen_space_transform.Scale(1.0 / effect_node->surface_contents_scale.x(), 1.0 / effect_node->surface_contents_scale.y());
