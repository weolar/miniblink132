...
class ImageAnimationControllerTest : public testing::Test, public ImageAnimationController::Client {
public:
    void SetUp() override
    {
        task_runner_ = new DelayTrackingTaskRunner(base::SingleThreadTaskRunner::GetCurrentDefault().get());
        controller_ = std::make_unique<ImageAnimationController>(task_runner_.get(), this, GetEnableImageAnimationResync());
        controller_->set_now_callback_for_testing(base::BindRepeating(&ImageAnimationControllerTest::Now, base::Unretained(this)));
    ...
    void LoopOnceNoDelay(PaintImage::Id paint_image_id, const std::vector<FrameMetadata>& frames, size_t num_of_frames_to_loop, int repetitions_completed,
        std::vector<base::TimeDelta> expected_delays = {}, bool restarting = false)
    {
        ...
        for (size_t i = 0; i < num_of_frames_to_loop; ++i) {
            ...
            // the animation.
            EXPECT_EQ(controller_->GetLastNumOfFramesSkippedForTesting(paint_image_id), 0u);

            EXPECT_EQ(controller_->GetFrameIndexForImage(paint_image_id, WhichTree::PENDING_TREE), i);
            if (i == 0u && !restarting) {
                ...
                EXPECT_EQ(controller_->GetFrameIndexForImage(paint_image_id, WhichTree::ACTIVE_TREE), active_index);
            } else if (i != 0u) {
                EXPECT_EQ(controller_->GetFrameIndexForImage(paint_image_id, WhichTree::ACTIVE_TREE), i - 1);
            }
...
protected:
    ...
    void AdvanceNow(base::TimeDelta delta)
    {
        now_ += delta;
    }
    viz::BeginFrameArgs BeginFrameArgs(base::TimeTicks now = base::TimeTicks())
    {
        ...
        return viz::BeginFrameArgs::Create(BEGINFRAME_FROM_HERE, 1 /* source_id */, 1 /* sequence_number */, now /* frame_time */,
            now + interval_ /* deadline */, interval_ /* interval */, viz::BeginFrameArgs::BeginFrameArgsType::NORMAL);
    }
    ...
    virtual bool GetEnableImageAnimationResync() const
    {
        return true;
    }
...
{
    std::vector<FrameMetadata> frames = { FrameMetadata(true, base::Milliseconds(5)), FrameMetadata(true, base::Milliseconds(3)),
        FrameMetadata(true, base::Milliseconds(4)), FrameMetadata(true, base::Milliseconds(3)) };

    DiscardableImageMap::AnimatedImageMetadata data(PaintImage::GetNextId(), PaintImage::CompletionState::kDone, frames, kAnimationLoopInfinite, 0);
    ...
    EXPECT_EQ(controller_->GetLastNumOfFramesSkippedForTesting(data.paint_image_id), 1u);

    // The pending tree displays the second frame while the active tree has the
    // last frame.
    EXPECT_EQ(controller_->GetFrameIndexForImage(data.paint_image_id, WhichTree::PENDING_TREE), 1u);
    EXPECT_EQ(controller_->GetFrameIndexForImage(data.paint_image_id, WhichTree::ACTIVE_TREE), 3u);
    ...
    controller_->DidActivate();
    EXPECT_EQ(controller_->GetFrameIndexForImage(data.paint_image_id, WhichTree::ACTIVE_TREE), 1u);
    AdvanceNow(data.frames[1].duration + data.frames[2].duration + data.frames[3].duration);
    RunFrameRequestAndInvalidation();
    animated_images = controller_->AnimateForSyncTree(BeginFrameArgs());
    EXPECT_EQ(animated_images.size(), 1u);
    EXPECT_EQ(animated_images.count(data.paint_image_id), 1u);
    EXPECT_EQ(controller_->GetLastNumOfFramesSkippedForTesting(data.paint_image_id), 2u);

    // The pending tree displays the first frame, while the active tree has the
    // second frame.
    EXPECT_EQ(controller_->GetFrameIndexForImage(data.paint_image_id, WhichTree::PENDING_TREE), 0u);
    EXPECT_EQ(controller_->GetFrameIndexForImage(data.paint_image_id, WhichTree::ACTIVE_TREE), 1u);
...
{
    std::vector<FrameMetadata> first_image_frames = { FrameMetadata(true, base::Milliseconds(2)), FrameMetadata(true, base::Milliseconds(3)) };
    DiscardableImageMap::AnimatedImageMetadata first_data(
        PaintImage::GetNextId(), PaintImage::CompletionState::kDone, first_image_frames, kAnimationLoopOnce, 0);
    controller_->UpdateAnimatedImage(first_data);
    FakeAnimationDriver first_driver;
    controller_->RegisterAnimationDriver(first_data.paint_image_id, &first_driver);

    std::vector<FrameMetadata> second_image_frames = { FrameMetadata(true, base::Milliseconds(5)), FrameMetadata(true, base::Milliseconds(3)) };
    DiscardableImageMap::AnimatedImageMetadata second_data(
        PaintImage::GetNextId(), PaintImage::CompletionState::kDone, second_image_frames, kAnimationLoopOnce, 0);
    controller_->UpdateAnimatedImage(second_data);
    FakeAnimationDriver second_driver;
    controller_->RegisterAnimationDriver(second_data.paint_image_id, &second_driver);
    ...
    controller_->UnregisterAnimationDriver(first_data.paint_image_id, &first_driver);
    controller_->UnregisterAnimationDriver(second_data.paint_image_id, &second_driver);
}

TEST_F(ImageAnimationControllerTest, RepetitionsRequested)
{
    std::vector<FrameMetadata> frames
        = { FrameMetadata(true, base::Milliseconds(2)), FrameMetadata(true, base::Milliseconds(3)), FrameMetadata(true, base::Milliseconds(4)) };

    DiscardableImageMap::AnimatedImageMetadata data(PaintImage::GetNextId(), PaintImage::CompletionState::kDone, frames, kAnimationLoopOnce, 0);
    controller_->UpdateAnimatedImage(data);
    FakeAnimationDriver driver;
    controller_->RegisterAnimationDriver(data.paint_image_id, &driver);
    controller_->UpdateStateFromDrivers();
...
{
    std::vector<FrameMetadata> frames
        = { FrameMetadata(true, base::Milliseconds(2)), FrameMetadata(true, base::Milliseconds(3)), FrameMetadata(false, base::Milliseconds(4)) };

    DiscardableImageMap::AnimatedImageMetadata data(PaintImage::GetNextId(), PaintImage::CompletionState::kPartiallyDone, frames, kAnimationLoopInfinite, 0);
    controller_->UpdateAnimatedImage(data);
    FakeAnimationDriver driver;
    controller_->RegisterAnimationDriver(data.paint_image_id, &driver);
    controller_->UpdateStateFromDrivers();
...
TEST_F(ImageAnimationControllerTest, DontLoopPartiallyLoadedImages)
{
    ...
    DiscardableImageMap::AnimatedImageMetadata data(PaintImage::GetNextId(), PaintImage::CompletionState::kPartiallyDone, frames, 2, 0);
    controller_->UpdateAnimatedImage(data);
    FakeAnimationDriver driver;
    controller_->RegisterAnimationDriver(data.paint_image_id, &driver);
    controller_->UpdateStateFromDrivers();
    ...
    RunFrameRequestAndInvalidation();
    auto animated_images = controller_->AnimateForSyncTree(BeginFrameArgs());
    EXPECT_EQ(controller_->GetFrameIndexForImage(data.paint_image_id, WhichTree::PENDING_TREE), 2u);
    EXPECT_EQ(animated_images.size(), 1u);
    EXPECT_EQ(animated_images.count(data.paint_image_id), 1u);
    controller_->DidActivate();
...
TEST_F(ImageAnimationControllerTest, DontAdvanceUntilDesiredTime)
{
    ...
    auto animated_images = controller_->AnimateForSyncTree(BeginFrameArgs());
    EXPECT_EQ(controller_->GetFrameIndexForImage(data.paint_image_id, WhichTree::PENDING_TREE), 0u);
    EXPECT_EQ(controller_->GetFrameIndexForImage(data.paint_image_id, WhichTree::ACTIVE_TREE), 0u);
    EXPECT_EQ(animated_images.size(), 0u);
    controller_->DidActivate();
...
{
    std::vector<FrameMetadata> frames = { FrameMetadata(true, base::Milliseconds(2)), FrameMetadata(true, base::Milliseconds(3)),
        FrameMetadata(true, base::Milliseconds(4)), FrameMetadata(true, base::Milliseconds(5)) };
    ...
    EXPECT_EQ(animated_images.count(data.paint_image_id), 1u);
    EXPECT_EQ(controller_->GetFrameIndexForImage(data.paint_image_id, WhichTree::PENDING_TREE), 0u);
    EXPECT_EQ(controller_->GetFrameIndexForImage(data.paint_image_id, WhichTree::ACTIVE_TREE), 2u);
    controller_->UnregisterAnimationDriver(data.paint_image_id, &driver);
}
...
TEST_F(ImageAnimationControllerTest, FinishRepetitionsDuringCatchUp)
{
    ...
    DiscardableImageMap::AnimatedImageMetadata data(PaintImage::GetNextId(), PaintImage::CompletionState::kDone, frames, 3, 0);
    controller_->UpdateAnimatedImage(data);
    FakeAnimationDriver driver;
    controller_->RegisterAnimationDriver(data.paint_image_id, &driver);
    controller_->UpdateStateFromDrivers();
    ...
    EXPECT_EQ(animated_images.size(), 0u);
    EXPECT_EQ(controller_->GetFrameIndexForImage(data.paint_image_id, WhichTree::PENDING_TREE), frames.size() - 1);
    EXPECT_EQ(controller_->GetFrameIndexForImage(data.paint_image_id, WhichTree::ACTIVE_TREE), frames.size() - 1);
...
TEST_F(ImageAnimationControllerTest, ResetAnimations)
{
    std::vector<FrameMetadata> frames
        = { FrameMetadata(true, base::Milliseconds(2)), FrameMetadata(true, base::Milliseconds(3)), FrameMetadata(true, base::Milliseconds(4)) };
    DiscardableImageMap::AnimatedImageMetadata data(PaintImage::GetNextId(), PaintImage::CompletionState::kDone, frames, 3, 0u);
    controller_->UpdateAnimatedImage(data);
    FakeAnimationDriver driver;
    controller_->RegisterAnimationDriver(data.paint_image_id, &driver);
    controller_->UpdateStateFromDrivers();
...
TEST_F(ImageAnimationControllerTest, ImageWithNonVsyncAlignedDurations)
{
    ...
    std::vector<FrameMetadata> frames
        = { FrameMetadata(true, base::Milliseconds(2.5)), FrameMetadata(true, base::Milliseconds(3.76)), FrameMetadata(true, base::Milliseconds(4.27)) };
    DiscardableImageMap::AnimatedImageMetadata data(PaintImage::GetNextId(), PaintImage::CompletionState::kDone, frames, 3, 0u);
    controller_->UpdateAnimatedImage(data);
    FakeAnimationDriver driver;
    controller_->RegisterAnimationDriver(data.paint_image_id, &driver);
    ...
    std::vector<base::TimeDelta> expected_delays = { base::Milliseconds(2), base::Milliseconds(4), base::Milliseconds(4) };
    LoopOnceNoDelay(data.paint_image_id, frames, frames.size(), 0, expected_delays);
...
TEST_F(ImageAnimationControllerTest, ImageWithLessThanIntervalDurations)
{
    ...
    DiscardableImageMap::AnimatedImageMetadata data(PaintImage::GetNextId(), PaintImage::CompletionState::kDone, frames, kAnimationLoopOnce, 0u);
    controller_->UpdateAnimatedImage(data);
    FakeAnimationDriver driver;
    controller_->RegisterAnimationDriver(data.paint_image_id, &driver);
    controller_->UpdateStateFromDrivers();
...
class ImageAnimationControllerNoResyncTest : public ImageAnimationControllerTest {
protected:
    bool GetEnableImageAnimationResync() const override
    {
        return false;
    }
};
...
TEST_F(ImageAnimationControllerNoResyncTest, ComputeConsistentContentFrameDuration)
{
...

    {
        ...
        DiscardableImageMap::AnimatedImageMetadata data(id1, PaintImage::CompletionState::kPartiallyDone, frames, kAnimationLoopInfinite, 0);
        controller_->UpdateAnimatedImage(data);
    ...
    {
        std::vector<FrameMetadata> frames = { FrameMetadata(true, base::Milliseconds(3)), FrameMetadata(true, base::Milliseconds(3)),
            FrameMetadata(true, base::Milliseconds(3)), FrameMetadata(true, base::Milliseconds(3)) };
        DiscardableImageMap::AnimatedImageMetadata data(id1, PaintImage::CompletionState::kPartiallyDone, frames, kAnimationLoopInfinite, 0);
        controller_->UpdateAnimatedImage(data);

        controller_->UpdateStateFromDrivers();

        std::optional<ImageAnimationController::ConsistentFrameDuration> consistent_duration = controller_->GetConsistentContentFrameDuration();
    ...
    {
        std::vector<FrameMetadata> frames = { FrameMetadata(true, base::Milliseconds(4)), FrameMetadata(true, base::Milliseconds(4)),
            FrameMetadata(true, base::Milliseconds(4)), FrameMetadata(true, base::Milliseconds(4)) };
        DiscardableImageMap::AnimatedImageMetadata data(id2, PaintImage::CompletionState::kPartiallyDone, frames, kAnimationLoopInfinite, 0);
        controller_->UpdateAnimatedImage(data);
