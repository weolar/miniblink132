...
TEST_F(ThrottleDeciderTest, BackdropFilter)
{
    ...
    render_passes[0]->SetNew(id1, render_pass_rect, gfx::Rect(), gfx::Transform());
    render_passes[0]->backdrop_filters.Append(FilterOperation::CreateBlurFilter(5.0));
    render_passes[1]->SetNew(id2, render_pass_rect, gfx::Rect(), gfx::Transform());

    auto* rpdq = render_passes[1]->CreateAndAppendDrawQuad<viz::CompositorRenderPassDrawQuad>();
    ...
    auto* surface_quad = render_passes[1]->CreateAndAppendDrawQuad<viz::SurfaceDrawQuad>();
    viz::SharedQuadState sqs2;
    surface_quad->shared_quad_state = &sqs2;
    surface_quad->material = viz::DrawQuad::Material::kSurfaceContent;
    surface_quad->surface_range = viz::SurfaceRange(std::nullopt, viz::SurfaceId(frame_sink_id, viz::LocalSurfaceId(1u, base::UnguessableToken::Create())));
    ...
    render_passes[0]->backdrop_filter_bounds = std::optional<gfx::RRectF>(gfx::RRectF(0.0f, 10.0f, 50.0f, 50.0f, 1.0f));
    ...
    rpdq->resources.ids[viz::RenderPassDrawQuadInternal::kMaskResourceIdIndex] = viz::ResourceId::FromUnsafeValue(1u);
