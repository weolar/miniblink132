...
public:
    ...
    static std::unique_ptr<LayerTreeHost> CreateThreaded(scoped_refptr<base::SingleThreadTaskRunner> impl_task_runner, InitParams params);
    ...
    // required.
    static std::unique_ptr<LayerTreeHost> CreateSingleThreaded(LayerTreeHostSingleThreadClient* single_thread_client, InitParams params);
    ...
    // LayerTreeFrameSink.
    void SetLayerTreeFrameSink(std::unique_ptr<LayerTreeFrameSink> layer_tree_frame_sink);
    ...
    virtual void SetTargetLocalSurfaceId(const viz::LocalSurfaceId& target_local_surface_id);
    ...
    bool StartDeferringCommits(base::TimeDelta timeout, PaintHoldingReason reason);
    ...
    void OnDeferCommitsChanged(bool defer_status, PaintHoldingReason reason, std::optional<PaintHoldingCommitTrigger> trigger);
    ...
    void CompositeForTest(base::TimeTicks frame_begin_time, bool raster, base::OnceClosure callback);
    ...
    int ScheduleMicroBenchmark(const std::string& benchmark_name, base::Value::Dict settings, MicroBenchmark::DoneCallback callback);
    ...
    void RequestPresentationTimeForNextFrame(PresentationTimeCallbackBuffer::Callback callback);

    // Registers a callback that is run when the next frame successfully makes it
    // to the screen (it's entirely possible some frames may be dropped between
    // the time this is called and the callback is run).
    void RequestSuccessfulPresentationTimeForNextFrame(PresentationTimeCallbackBuffer::SuccessfulCallbackWithDetails callback);
    ...
    void SetEventListenerProperties(EventListenerClass event_class, EventListenerProperties event_properties);
    EventListenerProperties event_listener_properties(EventListenerClass event_class) const
    {
        DCHECK(IsMainThread());
        return pending_commit_state()->event_listener_properties[static_cast<size_t>(event_class)];
    }

    void SetViewportRectAndScale(const gfx::Rect& device_viewport_rect, float device_scale_factor, const viz::LocalSurfaceId& local_surface_id_from_parent);
    ...
    // coordinate space.
    void SetVisualDeviceViewportIntersectionRect(const gfx::Rect& intersection_rect);
    ...
    void SetPageScaleFactorAndLimits(float page_scale_factor, float min_page_scale_factor, float max_page_scale_factor);
    ...
    void StartPageScaleAnimation(const gfx::Point& target_offset, bool use_anchor, float scale, base::TimeDelta duration);
    ...
    void SetLocalSurfaceIdFromParent(const viz::LocalSurfaceId& local_surface_id_from_parent);
    ...
    void RequestViewportScreenshot(const base::UnguessableToken& destination_token);

    // Request to set `primary_main_frame_item_sequence_number` on the next
    // frame's metadata.
    void SetPrimaryMainFrameItemSequenceNumber(int64_t primary_main_frame_item_sequence_number);
    ...
    void SetDisplayColorSpaces(const gfx::DisplayColorSpaces& display_color_spaces);
    ...
    // calculations.
    void SetExternalPageScaleFactor(float page_scale_factor, bool is_external_pinch_gesture_active);
    ...
    std::unique_ptr<CommitState> WillCommit(std::unique_ptr<CompletionEvent> completion, bool has_updates);
    ...
    std::unique_ptr<LayerTreeHostImpl> CreateLayerTreeHostImpl(LayerTreeHostImplClient* client);
    ...
    bool UpdateLayers();
    void DidPresentCompositorFrame(uint32_t frame_token, std::vector<PresentationTimeCallbackBuffer::Callback> presentation_callbacks,
    ...
    void RecordEndOfFrameMetrics(base::TimeTicks frame_begin_time, ActiveFrameSequenceTrackers trackers);
    void NotifyThroughputTrackerResults(CustomTrackerResults results);
    void NotifyImageDecodeFinished(int request_id, bool decode_succeeded);
    void NotifyTransitionRequestsFinished(const uint32_t sequence_id, const viz::ViewTransitionElementResourceRects& rects);
    ...
    void QueueImageDecode(const PaintImage& image, base::OnceCallback<void(bool)> callback);
    ...
    void SetDelegatedInkMetadata(std::unique_ptr<gfx::DelegatedInkMetadata> metadata);
    gfx::DelegatedInkMetadata* DelegatedInkMetadataForTesting()
    {
        return pending_commit_state()->delegated_ink_metadata.get();
    }

    void DidObserveFirstScrollDelay(int source_frame_number, base::TimeDelta first_scroll_delay, base::TimeTicks first_scroll_timestamp);

    void AddViewTransitionRequest(std::unique_ptr<ViewTransitionRequest> request);

    std::vector<ViewTransitionRequest::ViewTransitionCaptureCallback> TakeViewTransitionCallbacksForTesting();
    ...
    // web_tests.
    bool in_composite_for_test() const
    {
        return in_composite_for_test_;
    }
...
protected:
    ...
    void InitializeThreaded(scoped_refptr<base::SingleThreadTaskRunner> main_task_runner, scoped_refptr<base::SingleThreadTaskRunner> impl_task_runner);
    void InitializeSingleThreaded(LayerTreeHostSingleThreadClient* single_thread_client, scoped_refptr<base::SingleThreadTaskRunner> main_task_runner);
    void InitializeForTesting(std::unique_ptr<TaskRunnerProvider> task_runner_provider, std::unique_ptr<Proxy> proxy_for_testing);
    void SetTaskRunnerProviderForTesting(std::unique_ptr<TaskRunnerProvider> task_runner_provider);
    void SetUIResourceManagerForTesting(std::unique_ptr<UIResourceManager> ui_resource_manager);
...
private:
    ...
    virtual std::unique_ptr<LayerTreeHostImpl> CreateLayerTreeHostImplInternal(LayerTreeHostImplClient* client, MutatorHost* mutator_host,
    ...
    void UpdateScrollOffsetFromImpl(const ElementId&, const gfx::Vector2dF& delta, const std::optional<TargetSnapAreaElementIds>&);
    ...
    std::unique_ptr<RenderingStatsInstrumentation> rendering_stats_instrumentation_;
    ...
    std::unordered_map<ElementId, raw_ptr<Layer, CtnExperimental>, ElementIdHash> element_layers_map_;
    ...
    base::flat_map<uint32_t, ViewTransitionRequest::ViewTransitionCaptureCallback> view_transition_callbacks_;
